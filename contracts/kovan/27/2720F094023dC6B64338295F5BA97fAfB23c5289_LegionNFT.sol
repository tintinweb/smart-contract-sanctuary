...
contract LegionNFT is Context, IERC721, Ownable {
    ...
    function mint(string memory name, uint256[] memory beastIds, uint256[] memory warriorIds) external {
        ...
        tokenData[_tokenIds.current()] = Legion(name, "QmTyQ3Mab2711GWGMyvXqMVCQpK49WH1KQRzhUBJS96Q7W", beastIds, warriorIds, 0, attack_power, block.timestamp);
        _tokenIds.increment();
    }

    function getLegion(uint256 tokenId) external view 
        returns(string memory, string memory, uint256[] memory, uint256[] memory, uint256, uint256, uint256) {
        return (
