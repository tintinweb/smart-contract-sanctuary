...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, IStrategy _strategy, bool _withUpdate) public onlyOwner nonDuplicated(_lpToken) {
    ...
    function valueOfStakeForUser(
        ...
        UserInfo storage user = userInfo[_pid][_user];
        return user.amount.mul(getPricePerFullShare(_pid));
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        uint256 preRecordedSharePrice = getPricePerFullShare(_pid);
        if (_amount > 0) {
            ...
            if(amountToWithdraw > pool.lpToken.balanceOf(address(this))) {
                ...
                amountToWithdraw = Math.min(underlyingBalanceWithInvestment(_pid)
                    .mul(_amount)
        ...
        user.rewardDebt = user.amount.mul(pool.accCrabPerShare).div(1e12);
        emit Withdraw(msg.sender, _pid, _amount.mul(preRecordedSharePrice));
    }
    ...
    /// @notice Gets the price per share of the specified pool.
    /// @param _pid Pool to fetch the share price of.
    function getPricePerFullShare(uint256 _pid) public view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        uint256 unit = 10 ** uint256(ERC20(address(pool.lpToken)).decimals());
        return pool.totalSharesSupply == 0
            ? unit
            : unit.mul(underlyingBalanceWithInvestment(_pid)).div(pool.totalSharesSupply);
    }
}