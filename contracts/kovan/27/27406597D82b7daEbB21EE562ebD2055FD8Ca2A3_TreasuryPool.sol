...
contract TreasuryPool is Pausable, ReentrancyGuard, ITreasuryPool {
    ...
    event DebugMsgSenderRepayFail(
        address indexed account,
        uint256 underlyingAssetBalanceOfMsgSender,
        uint256 principal,
        uint256 interest,
        uint256 totalAmount,
        uint256 timestamp
        ...
        onlyBy(farmingPoolAccount)
    {
        ...
        uint256 underlyingAssetBalanceOfMsgSender =
            _underlyingAsset.balanceOf(msg.sender);
        if (underlyingAssetBalanceOfMsgSender < totalAmount) {
            emit DebugMsgSenderRepayFail(
                msg.sender,
                underlyingAssetBalanceOfMsgSender,
                principal,
                interest,
                totalAmount,
                block.timestamp
