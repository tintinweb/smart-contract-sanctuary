...
contract DPRBridge {
        ...
        event ConfirmWithdrawMessage(bytes32 ticket, uint256 amount);
        ...
        mapping(bytes32 => bool) public withdrawed_ticket;
       ...
       */
        ...
        */
        ...
        modifier withdrawMessage(bytes32 messageID) {
            require(messages[messageID].status ==  Status.PENDING, "Message is not withdrawed");
            _;
        }
        ...
        */
        ...
        function confirmWithdrawTransfer(uint256 id, uint256 amount, address to_addr, bytes memory signature) public 
        updateUserWithdrawInfo(to_addr, amount)
        updateContractWithdrawInfo(amount)
        {
            bytes32 ticket = keccak256(abi.encodePacked(id,amount, to_addr));
            require(!withdrawed_ticket[ticket], "Already withdraw");
            withdrawed_ticket[ticket] = true;
            bytes32 sign_data = keccak256(abi.encodePacked(SIGN_HASH_PREFIX, ticket));
            ...
            token.safeTransfer(to_addr, withdraw_amount);
            emit ConfirmWithdrawMessage(ticket, withdraw_amount);  
        }
        ...
        function cancelWithdraw(bytes32 ticket) onlyOwner external {
            withdrawed_ticket[ticket] = true;
        }
}