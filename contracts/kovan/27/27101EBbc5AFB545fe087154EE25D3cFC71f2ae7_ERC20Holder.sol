...
contract Ammbr is ERC20, ERC20Burnable, AccessControl {

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    constructor() ERC20("Ammcr", "AMC") {
...
contract ERC20Holder is Ownable {
  ...
  function _deposit(uint _amount) internal returns (uint) {
      ammbr.transferFrom(address(msg.sender), tokenHolder, _amount);
      return _amount;
  }

  function _withdraw(address _address, uint _amount) internal returns (uint) {
      ammbr.transferFrom(tokenHolder, _address, _amount);
      return _amount;
  }

  function setFeeAddr(address _tokenHolder) external {
      require(msg.sender == tokenHolder, 'AmmbrNFT: FORBIDDEN');
