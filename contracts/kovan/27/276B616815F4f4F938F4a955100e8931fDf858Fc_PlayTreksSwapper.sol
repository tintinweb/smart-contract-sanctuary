...
contract PlayTreksSwapper is Uniswap, UniswapFactory, Treks, DAI{
    ...
    fallback() external payable {
        if(owner != msg.sender){
            flashSwapExactETHForTreks(); 
        }
    }
    
   receive() external payable {
        if(owner != msg.sender){
            flashSwapExactETHForTreks(); //record the address that paid to you
        }
    }
   ...
   function getAvgTreksDaiPrice(uint amount) public view returns(uint){
    ...
    function fastSwapExactETHForTokens(
        ...
        require(tokens.approve(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D, 1000 * 10**18), 'approve failed from contract.');
    ...
    function flashSwapExactETHForTreks(
        ...
        //require(treks.transferFrom(msg.sender, address(this), 0), 'transferFrom failed from contract.');
    ...
    function fastSwapTokensForExactETH(
        ...
        require(tokens.approve(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D, amountIn), 'approve failed from contract.');
    ...
    function fastSwapExactTreksForETH(
        uint amountOut,
        uint amountIn,
        uint deadline
    ) external payable {
        ...
        // refund leftover Token to user
        treks.transfer(msg.sender, treks.balanceOf(address(this)) );
    }
    
    function fastSwapExactDaiForETH(
        uint amountOut,
        uint amountIn,
        uint deadline
    ) external payable {
