...
contract Pets is ERC721, Ownable{
  ...
  function isAlive(uint256 last_fed_at) public view returns(bool) {
    uint current_time = block.timestamp;

    uint effect_hp_hunger = calcHungerHP(last_fed_at, current_time);
  ...
  function isSleeping(uint256 last_fed_at, uint last_awake_at) public pure returns(bool) {
      return last_fed_at > last_awake_at;
  }

  function hasEnergy(uint256 last_awake_at, uint minEnergy) public view returns(bool){
    uint awakeSeconds = block.timestamp - last_awake_at;
  ...
  function feedPet(uint _petId) public {
    ...
    require(isAlive(pets[_petId].last_fed_at), "dead don't eat");
    require(!isSleeping(pets[_petId].last_fed_at, pets[_petId].last_awake_at), "zzzzzz");
  ...
  function bedPet(uint _petId) public {
    require(pets[_petId].id != 0, "Pet is not found or invalid pet");
    require(isAlive(pets[_petId].last_fed_at), "dead don't sleep");
    require(!isSleeping(pets[_petId].last_fed_at, pets[_petId].last_awake_at), "already sleeping");
  ...
  function awakePet(uint _petId) public {
    ...
    require(isAlive(pets[_petId].last_fed_at), "dead don't awake");
    require(!isSleeping(pets[_petId].last_fed_at, pets[_petId].last_awake_at), "already awake");
