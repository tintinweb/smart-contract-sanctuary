...
contract SmartTreasury is ERC20PresetMinterPauserUpgradeableDecimals, ERC20PermitUpgradeableDecimals {
    ...
    ///////////////////////////////////////////////////////////////////////////
    // SUBSIDIZED EXECUTION VARIABLES
    ///////////////////////////////////////////////////////////////////////////
    address public executionProxy; // has ability to call bonus spending/burning
    ...
    // NOTE !!!!!!!!!!!!!!!!!!!!!!!!!!!
    // BONUS TOKENS SHOULD CONTAIN SAME DECIMALS AS BASE ASSET (USDC=6)
    // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    function setEndowmentTreshold(uint256 _endowmentPercent) public {
    ...
    function pendingBonus(address _account) public view returns(uint256) {
        ...
        uint256 pendingBonusMove = userMove.amount.mul(accBonusPerShareMove).div(1e12).sub(userMove.rewardTally);
        return pendingBonusMove.add(userMoveEthLP.amount.mul(accBonusPerShareMoveEthLP).div(1e12).sub(userMoveEthLP.rewardTally));
    }
    ...
    function depositInternal(address _account, uint _tokenMoveAmount, uint _tokenMoveEthAmount, bool _skipTransfer) internal {
        ...
        if (userMove.amount > 0 || userMoveEthLP.amount > 0) {
            uint256 pending = userMove.amount.mul(accBonusPerShareMove).div(1e12).sub(userMove.rewardTally);
            pending = pending.add(userMoveEthLP.amount.mul(accBonusPerShareMoveEthLP).div(1e12).sub(userMoveEthLP.rewardTally));
            if(pending > 0) {
        ...
        userMove.rewardTally = userMove.amount.mul(accBonusPerShareMove).div(1e12);
        userMoveEthLP.rewardTally = userMoveEthLP.amount.mul(accBonusPerShareMoveEthLP).div(1e12);
    ...
    // called when profit distribution occurs
    function receiveProfit(uint256 _amount) public {
        ...
        if (totalStakedMove > 0) {
            accBonusPerShareMove = accBonusPerShareMove.add(bonusPortionMove.mul(1e12).div(totalStakedMove));
        }
        if (totalStakedMoveEthLP > 0) {
            accBonusPerShareMoveEthLP = accBonusPerShareMoveEthLP.add(bonusPortionMoveEthLP.mul(1e12).div(totalStakedMoveEthLP));
        }
    ...
    function spendBonusInternal(address _account, uint256 _amount) internal {
        ...
        UserInfo storage userMoveEthLP = userInfoMoveEthLP[msg.sender];
        uint256 pendingBonusMove = userMove.amount.mul(accBonusPerShareMove).div(1e12).sub(userMove.rewardTally);
        uint256 pendingBonusMoveEthLP = userMoveEthLP.amount.mul(accBonusPerShareMoveEthLP).div(1e12).sub(userMoveEthLP.rewardTally);
        ...
        if (pendingBonusMoveEthLP >= _amount) {
            ...
            userMove.rewardTally = userMove.rewardTally.add(_amount);
            ...
            userMove.rewardTally = userMove.amount.mul(accBonusPerShareMove).div(1e12);
            userMoveEthLP.rewardTally = userMoveEthLP.rewardTally.add(_amount.sub(pendingBonusMoveEthLP));
        } else {
    ...
    // subsidized deposit not requiring allowance of MOVE or MOVE-ETH LP for this contract
    ...
    function maxBurnAmount() public view returns(uint256) {
        uint256 totalSupply = IERC20Upgradeable(baseToken).totalSupply();
        return totalSupply.mul(1000000000000000000).div(burnLimit);
    }
    ...
    function claimAndBurnOnBehalf(address _beneficiary, uint256 _amount) public {
        ...
        IERC20Upgradeable(baseToken).safeTransfer(_beneficiary, endowmentPortion.add(bonusPortion));
        emit ClaimAndBurn(_beneficiary, _amount, endowmentPortion.add(bonusPortion));
    }
    ...
    function getDailyAPYPerMoveToken() public view returns(uint256) {
      ...
      uint256 totalMoveStakedEquivalent = totalStakedMove.add(totalStakedMoveEthLP.mul(moveInLP).div(totalLP));

      uint256 baseAssetPerDay = endowmentBalance.mul(100).mul(86400).div(secondsFromInception);
