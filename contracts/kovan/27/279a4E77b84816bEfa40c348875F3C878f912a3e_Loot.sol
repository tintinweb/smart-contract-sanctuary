...
// Part: OpenZeppelin/openzepp[email protected]/IERC721
...
contract Loot is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    string[] private gender = ["Male", "Female", "Non", "Bisexual"];
    ...
    function getFamilyName(uint256 tokenId)
        public
        view
        returns (string memory)
    {
        return pluck(tokenId, "FAMILYNAME", familyName);
    }
        ...
        returns (string memory)
    {
        ...
        parts[1] = string(
            abi.encodePacked(getFirstName(tokenId), " ", getFamilyName(tokenId))
        );
        ...
        parts[7] = string(abi.encodePacked("STR", " ", getSTR(tokenId)));

        parts[8] = '</text><text x="10" y="100" class="base">';

        parts[9] = string(abi.encodePacked("DEX", " ", getDEX(tokenId)));

        parts[10] = '</text><text x="10" y="120" class="base">';

        parts[11] = string(abi.encodePacked("CON", " ", getCON(tokenId)));

        parts[12] = '</text><text x="10" y="140" class="base">';

        parts[13] = string(abi.encodePacked("INT", " ", getINT(tokenId)));

        parts[14] = '</text><text x="10" y="160" class="base">';

        parts[15] = string(abi.encodePacked("WIS", " ", getWIS(tokenId)));

        parts[16] = '</text><text x="10" y="180" class="base">';

        parts[17] = string(abi.encodePacked("CHA", " ", getCHA(tokenId)));
