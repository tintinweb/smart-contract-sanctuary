...
contract DOPEE is Context, IERC20, Ownable {
    ...
    string private _name = "Dopameme";
    string private _symbol = "DOPEE";
    ...
    uint256 public _dopeFee = 2;
    uint256 private _previousDopeFee = _dopeFee;
    ...
    function setDopeAddress(address payable charity) public onlyOwner {
        _dopeAddress = charity;
    }
    ...
    function showDopeaddress() public view returns(address payable) {
        return _dopeAddress;
    }
    ...
    function setDopeFeePercent(uint256 dopeFee) external onlyOwner {
        _dopeFee = 0;
        if(dopeFee <= 5) {
	        _dopeFee = dopeFee;
	    }  
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateLiquidityPlusDopeFee(tAmount);
    ...
    function calculateLiquidityPlusDopeFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _dopeFee + _marketingAndDevBudget).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingAndDevBudget == 0 && _dopeFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousLiquidityFee = _liquidityFee;
        _previousDopeFee = _dopeFee;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalLiqFee = _marketingAndDevBudget + _liquidityFee + _dopeFee;
        ...
        uint256 charityBalance = newBalance.div(totalLiqFee).mul(_dopeFee);
        uint256 charityPortion = otherHalf.div(totalLiqFee).mul(_dopeFee);
        ...
        (bool sent, bytes memory data) = _dopeAddress.call{value: charityBalance}("");
        if(sent){
            ...
            addLiquidity(charityPortion, charityBalance, _dopeAddress);
        }
