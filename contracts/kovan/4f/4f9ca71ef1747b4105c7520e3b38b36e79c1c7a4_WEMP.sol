...
contract WEMP is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    mapping (address => bool) private setForNoFee;
    ...
    function tokenDeflation() external onlyOwner returns(bool){
        require(!_deflationStatus,"Deflation has already been performed");
        uint256 deflationAmount = _tTotal.mul(40).div(100);
        _burn(msg.sender, deflationAmount);
        _deflationStatus = true;
        return true;
    }
    ...
    function _getTValues(uint256 amount) private view returns (uint256, uint256, uint256, uint256) {
        uint256 tAmount = amount;
        uint256 tFee = calculateTaxFee(tAmount);
        uint256 tCharityFee = calculateCharityFee(tAmount);
        uint256 tBurn = 0;
        tBurn = calculateTransactionBurn(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tBurn).sub(tCharityFee);
        return (tTransferAmount, tFee, tBurn, tCharityFee);
    }

    function _getRValues(uint256 amount, uint256 tFee, uint256 tBurn, uint256 tCharityFee) private view returns (uint256, uint256, uint256) {
        uint256 currentRate = _getRate();
        uint256 tAmount = amount;
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rBurn).sub(rCharityFee);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _transactionBurn == 0 && _charityFee == 0) return;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tFee, uint256 tBurn, uint256 tCharityFee)  = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tBurn, tCharityFee);
