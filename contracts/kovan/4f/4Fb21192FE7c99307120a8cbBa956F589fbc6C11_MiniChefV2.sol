...
/// @notice The (older) MasterChef contract gives out a constant number of MAD tokens per block.
/// It is the only address with minting rights for MAD.
...
contract MiniChefV2 is BoringOwnable, BoringBatchable {
    ...
    /// `rewardDebt` The amount of MAD entitled to the user.
    struct UserInfo {
        uint256 amount;
        int256 rewardDebt;
    }
    ...
    /// Also known as the amount of MAD to distribute per block.
    struct PoolInfo {
    ...
    /// @notice Address of MAD contract.
    IERC20 public immutable MAD;
    ...
    /// @param _sushi The MAD token contract address.
    ...
    /// @notice Update the given pool's MAD allocation point and `IRewarder` contract. Can only be called by the owner.
    ...
    /// @notice View function to see pending MAD on frontend.
    /// @param _pid The index of the pool. See `poolInfo`.
    /// @param _user Address of user.
    /// @return pending MAD reward for a given user.
    ...
    /// @notice Deposit LP tokens to MCV2 for MAD allocation.
    /// @param pid The index of the pool. See `poolInfo`.
    ...
    /// @param pid The index of the pool. See `poolInfo`.
    /// @param to Receiver of MAD rewards.
    function harvest(uint256 pid, address to) public {
        ...
        // Interactions
        if (_pendingSushi != 0) {
            MAD.safeTransfer(to, _pendingSushi);
        }
    ...
    /// @param pid The index of the pool. See `poolInfo`.
    /// @param amount LP token amount to withdraw.
    /// @param to Receiver of the LP tokens and MAD rewards.
...
contract MasterChefV2 is BoringOwnable, BoringBatchable {
    ...
    /// @notice Deposits a dummy token to `MASTER_CHEF` MCV1. This is required because MCV1 holds the minting rights for MAD.
    ...
    /// @notice Calculates and returns the `amount` of MAD per block.
    ...
    /// @notice Harvests MAD from `MASTER_CHEF` MCV1 and pool `MASTER_PID` to this MCV2 contract.
