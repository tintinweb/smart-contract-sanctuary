...
contract VaultCore is VaultStorage, Ownable {
	...
	constructor() public {
		...
		supportedCollat[0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48] = true;
	...
	function _mint(
		...
		uint SPABurnAmt = USDsAmt.mul(chi).div(chiPresion).div(priceSPA).div(precisionSPA);
		if (swapFee > 0) {
		...
		//Deposit collaeral
		uint CollaDepAmt = USDsAmt.mul(chiPresion - chi).div(chiPresion).div(priceColla).div(precisionColla);
		if (swapFee > 0) {
	...
	function _redeem(
		...
		uint SPAMintAmt = USDsAmt.mul(chi).div(chiPresion).div(priceSPA).div(precisionSPA);
		if (swapFee > 0) {
		...
		//Unlock collaeral
		uint CollaUnlockAmt = USDsAmt.mul(chiPresion - chi).div(chiPresion).div(priceColla).div(precisionColla);
		if (swapFee > 0) {
