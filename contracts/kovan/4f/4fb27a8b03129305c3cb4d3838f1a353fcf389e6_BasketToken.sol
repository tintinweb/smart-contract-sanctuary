...
// File: contracts/interfaces/IBasketToken.sol
...
/**
 * Interface for operating with BasketTokens.
 */
...
// File: contracts/protocol/BasketToken.sol
...
contract BasketToken is ERC20, ReentrancyGuard {
    ...
    nonReentrant
  {
    ...
    for (uint256 i = 0; i < components.length; i++) {
        ...
        //abi.encodeWithSignature("invokeApprove(address,address,address,uint256", address(_basketToken), IERC20(components[i]), _to, componentQuantities[i]);
        abi.encodeWithSignature("invokeTransfer(address,address,address,uint256", address(_basketToken), IERC20(components[i]), _to, componentQuantities[i]);
    }
    ...
    nonReentrant
  {
    ...
    _basketToken.burn(msg.sender, _quantity);
    ...
    returns (address[] memory, uint256[] memory)
  {
        ...
        internal
    {
        if (_quantity > 0) {
            ...
            BasketToken.invokeTransfer(_basketToken, _token, _to, _quantity);
