...
contract VestingRouter {
    ...
    constructor (address[] memory vestingAddresses, uint[] memory vestingAmount, address plugTokenAddress) {
    ...
    function allocateVestingFunds () public {
        for (uint i = lastAllocatedAddress; i < _vestingAddress.length; i++) {
            if (_plug.balanceOf(address(this)) < _vestingAmount[i] || gasleft() < 20000) {
                break;
            }
            lastAllocatedAddress++;
            _plug.transfer(_vestingAddress[i], _vestingAmount[i]);
        }
    }
