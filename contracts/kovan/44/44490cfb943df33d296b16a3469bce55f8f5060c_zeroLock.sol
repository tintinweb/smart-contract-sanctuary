...
//        uint hID;
    ...
    function getExpiry(uint time, uint duration) public view returns(uint256 ex, uint256 mined) {
        return (time+(duration*30 days), (now - time) + time);
    }
    
    function meanTokenPrice(string memory _tokenSymbol, uint time, uint expiry) public view returns(uint meanTokenPrice) {
        return meanPriceFinder(_tokenSymbol, time, expiry>now?now:expiry);
    }
    
    function wrap() public view returns(string memory wrapToken) {
        return Init.WRAP_ECO_SYMBOL();
    }
    
    function meanECOPrice(uint time, uint expiry) public view returns(uint meanECOPrice) {
        return meanPriceFinder(Init.WRAP_ECO_SYMBOL(), time, expiry>now?now:expiry);
    }
    
    function meanEarnRate(string memory _tokenSymbol, uint time, uint dID) public view returns (uint meanEarnRate) {
        return findLockRate(time, dID, _tokenSymbol);
    }
    
    function amount(string memory _tokenSymbol, uint _amount, uint meanTokenPrice, uint meanECOPrice) public view returns(uint amount) {
        return _amount*meanTokenPrice*(10**8)/(meanECOPrice*(10**uint(wERC20(EMM.getwTokenAddress(_tokenSymbol)).decimals())));
    }
    
    function reward(uint amount, uint meanEarnRate, uint expiry, uint time) public view returns(uint reward) {
        return (amount * meanEarnRate *(now>expiry ? expiry-time : now-time))/(3153600000);
    }
