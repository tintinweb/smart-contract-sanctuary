...
contract NFT is ERC721 
{
    ...
    constructor(string memory _name, string memory _symbol, string memory _title, string memory _desc, string memory _image, uint _limit_supply) public
	{
		name = _name;
		symbol = _symbol;
		title = _title;
	...
	{
		require(totalSupply + 1 <= limit_supply, "limit_supply of mints already reached");
        require(owner == msg.sender); // 소유자만이 token을 생성할 수 있습니다
        uint256 tokenId = totalSupply; // 신규 token ID
