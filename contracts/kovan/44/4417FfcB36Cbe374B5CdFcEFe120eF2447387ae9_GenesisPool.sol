...
contract PowerTokenWrapper {
	using SafeMath for uint256;
	IERC20 public power;

	constructor(IERC20 _powerAddress) public {
		power = IERC20(_powerAddress);
	}
...
contract GenesisPool is PowerTokenWrapper, Ownable {
	ERC1155Tradable public pcl;
	...
	constructor(ERC1155Tradable _pclAddress, IERC20 _powerAddress) public PowerTokenWrapper(_powerAddress) {
		pcl = _pclAddress;
	}
	...
	// stake visibility is public as overriding PowerTokenWrapper's stake() function
	function stake(uint256 amount) public updateReward(msg.sender) {
		require(amount.add(balanceOf(msg.sender)) <= 10000000000, "Cannot stake more than 100 PowerBNB LP");
	...
	function redeem(uint256 card) public updateReward(msg.sender) {
		...
		require(pcl.totalSupply(card) < pcl.maxSupply(card), "Max cards minted");

		points[msg.sender] = points[msg.sender].sub(cards[card]);
		pcl.mint(msg.sender, card, 1, "");
