...
contract FundMe {
    ...
    function fund() public payable {
        uint256 minimumUSD = 1694550374488076609; //50 * 10**18;
        require(
    ...
    function EthereumToUSD(uint256 ethAmount) public view returns (uint256) {
        return (getPrice() * ethAmount) / (10**18); // / (10 ** _maxDec); // parametrs , uint8 _maxDec - 18
    }

    //getEntraceFee
    // USD To Ethereum
    function USDToEthereum(uint256 _usdAmount) public view returns (uint256) {
        uint256 usdAmount = _usdAmount * 10**18; // getprice() has 8 digit by own
