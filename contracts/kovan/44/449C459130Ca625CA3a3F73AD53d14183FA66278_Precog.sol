...
contract Precog is IPrecog {
    ...
    event Borrow(
        ...
        uint256 interest,
        uint256 fee);
    event Liquidate(address indexed user, address indexed futureContract, uint256 indexed timestamp);
                ...
                for (uint j = 0; j < futureContracts.length; j++) {
                    for (uint i = 0; i < exchanges.length; i++) {
                        ...
                        for (uint l = 0; l < loans.length; l++) {
                            ProfitInfo memory profitInfo = _calculateProfit(user, amount, fee, _tradeInfo, loans[l]);
                            if (profitInfo.profit > profit) {
                                profit = profitInfo.profit;
                                tradeInfo = _tradeInfo;
                                if (profitInfo.canBorrow && loans[l].borrowAmount > 0) {
                                    borrowInfo = loans[l];
                                }
                            }
                        }
    ...
    ) internal view returns (ProfitInfo memory profitInfo) {
        profitInfo.expiryDate = IFutureContract(tradeInfo.futureContract).expiryDate();
        if (profitInfo.expiryDate > block.timestamp) {
            ...
            if (pairs[0] != address(0)) {
                if (amount > fee.feeTrading) {
                    ...
                    uint revenue = _getRevenue(tradeAmount, profitInfo.expiryDate, pairs, tradeInfo);
                    if (revenue > amount) {
                        profitInfo.profit = revenue - amount;
                    }
                    if (loan.platformIndex > 0) {
                        ...
                        revenue = _getRevenue(tradeAmount, profitInfo.expiryDate, pairs, tradeInfo);
                        uint debtAmount = _getDebtAmount(loan, block.timestamp, profitInfo.expiryDate);
                        if (revenue > amount + debtAmount) {
                            uint profitLoan = revenue - amount - debtAmount;
                            if (profitLoan > profitInfo.profit) {
                                profitInfo.profit = profitLoan;
                                profitInfo.canBorrow = true;
                            }
                        }
                    }
                }
    ...
    function invest(
        ...
        ProfitInfo memory profitInfo = _calculateProfit(user, amount, fee, tradeInfo, loan);
        require(profitInfo.profit > 0, "PrecogV2: NOT_PROFITABLE");

        uint tradeAmount = amount - fee.feeTrading;
        if (profitInfo.canBorrow) {
            uint debtAmount = _getDebtAmount(loan, block.timestamp, profitInfo.expiryDate);
            uint interest = debtAmount - loan.borrowAmount;
            ILending(lendingContract).createLoan(platformIndex, usdc, loan.borrowAmount);
            emit Borrow(user, tradeInfo.futureContract, loan.platformIndex, loan.borrowAmount, interest, fee.feeLending);
        ...
        uint revenue = _executeTrade(tradeAmount, profitInfo.expiryDate, tradeInfo);
        require(revenue > amount + loan.borrowAmount, "PrecogV2: NOT_PROFITABLE");
        profitInfo.profit = revenue - amount - loan.borrowAmount;

        _updateTradingAmount(user, tradeInfo.futureContract, amount, profitInfo.profit);
        _removeTradingUser(user);

        emit Trade(user, tradeInfo.futureContract, profitInfo.expiryDate,
            amount, profitInfo.profit, fee.feeTrading, block.timestamp);
    }
    ...
    function _executeTrade(uint amount, uint expiryDate, TradeInfo memory tradeInfo) internal returns(uint) {
        address[] memory pairs = _getPairs(tradeInfo.futureContract);
