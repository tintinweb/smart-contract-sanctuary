...
contract FFWhitelistedCommit is Ownable, Pausable {
  ...
  modifier onlyWhitelisted() {
    require(allocations[msg.sender].isWhitelisted, "Wallet is not whitelisted");
    _;
  }
  ...
  {
    if (!allocations[_addr].isWhitelisted) {
      allocations[_addr].isWhitelisted = true;
  ...
  function addAddressesToWhitelist(
    ...
    uint256[] memory _usdcAllocations
  ) public onlyOwner returns (bool success) {
    for (uint256 i = 0; i < _addrs.length; i++) {
      if (addAddressToWhitelist(_addrs[i], _usdcAllocations[i])) {
        success = true;
      }
    }
  }
  ...
  {
    if (allocations[_addr].isWhitelisted) {
      allocations[_addr].isWhitelisted = false;
