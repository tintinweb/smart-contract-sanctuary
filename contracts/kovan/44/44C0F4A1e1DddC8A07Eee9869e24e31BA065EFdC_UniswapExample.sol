...
contract UniswapExample {
    function myconsult(address pool, uint32 period) public view returns (int24 timeWeightedAverageTick) {
        return OracleLibrary.consult(pool, period);
    }
    function mygetQuoteAtTick(
        int24 tick,
        uint128 baseAmount,
        address baseToken,
        address quoteToken
    ...
    function mygetOldestObservationSecondsAgo(address pool) public view returns (uint32) {
        return OracleLibrary.getOldestObservationSecondsAgo(pool);
    }
    function mygetBlockStartingTick(address pool) public view returns (int24) {
