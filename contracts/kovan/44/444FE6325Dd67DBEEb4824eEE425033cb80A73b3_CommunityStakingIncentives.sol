...
import "../../phase1/3rdParty/interfaces/INXMMaster.sol";
contract CommunityStakingIncentives is ReentrancyGuard {
  ...
  */
  constructor(uint _roundDuration, uint _roundsStartTime, address masterAddress) public {
    ...
    require(_roundsStartTime >= block.timestamp, "_roundsStartTime needs to be in the future");
  ...
  */
  function getCurrentRound() public view returns (uint) {
    require(roundsStartTime <= block.timestamp, "Rounds haven't started yet");
    return (block.timestamp - roundsStartTime) / roundDuration + 1;
  }
