...
struct StakeInfo {
    ...
    uint autorenewAt; // if true user don't have to do anything for continue staking
    uint claimed;
}
...
contract StakeRewardFixed is Initializable, BumperAccessControl {
    ...
    // amounts of stakes in each option
    mapping(uint16=>uint) public stakeOptionsAmount;

    // flexible staking contract address
    ...
    uint64 public withdrawWindow; // withdraw window for autorenew option. 

    // emitted when user successfuly staked tokens
    event Staked(address sender, uint256 amount, uint256 period, bool autorenew);
    ...
    event Withdrawn(address sender, uint256 amount, uint256 rewards );
    ...
    function calcUserStakeRewards(uint256 amount, uint32 bonusInPercentage)
        public
        pure
    ...
    /// @notice get amount of all stakes in all options
    function totalReservedAmount() public view returns (uint reserves) {
        for (uint16 i = 0; i < stakeOptions.length; i++) {
            reserves += stakeOptionsAmount[i];
        }        
    }
    ...
    function freeAmount() public view returns (uint256) {  
        return
            IERC20(stakeToken).balanceOf(address(this)) - totalReservedAmount();
    }
    ...
    function stake(uint256 amount, uint16 option, bool autorenew) external {
    ...
    function _stakeFor(
        ...
        StakeInfo memory newStake = StakeInfo(
            ...
            option,
            autorenew ? uint64(block.timestamp) : 0,
            0
        );
        usersStake[account].push(newStake);
        stakeOptionsAmount[option] += amount;

        // notify flexible staking contract
        notifyFlexible();

        emit Staked(account, amount, opt.periodInDays, autorenew);
    }

    /// @notice withdraw tokens
    /// @param stakeIndex - index in users stakes array
    function withdraw(uint16 stakeIndex) external {
        ...
        (uint rewards, , bool withdrawable) = calcRewards(s);
        ...
        emit Withdrawn( msg.sender, s.amount, rewards );
    }

    /// @notice set autorenew
    function switchAutorenew(uint16 stakeIndex) external {
        StakeInfo[] storage stakeInfoList = usersStake[msg.sender];
        require(stakeInfoList.length > stakeIndex, "!index");
        StakeInfo storage s = stakeInfoList[stakeIndex];
        // stake have to be in progress and not finished
        if (s.autorenewAt > 0) {
            s.autorenewAt = 0;
        }
        else {
            require( block.timestamp < s.start + s.interval * 1 days, "end" );
            s.autorenewAt = uint64(block.timestamp);
        }
    }
    ...
    function claimRewards(uint16 stakeIndex) external {
        ...
        (uint rewards, bool claimable, ) = calcRewards(s);

        require( claimable, "!rewards" );
    ...
    function calcRewards(StakeInfo memory s) public view returns (uint rewards, bool claimable, bool withdrawable) {
        if (s.autorenewAt > 0) {
            // calculate number of periods
            uint periods = (block.timestamp - s.start) / (1 days * s.interval);  
            // we must be in withdraw window
            uint endOfLastPeriod = s.start + (periods * 1 days * s.interval);
            // calculate rewards
            rewards = periods * s.rewards - s.claimed; 

            claimable = rewards > 0;
            withdrawable = block.timestamp > endOfLastPeriod && block.timestamp < endOfLastPeriod + withdrawWindow;
        }
        else { // no autorenew
            claimable = block.timestamp > (s.start + (s.interval * 1 days));
            withdrawable = claimable;
            // we must be after stake ends
            rewards = claimable ? s.rewards : 0;
        }
    }

    
    /// @notice Restake tokens of given stake to new stake with given option with or without rewards
    function restake(uint16 stakeIndex, uint16 option, bool withRewards, bool autorenew) external {
        require(option < stakeOptions.length, "!option");
        StakeOption memory opt = stakeOptions[option];
        ...
        StakeInfo memory newStake = StakeInfo(
            amount,
            calcUserStakeRewards(amount, opt.bonusInPercentage),
            uint64(opt.periodInDays),
            uint64(block.timestamp),
            option,
            autorenew ? uint64(block.timestamp) : 0,
        ...
        emit Staked(msg.sender, amount, opt.periodInDays, autorenew);        
    }
    ...
    function currentEmissionPerSecond() public view returns (uint256) {
        ...
        for (uint16 i = 0; i < stakeOptions.length; i++) {
            ...
            emission +=
                (stakeOptionsAmount[i] * option.bonusInPercentage) /
                (option.periodInDays * 1 days);
        }
        return emission;
    }

    /// @notice notify flexible staking contract about changes in amounts (for recalculation of cummulative index and change emission per second)
    ...
    /// @notice set options for staking (add new options ok, but not recomended to change options after run because of calculated emission will be incorect)
    /// will be applied to new stakes
    /// @param _options - array of options indexes
    /// @param _periods - array of periods for options
    /// @param _bonusesInPercentage - bonuses for each option in percents (3000 = 30%)
    function setStakeOptions(
        ...
        require(
            _options.length == _periods.length &&
                _options.length == _bonusesInPercentage.length,
            "!length"
        );
        delete stakeOptions;
        for (uint256 i = 0; i < _options.length; i++) {
            stakeOptions.push(
                StakeOption(_periods[i], _bonusesInPercentage[i])
            );
        }

        // notify flexible staking contract that we change percentage
        notifyFlexible();
    }
    ...
    /// @notice set withdraw window for autorenew option of staking
    function serWithdrawWindow(uint64 _window) external onlyGovernance {
        withdrawWindow = _window;
    }
    ...
    OwnableUpgradeable
{
    ...
    function _BumperAccessControl_init(address[] memory _whitelist)
        ...
        initializer
    {
        require(_whitelist.length > 0, "!whitelist" );
