...
contract ConstitutePharo is PharoPhactory {
  ...
  function addNewProvider(address _providerAddress, string memory _eventHash, uint256 _odds)
    public
    payable
  ...
  function _liquidityIn(string memory eventHash, address providerAddress, uint amount, uint odds)
    internal 
  {
    uint256 xx = pharoDict[eventHash].pharoProviderCount[eventHash] +1;
    uint[] memory tranches;
    tranches[0] = 0;
    pharoDict[eventHash].pharoProviders[0][xx] = Provider(providerAddress, odds, amount);
