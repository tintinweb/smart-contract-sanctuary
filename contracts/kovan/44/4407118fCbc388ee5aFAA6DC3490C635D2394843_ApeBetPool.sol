...
contract ApeBetPool is Ownable {
    ...
    function getRewardETH(uint256 amount, address account)
        public
        view
        returns (uint256 rewardETH)
    {
        rewardETH = bookMaker
            .maxPoolPayout()
            .mul(deposits[account].mul(1e4).div(supplierDeposit))
            .mul(amount.mul(1e4).div(betToken.balanceOf(account)))
            .div(1e8);
        return rewardETH;
    }

    function depositBetToken(uint256 amount) external {
        uint256 ethAmount = getRewardETH(amount, msg.sender);
