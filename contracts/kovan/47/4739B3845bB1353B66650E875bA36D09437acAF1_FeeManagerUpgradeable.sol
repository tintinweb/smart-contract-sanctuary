...
contract FeeManagerUpgradeable is Initializable, ContextUpgradeable, AccessControlUpgradeable {
    ...
    function __FeeManagerUpgradeable_init(address handlerAddress) internal initializer {
        ...
        _setupRole(FEE_SETTER_ROLE, handler);
    }
    ...
    /**
        ...
        @param  feeTokenAddrs addresses of the fee token.
        @param  transferFees Value {_transferFee}[] will be updated to.
        @param  exchangeFees Value {_exchangeFee}[] will be updated to.
    ...
    function setFees(
        ...
        require(feeTokenAddrs.length == transferFees.length, "feeTokenAddrs and transferFees len mismatch");
        require(feeTokenAddrs.length == exchangeFees.length, "feeTokenAddrs and exchangeFees len mismatch");
        require(feeTokenAddrs.length == accepted.length, "feeTokenAddrs and exchangeFees len mismatch");
        uint256 feeAddrCount = feeTokenAddrs.length;
        for (uint256 i = 0; i < feeAddrCount; i++) {
            setFee(destinationChainID, feeTokenAddrs[i], transferFees[i], exchangeFees[i], accepted[i]);
        }
    }
