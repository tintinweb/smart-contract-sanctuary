...
contract Precog is IPrecog {
    ...
    function liquidate(address futureContract, address user) external returns(uint256 amount) {
        ...
        _repayLoan(user, futureContract);
    ...
    function _repayLoan(address user, address futureContract) internal {
        BorrowInfo[] memory loans = userBorrowInfo[user][futureContract];
        if (loans.length > 0) {
            for (uint i = 0; i < loans.length; i++) {
                uint interestRate = loans[i].interestRate * (block.timestamp - loans[i].timestamp);
                uint repayAmount = loans[i].borrowAmount + loans[i].borrowAmount * interestRate / (10 ** 18);
                ILending(lendingContract).repayLoan(loans[i].platformIndex, usdc, repayAmount);
                ...
                for (uint j = 0; j < futureContracts.length; j++) {
                    for (uint i = 0; i < exchanges.length; i++) {
                        for (uint l = 0; l < loans.length; l++) {
                            ...
                            (uint _profit, bool canBorrow,,) = _calculateProfit(user, amount, _tradeInfo, loans[l]);
                            if (_profit > profit) {
                                profit = _profit;
    ...
    function _getAvailableLoan(uint platformIndex, uint borrowLimit) internal view returns (BorrowInfo memory loan) {
        (uint borrowAmount, uint interestRate) = ILending(lendingContract).getLendingPlatformInfo(platformIndex, usdc);
    ...
    function _calculateProfit(
        ...
        if (expiryDate > block.timestamp) {
            ...
            if (pairs[0] != address(0)) {
                (uint revenue, uint _tradeAmount) = _getRevenue(amount, expiryDate, pairs, tradeInfo);
                if (revenue > availableAmount[user]) {
                    profit = revenue - availableAmount[user];
                    tradeAmount = _tradeAmount;
                }
                
                (revenue, _tradeAmount) = _getRevenue(amount + loan.borrowAmount, expiryDate, pairs, tradeInfo);
                ...
                if (revenue > availableAmount[user] + loanAmount) {
                    uint profitLoan = revenue - availableAmount[user] - loanAmount;
    ...
    function _getRevenue(
        ...
        TradeInfo memory tradeInfo
    ) internal view returns (uint revenue, uint tradeAmount) {
        ...
        try exchange.getAmountsOut(amount, pairs) returns(uint[] memory amountsOut) {
            ...
            try futureExchange.getAmountsOutFuture(tradeAmount, pairs[1], usdc, expiryDate) returns(uint _revenue) {
                revenue = _revenue;
            } catch {}
        } catch {}
    }
    ...
    function invest(
        ...
        (uint profit, bool canBorrow, uint tradeAmount, uint expiryDate) = _calculateProfit(user, amount, tradeInfo, loan);
        ...
        _executeTrade(amount, tradeAmount, expiryDate, tradeInfo);
    ...
    function _executeTrade(uint amount, uint tradeAmount, uint expiryDate, TradeInfo memory tradeInfo) internal {
        address[] memory pairs = _getPairs(tradeInfo.futureContract);
