...
// File: contracts/ReethClaimableStaker.sol
...
contract ReethClaimableStaker is Ownable, ReentrancyGuard {
    ...
    function getETHSpentSinceAction(address _user) external view returns (uint256) {
        ...
        if(total > allUsersInfo[_user].lastETHSpent){
            return total.sub(allUsersInfo[_user].lastETHSpent);
        }else{
    ...
    function stakeZSToken(uint256 _amount) external nonReentrant
    {
        ...
        allUsersInfo[_msgSender()].lastActionTime = now;

        IERC20 _token = IERC20(zsTokenAddress); // Trusted token
        uint256 _before = _token.balanceOf(address(this));
        _token.safeTransferFrom(_msgSender(), address(this), _amount); // Transfer token to this address
        _amount = _token.balanceOf(address(this)).sub(_before);
        
        totalZSTokens = totalZSTokens.add(_amount);
        allUsersInfo[_msgSender()].depositAmount = allUsersInfo[_msgSender()].depositAmount.add(_amount);
        emit StakedToken(_msgSender(), _amount);
        if(ethSpentOracleAddress != address(0)){
    ...
    {
        internalClaimReward(_msgSender());
    }
    
    function claimAndExit(uint256 _amount) external nonReentrant
    {
        internalClaimReward(_msgSender());
        internalUnstake(_msgSender(), _amount);
    }
    ...
    function internalUnstake(address _user, uint256 _amount) internal {
        require(allUsersInfo[_user].depositAmount > 0, "There is no balance for this user");
        allUsersInfo[_user].lastActionTime = now;
        totalZSTokens = totalZSTokens.sub(_amount);
        allUsersInfo[_user].depositAmount = allUsersInfo[_user].depositAmount.sub(_amount);
        IERC20(zsTokenAddress).safeTransfer(_user, _amount);
        emit UnstakedToken(_user, _amount);
    }
    
    // Governance only functions
    function governanceSetZSToken(address _addr) external onlyGovernance {
        require(zsTokenAddress == address(0), "Token is already set");
    ...
    function startChangeClaimerPolicy(address _address) external onlyGovernance {
        ...
        _timelockType = 2;
        _timelock_address = _address;
        if(claimPolicyAddress == address(0)){
            _timelockType = 0;
            internalChangeClaimerPolicy(_timelock_address);
        }
    }
    ...
    function startChangeSpentOracle(address _address) external onlyGovernance {
        ...
        if(ethSpentOracleAddress == address(0)){
            _timelockType = 0;
            internalChangeSpentOracle(_timelock_address);
        }
    }
