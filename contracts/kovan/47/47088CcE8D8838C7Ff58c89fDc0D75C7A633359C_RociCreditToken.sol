  ...
  */
  function burnToken() internal {
    ...
    for (
      ...
      console.log(
        "Addr: ",
        _addressNonce[_tokenBundle[tokenOfOwnerByIndex(msg.sender, 0)][i]]
      );
      _addressNonce[
    ...
    // Burn the token
    super._burn(tokenOfOwnerByIndex(msg.sender, 0));
...
interface RociCreditTokenInterface {
    ...
    function mintToken(address[] memory bundle, bytes[] memory signatures, string memory _message, uint _nonce) external;
    // Remits the token, receives the same params as mintToken()
    function remintToken(address[] memory bundle, bytes [] memory signatures, string memory _message, uint _nonce) external;
