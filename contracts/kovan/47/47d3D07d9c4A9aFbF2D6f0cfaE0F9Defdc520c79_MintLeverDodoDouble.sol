...
contract MintLeverDodoDouble is MintLever {
    ...
    function setLiquidityEarningMine(ILiquidityDodo _liquidity, IEarning _earning, address _mine) external onlyOwner nonZeroAddress(address(_liquidity)) nonZeroAddress(address(_earning)) nonZeroAddress(_mine) {
        ...
        address capital = _earning.getStakeToken();
        require(address(0) != capital, "publics:capital_is_zero");
        (address tokenA, address tokenB) = liquidity.getPairDouble(capital);
    ...
    function openPosition(uint256 amountA, uint256 amountB, uint256 leverage, address borrowToken, uint256 deadLine) override external lock {
        ...
        data.addTokenAmount(_user, amountA, amountB);
        (amountA, amountB, ) = config.getBorrowProxy().borrowV1(_user, _userInfo.id, _userInfo.tokenA, amountA, _userInfo.tokenB, amountB, _userInfo.borrowToken, leverage, deadLine);
        IERC20(_userInfo.tokenA).approve(address(liquidity), amountA);
        IERC20(_userInfo.tokenB).approve(address(liquidity), amountB);
        uint256 _amount;
        (_amount, amountA, amountB) = liquidity.addDouble(_userInfo.capital, _userInfo.tokenA, amountA, _userInfo.tokenB, amountB, deadLine);
        data.updateCapitalAmount(_user, _amount, IOperateType.MathOperateType.Add);
        IERC20(_userInfo.capital).approve(address(earning), _amount);
        IEarning(earning).stake(_user, _amount);
        _safeTransfer(_userInfo.tokenA, _user, amountA);
        _safeTransfer(_userInfo.tokenB, _user, amountB);
    ...
    function closePosition(uint256 percentage, address receiveToken, uint256 deadLine) override external lock {
        ...
        (uint256 _amountA, uint256 _amountB) = liquidity.removeDouble(_userInfo.capital, _userInfo.tokenA, _userInfo.tokenB, _amount, deadLine);
        if (0 < _userInfo.borrowAmount) {
            uint256 _repayAmountA = _amountA.sub(_amountA.mul(100 - percentage).div(100));
            uint256 _repayAmountB = _amountB.sub(_amountB.mul(100 - percentage).div(100));
            _amountA = _amountA.sub(_repayAmountA);
            _amountB = _amountB.sub(_repayAmountB);
            _amount = _swap(_userInfo.tokenA, _userInfo.borrowToken, _repayAmountA, deadLine);
            _amount = _amount.add(_swap(_userInfo.tokenB, _userInfo.borrowToken, _repayAmountB, deadLine));
            (_amount, ) = _repay(_user, _amount);
            if (0 < _amount) {
                _repayAmountA = _amount.div(2);
                _repayAmountB = _amount.sub(_repayAmountA);
                _amountA = _amountA.add(_swap(_userInfo.borrowToken, _userInfo.tokenA, _repayAmountA, deadLine));
                _amountB = _amountB.add(_swap(_userInfo.borrowToken, _userInfo.tokenB, _repayAmountB, deadLine));
            }
        }
        if (address(0) == receiveToken) {
            _safeTransfer(_userInfo.tokenA, _user, _amountA);
            _safeTransfer(_userInfo.tokenB, _user, _amountB);
        }else {
            _amount = _swap(_userInfo.tokenA, receiveToken, _amountA, deadLine);
            _amount = _amount.add(_swap(_userInfo.tokenB, receiveToken, _amountB, deadLine));
    ...
    function directClearingPosition(address user, uint256 deadLine) override external lock {
        ...
        IERC20(_userInfo.capital).approve(address(liquidity), _amount);
        uint256 _amountB;
        (_amount, _amountB) = liquidity.removeDouble(_userInfo.capital, _userInfo.tokenA, _userInfo.tokenB, _amount, deadLine);
        _amount = _swap(_userInfo.tokenA, _userInfo.borrowToken, _amount, deadLine);
        _amount = _amount.add(_swap(_userInfo.tokenB, _userInfo.borrowToken, _amountB, deadLine));
        ...
        for (uint256 i = 0; i < _bondTokens.length; i++) {
            _amountB = _redeem(_bondTokens[i], _amounts[i]);
            _amount = _amount.add(_swap(_bondTokens[i], _userInfo.borrowToken, _amountB, deadLine));
        }
    ...
    function indirectClearingPosition(address user, address token, uint256 amount) override external lock {
        require(user != msg.sender, "publics:liquidator_equal_borrower");
