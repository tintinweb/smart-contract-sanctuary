...
contract BuryBone is ERC20("BuryBone", "tBONE"){
    using SafeMath for uint256;

    struct UserInfo {
        uint256 amount;
        uint256 rewardDaiDebt;
    ...
    constructor(IERC20 _bone, IERC20 _dai, IERC20 _usdt) public {
        bone = _bone;
        dai = _dai;
        usdt = _usdt;
    ...
    function enter(uint256 _amount) public {
        ...
        user.rewardDaiDebt = user.amount.mul(poolInfo[0].accRewardPerShare).div(1e12);
        user.rewardUsdtDebt = user.amount.mul(poolInfo[1].accRewardPerShare).div(1e12);
    }
    ...
    function sendRewards(uint256 _pid, uint256 _share) internal {
        UserInfo storage user = userInfo[msg.sender];
        PoolInfo storage pool = poolInfo[_pid];
        
        uint256 what;
        if(_pid == 0)
            what = user.amount.mul(pool.accRewardPerShare).div(1e12).sub(user.rewardDaiDebt);
        else
            what = user.amount.mul(pool.accRewardPerShare).div(1e12).sub(user.rewardUsdtDebt);
        
        pool.token.transfer(msg.sender, what);
        pool.lastRewardBalance = pool.token.balanceOf(address(this));
        if(_pid == 0)
            user.rewardDaiDebt = user.amount.mul(pool.accRewardPerShare).div(1e12);
        else
            user.rewardUsdtDebt = user.amount.mul(pool.accRewardPerShare).div(1e12);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 rewardBalance = pool.token.balanceOf(address(this));
        uint256 _totalReward = pool.totalReward.add(rewardBalance.sub(pool.lastRewardBalance));
        pool.lastRewardBalance = rewardBalance;
        pool.totalReward = _totalReward;
        
        uint256 lpSupply = bone.balanceOf(address(this));
        
        if(lpSupply != 0) {
            uint256 reward = _totalReward.sub(pool.lastTotalReward);
            pool.accRewardPerShare = pool.accRewardPerShare.add(reward.mul(1e12).div(lpSupply));
            pool.lastTotalReward = _totalReward;
        } else {
            pool.accRewardPerShare = 0;
            pool.lastTotalReward = 0;
            user.rewardDaiDebt = 0;
