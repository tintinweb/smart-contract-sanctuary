...
contract OrcSales_II is Ownable {
    ...
    function getCurrentPrice() public view returns (uint256) {
        ...
        //Todo voltar para 0.04(4e16) e .05(5e16)
        if (block.timestamp <= beginningOfPriceIncrease) {
            return uint256(1e16).add(calculateFee()); // 0.04 ETH + 10 USD in ETH
        } else {
            return uint256(1e16).add(calculateFee()); // 0.05 ETH + 10 USD in ETH
        }
    }
