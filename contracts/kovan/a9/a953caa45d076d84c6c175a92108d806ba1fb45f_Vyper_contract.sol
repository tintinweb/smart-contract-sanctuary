...
@view
@internal
def _totalToPaid() -> uint256:# Formula
...
def withdraw(_amount: uint256):
    ...
    rewards: uint256 = convert(
        convert(
            self.paidPerToken * self.holdings[msg.sender] - self.userRewardsPaid[msg.sender], int128)
             - self.compactor[msg.sender], uint256
             )/10**6  # Rewards are always positive.
...
def bal(user: address) -> (uint256, uint256):
    totalToPaid: uint256 = ((self.balance - self.totalStaked) * (block.number - self.lastBlock))/(self.endBlock - self.lastBlock) * 10**18


    return (
        self.holdings[user],
        convert(convert(totalToPaid / self.totalStaked * self.holdings[msg.sender] - self.userRewardsPaid[msg.sender], int128) - self.compactor[msg.sender], uint256)/10**18
        )
