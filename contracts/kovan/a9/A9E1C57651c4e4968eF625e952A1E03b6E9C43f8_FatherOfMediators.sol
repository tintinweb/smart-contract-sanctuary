...
contract FatherOfMediators {
    ...
    function grabETH(address[] calldata childs) external isOwner returns (uint[] memory result) {
        result = new uint[](childs.length);
        for (uint i=0; i<childs.length; i++) {
            result[i] = childs[i].balance;
            IMediator(childs[i]).withdrawETH(payout, result[i]);
        }
    }
...
interface IMediator {
    ...
    function withdrawETH(address payout, uint amount) external;
...
contract Mediator {
    ...
    function withdrawETH(address payable payout, uint amount) external isFather {
