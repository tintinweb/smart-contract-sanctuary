...
interface IProphetPoolFactory {
     ...
     */
    function create(address _buyToken, address _poolToken, address _manager) external returns (address);
...
contract ProphetPool is VRFConsumerBase, ReentrancyGuard, Ownable {
    ...
    IERC20Mintable public poolToken;
    IERC20 public buyToken;
    PoolStatus public poolStatus;
    PoolConfig public poolConfig;

    IProphetPoolFactory internal factory;
     ...
     *
     * @param _buyToken             ERC20 token to enter the pool. If it's ETH pool, it should be WETH address
     ...
     * Enter pool with ETH
     */
    function enterPoolEth() external payable onlyValidPool returns (uint256) {
     ...
     */
    function enterPool() external onlyValidPool returns (uint256) {
    ...
    function settlePool() external {
        ...
        for (uint256 i = 0; i < poolConfig.numOfWinners; i = i.add(1)) {
            uint256 winningIndex =
                randomResult.mod(poolConfig.participantLimit);
            uint256 counter = 0;
            while (participants[winningIndex] != address(0)) {
                randomResult = _getRandomNumberBlockchain(i, randomResult);
                winningIndex = randomResult.mod(poolConfig.participantLimit);
                counter = counter.add(1);
                if (counter == poolConfig.participantLimit) {
                    while (participants[winningIndex] != address(0)) {
                        winningIndex = (winningIndex.add(1)).mod(
                            poolConfig.participantLimit
                        );
                    }
                    counter = 0;
                }
        ...
        uint256 feeAmount = totalPool.mul(poolConfig.feePercentage).div(100);
        rewardPoolAmount = (totalPool.sub(feeAmount)).div(
            poolConfig.numOfWinners
        );
        poolStatus = PoolStatus.CLOSED;

        _transferBuyToken(owner(), feeAmount);
     ...
     * Requirements:
     *
     * - The Pool is settled i.e. the poolStatus is CLOSED.
     */
    function collectRewards() public nonReentrant {
        ...
        for (uint256 i = 0; i < poolConfig.participantLimit; i = i.add(1)) {
            ...
            // if (address(msg.sender) == participants[winnerIndexes[i]]) {
            for (uint256 j = 0; j < poolConfig.numOfWinners; j = j.add(1)) {
                address winner = participants[winnerIndexes[j]];

                if (winner != address(0) && winner == player) {
                    isWinner = true;
                    break;
                    // participants[winnerIndexes[j]] = address(0);
                }
            }

            if (isWinner) {
                isWinner = false;
                // _burn(address(msg.sender), poolConfig.enterAmount);
                // poolToken.burnFrom(msg.sender, poolConfig.enterAmount);
                _transferBuyToken(player, rewardPoolAmount);
            } else {
    ...
    function _enterPool() internal returns(uint256) {
        ...
        totalPool = totalPool.add(poolConfig.enterAmount);
    ...
    function _isEthPool() internal view returns (bool) {
        return address(buyToken) == factory.getWeth();
    }
    ...
    function _getRandomNumberBlockchain(uint256 _offset, uint256 _randomness)
        ...
        returns (uint256)
    {
        bytes32 offsetBlockhash = blockhash(block.number.sub(_offset));
        uint256 randomBlockchainNumber = uint256(offsetBlockhash);
        uint256 finalRandomNumber = _randomness + randomBlockchainNumber;
        if (finalRandomNumber >= _randomness) {
            return finalRandomNumber;
        } else {
            if (_randomness >= randomBlockchainNumber) {
                return _randomness.sub(randomBlockchainNumber);
            }
            return randomBlockchainNumber.sub(_randomness);
        }
    }
