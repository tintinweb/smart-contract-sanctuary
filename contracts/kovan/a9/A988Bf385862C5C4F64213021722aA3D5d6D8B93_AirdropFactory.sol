...
contract AirdropFactory {
    ...
    function createAirdrop(
        ...
        drop = Airdrop(Clones.cloneDeterministic(template, salt));
        drop.init(msg.sender, token, merkleRoot, expireTimestamp);
        emit CreateAirdrop(address(drop), ipfsHash);
    }

    function computeAirdropAddress(address template, bytes32 salt)
        external
        view
        returns (address)
    {
        return Clones.predictDeterministicAddress(template, salt, address(this));
    }
}
