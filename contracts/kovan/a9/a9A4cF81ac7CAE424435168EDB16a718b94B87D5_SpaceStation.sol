...
contract SpaceStation is ReentrancyGuard, EIP712 {
    ...
    function claim(uint256 _cid, IStarNFT _starNFT, uint256 _dummyId, uint256 _powah, address _account, bytes calldata _signature) external payable onlyNoPaused {
        require(!hasMinted[_dummyId], "Already minted");
        require(_verify(_hash(_starNFT, _dummyId, _powah, _account), _signature), "Invalid signature");
        hasMinted[_dummyId] = true;
        _payFees(_cid, Operation.Claim);
        uint256 nftID = _starNFT.mint(_account, _powah);
        emit EventClaim(_cid, _dummyId, nftID, msg.sender);
    }

    function claimBatch(uint256 _cid, IStarNFT _starNFT, uint256[] calldata _dummyIdArr, uint256[] calldata _powahArr, address _account, bytes calldata _signature) external payable onlyNoPaused {
        ...
        _payFees(_cid, Operation.Claim);
        require(_verify(_hashBatch(_starNFT, _dummyIdArr, _powahArr, _account), _signature), "Invalid signature");
        uint256[] memory nftIdArr = _starNFT.mintBatch(_account, _powahArr.length, _powahArr);
    ...
    /* ============ Internal Functions ============ */
    function _hash(IStarNFT _starNFT, uint256 _dummyId, uint256 _powah, address _account) public view returns (bytes32) {
        // TODO: change to internal on prd
        // function _hash(address account, uint256 id, uint256 powah) internal view returns (bytes32) {
    ...
    function _hashBatch(IStarNFT _starNFT, uint256[] calldata _dummyIdArr, uint256[] calldata _powahArr, address _account) public view returns (bytes32) {
        // TODO: change to internal on prd
        return _hashTypedDataV4(keccak256(abi.encode(
