...
contract Boring is ERC20Burnable, Ownable{
    ...
    IERC20 public bor;
    mapping(address=>uint) public borBalanceOf;
    uint public ratio;
    bool public switchOn = true;

    constructor(address _bor, uint _ratio) ERC20("BoringDAO Token", "BORING") {
        bor = IERC20(_bor);
        ratio = _ratio;
    }

    function setSwtichOn(bool _switchOn) public onlyOwner {
    ...
    function toBoring(uint borAmount) public {
        ...
        borBalanceOf[msg.sender] += borAmount;
    ...
    // If someone directly transfers boring tokens to the contract address,
    // this method can return his boring
    function returnBoring(address account, uint amount) public onlyOwner{
        transfer(account, amount);
        emit ReturnBoring(account, amount);
    }
    ...
    event ReturnBoring(address account, uint amount);
