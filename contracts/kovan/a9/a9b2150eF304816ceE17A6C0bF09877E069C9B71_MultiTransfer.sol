...
contract MultiTransfer is Ownable {
  ...
  function transfer(IERC20 _token, uint256 _total, address[] calldata _addresses, uint256[] calldata _amounts) external onlyOwner {
  ...
  function collect(IERC20[] calldata _tokens) external onlyOwner {
    for (uint256 i = 0; i < _tokens.length; i++) {
      _tokens[i].safeTransfer(owner(), _tokens[i].balanceOf(address(this)));
    }
  }
}
