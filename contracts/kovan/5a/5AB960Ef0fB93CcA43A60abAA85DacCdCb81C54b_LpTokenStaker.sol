...
// LP token staking contract for http://frozenyogurt.finance/
// LP tokens are staked within this contract to generate Froyo, Frozen Yogurt' value-capture token
...
contract LpTokenStaker is Ownable {
    ...
    // For pools handling USD-based Frozen Yogurt LP tokens the value per token is assumed as $1,
    // for non-USD pools a rate is queried from a Chainlink oracle.
    function _getAllocPoints()
        internal
        view
        returns (uint256[] memory allocPoints, uint256 totalAP)
    {
        ...
        for (uint256 pid = 1; pid < length; ++pid) {
            ...
            allocPoints[pid] = pool.allocPoint.mul(
                latestPrices[pool.oracleIndex]
            );
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            uint256 reward =
                duration.mul(rewardsPerSecond).mul(allocPoints[_pid]).div(
                    totalAP
                );
    ...
    function _updatePool(
        ...
        uint256 duration = block.timestamp.sub(pool.lastRewardTime);
        uint256 reward =
            duration.mul(rewardsPerSecond).mul(_allocPoint).div(
                _totalAllocPoint
            );
