...
// hevm: flattened sources of src/Refarm.sol
...
contract RefarmDividendTracker is IDividendDistributor {
...
contract Refarm is Ownable, IERC20 {
    ...
    RefarmDividendTracker public dividendTracker;
    ...
    function takeFee(
        ...
        uint256 totalFee;
        if (sender == pair) totalFee = buyFees.totalFee;
        else totalFee = sellFees.totalFee;
    ...
    function swapBack() internal swapping {
        ...
        if (!ignoreLimit) swapAmount = swapThreshold;
    ...
    function setExcludeFeeMultiple(address[] calldata _users, bool exempt)
        external
        onlyOwner
    {
    ...
    function setExcludeTxMultiple(address[] calldata _users, bool exempt)
        external
        onlyOwner
    {
        for (uint8 i = 0; i < _users.length; i++) {
    ...
    function setReceiver(address _treasuryWallet, address _liquidityWallet)
        external
        onlyOwner
    {
    ...
    function setDistributionCriteria(
        ...
        dividendTracker.setDistributionCriteria(
            _minPeriod,
            _minDistribution,
