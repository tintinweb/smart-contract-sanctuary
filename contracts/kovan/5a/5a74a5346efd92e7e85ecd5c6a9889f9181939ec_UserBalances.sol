...
// File: contracts/UserBalances.sol
...
contract UserBalances is ContractOwnable {
 ...
 function deposit(string memory matchNo , address account , uint8 position, uint256 amount ) public onlyFirstContract payable returns (uint256){
    ...
    if(position== 1){
      ...
      _drawBalance[matchNo][account] = _drawBalance[matchNo][account] + amount;
      _drawTotal[matchNo] = _drawTotal[matchNo] + amount; 
      _newBalance = _drawBalance[matchNo][account] ;
    }
    else if(position== 3){
      _awayBalance[matchNo][account] = _awayBalance[matchNo][account] + amount;
      _awayTotal[matchNo] = _awayTotal[matchNo] + amount; 
...
function depositTax(uint256 rewardAmount , uint256 depAmount) public onlyContractOwner {
     _rewardBalance = _rewardBalance + rewardAmount;
     _depBalance = _depBalance + depAmount;
 }
 ...
 function setKMatch (string memory matchNo , uint256 khome ,uint256  kdraw,uint256  kaway  ) public onlyFirstContract  returns (bool) {
      _kHome[matchNo] = khome;
      _kDraw[matchNo] = kdraw;
      _kAway[matchNo] = kaway;
    return  true;
  } 
  
 function setMatchStart (string memory matchNo   ,uint256 lockTime  ) public onlyFirstContract  returns (bool) {
      _matchName[matchNo] = true;
      _matchLock[matchNo] = lockTime;
    return  true;
  } 
 function setMatchUnlockStatus (string memory matchNo   ,bool isUnlock  ) public onlyFirstContract  returns (bool) {
      _matchUnLock[matchNo] = isUnlock;
    return  true;
  } 
 ...
 function _saveUserAccount(address account , string memory matchNo, uint8 position) internal {
  ...
  function matchName (string memory matchNo ) public view virtual  returns (bool) {
    return  _matchName[matchNo];
  } 
  function matchLock (string memory matchNo ) public view virtual  returns (uint256) {
    return  _matchLock[matchNo];
  } 
  function matchUnLock (string memory matchNo ) public view virtual  returns (bool) {
    return  _matchUnLock[matchNo];
  } 
  function kHome (string memory matchNo ) public view virtual  returns (uint256) {
    return  _kHome[matchNo];
  } 
  function kDraw (string memory matchNo ) public view virtual  returns (uint256) {
    return  _kDraw[matchNo];
  } 
  function kAway (string memory matchNo ) public view virtual  returns (uint256) {
    return  _kAway[matchNo];
  } 
