...
contract plafa {
   ...
   function mint$100() external payable returns(uint256){
       ...
       uint excess = 0;
       uint value = getEquivalentAmount(100000000000000000000);
       ...
       Transaction memory transaction =  Transaction(value,100,tokenPriceInitial_,0,0,0,block.timestamp,false,amountOfTokens);
   ...
   function mint$50() external payable returns(uint256){
       ...
       uint256 userDeposit = msg.value;
       uint excess = 0;
       uint value = getEquivalentAmount(50000000000000000000);
       require(userDeposit >= value, "Insufficeint funds");
       if(userDeposit > value)excess = SafeMath.sub(userDeposit, value); 
       if(excess > 0)payable(msg.sender).transfer(excess);
       userDeposit = value;
       uint256 amountOfTokens = ethereumToTokens_(userDeposit);
       Transaction memory transaction =  Transaction(value,50,tokenPriceInitial_,0,0,0,block.timestamp,false,amountOfTokens);
       transactions[msg.sender].push(transaction);
       userBalance[msg.sender] += amountOfTokens;
       tokenSupply_ = SafeMath.add(tokenSupply_, amountOfTokens);
   ...
   function burn(uint _index) external {
       ...
       require(SafeMath.div(dollarEquivalency,1e18) >= rewardLimit,"ERR_CANNOT_BURN");
      ...
      transactions[msg.sender][_index].dollarEquivalency = dollarEquivalency;
      transactions[msg.sender][_index].ethPrice = uint256(pfaddress.getLatestPrice());
    ...
    function emergencyBurn(uint _index) external  {
       ...
       rewardLimit = mintValue == 50? 125: 250;
       uint amount = tokensToEthereum_(transactions[msg.sender][_index].noOfTokensReceived);
       uint dollarEquivalency = SafeMath.sub(getEquivalentPrice(amount),35 ether);
          if(rewardLimit == 125) _burn(dollarEquivalency,30,4,1);  
   ...
   function totalContractBalance()
    ...
    function getEquivalentAmount(uint256 _dollarAmount)public view  returns(uint256){
        ...
        uint256 value = SafeMath.div(SafeMath.mul(_dollarAmount,1e8),uint256(currentPrice));
        return value;
    } 
     ...
     function getEquivalentPrice(uint256 _ethers)public view  returns(uint256){
        ...
        uint256 value = SafeMath.div(SafeMath.mul(_ethers,uint256(currentPrice)),1e8);
        return value;
    } 
