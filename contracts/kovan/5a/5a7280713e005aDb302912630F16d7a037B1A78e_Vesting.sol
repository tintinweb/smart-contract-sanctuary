...
abstract contract Core {
    ...
    event UserGranted(address user, uint256 start, uint256 amount, string schedule);
...
contract Vesting is Core {
    ...
    external {
        ...
        emit UserGranted(_user, _start, _amount, schedule.name);
    }
    ...
    function getAvailableAmount() public view returns (uint256 vestedTokens) {
        ...
        if (vestingDate > block.timestamp) {
            return uint256(0);
        }
        ...
        uint32 daysVested = uint32((block.timestamp - vestingDate) / 1 days);
        // Adjust result rounding down to take into consideration the interval.
        uint32 effective = (daysVested / _schedule.interval) * _schedule.interval;

        uint256 vested = _grant.amount * effective / _schedule.duration;
        return _grant.amount - vested;
    }
