...
contract ReservePool is Ownable, ReentrancyGuard {
	...
	* @param elasticPool_ - ElasticPool contract address
	*/
	function setElasticPool (
		address elasticPool_
	)
		external
		onlyOwner
	{
		require (address(elasticPool) == address(0), "ElasticPool address is already set");
		elasticPool = IElasticPool(elasticPool_);
	}
	...
	****************************************/
		...
		nonReentrant
	{
		...
		require(outToken.balanceOf(address(this)) >= amount_, "not enough Out Tokens available");
		inToken.burnFrom(msg.sender, amount_);
		outToken.safeTransfer(msg.sender, amount_);
		emit Exchanged(msg.sender, amount_);
	}

	/**
	 * @dev Transfers whole outToken balance to VPStorage in case of emergency stop on ElasticPool
	 */
