...
// File: sunderFactory.sol

contract SunderFactory {
    ...
    event Redeem(address indexed account,address indexed token, uint256 amount, uint256 due, uint256 fee);
    ...
    function locking(address token) public {
        require(msg.sender == operator || msg.sender == governance, "!operator");
        locks[token] = true;
    }

    function unlocking(address token) public {
        require(msg.sender == operator || msg.sender == governance, "!operator");
        locks[token] = false;
    }
    ...
    function createPair(address token) external  returns (address dtoken, address etoken) {
        require(msg.sender == governance, "!governance");
        require(token != address(0), " address(0)");
        require(dtokens[token] == address(0), "!address(0)");

        bytes memory nameD = abi.encodePacked("dToken ", ERC20(token).name());
        bytes memory symbolD = abi.encodePacked("d", ERC20(token).symbol());
        bytes memory nameE = abi.encodePacked("eToken ", ERC20(token).name());
        bytes memory symbolE = abi.encodePacked("e", ERC20(token).symbol());
        uint8 decimals = ERC20(token).decimals();

        bytes memory bytecodeD = type(DToken).creationCode;
        bytes32 saltD = keccak256(abi.encodePacked(token, nameD, symbolD));
        assembly {
            dtoken := create2(0, add(bytecodeD, 32), mload(bytecodeD), saltD)
        }
        DToken(dtoken).initialize(governance, decimals, nameD, symbolD);

        bytes memory bytecodeE = type(EToken).creationCode;
        bytes32 saltE = keccak256(abi.encodePacked(token, nameE, symbolE));
        assembly {
            etoken := create2(0, add(bytecodeE, 32), mload(bytecodeE), saltE)
        }
        EToken(etoken).initialize(governance, decimals, nameE, symbolE);

        dtokens[token] = dtoken;
        etokens[token] = etoken;

        emit PairCreated(token, dtoken, etoken);
    }
    ...
    function convert(address token, uint256 amount) external {
        ...
        _mint(token, msg.sender, amount);
    }

    function mint(address token, address minter, uint256 amount) external {
        require(msg.sender == controller, "!controller");
        require(dtokens[token] != address(0), "address(0)");
        _mint(token, minter, amount);
    }

    function _mint(address token, address minter, uint256 amount) internal {
        DToken(dtokens[token]).mint(minter, amount);
        EToken(etokens[token]).mint(minter, amount);
        emit Convert(minter, token, amount);
    }

    function redeem(address token, uint256 amount) external {
        require(!locks[token], "locking");
        require(dtokens[token] != address(0), "address(0)");

        DToken(dtokens[token]).burn(msg.sender, amount);
        EToken(etokens[token]).burn(msg.sender, amount);

        uint256 fee = amount.mul(withdrawalFee).div(withdrawalMax);
        uint256 due = amount.sub(fee);
        uint256 b = IERC20(token).balanceOf(address(this));
        if (b < amount) {
            uint256 _withdraw = amount.sub(b);
            IController(controller).withdraw(token, _withdraw);
            uint256 _after = IERC20(token).balanceOf(address(this));
            uint256 _real = _after.sub(b);
            if (_real < _withdraw) {
                uint256 _diff = _withdraw.sub(_real);
                if(_diff <= fee){
                    fee = fee.sub(_diff);
                }else{
                    fee = 0;
                    due = amount.sub(_diff);
                }
            }
        }

        IERC20(token).safeTransfer(reward, fee);
        IERC20(token).safeTransfer(msg.sender, due);
        emit Redeem(msg.sender, token, amount, due, fee);
    }
    ...
    function deposit(address token) public {
        ...
        IController(controller).earn(token, _bal);
    }
}