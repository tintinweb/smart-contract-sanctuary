...
contract CoinGeckoPrice is ChainlinkClient, Ownable, IPriceContract {
    ...
    function updatePrice(
        ...
        string memory name = IERC20(_tokens).name();

        // https://api.coingecko.com/api/v3/coins/chainlink/market_chart/range?vs_currency=usd&from=1639463340&to=1639464640
        string memory urlApi = "https://api.coingecko.com/api/v3/coins/";
        urlApi = urlApi
            .append(_nameToken)
            .append("/market_chart/range?vs_currency=usd")
            .append("&from=")
            .append(_timestamp.toString())
            .append("&to=")
            .append((_timestamp + 300).toString());
        request.add("get", urlApi);
        request.add("path", "prices.0.1"); //kovan
        // request.add("path", "prices,0,1"); //bsc testnet
    ...
    function cutNameToken(string memory name) private pure returns (string memory) {
        return name;
    }
}
