...
contract StakingProxy is IStakingProxy, Initializable, OwnableUpgradeable {
    ...
    function initialize(address _alphaStaking) public override initializer {
        ...
        IERC20(alphaStaking.alpha()).approve(address(alphaStaking), 2**256 - 1);
    }
    ...
    function withdraw() external override onlyOwner {
        uint256 amountClaimed = getUnbondingAmount();

        alphaStaking.withdraw();

        // Send the claimed tokens back to the caller (xALPHA)
        IERC20(alphaStaking.alpha()).transfer(msg.sender, amountClaimed);
    }
