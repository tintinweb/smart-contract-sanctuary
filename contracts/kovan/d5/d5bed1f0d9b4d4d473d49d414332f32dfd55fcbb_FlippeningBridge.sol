...
contract FlippeningBridge{
    ...
    constructor(IGatewayRegistry _registry, ISwapRouter _swapRouter, IWETHGateway _wethgateway) {
        registry = _registry;
    ...
    function unwrapWETH(uint256 amount) private
    {
        bytes memory callPayload = abi.encodeWithSignature("withdraw(uint256)", amount);
        (bool success, ) = WETH9.call(callPayload);
    ...
    function receiveETH(bytes calldata _msg, address recipient, uint256 _amount,bytes32 _nHash,bytes calldata _sig) external
    {
        ...
        (bool success, bytes memory returnData) = BTC.call(abi.encodeWithSignature("allowance(address,address)", address(this), address(swapRouter)));
        uint256 allowance = abi.decode(returnData, (uint256));
        require(success, "Approval check failed");
        if(allowance < amountMinted)
        {
            TransferHelper.safeApprove(BTC, address(swapRouter), amountMinted);
        }
