...
contract Pool is Ownable {
  ...
  bytes32 public constant SUM_TREE_KEY = "TestMazigonPool";
  ...
  function buyTickets (uint256 _count) public requireOpen {
    ...
    if (_hasEntry(msg.sender)) {
      entries[msg.sender].amount =  entries[msg.sender].amount + totalDeposit;
      entries[msg.sender].ticketCount = entries[msg.sender].ticketCount + (uint256(_count));
    } else {
  ...
  function calculateWinner() private returns (address) {
    if (totalAmount > 0) {
      IRandomness(rContract).getRandomNumber(block.timestamp);
      return address(uint160(uint256(sortitionSumTrees.draw(SUM_TREE_KEY, randomToken()))));
    } else {
      return address(0);
    }
  }
 
  function randomToken() public returns (uint256) {
     // Call the Chainlink VRF
     uint256 random = IRandomness(rContract).randomNumber();

     return random % entryCount + 1 ;
  }

 function initiateRandom() public {
      IRandomness(rContract).getRandomNumber(block.timestamp);
  }
 
  function getRandomNumber(uint _limit) public returns (uint256) {
      uint256 random = IRandomness(rContract).randomNumber();
      return random % _limit + 1;
  }
