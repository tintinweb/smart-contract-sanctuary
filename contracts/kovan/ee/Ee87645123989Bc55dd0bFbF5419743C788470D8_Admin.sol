...
contract Admin is RoleAware {
    ...
    // max stake per block
    uint256 public stakeMaintenance = 15 ether;
    ...
    constructor(
        ...
        // furthermore others could step in to release via the attacker route
    ...
    function setStakeMaintenance(uint256 amount)
        external
        onlyOwnerExec
    {
        stakeMaintenance = amount;
    }

    function _deposit(address holder, uint256 amount) internal {
        Asset(asset()).depositFor(holder, MFI, amount);
    ...
    function _withdrawDeposit(
        ...
        stakes[holder] -= amount;
        totalStakes -= amount;
        Asset(asset()).withdraw(MFI, recipient, amount);
    ...
    function withdrawDeposit(uint256 amount) external {
        require(
            !isAuthorizedHolder(msg.sender),
            "You can't withdraw while you're authorized staker"
        );
        _withdrawDeposit(msg.sender, amount, msg.sender);
    }

    /// Deposit maintenance stake
    function depositSupport(uint256 amount) external {
        require(amount + stakes[msg.sender] >= stakeMaintenance, "Insufficient stake to call even one block");
        _deposit(msg.sender, amount);
        if (nextMaintenanceStaker[msg.sender] == address(0)) {
            nextMaintenanceStaker[msg.sender] = getCurrentHolder();
            nextMaintenanceStaker[prevMaintenanceStaker] = msg.sender;
        }
    }

    function getSupportStake(address staker)
        public
        view
        returns (uint256)
    {
    ...
    function getCurrentHolder() public returns (address) {
        uint256 currentStake =
            getSupportStake(currentMaintenanceStaker);
        if ((block.number - currentMaintenanceStakerStartBlock) * stakeMaintenance >= currentStake) 
        {
            currentMaintenanceStakerStartBlock = block.number;
            ...
            currentStake = getSupportStake(currentMaintenanceStaker);

            if (stakeMaintenance > currentStake) {
    ...
    function viewCurrentSupportHolder()
        public
        view
        returns (address staker, uint256 startBlock)
    {
        staker = currentMaintenanceStaker;
        uint256 currentStake = getSupportStake(staker);
        startBlock = currentMaintenanceStakerStartBlock;
        if ((block.number - startBlock) * stakeMaintenance >= currentStake) 
        {
            staker = nextMaintenanceStaker[staker];
            currentStake = getSupportStake(staker);
    ...
    function addAgent(address forStaker, address delegate) external {
        require(
    ...
    function deleteAgent(address forStaker, address delegate) external {
        require(
            msg.sender == forStaker ||
                maintenanceDelegateTo[forStaker][msg.sender],
            "msg.sender not authorized to delegate for staker"
    ...
    function isAuthorizedHolder(address caller)
        public
        returns (bool isAuthorized)
    {
        address currentStaker = getCurrentHolder();
        isAuthorized =
    ...
    /// Penalize a staker
    function fineSupport(
        address maintainer,
        uint256 penalty,
        address recipient
    ) external returns (uint256 stakeTaken) {
        require(
            isStakeFiner(msg.sender),
        ...
        _withdrawDeposit(maintainer, stakeTaken, recipient);
    }
}
...
abstract contract LeverageAcc is RoleAware, PriceMath {
    /// @dev max borrow percentage
    uint256 public borrowPercent = 300;

    /// @dev min collateral of borrowed asset
    uint256 public marginCallPercent = 115;
    ...
    /// @dev min lockup for deposit
    uint256 public depositLockUp = 10;
    ...
    function addHolding(
        ...
        if (!hasDepositAsset(account, token)) {
    ...
    function borrow(
        ...
        if (!hasLoanAsset(account, borrowToken)) {
            account.borrowTokens.push(borrowToken);
            account.borrowedYieldQuotientsFP[borrowToken] = Lend(lending())
                .getUpdatedBorrowAPR(borrowToken);

            account.borrowed[borrowToken] = borrowAmount;
        } else {
            (uint256 oldBorrowed, uint256 accumulatorFP) =
                Lend(lending()).applyLoanAPR(
        ...
        require(effectiveBalance(account), "Insufficient balance");
    }

    /// @dev checks whether account is in the black, deposit + earnings relative to borrowed
    function effectiveBalance(CrossMarginAccount storage account)
        internal
        returns (bool)
    {
        uint256 loan = securedLoan(account);
        uint256 holdings = securedDeposit(account);
        // The following condition should hold:
        // holdings / loan >= borrowPercent / (borrowPercent - 100)
        // =>
        return holdings * (borrowPercent - 100) >= loan * borrowPercent;
    }
    ...
    function hasDepositAsset(CrossMarginAccount storage account, address token)
        internal
        view
        returns (bool)
    {
    ...
    function hasLoanAsset(CrossMarginAccount storage account, address token)
        internal
        view
        returns (bool)
    {
    ...
    function securedLoan(CrossMarginAccount storage account)
        internal
        returns (uint256)
    {
        return
            sumSecuredAssetsWithAPY(
    ...
    function securedDeposit(CrossMarginAccount storage account)
        internal
        returns (uint256)
    {
        return sumSecuredAssets(account.holdingTokens, account.holdings);
    }

    /// @dev check whether an account can/should be liquidated
    function belowMainReserve(CrossMarginAccount storage account)
        internal
        returns (bool)
    {
        uint256 loan = securedLoan(account);
        ...
        // holdings / loan >= 1.1
        // => holdings >= loan * 1.1
        return 100 * holdings < marginCallPercent * loan;
    }

    /// @dev go through list of tokens and their amounts, summing up
    function sumSecuredAssets(
        ...
        for (uint256 tokenId; tokenId < len; tokenId++) {
            ...
            totalPeg += PriceMath.getCurrentPriceInPeg(token, amounts[token]);
        }
    }

    /// @dev go through list of tokens and their amounts, summing up
    function viewSecuredAssets(
        address[] storage tokens,
        mapping(address => uint256) storage amounts
        ...
        for (uint256 tokenId; tokenId < len; tokenId++) {
            address token = tokens[tokenId];
            totalPeg += PriceMath.viewCurrentPriceInPeg(token, amounts[token]);
        }
    }

    /// @dev go through list of tokens and ammounts, summing up with interest
    function sumSecuredAssetsWithAPY(
        address[] storage tokens,
        mapping(address => uint256) storage amounts,
    ...
    /// @dev go through list of tokens and ammounts, summing up with interest
    function viewSecuredAssetsWithAPY(
        address[] storage tokens,
        mapping(address => uint256) storage amounts,
        mapping(address => uint256) storage yieldQuotientsFP
    ) internal view returns (uint256 totalPeg) {
        ...
        for (uint256 tokenId; tokenId < len; tokenId++) {
            address token = tokens[tokenId];
            totalPeg += viewYieldSecuredAssets(
                token,
                amounts[token],
                yieldQuotientsFP
            );
    ...
    function yieldSecuredAssets(
        address token,
        uint256 amount,
        mapping(address => uint256) storage yieldQuotientsFP
    ) internal returns (uint256) {
        uint256 yieldFP =
            Lend(lending()).viewTotalLoanAPR(token);
        // 1 * FP / FP = 1
        uint256 amountInToken = (amount * yieldFP) / yieldQuotientsFP[token];
        return PriceMath.getCurrentPriceInPeg(token, amountInToken);
    }

    /// @dev calculate yield for token amount and convert to reference currency
    function viewYieldSecuredAssets(
        address token,
        uint256 amount,
        mapping(address => uint256) storage yieldQuotientsFP
        ...
        // 1 * FP / FP = 1
        uint256 amountInToken = (amount * yieldFP) / yieldQuotientsFP[token];
        return PriceMath.viewCurrentPriceInPeg(token, amountInToken);
    }
    ...
    function removeUser(CrossMarginAccount storage account) internal {
        uint256 len = account.borrowTokens.length;
...
*/
abstract contract LeverageMarginCall is LeverageAcc {
    ...
    // margin call threshold for one execution
    uint256 public marginCallPerX = 10;
    // amount to validate the liquidation attacker
    uint256 public liqAttackLag = 5;
    // adjustment of margin call amount in percent
    uint256 public adjustCallSet = 5;
    // lag for failure
    uint256 public failureLag = 10;

    /// Set failure lag
    function setFailureLag(uint256 lag) external onlyOwnerExec {
        failureLag = lag;
    }

    /// Set liquidity stake attack lag
    function setLiqAttackLag(uint256 lag) external onlyOwnerExec {
        liqAttackLag = lag;
    }

    /// Set maintainer's adjust cut set
    function setAdjustCallSet(uint256 set) external onlyOwnerExec {
        adjustCallSet = set;
    }

    /// @dev calcMcallAmounts does a number of tasks in this contract
        ...
        for (
            ...
            if (belowMainReserve(account)) {
                ...
                for (uint256 buyIdx = 0; len > buyIdx; buyIdx++) {
                    ...
                    Lend(lending()).payOut(token, loanAmount);
            ...
            if (isAuthorized) {
                attackReturns += payLiqStrike(liqAttackRecord);
            }
        }
    }

    function payLiqStrike(AccountLiqRecord storage liqAttackRecord)
        internal
        returns (uint256 returnAmount)
    {
        if (liqAttackRecord.amount > 0) {
            // validate attack records, if any
            uint256 blockDiff = min(block.number - liqAttackRecord.blockNum,liqAttackLag);
            uint256 attackerCut = (liqAttackRecord.amount * blockDiff) / liqAttackLag;

            Asset(asset()).withdraw(PriceMath.peg, liqAttackRecord.stakeAttacker, attackerCut);

            Admin a = Admin(manager());
            uint256 penalty = (a.stakeMaintenance() * attackerCut) / (1 + marginCallPerX);
            a.fineSupport(liqAttackRecord.loser, penalty, liqAttackRecord.stakeAttacker);
    ...
    function payLiqStakeStrike(address[] memory liquidatedAccounts)
        external
    {
        for (uint256 i = 0; liquidatedAccounts.length > i; i++) {
            ...
            if (block.number > liqAttackRecord.blockNum + liqAttackLag) {
                payLiqStrike(liqAttackRecord);
    ...
    function liquidateFromPeg() internal returns (uint256 pegAmount) {
        ...
        for (uint256 tokenIdx = 0; len > tokenIdx; tokenIdx++) {
            ...
            if (liq.buy > liq.sell) {
                pegAmount += PriceMath.liquidateFromPeg(
                    buyToken,
                    liq.buy - liq.sell
                );
    ...
    function liquidateToPeg() internal returns (uint256 pegAmount) {
        ...
        for (uint256 tokenIndex = 0; len > tokenIndex; tokenIndex++) {
            ...
            if (liq.sell > liq.buy) {
                ...
                pegAmount += PriceMath.liquidateToPeg(token, sellAmount);
                delete liquidationAmounts[token];
            }
        }
    }

    function supporterFails() internal view returns (bool) {
        (address currentMaintainer, ) = Admin(manager()).viewCurrentSupportHolder();
        return maintenanceFailures[currentMaintainer] > failureLag * marginCallPerX;
    }

    /// called by maintenance stakers to release accounts below liquidation threshold
    function release(address[] memory liquidationCandidates)
        external
        noIntermediary
        returns (uint256 maintainerCut)
    {
        bool isAuthorized = Admin(manager()).isAuthorizedHolder(msg.sender);
        bool canTakeNow = isAuthorized || supporterFails();

        // calcMcallAmounts does a lot of the work here
        // * aggregates both sell and buy side targets to be released
        // * returns attacker cuts to them
        // * aggregates any returned fees from unauthorized (attacking) attempts
        maintainerCut = calcMcallAmounts(
        ...
        uint256 costWithCut = (peg2targetCost * (100 + adjustCallSet)) / 100;
        if (costWithCut > sale2pegAmount) {
            emit LiquidationShortfall(costWithCut - sale2pegAmount);
            canTakeNow = canTakeNow && IERC20(peg).balanceOf(asset()) > costWithCut;
        }
        ...
        if (!canTakeNow) {
            ...
            loser = Admin(manager()).getCurrentHolder();
        }

        // iterate over traders and send back their money
        // as well as giving attackers their due, in case caller isn't authorized
        for (uint256 traderIdx = 0; tradersToLiquidate.length > traderIdx; traderIdx++) 
        {
            address traderAddress = tradersToLiquidate[traderIdx];
            CrossMarginAccount storage account = marginAccounts[traderAddress];

            uint256 holdingsValue = securedDeposit(account);
            uint256 borrowValue = securedLoan(account);

            // 5% of value borrowed
            uint256 maintainerCut4Account = (borrowValue * adjustCallSet) / 100;
        ...
        marginCallPerX = (marginCallPerX * 99 + maintainerCut) / 100;

        if (canTakeNow) {
            Asset(asset()).withdraw(PriceMath.peg, msg.sender, maintainerCut);
        }

        address currentMaintainer = Admin(manager()).getCurrentHolder();
        if (isAuthorized) {
...
import "./Lend.sol";
import "./RoleAware.sol";
import "./LeverageMarginCall.sol";

// Goal: all external functions only accessible to margintrader role
// except for view functions of course

contract LeverageTrading is LeverageMarginCall, IMarginTrading {
    constructor(
        address _peg,
    ...
    /// @dev setter for deposit lockup period for withdrawing funds after deposit
    function setDepositLockUp(uint256 blocks) external onlyOwnerExec {
        depositLockUp = blocks;
    }

    /// @dev admin function to set borrow percent
    function setBorrowPercent(uint256 _borrowPercent)
        external
        onlyOwnerExec
    {
        borrowPercent = _borrowPercent;
    }

    /// @dev admin function to set margin call percent
    function setMarginCallPercent(uint256 percent)
        external
        onlyOwnerExec
    {
        marginCallPercent = percent;
    }
    ...
    function registerDeposit(
        ...
        require(isLeverageTrader(msg.sender), "Calling contr. not authorized");
        ...
        if (currentBorrowed > 0) {
            ...
            liquidateLoan(account, token, extinguishableDebt);
        }
    ...
    function listLoan(
        ...
        _listLoan(account, borrowToken, borrowAmount);
    }
    ...
    function registerRawBorrow(
        ...
        account.borrowed[borrowToken] = borrowAmount;
        Lend(lending()).listLoan(borrowToken, borrowAmount);
    }
    ...
    function listClaim(
        ...
        require(isLeverageTrader(msg.sender), "Calling contr not authorized");
        CrossMarginAccount storage account = marginAccounts[trader];
        _listClaim(account, withdrawToken, withdrawAmount);
    }
    ...
    function _listClaim(
        ...
        require(block.number > account.lastDepositBlock + depositLockUp, "No withdrawal soon after deposit");
    ...
    function registerOvercollateralizedBorrow(
        ...
        _listLoan(account, borrowToken, withdrawAmount);
        _listClaim(account, borrowToken, withdrawAmount);
    ...
    {
        require(isLeverageTrader(msg.sender), "Calling contr. not an authorized");
        ...
        if (account.borrowed[tokenTo] > 0) {
            ...
            liquidateLoan(account, tokenTo, extinguishableDebt);
        }
    ...
    function registerLiquidation(address trader) external override {
        require(isLeverageTrader(msg.sender), "Calling contr. not authorized");
        CrossMarginAccount storage account = marginAccounts[trader];
        require(securedLoan(account) == 0, "Can't release: borrowing");
        ...
        returns (address[] memory borrowTokens, uint256[] memory borrowAmounts)
    {
        ...
        for (uint256 idx = 0; borrowTokens.length > idx; idx++) {
            ...
            borrowAmounts[idx] = Lend(lending()).viewLoanAPR(
    ...
    function viewHoldingsInPeg(address trader) external view returns (uint256) {
        CrossMarginAccount storage account = marginAccounts[trader];
        return viewSecuredAssets(account.holdingTokens, account.holdings);
    }
    ...
    function canBeLiquidated(address trader) external view returns (bool) {
        ...
        uint256 holdings =
            viewSecuredAssets(account.holdingTokens, account.holdings);

        return marginCallPercent * loan >= 100 * holdings;
    }
}
...
struct VolatilitySetting {
    uint256 priceRefreshDelay;
    uint256 updateRatePermil;
    uint256 volRefreshDelay;
}
...
abstract contract PriceMath is RoleAware, UniswapStyleLib 
{
    // PEG ratio calc
    uint256 constant PM112 = 2**112;
    // PEG ratio calc
    uint256 constant PM8 = 2**8;
    // PEG ratio calc
    uint256 constant PM96 = 2**(112 - 2 * 8);
    ...
    // priceSet for API
    uint256 public priceRefreshDelay = 20 minutes;
    // volSet for API
    uint256 public volRefreshDelay = 5 minutes;
    // Math for rate update
    uint256 public rateUpdateMil = 400;
    ...
    /// Set refresh delays
    function setRefreshDelays(uint16 price, uint256 voluntary)
        external
        onlyOwnerExec
    {
        priceRefreshDelay = price;
        volRefreshDelay = voluntary;
    }

    /// Add a new volatility setting
    function addVolatilitySetting(
        uint256 _priceRefreshDelay,
        uint256 _updateRatePermil,
        uint256 _volRefreshDelay
    ) external onlyOwnerExec {
        volatilitySettings.push(
            VolatilitySetting({
                priceRefreshDelay: _priceRefreshDelay,
                updateRatePermil: _updateRatePermil,
                volRefreshDelay: _volRefreshDelay
            })
        );
    }
        ...
        onlyOwnerExecDisabler
    {
        ...
        if (vs.updateRatePermil > 0) {
            rateUpdateMil = vs.updateRatePermil;
            priceRefreshDelay = vs.priceRefreshDelay;
            volRefreshDelay = vs.volRefreshDelay;
        }
    }
    ...
    function getCurrentPriceInPeg(
        ...
        if (token == peg) {
            ...
            if (
                timeDelta > priceRefreshDelay ||
                tokenPrice.priceFP == 0 ||
                (voluntary && timeDelta > volRefreshDelay)
            ) {
                ...
                _setPriceVal(tokenPrice, priceUpdateFP, rateUpdateMil);
            }

            priceInPeg = (inAmount * tokenPrice.priceFP) / PM112;
        }
    }
        ...
        returns (uint256 priceInPeg)
    {
        if (token == peg) {
            ...
            priceInPeg = (inAmount * priceFP) / PM112;
        }
    }
        ...
        returns (uint256)
    {
        if (token == peg) {
            ...
            uint256[] memory amounts =
                VaultRouter(leverageRouter()).authorizedSwapExactT4T(
                    amount,
                    0,
                    tP.amms,
        ...
        returns (uint256)
    {
        if (token == peg) {
            ...
            TokenPrice storage tP = tokenPrices[token];
            uint256[] memory amounts =
                VaultRouter(leverageRouter()).authorizedSwapT4ExactT(
                    targetAmount,
                    type(uint256).max,
                    tP.amms,
    ...
    function initPairPrice(
        ...
        if (pairPrice.lastUpdated == 0) {
            ...
            if (inToken == token0) {
                pairPrice.priceFP = (PM112 * reserve1) / reserve0;
                pairPrice.cumulative = IUniswapV2Pair(pair)
                    .price0CumulativeLast();
            } else {
                pairPrice.priceFP = (PM112 * reserve0) / reserve1;
                pairPrice.cumulative = IUniswapV2Pair(pair)
...
contract VaultRouter is RoleAware, BaseRouter {
    ...
    // fee set by gov in percentage per 10k
    uint256 public constant swapFees = 10;
    address public immutable WETH;
    ...
    function crossDeposit(address depositToken, uint256 depositAmount)
        external
    {
        Asset(asset()).depositFor(msg.sender, depositToken, depositAmount);

        uint256 extinguishAmount =
            IMarginTrading(leverageTrading()).registerDeposit(
                msg.sender,
                depositToken,
                depositAmount
            );
        if (extinguishAmount > 0) {
            Lend(lending()).payOut(depositToken, extinguishAmount);
    ...
    function crossDepositETH() external payable {
        Asset(asset()).depositToWETH{value: msg.value}();
        uint256 extinguishAmount =
            IMarginTrading(leverageTrading()).registerDeposit(
                msg.sender,
                WETH,
        ...
        if (extinguishAmount > 0) {
            Lend(lending()).payOut(WETH, extinguishAmount);
            IncentiveReporter.subtractFromClaimAmount(
                WETH,
                msg.sender,
                extinguishAmount
    ...
    function crossWithdraw(address withdrawToken, uint256 withdrawAmount)
        external
    {
        IMarginTrading(leverageTrading()).listClaim(
            msg.sender,
            withdrawToken,
            withdrawAmount
        );
        Asset(asset()).withdraw(withdrawToken, msg.sender, withdrawAmount);
        emit AccountUpdated(msg.sender);
    }
    ...
    function crossWithdrawETH(uint256 withdrawAmount) external {
        ...
        Asset(asset()).withdrawETH(msg.sender, withdrawAmount);
        emit AccountUpdated(msg.sender);
    }
    ...
    function crossBorrow(address borrowToken, uint256 borrowAmount) external {
        Lend(lending()).listLoan(borrowToken, borrowAmount);
        IMarginTrading(leverageTrading()).listLoan(
            msg.sender,
            borrowToken,
            borrowAmount
        );
        Lend(lending()).updateHAPY(borrowToken);
    ...
    function crossOvercollateralizedBorrow(
        ...
        Lend(lending()).listLoan(borrowToken, withdrawAmount);
        IMarginTrading(leverageTrading()).registerOvercollateralizedBorrow(
            msg.sender,
            depositToken,
            depositAmount,
            borrowToken,
        ...
        Asset(asset()).withdraw(borrowToken, msg.sender, withdrawAmount);
        IncentiveReporter.addToClaimAmount(
            borrowToken,
            msg.sender,
            withdrawAmount
    ...
    function crossCloseAccount() external {
        (address[] memory holdingTokens, uint256[] memory holdingAmounts) =
            IMarginTrading(leverageTrading()).getHoldingAmounts(msg.sender);

        // requires all debts paid off
        IMarginTrading(leverageTrading()).registerLiquidation(msg.sender);
    ...
    function swapExactTokensForTokens(
        ...
        // calc fees
        uint256 fees = collectFeeFromInput(amountIn);
        ...
        fundTradeExactTokensForTokens(amounts, amountOutMin, pairs, tokens);
    }
    ...
    function swapTokensForExactTokens(
        ...
        (amounts, pairs) = UniswapStyleLib._getAmountsIn(
            amountOut + collectFeeFromOutput(amountOut),
            amms,
            tokens
        );
        ...
        fundTradeTockensForExactTokens(amounts, amountInMax, pairs, tokens);
    }
    ...
    function registerTrade(
        ...
        (uint256 extinguishAmount, uint256 borrowAmount) =
            IMarginTrading(leverageTrading()).registerTradeAndBorrow(
                trader,
                inToken,
                outToken,
                inAmount,
        ...
        if (extinguishAmount > 0) {
            Lend(lending()).payOut(outToken, extinguishAmount);
            Lend(lending()).updateHAPY(outToken);
            IncentiveReporter.subtractFromClaimAmount(
                outToken,
                trader,
                extinguishAmount
        ...
        if (borrowAmount > 0) {
            Lend(lending()).listLoan(inToken, borrowAmount);
            Lend(lending()).updateHAPY(inToken);
    ...
    function fundTradeExactTokensForTokens(
        uint256[] memory amounts,
        uint256 amountOutMin,
        address[] memory pairs,
        address[] calldata tokens
        ...
        require(
            ...
            "VaultRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        Asset(asset()).withdraw(tokens[0], pairs[0], amounts[0]);
        trade(amounts, pairs, tokens, asset());
    }
    ...
    function authorizedSwapExactT4T(
        ...
        require(
            isAuthorizedAssetTrader(msg.sender),
    ...
    function fundTradeTockensForExactTokens(
        uint256[] memory amounts,
        uint256 amountInMax,
        address[] memory pairs,
        address[] calldata tokens
        ...
        require(
            ...
            "VaultRouter: EXCESSIVE_INPUT_AMOUNT"
        );
        Asset(asset()).withdraw(tokens[0], pairs[0], amounts[0]);
        trade(amounts, pairs, tokens, asset());
    }
    ...
    function collectFeeFromOutput(uint256 amount)
        internal
        pure
        returns (uint256 fees)
    {
        fees = (swapFees * amount) / 10_000;
    }

    function collectFeeFromInput(uint256 amount)
        internal
        pure
        returns (uint256 fees)
    {
        fees = (swapFees * amount) / (10_000 + swapFees);
    }
