...
contract ActivityBase is Ownable{
    ...
    function setTreasury(address _treasury) public virtual onlyOwner {
        treasury = _treasury;
    }
    ...
    function calculateDividend(uint256 _pending, uint256 _pid, uint256 _userAmount, bool _isContractSender) internal view returns (uint256 _treasuryDividend, uint256 _devDividend, uint256 _userDividend){
        uint256 fee = 0;
        if(_isContractSender && contractFeeRateDenominator > 0){
            ...
            _treasuryDividend = _treasuryDividend.add(fee);
...
contract MarketingMining is ActivityBase{
    ...
    function updateAfterwithdraw(uint256 _pid, uint256 _amount) internal {
        ...
        (uint256 treasuryDividend, uint256 devDividend, uint256 userDividend) = calculateDividend(pending, _pid, existedAmount, isContractSender);
        user.userDividend = user.userDividend.add(userDividend);
        user.devDividend = user.devDividend.add(devDividend);
        if(treasuryDividend > 0){
            user.treasuryDividend = user.treasuryDividend.add(treasuryDividend);
        }
        if(isRevenueWithdrawable){
            devDividend = user.devDividend;
            userDividend = user.userDividend;
            treasuryDividend = user.treasuryDividend;
            if(devDividend > 0){
            ...
            if(treasuryDividend > 0){
                safeSHARDTransfer(treasury, treasuryDividend);
            }
            user.devDividend = 0;
            user.userDividend = 0;
            user.treasuryDividend = 0;
        }
