...
contract CompetitionPool is Ownable {
    ...
    struct BetSLip {
        IRegularCompetitionContract regularContract;
    ...
    modifier onlyExistedPool(address _pool) {
        require(existed[_pool], "CompetitionPool: Pool not found");
        _;
    }
    ...
    function setRewardRates(
        ...
        for (uint256 i = 0; i < bracketsAmountToken.length; i++) {
            ...
            setRewardRate(
                ...
                _rewardRateOfcreator[i],
                _rewardRateOfOwner[i],
                100 - _rewardRateOfcreator[i] - _rewardRateOfOwner[i]
            );
        }
        emit RewardRate(
    ...
    function betSlip(BetSLip[] memory _betSlipList) public payable {
        ...
        for (uint256 i = 0; i < _betSlipList.length; i++) {
            totalFee +=
                IRegularCompetitionContract(_betSlipList[i].regularContract)
                    .getEntryFee() *
        ...
        for (uint256 i = 0; i < _betSlipList.length; i++) {
            IRegularCompetitionContract regularContract = IRegularCompetitionContract(
                    _betSlipList[i].regularContract
                );

            require(
                existed[address(regularContract)],
                "CompetitionPool: Pool not found"
            );
            regularContract.placeBet{
                value: regularContract.getEntryFee() *
                    (_betSlipList[i].numberOfVotesPlayer1 +
                        _betSlipList[i].numberOfVotesPlayer2)
            }(
                msg.sender,
    ...
    function createNewRegularCompetition(
        ...
        address competitionContract = competitionFactory
            .createCompetitionCOntract(address(this), msg.sender);
        pools.push(competitionContract);
        existed[competitionContract] = true;
        ...
        ) = getCompetitionById(_competitionId);
    ...
    function depositeForRegularContract(address _regular)
        public
        payable
        onlyExistedPool(_regular)
        onlyBettingOwner(_regular)
    {
        require(msg.value > 0, "CompetitionPool: Please send value >0");
        IRegularCompetitionContract(_regular).depositEth{value: msg.value}();
    }
    ...
    function close(address _pool) public onlyExistedPool(_pool) {
        IRegularCompetitionContract(_pool).close();
    }
    ...
    function _setRateforCreator(address _pool) private {
        ...
        IRegularCompetitionContract(_pool).setRewardRate(rewardRateCopy);
    }
    ...
    function getCompetitionById(uint256 _competitionId)
        public
        returns (
            string memory _matchId,
            string memory _leagueId,
    ...
    {
        bytes32 requestId = IChainLinkOracleSportData(oracle)
            .requestInfoCompetition(_competitionId);
        _matchId = IChainLinkOracleSportData(oracle)
            .getInfoCompetition(requestId).matchId;
        _leagueId = IChainLinkOracleSportData(oracle)
            .getInfoCompetition(requestId).leagueId;
        _sportType = IChainLinkOracleSportData(oracle)
            .getInfoCompetition(requestId)
            .sportType;
        _matchName = IChainLinkOracleSportData(oracle)
            .getInfoCompetition(requestId)
            .matchName;
        _player1 = IChainLinkOracleSportData(oracle)
            .getInfoCompetition(requestId)
            .player1;
        _player2 = IChainLinkOracleSportData(oracle)
            .getInfoCompetition(requestId)
            .player2;
        return (_matchId, _leagueId, _sportType, _matchName, _player1, _player2);
        // return ("a", "b", "c", "d");
    }
