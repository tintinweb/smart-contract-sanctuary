    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(address payable _feeRecipient, uint256 _platformFee, IERC20 _RWT)
        public
        initializer
    {
    ...
    function _buyItem(string memory _bundleID, address _payToken) private {
        ...
        if (_payToken == address(0)) {
            ...
            RWT.safeTransferFrom(rewardsender, owner, price.sub(feeAmount).mul(5).div(100));
    ...
    /// @notice Method for set Reward Token
    /// @param _rwtbondleTokenAddr Reward Token Address

    function setbondleRewardToken(address _rwtbondleTokenAddr)
        external
        onlyOwner
    {
        RWT = IERC20(_rwtbondleTokenAddr);
    }

    /// @notice Method for set owner address
    /// @param _ownerbondleAddr New owner address

    function setbondleOwnerAddr(address _ownerbondleAddr)
        external
        onlyOwner
    {
        rewardsender = _ownerbondleAddr;
    }
