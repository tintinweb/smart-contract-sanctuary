...
contract TraderInvestor is Investor {
    ...
    constructor(
        ...
        token.approve(address(_revManager), ~uint256(1));
...
/**
 * @title Investor
 * @author Carson Case [carson[email protected]]
 * @dev TestToken will eventually be replaced with ERC-20
 */
contract Investor is TestToken, ERC20CollateralPayment {
    ...
    constructor(
        ...
        ERC20CollateralPayment(_bonds, _oracle)
    {
        token = IERC20(_token);
        bonds = IBonds(_bonds);
    }
    ...
    function borrow(
        ...
        ScoreOracleInterface(creditOracle).getLatestScore(
            _NCFSID,
            id,
            _hash,
            _signature
...
contract ERC20CollateralPayment is ERC20PaymentStandard, Ownable {
     ...
     */
    constructor(address _bonds, address _oracle)
...
interface ScoreOracleInterface {
  function getLatestScore(uint256, uint256, bytes32, bytes memory) external returns (uint16);
