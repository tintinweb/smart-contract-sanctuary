...
contract DependencyManager is RoleAware, IDependencyManager {
    ...
    function exeAsOwner(address performer) external onlyOwnerExec {
        uint256[] memory requiredRoles = Performer(performer).requiredTitle();

        for (uint256 i = 0; requiredRoles.length > i; i++) {
            _giveRole(requiredRoles[i], performer);
        }
        ...
        for (uint256 i = 0; len > i; i++) {
            _removeRole(requiredRoles[i], performer);
        }
    }
    ...
    /// Activate role
    function setRole(uint256 role, address actor) external onlyOwnerExec {
    ...
    function _setMainCharacter(uint256 character, address actor) internal {
        ...
        roles.setMainId(character, actor);
        updMainIdCache(character);
    }

    function updMainIdCache(uint256 character) public override {
        uint256 len = managedContracts.length;
        for (uint256 i = 0; len > i; i++) {
            RoleAware(managedContracts[i]).updMainIdCache(character);
        }
    }
    ...
    function updCaches(address contr) public {
        ...
        for (uint256 i = 0; len > i; i++) {
            RoleAware(contr).updMainIdCache(allCharacters[i]);
        }
