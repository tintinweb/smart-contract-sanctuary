...
contract OmakaseCore is KeeperCompatibleInterface {
    ...
    function setPolicyFlights(
        ...
        require(!s.policies[policyId].isEmpty(), Errors.OC_POLICY_EMPTY);
        require(
            !s.policies[policyId].hasFlight(),
            Errors.OC_POLICY_HAS_FLIGHT_ALREADY
        );

        _setPolicyFlights(policyId, flightData);
    }

    function updatePolicyFlights(
        bytes32 policyId,
        bytes32[] calldata previousFlightIds,
        FlightArgs[] calldata flightData
    ) external onlyOracle {
        Policy storage policy = s.policies[policyId];
        require(!policy.isEmpty(), Errors.OC_POLICY_EMPTY);

        bytes memory flightsSignature;
        for (uint256 i = 0; i < previousFlightIds.length; i++) {
            bytes32 prevFlightId = previousFlightIds[i];

            s.flights[prevFlightId].removePolicy(
                policy.idxInFlight[prevFlightId]
            );

            delete policy.idxInFlight[prevFlightId];
            emit PolicyFlightRemoved(policyId, prevFlightId);

            flightsSignature = abi.encodePacked(flightsSignature, prevFlightId);
        }

        require(
            keccak256(flightsSignature) == policy.flightsSignature,
            Errors.OC_INVALID_PREVIOUS_FLIGHTS
        );
    ...
    function _setPolicyFlights(
        ...
        for (uint256 i = 0; i < flightData.length; i++) {
            ...
            policy.idxInFlight[flightData[i].id] = flight.addPolicy(policyId);
            emit PolicyFlightAdded(policyId, flightData[i].id);

            flightsSignature = abi.encodePacked(
                flightsSignature,
                flightData[i].id
            );
        }

        policy.flightsSignature = keccak256(flightsSignature);
    }
...
library Errors {
    ...
    string constant OC_INVALID_PREVIOUS_FLIGHTS =
        "OmakaseCore: Invalid previous flights";
...
library LibPolicy {
    ...
    function hasFlight(Policy storage self) internal view returns (bool) {
        return self.flightsSignature != 0;
    }
}
