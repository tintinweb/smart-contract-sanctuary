...
contract DestinyWarCharacter is ERC721, ERC721Enumerable, Ownable {
    ...
    constructor(string memory baseURI, string memory _notrevealURI) ERC721("DestinyWarCharacter", "DWC") { 
        _baseURIextended = baseURI;
        notRevealedUri = _notrevealURI;
     }
     
    function setLevel(uint256 level) public onlyOwner{
        _level = level;
    }
  
    function random() private view returns (uint256) {
      return uint8(uint256(keccak256(abi.encodePacked(block.timestamp)))%maxSupply);
    }
    
    function addImage(string[] calldata _images) public onlyOwner {
        for (uint256 i = 0; i < _images.length; i++) {
            images.push(_images[i]);
        }
    }
    
    function removeImage(uint256 index) public onlyOwner {
    ...
    function createItem(uint256 numberOfTokens) public payable {
    ...
    function walletOfOwner(address _owner) external view returns (uint256[] memory) {
      ...
      function getItemProperty(uint256 _tokenId) external view returns (uint256 __number, uint256 __level, string memory __image) {
    ...
    function internalMint(address to) internal {
        ...
        string memory _image = images[random()];
