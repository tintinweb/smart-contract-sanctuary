...
contract StakingV2 is Ownable, ReentrancyGuard {
    ...
    uint256 private constant DAY = 1 minutes;
    uint256 private constant MONTH = 30 minutes;
    uint256 private constant YEAR = 365 minutes;
    ...
    uint256 public constant PARAM_UPDATE_DELAY = 7 minutes;
     ...
     */
    function deposit(address _sender,uint256 _depositId, uint256 _amount)
        public
        ...
        isNotPausedDepositAndLockupExtensions
    {
        ...
        _deposit(_sender, _depositId, _amount);
    ...
    function compound(
        ...
        uint256 minutesCount = _n.div(DAY);

        return
            ABDKMath64x64.mulu(
                pow(ABDKMath64x64.add(ABDKMath64x64.fromUInt(1), ABDKMath64x64.divu(_ratio, 10**18)), minutesCount),
                _principal
            );
    }
    ...
    function calcRewards(address _sender, uint256 _depositId) public view validDepositId(_depositId) returns (uint256) {
        ...
        if (_depositId == 1) {
            ratio = 800000000000000; // APY 30% InterestRate = 0.08
            lockPeriod = MONTH;
        } else if (_depositId == 2) {
            ratio = 900000000000000; // APY 35% InterestRate = 0.09
            lockPeriod = MONTH * 6; // 6 months
        } else if (_depositId == 3) {
            ratio = 1000000000000000; // APY 39% InterestRate = 0.1
            lockPeriod = MONTH * 12; // 12 months
        }
