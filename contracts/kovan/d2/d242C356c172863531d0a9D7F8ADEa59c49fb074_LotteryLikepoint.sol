...
contract LotteryLikepoint is LotteryOwnable {
    ...
    function reset() external onlyAdmin {
        ...
        for(uint256 x=0;x<3;x++){
           if(getMatchingRewardAmount(issueIndex-1, 4-x) == 0) {
            uint256 nextRewards = nextRewards.add(getTotalRewards(issueIndex-1).mul(allocation[x]).div(100));
            internalBuy(nextRewards, nullTicket);
           }
        }
    ...
    function buy(uint256 _price, uint8[4] calldata _numbers) external onlyAdmin {
        require(!drawed(), 'drawed, can not buy now');
    ...
    function  multiBuy(uint256 _price, uint8[4][] calldata _numbers) external onlyAdmin {
        require (!drawed(), 'drawed, can not buy now');
        require(!drawingPhase, 'drawing, can not buy now');
        require (_price >= minPrice, 'price must above minPrice');
    ...
    function  multiBuyLock(uint256 _price, uint8[4][] calldata _numbers) external {
        require (!drawed(), 'drawed, can not buy now');
        require(!drawingPhase, 'drawing, can not buy now');
