...
contract xSex is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function ownerBatchClaim(uint256 startTokenID) public nonReentrant onlyOwner {
        require(
            startTokenID == 10020 || startTokenID == 10070 || startTokenID == 10120 || startTokenID == 10170,
            "Token ID invalid"
        );
        for (uint256 i = startTokenID; i < startTokenID + 50; i++) {
            _safeMint(owner(), i);
        }
        sceneToken.mint(owner(), 2 *  50 * REWARDS_PER_CLAIM);
    }
