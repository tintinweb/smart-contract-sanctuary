...
contract ChainlinkPriceOracle is Ownable, ChainlinkClient {
    ...
    event FulfilledPrice(bytes32 requestId, uint256 price, uint256 timestamp);
    ...
    function priceData() external view returns (uint256 price, uint256 updateTime) {
        price = latestPrice;
    ...
    function fulfillPrice(bytes32 _requestId, uint256 _price) external recordChainlinkFulfillment(_requestId) {
    	...
    	latestPrice = _price;
        priceUpdateTime = curTimestamp;
        emit FulfilledPrice(_requestId, _price, curTimestamp);
    }
