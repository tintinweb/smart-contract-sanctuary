 ...
 */
contract PoolStakeSushi is Owned {
    //instantiate SafeMath library
    using SafeMath for uint;
    
    IERC20 internal weth;                       //represents weth.
    IERC20 internal token;                      //represents the project's token which should have a weth pair on sushiswap
    ...
    ISushiV2Router02 internal sushiswapRouter;          //Interface of sushiswap V2 router
    ISushiV2Factory internal iSushiswapV2Factory;       //Interface of sushiswap V2 factory
    ...
    struct User {
        ...
        uint liquidity;             //user liquidity gotten from sushiswap
        uint period;                //identifies users' current term period
        bool migrated;              //if migrated to sushiswap V3
    }
    ...
    //migration contract for Sushiswap V3
    ...
    constructor(address _token, address _Owner) public Owned(_Owner) {
        ...
        iSushiswapV2Factory = ISushiV2Factory(0xc35DADB65012eC5796536bD9864eD8773aBc74C4);
        sushiswapRouter = ISushiV2Router02(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506);   
        weth = IERC20(sushiswapRouter.WETH());
        address _lpToken = iSushiswapV2Factory.getPair(address(token), address(weth));
        require(_lpToken != address(0), "Pair must first be created on sushiswap");
    ...
    /* 
     * @dev makes migration possible for sushiswap V3 (only Owner).
     * ----------------------------------------------------------
    ...
    function addLiquidity(uint _term, uint _multiplier) external payable {
        ...
        token.approve(address(sushiswapRouter), rate);

        (uint amountToken, uint amountETH, uint liquidity) = 
        sushiswapRouter.addLiquidityETH{value: msg.value}(
            address(token), 
    ...
    function withdrawLiquidity(uint _lpAmount) external returns(bool) {
        ...
        lpToken.approve(address(sushiswapRouter), _lpAmount);                                         
    ...
    function _proportion(uint _amount, address _tokenA, address _tokenB) internal view returns(uint tokenBAmount) {
        ...
        (uint reserveA, uint reserveB) = SushiswapV2Library.getReserves(address(iSushiswapV2Factory), _tokenA, _tokenB);
        
        return SushiswapV2Library.quote(_amount, reserveA, reserveB);
    }
