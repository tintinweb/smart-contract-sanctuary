...
contract OracleTWAP {
    ...
    constructor(address factory, address mainToken, address _stableToken) public {
        IUniswapV2Pair _pair = IUniswapV2Pair(UniswapV2Library.pairFor(factory, mainToken, _stableToken));
        pair = _pair;
    ...
    function getData() external returns (uint amountOut, bool status) {
        update();
        uint price = 10**18;
        if (token0 != stableToken) {
            price = price0Average.mul(10**18).decode144();
        } else {
            require(token1 != stableToken, 'OraclePrice: INVALID_TOKEN');
