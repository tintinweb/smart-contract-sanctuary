...
contract MigrateResolver is MigrateHelpers {
    ...
    function createAndMigrate(MigrationData calldata data) external payable {
        ...
        emit LogCreateAndMigrate(address(this), newDsa, data.targets, data.calldatas);
    }
    ...
    function migrate(
        ...
        dsaContract.cast{value: 0}(targets, calldatas, address(0));

        emit LogCreateAndMigrate(address(this), newDsa, targets, calldatas);
    }
}
...
contract ConnectV2Migrate is MigrateResolver {
    ...
    */
    function connectorID() public pure returns(uint model, uint id) {
        (model, id) = (1, 80); // Replace this
    }

    string public constant name = "v1-to-v2-migrate-v1";
}
