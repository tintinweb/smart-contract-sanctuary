...
contract AutoLiquidity is UniV3Liquidity {
    ...
    function withdraw(address outputToken,address to, uint256 amount, uint256 scale) external onlyVault {
        ...
        if (surplusAmount < amount) {
            ...
            for (uint256 i = 0; i < underlyings.length(); i++) {
                ...
                uint256 swapAmount=balance.mul(scale).div(1e18);
                if (token != outputToken && swapAmount > 0) {
                    exactInput(token, outputToken, swapAmount, 0);
                }
            }
        }
