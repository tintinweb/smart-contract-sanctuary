...
def updateRewards():
    ...
    # totalToPaid: uint256 = (self.incentives - self.incentives * 98)/100
...
@view
@external
def viewRewards(_block: uint256) -> uint256:
    return (self.incentives - self.incentives * 98 ** (_block - self.lastBlock)) / (100**(_block- self.lastBlock))
...
@view
@external
def bal(user: address, _block: uint256) -> (uint256, uint256):
    totalToPaid: uint256 = (self.incentives - self.incentives * 98 ** (_block - self.lastBlock)) / (100**(_block - self.lastBlock))
