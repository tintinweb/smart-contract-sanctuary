...
{
    function getHash(address user, uint256 amountWithFee, bytes32 originalTxHash) internal pure returns (bytes32)
    {
...
contract swapContract is AccessControl, Pausable, ECDSAOffsetRecovery
{
        ...
        onlyRelayer
    {
        ...
        require(
            concatSignatures.length.div(SIGNATURE_LENGTH) >= minConfirmations,
    ...
    function setMinConfirmations(uint256 _minConfirmations) external onlyOwner {
        require(_minConfirmations > 0, "swapContract: At least 1 confirmation can be set");
        minConfirmations = _minConfirmations;
    }
