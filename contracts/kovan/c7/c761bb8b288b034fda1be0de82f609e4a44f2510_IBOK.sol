...
contract IBOK is IERC20 {
    ...
    uint8 constant public decimals = 18;       //发放质押通行证
    string constant public name = "IBOK";
    string constant public symbol = "IBOK";
    ...
    //Displays the current address contract
    function getAddrs() public view returns(address, address) {
      return (administrator,addrToken);
    }
    ...
    function  transfer(address to,  uint amount)  public returns (bool){
        DOC token = DOC(addrToken);
        require(token.balanceOf(this) >= amount,"not sufficient funds1");
        require((_tokenWK[msg.sender]>0)&&(amount>0),"not sufficient funds2");
        require(_tokenWK[msg.sender]>=amount,"not sufficient funds3");
    ...
    function balanceOf(address _owner) public view returns (uint) {
        if(_tokenWK[_owner]<=0){
            return 0;
        }else{
            return _tokenWK[_owner];
        }
    }
    
    
    //total balance
    function balanceTotal(address _owner) public view returns (uint) {
        DOC token = DOC(addrToken);
