...
interface IDsaRewardDistributor {
	function sendCheeseTo(address _EOA, address _account) external;
}
...
contract DsaRewardDistributor is Ownable, WhiteList, IDsaRewardDistributor {
	...
	event SendCheeseTo(address EOA, address account, uint256 newAccountRewardAmount);
	...
	function sendCheeseTo(address _EOA, address _account) external override onlyWhitelisted {
		require(!isAccountExist[_account], 'DsaRewardDistributor: account already exist!');
		isAccountExist[_account] = true;
		address(cheeseToken).safeTransfer(_EOA, newAccountRewardAmount);
		emit SendCheeseTo(_EOA, _account, newAccountRewardAmount);
	}
