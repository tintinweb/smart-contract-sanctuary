...
pragma solidity ^0.6.12;
    ...
    contract BoostToken is Context, IERC20, Ownable {
        ...
        //Total Fee
        uint256 private _totalFee = (_marketingFee + _buyBackFee + _devFee + _useFee);
        ...
        constructor (address payable devWalletAddress, address payable marketingWalletAddress, address payable buyBackWalletAddress, address payable useCaseWalletAddress) public {
        ...
        function sendETHToTeam(uint256 amount) private {
            ...
            _buyBackWalletAddress.transfer(amount.mul(_buyBackFee/_totalFee));
        ...
        function _transferStandard(address sender, address recipient, uint256 tAmount) private {
            (uint256 rAmount, uint256 rTransferAmount, uint256 rAll, uint256 tTransferAmount, uint256 tAll) = _getValues(tAmount);
            _rOwned[sender] = _rOwned[sender].sub(rAmount);
        ...
        function _takeFees(uint256 tAll) private {
            uint256 currentRate =  _getRate();
            uint256 rAll = tAll.mul(currentRate);
            _rOwned[address(this)] = _rOwned[address(this)].add(rAll);
            if(_isExcluded[address(this)])
                _tOwned[address(this)] = _tOwned[address(this)].add(rAll);
        }

        //Replace
        function _reflectFee(uint256 rAll, uint256 tAll) private {
            //Replace
            //_rTotal = _rTotal.sub(rFee);
            //_tFeeTotal = _tFeeTotal.add(tFee);
            _rTotal = _rTotal.sub(rAll);
            _tFeeTotal = _tFeeTotal.add(tAll);
        }
        ...
        function _seMarketingFee(uint256 marketingFee) external onlyOwner() {
        ...
        function _setBuyBackFee(uint256 buyBackFee) external onlyOwner() {
            require(buyBackFee >= 1 && buyBackFee <= 5, 'buyBackFee should be in 1 - 5');
        ...
        function _setUseFee(uint256 useFee) external onlyOwner() {
            require(useFee >= 1 && useFee <= 5, 'useFee should be in 1 - 5');
            _useFee = useFee;
        }
        ...
        function _setUseCaseWalletAddress(address payable useCaseWalletAddress) external onlyOwner() {
            _useCaseWalletAddress = useCaseWalletAddress;
        }
