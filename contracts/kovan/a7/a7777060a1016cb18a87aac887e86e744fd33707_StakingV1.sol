...
contract StakingV1 is ReentrancyGuard, Ownable {
    ...
    function inWeight(uint _s, uint _p, uint _amt) internal returns (uint weight) {
        ...
        weight = _amt.mul(1 - ((block.timestamp - p.start).mul(1e6).div(periodEpoc))).div(1e6); //if this is at the beginning then its = 1 otherwise its proportional to when they started
    ...
    function outWeight(uint _s, uint _p, uint _amt) internal returns (uint weight) {
        ...
        uint minTime = periodTime.min(totalTime).mul(1e6).div(periodEpoc);
        weight = _amt.mul(1 - minTime).div(1e6);
        st.periodTokens[_p] -= weight;
        p.totalStaked -= weight;
    }
