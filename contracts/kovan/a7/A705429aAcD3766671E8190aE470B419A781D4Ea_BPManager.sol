...
interface IBPContract {
    ...
    function getFastestValidUntil() external  view returns (uint256);
    function getBlackListValidUntil() external view returns (uint256);
    ...
    function getFastestBuyer() external returns (address[] memory);
...
contract BPManager is AccessControl{
    bytes32 public constant BOT_MANAGEMENT_RULE = keccak256("BOT_MANAGEMENT_RULE");
    ...
    constructor (IBPContract botProtectionContract, address owner) {
        ...
        _setupRole(BOT_MANAGEMENT_RULE, owner);
        _botProtectionContract = botProtectionContract;
                
    }
    
    
    function setLpPair(address lpPair) external onlyRole(BOT_MANAGEMENT_RULE){
        _botProtectionContract.setLpPair(lpPair);
    }
    // these senders will not be able to sell
    function manualBlackList(address addressToBlacklist) external onlyRole(BOT_MANAGEMENT_RULE) {
        _botProtectionContract.manualBlackList(addressToBlacklist);
    }
    
    function removeFromBlackList(address addressToRemove) external onlyRole(BOT_MANAGEMENT_RULE){
       _botProtectionContract.removeFromBlackList(addressToRemove); 
    }
    
    function setBuyCoolDown(uint256 buyCoolDown) external onlyRole(BOT_MANAGEMENT_RULE){
        _botProtectionContract.setBuyCoolDown(buyCoolDown);
    }
    
    function setSellCoolDown(uint256 sellCooldown) external onlyRole(BOT_MANAGEMENT_RULE){
        _botProtectionContract.setSellCoolDown(sellCooldown);
    }
    
    function setMaxBuyAmount (uint256 maxBuyAmount) external onlyRole(BOT_MANAGEMENT_RULE){
        _botProtectionContract.setMaxBuyAmount(maxBuyAmount);
        
    }
    
    function setMaxSellAmount(uint256 maxSellAmount) external onlyRole(BOT_MANAGEMENT_RULE){
        _botProtectionContract.setMaxSellAmount(maxSellAmount);
    }
    
    function excludeFromTransferLimitations (address addressToExclude) external onlyRole(BOT_MANAGEMENT_RULE){
        _botProtectionContract.excludeFromTransferLimitations(addressToExclude);
    }
    
    function includedAddressesFromTransferLimitations (address addressToRemove) external onlyRole(BOT_MANAGEMENT_RULE){
        _botProtectionContract.includedAddressesFromTransferLimitations(addressToRemove);
    }
    

    function getFastestBuyer() external returns (address[] memory) {
        return _botProtectionContract.getFastestBuyer();
    } 
    
   
    function getFastestBuyersValidUntil() external view returns (uint256) {
        return _botProtectionContract.listingTimestamp() + _botProtectionContract._fastestBuyersTimeframe();
    }
    
    function getBlackListValidUntil() external view returns (uint256) {
        return _botProtectionContract.listingTimestamp() + _botProtectionContract._blacklistTimeframe();
    } 
    function getMaxSellAmount() external view onlyRole(BOT_MANAGEMENT_RULE) returns (uint256){
        return _botProtectionContract._maxSellAmount();
    }
    
    function getMaxBuyAmount() external view onlyRole(BOT_MANAGEMENT_RULE) returns (uint256){
        return _botProtectionContract._maxBuyAmount();
    }
    
        
    function getSellCooldown() external view onlyRole(BOT_MANAGEMENT_RULE) returns (uint256){
        return _botProtectionContract._sellCooldown();
    }
    
    function getBuyCooldown() external view onlyRole(BOT_MANAGEMENT_RULE) returns (uint256){
