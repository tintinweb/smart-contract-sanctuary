...
// File: contracts\marketplaceProxyFactory.sol
...
//   function createThing(bytes memory _data) public returns (address){
//     address proxy = deployMinimal(implementationContract, _data);
//     return proxy;
//   }
  
  function createClone(address _owner) public returns (address){
      require(msg.sender == admin, "Only admin can clone contract");
      address clone = createClone(implementationContract); //create smart contract clone
      
      PlaytreksMarketplaceProxy(clone).initialize(_owner);
      return clone;
  }
}