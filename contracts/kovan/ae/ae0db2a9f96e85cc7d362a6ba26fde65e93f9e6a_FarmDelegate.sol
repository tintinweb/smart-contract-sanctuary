...
contract FarmDelegate is FarmStorageV1 {
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (number > usdtStartBlock) {
            uint256 multiplier = number.sub(pool.lastRewardBlock,"1");
    ...
    function phase(uint256 blockNumber) public view returns (uint256) {
        ...
        if (blockNumber > startBlockOfGan) {
            return (blockNumber.sub(startBlockOfGan,"2").sub(1)).div(halvingPeriod);
        }
        return 0;
    }
    ...
    function isHalvingPeriod0(uint256 blockNumber) public view returns (bool) {
        ...
        if (blockNumber > startBlockOfGan) {
            return
                blockNumber.sub(startBlockOfGan,"3").sub(1).mod(halvingPeriod) <
                halvingPeriod0;
        }
        ...
        returns (uint256, uint256)
    {
        ...
        if (switchBlock >= block.number) {
            //把最近周期的集中挖矿的收益计算出来
            blockLockReward = blockLockReward.add(
                (block.number.sub(_lastRewardBlock,"7")).mul(
            ...
            if (switchBlock > _lastRewardBlock) {
                blockLockReward = blockLockReward.add(
                    (switchBlock.sub(_lastRewardBlock,"8")).mul(
                        ganPerBlockConcentratedMining.div(2**m)
                    )
                );
                blockReward = blockReward.add(
                    (block.number.sub(switchBlock,"9")).mul(
                        ganPerBlockContinuousMining.div(2**m)
                    )
                );
            } else {
                blockReward = blockReward.add(
                    (block.number.sub(switchBlock,"10")).mul(
                        ganPerBlockContinuousMining.div(2**m)
                    )
                );
            }
        ...
        returns (uint256)
    {
        ...
        if (number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = number.sub(pool.lastRewardBlock,"11");
            uint256 usdtReward = multiplier
            .mul(usdtPerBlock)
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        ...
        return
            user.amount.mul(accUsdtPerShare).div(1e12).sub(user.rewardUsdtDebt,"12");
    }
            ...
            if (block.number > pool.lastRewardBlock) {
                if (pool.totalAmount == 0) {
                    ...
                    userReward = user.amount.mul(accGanPerShare).div(1e12).sub(
                        user.rewardGanDebt,"13"
                    );
                    
                    userLockAllReward = user
                    .amount;
                    userLockAllReward = userLockAllReward
                    .mul(accLockGanPerShare)
                    .div(1e12)
                    .sub(user.rewardLockGanDebt,"14")
                    .add(user.lockPending);
                    stillLockReward = getUserLockReward(_pid, _user);
                }
            }
            return userReward.add(userLockAllReward).sub(stillLockReward,"15");
        } else return 0;
    }
        ...
        returns (uint256)
    {
        ...
        if (m > n) {
            ...
            totalLockPendingAmount = user
            .amount;
            totalLockPendingAmount = totalLockPendingAmount
            .mul(pool.accLockGanPerShare)
            .div(1e12)
            .sub(user.rewardLockGanDebt,"17")
            .add(user.lockPending);
        }
        ...
        if (user.lastRewardBlock < switchBlock) {
            if (block.number > switchBlock) {
                ...
                .div(halvingPeriod1);
                realLockGanReward = totalLockPendingAmount.sub(unLockGanReward,"19");
            } else realLockGanReward = totalLockPendingAmount;
        } else {
            uint256 unLockGanReward = block
            .number
            .sub(user.lastRewardBlock,"20")
            .mul(totalLockPendingAmount)
            .div(halvingPeriod1.sub(user.lastRewardBlock,"21").add(switchBlock));
            realLockGanReward = totalLockPendingAmount.sub(unLockGanReward,"22");
        }
        ...
        returns (uint256)
    {
        ...
        while (m > n) {
            ...
            if (switchBlock > lastRewardBlock) {
                blockLockReward = blockLockReward.add(
                    (switchBlock.sub(lastRewardBlock,"23")).mul(
    ...
    function depositLPToken(
        ...
        if (user.amount > 0) {
            ...
            .div(1e12)
            .sub(user.rewardGanDebt,"24");

            uint256 userLockReward = user
            .amount
            .mul(pool.accLockGanPerShare)
            .div(1e12)
            .sub(user.rewardLockGanDebt,"25")
            .add(user.lockPending);
            stillLockReward = getUserLockReward(_pid, _user);
    ...
    function withdrawLPToken(
        ...
        if (user.amount >= 0) {
            ...
            .mul(pool.accGanPerShare)
            .div(1e12)
            .sub(user.rewardGanDebt,"a");

            uint256 userLockReward = user
            .amount
            .mul(pool.accLockGanPerShare)
            .div(1e12)
            .sub(user.rewardLockGanDebt,"b")
            .add(user.lockPending);
            stillLockReward = getUserLockReward(_pid, _user);
            uint256 pendingAmount = userReward.add(userLockReward).sub(
                stillLockReward,"111111111"
            );
            if (pendingAmount > 0) {
                safeGanTransfer(_user, pendingAmount);
            }
        ...
        if (_amount > 0) {
            user.amount = user.amount.sub(_amount,"D");
            pool.totalAmount = pool.totalAmount.sub(_amount,"E");
    ...
    function newReward(
        ...
        uint256 balance = afterAmount.sub(beforeAmount,"F");
        ...
        if (user.amount >= 0) {
            if (block.number > pool.lastRewardBlock) {
                if (lpSupply == 0) {
                    ...
                    .mul(accLockGanPerShare)
                    .div(1e12)
                    .sub(user.rewardLockGanDebt,"3333333")
                    .add(user.lockPending);
                    stillLockReward = getUserLockReward(_pid, _user);
                }
            }
