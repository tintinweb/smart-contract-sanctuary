...
interface LoanSharkInterface {
     ...
     * @param issueAmount The amount of DrugMoney debt to issue (considered to have 18 trailing decimals)
     * @return True if succeeds, otherwise reverts
     */
     ...
     * @param collateral The target bin's collateral
     * @param burnAmount The amount of DrugMoney debt to wipe (considered to have 18 trailing decimals)
     * @return True if succeeds, otherwise reverts
     */
     ...
     * @dev Returns `collateral value / debt` ratio
...
// File prb-math/contracts/PRBMathCo[email protected]
...
contract MobBoss is MobBossInterface, MobBossStorage, Ownable {
    ...
    function setVig(address collateral, uint256 newVig)
        ...
        returns (bool)
    {
        require(newVig >= PRBMathUD60x18.SCALE && newVig < PRBMathUD60x18.SCALE * 2, "MobBoss: invalid new vig");
    ...
    function realizeProtocolDebt(address collateral) public override returns (bool) {
        ...
        if (vigAccumilatorOf[collateral].accumilator == 0) {
            ...
            vigAccumilatorOf[collateral].accumilator = PRBMathUD60x18.mul(
                ...
                PRBMathUD60x18.pow(MobBoss(mobBossAddress).vigOf(collateral), timeSpan)
            );
