...
contract Proxator {
    ...
    */
    function createPool(address _tokenA, address _tokenB, address _router, uint _amountA, uint _amountB) external returns(uint) {
        ...
        uint amountAMin = _amountA - _amountA/100;
        uint amountBMin = _amountB - _amountB/100   ;
        uint deadline = 20 minutes;

        (uint amountOfToken,uint amountOfEth, uint liquidity) = router.addLiquidity(_tokenA, _tokenB, _amountA, _amountB, amountAMin, amountBMin, msg.sender, deadline);
