...
contract cRAIInsurance is SafeMath, ReentrancyGuard {
    ...
    */
    ...
    mapping (address => uint256) public collateralCover;
    ...
    function saveSAFE(address payable keeper, bytes32 collateralType, address safeHandler) virtual external returns (bool,uint256,uint256){
        ...
        require(mul(collateralCover[safeHandler], cToken.exchangeRateCurrent()) / WAD >= add(keeperPayout, tokenAmountUsed), "GeneralTokenReserveSafeSaviour/not-enough-cover-deposited");

        require(cToken.redeemUnderlying(add(keeperPayout, tokenAmountUsed))==0, "GeneralTokenReserveSafeSaviour/cToken-not-redeemable");
        
        // calculating cToken used for redeeming eth amount
        uint256 cTokenAmount = div(mul(tokenAmountUsed, WAD), cToken.exchangeRateCurrent());

        // Update the remaining cover
        collateralCover[safeHandler] = sub(collateralCover[safeHandler], cTokenAmount);
        totalCover = sub(totalCover, cTokenAmount);
    ...
    function canSave(address safeHandler) external returns (bool){
        ...
        return (mul(collateralCover[safeHandler], cToken.exchangeRateCurrent()) / WAD >= add(tokenAmountUsed, keeperPayout));
    }
    ...
    function deposit(uint256 safeID) payable external liquidationEngineApproved(address(this)) nonReentrant {
        ...
        uint256 cTokenAmount = div(mul(msg.value, WAD), cToken.exchangeRateCurrent());

        // Update the collateralToken balance used to cover the SAFE and transfer collateralToken to this contract
        collateralCover[safeHandler] = add(collateralCover[safeHandler], cTokenAmount);
        totalCover = add(totalCover, cTokenAmount);


        emit Deposit(msg.sender, safeHandler, cTokenAmount);
    }


    function withdraw(uint256 safeID, uint256 cTokenAmount) external controlsSAFE(msg.sender, safeID) nonReentrant{
        
        require(cTokenAmount > 0, "GeneralTokenReserveSafeSaviour/null-collateralToken-amount");

        address safeHandler = safeManager.safes(safeID);
        require(collateralCover[safeHandler] >= cTokenAmount, "GeneralTokenReserveSafeSaviour/not-enough-to-withdraw");
        ...
        emit Withdraw(msg.sender, safeID, safeHandler, cTokenAmount);
