...
contract SignData {
    ...
    constructor() public {
        ...
        LOCK_TYPE_HASH = keccak256("Data(string oraiAddress,uint256 amount,uint256 deadline,uint256 nonce)");
...
contract ETHToOraiLocker is Ownable, SignData {
    ...
    function lockOrai(string memory oraiAddress, uint256 amount) public {
        _lockOrai(msg.sender, oraiAddress, amount);
    }

    function lockOraiPermit(address sender, string memory oraiAddress, uint256 amount, uint256 deadline, uint8 v, bytes32 r, bytes32 s) public ensure(deadline) {
        verify(keccak256(abi.encode(LOCK_TYPE_HASH, oraiAddress, amount, deadline, nonces[sender]++)), sender, v, r, s);
        _lockOrai(sender, oraiAddress, amount);
    }
    ...
    function _lockOrai(address sender, string memory oraiAddress, uint256 amount) internal {
        ...
        SwapData memory _swapData = SwapData(sender, oraiAddress, amount);
