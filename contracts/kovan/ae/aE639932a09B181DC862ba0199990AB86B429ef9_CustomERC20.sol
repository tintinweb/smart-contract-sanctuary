...
contract CustomERC20 is Ownable, Burnable , Mintable {

    uint256 constant BASE = 10 ** 18;

    uint256 public saleStart = 1649977200;
    address public immutable lockedWallet;
    bool islocked = true;
    mapping(address => bool) public canPreSell; 


    constructor(string memory name_, string memory symbol_, address owner_,address lockedWallet_) ERC20(name_ , symbol_){
        _mint(owner_, 1_500_000_000 * BASE);
        _mint(lockedWallet_, 400_000_000 * BASE);
        addminter(owner_);
        addBurner(owner_);
        _transferOwnership(owner_);
        lockedWallet= lockedWallet_;
    }

    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        require(canPreSell[msg.sender] || block.timestamp >= saleStart,"Sales starts 15/04/2022");
    ...
    function transferFrom(
        ...
        require(canPreSell[sender] || block.timestamp >= saleStart,"Sales starts 15/04/2022");
        
        if(sender == lockedWallet)
            require(!islocked,"Locked wallet can not transfer before being unlocked");

        return super.transferFrom(sender, recipient, amount);
    }

    function setCanPreSell(address wallet_, bool canSell_) external {
        canPreSell[wallet_] = canSell_;
    }

    function unlockLockedWallet() external {
        islocked = false;
    }
}
