...
contract DebtProtocol is ERC20, Ownable {
    ...
    uint256 public debtLag = 12 hours;                         	    // After buying tokens on Uniswap, disable the debt timer for 'debtLag'
    ...
    constructor(address master) public ERC20("Debt Protocol", "DEBT") { 
        ...
        uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())               // Create the DEBT-BNB Uniswap V2 pair
            .createPair(address(this), address(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2));
        ...
        _isExcluded[address(uniswapV2Router)] = true;                               // Uniswap router excluded 
        _isExcluded[address(0x42C8Da3539dE51b74941A2530E9a82F8B08253B6)] = true;    // Unicrypt router excluded 
    	transferPauseExemptList[master] = true;

        emit Transfer(address(0x0), master, _totalSupply);
        
        addTier(50000000, 5, 1);                            // Tier 0
        addTier(250000000, 5, 1);                           // Tier 1
        addTier(575000000, 5, 1);                           // Tier 2
        addTier(1000000000, 5, 1);                          // Tier 3
        addTier(500000000000, 5, 1);                        // Tier 4
    }
...
//------------------------------------------------------------//	
//              functions related to the Seed Sale            //	
//------------------------------------------------------------//	
    ...
    function changeNbTokensSeed(uint256 _newAmount) external onlyOwner {	
            totalTokensSeed = _newAmount * 10 ** 18;	
    }	
    ...
    function buyTokens() onlyWhenRunning public payable {	
        ...
        require(_balances[_treasury] >= tokens.add(_totalSupply - totalTokensSeed), "not enough tokens left");	
	       	
        _transferFromExcluded(_treasury, msg.sender, tokens);	
    ...
    function stopSeed() onlyOwner public {	
        isStopped = true;	
    }	
	    	
    function resumeSeed() onlyOwner public {	
        isStopped = false;	
    }  
    ...
    function manageTier(address _user, uint256 _newBalance, uint256 _oldBalance, bool _debtLag) internal {
        ...
        for (uint256 i = 0; i < length; i++) {
            if (boostBalance < tiers[i].threshold) {
                ...
                if (oldTier != i) {
                    user.currentTier = i;
                    if (_balances[_user] < _totalSupply.div(10)) tierB.tierSupply = tierB.tierSupply.add(_newBalance);  // Prevents the LP contract to count as a creditor before it is excluded
...
//------------------------------------------------------------//  
    ...
    {
        require(!transfersPaused || transferPauseExemptList[msg.sender], "paused");
        require(now.sub(antiBotTimer) >= 300 || amount <= 100000000 * 10**DECIMALS, "Max buy 100,000,000 DEBT right after launch");
        ...
        override(ERC20) 
    {
        ...
        if (amount >= _totalSupply.div(10) || redistributionPaused) {	        // Allow contracts managing LP not to be part of the debt mechanism
