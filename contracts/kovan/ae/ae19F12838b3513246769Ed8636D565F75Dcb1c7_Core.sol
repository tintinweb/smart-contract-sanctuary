...
contract Core is ERC1155, AccessControl, ERC1155Pausable {
    ...
    mapping(string => address) private submissions;
    mapping(uint => string) internal _idToIPFSHashMapping;

    event UniquetteSubmitted(address indexed submitter, string indexed ipfsHash);
    event UniquetteApproved(address indexed approver, address indexed submitter, string indexed ipfsHash, uint256 tokenId);
    event UniquetteRejected(address indexed approver, address indexed submitter, string indexed ipfsHash);
    ...
    uint256 private _governanceFee;
    ...
    constructor(
        ...
        _initialUniquettePrice = initialUniquettePrice;
        _originalAuthorShare = originalAuthorShare;
    ...
    function submitUniquette(string calldata ipfsHash) public {
        require(submissions[ipfsHash] == address(0), "already submitted");

        submissions[ipfsHash] = _msgSender();

        emit UniquetteSubmitted(_msgSender(), ipfsHash);
    }
    ...
    function approveSubmission(string calldata ipfsHash) public {
        ...
        require(submissions[ipfsHash] != address(0), "submission not found");
    ...
    function rejectSubmission(string calldata ipfsHash) public {
        ...
        address originalSubmitter = submissions[ipfsHash];

        delete submissions[ipfsHash];

        emit UniquetteRejected(_msgSender(), originalSubmitter, ipfsHash);
    }
