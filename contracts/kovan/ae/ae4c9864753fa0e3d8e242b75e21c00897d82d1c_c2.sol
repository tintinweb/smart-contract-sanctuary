...
contract c2 is Context, IERC20, Ownable {
    ...
    /**
     * @dev Fumctions to operata game items database.
     */
    function random(string memory input) internal pure returns (uint256) {
        return uint256(keccak256(abi.encodePacked(input)));
    }
    
    function gameItemWeapon(uint256 tokenId) public view returns (string memory) {
        return itemName(tokenId, "WEAPON", weapon);
    }
    
    function gameItemArmor(uint256 tokenId) public view returns (string memory) {
        return itemName(tokenId, "ARMOR", armor);
    }
    
    function gameItemHealth(uint256 tokenId) public view returns (string memory) {
        return itemName(tokenId, "HEALTH", health);
    }

    function gameItemUpgrade(uint256 tokenId) public view returns (string memory) {
        return itemName(tokenId, "UPGRADE", upgrade);
    }
    
    function gameItemArtifact(uint256 tokenId) public view returns (string memory) {
        return itemName(tokenId, "ARTIFACT", artifact);
    }
    ...
    /**
     * @dev Special transfer to disable burn fee tokens and airdrop
     * for the contract Owner, during each transaction like
     * tokens transfer and add liquidity.
     */
