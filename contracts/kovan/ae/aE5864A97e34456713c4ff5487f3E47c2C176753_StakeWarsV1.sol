...
contract CharacterDetailsV1 is Administerable {
    ...
    constructor(function() external view _isAfterReveal)
        public
    ...
    function setRandom(uint256 _raritySeed, uint48 _rarityValue) public {
        // require(!_rarityLock);
    ...
    function addLand(Lands newLand, int8 index)
        public
    /**onlyAdminOrGameControl*/
    {
        land[index] = newLand;
    ...
    function setLand(Lands newLand, int8 index)
        public
    /**onlyAdminOrGameControl*/
    {
        land[index] = newLand;
    ...
    function updateExperience(
        ...
        uint256 amount,
        bool increase /**onlyAdminOrGameControl*/
    ) public {
        if (increase) {
...
contract StakeWarsV1 is ERC721Enumerable, Administerable, VRFConsumerBase {
    ...
    event RecievedRandomness(uint256 randomness, uint48 rand48, address sender);
        ...
        override
    {
        ...
        emit RecievedRandomness(randomness, rare48, sender);
        // stakeWarrior[sender][tokenId].setRandom(randomness, rare48);
        // rarityRatio[
    ...
    function reserve(uint256 num) public onlyAdmin {
        ...
        for (uint256 i = 0; i < num; i++) {
            ...
            requestIdToUser[requestId] = User(me, supply + i);
            stakeWarrior[me][supply + i] = new CharacterDetailsV1(
                this.isAfterReveal
            );
            // this.isAfterReveal
            emit RequestedRandomness(requestId, supply + i);
        }
    }

    function mintPresale(uint256 numberOfMints)
        public
        payable
        returns (bytes32)
    {
    ...
    function preMint(uint256 numberOfMints) public payable returns (bytes32) {
        uint256 supply = totalSupply();
        require(saleActive);
