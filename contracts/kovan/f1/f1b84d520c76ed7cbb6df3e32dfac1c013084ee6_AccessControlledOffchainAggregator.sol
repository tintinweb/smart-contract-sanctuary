  ...
  */
    ...
    internal
  {
    ...
    for (uint transmitteridx = 0; transmitteridx < transmitters.length; transmitteridx++) {
      ...
      uint256 linkWeiAmount =
        obsCount * uint256(billing.linkGweiPerObservation) * (1e8) + reimbursementAmountLinkWei;
      if (linkWeiAmount > 0) {
    ...
    returns (uint128 gasCostEthWei)
  {
    ...
    uint256 fullGasCostEthWei = gasUsed * gasPrice * (1e8);
    ...
    returns (uint256 linkDue)
  {
    ...
    linkDue *= uint256(billing.linkGweiPerObservation) * (1e8);
    address[] memory transmitters = s_transmitters;
    uint256[maxNumOracles] memory gasReimbursementsLinkWei =
      s_gasReimbursementsLinkWei;
    ...
    internal
  {
    ...
    uint256 gasPrice = impliedGasPrice(
      tx.gasprice / (1e8), // convert to ETH-gwei units
      billing.reasonableGasPrice,
      billing.maximumGasPrice
    );
    ...
    s_gasReimbursementsLinkWei[txOracle.index] =
      ...
      uint256(billing.linkGweiPerTransmission) * (1e8); // convert from linkGwei to linkWei
