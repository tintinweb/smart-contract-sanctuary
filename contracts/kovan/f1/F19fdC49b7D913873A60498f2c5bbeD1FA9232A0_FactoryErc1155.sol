...
contract ExchangeProvider is IExchangeProvider, AccessControl {
    ...
    function setExchange(address _exchange, address signer, bytes memory signature) external {
        _verifyAdmin(signer, signature);
        exchange = _exchange;
    }

    function _verifyAdmin(address signer, bytes memory signature)
        private
        view
    {
        address messageSigner =
            ECDSA.recover(keccak256(abi.encodePacked(signer)), signature);
        require(
            hasRole(DEFAULT_ADMIN_ROLE, messageSigner),
