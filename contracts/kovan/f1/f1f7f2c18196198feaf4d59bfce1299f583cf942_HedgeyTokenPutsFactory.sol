...
contract HedgeyTokenPutsV1 is ReentrancyGuard {
    using SafeMath for uint;
    ...
    address public weth = 0xd0A1E359811322d97991E03f863a0C30C2cF029C; //wrapped eth    
    uint public fee;
    ...
    constructor(address _asset, address _pymtCurrency, address _feeCollector, uint _fee, bool _feeCollectorSet) public {
        require(_asset != weth && _pymtCurrency !=weth);
        asset = _asset;
    ...
    function changeCollector(address _collector, bool _set) external returns (bool) {
        require(msg.sender == feeCollector, "only fee collector");
    ...
    function sellNewOption(uint _p, uint _assetAmt, uint _strike, uint _price, uint _expiry) public {
        ...
        require(put.short == address(0x0), 'this is not a new option');
    ...
    function cashClose(uint _p, bool dummy) public nonReentrant {
        ...
        require(!put.exercised, "p: This has already been exercised!");
        require(msg.sender == put.long, "p: You dont own this put");
        (uint pymtEst, uint wethIn) = getTo(put.assetAmt);
        require(pymtEst < put.totalPurch, "p: this put is not in the money"); 
        put.exercised = true;
        put.open = false;
        put.tradeable = false;
        swapTo(put.assetAmt, put.short);
    ...
    function swapTo(uint amountOut, address to) internal {
        ...
        (uint cashIn, uint wethIn) = getTo(amountOut);
        //swap payment token to Weth first
        swap(paymentPair, pymtCurrency, wethIn, cashIn, address(this)); //sends asset token into the pair, and delivers weth to us
        //now swap the received WETH into payment currency
        swap(assetPair, weth, amountOut, wethIn, to); //swaps to send the just received wethIn and finally gets the USD Out
    }
    ...
    //primary function to get the amounts in required to pay off the short position asset amount
    //amount out is the put asset amount out, amount in is the amount of cash needed to deliver in
    function getTo(uint amountOut) public view returns (uint amountIn, uint wethIn) {
        //working our way backwards
        //last set is getting the amount needed to pay asset- asset pair
        wethIn = estIn(amountOut, assetPair, asset);
        //step back is the amount to get the Weth needed from sending tokens in - paymentPair        
        amountIn = estIn(wethIn, paymentPair, weth);
...
contract HedgeyTokenPutsFactory {
    ...
    function createContract(address asset, address pymtCurrency) public {
        ...
        HedgeyTokenPutsV1 callContract = new HedgeyTokenPutsV1(asset, pymtCurrency, collector, fee, collectorSet);
