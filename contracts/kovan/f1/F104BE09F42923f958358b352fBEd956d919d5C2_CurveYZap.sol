...
contract CurveYZap is Ownable, BaseRelayRecipient {
    ...
    function _deposit(address _vault, uint256 _amount, address _coin, bool _stake) private returns (uint256 _daoERNBal) {
        ...
        if (_coin == address(_yTUSD) || _coin == address(_yUSDT) || _coin == address(_yUSDC) || _coin == address(_yDAI)) {
            if (_coin == address(_yTUSD)) {
                _amounts[3] = _amount;
            } else if (_coin == address(_yUSDT)) {
                _amounts[2] = _amount;
            } else if (_coin == address(_yUSDC)) {
                _amounts[1] = _amount;
            } else { // address(_yDAI)
                _amounts[0] = _amount;
            }
            curvePool.add_liquidity(_amounts, 0);
        } else if (_coin == address(_TUSD) || _coin == address(_USDT) || _coin == address(_USDC) || _coin == address(_DAI)) {
            if (_coin == address(_TUSD)) {
                _amounts[3] = _amount;
        ...
        uint256 _lpTokenBal = _lpToken.balanceOf(address(this));
