...
contract Registry is Ownable {
  ...
  function updateChain(address _chain) external onlyOwner {
    Registrable(_chain).register();

    bytes32 name = Registrable(_chain).getName();
    address oldChain = registry[name];
    registry[name] = _chain;

    Registrable(oldChain).unregister();

    emit LogRegistered(_chain, name);
  }
