...
contract TestMLBScheduleConsumer is ChainlinkClient {
    using Chainlink for Chainlink.Request;
    ...
    function requestSchedule(string calldata _sport, string calldata _date) public returns (bytes32 requestId) 
    {
    ...
    function getGame(uint256 _id) external view returns (uint256 _gameID, string memory _status, string memory _dateTime, string memory _awayTeam, string memory _homeTeam) {
        (_gameID, _status, _dateTime, _awayTeam, _homeTeam) = abi.decode(gameBytes[_id], (uint256, string, string, string, string));
    }
}