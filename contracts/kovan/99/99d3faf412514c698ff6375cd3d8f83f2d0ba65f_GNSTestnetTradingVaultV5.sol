...
// File: contracts\GNSTestnetTradingVaultV5.sol


pragma solidity 0.8.7;

contract GNSTestnetTradingVaultV5{
    ...
    function harvest() public{
        ...
        storageT.dai().transfer(msg.sender, pendingRewardDai());
        u.debtDai = u.daiDeposited * accDaiPerDai / 1e18;

        payable(msg.sender).transfer(pendingRewardMatic());
    ...
    // DAI incentives
    function distributeRewardDai(uint _amount) private{
        if(maxBalanceDai > 0){
    ...
    function receiveDaiFromTrader(address _trader, uint _amount, uint _vaultFee) external onlyCallbacks{
        ...
        currentBalanceDai += (_amount - _vaultFee);
