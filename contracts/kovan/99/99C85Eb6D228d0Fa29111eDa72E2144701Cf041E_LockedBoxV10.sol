...
contract LockedBoxV10 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function initialize(uint256 newValue) initializer public {
        ...
        cinoContract = 0x631f4695D178Dea8C8505d9D981f236A1f0a2b9c;
    ...
    function stakeTokens(uint256 _amount) external returns (bool success, uint256 amountStaked, uint256 unlockTimestamp){
        cino.transferFrom(_msgSender(), address(this), _amount);
        uint256 unlockDate = block.timestamp + (unixMinute * 5);
    ...
    function distribute() external onlyOwner {
        ...
        if(currentStakers.length > 0){
            ...
            for(uint256 i =0; i < currentStakers.length; i++){
                if(addressTotalStaked[currentStakers[i]] > 0){
                    ...
                    ethRewards[currentStakers[i]] = payout;
                }
            }
            pendingForDistribution = 0;
        } 
    ...
    function relock(uint256 unixTTL) external onlyOwner returns (uint256){
    ...
    function increment() public onlyOwner returns (uint256){
        value = value + 1;
