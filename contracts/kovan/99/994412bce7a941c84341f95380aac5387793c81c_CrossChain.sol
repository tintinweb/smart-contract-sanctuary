...
contract CrossChain is CrossChainStorage {
    ...
    function setAcceptToken(address token, bool isAccepted) external onlyAdmin{
        acceptToken[token] = isAccepted;
        emit AcceptToken(token, isAccepted);
    }

    function setAcceptChain(Chain chain, bool isAccepted) external onlyAdmin{
    ...
    function crossChainTransfer(address token, uint256 amount, address to, Chain chain) external payable whenNotPaused {
        ...
        sendTotalAmount[token] = addTotalAmount(token, sendTotalAmount[token], amount);
    ...
    function receiveToken(address token, uint256 amount, address receiveAddress, string memory info) external onlyRelayer whenNotPaused {
        bytes32 relayInfoHash = keccak256((abi.encodePacked(token,receiveAddress,amount,info)));
        require(!hasRelay(relayInfoHash), "Caller has already relayed");
        receiveTotalAmount[token] = addTotalAmount(token, receiveTotalAmount[token], amount);
        relayInfo[relayInfoHash].push(msg.sender);
        if(relayInfo[relayInfoHash].length < confirmRequireNum){
    ...
    function addTotalAmount(address token, uint256 totalAmount, uint256 amount) internal returns (uint256){
        ...
        require(totalAmount <= maxAmountPerDay[token], "Total amount is greater than max amount per day");
        return totalAmount;
    }
