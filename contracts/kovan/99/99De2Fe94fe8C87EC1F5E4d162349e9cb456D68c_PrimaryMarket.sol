...
// import '/Users/alone/ethereum/com.arbichess.contract/node_modules/@openzeppelin/contracts/math/Math.sol';
// import '/Users/alone/ethereum/com.arbichess.contract/node_modules/@openzeppelin/contracts/math/SafeMath.sol';
// import '/Users/alone/ethereum/com.arbichess.contract/node_modules/@openzeppelin/contracts/token/ERC20/IERC20.sol';
// import '/Users/alone/ethereum/com.arbichess.contract/node_modules/@openzeppelin/contracts/token/ERC20/SafeERC20.sol';
// import '/Users/alone/ethereum/com.arbichess.contract/node_modules/@openzeppelin/contracts/utils/ReentrancyGuard.sol';
// import '/Users/alone/ethereum/com.arbichess.contract/node_modules/@openzeppelin/contracts/access/Ownable.sol';
...
contract PrimaryMarket is IPrimaryMarket, ReentrancyGuard, ITrancheIndex, Ownable {
    ...
    function creationRedemptionOf(address account) external view returns (CreationRedemption memory) {
        return _currentCreationRedemption(account);
    }

    function create(uint256 underlying_, bytes32[] memory proof_) external payable nonReentrant onlyActive {
        ...
        if (block.timestamp < guardedLaunchStart + guardedCapTimeLimit) {
            ...
            if (addressWhitelist != address(0) && Address.isContract(addressWhitelist)) {
                require(IAddressWhitelist(addressWhitelist).check(msg.sender, proof_), 'You are not in the whitelist.');
            }
        }
    ...
    function split(uint256 inM) external onlyActive {
        require(block.timestamp >= guardedLaunchStart + guardedSplitTimeLimit, 'Guarded launch: split not ready yet');
        ...
        )
    {
        ...
        if (creationUnderlying > 0) {
            if (fundUnderlying > 0) {
                ...
                // no shares before.
                require(fundTotalShares == 0, 'Cannot create shares for fund with shares but no underlying');
                require(previousNav > 0, 'Cannot create shares at zero NAV');
                sharesToMint = creationUnderlying.mul(underlyingPrice).mul(fund.underlyingDecimalMultiplier()).div(
                    previousNav
                );
            }
        ...
        emit Settled(day, sharesToMint, sharesToBurn, creationUnderlying, redemptionUnderlying, fee);
    }

    function updateGuardedLaunchCap(uint256 newTotalCap, uint256 newIndividualCap) external onlyOwner {
    ...
    function _currentCreationRedemption(address account) private view returns (CreationRedemption memory cr) {
        ...
        if (oldDay < currentDay) {
            ...
            if (cr.version < rebalanceSize) {
                if (cr.createdShares > 0) {
                    (cr.createdShares, , ) = fund.batchRebalance(cr.createdShares, 0, 0, cr.version, rebalanceSize);
                }
