...
interface MSiG {
    ...
    function hasVoted(address owner) external view returns (bool voted, uint16 vWeight);
...
contract C98NSiGrc10 is MSiG {
    ...
    /// @return vWeight voting weight of owner
    function hasVoted(address owner) external view override returns (bool voted, uint16 vWeight) {
        VoteProgress memory progress = _voteProgress;
        uint16 weight = _voteWeights[owner];
        if (progress.requestId == 0) {
            return (false, weight);
        }
        return (progress.requestId == _votes[owner], weight);
    }
...
contract C98NSiGrc10Factory {
    ...
    function createMulitSig(address[] memory owners_, uint16 requiredVote_)
        external returns (C98NSiGrc10 wallet) {wallet = new C98NSiGrc10(owners_, requiredVote_);}
}