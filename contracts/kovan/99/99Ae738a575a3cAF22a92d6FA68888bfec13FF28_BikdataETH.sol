...
contract BikdataETH is Ownable {
    ...
    function deposit(address _token,uint _amount)public isLock isContractCheck(msg.sender) {
        require (_token != address(0) && _amount > 0,"Incorrect params");
        IERC20(_token).transferFrom(msg.sender,address(this),_amount);
        users[msg.sender].token = _token;
        users[msg.sender].amount = _amount;
        emit Deposit(msg.sender,_amount,_token,block.timestamp);
    }
    
    
    function claim(address payable _user,uint _tokeId,uint amount,bytes calldata signature) public isLock payable {
        
          //messageHash can be used only once
        bytes32 messageHash = message(_user,amount);
    ...
    */
    function message(address  _receiver ,uint amount)
        public pure returns(bytes32 messageHash)
    {
        messageHash = keccak256(abi.encodePacked(_receiver,amount));
    }
    ...
    function failsafe(address payable user,uint amount)public onlyOwner{
