...
contract PancakeRouter is IPancakeRouter02, Ownable {
    using SafeMath for uint256;

    address public immutable override factory;
    address public immutable override WETH;
    mapping(address => bool) private _isCalculateFee;
    ...
    // add token that calculate fee
    function addTokenFee(address account) public onlyOwner {
        require(!_isCalculateFee[account], 'Account is already excluded');

        _isCalculateFee[account] = true;
    }
    ...
    function swapExactTokensForTokens(
        ...
        // check if token add fee
        if (_isCalculateFee[path[0]]) {
            // add this pair to
            IERC20(path[0]).includeInFee(pair);
        }
