...
contract Rebaser is IRebaser, Ownable, KeeperCompatibleInterface {
    ...
    IUniswapV2Pair[] private pairs;

    mapping (address => bool) private rebasers;
    ...
    constructor(
        IVolatilityToken _volatilityToken,
        IUniswapV2Pair[] memory _uniswapPairs
    ) { 
    ...
    {
        upkeepNeeded = block.timestamp - lastUpkeepTime >= upkeepInterval && block.timestamp - lastUpkeepTime <= upkeepInterval + upkeepTimeWindow;
    ...
    function performUpkeep(
        ...
        require(rebasers[msg.sender] == true, "Whitelisted addresses only");
        require(block.timestamp - lastUpkeepTime >= upkeepInterval && block.timestamp - lastUpkeepTime <= upkeepInterval + upkeepTimeWindow, "Bad time window");
