...
contract ControllerStorage is ControllerAdminStorage {
    ...
    mapping(address => mapping(address => uint256)) public approvedStrategies;
    mapping(address => mapping(address => uint256)) public poolAmountInStrategy;
...
contract SVaultNetValue {
   ...
   struct PoolWeight{
    ...
    function getAll() view public returns(NetValue[] memory netValues, TokenAmountView memory tokenAmountView, PoolWeight[] memory poolWeights, uint256 totalWeight, uint256 totalAmountInUSD, uint256 totalAllocatedProfitInUSD){
        ...
        tokenAmountView = getTokenAmounts(fixedPools, flexiblePools);
        ...
        if(tokenAmountView.totalAmountInUSD < totalFixedPoolUSD) return (netValues, tokenAmountView, poolWeights, totalWeight, totalAmountInUSD, totalAllocatedProfitInUSD);
        tokenAmountView.totalAmountInUSD = tokenAmountView.totalAmountInUSD.sub(totalFixedPoolUSD);
        (poolWeights, totalWeight, totalAmountInUSD, totalAllocatedProfitInUSD) = getPoolWeights(flexiblePools);
