...
contract BoneToken is ERC20("BONE DOGECOIN KILLER", "BONE"), Ownable {
    ...
    mapping(address => uint256[]) public lockIdByUsers;
    
    
    // function to lock user reward bone tokens in token contract
    function lock(address _holder, uint256 _amount, bool _isDev) public onlyOwner {
        ...
        lockIdByUsers[_holder].push(lockId);
    }
    
    // fucnction called by user to claim the reward tokens after the locking period
    function claim(uint256 _lockId) public {
        ...
        require(lockTokensClaimed[_lockId] == false, "Tokens already claimed");
        require(now >= lockInfo._timestamp + lockingPeriodHere, "Can't claim, still under locking period");
        transfer(lockInfo._holder, lockInfo._amount);
        lockTokensClaimed[_lockId] = true;
    }

    // function to claim all locked tokens, locked at different times at once
    function claimAll() public {
        uint256[] memory arrayId = lockIdByUsers[msg.sender];
        uint i;
...
contract TopDog is Ownable {
    ...
    uint256 public shibPercent = 30;
    // xLeash reward percent
    uint256 public leashPercent = 5;
    // tBone reward percent
    uint256 public bonePercent = 10;
    ...
    // Update Percent
    function updatePercent(uint256 _devPercent, uint256 _shibPercent, uint256 _leashPercent, uint256 _bonePercent) public onlyOwner {
        massUpdatePools();
    ...
    function pendingBone(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 rewardPercent = devPercent.add(shibPercent).add(leashPercent).add(bonePercent);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 userReward = boneTotalReward.sub(devReward.add(shibReward).add(leashReward).add(boneReward));
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                ...
                safeBoneTransfer(address(bone), pending-sendAmount); // Rest amount sent to Bone token contract
                bool isDev; //Condition to check if the depositor(user) is a dev
                if(msg.sender == devaddr){
                    isDev = true;
                }
                bone.lock(msg.sender, pending-sendAmount, isDev); //function called for token time-lock
            }
        }
        if(_amount > 0) {
