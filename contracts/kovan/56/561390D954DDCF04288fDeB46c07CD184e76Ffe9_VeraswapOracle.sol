...
contract VeraswapOracle is ChainlinkClient, IOracle, Ownable {
    ...
    constructor(address _linkToken, address _priceFeed) public {
    ...
    function drainLink(address _to) external virtual override onlyOwner returns(bool){
        require(IBEP20(linkToken).balanceOf(address(this))>0,"Error:No tokens available");
        IBEP20(linkToken).transfer(_to, IBEP20(linkToken).balanceOf(address(this)));
        return true;
     }
}
...
interface IOracle {
    ...
    event VrapPriceUpdated(uint256 _newPrice);
    ...
    function drainLink(address _to) external returns(bool);
