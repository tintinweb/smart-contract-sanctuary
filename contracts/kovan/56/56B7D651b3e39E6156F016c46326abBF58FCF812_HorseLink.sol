...
contract HorseLink is Ownable {
    ...
    function addResult(bytes32 mnemonic, uint256 year, uint256 month, uint256 day, uint256 race, uint256[] memory _results) public {
        ...
        require(results[mnemonic][year][month][day][race].timestamp == 0, "Result already added");
        
        results[mnemonic][year][month][day][race] = Result(_results[0], _results[1], _results[2], _results[3], block.timestamp); //_results;
        count++;
        rewards[msg.sender]++;
