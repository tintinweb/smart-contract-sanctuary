...
contract SyntheticToken is ExpandedERC20, Lockable {
     ...
     * @param account The address to which the Minter role is added.
     */
    function addMinter(address account) external override {
        addMember(uint256(Roles.Minter), account);
    }
     ...
     * @param account The address to which the Burner role is added.
     */
    function addBurner(address account) external override {
        addMember(uint256(Roles.Burner), account);
    }
     ...
     * @param account The new holder of the Owner role.
     */
    function resetOwner(address account) external override {
        resetMember(uint256(Roles.Owner), account);
    }
...
contract LongShortPair is Testable, Lockable {
    ...
    enum ContractState { Uninitialized, Open, ExpiredPriceRequested, ExpiredPriceReceived }
    ...
    modifier onlyUninitialized() {
        require(contractState == ContractState.Uninitialized, "Contract already initialized");
        _;
    }
     ...
     * @dev the contract is disabled until it has been initialized.
     */
    constructor() Testable(address(0)) {}

    /**
     * @notice Initialize the LongShortPair
...
contract LongShortPairCreator is Testable, Lockable {
    ...
    address public longShortPairTemplate;
    ...
    constructor(
        ...
        address _longShortPairTemplate,
        address _timer
    ) Testable(_timer) nonReentrant() {
        finder = _finder;
        tokenFactory = _tokenFactory;
        longShortPairTemplate = _longShortPairTemplate;
    }
    ...
    function createLongShortPair(
        ...
        ExpandedIERC20 longToken =
            tokenFactory.createToken(
                ...
                _getSyntheticDecimals(collateralToken)
            );
        ...
        address lspAddress = Clones.clone(longShortPairTemplate);
        LongShortPair(lspAddress).initialize(
            expirationTimestamp,
            collateralPerPair,
            priceIdentifier,
            longToken,
        ...
        if (prepaidProposerReward > 0) collateralToken.safeTransferFrom(msg.sender, lspAddress, prepaidProposerReward);
