...
contract TreasuryPool is Pausable, ReentrancyGuard, ITreasuryPool {
    ...
    function addLiquidity(uint256 amount) external override nonReentrant {
        ...
        if (farmingPoolAccount != address(0)) {
            IFarmingPool farmingPool = IFarmingPool(farmingPoolAccount);
            ...
            lpLtokenAmount = _divExchangeRate(amount, borrowerInterestEarning);
        } else {
            lpLtokenAmount = amount;
        }
    ...
    function removeLiquidity(uint256 amount) external override nonReentrant {
        ...
        if (farmingPoolAccount != address(0)) {
            ...
            uint256 borrowerInterestEarning =
                farmingPool.computeBorrowerInterestEarning();
            lpUnderlyingAssetAmount = _mulExchangeRate(
                amount,
                borrowerInterestEarning
            );
        } else {
            lpUnderlyingAssetAmount = amount;
        }
        if (
            ...
            lpUnderlyingAssetAmount = totalUnderlyingAssetAvailable;
        }
        require(
            totalUnderlyingAssetAvailable >= lpUnderlyingAssetAmount,
            "TreasuryPool: insufficient liquidity"
        );
        ...
        returns (uint256)
    {
        if (farmingPoolAccount != address(0)) {
            ...
            return _divExchangeRate(amount, borrowerInterestEarning);
        } else {
            return amount;
        }
    }
        ...
        returns (uint256)
    {
        if (farmingPoolAccount != address(0)) {
            ...
            uint256 borrowerInterestEarning =
                farmingPool.estimateBorrowerInterestEarning();
            return _mulExchangeRate(amount, borrowerInterestEarning);
        } else {
            return amount;
        }
    }
    ...
    function removeFarmingPoolAccount() external onlyBy(governanceAccount) {
