...
contract FundPoolDelegate is FundPoolStorgeV1, FundPoolEvents {
    ...
    //     uint256 offset = now.sub(timeLast).div(86400).add(1);
    ...
    //             .div(max);
    //     uint256 managementFeePerShareLast =
    //         shares[msg.sender].managementFeePerShare;
    //     uint256 totalManagementFee =
    //         managementFeePerShareLast.mul(share).div(MAX).add(fee);
    //     shares[msg.sender].timestampForManagement = now;
    //     if (shareAmountIn > 0) {
    //         //申购时
    //         uint256 shareAmount =
    //             shares[msg.sender].shareAmount.add(shardAmountOut);
    //         shares[msg.sender].managementFeePerShare = totalManagementFee
    //             .mul(MAX)
    //             .div(shareAmount);
    ...
    //更新share 计算管理费
    // function update(uint256 totalTokens, bool _updateTime)
    //     private
    //     returns (uint256 totalManagementFee)
    // {
    //     uint256 timeLast = shares[msg.sender].timestamp;
    //     if (timeLast != 0) {
    //         uint256 offset = now.sub(timeLast).div(86400).add(1); // 不足一天按照一天计算？还是不足一天不算？还是按小时计算，这样最精确
    //         uint256 fee =
    //             shares[msg.sender]
    //                 .shareAmount
    //                 .mul(totalTokens)
    ...
    //         totalManagementFee = shares[msg.sender].managementFee.add(fee);
    //         shares[msg.sender].managementFee = totalManagementFee;
    //     }
    //     if (_updateTime) {
    //         shares[msg.sender].timestamp = now;
    //     }
    // }
