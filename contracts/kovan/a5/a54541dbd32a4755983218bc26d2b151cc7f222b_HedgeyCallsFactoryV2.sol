...
contract HedgeyCallsFactoryV2 {
    ...
    function getTotalContracts() public view returns (address[] memory contracts) {
        uint l = totalContracts.length;
        contracts = new address[](l);
        for (uint i; i < l; i++) {
            contracts[i] = totalContracts[i];
        }
    }
