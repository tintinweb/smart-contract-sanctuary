...
contract RichieNFT is ERC1155, Ownable {
	...
	constructor() ERC1155(_baseURI) {
		itemPrice = 1000000000000000; // 0.001 ETH
	}
	...
	function buyPresale(uint256 _howMany) public payable {
	    require(presaleSupply < 445 && preSale == true, "Presale inactive");
	...
	function buyRichie(uint256 _howMany) public payable {
	    require(getSaleStatus(), "Minting has not started yet");
		require(_howMany <= 10, "max 10 richies at once");
		require(itemPrice.mul(_howMany) == msg.value, "insufficient ETH");
		for (uint256 i = 0; i < _howMany; i++) {
			getRichie();
	...
	function newWallet(address wallet) public onlyOwner {
