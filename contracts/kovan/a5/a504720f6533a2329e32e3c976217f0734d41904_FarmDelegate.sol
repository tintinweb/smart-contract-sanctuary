...
contract FarmStorageV1 is FarmAdminStorage {
    ...
    struct PoolInfo {
        ...
        address refundToken; // The token need refund after migrate.
        uint256 refundAmount; // The amount of the above token need refund.
    ...
    struct RedepositUserInfo {
        uint256 redepositAmount;
        uint256[] poolIds;
    }

    mapping(address => RedepositUserInfo) public redepositUserInfo;
...
contract FarmDelegate is FarmStorageV1 {
    ...
    function depositLPToken(
        ...
        if (_amount > 0) {
            if (!switchBar && isAtoken(pool.lpToken)) {
            ...
            bool isInRedepositPids = checkRedepositPids(_user, _pid);
            if (!isInRedepositPids) redepositUserInfo[_user].poolIds.push(_pid);
        }
    ...
    function withdrawLPToken(
        ...
        if (_amount > 0) {
            if (!switchBar && isAtoken(pool.lpToken)) {
                uint256 scaledBalance = IMigrator(migrator).getAAVEScaledBalanceOfByBalance(pool.lpToken, _amount);
                scaledBalance = user.amount >= scaledBalance ? scaledBalance : user.amount;
                uint256 balance = IMigrator(migrator).getAAVEBalanceOfByScaled(pool.lpToken, scaledBalance);
                user.amount = user.amount.sub(scaledBalance);
                pool.totalAmount = pool.totalAmount.sub(scaledBalance);
                IERC20(pool.lpToken).safeTransfer(_user, balance);
            } else {
    ...
    function reDeposit(uint256 amount) public payable {
        require(router != address(0), "not ready");
        uint256 reDepositLimit = getReDepositLimit(msg.sender);

        require(
            amount <=
            reDepositLimit.sub(
                redepositUserInfo[msg.sender].redepositAmount
            ),
            "Insufficient reDepositLimit"
        );
        TransferHelper.safeTransferFrom(
            gandalf,
            msg.sender,
        ...
        redepositUserInfo[msg.sender].redepositAmount = redepositUserInfo[
        msg.sender
        ].redepositAmount.add(amountA);
    }

    // view function
    function checkRedepositPids(address _user, uint256 _pid)
    public
    view
    returns (bool)
    {
        uint256[] storage poolIds = redepositUserInfo[_user].poolIds;
    ...
    function getReDepositLimit(address _user) public view returns (uint256) {
        uint256[] storage poolIds = redepositUserInfo[_user].poolIds;
        uint256 reDepositLimit;
        for (uint256 i = 0; i < poolIds.length; i++) {
            ...
            ) = pendingGanWithLocked(_pid, _user);

            uint256 limit = user.claimedAmount.add(lockPendingAmount).add(
                pendingAmount
            );
            reDepositLimit = reDepositLimit.add(limit);
        }

        return reDepositLimit;
    }

    function pendingGanWithLocked(uint256 _pid, address _user)
    private
    view
        ...
        if (userAmount >= 0) {
            ...
            if (block.number > pool.lastRewardBlock) {
                if (pool.totalAmount == 0) {
                    ...
                    userReward = userAmount.mul(accGanPerShare).div(1e12).sub(
                        user.rewardGanDebt
                    );
    ...
    returns (uint256)
    {
        ...
        if (block.number > usdtStartBlock && lpSupply != 0) {
            ...
            return
            user.amount.mul(accUsdtPerShare).div(1e12).sub(
                user.rewardUsdtDebt
            );
        } else return 0;
    }
    ...
    function pendingLockGan(uint256 _pid, address _user)
    public
    view
    returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock) {
            if (pool.totalAmount == 0) {
                ...
                (, uint256 blockLockReward) = getGanBlockReward(
                    pool.lastRewardBlock
                );
    ...
    function getUserLockReward(
        ...
        if (m > n) {
            uint256 rewardDebt = userAmount.mul(lastAccLockGanPerShare).div(
                1e12
            );
    ...
    function migrateGToken(uint256[] calldata _pids) internal {
        ...
        uint256 accGanPerShareCoefficientBefore = pool.accGanPerShare.mul(pool.totalAmount);
        uint256 accLockGanPerShareCoefficientBefore = pool.accLockGanPerShare.mul(pool.totalAmount);
        ...
        pool.accGanPerShare = accGanPerShareCoefficientBefore.div(mintOut);
        pool.accLockGanPerShare = accLockGanPerShareCoefficientBefore.div(mintOut);
    ...
    function migrateLpToken(uint256[] calldata _pids) internal {
        ...
        (address token,address refundToken,uint refundAmount,uint256 refundLpIndex) = IMigrator(migrator).migrate(tokens, amounts);
        uint256 balanceAfter = IERC20(token).balanceOf(address(this));
        if (refundToken != address(0)) {
            if (refundLpIndex == 1) {
                poolInfo01.refundToken = refundToken;
                poolInfo01.refundAmount = refundAmount;
            } else {
                poolInfo02.refundToken = refundToken;
                poolInfo02.refundAmount = refundAmount;
            }
        }
        ...
        if (length == 2) {
            ...
            poolInfo02.totalOrigDep = poolInfo01.totalAmount;
    ...
    function checkRefundAndConvert(uint _pid) internal {
        PoolInfo memory pool = poolInfo[_pid];
        if (pool.hasMigrate) {
            UserInfo storage user = userInfo[_pid][msg.sender];
            if (!user.flag) {
                if (pool.refundAmount > 0 && pool.refundToken != address(0) && pool.convertRate == 0) {
                    //                lpToken to return
                    uint256 shouldRefund = user.amount.mul(pool.refundAmount).div(pool.totalOrigDep);
                    user.flag = true;
                    IERC20(pool.refundToken).safeTransfer(msg.sender, shouldRefund);
                } else if (pool.convertRate != 0) {
                    //                convert
                    //                gToken to convert
                    user.flag = true;
    ...
    function turnSwitchBar(bool state) external {
        require(msg.sender == admin, "unautherized");
        switchBar = state;
    }
    ...
    function getUserInfo(
        ...
        uint256 amount,
        uint256 rewardGanDebt,
        uint256 rewardLockGanDebt,
        ...
        rewardGanDebt = user.rewardGanDebt;
        rewardLockGanDebt = user.rewardLockGanDebt;
