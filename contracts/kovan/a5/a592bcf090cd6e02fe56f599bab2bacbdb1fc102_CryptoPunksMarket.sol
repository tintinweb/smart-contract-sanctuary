...
/**
 *Submitted for verrequireication at Etherscan.io on 2017-07-19
*/

pragma solidity 0.5.13;
contract CryptoPunksMarket {

    // You can use this hash to verrequirey the image file containing all the punks
    ...
    struct Offer {
        ...
        address onlySellTo;     // specrequirey to sell only to a specrequireic person
    }
    ...
    // A record of punks that are offered for sale at a specrequireic minimum value, and perhaps to a specrequireic person
    mapping (uint => Offer) public punksOfferedForSale;
    
    //Sprawdzanie ktory ziomek jest offer z isForSale = true
    function getMappingValue() public view returns (uint[] memory) {
        uint[] memory memoryArray = new uint[](1000);
        for(uint i = 0; i < 1000; i++) {
            memoryArray[i] = punksOfferedForSale[i].minValue;
        }
        return memoryArray;
    }
    
       //Sprawdzanie ktory ziomek jest offer z isForSale = true
    function getMappingValue2() public view returns (bool[] memory _isForSale, uint[] memory _punkIndex, uint[] memory _minValue) {
        bool[] memory isForSale = new bool[](1000);
          uint[] memory punkIndex = new uint[](1000);
            uint[] memory minValue = new uint[](1000);
        for(uint i = 0; i < 1000; i++) {
        ...
        return (isForSale, punkIndex, minValue);
    }
