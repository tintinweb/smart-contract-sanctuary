...
// File: contracts/DeftifyVesting_1.sol


pragma solidity ^0.8.0;
//DeftifyVesting_1 (new) - with monthly unlock terms for seed and private rounds but linear for public




contract Vesting_One is Ownable {
    IERC20 token; // interface for the Deployed DFTY token
    ...
    struct Round {
        ...
        uint onTGE;
        uint afterUnlock;
        uint afterUnlockDenominator;
    }

    constructor() {
        // Here ERC20 Token will be passed, later as an argument in constructor
        transferOwnership(msg.sender);
        WEEKS = 7 days;
        MONTHS = 30 days;

        round_['seed'] = Round({roundID: 0,
                                lockPeriod: MONTHS,
                                period: 12 * MONTHS,
        ...
        round_['private'] = Round({roundID: 1,
                                lockPeriod: MONTHS,
                                period: 12 * MONTHS,
                                timeUnit: MONTHS, //Time of withdraw should be in seconds.
                                onTGE: 8,
    ...
    mapping(address => uint) public BalanceClaimed;
    // Mapping to see how much token allocated for address for a specific round.
    mapping(string => mapping(address => uint)) public TotalTokenAllocations;
    ...
    */
    event Claimed(uint,address,uint);
    function claim() public TGEisStarted {
        uint claimed_ = claimed(msg.sender);
        require(claimed_ > 0, 'You dont have any tokens to claim.');
        //        require(token.balanceOf(address(this)) > claimed_, 'Vesting contract doesnt have enough tokens');
    ...
    */
    function claimed(address user) public view TGEisStarted returns (uint amount) {
        uint total = claimedInCategory(user, 'seed') + claimedInCategory(user, 'private') + claimedInCategory(user, 'public') - BalanceClaimed[user];
        return total;
    }
    ...
    */
    function claimedInCategory(address user, string memory categoryName) public view TGEisStarted returns (uint amount){
        Round memory round = round_[categoryName];

        // get round ID
        uint _roundID = round.roundID;
        if(_roundID == 0){
            uint vestingTime = block.timestamp - startTimestamp;

            // before lock period
            if (round.lockPeriod >= vestingTime) return 0;

            uint bank = TotalTokenAllocations[categoryName][user];
            uint amountOnUnlock = (bank * round.afterUnlock) /
            round.afterUnlockDenominator;

            uint timePassed = vestingTime - round.lockPeriod;
            uint totalUnits = (round.period - round.lockPeriod) / round.timeUnit;
            uint amountOfUnits = timePassed / round.timeUnit;
            uint amountAfterUnlock = ((bank - amountOnUnlock) * amountOfUnits) /
            totalUnits;

            if (amountOnUnlock + amountAfterUnlock > bank) return bank;
            ...
            uint bank = TotalTokenAllocations[categoryName][user];
            uint unlockAtTGE = round.onTGE;
            uint denom  = round.afterUnlockDenominator;
            uint reward = bank * unlockAtTGE/denom;
            // before lock period
            if (round.lockPeriod >= vestingTime) return reward;

            reward += (block.timestamp - (round.lockPeriod + startTimestamp)) * (bank - reward) / (round.period - round.lockPeriod);
    ...
    */
    function totalTokensAllocated(address _address) view public returns(uint) {
        uint totalTokens =  TotalTokenAllocations['seed'][_address] +
