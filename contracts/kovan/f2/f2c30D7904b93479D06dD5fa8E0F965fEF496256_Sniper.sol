...
contract Sniper is Ownable {
    ...
    function balanceOfContract(address _token) public view returns (uint256) {
        uint256 token_balance = IERC20(_token).balanceOf(address(this));
        return token_balance;
    }

    function balanceOfSender(address _token) public view returns (uint256) {
        uint256 token_balance = IERC20(_token).balanceOf(msg.sender);
        return token_balance;
    }

    function withdrawToWallet(address _token, uint256 _amount)
        external
        onlyOwner
    ...
    function swapFromWallet(address to) external {
        require(
    ...
    function swapFromContract() external {
        require(
