...
interface DogeV3CalculatorInterface {
  ...
  function getThreshhold(PriceRange priceRange)
    external
    view
    returns (uint16 threshhold);
...
contract DogeV3 is Ownable, DogeV3Interface, KeeperCompatibleInterface, IERC20 {
  ...
  function depositFirstStage(uint256 amount) external override onlyOwner {
    ...
    stage[1] = StageInfo(
      ...
      uint64(tokenId),
      0,
      uint80(calcs.getCurrentPrice(tick, isReversed))
    );
    currentStage = 1;
  }
  ...
  function incrementStage() public override {
    ...
    (
      int24 upperTick,
      int24 lowerTick,
      uint80 nextMidPriceE8,
      bool isSafeIncrement
    ) =
      calcs.newTicks(
        tick,
        spacing,
        stageInfo.midPriceE8,
        isReversed,
        nextPriceRange,
        currentPriceRange
      );
    ...
    )
  {
    ...
    uint80 midPrice = stage[currentStageId].midPriceE8;
    uint16 threshhold = calcs.getThreshhold(currentPriceRange);

    lowerPrice = (midPrice * threshhold) / 10;
    upperPrice = (midPrice * 10) / threshhold;
    currentPrice = calcs.getCurrentPrice(currentTick, isReversed);
  }
