...
contract UniswapReader {
    ...
    constructor(
        ...
        lpLAND20 = IUniswapV2Pair(lpLAND20Address);
    ...
    function getLPDetails(uint256 amount)
        ...
        (uint112 _reserve0,uint112 _reserve1, ) = lpLAND20.getReserves(); // IUniswapV2Pair(lpLAND20).getReserves();
        token0Balance = uint256(_reserve0);
        token1Balance = uint256(_reserve1); 
        lpTotalSupply = lpLAND20.totalSupply(); // IUniswapV2Pair(lpLAND20).totalSupply();
        ...
        landBalance = uint256(amount.mul(token0Balance).div(lpTotalSupply));
    }
