                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                '"image":"',
                                _imageURI,
                                ', "animation": "ipfs://bafybeieucgr7jnv7mknoca6t6tfoekx3alkbxemzv46sggyzrnmqwfalny"}'
                            )
                        )
                    )
                )
    ...
    Ownable
{
    ...
    uint256 public immutable START_BLOCK_TIMESTAMP;
    uint256 public immutable PRICE;
    bool public isGoing;
    bool public isPublic;
    // We have an allowlist, so the sale is split into two stages such that only
    // allowlist addresses can participate in the first stage whilst the second
    // stage is open to both allowlist and public addresses.
    // The allowlist merkle tree will be uploaded to IPFS.
    bytes32 public immutable MERKLE_ROOT;
    string public MERKLE_TREE_CID;
    address public hundoMultiSig; // withdraw funds to this address after sale
    mapping(address => bool) public hasReserved;
    ...
    constructor(
        ...
        PRICE = _price;
        START_BLOCK_TIMESTAMP = _startBlockTimestamp;
        MERKLE_TREE_CID = _merkleTreeCID;
    ...
    receive() external payable {
        // We can't verify if sending address is in either allowlist or
        // waitlist so just keep track of donations
    ...
    function reserve(
        ...
        string calldata _backerName
    ) external payable nonReentrant whenNotPaused saleConditions {
        require(_account == msg.sender, "Bad account");
        require(!hasReserved[_account], "Already reserved");
        require(
            MerkleProof.verify(
                _merkleProof,
                MERKLE_ROOT,
        ...
        hasReserved[_account] = true;
        _mintReceipt(_account, _backerName);
    }

    function reservePublic(string calldata _backerName)
        external
        payable
        nonReentrant
        whenNotPaused
        saleConditions
    {
        require(isPublic, "Allowlist only");
        _mintReceipt(msg.sender, _backerName);
    }

    function updateIsGoing(bool _isGoing) external {
        if (_isGoing) {
            require(block.timestamp >= START_BLOCK_TIMESTAMP, "Too early");
            require(
                !isGoing && _tokenIds.current() < MAX_SUPPLY,
                "Already ended"
            );
        } else if (!_isGoing) {
            require(_tokenIds.current() == MAX_SUPPLY, "Not yet ended");
        }
        isGoing = _isGoing;
    }

    function updateIsPublic() external {
        // open sale to public after the four day period if not already sold out
        require(block.timestamp >= START_BLOCK_TIMESTAMP + 4 days, "Too early");
        isPublic = true;
    }
    ...
    function withdrawProceeds() external onlyOwner {
        // TODO: could automate transfer over a certain ETH threshold
    ...
    function _saleConditions() internal {
