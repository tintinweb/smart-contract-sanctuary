...
contract Vault is Ownable, ReentrancyGuard {
    ...
    function getEstimatedCompoundDivsOwing(address account) public view returns (uint) {
        return getOneCTokenInUnderlyingStored().mul(cTokenBalance[account]).sub(depositTokenBalance[account]);
    }
    ...
    function _claimCompoundDivs() private {
        updateAccount(msg.sender);
        uint oneCTokenInUnderlying = getOneCTokenInUnderlying();
        uint amount = cTokenBalance[msg.sender].mul(oneCTokenInUnderlying).sub(depositTokenBalance[msg.sender]);
    ...
    function getOneCTokenInUnderlying() public returns (uint) {
        uint exchangeRateCurrent = CErc20(TRUSTED_CTOKEN_ADDRESS).exchangeRateCurrent();
        return exchangeRateCurrent / (1 * 10 ** (18 + UNDERLYING_DECIMALS - 8));
    }
    
    function getOneCTokenInUnderlyingStored() public view returns (uint) {
        uint exchangeRateStored = CErc20(TRUSTED_CTOKEN_ADDRESS).exchangeRateStored();
        return exchangeRateStored / (1 * 10 ** (18 + UNDERLYING_DECIMALS - 8));
    }
