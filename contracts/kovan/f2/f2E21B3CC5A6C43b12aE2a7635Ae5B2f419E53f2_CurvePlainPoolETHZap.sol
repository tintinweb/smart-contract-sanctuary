...
contract CurvePlainPoolETHZap is Ownable, BaseRelayRecipient {
    ...
    function deposit(address _vault, uint256 _amount, address _coin, bool _stake) external onlyEOAOrBiconomy returns (uint256 _daoERNBal) {
        ...
        require(_coin == address(_baseCoin), "Coin not acceptable");
        require(_amount > 0, "Amount must > 0");
        _baseCoin.safeTransferFrom(_msgSender(), address(this), _amount);
    ...
    function depositZap(address _vault, uint256 _amount, address _tokenAddr, bool _stake) external payable onlyEOAOrBiconomy returns (uint256) {
        ...
        if (_tokenAddr == address(0)) { // Deposit ETH
            ...
            _daoERNBal = _deposit(_vault, _amount, address(0), _stake);
        } else {
            ...
            uint256[] memory _amounts = _sushiRouter.swapExactTokensForETH(_amount, 0, _path, address(this), block.timestamp);
            _daoERNBal = _deposit(_vault, _amounts[1], address(0), _stake);
        }
        return _daoERNBal;
    }
    ...
    function withdraw(address _vault, uint256 _shares, address _coin) external returns (uint256) {
        require(msg.sender == tx.origin, "Only EOA");
        PoolInfo memory _poolInfo = poolInfos[_vault];
        uint256 _lpTokenBal = IEarnVault(_vault).withdrawZap(_shares, msg.sender);
        uint256 _coinAmount;
        if (_coin == address(0)) { // Withdraw ETH
            _coinAmount = _poolInfo.curvePool.remove_liquidity_one_coin(_lpTokenBal, 0, 0);
            (bool _r,) = msg.sender.call{value: _coinAmount}("");
            require(_r, "ETH transfer failed");
        } else {
            require(_coin == address(_poolInfo.baseCoin), "Coin not acceptable");
            _coinAmount = _poolInfo.curvePool.remove_liquidity_one_coin(_lpTokenBal, 1, 0);
            _poolInfo.baseCoin.safeTransfer(msg.sender, _coinAmount);
        }
