...
contract FantomAuction is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address payable _platformFeeRecipient, IERC20 _RWT, address _ownerAddr)
        public
        initializer
    {
        require(
        ...
        nonReentrant
    {
        ...
        if (payAmount > 0) {
            if (auction.payToken == address(0)) {
                ...
                // Transfer the reward token to the winner and owner
                RWT.safeTransferFrom(ownerAddr, IERC721(_nftAddress).ownerOf(_tokenId), payAmount.mul(5).div(100));
                RWT.safeTransferFrom(ownerAddr, winner, payAmount.mul(5).div(100));
            }
        }
