...
contract PoolStake is Owned {
    ...
    constructor(address _token, address _Owner) public Owned(_Owner) {
        ...
        multiplier1 = Multiplier(0x5B921C16ceE27972B8Ec4Ac027b6D48C85fA7F56);
    ...
    function _withMultiplier(
        ...
        if(_id == 1) 
        _resolveMultiplier(_term, amountETH, amountToken, wethRP, tokenRP, uToken1, multiplier1);
        else _resolveMultiplier(_term, amountETH, amountToken, wethRP, tokenRP, uToken2, multiplier2);
    }
    ...
    function _resolveMultiplier(
        uint _term, uint amountETH, uint amountToken, uint wethRP, 
        uint tokenRP, address _uToken, Multiplier _multiplier
    ) internal {
        ...
        uint multipliedETH = _proportion(_multiplier.balance(msg.sender), _uToken, address(weth));
    ...
    function _withdrawUserETHBonus(address payable _user, uint releasedAmount, uint _id) internal returns(bool) {
        ...
        (uint fee, uint feeInETH) = _calculateETHFee(releasedAmount, _id);
    ...
    function _withdrawUserTokenBonus(address _user, uint releasedAmount, uint _id) internal returns(bool) {
        ...
        (uint fee, uint feeInToken) = _calculateTokenFee(releasedAmount, _id);
        if(_id == 1) require(IERC20(uToken1).transferFrom(_user, platformWallet, fee), "must approve fee");
        else require(IERC20(uToken2).transferFrom(_user, platformWallet, fee), "must approve fee");
     ...
     */ 
    function _calculateTokenFee(uint _amount, uint _id) internal view returns(uint uTokenFee, uint tokenFee) {
        
        uint fee = (_amount.mul(10)).div(100);
        uint feeInETH = _proportion(fee, address(token), address(weth));
        uint feeInUtoken;
        
        if(_id == 1) feeInUtoken = _proportion(feeInETH, address(weth), uToken1); 
        else feeInUtoken = _proportion(feeInETH, address(weth), uToken2);
     ...
     */ 
    function _calculateETHFee(uint _amount, uint _id) internal view returns(uint uTokenFee, uint ethFee) {
        
        uint fee = (_amount.mul(10)).div(100);
        uint feeInUtoken;  
        
        if(_id == 1) feeInUtoken = _proportion(fee, address(weth), uToken1); 
        else feeInUtoken = _proportion(fee, address(weth), uToken2);
     ...
     */ 
    function calculateETHBonusFee(address _user, uint _id) external view uTokenVet(_id) returns(uint ETH_Fee) {
        
        uint wethReleased = _calculateETHReleasedAmount(_user);
        
        if (wethReleased > 0) {
            
            (uint feeForWethInUtoken,) = _calculateETHFee(wethReleased, _id);
     ...
     * returns the fee amount.
     */ 
    function calculateTokenBonusFee(address _user, uint _id) external view uTokenVet(_id) returns(uint token_Fee) {
        
        uint tokenReleased = _calculateTokenReleasedAmount(_user);
        
        if (tokenReleased > 0) {
            
            (uint feeForTokenInUtoken,) = _calculateTokenFee(tokenReleased, _id);
