...
contract PharoStakePool is Context, AccessControl, Ownable, ReentrancyGuard {
    ...
    uint256 public xPHRO_PER_BLOCK = 100 ether;

    uint256 constant MAX_xPHRO_SUPPLY = 1000000000 ether; // 1 Billion
    ...
    struct StakerInfo {
        ...
        // pending reward = (staker.amount * pool.accumulatedxPharoPerShare) - staker.rewardDebt
        // Whenever a user deposits or withdraws tokens to a pool. Here's what happens:
        //   1. The pool's `accumulatedxPharoPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accumulatedxPharoPerShare; // last block that PHRO dist occurred
    ...
    */
    ...
    /// @dev how many pools there are
    /// @return the length of the pool uint256
    function poolLength()
        public
        view
        returns(uint256) 
    ...
    function calculateApy(uint256 _poolId) 
        public
        view
        returns(uint256)
    {
        ...
        return pool.accumulatedxPharoPerShare.div(100);
    }

    function poolDetails(uint256 poolId) external view returns(uint[3] memory) {
        PoolInfo storage pool = poolInfoMap[poolId];
        return [
            pool.allocationPoint,
            pool.accumulatedxPharoPerShare,
            pool.lastRewardBlock
        ];
    }

    function stakerDetails(uint256 poolId, address staker) external view returns(uint[2] memory) {
    ...
    function depositDetails(uint256 poolId, address staker) external view returns(uint[2] memory) {
    ...
    // XXX DO NOT add the same staked token more than once. Rewards will be messed up if you do.
    function addStakePool(uint256 _allocationPoint, IERC20 _asset, bool _withUpdate) 
        public
        //onlyOwner
        nonDuplicated(_asset)
    {
        if(_withUpdate) {
            massUpdatePools();
        }
        ...
        poolInfo.push(
            PoolInfo({
                ...
                accumulatedxPharoPerShare: 0
            })
        );

        poolInfoMap[0] = poolInfo[0];

        emit LogPoolAddition(poolInfo.length.sub(1), _allocationPoint, asset);
    }

    // Update the given pool's PHRO allocation point. Can only be called by the owner.
    function setStakePoolAllocation(uint256 _poolId, uint256 _allocationPoint, bool _withUpdate) 
        public 
    {
        // todo: add require here for pool existence

        if(_withUpdate) {
            massUpdatePools();
        }
        totalAllocationPoint = totalAllocationPoint.add(poolInfo[_poolId].allocationPoint).add(_allocationPoint);
        poolInfo[_poolId].allocationPoint = _allocationPoint;
        
        emit LogSetPool(_poolId, _allocationPoint);
    }


    // What asset are we going to have the user stake first by default? PHRO, TUSD, USDT
    // Stake TUSD tokens for PHRO reward allocation.
    // ** Need to call approve on the token first  
    function stake(uint256 _poolId, uint256 _amount) 
        public
    {
        require(msg.sender != address(0));
        require(_amount > 0, "PharoStakePool:: _amount too small, must be greater than 0");
        uint256 contractBeforeBalance = xphroToken.balanceOf(address(this));

        PoolInfo storage pool = poolInfo[_poolId];
        StakerInfo storage staker = stakerInfo[_poolId][msg.sender];
        updatePool(_poolId);
        if(staker.amount > 0) {
            uint256 pending = staker.amount.mul(pool.accumulatedxPharoPerShare).div(1e12).sub(staker.rewardDebt);
            if(pending > 0) {
        ...
        // Mint the xPHRO token at a 1:1 ratio

        
        staker.rewardDebt = staker.amount.mul(pool.accumulatedxPharoPerShare).div(1e12);
    ...
    // Be careful of gas spending here, with a lot of pools
    // we may want to limit this...
    // 
        ...
        public 
    {
        ...
        uint256 pharoReward = multiplier.mul(xPHRO_PER_BLOCK).mul(pool.allocationPoint).div(totalAllocationPoint);
        // Burn 10% of the rewards
        uint256 burnAmount = pharoReward.div(10);
        phroToken.mint(burnAddress, burnAmount);
        uint256 rewardAfterBurn = pharoReward - burnAmount;
        phroToken.mint(address(this), rewardAfterBurn);

        // add the reward amount when its paid
        bonusSums[msg.sender] = bonusSums[msg.sender] + pharoReward;

        pool.accumulatedxPharoPerShare = pool.accumulatedxPharoPerShare.add(pharoReward.mul(1e18).div(poolTokenSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function getMultiplier(uint256 _from, uint256 _to)
        ...
        returns(uint256) 
    {
        if(phroToken.totalSupply() >= MAX_xPHRO_SUPPLY) return 0;
        ...
        returns(uint256) 
    {
        ...
        uint256 accumulatedxPharoPerShare = pool.accumulatedxPharoPerShare;
        uint256 stakedSupply = pool.asset.balanceOf(address(this));
        if(stakedSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 pharoReward = multiplier.mul(xPHRO_PER_BLOCK).mul(pool.allocationPoint).div(totalAllocationPoint);
            accumulatedxPharoPerShare = accumulatedxPharoPerShare.add(pharoReward.mul(1e12).div(stakedSupply));
        }

        return staker.amount.mul(accumulatedxPharoPerShare).div(1e12).sub(staker.rewardDebt);
    }
        ...
        nonReentrant
    {
        ...
        pool.asset.safeTransferFrom(address(this), address(msg.sender), _amount);

        // Burn the xPHRO token at a 1:1 ratio of PHRO withdrawn
        // todo: figure out why I cannot use burn()
        // To burn them we just send to the dead address
        xphroToken.transferFrom(address(msg.sender), 0x000000000000000000000000000000000000dEaD, _amount);
    ...
    function updateBurnAddress(address _burnAddress)
        ...
        onlyOwner 
    {
        require(msg.sender == burnAddress, "PharoRiskPool :: not the original burn address");
    ...
    {
        massUpdatePools();
        xPHRO_PER_BLOCK = _pharoPerBlock;
...
contract PharoToken is ERC20PresetMinterPauser, TokenRecover {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    */
    ...
    uint256 private constant _TIMELOCK = 1;
    ...
    constructor()
        public
        ERC20PresetMinterPauser("Pharo Token", "PHRO")
        //TokenTimelock(this, 0x3f15B8c6F9939879Cb030D6dd935348E57109637, _TIMELOCK)
    {
        // Set up Roles
    ...
    */
    ...
    function burn(uint256 _amount) public virtual override {
        require(hasRole(BURNER_ROLE, msg.sender), "Pharo Token:: Must be in a Burner role");
