...
interface IPugFactory {
    function claimRewards(uint256 _lastRewardBlock) external returns(uint256);
}
...
contract Pug is ERC20PresetMinterPauser, Ownable {
    ...
    function updateRewards() public {
        ...
        uint256 reward = getPugRewards(_lastRewardBlock);
        accPugTokenPerShare = accPugTokenPerShare + (reward * DENOMINATION / pugSupply);
        lastRewardBlock = block.number;
    }

    function getPugRewards(uint256 _lastRewardBlock) internal returns(uint256) {
        return IPugFactory(pugFactory).claimRewards(_lastRewardBlock);
    }
    ...
    function pug(uint256 _amount) external {
        ...
        uint256 mintAmount = _amount - (feeAmount * 2);

        ERC20(_baseToken).transferFrom(msg.sender, address(0), feeAmount);
        ERC20(_baseToken).transferFrom(msg.sender, address(this), mintAmount);
        address _pugStaking = pugStaking;
        ERC20(_baseToken).transferFrom(msg.sender, _pugStaking, feeAmount);
        IPugStaking(_pugStaking).addRewards(feeAmount);

        mint(msg.sender, mintAmount);
        userDebt[msg.sender] = (senderBalance + mintAmount) * accPugTokenPerShare / DENOMINATION;
    }
    ...
    function unpug(uint256 _amount) external {
        ...
        uint256 totalBaseTokensHeld = ERC20(_baseToken).totalSupply();
