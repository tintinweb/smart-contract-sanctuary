...
contract LosslessV2Pool is ILosslessV2Pool {
	...
	function initialize(
		...
		status.gameRound = 1;
		status.durationOfGame = 30 minutes;
		status.durationOfBidding = 15 minutes;
		// status.gameStartTimestamp - unchange for now
		// status.initialPrice - unchange for now
		// status.endPrice - unchange for now
		// status.winner - unchange for now
	...
	function startFirstRound() external override {
		...
		status.gameStartTimestamp = block.timestamp;
		// status.initialPrice - unchange for now
		// status.endPrice - unchange for now
		// status.winner - unchange for now
	 ...
	 **/
	function startGame() external override onlyAfter(status.gameStartTimestamp.add(status.durationOfBidding)) {
		...
		// transfer to aave
		supplyToAAVE(principalToken, IERC20(principalToken).balanceOf(address(this)));
	}

	/**
	 * @dev end the gaming, redeem assets from aave and get end price
	 **/
	function endGame() external override onlyAfter(status.gameStartTimestamp.add(status.durationOfBidding).add(status.durationOfGame)) {
		...
		// status.isFirstRound = false;
		// status.isFirstUser = true;
		status.currState = PoolStatus.Liquidating;

		// redeem from AAVE
		redeemFromAAVE(principalToken);
		// get end price
		status.endPrice = _getPrice();
	}

	/**
	 * @dev determine winner - either short group or long group, calculate share value
	 **/
	function selectWinners() external override onlyAfter(status.gameStartTimestamp.add(status.durationOfBidding).add(status.durationOfGame)) {
		require(status.currState == PoolStatus.Liquidating, "LosslessV2Pool: THE STATUS SHOULD BE LIQUIDATING WHEN SELECTWINNERS().");
		...
		emit WinnerWinner(status.winner, description);

		// update share value
		updateShareValue(totalShortPrincipal, totalLongPrincipal, totalPausePrincipal, totalPrincipal);
		// modify status variable
	 ...
	 * @param _shortPrincipalAmount how many principal in short pool does user want to deposit
	 * @param _longPrincipalAmount how many principal in long pool does user want to deposit
	 **/
	function addPrincipalAmount(uint256 _shortPrincipalAmount, uint256 _longPrincipalAmount) external override {
		require(status.currState == PoolStatus.Accepting, "LosslessV2Pool: WRONG POOL STATUS");
		require(_shortPrincipalAmount > 0 || _longPrincipalAmount > 0, "LosslessV2Pool: INVALID AMOUNT");
		...
		SafeERC20.safeTransferFrom(IERC20(principalToken), msg.sender, address(this), _shortPrincipalAmount.add(_longPrincipalAmount));
		mint(msg.sender, _shortPrincipalAmount, _longPrincipalAmount);
	}
	 ...
	 **/
	function reducePrincipalAmount(uint256 _shortPrincipalAmount, uint256 _longPrincipalAmount) external override {
		require(
		...
		require(shortShareBalance >= _shortShareAmount, "LosslessV2Pool: INVALID AMOUNT");
		require(longShareBalance >= _longShareAmount, "LosslessV2Pool: INVALID AMOUNT");
		...
		if (status.currState == PoolStatus.Locked) {
			...
			burnDuringLock(msg.sender, _shortPrincipalAmount, _longPrincipalAmount);
		} else {
			// if during the accepting time
			// burn user's shares
			burn(msg.sender, _shortPrincipalAmount, _longPrincipalAmount);
		}

		// transfer principal token back
		SafeERC20.safeTransfer(IERC20(principalToken), msg.sender, _shortPrincipalAmount.add(_longPrincipalAmount));
	}
	...
	function withdrawAll() external override {
		...
		inPoolTimestamp[msg.sender] = block.timestamp;
		// burn user's shares
		burn(msg.sender, shortShareBalance, longShareBalance);
		// transfer principal token back
		uint256 _shortPrincipalAmount = shortShareBalance.mul(valuePerShortShare).div(PRECISION);
		uint256 _longPrincipalAmount = longShareBalance.mul(valuePerLongShare).div(PRECISION);
		SafeERC20.safeTransfer(IERC20(principalToken), msg.sender, _shortPrincipalAmount.add(_longPrincipalAmount));
	}
	 ...
	 **/
	function swap(bool fromLongToShort, uint256 _swapShareAmount) external override {
		...
		if (fromLongToShort == true) {
			...
			_burn(msg.sender, 0, _swapShareAmount);
			_mint(msg.sender, _swapShareAmount.mul(valuePerLongShare).div(PRECISION), 0);
		} else {
			// user wants to shift from short to long, so burn short share and increase long share
			_burn(msg.sender, _swapShareAmount, 0);
			_mint(msg.sender, 0, _swapShareAmount.mul(valuePerShortShare).div(PRECISION));
		}
	}
	 ...
	 **/
	function pauseGaming() external override {
		require(status.currState == PoolStatus.Accepting, "LosslessV2Pool: WRONG POOL STATUS");
		uint256 shortShareBalance = shortShareBalanceOf[msg.sender];
		uint256 longShareBalance = longShareBalanceOf[msg.sender];
		...
		convertShareToPauseShareAmount(msg.sender, shortShareBalance, longShareBalance);
	}

	/**
	 * @dev user can resue gaming, and select which direction want to bid
	 *	   one limitation: user can only call this during the unlock time
	 * @param fromPauseToShort whether user swap to short or not
	 **/
	function resumeGaming(bool fromPauseToShort) external override {
		require(status.currState == PoolStatus.Accepting, "LosslessV2Pool: WRONG POOL STATUS");
		...
		if (fromPauseToShort == true) {
			...
			mint(msg.sender, userPrincipalAmount, 0);
		} else {
			// if user choose to resume long
			mint(msg.sender, 0, userPrincipalAmount);
		}
	}
	 ...
	 **/
	function convertShareToPauseShareAmount(
		address to,
	 ...
	 **/
	function supplyToAAVE(address asset, uint256 amount) private {
	 ...
	 * @param asset the address of the principal token
	 **/
	function redeemFromAAVE(address asset) private {
		// lendingPool
		address lendingPoolAddress = ILendingPoolAddressesProvider(addressProvider).getLendingPool();
		ILendingPool lendingPool = ILendingPool(lendingPoolAddress);
	 ...
	 **/
	function redeemFromAAVEwithPrincipalAmount(address _asset, uint256 _amount) private {
		// lendingPool
		address lendingPoolAddress = ILendingPoolAddressesProvider(addressProvider).getLendingPool();
		ILendingPool lendingPool = ILendingPool(lendingPoolAddress);
		// protocol data provider
	...
	function burnDuringLock(
		...
		// redeem from AAVE
		redeemFromAAVEwithPrincipalAmount(principalToken, _shortPrincipalAmount.add(_longPrincipalAmount));
	...
	function _mint(
		...
		shortShareBalanceOf[to] = shortShareBalanceOf[to].add(_shortShareAmount);
		longShareBalanceOf[to] = longShareBalanceOf[to].add(_longShareAmount);
		// emit Transfer(address(0), to, shortShareAmount, longShareAmount);
	}
	...
	function _burn(
		...
		totalShortShares = totalShortShares.sub(_shortShareAmount);
		totalLongShares = totalLongShares.sub(_longShareAmount);
		// emit Transfer(from, address(0), shortShareAmount, longShareAmount);
	}
	...
	function gameStartTimestamp() external view override returns (uint256) {
		return status.gameStartTimestamp;
	}
	...
	function winner() external view override returns (uint256) {
...
interface ILosslessV2Pool {
	...
	struct GameStatus {
		...
		uint256 gameStartTimestamp; // the timestamp when game starts
		uint256 initialPrice; // game initial price
		uint256 endPrice; // game end price
		uint256 winner; // 2 short user win, 3 long user win
	...
	/*  
      ...
      *****
        Liquidating - withdraw all funds from defi lending protocol
      ...
      *******  
                     ...
                     /FirstGame/            /Locked/           /Liquidating/            /Terminated/
                          |                     |                    |                       |
    [startFirstRound] ---------> [startGame] ---------> [endGame] ---------> [selectWinners] ---> [poolTermination]
                                      ^                                          |  |
                                      |                                          |  record time
                                      --------------------------------------------
                                                             |
    ...
    */
	...
	event WinnerWinner(uint256 winner, string description);
	...
	function gameStartTimestamp() external view returns (uint256);
    ...
    */
	...
	function selectWinners() external; // called after endGame, after selectWinners, users can add / reduce principals
	...
	/* 
		addPrincipalAmount: 	adding funds can be either just long or short or both
		reducePrincipalAmount: 	reduce funds can be either just long or short or both
		...
		pauseGaming:			stop actively speculate the market, user will instead just hold dai
		resumeGaming:			resume actively speculate the market, user will rejoin the regular game
    */
	function addPrincipalAmount(uint256 _shortPrincipalAmount, uint256 _longPrincipalAmount) external;

	function reducePrincipalAmount(uint256 _shortPrincipalAmount, uint256 _longPrincipalAmount) external;

	function withdrawAll() external;

	function swap(bool fromLongToShort, uint256 _swapAmount) external;

	function sponsorDeposit(uint256 principalAmount) external;

	function sponsorWithdraw(uint256 amount) external;

	function pauseGaming() external;

	function resumeGaming(bool fromPauseToShort) external;
}
...
interface ILosslessV2Factory {
	///@dev define poolType
	/*   
        Param     Value        Launched on
        poolType - 1: weekly - Monday 
        poolType - 2: weekly - Tuesday
        poolType - 3: weekly - Wednesday
        poolType - 4: weekly - Thursday
        poolType - 5: weekly - Friday
        poolType - 6: weekly - Saturday
        poolType - 7: weekly - Sunday
    */
