...
contract CrowdPresale {
    ...
    address payable public richFactoryAddress; // address that creates the presale contracts
    ...
    function cancelAndTransferTokensToPresaleCreator() external {
        ...
        if (cakeLiquidityAdded && richFactoryAddress != msg.sender) {
            revert();
        }
    ...
    function collectFundsRaised() external onlyRichFactory {
        require(!presaleCancelled);
        require(totalCollectedWei >= softCapInWei, "Soft cap reached");
        require(block.timestamp >= closeTime, "Not yet closed");
    ...
    function burnUnsoldTokens() external onlyRichFactory {
        require(cakeLiquidityAdded);
        require(!presaleCancelled);
        require(block.timestamp >= closeTime + 15 minutes);
    ...
    function setLiquidityAdded() external onlyRichFactory {
        require(!cakeLiquidityAdded, "Liquidity has already been added");
        cakeLiquidityAdded = true;
    }

    function setPresaleCancelled() external onlyPresaleCreatorOrRichFactory {
        require(!cakeLiquidityAdded, "Presale has already been canceled");
        presaleCancelled = true;    
        closeTime = block.timestamp;
    }

    function setClosedCrowdSale() external onlyRichFactory {
        closeTime = block.timestamp;  
    }
}