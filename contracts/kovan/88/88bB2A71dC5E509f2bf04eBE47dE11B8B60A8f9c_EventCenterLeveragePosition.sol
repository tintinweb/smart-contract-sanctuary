...
contract EventCenterLeveragePosition is Ownable {
    ...
    bool public epochRoundAutoStart = false;
    ...
    event StartEpoch(address indexed owner, uint256 epochRound);
    ...
    function triggerStartEpoch() external onlyOwner {
        tryAutoStartEpoch();
    }

    function tryAutoStartEpoch() internal {
        if (epochEnd < block.timestamp) {
            if (epochRoundAutoStart == true) {
                epochStart = epochEnd;
                epochEnd = epochEnd + epochInterval;
                if (epochStart == uint256(0)) {
                    epochStart = block.timestamp;
                    epochEnd = epochStart + epochInterval;
                    epochRound = epochRound + 1;
                    emit StartEpoch(msg.sender, epochRound);
                    ...
                    while (epochEnd < block.timestamp) {
                        epochRound = epochRound + 1;
                        epochEnd = epochEnd + epochInterval;
                        epochStart = epochStart + epochInterval;
                        emit StartEpoch(msg.sender, epochRound);
                    }
                }
            }
    ...
    function secToEpochEnd() public view returns ( uint256 _secToEpochEnd){
        if(epochEnd < block.timestamp){
            _secToEpochEnd = 0;
        } else {
    ...
    function isInRewardEpoch() public view returns ( bool _isInRewardEpoch){
        if(epochEnd < block.timestamp){
