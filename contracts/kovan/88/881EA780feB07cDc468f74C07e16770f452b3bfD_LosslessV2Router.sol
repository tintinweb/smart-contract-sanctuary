...
import "../interfaces/ILosslessV2Router.sol";
...
contract LosslessV2Router is ILosslessV2Router {
  ...
  function getBidToken(address poolAddress) public view override returns(address bidToken) {
    bidToken = LosslessV2Pool(poolAddress).bidToken();
  }

  function getPrincipalToken(address poolAddress) public view override returns(address principalToken) {
    principalToken = LosslessV2Pool(poolAddress).principalToken();
  }


  function getPoolTokens(address poolAddress) external view override returns (address longToken, address shortToken, address sponsorToken){
    (longToken, shortToken, sponsorToken) =  LosslessV2Pool(poolAddress).poolTokensInfo();
  }

  function getPoolTokenValue(address poolAddress) external view override returns (uint256 valuePerShortToken, uint256 valuePerLongToken, uint256 valuePerSponsorToken) {
    valuePerShortToken = LosslessV2Pool(poolAddress).valuePerShortToken();
    valuePerLongToken = LosslessV2Pool(poolAddress).valuePerLongToken();
    valuePerSponsorToken = LosslessV2Pool(poolAddress).valuePerSponsorToken();
  }

  function getPoolStatus(address poolAddress) external view override returns (ILosslessV2Pool.PoolStatus) {
    (bool isShortLastRoundWinner, bool isFirstUser, bool isFirstRound, uint256 gameRound, uint256 durationOfGame, uint256 durationOfBidding, uint256 lastUpdateTimestamp, int256 initialPrice, int256 endPrice,ILosslessV2Pool.PoolStatus state) = LosslessV2Pool(poolAddress).status();
    return state;
  }

  function getPoolDurations(address poolAddress) external view override returns (uint256, uint256) {
    (bool isShortLastRoundWinner, bool isFirstUser, bool isFirstRound, uint256 gameRound, uint256 durationOfGame, uint256 durationOfBidding, uint256 lastUpdateTimestamp, int256 initialPrice, int256 endPrice,ILosslessV2Pool.PoolStatus state) = LosslessV2Pool(poolAddress).status();
    return (durationOfBidding, durationOfGame);
  }

  function getInitialPrice(address poolAddress) external view override returns (int256) {
    (bool isShortLastRoundWinner, bool isFirstUser, bool isFirstRound, uint256 gameRound, uint256 durationOfGame, uint256 durationOfBidding, uint256 lastUpdateTimestamp, int256 initialPrice, int256 endPrice,ILosslessV2Pool.PoolStatus state) = LosslessV2Pool(poolAddress).status();
    return initialPrice;
  }

  function deposit(address poolAddress, uint256 shortPrincipalAmount, uint256 longPrincipalAmount) external override {
    // no need to check balance (transaferFrom gets revert with insuffient balance)
    address principalToken = getPrincipalToken(poolAddress);
    SafeERC20.safeTransferFrom(IERC20(principalToken), msg.sender, address(this), shortPrincipalAmount.add(longPrincipalAmount));
    LosslessV2Pool(poolAddress).deposit(shortPrincipalAmount, longPrincipalAmount, msg.sender);
    emit Deposit(poolAddress, shortPrincipalAmount, longPrincipalAmount);
  }

  function withdraw(address poolAddress, bool isAToken, uint256 shortTokenAmount, uint256 longTokenAmount) external override {
    // no need to check poolToken balance. (revert at pool contract when user has insuffient pool token balance)
    LosslessV2Pool(poolAddress).withdraw(isAToken, shortTokenAmount, longTokenAmount, msg.sender);
    emit Withdraw(poolAddress, isAToken, shortTokenAmount, longTokenAmount);
  }

  function sponsorDeposit(address poolAddress, uint256 principalAmount) external override {
    // no need to check balance (transaferFrom gets revert with insuffient balance)
    address principalToken = getPrincipalToken(poolAddress);
    SafeERC20.safeTransferFrom(IERC20(principalToken), msg.sender, address(this), principalAmount);
    LosslessV2Pool(poolAddress).sponsorDeposit(principalAmount, msg.sender);
    emit SponsorDeposit(poolAddress, principalAmount);
  }

  function sponsorWithdraw(address poolAddress, uint256 sponsorTokenAmount) external override {
    // no need to check poolToken balance. (revert at poolToken burn when user has insuffient pool token balance)
    LosslessV2Pool(poolAddress).sponsorWithdraw(sponsorTokenAmount, msg.sender);
    emit SponsorWithdraw(poolAddress, sponsorTokenAmount);
  }

  function swap(address poolAddress, bool fromLongToShort, uint256 swapTokenAmount) external override{
    LosslessV2Pool(poolAddress).swap(fromLongToShort, swapTokenAmount, msg.sender);
  }
...
// SPDX-License-Identifier: GPL-3.0
pragma solidity 0.6.12;
import "./ILosslessV2Pool.sol";

interface ILosslessV2Router {

  event Deposit			(address poolAddress,uint256 shortPrincipalAmount, uint256 longPrincipalAmount);
	event Withdraw			(address poolAddress, bool isAToken, uint256 shortTokenAmount, uint256 longTokenAmount);
	event SponsorDeposit	(address poolAddress, uint256 principalAmount);
	event SponsorWithdraw	(address poolAddress, uint256 sponsorTokenAmount);

  function factory() external view returns (address);

  function getBidToken(address poolAddress) external view returns (address bidToken);
  function getPrincipalToken(address poolAddress) external view returns (address principalToken);
  function getPoolTokens(address poolAddress) external view returns(address shortToken, address longToken, address sponsorToken);
  function getPoolTokenValue(address poolAddress) external view returns (uint256 valuePerShortToken, uint256 valuePerLongToken, uint256 valuePerSponsorToken);
  function getPoolStatus(address poolAddress) external view returns(ILosslessV2Pool.PoolStatus currState);
  function getPoolDurations(address poolAddress) external view returns(uint256 durationOfBidding, uint256 durationOfGame);
  function getInitialPrice(address poolAddress) external view returns(int256 initialPrice);
  
  function deposit(address poolAddress, uint256 shortPrincipalAmount, uint256 longPrincipalAmount) external;

  function withdraw(address poolAddress, bool isAToken, uint256 shortTokenAmount, uint256 longTokenAmount) external;

  function sponsorDeposit(address poolAddress, uint256 principalAmount) external;

  function sponsorWithdraw(address poolAddress, uint256 sponsorTokenAmount) external;

  function swap(address poolAddress, bool fromLongToShort, uint256 swapTokenAmount) external;
...
contract LosslessV2Pool is ILosslessV2Pool, KeeperCompatibleInterface{
	 ...
	 **/
	function startGame() public override onlyAfter(status.lastUpdateTimestamp.add(status.durationOfBidding)) {
	 ...
	 **/
	function endGame() public override onlyAfter(status.lastUpdateTimestamp.add(status.durationOfGame)) {
	 ...
	 **/
	function deposit(uint256 shortPrincipalAmount, uint256 longPrincipalAmount, address user) external override {
		require(status.currState == PoolStatus.Accepting, "LosslessV2Pool: WRONG STATUS");
		...
		if (_shortToken.balanceOf(user) == 0 && _longToken.balanceOf(user) == 0) {
			inPoolTimestamp[user] = block.timestamp;
		}
		// transfer principal to pool contract
		SafeERC20.safeTransferFrom(IERC20(principalToken), msg.sender, address(this), shortPrincipalAmount.add(longPrincipalAmount));
		_mintTokens(true, user, shortPrincipalAmount, longPrincipalAmount);

		emit Deposit(shortPrincipalAmount, longPrincipalAmount, user);
	}
	...
	function withdraw(
		...
		uint256 userShortTokenBalance = _shortToken.balanceOf(user);
		uint256 userLongTokenBalance = _longToken.balanceOf(user);
		...
		_burnTokens(false, user, shortTokenAmount, longTokenAmount);
		...
		if (isAToken == false) {
			...
			SafeERC20.safeTransfer(IERC20(principalToken), user, shortPrincipalAmount.add(longPrincipalAmount));
		} else {
			if (status.currState == PoolStatus.Accepting) {
				_supplyToAAVE(principalToken, shortPrincipalAmount.add(longPrincipalAmount));
			}
			SafeERC20.safeTransfer(IERC20(aToken), user, shortPrincipalAmount.add(longPrincipalAmount));
		}

		emit Withdraw(isAToken, shortTokenAmount, longTokenAmount, user);
	}
	 ...
	 **/
	function swap(bool fromLongToShort, uint256 swapTokenAmount, address user) external override {
		require(status.currState == PoolStatus.Accepting, "LosslessV2Pool: WRONG STATUS");
		uint256 shortTokenBalance = _shortToken.balanceOf(user);
		uint256 longTokenBalance = _longToken.balanceOf(user);
		...
		if (fromLongToShort == true) {
			...
			_burnTokens(false, user, 0, swapTokenAmount);
			_mintTokens(false, user, swapTokenAmount.mul(valuePerLongToken).div(valuePerShortToken), 0);
		} else {
			// user wants to shift from short to long, so burn short share and increase long share
			_burnTokens(false, user, swapTokenAmount, 0);
			_mintTokens(false, user, 0, swapTokenAmount.mul(valuePerShortToken).div(valuePerLongToken));
		}
	}
	 ...
	 **/
	function sponsorDeposit(uint256 principalAmount, address user) external override {
		...
		// mint sponsor token
		_sponsorToken.mint(user, principalAmount);

		emit SponsorDeposit(principalAmount, user);
	}
	 ...
	 **/
	function sponsorWithdraw(uint256 sponsorTokenAmount, address user) external override {
		require(sponsorTokenAmount > 0, "LosslessV2Pool: INVALID AMOUNT");
		// burn user sponsor token
		_sponsorToken.burn(user, sponsorTokenAmount);
		...
		SafeERC20.safeTransfer(IERC20(principalToken), user, sponsorTokenAmount);

		emit SponsorWithdraw(sponsorTokenAmount, user);
	}
...
interface ILosslessV2Pool {
    ...
    */
	...
	event Deposit			(uint256 shortPrincipalAmount, uint256 longPrincipalAmount, address user);
	event Withdraw			(bool isAToken, uint256 shortTokenAmount, uint256 longTokenAmount, address user);
	event SponsorDeposit	(uint256 principalAmount, address user);
	event SponsorWithdraw	(uint256 sponsorTokenAmount, address user);
    ...
    */
	function deposit(uint256 shortPrincipalAmount, uint256 longPrincipalAmount, address user) external;
	function withdraw(bool isAToken, uint256 shortTokenAmount, uint256 longTokenAmount, address user) external;
	function swap(bool fromLongToShort, uint256 swapTokenAmount, address user) external;
	
	function sponsorDeposit(uint256 principalAmount, address user) external;
	function sponsorWithdraw(uint256 sponsorTokenAmount, address user) external;
}
