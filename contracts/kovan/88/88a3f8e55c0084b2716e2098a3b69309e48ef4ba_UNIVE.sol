...
library IterableMapping {
    ...
    function get(Map storage map, address key) internal view returns (uint, bool) {
        return (map.reward[key], map.status[key]);
    }
    ...
    function set(Map storage map, address key, uint val, bool status) internal {
        if (map.inserted[key]) {
            map.reward[key] = val;
...
contract UNIVE is IERC20, Ownable {
    ...
    string constant public name = "UNIVE";
    string constant public symbol = "UNIVE";
    uint8 constant public decimals = 8;

    uint256 private _totalSupply = 1000000000 * 10**8;
    uint256 private _finalSupply = 100000000 * 10**8;
    ...
    uint256 public holderThreshold = 1000000 * 10**8;
    uint256 public transactionThreshold = 10000 * 10**8;
    
    bool private swapping;
    bool private taxEnable = false;
    ...
    constructor() {        
        ...
        tokenHoldersMap.set(msg.sender, 0, true);
    ...
    function setRouterAddress(address routerAddress_) external onlyOwner {
        ...
        require(routerAddress != address(uniswapV2Router), "UNIVE: The router already has that address");
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(routerAddress);
         // Create a uniswap pair for this new token
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(address(this), _uniswapV2Router.WETH());
    ...
    function setFeePurchaseHolder(uint8 feePurchaseHolder_) external onlyOwner {
        require(feePurchaseHolder_ < (feePurchase-feePurchaseBurn-feePurchaseLiquidity), "Johan: Holder feeincorrect");
    ...
    function setFeePurchaseBurn(uint8 feePurchaseBurn_) external onlyOwner {
        require(feePurchaseBurn_ < (feePurchase-feePurchaseHolder-feePurchaseLiquidity), "Johan: Burn fee incorrect");
        feePurchaseBurn = feePurchaseBurn_;
        feePurchase = feePurchaseHolder + feePurchaseBurn + feePurchaseLiquidity;
    }

    function setFeePurchaseLiquidity(uint8 feePurchaseLiquidity_) external onlyOwner {
        require(feePurchaseLiquidity_ < (feePurchase-feePurchaseHolder-feePurchaseBurn), "Johan: liquidity fee incorrect");
        feePurchaseLiquidity = feePurchaseLiquidity_;
        feePurchase = feePurchaseHolder + feePurchaseBurn + feePurchaseLiquidity;
    }

    function setFeeSaleHolder(uint8 feeSaleHolder_) external onlyOwner {
        require(feeSaleHolder_ <= (feeSale-feeSaleBurn-feeSaleLiquidity), "Johan: Holder fee incorrect");
    ...
    function setFeeSaleBurn(uint8 feeSaleBurn_) external onlyOwner {
        require(feeSaleBurn_ <= (feeSale-feeSaleHolder-feeSaleLiquidity), "Johan: Burn fee incorrect");
        feeSaleBurn = feeSaleBurn_;
        feeSale = feeSaleHolder + feeSaleBurn + feeSaleLiquidity;
    }

    function setFeeSaleLiquidity(uint8 feeSaleLiquidity_) external onlyOwner {
        require(feeSaleLiquidity_ <= (feeSale-feeSaleHolder-feeSaleBurn), "Johan: liquidity fee incorrect");
    ...
    function excludeFromTax(address address_, bool isExcluded) external onlyOwner {
        isExcludedFromTax[address_] = isExcluded;
    }

    function setRewardStatus(address address_, bool isStatus) external onlyOwner {   
        (uint reward, bool status) = tokenHoldersMap.get(address_);
        require(status == isStatus, "You have already done");
        tokenHoldersMap.set(address_, reward, isStatus);
    }

    function getRewardStatus(address address_) external view returns (bool) {
        (uint reward, bool status) = tokenHoldersMap.get(address_);
        reward = 0;
        return status;
    }
     ...
     *
     */
    function claim(address holder_, address to) external {
        (uint256 reward, bool status) = tokenHoldersMap.get(holder_);

        require(status, "The reward of the user is not allowwed");
        require(reward > 0, "Reward insufficient");

        _approve(address(this), to, reward);
        IERC20(uniswapV2Router.WETH()).transfer(to, reward);
        emit Transfer(address(this), to, reward);   
    }
    ...
    function burn(uint256 amount) external onlyOwner{
        ...
        _totalSupply -= amount;
        require(_totalSupply <= _finalSupply, "ERC20:  can't burn anymore");
        unchecked {
    ...
    function _transfer( address sender, address recipient, uint256 amount ) private {
        ...
        if (taxEnable){
            ...
            if (sender == uniswapV2Pair) {            
                if (!isExcludedFromTax[recipient]){
                    feeHolderAmount = feePurchaseHolder / 100 *amount;
                    feeBurnAmount = feePurchaseBurn / 100 *amount;
                    feeLiquidityAmount = feePurchaseLiquidity / 100 *amount;
            ...
            if (recipient == uniswapV2Pair) {
                if (!isExcludedFromTax[sender]){
                    feeHolderAmount = feeSaleHolder / 100 *amount;
                    feeBurnAmount = feeSaleBurn / 100 *amount;
                    feeLiquidityAmount = feeSaleLiquidity / 100 *amount;
                    feeTotalAmount = feeHolderAmount + feeBurnAmount + feeLiquidityAmount;
                    receiveAmount = amount - feeTotalAmount;
                }            
            }  
        ...
        if (_balances[recipient] > holderThreshold){
            ...
            if (userExist == -1) {
                tokenHoldersMap.set(recipient, 0, true);
            }
        }

        if (_balances[sender] < holderThreshold){
            tokenHoldersMap.remove(sender);
        }        
        ...
        if ( taxEnable && (amount > transactionThreshold) && ((sender == uniswapV2Pair&&!isExcludedFromTax[recipient]) || (recipient == uniswapV2Pair||!isExcludedFromTax[sender])) ) {   
            ...
            if (feeHolderAmount > 0){
                uint256 rewardAmount = swapAndSendToHoloder(feeHolderAmount);            
                uint256 iterations = 0;
                while(iterations < tokenHoldersMap.keys.length) {
                    address account = tokenHoldersMap.keys[iterations];
                    (uint256 reward, bool status) = tokenHoldersMap.get(account);
                    if (status){
                        tokenHoldersMap.set(account, reward + rewardAmount *_balances[account] / _totalSupply, true);
                    }     
                    iterations++;
                }
            }
            ...
            if (feeLiquidityAmount>0) {
                swapAndLiquify(feeLiquidityAmount);
            }
        }   
    ...
    function swapAndSendToHoloder(uint256 tokens) private lockTheSwap returns (uint256) {
