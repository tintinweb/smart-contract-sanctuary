    ...
    NFT721Mint
{
    ...
    Counters.Counter private nextCollectionId;
    ...
    struct Collection {
        ...
        uint256 protagonistId;
        string protagonistName;
        address owner;
        mapping(uint256 => bool) assets;
        string perk;
        address wearableToken;
        uint256 wearable;
        CollectionStatus collectionStatus;
    }
    mapping(uint256 => Collection) public collection;

    event CreatedCollection(uint256 collectionId);
    event UpdatedCollection(uint256 collectionId);
    event DeletedCollection(uint256 collectionId);

    function initialize(address minter, address admin) public initializer {
    ...
    function createCollection(
        ...
        uint256 protagonistId,
        string memory protagonistName,
        string memory perk,
        address wearableToken,
        uint256 wearable
    ) external returns (uint256 collectionId) {
        bytes memory validateCollectionName = bytes(collectionName);

        require(
            validateCollectionName.length > 0,
            "Collection Name can't be empty"
        );

        nextCollectionId.increment();
        collectionId = nextCollectionId.current();
        Collection storage initiateCollection = collection[collectionId];
        initiateCollection.collectionName = collectionName;
        initiateCollection.collectionDescription = collectionDescription;
        initiateCollection.protagonistId = protagonistId;
        initiateCollection.protagonistName = protagonistName;
        initiateCollection.owner = msg.sender;
        initiateCollection.perk = perk;
        initiateCollection.wearableToken = wearableToken;
        initiateCollection.wearable = wearable;
        initiateCollection.collectionStatus = CollectionStatus.Registered;

        for (uint256 i = 0; i < assets.length; i++) {
            collection[collectionId].assets[mint(assets[i])] = true;
        }

        emit CreatedCollection(collectionId);
    }

    function updateCollection(
        uint256 collectionId,
        uint256[] memory burnAssets,
        string[] memory newAssets
    ) external {
        Collection storage collectionUpdated = collection[collectionId];

        require(
            collectionUpdated.collectionStatus == CollectionStatus.Registered,
            "Action Not valid"
        );
        require(collectionUpdated.owner == msg.sender, "Not authorized");

        for (uint256 i = 0; i < burnAssets.length; i++) {
            require(
                collectionUpdated.assets[burnAssets[i]] == true,
                "Asset donot exist"
            );
            _burn(burnAssets[i]);
            delete collectionUpdated.assets[burnAssets[i]];
        }

        for (uint256 j = 0; j < newAssets.length; j++) {
            collection[collectionId].assets[mint(newAssets[j])] = true;
        }

        emit UpdatedCollection(collectionId);
    }

    function deleteCollection(uint256 collectionId, uint256[] memory burnAssets)
        external
    {
        // Collection storage collectionDelete = collection[collectionId];

        require(collection[collectionId].owner == msg.sender, "Not authorized");
        require(
            collection[collectionId].collectionStatus ==
                CollectionStatus.Registered,
            "Action not valid"
        );

        for (uint256 i = 0; i < burnAssets.length; i++) {
            require(
                collection[collectionId].assets[burnAssets[i]] == true,
                "Asset donot exist"
            );
            _burn(burnAssets[i]);
        }
        ...
        emit DeletedCollection(collectionId);
    }
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal virtual override(ERC721Upgradeable, NFT721Metadata, NFT721Mint) {
...
abstract contract NFT721Mint is Initializable, NFT721Creator, NFT721Metadata {
     ...
     */
    function mint(string memory tokenURIPath)
        internal
        virtual
    ...
    function _burn(uint256 tokenId)
        internal
        virtual
        override(NFT721Creator, NFT721Metadata)
    {
        super._burn(tokenId);
    }
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal virtual override(ERC721Upgradeable, NFT721Metadata) {
        super._beforeTokenTransfer(from, to, tokenId);
    }
