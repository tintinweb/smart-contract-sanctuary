...
contract AaveInvestmentStrategy is InvestmentStrategyBase {
    ...
    returns (uint)
    {
        ...
        IERC20Upgradeable(token).safeApprove(address(lendingPool), amount);
    ...
    function withdrawInvestment(address token, uint256 amount, bytes calldata)
    external
    onlyInvestor
    nonReentrant
    override
    {
        ILendingPool lendingPool = ILendingPool(lendingPoolAddressesProvider.getLendingPool());
        lendingPool.withdraw(token, amount, composableHolding);
    }


    function claimTokens(bytes calldata data) external override returns (address) {
        ...
        IERC20Upgradeable(reserve.aTokenAddress).safeTransfer(
            composableHolding, IERC20Upgradeable(reserve.aTokenAddress).balanceOf(address(this))
        );
