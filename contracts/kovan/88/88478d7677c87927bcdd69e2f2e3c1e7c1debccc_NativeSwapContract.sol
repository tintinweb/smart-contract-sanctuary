...
// File: contracts/swap-contracts/NativeSwapContract.sol
...
contract NativeSwapContract is Ownable
{
        ...
        uint128 _numOfThisBlockchain)
    {
        ...
        require(
            ...
            "NativeSwapContract: Wrong numOfTotalBlockchains"
        );
        require(
            _numOfThisBlockchain < _numOfTotalBlockchains,
            "NativeSwapContract: Wrong numOfThisBlockchain"
        );
        numOfTotalBlockchains = _numOfTotalBlockchains;
        numOfThisBlockchain = _numOfThisBlockchain;
    }

    function transferToOtherBlockchain(uint128 blockchain, string memory newAddress) external payable
    {
        require(
            bytes(newAddress).length > 0,
            "NativeSwapContract: No destination address provided"
        );
        require(
            blockchain < numOfTotalBlockchains && blockchain != numOfThisBlockchain,
            "NativeSwapContract: Wrong choose of blockchain"
        );
        require(
            msg.value > feeAmountOfBlockchain[blockchain],
            "NativeSwapContract: Not enough amount of tokens"
        );
        require(
            msg.value > 0,
            "NativeSwapContract: Amount must be > 0"
        );
        emit TransferToOtherBlockchain(blockchain, _msgSender(), msg.value, newAddress);
    }

    function transferToUserWithoutFee(address payable user, uint256 amount) external onlyOwner
    {
        TransferHelper.safeTransferETH(user, amount);
        emit TransferFromOtherBlockchain(user, amount);
    }

    function transferToUserWithFee(address payable user, uint256 amountToUser) external onlyOwner
    {
        uint256 fee = feeAmountOfBlockchain[numOfThisBlockchain];
        TransferHelper.safeTransferETH(user, amountToUser.sub(fee));
