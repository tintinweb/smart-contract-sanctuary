...
abstract contract CoreUtility {
    ...
    // 14 hours
    uint256 internal constant SETTLEMENT_TIME = 5 minutes;
    ...
    function _endOfWeek(uint256 timestamp) internal pure returns (uint256) {
        ...
        return ((timestamp.add(1 hours) - SETTLEMENT_TIME) / 1 hours) * 1 hours + SETTLEMENT_TIME;
    }
}
...
interface IChessController {
    function getFundRelativeWeight(address account, uint256 timestamp) external view returns (uint256);
}
...
contract Staking is ITrancheIndex, CoreUtility {
    ...
    function rewardWeight(
        ...
        return
            amountM.mul(REWARD_WEIGHT_M).add(amountA.mul(REWARD_WEIGHT_A)).add(amountB.mul(REWARD_WEIGHT_B)) /
            REWARD_WEIGHT_M;
    }
    ...
    function deposit(uint256 tranche, uint256 amount) public {
        ...
        _availableBalances[msg.sender][tranche] = _availableBalances[msg.sender][tranche].add(amount);
        ...
        )
    {
        ...
        if (amountVersion < rebalanceSize) {
            (amountM, amountA, amountB) = fund.batchRebalance(amountM, amountA, amountB, amountVersion, rebalanceSize);
        }
    ...
    function _checkpoint(uint256 rebalanceSize) private {
        ...
        uint256 weeklyPercentage = chessController.getFundRelativeWeight(address(this), endWeek - 1 weeks);
        uint256 version = _totalSupplyVersion;
        ...
        for (uint256 i = 0; i < MAX_ITERATIONS && timestamp_ < block.timestamp; i++) {
            ...
            if (weight > 0) {
                integral = integral.add(
                    rate.mul(endTimestamp.sub(timestamp_)).multiplyDecimal(weeklyPercentage).divideDecimalPrecise(
                        weight
                    )
                );
            }
    ...
    function _userCheckpoint(address account, uint256 targetVersion) private {
        ...
        // This scope is to avoid the "stack too deep" error.
        {
            ...
            integral = targetVersion == rebalanceSize ? _invTotalWeightIntegral : _historicalIntegrals[targetVersion];
        }
        ...
        for (uint256 i = oldVersion; i < targetVersion; i++) {
            uint256 weight = rewardWeight(availableM.add(lockedM), availableA.add(lockedA), availableB.add(lockedB));
            rewards = rewards.add(weight.multiplyDecimalPrecise(_historicalIntegrals[i].sub(userIntegral)));
            if (availableM != 0 || availableA != 0 || availableB != 0) {
                (availableM, availableA, availableB) = fund.doRebalance(availableM, availableA, availableB, i);
            }
