...
contract SPVWallet is ISPVWallet {
    ...
    constructor(address _MCBoard){
        require(
            _MCBoard != address(0),
            "SPVSC: INVALID_CONTRACT_ZERO_ADDRESSES"
        );
    ...
    function createFundTransferRequest(address currency, uint amount, address to)
        ...
        onlyMCBoard
    {
        require(currency != address(0), "SPVSC: INVALID_CURRENCY_ZERO_ADDRESSES");
        require(amount > 0,             "SPVSC: INVALID_AMOUNT");
        require(
            amount <= IERC20(currency).balanceOf(address(this)),
            "SPVSC: AMOUNT_TOO_LARGE"
        );
        require(to != address(0),       "SPVSC: INVALID_CONTRACT_ZERO_ADDRESSES");
    ...
    function approveFundTransferRequest() external override onlyMCBoard {
        ...
        require(request.amount != 0, "SPVSC: INVALID_AMOUNT");
