...
contract Precog {
    ...
    {
        address factory = IFutureExchangeRouter(trade.futureExchange).futureTokenFactory();
        uint reserveAExchange = IERC20(usdc).balanceOf(trade.exchange);
        uint reserveBExchange = IERC20(pairs[1]).balanceOf(trade.exchange);
        (uint reserveAFutureExchange, uint reserveBFutureExchange) = PrecogV2Library.getReservesFuture(factory, usdc, pairs[1], expiryDate);
        ...
        if (sqrtK > reserveAExchange * reserveAFutureExchange) {
...
interface test {
    ...
    function maxProfitable(address user) external returns (
        uint256 investAmount,
        uint256 profitAmount,
        TradeInfo memory trade,
        BorrowInfo memory borrow
