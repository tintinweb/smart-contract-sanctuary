    ...
    BumperAccessControl
{
    ...
    function getBumpRewards(uint256 _totalDeposit, uint256 _amountForPurchase)
        ...
        returns (uint256)
    {
        uint256 curentTimestamp = block.timestamp;
        if (curentTimestamp < swapAllowedPeriod.start ||
            curentTimestamp > swapAllowedPeriod.end
        ) {
            return 0;
        }
        if (
        ...
        uint256 bumpRewards = 
            (_amountForPurchase * 10**12 * 10**4) / (getSwapRateBumpUsdc() * 10);
        return bumpRewards;
    }
