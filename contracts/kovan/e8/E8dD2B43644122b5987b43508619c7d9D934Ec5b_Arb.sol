...
contract Arb is Ownable, Initializable {
    ...
    uint256 public decimalDistance = 12;
    ...
    function arb(
        ...
        (uint256 gyrOutput, ) = poolInstance.swapExactAmountIn(
            tokenIn,
            inputAmount,
            tokenOut,
            limitReturnAmount,
        ...
        require(gyrOutput >= minTotalAmountOut, "arb: ERR_LIMIT_OUT");

        address[] memory tokensOut = new address[](1);
        tokensOut[0] = usdc;

        uint256[] memory amountsOut = new uint256[](1);
        amountsOut[0] = getEstimateAmount(gyrOutput);
    ...
    function getEstimateAmount(uint256 gydAmount)
        public
        view
        returns (uint256)
    {
        uint256 usdcAmount = gyd2usdc(gydAmount);
        ...
        uint256 usdcResult = 0;
        for (uint256 i = 0; i < 100; i++) {
            usdcAmount = usdcDesc(usdcAmount);
            amountsOut[0] = usdcAmount;
            uint256 gydEstimate = GYDLib(gydLib).estimateRedeemedGyro(
                tokensOut,
                amountsOut
            );
            if (gydEstimate < gydAmount) {
                return usdcAmount;
            }
        }
        return 0;
    ...
    function gyd2usdc(uint256 gydAmount) public view returns (uint256) {
        if (gydAmount == 0) {
            return 0;
        }
        return gydAmount.div(10**decimalDistance);
    }

    function usdcDesc(uint256 usdcAmount) public view returns (uint256) {
        if (usdcAmount == 0) {
            return 0;
        }
        return usdcAmount.mul(99).div(100);
    }
