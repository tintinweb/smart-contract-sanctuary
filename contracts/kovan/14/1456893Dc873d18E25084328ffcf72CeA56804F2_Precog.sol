...
contract Precog {
    ...
    function maxProfitable(address user) public returns (
        ...
        for (uint256 i = 0; i < exchanges.length; i++) {
            ...
            IFutureExchangeFactory factory = IFutureExchangeFactory(IExchangeRouter(exchanges[i]).factory());
            for (uint j = 0; j < factory.allPairsLength(); j++){
                IFutureExchangePair _pair = IFutureExchangePair(factory.allPairs(j));
                if (_pair.token0() == usdc || _pair.token1() == usdc) {
