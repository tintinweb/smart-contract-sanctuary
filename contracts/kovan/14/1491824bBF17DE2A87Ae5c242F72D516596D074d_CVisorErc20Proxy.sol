...
import "./CToken.sol";
import "./PriceOracle/interfaces/IVisorVault.sol";
 ...
 * @author Compound
 */
contract CVisorErc20 is CToken, CExternalVaultErc20Interface {

    modifier vaultOwner(address vaultAddress) {
        require(IVisorVault(vaultAddress).owner() == msg.sender, "Not owner of vault");
        _;
    }
     ...
     * @param underlyingFrom The Visor vault address for the msg sender
    ...
    function mint(
        ...
        uint256 mintAmount        
    ) external vaultOwner(underlyingFrom) override returns (uint256) {
        (uint256 err, ) = mintInternal(MintParams({
            minter: msg.sender,
            underlyingFrom: underlyingFrom,
    ...
    ) external vaultOwner(underlyingTo) override returns (uint256) {
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _addReserves(address underlyingFrom, uint256 addAmount) external override returns (uint256) {
    ...
    function doTransferIn(
        ...
        IVisorVault(from).delegatedTransferERC20(underlying, address(this), amount);
...
contract CVisorErc20Proxy is TransparentUpgradeableProxy {
...
interface IVisorVault {
    ...
    function approveTransferERC20(address token, address delegate, uint256 amount) external;
