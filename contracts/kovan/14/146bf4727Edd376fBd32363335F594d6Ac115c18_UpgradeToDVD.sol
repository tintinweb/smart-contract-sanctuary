...
contract UpgradeToDVD is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function addWhilteList(address[] memory _addresses, uint256[] memory _amounts) external onlyOwner {
    ...
    function upgradeToDvd(uint256 _amount) external payable onlyEOA nonReentrant returns(uint256 dvdAmount) {
    ...
    function doAirdrop(address[] memory _addresses) external onlyOwner {
        require(0 < _addresses.length, "No address input");
