...
contract Collection is ERC721, Ownable, VRFConsumerBase {
     ...
     */
    uint256 internal link_fee = 0.1 * 10 * 18 wei; // 0.1 LINK (ETH Kovan)
    address internal _link = 0xa36085F69e2889c224210F603D836748e7dC0088; //Kovan
     ...
     */
    IReferrals referral;
    uint public constant referralFee = 20_000_000_000_000_000 wei; // 0.02 ETH

    /*
     * Variables/Constants for BASC Sale
     */
    uint256 public constant mint_fee = 330_000_000_000_000_000 wei; // 0.33 ETH
    ...
    modifier ValidMint(uint mintNumber) {
        if(saleIsActive){
            require(mint_fee.mul(mintNumber) == msg.value, "Ether value sent is not correct");
            require(mintNumber <= maxPurchase, "Can only mint a limited number of tokens at a time during presale");
    ...
    constructor(string memory name, string memory symbol, uint maxNftSupply, address _referrals, string memory _cid) 
        ERC721(name, symbol) 
        VRFConsumerBase(_vrfCoordinator, _link) {
            cid = _cid;
            collectionSize = maxNftSupply;
    ...
    /*
     * Returns the current number of minted collectables
     */
    function totalSupply() public view returns (uint minted) {
    ...
    */
    ...
    function getRandomNumber() public onlyOwner returns (bytes32 requestId) {
        ...
        require(randomness == 0, "Randomness has alreay been set.");
    ...
    /*
     * Sets the starting index for the collection
     */
    function setStartIndex() public onlyOwner {
        require(startIndex == 0, "The start index has already been set.");
        require(randomness != 0, "The randomness for the collection is not fullfilled.");
        startIndex = randomness % collectionSize;
    }

    /*
     * Overrides the base tokenuri to use randomness for collection sequence
     */
    function tokenURI(uint256 tokenId) public view virtual override(ERC721) returns (string memory) {
        ...
        uint collectableId = ( (tokenId + startIndex) % collectionSize );

        return string(abi.encodePacked(baseURI, collectableId));
    }
    ...
    * Mint Collectable
    */
    function mint(uint numberOfTokens, uint _refID) public payable ValidMint (numberOfTokens) returns(uint minterRefId) {
        require(totalSupply().add(numberOfTokens) <= collectionSize, "Purchase would exceed max supply of the collection");
