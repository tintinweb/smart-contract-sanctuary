...
contract MasterChef is Ownable {
    ...
    function emergencyWithdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(pending > 0) {
            ...
            uint256 emergencyClaimPenalties = 0;
            if (block.timestamp < user.lastDeposit.add(vestTimeLock)) {
                uint256 passedTime = block.timestamp.sub(user.lastDeposit);
                claimAble = claimAble.mul(passedTime).div(vestTimeLock);
                emergencyClaimPenalties = pending.sub(claimAble);
            }

            buni.mint(msg.sender, claimAble);

            if (emergencyClaimPenalties > 0) {
                buni.mint(treasury, emergencyClaimPenalties);
            }
        }
    ...
    function emergencyRedeemBuni(uint256 _tokenId) public {
        ...
        if (block.timestamp < vestedAt) {
            uint256 passedTime = block.timestamp.sub(createdAt);
            claimAble = claimAble.mul(passedTime).div(vestTimeLock);
            emergencyClaimPenalties = amount.sub(claimAble);
        }
