...
contract DogeIndex is Context, IERC20, Ownable {
    ...
    string private _name = "Doge Index";
    string private _symbol = "DOGE INDEX";
...
// This make it secure from bots
    ...
    modifier launchRestrict(address sender, address recipient, uint256 amount) {
        if (_tradingTime > now) {
            require(sender == owner() || recipient == owner(), "Doge Index: transfers are disabled");
        } else if (_tradingTime <= now && _restrictionLiftTime > now) {
            require(amount <= _maxRestrictionAmount, "Doge Index: amount greater than max limit in restricted mode");
            if (!_isWhitelisted[sender] && !_isWhitelisted[recipient]) {
                require(_lastTx[sender].add(60) <= now && _lastTx[recipient].add(60) <= now, "Doge Index: only one tx/min in restricted mode");
                _lastTx[sender] = now;
                _lastTx[recipient] = now;
            } else if (!_isWhitelisted[recipient]){
                require(_lastTx[recipient].add(60) <= now, "Doge Index: only one tx/min in restricted mode");
                _lastTx[recipient] = now;
            } else if (!_isWhitelisted[sender]) {
                require(_lastTx[sender].add(60) <= now, "Doge Index: only one tx/min in restricted mode");
                _lastTx[sender] = now;
            }
        }
        _;
