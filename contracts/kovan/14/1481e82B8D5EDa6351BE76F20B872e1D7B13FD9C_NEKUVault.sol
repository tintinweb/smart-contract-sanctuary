...
interface INEKUPool {
    function stakeBehalf(address _account, address _nToken, uint256 _amount, uint256 _pid) external;
    function balanceOf(address _account,address _nToken,uint256 _pid) external view returns (uint256);
...
// File: contracts/NEKUVaultBSC.sol
...
contract NEKUVault is ERC20, Ownable, INEKUVaultMigrateable{
    ...
    constructor (
        ...
        string(abi.encodePacked("V-", _symbol))
    ) {
        token = IERC20(_token);
    ...
    function setNekuPid(uint256 _pid) external onlyOwner{
        nekuPid = _pid;
    }
    ...
    function depositInternal(address _account, uint _amount, bool _autoFarm) internal {
        ...
        if (nekuPool != address(0) && _autoFarm && lendingToken != address(0)) {
            _mint(address(this), _shares);
            IERC20(address(this)).safeApprove(nekuPool, _shares);
            INEKUPool(nekuPool).stakeBehalf(_account, lendingToken, _shares, nekuPid);
        } else {
    ...
    function accrueReward(address _account) public view returns (uint) {
        ...
        if (nekuPool != address(0)) {
            gb = gb.add(INEKUPool(nekuPool).balanceOf(_account,lendingToken,nekuPid));
        }
        uint amount = 0;
