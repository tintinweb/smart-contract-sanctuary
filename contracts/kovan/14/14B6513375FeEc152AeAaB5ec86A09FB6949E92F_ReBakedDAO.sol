...
contract ReBakedDAO is IReBakedDAO, Ownable, ReentrancyGuard, Projects {
	...
	*/
    ...
    function approvePayment(bytes32 _projectId, bytes32 _packageId) external onlyOwner {
        _getMgp(_projectId, _packageId);
        _paidBonus(_projectId, _packageId);
    }
	...
	*/
	...
	{
        require(!isDispute[collaborator_][packageId_]);
		amount_ = packageData[projectId_][packageId_].bonus 
				* collaboratorData[projectId_][packageId_][msg.sender].bonusScore / PCT_PRECISION;
	...
	*/
		...
		returns(uint256 amount_)
	{
		...
		for (uint256 i = 0; i < packageIds_.length; i++) {
            require(!isDispute[collaborators_[i]][packageIds_[i]]);
			_amount = packageData[projectId_][packageIds_[i]].bonus 
				* collaboratorData[projectId_][packageIds_[i]][msg.sender].bonusScore / PCT_PRECISION;
			amount_ += _amount;
