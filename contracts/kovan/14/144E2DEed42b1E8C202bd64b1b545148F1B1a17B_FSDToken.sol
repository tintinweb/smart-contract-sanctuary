 ...
 *
 * Overview:
 * This is an implimentation of a dividend-paying token. the token supports income/dividends in FSC.
 * A fixed number of tokens are minted in the constructor, and initially owned by the contract owner.
 * Dividends are awarded token holders thusly:
 *
...
contract FSDToken is ERC20PresetFixedSupply, ERC20Permit, ERC20Votes, IDividendToken {

  string constant NAME = "FreeSpeechCoinDAO";
  string constant SYMBOL = "FSD";
  uint256 constant INITIAL_SUPPLY = 50000000 ether;

  struct tokenHolder {
    uint256 currentFSCPoints; // updated before token balance changes, or before a withdrawal. credit for owning tokens
    uint256 lastFSCSnapshot;  // snapshot of global TotalPoints (FSC), last time we updated this acct's currentFSCPoints
  }
  IERC20  public FSCToken;
  uint256 public holdoverFSCBalance;
  uint256 public totalFSCReceived;
  mapping (address => tokenHolder) public tokenHolders;


  constructor(address owner)
    ERC20Permit(NAME)
    ERC20PresetFixedSupply(NAME, SYMBOL, INITIAL_SUPPLY, owner)
  {
  }


  //
  // this function is for payment in FSC
  // caller must have already approved the FSC transfer
  //
  function payDividend(uint256 amount)
    external
    override
  {
    require(FSCToken.transferFrom(msg.sender, address(this), amount), "failed to transfer FSC");
    holdoverFSCBalance += amount;
    totalFSCReceived += amount;
  }


  //
  // updateFSCBalance
  // update the dia holdover balance, in case someone transfers FSC directly
  // to the contract. anyone can call this.
  //
  function updateFSCBalance()
    external
  {
    uint256 _actBalance = FSCToken.balanceOf(address(this));
    uint256 _amountFSC = _actBalance - holdoverFSCBalance;
    holdoverFSCBalance += _amountFSC;
    totalFSCReceived += _amountFSC;
  }
  ...
  // time we updated the account's snapshot of the running point count, totalFSCReceived. during the period the account's
  // number of tokens must not have changed. so always call this fcn before changing the number of tokens.
  //
  function _calcCurPointsForAcct(address acct)
    internal
  {
    uint256 _newDividends = totalFSCReceived - tokenHolders[acct].lastFSCSnapshot;
    uint256 _newPoints = _newDividends * balanceOf(acct);
    tokenHolders[acct].currentFSCPoints += _newPoints;
    tokenHolders[acct].lastFSCSnapshot = totalFSCReceived;
  }
  ...
  // check my dividends
  //
  function checkDividend(address acct)
    external
    view
    returns(uint256 amountFSC)
  {
    //don't call calcCurPointsForAcct here, cuz this is a constant fcn
    uint256 _newDividends = totalFSCReceived - tokenHolders[acct].lastFSCSnapshot;
    uint256 _newPoints = _newDividends * balanceOf(acct);
    uint256 _currentPoints = tokenHolders[acct].currentFSCPoints + _newPoints;
    amountFSC = _currentPoints / totalSupply();
  }
  ...
  {
    _calcCurPointsForAcct(msg.sender);
    amount = tokenHolders[msg.sender].currentFSCPoints / totalSupply();
    uint256 _pointsUsed = amount * totalSupply();
    tokenHolders[msg.sender].currentFSCPoints -= _pointsUsed;
    holdoverFSCBalance -= amount;
    require(FSCToken.transfer(msg.sender, amount), "failed to transfer FSC");
  }


  //
  // must calc current points before any token transfer
  //
