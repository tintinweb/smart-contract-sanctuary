...
contract PreSale{
    ...
    */
    ...
    constructor(address _vault, address _organisation){
        admin = payable(_organisation);
    ...
    modifier onlyVault(){
        require(msg.sender == vault, "Sender not Vault");
        _;
    }
        ...
        if(locked){
            ...
            vault.transfer(msg.value);
             ...
             IERC20(edgexContract).transfer(
                _reciever,
                SafeMath.div(SafeMath.mul(tokens,99),100)
            );
            IERC20(edgexContract).transfer(
                admin,
    ...
    function allocate(uint256 _tokens,address _user, uint256 _method) public onlyAdmin returns(bool){
        ...
        if(locked){
          ...
          purchased[_user] = SafeMath.add(purchased[_user],_tokens);
            ...
            IERC20(edgexContract).transfer(
                _user,
                SafeMath.div(SafeMath.mul(_tokens,99),100)
            );
            IERC20(edgexContract).transfer(
                admin,
    ...
    */
    ...
    function claim() public returns(bool){
        ...
        IERC20(edgexContract).transfer(
            msg.sender,
            SafeMath.div(SafeMath.mul(transferAmount,99),100)
            );
        IERC20(edgexContract).transfer(
            admin,
    ...
    function whitelist(address _user) public onlyAdmin returns(bool){
     ...
     */
    function updateCap(uint256 _minCap, uint256 _maxCap) public onlyAdmin returns(bool){
        maxCap = _maxCap;
        minCap = _minCap;
        return false;
    }

    function lockDistribution(bool _state) public onlyAdmin returns(bool){
        locked = _state;
        return true;
    }
    ...
    function updateVault(address _newVault) public onlyVault returns(bool){
        vault = payable(_newVault);
        return true;
    }
