...
contract OpLeveragePosition is OpCommon {
    ...
    event CleanLong(uint256 amountFLashLoan, uint256 amountTargetToken);
    ...
    function cleanShort(
        ...
        unitAmt = 1000000000000000000000000000000000000;
        closeShort(
            leverageToken,
            targetToken,
            amountTargetToken,
    ...
    function openPosition(
        ...
        if (direction == uint256(1)) {
            ...
            uint256 _temp = abi.decode(_data, (uint256)); // leverageToken price
            ...
            _temp = abi.decode(_data, (uint256)); // LTV
            ...
            _temp = abi.decode(_data, (uint256)); //TargetToken Price
            ...
            require(notOverflow == true, "CHFRY: overflow");
            amountTargetToken = _toWei(targetToken,amountTargetToken);
            emit CleanLong(amountTargetToken,amountFlashLoan);
            openShort(
                leverageToken,
                targetToken,
                amountTargetToken,
    ...
    function cleanPosition(
        ...
        if (direction == uint256(1)) {
            ...
            if (
                ...
                _temp = IERC20Metadata(targetToken).decimals();

                if (_temp < 18) {
                    _temp = 18 - _temp;
                    (notOverflow, amountTargetToken) = SafeMath.tryMul(
                        amountTargetToken,
                        10**_temp
                    );
                    require(notOverflow == true, "CHFRY: overflow");
                ...
                emit CleanLong(amountTargetToken, _temp);
            }
            ...
            (success, _data) = ProtocolCenterInterface(
                OperationCenterInterface(opCenterAddress)
                    ...
                    abi.encodeWithSignature("getUserAccountData()")
                );
            require(
                success == true,
            ...
            (notOverflow, _temp) = SafeMath.tryMul(
                totalDebtETH,
                amountTargetTokenValue
            );
            require(notOverflow == true, "CHFRY: overflow1");

            (notOverflow, _temp) = SafeMath.tryDiv(_temp, totalCollateralETH);
            require(notOverflow == true, "CHFRY: overflow2");

            (notOverflow, _temp) = SafeMath.tryDiv(_temp, price); // flash loan amount
            require(notOverflow == true, "CHFRY: overflow3");

            cleanLong(leverageToken, targetToken, _temp, unitAmt, 2);
        } else {
            ...
            require(notOverflow == true, "CHFRY: overflow 5");
            ...
            require(notOverflow == true, "CHFRY: overflow 6");

            (notOverflow, _temp) = SafeMath.tryDiv(_temp, totalDebtETH);

            require(notOverflow == true, "CHFRY: overflow 7");

            (notOverflow, _temp) = SafeMath.tryDiv(_temp, price); // withdraw amount

            require(notOverflow == true, "CHFRY: overflow 8");

            emit CleanLong(_temp, amountFlashLoan);
    ...
    function _toWei( address token, uint256 value) internal view returns (uint256 amount){
        ...
        if (
            ...
            _temp = IERC20Metadata(token).decimals();

            if (_temp < 18) {
                _temp = 18 - _temp;
                (notOverflow, amount) = SafeMath.tryMul(
                    value,
                    10**_temp
                );
                require(notOverflow == true, "CHFRY: overflow");
