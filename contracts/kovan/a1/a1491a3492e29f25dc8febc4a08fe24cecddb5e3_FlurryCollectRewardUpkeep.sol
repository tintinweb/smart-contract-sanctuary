...
contract FlurryCollectRewardUpkeep is OwnableUpgradeable, IFlurryUpkeep {
    ...
    function checkUpkeep(bytes calldata) external view override returns (bool upkeepNeeded, bytes memory performData) {
        ...
        uint256 encodeArrSize = 0;
        uint256 maxStrategyArrLength = 0;

        for (uint256 i = 0; i < vaults.length; i++) {
            uint256 arrLength = vaults[i].getStrategiesListLength();
            encodeArrSize += arrLength;
            if (arrLength > maxStrategyArrLength) maxStrategyArrLength = arrLength;
        }
        //max size
        encodeArrSize = encodeArrSize * 2;
        uint256[] memory vaultStrategyEncode = new uint256[](encodeArrSize);
        ...
        for (uint256 i = 0; i < vaults.length; i++) {
            ...
            for (uint256 j = 0; j < tempCollectList.length; j++) {
                if (tempCollectList[j] == true) {
                    vaultStrategyEncode[pointer++] = i;
                    vaultStrategyEncode[pointer++] = j;

                    upkeepNeeded = true;
                }
            }
            //delete tempCollectList;
        }
        if (pointer < encodeArrSize) vaultStrategyEncode[pointer] = MAX_INT;

        if (upkeepNeeded) {
            performData = abi.encode(vaultStrategyEncode, maxStrategyArrLength);
        }
    }

    function performUpkeep(bytes calldata performData) external override {
        (uint256[] memory vaultStrategyEncode, uint256 maxStrategyArrLength) =
            abi.decode(performData, (uint256[], uint256));

        bool isEven = true;
        uint256[] memory currentVaultStr = new uint256[](maxStrategyArrLength);
        uint256 currentVaultArrPointer = 0;
        uint256 currentVault;
        bool lastLoop = false;
        for (uint256 i = 0; i < vaultStrategyEncode.length; i++) {
            if (vaultStrategyEncode[i] == MAX_INT) break;
            if ((i + 1) >= vaultStrategyEncode.length) lastLoop = true;
            else if (vaultStrategyEncode[i + 1] == MAX_INT) lastLoop = true;

            if (isEven) {
                // Vault Number
                currentVault = vaultStrategyEncode[i];
            } else {
                //Strategy Number
                currentVaultStr[currentVaultArrPointer++] = vaultStrategyEncode[i];

                if (!lastLoop) {
                    if (vaultStrategyEncode[i + 1] != currentVault) {
                        if (currentVaultArrPointer < maxStrategyArrLength)
                            currentVaultStr[currentVaultArrPointer] = MAX_INT;

                        vaults[currentVault].collectStrategiesRewardTokenByIndex(currentVaultStr);
                        currentVaultArrPointer = 0;
                        currentVaultStr = new uint256[](maxStrategyArrLength);
                    }
                } else {
                    if (currentVaultArrPointer < maxStrategyArrLength)
                        currentVaultStr[currentVaultArrPointer] = MAX_INT;
