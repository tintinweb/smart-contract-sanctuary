...
contract ShyftBALV2LPStaking is Ownable {
  ...
  function pendingReward(
    ...
    if (user.lpAmount > 0 && totalPoolLP > 0 && _currentDate > pool.lastClaimDate) {
      ...
      shyftPerStock = shyftPerStock.add(totalReward.mul(1e12).div(totalPoolLP));
    }

    pendingAmount = user.lpAmount.mul(shyftPerStock).div(1e12).sub(user.preReward);
  }
  ...
  function claim(
    ...
    if (user.lpAmount > 0) {
      uint256 claimAmount = user.lpAmount.mul(pool.shyftPerStock).div(1e12).sub(user.preReward);
      shyftToken.safeTransfer(address(msg.sender), claimAmount);
  ...
  function deposit(
    ...
    user.preReward = user.lpAmount.mul(pool.shyftPerStock).div(1e12);    
  ...
  function readyPool(
    ...
    console.log("Bal pool id is %s and current date is %s", _balPoolId, _currentDate);
    ...
    pool.shyftPerStock = pool.shyftPerStock.add(totalReward.mul(1e12).div(totalPoolLP));
    pool.lastClaimDate = _currentDate;
  }
