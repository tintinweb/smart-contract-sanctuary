...
abstract contract MintLever is IMintLever, Lock, Owner {
    ...
    function init(uint256 _id, address _owner, address _config) override external nonZeroAddress(_owner) lock {
    ...
    function setConfig(address _config) public onlyOwner nonZeroAddress(_config) lock {
...
contract MintLeverRouter is IMintLeverRouter, ConfigBase, ParameterCheck {
    ...
    function getTokenAmountClosePosition(address mintLever, address user, uint256 percentage) external nonZeroAddress(mintLever) nonZeroAddress(user) view returns (address[] memory, uint256[] memory) {
        ...
        bool isBorrow = (0 < _data.getBorrowAmountV1(user));
        (uint256 _totalAmount, , address[] memory _tokens, uint256[] memory _amounts) = IEarning(_mintLever.earning()).getLPTokens();
        for (uint256 i = 0; i < _tokens.length; i++) {
            _amounts[i] = _amount.mul(_amounts[i]).div(_totalAmount);
            if (isBorrow) {
                _amounts[i] = _amounts[i].mul(percentage).div(100);
            }
        }
        return (_tokens, _amounts);
    }
