...
contract Controller is ControllerStorage {
    ...
    function setStrategy(address _pool, address _strategy) external onlyAdmin{
        ...
        require(
            IStrategy(_strategy).token0() == token ||
                IStrategy(_strategy).token1() == token,
            "Invalid strategy"
        );
    ...
    function withdraw(
        ...
        address token0 = IFundPool(_pool0).token();
        address token1 = IFundPool(_pool1).token();
        (amount0, amount1) = token0 == IStrategy(_strategy).token0()
            ? (amount0, amount1)
            : (amount1, amount0);
    ...
    //TODO 1.withraw（withdraw优先级，非提币token转到哪个池子）; 2.净值计算
    function withdraw(uint256 _amount, uint256 _profitAmount) external returns (TokenAmount[] memory){
        ...
        if(totalAmount != 0 || totalProfitAmount != 0){
            for (uint256 j = 0; j < settings.length; j++) {
                address token0 = IStrategy(settings[j].strategy).token0();
                address token1 = IStrategy(settings[j].strategy).token1();
                address[] memory tokens = new address[](2);
                (tokens[0], tokens[1]) = token == token0 ? (token0, token1) : (token1, token0);
...
contract SVaultNetValue {
   ...
   struct PoolWeight{
    ...
    function getNetValues() view external returns(NetValue[] memory netValues){
        ...
        (TokenAmountView memory tokenAmountView, TokenAmountView memory profitTokenAmountView) = getTokenAmounts(fixedPools, flexiblePools);
        ...
        for(uint256 i = 0; i < fixedPools.length; i++){
            ...
            netValues[i].totalTokensInETH = getTokenAmountInETH(netValues[i].token, netValues[i].totalTokensInETH);
            //sub principal
            tokenAmountView.totalAmountInETH = tokenAmountView.totalAmountInETH.sub(netValues[i].amountInETH);
            //sub profit
            profitTokenAmountView.totalAmountInETH = profitTokenAmountView.totalAmountInETH.sub(netValues[i].totalTokensInETH.sub(netValues[i].amountInETH));
            //TODO 处理本金token不够问题，profitToken不够问题
        }
        ...
        uint256 totalTokensInETH = tokenAmountView.totalAmountInETH.add(profitTokenAmountView.totalAmountInETH);
        if(totalTokensInETH < totalAmountInETH){
            totalAmountInETH = totalTokensInETH;
        }else{
            totalProfitAmountInETH = totalTokensInETH.sub(totalAmountInETH);
        }
    ...
    function getTokenAmounts(address[] memory fixedPools, address[] memory flexiblePools) view internal returns(TokenAmountView memory,TokenAmountView memory){
        ...
        TokenAmountView memory profitTokenAmountView = TokenAmountView(new TokenAmount[](strategies.length), 0);
        for(uint256 i = 0; i < strategies.length; i++) {
            (address[] memory tokens, uint256[] memory amounts) = IStrategy(strategies[i]).getTokenAmounts();
            for(uint256 j = 0; j < tokens.length; j++){
                if(j == tokens.length - 1)
                    AddTokenAmount(profitTokenAmountView, tokens[j], amounts[j]);
                else
                    AddTokenAmount(tokenAmountView, tokens[j], amounts[j]);
            }
        }
        return (tokenAmountView,profitTokenAmountView);
    }
