    ...
    ReentrancyGuardUpgradeable
{
        ...
        )
    {
        ...
        if (!isLiquidation) {
            // Calculate the redemption fee ratio.
            etherAmount = vaultLibrary
                .getLiquidationFee(from, token)
                .mul(redemptionFeeRatio)
                .div(1 ether);
            // The fee cannot be over 100%.
            require(etherAmount <= 1 ether, "0<R<=1");
            // Calculate and send protocol fees.
            // Ether amount of redemption fees that will be sent to protocol.
            uint256 protocolRedemptionFees =
                etherAmount.mul(protocolRedemptionFeeRatio).div(1 ether);
            if (protocolRedemptionFees > 0) {
                // Withdraw protocol fees.
                treasury.forceWithdrawAnyCollateral(
                    from,
                    handle.FeeRecipient(),
                    protocolRedemptionFees,
                    token,
                    false
                );
                // Update ether amount with user's cut of redemption fees.
                etherAmount = etherAmount.sub(protocolRedemptionFees);
            }
        }
        {
            // Vault must have enough collateral accounting for redemption fee.
            // Assign etherAmount to redemption value, including full fee.
            etherAmount = handle
                .getTokenPrice(token)
                .mul(amount)
                .mul(etherAmount.add(1 ether))
                .div(1e36); // (1 ether)^2
