...
contract BlackMarketGaming is AccessControl, ERC20("Black Market Gaming", "BMG"), IMintable {
    ...
    modifier onlyMinter() {
        require(hasRole(MINTER_ROLE, msg.sender), "BMG: only minter");
        _;
    }

    // A modifier which checks that the caller has the admin role.
    modifier onlyAdmin() {
        require(hasRole(ADMIN_ROLE, msg.sender), "BMG: only admin");
        _;
    }
