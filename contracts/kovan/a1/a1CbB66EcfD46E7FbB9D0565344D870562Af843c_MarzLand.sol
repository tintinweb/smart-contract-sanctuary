...
contract MarzLand is ERC721EnumerableUpgradeable {
    address public govt;
    ...
    function initialize(uint256[] memory seeds, address govt_, uint256[] memory premine) public initializer {
        ...
        // one-time government pre-mine as much as the block gas limit permits
        for (uint256 i = 0; i < premine.length; i++) {
            uint256 plotID = premine[i];
            // yolo require(plotID < MAX_PLOTS);
            _mint(govt, plotID);
        }
    }
    ...
    function hasAdjacentPlot(uint256 plotID) public view returns (bool) {
        ...
        return (
            _owners[getPlotID(x0, y0)] != address(0) ||
            _owners[getPlotID(x0, y1)] != address(0) ||
            _owners[getPlotID(x0, y2)] != address(0) ||
            _owners[getPlotID(x1, y0)] != address(0) ||
            _owners[getPlotID(x1, y2)] != address(0) ||
            _owners[getPlotID(x2, y0)] != address(0) ||
            _owners[getPlotID(x2, y1)] != address(0) ||
            _owners[getPlotID(x2, y2)] != address(0)
        );
    }
