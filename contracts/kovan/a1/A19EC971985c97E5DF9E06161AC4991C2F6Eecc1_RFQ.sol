    ...
    SignatureValidator
{
    ...
    *************************************************************/
    function fill(
        ...
        returns (uint256)
    {
        // check the order deadline
        require(_order.expirationTimeSeconds >= block.timestamp, "RFQ: expired order");

        // decode & assert
        TradeInfo memory tradeInfo = _assertTransaction(_order, _mmSignature, _userSignature);

        // Validate that the transaction is not seen before
        require(! permStorage.isRFQTransactionSeen(tradeInfo.transactionHash), "RFQ: transaction seen before");
        // Set transaction as seen
        permStorage.setRFQTransactionSeen(tradeInfo.transactionHash);
        ...
        if (address(weth) == tradeInfo.takerAssetAddr) {
            ...
            spender.spendFromUser(_order.takerAddress, tradeInfo.takerAssetAddr, _order.takerAssetAmount);
        }
        // Transfer from maker
        spender.spendFromUser(_order.makerAddress, tradeInfo.makerAssetAddr, _order.makerAssetAmount);

        // settle token/ETH to user
        uint256 settleAmount = _settle(
            _order.makerAddress,
        ...
        returns(TradeInfo memory tradeInfo)
    {
        ...
        tradeInfo.transactionHash = encodeRFQTransactionHash(order);
        tradeInfo.orderHash = getOrderHash(order, mmSignature);
        ...
        require(
            ...
            "RFQ: invalid fee factor"
        );

        require(
            order.feeRecipientAddress != address(0),
    ...
    function _settle(
        ...
        IERC20(takerAssetAddr).safeTransfer(makerAddr, takerAssetAmount);
    ...
    function encodeRFQTransactionHash(RFQLibEIP712.Order memory order) internal pure returns(bytes32 transactionHash) {
        transactionHash = keccak256(
            abi.encode(
                ...
                order.senderAddress,
                decodeERC20Asset(order.makerAssetData),
                decodeERC20Asset(order.takerAssetData),
...
contract RFQLibEIP712 is BaseLibEIP712 {
    ...
    // 0x EIP712 Domain Name value
    string public constant EIP712_ZEROX_DOMAIN_NAME = "0x Protocol";
    string public constant EIP712_ZEROX_DOMAIN_VERSION = "2";
    // 0x EIP712 Domain Separator Schema
    bytes32 public constant EIP712_ZEROX_DOMAIN_SEPARATOR_SCHEMA_HASH = keccak256(abi.encodePacked(
        "EIP712Domain(",
        "string name,",
        "string version,",
    ...
    bytes32 public constant EIP712_ZEROX_DOMAIN_SEPARATOR = keccak256(
        abi.encodePacked(
            EIP712_ZEROX_DOMAIN_SEPARATOR_SCHEMA_HASH,
            keccak256(bytes(EIP712_ZEROX_DOMAIN_NAME)),
            keccak256(bytes(EIP712_ZEROX_DOMAIN_VERSION)),
            bytes12(0),
            0x080bf510FCbF18b91105470639e9561022937712  // zxExchange address
        )
    );

    bytes32 public constant ORDER_TYPEHASH = keccak256(abi.encodePacked(
        "Order(",
        "address makerAddress,",
        "address takerAddress,",
        "address feeRecipientAddress,",
    ...
    /// @param order The order structure.
    /// @return orderHash Keccak-256 EIP712 hash of the order.
    function getOrderHash(Order memory order, bytes memory mmSignature)
        internal
        view
        returns (bytes32 orderHash)
    {
        // Pop last byte off of signature byte array.
        uint8 signatureTypeRaw = uint8(mmSignature[mmSignature.length - 1]);
        SignatureValidator.SignatureType signatureType = SignatureValidator.SignatureType(signatureTypeRaw);
        orderHash = hashEIP712Message(hashOrder(order), signatureType);
        return orderHash;
    }
    ...
    function hashEIP712Message(bytes32 hashStruct, SignatureValidator.SignatureType signatureType)
        internal
        view
        returns (bytes32 result)
    {
        bytes32 eip712DomainHash;
        if (signatureType == SignatureValidator.SignatureType.Wallet) {
            eip712DomainHash = EIP712_ZEROX_DOMAIN_SEPARATOR;
        } else {
            eip712DomainHash = EIP712_DOMAIN_SEPARATOR;
        }
