...
contract RewardPool is LPTokenWrapper, IRewardDistributionRecipient {
    ...
    function setEmiPriceData(
        address _emiPrice,
        address[3] memory _stables,
        uint8 _marketID,
        uint8 _id
    ) public onlyOwner {
        if (emiPrice != _emiPrice) {
            emiPrice = _emiPrice;
        }
        if (stables[0] != _stables[0] || stables[1] != _stables[1] || stables[2] != _stables[2]) {
            stables = _stables;
        }
    ...
    function getStakedValuesinUSD() public view returns (uint256 senderStake, uint256 totalStake) {
        ...
        if (tokenMode == 0) {
            ...
            uint256 _price = IEmiPrice2(emiPrice).getCoinPrices(tokens, stables, marketID)[id];
            senderStake = balanceOf(msg.sender).mul(_price).div(1e18);
            totalStake = totalSupply().mul(_price).div(1e18);
        }
    }
}
