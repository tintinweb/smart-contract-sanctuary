...
contract FeraToken is ERC20("FeraToken", "FERA"), Ownable {
...
interface IMigratorChef {
    // Perform LP token migration from legacy UniswapV2 to FeraSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // FeraSwap must mint EXACTLY the same amount of FeraSwap LP tokens or
...
contract MasterChef is Ownable {
    ...
    // Block number when bonus FERA period ends.
    ...
    // Update the given pool's FERA allocation point. Can only be called by the owner.
    ...
    function pendingFera(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return user.amount.mul(accFeraPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 feraReward = multiplier.mul(feraPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        fera.mint(devaddr, feraReward.div(10));
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                safeFeraTransfer(msg.sender, pending);
            }
        }
        if(_amount > 0) {
