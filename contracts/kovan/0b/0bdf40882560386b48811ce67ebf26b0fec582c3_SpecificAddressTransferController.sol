 ...
 * @dev Allow transfer only to OR from certain addresses
*/
contract SpecificAddressTransferController is TransferController, WhitelistedRole {
    ...
    mapping(address => bool) public _allowMapping;

    constructor() public {
        _addWhitelisted(msg.sender);

        addAllowedFrom(msg.sender);
    }

    function addAllowedFrom(address from) public onlyWhitelisted {
        _allowMapping[from] = true;
    }

    function removeAllowedFrom(address from) public onlyWhitelisted {
        _allowMapping[from] = false;
    }

    /**
     * @dev Only allow transfers to or from allowed addresses
     * @param from address the ether address of sender
     * @param to address the ether address of receiver
     * @return 0 if successful, positive integer if error occurred
     */
    function check(
        ...
        if (_allowMapping[from] == false && _allowMapping[to] == false) {
            return 403;
        }
