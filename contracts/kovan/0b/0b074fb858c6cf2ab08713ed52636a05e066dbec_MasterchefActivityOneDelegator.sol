...
contract ActivityBase is Ownable{
    ...
    address public timeLockAdminer;
    ...
    function checkTimeLockAdminer() internal view {
        require(timeLockAdminer == msg.sender, "invalid authorized");
    }
}
...
contract MasterchefActivityOne is ActivityBase{
    ...
    function initialize(
        ...
        SHARD = _SHARD;
        devAddress = _devAddress;
        timeLockAdminer = _timeLockAdminer;
        if(_shardPerBlock > 0){
    ...
    function setDistributeShardVelocity(uint256 _shardPerBlock, bool _withUpdate) public virtual {
        checkTimeLockAdminer();
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    returns (uint256 _pending, uint256 _potential, uint256 _blockNumber) {
        ...
        if (block.number > pool.lastDividendHeight && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastDividendHeight, block.number);
            uint256 totalUnupdateToken = multiplier.mul(shardPerBlock);
