...
contract RapidUnwind is Ownable, Pausable, ReentrancyGuard {
	...
	function getPath(address _tokenIn, address _tokenOut) private returns (address[] memory path) {
		address pair = IUniswapV2Factory(uniswapV2Factory).getPair(_tokenIn, _tokenOut);
		if (pair == address(0)) {
			path = new address[](3);
			path[0] = _tokenIn;
			path[1] = WETH;
	...
	function _uniswapTokenForToken(
		...
		IUniswapV2Router02(uniRouterV2).swapExactTokensForTokens(amount, expectedAmount, getPath(_tokenIn, _tokenOut), msg.sender, _deadline);
		return true;
	}
	...
	function _uniswapETHForToken(
		...
		IUniswapV2Router02(uniRouterV2).swapExactETHForTokens{ value: msg.value }(expectedAmount, getPath(WETH, _tokenOut), msg.sender, _deadline); // amounts[0] = WETH, amounts[1] = DAI
		return true;
	}

	function _uniswapRemoveLiquidity(
		address _tokenOut,
		address _lpToken,
		uint256 _lpTokenAmount,
		uint256 _deadline
	) private returns (bool) {
		address tokenA = IUniswapV2Pair(_lpToken).token0();
		address tokenB = IUniswapV2Pair(_lpToken).token1();
		(uint256 amountA, uint256 amountB) = IUniswapV2Router02(uniRouterV2).removeLiquidity(tokenA, tokenB, _lpTokenAmount, 0, 0, address(this), _deadline);
		if (tokenA == _tokenOut) {
			IERC20(tokenA).transfer(msg.sender, amountA);
		} else {
			_uniswapTokenForToken(tokenA, _tokenOut, amountA, 0, _deadline);
		}
		if (tokenB == _tokenOut) {
			IERC20(tokenB).transfer(msg.sender, amountB);
		} else {
			_uniswapTokenForToken(tokenB, _tokenOut, amountB, 0, _deadline);
		}
		return true;
	}
	...
	function rapidUnwind(
		...
		for (uint256 i = 0; i < eip712Tokens.length; i++) {
			...
			// If Lp token, do remove liquidity first
			if (IUniswapV2Pair(eip712Tokens[i]).factory() == uniswapV2Factory) {
				_uniswapRemoveLiquidity(_tokenOut, eip712Tokens[i], eip712TokensAmounts[i], deadline);
			} else {
		...
		if (msg.value > 0) {
			_uniswapETHForToken(_tokenOut, 0, deadline);
		}
