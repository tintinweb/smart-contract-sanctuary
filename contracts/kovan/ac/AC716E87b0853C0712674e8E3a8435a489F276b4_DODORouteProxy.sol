    ...
    function _multiSwap(
        ...
        for(uint256 i = 1; i < splitNumber.length; i++) { 
            ...
            for(uint256 j = splitNumber[i-1]; j < splitNumber[i]; j++) {
                ...
                (address pool, address adapter, uint256 direction, uint256 weight, address transferTo) = abi.decode(swapSequence[j], (address, address, uint256, uint256, address));

                if(assetFrom[i-1] == address(this)) {
                    uint256 curAmount = curTotalAmount;
                    if(curTotalWeight > weight) {
                        curAmount = curTotalAmount.div(curTotalWeight).mul(weight);
                    }
                    IERC20(midToken[i]).transfer(transferTo, curAmount);
                }
