 ...
 */
contract Puglife is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    uint256 private _tTotal = 500000 * 10**9 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "PUGLIFE";
    string private constant _symbol = "PUGL";
    ...
    uint256 unit = 1;
    uint256 public _liquidityFee = unit.div(2);
    ...
    uint256 private constant numTokensSellToAddToLiquidity = 10 * 10**18;

    
    uint256 public transferEntry;
    ...
    constructor () {
        ...
        marketing_address = 0xa3547a336973573744083114291de06072C722bf;
    }
    ...
    function _getTValues(uint256 amount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tBurn = 0;
        tBurn = calculateTransactionBurn(tAmount);
    ...
    function _getRValues(uint256 amount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tMarketingFee) private view returns (uint256, uint256, uint256) {
        uint256 currentRate = _getRate();
        uint256 tAmount = amount;
    ...
    function _takeMarketingFee(address sender, uint256 tMarketingFee) private {
        uint256 currentRate =  _getRate();
        uint256 rMarketingFee = tMarketingFee.mul(currentRate);
        ...
        emit Transfer(sender, marketing_address, tMarketingFee);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tBurn, tMarketingFee);

        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity);
        _reflectFee(rFee, tFee);
        _takeMarketingFee(sender, tMarketingFee);
        if(tBurn > 0) {
            _amount_burnt += tBurn;
