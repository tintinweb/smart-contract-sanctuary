...
contract Staking is ReentrancyGuard, Context, IStaking {
    ...
    function availDividends(address user) external view override onlyFactory returns (uint256) {
        (uint256 total, ) = _calcultateReward(user);
        return users[user].amountOfDividends + total;
    }
     ...
     */
    function _upgradeReward(address sender) private {
        (uint256 total, uint256 i) = _calcultateReward(sender);
        users[sender].amountOfDividends += total;
        users[sender].lastRewardId = i;
    }
     ...
     * @param user: address of user
     */
    function _calcultateReward(address user) private view returns (uint256, uint256) {
        ...
        for (; i < rewards.length; i++) {
            if (
                ...
                rewards[i].timestamp - info.lastTimestamp > IStakingFactory(FACTORY).freezeTime()
            ) {
