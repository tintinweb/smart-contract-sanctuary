...
contract WaveStakeISO {
    ...
    constructor(
        address _unistake, address _wave, address _whitelist, 
        address _owner,address _splitWallet, 
        uint _splitShare, uint _period) 
    {
    ...
    function closeEntryAndStartTerm() external onlyOwner() returns(bool) {
        
        require(initialized, "entry has not been initialized");
        require(isOpen, "entry is already closed");
        
        isOpen = false;
        start = block.timestamp;
        
        emit EntryClosed(msg.sender, block.timestamp);
        return true;
    }
    ...
    function stakeUnistake(uint _amount) external returns(bool) {
        ...
        require(initialized, "entry has not been initialized");
        require(isOpen, "cannot stake when entry is closed");
    ...
    function claim() external returns(bool) {
        ...
        require(start != 0, "term has not started");
        ...
        _users[msg.sender].lastTime = block.timestamp;
    ...
    function _calculateReleasedAmount(address _user) internal view returns(uint) {
        ...
        uint lastTime = _users[_user].lastTime;
        uint time = start > lastTime ? start : lastTime;
        uint releasedPct;
        
        if (block.timestamp >= period) releasedPct = 100;
        else releasedPct = ((block.timestamp - time) * 10000) / ((period - time) * 100);
