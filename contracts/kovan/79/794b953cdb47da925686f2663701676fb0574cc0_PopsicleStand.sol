...
contract PopsicleStand is Ownable, ReentrancyGuard {
        ...
        returns (uint256 pendingIce, uint256 pendingRewardToken)
    {
        ...
        if (block.number > lastRewardBlock && lpSupply != 0) {
            ...
            uint256 iceReward =
                multiplier.mul(icePerBlock);
            accIcePerShareTemp = accIcePerShareTemp.add(
                iceReward.mul(1e12).div(lpSupply)
            );
            uint256 pTokenReward =
                multiplier.mul(rewardTokenPerBlock);
            accRewardTokenPerShareTemp = accRewardTokenPerShareTemp.add(
                pTokenReward.mul(1e12).div(lpSupply)
            );
        }
        pendingIce = user.amount.mul(accIcePerShareTemp).div(1e12).sub(user.rewardDebtIce).add(user.remainingIceTokenReward);
        pendingRewardToken = user.amount.mul(accRewardTokenPerShareTemp).div(1e12).sub(user.rewardDebtToken).add(user.remainingTokenReward);
    }
    ...
    function updatePool() internal {
        ...
        uint256 iceReward =
            multiplier.mul(icePerBlock);
        accIcePerShare = accIcePerShare.add(
            iceReward.mul(1e12).div(lpSupply)
        );
        uint256 pTokenReward =
            multiplier.mul(rewardTokenPerBlock);
    ...
    function deposit(uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pendingIceReward =
                user.amount.mul(accIcePerShare).div(1e12).sub(
                    user.rewardDebtIce
                ).add(user.remainingIceTokenReward);
            uint256 pendingRewardToken = 
                user.amount.mul(accRewardTokenPerShare).div(1e12).sub(
                    user.rewardDebtToken
                ).add(user.remainingTokenReward);
        ...
        user.rewardDebtIce = user.amount.mul(accIcePerShare).div(1e12);
        user.rewardDebtToken = user.amount.mul(accRewardTokenPerShare).div(1e12);
