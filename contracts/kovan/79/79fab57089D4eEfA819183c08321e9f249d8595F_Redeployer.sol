...
contract Redeployer {
    ...
    function redeploy(
        ...
        assembly {
            ...
            let totalSize := add(bytecodeSize, argumentsSize)
            bytecode := mload(0x40)
            mstore(0x40, add(bytecode, and(add(totalSize, 0x3f), not(0x1f))))
            mstore(bytecode, totalSize)
            extcodecopy(contract_, add(bytecode, 0x20), 0, bytecodeSize)
            for {
                let i := 0
            } lt(i, argumentsSize) {
                i := add(i, 0x20)
        ...
        emit Redeployed(contract_, result, salt, amount, arguments);
        return result;
    }
}
