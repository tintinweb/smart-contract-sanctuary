...
contract ICO is IERC20 {
    ...
    uint256 private _priceInWei = 1000000000000000;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(!_icoEnd && ((sender != address(this)) || (recipient != _owner))); // no trasfers till ico ends
    ...
    function buy(uint256 count) external payable {
        ...
        uint256 weiValue = msg.value * (10 ^ _decimalsBase);
        uint256 tokens = weiValue / _priceInWei;

        require(tokens > count, "ICO: want more tokens than value");
        require(_balances[address(this)] > tokens, "ICO: less token than free");
