...
contract GenesisSale is ReentrancyGuard, Context, IGenesisSale {
     ...
     */
    function buyEdgex(address _reciever)
        public
        payable
        virtual
        override
        ...
        returns (bool)
    {
        ...
        uint256 lockedTill = p.time + 365 days;
        uint256 orgAmount = p.amount / 100;

        require(!p.isSettled, "Error: amount already claimed");
        require(block.timestamp > lockedTill, "Error: locked for 365 days");
...
interface IGenesisSale {
     ...
     * `_reciever` address has to be whitelisted.
     */
    function buyEdgex(address _reciever) external payable returns (bool);
