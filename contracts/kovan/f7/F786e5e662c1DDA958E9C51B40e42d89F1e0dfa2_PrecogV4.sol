...
contract PrecogV4 {
    ...
    function isInTradingSession(address token) public view returns(bool) {
        ...
        bool isEndTimeGreaterThanNow = tradingSessions[token][tradingSessions[token].length - 1].endTime > block.timestamp;
        bool isStartTimeLessThanNow = tradingSessions[token][tradingSessions[token].length - 1].startTime < block.timestamp;
        return isEndTimeGreaterThanNow && isStartTimeLessThanNow;
    }
