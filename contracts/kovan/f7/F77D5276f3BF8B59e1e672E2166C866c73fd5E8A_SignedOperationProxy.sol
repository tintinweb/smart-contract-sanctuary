...
contract SignedOperationProxy is OnlyDxln, Ownable {
    using SafeMath for uint256;

    // ============ Constants ============

    // bytes32 private constant FILESIGN = "SignedOperationProxy";
    ...
    bytes private constant EIP712_DOMAIN_STRING =
        abi.encodePacked(
            "EIP712Domain(",
            "string name,",
            "string version,",
    ...
    bytes private constant EIP712_OPERATION_STRING =
        abi.encodePacked(
            "Operation(",
            "Action[] actions,",
            "uint256 expiration,",
    ...
    bytes private constant EIP712_ACTION_STRING =
        abi.encodePacked(
            "Action(",
            "uint8 actionType,",
    ...
    bytes private constant EIP712_ASSET_AMOUNT_STRING =
        abi.encodePacked(
            "AssetAmount(",
            "bool sign,",
            "uint8 denomination,",
    ...
    /* solium-disable-next-line indentation */
    bytes32 private constant EIP712_OPERATION_HASH =
        keccak256(
            abi.encodePacked(
    ...
    bytes32 private constant EIP712_ACTION_HASH =
        keccak256(
            abi.encodePacked(EIP712_ACTION_STRING, EIP712_ASSET_AMOUNT_STRING)
        );
    ...
    function cancel(
        ...
        require(auth.header.signer == msg.sender, "Canceler must be signer");
        g_invalidated[operationHash] = true;
        emit LogOperationCanceled(operationHash, msg.sender);
    }

    /**
     * Submits an operation to DxlnMargin. Actions for accounts that the msg.sender does not control
     ...
     *
     * @param  accounts  The accounts to forward to DxlnMargin.operate()
     * @param  actions   The actions to forward to DxlnMargin.operate()
    ...
    function operate(
        ...
        for (uint256 authIdx = 0; authIdx < auths.length; authIdx++) {
            ...
            require(
                auth.header.expiration == 0 ||
                    auth.header.expiration >= block.timestamp,
                "Signed operation is expired"
            );

            // require that the sender matches the authorization
            require(
                auth.header.sender == address(0) ||
                    auth.header.sender == msg.sender,
            ...
            if (auth.signature.length != 0) {
                ...
                require(auth.header.signer == signer, "Invalid signature");
            ...
            for (
                uint256 actionIdx = actionStartIdx;
                actionIdx < actionEndIdx;
                actionIdx++
            ) {
                ...
                if (action.actionType == Actions.ActionType.Transfer) {
                    validateAccountOwner(
                        accounts[action.otherAccountId].owner,
                        signer
                    );
                }
            }
        ...
        require(actionStartIdx == actions.length, "Not all actions are signed");
     ...
     */
    function validateAccountOwner(address accountOwner, address signer)
        private
        view
    {
        bool valid = msg.sender == accountOwner ||
            signer == accountOwner ||
            DXLN_MARGIN.getIsLocalOperator(accountOwner, msg.sender) ||
            DXLN_MARGIN.getIsLocalOperator(accountOwner, signer);

        require(valid, "Signer not authorized");
    }
    ...
    function getOperationHash(
        ...
        bytes32 structHash = keccak256(
            abi.encode(EIP712_OPERATION_HASH, actionsEncoding, auth.header)
        );
    ...
    function getActionsEncoding(
        ...
        for (uint256 i = 0; i < auth.numActions; i++) {
            ...
            Account.Info memory otherAccount = (Actions.getAccountLayout(
                action.actionType
            ) == Actions.AccountLayout.OnePrimary)
     ...
     */
    function getAssetAmountHash(Types.AssetAmount memory amount)
        private
        pure
        returns (bytes32)
    {
