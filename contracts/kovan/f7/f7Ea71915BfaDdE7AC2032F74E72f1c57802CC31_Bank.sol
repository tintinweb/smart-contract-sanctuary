...
interface IGoblin {
    ...
    function work(uint256 id, address user, uint256 amount, uint256 debt, bytes calldata data) external;
...
contract Bank is ERC20, ReentrancyGuard, Ownable {
    ...
    constructor(address _token, IBankConfig _config) public {
        token = _token;
        config = _config;
        lastAccrueTime = now;
    }
    ...
    function deposit(uint256 amount) external payable accrue() nonReentrant {
    ...
    function work(uint256 id, address goblin, uint256 amount, uint256 loan, uint256 maxReturn, bytes calldata data)
        external
        onlyEOA accrue() nonReentrant
    {
        ...
        {
            token.safeTransferFrom(msg.sender, address(this), amount);
            uint256 sendAmount = amount.add(loan);
            uint256 beforeBalance = token.myBalance();
            require(sendAmount <= beforeBalance, "insufficient ETH in the bank");
            token.safeTransfer(goblin, sendAmount);
            IGoblin(goblin).work(id, msg.sender, sendAmount, debt, data);
            back = token.myBalance().add(sendAmount).sub(beforeBalance);
        }
        ...
        if (debt > 0) {
            ...
            uint256 health = IGoblin(goblin).health(id);
        ...
        if (back > lessDebt) token.safeTransfer(msg.sender, back - lessDebt);
    }
    ...
    function kill(uint256 id) external onlyEOA accrue() nonReentrant {
        ...
        uint256 back = token.myBalance().sub(beforeBalance);
        ...
        if (prize > 0) token.safeTransfer(msg.sender, prize);
        uint256 left = rest > debt ? rest - debt : 0;
        if (left > 0) token.safeTransfer(pos.owner, left);
    ...
    /// @param _token The token contract. Can be anything. This contract should not hold ERC20 tokens.
    /// @param to The address to send the tokens to.
    /// @param value The number of tokens to transfer to `to`.
    function recover(address _token, address to, uint256 value) external onlyOwner nonReentrant {
