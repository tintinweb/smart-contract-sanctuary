...
contract ControllerStorage is ControllerAdminStorage {
    ...
    mapping(address => mapping(address => bool)) public approvedStrategies; //pool ID => strategy合约地址 => 在poolStrategies中的index加一(0表示未使用该策略)
...
contract SVaultNetValue {
   ...
   struct PoolWeight{
    ...
    function getNetValues() view public returns(NetValue[] memory netValues){
        ...
        TokenPrice[] memory tokenPrices = new TokenPrice[](fixedPools.length + flexiblePools.length);
        ...
        (PoolWeight[] memory poolWeights, uint256 totalWeight, uint256 totalAmountInUSD, uint256 totalAllocatedProfitInUSD) = getPoolWeights(flexiblePools, tokenPrices);
    ...
    function getPoolWeights(address[] memory flexiblePools, TokenPrice[] memory tokenPrices) view internal returns (PoolWeight[] memory, uint256, uint256, uint256){
...
contract Controller is ControllerStorage {
    ...
    function removeStrategyForTest(address _strategy) external onlyOwner{
    ...
    function withdraw(address _strategy, address _token, address[] memory _pools, uint256[] memory _amounts) external onlyStrategistAndOwner{
        ...
        if(amount == 0) return;
        for(uint256 i = 0; i < _pools.length; i++){
            require(IFundPool(_pools[i]).token() == _token, "Invalid pool");
            ...
            emit Withdraw(_strategy,  _pools[i], _token, _amounts[i]);
        }
        if(amount == 0) return;
    ...
    //             uint256 withdrawAmount = i == length || totalAmount <= settings[j].tokenLimit[i]? totalAmount : settings[j].tokenLimit[i];
    //             if(withdrawAmount == 0 && totalProfitAmount == 0) continue;
    //             address[] memory tokens = new address[](1);
    //             tokens[0] = token;
    ...
    //     }
    //     if(totalAmount != 0 || totalProfitAmount != 0){
    //         uint256 withdrawAmount = totalAmount.add(totalProfitAmount);
    //         for (uint256 j = 0; j < settings.length; j++) {
    //             address[] memory tokens = IStrategy(settings[j].strategy).getTokens();
    //             if(tokens.length == 1) continue;
    //             (tokens[0], tokens[1]) = token == tokens[0] ? (tokens[0], tokens[1]) : (tokens[1], tokens[0]);
    //             (uint256 amount, , address[] memory withdrawTokens, uint256[] memory amounts) = IStrategy(settings[j].strategy).withdraw(tokens, withdrawAmount, 0);
    //             if(amount > 0 ) {
    //                 withdrawAmount = withdrawAmount.sub(amount);
    //                 //subStrategyPoolAmount(msg.sender, settings[j].strategy, amount);
    //                 emit Withdraw(settings[j].strategy, msg.sender, tokens[0], amount);
    //             }
    //             AddTokenAmount(tokenAmounts, withdrawTokens, amounts);
    ...
    //         }
    //         (totalAmount,totalProfitAmount) = withdrawAmount == 0 ? (0,0) : (totalAmount,totalProfitAmount);
    //     }
    //     require(totalAmount == 0 && totalProfitAmount == 0, "Insufficient balance");
    //     for(uint256 i = 0; i < tokenAmounts.length; i++){
