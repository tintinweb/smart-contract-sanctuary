...
contract Administerable is AccessControlEnumerable {
...
contract StakeWarsV1 is Administerable, ERC721Enumerable, VRFConsumerBase {
    ...
    function fulfillRandomness(bytes32 _requestId, uint256 _raritySeed)
        internal
        override
    {
        uint48 rare48 = uint48(_raritySeed);

        address sender = requestIdToUser[_requestId].sender;
        uint256 tokenId = requestIdToUser[_requestId].tokenId;
    ...
    function reserve(uint256 num) public onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 supply = totalSupply();
        for (uint256 i = 0; i < num; i++) {
            bytes32 requestId = requestRandomness(keyhash, linkFee, seed);
