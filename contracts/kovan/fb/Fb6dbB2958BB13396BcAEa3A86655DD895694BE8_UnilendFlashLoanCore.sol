...
contract UFlashLoanPool is ERC20 {
    ...
    function deposit(address _recipient, uint256 amount)
        external
        onlyCore
        returns (uint256)
    {
        ...
        uint256 ntokens =
            calculateShare(_totalSupply, tokenBalance.sub(amount), amount);
    ...
    function redeem(address _recipient, uint256 tok_amount)
        external
        onlyCore
        returns (uint256)
    {
        ...
        require(
            balanceOf(_recipient) >= tok_amount,
            "Balance Exeeds Requested"
        );
        ...
        uint256 poolAmount =
            getShareValue(tokenBalance, totalSupply(), tok_amount);
    ...
    function redeemUnderlying(address _recipient, uint256 amount)
        external
        onlyCore
        returns (uint256)
    {
        ...
        uint256 tok_amount =
            getShareByValue(tokenBalance, totalSupply(), amount);
    ...
    function balanceOfUnderlying(address _address)
        public
        view
        returns (uint256 _bal)
    {
        ...
        if (_balance > 0) {
            ...
            address donationAddress =
                UnilendFlashLoanCore(core).donationAddress();
            uint256 _balanceDonation =
                UnilendFDonation(donationAddress).getCurrentRelease(
                    token,
                    block.timestamp
                );
            uint256 _totalPoolAmount =  (tokenBalance.add(_balanceDonation));
...
contract UnilendFlashLoanCore is Context, ReentrancyGuard {
     ...
     **/
    modifier onlyAdmin {
        require(admin == msg.sender, "The caller must be a admin");
        _;
    }
     ...
     **/
    function getPools(address[] calldata _reserves)
        external
        view
        returns (address[] memory)
    {
        ...
        for (uint256 i = 0; i < _reserves_.length; i++) {
    ...
    function transferFlashLoanProtocolFeeInternal(
        ...
        if (_token != EthAddressLib.ethAddress()) {
            ...
            (bool result, ) =
                distributorAddress.call.value(_amount).gas(50000)("");
            require(result, "Transfer of ETH failed");
        }
    }
    ...
    function flashLoan(
    ...
    ) external nonReentrant onlyAmountGreaterThanZero(_amount) {
        //check that the reserve has enough available liquidity
        uint256 availableLiquidityBefore =
            _reserve == EthAddressLib.ethAddress()
        ...
        (uint256 totalFeeBips, uint256 protocolFeeBips) =
        ...
        returns (uint256 mintedTokens)
    {
        ...
        mintedTokens = UFlashLoanPool(Pools[_reserve]).deposit(
            msg.sender,
            _amount
        );
    ...
    function redeem(address _reserve, uint256 _amount)
        external
        returns (uint256 redeemTokens)
    {
        require(Pools[_reserve] != address(0), "UnilendV1: POOL NOT FOUND");

        UnilendFDonation(donationAddress).releaseTokens(_reserve);

        redeemTokens = UFlashLoanPool(Pools[_reserve]).redeem(
            msg.sender,
            _amount
        );
     ...
     **/
    function redeemUnderlying(address _reserve, uint256 _amount)
        external
        returns (uint256 token_amount)
    {
        ...
        token_amount = UFlashLoanPool(Pools[_reserve]).redeemUnderlying(
            msg.sender,
            _amount
        );
    ...
    function createPool(address _reserve) public returns (address) {
        require(
            Pools[_reserve] == address(0),
            "UnilendV1: POOL ALREADY CREATED"
        );
        ...
        UFlashLoanPool _poolMeta =
            new UFlashLoanPool(_reserve, aTokenName, aTokenSymbol);
    ...
    function createDonationContract() external returns (address) {
        require(
            ...
            "UnilendV1: DONATION ADDRESS ALREADY CREATED"
        );
