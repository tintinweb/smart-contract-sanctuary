...
contract MasterChefV2 is Ownable, ReentrancyGuard {
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 eggReward = (multiplier * eggPerBlock * pool.allocPoint) /
                totalAllocPoint;
            // I don't understand the multiplying by 1e12?
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 eggReward = (multiplier * eggPerBlock * pool.allocPoint) /
            totalAllocPoint;
        // We give 10% egg reward to devs
        egg.mint(devaddr, eggReward / 10);
        // We give the rest of the egg reward to masterchef for distribution
        egg.mint(address(this), eggReward);

        pool.accEggPerShare =
            pool.accEggPerShare +
            ((eggReward * 1e12) / lpSupply);
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
            // Why 1e12?
            uint256 pending = (user.amount * pool.accEggPerShare) /
                1e12 -
                user.rewardDebt;
            if (pending > 0) {
        ...
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                SafeERC20.safeTransfer(pool.lpToken, feeAddress, depositFee);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            user.amount = user.amount - _amount;
            SafeERC20.safeTransfer(pool.lpToken, address(msg.sender), _amount);
        }
        user.rewardDebt = (user.amount * pool.accEggPerShare) / 1e12;
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        user.amount = 0;
        user.rewardDebt = 0;
        SafeERC20.safeTransfer(pool.lpToken, address(msg.sender), amount);
