...
contract CFDViewer is OwnableUpgradeable {
    ...
    function initialize(ICFD _cfd, ISystemSettings _systemSettings) public initializer {
        cfd = _cfd;
    ...
    function getUnrealizedPnl(
        ...
        (, SignedDecimal.signedDecimal memory unrealizedPnl) =
            (cfd.getPositionNotionalAndUnrealizedPnl(_exchange, _trader, _pnlCalcOption));
     ...
     * @param _exchange IExchange address
     * @param _trader trader address
     * @return position CFD.Position struct
     */
    function getPersonalPositionWithFundingPayment(IExchange _exchange, address _trader)
        public
        view
        returns (ICFD.Position memory position)
    {
        position = cfd.getPosition(_exchange, _trader);
        SignedDecimal.signedDecimal memory marginWithFundingPayment =
            MixedDecimal
                .fromDecimal(position.margin)
                .addD(getFundingPayment(position, cfd.getLatestCumulativePremiumFraction(_exchange)))
                .subD(getOvernightFee(position, cfd.getLatestCumulativeOvernightFeeRate(_exchange)));
    ...
    function isPositionNeedToBeMigrated(IExchange _exchange, address _trader) external view returns (bool) {
        ICFD.Position memory unadjustedPosition = cfd.getUnadjustedPosition(_exchange, _trader);
    ...
    function getMarginRatio(IExchange _exchange, address _trader)
        ...
        returns (SignedDecimal.signedDecimal memory)
    {
        return cfd.getMarginRatio(_exchange, _trader);
    }
    ...
    function getOvernightFee(ICFD.Position memory _position, Decimal.decimal memory _latestCumulativeOvernightFee)
        private
        pure
        returns (Decimal.decimal memory)
    {
