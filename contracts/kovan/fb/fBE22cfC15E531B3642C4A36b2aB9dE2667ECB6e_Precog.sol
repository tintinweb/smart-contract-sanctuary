...
contract Precog {
    ...
    ) public view returns (ProfitInfo memory profit) {
                    ...
                    if (loan.platformIndex > 0 && tradeAmount + fee.trading > investAmount) {
                        ...
                        if (investAmount + loan.amount > fee.trading + feeLending) {
                            if (tradeAmount > investAmount + loan.amount - fee.trading - feeLending) {
                                profit.investAmount = tradeAmount = investAmount + loan.amount - fee.trading - feeLending;
                                profit.loanAmount = loan.amount - feeLending - fee.trading;
                            }
                            if (investAmount < tradeAmount && tradeAmount < investAmount + loan.amount - fee.trading - feeLending){
                                profit.investAmount = tradeAmount;
    ...
    function _getPairs(address futureContract) public view returns (address[] memory pairs) {
    ...
    function _getOptimizeTradeAmount(uint expiryDate, address[] memory pairs, TradeInfo memory trade)
        public view returns (uint tradeAmount)
    {
        ...
        uint sqrtK = sqrt(reserveAExchange) * sqrt(reserveBExchange)* sqrt(reserveAFutureExchange) * sqrt(reserveBFutureExchange);
        // unchecked {
