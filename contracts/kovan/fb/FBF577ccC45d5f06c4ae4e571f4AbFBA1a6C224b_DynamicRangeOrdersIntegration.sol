  ...
  IDynamicRangeOrdersIntegration
{
    ...
    onlyController
  {
    ...
    if (
      amount <= token.balanceOf(address(dynamicRangeOrdersIntegrationDeployer))
    ) {
      ...
      if (stablecoinReserveBalance > withdrawalAmountInBaseStablecoinValue) {
        // Only need to make swaps, don't need to close any positions
        dynamicRangeOrdersIntegrationDeployer.swapReservesForWithdrawalToken(
          tokenAddress
        );
        if (
          amount <=
          ...
          dynamicRangeOrdersIntegrationDeployer.closePositionsForWithdrawal(
  ...
  function harvestYield() external override onlyController {
    dynamicRangeOrdersIntegrationDeployer.harvestYield(liquidityPositionKeys);
  }
  ...
  function _deploy() private {
    ...
    for (
      ...
      if (
        positionActualBaseStablecoinValue < positionDesiredBaseStablecoinValue
      ) {
      ...
      uint256 token0Id = integrationMap.getTokenId(
        liquidityPositions[liquidityPositionKeys[liquidityPositionIndex]].token0
      );
      uint256 token1Id = integrationMap.getTokenId(
        liquidityPositions[liquidityPositionKeys[liquidityPositionIndex]].token1
      );
  ...
  function transferTokensTodynamicRangeOrdersIntegrationDeployer() private {
    ...
    for (uint256 tokenId; tokenId < tokenCount; tokenId++) {
      ...
      if (
        ...
        dynamicRangeOrdersIntegrationDeployer.tokenApprovals(address(token));
      }
  ...
  function increaseLiquidityPositions(
    ...
    for (
      ...
      if (
        liquidityPositions[liquidityPositionKeys[liquidityPositionIndex]].minted
      ) {
    ...
    onlyController
  {
    ...
    if (amount0Desired > 0 || amount1Desired > 0) {
      ...
      if (success) {
        ...
        liquidityPositions[liquidityPositionKey].id = liquidityPositionId;
      }
    }
    ...
    returns (uint256 baseStablecoinReserveBalance)
  {
    ...
    for (uint256 tokenId; tokenId < tokenCount; tokenId++) {
      ...
      if (tokenAddress != getBaseStablecoinAddress()) {
        ...
        baseStablecoinReserveBalance += IERC20MetadataUpgradeable(tokenAddress)
  ...
  /// @return The number of configured liquidity positions
  function getLiquidityPositionsCount() public view override returns (uint256) {
    ...
    returns (uint256 tokenValueInBaseStablecoin)
  {
    if (tokenAddress != getBaseStablecoinAddress()) {
      tokenValueInBaseStablecoin = IUniswapTrader(
        moduleMap.getModuleAddress(Modules.UniswapTrader)
      ).getEstimatedTokenOut(tokenAddress, getBaseStablecoinAddress(), amount);
    } else {
  ...
  function getIncreaseLiquidityAmounts(
    ...
    uint128 arbitraryLiquidityAmount = LiquidityAmounts.getLiquidityForAmount0(
      sqrtRatioAX96,
      sqrtRatioBX96,
  ...
  function getSqrtPriceX96(
    ...
    (sqrtPriceX96, , , , , , ) = IUniswapPool(
      IUniswapFactory(factoryAddress).getPool(token0, token1, feeNumerator)
    ).slot0();
  }
}
