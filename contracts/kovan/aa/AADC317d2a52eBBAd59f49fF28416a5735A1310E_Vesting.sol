...
contract Vesting is Core {
     ...
     */
    function getClaimableAmount() public view whenNotPaused
    ...
    function claim() external whenNotPaused { 
        ...
        require(claimable > claimed[msg.sender], "not enough to claim");
        uint256 amount;
        unchecked {
          amount = claimable - claimed[msg.sender];
        }
    ...
    function getBlockchainDataA() external view returns (
      ...
      return (block.timestamp, block.number, token.balanceOf(address(this)), token.balanceOf(msg.sender), paused());
    }
}