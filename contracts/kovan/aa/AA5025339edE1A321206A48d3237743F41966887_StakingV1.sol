...
contract StakingV1 is Ownable {
    ...
    uint public minPeriodTime = 3600; //one hour min times between periods of allocations
    ...
    function calcStakerTimes(uint _s, uint _period) public view returns (uint timeRewards) {
        ...
        } else if (st.timeIn <= p.start && st.timeOut >= p.start && st.timeOut <= p.end) {
            //in the case they were staking before the period but removed before allocation
            timerate = st.timeOut.sub(p.start); //pulls their stub ownership piece
            
        } else if (st.timeIn > p.start && st.isStaking) {
            //if they started staking after the period start and are still staking
            timerate = p.end.sub(st.timeIn);
            
        } else if (st.timeIn > p.start && st.timeOut <= p.end) {
            //if they started staking after period start and then unstaked before period end
            timerate = st.timeOut.sub(st.timeIn);
            
        } else {
            //they were not staking at all during the period
            timerate = 0;
        }
