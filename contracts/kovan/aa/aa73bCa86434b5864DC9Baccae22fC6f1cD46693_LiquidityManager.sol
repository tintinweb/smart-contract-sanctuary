 ...
 * @title Option Liquidity Manager
 */
contract LiquidityManager is Ownable {
    ...
    constructor(IOptionFactory factory, IUniswapV3Factory uniV3Factory, address liquidityVaultInstance) {
        optionFactory = factory;
        uniswapV3Factory = uniV3Factory;
        liquidityVault = liquidityVaultInstance;
    }
    ...
    function createVault(
        ...
        require(optionFactory.lsps(address(lsp)));
        require(vaults[address(lsp)] == address(0x0));

        bytes32 salt = keccak256(abi.encodePacked(address(lsp)));

        address vault = liquidityVault.cloneDeterministic(salt);
        ILiquidityVault(vault).initialize(
            lsp,
            uniswapV3Factory,
            sqrtPriceX96,
            sqrtPriceA,
        ...
        vaults[address(lsp)] = vault;
    }

    function setRebalancer(address newRebalancer) public onlyOwner {
        require(newRebalancer != address(0x0), "LiquidityManager::invalid-address");
