...
contract LosslessV2Pool is ILosslessV2Pool{
	...
	function initialize(
		...
		status.shortToken = shortToken_;
		status.longToken = longToken_;
		status.pauseToken = pauseToken_;
		status.sponsorToken = sponsorToken_;
	...
	function endGame() external override onlyAfter(status.lastUpdateTimestamp.add(status.durationOfGame)) {
		...
		uint256 totalSponsorPrincipal = _sponsorToken.totalSupply();
	...
	function withdraw(uint256 shortTokenAmount, uint256 longTokenAmount) external override {
		...
		require(userShortTokenBalance >= shortTokenAmount, 	"LosslessV2Pool: INSUFFICIENT BALANCE");
		require(userLongTokenBalance >= longTokenAmount, 	"LosslessV2Pool: INSUFFICIENT BALANCE");
	...
	function withdrawAll() external override {
		...
		require(shortTokenBalance > 0 || longTokenBalance > 0, "LosslessV2Pool: NO SHARE IN POOL");
		
		// user withdraw will cause timestamp update -> reduce their goverance reward
		inPoolTimestamp[msg.sender] = block.timestamp;

		// calculate withdraw principal amount
		uint256 shortPrincipalAmount = shortTokenBalance.mul(valuePerShortToken).div(PRECISION);
		uint256 longPrincipalAmount = longTokenBalance.mul(valuePerLongToken).div(PRECISION);

		// burn shares
		_burnTokens(false, msg.sender, shortTokenBalance, longTokenBalance);
	...
	function swap(bool fromLongToShort, uint256 swapTokenAmount) external override {
		require(swapTokenAmount > 0, "LosslessV2Pool: INVALID AMOUNT");
		require(status.currState == PoolStatus.Accepting, "LosslessV2Pool: WRONG STATUS");
		uint256 shortTokenBalance = _shortToken.balanceOf(msg.sender);
		uint256 longTokenBalance = _longToken.balanceOf(msg.sender);
		uint256 tokenBalanceOfTargetPosition = fromLongToShort ? longTokenBalance : shortTokenBalance;
		// check user balance
		require(swapTokenAmount <= tokenBalanceOfTargetPosition, "LosslessV2Pool: INSUFFICIENT BALANCE");
	...
	function sponsorDeposit(uint256 principalAmount) external override {
		...
		// transfer to zero token insdead
		_sponsorToken.mint(msg.sender, principalAmount);

		// emit sponsor share amount
		emit Sponsor(msg.sender, principalAmount);
	...
	function sponsorWithdraw(uint256 sponsorTokenAmount) external override {
		...
		// check current game state
		// uint256 principalAmount = shareAmount.mul(valuePerSponsorToken).div(PRECISION);
		if (status.currState == PoolStatus.Locked) {
			// if during the lock time
			// interact with AAVE to get the principal back
	...
	function resumePrediction(bool fromPauseToShort) external override {
		require(status.currState == PoolStatus.Accepting, "LosslessV2Pool: WRONG STATUS");
		require(_pauseToken.totalSupply() > 0, "LosslessV2Pool: NO PAUSE SHARES IN POOL");
		...
		// _pauseToken.totalSupply() = _pauseToken.totalSupply().sub(_pauseToken.balanceOf(msg.sender));
		...
		if (fromPauseToShort == true) {
			...
			_burnTokens(true, msg.sender, 0, userPrincipalAmount);
		}
	}
	...
	function _updateShareValue(
		...
		if (_pauseToken.totalSupply() != 0) {
			...
			valuePerPauseToken = totalPausePrincipal.mul(PRECISION).div(_pauseToken.totalSupply());
		}
		...
		if (status.isShortLastRoundWinner == true) {
			...
			valuePerShortToken = totalShortPrincipal.mul(PRECISION).div(_shortToken.totalSupply());
			...
			valuePerLongToken = totalLongPrincipal.mul(PRECISION).div(_longToken.totalSupply());
		}
	...
	function _convertTokenToPauseToken(
		...
		_shortToken.burn(to, _shortToken.balanceOf(to)); 
		_longToken.burn(to, _longToken.balanceOf(to));
		
		// update pause share info
		_pauseToken.mint(to, pauseShortTokenAmount.add(pauseLongTokenAmount));
	}
	 ...
	 * @param isPrincipal true: principal, false:share
	 * @param _shortAmount the amount of the principal token to short
	 * @param _longAmount the amount of the principal token to long
	 **/
	function _mintTokens(
		...
		if (_shortAmount != 0) {
			_shortToken.mint(to, _shortAmount);
		}
		if (_longAmount != 0) {
			_longToken.mint(to, _longAmount);		
		}
	}
	...
	function _burnTokens(
		...
		if (_shortAmount != 0) {
			_shortToken.burn(from, _shortAmount);
		}
		if (_longAmount != 0) {
			_longToken.burn(from, _longAmount);
		}
	}
...
interface ILosslessV2Pool {
    ...
    */
	...
	event Sponsor(address indexed sponsor, uint256 sponsorShareAmount);
...
interface ILosslessV2Factory {
	...
	// function getSponsorToken(address pool) external view returns (address sponsorToken);
	// function getShortToken(address pool) external view returns (address shortToken);
	// function getLongToken(address pool) external view returns (address longToken);
	// function getPauseToken(address pool) external view returns (address pauseToken);
	...
	// ) external returns (address sponsorToken, address shortToken, address longToken, address pauseToken);
