...
contract UNIVE is IERC20, Ownable {
    ...
    function getTotalLiquidityFee() external view returns (uint256) {
        return _totalLiquidityFee;
    }
    ...
    function claim(address to) external {
        (uint256 reward, bool status) = tokenHoldersMap.get(_msgSender());

        require(status, "The reward is not allowwed");
        ...
        _approve(address(this), to, newBalance);
        IERC20(uniswapV2Router.WETH()).transfer(to, newBalance);
