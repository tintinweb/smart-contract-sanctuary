...
contract Main is ERC721Full {
    ...
    mapping(address => uint256[]) private songs;

    event SongMinted(address creator, uint256 tokenId, string tokenURI, uint256 price);
    event SongPurchased(address buyer, uint256 tokenId, uint256 price);
    ...
    function userSongs(address user) external view returns (uint256[] memory) {
        return (songs[user]);
    }
    ...
    function createSong(string memory _tokenURI) public payable returns (bool) {
        ...
        emit SongMinted(msg.sender, tokenCount, _tokenURI, msg.value);
        return true;
    }
    ...
    function buySong(uint256 tokenId) public payable returns (bool) {
        ...
        songs[msg.sender].push(tokenId);
        emit SongPurchased(msg.sender, tokenId, msg.value);
        return true;
    }
        ...
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < tokenId.length; i++) {
            ...
            uint256 ownerFee = msg.value.mul(90).div(tokenId.length).div(100);
            adminFee = adminFee.add(ownerFee);
            owner.transfer(ownerFee);
            songs[msg.sender].push(tokenId[i]);
            emit SongPurchased(msg.sender, tokenId[i], msg.value.div(tokenId.length));
        }
        admin.transfer(msg.value.sub(adminFee));
        return true;
    }
