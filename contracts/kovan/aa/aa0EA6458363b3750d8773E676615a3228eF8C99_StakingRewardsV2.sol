...
contract StakingRewards is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    ...
    function notifyRewardAmount(
        ...
        uint256 rewardsDurationCached = rewardsDuration;

        if (block.timestamp >= periodFinish) {
            newRewardRate = _reward.div(rewardsDurationCached);
        } else {
            uint256 remaining = periodFinish.sub(block.timestamp);
            uint256 leftover = remaining.mul(rewardRate);
            newRewardRate = _reward.add(leftover).div(rewardsDurationCached);
        }
        ...
        require(newRewardRate <= balance.div(rewardsDurationCached), "Provided reward too high");

        lastUpdateTime = block.timestamp;
        periodFinish = block.timestamp.add(rewardsDurationCached);
        emit RewardAdded(_reward);
    }
