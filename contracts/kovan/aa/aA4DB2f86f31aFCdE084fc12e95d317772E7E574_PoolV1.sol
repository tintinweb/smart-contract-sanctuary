...
contract PoolV1 {
	...
	function configurePoolContracts(address _exchange, address _weth, address _dai) external systemOnly{
    ...
    function whitelistaddress(address _address, uint _poolIndex) external whitelistManager {
     ...
     */
	function addPublicPool(address[] calldata _tokens, uint[] calldata _weights,uint _threshold,uint _rebalanceTime,string calldata _name,string calldata _symbol) external{
    ...
    */
	function stakeAstra(uint _amount,bool premium)internal{
		IERC20(ASTRTokenAddress).approve(address(_poolConf),_amount);
		IMAsterChef(_poolConf).depositFromDAA(0,_amount,6,msg.sender,premium);
	}	
    ...
    */
    ...
    function getuserbalance(uint _poolIndex) external view returns(uint){
        return poolUserInfo[_poolIndex][msg.sender].currentBalance;
    }

	/**
    * @dev Get total number of pool
    */

    function getTotalInidices() external view returns(uint){
    ...
    */
	...
	function withdraw(uint _poolIndex, bool stakeEarlyFees,bool stakePremium, uint withdrawAmount) external {
		...
		if(_totalAmount>_balance){
			...
			withdrawStable(_totalAmount.sub(fees),stakePremium);
	...
	function withdrawStable(uint256 _amount,bool stake)internal{
		if(stake){
			...
			IERC20(DAI_ADDRESS).transfer(distributor, _amount);			
