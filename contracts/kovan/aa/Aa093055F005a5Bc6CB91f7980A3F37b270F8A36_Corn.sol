...
contract Corn is ERC777, IERC777Recipient, ReentrancyGuard {
    ...
    IERC777 private immutable _lptoken;
    ...
    function tokensReceived(
        ...
        require(
            _msgSender() == address(_token) || _msgSender() == address(_lptoken),
            "You can only build farms on LAND or lpLAND"
        );
...
//        _mint(msg.sender, 1000000 * (10**18), "", ""); // Only required if pre-mine is necessary for migration to L2
    }
     ...
     */
    function allocate(address farmerAddress, uint256 amount, bool lp)
        public
        nonReentrant()
        ...
        requireFarm(_msgSender(), false) // Ensure LAND is not already in a farm
    {
        ...
        // TODO - test for lp and multiply by 3 for lp boost 
        // TODO - get the underlying amount from Uniswap
        ...
        if (!lp) {
            IERC777(_token).operatorSend(_msgSender(), address(this), amount, "", ""); // [RE-ENTRANCY WARNING] external call, must be at the end
                ...
                IERC777(_lptoken).operatorSend(_msgSender(), address(this), amount, "", ""); // [RE-ENTRANCY WARNING] external call, must be at the end
        }
    }
        ...
        requireFarm(_msgSender(), true) // Ensure the address you are releasing has a farm on the LAND
    {
        ...
        if (!senderFarm.lp) {
            ...
            IERC777(_lptoken).send(_msgSender(), amount, ""); // [RE-ENTRANCY WARNING] external call, must be at the end
        }
    }
        ...
        )
    {
        ...
        bool isLPOperator =IERC777(_lptoken).isOperatorFor(address(this), targetAddress);
