...
contract ChainLinkPrice is ChainlinkClient, Ownable, IPriceContract {
    ...
    function updatePrice(
        ...
        urlApi = urlApi.append(symbol).append("-").append("USD").append(
            "/ticker"
        );
        request.add("get", urlApi);
        request.add("path", "price");
        uint decimals = uint(_priceDecimals);
        int times = int(10**decimals);
    ...
    function getBalanceLinkToken() public view returns (uint256) {
        return IERC20(chainlinkOracleAddress()).balanceOf(address(this));
    }
