...
contract LimitOrderManager is
    ...
    function placeLimitOrder(LimitOrderParams calldata params)
...

        {
            ...
            limitOrders[_tokenId] = LimitOrder({
                pool: address(_pool),
                monitor: address(_monitor),
                tickLower: _tickLower,
                tickUpper: _tickUpper,
                liquidity: _liquidity,
                opened: _toUint128(_blockNumber()),
                processed: 0,
    ...
    function processLimitOrder(
        ...
        require(msg.sender == limitOrder.monitor);
        ...
        limitOrder.processed = _toUint128(_blockNumber());
    ...
    function cancelLimitOrder(uint256 _tokenId) external returns (
        ...
        // stop monitor
        IOrderMonitor(limitOrder.monitor).stopMonitor(_tokenId);
        // collect the funds
        _collect(
            _tokenId,
            IUniswapV3Pool(limitOrder.pool),
    ...
    function _selectMonitor() internal returns (IOrderMonitor _monitor) {
        ...
        nextMonitor = uint16(_selectedIndex + 1);
    }
