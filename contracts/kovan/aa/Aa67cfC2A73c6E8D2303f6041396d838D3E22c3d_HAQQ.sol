...
contract HAQQ is ERC20, Ownable {
    ...
    */
    function _buyTokens() private returns (bool success){
        ...
        _mint(msg.sender, tokensAmount * (10 ** decimals()));
    ...
    */
    function exchangeTokens(uint _amountInAcceptedCoins, address _tokenAddress, uint _tier) external returns (bool success){
        ...
        uint amountToMint = acceptedCoinsSumInCents / tiers[currentTier].tokenPriceInUsdCents * (10 ** decimals());

        // if amount is less than required in curren tier, function should revert
        require(amountToMint >= tiers[currentTier].minPurchaseTokens, "The amount ot tokens to exchange is less than the required minimum ");
    ...
    function withdrawAcceptedCoinToOwner(address _acceptedCoinAddress, uint _amount) external onlyOwner returns (bool success){
