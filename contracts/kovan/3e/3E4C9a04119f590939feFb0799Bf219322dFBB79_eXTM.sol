...
contract eXTM is ERC20Interface, SafeMath, Ownable {
    ...
    event BridgeToBSC(address indexed fromAccount, address indexed toAccount, address indexed bridgeAddress, uint256 amount);
    ...
    function burnTo(address _to, uint _amount) public {
        require(_to != address(0), 'ERC20: from address is not valid');
        require(balances[_to] >= _amount, 'ERC20: insufficient balance');
        
        balances[_to] = safeSub(balances[_to], _amount);
    ...
    function transferToBridge(address _offChainToAddress, uint256 _tokenAmount, uint16 _chain) public returns (bool success) {
        ...
        burnTo(onChainBridgeAddress, actualTokenAmount);
        ...
        emit BridgeToBSC(msg.sender, _offChainToAddress, onChainBridgeAddress, actualTokenAmount);
        
        if(actualTransactionFee > 0){
            emit BridgeToBSC(msg.sender, feeVaultAddress, onChainBridgeAddress, actualTransactionFee);
        }
