...
library Constants {
    ...
    /* Oracle */
    address private constant USDC = address(0x1b3924f88A5ee1D9C86b97dce55d99d4B22dB569);
    ...
    address private constant TREASURY_ADDRESS = address(0xE725e8dEad7c1D849186a30a6C6cBaA675f074aF);

    /* PID controlled FOT */
    uint32 private constant INTEGRAL_WEIGHTS_FOT_0 = 5; // weight gets slightly heavier towards present
    uint32 private constant INTEGRAL_WEIGHTS_FOT_1 = 6;
    uint32 private constant INTEGRAL_WEIGHTS_FOT_2 = 7;
    uint32 private constant INTEGRAL_WEIGHTS_FOT_3 = 8;
    uint32 private constant INTEGRAL_WEIGHTS_FOT_4 = 9;
    int256 private constant COEFF_P_FOT = 2e17; // 0.2
    int256 private constant COEFF_I_FOT = 2e17; // 0.2
    int256 private constant COEFF_D_FOT = 8e17; // 0.8

    /* Fee Pool */
    bool private constant AUTOMATIC_LIQUIDITY_PROVIDING_ENABLED = false; // done manually first to ensure efficiency
    uint256 private constant MIN_TOKENS_FOR_LP = 2e21; // 2000 Token
    uint256 private constant LIQUIDITY_PROVIDING_RATIO = 5e17; // 50%
    
    /* Other */
    uint256 private constant FLAT_FEE = 1e16; // 1%
    ...
    function getIntegralWeightsFot() internal pure returns (uint32[] memory)
    {
        uint32[] memory integralWeightsFot = new uint32[](5);
        integralWeightsFot[0] = INTEGRAL_WEIGHTS_FOT_0;
        integralWeightsFot[1] = INTEGRAL_WEIGHTS_FOT_1;
        integralWeightsFot[2] = INTEGRAL_WEIGHTS_FOT_2;
        integralWeightsFot[3] = INTEGRAL_WEIGHTS_FOT_3;
        integralWeightsFot[4] = INTEGRAL_WEIGHTS_FOT_4;
        return integralWeightsFot; 
    }

    function getFlatFee() internal pure returns (uint256)
    {
        return FLAT_FEE;
    }

    function getCoeffPFot() internal pure returns (int256)
    {
       return COEFF_P_FOT;
    }

    function getCoeffIFot() internal pure returns (int256)
    {
       return COEFF_I_FOT;
    }

    function getCoeffDFot() internal pure returns (int256)
    {
       return COEFF_D_FOT;
    }

    function automaticLiquidityProvidingEnabled() internal pure returns (bool)
    {
        return AUTOMATIC_LIQUIDITY_PROVIDING_ENABLED;
    }
    
    function getMinTokensForLp() internal pure returns (uint256)
    {
        return MIN_TOKENS_FOR_LP;
    }

    function getLiquidityProvidingRatio() internal pure returns (uint256)
    {
        return LIQUIDITY_PROVIDING_RATIO;
    }
...
contract IDollar is IERC20 {
    ...
    function permanentlyEnableFees() public;
    function setExemptSender(address sender, bool exempt) public;
    function setExemptRecipient(address recipient, bool exempt) public;
    function setFlatFee(uint256 flatFee_) public;
    function setStabilizingFee(uint256 stabilizingFee_) public;
    function setFeePool(address feePool_) public;
}
...
contract Dollar is IDollar, MinterRole, ERC20Detailed, Permittable, ERC20Burnable, Ownable {

    bool public feeLess;
    mapping(address => bool) public exemptSenders;
    mapping(address => bool) public exemptRecipients;
    address public feePool;
    uint256 public flatFee;
    uint256 public stabilizingFee;

    constructor(address _owner) 
    ERC20Detailed("Flopsy Dollar", "flopsy", 18)
    Permittable()
    public {
        Ownable.init(_owner);
        feeLess = true; // start without FOT for initial token distribution
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public returns (bool) {
        ...
        if (allowance(sender, _msgSender()) != uint256(-1)) { // do not decrease in case it is MAX
            _approve(
                sender,
                _msgSender(),
    ...
    function transfer(address sender, address recipient, uint256 amount) internal {
        ...
        // no fees - buys and Liquidity removing
        if (feeLess || exemptSenders[sender] || exemptRecipients[recipient]) {
            _transfer(sender, recipient, amount);
        }
        // apply flat + stabilizing FOT - sells and general transfers
        else {
            uint256 feeAmount = amount.mul(flatFee.add(stabilizingFee)).div(10**18);
            uint256 actualTransferAmount = amount.sub(feeAmount);
            _transfer(sender, recipient, actualTransferAmount);
            _transfer(sender, feePool, feeAmount);   
        } 
    }

    function permanentlyEnableFees() public onlyMinter() {
        feeLess = false;
    }

    // ownership can be given to impl contract when all relevant contracts for launch are exempted, then new exemptions can be made by DAO
    function setExemptSender(address sender, bool exempt) public onlyOwner() {
        exemptSenders[sender] = exempt;
    }

    function setExemptRecipient(address recipient, bool exempt) public onlyOwner() {
        exemptRecipients[recipient] = exempt;
    }

    /**
    * set by impl contract
    */
    // 1e18 = 100%
    function setFlatFee(uint256 flatFee_) public onlyMinter() {
        flatFee = flatFee_;
    }

    // 1e18 = 100%
    function setStabilizingFee(uint256 stabilizingFee_) public onlyMinter() {
        stabilizingFee = stabilizingFee_;
    }

    function setFeePool(address feePool_) public onlyMinter() {
        feePool = feePool_;
    }
}