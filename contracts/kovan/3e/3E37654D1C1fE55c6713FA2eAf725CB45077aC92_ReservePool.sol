...
interface IElasticPool {
    ...
    function getBalanceBp()	external view returns(uint256);
    function totalSupply() external view returns(uint256);
}
...
contract ReservePool is Ownable, ReentrancyGuard {
	...
	// Interface of VPStogage
	address public vpStorage;
	// Interface of ElasticPool
	IElasticPool public elasticPool;
	...
	****************************************/
	...
	/** 
	* @dev Sets ElasticPool contract interface, can be called only once
	* @param _elasticPool - ElasticPool contract address
	*/
	function setElasticPool (
		address _elasticPool
	)
		external
		onlyOwner
	{
		require (address(elasticPool) == address(0), "ElasticPool address is already set");
		elasticPool = IElasticPool(_elasticPool);
	}
	...
	****************************************/
		...
		nonReentrant
	{
		...
		inToken.burnFrom(msg.sender, _amount);
	...
	{
		require(
			elasticPool.paused() && elasticPool.getBalanceBp() == 0 && elasticPool.totalSupply() == 0, 
			"emergency conditions not meet"
		);
		uint256 _balance = outToken.balanceOf(address(this));
		require(_balance != 0, "zero out token balance");
		outToken.safeApprove(vpStorage, _balance);
		IVPStorage(vpStorage).receiveOutToken(_balance);
	}
