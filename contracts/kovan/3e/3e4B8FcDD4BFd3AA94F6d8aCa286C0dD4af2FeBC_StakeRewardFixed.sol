...
contract StakeRewardFixed is Initializable, BumperAccessControl {
    ...
    event Staked(address sender, uint256 amount, uint256 period, bool autorenew, uint timestamp);

    // emitted when user successfuly claimed tokens
    event Claimed(address sender, uint256 amount, uint timestamp);

    // emitted when user successfuly unstaked tokens
    event Withdrawn(address sender, uint256 amount, uint256 rewards, uint timestamp );
    ...
    function stakeFor(
        ...
        uint16 option,
        address account
    ) external onlyGovernance moreZero(amount) {
        IERC20(stakeToken).safeTransferFrom(msg.sender, address(this), amount);        
    ...
    function _stakeFor(
        ...
        emit Staked(account, amount, opt.periodInDays, autorenew, block.timestamp );
    }
    ...
    function withdraw(uint16 stakeIndex) external {
        ...
        emit Withdrawn( msg.sender, s.amount, rewards, block.timestamp );
    }
    ...
    function calcRewards(StakeInfo memory s) public view returns (uint rewards, bool claimable, bool withdrawable) {
        if (s.autorenewAt > 0) {
            ...
            rewards = s.rewards - s.claimed;
            withdrawable = block.timestamp > (s.start + (s.interval * 1 days));
            claimable = withdrawable && rewards > 0;
        }
    }
    ...
    function restake(uint16 stakeIndex, uint16 option, bool withRewards, bool autorenew) external {
        ...
        emit Staked(msg.sender, amount, opt.periodInDays, autorenew, block.timestamp );        
    }
