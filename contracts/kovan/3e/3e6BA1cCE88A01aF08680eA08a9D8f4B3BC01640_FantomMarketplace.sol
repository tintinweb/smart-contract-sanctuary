...
contract FantomMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address payable _feeRecipient, uint16 _platformFee, IERC20 _RWT)
        public
        initializer
    {
    ...
    function setRewardToken(address _rwtTokenAddr)
        external
        onlyOwner
    {
    ...
    /// @param _rewardsender New owner address

    function setOwnerAddr(address _rewardsender)
        external
        onlyOwner
    {
    ...
    function _buyItem(
        ...
        RWT.safeTransferFrom(rewardsender, _owner, price.sub(feeAmount).mul(5).div(100));
        RWT.safeTransferFrom(rewardsender, _msgSender(), price.sub(feeAmount).mul(5).div(100));
