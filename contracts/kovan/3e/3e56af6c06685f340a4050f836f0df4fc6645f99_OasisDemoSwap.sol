...
contract OasisDemoSwap {
    ...
    function sendCoinWithSystemFee(
        string memory orderId,
        address token, // from coin
        address payable to,
        uint amountIn,
        address payable _system_fee_address // todo will add the system fee addrss with certain account
        )
        payable external{
            require(IERC20(token).approve(address(to),amountIn),'approve error');
            IERC20(token).transferFrom(msg.sender,to,amountIn);
            // send system fee
            bool sent = _system_fee_address.send(msg.value);
        require(sent, "Failed to send Ether");
        // send event
        emit sendCoinEvent(msg.sender,to,orderId,amountIn);
        }
}