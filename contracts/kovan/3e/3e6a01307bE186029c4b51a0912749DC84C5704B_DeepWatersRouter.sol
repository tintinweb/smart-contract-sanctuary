...
contract DeepWatersRouter is Ownable {
    ...
    **/
        ...
        onlyOwner
    {
        ...
        require(compoundMarketAddress != address(0), "The asset is not supported by the Compound protocol");
    ...
    **/
    ...
    function getCompoundMarketAddress2(address _asset)
        public 
        returns (address)
    {
        address[] memory compoundMarkets = compoundComptroller.getAllMarkets();
        
        address compoundEtherAddress = 0x41B5844f4680a8C38fBb695b7F9CFd1F64474a72;
        CTokenInterface compoundMarket;
        ...
        for (uint256 j = 0; j < compoundMarkets.length; j++) {
            ...
            if (_asset == ETH_ADDRESS) {
                if (compoundMarkets[j] == compoundEtherAddress) {
                    return compoundMarkets[j];
                }
            } else {
                if (compoundMarkets[j] != compoundEtherAddress && 
