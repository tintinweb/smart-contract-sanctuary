...
contract CrowdPresale {
        ...
        notYetClaimedOrRefunded
    {
        ...
        if (balance > 0) {
            presaleCreatorAddress.transfer(balance);
        }
    }
    ...
    function collectFundsRaised() external onlyRichFactory {
        ...
        if (address(this).balance > 0) {
            richFactoryAddress.transfer(address(this).balance);
        }
    }

    function burnUnsoldTokens() external onlyPresaleCreatorOrRichFactory {
    ...
    function setCakeLiquidityAdded( bool _liquidityAdded ) external onlyRichFactory {
        require(!cakeLiquidityAdded, "Liquidity has already been added");
        cakeLiquidityAdded = _liquidityAdded;    
    }
