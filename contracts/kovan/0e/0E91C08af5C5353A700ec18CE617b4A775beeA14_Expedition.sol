...
contract Expedition is Ownable {
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            return totalEligible.mul(accTokensPerShare).div(1e12).sub(totalRewardDebt);
            }
        }
    }
            return 0;
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            uint256[] storage rewardDebtHolders;
            ...
            for (uint256 index = 0; index < user.stakeInfo.length; index++) {
                ...
                if ((currentTimeStamp.sub((user.stakeInfo[index].timeStamp))) >= pool.lockPeriod ){
                    totalEligible = totalEligible.add(user.stakeInfo[index].amount);
                    if ( user.stakeInfo[index].rewardDebt > 0 ){
                        totalRewardDebt = totalRewardDebt.add(user.stakeInfo[index].rewardDebt);
                        rewardDebtHolders.push(user.stakeInfo[index].rewardDebt);
                    }
                }
            }
            ...
            uint256 pending = totalEligible.mul(pool.accTokensPerShare).div(1e12).sub(totalRewardDebt);
            
            if (pending > 0) {

                safeStakingTokensTransfer(msg.sender, pending);

                for (uint256 index = 0; index < rewardDebtHolders.length; index++) {
                    // calcualte reward debt for the stake enteries where reward is already claimed
                    user.stakeInfo[rewardDebtHolders[index]].rewardDebt = user.stakeInfo[rewardDebtHolders[index]].amount.mul(pool.accTokensPerShare).div(1e12);
                }
            }
        }
        }
    ...
    function getStakeInfo(uint256 index) public view returns (uint256, uint256){
        UserInfo storage user = userInfo[msg.sender];
        return (user.stakeInfo[index].amount, user.stakeInfo[index].timeStamp);
    }
            ...
            break;
             ...
             if ( user.stakeInfo[index].amount ==0 ){
                ...
                if ((currentTimeStamp.sub((user.stakeInfo[index].timeStamp))) < pool.lockPeriod ){
                    ...
                    uint256 rewardPerDebtShare  = (user.stakeInfo[index].rewardDebt).mul(10**12).div(user.stakeInfo[index].rewardDebt).div(10**12);
                    ...
                    uint256 rewardPenalty = currentAmountReward.mul(pool.penaltyPercentage).div(10000);
                    ...
                    user.stakeInfo[index].rewardDebt = (user.stakeInfo[index].rewardDebt).add( deductionAmount.mul(pool.accTokensPerShare).div(1e12));

                }else{
                rewardForTransfers = rewardForTransfers.add(deductionAmount.mul(pool.accTokensPerShare).div(1e12).sub(user.stakeInfo[index].rewardDebt));
                }
        }
