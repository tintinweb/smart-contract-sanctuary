        ...
        if (winners.length > 0) {
            ...
            for (uint256 i = winners.length - 1; i > 0; i--) {
                if (winners[i] != address(0x0)) {
                    ...
                    delete winners[i];
                    IERC20(IBettingPool(owner).getTokenPool()).safeTransfer(winner, reward);
                }
            }
            if (winners[0] != address(0x0)) {
                address winner = winners[0];
                delete winners[0];
                IERC20(IBettingPool(owner).getTokenPool()).safeTransfer(winner, getTotalToken());
            }
        }
...
contract CommunityBettingFactory {
