...
contract Pool {
    ...
    constructor(address _stakeToken, address _rewardToken, uint256 _start, uint256 _end, uint256 _totalReward) public {
        admin = msg.sender;
    ...
    function getReward(address account) public{
        uint256 reward = earn(account);
        if(reward > 0){
            _weights[account] = 0;
            _userTimeRecord[account] = block.timestamp;
            TransferHelper.safeTransfer(rewardToken,msg.sender,reward);
            emit GetRewarded(account,reward);
        }
    }
}