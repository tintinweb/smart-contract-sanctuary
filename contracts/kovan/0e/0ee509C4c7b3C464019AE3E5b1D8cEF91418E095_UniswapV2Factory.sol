...
contract UniswapV2Factory is IUniswapV2Factory {
    ...
    function setTopCoin(address _coinAddress, bool _status) external {
        require(msg.sender == feeToSetter, 'UniswapV2: FORBIDDEN');
        topCoins[_coinAddress] = _status;
    }

    function setTopCoinFee(uint _totalFee, uint _alpha, uint _beta) external {
        require(msg.sender == feeToSetter, 'UniswapV2: FORBIDDEN');
    ...
    function setRegularCoinFee(uint _totalFee, uint _alpha, uint _beta) external {
        require(msg.sender == feeToSetter, 'UniswapV2: FORBIDDEN');
        require(_alpha < _beta, 'UniswapV2: IMPROPER FRACTION');
    ...
    function updatePairFee(address token0, address token1, uint _totalFee, uint _alpha, uint _beta) external {
        require(msg.sender == feeToSetter, 'UniswapV2: FORBIDDEN');
        require(_alpha < _beta, 'UniswapV2: IMPROPER FRACTION');
