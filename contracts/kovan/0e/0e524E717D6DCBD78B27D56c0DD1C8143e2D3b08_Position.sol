...
contract Position is Initializable, ERC721EnumerableUpgradeable, IPosition {
  ...
  mapping(uint256 => Types.PositionDetails) public _positions;
  ...
  function getPositionRepartition(uint256 tokenId)
    external
    view
    override
    returns (uint256 bondsQuantity, uint256 normalizedDepositedAmount)
  {
  ...
  function deposit(
    ...
    (uint256 adjustedBalance, uint256 bondsIssuanceIndex) = pools.deposit(
      amount,
      rate,
      borrower,
      underlyingToken,
    ...
    emit Deposit(to, tokenId, amount, rate, borrower, address(pools));
  }
  ...
  function updateRate(uint256 tokenId, uint256 newRate) external override {
    ...
    (uint256 newAmount, uint256 newBondsIssuanceIndex) = pools.updateRate(
      _positions[tokenId].adjustedBalance,
      _positions[tokenId].borrower,
      oldRate,
      newRate,
    ...
    emit UpdateRate(_msgSender(), tokenId, newAmount, newRate, _positions[tokenId].borrower);
  }
  ...
  function withdraw(uint256 tokenId) external override {
    ...
    IBorrowerPools.WithdrawParams memory params = IBorrowerPools.WithdrawParams({
      adjustedAmount: _positions[tokenId].adjustedBalance,
      rate: _positions[tokenId].rate,
      borrower: _positions[tokenId].borrower,
      owner: _msgSender(),
      bondsIssuanceIndex: _positions[tokenId].bondsIssuanceIndex
    });
    ...
    (
      ...
      uint256 normalizedWithdrawnDeposit
    ) = pools.withdraw(params);
...
interface IPosition {
   ...
   * @param borrowerPools address of BorrowerPools contract
   **/
  event Deposit(
   ...
   * @param borrower Target borrower
   **/
  event UpdateRate(address indexed lender, uint256 tokenId, uint256 amount, uint256 rate, bytes32 borrower);
  ...
  /**
   * @notice Determines the repartition of an amount between bonds and deposited amount of the position
  ...
  /**
   * @notice Creates a new position and deposits tokens into the borrower's pool
  ...
  /**
   * @notice Updates the rate of an existing position in the relevant pool
