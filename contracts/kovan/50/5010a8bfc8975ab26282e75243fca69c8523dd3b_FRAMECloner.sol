...
interface FRAMEDeployer {
    function initialize(address _Owner, uint max_, uint price) external;
}
...
contract FRAMECloner {
    ...
    function _clone(uint max_, uint price) external {
        address identicalChild = clone(implementation);
        allClones[msg.sender].push(identicalChild);
        FRAMEDeployer(identicalChild).initialize(msg.sender, max_, price);
