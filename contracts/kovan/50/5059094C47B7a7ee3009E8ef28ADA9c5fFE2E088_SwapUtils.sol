...
library SwapUtils {
    ...
    function addLiquidity(
        ...
        // Dopple Fee
        uint256 mintFee = self.mintFee.mul(toMint).div(FEE_DENOMINATOR);

        self.dopple.safeTransferFrom(msg.sender, self.reserver, mintFee);
    ...
    function removeLiquidity(
        ...
        uint256 burnFee = self.burnFee.mul(amount).div(FEE_DENOMINATOR);

        self.dopple.safeTransferFrom(msg.sender, self.reserver, burnFee);
     ...
     * @param newMintFee new mint in dopple
     */
    function setMintFee(Swap storage self, uint256 newMintFee) external {
        self.mintFee = newMintFee;

        emit NewMintFee(newMintFee);
    }

    /**
     * @notice Sets the mint fee
     * @param self Swap struct to update
     * @param newBurnFee new mint in dopple
     */
    function setBurnFee(Swap storage self, uint256 newBurnFee) external {
        self.burnFee = newBurnFee;

        emit NewBurnFee(newBurnFee);
    }

    /**
     * @notice Sets the mint fee
     * @param self Swap struct to update
     * @param newReserver reserve address
     */
    function setReserver(Swap storage self, address newReserver) external {
        self.reserver = newReserver;

        emit NewReserver(newReserver);
    }
