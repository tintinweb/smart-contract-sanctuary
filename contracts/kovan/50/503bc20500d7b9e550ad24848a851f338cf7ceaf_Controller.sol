...
contract SVaultNetValue is SVaultNetValueStorage {
   ...
   //@notice  The number of seconds in a year
    ...
    function getNetValuesInView() view public returns(NetValue[] memory netValues,NetValue[] memory netValuesNew){
        ...
        if(allTokensInUSD < totalAllocatedProfitInUSD) return (netValues,netValues);
    ...
    function getNetValuesInAction() internal returns(NetValue[] memory netValues, uint256 newCommission){
        ...
        (PoolInfo[] memory poolInfos, uint256 totalWeight, uint256 totalProfitWeight, uint256 totalAmountInUSD, uint256 totalAllocatedProfitInUSD) = getPoolInfos(flexiblePools, tokenPrices);
        uint256 totalProfitAmountInUSD = 0;
        allTokensInUSD = allTokensInUSD.sub(totalAllocatedProfitInUSD, "Insufficient funds");
        if(allTokensInUSD < totalAmountInUSD){
            totalAmountInUSD = allTokensInUSD;
        }else{
            totalProfitAmountInUSD = allTokensInUSD.sub(totalAmountInUSD);
   ...
   function getProfit()view public returns(uint256 totalProfitAmountInUSD,uint256 blockHeight){
        ...
        if(allTokensInUSD < totalFixedPoolUSD) return (0,block.number);
        allTokensInUSD = allTokensInUSD.sub(totalFixedPoolUSD);
        if(allTokensInUSD > commissionAmountInPools) allTokensInUSD = allTokensInUSD.sub(commissionAmountInPools);
        else allTokensInUSD = 0;
        (PoolInfo[] memory poolInfos, uint256 totalWeight, uint256 totalProfitWeight, uint256 totalAmountInUSD, uint256 totalAllocatedProfitInUSD) = getPoolInfos(flexiblePools, tokenPrices);
        ...
        return (totalProfitAmountInUSD,block.number);
    }
    ...
    function getAvailableTakeCommision() public view returns(uint256,uint256){
         (uint256 totalProfit,)= getProfit();  
         uint256 excessLimit = totalProfit.mul(excessLimitInRatio).div(1e6);
         uint256 availableProfit = commissionAmountInPools > excessLimit ? commissionAmountInPools.sub(excessLimit) : 0;
         return (availableProfit,block.number);
    }
...
contract Controller is ControllerStorage {
    ...
    function setStrategies(PoolStrategy[] memory poolStrategies) external onlyAdmin{
        address[] memory _pools = new address[](poolStrategies.length);
        address[] memory _tokens = new address[](poolStrategies.length);
        address[] memory _strategies = new address[](poolStrategies.length);
        TokenArray[] memory _tokenArrays = new TokenArray[](poolStrategies.length);
        bool has;
        uint256 poolIndex;
        uint256 strategyIndex;
        for(uint256 i = 0;i < poolStrategies.length; i++){
            require(
                !approvedStrategies[poolStrategies[i].pool][poolStrategies[i].strategy],
                "Strategy was already set"
            );
            (has,poolIndex) = getOrAdd(_pools,poolStrategies[i].pool);
            if(!has){
                address token = IFundPool(poolStrategies[i].pool).token();
                require(acceptedPools[token][poolStrategies[i].pool], "Invalid pool");
                _tokens[poolIndex] = token;
            }
            (has,strategyIndex) = getOrAdd(_strategies,poolStrategies[i].strategy);
            if(!has){
                require(acceptedStrategies[poolStrategies[i].strategy], "Strategy doesn't exist");
                _tokenArrays[strategyIndex].tokens = IStrategy(poolStrategies[i].strategy).getTokens();
            }
            require(
                hasItem(_tokenArrays[strategyIndex].tokens, _tokens[poolIndex]),
                "Invalid strategy"
            );
            approvedStrategies[poolStrategies[i].pool][poolStrategies[i].strategy] = true;
            emit StrategyApproved(poolStrategies[i].pool, poolStrategies[i].strategy);
        }
    }
    ...
    function withdrawCommssionProfit(address[] memory withdrawTokens, uint256[] memory withdrawAmounts,address fundManager) public {
        ...
        TokenAmountArray[] memory tokenAmountArrays = new TokenAmountArray[](strategies.length);
        uint256 count = 0;
        for(uint256 i = 0; i < strategies.length; i++) {
            ...
            require(amount == withdrawAmounts[i],"insufficient tokens in strategies");
            count = count + withdrawBackTokens.length;
            tokenAmountArrays[i].tokens = withdrawBackTokens;
            tokenAmountArrays[i].amounts = withdrawBackAmounts;    
        }
        TokenAmount[] memory tokenAmounts = new TokenAmount[](count);
        for(uint256 i = 0; i < tokenAmountArrays.length; i++) {
            AddTokenAmount(tokenAmounts,tokenAmountArrays[i].tokens,tokenAmountArrays[i].amounts);  
        }

        for(uint256 j = 0; j < tokenAmounts.length; j++){
            if(tokenAmounts[j].amount > 0){
                IERC20(tokenAmounts[j].token).safeTransfer(fundManager, tokenAmounts[j].amount);
            }    
        }    
    ...
    function getOrAdd(address[] memory _array, address _item) internal view returns(bool, uint256){
        for(uint256 i = 0; i < _array.length; i++){
