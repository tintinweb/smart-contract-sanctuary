...
contract CompoundLens {
    ...
    function cTokenMetadata(CToken cToken) public returns (CTokenMetadata memory) {
        ...
        (bool isListed, uint256 collateralFactorMantissa, , ComptrollerV2Storage.Version version) = comptroller.markets(address(cToken));
        address underlyingAssetAddress;
        uint256 underlyingDecimals;
        uint256 collateralCap;
        uint256 totalCollateralTokens;
    ...
    function cTokenBalances(CToken cToken, address payable account) public returns (CTokenBalances memory) {
        ...
        bool collateralEnabled = Comptroller(comptroller).checkMembership(account, address(cToken));
        uint256 tokenBalance;
        uint256 tokenAllowance;
        uint256 collateralBalance;
    ...
    function getCompBalanceMetadataExt(EIP20Interface comp, Comptroller comptroller, address account) external returns (CompBalanceMetadataExt memory) {
        uint256 balance = comp.balanceOf(account);
        comptroller.claimCompAll(account);
    ...
    function getClaimableSushiRewards(CSLPInterface[] calldata cTokens, address sushi, address account) external returns (uint256[] memory) {
        uint256 cTokenCount = cTokens.length;
