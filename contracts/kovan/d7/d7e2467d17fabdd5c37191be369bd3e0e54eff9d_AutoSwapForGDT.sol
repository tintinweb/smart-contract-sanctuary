...
contract AutoSwapForGDT {
  ...
  function setAddressDAI(address _DAI) public returns(bool) {
      DAI = _DAI;
      return true;
  }
  
  function convertDAItoGDT(uint amountIn) public payable {
    uint[] memory _GDTAmount;
    _GDTAmount = uniswapRouter.getAmountsOut(amountIn, getPathForDAItoGDT());
    uint preGDTAmount = _GDTAmount[1];
    uint GDTAmount100 = preGDTAmount / 100;
    uint GDTAmount = GDTAmount100 * 88;

    uint deadline = block.timestamp; 
    uniswapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(amountIn, GDTAmount, getPathForDAItoGDT(), holderGDT, deadline);
  ...
  function getPathForDAItoGDT() public view returns (address[] memory) {
