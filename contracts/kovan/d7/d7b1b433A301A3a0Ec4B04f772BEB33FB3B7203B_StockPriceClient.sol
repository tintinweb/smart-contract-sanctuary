...
contract StockPriceClient is ChainlinkClient {
    using Chainlink for Chainlink.Request;
    uint256 public oraclePayment;
    bytes32 public jobId;
    address public oracle;
    mapping(string => int256) public latestAnswers;
    mapping (bytes32 => string) public pendingRequests;

    constructor(uint256 _oraclePayment, address _oracle, bytes32 _jobId) {
    ...
    function requestData(string memory _symbol) public {
        Chainlink.Request memory req = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);
        req.add("symbol", _symbol);
        bytes32 r_id = sendChainlinkRequestTo(oracle, req, oraclePayment);
        pendingRequests[r_id] = _symbol;
    }

    function fulfill(bytes32 _requestId, int256 _data) public recordChainlinkFulfillment(_requestId) {
        string memory _symbol = pendingRequests[_requestId];
        if (bytes(_symbol).length != 0) {
            latestAnswers[_symbol] = _data;
        }
    }
}
