    ...
    IIntegrationMap
{
    ...
    struct Integration {
        bool added;
        string name;
        mapping(address => uint256) weightsByTokenAddress;
    }
    ...
    function initialize(address[] memory controllers_, address moduleMap_, address wethTokenAddress_, address biosTokenAddress_) public initializer {
        __Controlled_init(controllers_, moduleMap_);
        wethTokenAddress = wethTokenAddress_;
        biosTokenAddress = biosTokenAddress_;

        _addToken(wethTokenAddress_, true, true, 1000, 50000, new uint256[](0));
        _addToken(biosTokenAddress_, true, true, 1000, 0, new uint256[](0));
    } 
    ...
    function addIntegration(
        ...
        require(weightsByTokenId.length == tokenAddresses.length, "IntegrationMap::addIntegration: Weights array incorrect length");
        ...
        for(uint256 tokenId; tokenId < tokenAddresses.length; tokenId++) {
            integrations[contractAddress].weightsByTokenAddress[tokenAddresses[tokenId]] = weightsByTokenId[tokenId];
        }
    ...
    function _addToken(
        ...
        require(!tokens[tokenAddress].added, 
            "IntegrationMap::addToken: Token already added");
        require(reserveRatioNumerator <= RESERVE_RATIO_DENOMINATOR, 
            "IntegrationMap::addToken: reserveRatioNumerator must be less than or equal to reserve ratio denominator");
        require(weightsByIntegrationId.length == integrationAddresses.length, 
            "IntegrationMap::addToken: Weights array incorrect length");

        for(uint256 integrationId; integrationId < integrationAddresses.length; integrationId++) {
            integrations[integrationAddresses[integrationId]].weightsByTokenAddress[tokenAddress] = weightsByIntegrationId[integrationId];
        }
    ...
    function enableTokenDeposits(address tokenAddress) external override onlyController {
        require(tokens[tokenAddress].added, 
            "IntegrationMap::enableTokenDeposits: Token does not exist");
        require(!tokens[tokenAddress].acceptingDeposits, 
    ...
    /// @param tokenAddress The address of the token ERC20 contract
    function disableTokenDeposits(address tokenAddress) external override onlyController {
        require(tokens[tokenAddress].added, 
    ...
    /// @param tokenAddress The address of the token ERC20 contract
    function enableTokenWithdrawals(address tokenAddress) external override onlyController {
        require(tokens[tokenAddress].added, 
            "IntegrationMap::enableTokenWithdrawals: Token does not exist");
        require(!tokens[tokenAddress].acceptingWithdrawals, 
    ...
    /// @param tokenAddress The address of the token ERC20 contract
    function disableTokenWithdrawals(address tokenAddress) external override onlyController {
        require(tokens[tokenAddress].added, 
    ...
    function updateTokenRewardWeight(address tokenAddress, uint256 rewardWeight) external override onlyController {
        require(tokens[tokenAddress].added, 
            "IntegrationMap::updateTokenRewardWeight: Token does not exist");
        require(tokens[tokenAddress].biosRewardWeight != rewardWeight, 
    ...
    function updateTokenIntegrationWeight(address integrationAddress, address tokenAddress, uint256 updatedWeight) external override onlyController { 
        require(integrations[integrationAddress].added, 
            "IntegrationMap::updateTokenIntegrationWeight: Integration does not exist");
        require(tokens[tokenAddress].added, 
            "IntegrationMap::updateTokenIntegrationWeight: Token does not exist");
        require(integrations[integrationAddress].weightsByTokenAddress[tokenAddress] != updatedWeight, 
            "IntegrationMap::updateTokenIntegrationWeight: Updated weight must not equal current weight");

        integrations[integrationAddress].weightsByTokenAddress[tokenAddress] = updatedWeight;
    }

    /// @param tokenAddress the address of the token ERC20 contract
    /// @param reserveRatioNumerator Number that gets divided by reserve ratio denominator to get reserve ratio
    function updateTokenReserveRatioNumerator(address tokenAddress, uint256 reserveRatioNumerator) external override onlyController {
        require(tokens[tokenAddress].added, 
    ...
    function getIntegrationAddress(uint256 integrationId) external view override returns (address) {
        require(integrationId < integrationAddresses.length, 
    ...
    function getTokenIntegrationWeight(address integrationAddress, address tokenAddress) external view override returns (uint256) {
        require(integrations[integrationAddress].added, "IntegrationMap::getTokenIntegrationWeight: Integration does not exist");
        require(tokens[tokenAddress].added, "IntegrationMap::getTokenIntegrationWeight: Token does not exist");
        return integrations[integrationAddress].weightsByTokenAddress[tokenAddress];
    }

    /// @param tokenAddress The address of the token ERC20 contract
    /// @return tokenWeightSum The sum of the specified token weights
    function getTokenIntegrationWeightSum(address tokenAddress) external view override returns (uint256 tokenWeightSum) {
        require(tokens[tokenAddress].added, "IntegrationMap::getTokenIntegrationWeightSum: Token does not exist");
        for(uint256 integrationId; integrationId < integrationAddresses.length; integrationId++) {
            tokenWeightSum += integrations[integrationAddresses[integrationId]].weightsByTokenAddress[tokenAddress];
        }
    }
