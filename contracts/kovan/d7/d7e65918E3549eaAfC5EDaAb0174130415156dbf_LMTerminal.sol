...
contract LMTerminal is Initializable {
    ...
    IxAssetCLR.UniswapContracts public uniContracts; // Uniswap V3 Contracts
     ...
     * @param proxyAdmin address of the admin of the clr pool and staked clr token proxy
     */
    function deployIncentivizedPool(
        ...
        // Deploy CLR
        IxAssetCLR clrPool = IxAssetCLR(clrDeployer.deployCLRPool(proxyAdmin));
        ...
        stakedToken.initialize("StakedCLRToken", symbol, address(clrPool));

        // Initialize CLR
        if (pool.token0 > pool.token1) {
            address tmp = pool.token0;
        ...
        IxAssetCLR.StakingDetails memory stakingParams = IxAssetCLR
            .StakingDetails({
        ...
        IERC20(pool.token0).approve(address(clrPool), type(uint256).max);
        IERC20(pool.token1).approve(address(clrPool), type(uint256).max);
        ...
        clrPool.mintInitial(pool.amount0, pool.amount1);

        // Setup duration of rewards
        clrPool.setRewardsDuration(rewardsProgram.duration);

        // Setup vesting period if rewards are escrowed
        if (rewardsAreEscrowed) {
            for (uint256 i = 0; i < rewardsProgram.rewardTokens.length; ++i) {
                address rewardToken = rewardsProgram.rewardTokens[i];
                rewardEscrow.setRewardsTokenVestingPeriod(
                    rewardToken,
    ...
    function initiateRewardsProgram(
        ...
        if (clrPool.rewardsAreEscrowed()) {
            ...
            for (uint256 i = 0; i < rewardTokens.length; ++i) {
                rewardEscrow.addRewardsToken(rewardTokens[i]);
            }
        }
    ...
    function _initiateRewardsProgram(
        ...
        for (uint256 i = 0; i < rewardTokens.length; ++i) {
            ...
            uint256 rewardAmount = totalRewardAmounts[i].sub(rewardAmountFee);
            ...
            rewardFeesTotal[rewardToken] += rewardAmountFee;
            ...
            clrPool.initializeReward(rewardAmount, i);
        }
     ...
     * @notice Address calling this function needs to approve t0 and t1 to Terminal
     */
    ...
    function withdrawFees() public onlyRevenueController {
        ...
        for (uint256 i = 0; i < deployedCLRPools.length; ++i) {
            address rewardToken = deployedCLRPools[i].rewardToken();
...
interface IRewardEscrow {
  ...
  function appendVestingEntry ( address token, address account, uint256 quantity ) external;
  function balanceOf ( address token, address account ) external view returns ( uint256 );
  function getNextVestingIndex ( address token, address account ) external view returns ( uint256 );
  function getNextVestingQuantity ( address token, address account ) external view returns ( uint256 );
  function getNextVestingTime ( address token, address account ) external view returns ( uint256 );
  function getVestingQuantity ( address token, address account, uint256 index ) external view returns ( uint256 );
  function getVestingTime ( address token, address account, uint256 index ) external view returns ( uint256 );
  function isRewardContract ( address ) external view returns ( bool );
  function numVestingEntries ( address token, address account ) external view returns ( uint256 );
  function owner (  ) external view returns ( address );
  ...
  function setRewardsTokenVestingPeriod ( address rewardToken, uint256 vestingPeriod ) external;
  ...
  function vestingSchedules ( address, address, uint256, uint256 ) external view returns ( uint256 );
}
...
interface IStakingRewards {
    ...
    function initializeReward(uint256 rewardAmount, uint256 tokenIndex)
        external;
}
