...
contract CrypTOW {
    ...
    function setRoundDuringTime(uint256 hour) public onlyOwner {
        require(hour > 0,  "Hour is wrong value");
        roundDuringTime = hour * 60 * 60;
    }
    ...
    function getFeePercent() public view returns (uint256) {
        ...
        uint256 percent = (currentTime - roundStartTime) * maxFee / roundDuringTime;
        if (percent > maxFee)
            percent = maxFee;
        return percent;
    }
    ...
    function getRoundDatas() public view returns (uint256[] memory datas) {
        ...
        for (uint256 i = currentRound + 1; i <= currentRound + 100; i++) {
            datas[i - currentRound + 1] = roundDatas[i];
        }
        return datas;
    }
