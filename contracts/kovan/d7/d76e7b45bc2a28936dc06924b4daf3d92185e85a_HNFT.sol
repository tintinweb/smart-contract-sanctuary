...
contract HNFT is ERC721Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    using CountersUpgradeable for CountersUpgradeable.Counter;
    CountersUpgradeable.Counter private _tokenIds;
    uint256 public constant totalSupply = 682;
    ...
    function initialize() public initializer {
        ...
        ERC721Upgradeable.__ERC721_init("HNFT", "HNFT");
        setBaseURI("ipfs://QmaqVgkZT5sBYgFeRYAXdnoX6Qnkc7jdBKXPi97HYobfmi/");
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        _mint(msg.sender, newItemId);
        circulatingSupply =++circulatingSupply;
        TokenMeta memory meta = TokenMeta(newItemId,100000000000000000, "none", "none", true);
    ...
    function setTokenMeta(uint256 _tokenId, string memory _tokenURI, 
        string memory _name, 
        uint256 _price, 
        bool _sale) external onlyOwner {
        require(_exists(_tokenId));
        _tokenMeta[_tokenId].uri = _tokenURI;
        _tokenMeta[_tokenId].name = _name;
        _tokenMeta[_tokenId].sale = _sale;
        _tokenMeta[_tokenId].price = _price;
    ...
    /**
     * @dev sale status _startSale
     * @param _startSale bool isSaleActive
     */
    ...
    {
          require(
            isSaleActive && circulatingSupply <= totalSupply - 10,
            "Sale Closed"
        );
