...
contract RewardPool is LPTokenWrapper, IRewardDistributionRecipient {
    ...
    function setEmiPriceData(address _emiPrice, address[3] memory _stables) public onlyOwner {
    ...
    function stake(uint256 amount) public updateReward(msg.sender) {
        ...
        require(block.timestamp <= periodFinish, "Cannot stake yet");
        super.stake(amount);
    ...
    function getStakedValuesinUSD(uint8 marketID, uint8 id) public view returns (uint256 returnPrice) {
        address[] memory tokens = new address[](1);
        tokens[0] = address(stakeToken);
        returnPrice = IEmiPrice2(emiPrice).getCoinPrices(tokens, stables, marketID)[id];
    }

    function getStakedValueinUSD() public view returns (uint256 returnPrices) {
        address[] memory tokens = new address[](1);
        //uint256[3] memory resPrices;
        //tokens[]
        tokens[0] = address(stakeToken);
        //for (uint8 marketID = 0; marketID < 3; marketID++) {
            /* resPrices[marketID] */ returnPrices = IEmiPrice2(emiPrice).getCoinPrices(tokens, stables, 0)[0];
            /* for (uint8 stableID = 0; stableID < stableID; stableID++) {
                if (resPrices[stableID] != 0) {
                    return resPrices[stableID];
                }
            } */
        //}
    }

    /* function getStakedValueinUSD() public view returns (uint256 returnPrice) {
        address[] memory tokens = new address[](1);
        //uint256[3] memory resPrices;
        //tokens[]
        tokens[0] = address(stakeToken);
        //for (uint8 marketID = 0; marketID < 3; marketID++) {
            resPrices[marketID] returnPrices[marketID] = IEmiPrice2(emiPrice).getCoinPrices(tokens, stables, marketID)[marketID];
            for (uint8 stableID = 0; stableID < stableID; stableID++) {
                if (resPrices[stableID] != 0) {
                    return resPrices[stableID];
                }
