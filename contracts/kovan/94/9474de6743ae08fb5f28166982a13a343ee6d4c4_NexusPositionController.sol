...
contract NexusPositionController is AbstractController, ERC20Upgradeable {
    ...
    function withdrawRewards() external override {
        ...
        //apply fee
        _nxmToken.transfer(address(index), _fee);
        _rewards = _rewards.sub(_fee);

        //wrap NXM into wNXM
        _wrapWNXM(_rewards);
        require(_wNXMToken.balanceOf(address(this)) == _rewards, "NexusPositionController: NPC7");

        //swap wXNM into base
        uint256 _baseAmount = _sellWNXM(_rewards);
        require(_baseAmount != 0, "NexusPositionController: NPC7");
    ...
    function _buyWNXM(uint256 _amount) internal returns (uint256) {
        ...
        //FIXME For ERC20
        //_checkApprovals(base, priceFeed.getUniswapRouter(), _amount);
        //For USDT
        _setOrIncreaseTetherAllowance(priceFeed.getUniswapRouter(), _amount);
        return priceFeed.buyWNXMforUSDT(_amount);
    }

    function _sellWNXM(uint256 _amount) internal returns (uint256) {
        IPriceFeed priceFeed = IPriceFeed(index.getPriceFeed());
        _checkApprovals(_wNXMToken, priceFeed.getUniswapRouter(), _amount);
        return priceFeed.sellWNXMforUSDT(_amount);
    }
    ...
    function _wrapWNXM(uint256 _amount) internal {
...
interface IPriceFeed {
    ...
    function sellWNXMforUSDT(uint256 wnxmAmount) external returns (uint256);
