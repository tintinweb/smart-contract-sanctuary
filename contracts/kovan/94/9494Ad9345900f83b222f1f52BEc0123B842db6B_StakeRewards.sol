...
contract StakeRewards is PausableUpgradeable, BumperAccessControl {
    ...
    event Staked(address sender, uint256 amount, uint256 period, bool autorenew, uint timestamp, uint stakeIndex);

    // emitted when user successfuly claimed tokens
    event Claimed(address sender, uint256 amount, uint timestamp, uint stakeIndex);

    // emitted when user successfuly unstaked tokens
    event Withdrawn(address sender, uint256 amount, uint256 rewards, uint timestamp, uint stakeIndex );

    // emitted when user successfuly requested withdraw
    event WithdrawRequested(address indexed sender, uint256 amount, uint256 timestamp, uint stakeIndex );
    ...
    function _stakeFor(
        ...
        StakeInfo memory newStake = StakeInfo(
            ...
            option,
            autorenew,
            autorenew ? 0 : uint64(block.timestamp + 1 days * opt.periodInDays),
            0,
            0
        );
        ...
        emit Staked(account, amount, opt.periodInDays, autorenew, block.timestamp, usersStake[account].length );
    }
    ...
    function switchAutorenew(uint16 stakeIndex) external whenNotPaused {
        ...
        if (s.autorenew) {
            uint64 optionPeriod = stakeOptions[s.option].periodInDays;       
            uint64 periods = uint64(block.timestamp - s.start) / (1 days * optionPeriod) + 1;

            s.end = s.start + periods * 1 days * optionPeriod;
            s.autorenew = false;     
        }
        else {
    ...
    function claimRewards(uint16 stakeIndex) external whenNotPaused {
        ...
        (uint rewards, bool claimable, , ) = calcRewards(s, stakeOptions[s.option]);
    ...
    function calcRewardsByIndex(uint16 stakeIndex) public view returns (uint rewards, bool claimable, bool withdrawable, uint endOfLastPeriod) {
        ...
        opt.index = calculateCumulativeIndex(opt.index, opt.emission, opt.lastTimestamp, uint64(block.timestamp), opt.total );            
    ...
    function restake(uint16 stakeIndex, uint16 option, bool withRewards, bool autorenew) external whenNotPaused  
    {
        ...
        emit Staked(msg.sender, amount, opt.periodInDays, autorenew, block.timestamp, stakeIndex );        
    }
    ...
    function _updateEmissions() internal {
        ...
        for (uint16 i = 0; i < stakeOptions.length; i++) {
            ...
            if (weithedAmountSum > 0) {
                option.emission = uint64(totalEmissionPerSecond  * (option.total * option.multiplier) / weithedAmountSum);
            }
            else 
                option.emission = 0;
        }
    ...
    function setTotalEmissionPerSecond(uint64 emission) external onlyGovernance {
        _updateIndexes();
