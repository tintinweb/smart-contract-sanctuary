...
abstract contract CoreUtility {
    ...
    uint256 internal constant SETTLEMENT_TIME = 10 minutes;
    ...
    function _endOfWeek(uint256 timestamp) internal pure returns (uint256) {
        return
            ((timestamp.add(1 days) - (SETTLEMENT_TIME - 22 hours)) / 1 days) *
            1 days +
            SETTLEMENT_TIME -
            22 hours;
    }
...
contract Fund is IFund, Ownable, ReentrancyGuard, FundRoles, CoreUtility, ITrancheIndex {
    ...
    constructor(
        ...
        uint256 lastDay = currentDay - 10 minutes;
    ...
    function endOfDay(uint256 timestamp) public pure override returns (uint256) {
        return
            ((timestamp.add(10 minutes) - SETTLEMENT_TIME) / 10 minutes) *
            10 minutes +
            SETTLEMENT_TIME;
    }
        ...
        )
    {
        ...
        uint256 previousDay = currentDay - 10 minutes;
        if (previousDay > timestamp) {
            previousDay = endOfDay(timestamp) - 10 minutes;
        }
    ...
    function _extrapolateNavM(
        ...
        uint256 totalValue = price.mul(
            historicalUnderlying[previousDay].mul(underlyingDecimalMultiplier)
        );
        uint256 accruedFee = totalValue
            .multiplyDecimal(dailyProtocolFeeRate)
            .mul(timestamp - previousDay)
            .div(10 minutes);
    ...
    function _extrapolateNavA(
        ...
        uint256 newNavA = navA
            .multiplyDecimal(
                UNIT.sub(dailyProtocolFeeRate.mul(timestamp - previousDay).div(10 minutes))
            )
            .multiplyDecimal(
                UNIT.add(historicalInterestRate[week].mul(timestamp - previousDay).div(10 minutes))
            );
    ...
    function settle() external nonReentrant {
        ...
        uint256 currentWeek = _endOfWeek(day - 10 minutes);
        ...
        IERC20 token = IERC20(tokenUnderlying);
        uint256 underlying;
        if (token == IERC20(address(0))) {
            underlying = address(this).balance;
        } else {
            underlying = IERC20(tokenUnderlying).balanceOf(address(this));
        }
        uint256 navA = _historicalNavs[day - 10 minutes][TRANCHE_A];
        uint256 navM;
        if (totalShares > 0) {
            navM = price.mul(underlying.mul(underlyingDecimalMultiplier)).div(totalShares);
            if (historicalTotalShares[day - 10 minutes] > 0) {
                // Update NAV of Token A only when the fund is non-empty both before and after
                // this settlement
                uint256 newNavA = navA
                    .multiplyDecimal(UNIT.sub(dailyProtocolFeeRate))
                    .multiplyDecimal(historicalInterestRate[currentWeek].add(UNIT));
                if (navA < newNavA) {
                    navA = newNavA;
                }
            }
            ...
            navM = _historicalNavs[day - 10 minutes][TRANCHE_M];
        }
        uint256 navB = calculateNavB(navM, navA);
        ...
        if (_shouldTriggerRebalance(navA, navB)) {
            ...
            fundActivityStartTime = day + 5 minutes;
            exchangeActivityStartTime = day + 5 minutes;
        } else {
            fundActivityStartTime = day;
            exchangeActivityStartTime = day + 30 seconds;
        }

        if (currentDay == currentWeek) {
            historicalInterestRate[currentWeek + 1 days] = _updateInterestRate(currentWeek);
        }
    ...
    function updateBallot(address newBallot) external onlyOwner {
        require(
            newBallot == address(0) || Address.isContract(newBallot),
    ...
    function _collectFee() private {
        ...
        if (token == IERC20(address(0))) {
            currentUnderlying = address(this).balance;
        } else {
            currentUnderlying = IERC20(tokenUnderlying).balanceOf(address(this));
        }
        uint256 fee = currentUnderlying.multiplyDecimal(dailyProtocolFeeRate);
        if (fee > 0) {
            if (token == IERC20(address(0))) {
                // payable(feeCollector).transfer(fee);
                (bool success, ) = payable(address(feeCollector)).call{value: fee}("");
    ...
    function _settlePrimaryMarkets(uint256 day, uint256 price) private {
        ...
        uint256 prevNavM = _historicalNavs[day - 10 minutes][TRANCHE_M];
        ...
        for (uint256 i = 0; i < primaryMarketCount; i++) {
            ...
            uint256 totalShares_ = totalShares; // Fix the "stack too deep" error
            uint256 price_ = price; // Fix the "stack too deep" error
            uint256 underlying_ = underlying;
            uint256 prevNavM_ = prevNavM;
            IERC20 token_ = token;
            ...
            (
                ...
                uint256 redemptionUnderlying,
                uint256 fee
            ) = pm.settle(day_, totalShares_, underlying_, price_, prevNavM_);
            ...
            if (creationUnderlying > redemptionUnderlying) {
                pm.transferToFund(creationUnderlying - redemptionUnderlying);
            } else if (redemptionUnderlying > creationUnderlying) {
                if (token_ == IERC20(address(0))) {
                    // payable(address(pm)).transfer(redemptionUnderlying - creationUnderlying);
                    uint256 amount = redemptionUnderlying - creationUnderlying;
                    (bool success, ) = payable(address(pm)).call{value: amount}("");
                    require(success, "eth transfer failure");
                } else {
                    token_.safeTransfer(address(pm), redemptionUnderlying - creationUnderlying);
                }
            }
            if (fee > 0) {
                if (token_ == IERC20(address(0))) {
                    // payable(address(feeCollector)).transfer(fee);
                    (bool success, ) = payable(address(feeCollector)).call{value: fee}("");
                    require(success, "eth transfer failure");
                } else {
                    token_.safeTransfer(address(feeCollector), fee);
                }
            }
        }
    }
