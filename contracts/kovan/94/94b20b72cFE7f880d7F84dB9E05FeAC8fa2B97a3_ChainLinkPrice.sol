...
contract ChainLinkPrice is ChainlinkClient, Ownable, IPriceContract {
    ...
    function updatePrice(
        ...
        urlApi = urlApi.append(symbol).append("-USD").append(
            "/candles?start=").append(_timestamp.convertTimestampToDateTime());
        urlApi = urlApi.append("&end=").append((_timestamp + 5*60).convertTimestampToDateTime());
...
library DateTime {
    ...
    function convertTimestampToDateTime(uint256 _timestamp)
        internal
        pure
        returns (string memory)
    {
        string memory prefix = "0";
        string memory year = ToString(getYear(_timestamp));
        string memory month = getMonth(_timestamp) < 10
            ? prefix.append(ToString(getMonth(_timestamp)))
            : ToString(getMonth(_timestamp));
        string memory day = getDay(_timestamp) < 10
            ? prefix.append(ToString(getDay(_timestamp)))
            : ToString(getDay(_timestamp));
        string memory hour = getHour(_timestamp) < 10
            ? prefix.append(ToString(getHour(_timestamp)))
            : ToString(getHour(_timestamp));
        string memory minute = getMinute(_timestamp) < 10
            ? prefix.append(ToString(getMinute(_timestamp)))
            : ToString(getMinute(_timestamp));
        string memory second = getSecond(_timestamp) < 10
            ? prefix.append(ToString(getSecond(_timestamp)))
            : ToString(getSecond(_timestamp));
        string memory date = year.append("-").append(month).append("-").append(
            day
        );
        string memory time = hour.append(":").append(minute).append(":").append(
            second
        );
        return date.append("T").append(time);
    }

    function ToString(uint16 _i) internal pure returns (string memory) {
        if (_i == 0) {
            return "0";
        }
        uint256 j = _i;
