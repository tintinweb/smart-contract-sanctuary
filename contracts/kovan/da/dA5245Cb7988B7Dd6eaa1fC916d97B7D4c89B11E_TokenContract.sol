...
interface TokenInterface is IERC20, IAccessControl {
    ...
    event TokensSalvaged(address indexed tokenAddress, address indexed reciever, uint amount);
...
contract TokenContract is ERC20, Pausable, AccessControl, TokenInterface {
    ...
    constructor() ERC20("CryptonitePRO Token", "CPRO") {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        
        _supplyCap = 21000000000 * 10**18;
    }

    // ===============================
    // ==== MODERATOR_ROLE REGION ====
    // ===============================
    function pause() onlyRole(MODERATOR_ROLE) public override returns(bool) {
        require(!paused(), "Contract is already paused!");
        _pause();
        return true;
    }
    function unpause() onlyRole(MODERATOR_ROLE) public override returns(bool) {
        require(paused(), "Contract is not paused!");
        _unpause();
        return true;
    }
    function mint(address to_, uint256 amount_) onlyRole(MODERATOR_ROLE) public override returns(bool) {
        require(totalSupply() <= getSupplyCap(), "New tokens cannot be minted!");
    ...
    function _beforeTokenTransfer(address from_, address to_, uint256 amount_) internal override {
        require(!paused() || from_ == address(0), "Pausable: paused");
    ...
    // ==== DEFAULT_ADMIN REGION ====
    // ==============================
    function setRoleAdmin(bytes32 role_, bytes32 adminRole_) onlyRole(DEFAULT_ADMIN_ROLE) public override returns (bool) {
    ...
    //* PUBLIC GETTERS REGION *//
    /////////////////////////////
    function getSupplyCap() public view override returns (uint) {
        return _supplyCap;
    }
}