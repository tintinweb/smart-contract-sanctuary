...
library AdaptiveFee {
    ...
    uint32 constant volumeGamma = 30;

    // Maybe uint112
    function getFee(uint128 volatility, uint256 volumePerLiquidity) internal pure returns(uint256 fee) {
        uint256 sigm1;
        if (volatility > beta1) {
            uint256 x = volatility - beta1;
            uint256 ex = exp(x, gamma1);
            sigm1 = ((10000*alpha1*(ex))/(gamma1**7 + ex))/10000;
        } else {
            uint256 x = beta1 - volatility;
            uint256 ex = gamma1**7 + exp(x, gamma1);
            sigm1 = ((10000*alpha1*gamma1**7)/(ex)) / 10000;
        }

        uint256 sigm2;
        if (volatility > beta2) {
            uint256 x  = volatility - beta2;
            uint256 ex = exp(x, gamma2);
            sigm2 = ((10000*alpha2*(ex))/(gamma2**7 + ex))/10000;
        } else {
            uint256 x = beta2 - volatility;
            uint256 ex = gamma2**7 + exp(x, gamma2);
            sigm2 = ((10000*alpha2*gamma2**7)/(ex)) / 10000;
        }

        fee = sigm1 + sigm2;

        if (volumePerLiquidity > volumeBeta) {
            uint256 x  = volumePerLiquidity - volumeBeta;
            uint256 ex = exp(x, volumeGamma);
            fee = 500 + ((10000*fee*(ex))/(volumeGamma**7 + ex))/10000;
        } else {
            uint256 x = volumeBeta - volumePerLiquidity;
            uint256 ex = volumeGamma**7 + exp(x, volumeGamma);
            fee = 500 + ((10000*fee*volumeGamma**7)/(ex)) / 10000;
        }
    }
