...
interface Locker{
    ...
    */
    ...
    function getTotalDepositsByTokenAddressAddress(address _token) external view returns (uint256);
}
...
contract LockerUtils {
    ...
    function locks(Locker _lock, address _token, uint256 _offset, uint256 _limit) external view returns(
        ...
        uint256 lockCount = _lock.getTotalDepositsByTokenAddressAddress(_token);
        ...
        for (lockId = _offset; lockId <= limit; lockId++) {
            (uint256 id,,,uint256 amount,,) = _lock.getDepositByTokenAddress(_token, lockId);
    ...
    function locksTimes(Locker _lock, address _token, uint256 _offset, uint256 _limit) external view returns(
        ...
        for (lockId = _offset; lockId <= limit; lockId++) {
            (,,,,uint256 unlock,) = _lock.getDepositByTokenAddress(_token, lockId);
    ...
    function locksDetails(Locker _lock, address _token, uint256 _offset, uint256 _limit) external view returns(
        ...
        for (lockId = _offset; lockId <= limit; lockId++) {
            (,, address withdrawAddress,,,bool withdrawn) = _lock.getDepositByTokenAddress(_token, lockId);
