...
contract MdexPair is IMdexPair, MdexERC20 {
        ...
        returns (bool feeOn)
    {
        ...
        feeOn =
            feeTo != address(0) &&
            IMdexFactory(factory).getPairRate(address(this)) != 9;
        uint256 _kLast = kLast;
        // gas savings
        if (feeOn) {
            if (_kLast != 0) {
                uint256 rootK =
                    SafeMath.sqrt(uint256(_reserve0).mul(_reserve1));
                ...
                if (rootK > rootKLast) {
                    ...
                    uint256 denominator =
                        rootK
                            .mul(
                            IMdexFactory(factory).getPairRate(address(this))
                        )
                            .add(rootKLast);
    ...
    function swap(
        ...
        );
        {
            ...
            uint256 balance0Adjusted =
                balance0.mul(1e4).sub(
                    amount0In.mul(
                        IMdexFactory(factory).getPairFees(address(this))
                    )
                );
            uint256 balance1Adjusted =
                balance1.mul(1e4).sub(
                    amount1In.mul(
                        IMdexFactory(factory).getPairFees(address(this))
                    )
                );
