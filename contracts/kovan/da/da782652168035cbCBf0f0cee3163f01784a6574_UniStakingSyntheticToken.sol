...
contract UniStaking is TwoStageOwnable, UniStakingTokensStorage {
    ...
    function stake(uint256 amount) public onlyPositiveAmount(amount) {
        ...
        require(
            currentBlockNumber >= intervals[0].startBlockNumber
            && currentBlockNumber < intervals[1].startBlockNumber,
            "Stake disabled in this interval"
        );
        _lockRewards(sender);
    ...
    function setInitialRewardStrategy(
        ...
        require(
            intervals[1].startBlockNumber <= startBlockNumber,
            "Initial reward strategy start block number less than second interval start"
        );
