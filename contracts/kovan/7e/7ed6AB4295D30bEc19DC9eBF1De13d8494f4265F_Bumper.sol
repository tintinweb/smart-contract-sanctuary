...
contract Bumper is IBumper, BumperAccessControl 
{
    ...
    function getTakerPosition(address token, uint id) public view override returns (TakerPosition memory) {
        address market = config.getMarket(token);
        return IMarket(market).getTakerPosition(id);
    }

    function getMakerPosition(address token, uint id) public view override returns (MakerPosition memory) {
        address market = config.getMarket(token);
        return IMarket(market).getMakerPosition(id);
    }

    /// @notice protect native token (ETH for Etherium blockchain)
    function protectNative(uint16 risk, uint16 term, bool autorenew) external override payable returns (uint id) {
        _wrapETH();
        address market = config.getWrappedNativeMarket();
        return IMarket(market).protect(msg.sender,msg.value,risk,term,autorenew);        
    }

    function protectWithPermit(address token, uint amount, uint16 risk, uint16 term, bool autorenew, 
        uint256 deadline,
        uint8 v,
        bytes32 r,
    ...
    /// @notice protect given amount of token with given risk, term 
    /// @param risk - percentage with 100 multiplier (9000 - 90%)
    function protect(address token, uint amount, uint16 risk, uint16 term, bool autorenew) external override returns (uint id) {
        address market = config.getMarket(token);
    ...
    /// @notice close opened position 
    function close(address token, uint id) external override {
        IMarket market = IMarket(config.getMarket(token));
        require(msg.sender == market.getTakerPosition(id).owner);
        return market.close(id);
    }

    /// @notice claim protection in stable coins if token price is below flor value
    function claim(address token, uint id) external override {
        IMarket market = IMarket(config.getMarket(token));
        require(msg.sender == market.getTakerPosition(id).owner);
        return market.claim(id);
    }

    /// @notice calculate primium using current risk factors
    function premium(address token, uint amount, uint16 risk,  uint16 term) external view override returns (uint) {
        address market = config.getMarket(token);
        return IMarket(market).premium(amount, risk, term);
    }

    /// @notice create new maker position
    function deposit(address token, uint amount, uint16 risk, uint16 term) external override returns (uint id) {
        address market = config.getMarket(token);
        IERC20(config.getStable()).safeTransferFrom( msg.sender, market, amount );
        return IMarket(market).deposit(msg.sender, amount, risk, term);
    }

    /// @notice close maker position
    function withdraw(address token, uint id) external override {
        IMarket market = IMarket(config.getMarket(token));
        require(msg.sender == market.getMakerPosition(id).owner);
        return market.withdraw(id);
    }

    function rebalance(address token) external override {
        address market = config.getMarket(token);
        return IMarket(market).rebalance();
    }
...
/// @notice Protocol facade smart contract 
interface IBumper {
    /// @notice get taker position
    function getTakerPosition(address token, uint id) external view returns (TakerPosition memory);
    /// @notice get maker position
    function getMakerPosition(address token, uint id) external view returns (MakerPosition memory);
    /// @notice protect native chain tokens
    function protectNative(uint16 risk, uint16 term, bool autorenew) external payable returns (uint id);
    /// @notice protect ERC20 tokens with permit
    function protect(address token, uint amount, uint16 risk, uint16 term, bool autorenew) external returns (uint id);
    /// @notice protect ERC20 tokens with permit
    function protectWithPermit(address token, uint amount, uint16 risk, uint16 term, bool autorenew,
    ...
    /// @notice claim stable coins for given position
    function claim(address token, uint id) external;
    /// @notice calculate premium 
    function premium(address token, uint amount, uint16 risk, uint16 term) external view returns (uint);
    /// @notice open maker position
    function deposit(address token, uint amount, uint16 risk, uint16 term) external returns (uint id);
    /// @notice close maker position
    function withdraw(address token, uint id) external;
    /// @notice rebalance market
