...
contract SourceDomainSideBridge {
    ...
    //bytes[] public transactionHashs;
    bytes32[] public merkleTreeRoots;
    mapping(bytes32 => uint) public rootsBalances;
    uint public nextRootToBePayed;
    
    bytes32[DEPOSIT_MERKLE_TREE_DEPTH] branch;
    bytes32[DEPOSIT_MERKLE_TREE_DEPTH] zero_hashes;
    ...
    /*
    function send(address  _tokenAddress, address  _destination,
        uint256  _amount,
        uint256  _fee,
        uint256  _startTime,
        uint256  _feeRampup) public payable{
            TransferData memory t;
            t.tokenAddress = _tokenAddress;
            t.destination = _destination;
            t.amount = _amount;
            t.fee = _fee;
            t.startTime = _startTime;
            t.feeRampup = _feeRampup;

            this.withdraw(t); 
        }
    function sendDefault() public payable{
            TransferData memory t;
            t.tokenAddress = 0xFf795577d9AC8bD7D90Ee22b6C1703490b6512FD;
            t.destination = 0xFf795577d9AC8bD7D90Ee22b6C1703490b6512FD;
            t.amount = 1;
            t.fee = 1;
            t.startTime = 1;
            t.feeRampup = 1;
...
*/
    function withdraw(TransferData memory _transferData) public payable{
        uint256 amountPlusFee = (_transferData.amount * 10005);
        //ERC20 token = ERC20(_transferData.tokenAddress);
        
        if(_transferData.tokenAddress == 0x0000000000000000000000000000000000000000){
        ...
        bytes32 transferDataHash = sha256(abi.encodePacked(_transferData.destination));
        ...
        bytes32 node = sha256(abi.encodePacked(transferDataHash, nexTransferIdHash));
        //transactionHashs.push(h);
        this.updateMerkleBranch(node);
        if(nextTransferID%MAX_DEPOSIT_COUNT==0){
            merkleTreeRoots.push(this.get_deposit_root());
            //branch = zero_hashes;
        }
        emit Transaction(_transferData,this,nextTransferID);
        nextTransferID +=1;

        bounty = bounty + (_transferData.amount * 5);
    }
