...
contract PoolV1 {
    ...
    */
     ...
     */
	function withdraw(uint _poolIndex, bool stakeEarlyFees) public {
	    require(_poolIndex<poolInfo.length, "Invalid Pool Index");
	    updateuserinfo(0,_poolIndex);
		...
		if(_totalAmount>_balance){
			...
			uint256 pendingEarlyfees = withdrawPendingAmount(_poolIndex);
	...
	*/
	...
	function withdrawPendingAmount(uint256 _poolIndex)internal returns(uint256){
		uint _earlyfee;
         if(poolUserInfo[_poolIndex][msg.sender].pendingBalance>0){
		 _earlyfee = calculatefee(msg.sender,poolUserInfo[_poolIndex][msg.sender].pendingBalance,_poolIndex);
		 IERC20(DAI_ADDRESS).transfer(msg.sender, poolUserInfo[_poolIndex][msg.sender].pendingBalance.sub(_earlyfee));
		}
		return _earlyfee;
	}
