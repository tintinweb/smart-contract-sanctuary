...
contract Expedition is Ownable {
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            return totalEligible.mul(pool.accTokensPerShare).div(1e12).sub(totalRewardDebt);
            }
        }
    }
            return 0;
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            for (uint256 index = 0; index < user.stakeInfo.length; index++) {
                ...
                if ((currentTimeStamp - (user.stakeInfo[index].timeStamp)) >= pool.lockPeriod ){
        ...
        //    staker.rewardDebt = _amount.mul(pool.accTokensPerShare).div(1e12);
        //    staker.timeStamp = now;
        ...
        for (uint256 index = 0; index < user.stakeInfo.length; index++) {
            ...
            break;
             ...
             if ( user.stakeInfo[index].amount ==0 ){
                ...
                if(sum + user.stakeInfo[index].amount >= _amount) {
                    deductionAmount = _amount - sum;
                    timeToBreak = true;
                } else {
                    deductionAmount = user.stakeInfo[index].amount;
                }

                sum = sum + deductionAmount;
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        for (uint256 index = 0; index < user.stakeInfo.length; index++) {
            if ( user.stakeInfo[index].amount > 0){
                totalLps.add(user.stakeInfo[index].amount);
    ...
    function safeStakingTokensTransfer(address _to, uint256 _amount) internal {
        uint256 stakingTokenBalanceOnChef =
