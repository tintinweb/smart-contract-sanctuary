...
contract ShadowFaxStorage is ShadowFaxAdminStorage {
    ...
    event Invest(uint256 publicId,address investor,address token,uint256 income,uint256 spend);
...
contract ShadowFax is ShadowFaxStorage {
    ...
    function addPublicOffering(
        ...
        require(_offeringAmount > 0, "Need deposit offering token.");
    ...
    function investPublicOfferingERC20(uint256 _publicId, uint256 _amount) external {
        PublicOffering memory offering = publicOfferings[_publicId];
        require(offering.wantToken!=WETH,"Functions not support");
        investPublicOfferingInternal(_publicId, _amount);
    }

    function investPublicOfferingInternal(uint256 _publicId, uint256 _amount) internal returns (uint256){
        PublicOffering storage offering = publicOfferings[_publicId];
        ...
        if (offering.wantToken != WETH) {
            IERC20(offering.wantToken).safeTransferFrom(msg.sender, address(this), actualUsed);
        }
        //        Now needStakeAount is the amout of raising amount deposit；
        UserInfo storage user = userInfo[_publicId][msg.sender];
        uint256 obtainAmount = offering.offeringAmount.mul(actualUsed).div(offering.wantTokenAmount);
        ...
        emit Invest(_publicId, msg.sender, offering.wantToken, actualUsed, obtainAmount);
        return actualUsed;
    }

    function investPublicOfferingEth(uint256 _publicId) public payable {
        //        eth->Weth
        IWETH(WETH).deposit{value : msg.value}();
        uint256 amount = investPublicOfferingInternal(_publicId, msg.value);
