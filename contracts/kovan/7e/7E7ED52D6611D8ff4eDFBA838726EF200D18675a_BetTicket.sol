...
interface IERC721Metadata is IERC721 {
     ...
     */
    function tokenURI(uint256 tokenId) external view returns (bytes memory);
}
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
     ...
     */
    function tokenURI(uint256 tokenId) public view virtual override returns (bytes memory) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");

        bytes memory _tokenURI = _tokenURIs[tokenId];
        ...
        if (bytes(_tokenURI).length > 0) {
            return abi.encodePacked(base, _tokenURI);
        }
        // If there is a baseURI but no tokenURI, concatenate the tokenID to the baseURI.
        return abi.encodePacked(base, tokenId.toString());
    }
...
// File: contracts/BetTicket.sol


/// @title  BetTicket
/// @description  ERC 721 collectible token representing Betting Ticket
/// @author Andrej
