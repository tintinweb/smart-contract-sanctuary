    ...
    SelfPermit
{
    ...
    function cachePoolKey(address pool, PoolAddress.PoolKey memory poolKey) private returns (uint80 poolId) {
        ...
        if (poolId == 0) {
            // pool可以拿到poolId，poolId可以拿到poolKey
        ...
        )
    {
        ...
        // idempotent set
        // 缓存poolkey，保存poolKey，poolId之间的对应关系
        // pool -> poolId -> poolKey
        // poolId是自增序列
        // poolKey是token0，token1，fee的hash
        uint80 poolId =
            cachePoolKey(
                address(pool),
    ...
    function increaseLiquidity(
        ...
        (, uint256 feeGrowthInside0LastX128, uint256 feeGrowthInside1LastX128, , ) = pool.positions(positionKey);
        // TODO 此处是否有必要触发tokensOwed的更新，新增操作，后续肯定会有移除操作，移除的时候触发即可
    ...
    function collect(
        ...
        if (position.liquidity > 0) {
            // TODO 触发更新，为什么用burn来触发position更新，
