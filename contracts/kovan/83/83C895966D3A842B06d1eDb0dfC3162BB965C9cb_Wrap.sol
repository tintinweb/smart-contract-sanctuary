...
contract Wrap {
    ...
    IWrapped777 private immutable token;

    constructor(
        address tokenAddress
        ) public {
        token = IWrapped777 (tokenAddress);
    }

    function wrapToken(uint256 amount) external {
        token.token().approve(address(token), amount);
        token.wrap(amount);
    }

    function wrapTokenWithGulp(uint256 amount) external {
    token.token().transfer(address(token), amount);
    token.gulp(msg.sender);
