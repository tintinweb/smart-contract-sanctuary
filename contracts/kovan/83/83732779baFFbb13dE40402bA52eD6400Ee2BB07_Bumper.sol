...
import "./interfaces/IBumper.sol";
import "./interfaces/IProtocolConfig.sol";
import "./interfaces/IMarket.sol";

/// @notice Main entry point for all markets
contract Bumper is IBumperETH, IBumperERC20, BumperAccessControl 
{
    ...
    function premiumETH(uint amount, uint32 risk, uint32 term) external view override returns (uint) {

    }
    function protectETH(uint amount, uint32 risk, uint32 term, bool autorenew) external override returns (uint id) {
        address market = config.getETHMarket();
        return IMarket(market).protectETH(msg.sender,amount,risk,term,autorenew);        
    }

    function closeETH(uint id) external override {
        return IMarket(config.getETHMarket()).close(msg.sender, id);
    }

    function claimETH(uint id) external override {
        return IMarket(config.getMarket(address(0))).close(msg.sender,id);        
    }

    function depositETH(uint amount, uint32 term) external override returns (uint id) {

    }

    function withdrawETH(uint id) external override {

    }

    function rebalanceETH() external override {

    }

    function protectWithPermit(address token, uint amount, uint32 risk, uint32 term, bool autorenew, 
        uint256 deadline,
        uint8 v,
        bytes32 r,
        bytes32 s) external override returns (uint id) {

    }

    function protect(address token, uint amount, uint32 risk, uint32 term, bool autorenew) external override returns (uint id) {
        address market = config.getMarket(token);
        IERC20(token).safeTransferFrom( msg.sender, market, amount );
        return IMarket(market).protect(msg.sender,amount,risk,term,autorenew);
    }


    function close(address token, uint id) external override {
        return IMarket(config.getMarket(token)).close(msg.sender,id);
    }

    function claim(address token, uint id) external override {

    }
    function premium(address token, uint amount, uint32 risk, uint32 term) external view override returns (uint) {

    }
    function deposit(address token, uint amount, uint32 term) external override returns (uint id) {

    }

    function withdraw(address token, uint id) external override {

    }

    function rebalance(address token) external override {
...
interface IBumperERC20 {
    function protect(address token, uint amount, uint32 risk, uint32 term, bool autorenew) external returns (uint id);
    function protectWithPermit(address token, uint amount, uint32 risk, uint32 term, bool autorenew,
        uint256 deadline, uint8 v, bytes32 r, bytes32 s) external returns (uint id);
    function close(address token, uint id) external;
    function claim(address token, uint id) external;
    function premium(address token, uint amount, uint32 risk, uint32 term) external view returns (uint);
    function deposit(address token, uint amount, uint32 term) external returns (uint id);
    function withdraw(address token, uint id) external;
    function rebalance(address token) external;
}

interface IBumperETH {
    function protectETH(uint amount, uint32 risk, uint32 term, bool autorenew) external returns (uint id);
    function closeETH(uint id) external;
    function claimETH(uint id) external;
    function premiumETH(uint amount, uint32 risk, uint32 term) external view returns (uint);
    function depositETH(uint amount, uint32 term) external returns (uint id);
    function withdrawETH(uint id) external;
    function rebalanceETH() external;
}
