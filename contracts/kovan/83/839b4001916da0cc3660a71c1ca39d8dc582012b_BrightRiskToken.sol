...
contract BrightRiskToken is ERC20Upgradeable, OwnableUpgradeable, IBrightRiskToken {
    ...
    // Ratio(%) to use for the first minting action (1% = 1e16, 100% = 1e18)
    mapping(address => uint256) internal _initialPositionUnits;
    ...
    function __BrightRiskToken_init(
        ...
        __ERC20_init("Bright Risk Token", "BRI");
        __Ownable_init();
        //TODO Mint initial liquidity
    ...
    function deposit(uint256 _amount) external {
        require(_amount > 0, "BrightRiskToken: BRI4");
        base.transferFrom(_msgSender(), address(this), _amount);
        deposits[_msgSender()] += _amount;
    }

    function stakeAt(address _controllerAddress, uint256 _amount) external {
        require(_amount > 0, "BrightRiskToken: BRI5");
        require(_amount <= pendingDeposits(), "BrightRiskToken: BRI6");
        require(_positionControllers.contains(_controllerAddress), "BrightRiskToken: BRI2");
        base.approve(_controllerAddress, _amount);
        uint256 _mintedPositionAmount = IPositionController(_controllerAddress).stake(_amount);
        _mintIndex(_controllerAddress, _mintedPositionAmount);
    }
    ...
    function stakeAt(
        ...
        require(_positionControllers.contains(_controllerAddress), "BrightRiskToken: BRI2");
        base.approve(_controllerAddress, _amount);
        uint256 _mintedPositionAmount = IPositionController(_controllerAddress).stake(
            _amount,
            _investmentAsset
        );
        _mintIndex(_controllerAddress, _mintedPositionAmount);
    ...
    function _mintIndex(address _controllerAddress, uint256 _mintedPositionAmount) internal {
        _mint(_msgSender(), convertPositionToIndex(_controllerAddress, _mintedPositionAmount));
        _accrueManagerFee();
    }

    function addController(address _controllerAddress, uint256 _virtualUnits) external onlyOwner {
        require(_controllerAddress != address(0), "BrightRiskToken: BRI1");
        _positionControllers.add(_controllerAddress);
        _initialPositionUnits[_controllerAddress] = _virtualUnits;
    }

    function convertIndexToPosition(address _controllerAddress, uint256 _amount)
        public
        view
        returns (uint256)
    {
        return _amount.mul(_positionToIndexRatio(_controllerAddress)).div(PERCENTAGE_100);
    }

    function convertPositionToIndex(address _controllerAddress, uint256 _amount)
        public
        view
        returns (uint256)
    {
        return _amount.mul(PERCENTAGE_100).div(_positionToIndexRatio(_controllerAddress));
    }

    function _positionToIndexRatio(address _controllerAddress) internal view returns (uint256) {
        uint256 _currentTotalSupply = totalSupply();

        if (_currentTotalSupply == 0) {
            return _initialPositionUnits[_controllerAddress];
        }
        uint256 _positionSupply = IPositionController(_controllerAddress).positionSupply();
        return _positionSupply.mul(PERCENTAGE_100).div(_currentTotalSupply);
    }
    ...
    function pendingDeposits() public view override returns (uint256) {
