...
import "./interfaces/IERC1155RG.sol";
import "./upgradablecontracts/token/ERC20/IERC20Upgradeable.sol";
import "./upgradablecontracts/utils/ReentrancyGuardUpgradeable.sol";
import "./TokensRecoverableUpg.sol";

contract ERC1155Game is
    OwnableUpgradeable,
    TokensRecoverableUpg,
    ReentrancyGuardUpgradeable
{
    ...
    mapping(uint256 => bool) public isDish; // tokenID => bool
    ...
    event LogCreateDish(
        ...
        string _Uri,
        bytes _data,
        uint256[] _burnTokenIdRequired,
        uint256[] _burnTokensQtyRequired
    );
    event CookedDish(
        uint256 tokenId,
        address owner,
        uint256[] ingredientIds,
        uint256[] ingredientQty
    );

    function initialize(address _tokenAddress, address _beneficiary)
        public
        initializer
    {
    ...
    // to get the token ids to burn particular dish NFT
    function getBurnTokensDish(uint256 id)
        public
        view
        returns (uint256[] memory, uint256[] memory)
    {
        uint256 newTotalSupply = ERC1155Token.totalSupply(id).add(1);

        uint256 multiplierQuantity = newTotalSupply.mul(newTotalSupply);
        uint256 length = tokenType[id].burnTokensQtyRequired.length;

        uint256[] memory newBurnTokenQtyReqd = tokenType[id]
            .burnTokensQtyRequired;

        for (uint256 i = 0; i < length; i++) {
            newBurnTokenQtyReqd[i] = newBurnTokenQtyReqd[i].mul(
                multiplierQuantity
            );
        }

        return (tokenType[id].burnTokenIdRequired, newBurnTokenQtyReqd);
    }

    // to create particular dish NFT by admin
    function createDishNFT(
        address to,
        uint256 initialSupply,
        string calldata _Uri,
        bytes calldata _data,
        uint256[] memory _burnTokenIdRequired,
        uint256[] memory _burnTokensQtyRequired
    ) public onlyAdmin nonReentrant returns (uint256) {
        uint256 newId = ERC1155Token._currentTokenID().add(1);
        isDish[newId] = true;

        uint256 id = ERC1155Token.create(to, initialSupply, _Uri, _data);

        require(newId == id, "Game: newId not equal to presumed id of ERC1155");
        ...
        emit LogCreateDish(
            ...
            _Uri,
            _data,
            _burnTokenIdRequired,
            _burnTokensQtyRequired
        );
        return id;
    }

    // user calls this function to cook particular dish NFT by burning ingredient NFTs required
    // Note: square law is used after every mint, so ingredient requirement rises every mint to square law - 1,4,9,16 and so on..
    function cookDish(
        uint256 id,
        address owner,
        bytes memory data
        ...
        require(
            owner == _msgSender() ||
                ERC1155Token.isApprovedForAll(owner, _msgSender()),
        ...
        ERC1155Token.burnBatch(
            owner,
            tokenType[id].burnTokenIdRequired,
            newBurnTokenQtyReqd
        );
...
import "./upgradablecontracts/token/ERC20/SafeERC20Upgradeable.sol";
import "./upgradablecontracts/access/OwnableUpgradeable.sol";
