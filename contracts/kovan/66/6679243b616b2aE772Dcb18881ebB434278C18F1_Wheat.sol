...
contract Wheat is ERC777, IERC777Recipient, ReentrancyGuard {
     ...
     */
    IWrapped777 private immutable lpToken;
    ...
    constructor(
        ...
        lpToken = IWrapped777(_token);
        lpToken20 = IUniswapV2Pair(_lpToken20Address);
        ...
        requireFarm(_msgSender(), true) // Ensure the address you are releasing has a farm on the LAND
    {
        ...
        IERC777(token).send(_msgSender(), amountLP, ""); // [RE-ENTRANCY WARNING] external call, must be at the end
        // Send a % to the treasury on release
        ...
        )
    {
        ...
        (,,, uint256 landBalance) = getLPDetails(lpLandBalance); // underlying Land balance
        uint256 myRatio = getAddressRatio(targetAddress);
        uint256 globalRatio = getGlobalRatio();
