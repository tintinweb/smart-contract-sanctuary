...
abstract contract Control is AccessControl {
	...
	uint32 internal period = 4 minutes;
	...
	function setPriceHistoryLength() external isPermitToChange {
		IUniswapV3Pool(_poolAddress).increaseObservationCardinalityNext(40320);
	}
...
import "@openzeppelin/contracts/utils/math/Math.sol"; // TODO math delete
...
contract Cryptium is CrptERC20, Control, ReentrancyGuard {
	...
	event Log(string msg, uint256 val1, uint256 val2, uint256 val4, uint256 val5);
	...
	address internal _crpto;
	address internal _crptp;
		...
		)
	{
		...
		emit Log(
			"curPrice monthsPassAll monthsPassFromRebase, oldPrice, newPrice",
			curPrice,
			monthsPassAll,
			oldPrice,
			newPrice
		...
		emit Log(
			"price1, price2, supplyDelta, isInflRebase",
			monthsPassFromRebase,
			price1,
			price2,
			supplyDelta
		);
	 ...
	 * @return newPrice
	 */
	function _getTWAT() private initialized returns (uint256 oldPrice, uint256 newPrice) {
		uint32 valueCount = 3;
	...
	function _getPrice(
		...
		uint32 period,
		uint8 decDiff
	) internal initialized returns (uint256 oldPrice, uint256 newPrice) {
