...
contract PoolStake is Owned {
    ...
    uint internal totalETHRemoved;
    uint internal totalTokenRemoved;
    ...
    function addLiquidity(uint _term, uint _multiplier) external payable {
        ...
        if (mul == 1) 
        _withMultiplier(term, provided_ETH, provided_token, wethRP,  tokenRP);
    ...
    function withdrawLiquidity(uint _lpAmount) external returns(bool) {
        ...
        totalETHRemoved = totalETHRemoved.add(amountETH);
        totalTokenRemoved = totalTokenRemoved.add(amountToken);
    ...
    function withdrawUserLP() external returns(bool) {
        ...
        totalTokenRemoved = totalTokenRemoved.add(provided_token);
        totalETHRemoved = totalETHRemoved.add(provided_ETH);
     ...
     */ 
    function getTotalLP() external view returns(uint) {
    ...
    function analytics() external view returns(uint Total_ETH_Provided, 
        ...
        uint Total_ETH_Removed, uint Total_Tokens_Removed, 
        uint Current_Term_1, uint Current_Term_2, 
        uint Current_Term_3, uint Current_Term_4
    ) {
        
        return(
            totalETHProvided, totalTokenProvided, totalProviders,totalETHRemoved, 
            totalTokenRemoved, _providers[1], _providers[2], _providers[3], _providers[4]);
    }
}