...
contract ShibaPalToken is ERC20("ShibaPalToken", "SHIBAPAL"), Ownable {
  ...
  function canUnlockAmount(address _account) public view returns (uint256) {
    // When block number less than startReleaseBlock, no SHIBAPALs can be unlocked
    if (block.number < startReleaseBlock) {
      return 0;
    }
    // When block number more than endReleaseBlock, all locked SHIBAPALs can be unlocked
    else if (block.number >= endReleaseBlock) {
      return _locks[_account];
    }
    // When block number is more than startReleaseBlock but less than endReleaseBlock,
    // some SHIBAPALs can be released
    else
    {
  ...
  function unlock() external {
    require(_locks[msg.sender] > 0, "no locked SHIBAPALs");
  ...
  // @dev move SHIBAPALs with its locked funds to another account
