    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(address => bool) public isValidLP;
    ...
    function claimReward(address user, address lpToken) external override {
        ...
        require (hasLP(lpToken), "LP Token not supported");
    ...
    function hasLP(address lpToken) internal view returns (bool) {
        for (uint256 i = 0; i < poolList.length; i++) {
            if (poolList[i] == lpToken)
                return true;
        }
        return false;
    }
    ...
    function addLP(address lpToken, uint256 allocPoint) public override onlyRole(DEFAULT_ADMIN_ROLE) {
        require(!hasLP(lpToken), "LP token already registered");
        updateRewardForAll();
