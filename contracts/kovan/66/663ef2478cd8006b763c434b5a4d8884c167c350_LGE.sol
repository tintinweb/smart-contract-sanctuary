...
contract LGE is Ownable, LivePrice {
    ...
    function endLGE() public onlyOwner {
      ...
      uint256 amountOut = swapETHtoDAI(amountToSend);
      ...
      (, , uint256 amountLP) = UniSwap.addLiquidity(address(SWN), address(STBL), amountSWN, amountSTBL, amountSWN.mul(97).div(100), amountSTBL.mul(97).div(100), address(this), block.timestamp.add(180));
    ...
    function concatenateInvestorArrays() internal returns(address[] memory, uint256[] memory){
      ...
      uint256 length = LGEInvestors.length;
      for (uint256 s = 0; s < investor.length; s += 1){
        LGEInvestors[length + s] = investor[s];
        LGEScores[length + s] = score[investor[s]];
      }
      return(LGEInvestors, LGEScores);
    }
    function swapETHtoDAI(uint256 amountToSend) internal returns (uint256 amountOut) {
        address[] memory path = new address[](2);
      path[0] = UniSwap.WETH();
      path[1] = DAI; // DAI
      uint256 priceFeed = uint(getLatestPrice());
      uint256 amountOutMin = amountToSend.mul(priceFeed).div(oracleValueDivisor).mul(97).div(100); // 3% slippage
