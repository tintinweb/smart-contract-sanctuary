...
contract EnzymeVaultManager is Ownable, ReentrancyGuard {
  using SafeERC20 for IERC20;
    ...
    event SharesRedeemed(uint32 _projectId);
    ...
    function setAttributes(uint32 _projectId, uint256 _deadline) public onlyOwner() returns(bool){
    ...
    *dev for now we only allow WETH deposits */
    function depositFunds(uint32 _projectId, uint256 amountToDeposit, uint256 amountMin) public returns(bool){
      ...
      projectDetails[_projectId].amount = amountToDeposit;
    ...
    /***
    *dev for now we redeem all amounts to contract that is for a project
    *TODO: decode shares in deposit and add mapping here to only redeem shares
    */
    function redeemAllFunds(uint32 _projectId) public nonReentrant() onlyOwner() returns(bool){
      enzymeVault.redeemShares();
      emit SharesRedeemed(_projectId);
      return true;
    }
