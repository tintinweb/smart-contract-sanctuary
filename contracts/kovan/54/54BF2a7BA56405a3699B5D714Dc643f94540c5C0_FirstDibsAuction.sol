...
contract FirstDibsAuction is PullPayment, Ownable, AccessControl, ReentrancyGuard {
    ...
    struct AuctionSettings {
        uint64 duration; // defaults to globalDuration
        uint64 minimumBidIncrement; // defaults to globalMinimumBidIncrement
        uint64 commissionRate; // defaults to globalMarketCommission
        uint64 creatorRoyaltyRate; // defaults to globalCreatorRoyaltyRate
    }

    struct Auction {
        uint64 startTime; // auction start timestamp
        uint64 pausedTime; // when was the auction paused
        uint64 currentBid; // current winning bid of the auction
        uint64 reservePrice; // minimum bid threshold for auction to begin
    ...
    event AuctionCanceled(uint256 indexed auctionId, address canceledBy);

    constructor(address _iFirstDibsMarketSettings, address _iERC721TokenCreatorRegistry) public {
        require(
            _iFirstDibsMarketSettings != address(0),
            'constructor: 0 address for _iFirstDibsMarketSettings'
        );
        require(
            _iERC721TokenCreatorRegistry != address(0),
            'constructor: 0 address for _iERC721TokenCreatorRegistry'
        );
        ...
        onlyOwner
    {
        require(
            _iERC721TokenCreatorRegistry != address(0),
            'setIERC721TokenCreatorRegistry: zero address not allowed'
        );
        iERC721TokenCreatorRegistry = IERC721TokenCreator(_iERC721TokenCreatorRegistry);
    }
    ...
    function setIFirstDibsMarketSettings(address _iFirstDibsMarketSettings) external onlyOwner {
        require(
            _iFirstDibsMarketSettings != address(0),
            'setIFirstDibsMarketSettings: 0 address now allowed'
        );
        iFirstDibsMarketSettings = IFirstDibsMarketSettings(_iFirstDibsMarketSettings);
    }
     ...
     *
     * @param _nftAddress address of ERC-721 contract (e.g. latest FirstDibsToken address)
     * @param _tokenId uint256
     * @param _reservePrice uint64 reserve price in ETH
     * @param _pausedArg (optional) create the auction in a paused state
    ...
    function createAuction(
        ...
        createAuctionFullAdmin(
            ...
            101, // createAuctionFullAdmin function ignores values > 100
            101 // createAuctionFullAdmin function ignores values > 100
        );
    }
     ...
     */
    function createAuctionFullAdmin(
        address _nftAddress,
        uint256 _tokenId,
        uint64 _reservePrice,
        bool _pausedArg,
        ...
        require(!globalPaused, 'createAuctionFullAdmin: auctions are globally paused');
        ...
        require(
            _msgSender() == IERC721(_nftAddress).ownerOf(_tokenId) ||
                hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            'createAuctionFullAdmin: must be token owner or admin'
        );

        require(
            auctionIds[_nftAddress][_tokenId] == 0,
            'createAuctionFullAdmin: auction already exists'
        );

        require(_reservePrice >= 0, 'createAuctionFullAdmin: Reserve must be >= 0');
        ...
        if (hasRole(DEFAULT_ADMIN_ROLE, _msgSender())) {
            if (_auctionDurationArg > 0) {
                require(
                    ...
                    'createAuctionFullAdmin: duration must be >= time buffer'
                );
            ...
            if (_startTimeArg > 0) {
                require(
                    ...
                    'createAuctionFullAdmin: start time must be in the future'
                );
    ...
    {
        require(_amount == msg.value, 'bid: amount does not match ETH value');
        require(_amount > 0, 'bid: must be > 0');
        require(_amount >= auctions[_auctionId].reservePrice, 'bid: must be >= reserve price');
        ...
        if (auctions[_auctionId].startTime == 0) {
            ...
            uint256 priorBidAmount = auctions[_auctionId].currentBid;
            address priorBidder = auctions[_auctionId].currentBidder;
            // make sure bid meets minimum bid increment
            require(
                _amount >=
                    priorBidAmount.add(
                        priorBidAmount.mul(auctions[_auctionId].settings.minimumBidIncrement).div(
                            100
                        )
                    ),
                'bid: Must be > last bid by minimum bid increment'
            );
            _tryTransferThenEscrow(priorBidder, priorBidAmount);
        }
        // upon successul bid, transfer the funds & update the new bidder on the contract
        auctions[_auctionId].currentBid = _amount;
        ...
        notPaused(_auctionId)
    {
        ...
        if (auction.settings.commissionRate > 0) {
            // send commission fee to commission address
            commissionFee = auction.currentBid.mul(auction.settings.commissionRate).div(100);
            _tryTransferThenEscrow(iFirstDibsMarketSettings.commissionAddress(), commissionFee);
        }
        address nftCreator =
        ...
        if (nftCreator == auction.payee) {
            _asyncTransfer(auction.payee, auction.currentBid.sub(commissionFee));
        } else {
            // calculate & send creator royalty to escrow
            creatorRoyaltyFee = auction.currentBid.mul(auction.settings.creatorRoyaltyRate).div(
                100
            );
            ...
            _asyncTransfer(
                auction.payee,
                auction.currentBid.sub(creatorRoyaltyFee).sub(commissionFee)
            );
        }
        ...
        emit AuctionEnded(
            ...
            commissionFee,
            creatorRoyaltyFee,
            auction.currentBid.sub(creatorRoyaltyFee).sub(commissionFee) // seller payment
        );
    }
    ...
    {
        // can't cancel an auction with bids
        require(
            auctions[_auctionId].currentBidder == address(0),
        ...
        _delete(_auctionId);
        emit AuctionCanceled(_auctionId, _msgSender());
    }
     ...
     */
    function _endTime(uint256 _auctionId) private view returns (uint64) {
