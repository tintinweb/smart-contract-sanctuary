...
contract Loot is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function randColor(string memory rgb, string memory name) public view returns(uint256) {
        uint256 seed = uint256(keccak256(abi.encodePacked(
                ...
                ((uint256(keccak256(abi.encodePacked(rgb))))) +
    ...
    function randomRgb(string memory name) public view returns (string memory){
        string memory result = strConcat('rgb(', toString(randColor("r", name)), ",");
        result = strConcat(result, toString(randColor("g", name)), ",");
        result = strConcat(result, toString(randColor("b", name)), ")");
        return result;
    }
    ...
    function oneElement(uint8 y, string memory name) public view returns (string memory) {
        ...
        result = strConcat('<text x="10" y="', toString(y), '" class="base" fill="');
        result = strConcat(result, randomRgb(name), '" >');
        result = strConcat(result, name, '</text>');
        return result;
    }
