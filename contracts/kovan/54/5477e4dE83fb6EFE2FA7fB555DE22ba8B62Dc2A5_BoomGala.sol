...
contract BoomGala is ERC721A, Ownable, Pausable {
    ...
    constructor(
        ...
        setupSaleConfig(false, false, 0.2 ether, 2, 0.3 ether, 5, 30);
        setBaseURI(baseURI_);
    }
     ...
     */
    function reserveGiveaway(uint256 quantity) public onlyOwner {
        SaleConfig memory config = saleConfig;
        bool _publicSaleStarted = config.publicSaleStarted;
        bool _wlSaleStarted = config.wlSaleStarted;
    ...
    uint _wlMaxAmount, uint _publicPrice, uint _publicMaxAmount, uint _reservedAmount) public onlyOwner {
