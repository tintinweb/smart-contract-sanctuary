...
contract Staking is IStaking, AccessControl, ReentrancyGuard {
	...
	event Log(string val, uint256 val2, uint256 val3, uint256 val4, uint256 val5, uint256 val6);
	...
	function deposit(uint256 amount) external nonReentrant {
		...
		for (i = 0; i < _numOfRewardTokens; i++) {
			...
			emit Log("curamount amount rewardDebt pending numToken", curAmount, user.amount, user.rewardDebt[0], pending, i);
		}
	...
	function withdraw(uint256 amount) external nonReentrant {
		...
		for (i = 0; i < _numOfRewardTokens; i++) {
			...
			user.rewardDebt[i] = (user.amount * (accTokenPerShare[i])) / (_PRECISION_FACTOR[i]);
			emit Log("curamount amount rewardDebt pending numberOfToken", curAmount, user.amount, user.rewardDebt[0], pending, i);
		}
