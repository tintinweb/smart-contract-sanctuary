...
contract IGPT is Context, IERC20, Ownable {
    ...
    string private    _name = "Indie Game Protocol Test";
    string private  _symbol = "IGPT";
    uint8 private _decimals = 18;
    
    uint256 public                _taxFee = 3; 
    uint256 public          _liquidityFee = 3; 
    uint256 public               _burnFee = 1; 
    uint256 public            _specialFee = 3; 
    uint256 private       _previousTaxFee = _taxFee;
    uint256 private _previousLiquidityFee = _liquidityFee;
    uint256 private      _previousBurnFee = _burnFee;
    uint256 private _previousSpecialFee = _specialFee;
    
     address payable public _specialWalletAddress;
   ...
   uint256 private _numOfTokensToExchangeForSpecial = 500 * 10**6 * 10**18;
    ...
    constructor (address cOwner, address payable specialWalletAddress, address routerAddress) Ownable(cOwner) {
        
         _specialWalletAddress = specialWalletAddress;
        
        _rOwned[cOwner] = _rTotal;
        
      // testnet address for PCS 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
          // Mainnet address for PCS 0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    function setRouterAddress(address newRouter) public onlyOwner() {
        ...
        uniswapV2PairNew = IUniswapV2Factory(_newPancakeRouter.factory()).createPair(address(this), _newPancakeRouter.WETH());
        uniswapV2RouterNew = _newPancakeRouter;
    }
    ...
    function removeAllFee() private {
        if (_taxFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _specialFee == 0) {
            return;
        }
    ...
    function restoreAllFee() private {
        ...
        _specialFee = _previousSpecialFee;
    }
    ...
    function _transfer(
       ...
       bool overMinTokenBalance = contractTokenBalance >= _numOfTokensToExchangeForSpecial;
            if (!inSwapAndLiquify && swapAndLiquifyEnabled && overMinTokenBalance && from != uniswapV2Pair) {
                // We need to swap the current tokens to ETH and send to the special wallet
                swapTokensForEth(contractTokenBalance);
                
                uint256 contractETHBalance = address(this).balance;
                if(contractETHBalance > 0) {
                    sendETHToSpecial(address(this).balance);
                }
            }
       ...
       function sendETHToSpecial(uint256 amount) private {
            _specialWalletAddress.transfer(amount.div(2));
           
        }
        
          function manualSend() external onlyOwner() {
            uint256 contractETHBalance = address(this).balance;
            sendETHToSpecial(contractETHBalance);
        }
    ...
    function calculateBurnFee(uint256 _amount) private view returns (uint256) {
        if(_tTotal > 30000000000000 * (10**9))
