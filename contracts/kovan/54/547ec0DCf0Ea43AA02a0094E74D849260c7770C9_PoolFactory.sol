...
/**
    PoolFactory contract will create and manage all the pools.
 */

contract PoolFactory is Ownable,VRFConsumerBase {
    ...
    modifier nonReentrant{
        require(!locked, "Invalid operation - Reentrancy");
         _;
    }
    ...
    function createPool(
        ...
        pools.push(address(currentPool));
    ...
    /**
     * @notice - To get the current lending pool address of AAVE
     * @return - The address of the lending pool
     */

    function currentLendingPool() public view returns (address) {
        return
    ...
    /**
     * @notice - Sets the address of lending pool address provider
     * Can only be called by the owner.
     */

    function _setLendingPoolAddressProvider(
        address _newLendingPoolAddressProvider
    ) external onlyOwner {
        lendingPoolAddressProvider = _newLendingPoolAddressProvider;
    }

    /**
     * @notice -Returns the information about the given pool address
     * Can only be called by the owner.
     */
    ...
    function poolInfo(
        ...
        return IPool(poolAddress).poolInfo();
     }

      
    function _setcurrentPool(address _poolAddress) private onlyOwner {
        currentPool = _poolAddress;
    }

    /** 
     * Requests randomness from a user-provided seed and completes the pool.
     */
    function complete(uint256 userProvidedSeed,address _poolAddress) external returns (bytes32 requestId){
         _setcurrentPool(_poolAddress);
         return requestRandomness(keyHash, rFee, userProvidedSeed);
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        //locked=false;
        IPool(currentPool)._setRandomNumber(randomness);
       }
   ...
   /**
     ...
     */
    function forceUnlock() public onlyOwner {
        locked=false;
    }
    
    function isLocked() public view onlyOwner returns(bool) {
         return locked;
    }

    function lockPool(address _poolContract) public onlyOwner{
         IPool(_poolContract).lockPool();
    }

    function unlockPool(address _poolContract) public onlyOwner{
        IPool(_poolContract).unlockPool();
    }
...
contract Pool is Ownable {
  ...
  function unlock() internal requireLocked {
    require(lockEndBlock < block.number, "pool cannot be unlocked until the lockEndBlock");
...
interface IPool {
    ...
    function _setRandomNumber(uint) external;
    function poolInfo() external;
