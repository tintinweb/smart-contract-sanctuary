    ...
    ReentrancyGuard
{
    ...
    uint256 public minMintDeviation = 5;
    ...
    function mintWithEth(
        ...
        require(
            IVaultLibrary(vaultLibrary)
                .getFreeCollateralAsEth(msg.sender, token)
                .add(msg.value) >=
                IVaultLibrary(vaultLibrary).getMinimumCollateral(
                    tokenAmount,
    ...
    function mintWithoutCollateral(
        ...
        uint256 quote = getTokenPrice(token);
        require(
            IVaultLibrary(vaultLibrary).getFreeCollateralAsEth(
                msg.sender,
                token
            ) >=
                IVaultLibrary(vaultLibrary).getMinimumCollateral(
    ...
    function burn(
        ...
        // Withdraw fee (triggers interest update).
        ...
        uint256 available =
            IVaultLibrary(vaultLibrary)
                ...
                unitPrice
                    ...
                    .mul(
                    IVaultLibrary(vaultLibrary)
                        .getVaultMinimumRatio(msg.sender, token)
                        .sub(minMintDeviation)
                )
                    .div(1 ether)
                    .div(100)
            );
        ...
        onlyOwner
    {
        ...
        require(
            _vaultLibrary != address(0),
            "VaultLibrary cannot be address 0"
        );
    ...
    function getMinMintDeviation() external view override returns (uint256) {
        return minMintDeviation;
    }
...
contract Treasury is ITreasury, AccessControl, ReentrancyGuard, IValidator {
        ...
        for (uint256 i = 0; i < collateralTokens.length; i++) {
            uint256 collateralAsEth =
                vault.collateralBalance[collateralTokens[i]]
                    .mul(
                    Comptroller(comptroller).getTokenPrice(collateralTokens[i])
                )
                    .div(1 ether);
    ...
    function _withdrawCollateralFrom(
        ...
        require(
            IVaultLibrary(vaultLibrary).getFreeCollateralAsEth(from, fxToken) >=
                amount,
            "Not enough free collateral to withdraw"
        );
