...
contract FarmDelegate is FarmStorageV1 {
        ...
        if (block.number > usdtStartBlock) {
            if (block.number <= usdtEndBlock) {
                subtractor = pool.lastRewardBlock > usdtStartBlock
                    ? pool.lastRewardBlock
                    : usdtStartBlock;
                multiplier = block.number.sub(subtractor,"1");
            } else {
                if (pool.lastRewardBlock < usdtEndBlock) {
                    subtractor = pool.lastRewardBlock > usdtStartBlock
                        ? pool.lastRewardBlock
                        : usdtStartBlock;
                    multiplier = usdtEndBlock.sub(subtractor,"2");
                }
            }
        }
    ...
    function phase(uint256 blockNumber) public view returns (uint256) {
        ...
        if (blockNumber > startBlockOfGan) {
            return (blockNumber.sub(startBlockOfGan,"4").sub(1,"3")).div(halvingPeriod);
        }
        return 0;
    }
    ...
    function isHalvingPeriod0(uint256 blockNumber) public view returns (bool) {
        ...
        if (blockNumber > startBlockOfGan) {
            return
                blockNumber.sub(startBlockOfGan,"6").sub(1,"5").mod(halvingPeriod) <
                halvingPeriod0;
        }
        ...
        returns (uint256, uint256)
    {
        ...
        if (switchBlock >= block.number) {
            blockLockReward = blockLockReward.add(
                (block.number.sub(_lastRewardBlock,"10")).mul(
            ...
            if (switchBlock > _lastRewardBlock) {
                blockLockReward = blockLockReward.add(
                    (switchBlock.sub(_lastRewardBlock,"11")).mul(
                        ganPerBlockConcentratedMining.div(2**m)
                    )
                );
                blockReward = blockReward.add(
                    (block.number.sub(switchBlock,"12")).mul(
                        ganPerBlockContinuousMining.div(2**m)
                    )
                );
            } else {
                blockReward = blockReward.add(
                    (block.number.sub(_lastRewardBlock,"13")).mul(
                        ganPerBlockContinuousMining.div(2**m)
                    )
                );
            }
        ...
        if (block.number > usdtStartBlock && lpSupply != 0) {
            ...
            if (block.number <= usdtEndBlock) {
                subtractor = pool.lastRewardBlock > usdtStartBlock
                    ? pool.lastRewardBlock
                    : usdtStartBlock;
                multiplier = block.number.sub(subtractor,"14");
            } else {
                if (pool.lastRewardBlock < usdtEndBlock) {
                    subtractor = pool.lastRewardBlock > usdtStartBlock
                        ? pool.lastRewardBlock
                        : usdtStartBlock;
                    multiplier = usdtEndBlock.sub(subtractor,"15");
                }
            }
            uint256 usdtReward = multiplier
                .mul(usdtPerBlock)
        ...
        returns (uint256)
    {
        ...
        if (user.amount >= 0) {
            ...
            return userReward.add(userLockAllReward).sub(stillLockReward,"19");
        } else return 0;
    }
        ...
        if (user.lastRewardBlock < switchBlock) {
            if (block.number > switchBlock) {
                uint256 unLockGanReward = block
                    ...
                    .div(halvingPeriod1);
                realLockGanReward = totalLockPendingAmount.sub(unLockGanReward,"23");
            } else realLockGanReward = totalLockPendingAmount;
        } else {
            uint256 unLockGanReward = block
                .number
                .sub(user.lastRewardBlock,"24")
                .mul(totalLockPendingAmount)
                .div(halvingPeriod1.add(switchBlock).sub(user.lastRewardBlock,"25"));
            realLockGanReward = totalLockPendingAmount.sub(unLockGanReward,"26");
        }
        ...
        if (m > n) {
            ...
            while (m > n) {
                ...
                if (switchBlock > lastRewardBlock) {
                    blockLockReward = blockLockReward.add(
                        (switchBlock.sub(lastRewardBlock,"27")).mul(
                            ganPerBlockConcentratedMining.div(2**(n - 1))
                        )
                    );
                }
    ...
    function depositLPToken(
        ...
        if (user.amount >= 0) {
            uint256 userReward = user
                ...
                .div(1e12)
                .sub(user.rewardGanDebt,"28");

            uint256 userLockReward = user
                .amount
                .mul(pool.accLockGanPerShare)
                .div(1e12)
                .sub(user.rewardLockGanDebt,"29")
                .add(user.lockPending);
            stillLockReward = getUserLockReward(
                _pid,
                _user,
    ...
    function withdrawLPToken(
        ...
        if (user.amount >= 0) {
            uint256 userReward = user
                ...
                .mul(pool.accGanPerShare)
                .div(1e12)
                .sub(user.rewardGanDebt,"32");

            uint256 userLockReward = user
                .amount
                .mul(pool.accLockGanPerShare)
                .div(1e12)
                .sub(user.rewardLockGanDebt,"33")
                .add(user.lockPending);
            stillLockReward = getUserLockReward(
                _pid,
                _user,
        ...
        if (_amount > 0) {
            user.amount = user.amount.sub(_amount,"36");
            pool.totalAmount = pool.totalAmount.sub(_amount,"37");
    ...
    function newReward(
        ...
        uint256 balance = afterAmount.sub(beforeAmount,"38");
    ...
    function reDeposit(uint256 amount) public payable {
        ...
        if (amount > amountA) safeGanTransfer(msg.sender, amount.sub(amountA,"42"));
        if (msg.value > amountB) msg.sender.transfer(msg.value.sub(amountB,"43"));
        ...
        returns (uint256, uint256)
    {
        ...
        if (user.amount >= 0) {
            ...
            return (
                userReward.add(userLockAllReward).sub(stillLockReward,"46"),
                stillLockReward
            );
        } else return (0, 0);
    }
