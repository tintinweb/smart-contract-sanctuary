...
// File: contracts\Token\OilerToken.sol
...
// File: contracts\IMultipleDistribution.sol
...
// File: contracts\MultipleDistribution.sol
...
contract MultipleDistribution is Ownable, IMultipleDistribution {
    ...
    constructor(uint8 _pool) public {
        ...
        if (POOL_NUMBER == 3) {
            TOTAL_STAKE = 13_621_102 ether; // Private Offering supply. `ether` is used as the token has 18 decimals
        }
        if (POOL_NUMBER == 4) {
            TOTAL_STAKE = 5_351_667 ether; // Advisors Reward supply. `ether` is used as the token has 18 decimals
        }
    }
    ...
    function withdraw() external {
        require(IDistribution(distributionAddress).isInitialized(), "Distribution should be initialized to enable withdrawals");
        if (POOL_NUMBER == 3) { // Private Offering timelock for the first 5 days
            require(_now() >= IDistribution(distributionAddress).distributionStartTimestamp().add(5 minutes), "Private Offering withdrawals are locked during LBP event");
        }
