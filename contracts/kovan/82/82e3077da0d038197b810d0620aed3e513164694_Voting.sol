...
contract Voting is Ownable, WhitelistedAddresses{
    ...
    event Vote(address indexed voter, bool indexed vote, string indexed reason);
    event Propose(address indexed proposer, string indexed proposal);
    ...
    function addProposal(address _callAddress, bytes calldata __calldata, uint88 _value, string calldata _proposal) public onlyProposer {
        ...
        emit Propose(msg.sender, _proposal);
    }
    ...
    function execute(uint proposalID) public{
        ...
        require(proposal.votesYes + proposal.votesNo >= quorum, "Under quorum");
