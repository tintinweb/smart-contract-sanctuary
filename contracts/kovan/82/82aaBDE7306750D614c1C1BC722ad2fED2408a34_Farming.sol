...
contract Farming is FarmingFromNFT {
    ...
    function COLLATERAL_POOL_pendingReward() external view returns (uint256) {
        return _COLLATERAL_POOL_pendingReward(collateralProvider[msg.sender]);
    }
        ...
        onlyPlatform
    {
        ...
        require(
            u.ethCollateralProvided >= amount,
...
contract FarmingFromNFT is VestingContract {
    ...
    // Allow to see the reward from this pool for msg.sender
    function NFT_POOL_pendingReward() external view returns (uint256) {
        return _NFT_POOL_pendingReward(nftProvider[msg.sender]);
    }
