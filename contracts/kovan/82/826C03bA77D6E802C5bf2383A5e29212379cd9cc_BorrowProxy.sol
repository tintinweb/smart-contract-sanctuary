...
contract BorrowProxy is IBorrowProxy, ConfigBase {
    ...
    event BorrowAmount(address indexed mintLever, address indexed user, address[] supplyTokens, uint256[] supplyAmounts, address indexed borrowToken, uint256 borrowAmount);
    ...
    function borrowV1(address owner, uint256 id, address tokenA, uint256 amountA, address tokenB, uint256 amountB, address borrowToken, uint256 leverage, uint256 deadLine) override external returns (uint256, uint256, uint256) {
        ...
        leverage = this.getBorrowAmountV1(tokenA, amountA, tokenB, amountB, borrowToken, leverage);
        // leverage->borrowAmount
        emitBorrowAmount(owner, tokenA, amountA, tokenB, amountB, borrowToken, leverage);
        if (0 < leverage) {
            ...
            if (tokenA == borrowToken) {
                ...
                amountA = amountA.add(swap(borrowToken, tokenA, leverage, deadLine));
            }
        }
        (amountA, amountB) = trim(tokenA, amountA, tokenB, amountB, deadLine);
    ...
    function borrowV2(address owner, uint256 id, address tokenA, uint256 amountA, address borrowToken, uint256 leverage, uint256 deadLine) override external returns (uint256, uint256) {
        require(borrowAccess[msg.sender], "publics:not_borrow_access");
        config.isBlacklist(owner);
        config.isOpen(msg.sender);
        config.isLeverage(msg.sender, leverage);
        ...
        require(0 < amountA, "publics:amounta_is_zero");
        config.getApproveProxy().claim(tokenA, owner, address(this), amountA);
        leverage = this.getBorrowAmountV2(tokenA, amountA, borrowToken, leverage);
        // leverage->borrowAmount
        emitBorrowAmount(owner, tokenA, amountA, address(0), 0, borrowToken, leverage);
        if (0 < leverage) {
            uint256 code = config.getLoanPublics(borrowToken).doCreditLoanBorrow(owner, leverage, id, ILoanTypeBase.LoanType.MINNING_SWAP_PROTOCOL);
            require(0 == code, "publics:loan_publics_borrow_error");
            amountA = amountA.add(swap(borrowToken, tokenA, leverage, deadLine));
        ...
        IERC20(tokenA).safeTransfer(msg.sender, amountA);
        return (amountA, leverage);
    }
    
    function getBorrowAmountV1(address tokenA, uint256 amountA, address tokenB, uint256 amountB, address borrowToken, uint256 leverage) external view returns (uint256) {
        leverage = leverage.sub(10);
        uint256 borrowAmount;
        if (0 < leverage) {
            IExchange exchange = config.getExchange();
            uint256 decimal = IERC20(tokenA).decimals();
            uint256 price = exchange.swapEstimateOut(tokenA, borrowToken, 10 ** decimal);
            require(0 < price, "publics:swap_estimate_out_is_zero");
            uint256 value = leverage.mul(amountA).mul(price);
            borrowAmount = value.div(10 ** decimal).div(10);
            decimal = IERC20(tokenB).decimals();
            price = exchange.swapEstimateOut(tokenB, borrowToken, 10 ** decimal);
            require(0 < price, "publics:swap_estimate_out_is_zero");
    ...
    function getBorrowAmountV2(address tokenA, uint256 amountA, address borrowToken, uint256 leverage) external view returns (uint256) {
        leverage = leverage.sub(10);
        uint256 borrowAmount;
        if (0 < leverage) {
            uint256 oneA = 10 ** IERC20(tokenA).decimals();
            uint256 price = config.getExchange().swapEstimateOut(tokenA, borrowToken, oneA);
            require(0 < price, "publics:swap_estimate_out_is_zero");
            uint256 value = leverage.mul(amountA).mul(price);
            borrowAmount = value.div(oneA).div(10);
        }
        return borrowAmount;
    }
    
    function trim(address tokenA, uint256 amountA, address tokenB, uint256 amountB, uint256 deadLine) internal returns (uint256, uint256) {
        IExchange exchange = config.getExchange();
        uint256 oneA = 10 ** IERC20(tokenA).decimals();
        uint256 price = exchange.swapEstimateOut(tokenA, tokenB, oneA);
        require(0 < price, "publics:swap_estimate_out_is_zero");
        ...
        if (valueA < valueB) {
            ...
            if (0 < swapB) {
                ...
                amountA = amountA.add(swap(tokenB, tokenA, swapB, deadLine));
            }
        }else {
            uint256 swapA = valueA.sub(valueB).div(price).div(2);
            if (0 < swapA) {
                amountA = amountA.sub(swapA);
                amountB = amountB.add(swap(tokenA, tokenB, swapA, deadLine));
            }
        }
        return (amountA, amountB);
    }
    
    function emitBorrowAmount(address owner, address tokenA, uint256 amountA, address tokenB, uint256 amountB, address borrowToken, uint256 borrowAmount) internal {
        uint8 len;
        if (0 < amountA) {
            len++;
        }
        ...
        address[] memory supplyTokens = new address[](len);
        uint256[] memory supplyAmounts = new uint256[](len);
        if (0 < amountB) {
            len--;
            supplyTokens[len] = tokenB;
            supplyAmounts[len] = amountB;
        }
        if (0 < amountA) {
            len--;
            supplyTokens[len] = tokenA;
            supplyAmounts[len] = amountA;
        }
        emit BorrowAmount(msg.sender, owner, supplyTokens, supplyAmounts, borrowToken, borrowAmount);
    }
