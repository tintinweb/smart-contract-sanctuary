...
contract swapContract is AccessControlEnumerable, Pausable, ECDSAOffsetRecovery
{
    ...
    mapping(address => bool) xdceSwapLimitsSaved;
    mapping(address => uint256 [3]) xdceSwapLimits;
    uint128 [] xdceSwapRatios = [1, 3, 5];
    ...
    event DepositXDCE(address user, uint256 xdceAmount, uint256 xdcAmount);
    ...
    ) external whenNotPaused
    {
        ...
        (uint256[3] memory xdceSwapLimitsNew, uint256 xdceAmountNew, uint256 xdcAmountNew) = calculateXdcAmount(signedAmounts, xdceAmount);
        ...
        emit DepositXDCE(sender, xdceAmountNew, xdcAmountNew);
    }

    function calculateXdcAmount(uint256[3] memory _xdceSwapLimits, uint256 xdceAmount) public view returns (
    ...
    )
    {
        ...
        address sender = _msgSender();
        uint256 balance = XDCEAddress.balanceOf(sender);
        require(balance >= depositAmount, "swapContract: Not enough balance");
        ...
        uint256 amount;
        if (depositAmount >= signedAmount) {
            amount = signedAmount;
        } else {
            amount = depositAmount;
            XDCEDepositLimit[signedAddress] = amount - depositAmount;
        }
        TransferHelper.safeTransfer(address(XDCEAddress), signedAddress, amount);
    }
    */
    /** 
