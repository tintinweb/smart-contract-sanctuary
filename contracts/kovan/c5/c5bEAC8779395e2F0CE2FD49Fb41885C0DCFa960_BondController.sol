 ...
 * totalInterestRate = 100000000
 * INTEREST_RATE_GRANULARITY = 100000000
 *
 * Day 0:
...
contract BondController is IBondController, Initializable, AccessControl {
    ...
    uint256 private constant INTEREST_RATE_GRANULARITY = 100000000;
    ...
    function init(
        ...
        everySecondInterestRate = totalInterestRate / (maturityDate - creationDate); // TODO this becomes zero if (maturityDate - creationDate) > totalInterestRate
    }
    ...
    function redeemMature(address iou, uint256 amount) external override {
        ...
        uint256 interest;
        uint256 interestSacrificedOnRedeemMature;

        if (iou == _ious[0]) {
            interest = getInterestOnRedeemMature(amount);
            interestSacrificedOnRedeemMature = getInterestSacrificedOnRedeemMature(amount);
            ...
            TransferHelper.safeTransfer(
              ...
              _msgSender(),
              interest + interestSacrificedOnRedeemMature
            );

            totalInterestSacrificed -= interestSacrificedOnRedeemMature;
