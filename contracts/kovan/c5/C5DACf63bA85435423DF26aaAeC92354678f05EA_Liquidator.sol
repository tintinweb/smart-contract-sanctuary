    ...
    ReentrancyGuardUpgradeable
{
    ...
    // Ratio of liquidation fee to be applied on redemptions.
    ...
    {
        bool isLiquidation;
        // This value is currently the return ratio for the redemption fees,
        // a new variable is not created here because of stack size limitations.
        etherAmount = 1 ether;
        {
        ...
        {
            if (!isLiquidation) {
                // Apply the redemption return ratio.
                etherAmount = vaultLibrary
                    .getLiquidationFee(from, token)
                    .mul(redemptionFeeRatio)
                    .div(1 ether)
                    .add(etherAmount);
                // Ensure the return ratio is within bounds -- check for < 2
                // so that addition can be done in the previous statement.
                require(etherAmount <= 2 ether, "0<R<=1");
            }
        }
        {
