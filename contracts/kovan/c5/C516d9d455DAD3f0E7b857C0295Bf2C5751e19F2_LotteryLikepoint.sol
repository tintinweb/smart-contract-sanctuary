...
contract LotteryLikepoint is LotteryOwnable {
    ...
    mapping (address => mapping (uint256 => ticketRound)) public countTicket;
    ...
    event MultiBuyLock(address indexed user, uint256 amount, uint256 issueIndex);
    constructor() public {
        minPrice = 500000000000000000000;
        maxNumber = 9;
    ...
    function changeTicket(ILOCKLIKE _new) public onlyAdmin {
        ticketPoint = _new;
    }   
    ...
    function  multiBuyLock(uint8[4][] calldata _numbers) external {
        require (!drawed(), 'drawed, can not buy now');
        require(!drawingPhase, 'drawing, can not buy now');
        uint256 ticket = ticketPoint.getAmountLottery(msg.sender);
        require (ticket > 0, 'you not have  ticket');
        if(countTicket[msg.sender][issueIndex].start == false) {
            countTicket[msg.sender][issueIndex].start = true;
            countTicket[msg.sender][issueIndex].initTicket = ticket;
            countTicket[msg.sender][issueIndex].leftTicket = ticket;
        }
        ...
        for (uint i = 0; i < _numbers.length; i++) {
            ...
            uint256 tokenId = lotteryNFT.newLotteryItem(msg.sender, _numbers[i], 0, issueIndex);
            lotteryInfo[issueIndex].push(tokenId);
            if (userInfo[msg.sender].length == 0) {
                totalAddresses = totalAddresses + 1;
            }
            ...
            lastTimestamp = block.timestamp;
            // totalPrice = totalPrice.add(0);
            uint64[keyLengthForEachBuy] memory numberIndexKey = generateNumberIndexKey(_numbers[i]);
            for (uint k = 0; k < keyLengthForEachBuy; k++) {
                userBuyAmountSum[issueIndex][numberIndexKey[k]]=userBuyAmountSum[issueIndex][numberIndexKey[k]].add(0);
            }
            countTicket[msg.sender][issueIndex].leftTicket = countTicket[msg.sender][issueIndex].leftTicket.sub(1);
        }
        // cake.safeTransferFrom(address(msg.sender), address(this), totalPrice);
        emit MultiBuyLock(msg.sender, countTicket[msg.sender][issueIndex].leftTicket, issueIndex);
    }    
...
interface ILOCKLIKE {
    ...
    function depositLockup(uint256 amount, uint256 _option) external returns (bool);
    function reInvest(uint256 _option) external returns (bool);
    function withdrawLockup(uint256 amount, uint256 _option) external  returns (bool);
    function getBalance() external view returns (uint256);
    function getOptionBalance(uint256 option) external view returns (uint256);
    function getOptionLength() external view returns (uint256);
    function getUser(address user, uint256 _option) external view returns (timeLock memory);
    function getAmountLottery(address user) external view returns (uint256);
    
    event ReInvest(address indexed sender, uint256 option, uint256 amount, uint256 expire);
    event DepositToken(address indexed sender, uint256 amount, uint256 expire, uint256 option, uint256 totalLock);
    event WithdrawToken(address indexed sender, uint256 amount, uint256 option, uint256 totalLock);
