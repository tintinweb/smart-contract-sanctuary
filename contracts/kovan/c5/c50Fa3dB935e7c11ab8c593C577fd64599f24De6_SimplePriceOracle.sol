...
contract PControllerErrorReporter {
    enum Error {
        NO_ERROR,
        UNAUTHORIZED,
        PCONTROLLER_MISMATCH,
...
contract TokenErrorReporter {
    enum Error {
        ...
        UNAUTHORIZED,
        BAD_INPUT,
        PCONTROLLER_REJECTION,
        PCONTROLLER_CALCULATION_ERROR,
    ...
    enum FailureInfo {
        ...
        BORROW_PCONTROLLER_REJECTION,
        LIQUIDATE_ACCRUE_BORROW_INTEREST_FAILED,
        LIQUIDATE_ACCRUE_COLLATERAL_INTEREST_FAILED,
        LIQUIDATE_COLLATERAL_FRESHNESS_CHECK,
        LIQUIDATE_PCONTROLLER_REJECTION,
        LIQUIDATE_PCONTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED,
        ...
        LIQUIDATE_SEIZE_PCONTROLLER_REJECTION,
        LIQUIDATE_SEIZE_LIQUIDATOR_IS_BORROWER,
        LIQUIDATE_SEIZE_TOO_MUCH,
        MINT_ACCRUE_INTEREST_FAILED,
        MINT_PCONTROLLER_REJECTION,
        ...
        REPAY_BORROW_PCONTROLLER_REJECTION,
        ...
        TRANSFER_PCONTROLLER_REJECTION,
...
/**
 * @title Pesa's PToken Contract
 * @notice Abstract base for PTokens
 */
contract PToken is PTokenInterface, Exponential, TokenErrorReporter {

    // testing event to check what is failing 
    event AllowedTestEvent(address indexed user, address indexed pToken, uint256 errorCode);
     ...
     */
    function initialize(PControllerInterface pController_,
        ...
        // Set the pController
        uint err = _setPController(pController_);
        require(err == uint(Error.NO_ERROR), "setting pController failed");

        // Initialize block number and borrow index (block number mocks depend on pController being set)
    ...
    function transferTokens(address spender, address src, address dst, uint tokens) internal returns (uint) {
        ...
        uint allowed = pController.transferAllowed(address(this), src, dst, tokens);
        if (allowed != 0) {
            return failOpaque(Error.PCONTROLLER_REJECTION, FailureInfo.TRANSFER_PCONTROLLER_REJECTION, allowed);
        }
        ...
        pController.transferVerify(address(this), src, dst, tokens);
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint, uint) {
        ...
        uint allowed = pController.mintAllowed(address(this), minter, mintAmount);

        emit AllowedTestEvent(minter, address(this), allowed);

        if (allowed != 0) {
            return (failOpaque(Error.PCONTROLLER_REJECTION, FailureInfo.MINT_PCONTROLLER_REJECTION, allowed), 0);
        }
        ...
        pController.mintVerify(address(this), minter, vars.actualMintAmount, vars.mintTokens);

        return (uint(Error.NO_ERROR), vars.actualMintAmount);
    }

    function claimEPN(uint256 id, address owner, uint256 phoneNumber) external returns(uint) {
        require(accountTokens[msg.sender] >= minimumClaimValue);
        require(isEPNClaimEnabled);

        return epnToken.createEPN(id, owner, phoneNumber);
    }
    ...
    function claimEPNwithENS(uint256 id, address owner, uint256 phoneNumber, string calldata ensName) external returns(uint) {
        ...
        return epnToken.createEPNwithENS(id, owner, phoneNumber, ensName);
    }
    ...
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal returns (uint) {
        ...
        uint allowed = pController.redeemAllowed(address(this), redeemer, vars.redeemTokens);

        emit AllowedTestEvent(redeemer, address(this), allowed);

        if (allowed != 0) {
            return failOpaque(Error.PCONTROLLER_REJECTION, FailureInfo.REDEEM_PCONTROLLER_REJECTION, allowed);
        }
        ...
        /* We call the defense hook */
        pController.redeemVerify(address(this), redeemer, vars.redeemAmount, vars.redeemTokens);
    ...
    function borrowFresh(address payable borrower, uint borrowAmount) internal returns (uint) {
        ...
        uint allowed = pController.borrowAllowed(address(this), borrower, borrowAmount);
        if (allowed != 0) {
            return failOpaque(Error.PCONTROLLER_REJECTION, FailureInfo.BORROW_PCONTROLLER_REJECTION, allowed);
        }
        ...
        /* We call the defense hook */
        pController.borrowVerify(address(this), borrower, borrowAmount);
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint, uint) {
        ...
        uint allowed = pController.repayBorrowAllowed(address(this), payer, borrower, repayAmount);
        if (allowed != 0) {
            return (failOpaque(Error.PCONTROLLER_REJECTION, FailureInfo.REPAY_BORROW_PCONTROLLER_REJECTION, allowed), 0);
        }
        ...
        /* We call the defense hook */
        pController.repayBorrowVerify(address(this), payer, borrower, vars.actualRepayAmount, vars.borrowerIndex);
    ...
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, PTokenInterface pTokenCollateral) internal returns (uint, uint) {
        ...
        uint allowed = pController.liquidateBorrowAllowed(address(this), address(pTokenCollateral), liquidator, borrower, repayAmount);
        if (allowed != 0) {
            return (failOpaque(Error.PCONTROLLER_REJECTION, FailureInfo.LIQUIDATE_PCONTROLLER_REJECTION, allowed), 0);
        }
        ...
        (uint amountSeizeError, uint seizeTokens) = pController.liquidateCalculateSeizeTokens(address(this), address(pTokenCollateral), actualRepayAmount);
        require(amountSeizeError == uint(Error.NO_ERROR), "LIQUIDATE_PCONTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED");
        ...
        emit LiquidateBorrowToken(liquidator, borrower, actualRepayAmount, address(pTokenCollateral), seizeTokens);

        /* We call the defense hook */
        pController.liquidateBorrowVerify(address(this), address(pTokenCollateral), liquidator, borrower, actualRepayAmount, seizeTokens);
    ...
    function seizeInternal(address seizerToken, address liquidator, address borrower, uint seizeTokens) internal returns (uint) {
        ...
        uint allowed = pController.seizeAllowed(address(this), seizerToken, liquidator, borrower, seizeTokens);
        if (allowed != 0) {
            return failOpaque(Error.PCONTROLLER_REJECTION, FailureInfo.LIQUIDATE_SEIZE_PCONTROLLER_REJECTION, allowed);
        }
        ...
        /* We call the defense hook */
        pController.seizeVerify(address(this), seizerToken, liquidator, borrower, seizeTokens);
    ...
    function _setPController(PControllerInterface newPController) public returns (uint) {
        ...
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_PCONTROLLER_OWNER_CHECK);
        }

        PControllerInterface oldPController = pController;
        // Ensure invoke pController.isPController() returns true
        require(newPController.isPController(), "marker method returned false");

        // Set market's pController to newPController
        pController = newPController;

        // Emit NewPController(oldPController, newPController)
        emit NewPController(oldPController, newPController);
    ...
    function _setEPN(EPNTokenInterface newEPNToken) external returns (uint) {
        ...
        EPNTokenInterface oldEPNToken = epnToken;

        // Set epnToken to newEPNToken
        epnToken = newEPNToken;

        emit NewEPNToken(oldEPNToken, newEPNToken);
        return uint(Error.NO_ERROR);
    }
    ...
    function _setMinimumClaimValue(uint newMinimumClaimValue) external returns (uint) {
        ...
        uint oldMinimumClaimValue = minimumClaimValue;

        minimumClaimValue = newMinimumClaimValue;

        emit NewMinimumClaimValue(oldMinimumClaimValue, newMinimumClaimValue);
    ...
    function _changeEPNClaimStatus() external returns (uint) {
        ...
        bool oldStatus = isEPNClaimEnabled;
        bool newStatus = !isEPNClaimEnabled;
        isEPNClaimEnabled = newStatus;

        emit NewIsEPNClaimStatus(oldStatus, newStatus);
...
contract PErc20 is PToken, PErc20Interface {
    ...
    function initialize(address underlying_,
        ...
        super.initialize(pcontroller_, interestRateModel_, initialExchangeRateMantissa_, name_, symbol_, decimals_);
...
contract SimplePriceOracle is PriceOracle {
    ...
    mapping(string => address) pesaTokens;
    ...
    function initialiseTokens(string[] memory symbols, address[] memory pTokens)
        public
    {
        // Check caller = admin
        require(msg.sender == admin);
        require(symbols.length == pTokens.length);

        // Associate symbols with tokens
        for (uint256 i = 0; i < symbols.length; i++) {
            pesaTokens[symbols[i]] = pTokens[i];
        }
    }
        ...
        public
    {
        ...
        for (uint256 i = 0; i < symbols.length; i++) {
            if (compareStrings(symbols[i], "ETH")) {
                setDirectPrice(
                    address(pesaTokens[symbols[i]]),
                    priceMantissa[i]
                );
            } else {
                setUnderlyingPrice(
                    PToken(pesaTokens[symbols[i]]),
                    priceMantissa[i]
                );
            }
        }
    ...
    function setUnderlyingPrice(PToken pToken, uint256 underlyingPriceMantissa)
        public
    {
        // Check caller = admin
        require(msg.sender == admin);
    ...
    function setDirectPrice(address asset, uint256 price) public {
        ...
        emit PricePosted("pETH", asset, prices[asset], price, price);
        prices[asset] = price;
    }
