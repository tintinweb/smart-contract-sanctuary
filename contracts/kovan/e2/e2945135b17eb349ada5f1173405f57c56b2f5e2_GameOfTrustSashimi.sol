...
contract GameOfTrustSashimi is GameOfTrustBase, GameOfTrustSashimiStorage {
    ...
    function initialize(uint256 _startBlock,
        ...
        require(GAN == address(0) && SASHIMI == address(0), "Already initialized");
        require(_GAN != address(0) && _SASHIMI != address(0), "Invalid param");
    ...
    function redeemGAN() external onlyAdmin {
        ...
        for (uint i = 0; i < poolInfo.length; i++) {
            ...
            totalSurplusAmount = totalSurplusAmount.add(calculateSurplusGan(pool, pool.rewardRate.add(totalRate)));
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external {
        ...
        if (pool.unlockBlock > 0) {
            //            unlock (withdraw all )
            (rewardGan, lockSashimi, fine) = unlockedWithdraw(pool, user);
        } else {
            //            lock
            (rewardGan, lockSashimi, fine) = lockedWithdraw(pool, user, _amount);
        }
        emit Withdraw(_pid, msg.sender, rewardGan, lockSashimi, fine);
    }
    ...
    function harvestLiquidateReward(uint256 _pid) external {
        ...
        //        burn sashimi
        IERC20(SASHIMI).safeTransfer(blackHole, userInfo[_pid][msg.sender].amount);
    ...
    function unlockedWithdraw(
        ...
        uint256 lockedSashimi = _user.amount.mul(surplusBlock).div(_pool.unlockCycle);
        uint256 rewardGan = finalQuantity.sub(lockedSashimi);
        uint256 burnSashimi = _user.amount.sub(lockedSashimi);
        _pool.totalAmount = _pool.totalAmount.sub(_user.amount);
        _pool.liquidateAmount = _pool.liquidateAmount.add(fine);
        _user.amount = 0;
        _user.rewardDebt = totalUnlock.sub(lockedSashimi);
        IERC20(SASHIMI).safeTransfer(msg.sender, lockedSashimi);
        IERC20(GAN).safeTransfer(msg.sender, rewardGan);
        //        burn.
        IERC20(SASHIMI).safeTransfer(blackHole, burnSashimi);
        return (rewardGan, lockedSashimi, fine);
    }
    ...
    function lockedWithdraw(PoolInfo storage _pool,
        ...
        if (block.number <= stakeEndBlock||!redeemGanClaimed) {
            //        stake period
    ...
    function withdrawToken(PoolInfo storage _pool,
        ...
        IERC20(SASHIMI).safeTransfer(msg.sender, finalQuantity);
    ...
    function pendingWithdraw(
        ...
        if (block.number >= startBlock) {
            if (pool.unlockBlock > 0) {
                ...
                return unlockedWithdrawView(pool,user,endBlock);
            } else {
    ...
    function unlockedWithdrawView(
        ...
        uint256 lockedSashimi = _user.amount.mul(surplusBlock).div(_pool.unlockCycle);
        uint256 rewardGan = finalQuantity.sub(lockedSashimi);
        return (fine,rewardGan,lockedSashimi);
    }
