    ...
    UUPSUpgradeable
{
    ...
    mapping(address => bool) public vaultsMap;
    ...
    function createReserve(
        ...
        emit CreateTokenReserve(reserveId, msg.sender, name, description);
    }
    ...
    function deleteReserve(bytes32 reserveId) public onlyOwner {
        Reserve storage reserve = reserveMap[reserveId];
        Vault[] storage reserveVaults = reserve.vaults;

        string storage name = reserve.name;
        string storage description = reserve.description;

        bytes memory nameBytes = bytes(name);
        require(nameBytes.length > 0, "no reserve available");

        uint256 reserveVaultsLength = reserveVaults.length;
        require(reserveVaultsLength > 0, "no vaults for reserve found");
        ...
        emit DeleteTokenReserve(reserveId, msg.sender, name, description);
    }
    ...
    function addReserveVault(
        ...
        require(!vaultsMap[vault], "vault is already a member of a reserve");
        ...
        vaultsMap[vault] = true;

        emit AddTokenReserveVault(
            reserveId,
            msg.sender,
            vault,
            calculator,
