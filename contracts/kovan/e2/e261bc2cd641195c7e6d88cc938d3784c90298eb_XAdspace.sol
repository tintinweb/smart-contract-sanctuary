...
contract XAdspace {
   ...
   mapping(uint256 => AdProgram) public adPrograms;  

   uint256 public adProgramNonce;  

   mapping(bytes32 => bool) public programNameExists;
   ...
   event BoughtAdspace(uint256 programId, address programOwner, address token, uint256 tokens, string adURL, address renter);
   event CreatedAdProgram(uint256 programId, address programOwner, address token, uint256 tokens, string adURL);
  ...
  function createAdProgram(address token, uint256 startPrice, string calldata programName, string calldata initialUrl ) public returns (uint256) {
    
    address from = msg.sender;  
 
    uint256 programId = adProgramNonce++;
    ...
    require(!programNameExists[keccak256(abi.encode(programName))], 'program of that name already exists');
    programNameExists[keccak256(abi.encode(programName))] = true;
  ...
  function buyAdspace(uint256 programId, address token, uint256 tokens, string calldata adURL) public returns (bool) {
  ...
  function setPaymentDelegate( uint256 programId, address delegate ) public returns (bool) {
  ...
  function setNewRentalsAllowed(  uint256 programId, bool allowed ) public returns (bool) {
  ...
  function setPriceForAdspace(uint256 programId, uint256 newPrice) public returns (bool) {
  ...
  function setTokenForAdspace(uint256 programId, address newToken) public returns (bool) {
  ...
  function adspaceTimeRemaining( uint256 programId ) public view returns (uint256){
  ...
  function adspaceIsDefined( uint256 programId ) public view returns (bool){
  ...
  function getRemainingAdspaceValue( uint256 programId ) public view returns (uint256){
  ...
  function getAdspaceRentalPremium( uint256 programId ) public view returns (uint256){
      if(adspaceIsDefined(programId) && adPrograms[programId].renter != address(0x0)){
