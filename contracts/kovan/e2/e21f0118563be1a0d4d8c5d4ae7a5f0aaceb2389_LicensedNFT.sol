...
import "contracts/ISharesToken.sol";
...
contract LicensedNFT is ERC721Preset {
    ...
    /* Events */
    event SharesTokensImplementationSet(address _tokenImplementation);
    event SharesDeploymentPaused();
    event SharesDeploymentResumed();
    event SharesDeployed(uint256 _tokenId, address _sharesAddress);
    ...
    function deploySubLicense(uint256 _tokenId, address _recipient) public {
        require(msg.sender == ownerOf(_tokenId), "ERR_CALLER");
        mint(_recipient);
    ...
    function revokeSubLicense(uint _subLicenseId) public {
        ...
        require(msg.sender == ownerOf(parentId), "ERR_CALLER");
        _burn(_subLicenseId);
    ...
    function deployShares(
        ...
        address newShares = ClonesUpgradeable.clone(tokenShareImplementation);
        ISharesToken(newShares).initialize(_name, _symbol, _supply, _recipient, _tokendId);
        sharesToken[_tokendId] = newShares;
        emit SharesDeployed(_tokendId, newShares);
        return newShares;
    }

    function pauseSharesDeployement() public {
        require(hasRole(ADMIN_ROLE, msg.sender), "ERR_CALLER");
        sharesTokenDeployementPaused = true;
        emit SharesDeploymentPaused();
    }

    function resumeSharesDeployement() public {
        require(hasRole(ADMIN_ROLE, msg.sender), "ERR_CALLER");
        sharesTokenDeployementPaused = false;
        emit SharesDeploymentResumed();
    }

    function setTokenImplementation(address _tokenImplementation) public {
        require(hasRole(ADMIN_ROLE, msg.sender), "ERR_CALLER");
        tokenShareImplementation = _tokenImplementation;
        emit SharesTokensImplementationSet(_tokenImplementation);
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(!frozenToken[tokenId] || msg.sender == issuer, "ERR_TOKEN_FROZEN");
    ...
    function getSharesTokenAddress(uint256 _tokenId) public view returns (address) {
        require(_exists(_tokenId), "ERR_TOKEN_ID");
