...
contract ApeStreetMaster is OwnableUpgradeable {
    mapping(bytes32 => address) public modules;
    mapping(address => bool) public activeContracts;

    // events
    event ModuleRegistered(bytes32 key, address module);

    function initialize() external {
        OwnableUpgradeable.__Ownable_init();
        modules[bytes32("ApeStreetMaster")] = address(this);
    }

    function registerModule(bytes32 key, address module) external onlyOwner {
        modules[key] = module;
        activeContracts[module] = true;
    ...
    function isModule(address addr) public view returns (bool) {
