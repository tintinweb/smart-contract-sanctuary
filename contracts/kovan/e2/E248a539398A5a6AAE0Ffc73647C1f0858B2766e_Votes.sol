...
contract Votes is Ownable, Initializable {
  ...
  address lockAsset = 0x4A3A65c742260Df1097FaAa4446f3fa4556d8bbf;
  ...
  function lockTokensVoter () private {
    IERC20(lockAsset).approve(msg.sender, minLockAmount);
  ...
  function unlockVotesTokens () private canUnlockVotesTokens(msg.sender) {
  ...
  function createNftEvent (string memory _nftUrl, uint _start, uint _end, uint _minParticipants, uint _maxParticipants, uint _rewardAmount) private canCreate(msg.sender, _rewardAmount) {
    transfer(address(this), msg.sender, _rewardAmount);
  ...
  function vote (string memory _answer, uint _eventId) private canVote(msg.sender) eventIsNotClosed(_eventId) {
  ...
  function setRewards (uint _eventId, bool _succeed, address[] memory _addresses) private onlyOwner {
  ...
  function claimRewards (uint _eventId) private canClaim(msg.sender, _eventId) {
  ...
  function collectFees () private onlyOwner feesAreNotZero() {
