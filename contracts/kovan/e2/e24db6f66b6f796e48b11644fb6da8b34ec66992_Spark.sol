...
/*
TO DO:
0.7.0: add "weekly batch claim" feature [done]
    1. add or modify data structures [done]
    2. add new functions and split "claim" into itself and its wrapper [done]
    3. update "claim", "claimBatch", "claimAndWithdraw", "claimAndWithdrawBatch" functions [done]
0.8.0:
    1. debug based on test 0.5.1 [done]
    2. add "extension" feature [done]
0.8.1:
    1. change all private var to public [done]
    2. compile-time debug [done]
    3. getter function clean up [done]
    4. deploy
0.9.0:
    1. fix comments
...
contract Spark is Ownable {
    ...
    event Staked(address user, uint ID, uint amount, uint stakedTimestamp, uint totalStakedLP);
    ...
    uint public extDays;
    uint public extAPR;
    bool public extSwitch; //once extended, extSwitch turns to true
    ...
    function extend(uint _extDays, uint _extAPR) external onlyOwner {
        ...
        require(extSwitch == false, 'extend: already extended');
        extDays = _extDays;
        extAPR = _extAPR;
        extSwitch = true;
    }
    ...
    function stake(uint amount) external returns (uint id) {
        ...
        emit Staked(msg.sender, id, amount, now, _userProfiles[msg.sender].numLP);
    }
    ...
    function _claim(address staker, uint index) private returns (uint) {
        ...
        _userProfiles[staker].cumReward = _userProfiles[staker].cumReward.add(claimableAmt);
        // 2. update global stats
     ...
     */
    function claimBatch(uint[] memory indices) public returns (uint totalClaimed) {
    ...
    function claimAndWithdraw(uint index) public returns (uint claimAmt, uint withdrawAmt) {
        ...
        // 2. withdraw
        require(_userDeposits[msg.sender][index].stakedLP > 0, "claimAndWithdraw: the deposit does not exist.");
        withdrawAmt = _userDeposits[msg.sender][index].stakedLP;
        // update global stats
        totalLP = totalLP.sub(withdrawAmt);
        // update user stats
        uint userLP = _userProfiles[msg.sender].numLP;
        _userProfiles[msg.sender].numLP = userLP.sub(withdrawAmt);
        uint numDepo = _userProfiles[msg.sender].numDeposit;
        _userProfiles[msg.sender].numDeposit = numDepo - 1;
    ...
    /**
     * @notice claim reward and withdraw LP tokens from mutiple deposits by one batch
     * @param indices the array of indices of the deposits from which msg.sender claims rewards and withdraw LP tokens
     * @return claimTotal the total amount of reward claimed, withdrawTotal the total amount of LP tokens withdrawn
     */
    function claimAndWithdrawBatch(uint[] memory indices) external returns (uint claimTotal, uint withdrawTotal) {
      uint claimAmt;
      uint withdrawAmt;
      for (uint i = 0; i < indices.length; i++) {
          (claimAmt, withdrawAmt) = claimAndWithdraw(indices[i]);
          claimTotal += claimAmt;
    ...
    /**
     * @notice place multiple free-claim requests in a single batch
     * @dev each request needs to be valid for the batch to proceed
     * @param indices the array of indices of the deposits to place free-claim requests on
     */
    function requestFreeClaimBatch(uint[] memory indices) external {
        for (uint i = 0; i < indices.length; i++) {
            requestFreeClaim(indices[i]);
        }
    }
    ...
    function computeUnitRewardFor(address staker, uint index) public view returns (uint) {
        ...
        uint amount = _userDeposits[staker][index].stakedLP;
        require(amount > 0, "computeUnitRewardFor: LP should be larger than zero.");
        ...
        (uint112 reserve0, uint112 reserve1, ) = _pair.getReserves();
         uint kLast = uint(reserve0).mul(reserve1);
         uint principal = computeLPValue(amount, _pair.totalSupply(), ma7Token0Price, kLast);
    ...
    function computeInterestWithPrecision(uint t) public view returns (uint) {
        if (t == 0) {
            ...
            return (300 * (t - 7)+ 200 * 7).mul(PERCENTAGE_PRECISION).div(365);
        } else if (t <= 21) {
            return (400 * (t - 14)+ (200 + 300) * 7).mul(PERCENTAGE_PRECISION).div(365);
        } else if (t <= 28) {
            return (500 * (t - 21)+ (200 + 300 + 400) * 7).mul(PERCENTAGE_PRECISION).div(365);
        } else if (t <= (28 + extDays)) {
            return (extAPR * (t - 28) + (200 + 300 + 400 + 500) * 7).mul(PERCENTAGE_PRECISION).div(365);
        } else {
            return (extAPR * extDays  + (200 + 300 + 400 + 500) * 7).mul(PERCENTAGE_PRECISION).div(365);
        }
    }
    ...
    function computeLPValue(uint amountInLP, uint poolLP, uint priceRaw, uint k) public pure returns (uint) {
        ...
        (uint sqrtRaw, uint powerPrecision) = Math.power(baseN, 1, 1, 2);
        uint sqrt = sqrtRaw >> powerPrecision;

        uint rawValue = amountInLP.mul(sqrt).div(poolLP);
