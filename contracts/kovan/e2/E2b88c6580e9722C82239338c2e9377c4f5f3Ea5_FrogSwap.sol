...
contract FrogSwap is ERC20, EIP712 {
    uint256 public constant MAX_SUPPLY = uint248(1e11 ether);
    ...
    uint256 public constant AMOUNT_MM = MAX_SUPPLY / 100 * 15;
    address public constant ADDR_MM = 0x11eDedebF63bef0ea2d2D071bdF88F71543ec6fB;

    // for airdrop
    uint256 public constant AMOUNT_AIREDROP = MAX_SUPPLY - (AMOUNT_DAO + AMOUNT_STAKING + AMOUNT_MM);

    constructor(string memory _name, string memory _symbol, address _signer) ERC20(_name, _symbol) EIP712("FrogSwap", "1") {
        _mint(ADDR_DAO, AMOUNT_DAO);
        _mint(ADDR_STAKING, AMOUNT_STAKING);
        _mint(ADDR_MM, AMOUNT_MM);
        _totalSupply = AMOUNT_DAO + AMOUNT_STAKING + AMOUNT_MM;
        cSigner = _signer;
    }
    ...
    function claim(uint256 amount, bytes memory sig) external {
        ...
        require(total <= MAX_SUPPLY, "FrogSwap: Exceed max supply");
        require(minted(msg.sender) == 0, "FrogSwap: Claimed");
        bytes32 digest = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", 
            ECDSA.toTypedDataHash(_domainSeparatorV4(),
                keccak256(abi.encode(MINT_CALL_HASH_TYPE, msg.sender, amount))
        )));
        require(ECDSA.recover(digest, sig) == cSigner, "FrogSwap: Invalid signer");
        _totalSupply = total;
