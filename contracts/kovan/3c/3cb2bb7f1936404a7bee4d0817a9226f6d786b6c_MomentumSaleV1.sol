...
contract MomentumSaleV1 {
   ...
   mapping(uint8 => uint256) public bonus;
   ...
   function createSaleContract(uint256 _allocated, uint8 _source) public onlyGovernor returns(bool) {
       ...
       i.end = Math.add(block.timestamp,10 minutes);
   ...
   function purchaseWithEth() public payable returns(bool){
   ...
   function fetchEthPrice() public view returns (int) {
    ...
    function claim(uint256 _saleId) public returns(bool){
    ...
    function resolveBonus(uint256 _saleId,address _user) public view returns(uint256){
        ...
        uint256 _bonusPercent = resolveBonusPercent(_saleId);
    ...
    function updateNewEdgexSource(address _newSource, uint8 index) public onlyAdmin returns(bool){
    ...
    function updateEthSource(address _newSource) public onlyAdmin returns(bool){
        ethPriceSource = _newSource;
        return true;
    }
    
     function updateEdgexTokenContract(address _newSource) public onlyAdmin returns(bool){
    ...
    function updateBonusPercent(uint8 _level, uint256 _percent) public onlyAdmin returns(bool){
        bonus[_level] = _percent;
        return true;
    }
