...
contract Oracle is IOracle, Ownable
{
  ...
  function _uintify (int256 val) private pure returns (uint256)
  {
  ...
  function _calcTokenETHRate (address token) private view returns (uint256)
  {
    ...
    {
      return ( _uintify(IFeed(_USDFeed[token]).latestAnswer()) * 1e18 ) / _uintify(IFeed(_USDFeed[_WETH]).latestAnswer());
    }
    else
    {
      return 0;
  ...
  function getRate (address from, address to) public view override returns (uint256)
  {
    ...
    uint256 srcRate = from == _WETH ? 1e18 : _calcTokenETHRate(from);
    uint256 destRate = to == _WETH ? 1e18 : _calcTokenETHRate(to);

    require(srcRate > 0 && destRate > 0, "no oracle");


    return (srcRate * 1e18) / destRate;
  }
  ...
  function _calcDestQty (uint256 srcQty, address from, address to, uint256 rate) private view returns (uint256)
  {
    ...
    {
      // difference = 10 ** (destDecimals - srcDecimals);


      return (srcQty * rate * ( 10 ** (destDecimals - srcDecimals) )) / 1e18;
    }
    else
    {
      // difference = 10 ** (srcDecimals - destDecimals);


      return (srcQty * rate) / (1e18 * (10 ** (srcDecimals - destDecimals)) );
    }
  }
