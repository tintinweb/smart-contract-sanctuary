...
contract Vault 
{
    function deposit() public payable {
        address RECEIPT_TOKEN_ADDRESS = address(0xb5D30C2b09B239480202C0C35FFfc1295bC4a6a8);
        ...
        uint256 receiptTokensOwed = (oldTokenSupply * depositValue) / oldVaultValue; //ensure arithmetic is correct and that this holds up in testing to be the right amount
    ...
    function withdraw(address payable customer, uint256 tokenValue, address RECEIPT_TOKEN_ADDRESS) private {
        ...
        if (IE25Token(RECEIPT_TOKEN_ADDRESS).burnFrom(customer, tokenValue)) {
    ...
    function receiveApproval(address _from, uint256 _value, address _token, bytes calldata /*_extraData*/) external {
        withdraw(payable(_from), _value, _token);
    }
    
    function mint(address customer, uint256 value, address RECEIPT_TOKEN_ADDRESS) private {
        IE25Token(RECEIPT_TOKEN_ADDRESS).mint(customer, value); //consider adding events and figuring out how they work. Everything important needs to be broadcasted
    }
    ...
    function calculateValueOfVault() private view returns (uint256 currentValueOfVault) {
        ...
        uint256 YFI_IN_VAULT = 1;
        uint256 MKR_IN_VAULT = 1;
        uint256 LINK_IN_VAULT = 1;
        uint256 UNI_IN_VAULT = 1;
    ...
    function getReceiptTokenTotalSupply(address RECEIPT_TOKEN_ADDRESS) private view returns (uint256 totalSupply) {
        return IE25Token(RECEIPT_TOKEN_ADDRESS).totalSupply();
    }
}