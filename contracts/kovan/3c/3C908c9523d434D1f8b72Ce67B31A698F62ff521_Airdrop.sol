...
contract Airdrop is Ownable, Pausable, ReentrancyGuard {
    ...
    /// @dev address of Mioochat token holder from which transfer to recipient when claiming tokens
     ...
     * @param _token Mioochat token address
     */
    constructor(
        ...
        require(_reserveAddress != address(0), "MioochatAirdrop: invalid reserve address");
        require(_claimSigner != address(0), "MioochatAirdrop: invalid claim signer address");
        require(_token != address(0), "MioochatAirdrop: invalid token address");
    ...
    function claimTokens(
        ...
        require(_recipient != address(0), "MioochatAirdrop: invalid recipient address");
        require(_amount > 0, "MioochatAirdrop: invalid amount");
        require(!claimed[_recipient], "MioochatAirdrop: token already claimed to recipient");

        claimed[_recipient] = true;

        bytes32 message = keccak256(abi.encodePacked(_recipient, _amount));

        require(Lib.recoverSigner(message, _signature) == claimSigner, "MioochatAirdrop: invalid signature");
...
/**
 * @dev Library for recovering signer from signed message
 */

library Lib {
    /**
     * @dev Recovers signer from signed messssage
     * @param _hash hash
     * @param _signature signed message
     * @return user
     */
    ...
    /**
     * @dev Splits signature into r, s, v from signed message
     * @param _signature signed message
     * @return r
     * @return s
     * @return v
