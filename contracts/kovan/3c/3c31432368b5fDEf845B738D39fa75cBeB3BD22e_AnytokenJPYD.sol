...
contract AnytokenJPYD is Ownable, EIP712MetaTransaction("AnytokenJPYD", "1")  {
  address public jpyc;
  address public jpyd;
  ...
  event Payment(address indexed from, address indexed to, address indexed from_token, address to_token, uint from_amount, uint to_amount, string ref);
  
  constructor(address _jpyc, address _jpyd, address _receiver) {
    jpyc = _jpyc;
    jpyd = _jpyd;
    receiver = _receiver;
  }
  ...
  function _buy(address to, uint amount, uint from_amount, address _token, string memory ref) internal {
    IERC20(jpyc).transfer(receiver, amount);
    IERC20(jpyd).mint(to, amount);
    emit Payment(msgSender(), to, _token, jpyc, from_amount, amount, ref);
  }

  function _checkParams(address[] memory _tokens, address swap) internal view {
    require(exchanges[swap] == true, "exchange not allowed");
    require(_tokens[_tokens.length - 1] == jpyc, "last token must be JPYC");
  }
  ...
  function buy(address to, uint amount, string memory ref) public {
    IERC20(jpyc).transferFrom(msgSender(), receiver, amount);
    IERC20(jpyd).mint(msgSender(), amount);
    emit Payment(msgSender(), to, jpyc, jpyc, amount, amount, ref);
  }
  ...
  function swapAndPayExactIn(address to, address[] memory _tokens, uint amount, string memory ref, uint min, uint deadline, address swap) public {
    ...
    _buy(to,_amounts[_amounts.length - 1], _amounts[0], _tokens[0], ref);
  }
