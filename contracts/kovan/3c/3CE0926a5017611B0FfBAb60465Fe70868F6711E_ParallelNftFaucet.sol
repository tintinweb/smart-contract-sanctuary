...
//        return (depositInfo[_userAddress].totalNftsReceived);
    ...
    function requestNft(bytes memory _ticket, uint16 _nftId) public {
//        require(depositInfo[msg.sender].totalNftsReceived < maxNftsPerUser, "max bonus received");
        ...
        require(verify(trustedSigner, msg.sender, _nftId, 1, ticketPhrase, _ticket));
...
//        depositInfo[msg.sender].totalNftsReceived = depositInfo[msg.sender].totalNftsReceived + 1;
    ...
    function getMessageHash(
        address _to, uint16 _nftId, uint16 _amount, string memory _message
    )
    public pure returns (bytes32)
    {
        return keccak256(abi.encodePacked(_to, _nftId, _amount, _message));
    }
    ...
    {
        bytes32 messageHash = getMessageHash(_to, _nftId, _amount, _message);
