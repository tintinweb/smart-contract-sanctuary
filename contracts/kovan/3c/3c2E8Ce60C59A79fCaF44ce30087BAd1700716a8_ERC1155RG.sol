  ...
  function create(
    ...
    */
    function totalSupply(
		 ...
		 */
		function getAllTokensOwned(address senderAddr) public view returns(uint256[] memory result){
			uint currIndx=0;
			for(uint count=1;count<_currentTokenID;count++){
				if(balanceOf(senderAddr,count)!=0){
...
abstract contract Owned {
    ...
    function _notOnlyOwner() private view {
        require(msg.sender != owner, "Only the contract owner cannot perform this action");
    }
