...
contract CryptoApe is Context, IERC20, Ownable {
    ...
    /// @dev Store if an address is excluded from tax
    ...
    constructor () {
        ...
        _reflectionFee = 3;
        _burnFee = 1;
        _feeTotal = _burnFee + _fundingWalletFee + _liquidityFee;
        _minimumToDistribute = 100e6 * 1e2;
    ...
    function totalFees() public view returns (uint256) {
        uint256 tFees = _feeTotal + _reflectionFee;
        return tFees;
    }
    ...
    function excludeAccountFromInTansfer(address account) external onlyOwner() {
    ...
    function includeAccountFromInTransfer(address account) external onlyOwner() {
        require(_isExcluded[account], "Account is already excluded");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(!_isBlacklisted[sender] && !_isBlacklisted[recipient], "Cannot transfer from or to a blacklisted address");

        /// @dev Require that trading is open, so nobody can add LP before owner does
        require(_tradingOpen || _isExcluded[sender], "Trading is not open");

        /// @dev See line 888 - 889
        uint256 beforeSellLiquidityFee = _liquidityFee;
        uint256 beforeExcludedReflectionFee = _reflectionFee;
        ...
        /// @dev If he is excluded from tax make reflection fee 0
        if (_isExcludedFromTax[sender]) {
            delete _reflectionFee;
        }

        /// @dev If its a distribution or trading isn't open or he is excluded
        /// from tax, don't take fees
        if (!_inDistribution && _tradingOpen && !_isExcludedFromTax[sender]){
        ...
        /// @dev And if he was excluded, make the reflection fee go back to normal
        _liquidityFee = beforeSellLiquidityFee;
        _reflectionFee = beforeExcludedReflectionFee;
    }
     ...
     */
    function distribute(address from, uint256 amount) private returns (uint256 _totalFees) {
        ...
        /// @dev Get fee amounts
        uint256 toBurn = (feesInContract*(_burnFee))/(_feeTotal);
        uint256 _toLP = (feesInContract*(_liquidityFee))/(_feeTotal);
        uint256 toFundingWallet = (feesInContract*(_fundingWalletFee))/(_feeTotal);

        /// @dev Swap to ETH and add liquidity
        uint256 split = _toLP/(2);
        swapToETH(split);
        _uniswapV2Router.addLiquidityETH{value: address(this).balance}(address(this), split, 0, 0, BURN, block.timestamp);

        /// @dev Swap to ETH and send to funding wallet
        uint256 fundingWalletETH = swapToETH(toFundingWallet);
        (bool success,) = _fundingWallet.call{value: fundingWalletETH}(new bytes(0));
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256) {
        ...
        uint256 total = tAmount + (tAmount * (_liquidityFee + _burnFee + _fundingWalletFee)/(90));
    ...
    function checkblacklist() public view returns(address[] memory){
        address[] memory ret = new address[](_list.length);
     ...
     */
    function changeValues(uint256 newBurnFee, uint256 newReflectionFee, uint256 newLiquidityFee, uint256 newFundingWalletFee, uint256 newMinimumToDistribute) external onlyOwner {
        _burnFee = newBurnFee;
   ...
   /* @dev Function to freeze an address
     * @param _address: The address to freeze/unfreeze
     */
    ...
    function fees() external view returns (uint256 burnFee, uint256 reflectionFee, uint256 liquidityFee, uint256 fundingWalletFee)  {
        
        return (_burnFee, _reflectionFee, _liquidityFee, _fundingWalletFee);
    }

    /** 
     * @return Minimum amount of tokens in the contract to swap and send to funding wallet in ETH
     */
    function minimumThresholdToDistribute() external view returns (uint256) {
    ...
    /**
     * @dev Excludes or includes someone in tax, only by the owner
     * @param _address: Who to exclude from tax
     * @param setTo: To include or exclude
     */
    function excludeFromTax(address _address, bool setTo) external onlyOwner {
        _isExcludedFromTax[_address] = setTo;
    }
