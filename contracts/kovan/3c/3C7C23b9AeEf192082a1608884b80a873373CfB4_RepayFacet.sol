...
library RepayLib {
    ...
    function claimEscrowTokensByValue(
        ...
        valueLeftToTransfer = claimEscrowToken(loanID, LibLoans.loan(loanID).lendingToken, recipient, valueLeftToTransfer);
        ...
        for (uint256 i = 0; i < EnumerableSet.length(tokens); i++) {
            ...
            valueLeftToTransfer = claimEscrowToken(loanID, EnumerableSet.at(tokens, i), recipient, valueLeftToTransfer);
        }
    }

    function claimEscrowToken(uint256 loanID, address token, address recipient, uint256 valueLeftToTransfer) private returns (uint256) {
