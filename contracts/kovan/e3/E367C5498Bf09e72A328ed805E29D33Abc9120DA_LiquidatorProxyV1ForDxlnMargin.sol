...
/**
 * Contract for liquidating other accounts in DxlnMargin. Does not take marginPremium into account.
 */
contract LiquidatorProxyV1ForDxlnMargin is OnlyDxln, ReentrancyGuard {
    ...
    // bytes32 constant FILELIQ = "LiquidatorProxyV1ForDxlnMargin";
    ...
    constructor(address dxlnMargin) OnlyDxln(dxlnMargin) {} /* solium-disable-line no-empty-blocks */
    ...
    function liquidate(
        ...
        for (
            uint256 owedIndex = 0;
            owedIndex < owedPreferences.length;
            owedIndex++
        ) {
            ...
            for (
                uint256 heldIndex = 0;
                heldIndex < heldPreferences.length;
                heldIndex++
            ) {
                ...
                if (
                    !DXLN_MARGIN
                        .getAccountPar(liquidAccount, owedMarket)
                        .isNegative()
                ) {
                    break;
                }
                ...
                if (
                    !DXLN_MARGIN
                        .getAccountPar(liquidAccount, heldMarket)
                        .isPositive()
                ) {
                    continue;
                }
                ...
                CacheLiq memory cache = initializeCache(
                    constants,
                    heldMarket,
                    owedMarket
                );
                ...
                totalValueLiquidated = totalValueLiquidated.add(
                    cache.toLiquidate.mul(cache.owedPrice)
                );
            }
        }
     ...
     */
    function calculateSafeLiquidationAmount(CacheLiq memory cache)
        private
        pure
    {
        ...
        if (owedGoesToZeroLast) {
            ...
            Types.Wei memory heldWeiDelta = Types.Wei({
                ...
                value: cache.owedWei.value.getPartial(
                    cache.owedPriceAdj,
                    cache.heldPrice
                )
            });
            ...
            Types.Wei memory owedWeiDelta = Types.Wei({
                ...
                value: cache.heldWei.value.getPartial(
                    cache.heldPrice,
                    cache.owedPriceAdj
                )
            });
    ...
    function calculateMaxLiquidationAmount(
        ...
        uint256 requiredOverhead = Decimal.mul(
            cache.borrowValue,
            constants.minLiquidatorRatio
        );
        uint256 requiredSupplyValue = cache.borrowValue.add(requiredOverhead);
        uint256 remainingValueBuffer = cache.supplyValue.sub(
            requiredSupplyValue
        );
        ...
        uint256 owedValueToTakeOn = Decimal.div(
            remainingValueBuffer,
            spreadMarginDiff
        );
    ...
    function checkRequirements(Constants memory constants) private view {
        ...
        require(
            constants.fromAccount.owner == msg.sender ||
                DXLN_MARGIN.getIsLocalOperator(
        ...
        require(liquidSupplyValue.value != 0, "Liquid account no supply");
        require(
            DXLN_MARGIN.getAccountStatus(constants.liquidAccount) ==
                Account.Status.Liquid ||
                !isCollateralized(
                    liquidSupplyValue.value,
                    liquidBorrowValue.value,
                    DXLN_MARGIN.getMarginRatio()
                ),
    ...
    function getCurrentAccountValues(
        ...
        for (uint256 m = 0; m < constants.markets.length; m++) {
            Types.Par memory par = DXLN_MARGIN.getAccountPar(account, m);
            if (par.isZero()) {
                continue;
            }
            Types.Wei memory userWei = Interest.parToWei(
                par,
                constants.markets[m].index
            );
            uint256 assetValue = userWei.value.mul(
                constants.markets[m].price.value
            );
            if (userWei.sign) {
    ...
    function getMarketsInfo() private view returns (MarketInfo[] memory) {
        uint256 numMarkets = DXLN_MARGIN.getNumMarkets();
        MarketInfo[] memory markets = new MarketInfo[](numMarkets);
        for (uint256 m = 0; m < numMarkets; m++) {
            markets[m] = MarketInfo({
                price: DXLN_MARGIN.getMarketPrice(m),
                index: DXLN_MARGIN.getMarketCurrentIndex(m)
            });
        }
        return markets;
    }
    ...
    function initializeCache(
        ...
        uint256 heldMarket,
        uint256 owedMarket
    ) private view returns (CacheLiq memory) {
        (
            Monetary.Value memory supplyValue,
            Monetary.Value memory borrowValue
    ...
    function constructActionsArray(CacheLiq memory cache)
        private
        pure
