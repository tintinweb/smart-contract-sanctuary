...
contract PolicyRegistry {
    ...
    event TestUpdate(uint indexed _subcourtID, string _policy);
     ...
     *  @param _policy The URI of the policy JSON.
     */
    function setPolicy(uint _subcourtID, string memory _policy) external onlyByGovernor {
        policies[_subcourtID] = _policy;
        emit PolicyUpdate(_subcourtID, policies[_subcourtID]);
    }

    function TestPolicy1(uint _subcourtID, string memory _policy) public onlyByGovernor {
        policies[_subcourtID] = _policy;
        emit TestUpdate(_subcourtID, policies[_subcourtID]);
    }
    function TestPolicy2(uint _subcourtID, string memory _policy) public onlyByGovernor {
        policies[_subcourtID] = _policy;
    }
}