...
contract ERC20Upgradeable is Initializable, ContextUpgradeable, IERC20Upgradeable {
    ...
    function EligibleSupply() public view virtual returns (uint256){
        uint256 ExcludedSupply = _totalSupply - ExchangeSupply - balanceOf(DayPot) - balanceOf(DevWallet) - balanceOf(MarketingWallet); 
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (sender == DayPot || sender == DevWallet || sender == MarketingWallet || recipient == DevWallet || recipient == DayPot || recipient == MarketingWallet) {
            feeAmount = 0;
        }        
    ...
    function AddGroupHolder(address holder) internal {
        if(!_isHolder[holder] && holder != address(0) && holder != DayPot && holder != DevWallet && holder != MarketingWallet && !holder.isContract()){
...
contract LLTest is Initializable, ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable, VRFConsumerBase  {
    ...
    function initialize() public virtual initializer {
        ...
        WinningWalletMojo = DayPot;
        WinningWalletLuck = DayPot;
    ...
    function ShowGroupBalanceList() public view returns(uint256[] memory){
    ...
    function RunMojoLottery() public onlyOwner {
        ...
        _mint(WinningWalletLuck, RollingGroupTotal/100);
    ...
    function PayMojoWinner() public onlyOwner {
        uint256 daypotBalance = balanceOf(DayPot);
        _burn(DayPot, daypotBalance);
        _mint(WinningWalletMojo, daypotBalance);
    }
