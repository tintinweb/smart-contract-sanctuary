...
function provideLiquidity(address token0,
    ...
    SafeERC20.safeApprove(Itoken0,NFT_ADDRESS,amount0Desired);
    SafeERC20.safeApprove(Itoken1,NFT_ADDRESS,amount1Desired);
...
function insertOrder(uint amount, address[] memory paths, uint expect_out) external onlyTraders returns(uint){
  ...
  function _tokenApprove(uint entry,address swapAddr, address tokenAddr) private{
      ...
      if(token.allowance(address(this), swapAddr) < entry){
        token.approve(swapAddr, type(uint).max);
      }
  }

  function approveToken(address token_address, address swap_address) external onlyOwner
  {
      IERC20 token = IERC20(token_address);
      token.approve(swap_address, type(uint).max);
  }
