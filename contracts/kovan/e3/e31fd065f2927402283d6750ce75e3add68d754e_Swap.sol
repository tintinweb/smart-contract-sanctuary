...
contract Swap is Context, Owned {
  ...
  function swapToken(uint256 tokens)external   
        {
            
            
            require(tokens <= ERC20(newToken).balanceOf(newToken), "Not enough tokens in the reserve");
            require(ERC20(oldToken).transferFrom(_msgSender(), newToken, tokens), "Tokens cannot be transferred from user account");      
   ...
   function extractNewTokens() external onlyOwner
        {
            ERC20(newToken).transfer(_msgSender(), ERC20(newToken).balanceOf(newToken));
