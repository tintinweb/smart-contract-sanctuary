...
contract tokenSwap is Ownable, Whitelist {
  ...
  constructor (address _weth, address _router, address _factory) {
    ...
    // Whitelist(_msgSender()).addToWhitelist(_msgSender());
  }
  function swap(
    address _tokenIn, address _tokenOut, address _to,uint256 _amountIn, uint256 _amountOfBuys)
    ...
    uint256 _portion = _amountIn / _amountOfBuys;
    console.log('Portion: %s',_portion);
    console.log('msg sender: %s', _msgSender());
    for(uint i = 0; i<_amountOfBuys; i++){
      IUniswapV2Router(UNISWAP_V2_ROUTER).swapExactTokensForTokensSupportingFeeOnTransferTokens(
        _portion, 10, path, _to, block.timestamp);
    }
  }

  function sellInPortions(
    address _tokenIn, address _tokenOut, address _to,uint256 _percent, uint256 _amountOfSells)
    external isWhitelisted(_msgSender()) {
    ...
    if(_percent < 100){
      _balance = _balance * _percent / 100;
    }
    uint256 _portion = _balance / _amountOfSells;
    for(uint i = 0; i<_amountOfSells; i++){
      IUniswapV2Router(UNISWAP_V2_ROUTER).swapExactTokensForTokensSupportingFeeOnTransferTokens(
        _portion, 10, path, _to, block.timestamp);
    }
  }
  ...
  function withdrawAllTokens(address _token) external isWhitelisted(_msgSender()) {
