...
Transaction fees: 	2%
	Redistribution:	1%
	Burn:			1%
Max transaction: 0.2% of total supply

--------------------

Created by 0f0crypto
Adapted by DJustice
...
import "./justicetoken-imports.sol";
abstract contract Tokenomics {
    
    using JusticeLib for uint256;
    
    // --------------------- Token Settings ------------------- //

    string internal constant NAME = "Justice Token";
    string internal constant SYMBOL = "JUSTICE";
     ...
     */
    uint256 public maxTransactionAmount = TOTAL_SUPPLY / 500; // 0.2% of the total supply
     ...
     * See: `Liquifier`
     */
    uint256 internal constant numberOfTokensToSwapToLiquidity = TOTAL_SUPPLY / 5000; // 0.02% of the total supply
    ...
    address public burnAddress = 0x0000000000000000000000000000000000000000;
      ...
      // Project wallets 
    address public Gameplay = 0x817B96EdCbBb448e9905c9741173Fd01498D32c8;
    address public Staking = 0x335D15Ba7b624f30D7065541c771dEBC882f24E1;
    address public Development = 0xe70Ed740A7d505327E4c4d7929d3dBA46383735d; 
    ...
    // sumOfFees is divided by FEES_DIVISOR (1000), so a fee of 5% would be 50. This if fine internally, but 
    // To avoid misunderstandings, we'll divide it by 10 (so a 5% fee will show as 5) to make the fees public
    uint256 public _sumOfFees = sumOfFees / 10;
...
abstract contract BaseRfiToken is IERC20, IERC20Metadata, Ownable, Presaleable, Tokenomics {
  ...
  function setmaxTransactionAmount(uint256 TransactionLimit) external onlyOwner() {
    ...
    function _isUnlimitedRecipient(address account) internal view returns(bool){
        // anyone should be able to burn as many tokens as he/she wants
        // locked project wallets have to be whitelisted here so its tokens won't be stuck forever in the locking contract
        return (account == owner() || account == burnAddress || account == Gameplay || account == Development || account == Staking);
    }
...
abstract contract JusticeToken is BaseRfiToken, Liquifier, Antiwhale {
...
contract Justice is JusticeToken{

    constructor() JusticeToken(Env.Testnet){
