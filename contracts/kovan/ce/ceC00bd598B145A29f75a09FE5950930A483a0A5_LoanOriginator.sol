...
contract LoanOriginator is ILoanOriginator {
    ...
    modifier onlyApprovedSPVWallet {
        require(msg.sender == IManagementCompany(MCBoard).SPVWalletAddress(), "Only SPV wallet");
         _;
    }
    ...
    function createLoanPool(uint rate1, uint rate2, address currency) public override onlyAdmins {
        require(currency != address(0), 'LoanOriginator: INVALID_CURRENCY_ZERO_ADDRESSES');
        (uint _minRate, uint _maxRate) = rate1 < rate2 ? (rate1, rate2) : (rate2, rate1);
        require(getLoanPoolIDByParams[_minRate][_maxRate][currency] == 0, 'LoanOriginator: LOAN_POOL_EXISTS');
        ...
        ILoanPool(loanPool).initialize(_minRate, _maxRate, currency, LRSCAddress, nextLoanPoolID);
    ...
    function closeLoanPool(uint loanPoolID)  external override onlyAdmins {
        ...
        require(_pool != address(0), "LoanOriginator: LOADNPOOL ID INVALID");
    ...
    // drawFund and payLoan function desgined for SPV wallet
    function payLoan(uint amount, uint loanPoolID, uint loanEntity) external onlyApprovedSPVWallet override {
        address _pool = getLoanPoolByID[loanPoolID];
        require(_pool != address(0), "LoanOriginator: LOADNPOOL ID INVALID");
        // spv pay loan pool 
        ILoanPool(_pool).payLoan(amount, loanEntity, msg.sender);
    }

    // one land could draw multiple funds
    function drawFund(uint amount, uint loanPoolID, uint landID, uint developerID) external onlyApprovedSPVWallet override {
        address _pool = getLoanPoolByID[loanPoolID];
        require(_pool != address(0), "LoanOriginator: LOADNPOOL ID INVALID");
        // update amount borrowed by developer    
        ILandRegistration(LRSCAddress).updateAmountBorrowedByDeveloper(landID, amount);
        // spv draw fund from exisitng pool
        uint nextLoanEntityID = ILoanPool(_pool).drawFund(landID, developerID, amount, msg.sender);
    }
    ...
    function landDebtVoid(uint payableDebtAmount, uint landID) external onlyAdmins override {
        require(payableDebtAmount > 0, "LoanOriginator: PLEASE INPUT CORRECT AMOUNT");
        require(
            ILandRegistration(LRSCAddress).isLandIDValid(landID) == true, 
            "LoanOriginator: LAND ID MUST BE VALID"
        );
        require(
            ILandRegistration(LRSCAddress).getLandAppraisalAmount(landID) == 0,
            "LoanOriginator: LAND APPRAISAL VALUE MUST BE CLEARED"
        ...
        for (uint i = 1; i < relatedLoanPoolIDs.length; i++){
    ...
    function debtVoid(uint payableDebtAmount, uint loanPoolID, uint loanEntity) public onlyAdmins override {
        ...
        ILoanPool(_pool).debtVoid(loanEntity, payableDebtAmount, msg.sender);
    }
}
