...
contract TrustMinimizedBond is
  ...
  AAVECreditDelegator
  {
    ...
    constructor(address _principalTokenAddress,address _providerAddress, address _protocolDataProvider)
...
abstract contract AAVECreditDelegator {
    ...
    event ApprovedDelegation(address indexed asset, address indexed delegator, uint256 amount, uint256 interestRateMode);
    ...
    function approveDelegation(uint256 amount, uint256 interestRateMode, address assetToBorrow)
      correctInterestRateMode(interestRateMode)
      external {
  
      (, address stableDebtTokenAddress, address variableDebtTokenAddress) = protocolDataProvider.getReserveTokensAddresses(assetToBorrow);
      ...
      emit ApprovedDelegation(assetToBorrow, msg.sender, amount, interestRateMode);
    }
