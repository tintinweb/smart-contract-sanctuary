...
contract Fauceth {
    ...
    function control() internal returns (bool) {
        require((msg.sender == tx.origin) || contractors[msg.sender] == true, "control: EOA or contractor required.");
        require((blocks[msg.sender] < (block.number + 1)) || contractors[msg.sender] == true, "control: Block alredy used. Try next block.");
        return true;
    }
}