...
contract ThorBNBTokenWrapper {
	using SafeMath for uint256;
	IERC20 public thorbnblp;

	constructor(IERC20 _thorbnblpAddress) public {
		thorbnblp = IERC20(_thorbnblpAddress);
	}
	...
	function stake(uint256 amount) public {
		...
		thorbnblp.transferFrom(msg.sender, address(this), amount);
	}

	function withdraw(uint256 amount) public {
		_totalSupply = _totalSupply.sub(amount);
		_balances[msg.sender] = _balances[msg.sender].sub(amount);
		thorbnblp.transfer(msg.sender, amount);
	}
}

contract GenesisPool is ThorBNBTokenWrapper, Ownable {
	ERC1155Tradable public agc;
	...
	constructor(ERC1155Tradable _agcAddress, IERC20 _thorbnblpAddress) public ThorBNBTokenWrapper(_thorbnblpAddress) {
		agc = _agcAddress;
	}
	...
	// stake visibility is public as overriding ThorBNBTokenWrapper's stake() function
	function stake(uint256 amount) public updateReward(msg.sender) {
		require(amount.add(balanceOf(msg.sender)) <= 1000 ether, "Cannot stake more than 1000 THORBNBLP");
	...
	function redeem(uint256 card) public updateReward(msg.sender) {
		...
		require(agc.totalSupply(card) < agc.maxSupply(card), "Max cards minted");

		points[msg.sender] = points[msg.sender].sub(cards[card]);
		agc.mint(msg.sender, card, 1, "");
