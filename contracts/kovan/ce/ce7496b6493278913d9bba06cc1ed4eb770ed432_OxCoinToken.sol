...
contract OxCoinToken is IERC20, Feeable {
    ...
    function burn(address tokenOwner, uint256 amount) public returns (uint256) {
       require(!(msg.sender != getOwner() && tokenOwner != msg.sender), "ERC20: You're not the owner of the address");
    ...
    function airDrops(address[] memory recipients, uint256 amount) public {
        require(balances[msg.sender] >= amount.mul(recipients.length), "ERC20: airdrops transfer sender amount exceeds balance");
