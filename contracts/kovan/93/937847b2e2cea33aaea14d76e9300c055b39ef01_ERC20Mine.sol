...
contract BaseMine is InitializableOwnable {
    ...
    function getIdByRewardToken(address rewardToken)
        public
        view
        returns (uint256)
    {
    ...
    function setEndBlock(uint256 i, uint256 newEndBlock) external onlyOwner {
        require(i < rewardTokenInfos.length, "DODOMineV2: REWARD_ID_NOT_FOUND");
    ...
    function _getUnrewardBlockNum(uint256 i) internal view returns (uint256) {
        ...
        uint256 start = rt.lastRewardBlock < rt.startBlock
            ? rt.startBlock
            : rt.lastRewardBlock;
        ...
        return
            rt.accRewardPerShare.add(
                DecimalMath.divFloor(
                    _getUnrewardBlockNum(i).mul(rt.rewardPerBlock),
                    totalSupply()
                )
            );
    }
...
contract ERC20Mine is BaseMine {
    ...
    function deposit(uint256 amount) external {
        ...
        uint256 actualStakeAmount = IERC20(_TOKEN_)
        .balanceOf(address(this))
        .sub(erc20OriginBalance);
