...
contract ETHToBSCLocker is Ownable, SignData {
    ...
    function lockOraiPermit(address sender, address bscAddress, uint256 amount, uint256 deadline, uint8 v, bytes32 r, bytes32 s) public ensure(deadline) {
    ...
    function unlockOraiPermit(address[] memory receives, uint256[] memory amounts, uint256 ethEpoch, uint256 deadline, address sender, uint8 v, bytes32 r, bytes32 s) external ensure(deadline) {
