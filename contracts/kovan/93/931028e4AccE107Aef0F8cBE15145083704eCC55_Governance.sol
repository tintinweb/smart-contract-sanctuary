...
contract Governance is Initializable, AccessControlUpgradeable {
    ...
    // IERC20 public constant BUNI_TOKEN = IERC20(0x0E7BeEc376099429b85639Eb3abE7cF22694ed49);
    // IBurToken public constant BUR_TOKEN = IBurToken(0xc1619D98847CF93d857DFEd4e4d70CF4f984Bd56);
    ...
    function initialize (
        ...
        voteFeeBurnRate = ABDKMath64x64.divu(7, 10); // 70%
        BUNI_TOKEN = IERC20(_buni);
        BUR_TOKEN = IBurToken(_bur);
    }
    ...
    IBurToken public BUR_TOKEN;

    // Fee for creating a proposal, paid by BUNI
    uint256 public creationFee;

    // Fee for voting a proposal, paid by BUR
    uint256 public voteFee;

    // The burning rate for vote fee
    int128 public voteFeeBurnRate;

    // Events
    event ProposalCreated(uint256 indexed proposalID);
    event Vote(uint256 indexed proposalID, bool isUpvote);
    event CancelProposal(uint256 indexed proposalID);
    event ExcuteProposal(uint256 indexed proposalID);
    ...
    function getProposalIDsByVoter(address _proposer) public view returns (uint256[] memory) {
        EnumerableSet.UintSet storage set = voters[address(_proposer)];
        uint256[] memory ids = new uint256[](set.length());
    ...
    function setCreationFee(uint256 _newCreationFee) public onlyGovernanceAdmin {
        creationFee = _newCreationFee;
    }

    function setVoteFee(uint256 _newVoteFee) public onlyGovernanceAdmin {
        voteFee = _newVoteFee;
    }

    function setVoteFeeBurnRate(uint256 _percent) public onlyGovernanceAdmin {
        voteFeeBurnRate = ABDKMath64x64.divu(_percent, 100);
    }

    function propose() public {
        require(BUNI_TOKEN.balanceOf(msg.sender) >= creationFee, "Insufficient buni");
        ...
        proposers[msg.sender].add(newProposal.id);

        BUNI_TOKEN.transferFrom(msg.sender, address(this), creationFee);

        emit ProposalCreated(proposalCount);
    }

    function vote(uint256 _proposalID, bool _isUpvote) public {
        require(BUR_TOKEN.balanceOf(msg.sender) >= voteFee, "Insufficient bur");
        ...
        uint256 burnFee = ABDKMath64x64.mulu(voteFeeBurnRate, voteFee);
        uint256 reward = voteFee.sub(burnFee);
        BUR_TOKEN.transferFrom(msg.sender, address(this), reward);
        BUR_TOKEN.burnFrom(msg.sender, burnFee);
        proposal.reward += reward;

        emit Vote(_proposalID, _isUpvote);
    }
    ...
    function executeProposal(uint256 _proposalID) public onlyGovernanceAdmin {
        Proposal storage proposal = proposals[_proposalID];
        uint256 reward = proposal.reward;
        require(BUR_TOKEN.balanceOf(address(this)) >= reward, "Insufficient bur");
        proposal.executed = true;
        BUR_TOKEN.transfer(address(proposal.proposer), reward);
    ...
    function withdraw(IERC20 _token, address _recipient, uint256 _amount) public onlyGovernanceAdmin {
        require(IERC20(_token).balanceOf(address(this)) >= _amount, "Insufficient bur");
