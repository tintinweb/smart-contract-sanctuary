...
contract LToken is Adminable {
    ...
    uint public totalSupply = 10000000e18; // 10 million Lever
    ...
    function mint(address account, uint96 amount) external onlyAdmin {
        require(account != address(0), "Lvr: mint to the zero address");
        totalSupply = totalSupply + amount;
        require(uint96(- 1) >= totalSupply, 'Lvr: total supply overflow');
        balances[account] = add96(balances[account], amount, 'Lvr: account amount overflow');
    ...
    function transfer(address dst, uint rawAmount) external returns (bool) {
        uint96 amount = safe96(rawAmount, "Lever::transfer: amount exceeds 96 bits");
