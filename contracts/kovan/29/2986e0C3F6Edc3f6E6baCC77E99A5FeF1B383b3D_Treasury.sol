...
contract Comptroller is IComptroller, IRegistry, IDeadline, Ownable, ReentrancyGuard {
    ...
    /**
     * @notice allows a user to deposit ETH and mint fxTokens in a single transaction. It should allow partial use of existing free collateral when minting fxTokens so that a user can top up their vault to mint.
    ...
    function mintWithEth(
        ...
        require(getMinimimCollateralETH(tokenAmountDesired, tokenDetails[token].mintCR, quote) <= msg.value.add(getFreeCollateral(msg.sender)), "Minimum collateral ratio not met");
