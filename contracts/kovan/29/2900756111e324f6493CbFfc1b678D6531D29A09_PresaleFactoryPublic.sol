    ...
    function createPresale(
        ...
        require(
            _info.openTime > block.timestamp &&
                _info.openVotingTime + safeLibrary.getVotingTime() + 900 <= // PROD: 86400 <=
                _info.openTime &&
                _info.closeTime - _info.openTime > 1200 && // PROD: 6900 &&
...
contract PresalePublic is ReentrancyGuard {
    ...
    modifier openRegister() {
        require(
            block.timestamp >= generalInfo.openTimePresale - 900 && // PROD: - 86400 &&
        ...
        notCreator
    {
        ...
        uint256 tokensLeft;
        uint256 nowTime = block.timestamp;
        if (nowTime < generalInfo.openTimePresale + 300) { // PROD: 3600) {
            require(
            ...
            tokensLeft =
                ...
                100;
        } else if (nowTime < generalInfo.openTimePresale + 600) { // PROD: 5400) {
            require(
        ...
        } else if (nowTime < generalInfo.openTimePresale + 900) { // PROD: 6300) {
            require(
        ...
        } else if (nowTime < generalInfo.openTimePresale + 1200) { // PROD: 6900) {
            require(
    ...
    function addLiquidity() external presaleIsNotCancelled nonReentrant {
        ...
        uniswapInfo.unlockTime =
            block.timestamp +
            (uniswapInfo.lpTokensLockDurationInDays * 60);  // PROD: 24 * 60 * 60);
    }
