...
contract R34P is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 10 * 10**6 * 10**7;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tBurnTotal;

    string private _name = 'R34P2';
    string private _symbol = 'R34P2';
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rBurn, uint256 tTransferAmount, uint256 tBurn) = _getValues(tAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tBurn) = _getTValues(tAmount, _burnFee);
        uint256 currentRate =  _getRate();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rBurn) = _getRValues(tAmount, tBurn, currentRate);
        return (rAmount, rTransferAmount, rBurn, tTransferAmount, tBurn);
    }

    function _getTValues(uint256 tAmount, uint256 burnFee) private pure returns (uint256, uint256) {
    ...
    function _setBurnFee(uint256 burnFee) external onlyOwner() {
        require(burnFee >= 0 && burnFee <= 10, 'taxFee should be in 1 - 10');
        _burnFee = burnFee;
    }
}