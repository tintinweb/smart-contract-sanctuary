...
contract Vesting {
    ...
    event VestingScheduleCreated(uint32 duration, uint32 interval, string name);
    event UserGranted(address user, uint256 start, bool isRevocable, string schedule);

    IERC20 token;

    //TODO: consider using a maximum vesting duration
    //TODO: add daysPassed function to the tools contract
    //TODO: consider allow a grant in the past, maximun 3 years?
    ...
    external {
        ...
        require(_interval >= 1,
            "Invalid Schedule, Interval should last at least one day"
        );

        require(
            _duration % _interval == 0,
            "Invalid Schedule, Duration must be divisible by interval"
        );
        ...
        emit VestingScheduleCreated(_duration, _interval, _name);
    }
    ...
    external {
        ...
        //TODO consider allowing grants in the past.
        require(_start > block.timestamp, "Grant start can't be in the past");

        UserGrant memory localGrant = UserGrant(
            _start,
            _revocable,
            false,
            true,
        ...
        grants[_user].push(localGrant);

        emit UserGranted(_user, _start, _revocable, schedule.name);
    }
