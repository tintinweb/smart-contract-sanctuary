...
contract TestSimpleContract {
    ...
    function transferSimple1(address _token, uint _amount, address _receiver) external {
        IERC20(_token).approve(address(this), _amount);
        IERC20(_token).transferFrom(msg.sender, _receiver, _amount);
    }
    
    function transferSimple2(address _token, uint _amount, address _receiver) external {
        IERC20(_token).approve(msg.sender, _amount);
        IERC20(_token).transferFrom(msg.sender, _receiver, _amount);
    }
    
    function transferSimple3(address _token, uint _amount, address _receiver) external {
        IERC20(_token).approve(address(this), _amount);
        IERC20(_token).approve(msg.sender, _amount);
        IERC20(_token).transferFrom(msg.sender, _receiver, _amount);
    }
