...
contract CUBAN is Context, IERC20, Ownable {
    ...
    string private _name = "Shark Cuban";
    ...
    constructor () {
        CommunityAddress = 0x8392283922bB78EbD8767c9cF462eC8dEA62F0fa;

        _rOwned[_msgSender()] = _rTotal;

        // IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);  //bsc mainnet
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);  //kovan testnet
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity.add(tBurn), _getRate());
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        tTransferAmount = tTransferAmount.sub(tBurn).sub(tCommunity);
        return (tTransferAmount, tFee, tLiquidity, tBurn);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tFeeTotal, uint256 currentRate) private view returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 tCommunity = calculateCommunityFee(tAmount);
        tFeeTotal = tFeeTotal.add(tCommunity);
        uint256 rFeeTotal = tFeeTotal.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rFeeTotal);
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 tCommunity = calculateCommunityFee(amount);
            
            if (takeFee) {
                feeCommunity = tCommunity;
            }
            //add liquidity
            swapAndLiquify(contractTokenBalance, feeCommunity);
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance, uint256 feeCommunity) private lockTheSwap {
        ...
        uint256 swapAmount = half.add(feeCommunity);
        uint256 rating = swapAmount.div(half);
        ...
        uint256 balanceHalf = newBalance.div(rating);
        uint256 communityBalance = newBalance.sub(balanceHalf);

        if(communityBalance > 0) {
            TransferEthToExternalAddress(communityBalance);
        }
    ...
    function TransferEthToExternalAddress(uint256 amount) private {
