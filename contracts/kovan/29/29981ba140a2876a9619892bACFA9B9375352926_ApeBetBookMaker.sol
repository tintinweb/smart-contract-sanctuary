...
contract ApeBetBookMaker is Ownable, ReentrancyGuard {
    ...
    mapping(bytes16 => mapping(uint256 => uint256)) public maxReserved;
        ...
        betsValidation(_bet_event_ids, _bet_winners, _bet_amounts)
    {
        for (uint256 i = 0; i < _bet_event_ids.length; i++) {
            ...
            if (maxReserved[_bet_event_ids[i]][affiliate_id] < estPayout) {
                maxPoolPayout = maxPoolPayout.add(maxReserved[_bet_event_ids[i]][affiliate_id]).sub(estPayout);
                maxReserved[_bet_event_ids[i]][affiliate_id] = estPayout;
            }
