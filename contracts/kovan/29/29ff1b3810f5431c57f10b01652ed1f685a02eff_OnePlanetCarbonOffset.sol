...
contract OnePlanetCarbonOffset is ERC20Interface, Owned, SafeMath {
    ...
    function offsetDirectWithETH(uint256 ethValue, string calldata message) external payable returns (bool success) {
        
        require(ethValue > 0, "You need to send at least some ETH");
        
        uint256 tokens = safeDiv(ethValue, ethPrice1PL);
        
        // Deposit(msg.sender, msg.value);
        
        tokens = safeDiv(tokens, offsetSigDigits);
        tokens = safeMul(tokens, offsetSigDigits); // only retire in kg
        address(this).transfer(ethValue);
