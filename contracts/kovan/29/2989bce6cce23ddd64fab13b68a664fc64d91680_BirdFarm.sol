...
contract BirdFarm is Ownable {
    ...
    uint256 public startRewardBlock = block.number;
    uint256 public endRewardBlock = block.number + 100000;
    ...
    function updatePool(uint256 _pid) public {
        ...
        require(
            (rewardToken.totalSupply().add(rewardTokenReward)) < cap,
            "cap is reached"
        );
    ...
    //this function will increace the supply
    function increaseSupply(uint256 _supply) external onlyOwner {
        require(
            (rewardToken.totalSupply().add(_supply)) < cap,
            "cap is reached"
        );

        //rewardToken.mint(address(this), _supply);
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            pool.accRewardTokenPerShare = pool.accRewardTokenPerShare.add(
                _supply.mul(1e12).div(lpSupply)
            );
            pool.lastRewardBlock = block.number;
        }
    }

    // Deposit LP tokens to BirdFarm for rewardToken allocation.
    ...
    // Withdraw LP tokens from BirdFarm.
    function withdraw(uint256 _pid, uint256 _amount) public {
        require(
            block.number > startRewardBlock && block.number < endRewardBlock,
            "Current block is not between startRewardBlock and endRewardBlock, You may use emergencyWithdraw()"
        );

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(
            user.unstakeTime >= now,
            "You can unstake after 72hrs of depositing LP Tokens"
        );
    ...
    function setRewardTokenPerBlock(uint256 _rewardTokenPerBlock) public {
        rewardTokenPerBlock = _rewardTokenPerBlock;
    }

    function setUnstakeFrozenTime(uint256 _unstakeFrozenTime) public {
    ...
    function setStartRewardBlock(uint256 _startRewardBlock) public {
        startRewardBlock = _startRewardBlock;
    }

    function setEndRewardBlock(uint256 _endRewardBlock) public {
    ...
    function setBonusEndBlock(uint256 _bonusEndBlock) public {
        bonusEndBlock = _bonusEndBlock;
    }
