...
contract LimitOrderManager is
    ...
    function initialize(IUniswapV3Factory _factory,
        ...
        ERC721Upgradeable.__ERC721_init("Kromatika Limit Orders NFT", "KROM-POS");
    }
    ...
    function processLimitOrder(uint256 _tokenId, uint256 _batchId) external override
        ...
        limitOrder.tokensOwed0 = limitOrder.tokensOwed0.add(_amount0);
        limitOrder.tokensOwed1 = limitOrder.tokensOwed1.add(_amount1);
    ...
    function canProcess(uint256 _tokenId, uint256 _gasPrice) external view override returns (bool) {
        ...
        // check the current pool position against the limit order
        (,int24 tick, , , , , ) = limitOrder.pool.slot0();

        if (limitOrder.orderType == 1 && tick > limitOrder.tickUpper) {
            return true;
        }

        if (limitOrder.orderType == 2 && tick < limitOrder.tickLower) {
            return true;
        }
    ...
    function _collect(uint256 _tokenId, address _owner) internal returns 
        ...
        uint256 balance = funding[_owner];
        (uint256 payment, address creator) = limitOrder.monitor.batchInfo(limitOrder.batchId);
        require(balance > 0 && payment <= balance);
        ...
        limitOrder.tokensOwed0 = limitOrder.tokensOwed0.add(_tokensToSend0);
        limitOrder.tokensOwed1 = limitOrder.tokensOwed1.add(_tokensToSend1);

        require(limitOrder.tokensOwed0 > 0 || limitOrder.tokensOwed1 > 0);

        _transferToOwner(limitOrder.pool.token0(), limitOrder.tokensOwed0, _owner);
        _transferToOwner(limitOrder.pool.token1(), limitOrder.tokensOwed1, _owner);
        _transferFees(payment, creator, address(limitOrder.monitor));
