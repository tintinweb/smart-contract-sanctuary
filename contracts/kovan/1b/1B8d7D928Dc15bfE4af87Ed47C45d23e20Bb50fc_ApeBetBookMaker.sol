...
contract ApeBetBookMaker is Ownable {
    ...
    modifier betsValidation(
        ...
        for (uint256 i = 0; i < _bet_event_ids.length; i++) {
            ...
            require(
                idToBet[bets[_bet_event_ids[i]][affiliate_id][msg.sender]]
                    .bet_amount == 0,
        ...
        for (uint256 i = 0; i < _event_ids.length; i++) {
            ...
            if (events[_event_ids[i]][affiliate_id].event_status == 2) {
                ...
                for (
                    ...
                    if (
                        idToBet[bets[event_id][affiliate_id][account]]
                            .bet_winner != events[event_id][affiliate_id].winner
                    ) {
    ...
    {
        for (uint256 i = 0; i < _bet_event_ids.length; i++) {
            bets[_bet_event_ids[i]][affiliate_id][msg.sender] = getBetId(
                _bet_event_ids[i],
                msg.sender
            );

            idToBet[bets[_bet_event_ids[i]][affiliate_id][msg.sender]] = Bet({
