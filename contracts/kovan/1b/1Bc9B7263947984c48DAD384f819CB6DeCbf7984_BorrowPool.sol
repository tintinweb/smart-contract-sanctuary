...
contract BorrowPool is Initializable, IBorrowPool, PoolSettingsManager {
    ...
    )
  {
    ...
    return (tick.adjustedTotalAmount, tick.adjustedRemainingAmount, tick.bondsQuantity, tick.adjustedPendingAmount);
  }
    ...
    )
  {
    ...
    availableDeposits = pool.state.normalizedAvailableDeposits;
  }
  ...
  function getAmountRepartition(
    ...
    if (bondsIssuanceIndex > tick.currentBondsIssuanceIndex) {
      uint256 yieldProviderLiquidityRatio = pool
        .parameters
        .YIELD_PROVIDER
        .getReserveNormalizedIncome(address(pool.parameters.UNDERLYING_TOKEN))
        .rayToWad();
      return (0, adjustedAmount.wadMul(yieldProviderLiquidityRatio));
    }

    (uint256 bondsQuantity, uint256 adjustedDepositedAmount) = pool.computeAmountRepartitionForTick(
      rate,
      adjustedAmount,
      bondsIssuanceIndex
    );
    ...
    uint256 accruedFeesShare = pool.peekAccruedFeesShare(adjustedDepositedAmount, rate, accruedFees);
    uint256 normalizedDepositedAmount = adjustedDepositedAmount.wadMul(jellyFiLiquidityRatio) + accruedFeesShare;

    return (bondsQuantity, normalizedDepositedAmount);
  }
    ...
    returns (uint256, uint256)
  {
    ...
    require(
      pool.state.normalizedAvailableDeposits + normalizedAmount < pool.parameters.MAX_TOKEN_DEPOSIT,
      Errors.BP_DEPOSIT_MAX_TOKEN_DEPOSIT_EXCEEDED
    );

    pool.initializeTick(rate);
    (ctx.adjustedAmount, ctx.bondsIssuanceIndex) = pool.depositToTick(rate, normalizedAmount);

    pool.depositToYieldProvider(sender, normalizedAmount, yieldProvider);

    emit Deposit(sender, borrower, ctx.adjustedAmount, rate, ctx.bondsIssuanceIndex);

    return (ctx.adjustedAmount, ctx.bondsIssuanceIndex);
  }
  ...
  function withdraw(Types.WithdrawInput calldata input)
    ...
    whenNotPaused
    onlyRole(POSITION_ROLE)
    onlyActiveOrderBook(input.borrower)
    returns (
      uint256,
      uint256,
      uint256,
  ...
  {
    Types.Pool storage pool = pools[input.borrower];
    WithdrawContext memory ctx;

    require(
      input.bondsIssuanceIndex <= (pool.ticks[input.rate].currentBondsIssuanceIndex + 1),
      Errors.BP_BOND_ISSUANCE_ID_TOO_HIGH
    );
    require(
      ((input.bondsIssuanceIndex <= pool.ticks[input.rate].currentBondsIssuanceIndex) &&
        pool.ticks[input.rate].adjustedRemainingAmount > 0) ||
        ((input.bondsIssuanceIndex > pool.ticks[input.rate].currentBondsIssuanceIndex) &&
          pool.ticks[input.rate].adjustedPendingAmount > 0),
    ...
    yieldProvider.withdraw(pool.parameters.UNDERLYING_TOKEN, ctx.normalisedDepositedAmountToWithdraw, input.owner);
  ...
  function updateRate(
  ...
  ) public override whenNotPaused onlyRole(POSITION_ROLE) returns (uint256, uint256) {
    Types.Pool storage pool = pools[borrower];
    ...
    (uint256 newAdjustedAmount, uint256 newBondsIssuanceIndex) = pool.depositToTick(newRate, normalizedAmount);

    emit UpdateRate(adjustedAmount, oldRate, newAdjustedAmount, newRate, newBondsIssuanceIndex);

    return (newAdjustedAmount, newBondsIssuanceIndex);
  }

  // BORROWER METHODS
  function topUpManagementFees(uint256 amount) external override whenNotPaused onlyRole(BORROWER_ROLE) {
    ...
    pool.topUpManagementFees(amount);

    emit TopUpManagementFees(borrowerAuthorizedAddresses[_msgSender()], amount);
  }
    ...
    onlyRole(BORROWER_ROLE)
  {
    ...
    emit Borrow(borrower, normalizedBorrowedAmount, pool.parameters.UNDERLYING_TOKEN);
  }

  function repay() external override whenNotPaused whenNotDefaulted(0) onlyRole(BORROWER_ROLE) {
    bytes32 borrower = borrowerAuthorizedAddresses[_msgSender()];
    Types.Pool storage orderBook = pools[borrower];
    ...
    if (block.timestamp > orderBook.state.currentMaturity + orderBook.parameters.LATE_REPAY_THRESHOLD) {
      normalizedRepayAmount += orderBook.parameters.LATE_REPAY_FEE;
      lateRepayFeePerBond = orderBook.parameters.LATE_REPAY_FEE.wadDiv(orderBook.state.bondsIssuedQuantity);
    }
    ...
    while (currentInterestRate >= orderBook.state.lowerInterestRate) {
      orderBook.repayForTick(currentInterestRate, lateRepayFeePerBond);
      currentInterestRate -= orderBook.parameters.RATE_SPACING;
    }

    orderBook.depositToYieldProvider(_msgSender(), normalizedRepayAmount, yieldProvider);

    if (block.timestamp > (orderBook.state.currentMaturity + orderBook.parameters.LATE_REPAY_THRESHOLD)) {
      emit LateRepay(borrower, normalizedRepayAmount, orderBook.parameters.UNDERLYING_TOKEN);
    } else {
      emit Repay(borrower, normalizedRepayAmount, orderBook.parameters.UNDERLYING_TOKEN);
    }
...
contract PoolSettingsManager is AccessControlUpgradeable, PausableUpgradeable, IGovernanceActions {
  ...
  function isOrderBookDefaulted(bytes32 borrower) public view override returns (bool) {
    return pools[borrower].state.defaulted;
  }
    ...
    onlyActiveOrderBook(keccak256(abi.encode(borrower)))
  {
    ...
    bytes32 borrowerHash = keccak256(abi.encode(borrower));
    emit BorrowerDisallowed(borrowerAddress, borrowerHash);
  }
    ...
    onlyActiveOrderBook(borrower)
  {
    ...
    emit SetMaxTokenDeposit(maxTokenDeposit, borrower);
  }

  function setManagementFeeDistributionRate(uint256 distributionRate, bytes32 borrower)
    external
    override
    onlyRole(GOVERNANCE_ROLE)
    onlyActiveOrderBook(borrower)
  {
    ...
    emit SetManagementFeeDistributionRate(distributionRate, borrower);
  }
...
interface IGovernanceActions {
  event OrderBookCreated(bytes32 borrower);
  ...
  event SetMaxTokenDeposit(uint256 maxTokenDeposit, bytes32 borrowerHash);
  event SetManagementFeeDistributionRate(uint256 distributionRate, bytes32 borrowerHash);
  ...
  ///@dev Check if a borrower is set as having defaulted
  function isOrderBookDefaulted(bytes32 borrower) external view returns (bool);
  ...
  /// @dev Set the order book management fee rate
  function setManagementFeeDistributionRate(uint256 distributionRate, bytes32 borrower) external;
...
library OrderBookLogic {
  ...
  function getBondIssuanceMultiplierForTick(
    ...
    Types.Tick storage tick = pool.ticks[rate];
    uint256 returnBondsIssuanceIndex = tick.bondsIssuanceIndexMultiplier[bondsIssuanceIndex];
    if (returnBondsIssuanceIndex == 0) {
      return ONE;
    }
    return returnBondsIssuanceIndex;
  }
  ...
  function getAccruedFeesShare(
    ...
    return tick.accruedFees.wadMul(adjustedAmount).wadDiv(tick.adjustedRemainingAmount);
  }
  ...
  function peekAccruedFeesShare(
    ...
    return accruedFees.wadMul(adjustedAmount).wadDiv(tick.adjustedRemainingAmount);
  }
  ...
  function depositToTick(
    ...
    uint256 adjustedAmount;
    uint256 returnBondsIssuanceIndex;
    if (pool.state.currentMaturity > 0) {
      adjustedAmount = normalizedAmount.wadDiv(tick.yieldProviderLiquidityRatio);
      tick.adjustedPendingAmount += adjustedAmount;
      returnBondsIssuanceIndex = tick.currentBondsIssuanceIndex + 1;
    } else {
      adjustedAmount = normalizedAmount.wadDiv(tick.jellyFiLiquidityRatio);
    ...
    return (adjustedAmount, returnBondsIssuanceIndex);
  }
  ...
  function computeAmountRepartitionForTick(
    ...
    if (tick.adjustedTotalAmount > 0) {
      adjustedAmountUsedForBondsIssuance = adjustedAmount
        .wadMul(pool.getBondIssuanceMultiplierForTick(rate, bondsIssuanceIndex))
    ...
    uint256 adjustedDepositedAmount = (adjustedAmount - adjustedAmountUsedForBondsIssuance).round();

    return (bondsQuantity, adjustedDepositedAmount);
  }
  ...
  function withdrawDepositedAmountForTick(
    ...
    uint256 normalizedAmountToWithdraw;
    if (bondsIssuanceIndex <= tick.currentBondsIssuanceIndex) {
      uint256 feesShareToWithdraw = pool.getAccruedFeesShare(rate, adjustedAmountToWithdraw);
      tick.accruedFees -= feesShareToWithdraw;
      tick.adjustedTotalAmount = tick.adjustedTotalAmount.roundApproxSub(adjustedAmountToWithdraw);
      tick.adjustedRemainingAmount = tick.adjustedRemainingAmount.roundApproxSub(adjustedAmountToWithdraw);
      normalizedAmountToWithdraw = adjustedAmountToWithdraw.wadMul(tick.jellyFiLiquidityRatio) + feesShareToWithdraw;
      pool.state.normalizedAvailableDeposits = pool.state.normalizedAvailableDeposits.roundApproxSub(
        normalizedAmountToWithdraw
      );
    } else {
      tick.adjustedPendingAmount = tick.adjustedPendingAmount.roundApproxSub(adjustedAmountToWithdraw);
      normalizedAmountToWithdraw = adjustedAmountToWithdraw.wadMul(tick.yieldProviderLiquidityRatio);
    }
    ...
    return normalizedAmountToWithdraw;
  }
  ...
  function repayForTick(
    ...
    if (tick.bondsQuantity > 0) {
      ...
      // update liquidity ratio with interests from bonds, yield provider and management fees
      ...
      tick.jellyFiLiquidityRatio += (tick.accruedFees + bondPaidInterests + lateRepayFees).wadDiv(
      ...
      tick.bondsIssuanceIndexMultiplier[tick.currentBondsIssuanceIndex] = ONE
        .wadMul(tick.yieldProviderLiquidityRatio)
        .wadDiv(tick.jellyFiLiquidityRatio);
      uint256 adjustedPendingAmount = tick.adjustedPendingAmount.wadMul(
        tick.bondsIssuanceIndexMultiplier[tick.currentBondsIssuanceIndex]
      );

      // update global orderbook state
      ...
      pool.state.normalizedAvailableDeposits +=
        tick.bondsQuantity +
        tick.bondsQuantity.wadMul(lateRepayFeePerBond) +
        tick.adjustedPendingAmount.wadMul(tick.yieldProviderLiquidityRatio);
  ...
  /**
   * @dev Top up management fees for later distribution
   **/
  function topUpManagementFees(Types.Pool storage pool, uint256 normalizedAmount) internal {
    pool.state.remainingManagementFeesReserve += normalizedAmount;
  }
  ...
  function collectFeesForTick(Types.Pool storage pool, uint256 rate) internal {
    ...
    if (tick.adjustedRemainingAmount > 0) {
      ...
      uint256 newYieldProviderLiquidityRatio = pool
        .parameters
        .YIELD_PROVIDER
        .getReserveNormalizedIncome(address(pool.parameters.UNDERLYING_TOKEN))
        .rayToWad();
      ...
      // compute management fees impact
      uint256 addedManagementFee = (pool.parameters.MGT_FEE_DISTRIBUTION_RATE *
        (block.timestamp - tick.lastFeeDistributionTimestamp))
        .wadMul(tick.adjustedRemainingAmount.wadMul(tick.jellyFiLiquidityRatio))
        .wadDiv(pool.state.normalizedAvailableDeposits);
      if (addedManagementFee > pool.state.remainingManagementFeesReserve) {
        addedManagementFee = pool.state.remainingManagementFeesReserve;
        pool.state.remainingManagementFeesReserve = 0;
      } else {
        pool.state.remainingManagementFeesReserve -= addedManagementFee;
      }

      // no ongoing loan, all deposited amount get the yield provider and management fees
      if (pool.state.currentMaturity == 0) {
        tick.jellyFiLiquidityRatio +=
          yieldProviderLiquidityRatioIncrease +
          addedManagementFee.wadDiv(tick.adjustedRemainingAmount);
      }
      // ongoing loan, fees accrued are calculated separately, global liquidity ratio will be updated at repay time
      else {
        tick.accruedFees +=
          yieldProviderLiquidityRatioIncrease.wadMul(tick.adjustedRemainingAmount) +
          addedManagementFee;
      }
      ...
      pool.state.normalizedAvailableDeposits +=
        addedManagementFee +
        tick.adjustedRemainingAmount.wadMul(yieldProviderLiquidityRatioIncrease).round();
    }
  }
   ...
   **/
  function peekFeesForTick(Types.Pool storage pool, uint256 rate) internal view returns (uint256, uint256) {
    Types.Tick storage tick = pool.ticks[rate];

    uint256 jellyFiLiquidityRatio = tick.jellyFiLiquidityRatio;
    uint256 accruedFees = tick.accruedFees;
    ...
    if (tick.adjustedRemainingAmount > 0) {
      ...
      // ongoing loan, fees accrued are calculated separately, global liquidity ratio will be updated at repay time
        ...
        accruedFees += yieldProviderLiquidityRatioIncrease.wadMul(tick.adjustedRemainingAmount) + addedManagementFee;
      }
    }

    return (jellyFiLiquidityRatio, accruedFees);
  }

  function getTickBondPrice(uint256 rate, uint256 loanDuration) public pure returns (uint256 price) {
    price = ONE.wadDiv(ONE + (rate * loanDuration) / SECONDS_PER_YEAR);
  }
