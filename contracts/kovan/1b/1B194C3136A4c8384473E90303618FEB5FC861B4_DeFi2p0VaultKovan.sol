...
contract DeFi2p0VaultKovan is Initializable, ERC20Upgradeable, OwnableUpgradeable, 
    ...
    event SetStrategist(address oldStrategist, address newStrategist);
    ...
    function withdraw(uint share, IERC20Upgradeable token, uint[] calldata amountOutMinList) external nonReentrant {
        ...
        //         if (!paused()) {
        //             withdrawAmt = withdrawFromStrategy(token, withdrawAmt, tokenAmtInVault, amountOutMinList);
        //         } else {
        ...
        if (token != DAI) withdrawAmt /= 1e12;
        token.safeTransfer(msg.sender, withdrawAmt);
        amountOutMinList; // To mute unused variable warning

        emit Withdraw(msg.sender, withdrawAmt, address(token), share);
    }

    function withdrawFromStrategy(
        IERC20Upgradeable token, uint withdrawAmt, uint tokenAmtInVault, uint[] calldata amountOutMinList
    ) private returns (uint) {
        strategy.withdraw(withdrawAmt - tokenAmtInVault, amountOutMinList);
        strategy.adjustWatermark(withdrawAmt - tokenAmtInVault, false);
        if (token != DAI) tokenAmtInVault /= 1e12;
        uint WETHAmt = WETH.balanceOf(address(this));
        withdrawAmt = (router.swapExactTokensForTokens(
            WETHAmt, amountOutMinList[0], getPath(address(WETH), address(token)), address(this), block.timestamp
        )[1]) + tokenAmtInVault;
        token.safeTransfer(msg.sender, withdrawAmt);
        return withdrawAmt;
    }

    function invest(uint[] calldata amountOutMinList) public whenNotPaused {
        require(
            msg.sender == admin ||
            msg.sender == owner() ||
        ...
        (uint WETHAmt, uint tokenAmtToInvest, uint pool) = swapTokenToWETH(USDTAmt, USDCAmt, DAIAmt, amountOutMinList);
        if (tokenAmtToInvest > 0) {
            strategy.invest(WETHAmt, amountOutMinList);
    ...
    function swapTokenToWETH(
        uint USDTAmt, uint USDCAmt, uint DAIAmt, uint[] calldata amountOutMinList
        ...
        if (USDTAmt > USDTAmtKeepInVault + 1e6) {
            ...
            WETHAmt = swap(address(USDT), address(WETH), USDTAmt, amountOutMinList[0]);
        ...
        if (USDCAmt > USDCAmtKeepInVault + 1e6) {
            ...
            uint _WETHAmt = swap(address(USDC), address(WETH), USDCAmt, amountOutMinList[1]);
            WETHAmt += _WETHAmt;
        ...
        if (DAIAmt > DAIAmtKeepInVault + 1e18) {
            ...
            uint _WETHAmt = swap(address(DAI), address(WETH), DAIAmt, amountOutMinList[2]);
            WETHAmt += _WETHAmt;
    ...
    function reimburse(uint farmIndex, address token, uint amount, uint[] calldata tokenPriceMin) external onlyOwnerOrAdmin {
        uint WETHAmt;
        WETHAmt = router.getAmountsOut(amount, getPath(token, address(WETH)))[1];
        WETHAmt = strategy.reimburse(farmIndex, WETHAmt, tokenPriceMin[0]);
        swap(address(WETH), token, WETHAmt, tokenPriceMin[1]);
    ...
    function reinvest(uint[] calldata amountOutMinList) external onlyOwnerOrAdmin whenPaused {
        _unpause();

        (uint USDTAmt, uint USDCAmt, uint DAIAmt) = transferOutFees();
        (uint WETHAmt, uint tokenAmtToInvest,) = swapTokenToWETH(USDTAmt, USDCAmt, DAIAmt, amountOutMinList);
        strategy.invest(WETHAmt, amountOutMinList);
        strategy.adjustWatermark(tokenAmtToInvest, true);
    ...
    function setStrategist(address _strategist) external onlyOwner {
        require(msg.sender == address(strategist) || msg.sender == address(admin) || msg.sender == owner(), "Not authorized");
        address oldStrategist = strategist;
        strategist = _strategist;
        emit SetStrategist(oldStrategist, _strategist);
    }
    ...
    function getAllPoolInUSD() public view returns (uint) {
        // WETHPriceInUSD amount in 8 decimals
        uint WETHPriceInUSD = uint(IChainlink(0x9326BFA02ADD2366b30bacB125260Af641031331).latestAnswer());
        ...
        // if (paused()) return WETH.balanceOf(address(this)) * WETHPriceInUSD / 1e8 + tokenKeepInVault - fees;
        // uint strategyPoolInUSD = strategy.getAllPoolInETH() * WETHPriceInUSD / 1e8;
