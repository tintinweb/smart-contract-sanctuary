...
contract GOLDENDRAAGON is Context, IERC20, Ownable {
    ...
    string private _name = "GOLDEN DRAAGON";
    ...
    uint256 public supplyAmountCompareForBNBRewards = 90 * 10**15 * 10**9;
    ...
    //uint256 public tokenHoldingMinForBNBrewards = 1 * 10**14 * 10**9;       // hold at least 0.1% of the token to get BNB rewards

    uint256 public maxNumOfTransfersToDoForReward = 5;

    uint256 public minimumAmountToHoldForRewards = 1000000 * 10**9;      // 0.01% - must hold at least  0.01% of the token to get rewards


    // uint256 public hoursToAddForReward = 24 hours; // CHANGEIT - make sure this is properly set, need to be 24 hours
    ...
    address public charityWallet = address(this);  
    ...
    uint256 public _maxTxAmount = 50000000 * 10**9; //0.5% of totalSupply
    ...
    uint256 public numTokensSellToAddToLiquidity = 30000000 * 10**9;      // 0.03%
    ...
    event Mylog(address from, address to, uint256 transferAmt, uint256 step);
    event methodCall(uint256 step);
    ...
    constructor () {
        ...
        address routerDEXAddress = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;      // TODO - Change this to the right router
    ...
    function setBurnFeePercent(uint256 burnFee) external onlyOwner() {
        require(burnFee <= 10, "Percentage too high. Please use a lower fee.");
        _burnFee = burnFee;
    }
    ...
    function _burn(uint256 burn) private {
        ...
        _rOwned[deadAddressOne] = _rOwned[deadAddressOne].add(rBurn);
    }
    ...
    function _transfer(
        ...
        emit Mylog(from, to, balanceBeforeTransfer, 1);

        if ((to == uniswapV2Pair) && (!_isAdminAccount[from] || !_isAdminAccount[to])) {
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
        }

        emit Mylog(from, to, balanceBeforeTransfer, 2);
        if(!inSwapAndLiquify){     
            emit Mylog(from, to, balanceBeforeTransfer, 3);
            ...
            if(_isExcluded[address(this)]){
                emit methodCall(1);
                amountToSellForLIQ = tokenFromReflection(tliquidityAmountTrackingSell);
                amountToSellForBNB = tokenFromReflection(tconvertBNBAmountTrackingSell);
            }
            emit Mylog(from, to, amountToSellForLIQ, 4);
            emit Mylog(from, to, amountToSellForBNB, 5);
...
emit methodCall(2);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        rAmount = rAmount.sub(tBurnAmount);
        ...
        emit Transfer(sender, deadAddressOne, tBurnAmount);
    }
