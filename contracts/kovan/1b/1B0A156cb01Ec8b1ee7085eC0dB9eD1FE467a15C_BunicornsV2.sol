...
contract BunicornsV2 is IBunicornsV2, Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    function unlockTransfer(uint256[] memory tokenIds) public {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            lastTransferTimestamp[tokenIds[i]] = 0;
        }
    }
    ...
    function getStakingHashrate(uint256 _tokenId) external view returns (uint256) {
        uint8 star = getStars(_tokenId);
        if (star <= 1) {
            return 0;
        }
        ...
        EnhancementCounters memory counters = enhancementCounters[_tokenId];
        uint256 bonusPoints = uint256(counters.bronzeEnhanced)
                                .add(counters.sliverEnhanced)
                                .add(counters.goldEnhanced);
        hashrate = hashrate.add(bonusPoints.mul(2));

        return hashrate.mul(10 ** 18);
    }
}
