...
contract LPStaking is AbstractLPStaking, ILPStaking {
    ...
    function streamRewards() external nonReentrant streaming(false) {
        address stakeholder = _msgSender();
        require(earned(stakeholder) > 0, "LPStaking: not earned anything yet");
    ...
    function claimRewards() external nonReentrant {
        address stakeholder = _msgSender();
        require(unlockedRewards(stakeholder) > 0, "LPStaking: no rewards to claim");
    ...
    function unlockedRewards(address stakeholder) public view returns (uint) {
        return unlocked_rewards[stakeholder].add(_unlockedRewards(stakeholder));
    }

    function streamingRewards(address stakeholder) external view returns (uint) {
        return streaming_rewards[stakeholder].sub(_unlockedRewards(stakeholder));
    }

    function earned(address account) public view returns (uint) {
        return UnsafeMath.divRoundingUp(_newEarned(account), 1e24).add(staking_rewards[account]);
    }

    function stakingAmount(address stakeholder) external view returns (uint) {
