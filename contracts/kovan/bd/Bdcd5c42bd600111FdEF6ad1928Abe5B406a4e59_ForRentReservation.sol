...
contract ForRentReservation is ForRentFactory {
    //DRTToken drtToken;
    IERC20 private _drtToken;
    event Reserve(uint256 _forRentId, address renter);

    constructor(address _tokenAddress) {
        _drtToken = DRTToken(_tokenAddress);
    }
    ...
    function reserve(uint256 _forRentId) public {
        ...
        uint256 renterBalance = _drtToken.balanceOf(msg.sender);
        require(
        ...
        _drtToken.approve(forRentToOwner[_forRentId], amountExpected + 1);
        bool sent = _drtToken.transferFrom(
            msg.sender,
            forRentToOwner[_forRentId],
            amountExpected
        );
        require(sent, "Failed to transfer tokens from renter to owner");
        emit Reserve(_forRentId, msg.sender);
