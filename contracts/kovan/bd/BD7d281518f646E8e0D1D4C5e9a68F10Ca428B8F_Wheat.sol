...
contract FarmlandEquipment is ERC721, Ownable {
    ...
    */
      function mintEquipment(address receiver, string memory tokenURI, string memory equipmentName, uint256 maxBoostLevel, uint256 blocksToMaxBoost, uint256 blocksToExpiry) external onlyOwner returns (uint256) {
        _tokenIds.increment();
        ...
        equipmentTrait.blockExpiry = 0;
        if ( blocksToExpiry > 0 ) {
            equipmentTrait.blockExpiry = block.number.add(blocksToExpiry);
        }
...
contract Wheat is ERC777, IERC777Recipient, ReentrancyGuard, IERC721Receiver, ERC165, ERC721Holder  {
    ...
    */
        ...
        requireEquipment(_msgSender(), false) // You can't release the LAND until you have released your farmland equipment
    {
        ...
        // TODO check amount of LAND is correctly calculated based % of amountLP being released - calc% = amount to release as a percentage of total LPAmount
        uint256 amount = senderFarm.amount.mul(amountLPToRelease).div(senderFarm.amountLP);

        //TODO check composted Crop is correctly calculated based % of amountLP being released
        uint256 compostedAmount = senderFarm.compostedAmount.mul(amountLPToRelease).div(senderFarm.amountLP);
        ...
        globalCompostedAmount = globalCompostedAmount.sub(compostedAmount);

        // Check if the farm will be closed completely
        if (amountLPToRelease == senderFarm.amountLP) {
        ...
        emit Released(_msgSender(), amountLPToRelease, compostedAmount);
    ...
    function allocateEquipment(uint256 tokenID) public
        ...
        (string memory equipmentName, uint256 maxBoostLevel,uint256 blocksToMaxBoost,uint256 blockExpiry) = nftToken.equipmentTraits(tokenID);
        ...
        farm.equipmentBlockExpiry = blockExpiry;
        ...
        returns (uint256)
    {
        ...
        // You don't get a boost if the your equipment has expired; if blockExpiry equals zero then no expiry
        if (targetFarm.equipmentBlockExpiry > 0 && block.number >= targetFarm.equipmentBlockExpiry) {
            return _percentMultiplier;
        }
