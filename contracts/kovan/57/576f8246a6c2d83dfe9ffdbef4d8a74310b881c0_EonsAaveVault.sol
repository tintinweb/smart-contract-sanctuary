...
contract EonsAaveVault is OwnableUpgradeable {
  ...
  function withdraw(uint _amount, uint256 _pid) external {
    ...
    UserInfo storage user = userInfo[msg.sender][_pid];
    uint256 totalStaked = router.totalStakedOf(_pid);
    require(_amount <= totalStaked, 'Withdraw not good');
    ...
    if (_amount > 0) {
      ...
      // }
      uint256 totalPending = rewardPending.add(devFeePending);
      user.amount = user.amount.sub(_amount.sub(totalPending));
      pool.totalStaked = pool.totalStaked.sub(_amount.sub(totalPending));
  ...
  function updateEmissionDistribution() public {
    ...
    for (uint i = 0; i < poolInfo.length; i++) {
      ...
      uint256 totalStaked = router.totalStakedOf(i);
      if (totalStaked > 0) {
        pool.accEonsPerShare = pool.accEonsPerShare.add(eonsBalance).mul(pool.allocPoint).div(totalAllocPoint).div(totalStaked);
      }
    }
  }
}
