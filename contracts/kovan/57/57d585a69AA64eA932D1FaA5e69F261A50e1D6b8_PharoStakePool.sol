...
contract PharoStakePool is AccessControl, ReentrancyGuard, TokenRecover {
    ...
    uint256 public PHRO_PER_BLOCK = 5 ether;
        ...
        ReentrancyGuard()
    {
        ...
        phroToken = IPHROToken(_phroToken);
        gphroToken = IGPHROToken(_gphroToken);
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            console.log("Pending Pharo for user", phroReward);
            accPhroPerShare = accPhroPerShare.add(
                phroReward.mul(1e12).div(lpSupply)
            );
            console.log("Acc PHRO per share ", accPhroPerShare);
        }
    ...
    function withdraw(uint256 _poolId, uint256 _amount) public {
        ...
        // take our 1% of the rewards and burn it
        // pending = pending - pending.div(100);

        safePhroTransfer(msg.sender, pending);
        console.log("Transferred ", pending, " gPHRO");
        // todo: need to set this to burn the gphro
        gphroToken.burnFrom(address(msg.sender), _amount);
        console.log("Burned ", _amount, " of gPHRO");
        safePhroTransfer(msg.sender, _amount);
        console.log("Transferred ", _amount, " PRHO");
    ...
    function safePhroTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount <= phroBal) {
            phroToken.transfer(_to, _amount);
        } else {
            phroToken.mintForContract(_to, _amount);
            // ** add event!!!
        }
    }

    /// @dev transfers PHRO from contract if available and mints if not
    function safegPhroTransfer(address _to, uint256 _amount) internal {
        uint256 gphroBal = gphroToken.balanceOf(address(this));
        if (_amount <= gphroBal) {
            gphroToken.transfer(_to, gphroBal);
        } else {
            gphroToken.mintStakerTokens(_to, _amount);
            // ** add event!!!
        }
    }
