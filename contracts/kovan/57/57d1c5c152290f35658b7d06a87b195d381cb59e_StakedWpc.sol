...
/**
 * @title StakedToken
 * @notice Contract to stake WPC token, tokenize the position and get rewards, inheriting from a distribution manager contract
 * @author Aave
 **/
contract StakedToken is IStakedWpc, ERC20Snapshot, DistributionManager, VersionedInitializable {
    ...
    ITransferHook public wpcGovernance;
    ...
    event WpcGovernanceChanged(address oldOne, address newOne);
    ...
    constructor(
        ...
        wpcGovernance = ITransferHook(_wpcGovernance);
    }
    ...
    function setWpcGovernance(address _wpcGovernance) external {
        require(msg.sender == EMISSION_MANAGER, 'ONLY_EMISSION_MANAGER');

        address old = address(wpcGovernance);
        wpcGovernance = ITransferHook(_wpcGovernance);
        emit WpcGovernanceChanged(old, _wpcGovernance);
    }
    ...
    function _beforeTokenTransfer(
        ...
        // caching the wpc governance address to avoid multiple state loads
        if (address(wpcGovernance) != address(0)) {
            wpcGovernance.onTransfer(from, to, amount);
        }
    }
}
...
/**
 * @title StakedWpc
 * @notice StakedToken with WPC token as staked token
 * @author Aave
 **/
contract StakedWpc is StakedToken {
    string internal constant NAME = 'xWPC';
    string internal constant SYMBOL = 'xWPC';
