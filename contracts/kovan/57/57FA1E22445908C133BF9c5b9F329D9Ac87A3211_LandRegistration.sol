...
contract LandRegistration is ILandRegistration {
    ...
    function addOrUpdateLandAppraisal(
        ...
        require(isLandIDValid[_landID], "LRSC: INVALID_LAND_ID");
        require(_appraisalAmount > 0,   "LRSC: EMPTY_APPRAISAL_AMOUNT");
        require(
    ...
    function updateAmountBorrowedByDeveloper(
        ...
        require(isLandIDValid[_landID], "LRSC: INVALID_LAND_ID");
        require(_amount > 0,            "LRSC: EMPTY_AMOUNT");
    ...
    function getLoanEntitiesByDeveloperID(uint _developerID) public view returns (ILoanPool.LoanEntity[] memory result) {
        require(isDeveloperIDValid[_developerID], "Developer should be valid.");

        address target = IManagementCompany(MCBoard).LOSCAddress();
        uint[] memory UniqueLoanEntityIDArray = getDeveloperByID[_developerID].myUniqueLoanEntityID;
        uint LoanPoolID;
        uint LoanEntityID;
        address LoanPoolAddress;
        for(uint i = 1; i < UniqueLoanEntityIDArray.length; i++) {
            (LoanPoolID, LoanEntityID) = getLoanPoolIDAndEntityIds(UniqueLoanEntityIDArray[i]);
            LoanPoolAddress = ILoanOriginator(target).getLoanPoolByID(LoanPoolID);
            ILoanPool.LoanEntity memory _loanEntity;
            // get part 1 data
            (_loanEntity.loanPoolName, _loanEntity.loanEntityID, 
            _loanEntity.developerID, _loanEntity.landID, 
            _loanEntity.interestRateAPY, _loanEntity.startDate, 
            _loanEntity.closeDate) = ILoanPool(LoanPoolAddress).getLoanEntityInfoPart1(LoanEntityID);
            // get part 2 data
            (_loanEntity.lastUpdateDate, _loanEntity.principal, 
            _loanEntity.interestObligated, _loanEntity.totalPaid, _loanEntity.status, 
            _loanEntity.projectType, 
            _loanEntity.projectDescription) = ILoanPool(LoanPoolAddress).getLoanEntityInfoPart2(LoanEntityID);

            // store data into result array: array starts from 0
            result[i - 1] = _loanEntity;
        }
    }
}
...
interface ILoanPool {
   ...
   function getLoanEntityInfoPart1(uint loanID) external view returns (string memory, uint, uint, uint, uint, uint, uint);
   function getLoanEntityInfoPart2(uint loanID) external view returns (uint, uint, uint, uint, bool, string memory, string memory);
}
