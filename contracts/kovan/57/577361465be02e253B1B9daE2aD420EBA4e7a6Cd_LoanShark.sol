...
// Replace by mainnet-deployed contract address
address constant LOANSHARK_ADDRESS = 0xAef39493194cEE5bc64CeC56C8CFFE5CABe0092F;
...
interface MobBossInterface {
     ...
     * @dev Requirements:
     * - Caller is the owner
     * - Collateral type to add is not an empty string
     * @param collateral The new collateral to support
     * @param typeOf The new collateral's type (e.g., ERC20, ERC721, ...)
     * @return True if succeeds, otherwise reverts
     */
    function addCollateralSupport(address collateral, string memory typeOf) external returns (bool);
...
contract MobBossStorage {
    ...
    /**
     * @notice Type of given collateral (e.g., ERC20, ERC721, ...)
     * @dev mapping(collateral => string)
     */
    mapping(address => string) public collateralTypeOf;
...
contract MobBoss is MobBossInterface, MobBossStorage, Ownable {
    ...
    modifier onlySupportedCollateral(address collateral) {
        require(bytes(collateralTypeOf[collateral]).length != 0, "MobBoss: collateral is not supported");
        _;
    }

    /// @inheritdoc MobBossInterface
    function addCollateralSupport(address collateral, string memory typeOf) public override onlyOwner returns (bool) {
        require(bytes(collateralTypeOf[collateral]).length != 0, "MobBoss: invalid collateral type");
        collateralTypeOf[collateral] = typeOf;
        setCollateralEnabled(collateral, true);
        return true;
    }

    /// @inheritdoc MobBossInterface
    function setCollateralEnabled(address collateral, bool enabled)
        public
        override
...
contract LoanShark is LoanSharkInterface, LoanSharkStorage, Ownable {
    ...
    function spawnBin(address collateral) public override returns (bool) {
        ...
        if (
            keccak256(abi.encodePacked(MobBoss(mobBossAddress).collateralTypeOf(collateral))) ==
    ...
    function flushBin(uint256 binId) public override validBinById(binId) returns (bool) {
        ...
        uint256[] memory values = fragments[binId].all();
        fragments[binId].empty();
        if (
            keccak256(abi.encodePacked(MobBoss(mobBossAddress).collateralTypeOf(bin.collateral))) ==
            keccak256(abi.encodePacked(ERC20))
        ) {
            for (uint256 i; i < values.length; i++) {
                Erc20Interface(bin.collateral).transfer(GRAVEYARD_ADDRESS, values[i]);
            }
        } else if (
            keccak256(abi.encodePacked(MobBoss(mobBossAddress).collateralTypeOf(bin.collateral))) ==
            keccak256(abi.encodePacked(ERC721))
            ...
            for (uint256 i; i < values.length; i++) {
                Erc721Interface(bin.collateral).transferFrom(address(this), GRAVEYARD_ADDRESS, values[i]);
            }
        }
        ...
        returns (bool)
    {
        ...
        require(!isBinUnderwater(binId), "LoanShark: bin is underwater");
        require(getOutstandingDebt(binId) + issueAmount < getCollateralValue(binId), "LoanShark: not enough credit");
        Bin memory bin = bins[binId];
        uint256 inc = PRBMathUD60x18.div(issueAmount, vigAccumilatorOf[collateral].accumilator);
        ...
        returns (bool)
    {
        ...
        require(!isBinUnderwater(binId), "LoanShark: bin is underwater");
        Bin memory bin = bins[binId];
        uint256 dec = PRBMathUD60x18.div(burnAmount, vigAccumilatorOf[collateral].accumilator);
        ...
        returns (bool)
    {
        ...
        if (
            ...
            for (uint256 i; i < values.length; i++) {
                Erc20Interface(bin.collateral).transferFrom(msg.sender, address(this), values[i]);
            }
        } else if (
            keccak256(abi.encodePacked(MobBoss(mobBossAddress).collateralTypeOf(bin.collateral))) ==
            keccak256(abi.encodePacked(ERC721))
            ...
            for (uint256 i; i < values.length; i++) {
                Erc721Interface(bin.collateral).transferFrom(msg.sender, address(this), values[i]);
            }
        }
        bins[binId] = bin;
        return true;
        ...
        returns (bool)
    {
        ...
        if (
            ...
            for (uint256 i; i < values.length; i++) {
                Erc20Interface(bin.collateral).transfer(msg.sender, values[i]);
            }
        } else if (
            keccak256(abi.encodePacked(MobBoss(mobBossAddress).collateralTypeOf(bin.collateral))) ==
            keccak256(abi.encodePacked(ERC721))
            ...
            for (uint256 i; i < values.length; i++) {
                Erc721Interface(bin.collateral).transferFrom(address(this), msg.sender, values[i]);
            }
        }
        bins[binId] = bin;
        return true;
    ...
    function realizeProtocolDebt(address collateral) public override returns (bool) {
        ...
        if (vigAccumilatorOf[collateral].accumilator == 0) {
            vigAccumilatorOf[collateral].accumilator = PRBMathUD60x18.SCALE;
            ...
            vigAccumilatorOf[collateral].accumilator = PRBMathUD60x18.mul(
                vigAccumilatorOf[collateral].accumilator,
                PRBMathUD60x18.powu(MobBoss(mobBossAddress).vigOf(collateral), timeSpan)
            );
            accumilatorDiff = vigAccumilatorOf[collateral].accumilator - accumilatorDiff;
            mintAmount = PRBMathUD60x18.mul(totalNormDebt[collateral], accumilatorDiff);
        }
    ...
    function getOutstandingDebt(uint256 binId) public view override validBinById(binId) returns (uint256) {
        Bin memory bin = bins[binId];
        return PRBMathUD60x18.mul(bin.normDebtAmount, vigAccumilatorOf[bin.collateral].accumilator);
    }

    /// @inheritdoc LoanSharkInterface
    function getCollateralizationRatio(uint256 binId) public view override returns (uint256) {
        return PRBMathUD60x18.div(getCollateralValue(binId), getOutstandingDebt(binId));
    }
