...
struct StakeOption {
    ...
    uint emission;          // calculated emission for each option
...
contract StakeRewards is PausableUpgradeable, BumperAccessControl {
    ...
    uint public totalEmissionPerSecond; // total emission per second for all options
    address public stakeToken; // address of token
    uint64 public unlockTimestamp; // timestamp where this contract will unlocked
    uint32 public withdrawWindow; // withdraw window for autorenew option. 
    ...
    event WithdrawRequested(address indexed sender, uint256 amount, uint256 timestamp );
    ...
    function initialize(
        ...
        totalEmissionPerSecond = uint(5000) * uint(10**18) / 24 / 60 / 60;

        cooldownPeriod = 10 days;

        // create stake options (it can be change later by governance)
        stakeOptions.push(StakeOption(0,  100, 0, 0, 0, 0)); //  0 days, 1
        stakeOptions.push(StakeOption(30, 150, 0, 0, 0, 0)); // 30 days, 1.5
        stakeOptions.push(StakeOption(60, 275, 0, 0, 0, 0)); // 60 days, 2.75
        stakeOptions.push(StakeOption(90, 600, 0, 0, 0, 0)); // 90 days, 6
    }
    ...
    function stake(uint256 amount, uint16 option, bool autorenew) external whenNotPaused {
    ...
    function _stakeFor(
        ...
        console.log("New stake from index %s", opt.index );
    ...
    function switchAutorenew(uint16 stakeIndex) external whenNotPaused {
        StakeInfo[] storage stakeInfoList = usersStake[msg.sender];
        require(stakeInfoList.length > stakeIndex, "!index");
        ...
        if (s.autorenewAt > 0) {
            ...
            require( block.timestamp < s.start + (stakeOptions[s.option].periodInDays * 1 days), "end" );
    ...
    function claimRewards(uint16 stakeIndex) external whenNotPaused {
        ...
        console.log("Claim rewards %s start", stakeIndex);

        _updateIndex(s.option);
    ...
    function calcRewards(StakeInfo memory s) public view returns (uint rewards, bool claimable, bool withdrawable, uint endOfLastPeriod) {
        StakeOption memory opt = stakeOptions[s.option];
        console.log("calcRewards option %s, total %s", s.option, opt.total);
        uint index = calculateCumulativeIndex(opt.index, opt.emission, opt.lastTimestamp, uint64(block.timestamp), opt.total );            
        console.log("index %s->%s, emission %s", s.lastCI, index, opt.emission );
        console.log("stake amount: %s", s.amount );

        if (opt.periodInDays == 0) { // flexible staking
            endOfLastPeriod = block.timestamp;
            rewards = (index - s.lastCI) * s.amount; 
            claimable = rewards > 0;
            withdrawable = block.timestamp > (s.requestedAt + cooldownPeriod) && 
                block.timestamp < (s.requestedAt + cooldownPeriod + withdrawWindow);
        }
        else if (s.autorenewAt > 0) { 
            // calculate number of periods
            uint periods = (block.timestamp - s.start) / (1 days * opt.periodInDays);  
            // we must be in withdraw window
            endOfLastPeriod = s.start + (periods * 1 days * opt.periodInDays);
            // calculate rewards
            rewards = (index - s.lastCI) * s.amount; 
         
            if (block.timestamp > endOfLastPeriod){
                rewards -= (block.timestamp - endOfLastPeriod) * opt.emission * s.amount * 10**18 / opt.total; 
            }                 
            ...
            endOfLastPeriod = s.start + (1 days * opt.periodInDays);
            ...
            withdrawable = block.timestamp > (s.start + (opt.periodInDays * 1 days));
        }
        ...
        console.log("rewards: %s, withdrawable: %s, claimable: %s", rewards, withdrawable, claimable );          
    }
    ...
    function calcRewardsByIndex(uint16 stakeIndex) public view returns (uint rewards, bool claimable, bool withdrawable, uint endOfLastPeriod) {
        (rewards,claimable,withdrawable,endOfLastPeriod) = calcRewards( usersStake[msg.sender][stakeIndex] );
    }

    /// @notice Restake tokens of given stake to new stake with given option with or without rewards
    function restake(uint16 stakeIndex, uint16 option, bool withRewards, bool autorenew) external whenNotPaused  
    {
        require(option < stakeOptions.length, "!option");
    ...
    function requestWithdraw(uint16 stakeIndex) external whenNotPaused { 
        require(stakeIndex < usersStake[msg.sender].length, "!index");
        StakeInfo storage s = usersStake[msg.sender][stakeIndex];
        StakeOption memory opt = stakeOptions[s.option];
        require(opt.periodInDays == 0, "!option");
        require(block.timestamp > s.requestedAt + cooldownPeriod, "requested" );

        s.requestedAt = uint64(block.timestamp);

        emit WithdrawRequested(msg.sender, s.amount, block.timestamp );
    }
    ...
    function totalAmount() public view returns (uint amount, uint weithedAmountSum) {
        for (uint16 i = 0; i < stakeOptions.length; i++) {
            if (stakeOptions[i].total == 0) continue; 

            amount += stakeOptions[i].total;
            weithedAmountSum += stakeOptions[i].multiplier * stakeOptions[i].total;
        }
    }
    ...
    function _updateIndexes() internal {
        ...
        for (uint16 i = 0; i < stakeOptions.length; i++) {
            StakeOption storage opt = stakeOptions[i];
            if (opt.total > 0)
                opt.index = calculateCumulativeIndex(opt.index, opt.emission, opt.lastTimestamp, ts, opt.total );            
            opt.lastTimestamp = ts;
            console.log("Opt: %s, index %s", i, opt.index );
        }
    }

    /// @notice calculate current emission rate per second by staked amount of tokens (it can be more than fact emission because of users can unstake with delay)
    function _updateEmissions() internal {
        (, uint weithedAmountSum) = totalAmount();
        ...
        for (uint16 i = 0; i < stakeOptions.length; i++) {
            ...
            if (option.total > 0)
                option.emission = totalEmissionPerSecond  * (option.total * option.multiplier) / weithedAmountSum;
            else 
                option.emission = 0;
            sumEmission += option.emission;
        }

        if (sumEmission > 0)
            require(sumEmission > totalEmissionPerSecond - 100 && sumEmission < totalEmissionPerSecond + 100, "!emission" );
    }

    /// ------------------- EXTERNAL OWNER/GOVERNANCE FUNCTIONS -------------------
    /// @notice Add new stake option
    /// @param periodInDays - option lookup period
    /// @param multiplier - multiplier for option multiply by 10000 (1000 equal 1)
    function addStakeOption(uint16 periodInDays, uint16 multiplier) external onlyGovernance {
        stakeOptions.push(
            StakeOption(periodInDays, multiplier, 0, 0, 0, 0)
        );
    }

    /// @notice Update stake option
    /// @param stakeIndex - index of stake to update
    /// @param periodInDays - option lookup period
    /// @param multiplier - multiplier for option multiply by 10000 (1000 equal 1)
    function updateStakeOption(uint stakeIndex, uint16 periodInDays, uint16 multiplier) external onlyGovernance {
        stakeOptions[stakeIndex].periodInDays = periodInDays;
        stakeOptions[stakeIndex].multiplier = multiplier;
    }
    ...
    function _updateIndex(uint16 option) internal {
        uint64 ts = uint64(block.timestamp);
        StakeOption storage opt = stakeOptions[option];
        if (opt.total > 0)
            opt.index = calculateCumulativeIndex(opt.index, opt.emission, opt.lastTimestamp, ts, opt.total );
        opt.lastTimestamp = ts;    
        console.log("_updateIndex option %s index %s", option, opt.index);
    }


    /// @notice set emission per second for all options
    function setTotalEmissionPerSecond(uint emission) external onlyGovernance {
        _updateIndexes();
        totalEmissionPerSecond = emission;
        _updateEmissions();
    }
    ...
    function serWithdrawWindow(uint32 _window) external onlyGovernance {
