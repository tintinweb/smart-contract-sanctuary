...
contract ReBakedDAO is Ownable {
	...
	mapping (bytes32 => mapping(address => uint256)) private observerData;
	...
	*/
		...
		onlyOwner
	{
		...
		require(_project.initiator != address(0), "non-existent project");
		require(_project.timeApproved == 0, "already approved project");
		_project.timeApproved = block.timestamp;
	...
	*/
		...
		returns(bytes32 projectId_)
	{
		...
		Project storage _project = projectData[projectId_];
		_project.initiator = msg.sender;
		_project.token = token_;
		_project.budget = budget_;
		_project.timeCreated = block.timestamp;
		emit CreatedProject(projectId_, msg.sender, address(token_), budget_);
	}
	...
	*/
	...
	{
		Project storage _project = projectData[projectId_];
		require(_project.timeApproved == 0, "can't add observers when project is approved");
		for(uint256 i = 0; i < observers_.length; i++){
			require(observers_[i] != address(0), "observer address is zero");
			observerData[projectId_][observers_[i]] = block.timestamp;
		}
		_project.totalObservers += observers_.length;
	...
	*/
	...
	{
		Project storage _project = projectData[projectId_];
		require(_project.timeApproved != 0, "project is not approved");
		if(address(_project.token) == address(0))
			_project.token = _deployIOUT(_project.budget);
		else
			_project.token.safeTransferFrom(msg.sender, address(this), _project.budget);
		uint256 _feeDaoAmount = _project.budget * feeDao / FEE_PRECISION;
		uint256 _feeObserversAmount = _project.budget * feeObservers / FEE_PRECISION;
		_project.token.safeTransfer(treasury, _feeDaoAmount);
		_project.budgetAllocated = _feeDaoAmount + _feeObserversAmount;
		_project.timeStarted = block.timestamp;
	...
	*/
	...
	{
		Project storage _project = projectData[projectId_];
		require(_project.timeStarted != 0, "project is not started");
		uint256 _projectBudgetAvaiable = _project.budget - _project.budgetAllocated;
		...
		_project.budgetAllocated += _totalPackageBudget;
		packageId_ = _generatePackageId(projectId_);
		Package storage _package = packageData[projectId_][packageId_];
		_package.budget = budget_;
		_package.bonus = bonus_;
		_package.timeCreated = block.timestamp;
	...
	*/
		...
		onlyAmountGreaterThanZero(mgp_)
	{
		...
		Package storage _package = packageData[projectId_][packageId_];
		uint256 _packageBudgetAvailable = _package.budget - _package.budgetAllocated;
		require(_packageBudgetAvailable >= mgp_, "not enough package budget left");
		Collaborator storage _collaborator = collaboratorData[projectId_][packageId_][collaborator_];
		require(_collaborator.mgp == 0, "collaborator already added to package");
		_package.budgetAllocated += mgp_;
		_collaborator.mgp = mgp_;
	...
	*/
	...
	/**
	* @dev Returns npn-zero value (time was added) if observer is registered for project id.
	* @param projectId_ project ID
	* @param observer_ observer's address
	* @return bool
	*/
	function isObserver(bytes32 projectId_, address observer_)
		external
		view
		returns(uint256)
	{
	...
	*/
	function getPackageData(bytes32 projectId_, bytes32 packageId_)
		external
		view
		returns(Package memory packageData_)
	{
		packageData_ = packageData[projectId_][packageId_];
	}
	...
	*/
	function getCollaboratorData(bytes32 projectId_, bytes32 packageId_, address collaborator_)
		external
		view
		returns(Collaborator memory collaboratorData_)
	{
		collaboratorData_ = collaboratorData[projectId_][packageId_][collaborator_];
	}
