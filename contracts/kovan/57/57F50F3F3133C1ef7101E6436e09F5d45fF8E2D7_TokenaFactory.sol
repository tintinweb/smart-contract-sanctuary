 ...
 */
contract TokenaFactory is ITokenaFactory, OwnableUpgradeable, ReentrancyGuardUpgradeable {
	...
	function initialize(
		...
		require(_dexFactory != address(0), "Not valid uniswap address");
		feeTaker = _feeTaker;
	...
	function createStaking(
		...
		for (uint256 i; i < rewardToken.length; i++) {
			IERC20Upgradeable(rewardToken[i]).safeTransferFrom(msg.sender, feeTaker, (rewardTokenAmounts[i] * feePercentage) / 100);
	 ...
	 */
	function setMasterStaking(address adr) external onlyOwner {
	...
	function changePromotedStaking(address pool, bool flag) external onlyOwner {
	...
	function changePromotedLM(address pool, bool flag) external onlyOwner {
		require(pool != address(0), "Must be valid address");
		if (flag) {
