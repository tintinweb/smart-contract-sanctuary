...
contract ERC1155Rubic is ERC1155, AccessControl {
    ...
    function mint(
        ...
        require(balanceOf(account, id) == 0, 'ERC1155Rubic: User can own only 1 token of each token type');
        _verifySender(account, id, nonce, signature);
        _mint(account, id, 1, "");
    }
    ...
    function _verifySender (
        ...
        address signer = ECDSA.recover(
            keccak256(abi.encodePacked(_TYPEHASH, account, id, nonce)),
            signature
        );
        require(
    ...
    function decode (
        ...
        return ECDSA.recover(
            keccak256(abi.encodePacked(_TYPEHASH)),
            signature
        );
    }
}
