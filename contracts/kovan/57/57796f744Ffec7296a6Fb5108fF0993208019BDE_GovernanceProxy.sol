...
// File: contracts\balancer-labs\configurable-rights-pool\interfaces\IERC20.sol
...
// File: contracts\balancer-labs\configurable-rights-pool\interfaces\IConfigurableRightsPool.sol
...
// File: contracts\balancer-labs\configurable-rights-pool\libraries\SafeApprove.sol
...
// File: contracts\balancer-labs\configurable-rights-pool\libraries\SmartPoolManager.sol
...
// File: contracts\GovernanceProxy.sol
...
contract GovernanceProxy {
    ...
    uint256 public proposalDelegationMinimum = 500;
...
//========================vote functions=============================//
    ...
    function newProposal(uint propType, uint[] calldata uints, address[] calldata addresses, bool[] calldata bools) public {
        ...
        uint startDate;
        if(lastProposed > (creationDate - 1 seconds) && lastProposed != 0) {
            startDate = lastProposed + 1 seconds;
        } else {
            startDate = creationDate;
        }
        delegation[msg.sender] = Delegation(startDate+3 seconds,proposalDelegationMinimum);
        //changed vote to +1 in order to expediate testing
        proposal[proposalCount] = Proposal(propType, creationDate, startDate, msg.sender, uints, addresses, bools, 1, 0, true);
    ...
    function castVoteERC20(uint propID, uint vote, uint voteWeight) public {
        ...
        //require(proposalActive == true, 'No active proposal'); change to check against the proposal itself
        ...
        //require(block.timestamp >= proposal[propID].startDate && block.timestamp <= proposal[propID].startDate+3 seconds, 'Proposal not active');
        BuoyERC20.transferFrom(msg.sender, address(this), voteWeight);
        delegation[msg.sender] = Delegation(proposal[propID].startDate+3 seconds,voteWeight);
        if(vote == 0) {
    ...
    function castVoteERC721(uint propID, uint vote,  uint id) public {
        ...
        delegation[msg.sender] = Delegation(proposal[propID].startDate+3 seconds,_voteWeight);
        if(vote == 0) {
    ...
    function endVote(uint id) public returns (bool) {
        ...
        require(block.timestamp > startDate+3 seconds && startDate != 0, 'Not an endable proposal');
    ...
    function enactProposal(uint id) public {
        ...
        if(proposal[id].proposaltype == 5) {
            ...
            addressIndex.setGovernanceProxy(proposal[id].addresses[0]);
        } else
        ...
        if(proposal[id].proposaltype == 7) {
            Mine mine = Mine(addressIndex.getMine());
            mine.setSwapingActive(proposal[id].bools[0]);
        } else
        ...
        if(proposal[id].proposaltype == 10) {
            SPool spool = SPool(addressIndex.getSmartPool());
            spool.updateWeightsGradually(proposal[id].uints, block.number, block.number + 2 days);
        } else
        //type 11 - add a new token to the pool
        if(proposal[id].proposaltype == 11) {
            SPool spool = SPool(addressIndex.getSmartPool());
            spool.proposeAddToken(proposal[id].addresses[0], proposal[id].uints[0], proposal[id].uints[1]);
        } else
    ...
    function purgeProposal(uint id) public {
        ...
        require(block.timestamp > proposal[id].startDate + 1 seconds);
        proposal[id].active = false;
