...
// Part: OpenZeppelin/openzeppel[email protected]/ERC721URIStorage
    ...
    AccessControl
{
    ...
    IUniswapV3Pool[] public pools = new IUniswapV3Pool[](3);
     ...
     * @param _nftEnable True - Use NFT functionality, False - Stop NFT functionality
     * @param _nft Address of NFT owned by the User
     * @param _admin address for setting an admin role
     */
    constructor(
        address[3] memory _pools,
        ...
        for (uint8 i=0; i<3; i++) pools[i] = IUniswapV3Pool(_pools[i]);
        currentPool = pools[_currentPoolIndex];
        token0 = IERC20(IUniswapV3Pool(currentPool).token0());
        token1 = IERC20(IUniswapV3Pool(currentPool).token1());
        tickSpacing = IUniswapV3Pool(currentPool).tickSpacing();
        ...
        )
    {
        ...
        if(nftEnable){
            require(
                (NFT_TOKEN.ownerOf(tokenNftId) == sender && !usedNft[tokenNftId]),
                "Wrong Invitational NFT parameters"
            );
            if(!info.isNotFirstDeposit){
                info.startTimeDeposit = block.timestamp;
                info.isNotFirstDeposit = true;
            }
            usedNft[tokenNftId] = true;
            /* if(burnNft) {
                NFT_TOKEN._burn(tokenNftId);
            } */
        }
        ...
        (uint256 sharesMax, , ) = _calcSharesAndAmounts(deposit0Max, deposit1Max);
        ...
        if (NFT_TOKEN.ownerOf(tokenNftId) == _msgSender() && nftEnable)
            require(totalSupply() <= maxTotalSupply + sharesMax, "maxTotalSupply");
        else
    ...
    function _poke(int24 tickLower, int24 tickUpper) internal {
        ...
        if (liquidity > 0) {
            currentPool.burn(tickLower, tickUpper, 0);
        }
    }
     ...
     * @param _tokenNftId ID of User's invitational NFT 
    ...
    function withdraw(
        ...
        // Works if NFT functionality is Enabled
        if (balanceOf(sender) == 0 && !(block.timestamp >= userInfo[sender].startTimeDeposit + daysAfter) && nftEnable) {
    ...
    function rerange(int24 _tickRange, uint8 _poolId) external {
        ...
        require(_poolId >= 0 && _poolId < 3);
        currentPool = pools[_poolId];

        int24 _tickSpacing = currentPool.tickSpacing();
        int24 _baseLowerPrev = baseLower; 
        int24 _baseUpperPrev = baseUpper;
        int24 _tick = tick;

        int24 deltaLower = _tick - _baseLowerPrev;
        int24 deltaUpper = _baseUpperPrev - _tick;
        int24 _baseLower = (_tick - deltaLower * _tickRange / (deltaLower + deltaUpper)) / _tickSpacing * _tickSpacing;
        int24 _baseUpper = (_tick + deltaUpper * _tickRange / (deltaLower + deltaUpper)) / _tickSpacing * _tickSpacing;

        (, _tick, , , , , ) = currentPool.slot0();
    ...
    {
        if (liquidity > 0) {
            (burned0, burned1) = currentPool.burn(tickLower, tickUpper, liquidity);
        }

        // Collect all owed tokens including earned fees
        (uint256 collect0, uint256 collect1) = currentPool.collect(
            address(this),
            tickLower,
            tickUpper,
            type(uint128).max,
    ...
    function _mintLiquidity(
        ...
        if (liquidity > 0) {
            currentPool.mint(address(this), tickLower, tickUpper, liquidity, "");
        }
    }
        ...
        )
    {
        ...
        return currentPool.positions(positionKey);
    }
    ...
    function emergencyBurn(
        ...
        currentPool.collect(address(this), tickLower, tickUpper, type(uint128).max, type(uint128).max);
    }
    ...
    // Set a period that sets how many days the user should keep the Deposit 
    ...
    {   
        address sender = _msgSender();
        if (nftEnable) {
            require(_to != address(0) && _to != address(this), "You can't send NFT to zero address");
            require(
                (block.timestamp >= userInfo[sender].startTimeDeposit + daysAfter),
                "The period of time before receiving Invitational NFT has not passed"
            );

            require(!userInfo[sender].haveAGift, "You already got Invitational NFT to share");
            userInfo[sender].haveAGift = true;
            NFT_TOKEN.mint(_tokenURI, _to);
        }
    }
    ...
    function setStartStopNFT(bool _startStopNft) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "onyGovernance");
        nftEnable = _startStopNft;
    } 

    // Show if Functionality connected with NFT is Enabled(True) or Disabled(False)
    function showNftFuncStatus() public view returns (bool) {
        return nftEnable;
    }
}