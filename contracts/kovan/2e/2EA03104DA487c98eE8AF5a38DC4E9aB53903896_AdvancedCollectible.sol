...
// Part: OpenZeppelin/openzeppelin-contract[email protected]/Address
...
contract AdvancedCollectible is ERC721, VRFConsumerBase {
    ...
    enum Paint{ MONOPOLY, RIQUINHO, TIO_PATINHAS}

    mapping(bytes32 => address) public requestIdToSender;
    mapping(bytes32 => string) public requestIdToTokenURI;
    mapping(uint256 => Paint) public tokenIdToPaint;
    mapping(bytes32 => uint256) public requestIdToTokenId;
    event requestedCollectible(bytes32 indexed requestId);


    constructor(address _VRFCoodinator, address _LinkToken, bytes32 _keyhash) public 
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override{
        ...
        address paintOwner = requestIdToSender[requestId];
        string memory tokenURI = requestIdToTokenURI[requestId];
        uint256 newItemId = tokenCounter;
        _safeMint(paintOwner, newItemId);
        _setTokenURI(newItemId, tokenURI);
        Paint paint = Paint(randomNumber % 3);
        tokenIdToPaint[newItemId] = paint;
    ...
    function setTokenURI(uint256 tokenId, string memory _tokenURI) public {
        require(
            ...
            "ECR721: Transfer caller is not owner not approved"
        );
