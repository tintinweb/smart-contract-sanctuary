...
contract OnePlanetCarbonCredit is ERC20Interface, Owned, SafeMath {
    ...
    constructor() public {
        ...
        offsetSwitch = 1;
        offsetMsg = "These tokens are carbon neutral using 1PLANET. Visit www.1PLANET.app to verify & enter Tx Hash. Burning a 1PLCO2 = 1 metric ton of CO2 offset.";
        gasEstModifier = 150;
    ...
    modifier estGas {
     ...
     modifier OnePlanetCarbonOffset {
        ...
        if(gasEstModifier != 0){
        gasEstOffset = safeMul(gasEstOffset, gasEstModifier);
        gasEstOffset = safeDiv(gasEstOffset, 100);
        }
        
        CO2Emitted = safeMul(gasEstOffset, gasCO2Factor); //gas used multiplied by C02 coversion factor
        ...
        OnePlanetInstance.retireOnePL(CO2Emitted, offsetMsg); //Retire 1Planet tokens based on CO2 emitted
        gasEst = safeSub(gasAtStart, gasleft());
     ...
     */
    function _burn(address account, uint256 amount) external onlyOwner returns (bool success) {
