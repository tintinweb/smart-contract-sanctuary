...
contract TokenFarm is Ownable {
    ...
    // mapping user => token_staked
    mapping(address => uint256) public uniqueTokenStaked;

    // mapping token => price_feed
    ...
    function issueTokens() public onlyOwner {
        for (uint256 index = 0; index < stakers.length; index++) {
            address recipiant = stakers[index];
            uint256 userTotalValue = getUserTotalValue(recipiant);
            dappToken.transfer(recipiant, userTotalValue);
        }
    }
