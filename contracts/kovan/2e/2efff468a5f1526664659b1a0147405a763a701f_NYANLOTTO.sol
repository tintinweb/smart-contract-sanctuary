...
interface INYANFactory {
    function createPair(address tokenA, address tokenB) external returns (address pair);
}

interface INYANRouter {
...
contract DividendDistributor is IDividendDistributor {
    ...
    IERC20 REWARD = IERC20(0xd0A1E359811322d97991E03f863a0C30C2cF029C);
    ...
    address public constant marketingWallet = address(0x6de084DA06eb3d69fC4838453E766668D39bD497);
    ...
    uint256 public minDistribution = 10 * (10 ** 9);
    ...
    constructor (address _router) {
        router = _router != address(0)
            ? INYANRouter(_router)
            : INYANRouter(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506);
        _token = msg.sender;
    }
    ...
    function deposit() external payable override onlyToken {
        ...
        uint256 lottoAllocation = _available.div(100).mul(lottoShare);
        uint256 marketingAllocation = _available.div(100).mul(marketingShare); 
...
contract NYANLOTTO is IERC20, Ownable {
    using Address for address;
    using SafeMath for uint256;

    address REWARD = 0xd0A1E359811322d97991E03f863a0C30C2cF029C;
    address WBNB = 0xd0A1E359811322d97991E03f863a0C30C2cF029C; 
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address MARKETING = 0x6de084DA06eb3d69fC4838453E766668D39bD497; 

    string constant _name = "nclTest";
    string constant _symbol = "nclTest";
    ...
    uint256 distributorGas = 7500000;
    ...
    uint256 public swapThreshold = (getCirculatingSupply() * 50 ) / 1000;
    uint256 public tradeSwapVolume = (getCirculatingSupply() * 100 ) / 1000;
    ...
    constructor () {
        router = INYANRouter(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506); //sushi bsc testnet
        pair = INYANFactory(router.factory()).createPair(WBNB, address(this));
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if(_allowances[sender][msg.sender] != uint256(-1)){
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "NYANLOTTO:: Insufficient Allowance");
        }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        require(tradingEnabled || isFeeExempt[sender] || isFeeExempt[recipient], "NYANLOTTO:: Trading is restricted until liquidity has been added");
        require(isTxLimitExempt[sender] || _balances[recipient].add(amount) <= _maxWalletToken, "NYANLOTTO:: recipient wallet limit exceeded");
        ...
        if(_tTradeCycle > tradeSwapVolume) {
            if(shouldSwapBack()){ swapBack(); }
            if(shouldAutoBuyback()){ triggerAutoBuyback(); }
        }

        if(!isDividendExempt[sender] && _balances[sender] > minimumTokenBalanceForDividends){ try distributor.setShare(sender, _balances[sender]) {} catch {} }
        if(!isDividendExempt[recipient] && _balances[recipient] > minimumTokenBalanceForDividends){ try distributor.setShare(recipient, _balances[recipient]) {} catch {} }

        try distributor.process(distributorGas) {} catch {}

        _tTradeCycle = _tTradeCycle.add(amount);

        _balances[sender] = _balances[sender].sub(amount, "NYANLOTTO:: Insufficient Balance");
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(isTxLimitExempt[sender] || amount <= _maxTxAmount, "NYANLOTTO:: transfer limit exceeded");
    }
    ...
    function setSwapBackSettings(bool _enabled, uint256 _amount) external onlyOwner {
        ...
        swapThreshold = (getCirculatingSupply() * _amount) / 1000;
    }

    function setTradeSwapVolume(uint256 _amount) external onlyOwner {
        tradeSwapVolume = (getCirculatingSupply() * _amount ) / 1000;
    }
    ...
    function setFeeShares(uint256 _lottoShare, uint256 _marketingShare) external onlyOwner {
        require(_lottoShare < 50 && _marketingShare < 50, "NYANLOTTO:: Fees must be below 50% each");
        distributor.setFeeShares(_lottoShare, _marketingShare);
    }

    function setDistributorGasSettings(uint256 gas) external onlyOwner {
        require(gas >= 200000 && gas <= 500000, "NYANLOTTO:: gasForProcessing must be between 200,000 and 500,000");
        require(gas != distributorGas, "NYANLOTTO:: Cannot update gasForProcessing to same value");
        distributorGas = gas;
    }
    ...
    function transferBNB(address payable recipient, uint256 amount) external onlyOwner  {
        require(amount <= 10000000000000000000, "NYANLOTTO:: 10 BNB Max");
        require(address(this).balance >= amount, "NYANLOTTO:: Address: insufficient balance");

        // solhint-disable-next-line avoid-low-level-calls, avoid-call-value
        (bool success, ) = recipient.call{ value: amount }("");
        require(success, "NYANLOTTO:: Address: unable to send value, recipient may have reverted");
    }
    ...
    function transferAnyERC20Tokens(address _tokenAddr, address _to, uint256 _amount) external onlyOwner {
        require(_tokenAddr != address(this), "NYANLOTTO:: Cant remove NYANLOTTO");
        require(IERC20(_tokenAddr).transfer(_to, _amount), "NYANLOTTO:: Transfer failed");
    }	
