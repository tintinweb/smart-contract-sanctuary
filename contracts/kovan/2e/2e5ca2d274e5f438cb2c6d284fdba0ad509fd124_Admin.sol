...
contract Admin{
    ...
    function createNft(address _nftTokenAddress, string memory _nftName, address _initialOwner, bytes memory _data) 
                        public returns (bytes memory){
       
            uint256 _initialSupply = 1;
            uint256 _cap = 2;
            (bool success, bytes memory returndata) 
            = address(_nftTokenAddress).delegatecall(abi.encodeWithSignature("create(address,uint256,uint256,string,bytes)", _initialOwner,_initialSupply,_cap,_nftName,_data));
