...
library DssExecLib {
    ...
    */
    ...
    //             addReaderToMedianWhitelist(address(OracleLike_2(co.pip).src()), co.pip);
    //         }
...
interface DAITokenBridgeLike {
    function escrow() external returns (address);
    function l1Token() external returns (address);
    function l2DAITokenBridge() external returns (address);
}
...
contract DssSpellAction is DssAction {
    ...
    address constant newL1DAITokenBridge = 0x0eCaC8370CdD58c9052cb45C64463f53143a3b4e;
    address constant l1GovRelay = 0x675383242Dbc07C8e130393037aa4C40cb06e1F3;
    address constant l2Spell = 0x83756F2F75329BE6Ce1a8B74d9F4931cBc1018D0;
    uint32 constant l2Gas = 1000000;

    function actions() public override {
        address escrow = DAITokenBridgeLike(newL1DAITokenBridge).escrow();
        address dai = DssExecLib.dai();

        // ---------------------------- Approve new bridge to spent DAI from escrow  ----------------------------
        EscrowLike(escrow).approve(dai, newL1DAITokenBridge, type(uint256).max);

        // ---------------------------- Send xchain message to L2 to approve new bridge counterpart to mint DAI  ----------------------------
        bytes memory _l2Data = abi.encodeWithSelector(L2KovanUpgradeSpellLike.upgradeBridge.selector);
        GovernanceRelayLike(l1GovRelay).relay(l2Spell, _l2Data, l2Gas);
    }
}
