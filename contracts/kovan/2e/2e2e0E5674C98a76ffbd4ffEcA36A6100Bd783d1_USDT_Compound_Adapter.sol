...
contract USDT_Compound_Adapter is IFinanceAdapter {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    address public governance;                    // Governance address
    IFinancePool public finance_pool;             // FinancePool that owns this adapter
    ...
    constructor(address _finance_pool, address CErc20_contract_address, address base_asset_address) {
        finance_pool = IFinancePool(_finance_pool);
        governance   = msg.sender; // finance_pool.get_governance();
    ...
    function deploy_capital(uint256 amount) external override {
        require(msg.sender == address(finance_pool), "must be pool");
