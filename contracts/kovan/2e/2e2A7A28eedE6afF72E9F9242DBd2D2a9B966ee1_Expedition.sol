        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                ...
                for (uint256 index = 0; index < rewardDebtHolders.length; index++) {
                    ...
                    uint256 indexToUpdate = rewardDebtHolders[index];
                    user.stakeInfo[indexToUpdate].rewardDebt = user.stakeInfo[indexToUpdate].amount.mul(pool.accTokensPerShare).div(1e12);
                }
            }
        }
        }
