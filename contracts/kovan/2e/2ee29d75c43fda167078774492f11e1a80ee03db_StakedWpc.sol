...
contract ERC20WithSnapshot is ERC20 {
    ...
    ITransferHook public _wpcGovernance;

    event SnapshotDone(address owner, uint128 oldValue, uint128 newValue);

    constructor(string memory name, string memory symbol, uint8 decimals) public ERC20(name, symbol, decimals) {}

    function _setWpcGovernance(ITransferHook wpcGovernance) internal virtual {
        _wpcGovernance = wpcGovernance;
    }
    ...
    */
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override {
        ...
        ITransferHook aaveGovernance = _wpcGovernance;
...
contract StakedToken is IStakedWpc, ERC20WithSnapshot, VersionedInitializable, DistributionManager {
  ...
  constructor(
    ...
    string memory symbol,
    uint8 decimals
  ) public ERC20WithSnapshot(name, symbol, decimals) DistributionManager(emissionManager, distributionDuration) {
   ...
   **/
  function initialize(ITransferHook wpcGovernance, string calldata name, string calldata symbol, uint8 decimals) external initializer {
    _setName(name);
    _setSymbol(symbol);
    _setDecimals(decimals);
    _setWpcGovernance(wpcGovernance);
  }
...
contract StakedWpc is StakedToken {
    string internal constant NAME = 'Staked WPC';
    string internal constant SYMBOL = 'stkWPC';
