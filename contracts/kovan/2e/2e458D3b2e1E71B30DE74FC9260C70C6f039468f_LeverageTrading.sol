...
contract LeverageTrading is LeverageMarginCall, IMarginTrading {
    ...
    function setAssetCap(address token, uint256 cap)
        external
        onlyOwnerExecActivator
    {
        tokenCaps[token] = cap;
    ...
    function setDepositLock(uint256 blocks) external onlyOwnerExec {
        depositLockUp = blocks;
    }

    /// @dev admin function to set borrow percent
    function setLoanPercent(uint256 _borrowPercent)
        external
        onlyOwnerExec
    {
    ...
    function setLiquidationLevel(uint256 percent)
        external
        onlyOwnerExec
    {
    ...
    function recordMarginCall(address trader) external override {
        require(isLeverageTrader(msg.sender), "Calling contr. not authorized");
        CrossMarginAccount storage account = marginAccounts[trader];
    ...
    function viewBalanceInAsset(address trader, address token)
        external
        view
        returns (uint256)
    {
    ...
    function viewSecuredLoan(address trader)
        external
        view
    ...
    /// @dev total of assets of account, expressed in reference currency
    function viewSecuredHoldings(address trader) external view returns (uint256) {
        CrossMarginAccount storage account = marginAccounts[trader];
