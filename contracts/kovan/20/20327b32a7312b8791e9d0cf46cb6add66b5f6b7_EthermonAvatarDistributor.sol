...
contract EthermonAvatarDistributor is AccessControl {
    ...
    function mintUsingEMON(uint256 _num) public payable returns (bool) {
        require(address(0) == upgradedToAddress, "Contract has been upgraded to a new address");
        ...
        emonToken.safeTransferFrom(msg.sender, address(this), (tokenPriceEMON * _num) );
    ...
    function togglePauseEMON(bool _pause) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Caller is not admin");
        require(_mintingUsingEMONPaused != _pause, "Already in desired pause state");

        _mintingUsingEMONPaused = _pause;
    }

    function toggleOnlyMintingByTokenHolders(bool _isRestricted) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || hasRole(TOGGLE_MINTING_ROLE, msg.sender), "Caller is not toggler or admin");
