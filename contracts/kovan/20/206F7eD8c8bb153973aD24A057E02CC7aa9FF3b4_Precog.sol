...
contract Precog {
    ...
    mapping(address => uint256) totalTradingAmount;

    mapping(address => mapping(address => uint256)) tradingAmount;
    mapping(address => mapping(address => uint256)) revenueAmount;
    mapping(address => mapping(address => bool)) isLiquidated;
    ...
    function getLiquidateInfo(address user, address futureContract) external view returns (bool isLiquidate, bool isTradeClose) {
        isLiquidate = isLiquidated[user][futureContract];
    ...
    function withdrawLiquidate(address futureContract, address to, address user) external {
        ...
        uint amount = revenueAmount[user][futureContract] - feeWithdrawByUSDC;
        IERC20(usdc).transfer(to, amount);
        IERC20(usdc).transfer(admin, feeWithdrawByUSDC);
    ...
    function reinvest(address futureContract, address user) external {
        require(!isTradeClosed[user][futureContract], "PrecogV2: TRADE_CLOSED");
        availableAmount[msg.sender] += revenueAmount[user][futureContract];
        isTradeClosed[user][futureContract] = true;
        _addTradingUser();
    }

    function liquidate(address futureContract, address user) external {
        require(isLiquidated[user][futureContract], "PrecogV2: ALREADY_LIQUIDATED");
        ...
        uint liquidateAmount = revenueAmount[user][futureContract];
        IFutureExchangeRouter(futureExchange).closeFuture(tokenInvest, usdc, expiryDate, address(this), liquidateAmount);
        _updateLiquidateAmount(user, futureContract);
        uint debt = _repayLoan(user, futureContract);
        uint actualProfit = revenueAmount[user][futureContract] - tradingAmount[user][futureContract] - debt;
