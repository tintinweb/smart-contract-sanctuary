 ...
 * 
 * MAPPY COIN DISTRIBUTION
 * 
 * - 10% for Team & Advisors
 * - 5% for Token Sale
 * - 5% for the Liquidity
 * - 80% for Farms & Pools
 */
...
contract Mappy is Context, IERC20, Ownable, Pausable {
    ...
    address private constant ROUTER = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D; // Pancakeswap Router
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        uint256 antiDumpMultiplier = _getAntiDumpMultiplier(isSellTxn);
        uint256 tFee = calculateTaxFee(tAmount).mul(antiDumpMultiplier);
        uint256 tBuyback = calculateBuybackFee(tAmount).mul(antiDumpMultiplier);
    ...
    function _transfer(
        ...
        // Sell txns
        if (to == pcsV2Pair && from != address(pcsV2Router) && !_isExcludedFromFee[to]) {
          // Enable anti dump feature
          isSellTxn = true;
        }
    ...
    function _getAntiDumpMultiplier(bool isSell) private view returns (uint256) {
        uint256 time_since_start = block.timestamp - tradingTime;
        uint256 hour = 60 * 60;
