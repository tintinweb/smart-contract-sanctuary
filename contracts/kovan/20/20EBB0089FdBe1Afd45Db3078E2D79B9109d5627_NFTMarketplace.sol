...
interface ICyclopsTokens {
  function mint(address to, uint256 tokenId, string calldata uri) external returns (bool);
...
contract NFTMarketplace is Ownable {
  ...
  */
  ...
  IERC20 internal _etnaToken; //ETNA
   ...
   */
  constructor(address newNftContractAddress, address newERC20Address) {
    ...
    _etnaToken = IERC20(newERC20Address);       //ETNA interface
  }
  ...
  */
  function setPaymentToken (address newERC20Contract) external onlyOwner returns (bool) {
    ...
    _etnaToken = IERC20(newERC20Contract);
    return true;
  }

  function getPaymentToken() external view returns (address) {
    return address(_etnaToken);
  }
  ...
  */
  ...
  function replaceNFTProfileAtId(
    ...
    _nftProfileIdsByUrl[keccak256(bytes(nftProfiles[index].url))] = 0;
    _nftProfileIdsByUrl[keccak256(bytes(url))] = profileID;
 ...
 /**
  ...
  function sellNFTBack (uint256 tokenId) external notContract returns (bool) { //'sell' == burn, burns and returns ETNA to user
    ...
    uint32 profileID = _nftProfileIdsByTokenId[tokenId];
    require(profileID > 0, "NFT profile ID not found");
    uint256 index = _nftProfileIndexes[profileID] - 1;
    uint256 sellPrice = nftProfiles[index].sell_price;
    
    require(_etnaToken.balanceOf(address(this)) >= sellPrice, "Insufficient ETNA on contract");

    delete _nftProfileIdsByTokenId[_nextTokenId];
    nftProfiles[index].limit ++;
    _nftProfileMinted[profileID] --;
    ...
    require(_etnaToken.transfer(msg.sender,  sellPrice), 'ETNA payment failed');
    return true;
  }
  ...
  function adminMint (   // mint for free as admin
    ...
    _nftProfileMinted[profileID] ++;
    _nftProfileIdsByTokenId[_nextTokenId] = profileID;
    _nftContract.mint(to, _nextTokenId, nftProfiles[index].url);
    emit AdminMinted(profileID, _nextTokenId, to, curPrice);
  
    return _nextTokenId; //success, return generated tokenId (works if called by another contract)
  }
  ...
  function adminBurn (uint256 tokenId) external  onlyOwnerOrPriceManager
    ...
    uint256 sellPrice = nftProfiles[index].sell_price;
    _nftProfileMinted[profileID] --;
    delete _nftProfileIdsByTokenId[tokenId];
    _nftContract.burn(tokenId);
  ...
  function getProfileIdByTokenId(uint256 tokenId) external view returns(uint32) {
    require(_nftProfileIdsByTokenId[tokenId] > 0, 'Profile for this tokenId not found');
    return _nftProfileIdsByTokenId[tokenId] - 1;
  }
  ...
  function getTokenPriceByTokenId (uint256 tokenId) public view returns(uint256) {
    require(_nftProfileIdsByTokenId[tokenId] > 0, 'Profile for this tokenId not found');
    return nftProfiles[_nftProfileIdsByTokenId[tokenId] - 1].sell_price;
  }
  ...
  */
  function getContractTokensBalance() external view returns (uint256) {
    return _etnaToken.balanceOf(address(this));
  }

  function depositTokens(uint256 amount) external onlyOwner returns (bool) {
    require(amount > 0, "Amount should be greater than zero");
    require(_etnaToken.transferFrom(msg.sender, address(this), amount),
      'Etna payment failed');
  ...
  function withdrawTokens(address to, uint256 amount) external onlyOwner
    ...
    require(amount > 0, "Amount should be greater than zero");
    require(to != address(0), 'Receiver address should not be zero');
    require(_etnaToken.balanceOf(address(this)) >= amount,
      'Amount can not be greater than contract balance');
    require(_etnaToken.transfer(to, amount), 'ETNA payment failed');
