    ...
    ReentrancyGuard
{
    ...
    /// fees.
    function poke(int24 tickLower, int24 tickUpper)
        public
        override
        onlyPeriphery
    {
    ...
    function burnLiquidityShare(
        ...
        if (liquidity > 0) {
            (
                ...
                uint256 fees0,
                uint256 fees1
            ) = burnAndCollect(tickLower, tickUpper, _toUint128(liquidity));
    ...
    function performOptimalSwap(
        ...
        uint256 amount0,
        uint256 amount1
    ) internal returns (bool isToken0Excess) {
        // Calculate optimal amounts for swap
        uint256 ratio = actualAmount0.mul(100).div(actualAmount1);
        (uint160 sqrtPriceX96, , , , , , ) = pool.slot0();
        uint256 price = uint256(sqrtPriceX96).mul(uint256(sqrtPriceX96)).mul(
            10**(uint256(token1.decimals()).sub(uint256(token0.decimals())))
        ) >> (96 * 2);
        price = price.mul(100);

        if (amount0.sub(actualAmount0) < amount1.sub(actualAmount1)) {
            //amount0 got used completely
            //amount1 is still left
            uint256 excessAmount = amount1.sub(actualAmount1);
            uint256 swapAmount1 = uint256(excessAmount).mul(ratio).div(
                price.add(ratio)
            );

            swapTokensFromPool(false, 0, swapAmount1);
            isToken0Excess = false;
        } else {
            uint256 excessAmount = amount0.sub(actualAmount0);
            uint256 swapAmount0 = uint256(excessAmount).mul(price).div(
                price.add(ratio)
            );
            swapTokensFromPool(true, 0, swapAmount0);
            isToken0Excess = true;
        }
    }
    ...
    function mintOptimalLiquidity(
        ...
        uint256 token0BeforeSwap = getBalance0();
        uint256 token1BeforeSwap = getBalance1();

        bool isToken0Excess = performOptimalSwap(
            actualAmount0,
            actualAmount1,
            amount0,
            amount1
        ...
        uint256 mintAmount0 = isToken0Excess
            ? getBalance0().sub(token0BeforeSwap)
            : token0BeforeSwap.sub(getBalance0());
        uint256 mintAmount1 = isToken0Excess
            ? getBalance1().sub(token1BeforeSwap)
            : token1BeforeSwap.sub(getBalance1());
    ...
    function mintLiquidity(
        ...
        int24 tickUpper,
        uint128 liquidity
    ) public override onlyPeriphery checkRange(tickLower, tickUpper) {
        if (liquidity > 0) {
            pool.mint(address(this), tickLower, tickUpper, liquidity, "");
        }
    }

    function autoCompound() public override onlyPeriphery {
    ...
    /// @param sqrtPriceLimitX96 Minimum price limit for swap after accounting for slippage
    /// @param amountIn Desired amount of token0 or token1 wished to swap
    function swapTokensFromPool(
        bool direction,
        uint160 sqrtPriceLimitX96,
        uint256 amountIn
    ) public override nonReentrant onlyPeriphery {
        pool.swap(
            address(this),
            direction,
            int256(amountIn),
    ...
    modifier onlyGovernance() {
        require(
            ...
            "Vault : onlyGovernance :: tx sender should be governance"
        );
        _;
    }
    ...
    modifier onlyStrategy() {
        require(
            ...
            "Vault : onlyStrategy :: tx sender should be either the strategy manager or periphery"
        );
        require(
            !pauseStrategy,
            "Vault : onlyStrategy :: operations paused for strategy manager"
        );
        _;
    }
    ...
    modifier onlyUser() {
        require(
            !pauseUser,
            "Vault : onlyUser :: operations paused for vault users"
        );
        _;
    }
    ...
    modifier checkRange(int24 tickLower, int24 tickUpper) {
