...
function insertOrderV2Buy(uint amount, address[] memory paths, uint expectInSize) external onlyTraders returns(uint) {
    ...
    swapPath[0] = paths[0];
    swapPath[1] = paths[1];
    amounts = V2Router.swapTokensForExactTokens(amounts, expectInSize, swapPath, address(this), deadline)[0];
    
    emit InsertOrderV2(amounts, amount, paths[0],paths[paths.length-1]);
    return amounts;
}
