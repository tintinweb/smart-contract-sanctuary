    ...
    SuperAppBase
{
    ...
    mapping(uint256 => uint96) flowRates;
    uint256 public nextId; // maybe replace by Counters.Counter?

    uint96 private _testFlowRate;
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(this) || from == address(0)) {
          ...
          _reduceFlow(from, flowRates[tokenId]);
          _increaseFlow(to, flowRates[tokenId]);
        }
    ...
    function _reduceFlow(address from, uint96 flowRate) internal {

    }

    function _increaseFlow(address to, uint96 flowRate) internal {
    ...
    function _createFlow(address to, uint96 flowRate) internal {
    ...
    function setFlowRate(uint96 flowRate) public returns (uint96 newFlowRate) {
