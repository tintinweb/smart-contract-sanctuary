...
contract StakingFactory is AccessControl, ReentrancyGuard, IStakingFactory {
    ...
    event RewardAdded(uint256 totalReward, uint256 currentTimestamp);
    ...
    modifier isInitialize {
        require(_isInitialized, "Not initialized yet");
        _;
    }
    ...
    /**
     * @notice Deposit token to stake
     * @param token: address of token which must have stake
     * @param amount: number of tokens
     */
    function deposit(address token, uint256 amount) external isInitialize nonReentrant {
        require(_isAllowedTokenCheck(token), "Wrong token");
        require(pools[token] != address(0), "Staking is not created yet");
        address sender = _msgSender();
        if (IStaking(pools[token]).deposit(amount, sender)) {
            users[sender].push(pools[token]);
        }
    ...
    /**
     * @notice Withdraw token from stake
     * @param token: address of token which must have stake
     * @param amount: number of tokens
     */
    function withdraw(address token, uint256 amount) external isInitialize nonReentrant {
        require(_isAllowedTokenCheck(token), "Wrong token");
        require(pools[token] != address(0), "Staking is not created yet");
        address sender = _msgSender();
        require(amount <= IStaking(pools[token]).getUserBalance(sender), "Not enough balance");

        if (IStaking(pools[token]).withdraw(amount, sender)) {
            for (uint256 i = 0; i < users[sender].length; i++) {
                if (users[sender][i] == pools[token]) {
        ...
        AssetLib.safeTransfer(token, sender, amount);
        emit Withdraw(token, sender, amount);
    }
     ...
     * @param rewards: {address token, amount of reward}
     */
    function inputReward(Reward[] calldata rewards)
        external
        onlyManagerOrAdmin
        isInitialize
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < rewards.length; i++) {
            IStaking(rewards[i].token).addReward(rewards[i].amount);
            totalReward += rewards[i].amount;
        }
        AssetLib.safeTransferFrom(YDR_TOKEN, _msgSender(), totalReward);
        emit RewardAdded(totalReward, block.timestamp);
    }

    /**
     * @notice Request all reward for user
     */
    function claimAllReward() external isInitialize nonReentrant {
        address sender = _msgSender();
        uint256 total;
        for (uint256 i = 0; i < users[sender].length; i++) {
            total += _claimReward(sender, users[sender][i]);
        }
        if (total > 0) {
            AssetLib.safeTransfer(YDR_TOKEN, sender, total);
        }
    }

    /**
     * @notice Get reward by stake index
     * @param index: index of stake
     */
    function claimReward(uint256 index) external isInitialize nonReentrant {
        require(index < stakes.length, "Wrong index");
        address sender = _msgSender();
        uint256 amount = _claimReward(sender, stakes[index]);

        if (amount > 0) {
            AssetLib.safeTransfer(YDR_TOKEN, sender, amount);
        }
    }

    /**
     * @notice Change time that user must wait before first reward
     * @param _freezeTime: time in seconds
     */
    function changeFreezeTime(uint256 _freezeTime) external onlyManagerOrAdmin {
    ...
    /**
     * @notice Get available to claim
     * @param sender: address of user
     * @param index: index of stake
     */
    function getAvailToClaim(address sender, uint256 index) external view returns (uint256) {
        require(index < stakes.length, "Not valid index");
        return IStaking(stakes[index]).availDividends(sender);
    }

    /**
     * @notice Get user balance
     * @param sender: address of user
     * @param index: index of stakes
     */
    function getUserBalance(address sender, uint256 index) external view returns (uint256) {
        require(index < stakes.length, "Not valid index");
        return IStaking(stakes[index]).getUserBalance(sender);
    }
     ...
     * @param index: index of stake
     */
    function getTotalStaked(uint256 index) external view returns (uint256) {
        require(index < stakes.length, "Not valid index");
        return IStaking(stakes[index]).totalStaked();
    }

    /**
     * @notice Internal claim reward
     * @param sender: address of user
     * @param stake: address of stake
     */
    function _claimReward(address sender, address stake) private returns (uint256 amount) {
        amount = IStaking(stake).claimDividends(sender);
    }
