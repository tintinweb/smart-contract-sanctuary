...
contract PPSwap is ERC20Interface, SafeMath{
    ...
    address public constant wethAddress = 0xfb0b52f3a2325ed3876eCe4b5280a08855f09A23;
 ...
 function PPSPrice() 
    ...
    function acceptOffer(uint offerID, uint wethAmt)
                        external
                        returns(bool)
                        {
        require(offerStatus[offerID] == 3, 'This order has been either canceled or filled.');
        ...
        uint buyAmt = safeMul(price, wethAmt);
        require(buyAmt <= maxbuy, "The amount you buy exceeds the buy limit.");
        
        acceptOfferImp(accountA, accountB, tokenA, wethAddress, buyAmt, wethAmt);
        emit AcceptOffer(offerID, accountA, accountB, wethAmt);
    ...
    function acceptOfferImp(address accountA, 
        ...
        require(B.balanceOf(accountB) >= wethAmt, "Not enough of weth balance in accontB.");
        require(B.allowance(accountB, address(this)) >= wethAmt, "Not enough alllowance of tokenB for this contract from accountB. ");
        B.transferFrom(accountB, accountA, safeDiv(safeMul(wethAmt, 95), 100));
        B.transferFrom(accountB, trustAccount, safeDiv(safeMul(wethAmt, 5), 100)); // 5 perncent of sale tax 
        return true;
    }
