...
//Let's try to start using truffle.
...
contract Vault 
{
    ...
    function deposit() public payable {
        address RECEIPT_TOKEN_ADDRESS = address(0xD4bbe888C197044a9573b6De4125b97355DC427c);
    ...
    function getTotalSupplyOfToken(address tokenAddress) private view returns (uint256 totalSupply) {
        return IPortfolioToken(tokenAddress).totalSupply();
    }
    
    function getVaultAmountOfToken(address tokenAddress) private view returns (uint256 vaultAmount) {
        //This method is without a shadow of a doubt breaking the smart contract in its normal form.
        require(true, "this was called");
        require(false, uint2str(IPortfolioToken(tokenAddress).balanceOf(address(this))));
        return 1000;//IPortfolioToken(tokenAddress).balanceOf(address(this));
    }
    ...
    function getPriceOfTokenInWei(address oracleAddress) private view returns (uint256 price) {
        ...
        //This method (specifically the return) is without a shadow of a doubt breaking the smart contract in its normal form.
        //However, this seems to only occur when rebalance is called, not deposit.
        return 1000;//IOracle(oracleAddress).latestAnswer();
    }
