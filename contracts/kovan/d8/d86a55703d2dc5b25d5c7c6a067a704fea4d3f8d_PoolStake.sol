...
contract PoolStake is Owned {
    ...
    event MigratedToLQDY(address indexed sender, address multiplier);
    event FeeReceiverUpdated(address oldFeeReceiver, address newFeeReceiver);
    event NewUToken(address indexed sender, address uToken2);
    ...
    constructor(address _token, address _Owner) public Owned(_Owner) {
        ...
        require(_lpToken != address(0x0), "Pair must first be created on uniswap");
        lpToken = IERC20(_lpToken);
        
        uToken1 = 0x3D95E5625AA194BB291D31A9ced44F4e811C06ae;
        platformWallet = 0x538f14c190ba4B81A6f3CfBfD8dE470e5293ba3A;
        multiplier = Multiplier(0x5B921C16ceE27972B8Ec4Ac027b6D48C85fA7F56);
        uniswapRouter = UniswapV2Router(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); 
    }

    function newUToken(address _uToken2) external returns(bool) {
        
        require(msg.sender == platformWallet, "only wallet can call");
        require(uToken2 == address(0x0), "already migrated to LQDY");
        require(_uToken2 != address(0x0), "cannot set the zero address");
        
        uToken2 = _uToken2;
        
        emit NewUToken(msg.sender, _uToken2);
        return true;
    }
    
    function completeUTokenMerge(address _multiplier) external returns(bool) {
        
        require(msg.sender == platformWallet, "only wallet can call");
        require(uToken2 != address(0x0), "must first set uToken2");
        require(!migratedToLQDY, "already migrated to LQDY");
        require(Address.isContract(_multiplier), "multiplier must be a contract");
        
        migratedToLQDY = true;
        uToken1 = uToken2;
        multiplier = Multiplier(_multiplier);
        
        emit MigratedToLQDY(msg.sender, uToken2);
        return true;
    }
    
    function changeFeeReceiver(address _feeReceiver) external returns(bool) {
    ...
    function _withMultiplier(
        ...
        uint multipliedETH = _proportion(multiplier.balance(msg.sender), uToken1, address(weth));
     ...
     * returns whether successfully withdrawn or not.
     */  
    function withdrawUserBonus(uint _asset) public returns(bool) {
        
        if(uToken2 == address(0x0)) require(_asset == 1, "currently accepts only uToken1");
        ...
        if (releasedToken > 0 && releasedETH > 0) {
            ...
            _withdrawUserTokenBonus(msg.sender, releasedToken, _asset);
            _withdrawUserETHBonus(msg.sender, releasedETH, _asset);
     ...
     * returns whether successfully withdrawn or not.
     */
    function _withdrawUserETHBonus(address payable _user, uint releasedAmount, uint _asset) internal returns(bool) {
        ...
        if(_asset == 1) require(IERC20(uToken1).transferFrom(_user, platformWallet, fee), "must approve fee");
     ...
     * returns whether successfully withdrawn or not.
     */
    function _withdrawUserTokenBonus(address _user, uint releasedAmount, uint _asset) internal returns(bool) {
     ...
     */ 
    function _calculateTokenFee(uint _amount) internal view returns(uint uToken1Fee, uint tokenFee) {
        
        uint fee = (_amount.mul(10)).div(100);
        uint feeInETH = _proportion(fee, address(token), address(weth));
        uint feeInUtoken1 = _proportion(feeInETH, address(weth), address(uToken1));  
        
        return (feeInUtoken1, fee);
     ...
     */ 
    function _calculateETHFee(uint _amount) internal view returns(uint uToken1Fee, uint ethFee) {
        
        uint fee = (_amount.mul(10)).div(100);
        uint feeInUtoken1 = _proportion(fee, address(weth), address(uToken1));  
    ...
    function calculateETHBonusFee(address _user) external view returns(uint ETH_Fee) {
        ...
        if (wethReleased > 0) {
            ...
            (uint feeForWethInUtoken1,) = _calculateETHFee(wethReleased);
            
            return feeForWethInUtoken1;
    ...
    function calculateTokenBonusFee(address _user) external view returns(uint token_Fee) {
        ...
        if (tokenReleased > 0) {
            ...
            (uint feeForTokenInUtoken1,) = _calculateTokenFee(tokenReleased);
            
            return feeForTokenInUtoken1;
