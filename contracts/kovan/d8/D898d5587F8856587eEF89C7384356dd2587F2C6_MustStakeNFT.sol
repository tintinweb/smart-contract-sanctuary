...
contract MuskTokenWrapper {
    using SafeMath for uint256;
    IERC20 public muskToken;

    constructor(IERC20 _muskToken) public {
        muskToken = IERC20(_muskToken);
    }
    ...
    function stake(uint256 amount) public virtual {
        ...
        muskToken.transferFrom(msg.sender, address(this), amount);
    }

    function withdraw(uint256 amount) public virtual {
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = _balances[msg.sender].sub(amount);
        muskToken.transfer(msg.sender, amount);
    }
}
...
import "./MuskTokenWrapper.sol";

contract Constant {
    uint256 public constant ZOOM_MUSK = 10**18;
    uint256 public constant ZOOM_TESLA = 10**18;
}

contract MustStakeNFT is MuskTokenWrapper, Ownable, Constant {
    ERC1155Tradable public muskNFTs;
    mapping(address => uint256) public lastUpdateTime;
    mapping(address => uint256) public teslas;
    mapping(uint256 => uint256) public cards;

    event CardAdded(uint256 card, uint256 teslas);
    ...
    modifier isValidAmount(uint256 _amount) {
        require(
            _amount.add(balanceOf(msg.sender)) >= 100 * ZOOM_MUSK,
            "Oops! Min-amount-is-100-Musk"
        );
        require(
            _amount.add(balanceOf(msg.sender)) <= 1000 * ZOOM_MUSK,
            "Oops! Max-amount-is-1000-Musk"
        );
        _;
    }

    constructor(ERC1155Tradable _muskNFTs, IERC20 _muskToken)
        public
        MuskTokenWrapper(_muskToken)
    {
        muskNFTs = _muskNFTs;
    }

    // NFTs token ID, amount tesla to redeem
    function addCard(uint256 cardId, uint256 amount) public onlyOwner {
        cards[cardId] = amount;
        emit CardAdded(cardId, amount);
    }

    // 100 MUSK staked will earn 1 teslas perday
    function earned(address account) public view returns (uint256) {
        uint256 blockTime = block.timestamp;
        return
            teslas[account].add(
                blockTime.sub(lastUpdateTime[account]).mul(1e18).div(86400).mul(
                    balanceOf(account).div(100 * ZOOM_TESLA)
                )
            );
    }

    // stake visibility is public as overriding MuskTokenWrapper's stake() function
    ...
    function redeem(uint256 card) public updateReward(msg.sender) {
        ...
        require(
            teslas[msg.sender] >= cards[card],
            "Not enough teslas to redeem for card"
        );
        require(
            muskNFTs.totalSupply(card) < muskNFTs.maxSupply(card),
            "Max cards minted"
        );
        teslas[msg.sender] = teslas[msg.sender].sub(cards[card]);
        muskNFTs.mint(msg.sender, card, 1, "");
