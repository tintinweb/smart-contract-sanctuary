...
contract KPIToken is Initializable, ERC20Upgradeable, IKPIToken {
    ...
    function finalize(uint256 _result) external override onlyOracle {
        if (finalized) revert(); // TODO: add custom error
        if (_result <= scalarData.lowerBound) {
            ...
            finalKpiProgress = _result >= scalarData.higherBound
                ? _kpiFullRange
                : _result - scalarData.lowerBound;
