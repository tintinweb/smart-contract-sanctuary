...
contract TokenFarm is Ownable {
    ...
    mapping(address => string) private _images;
    mapping(address => mapping(address => uint256)) private _balances;
    //mapping(address => mapping(address => mapping(address => uint256))) private _allowances;

    event Staked(address indexed token, address indexed owner, uint256 value);
    event Unstaked(address indexed token, address indexed owner, uint256 value);
    ...
    constructor(address token,string memory image) public {
        _token = token;
        _tokens.push(token);
        _images[token] = image;
    }
    ...
    function approve(address token, address spender, uint256 amount) public returns (bool) {
        _approve(token,_msgSender(), spender, amount);
        return true;
    }
    function increaseAllowance(address token, address spender, uint256 addedValue) public returns (bool) {
        _approve(token, _msgSender(), spender, _allowances[token][_msgSender()][spender] + addedValue);
        return true;
    }
    function decreaseAllowance(address token, address spender, uint256 subtractedValue) public returns (bool) {
        uint256 currentAllowance = _allowances[token][_msgSender()][spender];
        require(currentAllowance >= subtractedValue, "ERC20: decreased allowance below zero");
        unchecked {
            _approve(token,_msgSender(), spender, currentAllowance - subtractedValue);
        }
    ...
    function transfer(address token, address recipient, uint256 amount) public returns (bool) {
        _transfer(token,_msgSender(), recipient, amount);
        return true;
    }
    function transferFrom(
        ...
        uint256 currentAllowance = _allowances[token][sender][_msgSender()];
        require(currentAllowance >= amount, "ERC20: transfer amount exceeds allowance");
        unchecked {
            _approve(token, sender, _msgSender(), currentAllowance - amount);
        }
    ...
    function _transfer(
        ...
        _beforeTokenTransfer(token,sender, recipient, amount);

        uint256 senderBalance = _balances[token][sender];
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance");
        unchecked {
            _balances[token][sender] = senderBalance - amount;
        }
        _balances[token][recipient] += amount;

        emit Transfer(token, sender, recipient, amount);

        _afterTokenTransfer(token, sender, recipient, amount);
    }
    */

    function add(address token,string memory image) public onlyOwner {
        require(token != _token, "Farm: can add Farm Token");
        _tokens.push(token);
        _images[token] = image;
    }
    function remove(address token) public onlyOwner {
        require(token != _token, "Farm: can remove Farm Token");
    }
    function image(address token) public view returns ( string memory) {
    ...
    function Balance(address token,address account) public view returns (uint256) {
        return _balances[token][account];
    }

   function stake(address _token, uint256 _amount) public {
        require(_amount > 0, "amount cannot be 0");
        require(support(_token) , "Token currently isn't allowed");
        IERC20(_token).transferFrom(msg.sender, address(this), _amount);
        _balances[_token][msg.sender] += _amount;
        emit Staked(_token, msg.sender, _amount);
    }

    function unstake(address _token, uint256 _amount) public {
        require(_amount > 0, "amount cannot be 0");
        require(support(_token) , "Token currently isn't allowed");
        uint256 balance = _balances[_token][msg.sender];
        if (_amount > balance) _amount = balance;
        IERC20(_token).transfer(msg.sender, _amount);
        _balances[_token][msg.sender] -= _amount;
        emit Unstaked(_token, msg.sender, _amount);
    }
}
