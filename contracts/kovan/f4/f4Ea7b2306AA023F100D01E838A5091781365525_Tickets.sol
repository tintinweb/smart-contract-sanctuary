...
contract Tickets is ERC721URIStorage, VRFConsumerBase, Ownable {
    ...
    uint256 public ethTracker; // tracks contract balance
    uint256 public hardCap = 500 ether; // hard cap after which no more buys should be possible
    uint256 public endTime; // in case hard cap is never reached this would allow contract to continue after enough time has passed

    // Track the tokenId to their owner
     ...
     * @dev Most not have hit hard cap of 500 eth and all tokens have the same URI.
     * @param _baseURI URI Json data
     * @return tokenId
     */
    function claimTicket(string memory _baseURI)
        public
        payable
        returns (uint256)
    {
        ...
        _setTokenURI(tokenIds, _baseURI);

        return tokenIds;
    }

    /**
     * @dev External function to finialize the tickets.
    ...
    function rollDice() public onlyOwner returns (bytes32) {
        ...
        require(block.timestamp >= endTime, "Tickets: Not time to roll.");

        require(ethTracker >= hardCap, "Tickets: Tickets have sold out.");
