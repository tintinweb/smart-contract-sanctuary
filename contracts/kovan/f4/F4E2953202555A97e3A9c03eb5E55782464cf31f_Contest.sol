...
contract Contest{
    ...
    function beatTheKing() external payable{
        require(msg.value > kingsStake, "not enough value to beat the King");
        //returning eth back to the previous king
        (bool sent,) = theKing.call{value: kingsStake}("");
        require(sent, "fail sending eth back");

        kingsStake = msg.value;
        theKing = msg.sender;
    }

    // FUNCTION FOR CHECKER

    function checkTask() external payable onlyChecker{
        //require(msg.value > kingsStake, "not enough value to beat the King");
        (bool sent,) = theKing.call{value: kingsStake}("");
        require(sent, "fail sending eth back");
