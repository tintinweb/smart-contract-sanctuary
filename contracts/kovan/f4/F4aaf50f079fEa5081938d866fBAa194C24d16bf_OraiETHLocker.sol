...
contract SignData {
    ...
    constructor() public {
        ...
        LOCK_TYPE_HASH = keccak256("Data(address ethAddress, uint256 amount, uint256 deadline, uint256 nonce)");
...
contract OraiETHLocker is Ownable, SignData {
    ...
    function unlockOraiPermit(address[] memory receives, uint256[] memory amounts, uint256 ethEpoch, uint256 deadline, address sender, uint8 v, bytes32 r, bytes32 s) external {
        ...
        _unlockOrai(receives, amounts, ethEpoch, deadline, sender);
    }

    function unlockOrai(address[] memory receives, uint256[] memory amounts, uint256 ethEpoch, uint256 deadline) external {
        _unlockOrai(receives, amounts, ethEpoch, deadline, msg.sender);
    }

    function _unlockOrai(address[] memory receives, uint256[] memory amounts, uint256 ethEpoch, uint256 deadline, address sender) internal {
        require(unlockedRoles[sender], "Forbidden");
