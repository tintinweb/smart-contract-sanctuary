...
contract Trader is ITrader, TraderManager, TraderRelaysHandler, TraderPools, Initializable {
  ...
  function follow(address trader_) external override onlyManager {
    ...
    require(trader_ != address(this), "This contract instance cannot follow itself");
    require(trader_ != followedTrader, "You are already following this address");
    _follow(trader_);
  }

  /// @inheritdoc ITrader
  function setTradingStrategy(ITradingStrategy strategy_) external override onlyManager {
  ...
  function withdrawFromRelayPool(PoolCharge calldata withdrawal_) external override onlyManager {
  ...
  function relay(
    ...
    uint256 weiToBeRefunded = weiSpent.add(weiSpent.div(RELAYER_FEE_BASE).mul(relayerFee));
    uint256 refundAmount = PricesLib.tokenAmountFromWei(refundAsset_, weiToBeRefunded);
  ...
  function _setTradingStrategy(ITradingStrategy strategy_) internal {
    emit TradingStrategyChange(address(tradingStrategy), address(strategy_));
...
abstract contract TraderManager is ITrader {
  ...
  // TODO add signed approvals?.

  /**
   * @dev Allows external address to spend contract's tokens.
   */
  function setAllowance(AllowanceToSet calldata allowance_) external onlyManager {
   ...
   */
  function setAllowances(AllowanceToSet[] calldata allowances_) external onlyManager {
...
abstract contract TraderRelaysHandler {
  ...
  uint256 public constant AFTER_RELAY_ERC20_TRANSFER_GAS_USAGE_APPROXIMATION = 60000;
  uint256 public constant AFTER_RELAY_ETH_TRANSFER_GAS_USAGE_APPROXIMATION = 21000;
  ...
  function _relay(
    ...
    (bool signatureOk, bytes32 txHash) =
      _isRLPSignatureCorrect(transaction_, txSigV_, txSigR_, txSigS_, correctSigner_);

    require(signatureOk, "CopyTrader:_relay, invalid signature");
    require(!relayedTxns[txHash], "CopyTrader:_relay, transaction already relayed");
    ...
    require(eip155tx.chainId == chainId, "CopyTrader:_relay, invalid sig chainId");

    require(eip155tx.nonce >= relaySinceNonce, "CopyTrader:_relay, invalid nonce");

    bytes4 methodSignature = AbiUtils.extractMethodSignature(eip155tx.data);

    require(
      followedStrategy_.manipulators(eip155tx.to, methodSignature) != address(0),
      "CopyTrader:_relay, relayed tx.data format is not supported by strategy"
    );

    bytes memory abiManipulated =
      IABIManipulator(followedStrategy_.manipulators(eip155tx.to, methodSignature)).manipulate(
        eip155tx.data,
        address(this)
      );
...
// TODO? Add possibility to pull tokens not from msg sender but given address.
abstract contract TraderPools is ITrader {
  ...
  function _withdrawFromRelayPool(PoolCharge memory withdrawal_) internal {
    ...
    if (withdrawal_.asset == address(0)) {
      ...
      require(IERC20(withdrawal_.asset).transfer(msg.sender, withdrawal_.value));
    }
    emit PoolWithdrawn(withdrawal_, Pool.RELAY);
  }

  function _withdrawFromOperationPool(PoolCharge memory withdrawal_) internal {
    require(this.poolSize(Pool.OPERATIONS, withdrawal_.asset) >= withdrawal_.value);
  ...
  function _handleMultipleCharges(PoolCharge[] memory charges_, Pool[] memory chargedPools_)
    internal
  {
  ...
  function poolSize(Pool pool_, address asset_) external view override returns (uint256) {
    return pool_ == Pool.RELAY ? relayPools[asset_] : _balanceOf(asset_).sub(relayPools[asset_]);
  }
}
