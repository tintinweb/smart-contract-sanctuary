...
abstract contract MintLever is IMintLever, Lock, AddressCheck {
    ...
    mapping(address => mapping(address => uint256)) public bondsV3;//user-token-pamount
    ...
    modifier checkLiquidator(address user) {
    ...
    function redeem(address user, address token, uint256 amount) internal returns (uint256) {
        if (bondsV3[user][token] < amount) {
            amount = bondsV3[user][token];
        }
        if (0 < amount) {
            uint256 code;
            bondsV3[user][token] = bondsV3[user][token].sub(amount);
    ...
    function redeemUnderlying(address user, address token, uint256 amount) internal returns (uint256) {
        if (0 < amount) {
            ...
            bondsV3[user][token] = bondsV3[user][token].sub(pAmount);
        }
        return amount;
    }
    
    function getLoadPublicsRate(address user, address token) internal view returns (uint256) {
        (uint256 code, , , uint256 rate) = config.getLoanPublics(token).getAccountSnapshot(user, id, ILoanTypeBase.LoanType.MINNING_SWAP_PROTOCOL);
        require(0 == code, "publics:loan_publics_get_account_snapshot_error");
        return rate;
    }
    
    function getBondAmount(address user, address token) internal view returns (uint256) {
        return bondsV3[user][token].mul(getLoadPublicsRate(user, token));
    }
...
contract MintLeverDodoSingle is MintLever {
    ...
    function directClearingPosition(address user, uint256 deadLine) override external lock checkLiquidator(user) {
    ...
    function indirectClearingPosition(address user, address token, uint256 amount, uint256 deadLine) override external lock checkLiquidator(user) {
        ...
        if (capital == token) {
            ...
            usdToken = getUSD(token, getBondAmount(user, token), 18);
        }
        ...
        if (usdToken.mul(10000) <= usdBorrow.mul(clearingEarning)) {
            ...
            amount = usdToken.mul(10000).mul(10 ** tokenDecimal).div(clearingEarning).div(price);
            if (capital == token) {
                price = capitalAmounts[user];//amount->price
                capitalAmounts[user] = 0;
                price = IEarning(earning).unStake(user, price);
            }else {
                price = bondsV3[user][token];//amount->price
                bondsV3[user][token] = 0;
            }
        }else {
            if (capital == token) {
                ...
                price = usdBorrow.mul(clearingEarning).mul(10 ** tokenDecimal).div(price).div(10000);
                price = price.div(getLoadPublicsRate(user, token));
                if (bondsV3[user][token] < price) {
                    price = bondsV3[user][token];
                }
                bondsV3[user][token] = bondsV3[user][token].sub(price);
            }
        }
