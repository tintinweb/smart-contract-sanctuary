...
contract Pool is Ownable {
  ...
  */
    ...
    function buyTickets(uint256 _count) public {
        ...
        deposit(totalDeposit / 10**12, address(token), msgSender());
    }
...
*/
  ...
  */
    function depositNoChecks(uint256 amount, address asset) external {
        IERC20(asset).transferFrom(msgSender(), address(this), amount);
        ...
        lendingPool.deposit(asset, amount, msgSender(), 0);
    }
    ...
    function deposit(
        ...
        ILendingPool(lendingPool).deposit(asset, amount, wallet, 0);
    }
 ...
 */
    ...
    /**
     * @notice Allows the user to withdraw their funds from the pool
     * @param asset : Address of the underlying token
     */

    function withdraw(address asset) public {
        IProtocolDataProvider dataProvider =
            IProtocolDataProvider(address(protocolDataProvider));

        (address aTokenAddress, , ) =
            dataProvider.getReserveTokensAddresses(asset);
        uint256 assetBalance = IERC20(aTokenAddress).balanceOf(msgSender());
        ...
        lendingPool.withdraw(asset, assetBalance, msgSender());
    }
