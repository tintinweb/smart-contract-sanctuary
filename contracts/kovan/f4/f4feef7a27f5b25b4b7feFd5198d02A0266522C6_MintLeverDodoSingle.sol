...
contract MintLeverDodoSingle is MintLever {
    ...
    function closePosition(uint256 percentage, address receiveToken, uint256 deadLine) override external lock {
        ...
        if (address(0) == receiveToken) {
            _safeTransfer(_userInfo.tokenA, _user, _amountA);
        }else {
            _amount = _swap(_userInfo.tokenA, receiveToken, _amountA, deadLine);
            _safeTransfer(receiveToken, _user, _amount);
        }
    ...
    function directClearingPosition(address user, uint256 deadLine) override external lock {
        ...
        _safeTransfer(_userInfo.borrowToken, config.getPlatformFee(), _clearingPlatformEarning);
        _safeTransfer(_userInfo.borrowToken, msg.sender, _clearingEarning);
        _safeTransfer(_userInfo.borrowToken, user, _surplusAmount);
    ...
    function indirectClearingPosition(address user, address token, uint256 amount) override external lock {//nonZeroAmount(amount) {
        ...
        if (_usdToken.mul(10000) <= _usdBorrow.mul(_clearingEarning)) {
            ...
            amount = _usdToken.mul(10000).mul(10 ** IERC20(_userInfo.borrowToken).decimals()).div(_clearingEarning).div(_price);
            if (_userInfo.capital == token) {
                ...
                _safeTransfer(_userInfo.capital, msg.sender, _price);
            }else {
            ...
            if (_userInfo.capital == token) {
                ...
                _price = _usdBorrow.mul(_clearingEarning).mul(10 ** IERC20(token).decimals()).div(_price).div(10000);
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            if (0 < _amountV1) {
                ...
                if (_repay) {
                    ...
                    _safeTransfer(_token, config.getPlatformFee(), _amountV2);
                    _safeTransfer(_token, user, _amountV1);
                    _amountV2 = 0;
                }
            }
        }
