...
import "./OnePlanetOffset.sol";

contract AdvancedCollectible is ERC721, VRFConsumerBase, OnePlanetConnected{
    ...
    function createCollectible(string memory tokenURI, uint256 userProvidedSeed)
        public OnePlanetCarbonOffset returns (bytes32){
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal OnePlanetCarbonOffset override {
    ...
    function transferFrom(address from, address to, uint256 tokenId) public OnePlanetCarbonOffsetMinGas override {
...
contract Owned {
    ...
    constructor() public {
        owner = 0x667872db262A9deaf39FE47fafa99fBB8c2Ce960;
    }
...
abstract contract OnePlanetConnected is Owned{
    using SafeMath for uint256;
    //OnePlanet State Variables
    address public OnePlanetAddress;
	uint256 public gasCO2factor;
    uint256 public minGasEstModifier1PL;
	uint256 public gasEstModifier1PL;
    uint256 public gasEstModifierETH;
    uint256 public transferCounter;
    uint256 public counterStop;
    ...
    constructor() public {
        ...
        OnePlanetAddress = 0x0400A10034d8A75316e5daa0A0070A24B580d06d;
        ...
        gasEstModifier1PL = 100000;
        gasEstModifierETH = 150000;
        counterStop = 1;
    }
   //OnePlanet Modifiers
    modifier OnePlanetCarbonOffset {                    
        ...
        if(offsetSwitch == 1 && OnePlanetInstance.balanceOf(address(this)) >= 1e17 && transferCounter < counterStop){
        
            uint256 gasAtStart = gasleft();
            transferCounter = transferCounter.add(1);
            _;
            uint256 gasEst = gasAtStart.sub(gasleft());
        
            offsetWith1PL(gasEst);  
        
        } else if (offsetSwitch == 1 && address(this).balance >= 1e17 && transferCounter < counterStop){
    ...
    // This CarbonOffset modifier has limited error handling and no offset message concatination to minimize gas usage.
    // Make sure you have plenty of 1PLCO2 tokens in order to not stop transactions
    modifier OnePlanetCarbonOffsetMinGas {
     ...
     function offsetWith1PL(uint256 gasEstOffset) internal {
        ...
        gasEstOffset = gasEstModifier1PL != 0 ? gasEstOffset.add(gasEstModifier1PL): 0;
        uint256 CO2offset = gasEstOffset.mul(gasCO2factor); //gas used multiplied by C02 coversion factor
        ...
        OnePlanetInstance.retireOnePL(CO2offset, offsetMsgBase); //Retire 1Planet tokens based on CO2 emitted
        }
    ...
    function offsetWithETH(uint256 gasEstOffset) internal {
        ...
        gasEstOffset = gasEstModifierETH != 0 ? gasEstOffset.add(gasEstModifierETH): 0;
        uint256 CO2offset = gasEstOffset.mul(gasCO2factor); //gas used multiplied by C02 coversion factor
        ...
        //OnePlanetInstance.offsetDirectWithETH.value(CO2offsetETH)(offsetMsgBase); //Retire 1Planet tokens based on CO2 emitted in ETH -----DEPRECATED
        OnePlanetInstance.offsetDirectWithETH{value: CO2offsetETH}(offsetMsgBase); ////Retire 1Planet tokens based on CO2 emitted in ETH
        }
    
    function offsetWith1PLminGas(uint256 gasEstOffset) internal {
        OnePlanetInterface OnePlanetInstance = OnePlanetInterface(OnePlanetAddress);
        
        gasEstOffset = gasEstOffset.add(minGasEstModifier1PL);
        uint256 CO2offset = gasEstOffset.mul(gasCO2factor); //gas used multiplied by C02 coversion factor

        require(OnePlanetInstance.balanceOf(address(this)) >= CO2offset, "Not enough 1PLCO2 tokens to retire"); //Require that the NFT minter has enough 1PLANET tokens
        
        OnePlanetInstance.retireOnePL(CO2offset, ""); //Retire 1Planet tokens based on CO2 emitted
        }
    ...
    function setGasEstModifier1PL(uint256 safetyFactor) external onlyOwner {
        gasEstModifier1PL = safetyFactor;
    }

    function setGasEstModifierETH(uint256 safetyFactor) external onlyOwner {
        gasEstModifierETH = safetyFactor;
    }
