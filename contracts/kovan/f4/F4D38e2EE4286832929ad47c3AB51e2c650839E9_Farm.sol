...
contract Farm is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. DPIs to distribute per block.
        uint256 lastRewardBlock; // Last block number that DPIs distribution occurs.
    ...
    // View function to see pending DPIs on frontend.
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 blockDifference = pool.lastRewardBlock.sub(block.number);
            uint256 dpiReward =
