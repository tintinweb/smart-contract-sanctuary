...
interface ICurvePool {
    function add_liquidity(uint256[3] memory _amounts, uint256 _amountOutMin) external;
...
contract CurveSBTCZap is Ownable, BaseRelayRecipient {
    ...
    function _addLiquidity(uint256 _amount, address _vault) private returns (uint256 _lpTokenBal) {
        ...
        uint256[3] memory _amounts = [0, _amountsOut[1], 0];
        curvePool.add_liquidity(_amounts, 0);
    ...
    /// @notice Function to add new Curve pool (limit to Curve SBTC pool only)
    ...
    function addPool(address vault_, address curvePool_) external onlyOwner {
        ...
        // _renBTC.safeApprove(curvePool_, type(uint).max);
        // _WBTC.safeApprove(curvePool_, type(uint).max);
        // _sBTC.safeApprove(curvePool_, type(uint).max);
