 ...
 */
contract CanonicalOrders is Ownable, OnlyDxln, IAutoTrader, ICallee {
    ...
    // bytes32 private constant FILECANONIC = "CanonicalOrders";
     ...
     */
    function getOrderInfo(bytes memory data)
        private
        returns (OrderInfo memory)
    {
        ...
        require(orderInfo.fill.price != 0, "FillArgs loaded price is zero");
    ...
    function verifySignature(OrderInfo memory orderInfo, bytes memory data)
        private
        view
    {
    ...
    function verifyOrderInfo(
        ...
        // verify taker
        require(takerAccount.owner == g_taker, "Order taker mismatch");
    ...
    function getOutputAssetAmount(
        ...
        uint256 fee = orderInfo.fill.price.getPartial(
            orderInfo.fill.fee,
            PRICE_BASE
        );
        uint256 adjustedPrice = (isBuy(orderInfo.order) ==
            orderInfo.fill.isNegativeFee)
     ...
     */
    function updateFilledAmount(OrderInfo memory orderInfo, uint256 fillAmount)
        private
    {
     ...
     */
    function getCurrentPrice(uint256 baseMarket, uint256 quoteMarket)
        private
        view
        returns (uint256)
    {
        Monetary.Price memory basePrice = DXLN_MARGIN.getMarketPrice(
            baseMarket
        );
        Monetary.Price memory quotePrice = DXLN_MARGIN.getMarketPrice(
            quoteMarket
        );
     ...
     */
    function isNegativeLimitFee(Order memory order)
        private
        pure
        returns (bool)
    {
