...
contract StopLimitOrders is Ownable, OnlyDxln, IAutoTrader, ICallee {
    ...
    // bytes32 private constant FILESTOP = "StopLimitOrders";

    // EIP191 header for EIP712 prefix
    bytes2 private constant EIP191_HEADER = 0x1901;

    // EIP712 Domain Name value
    string private constant EIP712_DOMAIN_NAME = "StopLimitOrders";

    // EIP712 Domain Version value
    string private constant EIP712_DOMAIN_VERSION = "1.1";
    ...
    uint256 private constant NUM_ORDER_BYTES = 384;
    ...
    uint256 private constant NUM_CALLFUNCTIONDATA_BYTES = 32 + NUM_ORDER_BYTES;
    ...
    event ContractStatusSet(bool operational);
    ...
    constructor(address dxlnMargin, uint256 chainId) OnlyDxln(dxlnMargin) {
     ...
     */
    function startUp() external onlyOwner {
        g_isOperational = true;
     ...
     */
    function approveOrder(Order memory order) public {
    ...
    function getTradeCost(
        ...
        require(g_isOperational, "Contract is not operational");
     ...
     */
    function cancelOrderInternal(address canceler, Order memory order) private {
        require(canceler == order.makerAccountOwner, "Canceler must be maker");
     ...
     */
    function approveOrderInternal(address approver, Order memory order)
        private
    {
        require(approver == order.makerAccountOwner, "Approver must be maker");
        bytes32 orderHash = getOrderHash(order);
        require(
            g_status[orderHash] != OrderStatus.Canceled,
    ...
    function verifyOrderAndAccountsAndMarkets(
        ...
        // verify expriy
        require(
            orderInfo.order.expiration == 0 ||
                orderInfo.order.expiration >= block.timestamp,
            "Order expired"
        );
        ...
        // verify inputWei
        require(!inputWei.isZero(), "InputWei is zero");
        require(
    ...
    function verifyDecreaseOnly(
        ...
        require(
            newInputPar.isZero() ||
                (newInputPar.value <= oldInputPar.value &&
                    newInputPar.sign == oldInputPar.sign),
            "inputMarket not decreased"
        );

        // verify that the balance of outputMarketId is not increased
        Types.Wei memory oldOutputWei = DXLN_MARGIN.getAccountWei(
            makerAccount,
            outputMarketId
        );
        require(
            assetAmount.value == 0 ||
                (assetAmount.value <= oldOutputWei.value &&
                    assetAmount.sign != oldOutputWei.sign),
    ...
    function updateMakerFilledAmount(
        ...
        uint256 totalMakerFilledAmount = oldMakerFilledAmount.add(
            makerFillAmount
        );
        require(
     ...
     */
    function getCurrentPrice(uint256 makerMarket, uint256 takerMarket)
        private
        view
        returns (uint256)
    {
        Monetary.Price memory takerPrice = DXLN_MARGIN.getMarketPrice(
            takerMarket
        );
        Monetary.Price memory makerPrice = DXLN_MARGIN.getMarketPrice(
            makerMarket
        );
     ...
     */
    function getOrderAndValidateSignature(bytes memory data)
        private
        view
        returns (OrderInfo memory)
    {
        ...
        if (orderStatus == OrderStatus.Null) {
            ...
            address signer = TypedSignature.recover(
                orderInfo.orderHash,
                signature
            );
            require(
            ...
            require(orderStatus != OrderStatus.Canceled, "Order canceled");
    ...
    function getOrderHash(Order memory order) private view returns (bytes32) {
        ...
        return
            keccak256(
                abi.encodePacked(EIP191_HEADER, EIP712_DOMAIN_HASH, structHash)
            );
    }
...
library TypedSignature {
     ...
     */
    function recover(bytes32 hash, bytes memory signatureWithType)
        internal
        pure
        returns (address)
    {
