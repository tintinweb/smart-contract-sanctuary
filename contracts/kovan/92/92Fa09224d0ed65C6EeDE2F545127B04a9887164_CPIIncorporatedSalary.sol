    ...
    KeeperCompatibleInterface
{
    ...
    string public minimumFundAmountPair;
    ...
    mapping(address => Company) public companyToRecentData;
    ...
    constructor(
        ...
        uint256 _fee,
        address _link,
        address _companyPreferedPaymentERC20
    )
    {
        if (_link == address(0)) {
        ...
        companyToRecentData[company].preferedPaymentToken = IERC20(
            _companyPreferedPaymentERC20
        );
    }
    ...
    /**
     * Chang the prefered payment token in order to save on comissions.
     */
    function setCompanyPreferedPaymentERC20(
        address _companyPreferedPaymentERC20
    ) public onlyOwner {
        companyToRecentData[company].preferedPaymentToken = IERC20(
            _companyPreferedPaymentERC20
     ...
     *  @param _priceFeedProxy Chainlink Ethereum Data Feeds.
     */
    ...
    function payEmployee() public returns (bool) {
        for (uint256 i = 0; i < employeesToPayLength; i++) {
            ...
            string memory pair = employeeToRecentData[employee].pair;
            IERC20 companyPaymentToken = companyToRecentData[company].preferedPaymentToken;
            IERC20 employeePaymentToken = employeeToRecentData[employee].preferedPaymentToken;
            uint256 salaryInToken = getConversionRate(
                pair, salaryInUSD
            );
            if (companyPaymentToken == employeePaymentToken) {
                // Paying.
                companyPaymentToken.approve(address(this), salaryInToken);
                companyPaymentToken.transferFrom(address(this), employee, salaryInToken);
                ...
                TokenSwap swap;
                swap.convertCompanyTokensToEmployeeTokens(
                    companyPaymentToken,
                    employeePaymentToken,
                    companyPaymentToken.balanceOf(address(this)),
                    companyPaymentToken.balanceOf(address(this)),
                    salaryInToken,
                    employee
                );
    ...
    function withdraw() public onlyOwner {
        employee = payable(employees[0]);
        IERC20 employeePaymentToken = employeeToRecentData[employee].preferedPaymentToken;
        // Paying.
        uint256 bal1 = employeePaymentToken.balanceOf(address(this));
        employeePaymentToken.approve(address(this), bal1);
        employeePaymentToken.transferFrom(address(this), employee, bal1);
        //require(msg.sender == onlyOwner, "You are not allowed to perform this operation.");
