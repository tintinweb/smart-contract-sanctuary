...
contract Precog {
                    ...
                    for (uint i = 0; i < exchanges.length; i++) {
                        ...
                        for (uint l = 0; l < loans.length; l++) {
                            ...
                            if (profit.amount > profitAmount) {
                                ...
                                tradeAmount = profit.tradeAmount;
                                trade = _trade;
        ...
        if (profit.expiryDate > block.timestamp) {
            ...
            if (pairs[0] != address(0)) {
                if (amount > fee.trading) {
                    ...
                    uint optimizeTradeAmount = _getOptimizeTradeAmount(profit.expiryDate, pairs, trade);
                    uint revenue;
                    if (optimizeTradeAmount <= tradeAmount){
                        revenue = _getRevenue(optimizeTradeAmount, profit.expiryDate, pairs, trade);
                    } else {
                        revenue = _getRevenue(tradeAmount, profit.expiryDate, pairs, trade);
                    }
                    if (revenue > amount) {
                        profit.amount = revenue - amount;
                        profit.tradeAmount = optimizeTradeAmount <= tradeAmount ? optimizeTradeAmount : tradeAmount;
                    }
        ...
        internal view returns (uint tradeAmount)
    {
        ...
        tradeAmount = (sqrt(reserveAExchange * reserveBExchange * reserveAFutureExchange * reserveBFutureExchange) - (reserveAExchange * reserveAFutureExchange)) / (reserveAFutureExchange + reserveBExchange);
    }
