...
contract AdvancedCollectible1PLOffset is ERC721, VRFConsumerBase, OnePlanetConnected{
...
contract OnePlanetConnected is StringUtils, Owned{
	...
	bool public checkGasFactor; // used to turn off or on the auto updating of the factor from 1PLANET contract
	bool public offsetSwitch; // used to turn off or on the function or choose the carbon offseting option;
   ...
   //OnePlanet Modifiers
    ...
    modifier OnePlanetCarbonOffset {
        if(offsetSwitch == false){
            _;
        } else {
        uint256 gasAtStart = gasleft();
        _;
        uint256 gasEstOffset = gasAtStart.sub(gasleft());

        OnePlanetInterface OnePlanetInstance = OnePlanetInterface(OnePlanetAddress);
        gasCO2factor = checkGasFactor ? OnePlanetInstance.gasCO2factor() : 0;
    ...
    modifier OnePlanetCarbonOffsetDirect {
        if(offsetSwitch == false){
            _;
        } else {
        uint256 gasAtStart = gasleft();
        ...
        OnePlanetInterface OnePlanetInstance = OnePlanetInterface(OnePlanetAddress);
        gasCO2factor = checkGasFactor ? OnePlanetInstance.gasCO2factor() : 0;
        uint256 ethPrice1PL = OnePlanetInstance.ethPrice1PL();
        gasEstOffset = gasEstModifier != 0 ? gasEstOffset.add(gasEstModifier): 0;
        uint256 CO2offset = gasEstOffset.mul(gasCO2factor); //gas used multiplied by C02 coversion factor
        uint256 CO2offsetETH = CO2offset.mul(ethPrice1PL);
    ...
    function setOffsetSwitch(bool offsetSwitchState) external onlyOwner {
        offsetSwitch = offsetSwitchState;
    }
    
    function setGasCO2factorUpdateSwitch(bool autoUpdateSwitchState) external onlyOwner {
