...
contract AavedaiAdapter is AdapterBase, Initializable {
    ...
    //function approvetodepositfiftydai() external {
        //_underlyingAsset.safeApprove(address(this), 50 ether);
    //    bool isApproved = _underlyingAsset.approve(address(this), 50 ether);
    ...
    //function depositfiftydai() external {
        //bool isApproved = _underlyingAsset.approve(address(this), 50 ether);
        //require(isApproved, "token not approve for transfer");
        //uint256 allowance =
        //    _underlyingAsset.allowance(msg.sender, address(this));
        //require(allowance >= 50 ether, "Check the token allowance");
        //_underlyingAsset.safeTransferFrom(msg.sender, address(this), 50 ether);
    //    _underlyingAsset.transferFrom(msg.sender, address(this), 50 ether);
    //}
    ...
    //    bool isTransferred = _underlyingAsset.transfer(to, 50 ether);
    //    require(isTransferred, "token transfer not successful");
    }
    ...
    //function depositUnderlyingAsset(uint256 amount) external override returns (bool success) {
    //    require(amount != 0, "AavedaiAdapter: can't add 0");
    //    uint256 allowance =
    //        _underlyingAsset.allowance(msg.sender, address(this));
    //    require(allowance >= amount, "Check the token allowance");

        //bool isTransferred = _underlyingAsset.transferFrom(msg.sender, address(this), amount);
        //require(isTransferred, "token deposit not successful");
    //    bool isTransferred = true;
    //    return isTransferred;
    //}

    //function redeemUnderlyingAsset(address to, uint256 amount) external override returns (bool success) {
    //    bool isApproved = IERC20(_underlyingAsset).transfer(to, amount);
    //    require(isApproved, "failed to redeem 50 Dai");
    //    return isApproved;
    //}
    ...
    function setFarmPoolAddress(address FarmPoolAddress) external {
        require(
            FarmPoolAddress != address(0),
            "TokenApproval: FarmingPool address is the zero address"
        );
        farmingPoolAddress = FarmPoolAddress;
    }
