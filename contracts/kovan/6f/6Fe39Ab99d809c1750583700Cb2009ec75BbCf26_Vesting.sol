...
contract Vesting is Initializable, PausableUpgradeable, BumperAccessControl {
    ...
    */
    ...
    struct Recipient {
        uint64 startTime; // timestamp at which vesting period will start
        uint64 endTime; // timestamp at which vesting period will end
        uint64 cliffDuration; // time duration after startTime before which the recipient cannot call claim
        uint64 lastPausedAt; // latest timestamp at which vesting was paused
    ...
    address public bumpTokenAddress; // vesting token address
    ...
    uint256 public ESCROW_PAUSED_AT; // timestamp at which escow paused.
    ...
    modifier escrowPaused() {
        require(paused(), "escrowNotPaused: unpaused");
        _;
    }

    modifier escrowNotPaused() {
        require(!paused(), "escrowNotPaused: paused");
        _;
    }
    ...
    function terminateVestingEscrow()
        external
        onlyGovernance
        escrowNotTerminated
    {
    ...
    /// @notice Pause the vesting escrow.
    /// @dev All the vesting states will be freezed, recipients can still claim their vested tokens.
    ///      Only owner of the vesting escrow can invoke this function.
    function pauseVestingEscrow()
        external
        onlyGovernance
        escrowNotTerminated
        escrowNotPaused
    {
        _pause();
        ESCROW_PAUSED_AT = block.timestamp;
    }
    ...
    /// @param _amountsEnc An array of amounts to be vested by the corresponding recipient addresses
    /// @param _totalAmount Total sum of the amounts in the _amounts array
    function addRecipients(
        address[] calldata _recipients,
        bytes calldata _amountsEnc,
        bytes calldata _timesEnc,
        uint256 _totalAmount
    ) external onlyGovernance escrowNotTerminated escrowNotPaused {
        // Every input should be of equal length (greater than 0)
        uint256[][] memory _amounts = abi.decode(
                _amountsEnc,
                (uint256[][])
            );
        uint64[][] memory _times = abi.decode(
                _timesEnc,
                (uint64[][])
            );
        require(
        ...
        IERC20(bumpTokenAddress).safeTransferFrom(
            msg.sender,
            address(this),
            _totalAmount
        );
        ...
        for (uint256 i = 0; i < _amounts.length; i++) {
            ...
            require(
                recipients[_recipientsAddress].endTime < block.timestamp,
                "addRecipients: recipient already registered"
            );
            require(
                recipients[_recipientsAddress].recipientVestingStatus !=
                    Status.Paused,
                "addRecipients: recipient already registered"
            );
            require(
                recipients[_recipientsAddress].totalClaimed ==
                    recipients[_recipientsAddress].totalVestingAmount,
                "addRecipients: recipient did not claim entire amount"
            );

            uint64 _startTime = _times[i][0];
            uint64 _endTime = _times[i][1];
            uint64 _cliffDuration = _times[i][2];

            uint256 _amount = _amounts[i][0];
            uint256 _onStartAmount = _amounts[i][1];
    ...
    ///      Only owner of the vesting escrow can invoke this function.
    ///      Can only be invoked if the escrow is NOT terminated.
    /// @param activeRecipients The array recipient address for which vesting will be paused.
    function pauseRecipients(address[] calldata activeRecipients)
        external
        onlyGovernance
        escrowNotTerminated
        escrowNotPaused
    {
        for (uint256 i = 0; i < activeRecipients.length; i++) {
            address recipientAddresss = activeRecipients[i];

            require(
                recipientAddresss != address(0),
                "pauseRecipient: 0 address"
            );
            require(
                recipients[recipientAddresss].recipientVestingStatus ==
                    Status.UnPaused,
                "pauseRecipient: recipient NOT in UnPaused state"
            );
            require(
                recipients[recipientAddresss].endTime > block.timestamp,
                "pauseRecipient: cannot pause inactive recipient"
            );

            // set vesting status of the recipient as Paused
            recipients[recipientAddresss].recipientVestingStatus = Status
                .Paused;
            // set lastPausedAt timestamp
            recipients[recipientAddresss].lastPausedAt = uint64(block.timestamp);
        }
    }
    ...
    ///      Only owner of the vesting escrow can invoke this function.
    ///      Can only be invoked if the escrow is NOT terminated.
    /// @param activeRecipients The recipient address for which vesting will be unpaused.
    function unPauseEscrow(address[] calldata activeRecipients)
        external
        onlyGovernance
        escrowNotTerminated
        escrowPaused
    {
        ...
        uint256 pausedFor = block.timestamp - ESCROW_PAUSED_AT;

        for (uint256 i = 0; i < activeRecipients.length; i++) {
            address recipientsAddress = activeRecipients[i];
            // recipient should NOT be a 0 address
            require(
                recipientsAddress != address(0),
                "unPauseEscrow: 0 address"
            );
            // current recipient status should be UnPaused
            require(
                recipients[recipientsAddress].recipientVestingStatus ==
                    Status.UnPaused,
                "unPauseEscrow: cannot paused"
            );

            // extend the cliffDuration by the pause duration
            recipients[recipientsAddress].cliffDuration =
                uint64(recipients[recipientsAddress].cliffDuration +
                pausedFor);
            // extend the endTime by the pause duration
            recipients[recipientsAddress].endTime =
                uint64(recipients[recipientsAddress].endTime +
                pausedFor);
        }
    ...
    ///      Only owner of the vesting escrow can invoke this function.
    ///      Can only be invoked if the escrow is NOT terminated.
    /// @param pausedRecipients The array recipient address for which vesting will be unpaused.
    function unPauseRecipients(address[] calldata pausedRecipients)
        external
        onlyGovernance
        escrowNotTerminated
        escrowNotPaused
    {
        for (uint256 i = 0; i < pausedRecipients.length; i++) {
            address recipientAddresss = pausedRecipients[i];
            ...
            // calculate the time for which the recipient was paused for
            uint256 pausedFor =
                block.timestamp - recipients[recipientAddresss].lastPausedAt;
            // extend the cliffDuration by the pause duration
            recipients[recipientAddresss].cliffDuration =
                uint64(recipients[recipientAddresss].cliffDuration +
                pausedFor);
            // extend the endTime by the pause duration
            recipients[recipientAddresss].endTime =
                uint64(recipients[recipientAddresss].endTime +
                pausedFor);
        }
    }
    ...
    ///      Only owner of the vesting escrow can invoke this function.
    ///      Can only be invoked if the escrow is NOT terminated.
    /// @param activeRecipients The array recipient address for which vesting will be terminated.
    function terminateRecipients(address[] calldata activeRecipients)
        external
        onlyGovernance
        escrowNotTerminated
    {
        for (uint256 i = 0; i < activeRecipients.length; i++) {
            ...
            require(
                recipientAddresss != address(0),
                "recipientIsNotTerminated: 0 address"
            );
            // recipient should NOT be in Terminated status
            require(
                recipients[recipientAddresss].recipientVestingStatus !=
            ...
            if (canClaim(recipientAddresss)) {
                // transfer unclaimed tokens to the recipient
                _claimFor(
                    getClaimableAmountFor(recipientAddresss),
                    recipientAddresss
                );
            }
            // transfer locked tokens to the SAFE_ADDRESS
            uint256 _bal =
                recipients[recipientAddresss].totalVestingAmount -
                    recipients[recipientAddresss].totalClaimed;
            IERC20(bumpTokenAddress).safeTransfer(SAFE_ADDRESS, _bal);
            // set vesting status of the recipient as "Terminated"
            recipients[recipientAddresss].recipientVestingStatus = Status
                .Terminated;
            recipients[recipientAddresss].startTime = 0;
            recipients[recipientAddresss].endTime = 0;
            recipients[recipientAddresss].cliffDuration = 0;
            recipients[recipientAddresss].lastPausedAt = 0;
            recipients[recipientAddresss].vestingPerSec = 0;
            recipients[recipientAddresss].totalVestingAmount = 0;
            recipients[recipientAddresss].onStartVestingAmount = 0;
            recipients[recipientAddresss].totalClaimed = 0;
        }
    }

    /// @notice Transfer all vesting tokens to the recipient
    ...
    function transferAllVestingTokenToRecipient(address recipient)
        external
        onlyGovernance
        escrowNotTerminated
        recipientIsNotTerminated(recipient)
    {
        ...
        if (recipients[recipient].endTime > timeStamp) {
            recipients[recipient].endTime = uint64(timeStamp);

            if (recipients[recipient].startTime > timeStamp) {
                recipients[recipient].startTime = uint64(timeStamp);
            }

            recipients[recipient].cliffDuration = 0;
            if (recipients[recipient].recipientVestingStatus == Status.Paused) {
                recipients[recipient].recipientVestingStatus = Status.UnPaused;
            }
        }

        // transfer unclaimed tokens to the recipient
        _claimFor(getClaimableAmountFor(recipient), recipient);
    }
    ...
    function _claimFor(uint256 _amount, address _recipient) internal {
        ...
        uint256 claimableAmount = getClaimableAmountFor(_recipient);
        ...
        IERC20(bumpTokenAddress).safeTransfer(_recipient, _amount);

        emit ClaimAmount(_recipient, _amount, recipient.totalClaimed);
    }
        ...
        returns (uint256)
    {
        ...
        // totalVested = totalClaimed + getClaimableAmountFor
        return _recipient.totalClaimed + getClaimableAmountFor(recipient);
    }
        ...
        returns (bool)
    {
        ...
        return
            block.timestamp >= _recipient.startTime + _recipient.cliffDuration;
    }
    ...
    function getClaimableAmountFor(address recipient)
        public
        view
        recipientIsNotTerminated(recipient)
        returns (uint256)
    ...
    function getTotalUnvestedTokensOf(address[] memory _recipients)
        public
        view
        returns (uint256 totalAmount)
    {
        ...
        returns (uint256)
    {
        ...
        (
            _durationLock,
            _onStartVestingAmountLock
        ) = getDurationLockAndOnStartVesting(_recipient);
        return
    ...
    function getDurationLockAndOnStartVesting(Recipient storage _recipient)
        internal
        view
        returns (uint256, uint256)
    {
        ...
        if (ESCROW_TERMINATED) {
            if (
                ...
                return (_recipient.endTime - ESCROW_TERMINATED_AT, 0);
            } else {
                return (
                    _recipient.endTime -
                        (_recipient.startTime + _recipient.cliffDuration),
        ...
        // in case escrow is paused, locked amount stays the constant
        if (paused()) {
            if (
                ESCROW_PAUSED_AT >
                _recipient.startTime + _recipient.cliffDuration
            ) {
                return (_recipient.endTime - ESCROW_PAUSED_AT, 0);
            } else {
                return (
                    _recipient.endTime -
                        (_recipient.startTime + _recipient.cliffDuration),
        ...
        if (_recipient.recipientVestingStatus == Status.Paused) {
            if (_recipient.lastPausedAt >= _recipient.endTime) {
                ...
                return (_recipient.endTime - _recipient.lastPausedAt, 0);
            } else {
                return (
                    _recipient.endTime -
                        (_recipient.startTime + _recipient.cliffDuration),
        ...
        if (block.timestamp >= _recipient.endTime) {
            ...
            return (_recipient.endTime - block.timestamp, 0);
        } else {
            return (
                _recipient.endTime -
                    (_recipient.startTime + _recipient.cliffDuration),
    ...
    function transferDust() external onlyGovernance returns (uint256) {
        ...
        if (dust > 0) {
            uint256 _dust = dust;
            dust = 0;
            IERC20(bumpTokenAddress).safeTransfer(SAFE_ADDRESS, _dust);
            return _dust;
        }
        return 0;
    }
        ...
        returns (uint256 totalSeized)
    {
        ...
        IERC20(bumpTokenAddress).safeTransfer(SAFE_ADDRESS, totalSeized);
    }
}
