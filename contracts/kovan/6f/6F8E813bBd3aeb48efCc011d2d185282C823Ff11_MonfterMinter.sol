...
contract MonfterMinter is Context, AccessControl, ReentrancyGuard {
    ...
    uint256 public MAX_FOR_PRESALE = 500;
    ...
    mapping(address => uint256) public whitlistMintLog;
    ...
    bytes32 public immutable communityMerkleRoot;
    ...
    function beforePreMint(
        ...
        if (role == Role.communityMember) {
            require(
                whitlistMintLog[_msgSender()].add(amount) <= 1,
                "MonfterMinter: already mint or invalid amount"
            );
            require(
                whitelistMint.add(amount) <= MAX_FOR_PRESALE,
            ...
            require(
                monHolderMint.add(amount) <= MAX_FOR_PRESALE &&
            ...
            require(
                nftHolderMint.add(amount) <= MAX_FOR_PRESALE &&
            ...
            require(
                earlyTraderMint.add(amount) <= MAX_FOR_PRESALE &&
                    traderMintLog[_msgSender()].add(amount) <=
                    PRE_MAX_PER_ADDRESS,
                "MonfterMinter: invalid amount"
            );
            // Verify the merkle proof.
    ...
    function afterPreMint(Role role, uint256 amount) internal {
        if (role == Role.communityMember) {
            ...
            whitlistMintLog[_msgSender()] += amount;
        } else if (role == Role.monHolder) {
    ...
    function pubMint(uint256 amount) public payable nonReentrant onlyStartPubMint {
        uint256 weiAmount = msg.value;
        require(
        ...
        require(
            monfterNft.totalSupply().add(amount) <= monfterNft._maxSupply(),
            "MonfterMinter: invalid amount"
        );
...
contract MonfterNFT is ERC721, AccessControl {
    ...
    constructor() ERC721("Monfter", "Monfter") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
