...
contract UniswapTWAP is IUniswapTWAP, Ownable {
    ...
    **/
    function getVaderEthSpotPrice() public view returns (uint256) {
        ...
        // TODO: need to multiply 1e18 before casting to uin256?
        uint spotPrice = pairData.isFirst ?
                uint256(FixedPoint.fraction(reserve1, reserve0)._x) :
                uint256(FixedPoint.fraction(reserve0, reserve1)._x);

        return spotPrice * ONE_VADER;
    }

    /**
    * @notice Return absolute value of |x - y|
    */
    function _abs(uint x, uint y) private pure returns (uint) {
        if (x >= y) {
            return x - y;
        }
        return y - x;
    ...
    **/
    function _updateVaderPrice() private {
        ...
        if (maxPriceDiff < MAX_PRICE_DIFF_DENOMINATOR) {
            ...
            require(
                _abs(twapPrice, spotPrice) * MAX_PRICE_DIFF_DENOMINATOR / twapPrice <= maxPriceDiff,
                "TWAP::updateVadePrice: price diff > max"
            );
        }
    }
    ...
    **/
    function _setMaxPriceDiff(uint256 _maxPriceDiff) private {
        require(_maxPriceDiff <= MAX_PRICE_DIFF_DENOMINATOR, "TWAP::setMaxPriceDiff: diff > max");
