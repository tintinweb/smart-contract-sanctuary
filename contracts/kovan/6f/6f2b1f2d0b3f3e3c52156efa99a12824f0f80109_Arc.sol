...
contract Arc is Context, IERC20, Ownable {
    ...
    uint256 public _exchangeFee = 50; // 100 = 1%
    uint256 public _stakingpoolFee = 25; // 100 = 1%
    uint256 public _charityFee = 100; // 100 = 1%
    uint256 public _burnFee = 200; // 100 = 1%
    
    uint256 _2percentTransaction = 20000000000000 * (10**18);
    uint256 _3percentTransaction = 30000000000000 * (10**18);
  
    uint256 public _taxFeeTotal;
    uint256 public _marketingFeeTotal;
    uint256 public _stakingFeeTotal;
    uint256 public _exchangeFeeTotal;
    uint256 public _charityFeeTotal;
    
    address public charityAddress = 0x80F4aea6A4390f5265A690B68E6E3f822F7879b2; 
    address public marketingAddress = 0x80F4aea6A4390f5265A690B68E6E3f822F7879b2; 
    address public exchangeAddress = 0x80F4aea6A4390f5265A690B68E6E3f822F7879b2; 
    address public stakingAddress = 0x80F4aea6A4390f5265A690B68E6E3f822F7879b2; 
    ...
    function excludeAccount(address account) external onlyOwner() {
        ...
        require(account != address(this), 'Arc: The contract it self cannot be excluded');
        require(!_isExcluded[account], "Arc: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "Arc: Account is already included");
    ...
    function FeeSetOne() internal
    {
    _taxFee = 200; // 100 = 1%
    _marketingFee = 100; // 100 = 1%
    _exchangeFee = 50; // 100 = 1%
    _stakingpoolFee = 25; // 100 = 1%
    _charityFee = 25; // 100 = 1%
    _burnFee = 200; // 100 = 1%
    }
    
    function FeeSetTwo() internal
    { 
    _taxFee = 833; // 100 = 1%
    _marketingFee = 416; // 100 = 1%
    _exchangeFee = 208; // 100 = 1%
    _stakingpoolFee = 104; // 100 = 1%
    _charityFee = 104; // 100 = 1%
    _burnFee = 833; // 100 = 1%
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        {
            require(amount < _3percentTransaction, "Amount exceeded");
        }
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        uint256 transferAmount = amount;
        if(amount >= _2percentTransaction && amount <= _3percentTransaction)
        {
            FeeSetOne();
        }
        else
        ...
        uint256 exchangeFee = amount.mul(_exchangeFee).div(10000);
        uint256 stakingpoolFee = amount.mul(_stakingpoolFee).div(10000);
        ...
        if(marketingFee > 0){
            ...
            _reflectionBalance[charityAddress] = _reflectionBalance[marketingAddress].add(marketingFee.mul(rate));
            _marketingFeeTotal = _marketingFeeTotal.add(marketingFee);
            emit Transfer(account,marketingAddress,marketingFee);
        }
        if(exchangeFee > 0){
            transferAmount = transferAmount.sub(exchangeFee);
            _reflectionBalance[exchangeAddress] = _reflectionBalance[exchangeAddress].add(exchangeFee.mul(rate));
            _exchangeFeeTotal = _exchangeFeeTotal.add(exchangeFee);
            emit Transfer(account,exchangeAddress, exchangeFee);
        }
        if(stakingpoolFee > 0){
            transferAmount = transferAmount.sub(stakingpoolFee);
            _reflectionBalance[stakingAddress ] = _reflectionBalance[stakingAddress].add(stakingpoolFee.mul(rate));
            _stakingFeeTotal = _stakingFeeTotal.add(stakingpoolFee);
            emit Transfer(account,stakingAddress,stakingpoolFee);
        }
