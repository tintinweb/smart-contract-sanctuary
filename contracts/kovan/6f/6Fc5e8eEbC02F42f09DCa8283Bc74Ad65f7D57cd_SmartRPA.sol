...
contract SmartRPA is ERC721, Ownable, ChainlinkClient {
    ...
    mapping(address => address payable) requestToAddress;
    ...
    // delete contract
    function terminateContract(uint256 tokenID) public buyerOrSeller {
        selfdestruct(requestToAddress[ownerOf(tokenID)]);  
    }
    ...
    function respondToOffer(uint256 tokenId, uint256 responseCode) public {
        ...
        if(responseCode==2) {   // if else statement
            ...
            buyer = requestToAddress[ownerOf(tokenId)];
