...
//    uint public constant PERIOD = 1 seconds * 2;
    ...
    event Harvest(uint256 pid, address receiver, uint256 amount);
    event UpToStandard(uint256 pid, uint256 blocks, uint256 currentMarketcap);
    event HarvestLiquidateReward(uint256 pid, address receiver, uint256 amount);
    event CurrentMarketCap(uint256 totalSupply,uint256 marketCap,uint256 averagePrice);
}
...
contract GameOfTrustBase is BaseStorage {
    ...
    function initialize(uint256 _startBlock,
        ...
        require(rewardToken == address(0) && depositToken == address(0), "Already initialized");
        require(_rewardToken != address(0) && _depositToken != address(0), "Invalid param");
    ...
    function updateMarketCap() external onlyAdmin {
        updatePriceAverage();
        checkMarketcaps();
    }
    ...
    function getStepFineRate() external view returns (uint256){
        uint256 spandDays = calculateSpandDays(stakeEndBlock, block.number, blocksPerDay);
        return judgeFineRate(spandDays);
    }
    ...
    function calculateMarketcap() internal returns (uint256 amountOut){
        ...
        emit CurrentMarketCap(totalSupply, amountOut, getAveragePrice());
    }
    ...
    function harvestUnlockReward(PoolInfo memory _pool, UserInfo storage _user,bool _mint) internal returns (uint256){
        ...
        if(_mint){
            IRewardToken(rewardToken).mint(msg.sender,pendingUnlockAmount);
        }else{
            IERC20(rewardToken).safeTransfer(msg.sender, pendingUnlockAmount);
        }
    ...
    function pendingUnlockHarvestReward(uint256 _pid, address _user) external view returns (uint256 pendingAmount, uint256 currentBlock){
    ...
    function calculateSurplusRewardToken(PoolInfo memory pool, uint256 _rate) internal view returns (uint256){
        return (pool.totalAmountLimit.sub(pool.totalAmount)).mul(_rate).div(totalRate);
    }

    function inStakePeriodWithdrawView(
        UserInfo memory _user,
        uint256 _amount,
        uint256 _stakeFineRate
    ) internal view returns (uint256, uint256, uint256){
        _amount = _amount >= _user.amount ? _user.amount : _amount;
        uint256 fine;
        uint256 withdrawAmount;
        (fine, withdrawAmount) = calculateWithdrawalInternal(_amount, _stakeFineRate);
        return (fine, 0, withdrawAmount);
    }

    function afterStakePeriodWithdrawView(
        UserInfo memory _user,
        uint256 _amount
    ) internal view returns (uint256, uint256, uint256){
        uint256 spandDays = calculateSpandDays(stakeEndBlock, block.number, blocksPerDay);
        ...
        uint256 fine;
        uint256 withdrawAmount;
        (fine, withdrawAmount) = calculateWithdrawalInternal(amount, fineRate);
        return (fine, 0, withdrawAmount);
    }

    function calculateWithdrawalInternal(
        uint256 _amount,
        uint256 _fineRate
    ) internal view returns (uint256 fine, uint256 finalQuantity){
        fine = _amount.mul(_fineRate).div(totalRate);
        finalQuantity = _amount.sub(fine);
    }

    function addPoolInternal(
        uint256 _marketCap,
        uint256 _rewardRate,
        uint256 _totalAmountLimit,
        uint256 _unlockCycleDays,
        address _depositToken
    ) internal {
        require(block.number < startBlock, "Activity has started");
        require(_unlockCycleDays > 0, "Must Greater than 0");
        ...
        emit AddPool(pid, _marketCap, _rewardRate, _unlockCycleDays.mul(blocksPerDay), _totalAmountLimit,
            startBlock,stakeEndBlock,blocksPerDay,_depositToken,rewardToken);
    }
...
// File: contracts/sashimi/GameOfTrustSashimiStorage.sol

pragma solidity ^0.6.12;


contract GameOfTrustSashimiStorage {

    uint256 public constant stakeFineRate = 0;
    address public constant  blackHole = 0x000000000000000000000000000000000000dEaD;
}

// File: contracts/sashimi/GameOfTrustSashimi.sol

pragma solidity ^0.6.12;


contract GameOfTrustSashimi is GameOfTrustBase, GameOfTrustSashimiStorage {
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
      depositInternal(_pid,_amount,depositToken);
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external {
        ...
        uint256 rewardToken;
        uint256 fine;
        uint256 lockdepositToken;
        if (user.amount == 0) {
            return;
        }
        if (pool.unlockBlock > 0) {
            //            unlock (withdraw all )
            (rewardToken, lockdepositToken, fine) = unlockedWithdraw(pool, user);
        } else {
            //            lock
            (rewardToken, lockdepositToken, fine) = lockedWithdraw(pool, user, _amount);
        }
        emit Withdraw(_pid, msg.sender, rewardToken, lockdepositToken, fine);
    }
    ...
    function harvestLiquidateReward(uint256 _pid) external {
        ...
        //        burn depositToken
        IERC20(depositToken).safeTransfer(blackHole, userInfo[_pid][msg.sender].amount);
        IRewardToken(rewardToken).mint(msg.sender, amount);
    ...
    function unlockedWithdraw(
        ...
        uint256 countBlock = block.number > endBlock ? endBlock : block.number;
        uint256 surplusBlock = _pool.unlockCycle.add(_pool.unlockBlock).sub(countBlock);
        uint256 fine = surplusBlock
        ...
        //      Not claimed unlock reward.
        uint256 finalQuantity = totalUnlock.sub(_user.rewardDebt);
        uint256 lockeddepositToken = _user.amount.mul(surplusBlock).div(_pool.unlockCycle);
        uint256 rewardrewardToken = finalQuantity.sub(lockeddepositToken);
        uint256 burndepositToken = _user.amount.sub(lockeddepositToken);
        _pool.totalAmount = _pool.totalAmount.sub(_user.amount);
        _pool.liquidateAmount = _pool.liquidateAmount.add(fine);
        _user.amount = 0;
        _user.rewardDebt = totalUnlock.sub(lockeddepositToken);
        IERC20(depositToken).safeTransfer(msg.sender, lockeddepositToken);
        IRewardToken(rewardToken).mint(msg.sender, rewardrewardToken);
        //        burn.
        IERC20(depositToken).safeTransfer(blackHole, burndepositToken);
        return (rewardrewardToken, lockeddepositToken, fine);
    }


    function lockedWithdraw(PoolInfo storage _pool,
        UserInfo storage _user,
        uint256 _amount) internal returns (uint256, uint256, uint256){
    ...
    function inStakePeriodWithdraw(
        PoolInfo storage _pool,
        UserInfo storage _user,
        uint256 _amount
    ) internal returns (uint256, uint256, uint256) {
    ...
    function withdrawToken(PoolInfo storage _pool,
        ...
        uint256 _amount,
        uint256 _fineRate,
        uint256 _potentialReward) internal returns (uint256, uint256, uint256){
        ...
        IERC20(depositToken).safeTransfer(msg.sender, finalQuantity);
        return (0, finalQuantity, fine.add(_potentialReward));
    }

    function afterStakePeriodWithdraw(
        PoolInfo storage _pool,
        UserInfo storage _user
    ) internal returns (uint256, uint256, uint256){
        uint256 spandDays = calculateSpandDays(stakeEndBlock, block.number, blocksPerDay);
    ...
    function pendingWithdraw(
        ...
        address _user,
        uint256 _amount
    ) external view returns (uint256 fine, uint256 reward, uint256 withdrawAmount){
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo memory user = userInfo[_pid][msg.sender];
        if (block.number >= startBlock) {
            if (pool.unlockBlock > 0) {
                uint256 endBlock = pool.unlockBlock.add(pool.unlockCycle);
                (fine, reward, withdrawAmount) = unlockedWithdrawView(pool, user, endBlock);
            } else {
                (fine,,withdrawAmount) = lockedWithdrawInternal(user, _amount);
            }
            return (fine, reward, withdrawAmount);
        }
    }

    function unlockedWithdrawView(
        PoolInfo memory _pool,
        UserInfo memory _user,
        uint256 _endBlock
    ) internal view returns (uint256, uint256, uint256){
        uint256 countBlock = block.number > _endBlock ? _endBlock : block.number;
        uint256 surplusBlock = _endBlock.sub(countBlock);
        uint256 fine = surplusBlock
        .mul(_user.amount.mul(_pool.rewardRate))
        .div(totalRate.mul(_pool.unlockCycle));
        uint256 totalUnlock = _user.amount.mul(totalRate.add(_pool.rewardRate)).div(totalRate).sub(fine);
        ...
        uint256 lockeddepositToken = _user.amount.mul(surplusBlock).div(_pool.unlockCycle);
        uint256 rewardrewardToken = finalQuantity.sub(lockeddepositToken);
        return (fine, rewardrewardToken, lockeddepositToken);
    }

    function lockedWithdrawInternal(
        UserInfo memory _user,
        uint256 _amount
    ) internal view returns (uint256, uint256, uint256){
        if (block.number <= stakeEndBlock) {
            return inStakePeriodWithdrawView(_user, _amount, stakeFineRate);
        } else {
            return afterStakePeriodWithdrawView(_user, _amount);
        }
    }
    ...
    function harvest(uint256 _pid) external {
        ...
        uint256 unlockAmount = harvestUnlockReward(pool, user,true);
