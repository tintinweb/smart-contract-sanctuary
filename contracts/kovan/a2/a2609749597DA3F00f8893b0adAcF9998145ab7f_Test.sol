...
contract Test {
   ...
   function deposit(address _erc20Contract, uint256 _amount) public returns(bool success){
       ...
       IERC20(_erc20Contract).transferFrom(from, lendingPoolAddress, _amount);
       bool approve = IERC20(_erc20Contract).approve(lendingPoolAddress,_amount);
       ILendingPool(lendingPoolAddress).deposit(_erc20Contract, _amount, from,0);
       return approve;
   }

    function balanceOf(address _erc20Contract) public view returns(uint256){
        address from = msg.sender;
        uint256 balance = IERC20(_erc20Contract).balanceOf(from);
        return balance;
    }
