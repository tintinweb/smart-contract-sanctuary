...
contract IShardsMarketStorge is ShardsMarketAdminStorage {
    ...
    mapping(uint256 => NftToken[]) nftTokens;
    struct NftToken {
...
contract ShardsMarketDelegateV0 is IShardsMarket, ERC721Holder, ERC1155Holder {
    ...
    function createShard(
        ...
        bool success = _transferIn(shardPoolId, tokens, msg.sender);
    ...
    //    token{}
    // nftTokens[shardPoolId].push(token);
    ...
    function settle(uint256 _shardPoolId) external override {
        ...
        if (
            ...
            _transferTo(_shardPoolId, shardCreator);
        ...
        returns (bool)
    {
        ...
        if (result) {
            ...
            _transferTo(_shardPoolId, submitter);
    ...
    function _transferIn(
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            NftToken memory token = tokens[i];
            nftTokens[shardPoolId].push(token);
            if (token.tokenType == NftTokenType.ERC721) {
                IERC721(token.contractAddress).safeTransferFrom(
                    from,
            ...
            if (token.tokenType == NftTokenType.ERC1155) {
    ...
    function _transferTo(uint256 shardPoolId, address to)
        private
        returns (bool)
    {
        NftToken[] memory tokens = nftTokens[shardPoolId];
        for (uint256 i = 0; i < tokens.length; i++) {
            NftToken memory token = tokens[i];
            //  nftTokens[shardPoolId].pop(token);
            if (token.tokenType == NftTokenType.ERC721) {
                IERC721(token.contractAddress).safeTransferFrom(
                    address(this),
                    to,
