...
contract Precog {
    ...
    function deposit(uint256 amount, bool _isStaked) external {
    ...
    function withdrawLiquidate(address futureContract, address to, address user) external {
        ...
        if(isStaked[user] == true){
            (uint stakingAmount, uint reward, uint locktime) = IStakingPool(stakingPool).getUserStakingData(user);
            require(block.timestamp >= locktime, "Precog: can't withdraw in locktime");
            IStakingPool(stakingPool).withdraw(user);
            isStaked[user] == false;
            if (liquidatedAmount[user][futureContract] == 0) {
                this.liquidate(futureContract, user);
            }
            amount = liquidatedAmount[user][futureContract] + stakingAmount + reward - feeWithdrawByUSDC;
        } else {
            if (liquidatedAmount[user][futureContract] == 0) {
                this.liquidate(futureContract, user);
            }
    ...
    function reinvest(address futureContract, address user) external {
        ...
        if(isStaked[user] == true){
            (uint stakingAmount, uint reward, uint locktime) = IStakingPool(stakingPool).getUserStakingData(user);
            require(block.timestamp >= locktime, "Precog: can't reinvest in locktime");
            IStakingPool(stakingPool).withdraw(user);
            isStaked[user] == false;
            if (liquidatedAmount[user][futureContract] == 0) {
                this.liquidate(futureContract, user);
            }
            availableAmount[msg.sender] += (liquidatedAmount[user][futureContract] + stakingAmount + reward);
        } else {
            if (liquidatedAmount[user][futureContract] == 0) {
                this.liquidate(futureContract, user);
            }
    ...
    function liquidate(address futureContract, address user) external {
        ...
        while(isStaked[user] == true){
            if(IStakingPool(stakingPool).getEndTime() > expiryDate){
                IStakingPool(stakingPool).stakingOnBehalf(user, revenue);
                break;
            } else break;
        }
