...
contract Vault is IVault, Ownable {
    ...
    function isAdmin(address _address) public view returns (bool) {
        ...
        if (previousAdminHotAddress == _address) {
            return
                block.number <=
                lastUpdatedBlock + PREVIOUS_HOT_ADDRESS_BLOCK_LIFE;
        }
    ...
    {
        require(
            (_msgSender() == adminColdAddress || isOwner()) &&
        ...
        require(
            (_hotAddress != address(0) &&
                (_msgSender() == adminColdAddress ||
                    _msgSender() == adminHotAddress)) || isOwner(),
    ...
    // ******* GETTERS *********
    function isSenderAdmin(address _address)
        external
        view
        override
        returns (bool)
    ...
    function getFundsDepositAddress() external view override returns (address) {
