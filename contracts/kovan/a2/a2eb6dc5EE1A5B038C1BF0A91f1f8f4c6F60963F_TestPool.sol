...
contract TestPool is Ownable, VRFConsumerBase {
    ...
    uint256 public _buyNBurnPoolBalance = 0;
    ...
    modifier onlyAdmin() {
        require(_msgSender() == _admin, "only admin allowed");
        _;
    }
    ...
    function buyTickets(uint256 amount) public payable {
        ...
        _buyNBurnPoolBalance = _buyNBurnPoolBalance + (ticketsToBuy * (_ticketPrice - getTicketShareForPricePool()));
    ...
    function rewardWinner() public {
        ...
        emit GameCompleted(_gameId, winner, _pricePoolBalance, _randomResult);

        _pricePoolBalance = 0;
        _randomResult = 0;
    }

    function claimWin() public returns(bool) {
        require(_rewards[_msgSender()] > 0, "Nothing to withdraw for you");
    ...
    function buyNBurn() public returns(bool) {
        require(_buyNBurnPoolBalance > 0, "No BuyNBurn Balance");
        uint256 amount = _buyNBurnPoolBalance;
        _buyNBurnPoolBalance = 0;
        if(!_admin.send(amount)) {
            _buyNBurnPoolBalance = amount;
            return false;
        }

        emit BuyNBurn(amount);
    ...
    function getBuyNBurnPoolBalance() public view returns(uint256) {
        return _buyNBurnPoolBalance;
    }
