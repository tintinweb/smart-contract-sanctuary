...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/5dfe7215a9156465d550030eadc08770503b2b2f/contracts/introspection/IERC165.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/5dfe7215a9156465d550030eadc08770503b2b2f/contracts/token/ERC721/IERC721.sol
...
contract StakingContract is Pausable, Ownable {
    ...
    whenNotPaused
  {
    ...
    require(
      IERC721(erc721).balanceOf(msg.sender) > 0,
    ...
    require(
      _amount <= erc20.allowance(msg.sender, address(this)),
    ...
    require(
      _amount >= products[_product_id].individualMinimumAmount,
      'Has to be highger than minimum'
    );
  ...
  function getAPRAmount(
    ...
    return
      ((_endDate.sub(_startDate)).mul(_APR).mul(_amount)).div(year.mul(100));
  }
    ...
    whenNotPaused
  {
    ...
    require(
      products[_product_id].subscriptions[_subscription_id].finalized == false,
      'Subscription was finalized already'
    );

    /* Confirm Subscriptor is the sender */
    require(
      products[_product_id].subscriptions[_subscription_id].subscriberAddress ==
        msg.sender,
      'Not the subscription owner'
    );

    SubscriptionAPR memory subscription = products[_product_id].subscriptions[
      _subscription_id
    ];
    ...
    uint256 totalAmountWithFullAPR = subscription.amount.add(
      getAPRAmount(
        subscription.APR,
        subscription.startDate,
        products[_product_id].endDate,
    ...
    require(
      erc20.transfer(subscription.subscriberAddress, totalAmount),
      'Transfer has failed'
    );
  ...
  function getProduct(uint256 _product_id)
    external
    view
    returns (
      uint256,
    ...
    )
  {
    ...
    return (
      ...
      product.individualMinimumAmount,
