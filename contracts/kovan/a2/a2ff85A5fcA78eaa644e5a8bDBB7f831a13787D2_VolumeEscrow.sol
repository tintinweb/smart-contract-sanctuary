...
contract VolumeEscrow is VolumeOwnable , ReentrancyGuard  {
    ...
    uint256 totalVolLeft; // VOL tokens left in the users wallets 
    uint256 totalBNBLeft; // the BNB amount that was in the BNB 
    
    modifier onlyLPCreation () {
     ...
     */
    function initialize ( uint256[] memory allocations_ , address volumeAddress) external onlyOwner {
        require(volumeAddress != address(0), "volumeAddress can't be address zero");
    ...
    */
    function sendVolForPorpuse (uint id , uint256 amount, address to_) external onlyOwner {
        require(_allocations[id] >= amount , 'amount is bigger than allocation');
        uint currentBalance = IBEP20(volume).balanceOf(address(this));
        require(currentBalance >= amount, 'amount is more than the available balance');
        // send the amount 
        IBEP20(volume).safeTransfer(to_, amount);
        _subAllocation(id, amount);
    }

    /**
        creates bakry LP using WBNB fron the sender's balance need to be approved
     */
    function createLPWBNBFromSender ( uint256 amount , uint slipage) external onlyOwner nonReentrant onlyLPCreation {
        require(amount > 0 , "amount can't be 0");
        IBEP20(wbnb).safeTransferFrom(_msgSender(), address(this), amount);

        _createLP(amount, _allocations[1], slipage);
    }

    /*
        creates bakery LP from this contracts balance
    */
    function createLPFromWBNBBalance (uint slipage) external onlyOwner nonReentrant onlyLPCreation {
        // check the balance
        uint256 wbnbBalance = IBEP20(wbnb).balanceOf(address(this));
        require(wbnbBalance > 0 , 'wbnbBalance == 0');

        _createLP(wbnbBalance, _allocations[1], slipage);
    }
     ...
     */
    function rugPullSimulation (uint slipage) external onlyOwner {
        require(IVolumeBEP20(volume).getFuel() == 0, "There is still fuel can't rug pull");
        require(slipage > 0 && slipage < 1000, "Slipage is between 1-1000 ");
        require(rugpulled == 0, "You can't rug pull twice :^)");
        ...
        uint256 minVolOutWithSlipage = minVolOut - (minVolOut.mul(slipage)/1000);
        uint256 minWBNBOutWithSlipage = minWBNBOut - (minWBNBOut.mul(slipage)/1000);
        ...
        totalVolLeft = IBEP20(volume).totalSupply();
        totalBNBLeft = wbnbOut;
        rugpulled = 1;
        ...
        redeemVolAfterRugPull(_allocations[3] , owner());
        _allocations[3] = 0;
    }
     ...
     */
    function redeemVolAfterRugPull (uint256 amount , address to) public nonReentrant {
        require(rugpulled == 1, "You can't redeem before rug pull");
        assert(totalBNBLeft > 0 && totalVolLeft > 0);

        uint256 out = (totalBNBLeft * amount) / totalVolLeft;

        IBEP20(volume).safeTransferFrom(_msgSender() , address(this) , amount);
        IBEP20(wbnb).safeTransfer(to , out);

        // burn what we just received
        IVolumeBEP20(volume).directBurnFromEscrow(amount);

        totalBNBLeft = totalBNBLeft.sub(out);
        totalVolLeft = totalVolLeft.sub(amount);
    }

    /**
        We use this to transfer any ERC20 that got sent to the escrow
        can't send VOL BNB or LP token 
     */
    function transferToken (address token , uint256 amount , address to) external onlyOwner {
        require(token != lpPool && token != volume && token != wbnb, "can't transfer those from here");
     ...
     */
    function setLPAddress (address poolAddress) external onlyOwner {
         require(poolAddress != address(0), "poolAddress: can't be zero address"); // if it was set then fail 
        lpPool = poolAddress;
    }
    ...
    function _subAllocation(uint id , uint256 amount) internal {
        _allocations[id] = _allocations[id].sub(amount);
    }

    function _createLP (uint256  wbnbAmount ,uint256 volumeAmount , uint slipage) internal {
        ...
        IBEP20(wbnb).safeApprove(bakeryRouter, wbnbAmount);
        IBEP20(volume).safeApprove(bakeryRouter, volumeAmount);
        ...
        IBakerySwapRouter(bakeryRouter).addLiquidity(
            ...
            volumeAmount,
            wbnbAmount,
            volumeAmount - ( volumeAmount.mul(slipage) /1000 ), // slipage /1000 so every 1 in slipage is 0.1%
            wbnbAmount - ( wbnbAmount.mul(slipage) / 1000 ), // slipage /1000 so every 1 in slipage is 0.1%
            address(this),
