...
contract CryptidToken is ERC721, ERC721Enumerable, Pausable, Ownable {
    ...
    event Mint(address minter, uint8 mintAmount, uint256 LastTokenMinted);

    // ~ Sale stages ~
    // stage 0: Init (no minting)
    // stage 1: Whitelist mint (free)
    // stage 2: Presale (up to 3000 total, including free mint)
    // stage 3: Team Mint 
    // stage 4: Public sale
    ...
    function lastMintAddress() public view returns (address){
        ...
        require(totalSupply() > 0, "No cryptid generated");
    ...
    // Public
    function mint(uint8 _mintAmount) public whenNotPaused payable {
        require(totalSupply() < totalSaleSupply, "All cryptids are minted.");
        ...
        emit Mint(msg.sender, _mintAmount, totalSupply());
    }
    ...
    // Set baseURI - require tokenURI is not frozen
    ...
    // Check if tokenId has assigned address, return TokenURI
    ...
    // Move sale to next stage (4 max)
    function nextStage() public onlyOwner() {
        require(provenanceHashFrozen, "Provenance hash must be set before minting can start.");
        require(stage < 4, "No stages after public sale");
        stage++;
    }

    // Set new Team Mint Supply
    ...
    // Freeze base URI (once final cryptid is sold)
    ...
    // Get list of token IDs for single owner
