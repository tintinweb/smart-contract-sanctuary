...
/* 1.1.0
    1. remove "freeclaim" feature [done]
    2. gas optimization:
        a. profile [done]
        b. claimBatch [done]
*/
...
contract Spark is Ownable {
    ...
    event Staked(address indexed user, uint64 ID, uint amount);
    event Unstaked(address indexed user, uint ID, uint amount);
    ...
    function claim(uint index) public returns (uint) {
        // sanity check
        require(_userDeposits[msg.sender][index].stakedLP > 0, "_claim: the deposit does not exist.");
        ...
        if (claimableAmt <= 0) {
            emit TokensClaimed(msg.sender, index, 0);
            return 0;
        }
        ...
        emit TokensClaimed(msg.sender, index, claimableAmt);
    ...
    function claimBatch(uint[] memory indices) external returns (uint) {
        ...
        for (uint i = 0; i < indices.length; i++) {
            ...
            if (claimableAmt <= 0) {
                emit TokensClaimed(msg.sender, indices[i], 0);
                continue;
            }
            ...
            emit TokensClaimed(msg.sender, indices[i], claimableAmt);
        }
