...
contract LockedBoxV10 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function stakeTokens(uint256 _amount) external returns (bool success, uint256 amountStaked, uint256 unlockTimestamp){
        cino.transferFrom(msg.sender, address(this), _amount);
        uint256 unlockDate = block.timestamp + (unixMinute * 5);
        addressUnlockTime[msg.sender] = unlockDate; 
        addressTotalStaked[msg.sender] = addressTotalStaked[msg.sender] + _amount;
        _totalStaked = _totalStaked + _amount;
        currentStakers.push(msg.sender);
