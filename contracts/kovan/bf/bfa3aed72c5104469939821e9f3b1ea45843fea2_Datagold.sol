...
contract Datagold is ERC20Interface, SafeMath{
   ...
   // datagold coins
    ...
    returns (bool)
    {
        ...
        uint amtToContractOwner = safeDiv(safeMul(price, 3), 100);
        uint amtToSellerAgent = safeDiv(safeMul(price, 3), 100);
        uint amtTotheSeller = safeSub(price, amtToContractOwner);
        amtTotheSeller = safeSub(amtTotheSeller, amtToSellerAgent);
        
        address oldOwner = datasetOwners[datasetID];
        transferFrom(newOwner, contractOwner, amtToContractOwner);
        transferFrom(newOwner, sellerAgents[datasetID], amtToSellerAgent);
        transferFrom(newOwner, oldOwner, amtTotheSeller);
        
        datasetOwners[datasetID] = newOwner;
        listingStatus[listingID] = 2;
        emit Sold(listingID, datasetID, oldOwner, newOwner, listingPrice[listingID]);
