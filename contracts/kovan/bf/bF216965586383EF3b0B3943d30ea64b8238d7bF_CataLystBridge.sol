...
contract Manager is Ownable, Pausable, Round {
    ...
    function adminAddRound(uint256 _startTime, uint256 _duration)
        external
        whenNotPaused
        onlyOwner
    {
    ...
    function adminUpdateRound(uint256 _roundId, uint256 _startTime, uint256 _duration)
        external
        whenNotPaused
        onlyOwner
    {
...
contract CataLystBridge is Manager {
    ...
    function userDeposit(uint256 _roundId) isValidRound(_roundId) whenNotPaused external payable { 
        require(msg.value > 0, "Invalid-fund");
        uint256 fund = msg.value;
        RoundInfo memory round = rounds[_roundId];
        require(round.startTime <= block.timestamp && block.timestamp <= round.startTime.add(round.duration),"Cam-not-deposit");
    ...
    function userWithDrawn(uint256 _roundId) isValidRound(_roundId) whenNotPaused external {
        ...
        require(round.isAllowWithdrawn, "Can-not-withdrawn-now"); // deposit is closed and allow withdrawn
    ...
    function adminCollectFund(uint256 _roundId) isValidRound(_roundId) whenNotPaused external onlyOwner {
        require(rounds[_roundId].startTime.add(rounds[_roundId].duration) < block.timestamp, "Round-not-end-yet");
        RoundInfo memory round = rounds[_roundId];
