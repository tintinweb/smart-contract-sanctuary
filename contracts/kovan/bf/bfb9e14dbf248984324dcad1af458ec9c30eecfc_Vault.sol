...
contract Vault is Ownable, ReentrancyGuard {
    ...
    function getEstimatedCompoundDivsOwing(address account) public view returns (uint) {
        return (cTokenBalance[account].mul(getExchangeRateStored()).div(10**(18 + UNDERLYING_DECIMALS - 8))).sub(depositTokenBalance[account]);
    }
    ...
    function _claimCompoundDivs() private {
        ...
        uint amount = (cTokenBalance[msg.sender].mul(exchangeRateCurrent).div(10**(18 + UNDERLYING_DECIMALS - 8))).sub(depositTokenBalance[msg.sender]);
