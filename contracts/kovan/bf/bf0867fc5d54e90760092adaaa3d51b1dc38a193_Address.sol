...
pragma solidity ^0.6.12;
    ...
    contract SecretToken is Context, IERC20, Ownable {
        ...
        // Maps addresses to # of reflected tokens
        mapping (address => uint256) private _reflectedTokenBalance;

        // Map addresses to # of SecretToken tokens
        mapping (address => uint256) private _tokensBalance;

        // ERC20 allowances of SecretToken tokens
        mapping (address => mapping (address => uint256)) private _allowances;

        // Tracking when addresses last bought
        ...
        uint256 private _totalNumOfTokens = 17760000001984;  //17,760,000,001,984
        uint256 private _rTotal = (MAX - (MAX % _totalNumOfTokens));
        uint256 private _tFeeTotal;
        uint256 private RandReturned;

        // 1,000,000,000,000,000,000,000 internal tokens
        // 1,000,000,000,000 ESHARE external tokens
        ...
        string private _name = 'SecretToken';
        ...
        uint256 public _maxTxAmount = _totalNumOfTokens; 
        ...
        constructor (address payable devWalletAddress, address payable sharedWalletAddress) public {
            ...
            _reflectedTokenBalance[_msgSender()] = _rTotal;
            ...
            emit Transfer(address(0), _msgSender(), _totalNumOfTokens);
        }
        ...
        function totalSupply() public view override returns (uint256) {
            return _totalNumOfTokens;
        }

        function balanceOf(address account) public view override returns (uint256) {
            if (_isExcluded[account]) return _tokensBalance[account];
            return tokenFromReflection(_reflectedTokenBalance[account]);
        }
        ...
        function deliver(uint256 tAmount) public {
            ...
            // make sure addresses that are excluded from lottery cannot deliver tokens
            require(!_isExcluded[sender], "Excluded addresses cannot call this function");

            // 
            (uint256 rAmount,,,,,) = _getValues(tAmount);
            _reflectedTokenBalance[sender] = _reflectedTokenBalance[sender].sub(rAmount);
            _rTotal = _rTotal.sub(rAmount);
            _tFeeTotal = _tFeeTotal.add(tAmount);
        }

        function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
            require(tAmount <= _totalNumOfTokens, "Amount must be less than supply");
        ...
        function excludeAccount(address account) external onlyOwner() {
            ...
            if(_reflectedTokenBalance[account] > 0) {
                _tokensBalance[account] = tokenFromReflection(_reflectedTokenBalance[account]);
            }
      ...
      function addWalletToDevList(address account) private {
        ...
        function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
            _maxTxAmount = _totalNumOfTokens.mul(maxTxPercent).div(
                10**2
            );
        }
        ...
        function _transfer(address sender, address recipient, uint256 amount) private {
            ...
            // Don't let non owners trade before we're ready
            // Don't let non owners trade more than max txnAmount
            ...
            if (sender == uniswapV2Pair ) {  
                ...
                if (!_isExcluded[recipient] && feeEnabled) { 
                    if (amount >= minEligibleAmount) {
                        addWalletToDevList(recipient);
                    } 
                } 
            }
        ...
        returns(uint256)
    {
        ...
        function _transferStandard(address sender, address recipient, uint256 tAmount) private {
            ...
            _reflectedTokenBalance[sender] = _reflectedTokenBalance[sender].sub(rAmount);
            _reflectedTokenBalance[recipient] = _reflectedTokenBalance[recipient].add(rTransferAmount);
        ...
        function _transferToExcluded(address sender, address recipient, uint256 tAmount) private {
            (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tdev) = _getValues(tAmount);
            _reflectedTokenBalance[sender] = _reflectedTokenBalance[sender].sub(rAmount);
            _tokensBalance[recipient] = _tokensBalance[recipient].add(tTransferAmount);
            _reflectedTokenBalance[recipient] = _reflectedTokenBalance[recipient].add(rTransferAmount);    
            _takedev(tdev);           
            _reflectFee(rFee, tFee);
            emit Transfer(sender, recipient, tTransferAmount);
        ...
        function _transferFromExcluded(address sender, address recipient, uint256 tAmount) private {
            (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tdev) = _getValues(tAmount);
            _tokensBalance[sender] = _tokensBalance[sender].sub(tAmount);
            _reflectedTokenBalance[sender] = _reflectedTokenBalance[sender].sub(rAmount);
            _reflectedTokenBalance[recipient] = _reflectedTokenBalance[recipient].add(rTransferAmount); 
            _takedev(tdev);   
            _reflectFee(rFee, tFee);
        ...
        function _takedev(uint256 tdev) private {
            ...
            _reflectedTokenBalance[address(this)] = _reflectedTokenBalance[address(this)].add(rdev);
            if(_isExcluded[address(this)])
                _tokensBalance[address(this)] = _tokensBalance[address(this)].add(tdev);
        }
        ...
        function _getCurrentSupply() private view returns(uint256, uint256) {
            ...
            uint256 tSupply = _totalNumOfTokens;      
            for (uint256 i = 0; i < _excluded.length; i++) {
                if (_reflectedTokenBalance[_excluded[i]] > rSupply || _tokensBalance[_excluded[i]] > tSupply) return (_rTotal, _totalNumOfTokens);
                rSupply = rSupply.sub(_reflectedTokenBalance[_excluded[i]]);
                tSupply = tSupply.sub(_tokensBalance[_excluded[i]]);
            }
            if (rSupply < _rTotal.div(_totalNumOfTokens)) return (_rTotal, _totalNumOfTokens);
