...
contract PoolV2 is ReentrancyGuard,Initializable {
    ...
    */
	function buyAstraToken(uint _Amount) internal returns(uint256){ 
		...
		DexAggregator(EXCHANGE_CONTRACT).swapFromBestExchange(IERC20(baseStableCoin), IERC20(ASTRTokenAddress), _Amount, minReturn, _distribution);
		return _amount;
	}
    ...
    */
	...
	function withdrawTokens(uint _poolIndex,uint _balance) internal returns(uint256){
		...
		for (uint i = 0; i < poolInfo[_poolIndex].tokens.length; i++) {
			...
			DexAggregator(EXCHANGE_CONTRACT).swapFromBestExchange(IERC20(poolInfo[_poolIndex].tokens[i]), IERC20(baseStableCoin), withdrawBalance, _amount, _distribution);
		}
		return _totalAmount;
	}
