...
contract Cryptium is CrptERC20, Control, ReentrancyGuard {
	...
	function rebase() external nonReentrant initialized {
		...
		if (isInflRebase) {
			...
			_lastRebase = _checkMonths(_lastRebase)+1;
			// emit Log("startTime lastRebase", _startTime, _lastRebase, 0, 0);
		}
	...
	function _getMonthsPass() private view returns (uint256, uint256) {
		...
		monthsPassAll = _checkMonths(monthsPassAll);
		monthsPassFromRebase = _checkMonths(monthsPassFromRebase);
	...
	function _checkMonths(uint256 months) private view returns (uint256) {
		if (TimeLibrary.addMonths(_startTime, months) > block.timestamp) months--;
		return months;
	}
