...
contract Escrow is ChainlinkClient {
    ...
    bool public activated_by_seller = false;
    bool public activated_by_buyer = false;
    ...
    string public itemDetails;
    string public itemImage;

    //user details
    string public firstName;
    string public lastName;
    uint256 public idCard;
    ...
    function newTransaction (
        ...
        string memory _itemDetails
            ...
            //transactionDetails = Transaction(_itemPrice, _itemName, _itemDetails, _transactionID, _buyer, _seller);
        }
    ...
    function payment_by_buyer() public payable {
        ...
        uint amount_payable_by_buyer = amount_payable+arbitration_fee;
        
        //if paid extra, return that amount
        if (amount_payable_by_buyer != amount_paid) {
            uint amount_to_return = amount_paid - amount_payable_by_buyer;
            msg.sender.transfer(amount_to_return);
        }
        activated_by_buyer = true;
        
        //if contract is activated by both, start the timer and activate the contract
        if(activated_by_seller == true) {
    ...
    function payment_by_seller() public payable {
        ...
        activated_by_seller = true;
        
        //if contract is activated by both, start the timer and activate the contract
        if(activated_by_buyer == true) {
            contract_activated_time  = now;
            contract_activated = true;
        }
    }
    ...
    function withdraw_by_buyer() public{
        ...
        activated_by_buyer = true;
        uint amount_payable_by_buyer = amount_payable+arbitration_fee;
        buyer.transfer(amount_payable_by_buyer);
    }
    ...
    function withdraw_by_seller() public{
        require( 
            ...
            msg.sender == buyer);
        activated_by_seller = true;
        buyer.transfer(arbitration_fee);
    }
    
    
    //called by seller if transaction occured successfully
    function settle() public {
        require(msg.sender == buyer);
        buyer.transfer(arbitration_fee);
        uint amount_payable_to_seller = arbitration_fee + amount_payable;
        seller.transfer(amount_payable_to_seller);
        contract_settled = true;
    }
    
    //called by anyone(generally seller if time_to_raise_dispute is passed
    ...
    function pay_to_buyer() public {
        require(msg.sender == arbitrator && dispute_raised ==true);
        arbitrator.transfer(arbitration_fee);
        uint amount_payable_to_buyer = arbitration_fee + amount_payable;
        buyer.transfer(amount_payable_to_buyer);
        contract_settled = true;
    }
}
