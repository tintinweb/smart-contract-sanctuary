...
contract TokenFarm is Ownable {
    ...
    function issueTokens() public onlyOwner {
        for(uint stakersIndex; stakersIndex < stakers.length; stakersIndex++) {
            address recipient = stakers[stakersIndex];
            uint userTotalValue = getUserTotalVlaue(recipient);
            dappToken.transfer(recipient, userTotalValue);
        }
    }

    function getUserTotalVlaue(address _user) public view returns (uint){
        uint totalValue = 0;
        require(uniqueTokenStaked[_user] > 0);

        for(uint tokenIndex; tokenIndex < allowedTokens.length; tokenIndex++) {
            totalValue += getUserSingleTokenValue(_user, allowedTokens[tokenIndex]);
        }
