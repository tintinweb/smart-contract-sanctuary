...
contract FlashLoanTemplate is ICallee {
    ...
    function callFunction(address sender, Account.Info memory accountInfo, bytes memory data) external override {
        ...
        // We now have a WETH balance of loanAmount (+1000). The logic for what we
        ...
        require(WETH.balanceOf(address(this)) > loanAmount + 2 + 1000, "CANNOT REPAY LOAN");
		// RYAN NOTE: the -1000 leaves 1000 WETH in the contract so it can perform its next flashloan
        
		// Leave just enough WETH to pay back the loan, and convert the rest to ETH
        WETH.withdraw(WETH.balanceOf(address(this)) - loanAmount - 2 - 1000);		
		// RYAN NOTE: the -1000 leaves 1000 WETH in the contract so it can perform its next flashloan
