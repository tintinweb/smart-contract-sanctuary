...
contract SyndicateCorePool is SyndicatePoolBase {
  ...
  /// @dev Control parameter of how much reward in CRED user will get immediately upon staking.
   ...
   * @param _cred CRED ERC20 Token EscrowedSyndicateERC20 address
  ...
  constructor(
    ...
    uint64 _initBlock,
    uint32 _weight
  ) SyndicatePoolBase(_syn, _cred, _factory, _poolToken, _initBlock, _weight) {}
  ...
  /** @notice set the reward weight for CRED distributed upon staking
   *
   ...
   *
   * @param _useSSYN flag indicating whether to mint CRED token as a reward or not;
   *      when set to true - CRED reward is minted immediately and sent to sender,
   *      when set to false - new SYN reward deposit gets created if pool is an SYN pool
   *      (poolToken is SYN token), or new pool deposit gets created together with CRED minted
 ...
 *          - SYN token address
 *          - CRED token address, used to mint CRED rewards
...
abstract contract SyndicatePoolBase is IPool, SyndicateAware, ReentrancyGuard {
  ...
  /// @dev Link to CRED ERC20 Token EscrowedSyndicateERC20 instance
  address public immutable override cred;
   ...
   * @param sSyn flag indicating if reward was paid (minted) in CRED
  ...
  constructor(
    ...
    require(_cred != address(0), "CRED address not set");
    require(address(_factory) != address(0), "SYN Pool fct address not set");
    require(_poolToken != address(0), "pool token address not set");
    require(_initBlock > 0, "init block not set");
    require(_weight > 0, "pool weight not set");

    // verify CRED instance supplied
    require(
      EscrowedSyndicateERC20(_cred).TOKEN_UID() == 0xac3051b8d4f50966afb632468a4f61483ae6a953b74e387a01ef94316d6b7d62,
      "unexpected CRED TOKEN_UID"
    );
   ...
   * @param _useSSYN a flag indicating if previous reward to be paid as CRED
   */
  function stake(
    uint256 _amount,
    uint64 _lockUntil,
   ...
   * @param _useSSYN a flag indicating if reward to be paid as CRED
   */
  function unstake(
    uint256 _depositId,
    uint256 _amount,
   ...
   * @param depositId updated deposit ID
   * @param lockedUntil updated deposit locked until value
   * @param useSSYN used for _processRewards check if it should use SYN or CRED
   */
   ...
   * @param _useSSYN flag indicating whether to mint CRED token as a reward or not, see processRewards()
  ...
  function mintSSyn(address _to, uint256 _value) internal {
    ...
    EscrowedSyndicateERC20(cred).mint(_to, _value);
  }
...
interface IPool is ILinkedToSYN {
  ...
  function cred() external view returns (address);
...
contract SyndicatePoolFactory is Ownable, SyndicateAware {
  ...
  /// @dev CRED token address is used to create SYN core pool(s)
   ...
   * @param _cred CRED ERC20 token address
  ...
  function createPool(
    ...
    IPool pool = new SyndicateCorePool(syn, cred, this, poolToken, initBlock, weight);
...
contract EscrowedSyndicateERC20 is ERC20("SYN Credit", "CRED"), AccessControl {
  ...
  function mint(address recipient, uint256 amount) external {
    require(isSenderInRole(ROLE_TOKEN_CREATOR), "CRED: insufficient privileges (ROLE_TOKEN_CREATOR required)");
  ...
  function _transfer(
    ...
    require(isOperatorInRole(recipient, ROLE_WHITE_LISTED_RECEIVER), "CRED: Non Allowed Receiver");
