...
/**
 * Supported ISO 8601 date & datetime formats
 * ------------------------------------------
 * yyyy-MM-dd
 * yyyyMMdd
 * yyyy-MM-ddTHH:mm:ss
 ...
 */
/**
 * @title A consumer contract for Coinmetrics EA.
 * @author LinkPool.
 */
contract CoinmetricsConsumer is ChainlinkClient {
    ...
    /**
     * @notice Returns the burned amount of an asset.
     * @param _jobId the job ID.
     * @param _payment the LINK amount in Juels (i.e. 10^18 aka 1 LINK).
     * @param _asset the token ticker (see coinmetrics API docs).
     * @param _frequency at which interval to calculate the number of tokens burned (`1d` - day, or `1b` - block).
     * @param _pageSize the number of results per page (an integere from 1 to 10000).
     * @param _startTime the start time (see supported formats).
     * @param _endTime the end time (see supported formats).
     */
    function requestTotalBurned(
        bytes32 _jobId,
        uint256 _payment,
        string memory _asset,
        string memory _frequency,
        string memory _pageSize,
        string memory _startTime,
        string memory _endTime
    ) public {
        Chainlink.Request memory req = buildChainlinkRequest(_jobId, address(this), this.fulfillTotalBurned.selector);

        req.add("endpoint", "total-burned");
        req.add("asset", _asset);
        req.add("frequency", _frequency);
        req.add("pageSize", _pageSize);
        req.add("startTime", _startTime);
        req.add("endTime", _endTime);
     ...
     * @param _payment the LINK amount in Juels (i.e. 10^18 aka 1 LINK).
     */
    function requestTotalBurnedEth(bytes32 _jobId, uint256 _payment) public {
        Chainlink.Request memory req = buildChainlinkRequest(_jobId, address(this), this.fulfillTotalBurned.selector);

        req.add("endpoint", "total-burned"); // NB: not required if it has been hardcoded in the job spec
        req.add("asset", "eth"); // NB: not required if it has been hardcoded in the job spec
    ...
    function fulfillTotalBurned(bytes32 _requestId, uint256 _burnedETH) public recordChainlinkFulfillment(_requestId) {
