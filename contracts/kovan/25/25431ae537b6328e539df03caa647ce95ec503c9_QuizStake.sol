...
contract QuizStake is Ownable, Events {
     ...
     */
    function addPool(uint256 _price, uint256 _startDate, uint256 _publishDate) external onlyOwner returns (uint256){
    ...
    function publishPool(uint256 pid) external onlyOwner {
       ...
       require(block.timestamp > pool.publishDate, "The publish is has not expired yet");
    ...
    function claim() public {
        require(rewardInfo[msg.sender] > 0, "The user does not have the reward");
    ...
    function getPoolStatus(uint256 pid) public view returns(uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        PoolInfo storage pool = poolInfo[pid];
        return (pool.poolStatus, pool.userNumber, pool.price, pool.depositedAmount, pool.assignedReward, pool.rewardsAmount, pool.poolTimestamp, pool.startDate, pool.publishDate);
    }
