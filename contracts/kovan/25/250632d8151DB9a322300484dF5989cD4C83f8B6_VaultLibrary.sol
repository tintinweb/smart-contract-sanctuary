    ...
    OwnableUpgradeable
{
        ...
        )
    {
        ...
        for (uint256 i = 0; i < j; i++) {
            ...
            // collateralAmounts[i] is in 18 decimals, must adjust to collateral type's.
            int256 decimalDelta =
                18 - int256(uint256(IERC20(collateralTypes[i]).decimals()));
            if (decimalDelta > 0) {
                // Collateral has fewer decimals than ETH, e.g. BTC -- scale down.
                collateralAmounts[i] = collateralAmounts[i].div(
                    10**uint256(decimalDelta)
                );
            } else if (decimalDelta < 0) {
                // Collateral has more decimals than ETH -- scale up.
                collateralAmounts[i] = collateralAmounts[i].mul(
                    10**uint256(-decimalDelta)
                );
            }
