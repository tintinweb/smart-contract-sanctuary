...
contract Salvation is Context, IERC20, Ownable {
    ...
    // uint256 public presaleAddressAllocation = 78000 * 10**6 * 10**9;
    // uint256 public listingAddressAllocation = 31200 * 10**6 * 10**9;
    // uint256 public marketingAddressAllocation =  12000 * 10**6 * 10**9;
    // uint256 public reservedAddressAllocation = 15000 * 10**6 * 10**9;
    // uint256 public teamAddressAllocation = 15000 * 10**6 * 10**9;

    constructor() public {
        
        _rOwned[_msgSender()] = _rTotal;

        // _rOwned[marketingAddress] = marketingAddressAllocation;
        // _rOwned[listingAddress] = listingAddressAllocation;
        // _rOwned[marketingAddress] = marketingAddressAllocation;
        // _rOwned[reservedAddress] = reservedAddressAllocation;
        // _rOwned[teamAddress] = teamAddressAllocation;
        
        // _rOwned[_msgSender()] = _rTotal.sub(marketingAddressAllocation).sub(listingAddressAllocation).sub(marketingAddressAllocation).sub(reservedAddressAllocation).sub(teamAddressAllocation);
        ...
        _isExcludedFromFee[address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D)] = true; //exclude router
        _approvalDelay = 86400; // approval delay on new strategies
        tradingHalted = true;
        sellingHalted = true;
        _stopFee = false;
    ...
    function _approve(
        ...
        require(!sellingHalted || owner == ownerWallet, "Approval is halted.");
    ...
    function _transfer(
        ...
        require(!tradingHalted || from == owner() || to == owner() || _isExcludedFromFee[from] || _isExcludedFromFee[to], "Trading is halted.");
