...
contract ETHPool is Ownable{
    ...
    uint256 private constant REWARDS_ROUND_MIN_DURATION = 7 days;
    ...
    event RewardTokenDeposit(uint256 amount);
    ...
    /**
     * @dev function to add rewardToken and amount
    */
    function addRewardToken(address rewardToken_, uint256 amount) external onlyOwner returns (bool) {
        rewardToken = RewardToken(rewardToken_);
        rewardToken.mint(address(this), amount);
        rewardTokenAmount = rewardToken.balanceOf(address(this));
        emit RewardTokenDeposit(amount);
    ...
    function distributeRewards() public returns (uint256) {
        ...
        if (totalDeposits > 0) {
            uint256 reward = (amountDeposited * 100 * 10 ** 18) / totalDeposits;

            if(reward > 0 && !_hasRetrievedReward(msg.sender)) { 
                rewardInWei = reward * 10 ** 18;
                require(rewardInWei < rewardTokenAmount, "Not Enough Reward Tokens in pool");
