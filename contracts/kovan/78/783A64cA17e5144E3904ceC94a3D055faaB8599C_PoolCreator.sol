...
import "@yield-protocol/vault-interfaces/IFYToken.sol";
import "./fyTokens/BasicFYTokenFactory.sol";
import "./fyTokens/IBasicFYToken.sol";
import "./fyTokens/BasicFYToken.sol";
import "./interfaces/IPermit.sol";
import "./fyTokens/IBasicFYToken.sol";
import "./minters/DutchAuctionMinterFactory.sol";

contract PoolCreator {
  IPoolFactory public immutable poolFactory;
  BasicFYTokenFactory public immutable fyTokenFactory;
  DutchAuctionMinterFactory public immutable minterFactory;

  event Created(address indexed underlying, address indexed creator, address fyToken, address minter, address pool);

  constructor(address _fyTokenFactory, address _minterFactory, address _poolFactory) {
    fyTokenFactory = BasicFYTokenFactory(_fyTokenFactory);
    minterFactory = DutchAuctionMinterFactory(_minterFactory);
  ...
  function create(
    ...
    uint256[5] calldata minterParams,
    address treasury,
    uint256 initialLiquidity
  ) external {
    address fyToken = fyTokenFactory.createToken(underlying, maturity, name, symbol);

    address minter = createMinter(fyToken, underlying, minterParams, treasury);

    BasicFYToken(fyToken).setMinter(minter);
    BasicFYToken(fyToken).transferOwnership(msg.sender);

    address pool = poolFactory.createPool(underlying, fyToken);
    ERC20(underlying).transferFrom(msg.sender, address(this), initialLiquidity);
    ERC20(underlying).approve(pool, initialLiquidity);

    IPool(pool).mint(msg.sender, initialLiquidity);

    emit Created(underlying, msg.sender, fyToken, minter, pool);
  }

  function createMinter(
    address fyToken,
    address underlying,
    uint256[5] memory minterParams,
    address treasury
  ) private returns (address) {
    // We have to do this to avoid Stack Too Deep errors
    uint256 auctionStartPrice = minterParams[0];
    uint256 auctionEndPrice = minterParams[1];
    uint256 auctionStartTime = minterParams[2];
    uint256 auctionEndTime = minterParams[3];
    uint256 cap = minterParams[4];

    return minterFactory.createMinter(
      IBasicFYToken(fyToken),
      ERC20(underlying),
      auctionStartPrice,
      auctionEndPrice,
      auctionStartTime,
...
import "@yield-protocol/utils/contracts/access/Ownable.sol";
import "@yield-protocol/utils/contracts/token/ERC20.sol";
...
contract BasicFYToken is IBasicFYToken, ERC20, Ownable {
  ...
  function redeem(address _recipient, uint256 _amount) external override returns (uint256 redeemAmount) {
    ...
    redeemAmount = _amount * underlyingTokensAvailable / totalSupply();
