...
import "./TemporalLoot.sol";

contract GetUnclaimMLoot {
  TemporalLoot public temporalLoot;

  uint256[] private _unclaimMLootId;

  constructor(address _temporalLoot) {
    temporalLoot = TemporalLoot(_temporalLoot);
  }
  ...
  function unclaimMLootId(uint256 endTokenId) public returns (uint256[] memory) {
    require(endTokenId > 8000 && endTokenId < (block.number / 10) + 1, "Token ID invalid");

    for (uint256 i = 8001; i < endTokenId; i++) {
      if (temporalLoot.ownerOf(i) == address(0)) {
        _unclaimMLootId.push(i);
      }
    }

    return _unclaimMLootId;
  }

  function batchClaim(uint256[] memory tokenIds) public {
    for (uint256 i = 0; i < tokenIds.length; i++) {
      require(tokenIds[i] > 8000 && tokenIds[i] < (block.number / 10) + 1, "Token ID invalid");
      temporalLoot.claim(tokenIds[i]);
    }
  }
}
