...
contract Votes is Ownable {
  ...
  event NewSurvey(uint eventId, uint creationTimestamp, Survey survey);
  event NewVote(uint eventId, address indexed sender, string answer);
  event Claim(uint eventId, address indexed sender, uint amount);
...
//  modifier allowedToCreate(address user) {
//    require (user == owner() || userAllowedToCreate, "You are not allowed to create");
//   _;
//  }
  ...
  function createSurvey (string memory nftUrl, uint startTimestamp, uint endTimestamp, uint minVotes, uint maxVotes, uint rewardAmount)
    ...
    {
      emit NewSurvey(surveys.length - 1, block.timestamp, newSurvey);
    }
  }
