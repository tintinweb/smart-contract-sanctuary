...
contract Sales is Ownable {
    ...
    IUniswapV2Factory internal iUniswapV2Factory;
    ...
    function ethBuy(
        ...
        uint quantity = _proportion(
            msg.value, uniswapRouter.WETH(), usdt);
        require(
            quantity <= token.balanceOf(address(this)), 
            "Not enough tokens left for sale"
        );
        uint amount = _swapETHForUSDT(msg.value, 1);
        //need to convert usdt to 18 decimals
        uint usdtEquiv = amount.mul(10 ** 12);
        sold = sold.add(usdtEquiv);
        sales.push(Sale(_investor, usdtEquiv));
        require(
            token.transfer(_investor, usdtEquiv),
            "Error in sending token"
        );
        emit NewBuy(_investor, usdtEquiv);
    }
    ...
    function withdrawERC20(
        ...
        uint balance = IERC20(_token).balanceOf(
            address(this));
    ...
    function getRate(
        uint _amount
        ) external view returns(uint equivilent) {
        return _proportion(
            _amount, uniswapRouter.WETH(), usdt);
    }
    
    function getTimeRemaining(
        ) external view returns (uint timeLeft) {
        uint _end = end;
        if (_end > block.timestamp) return (_end.sub(block.timestamp));
        else return 0;
    }
    ...
    function _swapETHForUSDT(
        uint _ethValue,
        uint _amountOutMin
        ) private returns(uint usdtAmount) {
        require(_ethValue > 0, 'Insufficient Balance');
    ...
    function _proportion(
        uint _amount, 
        address _tokenA, 
        address _tokenB
        ) private view returns(uint tokenBAmount) {
        (uint reserveA, uint reserveB) = UniswapV2Library.getReserves(
            address(iUniswapV2Factory), _tokenA, _tokenB);    
