...
contract Trader is Ownable,Tradable {
    address internal constant V2_ROUTER_ADDRESS = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    address internal constant NFT_ADDRESS = 0xC36442b4a4522E871399CD717aBDD847Ab11FE88;
    address internal constant V3_ROUTER_ADDRESS = 0xE592427A0AEce92De3Edee1F18E0157C05861564;
    IUniswapV2Router02 public V2Router;
    ISwapRouter public V3Router;
    
constructor() {
    V2Router = IUniswapV2Router02(V2_ROUTER_ADDRESS);
    V3Router = ISwapRouter(V3_ROUTER_ADDRESS);
}

// ============ Util Functions ============
function setV2Router(address router_addr)external onlyOwner
{
    V2Router = IUniswapV2Router02(router_addr);
}

function setV3Router(address router_addr)external onlyOwner
{
    V3Router = ISwapRouter(router_addr);
}
...
function swapETHtoWETH(uint amounts) external onlyTraders {
    IWeth weth = IWeth(V2Router.WETH());
...
event InsertOrderV2(uint amountIn, uint amountOut, address inAddress, address outAddress);

function insertOrderV3Buy(uint amount, address[] memory paths, uint expectInSize, uint24[] memory fees) external onlyTraders {
    ...
    amounts = V3Router.exactOutputSingle(ExactOutputParams);
    emit InsertOrderV3(amounts,amount,paths[0],paths[paths.length-1]);
}

function insertOrderV3Sell(uint amount, address[] memory paths, uint expectOutSize, uint24[] memory fees) external onlyTraders{
    require(paths.length == 2);
    ...
    amounts = V3Router.exactInputSingle(ExactInputParam);
    
    emit InsertOrderV3(amount, amounts,paths[0],paths[paths.length-1]);
}

function insertOrderV2Buy(uint amount, address[] memory paths, uint expectInSize) external onlyTraders returns(uint) {
    require(paths.length == 2);
    uint deadline = block.timestamp + 15;
    uint amounts = amount;
    ...
    swapPath[0] = paths[0];
    swapPath[1] = paths[1];
    amounts = V2Router.swapTokensForExactTokens(amounts, expectInSize, swapPath, address(this), deadline)[1];
    
    emit InsertOrderV2(amount, amounts,paths[0],paths[paths.length-1]);
    return amounts;
}

function insertOrderV2Sell(uint amount, address[] memory paths, uint expectOutSize) external onlyTraders returns(uint) {
    require(paths.length == 2);
    uint deadline = block.timestamp + 15;
    uint amounts = amount;
    address[] memory swapPath = new address[](2);
    swapPath[0] = paths[0];
    swapPath[1] = paths[1];
    amounts = V2Router.swapExactTokensForTokens(amounts, expectOutSize, swapPath, address(this), deadline)[1];
