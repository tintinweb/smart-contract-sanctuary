...
contract EmiStaking02 is Ownable {
    ...
    constructor(
        ...
        require(_emiPrice != address(0), "emiPrice address cannot be empty");
        require(_stables.length > 0, "Stable coins must exist");
        ...
        maxUSDStakes = 100000e18; // 100000 by default
        marketId = 0; // Our by default
    ...
    function withdraw() external {
        ...
        for (uint256 i = 0; i < t.length; i++) {
            if (t[i].isWithdrawn == 0 && block.timestamp >= t[i].unlockDate) {
    ...
    function _checkMaxUSDCondition(address staker, uint256 amount) internal view returns (bool) {
       ...
       // calc price in usd
        ...
        uint256 stakesTotal;
        if (tokenMode==0) {
          stakesTotal = _bal.add(amount).mul(IEmiPrice2(emiPrice).getCoinPrices(tokens, stableCoins, marketId)[0]);
        } else if (tokenMode==1) {
    ...
    function _getStakesForLPToken(uint256 amount) internal view returns(uint256)
    {
       ...
       address [] memory tokens = new address[](1);
       tokens[0] = address(IEmiswap(lockToken).tokens(0));

       return rsv.mul(IEmiPrice2(emiPrice).getCoinPrices(tokens, stableCoins, marketId)[0]).mul(lpFraction);
    }
    ...
    /**
     * @dev Update emiPrice address
     * @param _emiPrice EmiPrice address
     */
    function updateEmiPrice(address _emiPrice) external onlyOwner {
        emiPrice = _emiPrice;
    }

    /**
     * @dev Update maxUSD value
     * @param _value Max USD value in 10**18
     */
    ...
    /**
     * @dev Update market Id for use with price calculator
     * @param _id Market ID (0 for our market, 1 for Uniswap, 2 for 1inch)
     */
    function updateMarket(uint8 _id) external onlyOwner {
        require(_id < 3, "Wrong market index");
        marketId = _id;
    }
