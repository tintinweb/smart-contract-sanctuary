...
contract CathyBank is Ownable {
    ...
    function getETHRewardsShare(
        ...
        require(PHXBalance[_shareholder] > 0);
        if (lastActionTime[_shareholder] == 0 || PHXBalance[_shareholder] == 0) return false;

        uint256 timeElapsed = block.timestamp - lastActionTime[_shareholder];
        uint256 secsYear = 60 * 60 * 24 * 360;
        uint256 accruedInterest = (timeElapsed *
            savingsInterestPercent *
            PHXBalance[_shareholder] *
            1000000) /
            secsYear /
            100000000;

        emit LogAmount(accruedInterest);
        _shareholder.transfer(accruedInterest);
        return true;
    }
    ...
    function depositPHX(uint256 _PHXAmount) public {
        ...
        cathyBankTokenHolders.push(msg.sender);
