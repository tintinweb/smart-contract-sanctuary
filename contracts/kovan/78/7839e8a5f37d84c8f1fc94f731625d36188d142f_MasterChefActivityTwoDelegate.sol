...
contract ActivityBase is Ownable{
    ...
    function calculateDividend(uint256 _pending, uint256 _userAmount, bool _isContractSender) internal view returns (uint256 _devDividend, uint256 _userDividend){
        uint256 fee = 0;
        if(_isContractSender && contractFeeRateNumerator > 0){
        ...
        if(tokenAmountLimit > 0 && _userAmount >= tokenAmountLimit){
...
contract MasterChefActivityTwo is Ownable, IInvitation, ActivityBase {
    ...
    function initialize(
        ...
        contractFeeRateNumerator = 0;
        contractFeeRateDenominator = 0;
    ...
    function withdraw(uint256 _pid) public virtual {
        ...
        if (totalDividend > 0) {
            ...
            (uint256 devDividend, uint256 userDividend) = calculateDividend(pending, amount, isUserContractSender[_pid][msg.sender]);
            if(devDividend > 0){
    ...
    {
        return (11111111111111, 1111111111111);
        PoolInfo storage pool = poolInfo[_pid];
        ...
        (, _pending) = calculateDividend(_pending, user.amount, isContractSender);
        ...
        _potential = poolReward
            .mul(userModifiedWeight)
            .div(pool.usersTotalWeight);
        (, _potential) = calculateDividend(_potential, user.amount, isContractSender);
    }
...
// File: contracts/MasterChefActivityTwoDelegate.sol

pragma solidity 0.6.12;



contract MasterChefActivityTwoDelegate is MasterChefActivityTwo, DelegateInterface {
    /**
