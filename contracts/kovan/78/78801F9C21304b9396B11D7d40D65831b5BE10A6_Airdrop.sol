...
contract Airdrop is IAirdrop, Ownable {
    ...
    function init(address owner_, address token_, bytes32 merkleRoot_, uint256 expireTimestamp_) external {
        require(!initialized, "Airdrop: Initialized");
        initialized = true;
    ...
    function sweep(address token_, address target) external onlyOwner {
        require(block.timestamp >= expireTimestamp || token_ != token, "Airdrop: Not expired");
...
contract AirdropERC721 is IAirdrop, Ownable {
    using SafeERC20 for IERC20;
...
contract AirdropFactory is CloneFactory {
    event CreateAirdrop(address airdrop, bytes32 ipfsHash);
    ...
    function createAirdrop(
        ...
        bytes32 salt,
        bytes32 ipfsHash
    ) external returns (Airdrop drop) {
        drop = Airdrop(createClone(template, salt));
        drop.init(msg.sender, token, merkleRoot, expireTimestamp);
        emit CreateAirdrop(address(drop), ipfsHash);
    }

    function computeAirdropAddress(
        address template,
        bytes32 salt
    ...
    function isAirdrop(address template, address query) external view returns (bool) {
