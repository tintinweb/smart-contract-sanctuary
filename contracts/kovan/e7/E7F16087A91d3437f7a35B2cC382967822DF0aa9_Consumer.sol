...
contract Consumer is ChainlinkClient, Ownable {
  ...
  function validateName(string memory _name) public returns (bytes32 requestId_) {
  ...
  function fulfillValidateName(bytes32 _requestId, bool _clean) public recordChainlinkFulfillment(_requestId) returns (bool) {
    (bool success,) = address(registry).call(abi.encodeWithSignature("fulfillRequest(bytes32,bool)", _requestId, _clean));
    return success;
  }
