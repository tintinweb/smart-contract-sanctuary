...
contract PoHTwitterV4 is PoHTwitterAPI {
    event Verification(
    ...
    function verify(string memory _tweetId) external {
        ...
        requestTweetAddress(_tweetId);
    }

    /**
     * Receive the response in the form of uint256
     */
    function fulfillTweetAddressRequest(bytes32 _requestId, bytes32 _data)
        public
        override
        recordChainlinkFulfillment(_requestId)
    {
        RequestData storage request = requests[_requestId];
        bytes32 _caller = keccak256(abi.encodePacked(request.caller));
        require(_caller == _data, "Not same address, caller and tweet");
        requestTweetAuthor(request);
    }
    ...
    function userId() external view returns (uint256) {
        return verified[_msgSender()];
    }

    function userId(address _someAddress) external view returns (uint256) {
        return verified[_someAddress];
    }
}
...
abstract contract PoHTwitterAPI is ChainlinkClient, Ownable {
    ...
    bytes32 public getBytes32JobId;
    bytes32 public postBytes32JobId;
    uint256 public oracleFee;
    string public apiAddressURL;
    string public apiAuthorURL;
    string public baseQuery;
    ...
    constructor() {
        ...
        getBytes32JobId = "50fc4215f89443d185b061e5d7af9490";
        getUInt256JobId = "29fa9aa13bf1468788b7cc4a500a45b8";
        postBytes32JobId = "b9fd06bb42dd444db1b944849cbffb11";
        oracleFee = (1 * LINK_DIVISIBILITY) / 10;
        apiAddressURL = "https://pohtwitter.web.app/tweet_address_hash";
        apiAuthorURL = "https://pohtwitter.web.app/tweet_author";
        baseQuery = "tweetId=";
    ...
    function makeRequest(
        ...
        // string memory _url = string(abi.encodePacked(_apiBaseURL, _tweetId));
        string memory _query = string(abi.encodePacked(baseQuery, _tweetId));
        ...
        emit NewRequest(_apiURL, _path, _tweetId, _caller, _query, requestId);
    }

    function requestTweetAddress(string memory _tweetId)
        internal
        returns (bytes32 requestId)
    {
        return
            makeRequest(
                getBytes32JobId,
                this.fulfillTweetAddressRequest.selector,
                apiAddressURL,
                "data",
                _tweetId,
                _msgSender()
    ...
    function requestTweetAuthor(RequestData storage _previousRequest)
        internal
        returns (bytes32 requestId)
    {
        return
     ...
     * Receive the response in the form of uint256
     */
    function fulfillTweetAddressRequest(bytes32 _requestId, bytes32 _address)
        public
        virtual;
