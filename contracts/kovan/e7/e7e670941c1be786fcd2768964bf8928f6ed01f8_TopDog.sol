...
contract BoneToken is ERC20("BONE DOGECOIN KILLER", "BONE"), Ownable {
    ...
    uint256 public lockNums = 0;
    uint256 public lockingPeriod = 15724800;
    uint256 public devLockingPeriod = 86400;

    mapping(uint256 => bool) public lockTokensClaimed;
    mapping(uint256 => LockInfo) public lockInfoBook;
    mapping(address => uint256[]) public claimedLockIdByUsers;
    mapping(address => uint256[]) public unclaimedLockIdByUsers;
    ...
    function lock(address _holder, uint256 _amount, bool _isDev) public onlyOwner {
        ...
        uint256 lockId = lockNums;
        lockNums++;
        lockInfoBook[lockId] = LockInfo(_holder, _amount, now, _isDev);
        // uint256[] storage arrayId = lockIdByUsers[_holder];
        // arrayId.push(lockId);
        // lockIdByUsers[_holder] = arrayId;
        unclaimedLockIdByUsers[_holder].push(lockId);
    }


    // function to claim all locked tokens, locked at different times at once
    function claimAll() public {
        uint256[] memory arrayId = unclaimedLockIdByUsers[msg.sender];
        uint256 totalTransferableAmount = 0;
        uint i;
        for(i = 0 ; i < arrayId.length ; i++){
            uint256 _lockId = arrayId[i];
            if(!lockTokensClaimed[_lockId]){
                LockInfo memory lockInfo = lockInfoBook[_lockId];
                require(msg.sender == lockInfo._holder, "Not the token owner");
                uint256 lockingPeriodHere=lockingPeriod;
                if(lockInfo._isDev){
                    lockingPeriodHere = devLockingPeriod;
                }
                
                if(lockTokensClaimed[_lockId] == false && now >= (lockInfo._timestamp + lockingPeriodHere)){
                    totalTransferableAmount += lockInfo._amount;
                    
                    lockTokensClaimed[_lockId] = true;
                    claimedLockIdByUsers[lockInfo._holder].push(_lockId);
                    
                }
            }
            
        }
        _transfer(address(this), msg.sender, totalTransferableAmount);
    }
...
contract TopDog is Ownable {
    ...
    address public tBone;
    address public xShib;
    address public xLeash;
    ...
    // reward percentage to be sent to user dirctly
    uint256 public rewardMintPercent = 33;
    // devReward percentage to be sent to user dirctly
    uint256 public devRewardMintPercent = 50;
    ...
    function updatePool(uint256 _pid) public {
        ...
        bone.mint(devaddr, devBoneReward.mul(devRewardMintPercent).div(100)); // partial devPercent rewards to dev address

        bone.mint(address(bone), devBoneReward.sub(devBoneReward.mul(devRewardMintPercent).div(100))); // rest devPercent rewards locked to bone token contract
        bone.lock(devaddr, devBoneReward.sub(devBoneReward.mul(devRewardMintPercent).div(100)), true);

        bone.mint(tBone, boneReward.mul(tBonePercent).div(100)); // tBonePercent rewards to tBone address
        bone.mint(xShib, boneReward.mul(xShibPercent).div(100)); // xShibPercent rewards to xShib address
        bone.mint(xLeash, boneReward.mul(xLeashPercent).div(100)); // xLeashPercent rewards to xLeash address
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                ...
                safeBoneTransfer(address(bone), pending.sub(sendAmount)); // Rest amount sent to Bone token contract
                bone.lock(msg.sender, pending.sub(sendAmount), false); //function called for token time-lock
            }
        }
        if(_amount > 0) {
    ...
    function devUpdate(address _devaddr) public onlyOwner {
        devaddr = _devaddr;
    }
    ...
    function setLockingPeriod(uint256 _newLockingPeriod, uint256 _newDevLockingPeriod) public onlyOwner{
        bone.setLockingPeriod(_newLockingPeriod, _newDevLockingPeriod);
    }

    // Update tBone address by the owner.
    function tBoneUpdate(address _tBone) public onlyOwner {
        tBone = _tBone;
    }

    // Update xShib address by the owner.
    function xShibUpdate(address _xShib) public onlyOwner {
        xShib = _xShib;
    }

    // Update xLeash address by the owner.
    function xLeashUpdate(address _xLeash) public onlyOwner {
        xLeash = _xLeash;
    }
    ...
    function tBonePercentUpdate(uint _tBonePercent) public onlyOwner {
        require(_tBonePercent <= 5, "topDog: Percent too high");
    ...
    function xShibPercentUpdate(uint _xShibPercent) public onlyOwner {
        require(_xShibPercent <= 5, "topDog: Percent too high");
    ...
    function xLeashPercentUpdate(uint _xLeashPercent) public onlyOwner {
        require(_xLeashPercent <= 5, "topDog: Percent too high");
