...
contract Auction is Ownable {
    ...
    uint256 public stateChangedTime = block.timestamp;
    uint256 public bidSpan;
    uint256 public minBidAmount;
    uint256 public bidderCount;
    IKeeperImport keeperImport;
    uint256 max_num_bids;
    ...
    event StateChanged(State newState, uint256 changedTime);
    ...
    function setMaxNumBids(uint256 num) external inState(State.Bid) onlyOwner {
        require(num > max_num_bids, "only increase");
        max_num_bids = num;
    }
    ...
    function bid(address _asset, uint256 _amount) external inState(State.Bid) {
        ...
        require(userBids.bidIds.length < max_num_bids, "exceed max_num_bids");
    ...
    {
        return (uint256(state), stateChangedTime, bidSpan);
    }
    ...
    function getUserBids(address user) public view returns (UserBids memory) {
        return allUserBids[user];
    }

    function getUserBidsCount(address user) public view returns (uint256) {
        return allUserBids[user].bidIds.length;
    }
    ...
    function changeState(State newState) private {
        ...
        emit StateChanged(newState, stateChangedTime);
    }

    function cancelAllOrRefund(bool isCancel) private {
        uint256[] memory assetAmounts = new uint256[](assetList.length);
        UserBids storage userBids = allUserBids[msg.sender];
