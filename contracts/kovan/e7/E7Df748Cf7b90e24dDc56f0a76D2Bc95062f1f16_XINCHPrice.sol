...
/// @title XINCH Asset Price
/// @notice This contract gets the price of xINCH in USDC terms
/// @dev This contract is expected to call the xINCH function to get the INCH balance held by xINCH in weis
contract XINCHPrice is Initializable, Price {
    using SafeMath for uint256;

    /// @notice Upgradeable smart contract constructor
    /// @dev Just provide the addresses of the price feeders and the xINCH asset
    /// @param _underlyingAssetAddress (address) xINCH asset address
    /// @param _underlyingPriceFeedAddress (address) ChainLink INCH-USD aggregator address
    ...
    /// @notice Provides the amount of INCH held by the xINCH asset in wei
    /// @return  (uint256) INCH held by xINCH in wei
    function getAssetHeld() public view virtual override returns (uint256) {
        return IXINCH(underlyingAssetAddress).getNav();
    }
}
