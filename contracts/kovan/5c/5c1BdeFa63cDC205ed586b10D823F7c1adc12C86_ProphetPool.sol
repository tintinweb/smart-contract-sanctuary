...
interface IProphetPoolFactory {
     ...
     */
    function create(string memory _poolName, address _buyToken, address _poolToken, address _manager) external returns (address);
...
contract ProphetPool is VRFConsumerBase, ReentrancyGuard, Ownable {
    ...
    IERC20Mintable private poolToken;
     ...
     * @param _poolToken            ERC20 token to be issued when enter the pool
    ...
    // ERC20("PoolTokens", "LOT") //Internal LOT ERC20 token
    {
        poolName = _poolName;
        enterToken = IERC20(_enterToken);
        poolToken = IERC20Mintable(_poolToken);
    ...
    function settlePool() external {
        ...
        _transferEnterToken(owner(), feeAmount);
     ...
     * from the pool contract to their own address. The winners will need to burn their
     * LOT tokens to claim the pool rewards. This is executed by the pool contract itself.
     */
    function collectRewards() public nonReentrant {
        ...
        for (uint256 i = 0; i < poolConfig.participantLimit; i = i.add(1)) {
            ...
            if (winnerIndexes.contains(i)) {
                ...
                // if loser
                poolToken.mint(player, poolConfig.enterAmount);
            }
        }
        _resetPool();
    }
     ...
     * @param index                 Index of the participants array
     */
     ...
     * The participant is also issued an equal amount of LOT tokens once entered.
     * This LOT tokens are fundamental to the pool contract and are used internally.
     * The winners will need to burn their LOT tokens to claim the pool winnings.
     * The other participants of the pool can keep hold of these tokens and use for other applications.
     */
    function _enterPool() internal returns(uint256) {
        participants.push(msg.sender);

        totalEnteredAmount = totalEnteredAmount.add(poolConfig.enterAmount);
        // call _mint from constructor ERC20
        // Not giving loser pool tokens !!
        // poolToken.mint(msg.sender, poolConfig.enterAmount);
        ...
        return (participants.length).sub(1);
    }
    ...
    function _resetPool() internal {
        ...
        // );
        uint256 tokenBalance = poolToken.balanceOf(address(this));
        ...
        if (tokenBalance > 0) {
            enterToken.transfer(owner(), tokenBalance);
        }
    }
