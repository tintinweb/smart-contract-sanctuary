...
contract PlennyDistribution is Ownable {
    ...
    function distribute(string memory name, string memory symbol, bytes32 salt,
        uint256 _plennyTotalSupply, address payable owner)
        public onlyOwner {
        ...
        PlennyERC20(plennyToken).initialize(name, symbol);
        PlennyERC20(plennyToken).mint(msg.sender, _plennyTotalSupply);

        PlennyERC20(plennyToken).transferOwnership(owner);
        PlennyERC20(plennyToken).addPauser(owner);
    ...
    function plennyTotalSupply() external view returns (uint256){
        return IPlennyERC20(plennyToken).totalSupply();
    }
}