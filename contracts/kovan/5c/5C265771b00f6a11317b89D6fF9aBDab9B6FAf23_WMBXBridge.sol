...
contract WMBXBridge is ReentrancyGuard, Context, Ownable {
  ...
  /*function exchangeTokens() public payable nonReentrant {
    require(!isFrozen, "Contract is frozen");
    require(msg.value >= feeRate, "Fee not met");
    require(TOKEN.allowance(msg.sender, address(this)) >= 0, "No allowance");
    uint256 amt2send = TOKEN.balanceOf(msg.sender);
    require(TOKEN.transferFrom(msg.sender, address(this), amt2send), "Unable to transfer funds");
    feeAddress.transfer(msg.value);
    emit Exchange(msg.sender, amt2send, msg.value);
  }*/
