...
contract ShadowFaxStorage is ShadowFaxAdminStorage {
    ...
    mapping(uint256 => mapping(address => UserInfo)) userInfo; // publicId=>user address =>UserInfo
    PublicOffering[] public publicOfferings;
    mapping(address => address) ascription; // token => pubilsher
    ...
    , uint256 wantTokenAmount, address publisher,uint256 startTime,uint256 endTime);
...
contract ShadowFax is ShadowFaxStorage, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function addPublicOffering(
        ...
        require(_endTime <= _startTime + maximalTimeSpan * 1 days, "Invaild end time");
        require(_endTime >= _startTime + minimalTimespan * 1 days, "Invaild end time");
        ...
        , _wantToken, _wantTokenAmount, msg.sender, _startTime, _endTime);
    }
    ...
    function harvest(uint256 _publicId) public nonReentrant {
    ...
    function resetTimeSpan(uint _maxTimespan, uint _minTimespan) onlyAdmin public {
        require(_maxTimespan > minimalTimespan, "Invalid parameter");
