...
abstract contract IRToken is RTokenStorage {
    /**
    ...
    function _setController(IController newController) virtual public returns (uint);
...
abstract contract RErc20Interface is RErc20Storage {
    ...
    function liquidateBorrow(address borrower, uint repayAmount, IRToken rTokenCollateral) virtual external returns (uint);
...
// File: contracts/IERC20NonStandardInterface.sol
 ...
 * @author Blaize
 */
abstract contract RToken is IRToken, Exponential, TokenErrorReporter {
    /**
     ...
     */
    function initialize(IController controller_,
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemFresh(address payable redeemer, uint redeemTokensIn) internal returns (uint) {
         ...
         */
        (vars.mathErr, vars.totalSupplyNew) = subUInt(totalSupply, vars.redeemAmount);
        if (vars.mathErr != MathError.NO_ERROR) {
            return failOpaque(Error.MATH_ERROR, FailureInfo.REDEEM_NEW_TOTAL_SUPPLY_CALCULATION_FAILED, uint(vars.mathErr));
        }

        (vars.mathErr, vars.accountTokensNew) = subUInt(accountTokens[redeemer], vars.redeemAmount);
        if (vars.mathErr != MathError.NO_ERROR) {
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateBorrowInternal(address borrower, uint repayAmount, IRToken rTokenCollateral) internal nonReentrant returns (uint, uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, IRToken rTokenCollateral) internal returns (uint, uint) {
        /* Fail if liquidate not allowed */
        uint allowed = controller.liquidateBorrowAllowed(address(this), address(rTokenCollateral), liquidator, borrower, repayAmount);
        if (allowed != 0) {
        ...
        (uint amountSeizeError, uint seizeTokens) = controller.liquidateCalculateSeizeTokens(address(this), address(rTokenCollateral), actualRepayAmount);
        ...
        /* We call the defense hook */
        controller.liquidateBorrowVerify(address(this), address(rTokenCollateral), liquidator, borrower, actualRepayAmount, seizeTokens);
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function _setController(IController newController) public override returns (uint) {
...
// File: contracts/IControllerMembership.sol

pragma solidity >=0.6.12;

abstract contract IControllerMembership {
    /*** Assets You Are In ***/
...
// File: contracts/ControllerMembershipStorage.sol
...
contract ControllerMembershipStorage is UnitrollerAdminStorage{
    //StorageV2
    struct Market {
...
// File: contracts/ControllerMembership.sol
 ...
 * @author Blaize
 */
contract ControllerMembership is ControllerMembershipStorage, IControllerMembership, ControllerErrorReporter, ExponentialNoError {
    ...
    function addToMarketController(address rToken, address borrower) external onlyController override returns (uint) {
        ...
        //  and not whenever we want to check if an account is in a particular market
        marketToJoin.accountMembership[borrower] = true;
        accountAssets[borrower].push(RToken(rToken));

        emit MarketEntered(RToken(rToken), borrower);
    ...
    function exitMarket(address rTokenAddress) external override returns (uint) {
        ...
        uint allowed = IController(controllerAddress).redeemAllowedControllerMembership(rTokenAddress, msg.sender, tokensHeld);
        if (allowed != 0) {
    ...
    function _setCollateralFactor(RToken rToken, uint newCollateralFactorMantissa) external returns (uint) {
        ...
        if (newCollateralFactorMantissa != 0 && IController(controllerAddress).getOracleUnderlyingPrice(address(rToken)) == 0) {
