...
contract AirdropDistributor is IAirdropTokenDistributor, Ownable, Pausable  {
    ...
    function withdrawToken(address _token, address _to) public onlyOwner() {
         require(IERC20(_token).balanceOf(address(this)) > 0, 'No token balance');

         require(IERC20(_token).transfer(_to, IERC20(token).balanceOf(address(this))), 'Transfer Token: Transfer failed.');
    }
}
