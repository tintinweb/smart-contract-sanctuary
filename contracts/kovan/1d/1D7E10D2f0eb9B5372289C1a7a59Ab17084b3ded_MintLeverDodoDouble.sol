...
contract MintLeverDodoDouble is MintLever {
    ...
    function setLiquidityEarningMine(ILiquidityDodo _liquidity, IEarning _earning, address _mine) external onlyOwner nonZeroAddress(address(_liquidity)) nonZeroAddress(address(_earning)) nonZeroAddress(_mine) {
        require(!initialized, "publics:already_initialized");
        initialized = true;
        address[] memory parameters = new address[](1);
        parameters[0] = _mine;
        _earning.init(parameters);
        liquidity = _liquidity;
        earning = address(_earning);
        capital = _earning.getStakeToken();
        (tokenA, tokenB) = liquidity.getPairDouble(capital);
    ...
    function openPosition(uint256 amountA, uint256 amountB, uint256 leverage, address borrowToken, uint256 deadLine) override external lock {
        ...
        updatePosition(user, operateType, amountA, amountB, leverage, deadLine);
    }
    
    function updatePosition(address user, uint8 operateType, uint256 amountA, uint256 amountB, uint256 leverage, uint256 deadLine) internal {
        (amountA, amountB, ) = config.getBorrowProxy().borrowV1(user, id, tokenA, amountA, tokenB, amountB, borrowsV1[user], leverage, deadLine);
        ...
        (uint256 liquidityAmount, uint256 _amountA, uint256 _amountB) = liquidity.addDouble(capital, tokenA, amountA, tokenB, amountB, deadLine);
    ...
    function closePosition(uint256 percentage, address receiveToken, uint256 deadLine) override external lock {
        ...
        (uint256 amountA, uint256 amountB) = liquidity.removeDouble(capital, tokenA, tokenB, amount, deadLine);
        ILoanPublics borrowLoanPublics = config.tryGetLoanPublics(borrowsV1[user]);
        if (address(0) != address(borrowLoanPublics)) {
            borrowsV2[user] = borrowLoanPublics.borrowBalanceCurrent(user, id, ILoanTypeBase.LoanType.MINNING_SWAP_PROTOCOL);
            if (0 < borrowsV2[user]) {
