...
contract PoolStake is Owned {
    ...
    struct User {
        ...
        bool isProvider;            //identifies if user has liquidity locked in 
    }
    ...
    function addLiquidity(uint _term, uint _multiplier) external payable {
        ...
        if (!_users[msg.sender].isProvider) {
            _users[msg.sender].isProvider = true;
            totalProviders++;
        }
    ...
    function withdrawLiquidity(uint _lpAmount) external returns(bool) {
        ...
        if (_users[msg.sender].liquidity == 0) {
            _users[msg.sender].isProvider = false;
