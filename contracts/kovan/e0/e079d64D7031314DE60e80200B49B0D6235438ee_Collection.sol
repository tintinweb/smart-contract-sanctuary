...
contract Collection is ERC1155Burnable, Ownable {
    mapping(uint256 => bytes) private _metadata;

    event StatusChanged(uint256 id, string status, string info);
    ...
    function readData(uint256 _id) public view returns (string memory) {
        return string(_metadata[_id]);
    }
    ...
    function setStatus(
        ...
        emit StatusChanged(_id, _status, _info);
    }
}