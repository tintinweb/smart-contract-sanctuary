...
contract BrightRiskToken is ERC20Upgradeable, OwnableUpgradeable, IBrightRiskToken {
    ...
    function stakeAllDeposits(address _controllerAddress) external {
        //TOOD Stake all available
        //TODO Mint and distrbute to 'depositors'
        //TODO DON'T mint for depositors == position controllers
        //TODO clean up 'deposits[]'
    }

    function stakeAt(address _controllerAddress, uint256 _amount) external {
        require(_amount > 0, "BrightRiskToken: BRI5");
        //TODO Fix the check for 'pending'
    ...
    function callUnstake(address _controllerAddress) external {
        uint256 _calledAmount = IPositionController(_controllerAddress).callUnstake();
        //TODO emit event
    }

    function unstake(address _controllerAddress) external {
        //TODO Allow the controller to spend balance of intestment asset
        uint256 _unstakedAmount = IPositionController(_controllerAddress).unstake();
        //TODO compare increase of 'base' balance here
        //TODO burn this in proportion (?) Or Don't
    }
    ...
    {
        return
            _amount.mul(PERCENTAGE_100).div(_positionToIndexRatio(_controllerAddress)).div(
                PRECISION
            );
    }
