...
contract NFTMarket is ReentrancyGuard, ChainlinkClient, Ownable {
    ...
    constructor() Ownable() {
        ...
        ORACLE = 0xb99A5f5a5530DF9926026CD62a2212cEc63632f5;
        //JOBID = "c51694e71fa94217b0f4a71b2a6b565a";  // initial setting, can be changed by Owner
        // JOBID = "b4f166b4c955493f80258aaa0c5181ae";
        JOBID = "d80aed6e720144ac82b18c1433068dab";
        FEE = 1 * 10 ** 17;
    ...
    function fulfill(bytes32 _requestId, bytes32 _commission) public recordChainlinkFulfillment(_requestId)
    {
        ...
        idToMarketItem[lastItemId].commission = asciiToInteger(stringToBytes32(splitArr[0]));
        isPremiumBytes = stringToBytes32(splitArr[1]);
        if(isPremiumBytes == "premium"){
            idToMarketItem[lastItemId].isPremium = true;
        }else{
            idToMarketItem[lastItemId].isPremium = false;
        }
    ...
    function createMarketSale(
        ...
        require(!idToMarketItem[itemId].sold, "Order was executed");
    ...
    event ResolveDeal(uint256 indexed itemId, uint256 value);
    event RejectDeal(uint256 indexed itemId, uint256 value);

    function resolveDeal(uint256 itemId) external onlyOwner returns(bool) {
        require(_itemIds.current() >= itemId, "Market item not found");
        require(!idToMarketItem[itemId].isResolved, "Payment has been made");
    ...
    function rejectDeal(uint256 itemId, uint256 fee) external onlyOwner returns(bool) {
        ...
        require(_itemIds.current() >= itemId, "Market item not found");
        require(!idToMarketItem[itemId].isRejected, "Eth refund");
