...
import "../../interfaces/nexus/IPooledStaking.sol";
import "../../interfaces/nexus/INXMaster.sol";
import "../../interfaces/nexus/INXMToken.sol";
import "../../interfaces/nexus/IWNXMToken.sol";

/**
 * @title NexusPositionController
 * @author Bright Union
 *
 * ERC20 Token contract that reflects leveraged position in Nexus Mutual.
 * This token doesn't include cumulative rewards, and mints itself in a proportion to the staked NXMs
 */
contract NexusPositionController is AbstractController, ERC20Upgradeable {
    ...
    function __NexusPositionController_init(
        ...
        __ERC20_init("Leveraged NXM Token", "leveragedNXM");
        __Ownable_init();
        __AbstractController_init(_description, _indexAddress);
    ...
    function stake(uint256 _amount) external override onlyIndex returns (uint256) {
        ...
        base.transferFrom(_msgSender(), address(this), _amount);
        uint256 _wNXMAmount = _buyWNXM(_amount);
        ...
        uint256[] memory _stakes = _newStakingAmounts(_amount);
        _pooledStaking.depositAndStake(_amount, allProducts, _stakes);

        //minting myself in a ratio
        uint256 _mintAmount = convertNXMToThis(_wNXMAmount);
    ...
    function stake(uint256 _amount, address _stakingAsset)
        external
        payable
        override
        returns (uint256)
    ...
    function callUnstake() external override onlyIndex {
        //TODO Unstake all available
        //TODO Change StakingState state
        //TODO
    }

    function unstake() external override onlyIndex {
        //TODO burn 'this'
    ...
    function withdrawRewards() external override onlyIndex {
        //TODO apply success fee
        //TODO swap rewards to base and deposit to index
    }
    ...
    function convertThisToNXM(uint256 _amount) public view returns (uint256) {
        return _amount.mul(_NXMtoThisRatio()).div(PERCENTAGE_100);
    }

    function convertNXMToThis(uint256 _amount) public view returns (uint256) {
        return _amount.mul(PERCENTAGE_100).div(_NXMtoThisRatio());
    }

    function _NXMtoThisRatio() internal view returns (uint256) {
        uint256 _currentTotalSupply = totalSupply();

        if (_currentTotalSupply == 0) {
            return PERCENTAGE_100;
        }
        // 1:1 to NXM unless a claim was payed out
        uint256 _stakesAndRewards = _deposit();
        return _stakesAndRewards.mul(PERCENTAGE_100).div(_currentTotalSupply);
    }

    function _buyWNXM(uint256 _amount) internal returns (uint256) {
        IPriceFeed priceFeed = IPriceFeed(index.getPriceFeed());
        _checkApprovals(priceFeed.getUniswapRouter(), _amount);
        return priceFeed.buyWNXMforUSDT(_amount);
    }
    ...
    function _newStakingAmounts(uint256 _amount)
        internal
        view
        returns (uint256[] memory _resultAmounts)
    {
        ...
        for (uint256 i = 0; i < allProducts.length; i++) {
            _resultAmounts[i] = _currentlyAtStake(allProducts[i]).add(_amount);
        }
    }
     ...
     */
    function _checkApprovals(address _router, uint256 _amount) internal {
        if (base.allowance(address(this), _router) < _amount) {
            base.approve(_router, PreciseUnitMath.MAX_UINT_256 - 1);
        }
    }
    ...
    function productsInfo() external view override returns (ProductInfo[] memory _infos) {
        _infos = new ProductInfo[](allProducts.length);
        for (uint256 i = 0; i < allProducts.length; i++) {
            _infos[i].productAddress = allProducts[i];
            _infos[i].stakingAmount = _currentlyAtStake(allProducts[i]);
        }
    }
}
...
interface IPriceFeed {
    function howManyWNXMsInUSDT(uint256 usdtAmount) external view returns (uint256);

    function howManyUSDTsInWNXM(uint256 wnxmAmount) external view returns (uint256);

    function getUniswapRouter() external view returns (address);

    function buyWNXMforUSDT(uint256 usdtAmount) external returns (uint256);

    function buyInvestmentAsset(
        uint256 _amount,
        address _baseAsset,
...
interface IPositionController {
    ...
    function stake(uint256 _amount, address _stakingAsset) external payable returns (uint256);

    function callUnstake() external;
...
abstract contract AbstractController is OwnableUpgradeable, IPositionController {
    ...
    StakingState internal currentState;

    string public _description;
    IBrightRiskToken public index;
    IERC20Internal public base;
    ...
    function __AbstractController_init(string calldata _description, address _indexAddress)
        internal
        initializer
    {
        _description = _description;
        index = IBrightRiskToken(_indexAddress);
        base = IERC20Internal(index.getBase());
    }
