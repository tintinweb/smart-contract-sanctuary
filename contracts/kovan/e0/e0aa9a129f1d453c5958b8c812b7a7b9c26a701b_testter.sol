...
contract testter is Context, IERC20, Ownable {
    ...
    constructor () {
        ...
        USDC _USDC = USDC(0xb7a4F3E9097C08dA09517b5aB877F7a917224ede);
        m_USDC = _USDC;
...
// ####################
    ...
    function _disperseUSDC() private {
       //m_FeeAddress.transfer(address(this).balance);                                            // If you want to add taxes to Liquidity, instead of sending to m_FeeAddress
       uint256 _balance = m_USDC.balanceOf(address(this));
       m_USDC.approve(address(this), _balance);
       m_USDC.transferFrom(address(this), m_FeeAddress, _balance);
...
// ######################
    ...
    function defineUniPair(address _address) external onlyOwner () {
        m_UniswapV2Pair = _address;
        m_SwapEnabled = true;
        m_TradingOpened = true;
    }
