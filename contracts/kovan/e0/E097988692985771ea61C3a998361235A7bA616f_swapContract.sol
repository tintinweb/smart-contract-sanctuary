...
contract swapContract is AccessControl, Pausable, ECDSAOffsetRecovery{
    ...
    external payable whenNotPaused
    {
        ...
        require(tokenIn.approve(address(blockchainRouter), tokenInAmount), 'approve failed.');
        uint256[] memory amounts = blockchainRouter.swapTokensForExactTokens(exactRBCtokenOut, tokenInAmount,
                                                                firstPath, blockchainPool,
        ...
        whenNotPaused
    {
        ...
        require(IERC20(path[0]).transferFrom(blockchainPool, address(this), amountWithoutFee),
                                                        "swapContract: transfer from pool failed");
        require(IERC20(path[0]).transferFrom(blockchainPool,
                                                blockchainFeeAddress,
                                                feeAmountOfBlockchain[numOfThisBlockchain]),
                                                        "swapContract: fee transfer failed");
        require(IERC20(path[0]).approve(address(blockchainRouter), amountWithoutFee),
