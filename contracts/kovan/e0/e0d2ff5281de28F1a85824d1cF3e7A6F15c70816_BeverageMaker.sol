...
interface IFeeSplitExtension {
    ...
    function setToken() external returns (IERC20 _setToken);
}
...
contract BeverageMaker is Ownable {
    ...
    function convert(IFeeSplitExtension _feeSplitExtension) external onlyEOA {
        _convert(_feeSplitExtension);
    }

    function convertMultiple(IFeeSplitExtension[] calldata _feeSplitExtensions) external onlyEOA {
        for (uint256 i = 0; i < _feeSplitExtensions.length; i++) {
            _convert(_feeSplitExtensions[i]);
        }
    }

    function _convert(IFeeSplitExtension _feeSplitExtension) private {
        // accrue and distribute SetToken fees 
        _feeSplitExtension.accrueFeesAndDistribute();

        address token0 = address(_feeSplitExtension.setToken());
...
contract BeverageMakerExploitMock {
    ...
    function convert(IFeeSplitExtension feeSplitExtension) external {
        beverageMaker.convert(feeSplitExtension);
    }
}
