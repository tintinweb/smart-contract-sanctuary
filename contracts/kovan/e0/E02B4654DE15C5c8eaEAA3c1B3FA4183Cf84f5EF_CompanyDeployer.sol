...
struct IdentityData {
    ...
    bytes8 lastName;
    bytes8 dateOfBirth;
    bytes8 placeOfLiving;
    bytes8 countryOfLiving;
    bytes8 email;
    bytes8 birthday;
}
...
contract BlockchainRegistryContract is AccessControl {
    ...
    Counters.Counter private _identityId;
    
    CompanyRegistryInterface private _companyRegistryInstance;

    mapping (uint => IdentityData) _IdDataMap;
    mapping (address => uint) _addressToIdMap;
    mapping (uint => address) _idToAddressMap;
    
    mapping (address => bool) _frozenAccounts;
    
    mapping (address => bool) _whitelistedContractAddresses;
    
    event IdentityAdded(address indexed userAddress, uint identityId);
    event IdentityEdited(uint identityId);
    
    constructor(address defaultAdminAddress, address companyRegistryAddress) {
        _identityId.increment();
    ...
    /// User section
    ///
    function editIdentityInformation(
        ...
        bytes8 countryOfLiving, 
        bytes8 email, 
        bytes8 birthday) public {
        
        require (hasRole(MODERATOR_ROLE, msg.sender) || msg.sender == querryAddress);
        uint identityId = getIdentityId(querryAddress);
        require(identityId != 0, "Address is not verified!");
        
        if (title.length != 0){
            _IdDataMap[identityId].title = title;
        }
        if (firstName.length != 0){
            _IdDataMap[identityId].firstName = firstName;
        }
        if (lastName.length != 0){
            _IdDataMap[identityId].lastName = lastName;
        }
        if (dateOfBirth.length != 0){
            _IdDataMap[identityId].dateOfBirth = dateOfBirth;
        }
        if (placeOfLiving.length != 0){
            _IdDataMap[identityId].placeOfLiving = placeOfLiving;
        }
        if (countryOfLiving.length != 0){
            _IdDataMap[identityId].countryOfLiving = countryOfLiving;
        }
        if (email.length != 0){
            _IdDataMap[identityId].email = email;
        }
        if (birthday.length != 0){
            _IdDataMap[identityId].birthday = birthday;
        }
        
         emit IdentityEdited(identityId);
    }
    ...
    function addNewIdentity(
        ...
        _IdDataMap[_identityId.current()] = IdentityData(title, firstName, lastName, dateOfBirth, placeOfLiving, countryOfLiving, email, birthday);
        _addressToIdMap[identityAddress] = _identityId.current();
        _idToAddressMap[_identityId.current()] = identityAddress;
        
         emit IdentityAdded(identityAddress, _identityId.current());
        
        _identityId.increment();
    }
    
    function changeIdentitiesAddress(uint identityId, address newAddress) public returns (bool) {
        require (hasRole(MODERATOR_ROLE, msg.sender));

        address oldAddress = _idToAddressMap[identityId];
        require(oldAddress != address(0x0), "Identity ID is not set!");
        _addressToIdMap[oldAddress] = 0;
        
        _idToAddressMap[identityId] = newAddress;
        _addressToIdMap[newAddress] = identityId;
    ...
    function whitelistContractsAddress(address contractAddress, bool option) public returns (bool) {
        ...
        require(getIdentityId(contractAddress) == 0, "Address already has user identity!");
    ...
    /// Public section
    ///
    function getIdentityId(address querryAddress) public view returns(uint) {
        uint identityId = _addressToIdMap[querryAddress];
        return identityId;
    }
    
    function getIdentityData(address querryAddress) public view returns(IdentityData memory) {
        return _IdDataMap[_addressToIdMap[querryAddress]];
    }
    ...
    function isUserAddressWhitelisted(address querryAddress) public view returns (bool) {
        if (_addressToIdMap[querryAddress] == 0) {
            return false;
        }else{
            return true;
        }
