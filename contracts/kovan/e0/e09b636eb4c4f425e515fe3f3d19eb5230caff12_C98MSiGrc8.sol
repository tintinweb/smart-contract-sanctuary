...
interface MSiG {
    ...
    function revoke() external returns (bool);
...
contract C98MSiGrc8 is MSiG {
    ...
    function owners() external view returns (address[] memory, uint256[] memory) {
...
contract C98MSiGrc8Factory {
    ...
    function createMulitSig(address[] memory owners_, uint16 requiredVote_)
        external returns (C98MSiGrc8 wallet) {
        wallet = new C98MSiGrc8(owners_, requiredVote_);
    }
}