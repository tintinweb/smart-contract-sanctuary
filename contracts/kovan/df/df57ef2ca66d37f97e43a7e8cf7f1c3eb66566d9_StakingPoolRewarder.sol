...
contract StakingPoolRewarder is OwnableUpgradeable, IStakingPoolRewarder {
    ...
    function calculateUserWithdrawable(address user) external view returns (uint256) {
        (uint256 withdrawable, ,) = calculateWithdrawableFromVesting(user);
        return withdrawable;
    }

    function calculateWithdrawableFromVesting(address user) private view returns (
        uint256 amount,
    ...
    function onReward(
        ...
        (uint256 lastVestedAmount,uint256 newClaimTime, bool allVested) = calculateWithdrawableFromVesting(user);
        ...
        if (lastVestedAmount > 0) {
            if (allVested) {
                ...
                vestingSchedules[user].lastClaimTime = uint32(newClaimTime);
            }
            emit TokenVested(user, lastVestedAmount);
        }

        uint256 newUnvestedAmount;
        uint256 newVestedAmount;
        if (amount > 0) {
        ...
        if (newUnvestedAmount > 0) {
            uint256 lastUnvestedAmount = (uint256(schedule.amount)).sub(lastVestedAmount);
            updateVestingSchedule(user, newUnvestedAmount.add(lastUnvestedAmount), 
                block.timestamp, 
