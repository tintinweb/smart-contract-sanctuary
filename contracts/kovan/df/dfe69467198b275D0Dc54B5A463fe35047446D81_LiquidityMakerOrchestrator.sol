...
import "./LiquidityMakerBondingCurve.sol";
import "../pcv/IPCVDeposit.sol";
import "@uniswap/lib/contracts/libraries/TransferHelper.sol";

/// @title a liquidity maker for token
/// @author Ring Protocol
contract ERC20LiquidityMakerBondingCurve is LiquidityMakerBondingCurve {
    ...
    constructor(
        ...
        address _rTokenVault
    )
        LiquidityMakerBondingCurve(
            core,
            pcvDeposits,
            ratios,
            duration,
    ...
    /// @notice purchase token for underlying tokens
    /// @param to address to receive liquidity maker token
    /// @param amountIn amount of underlying tokens input
    /// @return amountOut amount of liquidity maker token received
    function purchase(address to, uint256 amountIn)
        external
        payable
        virtual
...
interface ILiquidityMakerBondingCurve is IBondingCurve {
    
    event PurchaseLimitUpdate(uint256 _purchaseLimit);

    event RTokenVaultUpdate(address _rTokenVault);

    // ----------- State changing Api -----------
    function setPurchaseLimit(uint256 _purchaseLimit) external;

    function setRTokenVault(address _rTokenVault) external;
    ...
    function rTokenVault() external view returns (address);
}
...
import "./ILiquidityMakerBondingCurve.sol";
import "../refs/CoreRef.sol";
import "../pcv/PCVSplitter.sol";
import "../utils/Timed.sol";

/// @title an abstract liquidity maker for saving token
/// @author Ring Protocol
abstract contract LiquidityMakerBondingCurve is ILiquidityMakerBondingCurve, ERC20Burnable, CoreRef, PCVSplitter, Timed {
    ...
    /// @notice the total amount of token purchased in liquidity maker.
    uint256 public override totalPurchased;

    /// @notice the buffer applied on top of purchase amount
    ...
    /// @notice the limit of token can be purchase in liquidity maker.
    uint256 public override purchaseLimit;

    address public override rTokenVault;
    ...
    /// @param _purchaseLimit the limit of token can be purchase in liquidity maker
    /// @param _tokenName the token name of liquidity maker token
    /// @param _tokenSymbol the token symbol of liquidity maker token
    /// @param _rTokenVault vault to hold protocol fees
    constructor(
        address _core,
        ...
        Timed(_duration)
    {
        ...
        rTokenVault = _rTokenVault;

        _initTimed();
    }

    /// @notice sets the R roken vault address
    function setRTokenVault(address _rTokenVault) external override onlyGovernor {
        require(_rTokenVault != address(0), "LiquidityMakerBondingCurve: address is 0");
        rTokenVault = _rTokenVault;

        emit RTokenVaultUpdate(_rTokenVault);
    }

    /// @notice sets liquidity maker buffer
    function setBuffer(uint256 _buffer) external override onlyGovernor {
        require(
            _buffer < BUFFER_GRANULARITY,
            "LiquidityMakerBondingCurve: Buffer exceeds or matches granularity"
        );
        buffer = _buffer;
    ...
    /// @notice sets the purchase limit
    function setPurchaseLimit(uint256 _purchaseLimit) external override onlyGovernor {
        purchaseLimit = _purchaseLimit;
        emit PurchaseLimitUpdate(_purchaseLimit);
    }
    ...
    function allocate() external override whenNotPaused onlyGuardianOrGovernor {
        require((!Address.isContract(msg.sender)), "LiquidityMakerBondingCurve: Caller is a contract");
        uint256 amount = getTotalPCVHeld();
        require(amount != 0, "LiquidityMakerBondingCurve: No PCV held");
    ...
    /// @notice return amount of liquidity maker token received after a successful purchase
    /// @param amountIn the amount of underlying used to purchase
    /// @return amountOut the amount of liquidity maker token received
    function getAmountOut(uint256 amountIn)
        external
        view
        override
        returns (uint256 amountOut)
    {
        amountOut = _getBufferAdjustedAmount(amountIn);
    }

    /// @notice the amount of PCV held in contract and ready to be allocated
    function getTotalPCVHeld() public view virtual override returns (uint256);

    /// @notice mint liquidity maker token and send to liquidity maker destination
        ...
        returns (uint256 amountOut)
    {
        ...
        require(totalPurchased <= purchaseLimit, "LiquidityMakerBondingCurve: Save limit reached");
        _mint(to, amountOut);
        uint256 amountProtocol = amountIn - amountOut;
        if (amountProtocol > 0) {
            _mint(rTokenVault, amountProtocol);
        }
    ...
    /// @notice returns the buffer on the liquidity maker
    ...
    /// @notice permit spending of liquidity maker token
    /// @param owner the liquidity maker token holder
    ...
    function permit(
        ...
        require(deadline >= block.timestamp, "LiquidityMakerBondingCurve: EXPIRED");
        bytes32 digest =
            keccak256(
                abi.encodePacked(
                    "\x19\x01",
        ...
        require(
            ...
            "LiquidityMakerBondingCurve: INVALID_SIGNATURE"
        );
        _approve(owner, spender, value);
    }

    /// @notice this function is not used in liquidity maker
    function getAverageUSDPrice(uint256) external pure override returns (Decimal.D256 memory) {
        return Decimal.one();
    }
...
interface ISharedPCVDepositOrchestrator {
    function init(
        address core,
        address pool,
        address nft,
...
interface ISharedLMControllerOrchestrator {
    function init(
        address core,
        address erc20UniswapPCVDeposit,
        address liquidityMakerBondingCurve,
        address vaultAddress
    ) external returns (address);
}

interface ISharedLMBondingCurveOrchestrator {
    function init(
        address core,
        address erc20UniswapPCVDeposit,
...
import "../liquiditymaker/ERC20LiquidityMakerBondingCurve.sol";
import "./IOrchestrator.sol";
import "../oracle/IUniswapOracle.sol";
import "../pcv/IPCVDeposit.sol";
import "@uniswap/v3-periphery/contracts/interfaces/INonfungiblePositionManager.sol";

contract LiquidityMakerOrchestrator is IOrchestrator, Ownable {
    address public immutable core;
    address public immutable RUSD;
    address public immutable NFT;
    address public immutable ROUTER;
    address public immutable PCVDepositOrchestrator;
    address public immutable sharedLMControllerOrchestrator;
    address public immutable sharedLMBondingCurveOrchestrator;
    address public tokenRusdPool;
    address public tokenAddress;
    address public erc20UniswapPCVDeposit;
    address public uniswapOracle;
    address public liquidityMakerBondingCurve;
    address public liquidityMakerController;

    uint256 public constant BONDING_CURVE_ALLOCATE_INCENTIVE_FREQUENCY = 1 days; // 1 day duration

    uint256 public constant LIQUIDITY_MAKER_KEEPER_INCENTIVE = 0;

    event TokenUniswapV3Pool(address pool);
    ...
    constructor(
        ...
        address _sharedLMControllerOrchestrator,
        address _sharedLMBondingCurveOrchestrator
    ) Ownable() {
        core = _core;
        RUSD = _RUSD;
        NFT = _NFT;
        ROUTER = _ROUTER;
        PCVDepositOrchestrator = _PCVDepositOrchestrator;
        sharedLMControllerOrchestrator = _sharedLMControllerOrchestrator;
        sharedLMBondingCurveOrchestrator = _sharedLMBondingCurveOrchestrator;
    }

    function reset() public onlyOwner{
        tokenAddress = address(0);
        erc20UniswapPCVDeposit = address(0);
        uniswapOracle = address(0);
        tokenRusdPool = address(0);
        liquidityMakerBondingCurve = address(0);
    }

    /// @notice deploy step 1
    function setUniswapV3Pool(
        address _tokenAddress, 
        address _poolAddress
    ) external onlyOwner {
        reset();
        tokenRusdPool = _poolAddress;
        tokenAddress = _tokenAddress;
        emit TokenUniswapV3Pool(_poolAddress);
    }

    /// @notice deploy step 2
    function initPcvDeposit(uint32 twapDuration, int24 tickLower, int24 tickUpper) external onlyOwner {
        address _tokenRusdPool = tokenRusdPool;
        require(_tokenRusdPool != address(0) && tokenAddress != address(0), "tokenRusdPool or tokenAddress is 0");
        (address _erc20UniswapPCVDeposit, address _uniswapOracle) = 
        ISharedPCVDepositOrchestrator(PCVDepositOrchestrator).init(
            core,
            _tokenRusdPool,
            NFT,
            ROUTER,
        ...
        erc20UniswapPCVDeposit = _erc20UniswapPCVDeposit;
        uniswapOracle = _uniswapOracle;
    }

    /// @notice deploy step 3, deploy bonding curve
    function initBondingCurve(
        ...
        require(tokenAddress != address(0) &&
            erc20UniswapPCVDeposit != address(0),
            "tokenAddress or erc20UniswapPCVDeposit is 0"
        );
        liquidityMakerBondingCurve = address(ISharedLMBondingCurveOrchestrator(sharedLMBondingCurveOrchestrator)
        .init(
            core,
            erc20UniswapPCVDeposit,
            BONDING_CURVE_ALLOCATE_INCENTIVE_FREQUENCY,
            LIQUIDITY_MAKER_KEEPER_INCENTIVE,
            purchaseLimit,
            tokenName,
            tokenSymbol,
            tokenAddress,
     ...
     /// @notice deploy step 4, liquidity maker controller
    function initLiquidityMakerController(address vaultAddress) external onlyOwner {
       require(liquidityMakerBondingCurve != address(0) && erc20UniswapPCVDeposit != address(0), 
       "liquidityMakerBondingCurve or erc20UniswapPCVDeposit is 0");
       liquidityMakerController = 
       address(ISharedLMControllerOrchestrator(sharedLMControllerOrchestrator)
        .init(core, erc20UniswapPCVDeposit, liquidityMakerBondingCurve, vaultAddress));
    }

    // this is a helper function for test only.
    function initUniswapV3Pool(
        address _tokenAddress, 
        uint24 _fee,
        ...
        address _RUSD = RUSD;
        tokenRusdPool = INonfungiblePositionManager(NFT).createAndInitializePoolIfNecessary(
            _tokenAddress < _RUSD ? _tokenAddress : _RUSD, 
            _tokenAddress < _RUSD ? _RUSD : _tokenAddress, 
            _fee, 
            _sqrtPriceX96
        );
        tokenAddress = _tokenAddress;
        emit TokenUniswapV3Pool(tokenRusdPool);
    }
...
interface IPCVDeposit {
    ...
    event VaultAddressUpdate(address indexed _vaultAddress);
