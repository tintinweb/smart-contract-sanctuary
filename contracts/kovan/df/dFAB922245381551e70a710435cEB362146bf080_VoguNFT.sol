...
contract VoguNFT is VoguNFTBase {
    ...
    function reveal() public onlyOwner {
        ...
        // uint256 randomNumber = uint256(blockhash(block.number - 1)) % MAX_VOGU;
    ...
    function metadataOf(uint256 tokenId) public view returns (string memory) {
        ...
        for (uint256 i = 5; i < MAX_VOGU; i++) {
            uint256 j = (uint256(keccak256(abi.encode(ss, i))) % (MAX_VOGU - 5)) + 5;
...
contract VoguNFTBase is Ownable, ERC721 {
    uint256 public constant LIMIT_TOKEN_PER_TX = 7777;
