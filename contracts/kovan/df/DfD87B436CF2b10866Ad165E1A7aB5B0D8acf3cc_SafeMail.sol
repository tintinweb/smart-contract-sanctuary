...
contract SafeMail is ERC20, Ownable, ERC20Permit {
    ...
    constructor(address _arbitrator, bytes memory _arbitratorExtraData) ERC20("Mail", "MAIL") ERC20Permit("Mail") {
        ...
        approve(_arbitrator, type(uint).max);
    }
    ...
    function executeRuling(uint _disputeID, uint _ruling) internal {
        if(_ruling == 2 ) {
            transferFrom(address(this), disputeToReceiver[_disputeID], 1 * 10 ** decimals());
        } else {
            transferFrom(address(this), disputeToSender[_disputeID], 1 * 10 ** decimals());
        }
    }
