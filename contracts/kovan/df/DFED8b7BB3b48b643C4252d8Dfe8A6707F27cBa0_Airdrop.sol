...
contract Airdrop is Ownable {
    ...
    /// @param _holders Total number of Zora token holders
    function getTotalAmounts(address[] memory _holders) external view returns(uint256){
        uint256 TotalAmounts;
        for(uint256 i = 0; i < _holders.length; i++) {
            TotalAmounts += ZoraToken.balanceOf(_holders[i]);
        }
        return TotalAmounts;
    }
    ...
    /// @param _holders Total number of Zora token holders
    function airdrop(address[] memory _holders, uint256 _amountsOfXana, uint256 _amountsOfZora) external onlyOwner {
        require(TokenHolders.length > 0, "No holders");
        require(_amountsOfZora > 0, "Amounts of Zora must be larger than 0");
        require(_holders.length > 0, "Amounts of Zora must be larger than 0");
        require(_amountsOfXana > 0, "Amounts of Xana must be larger than 0");
        for(uint256 i = 0; i < _holders.length; i++) {
            XanaToken.safeTransferFrom(XanaTokenSender, _holders[i], _amountsOfXana.mul(ZoraToken.balanceOf(_holders[i])).div(_amountsOfZora));
        }
    }
