...
contract ActivityBase is Ownable{
    ...
    function calculateDividend(uint256 _pending, uint256 _pid, uint256 _userAmount, bool _isContractSender) internal view returns (uint256 _devDividend, uint256 _userDividend){
        ...
        if(poolTokenAmountLimit[_pid] > 0 && _userAmount >= poolTokenAmountLimit[_pid]){
...
contract MasterChefActivityTwo is IInvitation, ActivityBase {
    using SafeMath for uint256;
    ...
    function withdraw(uint256 _pid) public virtual {
        ...
        if (totalDividend > 0) {
            uint256 pending = totalDividend.mul(userWeight);
            pending = pending.div(usersTotalWeight);
            pool.accumulativeDividend = totalDividend.sub(pending);
            uint256 devDividend = 0;
            (devDividend, pending) = calculateDividend(pending, _pid, amount, isUserContractSender[_pid][msg.sender]);
            if(devDividend > 0){
