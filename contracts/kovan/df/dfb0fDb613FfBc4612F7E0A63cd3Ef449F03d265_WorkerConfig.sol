...
contract WorkerConfig is OwnableUpgradeSafe, IWorkerConfig {
  ...
  function isStable(address worker) public view returns (bool) {
    //fix me
    return true;
    IUniswapV2Pair lp = IUniswapV2Worker(worker).lpToken();
    ...
    require(lpPrice.mul(10000) <= price.mul(maxPriceDiff), "WorkerConfig::setConfigs:: price too high");
    require(lpPrice.mul(maxPriceDiff) >= price.mul(10000), "WorkerConfig::setConfigs:: price too low");
    // 3. Done
    return true;
  }
