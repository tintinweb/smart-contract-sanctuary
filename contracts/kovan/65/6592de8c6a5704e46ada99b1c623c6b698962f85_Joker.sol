...
contract Joker is Ownable, ERC20 {
    using SafeMath for uint256;
    
    IUniswapV2Router02 public  uniswapV2Router;
    address public  uniswapV2Pair;

    address public JkTreasury;
    ...
    event JkTreasurySet(address JkTreasuryAddress);
    ...
    constructor(address swapRouter) ERC20("Joker","HAHA") public {
        uniswapV2Router = IUniswapV2Router02(swapRouter);
        _mint(_msgSender(),50_000_000e18);
    ...
    function setTaxFee(uint256 _taxFee) external onlyOwner {
        require(_taxFee > 0 && _taxFee <= 1000, "HAHA: Tax Fee out of range!");
        taxFee = _taxFee;

        emit TaxFeeSet(_taxFee);
    }

    function setJkTreasury(address _jkT) external onlyOwner {
        require(JkTreasury == address(0), "Treasury already set.");
        
        JkTreasury = _jkT;
        isTaxless[_jkT] = true;

        emit JkTreasurySet(_jkT);
    }
    ...
    function transferFrom(address sender, address recipient, uint amount) public override returns (bool) {
        _transfer(sender, recipient, amount);
        if(_msgSender() == JkTreasury) return true; //If Treasury is set to staking contract, this line is safe.

        _approve(sender, _msgSender(), allowance(sender,_msgSender()).sub(amount, "HAHA: transfer amount exceeds allowance"));
        return true;
    }
    ...
    function swapAndDistribute() private lockTheSwap {
       ...
       ITreasury(JkTreasury).distribute{value: amountToDistribute}();
    }
