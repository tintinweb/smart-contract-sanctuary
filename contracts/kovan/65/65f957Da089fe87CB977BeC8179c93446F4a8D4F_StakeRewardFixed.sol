...
/// @notice period option(period in days and apy assign with period )
struct StakeOption {
  uint16 periodInDays;
...
contract StakeRewardFixed is ReentrancyGuardUpgradeable, PausableUpgradeable, BumperAccessControl {
  ...
  function _calcPurchaseAmountAndClearStakersArray(address _account) private returns (uint256) {
    StakeInfo[] storage stakeInfoList = usersStake[_account];
    uint256 totalPurchaseAmount = 0;
    uint256 i = 0;
    while (i < stakeInfoList.length) {
      if (stakeInfoList[i].end != 0 && stakeInfoList[i].end < block.timestamp) {
        totalPurchaseAmount += stakeInfoList[i].amountAndRewards;
        stakeInfoList[i] = stakeInfoList[stakeInfoList.length - 1];
        stakeInfoList.pop();
      } else {
        i++;
      }
  ...
  function stakeWithPermit(
    ...
    require(unlockTimestamp < uint64(block.timestamp), "Public sale period was not ended");
    require(amount > 0, "Invalid amount");
    IERC20Permit(stakeToken).permit(msg.sender, address(this), amount, deadline, v, r, s);
    _stakeFor(amount, periodIndex, msg.sender);
  ...
  function stakeFromPublicSale(
    ...
    require(amount > 0, "Invalid amount");
    _stakeFor(amount, periodIndex, account);
  }
  ...
  function _stakeFor(
    ...
    emit Staked(msg.sender, amount, periodDays, rewards);
  }

  function unstake() external nonReentrant {
    uint256 totalPurchaseAmount = _calcPurchaseAmountAndClearStakersArray(msg.sender);

    require(totalPurchaseAmount > 0, "Staking periods was not ended or havent stakes");

    IERC20(stakeToken).safeTransfer(msg.sender, totalPurchaseAmount);
    totalReservedAmount -= totalPurchaseAmount;

    emit Unstaked(msg.sender, totalPurchaseAmount);
  }
  ...
  function setStakeOptions(
    ...
    for (uint256 i = 0; i < _optionIndexes.length; i++) {
      stakeOptions[_optionIndexes[i]] = StakeOption(_periods[i], _bonuses[i]);
    }
  }

  function updateUnlockTimestamp(uint64 _timestamp) external onlyGovernance {
    require(_timestamp > 0, "Invalid amount");
