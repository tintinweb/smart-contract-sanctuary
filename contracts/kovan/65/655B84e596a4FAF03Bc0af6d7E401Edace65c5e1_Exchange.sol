...
interface ILiquidityPool {
	function PCT_PRECISION() external returns(uint256);
	...
	function borrowFee() external returns(uint256);
...
contract Exchange is ReentrancyGuard, Ownable {
		...
		returns (uint256)
	{
		...
		_amounts[outTokenIndex_] = calculateOut(inTokenIndex_, outTokenIndex_, inAmount_);
