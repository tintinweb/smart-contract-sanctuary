...
contract TscPool {
    event Created(address indexed _creator);
...
contract TSC is TerminateContractTemplate {
    ...
    function setupBasic(BasicInfoInput memory basicInfo)
        public
        onlyOwner
        onlyNotReady
        isLive
    ...
    function start() public payable onlyOwner onlyNotReady isLive {
        ...
        if (reward.tokens != address(0x0)) {
            require(
                IERC20(reward.tokens).transferFrom(
                    msg.sender,
                    address(this),
                    reward.value
                ),
                "TSC: Please approve reward token"
            );
        } else {
    ...
    function startTimming() public onlyPartner isLive {
        ...
        require(
            IERC20(startTimmingRequired.tokens).transferFrom(
                msg.sender,
                address(this),
                startTimmingRequired.value
            ),
            "TSC: Please approve transfer tokens for this contract"
        );
        ...
        onlyStartTimming
    {
        ...
        require(
            IERC20(listDepositERC20.list[_index].tokens).transferFrom(
                msg.sender,
                address(this),
                listDepositERC20.list[_index].value
            ),
        ...
        onlyStartTimming
    {
        ...
        uint256 preBalance = IERC20(listDepositERC20.list[_index].tokens)
            .balanceOf(listTransferERC20.list[_index].receiver);
        require(
            IERC20(listTransferERC20.list[_index].tokens).transferFrom(
                msg.sender,
