...
contract AaveV1Strategy is IRhoStrategy, PausableUpgradeable, AccessControlEnumerableUpgradeable {
    ...
    function initialize(
        ...
        address _aTokenAddr,
        address _lpAddressesProviderAddr,
        uint256 _lockDurationInBlock
    ) public initializer {
        PausableUpgradeable.__Pausable_init();
        AccessControlEnumerableUpgradeable.__AccessControlEnumerable_init();
        underlying = IERC20MetadataUpgradeable(_underlyingAddr);
        aToken = IAaveAToken(_aTokenAddr);
        provider = ILendingPoolAddressesProvider(_lpAddressesProviderAddr);
    ...
    function _supplyRate() private view returns (uint256) {
        address lpCoreAddr = provider.getLendingPoolCore();
        IAaveLendingPoolCore lpCore = IAaveLendingPoolCore(lpCoreAddr);
        uint256 rateInRay = lpCore.getReserveCurrentLiquidityRate(address(underlying));
        return (rateInRay / 1e9);
    }

    function supplyRate() external override view returns (uint256 _amount) {
        return _supplyRate();
    }
    ...
    function effectiveSupplyRate(uint256 delta, bool isPositive) external override view returns (uint256) {
        address lpCoreAddr = provider.getLendingPoolCore();
        IAaveLendingPoolCore lpCore = IAaveLendingPoolCore(lpCoreAddr);
        address _reserve = address(underlying);
        uint256 newAvailableLiquidity = isPositive ? lpCore.getReserveAvailableLiquidity(_reserve) + delta : lpCore.getReserveAvailableLiquidity(_reserve) - delta;
        IReserveInterestRateStrategy interestRateStrategy = IReserveInterestRateStrategy(lpCore.getReserveInterestRateStrategyAddress(address(underlying)));
        (uint256 rateInRay, ,) = interestRateStrategy.calculateInterestRates(
            _reserve,
            newAvailableLiquidity,
            lpCore.getReserveTotalBorrowsStable(_reserve),
            lpCore.getReserveTotalBorrowsVariable(_reserve),
            lpCore.getReserveCurrentAverageStableBorrowRate(_reserve) // assumption: user is ONLY depositing, NOT repaying
        );
        return rateInRay / 1e9;
    }
    ...
    // TODO: this updates but the above does not
    ...
    function deploy(uint256 _amount) public override onlyRole(VAULT_ROLE) {
        // require(_amount > 0, "AaveV1Strategy: deploy amount zero");
        underlying.safeIncreaseAllowance(provider.getLendingPoolCore(), _amount);
        IAaveLendingPool lendingPool = IAaveLendingPool(provider.getLendingPool());
        lendingPool.deposit(address(underlying), _amount, 0);
        emit Deploy(_amount);
    }

    function withdrawUnderlying(uint256 _amount) public override onlyRole(VAULT_ROLE) {
        require(_amount > 0, "AaveV1Strategy: Fail to withdraw from Aave V1");
        require(aToken.balanceOf(address(this)) >= _amount, "AaveV1Strategy: Fail to withdraw from Aave V1");
        require(aToken.isTransferAllowed(address(this), _amount), "AaveV1Strategy: Fail to withdraw from Aave V1");
    ...
    function withdrawAll() external override onlyRole(VAULT_ROLE) {
        withdrawUnderlying(aToken.balanceOf(address(this))); // 1:1 pegged
        emit WithdrawAll();
    }
...
interface IAaveAToken {
    ...
    **/
    ...
    /**
     * @notice Non-standard ERC20 function that checks if a transfer or a redeem will fail.
     * Specifically, a transfer/redeem will fail if the resulting Health Factor of the user
     * performing the action will end up being below 1.
     * Return true if the transfer is allowed, otherwise false
     */
...
interface IReserveInterestRateStrategy {
    ...
    * @param _totalBorrowsStable the total borrowed from the reserve at a stable rate
