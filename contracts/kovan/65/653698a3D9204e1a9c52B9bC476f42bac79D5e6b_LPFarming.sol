 ...
 */
contract LPFarming is Ownable, VotesSnapshot, ReentrancyGuard, Pausable {
    ...
    constructor(address _jpegd, uint256 _rewardPerBlock)
        ReentrancyGuard()
        Pausable()
    {
        jpegd = IERC20(_jpegd);
    ...
    function getVotes(address user) external view override returns (uint224) {
        uint256 votes = 0;
        for (uint256 i = 0; i < poolInfo.length; i++) {
            IERC20 lpToken = poolInfo[i].lpToken;
            uint256 lpAmount = userInfo[i][user].amount;
            votes +=
                (lpAmount * jpegd.balanceOf(address(lpToken))) /
                lpToken.totalSupply();
        }
