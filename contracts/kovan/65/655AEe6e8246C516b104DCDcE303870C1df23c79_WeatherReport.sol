...
contract WeatherReport is ChainlinkClient {
    ...
    mapping (string => Weather) public cityToWeather;
    mapping (bytes32 => string) private requestToCity;
    
    constructor() {
        setPublicChainlinkToken();
        operator = 0xa688AFd5A5D8dF13eb7E87b3dDD4f4956FB5eed7;
        jobId = "3e5d63dd70ea4c17923e71dcfd63cf21";
        fee = 0.01 * 10 ** 18;
        owner = msg.sender;
    }
    
    function requestWeather(string memory _city) public returns (bytes32 requestId_) {
        Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);
        request.add("extPath", _city);
        requestId_ = requestOracleDataFrom(operator, request, fee);
        requestToCity[requestId_] = _city;
    }
    
    function fulfill(bytes32 _requestId, bytes32 _temp, bytes32 _desc, bytes32 _wind) public recordChainlinkFulfillment(_requestId) {
        string memory city = requestToCity[_requestId];
        Weather memory weather;
        weather.temperature = string(abi.encodePacked(_temp));
        weather.description = string(abi.encodePacked(_desc));
        weather.wind = string(abi.encodePacked(_wind));
        cityToWeather[city] = weather;
    }
