...
contract MUSUBIV2 is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Musubi V2";
    string private constant _symbol = unicode"SPAM🍣V2";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (from == uniswapV2Pair && to != address(uniswapV2Router) && ! _isExcludedFromFee[to]) {
                ...
                require(amount <= _maxBuyAmount, "Too many tokens.");
            ...
            if (!inSwap && from != uniswapV2Pair && tradingOpen) {
                if(sellCooldownEnabled) {
                    // use longest cooldown
                    if (cooldown[from].buy > cooldown[from].sell) {
                        require(cooldown[from].buy < block.timestamp, "Your sell cooldown has not expired.");
                    } else {
                        require(cooldown[from].sell < block.timestamp, "Your sell cooldown has not expired.");
                    }
                }
                require(amount <= _maxSellAmount, "Too many tokens.");
                // set fee with price impact * 10 (1000 gets whole number)
                // price impact is amount sold / tokens in pool
                setFee((amount.div(balanceOf(uniswapV2Pair))).mul(1000));
    ...
    function addLiquidity() external onlyOwner() {
        ...
        _maxBuyAmount = 4500000000 * 10**9;
        _maxSellAmount = 9000000000 * 10**9;
    ...
    function testFee(uint256 amount) public view returns (uint) {
        return (amount.div(balanceOf(uniswapV2Pair))).mul(1000);
    }
