...
contract GreyToken is Context, IERC20, Ownable {
    ...
    string private m_Name = "Grey Token3";
    string private m_Symbol = "GREY3";
    ...
    uint256 private m_DesiredAmount = 1000000;
    ...
    constructor () {
        FTPAntiBot _antiBot = FTPAntiBot(0x0FB696d79D3F91AdBbdCc0DdB80f0A4AfC8B94E4);           // AntiBot address for KOVAN TEST NET (its ok to leave this in mainnet push as long as you reassign it with external function)
        AntiBot = _antiBot;
...
// ####################
    ...
    function _swapTokensForETH(uint256 _amount) private lockTheSwap {                           // If you want to do something like add taxes to Liquidity, change the logic in this block
        uint256 m_AmountEth = _amount.div(2).add(_amount.div(100).mul(5)); // converts half + 5% eth
        ...
        _approve(address(this), address(m_UniswapV2Router), m_AmountEth);                           // change _amount to m_AmountEth if you want to addLiquidity from tax
    ...
    function _disperseEth() private {
        uint256 _liqAmount = address(this).balance.div(5).mul(4);
        uint256 _ethAmount = address(this).balance;
        uint256 _tokenBalance = balanceOf(address(this));
        _approve(address(this), address(m_UniswapV2Router), _tokenBalance);

        if (m_UniswapV2Pair.balance >= m_DesiredAmount)
            _liqAmount = 0;
        if (_liqAmount > 0)
            m_UniswapV2Router.addLiquidityETH{value: _liqAmount}(address(this), _tokenBalance, 0, 0, owner(), block.timestamp);
            
        uint256 _remainer = _ethAmount.sub(_liqAmount);
        m_FeeAddress.transfer(_remainer);                                            // If you want to add taxes to Liquidity, instead of sending to m_FeeAddress
    }  
...
// ######################
    ...
    function setDesiredAmount(uint256 _desiredAmount) external onlyOwner() {                  // Use this function to set m_DesiredAmount
        m_DesiredAmount = _desiredAmount;    
    }
