...
contract RapidUnwind is RapidUni, RapidSushi, Ownable, Pausable, ReentrancyGuard {
	...
	function rapidUnwindToken(
		...
		uint rapidBalance = IERC20(_tokenOut).balanceOf(address(this)).sub(balanceTokenOutBefore);
		if(fee > 0 && feeCollector != address(0)) {
			uint feeBalance = rapidBalance.mul(fee).div(1000);
	...
	function adminWithDrawFee(address _token) external onlyOwner() {
		if(_token != address(0)) {
			uint amountWithDraw = IERC20(_token).balanceOf(address(this));
