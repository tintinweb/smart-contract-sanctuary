...
contract Sale is ReentrancyGuard, ERC721TokenReceiver {
    ...
    function buyWithUSDC(uint256 _amount, uint256 _punks) public virtual nonReentrant returns(bool){
        require(resolveStableCoins(_punks, msg.sender, 6) <= _amount, "Underflow Error");
        require(_punks <= limit,"Max Mint Limit Per Transaction");
    ...
    function resolveStableCoins(uint256 _decimal, address user, uint256 totalPunks) public virtual view returns(uint256){
        ...
        costPerPunk = SafeMath.sub(costPerPunk, discountedPrice);
