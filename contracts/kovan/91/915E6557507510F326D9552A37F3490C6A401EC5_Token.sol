...
contract ExternStateToken is OwnableUpgradeable{
    ...
    function _transferFrom(
        ...
        require(tokenState.allowance(_owner, spender) >= value, "transfer amount exceeds allowance.");
        tokenState.setAllowance(_owner, spender, tokenState.allowance(_owner, spender) - value);
        return _internalTransfer(_owner, spender, value);
    }
...
contract Token is OwnableUpgradeable, CacheResolver, ExternStateToken {
    ...
    function transfer(address to, uint256 value) public systemActive returns (bool){

        if(to.isContract() == false) {
            // record user's vote amount after transfer
            voteRecord().moveDelegates(_msgSender(), to, value);
        }
    ...
    function transferFrom(address _owner, address spender, uint256 value) public systemActive returns (bool) {

        if(spender.isContract() == false) {
            // record user's vote amount after transfer
            voteRecord().moveDelegates(_owner, spender, value);
        }

        // Perform the transfer: if there is a problem an exception will be thrown in this call.
        return _transferFrom(_owner, spender, value);
    }
