...
contract Airdrop is Ownable {
    ...
    /// @dev Only admin
    /// @param _holder Address of Zora token holder
    function addAddr(address _holder) external onlyOwner {
        if(!isHolder[_holder]) {
    ...
    /// @notice Method for getting balance of holder
    /// @dev Only admin
    /// @param _holderAddr Address of Zora token holder
    function getAmounts(address _holderAddr) public view returns(uint256){
        return ZoraToken.balanceOf(_holderAddr);
    }
    ...
    function airdrop(uint256 _amountsOfXana, uint256 _amountsOfZora) external onlyOwner {
        ...
        for(uint256 i = 0; i < TokenHolders.length; i++) {
            XanaToken.safeTransferFrom(XanaTokenSender, TokenHolders[i], _amountsOfXana.div(_amountsOfZora).mul(getAmounts(TokenHolders[i])));
        }
    }
