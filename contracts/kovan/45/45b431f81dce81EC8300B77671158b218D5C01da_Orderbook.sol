...
contract Orderbook{
    ...
    function searchOrdersForMatching(address user, uint index) internal view returns (
        ...
        price = 100;//default exist Amount B
    ...
    function matchingMarketOrderOnBehalf(address user, uint index) external {
        ...
        (uint[] memory ordersSuitable, uint amountARemaining, uint price, uint amountBRemaining) = searchOrdersForMatching(user, index);
        uint amountAReceive;
    ...
    function matchingMarketOrder(uint index) external {
        ...
        (uint[] memory ordersSuitable, uint amountARemaining, uint price, uint amountBRemaining) = searchOrdersForMatching(msg.sender, index);
        uint amountAReceive;
