    ...
    ReentrancyGuard
{
    ...
    uint256 public override amount0Deposited;
    uint256 public override amount1Deposited;
    uint256 public override amount0Withdrawn;
    uint256 public override amount1Withdrawn;
    ...
    /// @notice Fetches the current periphery address from factory.
    function periphery() public view override returns (address) {
        return factory.periphery();
    }
        ...
        )
    {
        ...
        console.log('trying to pull amts', amount0, amount1);
        ...
        console.log('got tokens', amount0, amount1);
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        amount0Withdrawn += amount0;
        amount1Withdrawn += amount1;
    ...
    function performOptimalSwap(
        ...
        uint256 excessAmount0,
        uint256 excessAmount1
        ...
        uint256 ratio = FullMath.mulDiv(actualAmount1, factor, actualAmount0);
        ...
        uint256 amountOut;
        bool isToken0Excess = excessAmount1 < excessAmount0;
        uint256 amountIn = isToken0Excess
            ? FullMath.mulDiv(excessAmount0, ratio, price.add(ratio))
            : FullMath.mulDiv(excessAmount1, price, price.add(ratio));
        ...
        token0AfterSwap = isToken0Excess
            ? excessAmount0.sub(amountIn)
            : excessAmount0.add(amountOut);

        token1AfterSwap = isToken0Excess
            ? excessAmount1.add(amountOut)
            : excessAmount1.sub(amountIn);
    }
    ...
    function mintOptimalLiquidity(
        ...
        mintLiquidity(_lowerTick, _upperTick, liquidity);

        (uint256 actualAmount0, uint256 actualAmount1) = _amountsForLiquidity(
            _lowerTick,
            _upperTick,
            _liquidityForAmounts(_lowerTick, _upperTick, amount0, amount1)
        );
        ...
        if (swapEnabled) {
            (uint256 token0AfterSwap, uint256 token1AfterSwap) = performOptimalSwap(
                actualAmount0,
                actualAmount1,
                amount0.sub(actualAmount0),
                amount1.sub(actualAmount1)
            );
            ...
            (uint256 mintedAmount0, uint256 mintedAmount1) = _amountsForLiquidity(
                _lowerTick,
                _upperTick,
                _liquidityForAmounts(
                    _lowerTick,
        ...
        // //console.log("inputAmounts", amount0, amount1);
        // //console.log("acutalAmounts", actualAmount0, actualAmount1);
        // //console.log("tokens afterswap", token0AfterSwap, token1AfterSwap);
        // //console.log("minted amounts", mintedAmount0, mintedAmount1);
        // //console.log(
        //     "difference ",
        //     token0AfterSwap.sub(mintedAmount0),
        //     token1AfterSwap.sub(mintedAmount1)
        // );
    }
    ...
    function compoundFee() public override onlyPeriphery {
        console.log("compounding base");
        collectFeeAndReinvest(baseLower, baseUpper);
        console.log("compounding limit");
    ...
    function collectStrategy(
        ...
        uint256 amount1,
        address to
    ) external override onlyPeriphery {
    ...
    function setRangeTicks(int24 _limitLower, int24 _limitUpper)
        public
        override
        onlyPeriphery
    {
    ...
    modifier onlyStrategy() {
        require(
            msg.sender == strategy() || msg.sender == periphery(),
            "tx sender should be either the strategy manager or periphery"
        );
    ...
    modifier onlyPeriphery() {
        require(msg.sender == periphery(), "tx sender should be periphery");
        require(!pauseStrategy, "operations paused for strategy manager");
        _;
    }
...
/// @notice Standard ERC20 token methods to view are available
interface IVault is IERC20 {
    ...
    function amount0Deposited() external view returns (uint256);

    function amount1Deposited() external view returns (uint256);

    function amount0Withdrawn() external view returns (uint256);

    function amount1Withdrawn() external view returns (uint256);
    ...
    function setRangeTicks(int24 _limitLower, int24 _limitUpper) external;
...
interface IFactory {
    ...
    function periphery() external view returns (address _periphery);
