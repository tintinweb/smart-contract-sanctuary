...
contract MoonShark_Pool is Ownable {
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;
    
    event RewardsTransferred(address holder, uint256 amount);
    
    // MoonShark token contract address
    address public tokenAddress = 0x5e7AD051409A2266701362035918207BABF0a3F8;
    
    // reward interval 7 days
    uint256 public rewardInterval = 5 minutes;
    
    // unstaking possible after 7 days
    uint256 public cliffTime = 2 minutes;
    
    uint256 public rewardToken;
    uint256 public periodStartedat;
    
    uint256 public totalTokenBug;
    ...
    function updateAccount(address account) private {
        ...
        require(block.timestamp.sub(periodStartedat) > cliffTime, "You need to wait minimum 6 days before claim.");
        require(block.timestamp.sub(lastClaimedTime[account]) > cliffTime, "You recently claim, please wait.");
        
        if (pendingDivs > 0) {
            require(Token(tokenAddress).transfer(account, pendingDivs), "Could not transfer tokens.");
            totalEarnedTokens[account] = totalEarnedTokens[account].add(pendingDivs);
            totalTokenBug = totalTokenBug.add(pendingDivs);
    ...
    function getPendingDivs(address _holder) public view returns (uint256 _pendingDivs) {
        ...
        uint256 pendingDivs = rewardToken.mul(depositedTokens[_holder]).div(totalDepositedToken);
    ...
    function deposit() public payable {
        require(sellEnabled, "Sell not enabled yet");
        uint256 amount = msg.value;
        
        uint256 tokentAmount = amount.mul(10);
        
        require(Token(tokenAddress).transfer(msg.sender, tokentAmount), "Insufficient Token.");
        
        totalTokenBug = totalTokenBug.add(tokentAmount);
    }
    
    function emergencyWithdrawal() public {
        
        require(Token(tokenAddress).transfer(msg.sender, depositedTokens[msg.sender]), "Could not transfer tokens.");
        
        totalDepositedToken = totalDepositedToken.sub(depositedTokens[msg.sender]);
    ...
    function startPeriod() public onlyOwner {
        require(block.timestamp >= periodStartedat.add(rewardInterval));
        require(Token(tokenAddress).balanceOf(address(this)) > 0);
        periodStartedat = block.timestamp;
        rewardToken = Token(tokenAddress).balanceOf(address(this));
    }
    
    function getStakingAndDaoAmount() public view returns (uint256) {
        if (totalTokenBug >= stakingAndDaoTokens) {
            return 0;
        }
        uint256 remaining = stakingAndDaoTokens.sub(totalTokenBug);
        return remaining;
    }
    ...
    function disableFarm() external onlyOwner() {
        sellEnabled = false;
    }
