...
contract LuckyLepRaffle is Ownable, VRFConsumerBase, KeeperCompatibleInterface {
    ...
    //checkUpkeep variables
    uint256 public mintingOverTimestamp = 1639094400; // default Dec 10th -- means the first lottery will be Dec 17th
    uint256 public interval = 604800; // 1 week
    bool public raffleOpen = true;

    //raffle info
    ILuckyLep public luckyLeprechauns; // interface to Lucky Leprechauns contract
    uint256 public numberOfLotteries = 3;
    uint256 public remainingLotteries = numberOfLotteries;
    uint256 public numberOfWinners = 10 /* 250 */; // default 250 winners
    mapping(uint256 => address) private entryToAddress;
    uint256[] public distributionSchedule = [30, 20, 15, 5]/* [1500, 1000, 500, 200, 65, 15] */; // divide by 100 to get % of prize pool
    uint256[] public winnersDist = [1, 2, 3, 10] /* [1, 2, 3, 10, 50, 250] */;
    address[] public lotteryPools;
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        upkeepNeeded = (((mintingOverTimestamp + interval) <= block.timestamp) &&
            hasLink &&
            raffleOpen &&
    ...
    function performUpkeep(
        ...
        mintingOverTimestamp = block.timestamp;
    ...
    function fulfillRandomness(
        ...
        for (uint256 i = 1; i <= supply; i++) {
            ...
            uint256 entries = (lengthHeld / (1 minutes) / 2) * scalingFactor;
            if (entries > 7) {
                entries == 7;
            }
        ...
        uint256 shareWon;
        // for loop that populates winners array and amounts array
        for (uint256 i = 1; i <= numberOfWinners; i++) {
            ...
            for (uint256 j = 0; j < distributionSchedule.length; j++) {
                if (i <= winnersDist[j]) {
                    shareWon = distributionSchedule[j];
                    break;
                }
            }
            _winners[i - 1] = randomWinner;
            _amounts[i - 1] = shareWon;
        }
        ...
        // create new PaymentSplitter contract and fund it
        uint256 lotteryInstance = numberOfLotteries - remainingLotteries;
        LuckyLepWinners winnersPoolContract = new LuckyLepWinners(
            _winners,
            _amounts,
            lotteryInstance + 1
        );
        address payable winnersPoolAddress = payable(
            address(winnersPoolContract)
        );
        uint256 amountToDistribute = address(this).balance / remainingLotteries;
        (bool success, ) = winnersPoolAddress.call{value: amountToDistribute}(
            ""
        );
        require(success, "Transfer failed");

        remainingLotteries -= 1;
        lotteryPools[lotteryInstance] = winnersPoolAddress;
    }
    ...
    function setDistributionSchedule(
        ...
        require(
            _distributionSchedule.length > 0 && _winnersDist.length > 0,
            "Must set distribution schedule"
        );
        numberOfWinners = _winnersDist[_winnersDist.length - 1];
       /*  uint256 sum = 0;
    ...
    function setMintingOverTimestamp(uint256 _mintingOverTimestamp) public onlyOwner {
        mintingOverTimestamp = _mintingOverTimestamp;
    }
