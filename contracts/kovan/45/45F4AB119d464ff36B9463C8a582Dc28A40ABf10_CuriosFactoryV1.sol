...
contract CuriosFactoryV1 is AccessControl {
    ...
    constructor(address _admin, address _contract_creator) {
        _setupRole(DEFAULT_ADMIN_ROLE, _admin);
        _setupRole(CONTRACT_CREATOR_ROLE, _contract_creator);
    }
    ...
    function createERC721ContractInstance (
        ...
        address[] memory _adminAddresses) external payable onlyRole(CONTRACT_CREATOR_ROLE){
        require(msg.value >=  2*10^16 wei, "CuriosFactory: Insufficient funds -- Please provide more ether for contract creation"); //.02 ETH
...
contract ERC1155ImplementationV1 is Initializable, AccessControlUpgradeable, ERC1155PausableUpgradeable, UUPSUpgradeable {
    ...
    **/
    function mint (address to, uint256 tokenId, uint256 amount, bytes memory data) public onlyRole(MINTER_ROLE) creatorOnly(tokenId) {
    ...
    */
    ...
    function burn(address account, uint256 tokenId, uint256 amount) public onlyRole(BURN_ROLE){
        _burn(account, tokenId, amount);
        tokenSupply[tokenId] -= amount;
    }

    function burnBatch(address account, uint256[] memory tokenIds, uint256[] memory amounts) public onlyRole(BURN_ROLE){
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 id = tokenIds[i];
            uint256 amount = amounts[i];
            tokenSupply[id] -= amount;
    ...
    //Required override of _authorizeUpgrade from UUPSUpgradeable
    function _authorizeUpgrade(address newImplementation) internal virtual override onlyRole(UPGRADER_ROLE){}
