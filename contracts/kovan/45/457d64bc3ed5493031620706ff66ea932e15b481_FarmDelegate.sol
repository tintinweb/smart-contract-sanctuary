...
contract FarmDelegate is FarmStorageV1 {
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (number > usdtStartBlock) {
            ...
            uint256 multiplier = number.sub(subtractor,"1");
    ...
    function phase(uint256 blockNumber) public view returns (uint256) {
        ...
        if (blockNumber > startBlockOfGan) {
            return (blockNumber.sub(startBlockOfGan,"2"

            ).sub(1)).div(halvingPeriod);
        }
        return 0;
    }
        ...
        returns (uint256)
    {
        ...
        if (number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = number.sub(pool.lastRewardBlock,"10");
            uint256 usdtReward = multiplier
            .mul(usdtPerBlock)
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        ...
        return
            user.amount.mul(accUsdtPerShare).div(1e12).sub(user.rewardUsdtDebt,"11");
    }
        ...
        returns (uint256)
    {
        ...
        if (user.amount >= 0) {
            ...
            return userReward.add(userLockAllReward).sub(stillLockReward,"13");
        } else return 0;
    }
    ...
    function getUserLockReward(
        ...
        if (user.lastRewardBlock < switchBlock) {
            if (block.number > switchBlock) {
                ...
                .div(halvingPeriod1);
                realLockGanReward = totalLockPendingAmount.sub(unLockGanReward,"17");
            } else realLockGanReward = totalLockPendingAmount;
        } else {
            uint256 unLockGanReward = block
            .number
            .sub(user.lastRewardBlock,"18")
            .mul(totalLockPendingAmount)
            .div(halvingPeriod1.add(switchBlock).sub(user.lastRewardBlock,"19"));
            realLockGanReward = totalLockPendingAmount.sub(unLockGanReward,"20");
        }
        ...
        returns (uint256)
    {
        ...
        while (m > n) {
            ...
            if (switchBlock > lastRewardBlock) {
                blockLockReward = blockLockReward.add(
                    (switchBlock.sub(lastRewardBlock,"21")).mul(
    ...
    function depositLPToken(
        ...
        if (user.amount >= 0) {
            ...
            .div(1e12)
            .sub(user.rewardGanDebt,"22");

            uint256 userLockReward = user
            .amount
            .mul(pool.accLockGanPerShare)
            .div(1e12)
            .sub(user.rewardLockGanDebt,"23")
            .add(user.lockPending);
            stillLockReward = getUserLockReward(
                _pid,
                _user,
    ...
    function withdrawLPToken(
        ...
        if (user.amount >= 0) {
            ...
            .mul(pool.accGanPerShare)
            .div(1e12)
            .sub(user.rewardGanDebt,"26");

            uint256 userLockReward = user
            .amount
            .mul(pool.accLockGanPerShare)
            .div(1e12)
            .sub(user.rewardLockGanDebt,"27")
            .add(user.lockPending);
            stillLockReward = getUserLockReward(
                _pid,
                _user,
        ...
        if (_amount > 0) {
            user.amount = user.amount.sub(_amount,"30");
            pool.totalAmount = pool.totalAmount.sub(_amount,"31");
    ...
    function newReward(uint256 _usdtAmount, uint256 _newPerBlock) public {
        ...
        uint256 balance = afterAmount.sub(beforeAmount,"32");
    ...
    function reDeposit(uint256 _pid) public payable {
        ...
        if (user.amount >= 0) {
            ...
            .mul(pool.accGanPerShare)
            .div(1e12)
            .sub(user.rewardGanDebt,"35");

            uint256 userLockReward = user
            .amount
            .mul(pool.accLockGanPerShare)
            .div(1e12)
            .sub(user.rewardLockGanDebt,"36")
            .add(user.lockPending);
            stillLockReward = getUserLockReward(
                _pid,
                msg.sender,
            ...
            pendingAmount = userReward.add(userLockReward).sub(stillLockReward,"37");

            uint256 usdtReward = user
            .amount
            .mul(pool.accUsdtPerShare)
            .div(1e12)
            .sub(user.rewardUsdtDebt,"38");
        ...
        if (pendingAmount > amountA)
            safeGanTransfer(msg.sender, pendingAmount.sub(amountA,"39"));
        if (msg.value > amountB) msg.sender.transfer(msg.value.sub(amountB,"40"));
