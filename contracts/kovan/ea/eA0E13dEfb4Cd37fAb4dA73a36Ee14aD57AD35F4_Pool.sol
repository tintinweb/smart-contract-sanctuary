...
contract Pool is Ownable {
    ...
    enum State {LOCKED, OPEN, UNLOCKED, COMPLETE}
  ...
  */
    ...
    /**
 @notice Deposits the tokens to AAVE LendingPool
 @param  amount : The amount of token to be deposited
   */
    function depositNoChecks(uint256 amount) external {
   ...
   * Can only be called by the owner when the pool is open.
    */
  function lock() external requireOpen onlyOwner {
    ...
    @param  amount : The amount of token to be deposited
   */
    function sendTokensToLendingPool(uint256 amount) internal {
        ILendingPoolAddressesProvider lendingPoolProvider =
            ILendingPoolAddressesProvider(address(lendingPoolAddressProvider));
        ILendingPool lendingPool =
            ILendingPool(lendingPoolProvider.getLendingPool());
...
function getTokensFromLendingPool(uint256 _amount) public  {
         ...
         lendingPool.withdraw(address(token), _amount, msgSender());
     }
