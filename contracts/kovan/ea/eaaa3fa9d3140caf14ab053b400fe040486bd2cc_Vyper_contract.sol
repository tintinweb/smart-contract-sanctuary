...
# # Set up the owner.
@external
def __init__(farmToken: address):
    self.owner = msg.sender
    self.farmToken = farmToken
...
def _totalToPaid() -> uint256:
    ...
    incentives: uint256 = ERC20(self.farmToken).balanceOf(self)  # (self.balance - self.totalStaked)
    ...
    if self.endBlock == 0:
        ...
        return (incentives * (min(block.number, self.endBlock) - self.lastBlock))/(self.endBlock - self.lastBlock) * 10**6
    else:
        return incentives
...
def withdraw(_amount: uint256):
    ...
    ERC20(self.farmToken).transfer(msg.sender, rewards)
...
def emergencyWithdraw():
    _amount: uint256 = self.holdings[msg.sender]
