...
contract WeatherNFT is ERC721, Ownable, ChainlinkClient, IERC721Receiver {
    ...
    {   
        response = _data;
        if(response == 0){
            transferFrom(address(this), requestIdToAttemptee[_requestId], 0);
        }
        if (response == 1){
            transferFrom(address(this), requestIdToAttemptee[_requestId], 1);
        }
        if (response == 2){
            transferFrom(address(this), requestIdToAttemptee[_requestId], 2);
        }
        if (response == 3){
            transferFrom(address(this), requestIdToAttemptee[_requestId], 3);
        }
    }    
