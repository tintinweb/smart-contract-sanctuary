...
contract TestOperationsprocessor is OperationsProcessor{
    

    function CW(uint256[] calldata tokensAmounts) external payable {
        OperationsProcessor.processCreateWallet(tokensAmounts);
    }

    function SW(uint256 amountFrom, uint256 indexFrom, uint256 amountTo, uint256 indexTo) external payable {
        OperationsProcessor.processSwap(amountFrom, indexFrom, amountTo, indexTo);
    }

    function with_draw(uint256[] calldata deltas, address recepient) external {
        OperationsProcessor.processWithdraw(deltas, recepient);
    }
}
