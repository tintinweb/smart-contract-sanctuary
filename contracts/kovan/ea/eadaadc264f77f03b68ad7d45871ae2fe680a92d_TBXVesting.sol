...
/**
 * @title TBX Cliffing and Vesting Contract.
 */
contract TBXVesting is Ownable {
    using SafeMath for uint256;
    using SafeMath for uint16;

    IERC20 public tbxToken;
    uint256 public totalAmountOfGrants = 49000000000000000000000000;
    ...
    /**
     * @dev set the address of the token contract,
     * and the start time (timpstamp in seconds).
     */
    constructor(address _tbxToken, uint256 _startTime) {
        tbxToken = IERC20(_tbxToken);
     ...
     *
     * Emits an {GrantAdded} event indicating the beneficiary address.
     *
     * Requirements:
     *
     * - The msg.sender is the owner of the contract.
     * - The beneficiary has no other Grants.
     * - The Grant amount + other grants is smaller or equal to the totalAmountOfGrants
     * - The amountVestedPerDay should be bigger than 0, _amount should be bigger than _vestingDurationInDays.
    ...
    function addTokenGrant(
        ...
        require(storedAddedAmount <= totalAmountOfGrants, "Amount exceeds grants balance!");
        uint256 amountVestedPerDay = _amount.div(_vestingDurationInDays);
        require(
            amountVestedPerDay > 0,
            "amountVestedPerDay should be bigger than 0"
        );
        require(tbxToken.transferFrom(owner(), address(this), _amount));
        Grant memory grant =
            Grant({
     ...
     *
     * This function is called by the investors to claim their vested tokens.
     *
     * Emits an {GrantTokensClaimed} event indicating
     *  the beneficiary address and the claimed amount.
     *
     * Requirements:
     *
     * - The vested amount to claim is bigger than 0
     * - The requirement described in function {calculateGrantClaim} for msg.sender
     * - The contract can transfer token to the beneficiary
     */
    function claimVestedTokens() external {
        ...
        require(
            tbxToken.transfer(tokenGrant.beneficiary, amountVested),
            "no tokens"
        );
    ...
    /**
     * @dev calculate the grant claim daysVested and the amount to claim
     *
     * Requirements:
     *
     ...
     *
     * @return the amount of vested tokens and the number of daysVested.
     */
        ...
        returns (uint16, uint256)
    {
        ...
        uint256 elapsedDays =
            currentTime().sub(tokenGrant.vestingStartTime - 1 days).div(1 days);
    ...
    /**
     * @dev Get the Grant total amount.
     * @return the total grant amount.
     */
    ...
    /**
     * @dev Get the timestamp from the block.
