...
contract longhu is VRFConsumerBase {
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        expand(randomResult, cardsCount);
        emit CardsEvent(requestId, randomResult, cards);
    }
    ...
    function expand(uint256 randomValue, uint256 n) private {
        for (uint256 i = 0; i < n; i++) {
            ...
            while(isExisted(cardVal)){ cardVal++; }
            cards.push(cardVal -1);
        }
    }
    ...
    function randomCards(uint _cardsCount) public isOwner returns (bytes32 requestId) {
