...
contract MARKET{
    ...
    function YOURPRODUCTPAYTOGO(uint256 item_id) public returns(string memory operatingsystem,
        uint256 vcpus,
        uint256 meromrycapicity,
        uint256 storagecapacity){
        User storage user = users[msg.sender];
        require(user.userperoducts[item_id].paytogo,"you are not a paytogo customer");
        if(block.timestamp <= user.userperoducts[item_id].paytogocheckpoint+user.userperoducts[item_id].duration)
        {
                return
    ...
    function YOURPRODUCTSUBCRIBED(uint256 item_id) public returns(string memory operatingsystem,
        uint256 vcpus,
        uint256 meromrycapicity,
        uint256 storagecapacity){
        User storage user = users[msg.sender];
        require(user.userperoducts[item_id].subcriber,"you are not a subcriber");
        if(block.timestamp <= user.userperoducts[item_id].subcriptioncheckpoint+user.userperoducts[item_id].duration)
        {
