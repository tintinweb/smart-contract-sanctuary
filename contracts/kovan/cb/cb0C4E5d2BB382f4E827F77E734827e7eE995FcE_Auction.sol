...
contract Auction is Ownable {
    ...
    event KeepersExported(uint256 uniAmount, address[] assets, address[] keepers);
    ...
    function exportKeepers() external inState(State.ChallengeEnded) onlyOwner {
        ...
        address[] memory assets = new address[](keepers.length);
        ...
        for (uint256 i = 0; i < keepers.length; i++) {
            ...
            assets[i] = userBids.asset;

            AssetMeta storage meta = assetMeta[userBids.asset];
            amounts[meta.index] += collateralAmount * meta.divisor;
        }

        changeState(State.Refund, REFUND_SPAN);

        for (uint256 i = 0; i < assetList.length; i++) {
            require(ERC20(assetList[i]).approve(address(keeperImport), amounts[i]));
        }
        keeperImport.importKeepers(collateralAmount, assets, keepers);
        emit KeepersExported(collateralAmount, assets, keepers);
    }
