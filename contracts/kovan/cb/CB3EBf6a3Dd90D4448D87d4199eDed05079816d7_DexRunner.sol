...
contract DexRunner is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    string private _symbol = "DexRunner";
    ...
    function _takeMarketingDevStakeLiquidBurnFee(uint256 tMarketingDevStakeLiquidBurnFee, address sender) private {
        ...
        accumulatedForStake = accumulatedForStake.add(tStake);
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                _transfer(address(this), address(msg.sender), pending);
            }
        }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            uint256 adjustedTokenPerShare =
                accTokenPerShare.add(accumulatedForStake.mul(PRECISION_FACTOR).div(stakedTokenSupply));
    ...
    function _updatePool() internal {
        ...
        accTokenPerShare = accTokenPerShare.add(accumulatedForStake.mul(PRECISION_FACTOR).div(stakedTokenSupply));
