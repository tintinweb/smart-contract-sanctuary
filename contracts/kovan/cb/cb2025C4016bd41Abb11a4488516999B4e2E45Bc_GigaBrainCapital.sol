...
contract GigaBrainCapital is Ownable, IERC20 {
	...
	function _transfer(
		...
		uint256 senderBalance = _balances[sender];
		require(senderBalance >= amount, "ReFi: transfer amount exceeds balance");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFees[account] != excluded, "ReFi: account is already set to requested state");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "ReFi: automated market maker pair is already set to that value");
	...
	function updateUniswapV2Router(address newAddress) public onlyOwner {
		require(newAddress != address(uniswapV2Router), "ReFi: the router is already set to the new address");
...
contract DividendTracker is Ownable, IERC20 {
	...
	function excludeFromDividends(address account, bool excluded) external onlyOwner {
		require(excludedFromDividends[account] != excluded, "ReFi_DividendTracker: account already set to requested state");
	...
	function _mint(address account, uint256 amount) private {
		require(account != address(0), "ReFi_DividendTracker: mint to the zero address");
		_totalSupply += amount;
		_balances[account] += amount;
		emit Transfer(address(0), account, amount);
	...
	function _burn(address account, uint256 amount) private {
		require(account != address(0), "ReFi_DividendTracker: burn from the zero address");
		uint256 accountBalance = _balances[account];
		require(accountBalance >= amount, "ReFi_DividendTracker: burn amount exceeds balance");
		_balances[account] = accountBalance - amount;
		_totalSupply -= amount;
		emit Transfer(account, address(0), amount);
		magnifiedDividendCorrections[account] = magnifiedDividendCorrections[account] + int256(magnifiedDividendPerShare * amount);
