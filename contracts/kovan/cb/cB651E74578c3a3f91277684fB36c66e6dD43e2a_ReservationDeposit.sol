...
contract ReservationDeposit {
    ...
    uint32 internal usdMinReservationDepositEntryFee;
    mapping(string => uint32)
        internal usdMinReservationDepositEntryFeeByBuilding;
    ...
    event FundDeposit(
        ...
        string indexed _emailHashed,
        string _email,
        uint32 _developerId,
        uint32 _buildingId,
        uint32 indexed _propertyId,
        uint256 _value
    );
    event ModifyFundDepositReservation(
        address _fromwallet,
        string indexed _emailHashed,
        string _email,
        uint32 _developerId,
        uint32 _buildingId,
        uint32 indexed _propertyIdPrevious,
        uint32 indexed _propertyIdModified,
        uint256 _value
    );

    event RefundDeposit(
        address indexed _toWallet,
        string indexed _emailHashed,
        string _email,
        uint32 _developerId,
        uint32 _buildingId,
    ...
    event AcceptDepositAndTransfer(
        address indexed _toWallet,
        string indexed _emailHashed,
        string _email,
        uint32 _developerId,
    ...
    mapping(string => UserWalletProjectAmountFunded[])
        internal addressToUserWalletProjectAmountFunded;
    ...
    function setDefaultMinUSDReservationDepositFee(
        uint32 minimumUSDReservationDepositFee
    ) public onlyOwner {
        usdMinReservationDepositEntryFee = minimumUSDReservationDepositFee;
    }

    function getBuildingUniqueIdentifier(uint32 developerId, uint32 buildingId)
        internal
        pure
        returns (string memory)
    {
        return _concatenate(_toString(developerId), _toString(buildingId), "-");
    }

    function setMinUSDReservationDepositFeeByBuilding(
        uint32 developerId,
        uint32 buildingId,
        uint32 minimumUSDReservationDepositFee
    ) public onlyOwner {
        usdMinReservationDepositEntryFeeByBuilding[
            getBuildingUniqueIdentifier(developerId, buildingId)
        ] = minimumUSDReservationDepositFee;
    }

    function getMinUSDReservationFeeByBuilding(
        uint32 developerId,
        uint32 buildingId
    ) public view returns (uint32) {
        uint32 minUSDFee = usdMinReservationDepositEntryFeeByBuilding[
            getBuildingUniqueIdentifier(developerId, buildingId)
        ];
        if (minUSDFee > 0) return minUSDFee;

        return usdMinReservationDepositEntryFee;
    }
    ...
    function fund(
        ...
        uint256 mimimumUSD = getMinUSDReservationFeeByBuilding(
            developerId,
            buildingId
        ) * 10**18;
        require(
            //getConversionRate(msg.value) >= mimimumUSD,
            (getConversionRate(msg.value)) >= ((mimimumUSD * 95) / 100),
        ...
        for (
            ...
            if (
                ...
                emit FundDeposit(
                    addressToUserWalletProjectAmountFunded[emailLower][i]
                        .walletAddress,
                    addressToUserWalletProjectAmountFunded[emailLower][i].email,
                    addressToUserWalletProjectAmountFunded[emailLower][i].email,
                    addressToUserWalletProjectAmountFunded[emailLower][i]
                        .developerId,
                    addressToUserWalletProjectAmountFunded[emailLower][i]
                    ...
                    addressToUserWalletProjectAmountFunded[emailLower][i]
                        .propertyId,
                    addressToUserWalletProjectAmountFunded[emailLower][i].amount
                );
            }
        }
        if (!found) {
            ...
            emit FundDeposit(
                addressToUserWalletProjectAmountFunded[emailLower][0]
                    .walletAddress,
                addressToUserWalletProjectAmountFunded[emailLower][0].email,
                addressToUserWalletProjectAmountFunded[emailLower][0].email,
                addressToUserWalletProjectAmountFunded[emailLower][0]
                    .developerId,
                addressToUserWalletProjectAmountFunded[emailLower][0]
                ...
                addressToUserWalletProjectAmountFunded[emailLower][0]
                    .propertyId,
                addressToUserWalletProjectAmountFunded[emailLower][0].amount
            );
        }
    }
    ...
    function getEntranceFee(uint32 developerId, uint32 buildingId)
        public
        view
        returns (uint256)
    {
    ...
    function cleanUp(string memory email) internal returns (bool) {
        string memory emailLower = _toLower(email);
    ...
    function modifyReservationProperty(
        string memory email,
        uint32 developerId,
        uint32 buildingId,
        uint32 propertyId,
        uint32 propertyIdModified
        ...
        for (
            ...
            if (
                ...
                uint32 propertyIdPrevious = addressToUserWalletProjectAmountFunded[
                        emailLower
                    ][i].propertyId;
                addressToUserWalletProjectAmountFunded[emailLower][i]
                    .propertyId = propertyIdModified;
                modified = true;
                emit ModifyFundDepositReservation(
                    addressToUserWalletProjectAmountFunded[emailLower][i]
                        .walletAddress,
                    addressToUserWalletProjectAmountFunded[emailLower][i].email,
                    addressToUserWalletProjectAmountFunded[emailLower][i].email,
        ...
        require(modified, "Reservation does not exist!");
        return modified;
    }

    function acceptAndTransfer(
        string memory email,
        uint32 developerId,
        uint32 buildingId,
        uint32 propertyId,
        address transferToWalletAddress
        ...
        for (
            ...
            if (
                ...
                if (value > 0) {
                    payable(transferToWalletAddress).transfer(value);
                    //if (payable(transferToWalletAddress).send(value)) {
                    emit AcceptDepositAndTransfer(
                        transferToWalletAddress,
                        addressToUserWalletProjectAmountFunded[emailLower][i]
                            .email,
                        addressToUserWalletProjectAmountFunded[emailLower][i]
                            .email,
