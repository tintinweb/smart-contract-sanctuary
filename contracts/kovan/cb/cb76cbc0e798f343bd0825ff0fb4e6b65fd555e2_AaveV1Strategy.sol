...
contract AaveV1Strategy is BaseRhoStrategy, PausableUpgradeable {
    ...
    function collectRewardToken() external override onlyRole(VAULT_ROLE) returns (bool) {}

    function shouldCollectReward(uint256) external pure override returns (bool) {
        return false;
    }
...
abstract contract BaseRhoStrategy is IRhoStrategy, AccessControlEnumerableUpgradeable {
    ...
    function setSlippage(uint256 _slippage) external override onlyRole(DEFAULT_ADMIN_ROLE) {
        _setSlippage(_slippage);
    }

    function _setSlippage(uint256 _slippage) internal {
        require(_slippage <= 1000, "Input value must <= 1000");
        slippage = _slippage;
    }
...
interface IRhoStrategy {
     ...
     */
    function collectRewardToken() external returns (bool);
