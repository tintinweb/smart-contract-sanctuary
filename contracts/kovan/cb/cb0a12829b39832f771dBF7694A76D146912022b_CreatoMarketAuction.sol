...
/**
 * @title Payments contract for Creato Marketplaces.
 */
...
// File: contracts/CreatoMarketAuction.sol
...
contract CreatoMarketAuction is ICreatoMarketAuction, Ownable, Payments {
    using SafeMath for uint256;
    ...
    ICreatoRoyaltyRegistry public iERC1155CreatorRoyalty;

    // Creato contract
    ICreato public creato;
    //erc1155 contract
     ...
     * @param _creato address of the creato contract
     */
    constructor(address _iMarketSettings, address _iERC1155CreatorRoyalty, address _creato)
    public
    {
        require(
        ...
        require(
            _creato != address(0),
            "constructor::Cannot have null address for _creato"
        );
        ...
        iERC1155CreatorRoyalty = ICreatoRoyaltyRegistry(_iERC1155CreatorRoyalty);

        creato = ICreato(_creato);
        erc1155 = IERC1155(_creato);
     ...
     * @param _address address of the IMarketplaceSettings.
     */
    function setCreato(address _address) public onlyOwner {
        require(
            _address != address(0),
            "setCreato::Cannot have null address for _ICreato"
        );

        creato = ICreato(_address);
    ...
    function setIERC1155CreatorRoyalty(address _address) public onlyOwner {
        ...
        iERC1155CreatorRoyalty = ICreatoRoyaltyRegistry(_address);
    }
    ...
    view
    {
        ...
        require(isOwner || msg.sender == address(creato), 'sender must be the token owner');
    }
    ...
    function setSalePrice(
        ...
        creato.setPrice(_amount, _tokenId, _owner);
        // creato.putOnSale(0, _tokenId, _owner);
        // creato.setPriceType(0, _tokenId);
        // creato.setIsForSale(true, _tokenId, _owner);
        emit SetSalePrice(_amount, _tokenId);
    }
    ...
    function restore(address _oldAddress, address _oldCreatoAddress, uint256 _startIndex, uint256 _endIndex) external onlyOwner {
        CreatoMarketAuction oldContract = CreatoMarketAuction(_oldAddress);
        ...
        for (uint i = _startIndex; i < _endIndex; i++) {
            ...
            address[] memory owners = oldCreatoContract.getOwners(tokenId);
    ...
    function buy(uint256 _tokenId, address _owner) public payable {
        require(creato.getIsForSale(_tokenId, _owner) == true, "bid::not for sale");
        // The owner of the token must have the marketplace approved
        ownerMustHaveMarketplaceApproved(_owner);
        ...
        uint8 priceType = creato.getPriceType(_tokenId, _owner);
        ...
        creato.removeFromSale(_tokenId, _owner);
    ...
    function bid(
        ...
        creato.setBid(_newBidAmount, msg.sender, _tokenId, _owner);
    ...
    function _setBid(
        ...
        //add bid balance
        uint256 serviceFee = _amount.mul(creato.getServiceFee(_tokenId)).div(100);
