...
contract DependencyManager is RoleAware, IDependencyManager {
    ...
    function implementAsOwner(address performer) external onlyOwnerExec {
        ...
        for (uint256 i = 0; requiredTitle.length > i; i++) {
            _setRole(requiredTitle[i], performer);
        }
        ...
        for (uint256 i = 0; len > i; i++) {
            _deleteRole(requiredTitle[i], performer);
        }
    }
    ...
    {
        _deleteRole(role, actor);
    }

    function _deleteRole(uint256 role, address actor) internal {
    ...
    function _setRole(uint256 role, address actor) internal {
        if (!knownRoles[role]) {
