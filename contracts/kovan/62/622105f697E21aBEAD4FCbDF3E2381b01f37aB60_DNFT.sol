...
contract DNFT is ERC721, Ownable {
    ...
    mapping(uint256 => string) private lowtokenURIs;
    mapping(uint256 => string) private hightokenURIs;
    mapping(uint256 => string) private raretokenURIs;
        ...
        returns (string memory)
    {
        ...
        if(block.number % 20 >= 10 ){
            if(keccak256(bytes(lowtokenURIs[tokenId])) == keccak256(bytes(tokenURIs[tokenId]))){
                return raretokenURIs[tokenId];
            }
            else{
