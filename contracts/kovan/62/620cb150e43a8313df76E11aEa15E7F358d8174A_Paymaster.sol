...
contract Paymaster is BasePaymaster {
    ...
    function checkValidPermit(address payer, address token, bytes memory approvalData) public view {
        (uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s) = abi.decode(approvalData, (uint256, uint256, uint8, bytes32, bytes32));
        require(_isValidPermit(payer, token, address(this), value, deadline, true, v, r, s), "invalid permit");
    }

    function _isValidPermit(address holder, address token, address spender, uint256 value, uint256 expiry, bool allowed, uint8 v, bytes32 r, bytes32 s) internal view returns(bool) 
    {
        ...
        bytes32 DOMAIN_SEPARATOR = keccak256(
            ...
            keccak256(bytes(tokenMeta.name())),
            keccak256(bytes("1")),
            chainID,
            token
        ));
        ...
        // require(nonce == IERC2612Permit(token).nonces(holder), "invalid-nonce");
        return true;
    }
