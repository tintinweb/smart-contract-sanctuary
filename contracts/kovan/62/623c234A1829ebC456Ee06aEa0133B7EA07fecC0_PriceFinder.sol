...
contract PriceFinder {
    function getSqrtRatioAtPrice(address token0, address token1, uint256 price) public pure returns (uint160) {
        return PriceMath.getSqrtRatioAtPrice(token0, token1, price);
    }

    function getPriceAtSqrtRatio(address token0, address token1, uint160 sqrtPriceX96) public pure returns (uint256) {
        return PriceMath.getPriceAtSqrtRatio(token0, token1, sqrtPriceX96);
    }

    function getTickAtSqrtRatioWithFee(uint160 sqrtPriceX96, int24 _tickSpacing) public pure returns (int24) {
        return PriceMath.getTickAtSqrtRatioWithFee(sqrtPriceX96, _tickSpacing);
    }
}
