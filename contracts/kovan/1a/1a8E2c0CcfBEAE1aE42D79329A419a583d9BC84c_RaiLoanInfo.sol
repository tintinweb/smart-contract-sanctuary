...
contract RaiLoanInfo is DSMath {
    ...
    struct SafeInfo {
        ...
        address safeAddr;
        bytes32 collType;
        uint256 collMulCollPrice;
        uint256 debtMulRaiPrice;
    }
    ...
    function getSafeInfo(uint256 _safeId) public returns (SafeInfo memory safeInfo) {
        ...
        // multiply debt amount with current rate 
        CollInfo memory collInfo = getCollateralTypeInfo(collType);
        uint256 calculatedDebt = mul(debt, collInfo.currRate);
        
        // get rai and collateraal price
        RaiInfo memory raiInfo = getRaiInfo();
        uint256 collPrice = getPrice(collType);
        
        uint256 collMulCollPrice = mul(coll, collPrice);
        uint256 debtMulRaiPrice = mul(calculatedDebt, raiInfo.currRaiPrice); 
        ...
        safeInfo = SafeInfo({
            ...
            safeAddr: safeAddr,
            collType: collType,
            collMulCollPrice: collMulCollPrice,
            debtMulRaiPrice: debtMulRaiPrice
        });
    }
