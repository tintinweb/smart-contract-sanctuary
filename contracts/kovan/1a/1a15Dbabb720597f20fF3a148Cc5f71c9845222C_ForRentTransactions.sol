...
contract ForRentFactory {
    ...
    uint256 public allowanceDebug;
    uint256 public allowanceDebug2;
...
contract ForRentTransactions is ForRentFactory {
    ...
    function reserve(uint256 _forRentId) external {
        ...
        bool approved = drentalToken.approve(forRentToOwner[_forRentId], 1000);
        require(approved, "Approve operation unsucceeded");

        approved = drentalToken.approve(address(this), 1000);
        require(approved, "Approve operation unsucceeded");

        approved = drentalToken.approve(msg.sender, 1000);
        require(approved, "Approve operation unsucceeded");
        allowanceDebug = drentalToken.allowance(
            msg.sender,
            forRentToOwner[_forRentId]
        );
        allowanceDebug2 = drentalToken.allowance(msg.sender, address(this));
