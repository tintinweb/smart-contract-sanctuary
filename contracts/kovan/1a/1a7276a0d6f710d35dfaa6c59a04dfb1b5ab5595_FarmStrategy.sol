...
contract FarmStrategy is StrategyStorage{
    ...
    function earn(address[] memory _tokens, uint256[] memory _amounts) external override{
    ...
    function withdraw(uint256 amount) external override returns (address[] memory tokens, uint256[] memory amounts){
        ...
        (amounts[0], amounts[1]) = IUniswapV2Router02(router).removeLiquidity(token0, token1, amount, 0, 0, address(this), block.timestamp.add(1));
        if(token0 == profit){
    ...
    function caculateLPTokenAmount(address token, uint256 amount) internal view returns (uint256){
        ...
        uint256 _lpTokenAmount = amount.mul(totalSupply).div(IERC20(token).balanceOf(lpToken));
