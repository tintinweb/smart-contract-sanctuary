...
contract StakingMock {
   ...
   struct Stake {
    ...
    function endVesting() public {
        vestingDuration = block.number - stakingProgramEndsBlock;
    }
    
    /// @notice Uniswap pool that we accept LP tokens from
    address public poolToken;
    /// @notice Oiler token that will be given as a reward
    address public oilerToken;
    
    function setPoolToken(address _address) public {
        poolToken = _address;
    }

    function setOilerToken(address _address) public {
        oilerToken = _address;
    }
