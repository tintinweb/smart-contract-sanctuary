...
abstract contract BondSubHour is LendMath {
    ...
    // // math - floating calc in getUpdatedHourlyYield()
    uint256 constant borrowMinYieldH = slot + (borrowMinRate * slot) / 1000 / hoursPY;

    function _makeHourlyBond(address issuer, address holder, uint256 amount) internal 
    {
    ...
    function updateHourlyBondAmount(address issuer, HourlyBond storage bond, address holder) internal 
    {
        uint256 yieldQuotientFP = bond.yieldQuotientFP;

        YieldAccumulator storage yA = getUpdatedHourlyYield(issuer, hourlyBondYieldAccumulators[issuer], callRateUpdate);
        LendingMetadata storage meta = lendingMeta[issuer];
    ...
    function viewHourlyBondAmount(address issuer, address holder)public view returns (uint256)
    {
        HourlyBond storage bond = hourlyBondAccounts[issuer][holder];
        uint256 yieldQuotientFP = bond.yieldQuotientFP;
        uint256 cumulativeYield = viewCumulativeYieldFP(hourlyBondYieldAccumulators[issuer], block.timestamp);
    ...
    function _withdrawHourlyBond(address issuer, HourlyBond storage bond, uint256 amount, address holder) internal 
    {
    ...
    function calcCumulativeYieldFP(YieldAccumulator storage yieldAccumulator, uint256 timeDelta) internal view returns (uint256 accumulatorFP) 
    {
    ...
    function updateHourlyYield(address issuer) public returns (uint256 hourlyYield)
    {
        return getUpdatedHourlyYield(issuer, hourlyBondYieldAccumulators[issuer], callRateUpdate).hourlyYieldFP;
    }

    /// @dev updates yield accumulators for both borrowing and lending
    function getUpdatedHourlyYield(address issuer, YieldAccumulator storage accumulator, uint256 window) internal returns (YieldAccumulator storage) 
    {
        ...
        if (timeDelta > window) 
        {
            ...
            accumulator.accumulatorFP = calcCumulativeYieldFP(accumulator, timeDelta);
    ...
    function updateBorrowYieldAccu(YieldAccumulator storage borrowAccumulator) internal
    {
        uint256 timeDelta = block.timestamp - borrowAccumulator.lastUpdated;

        if (timeDelta > callRateUpdate) {
            borrowAccumulator.accumulatorFP = calcCumulativeYieldFP(borrowAccumulator, timeDelta);
            borrowAccumulator.lastUpdated = block.timestamp;
        }
    }

    function getUpdatedBorrowYieldAccuFP(address issuer)external returns (uint256)
    {
    ...
    function viewCumulativeYieldFP(YieldAccumulator storage yA, uint256 timestamp) internal view returns (uint256) 
    {
    ...
    function addToTotalLending(LendingMetadata storage meta, uint256 amount) internal
    {
        updateIncentiveAllocation(meta);
        meta.totalLending += amount;
    }

    function subtractFromTotalLending(LendingMetadata storage meta, uint256 amount) internal 
    {
        updateIncentiveAllocation(meta);
        meta.totalLending -= amount;
    }

    function disburseIncentive(HourlyBond storage bond, LendingMetadata storage meta,address holder) internal virtual;
}
...
contract Lend is RoleAware, BondSubHour {
        ...
        onlyOwnerExecActivator
    {
        ...
        if (yieldAccumulator.accumulatorFP == 0) {
            ...
            YieldAccumulator storage yA = getUpdatedHourlyYield(issuer, yieldAccumulator, callRateUpdate);
    ...
    function registerBorrow(address issuer, uint256 amount) external {
        ...
        getUpdatedHourlyYield(issuer, hourlyBondYieldAccumulators[issuer], borrowRateDelay);
    ...
    function registerLend(address issuer, uint256 amount) external {
        ...
        getUpdatedHourlyYield(issuer, hourlyBondYieldAccumulators[issuer], callRateUpdate);
    }
