...
// Root file: contracts/governance/LPAChessRewardsV2.sol
...
contract LPAChessRewardsV2 is StakedTokenWrapper, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public constant EPOCH = 10 minutes;
    ...
    function initialize() external onlyOwner {
        rewardToken.safeTransferFrom(msg.sender, address(this), MAX_SUPPLY);
    }
    ...
    function getEpochSupply(uint256 index)
        public
        pure
    ...
    function getRate(uint256 timestamp) external view returns (uint256) {
        ...
        uint256 index = (timestamp.sub(startTime)).div(EPOCH);
        (, uint256 weeklySupply) = getEpochSupply(index);
        return weeklySupply.div(EPOCH);
    }
    ...
    function rewardPerToken() public view returns (uint256) {
        ...
        uint256 index1 = lastUpdateTime.sub(startTime).div(EPOCH);
        uint256 index2 = current.sub(startTime).div(EPOCH);

        uint256 rewardIndex = 0;
        uint256 index = index1;
        uint256 start = lastUpdateTime;

        for (; index <= index2; index++) {
            uint256 end = startTime.add(index.add(1).mul(EPOCH));
            if (end > current) {
                end = current;
            }
            uint256 duration = end.sub(start);
            (, uint256 weeklySupply) = getEpochSupply(index);
            uint256 rate = weeklySupply.div(EPOCH);
            rewardIndex = rewardIndex.add(duration.mul(rate).mul(1e18).div(totalStakedSupply));
            start = end;
        }

        return rewardPerTokenStored.add(rewardIndex);
    }
