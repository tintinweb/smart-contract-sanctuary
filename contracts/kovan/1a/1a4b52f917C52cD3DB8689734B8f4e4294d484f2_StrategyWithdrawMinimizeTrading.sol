...
contract StrategyWithdrawMinimizeTrading is Ownable, ReentrancyGuard, IStrategy {
  ...
  modifier onlyWhitelistedWorkers() {
    require(
      ...
      "UniswapV2StrategyWithdrawMinimizeTrading::onlyWhitelistedWorkers:: bad worker"
    );
    _;
  }
  ...
  /// @param _weth The wrapped BNB token.
  ...
  /// @dev Execute worker strategy. Take LOOKS tokens. Return BaseToken.
  ...
  function execute(
    ...
    path[0] = farmingToken;
    path[1] = baseToken;
    uint[] memory amounts = router.getAmountsOut(farmingToken.myBalance(), path);
    router.swapTokensForExactTokens(amounts[amounts.length-1], farmingToken.myBalance(), path, address(this), now);
    // 4. Return BaseToken back to the original caller.
    baseToken.safeTransfer(msg.sender, debt);
    // 5. Return remaining BaseToken to user.
    uint256 remainingBalance = baseToken.myBalance();
    if (remainingBalance > 0) {
      SafeToken.safeTransfer(baseToken, address(wNativeRelayer), remainingBalance);
      wNativeRelayer.withdraw(remainingBalance);
      SafeToken.safeTransferETH(user, remainingBalance);
    }
