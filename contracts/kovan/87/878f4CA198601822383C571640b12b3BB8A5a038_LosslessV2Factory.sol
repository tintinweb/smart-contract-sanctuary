...
contract LosslessV2Factory is ILosslessV2Factory {
	...
	// all pool related
	address[] public override activePools; // current active pool address array, the first element is factory address
	mapping(address => uint256) public override indexOfActivePools; // index of pool in above activePools
	...
	function activePoolsLength() external view override returns (uint256) {
		return activePools.length - 1;	// due to first one is factory address for iterative mapping
	}
	...
	function createPool(
		...
		indexOfActivePools[pool] = activePools.length - 1; //  index    0  1  2  active[1] = 1  active[2] = 2
	...
	function terminatePool(address pool) external returns (bool) {
		...
		require(indexOfActivePools[pool] != 0, "LosslessV2Factory: REDUNDENT CHECK - INDEX OF POOL SHOWS ITS INACTIVE");
		...
		uint256 _indexOfPool = indexOfActivePools[pool];
		...
		indexOfActivePools[pool] = 0; // reset it to point to factory address
...
contract LosslessV2Pool is ILosslessV2Pool {
	...
	function startFirstRound() external override {
		...
		// modify status variable
		// status.gameRound = 1;
		// status.durationOfGame = 5 days;
		// status.durationOfBidding = 2 days;
