    ...
    AccessControlUpgradeable
{
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        for (uint256 i = 0; i < claimableAddress.length; i++) {
            ...
            if(tiers[_role[i]] == TIER3_ROLE) {
                tierTotalClaimableChapter[claimableAddress[i]][tiers[_role[i]]][_chapter[i]] = tierTotalClaimableChapter[claimableAddress[i]][tiers[_role[i]]][_chapter[i]].add(_tokenAmount[i]);
            }
        }
    }

    function reSetClaimableRole(address[] memory claimableAddress, uint256[] memory _role, uint256[] memory _chapter, uint256[] memory _tokenAmount)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require((claimableAddress.length == _tokenAmount.length), "Invalid Data");
        for (uint256 i = 0; i < claimableAddress.length; i++) {
            require(tierTotalSupply[tiers[_role[i]]] > 0, "Role not added");
            _setupRole(tiers[_role[i]], claimableAddress[i]);
            tierTotalClaimableToken[claimableAddress[i]][tiers[_role[i]]] = tierTotalClaimableToken[claimableAddress[i]][tiers[_role[i]]].sub(_tokenAmount[i]);
            if(tiers[_role[i]] == TIER3_ROLE) {
                tierTotalClaimableChapter[claimableAddress[i]][tiers[_role[i]]][_chapter[i]] = tierTotalClaimableChapter[claimableAddress[i]][tiers[_role[i]]][_chapter[i]].sub(_tokenAmount[i]);
            }
        }
    }
    ...
    function _claimNFT(address _claimable, bytes32 _role, uint _tierCurrentIndex, uint256 _tierTotalClaimable, uint256 _chapter) internal returns(uint256 _nftAmount) {
        ...
        require(
            tierTotalClaimed[_role].sub(1).add(_tierTotalClaimable) < tierTotalSupply[_role],
    ...
    function claimAllTeamNFT(address[] memory claimableAddress, uint256[] memory _role, uint256[] memory _chapter) external whenNotPaused nonReentrant {
