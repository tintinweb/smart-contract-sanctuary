...
//        }
    ...
    function processBuyRequest(uint256 index) internal {
        ...
        if(request.operationTypeId == 1){
            return _buyTokensForETH(request.eth_usd_price, request.randomNumber, request.payedAmount );
        }
    }

    function _buyTokensForETH(uint256 price, uint256 randomNumber, uint256 amountPayed ) internal {
        IERC20 myTokenContract = IERC20(myTokenContractAddress);
