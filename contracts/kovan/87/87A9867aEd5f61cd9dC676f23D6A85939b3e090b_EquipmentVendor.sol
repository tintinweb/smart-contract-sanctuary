...
contract EquipmentVendor is IEquipmentVendor, Ownable {
    ...
    constructor(address _equipmentContract) {
        equipmentContract = _equipmentContract;
    }

    modifier onlyEquipmentContract() {
        require(
            equipmentContract == msg.sender,
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < itemIds.length; i++) {
            uint256 itemTier = (
                IEquipment(equipmentContract).getItem(itemIds[i])
            ).tier;
            uint256 itemMaxSupply = (
                IEquipment(equipmentContract).getItem(itemIds[i])
            ).maxSupply;
            uint256 itemMinted = (
                IEquipment(equipmentContract).getItem(itemIds[i])
            ).minted;
            require(itemTier == 1, "EquipmentVendor: cannot add");
            require(itemMinted < itemMaxSupply, "EquipmentVendor: cannot add");
    ...
    function mintRandomItems(
        ...
        uint256[] memory vendorItemIds = getVendorItemIdsInternal(vendorId);
    ...
    {
        return getVendorItemIdsInternal(vendorId);
    }

    function getVendorItemIdsInternal(uint256 vendorId)
        internal
        view
        returns (uint256[] memory)
    {
        uint256[] memory vendorItemIds;
