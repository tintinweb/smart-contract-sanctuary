...
// File: contracts/Pool/RisqUNIPool.sol
...
/**
 * @author macemclain
 * @title Risq UNI Liquidity Pool
 * @notice Accumulates liquidity in UNI from LPs and distributes P&L in UNI
 */
contract RisqUNIPool is
    IERCLiquidityPool,
    Ownable,
    ERC20("Risq UNI LP Token", "writeUNI")
{
    using SafeMath for uint256;
    ...
    /*
     * @nonce A provider supplies UNI to the pool and receives writeUNI tokens
    ...
    /*
     * @nonce Provider burns writeUNI and receives UNI from the pool
     * @param amount Amount of UNI to receive
    ...
    /*
     * @nonce Returns provider's share in UNI
     * @param account Provider's address
     * @return Provider's share in UNI
     */
    ...
    /*
     * @nonce Returns the amount of UNI available for withdrawals
    ...
    /*
     * @nonce Returns the UNI total balance provided to the pool
...
// File: contracts/Options/RisqUNIOptions.sol
...
/**
 * @author macemclain
 * @title Risq UNI (Uniswap) Bidirectional (Call and Put) Options
 * @notice Risq Protocol Options Contract
 */
contract RisqUNIOptions is Ownable, IRisqOptions {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    RisqUNIPool public pool;
    IUniswapV2Router01 public uniswapRouter;
    address[] public ethToUniSwapPath;
    IERC20 public uni;

    /**
     * @param _priceProvider The address of ChainLink UNI/USD price feed contract
     * @param _uniswap The address of Uniswap router contract
     * @param token The address of UNI ERC20 token contract
     */
    constructor(
        ...
        ethToUniSwapPath = new address[](2);
        ethToUniSwapPath[0] = uniswapRouter.WETH();
        ethToUniSwapPath[1] = address(uni);
        ...
        returns (uint256 optionID)
    {
        ...
        uint amountIn = swapToUNI(totalETH, total);
    ...
    function approve() public {
        uni.safeApprove(address(pool), uint(-1));
        uni.safeApprove(address(settlementFeeRecipient), uint(-1));
    }
        ...
        )
    {
        ...
        totalETH = uniswapRouter.getAmountsIn(total, ethToUniSwapPath)[0];
    }
     ...
     * @param period Option period in seconds (1 days <= period <= 4 weeks)
     * @param strike Strike price of the option
     * @param currentPrice Current price of UNI
    ...
    /**
     * @notice Sends profits in UNI from the UNI pool to an option holder's address
