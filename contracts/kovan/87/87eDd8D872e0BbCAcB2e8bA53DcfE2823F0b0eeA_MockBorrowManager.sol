...
contract MockBorrowManager is IBorrowManager, AccessControlEnumerable, ReentrancyGuard, Pausable
{
  ...
  event NewExtension(bytes32 indexed debtID, address indexed borrower, address debtToken, uint256 interestPaid);
  ...
  function extend (bytes32 debtID) external nonReentrant
  {
    ...
    ICoordinator(_ADDRESSES.coordinator()).increaseTokenCumulativeInterestPaid(debt.token, interest);
    ...
    emit NewExtension(debtID, msg.sender, debt.token, interest);
  }
  ...
  function _repay (Debt memory debt, bytes32 debtID, uint256 debtRepaid, uint256 collateralToWithdraw) private
  {
    ...
    ICoordinator coordinator = ICoordinator(_ADDRESSES.coordinator());
    ...
    coordinator.decreaseTokenTotalBorrowed(debt.token, debtRepaid);
    coordinator.increaseTokenCumulativeInterestPaid(debt.token, interest);
  ...
  function liquidate (bytes32 debtID) external nonReentrant
  {
    ...
    (address depositVault, address interestVault, address collateralVault) = coordinator.getPairVaultAll(debt.token, debt.collateralToken);

    (uint256 defaultFee, address burner) = _feeMgr().getDefaultFee(debt.collateral);

    uint256 interest = _calcPercentOf(debt.amount, debt.interestRate);

    uint256 maxCompensation = IOracle(_ADDRESSES.oracle()).convert(debt.token, debt.collateralToken, _calcPercentOf(debt.amount + interest, _MAX_LIQ_COMPENSATION));
    ...
    coordinator.decreaseTokenTotalBorrowed(debt.token, debt.amount);
    coordinator.increaseTokenCumulativeInterestPaid(debt.token, interest);
  ...
  {
    IOracle oracle = IOracle(_ADDRESSES.oracle());
  ...
  function liquidatePartial (bytes32 debtID, uint256 amount) external nonReentrant
  {
    ...
    coordinator.decreaseTokenTotalBorrowed(debt.token, amount);
    coordinator.increaseTokenCumulativeInterestPaid(debt.token, interest);
