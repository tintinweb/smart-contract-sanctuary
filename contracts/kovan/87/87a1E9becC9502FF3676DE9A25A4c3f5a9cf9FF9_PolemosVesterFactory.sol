...
contract PolemosVesterFactory is Ownable {
    using EnumerableSet for EnumerableSet.AddressSet;
    event VestingCreated(
        address recipient,
        address polemosVester,
        uint256 amount,
    ...
    constructor(address polemos_, address timelock_) {
        require(
            polemos_ != address(0) && timelock_ != address(0),
            "PolemosVesterFactory:INVALID_ADDRESS"
        );
        polemos = polemos_;
        timelock = timelock_;
    }
    ...
    function newPolemosVester(
        ...
        PolemosVester polemosVester = new PolemosVester(
            polemos,
            timelock,
            vestingAmount,
    ...
    function getPolemosVestersByRecipient(address recipient)
        external
        view
        returns (PolemosVester[] memory)
    {
...
contract PolemosVester is Ownable {
    event VestingInterrupted(
        address recipient,
    ...
    function activate() public onlyOwner {
        ...
        vestingBegin = lastClaimTimestamp =
            block.timestamp +
            (vestingStartDelayInDays * DAY);
    ...
    function claimable() public view returns (uint256 amount) {
        if (!active()) {
            ...
            amount =
                (vestingAmount * (block.timestamp - lastClaimTimestamp)) /
    ...
    function claim() public {
        require(
            active() && vestingBegin <= block.timestamp,
            "PolemosVester:NOT_STARTED"
        );
        uint256 amount = claimable();
        lastClaimTimestamp = block.timestamp;
        IPolemos(polemos).transfer(owner(), amount);
    }
    ...
    function interrupt(address collectionAccount) external onlyTimelock {
        ...
        require(
            collectionAccount != address(0),
            "PolemosVester:INVALID_ADDRESS"
        );
        IPolemos(polemos).transfer(collectionAccount, unvested());
