...
contract Controller {
    ...
    function setSunderFactory(address _sunderFactory) public {
        require(msg.sender == governance, "!governance");
        sunderFactory = _sunderFactory;
    }
    ...
    function setStrategy(address _token, address _strategy) public {
        ...
        if (_current != address(0)) {
           Strategy(_current).withdrawAll(sunderFactory);
        }
        strategies[_token] = _strategy;
    }

    function earn(address _token, uint256 _amount) public {
        require(msg.sender == sunderFactory, "!sunderFactory");
        _deposit(_token, _amount);
        address _strategy = strategies[_token];
        Strategy(_strategy).addDebt(_amount);
    }
    ...
    function withdrawAll(address _token) public {
        require(msg.sender == strategist || msg.sender == governance, "!strategist");
        Strategy(strategies[_token]).withdrawAll(sunderFactory);
    }
    ...
    function getExpectedReturn(address _strategy, address _token, uint256 _parts) public view returns (uint256 expected) {
        ...
        address _want = Strategy(_strategy).want();
        (expected,) = OneSplitAudit(onesplit).getExpectedReturn(_token, _want, _balance, _parts, 0);
    }
    ...
    function yearn(address _strategy, address _token, uint256 _parts) public {
        ...
        if (_after > _before) {
            ...
            (_expected, _distribution) = OneSplitAudit(onesplit).getExpectedReturn(_token, _want, _amount, _parts, 0);
    ...
    function withdraw(address _token, uint256 _amount) public {
        require(msg.sender == sunderFactory, "!sunderFactory");
        Strategy(strategies[_token]).withdraw(msg.sender, _amount);
    }

    function withdrawVote(address _token, uint256 _amount) public {
        require(msg.sender == voteController, "!voteController");
        Strategy(strategies[_token]).withdrawVote(msg.sender, _amount);
    }

    function mint(address _token, uint256 _amount) public {
        require(msg.sender == strategies[_token], "!token strategies");
        Factory(sunderFactory).mint(_token, msg.sender, _amount);
    }
}