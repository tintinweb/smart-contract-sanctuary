 ...
 * time spent accumulates for the user everytimes it takes over the throne
...
contract EthroneContract is KeeperCompatibleInterface {
    ...
    event ThroneTaken(address prevOwner, address newOwner, uint32 prevOwnerTimeSpent);
    event WinnerChosen(address winner, uint256 prize, uint32 totalTimeSpent, uint32 round);
    ...
    /**
      Main game function
      Only one player can take the throne within one block, usually the one that paid the most fees for that block
      or got lucky with the mining process, all other attempts to take the throne within the same block will be rejected.
      This is the law of the network, the miners will be the ones that will collectively decide who gets the throne each block.
      Everytime a new player takes the throne, the contract records the time spent by the last owner
      and adds it to their accumulated time.
     */
     // TODO only allow taking the throne once per user?
    function takeThrone() public payable {
        ...
        console.log("TakeOver attempt: Current block: %s", block.number);
        if (currentThroneOwner.hash == currentHash) {
          console.log("Rejecting transaction of %s - as %s already owns this block", newOwner, currentThroneOwner.user);
        }
        ...
        if (timeSpentMapping[newOwner] == 0) {
        ...
        emit ThroneTaken(previousOwner, newOwner, accumulatedTimeSpent(previousOwner));
        console.log("%s took over from %s (was on the throne for %s secs)", newOwner, previousOwner, accumulatedTimeSpent(previousOwner));
    }

    /**
     * Calculates the winner of the round and transfers the prize to it
     */
    function awardPrize() public {
        ...
        for (uint32 i = 0; i < participantSize; i++) {
            ...
            if (timeSpent > longestTimeSpent && participant != contractOwner) {
                ...
                console.log("Updating winner score : %s (%s)", timeSpent, winner);
            }
        }
        ...
        uint256 ownersCut = totalPrize - winnerPrize; // 10% for maintenance costs
        payable(winner).transfer(winnerPrize);
        payable(contractOwner).transfer(ownersCut);
        lastWinner = winner;
        emit WinnerChosen(winner, winnerPrize, accumulatedTimeSpent(winner), round);
        console.log("Transfered %s to %s", winnerPrize, winner);
        console.log("Owner cut %s to %s", ownersCut, contractOwner);
