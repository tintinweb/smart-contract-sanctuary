...
contract Greeter is Ownable, ChainlinkClient {
  ...
  uint256 private fee;
  bytes private latest;

  mapping (bytes32 => uint256) private requestPlayerIdx;
  ...
  function requestPlayerInfo(uint256 _playerIdx) public returns (bytes32 requestId_) {
    Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);
    request.addUint("idx", _playerIdx);
    requestId_ = requestOracleData(request, fee);
    requestPlayerIdx[requestId_] = _playerIdx;
  }

  function fulfill(bytes32 _requestId, bytes calldata _playerInfo) external recordChainlinkFulfillment(_requestId) returns (uint256 playerIdx_) {
    playerIdx_ = requestPlayerIdx[_requestId];
    latest = _playerInfo;
  }
