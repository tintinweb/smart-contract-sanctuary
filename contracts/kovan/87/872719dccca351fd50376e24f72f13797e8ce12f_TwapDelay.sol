    ...
    function deposit(
        ...
        enqueueDepositOrder(
            ...
            DepositOrder(
                ...
                depositParams.gasLimit,
                validAfterTimestamp,
                oracle.price0CumulativeLast(), // TODO: use getCumulativePrice
                oracle.blockTimestampLast() // TODO: use getCumulativePrice
            )
        );
    }
...
library AddLiquidity {
    ...
    function swapDeposit0(
        ...
        uint256 amount0In = ITwapPair(pair).getDepositAmount0In(amount0);
        amount1Left = ITwapPair(pair).getSwapAmount1Out(amount0In);
        if (amount1Left == 0) {
    ...
    function swapDeposit1(
        ...
        uint256 amount1In = ITwapPair(pair).getDepositAmount1In(amount1);
        amount0Left = ITwapPair(pair).getSwapAmount0Out(amount1In);
        if (amount0Left == 0) {
...
contract TwapDelay is ITwapDelay {
    ...
    function delay() public view override returns (uint32) {
        return orders.delay;
    }
    ...
    function setOrderDisabled(
        ...
        require(orderType != Orders.OrderType.Empty, 'TD_INVALID_ORDER_TYPE');
    ...
    function setDelay(uint32 _delay) public override {
        require(msg.sender == owner, 'TD_FORBIDDEN');
        orders.delay = _delay;
    ...
    function _executeDeposit(Orders.DepositOrder memory depositOrder) public {
        require(msg.sender == address(this), 'TD_FORBIDDEN');
        require(depositOrder.validAfterTimestamp + 48 hours >= block.timestamp, 'TD_EXPIRED');

        (address pair, address token0, address token1, uint256 amount0Left, uint256 amount1Left) = _initialDeposit(
            depositOrder
        );
        if ((amount0Left != 0 || amount1Left != 0) && depositOrder.swap) {
            if (amount0Left != 0) {
    ...
    function _executeWithdraw(Orders.WithdrawOrder memory withdrawOrder) public {
        require(msg.sender == address(this), 'TD_FORBIDDEN');
        require(withdrawOrder.validAfterTimestamp + 48 hours >= block.timestamp, 'TD_EXPIRED');
    ...
    function _executeBuy(Orders.BuyOrder memory buyOrder) public {
        require(msg.sender == address(this), 'TD_FORBIDDEN');
        require(buyOrder.validAfterTimestamp + 48 hours >= block.timestamp, 'TD_EXPIRED');
        ...
        uint256 amountIn = buyOrder.inverse
            ? pair.getSwapAmount1In(buyOrder.amountOut)
            : pair.getSwapAmount0In(buyOrder.amountOut);
        ...
        if (tokenOut == tokenShares.weth && buyOrder.unwrap) {
            pair.swap(amount0Out, amount1Out, address(this), buyOrder.priceAccumulator, buyOrder.timestamp);
            _forceEtherTransfer(buyOrder.to, buyOrder.amountOut);
        } else {
            pair.swap(amount0Out, amount1Out, buyOrder.to, buyOrder.priceAccumulator, buyOrder.timestamp);
        }
    }

    function _executeSell(Orders.SellOrder memory sellOrder) public {
        require(msg.sender == address(this), 'TD_FORBIDDEN');
        require(sellOrder.validAfterTimestamp + 48 hours >= block.timestamp, 'TD_EXPIRED');
        ...
        if (tokenOut == tokenShares.weth && sellOrder.unwrap) {
            pair.swap(amount0Out, amount1Out, address(this), sellOrder.priceAccumulator, sellOrder.timestamp);
            _forceEtherTransfer(sellOrder.to, amountOut);
        } else {
            pair.swap(amount0Out, amount1Out, sellOrder.to, sellOrder.priceAccumulator, sellOrder.timestamp);
        }
    }
    ...
    function cancelOrder(uint256 orderId) public lock {
        ...
        require(validAfterTimestamp.sub(delay()).add(ORDER_CANCEL_TIME) < block.timestamp, 'TD_ORDER_NOT_EXCEEDED');
