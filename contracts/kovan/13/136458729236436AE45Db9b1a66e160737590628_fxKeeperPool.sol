...
contract fxKeeperPool is IfxKeeperPool, IValidator, Ownable, ReentrancyGuard {
        ...
        )
    {
        ...
        // Minimum CR as a percentage (3 digit precision).
        ...
        uint256 liquidationFeeEther =
            VaultLibrary()
                .getLiquidationFee(account, fxToken)
                .mul(collateral)
                .div(1 ether);
        collateral = collateral.sub(liquidationFeeEther);
        // Get fxAmount to be used in liquidation.
        fxAmount = VaultLibrary()
            .tokensRequiredForCrIncrease(minimumCr.mul(10), debt, collateral)
            .mul(1 ether)
            .div(Comptroller().getTokenPrice(fxToken));
