 ...
 */
contract TraderInvestor is Investor {
    ITrader public trader;
    IRevenueManager public revManager;

    // config vars
    uint256 public tradeAmount = 1 ether / 100;
    uint256 public margin = 1 ether / 1000;
    address public pair;
     ...
     * @param _revManager is the revenue manager
    ...
    constructor(
        ...
        bytes1 _prefix,
        bytes1 _postfix
    ) Investor(_token, _bonds, _oracle, _ncfs, _prefix, _postfix) {
        token.approve(address(_revManager), uint256(2**256 -1));

        trader = ITrader(_trader);
        revManager = IRevenueManager(_revManager);
        pair = ITrader(_trader).initPair(address(this), _token);
    }
     ...
     * NOTE the ovveride diference is that this withdrawal doesn't actually transfer the tokens back. 
     *  Since the tokens are in the posession of the contract already anyways. It would only be trading it's own ERC-1155s for it's own tokens
     ...
     */
    function withdrawl(uint256 _id, uint256 _amm, address _receiver) public override {
        ...
        require(
            _amm <= loanLookup[_id].awaitingCollection,
            "There are not enough payments available for collection"
        );
        bonds.safeTransferFrom(_receiver, DEAD, _id, _amm, "");
        // erc20.transfer(_receiver, _amm); REMOVED
    }

    /**
     * @notice function override sends funds to the rev manager
     * @param _id to pay off
     * @param _erc20Ammount is a typo...
     */
    function payment(uint256 _id, uint256 _erc20Ammount) public override {
        super.payment(_id, _erc20Ammount);
        _sendAllToRevManager();
    }

    /**
     * @dev function handle liqidation logic. Override from collateralPayment
     *   liquidates collateral allways to this, then swaps to token
     */
    function _liquidate(uint256 _id, address _receiver) internal override {
        (address erc20Contract, uint256 amount) = manager.getCollateralLookup(
            address(this),
            _id
        );

        manager.withdrawal(_id, amount, address(this));
        IERC20(erc20Contract).approve(address(trader), amount);
        trader.trade(erc20Contract, address(token), amount);
    }
    ...
    function _balance() private {
        uint256 price = _getPrice(_getTradeAmount());
     ...
     */
    function _getTradeAmount() private view returns (uint256) {
        return (tradeAmount);
    }
    ...
    * @dev returns the expected output of investor tokens if tradeAmount
     ...
     */
    function _getPrice(uint256 _tradeAm) private view returns (uint256) {
        return trader.getAmountOut(address(this), address(token), _tradeAm);
    }
    ...
    function _buy() private returns (bool) {
        ...
        uint256 tradeAm = _getTradeAmount();
        if (revManager.balanceAvailable(tis, address(token)) >= tradeAm) {
            revManager.requestFunds(address(token), tradeAm);
            token.transfer(address(trader), tradeAm);
            trader.trade(address(token), tis, tradeAm);
 ...
 * @dev TestToken will eventually be replaced with ERC-20
 */
contract Investor is TestToken, ERC20CollateralPayment {
    uint256 public interestRateAnnual = 10 ether;
    IERC20 public token;
    IBonds public bonds;
    ...
    constructor(
        ...
        ERC20CollateralPayment(_bonds, _oracle, _ncfs)
    {
        token = IERC20(_token);
        bonds = IBonds(_bonds);
    }
     ...
     * @param _durration of loan in seconds
     * @param _NCFSID is the user's NCFS NFT ID from Roci's Credit scoring system
    ...
    function borrow(
        ...
        addCollateral(_collateral, _collateralAmount, id);

        // request a new score (this will eventually call borrowFulfill() )
        ...
        override
    {
        ...
        // withdrawal profit
        if (bal >= loanLookup[_id].awaitingCollection) {
            withdrawl(_id, loanLookup[_id].awaitingCollection, address(this));
    ...
    /// @dev function called by chainlink oracle to finish loan generation. An extention of borrow
    function borrowFulfill(
        ...
        require(
            msg.sender == creditOracle,
            "Only the credit scoring oracle can call this function"
        );

        // this function calls the issue function with requires non-delinquency
        bonds.newLoan(address(this), _id, _hash, _signature);
        uint256 amount = loanLookup[_id].principal;

        require(
            bonds.balanceOf(address(this), _id) == amount,
            "Issue minting bonds"
        );

        // stake the bonds
        stakingIndexes[_id] = bonds.stake(_id, amount);

        // fulfill loan
        token.transfer(loanLookup[_id].borrower, amount);
    }

    // comented out to save on size. May want this here. May not. Still here if we want it
    // /**
    ...
    //     uint256 index = bonds.stake(_id, _amount); // unused for now. But this is index in the staking linked list of this stake
...
interface IRevenueManager is IPaymentSplitter{
    ...
    function balanceAvailable(address, address) external returns(uint);
 ...
 */
contract ERC20CollateralPayment is ERC20PaymentStandard, Ownable {
     ...
     */
    constructor(address _bonds, address _oracle, address _ncfs)
        ERC20PaymentStandard(_bonds, _oracle, _ncfs)
        Ownable()
    {}
    ...
    function addCollateral(
        ...
        manager.deposit(msg.sender, _ERC20Contract, _loanId, _amount);
    }

    /**
     * @dev function to change the collateral manager
     * @param _new is the new address
     */
    function changeManager(address _new) external onlyOwner {
        manager = ICollateralManager(_new);
    }

    /**
     * @dev function to change priceFeed
     * @param _new is the new address
     */
    function changePriceFeed(address _new) external onlyOwner {
        priceFeed = IPriceFeed(_new);
    }
    ...
    {
        require(!isDelinquent(_id), "Not enough collateral to issue this loan");
        return (super.issueBonds(_id));
    }
     ...
     */
    function claimCollateral(uint256 _id, uint256 _amount) external {
        require(
            msg.sender == loanLookup[_id].borrower,
            "must be the borrower to claim collateral"
        );
        (
            int256 collateralUSD,
            int256 outstandingUSD, /*score*/
            ,
            uint256 ltv,
            int256 availableCollateral
        ) = _getPricedLoanInfo(_id);

        require(
            int256(_amount) <= int256(availableCollateral),
            "You may not remove this much collateral yet"
        );

        manager.withdrawal(_id, _amount, msg.sender);
    }

    /**
     * @dev function for admin to liquidate a loan manually
     */
    function liquidateByAdmin(uint256 _id) external onlyOwner {
        if (isDelinquent(_id)) {
    ...
    function isDelinquent(uint256 _id) public view override returns (bool) {
        ...
        // confusing if statement to put as much in one if as possible. Sorry reader
        if (
            (collateralUSD <= 0) || // loan is first delinquent if super delinquent function is true. OR collateral is 0
            !((outstandingUSD * int256(ONE_HUNDRED_PERCENT)) / collateralUSD <= // loan is ALSO delinquent if ratio of outstanding to collateral is NOT <= LTV
                int256(ltv))
        ) {
            return true;
        } else {
     ...
     * @return outstanding value in USD
     * @return Roci score for the loan's NCFS ID
     * @return LTV
     */
        ...
        )
    {
        ...
        (address erc20Contract, uint256 collateral) = manager
            .getCollateralLookup(address(this), _id);
        uint16 score = ScoreOracleInterface(creditOracle).getCurrentScore(
            ln.ncfsID
        );

        uint256 ltv = ScoreOracleInterface(creditOracle).getLtv(score);

        int256 collateralUSD = priceFeed.getLatestPriceUSD(erc20Contract) *
            int256(collateral);
        int256 outstanding = int256(ln.totalPaymentsValue) -
            int256(ln.paymentComplete); // cannot be 0 because of isComplete return false

        if (outstanding < 0) outstanding = 0;

        int256 availableCollateral = int256(collateral) -
            (int256(outstanding) * int256(ONE_HUNDRED_PERCENT)) /
            int256(ltv);

        int256 outstandingUSD = priceFeed.getLatestPriceUSD(ln.ERC20Address) *
            outstanding;

        return (collateralUSD, outstandingUSD, score, ltv, availableCollateral);
    }
    ...
    function _liquidate(uint256 _id, address _receiver) internal virtual {
        ...
        manager.withdrawal(_id, amount, _receiver);
    }
}
...
import {DEAD, LATE_PENALTY, ONE_HUNDRED_PERCENT} from "../Globals.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import {IERC1155} from "@openzeppelin/contracts/token/ERC1155/IERC1155.sol";
import {IERC721} from "@openzeppelin/contracts/token/ERC721/IERC721.sol";
import {ScoreOracleInterface} from "../interfaces/ScoreOracleInterface.sol";
...
contract ERC20PaymentStandard is ERC1155Holder {
    uint256 constant poolScore = 5; //hardcoded for now
     ...
     */
    constructor(address _bonds, address _creditOracle, address _ncfs) {
        bondContract = _bonds;
    ...
    modifier onlyBonds() {
        require(
            msg.sender == bondContract,
            "Only the bond contract can call this function"
        );
        _;
    }
    ...
    modifier incomplete(uint256 _id) {
        require(
            loanLookup[_id].paymentComplete <
        ...
        returns (uint256, address)
    {
        require(
            !loanLookup[_id].issued,
            "You have already issued bonds for this loan"
        );
     ...
     */
    function addInterest(uint256 _amm, uint256 _id)
        external
        virtual
        onlyBonds
        returns (bool)
     ...
     */
    function getLoanInfo(uint256 _id) public view virtual returns (uint256,uint256) {
        loan memory ln = loanLookup[_id];
        if(_isLate(_id)){
            return (
                (ln.interestRate * LATE_PENALTY) / ONE_HUNDRED_PERCENT, 
                ln.accrualPeriod
            );
        }
        return (ln.interestRate,ln.accrualPeriod);
    }
     ...
     */
    function isDelinquent(uint256 _id) public view virtual returns (bool) {
        return (_isLate(_id));
    }
    ...
    function configureNew(
        ...
        require(IERC721(NCFS).ownerOf(_NCFSID) == _borrower, "NCFS ID must bellong to the borrower");
     ...
     * @param _receiver is the receiver of erc20 tokens
     */
    function withdrawl(uint256 _id, uint256 _amm, address _receiver) public virtual {
        IERC1155 bonds = IERC1155(bondContract);
        IERC20 erc20 = IERC20(loanLookup[_id].ERC20Address);
        require(
            loanLookup[_id].issued,
        ...
        incomplete(_id)
    {
        loan memory ln = loanLookup[_id];
        require(
            _erc20Ammount >= ln.minPayment || //Payment must be more than min payment
        ...
        IERC20(ln.ERC20Address).transferFrom(
            msg.sender,
            address(this),
            _erc20Ammount
        );
     ...
     */
    function isComplete(uint256 _id) public view virtual returns (bool) {
        return
            loanLookup[_id].paymentComplete >=
            loanLookup[_id].totalPaymentsValue;
    }
    ...
    * @return true or false 
    ...
    function _isLate(uint256 _id) internal view virtual returns (bool) {
        return (block.timestamp >= loanLookup[_id].paymentDueDate &&
            !isComplete(_id));
    }
...
* @author Carson Case
* @notice An interface to manage the collateral of the Roci protocol
 */
interface ICollateralManager{
    function getCollateralLookup(address, uint) external view returns(address, uint);
    function deposit(address, address, uint, uint) external;
    function withdrawal(uint256, uint256, address) external;
}
...
interface ScoreOracleInterface {
  // Initiates logic to update score and fulfill loan.
  function requestUpdatedScoreBorrow(uint256, uint256, bytes32, bytes memory) external;
  // Returns the current scored for the token from the on-chain storage.
  function getCurrentScore(uint256) external view returns (uint16);
  // Gets the LTV for a given score
  function getLtv(uint16 _score) external view returns (uint256);
  // sets the LTV for a given score
  function setLtv(uint16 _score, uint256 _ltv) external view returns (uint256);
}
