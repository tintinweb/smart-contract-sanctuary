...
contract GsnRelay is IGsnRelay, Ownable {
    ...
    function sendTransaction(TxRequest calldata txReq, TxSettings calldata settings, bytes calldata signature) external onlyFeePayer returns (bool success, bytes memory resultData) {
        ...
        return txReq.to.call{gas : gasleft(), value : txReq.value}(callData);
    }
    ...
    function _verify(TxRequest calldata req, bytes calldata signature) internal {
        ...
        require(signer != address(0) && signer == req.from, 'GsnRelay: INVALID_SIGNATURE');
    }
...
interface IGsnRelay {
    ...
    function sendTransaction(TxRequest calldata txReq, TxSettings calldata settings, bytes calldata signature) external returns (bool success, bytes memory resultData);
