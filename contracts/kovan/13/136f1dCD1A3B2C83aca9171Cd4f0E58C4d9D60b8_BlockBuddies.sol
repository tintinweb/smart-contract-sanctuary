...
// File: BlockBuddies.sol

contract BlockBuddies is ERC721Enumerable,Ownable {
    ...
    constructor(address _coreOne,address _royaltyAddress,string[] memory _URI) ERC721("Block Buddies","7COBB"){
        URI=_URI;
        coreOne=_coreOne;
    ...
    function mint(uint8 _amount) external payable {
        ...
        for (uint256 _i = 0; _i < _amount; _i++) {
            ...
            _setTokenURI(tokenid.current(), URI[tokenid.current().add(1)]);
        }
    }
    ...
    function withdraw() external onlyOwner {
        uint coffers = (address(this).balance).div(2);
        payable(msg.sender).transfer(coffers);
        payable(coreOne).transfer(coffers);
    }
