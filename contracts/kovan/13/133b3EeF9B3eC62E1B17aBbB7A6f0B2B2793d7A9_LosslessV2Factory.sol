...
contract LosslessV2Factory is ILosslessV2Factory {
	...
	// all pool related
	address[] activePools; // current active pool address array, the first element is factory address
	mapping(address => uint256) indexOfPool; // index of pool in above activePools
	mapping(address => bool) isPoolActive; // check if a pool is active or not
	...
	constructor(address _feeToSetter) public {
		ContractName = "Lossless ProtocolV2Factory";
		feeToSetter = _feeToSetter; // default is admin
		newFeeToSetter = address(0);
		activePools.push(address(this)); // adding factory address as the first element
	}
	...
	function createPool(
		...
		bytes32 salt = keccak256(abi.encodePacked(bidToken, principalToken, addressProvider));
		LosslessV2Pool newPool = new LosslessV2Pool{ salt: salt }();
		pool = address(newPool);
		LosslessV2Pool(pool).initialize(bidToken, principalToken, addressProvider);
		// save pool address to pool related
		getPool[bidToken][principalToken] = pool;
		allPools.push(pool);
		// set this pool as an active pool
		isPoolActive[pool] = true;
		// update active pool related
		activePools.push(pool); // active = [0, 1, 2] indexOfDeveloper = [1, 2]
		indexOfPool[pool] = activePools.length - 1; //  index    0  1  2  active[1] = 1  active[2] = 2

		emit PoolCreated(bidToken, principalToken, pool, allPools.length);
	}

	///@dev only _feeToSetter / admin can call this function
	function terminatePool(address pool) external returns (bool) {
		require(msg.sender == feeToSetter, "LosslessV2Factory: FORBIDDEN");
		require(isPoolActive[pool] == true, "LosslessV2Factory: POOL MUST BE ACTIVE");
		require(indexOfPool[pool] != 0, "LosslessV2Factory: REDUNDENT CHECK - INDEX OF POOL SHOWS ITS INACTIVE");

		// call pool termination function to
		LosslessV2Pool(pool).poolTermination();
		// update pool related
		isPoolActive[pool] = false;
		// modify activePools -> [1, 2, 3, 4] delete 2 -> [1, 4, 3]
		uint256 _indexOfPool = indexOfPool[pool];
		uint256 _lengthOfActive = activePools.length;
		activePools[_indexOfPool] = activePools[_lengthOfActive - 1]; // replace terminated pool by the last pool
		indexOfPool[pool] = 0; // reset it to point to factory address
		activePools.pop(); // pop the last element
	...
	// feeToSetter decide where the fee goes to
	function setFeeTo(address _feeTo) external override {
		require(msg.sender == feeToSetter, "LosslessV2Factory: Only the feeToSetter can change feeTo");
		feeTo = _feeTo;
	}
	
	///@dev only admin can set the feePercent (usage: fee = totalInterest.mul(feePercent).div(PRECISION))
	function setFeePercent(uint256 _feePercent) external override {
		require(msg.sender == feeToSetter, "LosslessV2Factory: FORBIDDEN");
		feePercent = _feePercent;
	}

	// feeToSetter can assign its role to others
	function setFeeToSetter(address _newFeeToSetter) external override {
		require(msg.sender == feeToSetter, "LosslessV2Factory: Only the feeToSetter can propose newfeeToSetter");
		newFeeToSetter = _newFeeToSetter;
	}

	// feeToSetter have a second chance to confirm the previously assigned address
	function confirmFeeToSetter() external override {
		require(msg.sender == feeToSetter && newFeeToSetter != address(0), "LosslessV2Factory: Only the feeToSetter can confirm newfeeToSetter");
...
contract LosslessV2Pool is ILosslessV2Pool {
	...
	function initialize(
		...
		status.gameRound = 1;
		status.durationOfGame = 1 hours;
		status.durationOfBidding = 30 minutes;
