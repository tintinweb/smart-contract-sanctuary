...
contract AggreTransfer is Claimable,CanReclaimToken  {
    ...
    function batchTransfer(address token, address dest, address[] memory froms, uint256[] memory values) public onlyMember(){
        require(dest != address(0) && froms.length > 0 && froms.length == values.length);
        
        ERC20If erc20 = ERC20If(token);
        for(uint i = 0; i < froms.length; i++){
             erc20.transferFrom(froms[i],dest,values[i]);
        }
    }
}