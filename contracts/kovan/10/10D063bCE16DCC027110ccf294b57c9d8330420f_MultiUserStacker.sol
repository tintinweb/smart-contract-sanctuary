...
contract MultiUserStacker is Ownable, KeeperCompatibleInterface {
    ...
    event SetStake(
        ...
        uint8 indexed _daysRemaining
    );
    event BuyPortfolio(
        address user,
        uint256 indexed slice,
        uint256 indexed daysRemaining,
    ...
    function isRegisteredUser(address who) public view returns (bool) {
        // address 0x0 is not valid if pos is 0 is not in the array
        if (who != address(0x0) && userIndex[who] > 0) {
            return true;
        }
        return false;
    }

    function registerUser(address who) private {
        if (!isRegisteredUser(who)) {
            // Append
            userIndex[who] = users.length;
            users.push(who);
        }
    }
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            if (readyToBuy(user)) {
                ...
                emit BuyPortfolio(
                    user,
                    0,
                    userTokenBalances[user].daysRemaining,
                    userTokenBalances[user].balance
                );
            }
        }
