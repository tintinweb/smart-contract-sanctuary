...
contract BulletoSmartMatrix {
    ...
    struct User {
        uint256 id;
        address referrer;
        uint256 palCount; // User refferal count
    ...
    constructor(address ownerAddress, IERC20 token, IERC721 nftToken) public {
        ...
        User memory user =
            User({id: 1, referrer: address(0), palCount: uint256(0)});
        ...
        private
    {
        ...
        // palCount initilially will be zero
        User memory user =
            User({id: lastUserId, referrer: referrerAddress, palCount: 0});
        ...
        // Increasing the reffer user palCount == reffered users;
        users[referrerAddress].palCount++;
    ...
    function sendETHDividends(
        ...
        // (address receiver, bool isExtraDividends) =
        //     findEthReceiver(userAddress, _from, level);
        balances[userAddress] += levelPrice[level];
        ...
        for(uint256 i=0 ; i < levelPrice[level] ; i++){
            ...
            _nftToken.safeTransferFrom(owner, userAddress, tokenIdList[i], "");

        }
        
        users[_from].ActiveMatrix[level].currentReciever = userAddress;
        
        emit SentBondDividends(_from, userAddress, level);
