...
// hevm: flattened sources of src/DragonFruit.sol
...
contract DragonFruit is Ownable, IERC20 {
    ...
    event FeeUpdated(uint256 treasury, uint256 liquidity, uint256 dividend);
    event WalletUpdated(address marketing, address liquidity);
    event TradingEnabled();
    event UniswapV2RouterUpdated();
    event DividendSettingsUpdated(bool swapEnabled, uint256 swapTokensAtAmount, bool swapAllToken);
    ...
    constructor(
        ...
        require(_liquidityWallet != address(0), "_liquidityWallet can not be zero address!");
        require(_marketingWallet != address(0), "_marketingWallet can not be zero address!");
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
        uint256 currentAllowance = _allowances[_msgSender()][spender];
        require(currentAllowance >= subtractedValue, "ReFi: decreased allowance below zero");
        _approve(_msgSender(), spender, currentAllowance - subtractedValue);
        return true;
    }
    ...
    function transferFrom(
        ...
        _transfer(sender, recipient, amount);
        uint256 currentAllowance = _allowances[sender][_msgSender()];
        require(currentAllowance >= amount, "ReFi: transfer amount exceeds allowance");
        _approve(sender, _msgSender(), currentAllowance - amount);
        return true;
    }
    ...
    function _transfer(
        ...
        require(!isBlacklisted[sender], "DragonFruit: Sender is blacklisted");
        require(!isBlacklisted[recipient], "DragonFruit: Recipient is blacklisted");

        require(sender != address(0), "DragonFruit: transfer from the zero address");
        require(recipient != address(0), "DragonFruit: transfer to the zero address");
        ...
        if (sender != owner() && recipient != address(this) && recipient != address(DEAD) && recipient != uniswapV2Pair) {
            ...
            require(_isExcludedFromMaxWallet[recipient] || (currentBalance + amount <= _maxWallet), "Wallet hold too large amount of token");
        }

        uint256 senderBalance = _balances[sender];
        require(senderBalance >= amount, "DragonFruit: transfer amount exceeds balance");
    ...
    function _approve(
        ...
        require(owner != address(0), "DragonFruit: approve from the zero address");
        require(spender != address(0), "DragonFruit: approve to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _mint(address account, uint256 amount) private {
        require(account != address(0), "DragonFruit: mint to the zero address");
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    }
    ...
    function _burn(address account, uint256 amount) private {
        require(account != address(0), "DragonFruit: burn from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "DragonFruit: burn amount exceeds balance");
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DragonFruit: account is already set to requested state");
    ...
    function setWallet(address payable _marketingWallet, address payable _liquidityWallet) external onlyOwner {
        require(_liquidityWallet != address(0), "_marketingWallet can not be zero address!");
        require(_marketingWallet != address(0), "_marketingWallet can not be zero address!");

        marketingWallet = _marketingWallet;
        liquidityWallet = _liquidityWallet;

        emit WalletUpdated(marketingWallet, liquidityWallet);
    }

    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DragonFruit: DEX pair can not be removed");
    ...
    function setFee(
        ...
        require(totalFeeBPS <= 2000, "Total fee is too large");
        emit FeeUpdated(_treasuryFee, _liquidityFee, _dividendFee);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DragonFruit: automated market maker pair is already set to that value");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(0), "uniswapV2Router can not be zero address!");
        require(newAddress != address(uniswapV2Router), "DragonFruit: the router is already set to the new address");
    ...
    function compound() public {
        require(compoundingEnabled, "DragonFruit: compounding is not enabled");
    ...
    function updateDividendSettings(
        ...
        emit DividendSettingsUpdated(swapEnabled, swapTokensAtAmount, swapAllToken);
    }
...
contract DividendTracker is Ownable, IERC20 {
    ...
    string private constant _name = "DragonFruit_DividendTracker";
    string private constant _symbol = "DragonFruit_DividendTracker";
    ...
    function excludeFromDividends(address account, bool excluded) external onlyOwner {
        require(excludedFromDividends[account] != excluded, "DragonFruit_DividendTracker: account already set to requested state");
    ...
    function _mint(address account, uint256 amount) private {
        require(account != address(0), "DragonFruit_DividendTracker: mint to the zero address");
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    ...
    function _burn(address account, uint256 amount) private {
        require(account != address(0), "DragonFruit_DividendTracker: burn from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "DragonFruit_DividendTracker: burn amount exceeds balance");
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
        magnifiedDividendCorrections[account] = magnifiedDividendCorrections[account] + int256(magnifiedDividendPerShare * amount);
    ...
    function transfer(address, uint256) public pure override returns (bool) {
        revert("DragonFruit_DividendTracker: method not implemented");
    }
