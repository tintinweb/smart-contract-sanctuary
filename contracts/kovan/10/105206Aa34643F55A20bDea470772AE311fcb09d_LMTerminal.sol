...
contract LMTerminal is Initializable {
    ...
    function withdrawFees() public onlyRevenueController {
        ...
        for (uint256 i = 0; i < deployedCLRPools.length; ++i) {
            address[] memory rewardTokens = deployedCLRPools[i]
                .getRewardTokens();

            for (uint256 j = 0; j < rewardTokens.length; ++j) {
                uint256 fees = rewardFeesTotal[rewardTokens[j]];
                IERC20(rewardTokens[j]).safeTransfer(msg.sender, fees);
                rewardFeesTotal[rewardTokens[j]] = 0;
                emit TokenFeeWithdraw(rewardTokens[j], fees);
            }
        }
...
interface IxAssetCLR is IERC20, IStakingRewards {
    ...
    function transferStakedRewards(address from, address to, uint256 amount) external;
}
