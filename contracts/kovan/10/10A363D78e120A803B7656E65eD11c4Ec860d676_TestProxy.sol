...
contract TestProxy {
    ...
    function testAmount(PoolConfig memory poolConfig) external {
        console.log(poolConfig.amounts[1]);
    }
    ...
    function approveSender(PoolConfig memory poolConfig) external {
        for (uint256 index = 0; index < poolConfig.tokens.length; index++) {
            TransferHelper.safeApprove(
                poolConfig.tokens[index],
                LBPFactoryAddress,
    ...
    function testTransfer(PoolConfig memory poolConfig) external {
        // 1: deposit tokens and approve vault
        for (uint256 index = 0; index < poolConfig.tokens.length; index++) {
            console.log('1');
            TransferHelper.safeTransferFrom(
    ...
    // test only create and mapping
    function noTransferCreateLBPFactoryFromProxy(PoolConfig memory poolConfig) external {
        LBPFactory(LBPFactoryAddress).create(
            ...
            poolConfig.weights,
            poolConfig.swapFeePercentage,
            0x88CbbDC458B7F074630975A8Bf2AFcFf3AdEf892, //address(this), // owner set to this proxy
            false // swaps disabled on start
        );
        poolOwner[0x6Dec8d0496590521C8F4f1deE7Cfe9c7A6B7a480] = poolConfig.owner;
    }

    // test creation with no transfer
    function noTransferFull(PoolConfig memory poolConfig) external {
        // 2: pool creation
        address pool = LBPFactory(LBPFactoryAddress).create(
            poolConfig.name,
            poolConfig.symbol,
