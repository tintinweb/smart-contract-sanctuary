...
contract MagicShuffle is VRFConsumerBase, ConfirmedOwner(msg.sender) {
    uint256 private constant NUMBERS = 256;

    bytes32 private s_keyHash;
    uint256 private s_fee;
    mapping(bytes32 => string) private s_rollers;
    mapping(string => uint8[]) private s_results;

    event DiceRolled(bytes32 indexed requestId, string indexed roller);
    event DiceLanded(bytes32 indexed requestId, uint8[NUMBERS] indexed result);
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        uint8 d20Value = uint8((randomness % 20) + 1);

        uint8[NUMBERS] memory random;
        random[0] = d20Value;
     ...
     */
    function traits(string memory player) public view returns (uint8[] memory) {
