...
contract VaultCore is VaultStorage, Ownable {
	...
	function mintView(
		...
		uint8 collaAddrDecimal = collaAddrERC20.decimals();
		CollaDepAmtCorrected = CollaDepAmt.div(10**(18 - collaAddrDecimal));
		//collaAddrERC20.safeTransferFrom(msg.sender, collaValut, CollaDepAmtCorrected);
	...
	function redeemView(
		...
		//Unlock collaeral
		uint CollaUnlockAmtDiv = chiPrec.mul(priceColla);
		uint CollaUnlockAmt = USDsAmt.mul((chiPrec - chiRedeem()).mul(precisionColla)).div(CollaUnlockAmtDiv);
		if (swapFee > 0) {
			CollaUnlockAmt = CollaUnlockAmt.sub(CollaUnlockAmt.mul(swapFee).div(swapFeePresion));
		}
		CollaUnlockAmtCorrect =  CollaUnlockAmt.div(10**(18 - IERC20(collaAddr).decimals()));
