...
contract UniswapOracle is IUniswapOracle, CoreRef {
    ...
    bool public override isPrice0;

    /// @notice the window over which the initial price will "thaw" to the true peg price
    uint32 public override duration;
    uint256 public override decimalsMultiplier; // eg. USDT, DAI

    uint256 private constant USDC_DECIMALS_MULTIPLIER = 1e6; // to normalize USDC and ETH wei units
    ...
    function read() external view override returns (Decimal.D256 memory, bool) {
        ...
        uint256 priceX192 = (sqrtPriceX96**2).mul(decimalsMultiplier).div(USDC_DECIMALS_MULTIPLIER);
        if (isPrice0) {
            return (Decimal.ratio(priceX192, 2**192), valid);
        } else {
            return (Decimal.ratio(2**192, priceX192), valid);
        }
    }
