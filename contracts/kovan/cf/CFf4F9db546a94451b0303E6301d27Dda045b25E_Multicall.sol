...
contract Multicall {
    ...
    event BurnersApproval(address indexed from,address[] burnusers,uint time);
    ...
    function mintInitaiate(address user,uint256 amount,address _target)public {
        require(mintUsers[msg.sender] == true,"No access other to mint");
        mintId++;
    ...
    function burnInitaiate(address user,uint256 amount,address _target)public {
       require(burnUsers[msg.sender] == true,"No access other to burn");
       burnId++;
    ...
    function openMintApprove() public onlyOwner {
        ...
        if (openMintApproval[owner].status == true) {
            ...
            openMintApproval[owner].status = false;
            ...
            require(msg.sender == owner,"Owner only wants to approve");
            openMintApproval[superAdmin].status = false;
    ...
    function openBurnApprove() public onlyOwner {
        ...
        if (openBurnApproval[owner].status == true) {
            require(msg.sender == superAdmin,"Superadmin only wants to approve");
            openBurnApproval[owner].status = false;
            ...
            require(msg.sender == owner,"Owner only wants to approve");
             openBurnApproval[superAdmin].status = false;
