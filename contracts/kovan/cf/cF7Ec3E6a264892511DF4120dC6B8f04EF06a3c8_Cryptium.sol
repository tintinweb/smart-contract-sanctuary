...
contract Cryptium is CrptERC20, Control, ReentrancyGuard {
	 ...
	 */
	function getFreeToSell() external view initialized returns (uint256 limitAvail) {
		if (_isWhiteList[msg.sender]) {
			limitAvail = maxSub((balanceOf(msg.sender) * _feeFree) / 100, _userLimit[msg.sender].monthLimit);
		} else {
	 ...
	 * @return newPrice
	 */
	function _getTWAT() private view initialized returns (uint256 oldPrice, uint256 newPrice) {
		uint32 valueCount = 3;
	...
	function _getPrice(
		...
		uint32 period,
		uint8 decDiff
	) internal view initialized returns (uint256 oldPrice, uint256 newPrice) {
