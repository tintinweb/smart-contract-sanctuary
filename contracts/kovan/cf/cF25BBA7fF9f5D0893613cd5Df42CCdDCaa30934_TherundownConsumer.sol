...
contract TherundownConsumer is ChainlinkClient {
     ...
     * @param _market 'create' or 'resolve' the market for a given date.
     * @param _payment the LINK amount in Juels (i.e. 10^18 aka 1 LINK).
     * @param _sportId the sportId of the sport to query.
     * @param _gameIds the gameIds of the games to query (',' seperated list of gameIds).
     * @param _statusIds the statusIds of the games to query (',' seperated list of statusIds).
     * @param _date the date for the games to be queried (format YYYY-MM-DD).
     */
    ...
    function getGamesCreated(uint256 _idx) external view returns (GameCreated memory) {
        GameCreated memory game = abi.decode(games[_idx], (GameCreated));
        return game;
    }

    function getGamesResolved(uint256 _idx) external view returns (GameResolved memory) {
        GameResolved memory game = abi.decode(games[_idx], (GameResolved));
        return game;
    }
