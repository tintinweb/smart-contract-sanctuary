...
contract ERC20 is Context, IERC20 {
    ...
    uint256 public taxSplit = 50000; // 50% split

    string private _name;
    string private _symbol;
    uint8 private _decimals;
    
    event DevFundChanged(address indexed _devFund);
    event StakePoolChanged(address indexed _stakePool);
    ...
    function _setDevFund(address newDevFund) internal virtual {
        devFund = newDevFund;
        emit DevFundChanged(newDevFund);
    }
    
    function _setStakePool(address newStakePool) internal virtual {
        require(newStakePool != address(0), "Stake pool address must be set to something");
        stakePool = newStakePool;
        emit StakePoolChanged(newStakePool);
    }
    
    function _setTaxSplit(uint256 _split) internal virtual {
        require(_split <= 100000, "Split is an invalid value");
        taxSplit = _split;
    }
    ...
    function _tax(address account, uint256 amount) internal virtual {
        ...
        require(stakePool != address(0), "Stake pool must be set to something");

        _balances[account] = _balances[account].sub(amount, "ERC20: burn amount exceeds balance");
        uint256 devTax = 0;
        if(devFund != address(0)){
            devTax = amount.mul(taxSplit).div(DIVISION_FACTOR);
            _balances[devFund] = _balances[devFund].add(devTax);
            devTax = amount.sub(devTax);
        }
        _balances[stakePool] = _balances[stakePool].add(devTax);
        emit Transfer(account, devFund, amount);
    }
...
contract BonsaiToken is ERC20("Bonsai Token", "BONS"), Ownable {
    using SafeMath for uint256;

    // Variables
    uint256 private _maxSupply = 1000000e18; // The default max supply of tokens
    uint256 public taxRate = 3000; // Governance can update the tax rate as it gets closer to the max supply
    address public minter; // Address that can mint new tokens (the farm contract)
    ...
    constructor() public {
        // We will mint 2,000 tokens to the deployer to use for the initial dev funds and liquidity
        devFund = _msgSender(); // Default devfund is the deployer
        taxExemptAccounts[devFund] = true;
        _mint(_msgSender(), 2000e18);
    }

    modifier onlyMinter() {
        require(minter == _msgSender(), "Ownable: caller is not the claimer");
        _;
    }
    ...
    function mint(address _to, uint256 _amount) external onlyMinter returns (bool) {
        _mint(_to, _amount);
        require(totalSupply() <= _maxSupply, "Cannot mint any more tokens"); // After max supply is reached
        return true;
    }
    
    function _setMinter(address _newMinter) internal {
        require(_newMinter != address(0), "Minter address must be set to something");
        minter = _newMinter;
    ...
    function finishChangeMinter() external onlyGovernance timelockConditionsMet(3) {
    ...
    function finishChangeEffectiveSupply() external onlyGovernance timelockConditionsMet(5) {
        _maxSupply = _timelock_data;
    }
    // --------------------
    
    // Change the dev address
    // --------------------
    function startChangeDevFund(address _address) external onlyGovernance {
        _timelockStart = now;
        _timelockType = 6;
        _timelock_address = _address;       
    }
    
    function finishChangeDevFund() external onlyGovernance timelockConditionsMet(6) {
        taxExemptAccounts[_timelock_address] = true;
        _setDevFund(_timelock_address);
    }
    // --------------------
    
    // Change the stake pool address
    // --------------------
    function startChangeStakePool(address _address) external onlyGovernance {
        _timelockStart = now;
        _timelockType = 7;
        _timelock_address = _address;       
    }
    
    function finishChangeStakePool() external onlyGovernance timelockConditionsMet(7) {
        taxExemptAccounts[_timelock_address] = true;
        _setStakePool(_timelock_address);
    }
    // --------------------

    // Used to change the max supply
    // --------------------
    function startChangeTaxSplit(uint256 _split) external onlyGovernance {
        _timelockStart = now;
        _timelockType = 8;
        _timelock_data = _split;
    }
    
    function finishChangeTaxSplit() external onlyGovernance timelockConditionsMet(8) {
        taxSplit = _timelock_data;
    }
    // --------------------
}