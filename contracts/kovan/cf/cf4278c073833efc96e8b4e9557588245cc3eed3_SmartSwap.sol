...
// ============================ Internal functions ============================
    ...
    function _transferFee(address tokenA, uint256 amount, address user, address licensee) internal {
        ...
        uint256 _companyFee = isSystem[msg.sender] ? companySPFee : companyFee;
        if (tokenA < NATIVE_COINS) {
    ...
    function clearBalances(uint pairID, address[] calldata users) external onlyOwner {
        ...
        for (uint i = 0; i < users.length; i++) {
            ...
            _balanceOf[_getHashAddress(tokenA, tokenB, users[i], users[i])] = 0;
            _balanceOf[_getHashAddress(tokenB, tokenA, users[i], users[i])] = 0;
        }
    }
    // =============================== END for TEST only =============================================================
}