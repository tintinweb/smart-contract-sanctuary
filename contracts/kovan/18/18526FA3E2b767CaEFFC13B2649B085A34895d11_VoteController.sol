...
contract VoteController {
    ...
    function prepareVoteNumber(address _comp, uint256 _proposalId, uint256 _against, uint256 _for, uint256 _abstain) public {
    ...
    function finishVoteNumber(address _comp, uint256 _proposalId) public {
        require(msg.sender == operator || msg.sender == governance, "!operator");
        ...
        address _vote = againsts[_comp];
        if (_vote != address(0)) {
            uint256 _balance = _returnVote(_comp, _vote);
            balance.add(_balance);
        }
        _vote = fors[_comp];
        if (_vote != address(0)) {
            uint256 _balance = _returnVote(_comp, _vote);
        ...
        IController(controller).depositVote(_comp, balance);
    }

    function _tranferVote(address _comp, address _vote, uint256 _voteNumber) internal {
        require(_vote != address(0), "address(0)");
        uint256 _balance = IERC20(_comp).balanceOf(address(this));
        if (_voteNumber > _balance) {
            _voteNumber = _balance;
        }
        IERC20(_comp).safeTransfer(_vote, _voteNumber);
    }

    function _returnVote(address _comp, address _vote) internal returns (uint256 _balance) {
        _balance = IERC20(_comp).balanceOf(_vote);
        if (_balance > 0) {
            IERC20(_comp).transferFrom(_vote, controller, _balance);
        }
    }
