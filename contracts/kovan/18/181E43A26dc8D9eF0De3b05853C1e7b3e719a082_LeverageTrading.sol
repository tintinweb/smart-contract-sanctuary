...
contract Admin is RoleAware {
    ...
    function claimDeposit(uint256 amount) external {
        ...
        _claimDeposit(msg.sender, amount, msg.sender);
    }
    ...
    function fineSupport(
        ...
        _claimDeposit(maintainer, stakeTaken, recipient);
    }
}
...
abstract contract BaseRouter is UniswapStyleLib {
    ...
    /// and for pairs to be vetted (which getSumIn / getSumOut do)
    function trade(
...
abstract contract LeverageAcc is RoleAware, PriceMath {
    ...
    function sumTokensInPeg(
        ...
        for (uint256 tokenId; tokenId < len; tokenId++) {
            ...
            totalPeg += PriceMath.getCurrentPriceSec(token, amounts[token]);
        }
    }
    ...
    function viewTokensInPeg(
        ...
        for (uint256 tokenId; tokenId < len; tokenId++) {
            address token = tokens[tokenId];
            totalPeg += PriceMath.viewCurrentPriceSec(token, amounts[token]);
        }
    }
    ...
    function yieldTokenInPeg(
        ...
        return PriceMath.getCurrentPriceSec(token, amountInToken);
    }
    ...
    function viewYieldTokenInPeg(
        ...
        // 1 * FP / FP = 1
        uint256 amountInToken = (amount * yieldFP) / yieldQuotientsFP[token];
        return PriceMath.viewCurrentPriceSec(token, amountInToken);
    }
...
abstract contract LeverageMarginCall is LeverageAcc {
    ...
    function liquidateFromSec() internal returns (uint256 pegAmount) {
        ...
        for (uint256 tokenIdx = 0; len > tokenIdx; tokenIdx++) {
            ...
            if (liq.buy > liq.sell) {
                pegAmount += PriceMath.liquidateFromSec(
                    buyToken,
                    liq.buy - liq.sell
                );
    ...
    function liquidateToSec() internal returns (uint256 pegAmount) {
        uint256 len = liquidationTokens.length;
        for (uint256 tokenIndex = 0; len > tokenIndex; tokenIndex++) {
            ...
            if (liq.sell > liq.buy) {
                ...
                pegAmount += PriceMath.liquidateToSec(token, sellAmount);
        ...
        returns (uint256 maintainerCut)
    {
        ...
        uint256 sale2pegAmount = liquidateToSec();
        uint256 peg2targetCost = liquidateFromSec();
...
abstract contract PriceMath is RoleAware, UniswapStyleLib 
{
    ...
    /// Set refresh delays
    function setUpdateLag(uint16 price, uint256 voluntary)
        external
        onlyOwnerExec
    {
    ...
    function chooseVolatility(uint256 index)
        external
    ...
    function setRefreshRate(uint256 rate) external onlyOwnerExec {
        rateUpdateMil = rate;
    }

    function getCurrentPriceSec(address token, uint256 inAmount)
        internal
        returns (uint256)
    {
        return getCurrentPriceSec(token, inAmount, false);
    }

    function getCurrentPriceSec(
        address token,
        uint256 inAmount,
        bool voluntary
        ...
        if (token == peg) {
            ...
            if (
                ...
                priceUpdateFP = getPriceByPairAssets(
                    tokenPrice.liquidationTokens,
                    tokenPrice.amms
                );
                setPriceValue(tokenPrice, priceUpdateFP, rateUpdateMil);
            }
    ...
    function viewCurrentPriceSec(address token, uint256 inAmount)
        public
        view
    ...
    function setMarginCallRoute(bytes32 amms, address[] memory tokens)
        external
        onlyOwnerExecActivator
    {
        ...
        if (token != peg) {
            ...
            for (uint256 i = 0; tokens.length - 1 > i; i++) {
                initPairAssetsPrice(tokens[i], tokens[i + 1], amms[i]);
            ...
            tokenPrice.priceFP = getPriceByPairAssets(tokens, amms);
            tokenPrice.lastUpdated = block.timestamp;
        }
    }

    function liquidateToSec(address token, uint256 amount)
        internal
        returns (uint256)
    {
        if (token == peg) {
    ...
    function liquidateFromSec(address token, uint256 targetAmount)
        internal
        returns (uint256)
    {
        if (token == peg) {
    ...
    function getPriceByPairAssets(address[] memory tokens, bytes32 amms)
        internal
        returns (uint256 priceFP)
    {
        ...
        for (uint256 i; i < tokens.length - 1; i++) {
            ...
            if (timeDelta > volRefreshDelay) {
                ...
                priceFP = scaleMultiple(priceFP, pairPriceFP);

                pairPrice.priceFP = pairPriceFP;
                pairPrice.cumulative = cumulative;
                pairPrice.lastUpdated = pairLastUpdated;
            } else {
                priceFP = scaleMultiple(priceFP, pairPrice.priceFP);
            }
        }
    }

    function scaleMultiple(uint256 a, uint256 b) internal pure returns (uint256) {
        return ((a / PM8) * (b / PM8)) / PM96;
    }

    function initPairAssetsPrice(
        address inToken,
        address outToken,
        bytes1 amm
    ) internal {
...
contract VaultRouter is RoleAware, BaseRouter {
    ...
    function bridgeDeposit(address depositToken, uint256 depositAmount)
        external
    {
    ...
    function bridgeDepositETH() external payable {
    ...
    function bridgeClaim(address withdrawToken, uint256 withdrawAmount)
        external
    {
    ...
    function bridgeClaimETH(uint256 withdrawAmount) external {
        IMarginTrading(leverageTrading()).listClaim(
            msg.sender,
            WETH,
            withdrawAmount
    ...
    function bridgeCredit(address borrowToken, uint256 borrowAmount) external {
        Lend(lending()).listLoan(borrowToken, borrowAmount);
    ...
    function bridgeOvercollateralizedCredit(
        address depositToken,
        uint256 depositAmount,
        address borrowToken,
        uint256 withdrawAmount
    ...
    function bridgeCloseAccount() external {
    ...
    /// @notice entry point for swapping tokens held in cross margin account
    function tradeTokensForExactTokens(
        uint256 amountOut,
        uint256 amountInMax,
        bytes32 amms,
        address[] calldata tokens,
