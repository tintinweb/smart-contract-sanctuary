...
contract UniCreatePoolV3 is ActionBase{
    ...
    function executeAction(
        ...
        address pool = _createPool(inputData);
        return bytes32(uint256(pool));
    }
    ...
    function _createPool(Params memory _inputData) internal returns (address pool) {
        ...
        if (!(_inputData.token0 < _inputData.token1)){
            address temp = _inputData.token0;
            _inputData.token0 = _inputData.token1;
            _inputData.token1 = temp;
        }

        pool = positionManager.createAndInitializePoolIfNecessary(_inputData.token0, _inputData.token1, _inputData.fee, _inputData.sqrtPriceX96);

        logger.Log(
                address(this),
                msg.sender,
                "UniCreatePoolV3",
                abi.encode(_inputData, pool)
            );
    }
