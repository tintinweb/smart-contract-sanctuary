...
contract LGE is Ownable, LivePrice {
    ...
    function endLGE() public onlyOwner {
      ...
      (, , uint256 amountLP) = UniSwap.addLiquidity(address(SWN), address(STBL), amountSWN, amountSTBL, amountSWN.mul(90).div(100), amountSTBL.mul(90).div(100), address(this), block.timestamp.add(180));
    ...
    function swapETHtoDAI(uint256 amountToSend) internal returns (uint256 amountOut) {
      ...
      uint256 amountOutMin = amountToSend.mul(priceFeed).div(oracleValueDivisor).mul(70).div(100); // 30% slippage for test
