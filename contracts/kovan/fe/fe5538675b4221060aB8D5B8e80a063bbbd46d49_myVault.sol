...
contract myVault {
	uint public version = 1; // myVault version number
	/* Kovan Addresses */
	...
	address public uniswapV3FactoryAddress = 0x74e838ECf981Aaef2523aa5B666175DA319D8D31;
	address public uinswapV3RouterAddress = 0xE592427A0AEce92De3Edee1F18E0157C05861564;
	address public chainLinkETHUSD = 0x9326BFA02ADD2366b30bacB125260Af641031331;
	...
	IUniswapRouter public uniswapRouter = IUniswapRouter (uinswapV3RouterAddress);

	event logThis(string msg, uint ref);
	...
	function annualDividend() public {
		...
		console.log('Dividend Amount:', balance);
	...
	function updateEthPriceChainlink() public returns(uint) {
		int256 chainLinkEthPrice = EACAggregatorProxy(chainLinkETHUSD).latestAnswer();
	...
	function rebalance() public {
		...
		uint usdBalance4a = daiToken.balanceOf(address(this));
		uint usdBalancePercentage = 100 * usdBalance / totalBalance;
		uint usdBalance4b = daiToken.balanceOf(address(this));
		emit logThis('usdBalancePercentage', usdBalancePercentage);
		uint usdBalance4c = daiToken.balanceOf(address(this));
		uint usdBalance4d = daiToken.balanceOf(address(this));
		if (usdBalancePercentage < usdTargetPercentage) {
			uint amountToSell = totalBalance / 100 * (usdTargetPercentage - usdBalancePercentage);
			emit logThis('amountToSell', amountToSell);
			uint usdBalance4e = daiToken.balanceOf(address(this));
			...
			emit logThis('amountToBuy', amountToBuy);
			uint usdBalance5 = daiToken.balanceOf(address(this));
	...
	function buyWeth(uint amountUSD) internal {
		...
		emit logThis('amountIn', amountIn);
		...
		uint usdBalance6 = daiToken.balanceOf(address(this));
		uniswapRouter.exactInputSingle(params);
		uint usdBalance7 = daiToken.balanceOf(address(this));
