...
contract LPYieldFarm {
    ...
    // constants
    uint256 public constant TOTAL_DISTRIBUTED_AMOUNT = 2_384_000_000;
    ...
    // constructor
    constructor(
        address astoTokenAddress,
        address lpTokenAddress,
        address stakeContract,
        address communityVault
    ) public {
        _asto = IERC20(astoTokenAddress);
    ...
    function harvest(uint128 epochId) external returns (uint256) {
        ...
        if (userReward > 0) {
            _asto.transferFrom(_communityVault, msg.sender, userReward);
        }
    ...
    function _harvest(uint128 epochId) internal returns (uint256) {
        ...
        // if it fails either user either a asto account will init not init epochs
