...
contract MintLeverRouter is IMintLeverRouter, ConfigBase {
    ...
    function getAmountClosePosition(address mintLever, address user, uint256 percentage) external view returns (address[] memory, uint256[] memory) {
        ...
        bool isBorrow;
        ILoanPublics borrowLoanPublics = config.tryGetLoanPublics(_mintLever.borrowsV1(user));
        if (address(0) != address(borrowLoanPublics)) {
            (, , uint256 borrowAmount, ) = borrowLoanPublics.getAccountSnapshot(user, _mintLever.id(), ILoanTypeBase.LoanType.MINNING_SWAP_PROTOCOL);
            isBorrow = (0 < borrowAmount);
        }
        (totalSupply, tokens, amounts) = IEarning(_mintLever.earning()).getLPTokens();
        for (uint256 i = 0; i < tokens.length; i++) {
            amounts[i] = amount.mul(amounts[i]).div(totalSupply);
            if (isBorrow) {
                uint256 repayAmount = amounts[i].sub(amounts[i].mul(100 - percentage).div(100));
                amounts[i] = amounts[i].sub(repayAmount);
            }
        }
        return (tokens, amounts);
    }
