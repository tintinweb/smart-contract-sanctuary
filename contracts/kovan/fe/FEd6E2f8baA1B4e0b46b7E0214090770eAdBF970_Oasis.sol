...
import {IGovernance} from "./interfaces/Governance.sol";
import {IRandomNumberConsumer} from "./interfaces/RandomNumberConsumer.sol";
import {IWhitelistedContract} from "./interfaces/WhitelistedContract.sol";
...
contract Oasis is ERC721Enumerable, Ownable, ERC721Holder, KeeperCompatibleInterface, Accountable {
    ...
    {
        baseURL = _baseURL;
        winningURI = _winningURI;

        governance = IGovernance(_governance);

        assert(_whitelistedContracts.length == GIVEAWAY_WINNERS);
    ...
    function deposit(
        ...
        DepositedPrize memory depositedPrize = DepositedPrize(
            contractAddress, 
            ipfsURL,
            tokenId, 
            false /* awarded */
    ...
    function performUpkeep(bytes calldata /* checkData */) external override {
        ...
        giveawayState = GIVEAWAY_STATE.CALCULATING_WINNER;
        
        // for(uint i; i < GIVEAWAY_WINNERS; i++) { 
        //     IRandomNumberConsumer(governance.randomness()).getRandomNumber(i);
        // }
    }

    /*
        Note: The fastest time possible to be returned is 2 * block time, or about 30ish seconds.
    */
    function processPickedWinner(uint winnerId, uint256 randomness) external {
        require(giveawayState == GIVEAWAY_STATE.CALCULATING_WINNER, "Not ready to pick winners.");
        require(randomness > 0, "Random not found.");
        require(winners < GIVEAWAY_WINNERS, "Winners already chosen");
        // require(msg.sender == governance.randomness(), "Invalid sender.");

        uint256 tokenId = randomness;
        // uint256 tokenId = randomness % totalSupply();

        // Prevent the same tokenId from winning twice.
        if(tokenIdToWinnerId[tokenId] == 0) {
            ...
            // IRandomNumberConsumer(governance.randomness()).getRandomNumber(winnerId);
        }
    }
...
interface IRandomNumberConsumer { 
    function getRandomNumber(uint) external view returns (bytes32);
}
