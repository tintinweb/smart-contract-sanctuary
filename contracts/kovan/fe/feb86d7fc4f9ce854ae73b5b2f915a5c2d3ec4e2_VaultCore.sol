...
contract VaultCore is VaultStorage, Ownable {
	...
	function _mint(
		...
		uint USDsAmt;
		uint swapFeeAmount;
		uint CollaDepAmtDiv = chiPrec.mul(priceColla);

		if (valueType == 0) {
			USDsAmt = valueAmt;

			SPABurnAmt = USDsAmt.mul(chi).div(chiPrec).mul(precisionSPA).div(priceSPA);
			if (swapFee > 0) {
				SPABurnAmt = SPABurnAmt.add(SPABurnAmt.mul(swapFee).div(swapFeePresion));
			}

			//Deposit collaeral
			CollaDepAmt = USDsAmt.mul(chiPrec - chi).mul(precisionColla).div(CollaDepAmtDiv);
			if (swapFee > 0) {
			...
			USDsAmt = USDsAmt.mul(chiPrec).div(chi).mul(priceSPA).div(precisionSPA);
			...
			USDsAmt = CollaDepAmt.mul(CollaDepAmtDiv).div(precisionColla).div(chiPrec - chi);
	...
	function _redeem(
		...
		uint chi = chiRedeem();
		uint SPAMintAmt = USDsAmt.mul(chi).div(chiPrec).mul(precisionSPA).div(priceSPA);
		if (swapFee > 0) {
		...
		uint CollaUnlockAmt = USDsAmt.mul(chiPrec - chi).mul(precisionColla).div(CollaUnlockAmtDiv);
		if (swapFee > 0) {
