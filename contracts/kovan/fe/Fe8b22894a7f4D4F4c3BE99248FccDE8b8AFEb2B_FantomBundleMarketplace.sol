    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(address payable _feeRecipient, uint256 _platformFee, IERC20 _RWT, address _rewardTokenOwner, IFantomAddressRegistry _addressRegistry)
        public
        initializer
    {
    ...
    function _buyItem(string memory _bundleID, address _payToken) private {
        ...
        if (_payToken == address(0)) {
            ...
            if (price.sub(feeAmount) != 0) {
                RWT.safeTransferFrom(rewardTokenOwner, owner, price.sub(feeAmount).mul(5).div(100));
