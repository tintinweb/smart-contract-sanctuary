...
contract OwnableDistributor {
    ...
    function setDistributorAddress(address _address) external onlyOwner {
...
contract rd is OwnableDistributor {
    ...
    function rewardLimitPerGamer() public view returns (uint256) {
        ...
        //maximum amount of tokens per gamer, devied by decimals (9) for better clarity
    }
    
    function sendRewardToWinner(address _address, uint256 amount) external onlyDistributor {
        require (owner() == address(0), "first renouce owership. The Owner must be zero address");
        require (_address != address(0), "zero address not allowed");
        require (amount != 0, "amount cannot be zero");
        require (amount <= gamerRewardLimit, "amount of tokens is too high");
