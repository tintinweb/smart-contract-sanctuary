...
contract ERC20Template is ERC20Pausable {
    ...
    function changeTimes(uint256 new_maxTimes, uint256 new_maxMintAmount, uint256 new_timeInterval) public onlyFactory{
     ...
     function changeZeroTime(uint256 new_zeroTime) public onlyFactory{
        zoerTime=new_zeroTime;
    }

    function mint(address account, uint256 amount) public whenNotPaused onlyOperator {
        
        zoerTime = block.timestamp.sub(block.timestamp.mod(86400));

        require(block.timestamp >= zoerTime, "mint time interval is 24 hours");
        ...
        lastUpdateTime=block.timestamp.add(timeInterval);
    }
