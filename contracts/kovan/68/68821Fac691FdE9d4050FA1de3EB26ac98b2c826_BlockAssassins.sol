...
contract BlockAssassins is Ownable, ERC721, ERC721URIStorage, VRFConsumerBase {
    ...
    uint256 public immutable LINK_FEE;
    ...
    event SetClearingEntropy(bytes32 indexed requestId);
    event RevealPendingMetadata(bytes32 indexed requestId);
    ...
    /// @param _LINK_FEE fee to pay for Chainlink
    ...
    function setClearingEntropy() external returns (bytes32) {
        ...
        bytes32 requestId = requestRandomness(KEY_HASH, LINK_FEE);
      
        emit SetClearingEntropy(requestId);
    ...
    function revealPendingMetadata() external returns (bytes32) {
        ...
        emit RevealPendingMetadata(requestId);
    ...
    function tokenURI(uint256 tokenId) public view  override(ERC721, ERC721URIStorage) returns (string memory) {
        ...
        require(metadataCleared, "ERC721Metadata: URI for this token is not cleared yet.");

        randomizedTokenId = (randomness + tokenId) % AVAILABLE_SUPPLY;

        string memory baseURI = _baseURI();
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, _toString(randomizedTokenId))) : "";
    }
