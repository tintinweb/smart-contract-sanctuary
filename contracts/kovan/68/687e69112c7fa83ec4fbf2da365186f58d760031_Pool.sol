...
contract BaseFire is IERC20, Owned, ReentrancyGuard {
    ...
    event Dispersed(address from, uint256 amount);
    ...
    constructor() {
        ...
        _name = "BaseFire Token";
        _symbol = "BAFI";
        _decimals = 18;

        balances.tokenSupply = 100_000_000_000_000_000 ether; // quadrillion
        ...
        buyCounterLimit = 30;
        emissionPerBlock = 10000 ether; // BAFI per block
        ...
        _basedPool = new Pool("BaseFire's Lock Pool");
        pools[0] = _prizePool;
        ...
        // don't let these pools accrue rewards
        exclude(address(_nftPool));
        exclude(address(_basedPool));
        exclude(address(_farmPool));
        exclude(address(_buyBonusPool));
...
//        _pool = IPancakeFactory(IPancakeRouter02(_router).factory()).createPair(address(this), _pair);
        ...
        accounts[address(_basedPool)].feeless = true;
//        accounts[_msgSender()].feeless = true;
        accounts[_pool].transferPair = true;
        uint256 tenPercent = balances.tokenSupply / 10;
        uint256 sTenPercent = tenPercent * ratio();
        accounts[_msgSender()].nTotal = (sTenPercent / 10) * 9; // 10%
        accounts[address(_farmPool)].nTotal = sTenPercent * 3;
        accounts[address(_farmPool)].tTotal = tenPercent * 3;
        accounts[address(_buyBonusPool)].nTotal = sTenPercent * 3; // 20%
        accounts[address(_buyBonusPool)].tTotal = tenPercent * 3; // 20%
        accounts[address(_basedPool)].nTotal = sTenPercent * 3;
        accounts[address(_basedPool)].tTotal = tenPercent * 3;
        accounts[address(0)].nTotal = sTenPercent / 10;  // 1%

        _approve(_msgSender(), _router, tenPercent);

        emit Transfer(address(0), _msgSender(), (tenPercent / 10) * 9);
        emit Transfer(address(0), address(_farmPool), tenPercent * 3);
        emit Transfer(address(0), address(_buyBonusPool), tenPercent * 3);
        emit Transfer(address(0), address(_basedPool), tenPercent * 3);
        emit Transfer(address(0), address(0), tenPercent / 10);
    ...
    */
    function _bfTransfer(address sender, address recipient, uint256 amount) internal returns(bool) {
        ...
        emit Transfer(sender, address(_basedPool), t.burnFee);
    ...
    function _handleAdjustments(address recipient, uint256 rate, ITValues.TState ts, ITValues.TxValue memory t) internal {
        ...
        // half of the burn fee is sent to the pool for bases
        accounts[address(_basedPool)].nTotal += (t.burnFee / 2) * rate;
        accounts[address(_basedPool)].tTotal += (t.burnFee / 2);
        // this other half for permanent burn
        accounts[address(0)].nTotal += (t.burnFee / 2) * rate;
    ...
    function burn(bool permaBurn) external isNotContract(msg.sender) {
        ...
        _burn(_pool, tokenBurnRateDivisor, permaBurn);
    ...
    function _burn(address target, uint256 rate, bool isPerma) internal {
        ...
        if(inflationGuard == false && _inflationCheck == true) {
            ...
            if(inflation > t){
                inflation -= t;
                balances.tokenSupply -= t;
            }
        }
        if(rTarget > accounts[target].nTotal) {
            rTarget = accounts[target].nTotal;
        }
        accounts[target].nTotal -= rTarget;
        if(isPerma) {
            accounts[address(0)].nTotal += rTarget;
            emit Transfer(target, address(0), t);

        } else {
            accounts[address(_basedPool)].nTotal += rTarget;
            accounts[address(_basedPool)].tTotal += t;
            emit Transfer(target, address(_basedPool), t);
        }
    ...
    function base() external isNotContract(msg.sender) {
        ...
        if(inflationGuard == true && accounts[address(_basedPool)].tTotal > 0) {
            // we use the total of the burn address as a base here.
            amountToInflate = accounts[address(_basedPool)].tTotal / inflationRateDivisor;
            if(amountToInflate > accounts[address(_basedPool)].tTotal) {
                // just set it to the entire burn address if its greater
                amountToInflate = accounts[address(_basedPool)].tTotal;
            }
            require(amountToInflate <= accounts[address(_basedPool)].tTotal, "wait for burn to fill up");
            accounts[address(_basedPool)].nTotal -= amountToInflate * rate;
            accounts[address(_basedPool)].tTotal -= amountToInflate;
            // positive rebase
            balances.networkSupply -= amountToInflate * rate;
            emit Dispersed(address(_basedPool), amountToInflate);
            ...
            if(_inflationCheck){
                ...
                emit Dispersed(address(0), amountToInflate);
            }
        }
    ...
    function burnForSuccess(uint256 rate, uint256 pid, bool isPerma) external allowedOnly(msg.sender) {
        require(rate >= 4, "can't burn more than 25%");
        // rate of calling grows linearly
    ...
    function exclude(address account) public allowedOnly(msg.sender) {
        require(!accounts[account].excluded, "Account is already excluded");
    ...
    function harvest() public nonReentrant {
        ...
        uint256 tEmissions = (block.number - _lastRewardBlock) * emissionPerBlock; // blocks * 100 * rate
        accounts[address(_farmPool)].tTotal -= tEmissions;
        accounts[address(_farmPool)].nTotal -= tEmissions * ratio();
        farmed += tEmissions;
        _lastRewardBlock = block.number;
        balances.networkSupply -= tEmissions * ratio();
        emit Dispersed(address(_farmPool), tEmissions);
    }
    ...
    function disperseNFTFees() public { // anyone can disperse nft fees to nft holders at any time, will disperse all fees.
        if(_isNFTPoolActive) {
            ...
            uint256 n = accounts[address(_nftPool)].nTotal;
            uint256 share = n / owners;
            accounts[address(_nftPool)].nTotal = 0;
            accounts[address(_nftPool)].tTotal = 0;
            for (uint256 i = 0; i < owners; i++) {
                accounts[baseFireNFT.getNFTOwners(i)].nTotal += share;
            }
            emit Dispersed(address(_nftPool), n / ratio());
        }
    }
    ...
    function sendToBurn(uint256 amount) external {
        ...
        require(amount * rate < accounts[sender].nTotal, "too much");
        accounts[sender].nTotal -= (amount * rate);
        accounts[address(_basedPool)].nTotal += (amount * rate);
        accounts[address(_basedPool)].tTotal += (amount);
        syncPool();
        emit Transfer(address(this), address(_basedPool), amount);
    }
    ...
    function getSupplySubPools() public view returns(uint256) {
        
        return balances.tokenSupply - balanceOf(address(_basedPool)) - balanceOf(address(0)) - balanceOf(address(_nftPool)) - balanceOf(address(_farmPool)) - balanceOf(address(_buyBonusPool));
    }
