...
contract OriginatorStaking is StakedToken {
    ...
    uint256 public collateralizationGoal;

    event ObjectiveFulfilled(address indexed contributor, uint256 amount);
     ...
     * @param _proposalRate uint256 (value e.g. 0.5 DAI/ETHIX == 500000000000000000)
    ...
    function setUpProposal(
        ...
        uint256 _proposalRate, 
        uint256 _collateralizationPercentGoal, 
        uint256 _referenceGoal
    ) external onlyEmissionManager onProposal{
        ...
        collateralizationGoal = calculateCollateralizationGoal(_collateralizationPercentGoal, _referenceGoal, _proposalRate);
    ...
    function deposit(uint256 _amount) external payable onProposal{
        ...
        if (isObjectiveFulfilled()) {
            ...
            emit ObjectiveFulfilled(msg.sender, totalObjective);
        }
    }
     ...
     */
    function isObjectiveFulfilled() internal returns (bool) {
     ...
     * @param _rate (value e.g. 0.5 DAI/ETHIX == 500000000000000000)
     * @return (amount)
     **/
    function calculateCollateralizationGoal(uint256 _collateralizationPercent, uint256 _referenceGoal, uint256 _rate) internal pure returns (uint256) {
        return uint256(_referenceGoal.mul(_collateralizationPercent).mul(100000000000000).div(_rate));
    }
