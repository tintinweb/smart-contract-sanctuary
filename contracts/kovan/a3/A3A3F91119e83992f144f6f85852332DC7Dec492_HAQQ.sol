...
contract HAQQ is ERC20, Ownable {
    ...
    */
    event TierUpgrade(uint indexed from, uint indexed to);
    ...
    */
    function upgradeTier() public onlyOwner returns (bool success) {
        ...
        emit TierUpgrade(_from, currentTier);
        return true;
    }
    ...
    */
    ...
    /*
    * issues new tokens in exchange to accepted stable coins
    */
    function exchangeTokens(uint _amountInAcceptedCoins, address _tokenAddress, uint _tier) external returns (bool success){
        ...
        uint8 ourDecimals = decimals();

        uint amountToMint;
        if (acceptedCoinDecimals == ourDecimals) {
            amountToMint = _amountInAcceptedCoins;
        } else if (acceptedCoinDecimals > ourDecimals) {
            amountToMint = _amountInAcceptedCoins / (10 ** (acceptedCoinDecimals - ourDecimals));
        } else {
            amountToMint = _amountInAcceptedCoins * (10 ** (ourDecimals - acceptedCoinDecimals));
        }
