...
contract Bingo is ERC721, VRFConsumerBase, KeeperCompatibleInterface {
    ...
    */
    ...
    function checkForWinners() internal returns (uint256 winIdx) {
        ...
        if(winIdx > 0) {
            for(uint256 k=0; k<winIdx; k++) {
                winners[_winners[k]] += prizePool / winIdx;
                winnerPool += prizePool / winIdx;
                emit BingoWinner(_gameIds.current(), ownerOf(_winners[k]), _winners[k]);
            }
            // Reset game
            prizePool = 0;
            _gameIds.increment();
