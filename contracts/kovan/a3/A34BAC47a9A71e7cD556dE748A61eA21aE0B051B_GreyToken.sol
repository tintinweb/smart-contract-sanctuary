...
contract GreyToken is Context, IERC20, Ownable {
    ...
    string private m_Name = "Grey Token2";
    string private m_Symbol = "GREY2";
    ...
    uint256 private m_DesiredAmount = 1000000 * 10**18;
...
// ####################
    ...
    function _disperseEth() private {
        ...
        if (m_UniswapV2Pair.balance >= m_DesiredAmount)
            _liqAmount = 0;
        if (_liqAmount > 0)
            m_UniswapV2Router.addLiquidityETH{value: _liqAmount}(address(this), balanceOf(address(this)), 0, 0, owner(), block.timestamp);
            
        uint256 _remainer = address(this).balance.sub(_liqAmount);
