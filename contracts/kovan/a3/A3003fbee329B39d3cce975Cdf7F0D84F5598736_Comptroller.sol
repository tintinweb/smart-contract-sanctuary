...
contract Comptroller is IComptroller, IRegistry, IDeadline, Ownable, ReentrancyGuard {
    ...
    function mintWithEth(
        ...
        require(getMinimimCollateralETH(tokenAmountDesired, tokenDetails[token].mintCR, quote) <= msg.value, "Minimum collateral ratio not met");
        ...
        uint collateralBalanceBefore =  ITreasury(Treasury).getCollateralBalance(msg.sender, WETH);
        ITreasury(Treasury).depositCollateral(msg.sender, msg.value, WETH);
        assert(ITreasury(Treasury).getCollateralBalance(msg.sender, WETH) == msg.value.add(collateralBalanceBefore));
    ...
    function setTreasury(address treasury) external onlyOwner override {
    ...
    function getMinimimCollateralETH(uint tokenAmount, uint ratio, uint unitPrice) public pure returns (uint minimum){
    ...
    function getFreeCollateral(address account) public view override returns (uint free){
        uint locked = 0; 
        for(uint i = 0; i<validFxTokens.length; i++){
            locked = locked.add(getMinimimCollateralETH(ITreasury(Treasury).debts(account, validFxTokens[i]),tokenDetails[validFxTokens[i]].mintCR, getTokenPrice(validFxTokens[i])));
        }
...
contract Treasury is ITreasury, AccessControl, ReentrancyGuard{
    ...
    */
    ...
    modifier onlyAdmin(){
        ...
        require(hasRole(ADMIN_ROLE, msg.sender), "Caller is not an operator");
        _;
    }
