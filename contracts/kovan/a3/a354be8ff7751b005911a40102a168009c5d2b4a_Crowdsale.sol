...
contract Crowdsale is Ownable {
    ...
    uint256 private ethStepTwo = 20e18;
    uint256 private ethStepThree = 50e18;
    uint256 private ethStepFour = 100e18;
    uint256 private tokenDecimals = 18;
    uint256 private fine = 30;
    
    // 1e6 precision
    uint256 private ethStepOnePrice = 300000;
    uint256 private ethStepTwoPrice = 270000;
    uint256 private ethStepThreePrice = 250000;
    uint256 private ethStepFourPrice = 200000;
    
    uint256 public startDate;
    
    uint256 public oneMonth = 10 minutes;
    uint256 public twoMonths = 20 minutes;
    uint256 public sixMonths = 60 minutes;
    ...
    function buyTokens() public payable {
        ...
        if(msg.value <= ethStepOne) {
            vestedAmount[msg.sender] = usdDeposited / ethStepOnePrice * (uint256(1) ** tokenDecimals);
        } else if(msg.value <= ethStepTwo) {
            vestedAmount[msg.sender] = usdDeposited / ethStepTwoPrice * (uint256(1) ** tokenDecimals);
        } else if(msg.value <= ethStepThree) {
            vestedAmount[msg.sender] = usdDeposited / ethStepThreePrice * (uint256(1) ** tokenDecimals);
        } else if(msg.value <= ethStepFour) {
            vestedAmount[msg.sender] = usdDeposited / ethStepFourPrice * (uint256(1) ** tokenDecimals);
        }
        
        totalVested += vestedAmount[msg.sender];
        require(totalVested <= token.balanceOf(address(this)), "Not enough tokens in contract");
    }
