...
contract VanillaSuSim is ERC20, Ownable {
    
    constructor()  ERC20("VanillaSuSim", "SuSim", 18) public 
    {
...
contract MasterChef is Ownable {
    ...
    function pendingSushi(address _user)
        external
        view
        returns (uint256)
    {
        UserInfo storage user = userInfo[_user];
        uint256 accSushiPerShareTemp = accSushiPerShare;
        uint256 lpSupply = lpToken.balanceOf(address(this));
        if (block.number > lastRewardBlock && lpSupply != 0) {
            uint256 multiplier =
                getMultiplier(lastRewardBlock, block.number);
            uint256 sushiReward =
                multiplier.mul(sushiPerBlock);
            accSushiPerShareTemp = accSushiPerShareTemp.add(
                sushiReward.mul(1e12).div(lpSupply)
            );
        }
        return user.amount.mul(accSushiPerShareTemp).div(1e12).sub(user.rewardDebt);
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(accSushiPerShare).div(1e12).sub(
                    user.rewardDebt
                );
