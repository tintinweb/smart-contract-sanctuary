...
interface ISaft {
    ...
    struct PurchaserInfo {
        ...
        uint256 pricePerToken;          // (2) Price, per one suDAO token
                                        ...
                                        //     calc as totalTokens * pricePerToken / 1e18
        IERC20 paymentMethod;           // (4) Payment method, for example USDT
        uint256 paymentDeadline;        // (5) Payment date.
                                        // The date of this agreement (or any other date as agreed by the parties)
        uint256 fullVestingTimestamp;   // (6) 12 months vesting period with 3 months cliff.
        uint256 cliffTimestamp;         // See https://www.unixtimestamp.com/
...
contract Saft is ISaft, Ownable {
    ...
    function purchase(address purchaser, uint256 payAmount) external override {
        ...
        uint256 purchaseAmount = payAmount * p.pricePerToken / 1e18;
        require(p._tokensBought + purchaseAmount <= p.maximumTokenAllocation,
            "exceeded the purchase limit");
        // pay
        p.paymentMethod.safeTransferFrom(address(msg.sender), address(this), payAmount);
    ...
    function tokenVested(address purchaser) public view returns (uint256)  {
        ...
        uint256 linearVestingLength = p.fullVestingTimestamp - p.cliffTimestamp;
        uint256 linearVestingPassed = p.fullVestingTimestamp - block.timestamp;
        return p._tokensBought * linearVestingPassed / linearVestingLength;
    }

    function availableToClaim(address purchaser) external view override returns (uint256)  {
    ...
    function claimTokens() external override {
        ...
        require(p._tokensBought > 0,
            "nothing to claim");
        require(block.timestamp < p.cliffTimestamp,
            "cannot claim tokens before cliff is over");
        // TODO: gas optimize
        uint256 claimAmount = tokenVested(msg.sender) - p._tokensClaimed;
        p._tokensClaimed = p._tokensClaimed + claimAmount;
        suDAO.safeTransferFrom(address(this), msg.sender, claimAmount);
    }
    ...
    function addPurchaser (
        ...
        PurchaserInfo memory p = PurchaserInfo({
            ...
            fullVestingTimestamp: block.timestamp + fullVesting,
