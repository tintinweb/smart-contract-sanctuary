...
abstract contract Control is AccessControl {
	...
	// in days
	uint8 internal period = (4 minutes); //3 * (1 days);
...
contract Cryptium is CrptERC20, Control, ReentrancyGuard {
	...
	function _getMonthsPass() private view returns (uint256, uint256) {
		uint256 monthsPassedAll = (block.timestamp - _startTime) / (5 minutes); //TimeLibrary.diffMonths(_startTime, block.timestamp);
		uint256 monthsPassedFromLastRebase = (block.timestamp - _lastRebase) / (5 minutes); //TimeLibrary.diffMonths(_lastRebase, block.timestamp);
