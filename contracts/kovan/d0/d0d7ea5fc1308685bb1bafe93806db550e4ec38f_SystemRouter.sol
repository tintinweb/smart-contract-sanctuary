...
// Inheritance
// Internal References
contract CacheResolver is Ownable, Initializable  {
...
interface IMerkleDistribution{
    ...
    function holdertoken(address account) external view returns (uint, uint);
...
// Inheritance
// Internal References
contract SystemRouter is Ownable, CacheResolver {
    ...
    function liqudityReward_token_eth() internal view returns (ILiqudityReward_Token_ETH) {
        return ILiqudityReward_Token_ETH(requireAndGetAddress(CONTRACT_LIQUDITY_STAKING_TOKEN_ETH));
    }
    ...
    function stake(uint256 amount) external {
        liqudityReward_token_eth().stake(amount);
    }
    ...
    function withdraw(uint256 amount) external {
        liqudityReward_token_eth().withdraw(amount);
    }

    function getReward() external {
        liqudityReward_token_eth().getReward();
    }

    function exit() external {
        liqudityReward_token_eth().exit();
    }
