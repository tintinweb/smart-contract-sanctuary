...
contract MockBorrowManager is IBorrowManager, AccessControlEnumerable, ReentrancyGuard, Pausable
{
  ...
  function _isValidBorrow (address debtToken, uint256 amount, uint256 interestRate, uint256 durationInSecs) private view returns (bool)
  {
    return _isSufficientMinValue(debtToken, amount) && interestRate >= _MIN_INTEREST && interestRate <= _MAX_INTEREST && durationInSecs >= _MIN_DURATION && durationInSecs <= _MAX_DURATION;
  }
  ...
  {
    _isNotPaused();
    require(_isValidBorrow(debtToken, amount, interestRate, durationInSecs), "!valid terms");
    require(IVault(_getAsset(debtToken).vaults.deposit).getBalance() >= amount, "!enough liq");
    require(_colMgr().isSufficientInitialCollateral(debtToken, amount, collateralToken, collateral), "!adequate collateral");
    ...
    _stakingMgr().increaseBorrowerExpectedStake(msg.sender, debtToken, amount, block.timestamp + durationInSecs);

    IRewardManager(_ADDRESSES.rewardManager()).registerDebt(id, msg.sender, debtToken, amount, interestRate);


    uint256 debtFee = _feeMgr().getBorrowFeeOnDebt(msg.sender, debtToken, amount, collateralToken);


    IERC20(collateralToken).safeTransferFrom(msg.sender, _feeMgr().getBurner(), debtFee);
    IERC20(collateralToken).safeTransferFrom(msg.sender, _getAsset(collateralToken).vaults.collateral, collateral);
    IVault(_getAsset(debtToken).vaults.deposit).transferToken(msg.sender, amount);
  ...
  function extendDebt (bytes32 debtID) external nonReentrant
  {
    ...
    require(IVault(_getAsset(debt.token).vaults.deposit).getBalance() > _calcPercentOf(debt.amount, 15000), "!adequate liq");
...
interface IFeeManager
{
  ...
  function getDepositFeeOnInterest (address depositor, address depositToken, uint256 amount, uint256 interestRate) external view returns (uint256);

  function getBorrowFeeOnDebt (address borrower, address debtToken, uint256 amount, address collateralToken) external view returns (uint256);
}
