...
contract BuryShib is ERC20("BuryShib", "xSHIB"){
	...
	require(((_amount / (1000000 * 10**18)) > 0) && ((_amount % (1000000 * 10**18) ) == 0), "Shib must be stake in millions");
        ...
        user.rewardBoneDebt = user.amount.mul(poolInfo[0].accRewardPerShare).div(1e18);
        user.rewardEthDebt = user.amount.mul(poolInfo[1].accRewardPerShare).div(1e18);
    }
    ...
    function sendRewards(uint256 _pid) internal {
        ...
        uint256 what;
        if(_pid == 0) {
            what = user.amount.mul(pool.accRewardPerShare).div(1e18).sub(user.rewardBoneDebt);
	        pool.token.transfer(msg.sender, what);
	        pool.lastRewardBalance = pool.token.balanceOf(address(this));
	}
        else {
            what = user.amount.mul(pool.accRewardPerShare).div(1e18).sub(user.rewardEthDebt);
