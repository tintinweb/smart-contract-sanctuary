...
/**
 * @title Stake NFT Auction Contract
 *
 *
 * @dev Contract that manages interactions between NFT owners, patrons and owners.
 */
contract StakeAuction is VersionedInit, AuctionBase, ReentrancyGuard {
    using SafeERC20 for IERC20Permit;
    using SafeERC20 for IStakedAave;
    using SafeMath for uint256;

    uint256 public constant STAKEAUCTION_REVISION = 0x1;
    IERC20Permit public constant AAVE = IERC20Permit(0xB597cd8D3217ea6477232F9217fa70837ff667Af);
    IStakedAave public constant STKAAVE = IStakedAave(0xf2fbf9A6710AfDa1c4AaB2E922DE9D69E0C97fd2);

    mapping(address => mapping(uint256 => DataTypes.StakeAuctionFullData)) internal _nftData;
     ...
     */
    event BidSubmitted(address indexed nft, uint256 indexed nftId, address indexed bidder, uint256 amount);
     ...
     */
    event Redeemed(address indexed nft, uint256 indexed nftId);
    ...
    function initialize(
        ...
        _paused = false;
        AAVE.approve(address(STKAAVE), type(uint256).max);
    }
    ...
    function createAuction(
        ...
        DataTypes.StakeAuctionFullData storage nftData = _nftData[nft][nftId];
        ...
        DataTypes.StakeAuctionData memory auctionData = DataTypes.StakeAuctionData(
            startPrice,
            address(0),
            startTimestamp,
            endTimestamp
    ...
    function claimWonNFT(address nft, uint256 nftId) external nonReentrant whenNotPaused {
        DataTypes.StakeAuctionData storage auction = _nftData[nft][nftId].auction;
        ...
        STKAAVE.stake(clone, currentBid);
    ...
    function redeem(address nft, uint256 nftId) external nonReentrant whenNotPaused {
        ...
        delete(_nftData[nft][nftId]);
        IERC721(nft).safeTransferFrom(msg.sender, auctioner, nftId);
     ...
     *
     * @return The StakeAuctionFullData containing all data related to a given NFT.
     */
    function getNftData(address nft, uint256 nftId)
        external
        view
        returns (DataTypes.StakeAuctionFullData memory)
    {
