...
contract AlphaImpactFarm is Ownable, ReentrancyGuard, TokenList {
    ...
    function depositDataV2(
        ...
        if(hasDeposit){
            UserDeposits storage userDeposits = _deposits[user];
            uint256 depositId = userDeposits.ids[0];
            params = _deposits[_nonZeroAddr(user)].data[depositId];
        }else{
            params = Deposit(
                0,
    ...
    function idsToBitmask(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            uint256 id = ids[i];
            require(id != 0 && id <= 64, "AIFarms:unsupported NFT ID");
    ...
    function deposit(
        ...
        if (tS.nfTokenId == 0) {
            ...
            require(
                ...
                "AIFarms:disallowed NFT instance"
            );
        }
    ...
    function _withdraw(uint256 depositId, bool isInterim) internal {
        ...
        if (isInterim) {
            ...
            require(
                ...
                "AIFarms:withdrawal not yet allowed"
            );
    ...
    function _computeEarlyWithdrawal(
        ...
        require(d.lockedShare != 65535, "AIFarms:early withdrawals banned");
...
contract TokenList {
    ...
    address private constant AlphaImpactAddr = 0x6d7a1E1A22255b5151ED029e778Ccd172aeEfaB1;
