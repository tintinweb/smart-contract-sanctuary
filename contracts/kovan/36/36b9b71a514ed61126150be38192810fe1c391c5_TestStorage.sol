...
// File: contracts/mock/TestStorage.sol

pragma solidity ^0.6.12;


contract TestStorage is Ownable{
    mapping(address => uint8) internal tokenIndexes;
    ...
    address public addr = 0x9522368481c84250fD4b2A4ea03FB875024D9956;

    function setTokenIndex(address token, uint8 index) external onlyOwner{
    ...
    function addArray(address item) external onlyOwner{
        array.push(item);
    }
    ...
    function getArray() view public returns  (address[] memory){
        return array;
    }

    function getTest() view public returns(address){
        return test;
    }
