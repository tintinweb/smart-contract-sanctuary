...
contract AdvancedCollectible is ERC721Enumerable, Ownable, ChainlinkClient {
    ...
    function buildToken(string memory geohash) 
        external payable {
            ...
            checkID(geohash);
            if(flag == 1){
                _safeMint(send, supply);
                setTokenHash(supply, geohash);
                flag=0;
            } else {
                flag=0;
            }
   ...
   function checkID(string memory geohash) internal returns (bytes32 requestId) 
    {
