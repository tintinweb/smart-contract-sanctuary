...
contract TreasureFinder is BoringOwnable {
    ...
    function buryTokens(uint amountBONE) internal {
        ...
        uint amountBONEForSHIB = amountBONE / 3;
        uint amountBONEForLEASH = amountBONE / 3;

        uint amountBONEtoBury = amountBONE.sub(amountBONEForSHIB).sub(amountBONEForLEASH);
        uint amountSHIBtoBury = _swap(bone, shib, amountBONEForSHIB, address(this));
        uint amountLEASHtoBury = _swap(bone, leash, amountBONEForLEASH, address(this));

        IERC20(bone).safeTransfer(buryBone, amountBONEtoBury);
        IERC20(shib).safeTransfer(buryShib, amountSHIBtoBury);
        IERC20(leash).safeTransfer(buryLeash, amountLEASHtoBury);
    }
