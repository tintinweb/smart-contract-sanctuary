...
library UniswapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                //hex'af866c057d8c6cfbaab6e2bcb23fd85fe625c5ca98d0315f014a66210ac2f685' // init code hash for tests
            ))));
    }
...
contract JFeesCollector is OwnableUpgradeSafe, JFeesCollectorStorage, IJFeesCollector {
    ...
    */
    function swapTokenForEth(address _token, uint _amount) public {
        ...
        require(IERC20(_token).approve(address(uniV2Router02), _amount), 'approve failed.');
        // amountOutMin must be retrieved from an oracle of some kind
        //uint amountOutMin = getUniswapPrice(_amount);
        address[] memory path = new address[](2);
        path[0] = _token;
    ...
    */
    function swapTokenForToken(address _tokenSent, address _tokenBack, uint _amount) public {
        //require(IERC20(_tokenSent).transferFrom(msg.sender, address(this), _amount), 'transferFrom failed.');
        require(IERC20(_tokenSent).approve(address(uniV2Router02), _amount), 'approve failed.');
        // amountOutMin must be retrieved from an oracle of some kind
        //uint amountOutMin = getUniswapPrice(_amount);
        address[] memory path = new address[](2);
        path[0] = _tokenSent;
