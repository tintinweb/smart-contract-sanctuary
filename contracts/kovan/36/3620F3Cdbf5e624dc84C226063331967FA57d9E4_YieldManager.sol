    ...
    IYieldManager
{
    ...
    function rebalance() external override onlyController {
        closeExcessIntegrationPositions();
        deploy();
    }
    ...
    function deploy() public override onlyController {
        ...
        for(uint256 tokenId; tokenId < tokenCount; tokenId++) {
            ...
            uint256 tokenDesiredReserve = getDesiredReserveTokenBalance(address(token));
            uint256 tokenActualReserve = getReserveTokenBalance(address(token));
            uint256 tokenTotalBalance = IUserPositions(moduleMap.getModuleAddress(Modules.UserPositions)).totalTokenBalance(address(token));

            if(tokenActualReserve > tokenDesiredReserve) {
                uint256 integrationWeightSum = integrationMap.getTokenIntegrationWeightSum(address(token));

                if(integrationWeightSum > 0) {
                    for(uint256 integrationId; integrationId < integrationCount; integrationId++) {
                        ...
                        uint256 tokenIntegrationWeight = integrationMap.getTokenIntegrationWeight(integrationAddress, address(token));

                        uint256 integrationDesiredBalance = (tokenTotalBalance - tokenDesiredReserve) 
                            * tokenIntegrationWeight / integrationWeightSum;
                        uint256 integrationActualBalance = IIntegration(integrationAddress).getBalance(address(token));

                        if(integrationDesiredBalance > integrationActualBalance) {
                            uint256 tokenIntegrationDeployAmount = integrationDesiredBalance - integrationActualBalance;

                            if(token.balanceOf(moduleMap.getModuleAddress(Modules.Kernel)) >= tokenIntegrationDeployAmount) {
                                uint256 balanceBefore = token.balanceOf(integrationAddress);
                                token.safeTransferFrom(moduleMap.getModuleAddress(Modules.Kernel), integrationAddress, 
                                    tokenIntegrationDeployAmount);
                                uint256 balanceAfter = token.balanceOf(integrationAddress);
                                IIntegration(integrationAddress).deposit(address(token), balanceAfter - balanceBefore);
                            }
                        }
                    }
                }
        ...
        for(uint256 integrationId; integrationId < integrationCount; integrationId++) {
            IIntegration(integrationMap.getIntegrationAddress(integrationId)).deploy();
        }
    }

    /// @notice Partially closes any token/integration positions according to weights
    function closeExcessIntegrationPositions() private {
        IIntegrationMap integrationMap = IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap));
        uint256 tokenCount = integrationMap.getTokenAddressesLength();
        uint256 integrationCount = integrationMap.getIntegrationAddressesLength();
        ...
        for(uint256 tokenId; tokenId < tokenCount; tokenId++) {
            ...
            uint256 tokenDesiredReserve = getDesiredReserveTokenBalance(tokenAddress);
            uint256 tokenTotalBalance = IUserPositions(moduleMap.getModuleAddress(Modules.UserPositions)).totalTokenBalance(tokenAddress);
            uint256 integrationWeightSum = integrationMap.getTokenIntegrationWeightSum(tokenAddress);

            for(uint256 integrationId; integrationId < integrationCount; integrationId++) {
                address integrationAddress = integrationMap.getIntegrationAddress(integrationId);
                uint256 tokenIntegrationWeight = integrationMap.getTokenIntegrationWeight(integrationAddress, tokenAddress);

                uint256 integrationDesiredBalance;
                if(integrationWeightSum > 0) {
                    integrationDesiredBalance = (tokenTotalBalance - tokenDesiredReserve) 
                        * tokenIntegrationWeight / integrationWeightSum;
                } else {
                    integrationDesiredBalance = 0;
                }

                uint256 integrationActualBalance = IIntegration(integrationAddress).getBalance(tokenAddress);

                if(integrationDesiredBalance < integrationActualBalance) {
                    IIntegration(integrationAddress).withdraw(tokenAddress, integrationActualBalance - integrationDesiredBalance);
                }
            }
        }
    }
    ...
    function harvestYield() public override onlyController {
        ...
        for(uint256 integrationId; integrationId < integrationCount; integrationId++) {
            IIntegration(integrationMap.getIntegrationAddress(integrationId)).harvestYield();
        }
        ...
        for(uint256 tokenId; tokenId < tokenCount; tokenId++) {
            ...
            if(tokenDesiredReserve > tokenActualReserve) {
                ...
                if(tokenDesiredReserve - tokenActualReserve <= harvestedTokenAmount) {
                    ...
                    token.safeTransfer(moduleMap.getModuleAddress(Modules.Kernel), tokenDesiredReserve - tokenActualReserve);
                } else {
                    // Need to send all of harvested token to Kernel to partially replenish reserves
                    token.safeTransfer(moduleMap.getModuleAddress(Modules.Kernel), token.balanceOf(address(this)));
                }
            }
        }
    }
        ...
        for(uint256 tokenId; tokenId < tokenCount; tokenId++) {       
            ...
            if(token.balanceOf(address(this)) > 0) {
                ...
                if(address(token) != address(weth)) {
                    ...
                    token.safeTransfer(moduleMap.getModuleAddress(Modules.UniswapTrader), token.balanceOf(address(this)));

                    IUniswapTrader(moduleMap.getModuleAddress(Modules.UniswapTrader)).swapExactInput(
                        address(token),
                        address(weth),
                        address(this),
                    ...
                    wethReceived = weth.balanceOf(address(this)) - getProcessedWethByTokenSum();
                }
    ...
    function distributeEth() external override onlyController { 
        ...
        uint256 wethToDistribute = IERC20MetadataUpgradeable(wethAddress).balanceOf(address(this));

        if(wethToDistribute > 0) {
            uint256 biosBuyBackWethAmount = wethToDistribute * biosBuyBackEthWeight / getEthWeightSum();
            uint256 treasuryWethAmount = wethToDistribute  * treasuryEthWeight / getEthWeightSum();
            uint256 protocolFeeWethAmount = wethToDistribute * protocolFeeEthWeight / getEthWeightSum();
            uint256 rewardsWethAmount = wethToDistribute - biosBuyBackWethAmount - treasuryWethAmount - protocolFeeWethAmount;

            // Send WETH to SushiSwap trader for BIOS buy back
            IERC20MetadataUpgradeable(wethAddress).safeTransfer(moduleMap.getModuleAddress(Modules.SushiSwapTrader), biosBuyBackWethAmount);    
    ...
    function biosBuyBack() external override onlyController {  
        if(IERC20MetadataUpgradeable(IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap))
            .getWethTokenAddress()).balanceOf(moduleMap.getModuleAddress(Modules.SushiSwapTrader)) > 0) { 
                ...
                IUserPositions(moduleMap.getModuleAddress(Modules.UserPositions)).increaseBiosRewards();
        }
    }
    ...
    function ethToRewards(uint256 ethRewardsAmount) private {
        ...
        require(processedWethByTokenSum > 0, "YieldManager::ethToRewards: No processed WETH to distribute");
        ...
        for(uint256 tokenId; tokenId < tokenCount; tokenId++) {
            ...
            if(processedWethByToken[tokenAddress] > 0) {
                IEtherRewards(moduleMap.getModuleAddress(Modules.EtherRewards))
                    .increaseEthRewards(tokenAddress, ethRewardsAmount * processedWethByToken[tokenAddress] / processedWethByTokenSum);
        ...
        payable(moduleMap.getModuleAddress(Modules.Kernel)).transfer(ethRewardsAmount);
    }
    ...
    function ethToProtocolFeeAccrual(uint256 protocolFeeEthRewardsAmount) private {
        IIntegrationMap integrationMap = IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap));
        address biosAddress = integrationMap.getBiosTokenAddress();
        address wethAddress = integrationMap.getWethTokenAddress();

        if(IUserPositions(moduleMap.getModuleAddress(Modules.UserPositions)).totalTokenBalance(biosAddress) > 0) {
            ...
            payable(moduleMap.getModuleAddress(Modules.Kernel)).transfer(protocolFeeEthRewardsAmount);
        } else {
            // No BIOS has been deposited, send WETH back to Kernel as reserves
            IERC20MetadataUpgradeable(wethAddress).transfer(moduleMap.getModuleAddress(Modules.Kernel), protocolFeeEthRewardsAmount);
        }
    }
    ...
    function getHarvestedTokenBalance(address tokenAddress) external view override returns (uint256 harvestedTokenBalance) {
        if(tokenAddress == IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap)).getWethTokenAddress()) {
            harvestedTokenBalance = IERC20MetadataUpgradeable(tokenAddress).balanceOf(address(this)) - getProcessedWethSum();
        } else {
            harvestedTokenBalance = IERC20MetadataUpgradeable(tokenAddress).balanceOf(address(this));
        }
    }
    ...
    function getReserveTokenBalance(address tokenAddress) public view override returns (uint256) {
        require(IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap)).getIsTokenAdded(tokenAddress), "YieldManager::getReserveTokenBalance: Token not added");
    ...
    function getDesiredReserveTokenBalance(address tokenAddress) public view override returns (uint256) {
        require(IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap)).getIsTokenAdded(tokenAddress), "YieldManager::getDesiredReserveTokenBalance: Token not added");
        uint256 tokenReserveRatioNumerator = IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap)).getTokenReserveRatioNumerator(tokenAddress);
        uint256 tokenTotalBalance = IUserPositions(moduleMap.getModuleAddress(Modules.UserPositions)).totalTokenBalance(tokenAddress);
        return tokenTotalBalance * tokenReserveRatioNumerator / IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap)).getReserveRatioDenominator();
    }
    ...
    function getProcessedWethSum() public view override returns (uint256 processedWethSum) {
        uint256 tokenCount = IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap)).getTokenAddressesLength();
        
        for(uint256 tokenId; tokenId < tokenCount; tokenId++) {       
            address tokenAddress = IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap)).getTokenAddress(tokenId);
    ...
    function getProcessedWethByToken(address tokenAddress) public view override returns (uint256) {
        return processedWethByToken[tokenAddress];
    }

    /// @return processedWethByTokenSum The sum of processed WETH
    function getProcessedWethByTokenSum() public view override returns (uint256 processedWethByTokenSum) {
        IIntegrationMap integrationMap = IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap));
        uint256 tokenCount = integrationMap.getTokenAddressesLength();

        for(uint256 tokenId; tokenId < tokenCount; tokenId++) {
            processedWethByTokenSum += processedWethByToken[integrationMap.getTokenAddress(tokenId)];
        }
    }

    /// @param tokenAddress The address of the token ERC20 contract
    /// @return tokenTotalIntegrationBalance The total amount of the token that can be withdrawn from integrations
    function getTokenTotalIntegrationBalance(address tokenAddress) public view override returns (uint256 tokenTotalIntegrationBalance) {
        IIntegrationMap integrationMap = IIntegrationMap(moduleMap.getModuleAddress(Modules.IntegrationMap));
        uint256 integrationCount = integrationMap.getIntegrationAddressesLength();
    ...
    function getEthDistributionWeights() public view override returns (uint32, uint32, uint32, uint32) {
