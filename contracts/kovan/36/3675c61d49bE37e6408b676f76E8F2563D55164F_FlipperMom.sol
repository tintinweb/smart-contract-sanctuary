...
// hevm: flattened sources of src/FlipperMom.sol
...
interface FlipLike_2 {
    function wards(address) external returns (uint);
    function rely(address) external;
    function deny(address) external;
}

interface AuthorityLike_2 {
...
contract FlipperMom {
    ...
    function isAuthorized(address src, bytes4 sig) internal view returns (bool) {
        if (src == address(this)) {
            ...
            return AuthorityLike_2(authority).canCall(src, address(this), sig);
        }
    }
