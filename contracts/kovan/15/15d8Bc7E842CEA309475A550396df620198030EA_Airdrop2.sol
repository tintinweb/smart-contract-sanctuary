...
contract Airdrop2 is Ownable {
    ...
    constructor (address _admin,address _mcnToken, address _usdcToken) {        
        mcnToken= _mcnToken;
        usdcToken= _usdcToken;
        transferOwnership(_admin);
    }

    function disperseMCNToken(address[] memory _recipients, uint256[] memory _amount) public returns (bool) {
        ...
        for (uint256 i = 0; i < _recipients.length; i++) {
            ...
            require(IERC20(mcnToken).transferFrom(msg.sender, _recipients[i], _amount[i]));
        }

        return true;
    }

     function disperseUSDCToken(address[] memory _recipients, uint256[] memory _amount) public returns (bool) {
                                
        require(_recipients.length == _amount.length, 'Invalid data');
                                            
        for (uint256 i = 0; i < _recipients.length; i++) {
            require(_recipients[i] != address(0), 'Not allowed transfer to address 0');            
            require(IERC20(usdcToken).transferFrom(msg.sender, _recipients[i], _amount[i]));
        }
