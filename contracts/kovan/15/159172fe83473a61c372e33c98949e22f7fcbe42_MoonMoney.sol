...
contract MoonMoney is TokenInterface {
    ...
    function _claimYields(address staker) internal returns (uint256 amt){
         ...
         uint amountEarned = getFarmYieldForVault(staker, amountStaked);
    ...
    function getFarmYieldForVault(address accountAddress, uint256 stakedAmount) public view returns (uint amt)
    {
        uint blockDelta = MinStakingTimeBlocks;
        
        uint amountEarned = blockDelta.mul(stakedAmount); 
