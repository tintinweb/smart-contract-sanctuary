...
contract IDOMaster is Ownable {
    ...
    function removeOtherERC20Tokens(address _tokenAddress, address _to) external onlyOwner {
     ...
     function getMaxEthPayment(address user, uint256 maxEthPayment)
        ...
        returns (uint256)
    {
        uint256 _disBalance = feeToken.balanceOf(msg.sender);
        (uint256 _stakedAmount, ) = disStakingPool.getUserInfo(msg.sender);
        _disBalance += _stakedAmount;
        (uint256 _lpBalance, ) = lpDisStakingPool.getUserInfo(msg.sender);
        if(_lpBalance > 0) {
...
import "./interfaces/IidoMaster.sol";
 ...
 contract IDOPool is Ownable, Pausable, ReentrancyGuard, Whitelist {
    using SafeMath for uint256;
    using SafeERC20 for ERC20;
    ...
    function purchaise() payable external nonReentrant{
