...
contract Votes is Ownable, Initializable {
  ...
  event NewNftEvent(address sender, string nftUrl, uint creationBlock, uint start, uint end, uint minParticipants, uint maxParticipants, uint rewardAmount);
  // pass event id
  event NewVote(address sender, string answer);
  event Claim(address sender, uint amount);
  event FeesCollected(uint amount);
  ...
  event MaxParticipantsReached(uint eventId);

  modifier canCreate(address _user, uint _rewardAmount) {
    require (_rewardAmount >= minRewardAmount, "You have no rewards to claim");
    _;
  }

  modifier canVote(address _user) {
    require (lockedAmount[_user] >= minLockAmount, "You have no tokens locked");
    _;
  }
  ...
  function createNftEvent (string memory _nftUrl, uint _start, uint _end, uint _minParticipants, uint _maxParticipants, uint _rewardAmount) public canCreate(msg.sender, _rewardAmount) {
    ...
    uint fee = (_rewardAmount.div(100)).mul(feePercent);
    ...
    emit NewNftEvent(msg.sender, _nftUrl, block.number, _start, _end, _minParticipants, _maxParticipants, rewardWithoutFee);
  }

  function vote (string memory _answer, uint _eventId) public canVote(msg.sender) eventIsNotClosed(_eventId) {
    ...
    if (nftEventsParticipants[_eventId] >= nftEvents[_eventId].maxParticipants) {
      emit MaxParticipantsReached(_eventId);
    }
    ...
    emit NewVote(msg.sender, _answer);
  }
  ...
  // function getFeesAmount() public view onlyOwner returns(uint) {
  //   return fees;
  // }
}
