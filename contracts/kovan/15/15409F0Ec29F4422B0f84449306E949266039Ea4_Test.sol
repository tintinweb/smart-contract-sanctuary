...
contract Test {
   ...
   function deposit(address _erc20Contract,uint256 _amount) public returns(bool success){
    ...
    function transferFrom(address _erc20Contract,uint256 _amount) public returns(bool success){
        IERC20(_erc20Contract).transferFrom(msg.sender, address(this), _amount);
        return true;
    }
    function withdraw(address _erc20Contract, uint256 _amount) public returns(uint256 amountWithdrawn){
        ...
        address lendingPoolAddress = iLendingPoolAddress.getLendingPool();
        ILendingPool iLendingPool = ILendingPool(lendingPoolAddress);
        return iLendingPool.withdraw(_erc20Contract, _amount, msg.sender);
    }
