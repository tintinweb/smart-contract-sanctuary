...
contract ERC721TokenImplementationWrapped is Ownable, IERC165, IERC721, IERC721Metadata, IERC721Enumerable, INftMintBurn, Initializable {
    ...
    function mintTo(address _recipient, uint256 _tokenId, string calldata _tokenUri) external virtual override returns (bool) {
        _safeMint(_recipient, _tokenId);
        _setTokenURI(_tokenId, _tokenUri);
        return true;
    }

    function burn(uint256 _tokenId) external virtual override returns (bool) {
        require(_isApprovedOrOwner(_msgSender(), _tokenId), "ERC721: transfer caller is not owner nor approved");
