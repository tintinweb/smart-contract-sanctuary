...
interface INFTInterface {
    function offerPunkForSaleToAddress(uint256 _NFTId, uint256 MinSalePriceInWei, address ToAddress ) external payable;
    function punkIndexToAddress(uint256 _NFTId) external view returns (address owner);
    function buyPunk(uint256 _NFTId) external payable;
}
...
contract Lottery is VRFConsumerBase {
    ...
    uint256 public TARGET = 23000000000000000000; // Target of the Lottery (23 eth)
    ...
    // Interface of the cryptopunk contract (in order to interact with it)
    INFTInterface private NFTCore = INFTInterface(0xb47e3cd837dDF8e4c57F05d70Ab865de6e193BBB); // Smart Contract of the CryptoPunks
    ...
    */
    ...
    // Allows the owner to deposit the prize in the contract. In this case, it allows to deposit a cryptopunk.
    function depositNFT(uint256 _NFTId) external payable
    onlyBy(owner)
    {
        require(NFTCore.punkIndexToAddress(_NFTId) == msg.sender, "Only NFT owner is allowed");
        NFTCore.buyPunk(_NFTId);
        require(NFTCore.punkIndexToAddress(_NFTId) == address(this), "NFT cannot be transferred");
        NFTIsDepositedInContract[_NFTId] = true;
        OwnerOfTheNFT[1] = owner;
        emit DepositNFT(_NFTId);    
    }

    /* Allows the contract to give the cryptopunk to the winner address.
        IMPORTANT: The winner can retrieve his prize through the cryptopunk contract!
    */
    function GiveNFT(uint256 _NFTId) external 
    onlyBy(owner)
    {
        require(WithdrawNFT == WITHDRAWNFT.OPEN, "You aren't at that stage yet!");
        require(NFTIsDepositedInContract[_NFTId], "NFT not found in contract"); 
        NFTCore.offerPunkForSaleToAddress(_NFTId, 0, addressToSendNFT);  
