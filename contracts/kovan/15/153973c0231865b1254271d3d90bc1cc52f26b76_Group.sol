...
contract Group {
    ...
    function closeRegistration(uint32 tournamentId) payable external {
        ...
        uint32 length = (((currentTournament.participantsCount-1)/2)*2) + 2;
        for(uint32 i; i < currentTournament.participantsCount; i++){
            uint32 oponentIndex = length - i - 1;
	        if(oponentIndex == 0){
		        currentTournament.pairings[i] = pairingData(0, 0);
	        }else{
		        currentTournament.pairings[i] =  pairingData((oponentIndex -1) % (length - 1) + 1, 0);
        ...
        if(currentTournament.openPairings == 0){
            ...
            for(uint32 i; i < currentTournament.participantsCount; i++){
	            ...
	            if(oponentIndex == 0){
		            ...
		            currentTournament.pairings[pairingOffset + i] =  pairingData((oponentIndex + currentTournament.round - 2) % (length - 1) + 1, 0);
	            }
