...
contract Token is TokenERC20, Control, Oracle, ReentrancyGuard {
	 ...
	 */
	function initializeContract(uint256 _startTime, uint32 _initialPrice) external onlyRole(DEFAULT_ADMIN_ROLE) {
	...
	function rebase() external initialized {
		...
		_rebase(supplyDelta);
		emit Rebase(currentSupply, _totalSupply, isInflRebase);
	}

	function writeData(uint256 _currentPrice, uint256 _averagePrice1, uint256 _averagePrice2, uint256 _timestamp) external {
		...
		)
	{
		...
		if (daysPassFromRebase == 0) {
			...
			price2 = (((coeffInflations * daysPassAll * 10000) / 365 + 100000000) * initialPrice) / 10000; // 8
		}
		if (price1 > price2 && price1 > curPrice) {
			supplyDelta = ((_totalSupply * oldPrice * 10000) / newPrice) / 10000;
		} else if (price2 > curPrice) {
			price2 = ((coeffInflations * daysPassFromRebase * 10000) / 365 + 100000000) * initialPrice;
	...
	/**
	 * @notice Months pass
	 * @return months pass after startTime or 0 if (> 12 months) or (the rebase was done less than a day ago
	 */
...
abstract contract Control is AccessControl {
	...
	/**
	 * @notice Set percentages which takes from any transfer to whitelist pools
