...
contract AirDrop {
  ...
  function getInfos(address user_) public view returns (uint256 reward_, uint256 total_, uint256 claim_) {
      reward_ = getReward(user_);
      total_ = amounts[user_];
      claim_ = receives[user_];
  }
  /*
  function getInfos(address user_) public view returns (uint256, uint256, uint256) {
      uint256 _reward = getReward(user_);
      uint256 _total = amounts[user_];
      uint256 _claim = receives[user_];
      return (_reward, _total, _claim);
  }*/
