...
// File: contracts/libs/CAFE20.sol
 ...
 */
contract CAFE20 is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;
...
// File: contracts/CafeToken.sol

/*
 * Stake-Cafe 
 * App:      https://stake.cafe
 * Medium:   https://medium.com/@stakecafe    
 * Twitter:  https://twitter.com/stakecafe 
 * Telegram: https://t.me/stakecafe
 * GitHub:   https://github.com/Stake-Cafe
 */
...
contract CafeToken is CAFE20('StakeCafe Token', 'CAFE'), Authorizable {
    uint256 private _cap = 20000000e18;
    uint256 private _totalLock;

    // Cafe Rewards Lockup system
    ...
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterCafe).
    ...
    function lock(address _holder, uint256 _amount) public onlyOwner {
        ...
        require(_amount <= balanceOf(_holder), "BEP20: lock amount over balance");
    ...
    /**
     * @notice Gets amount available to be unlocked
     * @param _holder The address to get unlock amount
     * @return The amount currently unlocked for `address`
     */
    ...
    /**
     * @notice Unlocks available amount to msg.sender
     */
    function unlock() public {
        require(_locks[msg.sender] > 0, "BEP20: cannot unlock");
    ...
    // @dev function to migrate all caller's balance to a multi sig address
...
// BeanRoast with Governance.
contract BeanRoast is CAFE20('BeanRoast Token', 'BEAN') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterCafe.
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    ...
    // Safe cake transfer function, just in case if rounding error causes pool to not have enough CAFEs.
...
// MasterCafe is the master of CAFE AND BEAN. 
// He can make Cafe and he is a fair guy.
//
...
contract MasterCafe is Ownable, Authorizable {
    using SafeMath for uint256;
    ...
    // The CAFE TOKEN!
    CafeToken public cake;
    // The SECRET CAFE TOKEN!
    BeanRoast public bean;
    ...
    address public philanthropyaddr;
    // CAFE tokens created per block.

    uint256 public cafePerBlock;
    // Bonus muliplier for early cafe makers. Divide by 1e8 for actual rate
    uint256[] public BONUS_MULTIPLIER = [701408733,433494437,267914296,165580141,102334155,63245986,39088169,24157817,14930352,9227465,5702887,3524578,2178309,1346269,832040,514229,317811,196418,121393,75025,46368,28657,17711,10946,6765,4181,2584,1597,987,610,377,233,144,89,55,34,21,13,8,5,3,2,1,1,0];
    // 44 fibonacci sequences
    uint256[] public HALVING_AT_BLOCK = [28800,201600,403200,1753164,2629746,4382910,7012656,11395566,18408222,29803788,48212010,78015798,126227808,204243606,330471414,534715020,865186434,1399901454,2265087888,3664989342,5930077230,9595066572,15525143802,25120210374,40645354176,65765564550,106410918726,172176483276,278587402002,450763885278,729351287280,1180115172558,1909466459838,3089581632396,4999048092234,8088629724630,13087677816864,21176307541494,34263985358358,55440292899852,89704278258210,145144571158062,234848849416272,379993420574334,614842269990606];
    uint256 public FINISH_BONUS_AT_BLOCK;
    uint256 public userDepFee;
    uint256 public devDepFee;
    uint256[] public blockDeltaStartStage = [25];
    ...
    uint256 public PERCENT_FOR_DEV; // development and expansion
    uint256 public PERCENT_FOR_VAULT; // vault management
    uint256 public PERCENT_FOR_COM; // community fund
    uint256 public PERCENT_FOR_PHI; // philanthropy fund

    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event SendCafeReward(address indexed user, uint256 indexed pid, uint256 amount, uint256 lockAmount);

    constructor(
        CafeToken _cafe,
        BeanRoast _beanRoast,
        address _devaddr,
        address _vaultaddr,
        address _comfundaddr,
        address _philanthropyaddr,
        ...
        philanthropyaddr = _philanthropyaddr;
        ...
        //update halving blocks to correct block numbers
        for (uint256 i = 0; i < BONUS_MULTIPLIER.length - 1; i++) {
            HALVING_AT_BLOCK[i] = HALVING_AT_BLOCK[i].add(_startBlock).add(_halvingAfterBlock);
        }
        FINISH_BONUS_AT_BLOCK = _halvingAfterBlock.add(HALVING_AT_BLOCK[HALVING_AT_BLOCK.length - 1]).add(_startBlock);
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, bool _withUpdate) public onlyOwner {
        ...
        require(poolId1[address(_lpToken)] == 0, "MasterCafe::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // Updates total allocation points
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        ...
        for(uint256 phi = 0; phi < HALVING_AT_BLOCK.length; phi++) {
            uint256 blockUpdate = HALVING_AT_BLOCK[phi];

            if(_to <= blockUpdate) {
                // return totalBonus.add(_to.sub(_from).mul(getCurrentMXX(phi)));
                return totalBonus.add(_to.sub(_from).mul(BONUS_MULTIPLIER[phi]));
            }

            if(_from < blockUpdate) {
                totalBonus = totalBonus.add(blockUpdate.sub(_from).mul(BONUS_MULTIPLIER[phi]));
                _from = blockUpdate;
            } 
        }
        return totalBonus;
    ...
    // divide by golden ratio
    // function divGold(uint256 mxx) internal returns (uint256) {
    //     return mxx.mul(2).div(1.add(sqrt(5)));
    // }
    // babylonian method (https://en.wikipedia.org/wiki/Methods_of_computing_square_roots#Babylonian_method)
    // function divGold(uint256 y) internal pure returns (uint256 z) {
    //         w = 5;
    //         uint256 x = 5 / 2 + 1;
    //         while (x < w) {
    //             w = x;
    //             x = (5 / x + x) / 2;
    //         }
    //         z = y.mul(2).div(1.add(w));
    // }

    // Get current reward multiplier
    // @param x number of recursions to divide by golden ratio
    // function getCurrentMXX(uint256 x) internal pure returns (uint256 y) {
    //     uint256 mxx = 7014087330;
    //     while(x > 0) {
    //         mxx = divGold(mxx);
    //         x--;
    //     }
    ...
    function getPoolReward(uint256 _from, uint256 _to, uint256 _allocPoint) public view returns (uint256 forDev, uint256 forFarmer, uint256 forVault, uint256 forCom, uint256 forPhi) {
        uint256 multiplier = getMultiplier(_from, _to);
        uint256 amount = multiplier.mul(cafePerBlock).mul(_allocPoint).div(totalAllocPoint);
        uint256 CafeCanMint = cake.cap().sub(cake.totalSupply());

        if (CafeCanMint < amount) {
            forDev = 0;
            forFarmer = CafeCanMint;
            forVault = 0;
            forCom = 0;
            forPhi = 0;
        }
        else {
            forDev = amount.mul(PERCENT_FOR_DEV).div(100).div(1e8);
            forFarmer = amount.mul(90).div(100);
            forVault = amount.mul(PERCENT_FOR_VAULT).div(100).div(1e8);
            forCom = amount.mul(PERCENT_FOR_COM).div(100).div(1e8);
            forPhi = amount.mul(PERCENT_FOR_PHI).div(100).div(1e8);
        }
    }
    ...
    function pendingCafe(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            uint256 CafeForFarmer;
            (, CafeForFarmer, , ,) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
            accCafePerShare = accCafePerShare.add(CafeForFarmer.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public validatePool(_pid) {
        ...
        uint256 CafeForDev;
        uint256 CafeForFarmer;
        uint256 CafeForVault;
        uint256 CafeForCom;
        uint256 CafeForPhi;
        (CafeForDev, CafeForFarmer, CafeForVault, CafeForCom, CafeForPhi) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
        cake.mint(address(bean), CafeForFarmer);
        pool.accCafePerShare = pool.accCafePerShare.add(CafeForFarmer.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;

        if (CafeForDev > 0) cake.mint(address(devaddr), CafeForDev);
        if (CafeForVault > 0) cake.mint(vaultaddr, CafeForVault);
        if (CafeForCom > 0) cake.mint(comfundaddr, CafeForCom);
        if (CafeForPhi > 0) cake.mint(philanthropyaddr, CafeForPhi);
    }


    // lock 85% of reward if it come from farming bounus
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            ...
            uint256 masterBal = cake.balanceOf(address(this));
            ...
            if(pending > 0) {
                ...
                if (user.rewardDebtAtBlock <= FINISH_BONUS_AT_BLOCK) {
                    ...
                    cake.lock(msg.sender, lockAmount);
                }

                user.rewardDebtAtBlock = block.number;

                emit SendCafeReward(msg.sender, _pid, pending, lockAmount);
            }
    ...
    // Deposit LP tokens to MasterCafe for CAFE allocation.
    function deposit(uint256 _pid, uint256 _amount, address _ref) public validatePool(_pid) {
        require(_amount > 0, "MasterCafe::deposit: amount must be greater than 0");
        require (_pid != 0, 'deposit CAFE by staking');
        ...
        devr.rewardDebt = devr.amount.mul(pool.accCafePerShare).div(1e12);
        emit Deposit(msg.sender, _pid, _amount);

        if(user.firstDepositBlock > 0){
        } else { user.firstDepositBlock = block.number; }
        user.lastDepositBlock = block.number;
    }

    // Withdraw LP tokens from MasterCafe.
    function withdraw(uint256 _pid, uint256 _amount, address _ref) public validatePool(_pid) {
        require (_pid != 0, 'withdraw CAFE by unstaking');
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        UserGlobalInfo storage refer = userGlobalInfo[_ref];
        UserGlobalInfo storage current = userGlobalInfo[msg.sender];
        require(user.amount >= _amount, "MasterCafe::withdraw: not good");
        if(_ref != address(0)){
                refer.referrals[msg.sender] = refer.referrals[msg.sender] - _amount;
                refer.globalRefAmount = refer.globalRefAmount - _amount;
            }
    ...
    // Withdraw without caring about rewards. EMERGENCY ONLY.. Includes 25% fee as same block withdrawals to prevent abuse of this function.
    function emergencyWithdraw(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        //reordered from Pancake function to prevent risk of reentrancy
    ...
    function safeCafeTransfer(address _to, uint256 _amount) internal {
        uint256 CafeBal = cake.balanceOf(address(this));
        if (_amount > CafeBal) {
            cake.transfer(_to, CafeBal);
        } else {
            cake.transfer(_to, _amount);
        }
    }
    ...
    returns(address lpToken, uint256 allocPoint, uint256 lastRewardBlock, uint256 accCafePerShare) {
        return (address(poolInfo[_pid].lpToken),
            poolInfo[_pid].allocPoint,
            poolInfo[_pid].lastRewardBlock,
            poolInfo[_pid].accCafePerShare);
    }
    ...
    // Update vaultaddr
    function vaultUpdate(address _newLP) public onlyAuthorized {
       vaultaddr = _newLP;
    }
    ...
    // Update philanthropyaddr
    function phiUpdate(address _newPhilanthropy) public onlyAuthorized {
       philanthropyaddr = _newPhilanthropy;
    }
    
    // Update Reward Per Block
    function rewardUpdate(uint256 _newReward) public onlyAuthorized {
       cafePerBlock = _newReward;
    }
    
    // Update Bonus Mulitplier Array
    function updateMultiplier(uint256[] memory _newMulBonus) public onlyAuthorized {
       BONUS_MULTIPLIER = _newMulBonus;
    }
    ...
    // Update % for dev
    function devPctUpdate(uint _newDevPct) public onlyAuthorized {
       PERCENT_FOR_DEV = _newDevPct;
    }
    
    // Update % for LP
    function vauPctltUpdate(uint _newVaultPct) public onlyAuthorized {
       PERCENT_FOR_VAULT = _newVaultPct;
    }
    
    // Update % for COM
    function comPctUpdate(uint _newComPct) public onlyAuthorized {
       PERCENT_FOR_COM = _newComPct;
    }
    
    // Update % for Philanthropy
    function phiPctUpdate(uint _newPhilanthropyPct) public onlyAuthorized {
       PERCENT_FOR_PHI = _newPhilanthropyPct;
    }
    ...
    function getNewRewardPerBlock(uint256 pid1) public view returns (uint256) {
        ...
        if (pid1 == 0) {
            return multiplier.mul(cafePerBlock).div(1e8);
        }
        else {
            return multiplier
                .mul(cafePerBlock)
                .mul(poolInfo[pid1 - 1].allocPoint)
                .div(totalAllocPoint).div(1e8);
        }
    }
