...
contract ReBakedDAO is IReBakedDAO, Ownable, ReentrancyGuard, Projects {
	...
	*/
		...
		returns(uint256 amount_)
	{
        require(!isDispute[collaborator_][packageId_]);
		uint temp;
		temp = packageData[projectId_][packageId_].bonus * collaboratorData[projectId_][packageId_][msg.sender].bonusScore * 10e17;
	...
	*/
		...
		returns(uint256 amount_)
	{
		...
		for (uint256 i = 0; i < packageIds_.length; i++) {
            require(!isDispute[collaborators_[i]][packageIds_[i]]);
			uint256 temp;
			temp = packageData[projectId_][packageIds_[i]].bonus * collaboratorData[projectId_][packageIds_[i]][msg.sender].bonusScore * 10e17;
