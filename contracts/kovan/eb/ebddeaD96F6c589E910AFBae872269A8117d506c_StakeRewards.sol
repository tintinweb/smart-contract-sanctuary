...
contract StakeRewards is PausableUpgradeable, BumperAccessControl {
    ...
    function restake(uint16 stakeIndex, uint16 option, bool withRewards, bool autorenew) external whenNotPaused updateIndex(option) 
    {
        require(option < stakeOptions.length, "!option");
        require(stakeIndex < usersStake[msg.sender].length, "!index");

        _updateIndex(usersStake[msg.sender][stakeIndex].option);
    ...
    function totalAmount() public view returns (uint amount, uint multipliersSum, uint weithedAmountSum) {
        for (uint16 i = 0; i < stakeOptions.length; i++) {
            if (stakeOptions[i].total == 0) continue; 

            amount += stakeOptions[i].total;
            multipliersSum += stakeOptions[i].multiplier;
    ...
    function _updateEmissions() internal {
        (uint total, uint multipliersSum, uint weithedAmountSum) = totalAmount();
        for (uint16 i = 0; i < stakeOptions.length; i++) {
            StakeOption storage option = stakeOptions[i];
            if (option.total > 0)
                option.emission = (totalEmissionPerSecond * option.multiplier / multipliersSum) * (option.total * option.multiplier) / weithedAmountSum;
            else 
                option.emission = 0;

            console.log("Emission option %s: %s, total %s", i, option.emission, option.total );
        }
    }
    ...
    modifier updateIndex(uint16 option) {
         _updateIndex(option);
        _;    
    }
