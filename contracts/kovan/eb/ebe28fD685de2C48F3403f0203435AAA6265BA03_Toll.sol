...
contract Toll is TollCore {
  ...
  function addWhitelistedUsers(
    ...
    require(hasInitialized, "addWhitelistedUsers: Initialization should be done before calling this method!");
  ...
  function claimTokens(
    ...
    // If burn tokens available handle burning logic
    if (burnableTokens != 0) {
      _handleBurning(burnableTokens);
    }
  ...
  function _getBurnAndTransferPercents(address user) private view returns (uint256, uint256) {
    ...
    // Flexible claim rate calculation
    if (fullDistributionSeconds != 0) {
  ...
  function _getNaturalAvailableTokens(address user) internal view returns (uint256) {
    ...
    /// @dev Max amount for missedReleases is 12, it saved on init() method
...
contract TollCore is Pausable, Ownable {
  ...
  uint256 public burnSavingsPercent;
  ...
  function init(
    ...
    require(!hasInitialized, "init: Can not be hasInitialized twice!");
  ...
  /// @dev Init burn savings and percent rom burned tokens will be transferred
  function enableBurnSavings(
    address _burnReceiver,
    uint256 _percent
  ) external onlyOwner {
    require(_burnReceiver != address(0), "enableBurnSavings: Invalid receiver address!");
    require(_percent <= HUNDRED_PERCENT, "enableBurnSavings: Percent can not be bigger then 100%!");

    isFarmEnabled = false;

    burnReceiver = _burnReceiver;
    burnSavingsPercent = _percent;
  }

  /// @dev Init farming, and percent rom burned tokens will be transferred
  function enableBurnFarming(
    ...
    require(_farm != address(0), "enableBurnFarming: Invalid farm address!");
    require(_farmManager != address(0), "enableBurnFarming: Invalid farm manager address!");
    require(_burnReceiver != address(0), "enableBurnFarming: Invalid burn receiver address!");
    require(_percent <= HUNDRED_PERCENT, "enableBurnFarming: Percent can not be bigger then 100%!");
  ...
  /// @dev Emergency exit the tokens of the provided investor.
  /// Receiver will get investor left tokens back.
  function emergencyExitSingle(address investor, address receiver) external onlyOwner {
    uint256 claimableAmount = _users[investor].maxTokens.sub(_users[investor].distributedTokens);

    _users[investor].distributedTokens = _users[investor].distributedTokens.add(claimableAmount);
    _users[investor].naturallyReceivedTokens = _users[investor].naturallyReceivedTokens.add(claimableAmount);

    token.safeTransfer(receiver, claimableAmount);
  }
  ...
  /// In general the releases count are limited by 12, to ensure the block gas limit issue
  ...
  /// @dev Distributes burned tokens based on conditions
  function _handleBurning(uint256 amount) internal {
    uint256 burnableTokens = amount;

    // 1) Calculate transferrable amounts, if the saving percent is bigger zero
    if (burnSavingsPercent > 0) {
      uint256 transferrable = _percentToAmount(amount, burnSavingsPercent);
      burnableTokens = amount.sub(transferrable);

      // Farm (if not paused) or transfer to savings address
      if (isFarmEnabled && !IFarmManager(farmManager).getPaused()) {
        _fundTokens(transferrable);
      } else {
        _transferTokens(burnReceiver, transferrable);
      }
    }

    // 2) Burn tokens, or send to burn valley contract
    if (isBurnableToken) {
  ...
  /// @dev Fund tokens to the farm contact
  function _fundTokens(uint256 amount) internal {
    // Approve tokens before funding
    token.approve(farm, amount);

    IFarmManager(farmManager).fund(farmId, amount);
  }
}
...
interface IFarmManager {
  function fund(uint256 fid, uint256 amount) external;
