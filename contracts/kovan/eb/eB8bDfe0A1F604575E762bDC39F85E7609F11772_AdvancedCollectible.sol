...
contract AdvancedCollectible is ERC721Enumerable, Ownable, ChainlinkClient {
    using Chainlink for Chainlink.Request;
    using Strings for uint256;
    
    uint256 public flag;
    string _baseTokenURI;
    string _baseProvableURI;
    bool public _tokenPaused = true;
    ...
    address t1 = 0x6e491554cAccD8D1DA0aE97e0e1A3DE3b36C141d;
    address t2 = 0x6F622A0f4A6F9d661deCcC01743888D76EC2AaD1;
     ...
     * Fee: 0.1 LINK
     */
    mapping(uint256 => Metadata) id_to_geohash;
    
    constructor(string memory baseURI, string memory baseProURI)
    ERC721("GeoToken", "GEOTOKEN")
    {   
        setBaseURI(baseURI);
        setBaseProURI(baseProURI);
    ...
    function requestToken(string memory geohash) 
        public payable returns(bytes32) {
            require( !_tokenPaused, "Token production is paused");
            require( msg.value >= _price, "Ether sent is not correct" );
            Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);
            string memory test = append(_baseProvableURI, geohash);
            request.add("get", test);
            request.add("path", "check");
    ...
    function get(uint256 tokenId) external view returns (string memory geohash) {
        require(_exists(tokenId), "token not minted");
        return id_to_geohash[tokenId].geohash;
    }
    
    function fulfill(bytes32 _requestId, uint256 _flag) public recordChainlinkFulfillment(_requestId)
    {
        flag = _flag;
        uint256 supply = totalSupply();
        if (flag==1){
            _safeMint(requestToSender[_requestId], supply);
        }
    }
       
    function append(string memory baseProvableURI, string memory geohash) internal pure returns (string memory) {
        return string(abi.encodePacked(baseProvableURI, geohash));
    }   

    function setTokenHash(uint256 tokenId, string memory geohash) public onlyOwner{
        require(_exists(tokenId), "token not minted");
        id_to_geohash[tokenId].geohash = geohash;
    }
    ...
    function setBaseProURI(string memory baseProURI) public onlyOwner {
        _baseProvableURI = baseProURI;
    }
    ...
    function tokenPause(bool val) public onlyOwner {
        _tokenPaused = val;
    }
