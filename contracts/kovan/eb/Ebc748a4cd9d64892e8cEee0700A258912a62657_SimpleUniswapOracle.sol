...
import {SafeMath} from "@uniswap/v2-periphery/contracts/libraries/SafeMath.sol";
...
contract SimpleUniswapOracle is IUniswapOracleGetter {
    ...
    function update(address asset) external {
        address pair = UniswapV2Library.pairFor(factory, asset, weth);
        ...
        // owner can force update for unexpected conditions
        if (msg.sender != owner) {
    ...
    {
        pair = UniswapV2Library.pairFor(factory, asset, weth);
        ...
        returns (uint256)
    {
        ...
        require(
            observation.blockTimestampLast > 0,
            "SimpleUniswapOracle: MISSING_HISTORICAL_OBSERVATION"
        );

        if (asset == IUniswapV2Pair(pair).token0()) {
            // new erc20 token is always 18 decimals, so use 1 ether here
            return observation.price0Average.mul(1 ether).decode144();
        } else {
            return observation.price1Average.mul(1 ether).decode144();
        }
    }
}
