...
contract Flashloan is FlashLoanReceiverBase {
    ...
    function swapTokens(address assest, uint256 _amount) public {
        IERC20 dai = IERC20(assest);
        deadline = block.timestamp + 300;

        dai.approve(address(uniswapRouter), _amount);
        uint256 estimatedETH =
            getEstimatedETHForToken(_amount, address(assest))[1];

        uniswapRouter.swapExactTokensForETH(
            _amount,
            estimatedETH,
            getAssetPath(address(assest)),
            address(this),
            deadline
        );
