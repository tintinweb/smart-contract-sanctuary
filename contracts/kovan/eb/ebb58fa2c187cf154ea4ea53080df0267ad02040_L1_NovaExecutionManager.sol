...
contract L1_NovaExecutionManager is Auth, CrossDomainEnabled {
    ...
    /// @dev This should include the base transaction gas (currently 21000) as well
    /// as the gas cost of sending the cross domain message to complete the request.
    uint256 public missingGasEstimate = 21000 + 172814;

    /// @dev The amount of gas to assume for each byte of calldata.
    uint256 public gasEstimatePerCalldataByte = 13;

    function setMissingGasEstimate(uint256 newMissingGasEstimate) external requiresAuth {
        missingGasEstimate = newMissingGasEstimate;
    }

    function setGasEstimatePerCalldataByte(uint256 newGasEstimatePerCalldataByte) external requiresAuth {
        gasEstimatePerCalldataByte = newGasEstimatePerCalldataByte;
    }
    ...
    function exec(
        ...
        uint256 gasUsedEstimate =
            missingGasEstimate + /* Estimate of unaccounted for gas usage (base tx cost + sendMessage) */
                (msg.data.length * gasEstimatePerCalldataByte) + /* Calldata cost estimate */
