...
contract LiquidityInvestmentPool is PoolRoles {
    ...
    uint256 private _annualPercent;// Anual precent
    ...
    function _depositPoolRegistry(address sender, uint256 amount, uint256 feesMultipier) external onlyPoolRegistry returns (bool) {
        require(hasRole(LIMITED_PARTNER_ROLE, sender), "LiquidityInvestmentPool: the sender does not have permission");
    ...
    function _depositTokenPoolRegistry(address payable sender, uint256 amount) external onlyPoolRegistry returns (bool) {
        require(hasRole(STARTUP_TEAM_ROLE, sender), "LiquidityInvestmentPool: the sender does not have permission");
    ...
    function _removeLiquidity(address sender) private returns (uint) {
        ...
        uint amountToken;
        uint amountETH;
        (amountToken, amountETH) = uniswapV2Router02.removeLiquidityETH(_pairToken, _liquidity[sender], 0, 0, address(this), block.timestamp.add(10000));  /* function removeLiquidityETH(address token,uint liquidity,uint amountTokenMin,uint amountETHMin,address to,uint deadline) external returns (uint amountToken, uint amountETH);*/
        _liquidity[sender] = 0;
        address[] memory pathForSwap = new address[](2);
        pathForSwap[0] = address(_pairToken);
        pathForSwap[1] = uniswapV2Router02.WETH();
        uint[] memory amounts = uniswapV2Router02.swapExactTokensForETH(amountToken, 0, pathForSwap, address(this), block.timestamp.add(10000));/* function function swapExactTokensForETH(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline) external returns (uint[] memory amounts);*/
        uint amountETHreceivedFromSwap = amounts[1];
        emit DebugRemoveLiquidity(amountToken, amountETH, amounts[0], amounts[1]);
        return amountETHreceivedFromSwap.add(amountETH).mul(99).div(100);
    }
    ...
    function addLiquidityPartner() public payable returns (bool) {
        return  _deposit(msg.sender, msg.value, _poolRegistry.feesMultipier(msg.sender));
    }

    function withdrawLiquidityPartner() public returns (uint256) {
        uint256 totalAmountReturnFinal = _removeLiquidity(msg.sender);
        msg.sender.transfer(totalAmountReturnFinal);
        return totalAmountReturnFinal;
    }

    function _withdrawSuperAdmin(address payable sender, address token, uint256 amount) external onlyPoolRegistry returns (bool) {
        require(hasRole(SUPER_ADMIN_ROLE, sender), "LiquidityInvestmentPool: the sender does not have permission");
        if (amount > 0) {
            if (token == address(0)) {
                sender.transfer(amount);
                return true;
    ...
    function _activateDepositToPool() external onlyPoolRegistry returns (bool) {
        require(!_isPublicPool, "LiquidityInvestmentPool: the pool is already activated");
        _isPublicPool = true;
        return true;
    }
    function _disactivateDepositToPool() external onlyPoolRegistry returns (bool) {
        require(_isPublicPool, "LiquidityInvestmentPool: the pool is already deactivated");
        _isPublicPool = false;
        return true;
    }
    ...
    function _addLiquidity(uint256 totalDeposit, address sender) private returns (bool) {
        ...
        path[0] = UniswapV2Router02.WETH();
        path[1] = address(_pairToken);
        uint[] memory amounts = UniswapV2Router02.swapExactETHForTokens{value : totalDepositHalf}(0, path, address(this), block.timestamp.add(10000));
        uint amountTokenDesired = amounts[1];
        IERC20(_pairToken).approve(_routerContractAddress, IERC20(_pairToken).totalSupply());
        uint amountToken;
        uint amountETH;
        uint liquidity;
        (amountToken, amountETH, liquidity) = UniswapV2Router02.addLiquidityETH{value : totalDepositHalf}(_pairToken, amountTokenDesired, 0, 0, address(this), block.timestamp.add(10000));
        _liquidity[sender] = _liquidity[sender].add(liquidity);
        return true;
    }
    function _deposit(address sender, uint256 amount, uint256 feesMultipier) private returns (bool) {
        require(_isPublicPool, "LiquidityInvestmentPool: pool deposit blocked");
    ...
    function _depositToken(address payable sender, uint256 amount) private returns (bool) {
        ...
        require(amountConverted <= address(this).balance, "LiquidityInvestmentPool: contract balance is insufficient");
