...
contract Bridge is Ownable, ReentrancyGuard {
    ...
    /// Address for signature methods
    address public verifyAddress;

    /// chain -> index -> bool
    mapping(uint => mapping(uint => bool)) public claimed;

    /// Unique ID of each transfer request
    uint public currentIndex = 1;

    /// Current chain ID
	uint public chainId;

	/// NewStableFee start time. Before this time users have stableFee.
	...
	event NewTransferRequest(uint indexed tokenIndex, address indexed from, address indexed to, uint amount, uint fee, uint index, uint toChain);
    ...
    /**
	 * @dev Constructor
	 * @param _wPoktAddress POKT address on current network.
	 * @param _verifyAddress Verify address for signature methods.
	 * @param _chainId Current chain id. 
	 * @param _stableFee Initial stable fee.
	 */
	constructor(
		address _wPoktAddress,
		...
		public Ownable()
	{
		...
		tokenAdded[_wPoktAddress] = true;

		chainId = _chainId;

		emit StableFeeChanged(stableFeeUpdateTime, _stableFee);
		emit VerifyAddressChanged(_verifyAddress);
		emit NewActiveToken(_wPoktAddress, 1);
	}

	/**
	 * @dev transferRequest Creates a transfer request for token from this chain to other one. Tokens will be 
	 * transferred from caller to this contract, fees are collecting on current chain.
	 * @param _tokenIndex Index of transferring token (WPOKT - 1).
	 * @param _to Token receiver on destination chain.
	 * @param _amount Token amount including fees. 
	 * @param _chainId Destination chain ID. Can be defined manually if it is not ETH compatible chain.
	 */
	function transferRequest(uint _tokenIndex, address _to, uint _amount, uint _chainId) external nonReentrant {
		...
		emit NewTransferRequest(_tokenIndex, msg.sender, _to, _amount, _fee, currentIndex, _chainId);
		currentIndex++;
	}

	/**
	 * @dev transferReceipt Sends tokens from other chain to current one to user _to.
	 * @param _tokenIndex Index of receiving token (WPOKT - 1).
	 * @param _from Sender address defined from opposite chain.
	 * @param _to Receiver address defined from opposite chain.
	 * @param _amount Amount without fees defined from opposite chain.
	 * @param _chainId Opposite chain ID.
	 * @param _index Unique transaction index defined from opposite chain.
	 * @param signature Unique signature signed by verifyAddress. Look at verify method.
	 */
	...
	/**
	 * @dev Function to update verifyAddress. Can be used only by owner.
	 * @param _verifyAddress New verify address.
	 */
	...
	/**
	 * @dev Function to update daily token limit. Can be used only by owner.
	...
	/**
	 * @dev Function to set start time of each interval for token limits. Can be used only by owner.
	 * @param _tokenIndex Token index.
	 * @param _timestamp New interval date.
	 */
	...
	/**
	 * @dev Function to set new stable fee. Can be used only by owner.
	 * @param _newStableFee New stable fee.
	 */
	...
	/**
	 * @dev Function to set new token fee. Can be used only by owner.
	 * @param _tokenIndex Token index.
	 * @param _newTokenFee New token fee.
	 */
	function updateTokenFee(uint _tokenIndex, uint _newTokenFee) external onlyOwner {
		tokenList[_tokenIndex].feeUpdateTime = block.timestamp + FEE_UPDATE_DURATION;
		tokenList[_tokenIndex].newFee = _newTokenFee;
		emit TokenFeeChanged(tokenList[_tokenIndex].feeUpdateTime, _tokenIndex, _newTokenFee);
	}

	/**
	 * @dev Function to update current fees.
	 * @param _tokenIndex Token index.
	 */
	...
	/**
	 * @dev Function to withdraw token fees.
	 * @param _tokenIndex Token index.
	 */
	function withdrawFees(uint _tokenIndex) external nonReentrant onlyOwner {
	    TokenData storage tokenData = tokenList[_tokenIndex];
		...
		emit FeesWithdrawn(msg.sender, _tokenIndex, toTransfer);
	}

	/**
	 * @dev Function to add new token to the bridge.
	 * @param _index Token index.
	 * @param _tokenAddress New token address.
	 * @param _fee Token fee.
	 * @param _limit Token limits.
	 */
	...
	/**
	 * @dev Function to pause all transfer requests. Transfer requests cannot be created until unpause
	 * @param _tokenIndex Token index.
	 */
	...
	/**
	 * @dev Function to unpause all transfer requests.
	 * @param _tokenIndex Token index.
	 */
	...
	/**
	 * @dev Function to calculate fees on current request.
	 * @param _tokenIndex Token index.
	 * @param _amount Requested transfer amount
	 * @return Fees for calculated using input params
	 */
