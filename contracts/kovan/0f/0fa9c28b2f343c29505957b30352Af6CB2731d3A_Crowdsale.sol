...
contract Crowdsale is Context, ReentrancyGuard, Ownable {
    ...
    /**
     * @dev Calculates the rate based on the current price and the specified buyprice.
     * @param tokenPrice this is the price of 1 token in USD denominated in wei (1,OO == 1000000000000000000 USD)
     */
    function _calculateRate(uint256 tokenPrice) internal view returns (uint256){
        uint256 dollarToEthamount = _fetchPrice('USD', 'ETH');
        return dollarToEthamount.mul(
                    tokenPrice
                ).div(10**18);
    }
