...
contract ControllerV1 is ControllerInterface, ControllerStorage, Adminable {
    ...
    function createLPoolPair(address token0, address token1, uint32 marginRatio) external override {
        ...
        string memory token0Symbol = string(abi.encodePacked(LPOOL_TOKEN_PREFIX, ERC20(token1).symbol(), "-", ERC20(token0).symbol()));
        pool0.initialize(token0, this, interestRateModel, LPOOL_TOKEN_EXCHANGE_RATE_MANTISSA,
            token0Symbol, token0Symbol, LPOOL_TOKEN_DECIMALS, admin, lpoolImplementation);

        LPoolDelegator pool1 = new LPoolDelegator();
        string memory token1Symbol = string(abi.encodePacked(LPOOL_TOKEN_PREFIX, ERC20(token0).symbol(), "-", ERC20(token0).symbol()));
        pool1.initialize(token1, this, interestRateModel, LPOOL_TOKEN_EXCHANGE_RATE_MANTISSA,
            token1Symbol, token1Symbol, LPOOL_TOKEN_DECIMALS, admin, lpoolImplementation);
