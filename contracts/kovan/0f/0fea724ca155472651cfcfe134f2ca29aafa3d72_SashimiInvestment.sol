...
contract SashimiInvestment is ISashimiInvestment, Ownable {
    ...
    modifier onlyRouter(){
        require(
            routers[routerId[msg.sender]] == msg.sender,
            "Not router."
        );
        _;
    }
    ...
    modifier onlySupportRouter(address _router){
        require(
            routers[routerId[_router]] ==_router, "Not router");
        _;
    }
    ...
    function reBalance(address _token, address _router) notFrozen onlySupportRouter(_router) public {
    ...
    function addRouter(address _router) public onlyOwner {
        ...
        uint256 tail = routers.length.sub(1);
        routers.push(_router);
        routerId[_router] = tail.add(1);
        emit RouterAdded(_router, tail.add(1));
    }
}