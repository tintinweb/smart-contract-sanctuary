...
contract NiftyBuilderInstance is ERC721Full {
    ...
    function batchGiftNifty(address[] memory collector_address, uint[] memory niftyType) onlyValidSender public {
        for (uint i = 0; i < collector_address.length; i++) {
            giftNifty(collector_address[i], niftyType[i]);
        }
    }
    ...
    function giftNifty(address collector_address, uint niftyType) onlyValidSender public {
        ...
        uint tokenId = ((niftyType * 100000) + specificTokenId);
