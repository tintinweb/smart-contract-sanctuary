...
contract Vault is Ownable, ReentrancyGuard {
    ...
    // decimals of TRUSTED_DEPOSIT_TOKEN_ADDRESS
    uint public constant UNDERLYING_DECIMALS = 18;
    
    // ETH fee equivalent to 400k gas at 60 gwei
    uint public constant MIN_ETH_FEE_IN_WEI = 400000 * 60 * 10**9;
    
    // slippage tolerance to use while doing uniswap transactions
    uint public constant SLIPPAGE_TOLERANCE_X_100 = 300;
    
    address public constant TRUSTED_DEPOSIT_TOKEN_ADDRESS = 0x4F96Fe3b7A6Cf9725f59d353F723c1bDb64CA6Aa;
    address public constant TRUSTED_CTOKEN_ADDRESS = 0xF0d0EB522cfa50B716B3b1604C4F0fA6f04376AD;
    address public constant TRUSTED_PLATFORM_TOKEN_ADDRESS = 0xd8bB0ff7a267ab387015C18675591912D8035D9E;
    ...
    function getConvertedBalance(uint _cTokenBalance) public view returns (uint) {
        ...
        uint convertedBalance = _cTokenBalance.mul(exchangeRateStored).div(10**10);
    ...
    function _claimCompoundDivs() private {
        ...
        uint convertedBalance = cTokenBalance[msg.sender].mul(exchangeRateCurrent).div(10**10);
    ...
    function withdraw(uint amount) external noContractsAllowed nonReentrant payable {
        ...
        require(now.sub(depositTime[msg.sender]) > LOCKUP_DURATION, "You recently deposited, please wait before withdrawing.");
    ...
    function handleFee(uint feeAmount) private {
        ...
        increaseTokenBalance(TRUSTED_DEPOSIT_TOKEN_ADDRESS, remainingFeeAmount);
        // handle distribution
    ...
    function handleEthFee(uint feeAmount) private {
        ...
        uint buyBackFeeAmount = feeAmount.mul(FEE_PERCENT_TO_BUYBACK_X_100).div(ONE_HUNDRED_X_100);
        uint remainingFeeAmount = feeAmount.sub(buyBackFeeAmount);
        increaseTokenBalance(address(0), remainingFeeAmount);
        // handle distribution
