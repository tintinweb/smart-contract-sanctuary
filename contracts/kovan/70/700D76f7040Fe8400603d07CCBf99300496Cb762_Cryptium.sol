...
abstract contract Control is AccessControl {
	...
	// DEV func
	function resetStartTime(uint256 startTime) external onlyRole(DEFAULT_ADMIN_ROLE) {
	...
	function setFee(uint8 operateFee, uint8 holderFee) external isValidPercent(operateFee) isValidPercent(holderFee) isPermitToChange {
		require(operateFee + holderFee <= 100, "Fee sum must below 0");
		_operateFee = operateFee;
		_holderFee = holderFee;
		_burnFee = 100 - operateFee - holderFee;
	}
	...
	function setPeriod(uint32 _period) external isPermitToChange {
...
contract Cryptium is CrptERC20, Control, ReentrancyGuard {
	 ...
	 */
	function initializeContract(uint256 startTime, address poolAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
		require(!_isInitialized, "Already initialized");
		require(_poolAddress != address(0), "Must set pool address before");
		_isInitialized = true;
		_startTime = startTime;
		_lastRebase = startTime;
		_poolAddress = poolAddress;
		IUniswapV3Pool(_poolAddress).increaseObservationCardinalityNext(40);
		_initialPrice = uint80(_getCurrentPrice());
		_isWhitelist[_poolAddress] = true;
		_feeAddress = msg.sender;
	}
	...
	function rebase() external nonReentrant initialized {
		...
		if (isInflRebase) {
			...
			// _lastRebase = TimeLibrary.addDays(_lastRebase, daysPassFromRebase); // DEV
			_lastRebase = _lastRebase + daysPassFromRebase * (3 minutes);
		}
	 ...
	 */
	function getAvailToClaim(address user) external initialized returns (uint256) {
		...
		emit Log("avail to claim", (_gonBalances[user] / gonsPerFragment) - balanceBefore, 0, 0, 0);
	...
	function _transfer(
		...
		if (msg.sender == _swapRouter && _isWhitelist[to]) {
	...
	function _feeCount(
		...
		// emit Log2("from, to", from, to, msg.sender);
	...
	function _feeToHolders(uint256 fee) private {
		_totalFee = _totalFee + ((fee * _holderFee) / 100);
	}
		...
		)
	{
		...
		if (newPrice == 0) {
			...
			price1 = (curPrice * oldPrice * 10000) / newPrice / 10000;
		}
		if (daysPassFromRebase == 0) {
			price2 = 0;
		} else {
			price2 = ((_coeffInflations * daysPassAll * 10000) / 365 + 100000000) * _initialPrice / 10000;
		}

		if (price1 > price2 && price1 > curPrice) {
			supplyDelta = (_totalSupply * ((oldPrice * 10000) / newPrice)) / 10000;
		} else if (price2 > curPrice) {
			price2 = ((_coeffInflations * daysPassFromRebase * 10000) / 365 + 100000000) * _initialPrice;
			supplyDelta = ((_totalSupply * price2) / curPrice) / 100000000;
			isInflRebase = true;
		} else {
			isFall = false;
		}
		emit Log("curPrice daysPassAll daysPassFromRebase supplyDelta", curPrice, daysPassAll, daysPassFromRebase, supplyDelta);
	 ...
	 */
	function _getDaysPass() private returns (uint256, uint256) {
		// uint256 daysPassAll = TimeLibrary.diffDays(_startTime, block.timestamp); // DEV
		// uint256 daysPassFromRebase = TimeLibrary.diffDays(_lastRebase, block.timestamp);
		uint256 daysPassAll = (block.timestamp - _startTime) / (3 minutes);
		uint256 daysPassFromRebase = (block.timestamp - _lastRebase) / (3 minutes);
		emit Log("startTime timestamp", _startTime, block.timestamp, 0, 0);
		emit Log("lastRebase timestamp", _lastRebase, block.timestamp, daysPassAll, daysPassFromRebase);
	...
	function _getAveragePrices() private view returns (uint256, uint256) {
		...
		bool isDownFallBool = newPrice < (oldPrice * 99) / 100;
		if (isDownFallBool) {
	 ...
	 * @return oldPrice average price for previous period
	 * @return newPrice average price for current period
	 */
