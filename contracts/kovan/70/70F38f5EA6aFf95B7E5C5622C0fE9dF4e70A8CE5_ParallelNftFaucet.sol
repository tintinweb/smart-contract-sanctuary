...
//     }
//     function simpleMint(uint256 id, bytes32 r, bytes32 s, Fee[] memory fees, uint256 supply, string memory uri) public {
//         _mint(id, fees, supply, uri);
//     }
// }
...
contract ParallelNftFaucet {
    ...
    uint8 public maxBlockWait = 255;

//    uint16[20] public enabledNfts = [1,2];
    mapping (uint16 => bool) enabledNfts;

    uint16 public maxNftsPerUser = 1;
    uint16 public currentBonusCard = 3;
...
//        return (depositInfo[_userAddress].totalNftsReceived);
    ...
    function getNftEnabled(uint16 _nftId) public returns (bool){
        return enabledNfts[_nftId];
    }
    function setNftEnabled(uint16 _nftId, bool _enabled) public onlyOwner{
    ...
    function setCurrentBonusCard(uint16 _id) public onlyOwner {
...
//    }
    ...
    function requestNft(bytes memory _ticket, uint16 _nftId, uint _ticketBlock) public {
...
//        depositInfo[msg.sender].totalNftsReceived = depositInfo[msg.sender].totalNftsReceived + 1;
    ...
    function getMessageHash(
        address _to, uint16 _nftId, uint16 _amount, string memory _message, uint _ticketBlock
    )
