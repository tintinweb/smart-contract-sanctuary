...
contract Market {
     ...
     */
	function addToken(uint256 amount) external onlyAdmin{
    ...
    function stakeDog(uint256 dogId, uint256 price, uint256 start, uint256 end, uint256 apy, uint256 reward) public notContract {
        ...
        user.dogId.push(dogId);
        user.price.push(price);
        user.start.push(start);
        user.end.push(end);
        user.apy.push(apy);
     ...
     * @param dogId: Dog ID
     */
    function getStakeStatus1(uint256 dogId, address address1) public view returns (uint256 start, uint256 end, uint256 apy, uint256 reward) {
        
        UserInfo storage user = userInfo[address1];
     ...
     * @param dogId: Dog ID
     */
    function unStake(uint256 dogId, uint256 price, uint256 end, uint256 reward) public notContract {
        UserInfo storage user = userInfo[msg.sender];
		...
		if (label != 0){
		    require(user.end[label] < end, "Can't unstake before the redemption date.");
		    
			for (uint256 j = label; j < numberOfStake; j++) {
			    user.dogId[j - 1] = user.dogId[j];
			    user.price[j - 1] = user.price[j];
			    user.start[j - 1] = user.start[j];
			    user.end[j - 1] = user.end[j];
			    user.apy[j - 1] = user.apy[j];
			    user.reward[j - 1] = user.reward[j];
			}
			user.dogId.pop();
			user.price.pop();
			user.start.pop();
			...
			_token.code_approve(address(this), msg.sender, reward);
