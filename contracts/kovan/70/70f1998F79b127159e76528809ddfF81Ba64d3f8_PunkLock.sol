 ...
 * Punk Lock contract
 * How it works?
 * 1. punk owner calls lockPunk()
 * 2. punk owner transfers punk to the contract
 * 3. punk owner then approve JPEGD to the contract
 * 4. punk owner then calls stakeForPunk() ( 1 Year staking starts from this point )
 * 5. punk owner can call unlockpunk() after 1 year to withdraw punk and
 */
contract PunkLock is VotesSnapshot, ReentrancyGuard {
    ...
    constructor(address _jpegd, address _cryptopunks) ReentrancyGuard() {
        jpegd = IERC20(_jpegd);
        cryptopunks = ICryptoPunks(_cryptopunks);
    }

    function lockPunk(uint256 _punkIndex) external nonReentrant {
        require(msg.sender == _getPunkOwner(_punkIndex), "punk_not_owned");
        require(
            msg.sender != positions[_punkIndex].owner,
    ...
    function stakeForPunk(uint256 _punkIndex, uint256 _lockAmount)
        external
        nonReentrant
    {
        require(
            address(this) == _getPunkOwner(_punkIndex),
            "punk_not_deposited"
        );
        require(positions[_punkIndex].owner == msg.sender, "unauthorized");
        require(positions[_punkIndex].unlockAt == 0, "stake_started");

        jpegd.safeTransferFrom(msg.sender, address(this), _lockAmount);

        positions[_punkIndex].unlockAt = block.timestamp + SECS_YEAR;
        positions[_punkIndex].lockAmount = _lockAmount;

        _updateVotes(address(0), msg.sender, uint224(_lockAmount));

        emit Lock(msg.sender, _punkIndex, _lockAmount);
    }

    function unlockPunk(uint256 _punkIndex) external nonReentrant {
        ...
        if (position.lockAmount > 0) {
            // transfer jpegd back to owner
            jpegd.safeTransfer(msg.sender, position.lockAmount);
            _updateVotes(msg.sender, address(0), uint224(position.lockAmount));
        }

        // delete position
        positions[_punkIndex].owner = address(0);
    ...
    function _getPunkOwner(uint256 _punkIndex) internal view returns (address) {
        return cryptopunks.punkIndexToAddress(_punkIndex);
    }
}
