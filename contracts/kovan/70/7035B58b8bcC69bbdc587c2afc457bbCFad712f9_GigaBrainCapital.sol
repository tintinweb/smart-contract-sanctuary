...
GigaBrain Capital || GigaBrain.Capital

100% Fair Launch
Total Supply: 10B (10000000000)
LP Lock: 100% token, 1 Month at the launch

Fairness Protection:
- Anti 0-1 block Sniping bot.
- Max TX amount: 0.49% at the Beginning, uplift to 2% after 5 mins
- Anti-Whale: Max Wallet amount limit: 2% all time
...
Treasury:
Multi-Sig protection by Gnosis Safe
...
contract GigaBrainCapital is Ownable, IERC20 {
	address UNISWAPROUTER = address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
	address DEAD = 0x000000000000000000000000000000000000dEaD;
	address ZERO = 0x0000000000000000000000000000000000000000;

	string private _name = "GigaBrainCapital";
	string private _symbol = "Giga";
	...
	function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
		uint256 currentAllowance = _allowances[_msgSender()][spender];
		require(currentAllowance >= subtractedValue, "GigaBrain: decreased allowance below zero");
		_approve(_msgSender(), spender, currentAllowance - subtractedValue);
		return true;
	}
	...
	function transferFrom(
		...
		_transfer(sender, recipient, amount);
		uint256 currentAllowance = _allowances[sender][_msgSender()];
		require(currentAllowance >= amount, "GigaBrain: transfer amount exceeds allowance");
		_approve(sender, _msgSender(), currentAllowance - amount);
		return true;
	}
	...
	function _transfer(
		...
		require(!isBlacklisted[sender], "GigaBrain: Sender is blacklisted");
		require(!isBlacklisted[recipient], "GigaBrain: Recipient is blacklisted");

		require(sender != address(0), "GigaBrain: transfer from the zero address");
		require(recipient != address(0), "GigaBrain: transfer to the zero address");
		...
		uint256 senderBalance = _balances[sender];
		require(senderBalance >= amount, "GigaBrain: transfer amount exceeds balance");
	...
	function _approve(
		...
		require(owner != address(0), "GigaBrain: approve from the zero address");
		require(spender != address(0), "GigaBrain: approve to the zero address");
		_allowances[owner][spender] = amount;
		emit Approval(owner, spender, amount);
	}

	function _mint(address account, uint256 amount) private {
		require(account != address(0), "GigaBrain: mint to the zero address");
		_totalSupply += amount;
		_balances[account] += amount;
		emit Transfer(address(0), account, amount);
	}
	...
	function _burn(address account, uint256 amount) private {
		require(account != address(0), "GigaBrain: burn from the zero address");
		uint256 accountBalance = _balances[account];
		require(accountBalance >= amount, "GigaBrain: burn amount exceeds balance");
		_balances[account] = accountBalance - amount;
		_totalSupply -= amount;
		emit Transfer(account, address(0), amount);
	}
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFees[account] != excluded, "GigaBrain: account is already set to requested state");
	...
	function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
		require(pair != uniswapV2Pair, "GigaBrain: DEX pair can not be removed");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "GigaBrain: automated market maker pair is already set to that value");
	...
	function updateUniswapV2Router(address newAddress) public onlyOwner {
		require(newAddress != address(uniswapV2Router), "GigaBrain: the router is already set to the new address");
	...
	function compound() public {
		require(compoundingEnabled, "GigaBrain: compounding is not enabled");
...
contract DividendTracker is Ownable, IERC20 {
	...
	string private _name = "GigaBrain_DividendTracker";
	string private _symbol = "GigaBrain_DividendTracker";
	...
	function excludeFromDividends(address account, bool excluded) external onlyOwner {
		require(excludedFromDividends[account] != excluded, "GigaBrain_DividendTracker: account already set to requested state");
	...
	function _mint(address account, uint256 amount) private {
		require(account != address(0), "GigaBrain_DividendTracker: mint to the zero address");
		_totalSupply += amount;
		_balances[account] += amount;
		emit Transfer(address(0), account, amount);
	...
	function _burn(address account, uint256 amount) private {
		require(account != address(0), "GigaBrain_DividendTracker: burn from the zero address");
		uint256 accountBalance = _balances[account];
		require(accountBalance >= amount, "GigaBrain_DividendTracker: burn amount exceeds balance");
		_balances[account] = accountBalance - amount;
		_totalSupply -= amount;
		emit Transfer(account, address(0), amount);
		magnifiedDividendCorrections[account] = magnifiedDividendCorrections[account] + int256(magnifiedDividendPerShare * amount);
	...
	function transfer(address, uint256) public pure override returns (bool) {
		revert("GigaBrain_DividendTracker: method not implemented");
	}
