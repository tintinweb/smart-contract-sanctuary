...
contract FUN is ERC165, IERC721, IERC721Metadata, IERC721Enumerable, IERC2981, Ownable, Pausable { 
    ...
    event UpdateTokenName(string indexed previousName, string indexed newName);
    event UpdateTokenDescription(string indexed previousDescription, string indexed newDescription);
    ...
    function mintWithFullMetadata(uint collectionId, address to, string memory tokenUri, address author, string memory name, string memory description) public virtual onlyOwner returns(uint tokenId) {
        tokenId = _mint(collectionId, to);
        _setTokenURI(tokenId, tokenUri, true);
        _updateTokenAuthor(tokenId, address(0), author);
        _updateTokenName(tokenId, "", name);
        _updateTokenDescription(tokenId, "", description);
    }

    function safeMintWithFullMetadata(uint collectionId, address to, string memory tokenUri, address author, string memory name, string memory description, bytes memory _data) public virtual onlyOwner returns(uint tokenId) {
    ...
    function muliMintWithFullMetadata(uint[] memory collectionIds, address[] memory to, string[] memory tokenURIs, address[] memory tokenAuthors, string[] memory tokenNames, string[] memory tokenDescriptions) external virtual onlyOwner {
        require(to.length == tokenURIs.length && to.length == collectionIds.length && to.length == tokenAuthors.length && to.length == tokenNames.length && to.length == tokenDescriptions.length,"FUN: arrays have different lengths");
        for (uint i; i < to.length; i++) {
            mintWithFullMetadata(collectionIds[i], to[i], tokenURIs[i], tokenAuthors[i], tokenNames[i], tokenDescriptions[i]);
        }
    }

    function muliSafeMintWithFullMetadata(uint[] memory collectionIds, address[] memory to, string[] memory tokenURIs, address[] memory tokenAuthors, string[] memory tokenNames, string[] memory tokenDescriptions, bytes[] memory _data) external virtual onlyOwner {
        require(to.length == tokenURIs.length && to.length == collectionIds.length && to.length == tokenAuthors.length && to.length == tokenNames.length && to.length == tokenDescriptions.length && to.length == _data.length,"FUN: arrays have different lengths");
        for (uint i; i < to.length; i++) {
            safeMintWithFullMetadata(collectionIds[i], to[i], tokenURIs[i], tokenAuthors[i], tokenNames[i], tokenDescriptions[i], _data[i]);
        }
    }
    ...
    function updateTokenName(uint tokenId, string memory newName) external virtual onlyOwner {
        require(_exists(tokenId), "FUN: name set of nonexistent token");
        _updateTokenName(tokenId, _tokenMetaDatas[tokenId].name, newName);
    }

    function updateTokenDescription(uint tokenId, string memory newDescription) external virtual onlyOwner {
        require(_exists(tokenId), "FUN: description set of nonexistent token");
        _updateTokenDescription(tokenId, _tokenMetaDatas[tokenId].description, newDescription);
    }

    function updateTokenRoyaltyPolicy(uint tokenId, bool isRoyaltyFree) external virtual onlyOwner {
        require(_exists(tokenId), "FUN: author set of nonexistent token");
    ...
    function _updateTokenName(uint tokenId, string memory previousName, string memory newName) internal virtual {
        _tokenMetaDatas[tokenId].name = newName;
        emit UpdateTokenName(previousName, newName);
    }

    function _updateTokenDescription(uint tokenId, string memory previousDescription, string memory newDescription) internal virtual {
        _tokenMetaDatas[tokenId].description = newDescription;
        emit UpdateTokenDescription(previousDescription, newDescription);
    }
}