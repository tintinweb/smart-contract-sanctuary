...
contract Lending is ILending {
    ...
    function inititiate(
        ...
        if (_converter != address(0)) {
            IERC20(_usdc).approve(_converter, type(uint).max);
            IERC20(_dai).approve(_converter, type(uint).max);
            IERC20(_usdcAAVE).approve(_converter, type(uint).max);
            IERC20(_daiAAVE).approve(_converter, type(uint).max);
        }
    }
