...
contract MuskTestV2 is Context, IERC20, Ownable {
  ...
  mapping (address => uint256) public timestamp;
    ...
    string private _name = "MuskTest V2";
    string private _symbol = "MuskTestV2";
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner())
            ...
            if(from != owner() && from != uniswapV2Pair) {
                ...
                // dont apply below rules to excludede addresses like presale contracts

                if (!_isExcluded[from]) {

                        //you can't trade this yet, just be patient we're probably still in presale
                        ...
                        if(getSwapPercent(balanceOf(from),_maxTxAmount) > 10 ){

                            //now we know user has higher than 10% of max tx allowed, otherwise we can just let them swap amount is tiny
                            require(getSwapPercent(amount, balanceOf(from)) <= 25, "You cannot trade more than 25% per swap");
                        }
