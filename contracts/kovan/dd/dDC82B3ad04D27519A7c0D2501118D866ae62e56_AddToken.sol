...
// YFEToken with Governance.
contract AddToken is ERC20("AddToken", "ADD"), Ownable {
    ...
    function setTransferFee(uint256 _fee) public onlyOwner {
        require(
            _fee > 0 && _fee < 1000,
            "AddToken: fee should be between 0 and 10"
        );
        transferFee = _fee;
    }
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "AddToken::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "AddToken::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "AddToken::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "AddToken::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber =
            safe32(
                block.number,
                "AddToken::_writeCheckpoint: block number exceeds 32 bits"
            );
