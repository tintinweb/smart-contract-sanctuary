...
contract Precog is IPrecog {
    ...
    function liquidate(address futureContract, address user) external returns(uint256 amount) {
        ...
        if (loans.length > 0) {
            for (uint i = 0; i < loans.length; i++) {
                amount += loans[i].borrowAmount + loans[i].fee;
            }
        }
    ...
    function _getAmountTest(address user, address futureContract) external view returns (uint amount) {
        amount = tradingAmountOnFutureContract[user][futureContract] + profitAmountOnFutureContract[user][futureContract];
    ...
    function _getUserLoans(address user, address futureContract) public view returns(BorrowInfo[] memory userLoans) {
    ...
    function _getAvailableLoans(uint amount, uint fee) public view returns (BorrowInfo[] memory loans) {
    ...
    function _getAvailableLoan(uint platformIndex, uint borrowLimit, uint fee) internal view returns (BorrowInfo memory loan) {
        ...
        loan = BorrowInfo(platformIndex, borrowAmount, fee);
    }
