...
library ProportionalLiquidity {
    ...
    int128 public constant ONE_WEI = 0x12; // 1 * 10 ^ 1
        ...
        returns (uint256 curves_, uint256[] memory)
    {
        ...
        if (_oGLiq == 0) {
            for (uint256 i = 0; i < _length; i++) {
                ...
                deposits_[i] = Assimilators.intakeNumeraire(curve.assets[i].addr, _d.add(ONE_WEI)); // 1.5
            }
        } else {
            ...
            uint256 _baseWeight = curve.weights[0].mulu(1e18); // 0.25
            uint256 _quoteWeight = curve.weights[1].mulu(1e18); // 0.75     
...
library CurveMath {
    ...
    // USDC -> FXPHP
    // XSGD -> FXPHP:
    // Steps: XSGD -> USDC, USDC -> FXPHP
    

    // PROPORTIONAL DEPOSIT
    // 10 HLP (XSGD:USDC) that means he deposit 10 Numeraire
    // 10 Numeraire = 6.85 XSGD + 5 USDC
