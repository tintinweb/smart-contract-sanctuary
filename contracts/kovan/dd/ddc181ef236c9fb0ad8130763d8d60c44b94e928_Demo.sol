...
contract Demo is Context, IERC20, Ownable {
    ...
    string private _name = "Demo Token";
    ...
    uint256 internal _tokenTotal = 10000 *10**18;
    ...
    address public devAddress = 0xdddFE120F803b18159EcB807274fb37340a12dd4;
    
    uint256 public _taxFee = 1;
    uint256 public _devFee = 1;
    uint256 public _liquidityFee = 1;

    uint256 public _rebalanceCallerFee = 0;
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(account != 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D, "Demo: Uniswap router cannot be excluded.");
        require(account != address(this), 'Demo: The contract it self cannot be excluded');
        require(!_isExcluded[account], "Demo: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "Demo: Account is already included");
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        uint256 transferAmount = amount;
        uint256 devFee = amount.mul(_devFee).div(100);
        
        //@dev Tax fee
        if(_taxFee != 0){
            uint256 taxFee = amount.mul(_taxFee).div(100);
    ...
    function buyAndBurnToken(uint256 contractBalance) private lockTheSwap {
        ...
        uint256 rewardForCaller = swapedTokens.mul(_rebalanceCallerFee).div(100);
        uint256 amountToBurn = swapedTokens.mul(_burnFee).div(100);
    ...
    function _burn(uint256 amount) public onlyOwner {
        require(amount > 0, "Need token amount more than 0");
        uint256 rate =  _getReflectionRate();
    ...
    function transferAnyERC20Tokens(address _tokenAddress, address _to, uint256 _amount) public onlyOwner {
        ...
        if (_tokenAddress == address(this)) {
            require(_reflectionBalance[address(this)].sub( _amount) == _liquidityFeeTotal);
            Token(_tokenAddress).transfer(_to, _amount);
            
        } else if (_tokenAddress != address(this)) {
            Token(_tokenAddress).transfer(_to, _amount);
        }
    }
