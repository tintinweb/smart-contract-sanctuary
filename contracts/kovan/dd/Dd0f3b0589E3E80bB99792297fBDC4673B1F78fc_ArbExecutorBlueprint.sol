...
contract ArbExecutorBlueprint {
    ...
    function ST3(address[] memory _operationPath, uint256 amountIn) public {

        uint256 out1 = Uniswap_EthToToken(_operationPath[1], amountIn, 1);
        uint256 tempOut = out1;

        for(uint i=1; i<(_operationPath.length - 2); i++) {
            uint newOut = Uniswap_TokenToToken(_operationPath[i], _operationPath[i+1], tempOut, 1);
            tempOut = newOut;
        }

        uint256 finalOut = Uniswap_TokenToEth(_operationPath[_operationPath.length - 2], tempOut, 1);
