...
contract CollateralAndMint is Ownable {
    ...
    // Since we re-route the collateral deposits to Compound we will keep track of the TVL
    ...
    function calculateCollateralUse(uint256 _mint, uint256 _burn) private {
        ...
        if (_burn > 0) {
            collateralUsedEth[msg.sender] = collateralUsedEth[msg.sender].sub(
                _burn.mul(1e18).div(landPerEthPrice)
            );
            calculateBorrowLimit();
        }
    }
    ...
    function mintAsset(uint256 _amount) public {
        ...
        require(
            tokenPricePerEth.mul(currentBorrowLimitEth[msg.sender]).div(1e18) >=
                _amount,
    ...
    function collateralizeEth() public payable {
        ...
        cETH[msg.sender] = cETHNewBalance.add(cETHCurrentBalance).sub(
            cETHCurrentBalance
        );
