...
contract HeroNFT is IHeroNFT, ERC721, Operatable {
    ...
    uint constant SUMMON_SHARD_PRICE = 4;
    uint constant SUMMON_HER_PRICE = 80;
    uint constant MINT_PRICE = 100;

    uint16[MAX_RARITY + 1] raritysInPercent;

    constructor(address _heroVerseToken, address _heroShard, uint16[MAX_RARITY + 1] memory _raritysInPercent) ERC721("HeroVerseHero", "HERO") {
        uint16 rarityPercent;
        for (uint i = 0; i < _raritysInPercent.length; i++) {
            rarityPercent += _raritysInPercent[i];
        }
        require(rarityPercent == 10000, "Total rarity percentage not equal 100.00 %");
        heroVerseToken = _heroVerseToken;
        heroShard = _heroShard;
        raritysInPercent = _raritysInPercent;
    }
    ...
    function awakening(uint heroId) external override {
        ...
        uint awakenCost = 10 * (10 ** IERC20Metadata(heroVerseToken).decimals()); // 10 HER
    ...
    /**
     * @dev Generate a hero with 100 HER
     */
    function mintHero() external override {
        uint mintCost = MINT_PRICE * (10 ** IERC20Metadata(heroVerseToken).decimals()); // 100 HER
        require(IERC20(heroVerseToken).balanceOf(msg.sender) >= mintCost, "HeroNFT: Don't have enough HER token");

        IERC20(heroShard).transferFrom(msg.sender, address(this), mintCost);

        uint heroId = _generateHero(msg.sender, 0);
        emit MintHero(heroId);
    }

    /**
     * @dev Generate a hero with 100 HER
     */
    function mintHeroFromStaking(address account, uint8 heroRarity) external override onlyStakingPool {
        uint heroId = _generateHero(account, heroRarity);
        emit MintHero(heroId);
    }

    /**
     * @dev Generate a hero with 4 HRS Shard & 80 HER
     */
    function summonHero() external override{
        uint mintHeroShardCost = 4 * (10 ** IERC20Metadata(heroShard).decimals()); // 4 Hero Shards
        require(IERC20(heroShard).balanceOf(msg.sender) >= mintHeroShardCost, "HeroNFT: Don't have enough Hero Shard");

        uint mintHeroVerseTokenCost = 80 * (10 ** IERC20Metadata(heroVerseToken).decimals()); // 80 HER
        require(IERC20(heroVerseToken).balanceOf(msg.sender) >= mintHeroVerseTokenCost, "HeroNFT: Don't have enough HER Token");

        IERC20(heroShard).transferFrom(msg.sender, address(this), mintHeroShardCost);
        IERC20(heroVerseToken).transferFrom(msg.sender, address(this), mintHeroVerseTokenCost);
    ...
    /**
     * @dev Generate a new hero without stats
     */
    function _generateHero(address account, uint8 heroRarity) internal returns(uint heroId) {
        heroId = ++heroesCount;
        _safeMint(account, heroId);
        if (useOracleForRandomNumber == false) {
            uint256 randomNumberIndex = randomNumberByBlockInfo();
            _setHeroValueInfo(heroId, randomNumberIndex, heroRarity);
        } else {
    ...
    function _setHeroValueInfo(uint heroId, uint randomness, uint8 heroRarity) private {
        uint256 randomResult = randomness % 1000 ** 8;
        uint8[] memory numberArr =  new uint8[](8);

        for (uint i = 0; randomResult != 0; i++){
            numberArr[i] = (i == 0 || i == 1 || i == 2)
                ? uint8(randomResult % 10000)
                : uint8(randomResult % 1000);
            randomResult = randomResult / 1000;
        }

        if (heroRarity == 0) {
            while (heroRarity <= MAX_RARITY && raritysInPercent[heroRarity] <= numberArr[0]) {
                heroRarity++;
            }
        } else {
            heroRarity--;
        ...
        uint8 bgRarity = 0;
        while (bgRarity <= MAX_RARITY && raritysInPercent[bgRarity] <= numberArr[1]) {
            bgRarity++;
        }

        uint8 petRarity = 0;
        while (petRarity <= MAX_RARITY && raritysInPercent[petRarity] <= numberArr[2]) {
            petRarity++;
        }

        heroes[heroId].tokenId = heroId;
        heroes[heroId].element = numberArr[6] % (MAX_ELEMENT + 1);
        heroes[heroId].coinSignal = numberArr[7] % (MAX_COIN_SIGNAL + 1);

        heroes[heroId].heroRarity = heroRarity;
        heroes[heroId].bgRarity = bgRarity;
        heroes[heroId].petRarity = petRarity;

        heroes[heroId].heroRarityIdx = numberArr[3] % (heroRarityIdxMax[heroRarity] + 1);
        heroes[heroId].bgRarityIdx = numberArr[4] % (bgRarityIdxMax[bgRarity] + 1);
        heroes[heroId].petRarityIdx = numberArr[5] % (petRarityIdxMax[petRarity] + 1);
    ...
    function createHeroFromRNG(uint heroId, uint randomness) external override onlyRandomNumberGenerator {
        _setHeroValueInfo(heroId, randomness, 0);
    }
    ...
    function setStakingPool(address _stakingPool) external onlyOwner {
        ...
        emit UpdateRandomNumberGenerator(stakingPool);
    }
     ...
     * @param isUseOracle Use Oracle if true, use on contract RNG if false
     */
     ...
     * @dev .
     */
    function randomNumberByBlockInfo() private returns(uint256)
    {
...
interface IHeroNFT {
    ...
    event MintHero(uint heroId);
    event SummonHero(uint heroId);
    ...
    /**
     * @notice create new hero from staking
     */
    function mintHeroFromStaking(address user, uint8 heroRarity) external;
