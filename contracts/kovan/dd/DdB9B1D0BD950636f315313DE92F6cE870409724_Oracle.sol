...
contract Oracle is IOracle, Ownable
{
  ...
  function getRate (address from, address to) public view override returns (uint256)
  {
    ...
    uint256 srcRate = from == _WETH ? _DECIMALS : _calcTokenETHRate(from);
    uint256 destRate = to == _WETH ? _DECIMALS : _calcTokenETHRate(to);

    require(srcRate > 0 && destRate > 0, "no oracle");


    return (srcRate * _DECIMALS) / destRate;
  }
  ...
  function _calcDestQty (uint256 srcQty, address from, address to, uint256 rate) private view returns (uint256)
  {
    ...
    if (destDecimals >= srcDecimals)
    {
      ...
      return (srcQty * rate * difference) / _DECIMALS;
    }
    else
    {
      ...
      return (srcQty * rate) / (_DECIMALS * difference);
    }
  }
