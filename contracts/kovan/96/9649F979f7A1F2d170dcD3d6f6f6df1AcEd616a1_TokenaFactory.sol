...
contract TokenaFactory is ITokenaFactory, Ownable, ReentrancyGuard {
	...
	mapping(address => bool) public promotedPools;
	...
	function createStaking(
		...
		IStaking(address(newStaking)).initialize(stakedToken, rewardToken, rewardTokenAmounts, startBlock, endBlock, msg.sender);
		if (isLM) {
	...
	function changePromotedPool(address pool, bool flag) external onlyOwner {
		require(pool != address(0), "Must be valid address");
		promotedPools[pool] = flag;
	}
	 ...
	 */
	function checkValidLpAddress(address adr) internal view returns (bool) {
