 ...
 * At the end of the duration, the total reward amount is distributed
 * Rewards can be escrowed after unstaking
 */
    ...
    ReentrancyGuard
{
    ...
    uint256 public rewardRate = 0; // rate of the rewards
    ...
    bool rewardsAreEscrowed; // True if rewards are escrowed in RewardEscrow after unstaking
    ...
    /**
     * Get reward per staked token rate
     */
     ...
     */
    function claimReward(address sender) public override {
        ...
        if (reward > 0) {
            ...
            if (rewardsAreEscrowed) {
                ...
                rewardsToken.safeTransfer(sender, reward);
            }
        }
    }
     ...
     * Get token reward
     * @param sender address to unstake and claim rewards for
     */
    function exit(address sender) public override {
        unstakeRewards(_stakedBalances[sender], sender);
        claimReward(sender);
    }
     ...
     * If an address stakes or unstakes, update his accumulated rewards
     * Called on stake, unstake and initialization of the rewards
     */
...
interface IStakingRewards is IRewardDistributionRecipient {
    // Views
    ...
    function stakeRewards(uint256 amount, address sender) external;

    function unstakeRewards(uint256 amount, address sender) external;

    function exit(address sender) external;
