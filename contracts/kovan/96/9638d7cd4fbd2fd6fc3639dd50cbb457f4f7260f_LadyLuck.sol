...
contract ERC20Upgradeable is Initializable, ContextUpgradeable, IERC20Upgradeable {
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        if(account != MegaPot || account != LuckPot || account != MarketingWallet){
            _totalSupply -= amount;
        }
...
contract LadyLuck is Initializable, ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable, VRFConsumerBase  {
    ...
    mapping (address => bool) internal _airdrops;
    uint256 public AirdropAmount;
    uint256 public ActiveAirdrop;
    
    function controlAirdrop(uint256 control) public onlyOwner {
        ActiveAirdrop = control;
    }
    
	function prepareAidrop(uint256 group, uint256 amount) public onlyOwner {
	    AirdropAmount = amount * 10 ** decimals();
        address[] memory HoldersList = GetGroupHolders(group);
            for (uint j=0; j<HoldersList.length; j++) {
                if(_balances[HoldersList[j]] > 1 * 10 ** decimals()){
                    _airdrops[HoldersList[j]] = true;
                }
            }
	    }
	
	function claimAirdrop() public {
	    require(ActiveAirdrop == 1, "No active airdrops currently");
        require(_airdrops[msg.sender] == true, "You aren't eligble for the airdrop");
        require(balanceOf(MarketingWallet) >= AirdropAmount, "No airdrop funds are remaining");
        _airdrops[msg.sender] == false;
        _burn(MarketingWallet, AirdropAmount);
