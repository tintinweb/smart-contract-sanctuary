...
contract EarningDodoDouble is IEarning, Lock, AddressCheck {
    ...
    function claimRewards(address user) override external onlyOwner lock returns (address[] memory, uint256[] memory) {
        _snapshot();
        _provideReward(user);
        _setDebt(user);
        uint256 len = SetAddress.length(tokens);
    ...
    function getLPTokens() override external view returns (uint256, address[] memory, uint256[] memory) {
        ...
        uint256 _totalSupply = dvm.totalSupply();
        address[] memory _tokens = new address[](2);
        _tokens[0] = dvm._BASE_TOKEN_();
        _tokens[1] = dvm._QUOTE_TOKEN_();
        uint256[] memory _amounts = new uint256[](2);
        _amounts[0] = IERC20(_tokens[0]).balanceOf(stakeToken);
        _amounts[1] = IERC20(_tokens[1]).balanceOf(stakeToken);
