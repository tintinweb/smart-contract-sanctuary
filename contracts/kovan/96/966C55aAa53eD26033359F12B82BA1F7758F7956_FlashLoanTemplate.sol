...
// WORKED FLASHLOAN ON KOVAN!!!
// MUST TRANSFER WETH TO CONTRACT FIRST TO PAY FEES!
...
contract FlashLoanTemplate is ICallee {
    ...
    //IWETH private WETH = IWETH(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2);     //mainnet
    IWETH private WETH = IWETH(0xd0A1E359811322d97991E03f863a0C30C2cF029C);     //kovan
    ...
    //ISoloMargin private soloMargin = ISoloMargin(0x1E0447b19BB6EcFdAe1e4AE1694b0C3659614e4e);        //mainnet
    ISoloMargin private soloMargin = ISoloMargin(0x4EC3570cADaAEE08Ae384779B0f3A45EF85289DE);        //kovan 
    ...
    constructor() {
        ...
        WETH.approve(address(soloMargin), uint256(0xffffffffffffffffffffffffffffffff));
    }
    ...
    function callFunction(address sender, Account.Info memory accountInfo, bytes memory data) external override {
        ...
        // We now have a WETH balance of loanAmount (+10000000000). The logic for what we
        ...
        WETH.withdraw(WETH.balanceOf(address(this)) - loanAmount - 10000000000);		
		

        // Send any profit in ETH to the account that invoked this transaction
        actualSender.transfer(address(this).balance - 10000000000);
		// RYAN NOTE: the -10000000000 leaves 10000000000 wei WETH in the contract so it can perform its next flashloan
    }
}