...
contract FREther is FRToken {
     ...
     */
    constructor(IFController controller_,
                InterestRateModel interestRateModel_,
                string memory name_,
                string memory symbol_,
                uint8 decimals_,
        ...
        initialize(controller_, interestRateModel_, name_, symbol_, decimals_,slope1,slope2,optimalUtilisationRate_);
     ...
     */
    function mint(uint period) external payable {
        (uint err,) = mintInternal(msg.value, period);
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeem(uint supplyId) external returns (uint) {
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemUnderlying(uint depositId) external returns (uint) {
        return redeemUnderlyingInternal(depositId);
    }
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function borrow(uint borrowAmount, uint period) external returns (uint) {
     ...
     * @dev Reverts upon any failure
     */
    function repayBorrow(uint borrowId) external payable {
     ...
     * @param borrower the account with the debt being payed off
     */
    function repayBorrowBehalf(address borrower, uint borrowId) external payable {
        (uint err,) = repayBorrowBehalfInternal(borrower, borrowId);
    ...
    function getCashPrior() internal override view returns (uint)  {
        uint startingBalance = subUInt(address(this).balance, msg.value);
