...
contract CollectiveFactory is ICollectiveFactory {
    ...
    constructor(
        ...
        require(_priceOralceAggregator != address(0), 'CLFAC: INVALID_AGGREGATOR');
        require(_protocolFee > 0 && _protocolFee <= 10000, 'CLFAC: INVALID_FEE');
        require(_feeTo != address(0), 'CLFAC: INVALID_FEETO');
        require(_collectiveImp != address(0), 'CLFAC: INVALID_COLLECTIVE_ADDR');
        require(_cwTokenImp != address(0), 'CLFAC: INVALID_CWTOKEN_ADDR');

        priceOralceAggregator = IPriceOracleAggregator(_priceOralceAggregator);
        owner = _owner;
        collectiveImp = _collectiveImp;
        protocolData = DataTypes.ProtocolData(_cwTokenImp, _protocolFee, _feeTo, 10000);
    ...
    function createCollective(
        ...
        for (uint256 i = 0; i < length; i++) {
            require(isAcceptedToken[_acceptedTokens[i]], 'CLFAC: NOT_PROTOCOL_ACCEPTED_TOKEN');
            require(address(_strategies[i]) != address(0), 'CLFAC: INVALID_STRATEGY');
            priceOralceAggregator.viewPriceInUSD(_acceptedTokens[i]);
        }

        // calc collective Hash
        bytes32 pHash = keccak256(abi.encodePacked(msg.sender, _beneficiary, _name));
        ...
        ICollective(newCollective).initialize(metaData, _nominations, _acceptedTokens, _strategies);
    ...
    /// @notice add accepted Tokens
    function addAcceptedTokens(address[] memory _tokens) external override onlyOwner {
    ...
    function getProtocolData() external view override returns (DataTypes.ProtocolData memory) {
        return protocolData;
    }
    ...
    function setCWTokenImpl(address _cwToken) external override onlyOwner {
        require(_cwToken != address(0), 'CLFAC: INVALID_CWTOKEN_IMPL');
...
interface ICollectiveFactory {
    ...
    function allCollectives(uint256) external view returns (address);
...
interface ICollective {
    ...
    function getDefaultStrategy(address _token) external view returns (IStrategy);
    ...
    function updateTokenStrategy(address _token, IStrategy _newStrategy) external returns (bool);
}
