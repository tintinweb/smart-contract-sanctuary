...
contract OracleBinance {
    using SafeMath for uint;
    using FixedPoint for *;

    uint public constant PERIOD = 0;

    IPancakeSwapV2Pair immutable pair;
    ...
    constructor(address factory, address tokenA, address tokenB) public {
        IPancakeSwapV2Pair _pair = IPancakeSwapV2Pair(PancakeSwapV2Library.pairFor(factory, tokenA, tokenB));
        pair = _pair;
        token0 = _pair.token0();
        token1 = _pair.token1();
    ...
    function update() internal {
        (uint price0Cumulative, uint price1Cumulative, uint32 blockTimestamp) =
            PancakeSwapV2OracleLibrary.currentCumulativePrices(address(pair));
