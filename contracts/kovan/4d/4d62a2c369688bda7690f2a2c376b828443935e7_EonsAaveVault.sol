...
contract EonsAaveVault is OwnableUpgradeable {
  ...
  function totalStaked(uint _pid) public view returns (uint256) {
  ...
  function pendingRewardDevFeeOf(uint _pid, address _user) public returns (uint256) {
    UserInfo storage user = userInfo[_user][_pid];
    PoolInfo storage pool = poolInfo[_pid];
    uint256 userShareOfPool = user.amount.mul(1e12).div(pool.totalStaked);
    uint256 totalStaked = router.totalStakedOf(_pid);
    uint256 userStaked = totalStaked.mul(userShareOfPool).div(1e12);
  ...
  function pendingRewardOf(uint256 _pid, address _user) public view returns (uint256) {
    UserInfo storage user = userInfo[_user][_pid];
    PoolInfo storage pool = poolInfo[_pid];
    uint256 userShareOfPool = user.amount.div(pool.totalStaked).mul(1e12);
    uint256 totalStaked = router.totalStakedOf(_pid);
    uint256 userStaked = totalStaked.mul(userShareOfPool).div(1e12);
  ...
  function depositETH() external payable {
    ...
    (address aToken, address reserve, uint256 income) = router.getAsset(1);
  ...
  function withdraw(uint _amount, uint256 _pid) external {
    PoolInfo storage pool = poolInfo[_pid];
    UserInfo storage user = userInfo[msg.sender][_pid];
    require(_amount <= IEonsETH(pool.eToken).balanceOf(msg.sender), 'Withdraw not good');
  ...
  function updateEmissionDistribution() public {
    ...
    for (uint i = 0; i < poolInfo.length; i++) {
      ...
      uint totalSupply = IEonsETH(pool.eToken).totalSupply();
      if (totalSupply > 0) {
        pool.accEonsPerShare = pool.accEonsPerShare.add(eonsBalance).mul(pool.allocPoint).div(totalAllocPoint).div(totalSupply);
      }
    }
  }
}
