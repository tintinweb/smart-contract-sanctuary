...
contract WrappedNFT is ERC20 {
    ...
    function unwrap(uint256 _amount) public {
        require(!isContract(msg.sender), "WrappedNFT: Address must not be a contract.");
        unwrapFor(_amount, msg.sender);
    }

    function unwrapFor(uint256 _amount, address _recipient) public {
        require(!isContract(_recipient), "WrappedNFT: Recipient must not be a contract.");
    ...
    function onERC721Received(address _from, uint256 _tokenId, bytes calldata _data) external view returns (bytes4) {
        ...
        return WrappedNFT.onERC721Received.selector;
    }
}