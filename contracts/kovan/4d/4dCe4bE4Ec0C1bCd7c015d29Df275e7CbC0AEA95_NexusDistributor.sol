    ...
    ReentrancyGuardUpgradeable
{
  ...
  */
    ...
    function __NexusDistributor_init (
        ...
        buysAllowed = true;
        feePercentage = 0;
        treasury = 0x8B13f183e27AaD866b0d71F0CD17ca83A9a54ae2;
        master = INXMaster(0x2561D7f2436C121281388ecd54c702e55Aa24043);
        gateway = IGateway(master.getLatestAddress("GW"));
        nxmToken = IERC20Upgradeable(master.tokenAddress());
    }
   ...
   function buyCover(
        ...
        uint256 buyCoverValue = _defineBuyingCoverValue(
                             coverAsset,
                             msg.value,
                             coverPriceWithFee,
                             coverPrice,
    ...
    function _defineBuyingCoverValue(
        address coverAsset,
        uint256 msgValue,
        uint256 coverPriceWithFee,
        uint256 coverPrice,
        bytes calldata data
    ) internal returns(uint256 _buyCoverValue){
        
        (uint256 price, uint256 priceInNXM ) = abi.decode(data,(uint256, uint256 ));
        uint256 testTotal = price.add(priceInNXM);

        if (coverAsset == ETH) {
            require(msgValue >= coverPriceWithFee, "Distributor: Insufficient ETH sent");
            uint256 remainder = msgValue.sub(coverPriceWithFee);
            ...
            _buyCoverValue = coverPrice;

        } else {

            IERC20Upgradeable(coverAsset).safeTransferFrom(
                msg.sender,
                address(this),
                coverPriceWithFee
            );
            IERC20Upgradeable(coverAsset).approve(address(gateway), coverPrice);
        }
    ...
    function setwNXMToken(address _tokenAddress) external onlyOwner{
