...
// PoolToken contracts allow for different tokenized carbon credits to be pooled
...
contract PoolToken is Context, ERC20, Ownable, Pausable {

    // Initial supply = 0
    uint256 public _totalSupply = 0;
    address public contractRegistry;
    bool public isBaseCarbonPool;
    ...
    // Makes this pool accept any CO2KEN project
    function makebaseCarbonPool() public onlyOwner {
        isBaseCarbonPool=true;
    }
    ...
    // Whitelist selected ERC20 compliant tokenized carbon
    function addToWhiteList(address erc20Addr) public onlyOwner {
        require(isWhiteListPool, "Pool does not accept external contracts");
        whiteList[erc20Addr]=true;
    }
    ...
    function deposit(address erc20Addr, uint256 amount) public whenNotPaused {
        console.log("DEBUG sol: deposit(erc20Addr, amount):", erc20Addr, amount);

        // Control-Flow for Filtering not finalized nor tested
        if (isBaseCarbonPool==true) {
            require(IContractRegistry(contractRegistry).checkERC20(erc20Addr)==true, "pERC20 not official");
        }
        // If not baseCarbon, accept based on attribute sets
        else {
            require(IContractRegistry(contractRegistry).checkERC20(erc20Addr)==true, "pERC20 not official");
            require(checkAttributeMatching(erc20Addr)==true, "The token sent is not accepted");
        }

        if (isWhiteListPool==true) {
            require(whiteList[erc20Addr]==true, "Token not whitelisted");
        }
    ...
    // Redeems Pool tokens for multiple underlying pERC20s 1:1
    // User specifies in front-end the addresses and amounts they want
    // Pool token in User's wallet get burned
    function redeemMany(address[] memory erc20s, uint256[] memory amounts) public whenNotPaused {
        // require(overallAmount >= amount, "Cannot redeem more than is stored in contract");
     ...
     function redeemSingle(address account, address erc20, uint256 amount) public whenNotPaused {
        require(tokenBalances[erc20] >= amount, "Cannot redeem more than is stored in contract");
    ...
    function offset(uint256 amount) public whenNotPaused {}
    ...
    function checkAttributeMatching(address erc20Addr) public view returns (bool) {
        ...
        string memory m, 
        string memory r
        ) = ProjectERC20(erc20Addr).getAttributes();
