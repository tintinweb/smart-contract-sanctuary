...
contract OriginatorStaking is Initializable, StakingRewards, BaseTokenUpgradeable, IStaking, IProjectFundedRewards, IOriginatorManager {
    ...
    function stake(address _onBehalfOf, uint256 _amount) external override notZeroAmount(_amount) onlyOnStakingState {
        require(!this.hasReachedGoal(), 'GOAL_HAS_REACHED');
     ...
     */
    function changeDistributionEndDate(uint256 _distributionEndDate) external override onlyEmissionManager onlyOnStakingState {
    ...
    /**
     * @notice Check if fulfilled the objective
     */
    function hasReachedGoal() external override notZeroAmount(stakingGoal) view returns (bool) {
    ...
    /**
     * @notice Function to save DISTRIBUTION_END if timestamp is less than the initial one
     **/
    function _endDistributionIfNeeded() internal {
        if (block.timestamp <= DISTRIBUTION_END) {
            DISTRIBUTION_END = block.timestamp;
            emit DistributionEndChanged(DISTRIBUTION_END);
        }
    }
}
...
interface IOriginatorManager {
    ...
    function changeDistributionEndDate(uint256 distributionEndDate) external;
