...
contract NFTMarket is IMarket, ReentrancyGuard, TwoStageOwnable, ChainlinkClient {
    ...
    function fetchAllMarketItems(uint256 skip, uint256 limit) external view returns (MarketItem[] memory marketItems) {
        if (skip > _itemIds) return new MarketItem[](0);
        uint256 to = skip + limit;
        if (_itemIds < to) to = _itemIds;
        uint256 itemsLength = 0;
        if (_lastItemId > 0 && skip <= _lastItemId) {
            marketItems = _fromMarket.fetchAllMarketItems(skip, limit);
            itemsLength = marketItems.length;
        } else {
            marketItems = new MarketItem[](to - skip);
        }
        for (uint256 i = 0; i < to - skip; i++) {
            marketItems[i + itemsLength] = _idToMarketItem[skip + i + 1];
        }
    }
    ...
    function fetchMarketItemsByTokenId(
        ...
        uint256 marketItemsLength = marketItemsByTokenId.length;
        if (skip >= marketItemsLength) return new MarketItem[](0);
        uint256 to = skip + limit;
        if (marketItemsLength < to) to = marketItemsLength;
        uint256 itemsLength = 0;
        if (_lastItemId > 0 && skip <= _lastItemId) {
            marketItems = _fromMarket.fetchMarketItemsByTokenId(tokenId, skip, limit);
            itemsLength = marketItems.length;
        } else {
            marketItems = new MarketItem[](to - skip);
        }
        for (uint256 i = 0; i < to - skip; i++) {
            marketItems[i + itemsLength] = _idToMarketItem[marketItemsByTokenId[skip + i]];
        }
    }

    function fetchSpecificMarketItem(uint256 itemId) public view returns (MarketItem memory marketItem) {
        if (itemId > 0 && itemId <= _lastItemId) return _fromMarket.fetchSpecificMarketItem(itemId);
        marketItem = _idToMarketItem[itemId];
    }
    ...
    function fromMarket() external view returns (IMarket) {
        return _fromMarket;
    }

    function toMarket() external view override returns (IMarket) {
        return _toMarket;
    }
    ...
    function idTokenToMarket(uint256 tokenId) external view returns (uint256) {
        return _idTokenToMarket[tokenId];
    }
    ...
    function marketItemsByTokenIdLength(uint256 tokenId) external view returns (uint256 length) {
    ...
    function nftContract() external view returns (INFT) {
        return _nftContract;
    }
    ...
    constructor(
        ...
        require(oracle_ != address(0), "Oracle is zero address");
        require(linkToken_ != address(0), "Link token is zero address");
        require(nupayWalletAddress_ != address(0), "Nupay wallet is zero address");
    ...
    function createMarketSale(uint256 itemId) external payable whenNotStopped nonReentrant returns (bool) {
        ...
        require(_idTokenToMarket[item.tokenId] == 0, "Token already payed");
    ...
    function initialize(address market) external whenNotStopped onlyOwner returns (bool) {
        ...
        _itemIds = marketContract.lastItemId();
    ...
    function migrate(address market) external onlyOwner returns (bool) {
        require(market != address(0), "Market is zero address");
        require(address(_toMarket) == address(0), "Contract already migrated");
        _nftContract.pause();
        _nftContract.setMarket(market);
        _stopped = true;
        payable(market).transfer(address(this).balance);
        _linkToken.transfer(market, _linkToken.balanceOf(address(this)));
    ...
    function resolveDeal(
        ...
        uint256 itemId = _idTokenToMarket[tokenId];
        MarketItem memory item = fetchSpecificMarketItem(itemId);
...
interface IMarket {
    ...
    function toMarket() external view returns (IMarket);
