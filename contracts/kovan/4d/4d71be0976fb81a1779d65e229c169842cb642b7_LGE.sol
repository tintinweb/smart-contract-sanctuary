...
contract LGE is Ownable, LivePrice {
    ...
    constructor() {
      ...
      preLGE1 = PreLGEInterface(0xd9D314896F239b1B49Ab5Fc983576e2cD7982b0C);
      preLGE2 = PreLGEInterface(0x2aAD9f2BC3B59d06Fa9deC10f75cBbc5cA24F65f);
      ...
      decimals = 10**18;
      DAI = address(0xC4375B7De8af5a38a93548eb8453a498222C4fF2);
      maxMintableSWN = decimals.mul(5000000);
      DaiToken = TokenInterface(DAI);
      BalancerTrader = address(0xE7e70A9b773171D14D8b4B9aCB77287Ac7fbe62F);
    ...
    function pushLiquidity(address liquidityToken, uint256 amountLP) internal {
        ...
        uint256 totalLGEScore = getTotalScore().add(preLGE1.getTotalScore().add(preLGE2.getTotalScore()));
    ...
    function concatenateInvestorArrays() internal returns(address[] memory, uint256[] memory){
      ...
      uint256 length3 = investor.length;
      ...
      for (uint256 s = 0; s < length3; s += 1){
        investorArray[length1 + length2 + s] = investor[s];
        scoreArray[length1 + length2 + s] = score[investor[s]];
      }
