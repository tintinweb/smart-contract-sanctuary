...
contract Comptroller is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    function setCompSpeedInternal(PToken pToken, uint256 compSpeed) internal {
        ...
        if (currentCompSpeed != 0) {
            ...
            updateCompSupplyIndex(address(pToken));
            updateCompBorrowIndex(address(pToken), borrowIndex);
        } else if (compSpeed != 0) {
            // Add the PUBL market
            Market storage market = markets[address(pToken)];
     ...
     */
    function updateCompSupplyIndex(address pToken) internal {
     ...
     */
    function updateCompBorrowIndex(address pToken, Exp memory marketBorrowIndex) internal {
