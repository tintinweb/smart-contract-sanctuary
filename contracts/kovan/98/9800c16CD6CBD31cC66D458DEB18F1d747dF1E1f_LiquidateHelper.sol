        ...
        for (uint i = 0; i < owners.length; i++) {
            ...
            if (trade.marketValueOpen > marketValueCurrent) {// losing
                ...
                if (trade.depositFixedValue >= pnl) {
                    results[i] = Vars((uint128)(trade.depositFixedValue.sub(pnl).mul(10000).div(borrowed)), vars.marginRatio, trade.liqBlockNum == 0 ? 1 : 2);
                    continue;
                } else {
                ...
                results[i] = Vars((uint128)(trade.depositFixedValue.add(pnl).mul(10000).div(borrowed)), vars.marginRatio, trade.liqBlockNum == 0 ? 1 : 2);
                continue;
            }
        }
        return results;
