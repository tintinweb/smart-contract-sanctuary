...
contract Vault is TransferHelper, Ownable, ReentrancyGuard, ERC20 {

  uint private constant DISTRIBUTION_PERIOD = 45_800; // ~ 7 days

  IVaultRebalancer public rebalancer;
  ...
  // Call this only if the vaut doesn't have enough funds in the buffer
  ...
  function setRebalancer(IVaultRebalancer _value) external onlyOwner {
    rebalancer = _value;
    emit NewRebalancer(address(_value));
  }
  ...
  function _deposit(address _account, uint _amount) internal {
    require(depositsEnabled, "Vault: deposits not allowed");
    claim(_account);
