...
contract Synthetic is Ownable {
        ...
        external
    {
        ...
        if (_amount == mn.assetAmount) {
            ...
            uint256 assetBackedAtRateAmount =
                (assetBackedAmount * exchangeRate) / denominator; // 606872500000000000000

            uint256 requiredAmount =
                (assetBackedAtRateAmount * collateralRatio) / denominator;
            require(
                assetBackedAmount >= requiredAmount,
                "Synthetic::redeemSynthetic: under collateral"
            );
            uint256 canWithdrawRemainning = assetBackedAmount - requiredAmount;

            mn.assetAmount = mn.assetAmount - assetToBeBurned;
            mn.assetBackedAmount = assetBackedAmount;
            mn.currentRatio =
