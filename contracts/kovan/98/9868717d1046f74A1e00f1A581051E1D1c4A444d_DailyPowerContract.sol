...
contract DailyPowerContract is Ownable, ChainlinkClient {
        ...
        returns (bytes32 requestId)
    {
        ...
        request.add(
            "get",
            "https://neow2uke1e.execute-api.ca-central-1.amazonaws.com/default/getAesoDailyFlatPoolPrice?date=20220111"
        );
    ...
    function calculateSettlementAmounts() public onlyOwner {
        ...
        uint256 adjustedCadToUsdPrice = uint256(latestCadToUsdPrice) * 10**10; // it comes with 8 decimals already, so add 10 to get to 18 decimals
        ...
        // convert usd to eth
        (, int256 latestEthToUsdPrice, , , ) = ethUsdPriceFeed
            .latestRoundData();
        uint256 adjustedEthToUsdPrice = uint256(latestEthToUsdPrice) * 10**10; // it comes with 8 decimals already, so add 10 to get to 18 decimals
