...
contract CurveLendingPool3Zap is Ownable, BaseRelayRecipient {
    ...
    function _deposit(address _vault, uint256 _amount, address _coin, bool _stake) private returns (uint256 _daoERNBal) {
        ...
        uint256 _lpTokenBal;
        if (_coin == address(_coins[2]) || _coin == address(_coins[1]) || _coin == address(_coins[0])) {
            if (_coin == address(_coins[2])) {
                _amounts[2] = _amount;
            } else if (_coin == address(_coins[1])) {
                _amounts[1] = _amount;
            } else { // address(_coins[0])
                _amounts[0] = _amount;
            }
            _lpTokenBal = _curvePool.add_liquidity(_amounts, 0, false);
        } else if (_coin == address(_underlying[2]) || _coin == address(_underlying[1]) || _coin == address(_underlying[0])) {
            if (_coin == address(_underlying[2])) {
                _amounts[2] = _amount;
            } else if (_coin == address(_underlying[1])) {
                _amounts[1] = _amount;
            } else { // address(_underlying[0])
                _amounts[0] = _amount;
            }
