...
contract BaseProxy is Ownable {
    ...
    /**
     * @param agentManagerAddress: The agent manager address to connect to
     */
    constructor(address agentManagerAddress) {
 ...
 */
contract SwapProxy is BaseProxy, ReentrancyGuard {
     ...
     */
    constructor(address uniswapRouterAddress, address agentManagerAddress)
        BaseProxy(agentManagerAddress)
    {
    ...
    function swap(
        ...
        address[] calldata path,
        uint256 slippage
    ) external nonReentrant agentVerified(agentId, user) returns (bool) {
        ...
        tokenIn.safeApprove(address(uniswapRouter), balance);

        uint256[] memory amounts = uniswapRouter.getAmountsOut(balance, path);
        
        uint256 amountOutMin = amounts[amounts.length - 1].sub(
            amounts[amounts.length - 1].mul(slippage).div(100)
        );
        
        return _swap(user, path, balance, amountOutMin);
    }
