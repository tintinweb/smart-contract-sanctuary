...
/*
    1.1.5 (0x985d0fbe07C855aa5FB20a2fc37709B1713d79c0)
    1. added currentAPR()
*/
...
contract Spark is Ownable, OwnableExt {
    ...
    /**
     * @notice computer current APR
     * @param stakedTimeStamp timestamp of deposit creation time
     * @return current APR (precision = APR_PREC, no %)
     */
    function currentAPR(uint stakedTimeStamp) public view returns (uint) {
        require(stakedTimeStamp < now, "currentAPR: the deposit made in the future.");
        require(stakedTimeStamp > startTimeStamp, "currentAPR: the deposit made before the program started.");
        uint endTimeNoExt = startTimeStamp + 29 days;
        uint endTimeExt = endTimeNoExt + extDays * (1 days);
        require(stakedTimeStamp <= endTimeExt, "currentAPR: the deposit made after the program has ended.");
        if (now > endTimeExt) {
            return 0;
        }
        if (now > endTimeNoExt) {
