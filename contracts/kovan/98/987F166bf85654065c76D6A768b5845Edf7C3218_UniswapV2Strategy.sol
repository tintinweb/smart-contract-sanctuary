...
import '../interfaces/IUniswapV2Worker.sol';

import "hardhat/console.sol";

contract UniswapV2Strategy is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IStrategy {
  ...
  modifier onlyWhitelistedWorkers() {
    require(okWorkers[msg.sender], "UniswapV2Strategy::onlyWhitelistedWorkers:: bad worker");
    _;
  } 
  ...
  function setWhitelistTokens(address[] calldata tokens, bool[] calldata statuses)
    ...
    onlyOwner
  {
    require(tokens.length == statuses.length, 'UniswapV2Strategy::tokens & statuses length mismatched');
  ...
  function initialize(IUniswapV2Router02 _router, address _dao) public initializer {
  ...
  /// @dev Execute worker strategy. Take BaseToken + FarmingToken. Return Farming tokens.
  ...
  nonReentrant
  {
    ...
    console.log('UniswapV2Strategy::baseToken:',baseToken);
    console.log('UniswapV2Strategy::farmingToken',farmingToken);
    console.log('UniswapV2Strategy::farmingTokenAmount:',farmingTokenAmount);
    console.log('UniswapV2Strategy::minFarmingAmount',minFarmingAmount);

    require(whitelistedTokens[baseToken], 'token not whitelisted');
    require(whitelistedTokens[farmingToken], 'token not whitelisted');

    
    if(farmingTokenAmount > 0){
        IUniswapV2Worker worker = IUniswapV2Worker(msg.sender);
        IBank bank = IBank(worker.operator());
        bank.requestFunds(farmingToken, farmingTokenAmount);
    }
    uint256 balance = baseToken.myBalance();
    // baseToken.safeApprove(address(router), uint256(-1));
    baseToken.safeApprove(address(router), balance);
    if(balance > 0){
      if(DAO != address(0)){
        SafeToken.safeTransfer(baseToken, DAO, balance.mul(fee).div(10000));
        balance = baseToken.myBalance();
      }
    ...
    require(farmingTokenBalance >= minFarmingAmount, "UniswapV2Strategy::execute:: insufficient farming tokens received");
    SafeToken.safeTransfer(farmingToken, msg.sender, farmingTokenBalance);
