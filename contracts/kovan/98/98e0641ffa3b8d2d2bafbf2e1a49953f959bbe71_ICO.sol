...
contract ICO {
    ...
    uint public initialValue = 30000000;
    uint public secondValue = 50000000;
    uint public finalValue;
    //string phase;

    bool public released;

    enum Phase {Initial,Second,Final}
    Phase public phase;
    ...
    function setFinalValue(uint256 _finalValue) external onlyAdmin() {
      finalValue = _finalValue;
    }

    function setSalePhase(uint256 _phase) external onlyAdmin() {
      if(_phase == 0)
        phase = Phase.Initial;
      else if(_phase == 1)
        phase = Phase.Second;
