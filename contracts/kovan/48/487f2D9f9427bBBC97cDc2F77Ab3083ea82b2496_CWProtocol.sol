...
interface ICWProject {
    ...
    function back(address token, uint value) external payable returns (bool);
    function withdraw() external returns (bool);
    function redeemBToken(address token, uint valueToRemove) external payable returns (bool);
    function redeemCToken(address token, uint valueToRemove) external payable returns (bool);
    ...
    function removeAcceptedTokens(address[] memory _nomination) external;
...
contract CWToken is ERC20PresetMinterPauser, ICWToken {
    ...
    constructor(address _token, bool _isBToken) ERC20PresetMinterPauser(
        string(abi.encodePacked("CW ", isBToken(_isBToken), ERC20(_token).name(), " Token")),
        string(abi.encodePacked(isBToken(_isBToken), ERC20(_token).symbol))
    ) public {}

    function isBToken(bool _isBToken) public returns (string memory) {
        if (_isBToken) { return B; }
        else { return C; }
    }

    function approveOtherContract(IERC20 token, address recipient, uint256 amount) external override {
...
contract CWProject is ICWProject {
    ...
    constructor(
        ...
        creator = _creator;
        acceptedTokenAddresses = _tokens;
    ...
    function back(address token, uint value) external override payable returns (bool) {
        require(acceptedTokens[token], 'Requires backing with accepted token');
        require(value > 0, 'Requires non-zero backing amount');
        require(block.timestamp < deadline, 'Not able to back, deadline is passed');

        if(isProtocolVersion1()){
            // require(msg.value > 0, 'Requires non-zero balance');
            // require(msg.value > value, 'Requires set available baking amount'); // is this needed?

            address backingDepositTo = getDepositAddress(token, true);
            ...
            IERC20(token).transferFrom(msg.sender, backingDepositTo, value);
    ...
    function curate(address token, uint value) external payable returns (bool) {
        require(acceptedTokens[token], 'Requires curating with accepted token');
        require(value > 0, 'Requires non-zero curating amount');
        require(block.timestamp < deadline, 'Not able to curate, deadline is passed');

        if(isProtocolVersion1()){
            // require(msg.value > 0, 'Requires non-zero balance');
            // require(msg.value > value, 'Requires set available curating amount');

            address curatingDepositTo = getDepositAddress(token, false);
            
            // mints the same amount of token to the msg.sender
            CWToken(cToken).mint(msg.sender, value);

            // increment amounts from curator and for total project funding
            // _mintFee(value); // ignore for a little while. It was how we managed the "protocol"-level fee.

            // send curate fee to deposite
            IERC20(token).transferFrom(msg.sender, curatingDepositTo, value);
        }
    ...
    function withdraw() public override returns (bool) {
        ...
        require(block.timestamp >= deadline, 'Not able to withdraw or change parameters, they are locked in');
        ...
        uint withdrawablePercent = 100 - curatorFee - ICWProtocol(factory).protocolFee();
        ...
        emit Withdraw(msg.sender, totalFunding * withdrawablePercent / 100);
        return true;
    }

    function redeemBToken(address token, uint valueToRemove) public payable override returns (bool) {
        require(valueToRemove > 0, 'Requires non-zero redeem amount');
        require(block.timestamp >= deadline, 'Not able to redeem, they are locked in');

        require(CWToken(bToken).balanceOf(msg.sender) > 0, 'Backers can only call this method');
        require(CWToken(bToken).balanceOf(msg.sender) >= valueToRemove, 'Transfer amount exceeds your backed balance');


        if (funded) {
            return false;   // when project funding is successed
        }
        
        // redeem logic: redeem except fundingFailureFee(protocolFee)
        uint redeemablePercent = 100 - ICWProtocol(factory).protocolFee();

        if(isProtocolVersion1()){
            address backingDepositTo = getDepositAddress(token, true);

            // reclaim underlying collateral except fundingFailureFee
            uint256 removeValueToBacker = (valueToRemove.mul(redeemablePercent)).div(100);  // valueToRemove * (redeemablePercent / 100);
            IERC20(token).transferFrom(backingDepositTo, msg.sender, removeValueToBacker);

            // send fundingFailureFee to protocol
            uint256 fundingFaliureFee = (valueToRemove.mul(ICWProtocol(factory).protocolFee())).div(100);
            IERC20(token).transferFrom(backingDepositTo, ICWProtocol(factory).feeTo(), fundingFaliureFee);    // (valueToRemove * (ICWProtocol(factory).protocolFee()) / 100);

            // bonus from curation stack
            address curatingDepositTo = getDepositAddress(token, false);
            uint256 totalCurationStack = IERC20(token).balanceOf(curatingDepositTo);
            uint256 bonusAmount = valueToRemove.mul(totalCurationStack).div(totalFunding);  // (valueToRemove / totalFunding) * totalCurationStack;
            IERC20(token).transferFrom(curatingDepositTo, msg.sender, bonusAmount);

            // minus valueToRemove from totalFunding
            totalFunding = totalFunding.sub(valueToRemove);
            
            CWToken(bToken).burnFrom(msg.sender, valueToRemove);
        }
        return true;
    }

    function redeemCToken(address token, uint valueToRemove) public payable override returns (bool) {
        require(valueToRemove > 0, 'Requires non-zero redeem amount');
        require(block.timestamp >= deadline, 'Not able to redeem, they are locked in');

        require(CWToken(cToken).balanceOf(msg.sender) > 0, 'Curators can only call this method');
        require(CWToken(cToken).balanceOf(msg.sender) >= valueToRemove, 'Transfer amount exceeds your curated balance');

        if (!funded) {
            return false;   // when project funding is failed
        }
        ...
        if(isProtocolVersion1()){
            ...
            address curatingDepositTo = getDepositAddress(token, false);
            uint256 totalCurationStack = IERC20(token).balanceOf(curatingDepositTo);
            require(totalCurationStack >= valueToRemove, 'You are going to redeem greater than available amount');
            IERC20(token).transferFrom(curatingDepositTo, msg.sender, valueToRemove);

            // bonus for curatorFee: (curatorFee * totalFunding) * (cToken / totalCuration)
            address backingDepositTo = getDepositAddress(token, true);
            uint256 bonusAmount = curatorFee.mul(totalFunding).mul(valueToRemove).div(totalCurationStack).div(100); // (curatorFee * totalFunding / 100) * (valueToRemove / totalCurationStack);
            IERC20(token).transferFrom(backingDepositTo, msg.sender, bonusAmount);

            // bToken burn valutToRemove
            CWToken(cToken).burnFrom(msg.sender, valueToRemove);
        }
    ...
    function _addNomination(address _nomination) onlyCreator public {
        // IProject(_nomination).propose(action); // @TODO
    ...
    function addAcceptedTokens(address[] memory _tokens) onlyCreator override public {
    ...
    function _addAcceptedToken(address _token) onlyCreator public {
        ...
        if(isProtocolVersion1()) {
            ICWToken(bToken).approveOtherContract(IERC20(_token), address(this), uint256(-1));
            ICWToken(cToken).approveOtherContract(IERC20(_token), address(this), uint256(-1));
        }
    }

    function _removeAcceptedToken(address _nomination) onlyCreator public {
        // require that the balance isn't zero
    }
    function removeAcceptedTokens(address[] memory _nomination) onlyCreator override external {
        // require that the balance isn't zero
    }
...
contract CWProtocol is ICWProtocol, OwnableUpgradeable, Storage {
    ...
    function setFeeTo(address _feeTo) onlyOwner override external {
        feeTo = _feeTo;
    }
