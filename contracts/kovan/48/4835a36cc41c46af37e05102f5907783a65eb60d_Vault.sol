    ...
    AccessControl
{
    ...
    function rerange(int24 _tickRange, uint8 _poolId) external {
        ...
        if (_lowerCorrection >= 50) _baseLower = (_baseLower > 0) ? 
            (_baseLower + 1) * _tickSpacing : (_baseLower - 1) * _tickSpacing;
        if (_upperCorrection >= 50) _baseUpper = (_baseUpper > 0) ? 
            (_baseUpper + 1) * _tickSpacing : (_baseUpper - 1) * _tickSpacing;

        emit TEST_RERANGE(_baseLower, _baseUpper);

        _baseLower = _baseLower / _tickSpacing * _tickSpacing;
        _baseUpper = _baseUpper / _tickSpacing * _tickSpacing;
