 ...
 */
contract ArmyOfNicheman is IArbitrable ,Initializable {
    ...
    bytes32 private DOMAIN_SEPARATOR; // The EIP-712 domainSeparator specific to this deployed instance. It is used to verify the IsNichemanVoucher's signature.
    bytes32 private constant IS_NICHEMAN_VOUCHER_TYPEHASH = 0xa9e3fa1df5c3dbef1e9cfb610fa780355a0b5e0acb0fa8249777ec973ca78836; // The EIP-712 typeHash of IsNichemanVoucher. keccak256("IsNichemanVoucher(address vouchedSubmission,uint256 voucherExpirationTimestamp)").
    ...
    struct ArbitratorData {
        Arbitrator arbitrator; // Address of the trusted arbitrator to solve disputes.
     ...
     */
     function initialize(
        ...
        // EIP-712.
        bytes32 DOMAIN_TYPEHASH = 0x8cad95687ba82c2ce50e74f7b754645e5117c3a5bec8151c0726d58579802336; // keccak256("EIP712Domain(string name,uint256 chainId,address verifyingContract)").
        uint256 chainId;
        assembly { chainId := chainid() } // block.chainid got introduced in Solidity v0.8.0.
        DOMAIN_SEPARATOR = keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256("Army of Nicheman"), chainId, address(this)));
    }
    ...
    function addSubmissionManually(address[] calldata _submissionIDs, string[] calldata _evidence, string[] calldata _names) external onlyGovernor {
        ...
        for (uint i = 0; i < _submissionIDs.length; i++) {
            ...
            Request storage request = submission.requests[_newIndex];
    ...
    function removeSubmission(address _submissionID, string calldata _evidence) external payable {
        ...
        Arbitrator requestArbitrator = arbitratorDataList[arbitratorDataID].arbitrator;
     ...
     *  @param _signatures Array of EIP-712 signatures of struct IsNichemanVoucher (optional).
     *  @param _expirationTimestamps Array of expiration timestamps for each signature (optional).
     *  struct IsNichemanVoucher {
    ...
    function changeStateToPending(address _submissionID, address[] calldata _vouches, bytes[] calldata _signatures, uint[] calldata _expirationTimestamps) external {
        ...
        for (uint i = 0; i < _signatures.length && request.vouches.length < requiredNumberOfVouches; i++) {
            ...
            /* solium-disable indentation */
            {
                ...
                bytes32 messageHash = keccak256(abi.encode(IS_NICHEMAN_VOUCHER_TYPEHASH, _submissionID, _expirationTimestamps[i]));
