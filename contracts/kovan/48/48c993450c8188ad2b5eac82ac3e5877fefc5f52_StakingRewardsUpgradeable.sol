    ...
    PausableUpgradeable
{
    ...
    mapping(address => EnumerableSetUpgradeable.UintSet) _userSupply;
    ...
    function getTokenInfo(address account) public view override returns (uint256[] memory) {
    ...
    function _stakeNft(uint256 tokenId, address tokenOwner) internal normalMode nonReentrant whenNotPaused updateReward(tokenOwner) {
        ...
        tokenInfos[tokenId] = TokenInfo(tokenOwner, amount, block.timestamp);
    ...
    function availableReward(address account) public view override normalMode returns (uint256) {
        return _userBacked[account]
            .mul(rewardPerToken().sub(userRewardPerTokenPaid[account]))
            .div(1e18)
            .add(rewards[account]);
    ...
    function unstakeNft(uint256 tokenId) public override normalMode nonReentrant updateReward(msg.sender) {
    ...
    function updatePeriodFinish(uint256 timestamp)
        external normalMode onlyOwner updateReward(address(0))
    {
...
interface IStakingRewards {
    ...
    function getTokenInfo(address account) external view returns (uint256[] memory);
