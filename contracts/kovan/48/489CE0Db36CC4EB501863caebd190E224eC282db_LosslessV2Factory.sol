...
contract LosslessV2Factory is ILosslessV2Factory {
	...
	function createPool(
		...
		(address shortToken, address longToken, address sponsorToken) =  _initializeTokens(tokenName, tokenSymbol, ERC20(principalToken).decimals(), address(newPool));
	...
	function terminatePool(address pool) external onlyAdmin returns (bool) {
 	...
 	function _initializeTokens(
		...
		uint8 _decimals,
		address _pool
	) private returns (address shortToken, address longToken, address sponsorToken) {
	...
	/**
	 * @dev The default FeeToSetter is admin but admin can assign this role to others by calling `setFeeToSetter`
	 * @param _feeToSetter	new admin address
	 **/
	function setFeeToSetter(address _feeToSetter) external override onlyAdmin {
...
contract LosslessV2Pool is ILosslessV2Pool {
	...
	function withdraw(
		...
		/*  pool status | isAToken | Operation
				...
				lock 		 F		 redeem then transfer aToken
...
interface ILosslessV2Pool {
	...
	/*  
      ...
      *******  
                     ...
                     /FirstGame/            /Locked/         /Accepting/				/Terminated/
                          |                     |                | 							 |
