...
contract SimplePriceOracle is Ownable {
    ...
    event PriceUpdated(address underlying, string symbol, uint256 price, uint256 timestamp);
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < symbols.length; i++) {
            ...
            emit PriceUpdated(config.underlying, symbols[i], priceArr[i], timestamps[i]);
        }
    }
