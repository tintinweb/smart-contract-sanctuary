...
contract Precog {
        ...
        internal view returns (uint tradeAmount)
    {
        ...
        address factory = futureExchange.futureTokenFactory();
        uint reserveAExchange = IERC20(usdc).balanceOf(address(exchange));
        uint reserveBExchange = IERC20(pairs[1]).balanceOf(address(exchange));
