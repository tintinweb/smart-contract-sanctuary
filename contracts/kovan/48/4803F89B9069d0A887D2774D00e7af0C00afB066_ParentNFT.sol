...
contract ParentNFT is ERC721, ChainlinkClient {
     ...
     */
    function fulfillBytesOLD(bytes32 requestId, bytes memory bytesData)
        public
        recordChainlinkFulfillment(requestId)
    {
        ...
        string memory tokenURI = string(data); //to then call tokenURI on. only supports bytes32 right now...
    ...
    {
        emit EAreceived(_requestId);
        //probably need to add some checks here since this is public
        latestTokenURI = bytes32ToString(_result); //for testing TODO - figure out buffering for character limit stuff...
...
*/
    ...
    function requestBytes() public //returns (bytes32 requestId)
    {
        ...
        string[] memory childURIAry = new string[](5); //declare initial size since this string array is in memory
        string[] memory activeStrings = new string[](5);
        for (uint256 i = 0; i < 5; i++) {
            activeStrings[i] = "true"; //data still remains at location child_token_struct_id
            childURIAry[
                i
