...
contract assignmentToken {
    ...
    function simple_transfer(address _from,address _to, uint256 _value) private returns (bool) {
        require(balances[_from] >=_value +transferFee); // check if _from has enough tokens
        // update sender balance
    ...
    function transferFrom(
        ...
        // NOTE: transfer value needs to be sufficient to cover fee
        require(allowances[_from][msg.sender] >= _value+transferFee); // check allowance
        ...
        if(success){allowances[_from][msg.sender] -= _value+transferFee;}
