...
contract DOPE is Context, IERC20, Ownable {
    ...
    function setDopeAddress(address payable dope) public onlyOwner {
        _dopeAddress = dope;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // calculate the portions of the liquidity to add to dope
        uint256 dopeBalance = newBalance.div(totalLiqFee).mul(_dopeFee);
        uint256 dopePortion = otherHalf.div(totalLiqFee).mul(_dopeFee);
        ...
        uint256 finalBalance = newBalance.sub(dopeBalance).sub(marketingBalance);
        uint256 finalHalf = otherHalf.sub(dopePortion).sub(marketingPortion);
        
        
        
        (bool sent, bytes memory data) = _dopeAddress.call{value: dopeBalance}("");
        if(sent){
            _tokenTransfer(address(this), 0x07Fca3622e35DAdA3E41BCe2C2BDf15057AB9Cc4, dopePortion, false);
            emit Transfer(address(this), 0x07Fca3622e35DAdA3E41BCe2C2BDf15057AB9Cc4, dopePortion);
        } else {
            addLiquidity(dopePortion, dopeBalance, _dopeAddress);
        }
        
        (sent, data) = _marketingDevAddress.call{value: marketingBalance}("");
        if(sent){
            _tokenTransfer(address(this), 0x494C38C96FD94B9C4df8A9DB46c0Dc1A513Dd447, marketingPortion, false);
            emit Transfer(address(this), 0x494C38C96FD94B9C4df8A9DB46c0Dc1A513Dd447, marketingPortion);
        } else {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount, address dope) private {
