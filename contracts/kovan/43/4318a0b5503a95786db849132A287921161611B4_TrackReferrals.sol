...
contract TrackReferrals is ChainlinkClient, Ownable {
    ...
    struct Participant{
        ...
        uint256 whatsappNumber;
    }

    //mapping (address => Participant) userReferrals;
    mapping (uint => Participant) userReferrals;

    function addNewParticipant(address _user, string memory _nickName, address _referredBy, string memory _twitterLink, uint256 _whatsAppNumber) public returns (uint participantTracking) {
        participantTracking = numberOfReferrals++;
        userReferrals[participantTracking] = Participant(_user, _nickName, _twitterLink, _whatsAppNumber);
    }

    function checkParticipant(uint participantTracking) public returns (string memory value) {
        Participant storage p = userReferrals[participantTracking];
        bytes memory tempNickname = bytes(p.nickName); // Uses memory
        if (tempNickname.length == 0){
            return p.nickName;
        } else {   
            return p.twitterProfileLink;
        }
    }
    ...
    //referral management
    function addReferral(address _referrer, address _user) public {
        referralList.push([_referrer, _user]);
    }

    function getReferrals() public view returns (uint count) {
