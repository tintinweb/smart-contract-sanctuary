...
contract Crowdsale is Ownable {
    ...
    function buyTokens() public payable {
        ...
        if(msg.value <= ethStepOne) {
            token.transfer(msg.sender, usdDeposited / ethStepOnePrice * (uint256(1) ** tokenDecimals));
        } else if(msg.value <= ethStepTwo) {
            vested = usdDeposited / ethStepTwoPrice * (uint256(1) ** tokenDecimals);
            vestedAmount[msg.sender] += vested;
        } else if(msg.value > ethStepTwo) {
            vested = usdDeposited / ethStepThreePrice * (uint256(1) ** tokenDecimals);
            vestedAmount[msg.sender] += vested;
        } 
