...
contract AutonomousPayment is Ownable, ERC721URIStorage, ChainlinkClient {
  ...
  */
  ...
  function requestPayment(string memory _CID_asBuiltBIM, string memory _CID_rawProgressData)
    public inState(State.Agreed) returns(bytes32 requestId) {
    ...
    // /// @notice current paymentID is actually previous payment. 
    // /// PaymentIDs increment proceed in receiveUpdate function
    // /// which records Chainlink fulfillment.
    // /// If it is a first payment CID_previousPaymentProgress is empty string.
    // request.add("CID_previousPaymentProgress", payments[paymentIDs.current()].CID_currentPaymentProgress);
    ...
    // /// @notice in case of request failing paymentIDs increment is applied after require statement in receiveUpdate
    uint256 _paymentID = paymentIDs.current() + 1;
    request.addInt("paymentID", int(_paymentID));

    // Sends the request
    return sendChainlinkRequestTo(oracle, request, fee);  // sendChainlinkRequestTo(oracle, request, fee);
  }
