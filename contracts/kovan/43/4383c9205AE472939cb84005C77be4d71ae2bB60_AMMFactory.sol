...
contract AMMFactory {
    ...
    function createPool(
        ...
        for (uint256 i = 0; i < _market.shareTokens.length; i++) {
            ITurboShareToken _token = _market.shareTokens[i];
    ...
    function buy(
        ...
        ITurboShareToken _desiredToken = _market.shareTokens[_outcome];
    ...
    function sell(
        ...
        uint256 _minSetsToSell = _minCollateralOut;
        uint256 _setsToSell = MAX_UINT;
        ITurboShareToken _undesiredToken = _market.shareTokens[_outcome];
        for (uint256 i = 0; i < _market.shareTokens.length; i++) {
            if (i == _outcome) continue;
            ITurboShareToken _token = _market.shareTokens[i];
    ...
    function prices(IMarketFactory _marketFactory, uint256 _marketId) external view returns (uint256[] memory) {
        BPool _pool = pools[address(_marketFactory)][_marketId];
        IMarketFactory.Market memory _market = _marketFactory.getMarket(_marketId);
...
abstract contract IMarketFactory is TurboShareTokenFactory {
    using SafeMathUint256 for uint256;
    ...
    struct Market {
        address creator;
        TurboShareToken[] shareTokens;
        uint256 endTime;
        TurboShareToken winner;
    }
    ...
    function claimWinnings(uint256 _id, address _receiver) public returns (uint256) {
        ...
        if (_market.winner == TurboShareToken(0)) {
            resolveMarket(_id);
        }
...
abstract contract TurboShareTokenFactory {
    function createShareTokens(
        ...
        address _owner
    ) internal returns (TurboShareToken[] memory) {
        uint256 _numOutcomes = _names.length;
        TurboShareToken[] memory _tokens = new TurboShareToken[](_numOutcomes);

        for (uint256 _i = 0; _i < _numOutcomes; _i++) {
            _tokens[_i] = new TurboShareToken(_symbols[_i], _names[_i], _owner);
        }
        return _tokens;
    }
}
...
contract FeePot is VariableSupplyToken {
    using SafeMathUint256 for uint256;
