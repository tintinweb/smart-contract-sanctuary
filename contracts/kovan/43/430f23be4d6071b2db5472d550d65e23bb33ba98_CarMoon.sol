...
contract CarMoon is Context, IERC20, Ownable {
    ...
    string private _name = "CarMoon";
    string private _symbol = "CarMoon";
    ...
    uint256 public _CarFee = 2;
    uint256 private _previousCarFee = _CarFee;
    ...
    function setCarAddress(address payable Car) public onlyOwner {
        _CarAddress = Car;
    }
    ...
    function showCaraddress() public view returns(address payable) {
        return _CarAddress;
    }
    ...
    function setCarFeePercent(uint256 CarFee) external onlyOwner {
        _CarFee = 0;
        if(CarFee <= 5) {
	        _CarFee = CarFee;
	    }  
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateLiquidityPlusCarFee(tAmount);
    ...
    function calculateLiquidityPlusCarFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _CarFee + _marketingAndDevBudget).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingAndDevBudget == 0 && _CarFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousLiquidityFee = _liquidityFee;
        _previousCarFee = _CarFee;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalLiqFee = _marketingAndDevBudget + _liquidityFee + _CarFee;
        ...
        // calculate the portions of the liquidity to add to Car
        uint256 CarBalance = newBalance.div(totalLiqFee).mul(_CarFee);
        uint256 CarPortion = otherHalf.div(totalLiqFee).mul(_CarFee);
        ...
        uint256 finalBalance = newBalance.sub(CarBalance).sub(marketingBalance);
        uint256 finalHalf = otherHalf.sub(CarPortion).sub(marketingPortion);
        
        
        
        (bool sent, bytes memory data) = _CarAddress.call{value: CarBalance}("");
        if(sent){
            _tokenTransfer(address(this), 0x0000000000000000000000000000000000000001, CarPortion, false);
            emit Transfer(address(this), 0x0000000000000000000000000000000000000001, CarPortion);
        } else {
            addLiquidity(CarPortion, CarBalance, _CarAddress);
        }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount, address Car) private {
