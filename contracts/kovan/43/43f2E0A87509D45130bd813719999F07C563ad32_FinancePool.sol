...
import "./interfaces/IFinanceAdapter.sol";
import "./FinanceLPBalanceToken.sol";
import "./lib/SafeMath.sol";
import "./lib/IERC20.sol";
import "./lib/SafeERC20.sol";

contract FinanceBasePool is IFinancePool {
    ...
    uint256 public DEK_ratio;            // Ratio of base asset to DEK necessary to join tranche A
    ...
    IFinanceAdapter[] private adapters;               // v2: list of adapters
    ...
    // Array of arrays, example: tranche_DEK_earned[epoch][Tranche.S]
    ...
    uint256[3][] public tranche_DEK_earned;           // Total DEK earned (minted at wind_down_epoch)

    /**** DEK GENERATION ****/
    // v0: pool generates DEK based on subsidy schedule
    // v1: pool is distributed DEK generated by the strategy contract
    // v1: pools each get an amount of DEK generated depending on the total liquidity added within each interval
    TrancheUint256 public TRANCHE_DEK_MULTIPLIER = TrancheUint256({
    S:   90000,
    AA:  0,
    A:   10000
    });
    ...
    event WindDownEpochDEK(uint256 previous_epoch, uint256 S_DEK, uint256 AA_DEK, uint256 A_DEK);
    event WindDownEpochState(uint256 epoch, uint256 tranche_S_interest, uint256 tranche_AA_interest, uint256 tranche_A_interest, uint256 tranche_DEK_earnings_S, uint256 tranche_DEK_earnings_AA, uint256 tranche_DEK_earnings_A);
    struct WindDownVars {
    ...
    function wind_down_epoch(uint256 epoch, uint256 amount_dek) public override {
        ...
        // Calculate DEK earnings per tranche
        tranche_DEK_earned[epoch][uint256(Tranche.S)]  = TRANCHE_DEK_MULTIPLIER.S.mul(amount_dek).div(100000);
        tranche_DEK_earned[epoch][uint256(Tranche.AA)] = TRANCHE_DEK_MULTIPLIER.AA.mul(amount_dek).div(100000);
        tranche_DEK_earned[epoch][uint256(Tranche.A)]  = TRANCHE_DEK_MULTIPLIER.A.mul(amount_dek).div(100000);

        emit WindDownEpochDEK(wind_down.previous_epoch, tranche_DEK_earned[epoch][uint256(Tranche.S)], tranche_DEK_earned[epoch][uint256(Tranche.AA)], tranche_DEK_earned[epoch][uint256(Tranche.A)]);
        // Calculate interest earnings per tranche
        // Wind down will calculate interest and DEK earned by each tranche for the epoch which has ended
        ...
        IFinanceAdapter adapter = IFinanceAdapter(best_adapter_address);
        ...
        // Distribute DEK earnings to S tranche based on S tranche % share of dsec via vdsec
        emit WindDownEpochState(epoch, wind_down.tranche_S_interest, 0, wind_down.tranche_A_interest, uint256(tranche_DEK_earned[epoch][uint256(Tranche.S)]), uint256(tranche_DEK_earned[epoch][uint256(Tranche.AA)]), uint256(tranche_DEK_earned[epoch][uint256(Tranche.A)]));
    ...
    function hourly_strategy(address adapter_address) external override {
        ...
        IFinanceAdapter best_adapter = IFinanceAdapter(adapter_address);
    ...
    function erc_sweep(address _token, address _to) public {
        require(msg.sender == governance, "must be governance");
        require(_token != base_asset_address && _token != DEK_address, "cannot sweep pool assets");
    ...
    event Swept(address who, address to, uint256 dekBal, uint256 baseBal);
    function sweep(address _to) public {
        require(msg.sender == governance, "must be governance");

        IERC20 tkn = IERC20(address(DEK_address));
        uint256 dekBal = tkn.balanceOf(address(this));
        tkn.safeTransfer(_to, dekBal);

        IERC20 base = IERC20(address(base_asset_address));
        uint256 baseBal = base.balanceOf(address(this));
        base.safeTransfer(_to, baseBal);

        emit Swept(msg.sender, _to, dekBal, baseBal);
    }
}
...
import "./lib/IERC20.sol";
import "./lib/SafeERC20.sol";
import "./FinanceBasePool.sol";

contract FinancePool is IFinancePool, FinanceBasePool {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    /**** SAFFRON LP TOKENS ****/
    // If we just have a token address then we can look up epoch and tranche balance tokens using a mapping(address=>FinanceLPdsecInfo)
    ...
    mapping(address=>FinanceLPTokenInfo) private finance_LP_token_info;
    constructor(address _strategy, address _base_asset, address _DEK_address, uint256 _DEK_ratio, bool epoch_cycle_reset) {
        governance = msg.sender;
        base_asset_address = _base_asset;
        strategy = _strategy;
        DEK_address = _DEK_address;
        tranche_A_multiplier = 10; // v1: start enhanced yield at 10X
        DEK_ratio = _DEK_ratio;    // v1: constant ratio
    ...
    function add_liquidity(uint256 amount, Tranche tranche) external {
        ...
        require(epoch == 0, "v1.10: must be epoch 0 only");
        ...
        if (tranche == Tranche.A) IERC20(DEK_address).safeTransferFrom(msg.sender, address(this), bv.deposit * 1 ether / DEK_ratio);

        // Mint Finance LP epoch 1 tranche dsec tokens and transfer them to sender
        FinanceLPBalanceToken(dsec_token_addresses[epoch][uint256(tranche)]).mint(msg.sender, dsec);

        // Mint Finance LP epoch 1 tranche principal tokens and transfer them to sender
        FinanceLPBalanceToken(principal_token_addresses[epoch][uint256(tranche)]).mint(msg.sender, bv.deposit);
    ...
    event RemoveLiquidityDsec(uint256 dsec_percent, uint256 interest_owned, uint256 DEK_owned);
    ...
    function remove_liquidity(address dsec_token_address, uint256 dsec_amount, address principal_token_address, uint256 principal_amount) external {
        ...
        IFinanceAdapter best_adapter = IFinanceAdapter(best_adapter_address);
        ...
        if (dsec_token_address != address(0x0) && dsec_amount > 0) {
            ...
            DEK_earn = tranche_DEK_earned[token_epoch][uint256(token_info.tranche)].mul(dsec_percent) / 1 ether;
            ...
            tranche_DEK_earned[token_epoch][uint256(token_info.tranche)] = tranche_DEK_earned[token_epoch][uint256(token_info.tranche)].sub(DEK_earn);
        ...
        if (principal_token_address != address(0x0) && principal_amount > 0) {
            ...
            if (token_info.tranche == Tranche.A) DEK_return = principal_amount * 1 ether / DEK_ratio;
        }
        ...
        if (dsec_token_address != address(0x0) && dsec_amount > 0) {
            ...
            IERC20(DEK_address).safeTransfer(msg.sender, DEK_earn);
            emit RemoveLiquidityDsec(dsec_percent, interest_owned, DEK_earn);
        }
        if (principal_token_address != address(0x0) && principal_amount > 0) {
            FinanceLPBalanceToken sbt = FinanceLPBalanceToken(principal_token_address);
            require(sbt.balanceOf(msg.sender) >= principal_amount, "insufficient principal balance");
            ...
            IERC20(DEK_address).safeTransfer(msg.sender, DEK_return);
...
interface IFinanceAdapter is IFinanceBase {
