...
contract ShardingDAOMining is IInvitation, ActivityBase {
    ...
    uint256 public constant produceBlocksPerDay = 28800;
    ...
    uint256 public SHDPerBlock = 1246 * (1e13);
    // after each term, mine half SHD token
    uint256 public constant MINT_DECREASE_TERM = 42116667;
    ...
    uint256 public updateTokenPriceTerm = 600;
    ...
    function deposit(
        ...
        if (p.existedAmount > 0) {
            if(newEndBlock < endBlock){
                ...
                newOriginWeight = newOriginWeight.add(getDepositWeight(
                    p.existedAmount,
                    newEndBlock.sub(endBlock).div(temBlock)
                ));
            }
        } else {
    ...
    function withdrawRevenue(uint256 _pid) external virtual {
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount > 0, "user is not existed");
        require(user.endBlock < block.number, "token is still locked");
        mintSHARD(address(this), block.number);
        updatePoolDividend(_pid);
        uint256 userWeight = getUserModifiedWeight(_pid, msg.sender);
        PoolInfo storage pool = poolInfo[_pid];
        UserRevenueInfo storage senderRevenueInfo = userRevenueInfo[_pid][msg.sender];
        uint256 currentRewardDebt = poolAccs[_pid].mul(userWeight).div(1e12);
        uint256 pending = currentRewardDebt.sub(senderRevenueInfo.userRewardDebt).
                                                   add(senderRevenueInfo.revenue);
        senderRevenueInfo.userRewardDebt = currentRewardDebt;
        senderRevenueInfo.revenue = 0;
        if(pending > 0){
            pool.accumulativeDividend = pool.accumulativeDividend.sub(pending);
            dividendBonus(_pid, pending, user.amount, isUserContractSender[_pid][msg.sender]);
        }
    ...
    function modifyWeightByInvitation(
        ...
        if(p.existedAmount == 0){
            ...
            if(pool.usersTotalWeight == 0 && pool.accumulativeDividend > 0){
