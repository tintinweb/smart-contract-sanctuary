...
def updateRewards():
    ...
    if self.totalStaked > 0:
        self.paidPerToken += totalToPaid / self.totalStaked
    else:
        self.paidPerToken += 0
...
def bal(user: address) -> (uint256, uint256):
    ...
    a: decimal = convert(totalToPaid, decimal)


    return (
        self.holdings[user],
        (self.paidPerToken + self.totalStaked * totalToPaid/self.totalStaked) * self.holdings[user] - self.userRewards[user]
        )
