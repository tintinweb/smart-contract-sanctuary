...
// File: MultiSign/MultiSignCost.sol
...
contract MultiSignCost is EIP712,ReentrancyGuard,ERC721Holder {
    uint public nonces;
    ERC20Votes public immutable token;
    uint public passVote;

    constructor(ERC20Votes _token,uint _passVote) EIP712("MultiSignCost", "1") {
        token=_token;
        passVote=_passVote;
    }
    ...
    function execute(uint256 deadline,address target,bytes calldata data,address[] calldata signer,bytes[] calldata signature) external nonReentrant {
        require(block.timestamp <= deadline, "MultiSignCost: expired deadline");
        uint n=signer.length;
        require(n==signature.length,"MultiSignCost: length of signer must equals signature");
        uint nonce=useNonce();
        bytes32 hash=hashData(deadline,nonce,target,data);
        uint votes=0;
        for(uint i=0;i<n;i++){
            if(check(signer[i],hash,signature[i])){
                votes+=token.getVotes(signer[i]);
            }
        }
        if(votes>=passVote){
            ...
            if(success){
                ...
                revert("MultiSignCost: execute fail");
            }
        }else{
            revert("MultiSignCost: not pass");
        }
    }
    ...
    function hashData(uint256 deadline,uint256 nonce,address target,bytes memory data) public view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
            keccak256("MultiSignRequest(uint256 deadline,uint256 nonce,address target,bytes data)"),
            deadline,
            nonce,
            target,
            keccak256(data)
    ...
    function useNonce() internal returns(uint nonce) {
        nonce=nonces;
        nonces=nonces+1;
    }
}