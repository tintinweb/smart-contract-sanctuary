...
contract Smoody is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 public WL_PURCHASE_LIMIT = 10;
    ...
    function setWLPurchaseLimit(uint256 wlPurchaseLimit) external onlyOwner {
        WL_PURCHASE_LIMIT = wlPurchaseLimit;
    }
    ...
    function mintSmoody(uint256 numberOfTokens) public payable nonReentrant {
        ...
        uint256 supply = totalSupply();
        require(
            supply + numberOfTokens <= SMOODY_TOTAL - _reserved,
    ...
    // {
    //     require(
    //         numberOfTokens <= _purchase_limit,
    //         "Can only mint up to 10 token"
    //     );

    //     require(preSaleActive, "Allow List is not active");
    //     require(_whiteList[msg.sender], "You are not on the Allow List");
    //     require(
    //         _publicALIENFRENS.current() < SMOODY_TOTAL,
    //         "Purchase would exceed max"
    //     );
    //     require(
    //         numberOfTokens <= _wl_purchase_limit,
    ...
    //     require(
    //         _publicALIENFRENS.current() < SMOODY_TOTAL,
    //         "Purchase would exceed SMOODY_TOTAL"
    //     );
    //     for (uint256 i = 0; i < numberOfTokens; i++) {
    //         uint256 tokenId = _publicALIENFRENS.current();

    //         if (_publicALIENFRENS.current() < SMOODY_TOTAL) {
