...
contract CharacterDetailsV1 is Administerable {
    function() external view isAfterReveal;
    ...
    constructor() Administerable(msg.sender) {}

    function setRandom(
        uint256 _raritySeed,
        uint48 _rarityValue,
        function() external view _isAfterReveal
    ) public {
        require(!_rarityLock);
...
contract StakeWarsV1 is ERC721Enumerable, Administerable, VRFConsumerBase {
    event RequestedRandomness(bytes32 indexed requestId, uint256 tokenId);
    event RecievedRandomness(
        uint256 randomness,
        uint48 rand48,
        address sender,
        uint256 totalSupply
    ...
    mapping(address => mapping(uint256 => CharacterDetailsV1))
        public stakeWarrior;
        ...
        override
    {
        ...
        uint256 tokenId = requestIdToUser[requestId].tokenId;
        emit RecievedRandomness(randomness, rare48, sender, totalSupply());
        stakeWarrior[sender][tokenId].setRandom(
            randomness,
            rare48,
            this.isAfterReveal
        );
    ...
    function mintPresale(uint256 numberOfMints) public payable {
        ...
        require(reserved > 0);
        require(numberOfMints <= reserved);
        require(supply + numberOfMints <= maxSupply);
    ...
    function createMint(uint256 i) internal {
        uint256 tokenIndex = balanceOf(msg.sender) + i;
        bytes32 requestId = requestRandomness(keyhash, linkFee);
        requestIdToUser[requestId] = User(msg.sender, tokenIndex);
        stakeWarrior[msg.sender][tokenIndex] = new CharacterDetailsV1();
        emit RequestedRandomness(requestId, tokenIndex);
    }
    ...
    function toggleStatus(int8 status) public onlyAdmin {
        if (status == 0) presaleActive = !presaleActive;
        else if (status == 1) saleActive = !saleActive;
        else if (status == 2) showCharacterDetails = true;
    }
    ...
    function isAfterReveal() public view {
