...
contract Token is TokenERC20, Control, Oracle, ReentrancyGuard {
		...
		)
	{
		...
		if (daysPassFromRebase == 0) {
			...
			price2 = (((coeffInflations * daysPassAll * 10000) / 365 + 100000000) * initialPrice) / 100000000; // 18
		}
		if (price1 > price2 && price1 > curPrice) {
			supplyDelta = ((_totalSupply * oldPrice) / newPrice); // 0
		} else if (price2 > curPrice) {
			price2 = ((coeffInflations * daysPassFromRebase * 10000) / 365 + 100000000) * initialPrice / 100000000; // 18
