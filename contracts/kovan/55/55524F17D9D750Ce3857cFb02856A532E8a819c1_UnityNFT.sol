...
contract UnityNFT is ERC721, KeeperCompatibleInterface{
    ...
    modifier onlyOwner {
        require(msg.sender == ownerOf(1), "Owner Only function.");
        _;
    }

    constructor(string memory _baseURI, uint256 _interval, uint256 _MAX_ID) ERC721("UnityNFT", "UNFT") {
        baseURI = _baseURI;
        interval = _interval;
        MAX_ID = _MAX_ID;
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId) && tokenId == 1, "ERC721Metadata: URI query for nonexistent token");
