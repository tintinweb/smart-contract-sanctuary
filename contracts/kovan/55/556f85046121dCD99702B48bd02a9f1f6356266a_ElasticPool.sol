...
// 
contract ElasticPool is Ownable, ReentrancyGuard, ERC20, Pausable {
	using SafeERC20 for IERC20;

	// Internal token decimals 
	uint8 private tokenDecimals;

	// ReservePool contract address allowed to burn EPT without approval
	address public reservePool;
	// VPStorage contract address where extra profit will be sent
	...
	constructor (
		...
		address balToken_,
		address feeAddress_,
		uint256 feeCoefficient_,
		uint256 lossCoefficient_,
		uint256 profitCoefficient_,
		uint256 holdDays_
	)
		ERC20('ElasticPoolToken', 'EPT')
	{
		tokenDecimals = decimals_;

		bVault = IVault(bVault_);
		bPool.id = bPoolId_;

		inToken = IERC20(inToken_);
		updatebPool();
		inToken.safeApprove(bVault_, type(uint256).max);
		balToken = IERC20(balToken_);
		balToken.safeApprove(bVault_, type(uint256).max);

		feeAddress = feeAddress_;
		setFeeCoefficient(feeCoefficient_);
		setLossCoefficient(lossCoefficient_);
		setProfitCoefficient(profitCoefficient_);
		setHoldTime(holdDays_);
	}
	...
	*/
		...
		onlyOwner
	{
		...
		bVault.getPool(bPoolIdOne_);
		// Throws error if BAL Token is not registered with Pool ID One 
		bVault.getPoolTokenInfo(bPoolIdOne_, address(balToken));
		// Throws error if Intermediate Token is not registered with Pool ID One 
		bVault.getPoolTokenInfo(bPoolIdOne_, address(bIntermediateToken_));
		...
		if(address(inToken) != bIntermediateToken_){
			...
			bVault.getPool(bPoolIdTwo_);
			// Throws error if Intermediate Token is not registered with Pool ID Two 
			bVault.getPoolTokenInfo(bPoolIdTwo_, bIntermediateToken_);
			// Throws error if InToken is not registered with Pool ID Two 
			bVault.getPoolTokenInfo(bPoolIdTwo_, address(inToken));
	...
	* infinite approves inToken to VPStorage contract to save gas on receiveOutToken calls
	...
	*/
		...
		onlyOwner
	{
		...
		inToken.safeApprove(vpStorage_,  type(uint256).max);
	}
	...
	*/
		...
		onlyOwner
	{
		...
		profitCoefficient = coefficient_;
	} 
	...
	****************************************/
	...
	/**
	 * @dev Overrides ERC20 allowance(), returns infinite allowance for ReservePool
	 */
		...
		returns (bool)
	{
		...
		return block.timestamp - userData[address_].lastTime < holdTime;
	}
	...
	{
		uint256 _releaseTime = userData[address_].lastTime + holdTime; 
		...
		)
	{
		...
		if(inAmount_ > amount_){
			...
			_maxChange = inAmount_ * lossCoefficient / COEFFICIENT_PRECISION;
			...
			fee_ = (_change - _maxChange) * feeCoefficient / COEFFICIENT_PRECISION;
