...
contract EMISSIONAPIConsumer is  ChainlinkClient ,IEMISSIONORACLE{
      using Chainlink for Chainlink.Request;
    uint256 public emissionValue;
    uint256 public THRESHOLD=3e15;
     ...
     * data, then multiply by 10 ** 17 (to remove decimal places from data).
     ************************************************************************************
     *                                    STOP!                                         * 
     ...
     *                                                                                  *
     ************************************************************************************/
    function requestEmissionValue(string memory url) external returns (bytes32 ) 
    {
        
        require(msg.sender == owner,"only Owner");
        require(block.timestamp>= lastCall+EPOCH_PERIOD,"To early");
        Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);
        //"https://api.v2.emissions-api.org/api/v2/carbonmonoxide/statistics.json?polygon=30&polygon=10&polygon=40&polygon=40&polygon=20&polygon=40&polygon=10&polygon=20&polygon=30&polygon=10&interval=day&begin=2020-02-10&end=2020-02-11&limit=100&offset=0"
        // Set the URL to perform the GET request on
        request.add("get", url);
        
        
        request.add("path", "0.value.average");
    ...
    {
       require(requestId==_requestId,"invalid");
        emissionValue = _volume;
        lastCall=block.timestamp;
  ...
  /**
     * getEmission value from oracle
     * 
     * NOTE: this to be called from other contracts to get updated emission value.
     * 
     */
    function getEmission() external override  view returns (uint256){
