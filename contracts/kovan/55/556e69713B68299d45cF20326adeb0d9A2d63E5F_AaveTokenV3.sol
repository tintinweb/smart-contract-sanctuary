 ...
 * @author Aave
 */
contract AaveTokenV3 is GovernancePowerDelegationERC20, VersionedInitializable {
   ...
   * @param lendToAaveMigratorAddress the address of the LendToAaveMigrator contract
   * @param lendAmount the amount of LEND to migrate
   */
  function initialize(
    address assetRescuer,
    address safeVault,
    address lendToAaveMigratorAddress,
    uint256 lendAmount
    ...
    // LEND migration
    IERC20 lend = ILendToAaveMigrator(lendToAaveMigratorAddress).LEND();
    lend.approve(lendToAaveMigratorAddress, lendAmount);
    ILendToAaveMigrator(lendToAaveMigratorAddress).migrateFromLEND(lendAmount);
  }
  ...
  /**
   * @dev Rescue locked Aave tokens from a holder and redirect it to the safe vault
   * @param holderWithLockedTokens the holder address with locked Aave tokens
   * @param amountOfLockedTokens the amount of locked Aave tokens
   */
  function rescueLockedTokens(address holderWithLockedTokens, uint256 amountOfLockedTokens)
    external
  {
    require(msg.sender == _assetRescuer, 'ONLY_ASSET_RESCUER');
    _burn(holderWithLockedTokens, amountOfLockedTokens);
    _mint(_safeVault, amountOfLockedTokens);
  }
...
interface ILendToAaveMigrator {
  ...
  function LEND_AAVE_RATIO() external view returns (uint256);

  function REVISION() external view returns (uint256);

  function _totalLendMigrated() external view returns (uint256);
