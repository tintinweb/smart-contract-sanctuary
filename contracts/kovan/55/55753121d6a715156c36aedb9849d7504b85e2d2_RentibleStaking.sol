...
contract RentibleStaking is Ownable { 
    ...
    3) withdrawAllWithoutReward() = same as 3, amount is fixed/all (depositedLptOfTheUser())
    ...
    6) withdrawWithAllReward(uint256) = method 5, plus after that method 2
    7) withdrawAllWithAllReward() = method 6 amount is fixed/all (depositedLptOfTheUser()) (note: method 5 plus method 3 would be the same... meaning 7=6=5,2 would be the same as 7=5,3)
    ...
    */
    ...
    function takeOutSomeOfTheAccumulatedReward(uint256 _rewardAmountToBeTakenOut) public returns(uint256) {
        ...
        // (so addToTheTheUsersAssignedReward() call is needed before)
    ...
    function addToTheTheUsersAssignedReward() public returns(uint256) {
        ...
        for (uint256 i = rewardCountedUptoDayNextDay; i <= currentStakingDayNumberMinusOne; i++) {
            ...
            uint256 rew = (((usersCurrentLtpAmount.mul(10000)).div(dailyTotalLptAmount[i])).mul(dailyPlannedErc20RewardAmounts[i])).div(10000);
    ...
    // Current accumulated reward (RNB) of the user (depositor) 
    // (read only, does not save/alter state)
    function calculateCurrentTakeableRewardOfTheUser() public view returns(uint256) {

        // ---
        // --- copy of addToTheTheUsersAssignedReward(), but without the writes
        // ---
