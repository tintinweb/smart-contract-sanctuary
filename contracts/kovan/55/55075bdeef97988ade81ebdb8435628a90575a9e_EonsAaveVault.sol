...
contract EonsAaveVault is OwnableUpgradeable {
  ...
  //   UserInfo storage userInfo = userInfo[msg.sender][pid];
  ...
  function pendingRewardDevFeeOf(uint _pid, address _user) public returns (uint256) {
    uint totalPending = router.pendingRewardOf(_pid);
    uint totalStaked = totalStaked(_pid);
    UserInfo storage user = userInfo[_user][_pid];
    uint devPendingReward = totalPending.div(devFeeRate).mul(user.amount).div(totalStaked);
    return devPendingReward;
  }

  function pendingRewardOf(uint256 _pid, address _user) public returns (uint256) {
    uint totalPending = router.pendingRewardOf(_pid);
    uint totalStaked = totalStaked(_pid);
    UserInfo storage user = userInfo[_user][_pid];
    uint pendingReward = totalPending.div(poolRewardRate).mul(user.amount).div(totalStaked);
    return pendingReward;
  }

  // function depositFor(address _recipient, uint _amount, uint _pid) external {
  //   updateEmissionDistribution();
  //   uint pending = pendingEons(_pid, recipient);
  //   if (pending) {
  //     eons.transfer(_recipient, pending);
  //   }
  //   PoolInfo storage pool = poolInfo[_pid];
  //   IEonsETH(pool.eToken).mint(_recipient, _amount);
  //   UserInfo storage user = userInfo[_recipient][_pid];
  //   user.amount = amount;
  ...
  function depositETH() external payable {
    ...
    router.deposit(msg.value, 1, msg.sender);
