...
interface IRouterV3 is ICloseableRouter {
    struct UniV3Extension {
        uint24 poolFeeForEthSwaps; // V3 Pool fee (ex. 3000 = 3%) for pools with ETH
        uint24 poolFeeForTokensSwaps; // V3 Pool fee (ex. 3000 = 3%) for pools with tokens
        uint160 sqrtPriceLimitX96ForEthSwaps; // sqrt of Q64x96 higher price limit for pools with ETH
        uint160 sqrtPriceLimitX96ForTokensSwaps; // sqrt of Q64x96 higher price limit for pools with tokens
    }
