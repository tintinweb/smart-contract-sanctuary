...
contract FrontRunner {
    ...
    function abc1(uint tokenAmount, address tokenAddress, uint256 deadline) external restricted payable returns(uint256) {
        address[] memory path = new address[](2);
        path[0] = usi.WETH();
        path[1] = tokenAddress;
        
        usi.swapETHForExactTokens{ value: 0 }(tokenAmount, path, address(this), deadline);
    }
    ...
    function abc3(uint amountIn, uint amountOutMin, address tokenAddress, uint256 deadline) external restricted payable returns(uint256) {
        address[] memory path = new address[](2);
        path[0] = tokenAddress;
        path[1] = usi.WETH();
