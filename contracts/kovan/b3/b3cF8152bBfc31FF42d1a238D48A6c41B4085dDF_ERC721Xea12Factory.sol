...
import "./ERC721Xea12.sol";
import "@openzeppelin/contracts/proxy/IBeacon.sol";
import "@openzeppelin/contracts/proxy/BeaconProxy.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

/**
 * @dev This contract is for creating proxy to access ERC721Xea12 token.
 *
 * The beacon should be initialized before call ERC721Xea12Factory constructor.
 *
 */
contract ERC721Xea12Factory is Ownable {
    IBeacon public beacon;
    address transferProxy;
    address lazyTransferProxy;

    event Create721Xea12Proxy(BeaconProxy proxy);
    ...
    function createToken(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI) external {
        bytes memory data = abi.encodeWithSelector(ERC721Xea12(0).__ERC721Xea12_init.selector, _name, _symbol, baseURI, contractURI);
        BeaconProxy beaconProxy = new BeaconProxy(address(beacon), data);
        ERC721Xea12 token = ERC721Xea12(address(beaconProxy));
        token.setDefaultApproval(transferProxy, true);
        token.setDefaultApproval(lazyTransferProxy, true);
        token.transferOwnership(_msgSender());
        emit Create721Xea12Proxy(beaconProxy);
    }
}
