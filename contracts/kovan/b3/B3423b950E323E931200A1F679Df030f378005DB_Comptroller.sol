    ...
    ReentrancyGuard
{
    ...
    function mintWithEth(
        ...
        emit MintToken(quote, msg.value, token);
    }
        ...
        uint256 available =
            getFreeCollateral(msg.sender, token).add(
                unitPrice
                    .mul(tokenAmount)
                    .mul(
                    getVaultMinimumRatio(msg.sender, token).sub(
                        minMintDeviation
                    )
                )
                    .div(1 ether)
    ...
    /**
     * @notice returns an array of all valid collateral tokens
     * @return collateral the collateral token addresses
     */
    ...
    @param account the user who owns the vault
    @param fxToken the vault to check
    @return ratio the amount of free collateral
     */
    function getVaultCurrentRatio(address account, address fxToken)
        public
        view
        override
        returns (uint256 ratio)
    {
        (uint256 vaultRatio, uint256 count, ) =
            _getVaultRatio(account, fxToken, false);
        if (vaultRatio > 0) {
            ratio = vaultRatio.mul(1000).div(count);
            // Round up to next 1% if there's a remainder
            if (ratio.mod(10) > 0) {
                ratio = ratio.add(1000);
            }
            // Correct the ratio scale
    ...
    {
        (uint256 vaultRatio, uint256 count, uint256 depositTotalEth) =
            _getVaultRatio(account, fxToken, true);

        uint256 locked = 0;
        if (vaultRatio > 0) {
            vaultRatio = vaultRatio.mul(1000).div(count);
            // Round up to next 1% if there's a remainder
            if (vaultRatio.mod(10) > 0) {
                vaultRatio = vaultRatio.add(1000);
            }

            locked = getMinimumCollateral(
                ITreasury(Treasury).getDebt(account, fxToken),
                vaultRatio.div(1000),
                getTokenPrice(fxToken)
            );
        }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < collateralTokens.length; i++) {
            ...
            balance = ITreasury(Treasury).getCollateralBalance(
                account,
                collateralTokens[i],
                fxToken
            );
            ...
            if (balance > 0 && collateralTokens[i] != WETH) {
                if (getDepositAmount) {
                    depositTotalEth = depositTotalEth.add(
                        balance.mul(getTokenPrice(collateralTokens[i]))
                    );
                }
                ratio = ratio.add(
                    collateralDetails[collateralTokens[i]].mintCR
                );
                count = count + 1;
...
interface IComptroller {
    ...
    function getMinimumCollateral(
        ...
        uint256 ratio,
        uint256 unitPrice
    ) external pure returns (uint256 minimum);
