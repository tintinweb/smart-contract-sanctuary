...
contract DragonFactory is Ownable, ERC721Enumerable, ERC721URIStorage {
    ...
    mapping(uint256 => string) private hashToken;
...
constructor(address initialRoyaltiesReceiver) ERC721("Dragon Rascal", "DNFT") {
    ...
    /// @notice Returns all of the tokens owned by the address
    /// @param _owner - the address to query
    /// @return ownerTokens - an array containing the IDs of all tokens owned by the address  
    ...
    /// @notice mints tokens by contract owner 
    /// @param _hash - the ipfs hash of token
    /// @return tokenId - the id of the token
    function reserveNFT(
        string memory _hash
    ) onlyOwner public returns (uint256 tokenId) {
       return _mintSingleNFT(_hash);
    }

    /// @notice mints tokens by user with fee 
    /// @param _hash - the ipfs hash of token
    /// @return tokenId - the id of the token
    function mintNFT( string memory _hash) public payable returns (uint256 tokenId) {
        require(
            msg.value >= PRICE,
    ...
    function _mintSingleNFT(string memory _hash)
    private
    returns (uint256 tokenId)
    {
        ...
        hashToken[newItemId]=_hash;
        _safeMint(msg.sender,newItemId);
        emit Mint(newItemId);
        return newItemId;
    }

    /// @notice get hide max stats & dragon image hash by tokenId 
    /// @param _tokenId - the id of token
    /// @return _hashToken - Hash of json file that contains (max stats & image)
    function GetHashesByToken(uint256 _tokenId) onlyOwner public view returns (string memory _hashToken){
        return hashToken[_tokenId];
    }
}
