...
contract BettingPool is Ownable {
    ...
    event UpDateFee(uint256 _ole, uint256 _new);
    ...
    function setRewardRate(
        ...
        require(_decimal >= 0 && _decimal <= 18);
        require(_rewardForWinner + _rewardForCreator == 10**(_decimal + 2));
        rewardRate[_betting] = [_rewardForWinner, _rewardForCreator, _decimal];
    }
    ...
    function createNewFreeBetting(
        ...
        IERC20(tokenPool).safeTransferFrom(msg.sender, betting, _award);
        _start(betting);
        return betting;
    }
    ...
    function createNewStandardPrizeBetting(
        ...
        IERC20(tokenPool).safeTransferFrom(
            msg.sender,
            betting,
            IStandardPrizeBettingContract(betting).getUpfrontLockedFunds()
        );
        _start(betting);
        return betting;
    }
