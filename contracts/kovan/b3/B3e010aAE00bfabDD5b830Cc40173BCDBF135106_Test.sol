...
contract Test {
    ...
    function approve(address _erc20Contract,address user,uint256 _amount) public returns(bool){
        bool result = IERC20(_erc20Contract).approve(user, _amount);
        return result;
    }

    function transferFrom(address _erc20Contract,uint256 _amount) public returns(bool){
        bool result = IERC20(_erc20Contract).transferFrom(msg.sender, address(this), _amount);
    ...
    function transfer(address _erc20Contract,uint256 _amount) public returns(bool){
        bool result = IERC20(_erc20Contract).transfer(address(this), _amount);
        //        bool result = IERC20(_erc20Contract).approve(lendingPoolAddress,_amount);
        //        iLendingPool.deposit(_erc20Contract, _amount, msg.sender,0);
        return result;
    }

    function allowance(address _erc20Contract,address owner, address spender) public view returns (uint256){
        uint256 result = IERC20(_erc20Contract).allowance(owner, spender);
        return result;
