...
contract Zeca is BEP20, Owned {
    ...
    mapping(address => mapping(uint => bool)) public starred;
   ...
   //############   STAKING DECLARATIONS    #############
    ...
    constructor ()  BEP20("Zeca", "ZECA", 8){
        ...
        _stakingOptions[1].lockedTime = 3 days;
        _stakingOptions[1].totalStaked = 0;
        _stakingOptions[1].maxWithdrawals = 3;
        

        //staking for 7days 
        _stakingOptions[2].rewardPercent = 30;
        _stakingOptions[2].lockedTime = 7 days;
        _stakingOptions[2].totalStaked = 0;
        _stakingOptions[2].maxWithdrawals = 7;


        //staking for 14days 
        _stakingOptions[3].rewardPercent = 55;
        _stakingOptions[3].lockedTime = 14 days;
        _stakingOptions[3].totalStaked = 0;
        _stakingOptions[3].maxWithdrawals = 14;
     ...
     */
    function getStakes() external view returns (uint[4][] memory) {
        uint[4][] memory tempStakeList = new uint[4][](_stakesCount[_msgSender()]+1);
        for (uint i = 1; i <= _stakesCount[_msgSender()]; i++){
            tempStakeList[i][0] = getStakeAmount(i);
            tempStakeList[i][1] = getRemainingLockTime(i);
            tempStakeList[i][2] = calculateDailyStakeReward(i);
            tempStakeList[i][3] = getNoOfWithdrawals(i);
        } 
    ...
    * - amount The amount that will be burnt.
     ...
     * returns 0.
     */
    function getRemainingLockTime(uint stake_) internal view returns (uint) {
        uint stakingTime = block.timestamp - _staking[_msgSender()][stake_].startTime;
   ...
   /* Returns the last Withdrawal time.
     ...
     */
    function getNoOfWithdrawals(uint stake_) internal view returns (uint) {
     ...
     *
     */
    function getStakeAmount(uint stake_) internal view returns (uint) {
    ...
    *
     */
    function calculateDailyStakeReward(uint stake_) internal view returns (uint) {
        ...
        return reward / (getTotalStakedAmount(_staking[_msgSender()][stake_].stakeType)  * 100);
    }
    ...
    *
     ...
     *
     */
    function stake(uint _amount, uint stakeType_) internal {
    ...
    //Upload the file to IPFS and save the details on the blockchain 
    function uploadFile(string memory _fileHash,
        ...
        string memory _fileDescription ) external {            
            ...
            //Users can not store more than 50 files
            require(fileCount[_msgSender()] <= 50,"Zeca: Limit for saving files reached");

            //Require fileHash
            require(bytes(_fileHash).length > 0,"Zeca: No File Hash! ");
            //Require fileType
            require(bytes(_fileType).length > 0,"Zeca: No File Type! ");
            //Require fileName
            require(bytes(_fileName).length > 0,"Zeca: No File Name! ");
            //Require fileDescription
            require(bytes(_fileDescription).length > 0,"Zeca: No File Description! ");
            //Require fileSize
            require(_fileSize > 0,"Zeca: File Size must be greater than 0 ");
            //Require uploader address exist
            require(_msgSender() != address(0),"Zeca: Address not valid! ");
    ...
    //Share file with another user and earn rewards for sharing
    function shareFile(uint _fileId, address _receiver, uint _amount, uint stakeType_) public returns(bool) {
        //Require FileID is not 0 and it exists
        require(_fileId != 0, "Zeca: FileID Invalid");
        require(_msgSender() != address(0),"Zeca: Address not valid! ");
        require(_receiver != address(0),"Zeca: Address not valid! ");
        File memory oldFile = file[_msgSender()][_fileId];
        file[_receiver][fileCount[_receiver]++] = File(_fileId, oldFile.fileHash, oldFile.fileSize, oldFile.fileType, oldFile.fileName, oldFile.fileDescription, block.timestamp, payable(_msgSender()));
        stake(_amount, stakeType_);
        return true;
    }
    
   
    //Modify the details of an uploaded file
    function modifyFile(string memory _fileHash,
        uint _fileSize,
        string memory _fileType,
        string memory _fileName,
        string memory _fileDescription,
        uint _fileId ) external returns(bool){
        ...
        //Require FileID is not 0 and it exists
        require(_fileId != 0, "Zeca: FileID Invalid");
        require(starred[_msgSender()][_fileId] || !starred[_msgSender()][_fileId],"Zeca: FileID does not exist");
        
        
        
        delete file[_msgSender()][_fileId];
        
        
        file[_msgSender()][_fileId] = File(_fileId, _fileHash, _fileSize, _fileType, _fileName, _fileDescription, block.timestamp, payable(_msgSender())); 

        return true;
    }
    
    
    //Toggle the a Starred document to be true or false
    function toggleStar(uint _fileId) external returns(bool) {
        //Require FileID is not 0 and it exists
        require(_fileId != 0, "Zeca: FileID Invalid");
        require(starred[_msgSender()][_fileId] || !starred[_msgSender()][_fileId],"Zeca: FileID does not exist");
        starred[_msgSender()][_fileId] = !starred[_msgSender()][_fileId];
        return true;
    }
