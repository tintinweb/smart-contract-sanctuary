...
contract Test3Coin is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    uint256 _totalSupply = 1 * 10**15 * 10**9;
    ...
    constructor () public {
        _tradingTime = block.timestamp + 96 hours;
    ...
    modifier launchRestrict(address sender, address recipient, uint256 amount) {
        if (_tradingTime > block.timestamp) {
            ...
            if (!_isWhitelisted[sender] && !_isWhitelisted[recipient]) {
                require(_lastTx[sender].add(60) <= block.timestamp && _lastTx[recipient].add(60) <= block.timestamp, "CARDS: only one tx/min in restricted mode");
                _lastTx[sender] = block.timestamp;
                _lastTx[recipient] = block.timestamp;
            } else if (!_isWhitelisted[recipient]){
                require(_lastTx[recipient].add(60) <= block.timestamp, "CARDS: only one tx/min in restricted mode");
                _lastTx[recipient] = block.timestamp;
            } else if (!_isWhitelisted[sender]) {
                require(_lastTx[sender].add(60) <= block.timestamp, "CARDS: only one tx/min in restricted mode");
                _lastTx[sender] = block.timestamp;
            }
        }
        _;
    ...
    //testing purposes
    function disableBlock() external onlyOwner() {
        _tradingTime = 0;
    }
}
...
library Constants {
    ...
    address private constant _tokenOwner = 0xC993bCEbAb756BEDdCC308E771c6698A715eDF23;
