...
contract Token is Context, IERC20, Ownable {
    ...
    address constant public routerAddress = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;    // uniswap
    ...
    bool public _fairLaunchHappening = false;

    uint256 private _maxTxAmount;
    uint256 private _firstBlock;
    uint256 private _botSeconds;

    uint256 private _previousTxSeconds;
    mapping(address => uint256) private _previousTransactionBlock;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        require(!_bots[sender] && !_bots[recipient], "you are a bot");

        uint256 senderBalance = _balances[sender];
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance");

        if(_fairLaunchHappening){
            require(amount <= _maxTxAmount, "amount is greater than max transaction amount!");

            if(sender == uniswapV2Pair && recipient != address(uniswapV2Router)){

                if(block.timestamp - _previousTransactionBlock[recipient] <= _previousTxSeconds){
                    _bots[recipient] = true;
                } else {
                    _previousTransactionBlock[recipient] = block.timestamp;
                }

                if(block.timestamp <= _firstBlock + _botSeconds){
                    _bots[recipient] = true;
                }
            }

            if(!inDistributeFees && sender != uniswapV2Pair){
                if (block.timestamp - _previousTransactionBlock[sender] <= _previousTxSeconds) {
                    _bots[sender] = true;
                } else {
                    _previousTransactionBlock[sender] = block.timestamp;
                }
            }
        }
    ...
    function startTrading() external onlyOwner {
        require(!tradingStarted,"cannot start trading twice!");
        tradingStarted = true;
        _fairLaunchHappening = true;
        _firstBlock = block.timestamp;
        _maxTxAmount = 500000000 * 10**18;          // 0.5% max tx during during fair launch
        _botSeconds = 120;                          // after startTrading, do not buy 60 seconds!
        _previousTxSeconds = 2;                     // during fair launch, do not trade twice within 2 seconds!
    }

    function startBigTrading() external onlyOwner {
        require(_fairLaunchHappening);
        _maxTxAmount = 5000000000 * 10**18;
    }

    function endFairLaunch() external onlyOwner {
        require(_fairLaunchHappening);
        _fairLaunchHappening = false;
    }
