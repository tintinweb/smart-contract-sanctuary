...
contract AlgebraPool is PoolState, PoolImmutables, IAlgebraPool {
    ...
    function swap(
        ...
        (
            ...
            currentLiquidity,
            timepointIndex
        ) = _calculateSwap(zeroForOne, amountRequired, limitSqrtPrice);
        ...
        )
    {
        ...
        if (zeroForOne) {
            ...
            if (cache.communityFeeAccumulated > 0) communityFees.token0 += uint128(cache.communityFeeAccumulated);
        } else {
            totalFeeGrowth1Token = cache.totalFeeGrowth;
            if (cache.communityFeeAccumulated > 0) communityFees.token1 += uint128(cache.communityFeeAccumulated);
        }
    }
    ...
    function flash(
        ...
        uint256 fee0 = FullMath.mulDivRoundingUp(amount0, _fee, 1e6);
        uint256 fee1 = FullMath.mulDivRoundingUp(amount1, _fee, 1e6);
        uint256 balance0Before = balanceToken0();
        uint256 balance1Before = balanceToken1();
        ...
        if (paid0 > 0) {
            ...
            uint256 fees0 = _communityFeeToken0 == 0 ? 0 : paid0 / _communityFeeToken0;
        ...
        if (paid1 > 0) {
            ...
            uint256 fees1 = _communityFeeToken1 == 0 ? 0 : paid1 / _communityFeeToken1;
    ...
    function collectCommunityFee(
        ...
        (uint128 feesToken0, uint128 feesToken1) = (communityFees.token0, communityFees.token1); // SLOAD minimization

        amount0 = amount0Requested > feesToken0 ? feesToken0 : amount0Requested;
        amount1 = amount1Requested > feesToken1 ? feesToken1 : amount1Requested;

        if (amount0 > 0) {
            if (amount0 == feesToken0) amount0--; // communityFees should be at least 1 but not 0 (gas saving)
            communityFees.token0 -= amount0;
            TransferHelper.safeTransfer(token0, recipient, amount0);
        }
        if (amount1 > 0) {
            if (amount1 == feesToken1) amount1--; // communityFees should be at least 1 but not 0 (gas saving)
