   ...
   struct PoolInfo{
    ...
    function getNetValues() view public returns(NetValue[] memory netValues){
        ...
        for(uint256 i = 0; i < poolInfos.length; i++){
            ...
            (bool flag,) = totalAmountInUSD.tryMul(poolInfos[i].weight);
            require(flag,"mul failed 1");
            ...
            uint256 temp = poolInfos[i].profitWeight;
            (flag,) = totalProfitAmountInUSD.tryMul(temp);
            require(flag,"mul failed 2");
            netValue.totalTokensInUSD = netValue.amountInUSD.add(totalProfitWeight == 0 ? 0 : totalProfitAmountInUSD.mul(temp).div(totalProfitWeight)).add(allocatedProfitInUSD);
