...
contract UniswapV2Handler is Ownerable, ISwap {
    ...
    function swapEstimateOutWithPath(address tokenIn, address tokenOut, uint256 amountIn) external view returns (uint256 amountOut, address[] memory path) {
        (amountOut, path) = _getBestOut(tokenIn, tokenOut, amountIn);
    }

    function swapEstimateInWithPath(address tokenIn, address tokenOut, uint256 amountOut) external view returns (uint256 amountIn, address[] memory path) {
        (amountIn, path) = _getBestIn(tokenIn, tokenOut, amountOut);
    }
    ...
    function _getBestIn(address tokenIn, address tokenOut, uint256 amountOut) internal view returns (uint256, address[] memory) {
        ...
        for (uint256 i = 0; i < baseTokens.length; i++) {
            ...
            if (factory.getPair(baseTokens[i], tokenOut) != address(0)) {
                ...
                if (amounts[0] > 0) {
                    if (resultAmount > amounts[0] || resultAmount == 0) {
                        resultAmount = amounts[0];
                        resultPath = tempPath;
                    }
                }
