...
contract Tipping is Ownable {
    ...
    function transfer(address to, uint256 amount) public returns (bool) {
        ...
        _transferFrom(msg.sender, to, transAmt);
        emit Transfer(msg.sender, to, transAmt);
        _transferFrom(msg.sender, address(0), burnAmt);
        emit Burn(msg.sender, burnAmt);
        _transferFrom(msg.sender, _FUND_VAULT, fundAmt);
        emit Transfer(msg.sender, _FUND_VAULT, fundAmt);
        _transferFrom(msg.sender, _STAKING_VAULT, rewardAmt);
        emit Transfer(msg.sender, _STAKING_VAULT, rewardAmt);
    ...
    function _transferFrom(address _from, address _to, uint256 _amount) private {
        IERC20(_LIBERTAS).transferFrom(_from, _to, _amount);
    }

    function _transfer(address _to, uint256 _amount) private {
        IERC20(_LIBERTAS).transfer(_to, _amount);
    }
