...
//    }
    ...
    function swapPokeSwap(address smartPoolAddress, address tokenIn, uint inAmount, address tokenOut) public returns(uint){
        ...
        // sell the token
        (uint tokenAmountOut,) = bPool.swapExactAmountIn(tokenIn, inAmount, tokenOut, 0, uint(-1));

        // do the poke
        crpPool.pokeWeights();

        // buy the token back
        (uint tokenAmountOut2,) = bPool.swapExactAmountIn(tokenOut, tokenAmountOut, tokenIn, 0, uint(-1));
