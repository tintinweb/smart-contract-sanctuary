...
contract CollateralizationManager is ICollateralizationManager, Ownable
{
  ...
  function _isValidPairing (address tokenRegistry, address debtToken, address collateralToken) private view returns (bool)
  {
    return debtToken != _KAE && debtToken != collateralToken && ITokenRegistry(tokenRegistry).isValidPair(debtToken, collateralToken);
  }

  /*function isSufficientInitialCollateral (address tokenRegistry, address oracle, address debtToken, uint256 debt, address collateralToken, uint256 collateral) external view override returns (bool)
  {
    require(_isValidPairing(tokenRegistry, debtToken, collateralToken), "!valid pair");
    ...
    uint256 debtInCollateralTokenAtInitRatio = IOracle(oracle).convert(debtToken, collateralToken, _calcPercentOf(debt, _ratio[collateralToken].init));
