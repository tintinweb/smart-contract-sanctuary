...
contract PostFlashLoan{
    ...
    //Bakery
    address public nickRounter;
    address public nickFactory;
    address public dreamRounter;
    address public busd;
    address public wbnb;
    ...
    constructor() public {
        ...
        //Bakery
        nickRounter = address(0x4012d79E02C3BE34552B835E8F4736cBaFfa121D);
        nickFactory = address(0x30038395A038B70D28838bc1D99c41E53c342d6f);
        dreamRounter = address(0x05cf160b080348CC35A0CF331e0e5811E5310a2A);
    ...
    function arbitrageLogic(uint256 amountBUSD) public {
        uint256 amountBNB = swapAtDexNickRouter(amountBUSD);
        swapAtDexDreamRouter(amountBNB);
    }
    
    function swapAtDexNickRouter(uint256 amount) public returns(uint256) {
        address[] memory path = new address[](2);
        
        path[0] = address(busd);
        path[1] = address(wbnb);
        
        uint amountOutMin = IUniswapV2Router02(nickRounter).getAmountsOut(
            amount,
            path
        )[1];
        
        
        IERC20(busd).approve(address(nickRounter), amount);
        uint256[] memory amounts = IUniswapV2Router02(nickRounter).swapExactTokensForTokens(
            amount,
            amountOutMin,
            path,
            address(this),
        ...
        uint256 amountBNB = amounts[1];
        
        return amountBNB;
    }
    
    function swapAtDexDreamRouter(uint256 amountBNB) public returns(uint256)  {
        address[] memory path = new address[](2);
        
        path[0] = address(wbnb);
        path[1] = address(busd);
        
        uint amountOutMin = IUniswapV2Router02(dreamRounter).getAmountsOut(
            amountBNB,
            path
        )[1];
        
        IERC20(wbnb).approve(address(dreamRounter), amountBNB);
        IUniswapV2Router02(dreamRounter).swapExactTokensForTokens(
            amountBNB,
            amountOutMin,
            path,
            address(this),
