...
contract Volume is ERC20, ReentrancyGuard {
    ...
    mapping (address => uint8) private _freeloaders; // freeLoaders do not pay fuel on transactions 
    ...
    constructor (address escrow_ , address multiSig_) ERC20("Volume", "VOL") {
        ...
        _freeloaders[escrow_] = 1; // escrow is a freeloader 
    }
    ...
    modifier onlyIfCallerIs(address allowedCaller_) {
        require(_msgSender() == allowedCaller_, "Volume BEP20: caller is not the Escrow");
        _;
    }
    ...
    function addFreeLoader(address newfreeloader_) external onlyIfCallerIs(multiSig) {
            _freeloaders[newfreeloader_] = 1;
    }

    /**
        @dev adds an address to the freeloader, freeloaders are addresses that ignore the fuel function so their transactions don't add fuel
     */
    function removeFreeLoader(address newfreeloader_) external onlyIfCallerIs(multiSig) {
        require(newfreeloader_ != escrow , "Volume: escrow will always be a freeloader");
        _freeloaders[newfreeloader_] = 0;
    }
     ...
     */
    function directRefuel(uint256 fuel_) external returns (bool) {
        if (!_fly())
            return false; // Crashed

        _refuel(fuel_ /**Make this fuel a bit as efficient as if it was issued by a normal transaction */, fuel_, _msgSender()); 

        _burn(_msgSender(), fuel_);
     ...
     */
    function directBurnFromEscrow(uint256 amount_) external onlyIfCallerIs(escrow) {
        _burn(escrow, amount_);
    }
    ...
    function isFreeLoader(address potentialFreeloader_) external view returns (uint8){
    ...
    function _refuel(uint256 transferAmount_, uint256 refuelAmount_, address fueler_) private {
        // Calculate the % of supply that gets refueled
        uint256 fuel = refuelAmount_ * BASE * BASE / (totalSupply() - transferAmount_) / BASE * 300;

        userFuelPile[fueler_] += (fuelTank + userFuelPile[fueler_]) * fuel / BASE;

        if (userFuelPile[fueler_] > BASE) {
            uint256 leftOnUserPile = userFuelPile[fueler_] % BASE; //Leaving any fractional blocks on user pile
            uint256 integerUserFuel = userFuelPile[fueler_] - leftOnUserPile; // Separating the personally accumulated full blocks from the total pile
            ...
            userFuelPile[fueler_] = leftOnUserPile; // Resetting user pile to contain only fractional blocks
        }
    ...
    function _volumeTransfer(address sender_ , address recipient_ , uint256 amount_ ) internal returns (bool) {
        ...
        if(_freeloaders[sender_] == 0 && _freeloaders[recipient_] == 0) {
            // pay fuel
            ...
            if(sender_ != _getLPAddress())
                _refuel(amount_, fuel, sender_);
            else 
                _refuel(amount_, fuel, recipient_); // if the LP is the sender then add this fuel to who ever swapped wbnb for VOL
            _burn(sender_, fuel);
        }
...
contract TestVolume is Volume {
    ...
    constructor (address escrowAcc, address mulitisig) Volume(escrowAcc , mulitisig) {}
