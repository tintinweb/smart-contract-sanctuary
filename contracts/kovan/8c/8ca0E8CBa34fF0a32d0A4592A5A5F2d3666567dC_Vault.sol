    ...
    ReentrancyGuard
{
    ...
    function performOptimalSwap(
        ...
        uint256 ratio = mulDiv(actualAmount1, factor, actualAmount0);
        (uint160 sqrtPriceX96, , , , , , ) = pool.slot0();
        uint256 price = uint256(sqrtPriceX96).mul(uint256(sqrtPriceX96)).mul(
            factor
        ) >> (96 * 2);
        //console.log("price", price);
        uint256 token0Converted = mulDiv(ratio, amount0, factor);
        bool isToken0Excess = amount1 < token0Converted;

        uint256 excessAmount = isToken0Excess ? mulDiv(token0Converted.sub(amount1), factor, ratio) : amount1.sub(token0Converted);
        uint256 amountOut;
        uint256 amountIn = isToken0Excess
            ? mulDiv(excessAmount, ratio, price.add(ratio))
            : mulDiv(excessAmount, price, price.add(ratio));
