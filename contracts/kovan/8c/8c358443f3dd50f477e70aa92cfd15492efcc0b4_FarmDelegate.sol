...
contract FarmDelegate is FarmStorageV1 {
    ...
    event GanPerBlockSet(uint256 newGanPerBlock1,uint256 newGanPerBlock2);
    ...
    event UpdatePool(uint256 pid,uint256 ganAmount,uint256 usdtAmount,uint256 updateBlockHeight);
    function initialize(
        address _Gan,
        address _usdt,
        uint256 _block0,
    ...
    function setGanPerBlock(uint256 newPerBlock0, uint256 newPerBlock1) public {
        ...
        emit GanPerBlockSet(newPerBlock0,newPerBlock1);
    }
    ...
    function getEndBlock(uint256 restReward) internal view returns(uint256 blockHeightEnd){
        uint256 blockHeightBegin = block.number > startBlockOfGan ? block.number : startBlockOfGan;
    ...
    function _getEndBlock(uint256 blockHeightBegin, uint256 rewardPending)internal view returns(uint256 blockHeightEnd,uint256 rewardRest){
        uint256 halvingPeriod = halvingPeriod0.add(halvingPeriod1);
        uint256 _phase = blockHeightBegin.sub(startBlockOfGan).mod(halvingPeriod) == 0 && blockHeightBegin > startBlockOfGan ? phase(blockHeightBegin)+1 : phase(blockHeightBegin);
        require(ganPerBlockContinuousMining.div(2 ** _phase) > 0 && ganPerBlockConcentratedMining.div(2 ** _phase) > 0,"error");
        uint256 switchBlock = _phase.mul(halvingPeriod).add(startBlockOfGan).add(halvingPeriod0);
        uint256 _reward;
        if(switchBlock > blockHeightBegin){ 
           _reward = switchBlock.sub(blockHeightBegin).mul(ganPerBlockConcentratedMining.div(2 ** _phase));      
           if(rewardPending >= _reward)
               return(switchBlock,rewardPending.sub(_reward));
           else
               return(blockHeightBegin.add(rewardPending.div(ganPerBlockConcentratedMining.div(2 ** _phase))),0);
         }
        else{
            uint256 endRewardBlock = (_phase + 1).mul(halvingPeriod).add(startBlockOfGan);
            _reward = endRewardBlock.sub(blockHeightBegin).mul(ganPerBlockContinuousMining.div(2 ** _phase));
            if(rewardPending >= _reward)
                return(endRewardBlock,rewardPending.sub(_reward));
            else
                return(blockHeightBegin.add(rewardPending.div(ganPerBlockContinuousMining.div(2 ** _phase))),0);
        }
    }
    ...
    function get2(uint256 par)public view returns(uint){
      return 2**par;
    }
}