...
contract TRENDY is Context, IERC20, Ownable {
    ...
    string private _name = "Trendy Test 2";
    string private _symbol = "TRENDY2";
    ...
    uint256 private minimumTokensBeforeSwap = 2 * 10**8 * 10**5;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && to == uniswapV2Pair) {
            ...
            if (buyBackEnabled && balance > amountInEth) {
                
                if (amountInEth < buyBackUpperLimit) {
    ...
    function getEquivalentInEth(uint256 amountOfToken) public view returns(uint) {
        (uint Res0, uint Res1,) = pair.getReserves();
        return ((amountOfToken.mul(Res1)).div(Res0));
    }

    function getEquivalentInToken(uint256 amountOfEth) public view returns(uint) {
        (uint Res0, uint Res1,) = pair.getReserves();
        return ((amountOfEth.mul(Res0)).div(Res1));
    }

    function getReservesInPool() public view returns(uint, uint) {
        (uint Res0, uint Res1,) = pair.getReserves();
        return (Res0, Res1);
    }
     ...
     */
    function paycheck(uint256 amount, address recipient) private {
    ...
    function isAValidHolder(address wallet) public view returns(bool) {
    ...
    function getMarketingDivisor() public view returns (uint256) {
    ...
    function getMinimumTokensBeforeSwapAmount() public view returns (uint256) {
        return minimumTokensBeforeSwap;
    }
    
    function getBuyBackUpperLimitAmount() public view returns (uint256) {
    ...
    function setMarketingDivisor(uint256 divisor) external onlyTrendyAdmin {
        ...
        require(marketingDivisor != divisor, "This variable already has this value");
    ...
    function setNumTokensSellToAddToLiquidity(uint256 _minimumTokensBeforeSwap) external onlyTrendyAdmin {
        require(_minimumTokensBeforeSwap < 2 * 10**8 * 10**5, "Minimum token before swap cannot exceed the 0.02% of the totalSupply");
        require(minimumTokensBeforeSwap != _minimumTokensBeforeSwap, "This variable already has this value");
    ...
    function setBuybackUpperLimit(uint256 buyBackLimit) external onlyTrendyAdmin {
        ...
        require(buyBackUpperLimit != buyBackLimit, "This variable already has this value");
        buyBackUpperLimit = buyBackLimit * 10**18;
    }

    function setBuyBackEnabled(bool _enabled) external onlyTrendyAdmin {
        require(buyBackEnabled != _enabled, "This variable already has this value");
        buyBackEnabled = _enabled;
        emit BuyBackEnabledUpdated(_enabled);
    }

    function setSwapAndLiquifyEnabled(bool _enabled) external onlyTrendyAdmin {
        require(swapAndLiquifyEnabled != _enabled, "This variable already has this value");
    ...
    function sellTokensFromTrendyStoreToBuyProducts(uint256 tokenAmount) external onlyTrendyAdmin {
        ...
        _approve(trendyAdmin, address(uniswapV2Router), tokenAmount);
