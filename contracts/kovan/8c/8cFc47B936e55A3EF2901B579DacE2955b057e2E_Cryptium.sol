...
contract Cryptium is CrptERC20, Control, ReentrancyGuard {
		...
		)
	{
		...
		if (monthsPassFromRebase == 0) {
			...
			price2 = (_coeffInflations * monthsPassAll * _initialPrice) / 12 + _initialPrice;
		}

		if (price1 > price2 && price1 > curPrice) {
			supplyDelta = (_totalSupply * ((oldPrice * 1000) / newPrice)) / 1000;
		} else if (price2 > curPrice) {
			price2 = (_coeffInflations * monthsPassFromRebase * _initialPrice) / 12 + _initialPrice;
			supplyDelta = (_totalSupply * ((price2 * 100) / curPrice)) / 1000;
			isInflRebase = true;
		} else {
			isFall = false;
		}
