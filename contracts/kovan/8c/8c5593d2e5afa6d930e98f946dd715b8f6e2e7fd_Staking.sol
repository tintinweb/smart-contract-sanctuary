...
contract Staking is AccessControl {
  ...
  constructor (address pair, address yield) public {
    ...
    require(address(YLD).isContract(), "Staking: YIELD is not a contract");
    require(address(PAIR) != address(YLD), "Staking: pair and YIELD are the same");
  }
  ...
  function getReward() public updateReward(msg.sender) {
    ...
    if (reward > 0) {
      ...
      YLD.safeTransfer(msg.sender, reward);
  ...
  function pairInfo() public view returns (uint256 yield, uint256 other, uint256 totalSupply) {
    ...
    (yield, other) = address(YLD) == PAIR.token0() ? (reserves0, reserves1) : (reserves1, reserves0);
  }
