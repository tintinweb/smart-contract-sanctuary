  ...
  LendingPoolStorage
{
  ...
  function liquidationCall(
    ...
    vars.collateralAtoken = IMToken(collateralReserve.aTokenAddress);
...
library ReserveLogic {
  ...
  function _mintToTreasury(
    ...
    if (vars.amountToMint != 0) {
      IMToken(reserve.aTokenAddress).mintToTreasury(vars.amountToMint, newLiquidityIndex);
    }
  }
    ...
    )
  {
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      if (user != address(0)) {
        ...
        userReservesData[i].scaledATokenBalance = IMToken(reserveData.aTokenAddress)
...
contract MoveProtocolDataProvider {
  using ReserveConfiguration for DataTypes.ReserveConfigurationMap;
  using UserConfiguration for DataTypes.UserConfigurationMap;
...
contract MoveOracle is IPriceOracleGetter, Ownable {
  using SafeERC20 for IERC20;
...
contract LendingPool is VersionedInitializable, ILendingPool, LendingPoolStorage {
  ...
  function deposit(
    ...
    bool isFirstDeposit = IMToken(aToken).mint(onBehalfOf, amount, reserve.liquidityIndex);
  ...
  function withdraw(
    ...
    uint256 userBalance = IMToken(aToken).balanceOf(msg.sender);
    ...
    IMToken(aToken).burn(msg.sender, to, amountToWithdraw, reserve.liquidityIndex);
  ...
  function repay(
    ...
    IMToken(aToken).handleRepayment(msg.sender, paybackAmount);
  ...
  function flashLoan(
    ...
    for (vars.i = 0; vars.i < assets.length; vars.i++) {
      ...
      IMToken(aTokenAddresses[vars.i]).transferUnderlyingTo(receiverAddress, amounts[vars.i]);
    }
  ...
  function _executeBorrow(ExecuteBorrowParams memory vars) internal {
    ...
    if (vars.releaseUnderlying) {
      IMToken(vars.aTokenAddress).transferUnderlyingTo(vars.user, vars.amount);
    }
...

{
  ...
  "libraries": {
    "contracts/protocol/libraries/logic/ReserveLogic.sol": {
      "ReserveLogic": "0xe2d4c197bbb36c666f6796048cfca12226ee530e"
    },
    "contracts/protocol/libraries/logic/ValidationLogic.sol": {
      "ValidationLogic": "0x9e60bd616361a6b40ef55c973bd7e3c0d87cc88b"
    }
  }
}