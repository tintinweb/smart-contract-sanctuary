...
contract Arb is Ownable, Initializable {

    address public usdc = address(0x17D484e98402321551d39CF4A0050B18a343780F);

    address public gydLib = address(0x29e858A3d3AE4ab92426c8C279f8E8ae64Edfda7);

    address public gyd = address(0xd0474aEBA181987A81352842d446Fc6c65481417);

    address public router = address(0x4e67bf5bD28Dd4b570FBAFe11D0633eCbA2754Ec);

    address public pool = address(0x6EAe312f00B4EE0640BF4e3D0A08Cda36F206bcc);

    

    function initApprove() external onlyOwner {
        uint256 approveAmount = uint256(-1);
        ERC20(usdc).approve(router, approveAmount);
        ERC20(gyd).approve(gydLib, approveAmount);
    }
    ...
    function arb(uint256 inputAmount,uint minTotalAmountOut,uint limitReturnAmount,uint maxPrice) external onlyOwner {
        address tokenIn = usdc;

        address tokenOut = gyd;

        ERC20(tokenIn).transferFrom(msg.sender, address(this), inputAmount);
        
        
        PoolInterface poolInstance = PoolInterface(pool);

        TokenInterface SwapTokenIn = TokenInterface(tokenIn);

        SwapTokenIn.approve(pool,inputAmount);

        (uint tokenAmountOut,) = poolInstance.swapExactAmountIn(tokenIn,inputAmount,tokenOut,limitReturnAmount,maxPrice);

        require(tokenAmountOut >= minTotalAmountOut, "arb: ERR_LIMIT_OUT");
        ...
        uint256 pammOutAmount = GYDLib(gydLib).redeemToUnderlyingTokens(
            tokensOut,
            amountsOut,
            tokenAmountOut
        );
        require(pammOutAmount > inputAmount, "No Profit");
    }
