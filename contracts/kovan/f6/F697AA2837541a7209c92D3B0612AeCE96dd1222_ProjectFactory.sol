...
contract ProjectFactory is IProjectFactory {
    ...
    constructor(
        ...
        require(_priceOralceAggregator != address(0), 'PJFAC: INVALID_AGGREGATOR');
    ...
    function createProject(
        ...
        for (uint256 i = 0; i < length; i++) {
            require(isAcceptedToken[_acceptedTokens[i]], 'PJFAC: NOT_PROTOCOL_ACCEPTED_TOKEN');
            priceOralceAggregator.viewPriceInUSD(_acceptedTokens[i]);
        }

        // calc projrectHash
        bytes32 pHash = keccak256(abi.encodePacked(msg.sender, _beneficiary, _name, _threshold));
