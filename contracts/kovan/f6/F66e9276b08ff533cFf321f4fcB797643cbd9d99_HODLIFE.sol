...
contract HODLIFE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public immutable buyBackFees;
    uint256 public immutable yfFees;
         
    address payable yfWallet = 0x009f1D5aDf0dba4bBec722Bf050932B7C73138aD;
    ...
    // should not be able to make two transfers in a 10 minutes
    ...
    constructor() public ERC20("HODLIFE", "HODLIFE") {
        uint256 _BNBRewardsFee = 8;
        uint256 _liquidityFee = 5;
        uint256 _buyBackFees = 3;
        uint256 _yfFees = 2;
        ...
        totalFees = _BNBRewardsFee.add(_liquidityFee).add(_buyBackFees).add(_yfFees);
        ...
        address _bounceFixedSaleWallet = 0x009f1D5aDf0dba4bBec722Bf050932B7C73138aD;
    ...
    function _transfer(
        ...
        if(isFixedSaleBuy) {
            require(block.timestamp >= fixedSaleStartTimestamp);
            ...
            if(!openToEveryone) {
                require(fixedSaleEarlyParticipants[to]);
            }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 dividends = (address(this).balance).mul(BNBRewardsFee).div(totalFees.sub(liquidityFee));
        uint256 buyBack = (address(this).balance).mul(buyBackFees).div(totalFees.sub(liquidityFee));
        uint256 yf = (address(this).balance).sub(buyBack).sub(dividends);
        yfWallet.transfer(yf);
        swapETHForTokens(buyBack);
        (bool success,) = address(dividendTracker).call{value: dividends}("");

        if(success) {
   	 		emit SendDividends(tokens, dividends, buyBack, yf);
        }
    }
