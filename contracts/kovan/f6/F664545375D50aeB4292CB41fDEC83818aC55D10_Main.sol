...
contract Main is ERC721Full {
    ...
    mapping(address => uint256[]) private tickets;
     ...
     // Mapping event ID to organiser address
    mapping(uint256 => address) private events;
    ...
    function buyTicket(uint256 eventId,string memory _tokenURI) public payable returns (bool) {
        require(bytes(_tokenURI).length > 0, "Invalid URI");
        require(eventCount > 0, "Invalid Event ID.");
        ...
        address payable owner = address(uint160(events[eventId]));
    ...
    function createEvent(address _organiser) external returns (bool) {
        require(_organiser != address(0), "Zero address");
        events[eventCount]=_organiser;
        eventCount++;
        return true;
    }
