...
contract LandRegistration is ILandRegistration {
    ...
    modifier onlyAdmins {
        require(IManagementCompany(MCBoard).isMCAdmin(msg.sender) == true, "LRSC: FORBIDDEN ONLY ADMIN");
        _;
    }

    // only MC admins can procced
    modifier onlyLOSC {
        require(msg.sender == IManagementCompany(MCBoard).LOSCAddress(), "LRSC: FORBIDDEN ONLY LOSC");
        _;
    }
    ...
    modifier onlyLPSC {
        ...
        require(ILoanOriginator(LOSCAddress).isLoanPoolValid(msg.sender) == true, "LRSC: FORBIDDEN ONLY LPSC");
        _;
    }
    ...
    function addNewDeveloper(
        ...
        require(bytes(_companyName).length > 0, "LRSC: EMPTY_COMPANY_NAME");
        require(bytes(_location).length > 0,    "LRSC: EMPTY_LOCATION");
        require(bytes(_note).length > 0,        "LRSC: EMPTY_NOTE");
    ...
    function updateDeveloper(
        ...
        require(isDeveloperIDValid[_developerID],   "LRSC: INVALID_ID");
        require(bytes(_companyName).length > 0,     "LRSC: EMPTY_COMPANY_NAME");
        require(bytes(_location).length > 0,        "LRSC: EMPTY_LOCATION");
        require(bytes(_note).length > 0,            "LRSC: EMPTY_NOTE");
    ...
    function deleteDeveloper(uint _developerID) external override onlyAdmins {
        require(isDeveloperIDValid[_developerID], "LRSC: INVALID_ID");
        require(getDeveloperByID[_developerID].myActiveLandIDs.length == 1, "LRSC: LANDS_STILL_EXIST_IN_DEVELOPER");
    ...
    function addNewLand(
        ...
        require(_propertyIdentificationNumber != 0, "LRSC: INVALID_PIN");
        require(bytes(_propertyAddress).length > 0, "LRSC: EMPTY_ADDRESS");
        require(bytes(_propertyZoning).length > 0,  "LRSC: EMPTY_ZONE");
        require(bytes(_note).length > 0,            "LRSC: EMPTY_NOTE");
        require(isDeveloperIDValid[_developerID],   "LRSC: DEVELOPER_INVALID");
    ...
    function updateLandBasicInfo(
        ...
        require(isLandIDValid[_landID],             "LRSC: INVALID_LAND_ID");
        require(_propertyIdentificationNumber > 0,  "LRSC: INVALID_PIN");
        require(bytes(_propertyAddress).length > 0, "LRSC: EMPTY_ADDRESS");
        require(bytes(_propertyZoning).length > 0,  "LRSC: EMPRY_ZONE");
    ...
    function addOrUpdateLandAppraisal(
        ...
        require(
            ...
            "LRSC: INVALID_APPRAISIAL_DISCOUNT"
        );
    ...
    function updateAmountBorrowedByDeveloper(
        ...
        Land storage _land = getLandByID[_landID];
        Developer storage _developer = getDeveloperByID[_land.developerID];
        require(_land.isReady == true,                          "LRSC: LAND IS NOT READY");
        require(isDeveloperIDValid[_land.developerID] == true,  "LRSC: DEVELOPER INVALID");
    ...
    function approveLandAppraisal(
        ...
        // update in lands
        Land storage _land = getLandByID[_landID];
        require(!_land.isReady, "LRSC: only no-ready land needs approval");
        Developer storage _developer = getDeveloperByID[_land.developerID];
    ...
    function getVoteAddressByLandID(uint landID) external view returns(address[] memory result) {
        require(isLandIDValid[landID] == true, "LRSC: INVALID_LAND_ID");
    ...
    function addUniqueLoanEntityId(uint developerId, uint loanPoolID, uint loanEntityID) override external onlyLPSC {
        require(
            ...
            "LRSC: online loanpool can call updateLandforNewLoanEntity"
        );
