...
// File: contracts/DsaRewardDistributor.sol
...
interface IDsaRewardDistributor {
	function sendCheeseTo(address) external;
}

contract DsaRewardDistributor is Ownable, WhiteList, IDsaRewardDistributor {
	using SafeMath for uint256;
	using TransferHelper for address;

	event SettleDsaFlashLoan(
	...
	event SetWhiteList(address whitelist, bool state);
	event SetNewAccountRewardAmount(uint256 preAmount, uint256 newAmount);

	mapping(address => bool) public isAccountExist;
	uint256 public newAccountRewardAmount;
	...
	constructor(address _cheeseToken) public {
		cheeseToken = _cheeseToken;
	}
	...
	function settleFlashLoan(
		...
		require(CheeseToken(cheeseToken).balanceOf(address(this)) >= amount, 'Insufficient reward funds');
		...
		if (merkleDistributors.length > 0) {
			...
			require(md.settleBlockNumber < settleBlockNumber, '!settleBlockNumber');
		}

		merkleDistributors.push(MerkleDistributor(merkleRoot, index, amount, settleBlockNumber));
		emit SettleDsaFlashLoan(merkleRoot, index, amount, settleBlockNumber);
	}
	...
	function claimFlashLoan(
		...
		address(cheeseToken).safeTransfer(msg.sender, amount);
	...
	function setNewAccountRewardAmount(uint256 _newAmount) external onlyOwner {
		require(newAccountRewardAmount != _newAmount, 'DsaRewardDistributor: not changed!');
		uint256 preAmount = newAccountRewardAmount;
		newAccountRewardAmount = _newAmount;
		emit SetNewAccountRewardAmount(preAmount, newAccountRewardAmount);
	}

	function sendCheeseTo(address _to) external override onlyWhitelisted {
		require(!isAccountExist[_to], 'DsaRewardDistributor: account already exist!');
		isAccountExist[_to] = true;
		address(cheeseToken).safeTransfer(_to, newAccountRewardAmount);
	}

	function getBalance() public view returns(uint256) {
		return CheeseToken(cheeseToken).balanceOf(address(this));
	}
}