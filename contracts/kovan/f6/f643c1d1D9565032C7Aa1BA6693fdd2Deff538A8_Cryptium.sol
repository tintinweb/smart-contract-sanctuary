...
contract Cryptium is CrptERC20, Control, ReentrancyGuard {
	...
	function getAvailToClaim(address user) external initialized returns (uint256) {
		...
		emit Log("avail to claim, balanc before", (_gonBalances[user] / gonsPerFragment) - balanceBefore, balanceBefore, 0, 0);
		...
		)
	{
		...
		if (newPrice == 0) {
			...
			price1 = (curPrice * oldPrice * 10000) / newPrice; // 8
		}
		if (daysPassFromRebase == 0) {
			price2 = 0;
		} else {
			price2 = (((_coeffInflations * daysPassAll * 10000) / 365 + 100000000) * _initialPrice) / 10000; // 8
		}
		...
		emit Log("Init price, isInfl", _initialPrice, (isInflRebase) ? 1 : 0, 0, 0);
