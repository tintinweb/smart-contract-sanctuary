...
contract StackosNFT is ERC721, Ownable {
    ...
    function _transferOwnership(address from, address to, uint256 tokenId, bytes calldata data) internal {
        ...
        uint256 _price = _nftsToOwners[tokenId].price;
        uint256 _tax = (_price * 10) / 100;

        tokenContract.transferFrom(to, from, _price - _tax);
        tokenContract.transferFrom(to, owner(), _tax);
    ...
    function withdrawToken(uint256 _amount) external onlyOwner {
        require(tokenContract.balanceOf(address(this)) >= _amount, "StackosNFT: Insufficient balance to transfer");
