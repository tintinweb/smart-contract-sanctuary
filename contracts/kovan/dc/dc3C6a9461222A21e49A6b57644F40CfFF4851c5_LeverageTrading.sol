...
abstract contract LeverageMarginCall is LeverageAcc {
    ...
    function marginCallFromSecured() internal returns (uint256 pegAmount) {
        ...
        for (uint256 tokenIdx = 0; len > tokenIdx; tokenIdx++) {
            ...
            if (liq.buy > liq.sell) {
                pegAmount += PriceMath.marginCallFromSecured(
                    buyToken,
                    liq.buy - liq.sell
                );
    ...
    function marginCallToSecured() internal returns (uint256 pegAmount) {
        uint256 len = liquidationTokens.length;
        for (uint256 tokenIndex = 0; len > tokenIndex; tokenIndex++) {
            ...
            if (liq.sell > liq.buy) {
                ...
                pegAmount += PriceMath.marginCallToSecured(token, sellAmount);
        ...
        returns (uint256 maintainerCut)
    {
        ...
        uint256 sale2pegAmount = marginCallToSecured();
        uint256 peg2targetCost = marginCallFromSecured();
...
abstract contract PriceMath is RoleAware, UniswapStyleLib 
{
    ...
    function marginCallToSecured(address token, uint256 amount)
        internal
        returns (uint256)
    {
        if (token == peg) {
    ...
    function marginCallFromSecured(address token, uint256 targetAmount)
        internal
        returns (uint256)
    {
        if (token == peg) {
