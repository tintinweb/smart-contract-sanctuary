...
contract StableCoin is Context, ERC20Pausable, ERC20Mintable, ERC20Detailed {
    ...
    function transferWithData(
        ...
        if (Address.isContract(_account)) {
            ...
            receiver.tokenFallback(_msgSender(), _amount, _data); // Check kro balance h ixka pas k nae or do on success transfer
    ...
    {
        msgSender = msg.sender;
        bytes memory empty = hex"00000000"; // Added 01 for just testing need remove if require
        if (Address.isContract(_account)) {
            IERC223Recipient receiver = IERC223Recipient(_account);
            _transfer(_msgSender(), _account, _amount);
    ...
    function mint(address _account, uint256 _amount)
        ...
        returns (bool)
    {
        // require(isWhitelisted(_account), "Cannot send tokens outside Tokenism");
    ...
    // To Destroy Stable Coin Get All STable Coin Holder
    ...
    // Destruct Stable Coin Contract Address
