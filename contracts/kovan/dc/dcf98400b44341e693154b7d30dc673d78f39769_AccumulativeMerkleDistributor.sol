...
// File: AccumulativeMerkleDistributor.sol

contract AccumulativeMerkleDistributor is Ownable, ReentrancyGuard {
  ...
  function claim(
    ...
    bytes32 leaf = keccak256(abi.encodePacked(account, reward));
    require(MerkleProof.verify(proof, root, leaf), '!proof');

    uint claimAmt = reward - claimed[account];
    IERC20(token).safeTransfer(account, claimAmt);
    claimed[account] = reward;
    emit Claim(account, claimAmt);
  }
