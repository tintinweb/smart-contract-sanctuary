...
contract PriceOracleProxy is Initializable, PriceOracle, Exponential, Denominations {
     ...
     */
    function getUnderlyingPrice(CToken cToken) public override view returns (uint256) {
    ...
    event IsDAOVaultUpdated(address tokenAddress, DAOVaultId vaultId);
    ...
    function _setDAOVaults(address[] calldata tokenAddresses, uint256[] calldata vaultId) external {
        ...
        for (uint256 i = 0; i < tokenAddresses.length; i++) {
            ...
            emit IsDAOVaultUpdated(tokenAddresses[i], daoVaultId[tokenAddresses[i]]);
        }
    }
...
abstract contract PriceOracle {
     ...
     */
    function getUnderlyingPrice(CToken cToken) external virtual view returns (uint256);
}
