...
interface IBlackListUsers {
    function getBlackListUserStatus(address _user) external view returns (bool);
}
...
contract BuryBone is ERC20("tBone Staked Bone Dogecoin Killer", "tBONE"){
    ...
    IERC20 public bone;
    IBlackListUsers public blackList;

    // Define the Bone token contract
    constructor(IERC20 _bone, IBlackListUsers _blackListUsers) public {
        bone = _bone;
        blackList = _blackListUsers;
    }

    // Enter the doghouse. Pay some BONEs. Earn some shares.
    // Locks Bone and mints tBone
    function enter(uint256 _amount) public {
        require(!(blackList.getBlackListUserStatus(msg.sender)), "Black listed users can not stake tokens");
    ...
    function leave(uint256 _share) public {
        require(!(blackList.getBlackListUserStatus(msg.sender)), "Black listed users can not claim tokens");
