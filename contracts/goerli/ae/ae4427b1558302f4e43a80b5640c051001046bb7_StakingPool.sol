...
contract AllocationPool is Initializable, OwnableUpgradeable {
    ...
    // The time user need to wait when withdraw.
    uint256 public allocDelayWithdrawDuration;
    // Info of pending withdrawals.
    mapping(uint256 => mapping(address => AllocPendingWithdrawal))
        private allocPendingWithdrawals;
     ...
     */
    function allocAddPool(uint256 _allocPoint, IERC20 _lpToken)
        external
        onlyOwner
    {
        require(
     ...
     * @param _allocPoint the allocation point of the pool, used when calculating total reward the whole pool will receive each block
     */
    function allocSetPool(uint256 _pid, uint256 _allocPoint)
        external
        onlyOwner
    ...
    /**
     * @notice Set the pending withdraw time. Can only be called by the owner.
     * @param _delayWithdrawDuration the time user need to wait when withdraw
     */
    function allocSetDelayWithdraw(uint256 _delayWithdrawDuration)
        external
        onlyOwner
    {
        allocDelayWithdrawDuration = _delayWithdrawDuration;
    }
    ...
    function allocWithdraw(
        ...
        if (allocDelayWithdrawDuration == 0) {
