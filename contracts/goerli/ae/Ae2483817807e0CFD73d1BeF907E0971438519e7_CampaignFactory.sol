...
contract CampaignFactory is Ownable, Pausable, Initializable {
    ...
    event IcoCampaignCreated(address registedBy, address indexed token, address indexed campaign, uint256 campaignId);
    event PlatformFeeChanged(address changer, uint256 fee);
    event PlatformRevenueAddressChanged(address changer, address newRevenueAddress);

    function initialize(uint256 _platformFeeRate, address _revenueAddress) initializer public {
    ...
    */
    function getCreatedCampaignsByToken(address _creator, address _token) public view returns(address[] memory) {
    ...
    */
    function getCreatedCampaignsLengthByToken(address _creator, address _token) public view returns(uint) {
    ...
    */
    function setPlatformFeeRate(uint256 _fee) external onlyOwner returns (uint256)
    {
    ...
    */
    function setPlatformRevenueAddress(address _revenueAddress) external onlyOwner returns (uint256)
    {
        require(_revenueAddress != address(0), "ICOFactory::ZERO_ADDRESS");
        platformRevenueAddress = _revenueAddress;
    ...
    */
    function registerCampaign(
        ...
        uint256 tokenIndex = getCreatedCampaignsLengthByToken(msg.sender, _token);
        ...
        ICampaign(campaign).initialize(_name, _token, _duration, _openTime, _releaseTime, _ethRate, _ethRateDecimals, _wallet);
        getCampaigns[msg.sender][_token].push(campaign);
        allCampaigns.push(campaign);

        emit IcoCampaignCreated(msg.sender, _token, campaign, allCampaigns.length - 1);
    }
...
interface ICampaign {
    function initialize(string memory _name, address _token, uint256 _duration, uint256 _openTime, uint256 _releaseTime, uint256 _ethRate, uint256 _ethRateDecimals, address _walletAddress) external;
}
