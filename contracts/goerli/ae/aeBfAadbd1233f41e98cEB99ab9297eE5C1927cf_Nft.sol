...
abstract contract NFTAssociate is ERC721Enumerable {
    ...
    function setAssociation(uint256 _tokenId, address _address) public {
        ...
        //Owner can set to any address or scholar token is associated with can set it back to the owner
...
contract Nft is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, AccessControl, ERC721Burnable, NFTAssociate, RoyaltiesV2Impl, ERC2981Base  {
    ...
    uint public utilitySummoningFee = 5;
    uint public governanceSummoningFee = 5;
    address public utilityToken;
    address public governanceToken;
    uint256 public summoningCount = 5;
    ...
    function summon(address _to, uint256 _tokenId) external whenNotPaused payable {
        ...
        require(items[_tokenId].summonCount < summoningCount, 'Exceeded Summoning Count');
        
        if(utilitySummoningFee > 0) {
            require(ERC20(address(utilityToken)).transferFrom(msg.sender, address(this), utilitySummoningFee * (10 ** 18)), "Transfer failed");    
        }

        if(governanceSummoningFee > 0) {
            require(ERC20(address(governanceToken)).transferFrom(msg.sender, address(this), governanceSummoningFee * (10 ** 18)), "Transfer failed");
        }
    ...
    /// @dev allows for setting a fee for summoning with the utility token
    function updateUtilitySummoningFee(uint _fee) external onlyRole(CLEVEL) {
        utilitySummoningFee = _fee;
    }

    /// @param _fee the new fee in wei
    /// @dev allows for setting a fee for summoning with the governance token
    function updateGovernanceSummoningFee(uint _fee) external onlyRole(CLEVEL) {
        governanceSummoningFee = _fee;
    }
    ...
    function updateSummoningCount(uint256 _count) external onlyRole(CLEVEL) {
        summoningCount = _count;
    }
