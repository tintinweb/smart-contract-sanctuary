...
contract MerkleVesting is IMerkleVesting, Multicall, Ownable {
    ...
    {
        return (
            cohorts[cohortId].merkleRoot,
            cohorts[cohortId].distributionEnd,
            cohorts[cohortId].vestingEnd,
            cohorts[cohortId].vestingPeriod,
            cohorts[cohortId].cliffPeriod
        );
    }
    ...
    function addCohort(
        ...
        cohorts[cohortId].distributionEnd = uint64(block.timestamp + distributionDuration);
        cohorts[cohortId].vestingEnd = uint64(block.timestamp + vestingPeriod);
        cohorts[cohortId].vestingPeriod = vestingPeriod;
        cohorts[cohortId].cliffPeriod = cliffPeriod;
    }
    ...
    function claim(
        ...
        uint256 distributionEndLocal = cohort.distributionEnd;
        if (block.timestamp > distributionEndLocal) revert DistributionEnded(block.timestamp, distributionEndLocal);

        // Verify the merkle proof.
        bytes32 node = keccak256(abi.encodePacked(index, account, amount));
        if (!MerkleProof.verify(merkleProof, cohort.merkleRoot, node)) revert InvalidProof();

        // Calculate the claimable amount.
        uint256 claimableAmount;
        uint256 claimedSoFar = cohort.claims[account];
        uint256 vestingEnd = cohort.vestingEnd;
        uint256 vestingStart = vestingEnd - cohort.vestingPeriod;
        uint256 cliff = vestingStart + cohort.cliffPeriod;
        if (block.timestamp < cliff) {
            revert CliffNotReached(cliff, block.timestamp);
        } else if (block.timestamp < vestingEnd) {
            claimableAmount = amount / (block.timestamp - vestingStart) - claimedSoFar;
            cohort.claims[account] = claimedSoFar + claimableAmount;
        } else {
            claimableAmount = amount - claimedSoFar;
        }

        // Send the token.
        if (!IERC20(token).transfer(account, amount)) revert TransferFailed(token, address(this), account);

        emit Claimed(cohortId, account, amount);
    }
