 ...
 *
 */
contract CustomOpenEditions is
  NFTokenMetadata,
  ECRecovery,
  Ownable
{
  ...
  constructor(  uint256 chain_id, uint256 fee_pct, address fee_recipient ) 
  {
    ...
    "MintPacket(address artist,string uri,uint256 maxCopies,uint256 expirationBlock,address currencyToken,uint256 currencyAmount)"
    );
    ...
    function getPacketHash(address artist,string memory uri, uint256 maxCopies, uint256 expirationBlock, address currencyToken, uint256 currencyAmount) public pure returns (bytes32) {
    return keccak256(abi.encode(
        PACKET_TYPEHASH,
        artist,
        keccak256(bytes(uri)),
    ...
    function getTypedDataHash(address artist, string memory uri,uint256 maxCopies, uint256 expirationBlock,address currencyToken, uint256 currencyAmount) public view returns (bytes32) {
        bytes32 digest = keccak256(abi.encodePacked(
        "\x19\x01",
        getEIP712DomainHash('0xBTC Open Editions','1',_chain_id,address(this) ),
        getPacketHash(artist,uri,maxCopies,expirationBlock,currencyToken,currencyAmount)
        ));
        return digest;
    }
   ...
   * 
   */
  function mint(address artist, string memory uri, uint256 maxCopies, uint256 expirationBlock, address currencyToken, uint256 currencyAmount, bytes memory signature) external returns(bool) {  
  
    require(artist == owner());
    
    //check EIP712 signature from artist that validates the [uri, artist] 
    bytes32 sigHash = getTypedDataHash(artist, uri, maxCopies, expirationBlock, currencyToken, currencyAmount);
    ...
    if(currencyToken != address(0x0)){ 
        _transferCurrencyForSale(msg.sender, artist, currencyToken, currencyAmount );
    } 
