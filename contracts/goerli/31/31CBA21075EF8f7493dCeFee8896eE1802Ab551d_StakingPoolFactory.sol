...
import "./RewardManager.sol";
import "../BasisPoints.sol";
import "../Price/BTCPriceConsumer.sol";
import "../Distribution/BTCDistributor.sol";
import "../Distribution/USDRetriever.sol";

contract StakingPool is Context, Ownable, BTCPriceConsumer, USDRetriever {
    using BasisPoints for uint256;
    using SafeMath for uint256;
    ...
    constructor(
        ...
        // TODO: Replace maturity time to maturity days - Like 15 Days in epoch timestamp.
        ...
        prizeRewardRates.push(PrizeRewardRate({rank: 1, percentage: 275}));
        prizeRewardRates.push(PrizeRewardRate({rank: 2, percentage: 200}));
        prizeRewardRates.push(PrizeRewardRate({rank: 3, percentage: 100}));
        prizeRewardRates.push(PrizeRewardRate({rank: 10, percentage: 25}));
        prizeRewardRates.push(PrizeRewardRate({rank: 25, percentage: 10}));
    }

    function stake(uint256 _amount, uint256 _pricePrediction) public {
        require(!isLocked, "Pool is locked");
        require(_amount > 0, "Amount can't be zero");
        // TODO: Need to set limit range to avoid 0.1 TOTM left
        require(
            totalStaked.add((_amount)) <= sizeAllocation,
    ...
    function claimReward() public {
        uint256 reward;
        uint256 btcReward;
        (reward, btcReward) = _getTotalReward(_msgSender(), true);
        if (reward == 0) return;
        if (btcReward == 0) return;

        rewardManager.rewardUser(_msgSender(), reward);
        require(btcToken.transfer(_msgSender(), btcReward));

        emit Withdraw(_msgSender(), reward);
    }

    function purchaseBTC(uint256 usdAmount, uint256 deadline) public returns (uint256[] memory) {
        ...
        uint256 btcAmountWithSlippage = btcAmount.sub(btcAmount.mulBP(300));
        // uint256 btcAmountWithSlippage = btcAmount.sub(slippage);
    ...
    function getPredictionRange(uint256 amount) public pure returns (uint256) {
        ...
        for (uint256 i = 0; i < steps.length; i++) {
            uint256 step = amount > steps[i] ? steps[i] : amount;
            totalRange = totalRange.add(step.mul(ranges[i]).div(500));

            if (amount <= steps[i]) break;

            amount = amount.sub(steps[i]);
        }

        totalRange = totalRange.div(100);
        return totalRange;
    }
    ...
    function endPool() public {
        require(getBTCBalance() != 0, "Staking Pool: BTC Rewards not available");
        ...
        if (_getDifference(avgPricePrediction, 25) == 0) {
    ...
    function _getRewardRate() internal view returns (uint256) {
        ...
        return stakeApr.mul(maturityTime).div(yearInSeconds);
    }
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < userStakes.length; i++) {
            uint256 rewardPerStake =
                block
                    .timestamp
                    .sub(userStakes[i].lastWithdrawalTime)
                    .mul(_getRewardRate())
                    .div(10000)
                    .mul(userStakes[i].stakedBalance);

            uint256 enhancedRewardRate =
                _getEnhancedRewardRate(userStakes[i].stakedTime);

            rewardPerStake = rewardPerStake
                .mul(100 + enhancedRewardRate)
                .div(100)
                .sub(userStakes[i].amountWithdrawn);
    ...
    function _getEnhancedRewardRate(uint256 stakedTime)
        ...
        returns (uint256)
    {
        uint256 maturityDate = startDate.add(maturityTime);
        ...
        returns (uint256, uint256)
    {
            ...
            for (uint256 j = 0; j < prizeRewardRates.length; j++) {
                if (userStakes[i].rank <= prizeRewardRates[j].rank) {
                    ...
                    btcReward = btcReward.add(
                        btcReward = btcPrizeAmount.mulBP(prizeRewardRates[j].percentage)
                    );
                    break;
                }
            }
        ...
        if (collaborativeReward) {
            reward = reward.add(prizeAmount.mul(200).div(1000));
        }
...
contract BTCPriceConsumer is PriceConsumer {
    address public constant ORACLE_ADDRESS =
        0xECe365B379E1dD183B20fc5f022230C044d51404;

    constructor() PriceConsumer(ORACLE_ADDRESS) {}
}
