...
contract TradeableERC721Token is ERC721Full, Ownable {
  ...
  bytes32[] public GFBRCEDetails;
  bytes32[] public BPFHDetails;
  bytes32[] public BPLHDetails; 
   
  
  constructor(string memory _name, string memory _symbol, uint256 _supply, address _proxyRegistryAddress) ERC721Full(_name, _symbol, _supply) public {
  ...
  function mintTo(address _to, bytes32 _GFBRCE, bytes32 _BPFH, bytes32 _BPLH) public onlyOwner {
    if(_currentTokenId <= tsupply){
        uint256 newTokenId = _getNextTokenId();
        _mint(_to, newTokenId);
        GFBRCEDetails.push(_GFBRCE);
        BPFHDetails.push(_BPFH);
        BPLHDetails.push(_BPLH);
        _incrementTokenId();
    } else {
        return ;
    }
  ...
  /**
    ...
    * @return 
    */
  function _getDetails(uint256 _tokenidentiy) public view returns(bytes32 _gfbrce, bytes32 _bpfh, bytes32 _bplh) {

    (_gfbrce, _bpfh, _bplh) = (GFBRCEDetails[_tokenidentiy.sub(1)], BPFHDetails[_tokenidentiy.sub(1)], BPLHDetails[_tokenidentiy.sub(1)]);
    return (_gfbrce, _bpfh, _bplh);                
  ...
  /**
    ...
    * @return nothing
    */
  function _setDetails(uint256 _tokenidentiy, bytes32 _NEWGFBRCE, bytes32 _NEWBPFH, bytes32 _NEWBPLH) public onlyOwner {

    GFBRCEDetails[_tokenidentiy.sub(1)] = _NEWGFBRCE; 
    BPFHDetails[_tokenidentiy.sub(1)] = _NEWBPFH; 
    BPLHDetails[_tokenidentiy.sub(1)] = _NEWBPLH;
...
contract StorageNFT is TradeableERC721Token, ReentrancyGuard {
  ...
  ) TradeableERC721Token(_name, _symbol, _supply, _proxyRegistryAddress) public {
