...
contract LPLeverageLaunch is OwnableStatic {
  ...
  mapping( address => uint256 ) private totalLoanedForToken;

  mapping( address => address ) private lentTokenForUniV2PriceSource;

  mapping( address => uint256 ) private launchTokenDueForHolder;
  ...
  mapping( address => uint256 ) private lastTWAPForToken;
  mapping( address => uint32 ) private lastpriceTimestampForToken;

  function _calculateTWAP( address lentToken ) internal view returns ( uint ) {
    ...
    uint256 timeElapsed = uniV2PairLastBlockTimestamp_ - lastpriceTimestampForToken[lentToken];
    uint256 lastCumulativePrice = lastpriceTimestampForToken[lentToken];
