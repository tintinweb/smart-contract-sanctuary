...
// File: contracts/CNTPredicate.sol
...
contract CNTPredicate is ITokenPredicate, AccessControlMixin, Initializable {
    using RLPReader for bytes;
    using RLPReader for RLPReader.RLPItem;
    // using SafeERC20 for IERC20;

    bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE");
    bytes32 public constant TOKEN_TYPE = keccak256("CNT");

    bytes32 public constant CROSSCHAIN_BURN_EVENT_SIG =
        keccak256("CrossChainBurn(address,uint256)");
    ...
    function exitTokens(
        ...
        require(
            bytes32(logTopicRLPList[0].toUint()) == TRANSFER_EVENT_SIG ||
                ...
                CROSSCHAIN_BURN_EVENT_SIG,
            "Invalid exit transaction"
        );
        ...
        if (bytes32(logTopicRLPList[0].toUint()) == CROSSCHAIN_BURN_EVENT_SIG) {
            require(
                burner == address(logTopicRLPList[1].toUint()),
                "Invalid CrossChain Burn transaction"
            );
            IERC20(rootToken).burn(logTopicRLPList[2].toUint());
        }
    }

    /**
     * @notice Function to set burner address. can only be called by manager.
     * @param _burner Address of the burner contract
     */
    function setBurner(address _burner) public only(MANAGER_ROLE) {
        burner = _burner;
    }
}