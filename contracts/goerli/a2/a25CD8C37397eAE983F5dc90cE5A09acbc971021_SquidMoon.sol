...
contract SquidMoon is Initializable, Mintable, IERC20 {
    ...
    function _transfer(
        ...
        uint256 toBurn = (amount * 1e16) / 1e18; // 1%
        uint256 toReceiver = amount - toBurn;

        balanceOf[from] -= amount;
        balanceOf[to] += toReceiver;
        balanceOf[BURN_ADDRESS] += toBurn;

        emit Transfer(from, to, toReceiver);
        // emit Transfer(from, BURN_ADDRESS, toBurn);
    }
