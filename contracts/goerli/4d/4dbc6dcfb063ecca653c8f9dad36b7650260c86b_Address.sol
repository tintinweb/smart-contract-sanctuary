...
contract Ballot is Context, IERC20, Ownable {
    ...
    uint256 public _LiquidityandDevFee = 2;
    uint256 private _previousLiquidityandDevFee = _LiquidityandDevFee;
    ...
    address payable public devAddress = payable(0x3De92b2308f4acBA823fA58A0C02633380d570eE); 
    address payable public marketingAddress = payable(0x65b4eF486971839517d6FF08Af90fD69F26FbB1B);
    ...
    function _transfer(
        ...
        if (TDEnabled){
            if (!_isWhiteListed[to] && to != _addressUniswapV2Router02 && to != uniswapV2Pair && !_isWhiteListed[from]){
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
    	uint256 CombFee = contractTokenBalance.div(10);
    	uint256 LiqPortion = CombFee.mul(8);
    	uint256 DevPortion = CombFee.mul(2);
    
    	uint256 half = LiqPortion.div(2);
        uint256 otherHalf = LiqPortion.sub(half);
    	uint256 EthPortion = half.add(DevPortion);
        ...
        // swap tokens for ETH
        swapTokensForEth(EthPortion);

        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);
    	uint256 DevBalance = newBalance.div(3);
    	uint256 LiqBalance = DevBalance.mul(2);
        // add liquidity to uniswap
        addLiquidity(otherHalf, LiqBalance);
        _tDevTotal = _tDevTotal.add(DevBalance);
        TransferDevETH(devAddress, DevBalance);
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_LiquidityandDevFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 && _LiquidityandDevFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousBurnFee = _burnFee;
        _previousLiquidityandDevFee = _LiquidityandDevFee;
        
        _taxFee = 0;
        _burnFee = 0;
        _LiquidityandDevFee = 0;
    }
    
    function restoreAllFee() private {
        _taxFee = _previousTaxFee;
        _burnFee = _previousBurnFee;
        _LiquidityandDevFee = _previousLiquidityandDevFee;
    }
    function setPreAddy(address _presaleWallet) external onlyOwner {
        presaleWallet = _presaleWallet;
        _isExcludedFromFee[_presaleWallet] = true;
        _isWhiteListed[_presaleWallet] = true;
  	}
    ...
    function setLiquidityandDevFeePercent(uint256 LiquidityandDevFee) external onlyOwner() {
        _LiquidityandDevFee = LiquidityandDevFee;
    }
