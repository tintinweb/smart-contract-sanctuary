...
contract GroupWalletFactory {
    ...
    event ProxyGroupWalletCreation(ProxyGW proxy);
    ...
    function getOwners_internal(uint256 c) public view returns (address[] memory)
    {
    ...
    function inviteInstallToken_q31n(bytes32[] calldata _mem) external payable {      
      ...
      {
        ProxyGW proxyGW = new ProxyGW( address(uint160(uint256(_mem[4]))) );    // _mem[4] = masterCopy, GroupWalletMaster
    ...
    function createProxyGroupWallet(bytes32 _domainHash, address masterCopy, bytes memory data) public payable returns (ProxyGW proxy)
    {
      uint256 c = commitments[_domainHash];
