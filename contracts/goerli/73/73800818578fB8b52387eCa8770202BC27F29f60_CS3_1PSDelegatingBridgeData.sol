...
/*
 * Crypto stamp 3.1 Pre-sale Delegating BridgeData contract
 * Adds a unique string to the base token URI so CS3.1PS tokens can
...
// File: contracts/BridgeDataReadOnlyI.sol

/*
 * Interface for data storage of the bridge.
 */

interface BridgeDataReadOnlyI {
...
// File: contracts/CS3_1PSDelegatingBridgeData.sol

/*
 * Delegates to data storage for the bridge, but appends a fixed string to
 * tokenURIBase for correcting CS3.1 Presale.
 */

contract CS3_1PSDelegatingBridgeData is BridgeDataReadOnlyI {
    ...
    /*** Adjust tokenURIBase to differ from CS3PS ***/

    /**
     * @dev The base of ALL token URIs, e.g. https://example.com/
     */
    function tokenURIBase()
    external view override
    returns (string memory)
    {
        return string(abi.encodePacked(bridgeData.tokenURIBase(), "CS3.1"));
    }

    /*** Forward functionality to actual BridgeData ***/
     ...
     * all transfers of the tokens are frozen.
     */
    function tokenSunsetTimestamp()
    external view override
    returns (uint256)
    {
        return bridgeData.tokenSunsetTimestamp();
    }
    ...
    returns (address)
    {
        return bridgeData.getAddress(_name);
    }
