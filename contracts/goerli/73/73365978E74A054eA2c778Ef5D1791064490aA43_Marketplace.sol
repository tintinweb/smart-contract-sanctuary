...
/**
 * @title Marketplace
 * @notice Core contract of the marketplace.
 */
contract Marketplace is IMarketplace, ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                0x893ab71b0c6fc92bef28fc4ee2bc8018ee56220e13f8a287176138660baa87c4, // keccak256("Marketplace")
    ...
    function matchAskWithTakerBidUsingETH(
        ...
        require(takerBid.price == msg.value, "Order: Msg.value does not match takerBid.price");
        ...
        // Execution part 1/2 (transfer fees and funds with ETH)

        // Initialize the final amount that is transferred to seller
        uint256 finalSellerAmount = takerBid.price;

        // 1. Protocol fee
        {
            uint256 protocolFeeAmount = _calculateProtocolFee(makerAsk.strategy, takerBid.price);
        ...
        // 2. Royalty fee
        {
            (address royaltyFeeRecipient, uint256 royaltyFeeAmount) = royaltyFeeManager
                .calculateRoyaltyFeeAndGetRecipient(makerAsk.collection, tokenId, takerBid.price);
            ...
            if ((royaltyFeeRecipient != address(0)) && (royaltyFeeAmount != 0)) {
                ...
                emit RoyaltyPayment(makerAsk.collection, tokenId, royaltyFeeRecipient, address(0), royaltyFeeAmount);
            }
        }

        require((finalSellerAmount * 10000) >= (makerAsk.minPercentageToAsk * takerBid.price), "Fees: Higher than expected");

        // 3. Transfer final amount (post-fees) to seller
        {
            payable(makerAsk.signer).transfer(finalSellerAmount);
        }
    ...
    function _validateOrder(OrderTypes.MakerOrder calldata makerOrder, bytes32 orderHash) internal view {
        ...
        // Verify whether the currency is ETH or whitelisted
        require((makerOrder.currency == address(0)) || (currencyManager.isCurrencyWhitelisted(makerOrder.currency)), "Currency: Not whitelisted");
