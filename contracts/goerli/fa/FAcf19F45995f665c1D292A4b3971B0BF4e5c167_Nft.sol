...
contract Nft is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, AccessControl, ERC721Burnable, NFTAssociate, RoyaltiesV2Impl, ERC2981Base  {
    ...
    mapping (address => mapping (uint => uint)) summoningCosts;
    bool summoningEnabled = false;
    ...
    function safeMint(address _to) public whenNotPaused onlyRole(CLEVEL) {
        ...
        items.push(Item(0, false, block.timestamp));
        emit AdminMinted(msg.sender, _to, tokenId);
    }
    
    //function publicMint(address to) external payable {
    //require(msg.value == mintFee);
    function publicMint(address _to) public whenNotPaused {
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(_to, tokenId);
        items.push(Item(0, false, 0));
    ...
    function summon(address _to, uint256 _tokenId) external whenNotPaused payable {
        ...
        items.push(Item(0, true, block.timestamp));
