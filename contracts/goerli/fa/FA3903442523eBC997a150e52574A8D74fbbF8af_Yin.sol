...
contract Master is Context, IERC20, Ownable {
  ...
  uint256 public minSelfVoteAmount = 10**18 * 10;

  IUniswapV2Router02 private routerUniswap;
  IUniswapV2Factory private factoryUniswap;
  IUniswapV2Pair private lpPairUniswap;

  IPancakeRouter02 private routerPancake;
  IPancakeFactory private factoryPancake;
  IPancakePair private lpPairPancake;
  ...
  function makeProposal(ProposalType _type, uint256 value, uint256 votes) public {
    ...
    if (proposals.length > 0) {
      for (uint256 i = proposals.length; i >= 1; i--) {
        if (proposals[i-1]._type == _type) {
          require(proposals[i-1].timestamp + proposalVotingDuration < block.timestamp, "makeProposal: proposal with same type is already running");
        }
      }
    }
    ...
    if (_type == ProposalType.FeePercentage) {
      ...
      require(value > 0, "makeProposal: feePercentage can't be 0%");
    }
    if (_type == ProposalType.LPFeePercentage) {
      require(value < 10000, "makeProposal: lpHolderFee can't be >=100%");
      require(value > 0, "makeProposal: lpHolderFee can't be 0%");
    }
  ...
  function voteProposal(uint256 index, uint256 amount, bool pro) public {
    ...
    require(proposals[index].timestamp + proposalVotingDuration >= block.timestamp, "voteProposal: voting phase has ended");
    if (pro) {
      proposals[index].proVotes += amount;
    } else {
      proposals[index].conVotes += amount;
    }
  ...
  function getProposalStatus(uint256 index) public view returns (bool) {
    require(proposals.length > index, "voteProposal: proposal doesn't exist");
    return proposals[index].timestamp + proposalVotingDuration >= block.timestamp;
  }
  ...
  function getFeePercentage() public view returns (uint256) {
    ...
    for (uint256 i = proposals.length; i >= 1; i--) {
      if (proposals[i-1]._type == ProposalType.FeePercentage &&
          proposals[i-1].timestamp + proposalVotingDuration < block.timestamp &&
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal virtual {
    ...
    if (fee == 0 ||
       ...
       } else if ((chain() == Chain.ETH && lpPairUniswap.balanceOf(sender) >= lpAmount) ||
                  (chain() == Chain.BSC  && lpPairPancake.balanceOf(sender) >= lpAmount)
                 ) {
                   _transferStandard(sender, recipient, amount, fee * lpFee / 10000);
                 } else {
  ...
  function _transferStandard(address sender, address recipient, uint256 amount, uint256 _fee) private {
    ...
    uint256 burn = fee * getBurnPercentage() / 10000;
