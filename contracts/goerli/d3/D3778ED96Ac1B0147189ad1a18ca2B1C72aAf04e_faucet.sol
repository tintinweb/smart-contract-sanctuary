...
contract faucet is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public maxHead = 100 * 10**6  * 10**18; // 100 million
    uint256 public claimedHead;


    struct LastWrite {uint256 blockNum;}
    ...
    function setInit(address _headFractional, address _gHead) public onlyOwner {
        ...
        headFractional = IERC20(_headFractional);
        gHead = IHead(_gHead);

    }

	function deposit(uint256 amount) external blockExternalContracts blockIfChangingAddress whenNotPaused nonReentrant() {
        ...
        lastWriteAddress[tx.origin] = LastWrite(block.number);
    ...
    function withdraw(uint256 amount) external blockExternalContracts blockIfChangingAddress whenNotPaused nonReentrant() {
        address msgSender = _msgSender();
        ...
        totalDeposits -= Math.min(amount, userDeposits) ;
		deposits[msgSender] -= Math.min(amount, userDeposits) ;
