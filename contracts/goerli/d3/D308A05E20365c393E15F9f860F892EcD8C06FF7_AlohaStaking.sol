    ...
    */
    ...
    function _getRandomImage(uint256 _rarity) internal returns (uint256) {
        ...
        for (uint256 index = 1; index <= rarityByImagesTotal[_rarity]; index++) {
            if (limitByRarityAndImage[index][_rarity] == 0 ||
                totalTokensByRarityAndImage[index][_rarity] < limitByRarityAndImage[index][_rarity]
            ) {
                totalTokensByRarityAndImage[index][_rarity] += 1;
                return index;
            }
        }
