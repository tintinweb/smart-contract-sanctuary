...
contract upxReaper is Ownable {
    ...
    constructor(
        WrappeduPlexa _wupx,
        uint256 _wupxPerBlock,
        uint256 _startBlock,
        uint256 _bonusEndBlock
    ) public {
        wupx = _wupx;
        wupxPerBlock = _wupxPerBlock;
    ...
    function pendingUPX(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 wupxReward = multiplier.mul(wupxPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
