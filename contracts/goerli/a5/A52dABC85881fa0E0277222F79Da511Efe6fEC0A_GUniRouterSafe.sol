    ...
    OwnableUpgradeable
{
    ...
    address public immutable blacklistedRouter;

    constructor(IUniswapV3Factory _factory, IWETH _weth, address _blacklisted) {
        weth = _weth;
        factory = _factory;
        blacklistedRouter = _blacklisted;
    }
    ...
    function _hasRevoked(IGUniPool pool, address receiver) internal view {
        uint256 allowance0 = pool.token0().allowance(receiver, blacklistedRouter);
        uint256 allowance1 = pool.token1().allowance(receiver, blacklistedRouter);
        uint256 allowanceG = IERC20(address(pool)).allowance(receiver, blacklistedRouter);
        require(allowance0 == 0 && allowance1 == 0 && allowanceG == 0, "NEEDS REVOKE");
    }
