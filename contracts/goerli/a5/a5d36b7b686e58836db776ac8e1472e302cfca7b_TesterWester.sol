...
contract TesterWester is IERC20, Ownable {
    ...
    event SwapLiquify(uint256 fee_tokens, uint256 liq_tokens);
    function swapAndLiquify() private swapLock {
        ...
        emit SwapLiquify(fee_balance, liq_balance);
    ...
    function disableFees() private {
        if (burn_fee == 0 && liquidity_fee == 0 && reflection_fee == 0 && team_fee == 0) {
            return;
        }
    ...
    function transferStageToken(address from, address to, uint256 amount) private {
        ...
        if (cancel_fees) {
            ...
            emit ExcludedFromFees(false);
            transferStageStandard(from, to, amount);
        }
    }
    event FeeTaken(uint256 fee, uint256 fee_taken, uint256 total_tokens);

    function transferStageStandard(address from, address to, uint256 t_initial) private {
        uint256 current_rate = getRate();
        uint256 rteam_fee = 0;
        if (team_fee != 0) {
            rteam_fee = calcRValue(current_rate, t_initial, team_fee);
            fee_tokens.add(rteam_fee);
            emit FeeTaken(team_fee, rteam_fee, fee_tokens);
        }
        uint256 rliq_fee = 0;
        if (liquidity_fee != 0) {
            rliq_fee = calcRValue(current_rate, t_initial, liquidity_fee);
            liquidity_tokens.add(rliq_fee);
            emit FeeTaken(liquidity_fee, rliq_fee, liquidity_tokens);
        }
        uint256 rburn_fee = 0;
        if (burn_fee != 0) {
