...
contract YieldToken is ERC20("Banana", "BANANA") {
	...
	// updated_amount = (balanceOG(user) * base_rate * delta / 10) + amount * initial rate
	function updateRewardOnMint(address _user, uint256 _amount) external {
		...
		if (timerUser > 0)
			rewards[_user] = rewards[_user].add(kongzContract.balanceOG(_user).mul(BASE_RATE.mul((time.sub(timerUser)))).div(10)
	...
	function updateReward(address _from, address _to, uint256 _tokenId) external {
		...
		if (_tokenId < 2223) {
			...
			if (timerFrom > 0)
				rewards[_from] += kongzContract.balanceOG(_from).mul(BASE_RATE.mul((time.sub(timerFrom)))).div(10);
			if (timerFrom != END)
				lastUpdate[_from] = time;
			if (_to != address(0)) {
				uint256 timerTo = lastUpdate[_to];
				if (timerTo > 0)
					rewards[_to] += kongzContract.balanceOG(_to).mul(BASE_RATE.mul((time.sub(timerTo)))).div(10);
				if (timerTo != END)
					lastUpdate[_to] = time;
			}
		}
	...
	function getTotalClaimable(address _user) external view returns(uint256) {
		uint256 time = min(block.timestamp, END);
		uint256 pending = kongzContract.balanceOG(_user).mul(BASE_RATE.mul((time.sub(lastUpdate[_user])))).div(10);
