...
contract Kelekshen is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Kelekshen";
    string private _symbol = "KELEK";
    ...
    uint256 internal _tokenTotal = 33_000_000e18;
        ...
        returns (uint256)
    {
        ...
        if (!deductTransferFee) {
            ...
            return
                tokenAmount.sub(tokenAmount.mul(burnFee).div(10** feeDecimal + 2)).mul(
                    _getReflectionRate()
                );
        }
    }
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        if(burnFee != 0){
            ...
            _reflectionTotal = _reflectionTotal.sub(_burnFee.mul(rate));
    ...
    function burn(uint256 amount) public {
        ...
        //transfer reflection
        _reflectionBalance[_msgSender()] = _reflectionBalance[_msgSender()].sub(amount.mul(rate));

        //if any account belongs to the excludedAccount transfer token
        if (_isExcluded[_msgSender()]) {
            _tokenBalance[_msgSender()] = _tokenBalance[_msgSender()].sub(amount);
        }
    ...
    function withdrawAccidentlySentERC20Tokens(IERC20 token) external onlyOwner {
