...
contract ExchangeNFT is ERC721Holder, Ownable, Pausable {
    ...
    mapping(address => EnumerableSet.UintSet) _userSellingTokens;
    mapping(uint256 => address) public _tokenSellingCurrency;
    ...
    event UpdateBuyCurrencyTokenId(uint256 indexed tokenId, address prevCurrency, address newCurrency);
    ...
    function _verifyTokenSellingCurrency(uint256 _tokenId, address _currency) private view returns(bool) {
        require(_tokenSellingCurrency[_tokenId] == _currency, "Wrong selling currency for this asset");
    }

    function _verifyAllowance(address _token, address _owner, uint256 _amount) private view returns(bool) {
        IERC20 verifyToken = IERC20(_token);
        uint256 allowance = verifyToken.allowance(_owner, address(this));
        require(allowance > _amount, "Token not approved enough");
    }

    function buyToken(uint256 _tokenId, uint256 _price, address _currency) public whenNotPaused {
        _verifyAllowance(_currency, msg.sender, _price);
        _verifyTokenSellingCurrency(_tokenId, _currency);
        require(_tokenSellingCurrency[_tokenId] != address(0), 'Currency of this tokenId not exist');
        ...
        require(_price >= _asksMap.get(_tokenId), "Price you pay not enough for NFT asset");
        
        nft.safeTransferFrom(address(this), msg.sender, _tokenId);
        uint256 price = _asksMap.get(_tokenId);
        address buyCurrency = _tokenSellingCurrency[_tokenId];
        IERC20 paidToken = IERC20(buyCurrency);
        paidToken.safeTransferFrom(msg.sender, _tokenSellers[_tokenId], price);
    ...
    function setCurrentPrice(uint256 _tokenId, uint256 _price, address _currency) public whenNotPaused {
        _verifyTokenSellingCurrency(_tokenId, _currency);
    ...
    function setBuyCurrency(uint256 _tokenId, address _currency) public whenNotPaused {
        _verifyTokenSellingCurrency(_tokenId, _currency);
        require(_userSellingTokens[msg.sender].contains(_tokenId), 'Only Seller can update price');    
        require(_tokenSellingCurrency[_tokenId] != address(0), 'Currency of this tokenId not exist');
        address prevCurrency = _tokenSellingCurrency[_tokenId];
        _tokenSellingCurrency[_tokenId] = _currency;
        emit UpdateBuyCurrencyTokenId(_tokenId, prevCurrency, _currency);
    }

    function readyToSellToken(uint256 _tokenId, uint256 _price, address _currency) public whenNotPaused {
        _verifySupportToken(_currency);
    ...
    function cancelSellToken(uint256 _tokenId) public whenNotPaused {
        ...
        _userSellingTokens[_tokenSellers[_tokenId]].remove(_tokenId);
        delete _tokenSellers[_tokenId];
        delete _tokenSellingCurrency[_tokenId];
