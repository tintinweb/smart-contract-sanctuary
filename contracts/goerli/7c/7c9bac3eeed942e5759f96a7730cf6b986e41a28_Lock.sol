...
contract Lock is Ownable {
    ...
    function deposit (uint amount) public { //remember to ERC20.approve
         ...
         paymentSize = newBalance/paymentsRemaining;
    ...
    function getElapsedReward() public view returns (uint,uint,uint){
        // epochLength set on initialization
         if(epochLength == 0)
        ...
        // elapsedEpochs should not be greater than paymentsRemaining
        ...
        uint accumulatedFunds = paymentSize.mul(elapsedEpochs); // paymentSize updated on each deposit
        uint amountToTransfer = beneficiaryBalance.add(accumulatedFunds > balance ? balance:accumulatedFunds);
         return (amountToTransfer,newStartTime,newPaymentsRemaining);
    } 
