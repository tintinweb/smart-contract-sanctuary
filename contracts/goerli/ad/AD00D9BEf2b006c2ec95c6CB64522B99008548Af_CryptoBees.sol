...
contract CryptoBees is ICryptoBees, ERC721Enumerable, Ownable, ReentrancyGuard, Pausable {
    ...
    // uint256[] public unrevealedTokens;
    ...
    event MintRevealed(address indexed owner, uint256 indexed tokenId, uint256 _type);
    ...
    function tryReveal(uint256 revealIndex) public returns (bool) {
        if (revealIndex < unrevealedTokenIndex || revealIndex >= minted) return false;
        if (tokenData[revealIndex + 1]._type != 0) return true;

        uint256 seed = randomizerContract.revealSeed(revealIndex);
        if (seed == 0) return false;

        Token memory t = traitsContract.generate(revealIndex + 1, seed);
        tokenData[revealIndex + 1] = t;
        if (t._type == 2) bearsMinted++;
        if (t._type == 3) beekeepersMinted++;
        unrevealedTokenIndex += 1;
        emit MintRevealed(ownerOf(revealIndex + 1), revealIndex + 1, t._type);
        return true;
    }
    ...
    function mint(
        ...
        tryReveal(unrevealedTokenIndex);
    }
    ...
    function getPotValue(uint256[] calldata tokenIds) public view returns (uint256) {
        uint256 owed = 0;
...
interface IHive {
    ...
    function removeFromWaitingRoom(uint256 tokenId) external;
...
contract Traits is Ownable {
     ...
     */
    function generate(uint256 tokenId, uint256 seed) public returns (ICryptoBees.Token memory) {
        require(_msgSender() == address(beesContract), "DONT CHEAT!");
