...
contract MyToken is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable,AccessControlUpgradeable {
    ...
    //最大 数量
    uint public maxSupply=2100000000*10 ** decimals();
    //当前每天区块共识奖励
    uint public currConsensusMint=160*10 ** decimals();
    //共识奖励缩减步长 6400*365
    uint constant _consensusMintStep=100;
    // 共识mint时间间隔 6400
    uint constant _consensusMintInterval=10;
    //最后共识mint区块
    ...
    function consensusMint(address to)public onlyRole(CONSENSUS_MINT_ROLE){
        ...
        mint(to,getConsensus());
        lastConsensusMintBlock= block.number;
        uint step=(lastConsensusMintBlock-initBlock)/_consensusMintStep;
        if(step>0){
    ...
    //获得共识奖励大小
    function getConsensus()public view returns(uint amount){
        return (block.number-lastConsensusMintBlock)*currConsensusMint;
    }
    ...
    //[{"to":"0x24258A650b65E3cCB8Da22826d3c4B3Abf3B19d2","amount":10000000000000000000}]
