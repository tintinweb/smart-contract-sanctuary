...
contract FNDCollectionFactory is ICollectionFactory {
  ...
  // Used only for a centralized admin definition
  IAdminRole public adminRoleContract;

  address public implementation;

  // Read from the proxy contracts
  IProxyCall public proxyCallContract;

  uint256 public version;

  event AdminRoleContractUpdated(address indexed adminRoleContract);
  ...
  modifier onlyAdmin() {
    require(adminRoleContract.isAdmin(msg.sender), "FNDCollectionFactory: Caller does not have the Admin role");
    _;
  }
  ...
  constructor(
    ...
    address _adminRoleContract
  ) {
    _updateAdminRoleContract(_adminRoleContract);
  ...
  function createCollection(string calldata name, string calldata symbol)
    external
    onlyValidSymbol(symbol)
    returns (address)
  {
    // If the NFT was previously created, this call will revert
    address proxy = implementation.cloneDeterministic(_getSalt(msg.sender, symbol));

    ICollectionContractInitializer(proxy).initialize(payable(msg.sender), name, symbol);

    emit CollectionCreated(proxy, msg.sender, symbol, name, symbol, version);

    return address(proxy);
  }

  function adminUpdateAdminRoleContract(address _adminRoleContract) external onlyAdmin {
    _updateAdminRoleContract(_adminRoleContract);
  }
  ...
  function predictCollectionAddress(address creator, string calldata symbol)
    external
    view
    onlyValidSymbol(symbol)
    returns (address)
  {
    return implementation.predictDeterministicAddress(_getSalt(creator, symbol));
  }

  function _updateAdminRoleContract(address _adminRoleContract) private {
    require(_adminRoleContract.isContract(), "FNDCollectionFactory: AdminRoleContract is not a contract");
    adminRoleContract = IAdminRole(_adminRoleContract);

    emit AdminRoleContractUpdated(_adminRoleContract);
  }
  ...
  function _updateImplementation(address _implementation) private {
    ...
    ICollectionContractInitializer(_implementation).initialize(
      payable(address(adminRoleContract)),
      string(abi.encodePacked("Foundation Collection Template v", version.toString())),
      string(abi.encodePacked("FNDv", version.toString()))
    );
  ...
  function _updateProxyCallContract(address _proxyCallContract) private {
    require(_proxyCallContract.isContract(), "FNDCollectionFactory: Proxy call is not a contract");
    proxyCallContract = IProxyCall(_proxyCallContract);

    emit ProxyCallContractUpdated(_proxyCallContract);
  }

  function _getSalt(address creator, string memory symbol) private pure returns (bytes32) {
    return keccak256(abi.encodePacked(creator, symbol));
  }
}
