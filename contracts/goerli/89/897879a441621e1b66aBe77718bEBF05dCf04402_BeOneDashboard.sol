    ...
    returns (Dashboard memory)
  {
    ...
    for (uint256 i = 0; i < beOneRegistry.getCTokensLength(); i++) {
      ...
      CTokensArray[i] = CToken({
        ...
        userBorrowableInToken: min(metaInfo.collateralAvailableInUsd.div(beOnePriceOracle.getUnderlyingPrice(cToken)).mul(collateralFactor).div(1e18), cToken.totalReserves()),
