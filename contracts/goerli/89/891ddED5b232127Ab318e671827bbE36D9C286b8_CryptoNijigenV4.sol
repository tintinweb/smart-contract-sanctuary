...
contract CryptoNijigenV4 is IChildToken, Initializable, OwnableUpgradeable, ERC20Upgradeable {
    ...
    function safeMintNGN(address to, uint256 amount) external payable{
        require(msg.value == amount / 10 ** _ratio, 'CNGE002');
        this.mint(to, amount);
    }

    function setRatio(uint8 ratio) external {
        _ratio = ratio;
    }

    function getRatio() public view returns (uint8) {
        return _ratio;
    }

    function version() public view returns (string memory) {
        return "CryptoNijigenV4";
    }
