...
// flattened :  Wednesday, 11-Aug-21 21:54:56 UTC
...
interface IVLighthouse is IBase {
...
abstract contract AErc20 is IErc20, ABase {
    ...
    function transfer(
        ...
        require(from != address(0), "AErc20::transfer - from address cannot be zero");
        require(to != address(0), "AErc20::transfer - to address cannot be zero");
...
contract VErc20 is IVErc20, AErc20 {
    ...
    function mint(
        ...
        require(approve(address(this), vLighthouse().addressByKey(ERC20_PREDICATE_PROXY_ADDRESS), balanceOf(address(this))), "VErc20:mint - failed to approve ERC20_PREDICATE_PROXY_ADDRESS");
    ...
    function withdrawUnderlyingEquivalent(
        ...
        Signatures(nonce, keccak256(abi.encodePacked(address(this), "withdrawUnderlyingEquivalent", underlyingTokens, nonce)), V, R, S)
    ) override public {
        uint256 tokens = convert(underlying(), this, exchangeRate(), underlyingTokens);
        IVErc20(address(this)).withdraw(tokens);
