...
contract MRCRYPTO is ERC721Enumerable, Ownable {
	...
	uint256 public cost = 0.002 ether; //hacer como si ether fuera matic
	...
	constructor (
		...
		giveAway(1, 0xfd68F8B7DBC785bC0d0d6A6B2fC0C6f775D18d80); //JON
		//giveAway(1, VICTOR);
	...
	function setPriceIfInterval() internal {
		...
		if(_currSupply > 1000 && _currSupply <= 4000 && cost == 14 ether)
			cost = 19 ether;

		else if (_currSupply > 4000 && cost == 19 ether)
			cost = 25 ether;
	}
	...
	function withdraw() public payable onlyOwner { //TODO: carteras de los fundadores del contrato
		...
		// =============================================================================
			...
			require(payable(0x8407A400dAdFd053199b1e67Fe75ba1e9d76D3A1).send(each)); //YONA
			//require(payable(CARLOS).send(each));
			//require(payable(VICTOR).send(each));
			require(payable(0xC608d2dD5AFA199a0c0E487Fdd669c1AD7459bE4).send(each)); //TOYOS
		// =============================================================================
	}
