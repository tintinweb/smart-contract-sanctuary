...
contract CrocodileGameNFT is ICrocodileGameNFT, ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  mapping(uint256 => string) public preTraitsCroco;
  mapping(uint256 => string) public preTraitsBird;
  
  //string[] public preTraitsBird;
  ...
  // GEN 0 mint price
  uint32 public constant maxMINT = 20;
   ...
   */
  function _mint(uint32 amount, uint8 stake) internal {
    Kind kind;
    uint16[] memory tokenIdsToStake = stake!=0? new uint16[](amount) : new uint16[](0);
    uint8[] memory dilemmas = stake!=0? new uint8[](amount) : new uint8[](0);
    uint256 PiranhaCost;
    uint256 seed;
    
    for (uint32 i = 0; i < amount; i++) {
      minted++;
      seed = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, minted, i)));  
      PiranhaCost += getMintPiranhaCost(minted);
      kind = _SetPhaseOneTraits(minted, seed).kind; // set kind and phase one variables

      address recipient = _selectRecipient(seed);
      if (stake==0 || recipient != msg.sender) {
   ...
   * @param stake mint directly to staking or not. 
   * stake == 0 : Mint then No Staking
   * stake == 1 : Mint then Stake - Cooperate
   * stake == 2 : Mint then Stake - Betray
   */
  function mint(uint32 amount, uint8 stake) external payable nonReentrant {
   ...
   * And, the pseudo tokenID for the Semi-OnChain implementation.
  ...
  function _SetPhaseOneTraits(uint16 tokenId, uint256 seed) internal returns (Traits memory t) {    
    ...
    uint256 mod = (seed & 0xFFFF) % 50;

    if (crocodilebirdminted == 10000 || mod >= 25)
      {t.kind = Kind(0);
       crocodileminted++;
      // Semi-OnChain - Sampling the crocodile.
      while (!ExistpseudoTokenId[minted]){
        // generate the random number between 1~10000
        randid = uint256(keccak256(abi.encodePacked(msg.sender, seed_nonce, block.timestamp))) % (MAX_TOKENS/2);
      ...
      {t.kind = Kind(1);
      crocodilebirdminted++;
      // Semi-OnChain - Sampling the crocodile bird.
      while (!ExistpseudoTokenId[minted]){
        // generate random number between 10001~20000
        randid = 10000 + uint256(keccak256(abi.encodePacked(msg.sender, seed_nonce, block.timestamp))) % (MAX_TOKENS/2);
        if (!ExistRandId[randid])
          {
            // map the random token index to the current token index
            pseudoTokenId[minted] = randid;
...
interface ICrocodileGameNFT {
  ...
  struct Traits { Kind kind; uint8 dilemma; uint8 karmaP; uint8 karmaM; string traits;}
