...
contract ZoneStakingUpgradeable is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event AddType(bool enable, uint16 lockDay, uint256 rewardPerDay);
    event ChangeType(bool enable, uint16 lockDay, uint256 rewardPerDay);
     ...
     * @param _rewardPerDays rewards per day
     */
    function initialize(
        ...
        _addTypes(_typeEnables, _lockDays, _rewardPerDays);
    }
    ...
    function getTypes() public view returns(bool[] memory enables, uint16[] memory lockDays, uint256[] memory rewardPerDays) {
        enables = new bool[](types.length);
        lockDays = new uint16[](types.length);
        rewardPerDays = new uint256[](types.length);

        for (uint i = 0; i < types.length; i ++) {
            enables[i] = types[i].enabled;
            lockDays[i] = types[i].lockDay;
            rewardPerDays[i] = types[i].rewardPerDay;
        }
    }
    ...
    function addTypes(
        ...
        _addTypes(_enables, _lockDays, _rewardPerDays);
    }
    ...
    function _addTypes(
        ...
        require(
            _lockDays.length == _rewardPerDays.length
        ...
        for (uint256 i = 0; i < _lockDays.length; i ++) {
            Type memory _type = Type({
                enabled: _enables[i],
                lockDay: _lockDays[i],
                rewardPerDay: _rewardPerDays[i],
                stakedAmount: 0
            });
            types.push(_type);
            emit AddType (_type.enabled, _type.lockDay, _type.rewardPerDay);
        }
    }
    ...
    function changeType(
        ...
        _type.lockDay = _lockDays;
        _type.rewardPerDay = _rewardPerDays;
        emit ChangeType (_type.enabled, _type.lockDay, _type.rewardPerDay);
    }
    ...
    function endStake() external nonReentrant() {
        ...
        stakes[staker].rewardAmount = reward;
        ...
        zoneToken.safeTransfer(staker, reward.add(stakedAmount));
    ...
    function _calcReward(
        ...
        if (block.timestamp < unlockTs) {
            claimIn = unlockTs - block.timestamp;
            rewardAmount = 0;
        } else {
            claimIn = 0;
            rewardAmount = stakedAmount.mul(types[typeIndex].lockDay).mul(types[typeIndex].rewardPerDay).div(1e18);
        }
