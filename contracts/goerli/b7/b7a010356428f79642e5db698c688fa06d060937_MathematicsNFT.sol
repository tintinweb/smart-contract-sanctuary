...
contract MathematicsNFT is ERC721Burnable, AccessControlEnumerable, Pausable {
    ...
    constructor(string memory baseURI_) ERC721("No Pork On My Fork", "MATHPORK") {
        ...
        whenNotPaused
    {
        ...
        require(
            _lastId + numberOfTokens <= MAX_SUPPLY, "MAX_SUPPLY_EXCEEDED"
        );

        if (numberOfTokens == 1) {
            _mintOne(to);
        } else {
            _mintMultiple(numberOfTokens, to);
        }
    }

    function _mintOne(address to) private {
        uint16 nextId = _lastId + 1;
        _safeMint(to, nextId);
    ...
    function _mintMultiple(uint16 numberOfTokens, address to) private {
