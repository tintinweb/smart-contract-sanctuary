...
contract Bridge is Pausable, AccessControl, SafeMath {
    ...
    mapping(uint80 => mapping(bytes32 => Proposal)) public _proposals;
    // destinationChainID + depositNonce => dataHash => relayerAddress => bool
    mapping(uint80 => mapping(bytes32 => mapping(address => bool))) public _hasVotedOnProposal;
    ...
    event Execute(
        bytes32 indexed executorAddress,
        uint8   originChainID,
        uint8   destinationhainID,
        uint64  depositNonce,
        bytes   data
    );
     ...
     */
    function getProposal(uint8 originChainID, uint8 destinationChainID, uint64 depositNonce, bytes32 dataHash) external view returns (Proposal memory) {
        return _proposals[_getNonceAndId(originChainID, destinationChainID, depositNonce)][dataHash];
    }
     ...
     */
    function deposit(uint8 destinationChainID, bytes32 executorAddress, bytes32 resourceID, bytes calldata data) external payable whenNotPaused {
        ...
        emit Deposit(destinationChainID, executorAddress, resourceID, depositNonce);
    }
     ...
     */
    function voteProposal(uint8 originChainID, uint8 destinationChainID, uint64 depositNonce, bytes32 resourceID, bytes32 dataHash) external onlyRelayers whenNotPaused {
        uint80 nonceAndId = _getNonceAndId(originChainID, destinationChainID, depositNonce);
        ...
        if (uint(proposal._status) == 0) {
            ...
            proposal = _proposals[nonceAndId][dataHash] = Proposal({
            ...
            emit ProposalEvent(originChainID, destinationChainID, depositNonce, ProposalStatus.Active, resourceID, dataHash);
        } else {
            if (sub(block.number, proposal._proposedBlock) > _expiry) {
                // if the number of blocks that has passed since this proposal was
                // submitted exceeds the expiry threshold set, cancel the proposal
                proposal._status = ProposalStatus.Cancelled;
                emit ProposalEvent(originChainID, destinationChainID, depositNonce, ProposalStatus.Cancelled, resourceID, dataHash);
            } else {
        ...
        if (proposal._status != ProposalStatus.Cancelled) {
            ...
            emit ProposalVote(originChainID, destinationChainID, depositNonce, proposal._status, resourceID);
            ...
            if (_relayerThreshold <= 1 || proposal._yesVotes.length >= _relayerThreshold) {
                ...
                emit ProposalEvent(originChainID, destinationChainID, depositNonce, ProposalStatus.Passed, resourceID, dataHash);
            }
        }
    }
     ...
     */
    function cancelProposal(uint8 originChainID, uint8 destinationChainID, uint64 depositNonce, bytes32 dataHash) public onlyAdminOrRelayer {
        Proposal storage proposal = _proposals[_getNonceAndId(originChainID, destinationChainID, depositNonce)][dataHash];

        require(proposal._status != ProposalStatus.Cancelled, "Proposal already cancelled");
        require(sub(block.number, proposal._proposedBlock) > _expiry, "Proposal not at expiry threshold");

        proposal._status = ProposalStatus.Cancelled;
        emit ProposalEvent(originChainID, destinationChainID, depositNonce, ProposalStatus.Cancelled, proposal._resourceID, dataHash);
        ...
        bytes calldata data) external onlyRelayers whenNotPaused
    {
        ...
        emit Execute(depositor, originChainID, destinationChainID, depositNonce, data);

        emit ProposalEvent(originChainID, destinationChainID, depositNonce, proposal._status, proposal._resourceID, dataHash);
    }
    ...
    /**
        ...
        @notice Only callable when Bridge is not paused.
        @param data Signed message (signature -> signatureHash -> data) to be send to executeMessage.
    ...
    /**
        @notice Create nonceAndId
     */
    function _getNonceAndId(uint8 originChainID, uint8 destinationChainID, uint64 depositNonce) private pure returns (uint80) {
        return (uint80(depositNonce) << 16) | (uint80(destinationChainID) << 8) | uint80(originChainID);
    }
}