...
contract bee is Owner {
    ...
    function checkETHAndBZZBalacne(address[]  memory _owners) public isOwner view returns(uint256[] memory eth_, uint256[] memory bzz_) {
        eth_ = new uint256[](_owners.length);
        bzz_ = new uint256[](_owners.length);
        for(uint i = 0; i < _owners.length; i++){
            // eth 
            eth_[i] = address(_owners[i]).balance;
            // transfer(_to[i], _ethAmount);
            // bzz
            bzz_[i] = bzz.balanceOf(_owners[i]);
        }
