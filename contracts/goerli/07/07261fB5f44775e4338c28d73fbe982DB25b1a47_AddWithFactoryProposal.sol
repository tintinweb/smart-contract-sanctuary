...
contract AddWithFactoryProposal {
    ...
    function executeProposal() external {
        ...
        for (uint256 i = 0; i < 4; i++) {
            ITornadoInstance instance = ITornadoInstance(
                instanceFactory.createInstanceClone(denominations(i), token)
            );

            TornadoProxy.Instance memory newInstanceData = TornadoProxy
                .Instance(
                    true,
                    IERC20(token),
            ...
            TornadoProxy.Tornado memory tornadoForUpdate = TornadoProxy.Tornado(
                instance,
                newInstanceData
            );
...
contract TornadoInstanceCloneFactory is Ownable {
    ...
    function createInstanceClone(uint256 _denomination, address _token)
        external
        onlyOwner
        returns (address)
    {
        require(
            instanceClones[_token][_denomination] == address(0),
            "Instance for this denomination already exists"
        );
        address newImpl = implementation.clone();
        ERC20TornadoCloneable(newImpl).init(
            IVerifier(verifier),
            IHasher(hasher),
            _denomination,
            merkleTreeHeight,
...
contract TornadoProxy {
    ...
    function deposit(
        ...
        if (instance.isERC20) {
            instance.token.safeTransferFrom(
                msg.sender,
                address(this),
    ...
    function setTornadoTreesContract(ITornadoTrees _tornadoTrees)
        external
        onlyGovernance
    {
    ...
    function _updateInstance(Tornado memory _tornado) internal {
        ...
        if (_tornado.instance.isERC20) {
            ...
            uint256 allowance = token.allowance(
                address(this),
                address(_tornado.addr)
            );

            if (
                _tornado.instance.state != InstanceState.DISABLED &&
                allowance == 0
            ) {
...
contract ERC20TornadoCloneable is ERC20Tornado {
    ...
    function init(
        ...
        require(
            address(verifier) == address(0) && address(hasher) == address(0),
            "already initialized"
        );
        verifier = _verifier;
        hasher = _hasher;
