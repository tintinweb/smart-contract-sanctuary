...
contract PKKTFarm is Ownable {
    ...
    //remove an unused lp token, usually if mistakenly added, needs to be used very carefully
    function remove(IERC20 _lpToken) external onlyOwner {
        require(isAdded[address(_lpToken)], "Pool is not added");   
        
        uint256 length = poolInfo.length;
        PoolInfo memory lastPool = poolInfo[length-1];
        //due to the fact that we cannot copy a mapping and we should detect the error as soon as possible
        require(lastPool.lpToken == _lpToken, "Can only remove the last added pool");
        require(lastPool.accPKKTPerShare == 0, "Can only remove unused pool");
        uint256 lpSupply = lastPool.lpToken.balanceOf(address(this));
        require(lpSupply == 0, "Pool has staked lp"); 
        poolInfo.pop(); 
        isAdded[address(_lpToken)] = false;  
        totalAllocPoint = totalAllocPoint.sub(lastPool.allocPoint); 
