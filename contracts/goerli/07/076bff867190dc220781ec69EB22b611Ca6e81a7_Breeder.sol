...
contract Breeder is FxBaseRootTunnel, ERC721Holder, Ownable {
    ...
    mapping(uint => uint) public breedingTimeStamp;
    uint public bonusCoolDown = 604800;
    uint public baseCoolDown = 2419200;
    bool public matingSeason;
    bool public eggCreation = true;
    bool public madHouseCreation = true;
    uint public crowMaxSupply = 8181;
    ...
    function changeCoolDowns(uint _bonus, uint _base) external onlyOwner{
    ...
    function admitToMadHouse( uint mom, uint dad) external {
        ...
        uint babyID = CrowsContract.totalSupply();    
        CrowsContract.mintBabyCrow();
        ...
        bytes memory message = abi.encode(FIND_TRAITS, abi.encode(babyID, mom, dad));
        _sendMessageToChild(message);
    }

    function makeEgg(address recevier, uint mom, uint dad) external{
        require(eggCreation, 'Egg Creation not allowed');
        //make sure mom and dad can breed
        uint timestamp = block.timestamp;
        if(matingSeason){
            require(timestamp > (breedingTimeStamp[mom] + bonusCoolDown), 'Mom can not breed yet' );
            require(timestamp > (breedingTimeStamp[dad] + bonusCoolDown), 'dad can not breed yet' );
        }
        else{
            require(timestamp > (breedingTimeStamp[mom] + baseCoolDown), 'Mom can not breed yet' );
            require(timestamp > (breedingTimeStamp[dad] + baseCoolDown), 'dad can not breed yet' );
        }

        //requries to make sure caller owns mom and dad
        require(CrowsContract.ownerOf(mom) == msg.sender, 'Caller does not own mom');
        require(CrowsContract.ownerOf(dad) == msg.sender, 'Caller does not own dad');
    ...
    function _processMessageFromChild(bytes memory message) override internal{
        (address caller, uint tID) = abi.decode(message, (address, uint));
        require(CrowsContract.ownerOf(tID) == address(this), 'Crazy Crows: Token not Minted');
