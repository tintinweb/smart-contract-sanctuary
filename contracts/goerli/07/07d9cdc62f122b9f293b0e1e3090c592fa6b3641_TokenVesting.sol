...
contract TokenVesting is Ownable {
    ...
    function register(address _account, uint256 _start, uint256 _cliffDuration, uint256 _duration, uint256 _amount, bool _revocable) external onlyOwner {
        ...
        Beneficiary memory beneficiary = Beneficiary(_start, _start.add(_cliffDuration), _duration, _amount, 0, _revocable, false, true);
     ...
     */
    function _releasableAmount(Beneficiary storage _beneficiary) private view returns (uint256){
     ...
     * @param _beneficiary The related information of beneficiary
     */
    function _vestedAmount(Beneficiary storage _beneficiary)private view returns (uint256){
        if (block.timestamp < _beneficiary.cliff) {
            ...
            return _beneficiary.amount.mul(block.timestamp.sub(_beneficiary.start)).div(_beneficiary.duration);
        }
    }
}