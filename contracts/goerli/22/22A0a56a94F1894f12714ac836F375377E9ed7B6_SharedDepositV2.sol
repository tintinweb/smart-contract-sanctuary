    ...
    WithdrawQueueUpgradeable
{
    ...
    //     address _tokenUtilityModuleAddress,
    ...
    function initialize(
        ...
        uint256[] memory configurableUints,
        address[] memory configurableAddresses
    ) external initializer {
        uint256 _epochLength = _epochLengthInDays * 1 days;
        ...
        __WithdrawQueue_init_unchained(_epochLength);
    ...
    */
    ...
    function deposit() external payable nonReentrant whenNotPaused {
        ...
        if (address(tokenUtilityModule) != address(0)) {
            myAdminFee = tokenUtilityModule.getAdminFee(_msgSender(), address(this), value, myAdminFee);
        }
        ...
        tokenManager.mint(msg.sender, valMinusAdmin);
    }
    ...
    function stakeForWithdraw(uint256 amount) external nonReentrant whenNotPaused noContractAllowed {
        ...
        uint256 allowance = BETHToken.allowance(msg.sender, address(this));
    ...
    function withdrawETHRewardsWithQueue() external nonReentrant whenNotPaused noContractAllowed {
        ...
        if (address(tokenUtilityModule) == address(0)) {
            ...
            _epochLength = tokenUtilityModule.getEpochLength(_msgSender(), address(this), amount, epochLength);
        }
        require(
        ...
        BETHToken.approve(address(tokenManager), amount);
        userEntries[_msgSender()].amount = 0;
        tokenManager.burn(address(this), amount);
        uint256 amountToReturn = getAmountGivenShares(amount);

        _withdrawEthRewards(amountToReturn);
    }
    ...
    function withdraw(uint256 amount) external nonReentrant whenNotPaused noContractAllowed {
        ...
        tokenManager.burn(_msgSender(), amount);
        uint256 amountToReturn = getAmountGivenShares(amount);
        if (address(tokenUtilityModule) != address(0)) {
            amountToReturn = tokenUtilityModule.getWithdrawalTotal(_msgSender(), address(this), amount, amountToReturn);
        }
        _withdrawEthRewards(amountToReturn);
    }
    ...
    //     bool _depositsEnabled
    // )
    function setupState(uint256[] memory configurableUints, address[] memory configurableAddresses)
        public
        onlyAdminOrGovernance
    {
        uint256 _validatorsCreated = configurableUints[0];
        uint256 _performanceFeePercent = configurableUints[1];
        uint256 _epochLength = configurableUints[2];
        bool _depositsEnabled;
        if (configurableUints[3] == 1) {
            _depositsEnabled = true;
        } else {
            _depositsEnabled = false;
        }

        address _priceOracleAddress = configurableAddresses[0];
        address _tokenManagerAddress = configurableAddresses[1];
        address _blocklistAddress = configurableAddresses[2];
        address _tokenUtilityModuleAddress = configurableAddresses[3];
        ...
        require(_priceOracleAddress != address(0), "Price oracle needs to be defined");
        require(_tokenManagerAddress != address(0), "Tokenmgr needs to be defined");
        require(_blocklistAddress != address(0), "blocklist needs to be defined");

        curValidatorShares = BETHToken.totalSupply();
        _setCurrentShares(curValidatorShares);

        performanceFeePrct = _performanceFeePercent;
        ...
        blocklist = IBlocklist(_blocklistAddress);
        tokenUtilityModule = ITokenUtilityModule(_tokenUtilityModuleAddress);
        updateCostPerShare();
    ...
    function updateTotalShares() external onlyAdminOrGovernance {
        uint256 circulatingShares = BETHToken.totalSupply();
        if (circulatingShares != curShares) {
    ...
    function updateCostPerShare() public {
        uint256 priceOracleCostPerShare = priceOracle.getCostPerShare();
        uint256 actualCostPerShare = _getActualCostPerShare(priceOracleCostPerShare);
        if (actualCostPerShare != costPerShare) {
            _setCostPerShare(actualCostPerShare);
        }
    }
    ...
    function getTotalBeneficiaryGains() public view returns (uint256 totalGains) {
        ...
        return total.sub(sharesBurnt.add(curShares)).mul(costPerShare);
    }
    ...
    function _getActualCostPerShare(uint256 _costPerShare) internal view returns (uint256 actualCostPerShare) {
        ...
        uint256 beneficiaryCut = _costPerShare.sub(1e18).mul(performanceFeePrct.mul(1e18).div(BIPS_DENOM)).div(1e18);
        return _costPerShare.sub(beneficiaryCut);
    }
    ...
    function _withdrawEthRewards(uint256 amount) internal {
        ...
        _decrementShares(amount);
        sharesBurnt = sharesBurnt.add(amount);
        ...
        // from protocol
        if (blocklist.inBlockList(_msgSender())) {
