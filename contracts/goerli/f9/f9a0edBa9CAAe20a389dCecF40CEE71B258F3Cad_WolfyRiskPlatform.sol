...
contract WolfyRiskPlatform is Ownable {
    ...
    address[] _lowRiskWolfyUsers;
    address[] _highRiskWolfyUsers;
    ...
    function stakeWolfy(uint256 _amount, bool _isLowRisk) public  {
     ...
     if(_isLowRisk){
       require(depositLowRiskWolfy.add(_amount) <= (_ethBalanceRiskPool[owner()][_isLowRisk]).mul(6), "Target 6X reaching");   
       ...
       require(depositHighRiskWolfy.add(_amount) <= (_ethBalanceRiskPool[owner()][_isLowRisk]).mul(3), "Target 3X reaching");   
       require(_balancesRiskPool[msg.sender][_isLowRisk].add(_amount) <= (_balancesRiskPool[owner()][_isLowRisk]).mul(3), "Exceeding high risk WOLFY limit");
        ...
        updateLimitVariables(); // update all deposits
    }
    ...
    function updateLimitVariables() internal{
        ...
        for(uint i = 0; i < _lowRiskETHUsers.length ; i++){
           depositLowRiskETH += _balancesRiskPool[_lowRiskWolfyUsers[i]][true];
        }

        for(uint i = 0; i < _highRiskETHUsers.length ; i++){
           depositHighRiskETH += _balancesRiskPool[_lowRiskWolfyUsers[i]][false];
        }
