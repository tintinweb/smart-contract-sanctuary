...
contract PolygonMainnet is FxBaseRootTunnel {

  enum OperationType {Add, Substract}
  ...
  function changeMyBalance(uint256 newNumber) public {
    myBalance = newNumber;
  }
  ...
  function sendToPolygon(uint256 amount) public {
    ...
    bytes memory data = abi.encode(txInfo);
    _sendMessageToChild(data);
  }

  function receiveFromPolygon(uint256 amount) public {

    TransactionInfo memory txInfo = TransactionInfo({
      opType: OperationType.Substract,
      amount: amount
      });
  ...
  function _processMessageFromChild(bytes memory data) internal override {
        ...
        TransactionInfo memory txInfo = abi.decode(data, (TransactionInfo));

        if(txInfo.opType == OperationType.Add) {

          myBalance = myBalance + txInfo.amount;

        } else {

          myBalance = myBalance - txInfo.amount;

          TransactionInfo memory txInfo2 = TransactionInfo({
            opType: OperationType.Add,
            amount: txInfo.amount
            });

          bytes memory data2 = abi.encode(txInfo2);
          _sendMessageToChild(data2);
