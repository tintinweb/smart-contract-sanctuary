...
contract AtopiaShop is ERC1155Burnable {
	...
	function addItem(
		...
		for (uint16 i = 0; i < storeIndexes.length; i++) {
			...
			emit ItemAdded(items[itemId >> 128]);
				...
				abi.encodePacked(
					...
					Base64.encode(
						abi.encodePacked(
							...
							Base64.encode(bytes(item.store.getTraitContent(item.storeIndex))),
							'","attributes":[{"trait_type":"Store","value":"',
							item.store.name(),
							'"},{"trait_type":"Min Age","value":"',
							(item.minAge * 10) / 365 days,
							'"},{"trait_type":"Age Growth","value":"',
							(item.bonusAge * 10) / 1 days,
							'"},{"trait_type":"Wearable","value":"',
							item.bonusTrait > 0 ? "True" : "False",
							'"}]}'
						)
					)
				)
