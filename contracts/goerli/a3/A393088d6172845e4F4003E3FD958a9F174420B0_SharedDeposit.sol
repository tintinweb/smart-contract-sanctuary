...
import {DepositHelperUpgradeable} from "./util/DepositHelperUpgradeable.sol";
...
contract SharedDeposit is VaultWithAdminFeeUpgradeable, DepositHelperUpgradeable {
    ...
    function deposit() external payable nonReentrant whenNotPaused noContractAllowed {
        ...
        BETHToken.mint(msg.sender, newShares);
    }
...
contract VaultWithAdminFeeUpgradeable is UpgradeableSafeContractBase, VaultWithSharesAndCapUpgradeable {
    using SafeMathUpgradeable for uint256;
    ...
    function ensureAdminFeeIsRetainedInContractBalance(uint256 amount) public view {
        require(
    ...
    function getAdminFeeOnDeposit(uint256 amount) public view returns (uint256) {
        ...
        return amount.mul(ADMIN_FEE).div(BIPS_DENOM);
    }
    ...
    function _deposit(uint256 value) internal returns (uint256) {
        ...
        uint256 newShares = super.getSharesGivenAmount(valMinusAdmin);

        super.incrementShares(newShares);
    ...
    function _withdrawEth(uint256 amount) internal returns (uint256) {
        ...
        ensureAdminFeeIsRetainedInContractBalance(valBeforeAdmin);
    ...
    function setAdminFee(uint256 amount) external onlyAdmin {
        ADMIN_FEE = amount;
    }
...
contract DepositHelperUpgradeable is Initializable {
    using SafeMathUpgradeable for uint256;
...
contract VaultWithSharesAndCapUpgradeable is Initializable {
    ...
    function depositAndAccountShares(uint256 amount) internal {
        uint256 newShareTotal = getSharesGivenAmount(amount);
        require(newShareTotal <= buffer.add(maxShares), 
        "VaultWithSharesAndCapUpgradeable:: Amount too large; Exceeds Cap");
        curShares = newShareTotal;
    }

    function withdrawAndAccountShares(uint256 amount) internal {
        uint256 newShareTotal = getAmountGivenShares(amount);
        require(newShareTotal > 0,
        "VaultWithSharesAndCapUpgradeable:: newShareTotal cannot be negative");
        curShares = newShareTotal;
    }
