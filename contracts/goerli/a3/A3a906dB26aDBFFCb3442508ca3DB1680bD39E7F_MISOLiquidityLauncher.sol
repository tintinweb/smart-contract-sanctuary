...
// File interfaces/IMisoLiquidity.sol
...
// File contracts/MISOLiquidityLauncher.sol
...
contract MISOLiquidityLauncher is SafeTransfer {
    ...
    uint256 public launcherTemplateIndex;
    ...
    /// @notice mapping from liquidity template address to liquidity template index
    mapping(uint256 => address) private liquidityTemplateIndex;
     ...
     */
    function initMISOLiquidityLauncher(address _accessControls, address _WETH, address _bentoBox) external {
        require(!initialised);
        require(_WETH != address(0), "initMISOLiquidityLauncher: WETH cannot be set to zero");
        require(_accessControls != address(0), "initMISOLiquidityLauncher: accessControls cannot be set to zero");
        require(_bentoBox != address(0), "initMISOLiquidityLauncher: bentoBox cannot be set to zero");
    ...
    {
        // emit LauncherInitialized(address(newLauncher), _templateId, _data);
    ...
    function createLiquidityLauncher(uint256 _templateId) external returns (address launcher) {
        ...
        launcher = bentoBox.deploy(launcherTemplates[_templateId], "", false);
    ...
    /**
     * @notice Creates a liquidity launcher corresponding to _templateId.
     * @param _templateIndex Template index of the liquidity launcher to create.
     * @return launcher Liquidity launcher address.
     */
    function createLiquidityLauncherFromIndex(uint256 _templateIndex) external returns (address launcher) {
        address templateAddress = liquidityTemplateIndex[_templateIndex];
        require(templateAddress != address(0), "MISOLiquidityLauncher.createLiquidityLauncher: Template does not exist");
        require(liquidityTemplateToId[templateAddress] != 0, "MISOLiquidityLauncher.createLiquidityLauncher: Template removed");
        launcher = bentoBox.deploy(templateAddress, "", false);

        // isChildLiquidityLauncher[address(launcher)] = true;
        launchers.push(address(launcher));
        emit LiquidityLauncherCreated(msg.sender, address(launcher), templateAddress);
    }
    ...
    */
    function addLiquidityLauncherTemplate(address _template) external {
        ...
        uint256 templateID = IMisoLiquidity(_template).liquidityTemplate();
        require(templateID > 0, "MISOLiquidityLauncher: Incorrect template ID ");
        launcherTemplates[templateID] = _template;
        liquidityTemplateIndex[launcherTemplateIndex] = _template;
        liquidityTemplateToId[_template] = templateID;

        launcherTemplateIndex++;

        emit LiquidityTemplateAdded(_template, templateID);
    }
    ...
    function getLiquidityTemplateIndex(uint256 _indexId) external view returns (address) {
        return liquidityTemplateIndex[_indexId];
    }
