...
contract LAND is Context, AccessControlEnumerable, ERC721Enumerable, ERC721Pausable, Ownable, IMintableERC721{
    ...
    /**
     * @dev Grants `DEFAULT_ADMIN_ROLE`, `PREDICATE_ROLE` and `PAUSER_ROLE` to the
     ...
     * Requirements:
     *
     * - the caller must have the `PREDICATE_ROLE`.
     */
    function mint(address to, uint256 tokenId) public virtual override{
        require(hasRole(PREDICATE_ROLE, _msgSender()), "LAND: must have predicate role to mint");
        _mint(to, tokenId);
    }
     ...
     * @dev Creates a new token for `to`. Its token ID will be manually
     * assigned (and available on the emitted {IERC721-Transfer} event), and the token
     * URI autogenerated based on the base URI passed at construction. metaData will be ignored.
     *
     * See {ERC721-_mint}.
     *
     * Requirements:
     ...
     * - the caller must have the `PREDICATE_ROLE`.
     */
    function mint(address to, uint256 tokenId, bytes calldata metaData) public virtual override{
        require(hasRole(PREDICATE_ROLE, _msgSender()), "LAND: must have predicate role to mint");
        _mint(to, tokenId);
    ...
    function pause() public virtual {
        require(hasRole(PAUSER_ROLE, _msgSender()), "LAND: must have pauser role to pause");
        _pause();
    }
    ...
    function unpause() public virtual {
        require(hasRole(PAUSER_ROLE, _msgSender()), "LAND: must have pauser role to unpause");
        _unpause();
    }
