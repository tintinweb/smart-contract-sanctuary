...
import "./mixins/FoundationNode.sol";
  ...
  NFTMarketReserveAuction
{
   ...
   */
  function initialize(address payable f8n) public initializer {
    FoundationNode._initializeFoundationNode(f8n);
...
/**
 * @notice A mixin that stores a reference to the main Foundation contract.
 */
abstract contract FoundationNode is Initializable {
  using AddressUpgradeable for address payable;

  address payable private f8n;

  function _initializeFoundationNode(address payable _f8n) internal initializer {
    require(_f8n.isContract(), "FoundationNode: Foundation address is not a contract");
    f8n = _f8n;
  }

  /**
   * @notice Returns the address of the main Foundation contract.
   */
  function getF8n() public view returns (address payable) {
    return f8n;
  }
...
import "../FoundationNode.sol";

/**
 * @notice Allows a contract to leverage an admin role defined by the Foundation contract.
 */
abstract contract FoundationAdminRole is FoundationNode {
  modifier onlyFoundationAdmin() {
    require(isFoundationAdmin(), "FoundationAdminRole: caller does not have the Admin role");
    _;
  }

  function isFoundationAdmin() internal view returns (bool) {
    return AdminRole(getF8n()).isAdmin(msg.sender);
  }
...
abstract contract SendValueWithFallbackWithdraw is
  ReentrancyGuardUpgradeable,
  // Adding these unused dependencies to help with linearization
  FoundationAdminRole,
  NFTMarketCore
{
  using AddressUpgradeable for address payable;
 ...
 */
abstract contract NFTMarketCreators {
  /**
  ...
  SendValueWithFallbackWithdraw
{
  ...
  uint256 private _primaryF8nFeeBasisPoints;
  uint256 private _secondaryF8nFeeBasisPoints;
  ...
  function getIsPrimary(address nftContract, uint256 tokenId) public view returns (bool) {
    return !nftContractToTokenIdToPrimaryCompleted[nftContract][tokenId];
  }
  ...
  {
    return (_primaryF8nFeeBasisPoints, _secondaryF8nFeeBasisPoints, _secondaryCreatorFeeBasisPoints);
  }
  ...
  {
    uint256 f8nFeeBasisPoints;
    uint256 creatorFeeBasisPoints;
    if (getIsPrimary(nftContract, tokenId)) {
      f8nFeeBasisPoints = _primaryF8nFeeBasisPoints;
    } else {
      f8nFeeBasisPoints = _secondaryF8nFeeBasisPoints;
    ...
    f8nFee = price.mul(f8nFeeBasisPoints) / BASIS_POINTS;
    creatorSecondaryFee = price.mul(creatorFeeBasisPoints) / BASIS_POINTS;
    ownerRev = price.sub(f8nFee).sub(creatorSecondaryFee);
  }

  /**
   * @dev Distributes funds to F8N, creator, and NFT owner after a sale.
   */
  ...
  {
    (f8nFee, creatorFee, ownerRev) = getFees(nftContract, tokenId, price);

    nftContractToTokenIdToPrimaryCompleted[nftContract][tokenId] = true;

    _sendFunds(getF8n(), f8nFee);
    if (creatorFee > 0) {
  ...
  /**
   * @notice Allows F8N to change the market fees.
   */
  function _updateMarketFees(
    ...
    require(primaryF8nFeeBasisPoints < BASIS_POINTS, "NFTMarketFees: Fees >= 100%");
    require(
      secondaryF8nFeeBasisPoints.add(secondaryCreatorFeeBasisPoints) < BASIS_POINTS,
      "NFTMarketFees: Fees >= 100%"
    );
    _primaryF8nFeeBasisPoints = primaryF8nFeeBasisPoints;
    _secondaryF8nFeeBasisPoints = secondaryF8nFeeBasisPoints;
    _secondaryCreatorFeeBasisPoints = secondaryCreatorFeeBasisPoints;

    emit MarketFeesUpdated(primaryF8nFeeBasisPoints, secondaryF8nFeeBasisPoints, secondaryCreatorFeeBasisPoints);
  }
  ...
  NFTMarketAuction
{
  ...
  /**
   * @notice Allows Foundation to migrate NFTs originally sold on xDai.
   * @dev This attempts to replicate the history of a standard auction in order for the history
   * to be compatible with future sales.
   */
  function adminMigrateNFT(
    address nftContract,
    uint256 tokenId,
    address payable seller,
    ...
    uint256 saleAmount,
    uint256 saleTime,
    string memory historyMetadata
  ) public onlyFoundationAdmin {
    // Simulate Create Auction
    uint256 auctionId = _getNextAndIncrementAuctionId();
    emit ReserveAuctionCreated(seller, nftContract, tokenId, 0, 0, saleAmount, auctionId);

    // Simulate Place Bid
    emit ReserveAuctionBidPlaced(auctionId, buyer, saleAmount, block.timestamp);

    // Simulate Finalize Auction
    (uint256 f8nFee, uint256 creatorFee, uint256 ownerRev) = getFees(nftContract, tokenId, saleAmount);
    _enterSecondarySaleState(nftContract, tokenId);
    IERC721Upgradeable(nftContract).transferFrom(seller, buyer, tokenId);
    emit ReserveAuctionFinalized(auctionId, seller, buyer, f8nFee, creatorFee, ownerRev);

    emit NFTMigrated(nftContract, tokenId, seller, buyer, saleAmount, saleTime, historyMetadata);
  }
  ...
  NFT721Mint
{
  ...
  function adminUpdateConfig(
    ...
    _updateTokenMetadata(_name, _symbol, baseURI);
  }
...
/**
 * Copied from the OZ repository in order to make the following changes:
...
// SPDX-License-Identifier: MIT OR Apache-2.0
// From https://etherscan.io/address/0x44d6e8933f8271abcf253c72f9ed7e0e4c0323b3#code
...
/**
 * @notice An interface for communicating fees to 3rd parties such as OpenSea.
 */
abstract contract HasSecondarySaleFees is ERC165Upgradeable {
  /*
  ...
  function _initializeHasSecondarySaleFees() internal {
...
/**
 * @notice Holds a reference to the Foundation Market and communicates fees to 3rd parties such as OpenSea.
 */
abstract contract NFT721Market is FoundationAdminRole, HasSecondarySaleFees, NFT721Creator {
  using AddressUpgradeable for address;
  ...
  function getFeeBps(
    ...
    (, uint256 secondaryF8nFeeBasisPoints, uint256 secondaryCreatorFeeBasisPoints) =
      IFNDNFTMarket(nftMarket).getFeeConfig();
 ...
 */
abstract contract NFT721Creator is FoundationAdminRole, ERC721Upgradeable {
   ...
   * @dev This aligns with the API used by SuperRare.
   */
  ...
  /**
   * @notice Allows the current creator to change creators for a given token.
   */
  function updateTokenCreator(uint256 tokenId, address payable creator) public onlyCreator(tokenId) {
    _updateTokenCreator(tokenId, creator);
  }
...
/**
 * @notice A mixin to extend the OZ metadata implementation.
 */
abstract contract NFT721Metadata is FoundationAdminRole, NFT721Creator {
  using StringsUpgradeable for uint256;

  mapping(address => mapping(string => bool)) private creatorToHashToMinted;
   ...
   */
  function getHasCreatorMintedHash(address creator, string memory tokenIPFSPath) public view returns (bool) {
    return creatorToHashToMinted[creator][tokenIPFSPath];
  }

  /**
   * @notice Allows the creator to change the NFT if they currently own it.
   * @dev The IPFS path should be the CID + file.extension, e.g.
   * `QmfPsfGwLhiJrU8t9HpG4wuyjgPo9bk8go4aQqSu9Qg4h7/metadata.json`
   */
  function updateTokenIPFSPath(uint256 tokenId, string memory tokenIPFSPath) public onlyCreatorAndOwner(tokenId) {
    _setTokenIPFSPath(tokenId, tokenIPFSPath);
    emit TokenIPFSPathUpdated(tokenId, tokenIPFSPath, tokenIPFSPath);
  }
  ...
  function _updateTokenMetadata(
    ...
    emit NFTMetadataUpdated(name, symbol, _baseURI);
  }
  ...
  function _setTokenIPFSPath(uint256 tokenId, string memory _tokenIPFSPath) internal {
    ...
    require(!creatorToHashToMinted[msg.sender][_tokenIPFSPath], "NFT721Metadata: NFT was already minted");

    creatorToHashToMinted[msg.sender][_tokenIPFSPath] = true;
   ...
   */
  function _burn(uint256 tokenId) internal virtual override {
    delete creatorToHashToMinted[msg.sender][_tokenURIs[tokenId]];
    super._burn(tokenId);
  }
...
/**
 * @notice Allows creators to mint NFTs for Foundation.
 */
abstract contract NFT721Mint is ERC721Upgradeable, NFT721Creator, NFT721Market, NFT721Metadata {
