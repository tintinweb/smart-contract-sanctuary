...
contract MarvelousNFT is AccessControl, ERC20Burnable, ERC20Pausable, AccessControlMixin, ContextMixin, NativeMetaTransaction, AccessRights  {
    ...
    function _partialBurn(address from, uint256 amount) internal returns (uint256) {
        ...
        if (_burnAmount > 0) {
            ...
            stakingPool = stakingPool.add((_burnAmount.mul(1)).div(5));
    ...
    function setStakingPoolWallet(address payable wallet) external virtual only(CREATOR_ROLE) {
    ...
    function withdrawStakingPool(uint256 amount) external only(CREATOR_ROLE) {
