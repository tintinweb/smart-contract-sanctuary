...
contract StableSwap is Ownable, ReentrancyGuard, Pausable {
    ...
    function migrate(address _to) external onlyOwner {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            ERC20(target).safeTransfer(_to, ERC20(target).balanceOf(address(this)));
            balanceOf[target] = 0;
            tokens[i] = target;
        }
        for (uint256 i = 0; i < length; i++) {
            _whitelist.remove(tokens[i]);
        }
        lp.grantRole(lp.DEFAULT_ADMIN_ROLE(), _to);
        lp.grantRole(lp.MINTER_ROLE(), _to);
        lp.grantRole(lp.PAUSER_ROLE(), _to);
        lp.renounceRole(lp.DEFAULT_ADMIN_ROLE(), address(this));
        lp.renounceRole(lp.MINTER_ROLE(), address(this));
        lp.renounceRole(lp.PAUSER_ROLE(), address(this));
    }
    */
        ...
        )
    {
        ...
        if (amountFee > 0) {
            ...
            if (amountFeedback > 0) {
                ...
                require(
                    path.length == amounts.length && amounts[0] == amountFeedback,
