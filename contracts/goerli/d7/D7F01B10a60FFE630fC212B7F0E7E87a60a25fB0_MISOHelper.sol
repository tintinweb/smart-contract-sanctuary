...
interface IMisoCrowdsale {
    ...
    function getMarketPrice()
        external
        view
        returns (uint128 rate, uint128 goal);
...
interface IMisoMarket {
    ...
    function marketStatus() external view returns(uint128 commitmentsTotal, bool initialized, bool finalized, bool hasPointList);
}
...
contract MISOHelper is TokenHelper, DocumentHepler {
    ...
    {
        IMisoMarket dutchAuction = IMisoMarket(_dutchAuction);
    ...
    {
        IMisoMarket batchAuction = IMisoMarket(_batchAuction);
    ...
    {
        IMisoMarket hyperbolicAuction = IMisoMarket(_hyperbolicAuction);
