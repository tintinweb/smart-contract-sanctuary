...
contract Attack is IAttack, Ownable, Pausable {
    ...
    function manyBearsAttack(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            (uint256 o, uint256 s) = _attack(seed, tokenIds[i], hiveIds[i]);
            ...
            if (s >= 1) {
                ...
                // if (!transfer) beesContract.increaseTokensPot(tokenIds[i], uint32(o));
            }
            owed += o;
            successes += s;
            uint48 hiveAge = uint48(block.timestamp) - hiveContract.getHiveAge(hiveIds[i]);
            uint256 cooldown = (((hiveAge / 86400) * bearCooldownPerHiveDay) + bearCooldownBase);
    ...
    function _attack(
        ...
        for (uint256 y = 0; y < beesAffected; y++) {
            uint256 h = (random(seed + tokenId + y) & 0xFFFF);
            if ((h % 100) < bearChance) {
