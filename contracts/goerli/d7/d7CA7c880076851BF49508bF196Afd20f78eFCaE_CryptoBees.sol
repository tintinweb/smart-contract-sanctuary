...
contract CryptoBees is ICryptoBees, ERC721Enumerable, Ownable, ReentrancyGuard, Pausable {
    ...
    function tryReveal(uint256 revealIndex) public returns (bool) {
        if (revealIndex >= minted) return false;
        if (tokenData[revealIndex + 1]._type != 0) return true;

        uint256 seed = randomizerContract.revealSeed(revealIndex);
        if (seed == 0) return false;

        address trueOwner = ownerOf(revealIndex + 1) == address(hiveContract) ? hiveContract.getWaitingRoomOwner(revealIndex + 1) : ownerOf(revealIndex + 1);
        ...
        if (t._type == 1 && ownerOf(revealIndex + 1) == address(hiveContract)) {
            hiveContract.removeFromWaitingRoom(revealIndex + 1);
        }
        unrevealedTokenIndex += 1;
        emit MintRevealed(trueOwner, revealIndex + 1, t._type, ownerOf(revealIndex + 1) == address(hiveContract) ? true : false);
        return true;
    }
    ...
    function increaseTokensPot(uint256 tokenId, uint32 amount) external {
        ...
        require(_msgSender() == address(attackContract) || _msgSender() == address(hiveContract), "BEES:DONT CHEAT:TP");
