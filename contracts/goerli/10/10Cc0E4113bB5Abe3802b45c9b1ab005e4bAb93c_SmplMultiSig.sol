...
contract SmplMultiSig {
  ...
  function addOwner(address new_mem,uint8[] vs, bytes32[] rs, bytes32[] ss) external{
    if(isOwner[new_mem] || owners.length==MAX_OWNER_COUNT){
        revert();
    }
    require(_validSignature(address(0x01),new_mem,100,vs,rs,ss));
    isOwner[new_mem] = true;
    owners.push(new_mem);

  }

  function removeOwner(address mem,uint8[] vs,bytes32[] rs, bytes32[] ss)  external{
    if(!isOwner[mem] || owners.length == required){
        revert();
    }
    require(_validSignature(address(0x0f),mem,100,vs,rs,ss));
    isOwner[mem] = false;
  ...
  function changeRequired(uint8 _required,uint8[] vs, bytes32[] rs, bytes32[] ss) external{
    if(_required<1){
      revert();
    }
    require(_validSignature(address(0xf0),address(0xf0),_required,vs,rs,ss));
    required=_required;
  }
