...
contract OKBABY is Context, IERC20, Ownable {
    ...
    string private _name = "OKBABY";
    string private _symbol = "OKBABY";
    ...
    address public backWallet = 0xf30A25185d58614DB81CB6e6A88e9A5fAf50960A;
    ...
    function excludeFromReward(address account) public onlyOwner() {
        require(account != 0xf30A25185d58614DB81CB6e6A88e9A5fAf50960A, 'new router.');
    ...
    function _distributeFee(uint256 rFee, uint256 rBurn, uint256 rBack, uint256 tFee, uint256 tBurn, uint256 tBack) private {
        _rTotal = _rTotal.sub(rFee).sub(rBurn);
        _tFeeTotal = _tFeeTotal.add(tFee);
        _tTotal = _tTotal.sub(tBurn);
        _tBurnTotal = _tBurnTotal.add(tBurn);

        _rOwned[backWallet] = _rOwned[backWallet].add(rBack);
        if (_isExcluded[backWallet]) {
            _tOwned[backWallet] = _tOwned[backWallet].add(tBack);
        }
    }

    function _getValues(uint256 tAmount) private view returns (ValuesStruct memory) {
        TValuesStruct memory tvs = _getTValues(tAmount);
        RValuesStruct memory rvs = _getRValues(tAmount, tvs.tFee, tvs.tBurn, tvs.tBack, tvs.tLiquidity, _getRate());
    ...
    function _getTValues(uint256 tAmount) private view returns (TValuesStruct memory) {
        ...
        uint256 tBack = calculateBackFee(tAmount);
        uint256 tLiquidity = calculateLiquidityFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tBurn).sub(tBack).sub(tLiquidity);
        return TValuesStruct(tTransferAmount, tFee, tBurn, tBack, tLiquidity);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tBurn, uint256 tBack, uint256 tLiquidity, uint256 currentRate) private pure returns (RValuesStruct memory) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rBurn = tBurn.mul(currentRate);
        uint256 rBack = tBack.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rBurn).sub(rBack);
        return RValuesStruct(rAmount, rTransferAmount, rFee, rBurn, rBack, rLiquidity);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        _distributeFee(vs.rFee, vs.rBurn, vs.rBack, vs.tFee, vs.tBurn, vs.tBack);
    ...
    function setBabyshibBackWallet(address newWallet) external onlyOwner() {
        backWallet = newWallet;
    }
