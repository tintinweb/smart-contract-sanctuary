...
abstract contract ERC721Tradable is ERC721Enumerable, Ownable {
    ...
    uint16 private m_unCurrTokenId = 0;
		...
		uint16 m_unBurnItemCount;
		uint16 BurntTokenTotal;
		uint256 public BurnPayoutTotal = 0;
		uint256 public m_unBurntTokenTotal;
    ...
    constructor(string memory _name, string memory _symbol) 
		...
		{
			require(m_unCurrTokenId + unCount < MAX_TOKEN_COUNT, "Token supply has been exhausted.");
    ...
    function mintTokens(uint16 unCount) public payable {
				require(!m_bLock, "Contract locked to prevent reentrancy");
				m_bLock = true;

				require(m_bSalesEnabled == true, "Minting is currently unavailable.");
				require(unCount <= MAX_SINGLE_MINT_COUNT, "You may only mint up to 10 tokens at a time.");
				require(msg.value >= TOKEN_PRICE * unCount, "Ether value sent is less than total token price.");
				require(m_unCurrTokenId + unCount < MAX_TOKEN_COUNT, "Token supply has been exhausted.");
				require(m_adrSaleReceiver != address(0), "Payment collection is not setup.");
        	...
        	_safeMint(msg.sender, newTokenId);
				...
				m_TotalCowsMinted += unCount;

				(bool bFinalSuccess, ) = m_adrSaleReceiver.call{value: msg.value}("");
				require(bFinalSuccess, "Transfer to sale receiver failed.");
		...
		function burnToken(uint16 unTokenID) public 
		{
			...
			require(ownerOf(unTokenID) == msg.sender, "Surrender caller is not owner of token.");
			_burn(unTokenID);
		...
		{
			require(m_unBurnItemCount > 0, "There are no burn records.");
			require(m_unBurntTokenTotal > 0, "There are no burnt tokens.");
			...
			for(uint16 i = 1; i <= m_unBurnItemCount; ++i)
			{
				...
				require(bCurrSuccess, "Transfer to burn holder failed.");
			}
    ...
    function incrementTokenId() private 
		...
		{
			require(m_bLicenseLocked == false, "License has already been locked and cannot change.");
