...
contract PKKTFarm is Ownable {
    ...
    //remove an unused lp token, usually if mistakenly added
    function remove(IERC20 _lpToken, bool _withUpdate) external onlyOwner {
        ...
        for (uint256 pid = 0; pid < length; pid++) {
             ...
             if (pool.lpToken == _lpToken) {
                ...
                for (uint i = pid; i < length-1; i++) {
                    poolInfo[i] = poolInfo[i+1];
                }
