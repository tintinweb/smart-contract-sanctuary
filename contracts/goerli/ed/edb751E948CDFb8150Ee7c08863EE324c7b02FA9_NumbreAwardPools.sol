...
contract NumbreAwardPools is INumbreSeason, Initializable {
    ...
    function getEndBlackNum() public returns (uint256) {
        return endBlackNum;
    }
    ...
    function liquidationSeason() public returns (uint256) {
        ...
        uint256 _prize = (address(this).balance - info.unBonus) * info.winnerRatio / 100 / info.winnerNumber;
        info.winnerPrize = _prize * info.winnerNumber;
        info.poolPrize = (address(this).balance - info.unBonus) * info.poolRatio / 100;
        info.devPrize = (address(this).balance - info.unBonus) * info.daoRatio / 100;
        info.daoPrize = address(this).balance - info.unBonus - info.winnerPrize - info.poolPrize - info.devPrize;
        info.unBonus += info.winnerPrize *  info.winnerNumber;
