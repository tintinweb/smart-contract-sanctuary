...
contract DoubleMoon is Context, IERC20, Ownable {
	...
	event TestLog(uint256 contractTokenBalance, address from, address pancakeSwapPair);
    event TestLogSwap(string s);
	...
	function _transfer(
		...
		emit TestLog(contractTokenBalance, from, pancakeSwapPair);
		...
		if (
		    ...
		    emit TestLogSwap('Swaping');
			contractTokenBalance = numTokensSellToAddToLiquidity;
			//add liquidity
			swapAndLiquify(contractTokenBalance);
		} else {
		    emit TestLogSwap('No Swap');
		}
