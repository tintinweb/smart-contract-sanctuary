...
contract BullMorph is ERC721PresetMinterPauserAutoId, ReentrancyGuard {

    using Counters for Counters.Counter;

    uint256 public constant maxBULLS = 10000;
    uint256 public constant numTRAITS = 8;
    uint256 public constant bulkBuyLimit = 25;

    uint256 internal nonce;
    address public preMintWallet;

    uint8[numTRAITS][maxBULLS] internal bullTraits;
    uint16[][numTRAITS] internal traitProbs;
    ...
    constructor() ERC721PresetMinterPauserAutoId('Bulls', 'B') public {

        preMintWallet = msg.sender;
        unitBullPrice = 0.07 ether;
        traitProbs[0] = [1,2]; // Backgrounds
        traitProbs[1] = [1,2]; // Bull
        traitProbs[2] = [0,1,2]; // Mouth
        traitProbs[3] = [0,1,2]; // Accesories
        traitProbs[4] = [1,2]; // Eyes and Glasses
        traitProbs[5] = [0,1,2]; // Neck
        traitProbs[6] = [1,2]; // Horns
        traitProbs[7] = [0,1,2]; // Hast & Hair
        nonce = 12;
    ...
    function _premint() internal {
      for (uint256 i = 0; i < 50; i++) {
          ...
          _mint(preMintWallet, tokenId);
      }
    }

    function mint() public payable nonReentrant {
        require(_tokenIdTracker.current() < maxBULLS, "Total supply reached");
        require(msg.value == unitBullPrice, 'You need to send 0.07 eth');
    ...
    function addTraits(uint seed, uint tokenId) internal {
        for(uint8 i = 0; i < numTRAITS; i++){
            nonce++;
            bullTraits[tokenId][i] = determineTrait(i, seed);
        }
    }
    ...
    function getTraits(uint tokenId, uint i) public view returns (uint){
        require(_exists(tokenId), "ERC721Metadata: Trait query for nonexistent token");
        return (bullTraits[tokenId][i]);
    }
    ...
    function bulkBuy(uint256 amount) public payable nonReentrant {
        ...
        require(msg.value == unitBullPrice*amount, 'You need to send 0.1ether * nfts desired');
