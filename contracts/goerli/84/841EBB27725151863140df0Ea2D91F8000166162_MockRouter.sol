...
contract MockRouter is FxBaseRootTunnel {
    constructor(address fxRootAddress, address checkpointManagerAddress) {
        FxBaseRootTunnel.initialize(checkpointManagerAddress, fxRootAddress);
    }

    function sendMessage(address[] memory user) public {
        _sendMessageToChild(abi.encode(user));
    }
