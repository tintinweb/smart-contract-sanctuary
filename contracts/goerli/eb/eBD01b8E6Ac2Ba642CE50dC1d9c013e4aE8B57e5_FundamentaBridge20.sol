...
contract FundamentaBridge20 is SecureContract
{
    ...
    event NewTokenAdded(address sender, uint256 tokenId, bool isWrappedToken, uint256 numSigners, address tokenAddress);
    ...
    function setup(uint256 bridgeId) public initializer
    {
        SecureContract.setup();
    ...
    function addToken(uint256 id, bool isWrappedToken, uint256 numSigners, address tokenAddress) public isAdmin
    {
        ...
        emit NewTokenAdded(msg.sender, id, isWrappedToken, numSigners, tokenAddress);
    }
    ...
    function deposit(bytes memory clientSignature, bytes[] memory serverSignatures, bytes memory transactionData) public pause
    {
        ...
        require(tokens_[destToken].Info.Token != TokenInterface(address(0)), "Bridge: Token not registered");
        TokenData storage token = tokens_[destToken];

        token.DepositNonces[nonce] = true;
        token.Info.Token.mintTo(sender, amount);
    ...
    function depositAndUnwrap(bytes memory clientSignature, bytes[] memory serverSignatures, bytes memory transactionData) public pause
    {
        ...
        token.DepositNonces[nonce] = true;
        token.Info.Token.transferBackingTokenTo(sender, amount);
