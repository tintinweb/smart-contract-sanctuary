...
abstract contract NFT721Market is FoundationTreasuryNode, HasSecondarySaleFees, NFT721Creator {
  ...
  function getFeeRecipients(uint256 id) public view override returns (address payable[] memory) {
    ...
    result[1] = tokenPaymentAddress(id);
    return result;
  }
    ...
    returns (address payable[2] memory recipients, uint256[2] memory feesInBasisPoints)
  {
    ...
    recipients[1] = tokenPaymentAddress(tokenId);
    (, uint256 secondaryF8nFeeBasisPoints, uint256 secondaryCreatorFeeBasisPoints) = nftMarket.getFeeConfig();
...
abstract contract NFT721Creator is Initializable, ERC721Upgradeable {
  ...
  // TODO: Find a name for payment address without using address in the name
  // TODO: Maybe move this mapping to another mixin, since this one is
  // for creator specifically. Or generalize the name of this mixin
  // to include other onchain metadata about the token
  mapping(uint256 => address payable) private tokenIdToPaymentAddress;

  event TokenCreatorUpdated(address indexed fromCreator, address indexed toCreator, uint256 indexed tokenId);

  event TokenPaymentAddressSet(
  ...
  /*
   * bytes4(keccak256('tokenPaymentAddress(uint256)')) == {ADD_ME}
   */
  bytes4 private constant _INTERFACE_TOKEN_PAYMENT_ADDRESS = 0x0; // TODO: Use the real hex value
   ...
   */
  function tokenPaymentAddress(uint256 tokenId) public view returns (address payable) {
    // TODO: If the value is 0x0, return the token creator instead
    // for backwards compatibility
    return tokenIdToPaymentAddress[tokenId];
  }
   ...
   */
  function _setTokenPaymentAddress(uint256 tokenId, address payable paymentAddress) internal {
    if (paymentAddress != address(0)) {
      tokenIdToPaymentAddress[tokenId] = paymentAddress;
      emit TokenPaymentAddressSet(address(0), paymentAddress, tokenId);
    }
  }
...
abstract contract NFT721Mint is Initializable, ERC721Upgradeable, NFT721Creator, NFT721Market, NFT721Metadata {
   ...
   */
  function mintWithPaymentAddress(string memory tokenIPFSPath, address payable paymentAddress)
    public
    returns (uint256 tokenId)
  {
    tokenId = _mintToken(tokenIPFSPath, paymentAddress);
  }
  ...
  function mint(string memory tokenIPFSPath) public returns (uint256 tokenId) {
    tokenId = _mintToken(tokenIPFSPath, msg.sender);
  }
  ...
  function mintWithPaymentAddressAndApproveMarket(string memory tokenIPFSPath, address payable paymentAddress)
    public
    returns (uint256 tokenId)
  {
    tokenId = _mintToken(tokenIPFSPath, paymentAddress);
  ...
  function _mintToken(string memory tokenIPFSPath, address payable paymentAddress) private returns (uint256 tokenId) {
    tokenId = nextTokenId++;
    _mint(msg.sender, tokenId);
    _updateTokenCreator(tokenId, msg.sender);
    _setTokenPaymentAddress(tokenId, paymentAddress);
