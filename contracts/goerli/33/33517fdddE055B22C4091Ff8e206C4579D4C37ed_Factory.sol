...
/**
 * Implementation of IFactory.sol
 * For function explainer, please refer the interface.
 */

contract Factory is IFactory, FxBaseRootTunnel {
    // total valid detfs
    uint256 private currentDetfLength;

    // mapping of the detf name to it's address
    mapping(bytes => address) private detfMap;

    // mapping detf to it's constituents in progressive order.
    mapping(bytes => address[]) private detfConstituentsL1;

    // mapping detf to it's constituents in progressive order on (L2 POS)
    mapping(bytes => address[]) private detfConstituentsL2;

    // mapping of the stablecoin used to detf name
    mapping(bytes => address) private stableCoinL1;

    //mapping of the stablecoin address on Matic (L2 POS)
    mapping(bytes => address) private stableCoinL2;

    function init(address _checkpointManager, address _fxRoot) public {
    ...
    function addDetf(
        ...
        detfMap[bytes(_detfName)] = _detfAddress;
        stableCoinL1[bytes(_detfName)] = _stablecoinL1;
        stableCoinL2[bytes(_detfName)] = _stablecoinL2;
        currentDetfLength += 1;
        return true;
    }
    ...
    function updateDetfConstituents(
        ...
        bytes memory detfName = bytes(_detfName);

        detfConstituentsL1[detfName] = _l1;
        detfConstituentsL2[detfName] = _l2;

        // Sending message to tunnel
        FactoryData memory factoryData;
        factoryData.detfName = detfName;
        factoryData.l1 = _l1;
        factoryData.l2 = _l2;
        // Uncomment during deployment.
        sendMessageToChild(factoryData);
        return true;
    }

    function removeDetf(string memory _detfName)
        public
        virtual
        override
        returns (bool)
    {
        detfMap[bytes(_detfName)] = address(0);
        currentDetfLength -= 1;
        return true;
    }

    function allDetfsLength() public view virtual override returns (uint256) {
        return currentDetfLength;
    }
    ...
    function detfAddress(string memory _detfName)
        ...
        returns (address)
    {
        return detfMap[bytes(_detfName)];
    }
    ...
    {
        return (detfConstituentsL1[bytes(_detfName)]);
    }

    // sending message to child factory.
    function sendMessageToChild(FactoryData memory data) internal virtual {
