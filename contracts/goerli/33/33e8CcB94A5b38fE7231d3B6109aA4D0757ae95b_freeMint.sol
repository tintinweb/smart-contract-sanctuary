...
contract freeMint is Ownable, IERC721Receiver, ReentrancyGuard {
    using Strings for uint256;
    using SafeMath for uint256;
    using Address for address;
    EthJuanchos ethJuanchoContract;

    mapping(address => uint256) public whitelisted;
    event freeMinted(address indexed freeMinter, uint256 indexed tokenId);

    constructor(){

    }
    function whitelistUser(address[] memory _users, uint256[] memory _quantities) public onlyOwner {
        require(_users.length >0, "You passed an empty list of addresses");
        require(_quantities.length >0, "You passed an empty list of quantities");
        require(_users.length == _quantities.length, "Arrays are not of equal length");
        for(uint256 i=0; i<=_users.length-1;i++){
            whitelisted[_users[i]] = _quantities[i];
        }
    }
    ...
    function setContract(address _t) public onlyOwner {
        ethJuanchoContract = EthJuanchos(_t);
    }
    ...
    function freeMinter() public nonReentrant{
        require(whitelisted[msg.sender] > 0, "You cannot free mint anymore.");
        ethJuanchoContract.createCollectible();    
        if(ethJuanchoContract.balanceOf(address(this)) >0) { // if this freeMint smartContracts owns at least 1 NFT
            uint256 counter = ethJuanchoContract.tokenCounter();
            while(ethJuanchoContract.ownerOf(counter-1) != address(this)){ //Let's check which NFT does this smartcontract owns.
                counter--;
            }   
        ethJuanchoContract.safeTransferFrom(address(this), msg.sender,counter-1); //let's transfer the NFT owned by this smartcontract to the freeMinter recipient.
        whitelisted[msg.sender] = whitelisted[msg.sender] -1;
        emit freeMinted(msg.sender, counter-1);
        }   

    }

    function canImint(address _address) public returns (bool){
