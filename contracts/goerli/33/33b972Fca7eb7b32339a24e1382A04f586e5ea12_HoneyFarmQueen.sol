...
contract HoneyFarmQueen is Ownable, ReentrancyGuard {
    ...
    function burnBees(uint16[] calldata _beesIds) external nonReentrant {
        ...
        if (honey.totalSupply() + amountOfHoney > honey.maxSupply())
            amountOfHoney = (honey.maxSupply() - honey.totalSupply()) / 1e18;

        if (amountOfHoney > 0) honey.mint(msg.sender, amountOfHoney);
    }
    ...
    function stakeBees(uint16[] calldata _beesIds) external {
        ...
        require(honey.balanceOf(msg.sender) >= amountOfHoney * 1e18, "Not enough honey");
        if (amountOfHoney > 0) honey.burn(msg.sender, amountOfHoney * 1e18);
    }
...
abstract contract HoneyHiveI is Ownable, IERC721 {
    ...
    function eligibleToMint(uint16 _bearsId) external view virtual returns (bool eligible);
}
