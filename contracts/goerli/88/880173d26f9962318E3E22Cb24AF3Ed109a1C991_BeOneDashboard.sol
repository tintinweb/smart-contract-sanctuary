...
import "./CErc20.sol";
import "./BeOneRegistry.sol";
import "./EIP20Interface.sol";
import "./SafeMath.sol";
import "./Exponential.sol";

interface BeOnePriceOracle {
  function getUnderlyingPrice(CToken cToken) external view returns (uint256);
  function getStoredTokenPrice(address tokenAddress) external view returns (uint256 price, uint256 timestamp);
}
...
interface Unitroller {
  ...
  function isBorrowAllowed(CToken cToken) external view returns (bool);
...
contract BeOneDashboard is Exponential{
    using SafeMath for *;
  uint224 public constant compInitialIndex = 1e36;


  BeOneRegistry beOneRegistry;
  uint32 numberOfBlocksPerDay;

  constructor(address beOneRegistryAddress, uint32 _numberOfBlocksPerDay) public {
    beOneRegistry = BeOneRegistry(beOneRegistryAddress);
    numberOfBlocksPerDay = _numberOfBlocksPerDay;
  }

  struct Dashboard {
    uint256 beOneTokenPriceInUsd;
    uint256 apr;
    uint256 totalLendedInUsd;
    uint256 totalBorrowedInUsd;
    uint256 userBeOneTokenBalance;
    uint256 userTotalClaimableReward;
    CToken[] CTokens;
  }
  ...
  struct CToken {
    ...
    string tokenName;
    string tokenSymbol;
    uint8 tokenDecimalsToDisplay;
    uint256 tokenPriceInUsd;
    uint256 exchangeRate;
    bool isBorrowable;
    uint256 lendingApr;
    uint256 borrowingApr;
    uint256 borrowedInToken;
    uint256 userBalanceInUnderlyingToken;
    uint256 userBalanceInCToken;
    uint256 userBorrowedInToken;
    uint256 userBorrowableInToken;
    bool userIsCollateral;
    uint256 userClaimableReward;
    uint256 allowedAmountForToken;
    uint256 totalSupply;
    uint256 totalBorrows;

  }

  function getDashboard(address userAddress)
    external
    view
    returns (Dashboard memory)
  {
    BeOnePriceOracle beOnePriceOracle = BeOnePriceOracle(beOneRegistry.getPriceOracleAddress());
    Unitroller unitroller = Unitroller(beOneRegistry.getUnitrollerAddress());
    CToken[] memory CTokensArray = new CToken[](
      beOneRegistry.getCTokensLength()
    );
    uint256 totalSupplyInUsd;
    uint256 totalBorrowedInUsd;
    uint256 totalUserClaimableReward;
      
    
    for (uint256 i = 0; i < beOneRegistry.getCTokensLength(); i++) {
        CErc20 cToken = CErc20(beOneRegistry.getCtokenAddressByIndex(i));
        EIP20Interface underlying = EIP20Interface(cToken.underlying());
      CTokensArray[i] = CToken({
        cTokenAddress: address(cToken),
        cTokenDecimals: cToken.decimals(),
        underlyingAddress: address(underlying),
        underlyingDecimals: underlying.decimals(),
        tokenName: cToken.name(),
        tokenSymbol: cToken.symbol(),
        tokenDecimalsToDisplay: beOneRegistry.getTokenDecimalsToDisplay(cToken.name()),
        tokenPriceInUsd: beOnePriceOracle.getUnderlyingPrice(cToken),
        isBorrowable: unitroller.isBorrowAllowed(cToken),
        exchangeRate: cToken.exchangeRateStored(),
        // TODO / Add BeOne reward APR
        lendingApr: 10000.mul(cToken.supplyRatePerBlock().mul(numberOfBlocksPerDay).mul(365)).div(1e18),
        // TODO / Add BeOne reward APR
        borrowingApr: 10000.mul(cToken.borrowRatePerBlock().mul(numberOfBlocksPerDay).mul(365)).div(1e18),
        borrowedInToken: cToken.totalBorrows(),
        userBalanceInUnderlyingToken: underlying.balanceOf(userAddress),
        userBalanceInCToken: userBalanceInCToken(userAddress, cToken),
        userBorrowedInToken: cToken.borrowBalanceStored(userAddress),
        // TODO / See https://github.com/DecenterApps/defisaver-contracts/blob/aef5862217fe4fd8544a5182dd7789249ff9396f/contracts/compound/CompoundSafetyRatio.sol
        userBorrowableInToken: 4560000000000000000,
        userIsCollateral: unitroller.checkMembership(userAddress, cToken),
        userClaimableReward: getSuppyClaimableBalance(address(cToken), userAddress, unitroller),
        allowedAmountForToken: underlying.allowance(userAddress, address(cToken)),
        totalSupply: cToken.totalSupply(),
        totalBorrows: cToken.totalBorrows()
      });
        totalSupplyInUsd += cToken.totalSupply().mul(cToken.exchangeRateStored()).div(1e18).mul(CTokensArray[i].tokenPriceInUsd).div(uint(10) ** uint(cToken.decimals()));
        totalBorrowedInUsd += cToken.totalBorrows().mul(cToken.exchangeRateStored()).div(1e18).mul(CTokensArray[i].tokenPriceInUsd).div(uint(10) ** uint(cToken.decimals()));
        totalUserClaimableReward += CTokensArray[i].userClaimableReward;
    }      

    (uint256 beOneTokenPriceInUsd,) = beOnePriceOracle.getStoredTokenPrice(beOneRegistry.getRewardToken());
    return
      Dashboard({
        beOneTokenPriceInUsd: beOneTokenPriceInUsd,
        // TODO
        apr: 4321,
        totalLendedInUsd: totalSupplyInUsd,
        totalBorrowedInUsd: totalBorrowedInUsd,
        userBeOneTokenBalance: EIP20Interface(beOneRegistry.getRewardToken()).balanceOf(userAddress),
        userTotalClaimableReward: totalUserClaimableReward,
        CTokens: CTokensArray
      });
  }
    function userBalanceInCToken(address userAddress, CErc20 cToken) public view returns(uint256) {
      return cToken.balanceOf(userAddress).mul(cToken.exchangeRateStored()).div(1e18);
  }
    function getTotalClaimableBalance(address _cToken, address _user, Unitroller unitroller) public
        view
        returns (uint256 totalAccrued) {
            totalAccrued = getSuppyClaimableBalance(_cToken, _user, unitroller) + getBorrowClaimableBalance(_cToken, _user, unitroller);
        }

    function getSuppyClaimableBalance(address _cToken, address _supplier, Unitroller unitroller)
        public
        view
        returns (uint256 supplierAccrued)
    {
        Unitroller.CompMarketState memory supplyState = unitroller.compSupplyState(_cToken);
        Double memory supplyIndex = Double({mantissa: supplyState.index});
        uint deltaBlocks = (unitroller.getBlockNumber().add(1).sub(uint(supplyState.block)));
        uint supplySpeed = unitroller.compSpeeds(_cToken);
        if (deltaBlocks > 0 && supplySpeed > 0) {
            uint supplyTokens = CErc20(_cToken).totalSupply();
            uint compAccrued = mul_(deltaBlocks, supplySpeed);
            Double memory ratio = supplyTokens > 0 ? fraction(compAccrued, supplyTokens) : Double({mantissa: 0});
            supplyIndex = add_(Double({mantissa: supplyState.index}), ratio);
        }
        Double memory supplierIndex = Double({
            mantissa: unitroller.compSupplierIndex(_cToken, _supplier)
        });
        ...
        uint256 supplierTokens = CErc20(_cToken).balanceOf(_supplier);
        uint256 supplierDelta = mul_(supplierTokens, deltaIndex);
        supplierAccrued = supplierDelta;
    }

    function getBorrowClaimableBalance(address _cToken, address _borrower, Unitroller unitroller)
        public
        view
        returns (uint256 borrowerAccrued)
    {
        Unitroller.CompMarketState memory borrowState = unitroller.compBorrowState(_cToken);
        Double memory borrowIndex = Double({mantissa: borrowState.index});
        Double memory borrowerIndex = Double({
            mantissa: unitroller.compBorrowerIndex(_cToken, _borrower)
        });

        Exp memory marketBorrowIndex = Exp({mantissa: CErc20(_cToken).borrowIndex()});

        if (borrowerIndex.mantissa > 0) {
            Double memory deltaIndex = sub_(borrowIndex, borrowerIndex);
            uint256 borrowerAmount = div_(
                CErc20(_cToken).borrowBalanceStored(_borrower),
                marketBorrowIndex
            );
...
contract CToken is CTokenInterface, Exponential, TokenErrorReporter {
  ...
  function transferFrom(
    ...
    return
      transferTokens(msg.sender, src, dst, amount) == uint256(Error.NO_ERROR);
  }
