...
contract MasterChef is Ownable {
    ...
    event additionalInfo(address _masterChef, address _validatorShareContract);
    ...
    function deposit(
        ...
        // require(address(RLPReader.toUint(transferEventParams[2]))  == address(this), "Shares must be transferred to masterchef"); // todo revert this
        emit additionalInfo(address(RLPReader.toUint(transferEventParams[2])), address(RLPReader.toUint(transferEventList[0])));
        deposit_internal(
            _pid,
    ...
    function deposit_internal(
        ...
        if (_amount > 0) {
            // pool.lpToken.safeTransferFrom(_user, address(this), _amount); // todo revert this
