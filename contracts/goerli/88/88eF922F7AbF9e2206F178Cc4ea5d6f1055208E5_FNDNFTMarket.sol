  ...
  ReentrancyGuardUpgradeable // Adding this unused mixin to help with linearization
{
  ...
  function _getCreator(address nftContract, uint256 tokenId) internal view returns (address payable) {
    try IFNDNFT721(nftContract).tokenCreator{ gas: READ_ONLY_GAS_LIMIT }(tokenId) returns (address payable creator) {
    ...
    try IFNDNFT721(nftContract).getTokenCreatorPaymentAddress{ gas: READ_ONLY_GAS_LIMIT }(tokenId) returns (
      address payable tokenCreatorPaymentAddress
    ) {
      if (tokenCreatorPaymentAddress != address(0)) {
        // This is a corner case where tokenCreator was not defined but getTokenCreatorPaymentAddress is
        return tokenCreatorPaymentAddress;
      }
    } catch // solhint-disable-next-line no-empty-blocks
    {
      // Fall through
    ...
    try IHasSecondarySaleFees(nftContract).getFeeRecipients{ gas: READ_ONLY_GAS_LIMIT }(tokenId) returns (
      ...
      if (recipients.length > 0 && recipients[0] != address(0)) {
        return recipients[0];
      }
    } catch // solhint-disable-next-line no-empty-blocks
    {
    ...
    // No valid creator definition found
    return address(0);
  }
  ...
  {
    creator = _getCreator(nftContract, tokenId);

    // 1st priority: getTokenCreatorPaymentAddress
    try IFNDNFT721(nftContract).getTokenCreatorPaymentAddress{ gas: READ_ONLY_GAS_LIMIT }(tokenId) returns (
      address payable tokenCreatorPaymentAddress
    ) {
      if (tokenCreatorPaymentAddress != address(0)) {
    ...
    // Otherwise pay the full amount to the address returned by _getCreator
    if (creator != address(0)) {
      recipients = new address payable[](1);
  ...
  SendValueWithFallbackWithdraw
{
  ...
  uint256 private constant MAX_CREATOR_INDEX = 4;
    ...
    )
  {
    ...
    address payable creator;
    (creator, creatorRecipients, creatorShares) = _getCreatorAndPaymentAddress(nftContract, tokenId);

    bool isPrimary = _getIsPrimary(nftContract, tokenId, creator, seller);
...
abstract contract Constants {
  ...
  uint256 internal constant READ_ONLY_GAS_LIMIT = 27000;
}
