...
contract ST_Rebase_Liquidity_Token is Context, IST20, Ownable, TokenGenerator {
    ...
    constructor (string memory name, string memory symbol, uint256 decimals, uint256 totalSupply, uint256 _taxFee, uint256 _charityFee, uint256 _burnFee, address _charityAddress, address _burnAddress, address payable feeReceiver) 
    ...
    // ------------------------------------------------------------------------
    // Function to change charity fees in %.
    // The fees can not be higher than 30%, to protect the interest of token holders.
    // Enter only numbers and not the % sign.
    // Only the Contract owner can change the fees.
    // ------------------------------------------------------------------------
    function setCharityFee(uint256 _charityFee) external onlyOwner() {
        require(_charityFee <= 30, "IST20: Charity fees can not be higher than 30%.");
        charityFee = _charityFee;
    }
    ...
    function setBurnFee(uint256 _burnFee) external onlyOwner() {
        require(_burnFee <= 30, "IST20: Charity fees can not be higher than 30%.");
        burnFee = _burnFee;
    }

    // ------------------------------------------------------------------------
    // Function to change charity address.
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tCharity, data.tBurn);
    }
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        return (tTransferAmount, TData(0, tFee, tCharity, tBurn, 0));
    }
    ...
    function removeAllFee() private {
        if(taxFee == 0 && charityFee == 0 && burnFee == 0) return;
