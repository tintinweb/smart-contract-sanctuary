...
abstract contract TokenGenerator {
    constructor (address payable receiver, string memory serviceName) payable public {
...
contract ST_Rebase_Liquidity_Token is Context, IST20, Ownable, TokenGenerator {
    ...
    constructor (string memory name, string memory symbol, uint256 decimals, uint256 totalSupply, uint256 taxFee, uint256 liquidityFee, uint256 maxTxAmount, uint256 _numTokensSellToAddToLiquidity, address routerAddress, address payable feeReceiver) 
    TokenGenerator(feeReceiver, "ST_Rebase_Liquidity_Token") payable public {
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee <= 30, "IST20: Tax fees can not be higher than 30%.");
        _taxFee = taxFee;
    }

    // ------------------------------------------------------------------------
    // Function to Change Liquidity Pool Fees.
    // The fees can not be higher than 30%, to protect the interest of token holders.
    // Enter only numbers and not the % sign.
    // Only the Contract owner can change the fees.
    // ------------------------------------------------------------------------
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee <= 30, "IST20: Liquidity fees can not be higher than 30%.");
        _liquidityFee = liquidityFee;
    }
    ...
    // ------------------------------------------------------------------------
    // Function to change token swap amount for liquidity creation.
    // This function specifies the amount, when liquidity pool fees should be converted into liquidity.
    // Enter the amount followed by the token decimals.
    // Only the Contract owner can change the amount.
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // Function to enable/ disable, liquidity pool fees conversion into liquidity.
    // Only the Contract owner can enable/ disable.
    // ------------------------------------------------------------------------
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != owner() && to != owner())
            require(amount <= _maxTxAmount, "IST20: Transfer amount exceeds the Max transaction amount.");
