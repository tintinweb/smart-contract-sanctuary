...
contract RewardsCollector is AccessControlUpgradeable{
    ...
    bytes32 CONTROLLER_ROLE = keccak256(abi.encode("CONTROLLER_ROLE"));
    bool pauseAutocompenstionFlag;

    function initialize() public virtual initializer {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    function addController(address vault) external onlyRole(DEFAULT_ADMIN_ROLE){
        grantRole(CONTROLLER_ROLE, vault);
    }
