...
contract Treasury is ContractGuard {
    ...
    uint256 public printingRatioMultiplier = 5; //%
    uint256 public inksPrintingRatioMultiplier = 1; //%
        ...
        if (epoch < BootstrapEpochs) {
            ...
            if (paprPrice > paprPriceCeiling) {
                ...
                if (seigniorageSaved >= inkSupply.mul(inkDepletionFloorPercent).div(10000)) {
                    ...
                    uint256 _seigniorage = paprSupply.mul(_percentage).div(1e18);
                    uint256 _computingMint = _seigniorage.add(75000000000000000000000);
                    
                    _savedForFund = _computingMint.mul(fundAllocationRate).div(10000);
                    _savedForBoardRoom = _computingMint.sub(_savedForFund); 
                }
                    ...
                    if (_percentage > _mse) {
                     ...
                     // computing seignorage with % from debt phase
                    uint256 _seigniorage = paprSupply.mul(_percentage).div(1e18);
                    // adding the extra minting from the cents step
                    uint256 _seignorageAdding10CentsFloor = _seigniorage.add(_seigniorage.mul(printingRatioMultiplier.div(100).mul(_countTimesOverCents())));
                    // reducing boardroom allocation, allocating dedicated % only
                    _savedForBoardRoom = _seignorageAdding10CentsFloor.mul(seigniorageExpansionFloorPercent).div(10000);
                    // seignorage allocated to Treasury 
                    _savedForInk = _seignorageAdding10CentsFloor.sub(_savedForBoardRoom);
