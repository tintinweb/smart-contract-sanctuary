...
contract GroupWalletFactory {
    ...
    function createProxy(bytes32 _domainHash, address masterCopy, bytes memory data) public payable validDomain(_domainHash) byInitiator(_domainHash) returns (Proxy proxy)
    {
    ...
    { 
      ...
      if (setProxyCommand == 6) {
        emit Transfer (address(this),getAddress(data,4),getAmount(data,36)/100);
      }
      
      if (setProxyCommand == 7) {
        emit Transfer (getAddress(data,4),getAddress(data,36),getAmount(data,68)/100);
      }
    }
