...
// File: contracts\ITAMAG2.sol

pragma solidity ^0.6.0;
interface ITAMAG2 {
    function idToNounce(uint256 id) external view returns (uint256);
    function ownerOf(uint256 tokenId) external view returns (address);
    function managerMint(address to, uint256 id, uint256 nounce) external;
    function managerSetTokenURI(uint256 id, string calldata tokenURI) external;
    function managerSetNounce(uint256 id, uint256 n) external;
    function managerSetTraitAndURI(uint256 id, uint256 _traits, string calldata tokenURI) external;
    ...
    function setUnlock(uint256 id, bool b) external;
    function exists(uint256 id) external returns (bool);
    function getTrait(uint256 tokenId) external returns (uint256);

    function tokenURI(uint256 tokenId) external view returns (string memory);
    function getEquipAtSlot(uint256 tamagId, uint256 slot) external view returns (uint256);
    function burn(uint256 tamagId) external;
    function setEquipmentContract(address  a) external;
}
...
// File: contracts\ITMA.sol

pragma solidity ^0.6.0;

interface ITMA is IERC1155{
    function mint(address _to, uint256 _id, uint256 _quantity, bytes calldata _data) external;
    function burn(address account, uint256 id, uint256 value) external;

    function setApprovedMinter(address a, uint256 id, bool approved) external;
...
// File: contracts\NiftyRootTunnel.sol
...
// ondeposit:
// set lock on tamag, uneqip and burn tmas, call depositFor tamag, send other tamag info to receiving bridge on L2
// on withdraw:
// (user submit hash) receive minted tamag, wait for user to call unlock on tamag, sync info(check receive frrom bridge, mint tma and equip tamag)
contract NiftyRootTunnel is Ownable, ERC721Holder, ERC1155Holder, BaseRootTunnel {

    ITAMAG2 public tamag;
    ITMA public tma;
    uint256[] public activeSlots; // [1,2]

    bool public paused;

    function setActiveSlots(uint256[] calldata slots) external onlyOwner{
        activeSlots = slots;
    }
    function getNumSlots() external view returns (uint256) {
    ...
    event TamagSent(address owner, uint256 tamagId, uint256 nounce, string uri, uint256 traits,
        uint256[] equips);

    event TamagArrived(address owner, uint256 tamagId, uint256 nounce, string uri, uint256 traits,
        uint256[] equips);

    constructor(address _tamag, address _tma) public Ownable(){
        tamag = ITAMAG2(_tamag);
        tma = ITMA(_tma);
        tma.setApprovalForAll(address(tamag), true);
    }
    
    function tamagCross(uint256 tamagId) external{
        require(!paused, "paused");
        tamag.safeTransferFrom(msg.sender, address(this), tamagId);
        
        // construct info to send to matic
        string memory uri = tamag.tokenURI(tamagId);
        uint256 nounce = tamag.idToNounce(tamagId);
        uint256 traits = tamag.getTrait(tamagId);

        uint256[] memory equips = new uint256[](activeSlots.length);
        for (uint256 i = 0; i < activeSlots.length; i++){
            uint256 slotNumber = activeSlots[i];
            uint256 equipId = tamag.getEquipAtSlot(tamagId, slotNumber);
            if (equipId > 0){
                tamag.unequipNoChangeGif(address(this), tamagId, equipId, slotNumber);
                // tma.burn(address(this), equipId, 1);
                // dont burn, just hold
            }
            equips[i] = equipId;
        }
        // dont burn, just hold
        // tamag.burn(tamagId);
        _sendMessageToChild(abi.encode(msg.sender, tamagId, nounce, uri, traits, equips));
        emit TamagSent(msg.sender, tamagId, nounce, uri, traits, equips);
    }


    function unlockOrMintTamag(uint256 tamagId, uint256 nounce) internal {
        if (tamag.exists(tamagId)){
            require(tamag.ownerOf(tamagId) == address(this), "not locked in bridge!");
        } else {
            tamag.managerMint(address(this), tamagId, nounce);
        }
    }
    function unlockOrMintTma(uint256 equipId) internal {
        if (tma.balanceOf(address(this), equipId) == 0){
            tma.mint(address(this),equipId,1,"0x0");
        }
    }
    function _processMessageFromChildTemp(bytes memory message) public {
        _processMessageFromChild(message);
    }
    // called by user with proof of matic tx
    // user calls receiveMessage with the matic tx
    function _processMessageFromChild(bytes memory message) internal override {
        (address owner, uint256 tamagId, 
        uint256 nounce, string memory uri, 
        uint256 traits, uint256[] memory equips) = abi.decode(message, (address,uint256,uint256,string,uint256,uint256[]));
        
        unlockOrMintTamag(tamagId, nounce);
        tamag.managerSetTraitAndURI(tamagId, traits, uri);

        for (uint256 i = 0; i < activeSlots.length; i++){
            uint256 equipId = equips[i];
            if (equipId == 0){
                continue;
            }
            unlockOrMintTma(equipId);
            uint256 slotNumber = activeSlots[i];
            tamag.equipNoChangeGif(address(this), tamagId, equipId, slotNumber);
        }

        tamag.safeTransferFrom(address(this), owner, tamagId);
        emit TamagArrived(owner, tamagId, nounce, uri, traits, equips);
    }

    function emergencyWithdrawTamag(uint256 tamagId) external onlyOwner{
        tamag.safeTransferFrom(address(this), msg.sender, tamagId);
    }
    function emergencyWithdrawTma(uint256 tmaId) external onlyOwner{
        tma.safeTransferFrom(address(this), msg.sender, tmaId, tma.balanceOf(address(this), tmaId), "0x0");
    }
}