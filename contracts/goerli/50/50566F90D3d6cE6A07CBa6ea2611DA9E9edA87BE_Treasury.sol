...
contract Treasury is ContractGuard {
    ...
    uint256 public paprMultiplierStep;
    ...
    uint256 public printingRatioMultiplier = 500; //%
    uint256 public inksPrintingRatioMultiplier = 100; //%

    // paperprinter fund
    uint256 public fundAllocationRate = 500; // %
...
//==========================================//
    ...
    function setPaprPriceCeiling(uint256 _paprPriceCeiling) external onlyOperator {
    ...
    // Initialize PaperPrinter fund (onlyOperator) :

    function setFundAllocationRate(uint256 _rate) external onlyOperator {
        require(_rate <= 20); // _rate <= 20%
    ...
    function _sendToFund(uint256 _amount) internal {
     ...
     // rounding numbers to lower floor
    // a = number to round up
    // round up m 100000000000000000
...
// rounding numbers to lower floor
   ...
   function ceil(uint a) public returns (uint r) {
        return uint(a/10e16);
    }
    ...
    function _countTimesOverCents() public returns (uint256) {
        ...
        if (epoch < BootstrapEpochs) {
            ...
            if (paprPrice > paprPriceCeiling) {
                ...
                if (seigniorageSaved >= inkSupply.mul(inkDepletionFloorPercent).div(10000)) {
                    ...
                    uint256 ratio0 = printingRatioMultiplier.mul(1e14);
                    uint256 ratio1 = ratio0.mul(_countTimesOverCents());
                    uint256 percentCombined = _percentage.add(ratio1);
                    uint256 _seigniorage = paprSupply.mul(percentCombined).div(1e18);

                    _savedForFund = _percentage;
                    _savedForBoardRoom = percentCombined;
                    ...
                    uint256 ratio0 = inksPrintingRatioMultiplier.mul(1e14);
                    uint256 ratio1 = ratio0.mul(_countTimesOverCents());
                    uint256 percentCombined = _percentage.add(ratio1);
                    uint256 _seigniorage = paprSupply.mul(percentCombined).div(1e18);
