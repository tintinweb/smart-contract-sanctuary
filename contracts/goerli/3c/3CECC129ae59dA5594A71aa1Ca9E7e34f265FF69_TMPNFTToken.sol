...
contract TMPNFTToken is ERC721Tradable, EIP712 {
  ...
  function returnAddress(uint16 amount, uint pricePerNFT, uint nonce, bytes calldata signature) external returns (address) {
    require(whitelistNonce[nonce] == false, "TMPNFTToken: Already minted!");
  ...
  function returnAddresse() external returns (address) {
