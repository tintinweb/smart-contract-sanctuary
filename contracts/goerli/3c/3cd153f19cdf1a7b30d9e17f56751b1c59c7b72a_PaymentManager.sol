...
contract PaymentManager is Ownable {
    ...
    function withdrawFees(address _receiveWallet, uint256 _amount) external onlyOwner() {
    ...
    function sendRewardToWinner(address _coinAddress, address _receiveWallet, uint256 _winnerReward, uint256 _gameFee) external onlyOwner() {
        ...
        if (_gameFee > 0) {
            gameFeeCollected[_coinAddress] = gameFeeCollected[_coinAddress].add(_gameFee);
        }
    }
