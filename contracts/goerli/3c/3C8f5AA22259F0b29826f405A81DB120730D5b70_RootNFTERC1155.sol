    ...
    IMintableERC1155
{
    ...
    constructor(string memory _uri)
      ...
      ERC1155(_uri)
    {
      // This ensures that the base contract cannot be
      // initialized
      isBase = true;
    }
    function initialize (
      ...
      external
    {
        _setupContractId("RootNFTERC1155");
    ...
    function mint(
        ...
        // Restrict minting of tokens to only the minter role, and only for 10K tokens, 1.5K Supply
        require( ((id > uint(0)) && (id <= uint(MAX_TOKEN_ID))), "RootNFTERC1155: INVALID_TOKEN_ID");
        require(((amount > uint(0)) && (amount == restTotalSupply(id, amount))) , "RootNFTERC1155: INVALID_TOKEN_AMOUNT");
    ...
    function mintBatch(
        ...
        for (uint i = 0; i < ids.length; i++) {
            require(((ids[i] > uint(0)) && (ids[i] <= uint(MAX_TOKEN_ID))), "RootNFTERC1155: INVALID_TOKEN_ID");
            require(((amounts[i] > uint(0)) && (amounts[i] == restTotalSupply(ids[i], amounts[i]))) , "RootNFTERC1155: INVALID_TOKEN_AMOUNT");
        }
    ...
    */
    ...
    function restTotalSupply(uint256 _id, uint256 _amount) public view returns (uint256) {
      ...
      require(( subtotal <= maxSupply(_id)), "RootNFTERC1155: EXCEED MAX_AMOUNT");
      return _amount;
    }
