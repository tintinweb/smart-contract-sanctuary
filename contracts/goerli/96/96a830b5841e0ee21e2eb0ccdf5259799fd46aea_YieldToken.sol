...
contract YieldToken is ERC20("Bobo", "BOBO") {
	...
	// Thursday 31 December 2099 10:00:00
	uint256 constant public END = 4102394400000;
	...
	function updateRewardOnMint(address _user, uint256 _amount) external {
		require(msg.sender == address(ApesContract), "Can't call this");
		uint256 time = min(block.timestamp, END);
		uint256 timerUser = lastUpdate[_user];
		if (timerUser > 0)
			rewards[_user] = rewards[_user].add(ApesContract.balanceOG(_user).mul(BASE_RATE.mul((time.sub(timerUser)))).div(60) // day 86400
	...
	function updateReward(address _from, address _to, uint256 _tokenId) external {
		...
		if (_tokenId < 1001) {
			...
			if (timerFrom > 0)
				rewards[_from] += ApesContract.balanceOG(_from).mul(BASE_RATE.mul((time.sub(timerFrom)))).div(86400);
			if (timerFrom != END)
				lastUpdate[_from] = time;
			if (_to != address(0)) {
				uint256 timerTo = lastUpdate[_to];
				if (timerTo > 0)
					rewards[_to] += ApesContract.balanceOG(_to).mul(BASE_RATE.mul((time.sub(timerTo)))).div(86400);
				if (timerTo != END)
					lastUpdate[_to] = time;
			}
		}
	...
	function getTotalClaimable(address _user) external view returns(uint256) {
		uint256 time = min(block.timestamp, END);
		uint256 pending = ApesContract.balanceOG(_user).mul(BASE_RATE.mul((time.sub(lastUpdate[_user])))).div(60); //daily 
