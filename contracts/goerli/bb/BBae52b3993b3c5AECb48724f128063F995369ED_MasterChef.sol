...
contract MasterChef is Ownable {
    ...
    // Invest time lock
    uint256 public investTimeLock = 120;

    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event Harvest(address indexed user, uint256 indexed pid, uint256 amount);
    event Invest(address indexed user, uint256 indexed pid, uint256 amount, uint256 endInvestAt);
    ...
    // User can claim token after expired invest time lock
    function setTimeLock(uint256 _lockedIn) public onlyOwner {
        investTimeLock = _lockedIn;
    }
    ...
    function pendingBuni(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return user.amount.mul(accBuniPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(pending > 0) {
            mintInvestToken(_pid, pending);
        }

        if(_amount > 0) {
            user.amount = user.amount.sub(_amount);
            uint256 withdrawFee = getWithdrawFee(_amount);
    ...
    function mintInvestToken(uint256 _pid, uint256 _amount) internal {
        IERC721(vBuni).mint(msg.sender, _pid, _amount, block.timestamp.add(investTimeLock));
    }
}
