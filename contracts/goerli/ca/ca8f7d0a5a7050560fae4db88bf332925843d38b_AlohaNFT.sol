...
contract AlohaNFT is ERC721, AccessControl {
    ...
    */
    ...
    /**
    * @dev Returns the root chain ID for token.
    */
    function tokenRootChainId(uint256 tokenId) public view returns (uint256) {
        require(_exists(tokenId), "AlohaNFT: Query for nonexistent token");
        return _tokenRootChainId[tokenId];
    }
    ...
    */
    ...
    function _tokenRarity(uint256 tokenId) internal virtual returns (uint256) {
        if (tokenId.mod(3) == 0) {
            return _rarityRareModule;
        } else if (tokenId.mod(3) == 1) {
            return _rarityUltraModule;
