...
contract test5 is ERC20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor(address routerAddress) ERC20("TEST 5.0", "TST5.0") {
        //set amm info
    ...
    function topUpClaimCycleAfterTransfer(address recipient, uint256 amount) private {
        uint256 currentRecipientBalance = balanceOf(recipient).add(amount);
        ...
        if (nextAvailableClaimDate[recipient] + rewardCycleBlock > block.timestamp + rewardCycleBlock) {
            nextAvailableClaimDate[recipient] = nextAvailableClaimDate[recipient] + rewardCycleBlock;
        }
    }
}