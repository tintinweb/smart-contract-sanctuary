...
contract BNBSpark{
	...
	uint256[] public INVEST_MIN_AMOUNT = [0.01 ether, 1 ether, 10 ether, 50 ether]; 
	uint256[] public INVEST_MAX_AMOUNT = [0.99 ether, 9.99 ether, 49.99 ether, 100 ether]; 
	...
	constructor(address payable ceoAddr, address payable devAddr, uint256 start) public {
        ...
        plans.push(Plan(1,  1060)); // 106%
        plans.push(Plan(1,  1080)); // 108%
	}
	...
	function reinvest(uint8 plan) public {
		...
		require(block.timestamp > startDate, "contract does not launch yet");		
		require(totalAmount >= INVEST_MIN_AMOUNT[plan],"invalid min amount");
		require(totalAmount <= INVEST_MAX_AMOUNT[plan],"invalid max amount");
