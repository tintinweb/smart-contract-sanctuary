...
/*
 * Direct Sale V1 Factory for cryptoWine project
 *
...
// File: contracts/DirectSaleV1Factory.sol

/*
 * Factory for cryptoWine direct sales V1.
 */

contract DirectSaleV1Factory is DirectSaleV1FactoryI {

    AgriDataI public agriData;

    address public salePrototypeAddress;

    address[] public deployedSales;

    constructor(address _agriDataAddress, address _salePrototypeAddress)
    {
        agriData = AgriDataI(_agriDataAddress);
        require(address(agriData) != address(0x0), "You need to provide an actual agri data contract.");
        salePrototypeAddress = _salePrototypeAddress;
        require(salePrototypeAddress != address(0x0), "You need to provide an actual prototype address.");
    }

    modifier onlyCreateControl() {
        require(msg.sender == agriData.getAddress("directsaleCreateControl"), "DirectSale createControl key required for this function.");
        _;
    }
    ...
    // Create a new direct sale, which can own currency and tokens.
    function create()
    public
    onlyCreateControl
    returns (address)
    {
        address newSaleAddress = Clones.clone(salePrototypeAddress);
        emit NewSale(newSaleAddress);
        DirectSaleV1DeployI(newSaleAddress).initialRegister();
        shippingManager().setShopAuthorized(agriData.getAddress("CryptoWineToken"), newSaleAddress, true);
        deployedSales.push(newSaleAddress);
        return newSaleAddress;
    }

    function deployedSalesCount()
    public view
    returns (uint256)
    {
