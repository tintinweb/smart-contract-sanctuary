...
contract CrocodileGameNFT is ICrocodileGameNFT, ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  mapping(uint256 => bytes) public preTraitsCroco;
  mapping(uint256 => bytes) public preTraitsBird;
  ...
  function EnrollPseudoIDCrocoTraits(bytes[] memory traits) public onlyOwner {
    for (uint16 i = 0; i < traits.length; i++) {
        preTraitsCroco[i] = traits[i];
    }
  }

  function EnrollPseudoIDBirdTraits(bytes[] memory traits) public onlyOwner {
    for (uint16 i = 0; i < traits.length; i++) {
