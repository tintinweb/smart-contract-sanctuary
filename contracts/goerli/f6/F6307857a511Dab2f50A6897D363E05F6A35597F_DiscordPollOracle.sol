...
contract DiscordPollOracle is UsingWitnet {
  ...
  function checkResult(string calldata _id) external payable ended(_id) {
    ...
    WitnetRequest request = new WitnetRequest(
      bytes(
        abi.encodePacked(
          hex"0a5408c0b6eb8b061236122568747470733a2f2f766f74696e672e6775696c642e78797a2f6170692f7265616374732f311a0d82187782186766726573756c741a090a050808120180100222090a050808120180100210c0843d180a20e80728333080c8afa025"
        )
      )
    );
  ...
  function writeResult(string calldata _id) external ended(_id) {
    ...
    if (witnet.isOk(result)) {
      ...
      results[_id] = witnet.asString(result);
    } else {
