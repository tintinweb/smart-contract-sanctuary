...
contract BottleLauncher is FxBaseRootTunnel, Ownable {

    using BytesLib for bytes;

    uint256 public publicPrice;                 // price in utility token 1e18 units

    constructor(address _checkpointManager, address _fxRoot, uint256 _initialPrice) 
    FxBaseRootTunnel(_checkpointManager, _fxRoot)
    {
        publicPrice = _initialPrice;
    }

    function sendMessage(address to, string calldata uri) payable external {
        require(publicPrice <= msg.value, "INSUFFICIENT_ETH");

        _sendMessageToChild(
            abi.encodePacked(to, uri)
        );
    ...
    // * UTILITIES * //
    // * Required for FxBaseRootTunnel * //
    function _processMessageFromChild(bytes memory data) internal override {}
}


/* from https://github.com/GNSPS/solidity-bytes-utils */
library BytesLib {
