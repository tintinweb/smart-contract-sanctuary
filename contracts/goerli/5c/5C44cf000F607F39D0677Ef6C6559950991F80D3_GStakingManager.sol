...
contract GStakingManager is AccessControl {
    ...
    function updateCondition(uint256 condition_sliver, uint256 condition_gold)
        public
    {
    ...
    function deposit(uint256 _amount) public {
        require(
            gpoolToken.balanceOf(msg.sender) >= _amount,
            "not enough balance"
        );
    ...
    function unStaking(uint256 _amount) public {
        require(
            _amount < stakers[msg.sender].stakingAmount,
            "not enough balance"
        );
    ...
    function getBalance(address _staker) internal returns (uint256) {
        ...
        uint256 balanceInUSDC = router.exactInputSingle{
            value: staker.stakingAmount
        }(params);
    ...
    function createPool(
        ...
        require(
            _rewardToken.balanceOf(msg.sender) >= _totalReward,
            "not enough balance"
        );
        increasePoolLength();
    ...
    // withdrawAssetToken
    function withdrawAsset(IERC20 _tokenAddress, uint256 _amount)
        public
        onlyRole(ADMIN)
    {
    ...
    function getTier(address _staker) public view returns (Rank) {
        ...
        uint256 stakingTime = block.timestamp - staker.startStaking;
        // condition for bronze
        if (
            staker.startStaking == staker.lastUnStaking &&
            stakingTime <= CONDITION_SLIVER
        ) {
            rank = Rank.BRONZE;
        }
        ...
        // condition for gold
        if (
            staker.stakingAmount == staker.lastUnStaking &&
            stakingTime >= CONDITION_GOLD
        ) {
            rank = Rank.GOLD;
        }
