...
import "./interfaces/IFNDNFTMarketOldEvents.sol";
  ...
  NFTMarketPrivateSale
{
  ...
  function adminUpdateConfig(
    ...
    uint256 primaryF8nFeeBasisPointsForAuction,
    uint256 secondaryF8nFeeBasisPointsForAuction,
    uint256 primaryF8nFeeBasisPointsForPrivateSale,
    uint256 secondaryF8nFeeBasisPointsForPrivateSale
    ...
    _updateMarketFees(
      ...
      primaryF8nFeeBasisPointsForAuction,
      secondaryF8nFeeBasisPointsForAuction,
      primaryF8nFeeBasisPointsForPrivateSale,
      secondaryF8nFeeBasisPointsForPrivateSale
    );
  }
...
/**
 * @dev This file includes events which are no longer used.
 * We preserve these event signatures for backwards compatibility in our subgraph.
 */
interface IFNDNFTMarketOldEvents {
  /**
   * @dev V2 of this event added private sale fees.
   */
  ...
  SendValueWithFallbackWithdraw
{
  ...
  /**
   * @dev Each sales model may have a different fee structure.
   */
  enum FeeType {
    Auction,
    PrivateSale
  ...
  uint256 private _primaryFoundationFeeBasisPointsForAuction;
  uint256 private _secondaryFoundationFeeBasisPointsForAuction;
  uint256 private _secondaryCreatorFeeBasisPoints;

  mapping(address => mapping(uint256 => bool)) private nftContractToTokenIdToFirstSaleCompleted;

  uint256 private _primaryFoundationFeeBasisPointsForPrivateSale;
  uint256 private _secondaryFoundationFeeBasisPointsForPrivateSale;

  /**
   * @dev V2 of this event added private sale fees.
   */
  event MarketFeesUpdatedV2(
    uint256 secondaryCreatorFeeBasisPoints,
    uint256 primaryFoundationFeeBasisPointsForAuction,
    uint256 secondaryFoundationFeeBasisPointsForAuction,
    uint256 primaryFoundationFeeBasisPointsForPrivateSale,
    uint256 secondaryFoundationFeeBasisPointsForPrivateSale
  );
    ...
    )
  {
    return (
      ...
      _primaryFoundationFeeBasisPointsForAuction,
      _secondaryFoundationFeeBasisPointsForAuction,
      _primaryFoundationFeeBasisPointsForPrivateSale,
      _secondaryFoundationFeeBasisPointsForPrivateSale
    );
  }
  ...
  {
    address payable seller = _getSellerFor(nftContract, tokenId);
    // Ignoring unused return values creatorSecondaryFeeTo & ownerRevTo
    (foundationFee, , creatorSecondaryFee, , ownerRev) = _getFees(nftContract, tokenId, seller, price, feeType);
  }
  ...
  {
    return getFeesForType(nftContract, tokenId, price, FeeType.Auction);
  }
    ...
    )
  {
    ...
    if (_getIsPrimary(nftContract, tokenId, creator, seller)) {
      if (feeType == FeeType.Auction) {
        foundationFeeBasisPoints = _primaryFoundationFeeBasisPointsForAuction;
      } else {
        foundationFeeBasisPoints = _primaryFoundationFeeBasisPointsForPrivateSale;
      }
      // On a primary sale, the creator is paid the remainder via `ownerRev`.
      ownerRevTo = tokenCreatorPaymentAddress;
    } else {
      if (feeType == FeeType.Auction) {
        foundationFeeBasisPoints = _secondaryFoundationFeeBasisPointsForAuction;
      } else {
        foundationFeeBasisPoints = _secondaryFoundationFeeBasisPointsForPrivateSale;
      }
    ...
    )
  {
    ...
    address payable ownerRevTo;
    (foundationFee, creatorFeeTo, creatorFee, ownerRevTo, ownerRev) = _getFees(
      nftContract,
      tokenId,
      seller,
      price,
  ...
  function _updateMarketFees(
    ...
    require(primaryFoundationFeeBasisPointsForAuction < BASIS_POINTS, "NFTMarketFees: Fees >= 100%");
    require(primaryFoundationFeeBasisPointsForPrivateSale < BASIS_POINTS, "NFTMarketFees: Fees >= 100%");
    require(
      secondaryFoundationFeeBasisPointsForAuction.add(secondaryCreatorFeeBasisPoints) < BASIS_POINTS,
      "NFTMarketFees: Fees >= 100%"
    );
    require(
      secondaryFoundationFeeBasisPointsForPrivateSale.add(secondaryCreatorFeeBasisPoints) < BASIS_POINTS,
      "NFTMarketFees: Fees >= 100%"
    );
    _secondaryCreatorFeeBasisPoints = secondaryCreatorFeeBasisPoints;
    _primaryFoundationFeeBasisPointsForAuction = primaryFoundationFeeBasisPointsForAuction;
    _secondaryFoundationFeeBasisPointsForAuction = secondaryFoundationFeeBasisPointsForAuction;
    _primaryFoundationFeeBasisPointsForPrivateSale = primaryFoundationFeeBasisPointsForPrivateSale;
    _secondaryFoundationFeeBasisPointsForPrivateSale = secondaryFoundationFeeBasisPointsForPrivateSale;

    emit MarketFeesUpdatedV2(
      secondaryCreatorFeeBasisPoints,
      primaryFoundationFeeBasisPointsForAuction,
      secondaryFoundationFeeBasisPointsForAuction,
      primaryFoundationFeeBasisPointsForPrivateSale,
      secondaryFoundationFeeBasisPointsForPrivateSale
    );
  }

  // 2 slots were used for the private sale fees
