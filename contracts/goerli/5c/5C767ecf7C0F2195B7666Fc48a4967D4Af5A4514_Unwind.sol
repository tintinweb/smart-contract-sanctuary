 ...
 * Users can also redeem here their gDai for a Weth payout, using `redeem`.
 */
contract Unwind is Ownable(), DecimalMath {
    ...
    /// @dev Redeems GDai for weth for any user. GDai.redeem won't work if MakerDAO is in shutdown.
    /// @param maturity Maturity of an added series
    /// @param user Wallet containing the gDai to burn.
    function redeem(uint256 maturity, address user) public {
        IGDai gDai = controller.series(maturity);
        require(gDai.unlocked() == 1, "gDai is still locked");
        uint256 gDaiAmount = gDai.balanceOf(user);
        require(gDaiAmount > 0, "Unwind: Nothing to redeem");

        gDai.burn(user, gDaiAmount);
        require(
            weth.transfer(
                user,
                daiToFixWeth(muld(gDaiAmount, gDai.chiGrowth()), _fix)
            )
        );
    }
}
