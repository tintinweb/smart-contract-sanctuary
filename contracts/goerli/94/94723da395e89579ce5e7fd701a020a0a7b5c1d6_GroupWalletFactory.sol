...
contract GroupWalletFactory {
    ...
    modifier isProxy(bytes32 domainHash, address proxyT) {
          require(proxyTokenAddr[domainHash]==proxyT,"bad TokenProxy!");
        _;
    }
    ...
    function b_String(bytes32 _bytes32, uint len) private pure returns (string memory) {
    ...
    function getAddress(bytes memory d, uint off) internal pure returns(address addr) {
      return address( uint256(bytesMemoryTobytes32(substring(d,off,32)) & 0x000000000000000000000000ffffffffffffffffffffffffffffffffffffffff ));
    }
    ...
    function createProxy(bytes32 _domainHash, address masterCopy, bytes memory data) public payable validDomain(_domainHash) byInitiator(_domainHash) returns (ProxyToken proxy)
    {
    ...
    function setTokenProxy(bytes32 _domainHash, address tokenProxy, uint256 setProxyCommand, bytes memory data) public payable isProxy(_domainHash,tokenProxy) byInitiatorOrMember(_domainHash) returns (uint256)
    { 
      //revert(string(abi.encodePacked( b_String(bytes32( uint256( gasleft() ) ),32)))); // debugging
      //require(1==2,b_String(bytesMemoryTobytes32(substring(data,36,32)),32)); // debugging
      ...
      if (setProxyCommand == 6) {
        emit Transfer (address(this),getAddress(data,4),getAmount(data,36)/100); // transfer
      }
      
      if (setProxyCommand == 7) {
        emit Transfer (getAddress(data,4),getAddress(data,36),getAmount(data,68)/100); // transferFrom
      }
    }
