...
contract TokenList {
    ...
    address private constant AlphaImpactAddr = 0xC2A02b716e8c80C6968E53C3EeFe9Df8f786Cb45;
...
contract AlphaImpactFarm is Ownable, ReentrancyGuard, TokenList {
    ...
    function computeWithdrawalAmount(address _account, uint256 depositId, uint256 condition) public view returns(uint256 bef, uint256 aft) {
        ...
        if (condition == 0) {
            ...
            amountToUser = (uint256(_deposit.amountDue).mul(1e9).div(uint256(tS.rate))).mul(3).div(10);
        ...
        bef = (uint256(_deposit.amountDue).mul(1e9).div(uint256(tS.rate)));
        aft = amountToUser;
    ...
    function _withdraw(uint256 depositId, uint256 condition) internal {
        ...
        termIdToLiquidity[termsId] = termIdToLiquidity[termsId].sub((uint256(_deposit.amountDue).mul(1e9).div(uint256(tS.rate))));

        userDeposits.data[depositId] = _deposit;
        termIDtoIndex[termsId][msg.sender] = termIDtoIndex[termsId][msg.sender].add(1);
        totalDue[outTokenId] = totalDue[outTokenId].sub(_deposit.amountDue);
        _deposit.lastWithdrawTime = safe32(block.timestamp);
        _deposit.amountDue = uint176(amountDue);
        _hasDeposit[termsId][msg.sender] = false;
        require(IERC20(_tokenAddr(outTokenId)).balanceOf(treasury) >= amountToUser, "AIFarms: There was an error trying to withdraw your funds. We are sorry for the inconvenience. Please contact Alpha Impact community managers on Telegram or email [email protected] for help.");
