...
contract Gryldcheez is Context, IERC20, Ownable {
    using SafeMath for uint256;

    address payable public marketingAddress =
        payable(0x6558a63789BAE9E2f354077AD0337e7315596048); // Marketing Address

    address public buyBackAddress = payable(0x6558a63789BAE9E2f354077AD0337e7315596048);
    address public devAddress = payable(0x6558a63789BAE9E2f354077AD0337e7315596048);
    ...
    string private _name = "GRYLDCHEEZ";
    string private _symbol = "GRYLD";
    ...
    uint256 public _previousBuyBackFee = _buyBackFee;

    uint256 public _liquidationThreshold = 5000 * 10**9;
    uint256 private launchTime;

    uint256 public _maxTransaction = 50 * 10**6 * 10**9;
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner())
            require(amount <= _maxTransaction, "Koala Inu: Transfer amount exceeds the max transaction amount allowed");
        ...
        // don't take fee from excluded address
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        uint256 sharePerDivision = contractTokenBalance.div(
            _marketingFee.add(_liquidityFee).add(_devFee).add(_buyBackFee)
        );
        uint256 marketingShare = sharePerDivision.mul(_marketingFee);
        uint256 devShare = sharePerDivision.mul(_devFee);
        uint256 buyBackShare = sharePerDivision.mul(_buyBackFee);
        uint256 liquidityShare = sharePerDivision.mul(_liquidityFee);
        ...
        contractETHBalance = address(this).balance;
        if (contractETHBalance > 0) {
            sendETHToBuyBack(address(this).balance);
        }
    ...
    function sendETHToBuyBack(uint256 amount) private {
        payable(buyBackAddress).transfer(amount);
    }
    ...
    function calcualteMarketingFee(uint256 _amount)
        ...
        returns (uint256)
    {
        uint256 totalFee = _marketingFee.add(_devFee).add(_buyBackFee);
    ...
    function _removeSniper(address account) external onlyOwner {
        require(
            ...
            "We can not blacklist Pancakeswap"
        );
