...
// File: contracts/CryptoWineToken.sol

/*
 * Implements ERC 721 NFT standard: https://github.com/ethereum/EIPs/issues/721.
 */

contract CryptoWineToken is ERC721EnumerableSimple, CryptoWineTokenI {
    using Address for address payable;

    AgriDataI public agriData;

    string public baseURI;
    uint256 public storageFeeYearlyEurCent;
    uint256 public constant secondsPerYear = 864 * 36525;  // 86400 s/day * 365.25 day/yr

    mapping(uint256 => uint256) public wineID;
    mapping(uint256 => uint256) public depositEurCent;
    mapping(uint256 => uint256) public storageStart;

    mapping(address => uint256) public signedTransferNonce;

    event StorageFeeChanged(uint256 previousStorageFeeYearlyEurCent, uint256 newStorageFeeYearlyEurCent);
    event AgriDataChanged(address indexed previousAgriData, address indexed newAgriData);
    event BaseURI(string value);
    event PermanentURI(string _value, uint256 indexed _id);
    event StorageFundsDeposited(uint256 indexed tokenId, address indexed payTokenAddress, uint256 amountPayToken, uint256 amountEurCent);

    constructor(address _agriDataAddress, string memory _newBaseURI, uint256 _storageFeeYearlyEurCent)
    ERC721("cryptoWine", "WINE")
    {
        ...
        storageFeeYearlyEurCent = _storageFeeYearlyEurCent;
    }
    ...
    modifier onlyCreateControl() {
        require(msg.sender == agriData.getAddress("tokenCreateControl"), "Token createControl key required for this function.");
        _;
    }
    ...
    modifier onlyAuthorizedShop() {
        require(_shippingManager().authorizedShop(address(this), msg.sender), "Only an authorized shop can call this function.");
        _;
    }
    ...
    function setStorageFee(uint256 _newStorageFeeYearlyEurCent)
    public
    onlyCreateControl
    {
        emit StorageFeeChanged(storageFeeYearlyEurCent, _newStorageFeeYearlyEurCent);
        storageFeeYearlyEurCent = _newStorageFeeYearlyEurCent;
    }

    /*** Get contracts with their ABI ***/

    function _shippingManager()
    internal view
    ...
    // Issue a new asset, giving it to a specific owner address.
    function create(address _owner, uint256 _wineID)
    public
    onlyCreateControl
    {
        ...
        wineID[tokenId] = _wineID;
    }

    // Batch-issue assets with adjacent IDs.
    function createMulti(uint _amount, address _owner, uint256 _wineID)
    public
    onlyCreateControl
    {
        ...
        for (uint256 i = 0; i < _amount; i++) {
            ...
            _safeMint(_owner, tokenIdStart + i);
            wineID[tokenIdStart + i] = _wineID;
        }
    }
    ...
    /*** Shipping, storage and deposit management ***/

    function getShippingStatus(uint256 _tokenId)
    public view
    returns (ShippingManagerI.ShippingStatus)
    {
        require(_exists(_tokenId), "Token ID needs to exist.");
        return _shippingManager().deliveryStatus(address(this), _tokenId);
    }

    function startStorage(uint256 _tokenId, uint256 _depositEurCent)
    public
    onlyAuthorizedShop
    {
        require(_exists(_tokenId), "Token ID needs to exist.");
        depositEurCent[_tokenId] = _depositEurCent;
        storageStart[_tokenId] = block.timestamp;
        emit StorageFundsDeposited(_tokenId, address(0), 0, _depositEurCent);
    }

    function storageValidUntil(uint256 _tokenId)
    public view
    returns (uint256)
    {
        require(_exists(_tokenId), "Token ID needs to exist.");
        return storageStart[_tokenId] + depositEurCent[_tokenId] * secondsPerYear / storageFeeYearlyEurCent;
    }

    // Calculate current asset price in "wei" (subunits of the native chain currency or of a payment token).
    // use the zero address for _paymentTokenAddress to get the native currency rate.
    // Note: Price in EUR cent is available from public var getter storageFeeYearlyEurCent().
    function storageFeeYearlyWei(address _payTokenAddress)
    public view
    returns (uint256)
    {
        require(_oracle().tokenSupported(_payTokenAddress), "Payment token is not supported.");
        return storageFeeYearlyEurCent * _oracle().eurRate(_payTokenAddress) / 100;
    }

    // Prolong storage by depositing more funds.
    function depositStorageFunds(uint256[] memory _tokenIds, uint256[] memory _amounts)
    public payable
    {
        uint256 tokencount = _tokenIds.length;
        require(_amounts.length == tokencount, "Both lists need to have the same length.");
        uint256 totalAmount = 0;
        for (uint256 i = 0; i < tokencount; i++) {
            require(_exists(_tokenIds[i]), "Every token ID needs to exist.");
            uint256 amountEurCent = _amounts[i] * 100 / _oracle().eurRate(address(0));
            depositEurCent[_tokenIds[i]] += amountEurCent;
            emit StorageFundsDeposited(_tokenIds[i], address(0), _amounts[i], amountEurCent);
            totalAmount += _amounts[i];
        }
        require(totalAmount == msg.value, "Amounts need to match the sent amount of currency.");
        // Transfer the actual payment amount to the beneficiary.
        // Our own account so no reentrancy here but put at end to be sure.
        payable(agriData.getAddress("beneficiary")).sendValue(msg.value);
    }

    // Prolong storage by depositing more funds via an ERC20 token.
    function depositStorageFundTokens(address _payTokenAddress, uint256[] memory _tokenIds, uint256[] memory _payTokenAmounts)
    public
    {
        require(_oracle().tokenSupported(_payTokenAddress), "Payment token is not supported.");
        uint256 tokencount = _tokenIds.length;
        require(_payTokenAmounts.length == tokencount, "Both lists need to have the same length.");
        uint256 totalAmount = 0;
        for (uint256 i = 0; i < tokencount; i++) {
            require(_exists(_tokenIds[i]), "Every token ID needs to exist.");
            uint256 amountEurCent = _payTokenAmounts[i] * 100 / _oracle().eurRate(_payTokenAddress);
            depositEurCent[_tokenIds[i]] += amountEurCent;
            emit StorageFundsDeposited(_tokenIds[i], _payTokenAddress, _payTokenAmounts[i], amountEurCent);
            totalAmount += _payTokenAmounts[i];
        }
        IERC20 payToken = IERC20(_payTokenAddress);
        uint256 testAllowance = payToken.allowance(msg.sender, address(this));
        require(testAllowance >= totalAmount, "Need enough allowance to buy this key.");
        payToken.transferFrom(msg.sender, agriData.getAddress("beneficiary"), totalAmount);
    }
