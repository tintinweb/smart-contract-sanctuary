...
contract TheSevy is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("TheSevy123", "SEV123") {
        maxSupply = 30;
        reserveCount = 30;
    ...
    function setUpPresale(
        ...
        // Check params
        require(whitelistSigner != address(0), ": zero address");
        require(_startTime > 0 && _endTime > _startTime, ": invalid time range");
    ...
    function setUpSale(
        ...
        require(_initMaxCount > 0 && _unlockedMaxCount > 0, ": zero amount");
        require(_startTime > 0 && _maxCountUnlockTime > _startTime, ": invalid time range");
    ...
    function mintTokens(uint256 count) external payable {
        ...
        require(_nextTokenId + count <= maxSupply, ": max supply exceeded");

        string memory message = string(abi.encodePacked("Incorrect Ether value, TokenPrice:", Strings.toString(tokenPrice), "Msg Value:", Strings.toString(msg.value)));
        require(tokenPrice * count == msg.value, message);
