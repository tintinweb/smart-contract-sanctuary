...
interface IERC20 {
    ...
    function _mint(uint amount, address account) external returns(bool);
     function _burn(uint amount, address account) external returns(bool);
}
...
contract BridgeB {
   ...
   function checkTx(bytes32 TX) external view gnosisSafeOnly(msg.sender)
    ...
    function __mint(bytes memory arg) internal  {
        ...
        IERC20(tokenB)._mint(amountUnlocked,receiver);
