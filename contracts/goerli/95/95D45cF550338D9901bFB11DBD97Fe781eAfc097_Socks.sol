...
contract Socks is ERC721, Ownable, ReentrancyGuard {
    ...
    int256 private _maxSupply = -1;
    // the base uri for the token
    string private _baseUri;

    // Deposit address is where the ether from the mint is deposited too - this is a charity address.
    address private _depositAddress;

    constructor() ERC721("Charity Socks", "SOCKS") {}

    function setMaxSupply(int256 _total) external onlyOwner {
        // disable changing the max supply afterwards - prevents owner from changing
        // the supply after mint has started
        require(
            _maxSupply == -1,
            "Max supply is already set! Cannot change supply!"
        );
        require(_total > 0, "total must be a positive integer");
        require(_depositAddress != address(0), "deposit address not set");
        _maxSupply = _total;
    }

    function setDepositAddress(address _deposit) external onlyOwner {
        require(
            _depositAddress == address(0),
            "cannot change deposit address after it is set!"
        );
        _depositAddress = _deposit;
    }
    ...
    function mint(uint256 amount) external payable nonReentrant {
        ...
        require(
            _totalSupply + amount < uint256(_maxSupply),
        ...
        payable(_depositAddress).send(msg.value);
