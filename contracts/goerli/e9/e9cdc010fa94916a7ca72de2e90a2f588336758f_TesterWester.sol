...
contract TesterWester is IERC20, Ownable {
    ...
    //EVENTS FOR TESTING swapAndLiquify()
    event SwapLiquify(uint256 fee_tokens, uint256 liq_tokens);
    event TokenSplits(uint256 fee_balance, uint256 half, uint256 total_eth, uint256 liq_share);
    ...
    function swapAndLiquify() private swapLock {
        ...
        emit SwapLiquify(fee_balance, liquidity_tokens);
        if (liquidity_fee != 0 && over_liquify_threshold) {
            ...
            liquidity_tokens = 0;
            emit TokenSplits(fee_balance, half, eth_balance, liq_share);
        }
        if (fee_tokens > 0) { 
    ...
    function transferStageStandard(address from, address to, uint256 t_initial, bool cancel_fees) private {
        ...
        if (!cancel_fees) {
            ...
            if (team_fee != 0) {
                ...
                emit FeeTaken(team_fee, tteam_fee, fee_tokens);
            }
            if (liquidity_fee != 0) {
                uint256 rliq_fee;
                uint256 tliq_fee;
