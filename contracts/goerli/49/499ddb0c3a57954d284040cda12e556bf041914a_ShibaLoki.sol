...
contract ShibaLoki is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x9740067bf30daa1C8C86406822505C3905a9d71c);
    ...
    event onMarketingDivisorUpdated(uint256 divisor);
    event onNumTokensSellToAddToLiquidityUpdated(uint256 _minimumTokensBeforeSwap);
 
    event onMarketingAddressUpdated(address _marketingAddress);
    ...
    function setMarketingDivisor(uint256 divisor) external onlyOwner() {
        ...
        emit onMarketingDivisorUpdated(divisor);
    ...
    function setMarketingAddress(address _marketingAddress) external onlyOwner() {
        ...
        emit onMarketingAddressUpdated(_marketingAddress);

    }

    function getContractEth()external view onlyOwner returns(uint256){
        return address(this).balance;
    }

    function withdrawEth(uint256 amount) external onlyOwner{
        require(address(this).balance>=amount,"insufficient funds");
        transferToAddressETH(marketingAddress,amount);
    }
    ...
    // Withdraw ETH that gets stuck in contract by accident or for other purposes
