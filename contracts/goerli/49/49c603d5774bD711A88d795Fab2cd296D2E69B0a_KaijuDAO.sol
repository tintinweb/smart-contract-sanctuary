...
contract KaijuDAO is ERC721, ThreePerWallet, Ownable{
    ...
    uint256 public constant MINT_PRICE_ETH_GEN0_PRESALE = 0.03 ether;
    uint256 public constant MINT_PRICE_ETH_GEN0 = 0.04 ether;
    uint256 public constant MINT_PRICE_ETH_GEN1 = 0.04 ether;
    uint256 public constant MINT_PRICE_SOS_GEN0_PRESALE = 8000000000000000000;
    uint256 public constant MINT_PRICE_SOS_GEN0 = 10000000000000000000;
    uint256 public constant MINT_PRICE_SOS_GEN1 = 10000000000000000000;
    ...
    function preSaleETH(bytes32 _messageHash, bytes memory signature, uint16 amount) external payable threePerWallet(msg.sender, minted+1){
        require(preGen0Sale, "preSale closed");
        require(isWhitelisted(_messageHash, signature), "Not Whitelisted");
        require(MINT_PRICE_ETH_GEN0_PRESALE * amount == msg.value, "Invalid payment amount");
        require(amount < 4, "Too many amount");
        require(minted + amount <= MAX_TOKENS_GEN0-MAX_AIRDROPS_GEN0, "minted out");
        for(uint i=0; i<amount; i++){
            minted++;
            if(minted < MAX_TOKENS_GEN0+1) accum_eth_gen0 += MINT_PRICE_ETH_GEN0_PRESALE;
            else accum_eth_gen1 += MINT_PRICE_ETH_GEN0_PRESALE;
            _setTrait(minted);
    ...
    function publicETH(uint16 amount) external payable threePerWallet(msg.sender, minted+1){
        if(minted<10000){
            ...
            require(minted + amount <= MAX_TOKENS_GEN0+MAX_TOKENS_GEN1-MAX_AIRDROPS_GEN1, "minted out");
        } 
        require(amount < 4, "Too many amount");
        ...
        for(uint i=0; i<amount; i++){
            ...
            else accum_eth_gen1 += MINT_PRICE_ETH_GEN0;
            _setTrait(minted);
            _mint(msg.sender, minted);
            _autoTransfer();
        }
    }
    function preSaleSOS(bytes32 _messageHash, bytes memory signature, uint16 amount) external threePerWallet(msg.sender, minted+1){
        require(preGen0Sale, "preSale closed");
        require(isWhitelisted(_messageHash, signature), "Not Whitelisted");
        require(amount < 4, "Too many amount");
        require(minted + amount <= MAX_TOKENS_GEN0-MAX_AIRDROPS_GEN0, "minted out");
        ...
        for(uint i=0; i<amount; i++){
            minted++;
            if(minted < MAX_TOKENS_GEN0+1) accum_sos_gen0 += MINT_PRICE_SOS_GEN0_PRESALE;
            else accum_sos_gen1 += MINT_PRICE_SOS_GEN0_PRESALE;
            _setTrait(minted);
            _mint(msg.sender, minted);
            _autoTransfer();
        }
    ...
    function publicSOS(uint16 amount) external threePerWallet(msg.sender, minted+1){
        ...
        for(uint i=0; i<amount; i++){
            ...
            else accum_sos_gen1 += MINT_PRICE_SOS_GEN0;
            _setTrait(minted);
            _mint(msg.sender, minted);
            _autoTransfer();
        }
    ...
    */
    ...
    function isWhitelisted(bytes32 _messageHash, bytes memory signature) public view returns (bool) {
        return recoverSigner(_messageHash, signature) == signer;
    }
