...
contract ERC20TransferLiquidityLock is ERC20 {
    ...
    function rebalanceLiquidity() public {
        require(balanceOf(msg.sender) >= minRebalanceAmount, "You are not part of the RushMoon.");
    ...
    function remLiquidity(uint256 lpAmount) private returns(uint ETHAmount) {
        ...
        (ETHAmount) = IPancakeswapV2Router02(PancakeswapV2Router)
...
contract Balancer {
    ...
    function swapEthForTokens(uint256 EthAmount, uint callerRewardDivisor) private {
        ...
        PancakeswapPairPath[0] = IPancakeswapV2Router02(token.PancakeswapV2Router()).WETH();
...
contract RushMoon is 
    ERC20(1e12 ether), 
    ERC20Detailed("RushMoon", "RMOON", 18), 
    ERC20Burnable,
    ...
    WhitelistAdminRole 
{
    ...
    function setPancakeswapV2Pair(address _PancakeswapV2Pair) public onlyWhitelistAdmin {
        require(PancakeswapV2Pair == address(0), "RBKToken::setPancakeswapV2Pair: already set");
    ...
    function setPancakeswapV2Router(address _PancakeswapV2Router) public onlyWhitelistAdmin {
        require(PancakeswapV2Router == address(0), "RBKToken::setPancakeswapV2Router: already set");
    ...
    function setLiquidityLockDivisor(uint256 _liquidityLockDivisor) public onlyWhitelistAdmin {
        if (_liquidityLockDivisor != 0) {
            require(_liquidityLockDivisor >= 10, "RBKToken::setLiquidityLockDivisor: too small");
        }
    ...
    function setRebalanceDivisor(uint256 _rebalanceDivisor) public onlyWhitelistAdmin {
        if (_rebalanceDivisor != 0) {
            require(_rebalanceDivisor >= 10, "RBKToken::setRebalanceDivisor: too small");
        }        
    ...
    function setCallerRewardDivisior(uint256 _rewardDivisor) public onlyWhitelistAdmin {
        if (_rewardDivisor != 0) {
            require(_rewardDivisor >= 10, "RBKToken::setCallerRewardDivisor: too small");
        }        
    ...
    function Locktokens() public onlyWhitelistAdmin {
        locked = true;
    }
    ...
    function addLiquidity() public onlyWhitelistAdmin {
        ...
        IPancakeswapV2Router01(PancakeswapV2Router).addLiquidityETH.value(amountETH)(
            address(this),
            amountTokenDesired,
            0,
            0,
    ...
    function RBKBalanceInPancakeswap(address _user) public view returns (uint256) {
        uint256 PancakeswapPairRBKBalance = balanceOf(address(PancakeswapV2Pair));
        if (PancakeswapPairRBKBalance == 0) {
            return 0;
        }
        ...
        return PancakeswapPairRBKBalance.mul(userPercentOfLpOwned).div(1e12);
    }

    function RBKBalanceAll(address _user) public view returns (uint256) {
        return balanceOf(_user).add(RBKBalanceInPancakeswap(_user));
    }
