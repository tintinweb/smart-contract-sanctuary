...
contract GStakingManager is AccessControl, ReentrancyGuard {
    ...
    mapping(uint => mapping(address => LockedReward)) public lockingAmounts;
    ...
    mapping(PoolType => uint[]) public poolTypes;
    ...
    event CreatePool(uint256 poolId, PoolType poolType);
    ...
    function updateVaultAddress(address _vault) public onlyRole(ADMIN) {
    ...
    function claimReward(uint256 pid) public validatePoolById(pid) returns (uint256) {
        ...
        if (totalPending > 0) {
            uint rewardByTier = totalPending * uint(userTier) / uint(Tier.GOLD);
            uint lockedAmount = totalPending - rewardByTier;
    ...
    function pendingReward(uint256 pid, address userAddress) public view validatePoolById(pid) returns (uint256) {
        ...
        uint totalPendingReward = (user.pendingReward + (((staker.amount * accRewardPerShare) / ACCUMULATED_MULTIPLIER) - user.rewardDebt));
    ...
    function createPool(
        ...
        uint pid = poolInfo.length - 1;
        poolTypes[poolType].push(pid);

        emit CreatePool(pid, poolType);
    }
    ...
    function updatePoolTime(
        ...
        uint256 startTime,
        uint256 endTime
    ) external onlyRole(ADMIN) validatePoolById(pid) {
        PoolInfo storage pool = poolInfo[pid];
    ...
    function updatePoolReward(uint256 pid, uint256 amountReward) external onlyRole(ADMIN) validatePoolById(pid) {
        PoolInfo storage pool = poolInfo[pid];
        require(pool.openTime > block.timestamp, "pool started");
    ...
    function setTwapPeriod(uint32 _twapPeriod) external onlyRole(ADMIN) {
        twapPeriod = _twapPeriod;
    }

    function setTiers(SetTierParam[] calldata params) external onlyRole(ADMIN) {
    ...
    function withdrawReward(IERC20 token, uint256 amount) external onlyRole(ADMIN) {
        require(token != gpoolToken, "only reward token");
        token.safeTransfer(msg.sender, amount);
    }

    function _lockUserReward(address _user, uint _poolId, uint _lockedAmount) internal {
        if (_lockedAmount > 0) {
            PoolInfo memory pool = poolInfo[_poolId];
            LockedReward memory userLockedReward = lockingAmounts[_poolId][_user];

            uint updatedLockingAmount = userLockedReward.amount + _lockedAmount;
