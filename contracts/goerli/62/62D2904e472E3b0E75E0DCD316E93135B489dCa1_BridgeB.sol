...
contract BridgeB {
    ...
    event MintTokens(address receiver,uint amountUnlocked);
   ...
   function checkTx(bytes32 TX) external view gnosisSafeOnly(msg.sender)
    ...
    function burnTokens(address userFrom,uint amount) external {
        ...
        emit BurnTokens(keccak256(abi.encode(address(0),amount)),abi.encode(address(0),amount));
    }
    //tokenFunctionality
    ...
    function __mint(bytes memory arg) internal  {
        ...
        emit MintTokens(receiver, amountUnlocked);
