...
import "./mixins/OZ/solc8/ERC721URIStorageUpgradeable.sol";
...
import "./interfaces/solc8/IHasSecondarySaleFees.sol";
 ...
 * @notice All NFTs from this contract are minted by the same creator. A 10% royalty to the creator is included.
 */
  ...
  ERC721BurnableUpgradeable
{
   ...
   * @dev This value may be set at anytime, but once set it may not be increased.
   */
   ...
   */
  mapping(string => bool) private ipfsHashToMinted;
  ...
  modifier onlyOwner() {
    require(msg.sender == owner, "CollectionContract: Caller is not creator");
    _;
  }

  modifier onlyOperator() {
    require(
      collectionFactory.adminRoleContract().isOperator(msg.sender),
      "CollectionContract: Caller is not an operator"
    );
    _;
  }
  ...
  function initialize(
    ...
    require(_creator != address(0), "CollectionContract: Creator is required");
  ...
  /**
   * @notice Allows the owner to mint an NFT defined by its IPFS metadata path.
   */
  ...
  /**
   * @notice Allows the owner to mint and sets approval for the provided marketplace contract.
   ...
   */
  function mintAndApprove(string memory tokenIPFSPath, address market) public returns (uint256 tokenId) {
    tokenId = _mint(tokenIPFSPath);
  ...
  /**
   * @notice Allows the owner to mint an NFT and have creator revenue/royalties sent to an alternate address.
   * Also sets approval for the provided marketplace contract.
   * @dev This can be used by creators the first time they mint an NFT to save having to issue a separate approval
   * transaction before starting an auction.
   */
   ...
   * @param paymentAddressFactory The contract to call which will return address to use for payments.
  ...
  function updateMaxTokenId(uint256 _maxTokenId) external onlyOwner {
    ...
    require(lastTokenId + 1 <= _maxTokenId, "CollectionContract: Max token ID must be greater than last mint");
  ...
  function adminAccountMigration(
    ...
    if (migrateCreator && owner != newAddress) {
      require(owner == originalAddress, "CollectionContract: Not created by the given address");
      owner = newAddress;
   ...
   */
  function getHasMintedIPFSHash(string memory tokenIPFSPath) public view returns (bool) {
    return ipfsHashToMinted[tokenIPFSPath];
  }
   ...
   * @dev The payment address may be assigned for each individual NFT, but if not defined the owner is returned.
   */
  ...
  function _mint(string memory tokenIPFSPath) private onlyOwner returns (uint256 tokenId) {
    require(!ipfsHashToMinted[tokenIPFSPath], "CollectionContract: NFT was already minted");
    unchecked {
      tokenId = ++lastTokenId;
      ...
      ipfsHashToMinted[tokenIPFSPath] = true;
  ...
  function _burn(uint256 tokenId) internal override(ERC721Upgradeable, ERC721URIStorageUpgradeable) {
    delete ipfsHashToMinted[_tokenURIs[tokenId]];
...
import "@openzeppelin/contracts-upgradeable-solc-8/token/ERC721/ERC721Upgradeable.sol";
import "@openzeppelin/contracts-upgradeable-solc-8/proxy/utils/Initializable.sol";

/**
 * @dev ERC721 token with storage based token URI management.
 *
 * Copied from https://github.com/OpenZeppelin/openzeppelin-contracts-upgradeable/blob/v4.3.2
 * Modified to change `_tokenURIs` to `internal`
 */
...
interface ICollectionFactory {
  function adminRoleContract() external returns (IAdminRole);
