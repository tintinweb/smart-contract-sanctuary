...
contract CLeaves is ERC20Burnable, Ownable {

    using SafeMath for uint256;

    uint256 public MAX_WALLET_STAKED = 100;
    uint256 public EMISSIONS_RATE = 11574070000000000000;
    uint256 public CLAIM_END_TIME = 9641013200;

    address nullAddress = 0x0000000000000000000000000000000000000000;

    address public koalaAddress;

    //Mapping of koala to timestamp
    mapping(uint256 => uint256) internal tokenIdToTimeStamp;

    //Mapping of koala to staker
    mapping(uint256 => address) internal tokenIdToStaker;

    //Mapping of staker to koala
    mapping(address => uint256[]) internal stakerToTokenIds;



constructor(address _koalaAddress) ERC20("CLeaves", "CLV") {
        koalaAddress = _koalaAddress;
    }

    function setKoalaAddress(address _koalaAddress) public onlyOwner {
        koalaAddress = _koalaAddress;
        return;
    }
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        require(
            stakerToTokenIds[msg.sender].length + tokenIds.length <=
                MAX_WALLET_STAKED,
            "Must have less than 31 koala staked!"
        );

        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                IERC721(koalaAddress).ownerOf(tokenIds[i]) == msg.sender &&
                    tokenIdToStaker[tokenIds[i]] == nullAddress,
                "Token must be stakable by you!"
            );

            IERC721(koalaAddress).transferFrom(
                msg.sender,
                address(this),
                tokenIds[i]
            );
