...
contract Minter is ERC721Holder, Ownable{
  ...
  function mintRare(uint256 _mintAmount, uint16[] calldata colorTokenIds) payable public onlyOwner {
    ...
    string memory trait = subString(jsonBytes, traitIndex+17, traitIndex+28);
    emit metaShow(trait, tokenId);
    require(keccak256(abi.encodePacked(trait)) == keccak256(abi.encodePacked("Tokyo Drift")), "Not a valid rarity");
  }
