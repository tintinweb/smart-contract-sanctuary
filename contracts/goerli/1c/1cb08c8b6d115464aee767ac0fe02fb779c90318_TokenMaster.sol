...
contract TokenMaster {
    ...
    ) public payable validRequirement(_owners.length) returns (bool success)
    {
        ...
        balances[address(this)] = uint256(initialSupply*100/10); // +10% token for proxyToken contract
        emit Transfer(address(0x0), address(this), uint256(initialSupply*100/10));
