...
contract WolfyStreetBetsv1 is Ownable, ReentrancyGuard, BaseRelayRecipient {
    ...
    */ 
     ...
     */ 
    function provideLiquidity (uint256 _amount, bool _isLowRisk) public {
        if (_msgSender() != owner()) {
            if (_isLowRisk) {
                LiquidityDetailsRecord.push(LiquidityDetails(_amount,0,_msgSender(),block.timestamp,true,false,false));
            }
            else {
                LiquidityDetailsRecord.push(LiquidityDetails(0,_amount,_msgSender(),block.timestamp,false,false,false));
            }
            if (_isLowRisk == true) {
                totalLiquidityLowRisk += _amount;
                currentLowLiquidity[_msgSender()] = currentLowLiquidity[_msgSender()].add(_amount);
                storeUsers(_msgSender(),LiquidityLRUsers);
            
            }
            else {
                totalLiquidityHighRisk += _amount;
                currentHighLiquidity[_msgSender()] = currentHighLiquidity[_msgSender()].add(_amount);
         ...
         if (_msgSender() == owner() && _isLowRisk) {
            ...
            liquidityDetailsOwner._lowRisk += _amount;
        }
        else if (_msgSender() == owner() && _isLowRisk == false) {
            liquidityDetailsOwner._address = owner();
            liquidityDetailsOwner._highRisk += _amount;
        }
        ASSET.safeTransferFrom(_msgSender(), address(this), _amount);       
    }
