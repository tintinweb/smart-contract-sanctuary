...
/*
 * Crypto stamp Create Collection Helper
...
// File: contracts/CreateCollectionHelper.sol

/*
 * Helper for ERC721 contracts that support signed transfers for performing
 * those right into new L2 collections
 */

contract CreateCollectionHelper {

    BridgeDataI public bridgeData;

    event BridgeDataChanged(address indexed previousBridgeData, address indexed newBridgeData);
    // Events from ERC721, ERC721SignedTransferI, and Collections - never emitted in this contract but helpful for running our tests.
    event Transfer(address indexed from, address indexed to, uint256 indexed tokenId);
    event SignedTransfer(address operator, address indexed from, address indexed to, uint256 indexed tokenId, uint256 signedTransferNonce);
    event NewCollection(address indexed owner, address collectionAddress);
    ...
    public
    {
        ...
        ERC721SignedTransferI cstoken = ERC721SignedTransferI(_tokenContractAddress);
        CollectionsI collections = CollectionsI(bridgeData.getAddress("Collections"));
        address collectionAddress = collections.createFor(_newOwner, _notificationContract, _ensName, _ensSubdomainName, _ensSubdomainRegistrarAddress, _ensReverseRegistrarAddress);
