...
contract TokenControllerMultiSignature is AccessControl {
    ...
    function acceptAddMinter(address _requester, address _minter, uint _maxAmount, uint _requestAt, uint _expireAt, bytes32 _salt) external {
        ...
        if (signatureNum[hash] >= SIGNATURE_MIN_NUM) {
            timeLocker.schedule(address(token), 0, abi.encodeWithSignature("addMinter(address,uint256)", _minter, _maxAmount), _salt, _salt, timeLocker.getMinDelay());
        }
    }

    function executeAddMinter(address _minter, uint _maxAmount,  bytes32 _salt) external {
        timeLocker.execute(address(token), 0, abi.encodeWithSignature("addMinter(address,uint256)", _minter, _maxAmount), _salt, _salt); 
    }
    ...
    function acceptDelMinter(address _requester, address _minter, uint _requestAt, uint _expireAt, bytes32 _salt) external {
        ...
        if (signatureNum[hash] >= SIGNATURE_MIN_NUM) {
            timeLocker.schedule(address(token), 0, abi.encodeWithSignature("delMinter(address)", _minter), _salt, _salt, timeLocker.getMinDelay());
        }
    }

    function executeDelMinter(address _minter,  bytes32 _salt) external {
        timeLocker.execute(address(token), 0, abi.encodeWithSignature("delMinter(address)", _minter), _salt, _salt); 
    }
    ...
    function acceptSetPendingOwner(address _requester, address _owner, uint _requestAt, uint _expireAt, bytes32 _salt) external {
        ...
        if (signatureNum[hash] >= SIGNATURE_MIN_NUM) {
            timeLocker.schedule(address(token), 0, abi.encodeWithSignature("setPendingOwner(address)", _owner), _salt, _salt, timeLocker.getMinDelay());
        }
    }

    function executeSetPendingOwner(address _owner,  bytes32 _salt) external {
        timeLocker.execute(address(token), 0, abi.encodeWithSignature("setPendingOwner(address)", _owner), _salt, _salt); 
    }
}