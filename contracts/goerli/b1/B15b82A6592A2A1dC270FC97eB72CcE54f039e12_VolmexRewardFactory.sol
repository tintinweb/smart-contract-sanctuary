 ...
 */
contract VolmexRewardFactory is VestingToken, OwnableUpgradeable {
    // mapping of user wallet address to array of owned indexes
    mapping(address => address[]) private clonedTreasuryAddresses;

    mapping(address => bool) public eligibleBurner;
    ...
    // addresses of new treasury Contract
    address[] public newTreasuryContractAddresses;
    ...
    function initialize(
        ...
        _mint(msg.sender, VESTING_AMOUNT);
    }
    ...
    function getEligibleBurner(address account) external view returns (bool) {
        return eligibleBurner[account];
    }

    /**
     * @notice get the instances of treasure vesting contract
     * @param _user is address of cloned contract holder
     */
    function getVestingContract(address _user)
        external
        view
        returns (address[] memory)
    {
        return clonedTreasuryAddresses[_user];
    }
    ...
    function cloneTreasuryVesterContract(
        ...
        for (uint256 i = 0; i < _recipients.length; i++) {
            ...
            newTreasuryContractAddresses.push(newTreasuryContract);
            eligibleBurner[newTreasuryContract] = true;
            ...
            transferFrom(msg.sender, _recipients[i], _quantities[i]);
            _approve(_recipients[i], address(this), _quantities[i]);
            clonedTreasuryAddresses[_recipients[i]].push(newTreasuryContract);
        }
        indexCount = _indexCount;
        emit ClonedTreasureVester(
                _recipients,
                newTreasuryContractAddresses,
                _quantities
            );
    }

    function updateRecipient(address newRecipient, address treasureVester) external onlyOwner {
        require(newRecipient != address(0), "VolmexRewardFactory: Cannot be 0 address");
        address prevRecipient = ITreasuryVester(treasureVester).recipient();
        uint256 balance = IVestingToken(treasureVester).balanceOf(prevRecipient);
        transferFrom(prevRecipient, newRecipient, balance);
...
import "./GovernanceERC20VestingToken.sol";

/**
 * @notice implementation of the Volmex Vesting token contract
 * @author Volmex
 */
contract VestingToken is GovernanceERC20VestingToken {
    string internal constant NAME = "Vesting Token";
    string internal constant SYMBOL = "VST";
    uint8 internal constant DECIMALS = 18;

    uint256 public constant REVISION = 1;

    /// @dev the amount being distributed for the PSI and PEI
    uint256 internal constant VESTING_AMOUNT = 240000000 ether;
     ...
     */
    function init(address deployer) external initializer {
...
import "./interfaces/IGovernanceERC20VestingToken.sol";

/**
 * @notice implementation of the VOLMEX VESTING token contract
 * @author Volmex
 */
abstract contract GovernanceERC20VestingToken is
    ERC20Upgradeable,
    IGovernanceERC20VestingToken
{
...
interface IGovernanceERC20VestingToken {
