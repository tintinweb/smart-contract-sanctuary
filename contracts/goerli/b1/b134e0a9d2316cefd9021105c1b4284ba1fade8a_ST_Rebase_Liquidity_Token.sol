...
contract ST_Rebase_Liquidity_Token is Context, IST20, Ownable, TokenGenerator {
    ...
    constructor (string memory name, string memory symbol, uint256 decimals, uint256 totalSupply, uint256 _maxTxAmount, uint256 _taxFee, uint256 _burnFee, address _burnAddress, address payable feeReceiver) 
    ...
    function setBurnFee(uint256 _burnFee) external onlyOwner() {
        require(_burnFee <= 30, "IST20: Burn fees can not be higher than 30%.");
        burnFee = _burnFee;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        return (tTransferAmount, TData(0, tFee, tBurn, 0));
    }
