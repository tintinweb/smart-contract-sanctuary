...
contract Median_StakedETH_USD is LibNote {
    ...
    uint128        val;
    uint32  public age;
    bytes32 public constant wat = "stETH/USD";
    ...
    modifier toll {require(bud[msg.sender] == 1, "Median/contract-not-whitelisted");
        _;}
    ...
    function drop(address[] calldata a) external note auth {
        for (uint i = 0; i < a.length; i++) {
            orcl[a[i]] = 0;
            slot[uint8(uint160(a[i]) >> 152)] = address(0);
        }
    }
