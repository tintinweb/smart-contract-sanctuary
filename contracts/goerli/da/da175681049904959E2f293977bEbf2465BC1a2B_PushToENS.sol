...
contract PushToENS is SupportsENS {
  ...
  function pullEther2ENS (bytes32 nameHash) public {
    ...
    */
  function pullEther2Ethadd (address payable ethAddress) public {
      // Zeroes out full balance
      uint256 amount = _pull(keccak256(abi.encode(ethAddress)), 0x0000000000000000000000000000000000000000, 1);
  ...
  function pullAsset2ENS (bytes32 nameHash, address assetAddress, uint nftId) public {
    ...
    if (nftId==0){
      ...
      uint256 amount = _pull(keccak256(abi.encode(nameHash,assetAddress)), assetAddress, 2);

      // transfer tokens
      // getSafeENSAddress prevents returning empty address
      asset.transferFrom(address(this), getSafeENSAddress(nameHash), amount);
    } else {
      ...
      // transfer NFT
      // getSafeENSAddress prevents returning empty address
      asset.transferFrom(address(this), getSafeENSAddress(nameHash), nftId);
    }
  ...
  function _cancel (bytes32 hash, uint256 amount, address assetAddress, uint8 sendType) internal {
    ...
    require(balance.lastPushed[msg.sender] > balance.lastBlockPulled, "Recipient already withdrew");
  ...
  function cancelEther2Ethadd (address payable ethAddress, uint256 amount) public {
    ...
    * @dev Pulls any amount of a given token to a valid ENS address. Can be called by anyone
    */
  function cancelAsset2ENS (bytes32 nameHash, uint256 amount, address assetAddress, bool fungible) public {
       // instantiate token
     Transferable asset = Transferable(assetAddress);
  ...
  //   */
  // function cancelNFT2ENS (bytes32 nameHash, uint256 amount, address nftAddress, uint256 tokenId) public {
  //      // instantiate token
  //    Transferable NFT = Transferable(nftAddress);

  //    //generic pull
  //     _cancel(keccak256(abi.encode(nameHash, nftAddress, tokenId)), amount, nftAddress, 3);
