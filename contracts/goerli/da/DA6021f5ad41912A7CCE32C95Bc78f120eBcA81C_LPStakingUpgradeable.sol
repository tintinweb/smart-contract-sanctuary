...
contract LPStakingUpgradeable is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    */
    ...
    function getStakeInfo(address _account, uint256 _pid) external view returns (
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant() {
        ...
        if(0 < pendingZONE_ && claimIn == 0) {
            _reward(_account, _pid, pendingZONE_);
            pendingZONE_ = 0;
        } else if(0 < _amount) {
    ...
    function _reward(address _account, uint256 _pid, uint256 _pendingZONE) private {
