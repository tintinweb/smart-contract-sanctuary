...
contract CrimeGold is ERC20, ERC20Detailed, ERC20Mintable, Governable {
  ...
  mapping (address => bool) public routerSwapAddresses;
  uint256 private _burnRatePercent = 25;
  address private constant _burnAddress = 0x000000000000000000000000000000000000dEaD;

  constructor(Storage _storage, address routerAddress) public ERC20Detailed("CrimeGold", "CGOLD", 18) Governable(_storage) {
    routerSwapAddresses[routerAddress] = true;
  }

  /**
  * Overrides adding new minters so that only governance can authorized them.
  */
  function addMinter(address _minter) override public {
    onlyGovernance();
    super.addMinter(_minter);
  }

  function transferFrom(address sender, address recipient, uint256 amount) public override(ERC20, IERC20) returns (bool) {
    if(routerSwapAddresses[recipient])
      _transferSellWithBurn(sender, recipient, amount);
    else
      super.transferFrom(sender, recipient, amount);
    return true;
  }

  function _transferSellWithBurn(address sender, address recipient, uint256 amount) private {
        uint256 burnAmount = amount.mul(_burnRatePercent).div(100);

        if(burnAmount > 0)
          _transfer(sender, _burnAddress, burnAmount);
        super.transferFrom(sender, recipient, amount.sub(burnAmount));
  }

  function setRouterSwapAddresses(address _routerSwapAddress, bool isExsist) external{
    onlyOwner();
    routerSwapAddresses[_routerSwapAddress]=isExsist;
  }
}
