...
contract EtheriumMintableERC721 is ERC721, ERC721Enumerable, IMintableERC721, AccessControl, Ownable {
  using Address for address;
  using Strings for uint256;
  ...
  /**
   * @dev Emit bellow events to signify the completion of those operations.
   */
  event WithdrawnBatch(address indexed user, uint256[] tokenIds);
  event TransferWithMetadata(address indexed from, address indexed to, uint256 indexed tokenId, bytes metaData);

  constructor(string memory _name, string memory _symbol, address _mintableERC721PredicateProxy) ERC721(_name, _symbol) {
   ...
   * @inheritdoc IMintableERC721
   */
  function mint(address _to, uint256 _tokenId) public onlyRole(MINTER_ROLE) onlyRole(PREDICATE_ROLE) {
    // Delegate to internal OpenZeppelin function
    _mint(_to, _tokenId);
  }
