...
contract Pool is ILiquidityPool, Initializable, ERC20, Ownable, Pausable {
    ...
    function withdraw(uint256 requestedAmount) external override whenNotPaused nonReentrant {
        ...
        bytes32 eventSig = keccak256("Withdraw"); // Do we want full sig with params? Also, should I pass string and hash in function?
    ...
    // Change to take in address instead of Interface
    function setEventProxy(address _eventProxy) external onlyOwner {
        require(_eventProxy != address(0), "INVALID_ADDRESS");
        require(_eventProxy != address(eventProxy), "SAME_ADDRESS");

        eventProxy = IEventProxy(_eventProxy);
    }

    function setEventSend(bool _eventSendSet) external onlyOwner {
    ...
    function _deposit(
        ...
        bytes32 eventSig = keccak256("Deposit"); // Do we want full sig with params? Also, should I pass string and hash in function?
    ...
    function encodeAndSendData(bytes32 _eventSig, address _user) private onEventSend {
        ...
        bytes memory data = abi.encode(_user, address(this), userBalance);

        eventProxy.sendEvent(_eventSig, data);
    }
}
...
interface IEventProxy {
    ...
    event RegisterDestination(address sender, address l2Endpoint, address l2MessageRouter, bytes32 eventType);
    event UnregisterDestination(address sender, address l2Endpoint, address l2MessageRouter, bytes32 eventType);
    event EventSent(bytes32 eventType, address sender, Destination destination, bytes data);
    ...
    /// @notice For a sender, register destination contracts on Polygon that should receive sent events
    /// @param sender Address of sender
    /// @dev Should a contract be able to send an event, where is it going on Polygon
    /// @dev Consider multiple destinations or single
    /// @dev Calls register function on StateSender contract
    function registerDestination(address sender, address l2Endpoint, address l2MessageRouter, bytes32 eventType, bytes calldata functionSig) external;

    /// @notice For a sender, unregister destination contracts on Polygon
    /// @param sender Address of sender
    function unregisterDestination(address sender, address l2Endpoint, address l2MessageRouter, bytes32 eventType) external;

    /// @dev Should a caller not be registered, or have no destinations, make the a call no-op    
    /// @dev Calls syncState function in StateSender contract
    function sendEvent(bytes32 eventType, bytes calldata data) external;
}
...
import "../interfaces/IEventProxy.sol";
import "../interfaces/events/Destination.sol";
...
contract EventProxy is Initializable, IEventProxy, Ownable {
    ...
    mapping (address => mapping (bytes32 => Destination[])) public destinations; 
    ...
    // Can probably remove this and register both senders and destinations throught registerDestination()
    ...
    // Does this need to be able to register multiple destinations at once?
    function registerDestination(
        address _sender, 
        address _l2Endpoint, 
        address _l2MessageRouter,
        bytes32 _eventType,
        bytes calldata _functionSig
    ) external override onlyOwner {
        require(_sender != address(0), "INVALID_ADDRESS");
        require(_l2Endpoint != address(0), "INVALID_ADDRESS");
        require(_l2MessageRouter != address(0), "INVALID_ADDRESS");

        uint256 destinationLength = destinations[_sender][_eventType].length;
        Destination storage destination = destinations[_sender][_eventType][destinationLength];

        destination.l2MessageRouter = _l2MessageRouter;
        destination.l2Endpoint = _l2Endpoint;
        destination.functionSig = _functionSig;

        emit RegisterDestination(_sender, _l2Endpoint, _l2MessageRouter, _eventType);
    }
    ...
    function unregisterDestination(
        ...
        Destination[] storage destination = destinations[_sender][_eventType];
        
        uint256 index = 256 ** 2 - 1;
        for (uint256 i = 0; i < destination.length; i++) {
            if (destination[i].l2Endpoint == _l2Endpoint && destination[i].l2MessageRouter == _l2MessageRouter) {
                index = i;
                break;
            }
            ...
            if (index < 256 ** 2 - 1) {
                ...
                // Shifts array over -1
                for (uint256 i = index; i < destination.length; i++) {
                    destination[i] = destination[i + 1];
                }
            } else {
                revert("DESTINATION_DOES_NOT_EXIST");
            }
        }

        emit UnregisterDestination(_sender, _l2Endpoint, _l2MessageRouter, _eventType);
    }

    // Event wrapper
    function sendEvent(bytes32 _eventType, bytes calldata _data) external override {
        require(_data.length > 0, "NO_DATA");
        
        Destination[] memory destinationArray = destinations[msg.sender][_eventType];

        for (uint256 i = 0; i < destinationArray.length; i ++) {
            Destination memory destination = destinationArray[i];
            bytes memory data = abi.encode(EventWrapper({eventType: _eventType, data: _data}));
            destination.l2MessageRouter.call(abi.encodeWithSelector(
                bytes4(keccak256(destination.functionSig)), destination.l2Endpoint, data)
            );
            emit EventSent(_eventType, msg.sender, destination, _data);
        }
    }
}
