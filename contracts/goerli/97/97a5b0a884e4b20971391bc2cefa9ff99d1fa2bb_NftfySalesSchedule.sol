...
abstract contract TimeLockedAccounts
{
	...
	function _available(address _receiver, uint256 _when) private view returns (uint256 _newCurrentBalance, uint256 _newBaseTime, uint256 _newBasePeriodIndex, uint256 _newBasePeriodCount, uint256 _amount)
	{
		...
		while (true) {
			...
			uint256 _amountPerPeriod = (_initialBalance * _ratePerPeriod) / 1e18;
			...
			if (_newBasePeriodCount == 0) {
				if (++_newBasePeriodIndex == _periods.length) break;
				_newBasePeriodCount = _periods[_newBasePeriodIndex].periodCount;
			}
		}
	...
	{
		require(1 <= _amount && _amount <= uint256(-1) / 1e18, "invalid amount");
		...
		for (uint256 _i = 0; _i < _periods.length; _i++) {
			...
			uint256 _ratePerPeriod = _period.ratePerPeriod;
			uint256 _amountPerPeriod = (_amount * _ratePerPeriod) / 1e18;
		...
		_account.basePeriodCount = _periods[0].periodCount;
	}
	...
	function _addPlanPeriod(uint256 _planId, uint256 _periodDuration, uint256 _periodCount, uint256 _ratePerPeriod) internal
	{
		...
		require(_ratePerPeriod <= 1e18, "invalid rate");
		uint256 _maxPeriodCount = _ratePerPeriod == 0 ? 1 : 1e18 / _ratePerPeriod;
	...
	function _enablePlan(uint256 _planId) internal
	{
		...
		require(_sumRate == 1e18, "invalid rate sum");
		_plan.enabled = true;
	}
}
...
abstract contract ManagedTimeLockedAccounts is WhitelistGuard, TimeLockedAccounts
{
...
contract NftfySalesSchedule is ManagedTimeLockedAccounts
{
	constructor (address _token, address _treasury) ManagedTimeLockedAccounts(_token, _treasury) public
	{
		_createSalesPlan("30d => 8% + 16 x (30d => 5.75%)", 30 days, 8e16, 30 days, 16, 575e14);
		_createSalesPlan("30d => 10% + 12 x (30d => 7.5%)", 30 days, 10e16, 30 days, 12, 75e15);
		_createSalesPlan("36h => 12% + 8 x (30d => 11%)", 36 hours, 12e16, 30 days, 8, 11e16);
	}

	function _createSalesPlan(string memory _description, uint256 _cliffDuration, uint256 _ratePerCliff, uint256 _periodDuration, uint256 _periodCount, uint256 _ratePerPeriod) internal
	{
		uint256 _planId = _createPlan(_description);
		_addPlanPeriod(_planId, _cliffDuration, 1, _ratePerCliff);
		_addPlanPeriod(_planId, _periodDuration, _periodCount, _ratePerPeriod);
		_enablePlan(_planId);
	}
}

contract NftfyVestingSchedule is ManagedTimeLockedAccounts
{
	constructor (address _token, address _treasury) ManagedTimeLockedAccounts(_token, _treasury) public
	{
		...
		_rates[4] = 12e16;
		_rates[5] = 8e16;
		_createVestingPlan("52 x (1w => 24%/52) + 52 x (1w => 22%/52) + 52 x (1w => 18%/52) + 52 x (1w => 16%/52) + 52 x (1w => 12%/52) + 52 x (1w => 8%/52)", 1 weeks, 52, _rates);
	}

	function _createVestingPlan(string memory _description, uint256 _periodDuration, uint256 _periodCount, uint256[] memory _rates) internal
	{
		uint256 _planId = _createPlan(_description);
		uint256 _remRate = 0;
