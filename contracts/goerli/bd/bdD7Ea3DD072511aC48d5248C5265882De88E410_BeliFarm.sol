...
contract BeliFarm is Ownable, ReentrancyGuard {
    ...
    function deposit(
        ...
        uint256 oldDebt = user.rewardDebt;
        uint256 oldFeeReceiverDebt = user.rewardFeeReceiverDebt;
        uint256 sharesAdded;
        ...
        if (block.timestamp >= user.withdrawalFeeUntil) {
            ...
            user.rewardDebt = oldDebt.add(sharesAdded.mul(pool.accBELIPerShare).div(1e12));
            user.rewardFeeReceiverDebt = oldFeeReceiverDebt.add(
                sharesAdded.mul(pool.accBELIPerShareFromFees).div(1e12)
            );
            // user.rewardDebt = user.shares.mul(pool.accBELIPerShare).div(1e12).sub(oldDebt);
            // user.rewardFeeReceiverDebt = user.shares.mul(pool.accBELIPerShareFromFees).div(1e12).sub(oldFeeReceiverDebt);
        }
