...
contract GasFuture {
    ...
    function refund() external {
        ...
        status = State.IDLE;
        buyer.call{ value: payment + eps }(""); // the payment is sent to the seller anyway
    }

    function execute(MetaTx memory _metaTx, bytes memory _sig) external {
        require(block.number >= startBlock && block.number <= endBlock, "Block must be between start and end");
        require(status == State.REGISTERED, "Contract not registered");
        require(msg.sender == seller, "Wrong seller"); // necessary?
        verifyAndExecute(_metaTx, _sig);
        status = State.IDLE;
        seller.call{ value: collateral + payment + eps }(""); // the payment is sent to the seller anyway
    }
