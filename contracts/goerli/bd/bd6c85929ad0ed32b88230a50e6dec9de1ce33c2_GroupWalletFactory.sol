...
contract GroupWalletFactory {
    ...
    mapping (bytes32 => uint)       public transactionCount;

    mapping (bytes32 => mapping (uint => Transaction)) public transactions;
    mapping (bytes32 => mapping (address => bool)) public isOwner;
    mapping (bytes32 => mapping (uint => mapping (address => bool))) public confirmations;
    ...
    modifier ownerExists(bytes32 _domainHash,address owner) {
        require(isOwner[_domainHash][owner],"ownerExists!!!");
        _;
    }

    modifier confirmed(bytes32 _domainHash,uint transactionId, address owner) {
        require(confirmations[_domainHash][transactionId][owner],"confirmed!!!");
        _;
    }

    modifier notExecuted(bytes32 _domainHash,uint transactionId) {
        require(!transactions[_domainHash][transactionId].executed,"notExecuted!!!");
        _;
    }
    
    modifier ownerDoesNotExist(bytes32 _domainHash,address _owner) {
        require(!isOwner[_domainHash][_owner],"ownerDoesNotExist!!!");
        _;
    }
    
    modifier validRequirement(bytes32 _domainHash,uint ownerCount) {
        require(ownerCount <= MAX_OWNER_COUNT
            && required[_domainHash] <= ownerCount
            && required[_domainHash] != 0
            && ownerCount != 0,"validRequirement!!!");
        _;
    }


    function getRequired(bytes32 _domainHash) public view returns (uint count)
    {
      return required[_domainHash];
    }
    
    function getIsOwner(bytes32 _domainHash,address _owner) public view returns (bool resultIsOwner)
    {
      return isOwner[_domainHash][_owner];
    }
    
    function getTransactionsCount(bytes32 _domainHash) public view returns (uint count)
    {
      return transactionCount[_domainHash];
    }
    
    function getTransactions(bytes32 _domainHash,uint transactionId) public view returns (address destination,uint value,bytes memory data,bool executed)
    {
      return (transactions[_domainHash][transactionId].destination,transactions[_domainHash][transactionId].value,transactions[_domainHash][transactionId].data,transactions[_domainHash][transactionId].executed);
    }

    function getConfirmationCount(bytes32 _domainHash,uint transactionId) public view returns (uint count)
    {
        for (uint i=0; i<memberArr[_domainHash].length; i++)
            if (confirmations[_domainHash][transactionId][memberArr[_domainHash][i]]) count += 1;
    }

    function getTransactionCount(bytes32 _domainHash,bool pending, bool executed) public view returns (uint count)
    {
        for (uint i=0; i<transactionCount[_domainHash]; i++)
            if (pending && !transactions[_domainHash][i].executed || executed && transactions[_domainHash][i].executed)
              count += 1;
    }

    function getOwners(bytes32 _domainHash) public view returns (address[] memory)
    {
        return memberArr[_domainHash];
    }
    
    
    function addOwner(bytes32 _domainHash,address _owner) public ownerDoesNotExist(_domainHash,_owner) validRequirement(_domainHash,memberArr[_domainHash].length + 1)
    {
        require(msg.sender == address(this),"onlyWallet!!!");
        require(_owner != address(0x0),"notNull!!!");

        isOwner[_domainHash][_owner] = true;
        
        memberArr[_domainHash].push(_owner);
    }

    function removeOwner(bytes32 _domainHash, address _owner) public ownerExists(_domainHash,_owner)
    {
        require(msg.sender == address(this),"onlyWallet!!!");

        isOwner[_domainHash][_owner] = false;
        for (uint i=0; i<memberArr[_domainHash].length - 1; i++)
            if (memberArr[_domainHash][i] == _owner) {
                memberArr[_domainHash][i] = memberArr[_domainHash][memberArr[_domainHash].length - 1];
                break;
            }
        memberArr[_domainHash].length -= 1;
        if (required[_domainHash] > memberArr[_domainHash].length) changeRequirement(_domainHash,memberArr[_domainHash].length);
    }
    
    function replaceOwner(bytes32 _domainHash, address _owner, address _newOwner) public ownerExists(_domainHash,_owner) ownerDoesNotExist(_domainHash,_newOwner)
    {
        require(msg.sender == address(this),"onlyWallet!!!");

        for (uint i=0; i<memberArr[_domainHash].length; i++)
            if (memberArr[_domainHash][i] == _owner) {
                memberArr[_domainHash][i] = _newOwner;
                break;
            }
        isOwner[_domainHash][_owner]    = false;
        isOwner[_domainHash][_newOwner] = true;
    }

    function addressConfirmations(bytes32 _domainHash,uint _transactionId,address _owner) public view returns (bool wasExecuted)
    {
      return confirmations[_domainHash][_transactionId][_owner];
    }

    function changeRequirement(bytes32 _domainHash,uint _required) public
    {
      require(msg.sender == address(this),"onlyWallet!!!");
      
      required[_domainHash] = _required;
    }
    
    function addTransaction(bytes32 _domainHash,address destination, uint value, bytes memory data) internal returns (uint transactionId)
    {   
        require(destination != address(0x0),"notNull!!!");

        transactionId = transactionCount[_domainHash];
        transactions[_domainHash][transactionId] = Transaction({destination: destination,value: value,data: data,executed: false});
        transactionCount[_domainHash] += 1;
    }

    function submitTransaction(bytes32 _domainHash, address destination, uint value, bytes memory data) public returns (uint transactionId)
    {
        transactionId = addTransaction(_domainHash, destination, value, data);
        confirmTransaction(_domainHash,transactionId);
    }

    function confirmTransaction(bytes32 _domainHash,uint transactionId) public ownerExists(_domainHash,msg.sender)
    {
        require(transactions[_domainHash][transactionId].destination != address(0x0),"transactionExists!!!");

        require(!confirmations[_domainHash][transactionId][msg.sender],"notConfirmed!!!");
        confirmations[_domainHash][transactionId][msg.sender] = true;
        executeTransaction(_domainHash,transactionId);
    }

    function revokeConfirmation(bytes32 _domainHash,uint transactionId) public ownerExists(_domainHash,msg.sender) confirmed(_domainHash,transactionId, msg.sender) notExecuted(_domainHash,transactionId)
    {
        confirmations[_domainHash][transactionId][msg.sender] = false;
    }
    
    function getConfirmations(bytes32 _domainHash,uint transactionId) public view returns (address[] memory _confirmations)
    {
        address[] memory confirmationsTemp = new address[](memberArr[_domainHash].length);
        uint count = 0;
        uint i;
        for (i=0; i<memberArr[_domainHash].length; i++)
            if (confirmations[_domainHash][transactionId][memberArr[_domainHash][i]]) {
                confirmationsTemp[count] = memberArr[_domainHash][i];
                count += 1;
            }
    ...
    function isConfirmed(bytes32 _domainHash,uint transactionId) public view returns (bool)
    {
        uint count = 0;
        for (uint i=0; i<memberArr[_domainHash].length; i++) {
            if (confirmations[_domainHash][transactionId][memberArr[_domainHash][i]])
                count += 1;
            if (count == required[_domainHash])
                return true;
        }
    }

    function executeTransaction(bytes32 _domainHash,uint transactionId) public ownerExists(_domainHash,msg.sender) confirmed(_domainHash,transactionId, msg.sender) notExecuted(_domainHash,transactionId)
    {
        if (isConfirmed(_domainHash,transactionId)) {
            Transaction storage txn = transactions[_domainHash][transactionId];
            txn.executed = true;
    ...
    function inviteInstallToken(address[] memory _owners, bytes32[] memory _labels, bytes32[] memory _domainLabels, bytes memory data128, bytes memory data, bytes32 _domainHash, address masterCopy) public payable validOwners(_owners) byInitiator(_domainHash) returns (ProxyToken proxy) {      
      ...
      for (uint i=0; i<_owners.length; i++) {
        ...
        isOwner[_domainHash][_owners[i]] = true;
      }
