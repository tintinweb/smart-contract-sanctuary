 ...
 */
contract FxStateRootTunnel is FxBaseRootTunnel, Initializable {

    // DEPOSIT and MAP_TOKEN 
    bytes32 public constant DEPOSIT = keccak256("DEPOSIT");
    bytes32 public constant MAP_TOKEN = keccak256("MAP_TOKEN");
    bytes32 public constant SET_MULTIPLIER = keccak256("SET_MULTIPLIER");
    ...
    function initialize(address _checkpointManager, address _fxRoot) public initializer {
        _initialize(_checkpointManager, _fxRoot);(_checkpointManager, _fxRoot);
    }
    ...
    function mapToken(address rootToken, address childToken) public {
        ...
        IERC20 rootTokenContract = IERC20(rootToken);
        ...
        bytes memory message = abi.encode(MAP_TOKEN, abi.encode(rootToken, name, symbol, decimals, childToken));
    ...
    function deposit(address rootToken, address user, uint256 amount, bytes memory data) public {
        // Revert if not mapped
        require(rootToChildTokens[rootToken] != address(0x0), "Unmapped child token");
    ...
    function setMultiplier(address rootToken, uint256 multiplier) public {
        // Revert if not mapped
        require(rootToChildTokens[rootToken] != address(0x0), "Unmapped child token");
        // SET_MULTIPLIER, encode(rootToken, multiplier, extra data)
        bytes memory message = abi.encode(DEPOSIT, abi.encode(rootToken, multiplier));
        _sendMessageToChild(message);
    }
...
abstract contract FxBaseRootTunnel {
    ...
    function _initialize(address _checkpointManager, address _fxRoot) internal {
        checkpointManager = ICheckpointManager(_checkpointManager);
        fxRoot = IFxStateSender(_fxRoot);
    }
