...
// SPDX-License-Identifier: GNU-3 AND AGPL-3.0-or-later
pragma solidity >=0.4.23 >=0.5.12 >=0.6.12;
...
////// lib/ds-pause/lib/ds-note/src/note.sol
...
contract Dog {
    ...
    VatLike_3 immutable public vat;  // CDP Engine
    ...
    // --- Administration ---
    function file(bytes32 what, address data) external auth {
        if (what == "vow") vow = VowLike_2(data);
    ...
    function file(bytes32 ilk, bytes32 what, address clip) external auth {
        if (what == "clip") {
            require(ilk == ClipperLike_1(clip).ilk(), "Dog/file-ilk-neq-clip.ilk");
            ilks[ilk].clip = clip;
        } else revert("Dog/file-unrecognized-param");
    ...
    function bark(bytes32 ilk, address urn, address kpr) external returns (uint256 id) {
        ...
        {   // Avoid stack too deep
            ...
            id = ClipperLike_1(milk.clip).kick({
                tab: tab,
                lot: dink,
                usr: urn,
                kpr: kpr
...
contract End {
    ...
    DogLike_2  public dog;
    VowLike_3  public vow;   // Debt Engine
    PotLike  public pot;
    ...
    function file(bytes32 what, address data) external auth {
        ...
        if (what == "vat")  vat = VatLike_4(data);
        else if (what == "cat")   cat = CatLike_1(data);
        else if (what == "dog")   dog = DogLike_2(data);
        else if (what == "vow")   vow = VowLike_3(data);
...
contract Flopper {
    ...
    function dent(uint id, uint lot, uint bid) external {
        ...
        if (msg.sender != bids[id].guy) {
            ...
            if (bids[id].tic == 0) {
                uint Ash = VowLike_4(bids[id].guy).Ash();
                VowLike_4(bids[id].guy).kiss(min(bid, Ash));
            }
...
contract Spotter {
    ...
    function file(bytes32 ilk, bytes32 what, address pip_) external auth {
        ...
        if (what == "pip") ilks[ilk].pip = PipLike_3(pip_);
...
contract Vow {
    ...
    // --- Data ---
    VatLike_12 public vat;        // CDP Engine
...
contract ESM {
    GemLike_4 public immutable gem;   // collateral (MKR token)
    EndLike_1 public immutable end;   // cage module
