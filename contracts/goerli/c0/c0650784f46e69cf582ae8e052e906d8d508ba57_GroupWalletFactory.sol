...
contract GroupWalletFactory {
  ...
  //  }
    ...
    function createProxy(address masterCopy, bytes memory data) public payable returns (Proxy proxy)
    {
        proxy = new Proxy(masterCopy);
        //if (msg.value>0) require(address(proxy).send(msg.value),"Sending ether to TokenProxy failed.");
        ...
        if (data.length > 0)
            ...
            assembly {
                ...
                let success := delegatecall(gas, proxy, add(data, 0x20), mload(data), 0, 0)
                returndatacopy(0, 0, returndatasize())
                if eq(success, 0) { revert(0, returndatasize()) }
                return(0, returndatasize())
