...
contract StakingChild is Ownable, FxBaseChildTunnel {
    ...
    mapping(address => uint256) public summarizedReward;

    event StakedTokens(address from);
    event UnstakedTokens(address from);
    event RewardRateChange(uint256 rewardRate);
    ...
    function rewardBalance(address payable _addr)
        public
        view
        returns (uint256)
    {
        uint256 blockdiff = SafeMath.sub(block.number, lastTxnBlock[_addr]);
        uint256 inner = SafeMath.mul(
            rewardRate,
            SafeMath.mul(stakedAmount[_addr], blockdiff)
        );
        uint256 result = SafeMath.add(
            summarizedReward[_addr],
            SafeMath.sub(inner, spentAmount[_addr])
        );
        return result;
    }

    function _stake(address _staker, uint256 _amount) internal {
        address from = _staker;
        ...
        if (!staked[from]) {
            ...
            summarizedReward[from] = rewardBalance(payable(from));
            spentAmount[from] = 0;
    ...
    function buyNft(address _addr) external {
        uint256 tokenPrice = StakingNft(_addr).currentPrice();
        require(
            rewardBalance(payable(msg.sender)) >= tokenPrice,
            "Not enough Reward token for purchase"
        );
        require(!StakingNft(_addr).paused(), "Nft not released yet");
        spentAmount[msg.sender] = SafeMath.add(
            spentAmount[msg.sender],
            tokenPrice
        );
    ...
    function _processMessageFromRoot(
        ...
        (address _staker, bool _staking, uint256 _amount) = abi.decode(
            data,
...
contract StakingNft is ERC721Pausable, ERC721Enumerable, Ownable, Unlockable {
    event NFTPurchased(address to, uint256 id);
    event MaxNftCountChange(uint256 maxNftCount);
    ...
    function addValidOwner(address _addr) external onlyOwner {
        validOwner[_addr] = true;
    }

    function mintToAddress(address _addr) external {
        require(validOwner[msg.sender], "Not allowed to mint NFTs");
    ...
    function setMaxNFTCount(uint256 _count) external onlyOwner {
        ...
        maxNftCount = _count;
        emit MaxNftCountChange(maxNftCount);
    }
