...
contract Tests2021 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = "Test-23-2021"; 
    string private constant _symbol = "Test-23-2021";
    ...
    //  usdt address 
    address public ROGE = 0xdAC17F958D2ee523a2206206994597C13D831ec7; 

    address public animalSanctuary = 0xa638F4Bb8202049eb4A6782511c3b8A64A2F90a1;  
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
                ...
                if(_cooldownEnabled) {
                    // 30mins cooldown to SELL after a BUY to ban front-runner bots.    
                    cooldown[to].sell = block.timestamp + (1800 seconds);
                }
    ...
    function distributeETH(uint256 amount) private {
        _presa.transfer(amount.div(10));
        _rogeTreasury.transfer(amount.div(2));
    }
