...
contract MerkleVesting is IMerkleVesting, Ownable {
    ...
    function getCohort(uint256 cohortId) external view returns (bytes32, uint256) {
        return (cohorts[cohortId].merkleRoot, cohorts[cohortId].distributionEnd);
    }

    function isClaimed(uint256 cohortID, uint256 index) public view returns (bool) {
        uint256 claimedWordIndex = index / 256;
        uint256 claimedBitIndex = index % 256;
        uint256 claimedWord = cohorts[cohortID].claimedBitMap[claimedWordIndex];
        uint256 mask = (1 << claimedBitIndex);
        return claimedWord & mask == mask;
    }

    function _setClaimed(uint256 cohortID, uint256 index) private {
        uint256 claimedWordIndex = index / 256;
        uint256 claimedBitIndex = index % 256;
        cohorts[cohortID].claimedBitMap[claimedWordIndex] =
            cohorts[cohortID].claimedBitMap[claimedWordIndex] |
            (1 << claimedBitIndex);
    }

    function addCohort(bytes32 merkleRoot, uint256 distributionDuration) external onlyOwner {
        uint256 cohortId = cohorts.length;
        cohorts.push();
        cohorts[cohortId].merkleRoot = merkleRoot;
        cohorts[cohortId].distributionEnd = block.timestamp + distributionDuration;
    }
    ...
    function claim(
        ...
        uint256 distributionEndLocal = cohorts[cohortID].distributionEnd;
        if (block.timestamp > distributionEndLocal) revert DistributionEnded(block.timestamp, distributionEndLocal);
        if (isClaimed(cohortID, index)) revert DropClaimed();

        // Verify the merkle proof.
        bytes32 node = keccak256(abi.encodePacked(index, account, amount));
        if (!MerkleProof.verify(merkleProof, cohorts[cohortID].merkleRoot, node)) revert InvalidProof();

        // Mark it claimed and send the token.
        _setClaimed(cohortID, index);
    ...
    function withdraw(address recipient) external onlyOwner {
        uint256 distributionEndLocal = cohorts[cohorts.length].distributionEnd;
...
interface IMerkleVesting {
    ...
    struct Cohort {
        ...
        // Stores the individual claim statuses.
        mapping(uint256 => uint256) claimedBitMap;
    }

    // Returns the merkleRoot and the distributionEnd parameters of a specific cohort.
    function getCohort(uint256 cohortId) external view returns (bytes32, uint256);
    ...
    function isClaimed(uint256 cohortID, uint256 index) external view returns (bool);

    // Allows the owner to add a new cohort.
    function addCohort(bytes32 merkleRoot, uint256 distributionDuration) external;
