...
contract mLoot is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
     ...
     for(uint i = 0; i < 10; i++){
        ...
        string memory output = string(abi.encodePacked(toString(tokenId), parts[0], parts[1], parts[2], parts[3], parts[4], parts[5], parts[6], parts[7], parts[8]));
...
//        output = string(abi.encodePacked('data:application/json;', json));
    ...
    function claim(uint numberOfTokens) public nonReentrant {
