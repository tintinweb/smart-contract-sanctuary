...
contract Property {
     ...
     * @param eoa Address of the owner
     * @param _lot Official identifier (lot, plot) of the property.
     * @param _location Complete address of the property.
     */
    constructor(address payable eoa, string memory _lot, string memory _location){
        location = _location;
        owner = eoa;  
        lot = _lot;
...
contract PropertyCreator {
    ...
    */
    function createProperty(address owner, string memory lot, string memory location) public returns (address propertyAddress){
        require(creator == msg.sender, "Only the creator of this contract can create new properties!");
        Property p = new Property(payable(owner), lot, location);
        properties.push(p);
