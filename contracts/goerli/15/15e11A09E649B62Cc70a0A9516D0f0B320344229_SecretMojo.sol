...
contract SecretMojo is ERC721, Ownable {
    ...
    enum State {PENDING, AVAILABLE, PAID, FREE, PAUSED, REVEALED}
    ...
    function tokenSecretA(uint256 tokenId) public view virtual returns (uint256) {
        require(_exists(tokenId), "SecretMojo: URI query for nonexistent token");
        require(state == State.REVEALED, "SecretMojo: not in Revealed State");

        return (tokenId + secretAShifter) % secretARange;
    }
    function tokenSecretB(uint256 tokenId) public view virtual returns (uint256) {
        require(_exists(tokenId), "SecretMojo: URI query for nonexistent token");
        require(state == State.REVEALED, "SecretMojo: not in Revealed State");

        return (tokenId + secretBShifter) % secretBRange;
    }
