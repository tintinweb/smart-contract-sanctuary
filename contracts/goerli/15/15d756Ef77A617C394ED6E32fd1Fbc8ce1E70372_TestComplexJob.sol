...
contract TestComplexJob {
  ...
  modifier validateAndPayKeeper(address _keeper) {
    // keeper should have 0.1KP3R bonded
    if (!IKeep3rV1(keep3r).isBondedKeeper(_keeper, keep3r, 0.1 ether, 0, 0)) revert NoKeeper();
    _;
    // payment is fixed 0.1KP3R
    IKeep3rV1(keep3r).receipt(keep3r, _keeper, 0.1 ether);
  }
}
