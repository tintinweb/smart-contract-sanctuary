...
contract MoraStaking{
    ...
    uint private termofacontract = (300 * 5); // 5 Months
  ...
  function Stake(uint256 _amount) external returns(bool success) {
    ...
    if ( _amount >= 2000 * (10**18) && _amount < 7000 * (10**18) ) { _rewardRate = 580000;} // Multiply 10**4 - APY 50 %  - Hourly 0.0058
    if ( _amount >= 7000 * (10**18) && _amount < 16000 * (10**18) ) { _rewardRate = 810000;} // Multiply 10**4 - APY 70 %  - Hourly 0.0080
    if ( _amount >= 16000 * (10**18)) { _rewardRate = 1040000;}                              // Multiply 10**4 - APY 90 %  - Hourly 0.0104
    ...
    totalActiveStakes += 1;
    require(block.timestamp < (deployDate + termofacontract) - 60,"out-of-date"); // make non-stakable 6 hours before from end time. //21600
  ...
  function UnstakeAndClaim(uint _stakeID) external returns (bool result) {
    uint _stakePeriodInHour = (block.timestamp - stakeBoxs[_stakeID].stakeDate ) / 60; // Stake Period in Hour ****
    ...
    require(_stakePeriodInHour >= 2,"cant-unstake-before-24-hours"); //Unstake not permitted in first 24 hours ****
    ...
    require(block.timestamp > (deployDate + termofacontract) + 144,"out-of-date"); // Burnable after 24 hours from end time. //86400 ****
  ...
  function TotalActiveStakeAmount() external view returns (uint256 result) {
    ...
    return ((deployDate + termofacontract) - block.timestamp) / 60; // in hours
  }
