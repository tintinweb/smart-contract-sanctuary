...
contract DreamBeats is Ownable, ERC1155, ReentrancyGuard {
    ...
    function isOriginalLp(uint256 tokenID) public pure returns (bool) {
        ...
        // imagine doing over 255 of these clips we would die
        return (tokenID < 256);
    }
    ...
    function adjustBondingCurve(uint256 a, uint256 b, uint256 c, uint256 sigdigits) external onlyOwner {
    ...
    function dropLP(uint[] memory lpList) external onlyOwner {
        for (uint i=0; i<lpList.length; i++) {
            _lpDropped[lpList[i]] = true;
        }
    }

    function dropLPAndMint(uint[] memory lpList) external onlyOwner {
        for (uint i=0; i<lpList.length; i++) {
    ...
    function setBitShift(uint256 b) external onlyOwner {
        BITSHIFT_AMOUNT = b;
