    ...
    FxBaseRootTunnel
{
    ...
    event TransferToPolygon(address indexed owner, uint256 indexed tokenId, string indexed tokenURI);
    event ReceivedFromPolygon(address indexed owner, uint256 indexed tokenId, string indexed tokenURI);
    ...
    function transferToPolygon(uint256 tokenId) external {
        ...
        string memory tokenURI_ = _stripBaseURI(tokenId);
        _sendMessageToChild(abi.encode(_msgSender(), tokenId, tokenURI_));
        _burn(tokenId);
        emit TransferToPolygon(_msgSender(), tokenId, tokenURI_);
    }
    ...
    function _processMessageFromChild(bytes memory message) virtual internal override {
        (address owner, uint256 tokenId, string memory tokenURI_) = abi.decode(message, (address, uint256, string));
        _mint(owner, tokenId);
        _setTokenURI(tokenId, tokenURI_);
        emit ReceivedFromPolygon(owner, tokenId, tokenURI_);
    }
