...
returns(uint256 level, uint256 rewards, uint256 timestamp, uint256 inventory){
  ...
  rewards = camp.baseRewards + (_sector.mul(250).div(100));
  rewards = rewards.mul(1 ether);
 
  if(_useItem){
         _attackPoints = _inventory == 1 ? _attackPoints.mul(2)   : _attackPoints;
         _healthPoints = _inventory == 2 ? _healthPoints.mul(2)   : _healthPoints; 
          rewards      = _inventory == 3 ?  rewards.mul(2)        : rewards;
          level        = _inventory == 4 ? _level.add(2)          : _level;
         _healthPoints = _inventory == 5 ? _healthPoints.add(300) : _healthPoints; 
         _attackPoints = _inventory == 6 ? _attackPoints.mul(3)   : _attackPoints;
  ...
  uint256 attackTime = creatureHealth.div(_attackPoints);
  
  attackTime = attackTime > 0 ? attackTime.mul(TIME_CONSTANT) : 0;
  
  timestamp = REGEN_TIME.div(_healthPoints).add(block.timestamp).add(attackTime);
