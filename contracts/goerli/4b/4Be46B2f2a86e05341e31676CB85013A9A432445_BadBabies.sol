...
contract BadBabies is ERC721, Ownable{
    ...
    constructor (string memory uri_, uint256 whitelistDate_, uint256 publicSaleDate_) ERC721("Bad Babies", "BAD")  {
        _uri = uri_;
    ...
    */    
    ...
    function mintBatch(uint256 amount, bytes memory signature) external payable {
        require(recov(keccak256(abi.encodePacked(msg.sender)), signature) == _authorizer, "Bad Babies: Invalid whitelist singature");
        require(msg.value >= (_whitelistPrice*amount), "Bad Babies: msg.value less than price");
        require(_whitelistDate <= block.timestamp, "Bad Babies: Mintin not allowed");
        require(_publicSaleDate > block.timestamp, "Bad Babies: Only public sale");
        for (uint256 i = 0; i < amount; i++) {
            mint_(msg.sender);
