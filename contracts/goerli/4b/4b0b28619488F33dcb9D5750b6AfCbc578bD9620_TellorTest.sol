...
contract TellorStake is TellorTransfer {
    ...
    function beginDispute(
        ...
        disputeEndTime = block.timestamp + 60 * 15;
        emit NewDispute(uint256(1), _requestId, _timestamp, msg.sender);
    }
    ...
    function vote(uint256 _disputeId, bool _supportsDispute) external {
        int256 voteWeight = 1e18;
        voteCount++;
        if (_supportsDispute) {
            voteResults = voteResults + voteWeight;
        } else {
            voteResults = voteResults - voteWeight;
        }
        // require(_disputeId <= uints[_DISPUTE_COUNT], "dispute does not exist");
...
contract TellorGetters is TellorStorage, TellorVariables, Utilities {
        ...
        )
    {
        // Dispute storage disp = disputesById[_disputeId];
        return (
            keccak256(abi.encode(disputeSender)),
            false,
            false,
            false,
            disputeSender,
            ...
            [
                ...
                disputeEndTime, // disp.disputeUintVars[_MIN_EXECUTION_DATE],
                voteCount, //disp.disputeUintVars[_NUM_OF_VOTES],
                block.number, //disp.disputeUintVars[_BLOCK_NUMBER],
                4, //disp.disputeUintVars[_MINER_SLOT],
                0, // disp.disputeUintVars[keccak256("quorum")],
                300 * 1e18 // disp.disputeUintVars[_FEE]
            ],
            voteResults
        );
    }
...
contract Extension is TellorGetters {
    ...
    function tallyVotes(uint256 _disputeId) external {
        ...
        emit DisputeVoteTallied(
            _disputeId,
            1000, // _tally,
            msg.sender, // disp.reportedMiner,
            msg.sender, //disp.reportingParty,
            true //disp.disputeVotePassed
        );
    }
