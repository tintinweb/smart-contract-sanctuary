    ...
    IGame
{
    ...
    //gId => rId => reserve
    mapping(address => mapping(uint8 => mapping(string => bool)))
    ...
    event ExpireGame(uint8 indexed gId, string rId);
    ...
    function initialize(address _defaultTokenAddress, address _vault)
        public
        initializer
    {
        // OwnableUpgradeable
    ...
    function resolveGame(
        ...
        // require(game.gamePaid == false, "cashInOut:already paid");
        ...
        for (uint128 i = 0; i < _players.length; i++) {
            ...
            vault.subGameBalance(player, reward, _fee);
        }

        game.gameState = GameState.ResolvedGame;

        emit ResolveGame(_players, _amounts, _fee, _gId, _rId);
    }
    ...
    function gamesRewardMultiplier(uint8 _gId, uint128 _amount)
        internal
        view
        returns (uint128)
    {
