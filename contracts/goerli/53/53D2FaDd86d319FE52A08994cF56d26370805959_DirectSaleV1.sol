...
contract DirectSaleV1 is ERC165, DirectSaleV1DeployI, ReentrancyGuard {
    ...
    event AssetsPurchased(address indexed buyer, uint256 amount, uint256 netWeiPerAsset, uint256 paymentAmountWei, uint256 eurRate, string taxRegionIdentifier);
    ...
    requireStarted
    {
        _buyOnChain(_recipient, _assetCount,_taxRegionIdentifier, _acceptTerms, _acceptanceText, false);
    }
    ...
    function buyApiReserve(address _recipient, uint256 _assetCount, string memory _taxRegionIdentifier, bool _acceptTerms, string memory _acceptanceText,
                           uint256 signatureNonce, uint256 expirationTimestamp, bytes memory _signature)
    public payable
    requireActive
    requireStarted
    {
        require(signatureNonceUsed[signatureNonce] == false, "Signature nonce has been used before.");
        require(expirationTimestamp > block.timestamp, "Signature is expired.");
        bytes32 data = keccak256(abi.encodePacked(address(this), block.chainid, this.buyApiReserve.selector, signatureNonce, expirationTimestamp, _assetCount, msg.sender));
        ...
        signatureNonceUsed[signatureNonce] = true;
        // Now that we checked that the signature is correct, do the actual purchase.
        _buyOnChain(_recipient, _assetCount,_taxRegionIdentifier, _acceptTerms, _acceptanceText, true);
    }
    ...
    requireStarted
    {
        ...
        _transferAssets(_recipient, _assetCount, 0, 0, _taxRegionIdentifier);
    }

    function _buyOnChain(address _recipient, uint256 _assetCount, string memory _taxRegionIdentifier, bool _acceptTerms, string memory _acceptanceText, bool canUseApiReserve)
    internal
    {
        ...
        _transferAssets(_recipient, _assetCount, netWeiPerAsset, payAmountWei, _taxRegionIdentifier);
    ...
    function _transferAssets(address _recipient, uint256 _amount, uint256 _netWeiPerAsset, uint256 _payAmountWei, string memory _taxRegionIdentifier)
    internal
    {
        emit AssetsPurchased(msg.sender, _amount, _netWeiPerAsset, _payAmountWei, eurRate(), _taxRegionIdentifier);
