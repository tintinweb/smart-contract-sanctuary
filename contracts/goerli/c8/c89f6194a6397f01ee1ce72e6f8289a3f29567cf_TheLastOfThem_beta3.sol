...
// File contracts/TheLastOfThem_beta3.sol
...
contract TheLastOfThem_beta3 is
    IERC165,
    IERC721,
    IERC721Metadata,
    IERC2981,
    ...
    ReentrancyGuard
{
    ...
    constructor() Sale() ReentrancyGuard() {
        ...
        _name = "TheLastOfThem_beta3";

        _lastTokenId = 0;

        setPricePresale(0.0004 ether);

        setPriceSale(0.0005 ether);

        setMintCapPresale(80);
        ...
        setPresaleListCap(500);

        setUnrevealedUri(
            "ipfs://QmWuy2r585MznVwvH7cEEk6Wssbm1zwHXrSfkrRQ6taXFQ"
        );
        ...
        returns (string memory)
    {
        ...
        if (!_revealed) {
            require(bytes(_unrevealedUri).length > 0, "Unrevealed URI not set");
    ...
    function setUnrevealedUri(string memory _val) public onlyOwner {
        _unrevealedUri = _val;
    }

    function unrevealedUri() public view virtual returns (string memory) {
        return string(abi.encodePacked(_unrevealedUri));
    }
