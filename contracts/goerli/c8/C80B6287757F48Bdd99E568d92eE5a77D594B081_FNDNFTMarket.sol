...
{
  /**
   * @dev This is used to determine if primary sale fees may apply.
   */
  function _isCreator(
    address nftContract,
    uint256 tokenId,
    ...
    try IFNDNFT721(nftContract).getTokenCreatorPaymentAddress{ gas: READ_ONLY_GAS_LIMIT }(tokenId) returns (
      ...
      if (tokenCreatorPaymentAddress == creator) {
    ...
    try IHasSecondarySaleFees(nftContract).getFeeRecipients{ gas: READ_ONLY_GAS_LIMIT }(tokenId) returns (
      ...
      uint256 maxIndex = recipients.length;
      if (maxIndex > MAX_CREATOR_INDEX) {
        maxIndex = MAX_CREATOR_INDEX;
      }
      for (uint256 i = 0; i < maxIndex; i++) {
        if (recipients[i] == creator) {
          return true;
        }
      }
    } catch // solhint-disable-next-line no-empty-blocks
   ...
   */
  function _getCreatorPaymentAddress(address nftContract, uint256 tokenId)
    internal
    view
    returns (address payable[] memory recipients, uint256[] memory splitPerRecipientInBasisPoints)
  {
    ...
    try IHasSecondarySaleFees(nftContract).getFeeRecipients{ gas: READ_ONLY_GAS_LIMIT }(tokenId) returns (
      ...
      if (_recipients.length > 0) {
        ...
        if (_recipients[0] != address(0)) {
          recipients = new address payable[](1);
          recipients[0] = _recipients[0];
          return (recipients, splitPerRecipientInBasisPoints);
        }
      }
    } catch // solhint-disable-next-line no-empty-blocks
  ...
  SendValueWithFallbackWithdraw
{
  ...
  function getIsPrimary(address nftContract, uint256 tokenId) public view returns (bool isPrimary) {
    ...
    (isPrimary, ) = _getIsPrimary(nftContract, tokenId, seller);
  }
  ...
  function _getIsPrimary(
    ...
    uint256 tokenId,
    address seller
  ) private view returns (bool isPrimary, bool isCreator) {
    isCreator = _isCreator(nftContract, tokenId, seller);
    isPrimary = !nftContractToTokenIdToFirstSaleCompleted[nftContract][tokenId] && isCreator;
  }
  ...
  {
    (creatorRecipients, creatorShares) = _getCreatorPaymentAddress(nftContract, tokenId);

    (bool isPrimary, bool isCreator) = _getIsPrimary(nftContract, tokenId, seller);
