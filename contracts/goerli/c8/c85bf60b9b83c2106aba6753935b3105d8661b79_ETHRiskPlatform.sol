...
/**
    Admin can change range low risk , 
...
contract ETHRiskPlatform is Ownable,ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor(address _wolfy) public{
      WOLFY = IERC20(_wolfy);
    }
    ...
    // uint256 public netSNP;
    // uint256 public netDefi;
    ...
    address[] _lowRiskETHUsers;
    address[] _highRiskETHUsers;
    ...
    function stakeEth(bool _isLowRisk) public payable {
     ...
     require(WOLFY.balanceOf(msg.sender) >= wolfyLimit, "You must possess 10K WOLFY Tokens to Participate");
      ...
      {
          stakeAmount = msg.value - ((msg.value.mul(liquidityReward)).div(100)).div(10);
      }
      else
      {
     ...
     if(_isLowRisk){
      require(depositLowRiskETH.add(msg.value) <= (totalLiquidityLowRsik + liquidityDetailsOwner._etherLowRisk).mul(6), "Target 6X reaching");   
      require(msg.value <= (totalLiquidityLowRsik + liquidityDetailsOwner._etherLowRisk).mul(6), "Exceeding low risk ETH limit");
      
      
      liquidityRewardCollectedLowRsik += ((msg.value.mul(liquidityReward)).div(100)).div(10);
     
      depositLowRiskETH += stakeAmount;
      

     } else {

      require(depositHighRiskETH.add(msg.value) <= (totalLiquidityHighRsik + liquidityDetailsOwner._etherHighRisk).mul(3), "Target 3X reaching");
      require(msg.value <= (totalLiquidityHighRsik + liquidityDetailsOwner._etherHighRisk).mul(3), "Exceeding high risk ETH limit");
      
        liquidityRewardCollectedHighRsik += ((msg.value.mul(liquidityReward)).div(100)).div(10);
     
      
      depositHighRiskETH += stakeAmount;

     }

     _isLowRisk == true ? storeUsers(msg.sender, _lowRiskETHUsers): storeUsers(msg.sender, _highRiskETHUsers); 
     _ethBalanceRiskPool[msg.sender][_isLowRisk] = _ethBalanceRiskPool[msg.sender][_isLowRisk].add(stakeAmount);          //______| Record of ETH deposit |______
    ...
    {
        uint256 _etherLowRisk;
        uint256 _etherHighRisk;
        address _address;
        uint256 timestamp;
        bool isLowRisk;
        bool isWithdraw;
    ...
    function addEth(bool _isLowRisk) public payable { 
       // _ethBalanceRiskPool[owner()][_isLowRisk] = _ethBalanceRiskPool[msg.sender][_isLowRisk].add(msg.value); 
        
       // _ethBalanceRiskPool[msg.sender][_isLowRisk] = _ethBalanceRiskPool[msg.sender][_isLowRisk].add(msg.value);  
       // bool isRecordAdded = false;
        if (msg.sender != owner())
        {
            // for (uint256 i=0;i<LiquidityDetailsRecord.length;i++)
            // {
                
            //     LiquidityDetailsRecord.push(LiquidityDetails(msg.value,0,msg.sender,block.timestamp,_isLowRisk));
                
            //     if (LiquidityDetailsRecord[i]._address == msg.sender && _isLowRisk)
            //     {
            //         isRecordAdded = true;
            //         LiquidityDetailsRecord[i]._etherLowRisk += msg.value;
                    
            //     }
            //     else if (LiquidityDetailsRecord[i]._address == msg.sender && _isLowRisk == false)
            //     {
            //         isRecordAdded = true;
            //         LiquidityDetailsRecord[i]._etherHighRisk += msg.value;
            //     }
            // }
            ...
            // {
                if (_isLowRisk)
                {
                    LiquidityDetailsRecord.push(LiquidityDetails(msg.value,0,msg.sender,block.timestamp,true,false,false));
                }
                else
                {
                    LiquidityDetailsRecord.push(LiquidityDetails(0,msg.value,msg.sender,block.timestamp,false,false,false));
                }
           
           // }
            if (_isLowRisk == true)
            {
                totalLiquidityLowRsik += msg.value;
                currentLowLiquidity[msg.sender] = currentLowLiquidity[msg.sender].add(msg.value);
                storeUsers(msg.sender,LiquidityLRUsers);
            
            }
            else
            {
                totalLiquidityHighRsik += msg.value;
                currentHighLiquidity[msg.sender] = currentHighLiquidity[msg.sender].add(msg.value);
        ...
        if (msg.sender == owner() && _isLowRisk)
        {
            ...
            liquidityDetailsOwner._etherLowRisk += msg.value;
        }
        else if (msg.sender == owner() && _isLowRisk == false)
        {
            liquidityDetailsOwner._address = owner();
            liquidityDetailsOwner._etherHighRisk += msg.value;
        }
    ...
    function getOwnerLiquidity(bool _isLowRisk)  external view returns(uint256)
    {
        if (_isLowRisk)
        {
            return liquidityDetailsOwner._etherLowRisk;
        }
        else
        {
            return  liquidityDetailsOwner._etherHighRisk;
        }
    }
    function getLiquiidtyProviderEthBalance(address _account,bool _isLowRisk) external view returns(uint256)
    {
    //   uint256 amount;
    //     for (uint i=0;i<LiquidityDetailsRecord.length;i++)
    //     {
            
    //         if (_isLowRisk && _account == LiquidityDetailsRecord[i]._address)
    //         {
    //           amount = LiquidityDetailsRecord[i]._etherLowRisk;
    //           break;
                
    //         }
    //         else if (_isLowRisk == false && _account == LiquidityDetailsRecord[i]._address)
    //         {
    //              amount = LiquidityDetailsRecord[i]._etherLowRisk;
    //              break;
    //         }
    ...
    function withdrawLiquidityETH(uint256 _ether, bool _isLowRisk) external nonReentrant
    {
        
         require(!poolStarted, "Cannot withdraw ETH when pool is running");
         if (_isLowRisk)
         {
              require(_ether <= currentLowLiquidity[msg.sender],"Insufficient Balance");
              currentLowLiquidity[msg.sender] -= _ether;
              totalLiquidityLowRsik -= _ether;
              LiquidityDetailsRecord.push(LiquidityDetails(_ether,0,msg.sender,block.timestamp,true,true,false));
               payable(msg.sender).transfer(_ether); 
               
         }
         else
         {
              require(_ether <= currentHighLiquidity[msg.sender],"Insufficient Balance");
              currentHighLiquidity[msg.sender] -= _ether;
              LiquidityDetailsRecord.push(LiquidityDetails(0,_ether,msg.sender,block.timestamp,false,true,false));
              totalLiquidityHighRsik -= _ether;
              payable(msg.sender).transfer(_ether); 
         }
    }
    function getTotalLiquidity(bool _isLowRisk) external view returns(uint256)
    {
        if (_isLowRisk == true)
        {
            return totalLiquidityLowRsik + liquidityDetailsOwner._etherLowRisk;
        }
        else
        {
            return totalLiquidityHighRsik + liquidityDetailsOwner._etherHighRisk;
        }
    }
    ...
    function checkAdminBalance(uint256 _factor, bool _champ) internal  // for deducting LP balance if admin dont have enough stake
    {
          ...
          // ETH : ETH increased by 12% of all stakers.
            for(uint i = 0 ; i < _lowRiskETHUsers.length; i++)
            {
             ...
             // Reduce from Admin low risk wallet
                totalWinLowRisk += _ethBalanceRiskPool[_lowRiskETHUsers[i]][true].mul(120).div(1000);
            }
        } 
        ...
        if(_factor >= targetHigh && _champ == true){
            ...
            for(uint i = 0 ; i < _highRiskETHUsers.length ; i++){
                ...
                totalWinHighRisk += _ethBalanceRiskPool[_highRiskETHUsers[i]][false].mul(300).div(1000);
            }
        } 
       
        if (totalWinHighRisk >= liquidityDetailsOwner._etherHighRisk)
        {
            
             uint256 clonetotalLiquidityHighRsik = totalLiquidityHighRsik;
            uint256 diffrenceTobePaidHighRisk = totalWinHighRisk - liquidityDetailsOwner._etherHighRisk;
            ...
            liquidityDetailsOwner._etherHighRisk = 0;
            delete tempRecordForloss;
            
        }
        if (totalWinLowRisk >= liquidityDetailsOwner._etherLowRisk)
        {
             uint256 diffrenceTobePaidLowRisk = totalWinLowRisk - liquidityDetailsOwner._etherLowRisk;
            ...
            for (uint256 i=0;i<tempRecordForloss.length;i++)
            {
             ...
             liquidityDetailsOwner._etherLowRisk = 0;
            delete tempRecordForloss;
        }
    ...
    function rewardManager(uint256 _factor, bool _champ) internal {
       ...
       if(_factor >= targetLow && _champ == true){
          ...
          // ETH : ETH increased by 12% of all stakers.
            ...
            //_ethBalanceRiskPool[owner()][true] -= _ethBalanceRiskPool[_lowRiskETHUsers[i]][true].mul(targetLow.mul(10)).div(1000);
                if (liquidityDetailsOwner._etherLowRisk >= _ethBalanceRiskPool[_lowRiskETHUsers[i]][true].mul(120).div(1000))
                {
                    liquidityDetailsOwner._etherLowRisk -= _ethBalanceRiskPool[_lowRiskETHUsers[i]][true].mul(120).div(1000);
                    ...
                    // // address + isLowRisk + count ==> bool
                    previousResults[_lowRiskETHUsers[i]][true][runCount] = true;
                }
                else
                {
                    liquidityDetailsOwner._etherLowRisk = 0;
        ...
        else 
        {
            ...
            for(uint i = 0 ; i < _lowRiskETHUsers.length ; i++){
                ...
                liquidityDetailsOwner._etherLowRisk += _ethBalanceRiskPool[_lowRiskETHUsers[i]][true].mul(150).div(1000);
                ...
                //// address + isLowRisk + count ==> bool
                previousResults[_lowRiskETHUsers[i]][true][runCount] = false;
        ...
        if(_factor >= targetHigh && _champ == true){
            ...
            for(uint i = 0 ; i < _highRiskETHUsers.length ; i++){
                ...
                if (liquidityDetailsOwner._etherHighRisk >= _ethBalanceRiskPool[_highRiskETHUsers[i]][false].mul(300).div(1000))
                {
                     
                    liquidityDetailsOwner._etherHighRisk -= _ethBalanceRiskPool[_highRiskETHUsers[i]][false].mul(300).div(1000);
                    ...
                    // address + isLowRisk + count ==> bool
                    previousResults[_highRiskETHUsers[i]][false][runCount] = true;
                }
                else
                {
                    liquidityDetailsOwner._etherHighRisk = 0;
        ...
        // factor < 5x | High Risk Losing
            ...
            for(uint i = 0 ; i < _highRiskETHUsers.length ; i++){
                ...
                // ########### Increase for Admin wallet #############
              liquidityDetailsOwner._etherHighRisk += _ethBalanceRiskPool[_highRiskETHUsers[i]][false].mul(350).div(1000);
              ...
              _ethBalanceRiskPool[_highRiskETHUsers[i]][false] -= _ethBalanceRiskPool[_highRiskETHUsers[i]][false].mul(350).div(1000);
                // address + isLowRisk + count ==> bool
               previousResults[_highRiskETHUsers[i]][false][runCount] = false;
    ...
    function startPool(uint256 _player1, uint256 _player2) public onlyOwner { 
        ...
        require( totalLiquidityLowRsik + liquidityDetailsOwner._etherLowRisk > 0 ," LowRisk : Please add ETH liquidity");
        require( totalLiquidityHighRsik + liquidityDetailsOwner._etherHighRisk > 0 ," HighRisk : Please add ETH liquidity");


        player1 = _player1;         // defiPulse = _defiPulse;
        player2 = _player2;         // snp500 = _snp500;
    ...
    function getLosses(bool _isLowRisk) public view returns (uint256){
     ...
     //______| Total ETH liquidity low Risk |_______ // As per new change this function will return stake ethher balance by user for bet
    ...
    function withdrawETH(uint256 _ether, bool _isLowRisk) public nonReentrant {
      require(!poolStarted, "Cannot withdraw ETH when pool is running");
    ...
    function  distributeLiquidityRewards(bool _isLowRisk) internal  // for distributin rewards to LP based on their fund added
    {
        ...
        for (uint i=0;i<LiquidityDetailsRecord.length;i++)
        {
            if(_isLowRisk && LiquidityDetailsRecord[i].isLowRisk == true)
            {
                ...
                //((LiquidityDetailsRecord[i]._etherLowRisk.mul(10**18)).mul(100*10**18)).div(totalLiquidityLowRsik.mul(10**18));
                uint256 rewardPerc = ((LiquidityDetailsRecord[i]._etherLowRisk.mul(10**18)).mul(100*10**18)).div(totalLiquidityLowRsik.mul(10**18));
                
                uint256 rewardAmount = ((liquidityRewardCollectedLowRsik.mul(10**18) ).mul(rewardPerc) )/(100 * 10**18);
                
                rewardPaidRecord[LiquidityDetailsRecord[i]._address].push(RewardPaid(rewardAmount,block.timestamp,false));
            ...
            else if(_isLowRisk == false && LiquidityDetailsRecord[i].isLowRisk == false)
            {
                ...
                uint256 rewardPerc = ((LiquidityDetailsRecord[i]._etherHighRisk.mul(10**18)).mul(100*10**18)).div(totalLiquidityHighRsik.mul(10**18));
                uint256 rewardAmount = ((liquidityRewardCollectedHighRsik.mul(10**18) ).mul(rewardPerc) )/(100 * 10**18);
    ...
    function WithdrawReward(uint256 amount) external nonReentrant
    {
        ...
        payable(msg.sender).transfer(amount.div(10**18)); 
        rewardPaidRecord[msg.sender].push(RewardPaid(amount,block.timestamp,true));
        
    }
    function contractEth() external view returns(uint256)
    {
