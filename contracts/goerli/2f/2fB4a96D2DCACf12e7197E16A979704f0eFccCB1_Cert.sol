...
contract Cert is ERC1155 {
    ...
    function getHash(
        ...
        return
            keccak256(abi.encode(account, amount, metaDataHash, metaDataUri));
    }
    ...
    function mint(
        ...
        require(_tokenIdExists[id] == false, "tokenId already used.");
        _metaDataUris[id] = metaDataUri;
    ...
    function mintBatch(
        ...
        bytes32[] memory metaDataHashs,
        string[] memory metaDataUris,
        uint256[] memory ids
    ) external {
        require(
            ids.length == metaDataUris.length,
            "ERC1155: ids and metaDataUris length mismatch"
        );
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            require(_tokenIdExists[ids[i]] == false, "tokenId already used.");
            _metaDataUris[ids[i]] = metaDataUris[i];
