 ...
 * Controller keeps track of a number of gDai contracts.
 ...
 * Controller allows users to borrow gDai against their Chai and Weth collateral.
 * Controller allows users to repay their gDai debt with gDai or with Dai.
 * Controller integrates with gDai contracts for minting gDai on borrowing, and burning gDai on repaying debt with gDai.
...
contract Controller is IController, Orchestrated(), Delegable(), DecimalMath {
    ...
    mapping(uint256 => IGDai) public override series;                 // GDai series, indexed by maturity
    ...
    mapping(bytes32 => mapping(uint256 => mapping(address => uint256))) public override debtGDai;  // Debt owed by each user, by series
    ...
    constructor (
        address treasury_,
        address[] memory gDais

    ) public {
        treasury = ITreasury(treasury_);
        vat = treasury.vat();
        pot = treasury.pot();
        for (uint256 i = 0; i < gDais.length; i += 1) {
            addSeries(gDais[i]);
        }
    }
    ...
    /// @dev Adds an gDai series to this Controller
    /// After deployment, ownership should be renounced, so that no more series can be added.
    /// @param gDaiContract Address of the gDai series to add.
    function addSeries(address gDaiContract) private {
        uint256 maturity = IGDai(gDaiContract).maturity();
        require(
            !containsSeries(maturity),
            "Controller: Series already added"
        );
        series[maturity] = IGDai(gDaiContract);
        seriesIterator.push(maturity);
    }

    /// @dev Dai equivalent of an gDai amount.
    /// After maturity, the Dai value of an gDai grows according to either the stability fee (for WETH collateral) or the Dai Saving Rate (for Chai collateral).
    /// @param collateral Valid collateral type
    /// @param maturity Maturity of an added series
    /// @param gDaiAmount Amount of gDai to convert.
    /// @return Dai equivalent of an gDai amount.
    function inDai(bytes32 collateral, uint256 maturity, uint256 gDaiAmount)
        public view override
        validCollateral(collateral)
        returns (uint256)
    {
        IGDai gDai = series[maturity];
        if (gDai.isMature()){
            if (collateral == WETH){
                return muld(gDaiAmount, gDai.rateGrowth());
            } else if (collateral == CHAI) {
                return muld(gDaiAmount, gDai.chiGrowth());
            }
        } else {
            return gDaiAmount;
        }
    ...
    /// @dev gDai equivalent of a Dai amount.
    /// After maturity, the gDai value of a Dai decreases according to either the stability fee (for WETH collateral) or the Dai Saving Rate (for Chai collateral).
    /// @param collateral Valid collateral type
    /// @param maturity Maturity of an added series
    /// @param daiAmount Amount of Dai to convert.
    /// @return gDai equivalent of a Dai amount.
    function inGDai(bytes32 collateral, uint256 maturity, uint256 daiAmount)
        public view override
        validCollateral(collateral)
        returns (uint256)
    {
        ...
        if (gDai.isMature()){
            if (collateral == WETH){
                return divd(daiAmount, gDai.rateGrowth());
            } else if (collateral == CHAI) {
                return divd(daiAmount, gDai.chiGrowth());
            }
        } else {
            return daiAmount;
        }
    ...
    function debtDai(bytes32 collateral, uint256 maturity, address user) public view override returns (uint256) {
        return inDai(collateral, maturity, debtGDai[collateral][maturity][user]);
    }
    ...
    function totalDebtDai(bytes32 collateral, address user) public view override returns (uint256) {
        ...
        for (uint256 i = 0; i < _seriesIterator.length; i += 1) {
            if (debtGDai[collateral][_seriesIterator[i]][user] > 0) {
    ...
    /// @dev Mint gDai for a given series for wallet `to` by increasing the user debt in Yield vault `from`
    ...
    /// @param to Wallet to put the gDai in.
    /// @param gDaiAmount Amount of gDai to borrow.
    //
    // posted[user](wad) >= (debtGDai[user](wad)) * amount (wad)) * collateralization (ray)
    //
    // us(from) --- gDai ---> to
    // debt++
    function borrow(bytes32 collateral, uint256 maturity, address from, address to, uint256 gDaiAmount)
        public override
        validCollateral(collateral)
        ...
        onlyLive
    {
        ...
        debtGDai[collateral][maturity][from] = debtGDai[collateral][maturity][from].add(gDaiAmount);
        ...
        gDai.mint(to, gDaiAmount);
        emit Borrowed(collateral, maturity, from, toInt256(gDaiAmount));
    }

    /// @dev Burns gDai from `from` wallet to repay debt in a Yield Vault.
    /// User debt is decreased for the given collateral and gDai series, in Yield vault `to`.
    /// `from` can delegate to other addresses to take gDai from him for the repayment.
    /// @param collateral Valid collateral type.
    /// @param maturity Maturity of an added series
    /// @param from Wallet providing the gDai for repayment.
    /// @param to Yield vault to repay debt for.
    /// @param gDaiAmount Amount of gDai to use for debt repayment.
    //
    //                                                  debt_nominal
    // debt_discounted = debt_nominal - repay_amount * ---------------
    //                                                  debt_now
    //
    // user(from) --- gDai ---> us(to)
    // debt--
    function repayGDai(bytes32 collateral, uint256 maturity, address from, address to, uint256 gDaiAmount)
        public override
        validCollateral(collateral)
        validSeries(maturity)
        onlyHolderOrDelegate(from, "Controller: Only Holder Or Delegate")
        ...
        returns (uint256)
    {
        uint256 toRepay = Math.min(gDaiAmount, debtGDai[collateral][maturity][to]);
        ...
        returns (uint256)
    {
        ...
        _repay(collateral, maturity, to, inGDai(collateral, maturity, toRepay));
        return toRepay;
    }
    ...
    //                                          principal + interest
    //    
    function _repay(bytes32 collateral, uint256 maturity, address user, uint256 gDaiAmount) internal {
        debtGDai[collateral][maturity][user] = debtGDai[collateral][maturity][user].sub(gDaiAmount);

        emit Borrowed(collateral, maturity, user, -toInt256(gDaiAmount));
    }
        ...
        returns (uint256, uint256)
    {
        ...
        for (uint256 i = 0; i < _seriesIterator.length; i += 1) {
            ...
            delete debtGDai[collateral][maturity][user];
        } // We don't expect hundreds of maturities per controller
