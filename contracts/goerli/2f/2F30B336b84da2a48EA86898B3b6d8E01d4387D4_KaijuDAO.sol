...
contract KaijuDAO is ERC721, OnePerWallet, Ownable{
    using Address for address;
    using Strings for uint256;
    
    uint16 public minted;
    uint256 public constant MINT_PRICE_SOS = 10;
    uint256 public MAX_TOKENS = 100000;

    uint16 public round;

    address[] public teamA_leaders;
    address[] public teamB_leaders;
    address[] public teamC_leaders;
    ...
    function mint() external onePerWallet(msg.sender){
    ...
    function _mint(address to, uint256 tokenId) internal override(ERC721, OnePerWallet) {
        require(MINT_PRICE_SOS<=openDAO.balanceOf(to));
        openDAO.transferFrom(to, address(this), MINT_PRICE_SOS);
    ...
    function setLeaders(address teamA, address teamB, address teamC) external onlyOwner {
        teamA_leaders.push(teamA);
        teamB_leaders.push(teamB);
        teamC_leaders.push(teamC);
        round++;
    }
