...
// File: contracts/JohnLiquidityMining.sol
...
/// @notice Stake JOHN-ETH Uniswap LP tokens for JOHN rewards
contract JohnLiquidityMining is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeMath for uint256;
    using TransferHelper for address;

    address public johnEth;
    address public john;
    ...
    event Stake(address indexed staker, uint256 johnEthIn);
    event Payout(address indexed staker, uint256 reward);
    event Withdraw(address indexed staker, uint256 johnEthOut);

    /// @dev Expects a JOHN-ETH LP token address & the JOHN reward token address
    function initialize(address _johnEth, address _john) public initializer {
        __Ownable_init();
    ...
    function deposit(
        ...
        require(
            IERC20(john).balanceOf(address(this)) == _totalRewards,
    ...
    function stake(uint256 johnEthIn) public virtual update nonReentrant {
        require(johnEthIn > 0, "LiquidityMining::stake: missing stake");
        require(
            block.timestamp >= startTime,
            "LiquidityMining::stake: staking isn't live yet"
        );
        require(
            IERC20(john).balanceOf(address(this)) > 0,
            "LiquidityMining::stake: no JOHN balance"
        );
        ...
        if (johnEthIn > 0) {
            johnEth.safeTransferFrom(msg.sender, address(this), johnEthIn);
        }
        ...
        _stake(johnEthIn, msg.sender);

        emit Stake(msg.sender, johnEthIn);
    }
    ...
    function withdraw()
        ...
        update
        nonReentrant
        returns (uint256 johnEthOut, uint256 reward)
    {
        totalStakers = totalStakers.sub(1);

        (johnEthOut, reward) = _applyReward(msg.sender);

        if (johnEthOut > 0) {
            johnEth.safeTransfer(msg.sender, johnEthOut);
        }

        if (reward > 0) {
            john.safeTransfer(msg.sender, reward);
        ...
        emit Withdraw(msg.sender, johnEthOut);
    }
        ...
        returns (uint256 reward)
    {
        ...
        (uint256 johnEthOut, uint256 _reward) = _applyReward(msg.sender);
        ...
        _stake(johnEthOut, msg.sender);

        emit Payout(msg.sender, _reward);
    }

    function _stake(uint256 johnEthIn, address account) private {
        uint256 addBackJohnEth;

        if (totalUserStake(account) > 0) {
            (uint256 johnEthOut, uint256 reward) = _applyReward(account);
            addBackJohnEth = johnEthOut;
            _userStakedJohnEth[account] = johnEthOut;
            _userAccumulated[account] = reward;
        }

        _userStakedJohnEth[account] = _userStakedJohnEth[account].add(
            johnEthIn
        );

        _userWeighted[account] = _totalWeight;

        _totalStakeJohnEth = _totalStakeJohnEth.add(johnEthIn);

        if (addBackJohnEth > 0) {
            _totalStakeJohnEth = _totalStakeJohnEth.add(addBackJohnEth);
        }
    }
        ...
        returns (uint256 johnEthOut, uint256 reward)
    {
        ...
        johnEthOut = _userStakedJohnEth[account];
        ...
        _totalStakeJohnEth = _totalStakeJohnEth.sub(johnEthOut);

        _userStakedJohnEth[account] = 0;
    ...
    function rescueTokens(
        ...
        if (tokenToRescue == johnEth) {
            require(
                amount <=
                    IERC20(johnEth).balanceOf(address(this)).sub(
                        _totalStakeJohnEth
                    ),
                "LiquidityMining::rescueTokens: that JohnEth belongs to stakers"
            );
        } else if (tokenToRescue == john) {
            if (totalStakers > 0) {
                require(
                    amount <=
                        IERC20(john).balanceOf(address(this)).sub(
                            totalRewards.sub(totalClaimedRewards)
                        ),
                    "LiquidityMining::rescueTokens: that JOHN belongs to stakers"
                );
            }
        }
