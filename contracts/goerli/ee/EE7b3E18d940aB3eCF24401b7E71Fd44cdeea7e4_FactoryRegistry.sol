...
contract FactoryRegistry is Context {
    ...
    /**
     * @dev Setter of 'logic_contract'
     */
    function setLogicContract(address _logicAddress) public onlyProxyAdmin returns (bool success) {
        logic_contract = _logicAddress;
        return true;
    }

    /**
     * @dev Getter of 'logic_contract'
     */
...
contract FactoryStorage {
  ...
  /**
  * @dev Setter of 'last_upgrade' 
  */
  function setLastUpgrade(address _newUpgrade) public onlyAdmin
   {
    last_upgrade = _newUpgrade;
  }
  /**
  ...
  */
  function addNFT(string memory _name, uint256 _amount, uint24 _fee) public onlyAdmin {
 ...
 /**
  * @dev Returns the Artist of collection
  */
...
contract NFTFactory is Initializable, ERC1155Upgradeable, AccessControlUpgradeable, IERC2981 {
    ...
    // MAINNET
    // address public constant MintableERC1155PredicateProxy = 0x2d641867411650cd05dB93B59964536b1ED5b1B7;
    // TESTNET 
    address public constant MintableERC1155PredicateProxy = 0x72d6066F486bd0052eefB9114B66ae40e0A6031a;

    address public factory_admin;

    string internal contract_uri;

    function initialize(string memory _metadata, string memory _contract_uri, address _factoryStorage) virtual public initializer{
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override (ERC1155Upgradeable, AccessControlUpgradeable, IERC2981) returns (bool) {
        return (
    ...
    */
    function setContractURI(string memory _contract_uri) public onlyAdmin returns (string memory) {
        contract_uri = _contract_uri;
        return contract_uri;
    }
    /**
    ...
    */
    function createNFT (string memory _name, uint256 _amount, uint24 _fee) virtual public onlyAdmin{
        _mint(factory_storage.getArtist(), factory_storage.getNFT_ID(), _amount, "");
        emit NewMint(factory_storage.getArtist(), factory_storage.getNFT_ID(), _amount, _name, _fee);
        factory_storage.addNFT(_name, _amount, _fee);
    }
    /**
    * @dev 2981 Royalties
    */
    function royaltyInfo(uint256 tokenId, uint256 _salePrice) virtual override external view returns (address _receiver, uint256 _royaltyAmount) {
        ...
        return (factory_storage.getRoyaltyRecipient(), (factory_storage.getNFTModel(tokenId).fee * 10 * _salePrice / 100000));
    ...
    */
    function mint(address account, uint256 id, uint256 amount, bytes calldata data)  external virtual onlyRole(PREDICATE_ROLE) {
        _mint(account, id, amount, data);
    }

    function mintBatch(address to, uint256[] calldata ids, uint256[] calldata amounts, bytes calldata data)  external virtual onlyRole(PREDICATE_ROLE) {
