...
@external
def __init__(_name: String[64], _symbol: String[32], _fungibleContract: address):
    """
...
def tokenURI(_tokenId: uint256) -> String[128]:
    ...
    if (self.newURI == ZERO_ADDRESS):
        return 'https://cloudflare-ipfs.com/ipfs/QmZnbvcMXed5FvDBp3MGfnB841Dyzy9npzVkTYjKZdW9Cp'
    else:
...
@external
def claimFromFT(_to: address) -> bool: # , _tokenId: uint256
    """
    ...
    assert self.fungibleContract.burn(10**18)

    # Check if a FungibleToken has been burned
    # TODO: we could rely on this as we call the burn function ourselves
    _winesSupply: uint256 = self.fungibleContract.totalSupply()
    _winesBurned: uint256 = 216 * 10**18 - _winesSupply #TODO define in fungible contract (or init param)
    assert _tokenId * 10**18 < _winesBurned
    ...
    self._addTokenTo(_to, _tokenId)
    log Transfer(ZERO_ADDRESS, _to, _tokenId)
    return True # TODO: return tokenIDs?
