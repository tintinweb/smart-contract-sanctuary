...
contract Request {
  ...
  /// @dev A `Request` is constructed around a `bytes memory` value containing
  /// a well-formed Witnet data request serialized using Protocol Buffers.
  /// However, we cannot verify its validity at this point. This implies that
  /// contracts using the WRB should not be considered trustless before a valid
  /// Proof-of-Inclusion has been posted for the requests. The hash of the
  /// request is computed in the constructor to guarantee consistency.
  /// Otherwise there could be a mismatch and a data request could be resolved
...
library BufferLib {
  ...
  modifier notOutOfBounds(uint32 index, uint256 length) {
    require(
      index < length,
      "Tried to read from a consumed Buffer (must rewind it first)"
    );
    _;
  }
   ...
   */
  function read(Buffer memory _buffer, uint32 _length)
    internal
    pure
    returns (bytes memory)
  {
    ...
    require(
      _buffer.cursor + _length <= _buffer.data.length,
      "Not enough bytes in buffer when reading"
    );
    ...
    if (_length != 0) {
      ...
      memcpy(destinationPointer, sourcePointer, uint256(_length));
  ...
  function next(Buffer memory _buffer)
    internal
    pure
    notOutOfBounds(_buffer.cursor, _buffer.data.length)
    returns (bytes1)
  {
  ...
  function seek(
    ...
    if (_relative) {
      require(
        _offset + _buffer.cursor > _offset,
        "Integer overflow when seeking"
      );
      _offset += _buffer.cursor;
    }
    // Make sure not to read out of the bounds of the original bytes
    require(
      _offset <= _buffer.data.length,
      "Not enough bytes in buffer when seeking"
    );
   ...
   */
  function seek(Buffer memory _buffer, uint32 _relativeOffset)
    internal
    pure
    returns (uint32)
  {
  ...
  function readUint16(Buffer memory _buffer)
    internal
    pure
    notOutOfBounds(_buffer.cursor + 1, _buffer.data.length)
    returns (uint16)
  {
    bytes memory bytesValue = _buffer.data;
    uint32 offset = _buffer.cursor;
  ...
  function readUint32(Buffer memory _buffer)
    internal
    pure
    notOutOfBounds(_buffer.cursor + 3, _buffer.data.length)
    returns (uint32)
  {
    bytes memory bytesValue = _buffer.data;
    uint32 offset = _buffer.cursor;
  ...
  function readUint64(Buffer memory _buffer)
    internal
    pure
    notOutOfBounds(_buffer.cursor + 7, _buffer.data.length)
    returns (uint64)
  {
    bytes memory bytesValue = _buffer.data;
    uint32 offset = _buffer.cursor;
  ...
  function readUint128(Buffer memory _buffer)
    internal
    pure
    notOutOfBounds(_buffer.cursor + 15, _buffer.data.length)
    returns (uint128)
  {
    bytes memory bytesValue = _buffer.data;
    uint32 offset = _buffer.cursor;
  ...
  function readUint256(Buffer memory _buffer)
    internal
    pure
    notOutOfBounds(_buffer.cursor + 31, _buffer.data.length)
    returns (uint256)
  {
    bytes memory bytesValue = _buffer.data;
    uint32 offset = _buffer.cursor;
    ...
    if (exponent >= 0) {
      ...
      result = int32(
        ((int256(uint256(int256(significand)) | 0x400) * 10000) /
          int256(1 << uint256(int256(-exponent)))) >> 10
      );
    }
...
library CBOR {
  ...
  function decodeBool(Value memory _cborValue) public pure returns (bool) {
    _cborValue.len = readLength(
      _cborValue.buffer,
      _cborValue.additionalInformation
    );
    require(
      _cborValue.majorType == 7,
      "Tried to read a `bool` value from a `CBOR.Value` with majorType != 7"
    );
    if (_cborValue.len == 20) {
      ...
      revert(
        "Tried to read `bool` from a `CBOR.Value` with len different than 20 or 21"
      );
    }
  }
   ...
   */
  function decodeBytes(Value memory _cborValue)
    public
    pure
    returns (bytes memory)
  {
    ...
    if (_cborValue.len == UINT32_MAX) {
      ...
      uint32 itemLength = uint32(
        readIndefiniteStringLength(_cborValue.buffer, _cborValue.majorType)
      );
      if (itemLength < UINT32_MAX) {
        bytesData = abi.encodePacked(
          bytesData,
          _cborValue.buffer.read(itemLength)
        );
        itemLength = uint32(
          readIndefiniteStringLength(_cborValue.buffer, _cborValue.majorType)
        );
  ...
  function decodeFixed16(Value memory _cborValue) public pure returns (int32) {
    require(
      _cborValue.majorType == 7,
      "Tried to read a `fixed` value from a `CBOR.Value` with majorType != 7"
    );
    require(
      _cborValue.additionalInformation == 25,
      "Tried to read `fixed16` from a `CBOR.Value` with additionalInformation != 25"
    );
   ...
   * @return The value represented by the input, as an `int128[]` value.
   */
  function decodeFixed16Array(Value memory _cborValue)
    external
    pure
    returns (int32[] memory)
  {
    require(
      _cborValue.majorType == 4,
      "Tried to read `int128[]` from a `CBOR.Value` with majorType != 4"
    );
  ...
  function decodeInt128(Value memory _cborValue) public pure returns (int128) {
    ...
    revert(
      "Tried to read `int128` from a `CBOR.Value` with majorType not 0 or 1"
    );
  }
   ...
   * @return The value represented by the input, as an `int128[]` value.
   */
  function decodeInt128Array(Value memory _cborValue)
    external
    pure
   ...
   */
  function decodeString(Value memory _cborValue)
    public
    pure
    returns (string memory)
  {
    ...
    if (_cborValue.len == UINT64_MAX) {
      ...
      bool done;
      while (!done) {
        uint64 itemLength = readIndefiniteStringLength(
          _cborValue.buffer,
          _cborValue.majorType
        );
        if (itemLength < UINT64_MAX) {
          textData = abi.encodePacked(
            textData,
            readText(_cborValue.buffer, itemLength / 4)
          );
        } else {
          done = true;
        }
   ...
   */
  function decodeStringArray(Value memory _cborValue)
    external
    pure
    returns (string[] memory)
  {
    require(
      _cborValue.majorType == 4,
      "Tried to read `string[]` from a `CBOR.Value` with majorType != 4"
    );
  ...
  function decodeUint64(Value memory _cborValue) public pure returns (uint64) {
    require(
      _cborValue.majorType == 0,
      "Tried to read `uint64` from a `CBOR.Value` with majorType != 0"
    );
   ...
   * @return The value represented by the input, as an `uint64[]` value.
   */
  function decodeUint64Array(Value memory _cborValue)
    external
    pure
    returns (uint64[] memory)
  {
    require(
      _cborValue.majorType == 4,
      "Tried to read `uint64[]` from a `CBOR.Value` with majorType != 4"
    );
   ...
   * @return A `CBOR.Value` instance containing a partially decoded value.
   */
  function valueFromBuffer(BufferLib.Buffer memory _buffer)
    public
    pure
    returns (Value memory)
  {
  ...
  function readLength(
    BufferLib.Buffer memory _buffer,
    uint8 additionalInformation
    ...
    revert(
      "Invalid length encoding (non-existent additionalInformation value)"
    );
  }
  ...
  function readIndefiniteStringLength(
    ...
    require(
      length < UINT64_MAX && (initialByte >> 5) == majorType,
  ...
  // solium-disable-next-line security/no-assign-params
  function readText(BufferLib.Buffer memory _buffer, uint64 _length)
    private
    pure
    returns (bytes memory)
  {
    ...
    for (uint64 index = 0; index < _length; index++) {
      ...
      if (value & 0x80 != 0) {
        if (value < 0xe0) {
          value = ((value & 0x1f) << 6) | (_buffer.readUint8() & 0x3f);
          _length -= 1;
        } else if (value < 0xf0) {
          value =
            ((value & 0x0f) << 12) |
...
library Witnet {
   ...
   * @return A `Result` instance.
   */
  function resultFromCborValue(CBOR.Value memory _cborValue)
    public
    pure
    returns (Result memory)
  {
   ...
   */
  function asBytes(Result memory _result)
    external
    pure
    returns (bytes memory)
  {
   ...
   */
  function asErrorCode(Result memory _result)
    external
    pure
    returns (ErrorCodes)
  {
  ...
  function asErrorMessage(Result memory _result)
    public
    pure
    returns (ErrorCodes, string memory)
  {
    uint64[] memory error = asRawError(_result);
    if (error.length == 0) {
    ...
    if (errorCode == ErrorCodes.SourceScriptNotCBOR && error.length >= 2) {
      ...
      errorCode == ErrorCodes.SourceScriptNotArray && error.length >= 2
    ) {
      errorMessage = abi.encodePacked(
      ...
      errorCode == ErrorCodes.SourceScriptNotRADON && error.length >= 2
    ) {
      errorMessage = abi.encodePacked(
        "The CBOR value in script #",
        utoa(error[1]),
        " was not a valid RADON script"
      );
    } else if (
      errorCode == ErrorCodes.RequestTooManySources && error.length >= 2
    ) {
      errorMessage = abi.encodePacked(
      ...
      errorCode == ErrorCodes.ScriptTooManyCalls && error.length >= 4
    ) {
      errorMessage = abi.encodePacked(
        "Script #",
        utoa(error[2]),
      ...
      errorCode == ErrorCodes.UnsupportedOperator && error.length >= 5
    ) {
      errorMessage = abi.encodePacked(
        "Operator code 0x",
        utohex(error[4]),
   ...
   */
  function asRawError(Result memory _result)
    public
    pure
    returns (uint64[] memory)
  {
    require(
      !_result.success,
      "Tried to read error code from successful Result"
    );
  ...
  function asFixed16(Result memory _result) external pure returns (int32) {
    require(
      _result.success,
      "Tried to read `fixed16` value from errored Result"
    );
   ...
   * @return The `int128[]` decoded from the Result.
   */
  function asFixed16Array(Result memory _result)
    external
    pure
    returns (int32[] memory)
  {
  ...
  function asInt128(Result memory _result) external pure returns (int128) {
    require(
      _result.success,
      "Tried to read `int128` value from errored Result"
    );
   ...
   * @return The `int128[]` decoded from the Result.
   */
  function asInt128Array(Result memory _result)
    external
    pure
    returns (int128[] memory)
  {
   ...
   */
  function asString(Result memory _result)
    external
    pure
    returns (string memory)
  {
    require(
      _result.success,
      "Tried to read `string` value from errored Result"
    );
   ...
   * @return The `string[]` decoded from the Result.
   */
  function asStringArray(Result memory _result)
    external
    pure
    returns (string[] memory)
  {
  ...
  function asUint64(Result memory _result) external pure returns (uint64) {
    require(
      _result.success,
      "Tried to read `uint64` value from errored Result"
    );
   ...
   * @return The `uint64[]` decoded from the Result.
   */
  function asUint64Array(Result memory _result)
    external
    pure
    returns (uint64[] memory)
  {
   ...
   */
  function supportedErrorOrElseUnknown(uint64 _discriminant)
    private
    pure
    returns (ErrorCodes)
  {
...
abstract contract UsingWitnet {
  ...
  modifier witnetRequestResolved(uint256 _id) {
    require(
      witnetCheckRequestResolved(_id),
      "Witnet request is not yet resolved by the Witnet network"
    );
    _;
  }
   ...
   */
  function witnetCheckRequestResolved(uint256 _id)
    internal
    view
    returns (bool)
  {
   ...
   */
  function witnetReadResult(uint256 _id)
    internal
    view
   ...
   * @return The reward to be included for the given gas price.
   */
  function witnetEstimateGasCost(uint256 _gasPrice)
    internal
    view
    returns (uint256)
  {
...
// File: contracts/ViewCountOracle.sol

/// @title ViewCountOracle
/// @notice A YouTube viewcount oracle using Witnet
/// @author Shronk
contract ViewCountOracle is UsingWitnet {
  using Witnet for Witnet.Result;

  struct Oracle {
    // The current views on a video
    uint64 lastViewCount;
    // The timestamp of the last time the viewcount was updated
    uint64 timeStamp;
    // The ID of the last Witnet request
    uint32 lastRequestId;
    // Whether an update is pending
    uint32 pending;
    // The bytecode of the Witnet request
    Request request;
  }
  ...
  /// @notice Creates an oracle struct
  function createOracle(string calldata _id) public {
    oracles[_id] = Oracle(
      0, // lastViewCount
      0, // timeStamp
      0, // lastRequestId
  ...
  function requestUpdate(string calldata _id) public payable {
    Oracle memory oracle = oracles[_id];

    require(oracle.pending == 0, "There is a pending update");

    // Send the request to Witnet and store the ID for later usage
    // The `witnetPostRequest` method comes with `UsingWitnet`
    oracle.lastRequestId = uint32(witnetPostRequest(oracle.request));
  ...
  /// @dev The `witnetRequestAccepted` modifier comes with `UsingWitnet` and
  /// allows to protect your methods from being called before the request has
  /// been successfully relayed into Witnet.
  function completeUpdate(string calldata _id)
    public
    witnetRequestResolved(oracles[_id].lastRequestId)
  {
    Oracle memory oracle = oracles[_id];
    ...
    Witnet.Result memory result = witnetReadResult(oracle.lastRequestId);
    ...
    if (result.isOk()) {
      oracle.lastViewCount = result.asUint64();
      oracle.timeStamp = uint64(block.timestamp);
      emit ViewCountUpdated(oracle.lastViewCount);
    } else {
      string memory errorMessage;

      // Try to read the value as an error message, catch error bytes if read
      // fails
      try result.asErrorMessage() returns (
        Witnet.ErrorCodes,
        string memory e
      ) {
        errorMessage = e;
  ...
  /// @notice Returns the current viewcount for a video.
  function getLastViewCount(string calldata _id) public view returns (uint64) {
    return oracles[_id].lastViewCount;
  }

  /// @notice Returns the last request ID.
  function getLastRequestId(string calldata _id) public view returns (uint64) {
    return oracles[_id].lastRequestId;
  }

  /// @notice Returns tru if a request is pending, false otherwise.
  function isPending(string calldata _id) public view returns (bool) {
    return oracles[_id].pending == 1;
  }
}

// File: contracts/Monetizer.sol

contract Monetizer is ViewCountOracle {
  struct Video {
    bool notEmpty;
    string id;
    address payer;
  ...
  modifier notEmpty(string calldata _id) {
    require(videos[_id].notEmpty, "The video doesn't exist");
    _;
  }
  ...
  function withdraw(string calldata _id) external notEmpty(_id) {
    ...
    require(isPending(_id), "There is a pending request");

    Video memory video = videos[_id];

    // Check whether the timelock has already expired
    require(
      video.lockTime <= block.timestamp,
    ...
    if (video.viewCount > getLastViewCount(_id)) {
      // send the tokens back to the payer
      (bool sent1, ) = video.payer.call{value: video.amount}("");
      require(sent1, "Failed to send Ether to payer");
    } else {
      // if the requirements are fullfilled we can send the tokens to
      // the beneficiary
      (bool sent2, ) = video.beneficiary.call{value: video.amount}("");
  ...
  constructor(address _wrb) ViewCountOracle(_wrb) {}
}
