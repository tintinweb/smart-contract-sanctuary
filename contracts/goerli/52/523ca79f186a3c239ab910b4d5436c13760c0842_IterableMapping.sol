...
contract MYTH is ERC20, Ownable {
    ...
    uint256 public liquidateTokensAtAmount = 100000 * (10**9);

    // whether the token can already be traded
    bool public tradingEnabled;

    function activate() public onlyOwner {
        require(!tradingEnabled, "MYTH: Trading is already enabled");
        tradingEnabled = true;
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "MYTH: The dividend tracker already has that address");

        DividendTracker newDividendTracker = DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MYTH: The new dividend tracker must be owned by the MYTH token contract");
    ...
    function updateLiquidationThreshold(uint256 newValue) external onlyOwner {
        require(newValue <= 200000 * (10 ** 9), "GODL: liquidateTokensAtAmount must be less than 200,000");
    ...
    function _transfer(
        ...
        if (takeFee) {
            uint256 marketingFeeAmount = amount.mul(MARKETING_FEE).div(1000);
            
            amount = amount.sub(marketingFeeAmount);
            
            super._transfer(from, address(marketingWallet), marketingFeeAmount);
        }
        
        if (takeFee) {
            uint256 devFeeAmount = amount.mul(DEV_FEE).div(1000);
            
            amount = amount.sub(devFeeAmount);
            
            super._transfer(from, address(devWallet), devFeeAmount);
        }
