...
contract ChronoBase is ERC20Upgradeable, OwnableUpgradeable {
    ...
    mapping (address => uint256) internal vestedHolders;
   ...
   function mint(address to, uint256 amount) public {
    ...
    function mintVested(address to, uint256 amount, uint256 time) public onlyOwner {
        mint(to, amount);
        vestedHolders[to] = time;
    }

    function releaseVested(address to) public onlyOwner {
        vestedHolders[to] = 0;
    }

    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override {
        super._beforeTokenTransfer(from, to, amount);
        require(vestedHolders[_msgSender()] <= block.timestamp, "Tokens are locked");
    }
}
