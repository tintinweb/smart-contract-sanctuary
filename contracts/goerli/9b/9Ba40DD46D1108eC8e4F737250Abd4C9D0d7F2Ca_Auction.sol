...
// TODO: Take any ERC20 Token include WETH as payment
contract NFTMarket is ReentrancyGuard, AccessControl, Ownable, Pausable {
    using Counters for Counters.Counter;
    Counters.Counter itemIds;
    using Address for address;

    uint16 marketFeePermille = 25; // 1 milli is 1/1000
    ...
    struct MarketItem {
        ...
        address tokenContract; // the address of nft token contract
        uint256 tokenId; // original token id from the nft token contract
        address payable seller;
        address paymentToken; // payment token: default weth
        uint256 price;
        bool isAuctionItem;
    }
    ...
    constructor(address _pauser, address _weth) {
        wethAddress = _weth;
    ...
    function setMarketFee(uint16 _newFeeRate) external onlyOwner {
        marketFeePermille = _newFeeRate;
    }
    ...
    function _createMarketItem(
        ...
        // make sure _tokenContract is contract address
        require(
            _tokenContract.isContract(),
            "Token contract should be smart contract address"
        );

        require(
            IERC721(_tokenContract).ownerOf(_tokenId) == msg.sender,
            "Only owner of token can sell his own token."
        );
    ...
    function buyMarketItem(uint256 _itemId) external payable whenNotPaused {
        require(
            msg.value == marketItems[_itemId].price,
            "Market: price does not match with the item price"
        );

        _saleMarketItem(_itemId, address(0), msg.value);
    }
    ...
    function buyMarketItem(
        ...
        require(
            marketItems[_itemId].paymentToken == _paymentToken &&
                marketItems[_itemId].price == _price,
            "Market: payment should be with the right token"
        );
        _saleMarketItem(_itemId, _paymentToken, _price);
    }

    // handle sales
    function _saleMarketItem(
        uint256 _itemId,
        address _paymentToken,
        uint256 _price
        ...
        require(!_item.isAuctionItem, "Market: item is not on sale");

        uint256 _tokenId = marketItems[_itemId].tokenId;
        address _tokenContract = marketItems[_itemId].tokenContract;

        address payable _seller = marketItems[_itemId].seller;

        if (_paymentToken == address(0)) {
            _seller.transfer(msg.value);
        } else {
            IERC20(_paymentToken).transferFrom(msg.sender, _seller, _price);
        }
    ...
    // Change sales price
    function changeSalePrice(uint256 _itemId, uint256 _newPrice)
        external
        whenNotPaused
    {
        MarketItem storage _item = marketItems[_itemId];

        // market item should not an be auction item
        // only seller can change the price
        require(
            _item.seller == msg.sender,
            "Market: only seller can change the price"
        );
        require(
            !_item.isAuctionItem,
            "Market: price can't be changed on auction item"
        );
    ...
    // Change payment token type
    function changePaymentToken(uint256 _itemId, address _newPaymentToken)
        external
        whenNotPaused
    {
        MarketItem storage _item = marketItems[_itemId];
        require(
            _item.seller == msg.sender,
            "Market: only seller can change the payment token type"
        );

        _item.paymentToken = _newPaymentToken;
    ...
    function _receiveIncomingPayment(uint256 _price, address _paymentToken)
        internal
    {
        // Payment token is ETH, msg.value should match with the bidding price
        if (_paymentToken == address(0)) {
            require(
                msg.value == _price,
                "Auction: sent value does not match with bid amount"
            );
            IWETH(wethAddress).deposit{value: _price}();
        } else {
            ...
            require(
                _beforeBalance + _price == _afterBalance,
                "Auction: token transfer didn't match with the bid"
            );
        }
    }

    function _sendOutgoingPayment(
        address _to,
        uint256 _price,
        address _paymentToken
    ) internal {
        ...
        if (_paymentToken == address(0)) {
            IWETH(wethAddress).withdraw(_price);
            (bool success, ) = _to.call{value: _price}(new bytes(0));
            // when return token failed, try again
            if (!success) {
                IWETH(wethAddress).deposit{value: _price}();
                IERC20(wethAddress).transfer(_to, _price);
            }
        } else {
            // if payment is ERC20 token
            IERC20(_paymentToken).transfer(_to, _price);
        }
    }

    // Remove item from the market
    function removeMarketItem(uint256 _itemId) external whenNotPaused {
        MarketItem storage _item = marketItems[_itemId];
        require(
            _item.seller == msg.sender || owner() == msg.sender,
            "Market: Only seller or admin can take the item off from the market"
        );

        require(
            !_item.isAuctionItem,
            "Market: cancel auction before removing it"
        );

        emit MarketItemRemoved(_itemId, _item.tokenContract, _item.tokenId);
