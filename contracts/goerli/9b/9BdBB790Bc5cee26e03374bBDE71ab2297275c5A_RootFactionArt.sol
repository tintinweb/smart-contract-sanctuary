    ...
    IMintableERC1155
{
    ...
    */
    ...
    function restTotalSupply(uint256 _id, uint256 _amount) public view returns (uint256) {
      ...
      if (_id == uint(1)) {
        require(( subtotal <= uint256(645) ), "RootFactionArt: EXCEED MAX_AMOUNT");
      } else if (_id == uint(2)) {
        require(( subtotal <= uint256(1263) ), "RootFactionArt: EXCEED MAX_AMOUNT");
      } else if (_id == uint(3)) {
        require(( subtotal <= uint256(1267) ), "RootFactionArt: EXCEED MAX_AMOUNT");
      } else if (_id == uint(4)) {
        require(( subtotal <= uint256(3141) ), "RootFactionArt: EXCEED MAX_AMOUNT");
      } else if (_id == uint(5)) {
        require(( subtotal <= uint256(645) ), "RootFactionArt: EXCEED MAX_AMOUNT");
      } else if (_id == uint(6)) {
        require(( subtotal <= uint256(1263) ), "RootFactionArt: EXCEED MAX_AMOUNT");
