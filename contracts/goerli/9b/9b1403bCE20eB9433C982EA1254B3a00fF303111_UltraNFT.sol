...
contract UltraNFT is ERC721Pausable, AccessControl, Ownable {
    ...
    event MintFeeAddressTransfered (
    ...
    event SetMintFeeAmount (
        ...
        uint256 prevMintFeeAmount,
        uint256 newMintFeeAmount
    );

    constructor(uint256 _mintFeeAmount, address _mintFeeAddress) public ERC721('Ultrareum NFT', 'UNFT') {
        _setupRole(UPDATE_TOKEN_URI_ROLE, _msgSender());
        _setupRole(PAUSED_ROLE, _msgSender());
        initialize(_mintFeeAmount, _mintFeeAddress);
    }

    receive() external payable {}

    function initialize(uint256 _mintFeeAmount, address _mintFeeAddress) private {
        require(_mintFeeAddress != address(0), "Not able to set mint fee address");
        mintFeeAmount = _mintFeeAmount;
        mintFeeAddress = _mintFeeAddress;

        emit MintFeeAddressTransfered(address(0), msg.sender);
        emit SetMintFeeAmount(msg.sender, 0, _mintFeeAmount);
    }

    function mint(address to, string memory _tokenURI) public payable {
        require(msg.value >= mintFeeAmount, "Your mint fee is too low");
        address sender = address(msg.sender);
        TransferHelper.safeTransferETH(mintFeeAddress, mintFeeAmount);
        uint256 tokenId = nextTokenId;
        _mint(to, tokenId);
        _setTokenURI(tokenId, _tokenURI);
        nextTokenId++;
        if (msg.value > mintFeeAmount) {
            TransferHelper.safeTransferETH(sender, msg.value - mintFeeAmount);
        }
    }

    function transferMintFeeAddress(address _mintFeeAddress) public onlyOwner {
        require(_mintFeeAddress != address(0), "Not able to transfer mint fee address");
        mintFeeAddress = _mintFeeAddress;
        emit MintFeeAddressTransfered(msg.sender, _mintFeeAddress);
    }

    function setMintFeeAmount(uint256 _mintFeeAmount) public onlyOwner {
        require(_mintFeeAmount != mintFeeAmount, "Not allow to set same price");
        emit SetMintFeeAmount(msg.sender, mintFeeAmount, _mintFeeAmount);
        mintFeeAmount = _mintFeeAmount;
    }
