...
contract MasterChef is Ownable,Pausable {
    ...
    ICrossChainMigrator public migrator;
    ...
    mapping (uint256 => address[]) private _poolAccountList;
    ...
    event MigratorUpdated(ICrossChainMigrator indexed newMigrator);
    ...
    function setMigrator(ICrossChainMigrator _migrator) public onlyOwner {
        migrator = _migrator;
        emit MigratorUpdated(_migrator);
    }
    
    // Migrate lp token to another lp contract. Can be called by anyone. We trust that migrator contract is good.
    function migrate(uint256 _pid) public whenPaused{
    ...
    function deposit(uint256 _pid, uint256 _amount) public whenNotPaused{
        ...
        // data migration
        if (!_accountCheck[_pid][msg.sender]) {
            _accountCheck[_pid][msg.sender] = true;
            _poolAccountList[_pid].push(msg.sender);
        }
    ...
    function getAccountList(uint256 _pid) external whenPaused view returns(address[] memory){
        return _poolAccountList[_pid];
    }
    
    function migrateUserLP(uint256 _pid,address _user) external whenPaused returns(uint256){
        require(msg.sender == address(migrator), "MasterChef: not from CrossChain Migrator");
