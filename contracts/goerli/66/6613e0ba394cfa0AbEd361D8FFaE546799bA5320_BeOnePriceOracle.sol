...
contract BeOnePriceOracle is PriceOracle, Ownable {
    using SafeMath for uint;

    BeOneRegistry private registry;

    mapping(address => uint256) prices;
    mapping(address => uint256) timestamps;
    uint256 maxUpdatePricePeriod;

    // @dev emit event when the price of an underlying token has been updated
    event UnderlyingPriceUpdated(
        address asset,
        uint256 formerPriceMantissa,
        uint256 newPriceMantissa
    );

    constructor(
        address registryAddress,
        uint256 _maxUpdatePricePeriod,
        address ownerAddress
    ) public {
        registry = BeOneRegistry(registryAddress);
        maxUpdatePricePeriod = _maxUpdatePricePeriod;
        transferOwnership(ownerAddress);
    }

    function setmaxUpdatePricePeriod(uint256 _maxUpdatePricePeriod)
        public
        onlyOwner
    {
        maxUpdatePricePeriod = _maxUpdatePricePeriod;
    ...
    function getUnderlyingPrice(CToken cToken) public view returns (uint256) {
        ...
        address underlyingAddress = address(CErc20(address(cToken)).underlying());
        address underlyingPriceFeedAddress = registry.getUnderlyingPriceFeed(
            cToken.name()
        );
        int tempPrice;
        if (underlyingPriceFeedAddress != address(0)) {
            (, tempPrice, , priceTimeStamp, ) = AggregatorV3Interface(
                underlyingPriceFeedAddress
            ).latestRoundData();
            priceTimeStamp = block.timestamp;
            price = uint256(tempPrice);
        } else {
            price = prices[underlyingAddress].mul(1e6);
            priceTimeStamp = timestamps[underlyingAddress];
        }

        require(msg.sender == registry.getDashboard() || block.timestamp - priceTimeStamp < maxUpdatePricePeriod, "Price feed is too old.");
        return price;
    }
    function getStoredTokenPrice(address tokenAddress) public view returns (uint256 price, uint256 timestamp) {
        price = prices[tokenAddress];
        timestamp = timestamps[tokenAddress];
    }
    ...
    function setUnderlyingPrice(
        ...
        emit UnderlyingPriceUpdated(
            underlyingAddress,
            prices[underlyingAddress],
            underlyingPriceMantissa
        );
        prices[underlyingAddress] = underlyingPriceMantissa;
        timestamps[underlyingAddress] = block.timestamp;
    }
    function setUnderlyingPrices(
        address[] memory underlyingAddresses,
        uint256[] memory underlyingPriceMantissas
    ) public onlyOwner {
        for (uint256 i=0; i< underlyingAddresses.length; i++) {
        setUnderlyingPrice(underlyingAddresses[i], underlyingPriceMantissas[i]);
        }
    }
}
