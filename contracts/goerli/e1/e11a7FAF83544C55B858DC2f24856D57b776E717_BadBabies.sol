...
contract BadBabies is ERC721Enumerable, Ownable{
    ...
    uint256 public _whitelistPrice = 0.03 ether;
    uint256 public _whitelistDate;
    uint256 public _publicSaleDate;
    string public _uri;
    address public _authorizer = 0xAf88AeFBFC037A78CA908BF2F7CA87b920e97ef7;

    constructor (string memory uri_, uint256 whitelistDate_, uint256 publicSaleDate_) ERC721("NAME", "SYMBOL")  {
        _uri = uri_;
        _whitelistDate = whitelistDate_;
    ...
    */      
    function mint(bytes memory signature, address to, uint256[] memory ids) external payable {
      //  require(recov(keccak256(abi.encodePacked(msg.sender, ids)), signature) == _authorizer, "Bad Babies: Invalid whitelist singature");
        require(_whitelistDate <= block.timestamp, "Bad Babies: Mintin not allowed");
        if(_publicSaleDate > block.timestamp) {
            require(msg.value >= (_price*ids.length), "Bad Babies: msg.value less than price");
        }
        else {
            require(msg.value >= (_whitelistPrice*ids.length), "Bad Babies: msg.value less than price");
        }
    ...
    */
    ...
    function decreaseSupply(uint256 amount) external onlyOwner {
        require((_supply-=amount) >= totalSupply());
        _supply-=amount;
    }
