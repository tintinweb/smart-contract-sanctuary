...
contract GroupWalletFactory {
    ...
    event TransferToken(bytes32 domainHash);  
    event TransferFrom(bytes32 domainHash);  
    ...
    { 
      //revert(string(abi.encodePacked( b_String(bytes32( uint256( gasleft() ) ),32))));
      ...
      if (setProxyCommand == 6) emit TransferToken        (_domainHash);  
      if (setProxyCommand == 7) emit TransferFrom         (_domainHash);  
    }
