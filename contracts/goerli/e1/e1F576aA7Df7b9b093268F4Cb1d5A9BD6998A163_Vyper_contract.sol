...
def setGovernance(governance: address):
    ...
    """
    assert msg.sender == self.governance, "Only governance can change governance!"  # dev: unauthorized
...
def acceptGovernance():
    ...
    """
    assert msg.sender == self.pendingGovernance, "Only pending governance can accpet governance role!"  # dev: unauthorized
    self.governance = msg.sender
...
def newRelease(vault: address):
    ...
    """
    assert msg.sender == self.governance, "Only governance can add new releases!"  # dev: unauthorized
    ...
    if release_id > 0:
        ...
        ), "The new release should have a different version of the API!"  # dev: same api version
...
def _newProxyVault(
    ...
    assert release != ZERO_ADDRESS, "Unknow release!"  # dev: unknown release
...
def _registerVault(token: address, vault: address):
    ...
    if vault_id > 0:
        assert (
            ...
            != Vault(vault).apiVersion()
        ), "The newly registered Vault must have another API version!"  # dev: same api version
...
def newVault(
    ...
    """
    assert msg.sender == self.governance, "Only governance can add a new Vault!"  # dev: unauthorized
...
def endorseVault(vault: address, releaseDelta: uint256 = 0):
    ...
    @param releaseDelta Specify the number of releases prior to the latest to use as a target. Default is latest.
    """
    assert msg.sender == self.governance, "Only governance can endorse the Vault!"  # dev: unauthorized
    assert Vault(vault).governance() == msg.sender, "Governance addresses must be the same both in the Vault and in the Registry!"  # dev: not governed

    # NOTE: Underflow if no releases created yet, or targeting prior to release history
    releaseTarget: uint256 = self.numReleases - 1 - releaseDelta  # dev: no releases
    api_version: String[28] = Vault(self.releases[releaseTarget]).apiVersion()
    assert Vault(vault).apiVersion() == api_version, "The Vault API version to be approved does not match the target API version."  # dev: not target release
...
def setBanksy(tagger: address, allowed: bool = True):
    ...
    """
    assert msg.sender == self.governance, "Only governance can set tagger status!"  # dev: unauthorized
...
def tagVault(vault: address, tag: String[120]):
    ...
    if msg.sender != self.governance:
        assert self.banksy[msg.sender], "The sender does not have permission to add tags to this Vault!"  # dev: not banksy
