...
interface ICaller {
  function caller() external returns (address _caller);
}

interface IReturner {
  function returner() external returns (address _caller);
}

contract Returner is IReturner{
  function returner() external override returns (address _caller) {
    return ICaller(msg.sender).caller();
  }
}

contract TestState is ICaller{
  address public override caller;
  address public returner;

  constructor(address _returner) {
    returner = _returner;
  }
  ...
  function revertState() external payable setsCaller() returns (bool) {
    require(IReturner(returner).returner() == msg.sender, 'REVERTS');
    return true;
  }

  function payableState() external payable setsCaller() returns (address) {
    return IReturner(returner).returner();
  }

  // function viewState() external view setsCaller() returns (address) {
  //   return IReturner(returner).returner();
  // }
