...
// File: contracts/OZ_ERC1155.sol

/*
 * This is a 1:1 copy of OpenZeppelin ERC1155.sol from master branch as of 2021-05-03.
 * This contains a patch that enables us to do signedTransfer without an ugly workaround.
 * See https://github.com/OpenZeppelin/openzeppelin-contracts/pull/2636
 */
...
// File: contracts/CS3_1Presale.sol

/*
 * On-chain presale for Crypto stamp Edition 3.1 (on Layer 2 only).
 * Implements ERC 1155 token standard: https://eips.ethereum.org/EIPS/eip-1155
 */

// import "@openzeppelin/contracts/token/ERC1155/ERC1155.sol";

contract CS3_1Presale is ERC1155 {
    ...
    mapping(uint256 => uint256) public assetLimit;
    mapping(uint256 => uint256) public assetSupply;
    mapping(uint256 => uint256) public assetSold;
    uint256[] public assetTypes;
    ...
    event LimitChanged(uint256 assetType, uint256 previousLimit, uint256 newLimit);
    ...
    ERC1155("")
    {
        ...
        redeemer = bridgeData.getAddress("shopControl");
        ...
        uint256 typesNum = _assetLimits.length;
        for (uint256 i = 0; i < typesNum; i++) {
            setLimitInternal(i, _assetLimits[i]);
        }
    }
    ...
    /*** Manage opening/closing shop and sunset / disabling of transfers ***/
    ...
    /*** Manage asset types and sale limits ***/

    // Returns whether the specified type / token exists.
    function exists(uint256 id) public view returns (bool) {
        return assetLimit[id] > 0;
    }

    // Get the number of activated asset types.
    function assetTypesCount()
    public view
    returns(uint256)
    {
    ...
    // Set the limit of how many tokens of a certain type/id can exist.
    // If the limit was 0 beforehand, this asset type is being enabled.
    function setLimit(uint256 _type, uint256 _newLimit)
    public
    onlyShopControl
    {
        require(_newLimit > 0, "Cannot set to 0, use removeType().");
        require(assetSold[_type] <= _newLimit, "Type already over requested limit.");
        setLimitInternal(_type, _newLimit);
    }

    function setLimitInternal(uint256 _type, uint256 _newLimit)
    internal
    {
        if (assetLimit[_type] == 0) {
            assetTypes.push(_type);
        }
        emit LimitChanged(_type, assetLimit[_type], _newLimit);
        assetLimit[_type] = _newLimit;
    }

    // Remove a type, setting its limit to 0. Also needs the matching index in the assetTypes array.
    function removeType(uint256 _type, uint256 _typeIndex)
    public
    onlyShopControl
    {
        require(assetSold[_type] == 0, "Assets of this type have already been sold.");
        require(assetLimit[_type] > 0, "The type is not enabled.");
        require(assetTypes[_typeIndex] == _type, "Type index does not match.");
        // Move last index to the index position and then pop the last one.
        assetTypes[_typeIndex] = assetTypes[assetTypes.length - 1];
        assetTypes.pop();
        setLimitInternal(_type, 0);
    }

    /*** Report amounts of token supply, sold, and still available ***/
    ...
    returns (uint256)
    {
        ...
        uint256 typesNum = assetTypes.length;
        for (uint256 i = 0; i < typesNum; i++) {
            _totalSupply += assetSupply[assetTypes[i]];
        }
        return _totalSupply;
    }
    ...
    returns (uint256)
    {
        ...
        uint256 typesNum = assetTypes.length;
        for (uint256 i = 0; i < typesNum; i++) {
            _totalSold += assetSold[assetTypes[i]];
        }
        return _totalSold;
    }

    // Returns the amount of assets of that type still available for sale.
    function availableForSale(uint256 _type)
    public view
    returns (uint256)
    {
        return assetLimit[_type] - assetSold[_type];
    }

    // Returns true if the asset of the given type is sold out.
    function isSoldOut(uint256 _type)
    public view
    returns (bool)
    {
        return assetSold[_type] >= assetLimit[_type];
    }

    /*** Actual presale buy and redeem functionality with documentation parameters ***/

    // Buy assets of a single type/animal. The number of assets is determined from the amount of ETH sent.
    function acceptAndBuy(uint256 _type, bool _acceptTermsCryptoPostAt, string calldata /* _acceptanceText */)
    external payable
    requireOpen
    {
        buyFor(_type, payable(msg.sender), _acceptTermsCryptoPostAt);
    }

    // Buy assets of a single type/animal. The number of assets is determined from the amount of ETH sent.
    function buyFor(uint256 _type, address payable _recipient, bool _acceptTermsCryptoPostAt)
    public payable
    requireOpen
    {
        require(_acceptTermsCryptoPostAt, "You need to accept the terms.");
        require(exists(_type), "The requested asset does not exist.");
        uint256 curPriceWei = priceWei();
        require(msg.value >= curPriceWei, "Send enough currency to pay at least one item.");
        uint256 maxToSell = assetLimit[_type] - assetSold[_type];
        ...
        _mint(_recipient, _type, assetCount, bytes(""));
        assetSupply[_type] += assetCount;
        assetSold[_type] += assetCount;
    ...
    function buyBatch(uint256[] calldata _type, uint256[] calldata _count, bool _acceptTermsCryptoPostAt)
    external payable
    requireOpen
    {
        require(_acceptTermsCryptoPostAt, "You need to accept the terms.");
        ...
        for (uint256 i = 0; i < inputlines; i++) {
            payAmount += _count[i] * curPriceWei;
            assetSupply[_type[i]] += _count[i];
            assetSold[_type[i]] += _count[i];
            // If any asset in the batch would go over the limit, fail the whole transaction.
            require(assetSold[_type[i]] <= assetLimit[_type[i]], "At least one requested asset is sold out.");
        }
        require(msg.value >= payAmount, "Send enough currency to pay all specified items.");
        // Transfer the actual payment amount to the beneficiary.
        payable(bridgeData.getAddress("beneficiary")).sendValue(payAmount);
        // Generate and assign the actual assets.
        _mintBatch(msg.sender, _type, _count, bytes(""));
    ...
    function redeemBatch(address owner, uint256[] calldata _type, uint256[] calldata _count)
    external
    onlyRedeemer
    {
        uint256 inputlines = _type.length;
        ...
        _burnBatch(owner, _type, _count);
    }

    /*** Override ERC1155 internal functionality for special rules on approvals and transfers as well as improve URI functionality ***/
    ...
    function uri(uint256 _tokenId) public view override returns (string memory) {
        require(exists(_tokenId), "ERC1155MetadataURI: URI query for nonexistent token");
        return string(abi.encodePacked(bridgeData.tokenURIBase(), "CS3PS/meta/", _tokenId.toString()));
    }
    ...
    function signedBatchTransfer(address _from, address _to, uint256[] memory _ids, uint256[] memory _amounts, bytes memory _signature)
    public
    {
        // The signed data contains the token IDs, amounts, the transfer source and target as well as a nonce.
        bytes32 data = keccak256(abi.encodePacked(address(this), this.signedBatchTransfer.selector, _from, _to, _ids, _amounts, signedTransferNonce[_from]));
    ...
    function signedBatchTransferWithOperator(address _from, address _to, uint256[] memory _ids, uint256[] memory _amounts, bytes memory _signature)
    public
    {
        // The signed data contains the operator, the token IDs, amounts, and a nonce. Note that we use the selector of the external function here!
    ...
    function _signedBatchTransferInternal(address _from, address _to, uint256[] memory _ids, uint256[] memory _amounts, bytes32 _data, bytes memory _signature)
    internal
    {
        bytes32 hash = ECDSA.toEthSignedMessageHash(_data);
        address signer = ECDSA.recover(hash, _signature);
        require(signer == _from, "Signature needs to match parameters, nonce, and current owner.");
        // Now that we checked that the signature is correct, do the actual transfer and increase the nonce.
        emit SignedBatchTransfer(msg.sender, _from, _to, _ids, _amounts, signedTransferNonce[_from]);
        signedTransferNonce[_from]++;
