...
contract HeroSource is ISource, ERC721Enumerable, ReentrancyGuard, Ownable, AccessControl {
    ...
    function bridgeMint(address to, uint256 tokenId, uint256 seed) override external {
        seeds[tokenId] = seed;
        _safeMint(to, tokenId);
    }

    function bridgeBurn(uint256 tokenId) override external {
        seeds[tokenId] = 0;
        _burn(tokenId);
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override (ERC721Enumerable) {
        ...
        if (from == address(0))
            require(hasRole(MINTER_ROLE, msg.sender) || hasRole(BRIDGE_ROLE, msg.sender));
