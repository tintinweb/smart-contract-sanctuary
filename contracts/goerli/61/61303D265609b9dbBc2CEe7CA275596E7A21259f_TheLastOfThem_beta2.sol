...
// File contracts/TheLastOfThem_beta2.sol
...
contract TheLastOfThem_beta2 is
    IERC165,
    IERC721,
    IERC721Metadata,
    IERC2981,
    ...
    ReentrancyGuard
{
    ...
    function setTokenHashes(uint256[] memory _ids, string[] memory _hashes)
        public
        onlyOwner
    {
        require(_ids.length == _hashes.length, "Invalid parameters");

        for (uint256 i = 0; i < _ids.length; i++) {
            _tokenHashes[_ids[i]] = _hashes[i];
        }
    }
        ...
        returns (string memory)
    {
        ...
        string memory _hash = _tokenHashes[_id];

        return
            bytes(_hash).length > 0
                ? string(abi.encodePacked("ipfs://", _hash))
                : string(abi.encodePacked(_unrevealedUri));
    }
