...
contract MockRhoStrategy is IRhoStrategy, PausableUpgradeable, AccessControlEnumerableUpgradeable {
    ...
    uint256 public override supplyRate;
    uint256 private _effectiveSupplyRate;
    uint8 private constant _lastDeployGasFee = 0;
    uint8 private constant _lastWithdrawGasFee = 1;

    function initialize(address _underlying) public initializer {
        PausableUpgradeable.__Pausable_init();
        AccessControlEnumerableUpgradeable.__AccessControlEnumerable_init();
        underlying = IERC20MetadataUpgradeable(_underlying);
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    function setSupplyRate(uint256 a) external {
        supplyRate = a;
    }

    function setEffectiveSupplyRate(uint256 a) external {
        _effectiveSupplyRate = a;
    }

    function setSwitchLockUntil(uint256 a) external {
        switchLockedUntil = a;
    }

    function effectiveSupplyRate() external view override returns (uint256 _amount) {
        return _effectiveSupplyRate;
    }

    function effectiveSupplyRate(uint256, bool) external view override returns (uint256) {
        return _effectiveSupplyRate;
    }

    function deploy(uint256 _amount) public override {
        emit Deploy(_amount);
    }

    function withdrawUnderlying(uint256 _amount) public override {
    ...
    function collectRewardToken() external override {}

    function setRewardConversionThreshold(uint256 _threshold) external override {}
    ...
    function sweepERC20Token(address token, address to) external override {}

    function setSwitchLockDuration(uint256 numberOfBlocks) external override {
    ...
    function switchingLock(uint256 target) external override {
    ...
    function isLocked() external view override returns (bool) {
        return (_balanceOfUnderlying() < switchingLockTarget && block.number < switchLockedUntil);
    }
}
