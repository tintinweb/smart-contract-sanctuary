...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        uint256 intDivision = tokenId/1000;
...
contract SoundSprouts is ERC721Enumerable, ERC721Burnable, ERC721Pausable, ReentrancyGuard, Ownable {
    ...
    uint256 public constant maxSuperRare = 18;

    uint256 public SproutsMintedForPromotion;
    uint256 public unitSproutPrice;
    uint256 public numberOfSuperRare;

    uint256[] public superRareArray;

    mapping (uint256 => uint256) internal _genes;
    mapping(uint256 => string) private _tokenURIS;

    event SuperRareMinted(uint256 indexed tokenId, uint256 gene);
    event SproudMinted(uint256 indexed tokenId, uint256 gene);
    ...
    function mintForPromotion(uint256 amount, address whoToSend) public onlyOwner returns(bool){
    ...
    function mintHelper(address sender, uint256 amount) internal {
      ...
      for (uint256 i = 0; i < amount; i++) {
          ...
          uint256 lotteryNumber = _genes[tokenId]%20;
          if(lotteryNumber == 7 && numberOfSuperRare < maxSuperRare){
            superRareArray.push(tokenId);
            numberOfSuperRare += 1;
            _mint(sender, tokenId);
            emit SuperRareMinted(tokenId, _genes[tokenId]);
          }
          else{
            _mint(sender, tokenId);
            emit SproudMinted(tokenId, _genes[tokenId]);
          }
      }
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (bytes(_tokenURIS[tokenId]).length > 0){
          return string(abi.encodePacked(_serviceIPFS,_tokenURIS[tokenId], tokenId.toString(),".json"));
       }
    ...
    // https://gateway.pinata.cloud/ipfs/ + _URI + tokenId.json
    function setUniqueTokenURI(uint256 tokenId, string memory _URI) public onlyOwner {
