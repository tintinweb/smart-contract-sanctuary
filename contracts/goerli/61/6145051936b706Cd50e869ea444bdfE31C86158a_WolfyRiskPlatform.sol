...
contract WolfyRiskPlatform is Ownable {
       ...
       if(_factor >= 15 && _champ == true){
        ...
        if(_factor >= 50 && _champ == true){
            ...
            for(uint i = 0 ; i < _highRiskETHUsers.length ; i++){
                ...
                // address + isLowRisk + isWolfy + count ==> bool
                previousResults[_highRiskETHUsers[i]][false][false][runCount] = true;
            }
    ...
    function startPool(uint256 _defiPulse, uint256 _snp500) public onlyOwner { 
        ...
        require( _ethBalanceRiskPool[msg.sender][true] > 0 ," LowRisk : Please add ETH liquidity");
        require( _ethBalanceRiskPool[msg.sender][false] > 0 ," HighRisk : Please add ETH liquidity");
        require( _balancesRiskPool[owner()][true] > 0 , "LowRisk : Please add WOLFY liquidity");
        require( _balancesRiskPool[owner()][false] > 0 , "HighRisk : Please add WOLFY liquidity");
    ...
    function netEarning(bool _isWolfy, bool _isLowRisk) public view returns (uint256){
         ...
         if(getProfits(_isWolfy, _isLowRisk) > getLosses(_isWolfy, _isLowRisk)){
