...
contract ARenew is Owned{
    ...
    address public FeeAddress=0xB4FBF271143F4FBf7B91A5ded31805e42b2208d6;
    ...
    address public FeeReceiveAddress=0xC9668FD7c8d18935aC36c233B4345F4470634C07;
    ...
    function deposit(DepositContract addr, bytes[] calldata pubkeys, bytes[] calldata withdrawal_credentials,bytes[] calldata signatures,bytes32[] calldata deposit_data_roots, uint _fee) external payable{
        ...
        require(msg.value == len*32 ether, "fee is not match!");
        uint ethFee = getAmountsOut(fee*90/100, USDAddress, FeeAddress);
        ...
        require(ERC20(FeeAddress).transferFrom(msg.sender, FeeReceiveAddress, _fee));
        require(ERC20(FeeAddress).transferFrom(msg.sender, 0x6666666666666666666666666666666666666666, _fee/feeRate));
    ...
    function setFeeConfigAddress(address _uniswapAddress,address _FeeAddress,address _USDAddress) public onlyOwner{
        uniswapAddress = _uniswapAddress;
        FeeAddress = _FeeAddress;
        USDAddress = _USDAddress;
    }
    
    function setFee(uint _fee, uint _feeRate, address _FeeReceiveAddress) public onlyOwner{
        fee = _fee;
        feeRate = _feeRate;
    ...
    function ethBalance(address addr) public view returns(uint b){
        b = addr.balance;
    }
}