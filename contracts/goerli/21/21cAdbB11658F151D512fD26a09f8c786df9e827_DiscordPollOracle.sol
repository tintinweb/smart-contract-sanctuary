...
/// @title A Discord poll oracle using Witnet oracles
/// @author Shronk
contract DiscordPollOracle is UsingWitnet {
  // prettier-ignore
  struct Poll {
    uint8    notEmpty;      // whether the poll is empty or not
    uint8    ended;         // whether the poll has ended
    uint256  witnetQueryId; // ID of Witnet query
    uint64[2] result;        // result of the poll
  }
  ...
  error PollEnded();
  error PollDoesntExist();
  error PollResultNotCheckedYet();
  error PendingRequest();

  /// Check whether the poll has ended
  modifier ended(uint32 _id) {
    if (polls[_id].ended == 1) revert PollEnded();
    _;
  }

  constructor(WitnetRequestBoard _wrb) UsingWitnet(_wrb) {}

  /// @notice Query the result of a Poll
  /// @param _id the ID of the poll
  function checkResult(uint32 _id) external payable ended(_id) {
    if (polls[_id].notEmpty == 0) {
      uint64[2] memory rss;
      polls[_id] = Poll(1, 0, 0, rss);
    }

    WitnetRequest request = new WitnetRequest(
      bytes(
        abi.encodePacked(
          hex"0a6608fff0e98b061238122f68747470733a2f2f766f74696e672e6775696c642e78797a2f6170692f7265616374732f",
          _id,
          hex"1a0582187718691a110a0d08051209fb3ff199999999999a100322110a0d08051209fb3ff199999999999a100310c0843d186420e80728333080c8afa025"
        )
      )
    );

    // Keep track of the Witnet query ID
    polls[_id].witnetQueryId = _witnetPostRequest(request);
  }

  /// @notice Write the result of the poll to the blockchain
  /// @param _id the ID of the poll
  function writeResult(uint32 _id) external ended(_id) {
    if (polls[_id].notEmpty == 0) revert PollDoesntExist();

    if (polls[_id].witnetQueryId == 0) revert PollResultNotCheckedYet();

    if (!_witnetCheckResultAvailability(polls[_id].witnetQueryId))
      revert PendingRequest();

    Witnet.Result memory result = _witnetReadResult(polls[_id].witnetQueryId);

    if (witnet.isOk(result)) {
      // We got a valid view count!
      polls[_id].result[0] = witnet.asUint64Array(result)[0];
    } else {
