...
contract BundNFTFactory is ERC1155 {
    using SafeMath for uint256;
    ...
    event ArtistApproved(address indexed account, uint256 tokenId);
    ...
    // isSelf = true : When admin is caller | mints token for himself |
    function setNFT(uint8 _copies, uint256 _basePrice, uint256 _fee, bool _isSelf, string memory _metadata) public {
        ...
        if (msg.sender == owner()) {
            instanceNFT.minted = true;
        } else {
            instanceNFT.minted = false;
        }

        if(_isSelf && msg.sender == owner()){
        _mint(owner(), nextNFTIndex ,_copies,"");
    ...
    function isMinted(uint256 _tokenId) public view returns (bool) {
        return mappingNFT[_tokenId].minted;
    }

    //-------| check if token is created by Artist |--------
    function isArtistNFT(uint256 _tokenId) public view returns (bool) {
        if (mappingNFT[_tokenId].creator != owner()) {
            return true;
        } else {
            return false;
        }
    ...
    function approveArtist(uint256 _tokenId, bool _decision, string memory _statusApprove) public onlyOwner {
        require(mappingNFT[_tokenId].minted != true,"Token is already approved and minted");
        
        mappingNFT[_tokenId].statusApprove = _statusApprove;

        if(_decision) {
          mappingNFT[_tokenId].minted = true;
        _mint(mappingNFT[_tokenId].creator,_tokenId,mappingNFT[_tokenId].copies,"");

        }
        emit ArtistApproved(mappingNFT[_tokenId].creator, _tokenId);
    }
        ...
        )
    {
        ...
        return (
            ...
            mappingNFT[_index].statusApprove
        );
    }
    ...
    function purchaseNFT(uint256 _index, uint8 _quantity) public {
        require(
            mappingNFT[_index].creator == owner(),
        ...
        require(
            BundNFT.balanceOf(msg.sender) >=
                mappingNFT[_index].basePrice.mul(_quantity),
            "Insufficient BundNFT in your wallet"
        );

        _mint(msg.sender, _index, _quantity, "");
        BundNFT.safeTransferFrom(
            msg.sender,
            address(this),
            mappingNFT[_index].basePrice.mul(_quantity)
        );
        mintedNFT[_index] += _quantity;
    }

    //++++| Burn all bundNFT deposits |+++++
    function burnDeposits() public onlyOwner {
        require(BundNFT.balanceOf(address(this)) > 0, "No bundNFT deposits");
    ...
    function claimDeposits() public onlyOwner {
        require(
            BundNFT.balanceOf(address(this)) > 0,
            "No bundNFT deposits in this contract"
        );
