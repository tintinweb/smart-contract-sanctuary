...
interface IMinter {
    ...
    function pIQ() external view returns (address);
}
...
contract PTokenMinter is IMinter {
    ...
    constructor(IIQERC20 iQ, IERC20 pIQ) {
        _iQ = iQ;
        _pIQ = pIQ;
    }

    function mint(uint256 _amount) external override blockReentrancy {
        require(_pIQ.transferFrom(msg.sender, address(this), _amount), "Transfer has failed");
    ...
    function burn(uint256 _amount) external override blockReentrancy {
        ...
        require(_pIQ.transfer(msg.sender, _amount), "Transfer has failed");
    ...
    function pIQ() external view override returns (address) {
        return address(_pIQ);
    }
}