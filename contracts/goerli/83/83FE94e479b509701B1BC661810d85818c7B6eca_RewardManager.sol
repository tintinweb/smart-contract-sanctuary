...
contract RewardManager is Context, Ownable {
    ...
    function fundPool(address _poolAddress, uint256 _amount) public {
        require(
            _msgSender() == operator,
            "Only the operator contract can call this function"
        );

        totemToken.transfer(_poolAddress, _amount);
    }

    function getBalance() public view returns (uint256) {
        return totemToken.balanceOf(address(this));
    }
}
...
contract TotemToken is Context, ERC20, Ownable {
    ...
    uint256 public constant PUBLIC_SALE = 450000 * (10**uint256(DECIMALS)); // 4.5% for Public Sale
    ...
    uint256 public constant STRATEGIC_ROUND = 1500000 * (10**uint256(DECIMALS)); // 15% for Strategic Round
