...
import {ERC1155SupplyUpgradeable, ERC1155Upgradeable, IERC165Upgradeable, ContextUpgradeable} from "@openzeppelin/contracts-upgradeable/token/ERC1155/extensions/ERC1155SupplyUpgradeable.sol";
...
import {IMintableERC1155, IERC1155Upgradeable} from "./IMintableERC1155.sol";
import {ContextMixin} from "../../common/ContextMixin.sol";

contract RootNFTMetaKey is
    ...
    IMintableERC1155
{
        ...
        public initializer
    {
        ...
        _setupContractId("RootNFTMetaKey");
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(PREDICATE_ROLE, mintableERC1155PredicateProxy);
        _initializeEIP712(uri_);
        name = name_;
        symbol = symbol_;
        CONTRACT_URI = "https://api.cypherverse.io/os/collections/metakey";
    }
    ...
    function mint(
        ...
        require((id > uint(0)), "RootNFTMetaKey: INVALID_TOKEN_ID");
        require(((amount > uint(0)) &&
                 (((id == uint(1)) && (amount == restTotalSupply(id, amount))) ||
                  ((id == uint(2)) && (amount == restTotalSupply(id, amount))) ||
                  ((id == uint(3)) && (amount == restTotalSupply(id, amount))) ||
                  ((id == uint(4)) && (amount == restTotalSupply(id, amount))) ||
                  ((id == uint(5)) && (amount == restTotalSupply(id, amount))))), "RootNFTMetaKey: INVALID_AMOUNT");
    ...
    function mintBatch(
        ...
        for (uint i = 0; i < ids.length; i++) {
          require((ids[i] > uint(0)), "RootNFTMetaKey: INVALID_TOKEN_ID");
          require(((amounts[i] > uint(0)) &&
                 (((ids[i] == uint(1)) && (amounts[i] == restTotalSupply(ids[i], amounts[i]))) ||
                  ((ids[i] == uint(2)) && (amounts[i] == restTotalSupply(ids[i], amounts[i]))) ||
                  ((ids[i] == uint(3)) && (amounts[i] == restTotalSupply(ids[i], amounts[i]))) ||
                  ((ids[i] == uint(4)) && (amounts[i] == restTotalSupply(ids[i], amounts[i]))) ||
                  ((ids[i] == uint(5)) && (amounts[i] == restTotalSupply(ids[i], amounts[i])))) ), "RootNFTMetaKey: INVALID_AMOUNT");
        }
    ...
    function burn(
        ...
        require(
            ...
            "RootNFTMetaKey: caller is not owner nor approved"
        );
    ...
    function isApprovedForAll(
        address _owner,
        address _operator
    ) public override(ERC1155Upgradeable, IERC1155Upgradeable) view returns (bool isOperator) {
    ...
    */
     ...
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC1155Upgradeable, AccessControl, IERC165Upgradeable) returns (bool) {
    ...
    function restTotalSupply(uint256 _id, uint256 _amount) public view returns (uint256) {
      ...
      if (_id == uint(1)) {
        require(( subtotal <= uint256(500) ), "RootNFTMetaKey: EXCEED MAX_AMOUNT");
      } else if (_id == uint(2)) {
        require(( subtotal <= uint256(750) ), "RootNFTMetaKey: EXCEED MAX_AMOUNT");
      } else if (_id == uint(3)) {
        require(( subtotal <= uint256(1000) ), "RootNFTMetaKey: EXCEED MAX_AMOUNT");
      } else if (_id == uint(4)) {
        require(( subtotal <= uint256(7500) ), "RootNFTMetaKey: EXCEED MAX_AMOUNT");
      } else if (_id == uint(5)) {
        require(( subtotal <= uint256(255) ), "RootNFTMetaKey: EXCEED MAX_AMOUNT");
      }
    ...
    function _beforeTokenTransfer(
        ...
        require(!paused(), "RootNFTMetaKey: token transfer while paused");
    }
}
