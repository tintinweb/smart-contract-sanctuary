...
contract TestCoin is Ownable, IERC20 {
    ...
    string private constant _name = "Bulin Coin - Only to the Moon!"; //TBD
    string private constant _symbol = "BULINEQ"; //TBD    
    ...
    LaunchLimit[3] private _launchLimits;
    ...
    uint256 private constant _tTotal = 10**6 * 10**18; //TBD
    ...
    uint8 private _charityTax = 4; // Fee for charity TBD
    uint8 private _marketingTax = 4; // Fee for marketing TBD
    ...
    function setLaunchPhase() external onlyOwner {
        _launchPhase = true;
        _launchLimits[0].time = block.timestamp + 60 seconds;
        _launchLimits[1].time = block.timestamp + 120 seconds;
        _launchLimits[2].time = block.timestamp + 180 seconds;
        _launchLimits[0].amount = _tTotal / 1000;
        _launchLimits[1].amount = 2 * _tTotal / 1000;
        _launchLimits[2].amount = 3 * _tTotal / 1000;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (_launchPhase && from == _uniswapV2Pair && 
            to != address(_uniswapV2Router) && !_isExcludedFromFee[to]) {
            uint8 phase;
            if(_launchLimits[0].time > block.timestamp) {
                phase = 0;
            } else if(_launchLimits[1].time > block.timestamp) {
                phase = 1;
            } else if(_launchLimits[2].time > block.timestamp) {
                phase = 2;
            } else {
                _launchPhase = false;
            }
            ...
            if(_launchPhase) {
                require(amount <= _launchLimits[phase].amount, "Lower amount allowed during launch!");
                require(_setCoolDown[to] < block.timestamp, "Cool down, dude!");
