...
struct Camps {
    ...
    uint256 public TIME_CONSTANT = 1 seconds; //CHANGE!
    uint256 public REGEN_TIME = 300 seconds; //CHANGE!!
...
function initialize(address _elfcontract) public onlyOwner {
    ...
    require(!initialized, "Contract instance has already been initialized");
        ...
        camps[1] = Camps({baseRewards: 24, creatureCount: 3000, creatureHealth: 8,   expPoints:3,   minLevel:1});
        camps[2] = Camps({baseRewards: 48, creatureCount: 3000, creatureHealth: 48,  expPoints:6,   minLevel:1});
        camps[3] = Camps({baseRewards: 96, creatureCount: 3000, creatureHealth: 240, expPoints:18,  minLevel:1});
...
returns(uint256 level, uint256 rewards, uint256 timestamp, uint256 inventory){
  ...
  require(elfcontract == msg.sender); 
  require(camp.minLevel <= _level);
  require(camp.creatureCount > 0);
  
  camps[_campId].creatureCount = camp.creatureCount - 1;

  rewards = camp.baseRewards + (_sector * 25/10);
  ...
  uint256 creatureHealth =  DataStructures.calcCreatureHealth(_sector, camp.creatureHealth);
