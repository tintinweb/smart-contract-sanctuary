...
contract NFTAirdrop is Ownable, Freezable {
  ...
  // prettier-ignore
  //(DC Server ID -> (DC Role ID -> (NFT contract address -> Drop)))
  mapping(string => mapping(string => mapping(address => Drop))) public dropOfRole;
  // prettier-ignore
  //(User address -> (DC Server ID -> (DC Role ID -> (NFT contract address -> Claim))))
  ...
  event NewAirdrop(string serverId, string channelId);
  event Claimed(
  ...
  function setExternalUrl(string memory _newUrl) external onlyOwner {
    externalUrl = _newUrl;
  }
  ...
  function deployTokenContract(
    ...
    address roleToken = address(
      new DCRoleToken(_tokenName, _tokenSymbol, externalUrl)
    );
    contractsByDeployer[msg.sender][
      numOfDeployedContracts[msg.sender]
    ] = roleToken;
  ...
  /// @param _roles An array of structs from containing information about a role.
  ...
  function newAirdrop(
    ...
    for (uint256 i = 0; i < _roles.length; i++) {
      if (
        bytes(dropOfRole[_serverId][_roles[i].roleId][roleToken].tokenImageHash)
          .length == 0
      ) {
        Drop memory drop;
        drop.active = true;
        drop.tokenImageHash = _roles[i].tokenImageHash;
        drop.nftName = _roles[i].nftName;
        dropOfRole[_serverId][_roles[i].roleId][roleToken] = drop;

        DCRoleToken(roleToken).addRoleAttributes(
          _serverId,
          _roles[i].roleId,
          _roles[i].traitTypes,
          _roles[i].values
    ...
    emit NewAirdrop(_serverId, _channelId);
  }
  ...
  /// @param _signature Signature created by the bot.
  /// @param _serverId Id of the Discord server.
  /// @param _roleIds An array of role Ids.
  /// @param _contractId Id of the token contract deployed by the server owner.
  function stopAirdrop(
    ...
    for (uint256 i = 0; i < _roleIds.length; i++) {
      dropOfRole[_serverId][_roleIds[i]][roleToken].active = false;
    }
  }
  ...
  function claim(
    ...
    Claim memory userClaim = claims[msg.sender][_serverId][_roleId][
      tokenAddress
    ];
    Drop memory drop = dropOfRole[_serverId][_roleId][tokenAddress];
    require(drop.active || userClaim.approved, "No active airdrop");
    ...
    claims[msg.sender][_serverId][_roleId][tokenAddress].claimed = true;

    //Mint the NFT from the data and send it to the msg.sender
    DCRoleToken(tokenAddress).safeMint(
      msg.sender,
      _serverId,
      _roleId,
      drop.tokenImageHash,
  ...
  function grant(
    ...
    bytes32 message = keccak256(
      abi.encode(address(this), _serverId, _roleId, _user)
    ).toEthSignedMessageHash();
    require(message.recover(_signature) == bot, "Not valid signature");

    address roleToken = contractsByDeployer[msg.sender][_contractId];
    claims[msg.sender][_serverId][_roleId][roleToken].approved = true;
...
contract DCRoleToken is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
  ...
  mapping(uint256 => Data) private dataOfToken;
  mapping(string => mapping(string => Attribute[])) private tokenAttributes;
  ...
  function safeMint(
    ...
    data.roleId = _roleId;
    data.image = _imageHash;
    data.image = _tokenName;
    dataOfToken[_tokenIdCounter.current()] = data;
    ...
    returns (string memory)
  {
    ...
    string memory link = string(
      abi.encodePacked(_baseURI(), dataOfToken[tokenId].image)
    );
      ...
      bytes(
        string(
          abi.encodePacked(
            ...
            dataOfToken[tokenId].serverId,
            '"}, {"trait_type": "Role Id", "value": "',
            dataOfToken[tokenId].roleId,
            '"}',
            tokenAttributesToMetadata(tokenId),
            ']}'
          )
        )
      )
  ...
  function addRoleAttributes(
    ...
    for (uint256 i = 0; i < _traitTypes.length; i++) {
      Attribute memory attribute = Attribute(_traitTypes[i], _values[i]);
      tokenAttributes[_serverId][_roleId].push(attribute);
    }
  }
  ...
  function tokenAttributesToMetadata(uint256 _tokenId)
    internal
    view
    returns (string memory)
  {
    Attribute[] memory attributes = tokenAttributes[
      dataOfToken[_tokenId].serverId
    ][dataOfToken[_tokenId].roleId];
