...
contract StakeManager is
    ...
    function isOwner() public view returns (bool) {
        address _owner;
        bytes32 position = keccak256("bridge.network.proxy.owner");
        assembly {
    ...
    function validatorExit(
        ...
        (uint256 validatorId, uint256 nonce, uint256 deactivationEpoch) = abi.decode(data, (uint256, uint256, uint256));
        uint256 stakeNonce = logger.getValidatorNonce(validatorId).add(1);
        require(stakeNonce == nonce, "Invalid staking nonce");
        uint[3][] memory siglist = sigs;
        require(verifyConsensus(keccak256(abi.encodePacked(bytes(hex"01"), data)), siglist), "2/3+1 Power required");
        ...
        updateTimeline(-(int256(amount) + delegationAmount), -1);
    ...
    function signerUpdate(
        ...
        (uint256 validatorId, uint256 nonce, address signer) = abi.decode(data, (uint256, uint256, address));
        uint256 stakeNonce = logger.getValidatorNonce(validatorId).add(1);
        require(stakeNonce == nonce, "Invalid staking nonce");
        uint[3][] memory siglist = sigs;
        require(verifyConsensus(keccak256(abi.encodePacked(bytes(hex"01"), data)), siglist), "2/3+1 Power required");
    ...
    function _setValidator(
        ...
        signerToValidator[signer] = validatorId;
        updateTimeline(int256(amount + delegatedAmount), 1);
...
contract IStakeManager {
    ...
    function verifyConsensusWithSigners(bytes32 voteHash, uint256[3][] memory sigs) public view returns (bool,address[] memory,uint256);
