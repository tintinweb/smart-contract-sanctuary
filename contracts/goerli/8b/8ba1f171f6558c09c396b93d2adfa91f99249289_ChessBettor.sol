...
contract ChessBettor is Ownable {
    ...
    function getGameDetails(uint _gameId) public view returns(uint8 winner, uint gameId, uint totalAmount, uint amountWhite, uint amountBlack, uint numberBets, Vote[] memory voteList, bool isOn) {
       Game storage _game =  existingGames[_gameId];
       return (_game.winner, _game.gameId, _game.totalAmount, _game.amountWhite, _game.amountBlack, _game.numberBets, _game.voteList, _game.isOn);
    }
