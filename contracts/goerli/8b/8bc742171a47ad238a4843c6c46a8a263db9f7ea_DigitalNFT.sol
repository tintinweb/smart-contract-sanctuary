...
Permissioned NFTs that can be deployed by artists for zero gas 
  ...
  ERC721Metadata
{
  ...
  /**
   * @dev Mapping from NFT ID to artist address.
   */
  mapping (uint256 => address) internal idToArtist;
 ...
 * 
 * 
 *  NFT contract with offchain-mintable tokens 
 * 
 *  EIP712 
 *
 */
  ...
  ApproveAndCallFallBack
{
  ...
  mapping(bytes32 => uint256) public numberOfCopies;
  
  mapping(address => uint256) public internalVault;
  
   
   
   
  constructor(  uint256 chain_id, uint256 fee_pct, address stakeableCurrency, address reservePoolToken  ) 
  {
   ...
   _reservePoolToken = reservePoolToken;
    ...
    "MintPacket(address artist,address keypassToken,string uri,uint256 maxCount,uint256 expirationBlock,address currencyToken,uint256 currencyAmount)"
    );
    ...
    function getTypedDataHash(address artist, address keypassToken, string memory uri,uint256 maxCopies, uint256 expirationBlock,address currencyToken, uint256 currencyAmount) public view returns (bytes32) {
        ...
        "\x19\x01",
        getEIP712DomainHash('HyperNFT','1',_chain_id,address(this) ),
   ...
   * @param _to The address that will own the minted NFT.
   */
  function mint( address _to, address artist, address keypassToken, string memory uri, uint256 maxCopies, uint256 expirationBlock, address currencyToken, uint256 currencyAmount, bytes memory signature) external returns(bool) {  
    
    //check EIP712 signature from artist that validates the [ uri  , artist, and KeypassToken  ] 
    ...
    require(block.number > expirationBlock, 'Packet has expired.');
 
    if(keypassToken != address(0x0)){
        require( ERC721(keypassToken).balanceOf(_to) >= 1 ); 
    } 
    
    if(currencyToken != address(0x0)){
       // require( IERC20(currencyToken).transferFrom(_to,artist,currencyAmount) ); 
        _transferCurrencyForSale(_to, artist, currencyToken , currencyAmount );
    } 
    
    
    require(numberOfCopies[sigHash] < maxCopies);
    numberOfCopies[sigHash] = numberOfCopies[sigHash]+1;
    
    
     
    super._mint(_to,tokensMinted);
    super._setTokenUri(tokensMinted, uri);
    super._setTokenArtist(tokensMinted, artist);
   ...
   function _transferCurrencyForSale( address from, address to, address currencyToken, uint256 currencyAmount) internal returns (bool){
    ...
    require( IERC20(currencyToken).transferFrom(from, address(this), feeAmount ), 'unable to pay'  );
    
    internalVault[currencyToken] =  internalVault[currencyToken] + feeAmount;
  ...
  function stakeCurrency( address from,  uint256 currencyAmount ) public returns (bool){
     ...
     internalVault[_stakeableCurrency] =  internalVault[_stakeableCurrency] + currencyAmount;
  ...
  function unstakeCurrency( uint256 reserveTokenAmount, address[] memory claimTokenAddresses) public returns (bool){
      ...
      for(uint i =0; i< claimTokenAddresses.length; i++){
       ...
       IERC20(claimTokenAddress).transfer( msg.sender, _vaultOutputAmount(claimTokenAddress, reserveTokenAmount, totalReserveTokens) );
      } 
  ...
  function _reserveTokensMinted(address currencyToken, uint256 currencyAmount, uint256 totalReserveTokens) internal view returns (uint){
      ...
      uint256 internalVaultBalance = internalVault[currencyToken]; 
