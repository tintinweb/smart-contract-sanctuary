...
contract DomToretto {
    /* DOM TORETTO TOKEN
    * Yes, it is another shitcoin/memecoin of a trending meme. But it has some changed up "tokenomics" 
    * and it is not another Safemoon copy&paste.
    * TOKENOMICS:
    * Base tax: 8% burn
    
    * SELF-ADJUSTING TAX: 
    * Tax adjusts dynamically accordingly to buy&sell volumes. The contract records buy and sell volumes
    * for 1 hour, then updates the tax. Tax goes up if sell volume is high&buy is low, and vice versa.
    
    * HOLD-DURATION TAX:
    * The contract also records each addresses' buy times. If an address holds for <1h, it's taxed 1.5x
    * more than base. If 1h<{hold duration}<=3h, it's taxed 1.25x. If more than 3h, it's taxed normally.
    * TAX LIMITS:
    * -The tax can't go less than 5%.
    * -The tax can't go higher than 18%.
    */
    
    // token details
    string public name = "DBTEST";
