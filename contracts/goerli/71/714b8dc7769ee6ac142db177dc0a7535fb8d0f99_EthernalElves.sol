...
//STATE
    ...
    address validator;
    address public constant validatorAddress = 0x80861814a8775de20F9506CF41932E95f80f7035;
    ...
    function initialize(address _dev1Address, address _dev2Address) public {
       ...
       maxSupply            = 200; //6666 for live
       INIT_SUPPLY          = 96; //3300 for live
       initialized          = true;
       price                = .055 ether;  //CHANGE to .088!!
       _remaining           = [2,4,10]; //[200, 600, 2500]; //this is the supply of each whitelist role
...
function whitelistMint(uint256 qty, address to, uint256 roleIndex, bytes memory signature) public payable  {
    ...
    isPlayer();
    require(_isSignedByValidator(getEthSignedMessageHash(encodeForSignature(to, roleIndex)),signature), "invalid signature");  
    require(isWlOpen, "Whitelist is closed");
    require(whitelist[msg.sender] != 1,"Wallet used already"); //not on whitelist
    //Checkpoint to stop whitelist from minting more than the initial supply
    require(INIT_SUPPLY <= totalSupply, "Whitelist minting is closed");
    ...
    uint256 role = roleIndex;
    require(_remaining[role] > 0, "noneLeft");

    uint256 amount = msg.value;
    
    _remaining[role]--;
    whitelist[whitelistAddress] = 1; //indicate that adress is used
...
function calcCreatureHealth(uint256 sector, uint256 baseCreatureHealth) internal pure returns (uint256 creatureHealth) {
        ...
        creatureHealth = ((sector - 1) * 8) + baseCreatureHealth; 
...
returns (uint256 newWeaponTier, uint256 newWeapon, uint256 newInventory) {
   ...
   uint256 levelTier = uint256((level_/20) + 1);
