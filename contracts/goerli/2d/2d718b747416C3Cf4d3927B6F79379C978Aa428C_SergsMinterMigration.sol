...
contract SergsMinterMigration is IMinter, Ownable{
    ...
    function migrateSingle(uint256 _tokenId) external{
		...
		//require(keccak256(abi.encodePacked(id)).toEthSignedMessageHash().recover(_sig) == SIGNER, "Sig not valid");
    ...
    function onERC1155Received(address _operator, address _from, uint256 _id, uint256 _value, bytes calldata _data) external returns(bytes4) {
		...
		return Sergs.onERC1155Received.selector;
	}
    */
...
contract CryptoSergs is ERC721, Ownable {
	...
	event SergMigrated(uint256 tokenId);

    constructor(string memory _baseURI, address _minter) ERC721("CryptoSergs","SERGS") {
		setBaseURI(_baseURI);
        sergsMinter = IMinter(_minter);
