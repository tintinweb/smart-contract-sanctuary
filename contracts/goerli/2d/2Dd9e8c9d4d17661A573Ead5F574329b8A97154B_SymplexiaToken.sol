...
//   ======================================
    ...
    function _getValues(uint256 tAmount, bool applyFee) internal view returns (uint256, uint256, uint256, uint256) {
        ...
        if (applyFee) {tAmountFees = tAmount.mul(_sumOfFees);}

        uint256 tTransferAmount = tAmount.sub(tAmountFees);
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rTotalFees =tAmountFees.mul(currentRate);
...
contract SymplexiaToken is  AntiWhale,  BaseLiquidity,  BaseRFI {
  ...
  //
    ...
    constructor () BaseToken ( "DEBUG23", "DBG23", 5, 5, 5, 
