    ...
    ERC721Pausable
{
    ...
    mapping(uint256 => string[3]) private _characterAttributes;
    mapping(uint256 => uint256) private _meltTokensInToken;
    mapping(uint256 => uint256[]) private _parents;

    uint256 private _allocatedMeltTokens;
    uint256 private _minimumMeltValue = 100 * 10**18;

    address public meltTokenAddress; // ERC20 token used to bake value into tokens

    IERC20 private _MELT;
    ...
    function setNewMinter(address newMinter) public {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "Must have admin role to set minter"
        );
    ...
    function setNewPauser(address newPauser) public {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
    ...
    function newToken(
        ...
        require(
            availableMeltTokens() >= tokensAllocated,
    ...
    function availableMeltTokens()
        public
        view
        returns (uint256 availableBalance)
    {
        return (_MELT.balanceOf(address(this)) - _allocatedMeltTokens);
    }
...

{
  ...
  "libraries": {
    "/contracts/Utilities.sol": {
      "Utilities": "0x0d9A5EE22F49e108F71dacB8EDD53f7C11bF5d3b"
    }
  },
  "outputSelection": {
    "*": {
