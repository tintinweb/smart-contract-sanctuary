...
contract FxERC1155RootTunnel is FxBaseRootTunnel, Create2, ERC1155Holder {
    ...
    constructor(address _checkpointManager, address _fxRoot, address _fxERC1155Token) FxBaseRootTunnel(_checkpointManager, _fxRoot) {
        toshimonMinter = ToshimonMinter(_fxERC1155Token);
    }
    ...
    function deposit(address user, uint256 id, uint256 amount, bytes memory data) public {
        ...
        toshimonMinter.burn(msg.sender, id, amount);

        // DEPOSIT, encode(rootToken, depositor, user, id, amount, extra data)
        bytes memory message = abi.encode(DEPOSIT, abi.encode(msg.sender, user, id, amount, data));
    ...
    function depositBatch(address user, uint256[] memory ids, uint256[] memory amounts, bytes memory data) public {
        ...
        // transfer from depositor to this contract
        for(uint256 i = 0; i < ids.length; i++){
            toshimonMinter.burn(msg.sender, ids[i], amounts[i]);
        }

        // DEPOSIT_BATCH, encode(rootToken, depositor, user, id, amount, extra data)
        bytes memory message = abi.encode(DEPOSIT_BATCH, abi.encode(msg.sender, user, ids, amounts, data));
        _sendMessageToChild(message);
    }
    ...
    function _syncWithdraw(bytes memory syncData) internal {
        (address childToken, address user, uint256 id, uint256 amount, bytes memory data) = abi.decode(syncData, (address, address, uint256, uint256, bytes));
        toshimonMinter.mint(user, id, amount, data);
    }
    
    function _syncBatchWithdraw(bytes memory syncData) internal {
        (address childToken, address user, uint256[] memory ids, uint256[] memory amounts, bytes memory data) = abi.decode(syncData, (address, address, uint256[], uint256[], bytes));
        toshimonMinter.mintBatch(user, ids, amounts);
    }
}