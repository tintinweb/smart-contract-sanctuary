...
contract ST_Rebase_Liquidity_Token is Context, IST20, Ownable, TokenGenerator {
    ...
    constructor (string memory name, string memory symbol, uint256 decimals, uint256 totalSupply, uint256 _maxTxAmount, uint256 _taxFee, uint256 _mktFee, address _mktAddress, address payable feeReceiver) 
    ...
    // ------------------------------------------------------------------------
    // Function to change marketing fees in %.
    // The fees can not be higher than 30%, to protect the interest of token holders.
    // Enter only numbers and not the % sign.
    // Only the Contract owner can change the fees.
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // Function to change marketing address.
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tMkt);
    }
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        return (tTransferAmount, TData(0, tFee, tMkt, 0));
    }
    function _getRValues(TData memory _data) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rMkt);
