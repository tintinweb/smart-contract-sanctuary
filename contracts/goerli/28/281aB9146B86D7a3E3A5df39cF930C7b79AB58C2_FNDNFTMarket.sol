...
import "./mixins/AccountRecovery.sol";
  ...
  NFTMarketAuction
{
  ...
  ) public onlyAuthorizedAccountRecovery(originalAddress, newAddress, signature) {
 ...
 */
abstract contract AccountRecovery is FoundationOperatorRole {
   ...
   * signed by the original account.
   */
  modifier onlyAuthorizedAccountRecovery(
    address originalAddress,
    address newAddress,
    bytes memory signature
  ) {
    require(_isFoundationOperator(), "AccountRecovery: Caller is not an operator");
    bytes32 hash =
      _toEthSignedMessage(
    ...
    require(
      ...
      "AccountRecovery: Signature must be from the original account"
    );
    _;
  }
}
