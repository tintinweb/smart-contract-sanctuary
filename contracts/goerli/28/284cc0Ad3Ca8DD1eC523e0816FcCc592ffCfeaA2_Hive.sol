...
contract Hive is IHive, Ownable, IERC721Receiver, Pausable {
    ...
    function calculateBeeOwed(uint256 hiveId, uint256 tokenId) external view returns (uint256 owed) {
        ...
        owed = calculateOwed(since > hives[hiveId].startedTimestamp ? since : hives[hiveId].startedTimestamp);
    }
    ...
    function removeFromWaitingRoom(uint256 tokenId) external whenNotPaused {
        ...
        if (token.tokenId > 0 && beesContract.getTokenData(token.tokenId)._type == 1) {
            ...
            for (uint256 i = 0; i < totalHives; i++) {
    ...
    function _claimBeeFromHive(
        ...
        owed = calculateOwed(bee.since > hives[hiveId].startedTimestamp ? bee.since : hives[hiveId].startedTimestamp);
    ...
    /**
     * tracks $HONEY earnings to ensure it stops once 0.5 billion is eclipsed
     */
