...
contract MasterChef is Ownable,Pausable {
    ...
    event CrossChainMigratorUpdated(ICrossChainMigrator indexed newCrossChainMigrator);
    ...
    function setCrossChainMigrator(ICrossChainMigrator _crossChainMigrator) public onlyOwner {
    ...
    function crossChainMigrate(uint256 _pid) public whenPaused{
        require(address(crossChainMigrator) != address(0), "crossChainMigrate: no crosschain migrator");
    ...
    function totalFarmersInPool(uint256 _pid) external whenPaused view returns(uint256){
        return poolAccountList[_pid].length;
    }
    
    function getPoolAccountList(uint256 _pid) external whenPaused view returns(address[] memory){
        return poolAccountList[_pid];
    }
    
    function finalizeUserMigration(uint256 _pid,address _user) external view whenPaused returns(uint256,bool){
        require(msg.sender == address(crossChainMigrator), "MasterChef: not from CrossChain Migrator");
        UserInfo memory user = userInfo[_pid][_user];
        if (!user.everDeposited) return (0,false);
        uint256 pending = user.amount.mul(poolInfo[_pid].accCNTPerShare).div(1e12).sub(user.rewardDebt);
        if(pending > 0) {
