...
// File contracts/RootChain/QuartzRootTunnel.sol

// contracts/QuartzRootTunnel.so
pragma solidity 0.7.3;
// import "../AccessControl/WithRolesTunnel.sol";

interface IQuartz {
    function registerArchetype(uint256 archetypeId, uint256 limit) external returns(uint256);
}

/** 
 * @title QuartzRootTunnel
 */
contract QuartzRootTunnel is FxBaseRootTunnel {

    /**
     * @dev Quartz contract
     */
    address private quartz;
    ...
    constructor(address _quartz, address _checkpointManager, address _fxRoot) FxBaseRootTunnel(_checkpointManager, _fxRoot) {
        quartz = _quartz;
    }
    ...
    function _processMessageFromChild(bytes memory data) internal override {
        ...
        if (syncType == REGISTER_ARCHETYPE) {
            IQuartz _quartz = IQuartz(quartz);
            (uint256 id, uint256 limit) = abi.decode(syncData, (uint256, uint256));
            _quartz.registerArchetype(id, limit);
        } else {
            revert("QuartzRootChild: INVALID_SYNC_TYPE");
        }
    }
