...
contract GStakingManager is AccessControl, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        mapping(address => uint256) accRewardPerShares; // Accumulated rewards per share, times 1e12. See below.
        uint128 chainId;
        PoolType poolType;
    }
    ...
    mapping(uint256 => mapping(address => mapping(address => RewardInfo))) public rewardInfo;
    // Classify pools into CLAIMABLE AND REWARD_CALC
    mapping(PoolType => uint[]) public poolTypes;

    mapping(address => bool) public supportedTokens;
    mapping(address => uint) public supportedTokenToIndex;

    mapping(address => uint) public totalStakedByToken;

    uint public totalStaked;
    address[] internal _supportedTokensList;
    ...
    modifier validateSupportedToken(address token) {
        require(supportedTokens[token], "StakingPool: token is not exist");
        _;
    }
    ...
    constructor(
        ...
        _addTokenToSupported(address(_gpoolToken));
        _addTokenToSupported(address(_gpoolLP));
    }
    ...
    function getSupportedTokenInList(uint _index) public view returns(address) {
        return _supportedTokensList[_index];
    }

    function addSupportedToken(address _token) public onlyRole(GUARDIAN_ROLE) returns(bool) {
        return _addTokenToSupported(_token);
    }

    function removeSupportedToken(address _token) public onlyRole(GUARDIAN_ROLE) returns(bool) {
        return _removeTokenToSupported(_token);
    }
     ...
     */
    function stake(address token, uint256 amount) external nonReentrant validateSupportedToken(token) {
        require(IERC20(token).balanceOf(msg.sender) >= amount, "not enough gpool");
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            RewardInfo storage user = rewardInfo[pid][msg.sender][token];
            updatePool(pid);
            uint256 pending = ((staker.amounts[token] * pool.accRewardPerShares[token]) / ACCUMULATED_MULTIPLIER) - user.rewardDebt;
            user.pendingReward = user.pendingReward + pending;
            user.rewardDebt = ((staker.amounts[token] + amount) * pool.accRewardPerShares[token]) / ACCUMULATED_MULTIPLIER;
        }

        staker.amounts[token] += amount;
        totalStakedByToken[token] += amount;
        totalStaked += amount;

        require(_reachUSDCThreshHold(msg.sender), "minimum stake does not match");
     ...
     */
    function unstake(address token, uint256 amount, bool harvestReward) external nonReentrant validateSupportedToken(token) {
        StakeInfo storage staker = stakeInfo[msg.sender];
        require(amount <= staker.amounts[token], "not enough balance");

        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            _updateUserReward(pid, amount, token, harvestReward);
        }
        ...
        if (staker.amounts[token] == 0 || !_reachUSDCThreshHold(msg.sender)) {
            staker.startStake = 0;
        } else {
            staker.startStake = block.timestamp;
        }
    ...
    function _updateUserReward(
        ...
        RewardInfo storage user = rewardInfo[pid][msg.sender][token];
        StakeInfo storage staker = stakeInfo[msg.sender];
        if ((harvestReward || staker.amounts[token] == amount) && pool.poolType == PoolType.CLAIMABLE) {
            claimReward(pid);
        } else {
            updatePool(pid);
            uint256 pending = ((staker.amounts[token] * pool.accRewardPerShares[token]) / ACCUMULATED_MULTIPLIER) - user.rewardDebt;
     ...
     * @param pid id of the pool
     */
    function claimReward(uint256 pid) public validatePoolById(pid) returns (uint256[] memory totalPendings) {
        totalPendings = new uint[](_supportedTokensList.length);
        ...
        for (uint i = 0; i < _supportedTokensList.length; i++) {
            ...
            RewardInfo storage user = rewardInfo[pid][msg.sender][supportedToken];
            StakeInfo storage staker = stakeInfo[msg.sender];
            LockedReward memory lockedReward = lockingAmounts[pid][msg.sender];

            uint256 totalPending = pendingReward(pid, supportedToken, msg.sender);
            user.pendingReward = 0;
            user.rewardDebt = (staker.amounts[supportedToken] * pool.accRewardPerShares[supportedToken]) / (ACCUMULATED_MULTIPLIER);
            ...
            totalPendings[i] = totalPending;

        }

        emit ClaimReward(msg.sender, pid, totalPendings);
        return totalPendings;
    }
     ...
     */
    function totalPendingReward(uint256 pid, address userAddress) public view validatePoolById(pid) returns (uint256) {
        PoolInfo storage pool = poolInfo[pid];
        StakeInfo storage staker = stakeInfo[userAddress];

        uint256 totalPendingReward;

        for (uint i = 0; i < _supportedTokensList.length; i++) {
            address supportedToken = _supportedTokensList[i];
            uint256 accRewardPerShare = pool.accRewardPerShares[supportedToken];

            RewardInfo memory user = rewardInfo[pid][userAddress][supportedToken];
            ...
            if (endTime > pool.lastRewardSecond && totalStaked != 0) {
                ...
                accRewardPerShare = (accRewardPerShare + ((poolReward * totalStakedByToken[supportedToken] / totalStaked * ACCUMULATED_MULTIPLIER) / totalStakedByToken[supportedToken]));
            }

            totalPendingReward += (user.pendingReward + (((staker.amounts[supportedToken] * accRewardPerShare) / ACCUMULATED_MULTIPLIER) - user.rewardDebt));
        }
     ...
     * @param userAddress the address of the user
     */
    function pendingReward(uint256 pid, address token, address userAddress) public view validatePoolById(pid) returns (uint256) {
        PoolInfo storage pool = poolInfo[pid];
        RewardInfo memory user = rewardInfo[pid][userAddress][token];
        StakeInfo storage staker = stakeInfo[userAddress];

        uint256 accRewardPerShare = pool.accRewardPerShares[token];
        uint256 endTime = pool.closeTime < block.timestamp ? pool.closeTime : block.timestamp;

        if (endTime > pool.lastRewardSecond && totalStaked != 0) {
            uint256 poolReward = (pool.totalReward * (endTime - pool.lastRewardSecond)) /
                (pool.closeTime - pool.openTime);
            accRewardPerShare = (accRewardPerShare + ((poolReward *  ACCUMULATED_MULTIPLIER * totalStakedByToken[token] / totalStaked) / totalStakedByToken[token]));
        }

        uint256 totalPendingReward = (user.pendingReward + (((staker.amounts[token] * accRewardPerShare) / ACCUMULATED_MULTIPLIER) - user.rewardDebt));
        return totalPendingReward;
    }
    ...
    function updatePool(uint256 pid) public validatePoolById(pid) {
        ...
        for (uint256 i = 0; i < _supportedTokensList.length; i++) {
            address supportedToken = _supportedTokensList[i];
            if (totalStakedByToken[supportedToken] > 0) {
                uint256 deltaRewardPerShare = (poolReward * totalStakedByToken[supportedToken] / totalStaked * ACCUMULATED_MULTIPLIER) / totalStakedByToken[supportedToken];
                pool.accRewardPerShares[supportedToken] = pool.accRewardPerShares[supportedToken] + deltaRewardPerShare;
                totalRewardDelta += deltaRewardPerShare;
            }
        }

        if (totalRewardDelta == 0 && block.timestamp < pool.closeTime) {
            // wait for delta > 0
            return;
        }
    ...
    function createPool(
        ...
        pool.lastRewardSecond = openTime;
    ...
    function tokenInUSDC(address token, uint256 gpoolAmount) public view returns (uint256) {
        // twap is in second
        return oracle.assetToAsset(token, gpoolAmount, address(usdc), twapPeriod);
    }

    // getTier: user's gpass
    function getTier(address user) public view returns (Tier) {
        StakeInfo storage staker = stakeInfo[user];
        if (staker.startStake == 0) {
            return Tier.NORANK;
        }
    ...
    function withdrawReward(IERC20 token, uint256 amount) external onlyRole(GUARDIAN_ROLE) {
        require(!supportedTokens[address(token)], "only reward token");
    ...
    function _reachUSDCThreshHold(address _user) internal view returns(bool) {
        uint i = 0;
        uint totalAmountInUSDC = 0;
        while (i < _supportedTokensList.length) {
            address supportedToken = _supportedTokensList[i];
            StakeInfo storage staker = stakeInfo[_user];
            totalAmountInUSDC += tokenInUSDC(supportedToken, staker.amounts[supportedToken]);

            if (totalAmountInUSDC >= USDC_THRESHOLD) {
               return true;
            }
    ...
    function _addTokenToSupported(address _token) internal returns(bool) {
        require(!supportedTokens[_token], "Token is already in supported list!");

        supportedTokens[_token] = true;
        supportedTokenToIndex[_token] = _supportedTokensList.length;
        _supportedTokensList.push(_token);

        return true;
    }

    function _removeTokenToSupported(address _token) internal returns(bool) {
        require(supportedTokens[_token], "Token is not in supported list!");

        if (_supportedTokensList.length > 1) {
            address lastTokenInList = _supportedTokensList[_supportedTokensList.length - 1];
            uint removeTokenIndex = supportedTokenToIndex[_token];

            _supportedTokensList[removeTokenIndex] = lastTokenInList;
            supportedTokenToIndex[lastTokenInList] = removeTokenIndex;
        }

        delete supportedTokens[_token];
        delete supportedTokenToIndex[_token];
