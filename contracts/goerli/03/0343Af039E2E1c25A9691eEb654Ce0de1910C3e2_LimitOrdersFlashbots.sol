...
contract LimitOrdersFlashbots is IModule {
    ...
    struct StructData {
        uint256 _fees;
        uint256 _minerBalanceBefore;
        address[] _swapHandlers;
        bytes[] _swapData;
    }
    ...
    function execute(
        ...
        StructData memory structData;

        (structData._fees, structData._swapHandlers, structData._swapData) = abi
            .decode(_auxData, (uint256, address[], bytes[]));

        require(
            structData._swapHandlers.length == structData._swapData.length,
            "LimitOrdersFlashbots#execute: HANDLERS_LENGTH_MISMATCH"
        );

        structData._minerBalanceBefore = block.coinbase.balance;

        if (_inToken == WETH || address(_inToken) == ETH) {
            // Deduct ETH fees before swap
            if (_inToken == WETH) {
                if (structData._fees > 0) WETH.withdraw(structData._fees);
            } else {
                WETH.deposit{value: address(this).balance - structData._fees}();
            }
            ...
            bought = _swap(
                ...
                _minReturn,
                structData._fees,
                structData._swapHandlers,
                structData._swapData // needs to have user inputAmount - fees encoded
            );
            ...
            bought = _swap(
                ...
                structData._fees,
                structData._swapHandlers,
                structData._swapData // needs to have user minReturn + fees encoded
            );
        } else {
            bought = _swap(
                _owner,
                ...
                structData._fees,
                structData._swapHandlers,
                structData._swapData // needs to route through path that leaves WETH fees
            );
            if (structData._fees > 0) {
                uint256 wethBalance = WETH.balanceOf(address(this));
                if (wethBalance > 0) WETH.withdraw(wethBalance);
            }
        }

        // miner fee payment should not happen via multi-call,
        // only through _payFees
        require(
            block.coinbase.balance == structData._minerBalanceBefore,
            "LimitOrdersFlashbots#execute: EXCESSIVE_MINER_PAYMENT"
        );

        // At this stage if hasFees the fees have been retained or converted to ETH
        if (structData._fees > 0) _payFees(structData._fees);
    }
    ...
    function _payFees(uint256 _fees) private {
        GELATO.sendValue((_fees * 20) / 100);
