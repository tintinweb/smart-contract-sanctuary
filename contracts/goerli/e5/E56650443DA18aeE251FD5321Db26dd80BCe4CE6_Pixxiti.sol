...
contract Pixxiti is Office, Helpers {
  ...
  function getNextTrackIndex(
    ...
    return (_currentIndex + _length) % _totalTracks;
  }
  ...
  function moveCursor(uint16 _length) private {
    uint16 newIndex = getNextTrackIndex(nextIndex, _length, totalTracks);
    // if we wrap around
    if (newIndex < nextIndex || totalTracks <= _length) {
...
abstract contract Helpers {
  ...
  function getUnitPrice(
    ...
    require(_frameLength >= 1 && _frameLength <= _totalTracks);
    uint minPrice =
