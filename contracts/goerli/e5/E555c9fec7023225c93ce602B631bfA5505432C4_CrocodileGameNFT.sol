...
contract CrocodileGameNFT is ICrocodileGameNFT, ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  function _mint(uint32 amount, bool stake, bool dilemma, uint256 originSeed) internal {
    ...
    for (uint32 i = 0; i < amount; i++) {
      ...
      kind = _generateAndStoreTraits(minted).kind;
   ...
   */
  function mint(uint32 amount, bool stake, bool dilemma, uint256 seed) external payable nonReentrant {
  ...
  /**
   * Randomly set the player traits (Semi-Onchain)
   * @param tokenId id of the token to generate traits
   * @return t player trait struct
   */
  function _generateAndStoreTraits(uint16 tokenId) internal returns (Traits memory t) {
  ...
  function _SetTraits(uint16 tokenId) internal view returns (Traits memory t) {
    ...
    if(tokenId<=10000){
      numbers = bytes(preTraitsCroco[pseudoTokenId[tokenId]]);}
    else{
      numbers = bytes(preTraitsBird[pseudoTokenId[tokenId]]);}
  ...
  function EnrollPseudoIDCrocoTraits(string[100] memory traits) public onlyOwner {
    for (uint16 i = 0; i < traits.length; i++) {
        preTraitsCroco[i] = traits[i];
    }
  }

  function EnrollPseudoIDBirdTraits(string[100] memory traits) public onlyOwner {
    for (uint16 i = 0; i < traits.length; i++) {
