...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _baseURI() internal view virtual returns (string memory) {
        return 'https://gateway.pinata.cloud/ipfs/QmQMNvaEQCnqZZJJEchWur794a6a5TCgC5VDgoPa7nS48J/';
    }
...
contract Boteros is ERC721Enumerable, Ownable {
  ...
  uint256 public constant MAX_BOTEROS = 58;
  ...
  constructor() ERC721('Pedestrians', 'PDE'){
      saleIsActive = true;
  }
  ...
  function mintThreeBotero() external payable {
    require (saleIsActive == true, 'Sale is over');
    require (totalSupply() + 3 <= MAX_BOTEROS, 'Cant mint three frogs');
    require (msg.value >= tripletPrice, '3 frogs costs 0.24 ether');
  ...
  function mintFiveBotero() external payable {
    require (saleIsActive == true, 'Sale is over');
    require (totalSupply() + 5 <= MAX_BOTEROS, 'Cant mint five frogs');
    require (msg.value >= quintoPrice, '5 frogs costs 0.4 ether');

    _mintBotero(5, msg.sender);
  }

  function giftNFT(uint256 numberOfTokens, address recipient) external onlyOwner {

    require (totalSupply() + numberOfTokens <= MAX_BOTEROS, 'Cant mint three frogs');
    _mintBotero(numberOfTokens, recipient);
