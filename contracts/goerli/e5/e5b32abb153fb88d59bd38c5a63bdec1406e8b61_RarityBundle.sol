...
contract RarityBundle is IERC721Receiver {
    ...
    function adventureWithIndex(uint _bundle_index) public {
        ...
        Bundle memory bundle = ownedBundle[msg.sender][_bundle_index];
        require(block.timestamp > bundle.log);
        bundle.log = block.timestamp + DAY;
        for (uint summoner_id = bundle.start_id; summoner_id <= bundle.end_id; summoner_id++) {
