...
// File: contracts\GFarmNftSwapGoerli.sol
...
contract GFarmNftSwapGoerli{
	...
	function setBridgeableNfts(GFarmBridgeableNftInterface[5] calldata _bridgeableNfts) external{
		...
		require(bridgeableNfts[0] == GFarmBridgeableNftInterface(0));
	...
	function getBridgeableNft(uint nftType, uint tokenId) external{
	...
	function getBridgeableNfts(uint nftType, uint[] calldata ids) external{
		...
		for(uint i = 0; i < ids.length; i++){
			// 1. token id corresponds to type provided
			require(leverageToType(nft.idToLeverage(ids[i])) == nftType, "WRONG_TYPE");

			// 2. transfer nft to this contract
			nft.transferFrom(msg.sender, address(this), ids[i]);

			// 3. mint bridgeable nft of same type
			bridgeableNfts[nftType-1].mint(msg.sender, ids[i]);
		}
	}

	// Swap bridgeable nft for unbridgeable nft
	function getNft(uint nftType, uint tokenId) external{
		// 1. Burn bridgeable nft
		bridgeableNfts[nftType-1].burn(tokenId);

		// 2. transfer nft to msg.sender
		nft.transferFrom(address(this), msg.sender, tokenId);
	}

	// Swap bridgeable nft for unbridgeable nfts
	function getNfts(uint nftType, uint[] calldata ids) external{
		// 1. max 10 at the same time
		require(ids.length <= 10, "MAX_10");
		...
		for(uint i = 0; i < ids.length; i++){
			// 1. Burn bridgeable nft
			bridgeableNfts[nftType-1].burn(ids[i]);
