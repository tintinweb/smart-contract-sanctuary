...
contract GStakingManager is AccessControl, ReentrancyGuard {
    ...
    function updateVaultAddress(address _vault) public onlyRole(GUARDIAN_ROLE) {
    ...
    function updatePoolReward(uint256 pid, uint256 amountReward) external onlyRole(GUARDIAN_ROLE) validatePoolById(pid) {
        PoolInfo storage pool = poolInfo[pid];
        require(pool.openTime > block.timestamp, "pool started");
