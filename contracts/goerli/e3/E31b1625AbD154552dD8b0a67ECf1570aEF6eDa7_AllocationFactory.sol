...
contract AllocationFactory is IAllocationFactory, AccessControl {
    ...
    mapping(uint256 => address) public getAllocation;
    ...
    function createAllocation(
        ...
        address _signer
    ) external returns (address allocationAddress) {
        require(hasRole(ADMIN, msg.sender), "AllocationFactory: require ADMIN role");

        if (getAllocation[_id] != address(0)) {
            return getAllocation[_id];
        }

        allocationAddress = _deploy(
            _id,
            _stakeToken,
            _saleToken,
            _startTimeJoin,
        ...
        getAllocation[_id] = allocationAddress;

        emit AllocationCreated(allocationAddress, _id);
    }
    ...
    function _deploy(
        ...
        uint256 _lockDuration,
        address _signer
    ) private returns (address allocationAddress) {
...
contract Allocation is AccessControl, ReentrancyGuard, RedKiteWhitelist {
    ...
    constructor() {
        (
            ...
            uint256 _lockDuration,
            address _signer
        ) = IAllocationFactory(msg.sender).parameters();
        require(
        ...
        require(
            ...
            "StakingPool: invalid stake token address"
        );

        require(
            _endTimeJoin > _startTimeJoin && _startTimeJoin > 0,
            "StakingPool: Invalid join time"
        );

        factory = msg.sender;
        stakeToken = IERC20(_stakeToken);
        allocRewardToken = IERC20(_saleToken);
        allocStartTimeNumber = _startTimeJoin;
        allocEndTimeNumber = _endTimeJoin;
        lockDuration = _lockDuration;
        id = _id;
        signer = _signer;
    }
    ...
    function claimTokens(
        ...
        allocRewardToken.transfer(msg.sender, claimAmount);
