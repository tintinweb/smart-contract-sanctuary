...
contract TwitterPointCounter is UsingWitnet, Ownable {
  ...
  ///@notice Saves an address and a linked twitter account to be queried.
  function subscribe() public {
    require(!users[msg.sender].subscribed);
    string memory id = twitterAuth.getTwitterId(msg.sender);
    require(bytes(id).length > 0);
    users[msg.sender].subscribed = true;
    Subscriber memory subscriber;
    subscriber.twitterId = id;
    subscriber.userAddress = msg.sender;
  ...
  function getPoints() public notPending {
    ...
    if (witnet.isOk(result)) {
      uint64[] memory values = witnet.asUint64Array(result);
      for (uint256 i = 0; i < values.length; i++) {
        liked.push(values[i]);
        users[twitterAuth.addressFromId(uintToString(values[i]))].liked = true;
      }
    } else {
  ...
  ///@notice Returns the twitter id of every subscribed user.
  ///@dev This array is for off-chain use only.
  ///@dev The array size is not bound by anything so DO NOT iterate over it on-chain.
  ///@return An array containing structs with fields "userAddress" and "twitterId"
  function getSubscribers() external view returns (Subscriber[] memory) {
    return subscribers;
  }
  ...
  function getLikers() external view returns (uint64[] memory) {
    return liked;
  }
...
interface ITwitterAuth {
  function getTwitterId(address _user) external returns (string memory);
