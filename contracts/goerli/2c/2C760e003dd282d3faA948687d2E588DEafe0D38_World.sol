...
abstract contract WorldRandomDragon is WorldStorage {
    ...
    function getRandomDragonOwner(uint256 _randomSeed, Location _locationOfEvent) external view override returns(address) {
        uint256 _dragonsAtTG = numberOfDragonsStakedAtLocation(Location.TRAINING_GROUNDS);
        uint256 _dragonsAtRift = numberOfDragonsStakedAtLocation(Location.RIFT);

        uint256 _totalDragons = _dragonsAtTG + _dragonsAtRift;
        if(_totalDragons == 0) {
            return address(0);
        }

        // Would overflow IF we had an insane number of dragons. Not really a possiblity.
        ...
        if(_numberInRange < _dragonsAtTGRange) {
            ...
            _chosenDragonId = dragonAtLocationAtIndex(Location.TRAINING_GROUNDS, _indexOfChosenDragon);
            ...
            _chosenDragonId = dragonAtLocationAtIndex(Location.RIFT, _indexOfChosenDragon);
        } else {
...
abstract contract World721Receiver is IERC721Receiver {
...
abstract contract WorldStorage is Initializable, WorldContracts {
    ...
    function addWizardToWorld(uint256 _tokenId, address _owner, Location _location) external override onlyAdminOrOwner contractsAreSet {
        ...
        wnd.transferFrom(_owner, address(this), _tokenId);
    }
    ...
    function addDragonToWorld(uint256 _tokenId, address _owner, Location _location) external override onlyAdminOrOwner contractsAreSet {
        ...
        tokenIdToLocation[_tokenId] = _location;
        locationToDragonIdSet[_location].add(_tokenId);
        tokenIdToOwner[_tokenId] = _owner;
    ...
    function removeDragonFromWorld(uint256 _tokenId, address _owner) external override onlyAdminOrOwner contractsAreSet {
        ...
        Location _oldLocation = tokenIdToLocation[_tokenId];
        delete tokenIdToLocation[_tokenId];
        locationToDragonIdSet[_oldLocation].remove(_tokenId);
        delete tokenIdToOwner[_tokenId];
    ...
    function numberOfDragonsStakedAtLocation(Location _location) public view override returns(uint256) {
        return locationToDragonIdSet[_location].length();
    }
    ...
    function dragonAtLocationAtIndex(Location _location, uint256 _index) public view override returns(uint256) {
        return locationToDragonIdSet[_location].at(_index);
    }
...
abstract contract WorldContracts is Initializable, IWorld, WorldState {
    ...
    modifier contractsAreSet() {
        require(address(trainingGrounds) != address(0)
            && address(rift) != address(0)
            && address(wnd) != address(0), "Contracts not set");
...
contract WorldState is Initializable, AdminableUpgradeable {

    using EnumerableSet for EnumerableSet.UintSet;

    EnumerableSet.UintSet internal dragonIdSet;
    EnumerableSet.UintSet internal wizardIdSet;
    mapping(uint256 => Location) internal tokenIdToLocation;
    mapping(Location => EnumerableSet.UintSet) internal locationToDragonIdSet;
    mapping(Location => EnumerableSet.UintSet) internal locationToWizardIdSet;
