...
contract HeroSource is ISource, ERC721Enumerable, ReentrancyGuard, Ownable, AccessControl {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant MINTER_ADMIN = keccak256("MINTER_ADMIN");
    mapping (uint256 => uint256) override public seeds;
    ISourceComposer override public composer;

    constructor(address heroComposer_) ERC721("Test", "TEST") {
        composer = ISourceComposer(heroComposer_);
        _setRoleAdmin(MINTER_ROLE, MINTER_ADMIN);
        _setupRole(MINTER_ADMIN, msg.sender);
    }

    function mint(address to) override external nonReentrant {
        require(hasRole(MINTER_ROLE, msg.sender));
        uint256 tokenId = totalSupply() + 1;
        seeds[tokenId] = composer.generateSeed(to, tokenId, composer.currentEdition());
        _safeMint(to, tokenId);
    }

    function mint(address to, uint256 edition) override external nonReentrant {
        require(hasRole(MINTER_ROLE, msg.sender));
        uint256 tokenId = totalSupply() + 1;
        seeds[tokenId] = composer.generateSeed(to, tokenId, edition);
        _safeMint(to, tokenId);
    }
    
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(seeds[tokenId] != 0);
        return composer.compose(tokenId, seeds[tokenId]);
    }
...
interface ISource is IERC721Enumerable {
    ...
    function mint(address to, uint256 edition) external;
}
