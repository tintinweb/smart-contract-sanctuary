...
contract VErc20 is IVErc20, AErc20 {
    ...
    function balancesOf(
        ...
        uint256 underlyingTokens = toUnderlyingTokens(tokens);
    ...
    function mintAndDepositFor(
        ...
        uint256 tokens = toTokens(underlyingTokens);
    ...
    function distributeUnderlyingEquivalent(
        ...
        for (uint i = 0; i < who.length; i++) {
            tokens[i] = toTokens(underlyingTokens[i]);
    ...
    function withdrawAll(
        ...
        address[] memory who = new address[](_withdrawals.length);
        ...
        emit WithdrawnAll(transactionIds, who, tokens, underlyingTokens);
    }
    ...
    function toTokens(
        ...
        return toTokens(IVErc20(address(this)), underlyingTokens);
    }

    function toUnderlyingTokens(
        uint256 tokens
    ) internal view returns (uint256) {
        return toUnderlyingTokens(IVErc20(address(this)), tokens);
    }
}