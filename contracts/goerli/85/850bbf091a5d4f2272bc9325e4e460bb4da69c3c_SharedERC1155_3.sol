...
// for init
    ...
    function create(address to,uint256 nonce,uint256 amount,bool set_sig_disabled,uint256 blockNumber,string calldata extended_uri,bytes memory signature) external returns (uint256 id){
        ...
        id = uint(keccak256(abi.encodePacked(msg.sender,signature)));
...
*/
    ...
    function setURI(uint256 nonce,uint256 id,bool set_sig_disabled,uint256 blockNumber,string calldata extend_uri,bytes memory signature) external{
        bytes32 messagehash = keccak256(abi.encode(msg.sender,nonce,id,set_sig_disabled,blockNumber,extend_uri));
        validate(messagehash,signature,set_sig_disabled,blockNumber);
        extendURI[id] = bytes(extend_uri);
    }
