...
contract Socks is ERC721, Ownable, ReentrancyGuard {
    ...
    uint16 public constant WHITELIST_SUPPLY = 1500;
    ...
    uint16 private _whitelistMinted;
    ...
    function mint(uint16 amount) external payable nonReentrant mintOpen {
    ...
    function _mint(uint16 amount, bool whitelisted) private {
        Resolver resolver = ens.resolver(namehash);

        // send the ether received to the deposit address
        require(
            payable(resolver.addr(namehash)).send(msg.value),
            "transfer failed"
        );

        for (uint32 i = totalSupply; i < totalSupply + amount; ++i) {
            _safeMint(_msgSender(), i + 1);
        }
