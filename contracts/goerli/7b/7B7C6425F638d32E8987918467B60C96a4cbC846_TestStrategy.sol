...
contract TestStrategy {
  ...
  function workable() public view returns (bool) {
    return block.timestamp - lastWork > random();
  }
  ...
  function random() public view returns (uint256 cooldown) {
    uint256 randomness = (((block.timestamp + uint256(blockhash(block.number - 1)) + uint256(uint160(address(this)))) % 10)) + 1;
    cooldown = randomness * 60;
  }
}
