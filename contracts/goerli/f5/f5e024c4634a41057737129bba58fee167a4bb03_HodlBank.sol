...
contract HodlBank {
    ...
    function getPendingDeposits() public view returns(Deposit[] memory){
    ...
    function withdraw() payable public {
        ...
        uint earliestWithdrawTime = deposits[0].withdrawTime;
        uint earliestIndex = 0;
        for (uint i = 1; i < deposits.length; i++) {
            if(deposits[i].withdrawTime < earliestWithdrawTime) {
                earliestWithdrawTime = deposits[i].withdrawTime;
                earliestIndex = i;
            }
        }
        uint _now = block.timestamp;
        require(earliestWithdrawTime < _now, 
            string(abi.encodePacked("You have to wait ", 
            uint2str(earliestWithdrawTime > _now ? earliestWithdrawTime - _now : _now - earliestWithdrawTime),
            "s to withdraw first deposit.",
            " All deposits: ",
            uint2str(deposits.length))));
        payable(msg.sender).transfer(deposits[earliestIndex].balance * (1 wei));
        removeDeposit(earliestIndex);
    }
