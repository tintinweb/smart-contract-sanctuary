...
contract DiscordPollOracle is UsingWitnet {
  ...
  function checkResult(string calldata _id) external payable ended(_id) {
    ...
    bytecode = bytes(
      abi.encodePacked(
        hex"0a5008fc8eeb8b061240122f68747470733a2f2f766f74696e672e6775696c642e78797a2f6170692f7265616374732f",
        _id,
        hex"1a0d82187782186166726573756c741a0210032202100310c0843d186420e80728333080c8afa025"
      )
    );

    WitnetRequest request = new WitnetRequest(bytecode);
  ...
  function writeResult(string calldata _id) external ended(_id) {
    ...
    if (witnet.isOk(result)) {
      ...
      results[_id].push(witnet.asUint64Array(result)[0]);
    } else {
