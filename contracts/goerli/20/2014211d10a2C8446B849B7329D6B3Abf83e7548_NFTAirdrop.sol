...
import "./DCRoleToken.sol";
import "../utils/Freezable.sol";

contract NFTAirdrop is Ownable, Freezable {
    ...
    event Claimed(string indexed serverId, string roleId, address tokenContract, address user);
    event Granted(string indexed serverId, string roleId, address tokenContract, address user);
    ...
    function deployTokenContract(string calldata _tokenName, string calldata _tokenSymbol) external {
        address roleToken = address(new DCRoleToken(_tokenName, _tokenSymbol, externalUrl));
    ...
    function newAirdrop(
        ...
        for (uint256 i = 0; i < _roles.length; i++) {
            if (!claimables[_serverId][_roles[i].roleId][roleToken].dropped) {
                ...
                DCRoleToken(roleToken).addRoleData(
                    _serverId,
                    _roles[i].roleId,
        ...
        require(drop.roleIds.length > 0, "No valid roles");
    ...
    function stopAirdrop(
        ...
        // Check the signature
        bytes32 message = keccak256(abi.encode(address(this), _serverId, _roleId, roleToken, msg.sender))
            .toEthSignedMessageHash();
        require(message.recover(_signature) == bot, "Not valid signature");
    ...
    function claim(
        ...
        Claim storage userClaim = claims[msg.sender][_serverId][_roleId][tokenAddress];
        require(claimables[_serverId][_roleId][tokenAddress].active || userClaim.approved, "No active airdrop");
        require(!userClaim.claimed, "Already claimed");

        // Check the signature
        bytes32 message = keccak256(abi.encode(address(this), _serverId, _roleId, tokenAddress, msg.sender))
            .toEthSignedMessageHash();
        require(message.recover(_signature) == bot, "Not valid signature");
        ...
        DCRoleToken(tokenAddress).safeMint(msg.sender, _serverId, _roleId);
        emit Claimed(_serverId, _roleId, tokenAddress, msg.sender);
    }
    ...
    function grant(
        ...
        emit Granted(_serverId, _roleId, roleToken, _user);
    }
...
contract DCRoleToken is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    mapping(uint256 => Role) public roleOfToken;
    mapping(string => mapping(string => Data)) private roleData;
    ...
    function safeMint(
        ...
        roleOfToken[_tokenIdCounter.current()].serverId = _serverId;
        roleOfToken[_tokenIdCounter.current()].roleId = _roleId;
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        Data storage data = roleData[roleOfToken[_tokenId].serverId][roleOfToken[_tokenId].roleId];

        string memory link = string(abi.encodePacked(_baseURI(), data.imageHash));
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "',
                        data.tokenName,
                        '", "description": "Represents a role in a discord server.", "image": "',
                        link,
                        '", "external_url": "',
                        externalUrl,
                        _tokenId.toString(),
                        '", "attributes": [{"trait_type": "Server Id", "value": "',
                        roleOfToken[_tokenId].serverId,
                        '"}, {"trait_type": "Role Id", "value": "',
                        roleOfToken[_tokenId].roleId,
                        '"}',
                        tokenAttributesToMetadata(_tokenId),
                        "]}"
                    )
                )
            )
    ...
    function addRoleData(
        ...
        roleData[_serverId][_roleId].imageHash = _tokenImageHash;
        roleData[_serverId][_roleId].tokenName = _tokenName;
        for (uint256 i = 0; i < _traitTypes.length; i++) {
            roleData[_serverId][_roleId].attributes.push(Attribute(_traitTypes[i], _values[i]));
        }
    }
    ...
    function tokenAttributesToMetadata(uint256 _tokenId) internal view returns (string memory) {
        Attribute[] storage attributes = roleData[roleOfToken[_tokenId].serverId][roleOfToken[_tokenId].roleId]
            .attributes;
    ...
    function getDataOfRole(string calldata _serverId, string calldata _roleId)
        external
        view
        returns (
            string memory,
    ...
    {
        Data storage data = roleData[_serverId][_roleId];

        return (data.imageHash, data.tokenName, data.attributes);
    }
}
