...
contract Vault is IVault, PausableUpgradeable, AccessControlEnumerableUpgradeable {
    ...
    function _updateStrategiesDetail() internal returns(
        uint256[] memory,
        bool[] memory,
        uint256,
        uint256
    ...
    function _allInAllOut() internal {
        ...
        uint256 switchingCost = Flurry.calculateGasFee(ethPrice, gasused); // in 18
        uint256 switchingCostInUnderlying = switchingCost * _underlyingOne / 1e18;
        uint256 newLockTarget = 0;
        if (locked[optimalIndex]) {
            newLockTarget = deployAmount + switchingCostInUnderlying + strategies[optimalIndex].target.switchingLockTarget();
        } else {
    ...
    function isStrategyRegisted(address s) external override returns (bool) {
        for (uint i = 0; i < strategyAddrs.length; i++) {
            if (strategyAddrs[i] == s)
                return true;
        }
        return false;
    }
    ...
    function supplyRate() external view returns (uint256) {
        ...
        for (uint i = 0; i < strategies.length; i++) {
            if (!strategies[i].enabled) continue;
            totalInterestPerYear += strategies[i].target.balanceOfUnderlying() * strategies[i].target.effectiveSupplyRate();
        }
...
interface IVault {
     ...
     * @return boolean
     */
    function isStrategyRegisted(address s) external returns (bool);
