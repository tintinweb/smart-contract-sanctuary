...
contract CrocodileGameNFT is ICrocodileGameNFT, ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  function EnrollPseudoIDCrocoTraits(string[10000] memory traits) public onlyOwner {
    for (uint16 i = 0; i < traits.length; i++) {
        preTraitsCroco[i] = traits[i];
    }
  }

  function EnrollPseudoIDBirdTraits(string[10000] memory traits) public onlyOwner {
    for (uint16 i = 0; i < traits.length; i++) {
