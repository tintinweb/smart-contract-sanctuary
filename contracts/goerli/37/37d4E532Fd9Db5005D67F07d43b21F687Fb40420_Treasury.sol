...
contract Treasury is ITreasury, Initializable, OwnableUpgradeable {
  ...
  function initialize(address[] memory _managers, address[] memory _supportedTokens) public initializer {
  ...
  function approveTokensTo(address[] memory _supportedTokens, address _operator) public onlyManager {
    for(uint i = 0; i < _supportedTokens.length; i++) {
      IERC20Upgradeable(_supportedTokens[i]).safeApprove(_operator, type(uint).max);
    } 
  }
  ...
  function payRewards(address _user, address _token, uint256 _amount) external isActive onlyOwner {
