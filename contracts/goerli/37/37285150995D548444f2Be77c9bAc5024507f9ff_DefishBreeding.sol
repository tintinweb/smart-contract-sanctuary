...
contract DefishBase is ERC721Enumerable, DefishDependency, DefishAccessControl {
  string public tokenURIPrefix = "https://assets.de.fish/fish/";
  string public tokenURISuffix = "/image.png";
  ...
  function _beforeTokenTransfer(address from, address to, uint256 tokenId) whenNotPaused whenTransferAllowed(from, to, tokenId) internal virtual override {
        super._beforeTokenTransfer(from, to, tokenId);
    }
}
...
contract DefishBreeding is Ownable, Pausable {
    ...
    function _triggerCooldown(Fish memory _fish) internal returns (uint256){
        ...
        // Increment the breeding count, clamping it at 7, which is the length of the
        // cooldowns array. We could check the array size dynamically, but hard-coding
        // this as a constant saves gas. Yay, Solidity!
        if (_fish.breedTimes < 7) {
            _fish.breedTimes += 1;
        }
    ...
    function doBreed(Fish memory matron, Fish memory sire) internal returns (uint256 fishId){
        ...
        uint256 delay = _triggerCooldown(sire);
        // New fish will be delay alive depend on its parents breedTimes
        delay += _triggerCooldown(matron);
...
contract DefishCore is DefishBase {
  ...
  event UpdateBreeding(uint256 indexed _fishId, uint256 _nextBreed, uint8 _breedTimes);
  ...
  function updateBreedingProfile(uint256 _fishId, uint256 _nextBreed, uint8 _breedTimes) public onlySpawner whenNotPaused {
    fishes[_fishId].nextBreed = _nextBreed;
    fishes[_fishId].breedTimes = _breedTimes;
    emit UpdateBreeding(_fishId, _nextBreed, _breedTimes);
  }
}
