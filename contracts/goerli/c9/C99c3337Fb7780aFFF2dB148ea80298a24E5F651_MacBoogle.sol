...
contract MacBoogle is ERC721 , Ownable, Pausable{
    using Strings for uint256;
    using SafeMath for uint256;
    ...
    constructor(
        ) ERC721("MacBoogle Token Mk II","MBT2") {}

    
    function allocateManyCards(address[] memory buyers) external onlyOwner {
        for (uint j = 0; j < buyers.length; j++) {
            allocateCard(buyers[j]);
        }
    }
    
    function allocateCard(address buyer) public onlyOwner {
        _mint(buyer,nextTokenId++);
    }

    function allocateManyToOne(address buyer, uint num) public onlyOwner {
    ...
    function setTokenId(uint newTokenId) external onlyOwner {
        require(newTokenId > nextTokenId, "Up Up Up");
