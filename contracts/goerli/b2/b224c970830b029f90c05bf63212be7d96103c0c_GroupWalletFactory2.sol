...
contract GroupWalletFactory2 {
    ...
    function domainReport(string calldata _dom,uint command) external payable returns (uint256 report, bytes memory structure) { 
      ...
      if (hasCommitment) {
        (abi32type, abi32) = resolverContract.ABI(dHash,32);                                            // isABIstructure
        if ((abi32type == 32) && (uint256(abi32.length)>32) && (uint256(abi32.length)<0x1000)) report = uint256(uint(report)+32);

        (uint256 abi128type, bytes memory abi128) = resolverContract.ABI(dHash,128);                    // isABI128
        if ((abi128type == 128) && (uint256(abi128.length)>=224) && ((abi128.length%32)==0)) report = uint256(uint(report)+128);

        //if (abi128.length>0) nb = uint256(uint256((uint256(abi128.length)-uint256(0x80))>>5)-1);        // nb of members derived from ABI128 // abi128type = 0x80; len = 080 + (nbOfMem+1)*32
    ...
    //  if (command == 3) return (abi32type,abi32);
    //  if (command == 4) return resolverContract.ABI(dHash,128);
    }
