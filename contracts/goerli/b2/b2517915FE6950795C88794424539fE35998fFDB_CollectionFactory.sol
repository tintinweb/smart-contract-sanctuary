...
contract Collection is Ownable, ERC721 {
    ...
    event ItemCreated(address indexed owner, uint256 indexed tokenId);
    ...
    bytes4 public constant _INTERFACE_ID_ERC721_VERIFY_FINGERPRINT =
        bytes4(keccak256("verifyFingerprint(uint256,bytes32)"));
     ...
     */
    function create(string calldata _metaDataURI, string calldata _metaData)
        external
    {
        require(
            msg.sender == owner(),
            "Only collection owner can mint NFT in this collection"
        );
    ...
    function getMetaDataHash(string memory _metaData)
        public
        pure
        returns (bytes32)
    {
    ...
    function verifyFingerprint(uint256 _tokenId, bytes32 _fingerprint)
        public
        view
        returns (bool)
    {
...
contract CollectionFactory {
    ...
    function getCollectionsCount()
        public
        view
        returns (uint256 collectionsCount)
    {
    ...
    function createCollectionContract(
        ...
        Collection collectionContract =
            new Collection(_name, _symbol, _baseUri, msg.sender);
        collections.push(address(collectionContract));
        emit CollectionCreated(msg.sender, address(collectionContract));
