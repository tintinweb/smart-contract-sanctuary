...
contract StakingPoolImpl is StakingPool {
        ...
        returns (uint256 stakedBalance)
    {
        ...
        uint256 shares;
        if (b.stakingVoucher.queueEpoch + 2 <= currentStakeEpoch) {
            shares = b.stakingVoucher.amountQueued.mul(
                stakingVoucherValueAtEpoch[b.stakingVoucher.queueEpoch]
            );
            if (b.stakingVoucher.queueEpoch > 0) {
                shares = shares.add(
                    b.stakingVoucher.amountStaked.mul(
                        stakingVoucherValueAtEpoch[
                            b.stakingVoucher.queueEpoch - 1
                        ]
                    )
                );
            }
            ...
            b.stakingVoucher.queueEpoch > 0
        ) {
            shares = b.stakingVoucher.amountStaked.mul(
                stakingVoucherValueAtEpoch[b.stakingVoucher.queueEpoch - 1]
            );
        }
    ...
    function _updateUserBalances(address _user) internal {
        ...
        if (userLastUpdateEpoch + 1 == currentStakeEpoch) {
            ...
            if (user.stakingVoucher.amountStaked != 0) {
                // old maturing is now staked
                uint256 shares =
                    user.stakingVoucher.amountStaked.mul(
                        stakingVoucherValueAtEpoch[userLastUpdateEpoch - 1]
                    );
                user.stakedPoolShares = user.stakedPoolShares.add(shares);
            }
            // this is the only place stakingVoucher.amountStaked is ever set other than zeroed
            ...
            // stakedPoolShares += stakingVoucher.amountQueued * w1;
            require(
                user.stakingVoucher.amountStaked == 0,
                "@TODO Remove this check; At this point, there must be no balance maturing."
            );
            uint256 shares =
                user.stakingVoucher.amountQueued.mul(
                    stakingVoucherValueAtEpoch[userLastUpdateEpoch]
                );
            user.stakedPoolShares = user.stakedPoolShares.add(shares);
        } else if (userLastUpdateEpoch + 2 < currentStakeEpoch) {
            // a lot of time has passed
            // stakedPoolShares += stakingVoucher.amountStaked * w2 + stakingVoucher.amountQueued * w1;
