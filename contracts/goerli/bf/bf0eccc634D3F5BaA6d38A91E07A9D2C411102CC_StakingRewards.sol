...
contract StakingRewards is Ownable{
    ...
    uint256 public apy = 100e16; // 100e16 is 100% a year
    uint256 public timeLock = 30 * 86400; // 1 day - 86400 seconds
    ...
    function balances(address account) external view onlyOwner returns(uint256){
        return _balances[account];
    }

    function timeToUnlock() external view returns(uint256){
        uint256 timer = block.timestamp - _lastStakeTime[msg.sender];
        if(timer < timeLock){
            return timer;
        }else{
            return 0;
