...
contract LeedoNftVault is ERC721Enumerable, Ownable { 
    ...
    function _claimRewards(address account) private  {
        ...
        if (amount > 0) {
            ...
            require(IERC20(erc20Addr).transfer(account, amount), 'SquidVault: Squid Transfer failed');
        }        
    }  
    ...
    function _claimBridgeRewards(uint tokenId, address to) private {
        ...
        if (amount > 0) {
            ...
            require(IERC20(erc20Addr).transfer(to, amount), 'SquidVault: Squid Transfer failed');
        }        
    }       
    ...
    function stakeByCount(uint256 count) external returns (uint256[] memory){
        claimRewards();
        ILeedoNft nft = ILeedoNft(nftAddr);
        
        require(count <= nft.balanceOf(_msgSender()), 'NftVault: Count is more than the owner has');
    ...
    function tokensOf(address account) public view returns (uint[] memory) {
        uint[] memory tokenIds = new uint256[] (balanceOf(account));
        for (uint i; i<balanceOf(account); i++) {
