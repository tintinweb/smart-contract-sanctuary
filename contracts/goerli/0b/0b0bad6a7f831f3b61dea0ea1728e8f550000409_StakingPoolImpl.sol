...
contract StakingPoolImpl is StakingPool, StakingPoolManagementImpl {
    ...
    ) payable StakingPoolManagementImpl(_ens, _workerManagerAddress, _pos) {
    ...
    /// @notice this makes sure any funds coming to the pool
    /// goes to the poolManager or they get rejected for out-of-gas
    /// really, this is to enable the retirement process of a node
        ...
        returns (uint256 stakedBalance)
    {
        ...
        if (totalStakedShares > 0) {
            ...
            return _getStakedSharesInValue(shares);
        }
        return 0;
    }
    ...
    function _updateUserBalances(address _user) internal {
        ...
        user.stakedPoolShares = user.stakedPoolShares.add(
            _getUserMaturatedShares(user.stakingVoucher)
        );
...
contract StakingPoolManagementImpl is StakingPoolManagement, Ownable {
    ...
    ENS immutable ens;
    WorkerManagerAuthManagerImpl immutable workerManager;
    ...
    constructor(
        ...
        ens = ENS(_ens);
        // @dev if we dont assign to a temp variable .hire and .authorize wont work
        WorkerManagerAuthManagerImpl _workerManager =
            WorkerManagerAuthManagerImpl(_workerManagerAddress);
        workerManager = _workerManager;

        // pool needs to be both user and worker
        _workerManager.hire{value: msg.value}(payable(address(this)));
        _workerManager.authorize(address(this), _pos);
    }
