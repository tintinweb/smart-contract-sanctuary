...
// File: contracts/JohnGovernance.sol
...
/// @title Eleven-Yellow JOHN governance service
/// @notice Staked JOHN tokens represent a weighting for influence on governance issues
contract JohnGovernance is OwnableUpgradeable {
    ...
    /// @param john_ ERC20 contract address of JOHN token
    /// @dev JOHN token contract address is initialized
    function initialize(address john_) public initializer {
        __Ownable_init();
        john = john_;
    }

    /// @notice Stake JOHN tokens for governance purposes
    /// @param amount_ the amount of JOHN tokens to stake
    /// @dev Stake requires approval for governance contract to transfer & hold JOHN tokens
    function stake(uint256 amount_) public virtual {
        require(amount_ > 0, "Invalid amount");
        userStakes[msg.sender] = userStakes[msg.sender].add(amount_);
        IERC20(john).transferFrom(msg.sender, address(this), amount_);
    ...
    function unstake() public virtual {
        ...
        IERC20(john).transfer(msg.sender, amount);
    ...
    /// @dev Total token balance is recovered; only excess non-staked tokens in case of JOHN
