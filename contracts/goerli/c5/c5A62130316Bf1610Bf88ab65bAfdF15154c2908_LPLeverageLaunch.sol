...
contract LPLeverageLaunch {
  ...
  mapping (address => uint256) totalLoaned;
  ...
  function getTotalLoaned(address token) external view returns (uint256 total) {
      total = totalLoaned[token];
  } 
  ...
  function lendLiquidity( address loanedToken, uint amount ) external returns ( bool success ) {
    ...
    totalLoaned[loanedToken] = totalLoaned[loanedToken] + amount;
    success == true;
  ...
  function lendLiquidity() external payable returns ( bool success ) {
    ...
    totalLoaned[address(0)] = totalLoaned[address(0)] + msg.value;
    success == true;
