...
/**
 * @title Korean Consonants Card NFT 
...
contract KoCoCard is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint private constant _TIMELOCK_DURATION = 2 minutes;
    ...
    function tokenURI(uint256 _tokenId) override public view returns (string memory) {
        ...
        parts[2] = '<rect width="100%" height="100%" fill="#57CF5C" />';
    ...
    constructor() ERC721("Korean Consonants Card", "KoCoCard") Ownable() {
        mintPickSize();
...
contract MorphedCard {
    ...
    address private kocoCardAddress;
    address private deployer;

    constructor() public {
        deployer = msg.sender;
        kocoCardAddress = 0xC814aFD17170d2A8c02C9f0E8B7bA8Bf96aB75Ff;
    }

    function updateMainCardAddress(address addr) external {
        require(addr != address(0), "Bad ChildChainManagerProxy address");
        require(msg.sender == deployer, "You're not allowed");
        kocoCardAddress = addr;
    }

    function ownerOf(uint256 _tokenId) public view returns (address) {
        return KoCoCard(kocoCardAddress).ownerOf(_tokenId);
    }

    function getGenes(uint256 _tokenId) public view returns (uint8[8] memory) {
        return KoCoCard(kocoCardAddress).getGenes(_tokenId);
    }

    function tokenURI(uint256 _tokenId) public view returns (string memory) {
        return KoCoCard(kocoCardAddress).tokenURI(_tokenId);
    }

    function getConsonants(uint256 _tokenId) public view returns (string[3] memory) {
        return KoCoCard(kocoCardAddress).getConsonants(_tokenId);
    }
    function getConsonantsIndex(uint256 _tokenId) public view returns (uint8[3] memory) {
        return KoCoCard(kocoCardAddress).getConsonantsIndex(_tokenId);
    }


    function customizedTokenURI(uint256 _tokenId, string calldata _bgImageURI, string calldata _fontColor) external view returns (string memory) {      

        string memory bgImageURI = _bgImageURI;  
        string memory fontColor = _fontColor;  
        
        string[7] memory parts;
        string[24] memory attrParts;
        uint8[8] memory geneArray = getGenes(_tokenId);
        string[3] memory consArray = getConsonants(_tokenId);
        uint8[3] memory consIndex = getConsonantsIndex(_tokenId);        
        ...
        // white
        parts[1] = ';}</style><style>.base {font-family: Verdana; fill: white;}</style><image width="100%" height="100%" xlink:href="';
        // url
        ...
        // string memory output = string(abi.encodePacked(parts[0], parts[1], fontColor, parts[2], parts[3], bgImageURI, parts[4], parts[5], parts[6]));
        string memory output = string(abi.encodePacked(parts[0], parts[1], fontColor, parts[2], parts[3], bgImageURI));
        ...
        attrParts[21] = toString(geneArray[7]);
        if (consIndex[0] == 7 && consIndex[1] == 14 && consIndex[2] == 4) {
            attrParts[22] = '"}, {"trait_type": "Ojingeo", "value": "Y"}, {"trait_type": "Same Consonants", "value": "';
        } else {
            attrParts[22] = '"}, {"trait_type": "Ojingeo", "value": "N"}, {"trait_type": "Same Consonants", "value": "';
        }
        if (consIndex[0] == consIndex[1] && consIndex[0] == consIndex[2]) {
            attrParts[23] = 'Y"}]';
        } else {
        ...
        output = string(abi.encodePacked(attrParts[0], attrParts[1], attrParts[2], attrParts[3], attrParts[4], attrParts[5], attrParts[6], attrParts[7]));
        output = string(abi.encodePacked(output, attrParts[8], attrParts[9], attrParts[10], attrParts[11], attrParts[12], attrParts[13], attrParts[14]));        
        output = string(abi.encodePacked(output, attrParts[15], attrParts[16], attrParts[17], attrParts[18], attrParts[19], attrParts[20]));        
        output = string(abi.encodePacked(output, attrParts[21], attrParts[22], attrParts[23]));        

        // string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Korean Consonants Card NFT #', toString(_tokenId), '", "attributes": ', attrs ,', "description": "The Korean Consonants cards are invitation to enter the adventurous and mysterious metaverse games. Genes characteristics and other functionality are intentionally omitted for unlimited imagination and community-driven game development. Start your journey now!", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
        // output = string(abi.encodePacked('data:application/json;base64,', json));
        output = Base64.encode(bytes(string(abi.encodePacked('{"name": "Korean Consonants Card NFT #', toString(_tokenId), '", "attributes": ', output ,', "description": "The Korean Consonants cards are invitation to enter the adventurous and mysterious metaverse games. Genes characteristics and other functionality are intentionally omitted for unlimited imagination and community-driven game development. Start your journey now!", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
        output = string(abi.encodePacked('data:application/json;base64,', output));
        return output;
    }

    function morphedTokenURI(uint256 _tokenId) external view returns (string memory) {
        ...
        parts[2] = '<image width="100%" height="100%" xlink:href="https://res.feednews.com/assets/v2/7a1938ec6c38369cf2b951d604ca86dd?width=1280&height=720&quality=hq&category=us_News_Entertainment"/>';
        parts[3] = '<text x="50%" y="100" dominant-baseline="middle" text-anchor="middle" class="base" style="font-size:700%; letter-spacing: -0.2em;">';
        parts[4] = string(abi.encodePacked(consArray[0], ' ', consArray[1], ' ', consArray[2]));
        parts[5] = '</text><text x="50%" y="180" dominant-baseline="middle" text-anchor="middle" class="base" style="font-size:150%;">&#937; ';
        parts[6] = string(abi.encodePacked(toString(geneArray[0]), toString(geneArray[1]), toString(geneArray[2]), toString(geneArray[3]), ' '));
