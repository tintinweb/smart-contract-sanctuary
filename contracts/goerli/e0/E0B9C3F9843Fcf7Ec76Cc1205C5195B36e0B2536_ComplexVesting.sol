...
/**
  ...
  Here, we cover the case of complex vesting contract with a fixed rate of decay over a time period
...
contract ComplexVesting is Ownable, ReentrancyGuard {
    ...
    // length of the interval to adjust the vesting rate, in terms of vesting periods
    uint256 public immutable decayPeriodLength;
    ...
    constructor(
        ...
        decayPeriodLength = _decayPeriodLength;
        decayQty = _decayQty;
        baseQty = _baseQty;
    }
    ...
    function _calculateVestingAmount(
        ...
        /**
            Compute the cumulative amount vested by incrementing a `vestingAmount` variable.
            Each year, the amount vesting per month declines, according to the decay

            @param elapsedDecayPeriods is the number of whole years the contract has been running
            @param elapsedCarryover is the months in the current yera
        ...
        for (uint256 i; i <= elapsedDecayPeriods; i++) {
            ...
            if (i < elapsedDecayPeriods) {
                vestingAmount += periodVestingQty * decayPeriodLength;
    ...
    {
        /**
      Transfer all tokens currently vested to the whitelisted account.  
     */
    ...
    {
        /**
      Withdraw the full token balance of the contract to a the owner
