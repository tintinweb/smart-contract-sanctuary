...
contract Quiz {
  ...
  function evaluateQuiz(uint256[] memory _correctOptions, string memory _salt) public onlyOwner finishedQuiz {
    for (uint256 i = 0; i < _correctOptions.length; i++) {
      ...
      bytes32 h = keccak256(abi.encodePacked(qIdx, correctOption, _salt));
    ...
    // TODO do we need "evaluated" quiz status?
    // TODO consider if we just transfer and quiz statos to "over"
