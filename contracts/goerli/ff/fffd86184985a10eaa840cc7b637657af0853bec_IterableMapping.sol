...
contract Reflect is ERC20, Ownable {
    ...
    uint256 public constant TOTAL_SUPPLY = 1000000000 * (10**9);
    ...
    uint256 public constant TOTAL_FEES = ETH_REWARDS_FEE + LIQUIDITY_FEE + MARKETING_FEE + DEV_FEE;
    ...
    constructor() ERC20("Reflect", "RFT") {
        assert(TOTAL_FEES == 70);
    ...
    function _transfer(
        ...
        if (tradingIsEnabled &&
            ...
            uint256 swapTokens = contractTokenBalance.mul(TOTAL_FEES).div(1000);
        ...
        if (takeFee) {
            uint256 taxAmount = amount.mul(TOTAL_FEES).div(1000);
            uint256 liquidityAmount = amount.mul(TOTAL_FEES).div(1000);
            uint256 marketingAmount = amount.mul(MARKETING_FEE).div(1000); 
            uint256 devAmount = amount.mul(DEV_FEE).div(1000); 

            require(taxAmount == (liquidityAmount + marketingAmount + devAmount), "Reflect::transfer: Tax value invalid");

            uint256 sendAmount = amount.sub(taxAmount);
            
            require(amount == sendAmount + taxAmount, "Reflect::transfer: Tax value invalid");

            super._transfer(from, address(marketingWallet), marketingAmount);
            super._transfer(from, address(devWallet), devAmount);
