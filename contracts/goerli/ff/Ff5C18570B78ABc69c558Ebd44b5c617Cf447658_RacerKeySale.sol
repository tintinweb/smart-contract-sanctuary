    ...
    ContextMixin
{
    ...
    function canRedeemKey(address account, uint256 amount)
        public
        view
        returns (bool canRedeem)
    {
        canRedeem = true;
        if (balanceOf(account, KEY) < amount) {
            canRedeem = false;
        }
    }
    ...
    {
        require(
            canRedeemKey(account, amount),
            "balance is too low to redeem keys"
        );
        _burn(account, KEY, amount);
        wantsRedemption[account] = wantsRedemption[account] > amount
            ? wantsRedemption[account] - amount
            : 0;
    }
