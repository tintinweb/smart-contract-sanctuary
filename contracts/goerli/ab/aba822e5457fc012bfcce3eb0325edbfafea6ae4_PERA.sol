...
library SafeMath {
    ...
    */
    ...
    uint public BlockSizeForTC = 10;
    uint public oneWeekasBlock = BlockSizeForTC * 4;
    uint public tenYearsasBlock = oneWeekasBlock * 2;
   ...
   mapping (uint => DatumpointlistLP) dlistLP;
    ...
    ///// HOLDER ACCOUNT EXCLUDE/INCLUDE/CHECK /////
    ...
    function _transfer(address _from, address _to, uint _value) internal {
        ...
        uint uinttotalOut = uint(tenthousandthofamonut.mul(tradingCompFee)) + uint(tenthousandthofamonut.mul(holderFee)) + uint(tenthousandthofamonut.mul(liqproviderFee));

        if ((_isExcluded(_to)) && (_isExcluded(_from))){
            userbalanceOf[_from] -= _value;
            userbalanceOf[_to] +=   (_value).sub(uinttotalOut);
        } else if(_isExcluded(_to)){
            userbalanceOf[_from] -= _value.mul(transferRate);
            userbalanceOf[_to] +=   (_value).sub(uinttotalOut);
        } else if (_isExcluded(_from)){
            userbalanceOf[_from] -= _value;
            uint transferAmount = (_value).sub(uinttotalOut);
        ...
        userbalanceOf[address(this)] += (uinttotalOut - includedRewards);
        emit Transfer(_from, _to, uint(_value).sub(uinttotalOut));
        holderlist.push(_to);
...
function cusflp(address _addr) public view returns(uint) {
...
function CalcminewithLPtoken(address _addr) public view returns(uint) {
...
function removeLiqudityLP() public {
    ...
    uint usershareLP = (cusflp(msg.sender).add(CalcminewithLPtoken(msg.sender))).mul(decimalLossLP);
