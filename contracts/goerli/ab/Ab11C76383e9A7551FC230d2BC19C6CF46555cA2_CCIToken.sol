...
contract CCIToken is Context, ERC20 {
    ...
    address feeAddress, uint16 feePercent) ERC20("GemFi.vip", "CCI") {
        require(_feePercent <= 10000, "CCIToken: input value is more than 100%");
    ...
    function burnFrom(address account, uint256 amount) public virtual {
        ...
        require(currentAllowance >= amount, "CCIToken: burn amount exceeds allowance");
        unchecked {
    ...
    function setAccountTransactionBlockLock(address account, uint256 amount) internal virtual {
        require(_freezeBlock[account] == 0, "CCIToken: The account has received the balance and locked it, please change the account to receive");
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0)) {
            require(amount <= super.balanceOf(from) - getFreezeBalance(from), "CCIToken: The transfer amount is greater than the available amount");
        }
    }
