...
library ImpactConstants {
    ...
    address private constant _tokenOwner = 0x59eB4bd56409997415528A7d16495a7Ff79e40D7;
        ...
        if (_tradingTime == 0) {
            ...
            if (sender == launchPool) {
                ...
                if (timeDelay <= ImpactConstants.getMaxLock()) {
                    _lockTime[recipient] = now.add(ImpactConstants.getMaxLock().sub(timeDelay));
                }
                uint256 unlockAmount = amount.mul(curPriceMultiple)/(10**3 * ImpactConstants.getUnlockMultiple());
                if (unlockAmount <= amount) {
                    _lockedAmount[recipient] = amount.sub(unlockAmount);
                }
            }
        } else {
            if (!_isWhitelisted[sender] && _lockTime[sender] >= now) {
                require(amount.add(_lockedAmount[sender]) <= _balances[sender], "IMPACT: locked balance");
            }
        }
        _;
    }
