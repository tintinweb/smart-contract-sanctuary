...
contract CompoundStrategy is IRhoStrategy, PausableUpgradeable, AccessControlEnumerableUpgradeable {
    ...
    // uint256 public constant BLOCK_PER_YEAR = 2380075;
    ...
    function supplyRate() external view override returns (uint256 _amount) {
    ...
    function _bonusPerBlockPerUnderlying() internal view returns(uint256) {
        uint256 compPerBlockPerCtoken = comptroller.compSpeeds(address(cToken)) * 10**cToken.decimals() / cToken.totalSupply(); // 18, per cToken
        ...
        uint256 usdPerBlockPerUnderlying = compPerBlockPerUnderlying * Flurry.getPriceFromChainlink(compPriceFeed) / 10**compPriceFeed.decimals();
    ...
    function swapToken(uint256 amountIn, uint256 amountOutMin, address[] memory path, uint256 deadline) external onlyRole(VAULT_ROLE) {
        require(
