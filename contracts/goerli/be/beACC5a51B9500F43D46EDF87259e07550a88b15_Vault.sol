...
contract GameController is Initializable, AccessController {
    ...
    event AddGame(
        address indexed newGameAddress,
        string newGameName,
        bool isActive
    );

    event UpdateGameAddress(
        uint8 indexed gameIndex,
        address indexed oldGameAddress,
        address indexed newGameAddress
    );

    event UpdateGameName(uint8 indexed gameIndex, string newGameName);

    event EnableGame(uint8 indexed gameIndex, bool isEnable);
    ...
    function addGame(
        ...
        require(
            _newGameAddress != address(0),
            "Vault.sol:addGame, new game address can't be address(0)"
        );
        require(
        ...
        emit AddGame(_newGameAddress, _newGameName, _isActive);
    }
        ...
        onlyDeclaredGame(_gameIndex)
    {
        require(
            _newGameAddress != address(0),
            "Vault.sol:updateGameAddress, new game address can't be address(0)"
        );
        require(
            settings[_newGameAddress].status == GameStatus.Empty,
        ...
        emit UpdateGameAddress(_gameIndex, oldGameAddress, _newGameAddress);
    }
        ...
        onlyDeclaredGame(_gameIndex)
    {
        ...
        emit UpdateGameName(_gameIndex, _newGameName);
    }
        ...
        onlyDisabledGame(_gameIndex)
    {
        ...
        emit EnableGame(_gameIndex, true);
    }
        ...
        onlyEnabledGame(_gameIndex)
    {
        ...
        emit EnableGame(_gameIndex, false);
    }
}
...
contract TokenController is Initializable, AccessController {
    ...
    event NewGameToken(address indexed tokenAddress);

    function __TokenController_init() public initializer {
        AccessController.__AccessController_init();
    }

    function setToken(address _tokenAddress) internal onlyAdmin {
        require(
            _tokenAddress != address(0),
            "Vault.sol: Token Address can't be zero address."
        );

        tokenAddress = _tokenAddress;

        emit NewGameToken(_tokenAddress);
    }
...
contract Vault is GameController, TokenController, IVault {
    ...
    event AddGameBalance(address indexed player, uint128 amount);

    event SubGameBalance(address indexed player, uint128 amount, uint128 fee);

    event TokenInboundTransfer(address indexed player, uint128 amount);

    event TokenOutboundTransfer(address indexed player, uint128 amount);

    event WithdrawFee(uint8 indexed gameIndex, address to, uint128 amount);

    event DepositFund(uint8 indexed gameIndex, uint128 amount);

    event WithdrawFund(uint8 indexed gameIndex, address to, uint128 amount);

    event DeleteGame(uint8 indexed gameIndex);
        ...
        onlyRole(GAME_ROLE)
    {
        ...
        emit AddGameBalance(_player, _amount);
    }
    ...
    function subGameBalance(
        ...
        emit SubGameBalance(_player, _amount, _fee);
    }
        ...
        checkBalance(msg.sender, _amount)
    {
        ...
        emit TokenInboundTransfer(msg.sender, _amount);
    }
    ...
    function tokenOutboundTransfer(uint128 _amount) external {
        ...
        if (uint256(_amount) > tokenBal) {
            ...
            emit TokenOutboundTransfer(msg.sender, uint128(tokenBal));
        } else {
            balances[msg.sender] -= _amount;
            token.safeTransfer(msg.sender, _amount);

            emit TokenOutboundTransfer(msg.sender, _amount);
        }
    }
    ...
    function withdrawFee(uint8 _gameIndex, uint128 _amount) external onlyAdmin {
        ...
        emit WithdrawFee(_gameIndex, owner(), amount);
    }
        ...
        checkBalance(msg.sender, _amount)
    {
        ...
        emit DepositFund(_gameIndex, _amount);
    }
        ...
        onlyAdmin
    {
        ...
        emit WithdrawFund(_gameIndex, owner(), _amount);
    }

    function addReserveOperating(uint128 _amount)
        external
        override
        onlyRole(GAME_ROLE)
    {
    ...
    function subReserveOperating(uint128 _amount)
        external
        override
        onlyRole(GAME_ROLE)
    {
    ...
    function withdrawExcessTokens(address _token) external onlyAdmin {
        require(
            _token != tokenAddress,
            "Admin can't withdraw the game's token"
        );
    ...
    OwnableUpgradeable
{
    ...
    modifier onlyAdmin() {
        require(admin == msg.sender, "AC: Admin access denied");
        _;
    }
    ...
    function pause() external onlyRole(WORKER_ROLE) whenNotPaused {
        paused = true;
        emit Paused();
    }

    function unpause() external onlyRole(WORKER_ROLE) whenPaused {
        paused = false;
        emit Unpaused();
    }
}
