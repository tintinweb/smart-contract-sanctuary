...
contract BeliFarm is Ownable, ReentrancyGuard {
    ...
    mapping(address => bool) private poolLp;
    ...
    uint256 public constant BELIPerBlock = 214e16; // BELI tokens created per block, 2.14
    ...
    function add(
        ...
        if (_isLpReward == true) {
            require(_isFeeReceiverReward == false, "transfer fee reward and lp reward is exclusive to each other");
        }
        if (_withUpdate) {
            massUpdatePools();
        }
        require(poolLp[address(_want)] == false, "duplicate lp");
        poolLp[address(_want)] = true;
    ...
    function pendingFeeReceiverBeliReward(
        ...
        return _pendingFeeReceiverBeliReward(_pid, _user);
    }

    function _pendingFeeReceiverBeliReward(
        uint256 _pid,
        address _user
    ...
    function pendingFeeReceiverBeliLpReward(
        ...
        return _pendingFeeReceiverBeliLpReward(_pid, _user);
    }

    function _pendingFeeReceiverBeliLpReward(
        uint256 _pid,
        address _user
    ) private view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function harvestFeeUntil(uint256 _pid, uint256 _amount) external view returns (uint256) {
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function transferRewardFromFeeReceiver(
        ...
        if (pool.isFeeReceiverReward) {
            pendingReward = _pendingFeeReceiverBeliReward(_pid, _user);
            ...
            pendingReward = _pendingFeeReceiverBeliLpReward(_pid, _user);
