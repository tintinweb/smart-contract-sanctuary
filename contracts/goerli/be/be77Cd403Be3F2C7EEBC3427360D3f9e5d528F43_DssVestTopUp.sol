...
contract DssVestTopUp is Ownable {
    ...
    address public immutable TASK_TREASURY;
    address public immutable GELATO;

    constructor(address _pokeMe, address _newOwner) {
        POKE_ME = _pokeMe;
        TASK_TREASURY = IPokeMe(_pokeMe).taskTreasury();
    ...
    function topUp(
        ...
        IERC20(_paymentToken).approve(TASK_TREASURY, amt);

        TaskTreasury(TASK_TREASURY).depositFunds(owner(), _paymentToken, amt);
    }
    ...
    function checker(
        ...
        uint256 balance = TaskTreasury(TASK_TREASURY).userTokenBalance(
            owner(),
            _paymentToken
        );
