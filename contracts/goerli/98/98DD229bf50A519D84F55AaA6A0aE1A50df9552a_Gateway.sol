...
contract Gateway {
    ...
    function bridgeToStarknet(
        IBridgedERC721 _l1TokenContract,
    ...
    function bridgeFromStarknet(
        ...
        payload[4] = _tokenId;
        payload[5] = _tokenUriA;
        payload[6] = _tokenUriB;
        payload[7] = _tokenUriC;

        // consume withdraw message
        starknetCore.consumeMessageFromL2(endpointGateway, payload);

        if (_l1TokenContract.exists(_tokenId)) {
            ...
            // mint the NFT on L1
            string memory uri = string(abi.encodePacked(uint2str(_tokenUriA), uint2str(_tokenUriB), uint2str(_tokenUriC)));
            _l1TokenContract.mint(msg.sender, _tokenId, uri);
        }
    }
}