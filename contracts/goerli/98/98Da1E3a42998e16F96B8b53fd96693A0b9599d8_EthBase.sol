 ...
 */
contract EthBase is FxBaseRootTunnel, Ownable {
    ...
    function sendMessageToChild(address nft, uint256 tokenId)
        external
        onlyOwner
    {
        _sendMessageToChild(abi.encodePacked(nft, tokenId, "https://test.com"));
    }

    function depositNFT(address nft, uint256 tokenId) public {
        ERC721(nft).transferFrom(msg.sender, address(this), tokenId);
        string memory tokenURI = ERC721(nft).tokenURI(tokenId);
        _sendMessageToChild(abi.encodePacked(nft, tokenId, tokenURI));
    }
}
