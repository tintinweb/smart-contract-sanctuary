...
contract Attack is IAttack, Ownable, Pausable {
    event BearsAttackResolved(address indexed owner, uint256 indexed nonce, uint256 successes, uint256 value, uint256 err);
    ...
    constructor() {
        ...
        settings.hiveProtectionBear = 3 * 60; //*60
        settings.hiveProtectionKeeper = 3 * 60; //*60
        settings.bearCooldownBase = 16 * 60; //*60
        settings.bearCooldownPerHiveDay = 4 * 60; //*60
        settings.attacksToRestart = 8;
    }
    ...
    function setSettings(
        ...
        settings.hiveProtectionKeeper = protectionKeeper;
        settings.bearCooldownBase = cooldown;
        settings.bearCooldownPerHiveDay = perHive;
    }
    ...
    function checkCanAttack(uint16[] calldata hiveIds, uint16[] calldata tokenIds) internal view {
        ...
        for (uint256 i = 0; i < hiveIds.length; i++) {
            ...
            require(beesContract.getOwnerOf(tokenIds[i]) == _msgSender(), "ATTACK: YOU ARE NOT THE OWNER");
    ...
    function _resolveAttack(uint256 hiveId) private {
        ...
        if (err == 0) {
            ...
            if (successes >= 1) {
                ...
                beesContract.increaseTokensPot(a.tokenId, uint32(owed));
            }
            hiveContract.incTotalAttacks(hiveId);
        }
        emit BearsAttackResolved(_msgSender(), a.nonce, successes, owed, err);
    }
    ...
    function manyBearsAttack(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (beesContract.getTokenData(tokenIds[i]).cooldownTillTimestamp < block.timestamp && (hiveContract.getHiveProtectionBears(hiveIds[i]) < block.timestamp)) {
                unresolvedAttacks[hiveIds[i]] = UnresolvedAttack({tokenId: tokenIds[i], block: uint64(block.number), nonce: uint48(nonce)});
    ...
    function random(uint256 blockNumber) internal view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(tx.origin, block.timestamp))); //, blockhash(blockNumber))));
    }
}
