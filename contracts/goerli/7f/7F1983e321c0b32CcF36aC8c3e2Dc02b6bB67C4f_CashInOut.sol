...
import "./utils/IGameController.sol";
    ...
    IGame
{
    ...
    function resolveGame(
        ...
        // uint8 gameIndex = IGameController(address(vault)).getGameIndex(address(this));

        //pay out the game's winners.
        if (game.players.length > 0) {
            for (uint8 i = 0; i < game.players.length; i++) {
                address player = game.players[i];
                for (uint8 j = 0; j < _players.length; j++) {
                    address winner = _players[j];
                    if (player == winner) {
                        uint128 reward = _amounts[i];

                        // vault.depositFund(gameIndex,reward);
                        vault.addReserveOperating(reward);
                        vault.subGameBalance(_players[i], reward, _fee);
                    }
                }
                //delete player from the game.
                delete playerInGame[player][_gId][_rId];
            }
        }
        //for Dino cause players join through backend.
            ...
            for (uint128 i = 0; i < _players.length; i++) {
                ...
                //delete if users accidentally join Dino by contract.
                // delete playerInGame[player][_gId][_rId];
                //Dino's gameCount increases here;
                info.gameCount++;
            }
        }
...
interface IGameController {
    ...
    function getGameIndex(address _gameAddress) external view returns (uint8) ;
