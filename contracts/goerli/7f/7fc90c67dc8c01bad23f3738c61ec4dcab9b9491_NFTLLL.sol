...
contract NFTLLL is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(_mintAmount <= maxMintAmount, "mint amount per session exceeded");
    require(supply + _mintAmount <= maxSupply, "NFT limit exceeded");

    if (msg.sender != owner()) {
        if(onlyWhitelisted == true) {
            require(isWhitelisted(msg.sender), "user is not whitelisted");
            uint256 ownerMintedCount = mintedBalance[msg.sender];
            require(ownerMintedCount + _mintAmount <= whitelistLimit, "already minted max");
        }
