    ...
    * @dev The node must also verify it doesn't use banned opcodes, and that it doesn't reference storage outside the wallet's data.
    ...
    unchecked {
        ...
        if (paymentMode != PaymentMode.paymasterStake) {
            ...
            if (context.length > 0) {
                uint postOpGas = actualGas - op.callGas - op.verificationGas;
                if (mode != IPaymaster.PostOpMode.postOpReverted) {
                    IPaymaster(op.paymaster).postOp{gas: postOpGas}(mode, context, actualGasCost);
                } else {
                    try IPaymaster(op.paymaster).postOp{gas: postOpGas}(mode, context, actualGasCost) {}
