...
contract ANON_INU_COIN is Context, IERC20, Ownable {
    ...
    uint256 public minBnbForBuyBack = 0.001 ether;
    ...
    constructor () {
        ...
        nextBurnDate = block.timestamp + (2 minutes);
...
//---------------------------------//
    ...
    function checkForFirstSevenBurns() internal {
        if ((totalBurnCount < 7) && (block.timestamp > nextBurnDate)) {
            uint256 daysPassed = ((block.timestamp).sub(nextBurnDate)).div(2 minutes);
            ...
            if (totalBurnCount < 7) {
                nextBurnDate = nextBurnDate + ((2 minutes)*daysPassed);
            } else {
                nextBurnDate = 0;
            }
        }
