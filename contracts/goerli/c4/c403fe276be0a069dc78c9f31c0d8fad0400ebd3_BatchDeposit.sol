...
contract BatchDeposit is Pausable, Ownable {
    ...
    modifier feeTypeGwei(uint256 _newFee) {
        require(_newFee % 1 gwei == 0, "Fee must be a multiple of GWEI");
        _;
    }

    constructor(address depositContractAddr, uint256 initialFee) feeTypeGwei(initialFee) public {
    ...
    function batchDeposit(
        ...
        whenNotPaused feeTypeGwei(msg.value)
        external payable
    {
        // sanity checks
     ...
     * Can only be called by the current owner.
     */
    function withdraw(address payable receiver) onlyOwner public {
        require(receiver != address(0), "You can't burn these eth directly");

        uint256 amount = address(this).balance;
        require(amount > 0, "This contract balance is zero");
     ...
     * Can only be called by the current owner.
     */
    function changeFee(uint256 newFee) onlyOwner feeTypeGwei(newFee) public {
