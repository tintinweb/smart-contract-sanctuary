...
contract RabbitZillaDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("RabbitZilla_Dividend_Tracker", "RabbitZilla_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "RabbitZilla_Dividend_Tracker: No transfers allowed");
    }
    ...
    function withdrawDividend() public pure override {
        require(false, "RabbitZilla_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RabbitZilla contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "RabbitZilla_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "RabbitZilla_Dividend_Tracker: Cannot update claimWait to same value");
...
contract RabbitZilla is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    RabbitZillaDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 120000000000000 * (10**18);
    ...
    constructor() ERC20("Rabbit Zilla", "RabbitZilla") {
        BNBRewardsFee = 2;
        liquidityFee = 2;
        extraFeeOnSell = 20; // extra fee on sell
        DevelopmentFee = 3;
        DevelopmentWallet = payable(0xb376F66E136193A29D0706424c655D0Ba3c58a69);

        totalFees = BNBRewardsFee.add(liquidityFee).add(DevelopmentFee); // total fee transfer and buy

    	dividendTracker = new RabbitZillaDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "RabbitZilla: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "RabbitZilla: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "RabbitZilla: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "RabbitZilla: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "RabbitZilla: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "RabbitZilla: Cannot update gasForProcessing to same value");
