...
contract BidNFT is IBidNFT, ERC721Holder, Ownable, Pausable {
    ...
    event AddOfferedCurrency (address indexed owner, address currency);
    ...
    function buyTokenTo(uint256 _tokenId, address _to) public override whenNotPaused {
        ...
        address paidCurrency = _userSellingCurrency[_tokenId];
        IERC20 paidToken = IERC20(paidCurrency);
        if (feeAmount != 0) {
            paidToken.safeTransferFrom(_msgSender(), feeAddr, feeAmount);
        }
        paidToken.safeTransferFrom(_msgSender(), _tokenSellers[_tokenId], price.sub(feeAmount));
        ...
        delete _userSellingCurrency[_tokenId];
    }
    ...
    function readyToSellToken(uint256 _tokenId, uint256 _price, address _currency) public override whenNotPaused {
        readyToSellTokenTo(_tokenId, _price, address(_msgSender()), _currency);
    }

    function setOfferedCurrency(address _currency) public onlyOwner whenNotPaused {
        require(_currency != address(0) && !_offeredCurrecy.contains(_currency), "Currency is already add to the list or not valid!");
        _offeredCurrecy.add(_currency);
        emit AddOfferedCurrency(_msgSender(), _currency);
    }

    function _verifyOfferedCurrency (address _currency) private view {
        require(_currency != address(0) && _offeredCurrecy.contains(_currency), "Ready to sell currecy must be in offered currency list");
    }
    ...
    function readyToSellTokenTo(
        ...
        require(_price != 0, 'Price must be granter than zero');
        _verifyOfferedCurrency(_currency);
        ...
        _userSellingCurrency[_tokenId] = _currency;
...
interface IBidNFT {
    ...
    function readyToSellToken(uint256 _tokenId, uint256 _price, address _currency) external;
