...
contract MaltLaunch is Ownable, Initializable {
  ...
  IERC20 public pair;
  address account;
  ILiquidityMine public lpmine;

  function initialize(
    address _communityOffering,
    address _router,
    address _dao,
    address _malt,
    address _busd,
    ...
    communityOffering = IInitialCommunityOffering(_communityOffering);
    ...
    account = _account;
    pair = IERC20(_pair);
    lpmine = ILiquidityMine(_lpmine);
  }

  function launch(uint256 _amount) public onlyOwner {
    // add initial liquidity
    malt.safeTransferFrom(msg.sender, address(this), _amount);
    BUSD.safeTransferFrom(msg.sender, address(this), _amount);

    malt.approve(address(router), _amount);
    ...
    (,, uint256 liquidity) = router.addLiquidity(
      ...
      _amount,
      _amount,
      _amount.mul(95).div(100), // 5% slippage
      _amount.mul(95).div(100), // 5% slippage
      address(this),
      now
    );

    pair.approve(address(lpmine), liquidity);

    lpmine.bondToAccount(account, liquidity);

    dao.advance();
    communityOffering.provideLiquidity();
  }
}
...
interface IDAO {
  ...
  function advance() external view;
...
interface IInitialCommunityOffering {
