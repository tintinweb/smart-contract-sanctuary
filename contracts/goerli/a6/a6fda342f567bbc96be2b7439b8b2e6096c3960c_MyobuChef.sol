...
contract MyobuChef is Ownable, ReentrancyGuard, ERC721Holder {
    ...
    // Info of each pool.
    struct PoolInfo {
        IERC20 lpToken;             // Address of LP token contract. @dev Is that not super expensive? 
...
//--------------------------------------------------------VIEW FUNCTIONS --------------------------------------------------------
    ...
    function pendingRewards(uint256 _pid, address _user) external view returns (uint112, uint112) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint112 userWeightedAmount = user.weightedBalance;
        uint112 accRwtPerShare = pool.accRwtPerShare;
        uint112 accETHPerShare = pool.accETHPerShare;
        uint112 weightedBalance = pool.weightedBalance;
        if (block.number > pool.lastRewardBlock && weightedBalance != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint112 rwtReward = uint112(multiplier * rwtPerBlock * pool.allocPoint / totalAllocPoint);
            accRwtPerShare = (accRwtPerShare + rwtReward) * 1e12 / weightedBalance;
            uint112 ETHReward = uint112(multiplier * ETHPerBlock * pool.allocPoint / totalAllocPoint);
            accETHPerShare = (accETHPerShare + ETHReward) * 1e12 / weightedBalance;
        }
		
	uint112 PendingRWT = ((userWeightedAmount * accRwtPerShare) / 1e12) - user.rewardDebt;
    uint112 PendingETH = ((userWeightedAmount * accETHPerShare) / 1e12) - user.ETHrewardDebt;
...
//--------------------------------------------------------PUBLIC FUNCTIONS --------------------------------------------------------
	...
	//Receive ETH from the tax splitter contract
    fallback() external payable {
        //assert(msg.sender == ETHBank); //@dev actually probably keep that 
        ETHLeftUnshared = ETHLeftUnshared + msg.value;
        updateETHRewards();
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if(tokenReleased) {
            ...
            rwt.mint(address(this), rwtReward);
    ...
    function deposit(uint256 _pid, uint112 _amount, uint256 lockTime) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function withdraw(uint32 _pid, uint112 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.lockedUntil >= block.timestamp, "withdraw: Tokens locked"); // This makes me feel a little bad
    ...
    function addLiquidityNoFeeAndStake(uint256 amountTokensIn) public payable nonReentrant {
        ...
        (,,uint256 numLiquidityAdded) = Myobu.noFeeAddLiquidityETH{value: msg.value}(params);
        if (address(this).balance > balanceBefore){
            payable(msg.sender).transfer(address(this).balance - balanceBefore);
        }
        //@dev The Myobu/ETH pool MUST be pool 0
        userInfo[0][msg.sender].amount += uint112(numLiquidityAdded);
	...
	// @dev TODO router and WETH are inputs for testing purposes only. THIS IS UNSAFE DDONT LEAVE IT LIKE THAT 
    function reinvestETHRewards(address router, address WETH) public nonReentrant {
            ...
            if (ETHPending > 0) { 
                ...
                userInfo[1][msg.sender].amount += uint112(amountSwapped);
...
//--------------------------------------------------------RESTRICTED FUNCTIONS --------------------------------------------------------	
    ...
    function setRewardToken(address _RWT, uint _rwtPerBlock) public onlyOwner {
        ...
        // Mints 0.000000000000000001 token to the owner address to check that the token behaves as intended. 
        // @dev RWT's mint function MUST return a bool, not sure how standard that is for ERC20. 
        require(rwt.mint(msg.sender, 1), "MyobuStaking : Something's wrong with the token. Is masterchef the owner?");
        rwtPerBlock = _rwtPerBlock;
		tokenReleased = true;
...
//--------------------------------------------------------INTERNAL FUNCTIONS --------------------------------------------------------
    //Lock tokens up to 52 weeks for rewards boost, ( max rewards = x3, rewards increase lineraly with lock time)
	...
	function updateUserWeightedBalance(uint256 _pid, address user) public {
		...
		userInfo[_pid][user].weightedBalance = uint112(amount * (1000 + userInfo[_pid][user].lockTimeBoost) * (1000 + userInfo[_pid][user].boostPointsBP)) / 1000000;
		poolInfo[_pid].weightedBalance = uint112(poolBalance + userInfo[_pid][user].weightedBalance);
    ...
    // Safe transfer function, just in case if rounding error causes pool to not have enough EGGs.
