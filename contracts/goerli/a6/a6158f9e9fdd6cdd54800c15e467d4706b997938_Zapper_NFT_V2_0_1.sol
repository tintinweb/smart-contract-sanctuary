...
contract Zapper_NFT_V2_0_1 is ERC1155, Ownable {
     ...
     * @param tokenids An array of the cost of each ID. 0 if it cannot
     * be crafted
     * @param supplymax An array of higher rarity IDs which can be
     * crafted from the ID. 0 if max rarity.
     **/
    function add(uint256[] calldata tokenids, uint256[] calldata supplymax) external onlyOwner {
        require(tokenids.length == supplymax.length, "Mismatched array lengths");

        for (uint256 i = 0; i < tokenids.length; i++) {
            uint256 newId = tokenids[i];
            uint256 newmax = supplymax[i];
            //require(!exists[newId], "ID already exists");
            require(newId != 0, "Invalid ID");

            exists[newId] = true;
            tokenSupplyMax[newId] = supplymax[i];
            emit Add(newId,newmax);
        }
    }
    ...
    function mintBatch(
        ...
        require(tokenids.length == amount.length, "Mismatched array lengths");
        if (!accessoryContract[msg.sender]) {
            for (uint256 i = 0; i < tokenids.length; i++) {
                uint256 newamount = amount[i];
                // if(tokenSupply[tokenids[i]] > (tokenSupplyMax[tokenids[i]] + newamount) ){
                //     require(false, "Maximum supply exceeded");
                // }
                require(exists[tokenids[i]], "Invalid ID");
            }
        } else {
            require(false, "Invalid onlyOwner");
        }
        _mintBatch(msg.sender, tokenids, amount, data);
