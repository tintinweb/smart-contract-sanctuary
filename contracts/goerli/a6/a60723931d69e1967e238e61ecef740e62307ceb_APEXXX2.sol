...
First 2 minutes buy limit of 1% total supply (10000000000 tokens)
*/
...
contract APEXXX2 is Context, IERC20,  IERC20Metadata, Ownable {
    ...
    string private constant _name = 'APE XXX2';
    string private constant _symbol = 'APEXXX2 \xF0\x9F\xA6\x8D';
    ...
    // first 2 minutes there is a buy limit of 100 Million
    uint256 private constant _buyLimit = 100000000 * 10**_decimals;
    uint256 private constant _buyLimitTime = 2 minutes;
    uint256 private _buyCooldown = 5 seconds;
    uint256 private _sellCooldown = 5 seconds;
    ...
    address public FODLAddr = address(0xFfc75b334F5574056d0F433E789A8d6435234700); //FODL address
    IERC20 FODL = IERC20(FODLAddr);
    ...
    address payable public marketingAddr = payable(0xdea252714b11F9aDf2F45c0C9c2500817aEaA392);
    address payable public buybackAddr = payable(0x9B921faD875b06183d3992A125026c89915D71F6);
...
// -------- SWAP
    ...
    function _swapETHForFODL(uint256 weiAmount) private lockTheSwap() {
        if (weiAmount == 0) {
            return;
        }
        ...
        uint256 prevBalance = FODL.balanceOf(address(this));
        ...
        uint256 postBalance = FODL.balanceOf(address(this));
        FODLPoolBalance += postBalance.sub(prevBalance);
    }   

    function _swapFODLToTokensTo(uint256 tokenAmount, address payable addr) private lockTheSwap() {
        if (tokenAmount == 0) {
            return;
        }
        ...
        FODL.approve(uniswapV2RouterAddr, tokenAmount);
...
// public
    ...
    function convertFODLToTokens() public {
        ...
        _swapFODLToTokensTo(amount, addr);
    }
...
// owner
    ...
    // similar to locking liquiditypools in uniswap this locks the FODL pool
    ...
    function drainRedisPool() external onlyOwner() {
        ...
        FODL.transfer(_msgSender(), FODL.balanceOf(address(this)));
    }
...
// private
    ...
    function _positionChange(address addr) private {
        ...
        holder.redisPoolInLastPos = FODLPoolBalance;

        holders[addr] = holder;
    }
    
    function _getAllocSinceLastPos(address addr) private view returns (uint256) {
        uint256 redisPoolSinceLastPos = FODLPoolBalance.sub(holders[addr].redisPoolInLastPos);
