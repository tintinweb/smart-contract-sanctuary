    ...
    ERC721EnumerableUpgradeable
{
        ...
        nonReentrant
    {
        ...
        _fundsCheck(msg.value, _sentToTreasury);
        payable(owner()).transfer(_sentToTreasury);
        if (msg.value > _sentToTreasury) {
            payable(msg.sender).transfer(msg.value - _sentToTreasury);
        }
    ...
    function isMinted(uint256 _tokenId) external view override returns (bool) {
        if (_tokenId >= 14000 && _tokenId < 15000) {
            return (_tokenId < 14000 + _reservedCount);
        } else if (_tokenId >= 9000) {
            return (_tokenId < 9000 + _freeCount);
        } else if (_tokenId >= 0) {
    ...
    function _acquireUnchecked(
        ...
        if (_idToKeep == _targetId) {
            ...
            /**
             * If the caller is being burned, transfer over the caller's RP.
             * Burns an amount of the caller's RP before transferring, based
             * on `mergerBurnPercentage`. Rounded up to prevent burning zero.
             */
            uint256 _rpToBurn =
                ((_enterprises[_callerId].rp * _mergerBurnPercentage) /
                    PERCENT_DENOMINATOR) + 1;
            if (_rpToBurn > _enterprises[_callerId].rp) {
                _enterprises[_callerId].rp = 0;
            } else {
                _enterprises[_targetId].rp +=
                    _enterprises[_callerId].rp -
                    _rpToBurn;
                _enterprises[_callerId].rp = 0;
            }
        }
