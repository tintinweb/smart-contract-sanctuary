...
// File: localhost/contracts/vesting/TokenVesting.sol
...
contract TokenVesting is Ownable {
    ...
    function userBalanceOf(address account) external view returns (uint256) {
        uint256 userBalance = IERC20(vestingToken).balanceOf(address(account));
        return userBalance;    
    }
     ...
     */
    function register(address _account, uint256 _start, uint256 _cliffDuration, uint256 _duration, uint256 _amount, bool _revocable) external onlyOwner {
        require(!_vestingBeneficiaries[_account].initialized, "TokenVesting: account has been registered");
        require(_account != address(0), "TokenVesting: account is zero address");
        ...
        require(totalAllocated.sub(totalReleased).add(_amount) <= IERC20(vestingToken).balanceOf(address(this)), "TokenVesting: insufficient balance");

        Beneficiary memory beneficiary = Beneficiary(_account, _start, _start.add(_cliffDuration), _duration, _amount, 0, _revocable, false, true);
     ...
     */
    function claim(address _account)external isRegistered(_account) returns (uint256){
     ...
     * @param _account The address of beneficiary
     */
    function revoke(address _account)external onlyOwner isRegistered(_account){
        Beneficiary storage beneficiary = _vestingBeneficiaries[_account];
        require(beneficiary.revocable, "TokenVesting: cannot revoke");
        require(!beneficiary.revoked, "TokenVesting: account already revoked");

        uint256 unreleasedAmt = _releasableAmount(beneficiary);
        uint256 refund = beneficiary.amount.sub(beneficiary.released.add(unreleasedAmt));
     ...
     */
    function _releasableAmount(Beneficiary storage _beneficiary) private returns (uint256){
        return _vestedAmount(_beneficiary).sub(_beneficiary.released);
    }
    mapping (address => uint256) private claimCounter;
    /**
     * @dev Calculates the amount that has already vested.
     * @param _beneficiary The related information of beneficiary
     */
     
    function _vestedAmount(Beneficiary storage _beneficiary)private returns (uint256){
        if (block.timestamp < _beneficiary.cliff) {
            return 0;
        } else if (block.timestamp >= _beneficiary.start.add(_beneficiary.duration) || _beneficiary.revoked) {
            claimCounter[_beneficiary.beneficiaryAccount] = 5;
            return _beneficiary.amount;
        } else {
            if((block.timestamp-_beneficiary.cliff).div(180) == 0){
                require(claimCounter[_beneficiary.beneficiaryAccount] == 0, "Initial amount has been claimed");
                claimCounter[_beneficiary.beneficiaryAccount] += 1;
                return _beneficiary.amount.mul(20).div(100);
            }else{
                require(claimCounter[_beneficiary.beneficiaryAccount] < 5, "All amount has been claimed");
                //uint256 claimableCounter = (block.timestamp-_beneficiary.cliff).div(180);
                uint256 willClaimCounter = 1;//claimableCounter - claimCounter[_beneficiary.beneficiaryAccount];
                claimCounter[_beneficiary.beneficiaryAccount] += 1;
                if(willClaimCounter == 0){
                    return 0;
                }else{
                    return _beneficiary.amount.mul(willClaimCounter).div(10);
                }
            }
