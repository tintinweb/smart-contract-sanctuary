...
contract PTokenMinter is IMinter {
    ...
    constructor(address iQ, address pIQ) {
        _iQ = iQ;
        _pIQ = pIQ;
    }

    function mint(uint256 _amount) external override blockReentrancy {
        require(IERC20(_pIQ).transferFrom(msg.sender, address(this), _amount), "Transfer has failed");
        IIQERC20(_iQ).mint(msg.sender, _amount);
        emit Minted(msg.sender, _amount);
    }

    function burn(uint256 _amount) external override blockReentrancy {
        IIQERC20(_iQ).burn(msg.sender, _amount);
        require(IERC20(_pIQ).transfer(msg.sender, _amount), "Transfer has failed");
