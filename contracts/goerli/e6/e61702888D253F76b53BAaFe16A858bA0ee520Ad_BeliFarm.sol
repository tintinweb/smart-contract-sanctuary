...
contract BeliFarm is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        // We do some fancy math here. Basically, any point in time, the amount of BELI
    ...
    /**
     * @dev something to do with stack too deep
     */
    struct FeeRewardHelper {
    ...
    function setLpReceiver(address _newReceiver) external onlyOwner {
        feeLpReceiver = _newReceiver;
    }
    ...
    function transferRewardFromFee(uint _pid, address _user) private {
        ...
        if (pool.isTransferFeeReward) {
            if (pendingReward > 0 && feeReceiver != address(0)) {
                accWithdrawnBeliFromFees = accWithdrawnBeliFromFees.add(pendingReward);
                balance = IERC20(BELI).balanceOf(feeReceiver);
                if (balance < pendingReward) {
                    pendingReward = balance;
                }
                if (pendingReward > 0) {
                    IERC20(BELI).safeTransferFrom(feeReceiver, _user, pendingReward);
                }
            }
        }
        if (pool.isLpReward) {
            if (pendingReward > 0 && feeLpReceiver != address(0)) {
                accWithdrawnBeliLPRewardFromFees = accWithdrawnBeliLPRewardFromFees.add(pendingReward);
                balance = IERC20(BELI).balanceOf(feeLpReceiver);
                if (balance < pendingReward) {
                    pendingReward = balance;
                }
                if (pendingReward > 0) {
                    IERC20(BELI).safeTransferFrom(feeLpReceiver, _user, pendingReward);
                }
            }
        }
    }
    ...
    function pendingTransferFeeBeliReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (pool.isTransferFeeReward) {
            ...
            if (block.number > lastUpdateBeliFeesBlock) {
                uint256 beliReceived = IERC20(BELI).balanceOf(feeReceiver).add(accWithdrawnBeliFromFees);
    ...
    function pendingTransferFeeBeliLpReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (pool.isLpReward) {
            ...
            if (block.number > lastUpdateBeliFeesLPBlock) {
                uint256 beliReceived = IERC20(BELI).balanceOf(feeLpReceiver).add(accWithdrawnBeliLPRewardFromFees);
                if (beliReceived.sub(_accBeliFromFees) > 0) {
                    _accBeliFromFees = beliReceived;
                }
            }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (pool.isTransferFeeReward) {
            ...
            if (pool.lastAccBELIFromFees < accBeliFromFees && IStrategy(pool.strat).sharesTotal() != 0) {
                ...
                pool.accBELIPerShareFromFees = pool.accBELIPerShareFromFees.add(
                    beliFeeReward.mul(1e12).div(IStrategy(pool.strat).sharesTotal())
                );
        ...
        if (pool.isLpReward) {
            ...
            if (pool.lastAccBELIFromFees < accBeliLPFromFees && IStrategy(pool.strat).sharesTotal() != 0) {
                uint256 beliFeeReward =
    ...
    function deposit(uint256 _pid, uint256 _wantAmt, address _referrer) public nonReentrant {
        ...
        if (_wantAmt > 0) {
            ...
            // deposit using strat, deposit fee etc is already handled by strat
            uint256 sharesAdded =
    ...
    function setLocker(ILocker _locker) public onlyOwner {
        emit LockerUpdated(locker, _locker);
        locker = _locker;
    }

    function setLockerRate(uint16 _newRate) public onlyOwner {
        require(_newRate <= MAX_RATE, "locker rate exceed allowed value");
        emit LockerRateUpdated(lockerRate, _newRate);
        lockerRate = _newRate;
    }

    function setReferral(IReferral _newReferral) public onlyOwner {
        emit ReferralUpdated(referral, _newReferral);
    ...
    function mintShareToReceipt(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        IStrategy(pool.strat).mintShareToReceipt(msg.sender, _amount);
    ...
    function burnReceiptForShare(uint256 _pid, uint256 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        uint256 userBalance = IERC20(pool.strat).balanceOf(msg.sender);
        require(userBalance >= _amount, "insufficient balance");
        UserInfo storage user = userInfo[_pid][msg.sender];
        IStrategy(pool.strat).burnReceiptForShare(msg.sender, _amount);
...
interface IStrategy {
    ...
    function setProfitToBeliRate(uint256 _newRate) external;
