...
/// @title MetalApeRegistry
/// @author Gary Thung
/// @notice MetalApeRegistry is a registry defining permissions for the MetalApe
contract MetalApeRegistry is Ownable {
    ...
    function removeItemType(string memory _itemType) external onlyOwner {
        ...
        for (uint256 i = 0; i < itemTypesArray.length; i++) {
            if (
                keccak256(bytes(itemTypesArray[i])) ==
                keccak256(bytes(_itemType))
            ) {
                index = i;
                break;
            }
    ...
    function add721Contract(address _contract) external onlyOwner {
        require(
            _contract.supportsInterface(ChubbyApeEquipmentsId.INTERFACE_ID),
            "Must implement ChubbyApeEquipments interface"
        );
