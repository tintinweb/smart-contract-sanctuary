 ...
 */
abstract contract NFT721Market is IGetRoyalties, IGetFees, FoundationTreasuryNode, NFT721Creator {
  using AddressUpgradeable for address;
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
    if (interfaceId == type(IGetRoyalties).interfaceId || interfaceId == type(IGetFees).interfaceId) {
      return true;
    }
    return super.supportsInterface(interfaceId);
  }
