...
contract Master is Context, IERC20, Ownable {
  ...
  //mapping (address => bool) private _unlimitedAllowance;
  ...
  mapping (address => mapping (uint256 => uint256)) private _proVotes;
  mapping (address => mapping (uint256 => uint256)) private _conVotes;
  ...
  function makeProposal(ProposalType _type, uint256 value, uint256 votes) public {
    ...
    _proVotes[_msgSender()][proposals.length - 1] = votes;
  ...
  function voteProposal(uint256 index, uint256 amount, bool pro) public {
    ...
    if (pro) {
      ...
      _proVotes[_msgSender()][index] += amount;
    } else {
      proposals[index].conVotes += amount;
      _conVotes[_msgSender()][index] += amount;
    }
  }

  function getProposalStatus(uint256 index) public view returns (bool) {
    require(proposals.length > index, "getProposalStatus: proposal doesn't exist");
    return proposals[index].timestamp + proposalVotingDuration >= block.timestamp;
  }

  function getProposalProposerVotes(uint256 index) public view returns (uint256) {
    require(proposals.length > index, "getProposalProposerVotes: proposal doesn't exist");
    return _proVotes[proposals[index].proposer][index];
  }

  function getProposalSelfVotes(uint256 index) public view returns (uint256, uint256) {
    require(proposals.length > index, "getProposalSelfVotes: proposal doesn't exist");
    return (_proVotes[_msgSender()][index], _conVotes[_msgSender()][index]);
  }
  ...
  /*function unlimitedAllowance() public view returns (bool) {
    return _unlimitedAllowance[_msgSender()];
  }

  function enableUnlimitedAllowance() public {
    require(!_unlimitedAllowance[_msgSender()], "enableUnlimitedAllowance: already enabled");
    _unlimitedAllowance[_msgSender()] = true;
  }

  function disableUnlimitedAllowance() public {
    require(_unlimitedAllowance[_msgSender()], "enableUnlimitedAllowance: already disabled");
    _unlimitedAllowance[_msgSender()] = false;
  }*/
