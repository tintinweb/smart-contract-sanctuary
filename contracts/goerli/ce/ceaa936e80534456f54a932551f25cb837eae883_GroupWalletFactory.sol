...
contract GroupWalletFactory {
    ...
    function inviteInstallToken_q31n(bytes32[] calldata _mem) external payable {      
      ...
      uint256 amount = uint256(msg.value / uint256((l/5) + 1));                 // ether transferred to each member
      uint256 c      = commitments[_dHash];
...

      {
        ...
        do {
          ...
          proxyCommand  = abi.encodePacked(proxyCommand ,_mem[i+3]);            // 7, 12, 17 + domainName
          theAbiCommand = abi.encodePacked(theAbiCommand,_mem[i+4]);            // 8, 12, 16 + 1 abi extra word

          require(address(uint160(o)).send(amount),"Sending ether failed.");
          emit Deposit(address(this), amount);
          
          base.ens().setSubnodeRecord(_dHash, _mem[i+1], address(this), address(resolverContract), time); // peter.silvias-bakery.eth
      ...
      
      {
        ...
        proxyCommand = abi.encodePacked(bytes4(0x5de5cbe4),k_offset20,bytes32(uint256((l/5)+1)),proxyCommand,_mem[3],k_padding28);  // call(proxy)                   
