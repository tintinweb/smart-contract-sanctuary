...
contract TesterWester is IERC20, Ownable {
    ...
    function swapAndLiquify() private swapLock {
        ...
        if (liquidity_fee != 0 && over_liquify_threshold) {
            uint256 half = liquidity_tokens.div(2);
            uint256 other_half = liquidity_tokens.sub(half);
    ...
    function transferStageStandard(address from, address to, uint256 t_initial) private {
        ...
        if (team_fee != 0) {
            uint256 tteam_fee;
            (tteam_fee, rteam_fee) = calcRTValue(current_rate, t_initial, team_fee);
        ...
        if (liquidity_fee != 0) {
            uint256 tliq_fee;
            (tliq_fee, rliq_fee) = calcRTValue(current_rate, t_initial, liquidity_fee);
        ...
        if (burn_fee != 0) {
            uint256 tburn_fee;
            (tburn_fee, rburn_fee) = calcRTValue(current_rate, t_initial, burn_fee);
            burn_tokens += tburn_fee;
        }
        uint256 rrefl_fee = 0;
        if (reflection_fee != 0) {
            rrefl_fee = (t_initial.mul(reflection_fee).div(100)).mul(current_rate);
            reserve_total = reserve_total.sub(rrefl_fee);
        }
    ...
    function calcRTValue(uint256 current_rate, uint256 t_initial, uint256 fee) private returns (uint256, uint256) {
        uint256 tfee = t_initial.mul(fee).div(100);
        uint256 rfee = tfee.mul(current_rate);
