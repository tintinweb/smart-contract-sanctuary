...
contract MaltDAO is Initializable, Permissions {
  ...
  event TransferToken(address recipient, address token, uint256 amount);
  ...
  function initialize(
    ...
    // 10000 Malt for initial liquidity
    malt.mint(msg.sender, 10000e18);
    // 1000 Malt to 0xScotch
    malt.mint(0xa5DCdFe31e1955acFc20416513ae10c101C68445, 1000e18);
  }
  ...
  function sendTreasuryETH(uint256 amount)
    public
    onlyRole(TIMELOCK_ROLE, "Must have timelock role")
  {
    require(amount <= address(this).balance, "Insufficient funds to transfer");
    treasuryMultisig.transfer(amount);
    emit TransferETH(treasuryMultisig, amount);
  }

  function sendTreasuryToken(address _token, uint256 amount)
    public
    onlyRole(TIMELOCK_ROLE, "Must have timelock role")
  {
    ERC20 token = ERC20(_token);
    require(amount <= token.balanceOf(address(this)), "Insufficient funds to transfer");

    token.transfer(treasuryMultisig, amount);
    emit TransferToken(treasuryMultisig, _token, amount);
  }
