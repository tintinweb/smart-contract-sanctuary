...
contract MoraStaking{
    ...
    uint private termofacontract = (1200); // 5 Months //= (2592000 * 5); // 5 Months // 20 sec.
  ...
  function Stake(uint256 _amo) external returns(bool success) {
    ...
    require(block.timestamp < (deployDate + termofacontract) - 120,"out-of-date"); // make non-stakable 6 hours before from end time. //21600
  ...
  function UnstakeAndClaim(uint _stakeID) external returns (bool result) {
    ...
    require(_stakePeriodInHour >= 1,"cant-unstake-before-24-hours"); //Unstake not permitted in first 24 hours ****
    ...
    require(block.timestamp > (deployDate + termofacontract) + 180,"out-of-date"); // Burnable after 24 hours from end time. //86400 ****
