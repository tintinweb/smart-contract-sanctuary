...
interface JobLike {
  ...
  function work(address keeper) external;
}

contract TestAutolineJob {
  using EnumerableSet for EnumerableSet.AddressSet;

  EnumerableSet.AddressSet private _jobs;
  ...
  function getNextJob(bytes32 network)
    external
    returns (
      bool,
      address,
    ...
    )
  {
    ...
    uint256 _length = _jobs.length();
    for (uint256 i = 0; i < _length; i++) {
      address _job = _jobs.at(i);

      if (JobLike(_job).workable()) {
        return (true, address(_job), abi.encodeWithSelector(JobLike.work.selector));
      }
    }
