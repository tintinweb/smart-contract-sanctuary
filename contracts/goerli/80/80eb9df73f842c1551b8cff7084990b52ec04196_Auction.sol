...
contract Auction is Context, Ownable, Epoch {
    ...
    uint256 totalCycleStaked;
    uint256 totalFlipStaked;
    ...
    ) public Epoch(600, auctionStartTime, 0) {
        // 30 mins period for test
        ...
        if (lastEpoch < getCurrentEpoch()) {
            if (dailyTotalBNB[lastEpoch] > 0) {
                ...
                for (uint256 i = 0; i < auctionLobbyParticipaters.length; i++) {
                    ...
                    uint256 newReward =
                        DAILY_MINT_CAP.mul(ap.BNBParticipated[lastEpoch]).div(
                            dailyTotalBNB[lastEpoch]
                        );

                    ap.cycleEarned[lastEpoch] = newReward;
        ...
        updateEpoch
    {
        ...
        stakes.push(CycleStake(getCurrentEpoch(), amount, 0));
    ...
    function unstake(uint256 index) external distributeRewards updateEpoch {
        ...
        // Delete this stake
        while (index < cycleStakes[_msgSender()].length - 1) {
            cycleStakes[_msgSender()][index].epoch = cycleStakes[_msgSender()][
                index + 1
            ]
                .epoch;
            cycleStakes[_msgSender()][index].cycleStaked = cycleStakes[
                _msgSender()
            ][index + 1]
                .cycleStaked;
            cycleStakes[_msgSender()][index].BNBEarned = cycleStakes[
                _msgSender()
            ][index + 1]
                .BNBEarned;
            index++;
        }
        cycleStakes[_msgSender()].pop();

        if (cycleStakes[_msgSender()].length == 0) {
        ...
        checkStartTime
    {
        ...
        emit StakeLP(amount, getCurrentEpoch(), _msgSender());
    }

    function takeLPReward(address user) public {
