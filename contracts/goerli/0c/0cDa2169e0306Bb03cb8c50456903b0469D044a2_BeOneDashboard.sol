...
contract BeOneDashboard is Exponential{
    ...
    returns (Dashboard memory)
  {
    ...
    for (uint256 i = 0; i < beOneRegistry.getCTokensLength(); i++) {
      ...
      totalSupplyInUsd += cToken.totalSupply().mul(cToken.exchangeRateStored()).div(1e18).mul(tokenPriceInUsd).div(uint(10) ** uint(cToken.decimals()));
      totalBorrowedInUsd += cToken.totalBorrows().mul(cToken.exchangeRateStored()).div(1e18).mul(tokenPriceInUsd).div(uint(10) ** uint(cToken.decimals()));
      ...
      CTokensArray[i] = CToken({
        ...
        // TODO / See https://github.com/amplify-labs/contracts/blob/2f2a58e7d3e74202fbcc76c28157405c1ae88ab6/protocol/contracts/Controller/Rewards.sol
