...
// flattened :  Friday, 02-Jul-21 20:49:27 UTC
...
abstract contract AStorage is IStorage {
    ...
    bytes32 constant BRIDGE_ADDRESSES = keccak256(abi.encodePacked("BRIDGE_ADDRESSES"));
...
abstract contract ABase is IBase, AStorage {
    ...
    modifier onlyOwner(
        ...
        require(containsUserAddress(addresses) && containsVAddress(addresses), "VBase::onlyOwner - must include owner and Vesto signatures");
        _;
    }
    ...
    modifier onlyRecovery(
        ...
        address[] memory addresses = validateSignatures(signatures);
        require(containsRecoveryAddress(addresses) && containsUserAddress(addresses), "VBase::onlyRecovery - must include recovery and Vesto signatures");
        _;
    }
...
abstract contract AErc20 is IErc20, ABase {
    ...
    function _transfer(
        ...
        require(from != to, "AErc20::_transfer - from and to address cannot be the same");
        require(from != address(0), "AErc20::_transfer - from address cannot be zero");
        require(to != address(0), "AErc20::_transfer - to address cannot be zero");
        require(fromBalance >= tokens, "AErc20::_transfer - insufficient tokens");
...
contract VErc20 is IVErc20, AErc20 {
    ...
    function bridgeAddress(
        ...
        return vLighthouse().bridgeAddress(symbol());
    }
    ...
    function faucet(
        ...
        IRootChainManager(vLighthouse().addressByKey(ROOT_CHAIN_MANAGER_PROXY_ADDRESS)).depositFor(bridgeAddress(), address(this), abi.encodePacked(tokens));
    }
    ...
    function mint(
       ...
       IRootChainManager(vLighthouse().addressByKey(ROOT_CHAIN_MANAGER_PROXY_ADDRESS)).depositFor(bridgeAddress(), address(this), abi.encodePacked(balanceOf(address(this))));
    }
...
//        underlying().transfer(redeemAddress(), underlyingTokens);
    ...
    function withdraw(
        ...
        setBalance(msg.sender, balanceOf(msg.sender).sub(amount, "VErc20::withdraw - insufficient tokens"));
    ...
    function distributeUnderlyingEquivalent(
        ...
        for (uint i = 0; i < who.length; i++) {
            require(transfer(who[i], tokens[i]), "VErc20::distributeUnderlyingEquivalent - failed to transfer tokens");
        }
