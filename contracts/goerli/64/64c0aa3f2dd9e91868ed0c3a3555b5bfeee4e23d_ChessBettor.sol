...
contract ChessBettor is Ownable {
    ...
    function formatData(address _address, uint256[] memory gameList) internal view returns(ReturnableUserData memory fullPlayerData) {
    ...
    function getGameDetails(uint256 _gameId) public view returns(ReturnableGameData memory, Vote[] memory) {
    ...
    function getPlayerStatsForUnclaimedGames(address _address) external view returns(ReturnableUserData memory) {
        return formatData(_address, allPlayerData[_address].gamesWithUnclaimedBalance);
    }

    function getPlayerStatsForAllGames(address _address) external view returns(ReturnableUserData memory) {
