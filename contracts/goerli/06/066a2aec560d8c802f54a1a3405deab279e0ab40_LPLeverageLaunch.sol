...
contract LPLeverageLaunch is OwnableStatic {
  ...
  function _updateTWAP( address lentToken ) internal {
    ...
    address token0 = IUniswapV2Pair(priceSource).token0();
      ...
      if( token0 == address(_weth9) ) {
        ...
        twapForToken[lentToken] = FixedPoint.uq112x112( uint224( ( price0Cumulative_ - lastCumulativePrice) / timeElapsed ) ).decode();  
      } else {
        lastCumulativePriceForToken[lentToken] += price1Cumulative_;
        twapForToken[lentToken] = FixedPoint.uq112x112( uint224( ( price1Cumulative_ - lastCumulativePrice) / timeElapsed ) ).decode();
      }
    }
  }
  ...
  mapping( address => uint256 ) lastCumulativePriceForToken;
  mapping( address => uint256 ) twapForToken;
