...
contract AbachiTreasury is AbachiAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    /**
     * @notice allow approved address to deposit an asset for ABI
    ...
    function deposit(
        ...
        // mint ABI needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        ABI.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn ABI for reserves
     * @param _amount uint256
     * @param _token address
     */
    ...
    /**
     * @notice mint new ABI using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        ABI.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or ABI from the treasury, using sABI as collateral. This might allow an
     * sABI holder to provide ABI liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(ABI)) {
            require(permissions[STATUS.ABIDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sABI.changeDebt(value, msg.sender, true);
        require(sABI.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(ABI)) {
            ABI.mint(msg.sender, value);
            abiDebt = abiDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sABI.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with ABI
     * @param _amount uint256
     */
    function repayDebtWithABI(uint256 _amount) external {
        require(permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.ABIDEBTOR][msg.sender], notApproved);
        ABI.burnFrom(msg.sender, _amount);
        sABI.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        abiDebt = abiDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(ABI), _amount, _amount);
    }
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SABI) {
            // 9
            sABI = IsABI(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(ABI.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns ABI valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(ABI)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return ABI.totalSupply() - abiDebt;
    }
}
