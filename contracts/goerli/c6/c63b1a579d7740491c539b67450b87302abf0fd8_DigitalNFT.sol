  ...
  ERC721Metadata
{
   ...
   */
  mapping (uint256 => string) internal projectIdToUri;

  /**
   * @dev Mapping from NFT ID to artist address.
   */
  mapping (uint256 => address) internal projectIdToArtist;
 ...
 function tokenArtist(
  ...
  { 
    ...
    if(  keccak256(bytes(projectIdToUri[_tokenId])) == keccak256(bytes(""))  ){
        projectIdToUri[_tokenId] = _uri;
    }
 ...
 function _setProjectArtist(
    ...
    validNFToken(_tokenId)
  {
    ...
    if(  keccak256(bytes(projectIdToUri[_tokenId])) == keccak256(bytes(""))  ){
        projectIdToArtist[_tokenId] = _artist;
    }
