...
// File: contracts/JohnAirdrop.sol
...
/// @title Eleven-Yellow JOHN token airdrop service
...
contract JohnAirdrop is Ownable, ReentrancyGuard {
    using SafeMath for uint256;

    address public immutable john;
    ...
    /// @param john_ JOHN ERC20 contract address
    ...
    /// @dev Expects JOHN token contract address
    ...
    function claim(
        ...
        if (IERC20(john).transfer(claimant_, claim_) == true) {
    ...
    /// @dev Unclaimed JOHN tokens cannot be recovered through this function, only with end()
    ...
    function recover(
        ...
        require(address(john) != token_, "Recover JOHN on end");
        _recover(token_, amount_, recipient_);
    }

    /// @notice Ends airdrop functionality
    /// @param recipient_ address where re-claimed JOHN and ETH should be sent
    /// @dev After the end time, JOHN tokens are recovered to the given recipient & contract is destroyed
    function end(address payable recipient_) public onlyOwner {
        require(block.timestamp > endsAfter, "Cannot end yet");
        _recover(address(john), getBalance(), recipient_);
        selfdestruct(recipient_);
    }

    /// @notice Get balance of JOHN tokens owned by airdrop
    function getBalance() public view returns (uint256) {
        return IERC20(john).balanceOf(address(this));
    }
