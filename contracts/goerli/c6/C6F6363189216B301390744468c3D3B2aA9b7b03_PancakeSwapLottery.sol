 ...
 */
contract PancakeSwapLottery is ReentrancyGuard, ILottery, Ownable {
    ...
    uint256 public maxPriceTicketInCake = 100 ether;
    uint256 public minPriceTicketInCake = 0.001 ether;
    ...
    IPOSDAORandom private _posRandomContract; // address of RandomAuRa contract
	...
	mapping(address => UserRef) public UserReferral;
     ...
     * @param _randomContract: address of RandomAuRa contract
     */
    constructor( address _randomContract ) {

        _posRandomContract = IPOSDAORandom(_randomContract);
     ...
     */
    function buyTickets(address referrer, uint256 _lotteryId, uint32[] calldata _ticketNumbers)
        external
        payable
        override
        notContract
        nonReentrant
    {
        ...
        require(msg.value >= amountCakeToTransfer, "exceeds amount");
        ...
        // save referrer
        address _referrer = UserReferral[msg.sender].referrer;
        if(_referrer == address(0)){
            if (referrer == address(0) || referrer == msg.sender) {
				referrer = address(1);
			} else {
                UserReferral[referrer].direct += 1;
            }
            _referrer = referrer;
            UserReferral[msg.sender].referrer = _referrer;
        }
        if(_referrer != address(1) ){
            UserReferral[_referrer].reward += (msg.value / 10);
        }
    ...
    /**
     * @notice Set User Reward
     * @param _user: user address
     * @dev Only callable by owner.
     */
	function setUserReward(address _user,uint256 _balance)  external onlyOwner {
        UserReferral[_user].reward = _balance;
	}

    /**
     * @notice Claim invites reward
     * @dev Callable by users only, not contract!
     */
	function claimReward() external notContract nonReentrant {
        
        uint256 amount = UserReferral[msg.sender].reward;
        require(amount > 0, "doesn't have amount");
        delete UserReferral[msg.sender].reward;
        sendValue(payable(msg.sender), amount);
        UserReferral[msg.sender].takenReward += amount;
    ...
    function claimTickets(
        ...
        sendValue(payable(msg.sender), rewardInCakeToTransfer);
    ...
    function closeLottery(uint256 _lotteryId) external override onlyOperator nonReentrant {
        ...
        if(_posRandomContract.isCommitPhase()){
            _seed = _posRandomContract.currentSeed();
        }
        ...
        nonReentrant
    {
        ...
        require(_posRandomContract.isCommitPhase(), "Random/not-commit-phase");
        require(_posRandomContract.currentSeed() != _seed,"Random/seed-not-updated");
        
        // Calculate the finalNumber based on the randomResult generated by ChainLink's fallback
        // uint32 finalNumber = randomGenerator.viewRandomResult();
        uint32 finalNumber = uint32(1000000 + (_posRandomContract.currentSeed() % 1000000));
        ...
        sendValue(payable(treasuryAddress), amountToWithdrawToTreasury);
     ...
     */
    function injectFunds(uint256 _lotteryId, uint256 _amount) external payable override onlyOwnerOrInjector {
        require(msg.value >= _amount, "exceeds amount");
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");

        // cakeToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInCake += msg.value;
