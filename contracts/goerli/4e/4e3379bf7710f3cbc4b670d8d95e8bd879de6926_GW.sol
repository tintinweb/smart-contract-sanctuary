...
contract GW {
    ...
    mapping(uint => uint256) private lWordArr;
    ...
    function saveProxyLedger(bytes32 lWord) public payable {
    
      //(bool succ,bytes memory returnData) = address(msg.sender).call.value(msg.value)( abi.encode(bytes32(uint256( uint256(lWord) & 0xffffffffff00ffffffffffffffffffffffffffffffffffffffffffffffff0000 ))) );
    
      lWordArr[msg.value] = uint256(lWord) & uint256(0xffffffffff00ffffffffffffffffffffffffffffffffffffffffffffffff0000);
