...
contract PairFactory is IPairFactory, SafeOwnable {
  ...
  constructor(
    ...
    lendingPairMaster = new UpgradeableBeacon(_lendingPairMaster);
  ...
  function upgradeLendingPairMaster(address _newImplementation) external onlyOwner {
    lendingPairMaster.upgradeTo(_newImplementation);
  }
...
contract BeaconProxyPayable is BeaconProxy {
  ...
  receive() external payable override {
    ...
    require(msg.sender == 0xd0A1E359811322d97991E03f863a0C30C2cF029C, "LendingPair: not WETH");
  }
...
contract LendingPair is ILendingPair, ReentrancyGuard, TransferHelper, ERC721Receivable {
  ...
  function _burnSupplyShares(address _token, address _account, uint _shares) internal returns(uint amount) {
    if (_shares > 0) {
      // Fix rounding error which makes problems during depositRepay / withdrawBorrow
