...
contract TestArbitrage {
    ...
    event Swapped(uint256 value, address indexed from, address indexed to);
    ...
    function swap2(
        ...
        // 从用户账户转账到pair[0]，作为初始触发资金
        // 这里只需要提前授权一次即可，操作相对方便
        TransferHelper.safeTransferFrom(path[0], msg.sender, pairs[0], amountIn);

        for(uint i = 0; i < path.length-1; i++) {
            
            // 每个币都需要提前给testArbitrage授权一定的数量
            // 这一步执行会出错，应为此时的 msg.sender/owner 变成了合约，相当于合约给pair授权，但此时合约上还没钱，钱在用户账户上
            // 解决的方法有3：
            // 1.合约的账户上留一部分钱
            // 2.使用闪电贷功能，先从uniswap中借贷一部分钱
            // 3.预先每个token都对合约做好无限授权
            ...
            IUniswapV2Pair(pairs[i]).swap(
                amount0Out,
                amount1Out, 
                i == pairs.length -1 ? msg.sender : pairs[i+1], // 传给下一个pair，或者直接打给用户
                new bytes(0)
            );

            emit Swapped(i, path[i], path[i+1]);
        }
    }
}