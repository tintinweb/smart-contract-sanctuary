...
contract Campaign is Ownable, ReentrancyGuard, Pausable {
    ...
    constructor () {
        ...
        // Goerli Tellor Address:
        tellor = ITellor(0x20374E579832859f180536A69093A126Db1c8aE9);
        // TODO: Align with Tellor the response format of ETH/USD to integrate with Buy functions.
    }
    ...
    */
     ...
     */
    function initialize(string calldata _name, IERC20 _token, uint256 _duration, uint256 _openTime, uint256 _releaseTime, uint256 _ethRate, uint256 _ethRateDecimals, address _wallet) external {
        ...
        emit CampaignCreated(name, address(token), openTime, closeTime, releaseTime, etherConversionRate, etherConversionRateDecimals, fundingWallet, owner);
    }
     ...
     */
    function getErc20TokenConversionRate(IERC20 _token) public view returns (uint256) {
    ...
    function getBuyableTokens(address _address) public view returns (uint256) {
        return etherConversionRate.mul(1 ether).mul(100000).div(getCurrentEthValue()).div(10 ** etherConversionRateDecimals).sub(tokenSoldMapping[_address]);
    }
    ...
    function getClaimableTokens(address _address) public view returns (uint256) {
        ...
        /**
     * @dev Allows the contract to get the latest value for the ETH/USD price feed
     * @notice Return Zero if it isn't able to retreive a value. Dividing by 10000 we get the dollar value. ex 180000 ($1800.00)
     * @return value the value retrieved
     */
    function getCurrentEthValue() public view returns (uint256) {
        uint8 _ethRequestId = 1;
        uint256 _count = tellor.getNewValueCountbyRequestId(_ethRequestId);
        uint256 _time = tellor.getTimestampbyRequestIDandIndex(_ethRequestId, _count - 1);
        uint256 _value = tellor.retrieveData(_ethRequestId, _time);
        return _value.div(10000);
    }
     ...
     */
    function setEtherConversionRateDecimals(uint256 _rateDecimals) external onlyOwner {
        etherConversionRateDecimals = _rateDecimals;
        emit CampaignStatsChanged();
    }

    /**
     * @notice Owner can set the new tellor smart contract by address
     * @param _tellorContract Tellor smart contract address
     */
    function setTellorContract(ITellor _tellorContract) external onlyOwner {
        tellor = _tellorContract;
        emit CampaignStatsChanged();
    }
     ...
     * @param _token address of token to query token exchange rate
     */
    function setErc20TokenConversionRate(IERC20 _token, uint256 _rate) external onlyOwner {
    ...
    */
    function buyTokenByEther(address _beneficiary) public whenNotPaused nonReentrant  payable {
        ...
        require(tokenSoldMapping[_beneficiary] + tokens <= etherConversionRate.mul(1 ether).mul(100000).div(getCurrentEthValue()).div(10 ** etherConversionRateDecimals), "ICO_CAMPAIGN::MAX_1_ETH_TOTAL"); 
        require(tokens <= token.balanceOf(address(this)),"ICO_CAMPAIGN::TOKEN_AMOUNT_NOT_AVAILABLE");
    ...
    */
    function buyTokenByToken(address _beneficiary, IERC20 _token, uint256 _amount) public whenNotPaused nonReentrant {
        ...
        require(tokenSoldMapping[_beneficiary] + tokens <= erc20TokenConversionRate[_token].mul(1 ether).mul(100000).div(getCurrentEthValue()), "ICO_CAMPAIGN::MAX_1000_USD_TOTAL");
        require(tokens <= token.balanceOf(address(this)),"ICO_CAMPAIGN::TOKEN_AMOUNT_NOT_AVAILABLE");
        ...
        emit TokenPurchaseByToken(msg.sender, _beneficiary, address(_token), _amount, tokens);
    }
    ...
    * @return Number of tokens that can be purchased with the specified _weiAmount
    */
    function _getTokenToTokenAmount(IERC20 _token, uint256 _amount) internal view returns (uint256) {
    ...
    */
    function _forwardTokenFunds(IERC20 _token, uint _amount) internal {
    ...
    * @dev  Only pay when use Lemonade to register ICO Campaign
    */
    function _payPlatformTokenFee(IERC20 _token, uint256 _amount) private returns (uint) {
      address payable platformRevenueAddress = address(uint160(_getPlatformRevenueAddress()));
      uint256 platformFeeRate = _getPlatformFeeRate();
   ...
   * @param _amount Amount of wei to transfer
   */
    function _transferToken(IERC20 _token, address _to, uint256 _amount) private {
