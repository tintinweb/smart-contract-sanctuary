...
/* https://docs.openzeppelin.com/contracts/4.x/api/token/erc1155#ERC1155PresetMinterPauser */

contract Fraktal1155 is ERC1155PresetMinterPauser  {
  // should be a clonable contract (right?)
    ...
    mapping (address =>  mapping (address => uint)) public transferVotes;//private
    ...
    constructor(string memory _urlIpfs) public ERC1155PresetMinterPauser(_urlIpfs) {
        ...
        subIds = [0,1];
        amounts = [1,10000];
        mintBatch(tx.origin, subIds, amounts, ''); // later should be to _msgSender() but im calling this contract from the market
      }
    ...
    function unlockSharesTransfer(uint numShares, address _to) public {
      require(transferVotes[tx.origin][_to] >= numShares, 'You dont have so many shares locked'); //  [_to]  must have enough shares to unlock;
...
contract YourContract is Ownable, ReentrancyGuard, ERC1155Holder {
  ...
  /* fee? */
  /* bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE"); // see access control */
...
// all this calls should be made directly to each token contract
  ...
  function lockShares(uint _tokenId, uint256 _amount, address _to) public {
    // bug alert: listed auction can be locked.. and sended??
    ...
    nonReentrant
  {
    ...
    /* emit FractionsBought */
    emit Bought(_msgSender(), from, _tokenId, _numberOfShares);
    }

  /* function prepareNft(uint _tokenId) public {
    require(fraktalNFTs[_tokenId].balanceOf(_msgSender(),0) == 1, 'You must have this NFT');
    require(fraktalNFTs[_tokenId].balanceOf(_msgSender(),1) == 10000, 'You must have the 10k fraktions');
    lockShares(_tokenId, 10000, address(this));
    makeSafeTransfer(address(this), _tokenId, 0, 1);
    unlockShares(_tokenId, 10000, address(this));
    emit TokenTransfered(_msgSender(), address(this), _tokenId, 0, 1);
  } */
  ...
  function listItem( // owner needs to do approval for the market to handle the tokens listed
    ...
    // Warning!
    // require that the subId 0 is in the property of the market
    ...
    // get _tokenId address
    listings[_tokenId][_msgSender()] = listing;
    //we need to approve the contract to handle listings?
...
//-Getters-/////////////////////////////////////////////////////////////////////
  ...
  function getLockedShares(address _whom, uint _tokenId) public view returns(uint256){
    return fraktalNFTs[_tokenId].getLocked(_whom); //isnt working
  }
  ...
  function getFraktalAddress(uint _tokenId) public view returns(address){
    return address(fraktalNFTs[_tokenId]);//props.address try: .address
  }
...
///////////////////////////////////////////////////////////////////////////////////////
  ...
  require(fraktalNFTs[_id].hasRole(PAUSER_ROLE, _msgSender()), 'You have no pauser roles');
  fraktalNFTs[_id].pause();
  }

  function unpause(uint _id) public {
  require(fraktalNFTs[_id].hasRole(PAUSER_ROLE, _msgSender()),'You have no pauser roles');
  fraktalNFTs[_id].unpause();
  } */

  /* function getPaused(uint _id) public view returns(bool){
  return(fraktalNFTs[_id].paused());
  } */

  /* function getPauser(uint _id) public view returns(address){
    return(fraktalNFTs[_id].getCreator());
  } */

  /* function isPauser(address addr,uint _id) external view returns(bool){
  return fraktalNFTs[_id].hasRole(PAUSER_ROLE, addr);
  } */
  }
