...
contract OpenMetaToken is ERC20, ERC20Permit, Ownable {
    ...
    ) ERC20("Open Meta", "OM") ERC20Permit("Open Meta") {
    ...
    function claimTokens(uint256 amount, bytes32[] calldata merkleProof) external {
        ...
        require(valid, "OM: Valid proof required.");
        require(!isClaimed(index), "OM: Tokens already claimed.");
    ...
    function sweep(address dest) external onlyOwner {
        require(block.timestamp > claimPeriodEnds, "OM: Claim period not yet ended");
    ...
    function setMerkleRoot(bytes32 _merkleRoot) external onlyOwner {
        require(merkleRoot == bytes32(0), "OM: Merkle root already set");
    ...
    function mint(address dest, uint256 amount) external onlyOwner {
        require(amount <= (totalSupply() * mintCap) / 10000, "OM: Mint exceeds maximum amount");
        require(block.timestamp >= nextMint, "OM: Cannot mint yet");
