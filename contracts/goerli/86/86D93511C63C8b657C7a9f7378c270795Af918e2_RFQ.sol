...
contract RFQ is ReentrancyGuard, IRFQ, RFQLibEIP712, SignatureValidator {
     ...
     */
    function setAllowance(address[] calldata _tokenList, address _spender)
        external
        override
        onlyOperator
    {
    ...
    function closeAllowance(address[] calldata _tokenList, address _spender)
        external
        override
        onlyOperator
    {
    ...
    // settle
    function _settle(RFQLibEIP712.Order memory _order, GroupedVars memory _vars)
        internal
        returns (uint256)
    {
...
contract RFQLibEIP712 is BaseLibEIP712 {
    ...
    function _getOrderSignDigest(Order memory _order)
        internal
        view
        returns (bytes32 orderSignDigest)
    {
        orderSignDigest = keccak256(
            abi.encodePacked(EIP191_HEADER, EIP712_DOMAIN_SEPARATOR, _getOrderHash(_order))
        );
    }

    function _getOrderSignDigestFromHash(bytes32 _orderHash)
        internal
        view
        returns (bytes32 orderSignDigest)
    {
        orderSignDigest = keccak256(
            abi.encodePacked(EIP191_HEADER, EIP712_DOMAIN_SEPARATOR, _orderHash)
        );
    }
    ...
    function _getTransactionHash(Order memory _order)
        internal
        pure
    ...
    function _getTransactionSignDigest(Order memory _order)
        internal
        view
        returns (bytes32 transactionSignDigest)
    {
        transactionSignDigest = keccak256(
            abi.encodePacked(EIP191_HEADER, EIP712_DOMAIN_SEPARATOR, _getTransactionHash(_order))
        );
    }

    function _getTransactionSignDigestFromHash(bytes32 _txHash)
        internal
        view
        returns (bytes32 transactionSignDigest)
    {
        transactionSignDigest = keccak256(
            abi.encodePacked(EIP191_HEADER, EIP712_DOMAIN_SEPARATOR, _txHash)
        );
    }
}
  ...
  |__________________________________*/
    ...
    function isValidSignature(
        ...
        if (signatureType == SignatureType.Illegal) {
            ...
            isValid =
                ERC1271_MAGICVALUE == IERC1271Wallet(_signerAddress).isValidSignature(_data, _sig);
            return isValid;
