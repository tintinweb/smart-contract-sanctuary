...
contract TastyToastys is Ownable, ERC721A, ReentrancyGuard {
  ...
  mapping(address => bool) public OGwhitelistClaimed;
  constructor(
    uint256 maxBatchSize_,
    uint256 collectionSize_,
  ...
  function OGwhitelistMint(bytes32 [] calldata _merkleProof, uint256 quantity) public {
    //Basic data validation to ensure the wallet hasn't already claimed
    require(allowOGlistMintStarted(), 'Whitelist not started');
    require(!OGwhitelistClaimed[msg.sender], "Address has already claimed.");
    ...
    //Mark address as having claimed their token.
    OGwhitelistClaimed[msg.sender] = true;
