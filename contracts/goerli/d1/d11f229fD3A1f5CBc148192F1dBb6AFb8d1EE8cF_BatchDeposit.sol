...
/*
*   @dev
*   todo add safe math and ensure no math errors => can this be deprecated as > 0.8.0?
    todo = reentry protection / etc => investigate if this is needed -> do a reentrancy and see effect.
...
contract BatchDeposit is Pausable {
    ...
    uint256 constant MAX_VALIDATORS = 10;
    ...
    /**
     *   @dev Updates governance address
    ...
    function setGovernance(address _governance) onlyGovernance external {
        require(msg.sender != address(0x0), "G: ZeroAddress");
        governanceBuffer = _governance;
        emit GovernanceSet(_governance);
    }

    
    /** NOTE: This is to prevent the bricking of the governance address. <- It will always be recoverable
     *  @dev Allows the proposed governance to accept their governance upgrade
     **/
    ...
    /**
     *   @dev Updates deposit contract address
     *   @param _depositContract new governance address to control contract
     *
     **/
    function setDepositContract(address _depositContract) external onlyGovernance {
        depositContract = IDepositContract(_depositContract);
    }

    /**
     *   @dev Withdraws all money in contract to governance in case of disaster scenario
     *
     */
    ...
    /**
     *   @dev Pause access to functions marked "whenNotPaused" contract, required sender == governance
     *
     */
    ...
    /**
     *   @dev Unpause access to functions marked "whenNotPaused" contract, required sender == governance
     *
     */
    ...
    /**
     *   @dev Recovers any ERC20 tokens accidentally locked in contract
     *
     */
    function recoverFunds(address _token) external whenNotPaused onlyGovernance {
    ...
    function _pad64(bytes memory _b) internal pure returns (bytes memory) {
        ...
        if (32 == _b.length) return BytesLib.concat(_b, zero32);
            ...
            return
                BytesLib.concat(
                    _b,
                    BytesLib.slice(zero32, 0, uint256(64).sub(_b.length))
                );
    }
    ...
    function _batchStake(
        ...
        require(
            ...
            "BatchDeposit: You can deposit max 10 validators at a time"
        );
        ...
        for (uint256 i = 0; i < _pubkeys.length; i++) {
            _stake(_pubkeys[i], _withdrawalCredentials[i], _signatures[i], _dataroots[i]);
        }
    }
    ...
    function _stake(
        ...
        bytes32 signatureRoot = sha256(
            abi.encodePacked(
                ...
                sha256(
                    _pad64(
                        BytesLib.slice(_signature, 64, SIGNATURE_LENGTH.sub(64))
                    )
                )
            )
        );
...
/**
    @dev This was originally sol 0.6.11
    @todo Test it works with sol 0.8.6
 */

// taken from : https://github.com/lidofinance/lido-dao/blob/master/contracts/0.6.11/deposit_contract.sol
