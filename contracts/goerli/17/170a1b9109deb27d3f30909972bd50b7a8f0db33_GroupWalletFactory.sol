...
contract GroupWalletFactory {
    ...
    function getCommitment(bytes32 _domainHash) private view returns (uint64 comm) {
      comm = uint64( (uint256( uint256( commitments[_domainHash] ) & 0xffffffffffffffffffffffff0000000000000000000000000000000000000000 )>>160) & 0x0000000000000000000000000000000000000000ffffffffffffffffffffffff );
      return comm;
    }
    
    function getOwner(bytes32 _domainHash) public view returns (address iToken) {
          iToken = address( uint160( uint256( commitments[_domainHash] ) & 0x000000000000000000000000ffffffffffffffffffffffffffffffffffffffff ) );
          return iToken;
    }
    
    function saveOwner(address _iToken, bytes32 _domainHash) private {
          uint256 i = uint256(commitments[_domainHash]);
          commitments[ _domainHash ] = bytes32(uint256(uint160(_iToken)) + uint256(i & 0xffffffffffffffffffffffff0000000000000000000000000000000000000000));
    }

    function saveCommitment(bytes32 input, bytes32 _domainHash) private {
          uint256 i = uint256(commitments[_domainHash]);
          commitments[_domainHash] = bytes32(uint256( (uint256(input)<<160) & 0xffffffffffffffffffffffff0000000000000000000000000000000000000000 ) + uint256(i & 0x000000000000000000000000ffffffffffffffffffffffffffffffffffffffff));
    }
    ...
    function getInstallTime(bytes32 _domainHash) public view returns (uint256 iTime) {
          ...
          iTime = uint256( (uint256( uint256(i) & 0xffffffffffffffffffffffff0000000000000000000000000000000000000000 )>>160) & 0x0000000000000000000000000000000000000000ffffffffffffffffffffffff );
          return iTime;
    }

    function getProxyToken(bytes32 _domainHash) public view returns (address iOwner) {
          uint256 i = uint256(installations[ getCommitment(_domainHash) ]);
          iOwner = address( uint160( uint256(i) & 0x000000000000000000000000ffffffffffffffffffffffffffffffffffffffff ) );
          return iOwner;
    }
    ...
    function saveProxyToken(address _iOwner, bytes32 _domainHash) private {
          ...
          installations[ hsh ] = uint256(uint160(_iOwner)) + uint256(i & 0xffffffffffffffffffffffff0000000000000000000000000000000000000000);
    }

    function saveInstallTime(uint256 input, bytes32 _domainHash) private {
          uint64 hsh  = getCommitment(_domainHash);
          uint256 i = uint256(installations[ hsh ]);
          installations[ hsh ] = uint256( (uint256(input)<<160) & 0xffffffffffffffffffffffff0000000000000000000000000000000000000000 ) + uint256(i & 0x000000000000000000000000ffffffffffffffffffffffffffffffffffffffff);
    }
    ...
    modifier isProxy(bytes32 domainHash, address proxyT) {
          require(getProxyToken(domainHash)==proxyT,"bad TokenProxy!");
        _;
    }
    ...
    function invite(bytes memory _base32Hex,address[] memory _owners,bytes memory data128,bytes32 _domainHash) public payable validOwners(_owners) byInitiator(_domainHash)
    {
      uint256 amount;
      if (msg.value > 0) {
      ...
      if (_base32Hex.length > 0) resolverContract.setContenthash(_domainHash,abi.encodePacked(_base32Hex)); // save content = ipfs base32hex string
    }


    function install(address[] memory _owners, bytes32[] memory _labels, bytes32[] memory _domainLabels, bytes32 _domainHash) public payable validOwners(_owners) byInitiatorOrMember(_domainHash)
    {
    ...
    function inviteAndInstall(address[] memory _owners, bytes32[] memory _labels, bytes32[] memory _domainLabels, bytes memory data128, bytes32 _domainHash) public payable validOwners(_owners) byInitiator(_domainHash) {
      uint256 amount;
      uint64 time = uint64(now*1000) & uint64(0xffffffffffff0000);
      uint l = _owners.length;
      
      require(_labels.length       == l, "Nb of _labels != nb of owners!");
      require(_domainLabels.length == l, "Nb of _domainLabels != nb of owners!");  
    ...
    function inviteInstallToken(address[] memory _owners, bytes32[] memory _labels, bytes32[] memory _domainLabels, bytes memory data128, bytes memory data, bytes32 _domainHash, address masterCopy) public payable validOwners(_owners) byInitiator(_domainHash) returns (ProxyToken proxy) {      
      ...
      uint64 time = uint64(now*1000) & uint64(0xffffffffffff0000);
      uint256 amount = uint256(msg.value / (uint256(_owners.length*2)));
      ...
      for (uint i=0; i<_owners.length; i++) {
        ...
        require(address(uint160(_owners[i])).send(0.1 ether),"Sending ether failed.");
        emit Deposit(address(this), 0.1 ether);
