...
contract GroupWalletFactory2 {
    ...
    function getOwners(bytes32 _dHash) external view returns (address[] memory)
    {
      ...
      if (a!=msg.sender) return AbstractGroupWalletProxy( a ).getOwners();
    ...
    {
      uint256 c = commitments[_dHash];
      my_require(address( uint160(c & k_aMask) )==msg.sender,"- only by initiator.");                   // owner  getOwner(_dHash)
    ...
    function inviteInstallToken_q31n(bytes32[] calldata _mem) external payable {      
      ...
      
      {
       ...
       my_require(address( uint160(c & k_aMask) )==msg.sender, "- only by initiator.");
    ...
    function createProxyToken(bytes32 _domainHash, address masterCopy, bytes memory data) public payable returns (ProxyToken proxy)
    {
        ...
        my_require(address(uint160(c & k_aMask))==msg.sender, "- only by initiator.");
        my_require(masterCopy != k_add00, "MasterCopy != 0x0");
    ...
    function createProxyGroupWallet(bytes32 _domainHash, address masterCopy, bytes memory data) public payable returns (ProxyGroupWallet proxy)
    {
      ...
      proxy = new ProxyGroupWallet(masterCopy, reverseContract, "someProjectTitle.eth");
      uint256 val = msg.value;
    ...
    function withdraw() external {
      my_require(GWFowner==msg.sender,"Only GroupWalletFactory owner");
      my_require(address(uint160(msg.sender)).send(address(this).balance-1),"Withdrawing ETH failed.");
    }
    
    function() external payable
    {
      my_require(false,"GroupWalletFactory fallback! GWF does NOT accept payments.");
    }

    constructor (AbstractETHRegistrarController _controller, AbstractResolver _resolver, AbstractBaseRegistrar _base, AbstractENS _ens, AbstractReverseRegistrar _reverse) public {
