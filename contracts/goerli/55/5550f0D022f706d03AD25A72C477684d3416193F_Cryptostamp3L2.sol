...
// File: contracts/Cryptostamp3L2.sol

/*
 * Implements ERC 721 NFT standard: https://github.com/ethereum/EIPs/issues/721.
 */

contract Cryptostamp3L2 is ERC721EnumerableSimple, ERC721ExistsI, ERC721SignedTransferI, CS3PropertiesI {
    ...
    constructor(address _bridgeDataAddress, uint256 _finalSupply, uint256[5] memory _totalColorSupply)
    ERC721("Crypto stamp Edition 3", "CS3")
    {
        ...
        require(_totalColorSupply[0] + _totalColorSupply[1] + _totalColorSupply[2] + _totalColorSupply[3] + _totalColorSupply[4] == _finalSupply,
                "The total color supplies need to match the final supply.");
    ...
    internal
    {
        ...
        uint256 allUnrevealedCount = unrevealedColorCount[0] + unrevealedColorCount[1] + unrevealedColorCount[2] + unrevealedColorCount[3] + unrevealedColorCount[4];
        uint256 colorIndex = _getSemiRandomNumber(colorRevealBlock[tokenId], tokenId, allUnrevealedCount);
        SColors newColor;
        if (colorIndex < unrevealedColorCount[0]) {
            newColor = SColors.Black;
        }
        else if (colorIndex < unrevealedColorCount[0] + unrevealedColorCount[1]) {
            newColor = SColors.Green;
        }
        else if (colorIndex < unrevealedColorCount[0] + unrevealedColorCount[1] + unrevealedColorCount[2]) {
            newColor = SColors.Blue;
        }
        else if (colorIndex < unrevealedColorCount[0] + unrevealedColorCount[1] + unrevealedColorCount[2] + unrevealedColorCount[3]) {
        ...
        unrevealedColorCount[uint8(newColor) - 1] = unrevealedColorCount[uint8(newColor) - 1] - 1;
    ...
    public
    {
        ...
        require(getSColor(_helperTokenId1) == previousColor &&
                getSColor(_helperTokenId2) == previousColor,
                "All tokens involved must have the color");
    ...
    returns(SColors)
    {
        ...
        colorSupply[uint256(_previousColor) - 1] = colorSupply[uint256(_previousColor) - 1] - 1;
        colorSupply[uint256(newColor) - 1] = colorSupply[uint256(newColor) - 1] + 1;
        upgradesDone++;
        return newColor;
    }
