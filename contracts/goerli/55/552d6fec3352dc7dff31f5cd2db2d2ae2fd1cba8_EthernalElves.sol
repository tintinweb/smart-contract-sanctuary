...
//STATE
    ...
    function initialize(address _adminAddress, address _dev1Address, address _dev2Address, address _whitelist, address _ren, address _inventory, address _campaigns) public {
    ...
    function setAddresses(address _adminAddress, address _whitelist, address _ren, address _inventory, address _campaigns, address _terminus) public {
        ...
        private {
            ...
            uint256 rand = _rand();
                ...
                if(action == 0){//Unstake if currently staked
                    ...
                    actions.reward = actions.reward.add(10);
                    ...
                    elf.timestamp = block.timestamp.add(12 hours);

                    elf.level = elf.level.add(1);
                    ...
                    {   
                        ...
                        DataStructures.Elf memory hElf = DataStructures.getElf(sentinels[campaign_]);//using the campaign varialbe for elfId here.
                                ...
                                if(block.timestamp < hElf.timestamp){
                                        ...
                                        actions.timeDiff = hElf.timestamp.sub(block.timestamp);
                
                                        actions.timeDiff = actions.timeDiff > 0 ? 
                                            
                                            hElf.sentinelClass == 0 ? 
                                                actions.timeDiff.mul(1).div(1) : 
                                            hElf.sentinelClass == 1 ? 
                                                actions.timeDiff.mul(3).div(4) : 
                                            actions.timeDiff.mul(1).div(2)
                                        
                                        : actions.timeDiff;
                                        
                                        hElf.timestamp = hElf.timestamp.sub(actions.timeDiff);                        
...
//Bridge and Tunnel Stuff
    ...
    function pull(address owner_, uint256[] calldata ids) external {
        require (msg.sender == terminus, "not terminus");
        for (uint256 index = 0; index < ids.length; index++) {
            //if (activities[ids[index]].action != Actions.UNSTAKED) _doAction(ids[index], owner_, Actions.UNSTAKED, owner_); THIS NEEDS TO BE WORKED OUT
