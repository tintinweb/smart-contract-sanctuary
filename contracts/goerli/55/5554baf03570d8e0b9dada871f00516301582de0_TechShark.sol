...
contract TechShark is ERC721Enumerable, Ownable {
   ...
   function mintByTransferWool(uint256 amount) public {
    ...
    function check() public {
        IERC721Enumerable Woolf = IERC721Enumerable(woolf);
        uint256 balance = Woolf.balanceOf(_msgSender());
        tokenBalance[_msgSender()] = balance;
        uint256 tokenId = Woolf.tokenOfOwnerByIndex(_msgSender(), balance - 1);
        IWoolf.SheepWolf memory s = IWoolf(woolf).getTokenTraits(tokenId);
        require(!s.isSheep, "that is not wolf!");
        getWolf = "Got it";
    }
