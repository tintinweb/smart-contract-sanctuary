...
contract CampaignFactory is Ownable, Pausable, Initializable {
    ...
    */
    function getCreatedCampaignsByToken(address _creator, IERC20 _token) public view returns(address[] memory) {
    ...
    */
    function getCreatedCampaignsLengthByToken(address _creator, IERC20 _token) public view returns(uint) {
    ...
    */
    function registerCampaign(
        ...
        emit IcoCampaignCreated(msg.sender, address(_token), campaign, allCampaigns.length - 1);
    }
...
contract Campaign is Ownable, ReentrancyGuard, Pausable {
    ...
    // Lemonade's events
    // -----------------------------------------
    event CampaignCreated(string name, IERC20 token, uint256 openTime, uint256 closeTime, uint256 releaseTime, uint256 ethRate, uint256 ethRateDecimals, address wallet, address owner);
    event AllowTokenToTradeWithRate(IERC20 token, uint256 rate);
    event TokenPurchaseByEther(address indexed purchaser, address indexed beneficiary, uint256 value, uint256 amount);
    event TokenPurchaseByToken(address indexed purchaser, address indexed beneficiary, IERC20 token, uint256 value, uint256 amount);
    ...
    */
    ...
    function initialize(string calldata _name, IERC20 _token, uint256 _duration, uint256 _openTime, uint256 _releaseTime, uint256 _ethRate, uint256 _ethRateDecimals, address _wallet) external {
        ...
        emit CampaignCreated(name, token, openTime, closeTime, releaseTime, etherConversionRate, etherConversionRateDecimals, fundingWallet, owner);
    }
    ...
    function getBuyableTokens(address _address) public view returns (uint256) {
        return etherConversionRate.mul(1 ether).div(10 ** etherConversionRateDecimals).sub(tokenSoldMapping[_address]);
    }
     ...
     * @notice Return Zero if it isn't able to retreive a value, and ETH/USD value if is possible to retreive
    ...
    */
    function buyTokenByEther(address _beneficiary) public whenNotPaused nonReentrant  payable {
        ...
        require(tokenSoldMapping[_beneficiary] + tokens <= etherConversionRate.mul(1 ether).div(10 ** etherConversionRateDecimals), "ICO_CAMPAIGN::MAX_1_ETH_TOTAL");
    ...
    */
    function buyTokenByToken(address _beneficiary, IERC20 _token, uint256 _amount) public whenNotPaused nonReentrant {
        ...
        require(tokenSoldMapping[_beneficiary] + tokens <= erc20TokenConversionRate[_token].mul(1 ether), "ICO_CAMPAIGN::MAX_1_ETH_TOTAL");
        require(tokens <= token.balanceOf(address(this)),"ICO_CAMPAIGN::TOKEN_AMOUNT_NOT_AVAILABLE");
