...
contract LPLeverageLaunch is OwnableStatic {
  ...
  function lendLiquidity( address loanedToken, uint amount ) external returns ( bool success ) {
    ...
    uint256 lentTokenPrice = _calculateTWAP( loanedToken );

    launchTokenDueForHolder[msg.sender] += ( amount * lentTokenPrice );
  ...
  mapping( address => uint256 ) lastTWAPForToken;
  mapping( address => uint32 ) lastpriceTimestampForToken;
