...
contract clientWallet is Context, Ownable {
  ...
  function initialize(string memory _clientName, address payable _destination) external {
  ...
  function _transferETH() internal {
    ...
    destination.transfer(balanceThisContract);
...
contract clientWalletFactory{

  event walletCreated(string indexed _clientName, address payable _destination, uint256);
  address[] public allClients;

  function createClientWallet(string memory _clientName, address payable _destination) external returns (address payable wallet) {
    ...
    clientWallet(wallet).initialize(_clientName, _destination);
    clientWallet(wallet).transferOwnership(msg.sender);
