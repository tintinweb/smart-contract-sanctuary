...
contract HoneyFarmQueen is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    uint32 public HONEY_BEARS_REWARDS_PER_ROUND;

    uint32 public HONEY_UNSTAKED_BEE_REWARDS_PER_ROUND;

    uint32 public HONEY_STAKED_BEE_REWARDS_PER_ROUND;

    uint8 public MIN_USAGE_PER_HIVE;

    uint32 public REWARD_FOR_BURNING_BEE;

    uint16 public ROUNDS_BEFORE_INACTIVE_BEE;

    uint16 private lowestBeeId;

    uint32 public MIN_BURN_AMOUNT_FOR_STAKING_BEE;

    uint32 public MIN_AMOUNT_FOR_ACTIVATE_BEE;

    uint32 public AMOUNT_TO_KEEP_ACTIVE;

    uint256 public MIN_BURN_AMOUNT_FOR_BEE;

    uint256 public MIN_AMOUNT_FOR_ACTIVATE_HIVE;

    uint256 public REWARDS_COOLOFF;

    uint256 public HIVE_CLAIM_COOLOFF;
    ...
    mapping(uint16 => uint256) private lastClaimedBeePerHive;
    ...
    struct Bee {
        uint256 id;
        uint8 active;
        //used to know how many rounds this bee can claim honey before becoming inactive.
    ...
    function initialize() public initializer {
        HONEY_BEARS_REWARDS_PER_ROUND = 100; //this is 1

        HONEY_UNSTAKED_BEE_REWARDS_PER_ROUND = 13; //this is 0.13

        HONEY_STAKED_BEE_REWARDS_PER_ROUND = 9; //this is 0.09

        MIN_USAGE_PER_HIVE = 3;
        REWARD_FOR_BURNING_BEE = 1200; //this is 12

        // solhint-disable-next-line
        ROUNDS_BEFORE_INACTIVE_BEE = 10; //number of rounds that a bee can claim honey before becoming inactive
        ...
        MIN_BURN_AMOUNT_FOR_STAKING_BEE = 700; //this is 7

        MIN_AMOUNT_FOR_ACTIVATE_BEE = 700; //this is 7

        AMOUNT_TO_KEEP_ACTIVE = 23; //this is 0.23

        MIN_BURN_AMOUNT_FOR_BEE = 2300; //this is 23

        MIN_AMOUNT_FOR_ACTIVATE_HIVE = 6900; //this is 69

        REWARDS_COOLOFF = 86400; //one day

        HIVE_CLAIM_COOLOFF = 86400; //one day
    ...
    function claimBeesHoney(uint16[] calldata _beesIds) external nonReentrant {
        ...
        for (uint16 i = 0; i < _beesIds.length; i++) {
            ...
            if (bee.staked == 0) {
                amount += HONEY_UNSTAKED_BEE_REWARDS_PER_ROUND * roundsToReward;
                bee.claimCounter -= roundsToReward;
                if (bee.claimCounter == 0) {
                    bee.active = 0;
                    bee.becameInactiveTime = block.timestamp;
                }
            } else {
                amount += HONEY_STAKED_BEE_REWARDS_PER_ROUND * roundsToReward;
            }
    ...
    function mintBee(uint16 _hiveId) external nonReentrant {
        ...
        require(honey.balanceOf(msg.sender) >= MIN_BURN_AMOUNT_FOR_BEE * 1e16, "Not enough Honey");

        require(hive.getUsageOfMintingBee(_hiveId) < 3, "Inactive Hive, burn 69 Honey");
        ...
        idsAndBees[beeId] = Bee(beeId, 1, ROUNDS_BEFORE_INACTIVE_BEE, 0, STARTING_POINT, 0, 0);

        honey.burn(msg.sender, MIN_BURN_AMOUNT_FOR_BEE * 1e16);
    ...
    function activateHive(uint16 _hiveId) external nonReentrant {
        ...
        require(hive.getUsageOfMintingBee(_hiveId) >= MIN_USAGE_PER_HIVE, "Cap not reached");
        require(honey.balanceOf(msg.sender) >= MIN_AMOUNT_FOR_ACTIVATE_HIVE * 1e16, "Not enough Honey");

        honey.burn(msg.sender, MIN_AMOUNT_FOR_ACTIVATE_HIVE * 1e16);
    ...
    function activateBees(uint16[] calldata _beesIds) external nonReentrant {
        ...
        for (uint16 i = 0; i < _beesIds.length; i++) {
             ...
             * - reset claimCounter to MIN_USAGE_PER_BEE which is the max claiming before it becomes inactive
            ...
            bee.active = 1;
            bee.claimCounter = ROUNDS_BEFORE_INACTIVE_BEE;
            bee.lastRewardTime = block.timestamp;
    ...
    function stakeBees(uint16[] calldata _beesIds) external nonReentrant {
        ...
        for (uint16 i = 0; i < _beesIds.length; i++) {
            ...
            if (bees.ownerOf(_beeId) != msg.sender) continue;
            amountOfHoney += MIN_BURN_AMOUNT_FOR_STAKING_BEE;
            bee.staked = 1;
    ...
    function randBeeId() internal returns (uint16 _id) {
        uint16 entropy;
        uint16 maxSupply = bees.getMaxSupply();
        uint16 totalSupply = uint16(bees.totalSupply());
        require(totalSupply < maxSupply, "MAX_SUPPLY reached");
        while (true) {
            uint16 rand = uint16(
                uint256(
                    keccak256(
    ...
    function getLastRewardedByBees(uint16 _beeId) external view returns (uint256) {
    ...
    function setNoOfRoundsBeforeInactiveBee(uint16 _rounds) external onlyOwner {
        ROUNDS_BEFORE_INACTIVE_BEE = _rounds;
    }
...
abstract contract BearsDeluxeI is Ownable, IERC721 {
    function mint(address _owner, uint16 _tokenId) external virtual;

    function exists(uint16 _tokenId) external view virtual returns (bool);
...
abstract contract BeesDeluxeI is Ownable, IERC721, IERC721Enumerable {
    ...
    function getMaxSupply() external view virtual returns (uint16);
...
abstract contract HoneyHiveDeluxeI is Ownable, IERC721 {
    ...
    function increaseUsageOfMintingBee(uint16 _hiveId) external virtual;

    function getUsageOfMintingBee(uint16 _hiveId) external view virtual returns (uint8);

    function resetUsageOfMintingBee(uint16 _hiveId) external virtual;
