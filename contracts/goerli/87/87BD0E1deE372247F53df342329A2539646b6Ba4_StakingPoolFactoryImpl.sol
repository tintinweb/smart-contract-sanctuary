...
contract StakingPoolFactoryImpl is StakingPoolFactory {
    ...
    constructor(
        ...
        require(
            _workerManagerAuthManager != address(0),
            "parameter can not be zero address."
        );
        ctsi = _ctsiAddress;
...
contract StakingPoolImpl is StakingPool, StakingPoolManagementImpl {
    ...
    uint256 immutable public timeToStake;
    uint256 immutable public timeToRelease;
    ...
    function produceBlock(uint256 _index) external override returns (bool) {
        bool isLastStakeCycleOver = staking.getMaturingTimestamp(address(this)) <= block.timestamp;
