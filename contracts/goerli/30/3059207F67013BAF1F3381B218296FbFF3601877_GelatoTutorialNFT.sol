...
contract GelatoTutorialNFT is ERC721Enumerable, Ownable {
    ...
    constructor(address pokeMe_, uint256 blockThreshold_) ERC721("GelatoCones", "CONE") {
        pokeMe = pokeMe_;
    ...
    function tokenURI(uint256 tokenId_) public view virtual override returns (string memory) {
        ...
        parts[0] = base;
        parts[1] = _getView(numLicks);
        parts[2] = uint2str(numLicks);
        parts[3] = '</text></svg>';
    ...
    function _getView(uint256 numLicks) internal view returns (string memory) {
        if (numLicks < views.length) {
            return views[numLicks];
        } else {
