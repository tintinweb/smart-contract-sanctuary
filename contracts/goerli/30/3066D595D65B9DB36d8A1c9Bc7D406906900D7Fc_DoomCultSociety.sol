...
contract DoomCultSociety is ERC721 {
        ...
        assembly {
            {
                ...
                mstore(623, t15)
                mstore(643, '1999/xlink">fffffff<style>.heavy')
                mstore(675, '{font:00000700 30px sans-serif;f')
    ...
    function Beep(uint256 x) public view returns (string memory res) {
        assembly {
mstore(512, '<img src=\'data:image/svg+xml;utf') 
mstore(544, '8,<svg xmlns="http://www.w3.org/') 
mstore8(544, add(x, 0x30))
mstore(576, '2000/svg" xmlns:xlink="http://ww') 
mstore(608, 'w.w3.org/1999/xlink" version="1.') 
mstore(640, '1" id="Layer_1" x="0px" y="0px" ') 
mstore(672, 'viewBox="0 0 100 100" enable-bac') 
mstore(704, 'kground="new 0 0 100 100" xml:sp') 
mstore(736, 'ace="preserve" height="100px" wi') 
mstore(768, 'dth="100px"><g><path d="M28.') 
mstore(800, '1,36.6c4.6,1.9,12.2,1.6,20.9,1.1') 
mstore(832, 'c8.9-0.4,19-0.9,28.9,0.9c6.3,1.2') 
mstore(864, ',11.9,3.1,16.8,6c-1.5-12.2-7.9-2') 
mstore(896, '3.7-18.6-31.3   c-4.9-0.2-9.9,0.') 
mstore(928, '3-14.8,1.4C47.8,17.9,36.2,25.6,2') 
mstore(960, '8.1,36.6z"/><path d="M70.3,9.') 
mstore(992, '8C57.5,3.4,42.8,3.6,30.5,9.5c-3,') 
mstore(1024, '6-8.4,19.6-5.3,24.9c8.6-11.7,20.') 
mstore(1056, '9-19.8,35.2-23.1C63.7,10.5,67,10') 
mstore(1088, ',70.3,9.8z"/><path d="M16.5,5') 
mstore(1120, '1.3c0.6-1.7,1.2-3.4,2-5.1c-3.8-3') 
mstore(1152, '.4-7.5-7-11-10.8c-2.1,6.1-2.8,12') 
mstore(1184, '.5-2.3,18.7C9.6,51.1,13.4,50.2,1') 
mstore(1216, '6.5,51.3z"/><path d="M9,31.6c') 
mstore(1248, '3.5,3.9,7.2,7.6,11.1,11.1c0.8-1.') 
mstore(1280, '6,1.7-3.1,2.6-4.6c0.1-0.2,0.3-0.') 
mstore(1312, '4,0.4-0.6c-2.9-3.3-3.1-9.2-0.6-1') 
mstore(1344, '7.6   c0.8-2.7,1.8-5.3,2.7-7.4c-') 
mstore(1376, '5.2,3.4-9.8,8-13.3,13.7C10.8,27.') 
mstore(1408, '9,9.8,29.7,9,31.6z"/><path d=') 
mstore(1440, '"M15.4,54.7c-2.6-1-6.1,0.7-9.7,3') 
mstore(1472, '.4c1.2,6.6,3.9,13,8,18.5C13,69.3') 
mstore(1504, ',13.5,61.8,15.4,54.7z"/><path') 
mstore(1536, ' d="M39.8,57.6C54.3,66.7,70,73,8') 
mstore(1568, '6.5,76.4c0.6-0.8,1.1-1.6,1.7-2.5') 
mstore(1600, 'c4.8-7.7,7-16.3,6.8-24.8c-13.8-9') 
mstore(1632, '.3-31.3-8.4-45.8-7.7   c-9.5,0.5') 
mstore(1664, '-17.8,0.9-23.2-1.7c-0.1,0.1-0.2,') 
mstore(1696, '0.3-0.3,0.4c-1,1.7-2,3.4-2.9,5.1') 
mstore(1728, 'C28.2,49.7,33.8,53.9,39.8,57.6z"') 
mstore(1760, '/><path d="M26.2,88.2c3.3,2,6') 
mstore(1792, '.7,3.6,10.2,4.7c-3.5-6.2-6.3-12.') 
mstore(1824, '6-8.8-18.5c-3.1-7.2-5.8-13.5-9-1') 
mstore(1856, '7.2c-1.9,8-2,16.4-0.3,24.7   C20') 
mstore(1888, '.6,84.2,23.2,86.3,26.2,88.2z"/>') 
mstore(1920, '  <path d="M30.9,73c2.9,6.8,6.1,') 
mstore(1952, '14.4,10.5,21.2c15.6,3,32-2.3,42.') 
mstore(1984, '6-14.6C67.7,76,52.2,69.6,37.9,60') 
mstore(2016, '.7C32,57,26.5,53,21.3,48.6   c-0') 
mstore(2048, '.6,1.5-1.2,3-1.7,4.6C24.1,57.1,2') 
mstore(2080, '7.3,64.5,30.9,73z"/></g></svg>') 
mstore(2112, '\' />') 
            res := 480
            mstore(res, sub(2216, 480))
            mstore(0x40,4000)
        }        
    }
    ...
    function imageURI(uint256 tokenId) public pure returns (string memory) {
        // string memory result = Base64.encode(getImgData(tokenId), 66, 5);
        string memory result = getImgData(tokenId);
        
        assembly {
            let len := mload(result)
            result := add(result, 37)
            mstore(result, sub(len, 37))
            mstore(add(result, 0x20), or(",", and(0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff, mload(add(result, 0x20)))))
            let ptr := add(result, add(0x20, mload(result)))
            mstore(ptr, '\\,,')
            mstore(result, add(mload(result), 5))

            //let ptr := result
            //mstore(ptr, ',;64:image/svg+xmlbase<img src="')
            //result := sub(result, 32)
            //mstore(result, add(len, 32))
            //mstore(add(result, add(0x20, mload(result))), '"\\>,,')
            //mstore(result, add(5, mload(result)))
            /*
            mstore(result, add(len, 43))
            let ptr := add(result, 0x20)
            mstore(ptr, 'data:image/svg+xml;base64,')
            mstore(add(ptr, 26), '<img src="data:image/svg+xml;bas')
            mstore(add(ptr, 58), or('e64&#44', and(0xffffffffffffffffffffffffffffffffffffffffffffffffff, mload(add(ptr, 58)))))
            mstore(result, add(mload(result), 66))
            */
        }
        return result;
    }
