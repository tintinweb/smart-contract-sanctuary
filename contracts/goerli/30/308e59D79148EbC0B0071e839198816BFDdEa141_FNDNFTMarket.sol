...
abstract contract NFTMarketCore {
  ...
  function _getEscrowedSellerFor(address nftContract, uint256 tokenId) internal view virtual returns (address);
}
  ...
  SendValueWithFallbackWithdraw
{
  ...
  function getIsPrimary(address nftContract, uint256 tokenId) public view returns (bool) {
    ...
    if (owner == address(this)) {
      // If the NFT is in escrow, check the seller which started the auction
      return _getIsPrimary(nftContract, tokenId, _getEscrowedSellerFor(nftContract, tokenId));
    } else {
      return _getIsPrimary(nftContract, tokenId, owner);
    }
  }
  ...
  {
    return _getFees(nftContract, tokenId, _getEscrowedSellerFor(nftContract, tokenId), price);
  }
  ...
  NFTMarketAuction
{
   ...
   */
  function _getEscrowedSellerFor(address nftContract, uint256 tokenId) internal view override returns (address) {
    return auctionIdToAuction[nftContractToTokenIdToAuctionId[nftContract][tokenId]].seller;
  }
