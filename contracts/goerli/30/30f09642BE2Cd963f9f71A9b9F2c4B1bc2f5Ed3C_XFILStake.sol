...
contract Xfile is IERC20, ContextComp,ERC20DetailComp,AccessControlComp {
    /*
# TODO:
  - storage mint / burn log out
  - owner config pipes in
...
*/
    ...
    // inner-store
    address private p_owner;         // ! owner address,control config set methods access
    address private p_mintAuthority; // ! mint auth address (hot)
    address private p_feeRecipient; //  ! fee Receive address (cold)

    uint256 private p_feeBase;// 0.0000005 Fil
    uint256 private p_feeBIPS_denominator; // fee rate base
    uint256 private p_nextN; // mint or burn action sequence no
    uint256 private p_mintFeeBIPS; // ! 
    uint256 private p_burnFeeBIPS; // !
    uint256 private p_burnAmountMin ; // ! minimal burn amount 
    ...
    function mintWithSig(uint256 _amount,bytes32 _nHash,bytes memory _sig) external returns(uint256){
        ...
        uint256 amount_fee = _amount.mul(p_mintFeeBIPS).div(p_feeBIPS_denominator).add(p_feeBase);
    ...
    function burn(bytes memory _toFilAddress,uint256 _amount) external returns (uint256){
        ...
        _amount.notLt(p_feeBase).notLt(p_burnAmountMin).notGt(p_balances[_msgSender()]);

        uint256 amount_fee = _amount.mul(p_burnFeeBIPS).div(p_feeBIPS_denominator).add(p_feeBase);
