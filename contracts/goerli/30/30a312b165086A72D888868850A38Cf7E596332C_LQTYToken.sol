...
*
* 8) (64 + 2/3) million tokens are minted at deployment to the Liquity xHalfLife
*
*/
...
contract LQTYToken is CheckContract, ILQTYToken {
    ...
    string constant internal _NAME = "ERTY";
    string constant internal _SYMBOL = "ERTY";
    ...
    address public immutable lockInHalflifeAddress;

    address public immutable LQTYlpRewardsAddress;
    address public immutable LUSDlpRewardsAddress;
    ...
    // --- Functions ---
    /*
         _communityIssuanceAddress - CommunityIssuance addr, 
         _lqtyStakingAddress - LQTYStaking addr,
         _lockInHalflifeAddress - LockInHalflife addr
         _bountyAddress - _bountyAddress
         _LQTYlpRewardsAddress - Unipool.sol, Farming pool USDC-ERTY on Sushi
         _LUSDlpRewardsAddress - Unipool.sol, Farming pool USDC-mEUR on Sushi
    */
    constructor
    (
        ...
        public 
    {
        ...
        lockInHalflifeAddress = _lockInHalflifeAddress;
        LQTYlpRewardsAddress = _LQTYlpRewardsAddress;
        LUSDlpRewardsAddress = _LUSDlpRewardsAddress;
        ...
        // --- Initial LQTY allocations (1000M) ---     
        uint bountyEntitlement = _1_MILLION.mul(100); // Allocate 10%, 100 million for bounties/hackathons
        _mint(_bountyAddress, bountyEntitlement);

        uint depositorsAndFrontEndsEntitlement = _1_MILLION.mul(320); // Allocate 32%, 320 million to the algorithmic issuance schedule
        _mint(_communityIssuanceAddress, depositorsAndFrontEndsEntitlement);

        uint LQTYlpRewardsEntitlement = _1_MILLION.mul(150);  // Allocate 15%, 150 million for LP rewards
        _mint(_LQTYlpRewardsAddress, LQTYlpRewardsEntitlement);

        uint LUSDlpRewardsEntitlement = _1_MILLION.mul(150);  // Allocate 15%, 150 million for LP rewards
        _mint(_LUSDlpRewardsAddress, LUSDlpRewardsEntitlement);

        
        // Allocate the remainder to the LQTY LockInHalflife: (1000 - 100 - 320 - 150 - 150) million = 280 million
        uint teamLockInEntitlement = _1_MILLION.mul(1000)
            .sub(bountyEntitlement)
            .sub(depositorsAndFrontEndsEntitlement)
            .sub(LQTYlpRewardsEntitlement)
            .sub(LUSDlpRewardsEntitlement);

        _mint(_lockInHalflifeAddress, teamLockInEntitlement);
    }
    ...
    function sendToLQTYStaking(address _sender, uint256 _amount) external override {
        ...
        require(_sender != lockInHalflifeAddress, "LQTYToken: prevent the LockIn from staking LQTY");
