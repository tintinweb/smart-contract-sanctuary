...
contract Controller is IController, Initializable {
    ...
    function buy(
        ...
        vault.withdrawEthToAddress(priceOfTokens, address(_exchange));

        IExchange(_exchange).buy(_token, _amount, address(vault));
    ...
    function sell(
        ...
        uint256 sellPrice = _exchange.sell(_token, _amount, payable(address(vault)));
        balance = balance + int256(sellPrice);
        emit SellToken(_exchange, _token, _amount);
    }
