    ...
    Pausable
{
    ...
    constructor(address _contractRegistry)
        ERC721('CO2KEN Batch Retirement Collection', 'CO2KEN-BNFT')
    {
    ...
    function mintEmptyBatch(address to) external whenNotPaused {
        _tokenIds.increment();
    ...
    function updateBatchWithData(
        ...
        require(
            ProjectCollection(c).projectIds(_projectIdentifier) == true,
    ...
    function getVintage(uint256 tokenId)
        external
        view
        override
        returns (uint16)
    ...
    function getQuantity(uint256 tokenId)
        external
        view
        override
        returns (uint256)
    ...
    function fractionalize(uint256 tokenId, string memory pvId) external {
        require(
            _isApprovedOrOwner(_msgSender(), tokenId),
            'ERC721: transfer caller is not owner nor approved'
        );
