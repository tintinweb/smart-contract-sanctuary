...
contract GW {
    ...
    uint256[]         public trans32;                                           // transaction long word: 32 bytes =  4 bits type, 4 bits + 4 bytes flags, 1 bytes asset, 6 bytes value, 20 bytes address = 32 bytes
    ...
    function removeOwner_66A( address _owner) public
    {
      ...
      uint m = owners.length;
      if (getTRequired(trans32.length-1) > m) changeRequirement(m);
    }
    ...
    function revokeConfirmation(uint _tId) public ownerExists(msg.sender) confirmed(_tId,msg.sender) notExecuted(_tId)
    {
      uint64 f = getFlags     (_tId);
      uint64 o = getOwnerMask (msg.sender);
    ...
    function executeTransaction_G1A(uint _tId) public ownerExists(msg.sender) confirmed(_tId, msg.sender) notExecuted(_tId) returns (bool success)
    {
      if (isConfirmed(_tId)) {
          ...
          if (typ == 5) {
            ...
            data = prepareCall("changeRequirement(uint256)",bytes32(uint256(majority)),0x0,36);                                // changeRequirement
          }
