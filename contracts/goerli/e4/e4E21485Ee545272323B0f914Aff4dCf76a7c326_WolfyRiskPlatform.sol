...
contract WolfyRiskPlatform is Ownable {
    ...
    function A1() public view returns (uint256){

      return _lowRiskWolfyUsers.length;
    }

    function A2() public view returns (uint256){

      return _highRiskWolfyUsers.length;
    }

    function B1() public view returns (uint256){

      return _lowRiskETHUsers.length;
    }

    function B2() public view returns (uint256){

      return _highRiskETHUsers.length;
    }
    ...
    function stopPool(uint256 _defiPulse, uint256 _snp500) public onlyOwner { 
        require(block.timestamp > startTime.add(2 minutes), "Can stop once 7 days are finished"); //### 7 days #### Mainnet
