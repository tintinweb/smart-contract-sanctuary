...
interface IVolxDistributionManager {
  function configureAssets(DistributionTypes.AssetConfigInput[] calldata assetsConfigInput)
    external;
}

/**
 * @title VolxDistributionManager
 * @notice Accounting contract to manage multiple staking distributions
 * @author volmex.finance [[email protected]]
 **/
contract VolxDistributionManager is IVolxDistributionManager, Initializable {
  ...
  function _DistributionManager_init(address emissionManager, uint256 distributionDuration) internal initializer {
   ...
   * @param underlyingAsset The address used as key in the distribution, for example sVOLX or the aTokens addresses on Volx
...
/**
 * @notice implementation of the VOLX token contract
 * @author volmex.finance [[email protected]]
 */
abstract contract GovernancePowerDelegationERC20 is ERC20Upgradeable, IGovernancePowerDelegationToken {
   ...
   * the VOLX token and StakeToken already include a mapping for the snapshots, so we require contracts
...
abstract contract GovernancePowerWithSnapshot is GovernancePowerDelegationERC20 {
  ...
  /**
   * @dev The following storage layout points to the prior VolmexStaked.sol implementation:
   * _snapshots => _votingSnapshots
   * _snapshotsCounts =>  _votingSnapshotsCounts
   * _volxGovernance => _volxGovernance
   */
...
/**
 * @title VolmexStaked
 * @notice Contract to stake Volx token, tokenize the position and get rewards, inheriting from a distribution manager contract
 * @author volmex.finance [[email protected]]
 **/
contract VolmexStaked is
  IStakedVolx,
  GovernancePowerWithSnapshot,
  VolxDistributionManager
{
  using SafeMathUpgradeable for uint256;
  ...
  function initialize(
    ...
    _DistributionManager_init(emissionManager, distributionDuration);
    STAKED_TOKEN = stakedToken;
    REWARD_TOKEN = rewardToken;
    COOLDOWN_SECONDS = cooldownSeconds;
    UNSTAKE_WINDOW = unstakeWindow;
    REWARDS_VAULT = rewardsVault;
    ERC20Upgradeable._setupDecimals(18);
    ...
    // Update lastUpdateTimestamp of stkVolx to reward users since the end of the prior staking period
  ...
  function getNextCooldownTimestamp(
    ...
    if (minimalValidCooldownTimestamp > toCooldownTimestamp) {
      ...
      uint256 newFromCooldownTimestamp =
      ...
      if (newFromCooldownTimestamp < toCooldownTimestamp) {
        ...
        toCooldownTimestamp = (
          amountToReceive.mul(newFromCooldownTimestamp).add(toBalance.mul(toCooldownTimestamp))
        )
