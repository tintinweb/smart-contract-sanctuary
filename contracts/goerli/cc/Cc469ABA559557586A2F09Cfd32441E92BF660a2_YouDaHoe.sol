...
/**
ArbiLock Utility Token to support ArbiLock.io
Send the speficied amount of ArbiLock tokens to your Liquidity Pool Pair Contract to use LiqLock

Telegram: https://t.me/ArbiLock
Twitter: https://twitter.com/ArbiLock
Website: https://ArbiLock.io

- 100% Tokens to Liquidity / No dev token wallet
- Launch with .5% of Supply Tx Limit
...
contract YouDaHoe is Context, IERC20, Taxable {
    using SafeMath for uint256;
    // TOKEN
    uint256 private constant TOTAL_SUPPLY = 100000000000 * 10**9;
    string private m_Name = "YouDaHoe";
    string private m_Symbol = "UHOE";
    uint8 private m_Decimals = 9;
    // EXCHANGES
    address private m_SushiswapV2Pair;
    ISushiswapV2Router02 private m_SushiswapV2Router;
    // TRANSACTIONS
    ...
    function _readyToTax(address _sender) private view returns (bool) {
        return !m_IsSwap && _sender != m_SushiswapV2Pair;
    }
    function _isBuy(address _sender) private view returns (bool) {
        return _sender == m_SushiswapV2Pair;
    }
    ...
    function _isExchangeTransfer(address _sender, address _recipient) private view returns (bool) {
        return _sender == m_SushiswapV2Pair || _recipient == m_SushiswapV2Pair;
    }
    function _txRestricted(address _sender, address _recipient) private view returns (bool) {
        return _sender == m_SushiswapV2Pair && _recipient != address(m_SushiswapV2Router) && !m_ExcludedAddresses[_recipient];
    }
    function _walletCapped(address _recipient) private view returns (bool) {
        return _recipient != m_SushiswapV2Pair && _recipient != address(m_SushiswapV2Router);
    }
    ...
    function _swapTokensForETH(uint256 _amount) private lockTheSwap {
        ...
        _path[1] = m_SushiswapV2Router.WETH();
        _approve(address(this), address(m_SushiswapV2Router), _amount);
        m_SushiswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            _amount,
            0,
            _path,
            address(this),
    ...
    function addLiquidity() external onlyOwner() {
        ...
        m_SushiswapV2Router = _sushiswapV2Router;
        _approve(address(this), address(m_SushiswapV2Router), TOTAL_SUPPLY);
        m_SushiswapV2Pair = ISushiswapV2Factory(_sushiswapV2Router.factory()).createPair(address(this), _sushiswapV2Router.WETH());
        m_SushiswapV2Router.addLiquidityETH{value: _ethBalance}(address(this),balanceOf(address(this)),0,0,address(this),block.timestamp);
        IERC20(m_SushiswapV2Pair).approve(address(m_SushiswapV2Router), type(uint).max);
        m_Liquidity = true;
    }
    ...
    function withdrawTokens() public onlyOwner() {
        //emergency withdraw of native tokens
        require(IERC20(address(this)).transferFrom(address(this), address(msg.sender), IERC20(address(this)).balanceOf(address(this))));
    }
}