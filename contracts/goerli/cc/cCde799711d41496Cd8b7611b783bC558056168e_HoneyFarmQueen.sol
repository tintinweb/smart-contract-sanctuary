...
contract HoneyFarmQueen is Ownable, ReentrancyGuard {
    ...
    uint256 public constant MIN_BURN_AMOUNT_FOR_STAKING_BEE = 7;
    ...
    // we set it to 2^16 = 65,536 as bees max supply is 20700 so every id that is generated first, will be lower than this
    ...
    function claimBeesHoney(uint256[] calldata _beesIds) external nonReentrant {
        ...
        require(amount > 0, "Nothing to claim");
        // console.log(string(abi.encodePacked("Amount = ", Strings.toString(amount))));
        //can not mint more than maxSupply
        if (HoneyTokenI(honey).totalSupply() + amount > HoneyTokenI(honey).maxSupply()) {
            amount = (HoneyTokenI(honey).maxSupply() - HoneyTokenI(honey).totalSupply()) / 1e18;
        }
    ...
    function randBeeId() internal returns (uint16 _id) {
        uint16 entropy;
        uint16 MAX_SUPPLY = BeesDeluxeI(bees).getMaxSupply();
        uint16 TOTAL_SUPPLY = uint16(BeesDeluxeI(bees).totalSupply());
        require(TOTAL_SUPPLY < MAX_SUPPLY, "MAX_SUPPLY reached");
        while (true) {
            uint16 rand = uint16(
                uint256(
                    keccak256(
