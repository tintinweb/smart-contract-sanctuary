...
abstract contract SeasonBase {
    ...
    function __SeasonBase_init(
        ...
        require(anonToken == address(0), 'Season:NO');
        require(_anonToken != address(0), 'Season:IN');
        require(_startBlock <= _endBlock, 'Season:IB');
...
abstract contract SeasonFarm is SeasonNFT, SeasonVaultPools {
    ...
    function deposit(uint256 nftId, DepositArgs[] calldata depositArgsArr) public {
        require(_exists(nftId), 'Season:D');
        for (uint256 i; i < depositArgsArr.length; i++) {
            _deposit(nftId, depositArgsArr[i].vaultPoolAddr, depositArgsArr[i].amount);
        }
    }
    ...
    function _claim(
        ...
        require(beneficiary != address(0), 'Season:BN');
...
abstract contract SeasonNFT is SeasonBase, ERC721Upgradeable {
    ...
    mapping(uint256 => UserNFT) public userNfts;

    modifier onlyApprovedOrOwner(uint256 tokenId) {
        require(_isApprovedOrOwner(msg.sender, tokenId), 'Season:NO');
        _;
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (to != address(0)) {
            require(balanceOf(to) == 0, 'SeasonNFT: NFT already exists');
        }
...
abstract contract SeasonScores is SeasonFarm {
    ...
    event Submission(bytes32 ipfsHash);
    ...
    // // replaced by twitter
    // function submit(bytes32 ipfsHash) external {
    //     emit Submission(ipfsHash);
    // }
    ...
    function addMerkleRoot(bytes32 _ipfsHash, bytes32 _merkleRoot) public onlyGovernance {
        require(batchMerkleRoots[_ipfsHash] == bytes32(0), 'N');
    ...
    function prove(ProveArgs calldata args) public {
        ...
        require(root != bytes32(0), 'ZR');
        require(args.leaf.season == address(this), 'A');
        require(!proved[args.leaf.nftId][args.leaf.taskId], 'CM');
        require(
            MerkleProof.verify(
                args.proof,
                root,
...
abstract contract SeasonVaultPools is SeasonBase {
    ...
    function __SeasonVaultPools_init(ISeason.AddInitialVaultPoolArgs[] calldata _initialVaultPools) internal {
        require(_initialVaultPools.length > 0, 'Season:IL');
    ...
    function addOrUpdateVaultPoolWeight(
        ...
        require(vaultPoolAddrArray.length == numberOfVaultPools); // SLOAD
    ...
    function _updateVaultPool(address vaultPoolAddr) internal returns (VaultPool memory vaultPool) {
        ...
        require(vaultPool.isInitialized, 'Season: I');
