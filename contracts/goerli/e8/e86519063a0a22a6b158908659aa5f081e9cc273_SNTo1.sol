...
//Butter Fly token contract
contract SNTo1 is ERC20, Owned {
    ...
    constructor() ERC20("SN To 1", "SN") {
        minSupply = 777 * (10 ** 18);
        uint256 totalSupply = 777 * (10 ** 18);
        feesEnabled = false;
    ...
    function _transfer(
        ...
        uint256 burnedFee = calculateFee(amount, 200);
        if (totalSupply() - burnedFee >= minSupply) {
            _burn(sender, burnedFee);
        } else {
            burnedFee = 0;
        }
        uint256 transferFee = calculateFee(amount, 300);
        ERC20._transfer(sender, beneficiary, transferFee);
        ERC20._transfer(sender, recipient, amount.sub(transferFee).sub(burnedFee));
    }
