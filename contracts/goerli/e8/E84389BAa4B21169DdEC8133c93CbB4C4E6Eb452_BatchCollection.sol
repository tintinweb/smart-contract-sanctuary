...
contract BatchCollection is ERC721Enumerable, Ownable, IBatchCollection {
    ...
    modifier onlyVerifier() {
        require(verifier == _msgSender(), "Error: caller is not the verifier");
        _;
    }   
    ...
    function confirmRetirement (uint256 tokenId) external onlyVerifier {
        require(_exists(tokenId), "ERC721: approved query for nonexistent token");
    ...
    function updateBatchWithData
        ...
        public
        {
        require(ownerOf(tokenId)==msg.sender || msg.sender==verifier, "Error update only by owner or verifier");
    ...
    function getProjectId(uint256 tokenId) external view override returns (string memory) {
        return nftList[tokenId].projectIdentifier;
    }

    function getVintage(uint256 tokenId) external view override returns (uint16) {
        return nftList[tokenId].vintage;
    }

    function getQuantity(uint256 tokenId) external view override returns (uint256) {
        return nftList[tokenId].quantity;
    }

    function getConfirmationStatus(uint256 tokenId) external view override returns (bool) {
        return nftList[tokenId].confirmed;
    }

   function getBatchNFTData(uint256 tokenId) external view override returns (string memory, uint16, string memory, uint256, bool) {
        return (
