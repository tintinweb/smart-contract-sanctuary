...
contract BeOneDashboard {
    ...
    returns (Dashboard memory)
  {
    ...
    for (uint256 i = 0; i < beOneRegistry.getCTokensLength(); i++) {
      ...
      totalSupplyInUsd += cToken.totalSupply().mul(tokenPriceInUsd).div(uint(10) ** uint(cToken.decimals()));
      totalBorrowedInUsd += cToken.totalBorrows().mul(tokenPriceInUsd).div(uint(10) ** uint(cToken.decimals()));
      ...
      CTokensArray[i] = CToken({
        ...
        lendingApy: (cToken.supplyRatePerBlock().div(1e18).add(1) ** numberOfBlocksPerDay.mul(365)).sub(1).mul(100),
        lendingApr: cToken.supplyRatePerBlock().mul(numberOfBlocksPerDay).mul(365).div(1e18),
        borrowingApy: (cToken.borrowRatePerBlock().div(1e18).add(1) ** numberOfBlocksPerDay.mul(365)).sub(1).mul(100),
        borrowingApr: cToken.borrowRatePerBlock().mul(numberOfBlocksPerDay).mul(365).div(1e18),
