...
contract Genosphere is ERC721Enumerable, Ownable {
    ...
    string private _tokenBaseURI = "https://metadata.genosphere.io/";
    address private _signerAddress = 0x4EF748b2A2B2eE31798bE33C33c739B040f898De;
    ...
    function hashTransaction(address sender, string memory project, string memory state, string memory nonce, uint256 qty) private pure returns(bytes32) {
        bytes32 hash = keccak256(abi.encodePacked(
            "\x19Ethereum Signed Message:\n32",
            keccak256(abi.encodePacked(project, state, sender, nonce, qty)))
        );
    ...
    function buy(bytes32 hash, bytes memory signature, string memory nonce, uint256 tokenQuantity) external payable {
        ...
        require(hashTransaction(msg.sender, "Genosphere", "Public", nonce, tokenQuantity) == hash, "HASH_FAIL");
    ...
    function presaleBuy(bytes32 hash, bytes memory signature, string memory nonce, uint256 tokenQuantity) external payable {
        ...
        require(matchAddressSigner(hash, signature), "DIRECT_MINT_DISALLOWED");
        require(!_usedNonces[nonce], "HASH_USED");
        require(hashTransaction(msg.sender, "Genosphere", "Presale", nonce, tokenQuantity) == hash, "HASH_FAIL");
        require(totalSupply() < GENO_MAX, "OUT_OF_STOCK");
        require(privateAmountMinted + tokenQuantity <= GENO_PRIVATE, "EXCEED_PRIVATE");
        require(presalerPurchases[msg.sender] + tokenQuantity <= presalePurchaseLimit, "EXCEED_ALLOC");
        require(GENO_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
        
        for (uint256 i = 0; i < tokenQuantity; i++) {
            privateAmountMinted++;
            presalerPurchases[msg.sender]++;
            _safeMint(msg.sender, totalSupply() + 1);
        }
