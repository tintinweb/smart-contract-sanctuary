...
/**
    ...
    卖出有12%的手续费
    每笔交易的6%手续费自动销毁
    每笔交易的4%手续费合约自动添加到流动性,并且锁定,完美的资金池自我成长性
    每笔交易的2%手续费用于合约维护和gas
    总供应量的50%会燃烧到黑洞地址
...
contract SUBToken is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 39000000  * 10**18;


    string private _name = "SuperBounty";
    string private _symbol = "SUB";
    ...
    uint256 public waitWithdrawTaxFee;
    ...
    uint256 public waitAddLiquidity;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if(!_isExcluded[sender] && !_isExcluded[recipient] && isContract(recipient)){
    ...
    function totalLiquidities() public view returns (uint256) {
        return _tLiquidityTotal;
    }

    function exclude(address account, bool isExclude) public onlyOwner() {
        _isExcluded[account] = isExclude;
    }
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        ...
        waitAddLiquidity = waitAddLiquidity.add(tLiquidity);
    ...
    function _transfer(
        ...
        bool overMinTokenBalance = waitAddLiquidity >= numTokensSellToAddToLiquidity && contractTokenBalance >= waitAddLiquidity ;
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify &&
            from != uniswapV2Pair &&
            ...
            //add liquidity
            swapAndLiquify(waitAddLiquidity);
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 addLiquidityTokenAmount = initialTokenBalance.sub(balanceOf(address(this)));
        
        waitAddLiquidity = waitAddLiquidity.sub(addLiquidityTokenAmount);
