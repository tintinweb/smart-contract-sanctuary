...
/*
     ...
     3. StartPool : Frontend API provides DeFi and SNP value initially and start the pools.
     4. StopPool : Frotend API provides both value once pool is completed.
    __________________________________________________________________________________________
     ...
     Stakers must have 1,000 Wolfy tokens in their wallet to participate in any of the risk pools.
        Mainnet : 1,000
...
contract WolfyStreetBetsETH is Ownable,ReentrancyGuard {
    using SafeMath for uint256;
    using SafeMath for int256;
    using SafeERC20 for IERC20;

    uint256 public startTime = 0;
    // user must be a wolfy holder
    ...
    uint256 minWolfy = 10*10**3*10**8; // 1,000 $WOLFY ERC-20 Tokens required to participate in WolfyStreetBets pools
    bool checker ;
    ...
    // v 2.0 ----| deployer can determine low & high risk multipliers prior to each pool starting, allowing for suitable risk/reward in any market cycle |----
    ...
    function stakeEth(bool _isLowRisk) public payable {
     require(msg.value > 0 , "You can't stake 0 ETH! Choose an amount! :-)");
     require(poolStarted, "Cannot stake until pool has started!");
     ...
     require(WOLFY.balanceOf(msg.sender) >= minWolfy, "You must possess at least 1,000 WOLFY Tokens to Participate in WolfyStreetBets!");
     ...
     if(_isLowRisk){
         // total staking limit is 6x available liquidity
      require(depositLowRiskETH.add(msg.value) <= (totalLiquidityLowRsik + liquidityDetailsOwner._etherLowRisk).mul(6), "Low risk pool: ETH staking limit reached!");   
      require(msg.value <= (totalLiquidityLowRsik + liquidityDetailsOwner._etherLowRisk).mul(6), "Low risk pool: ETH staking limit reached!");
      ...
      require(depositHighRiskETH.add(msg.value) <= (totalLiquidityHighRsik + liquidityDetailsOwner._etherHighRisk).mul(3), "High risk pool: ETH staking limit reached!");
      require(msg.value <= (totalLiquidityHighRsik + liquidityDetailsOwner._etherHighRisk).mul(3), "High risk pool: ETH staking limit reached!");
    ...
    function withdrawLiquidityETH(uint256 _ether, bool _isLowRisk) external nonReentrant
    {
         ...
         require(!poolStarted, "Cannot withdraw ETH while pool is active!");
         if (_isLowRisk)
         {
    ...
    function getLosses(bool _isLowRisk) public view returns (uint256){
     ...
     //______| Total ETH liquidity low Risk |_______ // As per new change this function will return stake ether balance by user for bet
    ...
    //______| Net earnings |____|Profits must higher than loss|___
    ...
    /* 
     * For liquidity sustainability & expanded rewards capability:
     * Any user can become a liquidity provider (LP) / supply [ETH] liquidity to a pool 
     * 2% Staking fee ~ paid to LP proportional to # supplied
     */
    ...
    function  distributeLiquidityRewards(bool _isLowRisk) internal  // for distributing rewards to LP proportional to # supplied
    {
