...
import {ITRC721Receiver} from "../common/tokens/IERC721Receiver.sol";
import {IMintableERC721} from "../RootToken/IMintableERC721.sol";

contract MintableERC721Predicate is ITokenPredicate, AccessControlMixin, Initializable, IERC721Receiver, ITRC721Receiver {

    bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE");
    bytes32 public constant TOKEN_TYPE = keccak256("MintableERC721");
    ...
    /**
     * @notice accepts safe TRC721 transfer
     */
     ...
     * @param value Valid ERC721 burn log from child chain
     */
    function exitTokens(
        address sender,
        address localToken,
        ...
        only(MANAGER_ROLE)
    {
        ...
        IMintableERC721 token = IMintableERC721(localToken);
        if (value.length == 32) {
            uint256 tokenId = abi.decode(value, (uint256));
            emit ExitedERC721(sender, localToken, tokenId);
            if (token.exists(tokenId)) {
                IRootERC721(localToken).safeTransferFrom(
                    address(this), withdrawer, tokenId // tokenId field
                );
            } else {
            ...
            (uint256[] memory tokenIds) = abi.decode(value, (uint256[])); // data is tokenId list
            emit ExitedERC721Batch(sender, localToken, tokenIds);
            uint256 length = tokenIds.length;
            for (uint256 i; i < length; i++) {
                if (token.exists(tokenIds[i])) {
                    IRootERC721(localToken).safeTransferFrom(address(this), withdrawer, tokenIds[i]);
                } else {
                    token.mint(withdrawer, tokenIds[i]);
                }
            }
        }
