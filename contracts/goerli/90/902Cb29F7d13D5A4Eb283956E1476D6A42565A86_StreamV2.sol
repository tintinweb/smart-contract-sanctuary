...
contract StreamV2 is Ownable {
        ...
        for (uint256 index = 0; index < recipients.length; index++) {
            ...
            if (fundDuration[index] > 0) {
                ...
                if(Address.isContract(recipients[index])) {
                    ...
                    (bool success, bytes memory err) = recipients[index].call(abi.encodeWithSignature("fund(uint256,unit256)", balance.mul(shareBPS[index]).div(10_000), fundDuration[index]));
    ...
    function _functionCall(address target, bytes memory data) external onlyOwner {
        ...
        // perform call
        bytes memory returnData = Address.functionCall(target, data, "call failed");

        if (returnData.length > 0) { 
            require(abi.decode(returnData, (bool)), "call did not succeed");
        }
