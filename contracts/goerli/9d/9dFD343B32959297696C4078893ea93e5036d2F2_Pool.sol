...
/// @dev The Pool contract exchanges Dai for gDai at a price defined by a specific formula.
contract Pool is IPool, Delegable(), ERC20Permit {

    event Trade(uint256 maturity, address indexed from, address indexed to, int256 daiTokens, int256 gDaiTokens);
    event Liquidity(uint256 maturity, address indexed from, address indexed to, int256 daiTokens, int256 gDaiTokens, int256 poolTokens);
    ...
    int128 constant public g2 = int128(uint256((1000 << 64)) / 950); // To be used when selling gDai to the pool. All constants are `ufixed`, to divide them they must be converted to uint256
    uint256 immutable public override maturity;

    IERC20 public override dai;
    IGDai public override gDai;

    constructor(address dai_, address gDai_, string memory name_, string memory symbol_)
        public
    ...
    function sub(uint128 a, uint128 b) internal pure returns (uint128) {
        require(b <= a, "Pool: gDai reserves too low");
        uint128 c = a - b;
        ...
        returns (uint256)
    {
        ...
        // no gDai transferred, because initial gDai deposit is entirely virtual
    ...
    /// @dev Mint liquidity tokens in exchange for adding dai and gDai
    /// The liquidity provider needs to have called `dai.approve` and `gDai.approve`.
    /// @param from Wallet providing the dai and gDai. Must have approved the operator with `pool.addDelegate(operator)`.
    /// @param to Wallet receiving the minted liquidity tokens.
    /// @param daiOffered Amount of `dai` being invested, an appropriate amount of `gDai` to be invested alongside will be calculated and taken by this function from the caller.
        ...
        returns (uint256)
    {
        ...
        uint256 gDaiReserves = gDai.balanceOf(address(this));
        uint256 tokensMinted = supply.mul(daiOffered).div(daiReserves);
        uint256 gDaiRequired = gDaiReserves.mul(tokensMinted).div(supply);

        require(daiReserves.add(daiOffered) <= type(uint128).max); // daiReserves can't go over type(uint128).max
        require(supply.add(gDaiReserves.add(gDaiRequired)) <= type(uint128).max); // gDaiReserves can't go over type(uint128).max

        require(dai.transferFrom(from, address(this), daiOffered));
        require(gDai.transferFrom(from, address(this), gDaiRequired));
        _mint(to, tokensMinted);
        emit Liquidity(maturity, from, to, -toInt256(daiOffered), -toInt256(gDaiRequired), toInt256(tokensMinted));

        return tokensMinted;
    }

    /// @dev Burn liquidity tokens in exchange for dai and gDai.
    ...
    /// @param to Wallet receiving the dai and gDai.
    /// @param tokensBurned Amount of liquidity tokens being burned.
    /// @return The amount of reserve tokens returned (daiTokens, gDaiTokens).
        ...
        returns (uint256, uint256)
    {
        ...
        // use the actual reserves rather than the virtual reserves
        uint256 daiReturned;
        uint256 gDaiReturned;
        { // avoiding stack too deep
            uint256 gDaiReserves = gDai.balanceOf(address(this));
            daiReturned = tokensBurned.mul(daiReserves).div(supply);
            gDaiReturned = tokensBurned.mul(gDaiReserves).div(supply);
        }

        _burn(from, tokensBurned);
        dai.transfer(to, daiReturned);
        gDai.transfer(to, gDaiReturned);
        emit Liquidity(maturity, from, to, toInt256(daiReturned), toInt256(gDaiReturned), -toInt256(tokensBurned));

        return (daiReturned, gDaiReturned);
    }
    ...
    /// @return Amount of gDai that will be deposited on `to` wallet
    function sellDai(address from, address to, uint128 daiIn)
        external override
        onlyHolderOrDelegate(from, "Pool: Only Holder Or Delegate")
        returns(uint128)
    {
        uint128 gDaiOut = sellDaiPreview(daiIn);

        dai.transferFrom(from, address(this), daiIn);
        gDai.transfer(to, gDaiOut);
        emit Trade(maturity, from, to, -toInt256(daiIn), toInt256(gDaiOut));

        return gDaiOut;
    }

    /// @dev Returns how much gDai would be obtained by selling `daiIn` dai
    /// @param daiIn Amount of dai hypothetically sold.
    /// @return Amount of gDai hypothetically bought.
        ...
        returns(uint128)
    {
        ...
        uint128 gDaiReserves = getGDaiReserves();

        uint128 gDaiOut = YieldMath.gDaiOutForDaiIn(
            daiReserves,
            gDaiReserves,
            daiIn,
        ...
        require(
            sub(gDaiReserves, gDaiOut) >= add(daiReserves, daiIn),
            "Pool: gDai reserves too low"
        );
    ...
    /// The trader needs to have called `gDai.approve`
    /// @param from Wallet providing the gDai being sold. Must have approved the operator with `pool.addDelegate(operator)`.
    /// @param to Wallet receiving the dai being bought
    /// @param daiOut Amount of dai being bought that will be deposited in `to` wallet
    /// @return Amount of gDai that will be taken from `from` wallet
    function buyDai(address from, address to, uint128 daiOut)
        external override
        onlyHolderOrDelegate(from, "Pool: Only Holder Or Delegate")
        returns(uint128)
    {
        uint128 gDaiIn = buyDaiPreview(daiOut);

        gDai.transferFrom(from, address(this), gDaiIn);
        dai.transfer(to, daiOut);
        emit Trade(maturity, from, to, toInt256(daiOut), -toInt256(gDaiIn));

        return gDaiIn;
    }

    /// @dev Returns how much gDai would be required to buy `daiOut` dai.
    /// @param daiOut Amount of dai hypothetically desired.
    /// @return Amount of gDai hypothetically required.
    function buyDaiPreview(uint128 daiOut)
        public view override
        beforeMaturity
        returns(uint128)
    {
        return YieldMath.gDaiInForDaiOut(
            getDaiReserves(),
            getGDaiReserves(),
            daiOut,
            toUint128(maturity - now), // This can't be called after maturity
    ...
    /// @param from Wallet providing the gDai being sold. Must have approved the operator with `pool.addDelegate(operator)`.
    /// @param to Wallet receiving the dai being bought
    /// @param gDaiIn Amount of gDai being sold that will be taken from the user's wallet
    /// @return Amount of dai that will be deposited on `to` wallet
    function sellGDai(address from, address to, uint128 gDaiIn)
        external override
        onlyHolderOrDelegate(from, "Pool: Only Holder Or Delegate")
        returns(uint128)
    {
        uint128 daiOut = sellGDaiPreview(gDaiIn);
    ...
    /// @dev Returns how much dai would be obtained by selling `gDaiIn` gDai.
    /// @param gDaiIn Amount of gDai hypothetically sold.
    /// @return Amount of Dai hypothetically bought.
    function sellGDaiPreview(uint128 gDaiIn)
        public view override
        beforeMaturity
        returns(uint128)
    {
        return YieldMath.daiOutForGDaiIn(
            getDaiReserves(),
            getGDaiReserves(),
            gDaiIn,
            toUint128(maturity - now), // This can't be called after maturity
    ...
    /// @dev Buy gDai for dai
    /// The trader needs to have called `dai.approve`
    /// @param from Wallet providing the dai being sold. Must have approved the operator with `pool.addDelegate(operator)`.
    /// @param to Wallet receiving the gDai being bought
    /// @param gDaiOut Amount of gDai being bought that will be deposited in `to` wallet
    /// @return Amount of dai that will be taken from `from` wallet
    function buyGDai(address from, address to, uint128 gDaiOut)
        external override
        onlyHolderOrDelegate(from, "Pool: Only Holder Or Delegate")
        returns(uint128)
    {
        uint128 daiIn = buyGDaiPreview(gDaiOut);
    ...
    /// @dev Returns how much dai would be required to buy `gDaiOut` gDai.
    /// @param gDaiOut Amount of gDai hypothetically desired.
    /// @return Amount of Dai hypothetically required.
    function buyGDaiPreview(uint128 gDaiOut)
        public view override
        beforeMaturity
        returns(uint128)
    {
        ...
        uint128 daiIn = YieldMath.daiInForGDaiOut(
            daiReserves,
            gDaiReserves,
            gDaiOut,
            toUint128(maturity - now), // This can't be called after maturity
    ...
    /// @dev Returns the "virtual" gDai reserves
    function getGDaiReserves()
        public view override
        returns(uint128)
    {
        return toUint128(gDai.balanceOf(address(this)).add(totalSupply()));
    }
...
library YieldMath {
  /**
   * Calculate the amount of gDai a user would get for given amount of Dai.
   *
   * @param daiReserves Dai reserves amount
   * @param gDaiReserves gDai reserves amount
   * @param daiAmount Dai amount to be traded
   * @param timeTillMaturity time till maturity in seconds
   * @param k time till maturity coefficient, multiplied by 2^64
   * @param g fee coefficient, multiplied by 2^64
   * @return the amount of gDai a user would get for given amount of Dai
   */
  function gDaiOutForDaiIn (
    uint128 daiReserves, uint128 gDaiReserves, uint128 daiAmount,
  ...
  internal pure returns (uint128) {
    ...
    uint256 sum =
      ...
      pow (gDaiReserves, uint128 (a), 0x10000000000000000) -
      pow (uint128(xdx), uint128 (a), 0x10000000000000000);
    require (sum < 0x100000000000000000000000000000000, "YieldMath: Insufficient gDai reserves");

    uint256 result = gDaiReserves - pow (uint128 (sum), 0x10000000000000000, uint128 (a));
  ...
  /**
   * Calculate the amount of Dai a user would get for certain amount of gDai.
   *
   * @param daiReserves Dai reserves amount
   * @param gDaiReserves gDai reserves amount
   * @param gDaiAmount gDai amount to be traded
   * @param timeTillMaturity time till maturity in seconds
   * @param k time till maturity coefficient, multiplied by 2^64
   * @param g fee coefficient, multiplied by 2^64
   * @return the amount of Dai a user would get for given amount of gDai
   */
  function daiOutForGDaiIn (
    uint128 daiReserves, uint128 gDaiReserves, uint128 gDaiAmount,
    uint128 timeTillMaturity, int128 k, int128 g)
  internal pure returns (uint128) {
    // t = k * timeTillMaturity
    int128 t = Math64x64.mul (k, Math64x64.fromUInt (timeTillMaturity));
    ...
    // ydy = gDaiReserves + gDaiAmount;
    uint256 ydy = uint256 (gDaiReserves) + uint256 (gDaiAmount);
    require (ydy < 0x100000000000000000000000000000000, "YieldMath: Too much gDai in");
  ...
  /**
   * Calculate the amount of gDai a user could sell for given amount of Dai.
   *
   * @param daiReserves Dai reserves amount
   * @param gDaiReserves gDai reserves amount
   * @param daiAmount Dai amount to be traded
   ...
   * @param k time till maturity coefficient, multiplied by 2^64
   * @param g fee coefficient, multiplied by 2^64
   * @return the amount of gDai a user could sell for given amount of Dai
   */
  function gDaiInForDaiOut (
    uint128 daiReserves, uint128 gDaiReserves, uint128 daiAmount,
    uint128 timeTillMaturity, int128 k, int128 g)
  internal pure returns (uint128) {
    // t = k * timeTillMaturity
    ...
    uint256 sum =
      ...
      pow (gDaiReserves, uint128 (a), 0x10000000000000000) -
      pow (uint128 (xdx), uint128 (a), 0x10000000000000000);
    require (sum < 0x100000000000000000000000000000000, "YieldMath: Resulting gDai reserves too high");

    uint256 result = pow (uint128 (sum), 0x10000000000000000, uint128 (a)) - gDaiReserves;
    require (result < 0x100000000000000000000000000000000, "YieldMath: Rounding induced error");
  ...
  internal pure returns (uint128) {
    ...
    // ydy = gDaiReserves - gDaiAmount;
    uint256 ydy = uint256 (gDaiReserves) - uint256 (gDaiAmount);
    require (ydy < 0x100000000000000000000000000000000, "YieldMath: Too much gDai out");
