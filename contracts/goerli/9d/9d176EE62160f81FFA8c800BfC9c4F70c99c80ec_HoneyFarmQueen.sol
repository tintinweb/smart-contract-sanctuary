...
contract HoneyFarmQueen is Ownable, ReentrancyGuard {
    ...
    //this is 0.13
    uint16 public HONEY_BEE_REWARDS_PER_ROUND = 13;
    ...
    function claimBearsHoney(uint16[] calldata _bearsIds) external nonReentrant {
        uint256 amount;
        for (uint16 i; i < _bearsIds.length; i++) {
    ...
    function claimBeesHoney(uint256[] calldata _beesIds) external nonReentrant {
        uint256 amount;
        for (uint256 i; i < _beesIds.length; i++) {
            ...
            if (roundsToReward > bee.claimCounter) {
            ...
            amount += HONEY_BEE_REWARDS_PER_ROUND * roundsToReward;
    ...
    function mintBee(uint256 _hiveId) external nonReentrant {
        ...
        idsAndBees[beeId] = Bee(beeId, 1, 10, 0, STARTING_POINT);
    ...
    function setInitialStartingPoint(uint256 startingPoint) external onlyOwner {
