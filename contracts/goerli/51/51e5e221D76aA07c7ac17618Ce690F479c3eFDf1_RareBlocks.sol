...
contract RareBlocks is ERC721, ERC721Enumerable, Pausable, Ownable {
    ...
    uint256 RESERVED_FOUNDING_MEMBERS = 20;
    ...
    function preMint(address _to) whenNotPaused public payable  {
        require(preMintIsActive, "Premint is not active");
        require(msg.value == _price, "Wrong amount sent");
        require(_tokenIdCounter.current() < ACCESS_PASS_SUPPLY, "Can't mint over supply limit");
        require(_allowList[_to] > 0, "Exceeded max available to purchase");
    ...
    function addressIsWhitelisted(address _address) external view returns(uint8) {
