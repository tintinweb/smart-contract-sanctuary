...
contract VeToken is SafeOwnable, TransferHelper {
  ...
  address private lockedToken;     // WILD
  uint    public lastAccrueTime;   // Last time when RewardTokens distribution occurs.
  uint    public rewardPerToken;   // Reward per veToken. Increases over time.
  uint    public rewardRateStored;
  uint    public lastIncomeTime;
  ...
  function lock(uint _amount, uint _newLockedUntil) external {
  ...
  // over the next DISTRIBUTION_PERIOD time
  function addIncome(uint _addAmount) external onlyOwner {
    _accrue();
    _safeTransferFrom(lockedToken, msg.sender, _addAmount);

    uint timeElapsed    = Math.min(DISTRIBUTION_PERIOD, block.timestamp - lastIncomeTime);
    uint unvestedIncome = rewardRateStored * (DISTRIBUTION_PERIOD - timeElapsed);

    rewardRateStored = (unvestedIncome + _addAmount) / DISTRIBUTION_PERIOD;
    lastIncomeTime   = block.timestamp;
  ...
  // If no new income is added for more than DISTRIBUTION_PERIOD time,
  // then do not distribute any more rewards
  function rewardRate() public view returns(uint) {
    uint timeElapsed = block.timestamp - lastIncomeTime;
  ...
  function _updateLock(address _account, uint _newLockedUntil) internal {
    ...
    balanceOf[_account] = (lockedBalanceOf[_account] * lockSeconds) / MAX_LOCK_PERIOD;
    lockedUntil[msg.sender] = _newLockedUntil;
    _setBalance(_account, balanceOf[_account]);
  }
  ...
  function _pendingRewardPerToken() internal view returns(uint) {
    ...
    uint timeElapsed = block.timestamp - lastAccrueTime;
    return timeElapsed * rewardRate() * 1e18 / totalSupply;
  }
