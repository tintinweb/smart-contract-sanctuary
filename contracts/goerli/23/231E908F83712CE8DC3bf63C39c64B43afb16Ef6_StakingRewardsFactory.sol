...
contract StakingRewardsFactory is Ownable {
    ...
    constructor(address _rewardsToken) Ownable() {
        ...
        stakingRewardsGenesis = 0;
    }

    function setRewardsGenesis(uint256 _stakingRewardsGenesis)
        public
        onlyOwner
    {
        require(
    ...
    function deploy(
        ...
        require(
            ...
            "StakingRewardsFactory::deploy: rewardAmount must be less or equal rewardTotalAmount"
        );
    ...
    function notifyRewardAmounts() public onlyOwner {
        ...
        for (uint256 i = 0; i < stakingTokens.length; i++) {
            StakingRewardsInfo storage info =
                stakingRewardsInfoByStakingToken[stakingTokens[i]];
            require(
                info.stakingRewards != address(0),
        ...
        onlyOwner
    {
        ...
        if (info.rewardProgressAmount < info.rewardTotalAmount) {
            uint256 remaining =
                info.rewardTotalAmount.sub(info.rewardProgressAmount);
            require(
                remaining >= rewardAmount,
                "StakingRewardsFactory::notifyRewardAmount: incorrect rewardAmount"
            );
