...
contract PresaleToken is Pausable {
     ...
     */
    event Deposited(address indexed purchaser, uint256 value);
     ...
     */
    event Released(address indexed purchaser, uint256 token);
    ...
    constructor(
        address _token,
        address _wallet,
        uint256 _minInvestment, // 0.25BNB
        uint256 _maxInvestment, // 0.5BNB
        uint256 _rate // 8000/denom
    ) {
    ...
    function include(address[] memory _users) external onlyOwner {
    ...
    modifier validPurchase() {
        require(msg.value >= minInvestment, "Investment too low");
        require(
            deposited[msg.sender] + msg.value <= maxInvestment,
            "Investment too big"
        );
        _;
    }
        ...
        onlyWhitelist
    {
        ...
        uint256 releaseToken = calculateToken(msg.value);

        released[msg.sender] = releaseToken;
        totalRelease += releaseToken;
        token.transfer(msg.sender, releaseToken);
