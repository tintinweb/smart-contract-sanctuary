...
interface IPriceFeed {
    ...
    /**
     * @return last update time from the price source
     */
    function lastUpdate() external view returns (uint32);

    /**
     * @notice should raise "No data present"
     * if it has no data to report, instead of returning unset values
     * which could be misinterpreted as actual reported values.
     */
    function price() external view returns (uint256, uint32);

    /**
     * @notice should raise "No data present"
     * @param quoteCurrency addr for the quote currency token
     * if it has no data to report, instead of returning unset values
     * which could be misinterpreted as actual reported values.
     * @return midPrice - price implied by reserve0 / reserve0
     * @return timestamp - last update time
     */
    function price(address quoteCurrency) external view returns (uint256 midPrice, uint32 timestamp);

    /**
     * @return boolen to indicate if the feed has a valid price
     */
