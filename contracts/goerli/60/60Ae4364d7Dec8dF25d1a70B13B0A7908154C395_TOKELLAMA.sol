...
//Thanks to Hadrien Croubois for this fine data structure

library CardDeck {
  struct Manifest {
    uint256 length;
    mapping(uint256 => uint256) pairing;
  }
  ...
  function draw(Manifest storage manifest) internal returns (uint256) {
    uint256 i = seed() % manifest.length;
    uint256 l = --manifest.length;
    uint256 x = manifest.pairing[i];
    uint256 y = manifest.pairing[l];
    if (x == 0) { x = i + 1;}
    if (y == 0) { y = l + 1;}
    else        { delete manifest.pairing[l]; }

    if (i != l) {manifest.pairing[i] = y;     }
    return x - 1;
  }
  ...
  function seed() internal view returns (uint256) {
    return uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, msg.sender,gasleft(),blockhash(block.number-100)))) ;
  }
}
...
// Part: SproutGenerator

library SproutGenerator {
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        // https://gateway.pinata.cloud/ipfs/ + baseURI + tokenId.json
...
contract TOKELLAMA is Ownable, ERC721Enumerable, ERC721Burnable, ERC721Pausable {
    ...
    uint256 public constant bulkBuyLimit = 10;


    uint256 public SproutsMintedForPromotion;
    uint256 public unitSproutPrice;
    ...
    function initialize() public onlyOwner {
      unitSproutPrice = 0.01 ether;
      _manifest.setup(maxSupply);
    ...
    function setUnitSproutPrice(uint256 newUnitSproutPrice) public onlyOwner {
        unitSproutPrice = newUnitSproutPrice;
        emit SproudPriceChanged(newUnitSproutPrice);
    }

    function changeServiceIpfs(string memory newURL) public onlyOwner {
      _serviceIPFS = newURL;
    }


    function mint(uint256 amount) public payable onlyRemaining(amount) {
      require(amount <= bulkBuyLimit, "Cannot bulk buy more than the preset limit");
      require(msg.value == unitSproutPrice*amount, 'You need to send 0.1 ether per SoundSprouts desired');
    ...
    function mintForPromotion(uint256 amount, address whoToSend) public onlyOwner onlyRemaining(amount) returns(bool){
      require(SproutsMintedForPromotion + amount <= 100, 'Cant mint more SoundSprouts for promotion');
