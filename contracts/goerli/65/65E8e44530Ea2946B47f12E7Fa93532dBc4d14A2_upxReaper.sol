...
contract upxReaper is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accUPXPerShare; // Accumulated wUPX per share, times 1e6. See below.
    }

    // The wUPX TOKEN!
    WrappeduPlexa public wupx;
    // Block number when bonus wUPX period ends.
    uint256 public endBlock;
    // wUPX tokens created per block.
    uint256 public lastBlockReward;
    // Bonus muliplier for early wupx makers.
    ...
    event NewStakingPeriod(uint256 indexed oldEndHeight, uint256 indexed startHeight, uint256 indexed endHeight);
    ...
    function newPeriod(uint256 startHeight, uint256 endHeight, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
        uint256 oldStartHeight = startBlock;
        uint256 oldEndHeight = endBlock;
        startBlock = startHeight;
        endBlock = endHeight;
        emit NewStakingPeriod(oldEndHeight, startHeight, endHeight);
    }
    ...
    function pendingUPX(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            if(multiplier>0){
              // When sending new wUPX to upxReaper, we will need to massUpdatePools()
              uint256 wupxReward = multiplier.mul(lastBlockReward).mul(pool.allocPoint).div(totalAllocPoint);
              uint256 balanceCheck = balanceCheck(wupxReward);
              if(balanceCheck==1){
                accUPXPerShare = accUPXPerShare.add(wupxReward.mul(1e6).div(lpSupply));
              }
            }
        }
        return user.amount.mul(accUPXPerShare).div(1e6).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
      (uint256 reapBal) = reaperUPXBalance();
        PoolInfo storage pool = poolInfo[_pid];
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        if(multiplier>0){
          uint256 wupxReward = multiplier.mul(reapBal).mul(pool.allocPoint).div(totalAllocPoint);
          uint256 balanceCheck = balanceCheck(wupxReward);
          if(balanceCheck==1){
            //wupx.transfer(address(this), wupxReward);
            pool.accUPXPerShare = pool.accUPXPerShare.add(wupxReward.mul(1e6).div(lpSupply));
            pool.lastRewardBlock = block.number;
          }
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accUPXPerShare).div(1e6).sub(user.rewardDebt);
            safeUPXTransfer(msg.sender, pending);
        }
        pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accUPXPerShare).div(1e6);
    ...
    // Block rewards should be UPX balance / number of blocks left until end of this staking period.
    function reaperUPXBalance() private returns (uint256) {
      (uint256 allocatedUPX) = getAllocatedUPX();
      //Total balance available: balance - allocatedUPX
      // We should also add a failsafe to NOT be in-debted to a user by checking if there's enough balance to allocate to them.
        uint256 wupxBal = wupx.balanceOf(address(this)).sub(allocatedUPX);
        uint256 blocksRemain = endBlock - block.number;
        if(blocksRemain<1){
          uint256 reapBal = 0;
          return (reapBal);
        } else {
          uint256 reapBal = wupxBal.div(blocksRemain);
          lastBlockReward = reapBal;
          return (reapBal);
        }
    }

    // Ensure reward is not greater than (balance-allocatedUPX)
    function balanceCheck(uint256 _reward) public view returns (uint256) {
      (uint256 allocatedUPX) = getAllocatedUPX();
        uint256 wupxBal = wupx.balanceOf(address(this)).sub(allocatedUPX);
        if(_reward>wupxBal){
          return 0;
    ...
    // Update reward vairables for all pools. Be careful of gas spending!
    function getAllocatedUPX() public view returns (uint256) {
        uint256 length = poolInfo.length;
        uint256 allocatedUPX;
        for (uint256 pid = 0; pid < length; ++pid) {
            allocatedUPX.add(poolInfo[pid].accUPXPerShare);
        }
        return (allocatedUPX);
    }
