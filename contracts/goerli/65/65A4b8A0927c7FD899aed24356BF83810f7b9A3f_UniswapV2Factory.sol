...
contract PairWithL2Overlay is UniswapV2Pair, StarkWareAssetData {
  ...
  uint16 private starkWareState; // 0 - off, 1 - mint, 2 - burn
  ...
  function setupStarkware(uint _assetId, uint _tokenAAssetId, uint _tokenBAssetId) external operatorOnly {
    ...
    tokenBQuantum = starkEx.getQuantum(_tokenAAssetId);
  }
  ...
  */
  ...
  function initialize(address _token0, address _token1, address _weth) public {
  ...
  function swapStarkWare( 
    uint[] memory path,
    uint[] memory swapQuantisedAmounts,
    address exchangeAddress) internal returns(uint, uint) {

    // pathFrom is swap from Pair's perspective
    uint pathFrom = path[1];
    uint pathTo = path[0];
    uint amountFrom = swapQuantisedAmounts[1];
    uint amountTo = swapQuantisedAmounts[0];
    require(amountFrom > 0 && amountTo > 0, 'DVF_SWAP_AMOUNTS_INVALID');
    // Validate the swap amounts
    (uint112 _reserve0, uint112 _reserve1,) = getReserves();
    uint balance0;
    uint balance1;
    // Cleanup and avoid if/else
    if (pathFrom == tokenAAssetId) {
      balance0 = _reserve0 - fromQuantized(tokenAQuantum, amountFrom);
      balance1 =_reserve1 + fromQuantized(tokenBQuantum, amountTo);
    } else {
      balance0 = _reserve0 + fromQuantized(tokenBQuantum, amountTo);
      balance1 =_reserve1 - fromQuantized(tokenAQuantum, amountFrom);
    }
    IStarkEx starkEx = getStarkEx();

    validateK(balance0, balance1, _reserve0, _reserve1);
    (address fromToken, uint fromQuantum) = starkWareInfo(pathFrom);

    // TODO cleanup
    TransferHelper.safeApprove(fromToken, IUniswapV2Factory(factory).starkExContract(), fromQuantized(fromQuantum, amountFrom));
    depositStarkWare(starkEx, fromToken, fromQuantum, pathFrom, 0, amountFrom);
    getStarkExRegistry(starkEx).registerLimitOrder(exchangeAddress, pathFrom, pathTo,
      pathFrom, amountFrom, amountTo, 0, 0, 0, 0, nonce++, unsignedInt22);
  ...
  function swapAndMintStarkWare(
    ...
    (uint balance0, uint balance1) = swapStarkWare(path, swapQuantisedAmounts, exchangeAddress);
  ...
  function settleStarkWare() external operatorOnly returns(bool) {
    ...
    require(_starkWareState == 1 || _starkWareState == 2, 'DVF: NOTHING_TO_SETTLE');
