...
contract Pool is Ownable, ReentrancyGuard, Pausable, SotaWhitelist {
     ...
     */
    function getErc20TokenConversionRate(address _token)
        public
        view
        returns (uint256)
    {
     ...
     * @param _token address of token to query token exchange rate
     */
    function setErc20TokenConversionRate(address _token, uint256 _rate)
        public
        onlyOwner
    {
        require(
    ...
    /**
     * @notice Calculate the tokens user can receive. Receive User's trade ETH and transfer ETHs
     * @dev tokens = _weiAmount * token Conversion Rate Of _weiAmount
     * @param _beneficiary Address performing the token purchase
     * @param _candidate Address of buyer
     * @param _maxAmount Max amount of token buyer can buy
     * @param _deadline Timestamp of last moment buyer can buy
     * @param _signature Hash string of buyer signature
     */
    function buyTokenByEtherWithPermission(
        ...
        require(
            _verifyWhitelist(_candidate, _maxAmount, _deadline, _signature)
        );
    ...
    function buyTokenByTokenWithPermission(
        ...
        uint256 _deadline,
        bytes memory _signature
    ) public whenNotPaused nonReentrant tokenRateSetted(_token) {
     ...
     * @return Number of tokens that can be purchased with the specified _weiAmount
     */
    function _getTokenToTokenAmount(address _token, uint256 _amount)
        internal
        view
        returns (uint256)
    {
    ...
    function _verifyWhitelist(
        ...
        if (useWhitelist) {
            return (
                verify(owner, _candidate, _maxAmount, _deadline, _signature)
            );
        }
        return true;
    }
