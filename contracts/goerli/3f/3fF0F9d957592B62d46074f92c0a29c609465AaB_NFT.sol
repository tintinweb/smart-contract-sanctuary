...
contract NFT is ERC721, IMintableERC721, AccessControlMixin {
    bytes32 public constant PREDICATE_ROLE = keccak256("PREDICATE_ROLE");

  constructor(address _MintableERC721PredicateProxy) ERC721("Test NFT", "TNFT") {
    _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    _setupRole(PREDICATE_ROLE, _MintableERC721PredicateProxy);
    }

  function mint(address user, uint256 tokenid) public virtual override only(PREDICATE_ROLE){
        _mint(user, tokenid);
  }
  
   function mint(address user, uint256 tokenId, bytes calldata metaData) public virtual override only(PREDICATE_ROLE) {
        _mint(user, tokenId);
