...
contract HypervisorV3Migrator is IHypervisorV3Migrator {
    ...
    function migrate(address _hypervisor, uint8 percentageToMigrate, address recipient) external override {
        ...
        address v2Pair = uniswapV2Factory.getPair(hypervisor.token0(), hypervisor.token1());
        ...
        uint256 amount0V2ToMigrate = amount0V2.mul(percentageToMigrate) / 100;
        uint256 amount1V2ToMigrate = amount1V2.mul(percentageToMigrate) / 100;

        // approve the position manager up to the maximum token amounts
        TransferHelper.safeApprove(hypervisor.token0(), _hypervisor, amount0V2ToMigrate);
        TransferHelper.safeApprove(hypervisor.token1(), _hypervisor, amount1V2ToMigrate);
        ...
        TransferHelper.safeApprove(hypervisor.token0(), _hypervisor, 0);
    
        uint256 refund0 = amount0V2 - amount0V2ToMigrate;
        if (refund0 > 0) {
            TransferHelper.safeTransfer(hypervisor.token0(), msg.sender, refund0);
        }

        TransferHelper.safeApprove(hypervisor.token1(), _hypervisor, 0);

        uint256 refund1 = amount1V2 - amount1V2ToMigrate;
        if (refund1 > 0) {
            TransferHelper.safeTransfer(hypervisor.token1(), msg.sender, refund1);
        }
    }
}
