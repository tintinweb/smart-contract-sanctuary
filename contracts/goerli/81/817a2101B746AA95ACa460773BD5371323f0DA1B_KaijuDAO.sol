...
contract KaijuDAO is ERC721, ThreePerWallet, Ownable{
    ...
    //uint256 public constant MINT_PRICE_SOS = 50000000000000000000000000;
    //uint256 public constant MINT_PRICE_SOS_PRESALE = 40000000000000000000000000;
    uint256 public constant MINT_PRICE_ETH = 0.05 ether;
    uint256 public constant MINT_PRICE_ETH_PRESALE = 0.04 ether;
    uint256 public constant MINT_PRICE_SOS = 10000000000000000000;
    uint256 public constant MINT_PRICE_SOS_PRESALE = 8000000000000000000;
    uint256 public MAX_TOKENS = 100000;
    uint256 public MAX_AIRDROPS = 100;
    uint256 public remain_airdrops = 100;
    ...
    mapping(uint256 => uint16) public nations;
    ...
    constructor(address _openDAOAddress) ERC721("kaiju_test", "KAIJ"){
        openDAO = IOpenDAO(_openDAOAddress);
    }

    function _setNation(uint256 tokenId) internal {
    ...
    function airdrop(address[] calldata airdropAddresses) public onlyOwner {
        require(remain_airdrops-airdropAddresses.length>=0, "Too many airdrops");
        for(uint256 i = 0; i < airdropAddresses.length; i++) {
            minted++;
            _setNation(minted);
            _mint(airdropAddresses[i], minted);
        }
        remain_airdrops -= airdropAddresses.length;
    }

    function preSaleETH(bytes memory signature) external payable threePerWallet(msg.sender){
        require(!publicSale, "preSale closed");
        require(isWhitelisted(msg.sender, signature), "Not Whitelisted");
        require(MINT_PRICE_ETH_PRESALE == msg.value, "Invalid payment amount");
        require(minted + 1 <= MAX_TOKENS-MAX_AIRDROPS, "minted out");
        minted++;
        _mint(msg.sender, minted);
    }
    function preSaleSOS(bytes memory signature) external threePerWallet(msg.sender){
        require(!publicSale, "preSale closed");
        require(isWhitelisted(msg.sender, signature), "Not Whitelisted");
        require(minted + 1 <= MAX_TOKENS-MAX_AIRDROPS, "minted out");
        require(MINT_PRICE_SOS_PRESALE<=openDAO.balanceOf(msg.sender), "not enough balance");
        require(MINT_PRICE_SOS_PRESALE<=openDAO.allowance(msg.sender, address(this)), "low allowance");
        openDAO.transferFrom(msg.sender, owner(), MINT_PRICE_SOS_PRESALE);
        minted++;
        _mint(msg.sender, minted);
    }
    function publicETH() external payable threePerWallet(msg.sender){
        require(publicSale, "publicSale not poen");
        require(MINT_PRICE_ETH == msg.value, "Invalid payment amount");
        require(minted + 1 <= MAX_TOKENS-MAX_AIRDROPS, "minted out");
        minted++;
        _mint(msg.sender, minted);
    }
    function publicSOS() external threePerWallet(msg.sender){
        require(publicSale, "publicSale not poen");
        require(minted + 1 <= MAX_TOKENS-MAX_AIRDROPS, "minted out");
        require(MINT_PRICE_SOS<=openDAO.balanceOf(msg.sender), "not enough balance");
        require(MINT_PRICE_SOS<=openDAO.allowance(msg.sender, address(this)), "low allowance");
        openDAO.transferFrom(msg.sender, owner(), MINT_PRICE_SOS);
        minted++;
        _mint(msg.sender, minted);
    }
    ...
    function _transfer(address from, address to, uint256 tokenId) internal override(ERC721, ThreePerWallet) {
        ThreePerWallet._transfer(from, to, tokenId);
    }

    function setLeaders(address power, address money, address music) external onlyOwner {
    ...
    function getEthSignedMessageHashTest(address user) public pure returns (bytes32) {
        /*
        Signature is produced by signing a keccak256 hash with the following format:
        "\x19Ethereum Signed Message\n" + len(msg) + msg
        */
        bytes32 _messageHash = keccak256(abi.encode(user));
        return
        keccak256(
            abi.encodePacked("\x19Ethereum Signed Message:\n32", _messageHash)
        );
    ...
    function setPublicSale(bool isPublic) public onlyOwner {
        publicSale = isPublic;
    }
...
abstract contract ThreePerWallet is ERC721 {
    ...
    modifier threePerWallet(address wallet) {
        if (_isExternal(wallet)) {
            require(_ownedToken[wallet] < 3, "Can only hold three tokens per wallet");
        }
    ...
    function _mint(address to, uint256 tokenId) internal virtual override threePerWallet(to) {
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual override threePerWallet(to) {
