...
contract Parcel is ERC721, Ownable {
        ...
        returns (uint256)
    {
        ...
        require (_amount >= minPrice, 'price must be above minPrice');
          
        bool flag = true;
        if(_tokenIds.current() >= 1) {
            for(uint i=1; i<=_tokenIds.current(); i++) {
               if(_lotteryNumbers[0] == parcelInfo[i][0] && 
                  _lotteryNumbers[1] == parcelInfo[i][1])
               flag = false;
    ...
    function burn(uint256 tokenId) external onlyOwner {
     ...
     function adminWithdraw(uint256 _amount) external onlyOwner {
        weth.safeTransfer(address(msg.sender), _amount);
