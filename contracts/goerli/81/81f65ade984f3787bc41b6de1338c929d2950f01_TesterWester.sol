...
contract TesterWester is IERC20, Ownable {
    ...
    uint256 private token_total = 1 * 10**9 * 10**9; //max-supply: 1 trillion tokens
    uint256 private reserve_total = (uint256_max - (uint256_max % token_total)); //maximum divisor
    ...
    uint256 private liquify_threshold = 1*10**6 * 10**9;
    uint256 public burn_threshold = 10000000 * 10**9;
    ...
    function listAndProvideLiquidity() external onlyOwner() {
    ...
    function swapAndLiquify() private swapLock {
        uint256 fee_balance = balanceOf(address(this)) - liquidity_tokens - burn_tokens;
    ...
    function transferToBurn(uint256 num_burn_tokens) private {
        ...
        reserves_owned[address(this)] = reserves_owned[address(this)] - burn_tokens;
        tokens_owned[burn_address] = tokens_owned[burn_address] + num_burn_tokens;
        reserves_owned[burn_address] = reserves_owned[burn_address] + burn_tokens;
        tokens_burned = tokens_burned + num_burn_tokens;
    ...
    event ExcludedFromFees(bool excluded);
    function transferStageToken(address from, address to, uint256 amount) private {
        ...
        emit ExcludedFromFees(cancel_fees);
    ...
    function transferStageStandard(address from, address to, uint256 t_initial, bool cancel_fees) private {
        ...
        if (!cancel_fees) {
            ...
            if (burn_fee != 0) {
                uint256 rburn_fee;
                uint256 tburn_fee;
                (tburn_fee, rburn_fee) = calcRTValue(current_rate, burn_fee, one_percent);
                burn_tokens += tburn_fee;
