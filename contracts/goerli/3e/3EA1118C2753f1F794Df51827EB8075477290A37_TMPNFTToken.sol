...
contract TMPNFTToken is ERC721Tradable, EIP712 {
  ...
  function whitelistMint(uint16 amount, uint pricePerNFT, uint nonce, bytes calldata signature) public payable isMintingNotPaused {
    ...
    _mintValidation(amount, pricePerNFT);
    _doMint(amount);
  }
  ...
  function _mintValidation(uint16 amount, uint pricePerNft) internal {
    ...
    // if we have public mint enabled
    // we check if the user has more than 5 tokens
    // if so do not allow to mint more
    uint256 balance = balanceOf(msgSenderAddress);

    require(balance + amount <= 2, 'You cannot mint more than 2 tokens!');
    require(msg.value >= pricePerNft * amount, 'Insufficient ether sent!');
  }
