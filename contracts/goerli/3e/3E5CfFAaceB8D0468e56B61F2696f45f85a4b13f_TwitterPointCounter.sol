...
contract TwitterPointCounter is UsingWitnet, Ownable {
  ...
  ///@notice Saves a twitter account to be queried.
  ...
  function getPoints() public notPending {
    ...
    if (witnet.isOk(result)) {
      ...
      for (uint256 i = 0; i < values.length; i++) {
        string memory twitterId = uintToString(values[i]);
        liked.push(twitterId);
        users[twitterAuth.addressFromId(twitterId)].liked = true;
      }
    } else {
  ...
  function getSubscribers() external view returns (string[] memory) {
    return subscribers;
  }
