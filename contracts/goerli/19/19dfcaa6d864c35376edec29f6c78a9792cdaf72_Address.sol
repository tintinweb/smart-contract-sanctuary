...
contract Avatars is Ownable, ERC721 {
    ...
    function mintNFT(uint256 numberOfNfts) external {
        ...
        require(
            totalPublicSupply + numberOfNfts <= LOCALS_MAX,
            "Purchase would exceed LOCALS_MAX"
        );

        require(
            _publicSaleClaimed[msg.sender] + numberOfNfts <= publicSaleMaxMint,
     ...
     */
    function checkIfOriginalCrypteriorUsedForMint(uint256 _tokenId)
        public
        view
        returns (uint256)
    {
    ...
    function mintForOriginalCrypteriors(uint256[] calldata tokenIds) external {
        ...
        require(
            tokenIds.length < 10,
            "Cannot mint more than 10 in one transaction!"
        );
        require(
            tokenIds.length > 0,
        ...
        require(
            totalPublicSupply + tokenIds.length <= LOCALS_MAX,
            "Purchase would exceed LOCALS_MAX"
        );

        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                INFOriginalCrypteriors(_nftAddress).ownerOf(tokenIds[i]) ==
                    msg.sender,
                "Original Crypterior NFT have a different owner"
            );
            require(
                v1TokenIdUsedForMint[msg.sender][tokenIds[i]] <
                    MAX_MINTS_PER_ORIGINAL_CRYPTERIOR,
                "This Crypterior already been used for a free mint in your wallet"
            );
    ...
    function mintReserve(uint256 _count, address _to) public onlyOwner {
        ...
        require(
            totalGiftSupply + _count <= LOCALS_RESERVE,
    ...
    function changeMeme(
        ...
        require(
            validateString(_tokenMemeTextTop) == true,
            "Not valid string for top"
        );
        require(
            validateString(_tokenMemeTextBottom) == true,
            "Not valid string for bottom"
        );
        ...
        returns (uint256[] memory)
    {
        ...
        if (tokenCount == 0) {
            ...
            for (
                tokenId = 0;
                tokenId < (LOCALS_RESERVE + totalPublicSupply + 1);
                tokenId++
            ) {
                if (_exists(tokenId)) {
    ...
    function getToken(uint256 _tokenId)
        external
        view
        returns (string memory memeTextTop_, string memory memeTextBottom_)
    {
