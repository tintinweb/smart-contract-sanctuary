...
contract DustRoot is ERC20, Ownable, ExternalActor {
  constructor(string memory name, string memory symbol) ERC20(name, symbol) {
    _mint(msg.sender, 10000 * 10**uint256(decimals()));
  }

  function burn(address sender, uint256 amount) external onlyAllowedBurners {
    _burn(sender, amount);
  }

  function mint(address sender, uint256 amount) external onlyAllowedMinters {
    _mint(sender, amount);
  }
}
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function approve(address spender, uint256 amount) public virtual override returns (bool) {
        console.log("Trying to approve %s  %s", spender, amount);
