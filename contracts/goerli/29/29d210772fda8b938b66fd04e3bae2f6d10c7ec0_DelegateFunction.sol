...
contract Staking is IStaking, Initializable, Ownable, Pausable, ReentrancyGuard, IEventSender {
    ...
    function removeSchedule(uint256 scheduleIndex) external override onlyOwner {
        require(scheduleIdxs.remove(scheduleIndex), "REMOVE_FAIL");
...
interface IDelegateFunction {
    ...
    function delegateWithEIP1271(address contractAddress, DelegatePayload memory delegatePayload, bytes memory signature, SignatureType signatureType) external;
    ...
    /// @param signature Signature data 
    /// @param signatureType Type of signature used (EIP712|EthSign)
    function removeDelegationWithEIP1271(address contractAddress, FunctionsListPayload calldata functionsListPayload, bytes memory signature, SignatureType signatureType) external;
    ...
    /// @param signature Signature data 
    /// @param signatureType Type of signature used (EIP712|EthSign)
    function cancelPendingDelegationWithEIP1271(address contractAddress, FunctionsListPayload calldata functionsListPayload, bytes memory signature, SignatureType signatureType) external;
...
contract TokeMigrationPool is ILiquidityPool, Initializable, ERC20, Ownable, Pausable, IEventSender {
    using SafeMath for uint256;
    using SafeERC20 for ERC20;
    ...
    function withdrawAndMigrate() external whenNotPaused nonReentrant {
        address stakingContract = 0x96F98Ed74639689C3A11daf38ef86E59F43417D3;
        ...
        _approveStaking(amount, stakingContract);
        _burn(msg.sender, amount);

        IStaking(stakingContract).depositFor(msg.sender, amount, 0);
...
contract DelegateFunction is IDelegateFunction, Initializable, Ownable, Pausable, EventSender {
    ...
    function delegateWithEIP1271(
        ...
        _verifySignature(msg.sender, delegatePayloadHash, signature);
    ...
    function removeDelegationWithEIP1271(
        ...
        _verifySignature(msg.sender, functionsListPayloadHash, signature);
    ...
    function _delegate(address from, DelegateMap[] memory sets) internal whenNotPaused {
    ...
    function _hashDelegateMap(DelegateMap memory delegateMap) private pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
    ...
    function _verifyIERC1271Signature(
        ...
        bytes4 result = IERC1271(contractAddress).isValidSignature(payloadHash, signature);
    ...
    function _verifySignature(
        ...
        address signatureSigner = payloadHash.recover(signature);
        require(from == signatureSigner, "MISMATCH_SIGNER");
    }
    ...
    function _verifyNonce(address account, uint256 nonce) private {
        ...
        contractWalletNonces[account] = contractWalletNonces[account].add(1);
    }
