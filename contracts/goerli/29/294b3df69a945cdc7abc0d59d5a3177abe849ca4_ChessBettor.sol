...
contract ChessBettor is Ownable {
    ...
    struct UserData {
        ...
        uint256[] allGamesPlayed;
        uint256[] gamesWithUnclaimedBalance;
    }
    ...
    uint256[] private existingGameIds;
    mapping(uint256 => Game) private existingGames;
    ...
    event gameCreated(uint256 _gameId, address _voter, uint256 _amount);
    event votePlaced(uint256 _gameId, address _voter, uint256 _amount, uint8 _side);
    event gameEnded(uint256 _gameId, uint8 winner);
    event rewardWithdraw(address _benificiary, uint256 _amount);

    function createGame(uint256 _gameId, uint256 _amount) internal {
    ...
    function placeBet(uint256 _gameId, uint8 _vote, uint256 _amount) public payable {
    ...
    function endGame(uint256 _gameId, uint8 _winner, uint256 gameEndTimestamp) public onlyOwner {
        ...
        existingGames[_gameId].gameEndTimestamp = gameEndTimestamp;
    ...
    function claimRewards() external {
        (uint256 reward, uint256 fees, uint256[] memory claimedGames, int8[] memory results) = calculateRewards(msg.sender);
    ...
    function updateAllClaimedVotesAndListOfUnclaimedVotes(address _address, uint256[] memory toBeClaimedGames, int8[] memory results) internal {
        require(toBeClaimedGames.length == results.length, "Length Mismatch Error");

        uint256 currentIndex = 0;
        uint256 max = toBeClaimedGames.length;
        uint256[] storage unclaimedGames = allPlayerData[_address].gamesWithUnclaimedBalance;
        ...
        for(uint256 i = 0; i < unclaimedGames.length; i++) {
            ...
            if (currentIndex < max) {
                if (unclaimedGames[i] == toBeClaimedGames[currentIndex]) {
    ...
    function getActiveGameIndex(uint256 _gameId) internal view returns(uint) {
    ...
    function getCountOfClaimableGamesForAddress(address _address, uint256[] storage unclaimedGameIds) internal view returns(uint256) {
        uint256 count = 0;
    ...
    function getRewardAmountForPlayerForGame(Vote storage vote, Game storage game) internal view returns(uint256) {
        ...
        uint256 numerator = vote.amount.mul(game.totalAmount);
        uint256 denominator = (game.winner == 1) ? game.amountBlack : game.amountWhite;

        return numerator.div(denominator);
    }

    function formatData(address _address, uint256[] memory gameList) internal view returns(uint256, uint256, uint256, uint256, uint256, ReturnableGameSpecificUserData[] memory allGameData) {
        uint256 maxIndex = gameList.length;
        allGameData = new ReturnableGameSpecificUserData[](maxIndex);

        for (uint256 i = 0; i < maxIndex; i++) {
            uint256 gameId = gameList[i];
            ...
            allGameData[i] = ReturnableGameSpecificUserData(gameId, vote.voteFor, vote.amount, game.winner, result, vote.claimed);
        }

        return (
        allPlayerData[_address].totalGames,
        allPlayerData[_address].wins,
        allPlayerData[_address].draws,
        allPlayerData[_address].losses,
        allPlayerData[_address].undetermined,
        allGameData
        );
    }
    ...
    function gameExists(uint256 _gameId) public view returns(bool) {
        return existingGames[_gameId].gameExists;
    }

    function calculateRewards(address _address) public view returns(uint256 claimableReward, uint256 fees, uint256[] memory gamesHavingClaimableRewards, int8[] memory results) {
        uint reward = 0;
        uint256 index = 0;
        uint256[] storage unclaimedGameIds = allPlayerData[_address].gamesWithUnclaimedBalance;
        ...
        uint256[] memory removableGames = new uint256[](size);
    ...
    function getAllGamesCreatedTillDate() external view returns(uint256[] memory) {
        return existingGameIds;
    }

    function getGamesPlayedByPlayer(address _address) external view returns(uint256[] memory) {
        return allPlayerData[_address].allGamesPlayed;
    }

    function getPlayerStatsForUnclaimedGames(address _address) external view returns(uint256, uint256, uint256, uint256, uint256, ReturnableGameSpecificUserData[] memory) {
        return formatData(_address, allPlayerData[_address].gamesWithUnclaimedBalance);
    }

    function getPlayerStatsForAllGames(address _address) external view returns(uint256, uint256, uint256, uint256, uint256, ReturnableGameSpecificUserData[] memory) {
        return formatData(_address, allPlayerData[_address].allGamesPlayed);
    }
    ...
    function getGameDetails(uint256 _gameId) external view returns(
        ...
        uint256 amountBlack,
        uint256 numberBets,
        Vote[] memory voteList,
        bool isOn
    ) {
        Game storage _game =  existingGames[_gameId];
        require(_game.gameExists, "No game exists for the given game id.");

        return (_game.gameId, _game.winner, _game.gameEndTimestamp, _game.totalAmount, _game.amountWhite, _game.amountBlack, _game.numberBets, _game.voteList, _game.isOn);
    }
}