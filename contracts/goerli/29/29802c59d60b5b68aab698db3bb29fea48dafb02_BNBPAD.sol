...
contract BNBPADDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("BNBPAD_Dividend_Tracker", "BNBPAD_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BNBPAD_Dividend_Tracker: No transfers allowed");
    }
    ...
    function withdrawDividend() public pure override {
        require(false, "BNBPAD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BNBPAD contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BNBPAD_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "BNBPAD_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BNBPAD is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    BNBPADDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 1200000000000 * (10**18);
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _DevelopmentFee) public onlyOwner {
        BNBRewardsFee = _bnbRewardFee;
        liquidityFee = _liquidityFee;
        DevelopmentFee = _DevelopmentFee;

        totalFees = BNBRewardsFee.add(liquidityFee).add(DevelopmentFee); // total fee transfer and buy
    }
    ...
    function setDevelopmentWallet(address payable _newDevelopmentWallet) public onlyOwner {
        DevelopmentWallet = _newDevelopmentWallet;
    }

    constructor() ERC20("BNB PAD", "BNBPAD") {
        BNBRewardsFee = 5;
        liquidityFee = 2;
        extraFeeOnSell = 12; // extra fee on sell
        DevelopmentFee = 5;
        DevelopmentWallet = payable(0x21E4bf4f0B1bE37362177A0EeAf04d2D8D1388Db);

        totalFees = BNBRewardsFee.add(liquidityFee).add(DevelopmentFee); // total fee transfer and buy

    	dividendTracker = new BNBPADDividendTracker();
	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
        ...
        */
        _mint(owner(), 120000000000000 * (10**18));
    }
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BNBPAD: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BNBPAD: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BNBPAD: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BNBPAD: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BNBPAD: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BNBPAD: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // worthy Development fee
        uint256 DevelopmentAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee)).mul(DevelopmentFee);
    ...
    function swapAndSendBNBToDevelopment(uint256 tokenAmount) private {
        swapTokensForBnb(tokenAmount, DevelopmentWallet);
    }
