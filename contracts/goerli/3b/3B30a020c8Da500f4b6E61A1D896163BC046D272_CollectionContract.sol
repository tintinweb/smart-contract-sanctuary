  ...
  ERC721BurnableUpgradeable
{
  ...
  function initialize(
    ...
    owner = _creator;
    collectionFactory = ICollectionFactory(msg.sender);
  ...
  /**
   * @notice Allows a creator to mint an NFT and have creator revenue/royalties sent to an alternate address.
   * Also sets approval for the provided marketplace.  This can be used by creators the first time they mint an NFT to
  ...
  function updateMaxTokenId(uint256 _maxTokenId) external onlyOwner {
    ...
    require(nextTokenId + 1 <= _maxTokenId, "CollectionContract: Max token ID must be greater than last mint");
