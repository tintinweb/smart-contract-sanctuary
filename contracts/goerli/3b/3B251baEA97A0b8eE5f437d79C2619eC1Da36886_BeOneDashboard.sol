...
contract BeOneDashboard is Exponential{
  ...
  struct MetaInfo {
    uint256 collateralAvailableInUsd;
  ...
  struct CToken {
    ...
    bool isBorrowable;
    uint256 lendingApr;
    uint256 lendingAprReward;
    uint256 borrowingApr;
    uint256 borrowingAprReward;
    ...
    returns (Dashboard memory)
  {
    ...
    for (uint256 i = 0; i < beOneRegistry.getCTokensLength(); i++) {
      ...
      CTokensArray[i] = CToken({
        ...
        lendingAprReward: getLendingAprReward(cToken, unitroller),
        borrowingApr: 10000.mul(cToken.borrowRatePerBlock().mul(numberOfBlocksPerDay).mul(365)).div(1e18),
        borrowingAprReward: 0,
        userBalanceInUnderlyingToken: EIP20Interface(cToken.underlying()).balanceOf(userAddress),
        userBalanceInCToken: userBalanceInCToken(userAddress, cToken),
        userBorrowedInToken: cToken.borrowBalanceStored(userAddress),
        userBorrowableInToken: min(metaInfo.collateralAvailableInUsd.div(beOnePriceOracle.getUnderlyingPrice(cToken)).mul(collateralFactor).div(1e18), EIP20Interface(cToken.underlying()).balanceOf(address(cToken))),
        userIsCollateral: unitroller.checkMembership(userAddress, cToken),
        userClaimableReward: getSupplyClaimableBalance(address(cToken), userAddress, unitroller),
        allowedAmountForToken: EIP20Interface(cToken.underlying()).allowance(userAddress, address(cToken)),
        totalSupply: cToken.totalSupply(),
        totalBorrows: cToken.totalBorrows(),
        userRedeemableInToken: min(userBalanceInCToken(userAddress, cToken), min(metaInfo.collateralAvailableInUsd.div(beOnePriceOracle.getUnderlyingPrice(cToken)), EIP20Interface(cToken.underlying()).balanceOf(address(cToken))))
      });
    ...
    return
      Dashboard({
        ...
        apr: getApr(CTokensArray, beOneRegistry.getCTokensLength()),
        aprReward: getAprReward(CTokensArray, beOneRegistry.getCTokensLength()),
        totalLendedInUsd: metaInfo.totalSupplyInUsd,
        totalBorrowedInUsd: metaInfo.totalBorrowedInUsd,
    ...
    function getSupplyClaimableBalance(address _cToken, address _supplier, Unitroller unitroller)
        public
        view
        returns (uint256 supplierAccrued)
    {
        ...
        uint supplySpeed = unitroller.compSupplySpeeds(_cToken);
    ...
    function getMetaInfo(address userAddress, Unitroller unitroller)
        public
        view
        returns (MetaInfo memory) {

      uint256 usertotalSupplyInUsd;
      uint256 usertotalCollateralExploitedInUsd;
      uint256 tokenPriceInUsd;
      uint256 collateralFactor;
      uint256 totalSupplyInUsd;
      uint256 totalBorrowedInUsd;
      ...
      for (uint256 i = 0; i < beOneRegistry.getCTokensLength(); i++) {
          ...
          // collateralAvailable
          if (unitroller.checkMembership(userAddress, cToken) == true) {
            usertotalSupplyInUsd += userBalanceInCToken(userAddress, cToken).mul(tokenPriceInUsd);
          }
          
          (, collateralFactor) = unitroller.markets(address(cToken));          
          usertotalCollateralExploitedInUsd += getUserCollateralExploitedInUsd(userAddress, cToken, tokenPriceInUsd, collateralFactor);

          totalSupplyInUsd += getTotalSupplyInUsd(cToken, tokenPriceInUsd);
          totalBorrowedInUsd += getTotalBorrowsInUsd(cToken, tokenPriceInUsd);
      }
      return MetaInfo({
        collateralAvailableInUsd: usertotalSupplyInUsd.sub(usertotalCollateralExploitedInUsd),
        totalSupplyInUsd: totalSupplyInUsd,
        totalBorrowedInUsd: totalBorrowedInUsd
      });
}
      function getTotalSupplyInUsd(CErc20 cToken, uint256 tokenPriceInUsd)
        public
        view
        returns (uint) {
          return cToken.totalSupply().mul(cToken.exchangeRateStored()).div(1e18).mul(tokenPriceInUsd).div(uint(10) ** uint(cToken.decimals()));
        }

      function getTotalBorrowsInUsd(CErc20 cToken, uint256 tokenPriceInUsd)
        public
        view
        returns (uint) {
          return cToken.totalBorrows().mul(tokenPriceInUsd).div(uint(10) ** uint(cToken.decimals()));
        }

      function getUserCollateralExploitedInUsd(address userAddress, CErc20 cToken, uint256 tokenPriceInUsd, uint collateralFactor)
        public
        view
        returns (uint) {
          if (collateralFactor > 0) {
          return cToken.borrowBalanceStored(userAddress).mul(tokenPriceInUsd).mul(1e18).div(collateralFactor);
          } else {
            return 0;
          }
        }
    ...
    function getLendingAprReward(CErc20 cToken, Unitroller unitroller)
        public
        view
        returns (uint256)
    {
        BeOnePriceOracle beOnePriceOracle = BeOnePriceOracle(beOneRegistry.getPriceOracleAddress());
        (uint256 beOneTokenPriceInUsd,) = beOnePriceOracle.getStoredTokenPrice(beOneRegistry.getRewardToken());
        uint256 rewardTokensPerYearInUsd = unitroller.compSupplySpeeds(address(cToken)).mul(numberOfBlocksPerDay).mul(365).mul(beOneTokenPriceInUsd);
        uint256 totalSupplyInTokenInUsd = cToken.totalSupply().mul(cToken.exchangeRateStored()).div(1e18).mul(beOnePriceOracle.getUnderlyingPrice(cToken));
        if (totalSupplyInTokenInUsd > 0) {
          return 10000.mul(rewardTokensPerYearInUsd).div(totalSupplyInTokenInUsd);
        } else return 0;
    }

    function getAprReward(CToken[] memory CTokensArray, uint256 length)
        public
        pure
        returns (uint256 totalAprReward)
    {
        for (uint256 i = 0; i < length; i++) {
          totalAprReward += CTokensArray[i].lendingAprReward;
        }
        totalAprReward = totalAprReward.div(length);
    }


    function getApr(CToken[] memory CTokensArray, uint256 length)
        public
        pure
        returns (uint256 totalApr)
    {
        for (uint256 i = 0; i < length; i++) {
          totalApr += CTokensArray[i].lendingApr;
        }
        totalApr = totalApr.div(length);
    }
