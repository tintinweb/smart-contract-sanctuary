...
contract RarityBundle is IERC721Receiver {
    ...
    function bundleInfo(address _owner, uint _bundle_index) public view returns (uint _start_id, uint _end_id, uint _class_id, uint _log) {
        require(_bundle_index < ownedBundle[_owner].length);
        _start_id = ownedBundle[_owner][_bundle_index].start_id;
        _end_id = ownedBundle[_owner][_bundle_index].end_id;
        _class_id = ownedBundle[_owner][_bundle_index].class_id;
        _log = ownedBundle[_owner][_bundle_index].log;
    }
