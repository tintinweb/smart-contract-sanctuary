...
/**
 * @title HeroSourceBridgeEthereum
 */
contract HeroSourceBridgeEthereum is FxBaseRootTunnel {
    ...
    function depart(address to, uint256 tokenId) external {
        ...
        _sendMessageToChild(abi.encode(DEPART_SINGLE, abi.encode(msg.sender, to, tokenId, seed, block.number)));
    }

    function departBatch(address to, uint256[] memory tokenIds) external {
        require(tokenIds.length <= BATCH_LIMIT, "HeroSourceBridgeEthereum: EXCEEDS_BATCH_LIMIT");
        uint256[] memory seeds = new uint256[](tokenIds.length);
        for (uint i=0; i<tokenIds.length; i++) {
            seeds[i] = ISource(rootToken).seeds(tokenIds[i]);
            _depart(msg.sender, to, tokenIds[i], block.number);
        }
        _sendMessageToChild(abi.encode(DEPART_BATCH, abi.encode(msg.sender, to, tokenIds, seeds, block.number)));
    }

    function _syncDepartFromChildSingle(bytes memory syncData) private {
    ...
    function _syncDepartFromChildBatch(bytes memory syncData) private {
    ...
    function _depart(address from, address to, uint256 tokenId, uint256 blockNumber) private {
        ...
        if (tokenId % 7 == 1) {
            ...
            // polygon origin
            require(IERC721(rootToken).ownerOf(tokenId) == from);
            ISource(rootToken).bridgeBurn(tokenId);
        } else {
            revert();
        }
    ...
    function _arrive(address from, address to, uint256 tokenId, uint256 seed, uint256 blockNumber) private {
        ...
        if (tokenId % 7 == 1) {
            ...
            // polygon origin
            ISource(rootToken).bridgeMint(to, tokenId, seed);
        } else {
            revert();
        }
     ...
     */
    function arriveClaim(bytes memory inputData) external {
        ...
        if (syncType == DEPART_SINGLE) {
            _syncDepartFromChildSingle(syncData);
        }  else if (syncType == DEPART_BATCH) {
            _syncDepartFromChildBatch(syncData);
        } else {
            revert("HeroSourceBridgeEthereum: INVALID_SYNC_TYPE");
        }
    }
}
