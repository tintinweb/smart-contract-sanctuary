...
contract AnimeGirls is ERC721URIStorage, Ownable {
    using Strings for *;
    ...
    constructor() ERC721("Crypto Nijigen Girls", "CNG") {
        setURI("https://werewolf-resource.53site.com/NFT/api/item/");
    }
    ...
    function safeMintCNG(uint256 _tokenId) external payable{
        ...
        _setTokenURI(_tokenId, uri(_tokenId));
    ...
    function setTokenPrice(uint256[] memory tokenIds, uint256[] memory price) public virtual onlyOwner{
        require(tokenIds.length == price.length,"CNGE006");
        for(uint256 i=0;i<tokenIds.length;i++){
            _tokenPrice[tokenIds[i]] = price[i];
        }
    }
