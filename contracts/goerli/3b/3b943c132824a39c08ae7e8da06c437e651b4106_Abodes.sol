...
contract Abodes is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getAbodeName(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "ABODE", abodeName);
    }

    function getDwelling(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "DWELLING", dwelling);
    }

    function getVillageName(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "VILLAGE", village);
    }

    function getRegion(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "REGION", region);
    }
    ...
    function pluck(
        ...
        if (keccak256(abi.encodePacked(keyPrefix)) == keccak256("DWELLING") && greatness > 14) {
            output = string(abi.encodePacked(output, " ", material[rand % material.length]));
        }

        if (keccak256(abi.encodePacked(keyPrefix)) == keccak256("VILLAGE") && greatness > 19) {
            output = string(abi.encodePacked(output, " ", villageSuffix[rand % villageSuffix.length]));
        }

        if (keccak256(abi.encodePacked(keyPrefix)) == keccak256("REGION") && greatness > 15) {
            output = string(abi.encodePacked(output, regionSuffix[rand % regionSuffix.length]));
        }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        parts[3] = getAbodeName(tokenId);

        parts[4] = '</text><text x="10" y="40" class="base">';

        parts[5] = getDwelling(tokenId);

        parts[6] = '</text><text x="10" y="60" class="base">';

        parts[7] = getVillageName(tokenId);
        ...
        string memory output = string(
            abi.encodePacked(parts[0], parts[1], parts[2], parts[3], parts[4], parts[5], parts[6], parts[7], parts[8], parts[9], parts[10])
        );
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Character #',
                        toString(tokenId),
                        '", "description": "Abodes for Loots are randomized generated and stored on chain. Images and other functionality are intentionally omitted for others to interpret. Feel free to use Abodes in any way you want. Inspired and compatible with Loot (for Adventurers)", "image": "data:image/svg+xml;base64,',
    ...
    constructor() ERC721("Abodes (for Loot)", "ABODES") Ownable() {}
}
