...
contract ElfMetadataHandler {
        ...
        returns (string memory)
    {
        DataStructures.Token memory token = DataStructures.getToken(sentinel);
         ...
         console.log(token.sentinelClass, "sentinel");
         console.log("hair", token.hair);
         console.log("race", token.race);
         console.log("weapon", token.primaryWeapon);
         console.log("acc", token.accessories);
...
function getToken(uint256 character) internal pure returns(Token memory token) {
    ...
    token.primaryWeapon = (token.sentinelClass * 15) + (token.primaryWeapon + 1);
