    ...
    ReentrancyGuardUpgradeable
{
   ...
   /**
    ...
    function rewardsPerToken() public view override returns (uint256) {
        if (totalStakes == 0) {
            ...
            return rewardsPerTokenStored.add((lastBlockApplicable() - lastUpdateBlock) * _stakingRate * rewardsTokenOne / totalStakes);
        }
    }
    ...
    /**
     * @notice A method for ADMIN to set staking rate
   ...
   /**
     ...
     */
    function grantFlurryInternal(address user, uint256 amount) nonReentrant
        internal
        returns (uint256)
    {
...
interface ILPStakingRewards {
    ...
    /**
     * @return staking rate for all LP earned per block
     */
    function rewardsRate() external view returns (uint256);

    /**
     * @notice Admin function - set staking rate for all LP earned per block
     */
    function setStakingRate(uint256 _stakingRate) external;
     ...
     * this will fail silently and user rewards remains as a credit in this contract
     */
    function claimReward(address lpAddr) external;
     ...
     * @param rewardDuration Reward Duration in number of blocks
     */
    function startRewards(address lpAddr, uint256 rewardDuration) external;

    /**
     * @notice Admin function - End Rewards distribution earlier if there is one running
     * @param lpAddr Address of LP Token contract
     */
    function endRewards(address lpAddr) external;
