...
contract ChubbyMetalApe is ERC721Enumerable, ERC998TopDown, Ownable {
    ...
    //一次紀錄7個裝備
    mapping(uint256 => uint8) public pairCount;

    //配對的來源The source of the pairing
    mapping(uint256 => SourceOfPairing) public sourceOfPairingIds;

    //Track ChubbyApe produced
    mapping(uint256 => bool) public mintedChubbyApe;
    ...
    constructor(address _registry, address _chubbyApeAddress)
        ERC998TopDown("CMAP", "CMAPE")
    {
    ...
    modifier onlyAuthorized(uint256 _tokenId) {
        require(
            ...
            "MetalApe: Caller is not owner nor approved"
        );
        _;
    }

    modifier mintOnlyOnce(uint256 _chubbyApeTokenId) {
        require(!mintedChubbyApe[_chubbyApeTokenId], "Can only mint once");
        mintedChubbyApe[_chubbyApeTokenId] = true;
        _;
    }
    ...
    /**
    這個功能只能從 chubbyApeEquipment 來呼叫
    */
    function mintMetalApe(
    ...
    ) external mintOnlyOnce(_chubbyApeTokenId) {
        /**來源必須是合約 */
        require(
            registry.isValid1155Contract(msg.sender) == true,
            "Must from ChubbyApeEquipment"
        );

        require(
            _targetAddress == chubbyApe.ownerOf(_chubbyApeTokenId),
        ...
        //把自己的裝備，安裝到新的MetalApe身上
        for (uint256 i = 0; i < _itemIds.length; i++) {
            _equip(_tokenId, _itemAddress, _itemIds[i]);
        }
    }
        ...
        returns (uint256 newMetalApeId)
    {
        ...
        require(
            _targetAddress == ownerOf(_leftPairId),
            "Must own MetalApe token"
        );
        require(
            _targetAddress == ownerOf(_rightPairId),
            "Must own MetalApe token"
        );

        newMetalApeId = totalSupply();

        _safeMint(_targetAddress, newMetalApeId);

        //增加配對的次數
        addLeftPairCount(_leftPairId, _rightPairId);

        //增加配對的來源紀錄
        sourceOfPairingIds[newMetalApeId] = SourceOfPairing({
            leftPairId: _leftPairId,
            rightPairId: _rightPairId
        });

        //把自己的裝備，安裝到新的MetalApe身上
        for (uint256 i = 0; i < _itemIds.length; i++) {
            _equip(newMetalApeId, _itemAddress, _itemIds[i]);
        }
    ...
    /**增加左右配對的次數 */
    function addLeftPairCount(uint256 _leftPairId, uint256 _rightPairId)
        internal
    {
        //add Left pairCount
        uint8 currentLeftCount = pairCount[_leftPairId];
        pairCount[_leftPairId] = currentLeftCount + 1;

        //add Right pairCount
        uint8 currentRightCount = pairCount[_rightPairId];
        pairCount[_rightPairId] = currentRightCount + 1;
    }
    ...
    裝備一整套裝備
    用途:
    1.用 chubby ape 轉化出一隻MetalApe 的時候，帶入一整套裝備來安裝
    2.用兩隻 MetalApe 轉化出一隻 MetalApe 的時候，帶入一整套裝備來安裝
     * @dev Equip a complete set of equipment
     */
    ...
    */
    ...
    function _equip(
        ...
        require(
            ...
            "Adventurer: Item contract must be in the registry"
        );

        string memory itemType = IChubbyApeEquipments(_itemAddress).itemTypeFor(
            _itemId
        );
        require(
            registry.isValidItemType(itemType),
            "Adventurer: Invalid item type"
        );
    ...
    */
    ...
    function attributes(
        //uint256 _tokenId, //原本是要從合約來叫，現在是從後端
        address[] memory _itemAddress,
        uint256[] memory _id
        ...
        for (uint8 i = 0; i < allItemTypes.length; i++) {
            ...
            //Item memory item = equipped[_tokenId][itemType];
            if (_itemAddress[i] != address(0)) {
                string memory name = IChubbyApeEquipments(_itemAddress[i])
                    .nameFor(_id[i]);
                if (first) {
    ...
    function onERC721Received(
        ...
        require(
            ...
            "MetalApe: Only the MetalApe contract can pull items in"
        );
    ...
    function onERC1155Received(
        ...
        require(
            operator == address(this),
            "Only the MetalApe contract can pull items in"
        );
    ...
    /**
     取得猴子的配對使用數量  ;  
     */
    function getMetalApeCount(uint256 _metalApeId)
        public
        view
        returns (uint256)
    {
        return pairCount[_metalApeId];
    }
