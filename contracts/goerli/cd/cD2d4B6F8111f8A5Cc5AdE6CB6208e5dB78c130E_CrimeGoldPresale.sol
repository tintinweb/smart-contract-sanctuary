...
contract CrimeGoldPresale is PausableUpgradeable {
    ...
    uint256 _maxCGoldAmount, uint64 _timestampStart, uint64 _whitelistDurationPeriod, uint64 _presaleDuration) 
    public initializer {
    ...
    function payWithToken(uint256 stableCoinAmount) external whitelisted {
        ...
        IERC20Upgradeable(payWithTokenAddress).safeTransferFrom(msg.sender, address(this), stableCoinAmount);

        IERC20Upgradeable(cGoldAddress).safeTransfer(msg.sender, cGoldAmount);
    ...
    function addLiquidityToRouter(uint256 cGoldAmount, uint256 stableCoinAmount) external {
        ...
        require(stableCoinAmount <= liquidityStableAmount, "Too big stable amount");
        IERC20Upgradeable(cGoldAddress).approve(routerAddress, cGoldAmount);
        IERC20Upgradeable(payWithTokenAddress).approve(routerAddress, stableCoinAmount);
        IUniswapV2Router01(routerAddress).addLiquidity(payWithTokenAddress, cGoldAddress, stableCoinAmount, cGoldAmount, 0, 0, msg.sender, block.timestamp + 3600);
    }

    function getCGoldPurchaseAmount(uint256 stableCoinAmount) public view returns (uint256 cGoldAmount) {
        cGoldAmount = stableCoinAmount.div(pricePerTokenInStable.div(10**18));
    }
    ...
    modifier whitelisted {
        ...
        if(!whitelistedUsers[msg.sender])
            require(block.timestamp > timestampStart + whitelistDurationPeriod, "User is not whitelisted");
        require(block.timestamp <= timestampStart + whitelistDurationPeriod + presaleDuration, "Presale is finished");
        _;  
    }
}
