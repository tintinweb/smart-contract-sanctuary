  ...
  ExponentialNoError
{
  ...
  /// @notice Emitted when a registry is set by the admin
  ...
  function grantCompInternal(address user, uint256 amount)
    ...
    returns (uint256)
  {
    BeOneToken beOneToken = BeOneToken(
      BeOneRegistry(registryAddress).getRewardToken()
    );
    beOneToken.mint(user, amount);
    return 0;
  }
  ...
  /**
   * @notice Returns true if the given cToken market is borrowable
   * @param cToken The market to check if borrowable
   */
  function isBorrowAllowed(CToken cToken) public view returns (bool) {
    return borrowGuardianPaused[address(cToken)] == false;
  }
  ...
  /**
   * @notice Sets a new registry for the comptroller
   * @dev Admin function to set a new registry
   * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
   */
  function _setRegistry(address newRegistry) public returns (uint256) {
    // Check caller is admin
    if (msg.sender != admin) {
      return fail(Error.UNAUTHORIZED, FailureInfo.SET_NEW_REGISTRY);
    }

    address oldRegistry = registryAddress;
    registryAddress = newRegistry;

    emit NewRegistry(oldRegistry, newRegistry);
...
import "./mock/ERC20Token.sol";
import "./mock/AccessControl.sol";

contract BeOneToken is ERC20Token, AccessControl {
  constructor() public ERC20Token("BeOne Token", "BEONE", 4)  {
