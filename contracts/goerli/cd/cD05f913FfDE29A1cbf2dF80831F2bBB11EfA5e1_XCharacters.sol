...
contract XCharacters is ERC721Enumerable, ReentrancyGuard, Ownable {
    uint256 public price = 20000000000000000; //0.02 ETH

    //xLoot Contract
    address public xLootAddress = 0x44b325b7D243C0b02353514362d80C5Dd7B3C99F;
    xLootInterface public xLootContract = xLootInterface(xLootAddress);

    uint256 private offset = 4000;
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Extension Character #',
                        toString(tokenId),
                        '", "description": "xCharacters are randomized generated and stored on chain. Images and other functionality are intentionally omitted for others to interpret. Feel free to use xCharacters in any way you want. Inspired and compatible with xLoot (for Adventurers)", "image": "data:image/svg+xml;base64,',
    ...
    function mint(uint256 tokenId) public payable nonReentrant {
        require(tokenId > 20000 && tokenId <= 24000, "Token ID invalid");
    ...
    function multiMint(uint256[] memory tokenIds) public payable nonReentrant {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(tokenIds[i] > 20000 && tokenIds[i] < 24000, "Token ID invalid");
            _safeMint(msg.sender, tokenIds[i]);
        }
    }

    function mintWithxLoot(uint256 xLootId) public payable nonReentrant {
        require(xLootId > 8001 && xLootId <= 16000, "Token ID invalid");
        require(xLootContract.ownerOf(xLootId) == msg.sender, "Not the owner of this xLoot");
        _safeMint(_msgSender(), xLootId + offset);
    }

    function multiMintWithxLoot(uint256[] memory xLootIds) public payable nonReentrant {
        for (uint256 i = 0; i < xLootIds.length; i++) {
            require(xLootContract.ownerOf(xLootIds[i]) == msg.sender, "Not the owner of this xLoot");
            _safeMint(_msgSender(), xLootIds[i] + offset);
        }
    }
    ...
    constructor() ERC721("Extension Characters", "xCHAR") Ownable() {}
}
