...
contract Staking is Context, Ownable {
     ...
     */
    function listStakes(bool unlocked) external view returns (StakeItem[] memory) {
        address sender = _msgSender();
        uint256 lastStake = _lastStake[sender];

        StakeItem[] memory stakes = new StakeItem[](lastStake);
        ...
        for (uint256 i = 0; i < lastStake; i += 1) {
            ...
            if (unlocked ? stake.withdrawn > 0 : stake.withdrawn == 0) {
                ...
                item.amount = stake.amount;
                item.percent = stake.percent;
                item.unlockAt = stake.unlockAt;
                item.withdrawn = stake.withdrawn;
