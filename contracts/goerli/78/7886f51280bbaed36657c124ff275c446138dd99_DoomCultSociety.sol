...
contract DoomCultSociety is ERC721 {
    ...
    function imageURI(uint256 tokenId) public pure returns (string memory) {
        string memory result = Base64.encode(getImgData(tokenId), 36, 3);
        assembly {
            let ptr := add(result, 0x20)
            mstore(ptr, '<img src="data:image/svg+xml;bas')
            mstore(add(ptr, 0x20), or('e64,', and(0xffffffffffffffffffffffffffffffffffffffffffffffffffffffff, mload(add(ptr, 0x20)))))
            mstore(result, add(mload(result), 36))
