...
// File: contracts\tunnel\BaseRootTunnel.sol
...
// File: contracts\bridge\BridgeRootEnd_v1.sol
...
contract BridgeRootEnd_v1 is BaseRootTunnel {
    using SafeERC20 for IERC20;

    event TransferrRquestToPolygonSent(address, address, uint);
    event TokenFromPolygonClaimed(address, address, uint);
    ...
    */
    ...
    function _sendTransferrRquestToPolygon(address to, address token, uint amount) internal {
        _sendMessageToChild( abi.encode(to, token, amount));

        emit TransferrRquestToPolygonSent(to, token, amount);
    }

    function _parseMsg(bytes memory message) internal pure returns (address _to, address _token, uint _amount) {
        (_to, _token, _amount) = abi.decode(message, (address, address, uint));
    }

    /**
    * @dev may not work since it is not payable, tt should be removed?
    */
    function _processMessageFromChild(bytes memory message) internal override {
        (address _to, address _token, uint _amt) = _parseMsg(message);
        _claimAsset(_to, _token, _amt);       
    }

    /**
     * @dev Claim Asset from Polygon
     * @notice withdrawal a token from bridge
     * @param to the claiment
     * @param token The address of the token to transfer. (For ETH: 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE)
     * @param amount The amount of tokens to transfer. (For max: `uint256(-1)` (Not valid for ETH))
     */
    function _claimAsset (address to, address token, uint amount) internal {

        require( to == msg.sender, "DefiEezyV1: only-same-user-can-claim");

        address payable _receiver = payable(to);

        if (token == ethAddr) {
            require(address(this).balance >= amount, "DefiEezyV1: insufficient-balance");
            (bool claimed,) = _receiver.call{value: amount}("");
            require(claimed, "DefiEezyV1: failed-call-with-amount");
        } else {
            IERC20 tokenContract = IERC20(token);
            require( tokenContract.balanceOf(address(this)) >= amount, "DefiEezyV1: insufficient-token-balance");
            tokenContract.safeTransfer(_receiver, amount);
        }

        emit TokenFromPolygonClaimed(to, token, amount);
    }

    /**
     * @dev receive eth with calldata but doesn't match any functions
     */
    fallback () external payable {
        if(msg.value > 0){
            _sendTransferrRquestToPolygon(msg.sender, ethAddr, msg.value);
        }
        else
        {
            require(1 < 0, "DefiEezyV1: invalid-function");
        }
    }    
    ...
    /**
     * @dev Transfer Asset To Polygon
     * @notice Deposit a token to bridge
     * @param token The address of the token to transfer. (For ETH: 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE)
     * @param amount The amount of tokens to transfer. (For max: `uint256(-1)` (Not valid for ETH))
     */
    function transferAsset (address token, uint amount) external payable {
        uint _amt = amount;
        if(token == ethAddr){
            require(msg.value > 0, "DefiEezyV1: invalid-ether-amount");
            _amt = msg.value;
        }
        else
        {
            ...
            _amt = _amt == uint(-1) || _amt < _balance ? _balance : _amt;
            require(_amt > 0, "DefiEezyV1: invalid-token-amount");
            tokenContract.safeTransferFrom(msg.sender, address(this), _amt);
        }

        _sendTransferrRquestToPolygon(msg.sender, token, _amt);
    }

    /**
     * @notice receive asset from  L2 to L1, validated by proof
     * @dev This function verifies if the transaction actually happened on child chain
     *
     * @param inputData RLP encoded data of the reference tx containing following list of fields
     *  0 - headerNumber - Checkpoint header block number containing the reference tx
     ...
     *  9 - receiptLogIndex - Log Index to read from the receipt
     */
    function claimAsset(bytes calldata inputData) external payable {
        bytes memory message = _validateAndExtractMessage(inputData);
