...
contract hellonft is ERC721Enum, Ownable, ReentrancyGuard {
    ...
    string memory _initNotRevealedUri) ERC721P('hellonft', 'hellonft') {setBaseURI(_initBaseURI); setNotRevealedURI(_initNotRevealedUri);}
    ...
    function tokenURI(uint256 _tokenId) external view virtual override returns (string memory) {
       ...
       string memory notRevealed = _prerevealURI(); 
       if(revealed == false) {

        return bytes(notRevealed).length > 0	? string(abi.encodePacked(notRevealed, _tokenId.toString(), ".json")) : "";
