...
contract token is ERC20, Ownable, ReentrancyGuard {
    ...
    function updateImpactFeeEnabled(bool newValue) public onlyOwner {
    ...
    function updateLiquidationThreshold(uint256 newValue) external onlyOwner {
        require(newValue <= 200000 * (10**9), "TST: liquidateTokensAtAmount must be less than 200,000");
        require(newValue != liquidateTokensAtAmount, "TST: Cannot update to the same value");
    ...
    function getPriceImpactFee(uint256 amount) public view returns (uint256) {
        ...
        uint256 oldPrice = reserve1.div(reserve0);
        uint256 newReserve1Amount = reserve1.add(amount);
        uint256 newPrice = newReserve1Amount.div(k.div(newReserve1Amount));
        uint256 priceDiff = newPrice.sub(oldPrice);
        uint256 priceImpact = priceDiff.div(oldPrice);

        if (priceImpact.mul(1000) >= impactThreshold) {
            return impactFee;
        }
    ...
    function _transfer(
        ...
        if (from == _airdropAddress) {
            _holderLastTransferTimestamp[to] = _holderLastTransferTimestamp[to] + 3 hours;
        }

        //check the contract balance > swap amount threshold, then do it and distribute rewards to the reward pool, dev, markting wallet
        uint256 contractTokenBalance = balanceOf(address(this));
        bool canSwap = contractTokenBalance >= liquidateTokensAtAmount;
        if (canSwap) swapAndDistributeRewards(contractTokenBalance);
        ...
        if (takeFee) {
            ...
            uint256 priceImpactFee = getPriceImpactFee(amount.sub(rewardPoolAmount).sub(marketingAmount).sub(devAmount));
            uint256 sendAmount = amount.sub(rewardPoolAmount).sub(marketingAmount).sub(devAmount);

            //avoid stack problem
            sendAmount = sendAmount.sub(priceImpactFee);
    ...
    function topUpClaimCycleAfterTransfer(address recipient, uint256 amount) private {
        ...
        nextAvailableClaimDate[recipient] = rewardCycleBlock + additionalBlock;
    }
}