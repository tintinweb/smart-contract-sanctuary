...
contract NFTImplementationRoot is Initializable, ERC721Full, ERC721Mintable {
    using Counters for Counters.Counter;
    ...
    function setTokenMetadata(uint256 _tokenId, bytes memory _data) private {
    ...
    function mint(address user, uint256 _tokenId, bytes calldata metaData) external onlyMinter {
        _mint(user, _tokenId);

        setTokenMetadata(_tokenId, metaData);
    }
