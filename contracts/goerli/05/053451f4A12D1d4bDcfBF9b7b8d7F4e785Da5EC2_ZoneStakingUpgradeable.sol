...
contract ZoneStakingUpgradeable is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function getStakeInfo(
        address staker
    ) external view returns (uint256 stakedAmount, uint256 claimIn, uint256 rewardAmount, uint256 capacity) {
        Stake memory stake = stakes[staker];
        if (isStaked(staker)) {
            stakedAmount = stake.stakedAmount;
            (claimIn, rewardAmount) = _calcReward(stake.stakedTs, stake.stakedAmount, stake.typeIndex);
            return (stakedAmount, claimIn, rewardAmount, 0);
        }
        return (0, 0, 0, stakeLimit.sub(totalStakedAmount));
    }
