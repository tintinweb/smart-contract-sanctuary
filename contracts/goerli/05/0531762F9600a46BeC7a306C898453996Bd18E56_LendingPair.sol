...
contract LendingPair is ILendingPair, ReentrancyGuard, TransferHelper, ERC721Receivable {
  ...
  mapping (address => uint) public storedPrice;
  ...
  function operate(
    ...
    for (uint i = 0; i < _actions.length; i++) {
      ...
      // Consulting an oracle is expensive.
      // Cached prices are used only in checkAccountHealth
      // Liquidations are not affected by this and always use live prices
      else if (_actions[i] == 9) {
        updateStoredPrices();
      }
  ...
  function updateStoredPrices() public {
    (uint priceA, uint priceB) = lendingController.tokenPrices(tokenA, tokenB);
    storedPrice[tokenA] = priceA;
    storedPrice[tokenB] = priceB;
  }
  ...
  function supplyRatePerBlock(address _token) external view returns(uint) {
    ...
    return _interestRatePerBlock(_token) * utilizationRate(_token) * _lpRate(_token) / 100e18 / 100e18;
  }
  ...
  function checkAccountHealth(address _account) public view  {
    uint health = _accountHealth(_account, storedPrice[tokenA], storedPrice[tokenB]);
  ...
  function _liquidateAccount(
    ...
    updateStoredPrices();
    uint repayPrice = storedPrice[_repayToken];
    uint supplyPrice = storedPrice[supplyToken];
  ...
  function _unwrapUniPosition(address _account) internal {
    ...
    if (uniPosition[_account] > 0) {
      ...
      (uint amount0, uint amount1) = _positionAmounts(uniPosition[_account], storedPrice[tokenA], storedPrice[tokenB]);
