...
contract AlohaNFT is ERC721, AccessControl {
    ...
    address alohaNFTv1;
    mapping (uint256 => bool) migratedTokens;

    mapping (uint256 => uint256) private _tokenImages;
    mapping (uint256 => uint256) private _tokenBackgrounds;
    mapping (uint256 => uint256) private _tokenRootChainId;

    uint256 private _rarityVipModule    = 3;
    uint256 private _rarityUltraModule  = 2;
    uint256 private _rarityRareModule   = 1;
    
    uint256 private _lastRarityVipId    = 5; 
    uint256 private _lastRarityUltraId  = 4;
    uint256 private _lastRarityRareId   = 3;

    constructor(
        address _alohaNFTv1
    ) public ERC721("ALOHA NFT", "ANFT") {
        _setBaseURI("https://api.alohadefi.io/eth/");
    ...
    */
        ...
        returns (uint256)
    {
        ...
        uint256 newTokenId = _getNextTokenId(rarity);
    ...
    */
    function migrateToken(uint256 v1TokenId) public returns (uint256) {
        ...
        uint256 newTokenId = _getNextTokenId(v1TokenRarity);
    ...
    * @dev Ethereum => Matic.
    */
    function _transferToken(address wallet, uint256 tokenId) internal virtual returns (uint256) {
        uint256 transferringTokenRarity = _tokenRarity(tokenId);
        uint256 newTokenId = _getNextTokenId(transferringTokenRarity);
        
        _mint(wallet, newTokenId);
        _setRootChainId(newTokenId, tokenId);

        return newTokenId;
    }

    function _getNextTokenId(uint256 _tokenRarity) internal virtual returns (uint256) {
        uint256 nextIdForRarity;

        if (_tokenRarity == _rarityVipModule) {
            nextIdForRarity = _lastRarityVipId; 
            _lastRarityVipId += 3;
        } else if (_tokenRarity == _rarityUltraModule) {
            nextIdForRarity = _lastRarityUltraId;
            _lastRarityUltraId += 3;
        } else {
            nextIdForRarity = _lastRarityRareId;
            _lastRarityRareId += 3;
        }

        return nextIdForRarity;
    }
    ...
    function _setRootChainId(uint256 tokenId, uint256 _rootChainId) internal virtual {
        require(_exists(tokenId), "AlohaNFT: RootChainId set for nonexistent token");
        _tokenRootChainId[tokenId] = _rootChainId;
    }

    function _tokenRarity(uint256 tokenId) public view returns (uint256) {
        if (tokenId.mod(3) == 0) {
            return _rarityRareModule;
        } else if (tokenId.mod(3) == 1) {
            return _rarityUltraModule;
        }

        return _rarityVipModule;
    }
}