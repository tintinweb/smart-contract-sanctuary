...
contract Airdrop is Ownable {
  ...
  function addAirdrop(address[] memory _wallets, uint256[] memory _amounts) external onlyOwner{
    ...
    require(token.balanceOf( address(this) ) >= total, "You don't have enough balance for this airdrop");

    clearAllAirdrops();

    for (uint256 index = 0; index < _wallets.length; index++) {
      airdrop[_wallets[index]] = _amounts[index];
      wallets.push(_wallets[index]);
    }
  }
  ...
  function wallet(uint index) external view onlyOwner returns(address) {
