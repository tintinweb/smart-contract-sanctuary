...
contract HoneyHive is ERC721Enumerable, AccessControl {
    ...
    function mint(address _owner, uint256 _bearId) external onlyRole(GOVERNANCE_ROLE) {
        require(!_claimedByBearsIds[uint16(_bearId)], "Already minted");
        ...
        _claimedByBearsIds[uint16(_bearId)] = true;
    ...
    function eligibleToMint(uint256 _hiveId) external view returns (bool eligible) {
        return !_claimedByBearsIds[uint16(_hiveId)] && IERC721(bearsAddress).ownerOf(_hiveId) != address(0);
    }
