...
contract ChessBettor is Ownable {
    ...
    mapping(uint => Game) private existingGames;
    mapping(address => uint256[]) private allGamesPlayedByPlayers;
    mapping(address => uint256[]) private gamesWithUnclaimedBalancePerPlayer;
    ...
    function createGame(uint _gameId, uint _amount) internal {
       ...
       newGame.voteList.push(Vote(2, 0, true));
    ...
    function placeBet(uint _gameId, uint8 _vote, uint _amount) public payable {
       ...
       game.voteList.push(Vote(_vote, _amount, false));  // Index 0 will always be empty, implying that the player has not placed bets in the current game.
    ...
    function getActiveGames() public view returns(uint256[] memory) {
       uint256[] memory games = activeGameIds;
       return games;
    }
    ...
    function getGamesWithUnclaimedBalanceForPlayer(address _address) public view returns(uint256[] memory) {
        return gamesWithUnclaimedBalancePerPlayer[_address];
    }
    ...
    function getCountOfClaimableGamesForAddress(address _address, uint256[] memory unclaimedGameIds) internal view returns(uint256) {
        uint256 count = 0;
    ...
    function calculateRewards(address _address) public view returns(uint claimableReward, uint256[] memory gamesWithUnclaimedRewards) {
       ...
       uint256[] memory unclaimedGameIds = gamesWithUnclaimedBalancePerPlayer[_address];
       uint256[] memory removableGames = new uint256[](getCountOfClaimableGamesForAddress(_address, unclaimedGameIds));
       uint256 index = 0;
