...
contract ERC20 is Context, IERC20 {
     ...
     * called. However, we will be using 9.
     *
    ...
    // Transfer without tax, for burns.
    function _transferToExcluded(address sender, address recipient, uint256 amount) internal virtual {
        (uint256 tTransferAmount, uint256 tFee) = _getTValues(amount);
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
        _beforeTokenTransfer(sender, recipient, amount);
...
contract WrappeduPlexa is ERC20("Wrapped uPlexa", "wUPX"), Ownable {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (the Bridge in this case)
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
    }
    /// @notice Changes `_taxWallet` address to `_to`. Must only be called by the owner (the Bridge in this case)
