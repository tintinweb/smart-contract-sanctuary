...
contract Staking is Context, Ownable {
    ...
    function finishStake(
        ...
        emit StakeFinished(stakeIndex, account, withdrawn, unlockAt);
    }
     ...
     * @param unlocked Return only unlocked stakes
     *
     * @return List of stakes
     */
    function listStakes(bool unlocked) external view returns (Stake[] memory) {
        address sender = _msgSender();
        ...
        for (uint256 i = 0; i < lastStake; i += 1) {
            ...
            if (unlocked ? stake.withdrawn > 0 : stake.withdrawn == 0) {
                stakes[count] = stake;
