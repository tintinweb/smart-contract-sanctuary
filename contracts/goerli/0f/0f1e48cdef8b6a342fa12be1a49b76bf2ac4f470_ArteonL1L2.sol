...
contract ArteonL1L2 is Context, ERC165, EIP712Base, IERC1155, IERC1155MetadataURI, AccessControl, Pausable {
  ...
  string private _uri;
  uint256 public percentSettings = 2000; // = 2%
  ...
  function addToken(string memory name, uint256 decimals, uint256 _maxSupply) public virtual onlyRole(ADMIN_ROLE) {
    tokens.push(name);
  ...
  function _mintBatch(address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) internal virtual {
    ...
    for (uint256 i = 0; i < ids.length; i++) {
      uint256 id = ids[i];
      require(maxSupply[id] <= _totalSupply[id] + amounts[i], 'ARTEON: AMOUNT_EXCEEDS_SUPPLY');
  ...
  function _mint(address account, uint256 id, uint256 amount, bytes memory data) internal virtual {
    require(maxSupply[id] <= _totalSupply[id] + amount, 'ARTEON: AMOUNT_EXCEEDS_SUPPLY');
  ...
  function _beforeTokenTransfer(
    ...
    function buy(uint256 id, uint256 tokenId) external isListed(id, tokenId) {
      ...
      require(balance >= price, 'ArteonExchange: NOT_ENOUGH_TOKENS');

      safeTransferFrom(account, owner, 0, price, "");
      safeTransferFrom(owner, account, id, tokenId, "");
      _removeListing(id, tokenId, account);
    ...
    function burnPercentage(uint256 value) public view returns (uint256 percentage)  {
      unchecked {
        percentage = value + percentSettings;
        percentage -= 1;
        percentage = (percentage - percentSettings) * percentSettings;

        percentage = (percentage *  percentSettings) / 100000;
      }
      return percentage;
   }
    ...
    function unpause() internal virtual whenPaused onlyRole(ADMIN_ROLE) {
      super._unpause();
    }
}