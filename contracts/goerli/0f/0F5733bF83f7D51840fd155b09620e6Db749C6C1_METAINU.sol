...
contract METAINU is Context, IERC20, Ownable {
    ...
    string private _name = "METAINU.Finance";
    string private _symbol = "METAINU";
    uint8 private _decimals = 9;

    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 10**16 * 10**_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    address payable public _devWalletAddress  = payable(0xcBe21b85757d0B6915e64E34930771e1056DDcF5);
    address payable public _burnAddress  = payable(0x000000000000000000000000000000000000dEaD);


    uint256 public _devFee = 5;
    uint256 public _liqFee = 5;
    ...
    uint256 public _liquidityFee = _liqFee.add(_devFee).add(_BurnFee) ;   //  Dev + Burn + Liquidity
    ...
    uint256 public _maxHoldings = _maxTxAmount.mul(5);
    ...
    event TaxesSet(bool setT);
    event TaxesSetToZero(bool setT);
    ...
    function airdrop(address recipient, uint256 amount) external onlyOwner() {
        removeAllFee();
        _transfer(_msgSender(), recipient, amount * _decimals);
        restoreAllFee();
    }
    ...
    function airdropArray(address[] calldata newholders, uint256[] calldata amounts) external onlyOwner(){
        ...
        while(iterator < newholders.length){
            airdropInternal(newholders[iterator], amounts[iterator] * _decimals);
            iterator += 1;
        }
    }

// surprise
    function distributeToAllHolders(uint256 tAmount) public {
    ...
    function setCombinedFeePercent(uint256  _devFeeN ,uint256  _liqFeeN,  uint256 _BurnFeeN) external onlyOwner() {
        _devFee = _devFeeN;
        _liqFee = _liqFeeN;
        _BurnFee = _BurnFeeN;
        _liquidityFee = _liqFeeN.add(_devFeeN).add(_BurnFeeN) ;   //  Reflection + Dev + Burn + Liquidity
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        _maxTxAmount = maxTxAmount * _decimals;
    }

    function setMaxHoldings(uint256 maxHoldings) external onlyOwner() {
        _maxHoldings = maxHoldings * _decimals;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        // here we take dev, burn, and liquify

        uint256 BNBbalance = address(this).balance;
        uint256 devTokens = contractTokenBalance.mul(_devFee).div(_liquidityFee);
        uint256 liqTokens = contractTokenBalance.mul(_liqFee).div(_liquidityFee);
        uint256 burnTokens = contractTokenBalance.mul(_BurnFee).div(_liquidityFee);
        swapTokensForEth(devTokens);
        uint256 devNewBalance = address(this).balance.sub(BNBbalance);
        TransferDevETH(_devWalletAddress,devNewBalance);

        // burn what need to be burned
        uint256 currentRate =  _getRate();
        uint256 rBurn = burnTokens.mul(currentRate);
        _rOwned[_burnAddress] = _rOwned[_burnAddress].add(rBurn);
        if(_isExcluded[_burnAddress])
            _tOwned[_burnAddress] = _tOwned[_burnAddress].add(burnTokens);

        // split the contract balance into halves
        uint256 half = liqTokens.div(2);
        uint256 otherHalf = liqTokens.sub(half);
    ...
    function SetTokensSellToAddToLiquidity(uint256 amount) public onlyOwner  {
    ...
    function ResetTaxes () public onlyOwner  {
        ...
        _taxFee = 3;
        _previousTaxFee = _taxFee;
        _liquidityFee = _liqFee.add(_devFee).add(_BurnFee) ;
        _previousLiquidityFee = _liquidityFee;
        emit TaxesSet(true);
    }
    function ZeroTaxes () public onlyOwner  {
