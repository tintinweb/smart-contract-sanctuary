...
/*
 * Crypto stamp 3.0 On-Chain Shop
 * Ability to purchase digital-physical collectible postage stamps and to
 * redeem Crypto stamp 3.0 pre-sale vouchers in a similar manner
 *
...
// File: contracts/CS3OnChainShop.sol

/*
 * Implements ERC 721 NFT standard: https://github.com/ethereum/EIPs/issues/721.
 */

contract CS3OnChainShop {
    using Address for address payable;

    BridgeDataI public bridgeData;

    IERC721Enumerable internal CS3;
    CS3PresaleRedeemI internal CS3Presale;
    ...
    address[2] public tokenPools; // Pool 0 is "normal" OCS, 1 is presale.
    ...
    event ShopClosed();
    event AssetSold(address indexed buyer, address recipient, bool indexed presale, uint256 indexed tokenId, uint256 priceWei);

    constructor(address _bridgeDataAddress, address _shippingManagerAddress, address _CS3Address, address _CS3PresaleAddress, uint256 _priceEurCent, address[] memory _tokenPools)
    {
        ...
        CS3 = IERC721Enumerable(_CS3Address);
        require(address(CS3) != address(0x0), "You need to provide an actual Cryptostamp 3 contract.");
        CS3Presale = CS3PresaleRedeemI(_CS3PresaleAddress);
        require(address(CS3Presale) != address(0x0), "You need to provide an actual Cryptostamp 3 Presale contract.");
    ...
    function getPool(bool _presale)
    public view
    returns (address)
    {
        if (_presale) {
    ...
    // Returns the amount of assets that are still available for sale.
    function availableForSale(bool _presale)
    public view
    returns (uint256)
    {
        return CS3.balanceOf(getPool(_presale));
    }

    // Returns true if the asset of the given type is sold out.
    function isSoldOut(bool _presale)
    public view
    returns (bool)
    {
        return availableForSale(_presale) == 0;
    }

    // Buy assets. The number of assets is determined from the amount of ETH sent.
    function acceptAndBuy(bool _acceptTermsCryptoPostAt, string calldata _acceptanceText)
    external payable
    requireOpen
    {
        buyFor(msg.sender, _acceptTermsCryptoPostAt, _acceptanceText);
    }

    // Buy assets. The number of assets is determined from the amount of ETH sent.
    function buyFor(address _recipient, bool _acceptTermsCryptoPostAt, string calldata _acceptanceText)
    public payable
    requireOpen
    {
        ...
        address pool = getPool(isPresale);
        uint256 availAmount = CS3.balanceOf(pool);
        ...
        for (uint256 i = 0; i < assetCount; i++) {
            ...
            uint256 tokenId = CS3.tokenOfOwnerByIndex(pool, availAmount - i - 1);
            CS3.safeTransferFrom(pool, _recipient, tokenId);
            emit AssetSold(msg.sender, _recipient, isPresale, tokenId, curPriceWei);
            shippingManager.setShippingStatus(address(CS3), tokenId, ShippingManagerI.ShippingStatus.Sold);
        }
    ...
    function redeemVoucher(uint256 _amount, address _recipient, bool _acceptTermsCryptoPostAt, string calldata _acceptanceText)
    public
    requireOpen
    {
        require(_acceptTermsCryptoPostAt, "You need to accept the terms.");
        require(bytes(_acceptanceText).length > 0, "You need to send the acceptance text.");
        bool isPresale = true;
        uint256 CS3PresaleId = 0;
        address pool = getPool(isPresale);
        uint256 availAmount = CS3.balanceOf(pool);
        require(_amount <= availAmount, "Not enough assets available to buy that amount.");
        require(CS3Presale.balanceOf(msg.sender, CS3PresaleId) >= _amount, "You need to own enough presale vouchers to redeem the specified amount.");
        // Redeem the vouchers.
        uint256[] memory redeemTypes = new uint256[](1);
        uint256[] memory redeemAmounts = new uint256[](1);
        redeemTypes[0] = CS3PresaleId;
        redeemAmounts[0] = _amount;
        CS3Presale.redeemBatch(msg.sender, redeemTypes, redeemAmounts);
        for (uint256 i = 0; i < _amount; i++) {
            // Find the next stamp (last owned by pool) and transfer it.
            uint256 tokenId = CS3.tokenOfOwnerByIndex(pool, availAmount - i - 1);
            CS3.safeTransferFrom(pool, _recipient, tokenId);
            emit AssetSold(msg.sender, _recipient, isPresale, tokenId, 0);
            shippingManager.setShippingStatus(address(CS3), tokenId, ShippingManagerI.ShippingStatus.Sold);
        }
    }
