...
contract United {
    ...
    function respondToProposal(Proposal response, string calldata ens1, string calldata ens2) external {
        ...
        //if declined cancel and reset proposal
         if(uint8(response) == 2){
            ...
            acceptOrDecline.proposalStatus = uint(response);
   ...
   function getUnited( address _from , address _to, string calldata ens1, string calldata ens2)  internal {
    ...
    function getTokenID(address _add) external view returns (uint){
        (uint id) = IUnionNFT(nftContract).getTokenIdByAddress(_add);
        return id;
    }
...
interface IUnionNFT {
    ...
    function getTokenIdByAddress(address _add) external view returns(uint256);
 }