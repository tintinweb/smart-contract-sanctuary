...
contract ProjectCollection is IProjectCollection, ERC721, Ownable {
    ...
    function updateController(uint256 tokenId, address _controller) external {
        require(msg.sender==ownerOf(tokenId), "Error: Caller is not the owner");
        projects[tokenId].controller = _controller;
    }

    function mapStorageMethod(string[] memory methodology, string[] memory storageMethod) external onlyOwner {
    ...
    function updateProjectId(uint256 tokenId, string memory newProjectId) external onlyOwner {
        require(_exists(tokenId), "Project not yet minted");
    ...
    function removeProject(uint256 tokenId) external onlyOwner {
        delete projects[tokenId];
    }

    // Retrieve all data from ProjectNFT struct
    // Currently  not used
    function getProjectDataByTokenId(uint256 tokenId) external view
        returns (
            string memory,
            string memory,
            string memory,
        ...
        function getProjectDataByProjectId(string memory projectId) external view
