...
contract NFTAirdrop is Ownable, Freezable {
  ...
  mapping(string => mapping(string => Drop)) public imageOfRole;
  mapping(address => mapping(string => mapping(string => Claim))) public claims;
  ...
  /// @param _roles An array of structs from role Ids and the ifps hash of their images.
  function newAirdrop(
    ...
    for (uint256 i = 0; i < _roles.length; i++) {
      if (
        bytes(imageOfRole[_serverId][_roles[i].roleId].tokenImageHash).length ==
        0
      ) {
        Drop memory drop;
        drop.active = true;
        drop.tokenImageHash = _roles[i].tokenImageHash;
        imageOfRole[_serverId][_roles[i].roleId] = drop;
      }
    }
  }
  ...
  function stopAirdrop(
    ...
    for (uint256 i = 0; i < _roleIds.length; i++) {
      imageOfRole[_serverId][_roleIds[i]].active = false;
    }
  }
  ...
  function claim(
    ...
    require(
      imageOfRole[_serverId][_roleId].active || userClaim.approved,
      "No active airdrop"
    );
    require(!userClaim.claimed, "Already claimed");
    require(
      bytes(imageOfRole[_serverId][_roleId].tokenImageHash).length > 0,
    ...
    claims[msg.sender][_serverId][_roleId].claimed = true;
    ...
    DCRoleToken(roleToken).safeMint(
      ...
      _serverId,
      _roleId,
      imageOfRole[_serverId][_roleId].tokenImageHash
    );
  }
  ...
  function grant(
    ...
    claims[msg.sender][_serverId][_roleId].approved = true;
  }
}
