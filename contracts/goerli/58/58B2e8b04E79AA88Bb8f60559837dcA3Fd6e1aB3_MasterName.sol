...
//import './iStorageName.sol';
interface iStorageName {
...
contract MasterName is AccessControl {
    ...
    modifier onlyMaster() {
        require(hasRole(MASTER_ROLE, _msgSender()), "MasterName: not in MASTER_ROLE");
        _;
    }
    ...
    function addNameBySC (address scOwner, address scNameAddress, string memory name) external onlyMaster returns (bool) {
    ...
    function _addName (address scOwner, address scNameAddress,  string memory name) private returns (bool) {
        require (!existsOwner(scOwner), "MasterName: owner exists");
        require (!existsScName(scNameAddress), "MasterName: scName exists");
        
        iStorageName scName = iStorageName(scNameAddress);
        require (compareStrings(scName.getName(), name), "MasterName: different name");
        
        //Only owner's sc can add
        require (scName.owner() == scOwner, "MasterName: only sc owner");
    ...
    function deleteNameBySC (address scOwner) external onlyMaster returns (bool) {
        _deleteName (scOwner);
        return true;        
    }
    
    function _deleteName (address scOwner) private returns (bool) {
        require (existsOwner(scOwner), "MasterName: owner not exists");
    ...
    function getOwnerSC (address scAddress) public returns (address) {
        iStorageName scName = iStorageName(scAddress);
        return scName.owner();
    }
    
    function getNameByAddress (address scAddress) public view returns (string memory) {
        iStorageName scName = iStorageName(scAddress);
        return scName.getName();
    }

    function getNameByOwner (address account) public view returns (string memory) {
        return nameInfo[ownerIndex[account]-1].name;
    }
    
    function checkName (address scNameAddress, string memory name) public view returns (bool) {
        //Name stored in scName equal name_
        if (compareStrings(getNameByAddress(scNameAddress), name)) {
            return true;
        }
        return false;
    }

    function getNameInfoByAddress (address scNameAddress) public view returns (NameStruct memory) {
        return nameInfo[scNameIndex[scNameAddress]-1];
    }
    
    function getNameInfoByOwner (address account) public view returns (NameStruct memory) {
        return nameInfo[ownerIndex[account]-1];
    }
    ...
    NameStruct[] private namesList;
    function listNameInfo (uint256 from, uint256 to) public returns (NameStruct[] memory) {
        delete namesList;

        for(uint256 i=from; i<=to; i++) {
            namesList.push(nameInfo[i-1]);
        }
        return namesList;
    }
