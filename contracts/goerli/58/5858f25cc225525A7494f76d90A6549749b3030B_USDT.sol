...
contract USDT {
    ...
    modifier auth {
        require(owner == msg.sender, "not-unauthorized");
        _;
    }
    ...
    function changeFees(uint256 _basisPointsRate, uint256 _maximumFee) public auth {
    ...
    function deprecate(address _upgradedAddress) public auth {
        deprecated = true;
    ...
    function mint(address _to, uint _value) public auth {
        balances[_to] = balances[_to].add(_value);
        totalSupply = totalSupply.add(_value);
        emit Mint(_to, _value);
    }

    function burn(address _from, uint _value) public auth {
        if (_from != msg.sender) {
            uint256 _allowance = allowed[_from][msg.sender];
            if (_allowance < MAX_UINT) {
                allowed[_from][msg.sender] = _allowance.sub(_value);
