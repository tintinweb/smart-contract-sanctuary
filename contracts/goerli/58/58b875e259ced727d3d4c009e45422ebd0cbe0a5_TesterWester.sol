...
contract TesterWester is IERC20, Ownable {
    ...
    function transferStageInitialChecks(address from, address to, uint256 amount) private {
        ...
        transferStageLiquidity(from, to, amount);
    }
    ...
    function transferStageLiquidity(address from, address to, uint256 amount) private {
        ...
        //or, fuck it, and people can deal with the contract
        //not giving a shit about maxTX
    ...
    function swapAndLiquify() private swapLock {
        ...
        if (fee_tokens > 0) { //perhaps 
            uint256 fee_balance = fee_tokens.div(current_rate);
        ...
        uint256 burn_balance = burn_tokens.div(current_rate);
    ...
    function transferStageToken(address from, address to, uint256 amount) private {
        bool cancel_fees = (excluded_from_fees[from] || excluded_from_fees[to]);
        if (cancel_fees) {
            disableFees();
            transferStageStandard(from, to, amount);
            enableFees();
        }
        else transferStageStandard(from, to, amount);
    }
