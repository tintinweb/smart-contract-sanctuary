...
contract FNDCollectionFactory is ICollectionFactory {
   ...
   * @dev Used by the collections for a centralized operator definition.
   */
   ...
   * @dev Used by the collections to safely call another contract with arbitrary callData.
   */
  IProxyCall public proxyCallContract;

  /**
   * @notice The implementation version new collections will use.
   * @dev This is auto-incremented anytime the implementation is changed.
   */
  ...
  constructor(address _proxyCallContract, address _adminRoleContract) {
   ...
   * @dev The symbol is required and must be unique for the msg.sender + implementation version,
  ...
  function createCollection(string calldata name, string calldata symbol)
    ...
    returns (address)
  {
    // If the NFT was previously created using this implementation version + msg.sender + symbol, this will revert
  ...
  /**
   * @notice Returns the address of a collection given the current implementation version, creator, and symbol.
  ...
  /**
   * @dev The symbol is case sensitive when determining the salt for the create2 call.
   */
