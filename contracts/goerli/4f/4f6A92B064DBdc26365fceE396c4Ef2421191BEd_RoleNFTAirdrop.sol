...
import "./interfaces/IRoleToken.sol";
import "./interfaces/IRoleTokenFactory.sol";

contract RoleNFTAirdrop is Ownable, Pausable {
    ...
    mapping(uint256 => string) public urlById;
    mapping(string => Drop) private dropsByUrl;
    // User -> Server ID -> Role ID -> TokenAddress
    ...
    // Server ID -> Role ID -> TokenAddress
    ...
    event NewAirdrop(string dropUrl, string channelId);
    ...
    function newAirdrop(
        ...
        // Check the signature
        bytes32 message = keccak256(abi.encode(address(this), _serverId, _dropUrl, msg.sender))
        ...
        require(IDCRoleToken(roleToken).owner() == address(this), "Token owner is not the contract");
        ...
        urlById[numOfDrops] = _dropUrl;
        dropsByUrl[_dropUrl] = drop;

        emit NewAirdrop(_dropUrl, _channelId);
    ...
    function stopAirdrop(
        ...
        string memory serverId = drop.serverId;
        address roleToken = contractsByDeployer[msg.sender][drop.contractId];
        require(claimables[serverId][_roleId][roleToken].active, "No active airdrop");

        // Check the signature
        bytes32 message = keccak256(abi.encode(address(this), serverId, _roleId, roleToken, msg.sender, "stop"))
            .toEthSignedMessageHash();
        require(message.recover(_signature) == bot, "Not valid signature");

        claimables[serverId][_roleId][roleToken].active = false;
        drop.numOfActive -= 1;
    }
    ...
    function claim(
        ...
        require(
            claimables[_serverId][_roleId][_tokenAddress].active ||
                approvals[msg.sender][_serverId][_roleId][_tokenAddress],
            "No active airdrop"
        );
        require(!claims[_userId][_serverId][_roleId][_tokenAddress], "Already claimed");
        require(IDCRoleToken(_tokenAddress).owner() == address(this), "Token owner is not the contract");

        // Check the signature
        bytes32 message = keccak256(abi.encode(address(this), _serverId, _roleId, _tokenAddress, _userId, msg.sender))
            .toEthSignedMessageHash();
        require(message.recover(_signature) == bot, "Not valid signature");

        claims[_userId][_serverId][_roleId][_tokenAddress] = true;
    ...
    function grant(
        ...
        approvals[_user][_serverId][_roleId][roleToken] = true;
    ...
    {
        Drop storage drop = dropsByUrl[_dropUrl];
        return (drop.dropName, drop.serverId, drop.roleIds, drop.tokenAddress, drop.contractId, drop.numOfActive);
    }
    ...
    function transferTokenOwnership(address _to, uint256 _id) external {
        ...
        IDCRoleToken(tokenAddress).transferOwnership(_to);
    }
}
