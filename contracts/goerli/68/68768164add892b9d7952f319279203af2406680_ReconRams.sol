...
contract ReconRams is ERC721P, Ownable, PaymentSplitter {
    ...
    uint256 public constant GENERAL_MAX_SUPPLY = 200;
    ...
    modifier mintCheck(uint256 _mintAmount) {
        ...
        require(supply.current() + _mintAmount <= GENERAL_MAX_SUPPLY, "Mint amt exceeds max supply");
    ...
    function teamReserveMint(uint256 _mintAmount) public onlyOwner {
        require(supply.current() + _mintAmount <= MAX_SUPPLY, "Mint amt exceeds max supply");
        _doMint(msg.sender, _mintAmount);
    }
