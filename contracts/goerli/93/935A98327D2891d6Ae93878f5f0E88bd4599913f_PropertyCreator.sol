...
contract PropertyCreator {
    ...
    event PropertyCreated(string lot, string location, address owner);
    ...
    */
    function createProperty(string memory lot, string memory location, address owner) public returns (address propertyAddress){
        ...
        properties.push(p);
        emit PropertyCreated(lot, location, owner);
        return address(p);
    }
}
