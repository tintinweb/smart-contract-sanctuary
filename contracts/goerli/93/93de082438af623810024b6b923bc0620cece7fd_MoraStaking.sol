...
contract MoraStaking{
  using SafeMath for uint;
    ...
    event evNewStake(address _staker, uint _stakeID, uint256 _amount, uint _stakeDate);
    event evNewunstake(address _staker, uint _stakeID, uint _amount, uint _reward, uint256 _claimedAmount, uint _unstakeTime);
  ...
  function Stake(uint256 _amo) public returns(bool success) {
    ...
    uint _rewardRate;
    if ( _amount >= 2000 * (10**18) && _amount < 7000 * (10**18) ) { _rewardRate = 58;} // 0.0058 / 50
    if (_amount >= 7000 * (10**18) && _amount < 16000 * (10**18) ) { _rewardRate = 80;} //0,0080  / 70
    if (_amount >= 16000 * (10**18)) { _rewardRate = 102;} // 0.0102 / 90
    ...
    emit evNewStake(_staker, _stakeID, _amount, _stakeDate);
    return true;
  }
  ...
  function UnstakeAndClaim(uint _stakeID) public returns (bool result) {
    ...
    require(_stakePeriodInHour >= 1,"Can't unstake before 1 hour"); //1week 168 in hour
    uint _amount = stakeBoxs[_stakeID].amount;
    uint _reward = ((_stakePeriodInHour * stakeBoxs[_stakeID].rewardRate) * _amount / 100) / (10**3);
    ...
    emit evNewunstake(_staker, _stakeID, _amount, _reward, _claimedAmount, block.timestamp);
    return true;
  }

    //Burn function will be called by owner montly...
    function MontlyBurn () onlyOwner external returns (uint result) {
    uint _distrubutedReward = 0;
    uint _lastBurn = burnDates.length.sub(1);
    uint _lastBurnDate = burnDates[_lastBurn];
    uint _burnAmount;
    if (block.timestamp - _lastBurnDate >=  43200) { ////for 30 days = 2592000
      ...
      uint _remainingTime = block.timestamp.sub(_lastBurnDate);
      return (_remainingTime / 3600); //in  hour
  }
