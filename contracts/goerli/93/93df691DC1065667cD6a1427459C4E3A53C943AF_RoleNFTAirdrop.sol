...
contract RoleNFTAirdrop is RoleDropBase {
    ...
    event DropOnServer(string indexed serverId, string dropUrl);
    ...
    function newAirdrop(
        ...
        for (uint256 i = 0; i < _roles.length; i++) {
            if (!claimables[_serverId][_roles[i].roleId][roleToken].dropped) {
                ...
                IRoleToken(roleToken).addRoleData(
                    _serverId,
                    _roles[i].roleId,
        ...
        emit DropOnServer(_serverId, _dropUrl);
    }
    ...
    function claim(
        ...
        IRoleToken(_tokenAddress).safeMint(msg.sender, _serverId, _roleId);
