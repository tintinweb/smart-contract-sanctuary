...
contract GangFactory {
    ...
    mapping(address => address[]) gangYellowPages; // gangToken => gangmembers
    // somehow figure out "get main nft from this member nft", function in the member?

    uint private _gangIdCounter;

    constructor (address _gangBase, address _memberBase) {
        gangBase = _gangBase;
    ...
    function createGang(string memory name) external returns (address) {
        ...
        address newGang = Clones.clone(gangBase);
        gangYellowPages[newGang] = owners;

        Gang(newGang).initialize(name, _gangIdCounter, owners, msg.sender);  
...
contract Gang is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable  {
    ...
    function initialize(string memory name, uint _gangNumber, address[] calldata _owners, address _multisig) public initializer {
    ...
    function mint(address to) public {
        require(!minted, "gang already exists baby");
        _mint(to, 69);
        minted = true;
    }
...
contract GangMembers is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable  { 
    ...
    function mint(address to) public returns (uint) {
        require(_memberId <= maxMembers, "gang is full, join another one!");
        uint currentId;
        _mint(to, _memberId);
