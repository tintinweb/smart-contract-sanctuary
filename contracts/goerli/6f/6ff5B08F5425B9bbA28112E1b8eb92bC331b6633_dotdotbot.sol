...
contract dotdotbot is ERC721TokenReceiver {
    ...
    function onERC721Received(address _operator, address _from, uint256 _tokenId, bytes calldata _data) external override returns(bytes4) {
        ownedTokens.push(_tokenId); // add our owned token id
    ...
    function transferToOwner() public onlyOwner {
        ...
        for (uint i = 0; i < ownedTokens.length; i++) {
            ERC721Stripped(_dotdotdotContract).safeTransferFrom(address(this), _owner, ownedTokens[i]);
        }
    }
