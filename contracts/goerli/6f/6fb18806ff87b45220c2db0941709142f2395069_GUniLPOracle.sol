...
// hevm: flattened sources of src/GUniLPOracle.sol
...
contract GUniLPOracleFactory {
    ...
    function build(
        ...
        address tok0 = GUNILike_1(_src).token0();
        address tok1 = GUNILike_1(_src).token1();
...
contract GUniLPOracle {
    ...
    constructor (address _src, bytes32 _wat, address _orb0, address _orb1) public {
        ...
        src  = _src;
        wat  = _wat;
        uint256 dec0 = uint256(ERC20Like_1(GUNILike_1(_src).token0()).decimals());
        require(dec0 <= 18, "GUniLPOracle/token0-dec-gt-18");
        UNIT_0 = 10 ** dec0;
        TO_18_DEC_0 = 10 ** (18 - dec0);
        uint256 dec1 = uint256(ERC20Like_1(GUNILike_1(_src).token1()).decimals());
    ...
    function seek() internal returns (uint128 quote) {
        ...
        uint256 p0 = OracleLike_1(orb0).read();  // Query token0 price from oracle (WAD)
        require(p0 != 0, "GUniLPOracle/invalid-oracle-0-price");
        uint256 p1 = OracleLike_1(orb1).read();  // Query token1 price from oracle (WAD)
        ...
        (uint256 r0, uint256 r1) = GUNILike_1(src).getUnderlyingBalancesAtPrice(sqrtPriceX96);
        require(r0 > 0 || r1 > 0, "GUniLPOracle/invalid-balances");
        uint256 totalSupply = ERC20Like_1(src).totalSupply();
