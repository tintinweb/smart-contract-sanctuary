...
contract Hive is IHive, Ownable, IERC721Receiver, Pausable {
    ...
    event HiveRestarted(uint256 hiveId);
    ...
    uint256 public constant DAILY_HONEY_RATE = 400;
    ...
    // function computeProduction(uint256 dailyProduction, uint256 hiveAge) internal view returns (uint256 owed)  {
    //     ufixed8x1 dayGrow = 12;
    //     uint256 hourProduction = dailyProduction / 24;
    //     uint256 hourGrow = dayGrow**(1 / 24);
    //     uint256 hoursHive = (block.timestamp - hiveAge) / 3600;
    //     uint256 hiveTotal = 132 * (hourProduction * (hourGrow ** hoursHive)) - 2186;
    //     return hiveTotal;
    // }
    ...
    function getHiveProtectionKeepers(uint256 hiveId) external view returns (uint256 hiveProtectionKeepers) {
        hiveProtectionKeepers = hives[hiveId].hiveProtectionKeepers;
    }
    ...
    function setKeeperAttackData(
        ...
        hives[hiveId].hiveProtectionKeepers = protection;
    }
    ...
    function hiveTimestamps(uint256 hiveId) internal view returns (string memory) {
        return
            string(
                abi.encodePacked(
                    ...
                    ",",
                    uint32(hives[hiveId].hiveProtectionKeepers).toString(),
                    ",",
