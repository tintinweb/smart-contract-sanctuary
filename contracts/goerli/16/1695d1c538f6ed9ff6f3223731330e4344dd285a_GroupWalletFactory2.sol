...
contract GroupWalletFactory2 {
    ...
    function replicate_group_l9Y(bytes32[] calldata _m, bytes calldata data32, bytes32[] calldata _mem) external payable {
      uint256 v = uint256(_m[2])&k_rentMask;
      
      my_require(_m.length==4&&msg.value>0&&v>0&&msg.value>v,reportValue(msg.value," replicate funding")); // ok
      my_require(address( uint160( commitments[ _m[0] ] & k_aMask ) )==msg.sender,"initiator!");           // ok
