...
contract FloorToken is IFloorToken, Ownable, ERC721Checkpointable, FxBaseRootTunnel {
    ...
    constructor(
        ...
        ERC721('FLOOR', 'FLOOR')
        FxBaseRootTunnel(0x2890bA17EfE978480615e330ecB65333b880928e, 0x3d1d3E34f7fB6D26245E6640E1c50710eFFf15bA)
    {
     ...
     */
    function generateSize(uint256 floorId) private view returns(uint8) {
        uint256 x = randNumber(floorId);
        if (x < 100)    return 128;
        if (x < 300)    return 64;
        if (x < 1300)   return 32;
        if (x < 7900)   return 16;
        if (x < 10000)  return 8; // <- 21% because otherwise it wouldn't fit 100%
     ...
     */
    function setNoundersDAO(address _noundersDAO) override external onlyNoundersDAO {
        noundersDAO = _noundersDAO;
     ...
     * @dev This cannot be reversed and is only callable by the owner when not locked.
     */
    function lockDescriptor() override external onlyOwner whenDescriptorNotLocked {
     ...
     * @dev This cannot be reversed and is only callable by the owner when not locked.
     */
    function lockSeeder() override external onlyOwner whenSeederNotLocked {
        isSeederLocked = true;
    ...
    /**
     * @notice Mint a Noun with `floorId` to the provided `to` address.
     */
    function _mintTo(address to, uint256 floorId) internal returns (uint256) {
        ...
        sendMessageToChild(abi.encode(floorId, sizes[floorId]));
