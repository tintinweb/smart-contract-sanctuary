...
contract GoldenCake is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    uint256 public rewardCycleInterval = 1 days;
    ...
    uint256 public _cakeBankFee = 200;
    ...
    constructor(address _cake) public {
        ...
        marketingWallet = address(new Wallet());
        tokenPool = address(new Wallet());
        cakeBank = address(new Wallet());
    ...
    function collectFee(
        ...
        //@dev liquidity fee
        if (_liquidityFee != 0) {
            uint256 liquidityFee = amount.mul(sell ? _liquidityFee.mul(2) : _liquidityFee).div(10**(_feeDecimal + 2));
        ...
        //@dev marketing fee
        if (_marketingFee != 0) {
            uint256 fee = amount.mul(sell ? _marketingFee.mul(2) : _marketingFee).div(10**(_feeDecimal + 2));
        ...
        //@dev cakebank fee
        if (_cakeBankFee != 0) {
            uint256 fee = amount.mul(_cakeBankFee).div(10**(_feeDecimal + 2));
            transferAmount = transferAmount.sub(fee);
        ...
        //@dev cakebank fee
        if (_taxFee != 0) {
            uint256 fee = amount.mul(_taxFee).div(10**(_feeDecimal + 2));
            transferAmount = transferAmount.sub(fee);
    ...
    function swap() private lockTheSwap {
        ...
        if (swapAndLiquifyEnabled && liquidityTokenBalance >= minTokensBeforeSwap) {
            ...
            if (cakeBankBalance > 0) {
                ...
                swapTokensForEth(cakeBankBalance, cakeBank);
            }
            if (marketingWalletBalance > 0) {
                ...
                swapTokensForEth(marketingWalletBalance, marketingWallet);
            }
        }
    }
    ...
    function setCakeBankFee(uint256 fee) external onlyOwner {
        _cakeBankFee = fee;
    }
