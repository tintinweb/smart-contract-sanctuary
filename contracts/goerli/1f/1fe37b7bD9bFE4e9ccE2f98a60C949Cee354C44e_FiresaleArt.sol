    ...
    IERC721Receiver
{
    ...
    // single deposit ERC721 token using the built in ERC721Receiver.safeTranserFrom
    // does not require setApprovalForAll
    ...
    // batch deposit ERC721 tokens from possibly multiple contracts
    // requires that setApprovalForAll has been called on all contracts
        ...
        returns (bytes4)
    {
        ...
        for (uint256 i = 0; i < _bundles.length; i++) {
            if (whitelistActive) {
                require(
                    _verify(_leaf(msg.sender), _bundles[i].proof),
                    "Not a whitelisted contract"
                );
            }
...
contract FiresaleArtV2 is FiresaleArt {
    function setTreasury(address treasury) public override onlyOwner {
        require(false, "Treasury can not be updated");
...
contract LITTokenV2 is LITToken {
