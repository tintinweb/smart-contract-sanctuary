...
contract SimpleSale is ReentrancyGuard, Ownable {
    ...
    address payable private fundingAddress;
    ...
    uint256 public fixBuyPerWallet = 200000000000000000; // 0.2 ETH
    ...
    receive() external payable {
        require(msg.value == fixBuyPerWallet, "SimpleSale: You must send the required amount");
        require(generalParticipants[msg.sender] == 0, "SimpleSale: You already participated");
        require(getUserWhitelistStatus(msg.sender), "SimpleSale: You are not whitelisted");
        ...
        hasntStopped()
    {
        ...
        emit Deposit(_recepient, _value);
    ...
    function start(
        ...
        hasntStopped()
    {
        require(_fundingAddress != address(0), "SimpleSale: FundingAddress can't be 0x0");

        fundingAddress = _fundingAddress;
        started = true;
    }

    function editWhitelist(address[] calldata _users, bool _add) external onlyOwner() {
        if (_add) {
    ...
    modifier hasBeenStopped() {
        require(stopped, 'SimpleSale: Sale is not stopped');
        _;
    }

    modifier hasntStopped() {
        require(!stopped, 'SimpleSale: Sale is stopped');
        _;
    }

    modifier hasBeenStarted() {
        require(started, 'SimpleSale: Sale is not started');
        _;
    }

    modifier hasntStarted() {
        require(!started, 'SimpleSale: Sale is started');
        _;
    }
}