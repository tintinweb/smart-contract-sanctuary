...
contract GStakingManager is AccessControl {
    ...
    uint256 public BRONZE_RATE = 10;
    uint256 public SLIVER_RATE = 30;
    uint256 public GOLD = 60;
    ...
    struct Pool {
        ...
        uint256 openTime;
        uint256 closeTime;
        uint256 status; // 0:upcomming , 1: live , 2 :close
    ...
    // getBalance
    function getBalance(address _sender) internal returns (uint256) {
        StakerInfo memory staker = stakers[_sender];
        ...
        uint256 amountIn = gpoolToken.balanceOf(_sender);
    ...
    // update pool status
    function updatePoolStatus(uint256 _poolId, uint256 _status) internal {
    ...
    function updatePoolReward(uint256 _poolId, uint256 _amountReward) internal {
        pools[_poolId].totalReward = _amountReward;
        emit UpdatePoolRewardEvent(_poolId, _amountReward);
    }

    // function getTier
    function getTier(address _sender) public {}
