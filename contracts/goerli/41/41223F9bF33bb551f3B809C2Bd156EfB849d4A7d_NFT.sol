...
// File: @maticnetwork/pos-portal/contracts/child/ChildToken/ChildERC1155.sol
...
contract NFT is Ownable, ERC1155 {
  ...
  constructor(
    ...
    for (uint256 i = 0; i < _initialIds.length; i++) {
      _mint(msg.sender, _initialIds[i], _initialAmounts[i], "");
    }

    // Mint the initial NFTs that use the fallback _uri.
    // _mintBatch(msg.sender, _initialIds, _initialAmounts, "");
  }

  /** [owner] Mint a new token. Once minted, no more of that token can be minted. */
  function mint(
    uint256 _id,
    string memory _uri,
    uint256 _amount
  ) public onlyOwner {
    require(!mintedMap[_id], "TOKEN_ID_ALREADY_EXISTS");
    ...
    return _mint(msg.sender, _id, _amount, "");
  }

  /** [owner] Update the URI of a token. Set to "" to clear. */
  function updateURI(uint256 _id, string memory _uri) public onlyOwner {
    uriMap[_id] = _uri;
    emit URI(_uri, _id);
  }

  /** Override the uri to return the value in the uriMap, falling back to the URI pattern. */
  function uri(uint256 _id) public view override returns (string memory) {
    // return string(abi.encodePacked(_uri, _id.toString(), ".json"));

    string memory lookup = uriMap[_id];
