...
contract ViewCountOracle is UsingWitnet {
  ...
  function createOracle(string calldata _id) internal {
    // prettier-ignore
    oracles[_id] = Oracle(0, 0, 0, 0);
  }
  ...
  function requestUpdate(string calldata _id) public payable {
    ...
    oracle.lastRequestId = witnetPostRequest(
      new Request(
        bytes(
          abi.encodePacked(
    ...
    witnetRequestResolved(oracles[_id].lastRequestId)
  {
    ...
    require(oracle.pending == 1, "There is no pending update");
  ...
  function getLastRequestId(string calldata _id)
    external
    view
    returns (uint256)
  {
