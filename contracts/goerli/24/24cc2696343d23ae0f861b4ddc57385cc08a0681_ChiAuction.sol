...
contract ChiAuction is Ownable, ReentrancyGuard {
    ...
    *******************/
    ...
    PUBLIC FUNCTIONS
    *******************/
    constructor(address _chiERC20, address _chiERC721) public {
        require(address(_chiERC20) != address(0));
        require(address(_chiERC721) != address(0));

        chiERC20 = _chiERC20;
        chiERC721 = _chiERC721;
        feePercentages = [100];
    ...
    function createAuction(
        ...
        IERC721(chiERC721).transferFrom(_msgSender(), address(this), _tokenId);
    ...
    {
        IERC20(chiERC20).transferFrom(_msgSender(), address(this), _amount);
        ...
        nonEndedAuction(_auctionId)
    {
        require(
            ...
            'ChiAuction: Direct buy not available for this auction.'
        );
        require(
            IERC20(chiERC20).balanceOf(_msgSender()) > auctions[_auctionId].directBuyPrice,
            'ChiAuction: User has not enough balance.'
        );
        ...
        IERC20(chiERC20).transferFrom(_msgSender(), address(this), feeAmount);
        IERC20(chiERC20).transferFrom(_msgSender(), auctions[_auctionId].owner, priceWithoutFee);
        IERC721(chiERC721).transferFrom(address(this), _msgSender(), auctions[_auctionId].tokenId);
    ...
    {
        require(auctions[_auctionId].owner == _msgSender(), 'ChiAuction: Only NFT owner can cancel auction.');
    ...
    function claimToken(
        ...
        uint256 currentTime = _getTime();
        require(auction.directlyBought == false, 'ChiAuction: Auction was directly bought.');
        require(auction.endDate < currentTime, 'ChiAuction: Auction has not ended.');
        require(highestBid.bidder == _msgSender(), 'ChiAuction: Caller is not highest bidder.');

        uint256 price = highestBid.amount;
        uint256 feeAmount = price.mul(transactionFee).div(10000);
        uint256 priceWithoutFee = price.sub(feeAmount);

        IERC20(chiERC20).transfer(auction.owner, priceWithoutFee);
        IERC721(chiERC721).transferFrom(address(this), _msgSender(), auction.tokenId);
    ...
    */
    function setBuyingFee(
        ...
        require(_transactionFee < 5000, 'ChiAuction: Fee must be lower than 50');
     ...
     */
     function setFeeingAddresses(
        ...
        require(
             ...
             'ChiAuction: FeePercentages and FeeAddresses must be same size.'
        );
        uint16 totalPercentages = 0;
        for(uint index = 0; index < _feePercentages.length; index++) {
            totalPercentages += _feePercentages[index];
        }
        require(totalPercentages == 100, 'ChiAuction: FeePercentages must sum 100');
    ...
    function distributeFees() public onlyOwner {
        for(uint index = 0; index < feeAddresses.length; index++) {
            ...
            IERC20(chiERC20).transfer(feeAddresses[index], amountForAddress);
        }
        accumulatedFees = 0;
    }
    ...
    *******************/
        ...
        private
    {
        ...
        if(lastBid.amount > 0) {
            IERC20(chiERC20).transfer(lastBid.bidder, lastBid.amount);
        }
    } 
    ...
    *******************/
    modifier requiredBidAmount(uint256 _auctionId, uint256 _amount) {
        ...
        require(
            ...
            "ChiAuction: Insufficient amount for bid, step not surpassed."
        );
        _;
    }
    ...
    modifier nonBidAuction(uint256 _auctionId) {
        require(
            ...
            "ChiAuction: Auction has bidders."
        );
        _;
    }
