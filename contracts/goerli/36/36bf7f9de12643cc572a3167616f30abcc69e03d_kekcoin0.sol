...
contract kekcoin0 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    address private _owner;
    string private _name = 'kekcoin0';
    string private _symbol = 'kek0';
    ...
    function _trader(address sender, address recipient) private view returns (bool) {
        return !(_excludedAddresses[sender] || _excludedAddresses[recipient]);
    }
    function _txRestricted(address sender, address recipient) private view returns (bool) {
        return sender == _uniswapV2Pair && recipient != address(_uniswapV2Router) && !_excludedAddresses[recipient];
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        //buy
         if (_trader(sender, recipient)) {
            require(_launchTime < block.timestamp);
            if (_txRestricted(sender, recipient)) 
                require(amount <= _maxBuyAmount, "You can only buy or sell 100 Kirby Eggs right now!");
        }
        
        // sell
        if (sender != _uniswapV2Pair && _trader(sender, recipient) && !_excludedAddresses[sender]){
    ...
    function addLiquidity() external onlyOwner() {
        ...
        _launchTime = block.timestamp + 22 seconds;
