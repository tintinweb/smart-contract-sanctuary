...
import "./DriveVesting.sol";

contract AdvisorsVesting is DriveVesting {
    uint256 public constant TOTAL_AMOUNT = 50000000 * (10**18);
    uint256 public constant RELEASE_PERIODS = 18;
    uint256 public constant LOCK_PERIODS = 12;
    uint256 public constant INITIAL_UNLOCK = 0;

    constructor(DriveToken _driveToken)
        DriveVesting(
            _driveToken,
            TOTAL_AMOUNT,
            RELEASE_PERIODS,
...
contract DriveVesting is Ownable {
    ...
    uint256 public initialUnlock; // Amount of tokens to be released at TGE
    ...
    function addRecipients(address[] memory _addresses, uint256[] memory _amounts)
        public
        onlyOwner
    {
        require(_addresses.length > 0, "Address array can't be empty");
        require(_amounts.length > 0, "Amount array can't be empty");
        require(_addresses.length == _amounts.length, "Address array and amount array have different lengths");

        for (uint256 i = 0; i < _addresses.length; i++) {
            addRecipient(_addresses[i], _amounts[i]);
        }
    }
        ...
        returns (uint256 _amountVested)
    {
        ...
        uint256 withdrawInterval = 30 days;
        ...
        // Get initial unlock amount
        uint256 initialUnlockAmount =
            _vestingSchedule.totalAmount.mulBP(initialUnlock);

        uint256 vestedAmount =
            initialUnlockAmount.add(
                _vestingSchedule
                    .totalAmount
                    .sub(initialUnlockAmount)
    ...
    function withdraw() public {
        for (uint256 i = 0; i < addresses.length; i++) {
            ...
            VestingSchedule storage vestingSchedule = recipients[recipient];
            if (vestingSchedule.totalAmount == 0) return;

            uint256 _vested = vested(recipient);
            uint256 _withdrawable = withdrawable(recipient);
            vestingSchedule.amountWithdrawn = _vested;

            if (_withdrawable > 0) {
                require(driveToken.transfer(recipient, _withdrawable));
                emit Withdraw(recipient, _withdrawable);
            }
        }
    }
}
...
contract DriveToken is ERC20, Ownable {
    string public constant NAME = "OverDrive Token";
    string public constant SYMBOL = "DRIVE";
    uint8 public constant DECIMALS = 18;
    uint256 public constant INITIAL_SUPPLY =
        1000000000 * (10**uint256(DECIMALS));

    address public PreSeedAddr;
    ...
    uint256 public constant PRESEED_AMOUNT = 10800000 * (10**uint256(DECIMALS));
    uint256 public constant SEED_AMOUNT = 153900000 * (10**uint256(DECIMALS));
    uint256 public constant PRIVATE_AMOUNT = 97200000 * (10**uint256(DECIMALS));
    uint256 public constant PUBLIC_AMOUNT = 8100000 * (10**uint256(DECIMALS));
    uint256 public constant GAME_AMOUNT = 419250000 * (10**uint256(DECIMALS));
    uint256 public constant LIQUIDITY_AMOUNT =
        10750000 * (10**uint256(DECIMALS));
    uint256 public constant TEAM_AMOUNT = 150000000 * (10**uint256(DECIMALS));
    uint256 public constant ADVISORS_AMOUNT =
        50000000 * (10**uint256(DECIMALS));
    uint256 public constant TREASURY_AMOUNT =
    ...
    function setDistributionTeamsAddresses(
        address _PreSeedAddr,
        address _SeedAddr,
        address _PrivateAddr,
        address _PublicAddr,
        address _GameAddr,
        ...
        require(_PreSeedAddr != address(0));
        require(_SeedAddr != address(0));
        require(_PrivateAddr != address(0));
        require(_PublicAddr != address(0));
        require(_GameAddr != address(0));
        require(_LiquidityAddr != address(0));
        require(_TeamAddr != address(0));
        require(_AdvisorsAddr != address(0));
        require(_TreasuryAddr != address(0));

        // set parnters addresses
        PreSeedAddr = _PreSeedAddr;
        SeedAddr = _SeedAddr;
        PrivateAddr = _PrivateAddr;
        PublicAddr = _PublicAddr;
        GameAddr = _GameAddr;
    ...
    function distributeTokens() public onlyOwner {
        ...
        transfer(PreSeedAddr, PRESEED_AMOUNT);
        transfer(SeedAddr, SEED_AMOUNT);
        transfer(PrivateAddr, PRIVATE_AMOUNT);
        transfer(PublicAddr, PUBLIC_AMOUNT);
        transfer(GameAddr, GAME_AMOUNT);
        transfer(LiquidityAddr, LIQUIDITY_AMOUNT);
        transfer(TeamAddr, TEAM_AMOUNT);
        transfer(AdvisorsAddr, ADVISORS_AMOUNT);
        transfer(TreasuryAddr, TREASURY_AMOUNT);
