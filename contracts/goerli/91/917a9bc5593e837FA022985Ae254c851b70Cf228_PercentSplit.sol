...
contract PercentSplit is Constants, Initializable {
  ...
  /**
   * @notice Creates a new proxy contract and initializes it with the given split terms.
  ...
  function initialize(Share[] memory shares) public initializer {
    ...
    for (uint256 i = 0; i < shares.length; i++) {
      total = total.add(shares[i].shareInBasisPoints);
  ...
  receive() external payable {
    // Use the current balance instead of msg.value in order to recover any ETH already in the contract
    ...
    if (value > 0) {
      ...
      for (uint256 i = _shares.length - 1; i > 0; i--) {
        ...
        amountToSend = (value * share.shareInBasisPoints) / BASIS_POINTS;
  ...
  /**
   * @notice Anyone can call this function to split all available tokens at the provided address amongst the recipients.
   */
  ...
  /**
   * @dev Anyone can call this function to split all available tokens at the provided address amongst the recipients.
  ...
  function _splitERC20Tokens(IERC20 erc20Contract) internal returns (bool) {
    try erc20Contract.balanceOf(address(this)) returns (uint256 balance) {
      ...
      for (uint256 i = _shares.length - 1; i > 0; i--) {
        Share memory share = _shares[i];
        bool success;
        (success, amountToSend) = balance.tryMul(share.shareInBasisPoints);
        if (!success) {
          return false;
        }
   ...
   * It will first attempt a fair split of ERC20 tokens and then proceed if that fails.
   */
  function proxyCall(address payable target, bytes memory callData) public onlyRecipient {
    if (!_splitERC20Tokens(IERC20(target))) {
