...
import { ILiquidityManager } from "../interfaces/core/ILiquidityManager.sol";
...
contract LiquidityVault is Initializable, ERC20Upgradeable, ReentrancyGuardUpgradeable {
    using SafeERC20 for IERC20;
    using PercentageMath for uint256;

    /* ============ Immutables ============ */

    // Instance of Option Registry
    IOptionRegistry public immutable optionRegistry;
    // Address of Liquidity Manager
    ...
    // Token 0 (Uni v3) instance
    IERC20 public token0;
    // Token 1 (Uni v3) instance
    IERC20 public token1;
    // Instance of Uni v3 Factory
    IUniswapV3Factory public uniswapV3Factory;
    // Instance of Uni v3 pool
    IUniswapV3Pool public pool;

    constructor(IOptionRegistry optionRegistry_, ILiquidityManager liquidityManager_) {
        require(address(optionRegistry_) != address(0x0), "LiquidityVault::zero-addr");
        require(address(liquidityManager_) != address(0x0), "LiquidityVault::zero-addr");
    ...
    /**
     @notice Emitted when new liquidity is added
     @param user Address of the liquidity provider
     @param mintAmt Mint amount
     @param amt0 Amount of token 0 provided
     @param amt1 Amount of token 1 provided
     */
    event Deposit(address indexed user, uint256 mintAmt, uint256 amt0, uint256 amt1);

    /**
     @notice Emitted when liquidity is removed
     @param user Address of the liquidity provider
     @param burnAmt Burn amount
     @param amt0 Amount of token 0 removed
     @param amt1 Amount of token 1 removed
     */
    event Withdraw(address indexed user, uint256 burnAmt, uint256 amt0, uint256 amt1);

    /**
     @notice Emitted when liquidity is rebalanced
     @param oldLower Old lower tick
     @param oldUpper Old upper tick
     @param lower New lower tick
     @param upper New upper tick
     */
    event Rebalance(int24 oldLower, int24 oldUpper, int24 lower, int24 upper);
    
    /* ============ Modifiers ============ */

    // Access control to rebalancers
    modifier onlyRebalancer() {
        require(liquidityManager.rebalancers(msg.sender), "LiquidityVault::not-owner");
        _;
    }
    ...
    /**
     ...
     @param optionToken_ Address of the option token
     @param uniV3Factory Address of Uniswap v3 factor
     @param initialPrice Initial price (optioPrice / collateralPrice)
     @param priceA Price matching the lower tick
     @param priceB Price matching the upper tick
     */
    function initialize(
        ...
        require(msg.sender == address(liquidityManager), "LiquidityVault::not-manager");

        require(optionRegistry.isOption(address(optionToken_)), "LiquidityVault::invalid-option");

        require(address(uniswapV3Factory) == address(0x0), "LiquidityVault::initialized");
        require(address(token0) == address(0x0) && address(token1) == address(0x0), "LiquidityVault::initialized");
        // require(sqrtPriceX96 > sqrtPriceA && sqrtPriceX96 < sqrtPriceB, "LiquidityVault::invalid-range");

        uniswapV3Factory = uniV3Factory;

        address tokenA = optionToken_.data().collateral;
        address tokenB = address(optionToken_);
        (address token0_, address token1_) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);

        token0 = IERC20(token0_);
        token1 = IERC20(token1_);

        // Find sqrtPriceX96 corresponding to the prices given
        uint160 sqrtPriceX96 = PriceMath.getSqrtRatioAtPrice(tokenB, tokenA, initialPrice);
        uint160 sqrtPriceA = PriceMath.getSqrtRatioAtPrice(tokenB, tokenA, priceA);
        uint160 sqrtPriceB = PriceMath.getSqrtRatioAtPrice(tokenB, tokenA, priceB);

        if (sqrtPriceA > sqrtPriceB) {
            (sqrtPriceA, sqrtPriceB) = (sqrtPriceB, sqrtPriceA);
        }

        require(address(pool) == address(0x0) && totalSupply() == 0, "LiquidityVault::initialized");

        address uniPool = uniswapV3Factory.getPool(address(token0), address(token1), 10000);

        // Create pool if it doesn't exist
        if (uniPool == address(0x0)) {
            uniPool = uniswapV3Factory.createPool(address(token0), address(token1), 10000);

            pool = IUniswapV3Pool(uniPool);
            pool.initialize(sqrtPriceX96);
        }

        // Find lower and upper ticks matching the prices given
        ...
        __ERC20_init("Polynomial Liquidity Token", "PLT");
    }

    /**
     @notice Deposit token0 & token1 and mint vault tokens
     @param mintAmt Amount of vaults tokens to mint. Can be calculated using getMintAmount()
     */
    function deposit(uint256 mintAmt) external nonReentrant {
        require(mintAmt > 0, "LiquidityVault::zero-mint");
        ...
        if (totalSupply() == 0) {
            ...
            // Scaled mint amount based on the supply & balances
        ...
        emit Deposit(msg.sender, mintAmt, amount0, amount1);
    }

    /**
     @notice Withdraw token0 & token1 and burn vault tokens
     @param burnAmt Amt of vault tokens to burn
     */
    function withdraw(uint256 burnAmt) external nonReentrant {
        require(burnAmt > 0, "LiquidityVault::zero-burn");

        uint256 totalSupply = totalSupply();

        // Total liquidity owned by the contract
        (uint128 liq, , , , ) = pool.positions(getPositionId());

        _burn(msg.sender, burnAmt);

        // Scaled liquidity to burn
        uint256 liqBurned = FullMath.mulDiv(burnAmt, liq, totalSupply);
        require(liqBurned < type(uint128).max, "LiquidityVault::overflow");
        ...
        emit Withdraw(msg.sender, burnAmt, amount0, amount1);
    }

    /**
     @notice Rebalance to new concetrated liquidity range
     @param newLowerTick New lower tick
     @param newUpperTick New upper tick
     @param swapThresholdPrice Slippage param for Uniswap v3 swap (in sqrtX96)
     @param swapAmountBPS Maximum amount to swap (in 4 decimals. 1e4 = 100%, 1e2 = 1%)
     @param zeroForOne True for swapping from token0 to token1
     */
    function rebalance(
        int24 newLowerTick,
        int24 newUpperTick,
        ...
        require(newLowerTick % 200 == 0, "LiquidityVault::wrong-lower-tick");
        require(newUpperTick % 200 == 0, "LiquidityVault::wrong-upper-tick");
        ...
        emit Rebalance(lowerTick, upperTick, newLowerTick, newUpperTick);
        ...
        (liq, , , , ) = pool.positions(getPositionId());
        require(liq > 0, "LiquidityVault::no-liq");
    }

    /**
     @notice Burn liquidity and collect fees
     @param burnAmount Amount of tokens to burn
     @param supply Total supply before the burn (for scaling the amounts)
     @param liquidityBurned Amount of liquidity being burned
     */
    ...
    /**
     @notice Withdraw liquidity deployed in a range
     @param lower Lower tick
     @param upper Upper tick
     @param liq Liquidity being burned
     */
    ...
    /**
     @notice Add liquidity in a range
     @param lower Lower tick
     @param upper Upper tick
     @param amount0 Amount of token0
     @param amount1 Amount of token1
     @param swapThresholdPrice Slippage param for Uniswap v3 (in sqrtX96)
     @param swapAmountBPS Percentage amount being used for swapping (in 4 decimals. 1e4 = 100%)
     @param zeroForOne True for swapping from token0 to token1
     */
    function addToPool(
        int24 lower,
    ...
    /**
     @notice Swap tokens Add liquidity in a range
     @param lower Lower tick
     @param upper Upper tick
     @param amount0 Amount of token0
     @param amount1 Amount of token1
    ...
    /**
     @notice Get amount given max amount of token0 and token1
    ...
    function getMintAmounts(uint256 amount0Max, uint256 amount1Max) public view returns (uint256, uint256, uint256) {
        ...
        if (totalSupply() == 0) {
            ...
            if (amt0Current == 0 && amt1Current > 0) {
                ...
                require(amt0Mint > 0 && amt1Mint > 0, "LiquidityVault::fail");
    ...
    /**
     @notice Get mint amounts given max token0
     @param amount0 Maximum amount of token0
     */
    ...
    /**
     @notice Get mint amounts given max token
     @param amount1 Maximum amount of token1
     */
    ...
    /**
     @notice Get current holdings of the contract considering fees, liquidity and idle tokens
     */
    ...
    /**
     @notice Get balances of token0 and token1 that the user can claim
    ...
    /**
     @notice Get the position id of current range
     */
    ...
    /**
     @notice Get total fees earned in the tick
     */
    ...
    /**
     @notice Callback function for Uniswap v3 swapping
     */
...
interface ILiquidityManager {
    function rebalancers(address rebalancer_) external view returns (bool);
