...
//
    ...
    function _sendToFund(uint256 _amount) internal {
        IBasisAsset(dollar).mint(address(this), _amount);
        IERC20(dollar).safeApprove(fund, _amount);
        ISimpleERCFund(fund).deposit(dollar,_amount,'Treasury: Seigniorage Allocation');
        emit ContributionPoolFunded(now, _amount);
    }
        ...
        if (epoch < bdoip01BootstrapEpochs) {
            ...
            if (dollarPrice > dollarPriceCeiling) {
                ...
                if (seigniorageSaved >= bondSupply.mul(bondDepletionFloorPercent).div(10000)) {
                    ...
                    _savedForFund = _savedForBoardRoom.mul(fundAllocationRate).div(100);
                    //fundReserve = _savedForFund.mul(fundAllocationRate).div(100);
                }
