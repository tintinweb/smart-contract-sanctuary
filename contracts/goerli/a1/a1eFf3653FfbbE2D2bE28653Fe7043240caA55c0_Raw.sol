...
contract Raw is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public fomoDogWhiteListPrice = 0.01 ether;
    uint256 public tierSupply = 329;
    ...
    function setFomoDogWhiteListPrice(uint256 _fomoDogWhiteListPrice)
        public
        onlyOwner
    {
    ...
    function setFomoDogWhiteList(address[] calldata _fomoDogWhiteList)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < _fomoDogWhiteList.length; i++) {
    ...
    function preserveMint(uint256 numYoloCats, address to) public onlyOwner {
        require(
            totalSupply() + numYoloCats <= tierSupply,
            "Preserve mint would exceed tier supply"
        );
        require(
            totalSupply() + numYoloCats <= MAX_SUPPLY,
    ...
    function getYoloCatByOwner(address _owner)
        public
        view
    ...
    function mintRaw(uint256 numYoloCats) public payable {
        ...
        require(
            balanceOf(msg.sender) + numYoloCats <= maxBalance,
    ...
    function whiteListMintYoloCats(uint256 numYoloCats) public payable {
    ...
    function auctionMintYoloCat(uint256 numYoloCats) public payable {
        ...
        require(
            numYoloCats * getAuctionPrice() <= msg.value,
