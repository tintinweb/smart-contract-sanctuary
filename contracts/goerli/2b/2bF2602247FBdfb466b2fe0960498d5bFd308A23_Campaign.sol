...
contract Campaign is Ownable, ReentrancyGuard, Pausable {
    ...
    */
    ...
    function getBuyableTokens(address _address) public view returns (uint256) {
        return etherConversionRate.mul(1 ether).mul(1000000000).div(getCurrentEthValue()).div(10 ** etherConversionRateDecimals).sub(tokenSoldMapping[_address]);
    }
     ...
     * @notice Return Zero if it isn't able to retreive a value.
    ...
    */
    function buyTokenByEther(address _beneficiary) public whenNotPaused nonReentrant  payable {
        ...
        require(tokenSoldMapping[_beneficiary] + tokens <= etherConversionRate.mul(1 ether).mul(1000000000).div(getCurrentEthValue()).div(10 ** etherConversionRateDecimals), "ICO_CAMPAIGN::MAX_1_ETH_TOTAL"); 
    ...
    */
    function buyTokenByToken(address _beneficiary, IERC20 _token, uint256 _amount) public whenNotPaused nonReentrant {
        ...
        require(tokenSoldMapping[_beneficiary] + tokens <= erc20TokenConversionRate[_token].mul(1 ether).mul(1000000000).div(getCurrentEthValue()), "ICO_CAMPAIGN::MAX_1000_USD_TOTAL");
        require(tokens <= token.balanceOf(address(this)),"ICO_CAMPAIGN::TOKEN_AMOUNT_NOT_AVAILABLE");
