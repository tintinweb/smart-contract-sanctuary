...
contract EntryPoint is StakeManager {
     ...
     */
    function handleOp(UserOperation calldata op, address payable redeemer)
        public
    {
        ...
        ) = _validatePrepayment(0, op);
        uint256 preOpGas = preGas - gasleft() + op.preVerificationGas;

        uint256 actualGasCost;

        try
            this.internalHandleOp(op, context, preOpGas, prefund, paymentMode)
        returns (uint256 _actualGasCost) {
            actualGasCost = _actualGasCost;
        } catch {
            uint256 actualGas = preGas - gasleft() + preOpGas;
    ...
    function redeem(address payable redeemer, uint256 amount) internal {
        ...
        public
    {
        ...
        for (uint256 i = 0; i < opslen; i++) {
            ...
            (prefunds[i], paymentModes[i], context) = _validatePrepayment(
                i,
                op
            );
            assembly {
        ...
        for (uint256 i = 0; i < ops.length; i++) {
            ...
            uint256 preOpGasi = preOpGas[i];
            uint256 prefundi = prefunds[i];
            PaymentMode paymentModei = paymentModes[i];

            try
                this.internalHandleOp(
                    op,
                    context,
                    preOpGasi,
                    prefundi,
            ...
            returns (uint256 _actualGasCost) {
                ...
                uint256 actualGas = preGas - gasleft() + preOpGasi;
        ...
        returns (uint256 gasUsedByPayForSelfOp)
    {
        ...
        (uint256 requiredPreFund, PaymentMode paymentMode) = getPaymentInfo(
            userOp
        );
        (gasUsedByPayForSelfOp, ) = _validateWalletPrepayment(
            0,
            userOp,
            requiredPreFund,
            paymentMode
    ...
    function getPaymentInfo(UserOperation calldata userOp)
        internal
        view
    ...
    function getSenderAddress(bytes memory initCode, uint256 _salt)
        public
        view
        returns (address)
    {
    ...
    function _validateWalletPrepayment(
        ...
        PaymentMode paymentMode
    ) internal returns (uint256 gasUsedByPayForSelfOp, uint256 prefund) {
        uint256 preGas = gasleft();
        _createSenderIfNeeded(op);
        uint256 preBalance = address(this).balance;
        uint256 requiredEthPrefund = 0;
        ...
        try
            IWallet(op.getSender()).verifyUserOp{gas: op.verificationGas}(
                op,
                requiredEthPrefund
            )
        ...
        uint256 actualEthPrefund = address(this).balance - preBalance;
        ...
        if (paymentMode == PaymentMode.walletEth) {
            ...
            if (actualEthPrefund != 0) {
                revert FailedOp(
                    opIndex,
                    address(0),
                    "using wallet stake but wallet paid eth"
                );
            }
    ...
    function _validatePaymasterPrepayment(
        ...
        uint256 requiredPreFund,
        uint256 gasUsedByPayForSelfOp
    ) internal view returns (bytes memory context, uint256 gasUsedByPayForOp) {
        uint256 preGas = gasleft();
        if (!isValidStake(op, requiredPreFund)) {
            revert FailedOp(opIndex, op.paymaster, "not enough stake");
        }
        //no pre-pay from paymaster
        uint256 gas = op.verificationGas - gasUsedByPayForSelfOp;
        try
            IPaymaster(op.paymaster).verifyPaymasterUserOp{gas: gas}(
                op,
                requiredPreFund
            )
    ...
    function _validatePrepayment(uint256 opIndex, UserOperation calldata userOp)
        private
        returns (
            uint256 prefund,
            PaymentMode paymentMode,
        ...
        )
    {
        ...
        (gasUsedByPayForSelfOp, prefund) = _validateWalletPrepayment(
            opIndex,
            userOp,
            requiredPreFund,
            paymentMode
        ...
        uint256 gasUsedByPayForOp = 0;
        if (paymentMode == PaymentMode.paymasterStake) {
            (context, gasUsedByPayForOp) = _validatePaymasterPrepayment(
                opIndex,
                userOp,
                requiredPreFund,
                gasUsedByPayForSelfOp
    ...
    function getPaymastersStake(address[] calldata paymasters)
        external
        view
        returns (uint256[] memory _stakes)
    {
        _stakes = new uint256[](paymasters.length);
    ...
    function _refundSender(UserOperation calldata userOp, uint256 refund)
        internal
    {
    ...
    function _refundSenderStake(UserOperation calldata userOp, uint256 refund)
        internal
    {
    ...
    function isPaymasterStaked(address paymaster, uint256 stake)
        public
        view
        returns (bool)
    {
