...
contract BuyTheFloorExchange is Owned, ECRecovery  {
  ...
  bytes32 constant BIDPACKET_TYPEHASH = keccak256(
      "BidPacket(address bidderAddress,address nftContractAddress,address currencyTokenAddress,uint256 currencyTokenAmount,uint256 requiredProjectId,uint256 expires)"
  );
  ...
  function getBidPacketHash(address bidderAddress,address nftContractAddress,address currencyTokenAddress, uint256 currencyTokenAmount,uint256 requiredProjectId,uint256 expires) public pure returns (bytes32) {
          return keccak256(abi.encode(
              BIDPACKET_TYPEHASH,
              bidderAddress,
              nftContractAddress,
  ...
  function getBidTypedDataHash(address bidderAddress,address nftContractAddress,address currencyTokenAddress, uint256 currencyTokenAmount,uint256 requiredProjectId,uint256 expires) public view returns (bytes32) {
              ...
              bytes32 digest = keccak256(abi.encodePacked(
                  ...
                  getBidPacketHash(bidderAddress,nftContractAddress,currencyTokenAddress,currencyTokenAmount,requiredProjectId,expires)
              ));
              return digest;
          }
  ...
  function sellNFT(address nftContractAddress, uint256 tokenId, address from, address to, address currencyToken, uint256 currencyAmount, uint256 requiredProjectId, uint256 expires, bytes memory buyerSignature) public returns (bool){
      
      //require personalsign from buyer to be submitted by seller  
       bytes32 sigHash = getBidTypedDataHash(to,nftContractAddress,currencyToken,currencyAmount,requiredProjectId,expires);
      ...
      require(ProjectBasedNFT(nftContractAddress).tokenIdToProjectId(tokenId) == requiredProjectId , 'Incorrect Project Id');
  ...
  function cancelBid(address nftContractAddress, address to, address currencyToken, uint256 currencyAmount,  uint256 requiredProjectId, uint256 expires, bytes memory buyerSignature ) public returns (bool){
      bytes32 sigHash = getBidTypedDataHash(to,nftContractAddress,currencyToken,currencyAmount,requiredProjectId,expires);
      address recoveredSignatureSigner = recover(sigHash,buyerSignature);
