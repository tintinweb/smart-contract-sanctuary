...
contract SuperAuctionViewer {
    ...
    function isWinningConditionMeet(address auctionAddress, uint256 timestamp) external view returns(bool)
    {
        return ISuperAuction(auctionAddress).isWinningConditionMeet(timestamp);
    }
    ...
    returns(ISuperAuction.ViewBidder[100] memory top)
    {
        ...
        uint256 j;
        top[0] = _getViewBidder(auctionAddress, winner, timestamp);
        for(uint256 i = 0; i < 10000; i++) {
            if(i > listFrom && i < listTo) {
                ...
                top[j] = _getViewBidder(auctionAddress, _bidder, timestamp);
            }
        }
    }


    function getViewBidder(address auctionAddress, address account, uint256 timestamp) public view returns(ISuperAuction.ViewBidder memory) {
        return _getViewBidder(auctionAddress, account, timestamp);
    }

    function _getViewBidder(address auctionAddress, address account, uint256 timestamp) private view returns(ISuperAuction.ViewBidder memory) {
        ISuperAuction auction = ISuperAuction(auctionAddress);
        ...
        if(account == auction.winner()) {
            (uint256 _settleBalance, uint256 _cumulativeTimer) = auction.getSettledInfo(account, timestamp);
...
interface  ISuperAuction {
    ...
    function isWinningConditionMeet(uint256 timestamp) external view returns(bool);
    function getSettledInfo(address account, uint256 timestamp) external view returns(uint256 settleBalance,uint256 cumulativeTimer);
}
