...
contract LPTokenWrapper is AccessControl {
  ...
  State private _globals;
  mapping(address => State) private _accounts;

  event Bond(address indexed account, uint256 value);
  event Unbond(address indexed account, uint256 value, uint256 rewarded, uint256 maltReward);
  ...
  function _unbond(uint256 amountLPToken, uint256 amountReward, uint256 maltReward) internal notSameBlock {
    ...
    emit Unbond(msg.sender, amountLPToken, amountReward, maltReward);
  }
...
contract LiquidityMine is Initializable, AccessControl, LPTokenWrapper {
  ...
  struct UserState {
    ...
    mapping(uint256 => Withdrawals) epochWithdraws;
  }
  ...
  function declareReward(uint256 amount) external onlyRewarder {
    ...
    epochRewards[currentEpoch].reward = epochRewards[currentEpoch].reward.add(amount);
  ...
  function declareMaltReward(uint256 amount) external onlyRewarder {
    ...
    epochRewards[currentEpoch].maltReward = epochRewards[currentEpoch].maltReward.add(amount);
    ...
    bondingValidityCheck(amount)
  {
    ...
    // TODO unbonding sending earned rewards Wed 24 Feb 2021 12:01:47 GMT
    (uint256 rewardEarned, uint256 maltEarned) = earned(msg.sender);
    uint256 amountClaimable = rewardEarned.mul(amount).div(bondedBalance);
    uint256 maltClaimable = maltEarned.mul(amount).div(bondedBalance);
    ...
    _checkForForfeit(amount, bondedBalance, amountClaimable, maltClaimable);

    removeFromStakePadding(msg.sender, lessStakePadding, lessMaltStakePadding, "Insufficient stake padding");

    _declaredBalance = _declaredBalance.sub(amountClaimable);
    _declaredMaltBalance = _declaredMaltBalance.sub(maltClaimable);

    _clearSubsidizedLP(msg.sender, bondedBalance);

    _unbond(amount, amountClaimable, maltClaimable);
  }
    ...
    updateReward(msg.sender)
  {
    ...
    userState[msg.sender].epochWithdraws[currentEpoch].reward = userState[msg.sender].epochWithdraws[currentEpoch].reward.add(rewardAmount);
    userState[msg.sender].epochWithdraws[currentEpoch].malt = userState[msg.sender].epochWithdraws[currentEpoch].malt.add(maltAmount);
  ...
  function subsidizedReinvest(uint256 rewardLiquidity) external {
    ...
    userState[msg.sender].epochWithdraws[currentEpoch].reward = userState[msg.sender].epochWithdraws[currentEpoch].reward.add(amountReward);
    ...
    // TODO event for subsidizedReinvest Tue 16 Feb 2021 17:59:26 GMT
  ...
  function earned(address account) public view returns (uint256 earnedReward, uint256 earnedMalt) {
    // The amount earned and not the same as amount allocated. Allocation is
    // instantaneous upon reward being issued but earnings are timelocked.
    uint256 amountAllocated = balanceOfRewards(account);
    uint256 maltAllocated = balanceOfMaltRewards(account);

    if (amountAllocated == 0 && maltAllocated == 0) {
      // If there is no allocation to the user there is no need to do further calculations
      return (0, 0);
    }

    (uint256 rewardPerc, uint256 maltPerc) = getUnlockedPercentage(account);

    return (amountAllocated.mul(rewardPerc).div(10000), maltAllocated.mul(maltPerc).div(10000));
  }
  
  function getUnlockedPercentage(address account) public view returns (uint256, uint256) {
    uint256 userBondedEpoch = userState[account].bondedEpoch;
    uint256 currentEpoch = epoch();

    if (userBondedEpoch == currentEpoch) {
      return (0, 0);
    }

    UnlockData memory unlockingData = UnlockData({
      totalReward: 0,
      unlockedReward: 0,
      totalMalt: 0,
      unlockedMalt: 0,
      userBondedTotal: userState[account].epochBonds[userBondedEpoch]
    });

    for (uint i = userBondedEpoch.add(1); i <= currentEpoch; i += 1) {
      uint256 epochOwnership = 0;
      if (totalBondedAtEpoch(i) > 0) {
        epochOwnership = unlockingData.userBondedTotal.mul(1e6).div(totalBondedAtEpoch(i));
      }

      uint256 rewardValue = (epochRewards[i].reward.mul(epochOwnership).div(1e6));
      uint256 maltValue = (epochRewards[i].maltReward.mul(epochOwnership).div(1e6));

      unlockingData.totalReward = unlockingData.totalReward.add(rewardValue).sub(userState[account].epochWithdraws[i].reward);
      unlockingData.totalMalt = unlockingData.totalMalt.add(maltValue).sub(userState[account].epochWithdraws[i].malt);

      unlockingData.userBondedTotal = unlockingData.userBondedTotal.add(userState[account].epochBonds[i]);
      unlockingData.userBondedTotal = unlockingData.userBondedTotal.sub(userState[account].epochUnbonds[i]);

      if (block.timestamp >= epochRewards[i].completionTime) {
        unlockingData.unlockedReward = unlockingData.unlockedReward.add(rewardValue).sub(userState[account].epochWithdraws[i].reward);
        unlockingData.unlockedMalt = unlockingData.unlockedMalt.add(maltValue).sub(userState[account].epochWithdraws[i].malt);
      } else {
        uint256 rewardStart = getEpochStartTime(i);

        if (block.timestamp < rewardStart) {
          continue;
        }

        unlockingData.unlockedReward = unlockingData.unlockedReward.add(
          rewardValue.mul(
            block.timestamp.sub(rewardStart).mul(100).div(
              epochRewards[i].completionTime.sub(rewardStart)
            )
          ).div(100)
        ).sub(userState[account].epochWithdraws[i].reward);

        unlockingData.unlockedMalt = unlockingData.unlockedMalt.add(
          maltValue.mul(
            block.timestamp.sub(rewardStart).mul(100).div(
              epochRewards[i].completionTime.sub(rewardStart)
            )
          ).div(100)
        ).sub(userState[account].epochWithdraws[i].malt);
      }
    }

    uint256 rewardPerc = 0;
    uint256 maltPerc = 0;

    if (unlockingData.totalReward > 0) {
      rewardPerc = unlockingData.unlockedReward.mul(10000).div(unlockingData.totalReward);
    } 

    if (unlockingData.totalMalt > 0) {
      maltPerc = unlockingData.unlockedMalt.mul(10000).div(unlockingData.totalMalt);
    } 

    return (rewardPerc, maltPerc);
  }
  ...
  function balanceOfMaltRewards(address account) public view returns (uint256) {
    ...
    uint256 totalRewardedWithStakePadding = totalMaltRewarded().add(totalMaltStakePadding());
    uint256 balanceOfRewardedWithStakePadding = totalRewardedWithStakePadding
      .mul(balanceOfBonded(account).add(balanceOfSubsidizedLP(account)))
      .div(globalBondedTotal);
  ...
  function _updateBond(uint256 amountReward, uint256 maltRemoval, uint256 liquidity) internal {
    ...
    userState[msg.sender].epochWithdraws[currentEpoch].reward = userState[msg.sender].epochWithdraws[currentEpoch].reward.add(amountReward);
    userState[msg.sender].epochWithdraws[currentEpoch].malt = userState[msg.sender].epochWithdraws[currentEpoch].malt.add(maltRemoval);
    
    // TODO is this right? Wed 17 Feb 2021 19:58:53 GMT
    _handleStakePadding(msg.sender, liquidity);
    ...
    // TODO event for reinvest Tue 16 Feb 2021 17:59:26 GMT
    emit Bond(msg.sender, liquidity);
  }
  ...
  function _checkForForfeit(uint256 amount, uint256 bondedBalance, uint256 amountClaimable, uint256 maltClaimable) internal {
  ...
  modifier updateReward(address account) {
    ...
    // TODO maybe save some state per user for this  Wed 06 Jan 2021 21:08:23 GMT
