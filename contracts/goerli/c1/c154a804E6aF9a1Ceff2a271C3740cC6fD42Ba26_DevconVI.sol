...
// File contracts/pos-portal/contracts/common/AccessControlMixin.sol
...
// File contracts/pos-portal/contracts/root/StateSender/IStateSender.sol
...
// File contracts/pos-portal/contracts/lib/RLPReader.sol
...
// File contracts/pos-portal/contracts/lib/MerklePatriciaProof.sol
...
// File contracts/pos-portal/contracts/root/ICheckpointManager.sol
...
// File contracts/pos-portal/contracts/lib/Merkle.sol
...
// File contracts/pos-portal/contracts/tunnel/BaseRootTunnel.sol
...
// File contracts/DevconVI.sol

pragma solidity 0.6.6;

contract DevconVI is BaseRootTunnel {
    ...
    bool                         public  resultsAreIn;
    uint                         public  number_of_entries;
    uint256                      public  confirmation_date;
    mapping(address => bool)     public  claimed;

    uint256                     constant tMax = 10000;

    event SaleSet(uint256 start, uint256 end);
    event ReserveAndIncrement(uint256 reserve,uint increment);
    event WheresWallet(address wallet);
    event FirstBid(address bidder, uint256 bid);
    event IncreasedBid(address bidder, uint256 increment, uint256 newTotal);

    event TakenOnTimeout(address addr,uint  amount);
    event AuctionWinningAmountPaid(address addr,uint  amount);
    event RaffleRefundPaid(address  addr, uint balance);
    event CompleteRefund(address addr,uint amount);

    modifier sale_active() {
        require(block.timestamp >= start,"Sale not started");
        require(block.timestamp <= end,"Sale ended");
        require(number_of_entries < tMax, "Sorry. Sold out");
        _;
    }
    ...
    constructor(
        ...
        uint256 _reserve, uint256 _increment
        ) public {
        start = _start;
        end = _end;
        wallet = _wallet;
...
// need events
        ...
        emit ReserveAndIncrement(_reserve,_increment);
    }

    function bid() external payable sale_active() {
        require(block.timestamp >= start && block.timestamp <= end,"Raffle and Auction not currently running");
        uint256 oldBid = entries[msg.sender];
        entries[msg.sender] += msg.value;        
        if (oldBid > 0) {
            require(msg.value >= increment,"increment too small");            
            emit IncreasedBid(msg.sender,oldBid,msg.value);
        } else {
            number_of_entries++;
    ...
    function _processMessageFromChild(bytes memory message) internal override {
        uint256 temp = abi.decode(message,(uint256) );
        bool is_final = temp & 1 << 255 != 0;
        bool is_auction = temp & 1 << 254 != 0;
        address winner = address(temp & 0x00ffffffffffffffffffffffffffffffffffffffff);
        if (is_auction) {
        ...
        if (is_final) {
            resultsAreIn = true;
            confirmation_date = block.timestamp;
        }
     }

    function claimUnclaimedFunds(address addr) public {
        require(msg.sender == wallet,"Unauthorised");
        uint confirmationDate = confirmation_date;
        require(confirmationDate != 0,"Results not yet confirmed");
        require((block.timestamp - confirmationDate > 180 days),"Too early to claim");
        require(!claimed[addr],"Already claimed");
        claimed[addr] = true;
        uint256 amount = entries[addr];
        wallet.transfer(amount);
        emit TakenOnTimeout(addr, amount);
    }

    function claimMyFunds() public {
        uint confirmationDate = confirmation_date;
        require(confirmationDate != 0,"Results not yet confirmed");
        require(!claimed[msg.sender],"Already claimed");
        claimed[msg.sender] = true;
        uint256 amount = entries[msg.sender];
       if (auctionWinners[msg.sender]) {
            wallet.transfer(amount);
            emit AuctionWinningAmountPaid(msg.sender, amount);
            return;
        }
        if (raffleWinners[msg.sender]) {
            uint256 paid = amount;
            amount = reserve;
            uint256 balance = paid.sub(reserve);
            wallet.transfer(amount);
            emit AuctionWinningAmountPaid(msg.sender, amount);
            if (balance > 0) {
                payable(msg.sender).transfer(balance);
                emit RaffleRefundPaid(msg.sender, balance);
            }
            return;
        }
        payable(msg.sender).transfer(amount);
        emit CompleteRefund(msg.sender,amount);
    }
