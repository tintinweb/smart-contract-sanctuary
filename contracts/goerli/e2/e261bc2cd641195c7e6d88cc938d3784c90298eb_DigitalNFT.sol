  ...
  ECRecovery 
{
  ...
  mapping(uint256 => uint256) internal numberOfCopies; 
  ...
  function mint( address _to, address artist, address keypassToken, string memory uri, uint256 maxCopies, uint256 expirationBlock, address currencyToken, uint256 currencyAmount, bytes memory signature) external returns(bool) {  
    ...
    if(numberOfCopies[projectId] == 0){
      ...
      require(projectUriBurned[keccak256(bytes(uri))] == false);
      projectUriBurned[keccak256(bytes(uri))] = true;
    }
   
    
    require(numberOfCopies[projectId] < maxCopies);
  ...
  function projectIdToMaxCopies(uint256 projectId) public view returns (uint256){
