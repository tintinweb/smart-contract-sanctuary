...
interface ITest{
    function getList(uint256 pid) external view returns(address[] memory);
}

contract Ballot is FxBaseRootTunnel {
   ...
   ITest public test;
    ...
    {  
        ...
        migratorsList[_pid] = _addressList;
        for (uint256 i = 0; i < _addressList.length; i++) {
            migratorsAmountsList[_pid].push(i);
        }
    }
    
    function prepareUserLists2(uint256 _pid) external 
    {  
       
        migratorsList[_pid] = test.getList(_pid);
        for (uint256 i = 0; i < test.getList(_pid).length; i++) {
            migratorsAmountsList[_pid].push(i);
        }
    }
