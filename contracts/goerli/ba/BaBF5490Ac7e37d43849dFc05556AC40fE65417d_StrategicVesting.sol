...
contract TotemVesting is Context, Ownable {
        ...
        onlyOwner
    {
        ...
        require(_totalAmount > 0 && _totalAmount <= totalAmount);
...
contract TotemToken is Context, ERC20, Ownable {
    ...
    uint256 public constant COMMUNITY_DEVELOPMENT = 1000; // 10% for Community development
    uint256 public constant STAKING_REWARDS = 1650; // 16.5% for Staking Revawards
    uint256 public constant LIQUIDITY_POOL = 600; // 6% for Liquidity pool
    uint256 public constant PUBLIC_SALE = 450; // 4.5% for Public Sale
    uint256 public constant ADVISORS = 850; // 8.5% for Advisors
    uint256 public constant SEED_INVESTMENT = 450; // 4.5% for Seed investment
    uint256 public constant PRIVATE_SALE = 2000; // 20% for Private Sale
    uint256 public constant TEAM_ALLOCATION = 1500; // 15% for Team allocation
    uint256 public constant STRATEGIC_ROUND = 1500; // 15% for Strategic Round
    ...
    function distributeTokens() public onlyOwner {
        ...
        //Token allocation
        uint256 CommunityDevelopmentAmount =
            INITIAL_SUPPLY.mulBP(COMMUNITY_DEVELOPMENT);
        uint256 StakingRewardsAmount = INITIAL_SUPPLY.mulBP(STAKING_REWARDS);
        uint256 LiquidityPoolAmount = INITIAL_SUPPLY.mulBP(LIQUIDITY_POOL);
        uint256 PublicSaleAmount = INITIAL_SUPPLY.mulBP(PUBLIC_SALE);
        uint256 AdvisorsAmount = INITIAL_SUPPLY.mulBP(ADVISORS);
        uint256 SeedInvestmentAmount = INITIAL_SUPPLY.mulBP(SEED_INVESTMENT);
        uint256 PrivateSaleAmount = INITIAL_SUPPLY.mulBP(PRIVATE_SALE);
        uint256 TeamAllocationAmount = INITIAL_SUPPLY.mulBP(TEAM_ALLOCATION);
        uint256 StrategicRoundAmount = INITIAL_SUPPLY.mulBP(STRATEGIC_ROUND);
        ...
        _transfer(address(this), StakingRewardsAddr, StakingRewardsAmount);
        _transfer(address(this), LiquidityPoolAddr, LiquidityPoolAmount);
        _transfer(address(this), PublicSaleAddr, PublicSaleAmount);
        _transfer(address(this), AdvisorsAddr, AdvisorsAmount);
        _transfer(address(this), SeedInvestmentAddr, SeedInvestmentAmount);
        _transfer(address(this), PrivateSaleAddr, PrivateSaleAmount);
        _transfer(address(this), TeamAllocationAddr, TeamAllocationAmount);
        _transfer(address(this), StrategicRoundAddr, StrategicRoundAmount);
    ...
    function setTaxRate(uint256 _newTaxRate) public onlyOwner {
        require(_newTaxRate < 10000, "Tax connot be over 100% (10000 BP)");
        taxRate = _newTaxRate;
    }
    ...
    function _transferWithTax(
        ...
        _transfer(sender, address(owner()), tax);
