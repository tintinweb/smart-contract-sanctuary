...
contract GStakingManager is AccessControl, ReentrancyGuard {
    ...
    modifier validatePoolById(uint256 pid) {
        require(pid < poolInfo.length, "StakingPool: pool are not exist");
        _;
    }
    ...
    function unstake(uint256 amount, bool harvestReward) external nonReentrant {
        ...
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            _unstake(pid, amount, harvestReward);
        }
    ...
    function _unstake(
        ...
        RewardInfo storage user = rewardInfo[pid][msg.sender];
        StakeInfo storage staker = stakeInfo[msg.sender];
        if (harvestReward || staker.amount == amount) {
            claimReward(pid);
        } else {
            updatePool(pid);
            uint256 pending = ((staker.amount * pool.accRewardPerShare) / ACCUMULATED_MULTIPLIER) - user.rewardDebt;
     ...
     * @param pid id of the pool
     */
    function claimReward(uint256 pid) public validatePoolById(pid) nonReentrant returns (uint256) {
        ...
        user.rewardDebt = (staker.amount * pool.accRewardPerShare) / (ACCUMULATED_MULTIPLIER);
        if (totalPending > 0) {
            pool.rewardToken.safeTransfer(msg.sender, totalPending);
        }
    ...
    function pendingReward(uint256 pid, address userAddress) public view validatePoolById(pid) returns (uint256) {
        ...
        return user.pendingReward + (((staker.amount * accRewardPerShare) / ACCUMULATED_MULTIPLIER) - user.rewardDebt);
    }
    ...
    function updatePool(uint256 pid) public validatePoolById(pid) {
        ...
        if (poolReward == 0 && block.timestamp < pool.closeTime) {
            // amount may be not enough
            return;
        }
        pool.accRewardPerShare = (pool.accRewardPerShare + ((poolReward * ACCUMULATED_MULTIPLIER) / totalStaked));
        pool.lastRewardSecond = endTime;
    }
    ...
    function createPool(
        ...
        emit CreatePool(poolInfo.length - 1);
    }
    ...
    function updatePoolReward(uint256 pid, uint256 amountReward) external onlyRole(ADMIN) validatePoolById(pid) {
        ...
        if (amountReward > oldTotalReward) {
            pool.rewardToken.safeTransferFrom(msg.sender, address(this), amountReward - oldTotalReward);
        } else if (amountReward < oldTotalReward) {
            pool.rewardToken.safeTransfer(msg.sender, oldTotalReward - amountReward);
        }
