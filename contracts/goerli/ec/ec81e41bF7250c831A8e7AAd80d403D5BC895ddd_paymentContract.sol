...
interface IMint {
  ...
  function mintNew(address recipient, uint256 tokenID) external;
...
contract paymentContract is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable  {
  ...
  mapping(address => bool) newCommit;
  struct PendingCommitsNew {bool stake; uint16 amount; uint16[] tokenIDs;}
  mapping(address => mapping(uint256 => PendingCommitsNew)) private _pendingCommitsNew;        
  ...
  function commitMint(uint256 amount, bool stake) external payable whenNotPaused {
    ...
    _pendingCommitsNew[msgSender][_nextID] = PendingCommitsNew(stake, amt,tokenIds);
  ...
  function mintReveal() external whenNotPaused nonReentrant {
    address msgSender = _msgSender();
    require(tx.origin == msgSender && !msgSender.isContract(), "Only EOA1");

    if (newCommit[msgSender]) revealNew(_msgSender());
    else  reveal(_msgSender());
  ...
  function revealNew(address addr) internal {
    ...
    PendingCommitsNew memory commit = _pendingCommitsNew[addr][_seedID];
    amount = commit.amount;
    stake = commit.stake;
    tokenIDs = commit.tokenIDs;
    ...
    for (uint i = 0; i < amount; i++) {
      ...
      uint256 receip_seed = uint256(keccak256(abi.encodePacked(seed,tokenID)));                                                                                                        
      address recipient = selectRecipient(receip_seed,tokenID);    
      
      if (!stake) {                                           
        ERC721Contract.mintNew(recipient,tokenID);

      } else {
        ERC721Contract.mintNew(address(stakingContract),tokenID);
        tokenIds[i] = minted;
      }
    ...
    if (stake) stakingContract.stakeMany(addr, tokenIds);
    delete _pendingCommitsNew[addr][_seedID];
