...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _baseURI() internal view virtual returns (string memory) {
        return 'https://bfg-aa.s3.eu-west-1.amazonaws.com/Hand/';
    }
...
contract Emojis is ERC721Enumerable, Ownable {
  ...
  uint256 public constant MAX_EMOJIS = 100;



  event Minted(address indexed from, address indexed to, uint256 indexed tokenId, uint256 amount);

  constructor() ERC721('Emojis', 'EMJ'){
  }

  function mintEmoji(uint256 numberOfTokens) external payable {
    require (totalSupply() + numberOfTokens <= MAX_EMOJIS, 'Cant mint another emoji');
    require (unitPrice*numberOfTokens == msg.value , 'You need to send 0.05* numberOfTokens ether');

    _mintEmoji(numberOfTokens, _msgSender());
  }
  ...
  function _mintEmoji(uint256 numberOfTokens, address sender) internal {
    for(uint i = 0; i < numberOfTokens; i++) {
        ...
        emit Minted(address(0), sender, mintIndex, unitPrice);
    }
