...
contract Bank is IBank {
    ...
    // magic value for the token address when depositing/withdrawing eth
    ...
    // balances for deposits/withdraws to earn interest
    mapping(address => Account) public ethAccounts;
    mapping(address => Account) public hakAccounts;

    // amount owed from borrowing
    mapping(address => Account) public hakDebts;
    ...
    function getBalance(address token)
        ...
        returns (uint256)
    {
        bool isEth = token == ETH;
        Account memory account;
        if (isEth)
            account = ethAccounts[msg.sender];
        else if (token == address(hakToken))
    ...
    function deposit(address token, uint256 amount)
        ...
        if (isEth) {
            ...
            require(token == address(hakToken), "token not supported");
            account = hakAccounts[msg.sender];
        }
        ...
        console.log("   balanceBefore: ", account.deposit);
        ...
        // transfer the tokens
        if (!isEth) {
            hakToken.safeTransferFrom(msg.sender, address(this), amount);
        }
        // else
        // already paid once the transaction goes through
        console.log("   collateralAmount: ", account.deposit);
        return true;
    }
    ...
    function withdraw(address token, uint256 amount)
        ...
        require(isEth, "token not supported");
        Account storage account;
        if (isEth) {
            account = ethAccounts[msg.sender];
        } else {
        ...
        console.log("   balance (before update): ", account.deposit.add(account.interest));
        // make sure interest is up to date
        _updateAccount(account, DEPOSIT_INTEREST_RATE);

        uint256 balance = account.deposit;
        uint256 interest = account.interest;
        uint256 totalBalance = balance.add(interest);
        
        // check if they want to withdraw all of it or just the amount
        if (amount == 0) {
            amount = totalBalance;
            account.deposit = 0;
            account.interest = 0;
        } else {
            // check if we only need to draw from interest
            if (amount <= interest) {
                account.interest = interest - amount;
            } else {
            // exceeded interest amount so we need to adjust the balance as well
                account.interest = 0;
                uint256 amountDeductedFromBalance = amount -interest;
                account.deposit = account.deposit - amountDeductedFromBalance;
            }
        }
        ...
        if (isEth) {
            ...
            hakToken.safeTransfer(msg.sender, amount);
        }
    ...
    function borrow(address token, uint256 amount)
        ...
        Account storage hakAccount = hakAccounts[msg.sender];

        require(hakAccount.deposit > 0, "no collateral deposited");

        _updateAccount(hakAccount, DEPOSIT_INTEREST_RATE);
        uint256 collateralAmount = hakAccount.deposit.add(hakAccount.interest)
            .mul(priceOracle.getVirtualPrice(address(hakToken)))
            .div(TOKEN_DECIMAL_PRECISION);
        console.log("   collateralAmount: ", hakAccount.deposit);

        Account storage debtAccount = hakDebts[msg.sender];
        _updateAccount(debtAccount, BORROW_INTEREST_RATE);
        uint256 existingDebt = debtAccount.deposit.add(debtAccount.interest);

        uint256 collateralRatio = existingDebt == 0
            ? type(uint256).max
            : collateralAmount.mul(HUNDRED_PERCENT) / debtAccount.deposit;
        console.log("   collateralRatio: ", collateralRatio);
        // check if collateral ratio is already exceeded before borrowing
        require(collateralRatio >= 15000,
            "collateral ratio exceeded");

        // borrow maximum amount that still satisfies the 150% collateral ratio
        // collateralAmount / borrowAmount >= 1.5
        // if we are looking for the max borrowAmount allowed
        // borrowAmount <= collateralAmount / 1.5
        if (amount == 0) {
            amount = (collateralAmount
                .mul(HUNDRED_PERCENT) / MIN_COLLATERAL_RATIO)
                .sub(existingDebt);
        }

        debtAccount.deposit = debtAccount.deposit.add(amount);
        uint256 newCollateralRatio = collateralAmount
            .mul(HUNDRED_PERCENT) / (debtAccount.deposit.add(debtAccount.interest));
        console.log("   collateralAmount: ", collateralAmount);
        console.log("   debtAccount.deposit: ", debtAccount.deposit);
        console.log("   debtAccount.interest: ", debtAccount.interest);
        console.log("   newCollateralRatio: ", newCollateralRatio);
        console.log("   borrowedAmount:     ", amount);
        require(newCollateralRatio >= MIN_COLLATERAL_RATIO, "borrow would exceed collateral ratio");
        emit Borrow(msg.sender, ETH, amount, newCollateralRatio);
        (bool success, ) = msg.sender.call{value: amount}("");
        require(success, "eth transfer failed");
     ...
     *                set to 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE then 
     *                the token is ETH.
     * @param amount - the amount to repay including the interest. If this
     *                 parameter is equal to 0 then the caller wants to pay
     *                 the entire debt amount including interest.
     * @return - the amount still left to pay for this loan, excluding interest.
     */
    function repay(address token, uint256 amount)
        ...
        // Check existing debt
        Account storage account = hakDebts[msg.sender];
        _updateAccount(account, BORROW_INTEREST_RATE);
        uint256 balance = account.deposit;
        uint256 interest = account.interest;
        uint256 existingDebt = balance.add(interest);

        console.log("borrow:");
        console.log("   existingDebt:  ", existingDebt);
        console.log("   amount:  ", amount);

        require(existingDebt > 0, "nothing to repay");

        // Determine how much the caller intended to repay
        uint256 repayAmount = 0;
        // everything has been paid
        if (amount == 0 || amount >= existingDebt) {
            repayAmount = existingDebt;
            account.deposit = 0;
            account.interest = 0;
        }
        // partial payment
            ...
            // check if only interest is paid back
            if (repayAmount <= interest) {
                account.interest = interest.sub(repayAmount);
            } else {
            // exceeded interest amount so we need to adjust the balance as well
                account.interest = 0;
                uint256 amountDeductedFromBalance = repayAmount.sub(interest);
                account.deposit = account.deposit.sub(amountDeductedFromBalance);
            }
        }

        // check that we actually received enough eth
        require(repayAmount <= msg.value, "msg.value < amount to repay");

        uint256 remainingDebt = existingDebt - repayAmount;
        emit Repay(msg.sender, token, remainingDebt);
        console.log("   remainingDebt:  ", remainingDebt);
        // Send back surplus ETH to caller
        uint256 delta = msg.value - repayAmount;
        if (delta > 0) {
        ...
        returns (bool)
    {
        ...
        // Clear existing debt
        Account storage debtAccount = hakDebts[account];
        _updateAccount(debtAccount, BORROW_INTEREST_RATE);
        uint256 existingDebt = debtAccount.deposit.add(debtAccount.interest);
        delete hakDebts[account];

        console.log("liquidate:");
        console.log("   existingDebt:  ", existingDebt);
        console.log("   msg.value:  ", msg.value);
        Account storage collateralAccount = hakAccounts[account];
        _updateAccount(collateralAccount, DEPOSIT_INTEREST_RATE);
        uint256 totalCollateral = collateralAccount.deposit.add(collateralAccount.interest);

        // Transfer funds from liquidator to pay for debt
        require(msg.value >= existingDebt, "insufficient ETH sent by liquidator");
        uint delta = msg.value - existingDebt;
        emit Liquidate(msg.sender, account, token,
            totalCollateral,
            delta);
        if (delta > 0) { // send remaining ETH back to liquidator
            (bool success, ) = msg.sender.call{value: delta}("");
            require(success, "ETH transfer failed");
        }

        // Transfer collateral to liquidator and clear HAK account
        hakToken.safeTransfer(
            msg.sender,
            totalCollateral);
        delete hakAccounts[account];
        ...
        returns (uint256)
    {
        ...
        require(token == address(hakToken), "token not supported");
        Account memory collateralAccount = hakAccounts[account];
        uint256 collateralTokens = collateralAccount.deposit.add(
            _computeInterest(collateralAccount, DEPOSIT_INTEREST_RATE)
        );
        // no collateral
        if (collateralTokens == 0)
            return 0;

        Account memory debtAccount = hakDebts[account];
        uint256 debtAmount = debtAccount.deposit.add(
            _computeInterest(debtAccount, BORROW_INTEREST_RATE)
        );
        // collateral but no debt
        if (debtAmount == 0)
            return type(uint256).max;
        // account has collateral and debt, update balance

        // get token price and calculate collateralValue to debt ratio
        uint256 tokenEthPrice = priceOracle.getVirtualPrice(token);
        uint256 collateralInEth = collateralTokens * tokenEthPrice / TOKEN_DECIMAL_PRECISION;
        return collateralInEth.mul(HUNDRED_PERCENT) / debtAmount;
    }
    ...
    function _computeInterest(Account memory account, uint256 interestRate)
        internal
        view
        returns (uint256 interest)
    {
        ...
        if (interestFactor > 0) {
            interest = account.interest
                .add(interestFactor.mul(account.deposit)
                .div(HUNDRED_PERCENT));
        }
    }
}