...
contract NFT is ERC721URIStorage {
    ...
    uint public normalPrice =20000000000000000; // 0.02eth
    uint public totalMint=0;
    uint public maxMint=400;
    address[] public whitelist1F=[
        0xCDeF3CC7cDBdC8695674973Ad015D9f2B01dD4C4
    ];
    address[] public whitelistPS=[
        0x318a41870C948AFb035f7b75f532c5a617564E36
    ];
    uint totalPrivateSale=0;
    uint totalFreeSale=0;
    bool public salesOpen=false;
    string  public baseUrl="https://gateway.pinata.cloud/ipfs/QmcCcJCuVRjMkqo1FrLyW1kk8fFRrA3tcS14NW6rCz1ejy/";
    ...
    for (uint16 i=0;i<whitelist1F.length;i++){
            if(whitelist1F[i]==msg.sender){
                _whitelist1F=true;
                userIndex=i;
            }
        }

        require(((tokenCount*normalPrice<=msg.value)&&(!((maxMint-tokenCount)<=0))&&salesOpen)||(_whitelist1F)||(_whitelistPS));
        ...
        if(_whitelistPS){
            ...
            require(((tokenCount*normalPrice<=msg.value)&&(!((maxMint-tokenCount)<=0))));
            totalPrivateSale++;
            delete whitelist1F[0];
        }
    ...
    function setSales(bool status) external payable onlyOwner{
        salesOpen=status;
    }
