...
contract XFILStake is Ownable {
   ...
   // mapping (uint256 => mapping (address => UserInfo)) public userInfo;
    mapping (uint256 => mapping (address => UserPool)) public userPool;
    ...
    function deposit(uint256 _pid,uint256 _amount) public {
        ...
        PoolInfo storage pool = poolInfo[_pid];
        UserPool storage userPoolSize = userPool[_pid][msg.sender];
        UserInfo storage user = userInfo[msg.sender][userPoolSize.length];
        ...
        user.index = userPoolSize.length;
        ...
        userPoolSize.length = userPoolSize.length.add(1);
    ...
    function withdraw(uint256 _pid, uint256 _index) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserPool storage userPoolSize = userPool[_pid][msg.sender];
        require(_index < userPoolSize.length, "index fail");
