...
contract VolmexVestingToken is BaseToken, OwnableUpgradeable {
    ...
    function updateRecipient(address newRecipient, address treasureVester)
        external
        onlyOwner
    {
        require(
        ...
        require(
            treasureVester != address(0),
            "VolmexVestingToken: Cannot be 0 address"
        );
        address prevRecipient = ITreasuryVester(treasureVester).recipient();
        uint256 balance = balanceOf(prevRecipient);
        _transfer(prevRecipient, newRecipient, balance);
        ITreasuryVester(treasureVester).setRecipient(newRecipient);
    }
}
...
contract BaseToken is GovernanceBaseToken {
    ...
    string internal constant SYMBOL = "vVOLX";
     ...
     */
    function init(address deployer) internal initializer {
