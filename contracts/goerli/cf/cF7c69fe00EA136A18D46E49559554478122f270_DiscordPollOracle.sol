...
contract DiscordPollOracle is UsingWitnet {
  ...
  modifier ended(string calldata _id) {
  ...
  function checkResult(string calldata _id) external payable ended(_id) {
    if (polls[_id].notEmpty == 0) {
      polls[_id] = Poll(1, 0, 0);
    }

    WitnetRequest request = new WitnetRequest(
      bytes(
        abi.encodePacked(
          hex"0a5012380801122568747470733a2f2f766f74696e672e6775696c642e78797a2f6170692f7265616374732f",
          bytes(_id),
          hex"1a0d82187782186166726573756c741a090a050808120180100222090a050808120180100210c0843d180a20e80728333080c8afa025"
        )
      )
    );
  ...
  /// @param _id the ID of the poll
  function writeResult(string calldata _id) external ended(_id) {
    if (polls[_id].notEmpty == 0) revert PollDoesNotExist();
    ...
    if (witnet.isOk(result)) {
      ...
      uint64[] memory _results = witnet.asUint64Array(result);
