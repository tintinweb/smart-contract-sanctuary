...
@notice
    Valet Vaults. Holds an underlying token, and allows users to interact
    with the DeFi Yield Technology ecosystem through Strategies connected to the Vault.
    ...
    https://github.com/vladZokyo/DeFiYieldTechnology/blob/dev/SPECIFICATION.md
"""
...
def initialize(
    ...
    @dev
        If `nameOverride` is not specified, the name will be 'valVault'
        combined with the name of `token`.

        If `symbolOverride` is not specified, the symbol will be 'valv'
    ...
    if nameOverride == "":
        self.name = concat(DetailedERC20(token).symbol(), " valVault")
    else:
        self.name = nameOverride
    if symbolOverride == "":
        self.symbol = concat("valv", DetailedERC20(token).symbol())
    else:
    ...
    self.DOMAIN_SEPARATOR = keccak256(
        concat(
            DOMAIN_TYPE_HASH,
            keccak256(convert("Valet Vaults", Bytes[12])),
...
def apiVersion() -> String[28]:
    ...
    @notice
        ...
        can use this version number to compare with DeFi Yield Technology GitHub and
...
def _issueSharesForAmount(to: address, amount: uint256) -> uint256:
    ...
    if totalSupply > 0:
        ...
        shares = amount
    # dev: division rounding resulted in zero
    assert shares != 0
...
def deposit(_amount: uint256 = MAX_UINT256, recipient: address = msg.sender) -> uint256:
    ...
    log Deposit(amount, msg.sender, recipient, shares)
...
@view
@external
def shareValue(shares: uint256) -> uint256:
    return self._shareValue(shares)
...
def updateStrategyMaxDebtPerHarvest(
    ...
    assert msg.sender in [self.management, self.governance]
    assert self.strategies[strategy].activation > 0
    assert self.strategies[strategy].minDebtPerHarvest <= maxDebtPerHarvest, "59"
