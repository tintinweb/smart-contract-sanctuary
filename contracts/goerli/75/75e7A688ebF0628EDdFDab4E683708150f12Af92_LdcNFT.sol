...
contract LdcNFT is ERC721Enumerable, Ownable {
  ...
  uint256 public prizeAmount = 5000000000000000000; // 5 eth
  ...
  function preSaleMint(address _to, uint256 _mintAmount) public payable {
      ...
      require(msg.value >= preSaleCost * _mintAmount, "Ether value sent is not correct");
        ...
        if (prizeAmount > 0) {
          ...
          if (rnd < 1 && address(this).balance > 10000000000000000000) {
            payable(_to).transfer(1000000000000000000);
            prizeAmount -= 1000000000000000000;
          } else if (rnd < 4 && address(this).balance > 5000000000000000000) {
            payable(_to).transfer(500000000000000000);
            prizeAmount -= 500000000000000000;
          } else if (rnd < 80 && address(this).balance > 100000000000000000) {
            payable(_to).transfer(100000000000000000);
