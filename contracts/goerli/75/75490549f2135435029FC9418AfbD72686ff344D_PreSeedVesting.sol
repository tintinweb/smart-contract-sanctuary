...
contract PreSeedVesting is DriveVesting {
    uint256 public constant TOTAL_AMOUNT = 10800000 * (10**18);
    uint256 public constant RELEASE_PERIODS = 24;
    uint256 public constant LOCK_PERIODS = 0;
    uint256 public constant INITIAL_UNLOCK = 1000;
...
contract DriveToken is ERC20, Ownable {
    ...
    function distributeTokens() public onlyOwner {
        ...
        _transfer(address(this), PreSeedAddr, PRESEED_AMOUNT);
        _transfer(address(this), SeedAddr, SEED_AMOUNT);
        _transfer(address(this), PrivateAddr, PRIVATE_AMOUNT);
        _transfer(address(this), PublicAddr, PUBLIC_AMOUNT);
        _transfer(address(this), GameAddr, GAME_AMOUNT);
        _transfer(address(this), LiquidityAddr, LIQUIDITY_AMOUNT);
        _transfer(address(this), TeamAddr, TEAM_AMOUNT);
        _transfer(address(this), AdvisorsAddr, ADVISORS_AMOUNT);
        _transfer(address(this), TreasuryAddr, TREASURY_AMOUNT);
