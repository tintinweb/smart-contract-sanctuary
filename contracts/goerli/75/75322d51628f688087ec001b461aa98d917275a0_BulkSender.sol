...
contract BulkSender is Ownable{
    ...
    function bulksend(address payable [] memory _to,uint  _value) public payable{
        for (uint8 i = 0; i < _to.length; i++) {
            _to[i].transfer(_value);
        }
    }

    function multisendToken(address token, address[] memory _to, uint256 _balance) public {
    ...
    function multisendToken2(address token, address[] memory _to, uint256 _balance) public {
        ERC20 erc20token = ERC20(token);
        uint8 i = 0;
        for (i; i < _to.length; i++) {
