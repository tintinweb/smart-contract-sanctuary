...
contract BTIG is ERC20 {

    constructor () ERC20("BTIG_Token", "BTIG", 18) {
        admin = msg.sender;
        _mint(admin, 1000000000);
    }
    ...
    function mint(address _account, uint256 _qty) public {
        _mint(_account, _qty);
    }

    function burn(address _account, uint256 _qty) public {
        _burn(_account, _qty);
    }
...
contract AirdropBTIG {
    ...
    */
    ...
    therefore, if the participant sends the crowdsale 25 * 10^13 wei we should give them 10 ^ 18 TKNbits

    therefore the rate is 25 * 10^13 wei === 10^18 TKNbits, or 1 wei = 4000 TKNbits

    therefore, our rate is 4000
    ...
    */
    ...
    uint256 taxRate;    // 10x Percentage, If taxRate is 0.1%, input as 1.
    ...
    function sellTokens(uint256 tokens) public payable {
        require(token.balanceOf(msg.sender)>=tokens, "Not enough balance to sell");
        token.burn(msg.sender, tokens);
        // Calculate weiAmount to be paid
        uint256 weiAmount = tokens/(rate-rate*taxRate/1000);
    ...
    */
    ...
    STAKE
    */
    uint256 constant WEEK_TIME = 1; // 7*24*60*60;
    address stakeAdmin;
    uint256 stakeCounter;
    mapping (uint256 => Stake) staking;
    uint256 reward; //100xPercentage
    ...
    function setReward (uint256 _reward100xPercentage) public {
        reward = _reward100xPercentage;
    }

    function stake(uint256 _numTokens) public {
        stakeCounter++;
        staking[stakeCounter] = Stake(msg.sender, block.timestamp,_numTokens);
        token.transfer(stakeAdmin,_numTokens);
        
    }
    function viewMyStakes()public view returns(uint256[] memory){
        uint256[] memory temp ;
        uint256 j;
        for(uint i=1; i<=stakeCounter;i++){
            if(staking[i].staker==msg.sender){
                temp[j] = i;
                j++;
            }
        }
    ...
    function claimStakeReward() public {
        uint256[] memory temp = viewMyStakes();
        uint256 rewards;
        for (uint i=0; i<temp.length;i++){
            uint256 elapsedTimeWeek = (block.timestamp - staking[temp[i]].timeOfStake)/WEEK_TIME;
            rewards += staking[i].numTokens*elapsedTimeWeek*reward/10000;

        }        
        token.transferFrom(stakeAdmin,msg.sender,rewards);
    }
