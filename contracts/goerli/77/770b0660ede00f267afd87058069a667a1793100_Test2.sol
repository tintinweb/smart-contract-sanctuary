...
contract Test2 is ERC20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    //Token info
    uint256 public constant MAX_SELL_TRANSACTION_AMOUNT = 1000000 * (10**9);
    ...
    mapping(address => bool) private _hasBought;
    mapping(address => bool) private _hasSold;

    //Tax info
    uint256 public constant ETH_REWARDS_FEE = 25;
    uint256 public constant MARKETING_FEE = 25;
    uint256 public constant DEV_FEE = 25;
    uint256 public impactFee = 325;
    uint256 public constant TOTAL_FEES = ETH_REWARDS_FEE + MARKETING_FEE + DEV_FEE;
    address public devWallet = 0x6a75D57aB4AAF241462784b539b7188CD7d80D01;
    address public marketingWallet = 0x6a75D57aB4AAF241462784b539b7188CD7d80D01;
    address private _airdropAddress = 0x4C740cEB01d1c3289B4BbCF5Eeca345B15b258FC;
    ...
    constructor(address routerAddress) ERC20("Test 2.0", "TST2.0") {
        //set amm info
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "TST: The router already has that address");
    ...
    function _transfer(
        ...
        if (TDEnabled && !liquidating) {
            if (from == address(uniswapV2Pair)) {
                uint256 multiplier = _hasBought[to] ? 1 : 2;
                require(_holderLastTransferTimestamp[to].add(TD.mul(multiplier)) <= block.timestamp, "_transfer:: Transfer Delay enabled.  Please try again after the tx block passess");
                _holderLastTransferTimestamp[to] = block.timestamp;
                _hasBought[to] = true;
            } else if (to == address(uniswapV2Pair)) {
                uint256 multiplier = _hasSold[from] ? 1 : 2;
                require(_holderLastTransferTimestamp[from].add(TD.mul(multiplier)) <= block.timestamp, "_transfer:: Transfer Delay enabled.  Please try again after the tx block passess");
                _holderLastTransferTimestamp[from] = block.timestamp;
                _hasSold[from] = true;
            } else {
                require(_holderLastTransferTimestamp[from].add(TD) <= block.timestamp, "_transfer:: Transfer Delay enabled.  Please try again after the tx block passess");
                _holderLastTransferTimestamp[from] = block.timestamp;
            }
        }
        ...
        if (takeFee) {
            ...
            require(amount == sendAmount.add(taxAmount), "TST::transfer: Tax value invalid");
