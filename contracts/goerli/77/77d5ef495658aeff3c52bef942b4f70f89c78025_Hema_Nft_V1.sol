...
contract Hema_Nft_V1 is ERC1155, Ownable {
    ...
    function mintBatch(
        ...
        require(
            tokenids.length == amount.length && tos.length == amount.length,
    ...
    function adminBatchTransferFrom(
        ...
        require(
            tokenids.length == amounts.length && tos.length == amounts.length,
            "Mismatched array lengths"
        );
    ...
    function checkMintBatch(uint256[] memory tokenids)
        private
        view
        returns (bool)
    {
    ...
    function _mint(
        ...
        //emit Transfer(address(0), account, id);
    }
