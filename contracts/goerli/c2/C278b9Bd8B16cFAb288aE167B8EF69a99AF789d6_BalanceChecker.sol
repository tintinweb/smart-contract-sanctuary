...
contract BalanceChecker {
    /*
       This event helps us to tracker the balance of individual user in the transaction log
    */

    event logBalance(address indexed _of, uint256 value);
  ...
  */
  function tokenBalance(address user, address token) public returns (uint) {
    ...
    (bool success, ) = token.call(abi.encodeWithSignature("balanceOf(address)", user));
    if (tokenCode > 0 && success) {  
      uint256 value = Token(token).balanceOf(user);
      emit logBalance(user,value);
      return value;
    } else {
      return 0;
    }
  ...
  */
  ...
  /** 
  * @dev Checks the blocked balance of a wallet in a token contract
  * @param user address of user from whom you need to fetch the balance of token
  * @param token address of token contract
  * @dev Returns the balance of the token for user. Avoids possible errors:
      - return 0 on non-contract address 
      - returns 0 if the contract doesn't implement balanceOf
  */
  function blockedBalance(address user, address token) public returns (uint) {
    ...
    // is it a contract and does it implement balanceOf 
    (bool success, ) = token.call(abi.encodeWithSignature("blockedBalanceOf(address)", user));
    if (tokenCode > 0 && success) {  
      uint256 value = Token(token).blockedBalanceOf(user);
      emit logBalance(user,value);
      return value;
    } 
    else {
  ...
  /**
  * @dev Check the blocked balances of a wallet for multiple tokens.
  * @param users array of addresses of users from whom you need to fetch the balance of tokens array.
  * @param tokens array of addresses of token contracts
  ...
  * @dev Returns a one-dimensional that's user.length * tokens.length long. The
    array is ordered by all of the 0th users token balances, then the 1th
    user, and so on.
  */
