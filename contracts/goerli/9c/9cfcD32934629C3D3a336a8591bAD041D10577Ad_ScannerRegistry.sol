...
contract ScannerRegistryEnable is ScannerRegistryManaged {
    ...
    function enableScanner(uint256 scannerId, Permission permission) public virtual {
        if      (permission == Permission.ADMIN)   { require(hasRole(AGENT_ADMIN_ROLE, _msgSender()), "sender is not admin"); }
        else if (permission == Permission.SELF)    { require(uint256(uint160(_msgSender())) == scannerId, "sender isn't scanner"); }
        else if (permission == Permission.OWNER)   { require(_msgSender() == ownerOf(scannerId), "sender isn't owner"); }
        else if (permission == Permission.MANAGER) { require(isManager(scannerId, _msgSender()), "sender isn't manager"); }
        else                                       { revert("invalid permission slot"); }
