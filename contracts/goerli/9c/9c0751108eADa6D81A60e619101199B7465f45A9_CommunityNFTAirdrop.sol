...
contract CommunityNFTAirdrop is CommunityDropBase {
    ...
    mapping(string => Drop) private dropsByUrl;
    ...
    event NewAirdrop(string platform, string dropUrl, string channelId);
    event DropOnServer(string indexed platform, string indexed groupId, string dropUrl);
    ...
    /// @param _dataOfRoles An array of structs containing information about a role.
    ...
    function airdropOnServer(
        ...
        Data[] memory _dataOfRoles,
        uint256 _contractId,
        string memory _channelId
    ) external whenNotPaused {
        // Check the signature
        bytes32 message = keccak256(abi.encode(address(this), _platform, _serverId, _dropUrl, _roleIds, msg.sender))
            .toEthSignedMessageHash();
        require(_roleIds.length == _dataOfRoles.length, "Roles and their data are not linked");
        ...
        string[] memory roleIds = new string[](_roleIds.length);
        uint256 j;

        for (uint256 i = 0; i < _roleIds.length; i++) {
            if (
                keccak256(abi.encodePacked(_dataOfRoles[i].values[0])) != keccak256(abi.encodePacked(_serverId)) ||
                keccak256(abi.encodePacked(_dataOfRoles[i].values[1])) != keccak256(abi.encodePacked(_roleIds[i]))
            ) {
                continue;
            }

            if (!claimables[_platform][_roleIds[i]][roleToken].dropped) {
                roleIds[j] = _roleIds[i];

                claimables[_platform][_roleIds[i]][roleToken].dropped = true;
                claimables[_platform][_roleIds[i]][roleToken].active = true;

                ICommunityToken(roleToken).addRoleData(
                    _platform,
                    _roleIds[i],
                    _dataOfRoles[i].tokenImageHash,
                    _dataOfRoles[i].NFTName,
                    _dataOfRoles[i].traitTypes,
                    _dataOfRoles[i].values
                );
                j++;
            }
        }
        ...
        emit NewAirdrop(_platform, _dropUrl, _channelId);
        emit DropOnServer(_platform, _serverId, _dropUrl);
    }
    ...
    function airdropOnGroup(
        ...
        // Check the signature
        bytes32 message = keccak256(abi.encode(address(this), _platform, _groupId, _dropUrl, msg.sender))
            .toEthSignedMessageHash();
        require(message.recover(_signature) == bot, "Not valid signature");
        require(
            keccak256(abi.encodePacked(_data.values[0])) == keccak256(abi.encodePacked(_groupId)),
            "Id and data are not linked"
        );
        require(bytes(dropsByUrl[_dropUrl].dropName).length == 0, "Drop already exists");
        require(_contractId < numOfDeployedContracts[msg.sender], "Invalid NFT token");

        address roleToken = contractsByDeployer[msg.sender][_contractId];
        require(!claimables[_platform][_groupId][roleToken].dropped, "Group already has a drop");
        require(IOwnable(roleToken).owner() == address(this), "Token owner is not the contract");

        string[] memory roleIds = new string[](1);
        roleIds[0] = _groupId;
        ...
        claimables[_platform][_groupId][roleToken] = claimable;

        emit NewAirdrop(_platform, _dropUrl, _groupId);
        emit DropOnServer(_platform, _groupId, _dropUrl);
    }
    ...
    function stopAirdrop(
        ...
        require(claimables[platform][_roleId][roleToken].active, "No active airdrop");
        ...
        claimables[platform][_roleId][roleToken].active = false;
        drop.numOfActive -= 1;
    }

    /// @notice Mints an NFT for the user linked to the given role.
    /// @param _signature Signature created by the bot.
    /// @param _platform Id of platform.
    ...
    function claim(
        ...
        require(
            claimables[_platform][_roleId][_tokenAddress].active ||
                approvals[msg.sender][_platform][_roleId][_tokenAddress],
            "No active airdrop"
        );
        require(!claims[_userId][_platform][_roleId][_tokenAddress], "Already claimed");
        require(IOwnable(_tokenAddress).owner() == address(this), "Token owner is not the contract");

        // Check the signature
        bytes32 message = keccak256(abi.encode(address(this), _platform, _roleId, _tokenAddress, _userId, msg.sender))
            .toEthSignedMessageHash();
        require(message.recover(_signature) == bot, "Not valid signature");

        claims[_userId][_platform][_roleId][_tokenAddress] = true;

        // Mint the NFT from the data and send it to the msg.sender
        ICommunityToken(_tokenAddress).safeMint(msg.sender, _platform, _roleId);
        emit Claimed(_platform, _roleId, _tokenAddress, msg.sender);
    }
    ...
    function grant(
        ...
        bytes32 message = keccak256(abi.encode(address(this), _platform, _roleId, _user, roleToken, msg.sender))
            .toEthSignedMessageHash();
        require(message.recover(_signature) == bot, "Not valid signature");
        require(claimables[_platform][_roleId][roleToken].dropped, "No Token for this role");
    ...
    /// @param _dropUrl Url of the airdrop.
    function getDataOfDrop(string calldata _dropUrl)
        external
        view
        returns (
            string memory,
...
contract CommunityTokenManager is Ownable {
    ...
    /// @param _description Of the NFT.
    function deployTokenContract(
        string calldata _tokenName,
        string calldata _tokenSymbol,
        string memory _description
    ) external {
        address roleToken = ICommunityTokenFactory(tokenFactory).deployTokenContract(
            _tokenName,
            _tokenSymbol,
            externalUrl,
            _description
...
import "./CommunityTokenManager.sol";
