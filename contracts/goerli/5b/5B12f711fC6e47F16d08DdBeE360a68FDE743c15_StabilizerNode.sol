...
import "./SNPrivileged.sol";

contract StabilizerNode is Initializable, StabilizerNodePrivileged {
  function initialize(
    address _dao,
    address _timelock,
    address _stabilizer,
  ...
  function stabilize() external notSameBlock {
    ...
    if (!_shouldAdjustSupply(exchangeRate)) {
      uint256 constantReward = maltPoolPeriphery.calculateFixedReward(lastStabilize, annualYield);
  ...
  function purchaseArbitrageToken(uint256 amount) external notSameBlock {
    ...
    emit PurchaseArbTokens(msg.sender, amount);
  }
  ...
  function claimArbitrage(uint256 _auctionId) external notSameBlock {
    ...
    emit ClaimArbTokens(msg.sender, _auctionId, amountTokens);
  }
}
...
interface ILiquidityMine {
  ...
  function totalBondedRewarded() external view returns (uint256);
  function totalBondedMaltRewarded() external view returns (uint256);
...
import "./SNAuctionUtilities.sol";

contract StabilizerNodePrivileged is Permissions, StabilizerNodeAuctionUtilities {
  ...
  function setAnnualYield(uint256 yield)
    external
    onlyRole(TIMELOCK_ROLE, "Must have timelock role")
  {
    _setAnnualYield(yield);
  }
  ...
  function setArbTokenReplenishSplit(uint256 split)
    external
    onlyRole(TIMELOCK_ROLE, "Must have timelock role")
  {
  ...
  function setContractionDamping(uint256 amount)
    external
    onlyRole(TIMELOCK_ROLE, "Must have timelock role")
  {
    require(amount > 0, "No negative damping");
  ...
  function setDamping(uint256 expansionAmount, uint256 contractionAmount)
    external
    onlyRole(TIMELOCK_ROLE, "Must have timelock role")
  {
  ...
  function setPriceTarget(uint256 _target)
    external
    onlyRole(TIMELOCK_ROLE, "Must have timelock role")
  {
    require(_target > 0);
  ...
  function redistributeReserves(uint percentage)
    public
    onlyRole(TIMELOCK_ROLE, "Must have timelock role")
  {
    require(percentage <= 10000, "Percentage too large");

    // Redistribute a percentage (out of 10000) of the reserves as rewards
    uint256 balance = rewardToken.balanceOf(address(this)) - auction.claimableArbitrageRewards();

    uint256 rewarded = balance.mul(percentage).div(10000);

    _distributeRewards(rewarded);

    (uint256 newRatio, ) = maltPoolPeriphery.reserveRatio();

    emit RedistributeRewards(rewarded, newRatio);
  }

  function transferReserves(address destination)
    public
    onlyRole(TIMELOCK_ROLE, "Must have timelock role")
  {
    uint256 balance = rewardToken.balanceOf(address(this)) - auction.claimableArbitrageRewards();
    rewardToken.safeTransfer(destination, balance);
    emit ReserveTransfer(address(rewardToken), destination, balance);
  }

  function transferReserveToken(address _token, address destination)
    public
    onlyRole(TIMELOCK_ROLE, "Must have timelock role")
  {
    ...
    emit ReserveTransfer(_token, destination, balance);
  }

  function requestMint(uint256 amount)
    external
    onlyRole(LIQUIDITY_MINE_ROLE, "Must have liquidity mine role")
  {
    stabilizer.requestMint(amount);
    malt.safeTransfer(liquidityMine, amount);
    emit MintMalt(amount);
  }
    ...
    onlyRole(LIQUIDITY_MINE_ROLE, "Must have liquidity mine role")
  {
    ...
    emit BurnMalt(amount);
  }

  function auctionPreBurn(uint256 maxSpend, uint256 rRatio, uint256 decimals)
    external
    onlyRole(AUCTION_ROLE, "Must have auction role")
    returns (
      uint256 initialReservePledge,
      uint256 initialBurn
    )
  ...
  function setNewLiquidityMine(address _lpmine)
    external
    onlyRole(TIMELOCK_ROLE, "Must have timelock role")
  {
    _setupRole(LIQUIDITY_MINE_ROLE, _lpmine);
    liquidityMine = _lpmine;
    emit SetNewLiquidityMine(_lpmine);
  }

  function removeLiquidityMine(address _lpmine)
    external
    onlyRole(TIMELOCK_ROLE, "Must have timelock role")
  {
...
import "./SNUtilities.sol";

contract StabilizerNodeAuctionUtilities is StabilizerNodeUtilities {
...
contract StabilizerNodeUtilities is StabilizerNodeState {
  ...
  function _stabilityWindowOverride() internal view returns (bool) {
    // Stabilizer contract is always allowed to call stabilize
  ...
  function _distributeSupply() internal returns (uint256) {
  ...
  function _buyMalt(uint256 amountRewardToken) internal returns (uint256 purchased) {
  ...
  function _setAnnualYield(uint256 yield) internal {
    annualYield = yield;
    emit SetAnnualYield(yield);
  }
}
...
contract StabilizerNodeState {
  ...
  event ReserveTransfer(address token, address destination, uint256 amount);
  event RemoveLiquidityMine(address _lpmine);
  event PurchaseArbTokens(address account, uint256 amount);
  event ClaimArbTokens(address account, uint256 auctionId, uint256 amount);
