...
contract Hive is IHive, Ownable, IERC721Receiver, Pausable {
    ...
    function calculateOwed(uint256 since) internal view returns (uint256 owed) {
        uint256 d = (block.timestamp - since) / 6 hours; // / 1 days
        if (d > 6) d = 6;
        uint256 left = block.timestamp - since;
        if (left > (d * 6 hours)) left = left - (d * 6 hours);
        else left = 0;
        if (d > 0) owed = accCombined[d - 1];
        owed += ((left * accDaily[d]) / 6 hours);
    }

        // console.log('>>', d, left, (left * accDaily[d] / 1 days));
    function calculateBeeOwed(uint256 hiveId, uint256 tokenId) public view returns (uint256 owed) {
        ...
        if (since >= hives[hiveId].startedTimestamp) {
            ...
            if (since < hives[hiveId].lastCollectedHoneyTimestamp && hives[hiveId].collectionAmount > 0) {
                if (owed > hives[hiveId].collectionAmount) owed -= hives[hiveId].collectionAmount;
                else owed = 0;
            }
        } else if (since > 0) {
            owed = calculateOwed(hives[hiveId].startedTimestamp);
        }
    }
     ...
     */
    function removeFromWaitingRoom(uint256 tokenId) external whenNotPaused {
    ...
    function _claimBeeFromHive(
        ...
        beesContract.increaseTokensPot(tokenId, uint32(owed));
    ...
    function getInfoOnHives(uint256 _start, uint256 _to) public view returns (string memory) {
        ...
        uint256 to = _to > 0 ? _to : ((beesContract.getMinted() / 100) + extraHives);
