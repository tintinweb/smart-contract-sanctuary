...
contract FlashBotsMultiCall {
    ...
    function uniswapWeth2(
        ...
        uint256 _wethBalanceBefore = WETH.balanceOf(address(this));
        if (_amountToFirstMarket > 0) {
            IERC20(_tokenToFirstMarket).transfer(
                _targets[0],
    ...
    function uniswapV3FlashCallback(
        ...
        uint256 _wethBalanceBefore = WETH.balanceOf(address(this));
        if (decoded._amountToFirstMarket > 0) {
            IERC20(decoded._tokenToFirstMarket).transfer(
                decoded._targets[0],
                decoded._amountToFirstMarket
            );
        }
        for (uint256 i = 0; i < decoded._targets.length; i++) {
            ...
            require(i==0 || _success, "failed not at step 0");
            require(_success, "call swap function not successful");
            _response;
        }
        uint256 _wethBalanceAfter = WETH.balanceOf(address(this));
        uint256 _amountOwed = decoded._amountToFirstMarket + fee0;
        require(
            _wethBalanceAfter >
                _wethBalanceBefore + decoded._ethAmountToCoinbase + fee0,
            "profit is negative or profit less then miner fee"
        );
        ...
        _flashPool.flash(
            ...
            abi.encode(
                FlashCallbackData({
                    ...
                    _targets: _targets,
                    _payloads: _payloads,
                    _tokenToFirstMarket: _tokenToFirstMarket,
                    _amountToFirstMarket: _amountToFirstMarket
                })
            )
        );
    }
