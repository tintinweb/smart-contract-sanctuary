...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    function swapAndMintStarkWare(address tokenA, address tokenB, uint[] calldata path,
     uint[] calldata quantisedAmounts, uint quantisedAmount, uint tokenAAmount,
      uint tokenBAmount, address exchangeAddress, string calldata id) external returns(bool) {
      address pairAddress = UniswapV2Library.pairFor(factory, tokenA, tokenB);
      IUniswapV2Pair pair = IUniswapV2Pair(pairAddress);
      emit Sync(pairAddress, id, 'swapAndMintStarkWare');
      return pair.swapAndMintStarkWare(path, quantisedAmounts, quantisedAmount, tokenAAmount, tokenBAmount, exchangeAddress);
    }

    function swapAndBurnStarkWare(address tokenA, address tokenB, uint[] calldata path,
     uint[] calldata quantisedAmounts, uint quantisedAmount, address exchangeAddress, string calldata id) external returns(bool) {
      address pairAddress = UniswapV2Library.pairFor(factory, tokenA, tokenB);
      IUniswapV2Pair pair = IUniswapV2Pair(pairAddress);
      emit Sync(pairAddress, id, 'swapAndBurnStarkWare');
      return pair.swapAndBurnStarkWare(path, quantisedAmounts, quantisedAmount, exchangeAddress);
    }
    ...
    function getPairState(address tokenA, address tokenB) external view returns (uint reserveA, uint reserveB, uint totalSupply) {
      IUniswapV2Pair pair = IUniswapV2Pair(UniswapV2Library.pairFor(factory, tokenA, tokenB));
      (reserveA, reserveB) = UniswapV2Library.getReserves(factory, tokenA, tokenB);
      totalSupply = pair.totalSupply();
    }

    function setupStarkware(address tokenA, address tokenB, uint assetId, uint tokenAAssetId, uint tokenBAssetId) external {
      IUniswapV2Pair pair = IUniswapV2Pair(UniswapV2Library.pairFor(factory, tokenA, tokenB));
...
interface IUniswapV2Pair {
    ...
    function swapAndMintStarkWare(uint[] memory path, uint[] memory quantisedAmounts, uint quantisedAmount, uint tokenAAmount, uint tokenBAmount, address exchangeAddress) external returns(bool);
    function swapAndBurnStarkWare(uint[] memory path, uint[] memory quantisedAmounts, uint quantisedAmount, address exchangeAddress) external returns(bool);
}
