...
contract ERC1155Marketplace is Initializable, ERC1155HolderUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // Mapping from ERC-1155 token address to unique status
    mapping(address => bool) public isUnique;
    ...
    function setIsUnique(address itemContract, bool unique) external onlyOwner {
        isUnique[itemContract] = unique;
    }
    ...
    function buy(
        ...
        if (isUnique[itemContract]) {
            _handleCancelUniqueOffer(itemContract, itemId, buyer);
        }
    ...
    function takeOffer(
        ...
        if (isUnique[itemContract]) {
            _handleCancelUniqueListing(itemContract, itemId, seller);
        }
    ...
    function _handleCancelUniqueListing(
        address itemContract,
        uint256 itemId, 
        address seller
    ) internal {
        ...
        if (currentListing.amount > 0) {
            delete itemsOnSale[itemContract][itemId][seller];
            
            emit ItemDelisted(itemContract, itemId, seller);
        }
    }

    function _handleCancelUniqueOffer(
        address itemContract,
        uint256 itemId, 
        address buyer
    ) internal {
