...
contract MRCRYPTO is ERC721Enumerable, Ownable {
	...
	function withdraw() public payable onlyOwner { //TODO: carteras de los fundadores del contrato
		uint256 each = (address(this).balance * 100)/3;
		
		// =============================================================================
			require(payable(0x0AeaC6D1424EA6d0F87123A50CA5eEc9f16108c5).send(each)); //JOMMYS
			require(payable(0xbCbb32C0caC8833061D6281e934B20bb93004255).send(each)); //ALEXFU
			//require(payable(YONA).send(each));
			//require(payable(CARLOS).send(each));
			//require(payable(VICTOR).send(each));
			//require(payable(TOYOS).send(each));
		// =============================================================================
	}
