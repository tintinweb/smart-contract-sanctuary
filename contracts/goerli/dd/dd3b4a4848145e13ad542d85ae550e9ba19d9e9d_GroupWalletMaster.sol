...
contract AbstractGroupWalletMaster {
  address internal masterCopy;                                                  // ******* DO NOT CHANGE ORDER ******

  mapping(uint256 => uint256) private tArr;
  address[]                   private owners;

  address internal GWF;                                                         // GroupWalletFactory master contract

  AbstractResolver           public  resolverContract;
  AbstractENS                public  ensContract;
  AbstractBaseRegistrar      public  base;
...
contract GroupWalletMaster {
    ...
    function getENS() internal view returns (AbstractENS) {
      //return AbstractGroupWalletMaster(masterCopy).ensContract();               // GroupWalletProxy calls its GroupWalletMaster for ens access
      return AbstractENS( AbstractGroupWalletFactory(GWF).ens() );
    }

    function getRsv() internal view returns (AbstractResolver) {
      //return AbstractGroupWalletMaster(masterCopy).resolverContract();          // GroupWalletProxy calls its GroupWalletMaster for resolver access
    ...
    constructor(address[] memory _owners, AbstractBaseRegistrar _base, AbstractENS _ens, AbstractResolver _resolver) public payable
    {
      uint i;
      do {
      ...
      require(address(base.ens()) == address(ensContract),"ens NOT base.ens!!!!!");
    }
    
    uint constant     private MAX_OWNER_COUNT = 31;
    uint constant     private welcomeBudget   = 0.006 ether;                    // new members ether
    uint constant     private lowBudget       = 0.200 ether;                    // GWP keeps ether
