        ...
        if (epoch < BootstrapEpochs) {
            ...
            if (paprPrice > paprPriceCeiling) {
                ...
                if (seigniorageSaved >= inkSupply.mul(inkDepletionFloorPercent).div(10000)) {
                    ...
                    // uint256 _seigniorage = paprSupply.mul(_percentage);

                    // uint256 ratio1 = printingRatioMultiplier.div(100);
                    // uint256 ratiO = ratio1.mul(_countTimesOverCents());
                    // uint256 ratio2 = _seigniorage.mul(ratiO);

                    // uint256 _computingMint = _seigniorage.add(ratio2);
