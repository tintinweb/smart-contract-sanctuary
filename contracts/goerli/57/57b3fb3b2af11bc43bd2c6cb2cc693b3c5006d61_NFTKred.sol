...
contract NFTKred is ERC721 {
    ...
    // Mint an NFT within a batch without content
    function mint(
        address to,
        uint32 batch,
        uint32 sequence
    ) public {
        mint(to, batch, sequence, '');
    }
    ...
    /*
    function info(uint32 batch) public view returns (uint32, string memory, string memory, uint64, uint32, uint32, address, bool, bool, bool) {
        batch_info memory i;
        i = batches[batch];
        return (i.batch, i.name, i.created, i.time, i.limit, i.minted, i.controller, i.manageable, i.mintable, i.annotatable);
    }
    
    function info(uint256 tokenId) public view returns (uint32, string memory, string memory, uint64, uint32, uint32, address, bool, bool, bool) {
        batch_info memory i;
        i = batches[batchId(tokenId)];
        return (i.batch, i.name, i.created, i.time, i.limit, i.minted, i.controller, i.manageable, i.mintable, i.annotatable);
    }
    */
    ...
    /* Annotation functions removed because the bytecode size was too large
    function canAnnotate(uint256 tokenId) public view returns (bool) {
        uint256 batchTokenId;
        uint32 batch;
        batch_info memory b;
        ...
        return (_exists(batchTokenId) && b.annotatable &&
                (
                 isController(tokenId) ||
                 (isManager() && b.manageable)
                )
    ...
    function annotate(uint256 tokenId, string memory annotation) public returns (uint256) {
        require(canAnnotate(tokenId), 'You do not have permission to annotate this token.');
        annotations[tokenId] = annotation;
        return tokenId;
    }

    function getAnnotation(uint256 tokenId) public view returns (string memory) {
        return annotations[tokenId];
    }

    function allowAnnotations(uint32 batch) public {
        require(isManager(), settings_error);
        batches[batch].annotatable = true;
    }

    function denyAnnotations(uint32 batch) public {
        require(isManager(), settings_error);
        batches[batch].annotatable = false;
    }
    */
