...
// File: @openzeppelin/[email protected]/utils/introspection/IERC1820Registry.sol
...
// File: @openzeppelin/[email protected]/token/ERC777/IERC777Sender.sol
...
// File: @openzeppelin/[email protected]/token/ERC777/IERC777Recipient.sol
...
// File: @openzeppelin/[email protected]/token/ERC777/IERC777.sol
...
// File: @openzeppelin/[email protected]/token/ERC777/ERC777.sol
...
//import "@openzeppelin/[email protected]/token/ERC20/utils/TokenTimelock.sol";
...
abstract contract DeIDOffering is ERC777, AccessControl, ReentrancyGuard  {
  ...
  struct Rates{uint256 investor;
    ...
    require(currentStage == Stages.icoStart ||currentStage == Stages.genProc || msg.sender == contractcreator);
    require(msg.value > 0);
    uint256 amountWei = msg.value;
        
        if(currentStage == Stages.icoStart){
            transferIcoTokens(amountWei);
        }
        if(currentStage== Stages.genProc){
        }
        if(msg.sender == contractcreator){   
        }
    }

    function transferIcoTokens(uint256 amountWei)internal{
        uint256 currenttokens = amountWei.div(rate.ico).mul(1 ether);
        uint256 currentatoms = amountWei - (currenttokens.mul(rate.ico).div(1 ether));
     ...
     **/
    function startIco() onlyRole(FINANCE_ROLE)  public {
    ...
    **/
    function endIco() onlyRole(FINANCE_ROLE) public {
        currentStage = Stages.icoEnd;
    }

    function startGeneral() onlyRole(FINANCE_ROLE) public{
        currentStage = Stages.genProc;
    }

    function endGeneral() onlyRole(FINANCE_ROLE) public{

        currentStage = Stages.genEnd;
        payable(contractcreator).transfer(address(this).balance); 
    }

    function InvestorPurchase(address investorWallet) payable public onlyRole(INVESTORS_ROLE){
        uint256 amountWei = msg.value;
        require(amountWei > 0, "No ETH Sent");
        uint256 currentTokens = amountWei.div(rate.investor).mul(1 ether);
        uint256 currentAtoms = amountWei - (currentTokens.mul(rate.investor).div(1 ether));
        uint256 tokens= currentTokens+currentAtoms;
    ...
    function InvestorSell(address investorWallet, uint256 tokensToSell)  payable onlyRole(INVESTORS_ROLE) public{
        require(tokensToSell > 0, "No tokens sent for sale, You need to sell at least 1 token");
        uint256 currentWei = tokensToSell.mul(rate.investorSale).div(1 ether); 
       if(investorWallet ==address(0)){
        investorWallet=address(msg.sender);
      }  
    ...
    function ContributorPurchase(address contributorWallet) payable onlyRole(CONTRIBUTORS_ROLE) public{
                uint256 amountWei = msg.value;
        require(amountWei > 0, "No ETH Sent");
        uint256 currentTokens = amountWei.div(rate.contributor).mul(1 ether);
        uint256 currentAtoms = amountWei - (currentTokens.mul(rate.contributor).div(1 ether));
        uint256 tokens= currentTokens+currentAtoms;
    ...
    function ContributorSell(address contributorWallet, uint256 tokensToSell)  payable  onlyRole(CONTRIBUTORS_ROLE)  public{
        require(tokensToSell > 0, "No tokens sent for sale");
                
        uint256 currentWei = tokensToSell.mul(rate.contributorSale).div(1 ether);   
                       if(contributorWallet ==address(0)){
        contributorWallet=address(msg.sender);
      }  
    ...
    function IncubationPurchase(address incubatorWallet) payable onlyRole(INCUBATION_ROLE) public{
               uint256 amountWei = msg.value;
        require(amountWei > 0, "No ETH Sent");
        uint256 currentTokens = amountWei.div(rate.incubator).mul(1 ether);
        uint256 currentAtoms = amountWei - (currentTokens.mul(rate.incubator).div(1 ether));
        uint256 tokens= currentTokens+currentAtoms;
    ...
    function IncubationSell(address incubatorWallet, uint256 tokensToSell)  payable onlyRole(INCUBATION_ROLE) public{
        require(tokensToSell > 0, "No tokens sent for sale, You need to sell at least 1 token");
        
        
        uint256 currentWei = tokensToSell.mul(rate.incubatorSale).div(1 ether);   
        this.transferFrom(incubatorWallet, incubationFundOwner, tokensToSell);
       payable(incubatorWallet).transfer(currentWei);
       
    }
    
    function CustomPurchase(address walletFrom, address walletTo) payable onlyRole(CUSTOMSALE_ROLE) public{
                uint256 amountWei = msg.value;
        require(amountWei > 0, "No ETH Sent");
        uint256 currentTokens = amountWei.div(rate.custom).mul(1 ether);
        uint256 currentAtoms = amountWei - (currentTokens.mul(rate.custom).div(1 ether));
        uint256 tokens= currentTokens+currentAtoms;
    ...
    function CustomSell(address walletFrom, address walletTo, uint256 tokensToSell)  payable onlyRole(CUSTOMSALE_ROLE) public{
        require(tokensToSell > 0, "No tokens sent for sale, You need to sell at least 1 token");
     
        
        uint256 currentWei = tokensToSell.mul(rate.customSale).div(1 ether);   
    ...
    function ContractDraw(uint256 amountDraw) onlyRole(FINANCE_ROLE) public{
           payable(contractcreator).transfer(amountDraw); 
    }

    function SetInvestorRate(uint256 price) onlyRole(FINANCE_ROLE) public{
        rate.investor=price;
    }

    function SetContributorRate(uint256 price) onlyRole(FINANCE_ROLE) public{
      rate.contributor=price;
    }
    function SetICORate(uint256 price) onlyRole(FINANCE_ROLE) public{
      rate.ico=price;
    }
    function SetCustomRate(uint256 price) onlyRole(FINANCE_ROLE)public{
       rate.custom=price;
    }
    function SetInvestorSaleRate(uint256 price) onlyRole(FINANCE_ROLE) public{
        rate.investorSale=price;
    }

    function SetContributorSaleRate(uint256 price) onlyRole(FINANCE_ROLE)public{
      rate.contributorSale=price;
    }
    function SetICOSaleRate(uint256 price) onlyRole(FINANCE_ROLE)public{
      rate.icoSale=price;
    }
    function SetCustomSaleRate(uint256 price) onlyRole(FINANCE_ROLE)public{
...
contract DeIDToken is DeIDOffering {
    address[] operatorList=[address(this),address(msg.sender)];
