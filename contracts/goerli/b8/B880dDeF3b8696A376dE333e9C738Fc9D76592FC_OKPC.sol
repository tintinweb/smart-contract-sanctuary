...
contract OKPC is Insightful721, IERC2981, ReentrancyGuard {
    ...
    function _createArt(Art memory artwork) private {
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(artwork.data1, artwork.data2)
        );
    ...
    /// @notice Collect your payments if you're an artist.
    ...
    function deleteArt(uint256 artId, bool addToDenyList)
        external
        onlyCommunity
    {
...
import {IOKPCFont} from "./OKPCFont.sol";
...
contract OKPCMetadata is IOKPCMetadata {
    ...
    // * CONFIG * //
    OKPCColor[6] private COLORS;

    string[8] private HATS = [
    ...
    constructor(address fontAddress) {
        OKPCColor memory gray = OKPCColor(
            bytes8("#CCCCCC"),
            bytes8("#838383"),
            bytes8("#4D4D4D")
        ); // gray
        OKPCColor memory green = OKPCColor(
            bytes8("#54F8B5"),
            bytes8("#00DC82"),
            bytes8("#037245")
        ); // green
        OKPCColor memory blue = OKPCColor(
            bytes8("#80B3FF"),
            bytes8("#2E82FF"),
            bytes8("#003D99")
        ); // blue
        OKPCColor memory purple = OKPCColor(
            bytes8("#DF99FF"),
            bytes8("#C13CFF"),
            bytes8("#750DA5")
        ); // purple
        OKPCColor memory orange = OKPCColor(
            bytes8("#FBDA9D"),
            bytes8("#F8B73E"),
            bytes8("#795106")
        ); // orange
        OKPCColor memory pink = OKPCColor(
            bytes8("#FF99D8"),
            bytes8("#FF44B7"),
            bytes8("#99005E")
        ); // pink
    ...
    function draw(
        ...
        uint256 hatIndex = (tokenId + 1) % 8;
        uint256 borderRightIndex = (tokenId + 2) % 8;
        uint256 borderLeftIndex = (tokenId + 2) % 8;
        uint256 colorIndex = (tokenId + 3) % 6;
        uint256 wordIndex = (tokenId + 4) % 74;
        ...
        bytes memory svg = abi.encodePacked(
            abi.encodePacked(
                ...
                HATS[hatIndex],
                '" /><path transform="translate(1, 8)" transform-origin="16 16" d="',
                BORDERS[borderLeftIndex],
                '"/><path transform="scale(-1,1) translate(1, 8)" transform-origin="16 16" d="',
                BORDERS[borderRightIndex],
                '"/>'
            ),
            drawArt(art),
