...
contract LdcNFT is ERC721Enumerable, Ownable {
  ...
  uint256 public prizeAmount = 1000000000000000000; // 10 eth
  uint256 public contractMoney = 0; // TODO: - Remove after test
  ...
  function _baseURI() internal view virtual override returns (string memory) {
   ...
   function mint(address _to, uint256 _mintAmount) public payable {
      ...
      // TODO: - Remove
      // if (supply > 0) {
      //   payable(msg.sender).transfer(1000000000000000000); // 1 ETH
      // }
  ...
  function preSaleMint(address _to, uint256 _mintAmount) public payable {
      ...
      require(msg.value >= preSaleCost * _mintAmount, "Ether value sent is not correct");
        ...
        for (uint256 i = 1; i <= _mintAmount; i++) {
            _safeMint(_to, supply + i);
          //contractMoney = contractMoney + 55; // 0.055 eth
          if (prizeAmount > 0) {
              cashback();
          }
        }      
