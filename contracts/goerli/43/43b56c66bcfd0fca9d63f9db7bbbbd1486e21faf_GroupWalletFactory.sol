...
contract GroupWalletFactory {
    ...
    { 
      ...
      if (setProxyCommand == 6) {
        address reveiver = address( uint256(bytesMemoryTobytes32(substring(data,4,32)) & 0x000000000000000000000000ffffffffffffffffffffffffffffffffffffffff ));
        uint256 amount   = uint256( bytesMemoryTobytes32(substring(data,36,32)) );
        emit Transfer (address(this),reveiver,amount);
      }
