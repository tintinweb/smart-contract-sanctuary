...
contract Traits is Ownable {
     ...
     */
    function generate(uint256 tokenId, uint256 seed) public view returns (ICryptoBees.Token memory) {
        require(_msgSender() == address(beesContract), "DONT CHEAT!");

        ICryptoBees.Token memory t = selectTraits(seed);
        // console.log('pozor', structToHash(t), existingCombinations[structToHash(t)]);
