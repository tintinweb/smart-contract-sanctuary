...
contract Pyramid is Usecase, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function giveWinnerBonus(
        ...
        uint256 amt;
        if (_autoplay) {
            //return 100% bonus when autoplay
            amt = priceWei;
        } else {
            //return 190% bonus when final game. (10% for handle fee)
            amt = priceWei * 2 - (priceWei / 10);
        }

        pendingWithdrawals[_winner].amount += amt;
        pendingWithdrawals[_winner].limitBlockNumber = block.number + 12; //could be withdrawls after 12 block
        minimumAmount += amt;
