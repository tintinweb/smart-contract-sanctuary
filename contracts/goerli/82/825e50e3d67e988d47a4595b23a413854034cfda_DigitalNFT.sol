...
Create NFTs that can be defined offchain by artists for zero gas and then minted on-chain by users.
  ...
  ECRecovery 
{
  ...
  function mint( address _to, address artist, address keypassToken, string memory uri, uint256 maxCopies, uint256 expirationBlock, address currencyToken, uint256 currencyAmount, bytes memory signature) external returns(bool) {  
    ...
    super._setTokenProjectId(tokensMinted, uint256( sigHash ) );
    
    if(numberOfCopies[sigHash] == 0){
      super._setProjectArtist(uint256( sigHash ), artist);
      super._setProjectUri(uint256( sigHash ), uri);
    }
   ...
   function _transferCurrencyForSale( address from, address to, address currencyToken, uint256 currencyAmount) internal returns (bool){
    ...
    require( IERC20(currencyToken).transferFrom(from, address(_feeRecipient), feeAmount ), 'unable to pay'  );
