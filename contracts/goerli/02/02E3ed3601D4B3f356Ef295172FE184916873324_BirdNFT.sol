...
contract BirdNFT is ERC721("BirdNFT", "BNFT") {
    string public baseURI = "";
    bool public isSaleActive = false;
    uint256 public circulatingSupply = 0;
    uint256 public itemPrice = 0.1 ether;
    uint256 public constant totalSupply = 5000;
    uint256 public constant reserveAmount = 100;

    uint256 public constant maxBird = 10 ether;
    address public bird = 0x980A4769573F4c2274eCfA0cbF835ad55C36223c;

    uint256 public nftMaxMint = 2;
    mapping(address => uint256) public nftClaimedBy;
    ...
    {
        if (IERC20(bird).balanceOf(msg.sender) >= maxBird)
            require(
                nftClaimedBy[msg.sender] + _howMany <= nftMaxMint,
                "Purchase exceeds max allowed"
            );
        else
            require(
                nftClaimedBy[msg.sender] + _howMany <= 1,
                "Purchase exceeds max allowed"
            );

        require(
            isSaleActive && circulatingSupply <= totalSupply - reserveAmount,
            "Sale Closed"
        );
        require(msg.value >= _howMany * itemPrice, "Try to send more ETH");

        nftClaimedBy[msg.sender] += _howMany;
