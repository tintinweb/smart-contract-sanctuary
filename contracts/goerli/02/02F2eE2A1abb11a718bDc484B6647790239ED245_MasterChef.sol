...
interface ICrossChainMigrator {
    function migrate(IERC20 token, uint256 pid) external;
}
...
contract MasterChef is Ownable, Pausable {
    ...
    function ownerWithdraw(uint256 _amount) external onlyOwner whenPaused {
        safeCNTTransfer(msg.sender, _amount);
    }
        ...
        returns (uint256, bool)
    {
        ...
        uint256 userStakedAmount = user.amount;
        user.amount = 0;
        user.rewardDebt = 0;
        return (userStakedAmount, true);
    }
