...
contract SimpleTokenFab {
    function newSImpleToken(string memory symbol, string memory name) public returns (address token) {
        SimpleToken simpleToken = new SimpleToken(symbol, name);

        simpleToken.rely(msg.sender);
        simpleToken.deny(address(this));
