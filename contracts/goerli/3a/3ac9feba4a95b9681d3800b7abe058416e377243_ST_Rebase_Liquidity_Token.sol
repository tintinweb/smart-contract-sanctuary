...
contract ST_Rebase_Liquidity_Token is Context, IST20, Ownable, TokenGenerator {
    ...
    constructor (string memory name, string memory symbol, uint256 decimals, uint256 totalSupply, uint256 _taxFee, uint256 _charityFee, uint256 _mktFee, address _charityAddress, address _mktAddress, address payable feeReceiver) 
    ...
    function setMktFee(uint256 _mktFee) external onlyOwner() {
        require(_mktFee <= 30, "IST20: Charity fees can not be higher than 30%.");
        mktFee = _mktFee;
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tCharity, data.tMkt);
    }
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tCharity).sub(tMkt);
        return (tTransferAmount, TData(0, tFee, tCharity, tMkt, 0));
    }
    function _getRValues(TData memory _data) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rCharity).sub(rMkt);
    ...
    function removeAllFee() private {
        if(taxFee == 0 && charityFee == 0 && mktFee == 0) return;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tCharity, uint256 tMkt) = _getValues(tAmount);
