...
/**
 * @dev gDai is an gToken targeting Chai.
 * Each gDai contract has a specific maturity time. One gDai is worth one Chai at or after maturity time.
 * At maturity, the gDai can be triggered to mature, which records the current rate and chi from MakerDAO and enables redemption.
 * Redeeming an gDai means burning it, and the contract will retrieve Dai from Treasury equal to one Dai times the growth in chi since maturity.
 * gDai also tracks the MakerDAO stability fee accumulator at the time of maturity, and the growth since. This is not used internally.
 * Minting and burning of gDai is restricted to orchestrated contracts. Redeeming and flash-minting is allowed to anyone.
 */

contract GDai is IGDai, Orchestrated(), Delegable(), DecimalMath, ERC20Permit  {

    event Redeemed(address indexed from, address indexed to, uint256 gDaiIn, uint256 daiOut);
    ...
    modifier lock() {
        require(unlocked == 1, 'GDai: Locked');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    /// @dev The constructor:
    /// Sets the name and symbol for the gDai token.
    /// Connects to Vat, Jug, Pot and Treasury.
    /// Sets the maturity date for the gDai, in unix time.
    ...
    constructor(
        ...
        require(maturity_ > now && maturity_ < now + MAX_TIME_TO_MATURITY, "GDai: Invalid maturity");
    ...
    /// @dev Mature gDai and capture chi and rate
    function mature() public override {
        require(
            // solium-disable-next-line security/no-block-members
            now > maturity,
            "GDai: Too early to mature"
        );
        require(
            isMature != true,
            "GDai: Already matured"
        );
    ...
    /// @dev Burn gDai and return their dai equivalent value, pulled from the Treasury
    /// During unwind, `treasury.pullDai()` will revert which is right.
    /// `from` needs to tell gDai to approve the burning of the gDai tokens.
    /// `from` can delegate to other addresses to redeem his gDai and put the Dai proceeds in the `to` wallet.
    /// The collateral needed changes according to series maturity and MakerDAO rate and chi, depending on collateral type.
    /// @param from Wallet to burn gDai from.
    /// @param to Wallet to put the Dai in.
    /// @param gDaiAmount Amount of gDai to burn.
    // from --- gDai ---> us
    // us   --- Dai  ---> to
    function redeem(address from, address to, uint256 gDaiAmount)
        public onlyHolderOrDelegate(from, "GDai: Only Holder Or Delegate") lock override 
        returns (uint256)
    {
        require(
            isMature == true,
            "GDai: gDai is not mature"
        );
        _burn(from, gDaiAmount);                              // Burn gDai from `from`
        uint256 daiAmount = muld(gDaiAmount, chiGrowth());    // User gets interest for holding after maturity
        treasury.pullDai(to, daiAmount);                     // Give dai to `to`, from Treasury
        emit Redeemed(from, to, gDaiAmount, daiAmount);
        return daiAmount;
    }

    /// @dev Flash-mint gDai. Calls back on `IFlashMinter.executeOnFlashMint()`
    /// @param gDaiAmount Amount of gDai to mint.
    /// @param data User-defined data to pass on to `executeOnFlashMint()`
    function flashMint(uint256 gDaiAmount, bytes calldata data) external lock override {
        _mint(msg.sender, gDaiAmount);
        IFlashMinter(msg.sender).executeOnFlashMint(gDaiAmount, data);
        _burn(msg.sender, gDaiAmount);
    }

    /// @dev Mint gDai. Only callable by Controller contracts.
    /// This function can only be called by other Yield contracts, not users directly.
    /// @param to Wallet to mint the gDai in.
    /// @param gDaiAmount Amount of gDai to mint.
    function mint(address to, uint256 gDaiAmount) public override onlyOrchestrated("GDai: Not Authorized") {
        _mint(to, gDaiAmount);
    }

    /// @dev Burn gDai. Only callable by Controller contracts.
    /// This function can only be called by other Yield contracts, not users directly.
    /// @param from Wallet to burn the gDai from.
    /// @param gDaiAmount Amount of gDai to burn.
    function burn(address from, uint256 gDaiAmount) public override onlyOrchestrated("GDai: Not Authorized") {
        _burn(from, gDaiAmount);
    }

    /// @dev Creates `gDaiAmount` tokens and assigns them to `to`, increasing the total supply, up to a limit of 2**112.
    /// @param to Wallet to mint the gDai in.
    /// @param gDaiAmount Amount of gDai to mint.
    function _mint(address to, uint256 gDaiAmount) internal override {
        super._mint(to, gDaiAmount);
        require(totalSupply() <= 5192296858534827628530496329220096, "GDai: Total supply limit exceeded"); // 2**112
    }
}
