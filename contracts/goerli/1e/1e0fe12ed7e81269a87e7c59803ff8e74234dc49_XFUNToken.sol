 ...
 */
contract XFUNToken is ERC20PresetMinterPauserUpgradeable, ERC2771ContextUpgradeable, IEscrowCapable
{
  ...
  function initialize(string memory name, string memory symbol, address _forwarder)
  public
  initializer
  {
   ...
   *
   * See - {AcessControlUpgradeable - _grantRole }
   *
   * Requirements:
   *
  ...
  function grantMinter(address _minter) external {
    require(
      ...
      "XFUNToken: must have admin role to grant minter"
    );
  ...
  function revokeMinter(address _minter) external {
    require(
      hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
      "XFUNToken: must have admin role to revoke minter"
    );
  ...
  function allowEscrow(address _escrow) external {
    require(
      hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
      "XFUNToken: must have admin role to allow escrow"
    );
  ...
  function removeEscrow(address _escrow) external {
    require(
      hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
      "XFUNToken: must have admin role to remove escrow"
    );
...
interface IEscrowCapable {
   ...
   * @param _fee Escrow Fee Amount
   */
  function escrowReturn(address _to, uint256 _value, uint256 _fee) external returns (bool);
}
