...
/**
 * @dev An ERC20 token for HodlDAO.
...
contract HODLToken is ERC20, Ownable {
    ...
    function claimTokens(uint256 amount, bytes32[] calldata merkleProof) external {
        ...
        require(valid, "HodlDAO: Valid proof required.");
        require(!isClaimed(index), "HodlDAO: Tokens already claimed.");
    ...
    function sweep(address dest) external onlyOwner {
        require(block.timestamp > claimPeriodEnds, "HodlDAO: Claim period not yet ended");
    ...
    function setMerkleRoot(bytes32 _merkleRoot) external onlyOwner {
        require(merkleRoot == bytes32(0), "HodlDAO: Merkle root already set");
