...
contract FNDCollectionFactory is ICollectionFactory {
  ...
  IAdminRole public immutable adminRole;
  ...
  modifier onlyAdmin() {
    require(adminRole.isAdmin(msg.sender), "FNDCollectionFactory: Caller does not have the Admin role");
    _;
  }
  ...
  constructor(
    ...
    require(_adminRole.isContract(), "FNDCollectionFactory: Treasury is not a contract");
    require(_nftMarket.isContract(), "FNDCollectionFactory: Market is not a contract");
    require(bytes(_baseURI).length > 0, "FNDCollectionFactory: Base URI is required");
