...
contract Voting is Context, Ownable {
     ...
     *
     * @param finished Return only finished issues
     */
    function listIssues(bool finished) external view returns (IssueItem[] memory) {
        // solhint-disable-next-line not-rely-on-time
        uint256 finishAt = block.timestamp;

        uint256 count;

        IssueItem[] memory items = new IssueItem[](_lastIssue);

        for (uint256 i = 0; i < _lastIssue; i += 1) {
            if (finished ? _issues[i].finishAt <= finishAt : _issues[i].finishAt > finishAt) {
        ...
        IssueItem[] memory issues = new IssueItem[](count);
