...
// burn if needed, transfer tokens to presale participants
// call getPair and check if the address matches the pair, if not call setPair with correct address
// just before adding liquidity, call setLaunchPhase
// add liquidity immediately
// lock liquidity
// renounce
...
contract TestCoin is Ownable, IERC20Metadata {
    ...
    string private constant _name = "abc"; // TBD
    string private constant _symbol = "abc"; //TBD 
    ...
    bool private _launchPhase = false;
    ...
    modifier onlyOwnerOrAdmin {
        require(owner() == _msgSender() || _admin == _msgSender(), "Caller is not the owner nor the admin");
        _;
    }
    ...
    constructor() {
        ...
        _isExcludedFromFee[_admin] = true;
        ...
        emit AdminRightsTransferred(address(0), _msgSender());
    ...
    function _openTrading() private {
        _tradingOpen = true;
        _launchPhase = true;
        _launchLimits[0].blockNumber = block.number + 20;
        _launchLimits[1].blockNumber = block.number + 30;
        _launchLimits[2].blockNumber = block.number + 40;
        _launchLimits[3].blockNumber = block.number + 50;
    ...
    // the following funtions may be removed if not needed
    ...
    function setLiquidityFeePercent(uint16 newLiqTax) external onlyOwner {
    ...
    function setCharityFeePercent(uint16 newCharityTax) external onlyOwner {
    ...
    function setMarketingFeePercent(uint16 newMarketingTax) external onlyOwner {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(_coolDownEnabled) {
            if(from == _uniswapV2Pair) {
                require(_setCoolDown[to] <= block.number, "Cool down, dude!");
                _setCoolDown[to] = block.number + 3;
            } else if(to == _uniswapV2Pair) {
                require(_setCoolDown[from] <= block.number, "Cool down, dude!");
