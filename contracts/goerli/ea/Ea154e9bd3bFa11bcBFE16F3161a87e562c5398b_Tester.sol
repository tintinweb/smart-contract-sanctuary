...
contract Tester is PokeMeReady {
    ...
    function treasury(uint256 increment) external {
        count += increment;
    }

    function noTreasury() external {
        uint256 fee;
        address feeToken;

        (fee, feeToken) = IPokeMe(pokeMe).getFeeDetails();

        PokeMeReady._transfer(fee, feeToken);

        count += 1;
    }

    function checkerTreasury(uint256 increment)
        external
        view
        returns (bool canExec, bytes memory payload)
    {
        ...
        canExec = true;
        payload = abi.encodeWithSelector(this.treasury.selector, increment);
    }

    function checkerNoTreasury()
        external
        view
        returns (bool canExec, bytes memory payload)
    {
        // solhint-disable not-rely-on-time
        canExec = true;
        payload = abi.encodeWithSelector(this.noTreasury.selector);
    }
}