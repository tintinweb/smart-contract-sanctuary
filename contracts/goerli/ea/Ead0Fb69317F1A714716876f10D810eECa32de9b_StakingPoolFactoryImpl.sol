...
interface StakingPool is Staking, StakingPoolManagement {
    /// @notice Return the number of user in the pool
    /// @return number of users staking in the pool
    function getNumberOfStakers() external returns (uint256);
...
contract StakingPoolImpl is StakingPool {
    ...
    // @TODO to be removed -----------
    mapping(address => uint256) releasingBalance;
    // @TODO to be removed -----------

    address public poolManager;
    Fee public poolFee;
    mapping(uint256 => uint256) public epochReward; // using mapping to avoid initialization
    ...
    uint256 public currentMaturingTotal;
    uint256 public currentLockedTotal;
    ...
    /// @notice Return the number of user in the pool
    /// @return number of users staking in the pool
    function getNumberOfStakers() public pure override returns (uint256) {
        return 3;
    }
        ...
        returns (uint256 stakedBalance)
    {
        ...
        } else if (b.lockedEpoch + 1 == currentStakeEpoch) {
    ...
    function getReleasingTimestamp(address _userAddress)
        ...
        returns (uint256)
    {
        return block.timestamp + 43200; // 12hs
    }
    ...
    function getReleasingBalance(address _userAddress)
        ...
        returns (uint256)
    {
        return releasingBalance[_userAddress];
    }
    ...
    function produceBlock(uint256 _index) public override returns (bool) {
        ...
        epochReward[currentStakeEpoch] = epochReward[currentStakeEpoch].add(
            remainingReward
        );
    ...
    function unstake(uint256 _amount) public override {
        userBalance[msg.sender].maturedBalanceInvariant -= _amount;
        emit Unstake(msg.sender, _amount, block.timestamp + 43200);
    }
    ...
    function calcWeight() internal view returns (uint256) {
        ...
        uint256 totalValue =
            totalStaked.add(epochReward[currentStakeEpoch]).add(
                epochReward[currentStakeEpoch - 1]
            );
    ...
    {
        uint256 newValue =
            currentMaturingTotal.sub(epochReward[currentStakeEpoch - 1]);
    ...
    function cycleStakeMaturation() public override {
        ...
        currentMaturingTotal = currentLockedTotal.add(
            epochReward[currentStakeEpoch]
        );
        require(
            ctsi.approve(address(staking), currentMaturingTotal),
            "Failed to approve CTSI for staking contract"
        );
        staking.stake(currentMaturingTotal);
    ...
    function cycleWithdrawRelease() public override {
        this; // no opcode warning avoidance
    }
