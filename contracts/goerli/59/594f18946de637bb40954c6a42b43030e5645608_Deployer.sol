...
contract Panel is Ownable, ReentrancyGuard
{
	...
	function setRemoteBridge(uint256 _chainId, address _newRemoteBridge) external onlyOwner nonReentrant
	{
		require(chainId != _chainId, "invalid chain");
		require(_newRemoteBridge != address(0), "invalid operator");
		address _oldRemoteBridge = remotes[_chainId].bridge;
		remotes[_chainId].bridge = _newRemoteBridge;
		emit RemoteOperatorChange(_chainId, _oldRemoteBridge, _newRemoteBridge);
	}

	function setRemoteOperator(uint256 _chainId, address _newRemoteOperator) external onlyOwner nonReentrant
	{
...
library $
{
	...
	Network constant NETWORK = Network.Goerli;
...
{
	address constant NATIVE_TREASURY = 0xFDf35F1Bfe270e636f535a45Ce8D02457676e050;
	address constant PEGGED_TREASURY = 0xFDf35F1Bfe270e636f535a45Ce8D02457676e050; // G

	address constant NATIVE_DEFAULT_ADMIN = 0xFDf35F1Bfe270e636f535a45Ce8D02457676e050;
	address constant PEGGED_DEFAULT_ADMIN = 0xFDf35F1Bfe270e636f535a45Ce8D02457676e050; // S
	...
	$.Network constant NATIVE_NETWORK = $.Network.Kovan;
	uint256 constant NATIVE_TARGET_CHAIN_ID = 97; // bsctest
	uint256 constant NATIVE_TARGET_VARIABLE_FEE_RATE = 3e16; // 3%
	uint256 constant NATIVE_TARGET_FIXED_FEE_AMOUNT = 0e18; // 0 GRO

	$.Network constant PEGGED_NETWORK = $.Network.Bsctest;
	uint256 constant PEGGED_TARGET_CHAIN_ID = 42; // kovan
	...
	function deploy() external onlyOwner
	{
		...
		if (_nativeNetwork) {
			...
			registry = LibDeployer2.publishSignatureRegistry();
		}
		vault = LibDeployer2.publishTimeLockedVault();
		bridge = LibDeployer1.publishTrustedBridge(token);
		operator = LibDeployer2.publishOperator(bridge, vault);
		panel = LibDeployer1.publishPanel(bridge);
...
library LibDeployer1
{
	...
	function publishPanel(address _bridge) public returns (address _address)
	{
		bytes memory _bytecode = abi.encodePacked(type(Panel).creationCode);
		_address = Create2.deploy(0, bytes32(0), _bytecode);
		Panel(_address).construct($.chainId(), _bridge);
		return _address;
	}
}
