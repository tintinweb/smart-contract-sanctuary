...
contract ModelMintableERC721 is
ERC721,
AccessControlMixin,
...
ContextMixin
{
  ...
  function setTokenMetadata(uint256 tokenId, bytes memory data) internal virtual {
    ...
    (Modelnft memory _modelnft, string memory uri) = abi.decode(data, (Modelnft, string));

    Modelnft storage modelnft = ModelNFTMapping[tokenId];
    modelnft.name = _modelnft.name;
    modelnft.ipsToken = _modelnft.ipsToken;
    modelnft.description = _modelnft.description;
    modelnft.model = _modelnft.model;
    modelnft.id = _modelnft.id;
    modelnft.dataType = _modelnft.dataType;
