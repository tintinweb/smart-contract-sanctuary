...
contract CS3Presale is ERC1155 {
    ...
    /*** Actual presale buy and redeem functionality ***/

    // Buy assets of a single type/animal. The number of assets is determined from the amount of ETH sent.
    function buy(uint256 _type)
    external payable
    requireOpen
    {
        buyFor(_type, payable(msg.sender));
    }

    // Buy assets of a single type/animal. The number of assets is determined from the amount of ETH sent.
    function buyFor(uint256 _type, address payable _recipient)
    public payable
    requireOpen
    {
        ...
        if (msg.value > payAmount) {
            _recipient.sendValue(msg.value - payAmount);
        }
    }

    // Buy assets of a multiple types/animals at once.
    function buyBatch(uint256[] calldata _type, uint256[] calldata _count)
    external payable
    requireOpen
    {
