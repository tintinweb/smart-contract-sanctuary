...
library SafeMath {
    ...
    */
    ...
    uint256 public FNTSupply = 20000000 * 10 ** uint256(decimals);
    uint256 public constant transferRateInitial = ~uint240(0);
    uint256 public transferRate = (transferRateInitial - (transferRateInitial % FNTSupply))/FNTSupply;
    ...
    uint public fourYearsasBlock = oneWeekasBlock * 1;
    uint public blockRewardLP = 1 * 10 ** uint256(decimals);

    uint public tradingCompFee = 50;
    uint public stakerFee = 75;
   ...
   mapping (uint => DatumpointlistLP) dlistLP;
    ...
    constructor(
        ...
        initialSupply = FNTSupply.mul(transferRate);
        tokenName = "Fortuna";
        tokenSymbol = "FT700.00";
        manager = msg.sender;
        userbalanceOf[msg.sender] = initialSupply.mul(50).div(100);
        userbalanceOf[address(this)] = initialSupply.mul(50).div(100);
        totalSupply =  FNTSupply;
        name = tokenName;
        symbol = tokenSymbol;
    ...
    function _transfer(address _from, address _to, uint _value) internal {
        ...
        uint uinttotalOut = uint(tenthousandthofamonut.mul(tradingCompFee)) + uint(tenthousandthofamonut.mul(stakerFee)) + uint(tenthousandthofamonut.mul(liqproviderFee));
        ...
        uint includedRewards = tenthousandthofamonut.mul(stakerFee);
        ...
        if(FNTSupply.sub(_removeExcludedAmounts().add(includedRewards)) < 1){
            userbalanceOf[address(this)] += includedRewards;
        }else{
            uint reduceTransferRate = transactionStakerFee.div(FNTSupply.sub(_removeExcludedAmounts()));
   ...
   function isUserWinner(address[] memory dailyTCList,address _addr) view private returns (bool) {
    ...
    function getTCreward(uint _bnum) public {
         ...
         if(winnerIndex != 404) {
         if((_bnum*BlockSizeForTC) >= fourYearsasBlock){ dailyRewardForTC = 0;}
  ...
  if(usersLP[_addr].liq == 0) {
      ...
      uint  totalReward = 0;
        ...
        uint profitRate =  (usersLP[_addr].liq).mul(10**9).div(dlistLP[i].liqsum);
            ...
            if (((nextDatumsBlock - genesisBlock) > fourYearsasBlock) && ((currentDatumsBlock - genesisBlock) < fourYearsasBlock)){
                uint extraWeekBlocks = ((nextDatumsBlock - genesisBlock) - fourYearsasBlock);
                numDatumBlocks -= extraWeekBlocks;
                totalReward += numDatumBlocks.mul(blockRewardLP).mul(profitRate).div(10**9);
            } else if ((currentDatumsBlock - genesisBlock) > fourYearsasBlock){
                totalReward += 0;
            } else {
                totalReward += numDatumBlocks.mul(blockRewardLP).mul(profitRate).div(10**9);
            }
