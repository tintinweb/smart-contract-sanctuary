  ...
  ERC721BurnableUpgradeable
{
  ...
  event BaseURIUpdated(string baseURIOverride);
  event TokenCreatorPaymentAddressSet(address indexed paymentAddress, uint256 indexed tokenId);
  event NFTCreatorMigrated(address indexed originalAddress, address indexed newAddress);
  ...
  modifier onlyAdmin() {
    require(collectionFactory.adminRole().isAdmin(msg.sender), "CollectionContract: Caller does not have admin role");
    _;
  }

  modifier onlyCreator() {
    require(msg.sender == creator, "CollectionContract: Caller is not the creator");
    _;
  }

  function initialize(
    address payable _creator,
    // TODO init with optional payment address
    string memory _name,
    string memory _symbol
  ) external initializer {
    require(msg.sender.isContract(), "CollectionContract: Caller is not a contract");
  ...
  function mintWithCreatorPaymentFactory(
    ...
    address payable tokenCreatorPaymentAddress = collectionFactory
      .proxyCallContract()
      ._proxyCallAndReturnContractAddress(paymentAddressFactory, paymentAddressCallData);
    tokenId = mintWithCreatorPaymentAddress(tokenIPFSPath, tokenCreatorPaymentAddress);
  }
  ...
  // TODO account migration for creator, payment, owners, token payment using dual sig, anyone can broadcast
  ...
  function adminAccountMigration(
    ...
    require(creator == originalAddress, "NFT721Creator: Token was not created by the given address");
    creator = newAddress;
    emit NFTCreatorMigrated(originalAddress, newAddress);
  }
  ...
  function updateMaxTokenId(uint256 _maxTokenId) external onlyCreator {
  ...
  function _adminAccountRecoveryForPaymentAddresses(
    ...
    address payable originalPaymentAddress = collectionFactory.proxyCallContract()._proxyCallAndReturnContractAddress(
      paymentAddressFactory,
      paymentAddressCallData
    );
    ...
    address payable newPaymentAddress = collectionFactory.proxyCallContract()._proxyCallAndReturnContractAddress(
      paymentAddressFactory,
      paymentAddressCallData
    );
  ...
  function updateBaseURIOverride(string calldata _baseURIOverride) external onlyCreator {
  ...
  function getRoyalties(
    ...
    // TODO per-mint payment override
    // TODO global payment override
    recipients = new address payable[](1);
    recipients[0] = creator;
    feesInBasisPoints = new uint256[](1);
    feesInBasisPoints[0] = BASIS_POINTS;
  }
  ...
  {
    // TODO fallback to global default or to creator
  ...
  function _mint(string memory tokenIPFSPath) private onlyCreator returns (uint256 tokenId) {
    unchecked {
      tokenId = nextTokenId++;
      require(
        maxTokenId == 0 || nextTokenId <= maxTokenId,
        "CollectionContract: Max token count has already been minted"
      );
  ...
  function _setTokenCreatorPaymentAddress(uint256 tokenId, address payable tokenCreatorPaymentAddress) internal {
    ...
    emit TokenCreatorPaymentAddressSet(tokenCreatorPaymentAddress, tokenId);
  }

  function _baseURI() internal view override returns (string memory) {
    if (bytes(baseURIOverride).length > 0) {
...
interface ICollectionFactory {
  function adminRole() external returns (IAdminRole);
...
library AccountMigration {
  ...
  function requireAuthorizedAccountMigration(
    ...
    // TODO switch to Foundation signature
