...
contract PreSalePool is Ownable, ReentrancyGuard, Pausable, RedKiteWhitelist {
    ...
    // Amount of user refund token
    uint256 public totalRefundToken = 0;
    ...
    event RefundToken(address user, uint256 tokenAmount, address currency);
    event ClaimRefund(address user, uint256 tokenAmount, uint256 currencyAmount, address currency);
    ...
    function buyTokenByEtherWithPermission(
        ...
        require(userRefundToken[_candidate].tokenAmount == 0, "POOL::USER_REFUNDED");
    ...
    */
    function refundTokens(address _candidate, address _currency, bytes memory _signature) nonReentrant public {
        require(userClaimed[_candidate] == 0 && userPurchased[_candidate] > 0 && _verifyRefundToken(_candidate, _currency, _signature), "POOL::NOT_ALLOW_TO_REFUND");
        require(investedAmountOf[_currency][_candidate] > 0, "POOL::NOT_ALLOW_CURRENCY_TO_REFUND");

        userRefundToken[_candidate] = UserRefundToken({
            tokenAmount: userPurchased[_candidate],
            currency: _currency,
            isClaimed: false
        });

        uint256 currencyAmount = _getTokenAmountToOfferedCurrency(_currency, userPurchased[_candidate]);
        totalRefundToken = totalRefundToken.add(userPurchased[_candidate]);
        totalUnclaimed = totalUnclaimed.sub(userPurchased[_candidate]);
        tokenSold = tokenSold.sub(userPurchased[_candidate]);
        userPurchased[_candidate] = 0;
        weiRaised = weiRaised.sub(currencyAmount);
        investedAmountOf[_currency][_candidate] = 0;

        emit RefundToken(_candidate, userPurchased[_candidate], _currency); 
    }
    ...
    */
    function claimRefundTokens(address _candidate, address _currency, bytes memory _signature) nonReentrant public {
        require(userRefundToken[_candidate].tokenAmount > 0 && !userRefundToken[_candidate].isClaimed && _verifyRefundToken(_candidate, _currency, _signature), "POOL::NOT_ALLOW_TO_CLAIM");

        totalRefundToken = totalRefundToken.sub(userRefundToken[_candidate].tokenAmount);

        userRefundToken[_candidate].isClaimed = true;

        uint256 claimAmount = _getTokenAmountToOfferedCurrency(_currency, userRefundToken[_candidate].tokenAmount);
        ...
        emit ClaimRefund(_candidate, userRefundToken[_candidate].tokenAmount, claimAmount, _currency);
    }

    /**
     * @notice Number of currency need to be deposit into contract to user claim refund token
    */
    function getTotalCurrencyRefund(address _currency) public view returns (uint256){
        return _getTokenAmountToOfferedCurrency(_currency, totalRefundToken);
    }
    ...
    /**
     * @dev Override to extend the way in which tokens is converted to currency.
     * @param _tokenAmount Value in wei to be converted into currency
     * @return Number of currency that can be receipt after refund with the specified _tokenAmount
    */
    function _getTokenAmountToOfferedCurrency(address _currency, uint256 _tokenAmount)
        internal
        view
        returns (uint256)
    {
        uint256 rate = getOfferedCurrencyRate(_currency);
        uint256 decimals = getOfferedCurrencyDecimals(_currency);
     ...
     * @param _tokenAmount Number of tokens to be emitted
     */
    function _deliverCurrency(address _currency, address _beneficiary, uint256 _tokenAmount)
        internal
    {
        if(_currency == address(0)){
            _transfer(_beneficiary, _tokenAmount);
        } else {
            IERC20(_currency).transfer(_beneficiary, _tokenAmount);
        }
    }
    ...
    function _verifyRefundToken(
        ...
        return (verifyRefundToken(signer, _candidate, _currency, _signature));
    }
}
...
contract RedKiteWhitelist {
    ...
    function verifyRefundToken(
        ...
        bytes32 messageHash = getRefundMessageHash(_candidate, _currency);
        bytes32 ethSignedMessageHash = getEthSignedMessageHash(messageHash);
