  ...
  ReentrancyGuardUpgradeable // Adding this unused mixin to help with linearization
{
  ...
  /**
   * @dev Returns an alternate destination address for any payments to the creator.
   * If address(0) is returned then the creator's address should be used instead.
   */
    ...
    returns (address payable, address payable)
  {
    ...
    if (creator != address(0)) {
      try IFNDNFT721(nftContract).tokenPaymentAddress(tokenId) returns (address payable paymentAddress) {
        return (creator, paymentAddress);
  ...
  SendValueWithFallbackWithdraw
{
  ...
  {
    // The creatorPaymentAddress replaces the creator as the fee recipient if defined.
    (address payable creator, address payable creatorPaymentAddress) =
    ...
    if (_getIsPrimary(nftContract, tokenId, creator, seller)) {
      ...
      if (creatorPaymentAddress == address(0)) {
        ...
        ownerRevTo = creatorPaymentAddress;
      }
    } else {
      ...
      if (creatorPaymentAddress != address(0)) {
        ...
        creatorSecondaryFeeTo = creatorPaymentAddress;
      }
      ownerRevTo = seller;
    }
    // SafeMath is not required when dividing by a constant value > 0.
    ...
    )
  {
    ...
    if (creatorFee > 0) {
      if (creatorFeeTo == address(0)) {
        // If the creator payment address is unknown, send all revenue to the current seller instead.
        ownerRev = ownerRev.add(creatorFee);
        creatorFee = 0;
      } else {
        _sendValueWithFallbackWithdraw(creatorFeeTo, creatorFee);
      }
    }
    _sendValueWithFallbackWithdraw(ownerRevTo, ownerRev);
  }
  ...
  NFTMarketAuction
{
  ...
  function finalizeReserveAuction(uint256 auctionId) public nonReentrant {
    ...
    // TODO: Look up the the payment address for this auction seller or
    // modify the auction struct to contain this info
...
interface IFNDNFT721 {
  ...
  function tokenPaymentAddress(uint256 tokenId) external view returns (address payable);
}
