...
interface ICrossChainMigrator {
    function migrate(IERC20 token, uint256 pid) external returns (IERC20);
}
...
contract MasterChef is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        bool everDeposited; // Become true, if the user has ever deposited LP tokens
        //
    ...
    // The crosschain migrator contract. It has a lot of power. Can only be set through governance (owner).
    ICrossChainMigrator public crossChainMigrator;
    ...
    mapping(uint256 => address[]) private poolAccountList;
    ...
    event PoolCrossChainMigrated(uint256 indexed pid);
    event CrossChainMigratorUpdated(
        ICrossChainMigrator indexed newCrossChainMigrator
    );
    ...
    // Set the crosschain migrator contract. Can only be called by the owner.
    function setCrossChainMigrator(ICrossChainMigrator _crossChainMigrator)
        public
        onlyOwner
    {
        crossChainMigrator = _crossChainMigrator;
        emit CrossChainMigratorUpdated(_crossChainMigrator);
    }

    // Cross chain Migrate lp token to another chain. Can be called by anyone. We trust that crosschain migrator contract is good.
    function crossChainMigrate(uint256 _pid) public whenPaused {
        require(
            address(crossChainMigrator) != address(0),
            "MasterChef: no crosschain migrator"
        );

        IERC20 lpToken = poolInfo[_pid].lpToken;
        uint256 bal = lpToken.balanceOf(address(this));
        lpToken.safeApprove(address(crossChainMigrator), bal);
        crossChainMigrator.migrate(lpToken, _pid);
        emit PoolCrossChainMigrated(_pid);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public whenNotPaused {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accCNTPerShare).div(1e12).sub(
                    user.rewardDebt
                );
            if (pending > 0) {
        ...
        // data migration
        if (!user.everDeposited) {
            user.everDeposited = true;
            poolAccountList[_pid].push(msg.sender);
        }
    ...
    function totalFarmersInPool(uint256 _pid)
        external
        view
        whenPaused
        returns (uint256)
    {
        return poolAccountList[_pid].length;
    }

    function getPoolAccountList(uint256 _pid)
        external
        view
        whenPaused
        returns (address[] memory)
    {
        return poolAccountList[_pid];
    }

    function finalizeUserMigration(uint256 _pid, address _user)
        external
        whenPaused
        returns (uint256, bool)
    {
        require(
            msg.sender == address(crossChainMigrator),
            "MasterChef: not from CrossChain Migrator"
        );
        UserInfo memory user = userInfo[_pid][_user];
        if (!user.everDeposited) return (0, false);
        uint256 pending =
            user.amount.mul(poolInfo[_pid].accCNTPerShare).div(1e12).sub(
                user.rewardDebt
            );
        if (pending > 0) {
            safeCNTTransfer(_user, pending);
        }
        return (user.amount, true);
    }
