...
/* Specializing in locking LP Tokens to provide high quaility products centered around LP Lock.
...
contract ArbiLiqLock is Ownable {
    ...
    uint256 private _arbiLockTokenBurn = 10000;
    ...
    function lockTokensStandard(address _sushiPair, uint256 _epoch, address _tokenPayout, uint256 _amount) external payable {
        ...
        require(SushiV2Pair(_arbiLock).balanceOf(_sushiPair) >= _arbiLockTokenBurn.sub(_arbiLockTokenBurn.div(20)) * 10 ** 9, "Your LP Pair must hold the specified amount of ArbiLock Tokens");
    ...
    function addLPToLock(address _sushiPair, uint256 _amount) external {
        require(msg.sender == _PayoutAddress[_sushiPair]);
        require(Factory.getPair(SushiV2Pair(_sushiPair).token0(), SushiV2Pair(_sushiPair).token1()) == _sushiPair, "Please only deposit SushiV2 tokens");
        require(SushiV2Pair(_sushiPair).balanceOf(address(this)) > 0, "No tokens to add to!");
        SushiV2Pair(_sushiPair).transferFrom(address(msg.sender), address(this), _amount);
    }
    ...
    function getLockedTokens(address _sushiPair) external view returns (bool Locked, uint256 ReleaseDate, address PayoutAddress, bool Burned) {
        if(block.timestamp < _PairRelease[_sushiPair])
            return (true, _PairRelease[_sushiPair], _PayoutAddress[_sushiPair], _PairBurned[_sushiPair]);
        return (false, _PairRelease[_sushiPair], _PayoutAddress[_sushiPair], _PairBurned[_sushiPair]);
    }
