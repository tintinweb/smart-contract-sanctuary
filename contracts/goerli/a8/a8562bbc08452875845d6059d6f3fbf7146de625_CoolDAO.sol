...
contract CoolDAO is ERC20, EIP712 {
    ...
    constructor(string memory _name, string memory _symbol, address _signer) ERC20(_name, _symbol) EIP712("CoolDAO", "1") {
    ...
    function claim(uint256 amountV, bytes32 r, bytes32 s) external {
        ...
        require(total <= MAX_SUPPLY, "CoolDAO: Exceed max supply");
        require(minted(msg.sender) == 0, "CoolDAO: Claimed");
        bytes32 digest = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", 
            ECDSA.toTypedDataHash(_domainSeparatorV4(),
                keccak256(abi.encode(MINT_CALL_HASH_TYPE, msg.sender, amount))
        )));
        require(ecrecover(digest, v, r, s) == cSigner, "CoolDAO: Invalid signer");
        _totalSupply = total;
