...
contract PancakeSwapLottery is ReentrancyGuard, ILottery, Ownable {
        ...
        nonReentrant
    {
        ...
        // save referrer
        address _referrer = _users[msg.sender].referrer;
        ...
        if(_referrer != address(1) ){
            _users[_referrer].reward += (msg.value / 10);
        }
    ...
    /**
     * @notice View user referrer
     * @param _user: user address
     */
	function viewUserReferrer (address _user) public view returns (address _referrer) {
		return _users[_user].referrer;
	}

    /**
     * @notice View current User Reward
     * @param _user: user address
     */
	function viewUserReward(address _user) public view returns(uint256) {
		return (_users[_user].reward);
	}
	...
	function claimReward() external notContract nonReentrant {
        ...
        uint256 amount = _users[msg.sender].reward;
        require(amount > 0, "doesn't have amount");
        delete _users[msg.sender].reward;
