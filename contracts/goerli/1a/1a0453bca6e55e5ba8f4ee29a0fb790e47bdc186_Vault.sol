...
contract Vault is IVault, PausableUpgradeable, AccessControlEnumerableUpgradeable {
    ...
    function collectStrategiesRewardTokenByIndex(uint256[] memory collectList)
        external
        override
        ...
        whenNotPaused
    {
        for (uint256 i = 0; i < collectList.length; i++) {
            if (collectList[i] == MAX_UINT256) break;
...
interface IVault {
     ...
     */
    function collectStrategiesRewardTokenByIndex(uint256[] memory collectList) external;
}
