...
contract MSActivityCenter is ProxyAdmin {
    ...
    struct Charge {
        address receiver;
        uint256 partion; // 分潤百分比 !!! 還要檢查總合是不是100
    }
    ...
    modifier checkSale(uint256 activityId, uint256 tokenQuantity) {
        ...
        require(
            activities[activityId].price != 0,
            "MSActivityCenter: Price cannot be 0"
        );
        require(
            tokenQuantity > 0,
            "MSActivityCenter: TokenQuantity cannot be 0"
        );
        require(
    ...
    function setActivity(
        ...
        require(_price != 0, "MSActivityCenter: Price cannot be");
    ...
    function _mint(
        ...
        for (uint256 i = 0; i < _tokenQuantity; i++) {
            uint256 tokenId = nft.mint(_to, _activityId);
            emit Sale(
                _activityId,
    ...
    function chargeEth(uint256 _activityId, uint256 _tokenQuantity)
        internal
    {
        for (uint256 i = 0; i < activityCharges[_activityId].length; i++) {
            uint256 partion = activityCharges[_activityId][i].partion / 100;
            uint256 chargeValue = activities[_activityId].price * _tokenQuantity * partion;
            (bool success, ) = activityCharges[_activityId][i].receiver.call{value: chargeValue}("");
            require(success, "MSActivityCenter: Failed to send Ether");
            emit RevenueShare(activityCharges[_activityId][i].receiver, chargeValue);
        }
    }
    ...
    function checkChargePartion(Charge[] calldata _charges) internal pure {
        ...
        require(sum == 100, "MSActivityCenter: Profit is partitioned error");
    }
}
