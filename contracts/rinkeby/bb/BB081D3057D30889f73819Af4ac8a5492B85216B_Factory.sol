...
contract Collection is ERC1155, AccessControl {
    ...
    EnumerableSet.UintSet soldSeeds;
    string public seedName;
    ...
    function addCollecAndWeig(uint256[] memory tokenIds, uint64[] memory weights) public factoryOrAdmin {
        require(tokenIds.length == weights.length, "NftStakingMock: inconsistent array lengths");
    ...
    function mint(address _to, uint256 _id) external factoryOrAdmin {
        require(!(soldSeeds.contains(_id)), "This card already sold");
        ...
        emit Minted(address(this), _to, _id);
    }

    function mintBatch(address _to, uint256[] calldata _ids,  uint256[] calldata amount_) external factoryOrAdmin {
        require(available > _ids.length, "Sold Out");
        
        for(uint256 i=0; i < _ids.length; i++){
            require(!(soldSeeds.contains(_ids[i])), "This card already sold");

        }
       

        _mintBatch(_to, _ids, amount_, "");

        available -= _ids.length;
        sold += _ids.length;

        for(uint256 i=0; i < _ids.length; i++){
            soldSeeds.add(_ids[i]);
       
        }
        
        emit MintedBatch(address(this), _to, _ids);
    }
...
import "./mocks/staking/NftStakingMock.sol";
...
contract Factory is AccessControl {
    ...
    NftStakingMock stakeMock;

    bytes32 public constant COLLECTION_ROLE = bytes32(keccak256("COLLECTION_ROLE"));
  
    modifier onlyAdmin {
      require(hasRole(DEFAULT_ADMIN_ROLE,msg.sender),"This function can only be called by admin");
      _;
    }
    ...
    function createCollection(
        ...
        require(_total == _tokenIds.length && _total == _weights.length,"total seed should be equal list length");
        
        Collection collection =
            new Collection(_uri, _seedName, _total, _admin, address(this));

        collections.push(collection);

        seeds[address(collection)] = Seed(
            _total,
            _uri,
            _seedName,
            _tokenIds,
        ...
        NftStakingV2 nftStake = NftStakingV2(_stakingAdd);
        nftStake.addWhitelist(address(collection));

        //stakeMock = NftStakingMock(_stakeMockAdd);
        //stakeMock.addCollecAndWeig(address(collection), _stakeMockAdd,_tokenIds,_weights);
        collection.addCollecAndWeig(_tokenIds, _weights);

        emit CollectionCreated(address(collection), _uri, _seedName, _total, _tokenIds, _weights);
     ...
     function mint(address _collecAdd, address account, uint _id) onlyAdmin() external {
         Collection collection = Collection(_collecAdd);
         
         collection.mint(account, _id);
         
     }

    function mintBatch(address _collecAdd, address account, uint256[] calldata _ids, uint256[] calldata amount) onlyAdmin() external {
         Collection collection = Collection(_collecAdd);
         
         collection.mintBatch(account, _ids, amount);
...
contract NftStakingMock is NftStakingV2{
    ...
    function getWeight(address collecAdd) public returns(uint256){
       collection = Collection(collecAdd);
       return collection.weightByTokenAttribute(1);
    }
    

    

    function _validateAndGetNftWeight(uint256 nftId, address collecAdd) internal virtual override returns (uint64) {
        //uint256 tokenType = nftId >> 240 & 0xFF;
        //require(tokenType == 1, "NftStakingMock: Wrong NFT type");
        //uint256 attributeValue = nftId >> 176 & 0xFF;
        collection = Collection(collecAdd);
        return collection.weightByTokenAttribute(nftId);
...
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "../mocks/token/ERC20Mock.sol";
import "@animoca/ethereum-contracts-assets_inventory/contracts/token/ERC1155/ERC1155TokenReceiver.sol";
import "@openzeppelin/contracts/access/AccessControl.sol";


abstract contract NftStakingV2 is ERC1155TokenReceiver, AccessControl {
    ...
    event NftsBatchStaked(address staker, uint256 cycle, address collection, uint256[] tokenIds, uint256[] weights);

    event NftUnstaked(address staker, uint256 cycle, address collection, uint256 tokenId, uint256 weight);
    event NftsBatchUnstaked(address staker, uint256 cycle, address collection, uint256[] tokenIds, uint256[] weights);
    ...
    mapping(address => TokenId) tokenInfByCollec;
    ...
    constructor(
        ...
        require(periodLengthInCycles_ >= 1, "NftStaking: invalid period length");
    ...
    function withdrawLostCycleRewards(address to, uint16 cycle, int256 globalSnapshotIndex) external onlyAdmin {
    ...
    function onERC1155Received(
        ...
        for (uint256 i=0; i<whitelistedNftContract.length; i++){
            if(address(whitelistedNftContract[i]) == _msgSender()){
               collecAdd = _msgSender();
    ...
    function unstakeNft(uint256 tokenId, address collecAdd) external virtual {
        ...
        if (enabled) {
            ...
            tokenInfByCollec[collecAdd].tokenInfos[tokenId] = tokenInfo;
        }
        IERC1155721Transferrable  _whitelistedNftContract;
        _whitelistedNftContract = IERC1155721Transferrable(collecAdd);
        
        _whitelistedNftContract.safeTransferFromWithFallback(address(this), _msgSender(), tokenId, 1, "");
        emit NftUnstaked(_msgSender(), currentCycle, collecAdd, tokenId, weight);
    ...
    function batchUnstakeNfts(uint256[] calldata tokenIds, address collecAdd) external {
        ...
        _whitelistedNftContract.safeBatchTransferFromWithFallback(address(this), _msgSender(), tokenIds, values, "");
           
        _onUnstake(_msgSender(), uint256(totalUnstakedWeight));
        emit NftsBatchUnstaked(_msgSender(), currentCycle, collecAdd, tokenIds, weights);
    }
    ...
    {
        (ComputedClaim memory claim, ) = _computeRewards(_msgSender());
    ...
    function claimRewards() external isEnabled hasStarted {
        ...
        (ComputedClaim memory claim, NextClaim memory newNextClaim) = _computeRewards(_msgSender());
        ...
        if (claim.amount != 0) {
            totalRewards[_msgSender()] = totalRewards[_msgSender()].add(claim.amount);
    ...
    function _stakeNft(uint256 tokenId, address owner, address collecAdd) internal isEnabled hasStarted {
        uint64 weight = _validateAndGetNftWeight(tokenId, collecAdd);
    ...
    function _batchStakeNfts(uint256[] memory tokenIds, address owner, address collecAdd) internal isEnabled hasStarted {
        uint256 numTokens = tokenIds.length;
        require(numTokens != 0, "NftStaking: no tokens");
        ...
        for (uint256 index = 0; index < numTokens; ++index) {
            uint256 tokenId = tokenIds[index];
            require(currentCycle != tokenInfByCollec[collecAdd].tokenInfos[tokenId].withdrawCycle, "NftStaking: unstaked token cooldown");
            uint64 weight = _validateAndGetNftWeight(tokenId,collecAdd);
        ...
        emit NftsBatchStaked(owner, currentCycle, collecAdd, tokenIds, weights);
    ...
    function _validateAndGetNftWeight(uint256 tokenId, address collecAdd) internal virtual returns (uint64);
