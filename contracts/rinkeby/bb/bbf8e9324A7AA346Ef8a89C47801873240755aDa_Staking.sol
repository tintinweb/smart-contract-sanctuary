...
contract Staking {
    IERC20 public token;
    uint public depositedETH;

    mapping(address => uint) public depositAmount;

    event TokenDeposit(address sender, uint amount);
    event TokenWithdraw(address sender, uint amount);
    event ETHDeposit(address sender, uint amount);
    event ETHWithdraw(address sender, uint amount);
