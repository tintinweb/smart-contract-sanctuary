   ...
   // address cheethAddress;
        ...
        returns (string memory)
    {
        ...
        for (uint8 i = 0; i < 9; i++) {
           ...
           if(traitTypes[i].length <= thisTraitIndex)
             continue;
           svgString = string(abi.encodePacked(svgString, traitTypes[i][thisTraitIndex].desc));
        }

        svgString = string(
            abi.encodePacked(
                '<svg id="mouse-svg" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="131px" height="215px" viewBox="0 0 131 215" enable-background="new 0 0 131 215" xml:space="preserve"> ',
                svgString,
                "</svg>"
            )
        );
    ...
    function addTraitType(uint256 _traitTypeIndex, string memory _traitType, string memory _traitName, string memory _desc)
        public
        onlyOwner{
    ...
    function getTraitType(uint256 _traitTypeIndex, uint256 _subIdx) public view returns (Trait memory){
        return traitTypes[_traitTypeIndex][_subIdx];
    }
