...
contract Trade {
    ...
    // uint8 to uint
    event SellerFee(uint sellerFee);
    ...
    function getFees(address seller ,uint paymentAmt, BuyingAssetType buyingAssetType, address buyingAssetAddress, uint tokenId) internal view returns(Fee memory){
        ...
        royaltyFee = seller != tokenCreator ? price.mul(royaltyPermille).div(1000) 
                                            : 0 ;
        assetFee = price.sub(royaltyFee).sub(sellerFee);
        return Fee(platformFee, assetFee, royaltyFee, price, tokenCreator);
    }

    function tradeAsset(Order memory order, Fee memory fee, bool status) internal virtual {
        ...
        if(fee.platformFee > 0 ) {
            if(status) {
                ...
                emit SellerFee(fee.platformFee);
            }
            else if(!status) {
    ...
    function buyAsset(Order memory order, Sign memory sign) public payable returns(bool) {
        Fee memory fee;
        fee = order.directPay ? getFees(order.seller, msg.value, order.nftType, order.nftAddress, order.tokenId)
