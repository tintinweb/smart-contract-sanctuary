...
contract ScriptStorageNFT77 is ERC721, Pausable, AccessControl, ERC721Burnable {
    ...
    constructor() ERC721("NFT Storage 22", "NF4") {
...
//    }
    ...
    function formatSVG (uint256 tokenId) internal view returns (string memory){
        ...
        string memory hashyString = string(abi.encodePacked("<svg id='mySvg' onload='loadUp()' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' shape-rendering='crispEdges' xmlns:v='https://vecta.io/nano'><script><![CDATA[let tokenData={'hash':'",toHexA(tokenId),"'};"));
...
//slices a string up begin being the starting character in the string, end being the stop point string 'abcdef' w start of 1 and end of 3 would return 'abc' start of 2 end of 6 would return bcdef
    ...
    function formatTokenURI(string memory fullString) internal pure returns (string memory) {
        return string(abi.encodePacked("data:application/json;base64,",Base64.encode(bytes(abi.encodePacked('{"name":"sweet green baby boy", "description":"love this little guy", "image":"',fullString,'"}')))));
    }
