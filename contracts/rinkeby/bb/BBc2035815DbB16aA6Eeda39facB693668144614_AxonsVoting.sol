...
contract AxonsVoting is IAxonsVoting, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function vote(bool[10] memory votes, uint256 auctionCounter) public {
        ...
        require(axonBalance > 0 || IAxonsToken(axonsToken).isGenesisAddress(msg.sender) || IERC721Enumerable(filaments).balanceOf(msg.sender) > 0, 'Must own a Filament or Axon or be a genesis $AXON holder to vote');
        ...
        uint256 filamentBalance = IERC721Enumerable(filaments).balanceOf(msg.sender);
        for (uint i=0; i<filamentBalance; i++) {
            uint256 filamentId = IERC721Enumerable(filaments).tokenOfOwnerByIndex(msg.sender, i);
            require(filamentIdVotesForAuctionNumber[auctionCounter][filamentId] == false, 'Filament already voted today');
