...
contract MockSpaceStakeTreasury is Ownable {
  using SafeMath for uint256;
  ...
  address public sSPST;
  uint256 public sSPSTQueue; // Delays change to sSPST address
  ...
  constructor(
    ...
    // isLiquidityToken[ _SPSTDAI ] = true;
    // liquidityTokens.push( _SPSTDAI );

    blocksNeededForQueue = _blocksNeededForQueue;
  }

  /**
        @notice allow approved address to deposit an asset for SPST
        @param _amount uint
  ...
  function deposit(
    ...
    (_token, _amount);
    // mint SPST needed and store amount of rewards for distribution
    send_ = value.sub(_profit);
    IERC20Mintable(SPST).mint(msg.sender, send_);
  ...
  /**
        @notice allow approved address to burn SPST for reserves
        @param _amount uint
        @param _token address
     */
  function withdraw(uint256 _amount, address _token) external {
    ...
    uint256 value = valueOfToken(_token, _amount);
    ISPSTERC20(SPST).burnFrom(msg.sender, value);
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sSPST).balanceOf(msg.sender); // Can only borrow against sSPST held
  ...
  /**
        @notice allow approved address to repay borrowed reserves with SPST
        @param _amount uint
     */
  function repayDebtWithSPST(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");

    ISPSTERC20(SPST).burnFrom(msg.sender, _amount);

    debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
    totalDebt = totalDebt.sub(_amount);

    emit RepayDebt(msg.sender, SPST, _amount, _amount);
  }
  ...
  function mintRewards(address _recipient, uint256 _amount) external {
    ...
    IERC20Mintable(SPST).mint(_recipient, _amount);
  ...
  function excessReserves() public view returns (uint256) {
    return totalReserves.sub(IERC20(SPST).totalSupply().sub(totalDebt));
  }
  ...
  /**
        @notice returns SPST valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
  ...
  {
    if (isReserveToken[_token]) {
      // convert amount to match SPST decimals
      value_ = _amount.mul(10**IERC20(SPST).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SSPST) {
      // 9
      sSPSTQueue = block.number.add(blocksNeededForQueue);
    } else return false;
