...
contract Bless is ERC20Burnable, Ownable {
    ...
    // Tracks block specific ID was staked for reward tracking purposes
    mapping(uint256 => uint256) internal tokenIdToTimeStamp;

    // Maps ID -> address to track ownership while NFT is staked
    mapping(uint256 => address) internal tokenIdToStaker;

    //Mapping of staker to mice
    mapping(address => uint256[]) internal stakerToTokenIds;

    constructor(address _PHC) ERC20("BLESS", "BLESS") {
        PHC = _PHC;
    }

    /*
        Stakes array of tokenID's - all specified ID's must be owner by function caller and not already staked
    */
    function stakeByIds(uint256[] memory tokenIds) public {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require( IERC721(PHC).ownerOf(tokenIds[i]) == msg.sender, "Must own NFT with specified Token ID(s)");
            require( tokenIdToStaker[tokenIds[i]] == nullAddress, "Token ID(s) already staked");

            IERC721(PHC).transferFrom(msg.sender,address(this),tokenIds[i]);
    ...
    function unstakeAll() public {
        ...
        for (uint256 i = stakerToTokenIds[msg.sender].length; i > 0; i--) {
            ...
            IERC721(PHC).transferFrom( address(this), msg.sender, tokenId );
    ...
    function setPHCAddress(address _PHC) public onlyOwner {
        PHC = _PHC;
    }

    function setEmmissionRate(uint _rate) public onlyOwner {
    ...
    function getTokensStaked(address _staker) public view returns (uint256[] memory) {
