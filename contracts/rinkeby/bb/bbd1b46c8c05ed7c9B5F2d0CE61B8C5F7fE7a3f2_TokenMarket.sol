...
contract TokenMarket is TokenMarketBase {
    ...
    */
    ...
    function activateLoan(uint256[] memory loanIds, uint256[] memory stableCoinAmounts, bool[] memory _autoSell) public {
        ...
        totalApyFee = totalApyFee + this.getAPYFee(loanOffersToken[loanIds[i]]);
        loanAmountAfterAPYFee = loanAmountAfterAPYFee + loanOffersToken[loanIds[i]].loanAmountInBorrowed.sub(totalApyFee);

        stableCoinAPYFeeFromToken[loanOffersToken[loanIds[i]].borrowStableCoin] =  stableCoinAPYFeeFromToken[loanOffersToken[loanIds[i]].borrowStableCoin].add(totalApyFee);

        //approving token from the front end
        //keep the APYFEE  to govworld  before  transfering the stable coins to borrower.
        IERC20(loanOffersToken[loanIds[i]].borrowStableCoin).transferFrom(msg.sender, address(this), totalApyFee);
