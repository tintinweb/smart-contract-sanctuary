    ...
    Ownable
{
    ...
    // Lion price 0.04 ETH
    uint256 public constant drrtPrice = 40000000000000000;

    // Maximum purchase at once
    uint256 public constant maxPurchase = 20;

    // Maximum supply
    uint256 public constant maxDRRT = 9000;
    ...
    function mintLion(uint256 numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint ddrt");
        require(numberOfTokens > 0, "At least one lion must be minted");
        require(
            numberOfTokens <= maxPurchase,
            "Maximum 20 ddrt can be minted at once"
        );
        require(
            totalSupply().add(numberOfTokens) <= maxDRRT,
            "Purchase would exceed the max supply of ddrt"
        );
        require(
            msg.value >= drrtPrice.mul(numberOfTokens),
    ...
    function reserveddrt(address to, uint256 numberOfTokens) public onlyOwner {
        uint256 supply = totalSupply();

        require(numberOfTokens > 0, "At least one lion must be reserved");
        require(
            numberOfTokens <= reserve,
            "There's not enough ddrt left in reserve"
        );
        require(
            numberOfTokens <= reserveLimit,
            "Only maximum 30 ddrt can be reserved at once"
        );
