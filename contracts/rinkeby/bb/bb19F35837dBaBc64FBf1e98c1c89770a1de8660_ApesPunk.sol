...
contract ApesPunk is Ownable, ERC721Enumerable, ReentrancyGuard {
    ...
    uint256 public dumbMintCount = 0;
    ...
    function mintId(uint256 id) public payable nonReentrant() {
        require(isMintOn, "Sale hasn't started.");
        require(!_exists(id), "ID already exists");
        uint256 totalSupply = totalSupply();
        require(
            totalSupply + 1 <= numTotalPhunks(),
            "There aren't this many phunks left."
        );
        uint256 costForMintingPhunks = getCostForMintingPhunks(1);
        require(
            msg.value >= costForMintingPhunks,
            "Too little sent, please send more eth."
        );
