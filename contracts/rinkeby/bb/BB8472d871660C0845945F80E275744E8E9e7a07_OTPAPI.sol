...
contract OTPAPI is ChainlinkClient {
    ...
    constructor() public {
        ...
        jobId = "0a546a9302454524a4dec579c50b0579";
        fee = 0.01 * 10 ** 18; // (Varies by network and job)
        owner = msg.sender;
    }
    
    function requestOTP(string memory mail) public returns (bytes32 requestId) 
    {
        Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);
        
        // Set the URL to perform the GET request on
        request.add("post", "https://testapi.owens.market/user/verification-code/1");
        request.add("email",mail);
