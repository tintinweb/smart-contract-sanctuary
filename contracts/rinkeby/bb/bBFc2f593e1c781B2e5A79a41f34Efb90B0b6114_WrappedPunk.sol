...
contract WrappedPunk is Ownable, ERC721, Pausable {
    ...
    uint16 constant public totalPunkSupply = 10000; // supply is one more than in reality because of a hidden 0 index punk

    // Mapping from user address to proxy address
    mapping(address => address) private _proxies;

    // get an address from a punkIndex
    mapping (uint16 => address) public punkIndexToAddress;

    // array of wrapped punks, this is redundant with punkIndexToAddress but aids in data retrieval
    address[totalPunkSupply] public punksArray;
    ...
    mapping (uint16 => Offer) public punksOfferedForSale;

    // A record of the highest punk bid
    mapping (uint16 => Bid) public punkBids;

    mapping (address => uint) public pendingWithdrawals;

    event PunkOffered(uint16 indexed sigilIndex, uint minValue, address indexed toAddress);
    event PunkBidEntered(uint16 indexed sigilIndex, uint value, address indexed fromAddress);
    event PunkBidWithdrawn(uint16 indexed sigilIndex, uint value, address indexed fromAddress);
    event PunkBought(uint16 indexed sigilIndex, uint value, address indexed fromAddress, address indexed toAddress);
    event PunkNoLongerForSale(uint16 indexed sigilIndex);
   ...
   */
    ...
    whenNotPaused
    {
        super._transfer(from, to, punkIndex);
    }

    function punkNoLongerForSale(uint16 punkIndex) public whenNotPaused {
    ...
    function offerPunkForSale(uint16 punkIndex, uint minSalePriceInWei) public whenNotPaused {
        require(punkIndexToAddress[punkIndex] == msg.sender);
        require(punkIndex <= totalPunkSupply);
    ...
    function offerPunkForSaleToAddress(uint16 punkIndex, uint minSalePriceInWei, address toAddress) public whenNotPaused {
        require(punkIndexToAddress[punkIndex] == msg.sender);
        require(punkIndex <= totalPunkSupply);
    ...
    function enterBidForPunk(uint16 punkIndex) public payable whenNotPaused {
        require(punkIndex <= totalPunkSupply);
    ...
    function acceptBidForPunk(uint16 punkIndex, uint minPrice) public whenNotPaused {
        require(punkIndex <= totalPunkSupply);
        require(punkIndexToAddress[punkIndex] == msg.sender);
        ...
        punksArray[punkIndex] = bid.bidder;
        _transferFrom(seller, bid.bidder, punkIndex);
    ...
    function withdrawBidForPunk(uint16 punkIndex) public {
        require(punkIndex <= totalPunkSupply);
        require(punkIndexToAddress[punkIndex] != address(0x0));
        require(punkIndexToAddress[punkIndex] != msg.sender);
        Bid memory bid = punkBids[punkIndex];
    ...
    // return the big ass list of punkIds -> address
    function returnPunksArray() external view returns(address[totalPunkSupply] memory) {
        return punksArray;
    }
