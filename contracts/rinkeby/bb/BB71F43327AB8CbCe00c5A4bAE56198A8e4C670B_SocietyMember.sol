...
contract SocietyMember is ERC721, IERC2981, Ownable, ReentrancyGuard {
    ...
    uint256 constant SALE_PRICE = 0.3 ether;

    // A single wallet may only mint 2 memberships.
    uint256 constant MINTS_PER_WALLET = 2;

    // There can be only 5,000 members.
    // NOTE: permanently fixed upon contract creation
    uint256 public maximumMemberCount;

    // There need to be 2,000 members to proceed.
    // NOTE: permanently fixed upon contract creation
    uint256 public enoughMembersToProceed;
    ...
    // NOTE: permanently fixed upon contract creation
    uint256 public salesDeadline; // timestamp in seconds

    // If we fail to get 2,000 members then Members have until
    // this time to claim their #refund()
    //   See note re "Refund Warrant Process" for more details.
    // NOTE: permanently fixed upon contract creation
    uint256 public refundDeadline; // timestamp in seconds

    // During contract construction, we mint 7 tokens,
    // one for each member of the founding team and the first artist.
    // NOTE: permanently fixed upon contract creation
    uint256 private foundingTeamCount;
    ...
    constructor(
        ...
        maximumMemberCount = _maximumMemberCount;
        enoughMembersToProceed = _enoughMembersToProceed;
        ...
        foundingTeamCount = foundingTeam.length;
    ...
    function mint(bytes memory captcha) external payable {
        updateMode();
        require(
            mode == Mode.SellingPreThreshold ||
        ...
        require(msg.value == SALE_PRICE, "incorrect ETH payment amount");
        require(isProbablyHuman(captcha, msg.sender), "you seem like a robot");
        uint256 mintCount = mintCountsByAddress[msg.sender];
        require(
            mintCount < MINTS_PER_WALLET,
            "you have already minted your memberships"
        );

        mintCountsByAddress[msg.sender] = mintCount + 1;
    ...
    function updateMode() private {
        ...
        if (count >= maximumMemberCount) {
            mode = Mode.SoldOut;
        } else if (count >= enoughMembersToProceed) {
            mode = Mode.SellingPostThreshold;
        } else {
            // count < enoughMembersToProceed
            // When there are not enough members to proceed
            // then the mode depends on the time.
            if (block.timestamp < salesDeadline) {
                // Before sales deadline
                mode = Mode.SellingPreThreshold;
            } else if (block.timestamp < refundDeadline) {
