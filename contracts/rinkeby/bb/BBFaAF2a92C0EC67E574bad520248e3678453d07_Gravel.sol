...
contract Gravel is ERC721Enumerable, Ownable {
    ...
    uint256 public constant MAX_NFT_SUPPLY = 89;
    ...
    constructor() ERC721("Gravel", "GRAVEL") {
        //In production set this to the future
        REVEAL_TIMESTAMP = block.number;

        startingIndexBlock = block.number;
    }

    function _baseURI() internal view virtual override returns (string memory) {
        return "ipfs://QmNn6N2Gow5bNvYSfmT9NG2e5R7DPmnRgMwprtwNRamFSy";
    }

    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        //if token is not minted, do not return anything
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");

        //if offset is 0, return placeholder data
        if (startingIndex == 0) {
            return string(abi.encodePacked(_baseURI(), "placeholder"));
        }

        //starting index is RNG
        uint256 rawIndex = tokenId * startingIndex;

        if (rawIndex > MAX_NFT_SUPPLY) {
            rawIndex = rawIndex % MAX_NFT_SUPPLY;
        }
   ...
   function mint(uint256 amountToMint) public payable {
        require(saleActive == true, "This sale has not started.");
