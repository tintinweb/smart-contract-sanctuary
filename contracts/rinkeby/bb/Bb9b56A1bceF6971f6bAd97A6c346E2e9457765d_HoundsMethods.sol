...
contract HoundsMethods is Ownable, ERC721, ERC721Holder {
     ...
     * Admin only
     */
    function adminCreateHound(Hound.Default memory theHound) external {

        // Set the hound values
        hounds[houndId] = theHound;

        // Handle hound spawn, common code
        handleHoundSpawn();
    }
     ...
     */
    function breedHounds(uint32 hound1, uint32 hound2) external payable {

        // Perform verifications for breeding status
        require(hounds[hound2].breeding.breedCooldown < block.timestamp && hounds[hound1].breeding.breedCooldown < block.timestamp, Errors.HOUND_BREEDING_STILL_ON_COOLDOWN);

        // Checks to make sure the caller owns hound 1
        require(ownerOf(hound1) == msg.sender, Errors.NOT_THE_OWNER_OF_HOUND);
        ...
        if ( ownerOf(hound1) == msg.sender && ownerOf(hound2) == msg.sender )
            require(msg.value >= control.breedCost + control.breedFee, Errors.NOT_ENOUGH_PAID_TO_BREED);
        ...
        // Otherwise, he'll be charged with standard fees + that specific hound 2 breeding fee
            ...
            require(hounds[hound2].breeding.availableToBreed, Errors.HOUND_NOT_AVAILABLE_TO_BREED);

            // Then we check if the user has sent enough ETH for the fees
            require(msg.value >= control.breedCost + control.breedFee + hounds[hound2].breeding.breedingFee, Errors.NOT_ENOUGH_PAID_TO_BREED);
        ...
        Withdrawer.withdraw(address(0),control.breedFee);
        ...
        // Handle the hound spawn, common code
        handleHoundSpawn();

    }
    
    // Common function used to mint the ERC721, increment the id and emit the event
    function handleHoundSpawn() internal {
        emit NewHound(houndId,hounds[houndId]);
    ...
    function updateHoundStamina(uint256 theHoundId, uint32 staminaToConsume) external payable {
        ...
        /**
         * For hounds race contracts
         */
        if ( staminaToConsume > 0 ) {
            require(allowed[msg.sender], Errors.CALLER_NOT_ALLOWED);
            hounds[theHoundId].stamina.stamina -= uint32(staminaToConsume);
        }
        ...
        if ( !allowed[msg.sender] ) {
            require(msg.value >= hounds[theHoundId].stamina.staminaRefill1x, Errors.NOT_ENOUGH_PAID_TO_REFILL);
        }
    ...
    function updateHoundBreeding(bytes memory input) external payable {
        ...
        (uint256 theHoundId, uint32 breedingCooldownToConsume) = abi.decode(input,(uint256,uint32));
         ...
         * For anyone else
         */
        require(msg.value >= control.refillCost, Errors.NOT_ENOUGH_PAID_TO_REFILL);
    ...
    function putHoundForBreed(uint256 theHoundId, uint256 fee, bool status) external {
        require(ownerOf(theHoundId) == msg.sender, Errors.NOT_THE_OWNER_OF_HOUND);
        if ( status )
            require(hounds[theHoundId].breeding.breedCooldown < block.timestamp, Errors.HOUND_BREEDING_STILL_ON_COOLDOWN);
