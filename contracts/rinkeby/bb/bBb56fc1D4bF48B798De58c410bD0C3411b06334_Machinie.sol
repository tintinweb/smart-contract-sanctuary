...
contract Machinie is ERC721Enumerable, Ownable
{
    ...
    string private uri = "https://rest-api-machinie-nft.herokuapp.com/tokens/";
    address private floppy;
    address private humach;
    address private machinieLevel;
    address public constant blackHole = 0x000000000000000000000000000000000000dEaD;
    bool private enableStake;
    constructor(address floppy_,address machinieLavel_, address [] memory admins_ ) ERC721("Machinie", "MACH")
    {
        ...
        floppy = floppy_;
        machinieLevel = machinieLavel_;
        for(uint8 _i = 0; _i<admins_.length; _i++)
        {
            _admin[admins_[_i]] = true;
        }
    }
    ...
    function stakeMachinie(uint256 machinieId_,uint256 hamachId_) external {
        require(ownerOf(machinieId_) == _msgSender() , "Machinie : owner query for nonexistent machinie token");
        require(IHumach(humach).ownerOf(hamachId_) == _msgSender() , "Machinie : owner query for nonexistent humach token");
        require(!staking[machinieId_]  , "Machinie : MachinieID is staking");
        require(!IHumach(humach).isStaking(hamachId_) , "Machinie : HumachID is staking");
        staking[machinieId_] = true;
        stakeTime[machinieId_] = block.timestamp;
        humachId[machinieId_] = hamachId_;
        IHumach(humach).updateStakStatus(humachId[machinieId_], true);
    }

    function unStakeMachinie(uint256 machinieId_) external {
        require(ownerOf(machinieId_) == _msgSender() , "Machinie : owner query for nonexistent machinie token");
        require(staking[machinieId_] , "Machinie : MachinieID is not staking");
        uint256 _reward = getStakeReward(machinieId_);
        IFloppy(floppy).mint(_msgSender(), _reward);
        IHumach(humach).updateStakStatus(humachId[machinieId_], false);
        staking[machinieId_] = false;
        stakeTime[machinieId_] = 0;
        humachId[machinieId_] = 0;
    }
    ...
    function updateStakeRate(uint256 level_,uint256 rate_) external onlyWorker  {
...
interface IHumach  is IERC721Enumerable{
    ...
    function whiteListsMintHumach() external payable returns (uint256);

    function publicMintHumach(uint8 amount_) external payable returns (uint256 [] memory);
    
    function giveAway(address account_, uint8 amount_) external  returns (uint256 [] memory); //onlyAdmin

    function breedHumach(uint256 tokenId_) external returns (uint256);

    function stakeHumach (uint256 tokenId_) external ;

    function unStakeHumach (uint256 tokenId_) external ;
    ...
    function updateChangeNameFee(uint256 changeName_,uint256 changeDesc_,uint256 changeData_) external ;// onlyAdmin
    ...
    function  getChangeDataFee() external view returns (uint256,uint256,uint256);
    ...
    function getWhiteListminted(address account_) external view returns(bool) ;
