...
contract NiftyEntity {
   ...
   constructor(address _masterBuilderContract, address _niftyRegistryContract) public {
...
contract ERC721 is NiftyEntity, Context, ERC165, IERC721, IERC721Metadata {
   ...
   constructor (string memory name_, 
                ...
                address niftyRegistryContract_) NiftyEntity(masterBuilderContract_, niftyRegistryContract_) public {
       _name = name_;
       _symbol = symbol_;
    }
   ...
   /**
    * Xdev Returns an IPFS hash for a given token ID.
    * Throws if the token ID does not exist. May return an empty string.
    * Xparam tokenId uint256 ID of the token to query
    */
...
contract NiftyBuilderInstance is ERC721, ERC721Burnable, ERC721Enumerable {
  ...
  uint immutable public numNiftiesCurrentlyInContract;
  uint immutable public contractId;
  uint immutable private nifty_type;

  string public nameOfCreator;

  using Counters for Counters.Counter;

  // Mappings for token ID
  mapping (uint => Counters.Counter) public _numNiftyMinted;
  mapping (uint => uint) public _numNiftyPermitted;

  // Purchase & creation event(s)
  ...
  function isGiftNiftyPermitted() public view returns (bool) {
  ...
  function tokenIPFSHash(uint256 tokenId) external view override returns (string memory) {
    ...
    //uint value = dateTime.getMinute(block.timestamp) % 10;
    //uint value = dateTime.getHour(block.timestamp);
    //value = ((value + 11) % 12 + 1);
