...
contract IdeaFund is Operator, ContractGuard {
        ...
        ctrlRedeemable
    {
        ...
        require(
            IERC20(wbtc).balanceOf(address(this)) >= amount.mul(getControlPrice()).div(IOracle(theOracle).wbtcPriceOne()).div(1e10),
            'Idea Fund: Treasury does not currently hold enough wBTC to purchase'
        );

// console.log(getControlPrice());
// console.log(IERC20(wbtc).balanceOf(address(this)));
// console.log(amount.mul(getControlPrice()).div(IOracle(theOracle).wbtcPriceOne()).div(1e10));
        ...
        emit RedeemedCTRL(msg.sender, amount.mul(getControlPrice()).div(IOracle(theOracle).wbtcPriceOne()).div(1e10));
    }

    function redeemCTRLString(uint256 amount) public view returns (uint256) {
        return amount.mul(getControlPrice()).div(IOracle(theOracle).wbtcPriceOne()).div(1e10);
    }

    function balofWbtc() public view returns (uint256) {
