...
contract NFTsFactory is CloneFactory {
  ...
  function balanceOf(address nfts, address owner) public view returns (uint256) {
    return IERC721(nfts).balanceOf(owner);
  }

  function balanceOf(uint256 n, address owner) public view returns (uint256) {
    require(n < _implementations.length, "Implementation index out of range");

    return IERC721(_implementations[n]).balanceOf(owner);
  }

  function balancesOf(address owner) public view returns (uint256[] memory) {
    uint256[] memory balances;

    for (uint256 i = 0; i < _implementations.length; i++) {
      balances[i] = balanceOf(i, owner);
    }
