...
contract faFactory is Ownable, ERC721, ERC721Enumerable {
    ...
    function upgradeStats (uint16 tokenId, uint32[8] memory statsArray) public {
        ...
        if (_costToUpgrade > 0) {
            token.transferFrom(msg.sender, address(this), UPGRADE_PRICE.mul(_costToUpgrade).div(100).mul(100-devFee));
            token.burn(UPGRADE_PRICE.mul(_costToUpgrade).div(100).mul(100-devFee));
            token.transferFrom(msg.sender, _owner, UPGRADE_PRICE.mul(_costToUpgrade).div(100).mul(devFee));
