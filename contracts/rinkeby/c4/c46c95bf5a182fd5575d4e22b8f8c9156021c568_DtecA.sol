...
contract DtecA is ERC2771ContextUpgradeable, ERC1155PresetMinterPauserUpgradeable {

    ISecurityToken _policy;
    ...
    function setPolicy(ISecurityToken policy_) external onlyAdmin {
        _policy = policy_;
    }

    function setTrustedForwarder(address forwarder) external onlyAdmin {
        _trustedForwarder = forwarder;
    }

    function policy() public view returns (ISecurityToken){
        return _policy;
    }

    function certifyDocument(bytes32 documentType, bytes32 documentHash) external onlyAdmin {
        emit DocumentCertified(documentType, documentHash);
    }

    function batchCertifyDocument(bytes32[] memory documentType, bytes32[] memory documentHash) external onlyAdmin {
    ...
    function _beforeTokenTransfer(
        ...
        bytes memory data
    ) internal virtual override(ERC1155PresetMinterPauserUpgradeable) {
       uint8 restrictionCode = _policy.detectTransferRestriction(operator, from, to, ids, amounts, data);
        // Prevent an extra interaction to get the revert message
        if (restrictionCode != 0){
            revert(_policy.messageForTransferRestriction(restrictionCode));
        }
    }
...
interface ISecurityToken {
    function detectTransferRestriction (address operator, address from, address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) external view returns (uint8);
