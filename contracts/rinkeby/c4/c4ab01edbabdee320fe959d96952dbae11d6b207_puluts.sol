...
contract puluts{
    ...
    IERC20  plt = IERC20(address(0xBCBebe4Ed03CC4aE8d9FDC5597Fe3Bd941FE6587));
    ...
    function deposit(uint amount ,address recommender)public {
        ...
        plt_amount = plt_amount.mul(rate_info[4][1]).div(1000);
        ...
        uint cost = usdt_amount.mul(rate_info[4][3].add(rate_info[4][4])).div(1000);
        totalCash = totalCash.add(usdt_amount).sub(cost);
    ...
    function ProfitSharing(uint plt_amount)private {
        ...
        for(uint i=1; i<=3; i++){
            ...
            uint profit = plt_amount.mul(rate_info[3][i]).div(100);
