...
contract OXPreSale is Ownable {
    ...
    function initialize ( address _dai, address _iOX, uint _salePrice, uint _saleLength ) external onlyOwner() returns(bool) {
        require(saleStarted == false, 'Already initialized');
    ...
    function purchaseOX(uint _amountDAI) external returns(bool) {
    ...
    function sendRemainingiOX( address _sendiOXTo ) external onlyOwner() returns(bool) {
        require (saleStarted == true, 'Not started');
        require (block.timestamp >= endOfSale, 'Not ended');
        
        IERC20( iOX ).approve(msg.sender, IERC20( iOX ).balanceOf(address(this)));

        IERC20( iOX ).safeTransferFrom(address(this), _sendiOXTo, IERC20( iOX ).balanceOf(address(this)));
