...
contract DotcMain is owned {
    ...
    //事件
    event NewAdOrder(address indexed token, address indexed seller, uint256 fiat_price, uint256 low_limit, uint256 high_limit, uint256 dotc_amount, address[] collateral_tokens, uint256[] collateral_dotc_values);
	...
	User storage user = accounts[msg.sender];
	require(user.balance[token_addr] >= dotc_amount, "token amount values not match");
	...
	for(uint i=0; i<collateral_tokens.length; i++)
	{
		...
		sellerCollateral[msg.sender][total_ad_num][i].token_amount = collateral_dotc_values[i];
		user.frozen[tk.addr] = user.frozen[tk.addr].add(sellerCollateral[msg.sender][total_ad_num][i].token_amount);
	}
	if(ind != 999)
	{
		require(user.balance[token_addr] >= dotc_amount.add(collateral_dotc_values[ind]), "token values not match");
	...
	userAdsMap[msg.sender].push(total_ad_num);
    	...
    	emit NewAdOrder(token_addr, msg.sender, fiat_price, low_limit, high_limit, dotc_amount, collateral_tokens, collateral_dotc_values);
	return total_ad_num;
    }
	...
	for(uint i=0; i<collateral_tokens.length; i++)
	{
		...
		buyer.frozen[tk.addr] = buyer.frozen[tk.addr].add(buyerCollateral[msg.sender][total_ad_num][i].token_amount);
	}
