...
contract EBMarketRents is ERC721Holder, ERC1155Holder, Ownable, ReentrancyGuard, TokensHandler {
    uint public id;
    uint public fee = 20;
    enum Status {
        LISTED,
    ...
    modifier isAccesible(uint rentId) {
        require(rentId < id, "Sale ID it not valid");
        require(rents[rentId].owner != address(0), "Sale is no longer accessible");
        _;
    }
    ...
    function cancel(uint rentId) external isAccesible(rentId) nonReentrant {
        require(msg.sender == rents[rentId].owner, "You're not the owner of this sale");
    ...
    function rent(uint rentId) external payable isAccesible(rentId) nonReentrant {
        ...
        rents[rentId].expiration = block.timestamp + rents[rentId].valability * 1 days;
