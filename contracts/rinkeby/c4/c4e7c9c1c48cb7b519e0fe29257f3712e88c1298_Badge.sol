...
//    function randAddOneArr() external view returns(uint8[] memory);
}
...
contract Badge is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    //Whitelist Contract 0xF2A8F7ebD1e32D823534ebCAb91883C509E9fe1b
//    address public toolAddress = 0xF2A8F7ebD1e32D823534ebCAb91883C509E9fe1b;
...
//    }
    ...
    function randLevel(uint8 num) private pure returns (uint8) {
        uint8 maxNum=0;
        if (num <= maxNum)
        {
            return 4;
    ...
    function randAddOneArr() private view returns (uint8[] memory){
        uint8 level = randLevel(randomNum(100));
        uint8[] memory addOneArr;
        uint8 addOneCount =0;
        for (uint8 i = 1; i <= 4; i++)
        {
...
//        attrsIndex[tokenId].addOne3=addOneArr[2];
        attrsIndex[tokenId].addOne4=1;//addOneArr[3] test
