 ...
 * @dev See https://eips.ethereum.org/EIPS/eip-721
 */
abstract contract ERC721Enumerable is ERC721Burnable {
...
/**
 * @title Interface for METTA Protocol's Market
 */
interface IMarket {
    struct Bid {
...
/**
 * @title Interface for WhiteRare Protocol's Media
 */
interface IMedia {
...
// File contracts/WhiteRareMedia.sol
 ...
 */
contract WhiteRareMedia is IMedia, ERC721Enumerable, ReentrancyGuard {
    using Counters for Counters.Counter;
    using SafeMath for uint256;
    uint public maxBulkMint = 51; // allows to mint 50 artworks
    uint public CurrentBatchID;
     ...
     */
    constructor(address WhiteRareMarketContractAddress) public ERC721("WHITERARE", "WHITERARE") {
        marketContract = WhiteRareMarketContractAddress;
    ...
    function bulkmint( MediaData[] memory dataArray,IMarket.BidShares[] memory bidSharesArray) public nonReentrant {
        require((dataArray.length == bidSharesArray.length)&&(dataArray.length < maxBulkMint));
        for(uint i = 0; i < dataArray.length; i++){
            _mintForBulkCreator(msg.sender, dataArray[i], bidSharesArray[i]);
