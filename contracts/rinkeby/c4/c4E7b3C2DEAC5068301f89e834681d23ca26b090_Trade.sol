...
interface IERC1155 is IERC165 {
    ...
    */
    ...
    function royaltyFee(uint256 tokenId) external view returns(uint256);
    function getCreator(uint256 tokenId) external view returns(address);
    function mintAndTransfer(address from, address to, uint256 itemId, uint256 fee, uint256 _supply, string memory _tokenURI, uint256 qty, bytes memory data)external returns(bool);
...
contract Trade {
    ...
    enum BuyingAssetType {ERC1155, ERC721 , ERC1155Deprecated, ERC721Deprecated}
    ...
    function verifyOwnerSign(address buyingAssetAddress,string memory tokenURI, ownerSign memory sign) internal view {
        address _owner = IERC721(buyingAssetAddress).contractOwner();
        bytes32 hash = keccak256(abi.encodePacked(buyingAssetAddress,tokenURI));
    ...
    function getFees( Order memory order) internal view returns(Fee memory){
        ...
        if(order.nftType == BuyingAssetType.ERC721Deprecated) {
            royaltyPermille = order.fee;
            tokenCreator = order.seller;
        }
        if(order.nftType == BuyingAssetType.ERC1155Deprecated) {
            royaltyPermille = order.fee;
            tokenCreator = order.seller;
        }
    ...
    function mintAndBuyAsset(Order memory order, ownerSign memory ownersign, Sign memory sign) public returns(bool){
        Fee memory fee = getFees(order);
        require((fee.price >= order.unitPrice * order.qty), "Paid invalid amount");
        verifyOwnerSign(order.nftAddress, order.tokenURI, ownersign);
    ...
    function mintAndExecuteBid(Order memory order, ownerSign memory ownersign, Sign memory sign) public returns(bool){
        Fee memory fee = getFees(order);
        require((fee.price >= order.unitPrice * order.qty), "Paid invalid amount");
        verifyOwnerSign(order.nftAddress, order.tokenURI, ownersign);
