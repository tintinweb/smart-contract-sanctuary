...
contract BigMedia {
    ...
    event articleAdded(string agencyName, bytes32 ref);
    ...
    function addArticle(string memory title, string memory post, string memory agencyName) public {
        ...
        emit articleAdded(agencyName, ref);
    }
    
    function getArticle(string memory agencyName, bytes32 ref) public view returns (Article memory article) {
        return agencies[agencyName].articles[ref];
    }
}