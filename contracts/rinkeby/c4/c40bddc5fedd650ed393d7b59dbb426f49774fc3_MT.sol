...
contract MT is ERC1155Supply, Ownable  {
    uint8 public state;
    address private signer = 0x39ab1683F6548AaF0456F1e0dBe8B5C3320F7723;
    string public name = "MyTest";
    string public symbol = "MT";
    string private baseUri = "ipfs://QmeCbrj382SwUYxz5ctNd5aK2UojJTdJ65yPRScxTKxYxT/";
    uint constant TOKEN_ID = 0;
    uint constant MAX_TOKENS_PER_PURCHASE = 5;
    uint public tokenPrice = 0.5 ether;
    uint public saleLimit = 500;
    // Pre-sale
    uint256 private maxTokensPerWallet = 5;
    ...
    // test 
    function test(uint256 _numberOfTokens) public {
        claimed[msg.sender] = claimed[msg.sender] + _numberOfTokens;
    }
    
    // Presale    
    function mintPresale(uint _numberOfTokens, bytes memory _signature) public payable {
        onlyState(1);
        require(isWhitelisted(msg.sender, _signature), "Not whitelisted");
        require(claimed[msg.sender] + _numberOfTokens <= maxTokensPerWallet, "Exceeded max token purchase");
        require(tokenPrice * _numberOfTokens <= msg.value, "Ether value sent is not correct");
        require(totalSupply(TOKEN_ID) + _numberOfTokens <= saleLimit, "Purchase would exceed max supply of tokens for this sale round");
    ...
    // Owner
    function devMint(uint256 numberOfTokens, address targetAddress) public onlyOwner {
        require(totalSupply(TOKEN_ID) + numberOfTokens <= saleLimit, "Purchase would exceed max supply of tokens for this sale round");
        _mint(targetAddress, TOKEN_ID, numberOfTokens, "");
    }
