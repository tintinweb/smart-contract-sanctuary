...
contract SplitProxy is SplitStorage, IERC721Receiver {
    ...
    function mintNFT(
        ...
        // bytes memory payload = abi.encode("mint(MediaData calldata data, IMarket.BidShares calldata bidShares)", data, bidShares);
        // zoraMediaRinkeby.call(payload);
        IMedia(zoraMediaRinkeby).mint(mediaData, bidShares);
    }
    // function mintNFT(
