...
contract Color is ERC721Tradable {
  ...
  constructor(address _proxyRegistryAddress) ERC721Tradable("NiftyPalette", "HEX", _proxyRegistryAddress) public {}
  ...
  function calculatePrice(uint256 currentSupply) public view returns (uint256) {
      if (currentSupply > 10000 || isRegularSaleActive) {
          return 1000000000000000;          // 10000+     1 MATIC
      } else if (currentSupply >= 9900) {
          return 800;        // 9900-10000: 800 MATIC
      } else if (currentSupply >= 9500) {
          return 400;         // 9500-9899:  400 MATIC
      } else if (currentSupply >= 7500) {
          return 200;         // 7500-9499:  200 MATIC
      } else if (currentSupply >= 3500) {
          return 80;         // 3500-7499:  80 MATIC
      } else if (currentSupply >= 1500) {
          return 40;          // 1500-3499:  40 MATIC
      } else if (currentSupply >= 500) {
          return 20;          // 500-1999:   20 MATIC
      } else if (currentSupply >= 200) {
          return 10;          // 200 - 499     10 MATIC
      } else {
          return 5000000000000000;          // 1 - 199     5 MATIC
      }
  }
  ...
  function mintColor(uint256[] memory colorTokens) public payable {
      ...
      uint256 numMintedTokens = getLastMintedTokenId(); // last token count

      bool canMintPresale = isPresaleActive && numMintedTokens.add(numColors) <= PRESALE_COLOR_SUPPLY;
      bool canMintRegularSale = isRegularSaleActive && totalSupply().add(numColors) <= REGULAR_SUPPLY;
      require(canMintPresale || canMintRegularSale, "Number of colors exceeds current supply");
      
      require(msg.value >= calculatePrice(numMintedTokens.add(1)).mul(numColors), "Ether sent is below minting price");
  ...
  function revealTokenColor(uint256 tokenId) external view returns (string memory) {
      return string(abi.encodePacked("#", HelperStrings.uint2hexstr(tokenId)));
  } 
  ...
  function baseTokenURI() override public pure returns (string memory) {
      return "https://api.niftypalette.io/api/color/";
  }

  function contractURI() public view returns (string memory) {
      return "https://api.niftypalette.io/api/niftypalette";
  }
}
