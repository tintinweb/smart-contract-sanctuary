...
contract Project is ERC721Namable, Ownable {
    ...
    function airdrop(address[] memory giveawayList) external onlyOwner onlyStart {
        require(balanceOf(msg.sender) >= giveawayList.length, "not enough");
        uint256[] memory ownerWallet = walletOfOwner(msg.sender);
    
        for (uint256 i = 0; i < giveawayList.length; i++) {
          safeTransferFrom(msg.sender, giveawayList[i], ownerWallet[i], "0x00");
        }
    }
