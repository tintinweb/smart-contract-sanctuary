...
contract EGGTOMATONS is ERC721Enumerable, Ownable, Payment, Guard {
	...
	if(b>0 || wl>0){
	if(b>0){
	require(ba <= maxP  + (b * maxMP) + b, "Mint less");
	}
	else{
	require(ba <= maxP + maxWL, "Mint less");
	}
	require(msg.value >= priceMP * _tokenAmount, "ETH input is wrong");
	}
	//public
	else{
	require(ba <= maxP, "Mint less");
	if (_tokenAmount < 5){
	...
	require( (s - 1) + _tokenAmount <= maxSupply, "Mint less");
	require(msg.value >= priceMP * _tokenAmount, "ETH input is wrong");
	delete ba;
	delete b;
	for (uint256 i = 0; i < _tokenAmount ; ++i) {
	...
	//check if claim
	function checkclaim(uint256 n) public view returns (bool) {
	...
	// mintpass free claim
	function freeclaim(uint256 _tokenID) public {
	uint b = mintpass.balanceOf(msg.sender); // MINTPASS balance
	address check = mintpass.ownerOf(_tokenID); //check owner address
	uint256 s = totalSupply() + 1;
	require(mintpassStatus, "Mint pass is not active" );
	require(b > 0, "You need a mint pass");
	require( (s - 1) <= maxSupply, "Mint less");
	require(msg.sender == check, "You don't own this token"); //must be owner
	require(checkclaim(_tokenID) == false); //must not be claimed already
	delete b;
	//mint
	_safeMint(msg.sender, s, "");
	storeClaim.push(_tokenID);
    delete s;
	}
