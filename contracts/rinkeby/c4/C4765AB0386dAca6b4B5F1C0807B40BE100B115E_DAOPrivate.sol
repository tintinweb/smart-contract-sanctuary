...
contract DAOPrivate is Ownable, ReentrancyGuard, SignatureVerifyer {
    ...
    function updateEpoch(uint256 userID, uint256 round) public {
      if(round == 0) return;
    ...
    function claimTransitionBonusFor(uint256 round, uint256 userID) public {
      ...
      uint256 fullReward = levelBase[round].mul(10);
      uint256 rewardAmount = fullReward
        .mul(roundEpochs[round][epochNum].fullShare)
        .div(roundEpochs[round][epochNum].share[userID]);
