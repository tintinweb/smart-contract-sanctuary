...
contract CurseWords is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getSecondaryFont(uint256 tokenId) public view returns (string memory) {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory metadata = string(abi.encodePacked('"attributes": [{"trait_type": "Primary Word", "value": "', getCurseWord(tokenId), '"}], '));
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Curse Word #', toString(tokenId), '", "description": "Curse Word #', toString(tokenId), '", "description": "Curse Words are randomly generated, minimally artistic curse words generated on-chain. They do nothing...?", ', metadata, '"image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
