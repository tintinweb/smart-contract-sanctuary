...
contract WrappedPunk is Ownable, ERC721, Pausable {
   ...
   */
    ...
    function wrap(uint punkIndex) public {
        address sender = _msgSender();
        ICryptoPunk.Offer memory offer = _punkContract.punksOfferedForSale(punkIndex);
        require(offer.seller == sender, "Not punk owner, or punk is not for sale");

        _punkContract.buyPunk(punkIndex);

        punkIndexToAddress[punkIndex] = sender;
        _mint(sender, punkIndex);
        emit Wrapped(punkIndex);
    }

    function unwrap(uint punkIndex) public {
        address owner = ownerOf(punkIndex);
        require(owner == _msgSender(), "Not punk owner");
        punkIndexToAddress[punkIndex] = address(0x0);
        _punkContract.transferPunk(owner, punkIndex);
        _burn(punkIndex);
        emit Unwrapped(punkIndex);
    }
    ...
    function transferFrom(address from, address to, uint punkIndex) public override {
        _transferFrom(from, to, uint256(punkIndex));
