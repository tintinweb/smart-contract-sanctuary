...
contract unifarmV1Fixed is Ownable {
    ...
    mapping(address => uint256) public totalUnStakingFinal;
    ...
    function unStake(uint256 stakeId) external  {       
        ...
        uint totalUnstake = totalUnStakingFinal[tokenAddress[stakeId]];
        totalUnStakingFinal[tokenAddress[stakeId]] = totalUnStakingFinal[tokenAddress[stakeId]].add(stakedAmount[stakeId]);
          
        // Transfer staked token back to user
        IERC20(tokenAddress[stakeId]).transfer(msg.sender, stakedAmount[stakeId]);

        // Claim pending rewards
        claimRewards(startTime[stakeId], stakedAmount[stakeId], tokenAddress[stakeId], totalStaking.sub(totalUnstake));
    ...
    function updateV1Address(address v1Address) public onlyOwner returns(bool) {
        UniV1 = UnifarmV1(v1Address);
        return true;
    }
    
   
    function updateContractAStatus(bool status) public onlyOwner returns(bool) {
