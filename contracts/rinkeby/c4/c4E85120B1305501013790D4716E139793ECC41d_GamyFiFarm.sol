...
contract GamyFiFarm is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        gfx.mint(address(this), gfxReward);
    ...
    function safeGFXTransfer(address _to, uint256 _amount) internal {
        uint256 gfxBal = gfx.balanceOf(address(this));
        if (_amount > gfxBal) {
            gfx.transfer(_to, gfxBal);
        } else {
            gfx.transfer(_to, _amount);
        }
    }

    // Deposit tokens to GamyFiFarm Test for gfx allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        require(_pid != 0, "deposit GFX by staking");
        ...
        if (user.amount > 0) {
            uint256 pending =
                user
                    ...
                    .sub(user.rewardDebt);
            if (pending > 0) {
                safeGFXTransfer(msg.sender, pending);
            }
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        require(_pid != 0, "withdraw GFX by unstaking");
