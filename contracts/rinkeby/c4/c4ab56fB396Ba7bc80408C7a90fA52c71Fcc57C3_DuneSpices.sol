...
contract DuneSpices is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, OwnableUpgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable, UUPSUpgradeable {
    ...
    // avatar Address => AVATARINFO
    mapping(address=>MINTINFO) _avatarInfo;

    // start the game
    function start(uint256 landID, uint256 exitBlock) public returns (MINTINFO memory) {
        // check the landID
        require(landID <= 100 && landID >= 1, "DUNE: LandID is error!");
        // check the exitBlock
        require(exitBlock > block.number, "DUNE: ExitBlock is error!");
        // check the avatar
        require(_avatarInfo[msg.sender].landID == 0, "DUNE: You had into the DUNE!");
        _avatarInfo[msg.sender]=MINTINFO(landID, msg.sender, block.number, exitBlock);
    ...
    function over() public {
        ...
        uint256 vault = (_avatarInfo[msg.sender].exitBlock - _avatarInfo[msg.sender].entryBlock) * 1 ether;
