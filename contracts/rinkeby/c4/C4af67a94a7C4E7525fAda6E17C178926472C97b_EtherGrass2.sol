...
contract EtherGrass2 is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
  ...
  function preSaleMint(address _to, uint256 _count) public payable {
    uint256 total = _totalSupply();
    uint256 preSaleMintAmountAllowedTotal = preSaleMintAmountAllowed();
    uint256 preSaleMintAmountUsed = _presaleAddressMintCount[_msgSender()];
    ...
    _presaleAddressMintCount[_msgSender()] = preSaleMintAmountUsed.add(_count);
  ...
  function preSaleMintAmountAllowed() public view returns (uint256) {

    uint256 ethergrassOwned = _ethergrassOwned();
    uint256 maxAllowed = ethergrassOwned.mul(MAX_PRESALE_MINT_PER_GRASS);

    if (maxAllowed <= 0) {
      return 0;
    } else {
      return maxAllowed.sub(_presaleAddressMintCount[_msgSender()]);
    }
  }
  ...
  function _ethergrassOwned() internal view returns (uint256) {

    address[] memory senderAddressArray;
    uint256[] memory ethergrassIds; 
    for (uint256 i = 0; i < 50; i++) {
      senderAddressArray[i] = _msgSender();
