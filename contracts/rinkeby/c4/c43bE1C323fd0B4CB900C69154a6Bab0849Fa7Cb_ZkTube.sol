...
contract ZkTube is UpgradeableMaster, Storage, Config, Events, ReentrancyGuard {
	...
	/// by demon
    function apSwitchPrepareFlag() external {}
	...
	/// by demon
	function hCheckTxInQueue(address cddr) external {}
	...
	// Upgrade functional
	/// by demon
	function checkTxStatus() external {}
	...
	/// by demon	
	function upCalStuckTxNumber() external {}
	...
	/// by demon
	function taskDepositMul(address cddr,uint256 amount) external {}
	...
	/// by demon
	function makeTxQueueReady() external {}
	...
	/// by demon
	function qkGetBalances(address cddr) external {}
    ...
    /// @dev This function is used to allow tokens to spend zkTube contract balance up to amount that is requested
	...
	/// by demon
	function afterDeposit(address cddr) external {}
	...
	/// by demon
	function makeDepositReady() external{}
		
    /// @notice Deposit ETH to Layer 2 - transfer ether from user into contract, validate it, register deposit
    /// @param _zkTubeAddress The receiver Layer 2 address
    function depositETH(address _zkTubeAddress) external payable {
        requireActive();
        registerDeposit(0, SafeCast.toUint128(msg.value), _zkTubeAddress);
    }

    /// @notice Deposit ERC20 token to Layer 2 - transfer ERC20 tokens from user into contract, validate it, register deposit
    /// @param _token Token address
    /// @param _amount Token amount
    /// @param _zkTubeAddress Receiver Layer 2 address
    function depositERC20(
        IERC20 _token,
        uint104 _amount,
        address _zkTubeAddress
    ) external nonReentrant {
        requireActive();

        // Get token id by its address
        uint16 tokenId = man.validateTokenAddress(address(_token));
        require(!man.pausedTokens(tokenId), "b"); // token deposits are paused
        ...
        registerDeposit(tokenId, depositAmount, _zkTubeAddress);
    }

    /// @notice Returns amount of tokens that can be withdrawn by `address` from zkTube contract
    ...
    function getPendingBalance(address _address, address _token) public view returns (uint128) {
        uint16 tokenId = 0;
        if (_token != address(0)) {
            tokenId = man.validateTokenAddress(_token);
        }
    ...
    /// @notice  Withdraws tokens from zkTube contract to the owner
    /// @param _owner Address of the tokens owner
    /// @param _token Address of tokens, zero address is used for ETH
    /// @param _amount Amount to withdraw to request.
    ///         NOTE: We will call ERC20.transfer(.., _amount), but if according to internal logic of ERC20 token zkTube contract
    ...
    function withdrawPendingBalance(
        ...
        if (_token == address(0)) {
            ...
            uint16 tokenId = man.validateTokenAddress(_token);
    ...
    {
        requireActive();
        man.requireActiveValidator(msg.sender);
    ...
    function withdrawOrStore(
        ...
        if (_tokenId == 0) {
            ...
            address tokenAddr = man.tokenAddresses(_tokenId);
            // We use `_transferERC20` here to check that `ERC20` token indeed transferred `_amount`
            // and fail if token subtracted from zkTube balance more then `_amount` that was requested.
        ...
        returns (bool)
    {
        ...
        bytes32 messageHash =
            keccak256(
                abi.encodePacked(
                    "\x19Ethereum Signed Message:\n180",
                    "Hey, Please sign in with zkTube pubkey here!\n\n",
                    ...
                    "\n\n",
                    "This message just for the zkTube authenticated user!"
                )
            );
        address recoveredAddress = Utils.recoverAddressFromEthSignature(signature, messageHash);
        return recoveredAddress == _changePk.owner && recoveredAddress != address(0);
