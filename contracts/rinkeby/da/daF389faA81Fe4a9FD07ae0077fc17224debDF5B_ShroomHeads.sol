...
contract ShroomHeads is AccessControl, ReentrancyGuard, ERC1155Supply
{
    ...
    struct NFTStruct {
        ...
        uint256 price;
        uint8 editions;
        uint8 saleType; // 0 - public; 1 - private; 2 - auction
    }
    ...
    constructor(string memory uri_, address[] memory _team, uint256[] memory _shares, address wETH, address _signerAddress) ERC1155(uri_) {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(TEAM, _msgSender());
        owner = _msgSender();
        WETH = wETH;
     ...
     */
    function setSignerAddress(address addr) public onlyRole(TEAM) {
        signerAddress = addr;
    }
    ...
    */
    ...
    function craftNewNfts(bytes1[] calldata _traitsHex,
        ...
        for (uint256 i = 0; i < _traitsHex.length; i++) {
            ...
            require(_saleType[i] <= 2, "Unknown sale type");
    ...
    function correctExistingNftSaleType(uint256 tokenId, uint8 _saleType) external onlyRole(TEAM) {
        require(tokenId< Nfts.length, "NFT doesn't exist");
        require(_saleType <= 2, "Unknown sale type");
    ...
    */
    function privateMint(uint256 tokenId, bytes memory _signature) external nonReentrant payable {
        ...
        // check signature
        require(signerAddress == keccak256(abi.encode("privateMint", tokenId, _msgSender())).toEthSignedMessageHash().recover(_signature), "Not authorized. You need to be whitelisted on Discord");
        ...
        // 
        require(balanceOf(_msgSender(), tokenId) == 1, "Only 1 edition per account is allowed for private sale");
    }

    /**
    * @dev Mints for auction
    */
    function auctionMint(uint256 tokenId, address winningBidder, uint256 winningBid, bytes memory _signature) external onlyRole(TEAM) {
        require(Nfts[tokenId].saleType == 2, "This NFT is not on auction");

        // check signature. Just add additional security check
        require(signerAddress == keccak256(abi.encode("auctionMint", tokenId, winningBidder, winningBid)).toEthSignedMessageHash().recover(_signature), "Bad signature");
