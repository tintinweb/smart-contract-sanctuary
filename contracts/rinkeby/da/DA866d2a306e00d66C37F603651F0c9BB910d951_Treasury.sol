...
abstract contract AbstractLender is AbstractTreasury{
    ...
    function createCoin(string memory name, string memory symbol, uint currency) external onlyOwner{
        stableCoinFactory.createStableCoin(name, symbol, currency);
    }
...
contract StableCoinFactory is Ownable{
    ...
    function createStableCoin(string memory name, string memory symbol, uint currency) external onlyOwner{
        require (_getCoin[currency] == address(0), "Coin already exists!");
