 ...
 */
contract MightyManateesMerchCard is ERC721, Ownable {
    using SafeMath for uint256;

    uint256 public MerchCardPrice = 50000000000000000; // 0.05 ETH
    uint256 public MAX_CARDS = 1000;
    address payable public saleManager;
    bool public isLive = false;
    uint public constant maxMerchCardPurchase = 20;
    mapping(address => uint) public freeManateeCount;

    constructor() ERC721("Mighty Manateez Merch Card", "MMMC") {
    ...
    function mintMerchCards(uint numberOfTokens) public payable {
        require(isLive, "Can not mint Merch Card before the sale has started");
        require(numberOfTokens <= maxMerchCardPurchase, "Can only mint 20 merch cards at a time");
        require(totalSupply().add(numberOfTokens) <= MAX_CARDS, "Purchase would exceed max supply of Merch Cards");
        require(MerchCardPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");

        for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply();
            if (totalSupply() < MAX_CARDS) {
                _safeMint(msg.sender, mintIndex);
            }
        }
    }

    function burn() public returns(address owner){
        return msg.sender;
//        return balanceOf(owner);

//        require(_exists(tokenID), 'That merch card has already been burned or does not exist!');
//        _burn(tokenID);
    }
}
