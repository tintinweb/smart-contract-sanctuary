...
interface IWoolf {
  struct SheepWolf {bool isSheep; uint8[] traitarray; uint8 alphaIndex;}
...
contract BIGYO is IWoolf, ERC721Enumerable, Ownable, Pausable, VRFConsumerBase {
  ...
  function selectTraits(uint256 seed) internal view returns (SheepWolf memory t) {    
    ...
    if (t.isSheep) {
      ...
      uint8[] memory foo1 = new uint8[](14); // this is fine

      foo1[0] = selectTrait(uint16(seed & 0xFFFF), 0 + shift);
      seed >>= 16;
      foo1[1] = selectTrait(uint16(seed & 0xFFFF), 1 + shift);
      seed >>= 16;
      foo1[2] = selectTrait(uint16(seed & 0xFFFF), 2 + shift);
      seed >>= 16;
      foo1[3] = selectTrait(uint16(seed & 0xFFFF), 3 + shift);
      seed >>= 16;
      foo1[4] = selectTrait(uint16(seed & 0xFFFF), 4 + shift);
      seed >>= 16;
      foo1[5] = selectTrait(uint16(seed & 0xFFFF), 5 + shift);
      seed >>= 16;
      foo1[6] = selectTrait(uint16(seed & 0xFFFF), 6 + shift);
      seed >>= 16;
      foo1[7] = selectTrait(uint16(seed & 0xFFFF), 7 + shift);
      seed >>= 16;
      foo1[8] = selectTrait(uint16(seed & 0xFFFF), 8 + shift);
      ...
      uint8[] memory foo1 = new uint8[](14); // this is fine
      foo1[9] = selectTrait(uint16(seed & 0xFFFF), 0 + shift);
      seed >>= 16;
      foo1[10] = selectTrait(uint16(seed & 0xFFFF), 1 + shift);
      seed >>= 16;
      foo1[11] = selectTrait(uint16(seed & 0xFFFF), 2 + shift);
      seed >>= 16;
      foo1[12] = selectTrait(uint16(seed & 0xFFFF), 3 + shift);
      seed >>= 16;
      foo1[13] = selectTrait(uint16(seed & 0xFFFF), 4 + shift);
  ...
  function structToHash(SheepWolf memory s) internal pure returns (uint256) {
    return uint256(bytes32(abi.encodePacked(s.isSheep, s.traitarray[0], s.alphaIndex)));
  }
