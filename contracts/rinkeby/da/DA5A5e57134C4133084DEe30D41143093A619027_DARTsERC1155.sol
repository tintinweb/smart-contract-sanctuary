...
contract DARTsERC1155 is ERC1155, ERC1155Supply, AccessControl, ERC2981TokenRoyalties, ERC1155URIStorage {
    ...
    function grantMintRole(address user) external onlyRole(MINTER_ROLE) {
        require(hasRole(ADD_MEMBER_ROLE, user), Errors.NO_HAS_ADD_MEMBER_ROLE);
    ...
    function create(
        ...
        Rights[newItemId] = DataTypes.Right(
            ...
            _tokenUri, true, 1, DataTypes.royaltyTable(0, 10000, 0, 0, 0),
            DataTypes.RoyaltyVoting(address(0x0), address(0x0), 0, false), "");
    ...
    function clone(uint formerId, uint _initSupply, uint _editType) external onlyRole(MINTER_ROLE) {
        ...
        require(_msgSender() == Rights[formerId].owners[0], Errors.NOT_OWNER);
        ...
        Rights[newItemId] = DataTypes.Right(
            ...
            new address[](0), zero,
            new address[](0), zero,
            tokenURI(formerId), cloneable, _order, RT,
            DataTypes.RoyaltyVoting(address(0x0), address(0x0), 0, false), '');
        defaultRoyaltyTable(newItemId);
    ...
    public onlyRole(ADD_MEMBER_ROLE) returns (uint){
        ...
        } else if (_msgSender() == tempright.sponsors[0]) {
        ...
        } else if (_msgSender() == tempright.thanks[0]) {
...
//        Rights[_tokenId] = tempright;
    ...
    function admit(uint _tokenId) external onlyValidToken(_tokenId) {
        require(balanceOf(_msgSender(), Rights[_tokenId].formerId) == 1, Errors.TOKEN_BALANCE_LOW);
