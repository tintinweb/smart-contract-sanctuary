...
contract SalesPolicy is EIP712MetaTransaction("BuyPolicyMetaTransaction", "1"), ERC721URIStorage, ISalesPolicy, ReentrancyGuard {
    ...
    event LogUpdatePolicyStatus(address indexed _policyAddress, uint256 _policyId, address _policyOwner, uint256 _coverageAmount);
    ...
    function buyPolicy(
        ...
        ICapitalAgent(capitalAgent).policySale(_coverageAmount, lastIdx);

        policyIdx.increment();
        emit BuyPolicy(protocolIdx, lastIdx, msgSender(), _coverageAmount, _policyPriceInUSDT, _premiumCurrency, premiumPaid);
    }

    function updatePolicyStatus(uint256 _policyId) external nonReentrant {
        getPolicy[_policyId].exist = false;
        emit LogUpdatePolicyStatus(
            address(this),
            _policyId,
            getPolicy[_policyId].policyOwner,
            getPolicy[_policyId].coverageAmount
        );
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(getPolicy[tokenId].policyOwner == from, "UnoRe: not policy owner");
