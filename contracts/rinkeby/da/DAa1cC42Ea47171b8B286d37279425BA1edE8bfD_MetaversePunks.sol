...
contract MetaversePunks is ERC721EnumerableB, Ownable, PaymentSplitter, ReentrancyGuard {
  ...
  uint public PRICE = 0.4 ether;
  uint public MAX_SUPPLY = 334;
  uint public mintableCount = 0;

  bool public isActive = false;
  bool public isOpen = false;

  mapping (address => claimStatus) public _claims;

  constructor(address[] memory _payees, uint256[] memory _shares)
    ERC721B("Metaverse Punks", "MVP")
  ...
  function mint() external payable nonReentrant() {
    ...
    require(mintableCount > 0, "No mints" );
    require(isOpen || _claims[msg.sender] != claimStatus.Claimed, "Already Claimed");
    require(isOpen || _claims[msg.sender] == claimStatus.Unclaimed, "Not on the list");
    ...
    mintableCount--;
		if (!isOpen) {
    	_claims[msg.sender] = claimStatus.Claimed;
		}
  ...
  function setMintableCount(uint count) external onlyOwner {
    mintableCount = count;
  }
  ...
  function addToList(address[] memory members) external onlyOwner {
    for (uint256 i = 0; i < members.length; ++i) {
      _claims[members[i]] = claimStatus.Unclaimed;
    }
  }

  function removeFromList(address[] memory members) external onlyOwner {
    for (uint256 i = 0; i < members.length; ++i) {
      _claims[members[i]] = claimStatus.Invalid;
    }
  }
