...
contract NFTPawnShop is NFTfiAdmin, NFTfiSigningUtils, ERC721 {
    ...
    function giveLoan(
        ...
        Loan memory loan = Loan({
            ...
            loanStartTime: uint64(now), //_loanStartTime
            loanDuration: uint32(_loanDuration),
            loanInterestRateForDurationInBasisPoints: uint32(0),
            loanAdminFeeInBasisPoints: uint32(_adminFeeInBasisPoints),
            nftCollateralContract: _nftCollateralContract,
            loanERC20Denomination: _loanERC20Denomination,
        ...
        // uint256 value that they have not used yet for an off-chain NFTfi
        // signature).
        require(!_nonceHasBeenUsedForUser[_borrower][_borrowerNonce], 'Borrower nonce invalid, borrower has either cancelled/begun this loan, or reused this nonce when signing');
        _nonceHasBeenUsedForUser[_borrower][_borrowerNonce] = true;

        // Check that both signatures are valid.
        require(isValidBorrowerSignature2(
            loan.nftCollateralId,
            _borrowerNonce,//_borrowerNonce,
            loan.loanPrincipalAmount,
            loan.maximumRepaymentAmount,
            loan.loanDuration,
            loan.nftCollateralContract,
    ...
    function isValidBorrowerSignature2(
        uint256 _nftCollateralId,
        uint256 _borrowerNonce,
        uint256 _loanPrincipalAmount,
        uint256 _maximumRepaymentAmount,
