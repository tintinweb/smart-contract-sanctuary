...
contract metaZooSale is Ownable {
    ...
    */
    ...
    modifier initializeDone() {
        require(setupStatus, "Meta Zoo Set up not initialized yet");
        _;
    }
    ...
    uint256 sales_duration = 12 hours;
    address payable _multisig;
    bool public setupStatus = false;
    ...
    uint256[] public _shares = [150, 750, 100];
    address payable[] _wallets = [
        payable(0x31EFd75bc0b5fbafc6015Bd50590f4fDab6a3F22),
        payable(0xDb89C1C66191E6B3Af2A299fbfF2E22EB1Fe1AE0),
        payable(0x71230F34Fc920cB92c2D0FD555a29aeF73498bDc)
    ];
    ...
    function whiteListBySignature(address _recipient, uint256 tokenCount, bytes memory signature, uint64 _role)
        public
        payable
    {
        require(
        ...
        require(msg.value >= tokenCount * (whiteListPrice), "Price not met");
        uint256 this_taken = whitelist_claimed[msg.sender] + tokenCount;

        SkeletonCrew(nft_sales).mintCards(tokenCount, _recipient);
        (bool sent, bytes memory data) = _multisig.call{value: msg.value}(""); // don't use send or xfer (gas)
        require(sent, "Failed to send Ether");
        whitelist_claimed[msg.sender] = this_taken;
        require(_role >= whitelist_claimed[msg.sender], "Too many tokens requested");
        whiteListSold += tokenCount;
        require(whiteListSold <= 4300, "sold out");
        // Fix this again.
    }
    ...
    function initialize() public onlyOwner {
        require((setupStatus == false), "Initialized");
        // can use to check if being allowed in skeletonCrew.
        setupStatus = true;
    }

    function currentPrice() public view returns (uint256) {
        uint256 gap = block.timestamp - sales_start;
        uint256 counts = gap / (15 * 60);
    ...
    function decreaseRemainingTokens() public view returns (uint256) {
        return maxDecreaseNFTs - maxDecreaseSold;
    }

    function testPrice(uint256 gap) public view returns (uint256) {
        uint256 counts = gap / (15 * 60);
        if (gap >= sales_duration) {
            return 0.2 ether;
        }
    ...
    constructor(
        ...
        _multisig = __multisig;
        presigner = _presigner;
    }

    function purchase(uint256 _amount) public payable initializeDone {
        require(
            salesActive(),
            "Sales has not started or ended , please chill sir."
        );
        ...
        require(maxDecreaseSold <= 500, "sold out");
    }

    function reservedMint(uint64 _amount, address _receiver) public onlyOwner {
