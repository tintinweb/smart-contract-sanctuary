    ...
    Ownable
{
    ...
    uint256 private constant _batchSize = 1000;
    string private constant _uriExtension = "json";

    uint256 private _maxAmountPerMint = 5;
    uint256 private _reservedMints = 30;         
    uint256 private _currentBatchID;
    uint256 private _maxTotal = 10000;
    uint256 private _cost = 0.07 ether;
    
    bool private _initialBatchSet;
    bool private _isPaused = true;

    uint256 private _tokenIdTracker;

    /**
     * maps batch id to correspondend CID, which is used to build tokenURI
     * Example: 0 -> QmNzwDTWd8sXdedsxTZ5oRGGo43NMU6BbhBmUqDxBT7it7C
     */
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        return string(abi.encodePacked(_baseURI(),tokenURIforBatch(tokenId), "/", Strings.toString(tokenId), ".", _uriExtension));
    }
    
    function tokenURIforBatch(uint256 tokenId) internal view returns (string memory) {
    ...
    function mint(address to, uint256 amount) public payable {
        ...
        require(_initialBatchSet, "initial batch is not set");
        require(amount > 0, "need to mint at least 1");
        require(amount <= _maxAmountPerMint, "max amount per mint is exceeded");
        require(totalSupply() < ((_currentBatchID+1)*_batchSize), "current batch supply is exceeded"); // should not exceed current batch supply
    ...
    function mintReserved(address to) public onlyOwner {
        require(_initialBatchSet, "initial batch is not set");
        require(_tokenIdTracker < (_maxTotal - _reservedMints), "total supply is exceeded");
    ...
    function airDrop(address[] memory addresses) public onlyOwner {
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            _safeMint(addresses[i], _tokenIdTracker);
            emit Mint(msg.sender, totalSupply());
            _tokenIdTracker++;
        }
    }
