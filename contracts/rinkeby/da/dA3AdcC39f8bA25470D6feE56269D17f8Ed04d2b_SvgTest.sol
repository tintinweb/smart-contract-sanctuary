...
contract SvgTest is ERC721Enumerable, Ownable {
	...
	function generateSVG() private pure returns (string memory) {
		return
			string(
				abi.encodePacked(
					'<svg id="test" viewbox="0 0 250 250">',
						'<script>for (var i  = 0; i < 5; i++) { var test = document.getElementById("test"); var element = document.createElementNS("http://www.w3.org/2000/svg", "circle"); element.setAttribute("cx", 30 * (i + 1)); element.setAttribute("cy", 30 * (i + 1)); element.setAttribute("r", "10"); test.appendChild(element); }</script>',
					'</svg>'
				)
			);
	}
