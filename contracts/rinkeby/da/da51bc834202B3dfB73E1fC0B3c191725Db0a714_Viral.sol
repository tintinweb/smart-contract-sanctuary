...
contract Viral is IERC20, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (!_isExcludedFromFee[sender]) {
            ...
            */
            ...
            uint256 fAmount = amount.mul(totalFee).div(100);

            tAmount = amount.sub(fAmount);



            uint256 rAmount = referralFee.mul(fAmount).div(totalFee);

            uint256 vAmount = viralFee.mul(fAmount).div(totalFee);

            uint256 dAmount = fAmount.sub(rAmount).sub(vAmount);



            _updateAccount(defaultReferrer);

            address rReferrer = referrerOf(recipient);

            if (rReferrer != defaultReferrer && rReferrer != sender) {
            ...
            _balances[defaultReferrer] = _balances[defaultReferrer].add(vAmount);
    ...
    function _addReferrer(address _referee, address _referrer) internal {
        ...
        require(_isReferredYet[_referrer], "VIRAL: Referrer not a valid referrer");
