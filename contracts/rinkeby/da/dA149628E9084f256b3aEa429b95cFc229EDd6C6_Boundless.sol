...
contract Boundless is 
    ...
    struct chainData {
        address author; // Address of the caller of writeBoundless()
    ...
    constructor () public ERC721 ("Boundless", "JIP6") {
        tokenCounter = 1;
    ...
    function writeBoundless(uint256 tokenId, string memory text) public returns (uint256) {
        return writeBoundless(tokenId, 0, text);
    }

    // Writes input text onto the chain with a reference to a previous message
    function writeBoundless(uint256 tokenId, uint256 prevId, string memory text) public returns (uint256) { // TODO: TEST
        require(enabled, "Contract not enabled");
