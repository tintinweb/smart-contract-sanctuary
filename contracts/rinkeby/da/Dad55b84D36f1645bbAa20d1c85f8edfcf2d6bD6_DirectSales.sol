...
contract DirectSales is OwnableClone, RevenueSplit, SalesHistory {
  ...
  event ListingAdded(uint256 indexed listingId, address tokenAddress, uint256 offeringId, uint256 price, uint64 openTime, uint64 closeTime);
  event ListingUpdated(uint256 indexed listingsById, uint256 price, uint64 openTime, uint64 closeTime);
  ...
  function addListing(
    ...
    require(openTime < closeTime, "negative time span");
    // solhint-disable-next-line not-rely-on-time
    require(closeTime > block.timestamp, "already closed");
    ...
    emit ListingAdded(idx, tokenAddress, offeringId, price, openTime, closeTime);
  }

  function updateListing(uint256 listingId, uint256 price, uint64 openTime, uint64 closeTime) public onlyOwner {
    require(openTime < closeTime, "negative time span");
    // solhint-disable-next-line not-rely-on-time
    require(closeTime < block.timestamp, "already closed");
    require(currentListingIds.contains(listingId), 'No such listing');
    ...
    emit ListingUpdated(listingId, price, openTime, closeTime);
  }
