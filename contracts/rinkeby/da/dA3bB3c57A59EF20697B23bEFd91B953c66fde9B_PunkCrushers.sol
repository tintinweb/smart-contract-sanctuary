...
contract PunkCrushers is IPunkCrushers, Ownable, ERC721Enumerable {
    ...
    bool public override presaleActive = false;
    ...
    constructor(
        ...
        // premint punk crushers
        for (uint8 i; i < _premintCount; i++) {
            // incrementing by 1
    ...
    function presaleMint(uint256 _noOfTokens)
        public
        payable
        override
        returns (uint256)
    {
        ...
        return _mint(_noOfTokens, msg.value);
    }

    function mint(uint256 _noOfTokens)
        public
        payable
        override
        returns (uint256)
    ...
    function _mint(uint256 _noOfTokens, uint256 _value)
        internal
        returns (uint256 _tokenId)
    {
        require(
            totalSupply() + _noOfTokens <= MAX_SUPPLY,
            "Mint: max limit reached"
        );
        require(_value >= _tokensPrice(_noOfTokens), "Mint: invalid price");
    ...
    function _tokensPrice(uint256 _noOfTokens) internal pure returns (uint256) {
...
interface IPunkCrushers {
    ...
    event NFTMinted(uint256 tokenId, address beneficiary);
    ...
    function mint(uint256 _noOfTokens) external payable returns (uint256);
