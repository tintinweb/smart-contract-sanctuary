...
contract OlympusBondDepository is Ownable {
    ...
    function stakeOrSend( bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            ...
            address stakeToUse = staking;
            if ( stakingHelper != address(0) ) {
                stakeToUse = stakingHelper;
            } 
            IERC20( OHM ).approve( stakeToUse, _amount );
            IStaking( stakeToUse ).stake( _amount, msg.sender ); // stake payout
        }
        return _amount;
    }
    ...
    function adjust() internal {
        uint blockCanAdjust = adjustment.lastBlock.add( adjustmentBuffer );
    ...
    function currentDebt() public view returns ( uint ) {
        ...
        uint toDecay = totalDebt.mul( blocksSinceLast ).div( terms.vestingTerm );
        if ( toDecay > totalDebt ) {
            toDecay = totalDebt;
        }
        return totalDebt.sub( toDecay );
    }
    ...
    function decayDebt() internal {
        ...
        totalDebt = totalDebt.sub( toDecay );
    }
