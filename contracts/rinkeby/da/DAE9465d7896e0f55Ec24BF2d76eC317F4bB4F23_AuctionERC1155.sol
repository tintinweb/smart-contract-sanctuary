...
contract AuctionERC1155 {
    ...
    ArtabiaERC1155 public NFTContract1155;
    ...
    // The token token
    ERC20Interface public erc1155Token;
    ...
    address payable _Owner = 0xdF6e287f9a723afaA8B2B42a984Bdc0714b35974;
    ...
    constructor(address _NFTAddress) public {
        NFTContract1155= ArtabiaERC1155(_NFTAddress);
    }
        ...
        returns (uint256, address)
    {
        ...
        if (bidsLength > 0) {
            ...
            return (lastBid.amount, address(0));
        }
        return (0, address(0));
    }
    ...
    {
				uint256 quantity=NFTContract1155.balanceOf(msg.sender, _productId);
    ...
    // Fixed price Auction
    function ERC1155FixedAuction(
        uint256 _auctionId,
        uint256 _quantity,
        uint256 _singlePrice
    ) public payable {
        Auction memory myAuction = auctions[_auctionId];
        // if (myAuction.startPrice != _singlePrice) {
        //     revert();
        // }
        require(myAuction.startPrice == _singlePrice, "msg.value is incorrect" );

				(,,address payable creator,,,,uint256 royaltyFee,,)=NFTContract1155.ercTokens(myAuction.productId);

        // NFTContract.safeTransferFrom(myAuction.owner, to, tokenId);(msg.sender, myAuction.productId);

				uint256 serviceFeeAndComission = msg.value.mul(serviceFee.add(commission)).div(10**4);

				uint256 royalty = msg.value.mul(royaltyFee).div(100);

        if (!myAuction.owner.send(msg.value.sub(serviceFeeAndComission).sub(royalty))) {
            revert();
        }

				if(!creator.send(royalty)){
					revert();
				}

				if(!_webOwner.send(serviceFeeAndComission)){
					revert();
				}

        
        NFTContract1155 = ArtabiaERC1155(myAuction.deedRepositoryAddress);
        // myAuction.deedRepositoryAddress.delegatecall(
        //     abi.encodeWithSignature(
        //         "transferERC1155Token(address,address,uint256,uint256)",
        //         myAuction.owner,
        //         msg.sender,
        //         myAuction.productId,
        //         _quantity
        //     )
        // );
        NFTContract1155.transferERC1155Token(myAuction.owner, msg.sender, myAuction.productId,_quantity);

				if(NFTContract1155.balanceOf(myAuction.owner, myAuction.productId)==0){
    ...
    //trying bid place on unlimited auction
    /**
    ...
    */
    ...
    //         // require(
    //         //     erc1155Token.transferFrom(msg.sender, feeCollector, feeAmount),
    //         //     "Transfering the bid fee to the Marketplace owner failed"
    //         // );
    //     }

        uint256 expiresAt = _duration;
    ...
    */
    function _requireBidderBalance(address _bidder, uint256 _amount) internal view {
        require(
            erc1155Token.balanceOf(_bidder) >= _amount,
            "Insufficient funds"
        );
        require(
            erc1155Token.allowance(_bidder, address(this)) >= _amount,
    ...
    */
     ...
     */
    function bidOnUnlimitedAuction( address _tokenAddress, uint256 _tokenId, uint256 _price, uint256 _amount) public payable {
 ...
 */
contract ArtabiaERC1155 is ERC1155, ERC1155MintBurn, ERC1155Metadata, Ownable {
    ...
    mapping(address => uint256[]) public ercTokenOwner;
    mapping(address => uint256[]) public ercTokenOwned;
     ...
     * @param _owner address of the auction owner
     */
    function getTokensOfERC1155(address _owner)
        public
        view
        returns (uint256[] memory)
    {
        uint256[] memory ownedERCTokens = ercTokenOwner[_owner];
        return ownedERCTokens;
    }
     ...
     * @param _owner address of the auction owner
     */
    function getOwnedTokensOfERC1155(address _owner)
        public
        view
        returns (uint256[] memory)
    {
        uint256[] memory ownedERCTokens = ercTokenOwned[_owner];
        return ownedERCTokens;
    }
    ...
    function create(
        ...
        ercTokenOwner[msg.sender].push(_id);

        ercTokens[_id] = ercToken(
            _id,
            _initialOwner,
            _initialOwner,
            _initialSupply,
    ...
    function transferERC1155Token(
        ...
        _safeTransferFrom(seller, to, _tokenId, _amount);
        ercTokenOwned[to].push(_tokenId);
        ercToken memory _productERC1155 = ercTokens[_tokenId];
        _productERC1155.owner = to;
        _productERC1155.onSale = false;
        ercTokens[_tokenId] = _productERC1155;
    }
}