...
contract ZKTTVesting is Ownable, Initializable, Testable {
    ...
    event SetLockedPercentage(address indexed from, uint16 lockedPercentage);
    ...
    mapping(address => uint) private cumulativeReleased;

    mapping(address => mapping(uint => Vesting)) private vestings;
    ...
    uint16 private lockedPercentage;

    uint private locked;

    modifier lock() {
        require(locked == 0, 'ZKTTVesting: LOCKED');
        locked = 1;
        _;
        locked = 0;
    }
    ...
    function getLockedPercentage() external view returns(uint16) {
    ...
    function setLockedPercentage(uint16 lockedPercentage_) external onlyOwner {
        ...
        emit SetLockedPercentage(msg.sender, lockedPercentage_);
    }
    ...
    function hasCumulativeReleased(address account) external view returns(uint){
        return cumulativeReleased[account];
    }
    ...
    function _reward(address to, uint amount, string calldata memo) internal {
        ...
        if (amount > 0){
            // Direct release
            uint directRelease = amount * (100 - lockedPercentage) / 100;
            // Linear release
