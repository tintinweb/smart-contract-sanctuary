...
/**
   @title Interface ISporesMinter
   @dev This is an interface to define SporesNFTMinter contract
   The interface does not include mint batch of Spores NFT Tokens
   Please refer to ISporesMinterBatch.sol if you are interested
*/
interface ISporesMinter {
...
import "./Interface/ISporesMinter.sol";
import "./Interface/IERC1155Mintable.sol";
import "./Interface/IERC721Mintable.sol";
import "./SporesRegistry.sol";

/**
   @title SporesNFTMinter contract
   @dev This contract is used to handle minting Spores NFT Tokens. There are two supporting NFT standards:
      + ERC-721 (https://eips.ethereum.org/EIPS/eip-721)
      + ERC-1155 (https://eips.ethereum.org/EIPS/eip-1155)
   This contract uses an interface ISporesMinter.sol which is a non-batch minting version
   Please refert to SporesNFTMinterBatch.sol if you are interested in a batch minting version
*/
contract SporesNFTMinter is ISporesMinter, Ownable {
    enum TYPE { ERC721_MINTING, ERC1155_MINTING }
    ...
    */
    function updateRegistry(address _newRegistry) external onlyOwner {
        require(
            ...
            "SporesNFTMinter: Set zero address"
        );
    ...
    */
    function mintSporesERC1155(
        ...
        emit SporesNFTMint(_msgSender(), registry.erc1155(), _tokenId, _amount);
    }
    ...
    function _verifySignature(
        ...
        bytes32 _data =
            keccak256(abi.encodePacked(_to, _tokenId, uint256(_type)));
        bytes32 _msgHash = ECDSA.toEthSignedMessageHash(_data);

        require(
            ECDSA.recover(_msgHash, _signature) == registry.verifier(),
            "SporesNFTMinter: verifier signature invalid"
        );
    }
}
...
contract SporesRegistry is Initializable, OwnableUpgradeable {
    ...
    */
    function registerNFTContract(address _contractNFT, uint256 _opcode)
        external
        onlyOwner
    {
        ...
        if (_opcode == NFT721_OPCODE) {
            ...
            // and integrate malicious implementation
            require(
                IERC1155(_contractNFT).supportsInterface(
