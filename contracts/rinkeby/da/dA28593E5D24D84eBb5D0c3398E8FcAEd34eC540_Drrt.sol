...
contract Drrt is Context, ERC165, Ownable, IERC721Metadata {
    ...
    uint public constant TOKEN_REDUCED_PRICE = 65000000000000000;
    ...
    function balanceOf(address tokenOowner) public view override returns (uint) {
    ...
    function tokenOfOwnerByIndex(address tokenOowner, uint index) public view returns (uint) {
    ...
    function mint(uint tokenAmount) external payable {
        ...
        if (block.timestamp < _saleStart + 86400) {
            tokenPrice = TOKEN_REDUCED_PRICE;
        } else {
            tokenPrice = TOKEN_PRICE;
        }

        require(msg.value >= tokenPrice * tokenAmount, "Ether value sent is not correct");
        ...
        if (msg.value - tokenPrice > 0) {
            payable(_msgSender()).transfer(msg.value - tokenPrice);
        }
    }
