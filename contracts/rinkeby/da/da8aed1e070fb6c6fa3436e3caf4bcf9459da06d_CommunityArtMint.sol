...
contract CommunityArtMint is Ownable {
    ...
    //uint256[] public tokenSeed = new uint256[](9999);
    uint256[] public seedOfToken = new uint256[](9999);
    ...
    function mintMinor(uint256 edition, uint256 seed) public payable {

        require(minorMintOngoing[edition], "Mint is not active");
        require(msg.value == mintPrice, "Wrong price");
        require(seedOfToken[seed] == 0, "Seed already exists, choose another one");
        require(seed >= 100000000000000000000000000000, "Seed phrase to low");
        require(seed <= 999999999999999999999999999999, "Seed phrase to high");
        require(nextMinorId[edition] <= maxMinorId[edition], "All tokens available have been minted");
        
        uint256 tokenId = edition * EDITION_SIZE + nextMinorId[edition];
        nextMinorId[edition] += 1;
        //tokenSeed[tokenId] = seed;
        seedOfToken[seed] = tokenId; 
    ...
    function getSeed(uint256 seed) public view virtual returns (uint256) {
        return seedOfToken[seed];
    }
    // Only owner functions.
