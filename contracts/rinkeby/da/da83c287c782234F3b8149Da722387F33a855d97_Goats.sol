...
contract Goats is ERC721Enumerable, Ownable, ReentrancyGuard {
	...
	//presale pricing
	uint256 public priceForLessThan3Presale = 40000000000000000; //0.040 ETH each
	uint256 public priceForLessThan10Presale = 35000000000000000; //0.035 ETH each
	uint256 public priceForGreaterThan10Presale = 30000000000000000; //0.030 ETH each
	...
	/*
	 * buy max 20 tokens (PRESALE)
	 */
	function buyPresale(uint256 amount) public payable nonReentrant {
		require(amount > 0, "minimum 1 token");
		require(amount <= maxToMint - tokenIndex, "greater than max supply");
		...
		uint256 expectedPrice = 2**256 - 1;
		if (amount >= 10) {
			expectedPrice = amount * priceForGreaterThan10Presale;
		}
		if (amount < 10) {
			expectedPrice = amount * priceForLessThan10Presale;
		}
		if (amount < 3) {
			expectedPrice = amount * priceForLessThan3Presale;
		}
	...
	function changeName(uint256 nftID, string memory newName) external payable {
		require(ownerOf(nftID) == msg.sender, "ownership required");
		require(changeNamePrice == msg.value, "exact value needed");
	...
	function toggleMinting(bool _isActive) external onlyOwner {
