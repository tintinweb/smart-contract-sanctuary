...
contract Numbers is ERC721, Ownable {
    ...
    constructor() ERC721("Numbers", "Numbers") {}

    function withdrawAllFunds() public onlyOwner {
        payable(msg.sender).transfer(address(this).balance);
    }

    function mint(uint howMany) public {
        require(howMany > 0, "howMany must be greater than 0");
        require(howMany <= maxMint, "howMany must be at most maxMint");
        require(totalSupply + howMany < maxId, "Not enough numbers left");
    ...
    function mintNum(uint num) public onlyOwner returns (uint256) {
        require(num < maxId && num >= totalSupply, "We ran out of numbers");
        uint newItemId = num;
        totalSupply = num;
        _safeMint(msg.sender, newItemId);
        return newItemId;
    ...
    {
        string memory strId = ToString.numberToString(itemId);
        ...
        bytes memory imageBytes = abi.encodePacked(
            string(abi.encodePacked(
                "<svg viewBox='0 0 640 640' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' class='", classname, "'>",
                    ...
                    //         "stroke-width: 0.2em;",
                    //         "stroke-linecap: butt;",
                    //         "stroke-linejoin: miter;")),string(abi.encodePacked(
                    //         "stroke-opacity: 1;",
                    ...
                    //     // ".sqr { border: rgba(162, 59, 114, 0.5) 2em solid; }",
                    // "</style>",
        ...
        // string memory image = string(abi.encodePacked("data:image/svg+xml;base64,", imageBytes));
        string memory image = string(abi.encodePacked("data:image/svg+xml;utf8,", imageBytes));

        string memory strThisAddress = ToString.addressToString(address(this));

        bytes memory json = abi.encodePacked(
            string(abi.encodePacked(
                '{',
                    '"name": "Number ', strId, '",',
                    '"description": "A million numbers.",',
                    '"image": "', image, '",',
                    '"royaltyInfo": { "royaltyFeeByID": "10", "artistAddress":"',strThisAddress,'" },',
                    '"payout_address": "',strThisAddress, '",')),string(abi.encodePacked(
                    '"traits": [ ',
                        ...
                        traits.div7 ? '{"trait_type": "Divisible By 7", "value": "Divisible By 7" },' : '')),string(abi.encodePacked(
                        traits.div10 ? '{"trait_type": "Divisible By 10", "value": "Divisible By 10" },' : '',
                        // traits.div11 ? '{"trait_type": "Divisible By 11", "value": "Divisible By 11" },' : '',
                        traits.div42 ? '{"trait_type": "Divisible By 42", "value": "Divisible By 42" },' : '',
                        traits.div69 ? '{"trait_type": "Divisible By 69", "value": "Divisible By 69" },' : '')),string(abi.encodePacked(
                        ...
                        traits.div1000 ? '{"trait_type": "Divisible By 1000", "value": "Divisible By 1000" },' : '')),string(abi.encodePacked(
