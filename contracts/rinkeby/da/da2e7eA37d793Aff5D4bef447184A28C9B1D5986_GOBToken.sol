...
contract GOBToken is ERC1155, Ownable, ERC1155Burnable, Pausable {
    ...
    mapping (address => bool) public redeemableContracts;
    ...
    function calculateProof(uint amount) public {
        test = keccak256(abi.encodePacked(msg.sender, amount));
    }

    function claim(uint256 numPasses, uint256 mpIndex, uint amount, bytes32[] memory proof) public whenNotPaused {
        require(claimedMPs[msg.sender].add(numPasses) <= amount, "Claim: Not allowed to claim given amount");
        require(proof.verify(merkleRoot, keccak256(abi.encodePacked(msg.sender, amount))), "You are not in the list");
        claimedMPs[msg.sender] = claimedMPs[msg.sender].add(numPasses);
        _mint(msg.sender, mpIndex, numPasses, "");
        emit Claimed(msg.sender, numPasses);
    }
