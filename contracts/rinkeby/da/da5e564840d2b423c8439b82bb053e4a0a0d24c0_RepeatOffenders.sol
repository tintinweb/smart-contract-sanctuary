...
// REPEAT OFFENDERS
// https://pvnks.com/RepeatOffenders
...
contract RepeatOffenders is ERC721, Ownable {
    ...
    string public RO_PROVENANCE = ""; // PLVNDER to add our hash, as well as the seed used to generate the offenders

    uint256 public thePriceForFreedom = 30000000000000000; // 0.03 ETH

    uint public constant hostBodyMaxPurchase = 20;

    uint256 public availableHosts = 5000;
    ...
    function reserveRepeaters(address _to, uint256 _reserveAmount) public onlyOwner {        
        ...
        foundersReserve = foundersReserve.sub(_reserveAmount);
    }


    function setThePriceOfFreedom(uint256 newPrice) public onlyOwner {
        thePriceForFreedom = newPrice;
    }

    function setProvenanceHash(string memory provenanceHash) public onlyOwner {
        RO_PROVENANCE = provenanceHash;
    }
    ...
    function buyYourFreedom(uint numberOfTokens) public payable {
        ...
        require(block.number < 9654319); // edited for rinkeby blocks
        require(numberOfTokens > 0 && numberOfTokens <= hostBodyMaxPurchase, "This is not possible");
        require(totalSupply().add(numberOfTokens) <= availableHosts, "Supply would be exceeded");
        require(msg.value >= thePriceForFreedom.mul(numberOfTokens), "Ether value sent is incorrect");
        
        for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply();
            if (totalSupply() < availableHosts) {
