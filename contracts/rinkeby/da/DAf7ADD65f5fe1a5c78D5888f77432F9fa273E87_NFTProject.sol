...
import "./BlockVault.sol";


interface IScores {
    function getTokenScore(uint256 tokenId) external view returns (uint256 score);
}


contract NFTProject is ERC721, Ownable {

    uint32 constant public MAX_BLOCKS = 10000;
    uint8 constant public MAX_BLOCKS_PER_ADDRESS = 2;
    ...
    NFTStaking vault;
    IScores internal scores;

    event BlockMinted(address account, uint256 tokenId, bool staked);

    constructor() ERC721("NFT Blocks", "NFTB") {
        setBaseURI(baseTokenURI);
    }

    function setVault(NFTStaking addr) external onlyOwner {
      vault = addr;
    }

    function setScores(address _scores) public onlyOwner {
        scores = IScores(_scores);
    }
    ...
    function _mintNFT(uint256 amount, bytes32 signatureR, bytes32 signatureVS, bool stake) internal {
        require(saleActive == true, "Sales are disabled");
        ...
        uint24[] memory tokens = new uint24[](amount);
        for(uint256 i=0; i < amount; i++) {
          _tokenIdCounter += 1;

          uint256 newItemId = _tokenIdCounter;
          if (stake) {
            _mint(address(vault), newItemId);
          } else {
            _mint(msg.sender, newItemId);
          }
          tokens[i] = uint24(newItemId);

          emit BlockMinted(msg.sender, newItemId, stake);
        }
        if (stake) {
          vault.stakeDuringMint(msg.sender, tokens);
        }
    }
        ...
        returns (uint256[] memory ownerTokens)
    {
        ...
        if (tokenCount == 0) {
            ...
            for (Id = 1; Id <= tokenCount; Id++) {
                result[resultIndex] = tokenOfOwnerByIndex(owner, Id - 1);
                resultIndex++;
            }
        ...
        returns (uint256 tokenId)
    {
        ...
        unchecked {
            for (; tokenId < MAX_BLOCKS; ++tokenId) {
...
contract NFTStaking is Ownable, IERC721Receiver {
  ...
  function stakeDuringMint(address account, uint24[] calldata tokenIds) external {
  ...
  function stake(uint24[] calldata tokenIds) external {
    uint256 tokenId;
    totalStaked += tokenIds.length;
    for (uint i = 0; i < tokenIds.length; i++) {
      tokenId = tokenIds[i];
  ...
  function _unstakeMany(address account, uint24[] calldata tokenIds) internal {
    uint256 tokenId;
    totalStaked -= tokenIds.length;
    for (uint i = 0; i < tokenIds.length; i++) {
      tokenId = tokenIds[i];
  ...
  function claim(uint24[] calldata tokenIds) external {
      _claim(tokenIds, false);
  }

  function unstake(uint24[] calldata tokenIds) external {
      _claim(tokenIds, true);
  }

  function _claim(uint24[] calldata tokenIds, bool _unstake) internal {
    uint256 tokenId;
    uint256 score;
    uint256 earned = 0;
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      score = nft.getTokenScore(tokenId);
  ...
  function earningInfo(uint24[] calldata tokenIds) external view returns (uint256[2] memory info) {
     uint256 totalScore = 0;
     uint256 earned = 0;

     for (uint i = 0; i < tokenIds.length; i++) {
      uint256 tokenId = tokenIds[i];
      uint256 score = nft.getTokenScore(tokenId);
      totalScore += score;
    ...
    return [earned, totalScore * 1 ether / 1 days];
  }
