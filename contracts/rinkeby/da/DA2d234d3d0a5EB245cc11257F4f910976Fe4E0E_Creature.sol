...
//inherit ierc20: override function
...
//erc20
    ...
    mapping(address=>address) public pNFT_Token_addrs;
    mapping (address => uint256)public personal_NFTS;
    //address public constant me = 0x633C24904b6361B7eB6efe181722FD4c85160D7A;

    mapping(address => mapping(uint256 => bool)) public Is_deposit_person;
...
//get erc721 token name
        string memory nft_name =Strings.strConcat(erc721metadata.name(),"(", Strings.uint2str(_tokenId),")Shares"); 
        
//erc721 token
        Is_deposit_person[msg.sender][_tokenId] = true;
        erc721metadata.safeTransferFrom( msg.sender,receiver, _tokenId);
//erc20 token
        string memory cost_string = Strings.uint2str(shares);
        pNFT_Token_addrs[msg.sender] = address(new RobinToken(nft_name,cost_string));
        New_erc20_token = RobinToken(pNFT_Token_addrs[msg.sender]);
        New_erc20_token.transfer(msg.sender,shares);
        personal_NFTS[msg.sender] = shares;
...
// approve address(this) on erc20 token contract(New_erc20_token) first (by receiver)
    function withdrawNFT(address _nftAddress,uint256 _tokenId,uint shares) public{
          New_erc20_token = RobinToken(pNFT_Token_addrs[msg.sender]);
         //require(Is_deposit_person[msg.sender][_tokenId],"you are not the token owner");
         //require(keccak256(abi.encodePacked((Strings.uint2str(shares)))) == keccak256(abi.encodePacked((New_erc20_token.totaltoken()))));
//erc721 token
        Is_deposit_person[msg.sender][_tokenId] = false;
         erc721metadata = ERC721Metadata(_nftAddress);
