    ...
    ReentrancyGuard
{
    ...
    uint256 public whitelistStart; // time the whitelisted presale starts
    uint256 public mintStart; // the time public mint starts

    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;

    constructor(string memory _initBaseURI, address _RoyaltiesRecipient)
    ...
    function setTimes(uint256 _whitelistStart, uint256 _mintStart)
        external
        onlyOwner
    {
    ...
    function _isMintable(uint256 qty) private view returns (bool) {
        // if no times have been set, or if the public mint has started
        if (
            (whitelistStart == 0) ||
            (mintStart == 0) ||
        ...
        if (
            ...
            // check whitelist
            return (qty + balanceOf(msg.sender)) <= whitelist[msg.sender];
        }
        return false;
    }
    ...
    function mint(address to, uint256 qty) external payable nonReentrant {
        require(_isMintable(qty), "closed");
        require((qty + totalSupply()) <= tokenLimit, "sold out");
        require(
            (qty + balanceOf(to)) <= balanceLimit,
