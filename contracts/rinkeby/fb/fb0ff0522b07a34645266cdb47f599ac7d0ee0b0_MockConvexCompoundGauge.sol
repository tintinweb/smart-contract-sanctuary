...
interface IBooster {
   ...
   function batchKill( uint256 _pid, address[] memory users, uint256 totalToken ) external returns(address);
...
contract MockConvexCompoundGauge {
    ...
    struct UserLending {
        bytes16 lendingId;
        uint256 pid; // pool id
        // uint256 convexPid; // from pid lpToken
        // uint256 compoundPid; // out target token, like wethusdt
        uint256 token0Value;
        uint256 token1Value;
    ...
    function borrow(
        ...
        uint256 interestRate = ILendingInterest(pool.interest)
            .getBorrowInterestRate(utilizationRate.mul(1000).div(1e18), _pid);

        uint256 lendingAmount;
        uint256 interestAmount =  lendingAmount.mul(interestRate);
        ...
        IBooster(compoundBooster).lockToken(
            _pid,
            msg.sender,
            token1Amount.div(pool.lendingThreshold).div(1000)
        );
        ...
        userLendings[msg.sender].push(
            UserLending({
                lendingId: lendingId,
                pid: _pid,
                // convexPid: pool.convexPid,
                // compoundPid: pool.compoundPid,
                token0Value: token0Amount,
                // token1Value: token1Amount.div(pool.lendingThreshold).div(1000),
                token1Value: token0Amount,
        ...
        borrowInfo[_pid].borrowAmount = borrowInfo[_pid].borrowAmount.add(token0Amount);
        borrowInfo[_pid].supplyAmount = borrowInfo[_pid].supplyAmount.add(token1Amount);
    ...
    function DemoAddPool() public {
        ...
        address _interest = 0x12E04465eC448c4B3f3bD0830c6eb892b148F62c;
        // address _amountOracle = 0xd8b934580fcE35a11B58C6D73aDeE468a2833fa8;
        
        address[] memory sSupportTokens = new address[](2);
        uint256 _lendingThreshold = 1000;

        sSupportTokens[0] = 0xd8b934580fcE35a11B58C6D73aDeE468a2833fa8;
        sSupportTokens[1] = 0xd8b934580fcE35a11B58C6D73aDeE468a2833fa8;

        addPool(_convexPid,_compoundPid,_liquidate,_interest,sSupportTokens,_lendingThreshold);
    }
    ...
    function cursor(
        ...
        uint256 _offset,
        uint256 _length
    ) public view returns (bytes16[] memory) {
        PoolInfo memory pool = poolInfo[_pid];

        uint256 start = _offset > 0 && pool.borrowIndex > _offset
        ...
        bytes16[] memory userLendingIds = new bytes16[](start - finish);
        // mapping(uint256 => mapping(uint256 => bytes16)) poolLending; // pool id => (borrowIndex => user lendingId)

        for (uint256 _stakeIndex = start; _stakeIndex > finish; _stakeIndex--) {
            bytes16 userLendingId = poolLending[_pid][_stakeIndex];
            userLendingIds[i] = userLendingId;
            i++;
        }

        return userLendingIds;

        // for (uint256 i = 0; i < pool.borrowIndex; i++) {
        //     bytes16 userLendingId = poolLending[_pid][i];
        //     LendingInfo memory lendingInfo = lendings[userLendingId];

        //     for (
        //         uint256 j = 0;
        //         j < userLendings[lendingInfo.user].length;
        //         j++
        //     ) {
