...
contract Ribbit is Context, Ownable, ERC20 {
    address public lflcContractAddress =
        0xBD9D9923661DD5f981BC3f1c09D56Ad3073600A9;

    IERC721Enumerable public lflcContract;
    ...
    constructor() Ownable() ERC20("Ribbit", "RIBBIT") {
        lflcContract = IERC721Enumerable(lflcContractAddress);
    }

    function claimById(uint256 tokenId) external {
        require(
            _msgSender() == lflcContract.ownerOf(tokenId),
            "MUST_OWN_TOKEN_ID"
        );
    ...
    function claimAllForOwner() external {
        uint256 tokenBalanceOwner = lflcContract.balanceOf(_msgSender());

        require(tokenBalanceOwner > 0, "NO_FROGS_OWNED");

        for (uint256 i = 0; i < tokenBalanceOwner; i++) {
            _claim(
                lflcContract.tokenOfOwnerByIndex(_msgSender(), i),
                _msgSender()
            );
        }
    }
