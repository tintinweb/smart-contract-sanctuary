...
contract TestNutDistributor is NutDistributor {
  constructor (address nutAddr, address _governor, address oracleAddr, uint blocksPerEpoch) NutDistributor(nutAddr, _governor, oracleAddr, blocksPerEpoch) {}
//    using SafeMath for uint;
...
contract NutDistributor is Governable, INutDistributor {
    ...
    constructor (address nutAddr, address _governor, address oracleAddr, uint blocksPerEpoch)  {
    ...
    function collect() external  {
        ...
        if (totalAmount > 0) {
            IERC20(nut).safeApprove(address(this), 0);
            IERC20(nut).safeApprove(address(this), totalAmount);
            IERC20(nut).safeTransferFrom(address(this), lender, totalAmount);
        }
    }
...
contract MockNutmegV2 is Nutmeg {
