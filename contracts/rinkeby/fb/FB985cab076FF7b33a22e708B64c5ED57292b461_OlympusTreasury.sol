...
contract OlympusTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SSPST }
    ...
    uint public sSPSTQueue; // Delays change to sSPST address
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sSPST ).balanceOf( msg.sender ); // Can only borrow against sSPST held
    ...
    /**
        ...
        @param _amount uint
     */
    function repayDebtWithSPST( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );
    ...
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            ...
            value_ = _amount.mul( 10 ** IERC20( SPST ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SSPST ) { // 9
