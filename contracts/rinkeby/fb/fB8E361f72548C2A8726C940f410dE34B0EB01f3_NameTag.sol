...
contract NameTag is ERC721, Ownable, StringUpper, DenyList, Random {
    ...
    uint256 private _limitRandomSupply = 10000;
    ...
    constructor(string memory name_, string memory symbol_, uint256 price_, uint8 tokenAmountBuyLimit_) ERC721(name_, symbol_) {
        _price = price_;
        _tokenAmountBuyLimit = tokenAmountBuyLimit_;

        _wallet = address(uint160(owner()));
    ...
    function limitedSelling() public view virtual returns (bool) {
        return totalSupply() < limitSupply() && currentWaveIndex() < waves.length - 1;
    }
    ...
    function waveByIndex(uint8 waveIndex_) public view virtual returns (uint256, uint256) {
        require(waveIndex_ >= 0 && waveIndex_ < waves.length, "NameTag: Incorrect waveIndex");
    ...
    function setWaveStartTime(uint8 waveIndex_, uint256 startTime_) public virtual onlyOwner {
        ...
        require(startTime_ != 0, "NameTag: Value of startTime cannot be equal to 0");
        require(block.timestamp <= startTime_, "NameTag: Value of startTime is older than now time");

        require(waves[waveIndex_].startTime == 0);
    ...
    function setTokenAmountBuyLimit(uint8 tokenAmountBuyLimit_) public virtual onlyOwner {
        require(tokenAmountBuyLimit_ > 0, "NameTag: Cannot set 0 tokenAmountBuyLimit");
        _tokenAmountBuyLimit = tokenAmountBuyLimit_;
    }
    ...
    function _preValidatePurchase(address recipient, uint256 _weiAmount) internal view {
        require(recipient != address(0));
        require(_weiAmount != 0);

        uint256 time = waves[currentWaveIndex()].startTime;
        require(time != 0 && block.timestamp >= time, "NameTag: Payable function called too early");
    }
    ...
    function _processPurchase(address recipient, uint256 _tokenAmount, bool returnData) internal returns (uint256[] memory){
        require(returnData);

        uint256[] memory tokens = new uint256[](_tokenAmount);
        for (uint index = 0; index < _tokenAmount; index += 1) {
            tokens[index] = _processPurchaseToken(recipient);
        }
        return tokens;
    }
    ...
    function _setName(string memory _name, uint256 _token) internal returns (bool) {
        address recipient = msg.sender;

        require(recipient != address(0));

        if (recipient != ownerOf(_token)) {
            return false;
        }
        ...
        names[upperName] = _token;
        string memory oldName = upper(tokenURI(_token));
        names[oldName] = 0;

        _setTokenURI(_token, _name);

        return true;
    }

    function setNames(string[] memory _names, uint256[] memory _tokens) public returns (bool[] memory) {
        ...
        for (uint index = 0; index < _tokens.length; index += 1) {
            statuses[index] = _setName(_names[index], _tokens[index]);
    ...
    function _setNames(string[] memory _names, uint256[] memory _tokens) internal returns (bool[] memory) {

        uint256 length;
        if (_names.length < _tokens.length) {
    ...
    function _buyTokens() internal returns (uint256) {
        ...
        _preValidatePurchase(recipient, weiAmount);

        uint256 tokensAmount = _getTokenAmount(weiAmount);
        require(tokensAmount > 0);
        require(tokensAmount <= _tokenAmountBuyLimit);

        return tokensAmount;
    }


    function buyTokens(string[] memory _names) external payable returns (uint256[] memory, bool[] memory) {
        uint256 tokensAmount = _buyTokens();
        address recipient = msg.sender;

        uint256[] memory tokens = _processPurchase(recipient, tokensAmount, true);
        _forwardFunds();

        return (tokens, _setNames(_names, tokens));
    }
    ...
    receive() external payable {
        ...
        _processPurchase(recipient, tokensAmount);
        _forwardFunds();
    }
}
