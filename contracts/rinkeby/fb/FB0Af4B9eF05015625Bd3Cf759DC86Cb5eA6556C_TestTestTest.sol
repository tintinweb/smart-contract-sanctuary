...
contract TestTestTest is ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => string) public _tokenIdToHash;

    uint256 SEED_NONCE = 0;
    uint256 MAX_BABY_SUPPLY = 10000;
    uint256 BASE_CHEETH_COST = 30000000000000000000;
    uint256 CHEETH_COST_PER_BLOCK = 1000000000000000;

    uint256 BLOCKS_TILL_REVEAL = 50400;
    uint256 BLOCKS_TILL_RELEASE = 50500;
    ...
    function initiateBreeding(
        ...
        _addressToBreedingEvents[msg.sender].push(
            BreedingEvent(
                _currentBreedingEventId.current(),
                _parentId1,
                _parentId2,
                _tokenId,
    ...
    function speedUpParentRelease(
        ...
        uint256 thisBreedingEventIndex = findBreedingEventIndex(
            _breedingEventId
        );
        ...
        require(block.number < thisBreedingEvent.releaseBlock);

        //Calculate new release block
        uint256 newReleaseBlock = thisBreedingEvent.releaseBlock -
            (_cheethAmount / CHEETH_COST_PER_BLOCK);

        //Set the new release block
        ...
        public
    {
        ...
        require(block.number < _tokenToIncubator[_tokenId].revealBlock);

        //Calculate new reveal block
        uint256 newRevealBlock = _tokenToIncubator[_tokenId].revealBlock -
            (_cheethAmount / CHEETH_COST_PER_BLOCK);
    ...
    function findBreedingEventIndex(uint256 _breedingEventId)
        private
        view
        returns (uint256)
    {
        for (
            uint256 i = 0;
            i < _addressToBreedingEvents[msg.sender].length;
            i++
        ) {
            if (
                _addressToBreedingEvents[msg.sender][i].breedingEventId ==
                _breedingEventId
            ) return i;
        }

        revert("No breeding event with that id!");
    }

    function pullParentsByBreedingEventIndex(uint256 _breedingEventId) public {
        uint256 thisBreedingEventIndex = findBreedingEventIndex(
            _breedingEventId
        );
        ...
        if (
            thisBreedingEventIndex >=
            _addressToBreedingEvents[msg.sender].length
        ) return;

        for (
            uint256 i = thisBreedingEventIndex;
            i < _addressToBreedingEvents[msg.sender].length - 1;
            i++
        ) {
            _addressToBreedingEvents[msg.sender][i] = _addressToBreedingEvents[
                msg.sender
            ][i + 1];
        }
    ...
    function generateChildHash(
        ...
        for (uint8 i = 0; i < 9; i++) {
            SEED_NONCE++;
            uint256 geneticDecider = uint256(
                keccak256(
                    abi.encodePacked(
                        block.timestamp,
                        block.difficulty,
            ...
            if (geneticDecider >= 0 && geneticDecider <= 49) {
                childHashBytes = abi.encodePacked(
                    childHashBytes,
                    AnonymiceLibrary.substring(_parentHash1, i, i + 1)
                );
                continue;
            }
            if (geneticDecider >= 50 && geneticDecider <= 99) {
                childHashBytes = abi.encodePacked(
                    childHashBytes,
                    AnonymiceLibrary.substring(_parentHash2, i, i + 1)
                );
                continue;
            }
        }
    ...
    function revealBaby(uint256 _tokenId) public {
        ...
        _tokenIdToHash[_tokenId] = generateChildHash(
            _tokenId,
        ...
        hashToMinted[_tokenIdToHash[_tokenId]] = true;

        //Make it view as revealed
        _tokenToRevealed[_tokenId] = true;
    }

    function setAddresses(address _descriptorAddress) public onlyOwner {
    ...
    {
        return
            ITestTestTestDescriptor(ANONYMICE_BREEDING_DESCRIPTOR_ADDRESS)
                .tokenURI(_tokenId);
    }
...
interface ITestTestTestDescriptor is IERC721Enumerable {
