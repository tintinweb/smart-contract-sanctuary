...
import "../../token/ERC1155/ERC1155Standard.sol";
import "./interfaces/IMasterERC1155.sol";
import "../../access/ContractOwner.sol";

contract MasterERC1155Standard is ContractOwnable, IMasterERC1155 {
  ERC1155Standard[] private childERC1155Standard;
  ...
  function createERC1155(
  ...
  ) external override contractOwner(msg.sender) {
    ERC1155Standard child = new ERC1155Standard(
      name,
      symbol,
      tx.origin,
      amount,
    ...
    childERC1155Standard.push(child);
    emit ChildCreatedERC1155(address(child), name, symbol);
  }
}
...
contract ERC1155Standard is ERC1155, Ownable {
  ...
  function mintToken(
    ...
    require(owner() == tx.origin, "ERC1155Mint: caller is not owner");
  ...
  function burn(
    ...
    require(owner() == tx.origin, "ERC1155Burn: caller is not owner");
    require(
      account == tx.origin || isApprovedForAll(account, tx.origin),
  ...
  function burnBatch(
    ...
    require(owner() == tx.origin, "ERC11155Burn: caller is not owner");
    require(
      account == tx.origin || isApprovedForAll(account, tx.origin),
      "ERC1155: caller is not owner nor approved"
    );
...
interface IMasterERC1155 {
  ...
  event ChildCreatedERC1155(address childAddress, string name, string symbol);
}
