...
contract FeelineClub is ERC721Enumerable, Ownable {
    ...
    // User Data
    mapping(uint256 => uint256) private _userVals1;
    mapping(uint256 => uint256) private _userVals2;

    // If true, allows userVal1 and userVal2 functionality
    ...
    // Event called when userVal1 is changed
    event ChangeUserVal1(address caller, uint256 from, uint256 to);

    // Event called when userVal2 is changed
    event ChangeUserVal2(address caller, uint256 from, uint256 to);
    ...
    constructor() ERC721("FeelineClub", "FLC") {
        // Initial functionality for userVal1 and userVal2 will be disabled
            ...
            return
                string(
                    abi.encodePacked(
                        ...
                        feelChars,
                        "-",
                        uint2str(_userVals1[_tokenId]),
                        "-",
                        uint2str(_userVals2[_tokenId])
                    )
                );
        }
    }
    ...
    function mintTo(address _to) public onlyOwner {
        ...
        _userVals1[newTokenId] = 0;
        _userVals2[newTokenId] = 0;
    }
    ...
    function setFeel(
        ...
        if (_allowUserValues) {
            uint256 prevUserVal1 = _userVals1[_tokenId];
            uint256 prevUserVal2 = _userVals2[_tokenId];

            _userVals1[_tokenId] = _userVal1;
            _userVals2[_tokenId] = _userVal2;

            emit ChangeUserVal1(msg.sender, prevUserVal1, _userVal1);
            emit ChangeUserVal2(msg.sender, prevUserVal2, _userVal2);
        }
    }
    ...
    /**
     * @dev Gets the userVal1 value of a token
     */
    function getUserVal1(uint256 _tokenId) public view returns (uint256) {
        require(_allowUserValues == true);
        return _userVals1[_tokenId];
    }

    /**
     * @dev Gets the userVal2 value of a token
     */
    function getUserVal2(uint256 _tokenId) public view returns (uint256) {
        require(_allowUserValues == true);
        return _userVals2[_tokenId];
    }

    /**
     * @dev Allows userVal1 and userVal2 functionality. This action cannot be undone
     */
    function startAllowUserValues() public onlyOwner {
        require(_allowUserValues == false);
        _allowUserValues = true;
        emit AllowUserValues();
    }

    /**
     * @dev Returns true if userVal1 and userVal2 values are allowed to be stored
     */
