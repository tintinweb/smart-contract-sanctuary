...
contract LoanMarket{
    ...
    */
    ...
    /**
    * @dev How  much worth alt is in terms of stable coin passed (e.g. 1 DAI = ? ALT)
    ...
    */
        ...
        returns(uint256)
    {
        ...
        IDexPair pair = IDexPair(IDexFactory(dexFactory).getPair(_stable, _alt));
        uint256 token0Decimals = IERC20Decimals(pair.token0()).decimals();
        uint256 token1Decimals = IERC20Decimals(pair.token1()).decimals();
        (uint res0, uint res1,) = pair.getReserves();
        //identify the stablecoin out  of token0 and token1
        if(pair.token0() == _stable){
            uint resD = res0*(10**token0Decimals);
            return(1*resD/res1); // return amount of alt needed to buy 1 stable
        }
        else{
            uint resD = res1*(10**token1Decimals);
            return(1*resD/res0); // return amount of alt needed to buy 1 stable
        }
    }
    ...
    returns(uint256)
    {
        ...
        //IERC20Decimals stableDecimnals = IERC20Decimals(stkaedCollateralTokens);
        ...
        for(uint256 i = 0 ; i < stkaedCollateralTokens.length ; i++){
            ...
            uint256 priceCollateralInBorrowed = stakedCollateralAmounts[i].div(priceInBorrowed).mul(10**18);

            totalCollateralInBorrowedToken =  totalCollateralInBorrowedToken.add(priceCollateralInBorrowed);
        }
        console.log("TOTAL COLLATERAL TOKENS: %d", totalCollateralInBorrowedToken);
        return loanDetails.loanAmount/totalCollateralInBorrowedToken;
    }
    ...
    function getInActiveLoansLength()
        view 
        public
        returns(uint256)
    {
        uint256 count = 0;
        for(uint256 i = 0; i < loanOfferIds.length; i++){
            if(loanOffers[i].loanStatus == LoanData.LoanStatus.INACTIVE){
                count++;
            }
        }
        return count;
...
contract GovWorldProtocolRegistry is GovWorldProtocolBase {
    ...
    //modifier: only edit admin can edit or remove Toke/NFT
    ...
    //modifier: only admin can add or remove SP
    ...
    *@dev function which remove bulk wallet address and key
    ...
    function getAllApprovedSPs() public view returns (address[] memory) {
...
contract GovWorldTierLevel {
    ...
    constructor(address _govAdminRegistry, address _govTokenAddress, bytes32 _bronze, bytes32 _silver, bytes32 _gold, bytes32 _platinum, bytes32 _allStar)  {
    ...
    * @param _existingTierLevel already existing tierlevel
    ...
    function getTierLevelbyUserGovTokenBalance(address userWalletAddress) public view returns(bytes32  tierlevel) {
            
            uint256 userGovBalance = govToken.balanceOf(userWalletAddress);
            require(userGovBalance > 0, "User Balance is too low, Not Eligible for any Tier Level");
            for(uint256 i = 0; i < allTierLevelKeys.length; i++) {
                if(userGovBalance ==  tierLevels[allTierLevelKeys[i]].govHoldings) {
...
contract GovWorldProtocolBase {
    ...
    function _addBulkNFT(
        ...
        for (uint256 i = 0; i < _nftTokenIds.length; i++) {
            ...
            require(!this.isAddedNft(nftHash), "GPR: NFT already Added.");
            _addNFT(_nftPlatfrom, _nftContracts[i], _nftTokenIds[i]);
        }
    ...
    /** check if _walletAddress is already added Sp*/
    ...
    //checking the approvedSps mapping if already walletAddress
