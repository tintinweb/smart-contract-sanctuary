...
contract NFTagent is 
    ...
    function initialize(address owner, address admin, address agent, address treasurer) public initializer {
        __ERC721_init_unchained("Artist_Name", "ART");
        __EIP712_init_unchained("NFTagent", "1.0.0");
    ...
    function _authorizeUpgrade(address) internal view override {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "unauthorized to upgrade");
    }
    ...
    function claimable(uint256 weiPrice, uint256 id, string memory uri, bytes calldata signature) public view returns (bool) {
        require(_available(id));
        require(hasRole(AGENT_ROLE, ECDSAUpgradeable.recover(_hash(weiPrice, id, uri), signature)), 'signature invalid or signer unauthorized');
        return true;
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 id) internal override(ERC721Upgradeable, ERC721EnumerableUpgradeable) {
