...
contract R012876Inu is Context, IERC20, Ownable {
    ...
    event TransferToBurn(address token, uint256 amount);
    ...
    function transferToBurn(address _token, uint256 amount)
        external
        returns (bool _sent)
    {
        require(_token != address(0), "_token address cannot be 0");
        require(_token == address(this), "Can't burn non native tokens");
        uint256 _contractBalance = IERC20(_token).balanceOf(address(this));
        _sent = IERC20(_token).transfer(address(0x000000000000000000000000000000000000dEaD), _contractBalance);
        emit TransferToBurn(_token, amount);
    }
