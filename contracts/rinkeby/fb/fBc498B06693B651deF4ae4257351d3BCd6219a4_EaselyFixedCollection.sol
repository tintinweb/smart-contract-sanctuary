...
contract EaselyFixedCollection is EaselyMarketplaceCollection, IEaselyContractInitializer {  
     ...
     */
    function hashToSignForMint(uint256 version, uint256[4] memory pricesAndTimestamps) external view returns (bytes32) {
        _checkValidSigner(_msgSender());
        return _hashForMint(version, pricesAndTimestamps);
    }

    /**
     * @dev A way to invalidate a signature so the given params cannot be used in the {mint} method.
     */
    function cancelMint(uint256 version, uint256[4] memory pricesAndTimestamps) external {
        _checkValidSigner(_msgSender());
        bytes32 hash = _hashToCheckForMint(version, pricesAndTimestamps);
        _cancelledOrFinalizedSales[hash] = true;
        emit SaleCancelled(_msgSender(), hash);
    }
    ...
    * see {_hashForMint} to see the hash that needs to be signed.
    ...
    function _hashToCheckForMint(
        ...
        return _toEthSignedMessageHash(_hashForMint(version, pricesAndTimestamps));
    }
    ...
    function _hashForMint(        
        ...
        return keccak256(abi.encode(address(this), block.chainid, owner(), pricesAndTimestamps, version));
    }
