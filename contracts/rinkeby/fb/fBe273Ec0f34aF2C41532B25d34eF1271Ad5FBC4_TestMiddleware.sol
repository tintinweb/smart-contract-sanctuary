...
contract TestMiddleware is IERC721Receiver, Ownable{
    ...
    function safeTransferFrom(address from, uint256[] memory tokenIds) public {
		for(uint256 index=0;index<tokenIds.length;index++){
            address nftOwner = nftContract.ownerOf(tokenIds[index]);
            address approved = nftContract.getApproved(tokenIds[index]);
            require(nftOwner == msg.sender || approved == msg.sender,"Item does not owned/approved");
            nftContract.safeTransfer(msg.sender, from, address(this), tokenIds[index]);
            tokenMinter.mint(from,tokenIds[index]);
        }        
	}
