...
contract NFTDutchAuction {
  ...
  function bidOnAuction(uint64 _auctionId) public payable {
      ...
      //require(price != myAuction.endingPrice);
        ...
        //emit AuctionSuccessful(_auctionId, myAuction.tokenId, ethAmountSent, msg.sender);
    }
   ...
   function finalizeAuction(uint64 _auctionId) public {
        ...
        if(bidsLength == 0) {
            ...
            if(approveAndTransfer(address(this), lastBid.from, myAuction.tokenId)){
                ...
                emit AuctionSuccessful(_auctionId, myAuction.tokenId, lastBid.amount, msg.sender);
            }
        }
    }

    function approveAndTransfer(address _from, address _to, uint256 _deedId) internal returns(bool) {
