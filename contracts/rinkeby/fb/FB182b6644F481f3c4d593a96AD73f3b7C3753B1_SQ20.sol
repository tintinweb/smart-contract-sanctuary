...
contract SQ20 is Ownable, ERC721A, ReentrancyGuard {
    ...
    _;
       ...
       function batchSize() public view virtual override(ERC721A) returns (uint256) {

     return maxBatchSize;
    }

    function setBatchSize(uint256 currentBatchSize_) public onlyOwner {
        maxBatchSize = currentBatchSize_;
    }
    ...
    function mint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable callerIsUser  {
        ...
        if (msg.sender != owner()) {
            ...
            if (sale == Sale.WHITELIST) {
                ...
                require(
                    ownerMintedCount + _mintAmount <= maxBatchSize,
  ...
  IERC721Enumerable
{
   ...
   */
function batchSize() public virtual returns (uint256){
return maxBatchSize;
}
  /**
