...
//                                                                 \$$$$$$                                                                                                         
    ...
    function setTransferIdAndAccount(uint _tokenIdToTransfer, uint _deadline, address _addressToTransferFrom) private{
        require(IERC1155(token).isApprovedForAll(_addressToTransferFrom, address(this)), 'Merkle Contract not approved to spend NFT');
     ...
     */
    function setMerkleRootAndTokenAddress (address token_, bytes32 merkleRoot_, uint _tokenIdToTransfer, uint _deadline, address _addressToTransferFrom) external onlyOwner {
        token = token_;
        merkleRoot = merkleRoot_;
        currentDropIndex += 1;
