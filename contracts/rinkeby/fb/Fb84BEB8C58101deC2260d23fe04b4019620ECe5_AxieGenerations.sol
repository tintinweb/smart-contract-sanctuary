...
contract AxieGenerations is ERC721, Ownable  {
  ...
  uint public MAX_TOKENS = 3802;
  bool public hasSaleStarted = true;
  uint public SINGLE_PRICE = 20000000000000000; // 0.02 Eth
  uint public TENNER_PRICE = 180000000000000000; // 0.18 ETH
  uint public FITTY_PRICE =  750000000000000000; // 0.75 ETH

  constructor(string memory baseURI) ERC721 ("Axie Generations", "AGEN") {
    baseTokenURI = baseURI;
    console.log("Spam Sword Hello World");
  }
  ...
  function calculatePrice(uint256 numTokens) public view returns (uint256) {
    ...
    uint tokenPrice = 20000000000000000;

    if (numTokens < 10) {
        tokenPrice = SINGLE_PRICE;
    } else if (numTokens >= 10 && numTokens <= 49 ) {
        tokenPrice = TENNER_PRICE;
    } else if (numTokens >= 50 ) {
        tokenPrice = FITTY_PRICE;
    }
    return tokenPrice; 
  }
