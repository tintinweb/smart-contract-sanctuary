...
contract TestContract is ERC721URIStorage, Ownable {
    ...
    constructor(string memory baseURI) ERC721("Test6", "TST6") {
        setBaseURI(baseURI);
        totalSupply = 0;
    }

    modifier saleIsOpen {
        require(totalSupply < MAX_MINT, "Sale has ended");
        _;
    }
    
    event ReceiveEvent(string eventName);
    
    fallback () external payable{
        emit ReceiveEvent("fallback()");
    }
    
    receive() external payable {
        emit ReceiveEvent("receive()");
    }
    
    function mintThing(address _to, uint256 _count) public payable saleIsOpen {
        if (msg.sender != owner()) {
            require(!paused, "Sale is paused");
        }
        require(totalSupply+ _count <= MAX_MINT, "Not enough left to mint");
        require(totalSupply< MAX_MINT, "Sale has ended");
        require(_count <= 20, "Cannot mint more than 20");
        require(msg.value >= price(_count), "Incorrect value assigned");
    ...
    function price(uint256 _count) public view returns (uint256) {
        ...
        return 30000000000000000 * _count; // 0.03 ETH
    }
