...
contract Manager is Ownable, IERC721Receiver {
    ...
    uint public constant DAILY_GOLD_RATE = 10 ether; // 10 000 ether should be
    uint public constant MINIMUM_TIME_TO_EXIT = 120; // 120 seconds // 2 days;
    ...
    function _claimFromMiner(uint16 tokenId, bool unstake) internal returns (uint owed) {
        ...
        if (totalGoldEarned < MAXIMUM_GLOBAL_GOLD) {
            owed = ((block.timestamp - stake.value) * DAILY_GOLD_RATE) / 60; //   / 1 days
        } else if (stake.value > lastClaimTimestamp) {
            owed = 0; // $GOLD production stopped already
        } else {
            owed = ((lastClaimTimestamp - stake.value) * DAILY_GOLD_RATE) / 60; //    / 1 days // stop earning additional $GOLD if it's all been earned
        }
        if (unstake) {
    ...
    function _payTax(uint _amount) internal {
        if (totalPirateStaked == 0) {
            ...
            unaccountedRewards += _amount; // keep track of $GOLD due to wolves
            return;
        }
    ...
    modifier _updateEarnings() {
        if (totalGoldEarned < MAXIMUM_GLOBAL_GOLD) {
            totalGoldEarned += ((block.timestamp - lastClaimTimestamp) * totalGoldMinerStaked * DAILY_GOLD_RATE) / 60;//    / 1 days;
