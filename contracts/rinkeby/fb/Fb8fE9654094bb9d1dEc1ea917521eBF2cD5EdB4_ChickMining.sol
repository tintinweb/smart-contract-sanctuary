...
contract ChickMining is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor(address _mining_token, uint256 _mining_start_block) public {
        ...
        uint256 blockCountPerDay = 28800; // 3s one block, one day 28800 block.
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant onlyWhitelisted {
        require(_amount > uint256(1888).mul(busdBaseUnit), "single account investment not more than 1888 busd！");
        require(_pid < poolInfo.length, "invalid _pid");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
