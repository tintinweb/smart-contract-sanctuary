...
contract CodeMakesArt is ERC721Enumerable, Ownable {
    ...
    function claimFree(uint256 dropId, uint256 count) public {
        Drop storage drop = drops[dropId];
        require(
            drop.state == State.Presale || drop.state == State.Active,
            'Sale is not active'
        );
    ...
    function renderTokenHtml(uint256 tokenId)
        public
        view
        returns (string memory)
    {
        ...
        require(
            bytes(drops[dropId].script).length != 0 || drops[dropId].lib > 0,
            'Not available'
        );
        ...
        string memory dependency = drops[dropId].lib > 0
            ? string(
                abi.encodePacked(
                    ...
                    libIpfsHashes[drops[dropId].lib],
                    '></script>'
                )
            )
            : '';
    ...
    function _splitReward(uint256 dropId) internal {
        ...
        uint256 price = drops[dropId].price;
        ...
        if (artist != address(0)) {
            artistReward = (price / 100) * royalty;
            if (artistReward > 0) {
                artist.transfer(artistReward);
            }
        }

        uint256 rest = price - artistReward;
        if (rest > 0) {
