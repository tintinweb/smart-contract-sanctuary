...
import "./FancyNftHeroesS1Sale.sol";
...
contract FancyNftHeroesS1 is
    FancyNftHeroesS1Sale,
    TrustedContacts,
    ...
    IFancyNftHeroesS1
{
    ...
    address public immutable investorsPool;

    // IvestorsPool
    uint256 private _investorsRateTotal;
    mapping(address => uint256) private _investorsRate;
    ...
    // additional farming rating
    mapping(uint256 => uint256) private _tokenIdExtraRate;
    ...
    constructor(
        ...
        address investorsPool_
    )
        ERC721(name_, symbol_)
        FancyNftBase(contractURI_, baseTokenURI_, proxyRegistry_, "hero")
        FancyNftHeroesS1Sale(MAX_SUPPLY)
    {
        distributionPool = distributionPool_;
        investorsPool = investorsPool_;
    }

    function mintAirdrop(uint256 count, uint256 refId) external {
        ...
        _makeMintStats(_msgSender(), count);
        _internalMintBatch(_msgSender(), count);
        _distribute(_msgSender(), 0, refId);
    }

    function mintPresale(uint256 count, uint256 refId) external payable {
        ...
        _makeMintStats(_msgSender(), count);
        _internalMintBatch(_msgSender(), count);
        _distribute(_msgSender(), msg.value, refId);
    }

    function mintPrivsale(uint256 count, uint256 refId) external payable {
        require(isPrivsaleActive(), "Privsale is not active");
        require(privsaleWhitelist[_msgSender()], "Not whitelisted");
        require(!isPrivsaleLimit(_msgSender(), count), "Limits exceeded");
        require(msg.value >= privsaleValue(count), "Wrong value provided");
        _makePrivsaleStats(_msgSender(), count);
        _makeMintStats(_msgSender(), count);
        _internalMintBatch(_msgSender(), count);
        _distribute(_msgSender(), msg.value, refId);
    }

    function mintSale(uint256 count, uint256 refId) external payable {
    ...
    function _distribute(
        ...
        IDistributionPool(distributionPool).deposit{value: value}(
            referral,
            refId
        );
    }
    ...
    // NFT Staking
    function extraRate(uint256 tokenId)
        external
        view
        override
        returns (uint256)
    {
        return _tokenIdExtraRate[tokenId];
    }

    function addExtraRate(uint256 tokenId, uint256 rate)
        external
        override
        onlyTrusted
    {
        _tokenIdExtraRate[tokenId] += rate;
    }

    // IvestorsPool
    function mintsOf(address account) public view override returns (uint256) {
        return _mints[account];
    }

    function investorsRate() public view override returns (uint256) {
        return _investorsRateTotal;
    }

    function investorsRateOf(address account)
        public
        view
        override
        returns (uint256)
    {
        return _investorsRate[account];
    }

    function _getInvestorRate(uint256 count) private pure returns (uint256) {
        if (count >= 200) {
            return 7 ether;
        } else if (count >= 100) {
            return 3 ether;
    ...
    // Mint
    function _makeMintStats(address account, uint256 count) private {
        _mints[account] += count;

        uint256 rateNew = _getInvestorRate(_mints[account]);
        uint256 rateOld = _investorsRate[account];
        if (rateOld < rateNew) {
            uint256 rateDelta = rateNew - rateOld;
            _investorsRateTotal += rateDelta;
            _investorsRate[account] = rateNew;
            IInvestorsPool(investorsPool).stakeFrom(account, rateDelta);
        }
    }
    ...
    function _internalMint(address account) private returns (uint256 id) {
        id = _randomIndex();
    ...
    // Allowing system contracts
    function _setIsApproved(address spender, bool status) external onlyOwner {
        __setIsApproved(spender, status);
    }

    function _freezIsApproved() external onlyOwner {
        __freezIsApproved();
    }
}
...
interface INftType {
    function nftType() external view returns (string memory);
}
...
import "@openzeppelin/contracts/access/Ownable.sol";
import "./FancyNftBase.sol";

/// @custom:security-contact [email protected]
abstract contract FancyNftHeroesS1Sale is FancyNftBase, Ownable {
    // Airdrop
    uint256 public airdropStartTime; // TODO
    uint256 public airdropMintsLimit = 1; // TODO
    uint256 public airdropMintsTotalLimit = 10; // TODO
    uint256 public airdropMintsTotal;
    mapping(address => bool) public airdropWhitelist;
    mapping(address => uint256) public airdropMints;

    // Presale
    uint256 public presalePrice = 0.0001 ether; // TODO
    uint256 public presaleStartTime; // TODO
    uint256 public presaleMintsLimit = 1; // TODO
    uint256 public presaleMintsTotalLimit = 10; // TODO
    uint256 public presaleMintsTotal;
    mapping(address => bool) public presaleWhitelist;
    mapping(address => uint256) public presaleMints;

    // Privsale
    uint256 public privsalePrice = 0.0003 ether; // TODO
    uint256 public privsaleStartTime; // TODO
    uint256 public privsaleMintsLimit = 5; // TODO
    uint256 public privsaleMintsTotalLimit = 0; // TODO
    uint256 public privsaleMintsTotal;
    mapping(address => bool) public privsaleWhitelist;
    mapping(address => uint256) public privsaleMints;

    // Sale
    uint256 public salePrice = 0.0003 ether; // TODO
    uint256 public saleStartTime; // TODO
    uint256 public saleMintsLimit = 0; // TODO
    ...
    // Privsale
    function _makePrivsaleStats(address account, uint256 count) internal {
        privsaleMints[account] += count;
        privsaleMintsTotal += count;
    }
    ...
    function isPrivsaleActive() public view returns (bool) {
        return
            _isActive(
                privsaleMintsTotal,
                privsaleMintsTotalLimit,
                privsaleStartTime,
                saleStartTime
            );
    }

    function isPrivsaleLimit(address account, uint256 count)
        public
        view
        returns (bool)
    {
        return
            _greater(privsaleMints[account] + count, privsaleMintsLimit) ||
            _greater(privsaleMintsTotal + count, privsaleMintsTotalLimit) ||
            _greater(totalSupply() + count, _maxSupply);
    }

    function setPrivsaleMintsLimit(uint256 newLimit) external onlyOwner {
        privsaleMintsLimit = newLimit;
    }

    function setPrivsaleMintsTotalLimit(uint256 newLimit) external onlyOwner {
        privsaleMintsTotalLimit = newLimit;
    }

    function setPrivsaleStartTime(uint256 newTime) external onlyOwner {
        privsaleStartTime = newTime;
    }

    function addToPrivsaleWhitelist(address[] calldata accounts)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < accounts.length; i++) {
            privsaleWhitelist[accounts[i]] = true;
        }
    }

    function privsaleValue(uint256 count) public view returns (uint256) {
        return privsalePrice * count;
    }

    function setPrivsalePrice(uint256 newPrice) external onlyOwner {
        privsalePrice = newPrice;
    }
...
/// @custom:security-contact [email protected]
abstract contract FancyNftBase is
    ERC721,
    ERC721Enumerable,
    ERC721Burnable,
    INftType
{
    ...
    string private _nftType;

    // OpenSea
    address private _proxyRegistry;

    bool public _isApprovedFrozen;
    mapping(address => bool) public _isApproved;

    constructor(
        string memory contractURI_,
        string memory baseTokenURI_,
        address proxyRegistry_,
        string memory nftType_
    ) {
    ...
    // Allowing system contracts
    function __setIsApproved(address spender, bool status) internal {
        require(!_isApprovedFrozen, "Is frozen");
        _isApproved[spender] = status;
    }

    function __freezIsApproved() internal {
        _isApprovedFrozen = true;
    }

    // Metadata
    function nftType() public view override returns (string memory) {
        return _nftType;
    }
    ...
    function _setBaseURI(string memory baseTokenURI_) internal {
        _baseTokenURI = baseTokenURI_;
    }
...
abstract contract ERC721Burnable is Context, ERC721, IERC721Burnable {
    ...
    function burnFrom(address from, uint256 tokenId) public override {
        require(
