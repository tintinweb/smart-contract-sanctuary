...
contract EliteForces is ERC721Enumerable, Ownable {
    ...
    event SaleTokenPriceChanged(uint256 newTokenPrice, uint256 newInitTokenPrice);
    event PresaleConfigChanged(address whitelistSigner, uint32 startTime, uint32 endTime);
    event SaleConfigChanged(uint32 startTime, uint32 coldTime, uint32 initMaxCount, uint32 maxCountUnlockTime, uint32 unlockedMaxCount);
    ...
    constructor(uint256 _maxSupply, uint256 _reserveCount) ERC721("CSSs Token", "CSSS") {
        require(_reserveCount <= _maxSupply, "CSSs: reserve count out of range");
    ...
    function reserveTokens(address recipient, uint256 count) external onlyOwner {
        require(recipient != address(0), "CSSs: zero address");

        // Gas optimization
        uint256 _nextTokenId = nextTokenId;

        require(count > 0, "CSSs: invalid count");
        require(_nextTokenId + count <= maxSupply, "CSSs: max supply exceeded");

        require(tokensReserved + count <= reserveCount, "CSSs: max reserve count exceeded");
    ...
    function reserveTokens(address[] memory recipients) external onlyOwner {
        ...
        require(recipients.length > 0, "CSSs: invalid count");
        require(_nextTokenId + recipients.length <= maxSupply, "CSSs: max supply exceeded");

        require(tokensReserved + recipients.length <= reserveCount, "CSSs: max reserve count exceeded");
        tokensReserved += recipients.length;

        for (uint256 ind = 0; ind < recipients.length; ind++) {
            require(recipients[ind] != address(0), "CSSs: zero address");
    ...
    function setSaleTokenPrice(uint256 _saleTokenPrice, uint256 _saleInitTokenPrice) external onlyOwner {
    ...
    function setUpPresale(
        ...
        // Check params
        require(whitelistSigner != address(0), "CSSs: zero address");
        require(_startTime > 0 && _endTime > _startTime, "CSSs: invalid time range");
    ...
    function setUpSale(
        ...
        require(_initMaxCount > 0 && _unlockedMaxCount > 0, "CSSs: zero amount");
        require(
            _startTime > 0 && _coldTime > _startTime && _maxCountUnlockTime > _startTime,
            "CSSs: invalid time range"
        );
        ...
        emit SaleConfigChanged(_startTime, _coldTime, _initMaxCount, _maxCountUnlockTime, _unlockedMaxCount);
    }
    ...
    function mintPresaleTokens(
        ...
        require(_presaleConfig.whitelistSigner != address(0), "CSSs: presale not configured");

        require(treasury != address(0), "CSSs: treasury not set");
        require(presaleTokenPrice > 0, "CSSs: token price not set");
        require(count > 0, "CSSs: invalid count");
        require(block.timestamp >= _presaleConfig.startTime, "CSSs: presale not started");
        require(block.timestamp < _presaleConfig.endTime, "CSSs: presale ended");

        require(_nextTokenId + count <= maxSupply, "CSSs: max supply exceeded");
        require(presaleTokenPrice * count == msg.value, "CSSs: incorrect Ether value");
        ...
        require(
            ...
            "CSSs: invalid signature"
        );

        require(presaleBoughtCounts[msg.sender] + count <= maxCount, "CSSs: presale max count exceeded");
    ...
    function mintTokens(uint256 count) external payable {
        ...
        require(_saleConfig.startTime > 0, "CSSs: sale not configured");

        require(treasury != address(0), "CSSs: treasury not set");
        require(_tokenPrice > 0, "CSSs: token price not set");
        require(count > 0, "CSSs: invalid count");
        require(block.timestamp >= _saleConfig.startTime, "CSSs: sale not started");
        ...
        require(
            ...
            "CSSs: max count per tx exceeded"
        );
        require(_nextTokenId + count <= maxSupply, "CSSs: max supply exceeded");
        require(_etherAmount <= msg.value, "CSSs: unsufficient Ether sent");
    ...
    function tokenPrice() public view returns (uint256) {
        ...
        if (
            ...
            if (block.timestamp < _saleConfig.coldTime) {
                uint256 deltaTokenPrice = saleInitTokenPrice.sub(saleTokenPrice);
    ...
    function burn(uint256 tokenId) external {
        require(isBurnEnabled, "CSSs: burning disabled");
        require(_isApprovedOrOwner(msg.sender, tokenId), "CSSs: burn caller is not owner nor approved");
        _burn(tokenId);
    }
