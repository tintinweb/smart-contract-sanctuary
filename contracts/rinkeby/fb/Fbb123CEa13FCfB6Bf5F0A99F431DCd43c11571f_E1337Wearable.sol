...
contract E1337Wearable is ERC721 {
    ...
    constructor() ERC721("E1337Wearable", "EW") {}
    ...
    function create(
        ...
        emit ConsecutiveTransfer(from, to, address(0), msg.sender);
    }
    ...
    function removeMediaForModelByURI(uint256 _modelId, string memory _uri)
        external
        onlyOwner
    {
        for (uint256 i; i < media[_modelId].length; i++) {
    ...
    function removeTraitForModelByName(uint256 _modelId, string memory _name)
        external
        onlyOwner
    {
        for (uint256 i; i < traits[_modelId].length; i++) {
