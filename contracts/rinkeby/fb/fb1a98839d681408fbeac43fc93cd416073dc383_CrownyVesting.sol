...
contract CrownyVesting is Ownable {
    ...
    function changeVestingAddress(address _from, address _to) external onlyOwner {
    ...
    function returnUnallocatedTokens(address _beneficiary) external onlyOwner {
        ...
        emit ReturnUnallocated(_beneficiary, unallocatedAmount);
    }
    ...
    function claimTokens(uint256 _amount) external {
        ...
        require (_amount <= allowedToClaim(_msgSender()), "CrownyVesting: can't claim more than you deserve");
    ...
    function vestingAmount(address _beneficiary) external view returns (uint256) {
    ...
    function nextJuncture(address _beneficiary) external view returns (uint32) {
