...
contract CyBlocGeneScientist {
    ...
    uint256 public bitClass = 7;
    uint256 public bitTrait = 14;
    ...
    function decode(uint256 _genes) public view returns(uint256 class, uint256 trait) {
        ...
        class = _getBits(_genes, 0, bitClass);
        trait = encode(traits, bitTrait);
        return (class, trait);
    }  

    function _getBits(uint256 _input, uint256 _slot,uint256 _numOfBit) public view returns(uint8) {
    ...
    function encode(uint8[] memory _traits, uint256 _numOfBit) public view returns (uint256 _genes) {
        ...
        for(uint256 i = 0; i < numOfTraits; i++) {
            _genes = _genes << _numOfBit;
            // bitwise OR trait with _genes
            _genes = _genes | _traits[(_numOfBit - 1) - i];
        }
        return _genes;
    }    
}
