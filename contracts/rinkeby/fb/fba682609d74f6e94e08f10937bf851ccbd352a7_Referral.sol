...
/// @title sYSL/YSL Referral contract
/// @author Simranpal Dhillon, Hardev Dhillon
/// @author Blaize team: Artem Martiukhin & Pavlo Horbonos
/// @notice The contract keeps the information about referrals and referrers
contract Referral is Ownable, AccessControl {
    ...
    function adminAddReferal(address sender, address creator) external operatorOnly {
    ...
    function proccessReferral(address sender, address creator, bytes memory signature) external operatorOnly {
        if (hasReferral(sender)) {
            return;
        }
        bytes32 message = formReferral(creator, sender);
        require(creator == recoverAddress(message, signature), "Invalid signature provided");
        referrals[sender] = creator;
    }
    ...
    function hasReferral(address sender) public view returns(bool){
        return referrals[sender] != address(0);
    }

    /*****
     * INTENRAL ECDSA SIGNATURE HELPERS
     *****/ 
