...
contract Traits is Ownable, ITraits {
  ...
  function drawSVG(uint256 tokenId) public view returns (string memory) {
    ...
    uint8 shift = s.isBee ? 0 : 9;

    string memory svgString = string(abi.encodePacked(
      drawTrait(traitData[0 + shift][s.background]),
      drawTrait(traitData[1 + shift][s.body]),
      s.isBee ? '' : drawTrait(traitData[2 + shift][s.ears]),
      drawTrait(traitData[3 + shift][s.eyes]),
      drawTrait(traitData[4 + shift][s.face]),
      drawTrait(traitData[5 + shift][s.head]),
      drawTrait(traitData[6 + shift][s.mouth]),
      drawTrait(traitData[7 + shift][s.wingNeck])
    ));
  ...
  function compileAttributes(uint256 tokenId) public view returns (string memory) {
    ...
    if (s.isBee) {
      traits = string(abi.encodePacked(
        ...
        attributeForTypeAndValue(_traitTypes[1], traitData[1][s.body].name),',',
        attributeForTypeAndValue(_traitTypes[3], traitData[3][s.eyes].name),',',
        attributeForTypeAndValue(_traitTypes[4], traitData[4][s.face].name),',',
        ...
        attributeForTypeAndValue(_traitTypes[7], traitData[7][s.wingNeck].name),','
      ));
    } else {
      traits = string(abi.encodePacked(
        attributeForTypeAndValue(_traitTypes[0], traitData[9][s.background].name),',',
        attributeForTypeAndValue(_traitTypes[1], traitData[10][s.body].name),',',
        ...
        attributeForTypeAndValue(_traitTypes[4], traitData[13][s.face].name),',',
        ...
        attributeForTypeAndValue(_traitTypes[7], traitData[16][s.wingNeck].name),',',
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    string memory metadata = string(abi.encodePacked(
      ...
      s.isBee ? 'Bee #' : 'Bear #',
      tokenId.toString(),
      '", "description": "Thousands of Bees and Bears compete on a forest in the metaverse. A tempting prize of $HONEY awaits, with deadly high stakes. All the metadata and images are generated and stored 100% on-chain. No IPFS. NO API. Just the Ethereum blockchain.", "image": "data:image/svg+xml;base64,',
