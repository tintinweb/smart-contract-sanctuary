...
contract DistributorFactory is Operators, IDistributorFactory {
    ...
    function withdrawManyCampaigns(
        ...
        for (uint32 i = 0; i < _campaignIDs.length; i++) {
            if (getWithdrawableAmount(_poolID, _campaignIDs[i], tx.origin) > 0)
    ...
    function getWithdrawableAmountManyCampaigns(
        ...
        for (uint32 i = 0; i < _campaignIDs.length; i++) {
            withdrawable += distributors[_poolID].getWithdrawableAmount(_campaignIDs[i], _user);
        }
        return withdrawable;
    }
    ...
    function getWithdrawedAmountManyCampaigns(
        ...
        for (uint32 i = 0; i < _campaignIDs.length; i++) {
            withdrawed += distributors[_poolID].getWithdrawedAmount(_campaignIDs[i], _user);
        }
        return withdrawed;
    }
...
contract BasicDistributor is BaseDistributor {
    ...
    constructor(ITokenSale _tokenSale) BaseDistributor(_tokenSale) {}
...
abstract contract BaseDistributor is IDistributor, Ownable, ReentrancyGuard {
    ...
    function withdraw(string calldata _campaignID) public virtual {}
