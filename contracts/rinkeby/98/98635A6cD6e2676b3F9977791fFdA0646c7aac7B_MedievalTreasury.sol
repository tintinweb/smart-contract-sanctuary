...
contract MedievalTreasury is MedievalAccessControlled {
    ...
    function enableBlocksNeededforQueue(uint _blocksNeededForQueue) onlyAdmin external {
        require(blocksNeededForQueue == 0, "blocksNeededforQueue has been enabled.");
    ...
    /**
        ...
        @return bool
     */
    function queue( MANAGING _managing, address _address ) external onlyAdmin() returns ( bool ) {
    ...
    /**
        ...
        @return bool
     */
    function toggle( MANAGING _managing, address _address, address _calculator ) external onlyAdmin() returns ( bool ) {
        require( _address != address(0) );
        bool result;
        if ( _managing == MANAGING.RESERVEDEPOSITOR ) { // 0
