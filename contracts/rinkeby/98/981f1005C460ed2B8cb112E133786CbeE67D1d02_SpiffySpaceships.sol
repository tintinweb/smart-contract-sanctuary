...
// TODO - Does this really need to be ERC721Enumerable?
//contract SpiffySpaceships is ERC721Enumerable, Ownable, ReentrancyGuard {
...
//    uint256 private price = 25000000000000000; // 0.025 Ether
    ...
    event tokenNameChange(address by, uint tokenId, string name);
    ...
    // Withdraw balance from contract account
...
//        _tokenIdCounter.increment();
    ...
    function giftTokens(address to, uint256 numTokens) external onlyOwner {        
        require(numTokens < giftsRemaining + 1, "Exceeds remaining gifts.");
...
//        uint256 totalSupply = totalSupply();
        ...
        require(numTokens < MAX_TOKENS_PER_PURCHASE + 1, "Exceeds maximum tokens you can purchase in a single transaction.");
        require(totalSupply + numTokens < MAX_TOKENS + 1, "Exceeds maximum tokens available for purchase.");
        require(msg.value >= price * numTokens, "Ether value sent is not correct");
    ...
    /*
    // TODO - Does this actually work? or do you need to modify the URI JSON
    function nameToken(uint tokenId, string memory name) external {
        require(ownerOf(tokenId) == msg.sender, "Your wallet doesn't own this token!");
        require(sha256(bytes(name)) != sha256(bytes(tokenNames[tokenId])), "New name is same as the current one");
        tokenNames[tokenId] = name;
        
        emit tokenNameChange(msg.sender, tokenId, name);
    }*/
}
