...
contract ProxyProtocol {
    ...
    function createPToken(address underlying_) external returns (uint) {
        doTransferIn(msg.sender, feeRecipient, proxyToken, feeAmountCreatePool);

        return PTokenFactory(pTokenFactory).createPToken(underlying_);
    }
    ...
    function mint(address pToken, uint mintAmount) external returns (bool) {
        ...
        doTransferIn(msg.sender, feeRecipient, proxyToken, fee);
     ...
     */
    function repayBorrow(address pToken, uint repayAmount) external returns (bool) {
        ...
        PErc20Interface(pToken).repayBorrowBehalf(msg.sender, repayAmount);
     ...
     * @param repayAmount The amount to repay
     */
    function repayBorrowBehalf(address pToken, address borrower, uint repayAmount) external returns (bool) {
        uint fee = calcFee(feePercentRepayBorrow, pToken, repayAmount);
        doTransferIn(msg.sender, feeRecipient, proxyToken, fee);
    ...
    function repayBorrow() external payable {
        ...
        uint borrows = getETHBorrows(msg.sender);

        if (received > borrows) {
            fee = calcFee(feePercentRepayBorrow, pETH, borrows);

            msg.sender.transfer(received - borrows);
            Maximillion(maximillion).repayBehalf{value: borrows}(msg.sender);
        } else {
            fee = calcFee(feePercentRepayBorrow, pETH, received);
            Maximillion(maximillion).repayBehalf{value: received}(msg.sender);
        }
    ...
    function calcFee(uint feePercent, address pToken, uint amount) public view returns (uint) {
        ...
        uint proxyTokenPriceInUSD = oracle.getPriceInUSD(proxyToken);
        uint power = EIP20Interface(proxyToken).decimals();

        return calcFeeAmount(power, feeInUSD.div(proxyTokenPriceInUSD));
    }
