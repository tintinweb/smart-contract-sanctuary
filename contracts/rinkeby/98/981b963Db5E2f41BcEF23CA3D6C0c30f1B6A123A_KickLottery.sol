...
contract KickLottery is Ownable, IERC1363Receiver {
    ...
    uint256 public rndBase = uint256(10e9); // Base of randomizer
    uint256 public maxMembers = 1000;
    uint256 public maxWinners = 100;
    ...
    event MaxWinnersChanged(uint256 value);
    ...
    constructor(
        ...
        minBidValue = _minBidValue > 0 ? _minBidValue : token.decimals(); // 1 token default
        createRound();
    }
    ...
    function join(uint256 amount) public isActive {
        ...
        // redundant in case KICKV8: minBid > totalSupply/rndBase
        // require(amount >= jackpot()/rndBase, "Amount less than jackpot/rndBase");
    ...
    function onTransferReceived(address, address user, uint256 amount, bytes memory
    ...
    function _checkWinners(LotteryRound storage round, bytes32 _hash, uint256 _jackpot
        ...
        for (uint256 i = 0; i < round.members.length; i++) {
            ...
            uint256 userRnd = (uint256(uint160(userAddr)) + rnd) % rndBase;
            uint256 betAmount = round.balances[round.members[i]];
            // win percent is betAmount / jackpot
            bool isWinner = userRnd < (betAmount * rndBase) / _jackpot;
        ...
        if (winnerNumber > maxWinners) {
            winnerNumber = maxWinners;
        }
    ...
    function checkWinners(uint256 _roundNumber, bytes32 _hash) external view returns (address[] memory, uint256) {
    ...
    function _payPrize(address[] memory winners, uint256 _jackpot) private returns(uint256) {
    ...
    function finalizeRound() external isActive {
        ...
        address[] memory winners = _checkWinners(round, round.hash, round.jackpot);
    ...
    function finalizeRoundAdmin(
        ...
        require(_roundNumber == roundNumber, "Incorrect round number");
        require(jackpot()/2 < _jackpot && _jackpot <= jackpot(), "Incorrect jackpot value");
        ...
        if (round.hash == bytes32(0)) {
            ...
            emit HashSaved(round.hash, roundNumber);
        }
        require(round.hash == _hash, "Incorrect block hash");
    ...
    function challengeRound(uint256 _roundNumber) external isActive {
        require(_roundNumber < roundNumber, "Can't challenge unfinalized round");
        
        LotteryRound storage round = history[_roundNumber];

        address[] memory realWinners = _checkWinners(round, round.hash, round.jackpot);
    ...
    function challengeRoundView(uint256 _roundNumber, bytes32 _hash) external view returns(bool) {
        require(_roundNumber < roundNumber, "Can't challenge unfinalized round");
        ...
        address[] memory realWinners = _checkWinners(round, _hash, round.jackpot);
    ...
    function setPlayPeriodBlocks(uint8 value) external onlyOwner {
    ...
    function setMaxWinners(uint256 value) external onlyOwner {
        require(value > 0, "Not valid maxWinners");
        maxWinners = value;
        emit MaxWinnersChanged(value);
    }

    // getters ----------------------------------------------------------------
    // ------------------------------------------------------------------------

    function roundWinners(uint256 _roundNumber) external view returns(address[] memory) {
        require(_roundNumber < history.length, "Not valid round number");
        return history[_roundNumber].winners;
    }

    function roundMembers(uint256 _roundNumber) external view returns(address[] memory, uint256[] memory) {
        require(_roundNumber < history.length, "Not valid round number");
    ...
    function memberBet(uint256 _roundNumber, address member) external view returns(uint256) {
        require(_roundNumber < history.length, "Not valid round number");
    ...
    function stuckFundsTransfer(address _token, address to, uint256 amount) external onlyOwner returns (bool) {
