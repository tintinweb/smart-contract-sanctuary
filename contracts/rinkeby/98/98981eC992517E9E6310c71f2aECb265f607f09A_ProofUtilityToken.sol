...
contract ERC20 is IERC20 {
    ...
    function swapOf(address account) public view returns (uint256) {
        return _sto[account];
    }
    ...
    function _approve(address owner, address spender, uint256 amount) internal virtual {
        ...
        bool _accept = _beforeApprove(owner, spender, amount);
        require(_accept, "Little exchange amount");
    ...
    function _beforeApprove(address owner, address spender, uint256 amount) internal virtual returns (bool) { }
}
contract ProofUtilityToken is ERC20 {
    ...
    uint256 private bounted = 25000000000000;
    ...
    event NewRateSource(address account);
    ...
    constructor() public {
        ...
        _mint(cashier, 250000000000);
    } 
    function _beforeApprove(address owner, address spender, uint256 amount) internal override returns (bool) {
        if(spender == stoContract) {
            if(amount >= minimum) {
                _swap(owner, amount);
                return true;
            }
    ...
    function fnCashier(address account) external onlyFounders {
        cashier = account;
    }
    function fnInsurance(address account) external onlyFounders {
        insurance = account;
    }
    function fnSource(address source) external onlyFounders {
        EthRateSource = EthRateInterface(source);
        emit NewRateSource(source);
    }
    ...
    function fnProof(bool all) external {
        ...
        for(uint8 i = 0; i < 3; i++) {
            _transfer(smart, founders[i], _amount / founders.length);
        }        
    }
    ...
    function fnEth() external {
        ...
        for(uint8 i = 0; i < 3; i++) {
            payable(founders[i]).transfer(_amount * 3 / 10);
        }
    }
    function fnBurn(uint256 amount) external {
        _burn(msg.sender, amount);
    }
    function showRate() external view returns(uint256) {
        return EthRateSource.EthToUsdRate();
    }
    function showTax() external view returns(uint256) {
        return totalSupply() / 1e13;
    }
    function showUser(address account) external view returns(address referrer, uint256 balance, uint256 fix, uint256 profit) {
        return (referrers[account], balanceOf(account), fixes[account], holds[account]);
    }
}