...
interface IWastelandCactusCrew {
    function ownerOf(uint256 tokenId) external view returns (address);
    function transferFrom(address from, address to, uint256 tokenId) external;
    function isApprovedForAll(address owner, address operator) external view returns (bool);
}

contract Wasteland is ERC20, Ownable, ReentrancyGuard {

    IWastelandCactusCrew public WastelandCactusCrew;
    ...
    {
        WastelandCactusCrew = IWastelandCactusCrew(wastelandCactusCrew);
    }
    ...
    function stake(uint256[] calldata tokenIds) external isApprovedForAll {
    ...
    function unstake(uint256 tokenId, bool _claim) external nonReentrant {
        require(owners[tokenId] == msg.sender, "Caller is not token owner");

        if (_claim) claim(msg.sender);
    ...
    function unstakeMany(uint256[] calldata tokenIds, bool _claim) external nonReentrant {
        for (uint256 i = 0; i < tokenIds.length; i++) {
    ...
    function unstakeAll() external nonReentrant {
        ...
        require(ownedTokens.length > 0, "No tokens staked");
