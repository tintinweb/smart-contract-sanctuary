...
contract MyToken is ERC721, Ownable {
    ...
    uint256 public gift_buffer = 100;
    ...
    constructor(address _signer, uint256 _max_presale_mint, string memory _baseURI) ERC721("MyToken", "MTK") {
        signer = _signer;
        maxPresaleMint = _max_presale_mint;
        baseURI = _baseURI;
    }
    ...
    function gift(uint256 giftAmount, address to) public onlyOwner {
        require(currentState != ContractState.PAUSED);
        require(giftAmount > 0, "Mint != 0");
        require(giftAmount + _tokenIdCounter.current() <= MAX_SUPPLY, "Gift > Max");
    ...
    // Requires ensure that minting is 1-5 and that user is whitelisted. Does not allow to mint beyond the gift buffer or whitelisted allocation.
    function presaleMint1(uint256 mintAmount, bytes memory _signature) public payable {
        ...
        require(mintAmount + _tokenIdCounter.current() <= MAX_SUPPLY - giftsSent.current(), "Minting more than max supply");
    ...
    function presaleMint2(uint256 mintAmount, bytes memory _signature) public payable {
