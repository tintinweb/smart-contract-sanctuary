...
contract Bytes2 is Ownable, ERC20("BYTES", "BYTES") {
    using SafeMath for uint256;
    ...
    constructor() {
        identityContract = IIdentity(
            0x86357A19E5537A8Fba9A004E555713BC943a66C0
        );
        vaultBoxContract = IVaultBox(
            0xab0b0dD7e4EaB0F9e31a539074a03f1C1Be80879
        );
        maxRewardableCitizens = 2501;
    }

    function openVaultBox(uint256 identityTokenId, uint256 vaultBoxTokenId)
        public
    {
        require(
            identityBoxOpened[identityTokenId] == 0,
            "That identity has already opened a box"
        );
        require(
            vaultBoxOpenedByIdentity[vaultBoxTokenId] == 0,
            "That box has already been opened"
        );
        require(
            validateIdentity(identityTokenId),
            "You don't own that identity"
        );
        require(
            vaultBoxContract.ownerOf(vaultBoxTokenId) == msg.sender,
            "You don't own that vault box"
        );

        uint256 credits;
        bool valueReturned;
        (credits, valueReturned) = strToUint(
            vaultBoxContract.getCredits(vaultBoxTokenId)
        );

        uint256 payout = credits * 10**18;
    ...
    function updateReward(
        ...
        if (_tokenId < maxRewardableCitizens) {
            ...
            uint256 rateDelta = citizenContract.getRewardsRateForTokenId(
                _tokenId
            );

            if (timerFrom > 0) {
                rewards[_from] += citizenContract
                    .getRewardRate(_from)
    ...
    function burn(address _from, uint256 _amount) external {
        require(
            adminContracts[msg.sender],
            "You are not approved to burn tokens"
        );
        _burn(_from, _amount);
    }
    ...
    function getTotalClaimable(address _user) external view returns (uint256) {
        ...
        uint256 pending = citizenContract
            .getRewardRate(_user)
