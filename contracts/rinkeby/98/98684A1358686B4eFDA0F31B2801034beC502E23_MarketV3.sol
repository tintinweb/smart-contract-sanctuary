...
//		order.retailFee = _retailFee;
	...
	function burnVersion(
		...
		if (isERC721) {
			IERC721(_tokenAddress).safeTransferFrom(msg.sender, _toDead, _tokenId);
		} else {
		...
		nftVersionOnSale[_tokenAddress][_tokenId][_version] = false;
		nftVersion[_tokenAddress][_tokenId][_version] = _toDead;
	}
