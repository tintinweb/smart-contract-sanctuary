...
// File: contracts/Coin98Stake.sol
pragma solidity ^0.8.0;
...
contract Coin98Stake is ERC721Enumerable, Ownable {
    ...
    uint256 public maxStaked = 1000000000 ether;
    
    uint256 public Percent = 10000;
    uint256 yearTimestamp = 31104000;
    
    // Prefix first for C98 Ref ID
    string ref_id = 'C98';
    // Prefix first for NFT ID
    string nft_prefix = '9898';
     // The Fee charged when changing the Staking name
    uint256 public naming_fee = 1 ether;
     //The Fee charged when user wants to customize NFT ID
    uint256 public id_fee = 1 ether;
    
     // The Lock time after tokens are deposited
    uint256 public locked_time;
    // Floating rate
    uint256 public floating_rate;
    // C98 Token address
    IERC20 public C98Token ;
    ...
    struct StakeInfo {
        ...
        string packgeId; // Package ID
        
        bool isCustomID; // Flag for checking the custom ID
        uint256 packageTime; // Package time
        uint256 rate; // Package rate
        
        uint256 unstakedTime; // Unstaked Time
        uint256 earnStaked; // Earn Staked
        uint256 id; // NFT ID
        uint256 estStaked; // Estimated Staked
    }
    ...
    constructor(string memory baseURI,address C98Address) ERC721("Coin98 Staking Certificate", "C98SC"){
        C98Token = IERC20(C98Address);
        setBaseURI(baseURI);
        locked_time = 1296000;
        floating_rate = 200;
    }
        ...
        onlyOwner()
    {
       ...
       naming_fee = _naming_fee;
       id_fee = _id_fee;
       locked_time = _lockedTime;
       floating_rate = _floatingRate;
    }
    ...
    function validPackageCondition(uint256 _min, uint256 _max, uint256 _time, uint256 _rate) internal view{
        require(_min>0 && _max >0 && _min < _max , "Wrong numeric format");
        require(_time > locked_time && _rate > floating_rate,"Lower than minimum time & rate");
    }
    ...
    function configurePackage(
        ...
        onlyOwner()
    {
        require(validPackage(_package), "Package not found");
        validPackageCondition(_min,_max,_time,_rate);
        
        PackageInfos[_package].min = _min;
        PackageInfos[_package].max = _max;
        PackageInfos[_package].time = _time;
        PackageInfos[_package].rate = _rate;
    }
    ...
    function register(
        ...
        onlyOwner()
    {
        require(!validPackage(_package), "Package already existed");
        validPackageCondition(_min,_max,_time,_rate);
    ...
    */
    ...
    function getStakedByTokenId(uint256 _tokenId) private view returns (uint256) {
        ...
        if(!stakeInfo.flag || (timeStaked < locked_time)){
            return 0;
        } else {
            uint256 calRate = timeStaked < stakeInfo.packageTime ? floating_rate: stakeInfo.rate;
    ...
    function stake(uint256 _amount, string memory _name, string memory _package,uint256 _customID) public {
        ...
        require(_amount >= pkInfo.min && _amount < pkInfo.max , "Wrong min max format");
    
        uint256 totalStakedFinal = totalStaked.add(_amount);
        require(totalStakedFinal <= maxStaked, "Maximum number of staked");
        ...
        bool _isNotCustomname = Convertible.compareStrings(Convertible.sliceString(1,3,_name), ref_id) && nameSize == 10;
        
        if(!_isNotCustomname){
            require(nameSize <= 20 && nameSize > 0,"Not meet name condition");
        }
        
        uint256 totalStakedAmount =  _amount.add(_isCustomID ? id_fee : 0 ).add(_isNotCustomname ? 0 : naming_fee);
        
        require(C98Token.transferFrom(msg.sender, address(this), totalStakedAmount));
        ...
        if (_isCustomID){
            ...
            string memory randomConvert = Convertible.uint2str(uint256(keccak256(abi.encodePacked(total.add(1),_amount,block.timestamp,nft_prefix))));
        ...
        uint256 nftPackageId = Convertible.bytesToUInt(Convertible.stringToBytes32(string(abi.encodePacked(nft_prefix,randomID))));
        ...
        stakeInfo.packgeId = _package;
        
        
        stakeInfo.name = _name;
        stakeInfo.isCustomID = _isCustomID;
        
        stakeInfo.packageTime = pkInfo.time;
        stakeInfo.rate = pkInfo.rate;
        
        totalStaked = totalStakedFinal;
    ...
    function renaming (uint256 _tokenId,string memory _name) public {
        ...
        require(ownerOf(_tokenId) == msg.sender, "Not meet owner condition");
        require(C98Token.transferFrom(msg.sender, address(this), naming_fee));
        uint256 nameSize = bytes(_name).length;
        require(nameSize <= 20 && nameSize > 0,"Not meet name condition");
    ...
    function unstake(uint256 _tokenId) public {
        ...
        C98Token.safeTransfer(msg.sender, _profitTotal);
        
        stakeInfo.unstakedTime = block.timestamp;
        stakeInfo.earnStaked  = _profit;
     ...
     * - `amount` must be larger than zero.
     */
    function withdraw(uint256 _amount, IERC20 _token) public onlyOwner {
        require(_amount > 0);
    ...
    function walletOfOwner(address _owner) external view returns (StakeInfo[] memory) {
        ...
        for (uint256 i = 0; i < tokenCount; i++) {
            ...
            stakeInfoIndex.estStaked = getStakedByTokenId(tokenIndex);
