...
contract TransportMinter is Ownable, ContextMixin {
    ...
    function redeemTransport(uint256 tokenId, bytes32[] memory proof) public {
        ...
        require(
            proof.verify(merkleRoot,keccak256(abi.encodePacked(msg.sender))),"TransportMinter: Not Allocated");
    ...
    function batchRedeemTransports(uint256[] calldata tokenIds, bytes32[] memory proof) public 
    {
        require(merkleRoot != 0, "TransportMinter: no MerkleRoot yet");
        uint256 tokenId;        
        address minter = msg.sender;

        require(proof.verify(merkleRoot,keccak256(abi.encodePacked(minter))),"TransportMinter: Not Allocated");
