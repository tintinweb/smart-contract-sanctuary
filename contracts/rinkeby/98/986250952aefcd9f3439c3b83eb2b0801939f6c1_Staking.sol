...
contract SSGTGovernance is ERC721URIStorage, Ownable{
    ...
    mapping(address => User) userToken;
    ...
    function mintToken(address _toAddress, string memory _uri) public {
        require(msg.sender == minter, "You can't mint a token");
    ...
    function removeTokenIdFromUser(uint256 _tokenId) public {
        ...
        for(uint i = 0; i < userToken[ownerOf(_tokenId)].tokenId.length; i++){
            if(_tokenId == userToken[ownerOf(_tokenId)].tokenId[i]){
                for (uint j = userToken[ownerOf(_tokenId)].tokenId[i]; j < userToken[ownerOf(_tokenId)].tokenId.length - 1; j++) {
...
contract Staking is Ownable, IERC721Receiver{
    ...
    SSGTGovernance private nftToken;
    ...
    // unstaking possible after - value in seconds
    ...
    mapping (address => uint) public totalNFTTokens;


    // @update Initialize NFT contract
    
    //constructor params: "0x2a881131c3f8f825e74757eb5792fa12a162d878","0xF7150924fd7EC3dBaD80D7A415c13fB06e7e3759","7200","10800","150","50","120","25000000000000000000"
    constructor(address _tokenAddress, address _nftContractAddress, uint _rewardRate, uint _rewardInterval, uint _stakingFeeRate, uint _unstakingFeeRate, uint _cliffTime, uint _tokenAmount) {
        nftToken = SSGTGovernance(_nftContractAddress);
     ...
     */
    function returnsewardRate() public view returns(uint){
        return rewardRate;
    }
     ...
     */
    function RewardInterval() public view returns(uint){
     ...
     */
    function StakingFeeRate() public view returns(uint){
     ...
     */
    function UnstakingFeeRate() public view returns(uint){
     ...
     */
    function CliffTime() public view returns(uint){
        return cliffTime;
    }
     ...
     */
    function SSGTTokenAddress() public view returns(address){
        return tokenAddress;
    }
    
    function SSGTGovernanceTokenAddress() public view returns(SSGTGovernance){
        return nftToken;
    }
    
     /**
     * @notice minSSGTForGovernance
     * @return tokenAmount at which amount NFT will mint
     */
    function minSSGTForGovernance() public view returns(uint){
        return tokenAmount;
    }
     ...
     */
    function changeStakingFeeRate(uint _rate) public onlyOwner{
     ...
     */
    function changeUnstakingFeeRate(uint _rate) public onlyOwner{
     ...
     */
    function changeMinSSGTForGovernance(uint _tokenAmount) public onlyOwner{
        tokenAmount = _tokenAmount;
    }
     ...
     */
    function depositedTokensOfUser() public view returns(uint){
     ...
     */
    function totalNFTTokensOfUser() public view returns(uint){
        return totalNFTTokens[msg.sender];
    }
    ...
    function mintNFTToken(uint amountToStake) internal {
        ...
        if(userBalance > tokenAmount){
            uint amountOfToken = userBalance.div(tokenAmount);
            for(uint i = 0; i < amountOfToken; i++){
                nftToken.mintToken(msg.sender, "uri goes here");
            }
            depositedTokens[msg.sender] = userBalance.sub(amountOfToken.mul(tokenAmount));
            totalNFTTokens[msg.sender] = totalNFTTokens[msg.sender].add(amountOfToken.mul(tokenAmount));
        }else{
    ...
    function withdrawWithNFT(uint _tokenId) public {
        ...
        uint fee = tokenAmount.mul(unstakingFeeRate).div(1e4);
        uint amountAfterFee = tokenAmount.sub(fee);

        require(IERC20(tokenAddress).transfer(owner, fee), "Could not transfer withdraw fee.");
        require(IERC20(tokenAddress).transfer(msg.sender, amountAfterFee), "Could not transfer tokens.");
        totalNFTTokens[msg.sender] = totalNFTTokens[msg.sender].sub(tokenAmount);
    }
    ...
    function burnNFTOfContract(uint[] memory _tokenId) public {
        nftToken.burnInBulk(_tokenId);
    }
