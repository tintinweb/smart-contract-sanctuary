...
/*
FEG 4m vested over 2 years to a wallet we create ourselves and then hand over the key to FEG (no setter in the contract).
4m develeoper compensation vested linearly to 4 predefined wallets (one for each of us, no setter) over 2 years with first
vesting after 6 months. 6m staking reward pool reserve for v2. Will be burnt (V2 will simply mint it again). 6m for development/marketing
purposes and CEX listing funds. 100k daily release limit to an arbitrary wallet. Community needs to be informed about every transaction from this pool.
*/
contract DogeCity is Ownable {
    TokenVesting fegPartnershipVestingContract;
    TokenVesting teamRewardVestingContract;
    IERC20 dogira;
    ...
    constructor(
        ...
        _createVestingContracts(teamRewardAddress, fegPartnershipAddress);
    }
    ...
    function getTeamRewardVestingContract() public view returns (address) {
        return address(teamRewardVestingContract);
    }
    ...
    function _createVestingContracts(address teamRewardAddress, address fegPartnershipAddress) private {
        ...
        fegPartnershipVestingContract = new TokenVesting(
    ...
    function initialize(address fundingWallet) external onlyOwner {
        ...
        dogira.transferFrom(fundingWallet, address(teamRewardVestingContract), 4_000_000 ether);
