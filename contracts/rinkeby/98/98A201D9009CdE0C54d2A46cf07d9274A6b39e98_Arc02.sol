...
contract Arc02 is ERC721, AccessControl, Ownable {
    using Strings for uint256;

    mapping(address => uint256) private _mintedList;
    mapping(uint256 => bool) private _displayFullMode;

    string private _tokenOriginalBaseURI;
    string private _tokenDisplayBaseURI;
    address private _derivateContractList;
    
    uint256 public MAX_PER_ADDRESS = 2;
    uint256 public MAX_PUBLIC      = 2790;
    uint256 public MAX_RESERVED    = 210;
    uint256 public STARTING_RESERVED_ID;
    
    uint256 public totalReservedSupply = 0;
    uint256 public totalPublicSupply = 0;
    uint256 public temporaryPublicMax = 1000;
    ...
    constructor(uint256 maxPublic, uint256 maxReserved, uint256 startingReservedID, uint256 maxPerAddress, address[] memory whitelistAddresses) ERC721("Arc02", "ARC02") {
    ...
    function setTemporaryPublicMax(uint256 _temporaryPublicMax)
        public
    ...
    function derivateContractList() public view returns (address) {
        return _derivateContractList;
    }
    
    function setDerivateContractList(address newDerivateContractList)
        public
        onlyRole(OPERATOR_ROLE)
    {
        _derivateContractList = newDerivateContractList;
    }
    
    function _baseURI() internal override view returns (string memory) {
        return _tokenOriginalBaseURI;
    }

    function _baseDisplayURI() internal view returns (string memory) {
        return _tokenDisplayBaseURI;
    }
    ...
    function setDisplayBaseURI(string memory baseURI) public onlyRole(OPERATOR_ROLE) {
        ...
        private
    {
        ...
        require(tokenIds.length == modes.length);
        ...
        for(uint256 i = 0; i < tokenIds.length; i++) {
            ...
            _displayFullMode[tokenId] = modes[i];
    ...
    function tokenDisplayFullMode(uint256 tokenId) public view returns (bool)
    {
    ...
    function displayTokenURI(uint256 tokenId)
        ...
        returns (string memory)
    {
        string memory baseURI = _baseDisplayURI();
