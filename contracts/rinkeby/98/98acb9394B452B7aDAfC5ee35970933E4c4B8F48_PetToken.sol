...
contract PetToken is RawToken {
    ...
    function getTotalTokens() public override returns (uint256){
        return tokensToSale;
    }

    function incrementAmountToSell(uint256 totalTokens) public override {
        require(hasRole(MINTER_ROLE, _msgSender()), "PetToken: must have minter role to change total tokens");
        tokensToSale += totalTokens;
    }

    function decrementAmountToSell(uint256 totalTokens) public override {
        require(hasRole(MINTER_ROLE, _msgSender()), "PetToken: must have minter role to change total tokens");
        tokensToSale -= totalTokens;
    }
}
...
ERC721Burnable {
    ...
    function mintToken(address to, uint256 base_price, uint256 deposit_price, uint256 benefits_period,
    ...
    function getTotalTokens() public virtual returns (uint256);

    function incrementAmountToSell(uint256 totalTokens) public virtual;

    function decrementAmountToSell(uint256 totalTokens) public virtual;
