...
contract bhsbhdfTest01 is ERC721Enumerable, Ownable {
	using SafeMath for uint256;
	uint public constant maxTokenSupply = 25440;
	uint private startPrice;
	uint private capPrice;
	uint private maxQty;
	bool public mintingAllowed;

	constructor() ERC721("bhsbhdfTest01", "bhsbhdfTest01") { }
	...
	function getPresalePrice() public view returns(uint256) {
		require(mintingAllowed == true, "Minting not allowed yet.");
		require(totalSupply() < maxTokenSupply, "All tokens have been minted.");
		return (startPrice + ((totalSupply().div(100)) * ((capPrice - startPrice) / (maxTokenSupply / 100)))) * (10 ** 16);
	}
