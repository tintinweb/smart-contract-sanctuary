...
contract ThePixelPortraits {
    ...
    function rescindCommission (uint _commissionIndex, uint _beatenByCommissionIndex) 
    public
    callNotStarted
    {
        require(_commissionIndex < newCommissionIndex, "commission not valid"); // must be a valid previously instantiated commission
        ...
        if (_beatenByCommissionIndex > 0){
          require(_beatenByCommissionIndex < newCommissionIndex, "beaten by commission not valid"); // must be a valid previously instantiated commission
          Commission storage beatenByCommission = commissions[_beatenByCommissionIndex];
          require(beatenByCommission.status == CommissionStatus.queued, "commission not in queue");
          require(beatenByCommission.bid > selectedCommission.bid, "not beaten by commission"); // must be a valid previously instantiated commission
        } else {
          require(names[toLower(selectedCommission.name)] != 0, "original name not taken"); // the original
        }
