...
import "./managers/auction/ProjectAuctionManager.sol";
import "./managers/auction/RankAuction.sol";


contract AggregateRead {



    constructor() {}

    function aggreateGetTokenURI(address project, uint256[] calldata tokenIds) external view returns (string[] memory){
        string[] memory tokenURIs = new string[](tokenIds.length);
        for (uint256 i = 0; i < tokenIds.length; i++) {
            tokenURIs[i] = ERC721ProjectUpgradeable(project).tokenURI(tokenIds[i]);
        }
        return tokenURIs;
    }

    function aggregateGetProjectAuctions(address projectAuction, uint256[] calldata auctionIds) external view returns (ProjectAuctionManager.Auction[] memory){
        ProjectAuctionManager.Auction [] memory auctions = new ProjectAuctionManager.Auction[](auctionIds.length);
        for (uint256 i = 0; i < auctionIds.length; i++) {
            auctions[i] = ProjectAuctionManager(projectAuction).getAuction(auctionIds[i]);
        }
        return auctions;
    }

    function aggregateGetRankAuctions(address rankAuction, uint256[] calldata auctionIds) external view returns (RankAuction.Auction[] memory){
        RankAuction.Auction [] memory auctions = new RankAuction.Auction[](auctionIds.length);
        for (uint256 i = 0; i < auctionIds.length; i++) {
            auctions[i] = RankAuction(rankAuction).getAuction(auctionIds[i]);
        }
        return auctions;
    }
    ...
    UUPSUpgradeable
{
    ...
    event AuctionTokenURIUpdated(uint256 indexed auctionId, uint256 tokenId, string tokenURI);
    ...
    function placeBid(
        ...
        if (auction.bidder == address(0)) {
            ...
            require(
                msg.value >= auction.amount && msg.value > 0,
     ...
     */
    function setShouldVerifySignatureForBid(bool value) external onlyOwner {
     ...
     */
    function createAuction(Auction calldata auction) external onlyValidAuctionConfig(auction) onlyOwner {
    ...
    /**
     * @notice admin update auction token URI, emergency condition only
     */
    function emergencyUpdateAuctionTokenURI(uint256 auctionId, string calldata tokenURI) external onlyOwner {
        Auction storage auction = auctions[auctionId];
        auction.tokenURI = tokenURI;
        uint256 tokenId = auctionIdToTokenId[auctionId];
        if (auction.finalized && tokenId > 0) {
            auction.project.managerSetTokenURI(tokenId, tokenURI);
        }
        emit AuctionTokenURIUpdated(auctionId, tokenId, tokenURI);
    }

    /**
     * @notice If an auction has been created but has not yet received bids, it may be canceled by the owner.
     */
    function cancelAuction(uint256 auctionId) external nonReentrant onlyOwner {
        Auction storage auction = auctions[auctionId];
        require(bytes(auction.tokenURI).length > 0, "Auction not found");
     ...
     */
    function adminCancelAuction(uint256 auctionId, string calldata reason) external onlyOwner {
    ...
    UUPSUpgradeable
{
    ...
    //maxBidNumber：Number of current bidding items
    //amount：the lowest price that can be bid,Assignment at initialization，The amount are not modified afterwards
    struct Auction {
        uint256 startTime;
        uint256 endTime;
        uint256 extDurationMinutes;
    ...
    event AuctionCreated(uint256 indexed auctionId, Auction auction);
    event AuctionUpdated(uint256 indexed auctionId, Auction auction);
    event AuctionTokenURIUpdated(uint256 indexed auctionId, string[] tokenURI);
    event AuctionCanceled(uint256 indexed auctionId);
    event AuctionBidPlaced(uint256 indexed auctionId, address indexed bidder, uint256 amount, uint256 endTime);
    event AuctionOutBid(address indexed previousBidder, uint256 previousAmount);
    event AuctionFinalized(
    ...
    modifier onlyValidAuctionConfig(Auction calldata auction) {
        require(auction.bidderInfos.length == 0, "bad bidder");
        require(!auction.finalized, "finalized should be false");
        require(auction.tokenURI.length > 0, "bad tokenURI");
        require(address(auction.project).isContract(), "bad project address");
        require(auction.endTime > block.timestamp, "endTime should > now");
        require(auction.endTime - auction.startTime <= MAX_MAX_DURATION, "Duration must be <= 1000 days");
        require(
        ...
        require(auction.maxBidNumber > 0, "bad maxBidNumber");
        require(auction.maxBidNumber == auction.tokenURI.length, "bad tokenURI length");
        _;
    }
    ...
    /**
     *   Returns the minimum amount a bidder must spend to participate in an auction.
     */
    function getMinBidAmount(uint256 auctionId) public view returns (uint256) {
        Auction storage auction = auctions[auctionId];
        uint256 bidderInfosLength = auction.bidderInfos.length;
        if (bidderInfosLength < auction.maxBidNumber) {
            if (auction.amount == 0) {
                // The next bid must be at least 1 wei greater if initial amount is 0;
                return auction.amount + 1;
            } else {
        ...
        return
            _getMinBidAmountForAuction(
                auction.bidderInfos[bidderInfosLength - 1].amount,
                auction.minPercentIncrementInBasisPoints
            );
    }
    ...
    function placeBid(
        ...
        require(auction.tokenURI.length > 0, "RankAuction: Auction not found");
        require(auction.startTime <= block.timestamp, "RankAuction: Auction is not started yet");
        require(auction.endTime >= block.timestamp, "RankAuction: Auction is over");

        if (shouldVerifySignatureForBid) {
            bytes32 messageHash = keccak256(abi.encode(block.chainid, address(this), _msgSender(), auctionId));
            require(_verifySignedMessage(messageHash, v, r, s), "RankAuction: proper signature is required");
        }

        uint256 minAmount = getMinBidAmount(auctionId);

        require(
            msg.value > 0 && msg.value >= minAmount,
            "RankAuction: Bid must be at least the min price and higher than 0"
        );

        if (auction.bidderInfos.length == 0) {
            // it's the first bid
            // if the min price is 0, ensure it's > 0
            // if the min price larger than 0, ensure it's >= min price

            auction.bidderInfos.push(BidderInfo({amount: msg.value, bidder: payable(_msgSender())}));
        } else {
            (uint256 previousRank, uint256 previousAmount) = _getPreviousRankAndAmount(auctionId, _msgSender());
            uint256 currentRank;
            //if bidder already have rank
            if (previousRank > 0) {
                /*
                 *   Users who are already number one are not allowed to bid
                 *   Replace the original bidder's amount
                 *   Bubble sort the previousRank
                 *   Refund to the bidder
                 */
                require(previousRank != 1, "RankAuction: You are already the first one");
                auction.bidderInfos[previousRank - 1].amount = msg.value;
                currentRank = previousRank;
                uint256 targetRank = _sortRank(auctionId, currentRank);
                require(targetRank < currentRank, "RankAuction: Your bid must be higher than the last one");
                _refund(payable(_msgSender()), previousAmount);
            }
            //if bidder don't have rank
            else {
                /*
                 *  Add the bidder to the end of the array
                 *  Bubble sort the last element
                 *  If the array element exceeds the maxBidNumber, pop the last element and refund
                 *
                 */
                auction.bidderInfos.push(BidderInfo({amount: msg.value, bidder: payable(_msgSender())}));
                currentRank = auction.bidderInfos.length;
                _sortRank(auctionId, currentRank);
                uint256 bidderInfosLength = auction.bidderInfos.length;
                if (bidderInfosLength > auction.maxBidNumber) {
                    address payable outBidBidder = auction.bidderInfos[bidderInfosLength - 1].bidder;
                    uint256 outBidAmount = auction.bidderInfos[bidderInfosLength - 1].amount;
                    auction.bidderInfos.pop();
                    _refund(outBidBidder, outBidAmount);
                    emit AuctionOutBid(outBidBidder, outBidAmount);
                }
            }
        }
        emit AuctionBidPlaced(auctionId, _msgSender(), msg.value, auction.endTime);
    ...
    /**
     * different rankings mint different NFT
     */
    function finalizeAuction(uint256 auctionId) external {
        Auction storage auction = auctions[auctionId];
        require(!auction.finalized, "RankAuction: Auction already settled");
        require(auction.tokenURI.length > 0, "RankAuction: Auction not found");
        require(auction.endTime < block.timestamp, "RankAuction: Auction still in progress");
        auction.finalized = true;
        for (uint256 i = 0; i < auction.bidderInfos.length; i++) {
            _sendETHToTreasury(auction.bidderInfos[i].amount);
            uint256 tokenId = auction.project.managerMint(auction.bidderInfos[i].bidder, auction.tokenURI[i]);
            emit AuctionFinalized(
                auctionId,
                auction.bidderInfos[i].bidder,
                auction.bidderInfos[i].amount,
                address(auction.project),
                tokenId
            );
        }
    }
    ...
    function emergencyUpdateAuctionTokenURI(uint256 auctionId, string[] calldata tokenURI) external onlyOwner {
        ...
        for (uint256 i = 0; i < tokenURI.length; i++) {
            auction.tokenURI[i] = tokenURI[i];
        }
        emit AuctionTokenURIUpdated(auctionId, tokenURI);
    }
    ...
    {
        Auction storage auction = auctions[auctionId];
        require(auction.tokenURI.length > 0, "Auction not found");
        require(auction.startTime > block.timestamp, "Auction in progress");
    ...
    function cancelAuction(uint256 auctionId) external onlyOwner {
        Auction memory auction = auctions[auctionId];
        require(auction.tokenURI.length > 0, "Auction not found");
        require(auction.bidderInfos.length == 0, "Auction in progress");
     ...
     * This should only be used for extreme cases such as DMCA takedown requests. The reason should always be provided.
     */
    function adminCancelAuction(uint256 auctionId, string memory reason) external onlyOwner {
        require(bytes(reason).length > 0, "Include a reason for this cancellation");
        Auction memory auction = auctions[auctionId];
        require(auction.tokenURI.length > 0, "Auction not found");
        ...
        if (auction.bidderInfos.length != 0) {
            for (uint256 i = 0; i < auction.bidderInfos.length; i++) {
                _refund(auction.bidderInfos[i].bidder, auction.bidderInfos[i].amount);
            }
        }
    ...
    /**
     * return the previous bidder's ranking and bidding price
     */
    function _getPreviousRankAndAmount(uint256 auctionId, address bidder) internal view returns (uint256, uint256) {
        Auction storage auction = auctions[auctionId];
        for (uint256 i = 0; i < auction.bidderInfos.length; i++) {
            if (auction.bidderInfos[i].bidder == bidder) return (i + 1, auction.bidderInfos[i].amount);
        }
        return (0, 0);
    }

    /**
     * bubble sort the last element forward, return the sorted rank
     */
    function _sortRank(uint256 auctionId, uint256 currentRank) internal returns (uint256) {
        uint256 targetRank = currentRank;
        Auction storage auction = auctions[auctionId];
        address payable bidder = auction.bidderInfos[currentRank - 1].bidder;
        uint256 amount = auction.bidderInfos[currentRank - 1].amount;
        for (uint256 i = currentRank - 1; i > 0; i--) {
            if (
                amount >
                _getMinBidAmountForAuction(auction.bidderInfos[i - 1].amount, auction.minPercentIncrementInBasisPoints)
            ) {
                auction.bidderInfos[i].bidder = auction.bidderInfos[i - 1].bidder;
                auction.bidderInfos[i].amount = auction.bidderInfos[i - 1].amount;
                auction.bidderInfos[i - 1].bidder = bidder;
                auction.bidderInfos[i - 1].amount = amount;
                targetRank = i;
            }
        }
        return targetRank;
