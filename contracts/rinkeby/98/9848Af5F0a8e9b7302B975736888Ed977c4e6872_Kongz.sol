...
contract YieldToken is ERC20("Banana", "BANANA") {
	...
	uint256 constant public DURATION = 1931622407;

	mapping(address => uint256) public rewards;
	mapping(address => uint256) public lastUpdate;

	address public constant kongzContract = address(0x9848Af5F0a8e9b7302B975736888Ed977c4e6872);
	...
	function updateRewardOnMint(address _user, uint256 _amount, uint256 _userBalance) external {
		require(msg.sender == kongzContract);
		uint256 time = min(block.timestamp, DURATION);
		if (lastUpdate[_user] > 0)
			rewards[_user] += _userBalance.mul(BASE_RATE.mul((time.sub(lastUpdate[_user])))).div(86400)
	...
	// called on transfers
	function updateReward(address _from, address _to, uint256 _fromBalance, uint256 _toBalance, uint256 _tokenId) external {
		require(msg.sender == kongzContract);
		if (_tokenId < 1001) {
			uint256 time = min(block.timestamp, DURATION);
			if (lastUpdate[_from] > 0)
				rewards[_from] += _fromBalance.mul(BASE_RATE.mul((time.sub(lastUpdate[_from])))).div(86400);
			lastUpdate[_from] = time;
			if (_to != address(0)) {
				if (lastUpdate[_to] > 0)
					rewards[_to] += _toBalance.mul(BASE_RATE.mul((time.sub(lastUpdate[_to])))).div(86400);
				lastUpdate[_to] = time;
			}
		}
	}
...
contract ERC721Namable is ERC721 {
	...
	uint256 public nameChangePrice = 3 ether;
	uint256 constant public BIO_CHANGE_PRICE = 1 ether;
	...
	function changeName(uint256 tokenId, string memory newName) public virtual {
		...
		// yieldToken.burn(msg.sender, NAME_CHANGE_PRICE);
...
contract Kongz is ERC721Namable, Ownable {
	...
	address constant public SIGNER = 0x0000000000000000000000000000000000000000;
	uint256 constant public BREED_PRICE = 6 ether;


	mapping(uint256 => Kong) kongz;
	uint256 public bebeCount;

	YieldToken public constant yieldToken = YieldToken(0x80DDd8616969700031b23f03df9680E776e444E8);
	IBreedManager breedManager;

	// Events
	event KongIncubated (uint256 tokenId);
	event KongBorn(uint256 tokenId, uint256 genes);
	event KongAscended(uint256 tokenId);

	constructor(string memory _name, string memory _symbol) public ERC721Namable(_name, _symbol) {
		...
		emit KongIncubated(1001);
		emit KongIncubated(1002);
		emit KongIncubated(1003);
		bebeCount = 3;
	}
	...
	function returnCorrectId(uint256 _id) public pure returns(uint256) {
	...
	// do we need signer if OG have 0 genes?
	function ascend(uint256 _tokenId, uint256 _genes, bytes calldata _sig) external {
		...
		//require(recover(keccak256(abi.encodePacked(id, _genes)), _sig) == SIGNER);
	
		yieldToken.updateRewardOnMint(msg.sender, 1, balanceOf(msg.sender));
		kongz[id] = Kong(_genes, block.timestamp);
		_mint(msg.sender, id);
		// could be sent to 0xdead too
		// or they keep it and we preevnt more redeeming
		//OPENSEA_STORE.safeTransferFrom(msg.sender, burn, _tokenId, 1, "");
		emit KongAscended(id);
	}

	function breed(uint256 _sire, uint256 _matron) external {
		require(ownerOf(_sire) == msg.sender && ownerOf(_matron) == msg.sender);
		require(_sire < 1001 && _matron < 1001 && _sire != _matron);
	...
	function getReward() external {
		yieldToken.updateReward(msg.sender, address(0), balanceOf(msg.sender), 0, 0);
		yieldToken.getReward(msg.sender);
	}

	function transferFrom(address from, address to, uint256 tokenId) public override{
		yieldToken.updateReward(from, to, balanceOf(from), balanceOf(to), tokenId);
	...
	function recover(bytes32 _msgHash, bytes memory _sig) internal pure returns (address) {
		bytes32 r;
		bytes32 s;
		uint8 v;
