...
*/
  ...
  function initialize(
    ...
    address newpOLYToken_,
    address newBondingCalculator_,
    address newPrincipleDepository_
...
/**
  ...
  function incurDebt( uint principieTokenAmountDeposited_, uint bondScalingValue_ ) external override onlyPayment() returns ( bool ) {
    getDebtAmountDue = getDebtAmountDue.add(
        IBondingCalculator( getBondingCalculator )
          .calculateBondInterest( address(this), getPrincipleToken, principieTokenAmountDeposited_, bondScalingValue_ )
      );
    return true;
  }
...
*/
 ...
 function ownerDepositReserveToken( uint amount_ ) external onlyOwner() returns ( bool ) {
    IERC20( getReserveToken ).safeTransferFrom( msg.sender, address(this), amount_ );
    address managedToken_ = getManagedToken;
    IERC20Mintable( managedToken_ ).mint( daoWallet, amount_.div( 10 ** IERC20( managedToken_ ).decimals() ) );
    return true;
  }

  function ownerDepositPrinciple( uint depositAmount_ ) external onlyOwner() returns ( bool ) {
    address principleToken = getPrincipleToken;
    IERC20( principleToken ).safeTransferFrom( msg.sender, address(this), depositAmount_ );
    IERC20Mintable( getManagedToken ).mint( stakingContract, IBondingCalculator( getBondingCalculator ).principleValuation( principleToken, depositAmount_ ).div( 1e9 ) );
    return true;
  }
  ...
  function redeempOLY( uint amountToRedeem_ ) external returns ( bool ) {
    ...
    require( holderPOLYVestingTerm.amountOfpOLYHasBeenRedeemed < holderPOLYVestingTerm.maxpOLYCanRedeem, "Already redeemed all available" );
    require( redeemableAmount >= int( amountToRedeem_ ), "trying to redeem more than redeemable" );
    ...
    address managedToken_ = getManagedToken;
    IERC20Mintable( managedToken_ ).mint( msg.sender, amountToRedeem_.div( 10 ** IERC20( managedToken_ ).decimals() ) );
    return true;
  }
