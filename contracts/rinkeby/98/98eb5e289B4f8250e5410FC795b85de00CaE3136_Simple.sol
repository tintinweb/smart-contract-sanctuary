...
contract Simple is Ownable, VORConsumerBase {
    using SafeMath for uint256;

    event Request(bytes32 requestId, address sender, uint256 range);
    event Result(bytes32 requestId, uint256 rand);

    mapping(bytes32 => uint256) public requestIdToRange;

    constructor(address _vorCoordinator, address _xfund)
    public
    VORConsumerBase(_vorCoordinator, _xfund) {}

    function request(uint256 _range, uint256 _seed, bytes32 _keyHash, uint256 _fee) external returns (bytes32 requestId) {
        require(_range > 1, "range cannot be < 1");
        xFUND.transferFrom(msg.sender, address(this), _fee);
        requestId = requestRandomness(_keyHash, _fee, _seed);
        emit Request(requestId, msg.sender, _range);
        requestIdToRange[requestId] = _range;
    }

    function fulfillRandomness(bytes32 _requestId, uint256 _randomness) internal override {
        uint256 range = requestIdToRange[_requestId];
        uint256 rand = _randomness.mod(range);

        emit Result(_requestId, rand);

        // clean up
        delete requestIdToRange[_requestId];
    }
