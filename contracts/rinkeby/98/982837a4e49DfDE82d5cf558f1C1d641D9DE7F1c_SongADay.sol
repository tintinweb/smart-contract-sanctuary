...
contract SongADay is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    bytes32 private redeemMerkleRoot;
    ...
    function dailyMint(uint256 _tokenId, string memory _ipfsMetadataHash) external onlyOwner {
        uint256 rand = uint256(keccak256(abi.encodePacked(_ipfsMetadataHash))) + block.timestamp;

        if (rand % 2 == 0 && previousTreasuryMint + frequency <= block.timestamp) {
    ...
    function batchMint(uint256[] calldata _tokenIds, address[] calldata _owners) external onlyOwner {
        for (uint256 i; i < _tokenIds.length; i++) {
            string memory tokenURI = string(abi.encodePacked(ipfsMetadataHash, "/", _tokenIds[i].toString()));
    ...
    /**
     * @notice Public batch redeems tokens.
     ...
     * Requirements:
     *
     * - caller must be owner of `ticketId`.
     * - merkle proof must be valid.
     */
    function _redeem(uint256 _ticketId, uint256 _tokenId, bytes32[] calldata _proof, address _owner) private {
        ...
        require(MerkleProof.verify(_proof, redeemMerkleRoot, leaf), "Redeem: Invalid merkle proof");
    ...
    function _claim(uint256 _openStoreId, uint256 _tokenId, bytes32[] calldata _proof, address _owner) private {
        ...
        require(MerkleProof.verify(_proof, claimMerkleRoot, leaf), "Claim: Invalid merkle proof");
    ...
    /**
     * @notice Sets hash of merkle roots for public claim & redeem.
     */
    function setMerkleRoot(bytes32 _claimRoot, bytes32 _redeemRoot) external onlyOwner {
        require(claimMerkleRoot == bytes32(0) && redeemMerkleRoot == bytes32(0), "Merkle root already set");

        claimMerkleRoot = _claimRoot;
        redeemMerkleRoot = _redeemRoot;
    }
     ...
     */
    function setTicketContract(ISADTicket _contractAddress) external onlyOwner {
        sadTicketContract = _contractAddress;
    }
    ...
    /**
     * @notice Repairs metadata for specified token.
     */
    function repairMetadata(uint256 _tokenId, string memory _tokenURI) external onlyOwner {
        _setTokenURI(_tokenId, _tokenURI);
    }
...
// @title Interface for SADTicket (TICKET) contract.
