...
contract Orcs is ERC721 {
    ...
    function setZug(address nn) external {
    ...
    function claim(uint256 id) public noCheaters {
        ...
        if(block.timestamp <= activities[id].timestamp) return;
    ...
    function doSumthin(uint256 id) public ownerOfOrc(id) noCheaters {
        ...
        orcs[id].body = orcs[id].helm = orcs[id].mainhand = orcs[id].offhand = item;
    }
    ...
    function _mintOrc(uint256 rand) internal returns (uint16 id) {
        ...
        uint8  helm     = uint8(randHelm < nineFivePct ? 0 : randHelm % 4 + 5);

        uint16 randOffhand = uint16(_randomize(rand, "OFFHAND"));
        uint8  offhand     = uint8(randOffhand < nineFivePct ? 0 : randOffhand % 4 + 5);

        uint16 randMainhand = uint16(_randomize(rand, "MAINHAND"));
        uint8  mainhand     = uint8(randMainhand > nineEightPct ? randMainhand % 4 + 9 :
