...
contract Pseudopop is ERC721, ERC721Enumerable, Ownable {
    ...
    function popCaps(uint256 _amount) public  {
        bottleCaps.burn(msg.sender, _amount);
    }
    ...
    function getSVG(Soda memory soda) public view returns (string memory) {
...
contract BottleCaps is ERC20, ERC20Burnable {
    ...
    uint256 public constant INITIAL_ISSUANCE = 200 ether;
    ...
    function updateRewardOnMint(address to, uint256 num) public {
        ...
        uint256 timerUser = lastUpdate[to];
        if (timerUser > 0) {
            rewards[to] = rewards[to].add(
                pseudoPopContract
                    .capCount(to)
            ...
            rewards[to] = rewards[to].add(num.mul(INITIAL_ISSUANCE));
        }
        lastUpdate[to] = time;
        // _mint(to, 300 ether * num);
    }
