...
contract LockToken is Context {
    ...
    function getLockList() external view returns (LockInfo[] memory) {
        LockInfo[] memory lockList = new LockInfo[](_lockerAddresses.size());

        for (uint256 i = 0; i < _lockerAddresses.size(); i++) {
            address lockerAddress = _lockerAddresses.at(i);
            lockList[i] = _locks[lockerAddress];
        }
