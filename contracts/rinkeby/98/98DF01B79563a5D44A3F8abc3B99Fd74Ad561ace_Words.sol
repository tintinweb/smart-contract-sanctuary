...
contract Words {
    ...
    function getFiveSyllableLine(uint256 tokenId, uint256 lineNum) external view returns (string memory) {
        ...
        uint256 rand = random(string(abi.encodePacked(block.difficulty, block.timestamp, msg.sender, tokenId.toString()))) % TOTAL_ADJS;
        ...
        return string(abi.encodePacked(line[0], ' ', line[1]));
    }

    function getSevenSyllableLine(uint256 tokenId, uint256 lineNum) external view returns (string memory) {
        string[7] memory line;
        string memory baseSeed = string(abi.encodePacked(block.difficulty, block.timestamp, msg.sender, tokenId.toString()));
