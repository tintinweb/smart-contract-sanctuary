...
contract PortalHunter is PortalHunterERC721 {
    ...
    modifier hunterOwner(uint256 hunterId) {
        require(ownerOf(hunterId) == msg.sender, "Cannot interact with a Hunter you do not own");
        _;
    }

    IFCore public FCore;

    uint256 constant public NAME_CHANGE_PRICE = 15 ether;
    uint256 constant public BIO_CHANGE_PRICE = 15 ether;

    mapping(uint256 => HunterData) public hunterData;
    event NameChanged(uint256 hunterId, string hunterName);
    event BioChanged(uint256 hunterId, string hunterBio);

    constructor(string memory name, string memory symbol, uint256 supply, uint256 genCount) PortalHunterERC721(name, symbol, supply, genCount) {}

    function setFusionCore(address FCoreAddress) external onlyOwner {
        FCore = IFCore(FCoreAddress);
    }
    
    function changeName(uint256 hunterId, string memory newName) external hunterOwner(hunterId) {
        bytes memory n = bytes(newName);
        require(n.length > 0 && n.length < 25,                          "Invalid name length");
        require(sha256(n) != sha256(bytes(hunterData[hunterId].name)),    "New name is same as current name");
        
        FCore.burn(msg.sender, NAME_CHANGE_PRICE);
        hunterData[hunterId].name = newName;
        emit NameChanged(hunterId, newName);
    }

    function changeBio(uint256 hunterId, string memory newBio) external hunterOwner(hunterId) {
        FCore.burn(msg.sender, BIO_CHANGE_PRICE);
        hunterData[hunterId].bio = newBio;
        emit BioChanged(hunterId, newBio);
    }
...
contract PortalHunterERC721 is ERC721Enumerable, Ownable {
