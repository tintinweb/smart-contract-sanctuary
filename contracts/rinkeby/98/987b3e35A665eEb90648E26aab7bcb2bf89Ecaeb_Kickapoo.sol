    ...
    ReentrancyGuard
{
    ...
    uint256 constant bottomPrice = 69000000000000000; // 0.069
    uint256 constant maxTokenPerTx = 10;
    uint256 constant maxPresalePerWallet = 10;

    uint256 public discountBlockSize = 30;
    uint256 public discountStartBlock = 0;
    uint256 public maxSupply = 6969;
    uint256 public maxReserve = 169;
    uint256 public presaleCapped = 969;
    uint256 public presalePrice = 69000000000000000; // 0.069
    uint256 public publicSalePrice = 109000000000000000; // 0.109
    uint256 public revealBlock = 0;
    uint256 public seed = 0;
    uint256 public totalWhitelistToken = 0;
    uint256 public totalPresaleMinted = 0;
    uint256 public totalPublicMinted = 0;
    uint256 public totalReserveMinted = 0;

    mapping(uint256 => address) private _whitelistTokens;
    ...
    modifier airdropRoleOnly() {
        require(
            _airdropAllowed[msg.sender] || msg.sender == owner(),
            "Only airdrop role allowed"
        );
        _;
    }
        ...
        returns (bool)
    {
        ...
        if (getSaleMode() == 3) {
            require(
                amount <= maxTokenPerTx,
                "Mint limit per transaction exceeded."
            );
        ...
        if (getSaleMode() == 1) {
            require(
                isTokenOwnerWhitelist() || isWhitelisted(msg.sender),
                "Not whitelisted."
            );
            require(
                amount <= maxTokenPerTx,
            ...
            require(
                _presaleClaimed[msg.sender] + amount <= maxPresalePerWallet,
    ...
    function isTokenOwnerWhitelist() public view returns (bool) {
        if (totalWhitelistToken == 0) return false;
        for (uint256 i = 0; i < totalWhitelistToken; i++) {
            if (hasToken(_whitelistTokens[i])) {
                return true;
            }
        }
        return false;
    ...
    function addTokenWhitelist(address token) external onlyOwner {
        _whitelistTokens[totalWhitelistToken] = token;
        totalWhitelistToken = totalWhitelistToken + 1;
    }

    function hasToken(address token) public view returns (bool) {
        return ERC721(token).balanceOf(msg.sender) > 0;
    }

    function getSaleMode() public view returns (uint256) {
        if (totalSupply() == maxSupply) return 5;
    ...
    function startPublicSaleBlock() public view returns (uint256) {
        return publicSale.beginBlock;
    }

    function endPublicSaleBlock() public view returns (uint256) {
        return publicSale.endBlock;
    }

    function startSaleBlock() public view returns (uint256) {
        return getSaleMode() <= 1 ? presale.beginBlock : publicSale.beginBlock;
    }

    function endSaleBlock() public view returns (uint256) {
        return getSaleMode() <= 2 ? presale.endBlock : publicSale.endBlock;
    }

    function startPresaleBlock() public view returns (uint256) {
        return presale.beginBlock;
    }

    function endPresaleBlock() public view returns (uint256) {
    ...
    function setSeed(uint256 randomNumber) public onlyOwner {
        randomseedRequested = true;
        seed = randomNumber;
    ...
    function price() public view returns (uint256) {
        ...
        if (getSaleMode() == 3) {
            if (discountStartBlock > 0 && block.number > discountStartBlock) {
                uint256 passedBlock = block.number - discountStartBlock;
                uint256 discountTimes = passedBlock.div(discountBlockSize);
                if (discountTimes >= 4) {
                    return bottomPrice;
                } else {
                    return
                        publicSalePrice.sub(
                            discountTimes.mul(10000000000000000) // 0.01
                        );
                }
            } else {
    ...
    function updateDiscountBlockSize(uint256 block_number) external onlyOwner {
        discountBlockSize = block_number;
    }

    function updateDiscountStartBlock(uint256 block_number) external onlyOwner {
        discountStartBlock = block_number;
    }
