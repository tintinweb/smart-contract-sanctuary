...
contract Goat is IGoat, ERC721Enumerable, Ownable, Pausable {
    ...
    // reference to the Barn for choosing random Goat thieves
    IBarn public barn;
    ...
    bool public publicMintSale = true;
    ...
    */
    constructor(address _egg, address _traits, uint256 _maxTokens, address _whitelistSigner) ERC721("Goat Game", 'WGAME') { 
        egg = EGG(_egg);
    ...
    function _mint(uint256 amount, bool stake) internal {
        ...
        if (totalWoolCost > 0) egg.burn(_msgSender(), totalWoolCost);
    ...
    * The first 20% are free to claim, the remaining cost $EGG
    */
    function earlyAccessMint(uint256 _amount, bytes memory signature, bool _stake) external payable whenNotPaused {
    ...
    */
    function generate(uint256 tokenId, uint256 seed) internal returns (GoatTortoise memory t) {
        t = selectTraits(seed);
    ...
    */
    function selectRecipient(uint256 seed) internal view returns (address) {
        ...
        address thief = barn.randomGoatOwner(seed >> 144); // 144 bits reserved for trait selection
    ...
    */
    function selectTraits(uint256 seed) internal view returns (GoatTortoise memory t) {    
        ...
        seed >>= 16;
        t.fur = selectTrait(uint16(seed & 0xFFFF), 0);
        seed >>= 16;
