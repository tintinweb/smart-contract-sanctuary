...
/**
 * @title NervousDogs contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract NervousDogsV8 is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    // Prices
    uint256 public constant price = 10000000000000; //0.00001 ETH

    // Limits
    uint256 public constant MAX_DOGS = 8888;
    uint256 public constant TEAM_RESERVED_DOGS = 100;
    uint256 public constant COLLECTORS_DOGS = 200;
    uint8 public constant TRANSACTION_MINT_LIMIT = 30;
    uint8 public constant COLLECTORS_MINT_LIMIT = 1;
    uint256 private teamSupply = 0;
    uint256 private collectorsSupply = 0;
    ...
    // Team
    address w1 = 0xa13672d0918b00C172340e9b3E8D3C4E94f74a24;
    address w2 = 0x89f5154b2A497BFe72e4EB5973500185c7de43aF;

    // address w3 = 0xCA35b7d915458EF540aDe6068dFe2F44E8fa733c;
    // address w4 = 0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db;
    ...
    modifier whenSalePaused() {
        require(
            state == SaleState.Paused,
            "NervousDogs: Not allowed when sale in progress"
        );
        _;
    }
    ...
    function revealMetadata(string memory uri) public onlyOwner {
        metadataURI = uri;

        // Emit `event PermanentURI(string _value, uint256 indexed _id);`
    ...
    /// Mints Nervous Dogs
    function mintDog(uint8 numberOfTokens) public payable nonReentrant {
        require(
            state == SaleState.Sale,
            "NervousDogs: Sale state is not supported"
        );

        uint256 supply = totalSupply();
        require(
            supply + numberOfTokens <= MAX_DOGS,
            "NervousDogs: Purchase would exceed max supply"
        );

        require(
            numberOfTokens <= TRANSACTION_MINT_LIMIT,
            "NervousDogs: Can only mint 10 tokens at a time"
        );

        require(
            price * numberOfTokens <= msg.value,
            "NervousDogs: ETH value sent is not correct"
        );
        ...
        if (
            ...
            (totalSupply() == MAX_DOGS || block.timestamp >= revealTimeStamp)
        ) {
            randomStartingIndexBlock = block.number;
        }
    }

    function presaleMintDog(uint8 numberOfTokens) public payable nonReentrant {
        require(
            state == SaleState.Presale,
            "NervousDogs: Sale state is not supported"
        );
        ...
        require(
            ...
            "NervousDogs: Address is not in the presale list"
        );
        require(
            presaleList[msg.sender] - numberOfTokens >= 0,
            "NervousDogs: Address reached the presale mint limit"
        );
    ...
    function claimDog() public nonReentrant {
        require(
            state == SaleState.Collectors,
            "NervousDogs: Sale state is not supported"
        );
        ...
        require(
            supply + 1 <= MAX_DOGS,
            "NervousDogs: Claim would exceed max supply"
        );
        require(
            supply + 1 <= COLLECTORS_DOGS,
            "NervousDogs: Claim would exceed collectors supply"
        );
        require(
            collectorsList[msg.sender] > 0,
            "NervousDogs: Address is not in the collector's list"
        );
        require(
            collectorsList[msg.sender] - 1 >= 0,
            "NervousDogs: Address reached the collector's claim limit"
        );
    ...
    function calculateStartingIndex(uint256 blockNumber)
        ...
        returns (uint256)
    {
        return uint256(blockhash(blockNumber)) % MAX_DOGS;
    }

    /// Set the random starting index for the collection.
    function setRandomStartingIndex() public {
        require(
            randomStartingIndex == 0,
            "NervousDogs: Random starting index is already set"
        );
        require(
            randomStartingIndexBlock != 0,
            "NervousDogs: Random starting index block must be set"
        );
    ...
    function reserveTeamDogs(uint8 numberOfTokens) public onlyOwner {
        uint256 supply = totalSupply();
        require(
            supply + numberOfTokens <= MAX_DOGS,
            "NervousDogs: Mint would exceed max supply"
        );
        require(
            teamSupply + numberOfTokens <= TEAM_RESERVED_DOGS,
            "NervousDogs: Mint would exceed team supply"
        );
    ...
    event SplitsWithdrawn(address[2] _addresses, uint256[2] _amounts);
    ...
    function withdrawAll() public payable onlyOwner {
        ...
        emit SplitsWithdrawn([w1, w2], [_each, _each]);
    }
