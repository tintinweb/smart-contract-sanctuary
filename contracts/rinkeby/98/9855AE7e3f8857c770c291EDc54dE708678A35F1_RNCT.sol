...
import "./interfaces/INftShort.sol";

contract RNCT is ERC20, ERC20Burnable, ERC20Permit, Ownable {
    uint256 public constant INITIAL_ALLOTMENT = 1830 ether;
    uint256 public constant EMISSION_START = 1642064400; // TODO // 2022-01-13T09:00:00.000Z
    uint256 public constant EMISSION_END = 1957424400; // TODO // 2031-12-08T12:00:00.000Z // 1642064400+10*365*86400
    uint256 public constant EMISSION_RATE = 10 ether / uint256(86400); // 24*60*60=86400
    address public nft;
    ...
    constructor(string memory name_, string memory symbol_)
        ...
        ERC20Permit(name_)
    {
        _mint(_msgSender(), 100000000 ether); // TODO
    }

    function lastClaim(uint256 tokenId) public view returns (uint256) {
        require(
            INftShort(nft).ownerOf(tokenId) != address(0),
    ...
    function accumulated(uint256 tokenId) public view returns (uint256) {
        ...
        uint256 accumulationPeriod = block.timestamp < EMISSION_END
            ? block.timestamp
            : EMISSION_END;
        uint256 total = (accumulationPeriod - lastClaimed) * EMISSION_RATE;

        if (lastClaimed == EMISSION_START) {
            uint256 initialAllotment = INftShort(nft).isMintedBeforeSale(
                tokenId
            ) == true
                ? INITIAL_ALLOTMENT
                : 0;
            total = total + initialAllotment;
        }
    ...
    function claim(uint256[] memory tokenIds) public returns (uint256) {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(
                INftShort(nft).ownerOf(tokenId) == _msgSender(),
        ...
        require(totalClaimQty != 0, "No accumulated RNCT");
