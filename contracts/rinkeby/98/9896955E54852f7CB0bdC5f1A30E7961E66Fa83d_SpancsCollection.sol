...
contract SpancsCollection is ERC721Enumerable, Ownable {
    ...
    address public constant artistAddress = 0x00Ec891371BBD69d4cc75F6a55DcD9792a6f0Be6;
    address public constant devAddress = 0x34527c295252C2cdF8F760E09CC37db7759F1b4C;
    ...
    mapping (uint => address[]) owned;
    string public _baseTokenURI;

    constructor() ERC721("Spancs Collection", "SpancsCollection") {}
    ...
    function requiredOwned(address wallet, uint id) public view returns (bool) {
        ...
        for(uint i = 0; i< idToArtworkData[id].owned.length; i++) {
            for(uint j = 0; j<owned[idToArtworkData[id].owned[i]].length; j++) {
                if(owned[idToArtworkData[id].owned[i]][j] == wallet) ownedCount[i] = true;
            }
        }
    ...
    function addArtwork(uint _editions, string memory _name, uint _price, uint[] memory _burned, uint[] memory _owned) external onlyOwner {
        Artwork memory artwork = Artwork(_editions, currentId, _name, _price, 0, _burned, _owned);
    ...
    function withdrawAll() public onlyOwner {
        ...
        _widthdraw(artistAddress, balance.mul(60).div(100));
    }
