 ...
 */
contract ERC721PresetMinterPauserAutoId is Context, VRFConsumerBase, AccessControlEnumerable, ERC721Enumerable, ERC721Burnable, ERC721Pausable, ERC721URIStorage {
     ...
     * See {ERC721-tokenURI}.
     */
    constructor(string memory name, string memory symbol, string memory baseTokenURI, address _VRFCoordinator, address _LinkToken, bytes32 _keyhash) VRFConsumerBase(_VRFCoordinator, _LinkToken) ERC721(name, symbol) {
    ...
    function fulfillRandomness(
        ...
        tokenPower[_tokenIdTracker.current()] = randomness > 0 ? randomness % 5 : 1;
        _mint(requestToSender[requestId], _tokenIdTracker.current());
        _setTokenURI(_tokenIdTracker.current(), requestToTokenURI[requestId]);
        _updateTokenCreator(_tokenIdTracker.current(), payable(requestToSender[requestId]));
        _tokenIdTracker.increment();
    }

    function onTokenTransfer(address _sender, uint _value, bytes memory _data) public onlyLink {
        require(_value >= fee, "Not enough LINK - fill contract");
