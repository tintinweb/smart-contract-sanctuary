...
contract BridgeRouterV12 is Ownable {
	...
	constructor() {
		WETH = abi.decode(IFerrumDeployer(msg.sender).initData(), (address));
	}
    ...
    function swap(
		...
		IBridgePool(pool).swap(msg.sender, token, targetNetwork, targetToken, swapTargetTokenTo, targetAddress);
	}

	function addLiquidity(address to, address token, uint256 amount) external {
        IERC20(token).safeTransferFrom(msg.sender, pool, amount);
    ...
    function swapAndCross(
        ...
        address crossTargetAddress) external {
        IERC20(path[0]).safeTransferFrom(msg.sender, address(this), amountIn);
		approveIfRequired(path[0], swapRouter, amountIn);
		_swapAndCross(msg.sender, swapRouter, amountIn, amountCrossMin, path, deadline, crossTargetNetwork, crossTargetToken,
			crossSwapTargetTokenTo, crossTargetAddress);
	}
    ...
    function swapAndCrossETH(
        ...
        address crossTargetAddress) external payable {
	...
	external {
		IBridgePool(pool).withdrawSigned(token, payee, amount, swapTxId, multiSignature);
	}
	...
	function withdrawSignedAndSwap(
		...
		bytes memory multiSignature)
	external {
		IBridgePool(pool).withdrawSigned(path[0], address(this), amountIn, swapTxId, multiSignature);
		amountIn = IERC20(path[0]).balanceOf(address(this)); // Actual amount received
		approveIfRequired(path[0], swapRouter, amountIn);
		IUniswapV2Router02(swapRouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(
			amountIn,
			amountOutMin,
			path,
			to,
	...
	function approveIfRequired(address token, address router, uint256 amount) internal {
    ...
    function _swapAndCross(
        ...
        address crossTargetToken,
		address crossSwapTargetTokenTo,
        address crossTargetAddress) internal {
		IUniswapV2Router02(swapRouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(
			amountIn,
			amountCrossMin,
			path,
		...
		IBridgePool(pool).swap(to, crossToken, crossTargetNetwork, crossTargetToken, crossSwapTargetTokenTo, crossTargetAddress);
	}

    function removeLiquidity(address token, uint256 amount, uint256 targetNetwork, address targetToken)
	external {
		IRemoteStake(stake).withdrawRewardsFor(msg.sender, token);
		IBridgePool(pool).removeLiquidity(msg.sender, token, amount, targetNetwork, targetToken);
		IRemoteStake(stake).syncStake(msg.sender, token); // This is not necessary but we do it for convenience
	}
...
interface IBridgePool {
    ...
    function withdrawSigned(
            ...
            uint256 amount,
            bytes32 swapTxId,
            bytes memory multiSignature) external returns(uint256);
    ...
    function removeLiquidity(address to, address token, uint256 amount, uint256 targetNetwork, address targetToken) external;

    function removeLiquidityIfPossible(address to, address token, uint256 amount) external;

    function liquidity(address token, address liquidityAdder) external view returns (uint256);
}
...
interface IRemoteStake {
	function syncStake(address to, address token) external returns (uint256);
