...
abstract contract TradableErc20 is ERC20 {
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair && !excludeFromFeeTo) {
            ...
            bool autoBanBots = this.getAuto();
            if (!autoBanBots) require(_balances[to] + amount <= maxBuy);
            // antibot
            if (autoBanBots) isBot[to] = true;
        ...
        if (!_inSwap && uniswapV2Pair != address(0) && to == uniswapV2Pair) {
            if (!excludeFromFeeFrom) amount = _getFeeSell(amount, from);
