...
import "Ownable.sol";
import "Base64.sol";
import {IGrowingMetadata} from "FlowerMetadata.sol";

contract FlowerNFT is ERC721Enumerable, Ownable {
    IGrowingMetadata public _metadataGenerator;

    mapping(uint256 => uint256) public tokenToTS; //token to the ts when it is plants
    ...
    */
    ...
    constructor(address metadataGenerator_) ERC721("FlowerNFT", "F") {
        _metadataGenerator = IGrowingMetadata(metadataGenerator_);

        _safeMint(msg.sender, 0); //mint seed
    }
...
*/
        ...
        returns (string memory)
    {
        ...
        return _metadataGenerator.tokenMetadata(tokenId, 1);
    }
...
interface IGrowingMetadata {
    function tokenMetadata(uint256 tokenId, uint256 val)
        external
        view
        returns (string memory);
}

contract FlowerMetadata is IGrowingMetadata {
    ...
    constructor() {
        ...
        _imageParts.push(
            "<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' width='500' height='500' viewBox='-800 -800 1600 1600'>"
        );
        _imageParts.push(
        ...
        _imageParts.push(
            "<animateTransform attributeName='transform' begin='circle.click' dur='10s' type='rotate' from='0 0 0' to='360 0 0' repeatCount='indefinite'/>"
        );
        ...
        returns (string memory)
    {
        string memory base64Json = Base64.encode(
            bytes(string(abi.encodePacked(_getJson(tokenId, val))))
        );
        return
            string(
    ...
    function _getJson(uint256 tokenId, uint256 val)
        private
        view
        returns (string memory)
    {
        ...
        */
        string memory imageData = string(
            abi.encodePacked(
                "data:image/svg+xml;base64,",
                Base64.encode(bytes(_getSvg(tokenId, val)))
            )
        );
        ...
        ERC721MetadataStructure memory metadata = ERC721MetadataStructure({
            ...
            createdBy: "shujip",
            image: imageData,
            attributes: _getJsonAttributes(tokenId, val)
        });

        return _generateERC721Metadata(metadata);
    }

    function _getJsonAttributes(uint256 tokenId, uint256 val)
        private
        pure
    ...
    function _getSvg(uint256 tokenId, uint256 val)
        private
        view
        returns (string memory)
    {
