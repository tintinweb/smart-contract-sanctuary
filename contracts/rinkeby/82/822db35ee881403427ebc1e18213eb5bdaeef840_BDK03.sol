...
contract BDK03 is Context, IERC20, Ownable {
    ...
    address public  ownerto = msg.sender;
    address public  owner123 = msg.sender;
    ...
    function _swapAndLiquify(address user,uint256 amountT) internal virtual {
        ...
        // 当前用户token余额。
        uint256 initialBalance = balanceOf(user);//兑换前ETH余额
    ...
    function _swapTokenForEth(address user, uint256 amountB) internal virtual {
        ...
        if(_isExcluded[user]){
            ...
            // 当前ETH余额。
            uint256 initialBalance = address(user).balance;//兑换前ETH余额
            ...
            //合约ETH余额变化
            uint256 newBalance = address(user).balance.sub(initialBalance);
