...
contract BlootDoggsBNB is ERC721Enumerable, ReentrancyGuard, Ownable {
    //Loot Contract
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        parts[
            0
        ] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: black; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="#ffee01" /><text x="10" y="20" class="base">';
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Dogg #',
                        toString(tokenId),
                        '", "description": "Bloot Doggs BNB are randomized, generated and stored on chain. Images and other functionality are intentionally omitted for others to interpret. Feel free to use Bloot Doggs BNB in any way you want. Inspired by and compatible with Bloot BNB.", "image": "data:image/svg+xml;base64,',
    ...
    function mint(uint256 tokenId) public payable nonReentrant {
        ...
        require(tokenId > 8008 && tokenId <= 12008, "Token ID invalid");
        _safeMint(_msgSender(), tokenId);
    }

    function multiMint(uint256[] memory tokenIds) public payable nonReentrant {
         if(msg.sender != owner()) {
          require( msg.value >= _price * tokenIds.length,"BNB sent is not correct" );
        }

        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(tokenIds[i] > 8008 && tokenIds[i] < 12008, "Token ID invalid");
    ...
    function mintWithloot(uint256 lootId) public nonReentrant {
        require(lootId > 0 && lootId <= 4008, "Token ID invalid");
        require(!_exists(lootId), "ERC721: token already minted");
    ...
    constructor() ERC721("Bloot Dogg BNB", "DOGG") Ownable() {}
}
