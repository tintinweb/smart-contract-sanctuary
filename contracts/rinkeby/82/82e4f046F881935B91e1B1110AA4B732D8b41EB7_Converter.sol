...
// File: contracts/abstract/ERC20Votes.sol
 ...
 */
abstract contract ERC20Votes is ERC20Burnable  {
    using SafeMath for uint256;
...
contract Converter is ERC1155Receiver, IProxyTransaction, ERC20Votes {
    ...
    function setConverterTimeLock(address _converterTimeLock) public {
    ...
    function claimNFT(uint256 _nftIndex, address _to) external returns (bool) {
        ...
        if (ERC165Checker.supportsInterface(nfts[_nftIndex].contractAddr, 0xd9b67a26)){
            bytes memory data;
            IERC1155(nfts[_nftIndex].contractAddr).safeTransferFrom(address(this), _to, nfts[_nftIndex].tokenId, nfts[_nftIndex].amount, data);
        }
        else {
            IERC721(nfts[_nftIndex].contractAddr).safeTransferFrom(address(this), _to, nfts[_nftIndex].tokenId);
        }
