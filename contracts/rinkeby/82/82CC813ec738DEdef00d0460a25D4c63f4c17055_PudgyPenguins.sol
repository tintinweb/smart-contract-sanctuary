...
contract PudgyPenguins is ERC721Burnable, ERC721Pausable, RandomlyAssigned, WithSaleStart, WithContractMetaData, WithIPFSMetaData {
    ...
    uint256 public constant MAX_BY_MINT_PUBLIC = 5;
    ...
    mapping(address => uint256) public amountMinted; // to restrict wallets with multiple tickets to mint more than x NFTs
    ...
    constructor(Ticket ticket, uint256 maxElements, uint256 publicSaleStartTime, string memory contractUri, string memory cid)
        ...
        RandomlyAssigned(maxElements, 1) // Max. 1k NFTs available; Start counting from 1 (instead of 0)
    {
    ...
    function mint(address _to, uint256 _count) public payable saleIsOpen {
        ...
        amountMinted[_msgSender()] = amountMinted[_msgSender()].add(_count);

        require((!isPublicSale && amountMinted[_msgSender()] <= MAX_BY_MINT_PRIVATE) || (isPublicSale && amountMinted[_msgSender()] <= MAX_BY_MINT_PUBLIC), "Exceeds number");
        require(msg.value >= price(_count), "Value below price");
        uint256 requiredTicketAmount = _count.div(MAX_BY_MINT_PRIVATE); // e.g. 3 to mint, 3 mintables per ticket
        require(isPublicSale || privateSaleTicket.balanceOf(_msgSender()) >= requiredTicketAmount, "Not enough tickets");

        for (uint256 i = 0; i < _count; i++) {
            _mintAnElement(_to);
        }
        if (!isPublicSale) {
            for (uint256 i = 0; i < requiredTicketAmount; i++) {
...
abstract contract WithIPFSMetaData is ERC721, Ownable {
    ...
    function revealNFTs(string memory _cid) external onlyOwner {
        require(!isRevealed, "Already revealed"); // NFT metadata cannot be changed after they are revealed
        isRevealed = true; // this function can only be called once
        _setCID(_cid);
        emit MetadataFinalized(_cid);
    }
}
...
contract Ticket is ERC721Enumerable, Ownable, ERC721Burnable {
    ...
    function hasTickets(address _wallet) external view returns (bool) {
        return 0 < ERC721.balanceOf(_wallet);
    }
