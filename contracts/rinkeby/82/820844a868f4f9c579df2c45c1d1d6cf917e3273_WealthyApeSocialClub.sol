...
contract WealthyApeSocialClub is Ownable, ERC721, PaymentSplitter, Pausable {
    uint256 public constant TOTAL_QTY = 7777;
    uint256 public constant FREE_MINT_MAX_QTY = 3;
    ...
    constructor(address[] memory payees, uint256[] memory shares) ERC721("Wealthy Ape Social Club", "WASC") PaymentSplitter(payees, shares) {}
    ...
    function mint(uint256 mintQty) external payable whenNotPaused {
        ...
        if (mintedQty_ < FREE_MINT_MAX_QTY + TEAM_RESERVE_QTY) {
            ...
            require(mintedQty_ + mintQty <= FREE_MINT_MAX_QTY + TEAM_RESERVE_QTY, "MAXL");
            require(!freeMint[msg.sender], "Already minted free");
        }
        //paid
        else {
            require(mintedQty_ + mintQty <= TOTAL_QTY, "MAXL");
            require(
                minterToTokenQty[msg.sender] + mintQty <= maxQtyPerWallet_,
                "MAXP"
            );
            require(msg.value >= price_ * mintQty, "SETH");
        }
        ...
        uint256 totalSupplyBefore = mintedQty_;

        mintedQty_ += mintQty;
        minterToTokenQty[msg.sender] += mintQty;
        for (uint256 i = 0; i < mintQty; i++) {
            _mint(msg.sender, totalSupplyBefore + i);
        }
    }
