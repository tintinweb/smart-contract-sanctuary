...
contract pizzaNFT is ERC721, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    function bakePizzaAndMint(address from, string memory metadata, uint256 base, uint256 sauce, uint256 cheese, uint256[] memory meats, uint256[] memory toppings ) public {
      ...
      //  require(sauceType == 2, "Please select valid sauce type");
       //(cheeseType == 3, "Please select valid cheese type");
        uint256 meatType = 0;
    ...
    function setRandomBakeDateTime(uint256 _startTime, uint256 _endTime) public onlyOwner returns(bool) {
        startTime = _startTime;
        endTime = _endTime;
        emit setTime(msg.sender,startTime,endTime);
        return true;
    }
