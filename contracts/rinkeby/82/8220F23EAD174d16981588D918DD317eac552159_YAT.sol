...
contract Metadata {
	string public name = "Yat NFT (testnet)";
...
contract YAT {
	...
	modifier _onlyOwner() {
		require(msg.sender == owner(), "Not the admin of the contract");
		_;
	}
	...
	modifier _onlyTokenOwnerOrCosigner(uint256 _tokenId) {
		require(msg.sender == ownerOf(_tokenId) || msg.sender == cosignerOf(_tokenId), "Not the token owner or cosigner");
		_;
	}
	...
	function mint(string calldata _token, address _account, uint256 _expiry, bytes memory _signature) external {
		...
		require(_verifyMint(_token, _account, _expiry, _signature), "Invalid signature");
    ...
    **/
	...
	function updateRecord(uint256 _tokenId, string memory _key, string memory _value, bytes memory _signature) external {
		require(_verifyRecordUpdate(_tokenId, _key, _value, info.list[_tokenId].nonce++, _signature), "Cannot update record, signature must be from either the token owner or cosigner");
	...
	function safeTransferFrom(address _from, address _to, uint256 _tokenId, bytes memory _data) public {
		...
		if (_size > 0) {
			require(Receiver(_to).onERC721Received(msg.sender, _from, _tokenId, _data) == 0x150b7a02, "onERC721Received not implemented on receiving contract");
		}
	}
	...
	function idOf(string memory _token) public view returns (uint256) {
		...
		require(info.idOf[_hash] != 0, "Token not found");
	...
	function getRecordsTable(uint256 _tokenId, uint256 _limit, uint256 _page, bool _isAsc) public view returns (bytes32[] memory hashes, bytes32[] memory keys, bool[] memory keysTrimmed, bytes32[] memory values, bool[] memory valuesTrimmed, uint256 totalRecords, uint256 totalPages) {
		require(_limit > 0, "Page limit should be greater than zero");
		totalRecords = recordsOf(_tokenId);

		if (totalRecords > 0) {
			totalPages = (totalRecords / _limit) + (totalRecords % _limit == 0 ? 0 : 1);
			require(_page < totalPages, "The requested page number should be within the range");
	...
	function _transfer(address _from, address _to, uint256 _tokenId) internal {
		...
		require(_from == _owner, "From address is not the token owner");
		require(msg.sender == _owner || msg.sender == _approved || isApprovedForAll(_owner, msg.sender), "Not token owner or approved");
	...
	function _updateRecord(uint256 _tokenId, string memory _key, string memory _value) internal {
		require(bytes(_key).length > 0, "Invalid key");
		bytes32 _hash = keccak256(abi.encodePacked(_key));
	...
	function _deleteRecord(uint256 _tokenId, string memory _key, bytes32 _hash) internal {
		Token storage _token = info.list[_tokenId];
		require(_token.indexOf[_hash] != 0, "Token not found");
