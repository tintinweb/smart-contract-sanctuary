...
contract SanchiTest is ERC721Tradable {
    ...
    /// @notice Mints `tokenId` and transfers it to message sender
    /// @param amount Number tokens to mint
    function mvpMint(uint256 amount) external payable {
        require(msg.value >= price * amount, "Incorrect amount sent");
    ...
    function withdraw_invest() external {
        ...
        withdrawInvest = true;
        // uint256 amountToRecover = (5 * (address(this).balance - 15)) / 100;
        uint256 amountToRecover = (5 * (address(this).balance - 0)) / 100;
