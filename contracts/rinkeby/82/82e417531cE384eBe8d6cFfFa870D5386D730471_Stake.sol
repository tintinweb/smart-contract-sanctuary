...
contract Stake is Ownable {
    ...
    function aprLp(uint256 _pid, uint256 _lpAmount)
        public
        view
        returns (
            uint256 apr,
        ...
        )
    {
        ...
        dreamAmount = (_lpAmount * pool.accERC20PerShare) / 1e18;

        userPoolRatio = (_lpAmount * 1e18) / (totalStaked + _lpAmount);
        rewardOneBlock = (rewardPerBlock * userPoolRatio) / 1e18;
        rewardOneDay = rewardOneBlock * 28653;
        // Binance average 1 year total block count (28653 * 365) = 10458345
        rewardOneYear = rewardOneBlock * (28653 * 365);
    ...
    function apr(uint256 _pid) public view returns (uint256) {
        PoolInfo memory pool = poolInfo[_pid];
        uint256 totalERC20Amount = (totalStaked * pool.accERC20PerShare) / 1e18;
        // Binance average 1 year total block count (28653 * 365) = 10458345
        uint256 rewardOneYear = rewardPerBlock * (28653 * 365);
