...
contract PopsicleStand is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public totalStakingTokenDeposited;
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > lastRewardBlockTime && totalStakingTokenDeposited != 0) {
            uint256 multiplier =
                getMultiplier(lastRewardBlockTime, block.timestamp);
            uint256 iceReward = multiplier * icePerSecond;
            accIcePerShareTemp += iceReward * 1e12 / totalStakingTokenDeposited;
    ...
    function updatePool() internal {
        ...
        //uint256 stakingSupply = stakingToken.balanceOf(address(this));
        if (totalStakingTokenDeposited == 0) {
        ...
        uint256 iceReward = multiplier * icePerSecond;
        accIcePerShare += iceReward * 1e12 / totalStakingTokenDeposited;
    ...
    function deposit(uint256 _amount) public nonReentrant whenNotPaused {
        ...
        user.amount += _amount;
        totalStakingTokenDeposited += _amount;
