...
contract Bridge is Governance {
    ...
    event WithdrawToken(address token, address to, uint256 quantity);
    ...
    modifier isActive () {
        require (ACTIVE_STATE, "Bridge isnt active");
        _;
    }

    function depositToken (uint256 quantity, uint256 targetChain, address receiverAddress) public isActive {
        ...
        require(balanceBefore + quantity == balanceAfter, "Token isnt arrived");
        
        emit Deposit(_msgSender(), quantity, targetChain, receiverAddress);
    }
    
    function releaseTokens (address[] memory to, uint256[] memory quantity, uint256[] memory originChain, address[] memory senderAddress) public RelayerOnly isActive {
    ...
    function changeState (bool activate) public GovernanceOnly {
        require(ACTIVE_STATE != activate, "Already applied");

        ACTIVE_STATE = activate;
        emit StateChanged(ACTIVE_STATE);
    }

    function withdrawToken (address token, address to, uint256 quantity) public GovernanceOnly {
        IERC20 tokenObj = IERC20(token);
        tokenObj.transfer(to, quantity);
        emit WithdrawToken(token, to, quantity);
    }
}