...
contract Doji1155Accounting is Ownable{
	event LuckyHolder1155(uint256, address, uint, uint);
	event ChosenHolder1155(uint256, address, uint, uint);
    ...
    returns(bool success) 
  {
	...
	function random1155(address _contract, uint tokenID, uint _amount) external onlyClaimContract {
	  ...
	  emit LuckyHolder1155(luckyFuck, _contract, tokenID, _amount);
	}
	...
	function send1155(address _contract, uint tokenID, uint _amount, uint256 chosenHolder) public {
		...
		emit ChosenHolder1155(chosenHolder, _contract, tokenID, _amount);
	}
...
contract Doji721Accounting is Ownable {
    event LuckyHolder(uint256 holder, address sender);
    event ChosenHolder(uint256, address);
