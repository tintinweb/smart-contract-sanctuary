...
import "./JustBackgroundsColors.sol";
import "./JustBackgroundsTraits.sol";
...
contract JustBackgrounds is ERC721PresetCollectible, Ownable {
    ...
    /**
     * @dev Initializes metadata.
     */
    constructor() ERC721PresetCollectible("JustBackgrounds", "GRNDS", 256, 0.01 ether, 5, 10) {
    ...
    /**
     * @dev Selects random color from unused colors.
     */
    function _consumeUnusedColor(uint256 tokenId) private returns (bytes1) {

        uint256 index = _isReserved ? _generateRandomNum(tokenId) % _unusedColors.length : tokenId - 1;
        bytes1 color = bytes1(_unusedColors[index]);
        _unusedColors[index] = _unusedColors[_unusedColors.length - 1];
        _unusedColors.pop();
        return color;
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        bytes6 metadata = JustBackgroundsColors.getColorMetadata(_tokenToColorIndex[tokenId]);
        JustBackgroundsTraits.ColorTraits memory traits = JustBackgroundsTraits.getColorTraits(metadata);

        bytes memory displayName = abi.encodePacked(traits.name);
        if (!_checkIfMatch(traits.special, 'Transparent') && !_checkIfMatch(traits.special, 'Meme')) {
            displayName = abi.encodePacked(displayName, " #", traits.hexCode);
        }

        bytes memory byteString;
        byteString = abi.encodePacked(byteString, 'data:application/json;utf8,{');
        byteString = abi.encodePacked(byteString, '"name": "', displayName, '",');

        /// Description ///
        byteString = abi.encodePacked(byteString, '"description": "**Just Backgrounds** (b. 2021)\\n\\n');
        byteString = abi.encodePacked(byteString, displayName, '\\n\\n');
        ...
        byteString = abi.encodePacked(byteString,'{"trait_type": "Edition", "display_type": "number", "value": ', tokenId.toString(), ', "max_value": ', _tokenMaxSupply.toString(), '}');
     ...
     */
    function _generateSvgFromTraits(JustBackgroundsTraits.ColorTraits memory traits) private pure returns (bytes memory svg) {
...

{
  ...
  "libraries": {
    "contracts/JustBackgroundsColors.sol": {
      "JustBackgroundsColors": "0x90f72bdfb3bd2c40f13ac186859667b5cdedbb85"
    },
    "contracts/JustBackgroundsTraits.sol": {
      "JustBackgroundsTraits": "0x4a48eeb12decc0d1b93c7dc74cc375e817315d29"
    }
  }
}