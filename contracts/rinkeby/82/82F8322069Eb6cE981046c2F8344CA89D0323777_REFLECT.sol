...
contract REFLECT is Context, IERC20, Ownable {
    ...
    uint256 private constant _tPremint = 540 * 10**9 * 10**9;
    ...
    address public adminAddress = 0x7FbB19E6e0096bA8782BD420bB7bd74dCC0b819D; // update admin wallet here

    constructor () public {
        
        _rOwned[adminAddress] = _rTotal;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_isExcluded[sender]) {
            _transferBothExcluded(sender, address(0), burnAmt);
            if (_isExcluded[adminAddress]) {
                _transferBothExcluded(sender, adminAddress, adminAmt);
            }
            else {
                _transferFromExcluded(sender, adminAddress, adminAmt);
            }
        }
        else {
            _transferToExcluded(sender, address(0), burnAmt);
            if (_isExcluded[adminAddress]) {
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256) {
        uint256 tFee = tAmount.mul(_taxFee).mul(10).div(985);
