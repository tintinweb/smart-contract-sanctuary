...
contract RacesData is Ownable {
     ...
     */
    mapping(uint256 => bytes) public races;    
    ...
    function setGlobalParameters(
        ...
        (bool success, ) = control.methods.delegatecall(
            abi.encodeWithSignature(
                "setGlobalParameters((address,address,address,address,address,address,bool))",
                input
            )
        );
    ...
    function createQueues(Queue.Default[] memory theQueues) external onlyOwner {
        (bool success, ) = control.methods.delegatecall(
            abi.encodeWithSignature(
                "createQueues((uint256,address,uint256,uint32,uint32)[])",
                theQueues
            )
        );
        require(success,error);
    ...
    function uploadRace(bytes memory race) external onlyOwner {
        (bool success, ) = control.methods.delegatecall(
            abi.encodeWithSignature(
                "uploadRace((uint256,bytes,address,uint256,uint256,uint32))",
                race
            )
        );
        require(success,error);
