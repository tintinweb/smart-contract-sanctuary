	...
	Ownable
{
		...
		updateReward(address(0))
	{
		...
		// todo audit
		// For the pools where users get rewards in the staking token
		// deduct the staked amount to pay rewards only from the distribution pot
	...
	function stakeNFT(uint256 tokenId) external updateReward(msg.sender) {
		...
		_originalBalances[msg.sender] = _balances[msg.sender];
		if (_balances[msg.sender] > 0)
