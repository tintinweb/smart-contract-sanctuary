...
contract EventDistribution is VRFConsumerBase, AccessControl {
  ...
  PlatformStakingErc20 public PLATFORM_STAKINGE_RC20;
  ...
  constructor(
    ...
    VRFConsumerBase(_VRF_COORDINATOR, _LINK) {
      ...
      PLATFORM_STAKINGE_RC20 = PlatformStakingErc20(_PLATFORM_STAKING_ERC20);
  ...
  function buyWithPoints(address _nft, uint256 _amount) external payable onlyIfEventOpen(_nft) {
    ...
    uint256 ethUsdAmount = priceEth.mul(msg.value).div(decimalsEth);
    uint256 pointUsdAmount = pricePoint.mul(_amount).div(decimalsPoint);
  ...
  function buyWithDrops(address _nft, uint256 _amount) external onlyIfEventOpen(_nft) {
    ...
    uint256 usdAmount = priceDrops.mul(_amount).div(decimals);
  ...
  function buyWithNdr(address _nft, uint256 _amount) external onlyIfEventOpen(_nft) {
    ...
    uint256 usdAmount = priceNdr.mul(_amount).div(decimals);
  ...
  function cashback(address _account, uint256 _amount) private {
    (uint256 priceDrops, uint256 decimals, ) = PRICE_AGGREGATOR.getLatestPrice(DROPS_PAIR);
    (uint256 cashbackRate, uint256 cashbackDecimals) = PLATFORM_STAKINGE_RC20.cashbackRateOf(_account);
...
contract EventStore is AccessControl {
  ...
  function setEventNfts(
    ...
    uint256 pricesTotal = 0;
    for(uint8 i = 0; i < _counts.length; i++) {
      pricesTotal = pricesTotal.add(_counts[i]);
    }
    require(pricesTotal == events[_nft].nfts.length, "Invalid amount of prices for nfts");
...
contract PriceAggregator is Ownable {
  ...
  function setPrice(bytes32[] calldata _pairs, uint256[] calldata _prices, uint256[] calldata _decimals) external onlyController() {
    ...
    for(uint8 i = 0; i < _pairs.length; i++) {
      ...
      prices[_pairs[i]][block.timestamp] = Price(_prices[i], _decimals[i]);
    }
  }
}
