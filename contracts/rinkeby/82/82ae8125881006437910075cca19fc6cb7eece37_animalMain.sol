...
contract animalMain is  ERC721,Ownable {
  ...
  function createAnimal() public {
      ...
      bytes32 star_b = keccak256(abi.encodePacked(block.coinbase,blockhash(block.number-1)));
      star = uint(star_b) % 10 + 1;
      
      bytes32 roletype_b = keccak256(abi.encodePacked(block.coinbase,blockhash(block.number-1)));
      roletype = uint(roletype_b) % 1000 + 1;
