...
contract DepressedOG is ERC721Enumerable, Ownable {
    using Strings for uint256;
    
    mapping (uint256 => address) private _ogminter;
    
    uint256 public constant MAX_DEPRESSED_OGS = 8100;
    uint256 public constant MAX_PURCHASES_PER_TRANSACTION = 20;
    uint256 public constant TOTAL_RESERVED_DEPRESSED_OGS = 25;

    bool public isSaleActive = false;
    uint256 public price = 60000000000000000;
    string internal _baseTokenURI;
    
    constructor(string memory baseURI) ERC721("Depressed OGs", "DepressedOG")  {
        updateBaseURI(baseURI);

    }

    modifier saleHasNotEnded{
        require(totalSupply() < MAX_DEPRESSED_OGS, "Sale has ended.");
        _;
    }

    function mintDepressedOGs(uint256 numTokens) public payable saleHasNotEnded {        
        if(msg.sender != owner()){
            require(isSaleActive, "Sale is not active.");
        }
        require(SafeMath.add(totalSupply(), numTokens) <= MAX_DEPRESSED_OGS, "Exceeds total Depressed OG supply.");
        require(totalSupply() < MAX_DEPRESSED_OGS, "Sale has ended.");
