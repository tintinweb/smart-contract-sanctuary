...
contract PrizeController is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using SafeERC20 for QEntry;
    //Info of each prize pool.
    ...
    function createPool(IERC20 _prizeToken, uint256 _prizeUnit) public onlyOwner {
        require(_prizeToken.balanceOf(address(this)) >= _prizeUnit);
        poolCollection.push(PrizePool({
            prizeToken: _prizeToken,
            countEntry: 0,
            currentWinner: this.owner(),
            poolId: poolCollection.length-1,
            prizeUnit: _prizeUnit,
            timeLastEntered: block.timestamp
        }));
    }

    // enter to pool to win
    function enter(uint256 _pid) public nonReentrant {
        require(qentry.balanceOf(msg.sender) >= (poolCollection[_pid].prizeUnit/100));
        require(block.timestamp - poolCollection[_pid].timeLastEntered <= poolDuration);
        qentry.transferFrom(msg.sender, burnAddress, poolCollection[_pid].prizeUnit/100);
        poolCollection[_pid].timeLastEntered = block.timestamp;
        poolCollection[_pid].countEntry +=1;
    ...
    function donate(IERC20 _donateToken, uint256 _amount) public {
        _donateToken.transfer(address(this), _amount);
    }

    function claimPrice(uint256 _pid) public nonReentrant {
        require(block.timestamp- poolCollection[_pid].timeLastEntered > poolDuration);
        qentry.transferFrom(msg.sender,burnAddress, poolCollection[_pid].countEntry);
        poolCollection[_pid].prizeToken.transferFrom(address(this),poolCollection[_pid].currentWinner, poolCollection[_pid].prizeUnit);
    }

    function setPoolDuration(uint256 duration) public onlyOwner {
