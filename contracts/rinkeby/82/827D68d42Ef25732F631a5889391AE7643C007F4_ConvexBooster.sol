...
contract ConvexBooster {
    ...
    function addConvexPool(
        ...
        address rewardCvxPool = IConvexRewardFactory(convexRewardFactory)
            .CreateRewards(rewardCvxToken, virtualBalance, address(this));
    ...
    function withdrawFor(
        ...
        if (IConvexRewardPool(pool.rewardCrvPool).earned(_user) > 0) {
            IConvexRewardPool(pool.rewardCrvPool).getReward(_user, true);
        }
    ...
    function cliamRewardToken(uint256 _pid) public {
        ...
        address currentCrvRewards = pool.rewardCrvPool;
        ...
        if (cvxBal > 0) {
            ...
            IConvexRewardPool(pool.rewardCvxPool).queueNewRewards(cvxBal);
        }
    }
