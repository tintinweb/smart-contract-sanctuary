...
// SafeShrimpCredits which will be replaced by SafeShrimp
contract SafeShrimpCredit is BEP20('SafeShrimpCredit Token', 'SAFESHRIMPCREDIT') {
...
contract Presale is Ownable {
    ...
    // *** Config ***
    uint startPresale = 1615056052;           // unix timestamp for presale to go live
    uint256 presaleTokenAmount = 2500000000 ether; // 12,500 token presale - 150 BNB Hard Cap
    uint256 pricePresale = 56;                 // 17857142.8571 SafeShrimpCredit per BNB
    uint256 maxPerWallet = 1 ether;          // Max 1 BNB per Wallet
    // --- Config ---

    SafeShrimpCredit token = new SafeShrimpCredit();
    ...
    receive() external payable {
        ...
        uint256 amount = msg.value / pricePresale * 1000000000;
        require(amount <= token.balanceOf(address(this)), "Insufficient token balance in ICO");
        require((amount + token.balanceOf(address(msg.sender)) <= (maxPerWallet / pricePresale * 1000000000) ), "Over Max Per Wallet");
