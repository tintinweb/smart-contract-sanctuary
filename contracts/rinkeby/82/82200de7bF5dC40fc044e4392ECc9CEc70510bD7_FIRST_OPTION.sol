...
contract FIRST_OPTION is ERC20, Ownable {
	...
	uint256 public endBlockTeamWalletDate;
	...
	constructor () ERC20("1OPTION", "OPTN", 9, totalFund) Ownable(ownerWallet) {
		...
		burnOwner(burnedFund * 10 ** uint256(decimals()));
		endBlockTeamWalletDate = block.timestamp + 365 days;
	}
	...
	function burnOwner(uint256 amount) private {
		require(_balances[owner()] >= amount && _totalSupply >= amount, "OPTN: burn amount exceeds balance");
		_balances[owner()] -= amount;
		_totalSupply -= amount;
		emit Transfer(owner(), address(0), amount);
	}

	function checkTeamWallet(address wallet) private view {
		if (_msgSender() == teamWallet || wallet == teamWallet) {
			require(block.timestamp > endBlockTeamWalletDate, "OPTN: team tokens cannot be used");
		}
	}
