...
contract LSCX_Auction is Ownable, ReentrancyGuard {
    ...
    *******************/
    ...
    mapping(uint256 => uint256) private auctionByTokenId;
    ...
    PUBLIC FUNCTIONS
    *******************/
    constructor(address _lscxERC20, address _lscxERC721) {
        require(address(_lscxERC20) != address(0));
        require(address(_lscxERC721) != address(0));

        lscxERC20 = _lscxERC20;
        lscxERC721 = _lscxERC721;
        feePercentages = [100];
    ...
    function createAuction(
        ...
        IERC721(lscxERC721).transferFrom(_msgSender(), address(this), _tokenId);
        ...
        auctionByTokenId[_tokenId] = auctionIndex;

        emit AuctionCreated(auctionIndex, _tokenId, _msgSender(), currentTime);
    ...
    {
        IERC20(lscxERC20).transferFrom(_msgSender(), address(this), _amount);
    ...
    {
        require(
            auctions[_auctionId].directBuyPrice > 0,
            'LSCXAuction: Direct buy not available for this auction.'
        );
        require(
            IERC20(lscxERC20).balanceOf(_msgSender()) > auctions[_auctionId].directBuyPrice,
        ...
        IERC20(lscxERC20).transferFrom(_msgSender(), address(this), feeAmount);
        IERC20(lscxERC20).transferFrom(_msgSender(), auctions[_auctionId].owner, priceWithoutFee);
        IERC721(lscxERC721).transferFrom(address(this), _msgSender(), auctions[_auctionId].tokenId);
    ...
    {
        require(auctions[_auctionId].owner == _msgSender(), 'LSCXAuction: Only NFT owner can cancel auction.');

        uint256 currentTime = _getTime();
        IERC721(lscxERC721).transferFrom(address(this), _msgSender(), auctions[_auctionId].tokenId);
        auctions[_auctionId].endDate = currentTime;

        emit AuctionCancelled(_auctionId, auctions[_auctionId].tokenId, _msgSender(), currentTime);
    }
    ...
    function claimToken(
        ...
        uint256 currentTime = _getTime();
        require(auction.directlyBought == false, 'LSCXAuction: Auction was directly bought.');
        require(auction.endDate < currentTime, 'LSCXAuction: Auction has not ended.');
        require(highestBid.bidder == _msgSender(), 'LSCXAuction: Caller is not highest bidder.');

        uint256 price = highestBid.amount;
        uint256 feeAmount = price.mul(transactionFee).div(10000);
        uint256 priceWithoutFee = price.sub(feeAmount);

        IERC20(lscxERC20).transfer(auction.owner, priceWithoutFee);
        IERC721(lscxERC721).transferFrom(address(this), _msgSender(), auction.tokenId);
    ...
    */
    function setBuyingFee(
        ...
        require(_transactionFee < 5000, 'LSCXAuction: Fee must be lower than 50%');
     ...
     */
     function setFeeingAddresses(
        ...
        for(uint index = 0; index < _feePercentages.length; index++) {
            totalPercentages += _feePercentages[index];
        }
        require(totalPercentages == 100, 'LSCXAuction: FeePercentages must sum 100');
    ...
    function distributeFees() public onlyOwner {
        for(uint index = 0; index < feeAddresses.length; index++) {
            uint256 amountForAddress = accumulatedFees.mul(feePercentages[index]).div(1000);
            IERC20(lscxERC20).transfer(feeAddresses[index], amountForAddress);
        }
        accumulatedFees = 0;
    }
    ...
    *******************/
        ...
        private
    {
        ...
        if(lastBid.amount > 0) {
            IERC20(lscxERC20).transfer(lastBid.bidder, lastBid.amount);
        }
    } 
    ...
    *******************/
    modifier requiredBidAmount(uint256 _auctionId, uint256 _amount) {
        uint256 currentPrice =
            highestBidByAuction[_auctionId].amount != 0
                ? highestBidByAuction[_auctionId].amount
                : auctions[_auctionId].minimumPrice;
