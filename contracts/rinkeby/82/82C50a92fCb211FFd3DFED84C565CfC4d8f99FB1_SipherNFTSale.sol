...
contract SipherNFTSale is ISipherNFTSale, Whitelist {
  ...
  uint32 public constant MAX_PUBLIC_BOUGHT = 6500;
  uint32 public constant CAP_PER_WHITELISTED_ADDRESS = 5;
  uint32 public constant WHITELISTED_ADDRESS_FREE_MINT_CAP = 2;
  uint32 public constant PUBLIC_SALE_CAP_PER_ADDRESS = 5;
  uint32 public constant REDUCE_PRICE_INTERVAL = 180; //10 minutes
  uint256 public constant REDUCE_PRICE_LEVEL = 50000000000000000; //0.05 ether
  uint256 public constant SALE_BASE_PRICE = 150000000000000000; // 0.15 ether
  uint256 public constant SALE_PUBLIC_STARTING_PRICE = 900000000000000000; //1 ether
  ...
  event WhitelistBought(address indexed buyer, uint32 amount, uint256 amountWeiPaid);
  ...
  event Refund(uint256 refundAmount);
  ...
  function setMerkleRoot(bytes32 _root) external onlyOwner {
    require(
      _blockTimestamp() < _saleConfig.privateTime,
  ...
  function getElapsedPublicSaleTime() public view returns (uint256) {
    ...
    if (_blockTimestamp() < config.publicTime){
      return 0;
    } else {
      return 
      (config.publicTime <= timestamp) && (timestamp < config.privateTime)
      ? timestamp - config.publicTime 
      : config.privateTime - config.publicTime;
    }
  }

  function getPublicSaleCurrentPrice() public view returns (uint256) {
    uint256 elapsed = getElapsedPublicSaleTime();
    if (elapsed < REDUCE_PRICE_INTERVAL)
    {
        uint256 currentPrice = SALE_PUBLIC_STARTING_PRICE;
        return (currentPrice);
    } else {
        if(elapsed >= REDUCE_PRICE_INTERVAL && elapsed < 6*REDUCE_PRICE_INTERVAL)
        {
            uint256 currentPrice = SALE_PUBLIC_STARTING_PRICE - (elapsed / REDUCE_PRICE_INTERVAL)*REDUCE_PRICE_LEVEL;
            return (currentPrice); 
        }else{
            uint256 currentPrice = SALE_PUBLIC_STARTING_PRICE - 5*REDUCE_PRICE_LEVEL - ((elapsed % REDUCE_PRICE_INTERVAL) % 5 )*REDUCE_PRICE_LEVEL/2;
            return
            (elapsed >= (6*REDUCE_PRICE_INTERVAL) && elapsed < (10*REDUCE_PRICE_INTERVAL))
            ? currentPrice
            : SALE_BASE_PRICE; 
        }
    }
   ...
   */
  function buy(uint32 amount,uint32 privateCap, uint32 freeMintCap, bytes32[] memory proofs) external payable override {
    address buyer = msg.sender;
    // only EOA or the owner can buy, disallow contracts to buy
    require((!buyer.isContract() && buyer == tx.origin) || buyer == owner(), 'SipherNFTSale: only EOA or owner');
    require(merkleRoot != bytes32(0), 'SipherNFTSale: merkle root is not set yet');

    _validateAndUpdateWithBuyAmount(buyer, amount, privateCap, freeMintCap, proofs);

    nft.mintGenesis(amount, buyer,msg.value);
  }
  ...
  function rollStartIndex() external override {
    ...
    require((!sender.isContract()&& sender == tx.origin) || sender == owner(), 'SipherNFTSale: only EOA or owner');
  ...
  *       - can buy up to MAX_OWNER_BOUGHT_INITIAL before endTime with price = 0
   ...
   *      - each whitelisted buyer can buy up to CAP_PER_WHITELISTED_ADDRESS tokens with SALE_BASE_PRICE per token
   *    4. If the buy time is in public buy time:
   *      - each buyer can buy up to total of CAP_PER_ADDRESS tokens with currentPrice per token
   */
  function _validateAndUpdateWithBuyAmount(address buyer, uint32 amount, uint32 privateCap, uint32 freeMintCap, bytes32[] memory proofs) internal {
    SaleConfig memory config = _saleConfig;
    ...
    uint256 timestamp = _blockTimestamp();
    uint256 costToMint = getPublicSaleCurrentPrice()* amount;
    ...
    require(config.publicTime <= timestamp, 'Public Sale not started');
    require(timestamp <= config.endTime, 'SipherNFTSale: already ended');

    if (config.publicTime <= timestamp && timestamp < config.privateTime) {
      // anyone can buy up to CAP_PER_ADDRESS tokens with price of currentPrice eth per token
      // it is applied for total of whitelistBought + publicBought
      require((totalPaid >= costToMint) && (costToMint >= SALE_BASE_PRICE) , 'SipherNFTSale: invalid paid value');
      require(
    ...
    if (config.privateTime <= timestamp && timestamp < config.freeMintTime) {
      // only whitelisted can buy at this period
      require(config.privateTime <= timestamp && timestamp < config.freeMintTime, 'Private Sale not started');
      require(isWhitelistedAddress(buyer, privateCap, freeMintCap, proofs) && whitelistedMerkelRoot != bytes32(0), 'SipherNFTSale: only whitelisted buyer');
    ...
    if (config.freeMintTime <= timestamp && timestamp < config.endTime) {
      // only whitelisted can buy at this period
      require(config.freeMintTime <= timestamp && timestamp < config.endTime, 'Free Mint for Guidmaster not started');
      require(isWhitelistedAddress(buyer, privateCap, freeMintCap, proofs) && whitelistedMerkelRoot != bytes32(0), 'SipherNFTSale: only whitelisted buyer');
      // whitelisted address can buy up to CAP_PER_WHITELISTED_ADDRESS token
      require(
    ...
    if (msg.value > costToMint) {
      ...
      emit Refund(msg.value - costToMint);
    }
  }
...
contract Whitelist is IWhitelist, Ownable {
    ...
    function setWhitelistedMerkleRoot(bytes32 _whitelistedRoot)
        external
        override
        onlyOwner
    {
    ...
    function isWhitelistedAddress(address buyer, uint32 privateCap, uint32 freeMintCap , bytes32[] memory proofs)
        public
        view
        override
        returns (bool)
...
interface IWhitelist {
  ...
  function setWhitelistedMerkleRoot(bytes32 _whitelistedRoot) external;
...
interface ISipherNFT is IERC721Enumerable {
   ...
   */
  function mintGenesis(uint256 amount, address to, uint256 totalPaid) external;
