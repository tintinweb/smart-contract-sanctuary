...
contract Chains is ERC721Enumerable, Ownable {
    ...
    IChains chainsGenOne = IChains(address(0));
    
    
    constructor(address _chainsGenOne) ERC721("Chains", "CHAIN") {
        _owner = msg.sender;
        chainsGenOne = IChains(_chainsGenOne);
    ...
    function migrate(uint _tokenId) internal {
        ...
        tokenIdToHash[thisTokenId] = chainsGenOne._tokenIdToHash(_tokenId);

        hashToMinted[tokenIdToHash[thisTokenId]] = true;

        tokenIdToTimestamp[thisTokenId] = chainsGenOne.getTokenTimestamp(_tokenId);
    ...
    function burnForMint(uint256 _tokenId) public {
        ...
        /**
     * @dev Burns and mints new.
     * @param _tokenId The token to burn from gen one.
     */
    function burnGen1ForMint(uint256 _tokenId) public {
        require(chainsGenOne.ownerOf(_tokenId) == msg.sender);
    ...
    /**
     * @dev Adds to rarity count if rarity is great enough     
     * @param tokenId the token to edit Rarity count for
     * @param index position in the current TIER * 2
     */
    function setTokenIdRarity(uint256 tokenId, uint256 index) internal {
         if (index < 10) {  
           uint256 rScore = (10 - index);
           tokenIdToRarityCount[tokenId] += rScore; 
        }
    }
