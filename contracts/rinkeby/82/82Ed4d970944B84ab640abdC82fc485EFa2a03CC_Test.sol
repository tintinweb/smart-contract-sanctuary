...
contract Test is Ownable, ERC721Enumerable, ITest {
    ...
    // Wednesday, 18 August 2021, 18:18:18
    uint256 public immutable saleStartTimestamp = 1597774698;
    ...
    address internal constant OPEN_SEA_PROXY_REGISTRY = 0xF57B2c51dED3A29e6891aba85459d600256Cf317;

    /// @notice Creates Test NFTs, stores all the required parameters.
    ...
    function mint(uint256 amount) external payable override {
        ...
        if (amount > maxAmountPerMint)
            revert AmountExceedsMaxPerMint(amount, maxAmountPerMint);
        if (msg.value < price * amount)
            revert NotEnoughEther(msg.value, price * amount);
...
interface ITest is IERC721Enumerable {
    /**
