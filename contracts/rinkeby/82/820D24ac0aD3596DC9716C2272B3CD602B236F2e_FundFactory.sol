...
contract FundFactory {
    ...
    constructor(
        ...
        console.log("Deploying a Factory");
        ...
        DEFAULT_OWNERS.push(address(this));
    }

    function createFund(
        string calldata name,
        string calldata symbol,
        // FundFactory contract has to be approved to transfer all of these NFTs!
        address[] calldata nftAddresses,
        uint256[] calldata nftIds,
        bytes[] calldata ensResolverData
    ) external {
        console.log("Creating a fund with name: ", name);
        require(
            nftAddresses.length == nftIds.length,
            "NFT address and ID arrays must have same lengths"
        );
        bytes32 nameHash = keccak256(abi.encodePacked(name));
        console.log("Got nameHash", uint256(nameHash));
        bytes32 ensSubNode = keccak256(abi.encodePacked(BASE_ENS_NODE, nameHash));
        console.log("Got ens subnode", uint256(ensSubNode));
        require(ENS.owner(ensSubNode) == address(0), "ENS subname is already owned");

        bytes32 safeSalt = keccak256(abi.encodePacked(address(this), BASE_ENS_NODE, name));
        console.log("Got safeSalt", uint256(safeSalt));
        address safe = createAndSetupSafe(safeSalt);
        console.log("Got safe", safe);
        address token = createAndSetupToken(name, symbol, safe);
        console.log("Got token", token);
        address fund =
            createAndSetupFundManager(
                safe,
                token,
                nftAddresses,
                nftIds
            );
        console.log("Got fund", fund);
        
        ENS.setSubnodeOwner(BASE_ENS_NODE, nameHash, address(this));
        ENS.setResolver(ensSubNode, ENS_PUBLIC_RESOLVER_ADDRESS);
        IENSResolver(ENS_PUBLIC_RESOLVER_ADDRESS).multicall(ensResolverData);
        ENS.setOwner(ensSubNode, safe);

        emit FundCreated(name, fund, safe, token);
    }

    function createAndSetupSafe(bytes32 salt) internal returns (address) {
        // Deploy and initialize safe
        IGnosisSafe safe =
            IGnosisSafe(
                payable(
                    address(
        ...
        // TODO deploy safesnap module
        // TODO [optional] remove address(this) from safe owners
    ...
    function createAndSetupToken(
        ...
        // safeAddress is dependent on ens name and factory
        bytes32 salt = keccak256(abi.encodePacked(safeAddress));
        console.log("Salt for token: ", uint256(salt));
        ERC20PresetMinterPauser token =
            new ERC20PresetMinterPauser{salt: salt}(
                // TODO establish what we want recorded token name and symbol to be
                string(abi.encodePacked("sz", name)),
                string(abi.encodePacked("sz", symbol))
            );
        ...
        // TODO initialize token, with sale and initial distribution
        // TODO [optional] remove address(this) from all three of these roles

        return address(token);
    }

    function createAndSetupFundManager(
        address safeAddress,
        ...
        for (uint256 i = 0; i < nftAddresses.length; i++) {
            ...
            nft.safeTransferFrom(msg.sender, address(fund), nftIds[i]);
        }
        fund.deposit(nftAddresses, nftIds);
        return address(fund);
    }
}
...
contract FundManager is Ownable, ERC721Holder {
    // Buyout timeout. 7 days in seconds.
    uint256 public TIMEOUT = 60 * 60 * 24 * 7;
    ...
    // NFTs owned by this fund.
    address[] public nfts;
    uint256[] public ids;

    // The UCF token for this fund.
    IERC20 public token;
    ...
    function deposit(address[] memory _nfts, uint256[] memory _ids) public {
        require(
            _nfts.length == _ids.length,
            "Input array lenghts don't match."
        );
        require(msg.sender == FACTORY, "Bad msg.sender.");
        nfts = _nfts;
        ids = _ids;
        for (uint256 i = 0; i < nfts.length; i++) {
            require(
                IERC721(nfts[i]).ownerOf(ids[i]) == address(this),
                "FundManager doesn't own all of the deposited NFTs."
            );
        }
    }
    ...
    function setBuyout(address _buyer, uint256 _cost) public onlyOwner {
        require(
            buyer == address(0) || block.timestamp >= end,
            "A buyout is in progress."
        );
        buyer = _buyer;
        cost = _cost;
        end = block.timestamp + TIMEOUT;
    }

    function buyout() public payable {
        require(buyer != address(0), "No buyer.");
        require(buyer == msg.sender, "Caller is not the buyer.");
        require(msg.value == cost, "Not enough ETH.");
        require(block.timestamp < end, "Buyout timeout already passed.");
        for (uint256 i = 0; i < nfts.length; i++) {
    ...
    function claim(uint256 _amount) public {
        ...
        uint256 owed = (_amount / token.totalSupply()) * cost;
