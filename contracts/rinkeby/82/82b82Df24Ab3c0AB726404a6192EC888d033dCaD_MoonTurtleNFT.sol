...
contract MoonTurtleNFT is ERC721URIStorage, Ownable {
	...
	uint256 public constant REGULAR_MINT_COST = 0.00001 ether;
	uint public constant MAX_TOKENS = 10000;
	uint public constant MAX_TOKENS_INIT = 500;
	uint internal CURR_SUPPLY_CAP = MAX_TOKENS_INIT;
	
	bool public hasSaleStarted = true;
	string public baseURI = "https://moonturtlenft.com/api/json/";
	
	
    constructor() public ERC721("MoonTurtleNFT", "MoonTurtle") {
	}
        ...
        returns (uint256)
    {
        ...
        _setTokenURI(newItemId,  strConcat(baseURI,strConcat(uint2str(newItemId),"/index.json")));
...
function uint2str(uint _i) internal pure returns (string memory _uintAsString) {
  ...
  /*
   * The max tokens will always be 10000, initially 500 and will start as future characters increase
   */
  ...
  function getBaseURI() public onlyOwner view returns(string memory) {
	...
	function reserveGiveaway(uint256 numTokens, address recipient) public onlyOwner {
		...
		require((totalSupply() + numTokens) <= 500, "Exceeded giveaway supply");
		uint256 index;
		// Reserved for the people who helped build this project
		for (index = 0; index < numTokens; index++) {
			_tokenIds.increment();
			_mint(recipient, currentSupply + index);
			_setTokenURI(currentSupply + index, strConcat(baseURI,strConcat(uint2str(currentSupply + index),"/index.json")));
		}
	}
