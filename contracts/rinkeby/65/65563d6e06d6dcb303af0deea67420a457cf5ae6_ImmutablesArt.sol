...
contract ImmutablesAdminProject is ImmutablesAdmin {
  ...
  /// @dev whether or not artists can toggle if project active.
  bool public artistCanToggleActive = false;
  ...
  /** @dev Allows the teammember to set a featured project id
    ...
    */
  function teamSetFeaturedProject(uint256 _projectId) external onlyTeammember() {
  ...
  /** @dev Allows the contract owner to toggle whether projects are created
    * @dev in an active state.
    */
  function contractOwnerToggleProjectsActiveByDefault() external onlyOwner() {
      artistCanToggleActive = !artistCanToggleActive;
  }
