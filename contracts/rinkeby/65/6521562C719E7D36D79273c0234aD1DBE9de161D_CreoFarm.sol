...
contract CreoFarm is Ownable {
    ...
    mapping (address=> uint256) private swapBal;
    ...
    function _swapRewardForLp(address user) private {
        ...
        uint[] memory amounts = pancakeSwap.swapExactTokensForETH(toSwap,minSwapOut,path,address(this), now + 900);
        uint256 liqETH = amounts[1];
        // uint256 minLiqToken = liq.sub(liq.mul(3).div(100));
        // uint256 minLiqEth = liqETH.sub(liqETH.mul(3).div(100));
        pancakeSwap.addLiquidityETH{ value:liqETH }(address(creoToken),liq,liq.sub(liq.mul(3).div(100)) ,liqETH.sub(liqETH.mul(3).div(100)),user,now + 900);
