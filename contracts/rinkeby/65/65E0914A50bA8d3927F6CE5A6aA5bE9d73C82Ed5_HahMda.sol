...
//  _   _         _              _____ ___  ___ _____   _____  _            _   _  _              _     _         ___  ___        _    _          _____         _                      
// | | | |       | |            |  _  ||  \/  ||  __ \ |  ___|| |          | | | |(_)            | |   (_)        |  \/  |       | |  (_)        |_   _|       | |                     
// | |_| |  __ _ | |__    __ _  | | | || .  . || |  \/ | |__  | |_   ___   | | | | _  ___   ___  | | __ _  _   _  | .  . |  ___  | |_  _ __   __   | |    ___  | | __  ___  _ __   ___ 
// |  _  | / _` || '_ \  / _` | | | | || |\/| || | __  |  __| | __| / _ \  | | | || |/ __| / _ \ | |/ /| || | | | | |\/| | / _ \ | __|| |\ \ / /   | |   / _ \ | |/ / / _ \| '_ \ / __|
// | | | || (_| || | | || (_| | \ \_/ /| |  | || |_\ \ | |___ | |_ | (_) | \ \_/ /| |\__ \| (_) ||   < | || |_| | | |  | || (_) || |_ | | \ V /    | |  | (_) ||   < |  __/| | | |\__ \
// \_| |_/ \__,_||_| |_| \__,_|  \___/ \_|  |_/ \____/ \____/  \__| \___/   \___/ |_||___/ \___/ |_|\_\|_| \__, | \_|  |_/ \___/  \__||_|  \_/     \_/   \___/ |_|\_\ \___||_| |_||___/
//                                                                                                          __/ |                                                                      
//                                                                                                         |___/                                                                       
...
contract HahMda is ERC721, Ownable, ReentrancyGuard {
    using ECDSA for bytes32;
	using Strings for uint256;

    uint256 public totalSupply = 0;

	string private _baseTokenURI = "ipfs://xyx/";
	string private _contractURI = "ipfs:/yyyh";

    address public devyAddress = 0x4018A57fA542c1281EF781B8914B66505934bA18;

    // sale currency settings
    bool public ethEnabled = true;
    bool public devyEnabled = true;
    
 	// mint settings
	uint256 public maxSupply = 10000;
	uint256 public maxPerMint = 4;
	uint256 public pricePerToken = 1000000000000;
    uint256 public devyPrice = 100000;
    ...
    uint256 public devyPresalePrice = 100000;
    ...
    modifier presaleValid(bytes32 hash, bytes memory sig, uint256 qty) {
		...
		require(qty <= presaleMaxPerMint, "max pet mint amount exceeded");
    ...
    modifier saleValid(uint256 qty) {
		...
		require(qty <= maxPerMint, "max pet mint amount exceeded");
		require(totalSupply + qty <= maxSupply, "out of stock");
        _;
    }    

	constructor(address _signerAddress) ERC721("Haha OMG Eto Visokiy Motiv Tokens", "MDA") {
		signerAddress = _signerAddress;
	}

    function presaleMint(bytes32 hash, bytes memory sig, uint256 qty) external payable nonReentrant presaleValid(hash, sig, qty) {
    ...
    function presaleMintDevy(bytes32 hash, bytes memory sig, uint256 qty, uint256 value) external nonReentrant presaleValid(hash, sig, qty) {
        require(devyEnabled, "DEVY not enabled");
		require(devyPresalePrice * qty == value, "exact amount needed");

        IERC20(devyAddress).transferFrom(msg.sender, address(this), value);
    ...
    function mintDevy(uint256 qty, uint256 value) external nonReentrant saleValid(qty) {
        require(devyEnabled, "DEVY not enabled");
		require(devyPrice * qty == value, "exact amount needed");
    ...
    function changeDevyPrice(uint256 newPrice) external onlyOwner {
		devyPrice = newPrice;
	}
    ...
    function changePresaleDevyPrice(uint256 newPrice) external onlyOwner {
		devyPresalePrice = newPrice;
	}
    ...
    function setPresaleEndDate(uint256 _timestamp) external onlyOwner {
		presaleEndDate = _timestamp;
	}
    ...
    function setDevyEnabled(bool enabled) external onlyOwner {
        devyEnabled = enabled;
    }
