...
contract MessageToMartians is ERC721Enumerable, Ownable {
    ...
    uint constant public maxTokens = 7777;
    ...
    function setFontBase64(string memory fontBase64_) external onlyOwner {
        fontBase64 = fontBase64_;
    }
    ...
    // internal workers
    function stringLength(string memory string_) internal pure returns (uint) {
        return bytes(string_).length;
    }
    function onlyAllowedCharacters(string memory string_) internal view returns (bool) {
        ...
        for (uint i = 0; i < _strLen; i++) {
            ...
            if (_letterBytes1 < _bottomBytes || _letterBytes1 > 0x7A || _letterBytes1 == 0x26) {
                    return false;
            }     
        }
        return true;
    ...
    function tokenURI(uint tokenId_) public view override returns (string memory) {
        ...
        string memory _name = string(abi.encodePacked('{"name": "Message to Martians #', Strings.toString(tokenId_), '",'));
        string memory _image = string(abi.encodePacked('"image":', '"data:image/svg+xml;utf8, ', "<svg xmlns='http://www.w3.org/2000/svg' width='500' height='500' style='background-color:#ffffff'><style>@import url('", fontBase64, "');fill:white;</style><text font-family='Neuton' x='50%' y='50%' dominant-baseline='middle' text-anchor='middle' font-size='32px'>",_wordString,'</text></svg>",'));
        string memory _external_url = string(abi.encodePacked('"external_url": "https://messagetomartians.com",'));
        string memory _description = string(abi.encodePacked('"description": "Message to Martians is a message stored in the ethereum blockchain, awaiting the future Martians to read one day. As a token of appreciation, writers of our message receive an NFT of their written word as well as a Moon Bag for their Martian gear. https://messagetomartians.com",'));
        ...
        _attributes = string(abi.encodePacked(_attributes, '{"trait_type": "length", "value": "', Strings.toString(_wordLength), '"}, {"trait_type": "essay position", "value": "', Strings.toString(tokenId_ + 1), '"}]}'));
