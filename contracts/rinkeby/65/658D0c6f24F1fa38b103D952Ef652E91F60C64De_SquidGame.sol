...
contract SquidGame is Initializable {
    ...
    //nft price change events
    event NftPriceChange(address operator, uint256 value);
    //Fund address change events
    event FundAddressChange(address operator, address value);

    //sync stakeAmounts
    event SyncStakeAmountsEvent(
        address operator,
        uint256 oldVal,
        uint256 newVal
    );
    ...
    //mint batch
    function mint(uint256 times) public Reentrant {
        require(times > 0, "invalid times");
        uint256 amounts = squidNftPrice * times;
        ...
        for (uint256 i = 0; i < times; i++) {
            emit MintEvent(msg.sender, _squidNft.mint(msg.sender));
        }
    }
    ...
    function getFee(uint256 amounts_)
        public
        Reentrant
        calFee
        returns (uint256 fee_)
    {
        ...
        require(amounts_ <= _feeAmounts, "invalid amounts");
        fee_ = amounts_;
        _feeAmounts -= amounts_;
        SQUID.transfer(_recipientAddress, amounts_);
        emit GetFee(msg.sender, amounts_);
    }

    function getFunds(uint256 amounts_)
        public
        Reentrant
        calFee
        returns (uint256 funds_)
    {
        require(msg.sender == _fundsAddress, "funds address invalid");
        require(amounts_ <= _fundsAmounts, "invalid amounts");
        funds_ = amounts_;
        _fundsAmounts -= amounts_;
        SQUID.transfer(_fundsAddress, amounts_);
        emit GetFunds(msg.sender, amounts_);
    }
    ...
    function withdraw(
        ...
        require(stakeAmounts_ >= amount_, "stake insufficient");
        _stakeAmounts = stakeAmounts_ - amount_;
        orders[hash] = true;
        SQUID.transfer(msg.sender, amount_);
    ...
    function setSquidNftPrice(uint256 squidNftPrice_) public onlyOwner {
        require(squidNftPrice_ != 0, "invalid price");
        squidNftPrice = squidNftPrice_;
        emit NftPriceChange(msg.sender, squidNftPrice_);
    }

    function setFundsAddress(address fundsAddress_) public onlyOwner {
        require(fundsAddress_ != address(0), "invalid fundsAddress");
        _fundsAddress = fundsAddress_;
        emit FundAddressChange(msg.sender, fundsAddress_);
    }
    ...
    //Synchronize the number of _stakeAmounts
    function sync()
        public
        calFee
        onlyOwner
        Reentrant
        returns (uint256 stakeAmounts_)
    {
        uint256 balance_ = SQUID.balanceOf(address(this));
        stakeAmounts_ = _stakeAmounts;
        _stakeAmounts =
            balance_ -
            _feeAmounts -
            _rewardAmounts -
            _burnAmounts -
            _fundsAmounts;
        emit SyncStakeAmountsEvent(msg.sender, stakeAmounts_, _stakeAmounts);
    }

    //Return all developer rewards, community funds, juckpot prizes
    function deepSync() public onlyOwner {
        _feeAmounts = 0;
        _fundsAmounts = 0;
        _rewardAmounts = 0;
        sync();
