...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    function getOwner(uint256 id) public view virtual override returns(address) {
        return creators[id];
    }
    ...
    function balanceOf(address account, uint256 id) public view override returns (uint256) {
        require(_exists(id), "ERC1155Metadata: balance query for nonexistent token");
    ...
    function _mint(uint256 _id, uint256 _supply, string memory _uri, uint256 _fee) internal {
        require(!_exists(_id), "ERC1155: token already minted");
        require(_supply != 0, "Supply should be positive");
        require(bytes(_uri).length > 0, "uri should be set");

        creators[_id] = msg.sender;
        _tokenOwners.set(_id, msg.sender);
...
contract BafNft1155 is ERC1155 {
    ...
    function mint(string memory uri, uint256 supply, uint256 fee)  public {
        _mint(tokenId, supply, uri,fee);
        tokenId = tokenId+1;
    }

    function burn(uint256 id, uint256 supply) public {
        _burn(msg.sender, id, supply);
    }
}