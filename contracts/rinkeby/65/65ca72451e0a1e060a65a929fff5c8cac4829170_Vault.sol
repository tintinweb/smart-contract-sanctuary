...
interface ITreasury {
  ...
  function getBalanceOfReserveToken( address reserveToken ) external returns ( uint );
...
contract Vault is ITreasury, Ownable {
  ...
  mapping( address => uint ) public override getBalanceOfReserveToken;
  ...
  function depositReserveToken( address reserveToken_, uint amount_ ) external onlyOwner() onlyReserveToken( reserveToken_ ) returns ( bool ) {
    IERC20( reserveToken_ ).safeTransferFrom( msg.sender, address(this), amount_ );
    getBalanceOfReserveToken[reserveToken_] = getBalanceOfReserveToken[reserveToken_].add( amount_ );
  ...
  function depositPrincipleToken( address principleToken_, address reserveToken_, uint depositAmount_ ) external onlyOwner() onlyPrincipleToken( principleToken_ ) returns ( bool ) {
    IERC20( principleToken_ ).safeTransferFrom( msg.sender, address(this), depositAmount_ );
    uint riskFreeValue_ = IBondingCalculator( getBondingCalculatorForPrincipleTokenForReserveToken[reserveToken_][principleToken_] ).principleValuation( principleToken_, depositAmount_ );
    _updateIntrinsicValueFromReserveDeposit( reserveToken_, riskFreeValue_ );
    getPrincipleTokenBalanceForPrincipleTokenForReserveToken[reserveToken_][principleToken_] = getPrincipleTokenBalanceForPrincipleTokenForReserveToken[reserveToken_][principleToken_].add( riskFreeValue_ );
    return true;
  }
