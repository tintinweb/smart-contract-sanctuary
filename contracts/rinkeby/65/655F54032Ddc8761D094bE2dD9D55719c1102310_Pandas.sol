...
contract Pandas is ERC721Enumerable, Ownable {
    ...
    uint256 public constant MAXPANDAS = 15;
    uint256 public maxPandasPurchase = 5;
    uint256 public maxPreSalePandasPurchase = 2;
    ...
    function preSaleMintPanda(uint256 numberOfPandas) public payable {
        require(isPreSaleActive, "Presale is not active!");
        require(whiteListPreSale[msg.sender], "You are not on the whitelist!");
        require(numberOfPandas >= 0 && balanceOf(msg.sender) + numberOfPandas <= maxPreSalePandasPurchase,
            "You can only mint 2 Pandas in the presale!");
        require(totalSupply().add(numberOfPandas) <= MAXPANDAS - reservedPandas - reservedPandasCustom,
            "Hold up! You would buy more Pandas than available...");
        require(msg.value >= _price.mul(numberOfPandas),
            "Not enough ETH for this purchase!");
    ...
    function addToWhiteList(address[] calldata whiteList) external onlyOwner {
        for (uint256 i = 0; i < whiteList.length; i++) {
            whiteListPreSale[whiteList[i]] = true;
        }
    }

    // function addToWhiteList(address[] memory whiteList) internal onlyOwner {
    //     for (uint256 i = 0; i < whiteList.length; i++) {
    //         whiteListPreSale[whiteList[i]] = true;
    //     }
    // }
    ...
    function setMaxPreSalePandasPurchase(uint256 _maxPreSalePandasPurchase) public onlyOwner {
      maxPreSalePandasPurchase = _maxPreSalePandasPurchase;
    }
