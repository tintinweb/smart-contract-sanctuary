 ...
 */
abstract contract ERC721Marketplace is ERC721Enumerable, Ownable {
    ...
    function settleAuction(uint256 tokenId) external {
        ...
        if (tokenOwner == owner()) {
            IEaselyPayout(payoutContractAddress).splitPayable{ value: auction.price }(tokenOwner, revenueShare, revenueShareBPS);
        } else {
            IEaselyPayout(payoutContractAddress).splitPayable{ value: auction.price }(tokenOwner, _ownerRoyalties(), _ownerBPS());
        }
    ...
    function buyToken(
        ...
        if (seller == owner()) {
            IEaselyPayout(payoutContractAddress).splitPayable{ value: currentPrice }(seller, revenueShare, revenueShareBPS);
        } else {
    ...
    function _setRoyaltiesBPS(uint256 newBPS) internal {
        require(ownerRoyaltyBPS <= maxSecondaryBPS, "Cannot take more than 10% of secondaries");
    ...
    function _setRevenueShare(address[2] memory newAddresses, uint256[2] memory newBPS) internal {
        require(newBPS[0] + newBPS[1] <= maxRoyaltiesBPS, "Revenue share too high");
...
contract ERC721RandomizedCollection is ERC721Marketplace {  
    ...
    function mintAllow(
        ...
        payoutContract.splitPayable{ value: totalPrice }(owner(), revenueShare, revenueShareBPS);
