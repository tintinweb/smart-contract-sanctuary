...
contract Monkes is ERC721Enumerable, Ownable {
  ...
  modifier monkeOwner(uint256 _monkeId) {
    require(
      ownerOf(_monkeId) == msg.sender,
      "Monke does not belong to sender"
    );
    _;
  }

  constructor(string memory baseURI) ERC721("Monkes", "MONKE") {
    setBaseURI(baseURI);
  }
  ...
  function mint(uint256 num) public payable {
    ...
    require(num < 3, "Max mint amount exceeded");
    ...
    monkeOwner(_parent2)
  {
    ...
    bananContract.burn(msg.sender, 20);
...
contract Banan is ERC20, Ownable {
  ...
  function getPendingTokens(address _user) public view returns (uint256) {
    ...
    require(ownedMonkes.length > 0, "No monkes owned");
