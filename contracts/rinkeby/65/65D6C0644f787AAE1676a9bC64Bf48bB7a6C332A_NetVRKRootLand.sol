    ...
    ContextMixin
{
    ...
    function setParcelMinter(address _minterAddress) external onlyOwner {
    ...
    function _beforeTokenTransfer(
        ...
        if (
            parcelMinter.allocatedParcels(tokenId) != address(0) &&
            parcelMinter.allocatedParcels(tokenId) != auctionAddress &&
            from != address(0)
        ) {
            require(
                block.timestamp > (startDate + 24 days),
                "NetVRKRootLand: Locked for 24 days"
            );
        }
    }
}
