...
contract Upgradeable is ERC20Token, IERC20Upgradeable {
    ...
    event Legacy(address oldContract);
    ...
    function legacyContract(address oldContract) public onlyUpgrader {
        _legacyContract = oldContract;
        emit Legacy(oldContract);
    }
    ...
    function upgradedContract(address newContract) public onlyUpgrader {
        _upgraded = true;
    ...
    function transferByLegacy(address caller, address recipient, uint256 amount) override public onlyLegacy {
        _transfer(caller, recipient, amount);
    }

    function approveByLegacy(address caller, address spender, uint256 amount) override public onlyLegacy {
        _approve(caller, spender, amount);
    }
    
    function transferFromByLegacy(address caller, address sender, address recipient, uint256 amount) override public onlyLegacy {
