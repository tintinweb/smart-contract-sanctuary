...
contract REFLECT is IERC20, Context {
    ...
    uint256 public _previousReflectionFee = 0;
    uint256 public _previousBurnFee = 0;
    uint256 public _previousDonationFee = 0;
    ...
    constructor () public {
        ...
        excludeAccountFromReflection(_holdingAddress);
        excludeAccountFromReflection(_burnAddress);
        excludeAccountFromReflection(_donationAddress);
    ...
    function transfer(address recipient, uint256 amount) public override postTGE() blockTGEMembers() pausable() returns (bool) {
    ...
    function approve(address spender, uint256 amount) public override postTGE() blockTGEMembers() pausable() returns (bool) {
        _approve(_msgSender(), spender, amount);
        return true;
    }

    function transferFrom(address sender, address recipient, uint256 amount) public override postTGE() blockTGEMembers() pausable() returns (bool) {
        _transfer(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "ERC20: transfer amount exceeds allowance"));
        return true;
    }

    function increaseAllowance(address spender, uint256 addedValue) public virtual postTGE() blockTGEMembers() pausable() returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender] + addedValue);
        return true;
    }

    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual postTGE() blockTGEMembers() pausable() returns (bool) {
    ...
    function reflect(uint256 tAmount) public postTGE() blockTGEMembers() pausable(){
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256) {
        uint256 totalFees = _reflectionFee + _burnFee + _donationFee;
    ...
    function isFF(address accountToCheck) public view returns(bool) {
        for(uint256 i = 0; i < _ffList.length; i ++){
            if(_ffList[i] == accountToCheck){
                return true;
            }
        }
        return false;
    ...
    function unblockFF() public onlyAdmin() {
        _blockFF = false;
    }
    ...
    function setReflectionFee(uint256 newFee) public onlyAdmin() {
        _reflectionFee = newFee;
    }
    
    function setBurnFee(uint256 newFee) public onlyAdmin() {
        _burnFee = newFee;
    }
    ...
    modifier blockTGEMembers() {
        if(_blockFF){
            require(!isFF(_msgSender()), "Friends and family cannot transfer yet");
        }
        if(_blockInvestors) {
            require(!isInvestor(_msgSender()), "Early investores cannot transfer yet");
        }
        _;
    }
    ...
    function manageTrading(bool newState) public onlyAdmin() {
        _tradingPaused = newState;
    }
    ...
    function completeTGE(uint256 tokenAmount) public payable onlyAdmin() {
        ...
        if(balanceOf(_holdingAddress) > targetAmount) {
            _transferBothExcluded(_holdingAddress, _burnAddress, balanceOf(_holdingAddress) - targetAmount);
        }
        _tgeComplete = true;
        ...
        _transferBothExcluded(_holdingAddress, address(this), tokenAmount);
