...
contract Moon is ERC721Enumerable, ReentrancyGuard, Ownable {
    // NFT upper limit
    ...
    string[] private signNamesElements =[
        unicode"夏日炎天日最长，人人愁热闷非常。天地也解知人意，薰风拂拂自然凉。",
        unicode"金乌西坠兔东升，日夜循环至古今。僧道得之无不利，士农工商各从心。",
        unicode"茂林松柏正兴旺，雨雪风霜总莫为。异日忽然成大用，功名成就栋梁材。",
        unicode"开天辟地作良缘，吉日良时万物全。若得此签非小可，人行忠正帝王宣。",
        unicode"千年古镜复重圆，女再求夫男再婚。自此门庭重改换，更添福禄在儿孙。",
        unicode"欲求胜事可非常，争奈亲姻日暂忙。到头竟必成鹿箭，贵人指引贵人乡。",
        unicode"否去泰来咫尺间，暂交君子出于山。若逢虎兔佳音信，立志忙中事即闲。",
        unicode"锦上添花色愈鲜，运来禄马喜双全。时人莫恨功名晚，一举登科四海传。",
        unicode"阴阳道合总由天，女嫁男婚喜偶然。但见龙蛇相会合，熊熊入梦喜团圆。",
        unicode"君皇圣后总为恩，复待祈禳无损增。一切有情皆受用，人间天上得期亨。",
        unicode"天地变通万物全，自荣自养自安然。生罗万象皆精彩，事事如心谢圣贤。",
        unicode"温柔自古胜刚强，积善之门大吉昌。若是有人占此卦，宛如正渴遇琼浆。"
    ];
    ...
    function getSignNamesElements(uint256 tokenId) public view returns (string memory) {
        ...
        return string(signNamesElements[greatness]);
    }
    ...
    function getSign(uint256 tokenId) private view returns (uint256) {
        ...
        if (greatness < 2) {
            return 0;
        }else if (greatness <12 && greatness > 1) {  
            return 1;  
        }else if (greatness < 42 && greatness > 11){
            return 2; 
        } else {
            return 3;
        }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory moon = toString(getPhaseOfMoon(tokenId));
    ...
    constructor() ERC721("Moon", "MOON") Ownable() {}
}
