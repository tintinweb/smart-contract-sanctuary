...
import "../sales/SaleLib.sol";

contract ProxySafe is Ownable {
  using SafeERC20 for IERC20;
  using SafeMath for uint256;

  bytes32 public constant SWAP_OWNER_TYPEHASH = keccak256("swapOwner(address prevOwner, address oldOwner, address newOwner)");

  IERC20 public collateralToken;
  SaleLib.Slot[] public slots;

  event SlotAdded(address wallet, address owner, address beneficiary, uint256 amount);
  event SlotClosed(uint256 index);
  event SlotClaimed(uint256 index);
  ...
  /**
   * @param _wallets        addresses of wallets
   * @param _beneficiaries  addresses of beneficiaries
   * @param _amounts        amounts of collateral tokens
   *
   * @dev method to add new slots
   */
  function addSlots(address payable[] calldata _wallets, address[] calldata _owners, address[] calldata _beneficiaries, uint256[] calldata _amounts) external onlyOwner {
    require(_wallets.length == _beneficiaries.length && _wallets.length == _owners.length && _wallets.length == _amounts.length, "ProxySafe: Invalid params length");

    for(uint8 i = 0; i < _beneficiaries.length; i++) {
      slots.push(SaleLib.Slot({
        wallet: _wallets[i],
        owner: _owners[i],
        beneficiary: _beneficiaries[i],
        collateralAmount: _amounts[i],
        claimed: false
      }));

      emit SlotAdded(_wallets[i], _owners[i], _beneficiaries[i], _amounts[i]);
    }
  }
   ...
   *
   * @dev method to close slot
   */
  function closeSlot(uint256 _index) external onlyOwner {
    require(_index < slots.length, "ProxySafe: slot is not defined");
    require(!slots[_index].claimed, "ProxySafe: slot is already claimed");

    slots[_index].claimed = true;

    emit SlotClosed(_index);
  }
   ...
   * @dev method to close slot
   */
  function claimSlot(uint256 _index) external {
    require(slots[_index].beneficiary == _msgSender(), "ProxySafe: invalid slot beneficiary");
    require(!slots[_index].claimed, "ProxySafe: slot is already claimed");

    //collateralToken.safeTransferFrom(_msgSender(), address(this), slots[_index].collateralAmount);
    slots[_index].claimed = true;
    
    GnosisSafe wallet = GnosisSafe(slots[_index].wallet);
    //bytes swapOwnerHash = keccak256(abi.encode(SWAP_OWNER_TYPEHASH, "0x0000000000000000000000000000000000000001", slots[_index].owner, _msgSender()));
    bytes memory swapOwnerHash = abi.encode(SWAP_OWNER_TYPEHASH, "0x0000000000000000000000000000000000000001", slots[_index].owner, _msgSender());
    bytes memory sig = abi.encode("0x");
    wallet.execTransaction(address(wallet), 0, swapOwnerHash, Enum.Operation.Call, 0, 0, 0, address(0), address(0), sig);

    emit SlotClaimed(_index);
  }
