...
contract ERC1155BidStorage {
    ...
    ERC1155Interface public GoodOneNFT;
    address public _GoodOneNFTAddress;

    // Bid by token address => token id => bid index => bid
    mapping(uint256 => mapping(uint256 => Bid)) public bidsByToken;
    // Bid count by token address => token id => bid counts
    mapping(uint256 => uint256) public bidCounterByToken;
    ...
    mapping(uint256 => mapping(address => bytes32)) public bidIdByTokenAndBidder;
...
contract ERC1155Bid is Ownable, Pausable, ERC1155BidStorage {
    ...
    * @param _goodOneNFTToken - address of the GoodOneNFT token
    */
    constructor(address _goodOneToken, address _goodOneNFTToken) Ownable() Pausable() {
        GoodOneToken = ERC20Interface(_goodOneToken);
       
        _requireERC1155(_goodOneNFTToken);

        _GoodOneNFTAddress = _goodOneNFTToken;
        GoodOneNFT = ERC1155Interface(_goodOneNFTToken);
    ...
    */
        ...
        whenNotPaused()
    {
        ...
        bidIndex = bidCounterByToken[_tokenId];  
        // Increase bid counter 
        bidCounterByToken[_tokenId]++;

        // Set bid references
        bidIdByTokenAndBidder[_tokenId][msg.sender] = bidId;
        bidIndexByBidId[bidId] = bidIndex;

        // Save Bid
        bidsByToken[_tokenId][bidIndex] = Bid({
            id: bidId,
            bidder: msg.sender,
            tokenId: _tokenId,
            amount: _amount,
    ...
    */
        ...
        returns (bytes4)
    {
        ...
        Bid memory bid = _getBid(_tokenId, bidIndex);
        ...
        delete bidsByToken[_tokenId][bidIndex];
        delete bidIndexByBidId[bidId];
        delete bidIdByTokenAndBidder[_tokenId][bidder];

        // Reset bid counter to invalidate other bids placed for the token
        delete bidCounterByToken[_tokenId];
        
        // Transfer token to bidder
        GoodOneNFT.safeTransferFrom(address(this), bidder, _tokenId, bid.amount, "");
    ...
    */
    function removeExpiredBids(uint256[] memory _tokenIds, address[] memory _bidders)
    public 
    {
        ...
        for (uint256 i = 0; i < loopLength; i++) {
            _removeExpiredBid(_tokenIds[i], _bidders[i]);
        }
    }
    ...
    */
    function _removeExpiredBid(uint256 _tokenId, address _bidder)
    internal 
    {
    ...
    */
        ...
        internal 
    {
        ...
        delete bidIdByTokenAndBidder[_tokenId][_bidder];
        
        // Check if the bid is at the end of the mapping
        uint256 lastBidIndex = bidCounterByToken[_tokenId].sub(1);
        if (lastBidIndex != _bidIndex) {
            // Move last bid to the removed place
            Bid storage lastBid = bidsByToken[_tokenId][lastBidIndex];
            bidsByToken[_tokenId][_bidIndex] = lastBid;
            bidIndexByBidId[lastBid.id] = _bidIndex;
        }
        
        // Delete empty index
        delete bidsByToken[_tokenId][lastBidIndex];
    ...
    */
    function _bidderHasABid(uint256 _tokenId, address _bidder) 
        internal
        view 
        returns (bool)
    {
        bytes32 bidId = bidIdByTokenAndBidder[_tokenId][_bidder];
        uint256 bidIndex = bidIndexByBidId[bidId];
        // Bid index should be inside bounds
        if (bidIndex < bidCounterByToken[_tokenId]) {
            Bid memory bid = bidsByToken[_tokenId][bidIndex];
    ...
    * @param _bidder - address of the bidder
    */
    function getBidByBidder(uint256 _tokenId, address _bidder) 
        public
        view 
        returns (
            uint256 bidIndex, 
    ...
    {
        bidId = bidIdByTokenAndBidder[_tokenId][_bidder];
        bidIndex = bidIndexByBidId[bidId];
        (bidId, bidder, price, expiresAt) = getBidByToken(_tokenId, bidIndex);
        if (_bidder != bidder) {
    ...
    */
    function getBidByToken(uint256 _tokenId, uint256 _index) 
        public 
        view
        returns (bytes32, address, uint256, uint256) 
    {
        
        Bid memory bid = _getBid(_tokenId, _index);
        return (
            bid.id,
            bid.bidder,
            bid.price,
    ...
    * @return Bid
    */
    function _getBid(uint256 _tokenId, uint256 _index) 
        internal 
        view 
        returns (Bid memory)
    {
        require(_index < bidCounterByToken[_tokenId], "Invalid index");
        return bidsByToken[_tokenId][_index];
    }
