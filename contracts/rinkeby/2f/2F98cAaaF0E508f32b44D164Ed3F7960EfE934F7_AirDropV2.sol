...
contract AirDropV2 is ERC721URIStorageUpgradeable, OwnableUpgradeable {
    ...
    */
    ...
    function airDropNft(address[400] calldata _recipients, string memory _uri) onlyOwner external returns(uint256[] memory){
        ...
        for (uint i = 0; i < _recipients.length; i++) {
            ...
            if(_recipients[i] != address(0)){
                ...
                uri = uriConcate(_uri, newItemId, ".json"); 
                _setTokenURI(newItemId, uri); // set uri to new seed
                tokenIds.push(newItemId);
                tokenUris.push(uri);
                _transfer(msg.sender, _recipients[i], newItemId);
