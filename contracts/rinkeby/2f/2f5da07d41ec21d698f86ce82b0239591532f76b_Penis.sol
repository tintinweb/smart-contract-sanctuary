...
contract Penis is ERC2981, ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    function changeTokenPrice(uint256 newTokenPriceWei) public onlyOwner {
        require(newTokenPriceWei >= 0);
        _tokenPriceWei = newTokenPriceWei;
    }

    function changeRoyaltyPercentage(uint256 newRoyaltyPercentage)
        public
        onlyOwner
    {
        require(newRoyaltyPercentage >= 0);
    ...
    function mint(uint256 numberOfTokens, address destinationAddress)
        public
        payable
    {
        require(
            numberOfTokens > 0,
            "Cannot mint 0 or a negative amount of tokens"
        );
        require(
            numberOfTokens <= _tokenMaxSimultaneousPurchaseAmount,
            "Cannot mint so many tokens at the same time"
        );
        require(_saleIsActive, "Sale must be active to mint tokens");
        require(
            (totalSupply() + numberOfTokens) < _maxTokenSupply,
            "Purchase would exceed max supply of tokens"
        );

        if (msg.sender != owner()) {
            require(
                (_tokenPriceWei * numberOfTokens) <= msg.value,
    ...
    {
        Receiver = owner();
        RoyaltyAmount = ((salePrice * _royaltyPercentage) / 100).max(0);
    }
}