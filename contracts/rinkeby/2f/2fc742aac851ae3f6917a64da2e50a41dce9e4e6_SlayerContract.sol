...
/// @notice This contract mints Slayer for KaijuQueen holders and provides
/// administrative functions to the DAO. It allows:
/// * bloot holders to claim Slayer tokens
/// * A DAO to set seasons for new opportunities to claim Slayer
/// * A DAO to mint Slayer for use within the Kaiju Queens ecosystem
/// @custom:unaudited This contract has not been audited. Use at your own risk.
contract SlayerContract is Context, Ownable, ERC20 {
    // bloot contract is available at https://etherscan.io/address/0xff9c1b15b16263c61d017ee9f65c50e4ae0113d7
    address public kaijuQueensContractAddress =
        0x1B2c67d4F30ab26f6243B28d15efd6608ED22Ca0;
    IERC721Enumerable public kaijuQueensContract;

    // Give out 10,000 Slayer Tokens for every Queen that a user holds
    uint256 public SlayerPerTokenId = 10000 * (10**decimals());    
    ...
    constructor() Ownable() ERC20("Slayer", "SLAY") {
        ...
        kaijuQueensContract = IERC721Enumerable(kaijuQueensContractAddress);
    }

    /// @notice Claim Slayer for a given Queen ID
    /// @param tokenId The tokenId of the Queen NFT
    function claimById(uint256 tokenId) external {
        ...
        require(
            _msgSender() == kaijuQueensContract.ownerOf(tokenId),
            "MUST_OWN_TOKEN_ID"
        );
    ...
    /// @notice Claim Slayer for all tokens owned by the sender
    ...
    function claimAllForOwner() external {
        uint256 tokenBalanceOwner = kaijuQueensContract.balanceOf(_msgSender());
        ...
        for (uint256 i = 0; i < tokenBalanceOwner; i++) {
            ...
            // the _claim() function
            _claim(
                kaijuQueensContract.tokenOfOwnerByIndex(_msgSender(), i),
                _msgSender()
            );
        }
    }

    /// @notice Claim Slayer for all Queens owned by the sender within a
    /// given range
    ...
    /// @dev Internal function to mint Slayer upon claiming
    function _claim(uint256 tokenId, address tokenOwner) internal {
        ...
        // Check that Slayer have not already been claimed this season
        // for a given tokenId
        require(
        ...
        // Mark that Slayer has been claimed for this season for the
        // given tokenId
        seasonClaimedByTokenId[season][tokenId] = true;

        // Interactions

        // Send Slayer to the owner of the token ID
        _mint(tokenOwner, SlayerPerTokenId);
    }

    /// @notice Allows the DAO to mint new tokens for use within the Kaiju Queens
    /// Ecosystem
    /// @param amountDisplayValue The amount of Slayer to mint. This should be
    /// input as the display value, not in raw decimals. 
    function daoMint(uint256 amountDisplayValue) external onlyOwner {
        _mint(owner(), amountDisplayValue * (10**decimals()));
    }

    /// @notice Allows the DAO to set a new contract address for Kaiju Queens. This is
    /// relevant in the event that Kaiju Queens migrates to a new contract.
    /// @param kaijuQueensContractAddress_ The new contract address for KaijuQueens
    function daoSetKaijuQueensContractAddress(address kaijuQueensContractAddress_)
        external
        onlyOwner
    {
        kaijuQueensContractAddress = kaijuQueensContractAddress_;
        kaijuQueensContract = IERC721Enumerable(kaijuQueensContractAddress);
    }
    ...
    /// @notice Allows the DAO to set a season for new Slayer claims
    /// @param season_ The season to use for claiming bloot
    function daoSetSeason(uint256 season_) public onlyOwner {
        season = season_;
    }

    /// @notice Allows the DAO to set the amount of Slayer that is
    /// claimed per token ID
    /// @param SlayerDisplayValue The amount of bloot a user can claim.
    /// This should be input as the display value, not in raw decimals. 
    function daoSetSlayerPerTokenId(uint256 SlayerDisplayValue)
        public
        onlyOwner
    {
        SlayerPerTokenId = SlayerDisplayValue * (10**decimals());
    }

    /// @notice Allows the DAO to set the season and Slayer per token ID
    /// in one transaction. This ensures that there is not a gap where a user
    /// can claim more Slayer than others
    /// @param season_ The season to use for claiming bloot
    /// @param SlayerDisplayValue The amount of bloot a user can claim.
    /// This should be input as the display value, not in raw decimals. 
    /// @dev We would save a tiny amount of gas by modifying the season and
    /// Slayer variables directly. It is better practice for security,
    ...
    function daoSetSeasonAndSlayerPerTokenID(
        uint256 season_,
        uint256 SlayerDisplayValue
    ) external onlyOwner {
        daoSetSeason(season_);
        daoSetSlayerPerTokenId(SlayerDisplayValue);
    }
}