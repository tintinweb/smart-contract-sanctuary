...
contract CustomWagersController is Ownable {
    ...
    modifier checkBalance (uint256 wagerValue) {
        require (wagerValue >= admin.getMinWagerAmount(), "The value should be bigger than min wager amount");
        require (rewards.getUnlockedEthBalance(msg.sender) + msg.value >= wagerValue, "It should be bigger than wager value");
        _;
    }      
    
    modifier notPaused() {
        require (!mevu.getContractPaused(), "mevu paused");
        _;
    }
    ...
    modifier notMade (bytes32 wagerId) {        
        require (customWagers.getMaker(wagerId) == address(0), "It has been already made");
        _;
    }
    ...
    modifier validVote (uint256 vote) {
        ...
        payable
    {
        ...
        address payable mevuAddress = payable(address(mevu));
        mevuAddress.transfer(msg.value);
        ...
        payable
    {
        ...
        uint256 expectedValue = customWagers.getOrigValue(id) / (customWagers.getOdds(id) / 100);
        ...
        rewards.addEth(msg.sender, msg.value);
        customWagers.setTaker(id, payable(msg.sender));
    ...
    function submitJudgeVote (bytes32 wagerId, uint256 vote)
        mustBeEnded(wagerId)
        notSettled(wagerId)
        validVote(vote)
        external                       
    {
        ...
        uint256 origValue = customWagers.getOrigValue(wagerId);
        address payable maker = customWagers.getMaker(wagerId);
        address payable taker = customWagers.getTaker(wagerId);
        uint256 payoutValue = customWagers.getWinningValue(wagerId);
        uint256 judgeFee = (payoutValue/100);
        uint256 fee = judgeFee * 3; // Sevice fee is 3 percent, 1 percent goes to judge
        rewards.subEth(maker, origValue);                
        rewards.subEth(taker, payoutValue - origValue);   
        if (vote == 3) {
            tieJudged(payable(msg.sender), maker, taker, origValue, payoutValue);
        } 
        else {
            ...
            mevu.transferEth(payable(msg.sender), judgeFee);               
    ...
    function settle(bytes32 wagerId) internal {
        ...
        uint256 origValue = customWagers.getOrigValue(wagerId);
        uint256 payoutValue = customWagers.getWinningValue(wagerId); 
        uint256 fee = (payoutValue/100) * 2; // Sevice fee is 2 percent
        uint256 makerWinVote = customWagers.getMakerWinVote(wagerId);
        payoutValue -= fee; 
        ...
        if (makerWinVote == customWagers.getTakerWinVote(wagerId)) {
            if (makerWinVote == customWagers.getMakerChoice(wagerId)) {
                ...
                if (makerWinVote == 3) {
                    customWagers.setWinner(wagerId, payable(address(0)));                    
                } else {
                    rewards.subEth(taker, customWagers.getWinningValue(wagerId) - origValue);
                    customWagers.setWinner(wagerId, taker);                   
                    rewards.subEth(maker, origValue);
    ...
    function tieJudged(address payable judge, address payable maker, address payable taker, uint256 origValue, uint256 payoutValue ) internal {
        // give judge 1 percent and reimburse all else
        uint256 judgeFee = payoutValue/100;
        uint256 makerRefund = origValue - (payoutValue/200);
        uint256 takerRefund = (payoutValue-origValue) - (payoutValue/200);
    ...
    function payout(bytes32 wagerId, address maker, address taker, uint256 payoutValue, bool agreed) internal {  
        require(!customWagers.getSettled(wagerId));
    ...
    { 
        ...
        mevu.transferEth(payable(msg.sender), eth);         
    }        
