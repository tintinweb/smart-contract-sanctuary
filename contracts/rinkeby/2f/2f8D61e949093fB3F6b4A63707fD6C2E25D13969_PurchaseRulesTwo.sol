...
contract Pixel is Context, AccessControlEnumerable, ERC721Enumerable, ERC721Burnable, ERC721Pausable {
    ...
    //    mapping(uint => uint256) private pixelPrice;
    //    mapping(uint => bool) private auction;

    constructor(I1MIL _mil1) ERC721('Pixel', 'PXL') {
    ...
    function mint(address to, uint256 tokenId) public virtual {
        ...
        for (uint i = 0; i < reservePixels.length; i++) {
            ...
            && reservePixels[i].owner != tx.origin) {
                require(dotInRectangle(reservePixels[i], x, y), 'Already reserved another user');
                require(dotInRectangle(reservePixels[i], x, y), 'Already reserved another user');
                require(dotInRectangle(reservePixels[i], x, y), 'Already reserved another user');
                require(dotInRectangle(reservePixels[i], x, y), 'Already reserved another user');
            }
    ...
    function reserve(uint xTopLeft, uint yTopLeft, uint xBottomRight, uint yBottomRight) public {
        ...
        for (uint i = 0; i < reservePixels.length; i++) {
            if (block.timestamp <= reservePixels[i].timestamp + 30 * 1 minutes) {
                require(dotInRectangle(reservePixels[i], xTopLeft, yTopLeft), 'Already reserved another user');
                require(dotInRectangle(reservePixels[i], xTopLeft, yBottomRight), 'Already reserved another user');
                require(dotInRectangle(reservePixels[i], xBottomRight, yBottomRight), 'Already reserved another user');
                require(dotInRectangle(reservePixels[i], xBottomRight, yTopLeft), 'Already reserved another user');
                require(dotInRectangle(rectangle, reservePixels[i].xBottomRight, reservePixels[i].yTopLeft), 'Already reserved another user');
                require(dotInRectangle(rectangle, reservePixels[i].xBottomRight, reservePixels[i].yBottomRight), 'Already reserved another user');
                require(dotInRectangle(rectangle, reservePixels[i].xTopLeft, reservePixels[i].yBottomRight), 'Already reserved another user');
                require(dotInRectangle(rectangle, reservePixels[i].xTopLeft, reservePixels[i].yTopLeft), 'Already reserved another user');
            } else {
                needClean.push(i);
            }
        }
        ...
        if (needClean.length > 0) {
            for (uint i = needClean.length - 1; i >= 0; i--) {
                reservePixels[needClean[i]] = reservePixels[reservePixels.length - 1];
                reservePixels.pop();
            }
        }
...
contract PrivatRound {
    ...
    0xbce8bCEeF7bBA623067cd224691d86bbDE83b41E,
    0x40394c0F96c3cfAAcA7235776d1b01Cc4Df5c4b9,
    0x3d0c8228A0fF92300728cDFEd23fB378669da4E8,
    0xBfe663805129915942980bC86BD832aB031Bb2f9,
    0xB32FC7B40a89e0547bD16664a57fcFCeECbeEeEC,
    ...
    0xCbB74E8eAbCD36B160D1fC3BEd7bc6E52D327632,
    0x7c779bF7c2c8f405Af2D3426967B34EC818E43f5,
    0x08Bb489712a7ce446b3D20E1463793E99761c576,
    0x121D26685013baf726e309F5762ecEe520Fcc702,
    0xaADA0f64aA9e3Fa0461eF5efAcD1D879D5e66848,
    0xa44A524DEd85efCD0a671771327b5e75B0Fe6964,
    0xeB3Ed720c708152065d9119E63C3D7e727CFA789,
    0xb0dcE0A78E7602Ca791fFd8a6A9Aa86D4375452b,
    0x4f70eD6b19cc733D5C45A40250227C0c020Ab3cD,
    0xBa172e6BA2Adf181F18fcb698B77F3f9b5531F45,
    ...
    0x2fb8bd9D8D50acc377E2629d124C7dcD0388ab24,
    0xAB598751C57ca1b800e2c10D7E72d47314350f9A,
    0xa6A92391401C8188631784F608F9AfbA6E1B6cfA,
    0x716613bD3048fe4561CbC1537b078462449692Ac,
    0x2539Db6d5d7f31B7a05D043C96aAeBc2077f8f07,
    0x778C029675d3e2435Cf4C207E981D37c2174bec8,
    0x40895b2578041a5C284cD3B49B332d3aADF7162E,
    0x9fdEF1434eB4Aa72A06B90bE155e3Cf233820F6F,
    0x637E21Ac11F0E0Cf95D926d6bd2737b059A2858a,
    0x8888888888E9997E64793849389a8Faf5E8e547C,
    0x725F87a90deff5e0BFdbD0c3cd9178a4563Ab0cf,
    0x7083F5e82BD9B8294b5D0363e5888b7829c050D7,
    0x5796c956DbD81fE32e0475438ff896A46e95D9A2,
    0x54D07CFa91F05Fe3B45d8810feF05705117AFe53,
    0xb7d7B39958CD39C2dBCCbAD85248AD69B6F0D361,
    0x04C89495405538CCF74ee8bD2A3A86F5Af03f563,
    0x5215b81DC977EA9F1237DB57A14066663F899d9A,
    0x54f702696E7da059C31DB1cAf2D552e1E41A5Dd3,
    0xD62B29617F081775f739186C9989bAF66A8c78c6,
    0xa32361Cf8EE6343E67613CF63b046d7D85e21AbB
    ];
    ...
    function addAddress(address _address) public {
        require(owner == msg.sender, "Only owner can add address");
        vipMap[_address] = true;
    }
}
...
import "./1MIL.sol";
import "./Pixel.sol";
import "./PrivatRound.sol";

contract PurchaseRulesTwo {

    MIL1 mil1;
    Pixel pixel;
    Strategists private strategists;
    PrivatRound private privatRound;

    uint256 private price = 10 ** 18;

    constructor(MIL1 _mil1, Pixel _pixel, Strategists _strategists, PrivatRound _privatRound){
        mil1 = _mil1;
        pixel = _pixel;
        strategists = _strategists;
        privatRound = _privatRound;
    }
    ...
    function buy(uint xTopLeft, uint yTopLeft, uint xBottomRight, uint yBottomRight) public {
        ...
        require(strategists.isVip(msg.sender) ||
            privatRound.isVip(msg.sender), "Sorry, only partners can buy pixels today");

        uint numberPixels = (xBottomRight - xTopLeft + 1) * (yBottomRight - yTopLeft + 1);
        uint256 totalPrice = numberPixels * price;
        bool result = mil1.transferFrom(msg.sender, address(this), totalPrice);
