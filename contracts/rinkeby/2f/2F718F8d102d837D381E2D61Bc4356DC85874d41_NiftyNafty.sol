...
contract NiftyNafty is ERC721Enumerable, Ownable {
    ...
    uint256[5] private _currentOwnersTokenId = [0, 250, 500, 750, 1000];

    string private _uri;

    string private _notRevealedUri;

    uint256 public _presalePrice = 50000000000000000;

    uint256 public _mintPrice = 90000000000000000;

    uint256 public _wlMintPrice = 80000000000000000;
   ...
   constructor(address[4] memory owners) Ownable(owners[0]) ERC721(_name, _symbol) {
    ...
    function withdrawETH(uint256 amount, uint256[4] memory _p) public onlyOwner {
        ...
        for (uint i = 0; i < _owners.length; i++) {
            (bool sent, bytes memory data) = payable(_owners[i]).call{value: amounts[i]}("100000");
    ...
    function _incrementTokenId(uint _numTokens) private {
        _currentTokenId += _numTokens;
    }
    ...
    function totalMinted() private view returns (uint256) {
        uint256 result;
        for (uint i = 0; i < _owners.length; i++) {
            result += _currentOwnersTokenId[_ownerId[_owners[i]]];
        }
        return _currentTokenId - 1000 + result;
    }



    function mint(uint256 numTokens) public payable {
        require(_mintedTokens[_msgSender()] + numTokens <= 3, "Amount of tokens exceed allowed amount to mint");
        require((_presaleStartDate > 0 && _mintStartDate > 0 && _wlMintStartDate > 0), "Mint is not started yet");
        require(_currentTokenId + numTokens <= _maxTotalSupply, "Max Total Supply reached");
        require(block.timestamp > _presaleStartDate, "Presale is not started yet");
        uint256 newTokenId = _getNextTokenId();
        uint256 change;
        if (block.timestamp < _mintStartDate) {
            require(msg.value >= _presalePrice * numTokens, "Insufficient ETH amount");
            change = msg.value - _presalePrice * numTokens;
        } else if (_whiteList[_msgSender()]) {
            if (block.timestamp > _wlMintStartDate) {
                require(msg.value >= _wlMintPrice * numTokens, "Insufficient ETH amount");
                change = msg.value - _wlMintPrice * numTokens;
            } else {
                require(msg.value >= _mintPrice * numTokens, "Insufficient ETH amount");
                change = msg.value - _mintPrice * numTokens;
            }
        ...
        _incrementTokenId(numTokens);
        _mintedTokens[_msgSender()] += numTokens;
    ...
    function mintOwner(uint256 numTokens, address to) public {
        require(totalMinted() + numTokens <= _maxTotalSupply, "Max Total Supply reached");
        require(isOwner(_msgSender()), "Unauthorised access");
        require(to != address(0), "Zero address prohibited");
        uint256 newTokenId;
        for (uint i = 0; i < numTokens; i++) {
            if (_currentOwnersTokenId[_ownerId[_msgSender()]] < (_ownerId[_msgSender()] + 1) * 250) {
                newTokenId = _currentOwnersTokenId[_ownerId[_msgSender()]] + 1;
                _safeMint(to, newTokenId);
                _currentOwnersTokenId[_ownerId[_msgSender()]] += 1;
