...
contract UniswapLiquidityManager is PeripheryPayments, IUniswapLiquidityManager {
    ...
    function _distributeFeesInPilot(
        ...
        uint256 tokenAmount = getPilotAmountForTokens(_token0, _token1, _tokensOwed0, _tokensOwed1);
    ...
    function _distributeFeesInTokens(
        ...
        ) = getUserAndIndexShares(_tokensOwed0, _tokensOwed1);
    ...
    function getPilotAmountForWethPair(address tokenAlt, uint256 altAmount, uint256 wethAmount) private view returns(uint256 amount) {
            uint256 pilotAmount0 = IOracle(ORACLE).ethToAsset(PILOT, 3000, wethAmount);
            uint256 pilotAmount1 = getPilotAmount(tokenAlt, altAmount);
            amount = pilotAmount0.add(pilotAmount1);
    }

    function distributeFeesForETH(DistributeFeesParams memory params) public override {
        LiquidityPosition storage position = liquidityPositions[params.pool];
        (address token0, address token1, , , , , ) = getPoolDetails(params.pool);
        (
            address tokenAlt,
            uint256 altAmount,
            address tokenWeth,
            uint256 wethAmount
                ...
                ? (token1, params._tokensOwed1, token0, params._tokensOwed0)
                : (token0, params._tokensOwed0, token1, params._tokensOwed1);

        address ethTokenPair = checkWethPairsAndLiquidity(tokenAlt);

        if (params.pilotToken && ethTokenPair != address(0)) {
            uint256 tokenAmount = getPilotAmountForWethPair(tokenAlt, altAmount, wethAmount);
            unipilot.mintPilot(params.recipient, tokenAmount);

            if (params._tokensOwed0 > 0)
                TransferHelper.safeTransfer(tokenWeth, INDEX_FUND, params._tokensOwed0);
            if (params._tokensOwed1 > 0)
                TransferHelper.safeTransfer(tokenAlt, INDEX_FUND, params._tokensOwed1);

            if (params.wethToken) {
                sweepToken(tokenWeth, 0, params.recipient);
                sweepToken(tokenAlt, 0, params.recipient);
            } else {
                sweepToken(tokenAlt, 0, params.recipient);
                unwrapWETH9(0, params.recipient);
            }

            // indexAmount0 = params._tokensOwed0;
            // indexAmount1 = params._tokensOwed1;
        } else {
            // if pilotToken is true but alt does not found pair with eth
            if (params.wethToken) {
                ...
                (uint256 _indexAmount0, uint256 _indexAmount1, , ) = getUserAndIndexShares(params._tokensOwed0, params._tokensOwed1);

                    uint256 balanceWETH9 = IWETH9(WETH).balanceOf(address(this)); // contains fees plus userLiquidity
                    IWETH9(WETH).withdraw(balanceWETH9);
                    TransferHelper.safeTransferETH(INDEX_FUND, _indexAmount0);
                    TransferHelper.safeTransferETH(params.recipient, address(this).balance); // transfer userFees & liquidity

                    TransferHelper.safeTransfer(tokenAlt, INDEX_FUND, _indexAmount1); // transfer indexAmount 
                    sweepToken(tokenAlt, 0, params.recipient); // remaining alt amount will be transfered to user
            }
        }
        position.fees0 = position.fees0.sub(params._tokensOwed0);
        position.fees1 = position.fees1.sub(params._tokensOwed1);
    }
...
interface IUnipilot is IUnipilotEvents {
    ...
    function collectForETH(bool pilotToken, bool wethToken, address recipient, uint256 tokenId) external payable;
...
contract Unipilot is ERC721Permit, IUnipilot {
    ...
    function withdraw(
        ...
        liquidityManager.removeLiquidityWithFees(true, position.pool, recipient, liquidity);
    ...
    function withdrawForETH(
        ...
        Position storage position = positions[tokenId];
        require(position.liquidity >= liquidity, "UNIPILOT::INSUFFICIENT_LIQUIDITY");
        liquidityManager.removeLiquidityWithFees(false, position.pool, recipient, liquidity);
        ...
        liquidityManager.distributeFeesForETH(
                IUniswapLiquidityManager.DistributeFeesParams({
                    pilotToken: pilotToken,
                    wethToken: wethToken,
                    pool:position.pool,
                    recipient:recipient,
                    _tokensOwed0: position.tokensOwed0,
                    _tokensOwed1: position.tokensOwed1})
            );
    ...
    function collectForETH(bool pilotToken, bool wethToken, address recipient, uint256 tokenId)
        external
        payable
        override
