...
contract SmartLease {
    ...
    /*----------------------------Implement Purchase_car-Option at the termination of an closed-end lease------------------------------s---*/
        ...
        returns (bool)
    {
        if (penalties_have_been_paid == true) {
            ...
            revert(
                "There are still pending penalties that must be paid. The security deposit is not enough to cover this penalties OR The security deposit equals the penalties that must be paid, thus no refund will happen!"
            );
        }
    }
    ...
    function terminate_contract()
        ...
        payable
        only_lessor
        returns (CONTRACT_STATUS)
    {
        if (
            security_deposit_has_been_refunded == true &&
            (call_option_has_been_exercised == true ||
                call_option_has_been_exercised == false)
        ) {
