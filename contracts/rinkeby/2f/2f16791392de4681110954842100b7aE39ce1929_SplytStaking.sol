...
contract SplytStaking is Ownable {
  ...
  struct Compound {
    ...
    uint interestStartBlock; // Block that starts interest gaining
    uint interestEndBlock;   // Block that ends interest gained
  ...
  uint constant blocksPerMonth = 175200; //this is the max amount of blocks to multiply by
  uint constant secondPerBlock = 15;    //Number of seconds between blocks
  uint totalPrincipal;
  uint numUsersWithDeposits;
  uint feeDivisor = 10**3;
