...
contract Forest is Ownable, IERC721Receiver, Pausable, ReentrancyGuard {
   ...
   */
  function addManyToForestAndPack(address account, uint16[] calldata tokenIds) external nonReentrant {
   ...
   */
  function _addBeeToForest(address account, uint256 tokenId) internal whenNotPaused _updateEarnings nonReentrant {
   ...
   */
  function claimManyFromForestAndPack(uint16[] calldata tokenIds, bool unstake) external whenNotPaused _updateEarnings nonReentrant {
    uint256 owed = 0;
    for (uint i = 0; i < tokenIds.length; i++) {
      if (isBee(tokenIds[i]))
   ...
   */
  function _claimBeeFromForest(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
...
//payment splitter 
contract Bear is IBear, ERC721Enumerable, Ownable, Pausable, PaymentSplitter, ReentrancyGuard {
  ...
  constructor(address _honey, address _traits, uint256 _maxTokens, address[] memory treasuryWallets, uint256[] memory treasuryShares) ERC721("Bear Game", 'BGAME')  PaymentSplitter(treasuryWallets, treasuryShares)  { 
