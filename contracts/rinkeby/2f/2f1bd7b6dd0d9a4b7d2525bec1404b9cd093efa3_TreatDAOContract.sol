...
contract TreatDAOContract is ERC20, Ownable {
    ...
    */
    ...
    function calculateTokenFee(
      ...
      //0xa9059cbb is sig for transfer(address,uint256)
      //check if swapping for tax ON.  OR . check if LPing for tax OFF

      //0x23b872dd  transferFrom(address,address,uint256)
      //0xddf252ad  Transfer(address,address,uint256)
      //
      bool isTaxed = false;
      for(uint i=0; i < taxedContracts.length; i++) {
        if(taxedContracts[i] == msg.sender) {
          isTaxed = true;
        }
      }
      if(!isTaxed){
    ...
    address[] public taxedContracts;
    function addTaxedContract(address taxed) public {
      bool isAddedAlready = false;
      for(uint i=0; i < taxedContracts.length; i++) {
        if(taxedContracts[i] == taxed) {
          isAddedAlready = true;
        }
      }

      require(!isAddedAlready, "address is already taxed");
      taxedContracts.push(taxed);
    }
