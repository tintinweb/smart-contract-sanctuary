...
contract NFT is ERC721, PaymentSplitter {
  ...
  function mintSale_kvG(uint amount) external payable onlyEOA {
    ...
    _mintToken_xkg(msg.sender, amount);
  }
  ...
  function mintReserved(address receiver, uint amount) external onlyTeam {
    ...
    _mintToken_xkg(receiver, amount);
  }
  ...
  function mintWhitelist(bytes32[] calldata proof) external payable onlyEOA {
    ...
    _mintToken_xkg(msg.sender, 1);
  }

  function _mintToken_xkg(address to, uint amount) private {
    uint id;
