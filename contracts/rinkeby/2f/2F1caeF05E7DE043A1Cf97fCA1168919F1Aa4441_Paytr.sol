...
contract Paytr{
    ...
    event PaymentInfo(address, uint256, string, address, uint256 ); //To be added for Request: address to, invoice amount, payment reference, supplier, interest amount
    event PaymentInfoErc20(address, uint256, string, address, address );
   ...
   constructor() public {
    ...
    function supplyEthToCompound(address payable _cEtherContract, uint amount, address supplier, uint dueDate, string memory paymentReference)
        public
        payable
        returns (bool)
    {
    ...
    function createDueListAndPay() public payable {
        ...
        for (uint i = 0; i < invoices.length; i++) {
             ...
             if (block.timestamp >= invoices[i].dueDate) {
                ...
                string storage paymentReference = invoices[i].paymentReference;
    ...
    function createDueListAndPayErc20() public payable {
        ...
        for (uint i = 0; i < invoicesErc20.length; i++) {
             ...
             if (block.timestamp >= invoicesErc20[i].dueDate) {
                ...
                string storage paymentReference = invoicesErc20[i].paymentReference;
