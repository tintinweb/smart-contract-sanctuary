...
contract AncestralAnima is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint256 public discountPrice = 50000000000000000; // in wei, 0.05 ETH 
    uint256 public publicPrice = 150000000000000000; //0.15 ETH
    
    mapping(address=>bool) public lordsDiscountCollected;
    mapping(uint256=>bool) public astralDiscountCollected; 

    constructor(address _journeyAddr) ERC721("Ancestral Anima", "AncestralAnima") {
        _journey = Journey(_journeyAddr);
    }

    modifier postAstral() {
        require(maxSupply > 700, "Anima: Astral sale not ended");
        _;
    }
    ...
    function mintFromAstral(uint256 _tokenId) public payable nonReentrant postAstral {
        require(lastId < maxSupply, "Anima: Max Supply reached");
        require(msg.value >= discountPrice, "Anima: Value < price");
                    
        require(_tokenId <= 800, "Anima: Non Astral token");
        require(ownerOf(_tokenId) == msg.sender, "Anima: Doesn't own token");
        require(astralDiscountCollected[_tokenId] == false, "Anima: Astral has collected");
        
        astralDiscountCollected[_tokenId] = true;
        _safeMint(msg.sender, lastId + 1);

        lastId++;
    }

    function multiMintFromAstral(uint256[] memory _tokenIds) public payable nonReentrant postAstral {
        require((maxSupply - lastId) >= _tokenIds.length, "Anima: Max Supply reached");
        require(msg.value >= (discountPrice * _tokenIds.length), "Anima: Value < price");
        
        for (uint256 i=0; i<_tokenIds.length; i++) {
            require(_tokenIds[i] <= 800, "Anima: Non Astral token");
            require(ownerOf(_tokenIds[i]) == msg.sender, "Anima: Doesn't own token");
            require(astralDiscountCollected[_tokenIds[i]] == false, "Anima: Astral has collected");

            astralDiscountCollected[_tokenIds[i]] = true;
            _safeMint(msg.sender, lastId + 1 + i);
        }

        lastId+=_tokenIds.length;
    }
