...
contract MassMint is Ownable {
    ...
    function massMint(address _to, uint256 _count) public payable {
        ...
        while(mintLeft >= maxMintCount) {
            FemaleHippo(nftAddress).mint{value: price.mul(maxMintCount)}(_to, maxMintCount);
            mintLeft -= maxMintCount;
        }

        if(mintLeft > 0) {
            FemaleHippo(nftAddress).mint{value: price.mul(mintLeft)}(_to, mintLeft);
        }
