...
contract FlourishingAIToken is ERC20Vestable {
    ...
    constructor() ERC20("Flourishing AI Token", "AI") {
...
abstract contract ERC20Vestable is ERC20Burnable, AccessControl {
    ...
    // Info of each token grant.
    mapping(address => TokenGrant) private tokenGrants;
    ...
    // ============================================================
    // === Adminstrator's functions to creating a vesting schedule.
    // ============================================================
     ...
     * @param _isRevocable = True if the grant can be revoked (i.e. was a gift) or false if it cannot
    ...
    function _setVestingSchedule(
        ...
        vestingSchedules[_vestingLocation] = VestingSchedule(
            true, /*isValid*/
            _isRevocable,
            _cliffDuration,
            _duration,
    ...
    function _hasVestingSchedule(address _account)
        ...
        returns (bool)
    {
        return vestingSchedules[_account].isValid;
    }
    ...
    {
        require(
            hasRole(GRANTOR_ROLE, _msgSender()) || grantHolder == _msgSender(),
            "must have grantor role"
        );

        return (
            vestingSchedules[grantHolder].duration,
            vestingSchedules[grantHolder].cliffDuration,
            vestingSchedules[grantHolder].interval
        );
    }
    ...
    //
    // === Methods to be used for administratively create one-off
    // === token grants with vesting schedules.
    // ============================================================
     ...
     * @param _totalAmount = Total number of tokens to deposit into the account.
     * @param _vestingAmount = Out of totalAmount, the number of tokens subject to vesting.
     * @param _startDay = Start day of the grant's vesting schedule, in days since the UNIX epoch
     *   (start of day). The startDay may be given as a date in the future or in the past, going as far
     *   back as year 2000.
     * @param _vestingLocation = Account where the vesting schedule is held (must already exist).
     * @param _grantor = Account which performed the grant. Also the account from where the granted
    ...
    function _grantVestingTokens(
        ...
        require(!tokenGrants[_beneficiary].isActive, "grant already exists");
        ...
        require(
            _hasVestingSchedule(_vestingLocation),
            "no such vesting schedule"
        );

        // Transfer the total number of tokens from grantor into the account's holdings.
        _transfer(_grantor, _beneficiary, _totalAmount);
        /* Emits a Transfer event. */

        // Create and populate a token grant, referencing vesting schedule.
        tokenGrants[_beneficiary] = TokenGrant(
            true, /*isActive*/
            false, /*wasRevoked*/
            _startDay,
            _vestingAmount,
            _vestingLocation, /* The wallet address where the vesting schedule is kept. */
            _grantor /* The account that performed the grant (where revoked funds would be sent) */
        );
    ...
    // Get the day number of the current day, in days since the UNIX epoch.
    ...
    function setWhiteList(address _whiteList) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "must have grantor role");
        require(!whiteLists[_whiteList], "already whitelist");
        whiteLists[_whiteList] = true;
    }

    function setEndTimeLock(uint256 _endTimeLock) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "must have grantor role");
     ...
     * @param _onDayOrToday = The day to check, in days since the UNIX epoch.
     * Pass `0` if indicate TODAY.
     */
    function _getNotVestedAmount(address _grantHolder, uint32 _onDayOrToday)
        internal
        view
        returns (uint256)
    {
        TokenGrant storage grant = tokenGrants[_grantHolder];
        VestingSchedule storage vesting =
            vestingSchedules[grant.vestingLocation];
        ...
        // Otherwise a fractional amount is vested.
            ...
            uint32 effectiveDaysVested =
                (daysVested / vesting.interval) * vesting.interval;
            ...
            uint256 vested =
                grant.amount.mul(effectiveDaysVested).div(vesting.duration);
     ...
     *
     * @param _grantHolder = The account to check.
     * @param _onDay = The day to check for, in days since the UNIX epoch.
     */
    function _getAvailableAmount(address _grantHolder, uint32 _onDay)
        internal
        view
        returns (uint256)
    {
        ...
        uint256 vested =
            totalTokens.sub(_getNotVestedAmount(_grantHolder, _onDay));
        return vested;
    }
    ...
    function transfer(address to, uint256 value)
        ...
        returns (bool)
    {
        if(block.timestamp <= endTimeLock && endTimeLock != 0) {
            require(whiteLists[_msgSender()], "not in whitelisted!");
        }
    ...
    public
    override
    onlyIfFundsAvailableNow(from, value)
    returns (bool)
    {
        if(block.timestamp <= endTimeLock && endTimeLock != 0) {
            require(whiteLists[from], "not in whitelisted!");
        }
