...
contract KittyPartyFactory {
    ...
    returns (address kittyAddress) {
        ...
        console.log("%s is the owner",KittyPartyController(kitty).owner() );
        // (bool success, bytes memory returnData) = address(kitty).call(abi.encodeWithSignature("transferOwnership(address)", msg.sender));
        // require(success, "Not able to transfer ownership");
...
enum KittyPartyStages {
    ...
    uint256 public numberOfRounds = 2; // this data is duplicated and is equivalent to number of kittens minimum 2
