...
contract TorCoin is ERC20Burnable, Operator {
    ...
    IUniswapV2Factory public pantherSwapFactory;
    // The trading pair
    ...
    constructor() public ERC20("Tor Coin", "TOR") {
        ...
        pantherSwapRouter = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
        pantherSwapFactory = IUniswapV2Factory(0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f);
    }
        ...
        returns (bool)
    {
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            // );
            uint256 liquidityAmount = amount.mul(transferTaxRate).div(10000);

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(liquidityAmount);
            require(
                amount == sendAmount + liquidityAmount,
    ...
    {
        uint256 selfTokenBalance = balanceOf(address(this));
        uint256 maxTransferAmount = maxTransferAmount();

        selfTokenBalance = selfTokenBalance > maxTransferAmount
            ? maxTransferAmount
            : selfTokenBalance;

        if (selfTokenBalance >= minAmountToLiquify) {
    ...
    function createPantherSwapPair() public onlyOperator {
        pantherSwapPair = pantherSwapFactory.createPair(address(this), pantherSwapRouter.WETH());
    }
