    ...
    Ownable
{
    ...
    function publicMint(
        ...
        require(
            verify(_msgSender(), amount, message, nonce, signature),
            "Signature must be valid to mint."
        );

        require(msg.value == amount, "That is not the right price!");

        require(amount == msg.value, "ETH amount is incorrect");
        mint(_msgSender(), tokenId);
    }

    //The LilNoun-Nouns Virtuous Cycle. Mints lil noun at given tokenId and also - minter willing, mints a nounsDAO lil noun to the DAO for free.
    function publicMintWithGift(
        uint256 tokenId,
        uint256 nounsGiftTokenId,
        uint256 amount,
        string memory message,
        uint256 nonce,
        bytes memory signature
        ...
        require(
            nounsGiftTokenId % 10 == 0,
            "Every 10th Lil Noun is reserved for NounsDAO."
        );

        mint(nounsDAO, nounsGiftTokenId);
    }

    //Anyone can mint an array of 10 nounsDao lil nouns (tokenId % 10 == 0) to nounsDao
    function nounsDaoMint(uint256[] memory tokenIds) public virtual {
        require(!allSalesPaused, "Sales are currently paused");
        require(tokenIds.length < 10, "Can't mint too much at once!");

        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                tokenIds[i] % 10 == 0,
                "Can only mint every 10th Lil Noun reserved for NounsDAO."
            );
