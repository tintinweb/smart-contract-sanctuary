    ...
    controllerPanel
{
    ...
    function buyCard(uint256 _type) public payable saleActive() {
        uint256 _id = generic.getCardTypeNextID(_type);
        uint256 price = getCardPrice(_id);
        require(price != 0, "Price must not be zero");
        uint256 balance = payment_token.allowance(msg.sender, address(this));
        ...
        for (uint256 j = 0; j < 50; j++) {
            _id = generic.getCardTypeNextID(_type);
            if (balance < price || _id == 0) {
                if (j == 0) require(false, "Sold out.");
                //  console.log("Transfering , insufficient. - %s ", totalAmount  / 1 ether );
                require(
                    payment_token.transferFrom(msg.sender, wallet, totalAmount),
                    "Transfer failed"
                );
                return;
            }
            ...
            // console.log("Minting id - %s ", _id);
            _assignCard(_id);
            totalSold++;
        }
        // console.log("Transfering %s ", totalAmount / 1 ether );
        require(
            payment_token.transferFrom(msg.sender, wallet, totalAmount),
            "Transfer failed"
        );
...
interface IEC721 {
    ...
    function getCardTypeNextID(uint256 _cardID) external view returns (uint256);
}
