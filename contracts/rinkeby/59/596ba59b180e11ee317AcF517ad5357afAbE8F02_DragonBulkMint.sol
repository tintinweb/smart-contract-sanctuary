...
contract DragonBulkMint is Ownable {
    constructor() {}
    ...
    function bulkMint(
        ...
        require(
            urls.length == attack.length &&
                urls.length == defense.length &&
                urls.length == speed.length &&
                urls.length == endurance.length,
            "Not equal params length"
        );
        uint ii;
        uint len = urls.length;
        for(ii = 0; ii < len; ii++) {
            IDragonPlayerCharacter(_character).mintCharacter(urls[ii], attack[ii], defense[ii], speed[ii], endurance[ii]);
        }
    }
}
