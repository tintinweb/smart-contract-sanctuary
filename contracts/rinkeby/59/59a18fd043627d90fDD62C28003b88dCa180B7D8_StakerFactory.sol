...
interface IMintShopFactory{
  function createShop(address _owner, address _paymentReceiver, address _farm, uint256 _globalPurchaseLimit ) external returns ( address );
}
...
contract StakerFactory is Ownable, ReentrancyGuard {
  ...
  /// A mapping of a farm address to its corresponding mintshop address
  mapping (address => address[]) public shopMap;

  /// A mapping for an array of user whitelisted from factory fee.
  ...
  /// An event for tracking the manual addition of a new Staker
  event FarmAdded(address indexed farmAddress, address indexed updater);
  ...
  */
  function createFarm(
    ...
    address newMintshop = IMintShopFactory(mintShopFactory).createShop(msg.sender, feeReceiver, stakerAddress, uint(10000));
    
    shopMap[stakerAddress].push(newMintshop);
  ...
  */
  function addFarm(address _farmAddress) external {
    ...
    emit FarmAdded(_farmAddress, msg.sender);
  }

  /**
    Allow a user to create and add a new mintshop contract to the registry.

    @param _farmAddress The address of the Staker to which the mintshop will be added
  */
  function addShopToFarm(address _farmAddress) external {
    require(Staker(_farmAddress).owner() == address(msg.sender), "StakerFactory::addShopToFarm: Must be farm owner");

    // Create a new MintShop contract with the msg.sender set as owner
    address newMintshop = IMintShopFactory(mintShopFactory).createShop( msg.sender, feeReceiver, _farmAddress, uint(10000));
    shopMap[_farmAddress].push(newMintshop);

    emit FarmShopAdded(_farmAddress, newMintshop, msg.sender);
  }
  ...
  */
  ...
  /**
    Change user's whitelist status
    @param _listee The address entry being modified
    @param _status The new whitelist status
  */
  function updateWhitelist(address _listee, bool _status) external onlyOwner {
    whitelisted[_listee] = _status;
  }
}
