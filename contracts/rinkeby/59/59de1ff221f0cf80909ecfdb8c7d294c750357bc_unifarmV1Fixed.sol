...
contract unifarmV1Fixed is Ownable {
    ...
    constructor(address V1Address) Ownable(msg.sender)  {
        ...
        // add totalstaking of all tokens in init function
    }
    ...
    function sendToken(address stakedToken,address tokenAddress,uint256 amount) internal {
        ...
        if (tokenAddress != address(0)) {
            ...
            if (amount > tokenbal) {
               IERC20(tokenAddress).transfer(msg.sender, tokenbal);
               emit Claim(
                msg.sender,
                stakedToken,
                tokenAddress,
    ...
    function unStake(uint256 stakeId) external  {
        ...
        if(unstakeStatus[msg.sender][stakeId] == false && activeStatus[stakeId] == true) 
            unstakeStatus[msg.sender][stakeId] = true;
        else
            revert("Unstaked Already");
            
            
        if((block.timestamp.sub(startTime[stakeId])).div(DAYS) >= 35 days) {
        ...
        uint Total = totalStakingFinal[tokenAddress[stakeId]];
        totalStakingFinal[tokenAddress[stakeId]] = totalStakingFinal[tokenAddress[stakeId]].sub(stakedAmount[stakeId]);
          
        // Transfer staked token back to user
        IERC20(tokenAddress[stakeId]).transfer(msg.sender, stakedAmount[stakeId]);

        // Claim pending rewards
        claimRewards(startTime[stakeId], stakedAmount[stakeId], tokenAddress[stakeId], Total);
