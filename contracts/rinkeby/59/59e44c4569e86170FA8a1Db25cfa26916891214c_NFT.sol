...
contract NFT is ERC721Enumerable, Ownable, Pausable {
    ...
    uint256 private publicSaleSupply = 9111;
    ...
    function preSaleMint(address _to ,uint256 _mintAmount) public payable whenNotPaused{
        ...
        require(supply + _mintAmount <= preSaleSupply, "Supply limit reached!");
        require(_mintAmount > 0, "No amount to mint");
        require(_mintAmount <= 1, "You cant mint more than 10");      
        require(ownedToken[_to] <= 1, "You cant pre-buy more tokens");
        if (msg.sender != owner()) {
        ...
        ownedToken[_to] = ownedToken[_to] + _mintAmount;
        emit PreSaleMint(_to, _mintAmount, msg.value, block.timestamp);
    }

    function publicSaleMint(address _to ,uint256 _mintAmount) public onlyOwner whenNotPaused{
        uint256 supply = totalSupply();
