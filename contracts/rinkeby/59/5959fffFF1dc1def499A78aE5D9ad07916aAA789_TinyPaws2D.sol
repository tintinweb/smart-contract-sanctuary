...
contract TinyPaws2D is ERC721, Ownable {
    ...
    function airdrop1(address[] calldata addresses, uint[] calldata amounts) external onlyController {
        for (uint i = 0; i < addresses.length; i++) {
            mintNFTs(addresses[i], amounts[i]);
        }
    }

    function airdrop1(address[] calldata addresses) external onlyController {
        for (uint i = 0; i < addresses.length; i++) {
