...
contract GovWorldTierLevel is IGovWorldTierLevel, Ownable {
    ...
    function maxTierLevelbyNFTAddresses(address[] memory nftContracts, address _wallet) public view returns (TierData memory tierData) {
        ...
        uint256 max = tierLevels[tierLevelbyNFTContract[nftContracts[0]]].govHoldings;
        uint256 maxIndex = 0;

        for (uint256 i = 0; i < nftContracts.length; i++) {
            uint256 userNFTBalance = IERC721(nftContracts[i]).balanceOf(_wallet);
            require(userNFTBalance > 0, "GTL: NFT Balance Error");
            if (tierLevels[tierLevelbyNFTContract[nftContracts[i]]].govHoldings > max) {
                maxIndex = i;
                max = tierLevels[tierLevelbyNFTContract[nftContracts[i]]].govHoldings;
            }
        }

        return tierLevels[allTierLevelKeys[maxIndex]];
    }
