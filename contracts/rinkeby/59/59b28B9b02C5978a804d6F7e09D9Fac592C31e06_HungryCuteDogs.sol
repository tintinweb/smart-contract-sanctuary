...
contract HungryCuteDogs is ERC721, Ownable, ERC721Enumerable {
    using SafeMath for uint256;
                       using Strings for uint256;

                                         uint256 public constant tokenPrice = 1900000000000000;
    ...
    event HungryCuteDogMinted(uint256 tokenId, address owner);

    constructor() ERC721("HungryCuteDogs","HCT") {}
    ...
    function mintEmoji(uint256 numberOfTokens) external payable {
        ...
        for(uint256 i = 0; i < numberOfTokens; i++) {
            ...
            if(totalSupply() < MAX_TOKENS) {
                ...
                emit HungryCuteDogMinted(id, msg.sender);
            }
        }
    }
    ...
    function presaleEmoji(uint256 numberOfTokens) external payable {
        ...
        for(uint256 i = 0; i < numberOfTokens; i++) {
            uint256 id = totalSupply().add(1);
            if(totalSupply() < MAX_TOKENS) {
                _presaleListClaimed[msg.sender] += i;
                _safeMint(msg.sender, id);
                emit HungryCuteDogMinted(id, msg.sender);
            }
        }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if(isRevealed == false) {
            return "ipfs://QmWb8qR1xGuEuMCBGwP2Q5AtVTBf6ZS1ptDMFEGGLYp5G6/hidden.json";
        }
