...
//contract PlanetMutt is ERC721, ERC721URIStorage, ERC721Enumerable, Pausable, Ownable {
contract PlanetMutt is ERC721, ERC721Enumerable, Pausable, Ownable {
    ...
    uint256 public startingTimestamp =0;
    uint256 public endingTimestamp =0;
    uint256 public endingInterval =345600; // 4-days
    uint256 public endingTripCount = 1009; 

    bool public isLocked;
   
    //event MintToken(address minter, uint256 tokenId);
...
*/    
    function mintMutt(address to) whenNotPaused private onlyWhenUnlocked
    {
            uint256 tokenId = _tokenIdCounter.current();
            _safeMint(to, tokenId);
            _tokenIdCounter.increment();
            if ((tokenId == endingTripCount) && (endingTimestamp == 0))
            {
                // Trip the countdown to end minting
                endingTimestamp = block.timestamp + endingInterval;
            }
            //emit MintToken(to, tokenId);
    }
    
    function buyMutts(uint tokensNumber) whenNotPaused public onlyWhenUnlocked payable {
    ...
    function setEndingIntervalp(uint256 _endingInterval) public onlyOwner
    {
        endingInterval=_endingInterval;
    }
    
 
    function giveAway(address to, uint tokensNumber) public onlyOwner onlyWhenUnlocked {
        require(_tokenIdCounter.current().add(tokensNumber) <= nftsPublicNumber, "Tokens number to mint exceeds number of public tokens");
    ...
    function changeName(uint256 tokenId, string memory newName) public virtual {
        ...
        //require(_tokenIdCounter.current() >= 1000, "Not enough tokens have been minted");
    ...
    modifier onlyWhenLocked() {
        require(isLocked, "Contract must be locked first");
        _;
    }
    ...
    function muttPrice() public view returns (uint256)
    {
        return tokenPrice;
    }
