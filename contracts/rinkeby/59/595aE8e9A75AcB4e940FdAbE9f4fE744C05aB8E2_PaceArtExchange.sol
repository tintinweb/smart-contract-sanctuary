...
contract ExchangeCore is ReentrancyGuarded, Ownable {
        ...
        reentrancyGuard
    {
        ...
        (bool result, bytes memory ret) = AuthenticatedProxy(address(delegateProxy)).proxy(sell.target, sell.howToCall, sell.callData);
        require(result, "Exchange::Failed when call other contract");
        // Transfer Royalty Fee. Prevent stack too deep errors
        uint tokenId = abi.decode(ret, (uint));
        /* Execute funds transfer and pay fees. */
        uint price = executeFundsTransfer(buy, sell, RoyaltiesV2(target).getPaceArtV2Royalties(tokenId));
        /* Log match event. */
