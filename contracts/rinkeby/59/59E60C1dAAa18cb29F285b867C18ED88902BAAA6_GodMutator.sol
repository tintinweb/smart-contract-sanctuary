...
contract BaseMutator {
    Manifest internal callback;

    constructor(Manifest callback_) {
        callback = callback_;
    }

    function _setTokenData(uint256 tokenId, TokenData memory tokenData) internal {
...
contract GodMutator is Ownable, BaseMutator {
    constructor(Manifest callback, address newOwner) BaseMutator(callback) {
        transferOwnership(newOwner);
    }
    ...
    function incrementGeneration(uint256 tokenId) external onlyOwner {
        TokenData memory data = callback.getTokenData(tokenId);
        data.generation++;
