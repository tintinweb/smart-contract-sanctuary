    ...
    function unStake(uint256[] memory _ids) public isOnlyStaker{
        ...
        if(sumofgenesisbear >= 5){
            ...
            for(uint256 i = 0; i < stakes[msg.sender].length; i ++){
                ...
                uint256 claimAmountOfSROOTxToken = dd * 50 / 2;

                uint256 _amountIn = dd * 50 / 2 * (10**18);
                ...
                require(IERC20(SROOTxTokenAddress).balanceOf(address(this)) >= claimAmountOfSROOTxToken * (10 ** 18), "staking pool has insufficient funds");
                IERC20(SROOTxTokenAddress).transfer(msg.sender, claimAmountOfSROOTxToken * (10 ** 18));
    ...
    function claimALL() external isOnlyStaker returns(uint256, uint256, uint256){
        ...
        if(sumofgenesisbear >= 5){
            ...
            for(uint256 i = 0; i < stakes[msg.sender].length; i ++){
                ...
                uint256 claimAmountOfSROOTxToken = dd * 50 / 2;
                finalOfSROOTxToken += claimAmountOfSROOTxToken;
                uint256 _amountIn = finalOfSROOTxToken*(10**18);
                uint256 _amountOutMin = 0;
                ...
                finalOfWETH += _amountOutMin;
                require(IERC20(SROOTxTokenAddress).balanceOf(address(this)) >= claimAmountOfSROOTxToken * (10 ** 18), "SROOTxToken: staking pool has insufficient funds");
                IERC20(SROOTxTokenAddress).transfer(msg.sender, claimAmountOfSROOTxToken * (10 ** 18));
                stakes[msg.sender][i].claimedDate = block.timestamp;
            }
    ...
    function claimOf(address _addr) public view returns(uint256, uint256,uint256) {
        ...
        if(sumofgenesisbear >= 5){
            ...
            for(uint256 i = 0; i < stakes[_addr].length; i ++){
                ...
                claimAmountOfSROOTxToken += dd * 50 / 2;
                uint256 _amountIn = claimAmountOfSROOTxToken;
                if(_amountIn == 0) continue;
                uint256 _amountOutMin = getAmountOutMin(SROOTxTokenAddress, WETH, _amountIn*(10**18));
        ...
        return (claimAmountOfROOTxToken * (10 ** 18), claimAmountOfSROOTxToken *  (10 ** 18), claimAmountOfWETH);
    }
