...
// File contracts/tools/ClaimTool.sol
...
contract ClaimTool is Ownable {
    ...
    function _pending(IClaimable claimable, address user, uint256 startCycle, uint256 endCycle) internal view returns (uint256) {
        ...
        for (uint256 cycleNum = startCycle; cycleNum <= endCycle; cycleNum ++) {
            if (claimable.canClaim(user, cycleNum)) {
    ...
    function claimLottery(address user, uint256[] calldata cycleNums) external {
        require(address(lottery) != address(0), "No lottery");
        lottery.claimMulti(user, cycleNums);
    }

    function claimSwapMining(address user, uint256[] calldata cycleNums) external {
        require(address(swapMining) != address(0), "No swap mining");
        swapMining.claimMulti(user, cycleNums);
    }

    function claimTradeMining(address user, uint256[] calldata cycleNums) external {
        require(address(tradeMining) != address(0), "No trade mining");
        tradeMining.claimMulti(user, cycleNums);
    }


    function claimAll(address user, uint256[] calldata cycleNums) external {
        if (address(swapMining) != address(0)) {
            swapMining.claimMulti(user, cycleNums);
        }
