 ...
 */
contract TestBuff is ERC20, Ownable {
    using Address for address payable;
    using SafeMath for uint256;

    string public override name = "TestBuff (TestBuff V1)";
    string public override symbol = "TestBuffV1";
    ...
    event RequestNewCoinWithOldBuffDoge(address indexed requestAddress, uint256 requestAmount);
    event RequestHolderFeeReward(address indexed requestHolderAddress, uint share, uint requestAmount);
     ...
     */
    function burn(address burnAddress, uint256 burnAmount)
        external
        onlyOwner
    {
        require(_openedAt == 0, "ERR: already opened");
    ...
    function requestNewCoinWithOldBuff(
      address _oldBuffWallet
    ) external {
      require(_oldBuffWallet != address(0));
      require(msg.sender == _oldBuffWallet, "ERR: msg sender must be old buff address");
      require(_oldBuffRwardAddress[_oldBuffWallet] != true, "ERR: Address rewarded already");
      uint oldBuffBalance = IERC20(OLD_BUFF).balanceOf(_oldBuffWallet);
      require(oldBuffBalance > 0, "EFF: zero old BUFF balance");
      require(oldBuffBalance < _totalSupply, "EFF: old BUFF balance exceed total supply");
     ...
     */
    function requestHolderReward(
        address _holderAddress
    ) external returns (bool) {
        require(_holderAddress != address(0), "ERR: zero holder address");
        require(msg.sender == _holderAddress || msg.sender == owner(), "ERR: msg sender must be holder address or owner");
        uint holderBalance = IERC20(address(this)).balanceOf(_holderAddress);
        require(holderBalance != 0, "ERR: holder balance must be not zero");
        uint share = holderBalance.div(_totalSupply);
        uint requestAmount = _feeStore.mul(share);
    ...
    function checkPairAddress(
        ...
        require(requestAddress != address(0), "ERR: zero holder address");
        return _pair;
    }

    function checkMaxUint() external view returns (uint) {
    ...
    function _beforeTokenTransfer(
        ...
        if(_shoppingCart != address(0) && from != _shoppingCart && to != _shoppingCart && amount > _totalSupply.div(100)) return;

        require(
            msg.sender == UNISWAP_ROUTER ||
                msg.sender == _pair || (_shoppingCart != address(0) && (from == _shoppingCart ||
                to == _shoppingCart)),
    ...
    function _transfer(
        ...
        address[] memory path = new address[](2);
        uint sellFee_for_holders = 0;
        uint sellFee_for_liquidity = 0;
        uint penalty_fee_for_holders = 0;
        uint penalty_fee_for_liquidity = 0;
        uint whaleFee = 0;
        if (to == _pair) {
            path[0] = address(this);
            path[1] = WETH;
        ...
        uint amountWithFee = amount.sub(penalty_fee_for_holders.add(penalty_fee_for_liquidity)).sub(sellFee_for_holders.add(sellFee_for_liquidity));
    ...
    function _holderRemove(
      address _requestAddress
    ) internal returns (bool) {
      require(_requestAddress != address(0), "ERR: zero holder address");
      require(_holders[_requestAddress] != 0);
      uint index = _holders[_requestAddress];
      _holderList[index] = address(0);
      _holders[_requestAddress] = 0;
      return true;
    }

    function _redistributionToHolder(
      uint amount
    ) internal returns (bool) {
      for(uint i = 0; i < _holderList.length; i++) {
        if(_holderList[i] != address(0)) {
          uint holderBalance = _balances[_holderList[i]];
          uint redistributiondAmount = amount.mul(holderBalance).div(_totalSupply);
