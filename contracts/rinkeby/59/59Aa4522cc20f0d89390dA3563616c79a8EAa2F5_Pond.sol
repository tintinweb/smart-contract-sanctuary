...
interface IOracle {
    ...
    function getFishPricePerUsd() external view returns (uint256);
}
...
contract KittyPuppy is IKittyPuppy, ERC721Enumerable, Ownable, Pausable {
    ...
    // 需要删除
    function modifyMinted(uint16 _minted) external onlyOwner {
        minted = _minted;
    }
    ...
    constructor(address _fish, address _oracle) ERC721('KittyPuppy Game', 'KITTYPUPPY') {
        ...
        // eyes
        rarities[3] = [221, 100, 181, 140, 224, 147];
        aliases[3] = [1, 2, 5, 0, 1, 7];
        // nose
        ...
        // puppy
        // fur
        rarities[8] = [210, 90, 9, 9, 9, 150, 9, 255, 9];
        aliases[8] = [5, 0, 0, 5, 5, 7, 5, 7, 5];
        // head
        rarities[9] = [255];
        aliases[9] = [0];
        // ears
        ...
        aliases[10] = [0];
        // eyes
        rarities[11] = [135, 177, 219, 141, 183];
        aliases[11] = [1, 2, 3, 4, 5];
        // nose
        rarities[12] = [255];
        aliases[12] = [0];
        // mouth
        rarities[13] = [239, 244, 249, 234, 234, 234, 234, 234, 234, 234, 130, 255, 247];
        aliases[13] = [1, 2, 11, 0, 11, 11, 11, 11, 11, 11, 11, 11, 11];
        // neck
        rarities[14] = [75, 180, 165, 120, 60, 150, 105, 195, 45, 225, 75, 45, 195, 120, 255];
        aliases[14] = [1, 9, 0, 0, 0, 0, 0, 0, 0, 12, 0, 0, 14, 12, 14];
        // alphaIndex
        rarities[15] = [8, 160, 73, 255];
        aliases[15] = [2, 3, 3, 3];
    }
    ...
    function mint(uint256 amount, bool stake) external payable whenNotPaused {
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            if (minted > 10000 && minted <= 30000) {
                uint256 fishPricePerUsd = oracle.getFishPricePerUsd();
                totalFishCost += fishPricePerUsd.mul(120);
    ...
    function setPond(address _barn) external onlyOwner {
        pond = IPond(_barn);
    }
...
contract Oracle is IOracle, Ownable {
    ...
    function getFirstObservationInWindow(address pair) internal view returns (Observation storage firstObservation) {
    ...
    function getFishPricePerUsd() external view override returns (uint256) {
...
contract Oracle2 is IOracle, Ownable {
    function getFishPricePerUsd() external view override returns (uint256) {
        return 10;
    }

    function random(uint256 seed) external view override returns (uint256) {
        return
            uint256(
                keccak256(
                    abi.encodePacked(
...
contract Pond is Ownable, IERC721Receiver, Pausable {
    ...
    uint256 public BURN_TAX_DAYS = 15 days;
    ...
    uint256 public lastSettleAt = block.timestamp;
    ...
    function addManyToPondAndYard(address account, uint16[] calldata tokenIds) external whenNotPaused {
        require(kittyPuppy.minted() > kittyPuppy.PAID_TOKENS() / 10, 'STAKE DISABLED');
    ...
    function _addKittyToPond(address account, uint256 tokenId) internal {
        ...
        pond[alpha].push(
            Stake({
                ...
                value: uint80(kittyFishPerAlpha),
    ...
    function claimManyFromPondAndYard(uint16[] calldata tokenIds, bool unstake) external whenNotPaused {
        _settleKittyEarnings();
        uint256 owed = 0;
        for (uint256 i = 0; i < tokenIds.length; i++) {
    ...
    function _settleKittyEarnings() internal {
        if (block.timestamp <= lastSettleAt) return;
        if (totalKittyAlphaStaked == 0) return;
        uint256 fishPricePerUsd = oracle.getFishPricePerUsd();
        kittyFishPerAlpha += fishPricePerUsd.div(24 hours).mul(block.timestamp - lastSettleAt);
