...
contract RedlionStudios is Context, Ownable, ERC2981, ERC721, ERC721Enumerable {
    ...
    mapping (uint => string) public publicationToURISuffix;
    mapping (uint => Publication) public publications;
    mapping (uint => uint) public tokenIdToPublication;
    mapping (uint => uint) public publicationPrices;
    ...
    function setMintingPrice(uint publication, uint price) onlyOwner public {
        publicationPrices[publication] = price;
    }
    ...
    function setURISuffix(uint publication, string memory uriSuffix) onlyOwner public {
        publicationToURISuffix[publication] = uriSuffix;
    }

    function launchNewPublication(uint publication, uint saleSize, string memory uriSuffix) onlyOwner public {
    ...
    function claim(uint publication, bytes32[] calldata proof, uint amount) whenNotPaused(publication) public {
    ...
    function mint(uint publication, uint amount) whenNotPaused(publication) public payable {
        require(msg.value == publicationPrices[publication] * amount, "INCORRECT PRICE");
    ...
    function tokenURI(uint256 tokenId) public override(ERC721) view returns (string memory) {
        return string(abi.encodePacked(_baseURI(), publicationToURISuffix[tokenIdToPublication[tokenId]]));
    }
