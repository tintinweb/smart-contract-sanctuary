...
contract CGChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accStakedTokenPerShare; // Accumulated Staked Tokens per share, times 1e12. See below.
    ...
    // The block number when Staking Token mining starts.
    uint256 public startBlock;
    // The block number when Staking Token mining stops.
    ...
    function setStakingTokenPerBlock(uint256 _stakingTokenPerBlock) public onlyOwner {
        massUpdatePools();
        stakingTokenPerBlock = _stakingTokenPerBlock;
    }
    ...
    function setPoolMultipler(
        ...
        require(
            ...
            "CG Staking: Invalid Multiplier Info"
        );
    ...
    function addPoolMultiplier(
        ...
        require(
            ...
            "CG Staking: Multiplier Requires Pool to be added"
        );
        require(
            (_fromBlock < endBlock) && (_toBlock <= endBlock),
            "CG Staking: Invalid Multiplier Info"
    ...
    function add(IERC20 _lpToken) public onlyOwner {
        ...
        for (uint256 i = 0; i < poolInfo.length; i++) {
            require(
                ...
                "CG Staking: DO NOT add the same LP token more than once"
            );
        }
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 stakingReward = multiplier.mul(stakingTokenPerBlock);
            accStakedTokenPerShare = accStakedTokenPerShare.add(
                stakingReward.mul(1e12).div(lpSupply)
            );
        }
        return user.amount.mul(accStakedTokenPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accStakedTokenPerShare).div(1e12).sub(
                    user.rewardDebt
                );
            if (pending > 0) {
                safeStakingTokenTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accStakedTokenPerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        uint256 pending =
            user.amount.mul(pool.accStakedTokenPerShare).div(1e12).sub(user.rewardDebt);
        if (pending > 0) {
            safeStakingTokenTransfer(msg.sender, pending);
        }
        if (_amount > 0) {
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accStakedTokenPerShare = pool.accStakedTokenPerShare.add(
            stakingReward.mul(1e12).div(lpSupply)
        );
    ...
    function safeStakingTokenTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > stakingTokenBalance) {
            stakingToken.transfer(_to, stakingTokenBalance);
        } else {
