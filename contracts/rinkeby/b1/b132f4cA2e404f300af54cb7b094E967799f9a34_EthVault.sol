...
contract EthVault is Vault {
    ...
    constructor(IERC20Detailed underlying_, IERC20 reward_, address harvester_, string memory name_, string memory symbol_, IUniswapRouter router_)
        Vault(underlying_, reward_, harvester_, name_, symbol_, router_, address(reward_)) {}
    ...
    function withdrawETH(uint amount) external {
        ...
        uint received = swap(amount, address(underlying), address(weth), address(this));
...
import "../misc/IUniswapRouter.sol";
...
contract Vault is Ownable, Pausable, DividendToken {
    ...
    IERC20Detailed public underlying; //  e.g. DAI
    IStrat public strat;
    ...
    constructor(IERC20Detailed underlying_, IERC20 reward_, address harvester_, string memory name_, string memory symbol_, IUniswapRouter router_, address weth_)
    ...
    function swapAndInvest(uint amount) internal {
      if(depositLimit > 0) { // if deposit limit is 0, then there is no deposit limit
          require(totalSupply().add(amount) <= depositLimit);
      }

      uint received = swap(amount, address(target), address(underlying), address(strat));

      strat.invest();
      _mint(msg.sender, received);
    }

    function swap(uint amount, address from, address to, address depositAddress) internal returns (uint) {
        address[] memory path = new address[](2);
        path[0] = address(from);
        path[1] = address(to);

        // eg 1%
        uint acceptableSlippage = 1;
        target.approve(address(router), amount);

        uint amountOut = router.getAmountsOut(amount, path)[1];
        uint minAmountOut = amountOut.mul(100-acceptableSlippage).div(100);
        ...
        uint received = router.swapExactTokensForTokens(amount, minAmountOut, path, depositAddress, block.timestamp + 20 minutes)[1];
        return received;
    }
