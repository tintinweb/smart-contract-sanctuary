...
// File: TokenPoolv2.sol

// GP: send percentage of rewards to jelly treasury

contract TokenPoolv2 is ITokenPool, ERC20 {
    ...
    uint256 public override constant poolTemplate = 2;
        ...
        override
    {
        ...
        if (_user != address(0) && tSupply > 0) {
            ...
            for(uint i = 0; i < rewardTokens.length; i++) {
                ...
                /// GP: Only update if something has updated, Check if this if() makes sense,  
                if (uInfo.lastRewardPoints != rInfo.rewardsPerTokenPoints || uint256(rInfo.lastUpdateTime) != block.timestamp) {
                    ...
                    lastUpdateTime = BoringMath.to64(block.timestamp);
                    rewardsPerTokenPoints = rInfo.rewardsPerTokenPoints + BoringMath.to128(currentRewards * 1e18 * pointMultiplier / tSupply);
                    poolRewards[rewardToken] = RewardInfo(lastUpdateTime,rewardsPerTokenPoints);
                    
                    uint256 newRewardPerToken = uint256(rewardsPerTokenPoints - uInfo.lastRewardPoints);
    ...
    function rewardsOwing(
        ...
        returns(uint256)
    {
        uint256 newRewardPerToken = uint256(poolRewards[_rewardToken].rewardsPerTokenPoints - userRewards[_user][_rewardToken].lastRewardPoints);
        uint256 rewards = balanceOf(_user) * newRewardPerToken
                                                / 1e18
                                                / pointMultiplier;
        return rewards;
    ...
    function unclaimedRewards(address _user, address _rewardToken) public view returns(uint256 uRewards) {
            ...
            uint256 rewardsPerTokenPoints = poolRewards[_rewardToken].rewardsPerTokenPoints + BoringMath.to128(currentRewards * 1e18 * pointMultiplier / totalSupply());

            uint256 newRewardPerToken = uint256(rewardsPerTokenPoints - userRewards[_user][_rewardToken].lastRewardPoints);

            uint256 rewards = balanceOf(_user) * newRewardPerToken
                                                    / 1e18
                                                    / pointMultiplier;
            uRewards = rewards + uint256(_userRewards.rewardsEarned - _userRewards.rewardsReleased);
    }
    ...
    function claimRewards(address _user) public override {
        ...
        for(uint i = 0; i < rewardTokens.length; i++) {
            ...
            UserInfo storage _userRewards =  userRewards[_user][rewardToken];
            ...
            OZIERC20(rewardToken).safeTransfer(_user, payableAmount);
