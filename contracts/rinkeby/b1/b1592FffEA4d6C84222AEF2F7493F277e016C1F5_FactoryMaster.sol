...
contract FactoryMaster is Ownable {
  ...
  function getLatestChildrenErc20() external view returns (address, address) {
    if (childrenErc20.length > 0) {
      return (
        address(tokenOwner),
        address(childrenErc20[childrenErc20.length - 1])
      );
    }
  }

  function getLatestChildrenErc20Burnable()
    external
    view
    returns (address, address)
  {
    if (childrenErc20Burn.length >= 1) {
      return (
        address(tokenOwner),
        address(childrenErc20Burn[childrenErc20Burn.length - 1])
      );
    }
  }

  function getLatestChildrenErc20Mintable()
    external
    view
    returns (address, address)
  {
    if (childrenErc20Mint.length >= 1) {
      return (
        address(tokenOwner),
        address(childrenErc20Mint[childrenErc20Mint.length - 1])
      );
    }
  }
  ...
  function getLatestChildrenErc20MintableBurnable()
    ...
    returns (address, address)
  {
    if (childrenErc20MintBurn.length >= 1) {
      return (
        address(tokenOwner),
        address(childrenErc20MintBurn[childrenErc20MintBurn.length - 1])
      );
    }
  }
}
 ...
 */
abstract contract ERC20Burnable is Context, ERC20Standard {
    /**
...
contract ERC20Standard is Context, IERC20, IERC20Metadata {
  ...
  // function initialSupply(uint256 totalSupply_) public {
