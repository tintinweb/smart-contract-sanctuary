...
contract Airdrop is Ownable { 
    ...
    function distribute () internal  {
        for (uint256 i = 0; i < info.length; i++) {
            if (info[i].active == true && info[i].rewardDistributed == false) {
                uint256 n = info[i].rewardAmount.mul(rebaseRate);
                uint256 r = n.div(10000); 
    ...
    function setAccount(address _address, uint256 _vesting, uint256 _reward) public onlyOwner { 
