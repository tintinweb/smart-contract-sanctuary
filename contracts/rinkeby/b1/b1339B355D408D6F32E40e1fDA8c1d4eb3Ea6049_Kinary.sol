...
/**
Kinary Protocol v0.7 BETA

General usage / framework:

Kinary protocol stores deposit contracts for a specific amount of time rewarding the user
with KINA the first proof of time token available to mine

Frontend API:
1.- addUpdateAllowedToken(address token, uint totalTokenSupply) to
add an allowed token and his total supply

2.- addUpdateRate(address token, uint rate) to add a rate of rewards in relation to WETH
calculated in 181 function getRate(address token) 

3.- addDeposit(address payable owner, uint _amount, uint lockingPeriod, address token) 
to create your first deposit contract 'owner' is the owner of the deposit
you can create someone else a deposit, amount is in wei, locking period is in weeks from a range
of 0 - 360 and token is the token you will be locking up in the contract

This function trows an event in the transcaction indicating you the deposit ID

P.S. at this point the token should be allowed and with a rate, also you will need to 
execute the approve() function in the token contract before you can make your first
deposit, you can find the approve function on the contract of your token

4.- use harvest(index) to collect KINA at any time each day TODO hourly

5.- use withdrwa(index) when the period is over, index can be collected from the DepositCreated event
on each transaction 
...
contract Kinary is ChainlinkClient {
    ...
    event DepositCreated(uint indexed _id, address _owner, uint _amount, address _token, uint _lockingPeriod);
    ...
    public {
        if (allowedTokens[token] != 0) {
            ...
            deposits.push(
           Deposit({owner: owner, depositIndex: getDepositCount(), amount: _amount, depositTimestamp: now, 
           ...
           emit DepositCreated(getDepositCount(), owner, _amount, token, lockingPeriod);
        }
    }
