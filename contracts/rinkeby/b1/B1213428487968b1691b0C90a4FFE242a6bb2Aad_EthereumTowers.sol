...
contract EthereumTowers is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    // TODO: Random mint function
    // Have an array of IDs and pop each time that we mint an apartment
    function randomMint(uint256 amount) public payable nonReentrant {
        require(mintActive, "Minting not active");
        if (whitelistActive) {
            require(whitelist[msg.sender], "You are not whitelisted");
        }
        ...
        for (uint256 i = 1; i <= amount; i++) {
            uint256 randApartment = getRandom(apartmentIds);
            _safeMint(msg.sender, randApartment);
        }
    }
    ...
    function setApartmentIds(uint16[] calldata _ids) external onlyOwner {
        for (uint256 i = 0; i < _ids.length; i++) {
            apartmentIds.push(_ids[i]);
        }
    }
