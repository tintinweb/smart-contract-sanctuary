...
contract Forest is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    uint public constant MINIMUM_TIME_TO_EXIT = 5 hours;
    ...
    function getDailyBattRate() public view returns (uint) {
        if (metaMoose.totalSupply() <= 500) {
            return 10000 ether;
        } else if (metaMoose.totalSupply() <= 1000) {
            return 12000 ether;
        } else {
            return 15000 ether;
        }
    ...
    function _claimFromHarvester(uint16 tokenId, bool unstake) internal returns (uint owed) {
        ...
        if (stake.value >= stakingEndTimestamp) {
            ...
            owed = ((block.timestamp - stake.value) * getDailyBattRate()) / 1 hours;
        } else {
            owed = ((stakingEndTimestamp - stake.value) * getDailyBattRate()) / 1 hours;
        }
        if (unstake) {
