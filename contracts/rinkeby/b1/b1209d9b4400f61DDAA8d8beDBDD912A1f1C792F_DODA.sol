...
contract DODA is ERC721, ERC721Enumerable, Ownable {
    ...
    // Dev Address
    address public devAddress = 0x87FBF0079d5Bb9Ea8FBF3805111FeEFF1Dd696Fc;
    ...
    string public baseExtension = ".enc";
    string private _baseURIextended;
    string private testP;
    ...
    mapping(uint256 => string) public traitList;
    ...
    function getPermutation(string memory str, uint16 n, uint16 deep) public pure returns (string memory)  {
        string memory res;
        uint16 len = 7 ;
        uint16 d = deep;
        uint16 f = 1;
        ...
        if (n >= 0 && n < f) {
            for (res = ""; d > 0; d--) {
                f /= len;
                 i = n / f;
                n %= f;
                res = string(abi.encodePacked(res, DODALibrary.substring(str, i, i + 1) ));
                str = string(abi.encodePacked(DODALibrary.substring(str, 0, i), DODALibrary.substring(str, i + 1, bytes(str).length) ));
                len--;
            }
        }
        return res;
    ...
    function test(string memory str, uint16 n, uint16 deep) public returns(string memory) {
        testP = getPermutation(str, n, deep);
        return testP;
    }
    ...
    function reserveTokens(uint256[] memory _ids) public onlyOwner {
