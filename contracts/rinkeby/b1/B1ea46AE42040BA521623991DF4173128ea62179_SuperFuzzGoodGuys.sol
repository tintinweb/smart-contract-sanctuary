...
contract SuperFuzzGoodGuys is Ownable, ERC721Enumerable, PaymentSplitter, ReentrancyGuard {
    ...
    // address private GoldTicketAddress = ; // TODO: Get Mainnet address
//    address private GoldTicketAddress = 0xdB0A737883D60F7FEeE32E224dD5151a6Ed241B2;  // TODO: Localhost Address
     address private GoldTicketAddress = 0x191F2f591E5d4E4a004eb4BBf3CC90C4DF4A4b95;  // TODO: rinkeby Address
    SuperFuzzGoldTicket goldTicket;

    // address private GoldTicketAddress = ; // TODO:Get Mainnet address
//    address payable private BadBatchAddress = payable(0x191F2f591E5d4E4a004eb4BBf3CC90C4DF4A4b95);  // TODO: Localhost Address
    ...
    uint public reservedTokens = 260;
    ...
    uint public currentAirdropIndex = 0;
    ...
    constructor()
        ...
        PaymentSplitter(_team, _team_shares)
    {
        _baseURIextended = "ipfs://bafybeibrgdlav4stwuhvcozjudk6uvlndxlclx6gacj5bhmkfwo46p5wyq/"; // TODO: update to production ipfs link
    ...
    modifier verifyAirDrop(address _to, uint _amount) {
        ...
        require(totalSupply().add(_amount) <= tokensForSale, "Purchase would exceed max supply of Good Guys");
        _;
    }
    ...
    // create a helper function to help the team know how many they have left to airdrop and allows them to strategically do it.

    function remainingAirdrop() public view returns (uint) {
      uint goldTicketSupply = goldTicket._totalSupply();
      uint remaining = goldTicketSupply.sub(currentAirdropIndex);
      return remaining;
    }
    ...
    function airDropToGoldTicketOwners(uint _toDrop) external onlyTeam {
        ...
        for(uint i = currentIndex; i <= goldTicketSupply; i++) {
        ...
        currentAirdropIndex = currentAirdropIndex.add(_toDrop);
    }

    function teamClaim() external onlyTeam {
        address _to = msg.sender;
        uint claimAmount = NFTsToClaim[_to];
        require(claimAmount > 0, "Nice try! You don't have any NFTs to Claim");
        console.log("NFTs to claim", claimAmount);
        for (uint i = 1; i <= claimAmount; i++) {
            uint id = totalSupply() + 1;
            NFTsToClaim[_to]--;
