...
contract sYSLToken is ERC20, Ownable, AccessControl{
    ...
    mapping(address=>LockedFunds) usersLocked;
    ...
    constructor() ERC20("sYSL token", "sYSL") {
    ...
    function mintPurchased(address account, uint amount, uint lockTime) external _isMinter {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override {
        unlock(from);
        uint256 stillLocked = usersLocked[from].lockAmount - usersLocked[from].unlocked;

        require(amount <= balanceOf(from) - stillLocked, "Not enough available tokens");
    }

    function unlock(address account) private {
