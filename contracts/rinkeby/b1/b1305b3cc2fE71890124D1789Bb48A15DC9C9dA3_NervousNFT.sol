    ...
    Ownable
{
    ...
    uint256 public startPresaleAct4Date = 1642471200; // January 17, 6 pm PST
    ...
    function getHash(uint256 presaleAct) internal view returns (bytes32) {
        return keccak256(abi.encodePacked("NERVOUS", presaleAct, msg.sender));
    }
    ...
    function validateTicket(bytes memory pass) internal view returns (uint256) {
        ...
        (presaleAct, mintablePresale) = currentPresaleActWithMaxMint();

        bytes32 hash = getHash(presaleAct);
        address signer = recover(hash, pass);

        if (signer != preSaleSigner) {
            revert("Invalid ticket. Not valid for current presale act");
        }
    ...
    function currentPresaleActWithMaxMint()
        public
        view
        returns (uint256, uint256)
    {
        if (hasPreSaleStarted()) {
            if (block.timestamp >= startPresaleAct4Date) {
                return(4, MAX_ACT4_PRESALE_MINTS);
            }
            else if (block.timestamp >= startPresaleAct3Date) {
                return (3, MAX_ACT3_PRESALE_MINTS);
            } else if (block.timestamp >= startPresaleAct2Date) {
                return (2, MAX_ACT2_PRESALE_MINTS);
            } else if (block.timestamp >= startPresaleDate) {
                return (1, MAX_ACT1_PRESALE_MINTS);
            } else {
                return (0, 0);
            }
        } else {
