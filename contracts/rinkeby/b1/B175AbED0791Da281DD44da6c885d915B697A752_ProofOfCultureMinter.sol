...
contract ProofOfCultureMinter is Ownable {
  ...
  uint256 public reservationTokenType;

  uint256 public mintedReservationTokenCount;
  ...
  uint256 public constant supporterTokenCap = 30; // THIS WILL NEVER CHANGE!! WE ONLY MINT A SET AMOUNT FOR SUPPORTERS
  ...
  /**
  * @dev Gets the total supply, which is the sum of all reservation tokens bought and Hashtags minted.
  
         Note that NFTs have a maxIndex but FTs don't, so we just keep track of the total here.
  */
  function totalSupply() public view returns (uint256) {
    return mintableErc1155.maxIndex(hashtagTokenType).safeAdd(mintedReservationTokenCount);
  }
  ...
  */
  function getPrice(bool isReservation) public view returns (uint256) {
  ...
  */
  function mint(address _dst, string[] memory _hashtags) public payable {
    ...
    require(getPrice(false).safeMul(numberOfNfts) <= msg.value, "Ether value sent is not correct");

    // set price upfront before minting - we will need to use this to calculate refunds
    uint256 pricePerHashtag = getPrice(false);
  ...
  /**
  * @dev Reserve up to 20 hashtags at once. The reservation token is a fungible token
         so all we care about is total supply.
  */
  function reserve(address _dst, uint256 numberOfTokens) public payable {
    require(totalSupply() < MAX_NFT_SUPPLY, "Sale has already ended");
    require(numberOfTokens > 0, "numberOfTokens cannot be 0");
    require(numberOfTokens <= batchOrderLimit, "You may not buy more than the batch limit at once");
    require(totalSupply().safeAdd(numberOfTokens) <= MAX_NFT_SUPPLY, "Exceeds MAX_NFT_SUPPLY");

    uint256 totalPrice = getPrice(true).safeMul(numberOfTokens);
    ...
    mintableErc1155.mintFungible(reservationTokenType, dsts, amounts);

    mintedReservationTokenCount = mintedReservationTokenCount + numberOfTokens;

    treasury.transfer(totalPrice);
    msg.sender.transfer(msg.value - totalPrice);
  }

  /**
  * @dev Redeem reservation tokens for hashtags. This is so that at a later date reservation token
         owners can convert their tokens for a hashtag NFT. We will try our best to mint all but
         if the hashtag has already been claimed, then we will not use the reservation token.
  */
  function redeem(address _dst, string[] memory _hashtags) public {
    uint256 numberOfNfts = _hashtags.length;
    uint256 numberOfReservationTokens = mintableErc1155.balanceOf(_dst, reservationTokenType);

    require(numberOfNfts > 0, "numberOfNfts cannot be 0");
    require(numberOfReservationTokens >= numberOfNfts, "Not enough Reservation Tokens to redeem hashtags");
    ...
    mintableErc1155.burn(_dst, reservationTokenType, mintedCount);
    mintedReservationTokenCount = mintedReservationTokenCount - mintedCount;

    emit BurnedReservationTokens(_dst, mintedCount);
  }
