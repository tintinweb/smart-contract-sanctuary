...
contract PPAShuttlepass is ERC1155, Ownable {
    ...
    uint256 public goldPriceWei = 0.2 ether;
    uint256 public silverPriceWei = 0.12 ether;
    uint256 public bronzePriceWei = 0.07 ether;
    ...
    uint256 public constant FIRST_BUYERS = 1;
    uint256 public constant OG_PASSENGERS = 2;
    ...
    constructor()
        public
        ERC1155(
            "https://assets.jointheppa.com/shuttlepasses/metadata/{id}.json"
        )
    {}

    function mint(uint256 passType, uint256 amount) public payable {
        require(saleState == 3, "Public mint is not open");
        /**
         * Sale 3:
         * Public. 5 per address.
         */
        publicNumMinted[msg.sender] = SafeMath.add(
            publicNumMinted[msg.sender],
            amount
        );
        require(
    ...
    function earlyMint(
        ...
        if (saleState == 1) {
             ...
             */
            sale1NumMinted[msg.sender] = SafeMath.add(
                sale1NumMinted[msg.sender],
                amount
            );
            require(
            ...
            require(
                whitelistType == FIRST_BUYERS,
                "Must use First Buyers whitelist"
            );
             ...
             */
            sale2NumMinted[msg.sender] = SafeMath.add(
                sale2NumMinted[msg.sender],
                amount
            );
            require(
            ...
            revert(
                "The early sale is over. Use the public mint function instead."
            );
        }
    ...
    function _internalMint(uint256 passType, uint256 amount) internal {
    ...
    function ownerMint(uint256 passType, uint256 amount) public onlyOwner {
        incrementNumMinted(amount);
        require(
            passType == GOLD || passType == SILVER || passType == BRONZE,
            "Invalid passType"
        );
        _mint(msg.sender, passType, amount, "");
    }

    function incrementNumMinted(uint256 amount) internal {
    ...
    function checkPayment(uint256 amountRequired) internal {
    ...
    function setGoldPriceWei(uint256 newPrice) public onlyOwner {
        goldPriceWei = newPrice;
    }

    function setSilverPriceWei(uint256 newPrice) public onlyOwner {
        silverPriceWei = newPrice;
    }

    function setBronzePriceWei(uint256 newPrice) public onlyOwner {
