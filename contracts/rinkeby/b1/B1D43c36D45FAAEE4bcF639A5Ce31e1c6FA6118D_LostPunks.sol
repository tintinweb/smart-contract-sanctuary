...
contract LostPunks is ERC721Enumerable, Ownable {
    ...
    function tokenURI(uint256 index) public view override returns (string memory)
    {
        ...
        string memory json = base64Encode(bytes(string(abi.encodePacked(
            '{"name": "LostPunk #',
            toString(index), 
            '", "description": "LostPunks are descendants of the original CryptoPunks. Metadata and images are 100% generated and stored on-chain. Inspired by, not affiliated with Larva Labs.", "image": "data:image/svg+xml;base64,', 
