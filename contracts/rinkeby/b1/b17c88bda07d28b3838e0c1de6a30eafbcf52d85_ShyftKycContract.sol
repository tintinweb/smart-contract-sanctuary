...
interface IShyftKycContract is IErc20, IErc223, IErc223ReceivingContract {
    ...
    function withdrawTokenBip32XToErc223(address _erc223ContractAddress, address _to, uint256 _value) external returns (bool ok);
...
contract ShyftKycContract is IShyftKycContract, TokenConstants, AccessControl {
    ...
    event EVT_Bip32XTokenFallback(address msgSender, address _from, uint256 value, uint256 nativeTokenType, uint256 bip32X_type);

    /// @dev Event for transfer and minting of Bip32X type assets.
    event EVT_TransferAndMintBip32X(address contractAddress, address msgSender, uint256 value, uint256 bip32X_type);

    /// @dev Event for transfer and burning of Bip32X type assets.
    event EVT_TransferAndBurnBip32X(address contractAddress, address msgSender, address to, uint256 value, uint256 bip32X_type);

    /// @dev Event for transfer of Bip32X type.
    event EVT_TransferBip32X(address indexed from, address indexed to, uint256 tokens, uint256 bip32X_type);

    /// @dev Event for approval of Bip32X type.
    event EVT_ApprovalBip32X(address indexed tokenOwner, address indexed spender, uint256 tokens, uint256 bip32X_type);
...
//            bytes memory empty;
    ...
    function tokenFallback(address _from, uint _value, bytes memory _data) mutex public override returns (bool ok) {
        ...
        if (shyftKycContractRegistryAddress != address(0) && contractRegistry.isShyftKycContract(address(msg.sender)) == true) {
            ...
            emit EVT_Bip32XTokenFallback(msg.sender, _from, _value, nativeBip32X_type, bip32X_type);
            emit EVT_TransferAndMintBip32X(msg.sender, _from, _value, bip32X_type);
    ...
    function name() public pure returns (string memory _name) {
        //@note:@todo:@critical: change name?!
    ...
    function withdrawTokenBip32XToErc223(address _erc223ContractAddress, address _to, uint256 _value) mutex public override returns (bool ok) {
        ...
        if (fallbackSuccess == true && success == true) {
            emit EVT_TransferAndBurnBip32X(_erc223ContractAddress, msg.sender, _to, _value, bip32X_type);
    ...
    function transferFromErc20TokenToBip32X(address _erc20ContractAddress, uint256 _value) mutex public override returns (bool ok) {
        ...
        if (erc20Contract.allowance(msg.sender, address(this)) >= _value) {
            ...
            if (transferFromResult == true) {
                ...
                //@note: reverts since a transactional event has occurred.
                revert("transferFrom not completed successfully within the erc20 contract");
            }
        } else {
    ...
    function withdrawTokenBip32XToErc20(address _erc20ContractAddress, address _to, uint256 _value) mutex public override returns (bool ok) {
        ...
        if (success == true) {
            ...
            //@note: reverts since a transactional event has occurred.
            revert("transfer(address,uint256) event unsuccessful");
        }
    }
    ...
    function setV1EmergencyErc20RedemptionResponder(address _emergencyResponder) public returns(uint8 result) {
        if (msg.sender == owner) {
            ...
            EVT_setV1EmergencyResponder(_emergencyResponder);
    ...
    function getIncorrectlySentAssetsBalance() public view returns(uint256 result) {
        ...
        address ethMay26KycContractAddress = contractRegistry.getContractAddressOfVersion(0);

        if (ethMay26KycContractAddress != address(0)) {

            IErc20 may26Erc20 = IErc20(ethMay26KycContractAddress);

            uint256 currentBalance = may26Erc20.balanceOf(address(this));
        ...
        if (msg.sender == emergencyResponder) {
            ...
            if (ethMay26KycContractAddress != address(0)) {
                ...
                if (_amount <= incorrectlySentAssetBalance) {
                    bool success = may26Erc20.transfer(_destination, _amount);

                    if (success == true) {
                        EVT_redeemIncorrectlySentAsset(_destination, _amount);
