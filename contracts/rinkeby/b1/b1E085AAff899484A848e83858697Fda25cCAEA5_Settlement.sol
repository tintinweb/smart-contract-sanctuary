...
contract Settlement is ISettlement, Governed, Time {
    ...
    IPriceProvider public ethPriceProvider;

    
    
    
    uint public constant SETTLEMENT_PRICE_NO_CONFIDENCE_THRESHOLD = 0.10e18;
    
    uint public settlementPrice;
    
    ITCP public tcp;
    
    bool public priceConfidence = true;
    
    uint64 public priceDiscoveryStartTime;
    
    mapping(address => uint) public noPriceConfidenceUserVotes;

    constructor (IPriceProvider _ethPriceProvider) {
    ...
    function _initHook() internal override {
        ...
        validUpdate[this.setEthPriceProvider.selector] = true;
    }

    
     
    function getRecentPrice() public view returns (uint ethPrice) {
        (, int _ethPrice,, uint ethTimeStamp,) = ethPriceProvider.latestRoundData();

        require(ethTimeStamp.add(1 hours) > _currentTime(), 'Eth price is stale');
        require(_ethPrice > 0, 'Eth price invalid');

        
        ethPrice = _ethPrice.toUint256()._div(10 ** ethPriceProvider.decimals());
    }

    function settlementStage() public view returns (SettlementStage) {
        if (stopped) {
            return SettlementStage.ContractStopped;
        }
        if (!governor.isShutdown()) {
            return SettlementStage.NotShutdown;
        }
        if (priceDiscoveryStartTime == 0) {
            return SettlementStage.NotInitialized;
        }
        if (_currentTime() < priceDiscoveryStartTime) {
            return SettlementStage.WaitingForPriceTime;
        }
        if (settlementPrice > 0) {
            return SettlementStage.PriceConfirmed;
        }
        return priceConfidence
            ? SettlementStage.PriceConfidence
            : SettlementStage.NoPriceConfidence;
    }

     
    function initializeSettlement() external lockProtocol {
        _requireCorrectStage(SettlementStage.NotInitialized);

        uint64 latestAuctionCompletionTime = governor.auctions().latestAuctionCompletionTime();
        uint64 minShutdownTime = governor.shutdownTime().add(2 days);

        priceDiscoveryStartTime = latestAuctionCompletionTime < minShutdownTime
            ? minShutdownTime
            : latestAuctionCompletionTime;

        emit SettlementInitialized(priceDiscoveryStartTime);
    }

    
    function stakeTokensForNoPriceConfidence(uint countTCPToStake) external override lockProtocol {
        _requireCorrectStage(SettlementStage.WaitingForPriceTime);

        require(countTCPToStake > 0, 'Noop');

        
        noPriceConfidenceUserVotes[msg.sender] = noPriceConfidenceUserVotes[msg.sender].add(countTCPToStake);

        
        TransferHelper.safeTransferFrom(address(tcp), msg.sender, address(this), countTCPToStake);

        _updateThresholdMetSinceBlockNumber();

        emit StakedNoConfidenceTokens(msg.sender, countTCPToStake);
    }

    
    function unstakeTokensForNoPriceConfidence() external override lockProtocol {
        
        uint countUserVotes = noPriceConfidenceUserVotes[msg.sender];

        require(countUserVotes > 0, 'No tokens staked');

        noPriceConfidenceUserVotes[msg.sender] = 0;

        TransferHelper.safeTransfer(address(tcp), msg.sender, countUserVotes);

        _updateThresholdMetSinceBlockNumber();

        emit UnstakedNoConfidenceTokens(msg.sender, countUserVotes);
    }

    
    function confirmNoPriceConfidence() external lockProtocol {
        _requireCorrectStage(SettlementStage.WaitingForPriceTime);

        require(thresholdMetSinceBlockNumber < block.number, 'Insufficient no confidence votes');

        priceConfidence = false;

        emit NoConfidenceConfirmed(msg.sender);
    }

     
    function confirmPrice() external lockProtocol {
        _requireCorrectStage(SettlementStage.PriceConfidence);

        settlementPrice = getRecentPrice();
    }

    
    
    function withdrawCollateral(uint64 positionID) external lockProtocol {
        _requireCorrectStage(SettlementStage.PriceConfirmed);

        IPositionNFT zhuPositionNFT = governor.zhuPositionNFT();

        require(zhuPositionNFT.isApprovedOrOwner(msg.sender, positionID), 'Not Authorized');

        IAccounting.DebtPosition memory position = governor.market().systemGetUpdatedPosition(positionID);
        require(position.collateral > 0, 'No collateral');

        
        uint collateralToWithdraw = 0;
        if (position.debt == 0) {
            collateralToWithdraw = position.collateral;
        } else {
            uint collateralBackingDebt = position.debt._div(settlementPrice);
            require(collateralBackingDebt < position.collateral, 'No collateral to withdraw.[1]');
            collateralToWithdraw = position.collateral - collateralBackingDebt;
        }

        position.collateral -= collateralToWithdraw;

        
        accounting.setPosition(positionID, position);

        
        accounting.sendCollateral(payable(zhuPositionNFT.ownerOf(positionID)), collateralToWithdraw);

        emit SettlementWithdrawCollateral(positionID, msg.sender, collateralToWithdraw);
    }

    
    function getCollateralForZhu(uint64 positionID, uint zhuCount) external lockProtocol {
        _requireCorrectStage(SettlementStage.PriceConfirmed);

        IAccounting.DebtPosition memory position = governor.market().systemGetUpdatedPosition(positionID);

        
        require(position.debt >= zhuCount, 'Exceeds debt');

        uint collateralCount = zhuCount._div(settlementPrice);

        require(position.collateral >= collateralCount, 'Insufficient collateral');

        position.collateral -= collateralCount;
        position.debt -= zhuCount;

        
        accounting.setPosition(positionID, position);
        accounting.decreaseDebt(zhuCount);

        
        zhu.burnFrom(msg.sender, zhuCount);
        accounting.sendCollateral(msg.sender, collateralCount);

        emit SettlementCollateralForZhu(positionID, msg.sender, zhuCount, collateralCount);
    }

    
    function thresholdMet() public view returns (bool) {
        return tcp.balanceOf(address(this)) >= governor.distributedTCP()._mul(SETTLEMENT_PRICE_NO_CONFIDENCE_THRESHOLD);
    }

    
    function _requireCorrectStage(SettlementStage correctStage) internal view {
        require(settlementStage() == correctStage, 'Incorrect stage');
    }

    uint private constant THRESHOLD_NOT_MET = type(uint256).max;
    uint thresholdMetSinceBlockNumber = THRESHOLD_NOT_MET;

    function _updateThresholdMetSinceBlockNumber() internal {
        if (thresholdMet()) {
            if (thresholdMetSinceBlockNumber == THRESHOLD_NOT_MET) {
                thresholdMetSinceBlockNumber = block.number;
            }
        } else {
            if (thresholdMetSinceBlockNumber != THRESHOLD_NOT_MET) {
                thresholdMetSinceBlockNumber = THRESHOLD_NOT_MET;
            }
        }
    }
    ...
    function setEthPriceProvider(IPriceProvider provider) external override onlyGovernor {
        
        
        provider.decimals();
        provider.latestRoundData();

        ethPriceProvider = provider;
        priceConfidence = true;
        emit ParameterUpdatedAddress('ethPriceProvider', address(provider));
    }
