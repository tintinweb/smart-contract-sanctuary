...
interface IBondingCalculator {
  function valuation( address pair_, uint amount_ ) external view returns ( uint principleValuation_ );
}
...
contract OlympusBondingCalculator is IBondingCalculator {
    ...
    function valuation( address pair_, uint amount_ ) external view override returns ( uint principleValuation_ ) {
        uint totalValue = getTotalValue( pair_ );
        uint totalSupply = IUniswapV2Pair( pair_ ).totalSupply();

        principleValuation_ = totalValue.mul( FixedPoint.fraction( amount_, totalSupply ).decode112with18() ).div( 1e18 );
    }

    function markdown( address token_ ) external view returns ( uint ) {
        ( , uint reserve1, ) = IUniswapV2Pair( token_ ).getReserves();
        return reserve1.mul( 2e9 ).div( getTotalValue( token_ ) );
    }
}