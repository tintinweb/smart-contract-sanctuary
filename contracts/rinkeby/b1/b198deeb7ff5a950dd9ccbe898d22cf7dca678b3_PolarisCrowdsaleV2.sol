...
contract PolarisCrowdsaleV2 is Ownable,ReentrancyGuard {
        ...
        address _saleToken)
    {
        ...
        incentivesContractAddress = _incentivesContract;
    ...
    function addToWhiteList1(LockIInfoExt calldata _newBuyer) external onlyOwner {
        _whitelist[_newBuyer.user]=LockIInfo(_newBuyer.lockPeriod,_newBuyer.amount);
    }
    ...
    function distribute() external nonReentrant{
        ...
        (bool success,) = incentivesContractAddress.call(
            abi.encodeWithSignature("lockGovernance(address,address,uint256,uint256)",
            lockContractAddress,incentivesTokenAddress,distributeTokenAmount,unlock_time));
        require(success,"lockGovernance FAIL");
