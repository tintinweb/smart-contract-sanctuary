...
contract DarwinNFT is Ownable, ERC721, ERC721Enumerable, VRFConsumerBase, ReentrancyGuard {
    ...
    function airdrop(address[] memory _to, uint256 amount) public offline onlyOwner {
        require(totalAirdrop + (_to.length * amount) <= _maxAirdrop, "Exceed airdop allowance limit.");
    ...
    function getMintedBySaleMode() public view returns(uint256) {
        ...
        if (_mode == 2) return totalPublicSale();
        return 0;
    }
    ...
    function getMaxSupplyBySaleMode() public view returns(uint256) {
        if (_mode == 1) return maxPrivateSale;
        if (_mode == 2) return maxSupply - maxPrivateSale - _maxAirdrop;
        return 0;
    }
    ...
    function mint(uint256 amount) public payable online nonReentrant mintable(amount) returns (bool) {
     ...
     */
    function isOversoldPrivateSale(uint amount) internal view returns(bool) {
        return totalPrivateSale + amount <= maxPrivateSale;
    }
