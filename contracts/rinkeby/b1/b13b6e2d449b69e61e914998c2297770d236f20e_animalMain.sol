...
contract animalMain is  ERC721,Ownable {
  ...
  function createAnimal() public payable {
      ...
      //使用亂數來產生DNA, 星級, 動物種類
      dna=random();
      //5* 3% , 4* 12% , 3* 20% , 2* 30% , 1* 35%
      uint R = uint(random())%100;
      if(R<3){
          star=5;
      }
      else if(R<15){
      ...
      element = uint (random())%2 +1;
      
      atk = star*star+uint(random())%10+1;
       
      uint id = animals.push(animal(dna, uint8(star), uint8(roletype),uint8(element),uint16(atk))) - 1;
