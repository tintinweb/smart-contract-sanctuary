  ...
  */
    ...
    function _withdrawFromComp(address _asset,address _cToken,uint256 _amount) internal {
        ...
        if(_asset == ETHER){                                                                  // THIS IS WHERE YOU MAKE MONEY
            ...
            CEther cToken = CEther(_cToken);
            uint256 userProfit = _Computation(_cToken,msg.value, pTokenAmount);
            withdrawn = pBalances[msg.sender][_cToken].sub(userProfit);
            ...
            pBalances[msg.sender][_cToken] = pBalances[msg.sender][_cToken].sub(withdrawn);  // we adjust the balance of both asset and left the 
            ...
            uint256 userProfit = _Computation(_cToken,_amount, pTokenAmount);
            withdrawn = pBalances[msg.sender][_cToken].sub(userProfit);
            cToken.approve(address(cToken), withdrawn);                                    
    ...
    function _Computation(address _asset, uint256 _amount, uint256 _pAmount) internal view returns (uint256){

         CErc20 token = CErc20(_asset);
        uint256 rate = token.exchangeRateStored();                   // stored rate
        uint256 daiValue = rate * _pAmount / 1e18;                   // curent rate compare to original
        uint256 cDaiValue =  _amount * 1e18 / rate;                  // amount of cdai for requested withdraw amount
        uint256 profit = _pAmount - cDaiValue;                       // the interest
        uint256 fees = (profit / 10000) * protocolFees;
        uint256 userBalance = _pAmount - cDaiValue - fees;           // leave profit to userBalance
        uint256 balance = _pAmount - cDaiValue - fees + userBalance;
