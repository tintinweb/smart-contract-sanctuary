...
import "@hifi/protocol/contracts/core/fintroller/IFintrollerV1.sol";
import "@hifi/protocol/contracts/core/hToken/IHToken.sol";
import "@hifi/protocol/contracts/oracles/IChainlinkOperator.sol";
...
contract BalanceSheet {
    ...
    function getSeizableCollateralAmount(
        ...
        uint256 numerator,
        uint256 normalizedSeizableCollateralAmount,
        uint256 seizableCollateralAmount
    ) {
        // Grab the normalized USD price of the collateral.
        normalizedCollateralPrice = oracle.getNormalizedPrice(collateral.symbol());

        // Grab the normalized USD price of the underlying.
        normalizedUnderlyingPrice = oracle.getNormalizedPrice(bond.underlying().symbol());

        // Calculate the top part of the equation.
        uint256 liquidationIncentive = fintroller.getLiquidationIncentive(collateral);
        numerator = repayAmount.mul(liquidationIncentive.mul(normalizedUnderlyingPrice));

        // Calculate the normalized seizable collateral amount.
        normalizedSeizableCollateralAmount = numerator.div(normalizedCollateralPrice);
