...
contract YungApes is Ownable, ERC721Enumerable {
  ...
  //Squid ape contract to check balances and owner of tokens
  ...
  //mint for team, includes both ends
  function reserve(uint256 rangeStart, uint256 rangeEnd) external onlyOwner {
    for(uint i = rangeStart; i <= rangeEnd; i++){
      require(!_exists(i));
      mintInternal(msg.sender, i);
    }
  ...
  function doesTokenExist(uint tokenId) external view returns(bool tokenExists){
