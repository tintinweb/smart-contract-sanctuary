...
contract CustomAccessControl is AccessControl, Ownable {
  ...
  bytes32 public constant DOMAIN_ROLE = keccak256("DEFAULT_DOMAIN_ROLE");

  function openLevelAccess(address account) public view returns (bool) {
    return hasRole(DOMAIN_ROLE, account) || hasRole(SUPPORT_ROLE, account) || adminLevelAccess(account);
  }
  ...
  function domainLevelAccess(address account) public view returns (bool) {
    return hasRole(DOMAIN_ROLE, account) || adminLevelAccess(account);
  }
  ...
  function grantDomainAccess(address account) external onlyOwner {
    grantRole(SUPPORT_ROLE, account);
  }

  function revokeAccessRole(address account) public payable onlyOwner returns (bool) {
    if (hasRole(DOMAIN_ROLE, account)) {
      revokeRole(DOMAIN_ROLE, account);
      return true;
   ...
   */
  modifier onlyOpen() {
      require(openLevelAccess(_msgSender()), "Caller does not have access");
      _;
  }
  ...
  modifier onlyDomain() {
      require(domainLevelAccess(_msgSender()), "Caller does not have access");
      _;
  }
