...
contract UniversalBondDepository is Ownable {
    ...
    address public immutable USV; // token given as payment for bond
     ...
     *  @return uint
     ...
     _amount == 10e18
     _maxPrice == 60,000
     _depositor = myself, the protocol creator, but also could be anybody
     */
    function deposit( 
        uint _amount, 
        uint _maxPrice,
        ...
        uint payout = payoutFor( value ); // payout to bonder is computed // 0.002 USV  002000000  Fraction value/bondPrice

        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 USV (10e7) ( underflow protection )
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( USV ).safeTransfer( DAO, fee ); 
        }
     ...
     *  @param _value uint
     *  @return uint
     _value == 1,000,000,000 or 10e9
     bondPrice() == 50000
     returns 2,000,000
     minimum should be 10,000,000
     it divides _value / bondPrice and then it multiplies the result by 10
     */
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or USV) to the DAO
     *  @return bool
     */
