...
// File: contracts/LowGasTestMeth.sol
...
// Need to list what we need
//Two different Costs for pre mint and public mint
//Whitelist 
//Whitelist can only mint 2 per wallet address
//Max supply 
//hidden metadata and hidden image
//payment splitter
...
contract SimpleNftLowerGas is ERC721, Ownable {
  ...
  //TODO add maxMintPreSale TODO
  uint256 public preSaleMintAmount = 2;
  //We need to set the whitelist users after we deploy and 
  //then we need to turn on presale pasue to false to use presale
  ...
  constructor(address _payments) ERC721("NAME", "SYMBOL") {
  ...
  //TODO test the new require to see if owner can free mint
  ...
  //Need to test  this
   function PreSaleMint(uint256 _mintAmount) public payable {
      //perhaps custom supply?
    
    require(!preSalePaused);
    require(_mintAmount > 0);
    require(_mintAmount <= preSaleMintAmount);
    ...
    if (msg.sender != owner()) {
        if(preSalePaused == false) {
            ...
            require(ownerMintedCount + _mintAmount <= preSaleMintAmount, "max NFT per address");
        }
    ...
    //TODO test above and below TODO
   ...
   function isWhitelisted(address _user) public view returns (bool) {
    ...
    function setPreSaleMintAmount(uint256 _limit) public onlyOwner {
    preSaleMintAmount = _limit;
  }
  ...
  function setPaused(bool _state) public onlyOwner {
    ...
    //TODO need to setup payment splitter
