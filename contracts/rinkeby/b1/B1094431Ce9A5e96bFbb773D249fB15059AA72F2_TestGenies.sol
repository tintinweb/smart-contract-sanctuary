 ...
 */
contract TestGenies is ERC721Enumerable, Ownable {
    ...
    modifier whenSaleActive {
        require(isSaleActive, "TheSampleNft: Sale is not active");
        _;
    }

    modifier whenPresaleActive {
        require(isPresaleActive, "TheSampleNft: Presale is not active");
        require(presaleWhitelist[msg.sender] == true , "TheSampleNft: Address is not whitelisted");
        _;
    }

    constructor() ERC721("Test Genies 1", "TGEN1") {
        isSaleActive = false;
    ...
    function presaleMint(uint256 amount) external payable whenPresaleActive {
        ...
        require(publicMinted + amount <= PUBLIC_ITEMS, "TheSampleNft: Purchase would exceed supply cap");
        require(presaleClaimed[msg.sender] + amount <= maxPerPresaleMint, 'Purchase exceeds max allowed presale cap');
        require(mintPrice * amount <= msg.value, "TheSampleNft: Ether value sent is not correct");
