...
contract IMNOTBRICKS is Ownable, ERC721Enumerable, RoyaltiesV2 {
    ...
    function mint() public payable onlyActive onlyNonPaused maxMintCheck returns (uint256) {
        ...
        require(nextTokenId <= maxInvocations, "Must not exceed max invocations.");
        ...
        emit Mint(tokenId, tokenURI(tokenId), msg.sender);
    ...
    function contractDetails() public view returns (Details memory) {
        return Details({
            total: totalSupply(),
            max: maxInvocations,
            maxPerWallet: maxPerAddress,
    ...
    function latestMinted(uint256 numberOfTokens) public view returns (TokenInfo[] memory tokens) {
        uint256 total = totalSupply();
        if (total == 0) {
            tokens = new TokenInfo[](0);
        } else {
            if (numberOfTokens > total) {
                tokens = new TokenInfo[](total);
                uint256 index;
                ...
                tokens = new TokenInfo[](numberOfTokens);
                uint256 index;
                uint256 tokenId = total;
