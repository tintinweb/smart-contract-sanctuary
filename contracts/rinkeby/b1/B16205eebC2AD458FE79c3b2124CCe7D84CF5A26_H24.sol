...
contract H24 is ERC20, AccessControl {
    ...
    /// We run out of WBTC. Please, wait and watch the situation using canClaim
    error NoWBTC();
    /// You need to stake first
    error NoStake();
    /// You can't claim today, please wait
    error CantClaimYet();
    /// No reward set for `date` day
    error NoRewardSet(uint24 date);
    /// Reward must be in 0 <= reward <= 1e6
    error RewardTooBig();
    /// Unable to assign a reward for this day, as users have already collected money `lastClaim`
    error RewardClaimed();
...
//    event SetReward(uint24 indexed date, uint amount);
    ...
    function unstake(uint232 amount) public {
        require(miners[msg.sender].stake >= amount, "Amount is bigger than your stake");
    ...
    function unstakeAll() public {
        if (miners[msg.sender].stake == 0) revert NoStake();
        if (miners[msg.sender].date <= today()) {
            claim();
        }
    ...
    function canUnstake(address addr) public view returns (bool) {
        if (miners[addr].stake == 0) revert NoStake();
    ...
    function claim() public {
        ...
        catch Error(string memory) { revert NoWBTC(); }
    ...
    function canClaim(address addr) public view returns (bool) {
        ...
        if (reward > WBTCI(WbtcContract).balanceOf(WbtcBank) ||
            reward > WBTCI(WbtcContract).allowance(WbtcBank, address(this))) revert NoWBTC();
    ...
    function getUserReward(address addr) public view returns (uint) {
        ...
        if (miners[addr].stake == 0) revert NoStake();
        if (rewards[today_] == 0) revert NoRewardSet(today_);
        if (rewards[miner.date - 1] == 0) revert NoRewardSet(miner.date - 1);
        if (miner.date > today_) revert CantClaimYet();
    ...
    function setReward(uint24 date, uint amount) public onlyRole(ORACLE_ROLE) {
        if (date <= lastClaimed) revert RewardClaimed();
        if (amount > 1e6) revert RewardTooBig();
