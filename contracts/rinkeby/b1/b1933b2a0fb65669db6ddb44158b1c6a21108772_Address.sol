...
contract LockVaiyo is Ownable {
    ...
    function getUSDAmount(uint256 bnbAmount) public pure returns(uint256){
        // uint256 usdAmount = bnbAmount.mul(getBUSDPerBNB());
        // return usdAmount;
        return uint256(5e18);
    }
    
    function getTokensAmount(uint256 usdAmount) public pure returns(uint256) {
        
        uint256 getTokenRate = 0.12 ether; // 20 tokens per 0.1ETH
        uint256 tokensAmount = usdAmount.mul(1e18).div(getTokenRate);
        return tokensAmount;
    }
