...
contract humarsContract is ERC721Enumerable, Ownable {
    ...
    uint256 public stopingPrice = 0.01 ether;
    uint256 public decreaseFactor = 0.005 ether;
    ...
    uint256 public presaleActive = 1;

    mapping(address => bool) public whitelisted; //For public sale if needed
    ...
    function getSaleInfo() internal view virtual returns (uint256) {
        return presaleActive;
    }

    function getPublicsaleStartTime() internal view virtual returns (uint256) {
    ...
    function preSaleMint(address _to ,uint256 _mintAmount, bytes32[] calldata _proof) public payable {
        require(presaleActive == 1, "Presale ended");
    ...
    function publicSaleMint(address _to ,uint256 _mintAmount) public payable {
        require(presaleActive == 0, "Publicsale ended or not started yet");
        uint256 supply = totalSupply();
        require(supply + _mintAmount <= totalCount, "Supply limit reached!");
        require(_mintAmount > 0, "No amount to mint");
        require(_mintAmount <= 10, "You cant mint more than 10");

        if (currentPrice < stopingPrice) {
            currentPrice = stopingPrice;
        }
        else {
            uint256 timeElapsed = block.timestamp - publicsaleStartTime;
            uint256 deduction = (timeElapsed / 6 hours) * decreaseFactor;
            currentPrice = startingPrice - deduction;
        }

        if (msg.sender != owner()) {
            if(whitelisted[msg.sender] != true) {
            require(msg.value >= currentPrice * _mintAmount, "Wrong price!");
            }
        }
    ...
    function startPublicSale() public onlyOwner {
        require(presaleActive == 1, "Public sale already started!");
        _startPublicsale();
    }
