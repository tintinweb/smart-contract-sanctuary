...
contract BaseDutchAuction is ERC721Sequential, LinearDutchAuction, ReentrancyGuard {
    ...
    function mintPublic(uint256 numberOfTokens) external payable nonReentrant {
        ...
        require(nonReservedMinted + numberOfTokens <= nonReservedMax, "Sold out.");
    ...
    function _nonReservedMintHelper(uint256 numberOfTokens) internal {
        ...
        nonReservedMinted += numberOfTokens;
    ...
    function mintReserved(address to, uint256 numberOfTokens) external onlyOwner {
        require(reservedMinted + numberOfTokens <= reservedMax, 'Not enough tokens left in reserve.');

        reservedMinted += numberOfTokens;
        for (uint256 i = 0; i < numberOfTokens; i++) {
...
abstract contract ERC721Sequential is Context, ERC165, IERC721, IERC721Metadata {
...
abstract contract LinearDutchAuction is OwnerPausable {
    /**
