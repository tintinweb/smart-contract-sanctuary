...
contract PikaStaking is Owned {
    ...
    uint256 public minPeriodDuration = 14 minutes;
    uint256 public rewardPerToken = 0;

    event Staked(address indexed user, uint256 amount);
    event Withdraw(address indexed user, uint256 amount);
    event RewardClaimed(address indexed user, uint256 amount);
    event StakingPeriodStarted(uint256 addedRewards, uint256 periodFinish);
     ...
     * @param _rewards amount of tokens to transfer that will be distributed as rewards
     */
    function initNewRewardPeriod(uint256 _rewards) external onlyOwner {
        require(
            block.timestamp > periodFinish,
            "Wait for claim period to finish"
        );
        require(totalAmountStaked > 0, "No tokens staked in contract");
        pika.transferFrom(msg.sender, address(this), _rewards);
        ...
        emit StakingPeriodStarted(_rewards, periodFinish);
    }
    ...
    function setMinDuration(uint256 _days) external onlyOwner {
        emit MinPeriodDurationUpdated(minPeriodDuration / 1 minutes, _days);
        minPeriodDuration = _days * 1 minutes;
    }
}