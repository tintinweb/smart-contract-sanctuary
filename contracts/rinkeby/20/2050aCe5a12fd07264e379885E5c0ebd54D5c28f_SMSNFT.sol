...
contract SMSNFT is ERC721Enumerable, PaymentSplitter, Ownable {
    ...
    uint256 public constant MAX_ZEUS = 5000;
    uint256 public constant MAX_DANTE = 10000;

    uint8 public constant NONE = 0;
    uint8 public constant ZEUS = 1;
    ...
    constructor() ERC721("Sharp Metal Sticks", "SMS") PaymentSplitter(_team, _shares) {
        _apiURI = "https://sharpmetalsticks.com/meta/";
        mintPrice = 5e16;
        counter.dante = 5000;
    }
    ...
    function mintZeus(uint256 amount) public payable {
        ...
        if (army[msg.sender] == NONE) {
            army[msg.sender] = ZEUS;
        } else {
            require(army[msg.sender] == ZEUS, "You can't mint from both armys");
        }
        require(mintPrice.mul(amount) == amount, "Insufficient ETH");
        require(counter.zeus.add(amount) <= MAX_ZEUS, "Exeeds max supply");
        for (uint256 i = 0; i < amount; i++) {
            _safeMint(msg.sender, ++counter.zeus);
        }
    }
    ...
    function setWhitelistActive(bool _acitve) public onlyOwner {
        whitelistActive = _acitve;
    }
    ...
    function removeWhitelist(address[] memory wallets) public onlyOwner {
        for(uint256 i = 0; i < wallets.length; i++) {
