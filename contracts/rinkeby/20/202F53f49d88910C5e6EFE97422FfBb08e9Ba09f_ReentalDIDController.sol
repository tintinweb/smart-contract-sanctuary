...
import "./ReentalDIDFactory.sol";
import "./ReentalDeployer.sol";

contract ReentalDIDController is UUPSUpgradeableByRole {
    ...
    function setAddress(bytes32 id, address addr) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(!_lockedIds[id], "ReentalDIDController: ID is locked");
        IReentalDeployer(config).setId(id, addr);
        _verified[addr] = true;

        emit SetAddress(id, addr, _msgSender());
    }

    function lockId(bytes32 id) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _lockedIds[id] = true;
    ...
    //consider required elapsed time/blocks
    function upgradeImplementation(address newImplementation) public onlyRole(UPGRADER_ROLE) {
        UpgradeableBeacon(IReentalDeployer(config).get(keccak256("REENTAL_DID_FACTORY"))).upgradeTo(newImplementation);
    }

    function registerDID(address owner_, address did_) public {
        require(msg.sender == IReentalDeployer(config).get(keccak256("REENTAL_DID_FACTORY")), "ReentalDIDController: only callable by factory");
        require(isEmptyDID(did_), "ReentalDIDController: no empty");
        _registerDID(owner_, did_);
    }

    function updateRegistry(address owner_, address did_) public {
        require(did_ == msg.sender, "ReentalDIDController: only did can update registry");
        _registerDID(owner_, did_);
    }

    function isEmptyDID(address did_) public view returns(bool) {
        return ownerByDid[did_] == address(0);
    }
...
import "./utils/UUPSUpgradeableByRole.sol";
import "./interfaces/IReentalDIDController.sol";
import "./ReentalDID.sol";
import "./ReentalDeployer.sol";

contract ReentalDIDFactory is UUPSUpgradeableByRole, EIP712Upgradeable {
    address public beacon;

    // solhint-disable-next-line var-name-mixedcase
    bytes32 private immutable _DID_FACTORY_TYPEHASH =
        // keccak256("CreateProxy(uint256 deadline)");
        keccak256("CreateProxy(address controller,address owner,address master,uint256 deadline)");

    event NewDID(address proxy, address controller, address owner, address master, address creator);
    ...
    function initialize () public initializer {
        ...
        UpgradeableBeacon _beacon = new UpgradeableBeacon(address(new ReentalDID()));
        beacon = address(_beacon);

        address controller = IReentalDeployer(config).get(keccak256("REENTAL_DID_CONTROLLER"));
        require(controller != address(0), "ReentalDIDFactory: cant initialize without controller");
        _beacon.transferOwnership(controller); //Beacon upgrader
    }

    function createProxy(bytes calldata initializeCalldata) public onlyRole(keccak256("REENTAL_DID_CREATOR")) returns(address proxy) {
        ...
        returns(address proxy) 
    {
        ...
        require(hasRole(keccak256("REENTAL_DID_CREATOR"), signer), "ReentalDIDFactory: role cant createProxy");

        (address controller,address owner,address master) = abi.decode(initializeCalldata[4:], (address, address, address));
        bytes32 structHash = keccak256(abi.encode(_DID_FACTORY_TYPEHASH, controller, owner, master, deadline));
    ...
    // ) 
    //     public 
    //     returns(address proxy, address owner, address master) 
    // {
    //     require(block.timestamp <= deadline, "ReentalDIDFactory: expired deadline");

    //     address controller;
    //     (controller, owner, master) = abi.decode(initializeCalldata[4:], (address, address, address));
    //     bytes32 structHash = keccak256(abi.encode(_DID_FACTORY_TYPEHASH, controller, owner, master, deadline));
    //     bytes32 hash = _hashTypedDataV4(structHash);
    //     address signer = ECDSA.recover(hash, v, r, s);
    //     require(hasRole(REENTAL_DID_CREATOR, signer), "ReentalDIDFactory: role cant createProxy");
    ...
    function _createProxy(bytes calldata initializeCalldata) private whenNotPaused() returns (address proxyAddress){
        ...
        emit NewDID(proxyAddress, controller, owner, master, msg.sender);
    }

    function _registerDID(address owner_, address did_) private {
        IReentalDIDController(IReentalDeployer(config).get(keccak256("REENTAL_DID_CONTROLLER"))).registerDID(owner_, did_);
    }
...
interface IReentalDIDController {
    ...
    function registerDID(address owner_, address did_) external;
}
...
import "./utils/MasterableUpgradeable.sol";
import "./ReentalDIDStorage.sol";
import "./interfaces/IReentalDID.sol";
    ...
    ReentalDIDStorage 
{
    ...
    modifier masterOrReental() {
        require(_isMaster(msg.sender), "ReentalDID: onlyMaster");
        _;
    }

    modifier ownerOrMaster() {
        require(msg.sender == owner() || _isMaster(msg.sender), "ReentalDID: ownerOrMaster");
        _;
    }
    ...
    function transferMastership(address newMaster) masterOrReental() public override {
        _transferMastership(newMaster);
    }

    function pause() public ownerOrMaster(){
        _pause();
    }
    
    function unpause() public masterOrReental(){
        _unpause();
    }

    function canForward(address destination_) public view returns(bool) {
        return (!locked && _verificationCheck(destination_));
    }
    ...
    function _isMaster(address account) internal view returns(bool) {
        ...
        address config = controller.config();
        IAccessControlUpgradeable configInterface = IAccessControlUpgradeable(config);
        return configInterface.hasRole(REENTAL_DID_MASTER_ROLE, account);
    }
    ...
    function initialize(address controller_, address owner_, address master_) initializer public {
        ...
        __ReentalDID_init(controller_);
    }

    function __ReentalDID_init(address controller_) initializer internal {
        __ReentalDID_init_unchained(controller_);
    }

    function __ReentalDID_init_unchained(address controller_) initializer internal {
        controller = ReentalDIDController(controller_);
    }
}
...
import "./ReentalDIDController.sol";
...
contract ReentalDIDStorage {
    ...
    ReentalDIDController public controller;
