...
library NFTDescriptor {
    ...
    function generateDescriptionPartOne(
        ...
        return
            string(
                abi.encodePacked(
                    'This NFT represents a liquidity position in a BitrielSwap ',
                    quoteTokenSymbol,
                    '-',
                    baseTokenSymbol,
                    ' pool. ',
    ...
    function adjustForDecimalPrecision(
        ...
        if (difference > 0 && difference <= 18) {
            if (baseTokenDecimals > quoteTokenDecimals) {
                ...
                if (difference % 2 == 1) {
                    adjustedSqrtRatioX96 = adjustedSqrtRatioX96.mulDiv(sqrt10X128, 1 << 128);
                }
            } else {
                adjustedSqrtRatioX96 = sqrtRatioX96.div(10**(difference.div(2)));
                if (difference % 2 == 1) {
                    adjustedSqrtRatioX96 = adjustedSqrtRatioX96.mulDiv(1 << 128, sqrt10X128);
                }
            }
        } else {
    ...
    function fixedPointToDecimalString(
        ...
        uint256 value = adjustedSqrtRatioX96.mulDiv(adjustedSqrtRatioX96, 1 << 64);
        ...
        if (priceBelow1) {
            ...
            value = value.mulDiv(10**44, 1 << 128);
        } else {
            // leave precision for 4 decimal places + 1 place for rounding
            value = value.mulDiv(10**5, 1 << 128);
        }
...
/// @title NFTSVG
/// @notice Provides a function for generating an SVG associated with a BitrielSwap NFT
library NFTSVG {
    using Strings for uint256;
    ...
    function generateSVG(SVGParams memory params) internal pure returns (string memory svg) {
        ...
        msg: "Forged in SVG for BitrielSwap in 2021 by 0xe8ab59d3bcde16a29912de83a90eb39628cfc163",
