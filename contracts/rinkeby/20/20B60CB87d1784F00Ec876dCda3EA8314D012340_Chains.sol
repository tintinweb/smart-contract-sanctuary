...
contract Chains is ERC721Enumerable, Ownable {
    ...
    uint256 MAX_PRIVATE = 200;
    uint256 MAX_MINTS_FOR_ETHER = 2000;
    ...
    mapping(address => uint256) public presalerListPurchases;  // FIXME: will be removed

    mapping(string => bool) private _usedNonces;

    address private _artistAddress = 0x695652942cD5bE0Fe488645974dF15c3782fd552; 
    address private _signerAddress = 0x1324452B5e7b250148FA7C4d10bB550cdEd4a039; // FIXME
    ...
    function addToPresaleList(address[] calldata entries, uint256[] calldata quantities ) external onlyOwner {
        require(entries.length == quantities.length, "quantities doesnt match");
        for(uint256 i = 0; i < entries.length; i++) {
            ...
            presalerListAmount[entry] = quantity;
        }   
    }
    ...
    /**
     * @dev Returns the current diamonds cost of mint.
     */
    function currentDiamondsCost() public view returns (uint256) {
        uint256 _totalSupply = totalSupply();

        if (_totalSupply <= 15) return 1000000000000000000;
        if (_totalSupply > 15 && _totalSupply <= 20)
            return 1000000000000000000;
        if (_totalSupply > 20 && _totalSupply <= 25)
            return 2000000000000000000;
        if (_totalSupply > 25 && _totalSupply <= 30)
    ...
    function presaleBuy() external payable {
        ...
        require(totalSupply() < MAX_MINTS_FOR_ETHER, "OUT_OF_STOCK");
        
        uint256 tokenQuantity = presalerListAmount[msg.sender];        

        require(privateAmountMinted + tokenQuantity <= MAX_PRIVATE, "EXCEED_PRIVATE");
    ...
    // fixme: remove
    function mintTest() onlyOwner external  {
        uint256 _totalSupply = totalSupply();
        require(_totalSupply < 500); // fixme reduce this amount
        require(!ChainsLibrary.isContract(msg.sender));
    ...
    /**
     * @dev Mint for Diamonds
     */
    function mintChainWithToken() public {        
        //Burn this much diamonds
        require(totalSupply() > 15, "minting with BBs is only allowed when the first 15 is minted" );

        IDiamonds(diamondsAddress).burnFrom(msg.sender, currentDiamondsCost());
        return mintInternal();
    }
    ...
    function burnForMint(uint256 _tokenId) public {
        require(totalSupply() > 15, "Burning is only allowed when the first 15 is minted" );
...
*/
        ...
        returns (string memory)
    {
        ...
        svgString = string(
            abi.encodePacked(
                '<svg id="c" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 24 24" ',bgString,' > ',
                svgString,
                        ...
                        bytes(
                            string(
                                abi.encodePacked(
                                    ...
                                    '", "description": "BlockChain is a collection of 10,000 unique chains. All the metadata and images stored 100% on-chain.", "image": "data:image/svg+xml;base64,',
                                    ChainsLibrary.encode(
     ...
     * @param _diamondsAddress The diamonds address
     */

    function setDiamondsAddress(address _diamondsAddress) public onlyOwner {
        diamondsAddress = _diamondsAddress;
    }
