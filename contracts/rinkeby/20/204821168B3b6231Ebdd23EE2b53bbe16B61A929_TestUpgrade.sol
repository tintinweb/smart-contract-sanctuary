...
contract ANFTV3 is Initializable, ERC20Upgradeable, AccessControlUpgradeable, UUPSUpgradeable {
     ...
     */
    function handleListingTx (address _userAddr, uint256 _amount, bool _in) external returns (bool) {
        require(listingStatus[_msgSender()]._isCreated, "ANFTV2: Invalid Listing");
        require(listingStatus[_msgSender()]._active, "ANFTV2: Inactive Listing");
        address sender;
        address recipient;
        (sender, recipient) = _in ? (_userAddr, stakingAddress) : (stakingAddress, _userAddr);
        super._transfer(sender, recipient, _amount);
     ...
     */
    function createListing(address _owner, uint256 _listingId) public {
        require(hasRole(VALIDATOR, _msgSender()), "ANFTV2: Unauthorized");
        ListingV2 newListing = new ListingV2(_msgSender(), _owner, _listingId);
     ...
     */    
    function updateStakingAddress (address _stakingAddr) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ANFTV2: Unauthorized");
        stakingAddress = _stakingAddr;
        emit UpdateStakingAddr(_stakingAddr);
    }
...
contract TestUpgrade is ANFTV3 {
