...
contract BaboonBrigade is Ownable, ERC721 {
    ...
    uint256 public finalPresaleCount; // last premint token created
    ...
    function buyPremint(uint256 tokenQuantity) external payable {
        ...
        require(
            lastPresaleId + tokenQuantity <= finalPresaleCount,
    ...
    function premint(uint256 count) external onlyOwner {
        ...
        require( count + lastTokenId + 1 < BBG_MAX, "Sorry, there is not that many BBG left.");

        _premint(creatorAddress, lastTokenId + 1, count);
        lastTokenId += count;
        finalPresaleCount += count;
    }
