...
contract ExchangeNFT is Ownable {
    ...
    struct Data {
        address[3] tradeAddress;
        address[2] feeAddress;
    ...
    modifier verifySignature(
        uint256 nonce,
        address[3] memory _tradeAddress,
        address[2] memory _feeAddress,
    ...
    function checkFeeProductExits(
        address[2] memory _feeAddress,
        uint256[4] memory _attributes
    ) internal pure returns (uint256 amount, uint256 feeOwner, uint256 feeAdmin) {
        uint256 totalFeeTrade;
        // Check fee for owner
        if (_feeAddress[0] != address(0)) {
            feeOwner = _attributes[0].mul(_attributes[2]).div(100);
            totalFeeTrade.add(feeOwner);
        }
        // Check fee for admin
        if (_feeAddress[1] != address(0)) {
            feeAdmin = _attributes[0].mul(_attributes[3]).div(100);
            totalFeeTrade.add(feeAdmin);
        }
        amount = _attributes[0].sub(totalFeeTrade);
    }

    // Buy NFT normal by token ERC-20
    // address[3]: buyer, seller, token
    // address[2]: fee, feeAdmin
    ...
    function buyNFTNormal(
        ...
        uint256 nonce,
        bytes memory signature
    ) external verifySignature(nonce, _tradeAddress, _feeAddress, _attributes, signature) {
    ...
    function buyNFTETH(
        ...
        external
        payable
        verifySignature(nonce, _tradeAddress, _feeAddress, _attributes, signature)
    {
        Data memory tradeInfo = Data({
            tradeAddress: _tradeAddress,
            feeAddress: _feeAddress,
        ...
        if (feeOwner != 0) {
            TransferHelper.safeTransferETH(tradeInfo.feeAddress[0], feeOwner);
        }

        // transfer eth to admin address
        if (feeAdmin != 0) {
            TransferHelper.safeTransferETH(tradeInfo.feeAddress[1], feeAdmin);
        }
