...
contract Lootchadores is ERC721Enumerable, Ownable, ERC165Storage, PaymentSplitter {
    ...
    mapping(uint256 => uint256) internal luchaAbilites;
    ...
    function renderStars(uint8 starRating) internal pure returns (bytes memory) {
        return
            abi.encodePacked(
                renderStar(starRating >= 1, 255),
                renderStar(starRating >= 2, 285),
                renderStar(starRating >= 3, 315),
                renderStar(starRating >= 4, 345),
                renderStar(starRating >= 5, 375)
            );
    }
    ...
    function renderAttributes(uint256 tokenId) internal view returns (bytes memory) {
        return
            abi.encodePacked(
                renderAttribute(extractValue(IAbilities(ABILITY_SCORES_ADDRESS).getCharisma(tokenId)), " CHA", 128, 780),
                renderAttribute(extractValue(IAbilities(ABILITY_SCORES_ADDRESS).getConstitution(tokenId)), " CON", 320, 780),
                renderAttribute(extractValue(IAbilities(ABILITY_SCORES_ADDRESS).getDexterity(tokenId)), " DEX", 512, 780),
                renderAttribute(extractValue(IAbilities(ABILITY_SCORES_ADDRESS).getIntelligence(tokenId)), " INT", 128, 830),
                renderAttribute(extractValue(IAbilities(ABILITY_SCORES_ADDRESS).getStrength(tokenId)), " STR", 320, 830),
                renderAttribute(extractValue(IAbilities(ABILITY_SCORES_ADDRESS).getWisdom(tokenId)), " WIS", 512, 830)
            );
    }
    ...
    function extractValue(string memory ability) internal pure returns (string memory) {
        strings.slice memory _ability = ability.toSlice();
        _ability.beyond(": ".toSlice());
        return _ability.toString();
    }
