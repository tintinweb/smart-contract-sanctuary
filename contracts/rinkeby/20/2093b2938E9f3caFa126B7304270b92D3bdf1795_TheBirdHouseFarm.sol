...
1 bird seed per second

*/

    using SafeMath for uint256;

    uint256 emissionsRate = 1000000000000000000;
    ...
    //Mapping of OG bird to staker
    mapping(uint256 => address) internal tokenIdToStaker;

    //Mapping of OG bird to staker
    mapping(address => uint256[]) internal stakerToTokenIds;

    constructor(address theBirdHouseAddress) ERC20("BirdSeed", "SEED") {
    ...
    function removeTokenIdFromStaker(address staker, uint256 tokenId) internal {
        for (uint256 i = 0; i < stakerToTokenIds[staker].length; i++) {
            uint256 thisTokenId = stakerToTokenIds[staker][i];
            if (thisTokenId == tokenId) {
    ...
    function addTokenIdToStaker(address staker, uint256 tokenId) internal {
        stakerToTokenIds[staker].push(tokenId);
    }

    function stake(uint256 tokenId) public {
        require(
            TheBirdHouseContract.ownerOf(tokenId) == msg.sender,
            "You must own the token you are staking!"
        );
        require(
            tokenIdToStaker[tokenId] == nullAddress,
            "Token must be not staked!"
        );

        TheBirdHouseContract.transferFrom(msg.sender, address(this), tokenId);

        addTokenIdToStaker(msg.sender, tokenId);
    ...
    function unstake(uint256 tokenId) public {
        require(
            TheBirdHouseContract.ownerOf(tokenId) == address(this),
        ...
        require(
            tokenIdToTimeStamp[tokenId] < block.timestamp,
            "Can only unstake after one block!"
        );

        TheBirdHouseContract.transferFrom(address(this), msg.sender, tokenId);

        _mint(
            msg.sender,
            ((block.timestamp - tokenIdToTimeStamp[tokenId]) * emissionsRate)
        );
    ...
    function getRewardsByTokenId(uint256 tokenId) public view returns (uint256) {
        ...
        return secondsStaked * emissionsRate;
    }
