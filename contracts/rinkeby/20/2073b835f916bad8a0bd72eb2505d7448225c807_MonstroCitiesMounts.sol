...
uint256 public constant SNAPSHOT_TIME = 1630691100;
  ...
  function mintFREEMount(uint256 monstroTokenId) public {
      
      // first snapshot at time of contract deployment
      
    if (block.timestamp <= SNAPSHOT_TIME) {
    ...
    trySetStartingIndexBlock();
          ...
          //  if minted between snapshot and week after for free mints
    
    else if (block.timestamp >= SNAPSHOT_TIME) {
    
    require(isWhitelisted(msg.sender), "Is not whitelisted");
    require(saleIsActive, "Sale must be active to mint a Mount");
    require(totalSupply() < maxMounts, "Purchase would exceed max supply of Mounts");
    require(monstroTokenId < maxMounts, "Requested tokenId exceeds upper bound");
    require(monstro.ownerOf(monstroTokenId) == msg.sender, "Must own the Monster for requested tokenId to mint a Mount");
 ...
 function mintMountPAID(uint256 MAX_MINT, uint256 monstroTokenId) public payable {
        // Exceptions that need to be handled + launch switch mechanic
        require(block.timestamp >= SNAPSHOT_TIME);
         require(saleIsActive, "Sale must be active to mint a Mount");
        require(totalSupply() < maxMounts, "Purchase would exceed max supply of Mounts");
        require(monstroTokenId < maxMounts, "Requested tokenId exceeds upper bound");
      require(monstro.ownerOf(monstroTokenId) == msg.sender, "Must own the Monster for requested tokenId to mint a Mount");
