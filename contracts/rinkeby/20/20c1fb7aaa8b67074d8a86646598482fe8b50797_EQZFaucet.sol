...
contract EQZFaucet {
    ...
    modifier onlyModerators () {
        require (moderators[msg.sender], "only moderators");
        _;
    }
    ...
    function addModerator (address mod) public onlyModerators {
        moderators[mod] = true;
    }
    ...
    function faucet(address receiver) public onlyModerators {
        require (addressFilled[receiver] == false, "Already Requested");
        addressFilled[receiver] = true;
        (uint8 index1, uint8 index2, uint8 index3) = random();
        
        require(IERC20(tokens[index1]).transfer(receiver, 1000 * 10 ** IERC20(tokens[index1]).decimals()), "transfer failed");
        require(IERC20(tokens[index2]).transfer(receiver, 1000 * 10 ** IERC20(tokens[index2]).decimals()), "transfer failed");
        require(IERC20(tokens[index3]).transfer(receiver, 1000 * 10 ** IERC20(tokens[index3]).decimals()), "transfer failed");
