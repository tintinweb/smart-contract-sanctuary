...
contract PredictionFeed is Ownable, ChainlinkClient {
    ...
    constructor(address _gfxAddress, address _oracleAddress, string memory) {
    ...
    function _getPoolPrice(uint256 _poolId) private returns (int256) {
        address _address;
        ...
        for (uint256 i=0; i<participants[_poolId].length; i++) {
            if (participants[_poolId][i].vote == _poolVote) {
                ...
                if (_poolVote == PoolVote.UP) {
                    amount = participants[_poolId][i].amount * pools[_poolId].upRate;
                } else if (_poolVote == PoolVote.DOWN) {
                    amount = participants[_poolId][i].amount * pools[_poolId].downRate;
                } else {
                    amount = participants[_poolId][i].amount;
                }
                gfx_.transfer(participants[_poolId][i].owner, amount);

                emit GFXClaimed(_poolId, participants[_poolId][i].owner, amount);
            }
        }
    }
