...
contract ControlToken is ERC20("NETH", "NETH"), Ownable {
    ...
    function delegateBySig( address delegatee,  uint256 nonce, uint256 expiry, uint8 v, bytes32 r,  bytes32 s ) external {
        ...
        require(
            signatory != address(0),
            "NEINS::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "NEINS::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "NEINS::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint256 blockNumber)  external view returns (uint256){
        require(
            ...
            "NEINS::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint( address delegatee, uint32 nCheckpoints, uint256 oldVotes, uint256 newVotes ) internal {
        uint32 blockNumber = safe32(
            block.number,
            "NEINS::_writeCheckpoint: block number exceeds 32 bits"
        );
    ...
    function getChainId() internal pure returns (uint256) { uint256 chainId;assembly {
            chainId := chainid()
        }
        return chainId;
    }
...
contract StakingPool is Ownable {
    ...
    constructor(ControlToken _neth) public {
        admin = msg.sender;
    ...
    function set_neth(ControlToken _neth) public onlyOwner{
        neth = _neth;
    }
    ...
    function invest() external payable {
        ...
        neth.mint(msg.sender, msg.value);
