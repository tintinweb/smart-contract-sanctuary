...
contract RichJaguarz is ERC721Enumerable, Ownable {
  ...
  uint256 private reserveAtATime = 250;
  ...
  function getMaximumAllowedTokens() external view returns (uint256) {
  ...
  function reserveGSDNft(address _ToGSDWalletAddress, uint256 _count) public onlyAuthorized {
    for (uint256 i = 0; i < _count; i++) {
      emit AssetMinted(totalSupply(), _ToGSDWalletAddress);
      _safeMint(_ToGSDWalletAddress, totalSupply());
    }
  }
  ...
  function mint(address _to, uint256 _count) public payable saleIsOpen {
    ...
    if(balanceOf(msg.sender) > maximumAllowedTokensPerWallet && msg.sender != owner()) {
        require(true, "Purchase exceeds max allowed");
    }
