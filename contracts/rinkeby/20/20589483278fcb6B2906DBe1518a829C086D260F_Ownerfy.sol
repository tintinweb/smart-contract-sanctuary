// contracts/Ownerfy-Contract-1.sol
...
contract Ownerfy is Context, AccessControlEnumerable, ERC1155Burnable {
  ...
  string private _contractName = 'Ownerfy';
  ...
  // ImageHashes
    mapping (uint256 => bytes32) public imageHashes;
     ...
     */
    function mint(address to, uint256 id, uint256 amount, bytes memory data, bytes32 imageHash, string calldata _update) public virtual {
     ...
     */
    function mintBatch(address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data, bytes32[] memory _imageHashes, string[] calldata updates) public virtual {
        require(hasRole(MINTER_ROLE, _msgSender()), "Ownerfy: must have minter role to mint");

        _mintBatch(to, ids, amounts, data);

        for (uint i = 0; i < ids.length; i++) {
          if (_imageHashes[i].length > 0) {
            imageHashes[ids[i]] = _imageHashes[i];
          }
    ...
    function imageHashOf(uint256 id) public view returns (bytes32) {
