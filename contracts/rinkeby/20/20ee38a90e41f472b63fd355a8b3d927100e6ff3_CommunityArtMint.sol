...
contract CommunityArtNFT is ERC721Enumerable, Ownable {
    ...
    /// Maps a seed to its token.
    mapping (uint256 => uint256) public seed2tokenid;
    ...
    function getTokenBySeed(uint256 seed) public view virtual returns (uint256) {
        return seed2tokenid[seed];
    }
...
contract CommunityArtMint is Ownable {
    ...
    uint256 public mintPrice = 0.01 ether; // Change to 0.1 later
    ...
    function mintMinor(uint256 edition, uint256 seed) public payable {
        ...
        require(seed2tokenid[seed] == 0, "Seed already exists, choose another one");
        ...
        seed2tokenid[seed] = tokenId;
        nftContract.mint(msg.sender, tokenId, seed);
    ...
    function mintMajor(uint256 edition) external onlyOwner() {
        ...
        nftContract.mint(address(auctionContract), edition * EDITION_SIZE, 0);
    }
