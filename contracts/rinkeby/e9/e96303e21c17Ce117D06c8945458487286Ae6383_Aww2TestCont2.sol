...
abstract contract Aww2Test {
  function ownerOf(uint256 tokenId) public virtual view returns (address);
  function tokenOfOwnerByIndex(address owner, uint256 index) public virtual view returns (uint256);
  function balanceOf(address owner) external virtual view returns (uint256 balance);
}

contract Aww2TestCont2  is ERC721Enumerable, Ownable
{
    using SafeMath for uint256;
    using Address for address;
    using Strings for uint256;

    Aww2Test private aww = Aww2Test(0x4100aBf1cA888a9cE5C75dF87594A17d6D02B7b2); 
    ...
    constructor(string memory baseURI) ERC721("Aww2Test", "A2T")  {
        setBaseURI(baseURI);
    }
    ...
    function mintForOneToken(uint awwTokenId) public payable isSaleOpen{
        ...
        if (!_exists(awwTokenId)) {
            _safeMint(msg.sender, awwTokenId);
        }
    }
 
    function mintForNumberOfTokens(uint mintedTokens, uint chosenAmount) public payable isSaleOpen{
        require(isPaused == false, "Sale is not active at the moment");
        ...
        for(uint i = 0; i < balance && i < chosenAmount && i < maxQuantity; i++) {
            ...
            uint tokenId = aww.tokenOfOwnerByIndex(msg.sender, i + mintedTokens);
            if (!_exists(tokenId)) {
