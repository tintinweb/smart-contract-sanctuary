...
contract EscrowV1 is EscrowContract, AmendablePreSigned, WithPreSignedMilestones {
    ...
    string private constant ERROR_INVALID_SETTLEMENT = "Settlement must be 100%";
    ...
    function registerContract(
        ...
        for (uint8 _i=0; _i<_milestones.length; _i++) {
            ...
            _registerMilestoneStorage(
                ...
                _mp.deliveryDeadline,
                _mp.amount
            );
            emit NewMilestone(_cid, _index, _mid, _mp.paymentToken, _mp.escrowDisputeManager, _mp.deliveryDeadline, _mp.amount);
        }
    }
     ...
     * @param _deliveryDeadline UNIX timestamp for delivery deadline, pass 0 if none.
     ...
     */
    function dispute(bytes32 _cid, uint8 _index, bool _ignoreCoverage) external isDisputer(_cid) {
     ...
     */
    function submitEvidence(bytes32 _cid, uint8 _index, bytes calldata _evidence) external {
    ...
    function appealDispute(
        ...
        bytes32 _mid = _genMid(_cid, _index);
        milestones[_mid].escrowDisputeManager.appealDispute(msg.sender, _cid, _index, _lastRoundId, _ruling, _feeToken, _appealFee);
    }
    ...
    function confirmAppealDispute(
        ...
        bytes32 _mid = _genMid(_cid, _index);
        milestones[_mid].escrowDisputeManager.confirmAppealDispute(msg.sender, _cid, _index, _lastRoundId, _ruling, _feeToken, _confirmAppealFee);
    }
     ...
     */
    function resolveDispute(bytes32 _cid, uint8 _index) external {
        bytes32 _mid = _genMid(_cid, _index);
        Milestone memory _m = milestones[_mid];
     ...
     * @param _index Number of milestone in dispute.
     */
    function executeSettlement(bytes32 _cid, uint8 _index) external {
        bytes32 _mid = _genMid(_cid, _index);
        Milestone memory _m = milestones[_mid];
        uint _available = _m.fundedAmount - _m.claimedAmount;
        require(_available > 0, ERROR_NO_MONEY);
        ...
        if (_ruling == RULE_PAYER_WON || _ruling == RULE_PAYEE_WON) {
            ...
            address _arbiter = disputer.ARBITER();
...
abstract contract Amendable is ContractContext, MilestoneContext {
     ...
     * @return unique storage key for amendment.
     */
    function genSettlementKey(bytes32 _cid, uint8 _index, uint8 _revision) public pure returns(bytes32) {
     ...
     * @param _revision Current version of milestone extended terms.
     */
    function signApproveAndExecuteMilestoneSettlement(bytes32 _cid, uint8 _index, uint8 _revision) public {
    ...
    function _proposeMilestoneSettlement(
        ...
        settlementProposals[_key] = SettlementProposal({
            ...
            deliveryDeadline: _settlementParams.deliveryDeadline,
     ...
     * @param _revision Current version of milestone extended terms.
     */
    function _approveAndExecuteMilestoneSettlement(bytes32 _cid, uint8 _index, uint8 _revision) internal {
        bytes32 _key = genSettlementKey(_cid, _index, _revision);
        ...
        if (_sp.deliveryDeadline != _m.deliveryDeadline) milestones[_mid].deliveryDeadline = _sp.deliveryDeadline;
...
abstract contract WithMilestones is ContractContext, MilestoneContext {
     ...
     *
     * If milestone supports automatic releases by deliveryDeadline,
    ...
    function withdrawMilestone(bytes32 _cid, uint8 _index) public {
        ...
        if (_m.releasedAmount == 0 && _inEscrow > 0 && isAutoReleaseAvailable(_mid, _m.escrowDisputeManager, _m.deliveryDeadline)) {
    ...
    function _registerMilestone(
        ...
        emit NewMilestone(_cid, _index, _mid, _paymentToken, _escrowDisputeManager, _deliveryDeadline, _amount);
    }
     ...
     * @param _deliveryDeadline UNIX timestamp for maturity date.
    ...
    function isAutoReleaseAvailable(
        ...
        return _deliveryDeadline > 0 && block.timestamp > _deliveryDeadline &&
...
abstract contract WithPreSignedMilestones is EIP712, WithMilestones {
     ...
     */
    function refundPreApprovedMilestone(bytes32 _cid, uint8 _index, uint _amount, bytes calldata _payeeDelegateSignature) public {
...
interface IEscrowDisputeManager {
    ...
    function disputeSettlement(address _feePayer, bytes32 _cid, uint8 _index, bytes32 _termsCid, bool _ignoreCoverage) external;
    function executeSettlement(bytes32 _cid, uint8 _index, bytes32 _mid) external returns(uint256, uint256, uint256);
    
    function dispute(address _feePayer, bytes32 _cid, uint8 _index, bytes32 _termsCid, bool _ignoreCoverage) external;
    function submitEvidence(address _from, string memory _label, bytes32 _cid, uint8 _index, bytes calldata _evidence) external;
