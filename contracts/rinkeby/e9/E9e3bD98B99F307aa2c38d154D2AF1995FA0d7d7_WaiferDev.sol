...
contract WaiferDev is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    uint256 public maxContractAmount = 100 * 10 ** 9;
    ...
    event SetMaximumContractAmount(uint256 minimumTokensBeforeSwap);
    ...
    /**
     * @dev maximum contract limit setting function.
     * @param contractAmount Number's for contract amount.
     */
    function setMaximumContractAmount(uint256 contractAmount) external onlyOwner whenNotPaused {
        maxContractAmount = contractAmount;
        emit SetMaximumContractAmount(maxContractAmount);
    }
    ...
    function _transfer(
        ...
        bool overMaxMarketingWallet = minimumTokensBeforeSwap >= contractTokenBalance;
        if(overMaxMarketingWallet && marketingFeeEnabled) {
        ...
        bool overMaxTokenBalance = contractTokenBalance >= maxContractAmount;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) internal {
        (uint256 tTransferAmount, uint256 tFee, uint256 tMarketingFee) = getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 rMarketingFee) = getRValues(tAmount, tFee, tMarketingFee);
    ...
    function getTValues(uint256 amount) internal view returns (uint256, uint256, uint256) {
        ...
        return (tTransferAmount, tFee, tMarketingFee);
    }

    function getRValues(uint256 amount, uint256 tFee, uint256 tMarketingFee) internal view returns (uint256, uint256, uint256, uint256) {
        uint256 currentRate = getRate();
        uint256 tAmount = amount;
        uint256 rAmount = tAmount * currentRate;
        uint256 rFee = tFee * currentRate;
        uint256 rMarketingFee = tMarketingFee * currentRate;
        uint256 rTransferAmount = rAmount - rFee - rMarketingFee;
