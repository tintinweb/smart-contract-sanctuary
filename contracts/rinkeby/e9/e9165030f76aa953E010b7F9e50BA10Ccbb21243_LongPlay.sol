...
// Project Strange Noise
contract LongPlay is
    ERC721,
    ERC721Enumerable,
    ERC721URIStorage,
    Ownable
{
    ...
    uint256 public publicSupply = 195;
    ...
    function setAvailable(uint256 amount) public onlyOwner {
        require(availableSupply.add(amount) < publicSupply, 'exceeds public supply');
    ...
    function mint(uint256 num) public payable {
        require(
            _tokenIdCounter.current().add(num) <= availableSupply,
            "no availability"
        );
        require(
            _tokenIdCounter.current().add(num) <= publicSupply,
            "resource exhausted"
        );
        require(num <= maxPerTx, "request limit exceeded");
        require(price().mul(num) <= msg.value, "not enough funds");
        require(_tokenCount[msg.sender] < 3, "token max reached");
        require(_tokenCount[msg.sender].add(num) < 3, "exceeds token limit");
