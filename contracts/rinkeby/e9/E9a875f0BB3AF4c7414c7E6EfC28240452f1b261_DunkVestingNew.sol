...
contract DunkVestingNew is Ownable {
    ...
    function reset() public {
        ...
        for (uint8 i = 0; i < addresses.length; i++) {
            ...
            users[addresses[i]].isLocked = false;

            users[addresses[i]].unlockedAmount = users[addresses[i]].unlockedQuarters = 0;
        }

        token.transfer(owner(), token.balanceOf(address(this)));
    }

    function initQuarters() private {
    ...
    function initDistributions() private {
    ...
    function getUnlockablePercent (address addr) public view returns (uint256, uint256) {
        
        uint256 allowedPercent;

        uint256 unlockableQuarters = users[addr].unlockedQuarters;

        uint256 categoryIndex = users[addr].categoryIndex;
    ...
    function unlock() public {
        ...
        (uint256 availablePercent, uint unlockableQuarters) = getUnlockablePercent(msg.sender);
