...
contract ForgeToken is ERC1155PresetMinterPauser, Ownable {
    ...
    uint256 public ethBurnGasFee;
    uint256 public totalBurnGasFee = 0;
    event BurnGasFeeReceived(address fromAccount, uint256 amountBurnGasFee);
    event BurnGasFeeSent(address toAccount, uint256 amount);
    ...
    function buyWithETH(
        ...
        uint256 amountBurnGasFee = ethBurnGasFee;

        require(
            msg.value >= amountFee.add(amountBurnGasFee),
            "Not enough ETH sent"
        );
        ...
        // Refund
        if (msg.value > amountFee.add(amountBurnGasFee)) {
            _msgSender().transfer(
                msg.value.sub(amountFee).sub(amountBurnGasFee)
            );
        }

        // Register the received "ethBurnGasFee" and emit the event.
        totalBurnGasFee = totalBurnGasFee.add(amountBurnGasFee);
        emit BurnGasFeeReceived(_msgSender(), amountBurnGasFee);
    }
    ...
    function buyWithZUT(
        ...
        zut.safeTransferFrom(_msgSender(), feeRecipient, realZutFee);
    ...
    function setEthBurnGasFee(uint256 _ethBurnGasFee) external onlyAdmin {
        ethBurnGasFee = _ethBurnGasFee;
    }

    function withdrawLeftBurnGasFee(uint256 _amount) external onlyAdmin {
        require(_amount <= totalBurnGasFee, "Not enough gas fee to be sent");
        _msgSender().transfer(_amount);
        totalBurnGasFee = totalBurnGasFee.sub(_amount);
        emit BurnGasFeeSent(_msgSender(), _amount);
    }
}