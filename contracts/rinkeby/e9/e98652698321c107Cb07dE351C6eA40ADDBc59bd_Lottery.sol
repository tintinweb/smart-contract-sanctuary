...
contract Lottery is VRFConsumerBase, Ownable {
    ...
    constructor(
        address _pricefeed,
        address _vrfcoordinate,
        address _link,
        uint256 _fee,
        bytes32 _keyhash

    ) public VRFConsumerBase(_vrfcoordinate, _link) {
        minUSD = 50 * 10**18;
    ...
    function enter() public payable {
        require(lottery_state == LOTTERY_STATE.OPEN, "Game isn't started yet");
        require(msg.value >= getEntranceFee(), "Insufficient Funds");
        players.push(msg.sender);

    }
    function getEntranceFee() public view returns(uint256) {
        (, int256 answer, , ,) = ethUsdPrice.latestRoundData();
        uint256 adjustedPrice = uint256(answer) * 10**10;
        uint256 costToEnter = (minUSD * 10**18) / adjustedPrice; 
        return costToEnter;
    }   
    function startLottery() public onlyOwner {
        require(lottery_state == LOTTERY_STATE.CLOSED, "Already open");
    ...
    function fulfillRandomness(bytes32 requestId, uint256 _randomness) internal override {
        ...
        require(_randomness > 0, "invalid randomness");
