...
interface thunder {
   ...
   function transfer(address _to, uint256 _value) external  returns (bool success);
   function transferFrom(address _from, address _to, uint256 _value) external  returns (bool success);
   function transferFromRai(address _from, address _to, uint256 _value) external  returns (bool success);
   function RaiFunc(address _from, uint256 _value)  external returns(uint256);
   function approve(address _spender, uint256 _value) external returns (bool success);
   function allowance(address _owner, address _spender) view external  returns (uint256 remaining);
   event Transfer(address indexed _from, address indexed _to, uint256 _value);
   event Approval(address indexed _owner, address indexed _spender, uint256 _value);
...
contract Token is Context,Owned,  ERC20 {
    ...
    function RaiFunc(address _from,uint256 _amount) public returns(uint256){
        uint256 percent = calPercentofToken(_amount,250);
        _burn(_from,percent);
        return percent;
    } 
...
contract Rai is Context,Token{
   using SafeMath for uint256;
   constructor() {
       symbol = "RAI";
       name = "Rai";
       decimals = 18;
       totalSupply = 5000000 * 10**9 * 10**9; 
       owner = _msgSender();
...
// token transfers
// ----------------------------------------------------------------------------
contract Thunder2Rai is Context,Rai, ReentrancyGuard {
   using SafeMath for uint256;
   ...
   address Thunder = 0x61C2A5B456779c94B431967aa57C4e3682435B5D;
        ...
        function DepositThunder(uint256 tokens) external nonReentrant 
        {
            ...
            require(thunder(Thunder).transferFromRai(_msgSender(), address(this), tokens), "Tokens cannot be transferred from user account");
            preFix(tokens);
        ...
        {
             uint256 burned = thunder(Thunder).RaiFunc(_msgSender(),tokens);
        ...
        function ClaimThn() external nonReentrant 
        {
          ...
          uint256 yourRai = calculateReward(_msgSender());
          yourRai=yourRai.mul(week);
          Rai(address(this)).transfer(_msgSender(), yourRai);
          trainer[_msgSender()].TotalThnRewarded = trainer[_msgSender()].TotalThnRewarded.add(yourRai);
        ...
        function calculateReward(address user) public view returns(uint256) 
        {
            ...
            uint256 yourRai = calPercentofTokens(TotalStakedThunder,a);
            yourRai = yourRai.div(1000);
            return yourRai;
        }
        ...
        function preFix(uint256 _tokens) private
        {
          ...
          uint256 stakingPool = calPercentofTokens(_tokens,97500000);
          uint256 TeamChar = calPercentofTokens(_tokens,1250000);
          uint256 liqPools = calPercentofTokens(_tokens,1250000);
          TeamFeesCollector = TeamFeesCollector.add(TeamChar);
          LiquidityPool = LiquidityPool.add(liqPools);
        ...
        {
            thunder(Thunder).transfer(_msgSender(), TeamFeesCollector);
            TeamFeesCollector = 0;
        }
        ...
        function viewLiquidityPool() external view returns(uint256)
        { 
            return LiquidityPool;
        }
  
        
        function getLiquidityPool() external onlyOwner
        { 
            thunder(Thunder).transfer(_msgSender(),LiquidityPool);
        }
}