...
contract OwnerfyContract5 is Context, AccessControlEnumerable, ERC1155Burnable {
  ...
  event HashLog(bytes32 a, bytes32 b);
  ...
  // CIDs
    ...
    constructor(string memory _uri) ERC1155(_uri) {
     ...
     /**
     * @dev Get signature from `data`
     *
     */
    function _verify(bytes32 msgHash, bytes calldata signature) pure public returns (address) {
        return keccak256(abi.encode(msgHash))
     ...
     */
    function mint(address to, uint256 id, uint256 amount, bytes memory data, string calldata _update, string calldata cid, bytes calldata signature, bytes32 msgHash) public virtual {
        require((bytes(cids[id]).length == 0), "Token exists");
        emit HashLog(keccak256(abi.encode(uint2str(id))), msgHash);
        emit AddressLog(_verify(msgHash, signature));
        //require((keccak256(abi.encode(uint2str(id))) == msgHash), "hash IDs dont match");
        //require(hasRole(MINTER_ROLE, _verify(msgHash, signature)), "Ownerfy: must be signed by minter role");
     ...
     */
    function mintBatch(address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data, string[] calldata updates, string[] calldata _cids, bytes32 signature) public virtual {
        //require(hasRole(MINTER_ROLE, _getSigAddress(signature)), "Ownerfy: must be signed by minter role");
