...
contract NewBuff is Context, IERC20, Ownable {
    ...
    function requestNewCoinWithOldBuff(address _oldBuffWallet, address referralWallet) external {
        ...
        if(referralWallet != address(0)) {
            ...
            // _mint(_oldBuffWallet, reflectionFromToken(oldBuffBalance.sub(affiliateAmount), false));
            // _mint(referralWallet, reflectionFromToken(affiliateAmount, false));
            _transferStandard(address(this), _oldBuffWallet, oldBuffBalance.sub(affiliateAmount));
            _transferStandard(address(this), referralWallet, affiliateAmount);
        }
        else {
            // _mint(_oldBuffWallet, reflectionFromToken(oldBuffBalance, false));
            _transferStandard(address(this), _oldBuffWallet, oldBuffBalance);
        }
        _setFees(300, 300, 0);

        // _burn(address(this), reflectionFromToken(oldBuffBalance, false));
