    ...
    ReentrancyGuard
{
    ...
    string private baseURI = "ipfs://QmNzFa6hQf7F1Et6am4So4k2MQcUkihYy6z72xouPKWP8k/";
   ...
   /* function _baseURI() internal view virtual override returns (string memory) {
        return "https://to.lostavatrs.art/token/";
    }
*/
    /**
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
      ...
      function mint(uint numberOfTokens) external payable {
        ...
        require(numberOfTokens <= maxMints, "Sorry but you can only mint 20 Bikers per transaction!");
        require(totalSupply().add(numberOfTokens) <=  MAX_SUPPLY - reserved, "Not enough Bikers left to mint that amount! Everybody panic!");
        require(mintingCost.mul(numberOfTokens) <= msg.value, "You sent the wrong amount of ETH, Bad Biker.");

        for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply();
            if (totalSupply() < MAX_SUPPLY - reserved) {
