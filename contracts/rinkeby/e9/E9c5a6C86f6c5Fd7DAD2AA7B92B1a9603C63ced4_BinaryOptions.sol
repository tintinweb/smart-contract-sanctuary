...
contract BinaryOptions is Ownable, Authorizable, Pausable, Biddable, Oracle {
    ...
    function announceResult() public onlyAuthorized whenNotPaused {
        ...
        if (currentPrice > strikePrice) {
            ...
            uint256 amountPerInvestor = SafeMath.div(
                dvdToken.balanceOf(address(this)),
                longs.length
            );
            for (uint256 i = 0; i < longs.length; i++) {
                investor memory user = longs[i];
                approveWithdrawal(user.investorAddress, amountPerInvestor);
                payout[user.investorAddress] = SafeMath.sub(
                    amountPerInvestor,
                    getPayoutFees(amountPerInvestor)
                );
                require(
                    // approveWithdrawal(user.investorAddress, amountPerInvestor)
                    approveWithdrawal(
    ...
    function claim() public whenNotPaused {
        ...
        dvdToken.transfer(msg.sender, payout[sender]);
        payout[sender] = 0;
    }

    function getPayoutFees(uint256 amount)
        internal
        view
        whenNotPaused
        returns (uint256)
    ...
    function getPayout(uint256 _investmentPercentage)
        ...
        returns (uint256)
    {
        uint256 dvdAmount = dvdToken.balanceOf(address(this)) / (10**13);
        return
            SafeMath.mul(SafeMath.div(_investmentPercentage, 100), dvdAmount);
    }
    ...
    function approveWithdrawal(address _winner, uint256 _amount)
        ...
        returns (bool)
    {
        return dvdToken.approve(_winner, _amount);
    }
    ...
    function getContractDVDBalance()
        external
        view
        onlyAuthorized
        whenNotPaused
        returns (uint256)
    {
        ...
        return SafeMath.div(dvdToken.balanceOf(address(this)), 10**13);
    }
    ...
    function getDVDBalance() external view whenNotPaused returns (uint256) {
