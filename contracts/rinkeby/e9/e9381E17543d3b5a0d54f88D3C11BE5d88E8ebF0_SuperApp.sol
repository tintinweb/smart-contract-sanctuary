...
contract SuperApp is SuperAppBase {
    ...
    mapping(address => address) userToLockMapping;
    ...
    function updateOrDeleteFlow(int96 flowRate, address receiver) internal {
        if(flowRate < 0) {
            (bytes memory newCtx, ) = _host.callAgreementWithContext(
                _cfa,
        ...
        returns (bytes memory newCtx)
    {
        ...
        (,int96 userToSuperAppFlow,,) = _cfa.getFlow(_acceptedToken, flowSender, flowReceiver);
        address lockAddress = userToLockMapping[flowSender];

        if(userToSuperAppFlow >= _requiredFlowRate) {
