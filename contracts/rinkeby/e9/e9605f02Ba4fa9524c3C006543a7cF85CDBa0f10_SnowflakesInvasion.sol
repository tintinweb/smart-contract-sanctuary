...
contract SnowflakesInvasion is IERC1155Receiver {
  ...
  function stopPlaying(address collectible,uint256 tokenId) public {
    ...
    delete positions[ players[msg.sender].x][ players[msg.sender].y];
  ...
  function move(address collectible,uint256 tokenId,uint8 x,uint8 y) public {
    ...
    if(players[msg.sender].x > 10){
      require(players[msg.sender].x.sub(10) <= x,"Wrong position, maximum of 10 units of movement for x and y axis / Out of game limit");
    }
    if(players[msg.sender].y > 10){
      require(players[msg.sender].y.sub(10) <= y,"Wrong position, maximum of 10 units of movement for x and y axis / Out of game limit");
    }
    ...
    if(positions[x][y].owner != address(0)){
      ...
      if(win){
        ERC1155(players[positions[x][y].owner].collectible).safeTransferFrom(address(this),
                                                                             positions[x][y].owner,
                                                                             players[positions[x][y].owner].tokenId,
                                                                             1,'');
