...
contract PickRandomWinner is VRFConsumerBase, Ownable {
    ...
    function requestWinnerSigningHash(uint256 documentId, uint256 totalSigners) public view returns (bytes32) {
        return keccak256(abi.encodePacked(documentId, totalSigners));
    }

    function verify(bytes memory signature, uint256 documentId, uint256 totalSigners) internal view returns (bool) {
        bytes32 signingHash = requestWinnerSigningHash(documentId, totalSigners).toEthSignedMessageHash();
    ...
    function requestWinner(bytes memory signature, uint256 documentId, string memory urlHash, uint256 totalSigners) public payable {
        require(msg.value >= userChargePrice, "Ether sent not correct");
        require(verify(signature, documentId, totalSigners) == true, "Verification for signer failed");
