...
import "./interfaces/IFreezableToken.sol";
...
contract WishMultisender is Ownable, ReentrancyGuard
{
    ...
    modifier validLists(uint256 _contributorsLength, uint256 _balancesLength, uint256 _timesLength, bool _withFreeze) {
        ...
        if (_withFreeze) {
            require(
           _contributorsLength == _timesLength,
            "WishMultisender: Different arrays lengths"
        );
        }
        _;
    ...
    function multiFreezeToken(
        IFreezableToken _token,
        address[] calldata contributors,
        uint256[] calldata balances,
        uint64[] calldata freezeTimes,
        uint256 _total
    )
        external
        payable
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < contributors.length; i++) {
            ...
            //     "WishMultisender: Couldn't transfer tokens to user"
            // );
            _safeFreezeTo(address(_token), contributors[i],balances[i], freezeTimes[i]);
            /* _token.freezeTo(
                contributors[i],
                balances[i],
                freezeTimes[i]
    ...
    function _safeFreezeTo(address _token, address _address, uint256 _amount, uint64 _freezeTime) internal {
        (bool success, bytes memory data) = 
                _token.call(
                    abi.encodeWithSelector(0x3be1e952, _address,_amount, _freezeTime)
                );
             require(
                 success && (data.length == 0 || abi.decode(data, (bool))),
                 "freezeTo: freeze failed"
            );
    }
...
interface IFreezableToken  {
     ...
     */
    function freezeTo(address _to, uint _amount, uint64 _until) external ;
