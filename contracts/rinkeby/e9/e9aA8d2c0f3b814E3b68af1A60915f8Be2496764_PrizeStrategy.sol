    ...
    ReentrancyGuardUpgradeable
{
     ...
     * @dev It stores the "participations" of users in a specific party
     */
     ...
     */
    function initialize(address _backendAddress, address _multiTokenAddress)
        external
        initializer
    {
    ...
    function getWinners(
    ...
    ) external override onlyRole(BACKEND_ADMIN_ROLE) {
        ...
        winner[] memory winners = new winner[](randomNumbersBuffer.length);
        ...
        while (indexes.winnersIndex < randomNumbersBuffer.length) {
            if (
                ...
                multiToken
                    .balanceOf(_users[indexes.currentUserIndex], _partyId)
                    .add(indexes.currentIndex) &&
                !hasWon[indexes.currentUserIndex]
            ) {
                hasWon[indexes.currentUserIndex] = true;
                winners[indexes.winnersIndex].winnerAddress = _users[
                    indexes.currentUserIndex
                ];
                winners[indexes.winnersIndex].tier = randomNumbersBuffer[
                    indexes.bufferIndex
                ].tier;
            ...
            indexes.currentIndex = indexes.currentIndex.add(
                multiToken.balanceOf(_users[indexes.currentUserIndex], _partyId)
            );
     ...
     * @param _winners array of ther addresses of the users which won the party and the
     * prize tier that they have won
     ...
     * @param _prize the amount that reprends the amount from which the percentages
     * are calculated
     */
    function getWinnersPrizes(
        ...
        uint256[] memory winnersPrizes;
        for (uint256 i = 0; i < _percentages.length; i++) {
            prizes[i] = _percentages[i].mul(_prize).div(1000);
        }
        for (uint256 i = 0; i < _winners.length; i++) {
            winnersPrizes[i] = prizes[_winners[i].tier];
        }
        emit WinnersPrizes(_partyId, winnersPrizes);
    }
...
interface IPrizeStrategy {
    ...
    /**
     * @notice Emmited after winners are calculated inside 'getWinners'
     * @param partyId id of the party for which the winners are calculated
     * @param winners array containing the winners addresses and the tier
     * that they have won
     */
    event Winners(uint256 partyId, winner[] winners);

    /**
     * @notice Emmited after winners prizes are calculated inside 'getWinnersPrizes'
