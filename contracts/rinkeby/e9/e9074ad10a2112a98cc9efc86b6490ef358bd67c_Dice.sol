...
contract Dice {
    uint8 constant MAX_EXPIRATION_BLOCKS = 250;
    ...
    function placeBet(uint8 modulo, bytes5 userBets, uint40 requestBlock, bytes32 key, bytes32 secretSign, bytes32 r, bytes32 s) external payable {
        ...
        require(signAddr == ecrecover(sha3Hash, 27, r, s), "ECDSA signature is not valid.");
    ...
    function settleBet(bytes32 key, bytes32 halfRandom, bytes32 blockHash) external onlyCroupier {
        Bet storage bet = bets[key];
        ...
        if(block.number <= bet.placeBlockNumber + MAX_EXPIRATION_BLOCKS) {
            require(blockhash(bet.placeBlockNumber) == blockHash, "blockhash is invalid.");
            bytes32 _secretSign = keccak256(abi.encodePacked(halfRandom, key));
            require(bet.secretSign == _secretSign, "signature is invalid.");
