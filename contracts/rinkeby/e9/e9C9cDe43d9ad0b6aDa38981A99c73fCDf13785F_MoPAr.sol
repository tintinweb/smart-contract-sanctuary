...
import "./MoPArMetadata.sol";

contract MoPAr is Ownable, ERC721 {
    ...
    mapping(uint256 => string[]) private _catalogueEntryByToken;

    constructor(uint256 initialPrice_, bool isMuseumOpen_)
    ...
    function claimArt(uint256 tokenId, string[] memory catalogueEntry) external payable {
    ...
    function daoClaim(uint256 tokenId, string[] memory catalogueEntry) external onlyOwner {
        _mint(tokenId, catalogueEntry);
    }

    function getMetadata(uint256 tokenId, uint256 index) external view returns (string memory) {
        require(ownerOf(tokenId) != address(0));
        require(index < _catalogueEntryByToken[tokenId].length,"NO_VALUE_FOR_INDEX");
        
        return _catalogueEntryByToken[tokenId][index];
    }
    ...
    function setNewSignatures(bytes32[] memory signatures) external onlyOwner {
    ...
    function setSignature(uint256 tokenId, bytes32 signature) external onlyOwner {
        require(tokenId <= totalTokens, "INVALID_TOKEN_ID");
        _signatureByToken[tokenId] = signature;
    }
    ...
    function _mint(uint256 tokenId, string[] memory catalogueEntry) internal {
        require(tokenId <= totalTokens, "INVALID_TOKEN_ID");
    ...
    function _generateSignature(string[] memory catalogueEntry) private pure returns (bytes32) {
        string memory data = string(abi.encodePacked("permuted: "));
        for (uint8 i=0; i<catalogueEntry.length; i++) {
