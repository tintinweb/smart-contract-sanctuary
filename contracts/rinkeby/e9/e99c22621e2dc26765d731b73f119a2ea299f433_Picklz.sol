...
// Inspired/Copied fromm BGANPUNKS V2 (bastardganpunks.club) && Chubbies (chubbies.io) && Pixls (getpixls.com)
contract Picklz is ERC721, Ownable {
    ...
    constructor(string memory baseURI) ERC721("The Pickle Shop","PICKLZ")  {
        setBaseURI(baseURI);
    }
   ...
   function buyPicklz(uint256 numPicklz) public payable {
        require(totalSupply() < MAX_PICKLZ, "Sale has already ended");
        require(numPicklz > 0 && numPicklz <= 20, "You can adopt minimum 1, maximum 20 picklz");
