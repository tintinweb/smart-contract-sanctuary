...
contract HarbergerAsset is ERC721URIStorage, ReentrancyGuard {
  ...
  uint256 public baseTaxDenominator = 1000;
  ...
  uint256 public constant taxPercentage = 10;
  ...
  /**
   * @dev Lists asset for sale and sets corresponding tax price.
   * @param _tokenId ID of the token
   ...
   * Emits a {List} event.
   */
  function listAsset(uint64 _tokenId, uint256 _priceAmount) public validToken(_tokenId) onlyOwner(_tokenId) {
    ...
    assets[_tokenId].taxAmount = _priceAmount / taxDenominator;
    ...
    if (assets[_tokenId].previousListingPrice != _priceAmount && assets[_tokenId].totalDepositAmount > 0) {
      uint256 timeRemaining = assets[_tokenId].foreclosureTimestamp - block.timestamp - baseInterval;
      uint256 depositRemaining = (timeRemaining * currentBaseTaxValue) / baseInterval;

      assets[_tokenId].foreclosureTimestamp = block.timestamp + baseInterval;
      assets[_tokenId].foreclosureTimestamp += (depositRemaining * baseInterval) / newBaseTaxValue;
  ...
  function depositTax(uint64 _tokenId) public payable validToken(_tokenId) onlyOwner(_tokenId) nonReentrant {
    ...
    assets[_tokenId].foreclosureTimestamp += (msg.value * baseInterval) / baseTaxValues[_tokenId];
   ...
   *
   * Emits a {Refund} event if `timeRemaining` is more than `block.timestmap` plus `baseInterval`.
   */
  function refundTax(uint64 _tokenId, address _currentOwner, uint256 _baseTaxValue) private returns(uint256) {
    ...
    if (foreclosureTimestamp > block.timestamp + baseInterval) {
      uint256 remainingTimestamp = foreclosureTimestamp - block.timestamp - baseInterval;
      uint256 refundAmount = (remainingTimestamp * _baseTaxValue) / baseInterval;
   ...
   */
  function collectTax(uint64 _tokenId, address _currentOwner, uint256 _refundAmount) private {
  ...
  function transferPayments(uint256 _payment, address _currentOwner, address _creator) private {
    uint256 royaltyAmount = _payment / royaltyDenominator;
