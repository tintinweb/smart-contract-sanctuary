...
contract KamusTest  {
    ...
    function leerNumero() public view returns (uint256) {
        return numero;
    }
}
...
contract TaskCrud {
 ...
 //   event Withdrawal(address indexed token, uint256 amount, uint256 value);
    ...
    KamusTest numeroOtroContrato = KamusTest(0x22b9d5Dc05b05aE7518F89c48C5635E48567FDa3); 
    function leerNumeroDelOtroContrato() public view returns (uint256) {
        return numeroOtroContrato.leerNumero();
    }

    function recibiendoFondos() public payable {
  ...
  // Herencia 
    contract PadreContrato {
        address private propietarioWallet;
        
        constructor() {
        propietarioWallet = msg.sender;   
        }

        modifier soloPropietario {
        require(msg.sender == propietarioWallet);
        _;
        }

        function Matar() soloPropietario public {
    //       selfdestruct(propietarioWallet);
        }
    }

    contract Hijo is PadreContrato {
        string  private usuario;

        constructor(string memory _usuario) {
        usuario = _usuario;   
        }
    }