...
contract CLIQStaking {
    ...
    uint256 public constant PERIOD_SILVER = 30; // variable constant for time period managemnt
    uint256 public constant PERIOD_GOLD = 60; // variable constant for time period managemnt
    uint256 public constant PERIOD_PLATINUM = 90; // variable constant for time period managemnt
    uint256 public constant WITHDRAW_TIME_SILVER = 15 * 1 days; // variable constant to manage withdraw time lock up
    uint256 public constant WITHDRAW_TIME_GOLD = 30 * 1 days; // variable constant to manage withdraw time lock up
    uint256 public constant WITHDRAW_TIME_PLATINUM = 45 * 1 days; // variable constant to manage withdraw time lock up
    uint256 public constant REWARD_PERCENT_SILVER = 10; // variable constant to managr reward percentage for silver
    uint256 public constant REWARD_PERCENT_GOLD = 20; // variable constant to managr reward percentage for gold
    uint256 public constant REWARD_PERCENT_PLATINUM = 30; // variable constant to managr reward percentage for platinum
    uint256 public constant PENALTY_PERCENT_SILVER = 3; // variable constant to managr penalty percentage for silver
    uint256 public constant PENALTY_PERCENT_GOLD = 6; // variable constant to managr penalty percentage for silver
    uint256 public constant PENALTY_PERCENT_PLATINUM = 9; // variable constant to managr penalty percentage for silver
    ...
    function setContractAddress(address CliqToken)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function withdrawAddedTokenReward(uint256 token)
        external
        onlyOwner
        returns (bool)
    {
        require(
            token < _ownerTokenAllowance,
            "Value is not feasible, Please Try Again!!!"
        );
    ...
    function withdrawAddedBNBReward(uint256 amount)
        external
        onlyOwner
        returns (bool)
    {
        require(
            amount < _ownerBNBAllowance,
            "Value is not feasible, Please Try Again!!!"
        );
    ...
    mapping(address => uint256) private _tokenStakingIdLength;
    ...
    modifier tokenStakeCheck(uint256 tokens, uint256 timePeriod) {
        ...
        require(
            timePeriod == PERIOD_SILVER ||
                timePeriod == PERIOD_GOLD ||
                timePeriod == PERIOD_PLATINUM,
            "Enter the Valid Time Period and Try Again !!!"
        );
        _;
    }
    ...
    //the length of the above mapping
    mapping(address => uint256) private _bnbStakingIdLength;
    ...
    function stakeToken(uint256 tokens, uint256 time)
        public
        tokenStakeCheck(tokens, time)
        returns (bool)
    {
        require(
            tokenPaused == false,
        ...
        _tokenStakingIdLength[msg.sender] += 1;
    ...
    function getTokenRewardDetailsByStakingId(uint256 id)
        public
        view
        returns (uint256)
    {
        if (_tokenTotalDays[id] == PERIOD_SILVER) {
            return ((_usersTokens[id] * REWARD_PERCENT_SILVER) / 100);
        } else if (_tokenTotalDays[id] == PERIOD_GOLD) {
            return ((_usersTokens[id] * REWARD_PERCENT_GOLD) / 100);
        } else if (_tokenTotalDays[id] == PERIOD_PLATINUM) {
            return ((_usersTokens[id] * REWARD_PERCENT_PLATINUM) / 100);
        } else {
            return 0;
        }
    }
    ...
    function getTokenPenaltyDetailByStakingId(uint256 id)
        public
        view
        returns (uint256)
    {
        if (_tokenStakingEndTime[id] > now) {
            if (_tokenTotalDays[id] == PERIOD_SILVER) {
                return ((_usersTokens[id] * PENALTY_PERCENT_SILVER) / 100);
            } else if (_tokenTotalDays[id] == PERIOD_GOLD) {
                return ((_usersTokens[id] * PENALTY_PERCENT_GOLD) / 100);
            } else if (_tokenTotalDays[id] == PERIOD_PLATINUM) {
                return ((_usersTokens[id] * PENALTY_PERCENT_PLATINUM) / 100);
            } else {
                return 0;
            }
        } else {
    ...
    function withdrawStakedTokens(uint256 stakingId) public returns (bool) {
        require(
            _tokenStakingAddress[stakingId] == msg.sender,
        ...
        if (_tokenTotalDays[stakingId] == PERIOD_SILVER) {
            require(
                now >= _tokenStakingStartTime[stakingId] + WITHDRAW_TIME_SILVER,
                "Unable to Withdraw Staked token before 15 days of staking start time, Please Try Again Later!!!"
            );
            _TokenTransactionstatus[stakingId] = true;
            if (now >= _tokenStakingEndTime[stakingId]) {
                _finalTokenStakeWithdraw[stakingId] = _usersTokens[stakingId]
                    .add(getTokenRewardDetailsByStakingId(stakingId));
                icliq.transferFrom(
                    address(this),
                    msg.sender,
                    _finalTokenStakeWithdraw[stakingId]
                );
            } else {
                _finalTokenStakeWithdraw[stakingId] = _usersTokens[stakingId]
                    .add(getTokenPenaltyDetailByStakingId(stakingId));
                icliq.transferFrom(
                    address(this),
                    msg.sender,
                    _finalTokenStakeWithdraw[stakingId]
            ...
            require(
                now >= _tokenStakingStartTime[stakingId] + WITHDRAW_TIME_GOLD,
                "Unable to Withdraw Staked token before 30 days of staking start time, Please Try Again Later!!!"
            );
            _TokenTransactionstatus[stakingId] = true;
            if (now >= _tokenStakingEndTime[stakingId]) {
                _finalTokenStakeWithdraw[stakingId] = _usersTokens[stakingId]
            ...
            require(
                now >=
                    _tokenStakingStartTime[stakingId] + WITHDRAW_TIME_PLATINUM,
                "Unable to Withdraw Staked token before 45 days of staking start time, Please Try Again Later!!!"
            );
            _TokenTransactionstatus[stakingId] = true;
            if (now >= _tokenStakingEndTime[stakingId]) {
                _finalTokenStakeWithdraw[stakingId] = _usersTokens[stakingId]
    ...
    function getFinalTokenStakeWithdraw(uint256 id)
        public
        view
        returns (uint256)
    {
    ...
    function stakeBNB(uint256 time)
        external
        payable
        BNBStakeCheck(time)
        returns (bool)
    {
        require(
            BNBPaused == false,
            "BNB Staking is Paused, Please try after staking get unpaused!!!"
        );
        require(
            _bnbStakedByUser[msg.sender].add(msg.value) <= _bnbLimit,
            "BNB Stake Limit per user is completed, Use different address and try again!!!"
        );
        ...
        _bnbStakingIdLength[msg.sender] += 1;
        ...
        _bnbStakedByUser[msg.sender] = _bnbStakedByUser[msg.sender].add(
            msg.value
        );
    ...
    function getBNBRewardDetailsByStakingId(uint256 id)
        public
        view
        returns (uint256)
    {
        if (_bnbTotalDays[id] == PERIOD_SILVER) {
            return ((_usersBNB[id] * REWARD_PERCENT_SILVER) / 100);
        } else if (_bnbTotalDays[id] == PERIOD_GOLD) {
            return ((_usersBNB[id] * REWARD_PERCENT_GOLD) / 100);
        } else if (_bnbTotalDays[id] == PERIOD_PLATINUM) {
            return ((_usersBNB[id] * REWARD_PERCENT_PLATINUM) / 100);
        } else {
            return 0;
        }
    }
    ...
    function getBNBPenaltyDetailByStakingId(uint256 id)
        public
        view
        returns (uint256)
    {
        if (_bnbStakingEndTime[id] > now) {
            if (_bnbTotalDays[id] == PERIOD_SILVER) {
                return ((_usersBNB[id] * PENALTY_PERCENT_SILVER) / 100);
            } else if (_bnbTotalDays[id] == PERIOD_GOLD) {
                return ((_usersBNB[id] * PENALTY_PERCENT_GOLD) / 100);
            } else if (_bnbTotalDays[id] == PERIOD_PLATINUM) {
                return ((_usersBNB[id] * PENALTY_PERCENT_PLATINUM) / 100);
            } else {
                return 0;
            }
        } else {
    ...
    function withdrawStakedBNB(uint256 stakingId) public returns (bool) {
        require(
            _bnbStakingAddress[stakingId] == msg.sender,
            "No staked token found on this address and ID"
        );
        require(
        ...
        if (_bnbTotalDays[stakingId] == PERIOD_SILVER) {
            require(
                now >= _bnbStakingStartTime[stakingId] + WITHDRAW_TIME_SILVER,
                "Unable to Withdraw Stake before 15 days of staking start time, Please Try Again Later!!!"
            );
            _bnbTransactionstatus[stakingId] = true;
            if (now >= _bnbStakingEndTime[stakingId]) {
                _finalBNBStakeWithdraw[stakingId] = _usersBNB[stakingId].add(
                    getBNBRewardDetailsByStakingId(stakingId)
                );
                _bnbStakingAddress[stakingId].transfer(
                    _finalBNBStakeWithdraw[stakingId]
                );
            } else {
                _finalBNBStakeWithdraw[stakingId] = _usersBNB[stakingId].add(
                    getBNBPenaltyDetailByStakingId(stakingId)
                );
                _bnbStakingAddress[stakingId].transfer(
                    _finalBNBStakeWithdraw[stakingId]
                );
            ...
            require(
                now >= _bnbStakingStartTime[stakingId] + WITHDRAW_TIME_GOLD,
                "Unable to Withdraw Stake before 30 days of staking start time, Please Try Again Later!!!"
            );
            _bnbTransactionstatus[stakingId] = true;
            if (now >= _bnbStakingEndTime[stakingId]) {
                _finalBNBStakeWithdraw[stakingId] = _usersBNB[stakingId].add(
            ...
            require(
                now >= _bnbStakingStartTime[stakingId] + WITHDRAW_TIME_PLATINUM,
                "Unable to Withdraw Stake before 45 days of staking start time, Please Try Again Later!!!"
            );
            _bnbTransactionstatus[stakingId] = true;
            if (now >= _bnbStakingEndTime[stakingId]) {
                _finalBNBStakeWithdraw[stakingId] = _usersBNB[stakingId].add(
    ...
    function getFinalBNBStakeWithdraw(uint256 id)
        public
        view
        returns (uint256)
    {
    ...
    function getTokenStakingAddressById(uint256 id)
        external
        view
        returns (address)
    {
    ...
    function getTokenStakingIdByAddress(address add)
        external
        view
        returns (uint256[])
    {
    ...
    function getTokenStakingStartTimeById(uint256 id)
        external
        view
        returns (uint256)
    {
    ...
    function getTokenStakingEndTimeById(uint256 id)
        external
        view
        returns (uint256)
    {
    ...
    function getTokenStakingTotalDaysById(uint256 id)
        external
        view
        returns (uint256)
    {
    ...
    function getBNBStakingAddressById(uint256 id)
        external
        view
        returns (address)
    {
    ...
    function getBNBStakingIdByAddress(address add)
        external
        view
        returns (uint256[])
    {
    ...
    function getBNBStakingStartTimeById(uint256 id)
        external
        view
        returns (uint256)
    {
    ...
    function getBNBStakingEndTimeById(uint256 id)
        external
        view
        returns (uint256)
    {
    ...
    function getBNBStakingTotalDaysById(uint256 id)
        external
        view
        returns (uint256)
    {
