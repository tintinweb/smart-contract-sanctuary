...
contract Simple2 is ERC721Enumerable {
   ...
   event NFTCreated(address new_owner, uint _tokenId);
   ...
   constructor(
       ...
       string memory nameOfCreator_) ERC721(name_, symbol_) {
        
        nextTokenId = 0;
        nameOfCreator = nameOfCreator_;
    }
    
    function giftNFT(address collector_address, string memory IPFSHash) public {
        // mint token
        _safeMint(collector_address, nextTokenId);
        _setTokenIPFSHash(nextTokenId, IPFSHash);
           
        // events
        emit NFTCreated(collector_address, nextTokenId);
