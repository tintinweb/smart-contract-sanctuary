...
contract LoanMarket is LoanMarketBase {
    ...
    */
    ...
    function loanPaybackBeforeTermEnd(uint256 _loanId) public {
        ...
        uint256 apyFeeOriginal = getAPYFee(_loanId);
        uint256 loanTermLengthPassed = lenderDetails.activationLoanTimeStamp - block.timestamp;
        ...
        uint256 paybackAmounttoLender =  loanDetails.loanAmountInBorrowed - apyFeeOriginal;
        IERC20(loanDetails.borrowStableCoin).approve(address(this), paybackAmounttoLender + apyFeeBeforeLoanTermLengthEnd);
        IERC20(loanDetails.borrowStableCoin).transferFrom(loanDetails.borrower, lenderDetails.lender, paybackAmounttoLender + apyFeeBeforeLoanTermLengthEnd);  //
    ...
    function getTokenPriceInBorrowed(
        ...
        if (pair.token0() == _stable) {
            ...
            return (_altAmount.div(token1Decimals).mul(resD.div(res1))); // return amount of alt needed to buy 1 stable
        } else {
            //TODO Decimal Problem
            uint256 resD = res1 * (10**token1Decimals);
            return (_altAmount.div(token0Decimals).mul(resD.div(res0))); // return amount of alt needed to buy 1 stable
        }
    }
