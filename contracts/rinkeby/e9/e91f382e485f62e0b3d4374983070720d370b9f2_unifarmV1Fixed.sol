...
contract unifarmV1Fixed is Ownable {
    ...
    mapping(address => mapping(uint256 => stakeInfo)) public stakingDetailsNew;
    ...
    function stake(address tokenAddress, uint256 amount) external {
        ...
        (bool isExist, , uint256 userStakeLimit, uint256 maxStake) = UniV1.tokenDetails(tokenAddress);
        // checks 
        require(
            isExist == true,
        ...
        require(
            amount <= userStakeLimit,
            "STAKE : Amount should be within permit"
        );
        require(
             totalStakingFinal[tokenAddress].add(amount) <= maxStake,
        ...
        stakeInfo storage stakes = stakingDetailsNew[msg.sender][stakeID];
        
        stakes.isActive = true;
        stakes.user = msg.sender;
        stakes.tokenAddress = tokenAddress;
        stakes.stakedAmount = amount;
        stakes.startTime = block.timestamp;

        // Update total staking amount
        totalStakingFinal[tokenAddress] =  totalStakingFinal[tokenAddress].add(amount);
        ...
        // Emit state changes
        emit Stake(msg.sender, stakeID--, tokenAddress, amount, block.timestamp);
    }
    ...
    function unStake(uint256 stakeId) external  {
        ...
        uint stakeAmount; address token; uint starttime; 
        if(unstakeStatus[msg.sender][stakeId] == false && activeStatus[stakeId] == true) 
            stakeAmount = stakedAmount[stakeId];
            unstakeStatus[msg.sender][stakeId] = true;
            token = tokenAddress[stakeId];
            starttime = startTime[stakeId];
        
        if(stakingDetailsNew[msg.sender][stakeID].isActive == true)
            stakeAmount = stakingDetailsNew[msg.sender][stakeID].stakedAmount;
            stakingDetailsNew[msg.sender][stakeID].isActive = false;
            token = stakingDetailsNew[msg.sender][stakeID].tokenAddress;
            starttime = stakingDetailsNew[msg.sender][stakeID].startTime;
        ...
        uint Total = totalStakingFinal[token];
        totalStakingFinal[token] = totalStakingFinal[token].sub(stakeAmount);
          
        // Transfer staked token back to user
        IERC20(token).transfer(msg.sender, stakeAmount);

        // Claim pending rewards
        claimRewards(starttime, stakeAmount, token, Total);
     ...
     */
    function viewStakingDetails(address _user, uint256 stakeid)
        public
        view
        returns (
            address ,
    ...
    {
        return (
            stakingDetailsNew[_user][stakeid].tokenAddress,
            stakingDetailsNew[_user][stakeid].isActive,
            stakingDetailsNew[_user][stakeid].stakedAmount,
            stakingDetailsNew[_user][stakeid].startTime
        );
    }
}