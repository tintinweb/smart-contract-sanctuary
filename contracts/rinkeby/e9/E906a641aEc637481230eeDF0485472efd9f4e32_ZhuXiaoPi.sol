...
contract ZhuXiaoPi is ERC721, VRFConsumerBase {

    enum Breed{ Pig1, Pig2, Pig3 }
    ...
    mapping(uint256 => bool) public tokenIdToGenesis;
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override {
            ...
            uint256 randomIndex = randomNumber % 1002620;
            

            Breed breed = getBreed(randomNumber);  //  1 / 1,002,620

            tokenIdToBreed[newItemId] = breed;
            requestIdToTokenId[requestId] = newItemId;
            tokenCounter = tokenCounter + 1;
            cardsCounter[breed] = cardsCounter[breed] + 1;
            tokenIdToGenesis[newItemId] = true;   // is Genesis
    ...
    function getBreed(uint256 randomNumber) internal returns(Breed) {
        uint256 randomIndex = randomNumber % 1002620;
        Breed breed;
        if (randomIndex >= 0 && randomIndex < 200) {
            breed = Breed(0);   
        }
        if (randomIndex >= 200 && randomIndex < 2200) {
            breed = Breed(1);   
        }
        if (randomIndex >= 2200 && randomIndex < 2560) {
            breed = Breed(2);   
        } 
        return breed;
    }
