 ...
 * @dev see https://eips.ethereum.org/EIPS/eip-721
 */
contract AppolloAddress is Context, CustomOwnable, ICAMAEvents, ERC165, IERC721, IERC721Metadata {//change the name at the end
     ...
     * @dev Initializes the contract by setting a `name` and a `symbol` to the token collection.
     */
    function initialize(address owner, uint128 _batchSize, string memory symbol_) public {
        require(!_initialized, "CAMA: Already Initialized");
        _initialized = true;
        _setOwner(owner);
        _rootNode = owner;
        _name = unicode"Apollo Address🚀";
    ...
    // function setname() public onlyOwner returns (bool) {
    //     string memory name_ = unicode"Apollo Address 🚀";
    //     _name = name_;
    //     return true;
    // }
