  ...
  PausableUpgradeable
{
  ...
  function mintOnePresale(bytes32[] calldata proof)
  external payable
    whenPaused()
    withPayment(1 * UNIT_PRICE)
    onlyRemaining(1 + _currentPresaleLimit)
    onlyWhitelisted(_msgSender(), proof)
  {
    _mintBatch(_msgSender(), 1);
  }

  function mintThreePresale(bytes32[] calldata proof)
  external payable
    whenPaused()
    withPayment(3 * UNIT_PRICE)
    onlyRemaining(3 + _currentPresaleLimit)
    onlyWhitelisted(_msgSender(), proof)
  {
    _mintBatch(_msgSender(), 3);
  }

  function mintFivePresale(bytes32[] calldata proof)
  external payable
    whenPaused()
    withPayment(5 * UNIT_PRICE)
    onlyRemaining(5 + _currentPresaleLimit)
    onlyWhitelisted(_msgSender(), proof)
  {
    _mintBatch(_msgSender(), 5);
  }
