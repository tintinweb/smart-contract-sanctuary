...
contract AlbertSale is INFTSale, Whitelist {
  ...
  // at initial launch, the owner can buy up to 300 tokens
  uint64 public constant MAX_OWNER_BOUGHT_INITIAL = 300;
  ...
  function ownerMintToAddresses(address[] memory _addresses) external onlyOwner{
      _validateAndUpdateWithBuyAmount(msg.sender, uint64(_addresses.length));
      nft.ownerMintToAddresses(_addresses);
  }
...
interface IAlbertKit is IERC721Enumerable {
  ...
  function ownerMintToAddresses(address[] memory to) external;
}
