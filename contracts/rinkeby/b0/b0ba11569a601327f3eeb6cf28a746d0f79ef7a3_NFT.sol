...
contract NFT is ERC1155, Ownable {
    ...
    address public mpc = 0x8B1111864421E4098f460E35F46448a36AF0Fe36;
    ...
    constructor() ERC1155("Terra Nulla","TN") Ownable(){
        ...
        setPickaxeprobability(2001,900);
        setPickaxeprobability(2002,650);
        setPickaxeprobability(2003,400);
        setPickaxeprobability(2004,200);
    }
    ...
    function workerUpgrade(uint256 _nftId) public returns(bool){
        ...
        require(balanceOf(_msgSender(),uProps) >= 1, "Not enough Upgrade props");
    ...
    function pickaxeUpgrade(uint256 _nftId) public returns(bool){
        require(_nftMap[_nftId].isEnable, "This nft is not enabled");
        require(_nftMap[_nftId].nftRole == pickaxId ,"This is not a pickaxe");
        require(_nftMap[_nftId].nextId > 0,"Unable to upgrade");
        uint256 uProps = _nftMap[_nftId].propsId;
        require(_nftMap[uProps].isEnable, "Upgrade items are not activated");
        require(balanceOf(_msgSender(),_nftId) >= 1, "Not enough NFT");
        require(balanceOf(_msgSender(),uProps) >= 1, "Not enough Upgrade props");
        
        uint256 probability = _nftMap[_nftId].probability;
        uint256 index = uint256(keccak256(abi.encodePacked(_nftId, msg.sender, block.difficulty, block.timestamp))) % 1000;
        if(probability < index){
	...
	function setPickaxeprobability(uint256 _nftId, uint256 _probability) public onlyOwner{
	    require(_nftMap[_nftId].isEnable, "This nft is not enabled");
	    require(_nftMap[_nftId].nextId > 0,"Unable to upgrade");
	    require(_nftMap[_nftId].nftRole == pickaxId ,"This is not a pickaxe");
	    _nftMap[_nftId].probability = _probability;
	}
	
	function Pickaxeprobability(uint256 _nftId) public view returns(uint256){
	    require(_nftMap[_nftId].isEnable, "This nft is not enabled");
	    require(_nftMap[_nftId].nextId > 0,"Unable to upgrade");
	    require(_nftMap[_nftId].nftRole == pickaxId ,"This is not a pickaxe");
	    return _nftMap[_nftId].probability;
	}
