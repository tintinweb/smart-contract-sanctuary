...
interface IKongz {
    ...
    function balanceKing(address _user) external view returns(uint256);
    function balanceQueen(address _user) external view returns(uint256);
    function balanceJoker(address _user) external view returns(uint256);
    function balancePrince(address _user) external view returns(uint256);
    function detectCardz(uint256 indexNFT) external view returns(uint8);
}

contract YieldToken is ERC20("Roses", "ROSES") {
	using SafeMath for uint256;
	...
	function updateRewardOnMint(address _user, uint256 _amount) external {
        ...
        uint256 _tokenId = 0;
        uint8 cardtype = kongzContract.detectCardz(_tokenId);
		if (timerUser > 0)
            if (cardtype == 0) {
                rewards[_user] = rewards[_user].add(kongzContract.balanceJoker(_user).mul(BASE_RATE.mul((time.sub(timerUser)))).div(43200)
				.add(_amount.mul(INITIAL_ISSUANCE)));
            }
		    if (cardtype == 1) {
                rewards[_user] = rewards[_user].add(kongzContract.balanceKing(_user).mul(BASE_RATE.mul((time.sub(timerUser)))).div(86400)
				.add(_amount.mul(INITIAL_ISSUANCE)));
            }
            if (cardtype == 2) {
                rewards[_user] = rewards[_user].add(kongzContract.balanceQueen(_user).mul(BASE_RATE.mul((time.sub(timerUser)))).div(86400)
				.add(_amount.mul(INITIAL_ISSUANCE)));
            }
            if (cardtype == 3) {
                rewards[_user] = rewards[_user].add(kongzContract.balanceQueen(_user).mul(BASE_RATE.mul((time.sub(timerUser)))).div(172800)
				.add(_amount.mul(INITIAL_ISSUANCE)));
            }		
		else 
	...
	function updateReward(address _from, address _to, uint256 _tokenId) external {
        ...
        //balanceKing, balanceQueen, balanceJoker, balancePrince
        uint8 cardtype = kongzContract.detectCardz(_tokenId);
        uint256 time = min(block.timestamp, END);
        uint256 timerFrom = lastUpdate[_from];
        if (timerFrom > 0)
            if (cardtype == 0) {
                rewards[_from] += kongzContract.balanceJoker(_from).mul(BASE_RATE.mul((time.sub(timerFrom)))).div(43200);
            }
            if (cardtype == 1) {
                rewards[_from] += kongzContract.balanceKing(_from).mul(BASE_RATE.mul((time.sub(timerFrom)))).div(86400);
            }
            if (cardtype == 2) {
                rewards[_from] += kongzContract.balanceQueen(_from).mul(BASE_RATE.mul((time.sub(timerFrom)))).div(86400);
            }
            if (cardtype == 3) {
                rewards[_from] += kongzContract.balanceQueen(_from).mul(BASE_RATE.mul((time.sub(timerFrom)))).div(172800);
            }
        if (timerFrom != END)
        ...
        if (_to != address(0)) {
            ...
            if (timerTo > 0)
                if (cardtype == 0) {
                    rewards[_to] += kongzContract.balanceJoker(_to).mul(BASE_RATE.mul((time.sub(timerTo)))).div(43200);
                }
                if (cardtype == 1) {
                    rewards[_to] += kongzContract.balanceKing(_to).mul(BASE_RATE.mul((time.sub(timerTo)))).div(86400);
                }
                if (cardtype == 2) {
                    rewards[_to] += kongzContract.balanceQueen(_to).mul(BASE_RATE.mul((time.sub(timerTo)))).div(86400);
                }
                if (cardtype == 3) {
                    rewards[_to] += kongzContract.balancePrince(_to).mul(BASE_RATE.mul((time.sub(timerTo)))).div(172800);
                }
	...
	function getTotalClaimable(address _user) external view returns(uint256) {
		uint256 time = min(block.timestamp, END);
        uint256 pending_joker = kongzContract.balanceJoker(_user).mul(BASE_RATE.mul((time.sub(lastUpdate[_user])))).div(43200);
		uint256 pending_king = kongzContract.balanceKing(_user).mul(BASE_RATE.mul((time.sub(lastUpdate[_user])))).div(86400);
        uint256 pending_queen = kongzContract.balanceQueen(_user).mul(BASE_RATE.mul((time.sub(lastUpdate[_user])))).div(86400);
        uint256 pending_prince = kongzContract.balancePrince(_user).mul(BASE_RATE.mul((time.sub(lastUpdate[_user])))).div(172800);
		return rewards[_user] + pending_joker + pending_king + pending_queen + pending_prince;
	}
}
...
contract Kongz is ERC721Namable, Ownable {
    ...
    uint256 public maxSupply = 15010; //supply = j + k + q + p
    /*uint256 public maxSupplyJ = 10;   //000.1996%
    uint256 public maxSupplyK = 2500; //049.9000%
    uint256 public maxSupplyQ = 2500; //049.9000%
    uint256 public maxSupplyP = 10000;//100.0000% */
    uint256 public maxSupplyPresale = 250;
    uint256 public maxMintAmount = 2;
    uint256 public jokercount = 0;
    uint256 public kingcount = 0;
    uint256 public queencount = 0;
    uint256 public jokerstartAt = 0;
    uint256 public kingstartAt = 10;
    uint256 public queenstartAt = 2510;
    bool public lastmint = false;
    bool public paused = false;
    bool public revealed = false;
    bool public presale_mode = false;

	mapping(uint256 => Kong) public kongz;
	//mapping(address => uint256) public balanceOG;
    mapping(address => uint256) public balanceKing;
    mapping(address => uint256) public balanceQueen;
    mapping(address => uint256) public balanceJoker;
    mapping(address => uint256) public balancePrince;
    ...
    event CardMinted(uint256 tokenId, uint256 genes);
    //["Paco", "Manolo"]
	constructor(string memory _name, string memory _symbol) public ERC721Namable(_name, _symbol) {
		_setBaseURI("ipfs://QmZB3hAxTNvMXRJKm87GL2S9gUmyV3uKsT1ZBoiec6B9TA/");
		...
		_mint(msg.sender, 7);
		_mint(msg.sender, 8);
		kongz[0] = Kong(0, block.timestamp);
		kongz[1] = Kong(0, block.timestamp);
		kongz[2] = Kong(0, block.timestamp);
        kongz[3] = Kong(0, block.timestamp);
		kongz[4] = Kong(0, block.timestamp);
		kongz[5] = Kong(0, block.timestamp);
        kongz[6] = Kong(0, block.timestamp);
		kongz[7] = Kong(0, block.timestamp);
		kongz[8] = Kong(0, block.timestamp);
		emit KongIncubated(0, 0, 0);
		emit KongIncubated(1, 0, 0);
		emit KongIncubated(2, 0, 0);
        emit KongIncubated(3, 0, 0);
		emit KongIncubated(4, 0, 0);
		emit KongIncubated(5, 0, 0);
        emit KongIncubated(6, 0, 0);
		emit KongIncubated(7, 0, 0);
		emit KongIncubated(8, 0, 0);
		bebeCount = 9;*/
	}
    ...
    function mint() public payable {
        ...
        if (msg.sender != owner()) { 
            //require(walletOfOwner(msg.sender).length < 2);
            require(msg.value >= cost * _mintAmount); 
        }
        uint256 _genes = 0;
        uint256 supply = totalSupply();
        if (presale_mode == false) { require(supply + _mintAmount <= maxSupply); } else if (presale_mode == true) { require(supply + _mintAmount <= maxSupplyPresale); }
        uint256 id;
        if (!lastmint){
            lastmint = true;
            kingcount++;
            id = kingstartAt + kingcount;
            kongz[id] = Kong(_genes, block.timestamp);
            _mint(msg.sender, id);
            yieldToken.updateRewardOnMint(msg.sender, 1);//200 rosas
            //balanceOG[msg.sender]++;
            balanceKing[msg.sender]++;
            emit CardMinted(id, _genes);
        } else {
            lastmint = false;
            queencount++;
            id = queenstartAt + queencount;
            kongz[id] = Kong(_genes, block.timestamp);
            _mint(msg.sender, id);
            yieldToken.updateRewardOnMint(msg.sender, 1);//200 rosas
            //balanceOG[msg.sender]++;
            balanceQueen[msg.sender]++;
        }

        
    }

    function detectCardz(uint256 indexNFT) public view returns (uint8) {
        uint8 result = 3;
        // g1: 1 - 10 = jokers -> 50 rosas al dia, puede hacer breeding por si solo
        // g1: 10 - 2510 = reyes
        // g1: 2510 - 5010 = reinas 
        // g2: 5010 - 15010 = principes
        if (indexNFT <= 10) {result=0;} //joker
        if (indexNFT >= 10 && indexNFT <= 2510) {result=1;}//king
        if (indexNFT >= 2510 && indexNFT <= 5010) {result=2;}//queen

        return result;
    }

	function breed(uint256 _sire, uint256 _matron) external {
        //if (detectCardz(_sire) == "joker") {
        //    require(ownerOf(_sire) == msg.sender);
        //}else{
            require(ownerOf(_sire) == msg.sender && ownerOf(_matron) == msg.sender);
        //}
        
		//require(breedManager.tryBreed(_sire, _matron));
		yieldToken.burn(msg.sender, BREED_PRICE); // breedtax = breed_price * 0.05 -> yieldToken.burn(breed_price - breedtax)
		bebeCount++;
	...
	function transferFrom(address from, address to, uint256 tokenId) public override {
        ...
        uint8 cardtype = detectCardz(tokenId);
		if (cardtype == 0) {
            balanceJoker[from]--;
            balanceJoker[to]++;
        }
