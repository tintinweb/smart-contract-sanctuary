...
contract Transfer is ReentrancyGuard {
    ...
    modifier arrayNotEmpty(uint256[] memory _tokens) {
        require(
            _tokens.length > 0,
    ...
    modifier validTokenPrice(uint256 _tokenPrice) {
        require(_tokenPrice > 0, "Token price must be higher than zero");
        _;
    }

    modifier validAddress(address _erc721Address, address _poolAddress) {
        require(
            (_erc721Address != address(0)) && (_poolAddress != address(0)),
            "Not valid address"
        );
        _;
    }
    ...
    constructor(
        ...
        arrayNotEmpty(tokensArray)
        validTokenPrice(_tokenPrice)
        validAddress(_erc721Address, _poolAddress)
    {
        tokens = tokensArray;
    ...
    function buyTokens(uint256 quantity) external payable nonReentrant {
        ...
        require(tokens.length >= quantity, "Token distribution finished");

        uint256[] memory transferredTokens = new uint256[](quantity);

        for (uint256 i = 0; i < quantity; i++) {
            uint256 randomNumberIndex = randomGenerator.randomize(tokens.length);
        ...
        Address.sendValue(poolAddress, msg.value);
...
library RandomGenerator {
    ...
    function randomize(Random storage r, uint256 l) internal returns (uint256) {
        r.random =
            uint256(
                keccak256(
                    abi.encode(
