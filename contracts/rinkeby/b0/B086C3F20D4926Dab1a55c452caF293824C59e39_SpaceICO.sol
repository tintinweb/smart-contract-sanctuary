...
contract SpaceICO {
  ...
  uint constant public SEED_INDV_LIMIT = 1500 ether;
  uint constant public GENERAL_INDV_LIMIT = 1000 ether;
  ...
  function toggleSeedInvestors(address[] calldata addresses, bool toggle) external {
  ...
  function fundingCapacity() public view returns (uint) {
    ...
    uint256 balance = spaceCoin.balanceOf(address(this)) / ICO_EXCHANGE_RATE;

    return totalContributed > balance ? 0 : balance - totalContributed;
  }

  function availableToContribute() public view returns (uint) {
  ...
  function availableToContribute(address user) public view returns (uint) {
    ...
    if (phase == Phase.Seed) {
      ...
      uint limit = Math.min(available, SEED_INDV_LIMIT);
      return limit - spent;
    }
    if (phase == Phase.General) {
      uint limit = Math.min(available, GENERAL_INDV_LIMIT);
  ...
  function transitionToNextPhase(Phase expectedCurrent) external onlyOwner {
  ...
  function contribute() external payable {
    ...
    uint available = availableToContribute();
