...
contract TheORB is Context, IBEP20, Ownable {
    ...
    function endPresaleProvideLiquidity(bool isFirstProvider) external onlyOwner {
        ...
        if(!isFirstProvider){
            (uint reserveA, uint reserveB) = PancakeLibrary.getReserves(pancakeswapRouter.factory(), address(this), pancakeswapRouter.WETH());
            amountBOptimal = pancakeswapRouter.quote(ORBinContract, reserveA, reserveB);
        }


        approveInternal(address(this), address(pancakeswapRouter), ORBinContract);    
        pancakeswapRouter.addLiquidityETH{value: BNBinContract}(address(this),ORBinContract, ORBinContract, amountBOptimal, address(this), block.timestamp);     // adds the liquidity
    ...
    function getAmountBOptimal()  public view returns (uint256) {

        uint256 ORBinContract = balanceOf(address(this));

        (uint reserveA, uint reserveB) = PancakeLibrary.getReserves(pancakeswapRouter.factory(), address(this), pancakeswapRouter.WETH());
        uint256 amountBOptimal = pancakeswapRouter.quote(ORBinContract, reserveA, reserveB);
