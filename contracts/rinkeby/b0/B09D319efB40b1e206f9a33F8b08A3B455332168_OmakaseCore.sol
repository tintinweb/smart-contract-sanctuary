...
import {Flight, FlightInitArgs, LibFlight} from "./models/Flight.sol";
import {Policy, PolicyInitArgs, LibPolicy} from "./models/Policy.sol";
import {InsurancePackage, LibInsurancePackage} from "./models/InsurancePackage.sol";

import {FlightQueue, LibFlightQueue} from "./libraries/FlightQueue.sol";
...
contract OmakaseCore is KeeperCompatibleInterface {
    ...
    mapping(bytes32 => InsurancePackage) public _insurancePackages;
    ...
    modifier onlyOracle() {
        require(msg.sender == oracle, Errors.OC_CALLER_NOT_ORACLE);
        _;
    }
    modifier onlyAdmin() {
        require(msg.sender == admin, Errors.OC_CALLER_NOT_ADMIN);
        _;
    }
    ...
    function setPolicyFlight(
        ...
        _createFlightIfNotExists(flightId, flightArgs);
    ...
    function updateFlightStatus(
        ...
        if (departed) {
            uint256 hourDifference = (actualDepartureTime -
                flight.departureTime) / 1 hours;
            if (hourDifference != 0) {
                for (uint256 i = 0; i < flight.policies.length; i++) {
                    uint256 indemnity = _getInsurancePackage(flight.policies[i])
                        .calculateIndemnity(hourDifference);
    ...
    function addInsurancePackage(
        ...
        InsurancePackage storage insPackage = _getInsurancePackage(packageId);
        require(
            insPackage.isEmpty(),
    ...
    function setOracle(address oracle_) external onlyAdmin {
        require(oracle_ != address(0), Errors.OC_NEW_ORACLE_ZERO_ADDRESS);
    ...
    {
        return _getInsurancePackage(insPackageId);
    }
        ...
        private
    {
        ...
        policy.initialize(_insurancePackages, args);
    ...
    function _setPolicyFlight(bytes32 policyId, bytes32 flightId) private {
        ...
        _getFlight(flightId).addPolicy(
            policyId,
            _getInsurancePackage(policy.packageId).delayInterval
        );
    ...
    function _getInsurancePackage(bytes32 insPackageId)
        private
        view
        returns (InsurancePackage storage)
    {
        return _insurancePackages[insPackageId];
    }
}
...
library Errors {
    ...
    // Policy.sol
    string constant POL_ALREADY_REGISTERED =
        "Policy: Policy has already been registered";
    string constant POL_INVALID_INSURANCE_PACKAGE =
        "Policy: Invalid insurancePackage";
...
library LibFlightQueue {
    ...
    function addFlightToQueue(
        ...
        require(!flights[id].isEmpty(), Errors.FQ_FLIGHT_NOT_INSERTED);
...
library LibFlight {
        ...
        internal
    {
        ...
        self.nextCheck = args.arrivalTime;
...
library LibPolicy {
    ...
    function initialize(
        Policy storage self,
        mapping(bytes32 => InsurancePackage) storage insurancePackages,
        PolicyInitArgs calldata args
    ) internal {
        require(isEmpty(self), Errors.POL_ALREADY_REGISTERED);

        self.userHash = args.userHash;
        self.packageId = args.packageId;

        require(
            !insurancePackages[self.packageId].isEmpty(),
            Errors.POL_INVALID_INSURANCE_PACKAGE
        );
    }
