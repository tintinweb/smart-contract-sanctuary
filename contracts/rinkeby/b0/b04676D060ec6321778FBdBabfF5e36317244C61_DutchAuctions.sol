...
contract DutchAuctions is OwnableClone, RevenueSplit, SalesHistory {
  ...
  EnumerableSet.UintSet internal currentListingIds;
  ...
  function calculateCurrentPrice(uint256 listingId) public view returns (uint256) {
    ...
    uint256 premium =
      SafeMath.div(SafeMath.mul(delta, listing.endTime - currentTime), listing.endTime - listing.startTime);
  ...
  function bid(
    ...
    // solhint-disable-next-line not-rely-on-time
    require(listing.closeTime > block.timestamp && listing.openTime <= block.timestamp, "not open");
  ...
  function addListing(
    ...
    // solhint-disable-next-line not-rely-on-time
    require(startTime > block.timestamp, 'auction start is past');
    require(startTime < endTime, 'auction span is negative time');
    require(startPrice > endPrice, 'auction must decay in price');
    require(openTime <= startTime, "start before open");
    require(closeTime >= endTime, "close before end");
    ...
    listingsById[idx].openTime = openTime;
    listingsById[idx].closeTime = closeTime;

    currentListingIds.add(idx);

    emit ListingAdded(idx, tokenAddress, offeringId, startPrice, startTime, endPrice, endTime, openTime, closeTime);
  }
  ...
  function updateListing(
    ...
    // solhint-disable-next-line not-rely-on-time
    uint256 currentTime = block.timestamp;
    require(listingsById[listingId].startTime > currentTime, 'Auction has already started');
    require(startTime > currentTime, 'auction start is past');
    require(startTime < endTime, 'auction span is negative time');
    require(startPrice > endPrice, 'auction must decay in price');
    require(openTime <= startTime, "start before open");
    require(closeTime >= endTime, "close before end");
    ...
    listingsById[listingId].openTime = openTime;
    listingsById[listingId].closeTime = closeTime;

    emit ListingUpdated(listingId, startPrice, startTime, endPrice, endTime, openTime, closeTime);
  }
  ...
  function getAllListings() public view returns (OutputListing[] memory listings) {
    ...
    for (uint256 idx = 0; idx < numListings; idx++) {
      ...
      listings[idx].openTime = listingsById[lid].openTime;
      listings[idx].closeTime = listingsById[lid].closeTime;
    }
  }
}
