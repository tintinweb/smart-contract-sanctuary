...
contract TemplateFactory is ITemplateFactory, ReentrancyGuardUpgradeable, NetworkAccessUpgradeable {
    ...
    function predictDeployedAddress(bytes32 templateId, uint256 _version, bytes memory arguments) external override view returns (address) {
        return Deployer.predictAddress(address(this), _deployCode(templateId, _version, arguments), _salt(templateId, _version));
    }

    function deployTemplateVersionLatest(bytes32 templateId, bytes memory arguments, bytes[] memory functionCalls) external override returns (address instance) {
        return deployTemplateVersion(templateId, latestVersion(templateId), arguments, functionCalls);
    }

    function deployTemplateVersion(bytes32 templateId, uint256 _version, bytes memory arguments, bytes[] memory functionCalls) public override nonReentrant returns (address instance) {
        ...
        instance = Deployer.deploy(_deployCode(templateId, _version, arguments), _salt(templateId, _version));
        ...
        emit TemplateDeployed(instance, templateId, arguments, functionCalls, _msgSender());
    }
    ...
    function _deployCode(bytes32 templateId, uint256 _version, bytes memory arguments) private view returns (bytes memory) {
        return abi.encodePacked(_template[templateId].version[_version].creationCode, arguments);
    }
}
...
interface ITemplateFactory {
    ...
    event TemplateDeployed(address indexed instance, bytes32 indexed templateId, bytes arguments, bytes[] functionCalls, address indexed sender);
    ...
    function predictDeployedAddress(bytes32 templateId, uint256 version, bytes memory arguments) external view returns (address);

    function deployTemplateVersionLatest(bytes32 templateId, bytes memory arguments, bytes[] memory functionCalls) external returns (address instance);

    function deployTemplateVersion(bytes32 templateId, uint256 version, bytes memory arguments, bytes[] memory functionCalls) external returns (address instance);
