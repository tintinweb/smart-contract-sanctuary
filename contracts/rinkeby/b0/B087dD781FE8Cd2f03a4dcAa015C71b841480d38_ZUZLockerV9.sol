...
contract ZUZLockerV9 is owned, ERC721Holder, ERC1155Holder {
    
    address zuzAddress;
    address zutAddress;
    address usdtAddress;
    address usdcAddress;
    function setZUZAddress(address _zuzAddress) public onlyManager {
        zuzAddress = _zuzAddress;
    }
    function setZUTAddress(address _zutAddress) public onlyManager {
        zutAddress = _zutAddress;
    }
    function setUSDTAddress(address _usdtAddress) public onlyManager {
        usdtAddress = _usdtAddress;
    }
    function setUSDCAddress(address _usdcAddress) public onlyManager {
    ...
    function lockTokens(
        ...
        bool _isLiquidityToken, uint _tokenId, uint256 _tokenAmount, 
        string memory _serviceFeeToken, uint _serviceFee,
        ...
        if(keccak256(abi.encodePacked(_serviceFeeToken)) ==  keccak256(abi.encodePacked("eth"))) {
            require(msg.value >= _serviceFee, "Service Fee in ETH is less than required");
        }
        else if(keccak256(abi.encodePacked(_serviceFeeToken)) ==  keccak256(abi.encodePacked("zuz"))) {
            IERC20(zuzAddress).transferFrom(msg.sender, feeRecipient, _serviceFee);
        }
        else if(keccak256(abi.encodePacked(_serviceFeeToken)) ==  keccak256(abi.encodePacked("zut"))) {
            IERC20(zutAddress).transferFrom(msg.sender, feeRecipient, _serviceFee);
        }
        else if(keccak256(abi.encodePacked(_serviceFeeToken)) ==  keccak256(abi.encodePacked("usdt"))) {
            IERC20(usdtAddress).transferFrom(msg.sender, feeRecipient, _serviceFee);
        }
        else if(keccak256(abi.encodePacked(_serviceFeeToken)) ==  keccak256(abi.encodePacked("usdc"))) {
            IERC20(usdcAddress).transferFrom(msg.sender, feeRecipient, _serviceFee);
        }
        else {
            require(false, "ServiceFeeToken not listed in contract");
        }
