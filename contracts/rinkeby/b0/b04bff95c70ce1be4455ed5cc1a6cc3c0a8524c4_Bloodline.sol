...
contract Bloodline is ERC721Tradable, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) public migrantCount;
    mapping(uint256 => bool) public isMigrant;
    ...
    function getAppearance(uint256 tokenId) public view returns (string memory) {
        ...
        if (greatness == 4) {
            ...
            if ((keccak256(bytes(appearance)) == keccak256(bytes("skinny")) || keccak256(bytes(appearance)) == keccak256(bytes("overweight"))) 
    ...
    // function migrate(uint256 generation) public nonReentrant {
    //     require(generation > 1 && generationCount[generation - 1] > 1 << generation, "Invalid migration");
    //     require(migrantCount[generation] < 500, "Migration limit reached");
    //     _safeMint(_msgSender(), descendentID);
    //     migrantCount[generation]++;
    //     generationCount[generation]++;
    //     isMigrant[descendentID] = true;
    //     generationMap[descendentID] = generation;
    //     descendentID++;
    // }
