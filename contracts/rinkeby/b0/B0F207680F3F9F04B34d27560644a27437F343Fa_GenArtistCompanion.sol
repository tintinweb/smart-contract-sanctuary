...
contract GenArtistCompanion is ReentrancyGuard, ERC721Enumerable, Ownable {
    ...
    constructor(address parentAddress, uint _parentSupply) ERC721('GenArtistCompanion', 'GENARTISTCOMPANION') {
        parent = Parent(parentAddress);
        PARENT_SUPPLY = _parentSupply;      
    }


    function claim(uint256 _projectId, uint256 startingIndex, uint256 numberOfTokens) public {
    ...
    function claimAll(uint256 _projectId) public {
        claim(_projectId, 0, parent.balanceOf(msg.sender));
    }    

    function claimByTokenIds(uint256 _projectId, uint256[] calldata _parentTokenIds) external {
        require(projects[_projectId].isClaimActive, 'Claim must be active to mint tokens');
    ...
    function _mintToken(address _to, uint256 _tokenId) internal {
        bytes32 seed = keccak256(abi.encodePacked(_tokenId, _to, block.number, block.difficulty));
    ...
    function setProjectScriptJSON(uint256 _projectId, string memory _projectScriptJSON) external onlyOwner {
