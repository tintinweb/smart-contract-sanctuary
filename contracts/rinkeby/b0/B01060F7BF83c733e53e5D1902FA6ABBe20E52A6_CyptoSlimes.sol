...
contract CyptoSlimes is ERC721, ERC721URIStorage, ERC721Burnable, Ownable  {  
    struct Slimes {
        string  url;   
        string  rare; // H2O, Gold, Diamond, Californium, AntiMatter, SecretMetaverse 
    }

    string[] public slimes; // First Item has Index 0 
    mapping(string => bool) _slimeExists;
      
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;
    constructor() ERC721("CyptoSlimes", "CSL")  { 
    }  

    function slimeBorn(address to, string memory uri, string memory slimeHash) public onlyOwner returns (uint256) {
        require(!_slimeExists[slimeHash]);
        slimes.push(slimeHash);
