...
address[] internal freeTokenRecivers;
...
constructor() ERC721("Homospacien NFT", "Homospacien") {
    ...
    maxiumum_Early_Bird_TPT=default_token_per_transactions;
...
returns(uint256)
{
    ...
    payPrice( msg.sender,  _earlyBirdPrice);
...
returns(uint256)
{
    ...
    payPrice( msg.sender,  _preSalePrice);
    _preSales.increment();
    }
...
returns(uint256){
    ...
    payPrice( msg.sender,  _mainSalePrice);
...
function mintFreeHomospacien()
public
returns (uint256)
{

    (bool _isListed, ) = isWhiteListed(msg.sender, TOKEN_TYPE_FREE);
    (bool _isMember, ) = isTeamMember(msg.sender);
    require((_isMember || _isListed), "Free token can be minted only by team member and whitelisted users");
    _tokenIds.increment();
    ...
    _safeMint(msg.sender, newItemId);
    _setTokenURI(newItemId, tokenURI);
    tokens[newItemId] = TokenData({price: 0.0});
    _homospacienExists[tokenURI] = true;
...
{    address _platform= payable(owner());
    require(_address != address(0) && _platform != _address, "Invalid address.");
    for (uint256 s = 0; s < team.length; s += 1){
    if (_address == team[s]) return (true, s);
    }
    return (false, 0);
}
 ...
 * @param _address The address to add.
 */
function addToTeam(address _address)
public
onlyOwner
{
    address _platform= payable(owner());
    ...
    if(!_isTeamMember) team.push(_address);
}
