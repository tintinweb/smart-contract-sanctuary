...
contract Marketplace {
    ...
    function finding(Board storage self, User[] memory user, string memory _name, uint256 num)
    ...
    function findID(User[] memory user, address payable _addr)
    ...
    function findAddr(User[] memory user, string memory _id)
    ...
    function search(Board storage self, User[] memory user, address payable _addr, string memory _name) 
    ...
    internal view returns(string memory, uint256, address payable) {
        string memory _name;
        uint256 _price;
    ...
    function checkRec(User[] memory user, address _addr) 
    ...
    public payable {
        ...
        string memory _message = PublicBoard.board[SellerAddr][_count].message;
        require(msg.value == GoodsPrice * 2);
        insert(SellerBoard, seller, SellerAddr, _name, GoodsPrice, payable(msg.sender), SellerAddr, _message);
        insert(BuyerBoard, buyer, payable(msg.sender), _name, GoodsPrice, payable(msg.sender), SellerAddr, _message);
