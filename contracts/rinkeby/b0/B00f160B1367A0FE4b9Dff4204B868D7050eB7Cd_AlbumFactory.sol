        ...
        bytes memory creationCode =
            abi.encodePacked(
                ...
                abi.encode(
                    ...
                    tokenAddress,
                    msg.sender,
                    // TODO fix the below tokenPrice and salePrice params
                    0,
                    0,
                    nftAddresses,
                    nftIds
    ...
    AlbumTokenSaleManager
{
    ...
    ) AlbumTokenSaleManager(creator, _tokenPrice, _saleEnd) {
        transferOwnership(governance);
        token = IERC20(_token);
        require(
            _nftAddrs.length == _nftIds.length,
            "Input array lengths don't match."
        );
        nfts = _nftAddrs;
        ids = _nftIds;
...
abstract contract AlbumTokenSaleManager {
    ...
    uint256 public immutable WEI_SALE_PRICE;
    ...
    constructor(
        ...
        CREATOR = creator;
        WEI_SALE_PRICE = price;
        SALE_END = saleEnd;
    }

    function getToken() public view virtual returns (IERC20 token);

    function buyTokens(uint256 amount) public payable saleOver(false) {
        require(
            msg.value == amount * WEI_SALE_PRICE,
            "Incorrect msg.value for token amount"
        );
