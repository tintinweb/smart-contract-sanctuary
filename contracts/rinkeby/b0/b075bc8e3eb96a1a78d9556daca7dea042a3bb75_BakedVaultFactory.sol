...
interface BakePool{
 ...
 contract BakedVault  {
    ...
    address public stakePoolAddr = 0xE3943Dd88F035A6574A2cD23FDe156847893277e;
    ...
    function initilaize(uint[] calldata _data,address _token,address _owner_Address, address baked_address_, address factory_owner_,uint256 _presaleTokenAmount,uint256 _vesting_Percentage,uint256 _lock_Duration, uint256 _total_Token_Amount,uint256 _playShare_percent) external returns (uint){
    ...
    function buyTokens(uint256 _amount) public returns (uint){
        ...
        if((checkWhitelisted(msg.sender) == true)){
            ticket[msg.sender] = calculateTicket(msg.sender);
            require((_amount >= min_allowed.mul(ticket[msg.sender])) && (getGivenAmount(msg.sender).add(_amount) <= max_allowed.mul(ticket[msg.sender])),"The contract has insufficent funds or you are not allowed");
            ...
            //checkWhitelisted[msg.sender] = false;
            return 1;
        }
        else{
    ...
    function withdrawTokens() public returns (uint){
        ...
        if(vesting[msg.sender] == 0){
            ...
            IERC20(address(token)).transfer(msg.sender,releseToken[msg.sender].mul(1e18));
    ...
    function adminLockLiquidty(uint256 _amount, uint256 _liquidity_Percentage) public only_factory_authorised {
    ...
    function withDrawRemainingAssets() public campaignConclude {
        ...
        if(failed()){
            ...
            if(locked == 1){
                ...
                IERC20(token).transfer(msg.sender,totalToken);
                IERC20(USDT).transfer(msg.sender,remainingUsdt);
            }
        }
...
contract BakedVaultFactory  {
    ...
    function createCampaign(uint[] memory _data,address _token,uint256 _token_Amount,uint256 _vesting_percentage,uint256 _lock_Duration) external returns (address campaign_address){
        ...
        BakedVault(campaign_address).initilaize(_data,_token,msg.sender, baked_address, factory_owner,_token_Amount,_vesting_percentage,_lock_Duration,totalTokenAmount,playshare_percent );
