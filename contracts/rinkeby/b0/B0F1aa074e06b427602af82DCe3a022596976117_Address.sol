...
contract LpTokenLocker {
  ...
  uint256 penaltyFeeTokenPercentage;
  ...
  constructor(IPancakeFactory _pancakeFactory, address payable _feesReceiver, uint256 _penaltyFeeTokenPercentage) {
      pancakeFactory = _pancakeFactory;
      feeReceiver = _feesReceiver;
      penaltyFeeTokenPercentage = _penaltyFeeTokenPercentage;
  }

  function lockTokens(address token, uint256 amount, uint256 unlockTime,
             address payable withdrawer) public returns (uint256 lockId) {
    ...
    emit OnTokenLock(lockId, token, withdrawer, amount, unlockTime);
    return lockId;
  }
  ...
  function withdrawPartially(uint256 lockId, uint256 amount) public onlyLockOwner(lockId) {
    TokenLock storage lock = tokenLocks[lockId];
  ...
  function checkLpTokenIsPancake(address _lpToken) public view returns (bool) {
