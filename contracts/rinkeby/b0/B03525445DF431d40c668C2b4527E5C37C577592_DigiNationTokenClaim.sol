	...
	OwnableUpgradeable
{
	...
	mapping(address => bool) private _supportTokenContracts;
	mapping(address => mapping(address => uint256)) private _claimNonce;

	event Claimed(address sender, address tokenContract, uint256 amount, uint256 nonce);
	...
	function setSupportTokenContract(address tokenContract) external onlyOwner {
		_supportTokenContracts[tokenContract] = true;
	}

	function isSupportTokenContract(address tokenContract) external view returns (bool) {
		return _supportTokenContracts[tokenContract];
	}
    ...
    function claim(
		...
		// Check sender
		require(sender == _msgSender(), "Sender error");

		// Check support token contract
		require(_supportTokenContracts[tokenContract] == true, "Unsupported token contract");

		// Check Signature
		bytes32 message = keccak256(abi.encodePacked(sender, tokenContract, amount, nonce));
		...
		// Check token balance
    	IERC20Upgradeable claimToken = IERC20Upgradeable(tokenContract);
		require(claimToken.balanceOf(address(this)) >= amount, "Insufficient available balance");
        ...
        emit Claimed(sender, tokenContract, amount, nonce);
	}
