...
// File contracts/interfaces/ISATOERC20.sol
...
contract SatoshiTreasury is Ownable {
  ...
  address public sSATO;
  uint256 public sSATOQueue; // Delays change to sSATO address
  ...
  /**
        @notice allow approved address to deposit an asset for SATO
        @param _amount uint
  ...
  function deposit(
    ...
    // mint SATO needed and store amount of rewards for distribution
    send_ = value.sub(_profit);
    IERC20Mintable(SATO).mint(msg.sender, send_);
  ...
  /**
        @notice allow approved address to burn SATO for reserves
        @param _amount uint
        @param _token address
     */
  function withdraw(uint256 _amount, address _token) external {
    ...
    uint256 value = valueOf(_token, _amount);
    ISATOERC20(SATO).burnFrom(msg.sender, value);
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sSATO).balanceOf(msg.sender); // Can only borrow against sSATO held
  ...
  /**
        @notice allow approved address to repay borrowed reserves with SATO
        @param _amount uint
     */
  function repayDebtWithSATO(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");

    ISATOERC20(SATO).burnFrom(msg.sender, _amount);

    debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
    totalDebt = totalDebt.sub(_amount);

    emit RepayDebt(msg.sender, SATO, _amount, _amount);
  }
  ...
  function mintRewards(address _recipient, uint256 _amount) external {
    ...
    IERC20Mintable(SATO).mint(_recipient, _amount);
  ...
  function excessReserves() public view returns (uint256) {
    return totalReserves.sub(IERC20(SATO).totalSupply().sub(totalDebt));
  }
  ...
  /**
        @notice returns SATO valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
  ...
  function queue(MANAGING _managing, address _address) external onlyManager returns (bool) {
    ...
    } else if (_managing == MANAGING.XSATO) {
      // 9
      sSATOQueue = block.number.add(blocksNeededForQueue);
    } else return false;
