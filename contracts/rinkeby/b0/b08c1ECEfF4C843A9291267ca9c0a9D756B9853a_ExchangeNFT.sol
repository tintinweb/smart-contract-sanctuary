...
contract ExchangeNFT is ERC721Holder, Ownable, Pausable {
    ...
    event NewOneTimeOfferETH(address indexed seller, uint256 indexed tokenId, uint256 price);
    event NewOneTimeOfferERC20(address indexed seller, uint256 indexed tokenId, uint256 price);
    ...
    function readyToSellTokenETH(uint256 _tokenId, uint256 _price) public whenNotPaused {
        ...
        emit AskETH(msg.sender, _tokenId, _price);
        emit NewOneTimeOfferETH(msg.sender, _tokenId, _price);
    }
    //上架一口价销售[ERC20]
    function readyToSellTokenERC20(uint256 _tokenId, uint256 _price) public whenNotPaused {
        ...
        _userSellingTokens[msg.sender].add(_tokenId);
        emit AskERC20(msg.sender, _tokenId, _price);
        emit NewOneTimeOfferERC20(msg.sender, _tokenId, _price);
    }
	...
	_deadlineMapErc20.set(_tokenId, ts);
     ...
     //获取自己对某个NFT的出价
     function getMyOfferPrice(uint256 _tokenId) public view returns (uint256) {
         return _tokenBidOffers[_tokenId][msg.sender];
     }
    ...
    // Withdraw without caring about rewards. EMERGENCY ONLY.
     ...
     function distributeNFTERC20(address _receiver, uint256 _tokenId, uint256 _price) external onlyOperator returns (bool){
         ...
         quoteErc20.safeTransferFrom(address(this), _tokenSellers[_tokenId], _price);
    ...
    /***********************************|
     ...
     function getOperatorh(uint256 _index) public view onlyOwner returns (address){
