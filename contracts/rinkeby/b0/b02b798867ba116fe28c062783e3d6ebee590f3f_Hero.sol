...
contract Hero is ERC721Tradable {
    ...
    function baseTokenURI() override public pure returns (string memory) {
        return "ar://NAZtYElhp-JzWZA_HHSIcnZUC5QNiqtdtoSlZG2VHyA/";
    }

    function contractURI() public pure returns (string memory) {
        return "ar://Yg4VNMK8sX7aJaXdjKMiyTaSfR-bRwrEORkHHBN2rQE";
    }
    
    function mintRandomTo(address _to) internal {
        ...
        if (selector < prob_numerators[0]) {
            mintRandomMobTo(_to, MOB_START_LEVEL, random);
        } else if (selector < prob_numerators[0] + prob_numerators[1]) {
            mintRandomBossTo(_to, BOSS_START_LEVEL, random);
        } else {
            mintRandomHeroTo(_to, HERO_START_LEVEL);
        }
    }
    ...
    function mintSpecificTokenTo(address _to, CharacterType _charType, uint256 _level, uint256 _character, uint256 _card) internal {
    ...
    function mintRandomMobTo(address _to, uint256 _level, uint256 random) internal {
        ...
        mintSpecificTokenTo(_to, CharacterType.Mob, _level, character, state[_level].mobs[character]);
    }    

    function mintRandomBossTo(address _to, uint256 _level, uint256 random) internal {
        //Look up characters that have available cards
        ...
        mintSpecificTokenTo(_to, CharacterType.Boss, _level, character, state[_level].bosses[character]);
    }    
    
    function mintRandomHeroTo(address _to, uint256 _level) internal {

        if (state[_level].heroes >= HERO_CHARACTERS) {
            revert("No more heroes available");
        }
        
        mintSpecificTokenTo(_to, CharacterType.Hero, HERO_START_LEVEL, 0, state[HERO_START_LEVEL].heroes);
    }
    ...
    function upgrade(uint256[] calldata tokens) external {
        ...
        require((tokens[0] != tokens[1])&&(tokens[0] != tokens[2]),"Duplicate cards were supplied");

        //Check that caller owns all supplied cards
        for (uint i = 0; i<tokens.length; i++) {
            require(_isApprovedOrOwner(msg.sender, tokens[i]), "Only onwed or approved cards may be used for upgrade");
        }

        //Check that all cards are of same character type
        CharacterType charType;
        charType = getCharacterType(tokens[0]);
        for (uint i = 1; i<tokens.length; i++) {
            require(charType == getCharacterType(tokens[i]), "All cards should be of the same character type");
        }

        //Check that hero cards are not provided
        require(charType != CharacterType.Hero, "Hero cannot be upgraded");

        //Check that all cards are of same character type
        uint256 char;
        char = getCharacter(tokens[0]);
        for (uint i = 1; i<tokens.length; i++) {
            require(char == getCharacter(tokens[i]), "All cards should be of the same character");
        }

        //Check that all cards are of same level
        ...
        require(level<MAX_LEVELS, "Maxxed already");
    ...
    function presaleMint(address _toAddress) external payable {
    ...
    function publicSaleMint(uint _qty, address _toAddress) external payable {
