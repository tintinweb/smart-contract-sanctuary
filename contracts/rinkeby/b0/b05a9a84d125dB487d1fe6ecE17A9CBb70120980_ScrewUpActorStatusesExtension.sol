...
contract ScrewUpActorStatusesExtension is ScrewUpInGameActorExtension, IScrewUpActorStatusesExtension {
    ...
    function getStatusPresetDefinitions() external view virtual override returns (FStatusPresetDefinition[] memory){
        ...
        for(uint256 pid = 1; pid <= lastPresetId; pid++){
            uint256[] memory _preset_statusIds = _getPresetStatusIds(pid);
            FStatusPresetDefinition memory _definition;
            _definition.Id = pid;
            _definition.Name = _preset_names[pid];
            _definition.Values = new FPointValue[](_preset_statusIds.length);
            for(uint256 s = 0; s <  _preset_statusIds.length; s++)
                _definition.Values[s] = FPointValue(_preset_statusIds[s], _presets[pid][_preset_statusIds[s]]);
    ...
    function getActorStatuses(uint256 actorId) external view virtual override returns(uint256[] memory sIds,uint256[] memory sValues){
        sIds = _getPresetStatusIds(_actor_st_presets[actorId]);
        sValues = new uint256[](sIds.length);
        for(uint256 i = 0; i < sIds.length; i++)
            sValues[i] = _actor_statuses[actorId][sIds[i]];
    }
    ...
    function _getPresetStatusIds(uint256 presetId) internal view returns(uint256[] memory){
        uint256 statusCount = 0;
        uint256 lastStatusId = _statusIds.current();
        for(uint256 sId= 1; sId<= lastStatusId; sId++){
            if(_presets[presetId][sId] > 0)
               statusCount++;
        }
    ...
    function _applyActorStatusesWithPreset(uint256 actorId,uint256 presetId) internal{
        uint256[] memory _sIds = _getPresetStatusIds(presetId);
        for(uint256 i = 0; i < _sIds.length; i++)
        {
            if(_status_defined[_sIds[i]]){
                _actor_statuses[actorId][_sIds[i]] = _presets[presetId][_sIds[i]];
                emit OnActorStatusUpdated(actorId,_sIds[i],_presets[presetId][_sIds[i]]);
            }
        }
    }
...
interface IScrewUpActorStatusesExtension {
    ...
    function getActorStatuses(uint256 actorId) external view returns(uint256[] memory sIds,uint256[] memory sValues);   
...
interface IScrewUpInGameActor {
    ...
    function createActorFor(address _add,string memory newName,uint256 statusPresetId,uint256 appearancePresetId) external;
