...
contract HashBridge is Ownable {
	...
	event OfferAdd(uint indexed offerId, address indexed token, address indexed payToken, address ownerAddress, address payAddress, uint amount, uint rate, uint fee, uint minPurchase, bool active);
	event OfferUpdate(uint indexed offerId, address payAddress, uint amount, uint rate, uint fee, uint minPurchase, bool active);
	event OrderAdd(uint indexed orderId, uint indexed offerId, address indexed ownerAddress, uint rate, uint fee, address withdrawAddress, uint amount, uint payAmount, address payAddress, uint reservedUntil);
	...
	function addOffer(address _token, uint _amount, address _payToken, uint _rate, uint _fee, address _payAddress, uint _minPurchase) public {
		...
		offers.push(Offer(_token, _amount, _payToken, _rate, _fee, msg.sender, _payAddress, _minPurchase, true));
		emit OfferAdd(offerId, _token, _payToken, msg.sender, _payAddress, _amount, _rate, _fee, _minPurchase, true);
	}
	
	function updateOffer(uint _offerId, uint _amount, uint _rate, uint _fee, address _payAddress, uint _minPurchase) public {
		...
		offers[_offerId].fee = _fee;
		offers[_offerId].payAddress = _payAddress;
		offers[_offerId].minPurchase = _minPurchase;
		emit OfferUpdate(_offerId, _payAddress, _amount, _rate, _fee, _minPurchase, offers[_offerId].active);
	}
	...
	function activateOffer(uint _offerId) public {
		...
		emit OfferUpdate(_offerId, offers[_offerId].payAddress, offers[_offerId].amount, offers[_offerId].rate, offers[_offerId].fee, offers[_offerId].minPurchase, true);
	}
	...
	function deactivateOffer(uint _offerId) public {
		...
		emit OfferUpdate(_offerId, offers[_offerId].payAddress, offers[_offerId].amount, offers[_offerId].rate, offers[_offerId].fee, offers[_offerId].minPurchase, false);
	}
	...
	function addOrder(uint _offerId, address _withdrawAddress, uint _amount, uint _payAmount) public {
		...
		uint fee = offers[_offerId].fee;
		if (_amount > 0) {
			_payAmount = _amount * rate / (10 ** RATE_DECIMALS) + fee;
		} else {
			_amount = (_payAmount - fee) * (10 ** RATE_DECIMALS) / rate;
		}
		...
		orders.push(Order(_offerId, rate, fee, msg.sender, _withdrawAddress, _amount, _payAmount, _payAddress, reservedUntil, false));
		emit OrderAdd(orderId, _offerId, msg.sender, rate, fee, _withdrawAddress, _amount, _payAmount, _payAddress, reservedUntil);
	}
