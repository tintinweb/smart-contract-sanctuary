...
contract SuperValve is SuperAppBase, AccessControl {
    ...
    uint16 public numValidPipeAddresses;
    ...
    function setUserFlowData(PipeFlowData[] memory _pipeFlowData) public {
        require(numValidPipeAddresses == _pipeFlowData.length, "SuperValve: Your number of allocations is incorrect.");
        isFullyAllocated(_pipeFlowData);
        uint256 userPipeFlowDataIndex = userPipeFlowData.length;
        uint256 userPipeFlowId = userPipeFlowDataIndex++;
        userAllocations[msg.sender].userPipeFlowId = userPipeFlowId;
        for (uint256 i; i < _pipeFlowData.length; i++) {
            require(
                validPipeAddresses[_pipeFlowData[i].pipeAddress] == true,
                "SuperValve: This is not a registered vault address."
            );
            userPipeFlowData[userPipeFlowId].push(_pipeFlowData[i]);
        }
    }
