...
contract Repurchase is IRepurchase, Ownable {
    ...
    uint256 public lotteryAllocPoint = 2000;
    uint256 public swapMiningAllocPoint = 4000;
    uint256 public destroyAllocPoint = 4000;
    ...
    function setSwapMiningAddress(address _newAddress) public onlyOwner {
        require(_newAddress != address(0), "Repurchase: zero address");
        swapMiningAddress = _newAddress;
    }

    function setSwapMiningAllocPoint(uint256 _swapMiningAllocPoint) public onlyOwner {
    ...
    function setDestroyAllocPoint(uint256 _destroyAllocPoint) public onlyOwner {
        destroyAllocPoint = _destroyAllocPoint;
    }
    ...
    function addCaller(address _caller) public onlyOwner returns (bool) {
        require(_caller != address(0), "Repurchase: zero address");
        return EnumerableSet.add(_callers, _caller);
    }
    ...
    function getCaller(uint256 _index) public view returns (address){
        require(_index <= getCallerLength() - 1, "Repurchase: index out of bounds");
    ...
    function burn(address pair) public onlyCaller returns (bool) {
        ...
        ICCPair(pair).transfer(pair, balance);
        // send liquidity to pair
        (uint amount0, uint amount1) = ICCPair(pair).burn(address(this));
    ...
    function autoSwap(uint256 amountIn, address token) public onlyCaller returns(uint256) {
        ...
        if (token == targetToken) {
        ...
        else if (ICCFactory(factory).getPair(token, targetToken) != address(0)) {
            // exist direct pair
            // console.log("direct");
            ...
            for (uint256 index = 0; index < length; index++) {
                ...
                if (ICCFactory(factory).getPair(token, intermediate) != address(0) && ICCFactory(factory).getPair(intermediate, targetToken) != address(0)) {
    ...
    function swap(uint256 amountIn, address tokenIn, address tokenOut) public onlyCaller returns(uint256) {
        address pair = ICCFactory(factory).getPair(tokenIn, tokenOut);
        require(pair != address(0), "Repurchase: pair not exists");

        (uint reserveIn, uint reserveOut) = ICCFactory(factory).getReserves(tokenIn, tokenOut);
        uint amountOut = ICCFactory(factory).getAmountOut(amountIn, reserveIn, reserveOut);
    ...
    function purchase(address pair) public override onlyCaller {
        burn(pair);
        address token0 = ICCPair(pair).token0();
        address token1 = ICCPair(pair).token1();
    ...
    function updatePurchase(address pair) public override onlyCaller {
        ICCPair lp = ICCPair(pair);
        lp.updateFee();
    ...
    function toSpender() public override  {
        uint256 balance = IERC20(targetToken).balanceOf(address(this));
        require(balance > 0, "Repurchase: balance equal 0");
        uint256 totalAllocPoint = lotteryAllocPoint + swapMiningAllocPoint + destroyAllocPoint;
        ...
        if (lotteryAmount > 0) {
            ...
            ISpender(lotteryAddress).topUp(lotteryAmount);
        }
        if (swapMiningAmount > 0) {
            IERC20(targetToken).approve(swapMiningAddress, swapMiningAmount);
            ISpender(swapMiningAddress).topUp(swapMiningAmount);
        }
        if (destroyAmount > 0) {
