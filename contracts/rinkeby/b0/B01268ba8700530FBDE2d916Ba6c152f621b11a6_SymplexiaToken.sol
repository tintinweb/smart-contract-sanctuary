...
//   ======================================
    ...
    function swapAndLiquify(uint256 numTokensToLiquidity) internal nonReentrant {
...
//   ==========================================
    ...
    function _beforeTokenTransfer(address sender, address, uint256) internal override {    
        ...
        if ( autoLiquidity && !inSwapAndLiquify &&  sender != liquidityPair ) {
            ...
            uint256 _tokenToLiquidity = (_tokenBalance > _maxTokensPerTx) ? _maxTokensPerTx : _tokenBalance;

            if ( _tokenToLiquidity >= _liquidityThreshold ) { 
                swapAndLiquify (_liquidityThreshold); }	                //*** Add Liquidity  *** 
        }
    }
...
//   ==========================================
    ...
    function _afterTokenTransfer(address sender, address, uint256 amount) internal override {    

        if ( autoLiquidity && !inSwapAndLiquify &&  sender == liquidityPair && isAdjustable ) {
            uint256 _tokenToLiquidity;
            uint256 _stabilityRatio = (Wallet[liquidityPair].balance * 10) / 100;
            _liquidityDeviation    += amount;
...
contract SymplexiaToken is  Taxable {
  ...
  //
    ...
    constructor ()  ERC20("BETA21", "BTG21") BaseToken ( 
