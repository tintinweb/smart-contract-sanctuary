...
contract DirectLoanFixed is DirectLoanBase {
    ...
    function acceptOffer(
        ...
        LoanChecksAndCalculations._loanSanityChecks(_offer, nftWrapper, hub);
        LoanChecksAndCalculations._loanSanityChecksOffer(_offer);
        _acceptOffer(
            LOAN_TYPE,
    ...
    function acceptListing(
        ...
        address nftWrapper = _getWrapper(_offer.nftCollateralContract);
        LoanChecksAndCalculations._loanSanityChecks(_offer, nftWrapper, hub);
        LoanChecksAndCalculations._bindingTermsSanityChecks(_listingTerms, _offer);
        _acceptListing(
            LOAN_TYPE,
        ...
        returns (uint256 adminFee, uint256 payoffAmount)
    {
        ...
        adminFee = LoanChecksAndCalculations._computeAdminFee(
            interestDue,
...
abstract contract DirectLoanBase is IDirectLoanBase, BaseLoan, NftReceiver, LoanData {
     ...
     */
    function getWhetherNonceHasBeenUsedForUser(address _user, uint256 _nonce) public view override returns (bool) {
    ...
    function _renegotiateLoan(
        ...
        (address borrower, address lender) = LoanChecksAndCalculations._renegotiationChecks(
            loan,
            _loanId,
            _newLoanDuration,
            _newMaximumRepaymentAmount,
        ...
        if (_renegotiationFee > 0) {
            renegotiationAdminFee = LoanChecksAndCalculations._computeAdminFee(
                _renegotiationFee,
    ...
    function _createLoanNoNftTransfer(
        ...
        uint256 referralfee = LoanChecksAndCalculations._computeReferralFee(
    ...
    function _payBackLoan(
        ...
        uint256 revenueShare = LoanChecksAndCalculations._computeRevenueShare(
            adminFee,
        ...
        returns (LoanExtras memory)
    {
        ...
        return
            LoanExtras({
                ...
                revenueShareInBasisPoints: LoanChecksAndCalculations._getRevenueSharePercent(_revenueSharePartner, hub),
...
library LoanChecksAndCalculations {
     ...
     */
    function payBackChecks(uint256 _loanId) public view {
        // Sanity check that payBackLoan() and liquidateOverdueLoan() have never been called on this loanId.
        // Depending on how the rest of the code turns out, this check may be unnecessary.
     ...
     */
    function _getRevenueSharePercent(address _revenueSharePartner, INftfiHub _hub) public view returns (uint16) {
     ...
     *
     */
    function _loanSanityChecksOffer(LoanData.Offer memory _offer) public pure {
        require(
    ...
    function _renegotiationChecks(
        ...
        require(loanCoordinatorData.loanContract == address(this), "invalid loan contract");
     ...
     *
     */
    function _bindingTermsSanityChecks(LoanData.ListingTerms memory _listingTerms, LoanData.Offer memory _offer)
        public
        pure
    {
     ...
     */
    function _computeRevenueShare(uint256 _adminFee, uint256 _revenueShareInBasisPoints) public pure returns (uint256) {
     ...
     * fee.
     */
    function _computeAdminFee(uint256 _interestDue, uint256 _adminFeeInBasisPoints) public pure returns (uint256) {
