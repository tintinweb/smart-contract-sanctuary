...
contract NewBuff is Context, IERC20, Ownable {
        ...
        if(!_isExcluded[sender] || !_isExcluded[recipient]) {
            ...
            if(recipient == _pair) {
                ...
                if (basisOf(sender) <= (1 ether) * amounts[1] / amount) {
                    _setFees(300, 300, 0);
                    uint tTransferAmountForSale = _getValuesForSale(amount);
                    _transferToExcluded(sender, recipient, tTransferAmountForSale);
                //    uint holderFee = amount.mul(_holderUsualFee).div(100);
                //     uint affiliateFee = amount.mul(_affiliateUsualFee).div(100);
    ...
    function _getValuesForSale(uint256 tAmount) private view returns (uint256) {
        (uint256 tFee, uint256 tBurn, uint256 tMarket) = _getTBasics(tAmount, _TAX_FEE, _BURN_FEE, _MARKET_FEE);
        uint256 tTransferAmountForSale = tAmount.add(tFee).add(tBurn).add(tMarket);
        return tTransferAmountForSale;
    }
