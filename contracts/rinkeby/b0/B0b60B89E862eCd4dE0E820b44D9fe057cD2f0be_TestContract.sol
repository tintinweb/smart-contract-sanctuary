...
contract TestContract is ERC721Enumerable, Ownable {
    ...
    uint256 internal price = 5000000000000000; // 0.005 ETH as default
    ...
    function beginSale(uint256 _price) public onlyOwner {
        require(!saleHasBegun, "Sale has already begun.");
        require(_price > 0.001 ether, "Price must be set higher.");
        price = _price;
        saleHasBegun = true;
    }
    ...
    function mint(uint256 numToMint) public payable {
        require(saleHasBegun, "Cannot be done - sale has not yet begun.");
        require(
            numTokens < MAX_SUPPLY,
            "All Rabbits have been claimed from the burrow."
        );
        require(
            numToMint > 0 && numToMint <= 20,
            "You can mint no fewer than 1, and no more than 20 at a time."
        );
        require(
            numTokens + numToMint <= MAX_SUPPLY,
            "Cannot be done - this will mint more than the maximum supply."
        );
        require(
    ...
    function randomIndex() internal returns (uint256) {
        uint256 totalSize = MAX_SUPPLY - numTokens;
    ...
    function getSaleHasBegun() external view returns (bool) {
        return saleHasBegun;
    }
    ...
    /* Owner functions */
    function reserve(uint256 numReserved, address[] memory reservedAddresses)
        public
        onlyOwner
    {
        require(!saleHasBegun, "Sale has already begun.");
        require(numTokens <= MAX_SUPPLY, "All tokens have been minted.");
        require(
            numTokens + numReserved <= MAX_SUPPLY,
            "Cannot be done - this will mint more than the total supply."
        );

        for (uint256 i = 0; i < numReserved; i++) {
            _safeMint(reservedAddresses[i], i);
            numTokens = numTokens + 1;
        }
    }
    ...
    function withdrawFunds(uint256 amount) public onlyOwner {
        require(address(vault) != address(0), "Vault does not exist yet.");
        require(payable(vault).send(amount));
    }
