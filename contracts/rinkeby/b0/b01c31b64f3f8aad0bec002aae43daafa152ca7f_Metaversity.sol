...
// File: src/contracts/Metaversity.sol
...
contract Metaversity is ERC721Enumerable, Ownable {
    ...
    uint256 public constant maxTokensPublicMint = 6;
    ...
    constructor(string memory baseURI) ERC721("Metaversity", "MU") {
        setBaseURI(baseURI);
    }
    ...
    // Setting & Getting Sale Activated State

    function setPublicSaleActive(bool newState) public onlyOwner {
    ...
    function setAllowListActive(bool newState) public onlyOwner {
        allowListActive = newState;
    }

    function getAllowListActive() external view returns (bool) {
    ...
    function mintAllowList(uint256 numberOfTokens) public payable {
        uint256 ts = totalSupply();
        require(allowListActive, "Allow list is not active");
        require(numberOfTokens <= _allowList[msg.sender], "Exceeded max token purchase");
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            emit AssetMinted(ts + i, msg.sender);
    ...
    function mint(uint256 numberOfTokens) public payable {
        ...
        require(numberOfTokens <= maxTokensPublicMint, "Exceeded max token purchase");
        require(ts + numberOfTokens <= maxSupply, "Purchase would exceed max tokens");
        require(basePrice * numberOfTokens <= msg.value, "Insufficient ETH sent");
        for (uint256 i = 0; i < numberOfTokens; i++) {
            emit AssetMinted(ts + i, msg.sender);
