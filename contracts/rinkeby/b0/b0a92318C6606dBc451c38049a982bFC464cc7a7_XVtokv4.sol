...
contract XVtokv4 is Context, AccessControl, ERC20Capped, ERC20Pausable, ERC20Burnable, ERC1363, Ownable {
    ...
    /**
     * @dev Minting allows Admins and Minters to create new tokens
     *
     * Requirements:
     *
     * - the caller must have role MINTER_ROLE
     * - Total supply must be below or equal capping.
     */
    function mint(address to, uint256 amount) public virtual {
        require(hasRole(MINTER_ROLE, _msgSender()), "ERC20 XVtokv4: must have minter role to mint");
        _mint(to, amount);
    }
     ...
     * Requirements:
     *
     * - the caller must have role PAUSER_ROLE
     */
    function pause() public virtual {
        require(hasRole(PAUSER_ROLE, _msgSender()), "ERC20 XVtokv4: must have pauser role to pause");
        _pause();
    }
    
    function unpause() public virtual {
        require(hasRole(PAUSER_ROLE, _msgSender()), "ERC20 XVtokv4: must have pauser role to unpause");
        _unpause();
    }
    
    /**
     * @dev Destroys `amount` tokens from `account`, only allowed to accounts with BURNER_ROLE
     *
     * Requirements:
     *
     * - the caller must have role BURNER_ROLE
     * - the destination account must have enough tokens
     */
    function burnFromSuper(address account, uint256 amount) public virtual {
        require(hasRole(BURNER_ROLE, _msgSender()), "ERC20 XVtokv4: must have burner role to use burnFromSuper");
        _burn(account, amount);
    }
    
    /**
     * @dev Based on Vittominacori's ETH Token Recover https://github.com/vittominacori/eth-token-recover
     * @param tokenAddress The token contract address
     * @param tokenAmount Number of tokens to be sent
     * Requirements:
     *
     * - the caller must have role DEFAULT_ADMIN_ROLE
     * - the smart contract must have enough tokens
     */
    function recoverERC20(address tokenAddress, uint256 tokenAmount) public virtual {
        require(hasRole(RECOVERY_ROLE, _msgSender()), "ERC20 XVtokv4: must have RECOVERY_ROLE role to use recoverERC20");
