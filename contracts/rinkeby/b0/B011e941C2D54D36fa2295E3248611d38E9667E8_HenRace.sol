...
contract HenRace is Ownable, VRFConsumerBase {
  struct Racer {
    uint256 tokenId;
    address account;
    uint8 position;
  ...
  mapping(uint256 =>mapping(address=>bool)) registered;

  uint256 public currentRace;
  uint8 public maxRacersPerRace = 2;
  uint256 public entryCost = 10 ether;
  mapping(uint256=>Race) public races;
  mapping(address=>uint256[]) public racesByUser;
  mapping(bytes32=>uint256) requestIdToRace;
  ...
  event RacerAdded(uint256 indexed raceId, uint256 indexed tokenId, address indexed account, uint256 racerCount);
  event RaceStarted(uint256 indexed raceId);
  event RaceEnded(uint256 indexed raceId, uint8[] positions);

  constructor(address _eggs, address _foxHen, address _vrf, address _link, bytes32 _keyHash, uint256 _fee) VRFConsumerBase(_vrf, _link) {
    eggs = Eggs(_eggs);
    ...
    Race storage race = races[currentRace];
    race.state = 1;
  }

  function setMaxRacersPerRace (uint8 m) public onlyOwner {
    maxRacersPerRace = m;
  }

  function manualFulfillRandomness(bytes32 requestId, uint256 randomness) public onlyOwner {
    fulfillRandomness(requestId, randomness);
  }

  function racesCount(address user) public view returns (uint256) {
    return racesByUser[user].length;
  }

  function userRaces(address user) public view returns (Race[] memory) {
    uint256[] memory raceIds = racesByUser[user];
    Race[] memory rs = new Race[](raceIds.length);

    for (uint16 i = 0; i < rs.length; i++) {
      rs[i] = races[raceIds[i]];
    }
    return rs;
  }

  function raceParticipants(uint256 raceId) public view returns (RaceParticipant[] memory) {
    Race storage race = races[raceId];

    RaceParticipant[] memory participants = new RaceParticipant[](race.racers.length);

    for (uint8 i = 0; i < participants.length; i++) {
      participants[i] = RaceParticipant(race.racers[i], foxHen.tokenURI(race.racers[i].tokenId));
    }

    return participants;
  }

  function enterRace(uint256 tokenId) public {
    require(foxHen.ownerOf(tokenId) == msg.sender, "You must own that fox or hen");
    require(eggs.balanceOf(msg.sender) >= entryCost, "Must have enough eggs");
    require(eggs.allowance(msg.sender, address(this)) >= entryCost, "Must have approved eggs");

    Race storage race = races[currentRace];
    require(race.state == 1, "Incorrect race state");
    require(!registered[currentRace][msg.sender], "Already in race");
    eggs.transferFrom(msg.sender, address(this), entryCost);

    Racer memory racer = Racer(tokenId, msg.sender, 0);

    race.racers.push(racer);
    registered[currentRace][msg.sender] = true;
    racesByUser[msg.sender].push(currentRace);
    emit RacerAdded(currentRace, tokenId, msg.sender, race.racers.length);

    if (race.racers.length >= maxRacersPerRace) {
      race.state = 2;
      emit RaceStarted(currentRace);
      bytes32 requestId = requestRandomness(keyHash, fee);
      requestIdToRace[requestId] = currentRace;

      currentRace++;
      Race storage nextRace = races[currentRace];
      nextRace.state = 1;
    }
  }

  function getPositions(uint256 randomValue, uint256 count) public pure returns (uint8[] memory values) {
  ...
  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    uint256 raceId = requestIdToRace[requestId];
    Race storage race = races[raceId];

    uint8[] memory positions = getPositions(randomness, race.racers.length);

    uint256 prizePool = entryCost * race.racers.length;

    for (uint8 i = 0; i < positions.length; i++) {
      race.racers[i].position = positions[i];
      if (positions[i] < 3) {
        uint256 prize = prizePool * (4 - positions[i]) * 10 / 100;
        eggs.transfer(race.racers[i].account, prize);
      }
    }

    race.state = 2;
    emit RaceEnded(raceId, positions);
  }
}
