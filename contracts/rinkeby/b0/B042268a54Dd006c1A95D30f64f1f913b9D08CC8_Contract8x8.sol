...
contract Contract8x8 is ERC721Tradable, ReentrancyGuard {
    ...
    function checkMintableAndGetPrice(uint256 tokenId) public view returns (uint256) {
        // 0.001 min (1e15 wei)
        require(tokenId > 0 && tokenId < 18446744073709551615, "Token ID must be between 1 and 2^64-2");
        require(!_exists(tokenId), "token already minted");
        return Math.max( (64 ether) / tokenId, 1e15);
    }

    function checkMintableAndGetPriceBatch(uint256[] memory tokenIds) public view returns (uint256) {
        uint length = tokenIds.length;
        require(length <= 1000, "Maximum batch size of 1000.");
        uint256 price = 0;
        for (uint i=0; i<length; i++) {
            price = price + checkMintableAndGetPrice(tokenIds[i]);
        }
        return price;
    }
    ...
    function claimBatch(uint256[] memory tokenIds) public payable nonReentrant {
        uint length = tokenIds.length;
        uint256 price = checkMintableAndGetPriceBatch(tokenIds);
    ...
    function ownerClaim() internal nonReentrant onlyOwner {
        // To mark the start and end of the experiment
        _safeMint(owner(), 0);
