...
contract SuperBunnies is ERC721Tradable {
    ...
    uint256 public constant NFT_PRICE_LOVE = 69000000000000000; // 0.069 ETH for loving community
    ...
    function setTigerContractAddress(address contractAddress) public onlyOwner {
        tigerContract = contractAddress;
    }
    ...
    function levelUp(uint256 useTokenId1, uint256 useTokenId2, uint256 useTokenId3) public {
        require(tigerContract != address(0), "Tiger contract address need be set");
        ...
        Tiger tiger = Tiger(tigerContract);
        bool result = tiger.composeTiger(from);
        require(result, "Tiger compose failed");
    }
}

interface Tiger {
    function composeTiger(address owner) external returns (bool);
}
