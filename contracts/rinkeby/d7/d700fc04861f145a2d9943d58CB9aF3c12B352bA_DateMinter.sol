...
contract DateMinter is Ownable {
  ...
  */
  function getPrice() public view returns (uint256) {
    ...
    if (curTokenCount >= 3000) {
        return 1000000000000000000; // 3000 - 3649 1 ETH
    } else if (curTokenCount >= 2500) {
        return 640000000000000000; // 2500 - 2999 0.64 ETH
    } else if (curTokenCount >= 2000) {
        return 320000000000000000; // 2000  - 2499 0.32 ETH
    } else if (curTokenCount >= 1500) {
        return 160000000000000000; // 1500 - 1999 0.16 ETH
    } else if (curTokenCount >= 1000) {
        return 80000000000000000; // 1000 - 1499 0.08 ETH
    } else if (curTokenCount >= 500) {
        return 40000000000000000; // 500 - 999 0.04 ETH
    } else {
  ...
  */
  function reserveTokens(int256[] memory _timestamps) external onlyOwner() {
    ...
    for (uint i = 0; i < _timestamps.length; i++) {
      ...
      timestampToTokenId[dateTimestamp] = tokenId;
