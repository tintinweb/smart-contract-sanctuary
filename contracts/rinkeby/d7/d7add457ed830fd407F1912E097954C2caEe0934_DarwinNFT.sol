...
contract DarwinNFT is Ownable, ERC721, ERC721Enumerable, VRFConsumerBase, ReentrancyGuard {
    using SafeMath for uint256;
    
    event DarwinNFTRandomnessRequest(uint timestamp);
    event DarwinNFTRandomnessFulfil(uint timestamp, bytes32 requestId, uint256 seed);
    event DarwinNFTChainlinkError(uint timestamp, bytes32 requestId);
    event DarwinNFTReveal(uint timestamp);
    event DarwinManualSetSeed(uint timestamp);
    event DarwinWhitelist(address adress);
    ...
    /**
     * @dev ensure collector pays for mint token and message sender is not a contract
    ...
    function addAllowlist(address[] memory allowlist) public onlyOwner {
        for(uint i = 0; i < allowlist.length; i+=1) {
            ...
            emit DarwinWhitelist(allowlist[i]);
        }
    }
    ...
    function airdrop(address[] memory _to, uint256 amount) public onlyOwner {
        ...
        for (uint i = 0; i < _to.length; i+=1) {
            mintDarwin(_to[i], amount, true); // mint for marketing & influencer
        }
    }
    ...
    /**
     * @dev get sale mode 1 - Private sale/2 - Public sale
     */
    ...
    function getPrice() public view returns(uint256) {
        ...
        if (_mode == 3) return _publicSalePrice;
        return 0;
    }

    /**
     * @dev get current token available by sale mode
     */
    ...
    /**
     * @dev get current token minted by sale mode
     */
    ...
    function frozenMetadata() public onlyOwner {
    ...
    function mintNFT(uint256 amount) public payable nonReentrant mintable(amount) returns (bool) {
        ...
        return mintDarwin(_msgSender(), amount, false);
    }
    ...
    function requestChainlinkVRF() public onlyOwner {
        ...
        _requestedVRF = true;
        emit DarwinNFTRandomnessRequest(block.timestamp);
    }
    ...
    function setSeed(uint randomNumber) public onlyOwner {
        _requestedVRF = true;
        seed = randomNumber;
        emit DarwinManualSetSeed(block.timestamp);
    }
     ...
     */
    function mintDarwin( address collector, uint256 amount, bool isAirdrop) internal returns (bool) {
        // private sale
        if(getSaleMode() == 1) {
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override {
        if (randomNumber > 0) {
            seed = randomNumber;
            emit DarwinNFTRandomnessFulfil(block.timestamp, requestId, seed);
        }
        else {
            seed = 1;
            emit DarwinNFTChainlinkError(block.timestamp, requestId);
        } 
    }
