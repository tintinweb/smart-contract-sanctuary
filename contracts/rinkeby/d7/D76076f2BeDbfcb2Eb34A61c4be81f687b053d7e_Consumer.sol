...
contract Consumer is ChainlinkClient, ConfirmedOwner {
    ...
    uint256 public sum;
    bytes32 public highestBalance;
    bytes32 public lowestBalance;

    address public immutable oracle;
    bytes32 public immutable sumJobId;
    bytes32 public immutable balanceJobId;
    uint256 public constant fee = 10**18;

    event RequestSumFulfilled(bytes32 indexed requestId, uint256 indexed sum);
    event RequestHighestBalanceFulfilled(
        bytes32 indexed requestId,
        bytes32 indexed highestBalance
    );
    event RequestLowestBalanceFulfilled(
        bytes32 indexed requestId,
        bytes32 indexed lowestBalance
    );

    constructor(
        address _oracle,
        string memory _sumJobId,
        string memory _balanceJobId
    ) ConfirmedOwner(msg.sender) {
        setPublicChainlinkToken();
        oracle = _oracle;
        sumJobId = stringToBytes32(_sumJobId);
        balanceJobId = stringToBytes32(_balanceJobId);
    }

    function requestSum() public onlyOwner {
        Chainlink.Request memory req = buildChainlinkRequest(
            sumJobId,
            address(this),
            this.fulfillSum.selector
        );
        req.add("endpoint", "sum");
        sendChainlinkRequestTo(oracle, req, fee);
    }

    function requestHighestBalance() public onlyOwner {
        Chainlink.Request memory req = buildChainlinkRequest(
            balanceJobId,
            address(this),
            this.fulfillHighestBalance.selector
        );
        req.add("endpoint", "balance");
        req.add("option", "highest");
        sendChainlinkRequestTo(oracle, req, fee);
    }

    function requestLowestBalance() public onlyOwner {
        Chainlink.Request memory req = buildChainlinkRequest(
            balanceJobId,
            address(this),
            this.fulfillLowestBalance.selector
        );
        req.add("endpoint", "balance");
        req.add("option", "lowest");
        sendChainlinkRequestTo(oracle, req, fee);
    }

    function fulfillSum(bytes32 _requestId, uint256 _sum)
        public
        recordChainlinkFulfillment(_requestId)
    {
        emit RequestSumFulfilled(_requestId, _sum);
        sum = _sum;
    }

    function fulfillHighestBalance(bytes32 _requestId, bytes32 _highestBalance)
        public
        recordChainlinkFulfillment(_requestId)
    {
        emit RequestHighestBalanceFulfilled(_requestId, _highestBalance);
        highestBalance = _highestBalance;
    }

    function fulfillLowestBalance(bytes32 _requestId, bytes32 _lowestBalance)
        public
        recordChainlinkFulfillment(_requestId)
    {
        emit RequestLowestBalanceFulfilled(_requestId, _lowestBalance);
