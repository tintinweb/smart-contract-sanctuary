...
contract GalacticApesHero is ERC721, Ownable, ReentrancyGuard {
  ...
  mapping (uint256 => uint256) private ta;
  mapping (uint256 => uint256) private tl;
  
  //mapping (uint256 => uint8) private participation;
  uint256 public Participated;
  uint256 public TotalGameParticipant;
  ...
  * Note: `cost` is in wei. 
  */
  constructor(string memory tokenName, string memory symbol, string memory uri, string memory uri2, string memory curi, uint256 cost, uint256 supply) ERC721(tokenName, symbol) Ownable() {
    _mintCost = cost;
    _maxSupply = supply;
    ...
    _curi = curi;
    Participated=0;
    TotalGameParticipant=0;
  }

  function PlaySaveTheGalaxy(uint256 tokenId) public virtual returns (string memory) {
    //require(participation[tokenId] == 0, "Only one game per one SaveTheGalaxy");
    require(Participated < TotalGameParticipant, "More than 300 Ape Heroes are gathered");
    address owner = ERC721.ownerOf(tokenId);
    require(_msgSender() == owner, "ERC721: approve caller is not owner nor approved for all");

    if (reputation[tokenId] >= 0){
        reputation[tokenId]++;
    }
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    string memory baseURI = _baseURI();
    string memory baseHeroURI = _baseHeroURI();

    if (reputation[tokenId] >= 2){
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseHeroURI, tokenId.toString())) : "";
        }
    else{
        //return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenId.toString())) : "";   
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenId.toString())) : "";   
  ...
  function StartSaveTheGalaxy(uint256 count) public onlyOwner{
    Participated=0;
    TotalGameParticipant=count;
  }

  function ClaimApe(uint256 tokenId)
  public
  nonReentrant{
    require(_isPublicMintEnabled, "Mint disabled");
    require(owner() == msg.sender && reputation[tokenId] == 2, "Need exact '2' reputation to claim GalacticApe Hero");
    _mint(msg.sender);
    reputation[tokenId]++;
    }
  ...
  * Can only be called by the current owner.
  */
  function giveEventSupply(address[] calldata recipients)
  public
  payable
  onlyOwner{
  ...
  */
  ...
  function _baseHeroURI() internal view returns (string memory) {
    return _uri2;
  }
