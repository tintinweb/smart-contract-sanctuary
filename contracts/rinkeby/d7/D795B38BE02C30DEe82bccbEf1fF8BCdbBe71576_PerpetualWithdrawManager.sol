...
contract PerpetualWithdrawManager is PerpetualRebalanceFunctions, IFunctionList, IPerpetualWithdrawManager {
    using ABDKMath64x64 for int128;
    using ConverterDec18 for int128;
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;
     ...
     */
    function withdraw(bytes32 _iPerpetualId, int128 _fAmount) external override nonReentrant updateFundingAndPrices(perpetualPoolIds[_iPerpetualId]) {
        PerpetualData storage perpetual = _getPerpetual(_iPerpetualId);
        address traderAddr = msgSender();
        _validateInputData(perpetual, _fAmount, traderAddr);
    ...
    //  * @param   _iPerpetualId   The id of the perpetual in the liquidity pool
    //  */
    // function withdrawAll(bytes32 _iPerpetualId) external override nonReentrant updateFundingAndPrices(perpetualPoolIds[_iPerpetualId]) {
    //     PerpetualData storage perpetual = _getPerpetual(_iPerpetualId);
    //     address traderAddr = msgSender();
    ...
    //     _validateInputData(perpetual, fAmount, traderAddr);
    ...
    function _rebalanceAndWithdraw(
        ...
        emit TokensWithdrawn(_perpetual.id, _traderAddr, _fAmount, block.timestamp);
    }
    ...
    function _withdraw(
        ...
        require(_getTraderPosition(_perpetual, _traderAddr) == 0 || !ISpotOracle(_perpetual.oracleS2Addr).isMarketClosed(), "market is closed");
        _updateTraderMargin(_perpetual, _traderAddr, _fAmount.neg());
        require(_isInitialMarginSafe(_perpetual, _traderAddr), "margin is unsafe after withdrawal");
    ...
    function _validateInputData(
        ...
        MarginAccount storage account = marginAccounts[_perpetual.id][_traderAddr];
        //it will fail even without this check with error: if margin is unsafe after withdrawal
        require(account.fCashCC >= _fAmount, "amount isn't available");
    }

    function getFunctionList() external pure override returns (bytes4[] memory) {
        bytes4[] memory functionList = new bytes4[](2);
        functionList[0] = this.withdraw.selector;
        // functionList[1] = this.withdrawAll.selector;
        return functionList;
    }
}
