...
contract Woolf is IWoolf, ERC721Enumerable, Ownable, Pausable {
  ...
  // *** XXX
  uint16 public sheepStolen = 0;
  uint16 public wolfStolen = 0;
  uint16 public sheepMinted = 0;
  ...
  function mint(uint256 amount, bool stake) external payable whenNotPaused {
    ...
    totalWoolCost = mintCost(amount);
    require(wool.balanceOf(msg.sender) >= totalWoolCost, "Not enough Wool");
    if (totalWoolCost > 0) {
    ...
    // *** XXX
      if (barn.isSheep(minted)) {
        sheepMinted += 1;
      }

      if (recipient != msg.sender) {
        barn.isSheep(minted) ? sheepStolen += 1 : wolfStolen += 1;
      }
    // *** XXX
    }
  ...
  function mintCost(uint256 tokenId) public view returns (uint256) {
    if (minted + tokenId <= PAID_TOKENS) return 20000 ether * tokenId;
    if (minted + tokenId <= MAX_TOKENS * 2 / 5) return 20000 ether * tokenId;
    if (minted + tokenId <= MAX_TOKENS * 4 / 5) return 40000 ether * tokenId;
    return 80000 ether * tokenId;
  }
