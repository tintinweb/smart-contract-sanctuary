...
contract EchoBase is AccessControl, IERC20, VolumeLimiter {
    ...
    // Charity Address (can be any valid address - changeable. Will not be the address below at launch)
    address payable public charityAddress;
    address payable public constant multisigAddress = payable(0x75311b096Bce494E7441c2A69AdAf33e8905C12B);
    // mainnet multisig: 0x32cD2c588D61410bAABB55b005f2C0ae520f8Aa5
    ...
    // Set the maximum amount that can be liquidated in the uniswap pool in a single
    // call to transfer
    uint256 private _maxTxLiquidationAmount = 12 * 10**9 * 10**18;
    ...
    // No need to call preValidateTransaction once the initial trading restrictions are lifted
    ...
    // RFI's token transfer function - with an addtional takeFee parameter based on if called from
    ...
    // this contract CANNOT blacklist anybody!
    ...
    // Add the liquidity (donation fee) from each transaction to the contract's balance itself
    // to then be sent to the charity address upon next transfer
    ...
    function setSwapEnabled(bool enabled) external {
        require(hasRole(COMMUNITY_DECISION_ROLE, _msgSender())); // , "Caller is not admin");
        swapEnabled = enabled;
    }
    ...
    function setRedistFeePercent(uint256 redistFee) external {
        require(hasRole(COMMUNITY_DECISION_ROLE, _msgSender()));//, "[0]"); // Caller is not decision maker

        uint256 newChangeDate = _redistFeeLastChangeDate.add(percentChangeTimespan);
        require(block.timestamp >= newChangeDate);//, '[0]'); // Attempting to change value before enough time elapsed
        require(redistFee <= 5 && redistFee >= 0);//, '[T0]'); // percentage outside bounds
        require(redistFee < _donationFee);//, '[T1]'); // Cannot redistribute more than is donated
        require(_redistFeeModCounter <= 5);//, '[T2]'); // can no longer change the percentage
    ...
    function setBurnFeePercent(uint256 burnFee) external {
        ...
        require(block.timestamp >= newChangeDate);//, '[T0]'); // Attempting to change value before enough time elapsed
        require(burnFee <= 3 && burnFee >= 0);//, '[T1]'); // percentage outside bounds
        require(burnFee < _donationFee);//, '[T2]'); // Cannot burn more than is donated
        require(_burnFeeModCounter <= 5);//, '[T3]'); // can no longer change the percentage
    ...
    function setDonationFeePercent(uint256 donationFee) external {
        ...
        require(block.timestamp >= newChangeDate); //, '[T0]' Attempting to change value before enough time elapsed
        require(donationFee <= 25 && donationFee >= 0);//, '[T1]'); // percentage outside bounds
        require(_donationFeeModCounter <= 5);//, '[T2]'); // can no longer change the percentage
    ...
    // update the minimum amount of tokens that the token contract must have
...
abstract contract VolumeLimiter is AccessControl {
    ...
    function preValidateTransaction(address from, address to, address uniswapPool, uint256 amount) internal virtual {
        ...
        if (block.timestamp < launchStartTime) {
            ...
            require(from != uniswapPool, 'NS');
            require(to != uniswapPool, 'NR');
        }
