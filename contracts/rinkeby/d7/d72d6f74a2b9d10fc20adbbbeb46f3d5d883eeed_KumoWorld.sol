...
contract KumoWorld is ERC721Enumerable, Ownable {
    constructor() ERC721("Kumo x World", "KUMO") {}

    uint256 public PRICE = 0.0001 ether;
    uint256 public constant KUMO_EXPEDITION_SUPPLY = 50;
    uint256 public constant KUMO_GIFT_SUPPLY = 50;
    uint256 public constant KUMO_PUBLIC_SUPPLY = 8788;
    uint256 public constant MAX_SUPPLY = KUMO_GIFT_SUPPLY + KUMO_PUBLIC_SUPPLY;
    ...
    //WARNING WARNING WARNING SAMPLE MUST DELETE THIS PLEASE DELETE THIS
    function mintAll(uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = KUMO_EXPEDITION_SUPPLY +
                KUMO_GIFT_SUPPLY +
                totalPublicSupply;
            totalPublicSupply += 1;
    ...
    //WARNING WARNING WARNING SAMPLE MUST DELETE THIS PLEASE DELETE THIS
    function giftAll(uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = KUMO_EXPEDITION_SUPPLY + totalGiftSupply;
            totalGiftSupply += 1;
            _safeMint(msg.sender, tokenId);
        }
    }
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(
            totalPublicSupply < KUMO_PUBLIC_SUPPLY,
            "Public supply is sold out"
        );
        require(
            numberOfTokens <= PURCHASE_LIMIT,
            "Cannot exceed PURCHASE_LIMIT"
        );
        require(
            _purchaseList[msg.sender] < ADDRESS_PURCHASE_LIMIT,
            "Cannot exceed ADDRESS_PURCHASE_LIMIT"
        );
        require(
    ...
    function purchaseFromWhitelist(uint256 numberOfTokens) external payable {
        ...
        require(_whitelist[msg.sender] == true, "Address is not in Whitelist");
        require(totalSupply() < MAX_SUPPLY, "Tokens are sold out");
        require(
            totalGiftSupply < KUMO_PUBLIC_SUPPLY,
            "Public supply is sold out"
        );
        require(
            _whitelistPurchased[msg.sender] < WHITELIST_PURCHASE_LIMIT,
            "Cannot exceed WHITELIST_PURCHASE_LIMIT"
        );
        require(
            numberOfTokens <= PURCHASE_LIMIT,
    ...
    function gift(address[] calldata giftAddress) external onlyOwner {
        require(totalSupply() < MAX_SUPPLY, "Tokens are sold out");
        require(totalGiftSupply < KUMO_GIFT_SUPPLY, "Gift supply is sold out");

        for (uint256 i = 0; i < giftAddress.length; i++) {
            uint256 tokenId = KUMO_EXPEDITION_SUPPLY + totalGiftSupply;
            totalGiftSupply += 1;
            _safeMint(giftAddress[i], tokenId);
        }
    }
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < addressList.length; i++) {
            require(addressList[i] != address(0), "Address can't be null");
            _whitelist[addressList[i]] = false;
        }
    }
    ...
    function setWhiteListSaleStatus(bool active) external onlyOwner {
        isWhitelistSaleActive = active;
    }
