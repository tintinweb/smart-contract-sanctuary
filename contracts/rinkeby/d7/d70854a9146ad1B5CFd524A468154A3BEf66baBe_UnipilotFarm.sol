...
contract UnipilotFarm is IUnipilotFarm, IERC721Receiver {
    ...
    uint256 internal constant DEFFAULT_REWARD_MULTIPLIER = 1e18;
    ...
    function insertPool(address poolAddress) internal {
        ...
        poolInfo[poolAddress] = PoolInfo({
            ...
            totalSupply: 0,
            rewardMultiplier: DEFFAULT_REWARD_MULTIPLIER,
        ...
        returns (uint256)
    {
        ...
        return poolInfo[poolAddress].rewardMultiplier;
    }
        ...
        returns (bool)
    {
        ...
        poolInfo[poolAddress].totalSupply = poolInfo[poolAddress].totalSupply.add(
            liquidity
        );
        ...
        emit Deposit(
            ...
            poolInfo[poolAddress].totalSupply,
            poolInfo[poolAddress].globalReward,
            poolInfo[poolAddress].rewardMultiplier,
            poolInfo[poolAddress].rewardPerBlock
        );
    ...
    function getGlobalReward(
        ...
        globalReward = FullMath
            .mulDiv(blockDifference.mul(temp), 1e18, poolInfo[poolAddress].totalSupply)
            .add(_globalReward);
    }
    ...
    {
        address poolAddress = userInfo[tokenId].poolAddress;
        uint256 blockDifference = (block.number).sub(
            poolInfo[poolAddress].lastRewardBlock
        );
    ...
    function emergencyNFTWithdraw(uint256 tokenId) external {
        require(userInfo[tokenId].userAddress == msg.sender, "NOO");
        IERC721(UNIPILOT).safeTransferFrom(
            address(this),
            userInfo[tokenId].userAddress,
            tokenId
        );

        uint256 index = callIndex(userInfo[tokenId].poolAddress, tokenId);
        delete userToPoolToTokenIds[msg.sender][userInfo[tokenId].poolAddress][index];
        delete userInfo[tokenId];

        if (poolInfo[userInfo[tokenId].poolAddress].totalSupply == 0) {
            delete poolInfo[userInfo[tokenId].poolAddress];
            insertPool(userInfo[tokenId].poolAddress);
        }
    }
    ...
    function withdrawNFT(uint256 tokenId) external override returns (bool isLeft) {
        ...
        poolInfo[poolAddress].totalSupply = poolInfo[poolAddress].totalSupply.sub(
            userInfo[tokenId].liquidity
        );
        ...
        if (poolInfo[poolAddress].totalSupply == 0) {
            delete poolInfo[poolAddress];
            insertPool(poolAddress);
        }
    ...
    function withdrawReward(uint256 tokenId) public override returns (bool isSent) {
        address poolAddress = userInfo[tokenId].poolAddress;
        require(userInfo[tokenId].userAddress == msg.sender, "NO");
        ...
        IERC20(PILOT_TOKEN).safeTransfer(userInfo[tokenId].userAddress, pilotReward);
    ...
    /// @notice Updating alt token state
    /// @param poolAddress pool address for alt token
    /// @param tokenId tokenID for alt token active
    ...
    function toggleActiveAlt(address poolAddress) external onlyGovernance returns (bool) {
        bool old = poolInfo[poolAddress].isAltActive;
        poolInfo[poolAddress].isAltActive = !poolInfo[poolAddress].isAltActive;
        emit UpdateActiveAlt(poolAddress, old, poolInfo[poolAddress].isAltActive);
        return poolInfo[poolAddress].isAltActive;
    }

    function updateAltToken(address poolAddress, address altTokenAddress)
        external
        override
        onlyGovernance
        returns (address)
    {
        address old = poolInfo[poolAddress].altToken = altTokenAddress;

        poolInfo[poolAddress].altToken = altTokenAddress;
        emit UpdateAltToken(poolAddress, old, poolInfo[poolAddress].altToken);
        return poolInfo[poolAddress].altToken;
    }
...
interface IUnipilotFarm {
    ...
    event UpdateActiveAlt(address poolAddress, bool old, bool updated);
