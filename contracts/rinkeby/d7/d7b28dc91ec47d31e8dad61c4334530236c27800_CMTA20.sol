...
//Roles utilities contracts for pausable access rights
...
contract CMTA20 is ERC20, Ownable, Pausable {
  using SafeMath for uint256;
  ...
  */
  function issue(uint256 _value) public onlyOwner {
    ...
    //emit LogIssued(_value); //Removed to go faster
  }
  ...
  */
  function redeem(uint256 _value) public onlyOwner {
    ...
    //emit LogRedeemed(_value); //Removed to go faster
  }
  ...
  * @param replacement - replacement address
  ...
  function reassign(address original, address replacement) external onlyOwner whenNotPaused {
    ...
    //emit LogReassigned(original, replacement, originalBalance); //Removed to go faster
  }
  ...
  */
  function destroy(address[] calldata shareholders) external onlyOwner {
    ...
    //emit LogDestroyed(shareholders); //Removed to go faster
  }
}