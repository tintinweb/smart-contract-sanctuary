...
contract FancyNftStaking is IERC721Receiver, RecoverableErc20ByOwner {
    ...
    event Unstaked(address indexed from, address indexed to, uint256 tokenId);
    ...
    function stake(uint256[] memory tokenIds) external {
        _stakeFrom(_msgSender(), tokenIds);
    }

    function _stakeFrom(address owner, uint256[] memory tokenIds) private {
        uint128 rate;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            _ownerTokens[owner].add(tokenIds[i]);
            rate += uint128(
                1 ether + IFancyNftHeroesS1(nft).extraRate(tokenIds[i])
            );
            emit Staked(owner, tokenIds[i]);
        }

        Staker storage staker = _updateStateAndStaker(owner);

        state.totalStaked += rate;
        staker.amount += rate;
    }

    // Unstake
    function unstake(uint256[] memory tokenIds) external {
        _unstake(_msgSender(), _msgSender(), tokenIds);
    }
    ...
    function _unstake(
        ...
        require(
            _ownerTokens[from].length() >= tokenIds.length,
            "NOT_ENOUGH_STAKED"
        );
        uint128 rate;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                _ownerTokens[from].contains(tokenIds[i]),
                "This tokenId is not yours"
            );
            IERC721Enumerable(nft).transferFrom(address(this), to, tokenIds[i]);
            _ownerTokens[from].remove(tokenIds[i]);
            rate += uint128(
                1 ether + IFancyNftHeroesS1(nft).extraRate(tokenIds[i])
            );
            emit Unstaked(from, to, tokenIds[i]);
        }
        ...
        returns (uint256)
    {
        require(
            tokenAddress != token,
            "FancyNftStaking: cannot withdraw native token"
        );
