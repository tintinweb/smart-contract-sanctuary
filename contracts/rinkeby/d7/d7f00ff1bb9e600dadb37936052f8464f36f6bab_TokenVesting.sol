...
contract TokenVesting is Ownable {
  ...
  function release(address token) public {
    ...
    ERC20Basic(token).safeTransfer(beneficiary, unreleased);
  ...
  function revoke(address token) public onlyOwner {
    ...
    uint256 balance =  ERC20Basic(token).balanceOf(address(this));
    ...
    ERC20Basic(token).safeTransfer(owner, refund);
  ...
  function vestedAmount(address token) public view returns (uint256) {
    uint256 currentBalance = ERC20Basic(token).balanceOf(address(this));
