...
contract BigRichCard is ERC1155Tradable {
    ...
    constructor (address _proxyRegisteryAddress, string memory baseUrl)
        ERC1155Tradable(
            "Big Rich Card",
            "BRC",
            string(abi.encodePacked(baseUrl, "/api/v1/nft/metadata/{id}")),
    ...
    function contractURI() public view returns (string memory) {
        return string(
            abi.encodePacked(_baseUrl, "/api/v1/nft/contract/big-rich-erc1155")
        );
    }

    function setBaseUrl(string memory _newUrl) public onlyOwner {
        _baseUrl = _newUrl;
        setURI(string(abi.encodePacked(_baseUrl, "/api/v1/nft/metadata/cat/{id}")));
    }
}
...
import "./ERC1155Tradable.sol";
import "./BigRichCard.sol";

contract BigRichCardFactory is FactoryERC1155, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        address _nftCardAddress,
        string memory _baseMetadataURI
    ) {
        proxyRegistryAddress = _proxyRegistryAddress;
        nftCardAddress = _nftCardAddress;
    ...
    function setBaseUrl(string memory _newBaseURI) public onlyOwner {
        ...
        BigRichCard(nftCardAddress).setBaseUrl(_newBaseURI);
    }
    ...
    function _mint(
        ...
        require(
            ...
            "BigRichCardFactory#_mint: CANNOT_MINT_MORE"
        );

        require(_isOwnerOrProxy(_msgSender()), "Caller cannot mint item");
        // BigRichCard are not premined, so we need to create or mint them.
    ...
    returns (uint256)
    {
        ...
        ERC1155Tradable nft = ERC1155Tradable(nftCardAddress);
    ...
    function safeTransferFrom(address _from, address _to, uint256 _optionId, uint256 _amount, bytes calldata _data) override public {
        require(_from == owner(), "Only allow to transfer from owner ");
        require(_isOwnerOrProxy(_msgSender()), "Caller is not Owner or Proxy");

        _createOrMint(nftCardAddress, _to, _optionId, _amount, _data);
    }
}
