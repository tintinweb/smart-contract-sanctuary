...
contract Bags is ERC20, Ownable {
    ...
    constructor(uint256 _maxBags) ERC20("BAGs with Something v0.0.1", "BAG")  {
        MAX_BAGS = _maxBags;
    }
...
contract BagSale is Ownable {
    ...
    //Price =  A` * B`^N,
    //Price =  0,050*(1,0028^N) where N - buy round
    //Price ~ A * B^N *1e18/1e4
    // B is expressed as last_price
    uint256 public A = 10028; //multiplied on 1e4
    uint256 public percentWant = 3;

    uint256 public last_price;
    uint256  public weiRaised;
    mapping(uint8 => itemPrice) salesBook; 
    event Sold(address buyer, uint256 _amount);
    ...
    function buyBag(uint256 _wantPrice) external payable {
        //TODO  Check math when _wantPrice < 3%
        uint256 _price = _nextPrice();
        uint256  _diff;
        ...
        emit Sold(msg.sender, mintAmount);
        last_price = _price;
    }
    ...
    function setPriceParams(uint256 _A, uint256 _percentWant) external onlyOwner {
        A = _A;
        percentWant = _percentWant;
    }
    ...
    function _nextPrice() internal view returns (uint256) {
        ...
        uint256 _price = last_price.mul(A).div(1e4);
        return _price;
    }
}