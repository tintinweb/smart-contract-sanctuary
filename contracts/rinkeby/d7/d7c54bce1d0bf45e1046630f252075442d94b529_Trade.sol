...
contract Trade {
    ...
    function getFees(uint paymentAmt, BuyingAssetType buyingAssetType, address buyingAssetAddress, uint tokenId) internal view returns(Fee memory){
        ...
        if(buyingAssetType == BuyingAssetType.ERC721) {
            royaltyPermille = ((IERC721(buyingAssetAddress).getFee(tokenId)));
            tokenCreator = ((IERC721(buyingAssetAddress).getOwner(tokenId)));
        }
        if(buyingAssetType == BuyingAssetType.ERC1155)  {
            royaltyPermille = ((IERC1155(buyingAssetAddress).getFee(tokenId)));
    ...
    function tradeAsset(address paymentAssetAddress, address assetOwner, address buyer, BuyingAssetType buyingAssetType, address buyingAssetAddress, uint tokenId, uint8 buyingAssetQty, Fee memory fee, bytes memory data) internal virtual {
        require(fee.royaltyFee != 0 && fee.platformFee != 0,"transaction failed" );
        if(buyingAssetType == BuyingAssetType.ERC721) {
