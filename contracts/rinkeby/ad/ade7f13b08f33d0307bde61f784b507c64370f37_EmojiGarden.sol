...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseURI).length > 0 ? string(abi.encodePacked("Emoji Garden Token")) : "";
    }
...
contract EmojiGarden is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function row(uint256 tokenId, uint256 rowNum) public view returns (string memory) { 
        if(tokenId > 4420 && tokenId < 4445) { }
        else { 
            require(tokenId > 0 && tokenId <= totalSupply(), "Token ID invalid");
        }
        require(rowNum > 0 && rowNum < 4, "Row invalid");
        // require(tokenId <= totalSupply(), "Token does not exist"); 
        
        uint256 rand; 
        
        string[3] memory rowContent; 
        
        string memory selfAddress = string(abi.encodePacked(address(this))); 
        
        rand = uint256(keccak256(abi.encodePacked(tokenId, selfAddress, rowNum))) % 100; // pseudo-random  
        
        rowContent[0] = string(abi.encodePacked(pluck(rand))); 
        
        if(tokenId > 4420 && rowNum==2) {
            ...
            rowContent[1] = hons[tokenId - 4421]; 
        }
        else { 
            rand = uint256(keccak256(abi.encodePacked(selfAddress, rowNum, tokenId))) % 100; 
            rowContent[1] = string(abi.encodePacked(pluck(rand))); 
        }
        
        if(tokenId == 4425) { 
            rowContent[2] = unicode"🦆"; 
        }
        else { 
            rand = uint256(keccak256(abi.encodePacked(rowNum, tokenId, selfAddress))) % 100; 
            rowContent[2] = string(abi.encodePacked(pluck(rand))); 
        }
        
        return string(abi.encodePacked(rowContent[0], rowContent[1], rowContent[2])); 
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[1] = row(tokenId, 1); 
        
        parts[2] = '</text><text x="50%" y="50%" class="e" dominant-baseline="middle" text-anchor="middle">'; 
        
        parts[3] = row(tokenId, 2); 
        
        parts[4] = '</text><text x="50%" y="75%" font-size="62" letter-spacing=".25em" dominant-baseline="middle" text-anchor="middle">'; 
        
        parts[5] = row(tokenId, 3); 
    ...
    function ownerClaim() public nonReentrant onlyOwner {
        require(4420 >= totalSupply(), "Public mint is still running");
    ...
    function mintTwo() public payable {
        ...
        require( msg.value == 0.05 ether, "Price is 0.05 ETH, please check price and try again" );
        _safeMint( msg.sender, supply + 1 );
    ...
    function mintThree() public payable {
        ...
        require( msg.value == 0.08 ether, "Price is 0.08 ETH, please check price and try again" );
        _safeMint( msg.sender, supply + 1 );
        _safeMint( msg.sender, supply + 2 );
