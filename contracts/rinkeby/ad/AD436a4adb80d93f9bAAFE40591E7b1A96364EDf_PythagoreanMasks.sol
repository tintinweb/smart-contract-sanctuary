...
/*
    ...
    Pythagorean Masks are the first of the creations of the @_n_Collective (https://twitter.com/_n_collective).
...
contract PythagoreanMasks is ERC721, Ownable, ReentrancyGuard, ERC721Holder {
    ...
    constructor(uint256 _endMintingPeriodDateAndTime) ERC721("Pythagorean Masks", "PythagoreanMasks") {
        ...
        nextVestingPeriodDataAndTime = block.timestamp + (90 * 24 * 60 * 60);
    }
    ...
    function claimVestedTeamTokens(uint256[] memory tokenIds) public onlyOwner onlyWhenFinishInit {
        ...
        // Vesting period every 3 months
        nextVestingPeriodDataAndTime = nextVestingPeriodDataAndTime + (90 * 24 * 60 * 60);
        for (uint256 i; i < tokenIds.length && i < 264; i++) {
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        toString(tokenId),
                        '", "description": "Numbers are the basis of the entire universe, the base layer of perceived reality. The rest is but a mere expression of those. Numbers are all around us, have always been, will always be. Welcome to the n Collective.", "image": "data:image/svg+xml;base64,',
