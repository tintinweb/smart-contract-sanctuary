...
contract Bear is Ownable, ERC721URIStorage {
    ...
    function getPreSaleNFTAmount() public view  returns (uint256){
        return PRE_SALE;
    }
    ...
    function claim_2( bytes32[] memory tokenURI) external payable returns (uint256[] memory){
        require(MAX_NFTS > 0);
    ...
    function claim_5( bytes32[] memory tokenURI) external payable returns (uint256[] memory){
        require(MAX_NFTS > 0);
    ...
    function claim_10(bytes32[] memory tokenURI) external payable returns (uint256[] memory){
        require(MAX_NFTS > 0);
    ...
    function claim_20(bytes32[] memory tokenURI) external payable returns (uint256[] memory){
        require(MAX_NFTS > 0);
    ...
    function claim_10_GiveAway(bytes32[] memory tokenURI) onlyOwner external returns (uint256[] memory){
        require(MAX_NFTS > 0);
        uint256 rarity = 2; // This should generated based on the probablity map 
        uint256[] memory id = mintBatch(tokenURI,rarity,10);
        return id;
    ...
    function mintBatch( bytes32[] memory tokenURI,uint256 rarity,uint256 numberOfTokens) private returns (uint256[] memory){
        ...
        for(uint temp = 0; temp < numberOfTokens; temp++) {
             string memory URI = bytes32ToString(tokenURI[temp]);
