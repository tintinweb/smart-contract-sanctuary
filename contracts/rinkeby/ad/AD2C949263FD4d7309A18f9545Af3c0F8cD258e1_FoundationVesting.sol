...
contract FoundationVesting is Context, Ownable {
    ...
    uint256 private percentDenominator = 1000;
    uint256 private oneYear = 365 days;

    constructor(address _tokenAddress, uint256 _newStartTime) {
        snowToken = IERC20(_tokenAddress);
        startTime = _newStartTime;

        totalAmount = 40000e18; // 8% of total supply
        initialUnlock = 125; // 12.5% inital unlock
        unallocatedAmount = totalAmount;
        withdrawInterval = 365 days;
    }
    ...
    function addRecipients(
        ...
        for (uint256 i = 0; i < _newRecipients.length; i++) {
            ...
            recipients[_newRecipient] = VestingSchedule({
                totalAmount: _totalAmount,
                amountWithdrawn: _withdrawnAmount
            });
            unallocatedAmount = unallocatedAmount.sub(_totalAmount);
        }
        ...
        returns (uint256 _amountVested)
    {
        ...
        if (
            (_vestingSchedule.totalAmount == 0) || (block.timestamp < startTime)
        ) {
            return 0;
        }

        uint256 initialUnlockAmount =
            _vestingSchedule.totalAmount.mul(initialUnlock).div(
                percentDenominator
            );
        ...
        if (period < oneYear) {
            ...
            uint256 unlockRate =
                _vestingSchedule
                    ...
                    .div(oneYear);
            vestedAmount = unlockRate.mul(period).add(initialUnlockAmount);
        } else {
            // after one year, 25% unlock per year
            uint256 unlockRate =
                _vestingSchedule
                    .totalAmount
                    .mul(250)
                    .div(percentDenominator)
                    .div(oneYear);
            vestedAmount = unlockRate.mul(period.sub(oneYear)).add(
                _vestingSchedule.totalAmount.div(2) // 50% already unlocked
            );
        }
    ...
    function withdraw() external {
        ...
        require(snowToken.transfer(_msgSender(), _withdrawable));
