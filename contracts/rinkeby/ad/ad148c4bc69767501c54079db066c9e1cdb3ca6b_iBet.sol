...
contract iBet is Ownable , ReentrancyGuard {
   ...
   function bet(uint8 selected , uint256 betId) public payable {
      require(betId > 0 && betId <= currentBet );
      require(!bets[betId].playerInfo[msg.sender].betted && bets[betId].closeTime > block.timestamp );
    ...
    function claim(uint256 betId) external nonReentrant notContract {
        require(betId > 0 && betId <= currentBet );
        require(bets[betId].gameWinner > 0 && bets[betId].gameWinner==bets[betId].playerInfo[msg.sender].selected);
        require(!bets[betId].playerInfo[msg.sender].claimed );
         uint256 totalWin = 0;
         uint256 totalLost = 0;
    ...
    function setWinner(uint16 _winner , uint256 betId) public onlyOwner{
       ...
       require(betId > 0 && betId <= currentBet );
       require(bets[betId].closeTime < block.timestamp );
     ...
     function setCloseTime(uint256 betId,uint256 _closeTime) public onlyOwner{
       ...
       require(betId > 0 && betId <= currentBet );
       require(bets[betId].gameWinner == 0 );
