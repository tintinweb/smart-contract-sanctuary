...
import "./token/extensions/ERC1155MintTokenControl.sol";
...
contract OriginalsHandDrawnLoops is ERC1155, ERC1155Burnable, ERC1155RandomMint, ERC1155MintTokenControl, Pausable, ReentrancyGuard, AdminControl, Ownable, IRemixOriginal {
  ...
  ) ERC1155("https://storageapi.fleek.co/apedao-bucket/hand-drawn-loops/")
    ERC1155RandomMint(tokenSupply)
    ERC1155MintTokenControl(_MINT_TOKEN_CONTRACT_ADDRESS, TokenRequirementType.NONE)
  {
  ...
  function mintWithTokens(uint256 _numGoldTokens, uint256 _basicGoldTokens) public payable whenNotPaused nonReentrant {
    ...
    if((mintTokenRequirement == TokenRequirementType.GOLD || mintTokenRequirement == TokenRequirementType.ANY) && _numGoldTokens > 0) {
      mintClubContract.burn(msg.sender, GOLD_TOKEN_ID, _numGoldTokens);
    }

    // If Basic token is required
    if((mintTokenRequirement == TokenRequirementType.BASIC || mintTokenRequirement == TokenRequirementType.ANY) && _basicGoldTokens > 0) {
  ...
  function canMint(uint256 _quantity) internal view {
    ...
    require(walletLimit == 0 || (numberOfTokensMintedByAddress[msg.sender] + _quantity <= walletLimit), "You have reached your wallet limit");
...
abstract contract ERC1155MintTokenControl is Context {
  ...
  constructor(
    address payable _MINT_TOKEN_CONTRACT_ADDRESS,
    TokenRequirementType _mintTokenRequirement
  ) {
  ...
  /**
  * @dev Modifier to make a function callable only when mint tokens are required
  *
  * Requirements:
  *
  * - Mint token requirment is set to None
  */
  modifier whenMintTokensRequired() {
    require(mintTokenRequirement == TokenRequirementType.NONE, "Mint tokens are required to mint.");
    _;
  }

  /**
  * @dev Modifier to make a function callable only when mint tokens are not required
  *
  * Requirements:
  *
  * - Min token requirement is set set to GOLD, BASIC or ANY
  */
  modifier whenMintTokensNotRequired() {
    require(mintTokenRequirement != TokenRequirementType.NONE, "Mint tokens are not required to mint.");
    _;
  }

  /**
  * @dev Function to set the Mint Token requirement type
  */
  function _setMintTokenRequirement(TokenRequirementType _mintTokenRequirement) internal {
    mintTokenRequirement = _mintTokenRequirement;
  }
}
