...
contract NiftyStonks is ERC721, ERC721Enumerable, Pausable, Ownable, ERC721Burnable {
  using Counters for Counters.Counter;

  Counters.Counter private _tokenIdCounter;

  // Where funds should be sent to
  address payable public FUNDS_TO;

  uint256 public MINT_PRICE = 100000000000000000; // 0.1 ETH

  // Maximum mints per transaction
  uint256 public MAX_MINTS_PER_TX = 1;

  // Maximum items per wallet
  uint256 public MAX_ITEMS_PER_WALLET = 1;

  // Maximum supply of the NFT
  uint256 public constant MAX_SUPPLY = 666;

  // Number of licenses available for mint
  uint256 public AVAILABLE_FOR_MINT = 30;

  constructor() ERC721("Nifty Stonks", "NiftyStonks") {
    // Do not sale on deploy
    pauseSale();

    // Make sure we start from 1, not from 0
    _tokenIdCounter.increment();

    FUNDS_TO = payable(msg.sender);
  }
  ...
  function setAvailableForMint(uint256 _availableForMintpply) public onlyOwner {
    AVAILABLE_FOR_MINT = _availableForMintpply;
  }

  function updateFundsTo(address payable _fundsTo) public onlyOwner {
    FUNDS_TO = _fundsTo;
  }

  function withdraw() public onlyOwner {
    (bool success, ) = FUNDS_TO.call{value: address(this).balance}("");
    require(success, "transfer failed");
  }

  function mintLicense(uint256 quantity) public payable whenNotPaused {
    // Cannot mint 0
    require(quantity >= 1, "Requested quantity cannot be zero");
    // Cannot mint more than maximum per transaction
    require(quantity <= MAX_MINTS_PER_TX, "Requested quantity more than maximum per transaction");
    // Cannot mint more than maximum per wallet
    require(
      quantity <= MAX_ITEMS_PER_WALLET,
      "Requested quantity more than maximum allowed per wallet"
    );
    // Can't mint more licenses than available
    require(
  ...
  function _baseURI() internal view virtual override(ERC721) returns (string memory) {
    return "https://niftystonks.com/api/licenses/";
  }
