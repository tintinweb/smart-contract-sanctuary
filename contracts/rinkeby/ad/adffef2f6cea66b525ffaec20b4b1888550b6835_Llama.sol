...
import "contracts/BaseOpenSea.sol";
...
contract Llama is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ReentrancyGuard, VRFConsumerBase {
    ...
    constructor(uint256 maxPurchase, uint256 maxTokens) VRFConsumerBase(
        ...
        unitCost = 10000000000000000; //start price - 0.01 ether
    ...
    function drawLotto() public onlyOwner nonReentrant{
        ...
        NextLottoDraw = block.timestamp.add(2419200); //next draw must be 28 days after the preveious one
   ...
   function currentWinners(uint256 n, uint256 maxNumber) public view returns (uint256[] memory expandedValues) {
    ...
    function mintLlama(uint256 numberToMint) public payable nonReentrant {
        ...
        do {
            ...
            uint256 jp = unitCost.mul(i).div(100).mul(33); //fixed 33% jackpot cut
	...
	function withdraw() public onlyOwner {
		...
		require(balance.sub(TotalJackpotInWei) > 0, '"I\'ve got nothing left!!" - Ian Beale 2014');
    ...
    function getProjectData() public view returns (uint256 _unitCost, uint256 _currentJackpot, uint256 _totalJackpot, uint256 _nextLottoTimestamp, uint256 _totalSupply, bool _isPaused) {
