...
contract DarkOdyssey is ERC721, Ownable, PaymentSplitter { 
  ...
  function mintWhitelist(uint amount, bytes32[] memory proof) external payable onlyEOA {
    require(whitelistOpen , "Whitelist Sale not started yet");
    require(totalSupply() + amount <= maxPresaleSupply, "max supply reached");
    require(mintsByAddress[msg.sender] + amount <= maxMintsPerWhitelistedAddress, "not enough mintsByAddress balance");
  ...
  function switchWhitelistState() external onlyOwner {
