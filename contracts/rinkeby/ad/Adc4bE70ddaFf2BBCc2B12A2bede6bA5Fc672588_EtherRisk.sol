...
contract EtherRisk is GameController, Pausable {
    ...
    function play(uint256 betId, address tokenAddress, uint256 betAmount, uint256 safeSpace) external whenNotPaused payable {
        require(safeSpace > 1 && safeSpace <= CYLINDER_SIZE, "incorrect safeSpace");
        ...
        uint256 potentialWin = betAmount.mul(CYLINDER_SIZE).div(safeSpace); // bet * cylinderSize / safeSpace
        ...
        request.games[request.tailIndex] = Game(betId, _msgSender(), tokenAddress, betAmount, safeSpace);
        request.tailIndex = request.tailIndex.add(1);
        emit EtherRiskRun(betId, _msgSender(), tokenAddress, betAmount, CYLINDER_SIZE, safeSpace, _lastRequestId, request.tailIndex.sub(1));
    }
    ...
    function _calculateResults(uint256 randomNumber, uint256 requestId, uint256 numberOfGames) internal {
        ...
        for (uint256 index = requests[requestId].headIndex; index < tailIndex && counter < numberOfGames; index = index.add(1)) {
            ...
            uint256 winCondition = randomNumber % CYLINDER_SIZE + 1;
            if (winCondition <= currentGame.safeSpace) {
                uint256 gasFee = _poolController.getOracleGasFee(currentGame.tokenAddress);
                uint256 winAmount = currentGame.bet.mul(CYLINDER_SIZE).div(currentGame.safeSpace); // bet * cylinderSize / safeSpace
