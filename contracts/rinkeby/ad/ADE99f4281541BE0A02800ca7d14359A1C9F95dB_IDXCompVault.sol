...
contract IDXCompVault is Ownable{
    ...
    function deposit(address _asset, uint256 _amount) external payable onlyOwner {
        ...
        underlyingBalances[tx.origin][_asset] = underlyingBalances[tx.origin][_asset].add(_amount);          // ajust balance in underlying token
    ...
    function _Computation(address _asset) internal view returns (uint256[] memory ){
        ...
        uint256 underlyingAmount = rate.mul(collateralBalances[tx.origin][_cToken]).div(1e18);     // the total position value  
       ...
       // collateralBalances[address(this)][_asset] = collateralBalances[address(this)][_asset].add(feesAdjusted);
        ...
        txData[0] =  collateralBalances[tx.origin][_cToken];               // collateral balance                              
        txData[1] =  rate.mul(withdrawn).div(1e18);                        // the underlying asset with interest minus feww (the amount withdrawn in underlying after all)                                      
        txData[2] =  feesAdjusted;                                         // fees
        txData[3] =  withdrawn;                                            // withdrawn amount in collateral
        return txData;   
    }
