...
interface ITreasury {
    ...
    function isReserveToken( address token_ ) external returns ( bool );
    function isPrincipleToken( address token_ ) external returns ( bool );
}
...
contract OlympusBondDepository is Ownable {
    ...
    internal returns ( bool ) {
        ...
        if ( ITreasury( treasury ).isReserveToken( token_ ) ) {
            value = amount_.div( 1e9 );
            premium = 1e4; // minimum value
            markdown = 1e9; // minimum value
        } else if ( ITreasury( treasury ).isPrincipleToken( token_ ) ) { // adjust for LP token
        ...
        } else if ( ITreasury( treasury ).isPrincipleToken( token_ ) ) {
