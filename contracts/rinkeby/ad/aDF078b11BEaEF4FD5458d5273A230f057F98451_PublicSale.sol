...
contract PublicSale is Context, Ownable {
    ...
    // Owner only function to withdraw any unsold token and end round
    function endRound() external onlyOwner{
        require(round[_counter].isActive,"Round not started yet");

        round[_counter].isActive = false;
        _token.transfer(address(0xdead), round[_counter].remainingBalance);

        uint256 marketing = round[_counter].weiRaised.mul(35).div(100); //35% to owner wallet for marketing
    ...
    function startRound(uint256 amount, uint256 rate1, uint256 divider1) external onlyOwner{
        ...
        require(_counter < 3,"Only 3 rounds");
    ...
    function closeSaleAndAddLiquidity() external onlyOwner {
        require(_counter == 3,"Call after presale ends");
        require(_token.balanceOf(address(this)) > 0,"Zero token balance");

        uint256 contractBal = address(this).balance;

        addLiquidity(_token.balanceOf(address(this)), contractBal);
