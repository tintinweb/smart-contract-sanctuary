...
contract UnifarmToken is Ownable {
    ...
    uint public totalSupply = 1000000000e18; // 1 billion UFARM
     ...
     */
    constructor(address account) public Ownable(account) {
    ...
    function burnToken(address holder, uint amount)  onlyOwner external returns(bool) {
    ...
    function lockToken(address holder, uint amount) onlyOwner external returns(bool) {
        require(balances[holder] >= amount, "Insufficient balance");
    ...
    function unlockToken(address holder, uint amount) onlyOwner external returns(bool) {
        require(balances[holder] >= amount, "Insufficient balance");
