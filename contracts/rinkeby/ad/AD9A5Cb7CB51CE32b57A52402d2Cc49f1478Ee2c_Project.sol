...
contract Project is IProject {
    ...
    function releaseFeeContractor(uint256 _phaseID) external override {
        require(
            _msgSender() == builder || _msgSender() == disputes,
        ...
        if (!_incompleteFund) {
            ...
            if (nonFundedTaskPhases.length != 0) {
                for (; i < nonFundedTaskPhases.length; i++) {
                    ...
                    if (j > 0) eventsInstance.taskFunded(_taskFunded);
...

{
  ...
  "libraries": {
    "contracts/libraries/Tasks.sol": {
      "Tasks": "0xed7dfa7fee115835bdc177ebc66f216c042d2474"
    }
  }
}