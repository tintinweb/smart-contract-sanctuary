...
contract LostPoetPages is ReentrancyGuard, AdminControl, ERC1155 {
    ...
    uint256 public constant maxPages = 65536;
    ...
    constructor()
        ERC1155(
            "https://arweave.net/Fx_J8h0B1q6BQYBEeB2bi2Cp4kdTepgM73SpDqK1iAU"
        )
    {}
    ...
    function withdraw(uint256 amount) external adminRequired {
        require(
            _royaltyRecipient != address(0x0),
            "Must set royalty recipient"
        );
    ...
    function activate(
        uint256 startTime,
        uint256 privateSaleInterval,
        uint256 publicSaleInterval,
        address ashContract_,
        uint256 ashThreshold_
     ...
     */
    function distributeRemainder(Reward[] memory rewards)
        external
        adminRequired
    {
        require(
            endTimestamp > 0 && block.timestamp > endTimestamp,
            "Can only distribute remainder after sale end"
        );
        for (uint256 i = 0; i < rewards.length; i++) {
            claimedPages += rewards[i].amount;
            require(claimedPages <= maxPages, "Too many requested");
    ...
    function claimPages(uint256 amount) external payable nonReentrant {
        require(
            active &&
                block.timestamp >= privateActiveTimestamp &&
        ...
        if (block.timestamp < publicActiveTimestamp) {
            ...
            require(
                IERC20(ashContract).balanceOf(msg.sender) >= ashThreshold,
                "You do not have enough ASH to participate in the private sale"
            );
        }
