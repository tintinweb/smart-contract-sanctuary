...
contract NFTDistributor is Ownable {
    ...
    uint16 pendingCount = total_count - reserve_count;
    ...
    function discountedPurchase(uint256 index, uint256 nonce, bytes32[] calldata merkleProof) payable external {
    ...
    function _purchase() internal {
        ...
        while(totalCost < msg.value) {
            uint256 price = _processPurchase(msg.value - totalCost);
            if (price == 0) {
                break;
            }
            totalCost += price;
        ...
        emit Purchased(msg.sender, purchaseCount, totalCost);
    }

    function _randomMint(address _to, uint256 _idMin) internal {
    ...
    /* Get price for minting _index"th NFT */
    function _processPurchase(uint256 maxPrice) internal returns(uint256 _price) {
        if (pendingCount == 0) return 0;
        uint256 index = total_count - pendingCount;

        uint256 currentLimit = 1800 + reserve_count;
        uint256 currentPrice = 0.001 ether;
        if (index < currentLimit) {
            if (maxPrice < currentPrice) return 0;
            _randomMint(msg.sender, total_count - currentLimit);
            return currentPrice;
        }

        currentLimit += 2200;
        currentPrice = 0.3 ether;
        if (index < currentLimit) {
            if (maxPrice < currentPrice) return 0;
            _randomMint(msg.sender, total_count - currentLimit);
            return currentPrice;
