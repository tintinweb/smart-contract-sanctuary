...
contract Acrocalypse is ERC721Enumerable, Ownable {
  ...
  uint256 public costPublic = 0.08 ether;
  uint256 public costVIP = 0.08 ether;
  uint256 public costPremium = 0.08 ether;
  uint256 public maxSupply = 10000;
  uint256 private _currentTokenId = 0;
  uint256 public ownerMaxSupply = 200;
  uint256 public ownerTokenId = 9801;
  ...
  // mint by owner
  function mintOwner(uint8 _amount, address _to) public onlyOwner {
      require(ownerTokenId.add(_amount) <= maxSupply, "amount is exceeding the maxSupply");
      for (uint i = ownerTokenId; i < ownerTokenId + _amount; i++){
          _safeMint(_to, i);
          ownerTokenId++;
      } 
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(_currentTokenId.add(_mintAmount) < (supply - ownerMaxSupply), "Not allowed to mint last 200");
    ...
    if (msg.sender != owner()) {
      if(onlyVIPWhitelisted == true) {
        ...
        require(ownerMintedCount.add(_mintAmount) <= vipNftPerAddressLimit, "max NFT per address exceeded");
        ...
        require(ownerMintedCount.add(_mintAmount) <= premiumNftPerAddressLimit, "max NFT per address exceeded");
