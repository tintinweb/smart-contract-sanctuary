...
contract Presale {
    ...
    event FinishedBuyTokens(IERC20 token_, uint256 amount);
    ...
    /**
     * @dev Creates a presale contract that allows all users to buy TsurugiCoin tokens.
     * @param _ownerWallet wallet address of owner to receive ethers
     * @param _baseTokenAddr TsurugiCoin contract addresss that is already deployed
     */
    constructor(
        address payable _ownerWallet,
        IERC20 _baseTokenAddr
    ) {
        ...
        TsurugiCoinToken = _baseTokenAddr;
    }
    ...
    /**
     * @notice A method to get paid and transfer tokens to the buyer.
     * @param _buyerAddr the buyer wallet address.
     * @param _amount the amount of tokens to buy.
     */
    function buyTokens(address _buyerAddr, uint256 _amount) external payable {
        require(_buyerAddr != address(0));
        require(msg.value != 0);

        uint256 amount = _amount * 10**18;
        totalEarned = totalEarned.add(msg.value);

        TsurugiCoinToken.transfer(_buyerAddr, amount);
        ownerWallet.transfer(msg.value);
        sentEther = msg.value;

        emit FinishedBuyTokens(TsurugiCoinToken, _amount);
    }
