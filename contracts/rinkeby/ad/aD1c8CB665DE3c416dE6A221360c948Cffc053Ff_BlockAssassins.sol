...
contract BlockAssassins is Ownable, ERC721, ERC721URIStorage, VRFConsumerBase {
    ...
    function claimRaffle(uint256[] calldata tickets) external {
        ...
        uint256 totalEntries = entriesPerAddress[msg.sender];

        // Refund losing tickets
        if (winningTickets != totalEntries) {
            // Payout value equal to number of bought tickets - paid for winning tickets
            (bool sent, ) = payable(msg.sender).call{
                value: (totalEntries - winningTickets) * MINT_COST
            }("");
            require(sent, "Unsuccessful in refund");
        }

        // Emit claim event
        emit RaffleClaimed(msg.sender, winningTickets, totalEntries - winningTickets);
    }
