...
//    mapping(uint256 => mapping(address => BetInfo)) public ledger;
    ...
    constructor(address _ptToken,uint256 _bettingBlockCount,address _teamAddress){
        ptToken = _ptToken;
        symbolOracle['ETH'] = 0x9326BFA02ADD2366b30bacB125260Af641031331;
        symbolOracle['BNB'] = 0x8993ED705cdf5e84D0a3B754b5Ee0e1783fcdF16;
        symbolOracle['BTC'] = 0x6135b13325bfC4B00278B4abC5e20bbce2D6580e;
        symbolSwitch['ETH'] = 1;
        symbolSwitch['BNB'] = 1;
        symbolSwitch['BTC'] = 1;
    ...
    */
    ...
    function startRoundAdmin(string memory _symbol) external onlyAdminOrOperator  whenNotPaused{
        startRound(_symbol);
    }

    function bettingBull(string memory _symbol,string memory direction,uint256 amount) external whenNotPaused returns(bool){
        ...
        TransferHelper.safeTransfer(ptToken,address(this),amount);
    ...
    function bettingBear(string memory _symbol,string memory direction,uint256 amount) external whenNotPaused returns(bool){
        uint256 currentEpoch = currentRoundMap[_symbol];
        require(canItBeInvested(_symbol,currentEpoch),"not_allowed_to_invest");
        require(ledgerMap[_symbol][currentEpoch][msg.sender].amount == 0, "Can only bet once per round");
        TransferHelper.safeTransfer(ptToken,address(this),amount);
