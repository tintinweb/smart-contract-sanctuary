 ...
 */
contract EaselyStandardCollectionV2 is EaselyMarketplaceCollection, IEaselyContractInitializer {   
    using ECDSA for bytes32; 
    using Strings for uint256;
    ...
    event TokenMinted(uint256 indexed tokenId, string indexed ipfsHash);
    ...
    function _mintClaimable(address receiver, bool claimable, string memory ipfsHash, string memory claimedIpfsHash) internal {
        ...
        emit TokenMinted(nextTokenId, ipfsHash);
    }
    ...
    function claimToken(uint256 tokenId) external {
        ...
        if (bytes(_tokenIdToPostClaimURI[tokenId]).length > 0) {
            ...
            emit TokenClaimed(_msgSender(), tokenId, nextTokenId);
            nextTokenId = nextTokenId + 1;
    ...
    function buyNewToken(
        ...
        emit TokenSold(_msgSender(), nextTokenId, currentPrice, hash);
        nextTokenId = nextTokenId + 1;
    ...
    function _hashToCheckForMinting(
        ...
        return _toEthSignedMessageHash(_hashForMinting(version, nonce, pricesAndTimestamps, ipfsHash, claimedIpfsHash, claimable));
    }
