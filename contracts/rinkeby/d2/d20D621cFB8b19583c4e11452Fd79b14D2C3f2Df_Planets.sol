...
* @author Niftydude
*/
contract Planets is VRFConsumerBase, AbstractERC1155Factory  {
    using SafeMath for uint256;
    using Counters for Counters.Counter;

    Counters.Counter private counter; 

    uint256 MAX_PURCHASE = 10;

    // ORDER: 0=MERCURY, 1=VENUS, 2=EARTH, 3=MARS, 4=JUPITER, 5=SATURN, 6=URANUS, 7=NEPTUNE, 8=PLUTO, 9=MOON
    uint256[] public maxSupply = [
        2440, 6052, 6378, 3396, 71492, 
        60268, 25559, 24764, 1195, 1738
    ];   

    uint256[] public prices = [
        1500000000000000000, 1000000000000000000, 1000000000000000000, 750000000000000000, 100000000000000000, 
        100000000000000000, 80000000000000000, 80000000000000000, 150000000000000000
    ]; 

    bool saleClosed;

    uint256 public claimWindowOpens;
    uint256 public claimWindowCloses;
    ...
    IMetaHero public metaHeroContract;
    ComicMinter public comicContract;
    address public upDAOAddress;  
    ...
    constructor(
        ...
        bytes32 _keyHash,
        uint256 _fee,        
        address _metaHeroContract,
        address _comicContract,
        address _upDAOAddress
        ...
        metaHeroContract = IMetaHero(_metaHeroContract);
        comicContract = ComicMinter(_comicContract);
        upDAOAddress = _upDAOAddress;
    }
    ...
    function editPlanet(
        ...
        planets[_planetIndex].maxSupply = _maxSupply;            
    ...
    function editWindows(
        ...
        require(_claimWindowOpens < _claimWindowCloses && _purchaseWindowOpens > _claimWindowCloses, "Edit windows: window combination prohibited");
    ...
    function purchase(uint256 planetID, uint256 amount) external payable {
        require(!paused(), "Purchase: is paused");
        require(!saleClosed, "Purchase: sale is closed");
        require (block.timestamp >= purchaseWindowOpens, "Claim: time window closed");
        require(totalSupply(planetID) + amount <= maxSupply[planetID], "Purchase: Max supply reached");
        require(amount <=  MAX_PURCHASE, "Purchase: Max amount exceeded");
        require(msg.value >= amount * prices[planetID], "Purchase: Insufficient payment");

        uint256 excessPayment = msg.value.sub(amount * prices[planetID]);
        if (excessPayment > 0) {
    ...
    function expand(uint256 numWinners, uint256 tokenId, uint256 drawId, uint256 snapshotEntries) external onlyOwner {
    ...
    function burnComicForMoon(uint256[] calldata tokenIds) external {
        uint256 count = tokenIds.length;

        require(count <= 40, "Too many tokens to be burned");
        require(totalSupply(9) + count <= maxSupply[9], "Purchase: Max supply reached");
    ...
    function claim(
        ...
        require(planets[planetId].claimed[msg.sender].add(amount) <= maxAmount, "Claim: Not allowed to claim given amount");
        ...
        planets[planetId].claimed[msg.sender] = planets[planetId].claimed[msg.sender].add(amount);
    ...
    function withdraw(address payable _to) external onlyOwner {
        ...
        _to.transfer(balance/10);
        payable(upDAOAddress).transfer(address(this).balance);
    }
