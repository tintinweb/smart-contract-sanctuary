...
contract MockConvergenceBondDepository is Ownable {
  ...
  address public immutable CONV; // token given as payment for bond
  address public immutable principle; // token used to create bond
  address public immutable treasury; // mints CONV when receives principle
  ...
  struct Bond {
    uint256 payout; // CONV remaining to be paid
  ...
  function deposit(
    ...
    require(payout >= 10000000, "Bond too small"); // must be > 0.01 CONV ( underflow protection )
    ...
    /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) CONV
         */
    ...
    if (fee != 0) {
      ...
      IERC20(CONV).safeTransfer(DAO, fee);
    }
  ...
  function stakeOrSend(
    ...
    if (!_stake) {
      ...
      IERC20(CONV).transfer(_recipient, _amount); // send payout
    } else {
      // if user wants to stake
      if (useHelper) {
        // use if staking warmup is 0
        IERC20(CONV).approve(stakingHelper, _amount);
        IStakingHelper(stakingHelper).stake(_amount, _recipient);
      } else {
        IERC20(CONV).approve(staking, _amount);
  ...
  function maxPayout() public view returns (uint256) {
    return IERC20(CONV).totalSupply().mul(terms.maxPayout).div(100000);
  }
  ...
  /**
   *  @notice calculate current ratio of debt to CONV supply
   *  @return debtRatio_ uint
   */
  function debtRatio() public view returns (uint256 debtRatio_) {
    uint256 supply = IERC20(CONV).totalSupply();
  ...
  /**
   *  @notice calculate amount of CONV available for claim by depositor
   *  @param _depositor address
  ...
  /**
   *  @notice allow anyone to send 
    tokens (excluding principle or CONV) to the DAO
   *  @return bool
   */
