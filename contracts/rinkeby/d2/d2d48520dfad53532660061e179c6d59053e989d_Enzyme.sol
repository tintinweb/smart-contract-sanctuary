...
contract Enzyme is ERC721Enumerable, Ownable {
    ...
    function buy(uint256 [] memory _tokenIds) public payable {
        ...
        require(_tokenIds.length <= MAX_PER_TX, "No more than 2 tokens per transaction");
        require(totalSupply().add(_tokenIds.length) <= MAX_SUPPLY, "Public Sale Exceeded");
        require( msg.value >= PUBLIC_MINT_PRICE * _tokenIds.length, "Payment too low");
        require(!Address.isContract(msg.sender));

        for (uint i = 0; i < _tokenIds.length; i++) {
            require(_tokenIds[i] < MAX_SUPPLY);
            if (totalSupply() < MAX_SUPPLY) {
                _mint(msg.sender, _tokenIds[i]);
                emit Minted(msg.sender, _tokenIds[i]);
            }
        }
