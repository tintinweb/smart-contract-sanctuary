...
contract Bridgeable is ContractGuard {
    ...
    address public TICKET; // "Bond"
    address public paprOracle;

    /* =================== Events =================== */
 
    event Initialized(address indexed executor, uint256 at);
    event RedeemedTICKETs(address indexed from, uint256 amount);
    event BoughtTICKETs(address indexed from, uint256 amount);
...
//==========================================//
    ...
    function checkOperator() private {
        require(
                IBasisAsset(TICKET).operator() == address(this) &&
    ...
    function setPapr(address _papr) external onlyOperator {
        papr = _papr;
    }
    ...
    function buyTICKETs(uint256 amount, uint256 targetPrice) external onlyOneBlock {
        checkOperator();
        require(amount > 0, "Cannot purchase TICKETs with zero amount");

        uint256 paprPrice = IPapr(papr).getPaprPrice();
        require(paprPrice == targetPrice, "Papr price moved");

        //uint256 _boughtTICKET = amount.mul(1e18).div(paprPrice);
        //uint256 paprSupply = IERC20(papr).totalSupply();
        //uint256 newTICKETSupply = IERC20(TICKET).totalSupply().add(_boughtTICKET);

        IBasisAsset(papr).burnFrom(msg.sender, amount);
        IBasisAsset(TICKET).mint(msg.sender, amount);

        _updatePaprPrice();

        emit BoughtTICKETs(msg.sender, amount);
    }

    function redeemTICKETs(uint256 amount, uint256 targetPrice) external onlyOneBlock {
        checkOperator();
        require(amount > 0, "Cannot redeem TICKETs with zero amount");

        uint256 paprPrice = IPapr(papr).getPaprPrice();
        require(paprPrice == targetPrice, "Papr price moved");

        IBasisAsset(TICKET).burnFrom(msg.sender, amount);
        IBasisAsset(papr).mint(msg.sender, amount);

        _updatePaprPrice();

        emit RedeemedTICKETs(msg.sender, amount);
    }
    ...
    function governanceRecoverUnsupported(
        ...
        require(address(_token) != address(TICKET), "TICKET");
