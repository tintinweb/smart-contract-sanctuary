...
contract Ecoz is ERC721, Ownable {
    ...
    function ownerMint(uint256 species, uint256 amt) public payable onlyOwner {
        if (species == 0) {
            require(jagSupply + amt <= 450);
    ...
    function changeURI(address newJagAddress, address newBuckAddress, address newTreeAddress) public onlyOwner {
        jaguarsURI = IJag(newJagAddress);
        bushBucksURI = IBuck(newBuckAddress);
        bananaTreesURI = ITree(newTreeAddress);
    }
