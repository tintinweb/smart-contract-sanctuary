...
contract TWTR is ERC20Permit, Configurable {
    ...
    uint224 internal _buyBuf;               // uses single storage slot
    ...
    function _updateBuf(uint amt) internal {
        ...
        require(buf <= uint224(-1), "buyBuf OVERFLOW");
        (_buyBuf, _lastUpdateBuf) = (uint224(buf), uint32(now));
    }
    ...
    function setBuf_(uint buf, uint price) external governance {
        ...
        require(buf <= uint224(-1), "buyBuf OVERFLOW");
        (_buyBuf, _lastUpdateBuf) = (uint224(buf), uint32(now));
        config[_factorPrice_] = price.mul(_totalSupply).div(buf);
    }

    function price1At(uint vol) public view returns(uint) {
        //if(_totalSupply == 0)
        //    return config[_factorPrice_];
        return config[_factorPrice_].mul(buyBuf().add(vol).add(1)).div(_totalSupply.add(vol).add(1));
    }
 ...
 */
    ...
    function _setAcct(address sender, uint quota, uint locked, uint lockSpan, address referrer, bool isCmpd) internal {
        ...
        _accts[sender] = Account(uint112(quota), uint112(locked), uint32(unlockEnd), referrer, isCmpd, 0);
    ...
    function buyMore(bytes32 referrer, Twitter[] memory twitters, Signature[] memory signatures) external payable compound(referrer) {
        address sender = _msgSender();
        verify(sender, referrer, twitters, signatures);
    ...
    function buyMoreWithToken(address token, uint amt, bytes32 referrer, Twitter[] memory twitters, Signature[] memory signatures) external compound(referrer) {
        address sender = _msgSender();
        verify(sender, referrer, twitters, signatures);
        _updateQuota(sender, twitters);
    ...
    function calcOut1(uint v) public view returns(uint a) {
        v = v.mul(config[_factorPrice_]).div(1e18);
        uint b = buyBuf().add(1);
        uint s = _totalSupply.add(1);
        a = Math.sqrt(v.mul(v).add(b.mul(b)).add(v.mul(s).mul(2)));
        a = a.add(v).sub(b).mul(s).div(s.add(b));
    }
    ...
    function _settleReward(address sender, uint amt) internal {
        ...
        for(uint i=1; i<=config[_reward_]; i++) {
            ...
            uint rwd = amt.mul(getConfigI(_reward_, i)).div(1e18);
