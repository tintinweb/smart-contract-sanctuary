...
contract NFT is ERC721, PaymentSplitter {
  ...
  function mintSale(uint amount) external payable onlyEOA {
    ...
    require(mintedTokensByAddress[msg.sender] + amount <= maxMintsPerAddress, "Address already minted");
  ...
  function mintWhitelist(uint amount, bytes32[] calldata proof) external payable onlyEOA {
    ...
    require(mintedTokensByAddress[msg.sender] + amount <= maxMintsPerAddress, "already minted max amount");
