...
contract TestCatryoshkas is ERC721, ERC721Enumerable, Ownables {
    ...
    uint256 private constant _MAX_RESERVE_SUPPLY = 8;
    uint256 private constant _MAX_MINT_SUPPLY = 18;
    ...
    function mintTokens(uint256 numberOfTokens_) public payable isSupplyAvailable(numberOfTokens_) {
        ...
        if(totalMintedSupply() <= _presaleMints){
    ...
    function _mintReserveTokens(uint256 startIndex_, uint256 endIndex_, address to_ ) internal {
        ...
        require( _reserveSupply <= _MAX_RESERVE_SUPPLY, "Exceeds reserve");

        uint256 startPosition = startIndex_;

        for (uint256 index = startIndex_; index < endIndex_; index++) {
            
            _safeMint(to_, startPosition);

            startPosition = startPosition.add(1);
            _reserveSupply = _reserveSupply.add(1);
