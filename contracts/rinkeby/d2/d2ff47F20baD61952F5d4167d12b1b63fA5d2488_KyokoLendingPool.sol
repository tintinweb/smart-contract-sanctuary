...
contract KyokoLendingPool is ILendingPool, LendingPoolStorage, AccessControlEnumerable {
    ...
    */
    ...
    event getUserAccountData_event(uint256 totalDebtInUSD, uint256 availableBorrowsInUSD);

    event validation(uint256 usd, DataTypes.ReserveData reserve, uint256 amount);

    event debtMint(address user, address onBehalfOf, uint256 amount, uint256 borrowIndex);

    event updateRates(address asset, address kTokenAddress, uint256 add, uint256 taken);

    event tokenMint(address user, uint256 amount);
    function _executeBorrow(ExecuteBorrowParams memory vars) internal {
        emit ExecuteBorrowParams_event(vars.asset, vars.user, vars.onBehalfOf, vars.amount, vars.kTokenAddress, vars.releaseUnderlying);
        DataTypes.ReserveData storage reserve = _reserves[vars.asset];

        (uint256 totalDebtInUSD, uint256 availableBorrowsInUSD) = getUserAccountData(vars.user);
        ...
        emit debtMint(vars.user, vars.onBehalfOf, vars.amount, reserve.variableBorrowIndex);
        ...
        emit updateRates(vars.asset, vars.kTokenAddress, 0, vars.releaseUnderlying ? vars.amount : 0);
    ...
    {
        totalDebtInUSD = GenericLogic.calculateUserAccountData(
            user,
            _reserves,
            _reservesList,
            _reservesCount
        ...
        availableBorrowsInUSD = creditContract.getCreditLine(user).sub(totalDebtInUSD);
    }
    ...
    **/
        ...
        returns (uint256)
    {
        ...
        for (vars.i = 0; vars.i < reservesCount; vars.i++) {
            ...
            uint256 currentReserveBorrows = IERC20(currentReserve.variableDebtTokenAddress).balanceOf(user);
            if (currentReserveBorrows > 0) {
                vars.compoundedBorrowBalance = vars.compoundedBorrowBalance.add(
                    currentReserveBorrows
                );

                vars.totalDebtInUSD = vars.totalDebtInUSD.add(
                    uint256(1).mul(vars.compoundedBorrowBalance).div(vars.tokenUnit)
                );
            }
...
contract CreditSystem is AccessControlEnumerable {
    ...
    {
        require(
            whiteList[user].creditLine == 0,
            "ADDRESS_ALREADY_SET_CREDIT_LINE"
        );
        whiteList[user].creditLine = amount;
        whiteList[user].isActive = true;
    }

    function setPaused(address user, bool flag) public onlyCreditManager {
        whiteList[user].isActive = flag;
    }
    ...
    /**
     * @dev query the address's credit line
     */
    function getCreditLine(address user) public view returns (uint256) {
        return whiteList[user].creditLine;
    }
