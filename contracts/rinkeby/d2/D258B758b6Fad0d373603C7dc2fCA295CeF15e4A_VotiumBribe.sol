...
abstract contract Ownable is Context {
	  ...
	  address private _owner = 0xdC7C7F0bEA8444c12ec98Ec626ff071c6fA27a19; // Votium multi-sig address (except on rinkeby)
...
contract VotiumBribe is Ownable {
	...
	/* ========== PUBLIC FUNCTIONS ========== */
		...
		function depositBribe(address _token, uint256 _amount, bytes32 _proposal, uint256 _choiceIndex) public {
			...
			if(tokenInfo[_token].distributor == address(0)) {
				IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);	// if distributor contract is not set, store in this contract until ready
			} else {
				IERC20(_token).safeTransferFrom(msg.sender, tokenInfo[_token].distributor, _amount); // if distributor contract is set, send directly to distributor
			}
			emit Bribed(_token, _amount, _proposal, _choiceIndex);
		}
	...
	/* ========== APPROVED TEAM FUNCTIONS ========== */
		...
		// transfer stored bribes to distributor (bribes made before token distributor has been set)
	...
	/* ========== MUTLI-SIG FUNCTIONS ========== */
	  ...
	  function modifyTeam(address _member, bool _approval) public onlyOwner {
		...
		// register token and add to whitelist
		function registerToken(address _token) public onlyOwner {
			require(tokenInfo[_token].registered == false, "exists");
			tokenInfo[_token].registered = true;
			tokenInfo[_token].whitelist = true;
		}

		// register multiple tokens and add to whitelist
		function registerTokens(address[] memory _tokens) public onlyOwner {
			for(uint256 i=0;i<_tokens.length;i++) {
				require(tokenInfo[_tokens[i]].registered == false, "exists");
				tokenInfo[_tokens[i]].registered = true;
				tokenInfo[_tokens[i]].whitelist = true;
			}
		}

		// toggle whitelist status
		function updateWhitelist(address _token, bool _whitelist) public onlyOwner {
			require(tokenInfo[_token].registered == true, "!registered");
			tokenInfo[_token].whitelist = _whitelist;
		}
