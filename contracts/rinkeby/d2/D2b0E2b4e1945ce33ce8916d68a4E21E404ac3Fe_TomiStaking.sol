...
contract TomiStaking is Ownable, ReentrancyGuard, Pausable {
    ...
    constructor(address _rewardsDistribution, address _tomiToken, address _tomiPlatform) public {
    ...
    function getTotalRewardPool() external view returns (uint256) {
        ...
        return ((periodFinish.sub(block.timestamp)).mul((rewardRate.mul(rewardsDuration))).div(periodFinish)).add(_revenueShared);
    }
    ...
    function updateRevenueShare(uint256 revenueShared) external onlyPlatform {
        ...
        _accAmountPerRevenueShared = _accAmountPerRevenueShared.add(_revenueShared.div(_totalSupply).mul(1e18));
    }
