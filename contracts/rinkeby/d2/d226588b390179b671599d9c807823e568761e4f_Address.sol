...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.2.0/contracts/token/ERC721/extensions/IERC721Enumerable.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.2.0/contracts/token/ERC721/extensions/ERC721Enumerable.sol
...
// File: contracts/nymphomaniacs.sol


pragma solidity ^0.8.0;

/**
 * @title Nymphomaniacs contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract Nymphomaniacs is ERC721, Ownable, ERC721Enumerable {
    using SafeMath for uint256;

    string public NYMPHS_PROVENANCE = "";

    uint256 public startingIndexBlock;

    uint256 public startingIndex;

    uint256 public constant nymphsPrice = 50000000000000000; //0.05 ETH

    uint public constant maxNimphPurchase = 20;

    uint256 public MAX_NYPHS;
    ...
    /**
     * Set some Nymphomaniacs aside
     */
    function reserveNimphos() public onlyOwner {    
        require(totalSupply().add(maxNimphPurchase) <= MAX_NYPHS, "Reserve would exceed max supply of Apes");
        uint supply = totalSupply();
        uint i;
        for (i = 0; i < maxNimphPurchase; i++) {
    ...
    */
    function setProvenanceHash(string memory provenanceHash) public onlyOwner {
        NYMPHS_PROVENANCE = provenanceHash;
    }
    ...
    * Mints Bored Apes
    */
    function mintNimpho(uint numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint Ape");
        require(numberOfTokens <= maxNimphPurchase, "Can only mint 20 tokens at a time");
        require(totalSupply().add(numberOfTokens) <= MAX_NYPHS, "Purchase would exceed max supply of Apes");
        require(nymphsPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
        
        for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply();
            if (totalSupply() < MAX_NYPHS) {
        ...
        if (startingIndexBlock == 0 && (totalSupply() == MAX_NYPHS || block.timestamp >= REVEAL_TIMESTAMP)) {
    ...
    function setStartingIndex() public {
        ...
        startingIndex = uint(blockhash(startingIndexBlock)) % MAX_NYPHS;
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number.sub(startingIndexBlock) > 255) {
            startingIndex = uint(blockhash(block.number - 1)) % MAX_NYPHS;
        }
