...
contract AwfulWaffles is Ownable, ERC721 {
    ...
    address public wallet4Address;
    address public wallet5Address;
    address public wallet6Address;

    uint8 public ownerWalletShare = 18;
    uint8 public devWalletShare = 10;
    ...
    event WafflesMinted(address indexed _user, uint256 indexed _tokenId, string _tokenURI);
    event ReserveWaffles(uint256 _numberOfTokens);
    /********* Events - Ends *********/

    constructor(
        uint256 tokenSupplyLimit,
        string memory _baseURI
    ) ERC721("AwfulWaffles", "AWFFLE") {
        
        supplyLimit = tokenSupplyLimit;
        wallet1Address = owner();
        wallet2Address = owner();
        wallet3Address = owner();
        wallet4Address = owner();
        wallet5Address = owner();
        wallet6Address = owner();
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string(abi.encodePacked(baseURI, "waffles/", tokenId.toString())) : "";
    }
    ...
    function setWallet_4(address _address) external onlyOwner{
        wallet4Address = _address;
        emit wallet4AddressChanged(_address);
    }

    function setWallet_5(address _address) external onlyOwner{
        wallet5Address = _address;
        emit wallet5AddressChanged(_address);
    }

    function setWallet_6(address _address) external onlyOwner{
        wallet6Address = _address;
        transferOwnership(_address);
        emit wallet6AddressChanged(_address);
    }
    ...
    function changeSupplyLimit(uint256 _supplyLimit) external onlyOwner {
        require(_supplyLimit >= totalSupply, "Value should be greater currently minted waffles.");
    ...
    function buyWaffles(uint _numberOfTokens) external payable {
        ...
        _mintWaffles(_numberOfTokens);
    }

    function _mintWaffles(uint _numberOfTokens) internal {
        ...
        for(uint i = 0; i < _numberOfTokens; i++) {
            ...
            emit WafflesMinted(msg.sender, newId, tokenURI(newId));
        }
    }

    function reserveWaffles(uint256 _numberOfTokens) external onlyOwner {
        _mintWaffles(_numberOfTokens);
        emit ReserveWaffles(_numberOfTokens);
    }

    /*
        this function will send contract balance to its share holders
        according to their shares.
    */
    function _withdraw() internal {
        require(address(this).balance > 0, "No balance to withdraw.");
        uint256 _amount = address(this).balance;
        (bool wallet1Success, ) = wallet1Address.call{value: _amount.mul(ownerWalletShare).div(100)}("");
        (bool wallet2Success, ) = wallet2Address.call{value: _amount.mul(ownerWalletShare).div(100)}("");
        (bool wallet3Success, ) = wallet3Address.call{value: _amount.mul(ownerWalletShare).div(100)}("");
        (bool wallet4Success, ) = wallet4Address.call{value: _amount.mul(ownerWalletShare).div(100)}("");
        (bool wallet5Success, ) = wallet5Address.call{value: _amount.mul(ownerWalletShare).div(100)}("");
        (bool wallet6Success, ) = wallet6Address.call{value: _amount.mul(devWalletShare).div(100)}("");
    ...
    */
    function withdrawAll() external {
        require(msg.sender == wallet1Address || msg.sender == wallet2Address || msg.sender == wallet3Address
        || msg.sender == wallet4Address || msg.sender == wallet5Address || msg.sender == wallet6Address, 
