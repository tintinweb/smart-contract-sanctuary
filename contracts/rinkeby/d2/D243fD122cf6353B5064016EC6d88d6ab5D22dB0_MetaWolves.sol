...
contract MetaWolves is ERC721Enumerable {
    ...
    modifier haveLimit (uint256 amount) {
        for(; mintLimits[mintLimitIndex].expireTime <= block.timestamp && mintLimits[mintLimitIndex].expireTime != 0; mintLimitIndex++){}
        address global = address(this);

        require((minted[msg.sender].mintLimitIndex < mintLimitIndex
            || minted[msg.sender].minted + amount <= mintLimits[mintLimitIndex].limit)
            && (minted[global].minted + amount <= mintLimits[mintLimitIndex].globalLimit
            || minted[global].mintLimitIndex < mintLimitIndex)
        , "Cannot mint more than limit");

        if (minted[msg.sender].mintLimitIndex < mintLimitIndex) {
            minted[msg.sender].minted = uint32(amount);
            minted[msg.sender].mintLimitIndex = mintLimitIndex;
        } else {
            minted[msg.sender].minted += uint32(amount);
        }

        if (minted[global].mintLimitIndex < mintLimitIndex) {
    ...
    constructor () ERC721("MetaWolves", "MWL") {
        mintLimits.push(MintLimit(1643046600, 1, 68));
        mintLimits.push(MintLimit(1643046900, 215, 215));
        ...
        baseURIRoots.push(BaseURIRoot(6, "ipfs://bafkreib6vllfcohj5wfre7mldpagnpaykuxdha4gr4tvdiiafpngaae6pu/?"));
    ...
    function remainMintable(address minter) external view returns (uint256) {
        uint256 mintLimitIndex_ = mintLimitIndex;
        for(; mintLimitIndex_ < mintLimits.length && mintLimits[mintLimitIndex_].expireTime <= block.timestamp && mintLimits[mintLimitIndex_].expireTime != 0; mintLimitIndex_++) {}
        address global = address(this);
