...
contract LostPunks is ERC721Enumerable, Ownable {
    ...
    function metadataAttributes(bytes memory punkAssets, uint16 index) internal view returns (string memory text) {
        ...
        text = appendAttribute(text, "# Traits", toString(accessoryCount), false, false, true);
        text = appendAttribute(text, "Generation", toString(generationMapping[index]), false, false, true);
        text = appendAttribute(text, "Father", toString(fatherMapping[index]), false, true, true);
        text = appendAttribute(text, "Mother", toString(motherMapping[index]), false, true, true);
    }
    ...
    string private constant SVG_HEADER = '<svg id="crisp" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMin slice" viewBox="0 0 360 360">';
