...
contract Auction is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, EIP712Upgradeable {
    ...
    /// @dev Maximum difference between End and current timestamp
    uint256 public maxOrderLeft;
    ...
    event BidAccepted(
        ...
        IAuction.CreateAuction auction,
        IAuction.CreateBid bid
    );
     ...
     * @param oldPlatformFee Old platform fee
     ...
     *
     * @param by Updater
     * @param oldAddressRegistry Old AddressRegistry contract address
     * @param newAddressRegistry New AddressRegistry contract address
     */
        ...
        view
    {
        ...
        validNFTAndOwner(_createAuction.tokenAddress, _createAuction.tokenId, msg.sender);
        validPayment(
            _createAuction.quantity,
            _createAuction.priceTokenAddress,
            _createAuction.auctionInitialLotPrice,
        ...
        validOrderDuration(_createAuction.auctionStartsAt, _createAuction.auctionExpiresAt);
    }
    ...
    function _validateBid(
        ...
        // check timestamp
        require(_createBid.bidStartsAt <= _getNow() && _getNow() <= _createBid.bidExpiresAt, "Invalid bid");
        require(
        ...
        require(_createBid.bidInitialLotPrice >= _auctionInitialLotPrice, "Invalid bid price");

        // check bid lot price, bid unit price and order duration
        require(
            maxLotPrice >= _createBid.bidInitialLotPrice &&
                _createBid.bidInitialLotPrice >= minLotPrice &&
                _createBid.bidInitialLotPrice > 0,
            "Variable lot price not supported yet"
        );
        require(
            maxUnitPrice >= _createBid.bidInitialUnitPrice &&
                _createBid.bidInitialUnitPrice >= minUnitPrice &&
                _createBid.bidInitialUnitPrice > 0,
            "Variable unit price not supported yet"
        );
        validOrderDuration(_createBid.bidStartsAt, _createBid.bidExpiresAt);
    }
