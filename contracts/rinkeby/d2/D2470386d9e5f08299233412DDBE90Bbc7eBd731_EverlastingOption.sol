...
contract EverlastingOption is IEverlastingOption, Ownable {
    ...
    address private immutable _governanceData;
    address private immutable _volatilityChainlinkOracle;
    address private immutable _volatilityOracleAddress;
    ...
    // symbolId => SymbolInfo
    mapping(uint256 => SymbolInfo) private _symbols;
    ...
    constructor(
        ...
        _governanceData = addresses[5];
        _volatilityChainlinkOracle = addresses[6];
        _volatilityOracleAddress = addresses[7];
    ...
    function removeSymbol(uint256 symbolId) external override onlyOwner {
        delete _symbols[symbolId];
        IPTokenOption(_pTokenAddress).removeSymbolId(symbolId);
    }

    function toggleCloseOnly(uint256 symbolId) external override onlyOwner {
    ...
    function tradeWithChainlinkOracle(uint256 symbolId, int256 tradeVolume) external {
        require(IPTokenOption(_pTokenAddress).isActiveSymbolId(symbolId), "inv symbolId");
        require(tradeVolume != 0 && (tradeVolume / ONE) * ONE == tradeVolume, "inv Vol");
        _updateVolatilityFromOracle();
        _trade(msg.sender, symbolId, tradeVolume);
    }
    ...
    function _trade(
        ...
        params.tvCost = _queryTradePMM(
            symbolId,
            (tradeVolume * _symbols[symbolId].multiplier) / ONE,
            timePrices[index]
        );
        ...
        if (
            !(positions[index].volume >= 0 && tradeVolume >= 0) &&
            !(positions[index].volume <= 0 && tradeVolume <= 0)
        ) {
        ...
        params.protocolFee = (params.fee * IGovernanceData(_governanceData).protocolFeeCollectRatio()) / ONE;
    ...
    function _liquidate(address liquidator, address account) internal _lock_ {
        ...
        for (uint256 i = 0; i < symbolIds.length; i++) {
            if (positions[i].volume != 0) {
                int256 tvCost = _queryTradePMM(
                    symbolIds[i],
                    (-positions[i].volume * _symbols[symbolIds[i]].multiplier) / ONE,
                    timePrices[i]
                );
                ...
                int256 curCost = (((-positions[i].volume * _symbols[symbolIds[i]].intrinsicValue) / ONE) *
                    _symbols[symbolIds[i]].multiplier) /
                    ONE +
                    tvCost;
        ...
        int256 reward;
        int256 minLiquidationReward = IGovernanceData(_governanceData).minLiquidationReward();
        int256 maxLiquidationReward = IGovernanceData(_governanceData).maxLiquidationReward();
        ...
        if (netEquity <= minLiquidationReward) {
            ...
            reward =
                ((netEquity - minLiquidationReward) *
                    IGovernanceData(_governanceData).liquidationCutRatio()) /
                ONE +
    ...
    // Helpers
    //================================================================================
    function _updateVolatility(SignedPrice memory volatility) internal {
        IVolatilityOracle(_volatilityOracleAddress).updateVolatility(
    ...
    function _updateVolatilityFromOracle() internal {
        IVolatilityOracle(_volatilityOracleAddress).updateVolatilityFromChainlink(
            block.timestamp, // solhint-disable-line
            IVolatilityChainlinkOracle(_volatilityChainlinkOracle).latestVolatility()
        );
    }
    ...
    function getTvMidPrice(uint256 symbolId)
        public
        view
        returns (
            int256,
        ...
        )
    {
        ...
        int256 volatility = IVolatilityOracle(_volatilityOracleAddress).getVolatility().utoi();
        ...
        int256 midPrice = PmmPricer.getTvMidPrice(
            timePrice,
            ((s.tradersNetVolume * s.multiplier) / ONE),
            _liquidity + s.quote_balance_offset,
            s.K
        );
        return (timePrice, midPrice, delta);
    }

    function _queryTradePMM(
        uint256 symbolId,
        int256 volume,
        int256 timePrice
    ) internal view returns (int256 tvCost) {
        ...
        )
    {
        ...
        for (uint256 i = 0; i < symbolIds.length; i++) {
            ...
            int256 oraclePrice = getOraclePrice(s.oracleAddress);
            int256 intrinsicPrice = s.isCall
                ? (oraclePrice - s.strikePrice).max(0)
                : (s.strikePrice - oraclePrice).max(0);
            ...
            if (s.tradersNetVolume != 0) {
                int256 cost = (((s.tradersNetVolume * (intrinsicPrice + midPrice)) / ONE) * s.multiplier) /
                    ONE;
                totalDynamicEquity -= cost - s.tradersNetCost;
                int256 notionalValue = ((((s.tradersNetVolume * oraclePrice) / ONE) * s.multiplier) / ONE);
                minPoolRequiredMargin +=
                    (notionalValue.abs() *
                        _dynamicInitialMarginRatio(oraclePrice, s.strikePrice, s.isCall) *
                        10) /
                    ONE;
            }
        }
        ...
        if (curTimestamp > preTimestamp && _liquidity > 0) {
            for (uint256 i = 0; i < symbolIds.length; i++) {
                ...
                if (s.tradersNetVolume != 0) {
                    ...
                    params.ratePerSec1 =
                        (((((((((((deltas[i] * s.tradersNetVolume) / ONE) * params.oraclePrice) / ONE) *
                            params.oraclePrice) / ONE) * s.multiplier) / ONE) * s.multiplier) / ONE) *
                            s.deltaFundingCoefficient) /
                        totalDynamicEquity;
                    params.offset1 = params.ratePerSec1 * int256(curTimestamp - preTimestamp);
                    unchecked {
                        s.cumulativeDeltaFundingRate += params.offset1;
                    }

                    params.ratePerSec2 =
                        (((s.timeValue * s.multiplier) / ONE) * _premiumFundingCoefficient) /
                        ONE;
                    params.offset2 = params.ratePerSec2 * int256(curTimestamp - preTimestamp);
                    unchecked {
                        s.cumulativePremiumFundingRate += params.offset2;
                    }
                }
            }
        }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < symbolIds.length; i++) {
            if (positions[i].volume != 0) {
                ...
                int256 delta;
                unchecked {
                    delta = cumulativeDeltaFundingRate - positions[i].lastCumulativeDeltaFundingRate;
                }
                ...
                unchecked {
                    delta = cumulativePremiumFundingRate - positions[i].lastCumulativePremiumFundingRate;
                }
                funding += (positions[i].volume * delta) / ONE;
    ...
    function _getTraderMarginStatus(
        ...
        for (uint256 i = 0; i < symbolIds.length; i++) {
            if (positions[i].volume != 0) {
                ...
                int256 cost = (((positions[i].volume * (s.intrinsicValue + s.timeValue)) / ONE) *
                    s.multiplier) / ONE;
                ...
                totalMinInitialMargin +=
                    (notionalValue.abs() * _dynamicInitialMarginRatio(oraclePrice, s.strikePrice, s.isCall)) /
                    ONE;
            }
        }
        int256 totalMinMaintenanceMargin = (totalMinInitialMargin *
            IGovernanceData(_governanceData).maintenanceMarginRatio()) /
            IGovernanceData(_governanceData).initialMarginRatio();
    ...
    function _dynamicInitialMarginRatio(
        ...
        if ((strikePrice >= spotPrice && !isCall) || (strikePrice <= spotPrice && isCall)) {
            return IGovernanceData(_governanceData).initialMarginRatio();
        } else {
            int256 OTMRatio = isCall
                ? (((strikePrice - spotPrice) * ONE) / strikePrice)
                : (((spotPrice - strikePrice) * ONE) / strikePrice);
            int256 dynInitialMarginRatio = (((ONE - OTMRatio * 3) *
                IGovernanceData(_governanceData).initialMarginRatio()) / ONE).max(MinInitialMarginRatio);
