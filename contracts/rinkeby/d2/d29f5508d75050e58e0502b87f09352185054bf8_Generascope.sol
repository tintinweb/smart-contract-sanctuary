...
contract Generascope is ERC721Enumerable, Ownable {
    ...
    uint256 public constant GS_PRIVATE = 10;
    uint256 public constant GS_PUBLIC = 15;
    uint256 public constant GS_MAX = GS_PRIVATE + GS_PUBLIC;
    uint256 public constant GS_PRICE = 0.0001 ether;
    uint256 public constant GS_PURCHASE_LIMIT = 10;
    ...
    function buy(uint256 tokenQuantity) external payable {
        ...
        require(
            privateAmountMinted + publicAmountMinted + tokenQuantity <= GS_MAX,
            "EXCEED_AVAILABLE"
        );
        require(
            allPurchases[msg.sender] + tokenQuantity <=
                GS_PURCHASE_LIMIT,
            "EXCEED_ALLOC"
        );
        require(GS_PRICE * tokenQuantity == msg.value, "INSUFFICIENT_ETH");
    ...
    function presaleBuy(uint256 tokenQuantity) external payable {
        ...
        require(totalSupply() < GS_MAX, "OUT_OF_STOCK");
        require(
            privateAmountMinted + tokenQuantity <= GS_PRIVATE,
            "EXCEED_PRIVATE"
        );
        require(
            allPurchases[msg.sender] + tokenQuantity <=
