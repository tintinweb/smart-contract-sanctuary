...
contract qdexswapLocker is Ownable, ReentrancyGuard {
  using SafeMath for uint256;
  using EnumerableSet for EnumerableSet.AddressSet;

  IqdexswapFactory public qdexswapFactory;
  ...
  constructor(IqdexswapFactory _qdexswapFactory) public {
    devaddr = msg.sender;

    qdexswapFactory = _qdexswapFactory;
  }
  ...
  function lockLPToken (address _lpToken, uint256 _amount, uint256 _unlock_date, address payable _withdrawer) external payable nonReentrant {
    ...
    IqdexswapV2Pair lpair = IqdexswapV2Pair(address(_lpToken));
    address factoryPairAddress = qdexswapFactory.getPair(lpair.token0(), lpair.token1());
