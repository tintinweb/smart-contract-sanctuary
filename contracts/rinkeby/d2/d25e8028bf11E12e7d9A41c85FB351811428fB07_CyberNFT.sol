...
// File: contracts/CyberNFT.sol


pragma solidity ^0.8.4;


contract CyberNFT is ERC1155 {
    address _admin;
    ...
    event CyberNFTMint(uint256 id, uint256 amount, string metadataUri, address initialOwner, bytes data);
    event CyberNFTTransfer(uint256 id, uint256 amount, address from, address to);
    ...
    function mint(uint256 id, uint256 amount, string memory metadataUri, address initialOwner, bytes calldata data) public isAdmin() nonExist(id) {
        ...
        emit CyberNFTMint(id, amount, metadataUri, initialOwner, data);
    }
    ...
    function _beforeTokenTransfer(
        ...
        for (uint i = 0; i < ids.length; i++) {
            emit CyberNFTTransfer(ids[i], amounts[i], from, to);
        }
