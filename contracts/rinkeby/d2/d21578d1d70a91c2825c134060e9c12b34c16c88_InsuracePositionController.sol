...
contract InsuracePositionController is AbstractController {
    ...
    function __InsuracePositionController_init(
        ...
        _checkApprovalByAssetDecimals(_stkingAssetsAddress,address(_stakingController),PreciseUnitMath.MAX_UINT_256);
        //unstaking
    ...
    function stake(uint256 _amount) external override onlyIndex {
        ...
        if (address(base) != stakingAsset) {
            _checkApprovalByAssetDecimals(address(base), address(_uniswapRouter), _amount);
    ...
    function unstake() external override onlyIndex returns (uint256) {
        ...
        _checkApprovalByAssetDecimals(address(stakingAsset), address(_uniswapRouter), _unstakedAmount);
    ...
    function withdrawRewards() external override {
        ...
        _checkApprovalByAssetDecimals(address(insurToken), address(_uniswapRouter), _rewards);
...
abstract contract AbstractController is OwnableUpgradeable, IPositionController {
    ...
    function _checkApprovalByAssetDecimals(address _assetAddress, address _spender, uint256 _amount) internal  {
        uint256 stblDecimals = ERC20(_assetAddress).decimals();
        if(stblDecimals == 6){
            _checkTetherApprovals(address(_assetAddress), address(_spender), _amount );
        }else{
            ERC20(_assetAddress).safeApprove(address(_spender), _amount);
        }
    }
}
