...
/*
Let's plan on the following for drop dynamics:
For BlockChains:
4 rounds of increased pricing of baller bars per mint
4 baller bars for 2001-3000, 
8 baller bars for 3001-4000, 
16 baller bars for 4001-5000,
24 baller bars for 5001-6000

for gems:
5000 total
10 baller bars per gem

For Baller Chains
500 Chains possible
50 in top 10% leaderboard eligible for prize
*/
...
contract Chains is ERC721Enumerable, Ownable {
    ...
    uint256 MAX_MINTS_FOR_ETHER = 100;
    ...
    // address private _signerAddress = 0xCBf2a728bDFDb683573a88E1c359FE892f9De049; // Live 
    address private _signerAddress = 0x1324452B5e7b250148FA7C4d10bB550cdEd4a039; // Development
    ...
    function currentBallerBarsCost() public view returns (uint256) {
        ...
        if (_totalSupply <= MAX_MINTS_FOR_ETHER) return 1 ether;

        if (_totalSupply > MAX_MINTS_FOR_ETHER && _totalSupply <= 3000)
            return 4 ether;
        if (_totalSupply > 3000 && _totalSupply <= 4000)
            return 8 ether;
        if (_totalSupply > 4000 && _totalSupply <= 5000)
            return 16 ether;
        if (_totalSupply > 5000 && _totalSupply <= 6000)
            return 24 ether;
    ...
    function mintReserve(uint256 tokenQuantity) onlyOwner external  {
        for (uint256 i = 0; i < tokenQuantity; i++) {
            require(totalSupply() < 30); // Reserved for teams and giveaways
            mintInternal();
        }
    }
    ...
    function mintChainWithToken(uint256 tokenQuantity) public {        
        ...
        for (uint256 i = 0; i < tokenQuantity; i++) {
            IBallerBars(diamondsAddress).burnFrom(msg.sender, currentBallerBarsCost());
            mintInternal();
        }
    }
    ...
    function setBallerBarsAddress(address _diamondsAddress) public onlyOwner {
