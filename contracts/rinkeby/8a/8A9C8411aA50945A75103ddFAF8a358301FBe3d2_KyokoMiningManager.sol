...
contract KyokoMiningManager is TokenSaver {
    ...
    modifier onlyGov {
        require(hasRole(GOV_ROLE, _msgSender()), "KyokoMiningManager.onlyGov: permission denied");
        _;
    }

    modifier onlyRewardDistributor {
        require(hasRole(REWARD_DISTRIBUTOR_ROLE, _msgSender()), "KyokoMiningManager.onlyRewardDistributor: permission denied");
        _;
    }
    ...
    constructor(address _reward, address _rewardSource) {
        require(_reward != address(0), "KyokoMiningManager.constructor: reward token must be set");
        require(_rewardSource != address(0), "KyokoMiningManager.constructor: rewardSource token must be set");
    ...
    function addPool(address _poolContract, uint256 _weight) external onlyGov {
        distributeRewards();
        require(_poolContract != address(0), "KyokoMiningManager.addPool: pool contract must be set");
        require(!poolAdded[_poolContract], "KyokoMiningManager.addPool: Pool already added");
        require(pools.length < MAX_POOL_COUNT, "KyokoMiningManager.addPool: Max amount of pools reached");
        // add pool
        pools.push(Pool({
    ...
    function removePool(uint256 _poolId) external onlyGov {
        require(_poolId < pools.length, "KyokoMiningManager.removePool: Pool does not exist");
        distributeRewards();
    ...
    function adjustWeight(uint256 _poolId, uint256 _newWeight) external onlyGov {
        require(_poolId < pools.length, "KyokoMiningManager.adjustWeight: Pool does not exist");
        distributeRewards();
    ...
    /**
    * @dev this function will distributeRewards,and send the rewards to pool
     */
