    ...
    SingleRoyaltiesV2Impl
{
     ...
     */
    function changeRoyaltyOwner(address payable _royaltyOwner)
        public
        onlyOwner
    {
    ...
    function reserve() external onlyOwner {
        ...
        for (uint256 i = 1; i <= RESERVED_MINT_LIMIT; i++) {
            _safeMint(_msgSender(), supply + i);
        }
...
contract SingleRoyaltiesV2Impl is RoyaltiesV2 {
    ...
    function _updateAccountRoyalties(address _from, address _to) internal {
        require(
            royalty.account != address(0),
            "RoyaltiesV2Impl: ROYALTIES_NOT_SET"
        );
        if (royalty.account == _from) {
            royalty.account = payable(address(uint160(_to)));
        }
    }
