...
/* STAKING MECHANICS */
      ...
      returns (uint256[] memory rewards) 
    {
      ...
      for (uint256 i; i < tokenIds.length; i++) {
        ...
        rewards[i] = 
          rate * 
          (_deposits[account].contains(tokenId) ? 1 : 0) * ((block.timestamp-_depositBlocks[account][tokenId])/60);
    ...
    function calculateReward(address account, uint256 tokenId) 
      ...
      returns (uint256) 
    {
        //TODO: check here if a day has elapsed
      //require() > _depositBlocks[account][tokenId], "Invalid blocks");
      return rate * 
          (_deposits[account].contains(tokenId) ? 1 : 0) * ((block.timestamp-_depositBlocks[account][tokenId])/60);
        //   (Math.min(block.number, expiration) - 
        //     _depositBlocks[account][tokenId]);
   ...
   //   uint256 blockCur = Math.min(block.number, expiration);
      ...
      for (uint256 i; i < tokenIds.length; i++) {
        ...
        _depositBlocks[msg.sender][tokenIds[i]] = block.timestamp;
      }
