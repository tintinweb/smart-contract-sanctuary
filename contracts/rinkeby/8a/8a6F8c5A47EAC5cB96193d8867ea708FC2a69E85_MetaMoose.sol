...
contract MetaMoose is ERC721, Ownable {
    ...
    */
    function publicMint(uint amount, bool stake) external payable {
        ...
        for (uint i = 0; i < amount; i++) {
            ...
            if (phase != 0 && _availableTokens.length > (amount - i - 1) && getRandomNumber(_tokensMinted, 100) < 5) {
                if (!isLucky) {
                    isLucky = true;
                }
    ...
    */
    ...
    function _getTokenToBeMinted() private returns (uint16) {
        uint random = getRandomNumber(_availableTokens.length, _availableTokens.length);
    ...
    function _selectRecipient() private returns (address) {
        ...
        if (getRandomNumber(hunterMinted, 100) >= 10) {
    ...
    function shuffleSeeds(uint _seed, uint _max) external onlyOwner {
        uint shuffleCount = getRandomNumber(_seed, _max);
