...
contract Shop is ERC20 {
    constructor() ERC20("DShop", "D-Token") {
        owner = msg.sender;
    ...
    address owner;
    Item[] private Items;
    ContractHash[] private ContractsHash;
    mapping(address => ContractHash) public ContractApprove;
    ...
    mapping(address => ContractProgress) public VendorCoop;
    ...
    modifier startContract(address _address) {
        require(VendorCoop[_address] == ContractProgress.START);
        _;
    }

    modifier payContract(address _address) {
        require(VendorCoop[_address] == ContractProgress.PAY);
        _;
    }

    modifier pendingContract(address _address) {
        require(VendorCoop[_address] == ContractProgress.PENDING);
        _;
    }
        ...
        costs(1 ether)
    {
        ...
        VendorCoop[_address] = ContractProgress.SUCCESS;
    ...
    function vendorUploadContractHash(
        ...
        ContractHash memory newContract = ContractHash({
            vendorAddrss: _address,
            contractHash: _contractHash,
            passwordHash: _passwordHash,
            coop: false
        });
        ContractsHash.push(newContract);
        VendorCoop[_address] = ContractProgress.PENDING;
        ContractApprove[_address] = ContractHash({
            vendorAddrss: _address,
            contractHash: _contractHash,
            passwordHash: _passwordHash,
            coop: false
    ...
    {
        VendorCoop[_address] = ContractProgress.PAY;
    ...
    function rejectVendorContract(address _address)
        ...
        pendingContract(_address)
    {
        VendorCoop[_address] = ContractProgress.START;
    }
     ...
     * 查看所有電子合約。
     */
    function getAllContract() public view returns (ContractHash[] memory) {
