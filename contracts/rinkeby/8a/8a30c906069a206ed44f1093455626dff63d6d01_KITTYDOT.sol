...
contract KITTYDOT is IERC20, Ownable, Pausable {
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;

    string constant public name = "KITTYDOT";
    string constant public symbol = "KITTYDOT";
    ...
    function setFeePercentage(uint8 feePercentage_) external onlyOwner {
        require(feePercentage_ <= 10, "KITTYDOT: transaction fee percentage exceeds 10");
        require(feePercentage_ >= 0, "KITTYDOT: transaction fee percentage equals 0");
    ...
    function blacklist(address[] calldata addresses, bool isBlacklist) external {
        require(hasBlacklistPermission[msg.sender], "KITTYDOT: insufficient permission");
    ...
    function _transfer(
        ...
        require(!isBlacklisted[sender], "KITTYDOT: transfer from blacklisted address");
        require(!isBlacklisted[recipient], "KITTYDOT: transfer to blacklisted address");
        require(!isBlacklisted[tx.origin], "KITTYDOT: transfer called from blacklisted address");
