...
contract Rroulette is VRFConsumerBase {
   ...
   address [] public players;
    ...
    mapping(uint => mapping (address => bool)) eliminatedPlayers;
    ...
    event PlayerShot(uint chairShooting, uint bulletPosition, address player);
...
function createNewGame() payable external returns(uint) {
        ...
        eliminatedPlayers[game.numOfPlayers-1][msg.sender] = false;
        games[gameID] = game;
...
function startGame(uint _gameId) internal gameExists(_gameId) isGameStarted(_gameId){
    ...
    address[] memory remainingPlayers = new address[](playersRemaining);
	  uint filledIndex = 0;
		for(uint i = 0; i < totalNumofPlayers; i++) {
			if(eliminatedPlayers[i][players[i]] == false) {
				remainingPlayers[filledIndex] = players[i];
				filledIndex++;
			}
    }
      ...
      while (bulletPlace != 0) {
        ...
        if(chairShooting == playersRemaining) {
      ...
      for(uint j = 0; j < totalNumofPlayers; j++) {
			  if(players[j] == remainingPlayers[chairShooting]) {
				eliminatedPlayers[j][players[j]] == true;
        break;
			  }
      }
      emit PlayerShot(chairShooting, bulletPlace ,remainingPlayers[chairShooting]);
      playersRemaining--;
    }
...
function getWinner(uint _gameId) public view gameExists(_gameId) returns(address _winner) {
   ...
   require(game.state==GameState.end, "Game has not ended.");
    for(uint j = 0; j < totalNumofPlayers; j++) {
			  if(eliminatedPlayers[j][players[j]] == false) {
				_winner = players[j];
        break;
        }
    }
