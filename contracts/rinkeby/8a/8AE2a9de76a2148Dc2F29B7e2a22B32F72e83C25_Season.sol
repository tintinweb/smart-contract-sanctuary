...
contract Season is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getCardType(uint256 tokenId, uint256 offset) public view returns (string memory) {
    ...
    string seasonName = "Adventure Cards Expansion Packs";
    string seasonDesc =
        "Cards is an on chain, collectable card game, based on crypto primitives. Each Expansion Pack is 15 cards, a mix of rarities and types. The packs can be unbundled and used however you like.";
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (seedSet == false) {
            output = string(abi.encodePacked(topString, bottomString));
            json = Base64.encode(bytes(string(abi.encodePacked(prefix, Base64.encode(bytes(output)), '"}'))));
        ...
        json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        prefix,
                        Base64.encode(bytes(string(abi.encodePacked(topString, output, bottomString)))),
                        '"}'
                    )
                )
            )
