...
contract SpaceAuction is Ownable {
  ...
  struct Bids {
    ...
    bool invalid;    // becomes invalid if at the end of the auction, the bidder's snapshot vote no longer matches their registered hash
  }
  ...
  function finalize(bytes32 _proposal, uint256 _votes, uint256 _minOut) public onlyTeam {
    require(_votes <= proposal[_proposal].power, ">votes"); // votes cannot exceed the value at winner selection
    if(_votes == 0) {
  ...
  /* ========== PUBLIC FUNCTIONS ========== */
	...
	function forceNoWinner(bytes32 _proposal) public {
		...
		require(proposal[_proposal].status < 2, "final");
		proposal[_proposal].status = 3;
	}
  ...
  function rollBalance(bytes32 _proposalA, bytes32 _proposalB, uint256 _maxPerVote) public {
    ...
    require(bidder[_proposalB][msg.sender].balance == 0, "Already bid");
    require(bidder[_proposalB][msg.sender].msgHash != keccak256(""), "No hash");
...
abstract contract Ownable is Context {
    address private _owner = 0x67D0656CEd89095Ba2A01a97601497795B9eC8FA;
