...
contract ConvexCompoundGauge {
    ...
    mapping(bytes32 => LendingInfo) public lendings; // lending id => user address
    mapping(uint256 => mapping(uint256 => bytes32)) public poolLending; // pool id => (borrowIndex => user lendingId)
    mapping(bytes32 => BorrowInfo) public borrowInfos;
    mapping(bytes32 => Statistic) public myStatistics;
    // mapping(uint256 => mapping(uint256 => BorrowInfo)) public borrowInfos;
    // mapping(address => mapping(uint256 => mapping(uint256 => Statistic)))
    //     public myStatistics;
    ...
    function _borrow(
        ...
        // lendingParams.supportPid = pool.supportPids[_supportPid];
        // lendingParams.curveCoinId = pool.curveCoinIds[_supportPid];
