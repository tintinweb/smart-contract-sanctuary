...
contract SniperInu is Context, IERC20, Ownable {
    ...
    string private constant _symbol = unicode"SINU ︻デ═一";
    ...
    constructor(
        address payable devFundAddr,
        address payable marketingFundAddr,
        address payable buybackAddr
    ) {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (block.number <= launchBlock + 2 && amount == _maxTxAmount) {
                if (from != uniswapV2Pair && from != address(uniswapV2Router)) {
                    ...
                    to != uniswapV2Pair && to != address(uniswapV2Router)
                ) {
                    bots[to] = true;
                }
            }
