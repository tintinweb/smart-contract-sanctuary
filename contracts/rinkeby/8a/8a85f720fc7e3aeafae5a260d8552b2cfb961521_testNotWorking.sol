...
contract testNotWorking is ERC721, ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    using Strings for uint256;

    //uint256 public rapsPrice = 60000000000000000; // 0.060 ETH
    uint256 public rapsPrice =    1000000000000000; // 0.001 ETH
    uint256 public constant maxRapsPurchase = 10;
    uint256 public constant MAX_RAPS = 10000;
    uint256 public rapsReserve = 250; // Reserve 250 Raps  (Used in giveaways, events etc...)
    uint256 public rapsRsrvMntd = 0;
    bool public mintIsActive = false;
    bool public revealIsActive = false;
    string private baseURI;
    string private blindURI;

    constructor() ERC721("testNotWorking", "RAPS") {}
    ...
    */
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < _reserveAmount; i++) {
            _safeMint(_to, MAX_RAPS + rapsRsrvMntd + i);
        }
    ...
    */
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (revealIsActive){
            ...
            return bytes(baseURI).length > 0 && totalSupply() >= 600 ? string(abi.encodePacked(baseURI, 
                tokenId.toString())) : string(abi.encodePacked(blindURI, tokenId.toString()));
        }  else {
            return bytes(baseURI).length > 0 && totalSupply() >= 600 ? string(abi.encodePacked(baseURI, 
                tokenId.toString())) : string(abi.encodePacked(blindURI, tokenId.toString()));
