...
contract BorpaDAO is Ownable, ERC721A, ReentrancyGuard {
    ...
    function Mint(uint256 quantity) external payable callerIsUser {
        ...
        require(
            remainingFree() > quantity || msg.value >= (quantity - remainingFree()) * priceAfterFree,
            "Your quantity exceeds the number of free remaining or the Price you inputted does not account for the tokens minted after free"
        );
    ...
    function remainingFree() public view returns (uint256) {
        if(totalSupply() > amountForFree - 1){
            return 0;
        } else {
