...
import "./IGovPriceConsumerV3.sol";
import "../admin/admininterfaces/IGovWorldAdminRegistry.sol";
import "hardhat/console.sol";


contract GovPriceConsumerV3 is IGovPriceConsumerV3 {
    ...
    constructor(
        ...
        // require(_govWorldAdminRegistry != address(0), "GPCV3: Admin Contract Must be provided");
        // require(_stableFeedTokens.length > 0, "GPCV3: No tokens provided for price feed");
        // require(_chainLinkFeedContracts.length > 0, "GPCV3: No price feed chainlink contract address provided");
        // require(_stableFeedTokens.length == _chainLinkFeedContracts.length, "GPCV3: Price feed tokens and contracts should be of same length");
        // require(_enabled.length == _decimals.length, "GPV3: Length not match");
     ...
     modifier onlyPriceFeedTokenRole(address admin) {
        require(govWorldAdminRegistry.isAddTokenRole(admin),
            "GPCV3: No admin right to add price feed tokens.");
        _;
    }
    ...
    function _isAddedChainlinkFeedAddress(address _chainlinkFeedAddress)
        ...
        returns (bool)
    {
        for (uint256 i = 0; i < allFeedTokens.length; i++) {
            if (allFeedTokens[i] == _chainlinkFeedAddress) {
                return true;
            }
        }
        return false;
    ...
    */
    ...
    {
        require(!_isAddedChainlinkFeedAddress(_chainlinkFeedAddress), "GPCV3: already added price feed");
        usdPriceAggrigators[_tokenAddress] = ChainlinkDataFeed(AggregatorV3Interface(_chainlinkFeedAddress), _enabled, _decimals);
        allFeedTokens.push(_chainlinkFeedAddress);
    ...
    {   
        ...
        for(uint256 i = 0 ; i < _tokenAddress.length; i++){
            require(!_isAddedChainlinkFeedAddress(_chainlinkFeedAddress[i]), "GPCV3: already added price feed");
            this.addUsdPriceAggrigator(_tokenAddress[i], _chainlinkFeedAddress[i], _enabled[i], _decimals[i]);
            allFeedTokens.push(_chainlinkFeedAddress[i]);
        }
    ...
    */
    ...
    function getDexTokenPrice(
        ...
        IDexPair pair = IDexPair(IDexFactory(_dexFactory).getPair(_stable, _alt));
        ...
        if (pair.token0() == _stable) {
            ...
            return (_amount.mul(resD.div(res1))); // return amount of alt needed to buy 1 stable

        } else {

            uint256 resD = res1 * (10**token1Decimals);
            return (_amount.mul(resD.div(res0))); // return amount of stable needed to buy 1 altcoin
        }
    }
