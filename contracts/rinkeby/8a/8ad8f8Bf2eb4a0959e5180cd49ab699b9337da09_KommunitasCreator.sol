...
contract KommunitasVesting{
    ...
    address public tokenProject;
    address public senderProject;
    uint public tge; // Token Generation Event
    uint public sold;
    uint public distributed; // token received
    uint public completed; // token received in percent
    uint public period; // token received period in month
    ...
    function migratePurchase(address _buyer, uint _purchase) public onlyOwner {
        ...
        invoice[_buyer].claimedAll = false;
    }
    
    function distributeToken(uint _amount, uint _percent) public onlySenderProject {
        require(_amount != 0 && _percent != 0, "Amount & percent can't be 0");
        require(block.timestamp >= tge, "TGE haven't started yet");
        require(tokenProject != address(0), "Token project still address(0)");
        require(completed < 100, "You have distributed 100% token");
    ...
    function claimToken() public onlyBuyer(msg.sender) {
        require(block.timestamp >= tge, "TGE haven't started yet");
        require(!invoice[msg.sender].claimedAll, "You have claimed all");
        ...
        if(invoice[msg.sender].progress == 100){
            invoice[msg.sender].claimedAll = true;
        }
    }
    
    function setTokenProject(address _tokenProject) public onlyOwner {
        require(_tokenProject != address(0), "Can't set to address(0)");
        tokenProject = _tokenProject;
    }
    
    function setSenderProject(address _senderProject) public onlyOwner {
        require(_senderProject != address(0), "Can't set to address(0)");
        require(_senderProject != senderProject, "Already set");
...
contract KommunitasCreator is IKommunitasCreator{
    address[] public override allVestings;
