...
contract mightygojirascontract is ERC721, Ownable {
    ...
    uint public constant MAX_mightygojira_PURCHASE = 3;
    uint256 public  MAX_mightygojiraS = 9000;
    uint256 public maxGenCount = 3000;
    uint256 constant public FUSION_PRICE = 500 ether;
    ...
    // Reserve 300 mightygojira for team
    uint public mightygojiraReserve = 300;
    event mightyCityTypeidChange(address _by, uint _tokenId,  uint256 _internaltokenID);
    ...
    modifier mightygojiraOwner(uint256 mightygojiraId) {
        require(ownerOf(mightygojiraId) == msg.sender, "Cannot interact with a mightygojira you do not own");
        _;
    }
    ...
    function Reserve(address _to, uint256 _reserveAmount) public onlyOwner {        
        ...
        require(_reserveAmount > 0 && supply.add(_reserveAmount) <= MAX_mightygojiraS, "Not enough reserve left");
	...
	function breed(uint256 parent1, uint256 parent2) external {
        ...
        require(parent1 < maxGenCount && parent2 < maxGenCount,   "Cannot breed with baby mightygojira");
        require(parent1 != parent2,                               "Must select two unique parents");

		GOJ.burn(msg.sender, FUSION_PRICE);
    ...
    function mintmightygojiras(uint numberOfTokens) public payable {
        ...
        require(numberOfTokens > 0 && numberOfTokens <= MAX_mightygojira_PURCHASE, "Can only mint 3 mightygojira at a time");
        require(totalSupply().add(numberOfTokens) <= maxGenCount, "Purchase would exceed max supply of mightygojiras");
    ...
    function presalemightygojiras(uint numberOfTokens) public payable {
        require(PresaleIsActive, "Pre Sale must be active to mint mightygojira");
        require(numberOfTokens > 0 && numberOfTokens <= MAX_mightygojira_PURCHASE, "Can only mint 3 mightygojira at a time");
        require(totalSupply().add(numberOfTokens) <= maxGenCount, "Purchase would exceed max supply of mightygojiras");
        require(msg.value >= mightygojiraPrice.mul(numberOfTokens), "Ether value sent is not correct");
        require(minted[msg.sender] <= MAX_mightygojira_PURCHASE, "Max 3 mightygojira per wallet");
