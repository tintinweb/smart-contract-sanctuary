...
contract BlockchainOfFame is ERC721URIStorage, Ownable {
	...
	mapping (uint256 => string) private _tokenFont;
    ...
    event NameChange (uint256 indexed maskIndex, string newName, string newFont);
    ...
    function getTokenURI(uint256 tokenId) public view returns (string memory) {
     ...
     function mintBof(string memory newName, string memory newFont,string memory _tokenURI) public payable {
        ...
        require(max_mints_per_address[msg.sender].add(1) <= 10,"Max 10 mints per wallet allowed");
            ...
            if (tokenCounter < MAX_ELEMENTS) {
				...
				changeName(tokenCounter,newName,newFont);
    ...
    function reserveBOF(address _to, string memory newName, string memory newFont) public onlyOwner {        
        require(bofReserve >= 1, "Not enough reserve");
            supply = tokenCounter;
            _safeMint(_to, supply);
			changeName(supply,newName,newFont);
            bofReserve = bofReserve.sub(1);
            tokenCounter = tokenCounter + 1;
    }


    function changeNameAfterMint(uint256 tokenId, string memory newName, string memory newFont) public {
		address owner = ownerOf(tokenId);

        require(_msgSender() == owner, "ERC721: caller is not the owner");
		changeName(tokenId, newName, newFont);
	}

    function changeName(uint256 tokenId, string memory newName, string memory newFont) private {

        require(validateName(newName) == true, "Not a valid new name");
        require(sha256(bytes(newName)) != sha256(bytes(_tokenName[tokenId])), "New name is same as the current one");
        require(isNameReserved(newName) == false, "Name already reserved");
        require(sha256(bytes(_toLower(newFont))) == sha256(bytes('box')) || sha256(bytes(_toLower(newFont))) == sha256(bytes('script')),"Not an allowed font");
		...
		_tokenFont[tokenId] = newFont;
        emit NameChange(tokenId, newName, newFont);
    }
	...
	function tokenFontByIndex(uint256 index) public view returns (string memory) {
        return _tokenFont[index];
    }
