...
contract animalMain is  ERC721,Ownable {
  ...
  function seeAnimalRole(uint256 _tokenId) public view returns (uint16 roletype) {
    return animals[_tokenId].animal_type;
  }
  ...
  function createAnimal() public payable {
       ...
       // generate dna from random
        dna = bytes32(random());
       
        // generate star from random
        uint rand = uint(random()) % 100;
        if(rand < 40)
        {
            star = 1;
        }
        ...
        // generate animal type from random
        animal_type = uint(random()) % 6 + 1;
       
       
        uint id = animals.push(animal(dna, uint8(star), uint8(animal_type))) - 1;
