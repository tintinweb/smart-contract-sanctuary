...
contract GRPSBeneficiaryTokenVesting is Ownable {
  ...
  function addBeneficiaries(address[] memory _beneficiaryes, uint256[] memory _sharesAmounts) onlyOwner public {
    require(_beneficiaryes.length == _sharesAmounts.length, "Invalid params");
    
    for (uint i = 0; i <_beneficiaryes.length; i++) {
      addBeneficiary(_beneficiaryes[i], _sharesAmounts[i]);
    }

    require(totalShares() == BP, "Invalid shares amount");
  }

  function addBeneficiary(address _beneficiary, uint256 _sharesAmount) private {
    require(block.timestamp < lockupTime || (lockupTime == start && block.timestamp < start + period), "Invalid timing");
  ...
  function releasableAmount() public view returns (uint256) {
    ...
    if (block.timestamp < lockupTime) {
      ...
      if(!lockupReleased[msg.sender]) {
        result = totalBalance * percentUpfront / BP;
      }
      
      uint256 periodsPassed;

      if(block.timestamp >= start + duration) {
        periodsPassed = ((start + duration + (period - 1)) - lastReleaseDate[msg.sender]) / period;
      } else {
        periodsPassed = (block.timestamp - lastReleaseDate[msg.sender]) / period;
      }

      if (periodsPassed > 0) {
        result += totalBalance * (periodsPassed * percent) / BP;
      } 
  ...
  function calculateShares(uint256 _amount, address _beneficiary) public view returns (uint256) {
    return _amount * shares[_beneficiary] / BP;
  }
