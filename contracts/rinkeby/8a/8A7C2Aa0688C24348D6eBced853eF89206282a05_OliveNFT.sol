...
contract OliveNFT is ERC721Enumerable, Ownable {
    ...
    event welcomeToOlive(uint256 indexed id);
    event NewPriceEvent(uint256 price);
    event NewMaxElement(uint256 max);

    constructor(string memory baseURI) ERC721("Olive", "OliveNFT"){
        setBaseURI(baseURI);
    }
