...
contract DungCompost is LPTokenWrapper, Ownable {
    ...
    function apy() external view returns (uint) {
        ...
        if (lptoken.token0() == address(dung)) {
            ...
            LPperETH = totalSupplyLP/reserve1;
        } else {
            dungPrice = (uint256)(reserve0) * 1 ether/reserve1;
            LPperETH = totalSupplyLP/reserve0;
        }

        uint stakedLPinETH = stakedLP/LPperETH;

        uint earnedDungPerYear = rewardRate * 356 days;
        uint earnedEthPerYear = earnedDungPerYear*dungPrice;
        uint earnerEthPerYearForStakedETH = earnedEthPerYear / stakedLPinETH;
