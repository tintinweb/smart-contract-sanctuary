...
contract SQ19 is Ownable, ERC721A, ReentrancyGuard {
    ...
    ) ERC721A(_name, _symbol, collectionSize_) {
        baseURI = _uri;
        currentBatchSize = maxBatchSize_;
    ...
    _;
       ...
       function batchSize() internal view virtual override(ERC721A) returns (uint256) {

     return currentBatchSize;
    }

    function setBatchSize(uint256 currentBatchSize_) public onlyOwner {
        currentBatchSize = currentBatchSize_;
    }
    ...
    function mint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable callerIsUser  {
        ...
        uint256 actualCost = (sale == Sale.WHITELIST) ? cost : cost;
        ...
        if (msg.sender != owner()) {
            ...
            require(_mintAmount <= currentBatchSize, "Too many items minted");
            if (sale == Sale.WHITELIST) {
                ...
                require(
                    ownerMintedCount + _mintAmount <= currentBatchSize,
  ...
  IERC721Enumerable
{
  ...
  /**
   * @dev allow varying BatchSize
   */
function batchSize() internal virtual returns (uint256){
return maxBatchSize;
}
  /**
