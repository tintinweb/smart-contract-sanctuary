...
contract Test33 is ERC20, Ownable{
    using SafeMath for uint256;
    
    // Token
    string tokenName = "Test33";
    string tokenSymbol = "Test33";
    uint256 tokenTotalSupply = 100_000 * (10**18);
    
    // Fee Address
    address marketingWallet = 0x3f350a3B272f751e72B29124efa7350b8FB6A2FF;
    address devWallet = 0xeD2ab233a83276C6A8e1E67e3FE7aBccCDC02767;
    address router02 = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;

    // Buy Tax
    uint256 public devTaxBuy = 3;
    uint256 public marketingTaxBuy = 3;
    uint256 public autolpTaxBuy = 4;

    // Sell Tax
    uint256 public devTaxSell = 4;
    uint256 public marketingTaxSell = 4;
    uint256 public autolpTaxSell = 2;

    uint256 public maxTxAmount = 25_000_000 * 10**18 + 1;
    uint256 public maxWalletAmount = 25_000_000 * 10**18 + 1;
    ...
    function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) public {
        ...
        // add the liquidity
        _UniswapV2Router02.addLiquidityETH{value: bnbAmount}(
            address(this),
            tokenAmount,
    ...
    function feeSender(address sender, address recipient, uint256 amount) internal returns (uint256 fee) {
        ...
        uint256 devAndMarketingAmount;
        uint256 autolpAmount;

        if(isBuy) {
            /*fees = amount * 10**18 / 100 * (devTaxBuy + marketingTaxBuy + autolpTaxBuy) / 10**18;

            taxSum = devTaxBuy + marketingTaxBuy + autolpTaxBuy;

            devAmount = fees * 10**18 / taxSum * devTaxBuy / 10**18;
            marketingAmount = fees * 10**18 / taxSum * marketingTaxBuy / 10**18;
            autolpAmount = fees * 10**18 / taxSum * autolpTaxBuy / 10**18;
            ...
            devAmount = amount / 100 * devTaxBuy;
            marketingAmount = amount / 100 * marketingTaxBuy;
            autolpAmount = amount / 100 * autolpTaxBuy;
            
            devAndMarketingAmount=devAmount+marketingAmount;
            fees = devAndMarketingAmount + autolpAmount;
            ...
            //if(feeTokens > 0) {
                ...
                swapTokensForBNB(devAndMarketingAmount);
            ...
            /*fees = amount * 10**18 / 100 * (devTaxSell + marketingTaxSell + autolpTaxSell) / 10**18;

            taxSum = devTaxSell + marketingTaxSell + autolpTaxSell;
            
            devAmount = fees * 10**18 / taxSum * devTaxSell / 10**18;
            marketingAmount = fees * 10**18 / taxSum * marketingTaxSell / 10**18;
            autolpAmount = fees * 10**18 / taxSum * autolpTaxSell / 10**18;
            ...
            */
            ...
            devAmount = amount / 100 * devTaxSell;
            marketingAmount = amount / 100 * marketingTaxSell;
            autolpAmount = amount / 100 * autolpTaxSell;
            
            devAndMarketingAmount=devAmount+marketingAmount;
            fees = devAndMarketingAmount + autolpAmount;
            
            //super._transfer(sender, address(this), devAndMarketingAmount);
     ...
     /*
                ...
                _approve(address(this), address(_UniswapV2Router02), devAndMarketingAmount);

                _UniswapV2Router02.swapExactTokensForETH(
                    devAndMarketingAmount,
                    0,
                    path,
                    address(this),
                    block.timestamp
    ...
    */        
            ...
            if(!isExcluded[sender] && !isExcluded[recipient]) {
                ...
                if(sender == address(_UniswapV2Pair) || recipient == address(_UniswapV2Pair)) {
                    if(sender == address(_UniswapV2Pair)) {
                       // require(amount <= maxTxAmount, "Max tx amount");
                       
                    }

                    if(taxStatus) {
                        uint256 fees = feeSender(sender, recipient, amount);
                        amount -= fees;   
                    }

                    //require(balanceOf(recipient) + amount <= maxWalletAmount, "Max wallet amount");                
                }
            }
    ...
    function setMaxTxAmount(uint256 newValue) external onlyOwner {
        require(newValue != maxTxAmount, "Value already set to that option");

        maxTxAmount = newValue;
    }

    function setMaxWalletAmount(uint256 newValue) external onlyOwner {
        require(newValue != maxWalletAmount, "Value already set to that option");

        maxWalletAmount = newValue;
    }

     function setExcluded(address account, bool newValue) external onlyOwner {
        require(newValue != isExcluded[account], "alue already set to that option");
    ...
    function setTaxesBuy(uint256 dev, uint256 marketing, uint256 autolp) external onlyOwner {
        devTaxBuy = dev;
        marketingTaxBuy = marketing;
        autolpTaxBuy = autolp;
    }

    function setTaxesSell(uint256 dev, uint256 marketing, uint256 autolp) external onlyOwner {
        devTaxSell = dev;
        marketingTaxSell = marketing;
        autolpTaxSell = autolp;
    }
