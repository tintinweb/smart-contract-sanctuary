...
contract TicketVariantVRF721NFT is ERC721, Ownable {
  ...
  function mint(uint256[] memory _mintIds) external {
    ...
    for(uint256 i = 0; i < _mintIds.length; i++) {
      ...
      _mint(msg.sender, ticketIdsMemory[i]);
    }
    buyerToRedeemedTicketCountMemory += _mintIds.length;
  }
  ...
  function initiateRandomDistribution() external {
    ...
    uint256 supply = _tokenIds.current();
    require(supply > 0, "TicketVariantVRF721NFT::beginReveal: supply must be more than 0");
