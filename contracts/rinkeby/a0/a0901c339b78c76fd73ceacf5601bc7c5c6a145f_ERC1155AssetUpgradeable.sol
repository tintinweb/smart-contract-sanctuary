...
import "../deps/ProxyRegistry.sol";

contract ERC1155AssetUpgradeable is
    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public openMintTime;

    mapping(uint256 => uint256) internal currentSupply;
    mapping(uint256 => string) internal _tokenURI;
    mapping(uint256 => address) internal idCreators;

    mapping(address => bool) public managers;
    mapping(address => bool) public musiciansWhiteList;

    event CreatorChanged(uint256 indexed _id, address indexed _creator);
    event SetMusician(address indexed _musician, bool flag);
    event SetManager(address indexed _manager, bool flag);
    event SetOpenTime(uint256 openTime);

    modifier onlyCreatorOrProxy(uint256 _id) {
        require(
            _isCreatorOrProxy(_id, _msgSender()),
            "ERC1155Asset#onlyCreatorOrProxy: ONLY_CREATOR_ALLOWED"
        );
        _;
    }

    // contract owner. managers.
    modifier onlyOwnerOrProxy() {
        require(
            owner() == _msgSender() || _isProxyForUser(owner(), _msgSender()),
            "ERC1155Asset#onlyOwnerOrProxy: CALLER_IS_NOT_OWNER"
        );
        _;
    }
    ...
    function initialize(
        ...
        __ERC1155_init(templateURI);

        name = _name;
        symbol = _symbol;
        proxyRegistryAddress = _proxyRegistryAddress;
        openMintTime = _openTime;
    }
    ...
    {
        require(_exists(_id), "ERC1155Asset#uri: NONEXISTENT_TOKEN");
    ...
    function create(
        ...
        require(!_exists(_id), "ERC1155Asset#create: token _id already exists");
    ...
    // *************** token operation. change status and public **************************//
    function safeTransferFrom(
        ...
        uint256 mintedBalance = ERC1155Upgradeable.balanceOf(_from, _id);
    ...
    function safeBatchTransferFrom(
        ...
        for (uint256 i = 1; i < _ids.length; i++) {
            require(
                ...
                "ERC1155Asset#safeBatchTransferFrom: MIXED_EXISTENCE_NOT_ALLOWED"
            );
        }
        if (isMinting) {
    ...
    {
        if (_proxyRegistry(_account, _operator)) {
            return true;
        }
    ...
    // *********************** set params and config of contract. onlyOwner and external  **************************//
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < _musicians.length; i++) {
            require(_musicians[i] != address(0), "_musician is zero address");
            musiciansWhiteList[_musicians[i]] = flag;
            emit SetMusician(_musicians[i], flag);
        }
    }
    ...
    function setOpenTime(uint256 _openTime) external onlyOwner {
        require(
            ...
            "ERC1155Asset#setOpenTime: require block.timestamp < openMintTime"
        );
        openMintTime = _openTime;
        emit SetOpenTime(openMintTime);
    }
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < _managers.length; i++) {
            ...
            managers[_managers[i]] = flag;
            emit SetManager(_managers[i], flag);
        }
    }

    // *********************** set config of token.  onlyCreatorOrProxy and external  **************************//
    function setURI(uint256 _id, string memory _uri)
        external
        onlyCreatorOrProxy(_id)
    {
        _setURI(_id, _uri);
    }

    function setCreatorForIds(address _to, uint256[] memory _ids) external {
        require(
            _to != address(0),
            "ERC1155Asset#setCreatorForIds: INVALID_ADDRESS."
        );
        for (uint256 i = 0; i < _ids.length; i++) {
    ...
    // *********************** set config of token.  onlyCreatorOrProxy and internal  **************************//
    ...
    // ********************************** query status of token. and public ************************************//
    function balanceOf(address _owner, uint256 _id)
        ...
        returns (uint256)
    {
        uint256 balance = ERC1155Upgradeable.balanceOf(_owner, _id);
        return
    ...
    // ********************************** operation authority . and internal ************************************//
    function _requireMintable(
        ...
        require(
            ...
            "ERC1155Asset#_requireMintable: ONLY_CREATOR_ALLOWED"
        );
        require(
            _remainingSupply(_id) >= _amount,
            "ERC1155Asset#_requireMintable: SUPPLY_EXCEEDED"
        );
        require(
            musiciansWhiteList[_address] || block.timestamp > openMintTime,
            "ERC1155Asset#_requireMintable: WAIT_OPEN_TIME"
        );
    }

    function _remainingSupply(uint256 _id) internal view returns (uint256) {
        return maxSupply(_id) - currentSupply[_id];
    }

    function _exists(uint256 _id) internal view returns (bool) {
        return currentSupply[_id] > 0;
    }
    ...
    function _isProxyForUser(address _user, address _address)
        ...
        returns (bool)
    {
        if (managers[_address] || _proxyRegistry(_user, _address)) {
            return true;
        }
        return false;
    }

    function _proxyRegistry(address _user, address _address)
        internal
        view
        returns (bool)
    {
