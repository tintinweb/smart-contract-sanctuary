...
contract LootAlchemyV2 is ERC721Enumerable, ReentrancyGuard {

    IERC721 constant LOOT = IERC721(0xFF9C1b15B16263C61d017ee9F65C50e4AE0113D7);
    IERC721 constant CHEMVERSE = IERC721(0x25a9c026e7Ef116a00892fB6a04220AD2179E4E8);
    
    constructor() ERC721("LootAlchemyV2", "LOOTA2") {
    }
    
    string[] private vessels = [
        "Alembic",
        "Retort",
        "Aludel",
        "Crucible",
    ...
    function getVessel(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "VESSEL", vessels);
    }
    
    function getHeat(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "HEAT", heat);
    }
    ...
    function getReactor(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "REACTOR", reactors);
    }
    
    function getComponent(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "COMPONENT", components);
    }
    ...
    function pluck(uint256 tokenId, string memory keyPrefix, string[] memory sourceArray) internal view returns (string memory) {
        ...
        if (greatness >= 19) {
            ...
            name[0] = itemPrefixes[rand % itemPrefixes.length];
            name[1] = itemSuffixes[rand % itemSuffixes.length];
            if (greatness == 19) {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[3] = getHeat(tokenId);

        parts[4] = '</text><text x="10" y="60" class="base">';

        parts[5] = getInstrument(tokenId);

        parts[6] = '</text><text x="10" y="80" class="base">';

        parts[7] = getTool(tokenId);

        parts[8] = '</text><text x="10" y="100" class="base">';

        parts[9] = getReactor(tokenId);
