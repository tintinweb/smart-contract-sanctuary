...
contract PolyversePass is ERC721, ERC721Enumerable, ERC721Burnable, ReentrancyGuard, Ownable {
    ...
    constructor() ERC721("PolyversePass", "POLYPASS")
    {
    ...
    uint256 public price = 0.3333 ether; // Pre-pay 3 PolyPixos
    ...
    function mint() external nonReentrant payable {
        ...
        require(whitelist[msg.sender] >= 1, "Not enough allowance");
    ...
    function addWhitelists(address[] calldata addresses, uint256 tokenAmount) onlyOwner external {
        for (uint256 i = 0; i < addresses.length; i++) {
            whitelist[addresses[i]] = tokenAmount;
        }
    }

    function addFreeMintWhitelist(address[] calldata addresses, bool flag) onlyOwner external {
        for (uint256 i = 0; i < addresses.length; i++) {
            freeMintWhitelist[addresses[i]] = flag;
        }
    }
