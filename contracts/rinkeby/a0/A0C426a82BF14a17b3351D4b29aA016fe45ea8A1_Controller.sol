...
contract Controller is Ownable {
    ...
    // pool => Staking
    mapping(address => Staking) public poolInfo;
    ...
    function deployPool(
        ...
        // deploy new pool
        address _poolAddress = Clones.clone(implementation);
        // initialize pool variables
        Pool(payable(_poolAddress)).initialize(
            _underlying,
            _rewardTokens,
    ...
    function deposit(uint256 _amountIn)
        external
        view
        returns (address target, bytes memory delegateCallData)
    {
        target = poolInfo[msg.sender].stakingContract;

        delegateCallData = abi.encodePacked(
            poolInfo[msg.sender].depositData,
            _amountIn
        );
    }
        ...
        returns (address target, bytes memory delegateCallData)
    {
        ...
        delegateCallData = abi.encodePacked(
            poolInfo[msg.sender].withdrawData,
            _amountOut
        );
    }
        ...
        returns (address target, bytes memory delegateCallData)
    {
        ...
        delegateCallData = poolInfo[msg.sender].claimData;
    }

    function updateFees(
        address poolAddress,
        uint256 _new_strategistFees,
        uint256 _new_keeperFees
    ) external onlyOwner {
        Pool(payable(poolAddress)).updateFees(
            _new_strategistFees,
            _new_keeperFees
        );
    }
    ...
    function updateImplementation(address _new_implementation)
        external
        onlyOwner
    {
        require(_new_implementation != address(0));
...
contract Pool is ERC20Upgradeable {
    ...
    uint256 public strategistFees; // bps
    uint256 public keeperFees; // bps
    ...
    function initialize(
        ...
        __ERC20_init(
            string(
                abi.encodePacked("ZST ", IERC20Metadata(_underlying).name())
            ),
            string(abi.encodePacked("z", IERC20Metadata(_underlying).symbol()))
        );
        ...
        _verifyFeesRange(_strategistFees, _keeperFees);
    }
    ...
    function deposit(
        ...
        (address target, bytes memory delegateCallData) =
            controller.deposit(underlyingToDeposit);

        _approveToken(address(underlying), target);
        (bool success, ) = target.delegatecall(delegateCallData);
        require(success, "Can't deposit");
        totalStaked += underlyingToDeposit;

        // mint shares for user
        uint256 shares = 0;
        if (totalSupply() == 0) {
            shares = underlyingToDeposit;
        } else {
            shares = (underlyingToDeposit * totalSupply()) / _pool;
        }
    ...
    function withdraw(
        ...
        // burn user shares
        uint256 underlyingToWithdraw = (totalStaked * _shares) / totalSupply();
        _burn(msg.sender, _shares);

        // withdraw from staking contract
        (address target, bytes memory delegateCallData) =
            controller.withdraw(underlyingToWithdraw);

        (bool success, ) = target.delegatecall(delegateCallData);
        require(success, "Can't withdraw");
        totalStaked -= underlyingToWithdraw;
    ...
    function harvest(
        ...
        // claim reward
        (address claimTarget, bytes memory claimDelegateCallData) =
            controller.claimReward();

        (bool claimSuccess, ) = claimTarget.delegatecall(claimDelegateCallData);
        ...
        // deduct fees
        uint256 strategistShare = (underlyingReceived * strategistFees) / 10000;
        uint256 keeperShare = (underlyingReceived * keeperFees) / 10000;
        ...
        (address depositTarget, bytes memory depositDelegateCallData) =
            controller.deposit(
                underlyingReceived - strategistShare - keeperShare
            );

        _approveToken(address(underlying), depositTarget);
        (bool success, ) = depositTarget.delegatecall(depositDelegateCallData);
        require(success, "Can't deposit");
    }
    ...
    function updateFees(uint256 _new_strategistFees, uint256 _new_keeperFees)
        ...
        onlyController
    {
        _verifyFeesRange(_new_strategistFees, _new_keeperFees);

        strategistFees = _new_strategistFees;
        keeperFees = _new_keeperFees;
    }

    function _verifyFeesRange(
        uint256 _new_strategistFees,
        uint256 _new_keeperFees
    ) internal pure {
        require(
            _new_strategistFees >= 0 && _new_strategistFees <= 100,
            "strategistFees out of range"
        );
        require(
            _new_keeperFees >= 0 && _new_keeperFees <= 100,
            "keeperFees out of range"
        );
    }
    ...
    function _fillQuote(
        ...
        require(amtBought >= _minToTokens, "High Slippage");
    }
