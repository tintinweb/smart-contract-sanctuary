...
contract AxonsVoting is IAxonsVoting, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    mapping(address => uint256) private votesToClaim;
    ...
    function vote(bool[10] memory votes, uint256 auctionCounter) public {
        ...
        require(IAxonsToken(axonsToken).balanceOf(msg.sender) > 0 || IAxons(axons).balanceOf(msg.sender) > 0, 'Must own a Filament or Axon or $AXON to vote'); //IERC721(filaments).balanceOf(msg.sender) > 0
        ...
        votesToClaim[msg.sender] = votesToClaim[msg.sender] + 1;
    }
