...
contract sleeperHitsVolume1 is ERC721Enumerable {
    ...
    mapping(uint256 => uint256) usedMembershipToken;
    
    event NftBought(address indexed, uint256 tokenId, uint256 memberShipTokenId);
    
    constructor(address _tokenAddress,string memory _uri) ERC721("Sleeper Hits Volume 1", "SHV"){
    ...
    function buyNft(uint256 _count,uint256[] memory tokenId) public payable {
        require(totalSupply() + _count <= maxSupply,"ERROR: mac limit reached");
        ...
        for(uint256 i=0; i<tokenId.length; i++){
            ...
            if(usedMembershipToken[tokenId[i]] != 0) require(usedMembershipToken[tokenId[i]] >= lockedTime ,"ERROR: this Membership Token is already used");
        }
        ...
        for(uint256 j=0; j< _count; j++){
            ...
            usedMembershipToken[tokenId[j]] = block.timestamp;
