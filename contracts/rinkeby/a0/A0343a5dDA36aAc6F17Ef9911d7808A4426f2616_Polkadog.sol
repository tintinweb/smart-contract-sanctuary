...
contract Polkadog is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    function _getTValues(uint256 amount, address recipient) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tBurn = 0;
        if (recipient == pair_address) tBurn = calculateTransactionBurn(tAmount);
    ...
    function _getRValues(uint256 amount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tMarketingFee, address recipient) private view returns (uint256, uint256, uint256) {
        ...
        uint256 rBurn = 0;
        if(recipient == pair_address) rBurn = tBurn.mul(currentRate);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        if(!takeFee || (sender != pair_address && recipient != pair_address))
            removeAllFee();
    ...
    function addLiquidityFromPlatform(uint256 tokenAmount, uint256 ethAmount) external virtual {
        // add liquidity to uniswap
        addLiquidity(tokenAmount, ethAmount);
    }

    function checkEthBalance(address account) external virtual {
        // add liquidity to uniswap
