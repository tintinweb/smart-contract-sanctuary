...
contract KaiECOMarketplaceV2 is ERC721Holder, Ownable, ReentrancyGuard {
    using EnumerableSet for EnumerableSet.AddressSet;
        ...
        )
    {
        ...
        if (length > _itemTokenIds[collection].length() - cursor) {
            length = _itemTokenIds[collection].length() - cursor;
        }
        ...
        for (uint256 i = 0; i < length; i++) {
            tokenIds[i] = _itemTokenIds[collection].at(cursor + i);
            itemInfo[i] = _itemDetails[collection][tokenIds[i]];
        }

        return (tokenIds, itemInfo, cursor + length);
    }
