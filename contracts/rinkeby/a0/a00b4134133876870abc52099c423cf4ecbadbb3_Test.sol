...
contract Test is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("Testing", "TEST") {}
     ...
     * @param _uri String representing metadata URI.
     */
    ...
    /**
     * @notice This is an external function which should be called from owner account.
     * Its purpose is to add Token URI in case it is not updated at the time of minting NFT.
     * @param _tokenId Id for which we want URI.
     * @param _uri String representing metadata URI.
     */
    function setTokenUri(uint256 _tokenId, string memory _uri)
        ...
        onlyOwner
    {
        require(bytes(_tokenURIs[_tokenId]).length > 0, "URI already updated");
        _setTokenUri(_tokenId, _uri);
    }

    function setDragonContractAddress(address contractAddress)
        public
        onlyOwner
    {
    ...
    function mint(uint256 numberOfTokens, string[] memory _uri) public payable {
        ...
        require(
            ...
            "Can only mint up to 10 per purchase"
        );
        require(
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            _setTokenUri(totalSupply(), _uri[i]);
    ...
    function bet(uint256 useTokenId1) public {
        require(
            dragonContract != address(0),
            "Dragon contract address need be set"
        );

        address from = msg.sender;
        require(
            ERC721.ownerOf(useTokenId1) == from,
            "ERC721: use of token1 that is not own"
        );
    ...
    function compose(
        ...
        require(
            ERC721.ownerOf(useTokenId2) == from,
            "ERC721: use of token2 that is not own"
        );
        require(
            ERC721.ownerOf(useTokenId3) == from,
            "ERC721: use of token3 that is not own"
        );
