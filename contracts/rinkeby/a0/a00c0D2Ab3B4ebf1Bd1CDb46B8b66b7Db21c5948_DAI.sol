...
contract DAI is owned, ReentrancyGuard{
    ...
    uint256 public totalSupply = 1000000000e6; // supply of GFB
	...
	require(to != address(0), "GFBD: INVALID_ADDRESS");
	require(amount > 0, "GFBD: INVALID AMOUNT");
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public {
        ...
        require(block.timestamp <= expiry, "GFB::delegateBySig: signature expired");
    ...
    function addMinter(address _addMinter) public onlyOwner returns (bool) {
        require(_addMinter != address(0), "GFBD: _addMinter is the zero address");
        return EnumerableSet.add(_minters, _addMinter);
    }

    function delMinter(address _delMinter) public onlyOwner returns (bool) {
        require(_delMinter != address(0), "GFBD: _delMinter is the zero address");
    ...
    function getMinter(uint256 _index) public view onlyOwner returns (address){
        require(_index <= getMinterLength() - 1, "GFBD: index out of bounds");
        return EnumerableSet.at(_minters, _index);
    }

    // modifier for mint function
    modifier onlyMinter() {
        require(isMinter(msg.sender), "GFBD: caller is not the minter");
        _;
    }
}
