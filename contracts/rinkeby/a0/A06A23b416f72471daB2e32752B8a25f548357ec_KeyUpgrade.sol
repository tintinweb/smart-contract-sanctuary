...
contract KeyUpgrade is Ownable {
  ...
  uint256 STELLAR_KEY_ID = 5;
  uint256 DATA_KEY_ID = 27;
  ...
  function transferWatcher(uint256 _raribleTokenId, uint256 _amount) public {
    ...
    if (_isERC721) {
      ...
      RaribleERC1151 raribleERC1151 = RaribleERC1151(_raribleContract);

      require(raribleERC1151.isApprovedForAll(msg.sender, address(this)) == true, "Contract is not authorized");
      require(raribleERC1151.balanceOf(msg.sender, _raribleTokenId) >= _amount, "User does not own this quantity of NFTs");

      raribleERC1151.burn(msg.sender, _raribleTokenId, _amount);
    }
...
abstract contract RaribleERC1151 {
  function isApprovedForAll(address _owner, address _operator) virtual public view returns (bool);
