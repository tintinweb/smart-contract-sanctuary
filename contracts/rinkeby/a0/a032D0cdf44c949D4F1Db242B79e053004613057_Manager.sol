...
contract Manager is Ownable, IERC721Receiver {
    ...
    function addTokensToStake(address account, uint16[] calldata tokenIds) external {
        require(account == msg.sender || msg.sender == address(token), "You do not have a permission to do that");
    ...
    function _stakeGoldMiners(address account, uint16 tokenId) internal whenNotPaused _updateEarnings {
        ...
        goldMinerIndices[tokenId] = goldMinerStake[account].length; // Store the location of the wolf in the Pack
    ...
    function _stakePirates(address account, uint16 tokenId) internal {
        ...
        pirateIndices[tokenId] = pirateStake[account].length; // Store the location of the wolf in the Pack
    ...
    function _claimFromMiner(uint16 tokenId, bool unstake) internal returns (uint owed) {
        ...
        if (unstake) {
            ...
            goldMinerStake[msg.sender][goldMinerIndices[tokenId]] = lastStake; // Shuffle last Wolf to current position
            goldMinerIndices[lastStake.tokenId] = goldMinerIndices[tokenId];
            goldMinerStake[msg.sender].pop(); // Remove duplicate
            delete goldMinerIndices[tokenId]; // Delete old mapping

            token.safeTransferFrom(address(this), msg.sender, tokenId, ""); // send back Sheep
        } else {
            _payTax((owed * TAX_PERCENTAGE) / 100); // percentage tax to staked wolves
            owed = (owed * (100 - TAX_PERCENTAGE)) / 100; // remainder goes to Sheep owner
    ...
    function _claimFromPirate(uint16 tokenId, bool unstake) internal returns (uint owed) {
        ...
        require(stake.owner == msg.sender, "This NTF does not belong to address");
        owed = (pirateReward - stake.value); // Calculate portion of tokens based on Alpha
        ...
        if (unstake) {
            ...
            pirateStake[msg.sender][pirateIndices[tokenId]] = lastStake; // Shuffle last Wolf to current position
            pirateIndices[lastStake.tokenId] = pirateIndices[tokenId];
            pirateStake[msg.sender].pop(); // Remove duplicate
            delete pirateIndices[tokenId]; // Delete old mapping
    ...
    function _payTax(uint _amount) internal {
        if (totalPirateStaked == 0) {
            ...
            unaccountedRewards += _amount; // keep track of $GGOLD due to wolves
            return;
        }
        // makes sure to include any unaccounted $GGOLD
