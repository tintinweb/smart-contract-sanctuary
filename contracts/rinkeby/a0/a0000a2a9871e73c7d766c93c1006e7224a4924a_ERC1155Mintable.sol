...
contract ERC1155Mintable is ERC1155 {
    ...
    constructor(uint256 _fee,address _token,address _feeTo) public{
        token = _token;
        fee = _fee.mul(uint256(10) ** (IERC20(token).decimals()));
        feeTo = _feeTo;
    }
    ...
    function create(uint256 _initialSupply, string calldata _uri,address _from) external returns(uint256 _id) {
        
        IERC20(token).transferFrom(_from, feeTo, fee);
