...
contract CryptoSquatches is ERC721, Ownable {
    ...
    constructor() ERC721("Crypto Squatches", "CSQUATCH") {
        MAX_CRYPTO_SQUATCH_SUPPLY = 100;
        REVEAL_TIMESTAMP = block.timestamp + 50; //(86400 * 7);
    ...
    */
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        require(startingIndex != 0, "startingIndex must be set before Squatches are revealed");

        string memory base = baseURI();

        return string(abi.encodePacked(base, ((tokenId + startingIndex) % MAX_CRYPTO_SQUATCH_SUPPLY)));
    }
