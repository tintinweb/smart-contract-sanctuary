...
contract PerpetualTradeLogic is PerpetualUpdateFunctions, IFunctionList, IPerpetualTradeLogic {
    ...
    function executeTrade(
        ...
        int128 _fDeltaLockedValue,
        bool _isClose
    ) external override virtual returns (int128) {
        PerpetualData storage perpetual = _getPerpetual(_iPerpetualId);
        return _executeTrade(perpetual, _traderAddr, _fTraderPos, _fTradeAmount, _fDeltaLockedValue, _isClose);
    }
    ...
    function preTrade(
        ...
        int128 _fLimitPrice,
        uint32 _flags
    ) external virtual override returns (int128, int128) {
        PerpetualData storage perpetual = _getPerpetual(_iPerpetualId);
        int128 fPrice;
        int128 fAmount;
        (fPrice, fAmount) = _preTrade(perpetual, _traderAddr, _fAmount, _fLimitPrice, _flags);
    ...
    function distributeFees(
        ...
        int128 _fDeltaPositionCC,
        bool _hasOpened
    ) external override virtual returns (int256 lpFee, int256 totalFee) {
        PerpetualData storage perpetual = _getPerpetual(_iPerpetualId);
        return _distributeFees(perpetual, _traderAddr, _referrerAddr, _fDeltaPositionCC, _hasOpened);
    }
     ...
     * @param   _fDeltaLockedValue  Amount*price (quote currency)
     * @param   _isClose            true if trade (partially) closes position
     * @return  isClose * 'Profit & Loss on traded amount'
     */
    function _executeTrade(
        ...
        int128 _fTraderPos,
        int128 _fTradeAmount,
        int128 _fDeltaLockedValue, // this is dL = new_price * delta_pos when this function is called
        bool _isClose
        ...
        if (_isClose) {
            ...
            int128 fPnL = fAvgPrice.mul(_fTradeAmount).sub(_fDeltaLockedValue);
            ...
            _fDeltaLockedValue = _fDeltaLockedValue.add(fPnL);
            // equivalently, L <- L * new position / old position, 
            // i.e. if we are selling 10%, then the new locked in value is the 90% remaining
            int128 fFXrate = _getCollateralToQuoteConversionMultiplier(_perpetual);
            fDeltaCashCC = fPnL.div(fFXrate);
        }

        // execute trade: update margin, position, and open interest:
        _updateMargin(_perpetual, address(this), _fTradeAmount.neg(), fDeltaCashCC.neg(), _fDeltaLockedValue.neg());
        _updateMargin(_perpetual, _traderAddr, _fTradeAmount, fDeltaCashCC, _fDeltaLockedValue);
        if (!_isClose) {
    ...
    function _preTrade(
        ...
        int128 fTraderPos = _getTraderPosition(_perpetual, _traderAddr);
        // don't leave dust. If the resulting position is smaller than a lot-size,
        // we close the position
        bool closePos = !_hasTheSameSign(fTraderPos, _fAmount) &&
            fTraderPos.add(_fAmount).abs()<=_perpetual.fLotSizeBC;
        if (closePos) {
     ...
     * @param _fTraderExposureEMA    Current trader exposure EMA (perpetual.fCurrentAMMExposureEMA)
     * @param _fBumpUp               How much do we allow to increase the trade size above the current
     *                               fCurrentAMMExposureEMA? (e.g. 1.25 for a 25% increase)
     * @return the maximal position size currently allowed
     */
    function _getMaxPositionSize(int128 _fTraderExposureEMA, int128 _fBumpUp) 
        internal pure returns (int128) 
    {
        require(_fTraderExposureEMA>0, "precondition: _fTraderExposureEMA must be positive");
        // maxAbs = emwaTraderK*bumpUp
        return _fTraderExposureEMA.mul(_fBumpUp);
    }
     ...
     * @param _fkStar                Trade that brings AMM risk to minimum
     * @param _fTraderExposureEMA    Current trader exposure EMA (perpetual.fCurrentAMMExposureEMA)
     * @param _fBumpUp               How much do we allow to increase the trade size above the current
     *                               fCurrentAMMExposureEMA? (e.g. 0.25)
     * @return signed maximal trade sizes (short, long)
     */
    function getMaxTradeSizesForPos(int128 _fCurrentTraderPos, int128 _fkStar, int128 _fTraderExposureEMA, int128 _fBumpUp)
        override virtual external view returns (int128, int128)
    {
        int128 fMaxPos = _getMaxPositionSize(_fTraderExposureEMA, _fBumpUp);
        int128 fMaxTradeLong = fMaxPos.sub(_fCurrentTraderPos);
        fMaxTradeLong = fMaxTradeLong<0 ? 0 : fMaxTradeLong;
        fMaxTradeLong = _fkStar > fMaxTradeLong ? _fkStar : fMaxTradeLong;
        int128 fMaxTradeShort = fMaxPos.neg().sub(_fCurrentTraderPos);
        fMaxTradeShort = fMaxTradeShort>0 ? 0 : fMaxTradeShort;
        fMaxTradeShort = _fkStar < fMaxTradeShort ? _fkStar : fMaxTradeShort;
        return (fMaxTradeShort, fMaxTradeLong);
    } 
...
//            int128 tmp = _perpetual.fCurrentAMMExposureEMA[iIndex].mul(fLambda).add(ONE_64x64.sub(fLambda).mul(fCurrentObs));
    ...
    function _updateMargin(
        ...
        if (fOldPosition != 0) {
            fFundingPayment = _perpetual.fUnitAccumulatedFunding.sub(account.fUnitAccumulatedFundingStart).mul(_fDeltaPosition);
        }
        //position
        account.fPositionBC = account.fPositionBC.add(_fDeltaPosition);
        //cash
        account.fCashCC = account.fCashCC.add(_fDeltaCashCC).add(fFundingPayment);
        ...
        emit UpdateMarginAccount(_perpetual.id,
            _traderAddr, 
            account.fPositionBC, 
            account.fCashCC, 
     ...
     * @param   _fTreasuryFee               amount to be sent to treasury
    ...
    function _transferFee(
        ...
        require(_fTreasuryFee >= 0, "treasury should earn fee");
        ...
        emit UpdateParticipationFundCash(_liqPool.id, _fPnLparticipantFee, _liqPool.fPnLparticipantsCashCC, block.timestamp);

        _liqPool.fDefaultFundCashCC = _liqPool.fDefaultFundCashCC.add(_fDefaultFundContribution);
        emit UpdateDefaultFundCash(_liqPool.id, _fDefaultFundContribution, _liqPool.fDefaultFundCashCC, block.timestamp);
        ...
        emit UpdateAMMFundCash(_perpetual.id, 
            _perpetual.fAMMFundCashCC, 
            _liqPool.fAMMFundCashCC, 
            block.timestamp);

        address mgnTokenAddr = _liqPool.marginTokenAddress;
        address treasuryAddress = _liqPool.treasuryAddress;
        _transferFromVaultToUser(mgnTokenAddr, _referrerAddr, _fReferralRebate);
        _transferFromVaultToUser(mgnTokenAddr, treasuryAddress, _fTreasuryFee);
        emit TransferFeeToTreasury(treasuryAddress, _fTreasuryFee);
    ...
    function _queryPriceFromAMM(PerpetualData storage _perpetual, int128 _fTradeAmount) internal view returns (int128) {
        ...
        // minimal spread unit is quote currency
        int128[2] memory spread;
        spread[0] = _perpetual.fMinimalSpread;
        spread[1] = _perpetual.fIncentiveSpread;

        int128 fPrice = _getAMMPerpLogic().calculatePerpetualPrice(ammState, marketState, _fTradeAmount, spread);
     ...
     * @param   _traderAddr         The address of trader.
     * @param   _referrerAddr       The address of referrer who will get rebate from the deal.
     * @param   _fDeltaPositionCC   The signed position of the trade (in collateral currency).
     * @param   _hasOpened          Does the trader open a position or close?
     * @return  lpFee totalFee      The total fee collected from the trader after the trade
     */
    function _distributeFees(
        ...
        {
            (fPnLparticipantFee, fTreasuryFee, fReferralRebate) = _calculateFees(_perpetual, _traderAddr, _referrerAddr, _fDeltaPositionCC.abs(), _hasOpened);
        }
        _updateTraderMargin(_perpetual, _traderAddr, fPnLparticipantFee.add(fTreasuryFee).add(fReferralRebate).neg());

        int128 fAMMFundContribution;
        int128 fDefaultFundContribution;
        (fAMMFundContribution, fDefaultFundContribution, fTreasuryFee) = _calculateContributions(liqPool, fTreasuryFee);
    ...
    {
        // allocate fees to AMM or Default Fund
        // if size exceeds target size, the excess amount is sent to the protocol treasury
        int128 fAMMFundContribution = _pool.fTargetAMMFundSize.sub(_pool.fAMMFundCashCC);
        // default fund contributions
        int128 fDefaultFundContribution = _pool.fTargetDFSize.sub(_pool.fDefaultFundCashCC);
        int128 fAMMFundGap;
        // contribute the whole treasury-fee or what is needed to reach target size
        if (fAMMFundContribution > _fTreasuryFee) {
            fAMMFundGap = fAMMFundContribution.sub(_fTreasuryFee);
            fAMMFundContribution = _fTreasuryFee;
        }
        _fTreasuryFee = _fTreasuryFee.sub(fAMMFundContribution);

        fDefaultFundContribution = fDefaultFundContribution > _fTreasuryFee ? _fTreasuryFee : fDefaultFundContribution;
        if (fDefaultFundContribution < 0 && fAMMFundGap > 0) {
            // default fund exceeds target size, but
            // AMM pool has not reached target size
            int128 fDefaultFundRefund = fAMMFundGap > fDefaultFundContribution.neg() ? fDefaultFundContribution.neg() : fAMMFundGap;
            fAMMFundContribution = fAMMFundContribution.add(fDefaultFundRefund);
            // shrink default fund contribution (negative since refund to treasury) by amount needed for AMM pool
            fDefaultFundContribution = fDefaultFundContribution.add(fDefaultFundRefund);
        }
        // fDefaultFundContribution < 0 -> treasury gets refund, > 0 protocol treasury gets less fee.
        if (fDefaultFundContribution > 0) {
            _fTreasuryFee = _fTreasuryFee.sub(fDefaultFundContribution);
        } else {
            _fTreasuryFee = _fTreasuryFee.add(fDefaultFundContribution);
        }
        if (_fTreasuryFee < 0) {
            _fTreasuryFee = 0;
        }
        return (fAMMFundContribution, fDefaultFundContribution, _fTreasuryFee);
    }
     ...
     * @param   _traderAddr         The address of trader.
     * @param   _referrerAddr       The address of referrer who will get rebate from the deal.
     * @param   _fDeltaPosCC        The (abs) position size in collateral currency.
    ...
    {
        require(_fDeltaPosCC >= 0, "trade value is negative");

        int128 fTreasuryFee = _fDeltaPosCC.mul(_perpetual.fTreasuryFeeRate);
        int128 fPnLparticipantFee = _fDeltaPosCC.mul(_perpetual.fPnLPartRate);
        int128 fTotalFee = fPnLparticipantFee.add(fTreasuryFee);
        int128 fReferralRebate;
        // if the trader opens the position, 'available margin' is the margin balance - initial margin
        // requirement. If the trader closes, 'available margin' is based on maintenance margin
        int128 fAvailableMargin = _getAvailableMargin(_perpetual, _traderAddr, _hasOpened);
        ...
        if (!_hasOpened) {
            if (fAvailableMargin <= 0) {
                ...
                fTreasuryFee = 0;
                fReferralRebate = 0;
            } else if (fTotalFee > fAvailableMargin) {
                // make sure the sum of fees < available margin
                int128 fRate = fAvailableMargin.div(fTotalFee);
                fTreasuryFee = fTreasuryFee.mul(fRate);
                fPnLparticipantFee = fAvailableMargin.sub(fTreasuryFee);
            }
        } else {
            //_hasOpened
            // If the margin of the trader is not enough for fee: If trader open position, the trade will be reverted.
            require(fTotalFee <= fAvailableMargin, "margin isn't enough");
        }

        if (_referrerAddr != address(0) && _perpetual.fReferralRebateRate > 0 && fPnLparticipantFee.add(fTreasuryFee) > 0) {
            int128 fPnLparticipantFeeRebate = fPnLparticipantFee.mul(_perpetual.fReferralRebateRate);
            int128 fTreasuryFeeRebate = fTreasuryFee.mul(_perpetual.fReferralRebateRate);
            fReferralRebate = fPnLparticipantFeeRebate.add(fTreasuryFeeRebate);
            fPnLparticipantFee = fPnLparticipantFee.sub(fPnLparticipantFeeRebate);
            fTreasuryFee = fTreasuryFee.sub(fTreasuryFeeRebate);
        }
    ...
    function getFunctionList() external view virtual override returns (bytes4[] memory) {
        ...
        functionList[3] = this.getMaxTradeSizesForPos.selector;
        return functionList;
    }
}
