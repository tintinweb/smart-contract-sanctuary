...
contract EthereumToEchoInternalStorage is InternalStorage {
    ...
    event Deposited(uint152 indexed receiver, address indexed payer, uint256 amount);

    constructor(bytes32 genesisHash_, WrappedECHO token_) InternalStorage(genesisHash_) {
        token = token_;
    }

    function deposit(uint152 receiver, uint256 amount) external returns (bool success) {
        ...
        emit Deposited(receiver, payer, amount);
        return true;
    }
}
...
contract WrappedECHO is ERC20 {
    ...
    constructor() ERC20("Wrapped ECHO", "wECHO") {
        return;
    }
    ...
    function setBurner(address burner_) external returns (bool success) {
        require(burner == address(0), "Burner already set");
        burner = burner_;
        return true;
    }
    ...
    function burn(address owner, uint256 amount) external returns (bool success) {
