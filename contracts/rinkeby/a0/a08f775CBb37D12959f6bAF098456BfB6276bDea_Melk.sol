...
import "./CrypticCows.sol";

contract Melk is ERC20, Ownable, ReentrancyGuard {
  using Strings for uint256;

  uint256 constant nameChangeCost = 250 ether;
  uint256 constant descChangeCost = 350 ether;
  uint256 constant rewardPerDay = 10 ether;

  CrypticCows crypticCows;

  mapping(uint256 => uint256) stakeTimes;
  mapping(uint256 => address) stakedBy;

  constructor(address crypticCowsAddress) ERC20("Melk", "MELK") {
    crypticCows = CrypticCows(crypticCowsAddress);
  }

  function stakeToken(uint256 tokenId) public nonReentrant {
    require(crypticCows.getApproved(tokenId) == address(this), "Not approved to transfer token");
    require(crypticCows.ownerOf(tokenId) == msg.sender, "You do not own this token");
    crypticCows.transferFrom(msg.sender, address(this), tokenId);

    stakeTimes[tokenId] = block.timestamp;
    stakedBy[tokenId] = msg.sender;
  }

  function unStakeToken(uint256 tokenId) public nonReentrant {
    require(stakedBy[tokenId] == msg.sender, "You did not stake this cow");

    crypticCows.transferFrom(address(this), msg.sender, tokenId);
    _mint(msg.sender, rewardPerDay * stakedForDays(tokenId));
    delete stakeTimes[tokenId];
    delete stakedBy[tokenId];
  }

  function stakedByUser(address user) public view returns (uint256[] memory) {
    uint256[] memory res;

    for (uint256 i = 0; i < crypticCows.totalSupply();i ++) {
      if (stakedBy[i] == user) {
        res[res.length] = i;
      }
    }
  ...
  function stakedForDays(uint256 tokenId) public view returns (uint256) {
    require(stakeTimes[tokenId] != 0, "Token is not staked");

    return (block.timestamp - stakeTimes[tokenId]) / 1 days;
  }
  ...
  function setCustomName(
    ...
    require(verifySignature(abi.encodePacked("name", url), hash, v, r, s), "Signature invalid");
    require(crypticCows.ownerOf(tokenId) == msg.sender, "You do not own this token");
    require(balanceOf(msg.sender) >= nameChangeCost, "You do not have enough MELK");

    _burn(msg.sender, nameChangeCost);
    assert(balanceOf(msg.sender) >= 0);

    crypticCows.setCustomTokenURI(tokenId, url);
  }
  ...
  function setCustomDescription(
    ...
    require(verifySignature(abi.encodePacked("desc", url), hash, v, r, s), "Signature invalid");
    require(crypticCows.ownerOf(tokenId) == msg.sender, "You do not own this token");
    require(balanceOf(msg.sender) >= descChangeCost, "You do not have enough MELK");

    _burn(msg.sender, descChangeCost);
    assert(balanceOf(msg.sender) >= 0);
