...
contract FAMECollectible is ERC1155Tradable {
  ...
  function legacyConversion() public
  {
    ...
    _mint(msg.sender, 1, convertingBalance, "");
  }

  function convertBaseCredits(uint256 targetID, uint256 amount) public
  {
    require(_exists(targetID),"INVALID_TARGET");
    require(targetID==2||targetID==3,"INVALID_CONVERSION");
    require(balanceOf(msg.sender,1)>=amount,"INSUFFICIENT_CREDIT");
    uint256 receiveAmount = 0;
    uint256 burnAmount = 0; 
    uint16 legacy_zeros = 12;
    uint256 sourceAmount = amount / (legacy_zeros*10);
    if(targetID==2) 
    {
      receiveAmount = sourceAmount * 1000;  //1000 FAME Tokens per Founder Credit
      burnAmount = receiveAmount / 1000;
    }
    if(targetID==3) 
    {
      receiveAmount = sourceAmount / 10;  //0.1 Senate Seat per Founder Credit
      burnAmount = receiveAmount * 10;
    }
    burnAmount *= (legacy_zeros*10);
    require(receiveAmount>0,"INSUFFICIENT_EXCHANGE");
    require(burnAmount>0,"BURN_ERROR");
    _burn(msg.sender,1,burnAmount);
    _mint(msg.sender,targetID,receiveAmount,"");
  }
