...
/**
 * @dev An ERC20 token for LandDao.
 ...
 */
contract LandDaoToken is ERC20, ERC20Permit, ERC20Votes, Ownable {
    ...
    function startVest(address tokenLockAddress) public onlyOwner {
        require(!vestStarted, "LandDao: Vest has already started.");
        vestStarted = true;
        _approve(address(this), tokenLockAddress, devSupply);
        LandDaoTokenLock(tokenLockAddress).lock(0x73ea708dC6e7A629AE3c89322320F1107537e200, 25_000_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0xd96bfADF43F106C5882c30B96E8a0769dbD5486B, 25_000_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0x7a1B5439c870a062c5701C78F52eE83FAFBb9274, 25_000_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0xF0591DA70765fE40B99E8B0e2bD0bF1F6A1AE797, 10_000_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0x379C326e6443c34Fa6a6E21e4D48A2F6CDd8cE23, 10_000_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0xD75fB66E71bfFbB1C9d09F7Ae2C3270d9F71ecfb, 10_000_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0x87892947e4AE5a208f647b0128180032145837cC, 6_000_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0xf28E9401310E13Cfd3ae0A9AF083af9101069453, 5_000_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0x5F2c4aa7d6943f1B5F8dFc6cc58f3A52F889B8e8, 3_000_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0x85A83A4810213Eb84dde3053C923f0Ee565Fb14f, 3_000_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0x3d2B46E9c730975415ef283f0D5254662358887D, 2_500_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0x9E51bB806f126EFDCa437dD8a939C7f1c840Eb1f, 2_500_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0x9FF216d036872222CB18683F0A72CAd6ee1E736F, 2_500_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0x15f3E2B44F6c8F832EFB60f588252AB001653320, 2_500_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0xa0eE3c95AB7099e91c5E1009c28b093d4c1faA50, 2_500_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0x9949a8335948491F1b126f3c3Ba247F63d34970A, 2_500_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0xFCb1365fD8d2033Fc5b5258EA3fe80D2D6CE2DA1, 2_500_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0x6dF5cb181c362EF37977F99adefCCd51FA45dC27, 2_000_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0x5407a5eF608d01544dbCc57EBaaEa235eFa9055A, 2_000_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0x96273F34b18F096903f8a683FB01BA9ed35cce98, 2_000_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0x8E37DEC70b948077BCeeb2dCA3BB2aF1CC183A32, 1_000_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0xb3968575aB0A5892eC3965E9fb9C74f6d2651f17, 1_000_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0x0cE90F4cB5D5fEbb3189E0F376F976E8e8Ea2020, 1_000_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0x36b3843d782b48481f277c3f047cc2e143C49dA5, 1_000_000_000e18);
        LandDaoTokenLock(tokenLockAddress).lock(0x060de4538bDf72C4785840e79034ad722722b7eB, 500_000_000e18);
    }
    ...
    function claimTokens(uint256 amount, bytes32[] calldata merkleProof) public {
        ...
        require(valid, "LandDao: Valid proof required.");
        require(!claimed[msg.sender], "LandDao: Tokens already claimed.");
    ...
    function sweep(address dest) public onlyOwner {
        require(block.timestamp > claimPeriodEnds, "LandDao: Claim period not yet ended");
