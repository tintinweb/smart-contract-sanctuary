...
contract Award is Context, Ownable{
    ...
    function _StartLottery(uint64 _ig) private {
                ...
                _Dev_fee = lottToken.mul(100).div(10000);
    ...
    function _ChangeBurnAddress(address _adr) private returns(bool) {
        emit ChangeDevAddressEvent(_burn_addr, _adr);
        _burn_addr = _adr;
    ...
    function HowMuchTokenMustPay() public view returns(uint){
        ...
        if(pair.token0() == token_address){
            howmuchtoken = getTokenPrice0(10) * 10 ** 18;
        }else{
            howmuchtoken = getTokenPrice1(10) * 10 ** 18;
        }
        return howmuchtoken;
    }
