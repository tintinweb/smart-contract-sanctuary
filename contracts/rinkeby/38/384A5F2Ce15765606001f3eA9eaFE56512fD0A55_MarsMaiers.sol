...
contract MarsMaiers is ERC721, AccessControl, ReentrancyGuard {
  ...
  // tracks balances held in escrow by address
  ...
  event bidAccepted(string bidToken, address bidder, uint256 value, uint256 total);
  event refundAccepted(string bidToken, address bidder, uint256 total);
  ...
  // wait for the project to be concluded, once drainEscrow() is available
  //
  // ======================================
  // ==                                  ==
  ...
  function bid(string calldata bidToken) public payable {
    ...
    // add value sent to senders escrow balance
    escrow[_msgSender()] = escrow[_msgSender()] + msg.value;

    // emit event
    emit bidAccepted(bidToken, msg.sender, msg.value, bids[bidToken]);
  }
  ...
  // returns current escrow balance (all pending + lost bids)
  function getEscrow(address holder) public view returns (uint256) {
    return escrow[holder];
  }
  ...
  function settle(uint256 tokenId, string memory bidToken) public onlyRole(AUCTIONEER_ROLE) {
    ...
    // ensure winner has enough in escrow
    require(escrow[winner] - amount >= 0, 'amount exceeds winners escrow');

    // subtract amount from winners escrow
    escrow[winner] = escrow[winner] - amount;
  ...
  // allow draining entire escrow balance when project has concluded
  function drainEscrow(address recipient) public nonReentrant {
    // we can assume all remaining bids are lost once final winner is chosen
    require(projectConcluded, 'project has not concluded yet');

    // validate escrow balance
    require(escrow[recipient] > 0, 'escrow is empty');

    // stash and zero escrow balance
    uint256 amount = escrow[recipient];
    escrow[recipient] = 0;

    // send escrow balance to recipient
  ...
  function refund(
    ...
    // only allow per-bid refunds while project is running
    require(!projectConcluded, 'project has concluded, use drainEscrow instead');

    // ensure bidToken exists with a balance, and was placed by _msgSender
    require(bids[bidToken] > 0, 'bid does not exist');
    require(bidders[bidToken] == _msgSender(), 'not your bid');
    ...
    // validate inferred message matches input params, and was signed by an address with BANKER_ROLE
    // if this passes we can trust the input params match
    require(hasRole(BANKER_ROLE, message.toEthSignedMessageHash().recover(signature)), 'invalid receipt');

    // stash and zero bid
    uint256 amount = bids[bidToken];
    bids[bidToken] = 0;

    // don't zero bidder (prevent future reuse of bidToken)
    address bidder = bidders[bidToken];

    // validate and adjust bidder escrow balance
    require(escrow[bidder] - amount >= 0, 'amount exceeds escrow');
    escrow[bidder] = escrow[bidder] - amount;

    // emit event
    emit refundAccepted(bidToken, bidder, amount);
