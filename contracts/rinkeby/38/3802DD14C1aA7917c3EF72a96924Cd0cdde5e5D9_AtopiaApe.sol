...
contract AtopiaApe is AtopiaBase {
	...
	function setName(uint256 tokenId, string memory name) external onlyTokenOwner(tokenId) {
	...
	function useItem(
		...
		(uint256 job, uint256 package, ) = space.getLife(tokenId);
		require(job == 0 || package > 0);
...
abstract contract AtopiaBase is ERC721 {
	...
	function getRewardsInternal(uint256 tokenId, uint256 info, uint256 timestamp) internal view returns (uint256) {
	...
	function getRewards(uint256[] memory tokenIds) external view returns (uint256 rewards) {
		...
		for (uint256 i = 0; i < tokenIds.length; i++) {
			...
			if (job == 0 || package > 0) {
				rewards += (info >> 128) + getRewardsInternal(tokenId, info, timestamp);
			} else {
				rewards += info >> 128;
			}
		}
	...
	function claimRewards(uint256[] memory tokenIds) public {
		...
		for (uint256 i = 0; i < tokenIds.length; i++) {
			...
			(uint256 job, uint256 package, ) = space.getLife(tokenId);
			uint256 info = infos[tokenId];
			uint256 pending = info >> 128;
			if (job == 0 || package > 0) {
				pending += getRewardsInternal(tokenId, info, timestamp);
			}
			if (pending > 0) {
				rewards += pending;
