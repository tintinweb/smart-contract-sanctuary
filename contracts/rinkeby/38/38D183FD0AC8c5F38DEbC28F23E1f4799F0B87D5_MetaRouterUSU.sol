...
contract MetaRouterUSU is Context {
    ...
    constructor(address _synthesis, address _wrapper) public {
    ...
    function metaRouteNative(
        MetaRouteStructs.MetaRouteTransaction memory _metarouteTransaction
    ) external payable returns (bytes32) {
        uint256 amount = _metarouteTransaction.amount;
    ...
    function _metaRouteInternal(
        ...
        console.log('bridgingFee', bridgingFee);
        return
            ISynthesis(synthesis).metaBurnSyntheticToken{value: bridgingFee}(
                metaBurnTransaction
            );
    }
}
