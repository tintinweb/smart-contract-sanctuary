...
interface IVXG_ERC20 is IERC20Metadata {
...
interface IGamePoolUSDC {
	...
	function VXG() external view returns (IVXG_ERC20 VXG);
	function withdrawalTime() external view returns (uint withdrawalTime);
	function lastFetchId() external view returns (uint lastFetchId);
...
contract VXG_token is IVXG_ERC20 {
	...
	function name() external pure returns (string memory) {
		return "Venture X Gaming testet token 0x0002";
	}
