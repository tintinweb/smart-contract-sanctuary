...
contract Dracarys is Context, IERC20, Ownable {
    ...
    uint256 public numTokensSellToAddToLiquidity = 9000000 * 10**9;
    // address public marketingWallet = 0x6d9b2E1B8F7144D673E1B33c021794652d7F1B43;
    address public marketingWallet = 0x41f9BB7b93156d33af3077bb53fA1d3fbC7f0Aeb;  //test wallet
    ...
    // address public dracWallet = 0x41f9BB7b93156d33af3077bb53fA1d3fbC7f0Aeb;  //test wallet
    address payable private _dracPayable;
    ...
    function setFees(uint256 taxFee, uint256 marketingFee, uint256 charityFee, uint256 dracFee, uint256 liquidityFee, uint256 SwapThresholdAmount) external onlyOwner {
        _taxFee = taxFee;
    ...
    function _transfer(
        ...
        uint256 dracFee = amount * _dracFee / 100;
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
         ...
         if (takeFee) {
            if (marketingFee > 0) {
                _tokenTransfer(from, marketingWallet, marketingFee, takeFee);
            }
            if (charityFee > 0) {
                _tokenTransfer(from, charityWallet, charityFee, takeFee);
            }
            if (dracFee > 0) {
                _tokenTransfer(from, dracWallet, dracFee, takeFee);
            }
            _tokenTransfer(from, to, amount.sub(marketingFee).sub(dracFee).sub(charityFee), takeFee);
        } else {
