...
contract Coof1155 is PermitControl, OnChainMeta, ERC165, IERC1155, IERC1155MetadataURI {
  ...
  */
  function coof(uint256 _coofId, address[] calldata _hosts) external {
    require(balanceOf(_msgSender(), _coofId) > 0, "cant spread the coof if you dont have it");
    require(_hosts.length < rNaught, "spreading to too many hosts");
  ...
  */
  function vaccinate(uint256 _vaxId) external {
    require(vaxx.balanceOf(_msgSender(), _vaxId) > 0, "cant vax if you dont have one");
    ...
    // Edit an existing item group. The name may always be updated.
      ...
      if (itemGroups[_groupId].supplyType == SupplyType.Capped) {
        require(_data.supplyType == SupplyType.Capped,
          "you may not uncap a capped supply type");
        require(_data.supplyData <= itemGroups[_groupId].supplyData,
          "you may not increase the supply of a capped type");
      ...
      require(_data.supplyData >= itemGroups[_groupId].circulatingSupply,
        "you may not decrease supply below the circulating amount");
      ...
      if (itemGroups[_groupId].itemType == ItemType.Nonfungible) {
        require(_data.itemType == ItemType.Nonfungible,
          "you may not alter nonfungible items");
      ...
      // A fungible item may change type if it is unique enough.
        ...
        if (_data.itemType == ItemType.Nonfungible) {
          require(itemGroups[_groupId].circulatingSupply <= 1,
            "the fungible item is not unique enough to change");
  ...
  function mintBatch(address _recipient, uint256[] calldata _ids,
    ...
    for (uint256 i = 0; i < _ids.length; i++) {
      require(_hasItemRight(_ids[i], keccak256("MINT")),
        "you do not have the right to mint that item");
...
library NFTSVG {
  ...
  function _generateSVGDefs(SVGParams memory params) private view returns (string memory svgDef) {
    ...
    ( uint256 r1, string memory rgbSecondary ) = idToColor(params.tokenId);
    ...
    for(uint256 i = 0; i < 5; ++i){
      ...
      subDefs = string(abi.encodePacked(
        ...
        (subIndex + 1).toString(),
        '" >',
        '<animate attributeName="d" values="m240,',
        space.toString(),
        'l0,0 0,0;m190,190l30,-',
        drift1.toString(),
        ' 30,',
        drift1.toString(),
        ';m110,60l0,0 0,0;" dur="12s" repeatCount="indefinite" begin="p1.begin+',
        (subIndex * 2).toString(),
        's"/>',
    ...
    svgDef = string(
      abi.encodePacked(
        ...
        '<defs>',
        '<path id="r1" >',
        '<animate id="p1" attributeName="d" values="m240,',
        space.toString(),
        'l0,0 0,0;m190,',
        r1.toString(),
        'l30,-',
        ...
        ' 30,',
        drift1.toString(),
        ';m110,60l0,0 0,0;" dur="12s" repeatCount="indefinite" />',
        '<animate attributeName="fill" values="',
        rgbPrimary,
        rgbSecondary,
        '" dur="12s" repeatCount="indefinite" begin="p1.begin"/>',
  ...
  function idToColor(uint256 _id) public view returns (uint256, string memory) {
    ...
    return (256 - firstChunk, rgbString);
   }
}