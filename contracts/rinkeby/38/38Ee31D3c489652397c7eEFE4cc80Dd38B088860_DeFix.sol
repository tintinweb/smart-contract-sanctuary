...
//        address USDT = 0xdAC17F958D2ee523a2206206994597C13D831ec7;
    ...
    function swapTokensForUsdt(address tokenAddress, uint256 tokenAmount) internal swapping pcsInitialized returns (uint256, uint256) {
        ...
        uint256 usdtReceived = IERC20(USDT).balanceOf(address(this)) - initialUsdtBalance;
...
interface IDeFiX {
    ...
    // покупка и продажа
    function investInUsdt(uint256 _projectId, uint256 _amount) external payable returns (uint256); // {from: account, value: value}
...
//        IERC20(USDT).approve(address(router), type(uint256).max);
    ...
    function investInUsdt(uint256 _projectId, uint256 _amount) external payable override returns (uint256) {
        require(IERC20(USDT).allowance(msg.sender, address(this)) >= _amount, "Owner must allow the transaction");
        IERC20(USDT).transferFrom(msg.sender, address(this), _amount);
        Share[] memory shares = projects[_projectId].tokens;
        uint256 oneSharePart = _amount / projects[_projectId].totalPoints;
        uint256 investedInUSDT;
        for (uint256 i = 0; i < shares.length; i++) {
            uint256 usdtForSwap = oneSharePart * shares[i].point;
    ...
    function withdrawInUsdt(uint256 _projectId, uint256 _percent) external override returns (uint256) {
        ...
        IERC20(USDT).transfer(msg.sender, withdrawnInUSDT);
