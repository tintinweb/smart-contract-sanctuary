...
contract ssof_royalties_distribution is Ownable {
    ...
    ERC721 ssofNFT = ERC721(0xDf2558Dc209e50b1D88840030fC3989cBE149E9e);
    ...
    function updateDistribution() public onlyOwner {
        require(sessionReward > 0, "don't have any amount for reward");
        uint halfAmount = sessionReward.mul(DistributionPercent).div(100);
        uint newAmount = halfAmount.div(ssofNFT.totalSupply());
        for(uint i = 0; i < ssofNFT.totalSupply(); i++){
            if(ssofNFT.tokenByIndex(i) > 0){
                ...
                totalReward = totalReward.add(newAmount);
            }
        }
        sessionReward = 0;
    } 
    ...
    function claimReward() public {
        ...
        totalReward = totalReward.sub(userRewards[msg.sender]);
    }


    function withdraw() public onlyOwner {
        uint balance = address(this).balance.sub(totalReward);
        payable(msg.sender).transfer(balance);
    }

    function updateDistributionPercent(uint256 Percent) public onlyOwner {
