...
contract BambooFactory is ERC20Burnable, Ownable, IERC721Receiver, Pausable, ReentrancyGuard {
    ...
    IERC721 public immutable pandaContract;
    ...
    constructor(        
        ...
        pandaContract = IERC721(_pandaContract);
    ...
    function unstake(uint256[] calldata tokenIds) external whenNotPaused nonReentrant {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            delete lastClaimedBlockForToken[tokenIds[i]];
           
            pandaContract.safeTransferFrom(address(this), msg.sender, tokenIds[i]);
        }
    ...
    function claimRewards(uint256[] calldata tokenIds) external whenNotPaused {
        require(tokenIds.length > 0, "no panda id given");
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            rewards += calculateRewards(tokenIds[i]);
            lastClaimedBlockForToken[tokenIds[i]] = block.number;
        }
    ...
    function calculateRewards(uint256 tokenID) public view returns (uint256) {
        ...
        return rewardPerBlock * (block.number - lastClaimedBlockForToken[tokenID]);    
    }      
