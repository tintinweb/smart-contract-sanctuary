...
contract GhostsProject is ERC721Enumerable, Administration, GhostBase {
    ...
    string private tokenUriBasePreview;
    ...
    mapping(address => uint256) private _allowListClaimable;
    ...
    function setGhostPrice(uint256 _price) external onlyRole(MODERATOR_ROLE) {
        ghostPrice = _price;
    }

    function flipSaleState() external onlyRole(MODERATOR_ROLE) {
    ...
    function setRandomSeed() external onlyRole(MODERATOR_ROLE) {
        _setRandomSeed();
    }

    function addToAllowList(address[] calldata addresses, uint256[] calldata values) external onlyRole(MODERATOR_ROLE) {
        require(addresses.length == values.length, "length of address and values must be same");
        for (uint256 i = 0; i < addresses.length; i++) {
            require(addresses[i] != address(0), "Can't add null address");
            _allowListClaimable[addresses[i]] = values[i];
        }
    }

    function onAllowList(address addr) external view returns (bool) {
        return (_allowListClaimable[addr] > 0);
    }

    function removeFromAllowList(address[] calldata addresses) external onlyRole(MODERATOR_ROLE) {
        for (uint256 i = 0; i < addresses.length; i++) {
            require(addresses[i] != address(0), "Can't remove null address");
            _allowListClaimable[addresses[i]] = 0;
        }
    }

    function allowListClaimableBy(address owner) external view returns (uint256) {
        require(owner != address(0), 'Null address cant be in allow-list');
        return _allowListClaimable[owner];
    }
    ...
    function setTokenUriBase(string calldata _tokenUriBase) external onlyRole(MODERATOR_ROLE) {
        tokenUriBase = _tokenUriBase;
        emit TokenUriBaseSet(_tokenUriBase);
    }

    function setTokenUriBasePreview(string calldata _tokenUriBase) external onlyRole(MODERATOR_ROLE) {
        tokenUriBasePreview = _tokenUriBase;
        emit TokenUriBaseSet(_tokenUriBase);
    }
    ...
    function mintGhost(uint256 numberOfTokens) public payable {
        require(saleIsActive, "Activate sale state");
    ...
    function mintGhostInPresale(uint256 numberOfTokens) public payable {
        require(presaleIsActive, "Activate presale state");
        require(_allowListClaimed[msg.sender] + numberOfTokens <= _allowListClaimable[msg.sender], "Tried to mint too many ghosts");
        require(totalSupply() + numberOfTokens <= MAX_GHOSTS, "Purchase would exceed max supply of ghosts");
        require(ghostPrice * numberOfTokens <= msg.value, "inefficient ether");
    ...
    returns (string memory uri) {
        ...
        uint256 endpoint = (_tokenId <= 10) ? _tokenId : 11 + (randomSeed + _tokenId) % (MAX_GHOSTS - 11);
        uri = bytes(tokenUriBase).length > 0 ? string(abi.encodePacked(tokenUriBase, StringLib.uint2str(endpoint))) : tokenUriBasePreview;
    }
    ...
    function _mintTeamGhost() internal onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        for (uint256 i = 1; i < 11; i++) {
            _safeMint(msg.sender, i);  // OFF Team Ghost
        }
    }
