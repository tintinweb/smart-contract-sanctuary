...
contract Ceramic is ERC721Burnable {
    ...
    */
    function mintCeramics(uint256 numberOfTokens) external payable {
        ...
        require(totalSupply().add(numberOfTokens) <= MAX_CERAMIC_SUPPLY, "Purchase would exceed max supply");
        require(mintPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
        
        for(uint256 i = 0; i < numberOfTokens; i++) {
            uint256 mintIndex = totalSupply();
            if (totalSupply() < MAX_CERAMIC_SUPPLY) {
        ...
        if (startingIndexBlock == 0 && (totalSupply() == MAX_CERAMIC_SUPPLY || block.timestamp >= REVEAL_TIMESTAMP)) {
