...
contract ZKTTVesting is Ownable, Initializable, Testable {
    ...
    event SetLockedPercentage(address indexed from, uint lockedPercentage);
    ...
    function getLockedPercentage() external view returns(uint) {
        return lockedPercentage;
    }

    function setLockedPercentage(uint lockedPercentage_) external onlyOwner {
        require(lockedPercentage_ <= 100, "ZKTTVesting: lockedPercentage_ too big");
