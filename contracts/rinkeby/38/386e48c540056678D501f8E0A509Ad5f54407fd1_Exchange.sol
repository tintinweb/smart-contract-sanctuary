...
contract Exchange is Ownable, Pausable, ReentrancyGuard {
  ...
  mapping(bytes => bool) orderDeactivations;
  ...
  /* This contains all data of the trade */
   ...
   *
   * Emits a {SellOrderFilled} event via `_trade`.
   */
  function trade(
    address payable seller,
    address erc721address,
    ...
    require(
      msg.value >= price,
      "You're transaction doesn't have the required payment"
    );
    ...
    _trade(sellOrder, signature, buyer);
  }
   ...
   */
  function _trade(
    ERC721SellOrder memory sellOrder,
    bytes memory signature,
    address payable buyer
  ) internal {
    ...
    /* Make sure the seller address is not frozen */
    require(
      !isAddressFrozen(sellOrder.seller),
      "The seller address is currently frozen."
    );
    ...
    require(
      sellOrder.erc721address.supportsInterface(InterfaceId_ERC721),
      "IS_NOT_721_TOKEN"
    );

    // Check not expired
    require(
      (block.timestamp < sellOrder.expiration),
      "This sell order has expired."
    );
    ...
    require(
      (erc721.ownerOf(sellOrder.tokenId) == sellOrder.seller),
      "The seller does not own this NFT"
    );
    // Might be unnecessary

    /* require is approved for all */
    require(
      erc721.isApprovedForAll(sellOrder.seller, address(this)),
      "The Trading contract is not approved to operate this NFT"
    );

    /* Do the trade */

    // Step 1: Send the ETH
    uint256 royaltyPayout = (payoutPerMille[sellOrder.erc721address] *
      msg.value) / 1000;
    uint256 makerPayout = (_makerFeePerMille * msg.value) / 1000;
    uint256 remainingPayout = msg.value - royaltyPayout - makerPayout;

    if (royaltyPayout > 0) {
      address payable royaltyPayoutAddress = nftPayoutAddresses[
        sellOrder.erc721address
      ];
      royaltyPayoutAddress.call{ value: royaltyPayout }("");
    }

    _makerWallet.call{ value: makerPayout }("");
    sellOrder.seller.call{ value: remainingPayout }("");
  ...
  function setRoyalty(
    ...
    require(
      _erc721address.supportsInterface(InterfaceId_ERC721),
      "IS_NOT_721_TOKEN"
    );
  ...
  /*
   * @dev Freezes all sell orders for a specific address. Use this if the address is compromised.
   */
  function freezeAddress(address addressToFreeze) external {
    frozenAddresses[addressToFreeze] = true;
  }

  /*
   * @dev Unfreeze the sell orders for a specific address.
   */
  function unFreezeAddress(address addressToUnFreeze) external {
    frozenAddresses[addressToUnFreeze] = false;
  }

  /*
   * @dev Check if an address has been frozen.
   */
  function isAddressFrozen(address addressToCheck) public view returns (bool) {
    return frozenAddresses[addressToCheck];
  }
