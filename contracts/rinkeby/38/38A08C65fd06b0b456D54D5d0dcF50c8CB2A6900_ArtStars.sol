...
contract ArtStars is ERC721  {
    ...
    uint256 constant CONTRACT_ROYALTY = 2;//Contract royalty in percentage
    ...
    function setTokenURI(uint[] memory tokenIds,string[] memory hashes) external returns(bool){
        require(hasRole(DEFAULT_ADMIN_ROLE,_msgSender()), "You must have minter or an admin role to change tokenURI");
        require(tokenIds.length == hashes.length,"Wrong count of tokenIds or hashes");
        for (uint8 i=0;i<tokenIds.length;i++){
            _setTokenURI(tokenIds[i],hashes[i]);
        }
        return true;
    }
