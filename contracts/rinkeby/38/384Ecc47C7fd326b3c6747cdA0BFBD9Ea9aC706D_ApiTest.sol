...
/** 
 *  SourceUnit: d:\repos\NFTurbo\contracts\moonchad\moonchad_project.sol
*/
...
contract ApiTest is ERC721Enumerable, ERC721Burnable, Ownable {
    string public provenanceHash = "";
    string private baseTokenURI = "";
    uint256 public price = 30000000000000000;
    uint256 public supplyCap = 5555;
    uint256 public perTxLimit = 20;
    uint256 public reservedMints = 20;
    uint256 public tokenCounter = 0;
    uint256[] public firedChads;
    bool public salePaused = true;
    bool public burnPaused = true;

    constructor(string memory _baseTokenURI)
        payable
        ERC721("API Test", "ApiTst")
    {
        baseTokenURI = _baseTokenURI;
    }

    function mint(address _sender, uint256 _amountMinted) public payable {
        if (_sender != owner()) {
            require(!salePaused, "ERC721: sale is paused.");
        }
        require(totalSupply() < supplyCap, "ERC721: sale is finished.");
        require(
            _amountMinted > 0 && _amountMinted <= perTxLimit,
            "ERC721: you may only mint up to 20 chads"
        );
        require(
        ...
        for (uint256 i = 0; i < _amountMinted; i++) {
            _safeMint(_sender, tokenCounter);
            tokenCounter++;
        }
    }

    function regenerate(address _sender, uint256[] memory tokenIds) public {
        require(!burnPaused, "ERC721Burnable: burning is currently paused.");
        require(tokenIds.length > 1, "ERC721Burnable: you must burn at least 2 Chads.");
        require(tokenIds.length < 20, "ERC721Burnable: you may only burn 20 Chads at a time.");
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                _isApprovedOrOwner(_sender, tokenIds[i]),
                "ERC721Burnable: caller is not owner nor approved."
            );
        }

        firedChads.push(tokenIds[0]);
        _burn(tokenIds[0]);
    ...
    function setBurnPaused(bool _burnPaused) public onlyOwner() {
