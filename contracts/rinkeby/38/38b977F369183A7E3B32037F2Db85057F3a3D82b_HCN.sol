 ...
 */
contract ERC20PresetMinterPauserUpgradeableModified is
    Initializable,
    ContextUpgradeable,
    ERC20BurnableUpgradeable,
    ERC20PausableUpgradeable // MODIFIED
{
...
import "./HHHmainnet.sol";

contract HCN is HHHmainnet {

    // dummy funciton so that generated bycode of the contract will be different
    // see: https://forum.openzeppelin.com/t/deploying-2-identical-contracts-and-referencing-them-in-the-future-upgrades/5287
    function returnHCN() external pure returns(string memory) {
        return "HCN";
    }
}
 ...
 */
contract HHH is ERC20PresetMinterPauserUpgradeableModified, ERC1404, ERC165 {
     ...
     */
    function setNonWhitelistedDustThreshold(uint256 _nonWhitelistedDustThreshold) external onlyAdmin {
    ...
    */
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual override notRestricted(from, to, amount) {
        ...
        if (managementContract.isSuperWhitelisted(to)) {
            ...
            if (ub < amount) {
                ...
                removePendingDeposits(from, amountToUnlock);
            }
        } else {
     ...
     */
    function detectTransferRestriction (address from, address to, uint256 value) public override view returns (uint8) {
     ...
     * @dev Member of ERC-1404 Simple Restricted Token Standard: https://github.com/ethereum/EIPs/issues/1404
     */
    function messageForTransferRestriction (uint8 restrictionCode) public override view returns (string memory) {
    ...
    /**
     * @dev Removes `amount` from `from` user's {pendingDeposits}.
     */
    function removePendingDeposits(address from, uint256 amount) internal {
        uint256 nonWhitelistedDelay = managementContract.nonWhitelistedDelay();
        ...
        for (uint256 i = pendingDepositsLength - 1; i != uint256(-1); i--) {
            // slither-disable-next-line timestamp
            if (now > pendingDeposits[from][i].time + nonWhitelistedDelay) { // solium-disable-line security/no-block-members
                break;
            }
     ...
     * It calls {removePendingDeposits} by passing `from` and {ERC20-balanceOf}
     */
    function removeAllPendingDeposits(address from) internal {
        removePendingDeposits(from, balanceOf(from)); // balanceOf(from) is ok, even if actual lockedBalance < balanceOf(from). In both cases removePendingDeposits does the same number of iterations.
    }

    /**
     * @dev Removes all pending deposits. See more at {pendingDeposits}. Can be called only from management contract.
     * It calls {removePendingDeposits} by passing `addr`
     *
     * Requirements:
     *
     * - the caller must be `managementContract`.
     */
    function removeAllPendingDepositsExternal(address addr) external onlyManagementContract {
        removeAllPendingDeposits(addr);
    }
     ...
     * - the caller must be `managementContract`.
     */
    function putTotalBalanceToLock(address addr) external onlyManagementContract {
        // slither-disable-next-line controlled-array-length timestamp
    ...
    function lockedBalance(address user) public view returns (uint256) {
        ...
        for (uint256 i = pendingDepositsLength - 1; i != uint256(-1); i--) {
            ...
            if (now > pendingDeposits[user][i].time + nonWhitelistedDelay) { // slither-disable-next-line timestamp
     ...
     * balance.
     */
    function unlockedBalance(address user) public view returns (uint256) {
...
contract HHHmainnet is HHH {
    ...
    function changeMintingAddress(address newMintingAddress) external onlyAdmin {
        // slither-disable-next-line missing-zero-check
        mintingAddress = newMintingAddress;
    }
