...
contract OlympusBondingCalculator is IBondingCalculator {
  ...
  function _principleValuation( uint k_, uint amountDeposited_, uint totalSupplyOfTokenDeposited_ ) internal pure returns ( uint principleValuation_ ) {
    ...
    uint totalPrinciple_ = k_.sqrrt().mul(2);
    principleValuation_ = totalPrinciple_.mul( FixedPoint.fraction( amountDeposited_, totalSupplyOfTokenDeposited_ ).decode112with18().div( 1e18 ) );
    require(principleValuation_ >= 1e6, "Bond too small");
  }
