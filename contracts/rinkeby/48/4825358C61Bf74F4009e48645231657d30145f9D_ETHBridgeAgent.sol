...
abstract contract AbstractBridgeAgent is Ownable, Pausable {
	...
	function _register(
		...
		internal
		whenNotPaused
		sigNotReused(bridgeSig)
		sigNotExpired(sigExpire)
		returns (bool)
	{
		...
		emit Register(toChain, fromToken, toToken, tokenSigner);
		return true;
	}

	modifier sigNotReused(bytes memory bridgeSig) {
	...
	modifier sigNotExpired(uint256 sigExpire) {
...
contract ETHBridgeAgent is AbstractBridgeAgent {
	...
	function claim(
		...
		bytes memory tokenSig,
		bytes memory bridgeSig
	) external override whenNotPaused sigNotReused(bridgeSig) returns (bool) {
		require(!tokenPaused[token], "token is paused");
