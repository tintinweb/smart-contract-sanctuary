    ...
    ILendingPool
{
    ...
    struct UserAssetInfo {
        ...
        uint256 supplyIndex; // lender
        uint256 pendingDtokenDebt;
    }
    ...
    modifier onlyDeed(address _address) {
        require(
            coinDeedFactory.isDeed(msg.sender) || deedInfo[_address].isValid,
        ...
        returns (uint256)
    {
        ...
        return
            deed.totalBorrow.mul(borrowIndex.sub(deed.borrowIndex)).div(
                10**pool.decimals
            );
    }
    ...
    function claimDToken(address _tokenAddress) external {
        UserAssetInfo storage user = userAssetInfo[msg.sender][_tokenAddress];
        PoolInfo storage pool = poolInfo[_tokenAddress];
        ...
        uint256 rewardToken = user
            .amount
            .mul(pool.supplyIndex.sub(user.supplyIndex))
            .div(10**pool.decimals)
            .mul(REWARD_FACTOR)
            .div(BASE_DENOMINATOR)
            .add(user.pendingDtokenDebt);
        dao.claimDToken(_tokenAddress, msg.sender, rewardToken);
        user.pendingDtokenDebt = 0;
    }
        ...
        nonReentrant
    {
        ...
        if (userAsset.amount > 0) {
            ...
            if (pending > 0) {
                ...
                userAsset.pendingDtokenDebt = userAsset.pendingDtokenDebt.add(pending);
            } //Send reward token
        }
        if (_amount > 0) {
            if (_tokenAddress == address(0)) {
    ...
    function getCash(address _token) internal view returns (uint256) {
        if (_token == address(0)) {
    ...
    function updatePool(address _user, address _token) internal {
        ...
        pool.supplyIndexDebt = pool.supplyIndex;
    }
        ...
        returns (uint256)
    {
        ...
        vars.borrowIndexNew = vars.totalBorrowNew == 0
            ? pool.borrowIndex
