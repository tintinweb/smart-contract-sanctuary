...
contract PredictBTC is ERC20 {
    ...
    // 参加者のアドレスの重複の禁止
    mapping(address => bool) private done_play;
    // 参加者のアドレスの管理
    mapping(uint256 => address) private participant;
    // 参加者の予想した価格の管理、自分で確認できる
    mapping(address => int256) private predict_price;
    // 締め切る日時
    ...
    // 締め切り、スナップショットの間隔
    uint256 public interval = 300; // 5分（デモ用）
    ...
    constructor() ERC20("PredictBitcoinToken", "PBT") {
        ...
        avg = 0;
        index = 0;
        deadline_time = block.timestamp;
        snapshot_time = block.timestamp + interval;
    ...
    function getPredictPrice() public view returns (int256) {
        if(done_play[msg.sender] == false) return 0;
        return predict_price[msg.sender];
    }
    
    // 予想価格を引数に賭ける
    function play(int256 _price) public {
        require(rotation, "not play term");
        require(done_play[msg.sender] == false, "already played");
        done_play[msg.sender] = true;
        index++;
        participant[index] = msg.sender;
        predict_price[msg.sender] = _price;
        // 平均の計算
        avg = (avg * int256(index-1) + _price) / int256(index);
    }
    ...
    function snapshot() public {
        ...
        snapshot_time = snapshot_time + interval;
        deadline_time = deadline_time + interval;
        _mint(msg.sender, 1000 * 10 ** 18);
        // トークン配布
        BTCprice = getLatestBtcJpyPrice();
        for(uint i = 1; i <= index; i++) {
            _mint(participant[i], calculate(BTCprice, predict_price[participant[i]]) * 10 ** 18);
            done_play[participant[i]] = false;
        }
        // 情報をリセット
        index = 0;
        avg = 0;
