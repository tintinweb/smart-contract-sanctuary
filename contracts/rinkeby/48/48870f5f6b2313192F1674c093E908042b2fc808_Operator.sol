...
interface IERC721 {
    ...
    function createCollectible(address from, uint256 tokenId, string memory tokenURI, uint256 fee) external returns (uint256);
...
contract Operator {
    ...
    function mintSingle(address nftAddress, uint256 tokenId, string memory tokenURI, uint256 fee) public onlyMinters returns(bool) {
        IERC721(nftAddress).createCollectible(msg.sender, tokenId, tokenURI, fee);
        return true;
    }
    ...
    function mintMultiple(address nftAddress, uint256 tokenId, string memory tokenURI, uint256 fee, uint256 supply) public onlyMinters returns(bool) {
        IERC1155(nftAddress).mint(msg.sender, tokenId, tokenURI, supply, fee);
        return true;
    }

    function safeTransferMultiple(address nftAddress, uint256 tokenId, address[] memory receivers, uint256[] memory amounts) public onlyMinters returns(bool) {
        require(receivers.length == amounts.length, "ERC1155: ids and amounts length mismatch");
        for(uint i = 0; i < receivers.length; i++) {
            IERC1155(nftAddress).safeTransferFrom(address(this), receivers[i], tokenId, amounts[i], "");
        }
