...
// TODO: rename contract from codename and filename
contract Nelly is ERC721, PaymentSplitter, Ownable {
    ...
    bool internal merkleRootSet;

    uint internal price = 0.1 ether;
    ...
    event AllowListMinted(address indexed _who, uint indexed _amount);
    ...
    constructor(address[] memory _payees, uint256[] memory _shares) ERC721("Nelly", "NELL") PaymentSplitter(_payees, _shares) {
        // _mintChains(2); // TODO: remove
    }
    ...
    function allowListMint(bytes32[] calldata _merkleProof, uint _amount) external payable isNotPaused(allowListMintEnabled) {
        ...
        require(chainsMinted() + _amount < 10_001, 'Exceeds max supply');
        ...
        emit AllowListMinted(msg.sender, _amount);
    }
    ...
    function _mintChains(address _to, uint _amount) internal {
        for(uint i = 0; i < _amount; i++) {
            ...
            _safeMint(_to, chainsMinted());
        }
    }

    function chainsMinted() public view returns (uint) {
    ...
    // A one-time setter function for the allowList Merkle root hash
    function setMerkleRoot(bytes32 _root) public onlyOwner {
        // require(!merkleRootSet, 'Already set'); // TODO: bring back in (and last line)
        merkleRoot = _root;
        // merkleRootSet = true;
    }
    ...
    function setAllowListMintEnabled(bool _val) public onlyOwner {
