...
contract Lottery is Ownable, VRFConsumerBase {
    ...
    uint256 private constant MIN_FEE = 50 * PRECISION; // US$50
    ...
    LotteryState internal currentState;
    ...
    uint256 internal recentRandomness;
    ...
    constructor(
        ...
        uint256 oracleFeeArg,
        address linkTokenArg,
        address vrfCoordinatorArg,
        bytes32 keyHashArg
    ) public VRFConsumerBase(vrfCoordinatorArg, linkTokenArg) {
        usdEntryFee = MIN_FEE;
        priceFeed = AggregatorV3Interface(priceFeedAddressArg);
        currentState = LotteryState.CLOSED;
        recentRandomness = 0;
        recentWinner = address(0x0);

        keyHash = keyHashArg;
        oracleFee = oracleFeeArg;
    }

    function player(uint16 index) public view returns (address) {
    ...
    function entranceFee() public view returns (uint256) {
        // mimimumUSD
        return (usdEntryFee * PRECISION) / usdPrice();
    }
    ...
    function enter() public payable {
        ...
        require(
            ...
            "Lottery isn't open for new participants!"
        );
    ...
    function fulfillRandomness(bytes32 requestIdArg, uint256 randomness)
        internal
        override
    {
        require(
            currentState == LotteryState.CALCULATING_WINNER,
            "Not ready to pick winner yet!"
        );
        require(randomness > 0, "random-not-found");
        recentRandomness = randomness;
        recentWinner = players[recentRandomness % players.length];
