...
contract DungeonsAndDragonsCharacter is ERC721URIStorage, VRFConsumerBase, Ownable {
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber)
        ...
        override
    {
        uint256[] memory expandedValues = new uint256[](7);
        for (uint256 i = 0; i < 7; i++) {
            (expandedValues[i], , ) = randomInRange(uint256(keccak256(abi.encode(randomNumber % 10**3, i))),400,700);
        }

        uint256 newId = characters.length;
        uint256 strength = expandedValues[0];
        uint256 dexterity = expandedValues[1];
        uint256 constitution = expandedValues[2];
        uint256 intelligence = expandedValues[3];
        uint256 wisdom = expandedValues[4];
        uint256 charisma = expandedValues[5];
    ...
    {
        if (num < min) {
            return randomInRange(num + min, min, max);
        }
        if (num > max) {
            return randomInRange(num - max, min, max);
        }
        return (num, min, max);
    }
