...
contract DragonPlayerCharacter is ERC721URIStorage, ERC721Holder, Authorizable, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(uint8 => uint256)) private equippedEquipment;

    constructor(
        address _DCAU,
        address _DRAGON_EQUIPMENT
    ) ERC721("Dragon Player Character", "DCPC") {
        require(_DCAU != address(0) && _DRAGON_EQUIPMENT != address(0), "Dragon: ZERO_ADDRESS");
        DCAU_TOKEN = _DCAU;
        DRAGON_EQUIPMENT = _DRAGON_EQUIPMENT;
    }

    function totalNrOfDragons() external view returns(uint) {
        return _tokenIds.current();
    }

    function getDragonPlayer(uint _id) external view returns(Dragon memory) {
        return dragonPlayers[_id];
    }
    ...
    //     uint256 startHealth = ( endurance * rank ) * HEALTH_MULTIPLIER;
    ...
    function addEquipment(
        ...
        if( _body > 0 && _body != _dragon.equipmentBody )
        {
            if( _dragon.equipmentBody > 0 )
            {
                IERC721( DRAGON_EQUIPMENT ).transferFrom(address(this), msg.sender, _dragon.equipmentBody);
            }

            IERC721( DRAGON_EQUIPMENT ).transferFrom(msg.sender, address(this), _body);

            _dragon.equipmentBody = _body;
            equippedEquipment[_playerId][_dgEquipType.EQUIPMENT_BODY()] = _body;
        }

        if( _hand1 > 0 && _hand1 != _dragon.equipmentHand1 )
        {
            if( _dragon.equipmentHand1 > 0 )
            {
                IERC721( DRAGON_EQUIPMENT ).transferFrom(address(this), msg.sender, _dragon.equipmentHand1);
            }

            IERC721( DRAGON_EQUIPMENT ).transferFrom(msg.sender, address(this), _hand1);
            
            _dragon.equipmentHand1 = _hand1;
            equippedEquipment[_playerId][_dgEquipType.EQUIPMENT_ONE_HANDED()] = _hand1;
        }

        if( _hand2 > 0 && _hand2 != _dragon.equipmentHand2 )
        {
            if( _dragon.equipmentHand2 > 0 )
            {
                IERC721( DRAGON_EQUIPMENT ).transferFrom(address(this), msg.sender, _dragon.equipmentHand2);
            }

            IERC721( DRAGON_EQUIPMENT ).transferFrom(msg.sender, address(this), _hand2);
            
            _dragon.equipmentHand2 = _hand2;
            equippedEquipment[_playerId][_dgEquipType.EQUIPMENT_SHIELD()] = _hand2;
        }

        if( _both > 0 && _both != _dragon.equipmentBoth )
        {
            if( _dragon.equipmentBoth > 0 )
            {
                IERC721( DRAGON_EQUIPMENT ).transferFrom(address(this), msg.sender, _dragon.equipmentBoth);
            }

            IERC721( DRAGON_EQUIPMENT ).transferFrom(msg.sender, address(this), _both);
            
            _dragon.equipmentBoth = _both;
            equippedEquipment[_playerId][_dgEquipType.EQUIPMENT_BOTH_HANDED()] = _both;
        }
    }
...
interface IDragonEquipmentType {
    ...
    function EQUIPMENT_ONE_HANDED() external view returns (uint8);

    function EQUIPMENT_SHIELD() external view returns (uint8);

    function EQUIPMENT_BOTH_HANDED() external view returns (uint8);
}
