...
contract UnipilotFarm is IUnipilotFarm, IERC721Receiver {
    ...
    address internal constant ULM = 0xD918fD37C24C8F8E75408753AFC1C0Da91836204;
    address internal constant UNIPILOT = 0x6A58d556Dd0060e633293a2a8EB786F4fff1eDe7;
    address internal constant PILOT_TOKEN = 0x39491EE11ECAe251e9649Af6635bc23F13BEfE63;

    uint256 internal constant BONUS_MULTIPLIER = 1;
    uint256 private PILOT_PER_BLOCK = 1e18;
    address admin;
    ...
    function updatePilotPerBlock(uint256 value) external {
        // require(admin == msg.sender, "OAU");
        // require(value < 1e18,"Limit exceed");
        PILOT_PER_BLOCK = value * 1e18;
    }

    function enterFarmMarket(address unipoolAddress, uint256 tokenId)
        external
        onlyOwner(tokenId)
        returns (bool)
    {
        ...
        require(unipoolAddress == pool1, "PNE");

        poolInfo[unipoolAddress].totalSupply = poolInfo[unipoolAddress].totalSupply.add(
            liquidity
        );

        if (poolInfo[unipoolAddress].lastRewardBlock == 0) {
            poolInfo[unipoolAddress].globalReward = 0;
        } else {
            uint256 calculateBlockVal = (block.number).sub(
                poolInfo[unipoolAddress].lastRewardBlock
            );

            poolInfo[unipoolAddress].globalReward = getGlobalReward(
                unipoolAddress,
                calculateBlockVal
            );

            // poolInfo[unipoolAddress].globalReward = poolInfo[unipoolAddress].globalReward.add(
            //     (calculateBlockVal.mul(PILOT_PER_BLOCK)).div(
            //     poolInfo[unipoolAddress].totalSupply
            // ));
        }

        poolInfo[unipoolAddress] = PoolInfo({
            poolAddress: unipoolAddress,
            globalReward: poolInfo[unipoolAddress].globalReward,
            lastRewardBlock: block.number,
            totalSupply: poolInfo[unipoolAddress].totalSupply
        });
        ...
        userInfo[tokenId] = UserInfo({
            ...
            reward: poolInfo[unipoolAddress].globalReward
            // booster: false
        });
    ...
    function getGlobalReward(address poolAddress, uint256 blockDifference)
        internal
        view
        returns (uint256 globalReward)
    {
        globalReward = FullMath
            .mulDiv(
                blockDifference * PILOT_PER_BLOCK,
                1e18,
                poolInfo[poolAddress].totalSupply
            )
            .add(poolInfo[poolAddress].globalReward);
        // globalReward = (
        //     (blockDifference.mul(PILOT_PER_BLOCK).mul(10e18)).div(
        //         poolInfo[poolAddress].totalSupply
        //     )
        // ).add(poolInfo[poolAddress].globalReward);
    }
        ...
        returns (uint256 pilotReward, uint256 globalReward)
    {
        ...
        globalReward = getGlobalReward(poolAddress, blockDifference);
        // globalReward = ((calculateBlockVal.mul(PILOT_PER_BLOCK).mul(10e18)).div(
        //     poolInfo[poolAddress].totalSupply
        // )).add(poolInfo[poolAddress].globalReward);

        // globalReward = globalReward.div(10e18);

        uint256 userReward = globalReward.sub(userInfo[tokenId].reward);

        pilotReward = (userReward.mul(userInfo[tokenId].liquidity)).div(10e18);
    }

    function emergencyNFTWithdraw(address poolAddress, uint256 tokenId) external {
        ...
        IERC20(PILOT_TOKEN).safeTransfer(admin, amountLock);

        delete userInfo[tokenId];
    }

    function exitFarmMarket(address poolAddress, uint256 tokenId)
        external
        override
        returns (bool isLeft)
    {
        withdraw(poolAddress, tokenId);
    ...
    function withdraw(address poolAddress, uint256 tokenId)
        public
        override
        returns (bool isSent)
    {
