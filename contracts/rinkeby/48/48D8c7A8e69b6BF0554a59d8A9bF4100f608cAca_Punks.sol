...
contract PunksDNA {
    ...
    // In production use chainlink or other solution.
...
contract Punks is ERC721, ERC721Enumerable, PunksDNA {
        ...
        returns (string memory)
    {
        require(
            _exists(tokenId),
            "ERC721 metada: URI query for not found token"
        );
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "Punks #',
                tokenId.toString(),
                '", "description": "Example description", "image": "',
                image,
                '"}'
            )
        );
    ...
    // Override required for support erc721Enumerable
    function _beforeTokenTransfer(
        address from,
        address to,
        uint256 tokenId
