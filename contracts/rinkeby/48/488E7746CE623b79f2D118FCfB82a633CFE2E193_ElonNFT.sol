...
/**
 * @title ElonNFT
 * ElonNFT - a contract for Professor Elon NFTs
 */
contract ElonNFT is ERC721Tradable {
    using SafeMath for uint256;
    address constant WALLET1 = 0x6e25cFa7e590B0d2a86c3f3d60c78e2816b74dF0;
    ...
    function mint(address to, uint numberOfTokens) public payable {
        ...
        if(preSaleIsActive) {
            require(numberOfTokens <= maxToMintWhitelist, "Exceeds 20 NFTs per wallet pre-sale limit.");
            require(isWhitelisted(to), "Your address is not whitelisted.");
            require(whitelist[to].hasMinted.add(numberOfTokens) <= maxToMintWhitelist, "Exceeds 20 NFTs per wallet pre-sale limit.");
            require(whitelist[to].hasMinted <= maxToMintWhitelist, "Exceeds 20 NFTs per wallet pre-sale limit.");
            whitelist[to].hasMinted = whitelist[to].hasMinted.add(numberOfTokens);
        } else {
            require(numberOfTokens <= maxToMint, "Exceeds 10 NFTs per transaction limit.");
        }
