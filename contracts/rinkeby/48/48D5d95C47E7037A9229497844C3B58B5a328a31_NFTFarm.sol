...
contract NFTFarm is Ownable {
	...
	event NFTClaimed(address nftAddress, uint256 id, uint256 quantity);
	event NFTAdded(address nftAddress, uint256 id, uint256 quantity, uint256 price);
	...
	// Only ERC-1155 NFT (must have approval)
	function addNFT(
		address contractAddress,
		uint256 id,
		uint256 total,
		...
		nftInfo.push(
			NFTInfo({ contractAddress: contractAddress, id: id, remaining: total, price: price })
		);
	...
	function claim(uint256 _nftIndex, uint256 _quantity) public {
		...
		require(nft.remaining > 0, "all NFTs from this index were farmed");
		...
		emit NFTClaimed(nft.contractAddress, nft.id, _quantity);
	}
	...
	//returns the total available NFTs in the contract
