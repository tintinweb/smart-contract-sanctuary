...
contract TestNFT is ERC721, ERC165Storage, IERC2981, Ownable {
  uint256 private _airdropTokenId;
  uint256 private _tokenId = 450;
  ...
  function getCreator(uint256 tokenId) public view returns(address) {
    ...
    if(tokenId <= 450) return address(0xE2D6805E20b2e0f9967D6D2A766b9cC2E383b866);
  ...
  function mint(address creator, address to, string memory tokenUri) external onlyOwner {
    _tokenId++;
  ...
  function airdrop(address[] memory to) external onlyOwner {
    for (uint256 i = 0; i < to.length; i++) {
      _airdropTokenId++;
      require(_airdropTokenId <= 450, "cannot airdrop >450 NFTs");
      _safeMint(to[i], _airdropTokenId);
    }
  }
