...
import "./BondlyAccessControl.sol";
import "./RewardDistributor.sol";

import "hardhat/console.sol";

contract BondlyStaking is Context {
    using SafeMath for uint256;

    IERC20 bondlyToken;
    BondlyAccessControl accessControl;
    ...
    constructor(
        BondlyAccessControl _accessControl,
     ...
     */
    function updateAccessControl(BondlyAccessControl _accessControl) external {
        require(
     ...
     * @return rewardAPY
     */
    function getRewardAPY() public view returns (uint256) {
        return rewardAPY;
    }
     ...
     */
    function updateAPY(uint256 _newApy) public onlyManager {
        rewardAPY = _newApy;
    }
    ...
    function rewardOf(address _staker) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < _stakeDetail.stakes.length; i = i.add(1)) {
            ...
            uint256 _reward =
                _stake.amount.mul(rewardAPY).div(100).mul(timePassed).div(
                    365 days
                );
    ...
    /**
     * @notice stake BONDLY
     * @param _amount is the bondly amount to stake
     */
    function stake(uint256 _amount) public onlyWhitelisted {
        ...
        // require(
        //     bondlyToken.balanceOf(_msgSender()) >= _amount,
        //     "Not enough balance"
        // );
        require(
    ...
    function unstake() public onlyWhitelisted {
        ...
        distributor.distribute(_msgSender(), reward);
        require(
            bondlyToken.transfer(_msgSender(), total),
...
contract BondlyAccessControl is AccessControl {
...
contract RewardDistributor is Context {
    ...
    constructor(IERC20 _bondlyToken, BondlyAccessControl _accessControl) {
        bondlyToken = _bondlyToken;
        accessControl = _accessControl;
    }
    ...
    {
        require(
            bondlyToken.transfer(_beneficiary, _amount),
            "Distribute: BondlyToken.Transfer: Failed to Distribute!"
        );
    ...
    function depositeReward(uint256 _amount) external onlyManager {
        require(
            bondlyToken.transferFrom(_msgSender(), address(this), _amount),
            "Deposite: BondlyToken.TransferFrom: Failed to Deposite!"
        );
    ...
    function withdrawReward(uint256 _amount) external onlyManager {
        require(
            bondlyToken.transfer(_msgSender(), _amount),
            "Withdraw: BondlyToken.TransferFrom: Failed to Withdraw!"
        );
        totalRewards = totalRewards.sub(_amount);
    }
}
