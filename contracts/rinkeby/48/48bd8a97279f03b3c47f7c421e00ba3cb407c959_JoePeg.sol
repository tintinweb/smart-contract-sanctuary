...
// File: contracts/interfaces/IJoeERC20.sol
...
{
    using Strings for uint256;
    uint256 public constant ROYALTY_VALUE = 100; // 1.0% 
    uint256 public constant MAX_MINTABLE = 10000;
    uint256 public constant MAX_PER_CLAIM = 10;
    uint256 public constant MAX_PER_WALLET = 10;
    uint256 public constant CLAIM_PRICE = 0.002 ether;
    uint256 public constant REQUIRED_JOE = 1000; // do we want to 10x this? 
    uint256 public constant REQUIRED_XJOE = 850; // do we want to 10x this?
    bool public canClaim = false;

    /// @dev Placeholder for now
    string baseUri = "https://gateway.pinata.cloud/";
    ...
    event NewJoePegMinted(address sender, uint256 tokenId);
    event Claim(uint256 indexed _id);

    IJoeERC20 public joe; // IJoeERC20(0x6e84a6216eA6dACC71eE8E6b0a5B7322EEbC0fDd);
    IJoeERC20 public xJoe; // 0x57319d41F71E81F3c65F2a47CA4e001EbAFd4F33
    ...
    /// @dev Think of it as an array of 10,000 elements, where we take
    //    a random index, and then we want to make sure we don't
    //    pick it again. If it hasn't been picked, the mapping points to 0, 
    //    otherwise it will point to the index which took its place
    ...
    /// @dev this should have eligibleToMint modifier, left out for testing purposes
    function claim(uint256 n) public payable nonReentrant {
        require(
            canClaim,
            "JoePeg: It's not possible to claim just yet."
        );
        require(
            MAX_MINTABLE - minted > 0,
            "JoePeg: No more JoePegs available to mint."
        );
        require(n > 0, "JoePeg: Must mint at least 1 JoePeg");
        require(n <= MAX_PER_CLAIM);
        require(_minters[msg.sender] + n <= 10, "JoePeg: Trying to mint over allowed amount (10)");

        if (n > MAX_MINTABLE - minted) {
            n = MAX_MINTABLE - minted;
        }
        
        uint256 total_cost = n * CLAIM_PRICE;
        ...
        for (uint256 i = 0; i < n; i++) {
            require(_minters[msg.sender] < MAX_PER_WALLET, "You have minted max limit");
            uint256 nextIndex = minted;
            minted += 1;
            _minters[msg.sender] += 1;

            mintJoePeg(_msgSender(), nextIndex);
            _setTokenRoyalty(nextIndex, address(xJoe), ROYALTY_VALUE);

            emit Claim(nextIndex);
            emit NewJoePegMinted(msg.sender, nextIndex);
        }
    ...
    function mintJoePeg(address receiver, uint256 nextTokenIndex) internal {
        ...
        /// @dev Using this logic for now instead of code above for testing purposes
        _safeMint(receiver, nextTokenIndex);
        tokenURI(nextTokenIndex);
    }

    /// @notice Determines if msg.sender is eligible to mint based on xJoe/Joe balances
    modifier eligibleToMint() {
        require(joe.balanceOf(msg.sender) > REQUIRED_JOE || xJoe.balanceOf(msg.sender) > REQUIRED_XJOE);
        _;
    }

    /// @notice Determines if msg.sender is eligible to mint based on xJoe/Joe balances
    /// @dev to be called by front end
    function userEligibleToMint() public view returns(bool) {
        return joe.balanceOf(msg.sender) > REQUIRED_JOE || xJoe.balanceOf(msg.sender) > REQUIRED_XJOE;
    }

    /// @notice Return number of JoePegs a user has minted so far
    function userNumMinted() public view returns(uint256) {
        ...
        returns (string memory)
    {
        ...
        string memory imageID = "ipfs/QmUEJvxwWrSjnEbUT7Zx6v1Bo8bdab7o4UNgtzrVGkX6gp/1";
        return
            bytes(baseURI).length > 0
                ? string(abi.encodePacked(baseURI, imageID))
                : "";
        // return
        //     bytes(baseURI).length > 0
