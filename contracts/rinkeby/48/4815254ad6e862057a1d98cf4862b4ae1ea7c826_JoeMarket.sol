...
abstract contract ERC2981PerTokenRoyalties is IERC165, IERC2981Royalties {
    struct Royalty {
        address recipient;
        uint256 value;
    }
...
contract JoeMarket is Ownable {
    ...
    constructor(address nftAddress) {
        nftContract = IERC721Full(nftAddress);
        royaltyInterface = IERC2981Royalties(nftAddress);
    }
    ...
    function getUserActiveListingsCount(address account)
        external
        view
        returns (uint256)
    {
        return userActiveListings[account].length;
    }
    ...
    function getUserActiveListings(
        ...
        uint256 numActive = userActiveListings[account].length;
    ...
    function feeInfo(uint256 listingId)
        external
        view
        returns (
            uint256 royaltyFee,
        ...
        )
    {
        ...
        (, royaltyFee) = royaltyInterface.royaltyInfo(
            listing.tokenId,
            listing.price
        );
        marketFee = listing.price.mul(marketFeePercent) / 10000;
        sellerProceeds = listing.price - marketFee - royaltyFee;
    }
    ...
    function fulfillListing(uint256 id) external payable {
        ...
        (address originalMinter, uint256 royaltyFee) = royaltyInterface
            .royaltyInfo(listing.tokenId, listing.price);
        uint256 marketFee = listing.price.mul(marketFeePercent) / 10000;
        uint256 sellerProceeds = listing.price - marketFee - royaltyFee;
        ...
        payable(originalMinter).transfer(royaltyFee);
