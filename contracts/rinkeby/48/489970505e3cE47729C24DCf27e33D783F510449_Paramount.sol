...
contract Paramount is Context, IERC20, Ownable {
    ...
    string private _name = 'Paramount';
    string private _symbol = 'Paramount';
    ...
    address payable private _marketingWalletAddress = payable(0x8d4285831E875C5B76E8C618fF6474737c699453);
    address payable private _developmentWalletAddress = payable(0xA7cFcBf513181D2F11DB2056bfE5DB1a45803982);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _marketingFee == 0 && _developmentFee == 0 && _buyBackFee == 0) return;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tMarketingDevelopmentBuyBack) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tMarketingDevelopmentBuyBack) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tMarketingDevelopmentBuyBack);
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tMarketingDevelopmentBuyBack);
    }

    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        uint256 tFee = tAmount.mul(_taxFee).div(100);
        uint256 tMarketingDevelopmentBuyBack = tAmount.mul(_marketingFee.add(_developmentFee).add(_buyBackFee)).div(100);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tMarketingDevelopmentBuyBack);
        return (tTransferAmount, tFee, tMarketingDevelopmentBuyBack);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tMarketingDevelopmentBuyBack) private view returns (uint256, uint256, uint256) {
        uint256 currentRate = _getRate();
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        // thanks myobu for finding bug here, now everybody need to deploy new contracts lmao..
        uint256 rMarketingDevelopmentBuyBack = tMarketingDevelopmentBuyBack.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rMarketingDevelopmentBuyBack);
