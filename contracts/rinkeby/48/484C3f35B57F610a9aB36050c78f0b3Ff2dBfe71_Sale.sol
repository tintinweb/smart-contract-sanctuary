...
contract Sale is Ownable, AccessControl {
    ...
    enum stages {UNKNOWN, REGISTER, APPROVE, SALE, CLAIM}
    ...
    // saleID => wallet => payment {paid_value, is_claimed}
    mapping(uint256 => mapping(address => payment)) saleAllocations;
    ...
    constructor(address _stakingContract) public {
        ...
        totalSales = 0;
        stakingContract = ISwapStakingContract(_stakingContract);
    ...
    function registerStake(uint256 saleID) public {
        require(getSalestage(saleID) == stages.APPROVE, INCORRECTSTATE);
        require(stakingContract.getCurrentStake(msg.sender) > 0, "Sale:have no stake!");

        // Get current allocation
        uint256 curAllocation = saleAllocations[saleID][msg.sender].paid;
        // Get current stake
        uint256 curStake = stakingContract.getCurrentStake(msg.sender);

        require(curStake > curAllocation, "Sale:only add stake!");

        saleAllocations[saleID][msg.sender].paid = saleAllocations[saleID][msg.sender].paid.add(
            curStake.sub(curAllocation)
        );
        sales[saleID].stakeAllocationAmount = sales[saleID].stakeAllocationAmount.add(curStake.sub(curAllocation));

        emit SaleAllocAdded(msg.sender, saleID, curStake.sub(curAllocation));
    }

    /**
     * @dev register token sale
    ...
    function registerSale(uint256 saleID, uint256 paymentAmount) public {
        ...
        require(salePayments[saleID][msg.sender] == 0, "Sale:only one");
        require(
            paymentAmount <=
                (stakingContract.getCurrentStake(msg.sender).mul(1e9).div(stakingContract.getCurrentTotalStake()))
        ...
        salePayments[saleID][msg.sender] = paymentAmount;
    ...
    function claim(uint256 saleID) public {
        ...
        (uint256 purchasedTokens, ) = viewSale(saleID);
    ...
    function getSaleRoleList() public onlyOwner view returns(address[] memory saleAdmins) {
    ...
    function viewSale(uint256 saleID) public view returns (uint256 previewTokens, bool claimed) {
        if (getSalestage(saleID) == stages.SALE || getSalestage(saleID) == stages.CLAIM) {
            return (
                salePayments[saleID][msg.sender].mul(1e3).div(sales[saleID].tokenPrice),
                saleAllocations[saleID][msg.sender].claimed
            );
        } else {
            return (0, saleAllocations[saleID][msg.sender].claimed);
        }
    }
    ...
    {
        return (
            sales[saleID].whiteListAllocationAmount,
