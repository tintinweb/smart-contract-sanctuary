...
contract NFT is ERC721Enumerable, Ownable {
  ...
  constructor(
    ...
    setCollectionURI(_CollectionURL);
...
// Mainly for Sales Order use
  function mintto(address _to, uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
        if(onlyWhitelisted == true) {
            require(isWhitelisted(msg.sender), "user is not whitelisted");
            uint256 ownerMintedCount = addressMintedBalance[_to];  // Is it necessary to control total NFTs brought per account?
            require(ownerMintedCount + _mintAmount <= nftPerAddressLimit, "max NFT per address exceeded");
        }
        require(msg.value >= cost * _mintAmount, "insufficient funds");
    }
  ...
  function contractURI() public pure returns (string memory) {
    return "https://colorwords.herokuapp.com/coninfo";
  }
