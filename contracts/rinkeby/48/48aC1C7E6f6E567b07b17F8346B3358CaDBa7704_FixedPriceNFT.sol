...
contract FixedPriceNFT is Configurable, IERC721ReceiverUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    bytes32 internal constant TxFeeRatio = bytes32("FPNFT::TxFeeRatio");
    bytes32 internal constant TotalBonusDays = bytes32("FPNFT::TotalBonusDays");
    bytes32 internal constant FeeAccount = bytes32("FPNFT::FeeAccount");
    ...
    function initialize01() public initializer {
        require(msg.sender == governor || governor == address(0), "invalid governor");
    ...
    function close(uint256 index) external isPoolNotClosed(index) isPoolNotSwap(index) {
        require(isCreator(msg.sender, index), "is not creator");
        pools[index].closeAt = block.timestamp - 1;
    }

    function creatorClaim(uint256 index) external
    isPoolExist(index)
    isPoolClosed(index)
    isPoolNotSwap(index)
    {
