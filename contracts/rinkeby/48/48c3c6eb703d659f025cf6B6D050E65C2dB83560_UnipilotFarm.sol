...
contract UnipilotFarm is IUnipilotFarm, ReentrancyGuard, IERC721Receiver {
    ...
    modifier isRewardActive(address pool) {
        require(poolInfo[pool].isRewardActive == true, "RNA");
        _;
    }

    address private ULM;
    address private UNIPILOT;
    address private PILOT_TOKEN;
    ...
    function withdrawNFT(uint256 tokenId) external override {
        address pool = userInfo[tokenId].pool;
        withdrawReward(tokenId);
        poolInfo[pool].totalLockedLiquidity = poolInfo[pool].totalLockedLiquidity.sub(
        ...
        if (poolInfo[pool].totalLockedLiquidity == 0) {
            poolInfo[pool].startBlock = block.number;
            poolInfo[pool].lastRewardBlock = block.number;
            poolInfo[pool].globalReward = 0;

            poolAltInfo[pool].startAltBlock = block.number;
            poolAltInfo[pool].lastAltRewardBlock = block.number;
            poolAltInfo[pool].altGlobalReward = 0;
        }

        uint256 index = callIndex(pool, tokenId);

        remove(index, userInfo[tokenId].user, userInfo[tokenId].pool);
        // delete userToPoolToTokenIds[msg.sender][pool][index];
    ...
    function emergencyNFTWithdraw(uint256 tokenId) external {
        ...
        poolInfo[userInfo[tokenId].pool].totalLockedLiquidity = poolInfo[
            userInfo[tokenId].pool
        ].totalLockedLiquidity.sub(userInfo[tokenId].liquidity);
        ...
        if (poolInfo[userInfo[tokenId].pool].totalLockedLiquidity == 0) {
            poolInfo[userInfo[tokenId].pool].startBlock = block.number;
            poolInfo[userInfo[tokenId].pool].lastRewardBlock = block.number;
            poolInfo[userInfo[tokenId].pool].globalReward = 0;

            poolAltInfo[userInfo[tokenId].pool].startAltBlock = block.number;
            poolAltInfo[userInfo[tokenId].pool].lastAltRewardBlock = block.number;
            poolAltInfo[userInfo[tokenId].pool].altGlobalReward = 0;
        }
    ...
    function updatePilotPerBlock(address pool, uint256 value)
        external
        override
        onlyGovernance
        returns (uint256)
    {
        ...
        if (poolInfo[pool].totalLockedLiquidity > 0) {
            ...
            poolInfo[pool].globalReward = currentGlobalReward;
        }
        uint256 old = poolInfo[pool].rewardPerBlock;
        poolInfo[pool].rewardPerBlock = value;
        emit UpdatePilotPerBlock(pool, old, poolInfo[pool].rewardPerBlock);
        return poolInfo[pool].rewardPerBlock;
    }

    function updateMultiplier(address pool, uint256 value)
        external
        override
        onlyGovernance
    {
        ...
        uint256 old = poolInfo[pool].rewardMultiplier;
        poolInfo[pool].rewardMultiplier = value;
        emit UpdateMultiplier(pool, old, poolInfo[pool].rewardMultiplier);
    }

    /// @notice User total nft(s) with respect to pool
    /// @param userAddress particular user address
    /// @param pool particular pool address
    ...
    {
        tokenCount = userToPoolToTokenIds[userAddress][pool].length;
        tokenIds = userToPoolToTokenIds[userAddress][pool];
    }
        ...
        returns (bool)
    {
        ...
        (, address pool, uint256 liquidity, , , , ) = IUniswapLiquidityManager(ULM)
            .positions(tokenId);

        require(poolWhitelist[pool] == true, "PNW");

        require(liquidity > 0, "ZL");

        // uint256 lastGR = poolInfo[pool].globalReward;

        // if (poolInfo[pool].lastRewardBlock == poolInfo[pool].startBlock) {
        //     poolInfo[pool].globalReward = 0;
        // } else
        if (poolInfo[pool].lastRewardBlock != poolInfo[pool].startBlock) {
            uint256 blockDifference = (block.number).sub(poolInfo[pool].lastRewardBlock);

            poolInfo[pool].globalReward = getGlobalReward(
                pool,
                blockDifference,
                poolInfo[pool].rewardPerBlock,
                poolInfo[pool].rewardMultiplier,
                poolInfo[pool].globalReward
            );
        }

        poolInfo[pool].totalLockedLiquidity = poolInfo[pool].totalLockedLiquidity.add(
            liquidity
        );
        ...
        userInfo[tokenId] = UserInfo({
            ...
            liquidity: liquidity,
            user: sender,
            reward: poolInfo[pool].globalReward,
            altReward: userInfo[tokenId].altReward
        });

        userToPoolToTokenIds[sender][pool].push(userInfo[tokenId].tokenId);

        farmingActive[tokenId] = true; // user's farming active

        // globalFarmingReward = globalFarmingReward.add(
        //     poolInfo[pool].globalReward.sub(lastGR)
        // );

        IERC721(UNIPILOT).safeTransferFrom(sender, address(this), tokenId);

        if (poolInfo[pool].isAltActive == true) {
            altDeposit(pool, tokenId);
        }
        ...
        isRewardActive(userInfo[tokenId].pool)
    {
        ...
        require(poolWhitelist[pool] == true, "PIBL");
        ...
        // uint256 lastGR = poolInfo[pool].globalReward;
        poolInfo[pool].globalReward = globalReward;
        poolInfo[pool].lastRewardBlock = block.number;
    ...
    /// @param pool pool address
    function insertPool(address pool) internal {
        ...
        poolInfo[pool] = PoolInfo({
            ...
            rewardPerBlock: DEFAULT_PILOT_PER_BLOCK,
            isRewardActive: true,
            isAltActive: poolInfo[pool].isAltActive
        });
        ...
        emit NewPool(
            ...
            poolInfo[pool].rewardPerBlock,
            poolInfo[pool].rewardMultiplier,
            poolInfo[pool].lastRewardBlock,
            poolWhitelist[pool]
        );
    }

    /// @notice alt token Updating state
    /// @param pool pool address for alt token
    function toggleActiveAlt(address pool, address altTokenAddress)
        external
        onlyGovernance
        returns (bool)
    {
        emit UpdateActiveAlt(
            poolInfo[pool].isAltActive,
            !poolInfo[pool].isAltActive,
            pool,
            altTokenAddress
        );
        if (poolInfo[pool].isAltActive && altTokenAddress == address(0)) {
            poolInfo[pool].isAltActive = !poolInfo[pool].isAltActive;
            poolAltInfo[pool].altToken = altTokenAddress;
        } else {
            poolInfo[pool].isAltActive = true;

            poolAltInfo[pool] = PoolAltInfo({
                ...
                startAltBlock: block.number
            });
        }

        return poolInfo[pool].isAltActive;
    }

    function altDeposit(address pool, uint256 tokenId) internal {
        // if (
        //     poolInfo[pool].lastAltRewardBlock == poolInfo[pool].startBlock
        // ) {
        //     poolInfo[pool].altGlobalReward = 0;
        // } else {
        if (poolAltInfo[pool].lastAltRewardBlock != poolAltInfo[pool].startAltBlock) {
            uint256 blockDifference = (block.number).sub(
                poolAltInfo[pool].lastAltRewardBlock
            );

            poolAltInfo[pool].altGlobalReward = getGlobalReward(
                pool,
                blockDifference,
                poolAltInfo[pool].altRewardPerBlock,
                poolAltInfo[pool].altRewardMultiplier,
                poolAltInfo[pool].altGlobalReward
            );
        }

        poolAltInfo[pool].lastAltRewardBlock = block.number;

        userInfo[tokenId].altReward = poolAltInfo[pool].altGlobalReward;
    }

    function callIndex(address pool, uint256 tokenId)
        internal
        view
        returns (uint256 index)
    {
        ...
        for (uint256 i = 0; i <= tokens.length; i++) {
            if (tokenId == userToPoolToTokenIds[msg.sender][pool][i]) {
                index = i;
                break;
            }
        }
    ...
    function remove(
        ...
        uint256 temp = userToPoolToTokenIds[user][pool][
            userToPoolToTokenIds[user][pool].length - 1
        ];
        ...
        returns (uint256 altReward, uint256 altGlobalReward)
    {
        ...
        if (!isFarmingActive && block.number > farmingGrowthBlockLimit) {
            altGlobalReward = poolAltInfo[pool].altGlobalReward;
        } else {
    ...
    function altWithdraw(uint256 tokenId) internal {
        ...
        require(
            IERC20(poolAltInfo[pool].altToken).balanceOf(address(this)) >= altReward,
            "IF"
        );
        poolAltInfo[pool].altGlobalReward = altGlobalReward;
        userInfo[tokenId].altReward = altGlobalReward;
        IERC20(poolAltInfo[pool].altToken).safeTransfer(
            userInfo[tokenId].user,
            altReward
        );
        poolAltInfo[pool].lastAltRewardBlock = block.number;
    ...
    function updateAltPerBlock(address pool, uint256 value)
        external
        onlyGovernance
        returns (uint256)
    {
        PoolAltInfo storage poolState = poolAltInfo[pool];
        if (poolInfo[pool].totalLockedLiquidity > 0) {
            uint256 currentGlobalReward = getGlobalReward(
                pool,
            ...
            poolAltInfo[pool].altGlobalReward = currentGlobalReward;
        }
        uint256 old = poolAltInfo[pool].altRewardPerBlock;
        poolAltInfo[pool].altRewardPerBlock = value;
        emit UpdateAltPerBlock(pool, old, poolAltInfo[pool].altRewardPerBlock);
        return poolAltInfo[pool].altRewardPerBlock;
    }

    function updateAltMultiplier(address pool, uint256 value)
        external
        onlyGovernance
        returns (uint256)
    {
        ...
        uint256 old = poolAltInfo[pool].altRewardMultiplier;
        poolAltInfo[pool].altRewardMultiplier = value;
        emit UpdateAltMultiplier(pool, old, poolAltInfo[pool].altRewardMultiplier);
        return poolAltInfo[pool].altRewardMultiplier;
    }

    function toggleRewardStatus(address pool) external override onlyGovernance {
        bool currentStatus = poolInfo[pool].isRewardActive;
        poolInfo[pool].isRewardActive = !poolInfo[pool].isRewardActive;

        emit RewardStatus(pool, currentStatus, poolInfo[pool].isRewardActive);
    }
...
interface IUnipilotFarm {
    ...
    event UpdateAltPerBlock(address pool, uint256 old, uint256 updated);

    event UpdateAltMultiplier(address pool, uint256 old, uint256 updated);

    event UpdateActiveAlt(bool old, bool updated, address pool, address altToken);

    event UpdateFarmingLimit(uint256 old, uint256 updated);

    event UpdateAltToken(address pool, address old, address updated);
    ...
    function updatePilotPerBlock(address pool, uint256 value) external returns (uint256);
