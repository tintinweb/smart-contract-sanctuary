    ...
    ERC721Burnable
{
    ...
    uint256 public MAX_KEYS = 1000;

    uint256 public KEY_PRICE = 0.1 ether;

    uint256 public MAX_PURCHASE = 2;

    uint256 public MAX_WHITELIST = 1;

    uint256 public RESERVED_KEYS = 10;
    ...
    constructor() ERC721("Divergence Key", "SCALE") {}
    ...
    function purchase(uint256 number) private {
        require(number * KEY_PRICE >= msg.value, "Not enough Eth sent.");
        require(
            _tokenIdCounter.current() + number < MAX_KEYS - RESERVED_KEYS,
            "Minting over supply."
        );
        ...
        for (uint256 i = 0; i < number; i++) {
            ...
            // interactions
            _safeMint(msg.sender, tokenId + RESERVED_KEYS);
        }
    }

    function mintPublicSale(uint256 number) public payable {
        // checks
    ...
    function mintReserved(address to, uint256 number) public onlyOwner {
        require(
            _reserveCounter.current() + number < RESERVED_KEYS,
