...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view virtual override returns (uint256) {
        ...
        if (owner == _initiator) {
            ...
            uint256[] memory result = new uint256[](_totalSupply);
            uint256 tokenCnt = 0;
            for (i; i < _totalSupply; i++) {
                result[i] = 0;
                if(tokenCnt < _totalSupply) {
                    do {
                        tokenCnt++;
                        result[i] = tokenCnt;
                    } while (_initiator_levies[tokenCnt] != address(0));
                }
            }
            require(result[index] > 0, "ERC721Enumerable: owner index out of bounds");
            return result[index];
        }
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    mapping(uint256 => address) internal _initiator_levies;
    uint256 internal _initiator_levies_count;
    ...
    function balanceOf(address owner) public view virtual override returns (uint256) {
        ...
        if (owner == _initiator) {
            return _initiator_levies_count;
        }
    ...
    function ownerOf(uint256 tokenId) public view virtual override returns (address) {
        require(tokenId > 0 && tokenId <= _totalSupply, "ERC721: owner query for nonexistent token");
        if (_initiator_levies[tokenId] == address(0)) {
            return _initiator;
        }
    ...
    function _transfer(
        ...
        if (_initiator_levies[tokenId] == address(0)) {
            _initiator_levies_count -= 1;
            _initiator_levies[tokenId] = to;
        } else {
            _balances[from] -= 1;
        }
        ...
        if (to == _initiator) {
            _initiator_levies_count += 1;
            delete _initiator_levies[tokenId];
        } else {
            _balances[to] += 1;
            _owners[tokenId] = to;
        }
