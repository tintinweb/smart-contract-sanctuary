...
contract SpaceMonkeysMintPass is ERC721 {
    ...
    // disables mint pass sale
    bool private disabled;

    event NewMintPass(uint id);
    event Withdrawn(address _address, uint amount);
    
    // track token ID
    Counters.Counter private _tokenId;

    ISpaceMonkeys private spaceMonkeysContract = ISpaceMonkeys(spaceMonkeysContractAddress);

    constructor() ERC721("Space Monkeys Mint Pass", "SMMP") { }
    ...
    function buyMintPass() public payable {
        require(!disabled, "Mint pass selling phase is over!");
        require(!(balanceOf(msg.sender) > 0), "Cannot buy more than one mint pass!");
        require(_tokenId.current() < 8000, "Mint pass sold out!");
        require(msg.value == 77700000000000000, "Ether value sent is not correct");

        _safeMint(msg.sender, _tokenId.current());
        emit NewMintPass(_tokenId.current());
        _tokenId.increment();
    }

    function toggleDisableSale() external {
        require(msg.sender == spaceMonkeysContractAddress, "This function can only be called by the Space Monkeys contract!");
        disabled = !disabled;
    }

    function getSaleDisabledState() external view returns(bool) {
        return disabled;
    }
