...
contract Misfit is ERC721Enumerable, EIP712, Ownable {
    ...
    /// @dev Client's redeem ID to check if voucher redeemed
    mapping(address => uint256) private _clientRedeemId;

    /// @notice Sale ID to control sale stage
    ...
    function redeem(NFTVoucher calldata voucher, bytes calldata signature, uint numberWantToBuy)
        external
        payable
    {
        ...
        // make sure this voucher hasn't been redeemed
        require(
            _clientRedeemId[_msgSender()] < voucher.saleId,
        ...
        console.log("before mint",numberOfTokens);
        ...
        // Update client's redeem ID
        _clientRedeemId[_msgSender()] = currentSaleId;
        ...
        view
    {
        bytes32 digest = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256(
                        "NFTVoucher(address client,uint256 saleId,uint256 price,uint256[] tokenIds)"
                    ),
                    _msgSender(),
                    currentSaleId,
                    voucher.price,
                    keccak256(abi.encodePacked(voucher.tokenIds)) //encodeData hashes the array itself first
    ...
    /// @notice Set sale ID to control sale stage (set 0 to disable saling)
    /// @param newSaleId New sale ID, should be set to currentSaleId+1
    function setSaleId(uint256 newSaleId) external onlyOwner {
