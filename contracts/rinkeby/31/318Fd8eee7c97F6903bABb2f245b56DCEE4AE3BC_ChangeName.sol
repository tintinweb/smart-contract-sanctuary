...
contract ChangeName is Ownable {
    ...
    uint256 public constant NAME_CHANGE_PRICE = 77 * (10 ** 18);
    IERC721 public woolly;
    ...
    function changeName(uint256 tokenId, string memory newName, string memory _tag, uint256 _price) public {
        address owner = woolly.ownerOf(tokenId);
        require(tokenAddress != address(0), "");
        require(shepherd != address(0) && IERC721(shepherd).balanceOf(owner) > 0, "");
        ...
        if(_price >= NAME_CHANGE_PRICE) {
            IERC20(tokenAddress).transferFrom(msg.sender, address(this), NAME_CHANGE_PRICE);
            IERC20(tokenAddress).transfer(vault, NAME_CHANGE_PRICE);
        }
    ...
    function setWoolly(IERC721 _woolly) public onlyOwner {
        woolly = _woolly;
    }
