...
contract MoonrayPresalePass is MoonrayPresalePassBase {
    constructor()
        MoonrayPresalePassBase('https://gateway.pinata.cloud/ipfs/QmV4HKGmFpPV11vA1VE5CmsXy2J8CAzneQdDQk9CuvLw1G/1.json', 1, 9000, 0)
    {
        //'https://gateway.pinata.cloud/ipfs/QmV4HKGmFpPV11vA1VE5CmsXy2J8CAzneQdDQk9CuvLw1G/{id}.json' is not well supported by OpenSea.
    ...
    NFTCWhitelist
{
    ...
    function mintPassesFromWhitelist(uint256 count) external payable {
        ...
        require(count <= _whitelist[_msgSender()], 'Too many');

        _mintTokens(_msgSender(), count);

        _whitelist[_msgSender()] -= count;
    }
    ...
    function _mintPassFromClaim(
        ...
        for (uint256 i = 0; i < claims.length; i++) {
            require(
                _validateClaim(
                    claims[i],
                    _generateLeaf(minter, _nextClaimIndex[minter] + i)
                ),
                'Claim invalid'
            );
        }
        ...
        _nextClaimIndex[_msgSender()] += count;
    }
...
contract NFTCClaimlist is Ownable {
    ...
    function checkNextClaim(address wallet, bytes32[] memory claim)
        external
        view
        returns (bool)
    {
        return
            claim.verify(
                _masterClaim,
                _generateLeaf(wallet, _nextClaimIndex[wallet])
            );
    }
...
contract NFTCWhitelist is Ownable {
    ...
    function getPassesAvailable(address wallet)
        external
        view
        returns (uint256)
    {
