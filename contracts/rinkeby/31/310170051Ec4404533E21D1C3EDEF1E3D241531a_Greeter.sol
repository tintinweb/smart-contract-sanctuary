...
/**
    How this contract works:
      ...
      - 5001-10K: only non-Tune owners can mint (remixes)
...
contract Greeter is ERC721 {
  ...
  modifier lessThanTuneSupply(uint256 tokenId) {
    require(tokenId <= MAX_ORIGINAL_SUPPLY, "Too high");
    require(tokenId > 0, "Cannot be 0");
    _;
  }
  ...
  function mintOriginalTrack(
    ...
    uint256 tune_token_id
  ) public lessThanTuneSupply(tune_token_id) returns (uint256 token_id) {
    require(hasLength(track_name), "Need a track name");
    require(hasLength(audio_uri), "Need an audio URI");
    require(
      tunes.ownerOf(tune_token_id) == msg.sender,
  ...
  function tuneTokenId(uint256 token_id) public view returns (uint256) {
    return
      token_id <= MAX_ORIGINAL_SUPPLY
        ? _original_tracks[token_id].tune_token_id
        : _remix_tracks[token_id].tune_token_id;
  }

  function isOriginal(uint256 token_id) public pure returns (bool) {
    return token_id <= MAX_ORIGINAL_SUPPLY;
  }
    ...
    returns (string memory)
  {
    ...
    Track memory track = getTrack(token_id);
    require(track.tune_token_id > 0, "Not minted yet");
  ...
  function _burn(uint256 token_id) internal override(ERC721) {
