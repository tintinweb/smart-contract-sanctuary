...
contract BrainDanceNft is ERC721, Ownable, HeroFactory {
    ...
    function mintBreedToken(
        ...
        , uint256 heroId1_
        , uint256 heroId2_
    ) public payable verifySignatureModifier(sign) {
    ...
    modifier verifySignatureModifier(uint256 sign) {
        uint256 m = sign * sign % _signToken;
        m = m * sign % _signToken;
        require (block.timestamp >= m && block.timestamp - m < 200, "permission error");
        _;
...
contract HeroFactory {
    ...
    function _mintHero(uint256 tokenId_) internal {
        _heros[tokenId_] = Hero(0, block.timestamp, MAX_INT, MAX_INT, new uint256[](0));
    }

    function _breedHero(uint256 heroId1_, uint256 heroId2_, uint256 tokenId_) internal {
        // create a child
        _heros[tokenId_] = Hero(0, block.timestamp, heroId1_, heroId2_, new uint256[](0));
