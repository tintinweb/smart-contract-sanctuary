...
contract NftToken is ERC721 {
    ...
    constructor (string memory _uri) ERC721("Cosmic NftToken", "COSMICNFT"){
        defaultUri = _uri;
        owner = msg.sender;
    }
    ...
    function createItemWithUri(string memory uri) public returns (uint256){
        _tokenIds.increment();
    ...
    function createDefaultItem(address creator) internal {
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        _safeMint(creator, newItemId);
        Items[newItemId] = Item(newItemId, creator, defaultUri);
    }

    function airDropNfts(address[] memory addresses) public onlyOwner {
        for (uint256 index = 0; index < addresses.length; index++) {
            createDefaultItem(addresses[index]);
        }
    }
