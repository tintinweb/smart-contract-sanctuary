...
contract WebacyMembership is Ownable {
    ...
    function payMembership() external {
        require(webacyAddress != address(0), "Webacy address is not seted");
        require(address(proxyFactory.deployedContractFromMember(msg.sender)) == address(0), "Sender already has a Proxy deployed");
        proxyFactory.createProxyContract(msg.sender, webacyAddress);
    ...
    function setProxyFactory(address _address) external onlyOwner() {
       proxyFactory = WebacyProxyFactory(_address);
    }

    function setWebacyAddress(address _address) external onlyOwner() {
...
interface IWebacyProxyFactory {
  function createProxyContract(address _memberAddress, address _webacyAddress) external;
...
contract WebacyProxyFactory is IWebacyProxyFactory {
    mapping(address => WebacyProxy) _memberToContract;
    ...
    function createProxyContract(address _memberAddress, address _webacyAddress) external override {
        WebacyProxy webacyProxy = new WebacyProxy(_memberAddress, _webacyAddress);
        _memberToContract[_memberAddress] = webacyProxy;
    }

    function deployedContractFromMember(address _memberAddress) external view override returns (WebacyProxy) {
        return _memberToContract[_memberAddress];
    }
}
