...
contract MrEverybody is ERC721, ERC721Enumerable,ERC721URIStorage, Ownable {
   using Counters for Counters.Counter;
    ...
    uint256 public _next24hour = 300;           // 86400 24 hour
    uint256 public _next12hour = 300;           // 43200 12 hour
    
    uint256 public _privateSalePrice =  6E16;   //0.06 ETH per NFT initiallly it should be
    uint256 public _publicSalePrice =  8E16;   //0.08 ETH per NFT it should be
    
    SodaPassNFT  soda_contract;
    
    address sodaContract = 0x33442D6a34CaE989bACAB8234322916E146cE843;
    
    mapping(address => uint256) public countForAddress;


    constructor() ERC721("Mister Everybody", "MEB") {
    ...
    function mint(uint8 _quantityToMint) public payable {
       ...
       require((_quantityToMint + totalSupply()) <= maxSupply,"Exceeds maximum supply");
        ...
        if(block.timestamp <= (_startDate + _next24hour))
        {
            ...
            require(msg.value == (_privateSalePrice * _quantityToMint),"Ether submitted does not match current price");
       ...
       else if(block.timestamp > (_startDate + _next24hour) && block.timestamp < (_startDate + _next24hour + _next12hour)){
            require(getSodaPassBalance(msg.sender) > 0 , "Needs to have SodaPass");
            require(msg.value == (_privateSalePrice * _quantityToMint),"Ether submitted does not match current price");
             ...
             require(msg.value == (_publicSalePrice * _quantityToMint),"Ether submitted does not match current price");
    ...
    function withdraw() public onlyOwner {
        ...
        uint256 _balance = address(this).balance;
        address _coldWallet = 0x06b639618826CE6882556e15b834E86913b3fDd9;
        payable(_coldWallet).transfer((_balance * 6)/10);
        
        address _devWallet = 0xBb165CdE6A1c60a10aed6135d9295788016030B8;
        payable(_devWallet).transfer((_balance * 3)/10);
        
        address _devWallet2 = 0x24AE65e5DeeCfcFC8415D64e6b1891C5FFF90374;
