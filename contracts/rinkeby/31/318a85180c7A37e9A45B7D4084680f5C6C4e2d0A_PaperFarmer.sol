...
contract PaperFarmer is Ownable {
    using SafeMath for uint256;
    ...
    function deposit(uint8 _pid, uint256 _amount) public {
        ...
        if (user.balance > 0) {
            user.rewards = user.rewards.add(getPending(_pid, msg.sender));
        }
        pool.balance = pool.balance.add(_amount);

        user.balance = user.balance.add(_amount);
        user.loss = user.balance.mul(getPending(_pid, msg.sender));
    ...
    function withdraw(uint8 _pid) public {
        ...
        pool.accPerShare = getAccPerShare(_pid).add(user.rewards.mul(1e12).div(pool.balance));
        pool.lastBlock = getLastBlockNumber();
    ...
    function makeHarvest(uint256 _pid) internal returns(uint256){
        ...
        uint256 reward = getReward(_pid, msg.sender);
        
        if (!(reward > 0)) {
            return 0;
        }
        paperToken.safeTransfer(
            msg.sender,
            reward
        );
        user.rewards = user.rewards.sub(reward);
    ...
    function getAccPerShare(uint256 _pid) public view returns(uint256) {
        ...
        return pool.accPerShare.add(
            blockNumber.sub(pool.lastBlock).mul(tokensPerBlock).mul(1e12).mul(pool.share).div(totalShare).div(pool.balance)
        );
    }
    ...
    function getReward(uint256 _pid, address _user) public view returns(uint256) {
        User storage user = users[_pid][_user];
        uint256 hodl = block.timestamp - user.date;
        if (hodl < 10 minutes) {
            return 0;
        }
        if (hodl < 12 minutes) {
            return user.rewards.mul(25).div(100);
        }
        if (hodl < 14 minutes) {
            return user.rewards.mul(50).div(100);
        }
        if (hodl < 16 minutes) {
            return user.rewards.mul(75).div(100);
        }
        return user.rewards;
    }

    function safeRewardTransfer(address _user, uint256 _amount) internal {
        uint256 balance = paperToken.balanceOf(address(this)).sub(pools[paperPool].balance);
        if (_amount > balance) {
    ...
    function getBlockchainData()
        public
        view
        returns (uint256 _block, uint256 _timestamp)
    {
        _block = block.number;
