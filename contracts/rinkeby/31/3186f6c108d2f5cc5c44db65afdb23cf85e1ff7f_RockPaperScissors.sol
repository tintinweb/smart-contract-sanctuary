...
contract RockPaperScissors {
  string private constant ERROR_ROUND_DOES_NOT_EXIST =
  ...
  string private constant ERROR_MOVE_ALREADY_REVEALED =
    'ERROR_MOVE_ALREADY_REVEALED';
  string private constant ERROR_COMMITTED_MOVE_REFUSED =
    'ERROR_COMMITTED_MOVE_REFUSED';
  string private constant ERROR_INVALID_HASHING_SALT =
    'ERROR_INVALID_HASHING_SALT';
  string private constant ERROR_INVALID_REVEALED_MOVE =
    'ERROR_INVALID_REVEALED_MOVE';
   ...
   */
  function _isValidRevealedMove(Round storage _round, uint8 _revealedMove)
    internal
    view
    returns (bool)
  {
    return
      _revealedMove > REVEALED_MOVE_MISSING &&
      _revealedMove <= _round.maxAllowedMoves;
  }
  ...
  function reveal(
    ...
    require(
      _isValidRevealedMove(round, _revealedMove),
      ERROR_INVALID_REVEALED_MOVE
    );
   ...
   */
  function _ensurePlayerJoined(uint256 _roundId, address _player)
    internal
    view
  {
    Round storage round = roundRecords[_roundId];
    require(
      round.bob == _player || round.alice == _player,
      ERROR_ROUND_PLAYER_DOES_NOT_EXIST
    );
  }
   ...
   */
  function getWinningMove(uint256 _roundId)
    external
    view
    roundExists(_roundId)
    returns (uint8)
  {
    ...
    return
      winningMove == REVEALED_MOVE_MISSING
        ? REVEALED_MOVE_MISSING
        : winningMove;
  }
   ...
   * @return Hashed move
   */
  function hashMove(uint8 _commitedMove, bytes32 _salt)
    external
    pure
    returns (bytes32)
  {
   ...
   */
  function _hashMove(uint8 _commitedMove, bytes32 _salt)
    internal
    pure
    returns (bytes32)
  {
   ...
   */
  function _checkValidSalt(
    CastedMove storage _castedMove,
    uint8 _commitedMove,
    bytes32 _salt
  ) internal view {
    require(
      _castedMove.revealedMove == REVEALED_MOVE_MISSING,
      ERROR_MOVE_ALREADY_REVEALED
    );
    require(
      _castedMove.committedMove == _hashMove(_commitedMove, _salt),
      ERROR_INVALID_HASHING_SALT
    );
  }
}