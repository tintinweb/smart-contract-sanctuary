...
contract WavepoolICO is Crowdsale, TimedCrowdsale, WhitelistedRole, Ownable {
  ...
  uint256 public cap = 10000000000000000000;
  ...
  function closePresale() public onlyOwner {
    require(stage == ICOStage.PRESALE, "Presale already closed");
    stage = ICOStage.ICO;
  }
  ...
  function _preValidatePurchase(address _beneficiary, uint256 _weiAmount) internal view {
    if (stage == ICOStage.PRESALE) {
      require(isWhitelisted(_beneficiary), "Wallet address has not been whitelisted");
    }

    uint256 _existingContribution = contributions[_beneficiary];
    uint256 _newContribution = _existingContribution.add(_weiAmount);
    require(_newContribution <= cap, "Maximum investment amount exceeded");
