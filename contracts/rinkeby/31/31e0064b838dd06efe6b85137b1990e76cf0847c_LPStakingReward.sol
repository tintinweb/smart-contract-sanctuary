...
interface ILPStakingRewards {
    ...
    // TODO - add a function for user to query list of LP tokens he staked
     ...
     * @return Accumulated rewards of addr holder (in wei)
     */
    function rewardOf(address addr, address lpAddr) external view returns (uint256);

    // TODO - when staking, revert if a pool is not added for lpToken adddress
    ...
    // TODO - add a function for user to claim all his rewards (from all pools)

    /**
     * @notice A method to allow a rhoToken holder to claim his rewards from all LP staking
     * @param lpAddr Address of LP Token contract
    ...
    // TODO - when withdraw, revert if no pool is registered for lpToken addr
    ...
    // TODO - when exit, revert if no pool is registered for lpToken addr
    ...
    // TODO - when rewardsPerToken is called, revert if no pool is registered for lpToken addr
    ...
    // TODO - when getRewardForDuration is called, revert if no pool is registered for lpToken addr
     ...
     */
    //  * @param lpAddr Address of LP Token contract
    //function getRewardForDuration(address lpToken) external view returns (uint256);
