...
contract RoyaltyDistributor is iRoyaltyDistributor, Ownable, ReentrancyGuard {
    ...
    // TODO set content token as constant
    ContentToken private contentToken;

    constructor(
        address _contentToken, // TODO tmp as argument, can be assign as constant 
        ...
        contentToken = ContentToken(_contentToken);
    }
    ...
    function withdrawCommunityRoyalty(
        ...
        uint256 allocation = communityAllocation.mul(allowance).div(contentToken.totalSupply());
        communityAllocation = communityAllocation.sub(allocation);

        contentToken.burnFrom(msg.sender, allowance);
    ...
    function withdrawableCommunityRoyalty(
        ...
        uint256 balance = address(this).balance;
        uint256 balanceDiff = balance.sub(addressesAllocation.add(communityAllocation));
        uint256 _communityAllocation = communityAllocation.add((balanceDiff.div(10)));
        // * 10 / 100 

        uint256 totalSupply = contentToken.totalSupply();
        require(allowance <= totalSupply, "Exceed total supply");
        return _communityAllocation.mul(allowance).div(contentToken.totalSupply());
    }

    function rescueCommunityToken() external {
        require(!isAllContentTokenBurned, "Already rescued");
        require(contentToken.totalSupply() == 0, "All content token must be burned");
...
contract ContentToken is ERC20Burnable {
    // TODO TBD name and symbol
    constructor(uint256 recipients) ERC20("Content Token", "CT") {
        // 100 token per recipient, mint extra 1% for SNS rescue
        _mint(msg.sender, recipients * 101 / 100 * 100 * 1 ether);
    }
}
