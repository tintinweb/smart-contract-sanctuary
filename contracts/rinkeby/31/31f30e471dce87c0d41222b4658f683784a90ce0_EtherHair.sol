...
contract EtherHair is ERC721Enumerable, Ownable {
  using Strings for uint256;

  uint256 public constant HAIR_GIFT = 1;
  uint256 public constant HAIR_PUBLIC = 10;
  uint256 public constant HAIR_MAX = HAIR_GIFT + HAIR_PUBLIC;
  ...
  */
  ...
  function purchase(uint256 numberOfTokens) external payable {
    ...
    require(totalSupply() < HAIR_MAX, 'All tokens have been minted');
    ...
    */
    require(totalPublicSupply < HAIR_PUBLIC, 'Purchase would exceed HAIR_PUBLIC');
    require(PRICE * numberOfTokens <= msg.value, 'ETH amount is not sufficient');

    for (uint256 i = 0; i < numberOfTokens; i++) {
      /**
      * @dev Since they can get here while exceeding the HAIR_MAX,
      * we have to make sure to not mint any additional tokens.
      */
      if (totalPublicSupply < HAIR_PUBLIC) {
        /**
        * @dev Public token numbering starts after HAIR_GIFT.
        * And we don't want our tokens to start at 0 but at 1.
        */
        uint256 tokenId = HAIR_GIFT + totalPublicSupply + 1;
  ...
  function purchaseAllowList(uint256 numberOfTokens) external payable {
    ...
    require(totalPublicSupply + numberOfTokens <= HAIR_PUBLIC, 'Purchase would exceed HAIR_PUBLIC');
  ...
  function gift(address[] calldata to) external onlyOwner {
    require(totalSupply() < HAIR_MAX, 'All tokens have been minted');
    require(totalGiftSupply + to.length <= HAIR_GIFT, 'Not enough tokens left to gift');
