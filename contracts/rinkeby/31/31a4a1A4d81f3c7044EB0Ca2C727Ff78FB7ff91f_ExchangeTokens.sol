...
contract ExchangeTokens is Context, Ownable {
    ...
    function exchange() public {
        ...
        uint256 amount = qiair.balanceOf(msg.sender);

        require(amount > 20000000000, "exchange: amount must be greater than 20000000000");
        require(qiair.allowance(msg.sender, address(this)) >= amount, "exchange: contract not allowed to transfer enough QIAIR for this exchange");
        require(qi.balanceOf(address(this)) >= amount, "exchange: not enough QI token in the contract for this exchange");

        qiair.transferFrom(_msgSender(), address(this), amount);
        qi.transfer(_msgSender(), amount);
