...
contract NewManager is Ownable, IERC721Receiver, Pausable {
    ...
    function stakeTokens(address _account, uint16 _shipId, uint16[] calldata _tokenIds, uint16 _rumAmount) public {
        ...
        for (uint16 i = 0; i < _rumAmount; i++) {
            require(rum.balanceOf(_account, RUM_TICKET_ID) > 0, "Trying to use a booster without having it on the wallet");
            rum.burn(_account, 1);
        ...
        for (uint i = 0; i < _tokenIds.length; i++) {
            ...
            if (_shipId != 0) {
                speed += goldHunter.isPirate(_tokenIds[i]) ? 30 : 15;
            }
        }
        ...
        emit OceanTokenStaked(msg.sender, _shipId, _tokenIds, _rumAmount);
    }
