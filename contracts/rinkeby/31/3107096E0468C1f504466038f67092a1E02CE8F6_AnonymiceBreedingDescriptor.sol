...
*/
    ...
    {
        if (
            IAnonymiceBreeding(ANONYMICE_BREEDING_ADDRESS)._tokenToRevealed(_tokenId)
        ) {
            //This is a revealed baby

            if (
                IAnonymiceBreeding(ANONYMICE_BREEDING_ADDRESS)._tokenIdToLegendary(
                    _tokenId
                )
            ) {
                uint8 legendaryId = IAnonymiceBreeding(ANONYMICE_BREEDING_ADDRESS)
                    ._tokenIdToLegendaryNumber(_tokenId);
                return legendaries[legendaryId].metadata;
            }
            return
                hashToMetadata(
                    IAnonymiceBreeding(ANONYMICE_BREEDING_ADDRESS)._tokenIdToHash(
                        _tokenId
                    )
                );
        } else {
