...
contract MiddlePunks is Ownable, ERC721Enumerable, ReentrancyGuard {
  using Counters for Counters.Counter;
  using Strings for uint256;

  constructor() ERC721("MiddlePunks", "PHUNK") {}

  bool public isFreeSaleOn = false;

  uint256 public constant MAX_MINTABLE_AT_ONCE = 500;
  ...
  function maxNumFreePhunks() public view virtual returns (uint256) {
    ...
    // the number we give away will be (10000 - however many free dev phunks are minted)
    return 10000;
  }
  ...
  function mint(uint256 _numToMint) public payable nonReentrant() {
    ...
    if (isFreeSaleOn) {
      require(
        ...
        "No more free phunks to claim."
      );

      require(_numToMint <= 2, "Can only mint 2 free phunk.");
      require(
        balanceOf(msg.sender) <= 1,
        "Can only receive a free phunk if you don't have 2 phunk already."
      );
      require(
