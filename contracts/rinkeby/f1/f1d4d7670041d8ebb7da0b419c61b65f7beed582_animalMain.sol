...
contract animalMain is  ERC721,Ownable {
  ...
  struct animal {
    string dna; //DNA
    string star; //幾星級(抽卡的概念)
    string roletype; //(哪種動物 1老虎  2狗  3獅子  4鳥)
  }
  ...
  function seeAnimalDna(uint256 _tokenId) public view returns (string dna) {
    return animals[_tokenId].dna;
  }

  function seeAnimalStar(uint256 _tokenId) public view returns (string star) {
  ...
  string[] dnas = ["火", "水", "木", "光", "暗"];
  string[] stars = ["★★★", "★★★★", "★★★★★", "★★★★★★", "★★★★★★★"];
  string[] roletypes = ["Human", "Orcs", "Protoss", "goblin", "Machine", "Dragon"];
  
  function createAnimal() public payable {
       
      //TO DO 
      //動手玩創意，可以限制每次建立動物需要花費多少ETH
      require(msg.value == 1000 wei, "needs 1000 wei");
      //使用亂數來產生DNA, 星級, 種類
      string storage dna = dnas[uint(keccak256(abi.encodePacked(block.coinbase, blockhash(block.number-1)))) % 5];
      string storage star = stars[uint(keccak256(abi.encodePacked(block.coinbase, blockhash(block.number-1)))) % 5];
      string storage roletype = roletypes[uint(keccak256(abi.encodePacked(block.coinbase, blockhash(block.number-1)))) % 6];
      uint id = animals.push(animal(dna, star, roletype)) - 1;
