...
contract LoanMarket is LoanMarketBase {
    ...
    */
    ...
    function tokenLoanOfferAdjusted(
        ...
        uint256 ltv  = this._calculateLTV(loanOffersToken[_loanIdAdjusted].stakedCollateralAmounts, loanOffersToken[_loanIdAdjusted].stakedCollateralTokens, _newBorrowStableCoin, _newLoanAmountBorrowed, false);
        uint256 maxLtv = this.getMaxLtv(collatetralInBorrowed, msg.sender);
    ...
    function activateLoanFullToken(uint256 _loanId, uint256 _stableCoinAmount) public {
        ...
        require(loanOffersToken[_loanId].loanAmountInBorrowed == _stableCoinAmount, "GLM, stable coin amount must equal to borrower loan amount requested");

        IERC20(loanOffersToken[_loanId].borrowStableCoin).transferFrom(lenderAddress, loanOffersToken[_loanId].borrower, _stableCoinAmount); //TODO APY FEE CUT AFTER LOAN ACTIVATION
