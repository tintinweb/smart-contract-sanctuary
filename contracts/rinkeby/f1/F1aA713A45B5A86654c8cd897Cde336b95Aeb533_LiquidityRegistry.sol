...
contract LiquidityRegistry is ILiquidityRegistry, AbstractDependant {
    ...
    uint256 public override withdrawlListCounter;
    ...
    function findWithdrawlAfterTime(uint256 _time)
        external
        view
        override
        returns (uint256 _index)
    {
        for (uint256 i = 0; i < withdrawlListCoverage.length; i++) {
            address _coverage = withdrawlListCoverage[i];
            address _user = withdrawlListUser[i];
            (, uint256 _date, ) = IPolicyBook(_coverage).withdrawalsInfo(_user);
            if (_date > _time) {
                return i;
            }
            _index = i;
    ...
    function adjustWithdrawlListCounter(uint256 _index) external override onlyCapitalPool {
        withdrawlListCounter = _index;
    }
        ...
        onlyEligibleContracts
    {
        ...
        if (withdrawlListCoverage.length > 0) {
            ...
            for (uint256 i = withdrawlListCounter; i <= _lastIndex; i++) {
                if (withdrawlListCoverage[i] == _policyBook && withdrawlListUser[i] == _user) {
                    revert("LR: withdrawl already pending");
                }
            }
        }
        ...
        )
    {
        ...
        for (uint256 i = _start; i < withdrawlListCoverage.length; i++) {
            address _pbAddress = withdrawlListCoverage[i];
            address _pbUser = withdrawlListUser[i];
            // TODO: check allowed restrictions
            (_amount, _date, _allowed) = IPolicyBook(_pbAddress).withdrawalsInfo(_pbUser);

            // TODO debug would skip
            // if (_date < _startTime) {
            //     _start = i;
            //     continue;
            // }
            ...
            IPolicyBook.WithdrawalStatus _status =
                IPolicyBook(_pbAddress).getWithdrawalStatus(_pbUser);
