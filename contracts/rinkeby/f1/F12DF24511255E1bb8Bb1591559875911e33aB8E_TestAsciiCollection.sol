...
contract TestAsciiCollection is ERC721, Ownable {
    ...
    uint256 private constant totalSupply = 101;

    constructor(address asciiArtContractAddress)
    ERC721("101 ASCII People", "TACNFT")
    {
    ...
    function mintSingle() public {
        require(minters[msg.sender] == false, "Only single mint per wallet.");
        require(_tokenIdCounter.current().add(1) <= totalSupply, "All items minted.");
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        for (uint i = 0; i < 32; i++) {
            string memory idString = Strings.toString(i);
            string memory yString = Strings.toString(i * 11 + 6);
            string memory beginMsString = i == 0 ? "0" : Strings.toString(i * 50);
            finalSvg = string(abi.encodePacked(
                finalSvg,
                "<animate href='#row-visible-", idString ,"' attributeType='CSS' attributeName='opacity' values='1;0;0;1;1' keyTimes='0;0;0.2;0.2;1' dur='10000ms' begin='", beginMsString, "ms' fill='freeze' repeatCount='indefinite' />",
                "<animate href='#row-hidden-", idString,"' attributeType='CSS' attributeName='opacity' values='0;1;1;0;0' keyTimes='0;0;0.2;0.2;1' dur='10000ms' begin='", beginMsString, "ms' fill='freeze' repeatCount='indefinite' />",
                "<text id='row-hidden-", idString,"' x='0' y='", yString, "' width='100%' dominant-baseline='middle' textLength='319' font-size='8px' font-family='monospace' fill='#fff' style='opacity: 0'>8888888888888888888888888888888888888888888888888888888888888888</text>"
            ));
            finalSvg = string(abi.encodePacked(finalSvg, "<text id='row-visible-", idString,"' x='0' y='", yString, "' width='100%' dominant-baseline='middle' textLength='319' font-size='8px' font-family='monospace' fill='#fff'>"));
        ...
        return formatTokenURI(svgToImageURI(finalSvg), tokenId);
    }
    ...
    function formatTokenURI(string memory imageURI, uint256 tokenId) private pure returns (string memory) {
        return string(
            abi.encodePacked(
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked(
                            '{"name":"',
                            string(abi.encodePacked("ASCII Person #", Strings.toString(tokenId))),
                            '", "description":"First ASCII collection of 101 persons that does not exist in this world.", "attributes":"", "image":"',
                            imageURI,
                            '"}'
                        )
                    )
