...
enum KittyPartyStages {
    ...
    modifier timedTransitions() {
        ...
        if (stage == KittyPartyStages.Collection && block.timestamp >= creationTime + 10 minutes) {
            nextStage();
        }
        if (stage == KittyPartyStages.Staking && block.timestamp >= creationTime + durationInDays * 10 minutes) {
            nextStage();
        }
        _;
    }
