...
contract BasicStakePolicy is IStakePolicy, ERC20, Ownable {
  ...
  /// @notice the address for kibble factory
  address public kibbleFactory;
  ...
  /// @param _kibbleFactory the address for kibble factory
  constructor(
    address _lpToken,
    address _feeAddress,
    address _kibbleFactory
  ) ERC20('DogPark V1', 'DP-V1') {
    lpToken = IERC20(_lpToken);
    feeAddress = _feeAddress;
    kibbleFactory = _kibbleFactory;
  }

  modifier onlyKibbleFactory {
    require(
      msg.sender == kibbleFactory,
      'BasicStakePolicy: only kibble factory'
    );
    _;
  }
  ...
  function deposit(
    ...
    address _owner,
    uint16 _depositFee
  ) external override onlyKibbleFactory returns (uint256 amount_) {
  ...
  /// @notice set a new fee address only fee holder can do this
  function setKibbleFactory(address _kibbleFactory)
    external
    override
    onlyOwner
  {
...
interface IStakePolicy {
  ...
  function setKibbleFactory(address _kibbleFactory) external;
}
