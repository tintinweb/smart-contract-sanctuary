...
contract Polkadog is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    function getTaxTValues(uint256 tAmount, address recipient) internal returns(uint256, uint256, uint256, uint256, uint256) {
        address receiver = recipient;
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tMarketingFee) = _getTValues(tAmount, receiver);
        return(tTransferAmount, tFee, tLiquidity, tBurn, tMarketingFee);
    }

    function getTaxRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tMarketingFee, address recipient) internal returns(uint256, uint256, uint256) {
        address receiver = recipient;
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tBurn, tMarketingFee, receiver);
        return(rAmount, rTransferAmount, rFee);
    }

    function _transferStandard(address sender, address recipient, uint256 tAmount) private {

        Fee memory taxAmount;

        if(_takeBothTax) {
            address receiver = recipient;
            uint256 pastAmount = _transactionAmount[sender];
            uint256 remainingTxLimit = transactionLimit - pastAmount;
            uint256 remainingtAmount = tAmount - remainingTxLimit;

            (taxAmount.tTransferAmount, taxAmount.tFee, taxAmount.tLiquidity, taxAmount.tBurn, taxAmount.tMarketingFee) = getTaxTValues(remainingtAmount, receiver);
            (taxAmount.rAmount, taxAmount.rTransferAmount, taxAmount.rFee) = getTaxRValues(remainingtAmount, taxAmount.tFee, taxAmount.tLiquidity, taxAmount.tBurn, taxAmount.tMarketingFee, receiver);
        } else {
            address receiver = recipient;
            (taxAmount.tTransferAmount, taxAmount.tFee, taxAmount.tLiquidity, taxAmount.tBurn, taxAmount.tMarketingFee) = _getTValues(tAmount, receiver);
            (taxAmount.rAmount, taxAmount.rTransferAmount, taxAmount.rFee) = _getRValues(tAmount, taxAmount.tFee, taxAmount.tLiquidity, taxAmount.tBurn, taxAmount.tMarketingFee, receiver);
        }

        _rOwned[sender] = _rOwned[sender].sub(taxAmount.rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(taxAmount.rTransferAmount);
        _takeLiquidity(taxAmount.tLiquidity);
        _reflectFee(taxAmount.rFee, taxAmount.tFee);
        _takeMarketingFee(taxAmount.tMarketingFee);
        if(taxAmount.tBurn > 0) {
            _amount_burnt += taxAmount.tBurn;
            emit Transfer(sender, address(0), taxAmount.tBurn);
        }
        emit Transfer(sender, recipient, taxAmount.tTransferAmount);
    }
