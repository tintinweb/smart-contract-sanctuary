...
contract SemiFungible is ERC1155, AccessControl {
    event Sold(
    ...
    event PaymentShared(address account, uint256 amount, uint256 id);
    event PaymentTreasure(address account, uint256 amount, uint256 id);
    event SoldWithStones(address buyer, uint256 amount, uint256 id);
    event NewStartTime(uint256 startTime, uint256 id);
    event NewEndTime(uint256 endTime, uint256 id);
    event NewUsdAmount(uint256 amount, uint256 id);
    event NewPercent(uint256 amount, uint256 id);
    event SetStaticAddresses(address treasury, address moneyHandler);
    event SetExternalAddresses(uint256 id, address token, address stone);
    event CollectionCreated(uint256 tokenId);
    ...
    constructor(
        ...
        setStaticAddresses(treasury, moneyHandler);
    }

    modifier exist(uint256 _id) {
        require(collections[_id].total != 0, "This Collection not exist.");
        _;
    }

    function setStaticAddresses(address _treasury, address _moneyHandler)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        emit SetStaticAddresses(_treasury, _moneyHandler);
    }
    ...
    function setExternalAddresses(
        ...
        address _token,
        address _stone
    ) public exist(_id) onlyRole(DEFAULT_ADMIN_ROLE) {
        collections[_id].token = _token;
        collections[_id].farm = _stone;

        emit SetExternalAddresses(_id, _token, _stone);
    }
    ...
    function createCollection(
        uint256 _id,
        CollectionData memory _collectionData
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(collections[_id].total == 0, "This collection already exist");

        collections[_id] = _collectionData;
        available[_id] += _collectionData.total;

        emit CollectionCreated(_id);
    }

    function buy(uint256 _id, uint256 _quantity) external exist(_id) {
        require(available[_id] > 0, "Sold Out.");
        require(
            available[_id] >= _quantity,
            "There are not so many cards available."
        );
        require(
            collections[_id].startTime <= block.timestamp &&
                collections[_id].endTime > block.timestamp,
            "Sale did not start yet."
        );

        collections[_id].farm == address(0)
            ? _withToken(msg.sender, _id, _quantity)
            : _withStones(msg.sender, _id, _quantity);
        ...
        emit Sold(address(this), msg.sender, _id, collections[_id].amount);
    }
    ...
    function mint(
        ...
        uint256 _id,
        uint256 _quantity
    ) external exist(_id) onlyRole(DEFAULT_ADMIN_ROLE) {
        require(available[_id] > 0, "Sold Out");
        require(
            available[_id] >= _quantity,
            "There are not so many cards available."
    ...
    function setStarTime(uint256 _id, uint256 _starTime)
        external
        exist(_id)
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        collections[_id].startTime = _starTime;

        emit NewStartTime(_starTime, _id);
    }

    function setEndTime(uint256 _id, uint256 _endTime)
        external
        exist(_id)
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        collections[_id].endTime = _endTime;

        emit NewEndTime(_endTime, _id);
    }

    function setAmount(uint256 _id, uint256 _newAmount)
        external
        exist(_id)
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        collections[_id].amount = _newAmount;

        emit NewUsdAmount(_newAmount, _id);
    }

    function setPercent(uint256 _id, uint256 _newPercent)
        external
        exist(_id)
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        collections[_id].percent = _newPercent;

        emit NewPercent(_newPercent, _id);
    }
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        emit SetPfeedAddress(_priceFeed);
    }
    ...
    function getTokenPrice(uint256 _id) public view returns (uint256) {
        address priceOracle = getPriceOracle();
        address tokenFeed = IPriceFeed(priceOracle).getFeed(
            collections[_id].token
        );
    ...
    function getCardPrice(uint256 _id, uint256 _quantity)
        public
        view
        returns (uint256)
    {
        uint256 tokenPrice = getTokenPrice(_id);
        uint256 amount = collections[_id].amount * _quantity;
    ...
    function _withStones(
        ...
        address farmAddress = collections[_id].farm;
        uint256 stones = FarmV2(farmAddress).rewardedStones(_buyer);
        uint256 amount = collections[_id].amount * _quantity;
        require(stones >= amount, "You do not have enough points !");
        require(
            FarmV2(farmAddress).payment(_buyer, amount),
            "Payment was unsuccessful"
        );

        emit SoldWithStones(_buyer, amount, _id);
    }
    ...
    function _withToken(
        ...
        uint256 price = getCardPrice(_id, _quantity);
        address token = collections[_id].token;

        require(
            IERC20(token).balanceOf(_buyer) >= price,
            "Insufficient funds: Cannot buy this NFT"
        );

        uint256 treasAmount = calcPerc(price, collections[_id].percent);
        uint256 shareAmount = SafeMath.sub(price, treasAmount);

        IERC20(token).transferFrom(_buyer, treasury, treasAmount);
        IERC20(token).transferFrom(_buyer, address(moneyHand), shareAmount);

        moneyHand.updateCollecMny(address(this), shareAmount);

        emit PaymentTreasure(address(this), treasAmount, _id);
        emit PaymentShared(address(this), shareAmount, _id);
    }
}
