...
// NOTE: Compilation is only successful with optimization turned off.
// The error with optimization mentions "variable length_1 is 1 slot(s) too deep inside the stack"
// which usually is too many local variables, but there doesn't appear to be anywhere
// where we have too many local vars. For now, just compile without optimization.
