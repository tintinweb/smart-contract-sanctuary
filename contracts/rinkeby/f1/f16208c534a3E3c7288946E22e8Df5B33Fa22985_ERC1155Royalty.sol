    ...
    AbstractRoyalties
{
     ...
     * @param _royalties array of payable account for royalty value
    ...
    function create1155(
        ...
        mint(_initialOwner, _id, _initialSupply, _royalties, _data);
     ...
     * @param _royalties   array of struct part include payable royalty owner account and royalty value 
     * @param _data        Data to pass if receiver is contract
     */
    function mint(
        address _to,
     ...
     * @param _royalties   Array of royalty addresses and royalty values
     * @param _data        Data to pass if receiver is contract
     */
    function batchMint(
        address _to,
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            ...
            _saveRoyalties(_ids[i], _royalties);
    ...
    // set address that royalties are sent to
    function setRoyaltyCollectionNetadataCID(string memory cid)
        public
        onlyOwner
    {
    ...
    // rarible royalties
    function getFeeBps1155(uint256 tokenId, uint256 quantity)
        public
        view
        override
        returns (uint256[] memory)
    {
        ...
        ret[0] = uint256(_feeBps * quantity);
        return ret;
    }
    ...
    {
        require(exists(tokenId), "invalid token");
        return (_royaltyRecipient, _feeBps);
    }
...
interface RoyaltiesV1 {
    ...
    //fee basis points for ERC1155 Token
    function getFeeBps1155(uint256 tokenId, uint256 quantity) external view returns (uint[] memory);
}
