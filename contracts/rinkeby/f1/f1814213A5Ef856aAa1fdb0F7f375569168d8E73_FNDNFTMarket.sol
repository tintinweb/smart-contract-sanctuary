 ...
 * @dev This also leaves a gap which can be used to add a new mixin to the top of the inheritence tree.
 */
abstract contract NFTMarketCore {
  // 50 gaps were consumed by adding ReentrancyGuardUpgradeable
...
abstract contract SendValueWithFallbackWithdraw is ReentrancyGuardUpgradeable {
  ...
  function _sendValueWithFallbackWithdraw(address payable user, uint256 amount) internal {
    // Cap the gas so that reentrancy will not occur
    // This also prevents reverts or consuming all available gas to block the tx
  ...
  SendValueWithFallbackWithdraw
{
  ...
  /**
   * @notice Returns true if the given NFT has not been sold in this market previously.
   */
    ...
    )
  {
    ...
    _sendFunds(getFoundationTreasury(), foundationFee);
    if (creatorFee > 0) {
      ...
      if (creator == address(0)) {
        ...
        _sendFunds(creator, creatorFee);
      }
    }
    _sendFunds(seller, ownerRev);
  }

  function _sendFunds(address payable account, uint256 amount) private {
    if (amount > 0) {
      _sendValueWithFallbackWithdraw(account, amount);
    }
  }
  ...
  NFTMarketAuction
{
  ...
  uint256 private _maxBidIncrementRequirement;
  uint256 private _duration;
  uint256 private _extensionDuration;
  uint256 private _goLiveDate;
    ...
    )
  {
    ...
    maxBidIncrementRequirement = _maxBidIncrementRequirement;
    duration = _duration;
    extensionDuration = _extensionDuration;
  }

  function _initializeNFTMarketReserveAuction() internal {
    // Setting the max to MAX_UINT is effectively ignoring this requirement
    _maxBidIncrementRequirement = 2**256 - 1;
  ...
  function _updateReserveAuctionConfig(
    ...
    require(maxBidIncrementRequirement > 0, "NFTMarketReserveAuction: Max increment must be greater than 0");
    ...
    require(extensionDuration <= duration, "NFTMarketReserveAuction: Extension must be <= duration");
    _minBidIncrementInBasisPoints = minBidIncrementInBasisPoints;
    _maxBidIncrementRequirement = maxBidIncrementRequirement;
    _duration = duration;
    _extensionDuration = extensionDuration;
    _goLiveDate = goLiveDate;
  ...
  function placeBid(uint256 auctionId) public payable nonReentrant {
    require(block.timestamp >= _goLiveDate, "NFTMarketReserveAuction: Market is not yet live");
    ReserveAuction storage auction = auctionIdToAuction[auctionId];
    require(auction.duration != 0, "NFTMarketReserveAuction: Auction not found");
  ...
  function _getMinBidAmountForReserveAuction(uint256 currentBidAmount) private view returns (uint256) {
    ...
    if (minIncrement > _maxBidIncrementRequirement) {
      return _maxBidIncrementRequirement.add(currentBidAmount);
    }
