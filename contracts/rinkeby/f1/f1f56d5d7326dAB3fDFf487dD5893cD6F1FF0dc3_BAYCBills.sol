...
contract BAYCBills is ERC721URIStorage {
  ...
  function genBillItem(uint256 _tokenId) public view returns (string memory) {
    string memory draftLagersRow = "";
    uint totalDraftBeers = tokenIdToNumberOfLagers[_tokenId];
    if (totalDraftBeers > 0) {
      draftLagersRow = string(
        abi.encodePacked(
          '<text x="320" y="375" class="t">',
          uint2str(totalDraftBeers),
          '</text>',
          '<text x="425" y="375" class="t">Draft Lager</text>',
          '<text x="600" y="375" class="t">',
          uint2str(totalDraftBeers * 1 / 10),
          'ETH</text>'
        )
      );
    }
    return draftLagersRow;
