...
contract ExercisePOLYForOHM {
    ...
    mapping( address => uint ) public percentOfOHMSupplyVested;
    ...
    function setMaxAllowedToClaim(address _vester, uint _amountCanClaim ) external returns ( bool ) {
        ...
        require( _amountCanClaim > maxAllowedToClaim[ _vester ] );
    ...
    function setVestingRate( address _vester, uint _rate) external returns ( bool ) {
        require( msg.sender == owner );
        require( _rate > percentOfOHMSupplyVested[ _vester ] );

        percentOfOHMSupplyVested[ _vester ] = _rate;
    ...
    function getPOLYAbleToClaim( address _vester ) public view returns (uint) {
        return ( IERC20( OHM ).totalSupply().mul( percentOfOHMSupplyVested[ _vester ] ).div( 10000 ).mul( 1e9 ) ).sub( amountClaimed[ _vester ] );
    }
