...
contract CarbonCoin is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    address private _btcb = 0x577D296678535e4903D59A4C929B718e1D575e0A;   //testnet ether
    address private _usdt = 0x2fB298BDbeF468638AD6653FF8376575ea41e768;   //testnet ether

    IERC20 btcb = IERC20(_btcb);
    ...
    function swapEthForRewardTokens(uint256 ethAmount) private {
        ...
        uint256 ethAmountForBtcb = ethAmount.div(2);
        uint256 ethAmountForUsdt = ethAmount.sub(ethAmountForBtcb);
        ...
        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: ethAmountForBtcb}(
            0, // accept any amount of btcb
            path1,
            address(this),
            block.timestamp
        );
        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: ethAmountForUsdt}(
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        if (uniswapV2Pair == sender && !_isExcludedFromFee[recipient]){  
            deductFee();
            uint256 currentRate =  _getRate();
    ...
    function checkReward() public view returns (uint256,uint256) {
    ...
    function _calculateReward(
        ...
        uint256 currentBtcbPool,
        uint256 currentUsdtPool,
        uint256 cTotalSupply
    ) private pure returns (uint256,uint256) {
        uint256 multiplier = 100;
        uint256 btcbReward = currentBtcbPool.mul(multiplier).mul(currentBalance).div(100).div(cTotalSupply);
        uint256 usdtReward = currentUsdtPool.mul(multiplier).mul(currentBalance).div(100).div(cTotalSupply);
        return (btcbReward,usdtReward);
    }
    ...
    function claimReward() isHuman nonReentrant public {
        ...
        (uint256 btcbR, uint256 usdtR) = checkReward();

        _approve(address(this), _btcb, btcbR);
        _approve(address(this), _usdt, btcbR);
        btcb.transfer(msg.sender, btcbR);
        usdt.transfer(msg.sender, usdtR);
        // update rewardCycleBlock
        nextAvailableClaimDate[msg.sender] = block.timestamp + rewardCycleBlock;
        emit ClaimBTCBSuccessfully(msg.sender, btcbR, nextAvailableClaimDate[msg.sender]);
