...
contract MoreOrLessVote is ERC721Enumerable, Ownable {
    ...
    uint16 public highVotes;
    uint16 public lowVotes;
    ...
    function _generateImage(uint256 mintNum) private view returns (string memory) {
        string memory triangles = MoreOrLessArt._generateTriangles(seeds[msg.sender], artInfos[mintNum].numTriangles);
        string memory circles = MoreOrLessArt._generateCircles(seeds[msg.sender], artInfos[mintNum].numCircles);
        string memory rectangles = MoreOrLessArt._generateRectangles(seeds[msg.sender], artInfos[mintNum].numRects);
        string memory lines = MoreOrLessArt._generateLines(seeds[msg.sender], artInfos[mintNum].numLines);
    ...
    function _saveImageInfo(uint256 mintNum) private {
        artInfos[mintNum].whichShape = MoreOrLessArt.seededRandom(0, 4, seeds[msg.sender], voteInfos[mintNum].nonce);
        voteInfos[mintNum].nonce++;
        artInfos[mintNum].numRects = MoreOrLessArt.seededRandom(1,10, seeds[msg.sender], voteInfos[mintNum].nonce);
        voteInfos[mintNum].nonce++;
        artInfos[mintNum].numCircles = MoreOrLessArt.seededRandom(1,10, seeds[msg.sender], voteInfos[mintNum].nonce);
        voteInfos[mintNum].nonce++;
        artInfos[mintNum].numTriangles = MoreOrLessArt.seededRandom(1,10, seeds[msg.sender], voteInfos[mintNum].nonce);
        voteInfos[mintNum].nonce++;
        artInfos[mintNum].numLines = MoreOrLessArt.seededRandom(1,10,seeds[msg.sender], voteInfos[mintNum].nonce);
        voteInfos[mintNum].nonce++;
    }
...

{
  ...
  "libraries": {
    "contracts/MoreOrLessArt.sol": {
      "MoreOrLessArt": "0x617d99feb8c43511510fd08b2d00390aa26d30e3"
    }
  }
}