...
contract IslandBoys is ERC721Tradable{
  ...
  function baseTokenURI() override public pure returns (string memory) {
        return "https://storage.googleapis.com/islandboys/JSON/";
  }
...
contract IslandBoysMinter is Ownable {
    using Strings for string;
    using SafeMath for uint256;
    ...
    function publicMint(address _toAddress, uint256 amount) external payable {
        ...
        require(currentMints + amount <= 10);

        IslandBoys islandBoys = IslandBoys(nftAddress);
    ...
    function ownerMint(address _toAddress, uint256 amount) external onlyOwner {
        require(counter + amount <= 250, "Purchase would exceed max supply for owner");

        IslandBoys playersOnlyNFT = IslandBoys(nftAddress);
        for (uint256 i = 0; i < amount; i++) {
