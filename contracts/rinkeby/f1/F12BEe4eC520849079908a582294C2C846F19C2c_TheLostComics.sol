...
// Latest deployment to rinkeby: 0xDB23D0112c440829bA9d191C8169F01de5d58C22
contract TheLostComics is ERC721, ERC721Enumerable, Ownable {
    ...
    modifier whenSaleActive {
      require(saleIsActive, "TheLostComics: Sale is not active");
      _;
    }

    modifier whenPresaleActive {
      require(presaleIsActive, "TheLostComics: Presale for GlitchHolders is not yet active");
      _;
    }

    modifier whenMetadataNotFrozen {
      require(!METADATA_FROZEN, "TheLostComics: Metadata already frozen.");
      _;
    }

    modifier isGlitchOwner {
      require(tlg.tokensOfOwner(msg.sender).length > 0, "TheLostComics: Only for Glitch Owners.");
      _;
    }

    function glitchOwnerTestView(address wallet) public view returns (uint amount) {
      return tlg.tokensOfOwner(wallet).length;
    }

    function glitchOwnerTestWrite(address wallet) external view returns (uint amount) {
      return tlg.tokensOfOwner(wallet).length;
    }

    Counters.Counter private _tokenIdCounter;

    constructor(address _tlg) ERC721("TheLostComics", "LSTCMC") {
    ...
    function mintComicsPresale(uint256 amount) external payable whenPresaleActive isGlitchOwner {
      require(amount <= maxPerMint, "TheLostComics: Amount exceeds max per mint");
      require(balanceOf(msg.sender) + amount <= maxPerWallet, "TheLostComics: Amount exceeds max per wallet");
      require(_tokenIdCounter.current() + amount <= MAX_COMICS, "TheLostComics: Purchase would exceed cap");
      mintWithDiscount(amount);
    }

    function mintWithDiscount(uint256 amount) internal {
      require((mintPrice * (100 - discountPercentage) / 10000) * amount <= msg.value, "TheLostComics: Ether value sent is not correct");
    ...
    function mintComics(uint256 amount) external payable whenSaleActive {
      ...
      require(balanceOf(msg.sender) + amount <= maxPerWallet, "TheLostComics: Amount exceeds max per wallet");
      require(_tokenIdCounter.current() + amount <= MAX_COMICS, "TheLostComics: Purchase would exceed cap");
      if (tlg.tokensOfOwner(msg.sender).length > 0) {
        mintWithDiscount(amount);
        return;
      }
      require(mintPrice * amount <= msg.value, "TheLostComics: Ether value sent is not correct");
      _mintMultiple(msg.sender, amount);
    }

    function mintForCommunity(address _to, uint256 _numberOfTokens) external onlyOwner {
      require(_to != address(0), "TheLostComics: Cannot mint to zero address.");
      require(totalSupply() + _numberOfTokens <= MAX_COMICS, "TheLostComics: Minting would exceed cap");
