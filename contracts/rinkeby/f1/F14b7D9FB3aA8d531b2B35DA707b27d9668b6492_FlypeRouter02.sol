...
contract FlypeLibrary is Ownable {
    ...
    function getAmountIn(uint amountOut, uint priceIn, uint priceOut, address user) internal view returns (uint amountIn, uint256 fee) {
        ...
        uint numerator = priceOut * amountOut * 100000;
        uint denominator = priceIn * (99800 + 2 * discountCalculator(user));
        amountIn = numerator / denominator + 1;
        fee = (amountIn - priceOut * amountOut / priceIn) * priceIn / 10**18;
    }
    ...
    function getAmountsOut(uint amountIn, address[] memory path, address user) public view returns (uint[] memory amounts, uint256[] memory fees) {
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            (path[i] == _WETH)?
             (amounts[i + 1], fees[i]) = getAmountOut(amounts[i], priceETH, priceLp, user)
             :
             (amounts[i + 1], fees[i]) = getAmountOut(amounts[i], priceLp, priceETH, user);
        }
    }
    ...
    function getAmountsIn(uint amountOut, address[] memory path, address user) public view returns (uint[] memory amounts, uint256[] memory fees) {
        ...
        for (uint i = path.length - 1; i > 0; i--) {
            ...
            (path[i] == _WETH)?
             (amounts[i - 1], fees[i - 1]) = getAmountIn(amounts[i], priceLp, priceETH, user)
             :
             (amounts[i - 1], fees[i - 1]) = getAmountIn(amounts[i], priceETH, priceLp, user);
        }
    }
}
