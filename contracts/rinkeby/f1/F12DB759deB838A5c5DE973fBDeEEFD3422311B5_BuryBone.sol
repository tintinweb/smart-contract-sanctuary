...
contract BuryBone is Ownable, ERC20("BuryBone", "yBONE"){
    using SafeMath for uint256;
    IERC20 public bone;
    ...
    uint releaseDuration= 120 * 1 seconds;   // can be changed later, 86400*30
    uint resetDuration= 60 * 1 seconds ; // 86400
    uint timeStone; // current Epoch
    uint penaltyGraceDuration= 120 seconds; // 7 days
    ...
    function checkEpoch() internal{
        uint epochsOver= (block.timestamp- timeStone)/ resetDuration;
    ...
    function lock(uint256 _amount) public {
        ...
        checkEpoch();  // returns the current timeStone  
        uint penalty; 
        uint totalClaimable;
        uint lockStartIndex;
    ...
    function userBalanceData(address _account) public view returns(uint, uint, uint, uint){
        ...
        uint i;
        uint unlockStartIndex;   // index where released funds start
        uint lockStartIndex; // index where lockedfunds start
        uint penaltyAmount;
        for(i= length; i>0 ; i--){
