...
contract VIDTC is Pausable, IERC165, IERC721, IERC721Metadata, IERC721Enumerable {
	...
	mapping (string => uint256) internal internalHashes;
	...
	function verifyHash(string calldata hash) external view returns (address, uint256) {
		if (internalHashes[hash] > 0) {
			return (address(this),internalHashes[hash]);
		} else if (externalHashes[hash].tokenID > 0) {
			return(externalHashes[hash].nftContract,externalHashes[hash].tokenID);
		} else {
			return (address(0),0);
		}
	}
	...
	function createMyNFT(bytes calldata data) public whenNotPaused returns (uint256) { 
		...
		require(internalHashes[hash] == 0,"C3i - NFT exists already internally");
		require(externalHashes[hash].tokenID == 0,"C3e - NFT exists already externally");
	...
	function createNFT(bytes calldata data) public whenNotPaused onlyTrusted returns (uint256) { 
		...
		string memory hash = string(data);
		require(internalHashes[hash] == 0,"CP2i - NFT exists already internally");
		require(externalHashes[hash].tokenID == 0,"CP2e - NFT exists already externally");
	...
	mapping (string => externalHash) private externalHashes;
	...
	function approveVerification(address nftContract, uint256 tokenID, address originalCreator, string memory hash) public onlyController returns (bool) {
		...
		externalHashes[hash] = externalHash({
	...
	function denyVerification(address nftContract, uint256 tokenID) public onlyController returns (bool) {
		if (externalHashes[Verifications[nftContract][tokenID].hash].tokenID > 0) {
			delete externalHashes[Verifications[nftContract][tokenID].hash];
		}
