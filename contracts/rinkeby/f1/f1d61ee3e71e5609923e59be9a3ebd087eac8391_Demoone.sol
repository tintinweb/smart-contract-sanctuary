...
contract Demoone is Context, Ownable, IERC20 {

	using SafeMath for uint256;
    using Address for address;
    string private _name = "Demoone";
    string private _symbol = "DEMOONE";
    uint8 private _decimals = 9;
    mapping (address => uint256) private _balance;
    mapping (address => mapping (address => uint256)) private _allowances;
    uint256 private _total = 481516000 * 10**6 * 10**9;
    uint256 private maxTxAmount = 2342000 * 10**6 * 10**9;
    uint256 private numTokensSellToAddToLiquidity = 234200 * 10**6 * 10**9;
    uint256 private minHoldingThreshold = 2842000 * 10**6 * 10**9;
    uint8 public ripeBananasTax = 5;
    uint8 public burnableFundRate = 5;
    uint8 private operationalFundRate = 1;
    uint8 private teamFundRate = 1;
    ...
    function _getValues(uint256 amount) private returns (uint256) {
    	...
    	uint256 teamFundTax = _getExtractableFund(amount, teamFundRate);
    	uint256 businessTax = operationalTax.add(burnableFundTax).add(ripeBananasTaxFee).add(charityTax).add(teamFundTax);
    	uint256 transactionAmount = amount.sub(businessTax);
    	
    	uint256 initialBalance = address(this).balance;
        swapTokensForEth(teamFundTax);
        uint256 newBalance = address(this).balance.sub(initialBalance);
        sendBananas(newBalance);
