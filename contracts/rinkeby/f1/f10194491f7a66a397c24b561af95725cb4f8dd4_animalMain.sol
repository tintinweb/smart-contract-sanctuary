...
contract animalMain is  ERC721,Ownable {
  ...
  function seeAnimalHealth(uint256 _tokenId) public view returns (uint8 health) {
    return animals[_tokenId].health;
  }
  ...
  function createAnimal() public payable {
      ...
      roletype = uint(random()) % 4 + 1; // 1:橘貓 2:虎斑貓 3:白貓 4:黑貓
      
      health = uint(random()) % 2 + 1; // 1:健康 2:不健康
      
      //動手玩創意，可以限制每次建立動物需要花費多少ETH
       
      require(msg.value == 100 wei);
       
      uint id = animals.push(animal(dna, uint8(star), uint8(roletype), uint8(health))) - 1;
