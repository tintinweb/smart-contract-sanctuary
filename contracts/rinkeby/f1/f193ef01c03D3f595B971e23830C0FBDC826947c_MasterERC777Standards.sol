...
import "../token/ERC777/ERC777Standards.sol";
import "../libraries/Dictionary.sol";

contract MasterERC777Standards is Ownable {
  ERC777Standards[] private childrenERC777Standards;
  ...
  function createTokenERC777(
    ...
    require(
      ...
      "ERC777:feeContract must be compare payableAmount"
    );

    ERC777Standards child = new ERC777Standards(
      name,
      symbol,
      decimal,
    ...
    childrenERC777Standards.push(child);
  }
...
contract ERC777Standards is ERC777 {
  // @dev Governance
  ...
  modifier checkTypes(string memory _type) {
    require(
      types[keccak256(abi.encodePacked(_type))] == 1,
      "_keyTypes is required"
    );
    _;
  }
  ...
  ) public payable virtual checkTypes("777mint") {
    _mint(account, amount, userData, operatorData);
    if (types[keccak256(abi.encodePacked("777governance"))] == 1) {
  ...
  function delegate(address delegatee) external checkTypes("777governance") {
  ...
  function delegateBySig(
    ...
    bytes32 r,
    bytes32 s
  ) external checkTypes("777governance") {
