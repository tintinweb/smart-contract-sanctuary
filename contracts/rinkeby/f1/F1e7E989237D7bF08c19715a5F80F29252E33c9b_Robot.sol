...
contract Robot is ERC721Enumerable, Ownable, IRobot, ReentrancyGuard {
    /// @dev Price for one Robot
    uint256 public unitPrice = 0.001 ether;

    /// @dev if the mint is enabled
    bool public mintEnabled = true;

    /// @dev The token ID Robot detail
    ...
    constructor(address _tokenDescriptor_) ERC721("Robot", "ROBOROBO") {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return IRobotDescriptor(_tokenDescriptor).tokenURI(this, tokenId);
    }

    /// @notice Create randomly an Robot
    ...
    function create(uint256 qty) public payable nonReentrant {
        ...
        require(totalSupply() + qty <= 10, "Cant mint more than 10 Robot");
        require(qty <= 3, "Robot max mintable quantity is 3");

        for (uint256 i; i < qty; i++) {
            uint256 seed = block.timestamp << (i + 1);
            uint256 nextTokenId = totalSupply() + 1;
            Detail memory newDetail = Detail({
                hair: IRobotDescriptor(_tokenDescriptor).generateHairId(nextTokenId, seed),
                eye: IRobotDescriptor(_tokenDescriptor).generateEyeId(nextTokenId, seed),
                eyebrow: IRobotDescriptor(_tokenDescriptor).generateEyebrowId(nextTokenId, seed),
                nose: IRobotDescriptor(_tokenDescriptor).generateNoseId(nextTokenId, seed),
                mouth: IRobotDescriptor(_tokenDescriptor).generateMouthId(nextTokenId, seed),
                mark: IRobotDescriptor(_tokenDescriptor).generateMarkId(nextTokenId, seed),
                earrings: IRobotDescriptor(_tokenDescriptor).generateEarringsId(nextTokenId, seed),
                accessory: IRobotDescriptor(_tokenDescriptor).generateAccessoryId(nextTokenId, seed),
                mask: IRobotDescriptor(_tokenDescriptor).generateMaskId(nextTokenId, seed),
                skin: IRobotDescriptor(_tokenDescriptor).generateSkinId(nextTokenId, seed),
    ...
    /// @notice Update the price of one Robot
    function updatePrice(uint256 _unitPrice) external onlyOwner {
        require(_unitPrice <= 0.001 ether, "Cant be more than base price of 0.001 FTM");
