...
contract ERC1155 is IERC165 {
  ...
  |__________________________________*/
	...
	function _callonERC1155Received(
		...
		if (_to.isContract()) {
			bytes4 retval =
				IERC1155TokenReceiver(_to).onERC1155Received(msg.sender, _from, _id, _amount, _data);
			require(
	...
	function _callonERC1155BatchReceived(
		...
		if (_to.isContract()) {
			bytes4 retval =
				IERC1155TokenReceiver(_to).onERC1155BatchReceived(msg.sender, _from, _ids, _amounts, _data);
			require(
...
library Strings {
	...
	function strConcat(
		...
		string memory abcde =
			new string(_ba.length + _bb.length + _bc.length + _bd.length + _be.length);
 ...
 * AIPod - AIPods
 */
contract AIPod is ERC1155Tradable {
	uint256 public price1 = 1000000000000000000; //  1 ETH
	uint256 public price2 = 500000000000000000; //   0.5 ETH
	uint256 public price3 = 250000000000000000; //   0.25 ETH
	uint256 public price4 = 125000000000000000; //   0.125 ETH
	uint256 public itemPrice;

	constructor(address _proxyRegistryAddress)
		public
		ERC1155Tradable("AIPOD", "AIPOD", _proxyRegistryAddress)
	{
		_setBaseMetadataURI(
	...
	// sets the price for a range
	function setPrice3(uint256 _price) public onlyOwner {
		price3 = _price;
	}

	// sets the price for a range
	function setPrice4(uint256 _price) public onlyOwner {
		price4 = _price;
	}
	...
	function buyOne(uint256 _id) public payable {
		...
		require(msg.value >= expectedPrice, "you should send the exact amount of ETH to buy this");
