...
contract BorgFactory is CommonObjects, Editable{
    ...
    function getBorgAttribute(string memory borgAttributeName) public view onlyOwner returns (string[] memory imagePixals){
        BorgAttribute storage borgAttribute = _borgAttributes[borgAttributeName];
        require(borgAttribute.exists, "Borg attribute doesn't exist");
...
contract Borgs is ERC721Enumerable, BorgFactory, NumberUtils{
    ...
    mapping (string => uint256) public _borgAttributesUsed;
    
    // Borgs start from id1
    uint256 _currentBorgId = 1;
    ...
    function getBorgImage(uint256 borgId) public view returns(string[] memory){
        ...
        return _combineBorgAttributes(borg.borgAttributeNames);
    }
    
    function getBorgsParents(uint256 borgId) public view returns(uint256 parentAId, uint256 parentBId){
        // Find the borg in question
        Borg memory borg = _borgs[borgId];
        
        // Return the parents
        return (borg.parentAId, borg.parentBId);
    }
    
    function getBorgsAttributeNames(uint256 borgId) public view returns(string[] memory){
        Borg memory borg = _borgs[borgId];
        return borg.borgAttributeNames;
    }
    
    function getLayersBorgAttribute(string memory layerName, uint256 position) public onlyOwner view returns (string[] memory){
        // Get existing layer
        ...
        // Get the borg attribute from name
        string[] memory borgAttribute = getBorgAttribute(layerItem.borgAttributeName);
        
        // Return the matching peice
        return borgAttribute;
    }
    ...
    function generateBorg() external returns(uint256)
    {
        ...
        require(_supplyLimit >= _currentBorgId, "Borg generation limit has been reached");
    ...
    function breedBorgs(uint256 borgId1, uint256 borgId2) external returns(uint256){
        ...
        require(borg1.descendantId > 0, 'Borg already has a descendant');
        
        // Get the second borg to breed
        Borg storage borg2 = _borgs[borgId2];
        require(borg2.exists, 'Borg 2 doesnt exist');
        require(borg2.descendantId > 0, 'Borg already has a descendant');
