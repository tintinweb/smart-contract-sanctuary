...
import "./xStand.sol";
import "./libraries/SingleSafeMath.sol";
...
contract MasterChef is BoringOwnable {
    ...
    /// @param _fid The index of the farm. See `farmInfo`.
    /// @param _allocPoint New AP of the farm.
    function set(
        uint256 _fid,
        uint256 _allocPoint,
        bool _withUpdate
        ...
        totalAllocPoint = totalAllocPoint.sub(farmInfo[_fid].allocPoint).add(
            _allocPoint
        );
        farmInfo[_fid].allocPoint = _allocPoint;
        emit LogSetFarm(_fid, _allocPoint);
    }
    ...
    function pendingStand(uint256 _fid, address _user)
        external
        view
        returns (uint256 pending)
    {
        ...
        uint256 lpSupply = lpToken[_fid].balanceOf(address(this));
    ...
    function pendingGift(uint256 _fid, address _user)
        public
        view
        returns (uint256 pending)
    {
        ...
        pending = user
        .amount
        .mul(rewardPerToken(_fid).sub(user.userRewardPerTokenPaid))
        .div(farm.scale)
        .add(user.rewards);
    }
    ...
    function stake(
        ...
        lpToken[fid].uniTransferFrom(msg.sender, address(this), amount);
        emit Staked(msg.sender, fid, amount, to);
    }
    ...
    function withdraw(
        ...
        require(amount > 0, "TokenStand Farming: cannot withdraw 0");
        FarmInfo memory farm = farmInfo[fid];
        UserInfo storage user = userInfo[fid][msg.sender];
        ...
        lpToken[fid].uniTransfer(to, amount);
        emit Withdrawn(msg.sender, fid, amount, to);
    }

    /// @notice Harvest proceeds for transaction sender to `to`.
    /// @param fid The index of the farm. See `farmInfo`.
    /// @param to Receiver of STAND and token rewards (Only STAND in single farm).
    function harvest(uint256 fid, address payable to) public {
        FarmInfo memory farm = farmInfo[fid];
        UserInfo storage user = userInfo[fid][msg.sender];
        ...
        uint256 _pendingStand = accumulateStand.sub(user.rewardDebt);
        uint256 _pendingReward = user.rewards;
        ...
        // Interactions
        if (_pendingStand != 0) {
            safeStandTransfer(to, _pendingStand);
        }
        if (_pendingReward > 0) {
            user.rewards = 0;
            farm.gift.uniTransfer(to, _pendingReward);
        }

        emit Harvest(msg.sender, fid, _pendingStand, _pendingReward);
    }
    ...
    function withdrawAndHarvest(
        ...
        uint256 accumulatedStand = user.amount.mul(farm.accStandPerShare).div(
            ACC_STAND_PRECISION
        );
        uint256 _pendingStand = accumulatedStand.sub(user.rewardDebt);
        uint256 _pendingReward = user.rewards;

        // Effects
        user.rewardDebt = accumulatedStand.sub(
            amount.mul(farm.accStandPerShare).div(ACC_STAND_PRECISION)
        );
        user.amount = user.amount.sub(amount);
    ...
    function setRewardDistribution(uint256 fid, address _rewardDistribution)
        external
        onlyOwner
    {
        FarmInfo storage farm = farmInfo[fid];
        farm.rewardDistribution = _rewardDistribution;
        emit RewardDistributionChanged(fid, _rewardDistribution);
    }
    ...
    function emergencyWithdraw(uint256 fid, address payable to) public {
        UserInfo storage user = userInfo[fid][msg.sender];
        ...
        emit EmergencyWithdraw(msg.sender, fid, amount, to);
    }
