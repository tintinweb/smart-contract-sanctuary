...
contract LionStaking is ERC20, Ownable {
    ...
    uint public constant MAX_STAKING_SUPPLY = 6 * (10 ** 6) * (10 ** 18);

    uint public constant MAX_DAO_SUPPLY = 6 * (10 ** 6) * (10 ** 18);

    uint public constant MAX_TEAM_AND_INVESTORS_SUPPLY = 6 * (10 ** 6) * (10 ** 18);

    uint public constant MAX_ECOSYSTEM_SUPPLY = 3 * (10 ** 6) * (10 ** 18);

    uint public constant MAX_LIQUIDITY_SUPPLY = 3 * (10 ** 6) * (10 ** 18);

    uint public constant MAX_PARTNERSHIPS_SUPPLY = 225 * (10 ** 4) * (10 ** 18);

    uint public constant MAX_RESERVE_SUPPLY = 375 * (10 ** 4) * (10 ** 18);

    uint public constant STAKING_PER_DAY = 5053353 * (10 ** 10);

    uint private _totalStakingSupply;

    uint private _totalDaoSupply;

    uint private _totalTeamAndInvestorsSupply;

    uint private _totalEcosystemSupply;

    uint private _totalLiquiditySupply;

    uint private _totalPartnershipsSupply;

    uint private _totalReserveSupply;
    ...
    function mintDao(address to, uint amount) public onlyOwner {
        require(_totalDaoSupply + amount <= MAX_DAO_SUPPLY, "Dao token amount is exceeded");

        _mint(to, amount);

        _totalDaoSupply += amount;
    }

    function mintTeamAndInvestors(address to, uint amount) public onlyOwner {
        require(_totalTeamAndInvestorsSupply + amount <= MAX_TEAM_AND_INVESTORS_SUPPLY, "Team and investors token amount is exceeded");

        _mint(to, amount);

        _totalTeamAndInvestorsSupply += amount;
    }

    function mintEcosystem(address to, uint amount) public onlyOwner {
        require(_totalEcosystemSupply + amount <= MAX_ECOSYSTEM_SUPPLY, "Ecosystem token amount is exceeded");

        _mint(to, amount);

        _totalEcosystemSupply += amount;
    }

    function mintLiquidity(address to, uint amount) public onlyOwner {
        require(_totalLiquiditySupply + amount <= MAX_LIQUIDITY_SUPPLY, "Liquidity token amount is exceeded");

        _mint(to, amount);

        _totalLiquiditySupply += amount;
    }

    function mintPartnerships(address to, uint amount) public onlyOwner {
        require(_totalPartnershipsSupply + amount <= MAX_PARTNERSHIPS_SUPPLY, "Partnerships token amount is exceeded");

        _mint(to, amount);

        _totalPartnershipsSupply += amount;
    }

    function mintReserve(address to, uint amount) public onlyOwner {
        require(_totalReserveSupply + amount <= MAX_RESERVE_SUPPLY, "Reserve token amount is exceeded");

        _mint(to, amount);

        _totalReserveSupply += amount;
    }
    ...
    */
    function stakingEndTimestamp() public view  returns (uint) {
        ...
        return _stakingStart + 3 * 86400 + SECONDS_IN_A_DAY * 365 * 3;
    }

    function claim(uint[] memory tokenIndices) public {
        ...
        require(totalClaimAmount != 0, "No LNSTKNRWRD accumulated");
