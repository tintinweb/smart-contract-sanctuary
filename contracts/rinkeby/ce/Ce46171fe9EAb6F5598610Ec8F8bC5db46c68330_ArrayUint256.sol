...
library ArrayUint256 {
    ...
    function push(uint256[] storage data, uint256 v) public
    {
    ...
    function remove(uint256[] storage data, uint256 v) public
    {
    ...
    // 升序
    function ascendingInsert(uint256[] storage data, uint256 price) public
    {
        if (data.length == 0)
        {
            data.push(price);
    ...
    // 降序
    function descendingInsert(uint256[] storage data, uint256 price) public
    {
        if (data.length == 0)
        {
            data.push(price);
...
contract OrderManagerReal is OrderManager {
    ...
    address USDT = 0xd9424bEEdF7C21EAd6fdC54088F2f38b252bACd8;
    ...
    function createOrder(address _tokenContract, address _sender, uint256 _price, uint256 _token, uint256 _usdt, uint8 _type) public override returns (uint256 _orderId)
    {
    ...
    // 插入订单
    function insertOrder(address _tokenContract, uint256 _orderId, address _sender) public override returns (bool _flag)
    {
        Data storage data = map[_tokenContract];
        Order storage order = data.orderList[_orderId];
    ...
    // 移除订单
    function removeOrder(address _tokenContract, uint256 _orderId, address _sender) public override returns (bool _flag)
    {
        Data storage data = map[_tokenContract];
        Order storage order = data.orderList[_orderId];
    ...
    function handleMatchOrder(address _tokenContract, uint256 _orderId) public override
    {
        Data storage data = map[_tokenContract];
        Order storage order = data.orderList[_orderId];
