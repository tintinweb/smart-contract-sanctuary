...
contract EthereumTowers is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    // Prep data for a new tower
    function setMaxStandard(uint256 _max) external onlyOwner {
        maxStandard = _max;
    }

    function setMaxLuxury(uint256 _max) external onlyOwner {
        maxLuxury = _max;
    }

    function setMaxPenthouse(uint256 _max) external onlyOwner {
        maxPenthouse = _max;
    }

    function setTokenIndexStandard(uint256 _tokenIndex) external onlyOwner {
        tokenIndexStandard = _tokenIndex;
    }

    function setTokenIndexLuxury(uint256 _tokenIndex) external onlyOwner {
        tokenIndexLuxury = _tokenIndex;
    }

    function setTokenIndexPenthouse(uint256 _tokenIndex) external onlyOwner {
        tokenIndexPenthouse = _tokenIndex;
    }

    function resetCounterStandard() external onlyOwner {
        require(
            counterStandard == maxStandard,
            "Current tower not fully minted"
        );
        counterStandard = 0;
    }

    function resetCounterLuxury() external onlyOwner {
        require(counterLuxury == maxLuxury, "Current tower not fully minted");
        counterLuxury = 0;
    }

    function resetCounterPenthouse() external onlyOwner {
        require(
            counterPenthouse == maxPenthouse,
            "Current tower not fully minted"
        );
        counterPenthouse = 0;
    }
