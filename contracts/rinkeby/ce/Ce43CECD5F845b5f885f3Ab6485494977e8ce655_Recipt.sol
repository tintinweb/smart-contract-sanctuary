...
contract Recipt is 
    ...
    uint256 public constant maxSupply = 6; // Genesis set
    ...
    function writeRecipt(uint256 tokenId, string memory text) public returns (uint256) {
        return writeRecipt(tokenId, 0, text);
    }

    // Writes input text onto the chain with a reference to a previous message
    function writeRecipt(uint256 tokenId, uint256 prevId, string memory text) public returns (uint256) { // TODO: TEST
        require(enabled, "Contract not enabled");
