 ...
 *      EthBank manages the minting and burning of tokens which
 *      represent evrnet based assets, while EvrnetBank manages
 *      the locking and unlocking of ETH/ERC20/ERC721 token assets
 *      based on eth.
 **/

contract BridgeBank is Initializable, EthBank, BridgeBankPausable {
    ...
    /*
     * @dev: Locks received ERC721 funds.
     *
     * @param _recipient: representation of destination address.
    ...
    function lock721(
        ...
        require(_token != address(0), "Token Contract Address cannot be the zero address.");

        // ERC721 deposit
        BridgeToken721(_token).transferFrom(
            msg.sender,
            address(this),
            _tokenId
        );
        // Set symbol to the ERC721 token's symbol
        string memory symbol = BridgeToken721(_token).symbol();

        lockFunds721(payable(msg.sender), _recipient, _token, symbol, _tokenId, _chainName);
    }

    /*
     * @dev: Unlocks ERC721 tokens held on the contract.
     *
     * @param _recipient: recipient's eth address
     * @param _token: token contract address
     * @param _symbol: token symbol
     * @param _tokenId: Token ID of ERC721.
     */
    function unlock721(
        ...
        // Confirm that the bank owns this token
        require(
            BridgeToken721(_tokenAddress).ownerOf(_tokenId) == address(this),
            "The Bank does not own this token."
        );
        require(
            unlockCompleted[_interchainTX].isUnlocked == false,
            "Transactions has been processed before"
        );
        unlockFunds721(_recipient, _tokenAddress, _symbol, _tokenId);
    }
...
abstract contract BridgeToken721 is ERC721
{
}
...
contract EthBank {
    ...
    function lockFunds(
        ...
        emit LogLock(_sender, _recipient, _token, _symbol, _amount, lockBurnNonce, _chainName);
    }
    ...
    function refunds(
        ...
        if (_tokenAddress == address(0)) {
            ...
            require(
                BridgeToken(_tokenAddress).transfer(_recipient, _amount),
                "Token transfer failed"
            );
        }
        refundCompleted[_nonce].isRefunded = true;

        emit LogRefund(_recipient, _tokenAddress, _symbol, _amount);
    }

    /*
     * @dev: Creates a new Eth deposit with a unique id.
     *
     * @param _sender: The sender's eth address.
     * @param _recipient: The intended recipient's evrnet address.
     * @param _token: The currency type, either erc20 or eth.
     * @param _tokenId: Token ID of ERC721.
     */
    function lockFunds721(
        address payable _sender,
        address _recipient,
        address _token,
        string memory _symbol,
        ...
        lockBurnNonce++;
        emit LogLock721(_sender, _recipient, _token, _symbol, _tokenId, lockBurnNonce, _chainName);
    }
    ...
    function unlockFunds721(
        ...
        // Transfer funds to intended recipient
        BridgeToken721(_token).transferFrom(address(this), _recipient, _tokenId);
        emit LogUnlock721(_recipient, _token, _symbol, _tokenId);
    }
}
