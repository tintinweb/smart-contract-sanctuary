...
/**
 * Paymaster that allows to pay for fees with RBTC (bridged RSK BTC).
 ...
 */
contract LogPaymaster is BasePaymaster {
    using SafeMath for uint256;

    uint256 public gasUsedByPost;
    IERC20 public rbtc;
    AggregatorV3Interface private btcbnbFeed;

    function versionPaymaster() external view virtual override returns (string memory) {
        return "2.2.0+opengsn.rbtc.ipaymaster";
    }

    event TokensCharged(uint256 gasUseWithoutPost, uint256 gasUsedByPost, uint256 bnbActualCharge, uint256 tokenActualCharge);
    event PreRelayedValues(address from, address to, uint256 maxPossibleGas);
    event PostRelayedValues(bytes context, bool success, uint256 tokenRefund);

    constructor(IERC20 _rbtc, AggregatorV3Interface _btcbnbFeed) {
        rbtc = _rbtc;
    ...
    function _getTokenAmountNeeded(uint256 bnbMaxCharge) internal view returns (uint256) {
        (, int256 price, , , ) = btcbnbFeed.latestRoundData();
        // The price value represents how many smallest division of BNB costs 1 RBTC
        // So to convert bnbMaxCharge to the smallest division of RBTC we would do: bnbMaxCharge / (price / 10**18)
        // But for precision, we will rather multiply the nominator instead of divising the denominator: bnbMaxCharge * 10**18 / price
        return bnbMaxCharge.mul(10**18).div(uint256(price));
    }
    ...
    function preRelayedCall(
        ...
        emit PreRelayedValues(relayRequest.request.from, relayRequest.request.to, maxPossibleGas);
        address payer = relayRequest.request.from;
        uint256 bnbMaxCharge = relayHub.calculateCharge(maxPossibleGas.add(gasUsedByPost), relayRequest.relayData);
        bnbMaxCharge += relayRequest.request.value;
        uint256 tokenPrecharge = _getTokenAmountNeeded(bnbMaxCharge);
    ...
    function postRelayedCall(
        ...
        uint256 bnbActualCharge = relayHub.calculateCharge(gasUseWithoutPost.add(gasUsedByPost), relayData);
        uint256 tokenActualCharge = _getTokenAmountNeeded(bnbActualCharge);
        ...
        emit PostRelayedValues(context, success, tokenRefund);
        emit TokensCharged(gasUseWithoutPost, gasUsedByPost, bnbActualCharge, tokenActualCharge);
    }
}
...
library GsnEip712Library {
        ...
        )
    {
        ...
        (forwarderSuccess, ret) = relayRequest.relayData.forwarder.call(
            abi.encodeWithSelector(IForwarder.execute.selector, relayRequest.request, _domainSeparator, RELAY_REQUEST_TYPEHASH, suffixData, signature)
        );
        if (forwarderSuccess) {
    ...
    function domainSeparator(address forwarder) internal pure returns (bytes32) {
        return hashDomain(EIP712Domain({ name: "GSN Relayed Transaction", version: "2", chainId: getChainID(), verifyingContract: forwarder }));
    }
    ...
    function hashDomain(EIP712Domain memory req) internal pure returns (bytes32) {
        return keccak256(abi.encode(EIP712DOMAIN_TYPEHASH, keccak256(bytes(req.name)), keccak256(bytes(req.version)), req.chainId, req.verifyingContract));
    }
