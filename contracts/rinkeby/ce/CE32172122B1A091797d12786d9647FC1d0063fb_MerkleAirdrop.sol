...
contract MerkleAirdrop {
    ...
    function claim(
        ...
        require(_amount > 0);
        require(
            checkProof(_proof, getLeaf(_who, _amount)),
            "failed proof check"
        );

        hasClaimed[_who] = true;

        // IERC20(airdropToken).safeTransfer(_who, _amount);

        emit Claim(_who, _amount);
        return true;
    }

    function checkProof(bytes32[] calldata _proof, bytes32 _hash)
        internal
        view
        returns (bool)
    {
