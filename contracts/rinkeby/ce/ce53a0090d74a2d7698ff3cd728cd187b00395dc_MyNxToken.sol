...
contract RequestableOwnable is Ownable, Pausable {
    ...
    /**
     * @dev Destroys the contract and refunds the owner.
...
contract MyNxToken is RequestableOwnable, ERC721Full {

    string constant internal NAME = "NXTokenFoo";
    string constant internal SYMBOL = "NXTF";
    uint256 constant internal TOKENID = 1;

    event ApprovalRecipient(address indexed admin, address indexed approved, uint256 indexed tokenId);
    event TokenUriSet(address indexed admin, uint256 indexed tokenId, string uri);

    // Mapping from token ID to recipient address approved by the admin.
    mapping (uint256 => address) private _tokenApprovalRecipients;
    ...
    function transferFrom(address from, address to, uint256 tokenId) public whenNotPaused {
        require(_isApprovedRecipientOrIsAdmin(_msgSender(), to, tokenId), "MyNxToken: recipient is not approved by admin");
        _clearApprovalRecipient(tokenId);
    ...
    function setApprovalForAll(address, bool) public {
        revert("MyNxToken: setApprovalForAll is closed off");
    }
    ...
    /**
     * @dev Approves another address to transfer to the given token ID
     * The zero address indicates there is no approved address.
     * There can only be one approved address per token at a given time.
     * Can only be called by the contract owner.
     * @param to address to be approved as a recipient for the given token ID
     * @param tokenId uint256 ID of the token to be approved
     */
    function approveRecipient(address to, uint256 tokenId) public onlyOwner whenNotPaused {
        require(to != ownerOf(tokenId), "MyNxToken: approval to current owner");

        _tokenApprovalRecipients[tokenId] = to;
        emit ApprovalRecipient(_msgSender(), to, tokenId);
    }

    /**
     * @dev Gets the admin-approved recipient address for a token ID, or zero if no address set
     * Reverts if the token ID does not exist.
     * @param tokenId uint256 ID of the token to query the admin approval of
     * @return address currently admin-approved as a recipient for the given token ID
     */
    function getApprovedRecipient(uint256 tokenId) public view returns (address) {
        require(_exists(tokenId), "MyNxToken: approved query for nonexistent token");
        return _tokenApprovalRecipients[tokenId];
    }

    /**
     * @dev Returns whether the given spender can transfer the given token ID to the given
     * recipient.
     * @param spender address of the spender to query
     ...
     * is an operator of the owner, or is the owner of the token
     */
    function _isApprovedRecipientOrIsAdmin(address spender, address to, uint256 tokenId) internal view returns (bool) {
        return (spender == owner() || getApprovedRecipient(tokenId) == to);
    }

    /**
     * @dev Private function to clear current approval by admin of a given token ID.
     * @param tokenId uint256 ID of the token to be cleared
     */
    function _clearApprovalRecipient(uint256 tokenId) private {
        if (_tokenApprovalRecipients[tokenId] != address(0)) {
            _tokenApprovalRecipients[tokenId] = address(0);
        }
    }
}