...
contract BattleDoge is ERC721, VRFConsumerBase, Ownable {
   ...
   function Attack (uint256 tokenId, uint256 _tokenId , uint256 _number)  public  returns (uint256 _winner)   {
     ...
     Game memory newGame = Game(index, tokenId, _tokenId, _number, ownerOf(_winner));
       ...
       if ((characters[tokenId].strength ** 10/3) + (characters[tokenId].intelligence ** 10/3) > (characters[_tokenId].strength ** 10/3) + (characters[_tokenId].intelligence ** 10/3) ** _number / i){
    ...
    function get_game_info(uint256 _index) external view returns(uint256 _i, uint256 tokenId, uint256 _tokenId, uint256 _number, address _winner) {
       
        Game memory game = gameIndex[_index];
        return (_index, game.player1, game.player2, game.amount, game.winner);
    }
