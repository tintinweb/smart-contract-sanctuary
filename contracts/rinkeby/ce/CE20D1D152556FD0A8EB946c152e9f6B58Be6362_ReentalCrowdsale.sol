...
import "./interfaces/IReentalDividends.sol";
import "./interfaces/IReentalFactory.sol";
import "./interfaces/IWETH.sol";
import "./utils/UUPSUpgradeableByRole.sol";
import "./interfaces/IUniswapV2Router01.sol";

contract ReentalCrowdsale is UUPSUpgradeableByRole {
  ...
  function setMinimum(uint256 min) public onlyRole(DEFAULT_ADMIN_ROLE) {
    minimum = min;
  }
  ...
  function getRates() public view returns(uint256 usdtRate, uint256 eurRate) {
    return (_getRate(IReentalDeployer(config).get(keccak256("USDT_USD_FEED"))), _getRate(IReentalDeployer(config).get(keccak256("EUR_USD_FEED"))));
  }
  ...
  function _contribute(address account, address token, uint256 amountIn) internal returns (uint256, uint256) {
    ...
    if(payback > 0){
      require(IERC20(IReentalDeployer(config).get(keccak256("USDT"))).transfer(account, payback), "ReentalCrowdsale: payback not transfered");
    }
  ...
  function contributionETH(address token, uint256 amountOutMin, address[] calldata path, uint256 deadline) public payable whenNotPaused {
    ...
    address router = IReentalDeployer(config).get(keccak256("ROUTER"));
  ...
  function contributionTokens(address token, uint256 amountIn, uint256 amountOutMin, address[] calldata path, uint256 deadline) public payable whenNotPaused {
    address account = _msgSender();
    address entryToken = path[0];
    address router = IReentalDeployer(config).get(keccak256("ROUTER"));

    require(IERC20(entryToken).transferFrom(account, address(this), amountIn), "ReentalCrowdsale: entry transfer not successful");

    if (entryToken == IReentalDeployer(config).get(keccak256("USDT"))) {
      ...
      require(path[path.length - 1] == IReentalDeployer(config).get(keccak256("USDT")), "ReentalCrowdsale: USDT must be the end of the path");
      require(IERC20(entryToken).approve(router, amountIn), "ReentalCrowdsale: router approval failed");
  ...
  function contributionDividends(address token, uint256 index, uint256 amount, bytes32[] calldata merkleProof, uint256 amountIn) public whenNotPaused {
    address account = _msgSender();
    IReentalDividends dividends = IReentalDividends(IReentalDeployer(config).get(keccak256("REENTAL_DIVIDENDS")));
    dividends.reinvest(index, account, amount, merkleProof, address(this), amountIn);

    (uint256 tokens, uint256 payback) = _contribute(account, token, amountIn);
    emit Contribution(token, IReentalDeployer(config).get(keccak256("USDT")), account, tokens, payback, true);
  }
  ...
  function getTokensByContribution(address token, uint256 amount) public view returns (uint256, uint256) {
    ...
    require(tokenData.enabled, "ReentalCrowdsale: project does NOT exist");
  ...
  function enableToken(address token, uint256 price, uint256 poolPercent) public onlyRole(DEFAULT_ADMIN_ROLE) {
    require(token != address(0), "ReentalCrowdsale: token address is the zero address");
    address factory = IReentalDeployer(config).get(keccak256("REENTAL_FACTORY"));
  ...
  function extractTokens(address token) public onlyRole(DEFAULT_ADMIN_ROLE) {
      ...
      require(tokenInterface.transfer(_msgSender(), balance), "ReentalCrowdsale: extraction failed");
  }

  function extractETH() public onlyRole(DEFAULT_ADMIN_ROLE) {
      require(payable(_msgSender()).send(address(this).balance), "ReentalCrowdsale: funds transfer failed");
  }
}
