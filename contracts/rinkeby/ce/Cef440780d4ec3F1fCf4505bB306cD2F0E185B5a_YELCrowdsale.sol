...
contract YELCrowdsale is Ownable {
    ...
    function buyTokens() public payable isPresale {
        ...
        uint256 tradeAmountInUSD = tokens * tokenPriceUSD;
    ...
    function getPriceInWeiPerToken(uint256 _tokenPriceUSD) public view returns(uint256) {
        int oraclePriceTemp = getLatestPriceETHUSD(); // with 1e8
        require(oraclePriceTemp > 0, "Invalid price");

        return _tokenPriceUSD / uint256(oraclePriceTemp) * 1e8; // result with 1e18
    }
