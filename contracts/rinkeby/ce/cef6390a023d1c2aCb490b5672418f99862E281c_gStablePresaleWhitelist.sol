...
contract gStablePresaleWhitelist is ReentrancyGuard, Ownable {
    ...
    function isWhitelisted(address _address) public view returns(bool isIndeed) {
        return whitelisted[_address];
    }
   

    function whitelist(address newAddress) public onlyOwner returns(bool) {
        require(!isWhitelisted(newAddress));
        whitelisted[newAddress] = true;
        return true;
    }

    function removeSenderExcludedAddresses(address oldAddress) public onlyOwner returns(bool) {
        require(isWhitelisted(oldAddress));
    ...
    function buyTokens() public payable nonReentrant isContractActive onlyWhitelisted {
