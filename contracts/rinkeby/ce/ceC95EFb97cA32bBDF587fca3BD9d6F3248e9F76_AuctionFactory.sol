...
contract AuctionFactory is IAuctionFactory, ERC721Holder {
    ...
    function bid(uint256 _auctionId) external payable override returns (bool) {
        ...
        if (auction.balanceOf[_bidder] > auction.highestBid) {
            auction.highestBid = auction.balanceOf[_bidder];
        }
    ...
    function finalize(uint256 auctionId, address[] calldata winners)
        external
        override
        returns (bool)
    {
        ...
        require(
            auction.balanceOf[winners[0]] == auction.highestBid,
...
interface IAuctionFactory {
    ...
    function finalize(uint256 auctionId, address[] calldata winners) external returns (bool);
