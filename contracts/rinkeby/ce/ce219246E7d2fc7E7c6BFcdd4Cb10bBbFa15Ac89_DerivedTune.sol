...
contract DerivedTune is IERC721, ERC165, IERC721Metadata {
    ...
    TunesArtV2 public tuneArt = TunesArtV2(0xe665f007f1f472073541539c35AC774dB6855853);

    constructor() public {
        emit Transfer(address(0), 0x45af3Bd5A2c60B7410f33C313c247c439b633446, 2700);
    }
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                'Derived Tune Art',
                                '", "description":"',
                                'An on chain representation of Tune art, owned by the Tune owner. Non trasferrable',
                                '", "image": "',
                                tuneArt.getApprovedMetadata(tokenId, 'ofc'),
                                '"}'
                            )
                        )
                    )
...
contract TunesArtV2 is Ownable{


    IERC721Enumerable public tunes = IERC721Enumerable(0x52B1dd5c27705aa4DFd3889db223b5C4c84f6B54);

    function getTokenOwner(uint tokenId) public view returns (address owner) {
        require (tunes.ownerOf(tokenId) != address(0), 'invalid tokenID');
        return tunes.ownerOf(tokenId);
    }
    ...
    mapping(address => mapping(string => mapping(uint => string))) public metadata;

    function addOfficialMetadata(address _devAddress, string memory _key, string memory _metadataKeyAlias) public onlyOwner {
    ...
    function getApprovedMetadata(uint _tokenID, string memory _metadataKeyAlias) public view returns (string memory) {
        OfficialMetadata memory officialMetadataInstance = officialMetadata[_metadataKeyAlias];
        return metadata[officialMetadataInstance.devAddress][officialMetadataInstance.key][_tokenID];
    }

    function getMetadata(address _devAddress, string memory _key, uint _tokenID) public view returns (string memory) {
        return metadata[_devAddress][_key][_tokenID];
    }

    function setMetadata(address _devAddress, string memory _key, uint _tokenID, string memory _data) public {
        require (msg.sender == getTokenOwner(_tokenID), 'invalid token owner');
        metadata[_devAddress][_key][_tokenID] = _data;
    }

    function setApprovedMetadata(string memory _metadataKeyAlias, uint _tokenID, string memory _data) public {
        require (msg.sender == getTokenOwner(_tokenID), 'invalid token owner');
        OfficialMetadata memory officialMetadataInstance = officialMetadata[_metadataKeyAlias];
        metadata[officialMetadataInstance.devAddress][officialMetadataInstance.key][_tokenID] = _data;
    }
