...
contract WholeCont  is ERC721Enumerable, Ownable
{
    ...
    uint public constant MAX_QT_PER_TRANSACTION = 10;
    ...
    function mint(uint chosenAmount) public payable isSaleOpen{
        ...
        require(chosenAmount <= MAX_QT_PER_TRANSACTION,"Can only mint up to 10 per purchase");
        require(totalSupply().add(chosenAmount + RESERVE) <= _TOTALSUPPLY, "Purchase would exceed max supply of Bulls");
    ...
    function calcStartingIndex() public onlyOwner { 
        ...
        startingIndex = uint(blockhash(startingIndexBlock)) % _TOTALSUPPLY;
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if(block.number.sub(startingIndexBlock) > 255) {
            startingIndex = uint(blockhash(block.number - 1)) % _TOTALSUPPLY;
        }
