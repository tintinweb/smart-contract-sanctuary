...
contract RiverSide is OwnableUpgradeable, IERC721ReceiverUpgradeable, PausableUpgradeable, VRFConsumerBaseUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function calcRewardFisherman(uint256 tokenId) public view returns (uint256 owed) {
    ...
    if (totalSalmonEarned < MAXIMUM_GLOBAL_SALMON) {
        owed = (MathUpgradeable.max(block.timestamp,resetTime) - stake.value) * rate / 1 days;

    } else if (stake.value > lastClaimTimestamp) {
        owed = 0;

    } else {
        owed = (lastClaimTimestamp - stake.value) * rate / 1 days; // stop earning additional $WOOL if it's all been earned
    }
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      if (!migrated[tokenID]) {
          ...
          if (fisherman) {
            ...
            uint80 value;
            address owner;
            (tokenId,value,owner) = genXStaking.Bears(alpha,packIndices[tokenID]);        
