...
contract SUPERHERO is ERC721Enumerable, Whitelist {
  using Strings for uint256;
...
function mintAsOwner(uint _mintAmount) public onlyOwner{
    ...
    if(isSaleStarted) {
        require(_mintAmount <= nftPerTxLimitOnGeneralSale, "max NFT per address exceeded");
    }
    ...
    function mint(uint _mintAmount) public payable {
        ...
        if(block.timestamp < generalSaleTime) {
            ...
            require(ownerMintedCount + _mintAmount <= nftPerAddressLimitOnPreSale, "max NFT per address exceeded  for pre sale");
            for (uint256 i = 1; i <= _mintAmount; i++) {
  ...
  function startPresale() public onlyOwner {
      isSaleStarted=true;
      generalSaleTime = block.timestamp + 30 minutes;
  }
