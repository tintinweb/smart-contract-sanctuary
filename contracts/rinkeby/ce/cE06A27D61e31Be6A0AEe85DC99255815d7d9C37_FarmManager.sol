...
interface IBusToken {
    ...
    function transferFrom(address from, address to, uint tokenId) external;
    function safeTransferFrom(address from, address to, uint tokenId, bytes memory _data) external;
    function isPirate(uint16 id) external view returns (bool); // 5 or 7 tokens to stake
}
interface IPennyNew {
...
contract FarmManager is Ownable, IERC721Receiver, Pausable {
    // Global constants
    uint public constant DAILY_PENNY_RATE = 400000 ether; //200 ether;
    uint public constant DAILY_SHIP_PENNY_RATE = 1600000 ether; //600 ether;
    ...
    IBusToken public busToken; // ship
    IPennyNew public pennyNew; // new in-game currency
    INewManager public manager;

    // Global stats fields
    uint public totalNewPennyClaimed;
    ...
    struct LandStake {
        address owner;
        uint16 tokenId;
        uint80 value; // start date either WEI for pirates
    }

    mapping(address => bool) public approvedManagers;

    // Land stake fields
    mapping(uint256 => uint256) shipIndices;
    mapping(uint256 => uint256) goldMinerIndices;
    mapping(uint256 => uint256) pirateIndices;
    mapping(address => LandStake[]) public goldMinerStake;
    mapping(address => LandStake[]) public pirateStake;
    mapping(address => LandStake[]) public shipStake;

    // For STEAL mechanics
    mapping(address => uint256) pirateHolderIndex; // index of a concrete pirate owner
    ...
    function setNewManager(address _address) external onlyOwner {
        manager = INewManager(_address);
    }
    ...
    // ships arrived (every unstake: total += 1)
    // ships sailing
    // ships total supply


    function setToken(address _address) external onlyOwner {
        addManager(_address); // to keep stake/unstake transfers
        testToken = IToken(_address);
    }
    function setBus(address _address) external onlyOwner {
        addManager(_address); // to keep stake/unstake transfers
        busToken = IBusToken(_address);
    }
    function setPenny(address _addressNew) external onlyOwner {
        pennyNew = IPennyNew(_addressNew);
    }
    
    function stakeTokens(address account, uint16 shipId, uint16[] memory tokenIds) public {
        // 1. Handle ships
        if (shipId != 0) {
            require(busToken.ownerOf(shipId) == msg.sender, "This NTF does not belong to address");
            require(manager.isWaterproof(shipId) == true, "The ship should be able to go Phase 2");
            busToken.transferFrom(msg.sender, address(this), shipId);
            _stakeShips(account, shipId);
        }
        // 2. Stake the rest of the tokens
        for (uint i = 0; i < tokenIds.length; i++) {
            require(testToken.ownerOf(tokenIds[i]) == msg.sender, "This NTF does not belong to address");
            require(manager.isWaterproof(tokenIds[i]) == true, "The token should be able to go Phase 2");
            testToken.transferFrom(msg.sender, address(this), tokenIds[i]);

            if (testToken.isPirate(tokenIds[i])) {
    ...
    function stakeTokensToLand(address account, uint16 shipId, uint16[] memory tokenIds) public {
        require(account == msg.sender || msg.sender == address(manager), "Only manager can do this");
        // 1. Handle ships
        if (shipId != 0) {
            require(busToken.ownerOf(shipId) == address(manager), "This NTF does not belong to address");
            require(manager.isWaterproof(shipId) == true, "The ship should be able to go Phase 2");
            busToken.transferFrom(address(manager), address(this), shipId);
            //_stakeShips(account, shipId);
        }
        // 2. Stake the rest of the tokens
        for (uint i = 0; i < tokenIds.length; i++) {
            require(testToken.ownerOf(tokenIds[i]) == address(manager), "This NTF does not belong to address");
            require(manager.isWaterproof(tokenIds[i]) == true, "The token should be able to go Phase 2");
            testToken.transferFrom(address(manager), address(this), tokenIds[i]);
    ...
    function _stakeShips(address account, uint16 tokenId) internal whenNotPaused {
        ...
        shipStake[account].push(LandStake({
            owner: account,
    ...
    function _stakeGoldMiners(address account, uint16 tokenId) internal whenNotPaused {
        ...
        goldMinerStake[account].push(LandStake({
            owner: account,
            tokenId: uint16(tokenId),
            value: uint80(block.timestamp)
        }));
    ...
    function _stakePirates(address account, uint16 tokenId) internal {
        ...
        pirateStake[account].push(LandStake({
            owner: account,
            tokenId: uint16(tokenId),
    ...
    function claimFromStake(uint16[] calldata shipTokenIds, uint16[] calldata tokenIds, bool unstake) external whenNotPaused {
        ...
        if (owed == 0) return;
        pennyNew.mint(msg.sender, owed);

        // Update total claimed value
        totalNewPennyClaimed += owed;
    }

    function _claimFromMiner(uint16 tokenId, bool unstake) internal returns (uint owed) {
        LandStake memory stake = goldMinerStake[msg.sender][goldMinerIndices[tokenId]];
        ...
        owed = ((block.timestamp - stake.value) * DAILY_PENNY_RATE) / 1 days;
        if (unstake) {
            if (tokenId >= 10000) { // For GEN 1 a 5% chance to STEAL the token by some pirate
                if (getSomeRandomNumber(tokenId, 100) <= 5) {
                    // Congratz, this token goes to somebody
                    address luckyPirate = randomPirateOwner();
                    testToken.safeTransferFrom(address(this), luckyPirate, tokenId, "");
                    _payTax(owed);
                    owed = 0;
                }
            }
            ...
            LandStake memory lastStake = goldMinerStake[msg.sender][goldMinerStake[msg.sender].length - 1];
            ...
            testToken.safeTransferFrom(address(this), msg.sender, tokenId, "");
        } else {
            ...
            goldMinerStake[msg.sender][goldMinerIndices[tokenId]] = LandStake({
                owner: msg.sender,
                tokenId: uint16(tokenId),
                value: timestamp
            }); // reset stake
    ...
    function _claimFromShip(uint16 tokenId, bool unstake) internal returns (uint owed) {
        LandStake memory stake = shipStake[msg.sender][shipIndices[tokenId]];
        require(stake.owner == msg.sender, "This NTF does not belong to address");
        require(!(unstake && block.timestamp - stake.value < MINIMUM_TIME_TO_EXIT), "Need to wait 2 days since last claim");

        owed = ((block.timestamp - stake.value) * DAILY_SHIP_PENNY_RATE) / 1 days;
        if (unstake) {
            if (tokenId >= 10000) { // For GEN 1 a 5% chance to STEAL the token by some pirate
                if (getSomeRandomNumber(tokenId, 100) <= 5) {
                    // Congratz, this token goes to somebody
            ...
            LandStake memory lastStake = shipStake[msg.sender][shipStake[msg.sender].length - 1];
            ...
            shipStake[msg.sender][shipIndices[tokenId]] = LandStake({
                owner: msg.sender,
                tokenId: uint16(tokenId),
                value: timestamp
            }); // reset stake
    ...
    function _claimFromPirate(uint16 tokenId, bool unstake) internal returns (uint owed) {
        require(testToken.ownerOf(tokenId) == address(this), "This NTF does not belong to address");

        LandStake memory stake = pirateStake[msg.sender][pirateIndices[tokenId]];
        ...
        if (unstake) {
            ...
            LandStake memory lastStake = pirateStake[msg.sender][pirateStake[msg.sender].length - 1];
            ...
            pirateStake[msg.sender][pirateIndices[tokenId]] = LandStake({
                owner: msg.sender,
                tokenId: uint16(tokenId),
                value: uint80(pirateReward)
            }); // reset stake
