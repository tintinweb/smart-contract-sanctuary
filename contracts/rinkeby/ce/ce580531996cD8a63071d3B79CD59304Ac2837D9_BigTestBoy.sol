...
// Hey, it me @roleswtf, generative role theorist. gm.
...
// wagmi.
contract BigTestBoy is ERC721Enumerable, ReentrancyGuard, Ownable {

    uint256 public maxSupply = 6969;
    uint256 public price = 0;
    uint256 public maxMint = 6969;
    uint256 public numTokensMinted;
    uint256 public maxPerAddress = 6969;

    bool public allSalesPaused = true;

    mapping(address => uint256) private _mintPerAddress;

    string[] private rolePrefixes = [
        "abstract",
        "accidental",
        "aesthetic",
        "AI",
    ...
    function getPrefix(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "prefix", rolePrefixes);
    }
    
    function getRoot(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "root", roleRoots);
    }
    
    function getSuffix(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "suffix", roleSuffixes);
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" width="500" height="500" viewBox="0 0 350 350">';
        parts[1] = '<style>.base {fill: white; font-family: monospace; font-size: 13px;}</style>';
        parts[2] = '<rect width="100%" height="100%" fill="black" />';
        parts[3] = '<text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" class="base">';
        parts[4] = getPrefix(tokenId);
        parts[5] = ' ';
        parts[6] = getRoot(tokenId);
        parts[7] = ' ';
        parts[8] = getSuffix(tokenId);
        parts[9] = '</text>';
        parts[10] = '</svg>';
        ...
        bytes(
            ...
            abi.encodePacked(
                ...
                '",',
                '"description": "Assigning on-chain generative roles for the metaverse and beyond. 6969 Roles available. Rarity is up to you. Brought to you by @roleswtf.",',
    ...
    constructor() ERC721("BigTestBoy", "BTBY") Ownable() {}
}
