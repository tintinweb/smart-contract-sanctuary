...
contract DogHouseToken is Context, IERC20, Ownable {
    ...
    string constant private _name = "DogHouse";
    string constant private _symbol = "DHT";
    ...
    uint256 private numTokensSellToAddToLiquidity = 5000  * 10**9;  // 50 000 000 (+ 9 decimals)
    ...
    constructor () public {
        ...
        _devWalletAddress = 0x783e8529602E4492A55a51c1655B22a8AE703700;
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount,uint256 tax_fee) private {
        (uint256[3] memory rValues, uint256[6] memory tValues) =  _getValues(tAmount, tax_fee);
        uint256 rAmount = rValues[0];
        uint256 rTransferAmount = rValues[1];
    ...
    function _getValues(uint256 tAmount, uint256 tax_fee) private view returns (uint256[3] memory, uint256[6] memory) {
        uint256[6] memory tValues = _getTValues(tAmount, tax_fee);
    ...
    function _getTValues(uint256 tAmount, uint256 tax_fee) private view returns (uint256[6] memory) {
        uint256[6] memory tValues = [0, calculateTaxFee(tAmount, tax_fee), calculateLiquidityFee(tAmount), calculateMarketingFee(tAmount), calculateBurnFee(tAmount), calculateDevFee(tAmount)];
        tValues[0] = _getTTransferAmount(tAmount, tValues);
        return tValues;
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 tBurn, uint256 tDev,uint256 currentRate) private pure returns (uint256[3] memory) {
        ...
        uint256[6] memory tempRValues = [rAmount, rLiquidity, rFee, rMarketing, rBurn, rDev];
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, amount, _taxFee);
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, amount,  _taxFee);
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, amount, _taxFee);
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
