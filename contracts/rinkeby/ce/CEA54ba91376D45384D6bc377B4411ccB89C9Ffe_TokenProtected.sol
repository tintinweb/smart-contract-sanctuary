...
contract LGEWhitelisted is Context {
    ...
    event BlackListEnabled(uint256 indexed blockNumber);
    ...
    returns (bool){
        require(_blacklist[_msgSender()]==0 && _blacklist[sender]==0, 'Blocked');
        if (_msgSender() == _lgePairAddress) {
    ...
    function _applyLGEWhitelist(
        ...
        if (_lgeTimestamp == 0 && _msgSender() == _lgePairAddress && amount > 0){
            ...
            emit BlackListEnabled(_lgeBlockNumber);
        }
