...
contract FactoryMaster {
    ...
    function createChild(Types types,string memory name,string memory symbol,uint256 _interestsRate) external {
        //check input
        ...
        if (types == Types.erc2917) {
            require(_interestsRate >= 0, "gia tri phair lon hon ko");
            //check fee
    ...
    function testPayable(uint fee) public payable {
...
contract ERC2917 is IERC2917,ERC20,UpgradableProduct,UpgradableGovernance,ReentrancyGuard
{
    ...
    constructor(string memory name_,string memory symbol_,uint256 _interestsRate)
        ERC20(name_,symbol_)
        UpgradableProduct()
