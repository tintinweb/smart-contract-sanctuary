...
// File: new_nightworld/NightWorld.sol
...
contract NightWorld is ERC721Enumerable, Ownable {
  ...
  uint256 public _rewardAmount = 0.1 ether;
  ...
  function getSalePhase() external view returns(bool, bool){
      return(_PreSaleOpen, _PubSaleOpen);
  }
  ...
  function mintWL(uint256 numberOfTokens) external payable {
    require(numberOfTokens > 0, 'numberOfTokens err.');
    require(_PreSaleOpen, 'Contract is not active');
    require(_WhilteList[msg.sender] > 0, 'You are not on the Allow List');
