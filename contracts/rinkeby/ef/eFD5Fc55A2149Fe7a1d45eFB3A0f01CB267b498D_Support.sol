...
contract Support {
    ...
    function generateBytesAuction1155(address erc721Token, uint256 tokenId, address seller, address buyer, address erc20Token, uint256 bid, uint256 tokens, string memory start, string memory end, uint256 nonce) public pure returns (bytes memory) {
        return abi.encodePacked(erc721Token, tokenId, seller, buyer, erc20Token, bid, tokens, start, end, nonce);
    }
}