    ...
    function    mint(uint256 quantity)                      external payable callerIsUser
    {
        ...
        if (block.timestamp>=salesTimestamp)                                // On est dans le public sales
        {
            uint256 mintedNftCount = totalSupply();
                 ...
                 if (mintedNftCount<1000)       isStakingAllowed = true;    // Permettre au 1000 premiers de recevoir le STAKING gratuitement, sinon passer au mode standard
            else if (mintedNftCount>=9000)      isStakingAllowed = true;    // Les 1000 derniers aussi pourront staker Grauitement
        }
        else
        {
