...
* @author Niftydude
*/
contract ComicStaking is Ownable, IERC721Receiver, Pausable {
    using EnumerableSet for EnumerableSet.UintSet; 

    IERC20 public rewardToken;
    IERC721 public stakedTokenContract;
    ...
    * 
    * @param _lockupPeriod the reward amount
    */
    ...
    * 
    * @param _user the address owning the tokens
    * @param _tokenIds the token ids to stake
    */
    function stakeFor(address _user, uint256[] calldata _tokenIds) external whenNotPaused {
        _stake(_user, _tokenIds);
    }

    /**
    * @notice stake given token ids
    * 
    ...
    * @param tokenIds the token ids to stake
    */
    function _stake(address _user, uint256[] calldata tokenIds) internal {
        require(tokenIds.length <= 40, "Stake: Max amount exceeded");

        for (uint256 i; i < tokenIds.length; i++) {
            stakedTokenContract.safeTransferFrom(_user, address(this), tokenIds[i]);

            stakes[tokenIds[i]] = Stake(uint128(block.timestamp + lockupPeriod), uint128(block.number));
            stakedTokens[_user].add(tokenIds[i]);
        }        
    ...
    */
    function unstake(uint256[] calldata tokenIds) external whenNotPaused {
        require(tokenIds.length <= 40, "Unstake: Max amount exceeded");
    ...
    */
    function calculateRewardsByAccount(address account) public view returns (uint256) {
