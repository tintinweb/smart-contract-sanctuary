...
contract TestCatryoshkas is ERC721, ERC721Enumerable, Ownables {

    using SafeMath for uint256;

        string private _metadataAPI;
    uint256 private _bigCatsMinted;
    uint256 private _reserveSupplyMinted;
    uint256 private _presaleMints = 10;
    uint256 private _saleStartUTS = 1629219600;
    uint256 private constant _MAX_BIG_CATS = 3;
    uint256 private constant _MAX_RESERVE_SUPPLY = 12;
    uint256 private constant _MAX_MINT_SUPPLY = 28;
    uint256 private constant _MINT_PRICE = 0 ether;
    uint256 private constant _MAX_SPECIAL_CATS = 6;
    uint256[_MAX_SPECIAL_CATS] private _specialCats;
    ...
    modifier isSupplyAvailable(uint256 numberOfTokens_) {
        
        require(
            totalMintedSupply().add(numberOfTokens_) <= availableMints(),
            "Exceeds max supply"
        );
    ...
    function mintTokens(uint256 numberOfTokens_) public payable isSupplyAvailable(numberOfTokens_) {
        
        require(numberOfTokens_.mul(_MINT_PRICE) >= msg.value,  "Invalid ether value");
        
        if(!hasSaleStarted()){

            require(totalMintedSupply().add(numberOfTokens_) <= _presaleMints, "Exceeds presale limit");
    ...
    function _mintTokens(uint256 numberOfTokens_) internal {
        ...
        for (uint256 index = 0; index < numberOfTokens_; index++) {
            ...
            _safeMint(msg.sender, totalMintedSupply());

        }

    }

    function giveAway(uint256 startIndex_,  uint256 numberOfTokens_, address to_) public onlyOwners {
        
        _mintReserveTokens(startIndex_, numberOfTokens_, to_);

    }

     function _mintReserveTokens(uint256 startIndex_, uint256 numberOfTokens_, address to_ ) internal {
        
        require( _reserveSupplyMinted.add(numberOfTokens_) <= _MAX_RESERVE_SUPPLY, "Exceeds reserve");

        _reserveSupplyMinted = _reserveSupplyMinted.add(numberOfTokens_);

        for (uint256 counter = 0; counter < numberOfTokens_; counter++) {
            
            _safeMint(to_, startIndex_.add(counter));
    ...
    function breed(uint256 idCat1_ , uint256 idCat2_) public {
        ...
        require(_bigCatsMinted <= _MAX_BIG_CATS, "all big cats minted");
        ...
        uint256 mintIndex = availableMints().add(_bigCatsMinted);
        _mintReserveTokens(mintIndex, 1, _msgSender());
    ...
    function setPresaleMints(uint256 mints_) public onlyOwner {

        _presaleMints = mints_;
    ...
    function getMaxMintSupply() public pure returns (uint256) {
    ...
    function availableMints() public pure returns(uint256) {

        return _MAX_MINT_SUPPLY.sub(_MAX_RESERVE_SUPPLY);

    }
    
    function getMaxReserveSupply() public pure returns(uint256) {

        return _MAX_RESERVE_SUPPLY;
    ...
    function isSpecialCat(uint256 id_) public view returns(bool) {
        ...
        for (uint256 index = 0; index < _specialCats.length; index++) {

            if(_specialCats[index] == id_) {
                isSpecial = true;
            }
    ...
    function getSaleStartDate() public view returns (uint256) {

        return _saleStartUTS;
    ...
    function lengthArr() public view returns(uint256) {
        ...
        return _specialCats.length;

    }

    function totalMintedSupply() public view returns(uint256) {

        return totalSupply().sub(_reserveSupplyMinted);

    }

    function presaleMints() public view returns(uint256) {

        return _presaleMints;

    }

    function getReserveSupply() public view returns(uint256) {

        return _reserveSupplyMinted;

    }

    function bindSpecialCats(uint256 insertIndex, uint256[] memory specials_) public onlyOwners {

        for (uint256 index = 0; index < specials_.length; index++) {
             
            _specialCats[insertIndex] = specials_[index];
