...
contract QqqCollection is ERC721Enumerable, Ownable {
  ...
  uint256 public maxSaleMintAmount;
  ...
  constructor(
   ...
   uint256 _maxPreSaleMintAmount
    ...
    maxSaleMintAmount = _maxSaleMintAmount;
  ...
  function mint(address _to, uint256 _mintAmount) public payable {
    ...
    require(block.timestamp > startPreSaleDate, "pre-sale not started");
    require(_mintAmount > 0, "mint amount should be more than 0");
    require(supply + _mintAmount <= maxAmount, "max collection supply");

    if (block.timestamp - startPreSaleDate < 24 hours) {
      require(_mintAmount + balanceOf(_to) <= maxPreSaleMintAmount, "max pre-sale mint amount");
      ...
      require(_mintAmount + balanceOf(_to) <= maxSaleMintAmount, "max sale mint amount");
