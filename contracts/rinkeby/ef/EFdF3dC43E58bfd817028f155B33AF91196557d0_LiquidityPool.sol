...
contract LiquidityPool is ILiquidityPool, ERC20Upgradeable, AbstractDependant {
        ...
        returns (uint256)
    {
        uint256 _normalizedAmount =
            _isIntegration
                ? integrationBorrowInfos[_userAddr].normalizedAmount
                : borrowInfos[_userAddr].normalizedAmount;

        return _getAbsoluteAmount(_normalizedAmount, compoundRateKeeper.getCurrentRate());
    }
        ...
        returns (uint256)
    {
        ...
        uint256 _availableLiquidityInUSD;

        if (_isIntegration) {
            (_availableLiquidityInUSD, ) = integrationCore.getAvailableLiquidity(_userAddr);
        } else {
            (_availableLiquidityInUSD, ) = defiCore.getAvailableLiquidity(_userAddr);
        }
    ...
    function getMaxToRepay(address _userAddr, bool _isIntegration)
        external
        view
        override
        returns (uint256)
    {
        ...
        return
            Math.min(
                _convertFromUnderlyingAsset(_userBalance),
                _getAbsoluteAmount(_normalizedAmount, _newCompoundRate)
            );
    }
    ...
    function _getNormalizedAmount(
        ...
        if (_isAdding || _amountWithoutInterest != 0) {
            uint256 _normalizedAdditionalAmount = (_additionalAmount * DECIMAL) / _currentRate;
    ...
    function _updateUserBorrowInfo(
        ...
        borrowInfo.normalizedAmount = _getNormalizedAmount(
            0,
    ...
    function _borrowInternal(uint256 _amountToBorrow) internal returns (uint256 _currentRate) {
        ...
        _currentRate = _updateCompoundRate();
    ...
    function _getRepayBorrowVars(
        ...
        _repayBorrowVars.currentAbsoluteAmount = _getAbsoluteAmount(
            _repayBorrowVars.normalizedAmount,
            _repayBorrowVars.currentRate
        );
