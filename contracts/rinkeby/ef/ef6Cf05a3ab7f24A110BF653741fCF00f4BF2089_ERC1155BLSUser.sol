...
contract ERC1155BLSUser is OwnableUpgradeable, ERC1155BurnableUpgradeable, ERC1155Lazy, HasContractURI {

    event CreateERC1155BLSUser(address owner, string name, string symbol);

    string public name;
    string public symbol;

    function __ERC1155BLSUser_init(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators) external initializer {
        ...
        __ERC1155BLSUser_init_unchained(_name, _symbol);
        _setBaseURI(baseURI);
        for(uint i = 0; i < operators.length; i++) {
            setApprovalForAll(operators[i], true);
        }
        emit CreateERC1155BLSUser(_msgSender(), _name, _symbol);
    }

    function __ERC1155BLSUser_init_unchained(string memory _name, string memory _symbol) internal initializer {
        name = _name;
        symbol = _symbol;
    }
