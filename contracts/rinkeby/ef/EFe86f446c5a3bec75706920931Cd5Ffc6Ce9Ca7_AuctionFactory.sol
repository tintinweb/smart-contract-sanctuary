...
contract AuctionFactory is IAuctionFactory, ERC721Holder, Ownable {
        ...
        returns (bool)
    {
        ...
        addBidder(_auctionId, _bidder, _bid);

        // uint256 bidderBalance = auction.bidBalance[_bidder];
        // auction.bidBalance[_bidder] = auction.bidBalance[_bidder].add(_bid);

        // if (bidderBalance == 0) {
        //     auction.numberOfBids = auction.numberOfBids.add(1);
        // }

        // if (auction.bidBalance[_bidder] > auction.highestTotalBid) {
        //     auction.highestTotalBid = auction.bidBalance[_bidder];
        // }

        // if (
        //     auction.bidBalance[_bidder] < auction.lowestTotalBid ||
        //     auction.lowestTotalBid == 0
        // ) {
        //     auction.lowestTotalBid = auction.bidBalance[_bidder];
        // }
        ...
        returns (bool)
    {
        ...
        require(
            auction.bidBalance[bidders[0]] == auction.highestTotalBid,
            "First address should have the highest bid"
        );

        for (uint256 i = 1; i < bidders.length; i++) {
            if (
                auction.bidBalance[bidders[i - 1]] <
        ...
        returns (bool)
    {
        ...
        auction.bidBalance[_recipient] = 0;
    ...
    function addBidder(uint256 auctionId, address bidder, uint256 bid) internal {
