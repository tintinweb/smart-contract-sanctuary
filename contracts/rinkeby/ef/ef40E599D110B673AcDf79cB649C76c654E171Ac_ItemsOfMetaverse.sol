...
contract KingsOfMetaverse is ERC721Enumerable, ERC721URIStorage, ERC1155Holder {
    ...
    mapping (uint256 => Metadata) public stats;
    ...
    function tokenURI(uint256 id) public view override(ERC721, ERC721URIStorage) returns (string memory) {
      ...
      return itemsOfMetaverse.uriBatch(stats[id].lockedItems);
  }
    
    function mintKing(address player) public payable returns(uint256) {
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        _mint(player, newItemId);
        uint256[] memory theArray = new uint256[](7);
        stats[newItemId] = Metadata(100, 10, 100, 1, theArray);
        return newItemId;
    }

    function equipItems(uint256 kingId, uint256[] memory ids, uint256[] memory ammounts) public payable returns (uint256[] memory) {
        require(exists(kingId), "Mint King first.");
        require(ownerOf(kingId) == msg.sender, "Not your king");
        require(ids.length == ammounts.length);
        uint256[] memory ammountsUnequip = new uint256[](6);
        uint256[] memory idsUnequip = new uint256[](6);
        bool toUnequip = false;
        for (uint i = 0; i < ids.length; i++) {
            require(ammounts[i] == 1, "Ammount should be 1");
            uint256 _itemType = itemsOfMetaverse.getItemType(ids[i]);
            if (stats[kingId].lockedItems[_itemType] == 1) {
                ammountsUnequip[i] = 1;
                idsUnequip[i] = stats[kingId].lockedItems[_itemType];
                toUnequip = true;
            }
        }   
        if (toUnequip) {
            itemsOfMetaverse.safeBatchTransferFrom(address(this), msg.sender, idsUnequip, ammountsUnequip, "0x0");
        }     
        itemsOfMetaverse.safeBatchTransferFrom(msg.sender, address(this), ids, ammounts, "0x0");
        for (uint i = 0; i < ids.length; i++) {
            uint256 _itemType = itemsOfMetaverse.getItemType(ids[i]);
            stats[kingId].lockedItems[_itemType] = ids[i];
        }
        uint256 newHealth = 100;
        uint256 newDamage = 10;
        uint256 newArmor = 100;
        uint256 newMultiplier = 0;

        for (uint i = 0; i < 7; i++) {
            uint256 item = stats[kingId].lockedItems[i];
            newHealth += itemsOfMetaverse.getMetadata(item).health;
            newDamage += itemsOfMetaverse.getMetadata(item).damage;
            newArmor += itemsOfMetaverse.getMetadata(item).armor;
            newMultiplier += itemsOfMetaverse.getMetadata(item).multiplier;
        }        

        stats[kingId].health = newHealth;
        stats[kingId].damage = newDamage;
        stats[kingId].armor = newArmor;
        stats[kingId].multiplier = newMultiplier;
        return stats[kingId].lockedItems;
...
contract ItemsOfMetaverse is ERC1155 {
    ...
    constructor() ERC1155("") {
      minter = msg.sender;
      owner = msg.sender;
      addItemMetadata("EXPLORER'S CHEST", 1, 10, 0, 10, 1, '<defs><image  width="93" height="124" id="chest" href="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAF0AAAB8AgMAAAB271TDAAAAAXNSR0IB2cksfwAAAAlQTFRFAAAAOT1EMzhB3biazAAAAAN0Uk5TAP//RFDWIQAAADRJREFUeJxjYIAAxlAICGBAA6MSoxIjQCIUO3AYlRiVGEESq7CDhlGJUYlRiVGJUYkRJAEAQxQYmQX6ijQAAAAASUVORK5CYII="/></defs><style>tspan { white-space:pre }</style><use  href="#chest" x="93" y="93" />');
      addItemMetadata("EXPLORER'S LEGS", 2, 5, 0, 5, 1, '<defs><image  width="93" height="62" id="legs" href="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAF0AAAA+AgMAAAA0f/+kAAAAAXNSR0IB2cksfwAAAAlQTFRFOT1EGh0hAAAAjc4yGgAAAAN0Uk5T//8A18oNQQAAAB9JREFUeJxjYBgFo2AUhEJA2CoIWArlO4xKjEqMHAkAPqBGaS5Pkd8AAAAASUVORK5CYII="/></defs><style>tspan { white-space:pre }</style><use  href="#legs" x="93" y="217" />');
      addItemMetadata("EXPLORER'S HEAD", 3, 5, 0, 5, 1, '<defs><image  width="93" height="93" id="head" href="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAF0AAABdBAMAAAA1X3fwAAAAAXNSR0IB2cksfwAAAA9QTFRFAAAAGh0hKSsuXzQ0LBkZYRx+xQAAAAV0Uk5TAP////8c0CZSAAAAQklEQVR4nO3KsQ2AMBAEwacEOrCoAAlKoP+aHJ8Ti+yD2XSnKjvOrDbxPM/zTfz1q8HzPM839U/2ftmyb57neb6nn/JhIUk2hXQcAAAAAElFTkSuQmCC"/></defs><style>tspan { white-space:pre }</style><use  href="#head" x="93" y="0" />');
      addItemMetadata("EXPLORER'S BOOTS", 4, 0, 0, 2, 1, '<defs><image  width="93" height="31" id="boots" href="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAF0AAAAfAQMAAAC/L1PnAAAAAXNSR0IB2cksfwAAAAZQTFRFKSsuAAAAczgrKQAAAAJ0Uk5T/wDltzBKAAAAGElEQVR4nGNgYGBg/P///x8GKBjlDDQHAA+0e0cUnWvcAAAAAElFTkSuQmCC"/></defs><style>tspan { white-space:pre }</style><use  href="#boots" x="93" y="279" />');
      addItemMetadata("EXPLORER'S SHIELD", 5, 50 ,5, 100, 1, '<defs><image  width="93" height="93" id="shield" href="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAF0AAABdAQMAAAD9v/iAAAAAAXNSR0IB2cksfwAAAAZQTFRFKSsuGh0h0YxxzQAAABtJREFUeJxjYBgFgxAw/v///88oZzBxRsGgAgAtantH42VRCQAAAABJRU5ErkJggg=="/></defs><style>tspan { white-space:pre }</style><use  href="#shield" x="31" y="124" />');
      addItemMetadata("EXPLORER'S SWORD", 6, 0, 10, 0, 1, '<defs><image  width="93" height="155" id="sword" href="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAF0AAACbAgMAAAB9rn/5AAAAAXNSR0IB2cksfwAAAAlQTFRFAAAAQkhQGh0hwMkEaQAAAAN0Uk5TAP//RFDWIQAAADxJREFUeJzt07EVACAIA9HIhDbu4yjUTmkBFc8J5K7MryNFY0VTJQAAAAAAgBZw3m0AaASZ5eD1OADwP1zXAoJ0yps+ngAAAABJRU5ErkJggg=="/></defs><style>tspan { white-space:pre }</style><use  href="#sword" x="186" y="31" />');
