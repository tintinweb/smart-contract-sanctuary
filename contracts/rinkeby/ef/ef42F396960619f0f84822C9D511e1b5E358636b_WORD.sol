...
contract WORD is ERC721Full {
    using SafeMath for uint256;

    mapping(string  => uint256) value_to_id;
    mapping(uint256 => string) id_to_value;

    constructor() ERC721Full("WORD", "WORD") public {
        _mint(msg.sender, 0);

        string[27] memory basic_characters = ["a","b","c","d","e","f","g","h","i","j","k","l","m","n","o","p","q","r","s","t","u","v","w","x","y","z","-"];

        for (uint256 i = 0; i < basic_characters.length; ++i )
        {
            string memory character = basic_characters[i];
            _registerToken(character);
        }
    }
    ...
    function getWord(uint256 id) public view returns (string memory word) {
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        ...
        parts[0] = '<svg   xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><defs><style>.cls-1{font-size:300px;fill:#f2f2f2;font-family:CornerA-Regular, Corner A;}</style></defs><title>';
        parts[1] = letter;
        parts[2] = '</title><rect x="-12" y="-12" width="534" height="534"/><text class="cls-1" transform="translate(165.32 350.47)">';
        parts[3] = letter;
        parts[4] = '</text></svg>';
        string memory output = string(abi.encodePacked(parts[0], parts[1], parts[2], parts[3], parts[4]));
        parts[5] = '{ "name": "HELLO WORD",';
        parts[6] = '"description":"HELLO WORD",';
        parts[7] = '"image": "data:image/svg+xml;base64",';
        parts[8] =  Base64.encode(bytes(output));
        string memory json = Base64.encode(bytes(string(abi.encodePacked(parts[5],parts[6],parts[7],parts[8]))));
        string memory json1 = string(abi.encodePacked(parts[5],parts[6],parts[7],parts[8]));
