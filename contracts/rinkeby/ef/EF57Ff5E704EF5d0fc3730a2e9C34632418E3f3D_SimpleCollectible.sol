...
contract SimpleCollectible is ERC721, Ownable {
    ...
    function batchMint() public onlyOwner payable {
        uint256 numberOfNfts = 2;
        string memory url = append("https://gateway.pinata.cloud/ipfs/QmearqU5XrKTE6eCjpPnggxnXcr3qD3TAGmfvbDpmueKnT/metadata-", uint2str(tokenCounter), ".json");
    ...
    // function appendToFourDigits(string memory number) public returns (string memory _appendedNumber) {
    //     string memory s;

    //     while (bytes(number).length < 3) {
    //           s = append(s, "0", "");
    //     }
