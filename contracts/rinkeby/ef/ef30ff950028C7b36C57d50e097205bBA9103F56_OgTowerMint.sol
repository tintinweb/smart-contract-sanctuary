...
contract OgTowerMint is ERC721PresetMinterPauserAutoId {
    ...
    function setGateway(string memory gatewayURI) public ownerOnly {
        ...
        setBaseURI(
            string(
                abi.encodePacked(
                    _currentGatewayURI,
                    "QmRt2wr65DnVt8KMnYDYEcybhq7kcdX4S37MfLYpTSTvPY/"
                )
            )
        );
    }

    function addToWhitelist(address[] memory toAdd, uint256[] memory pick)
        external
        ownerOnly
    {
    ...
    function bulkMint(uint256[] memory pick) external ownerOnly {
        for (uint256 i = 0; i < pick.length; i++) {
            _mintToken(pick[i]);
        }
    }

    function singleMint(uint256 pick) external {
         require(
           pick <= 100 && pick >= 0,
    ...
    function _mintToken(uint256 pick) private returns (uint256) {
    ...
    function getOg(uint256 tokenId) public view returns(address) {
     require(addressExist[tokenId], "No address available for token");
     return Ogs[tokenId];
   }
}
