...
contract HydraBridge is Pausable, HydraAccessControl {
    ...
    event Deposit(uint8 chainId, bytes32 assetId, uint64 lockNonce);
    event Voted(uint8 chainId, uint64 lockNonce, ProposalStatus status, bytes32 dataHash);
    event StatusChanged(uint8 chainId, uint64 lockNonce, ProposalStatus status, bytes32 dataHash);
    ...
    function hasVotedOnProposal(
        uint72 _chainIdAndNonce,
        bytes32 _dataHash,
        address _observer
    ) public view returns (bool) {
        return hasVoted(proposals[_chainIdAndNonce][_dataHash], _observer);
    }
    ...
    function getProposal(
        ...
        uint72 key = (uint72(_lockNonce) << 8) | uint72(_chainId);
    ...
    function deposit(
        ...
        uint64 nonce = ++lockNonce[_chainId];

        IVault depositHandler = IVault(vaultAddress);
        depositHandler.lock(_assetId, _chainId, nonce, msg.sender, data);

        emit Deposit(_chainId, _assetId, nonce);
    }
    ...
    function voteProposal(
        ...
        uint72 nonceAndChainId = (uint72(_lockNonce) << 8) | uint72(_chainId);
        Proposal memory proposal = proposals[nonceAndChainId][_dataHash];
        ...
        if (proposal.status == ProposalStatus.Inactive) {
            ...
            emit StatusChanged(_chainId, _lockNonce, ProposalStatus.Active, _dataHash);
        }
        // Expired
        else if ((block.number - proposal.proposedInBlock) > expirationInBlocks) {
            proposal.status = ProposalStatus.Expired;
            emit StatusChanged(_chainId, _lockNonce, ProposalStatus.Expired, _dataHash);
        }
        ...
        if (proposal.status != ProposalStatus.Expired) {
            ...
            emit Voted(_chainId, _lockNonce, proposal.status, _dataHash);

            if (proposal.votes >= votesTreshold) {
                proposal.status = ProposalStatus.Confirmed;

                emit StatusChanged(_chainId, _lockNonce, ProposalStatus.Confirmed, _dataHash);
            }
        }

        proposals[nonceAndChainId][_dataHash] = proposal;
    }
    ...
    function executeProposal(
        ...
        Proposal storage proposal = proposals[nonceAndChainId][dataHash];

        require(proposal.status == ProposalStatus.Confirmed, "HydraBridge: Proposal must be Confirmed");
        ...
        emit StatusChanged(_chainId, _lockNonce, ProposalStatus.Completed, dataHash);
    }
