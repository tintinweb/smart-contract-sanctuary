  ...
  PausableUpgradeable
{
  ...
  string private _rootFolder;
  bytes32 private _currentPresaleRoot;
  uint256 private _currentPresaleLimit;

  event Presale(bytes32 whitelistRoot, uint256 amount);
  ...
  modifier onlyWhitelisted(address account, bytes32[] memory proof) {
    require(MerkleProofUpgradeable.verify(proof, _currentPresaleRoot, bytes32(bytes20(account))), 'Whitelist proof is not valid');
    _;
  }
  ...
  function initialize(string memory __name, string memory __symbol, uint256 __length, string memory __rootFolder)
  public
    initializer()
  {
    __Ownable_init();
  ...
  function mintPresale(uint256 numberOfTokens, bytes32[] calldata proof)
  external payable
    whenPaused()
    withPayment(numberOfTokens * UNIT_PRICE)
    onlyRemaining(numberOfTokens + _currentPresaleLimit)
    onlyWhitelisted(_msgSender(), proof)
  {
  ...
  function currentPresaleId() public view returns (bytes32) {
    return _currentPresaleRoot;
  }
  ...
  function startPresale(bytes32 whitelistRoot, uint256 amount)
  external
    onlyOwner()
  {
    _currentPresaleRoot  = whitelistRoot;
    _currentPresaleLimit = remaining() - amount;
    emit Presale(whitelistRoot, amount);
  }
