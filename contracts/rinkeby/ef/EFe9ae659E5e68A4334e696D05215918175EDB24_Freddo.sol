...
// iCheetah is the contract for cheetah
//
interface iCheetah {
...
contract Freddo is ERC20, Ownable {
    ...
    iCheetah public Cheetah;

    uint256 constant public BASE_RATE = 5 ether; //rate to be ul.
    uint256 public START; // number of timestamp, it's when contract launched
    bool rewardPaused = false;

    mapping(address => uint256) public rewards; //amount of rewards,
    mapping(address => uint256) public lastUpdate;

    mapping(address => bool) public allowedAddresses;

    constructor(address cheetahAddress) ERC20("FFF", "FOKA") {
        Cheetah = iCheetah(cheetahAddress);
        START = block.timestamp;
    }

    //only cheetah contact can interact, enabled once nft is transfered from owner to seller, it migrates rewards and updates lastupdates.
    function updateReward(address from, address to) external {
        require(msg.sender == address(Cheetah));
        if(from != address(0)){
    ...
    //nft owner mints the token with rewards and pending rewards
    ...
    function burn(address user, uint256 amount) external {
        require(allowedAddresses[msg.sender] || msg.sender == address(Cheetah), "Address does not have permission to burn");
        _burn(user, amount);
    }
    ...
    function getPendingReward(address user) internal view returns(uint256) {
        return Cheetah.balanceGenesis(user) * BASE_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START)) / 86400;
    }
