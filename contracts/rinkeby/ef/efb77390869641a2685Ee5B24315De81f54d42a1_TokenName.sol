...
import "./token_nameERC721.sol";

contract TokenName is TokenName721 {
    
    modifier onlyOwnerToken(uint256 tokenId) {
        require(msg.sender == ownerOf(tokenId), "Do not own _"); 
        _;
    }
   
    uint256 constant public BREED_PRICE = 750 ether;

    mapping (address => uint256) public numberOfLegendaries;

    event breedhappened(uint256 tokenId, uint256 parent1, uint256 parent2);

    constructor(string memory baseURI) TokenName721(baseURI) {}

    function breed(uint256 parent1, uint256 parent2) external onlyOwnerToken(parent1) onlyOwnerToken(parent2) {
        uint256 supply = totalSupply();
        require(supply < maxSupply,"No more baby can be bred");
        require(parent1 < totalAmtGenesis && parent2 < totalAmtGenesis,   "Cannot fuse with baby");
        require(parent1 != parent2,"Must be two different parents");

        Token.burn(msg.sender, BREED_PRICE);
        uint256 tokenId = totalAmtGenesis + babyCount;
        babyCount++;
        _safeMint(msg.sender, tokenId);
        emit breedhappened(tokenId, parent1, parent2);
    }
    ...
    function removeReservedAmt(address[] calldata reservedAddresses, uint256 amount) external onlyOwner {
        for(uint256 i; i < reservedAddresses.length; i++){
...
interface iTTokenERC20 {
    function burn(address _from, uint256 _amount) external;
    function updateReward(address _from, address _to) external;
    function timeStamp(address user) external;
} 

contract TokenName721 is ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private baseTokenURI;

    uint256 public maxSupply = 13332;
    ...
    iTTokenERC20 public Token;

    constructor(string memory baseURI) ERC721("TokenName123", "TN123") {
        setBaseURI(baseURI);
    }
    ...
    function presaleMint(uint256 _count) public payable {
        ...
        require(reserved > 0,"Address is not on the whitelist");
        require(_count <= reserved,"Can't mint more than reserved");
        require(supply.add(_count) <= totalAmtGenesis - amtReserved,"Purchase would exceed max supply of __"); 
        require(price.mul(_count) == msg.value,"Please send correct amount of Ether");
   ...
   function mint(uint256 _count) public payable {
        ...
        require(supply.add(_count) <= totalAmtGenesis - amtReserved, "Purchase would exceed supply of ___");
    ...
    function reserveMint(uint256 _count) public payable {
        ...
        require(reserveActive,"Reserve not active");
        require(_reserved > 0,"Address is not on the reserved list");
        require(_count <= _reserved,"Can't mint more than reserved");
        require(supply.add(_count) <= totalAmtGenesis,"Purchase would exceed max supply of __"); 
    ...
    function settokenERC20(address tokenERC20Address) external onlyOwner {
        Token = iTTokenERC20(tokenERC20Address);
    }
