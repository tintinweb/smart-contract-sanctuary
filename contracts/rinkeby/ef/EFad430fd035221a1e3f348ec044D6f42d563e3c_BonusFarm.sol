...
import "../interfaces/IBonusFarm.sol";
...
abstract contract BaseReward is ReentrancyGuard {
...
contract BonusFarm is IBonusFarm, BaseReward {

    /* ========== STATES ========== */
    address public governance;
    IOolongFarm private parentFarm;
    ...
    constructor(
        address _rewardsToken,
        uint _rewardsDuration,
        IOolongFarm _parentFarm,
        uint _parentPoolId,
        address _governance
    )
        public
        BaseReward(_rewardsToken, _rewardsDuration)
    {
        parentFarm = _parentFarm;
        parentPoolId = _parentPoolId;
        (address lpToken_, , , , ) = _parentFarm.poolInfo(_parentPoolId);
        parentStakingToken = IERC20(lpToken_);
        governance = _governance;
    }

    /* ========== VIEWS ========== */

    function getParentFarm() external override view returns(address) {
        return address(parentFarm);
    }

    function getParentPoolId() external override view returns(uint) {
        return parentPoolId;
    }

    function getParentStakingToken() external override view returns(address) {
    ...
    function onParentReward(address _account) external override onlyParentFarm {
    ...
    function rescue(address _token) external onlyGov {
        if (_token == address(rewardsToken)) {
            // If parents BonusFarm reference is no longer address(this)
            // It should mean this farm has been retired, and is safe to rescue
            ( , , , , address _bonusFarm) = parentFarm.poolInfo(parentPoolId);
            require(_bonusFarm != address(this), "!retired");
        }
