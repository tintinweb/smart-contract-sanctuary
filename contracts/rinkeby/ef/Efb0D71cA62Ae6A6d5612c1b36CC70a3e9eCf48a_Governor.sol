...
contract Governor is IGovernor, PeriodTime(24 hours), ReentrancyGuard {
    using SafeMath for uint256;
    using TCPSafeMath for uint256;

    
    
    uint64 public override shutdownTime;
    
    mapping(address => uint) public emergencyShutdownTokensLocked;
    
    uint public constant EMERGENCY_SHUTDOWN_THRESHOLD = 0.25e18;
    
    uint public tcpMinted;
    
    uint public liquidationAccountRewards;
    
    uint public distributionScheduleTokens;

    
    
    address internal immutable genesisAuthenticator;
    
    string public constant NAME = 'Zhu Protocol Governor';

    
    
    IUniswapV3Pool public override protocolPool;
    
    IUniswapV3Pool public override collateralPool;
    
    IUniswapV3Pool[] public referencePool;

    
    
    IAccounting public override accounting;
    
    IAuctions public override auctions;
    
    IAuctions internal previousAuctions;
    
    ITCP public override tcp;
    
    IZhu public override zhu;
    
    IPositionNFT public override zhuPositionNFT;
    
    IEnforcedDecentralization public override enforcedDecentralization;
    
    ILend public override lend;
    
    ILendZhu public override lendZhu;
    
    ILiquidations public override liquidations;
    
    IMarket public override market;
    
    IPrices public override prices;
    
    IProtocolLock public override protocolLock;
    
    IRates public override rates;
    ...
    bool public pricePoolsInitialized;

    
    constructor(
        IAccounting _accounting,
        IZhu _zhu,
        IPositionNFT _zhuNFT,
        ILendZhu _lendZhu,
        IEnforcedDecentralization _enforcedDecentralization,
        address _timelock,
        address _genesisAuthenticator,
        uint _distributionScheduleTokens
      ) {
        protocolDeployer = msg.sender;

        genesisAuthenticator = _genesisAuthenticator;

        accounting = _accounting;
        zhu = _zhu;
        zhuPositionNFT = _zhuNFT;
        lendZhu = _lendZhu;
        enforcedDecentralization = _enforcedDecentralization;
        timelock = _timelock;

        distributionScheduleTokens = _distributionScheduleTokens;
    }
    ...
    function initContracts(
        ...
        ITCP _tcp,
        ILend _lend,
        ILiquidations _liquidations,
        IMarket _market,
        IRates _rates,
        IPrices _prices,
        IProtocolLock _protocolLock,
        IRewards _rewards,
        ISettlement _settlement) external onlyProtocolDeployer {

        
        require(!contractsInitialized && !pricePoolsInitialized);
        contractsInitialized = true;

        protocolLock = _protocolLock;
        protocolLock.authorizeCaller(address(this));
        protocolLock.authorizeCaller(address(enforcedDecentralization));

        auctions = _auctions;
        protocolLock.authorizeCaller(address(auctions));

        tcp = _tcp;
        
        protocolLock.authorizeCaller(address(zhu));

        lend = _lend;
        protocolLock.authorizeCaller(address(lend));

        liquidations = _liquidations;
        protocolLock.authorizeCaller(address(liquidations));

        market = _market;
        protocolLock.authorizeCaller(address(market));

        prices = _prices;
        protocolLock.authorizeCaller(address(prices));

        rates = _rates;
        protocolLock.authorizeCaller(address(rates));

        rewards = _rewards;
        protocolLock.authorizeCaller(address(rewards));

        settlement = _settlement;
        protocolLock.authorizeCaller(address(settlement));
    }

    
    function initPools(
        IUniswapV3Pool _protocolPool,
        IUniswapV3Pool _collateralPool,
        IUniswapV3Pool[] memory _referencePool
    ) external onlyProtocolDeployer {
        
        require(contractsInitialized && !pricePoolsInitialized);
        pricePoolsInitialized = true;
        ...
        rewards.completeSetup();
        lend.completeSetup();
        prices.completeSetup();
        rates.completeSetup();
        liquidations.completeSetup();
        auctions.completeSetup();
        market.completeSetup();

        bytes4[4] memory validUpdates = [
            this.addReferencePoolToProtocol.selector,
            this.removeReferencePoolFromProtocol.selector,
            this.executeShutdown.selector,
            this.setPhaseOneStartTime.selector];

        for(uint i = 0; i < validUpdates.length; i++) validUpdate[validUpdates[i]] = true;

        
        delete protocolDeployer;
    }

    
    
    function lockTokensForEmergencyShutdown(uint count) external nonReentrant {
        address msgSender = msg.sender;
        require(count > 0, 'Noop');
        emergencyShutdownTokensLocked[msgSender] = emergencyShutdownTokensLocked[msgSender].add(count);

        TransferHelper.safeTransferFrom(address(tcp), msgSender, address(enforcedDecentralization), count);

        _updateThresholdMetSinceBlockNumber();

        emit ShutdownTokensLocked(msgSender, count);
    }

    
    function unlockTokensForEmergencyShutdown() external nonReentrant {
        address msgSender = msg.sender;
        uint count = emergencyShutdownTokensLocked[msgSender];
        require(count > 0, 'Noop');

        emergencyShutdownTokensLocked[msgSender] = 0;

        enforcedDecentralization.transferEmergencyShutdownTokens(msgSender, count);

        _updateThresholdMetSinceBlockNumber();

        emit ShutdownTokensUnlocked(msgSender, count);
    }

    function emergencyShutdownThresholdMet() public view returns (bool) {
        return tcp.balanceOf(address(enforcedDecentralization)) > distributedTCP()._mul(EMERGENCY_SHUTDOWN_THRESHOLD);
    }

    uint private constant THRESHOLD_NOT_MET = type(uint256).max;
    uint internal thresholdMetSinceBlockNumber = THRESHOLD_NOT_MET;

    function _updateThresholdMetSinceBlockNumber() internal {
        if (emergencyShutdownThresholdMet()) {
    ...
    function executeEmergencyShutdown() external nonReentrant {
        require(thresholdMetSinceBlockNumber < block.number, 'Insufficient TCP Locked');

        _executeShutdown();
    }

    
    function executeShutdown() external override nonReentrant onlyThis {
        _executeShutdown();
    }

    
    function _executeShutdown() internal {
        require(shutdownTime == 0, 'Already shutdown');

        market.accrueInterest();
        rewards.accrueRewards();

        shutdownTime = _currentTime();
    ...
    function upgradeProtocol(address newGovernor) external override nonReentrant onlyThis {
        require(shutdownTime > 0, 'Not shutdown');

        
        tcp.burn(tcp.balanceOf(address(this)));

        tcp.addGovernor(newGovernor);

        emit ProtocolUpgraded(newGovernor);
    }

    
    
    function upgradeAuctions(IAuctions _auctions) external override nonReentrant onlyThis {
        if (address(previousAuctions) != address(0)) {
            protocolLock.unauthorizeCaller(address(previousAuctions));
            previousAuctions.stop();
        }
        previousAuctions = auctions;
        auctions = _auctions;
        protocolLock.authorizeCaller(address(auctions));

        emit ContractUpgraded('auctions', address(auctions));
    }

    
    function upgradeLend(ILend _lend) external override nonReentrant onlyThis {
        protocolLock.unauthorizeCaller(address(lend));
        lend.stop();
        lend = _lend;
        protocolLock.authorizeCaller(address(lend));

        emit ContractUpgraded('lend', address(lend));
    }

    
    function upgradeLiquidations(ILiquidations _liquidations) external override nonReentrant onlyThis {
        protocolLock.unauthorizeCaller(address(liquidations));
        liquidations.stop();
        liquidations = _liquidations;
        protocolLock.authorizeCaller(address(liquidations));

        emit ContractUpgraded('liquidations', address(liquidations));
    }

    
    function upgradeMarket(IMarket _market) external override nonReentrant onlyThis {
        protocolLock.unauthorizeCaller(address(market));
        market.stop();
        market = _market;
        protocolLock.authorizeCaller(address(market));

        emit ContractUpgraded('market', address(market));
    }

    
    function upgradePrices(IPrices _prices) external override nonReentrant onlyThis {
        protocolLock.unauthorizeCaller(address(prices));
        prices.stop();
        prices = _prices;
        protocolLock.authorizeCaller(address(prices));

        emit ContractUpgraded('prices', address(prices));
    }

    
    function upgradeRates(IRates _rates) external override nonReentrant onlyThis {
        protocolLock.unauthorizeCaller(address(rates));
        rates.stop();
        rates = _rates;
        protocolLock.authorizeCaller(address(rates));

        emit ContractUpgraded('rates', address(rates));
    }

    
    function upgradeRewards(IRewards _rewards) external override nonReentrant onlyThis {
        protocolLock.unauthorizeCaller(address(rewards));
        rewards.stop();
        rewards = _rewards;
        protocolLock.authorizeCaller(address(rewards));

        
        accounting.onRewardsUpgrade(address(rewards));

        emit ContractUpgraded('rewards', address(rewards));
    }

    
    function upgradeSettlement(ISettlement _settlement) external override nonReentrant onlyThis {
        protocolLock.unauthorizeCaller(address(settlement));
        settlement.stop();
        settlement = _settlement;
        protocolLock.authorizeCaller(address(settlement));

        emit ContractUpgraded('settlement', address(settlement));
    }

    
    
    function setPhaseOneStartTime(uint64 phaseOneStartTime) external nonReentrant onlyThis {
        require(firstRewardsPeriod == 0, 'Already set');

        firstRewardsPeriod = _timeToPeriod(phaseOneStartTime);

        enforcedDecentralization.setPhaseOneStartTime(phaseOneStartTime);
    }

    mapping(IUniswapV3Pool => uint64) public override poolRemovalTime;

    
    
    function addReferencePoolToProtocol(IUniswapV3Pool pool) external override nonReentrant onlyThis {
        require(!_contains(referencePool, pool), 'Already added');
        require(poolRemovalTime[pool] == 0, 'Previously removed');

        referencePool.push(pool);

        lend.addReferencePool(pool);
        rates.addReferencePool(pool);
        rewards.addReferencePool(pool);
        prices.addReferencePool(pool);
    }

    
    
    function removeReferencePoolFromProtocol(IUniswapV3Pool pool) external override nonReentrant onlyThis {
        require(referencePool.length > 1, 'Only one pool');

        poolRemovalTime[pool] = _currentTime();

        referencePool = _removePoolFromPools(referencePool, pool);

        lend.removeReferencePool(pool);
        rates.removeReferencePool(pool);
        rewards.removeReferencePool(pool);
    }

    
    function _removePoolFromPools(
        IUniswapV3Pool[] memory pools,
        IUniswapV3Pool pool
    ) internal pure returns (IUniswapV3Pool[] memory) {
        require(_contains(pools, pool), 'Not found');

        IUniswapV3Pool[] memory  newPools = new IUniswapV3Pool[](pools.length - 1);
        uint newPoolsIndex;
        for (uint i = 0; i < pools.length; i++) {
            if (address(pools[i]) != address(pool)) {
                newPools[newPoolsIndex] = pools[i];
                newPoolsIndex++;
            }
        }
        return newPools;
    ...
    function _contains(IUniswapV3Pool[] memory pools, IUniswapV3Pool pool) internal pure returns (bool) {
        for (uint i = 0; i < pools.length; i++) if (address(pools[i]) == address(pool)) return true;
        return false;
    }

    function increaseLiquidationAccountRewards(uint count) external override {
        _requireAuthorized(msg.sender == address(market));

        liquidationAccountRewards = liquidationAccountRewards.add(count);
    }

    
    function mintTCP(address to, uint count) external override nonReentrant {
        address caller = msg.sender;
        _requireAuthorized(caller == address(auctions) || caller == address(rates));

        tcpMinted = tcpMinted.add(count);

        tcp.mintTo(to, count);
    }

    
    function distributeLiquidityRewards(address to, uint count) external override nonReentrant {
        address caller = msg.sender;
        _requireAuthorized(caller == address(market) || caller == address(rewards));

        
        uint remaining = tcp.balanceOf(address(this));

        if (remaining < count) {
            if (remaining > 0) TransferHelper.safeTransfer(address(tcp), to, remaining);
            tcp.mintTo(to, count - remaining);
        } else {
            TransferHelper.safeTransfer(address(tcp), to, count);
        }
    }

    uint private unlocked = 1;
    
    modifier executeLock() {
    ...
    function execute(
        ...
        bytes memory data
    ) external override executeLock returns (bool success, bytes memory returnData) {
        
        _requireAuthorized(msg.sender == timelock);

        
        enforcedDecentralization.requireValidAction(target, signature);

        
        (success, returnData) = target.call(abi.encodePacked(_sig4(signature), data));
    ...
    modifier onlyProtocolDeployer() {
        _requireAuthorized(msg.sender == protocolDeployer);
        _;
    }
    ...
    function requireDecreaseDebtAccess(address caller) external view override {
        _requireAuthorized(
          caller == address(market)
          || caller == address(liquidations)
          || caller == address(settlement));
    }

    
    function requireLentZhuCountAccess(address caller) external view override {
        _requireAuthorized(caller == address(market) || caller == address(lend));
    }

    
    function requirePositionWriteAccess(address caller) external view override {
        _requireAuthorized(caller == address(market)
            || caller == address(liquidations)
            || caller == address(settlement));
    }

    
    function requireStoredCollateralAccess(address caller) external view override {
        _requireAuthorized(caller == address(market)
            || caller == address(liquidations)
            || caller == address(settlement));
    }
    ...
    function requireZhuMintingAccess(address caller) external view override {
        _requireAuthorized(caller == address(market) || caller == address(lend));
    }

    
    function requireZhuReservesBurnAccess(address caller) external view override {
        _requireAuthorized(caller == address(market) || caller == address(liquidations));
    }

    
    function requireUpdatePositionAccess(address caller) external view override {
        _requireAuthorized(caller == address(liquidations) || caller == address(settlement));
    }

    
    function getIsGenesisPhaseAndRequireAuthIfSo(
        address caller,
        GenesisAuth calldata ga
    ) external view override returns (bool isGenesis) {
        if (enforcedDecentralization.currentPhase() >= 2) return false;

        address signer =
            ECDSA.recover(
                ECDSA.toEthSignedMessageHash(
                    getSignaturePayloadForUserAddress(caller)
                ),
                ga.v,
                ga.r,
                ga.s
        ...
        require(signer == genesisAuthenticator, 'GenesisAuth failed');
        return true;
    }

    
    function requireValidAction(address target, string calldata signature) external view override {
        enforcedDecentralization.requireValidAction(target, signature);
    }

    
    function getReferencePools() external view override returns(IUniswapV3Pool[] memory) {
        return referencePool;
    }

    
    function distributedTCP() public view override returns (uint) {
        return totalLiquidityRewards().add(tcpMinted).sub(liquidationAccountRewards);
    }

    
    function currentPhase() external view override returns (uint8) {
        return enforcedDecentralization.currentPhase();
    }

    
    
    function getSignaturePayloadForUserAddress(address userAddress) public view returns (bytes32) {
        return keccak256(abi.encodePacked(bytes(NAME), chainID(), address(this), userAddress));
    }

    
    function chainID() internal pure returns (uint) {
        uint _chainID;
    ...
    uint64 internal firstRewardsPeriod;
    
    uint64 public override constant GENESIS_PERIODS = 20;
    
    uint64 private constant BOOTSTRAP_PERIODS = 365;
    
    
    uint64 private constant NON_EXTENDED_PERIODS = GENESIS_PERIODS + BOOTSTRAP_PERIODS;

    
    uint private constant GENESIS_PERIOD_REWARD_COUNT = 2e24;
    
    uint private constant TOTAL_GENESIS_REWARDS = GENESIS_PERIOD_REWARD_COUNT * GENESIS_PERIODS;

    
    uint private constant FIRST_BOOTSTRAP_PERIOD_REWARDS = 1e24;
    
    
    uint private constant TOTAL_BOOTSTRAP_REWARDS = 2012e23;
    
    uint private constant DAILY_BOOTSTRAP_DECREASE = 2465753424657534246575;
    
    uint private constant EXTENDED_PERIOD_REWARDS = 1e23;


    
    function currentDailyRewardCount() external view override returns (uint) {
        if (shutdownTime > 0) return 0;

        uint64 currentPeriod = _currentPeriod();
        if (firstRewardsPeriod == 0 || currentPeriod < firstRewardsPeriod) return 0;
        return _currentDailyRewardCount(currentPeriod - firstRewardsPeriod);
    }

    
    function totalLiquidityRewards() public view returns (uint) {
        uint64 currentPeriod = _currentPeriod();
        if (shutdownTime > 0) currentPeriod = _timeToPeriod(shutdownTime);
        if (firstRewardsPeriod == 0 || currentPeriod < firstRewardsPeriod) return 0;
        return _totalLiquidityRewards(currentPeriod - firstRewardsPeriod).add(distributionScheduleTokens);
    }

    
    function _currentDailyRewardCount(uint64 rewardsPeriod) internal pure returns (uint) {
        if (rewardsPeriod >= NON_EXTENDED_PERIODS) return EXTENDED_PERIOD_REWARDS;
        if (rewardsPeriod >= GENESIS_PERIODS) return _rewardsForBootstrapPeriod(rewardsPeriod - GENESIS_PERIODS);
        return GENESIS_PERIOD_REWARD_COUNT;
    }

    
    function _totalLiquidityRewards(uint64 rewardsPeriod) internal pure returns (uint) {
        
        
        if (rewardsPeriod >= NON_EXTENDED_PERIODS) {
            uint64 extendedPeriods = (rewardsPeriod - NON_EXTENDED_PERIODS) + 1;
            return
                TOTAL_GENESIS_REWARDS +
                TOTAL_BOOTSTRAP_REWARDS +
                extendedPeriods * EXTENDED_PERIOD_REWARDS;
        }

        
        if (rewardsPeriod >= GENESIS_PERIODS) {
            uint64 bootstrapPeriod = rewardsPeriod - GENESIS_PERIODS;
            return
                TOTAL_GENESIS_REWARDS +
                ((FIRST_BOOTSTRAP_PERIOD_REWARDS + _rewardsForBootstrapPeriod(bootstrapPeriod)) / 2) * (bootstrapPeriod + 1);
        }

        
        return (rewardsPeriod + 1) * GENESIS_PERIOD_REWARD_COUNT;
    }

    
    function _rewardsForBootstrapPeriod(uint64 bootstrapPeriod) internal pure returns (uint) {
        return uint(1e24) - (DAILY_BOOTSTRAP_DECREASE * bootstrapPeriod);
    }
}
