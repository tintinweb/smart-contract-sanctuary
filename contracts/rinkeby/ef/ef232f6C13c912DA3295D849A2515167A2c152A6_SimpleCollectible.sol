...
contract SimpleCollectible is ERC721URIStorage, Ownable {
    ...
    function createCollectible(address to, string memory tokenURI)
        public
        onlyOwner
        returns (uint256)
    {
    ...
    function updateTokenURI(
        ...
        string memory oldTokenURI,
        string memory newTokenURI
    ) public onlyOwner {
        string memory currentTokenURI = tokenURI(collectibleId);
        require(
            keccak256(abi.encodePacked(currentTokenURI)) ==
                keccak256(abi.encodePacked(oldTokenURI)),
            "Incorrect collectible ID"
        );
        _setTokenURI(collectibleId, newTokenURI);
    }
}