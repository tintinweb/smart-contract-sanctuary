...
import "../security/LGEWhitelisted.sol";
import "@openzeppelin/contracts/utils/math/SafeMath.sol";

contract TokenProtected is Context, IERC20, Ownable, LGEWhitelisted {
    ...
    */
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        _applyLGEBlacklist(sender, recipient);
...
contract LGEWhitelisted is Context {
    ...
    uint256 private _lgeTimestamp;
    address private _lgePairAddress;
    address private _lgePairBlockNumber;
    uint256 private _lgeBlockNumber;
    ...
    event BlackListAdd(address indexed blockAddress);
    event BlackListRemove(address indexed blockAddress);
    ...
    function removeFromBlackList(address blockAddress) external onlyWhitelister {
        require(_blacklist[blockAddress]>0, "address not found in blacklist");
        delete _blacklist[blockAddress];
        emit BlackListRemove(blockAddress);
    }
    ...
    function _applyLGEBlacklist(
        address sender,
        address recipient
    ) internal {
        require(_blacklist[sender]==0, 'Blocked');
        if (sender == _lgePairAddress && recipient != _lgePairAddress) {
            if(block.number - _lgeBlockNumber <= 3){
                //block forever
                _blacklist[sender] = block.timestamp;
                emit BlackListAdd(sender);
                require(false, 'Blocked Forever');
            }
        }
    }
