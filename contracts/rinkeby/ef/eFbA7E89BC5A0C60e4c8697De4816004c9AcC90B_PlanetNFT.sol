...
contract PlanetNFT is ERC721Tradable {
    ...
    function batch_create_random(
        ...
        uint256[] calldata seed_,
        string[] calldata bkgColor_,
        string[] calldata color_,
        string[] calldata color2_,
        string[] calldata linecolor_)  public onlyAdmin {
        for (uint256 i = 0; i < seed_.length; i++) {
            create_random(tokenId_[i], seed_[i], bkgColor_[i], color_[i], color2_[i], linecolor_[i]);
        }
    }
    ...
    function batch_mint_create_random(
        ...
        for (uint256 i = 0; i < seed_.length; i++) {
            mintTo(_msgSender());
            create_random(mintTokenId, seed_[i], bkgColor_[i], color_[i], color2_[i], linecolor_[i]);
        }
    }
    ...
    function create_random(uint256 tokenId,
        ...
        //random size
        newPlanet.radius=10+uint16(randomNum(140, newPlanet.randomness, block.timestamp));
        ...
        if(_angletype==0){
            ...
            newPlanet.positionY=int16(700-newPlanet.radius-2*newPlanet.rotationY)-int16(uint16(randomNum((700-newPlanet.radius-newPlanet.rotationY)*2, newPlanet.randomness, block.timestamp)));

        }else if(_angletype==1){
            newPlanet.rotationAngle=45;
            newPlanet.positionX=int16(700-newPlanet.radius-uint16(uint256(newPlanet.rotationX)*707/1000))-int16(uint16(randomNum(uint256(700-newPlanet.radius-uint256(newPlanet.rotationX)*707/1000)*2, newPlanet.randomness, block.timestamp)));
            newPlanet.positionY=int16(700-newPlanet.radius-uint16(2*uint256(newPlanet.rotationY)*707/1000))-int16(uint16(randomNum(uint256(700-newPlanet.radius-uint256(newPlanet.rotationY)*707/1000)*2, newPlanet.randomness, block.timestamp)));
