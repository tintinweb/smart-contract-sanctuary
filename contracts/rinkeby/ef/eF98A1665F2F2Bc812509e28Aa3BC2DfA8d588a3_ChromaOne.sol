...
contract ChromaOne is CollectCode
{
    ...
    constructor() ERC721("ChromaOne", "CH1") CollectCode()
    {
        config_ = Config (
            "chroma1",  // (seriesCode)
            10,         // (initialSupply)
...
abstract contract CollectCode is ERC721, Ownable
{
    ...
    function getTokenInfo(uint256 tokenId) public view returns (TokenInfo memory)
    {
        ...
        return TokenInfo(
            ...
            (msg.sender == ownerOf(tokenId)),   // sender owns it
    ...
    {
        uint256 quantity = 0;
        // return prices always, independent of availability
    ...
    {
        uint256[] memory tokenIds = new uint[](balanceOf(msg.sender));
    ...
    function buildCode_(address to, uint256 tokenId) internal
    {
        // make color palette
        ...
        // make pixels
        uint8 i = 0;
        uint8[] memory pixels = new uint8[](config_.width*config_.width*3);
        for(uint8 y = 0 ; y < config_.width ; y++) {
            for(uint8 x = 0 ; x < config_.width ; x++) {
                pixels[i++] = Utils.step_uint8( c3[x*3+0], c3[(x+config_.width)*3+0], config_.width, y );
                pixels[i++] = Utils.step_uint8( c3[x*3+1], c3[(x+config_.width)*3+1], config_.width, y );
                pixels[i++] = Utils.step_uint8( c3[x*3+2], c3[(x+config_.width)*3+2], config_.width, y );
            }
        }
        _pixels[tokenId] = Utils.convertIntsToBytes(pixels);
...
library Utils
{
    ...
    function convertBlockHashToBytes20(uint256 blockNumber) public view returns (bytes20)
    {
        // https://docs.soliditylang.org/en/v0.7.6/types.html#address
        return bytes20(blockhash(blockNumber));
    }
    ...
    function convertIntsToBytes(uint8[] memory values) public pure returns (bytes memory)
    {
        bytes memory result = new bytes(values.length);
    ...
    function convertBytesToHexString(bytes20 values) public pure returns (string memory)
    {
    ...
    // function clamp_uint8(uint8 value, uint8 min, uint8 max) public pure returns (uint8)
    // {
    ...
    // function min_uint8(uint8 a, uint8 b) public pure returns (uint8) {
    //     return a < b ? a : b;
    // }
    ...
    // function max_uint8(uint8 a, uint8 b) public pure returns (uint8) {
    //     return a > b ? a : b;
    // }
    ...
    function percent_uint8(uint8 value, uint8 percentage) public pure returns (uint8)
    {
        return uint8(int(value) * int(percentage) / 100);
    }
    ...
    function reduceColors(bytes20 s0, bytes20 s1, bytes20 s2, uint8 colorCount, uint256 offset) public pure returns (uint8[] memory)
    {
        ...
        uint256 sum0 = sum_bytes20(s0) + (offset*8);
        uint256 sum1 = sum_bytes20(s1) - (offset*8);
        bytes20 hues = lshift_bytes20(s2, sum1);
        uint256 lh = sum_bytes20(s2) + sum1;
        for(uint8 c = 0 ; c < colorCount ; c++) {
            uint8 h = uint8(hues[c]);
            uint8 s = uint8((uint256(uint8(hues[c+colorCount])) + sum0) % 256);
            uint8 v = uint8((uint256(uint8(hues[c+colorCount])) + sum1) % 256);
            s = map_uint8(s, (64+(5-(colorCount/2))*24), 255);
            v = map_uint8(h|v, (127+(5-(colorCount/2))*16), 255);
            h = uint8(map_uint256(h, lh, lh+128)%256);
    ...
    function itoa(int n) public pure returns (string memory)
    {
        return itoa(n, 10);
    }
    function itoa(int n, uint8 radix) public pure returns (string memory)
    {
