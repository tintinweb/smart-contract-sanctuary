...
contract MSBreed is Context, Pausable, AccessControlEnumerable {
    ...
    mapping(address => bool) internal nftContracts;
    mapping(address => Charge[]) internal charges;

    uint256 public maxBreedLimit;
    address internal serverAddress;

    event NFTContract(address nftContract, bool status);
    ...
    constructor(
        ...
        Charge[] memory _charges,
        uint256 _maxBreedLimit,
        ...
        for (uint256 i = 0; i < _charges.length; i++) {
            charges[_nftContract].push(_charges[i]);
        }
        maxBreedLimit = _maxBreedLimit;
        serverAddress = _serverAddress;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(roleName["PAUSER_ROLE"], _msgSender());
        _setupRole(roleName["CREATOR_ROLE"], _msgSender());
        _setupRole(roleName["WITHDRAWER_ROLE"], _msgSender());
    }

    modifier onlyPauser() {
        require(
            hasRole(roleName["PAUSER_ROLE"], _msgSender()),
            "MSBreed: Must have pauser role"
        );
        _;
    }
    ...
    modifier onlyCreator() {
        require(
            ...
            "MSBreed: Must have creator role"
        );
        _;
    }
    ...
    modifier onlyWithdrawer() {
        require(
            ...
            "MSBreed: Must have withdrawer role"
        );
        _;
    }
    ...
    function withdraw(address payable to) public whenNotPaused onlyWithdrawer {
        require(
            ...
            "MSBreed: No enough ETH to withdraw"
        );
        uint256 value = address(this).balance;
        (bool success, ) = to.call{value: value}("");
        require(
            success,
            "MSBreed: Unable to send value, recipient may have reverted"
        );
        ...
        onlyWithdrawer
    {
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            require(balance != 0, "MSBreed: No enough ERC20 token to withdraw");
    ...
    function setMaxBreedLimit(uint256 _maxBreedLimit) public onlyCreator {
        maxBreedLimit = _maxBreedLimit;
    }

    function setNFTContract(address _nftContract, bool _status)
        public
        whenNotPaused
        onlyCreator
    {
        nftContracts[_nftContract] = _status;
        emit NFTContract(_nftContract, _status);
    }

    function checkNFTContract(address _nftContract) public view returns (bool) {
        return nftContracts[_nftContract];
    }

    function setCharges(address _nftContract, Charge[] calldata _charges)
        public
        whenNotPaused
        onlyCreator
    {
        delete charges[_nftContract];
        for (uint256 i = 0; i < _charges.length; i++) {
            charges[_nftContract].push(_charges[i]);
        }
    }

    function getCharges(address _nftContract)
        public
        view
        returns (Charge[] memory)
    {
        return charges[_nftContract];
    }

    function setServerAddress(address _serverAddress) public onlyCreator {
        serverAddress = _serverAddress;
    }

    function getServerAddress() public view onlyCreator returns (address) {
    ...
    function checkBreed(
        ...
        require(
            nftContracts[_nftContract] == true,
            "MSBreed: Not a authorized nftContract"
        );
        require(matronId != 0, "MSBreed: matronId is equal to 0");
        require(sireId != 0, "MSBreed: sireId is equal to 0");
        bytes32 serverHash = keccak256(abi.encode(isEgg))
            .getEthSignedMessageHash();
        require(
            serverAddress == serverHash.recoverSigner(serverSig),
            "MSBreed: Signature error"
        );
        require(!isEgg, "MSBreed: One of Matron/Sire is egg and cannot breed");
        require(matronId != sireId, "MSBreed: matronId is equal to sireId");
        MSNFT msNFT = MSNFT(_nftContract);
        require(
            msNFT.ownerOf(matronId) == _msgSender(),
            "MSBreed: Matron token is not belong to owner"
        );
        require(
            msNFT.ownerOf(sireId) == _msgSender(),
            "MSBreed: Sire token is not belong to owner"
        );
        require(
            msNFT.getTokenBreedCount(matronId) < maxBreedLimit,
            "MSBreed: Matron cannot breed beyond limit"
        );
        require(
            msNFT.getTokenBreedCount(sireId) < maxBreedLimit,
            "MSBreed: Sire cannot breed beyond limit"
        );
    }
    ...
    function breed(
        ...
        checkBreed(_nftContract, matronId, sireId, isEgg, serverSig);
        chargeTokens(_nftContract, 1);
        MSNFT msNFT = MSNFT(_nftContract);
        uint256 tokenId = msNFT.mint(_msgSender(), matronId, sireId, 0, true);
        msNFT.increaseBreedCount(matronId);
        msNFT.increaseBreedCount(sireId);
    ...
    function chargeTokens(address _nftContract, uint256 tokenQuantity)
        internal
    {
        for (uint256 i = 0; i < charges[_nftContract].length; i++) {
            if (charges[_nftContract][i].token == address(0)) {
                require(
                    msg.value == charges[_nftContract][i].price * tokenQuantity,
                    "MSBreed: Send wrong ETH value"
                );
            } else {
                require(
                    ERC20(charges[_nftContract][i].token).allowance(
                        _msgSender(),
                        address(this)
                    ) >= charges[_nftContract][i].price * tokenQuantity,
                    "MSBreed: Approved insufficient ERC20 tokens"
                );
                ERC20(charges[_nftContract][i].token).safeTransferFrom(
                    _msgSender(),
                    address(this),
                    charges[_nftContract][i].price * tokenQuantity
                );
