        ...
        returns (uint256, uint256)
    {
        ...
        for (uint64 i = userIndex; i < index; i++) {
            ...
            if (endTime < rates[i + 1].timeStamp) {
                ...
                if (token == tokenAddressA) {
                    uint256 test = payOut[tokenAddressA].div(
                            payOut[tokenAddressA].add(payOut[tokenAddressB])
                        );
                    interest = interest.mul(
                        test
                    );
                } else {
                    uint256 test = payOut[tokenAddressB].div(
                            payOut[tokenAddressA].add(payOut[tokenAddressB])
                        );
                    interest = interest.mul(
                        test
                ...
                amount = amount.add(interest.div(100));
        ...
        if (token == tokenAddressA) {
            ...
            uint256 test = payOut[tokenAddressB].mul(1000000).div(payOut[tokenAddressA]);
            rewardB = rewardA.mul(conversionAtoB).mul(test).div(10**12);
        } else {
            rewardB = amount.sub(amount1);
            uint256 test = payOut[tokenAddressA].mul(1000000).div(payOut[tokenAddressB]);
            rewardA = rewardB.mul(10**6).mul(test).div(conversionAtoB).div(1000000);
        }
