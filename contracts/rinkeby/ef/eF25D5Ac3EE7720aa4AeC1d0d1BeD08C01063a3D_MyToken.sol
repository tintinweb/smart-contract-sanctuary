...
contract MyToken is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable,  OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable, VRFConsumerBase {
    ...
    //tax rates
    uint public liqTaxRate = 2;
    uint public marketingTaxRate = 1;
    uint public lotteryTaxRate = 2;
    ...
    function drawNumberdome2() public onlyOwner {
        require(!numberdomeActive);
        uint256 numberdomeResult = (uint8(uint256(keccak256(abi.encodePacked(requestIdToRandomNumber[numberdomeLastRequestID]))) % 444)) + 111;
    ...
    function drawGridlock2() public onlyOwner {
        require(!gridlockActive);
        uint256 gridlockResult = (uint8(uint256(keccak256(abi.encodePacked(requestIdToRandomNumber[gridlockLastRequestID]))) % 12));
    ...
    function payLotteryWinner() public onlyOwner {
        ...
        winningLotteryNumber = uint8(uint256(keccak256(abi.encodePacked(requestIdToRandomNumber[lotteryLastRequestID]))) % participatingTokens);
