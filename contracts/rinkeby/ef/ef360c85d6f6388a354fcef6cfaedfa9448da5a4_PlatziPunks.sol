...
contract PlatziPunksDNA {
	...
	// This function is deterministic and will always return the same value for a given seed
	function deterministicPseudoRandomDNA (uint256 _tokenId, address _minter) public pure returns (uint256) {
		uint256 seed = _tokenId + uint160(_minter);
		bytes memory encodedParams = abi.encodePacked(seed);
...
contract PlatziPunks is ERC721, ERC721Enumerable, PlatziPunksDNA {
	...
	function tokenURI(uint256 tokenId)
		...
		override
		returns(string memory) {
		require (_exists(tokenId), "ERC721: unknown token ID");
	...
	// The following functions are used to support the ERC721Enumerable interface
