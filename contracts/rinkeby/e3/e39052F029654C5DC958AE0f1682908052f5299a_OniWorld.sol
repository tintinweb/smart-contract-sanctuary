    ...
    Ownable
{
    ...
    uint256 public NFT_small_price = 0.039 ether; 
    uint256 public NFT_big_price = 0.099 ether; 

    uint256 public startTime = 1633068901; // 04/10/2021 18:30:01 UTC+7 - 1633347001
    string _name = "OniWorld";
    string _symbol = "ONINFT";
    
    bool public is_one_get_one = true;
    ...
    function set_Buy_1_Get_1(bool result) external onlyOwner{
        is_one_get_one = result;
    }
    ...
    function set_small_price(uint256 small_price) external onlyOwner{
        NFT_small_price = small_price;
    }
    
    function set_big_price(uint256 big_price) external onlyOwner{
        NFT_big_price = big_price;
    }
    
    //Public Mint
    function claimFree() external {
        require(is_free_mint == true, "There is no more Free NFT.");
    ...
    function _mintOni(uint8 num) internal {
        ...
        if(is_one_get_one == true){
            num = num * 2;
        }
        if(balanceOf(_msgSender()) == 0 && is_free_mint){
            num = num + 1;
        }
