...
contract NFTI is IERC721, IERC721Metadata, IERC721Enumerable, Ownable {
    ...
    uint256 private _start_sold_time = 1614856200;
    ...
    constructor(address nctAddress) {
        _nctAddress = nctAddress;
    }

    function _getGiftTokenId(address customer) private view returns(uint256) {
        if (customer == 0xE2E367e07561E3635430Ff3142b8244aE69C3002) {
            return _SALE_SUPPLY + _UPGRADE_SUPPLY + 1;
        } else if (customer == 0x4a384E9bf528A7D2026Dc2C5FB0441b1091D804D) {
            return _SALE_SUPPLY + _UPGRADE_SUPPLY + 2;
        } else if (customer == 0x3516D57e5A021db372A228d58f816F524A8cfa34) {
            return _SALE_SUPPLY + _UPGRADE_SUPPLY + 3;
        }
        if (_privateTokens[customer] != 0) {
            return _privateTokens[customer];
        }
        return 0;
    }
    ...
    function setAirdropAddress(address[] memory customers) public onlyOwner {
        ...
        for (uint256 i = 0; i < 10; ++i) {
            _privateTokens[customers[i]] = _SALE_SUPPLY + _UPGRADE_SUPPLY + 10 + i;
        }
    }
    ...
    function upgradeToken(uint256 tokenId) public returns (uint256) {
     ...
     function claim() public returns (uint256) {
        require(_getGiftTokenId(_msgSender()) != 0, "You don't have private tokens");
        require(ownerOf(_getGiftTokenId(_msgSender())) == address(0), "You gift is already minted");

        _safeMint(_msgSender(), _getGiftTokenId(_msgSender()));

        return _getGiftTokenId(_msgSender());
    }
}