...
// File: BlindGotchi.sol



/*
  (b) (l) (i) (n) (d) gotchi
  by guys from BlindBox, inspired from dhof
*/
...
contract Blindpet {
    ...
    uint256 internal lastFeedBlock;
    uint256 internal lastCleanBlock;
    uint256 internal lastPlayBlock;
    uint256 internal lastSleepBlock;
    ...
    address public tracktoken = 0xB5644Fc316860481183DBE991b1d3aa843ABe06D;
    ...
    function getBlindBoxBalance() internal view returns (uint256){
        return IERC20(tracktoken).balanceOf(msg.sender);


    }
    
    function feed() public {
        require(getBlindBoxBalance() > 0, "Only BlindBox allowed to feed");
    ...
    function clean() public {
        require(getBlindBoxBalance() > 0, "Only BlindBox allowed to clean");
        require(getAlive(), "no longer with us");
    ...
    function play() public {
        require(getBlindBoxBalance() > 0, "Only BlindBox allowed to play");
        require(getAlive(), "no longer with us");
    ...
    function sleep() public {
        require(getBlindBoxBalance() > 0, "Only BlindBox allowed to ask it to sleep");
        require(getAlive(), "no longer with us");
