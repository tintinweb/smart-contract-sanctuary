...
// SPDX-License-Identifier: MIT
    ...
    */
    ...
    // File contracts/StudentGirlnft.sol

    pragma solidity ^0.8.4;
    pragma abicoder v2;

    /// @custom:security-contact [email protected]
    contract StudentGirlNFT is ERC721, Ownable, ERC721Enumerable {
        using SafeMath for uint256;
        using Strings for uint256;

        uint256 public constant tokenPrice = 0.013 ether; // 0.013 MOVR
        uint256 public constant maxTokenPurchase = 13;
        uint256 public constant maxSupply = 100;

        string public baseURI = ""; // IPFS URI WILL BE SET AFTER ALL TOKENS SOLD OUT OR INITIAL SALE PERIOD ENDS   
        ...
        event StudentGirlNFTMinted(uint256 tokenId, address owner);

        constructor() ERC721("Student Girl NFT", "SGN") {}
            ...
            onlyOwner
        {
            require(
                ...
                "Not enough developer reserve left"
            );
        ...
        function saleMint(uint256 numberOfTokens) external payable {
            require(saleIsActive, "Sale must be active to mint StudentGirlNFT!");
            require(
                numberOfTokens > 0 && numberOfTokens <= maxTokenPurchase,
                "Invalid Mint Amount (Min >0 Max <=13). Please try again."
            );
            require(
                totalSupply().add(numberOfTokens) <= maxSupply,
                "Purchase would exceeed maximum supply!"
            );
            require(
                msg.value >= tokenPrice.mul(numberOfTokens),
                "MOVR value sent is incorrect."
            );
            ...
            for (uint256 i = 0; i < numberOfTokens; i++) {
                ...
                if (totalSupply() < maxSupply) {
                    ...
                    emit StudentGirlNFTMinted(id, msg.sender);
                }
            }
        }
        ...
        function presaleMint(uint256 numberOfTokens) external payable {
            ...
            require(
                ...
                "Invalid Presale Mint Amount (Min >0 Max <=3). Please try again."
            );
            require(
                msg.value >= tokenPrice.mul(numberOfTokens),
                "MOVR value sent is incorrect."
            ...
            require(
                ...
                "You can only purchase a maximum of three Student Girls during the presale."
            );
        ...
        function addToPresaleList(address[] calldata addresses) external onlyOwner {
            for (uint256 i = 0; i < addresses.length; i++) {
                require(addresses[i] != address(0), "Address can't be NULL");
            ...
            returns (string memory)
        {
            ...
            if (isRevealed == false) {
                return
                    "ipfs://QmViEkh11VWBXCmASRttMzbVeQfReNr823Kanq4HbRziDn/hidden.json";
            }
