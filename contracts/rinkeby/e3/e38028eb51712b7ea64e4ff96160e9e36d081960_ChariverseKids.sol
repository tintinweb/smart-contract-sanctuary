...
contract ChariverseKids is 
	...
	uint256 public MAX_SUPPLY_FOR_GIVEAWAY = 50;
	...
	uint256 public maxItemsPerMint = 10;
    ...
    function setMaxItemPerSingleMint(uint256 maxItemsPerMint_) public onlyOwner{
    ...
    function mintChariverseItem(address to, uint count) public payable notPaused timed {
        ...
        require(count <= maxItemsPerMint, "To much minting");
     ...
     function mintGiveAwayItems(address to, uint count) public onlyOwner{
        require(totalSupply() < MAX_SUPPLY_FOR_GIVEAWAY, "Max GiveAway Limit");
	    for(uint i = 0; i < count; i++){
            _safeMint(to, totalSupply());
        }
	}
    ...
    function _minutesToPrice(uint256 minute) private view returns (uint256){
        return priceRanges[Math.min(minute.div(minutesToDropSalePrice), priceRanges.length - 1)];
    }
    
    function price(uint count) public view returns (uint256) {
        if(isPreSale()) return presalePrice.mul(count);
