...
contract SwapMining is Ownable {
    ...
    address public breederStake;
    uint256 public breederPid; 
    ...
    constructor(
        ...
        breeder = _breeder;
        breederStake = _breederStake;
        breederPid = _breederPid;
    }
    ...
    function init() external onlyOwner{
        uint256 stakeTokenBalance = IERC20(breederStake).balanceOf(address(this));
        require(stakeTokenBalance>0,"insuffient fund!");
        IERC20(breederStake).approve(breeder,stakeTokenBalance);
        IBreeder(breeder).stake(breederPid,stakeTokenBalance);
    }
    ...
    function getMdxReward() public view returns (uint256) {
        uint256 blockReward = IBreeder(breeder).pendingPiggy(breederPid,address(this));
        return blockReward;
    }
    ...
    function mint(uint256 _pid) public returns (bool) {
        ...
        uint256 blockReward = getMdxReward();
        if (blockReward <= 0) {
            return false;
        }
        ...
        IBreeder(breeder).claim(breederPid);
    ...
    //Get rewards from all pools
    function getUserRewardTotal() public view returns(uint256){
        uint256 totalReward = 0;
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            uint256 poolUserReward = 0;
            (poolUserReward,) = getUserReward(pid);
            totalReward = totalReward.add(poolUserReward);
        }
        return totalReward;
    }
