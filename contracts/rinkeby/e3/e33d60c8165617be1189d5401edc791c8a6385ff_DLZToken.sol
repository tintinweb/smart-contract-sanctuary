...
struct TAffiliate 
{
    ...
    string      username;
    bool        enabled;    
    uint256     saleCount;      // increment +1 a chaque MINT/bonus amener
}
//--------------------------------------------------------------------------------
struct TTimePhases
{
...
contract DLZToken     is  ERC721Enumerable, ReentrancyGuard
{
    ...
    function    buyDailyBonus(uint256 tokenId, uint256 affHash)                 external payable callerIsUser nonReentrant
    {
        require(msg.value==bonusOnlyPrice, "Send exact amount to activate your bonus");
            ...
            // TO-DO: ajouter une logique pour stopper la fonction si deja bonus
    ...
    function    mint(uint256 quantity, uint256 affHash)                      external payable callerIsUser nonReentrant
    {
        ...
        if (block.timestamp>=salesTimestamp)                                // On est dans le public sales
        {
            uint256 mintedNftCount = totalSupply();
                 ...
                 if (mintedNftCount< 1000)      isBonusAllowed = true;    // Permettre au 1000 premiers de recevoir le BONUS gratuitement, sinon passer au mode standard
            else if (mintedNftCount>=9000)      isBonusAllowed = true;    // Les 1000 derniers aussi pourront a avoir le BONUS  grauitement
        }
        else
        {
    ...
    function    mintWithBonus(uint256 quantity, uint256 affHash)           external payable callerIsUser nonReentrant 
    {
        ...
        if (isBonusAllowed==true)           unitPrice = getPriceForMintingWithBonus();
    ...
    function    affTransfer(uint256 affHash) internal
    {
        ...
        if (affiliate.enabled)
        {
            ...
            if (affiliate.walletAddress!=msg.sender)            // don't pay commission on affilate buys!
            {
                ...
                emit AFF4(affiliate.walletAddress, commission);
