...
contract StablePool {
    ...
    function chain( 
        ...
        uint _amount,
        address _recipient
    ) external { // there is always one more token than pools to route through
        require( _pools.length == _tokens.length.sub( 1 ), "Arguments misconfigured" );
        
        for( uint i = 0; i < _pools.length; i++ ) {
            if( _tokens[ i ] == _firstToken ) {
                uint j = i++;

                uint amount = _swap( _tokens[ i ], _amount, _tokens[ j ]);

                IERC20( _tokens[ j ] ).safeTransfer( _recipient, amount );

                /*if( j < _pools.length ) {
                    IERC20( _tokens[ j ] ).approve( _pools[ j ], amount );
                    IStablePool( _pools[ j ] ).chain( _tokens, _pools, _tokens[ j ], amount, _recipient );
                } else {
                    IERC20( _tokens[ j ] ).safeTransfer( _recipient, amount );
                }*/
            }
    ...
    function removeAll( uint _amount ) external {
        ...
        for ( uint i = 0; i < poolTokens.length; i++ ) {
            ...
            uint send = _amount.mul( token.balanceOf( address(this) ) ).div( totalTokens );
            token.safeTransfer( msg.sender, send );
        }
        totalTokens = totalTokens.sub( _amount ); // remove amount from pool less fees
    }
