...
contract BlueCircles is ERC721URIStorage {
    uint8 private tokenId;
    uint8 totalSupply = 16;
    address payable public owner;
    
    event NFTMinted(uint256 indexed tokenId, string tokenURI);

    constructor() 
        ERC721("Blue Circles Club", "BCC")
    {
    ...
    modifier supplyCheck {
        require(tokenId <= 16, "The collection has run out of items");
        _;
    }
    ...
    function getTokenURI(uint8 _id) private pure returns(string memory) {
        return string(abi.encodePacked(
            "https://ipfs.io/ipfs/QmP8RkVbqxLuk6yc8QXVBtpYPJ3mByNrFPJQ84YErPDP7j/", 
