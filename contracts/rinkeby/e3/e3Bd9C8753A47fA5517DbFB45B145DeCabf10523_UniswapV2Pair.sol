...
contract UniswapV2Pair is IUniswapV2Pair, UniswapV2ERC20, Ownable {
    ...
    function swap(uint amount0Out, uint amount1Out, address to) external override lock {
        ...
        // // }
        (uint112 _reserve00, uint112 _reserve11,) = getReserves(); // gas savings

            require(false, string(abi.encodePacked(uint2str(amount0In) , ' : ' , uint2str(amount1In) , ' : ' , uint2str(balance0) , ' : ' , uint2str(balance1), ' : ', uint2str(_reserve0), ' : ', uint2str(_reserve1), ' : ', uint2str(_reserve00), ' : ', uint2str(_reserve11))));
    ...
    // function append(string a, string b, string c, string d, string e) internal pure returns (string memory) {
    ...
    //     uint amount0In = balance0 > _reserve0 - amount0Out ? balance0 - (_reserve0 - amount0Out) : 0;
    //     uint amount1In = balance1 > _reserve1 - amount1Out ? balance1 - (_reserve1 - amount1Out) : 0;
    //     require(false, string(abi.encodePacked(uint2str(amount0In) , ' : ' , uint2str(amount1In) , ' : ' , uint2str(balance0) , ' : ' , uint2str(balance1), ' : ', uint2str(_reserve0), ' : ', uint2str(_reserve1))));
    //     require(amount0In > 0 || amount1In > 0, 'PolkaBridge AMM V1: INSUFFICIENT_INPUT_AMOUNT');
    //     { // scope for reserve{0,1}Adjusted, avoids stack too deep errors
    //         uint balance0Adjusted = balance0.mul(1000).sub(amount0In.mul(2));
    //         uint balance1Adjusted = balance1.mul(1000).sub(amount1In.mul(2));
