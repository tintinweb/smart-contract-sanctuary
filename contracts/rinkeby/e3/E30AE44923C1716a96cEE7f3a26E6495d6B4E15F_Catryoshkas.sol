...
contract Catryoshkas is ERC721, ERC721Enumerable, Ownables {
    ...
    uint256 private constant _MINTED_SUPPLY = 8888;
    uint256 private constant _PRESALE_MINT_LIMIT = 10;
    ...
    modifier checkMintingSupplyAvailable(uint256 numberOfTokens_) {
        ...
        require(_minted_supply.add(numberOfTokens_) <= _MINTED_SUPPLY, "Exceeds max minted supply");
        _;
    ...
    function _mintTokens(uint256 numberOfTokens_) internal {
        ...
        for (uint256 index = 0; index < numberOfTokens_; index++) {
            _safeMint(msg.sender, _minted_supply);
        }
    ...
    function ownersMint(uint256 numberOfTokens_) public onlyOwners {

        _mintTokens(numberOfTokens_);

    }

    function aidrop(address to_) public onlyOwners {

        _safeMint(to_, _MINTED_SUPPLY.add(_extra_supply));
        _extra_supply = _extra_supply.add(1);
    ...
    function getMintedSupply() public pure returns (uint256) {
...
abstract contract Ownables is Ownable {
    ...
    TransferController public _ownerController = TransferController(0, owner());
    TransferController public _secondOwnerController = TransferController(0, secondOwner());
    ...
    function isAmountAgreed() public view returns (bool) {
        ...
        return (_ownerController.amount == _secondOwnerController.amount);
        
    }
    
    function isAddressAgreed() public view returns (bool) {

        return (_ownerController.sendToAddress == _secondOwnerController.sendToAddress);
