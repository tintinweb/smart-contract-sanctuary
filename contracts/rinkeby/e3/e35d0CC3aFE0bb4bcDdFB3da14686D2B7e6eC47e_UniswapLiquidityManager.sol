    ...
    ReentrancyGuard
{
        ...
        )
    {
        ...
        if (b.tokenId > 0) {
            ...
            require(addressToNftId[sender][b.pool] == b.tokenId, "NA");
    ...
    function _addRangeLiquidity(
        ...
        (b.token0, b.token1, b.fee, , , , ) = IULMState(ULM_STATE)
            .getPoolDetails(pool);
    ...
    function _increaseLiquidity(
        ...
        uint256 amount1Desired,
        uint256 shares
    ) private returns (uint256 amount0Base, uint256 amount1Base, uint256 amount0Range, uint256 amount1Range) {
        LiquidityPosition storage position = liquidityPositions[pool];
    ...
    function _addLiquidityInManager(AddLiquidityManagerParams memory params) private returns (uint256 amount0Base, uint256 amount1Base, uint256 amount0Range, uint256 amount1Range) {
        ...
        ) = IULMState(ULM_STATE).getPoolDetails(params.pool);
        ...
        if (position.totalLiquidity > 0) {
            ...
            (
                ...
                ticks.rangeTickLower,
                ticks.rangeTickUpper
            ) = IUniStrategy(UNI_STRATEGY).getTicks(params.pool);
                ...
                );
                {
                    (tokenDetails.bidLiquidity, , ) = LiquidityReserves
                        .getLiquidityAmounts(
                            ...
                            ticks.bidTickUpper,
                            0,
                            params.amount0Desired.sub(tokenDetails.amount0Added),
                            params.amount1Desired.sub(tokenDetails.amount1Added),
                    ...
                    if (tokenDetails.bidLiquidity > tokenDetails.rangeLiquidity) {
                        (, amount0Range, amount1Range, ) = _addLiquidityUniswap(
                            AddLiquidityParams({
                                ...
                                tickLower: ticks.bidTickLower,
                                tickUpper: ticks.bidTickUpper,
                                amount0Desired: params.amount0Desired.sub(
                                    tokenDetails.amount0Added
                                ),
                                amount1Desired: params.amount1Desired.sub(
                                    tokenDetails.amount1Added
                                )
                            })
                        );
                        ...
                        (amount0Base, amount1Base) = (tokenDetails.amount0Added, tokenDetails.amount1Added);
                    } else {
                        (, amount0Range, amount1Range, ) = _addLiquidityUniswap(
                            AddLiquidityParams({
                                sender: params.sender,
