...
contract AuctionV3 is ManagerAuction {
	...
	function migrateAuction(
		...
		if (_toId > totalAuctions) _toId = totalAuctions;
		...
		for (uint256 i = _fromId; i < _toId; i++) {
			...
			if (isERC721 && IAuctionVersion(_oldContract).versionOnAuction(tokenAddress, tokenId, 1)) {
				...
				newAuction.listBidId = listBidId;
