...
// Root file: contracts/NFT.sol

// contracts/GameItem.sol

pragma solidity >=0.6.0 <0.8.0;

// import '/Users/stone/Desktop/CHFRY/node_modules/@openzeppelin/contracts/token/ERC721/ERC721.sol';
// import '/Users/stone/Desktop/CHFRY/node_modules/@openzeppelin/contracts/utils/Counters.sol';
...
contract GameItem is ERC721 {
	...
	uint256 private constant INITIAL = 30;

	constructor() ERC721('DEFI GTD', 'DAO') {
		for (uint256 index = 0; index < INITIAL; index++) {
			_awardItem(msg.sender);
		}
	}

	function _awardItem(address player) internal returns (uint256) {
		_tokenIds.increment();
		uint256 newItemId = _tokenIds.current();
		_mint(player, newItemId);
		_setTokenURI(newItemId, 'https://60700aa685c3f0001746f4ca.mockapi.io/api/v1/1');
		return newItemId;
	}

	function setAvaiable(bool _bool) public onlyAdmin {
		avaiable = _bool;
	}
	...
	modifier onlyAvaiable {
		require(avaiable == true, '!avaiable');
		_;
	}

	function mint() public onlyAvaiable returns (uint256) {
		require(balanceOf(msg.sender) == 0, '!account balance');
		return _awardItem(msg.sender);
	}
}