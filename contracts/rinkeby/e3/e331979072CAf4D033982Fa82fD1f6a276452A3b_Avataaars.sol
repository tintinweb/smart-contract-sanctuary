...
contract Avataaars is ERC721Enumerable, Ownable {
    ...
    mapping(address => bool) private hasMint;
    mapping(uint256 => address) private tokenToHash;
    mapping(address => uint256) private hashToToken;
    ...
    function createAvataaars() public payable {
        ...
        tokenToHash[supply] = msg.sender;
        hashToToken[msg.sender] = supply;
        
        emit mint(msg.sender,tokenId);

    }
    
    function tokenIdToHash(uint256 _tokenId) onlyValidTokenId(_tokenId) public view returns(address){
        return tokenToHash[_tokenId];
    }
    function hashToTokenId(address _add) public view returns(uint256){
        return hashToToken[_add];
    }
