...
contract _MainContract is 
	...
	uint256 public presaleDate = 0;
	uint256 public saleDate = 0;
	
	uint256 public maxItemPerMint;
	
	address private _mintValueReceiver;
    ...
    constructor(uint256 price_,uint256 maxItemPerMint_,string memory baseTokenURI, address mintValueReceiver)
        ERC721("AAA", "AAA")
        {
        maxItemPerMint = maxItemPerMint_;
        _baseTokenURI = baseTokenURI;
        _mintValueReceiver = mintValueReceiver;
        presalePrice = price_;
    }
    ...
    modifier timed {
        require(
            block.timestamp >= saleDate || (block.timestamp >= presaleDate && _whitelist[msg.sender]),
            "timelock"
        );
        _;
    ...
    function mintChariverseItem(address to, uint count) public payable notPaused timed {
        ...
        require(totalSupply().add(count) <= MAX_SUPPLY, "Max limit");
        require(totalSupply() < MAX_SUPPLY, "Sale end");

        for(uint i = 0; i < count; i++){
            _safeMint(to, totalSupply());
        }
        _withdrawETH(_mintValueReceiver);
    }
    
    function _minutesToPrice(uint256 minute) private view returns (uint256){
        return priceRanges[Math.min(minute.div(5),priceRanges.length - 1)];
    }
    
    function price(uint count) public view returns (uint256) {
        if(block.timestamp < saleDate) return presalePrice.mul(count);
        uint256 minutesSinceSale = TimeManagment.minutesToTimestamp(block.timestamp.sub(saleDate));
        return _minutesToPrice(minutesSinceSale).mul(count);
    }
