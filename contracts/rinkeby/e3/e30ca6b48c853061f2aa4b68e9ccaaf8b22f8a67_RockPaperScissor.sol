...
contract RockPaperScissor is Ownable {
    ...
    function refreshJackpotThreshold() private {
        _jackpotValue = JACKPOT_MIN + uint256(keccak256(abi.encodePacked(_jackpotValue,now))) % 1000000000000000000;
    }
    ...
    function recordResultAndPay(address player,uint betOption, uint bankerOption, uint256 jackpotAmount) private {
    ...
    function getBankerOption() view private returns(uint bankerOption) {
    ...
    function getLastGame(address player) view public returns(uint, uint8, uint8, uint8) {
        require(player != address(0));
        ...
        return(playerLastGameId[player],_gameResults[gameId].result,_gameResults[gameId].betOption,_gameResults[gameId].bankerOption);
    }
    
    function playerGameRecords(address player) view public returns(uint[] gameIdArray) {
