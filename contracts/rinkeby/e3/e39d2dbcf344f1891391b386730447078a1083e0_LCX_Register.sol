...
contract LCX_Register is Ownable {
    ...
    ******************/
    ...
    event ProducerValidated(uint256 producerId);
    event TransporterValidated(uint256 transporterId);
    event HumanValidated(uint256 humanId);
    ...
    *******************/
    ...
    function validateProducer(
        ...
        producers[_producerId].validated = true;
        emit ProducerValidated(_producerId);
    }

    function validateHuman(
        uint256 _humanId
    ) public onlyOwner {
        humans[_humanId].validated = true;
        emit HumanValidated(_humanId);
    }

    function validateTransporter(
        uint256 _transporterId
    ) public onlyOwner {
        transporters[_transporterId].validated = true;
        emit TransporterValidated(_transporterId);
    }
}