...
contract AbstractWrapper is ERC1155 {
    ...
    */
     ...
     */
    function wrap(uint256 _id, uint256[200] calldata _printIndexes) external {
        ...
        for (uint256 i=0; i < _quantity; ++i) {
            ...
            bool success = _DigitalArtCollectibleContract.transfer(address(this), _id, _printIndex);
     ...
     */
    function wrapBatch(uint256[] calldata _ids, uint256[] calldata _printIndexes) external {
        uint256 j=0;
        for (uint256 i=0; i < _ids.length; ++i) {
            uint256 _id = _ids[i];
            uint256[200] memory _currentDrawingIndexes;
            uint printCounter = 0;
            while(j < _printIndexes.length) {
                uint256 _printIndex = _printIndexes[j];
                j = j + 1;
                if (_printIndex == 0) {
                    break;
                } 
                _currentDrawingIndexes[printCounter] = _printIndex;
                printCounter++;
            }

            require(_currentDrawingIndexes.length > 0, "invalid print indexes for drawing");
            
            this.wrap(_id, _currentDrawingIndexes);
        }
    }
    ...
    /**
        ...
        tokens to caller.
     */
    function unwrap(uint256 _id, uint256[200] calldata _printIndexes) external {
        uint256 _quantity = _printIndexes.length;
     ...
     */
    function unwrapBatch(uint256[] calldata _ids, uint256[] calldata _printIndexes) external {
        uint256 j=0;
        for (uint256 i=0; i < _ids.length; ++i) {
            uint256 _id = _ids[i];
            ...
            this.unwrap(_id, _currentDrawingIndexes);
        }
    }
}
