...
contract HashStaking is Ownable {
    ...
    uint constant public STAKING_TOKENS_LIMIT = 25000 * 10**18;
    uint constant public PARTNER_TOKENS_LIMIT = 2500 * 10**18;
    uint constant public BLOCKS_TOTAL = 40320;
    ...
    function unstakeAll() external {
        _unstake(msg.sender, users[msg.sender].stakedAmount);
    }
    ...
    function getPendingReward(address _address) external view returns (uint) {
    ...
    function _unstake(address _address, uint _amount) internal {
        require(_amount > 0, "Incorrect amount");
        if (block.number >= endBlock && finalStakedAmount == 0) {
            finalStakedAmount = currentStakedAmount;
        }
        (uint reward, uint partnerReward) = _collect(_address);
        ...
        emit Unstake(_address, user.partner, user.referral, _amount, user.stakedAmount, reward, partnerReward);
    }
