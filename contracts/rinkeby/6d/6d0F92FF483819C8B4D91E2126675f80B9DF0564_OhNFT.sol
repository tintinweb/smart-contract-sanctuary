...
import "./OhTradable.sol";
import "./OhRevealable.sol";

contract OhNFT is Ownable, ERC721, ERC721Enumerable, OhTradable, OhRevealable, ReentrancyGuard {
  ...
  constructor(
    ...
    OhTradable(150, 2222) 
    OhRevealable(vrfCoordinator, linkToken, keyHash) {
    _maxAirdrop = maxAirdrop;
    _maxPrivateSale = maxPrivateSale;
  ...
  function mint(uint256 amount) public
    ...
    mintable(amount)
    returns (bool){
    return OhMint(_msgSender(), amount, false);
  }

  function OhMint(address collector,uint256 amount, bool isAirdrop) internal returns (bool) {
    // private sale
    if(getSaleMode() == 1) {
      require(isWhitelisted(collector), "Only whitelist addresses allowed.");
      require(isValidPrivateSaleAmount(collector, amount), "Max presale amount exceeded.");
      require(isOversoldPrivateSale(amount), "Cannot oversell");
    }

    for (uint256 i = 0; i < amount; i+=1) {
      uint256 tokenIndex = totalSupply();
      
      if (tokenIndex < _maxSupply) {
  ...
  function airdrop(address[] memory _to, uint256 amount) public offline onlyOwner {
    require(_totalAirdrop + (_to.length * amount) <= _maxAirdrop, "Exceed airdop allowance limit.");
    for (uint i = 0; i < _to.length; i+=1) {
      OhMint(_to[i], amount, true); // mint for marketing & influencer
    }
  }
  ...
  function metadataOf(uint256 tokenId) public view returns (string memory) {
    ...
    uint256[] memory metaIds = new uint256[](_maxSupply+1);
    uint256 ss = getSeed();

    for (uint256 i = 1; i <= _maxSupply; i+=1) {
      metaIds[i] = i;
    }

    // shuffle meta id
    for (uint256 i = 1; i <= _maxSupply; i+=1) {
      uint256 j = (uint256(keccak256(abi.encode(ss, i))) % (_maxSupply));
    ...
    returns (string memory)
  {
    ...
    return isRevealed() ? string(abi.encodePacked(_baseURI(), metadataOf(tokenId), ".json")) :_defaultURI;    
  ...
  function originalOwns(address collector) public view returns(uint){
...
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/utils/math/SafeMath.sol";
abstract contract OhTradable is Ownable {
  using SafeMath for uint256;

  event OhWhitelist(address adress);

  uint private _mode = 0;
  uint private _limitPrivateSaleTx = 2;
  uint private _limitPublicSaleTx = 20;
  uint public _maxAirdrop;
  uint public _maxPrivateSale;
  uint public _totalAirdrop;
  uint public _totalPrivateSale;  
  uint public _totalPublicSale;  
  ...
  constructor(uint maxAirdrop, uint maxPrivateSale){
    _maxAirdrop = maxAirdrop;
    _maxPrivateSale = maxPrivateSale;
  }
  ...
  /**
    ...
    */
  modifier online() {
    require(_mode > 0, "Contract must be online.");
    _;
  }
  ...
  /**
    ...
    */
  modifier offline() {
    require(_mode == 0, "Contract must be offline.");
    _;
  }
  ...
  function getPrice() public view returns(uint256) {
    if (_mode == 1) return _privateSalePrice;
    if (_mode == 2) return _publicSalePrice;
    return 0;
  }
  ...
  function whitelist(address[] memory allowlist) public onlyOwner {
    for(uint i = 0; i < allowlist.length; i+=1) {
      _presaleAllowed[allowlist[i]] = true;
      emit OhWhitelist(allowlist[i]);
    }
  }

  function isWhitelisted(address collector) public view returns(bool) {
    return _presaleAllowed[collector];
  }

  function isValidPrivateSaleAmount(address collector,uint amount) public view returns(bool) {
    return _presaleMinted[collector] + amount <= _limitPrivateSaleTx;
  }

  function isOversoldPrivateSale(uint amount) public view returns(bool) {
    return _totalPrivateSale + amount <= _maxPrivateSale;
  }

  function logTrade(address collector,uint amount, bool isAirdrop) internal{
    if (isAirdrop) {
      _totalAirdrop += amount;
      return;
    }

    if (_mode == 1) {
      _totalPrivateSale += amount;
      _presaleMinted[collector] += amount;
    }
    
    if (_mode == 2) _totalPublicSale += amount;
  }
...
abstract contract OhRevealable is Ownable, VRFConsumerBase {
  using SafeMath for uint256;

  event OhNFTRandomnessRequest(uint timestamp);
  event OhNFTRandomnessFulfil(uint timestamp, bytes32 requestId, uint256 seed);
  event OhNFTChainlinkError(uint timestamp, bytes32 requestId);
  event OhNFTReveal(uint timestamp);
  event OhManualSetSeed(uint timestamp);
  ...
  function requestChainlinkVRF() public onlyOwner {
    ...
    _requestedVRF = true;
    emit OhNFTRandomnessRequest(block.timestamp);
  }
  ...
  function fulfillRandomness(bytes32 _requestId, uint256 randomNumber)
    ...
    if (randomNumber > 0) {
      _seed = randomNumber;
      emit OhNFTRandomnessFulfil(block.timestamp, _requestId, _seed);
    }
    else {
      _seed = 1;
      emit OhNFTChainlinkError(block.timestamp, _requestId);
    } 
  }
  ...
  function setSeed(uint randomNumber) public onlyOwner {
    _requestedVRF = true;
    _seed = randomNumber;
    emit OhManualSetSeed(block.timestamp);
  }
}
