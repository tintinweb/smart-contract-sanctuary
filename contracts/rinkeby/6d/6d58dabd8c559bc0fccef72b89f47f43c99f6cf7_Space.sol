...
contract Space is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721('Golff BBB', "Golff BBB") {

    }

    function claim() public returns (uint256){
        require(totalSupply() + 1 <= MAX_SPACE_SUPPLY, "claim would exceed max supply");
        //require(isMinter(msg.sender), 'claim only allowed from Space');
    ...
    function claim_size(uint256 size) public{
        require(totalSupply() + size <= MAX_SPACE_SUPPLY, "claim would exceed max supply");
        //require(isMinter(msg.sender), 'claim only allowed from Space');
        //require(!isClaimed(msg.sender), 'already claimed');
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        bytes memory tokenName = abi.encodePacked('Golff BBB #', Strings.toString(_tokenId));
    ...
    function getBoardFactor(uint256 tokenId) public view returns (uint256){
        require(_tokenIds.current() >= tokenId, 'query for nonexistent token');
