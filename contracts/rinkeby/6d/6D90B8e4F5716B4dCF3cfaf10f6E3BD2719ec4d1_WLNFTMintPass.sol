...
contract WLNFTMintPass is ERC721Tradable {
    ...
    SOSToken sos;
    enum WhitelistType { Address, Contract, SOS}
    ...
    function getPrice() public view returns (uint256) {
        ...
        if (currentSupply >= (baseSupply.mul(2) + 1)) {
            return basePrice.mul(2**2);
        } else if (currentSupply >= (baseSupply + 1)) {
    ...
    function withdraw() external onlyOwner {
        ...
        uint256 split = balance.mul(30).div(100);
        payable(WALLET1).transfer(split);
        payable(WALLET2).transfer(split);
        payable(WALLET3).transfer(split);
        payable(WALLET4).transfer(
            balance.sub(split.mul(3))
        );
    }
}
