...
contract ParadisePass is ERC1155Tradable {
    ...
    function reserve(address _to, uint256 numberOfTokens, uint256 _id) public onlyOwner {
        _mint(_to, _id, numberOfTokens, "");
        tokenSupply[_id] = tokenSupply[_id].add(numberOfTokens);
    }

    function mint(uint256 numberOfTokens, uint256 _id) public payable {
        require(saleIsActive, "Sale is not active.");
        require(
            totalSupply(_id).add(numberOfTokens) <= maxSupply(_id),
            "Mint has already ended."
        );
        require(numberOfTokens > 0, "numberOfTokens cannot be 0.");
        require(
                pubSalePrice.mul(numberOfTokens) <= msg.value,
                "ETH sent is incorrect."
            );
        require(
                balanceOf(msg.sender, _id).add(numberOfTokens) <= maxPerWallet,
                "Exceeds limit."
            );
        require(
                numberOfTokens <= maxPerWallet,
                "Exceeds per transaction limit."
            );

        for (uint256 i = 0; i < numberOfTokens; i++) {
            _mint(msg.sender, _id, numberOfTokens, "");
            tokenSupply[_id] = tokenSupply[_id].add(numberOfTokens);
        }
    }
