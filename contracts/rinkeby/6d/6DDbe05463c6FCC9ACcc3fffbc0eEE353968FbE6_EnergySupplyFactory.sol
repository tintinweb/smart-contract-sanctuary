...
contract EnergySupply is Ownable{
  ...
  function add_energy_supply(string memory _energy_category,
    ...
    energy_supply_item memory item;
    item.category = _energy_category;
    item.month = _month;
    item.area_name = _area_name;
    item.value = _value;
    item.value_unit = _value_unit;
    item.prod = _prod;
    energy_supply_info[ehash].push(item);
  ...
  function get_energy_supply_info(string memory _energy_category,
                                  string memory _month,
                                  uint256 index) public view returns(string memory energy_category,
                                                                     string memory month,
                                                                     ...
                                                                     uint256 prod){
    bytes32 ehash = keccak256(abi.encodePacked(_energy_category, _month));
    require(index < energy_supply_info[ehash].length, "array out of range");
    energy_supply_item storage item = energy_supply_info[ehash][index];
  ...
  function energy_supply_exists(string memory _energy_category,
    ...
    bytes32 ehash = keccak256(abi.encodePacked(_energy_category, _month));
    return energy_supply_info[ehash].length != 0;
  }

  function energy_supply_info_size(string memory _energy_category,
                                   string memory _month) public view returns(uint256) {
    bytes32 ehash = keccak256(abi.encodePacked(_energy_category, _month));
    return energy_supply_info[ehash].length;
  }
}
