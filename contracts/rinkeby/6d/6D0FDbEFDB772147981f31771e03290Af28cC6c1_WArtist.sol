	...
	Whitelist
{
	...
	// Mapping from rarity to token URI availables to mint
	mapping(Rarity => string[]) public metadataAvailable;
	...
	function publicMint() external payable whenNotPaused nonReentrant {
		...
		require(idCounter.current() + 1 <= supplyAvailable, "Supply available reached");
	...
	function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override {
		...
		// Test what you happen's if some other call update the metadataAvailable while this call read from storage?
		uint256 index = ((randomNumber % 1000) * metadataAvailable[rarity].length) / 1000;
		string memory uri = metadataAvailable[rarity][index];
		delete metadataAvailable[rarity][index];

		Artist memory artist = Artist(
			rarity,
			PaintType(((randomNumber % 10000) * 5) / 10000 + 1),
			uint8(((randomNumber % 100000) * 10) / 100000 + 1),
			2,
			uri
		);
	...
	function tokenURI(uint256 _tokenId) public view override returns (string memory) {
		...
		return string(abi.encodePacked(_baseURI(), artists[_tokenId].tokenURI));
	}
