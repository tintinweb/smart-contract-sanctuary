...
contract MegaSena is VRFConsumerBase {
     ...
     */
    function expand(uint256 randomValue, uint8 n) public pure returns (uint8[] memory expandedValues){
        ...
        while(inserted < n){
            uint8 value = uint8(uint256(keccak256(abi.encode(randomValue, j))) % 61 + 1);
    ...
    function drawNumbers(uint256 randomValue, uint8 n) public{
        winningNumbers = expand(randomValue, n);
        distributePrize();
        draw++;
    }
    ...
    * To test wins. 
    ...
    function placeBet(uint8[] memory numbers) public payable{
        require(ascending(numbers), "Numbers must be unique and in ascending order.");
