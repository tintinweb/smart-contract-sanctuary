...
contract TestCollection is ERC721Enumerable, Ownable, PaymentSplitter {
    ...
    uint256 private constant ANGELS_MAINSALES_LIMIT = 8;
    uint256 private constant ANGELS_PRESALES_LIMIT = 4;
    ...
    ) ERC721("TEST", "TST") PaymentSplitter(team, team_shares) {
        _team = team;
    ...
    function deactivateMainSale() public onlyOwner {
        isSaleLive = false;
    }
    ...
    // function setSaleTimes(uint256[] memory _newTimes) public onlyOwner {
    //     require(_newTimes.length == 3, "You must update all times.");
    ...
    function presaleMint(uint256 _mintAmount) public payable {
        require(isPreSaleLive, "The contract is paused");
        require(!isSaleLive, "Main sale is active");
        require(_mintAmount > 0, "Need to mint at least 1 NFT");
        require(_mintAmount <= ANGELS_PRESALES_LIMIT, "Mint limit exceeded.");
        require(
            totalSupply() + _mintAmount <= ANGELS_PRESALE,
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        if (msg.sender != owner()) {
            ...
            require(isSaleLive, "The contract is paused");
            require(!isPreSaleLive, "The contract is paused");
        }
        require(
            totalSupply() + _mintAmount <= ANGELS_TOTAL - totalSupply(),
