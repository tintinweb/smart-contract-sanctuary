...
contract Monkes is ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  modifier monkesOwner(uint256 _monkeId) {
    require(ownerOf(_monkeId) == msg.sender, "Pig does not belong to sender");
    _;
  }
  ...
  function mintPreSale(bytes32[] memory _proof) external payable nonReentrant {
    ...
    require(supply + 1 <= 3333 - _reserved, "Exceeds maximum supply");
  ...
  function breed(uint256 _parent1, uint256 _parent2)
    external
    monkesOwner(_parent1)
    monkesOwner(_parent2)
  {
...
contract BabyMonkes is ERC721Enumerable, Ownable {
  ...
  constructor(string memory baseURI) ERC721("BabyMonkes", "BABYMONKES") {
    setBaseURI(baseURI);
  }
