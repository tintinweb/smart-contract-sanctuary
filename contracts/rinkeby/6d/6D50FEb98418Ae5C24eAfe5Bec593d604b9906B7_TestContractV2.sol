...
contract TestContractV2 is ERC721, Ownable {
    ...
    uint256 public publicTokenPrice = 0.00001 ether;
    ...
    function mint(uint numberOfTokens) external payable {
        ...
        require(_tokenSupply.current() + numberOfTokens <= 30, "Purchase would exceed max supply of tokens");
        
        if (_tokenSupply.current() + numberOfTokens >= 20) {
            require((publicTokenPrice * numberOfTokens) == msg.value, "Don't send over or under.");
        } else {
			require(addressFreeMinted[msg.sender] < maxFreeMintPerWallet, "You can only mint 10 per wallet.");
		}
