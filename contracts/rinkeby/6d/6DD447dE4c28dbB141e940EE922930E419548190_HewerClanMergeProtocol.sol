...
contract HewerClanMergeProtocol is Ownable, ERC721Enumerable {
    ...
    mapping(string => mapping(uint256 => MergedEntry)) nftTokenIdToMergeState;
    ...
    function mintMergeRequest(uint256 tagTokenId, uint256 tokenId, string memory collectionName) public {
        require(collectionNameToCollectionEntry[collectionName].registered, "Collection is not registered for the Merge Protocol");
        // although there's no need to check the merge state of the given tokenId
        // as the tagTokenId can only be used once in a merge
        // just making it clear here
        require(!tagIsMerged(tagTokenId), "Tag with the given tokenId has already been used in a merge");
        require(!nftIsMerged(collectionName, tokenId), "Nft with the given tokenId has already been used in a merge");

        require(hewerClan.ownerOf(tagTokenId) == msg.sender, "Must be the owner of the tag");
        require(collectionNameToCollectionEntry[collectionName].collection.ownerOf(tokenId) == msg.sender, "Must be the owner of the nft that is merged with the tag");
        ...
        nftTokenIdToMergeState[collectionName][tokenId] = entry;
    ...
    function nftIsMerged(string memory collectionName, uint256 tokenId) public view returns (bool) {
        return nftTokenIdToMergeState[collectionName][tokenId].merged;
    }

    function addCollection(string memory name, address _addr) public onlyOwner {
        require(!collectionNameToCollectionEntry[name].registered, "Adding already existing collection");
