...
contract Gswap_stake is Authorizable{
    ...
    uint public ipoCost = 3000*1000000000;               // 3000000000000
    uint public fileCost = 100*1000000000;             //upload file cost 100000000000
    ...
    function ipo(string memory _symbol, string memory _name, string memory _icon,uint _goal) public {
        require(_goal >= 2000000000000000, "!goal");
        uint allowed = usdg.allowance(msg.sender,address(this));
        uint balanced = usdg.balanceOf(msg.sender);
        require(allowed >= ipoCost, "!allowed");
        require(balanced >= ipoCost, "!balanced");
        usdg.transferFrom( msg.sender,address(this), ipoCost);
    ...
    function addFiles(uint _tokenId, string[] memory _files,string[] memory _fileNames)public{
        ...
        uint allowed = usdg.allowance(msg.sender,address(this));
        uint balanced = usdg.balanceOf(msg.sender);
        require(allowed >= fileCost, "!allowed");
        require(balanced >= fileCost, "!balanced");
        usdg.transferFrom( msg.sender,address(this), fileCost);
    ...
    function setParams(uint _usdg2OwnerKRate,uint _token2OwnerKRate,uint _token2OtherKRate, uint _ipoCost,uint _fileCost, uint _windowSeconds ) onlyOwner public {
