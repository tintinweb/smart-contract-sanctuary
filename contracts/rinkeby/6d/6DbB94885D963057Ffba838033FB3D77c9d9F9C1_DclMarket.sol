...
interface IMarket {
    ...
    event OnLent(address lender,address nftAddress,uint256 nftId,uint64 maxEndTime,uint256 secondPrice);
...
interface IBaseDoNFT is IERC721Receiver {
    ...
    function getDoNftInfo(uint256 tokenId) external view returns(uint256 oid, uint64[] memory starts,uint64[] memory ends,uint64 nonce);
    ...
    function getDuration(uint256 tokenId,uint256 index) external view returns(uint64 start, uint64 end);
...
contract Market is OwnableContract,IMarket{
    ...
    function mintWNftAndOnLent (
        ...
        onLent(doNftAddress, nftId, maxEndTime, secondPrice);
    }
    ...
    function onLent(
        ...
        if(isOnLent(nftAddress,nftId)){
            ...
            creditMap[nftAddress].lendingMap[nftId] = Lending(msg.sender,nftAddress,nftId,secondPrice,maxEndTime,IBaseDoNFT(nftAddress).getNonce(nftId),true);
        }
        emit OnLent(msg.sender,nftAddress, nftId, maxEndTime, secondPrice);
    }
...
// File: contracts/dcl/DclMarket.sol


pragma solidity 0.8.10;


contract DclMarket is Market{

    constructor(string memory name_) Market(name_){}
    
    function makeDealNow(address nftAddress,uint256 tokenId,uint256 durationId,uint64 duration) public payable override returns(uint256 tid){
        tid = Market.makeDealNow(nftAddress, tokenId, durationId, duration);
        IDCL(nftAddress).setUpdateOperator(tid, msg.sender);
    }
}