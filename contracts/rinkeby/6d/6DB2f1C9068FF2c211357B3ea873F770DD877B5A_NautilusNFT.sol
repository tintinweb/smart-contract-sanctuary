...
contract NautilusNFT is ERC721Enumerable, Ownable, ReentrancyGuard, Pausable{
    ...
    // Maximum free mint for address
    uint256 public constant MAX_FREE_MINT = 3;
    ...
    function freeMint() public{
        ...
        require(tx.origin == msg.sender, "Contracts not allowed");
        require(presaleStart || saleStart,"Sale is not open");
        require(transactionsForAddress[msg.sender].length < TOTAL_LIMIT + MAX_FREE_MINT,"Cannot buy more than free mint limit");
        require(totalSupply() + 1 <= MAX_SUPPLY,"All has been purchased");
    ...
    function _mintToken(address _minter, uint256 _amount) private{
        ...
        // Store NFT Metadata
        nfts[totalSupply()] = NFT(totalSupply(),getRandomType(rand),getRandomLevel(rand));
    ...
    // 
    function addFreeMint(address _toFreeMint) public onlyOwner{
        isFreeMint[_toFreeMint] = true;
    ...
    function marketingAirdrop(address _receiver) external onlyOwner whenNotPaused{
    ...
    function claimSale() public nonReentrant whenPaused{
        require(tx.origin == msg.sender, "Contracts not allowed");
