...
contract AtopiaApe is AtopiaBase {
	uint256 public constant maxSale = 10_000;
	uint256 public constant saleFee = 0.07 ether;
	uint256 public constant maxSupply = 50_000;
	uint256 public constant mintFee = 280_000_000;

	address public constant dev = 0x0000000000000000000000000000000000000000;

	bool public paused;

	constructor(address bucks) AtopiaBase(bucks) ERC721("Atopia Apes", "ATPAPE") {}

	modifier onlyPaused(bool _paused) {
		require(paused == _paused, "Not Allowed");
		_;
	}
	...
	function mint(uint256 amount) external payable onlyPaused(false) {
		require(totalSupply() + amount <= maxSale, "Max Supply Reached");
		require(msg.value >= saleFee * amount, "Fee Insufficient");
		_batch(msg.sender, amount, 365 days);
	}

	function withdraw() external onlyOwner {
		uint256 funds = address(this).balance;
		uint256 devFund = (funds * 10) / 100;
		payable(dev).transfer(devFund);
		payable(owner()).transfer(funds - devFund);
	}
	...
	function useItem(
		...
		(uint256 bonusAge, uint256 bonusTrait, uint256 maxAge) = IShop(shop).itemInfo(itemId - 1);
		require(getAge(tokenId) < maxAge);
		IShop(shop).burn(msg.sender, itemId, amount);
		// ** bonusTrait
		useItemInternal(tokenId, bonusAge * amount);
	}
...
interface IShop {
	function itemInfo(uint256 index) external view returns (uint256, uint256, uint256);
...
abstract contract AtopiaBase is ERC721, Ownable {
	...
	uint256 public multiplier = 12;
	mapping(uint256 => uint256) public birth;
	mapping(uint256 => uint256) claims;
	mapping(uint256 => uint256) pendings;

	mapping(uint256 => uint256) lives;

	constructor(address _bucks) {
		bucks = IBucks(_bucks);
	}
	...
	function enter(address to, uint256 tokenId, uint256 age) internal {
		_mint(to, tokenId);
		birth[tokenId] = block.timestamp - age;
		claims[tokenId] = block.timestamp;
	}

	function _batch(address to, uint256 amount, uint256 age) internal {
	...
	function getAge(uint256 tokenId) public view returns (uint256) {
		return (block.timestamp - birth[tokenId]) * 10;
	}

	function getRewardsInternal(uint256 tokenId) internal view returns (uint256) {
		uint256 duration = block.timestamp - claims[tokenId];
		uint256 averageSpeed = block.timestamp + claims[tokenId] - birth[tokenId] * 2;
		return (averageSpeed * duration * multiplier) / 2;
	}
	...
	function useItemInternal(uint256 tokenId, uint256 bonusAge) internal {
		require(birth[tokenId] > bonusAge);
		pendings[tokenId] += getRewardsInternal(tokenId);
		claims[tokenId] = block.timestamp;
		birth[tokenId] -= bonusAge;
	}

	modifier onlySpace() {
		require(space == msg.sender);
		_;
	}
	...
	function getLife(uint256 tokenId) public view returns (uint256 job, uint256 data) {
		uint256 life = lives[tokenId];
		job = life >> 192;
		data = life & (1 << (129 - 1));
	}

	modifier notBusy(uint256 tokenId) {
		(uint256 job, ) = getLife(tokenId);
		require(job == 0);
		_;
	}

	function setLife(uint256 tokenId, uint256 life) external onlySpace notBusy(tokenId) {
		lives[tokenId] = life;
		pendings[tokenId] += getRewardsInternal(tokenId);
		claims[tokenId] = block.timestamp;
	}
	...
	function exitCenter(
		...
		uint256 rewards = getRewardsInternal(tokenId);
		uint256 fee = (rewards * ICenter(center).enjoyFee()) / 10000;
		claims[tokenId] = block.timestamp;
