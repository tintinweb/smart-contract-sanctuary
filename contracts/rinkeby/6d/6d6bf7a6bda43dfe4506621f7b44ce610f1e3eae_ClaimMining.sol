...
// File: contracts/ClaimMining3.sol
...
contract ClaimMining is OwnableContract, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    IERC20 public claimToken; //  Claim TOKEN
    ...
    constructor(address _mining_token, uint256 _mining_start_block) public {
        claimToken = IERC20(_mining_token);

        uint256 blockCountPerDay = 28800; //  3 sec per block in bsc , 20 * 60 * 24 =  28800

        phase1StartBlockNumber = _mining_start_block;
        uint256 phase1EndBlockNumber =  phase1StartBlockNumber.add(blockCountPerDay.mul(30)); // 创世30天
        phaseEndBlockNumberArray.push(phase1EndBlockNumber); 

        uint256 phase1PerBlockReword = 150 * 1e18; // 150 Claim per block
    ...
    function safeMiningTokenTransfer(address _to, uint256 _amount) internal {
        uint256 bal = claimToken.balanceOf(address(this));
        require(bal >= _amount, "ClaimMiningContract' balance is not enough.");
        claimToken.safeTransfer(_to, _amount);
    }
