...
contract PublicSale is Context, Ownable {
    ...
    function closeSaleAndAddLiquidity() external onlyOwner {
        ...
        uint256 liqAmount = contractBal.mul(65).div(100); //65% to liquidity

        addLiquidity(_token.balanceOf(address(this)), liqAmount);

        payable(owner()).transfer(address(this).balance); //Rest to owner wallet
