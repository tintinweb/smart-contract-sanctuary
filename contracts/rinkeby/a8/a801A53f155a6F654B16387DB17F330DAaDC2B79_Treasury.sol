...
contract Treasury is Ownable{
    ...
    mapping(uint => Currency) internal currencies;
    ...
    function addSupportedCurrency(uint currencyId, string memory name) onlyOwner external{
        require(!currencyNames[name].isSupported, "Currency already supported");
        currencies[currencyId] = Currency({isSupported: true, name: name});
        currencyNames[name] = Currency({isSupported: true, name: name});
        allCurrencyNames.push(name);
    }
    function addSupportedAsset(address _asset, uint currency) onlyOwner external {
        require(!assets[_asset].isSupported, "Asset already supported");
        assets[_asset] = Asset({ isSupported: true, currency: currency });
        allAssets.push(_asset);
        _assetDecimals[_asset] = ERC20(_asset).decimals();
    }

    function addSupportedToken(address _token, uint currency) onlyOwner external {
        ...
        tokens[_token] = Token({ isSupported: true, currency: currency });
