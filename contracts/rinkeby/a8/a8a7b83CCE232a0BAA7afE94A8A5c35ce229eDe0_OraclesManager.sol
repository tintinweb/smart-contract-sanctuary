...
contract OraclesManager is Ownable, IOraclesManager {
    ...
    address public workersToken;
    mapping(address => Template) public template;

    constructor(
        address _factory,
        address _workersToken,
        address _workersJobsRegistry
    ) {
        if (_factory == address(0)) revert ZeroAddressFactory();
        if (_workersToken == address(0)) revert ZeroAddressWorkersToken();
        if (_workersJobsRegistry == address(0))
            revert ZeroAddressWorkersJobsRegistry();
        factory = _factory;
        workersToken = _workersToken;
        workersJobsRegistry = _workersJobsRegistry;
    }

    function instantiate(
        address _kpiToken,
        OracleCreationData calldata _oracleData
    ) external override returns (address) {
        if (msg.sender != factory) revert Forbidden();
        address _template = _oracleData.template;
        Template storage _templateFromStorage = template[_template];
        if (!_templateFromStorage.exists) revert NonExistentTemplate();
        address _instance = Clones.clone(_template);
        uint256 _jobFunding = _oracleData.jobFunding;
        if (_jobFunding > 0) {
        ...
        IOracle(_instance).initialize(
            _kpiToken,
            _oracleData.initializationData
        );
        return _instance;
    }
    ...
    function instantiate(
        address _kpiToken,
        OracleCreationData[] calldata _oracleDatas
    ) external override returns (address[] memory) {
        if (msg.sender != factory) revert Forbidden();
        address[] memory _instances = new address[](_oracleDatas.length);
        for (uint256 _i = 0; _i < _oracleDatas.length; _i++) {
            OracleCreationData memory _oracle = _oracleDatas[_i];
            address _template = _oracle.template;
            Template storage _templateFromStorage = template[_template];
            if (!_templateFromStorage.exists) revert NonExistentTemplate();
            address _instance = Clones.clone(_template);
            uint256 _jobFunding = _oracle.jobFunding;
            if (_jobFunding > 0) {
                IJobsRegistry(workersJobsRegistry).addJob(_instance);
                IJobsRegistry(workersJobsRegistry).addCredit(
                    _instance,
            ...
            IOracle(_instance).initialize(
                _kpiToken,
                _oracle.initializationData
            );
            _instances[_i] = _instance;
        }
        return _instances;
    }

    function addTemplate(address _template, string calldata _description)
        external
        override
    {
        ...
        Template storage _templateFromStorage = template[_template];
        if (_templateFromStorage.exists) revert AlreadyAdded();
    ...
    function updgradeTemplate(
        ...
        if (!_templateFromStorage.exists) revert NonExistentTemplate();
        delete template[_template];
        template[_newTemplate] = Template({
            description: _newDescription,
            exists: true
        });
    }

    function increaseJobsRegistryAllowance() external {
        IERC20(workersToken).approve(
            workersJobsRegistry,
