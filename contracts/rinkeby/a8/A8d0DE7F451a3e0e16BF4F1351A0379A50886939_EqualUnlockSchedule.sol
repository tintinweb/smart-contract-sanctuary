...
contract EqualUnlockSchedule is BaseGovernanceWithUserUpgradable {
    ...
    function __EqualUnlockSchedule_init(ILock _lock, bytes calldata _data) internal initializer {
        __EqualUnlockSchedule_init_unchained(_lock, _data);
    }

    function __EqualUnlockSchedule_init_unchained(ILock _lock, bytes calldata _data) internal initializer {
        (
            uint32 _lockLenght, 
            uint32 _lockPeriod,
        ...
        lockContract = _lock;
        endTimeDay = uint32(block.timestamp) + (_lockLenght * 1 days);
        unlockTimes = _lockLenght / _lockPeriod;
        unlockCliff = uint32(block.timestamp) + (_lockPeriod * 1 days);
    }

    /**
     * @notice Calculate the amount of unlocked tokens
     * @dev The lock contract pass the arguments
     * @param lockStartTimestamp Timestamp when the Lock start (provide by the Lock Contract)
     * @param initialAmount Initial amount of the lock
     * @return The unlocked tokens
     */
    function unlockedAmount(uint256 lockStartTimestamp, uint256 initialAmount) external view returns(uint256) {
        uint256 _unlockedAmount;
        if(block.timestamp < unlockCliff) {
            _unlockedAmount = 0;
        } else if (block.timestamp >= endTimeDay) {
            _unlockedAmount = initialAmount;
        } else {
            uint32 _lockLenght = (endTimeDay - uint32(lockStartTimestamp)) / 1 days;
            uint32 _lockPeriod = _lockLenght / unlockTimes;
            uint32 _multiplier = (uint32(block.timestamp - lockStartTimestamp) / 1 days) / _lockPeriod;
            _unlockedAmount = (initialAmount / uint256(unlockTimes)) * _multiplier;
        }
     ...
     */
    function endLockTime() external view returns(uint) {
        return endTimeDay;
    }
...
abstract contract BaseGovernanceWithUserUpgradable is Initializable, ContextUpgradeable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function __BaseGovernanceWithUser_init(address governer) internal initializer {
    ...
    function __BaseGovernanceWithUser_init_unchained(address governer) internal initializer {
...
interface ILock is IERC721Upgradeable{
     ...
     */
    function claimUnlocked(uint256 nftId, uint256 amountToClaim, bytes calldata ownershipProof) external;
     ...
     * @param ownershipProof ABI-encoded data to verify in MerkleTree (specific to DepositManager with Merkle Tree)
     */
    function split(uint originId, uint[] memory splitedIds, uint[] memory splitParts, address[] memory addresses, bytes calldata ownershipProof) external;
    ...
    function burnNFTs(uint[] memory IDs) external;
}
...
interface IDepositManager {
     ...
     * @param data ABI-encoded data of beneficiaries IDs
     * @return List of NFTs to be removed
     */
    function removeDeposits(bytes calldata data) external returns(uint[] memory);
     ...
     * @param Id NFT ID to verify
     * @param beneficiary The beneficiary/caller
     * @param data Empty OR initial amount and array of hashes of merkle tree leafs
     */
    function verifyOwnership(uint Id, address beneficiary, bytes calldata data) external returns(bool, uint);
}
...
interface ISplitManager {
    /**
     * @notice Get the initial locked part of the lock
     * @return The initial locked part proportion normalized
     */
    function initialLockedPart() view external returns(uint);
