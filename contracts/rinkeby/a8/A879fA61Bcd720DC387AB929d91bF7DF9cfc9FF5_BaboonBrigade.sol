...
contract BaboonBrigade is ERC721Enumerable, Ownable {
	...
	function buyPremint(uint256 tokenQuantity) external payable {
        require(tokenQuantity <= BBG_PER_MINT, "EXCEED_PER_MINT");
        require(tokenQuantity <= BBG_PER_MINT, "EXCEED_PER_MINT");
		require(_state == State.PRESALE, "NOT_ON_PUBIC_SALE");
		require(
			BBG_PER_PRICE * tokenQuantity <= msg.value,
			"Not Enough payments"
		);
		require(lastPresaleId + tokenQuantity <= 777, "EXCEED_PREMINT_AMOUNT");
        for (uint256 i = 0; i < tokenQuantity; i++) {
	...
	/**
	 * @notice premint
	 * @param quantity total amount of nfts to be pre-minted
	 */
	function premint(uint256 quantity) external onlyOwner {
        require(
            totalSupply().add(quantity) <= BBG_MAX,
            "Sorry, there's not that many BBG left."
        );
        for (uint256 i = 0; i < quantity; i++) {
			uint256 tokenId = totalSupply() + 1;
            _safeMint(creatorAddress, tokenId);
        }
	}
     ...
     * @param _tokenBaseURI token ID which need to be finished
     */
	function setTokenBaseURIPublic(string memory _tokenBaseURI) public onlyOwner {
