...
contract TDNButterflyCheckpoint is ERC1155Burnable, Ownable {
    ...
    function mintOne(uint256 id) external {
        require(!minted[msg.sender], "already minted");
        require(mintActive, "mint not active");

        require(id == GOLD_NINJA_SKULLY || id == BLUE_NINJA_SKULLY || id == GIMPDICKBUTT, "wrond id");
    ...
    function mintTwo(uint256 first, uint256 second) external {
        require(!minted[msg.sender], "already minted");
        require(mintActive, "mint not active");

        require(first != second, "ids are the same");
        require(first == GOLD_NINJA_SKULLY || first == BLUE_NINJA_SKULLY || first == GIMPDICKBUTT, "wrond id 1");
        require(second == GOLD_NINJA_SKULLY || second == BLUE_NINJA_SKULLY || second == GIMPDICKBUTT, "wrond id 2");
