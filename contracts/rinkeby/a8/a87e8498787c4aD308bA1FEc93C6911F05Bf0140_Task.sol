...
contract UniStaker is IUniStaker, Powered, Ownable {
    using SafeMath for uint256;
    ...
    /// token transfer: none
    /// @param ownerAddress address The owner address
    ...
    constructor(
        ...
        // deploy reward pool
        address rewardPoolAddress = IFactory(rewardPoolFactory).create(abi.encode(powerSwitch));
    ...
    function fund(address token, uint256 amount) external onlyOwner onlyOnline {
    ...
    /// token transfer: none
    /// @param bonusToken address The address of the bonus token
    function removeBonusToken(address bonusToken) external onlyOwner notShutdown {
    ...
    function addRewardCalcTemplate(string calldata name, address template) external onlyOwner notShutdown {
    ...
    function setRewardCalcActiveTemplate(string calldata name) external onlyOwner onlyOnline {
    ...
    function startLMRewards(address token, uint256 amount, uint256 duration) external onlyOwner onlyOnline {
        startLMRewards(token, amount, duration, activeRewardCalcTemplate);
    }

    function startLMRewards(address token, uint256 amount, uint256 duration, string memory rewardCalcTemplateName) public onlyOwner onlyOnline {
    ...
    function setImplementorForRewardsCalculator(address token, address newImplementor) public onlyOwner notShutdown {
    ...
    function setLMRewardsPerBlock(address token, uint value) public onlyOwner onlyOnline {
        require(lmRewards[token].startedAt != 0, "No reward program currently live for this token");
        address rewardCalcInstance = lmRewards[token].rewardCalcInstance;
        IERC2917(rewardCalcInstance).changeInterestRatePerBlock(value);
    }

    function addBonusTokenToLMRewards(address lmToken, address bonusToken, uint256 bonusTokenAmount) public onlyOwner onlyOnline {
    ...
    function endLMRewards(address token, bool removeBonusTokenData) public onlyOwner notShutdown {
    ...
    function setWeeklyAirdropAmount(string calldata tier, uint256 amount) external onlyOwner onlyOnline {
        weeklyAirdropAmounts[tier] = amount;
    }

    function setBalanceRequiredToClaim(string calldata tier, uint256 amount) external onlyOwner notShutdown {
        balancesRequiredToClaim[tier] = amount;
    }

    function setMaxStakesPerVault(uint256 amount) external onlyOwner notShutdown {
        MAX_TOKENS_STAKED_PER_VAULT = amount;
    }

    function setMaxBonusTokens(uint256 amount) external onlyOwner notShutdown {
        MAX_BONUS_TOKENS = amount;
    }

    function setMinRewardClaimFrequency(uint256 amount) external onlyOwner notShutdown {
        MIN_AIRDROP_REWARD_CLAIM_FREQUENCY = amount;
    }

    function setPlatinumLMRewardMultiplier(uint256 numerator, uint256 denominator) external onlyOwner notShutdown {
        PLATINUM_LM_REWARD_MULTIPLIER_NUM = numerator;
        PLATINUM_LM_REWARD_MULTIPLIER_DENOM = denominator;
    }

    function setGoldLMRewardMultiplier(uint256 numerator, uint256 denominator) external onlyOwner notShutdown {
        GOLD_LM_REWARD_MULTIPLIER_NUM = numerator;
        GOLD_LM_REWARD_MULTIPLIER_DENOM = denominator;
    }

    function setMintLMRewardMultiplier(uint256 numerator, uint256 denominator) external onlyOwner notShutdown {
        MINT_LM_REWARD_MULTIPLIER_NUM = numerator;
        MINT_LM_REWARD_MULTIPLIER_DENOM = denominator;
    }

    function setBlackLMRewardMultiplier(uint256 numerator, uint256 denominator) external onlyOwner notShutdown {
        BLACK_LM_REWARD_MULTIPLIER_NUM = numerator;
        BLACK_LM_REWARD_MULTIPLIER_DENOM = denominator;
    }

    function setLMRewardVestingPeriod(uint256 amount) external onlyOwner notShutdown {
        LM_REWARD_VESTING_PERIOD = amount;
    }

    function setLMRewardVestingPortion(uint256 numerator, uint denominator) external onlyOwner notShutdown {
    ...
    function calculateAirdropReward(address owner, address nftFactory) public view returns (uint256 amount, uint256 balanceRequiredToClaim, uint256 balanceLocked) {
        uint256[] memory nftIds = getNftsOfOwner(owner, nftFactory);
        return calculateAirdropReward(nftFactory, nftIds);
    }

    function calculateAirdropReward(address nftFactory, uint256[] memory nftIds) public view returns (uint256 amount, uint256 balanceRequiredToClaim, uint256 balanceLocked) {
    ...
    function calculateAirdropReward(address nftFactory, uint256 nftId) public view returns (uint256 amount, uint256 balanceRequiredToClaim, uint256 balanceLocked) {
        uint256 timeSinceLastRewardClaim = block.timestamp.sub(nftLastClaimedRewardAt[nftId]);
        require(timeSinceLastRewardClaim > MIN_AIRDROP_REWARD_CLAIM_FREQUENCY, "Claimed reward recently");
    ...
    function claimVestedReward(address token, uint numVests) public onlyOnline {
        ...
        for (uint index = 0; index < numVests; index++) {
            ...
            if (vested >= vest.amount) {
                ...
                // last item is not processed in this call
