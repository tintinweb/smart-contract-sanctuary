...
contract RecyclingPaper is RawToken {
    ...
    constructor() RawToken("RecyclingPaper", "PAPER", "https://google.com"){
    ...
    function setPurchaseRules(
        ...
        require(hasRole(BACKEND_ROLE, _msgSender()), "RecyclingPaper: must have backend role to change price");
    ...
    function alreadyToSold(uint256 totalTokens, bool isResource) public override {
        require(hasRole(BACKEND_ROLE, _msgSender()), "RecyclingPaper: must have backend role to change total tokens");
    ...
    function futureToResource(uint256 tokenId) public override {
        require(hasRole(BACKEND_ROLE, _msgSender()), "RecyclingPaper: must have backend role to change token type");
        require(!tokenInfo[tokenId].is_resource_token, 'RecyclingPaper: Token already is resource');
        require(block.timestamp - tokenInfo[tokenId].purchase_date
            <= tokenInfo[tokenId].benefits_period, 'RecyclingPaper: token expired');
        require(tokenInfo[tokenId].purchase_date / TROPICAL_YEAR == block.timestamp / TROPICAL_YEAR,
            'RecyclingPaper: forbidden - FToken was minted at past year');
