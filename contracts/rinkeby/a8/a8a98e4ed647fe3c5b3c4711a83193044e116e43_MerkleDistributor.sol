...
import '../reward/KombatPool.sol';

contract MockKombatPool is KombatPool {
    constructor(IERC20 _lpToken, IERC20 _rewardToken) public {
        lpToken = _lpToken;
...
contract LaunchPoolWithReward is Context, StakableTokenWrapper, AccessControl {
    bytes32 public constant MINTER_ROLE = keccak256('MINTER_ROLE');
    ...
    event Rewarded(address indexed rewarder, address indexed user, uint256 amount);
    event RewardedBatch(address indexed rewarder, address[] indexed users, uint256[] amounts);
    ...
    function reward(address account, uint256 amount) public virtual updateReward(account) {
        require(hasRole(MINTER_ROLE, _msgSender()), 'LaunchPool: Must have minter role to reward');

        require(account != address(0), 'LaunchPool: Cannot reward zero address');

        require(amount > 0, 'LaunchPool: Amount should be gt 0 to reward');

        require(super.balanceOf(account) > 0, 'LaunchPool: Account stake should be gt 0 to reward');

        vombats[account] = vombats[account].add(amount);
        emit Rewarded(_msgSender(), account, amount);
    }

    function rewardBatch(address[] memory _accounts, uint256[] memory _amounts) public virtual {
        require(hasRole(MINTER_ROLE, _msgSender()), 'LaunchPool: Must have minter role to reward');

        require(_accounts.length == _amounts.length, 'LaunchPool: accounts and amounts length mismatch');
        for (uint256 i = 0; i < _accounts.length; i++) {
            address account = _accounts[i];
            uint256 amount = _amounts[i];
            require(account != address(0), 'LaunchPool: reward to the zero address');
            require(amount > 0, 'LaunchPool: Amount should be gt 0 to reward');
            vombats[account] = vombats[account].add(amount);
        }
        emit RewardedBatch(_msgSender(), _accounts, _amounts);
    }
}
