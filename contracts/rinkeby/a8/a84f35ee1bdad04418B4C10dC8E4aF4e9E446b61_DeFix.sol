...
abstract contract Uniswap is Ownable {
    ...
    address USDTAddress = 0xc7AD46e0b8a400Bb3C915120d284AafbA8fc4735;
    ...
    function swapUsdtForTokens(address tokenAddress, uint256 usdtAmount) internal swapping pcsInitialized returns (uint256) {
        ...
        console.log('initial token Balance: %s', IERC20(tokenAddress).balanceOf(address(this)));
        ...
        console.log('usdtAmount: %s', usdtAmount);
        router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            usdtAmount,
        ...
        console.log('Tokens price in USDT: %s', calculateTokensPriceInUSDT(tokenAddress, tokenReceived));
        console.log('swapUsdtForTokens, tokenReceived: %s', tokenReceived);
        return tokenReceived;
    }

    function swapBnbForTokens(address tokenAddress, uint256 tokenAmount) internal swapping pcsInitialized returns (uint256) {
        if (tokenAddress == router.WETH()) {
            return tokenAmount;
        } else {
            uint256 initialBalance = IERC20(tokenAddress).balanceOf(address(this));
...
interface IDeFiX {
    ...
    // начало ----------------------- все что касается самих индексов --------------------- начало
    enum Status {
        CREATED, // автомотически назначается как только создали индекс (yellow)
        ACTIVE, // с этого статуса и ниже запрещается вносить изменения в индекс. разрешается закидывать и снимать бабки с индекса. (green)
        PAUSED, // нельзя вкидывать, но можно снять бабки. может быть восстановлен оунером. (orange)
        DELETED // нельза вкидываьт, но можно снять. не восстанавливаемый индекс.
    } // статус индекса. активен или на паузе.

    struct Share {
        address tokenAddress; // адрес монеты
        uint256 point; // доля монеты в портфеле в поинтах
        // (например токен1 - 10 поинтов, токен2 - 5 поинтов. всего 15 поинтов. доля 10/15 и 5/15 соответственно)
    }

    struct Project {
        uint256 id;
        Status status; // статус индекса. см.выше.
        uint256 totalPoints;
        string name;
        string description;
        ...
        // Всегда должны быть на последнем месте
        Share[] tokens;
    }

    // возвращает все индексы кроме удаленных с полной информацией
    ...
    // принимает всю необходимую инфу для создания индекса и возвращает созданный индекс.
    ...
    // обновляет только имя и описание. если будет еще какой-то параметр то и его обновить можно.
    ...
    // конец ----------------------- все что касается самих индексов --------------------- конец

    // начало ----------------------- все что касается инвестора --------------------- начало

    struct Investor {
        uint256[] investedIds; // айдишники индексов куда чел вкинул бабки
        mapping(uint256 => uint256) investedAmountInUSDT; //сколько в каждый индекс вкинул
        mapping(uint256 => bool) invested; //для проверки, инвестировал ли уже в какой-то индекс
    }

    // выдавать данные по msg.sender. но иногда этот метод тупить, тогда просто передавать свой адрес аргументом
...
contract DeFix is IDeFiX, Uniswap {
    ...
    constructor() {
        initDEXRouter(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
    }
    ...
    function createProject(
        ...
        //should we check that all sent addresses exist in pancake?
    ...
    function getInvested(address _sender) external view override returns (uint256[] memory, uint256[] memory) {
        ...
        for (uint256 i = 0; i < investedIds.length; i++) {
            investedAmountInUSDT[i] = investor.investedAmountInUSDT[i];
        }
    ...
    function investInUsdt(uint256 _projectId, uint256 _amount) external override returns (uint256) {
        ...
        uint256 investedInUSDT = this.getWithdrawable(0, msg.sender);
    ...
    function investInBnb(uint256 _projectId) external payable override returns (uint256) {
        ...
        Share[] memory shares = projects[_projectId].tokens;
        for (uint256 i = 0; i < shares.length; i++) {
            uint256 tokensToBuy = amountToInvest * shares[i].point / projects[_projectId].totalPoints;
            uint256 tokenReceived = swapBnbForTokens(shares[i].tokenAddress, tokensToBuy);
            balanceOf[_projectId][msg.sender][shares[i].tokenAddress] += tokenReceived;
        }
    ...
    function withdrawInUsdt(uint256 _projectId, uint256 _percent) external override returns (uint256) {
        ...
        for (uint256 i = 0; i < shares.length; i++) {
            ...
            withdrawnInUSDT += usdtReceived;
        }

        Investor storage investor = investors[msg.sender];
        if (_percent == 100) {
            delete investor.investedIds[_projectId];
            investor.invested[_projectId] = false;
            projects[_projectId].totalInvestorsAmount -= 1;
        }

        if (withdrawnInUSDT > investor.investedAmountInUSDT[_projectId]) {
            investor.investedAmountInUSDT[_projectId] = 0;
        } else {
            investor.investedAmountInUSDT[_projectId] -= withdrawnInUSDT;
        }
        if (withdrawnInUSDT > projects[_projectId].totalInvestedInUSDT) {
            projects[_projectId].totalInvestedInUSDT = 0;
        } else {
            projects[_projectId].totalInvestedInUSDT -= withdrawnInUSDT;
        }

        IERC20(USDTAddress).transfer(msg.sender, withdrawnInUSDT);
        return withdrawnInUSDT;
    }

    function withdrawInBnb(uint256 _projectId, uint256 _percent) external override returns (uint256) {
        Share[] memory shares = projects[_projectId].tokens;
        uint256 withdrawnInUSDT;
        uint256 withdrawnInBNB;
        for (uint256 i = 0; i < shares.length; i++) {
            uint256 tokensToSell = balanceOf[_projectId][msg.sender][shares[i].tokenAddress] * _percent / 100;
            (uint256 bnbReceived, uint256 tokenSwapped) = swapTokensForBnb(shares[i].tokenAddress, tokensToSell);
            balanceOf[_projectId][msg.sender][shares[i].tokenAddress] -= tokenSwapped;
            withdrawnInUSDT += calculateTokensPriceInUSDT(shares[i].tokenAddress, tokenSwapped);
            withdrawnInBNB += bnbReceived;
        }
        ...
        payable(msg.sender).transfer(withdrawnInBNB);
        return withdrawnInUSDT;
    }
...
//["0x154Fc18dDB016a26451B034F48331FedeB45A9B4", "0x154Fc18dDB016a26451B034F48331FedeB45A9B4"],[25, 10],"test2","test desc2"
