...
import "./library/ERC721Holder.sol";
...
contract Item is ERC1155, ERC721Holder, ERC1155Holder, ItemComponents, MaterialMetadata {

    //recipe contract (RINKEBY TEST NET)
    IERC721 recipe = IERC721(0x5141D0b2897f9C144E1e175B366b75459B6608EE);
    RecipeInterface recipeContract = RecipeInterface(0x5141D0b2897f9C144E1e175B366b75459B6608EE);
    IERC1155 materialBagItems = IERC1155(0x7276E65Ad5968FF9337D8dCE5890D96365F800Fd);

    //for testing
    //IERC721 recipe;
    //IERC1155 materialBagItems;

    constructor() ERC1155("") {
        //testing
        //recipe = IERC721(_recipe);
        //materialBagItems = IERC1155(_materialBagItems);
        _mint(0x9d6C5Ba61C6348878623E90ba0934fa8Cd58169C, 2, 1, "");
    ...
    modifier onlyOwner(uint tokenId) {
        require(recipeContract.ownerOf(tokenId) == msg.sender, 'Not owner of token');
        _;
    }

    function tokenName(uint256 id) private view returns (string memory) {
    ...
    function itemName(uint256 itemType, uint256 idx) private view returns (string memory) {
        string[] storage arr;
        if (itemType == WEAPON) {
            arr = weapons;
    ...
    function seeRecipeName(uint tokenId) onlyOwner(tokenId) public view returns(string memory) {
        return getRecipeName(tokenId, false);
    }

    function getRecipeName(uint tokenId, bool vanilla) private view returns(string memory) {
    ...
    function seeRecipeRequirements(uint tokenId) onlyOwner(tokenId) public view returns(RecipeItemNames memory, RecipeItemIds memory){
        return getRecipeRequirements(tokenId);
    }

    function getRecipeRequirements(uint tokenId) private view returns(RecipeItemNames memory, RecipeItemIds memory){
        ...
        if (compareStrings(recipeNames.recipe, 'Light Saber')) {
            ...
            index[0] = 16;
            recipeIds.material = MaterialTokenId.toId(index, MaterialMetadata.MATERIALS);
            legendary = true; 
        }
            ...
            index[0] = 25;
            recipeIds.charm = MaterialTokenId.toId(index, MaterialMetadata.CHARMS);
            legendary = true; 
        }
            ...
            index[0] = 9;
            recipeIds.element = MaterialTokenId.toId(index, MaterialMetadata.ELEMENTS);
            legendary = true; 
        }
        ...
        //check if rare
            ...
            //require rare item
            if (rand % 5 == 0) {
                (recipeNames.gem, recipeIds.gem) = getRareItem(tokenId, MaterialMetadata.GEMS);
            }
            else if (rand % 5 == 1) {
                (recipeNames.rune, recipeIds.rune) = getRareItem(tokenId, MaterialMetadata.RUNES);
            }
            else if (rand % 5 == 2) {
                (recipeNames.charm, recipeIds.charm) = getRareItem(tokenId, MaterialMetadata.CHARMS);
            }
            else if (rand % 5 == 3) {
                (recipeNames.element, recipeIds.element) = getRareItem(tokenId, MaterialMetadata.ELEMENTS);
            }
            else {
                (recipeNames.material, recipeIds.material) = getRareItem(tokenId, MaterialMetadata.MATERIALS);
            }
        }
        ...
        if (compareStrings(recipeNames.gem, '')){
            recipeIds.gem = MaterialMetadata.gemId(tokenId);
            recipeNames.gem = MaterialMetadata.materialTokenName(recipeIds.gem);
        }
        if (compareStrings(recipeNames.rune, '')){
            recipeIds.rune = MaterialMetadata.runeId(tokenId);
            recipeNames.rune = MaterialMetadata.materialTokenName(recipeIds.rune);
        }
        if (compareStrings(recipeNames.charm, '')){
            recipeIds.charm = MaterialMetadata.charmId(tokenId);
            recipeNames.charm = MaterialMetadata.materialTokenName(recipeIds.charm);
        }
        if (compareStrings(recipeNames.element, '')){
            recipeIds.element = MaterialMetadata.elementId(tokenId);
            recipeNames.element = MaterialMetadata.materialTokenName(recipeIds.element);
        }
        if (compareStrings(recipeNames.material, '')){
            recipeIds.material = MaterialMetadata.materialId(tokenId);
            recipeNames.material = MaterialMetadata.materialTokenName(recipeIds.material);
        }

        recipeIds.tool = MaterialMetadata.toolId(tokenId);
        recipeNames.tool = MaterialMetadata.materialTokenName(recipeIds.tool);
        recipeIds.requirement = MaterialMetadata.requirementId(tokenId);
        recipeNames.requirement = MaterialMetadata.materialTokenName(recipeIds.requirement);
    ...
    function getRareItem(uint tokenId, uint itemType) private view returns(string memory, uint) {
        ...
        if (itemType == MaterialMetadata.GEMS) {
            rand = rand % rareGemsLength;
            index[0] = rareGemsIndices[rand];
            return (rareGems[rand], MaterialTokenId.toId(index, MaterialMetadata.GEMS));
        }
        else if (itemType == MaterialMetadata.RUNES) {
            rand = rand % rareRunesLength;
            index[0] = rareRunesIndices[rand];
            return (rareRunes[rand], MaterialTokenId.toId(index, MaterialMetadata.RUNES));
        }
        else if (itemType == MaterialMetadata.MATERIALS) {
            rand = rand % rareMaterialsLength;
            index[0] = rareMaterialsIndices[rand];
            return (rareMaterials[rand], MaterialTokenId.toId(index, MaterialMetadata.MATERIALS));
        }
        else if (itemType == MaterialMetadata.CHARMS) {
            rand = rand % rareCharmsLength;
            index[0] = rareCharmsIndices[rand];
            return (rareCharms[rand], MaterialTokenId.toId(index, MaterialMetadata.CHARMS));
        }
        else {
            rand = rand % rareElementsLength;
            index[0] = rareElementsIndices[rand];
            return (rareElements[rand], MaterialTokenId.toId(index, MaterialMetadata.ELEMENTS));
        }
    }
    ...
    function attributes(uint256 id) private view returns (string memory) {
        (uint256[5] memory components, uint256 itemType) = ItemTokenId.fromId(id);
    ...
    /// @notice Burns the erc1155 recipe item from your account and then transfers the 721 
    /// recipe item from the contract to you, along with the rest of the 1155 materials.

    function breakdownItem (uint256 recipeTokenId) external {

        uint256 itemID;
        
        RecipeItemNames memory recipeNames;
        RecipeItemIds memory recipeIds;
        bytes memory data = bytes('random');

        itemID = getItemId(recipeTokenId);
        (recipeNames, recipeIds) = getRecipeRequirements(recipeTokenId);

        uint256[] memory materialIds = new uint256[](7);
        materialIds[0] = recipeIds.gem;
        materialIds[1] = recipeIds.rune;
        materialIds[2] = recipeIds.material;
        materialIds[3] = recipeIds.charm;
        materialIds[4] = recipeIds.tool;
        materialIds[5] = recipeIds.element;
        materialIds[6] =  recipeIds.requirement;
        ...
        // 2. give back materials
        materialBagItems.safeBatchTransferFrom(address(this), msg.sender, materialIds, amounts, data);

        // 3. give back recipe
        recipe.safeTransferFrom(address(this), msg.sender, recipeTokenId);
    }

    /// @notice Transfers the erc721 recipe from your account to the contract and then
    /// burns 7 materialBagItems, to mint you one (1) item. Use it if you have already approved the transfer, else consider
    /// just transferring directly to the contract and letting the `onERC721Received`
    /// do its part
    function combine(uint256 recipeTokenId) external {
        recipe.safeTransferFrom(msg.sender, address(this), recipeTokenId);
        transferMaterials(recipeTokenId);
        combine(msg.sender, recipeTokenId);
    }

    /// @notice ERC721 callback which will combine the bag
    function onERC721Received(
        address,
        bytes calldata
    ) external view returns (bytes4) {
        // only supports callback from the recipe contract
        require(msg.sender == address(recipe), 'Callback needs to be from recipe contract');
        return Item.onERC721Received.selector;
    }

    /// @notice ERC1155 callback which will combine the bag
    function onERC1155BatchReceived(
        address,
        bytes calldata
    ) external view returns (bytes4) {
        // only supports callback from the recipe contract
        require(msg.sender == address(materialBagItems), 'Callback needs to be from materialBagItems contract');
        return Item.onERC1155BatchReceived.selector;
    }

    function transferMaterials(uint256 recipeTokenId) private {
        ...
        materialBagItems.safeBatchTransferFrom(msg.sender, address(this), materialIds, amounts, data);

    }
    /// @notice Combines your recipe and 7 materials and mints you 1 ERC-1155 RecipeItem
    function combine(address who, uint256 recipeTokenId) private {

        uint256 itemID;
        itemID = getItemId(recipeTokenId);

        // 2. mint item 
        _balances[itemID][who] += 1;
        emit TransferSingle(_msgSender(), address(0), who, itemID, 1);
    }

    /// @notice Returns an SVG for the provided 1155 token id
    function uri(uint256 tokenId) override public view returns (string memory) {
        string[3] memory parts;
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 16px; }</style><rect width="100%" height="100%" fill="black" /><text class="base">';
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Item #', Strings.toString(tokenId), '", "description": "Item allows you to create items from Material Bag Item tokens and your Recipe token, or breakdown your 1155 Item to retrieve the original ERC721 Recipe and ERC1155 Material Bag Items.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
