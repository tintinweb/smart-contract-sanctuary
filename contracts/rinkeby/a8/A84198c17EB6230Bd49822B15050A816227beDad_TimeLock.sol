...
contract TimeLock is Initializable, OwnableUpgradeable {
    ...
    modifier onlyAfterStart {
        uint256 finalEndTime = project.finalEndTime();
        require(
            finalEndTime != 0 && block.timestamp > finalEndTime,
            "Has not yet started."
        );
        _;
    }
    ...
    function setProject(Project _project) external onlyOwner {
        project = _project;
    }

    function getCurrentCycleCount() public view returns (uint256) {
        uint256 finalEndTime = project.finalEndTime();
        if (block.timestamp <= finalEndTime || finalEndTime == 0) return 0;
        return block.timestamp.sub(finalEndTime).div(releaseInterval).add(1);
    }
