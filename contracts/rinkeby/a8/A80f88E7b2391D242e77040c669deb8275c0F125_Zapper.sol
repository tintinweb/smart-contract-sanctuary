...
contract Zapper is Ownable, IZapper {
    ...
    constructor (address _pcsV1Router, address _WBNB, address _ULTRA, address _WAGMI, address _BUSD, address _ULTRABUSD, address _WAGMIBUSD) public {
        ...
        IERC20(_ULTRA).safeApprove(address(ROUTER), MAX_INT);
        IERC20(_WAGMI).safeApprove(address(ROUTER), MAX_INT);
        IERC20(_BUSD).safeApprove(address(ROUTER), MAX_INT);

        //approve to breakLP
        IERC20(_ULTRABUSD).safeApprove(address(ROUTER), MAX_INT);
        IERC20(_WAGMIBUSD).safeApprove(address(ROUTER), MAX_INT);

        // set route pairs for our tokens
        routePairAddresses[_WAGMI] = _BUSD;
        routePairAddresses[_ULTRA] = _BUSD;
    }
    ...
    function zapTokenToLP(address _from, uint amount, address _to) external override {
        ...
        // WAGMI, ULTRA, BUSD to create WAGMI-BUSD or ULTRA-BUSD will hit this if
     ...
     * Not very useful for our current use case as bolt input currencies will only be BUSD, WBNB, ULTRA, WAGMI
    ...
    function _swap(address _from, uint amount, address _to, address receiver) private returns (uint) {
        ...
        if (intermediate != address(0) && (_from == WBNB || _to == WBNB)) {
            ...
            // [WAGMI/WAGMI, BUSD, WBNB, xRoute, xToken]
            path = new address[](5);
            path[0] = _from;
            ...
            // E.g. [WAGMI/ULTRA, BUSD, WBNB, xTokenWithWBNBLiquidity]
            path = new address[](4);
            path[0] = _from;
            path[1] = intermediate;
            path[2] = WBNB;
            ...
            // E.g. [xTokenWithWBNBLiquidity, WBNB, BUSD, WAGMI/ULTRA]
            path = new address[](4);
            path[0] = _from;
            path[1] = WBNB;
            path[2] = intermediate;
    ...
    function approveNewInputToken(address token) external onlyOwner {
        if (IERC20(token).allowance(address(this), address(ROUTER)) == 0) {
            IERC20(token).safeApprove(address(ROUTER), MAX_INT);
        }
    }

    // function to allow owner to claim ERC20 tokens sent to this contract
