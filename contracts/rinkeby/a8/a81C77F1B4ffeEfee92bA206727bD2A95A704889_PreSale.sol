...
contract PreSale {
    ...
    mapping(address => bool) boughts;
    mapping(address => bool) whiteListed;

    event Purchase(address indexed account, uint256 amountIn, uint256 amountOut);
    ...
    function whiteListBuyers(address[] memory buyers_) external returns ( bool ) {
        require(msg.sender == governance, "!governance");
        require(saleStarted == false, 'Already initialized');

        totalWhiteListed = totalWhiteListed.add(buyers_.length);

        for(uint i; i < buyers_.length; i++){
    ...
    function initialize(address emoji_, address dai_, address addressTreasury_, uint salePrice_, uint allotment_, uint saleLength_) external returns(bool) {
        require(msg.sender == governance, "!governance");
        require(saleStarted == false, 'Already initialized');
        ...
        endOfSale = saleLength_.add(block.timestamp);
    ...
    function purchase(uint amountDAI_) external returns(bool) {
        require(saleStarted == true, 'Not started');
        require(whiteListed[msg.sender] == true, 'Not whitelisted');
        require(boughts[msg.sender] == false, 'Already participated');
        require(block.timestamp < endOfSale, 'Sale over');

        boughts[msg.sender] = true;

        uint _purchaseAmount = _calculateSaleQuote(amountDAI_);

        require(_purchaseAmount <= getAllotmentPerBuyer(), 'More than alloted');
        totalSale++;

        IERC20(DAI).safeTransferFrom(msg.sender, addressTreasury, amountDAI_);
        IERC20(Emoji).safeTransferFrom(addressTreasury, msg.sender, _purchaseAmount);
        emit Purchase(msg.sender, amountDAI_, _purchaseAmount);
