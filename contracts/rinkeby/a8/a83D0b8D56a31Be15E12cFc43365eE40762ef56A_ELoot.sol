...
//    address private _lootAddress = address(0x835d6c7e9f9b466732cc0cd606ed080e9f502808);
    Loot loot = Loot(0x835d6c7E9f9B466732cC0CD606ed080e9F502808);

    function random(string memory input) internal pure returns (uint256) {
        return uint256(keccak256(abi.encodePacked(input)));
    }

    function getHealthy(uint256 tokenId) public view returns (string memory) {
    ...
    function getLeadership(uint256 tokenId) public view returns (string memory) {
    ...
    function rowString( string memory parts, uint index) internal view returns (string memory){
       return string(abi.encodePacked( '<text x="10" y="'  ,index*20 ,'" class="base">',parts,'</text>'));
    }
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Bag #',tokenId, '", "description": "ELoot is randomized adventurer gear generated and stored on chain.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
...
//    }
    ...
    constructor() ERC721("Loot extend", "ELoot") Ownable() {}
}
