...
contract PLUMX is Context, IERC20, IERC20Extended, Ownable {
    using SafeMath for uint256;
    string private constant _name = "PlumX";
    string private constant _symbol = "PLUMX";
    ...
    address payable constant private _marketingAddress = payable(0x1eE134E4Fccd51aEbB0B2d1e475ae4f57d41ac70);
    address payable constant private _buybackAddress = payable(0xA31EDd1e3347c8448a3BfB732951370f1F25383A);
    address payable constant private _projectMaintainence = payable(0xCFb1c621ae171875b14c530e9bb33935FFD4821E);
    ...
    event Token1Addr (string value,address addr);

    AggregatorV3Interface internal priceFeed;
    address public _oraclePriceFeed = 0x8A753747A1Fa494EC906cE90E9f37563A8AF630e;//rinkeby 0x8A753747A1Fa494EC906cE90E9f37563A8AF630e;// bnb testnet 0x2514895c72f50D8bd4B4F9b1110F0D6bD2c97526;// bnb mainnet 0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE;
    ...
    constructor() {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);//ropsten 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); //bsc test 0xD99D1c33F9fC3444f8101754aBC46c52416550D1);//bsc main net 0x10ED43C718714eb63d5aA57B78B54704E256024E);
    ...
    function getTokenPrice() external view returns(uint256) {
        IERC20Extended token0 = IERC20Extended(IUniswapV2Pair(uniswapV2Pair).token0());//plumx
        IERC20Extended token1 = IERC20Extended(IUniswapV2Pair(uniswapV2Pair).token1());//bnb
        (uint112 Res0, uint112 Res1,) = IUniswapV2Pair(uniswapV2Pair).getReserves();
        if(pairSwapped) {
            token0 = IERC20Extended(IUniswapV2Pair(uniswapV2Pair).token1());//plumx
            token1 = IERC20Extended(IUniswapV2Pair(uniswapV2Pair).token0());//bnb
