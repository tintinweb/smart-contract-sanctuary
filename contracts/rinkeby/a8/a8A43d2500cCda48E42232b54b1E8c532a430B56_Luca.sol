...
interface LucaInterface is IERC20{
    ...
    function setMinter(address user) external returns (bool);
    function rebaseByMilli(uint256 epoch, uint256 milli, bool positive) external returns (uint256);
    function rebase(uint256 epoch, uint256 indexDelta, bool positive) external returns (uint256);
    function setRebaser(address user) external returns (bool);
}
...
contract Token is LucaInterface, LucaTokenStorage {
    ...
    function setMinter(address user) override external onlyMinter returns(bool){
        _setMinter(user);
        return true;
    }
