...
contract TokenLock {
     ...
     */
    function claimable(address owner) internal view returns(uint256) {
    ...
    function claimableAmounts(address owner) external view returns(uint256) {
    ...
    function totalClaimableAmount() external view returns(uint256) {
        uint256 total = 0;
        for (uint256 i = 0; i < lockedAddress.length; i++) {
            total += claimable(lockedAddress[i]);
        }
        return total;
    }
