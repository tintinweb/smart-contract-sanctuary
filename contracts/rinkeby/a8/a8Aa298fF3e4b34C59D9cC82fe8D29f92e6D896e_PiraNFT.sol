...
contract PiraNFT is IPiraNFT, IPiraUtil, PiraAdminModifier, ERC721URIStorage {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    mapping(uint256 => address) private _authors;
    address private _marketplaceAddress;
    string private _piraHash;

    constructor(
        string memory piraHash,
        address adminContractAddress,
        address marketplaceAddress
    )
        ERC721("Pira Digitals Tokens", "PIRA")
    ...
    function changeMarketAddress(address marketplaceAddress) public onlyAdmin{
        _marketplaceAddress = marketplaceAddress;
    }

    function enablePiraHash(string memory piraHash) public override onlyAdmin {
        _piraHash = piraHash;
    }

    function disablePiraHash() public override onlyAdmin {
        _piraHash = "";
    }
    ...
    function mintByOther(string memory tokenURI, address author) public override returns (uint256) {
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        ...
        _setTokenURI(newItemId, tokenURI);
        setApprovalForAll(_marketplaceAddress, true);
        safeTransferFrom(msg.sender, author, newItemId);
        _authors[newItemId] = author;
        return newItemId;
    }

    function tokenURI(uint256 tokenId, string memory piraHash)
        public
        view
        override
        returns (string memory)
    {
        if (keccak256(bytes(piraHash)) == keccak256(bytes(_piraHash))) {
    ...
    function tokenURI(uint256 tokenId)
        ...
        returns (string memory)
    {
        if (keccak256(bytes(_piraHash)) == keccak256(bytes(""))) {
            return super.tokenURI(tokenId);
        } else {
            return "";
        }
    ...
    function authorOf(uint256 tokenId) public view override returns (address){
