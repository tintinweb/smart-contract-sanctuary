...
contract PlayersOnlyNFTMinter is Ownable {
    using Strings for string;
    using SafeMath for uint256;
    ...
    function publicMint(address _toAddress, uint256 amount) external payable {
        require(public_minting, "Public minting isn't allowed yet");
        require(counter + amount <= MAX_SUPPLY, "Purchase would exceed max supply");
        require(price * amount <= msg.value, "Ether value sent is not correct");

        uint256 currentMints = publicSale[_toAddress];
        require(currentMints + amount <= 3);

        PlayersOnlyNFT playersOnlyNFT = PlayersOnlyNFT(nftAddress);
    ...
    function whitelistMint(address _toAddress, uint256 amount) external payable {
    ...
    function dogeWhitelistMint(address _toAddress, uint256 amount) external payable {
    ...
    function ownerMint(address _toAddress, uint256 amount) external onlyOwner {
        require(counter + amount <= 100, "Purchase would exceed max supply for owner");
