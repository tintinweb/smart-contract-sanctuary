...
contract AllowanceModule is SignatureDecoder, Ownable {
    ...
    function getInfo(address safe, uint48 start, uint8 pageSize) external view returns (Info[] memory) {

        Info[] memory info;
        uint counter;
        (address[] memory allDelegates, ) = getDelegates(safe, start, pageSize);
        for (uint i = 0; i < allDelegates.length; i++) {
            ...
            for (uint j = 0; j < allTokens.length; j++) {
                ...
                info[counter].allowance = getAllowance(safe, allDelegates[i], allTokens[j]);
                counter++;
            }
        }
        return info;
