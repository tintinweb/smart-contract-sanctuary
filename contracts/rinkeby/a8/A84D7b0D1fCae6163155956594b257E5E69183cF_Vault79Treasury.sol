...
interface IPaxgBondDepository {
    function assetPrice() external pure returns (int);
}
...
contract Vault79Treasury is Ownable {
    ...
    function allocatedRate() public view returns ( uint ) {
        ...
        uint paxgReserve = uint( IPaxgBondDepository( reserveDepositors[0] ).assetPrice() )
            .mul( valueOf( reserveTokens[ 0 ], IERC20( reserveTokens[ 0 ] ).balanceOf( address(this) ) ) )/ 1e8;
        reserves = reserves.add( paxgReserve );
