...
contract ScrewUpActorAppearanceExtension is ScrewUpInGameActorExtension,IScrewUpActorAppearanceExtension {
    ...
    string constant CONST_ENUM = "enum";
    string constant CONST_RANGE = "range";
    string constant CONST_VEC4 = "vector4";
    ...
    function defineParts(string[] memory partNames, string[] memory  valueTypes) external onlyOwner returns(uint256[] memory) {
        ...
        for(uint256 i = 0; i < partNames.length; i++){
            ...
            _part_valuetypes[newPartId] = _getValueTypeByTypename(valueTypes[i]);
    ...
    function updatePartNames(uint256[] memory partIds,string[] memory partNames) external onlyOwner {
        ...
        for(uint256 i = 0; i < partIds.length; i++){
            if(_part_defined[partIds[i]]){
                ...
                emit OnPartDefinitionUpdated(partIds[i],partNames[i],uint8(valueType),_getValueTypeText(valueType));
            }
        }
    }
    ...
    function getPartDefinition(uint256 _partId) external view virtual override returns (FAppearancePart memory){
        return FAppearancePart(_partId,_part_valuetypes[_partId],_part_valueranges[_partId],_part_names[_partId],_part_valueenums[_partId]);
    }
    ...
    function _getValueTypeByTypename(string memory valueType) internal pure returns (EValueType) {
        if(_compareStrings(valueType,CONST_ENUM))
            return EValueType.Enum;
        if(_compareStrings(valueType,CONST_RANGE))
            return EValueType.Range;
        if(_compareStrings(valueType,CONST_VEC4))
           return EValueType.Vector4;
        return EValueType.None;
    }
    function _getValueTypeText(EValueType valueType) internal pure returns (string memory) {
        if(valueType == EValueType.Enum)
            return CONST_ENUM;
        if(valueType == EValueType.Range)
            return CONST_RANGE;
        if(valueType == EValueType.Vector4)
            return CONST_VEC4;
        return "";
    }
}
