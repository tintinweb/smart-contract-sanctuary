...
import "./HeroInfo.sol";
import "../main/ERC721Tradable.sol";

contract AbbeyOfHeroes is ERC721Tradable, ReentrancyGuard {

    string private _contractURI;
    address private _admin;
    HeroInfo private heroInfo;
    ...
    constructor(address _proxyRegistryAddress, address _heroInfo)
        ERC721Tradable(
            "Abbey of Heroes (for Loot) - DnD complete characters", "AHLoot",
            _proxyRegistryAddress
        )
    {
        _admin = _msgSender();
        heroInfo = HeroInfo(_heroInfo);
    }
    ...
    {
        return heroInfo.build(_tokenId);
    }
    ...
    //
    //     string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "', getName(tokenId), '", "external_url": ', getCharacterSheet(tokenId), '"description": "This special character for D&D role-play game is handmade, balanced and stored on chain. Skills, equipment, and other functionality are intentionally specified so you will not be bothered creating ones. Feel free to use this complete D&D characters in any way you want.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    function claim(uint256 tokenId) public nonReentrant {
        require(tokenId >= 0 && tokenId < 7778, "invalidId");
        _safeMint(_msgSender(), tokenId);
    }

    function adminClaim(uint256 tokenId) public nonReentrant onlyAdmin {
        require(tokenId > 7777 && tokenId < 8001, "invalidId");
...
contract ERC721Tradable is ERC721Enumerable, Ownable {
     ...
     */
    function mintTo(address _to) public onlyOwner returns(uint256 newTokenId) {
...
contract HeroInfo is Ownable {
        ...
        returns (string memory)
    {
        ...
        bytes memory _shortDescription = abi.encodePacked(getShortDescription(tokenId));
    ...
    function makeJson(
        ...
        string memory json = Base64.encode(
            abi.encodePacked(
                ...
                '", "description": "', _shortDescription, '\nThis special character for D&D role-play game is handmade, balanced and stored on chain. Skills, equipment, and other functionality are intentionally specified so you will not be bothered creating ones. Feel free to play this complete D&D character!", "image": "data:image/svg+xml;base64,',
                Base64.encode(_svg),
                '", "attributes": [',
                _attrs,
                "]}"
        ...
        returns (bytes memory)
    {
        ...
        return
            abi.encodePacked(
                head,
                abi.encodePacked(
                    '<text x="8" y="335" class="tag">',
                    _name,
                    "</text>",
                    '<text x="8" y="359" class="tag">AH: ',
                    _tokenId,
                    "</text>"
                ),
                loots,
