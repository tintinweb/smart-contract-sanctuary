...
contract OrderManagerReal is OrderManager {
    ...
    function safeTransfer(address _tokenContract, address _to, uint256 _amount) external
    {
        SafeERC20.safeTransferFrom(IERC20(_tokenContract), address(this), _to, _amount);
    }
            ...
            if (data.orderSellList.length > 0)
            {
                for (uint i=data.orderSellList.length-1; i>=0; i--)
                {
                    ...
                    for (uint j=orderList.length-1; j>=0; j--)
                    {
                        ...
                        this.safeTransfer(USDT, matchOrder.sender, data.matchMap[matchId].usdtDeal);
                    ...
                    if (amount > 0)
                    {
                        this.safeTransfer(_tokenContract, order.sender, amount);
                    }
                }
            }
        }
            ...
            if (data.orderBuyList.length > 0)
            {
                for (uint i=data.orderBuyList.length-1; i>=0; i--)
                {
                    ...
                    for (uint j=orderList.length-1; j>=0; j--)
                    {
                        ...
                        this.safeTransfer(USDT, matchOrder.sender, data.matchMap[matchId].tokenDeal);
                ...
                if (amount > 0)
                {
                    this.safeTransfer(USDT, order.sender, amount);
                }
            }
        }
    }
