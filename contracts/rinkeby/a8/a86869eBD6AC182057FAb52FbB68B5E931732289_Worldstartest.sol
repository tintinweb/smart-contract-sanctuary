...
contract Worldstartest is ERC721, Ownable {
  ...
  string public hiddenTokenURI = "https://gateway.mypinata.cloud/ipfs/Qma9htfc4u7QzA4VMkVqEHANFCasMAhdZUaqdjMCXKWR6L";
  ...
  function mintWithMintPass(uint mintPassId) external payable onlyEOA {
    require(presaleOpen , "PreSale not started yet");
    require(totalSupply() + 1 <= maxSupply - (reservedAmount - reservedMintedAlready), "max supply reached");
    require(mintPass.ownerOf(mintPassId) == msg.sender, "mintpass Id not owned");
    require(!mintPassIdHasMinted[mintPassId], "mintpass id already minted");
    require(msg.value == salePrice, "sent incorrect price");
    mintPassIdHasMinted[mintPassId] = true;
