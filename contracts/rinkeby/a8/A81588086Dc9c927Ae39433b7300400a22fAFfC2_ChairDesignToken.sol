...
/**
 * @title ChairDesignToken
 * @author ChairXYZ
 *
 *  An ERC20 that grants access to the ENS namespace through a
 *  burn-and-register model.
 */
contract ChairDesignToken is IChairDesignToken {
    using SafeMath for uint256;

    // ============ Immutable ERC20 Attributes ============

    string public constant override symbol = "Chair";
    string public constant override name = "ChairDesignToken";
    ...
    modifier canRegister() {
        require(registrable, "ChairDesignToken: registration is closed.");
        _;
    }

    modifier onlyOwner() {
        require(isOwner(), "ChairDesignToken: caller is not the owner.");
        _;
    }
    ...
    modifier onlyNextOwner() {
        require(
            isNextOwner(),
            "ChairDesignToken: current owner must set caller as next owner."
        );
        _;
    }
     ...
     * @param label The user's ENS label, e.g. "dev" for dev.Chair.xyz.
        ...
        canRegister
    {
        ...
        IChairENSRegistrar(ensRegistrar).register(label, owner);
    }
    ...
    function transferOwnership(address nextOwner_) external onlyOwner {
        require(
            ...
            "ChairDesignToken: next owner is the zero address."
        );
    ...
    function permit(
        ...
        require(deadline >= block.timestamp, "ChairDesignToken: EXPIRED");
        bytes32 digest =
            keccak256(
                abi.encodePacked(
                    "\x19\x01",
        ...
        require(
            ...
            "ChairDesignToken: INVALID_SIGNATURE"
        );
