...
contract CatFishFarm is IERC721Receiver, Ownable {
    using EnumerableSet for EnumerableSet.UintSet;

    

    address public CATFISH;
    ...
    constructor(
        address catfish_erc20_token_address,
        address zombiecats_erc721_address,
        uint256 expiration
    ) {
        CATFISH = catfish_erc20_token_address; 
    ...
    function claimRewards(uint256[] calldata tokenIds) public {
        ...
        if (reward > 0) {
            try IERC20(CATFISH).transfer(msg.sender, reward) returns (bool v) {
