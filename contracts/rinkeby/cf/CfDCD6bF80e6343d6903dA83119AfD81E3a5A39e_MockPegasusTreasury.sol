...
contract MockPegasusTreasury is Ownable {
  using SafeMath for uint256;
  ...
  // address public PEGA;
  // uint256 public sFUELQueue; // Delays change to sFUEL address
  ...
  constructor(
    ...
    // isLiquidityToken[ _FUELDAI ] = true;
    // liquidityTokens.push( _FUELDAI );
  ...
  function withdraw(uint256 _amount, address _token) external {
    ...
    uint256 value = valueOfToken(_token, _amount);
    IFUELERC20(PEGA).burnFrom(msg.sender, value);
  ...
  //   uint256 maximumDebt = IERC20(sFUEL).balanceOf(msg.sender); // Can only borrow against sFUEL held
  ...
  //       @param _amount uint
  //    */
  // function repayDebtWithFUEL(uint256 _amount) external {
  //   require(isDebtor[msg.sender], "Not approved");
  ...
  function mintRewards(address _recipient, uint256 _amount) external {
    ...
    IERC20Mintable(PEGA).mint(_recipient, _amount); // no
  ...
  function excessReserves() public view returns (uint256) {
    return
      totalReserves.sub(IERC20(PEGA).totalSupply().sub(totalDebt)).sub(
        1 * 10**9
      ); // change later each pegapal backed
  }
    ...
    returns (uint256 value_)
  {
    if (isReserveToken[_token]) {
      ...
      value_ = _amount.mul(10**IERC20(PEGA).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.sFUEL) {
      // 9
      // sFUELQueue = block.number.add(blocksNeededForQueue);
    } else return false;
