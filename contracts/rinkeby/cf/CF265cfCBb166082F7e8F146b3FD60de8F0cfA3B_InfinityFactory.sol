...
contract InfinityFactory is Ownable {
  ...
  function updateCosts(uint256 newTeamBPS) public onlyOwner {
    teamBPS = newTeamBPS;
  }
...
contract ExternalAuthSales is OwnableClone, RevenueSplit, SalesHistory {
  ...
  function purchase(
    ...
    // solhint-disable-next-line not-rely-on-time
    uint256 currentTime = block.timestamp;
    require(expiration >= currentTime, 'Bearer token expired');
    ...
    require(
      listing.authorizers[signer],
      string(abi.encodePacked('Signer is not authorized to sell this listing: ', Strings.toHexString(uint160(signer))))
    );
    ...
    postSale(listing.tokenAddress, listing.offeringId, msg.sender, _recipient, price, currentTime);
