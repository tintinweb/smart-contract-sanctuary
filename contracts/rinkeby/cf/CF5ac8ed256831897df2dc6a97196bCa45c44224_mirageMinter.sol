...
contract mirageMinter {
  ...
  function purchase(uint256 _projectId) public payable returns(uint256 _tokenId){
    if (keccak256(abi.encodePacked(mirageContract.projectIdToCurrencySymbol(_projectId))) != keccak256(abi.encodePacked("ETH"))){
      ...
      require(ERC20(mirageContract.projectIdToCurrencyAddress(_projectId)).balanceOf(msg.sender) >= mirageContract.projectIdToPricePerTokenInWei(_projectId), "Insufficient balance.");
    ...
    uint256 tokenId = mirageContract.mint(msg.sender, _projectId, msg.sender);
    return tokenId;
  }
  
  function earlyPurchase(uint256 _projectId, uint256 _membershipId) public payable returns(uint256 _tokenId){
    if (keccak256(abi.encodePacked(mirageContract.projectIdToCurrencySymbol(_projectId))) != keccak256(abi.encodePacked("ETH"))){
      require(msg.value==0, "this project accepts a different currency and cannot accept ETH");
      require(ERC20(mirageContract.projectIdToCurrencyAddress(_projectId)).allowance(msg.sender, address(this)) >= mirageContract.projectIdToPricePerTokenInWei(_projectId), "Insufficient Funds Approved for TX");
      require(ERC20(mirageContract.projectIdToCurrencyAddress(_projectId)).balanceOf(msg.sender) >= mirageContract.projectIdToPricePerTokenInWei(_projectId), "Insufficient balance.");
      require((membershipContract.balanceOf(msg.sender,50) > 0) || (membershipContract.balanceOf(msg.sender,_membershipId) == 1), 'No membership tokens in this wallet');
      _splitFundsERC20(_projectId);
    } else {
      require(msg.value>=mirageContract.projectIdToPricePerTokenInWei(_projectId), "Must send minimum value to mint!");
    ...
    uint256 tokenId = mirageContract.earlyMint(msg.sender, _projectId, msg.sender);
    return tokenId;
  }

  function _splitFundsETH(uint256 _projectId) internal {
    if (msg.value > 0) {
      uint256 pricePerTokenInWei = mirageContract.projectIdToPricePerTokenInWei(_projectId);
      uint256 refund = msg.value.sub(mirageContract.projectIdToPricePerTokenInWei(_projectId));
      if (refund > 0) {
        msg.sender.transfer(refund);
      }
      uint256 foundationAmount = pricePerTokenInWei.div(100).mul(mirageContract.miragePercentage());
