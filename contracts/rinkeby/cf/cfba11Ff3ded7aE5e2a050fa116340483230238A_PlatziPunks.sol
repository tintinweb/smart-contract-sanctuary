...
contract PlatziPunks is ERC721, ERC721Enumerable, Ownable {
    ...
    function mint() public {
        ...
        _tokenDNA[current] = deterministicPseudoRandom(current, msg.sender);
        _idCounter.increment();
        _safeMint(msg.sender, current);
    }

    // This pseudo random function is deterministic and should not be used in production
    function deterministicPseudoRandom(uint256 _tokenId, address _minter)
        public
        pure
        returns (uint256)
    {
        ...
        returns (uint8)
    {
        ...
        return
            uint8(
                (_dna % (1 * 10**(_rightDiscard + sectionSize))) /
                    (1 * 10**_rightDiscard)
            );
    }
    ...
    function buildDNAImageData(uint256 _dna)
        public
        view
        returns (string memory)
    {
        ...
        returns (string memory)
    {
        ...
        string memory imageData = buildDNAImageData(dna);
