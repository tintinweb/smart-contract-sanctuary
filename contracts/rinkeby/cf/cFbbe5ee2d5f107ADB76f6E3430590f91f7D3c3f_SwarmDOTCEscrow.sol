...
contract SwarmDOTCEscrow is IEscrow, AccessControl {
        ...
        returns (bool status)
    {
        ...
        uint256 _amount = IdOTC(dOTC).getTakerOrders(orderId).amountToSend;
        ...
        Withdraw(offerId, orderId, _receiver, _amount);
        return true;
    }
    ...
    */
        ...
        returns (bool status)
    {
        ...
        emit offerFrozen(offerId, deposits[offerId].maker, _amount, _account);
        return true;
    }
    ...
    event offerFrozen(uint256 offerId, address offerOwner, uint256 amountReverted, address frozenBy);
    event EscrowFrozen(address frozenBy, address calledBy);
    event Withdraw(uint256 offerId, uint256 orderId, address taker, uint256 amount);
}
