...
contract MonsterbudsV2 is MonsterBudHolders, ERC721URIStorageUpgradeable, IMonsterBuds {
    ...
    mapping (address => breedInfomation) breedInfo;
    ...
    */
    ...
    function hybreedCollectiable( uint256 req_token_id,uint256 accept_token_id, uint256 breed_req_id) external override payable returns (uint256) {
        ...
        require(breedInfo[msg.sender].breedCount <= 2 && breedInfo[accepter_token_address].breedCount <= 2,"$MONSTERBUDS: Exceeds max breeds");
       ...
       // uint countOfReq = breedInfo[msg.sender].breedCount;
       // uint countOfAccept = breedInfo[accepter_token_address].breedCount;

        tokenCounter = tokenCounter + 1;
        breedInfomation storage data = breedInfo[msg.sender];
        data.user = msg.sender;
        data.breedCount = breedInfo[msg.sender].breedCount + 1;
        data.timstamp = block.timestamp + 900;

        breedInfomation storage data1 = breedInfo[accepter_token_address];
        data1.user = accepter_token_address;
        data1.breedCount = breedInfo[accepter_token_address].breedCount + 1;
        data1.timstamp = block.timestamp + 900;
    ...
    */
    ...
    function selfBreedCollectiable(uint256 req_token_id, uint256 accept_token_id) external override payable returns (uint256) {
        ...
        require(owner_req == owner_accept && owner_req == msg.sender, "$MONSTERBUDS: Cannot Self Breed");
        require(breedValue == msg.value && breedInfo[msg.sender].breedCount <= 2, "$MONSTERBUDS: Amount is incorrect OR exceeds max breed count");
        ...
        breedInfomation storage data = breedInfo[msg.sender];
        data.user = msg.sender;
        data.breedCount = countOfReq + 1;
        data.timstamp = block.timestamp + 900;
        //breedCount[msg.sender] = breedCount[msg.sender] + 1;
    ...
    */
    ...
    function freeMint() external override returns(uint256) {
        ...
        emit FreeTokenDetails(msg.sender, _uri, newItemId);
