...
contract DHFillerArtNFT is ERC721Enumerable, Ownable {
    ...
    uint256 public constant PRESALE_MAX_PURCHASABLE = 1;
    ...
    // Base URI
    string private _baseURIextended = "ipfs://QmXWterRf7TsZsbbFvRUmYyw42fW7fyAjd21yoRXePrvnv/";
    ...
    constructor() ERC721("DHFillerArt", "DHFillerArt") {
    }
    ...
    function mint(uint256 amountToMint) public payable {
        ...
        require(_totalSupply < TOTAL_SUPPLY, "All arts have been minted.");
        require(amountToMint > 0, "Minimum mint is 1 art.");
        // require(amountToMint <= mintLimit, string(abi.encodePacked("Maximum mint is ", mintLimit.toString(), " arts.")));
        require(balance.add(amountToMint) <= mintLimit, string(abi.encodePacked("Maximum mint is ", mintLimit.toString(), " arts. You have already minted ", balance.toString(), " arts.")));

        require(_totalSupply.add(amountToMint) <= TOTAL_SUPPLY, "The amount of arts you are trying to mint exceeds the max supply.");

        if (_totalSupply >= FREE_SUPPLY) {
            require(MINT_PRICE.mul(amountToMint) == msg.value, "Incorrect Ether value.");
        } else {
            require(amountToMint == 1, "You can mint 1 art per transaction during free minting.");
        }

        _mint(amountToMint);
    }

function freeMint(uint256 amountToMint) public {
        require(totalSupply() < FREE_SUPPLY, "Free sale finished");
        require(amountToMint == 1, "You can mint 1 art per transaction during free minting.");
    ...
    function ownerMint(uint256 amountToMint) public onlyOwner {
        uint256 _totalSupply = totalSupply();
        require(_totalSupply < TOTAL_SUPPLY, "All arts have been minted.");
        require(_totalSupply + amountToMint <= TOTAL_SUPPLY, "The amount of arts you are trying to mint exceeds the max supply.");
