...
contract H24 is ERC20, AccessControl {
    ...
    mapping(uint24 => uint) rewards; // date => amount

    uint24 lastClaimed; // последний день, за который была сделана выплата любому клиенту
    ...
    /// @notice Unstake some H24 tokens
    /// @param amount Amount of tokens to unstake
    function unstake(uint232 amount) public {
        require(miners[msg.sender].stake >= amount, "The amount is bigger than your stake");
    ...
    /// @notice Check if address can unstake tokens
    /// @param addr Staker address
    function canUnstake(address addr) public view returns (bool) {
        require(miners[addr].stake != 0, "You need to stake first");
    ...
    /// @notice Get amount of staked tokens
    /// @param addr Staker address
    ...
    /// @notice Claim reward for staking tokens
    function claim() public {
        uint reward = getUserReward(msg.sender);
        lastClaimed = today();
        miners[msg.sender].date = today() + 1;
        WBTCI(WbtcContract).transferFrom(WbtcBank, msg.sender, reward);
    }

    /// @notice Check if address can claim reward
    /// @param addr Staker address
    function canClaim(address addr) public view returns (bool) {
        uint reward = getUserReward(addr);
//        require(reward > 10, "Reward too low");  // todo не пускать пыль?
        require(reward <= WBTCI(WbtcContract).balanceOf(WbtcBank), "We are out of WBTC");
        require(reward <= WBTCI(WbtcContract).allowance(WbtcBank, address(this)), "We are out of WBTC");
        return true;
    }

    /// @notice Check how much wbtc user will receive
    /// @param addr Staker address
    function getUserReward(address addr) public view returns (uint){
        Miner memory miner = miners[addr];
        require(miner.stake != 0, "You need to stake first");
        require(rewards[today()] != 0, "No reward for today");
        require(miner.date <= today(), "You can't claim yet");

        return miner.stake * (rewards[today()] - rewards[miner.date - 1]);
    }

    /// @notice Set WBTC reward for day
    /// @param date Day
    /// @param amount Amount. This is the sum of all rewards from day 0 to the current one.
    function setReward(uint24 date, uint amount) public onlyRole(ORACLE_ROLE) {
        require(date > lastClaimed, "There were already payments on this day");
        require(amount <= 1e6, "Reward too big");
        require(amount > rewards[date - 1], "Reward can't be negative");
        rewards[date] = amount;
    }

    /// @notice Get WBTC reward for day
    /// @param date Day
    function getReward(uint24 date) public view returns (uint){
        return rewards[date];
    }

    /// @notice Set WBTC bank address
    /// @param addr Address with WBTC tokens
    function setWbtcBank(address addr) public onlyRole(DEFAULT_ADMIN_ROLE) {
        WbtcBank = addr;
    }


    /// @notice Mint H24 tokens
