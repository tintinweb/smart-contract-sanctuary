...
contract DrawA is ERC721, ERC721Enumerable {
    struct Character {
        uint256 blocknumber;
        address creator;
        string name;
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    "' y2='",
                    uint2str(y2),
                    "' stroke-linecap='round' stroke-width='2' stroke='#FFFFFF'></line>"
                )
            );
    }
        ...
        returns (string memory)
    {
            ...
            paths = string(
                abi.encodePacked(
                    ...
                    makePath(
                        c.x,
                        c.y,
                        random(600, 950, _seed + index),
                        random(50, 600, _seed + index),
                        _seed + index
                    )
                )
            );
        ...
        paths = string(
            abi.encodePacked(
                paths,
                makeLine(0, 10, 1000, 10),
                makeLine(990, 0, 990, 1000),
                makeLine(0, 990, 1000, 990),
                makeLine(10, 0, 10, 1000),
                //makeLine(0, 840, 1000, 840),
                makeText(20, 950, c.name)
            )
        );
        return
            string(
                abi.encodePacked(
                    "<svg xmlns='http://www.w3.org/2000/svg' version='1.1' xmlns:xlink='http://www.w3.org/1999/xlink' width='1000' height='1000' viewBox='0 0 1000 1000'>",
    ...
    function freeMint(string memory name) external {
        ...
        require(supply <= 10000, "!max reached");
        ...
        uint256 leafs = random(1, 10, supply);
    ...
    function mint(uint256 qty, string[] memory names) public payable {
        require(isOpen, "!start");
        require(tx.gasprice <= 120 gwei, "!gas");
        require(qty == names.length, "pick names");
    ...
    function _beforeTokenTransfer(
        ...
        require(infos[tokenId].isFree == false || from == address(0x0));
