    ...
    ReentrancyGuardUpgradeSafe
{
    ...
    function getInitiatedWithdrawal(address account) 
        external 
        view 
        returns (uint256, uint256) 
    {
        return(
            _withdrawStates[account].amount,
            _withdrawStates[account].initiateDate + unstakingPeriod *  dayLength
        );
    }
...
contract MockSwapStakingContract2 is  SwapStakingContract {    
    ...
    constructor  (address _token, address _rewardsAddress, uint256 _maxStakingAmount, uint256 _unstakingPeriod) public {
        // _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        // _setupRole(PAUSER_ROLE, _msgSender());
        // _setupRole(OWNER_ROLE, _msgSender());
        // _setupRole(REWARDS_DISTRIBUTOR_ROLE, _msgSender());
