...
contract KommunitasProject{
    ...
    function getUserAllocToken(address _user) internal view returns(uint userAllocToken){
        if(boosterProgress() == 1){
            ...
            require(sale.sub(booster[1].achieve) > 0, "You are out of token sale");
            userAllocToken = getUserAllocation_d8(_user).mul(sale.sub(booster[1].achieve)).div(10 ** 8);
    ...
    function setBuyer(address _user) internal returns(uint buyerId){
        if(!isBuyer(_user)){
            ...
            if(userRecipient.length == 0){
                recipient[_user] = KommunitasLibrary.toAsciiString(_user);
            }
        }else{
