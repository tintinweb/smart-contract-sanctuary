...
contract HashBridge is Ownable {
	...
	function getOrdersForArbitration(address arbitratorAddress) external view returns(bytes[] memory response) {
	    uint k = 0;
	    for (uint i = 0; i < orders.length; i++) {
			if (orders[i].paid == false || orders[i].complete == true || orders[i].declined == true) {
			    continue;
			}
			bool addToResponse = true;
		    ...
		    if (addToResponse) {
		        response[k] = abi.encodePacked(i, orders[i].payAmount, offers[orders[i].offerId].payToken, orders[i].payAddress);
		        k++;
		    }
		}
	}
