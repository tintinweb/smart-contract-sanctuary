...
contract Collection is Ownable, ReentrancyGuard {
    ...
    function buyPacks(uint256 _packId, uint256 _quantity) external payable mintLock {
        require (packsInCollection[_packId], "Pack not in collection");
        (,uint cost,,,) = dropper.getPack(_packId);
        cost = cost  * _quantity;
        // Pay with $DROPP instead of ether
        // if (msg.value == 0){}
    ...
    function tokenURI(uint256 _tokenId)
        external view returns (string memory)
    {
        return dropper.tokenURI(_tokenId);
    }
    ...
    function getSubId(uint256 _momentId, uint256 _rarityId, uint256 _seed) internal returns (uint256) {
        ...
        (string memory uri, uint256 totalMinted, uint256 mintRange) = dropper.getMoment(_momentId);
        ...
        dropper.setMoment(uri, (totalMinted + 1), mintRange, _momentId);
        return subId;
    }
...
//Use Oracle to charge based on fixed USD
//import "@chainlink/contracts/src/v0.8/interfaces/FeedRegistryInterface.sol";
//import "@chainlink/contracts/src/v0.8/Denominations.sol";

// Dropper.deployed().then(function(instance) { d = instance })
...
/*
    function VRF() external onlyDropper returns (uint256){
        //This will provide random numbers when LINK is supported on Arbitrum
        // Only have to keep this storage contract stocked with LINK
    }

    function getPrice() external onlyDropper returns (uint256){
