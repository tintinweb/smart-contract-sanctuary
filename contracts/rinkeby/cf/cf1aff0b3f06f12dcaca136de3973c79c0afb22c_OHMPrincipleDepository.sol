...
contract OHMPrincipleDepository is IPrincipleDepository, Ownable {
    ...
    external view override returns ( uint _principleValue, uint _payoutRemaining, 
    uint _lastBlock, uint _vestingPeriod ) {
    ...
    function _depositBondPrinciple( uint amountToDeposit_ ) internal returns ( bool ){
        ...
        ITreasury( treasury ).depositPrinciple( amountToDeposit_ );
    ...
    function redeemBond() external override returns ( bool ) {
        ...
        uint payout_ = payoutRemaining_.mul( percentVested_ ).div( 1000 );
        IERC20( OHM ).safeTransfer( msg.sender, payout_ );

        uint principleUsed_ = principleValue_.mul( percentVested_ ).div( 1000 );
    ...
    function _calculatePercentVested( address depositor_ ) internal view returns ( uint _percentVested ) {
        ...
        if ( vestingPeriod_ > 0 ) {
            _percentVested = blocksSinceLast_.mul( 1000 ).div( vestingPeriod_ );
        } else {
            _percentVested = 0;
        }
    }
    ...
    function calculatePendingPayout( address depositor_ ) external view override returns ( uint _pendingPayout ) {
        ...
        _pendingPayout = payoutRemaining_.mul( percentVested_ ).div( 1000 );
