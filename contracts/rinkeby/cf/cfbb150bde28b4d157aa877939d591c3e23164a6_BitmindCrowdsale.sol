...
contract BitmindCrowdsale {
  ...
  /**
    ...
    function getPrice() public view returns(uint256){
        ...
        uint256 div_time;
        if(timeRanged <= 1 days){
            div_time = 1 hours;
        }else if(timeRanged <= 1 hours){
            div_time = 1 minutes;
        }else{
            div_time = 1 days;
        }
        uint256 multiplier_days = timeRanged.div(div_time).div(multiplier);
        uint256 phase = multiplier_days.mul(div_time);
        uint256 acc;
        ...
        for(uint256 i = 1; i <= multiplier; i++){
            if(block.timestamp >= start_time.add(phase.mul(i.sub(1))) && block.timestamp <= start_time.add(phase.mul(i))){
                acc = price.add(rate.mul(i.sub(1)));
            }
        }
        return acc;
    }
