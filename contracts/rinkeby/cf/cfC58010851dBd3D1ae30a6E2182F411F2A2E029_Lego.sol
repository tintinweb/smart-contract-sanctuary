...
contract Lego is Ownable, IUniswapV2Callee {
	...
	function execBatch(bytes memory _legos, uint amount) public {
		_executeBatch(_legos, amount);
	}

	function execBatchWithLoan(bytes memory _legos, address pairAddr, uint amount0, uint amount1) public {
    ...
    function uniswapV2Call(address _sender, uint _amount0, uint, bytes calldata _data) external override {
