...
contract HypeBeasts is HypeBeastCoreERC721, ReentrancyGuard, VRFConsumerBase {
  ...
  mapping(uint256 => HypeBeast) hypeBeasts;
  ...
  function tokenDna(uint256 _tokenId) public view returns (uint256) {
    require(_exists(_tokenId), "Token not yet minted");
    require(hypeBeasts[_tokenId].dna != 0, "Token dna not yet generated");

    return hypeBeasts[_tokenId].dna;
  }
  ...
  function traitMetadata(uint256 _tokenId) public view returns (string memory) {
    ...
    uint256[] memory expandedDnaNumbers = expandDna(hypeBeasts[_tokenId].dna, traitCategoryCount);
  ...
  function fulfillRandomness(bytes32 _requestId, uint256 _randomNumber) internal override {
    hypeBeasts[VRFRequests[_requestId].id].dna = _randomNumber;

    HypeBanners(bannerContractAddress).fulfillRandomness(VRFRequests[_requestId].id, _randomNumber);

    emit UpdateHypeBeast(VRFRequests[_requestId].id, _randomNumber);
  }
