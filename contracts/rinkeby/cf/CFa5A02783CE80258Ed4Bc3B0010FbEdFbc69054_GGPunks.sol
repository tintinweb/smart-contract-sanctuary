...
contract GGPunksDNA {
    ...
    // This pseudo random function is determiniistic and should not be used on production
...
contract GGPunks is ERC721, ERC721Enumerable, GGPunksDNA {
    ...
    constructor(uint256 _maxSupply) ERC721("GGPunks", "GGPKS") {
        maxSupply = _maxSupply;
    }

    function mint() public {
        uint256 current = _idCounter.current();
        require(current < maxSupply, "No GGPunks left :(");
        ...
        returns (string memory)
    {
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "GGPunks #',
                tokenId.toString(),
                '", "description": "GG Punks are randomized Avataaars stored on chain to teach DApp development", "image": "',
                image,
                '"}'
            )
        );
