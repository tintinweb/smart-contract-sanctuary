...
contract RewardsGenerator is IRewardsGenerator, OwnableUpgradeable, AbstractDependant {
    ...
    function hasToMigrate(address staker) external view returns (bool) {
        IBMICoverStaking bmiCoverStaking = IBMICoverStaking(bmiCoverStakingAddress);

        uint256 nftsCount = bmiCoverStaking.balanceOf(staker);

        for (uint256 i = 0; i < nftsCount; i++) {
            uint256 nftIndex = bmiCoverStaking.tokenOfOwnerByIndex(staker, i);

            if (_stakes[nftIndex].stakeBlock > 0) {
                return true;
            }
        }
    ...
    function migrate() external {
        ...
        uint256 nftsCount = bmiCoverStaking.balanceOf(_msgSender());

        for (uint256 i = 0; i < nftsCount; i++) {
            uint256 nftIndex = bmiCoverStaking.tokenOfOwnerByIndex(_msgSender(), i);
            ...
            address policyBookAddress =
                bmiCoverStaking.stakingInfoByToken(nftIndex).policyBookAddress;
