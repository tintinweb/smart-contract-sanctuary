...
library UniswapV2Library {
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex'ac1b4340bbfa3ae8c3ffc689181fa7f1f46f16967e67a594ce518606947ca827' // init code hash of factory
                    )
                )
            )
        );
    ...
    function getAmountIn(
        ...
        require(reserveIn > 0 && reserveOut > 0, 'UniswapV2Library: INSUFFICIENT_LIQUIDITY');
        uint256 numerator = reserveIn.mul(amountOut).mul(10000);// uint256 numerator = reserveIn.mul(amountOut).mul(1000);
        uint256 denominator = reserveOut.sub(amountOut).mul(9984);// uint256 denominator = reserveOut.sub(amountOut).mul(998);
