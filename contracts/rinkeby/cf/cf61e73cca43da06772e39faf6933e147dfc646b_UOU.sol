...
contract UOU is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
	...
	uint256 public _whiteListMintPrice = 0.03 ether;
    ...
    /*constructor*/
    constructor(string memory name, string memory symbol, uint256 maxTokenSupply) ERC721(name, symbol) {
        _maxTokenSupply = maxTokenSupply;
    }
    
    function getMessageHash(address wallet) public pure returns (bytes32){
	    return keccak256(abi.encode(wallet));	
    }
	
    function verifyWallet(address wallet, bytes memory _signature) public pure returns (address){
		bytes32 messageHash = getMessageHash(wallet);
	...
	/*public call*/
    function mint(uint256 numberOfTokens, bytes memory _signature) public payable {
        ...
        address signerOwner = verifyWallet(wallet, _signature);

		if(signerOwner == owner())
			require(_whiteListMintPrice * numberOfTokens <= msg.value, "Ether value sent is not correct");
		else
