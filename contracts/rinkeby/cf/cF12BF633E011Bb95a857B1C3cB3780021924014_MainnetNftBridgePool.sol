...
contract MainnetNftBridgePool is IERC721Receiver, IERC1155Receiver, AccessControl, ReentrancyGuard {
  ...
  mapping(uint256 => bool) public cardPackClassDisabled;  
  ...
  function setCardPackDisabledClasses(uint256[] calldata _ids, bool[] calldata _disabled) 
    external onlyOwner {
      for (uint256 i; i < _ids.length; i++) {
        cardPackClassDisabled[_ids[i]] = _disabled[i];
      }
  }
  ...
  function depositCardPacks(address _recipient, uint256[] calldata _tokenIds) 
    external payable nonReentrant {
      ...
      for (uint256 i; i < _tokenIds.length; i++) {
        uint256 packClassId = IRainiCardPack(cardPackToken).getPackClass(_tokenIds[i]); 
        require(!cardPackClassDisabled[packClassId], "MainnetNftBridgePool: bridging this class disabled");               
        IRainiCardPack(cardPackToken).safeTransferFrom(_msgSender(), address(this), _tokenIds[i]);
        requestId++;
        emit CardPackDeposited(_msgSender(), _recipient, packClassId, requestId);
      }
  ...
  function withdrawNftV1(address _recipient, uint256 _requestsId, uint256 _cardId, uint256 _cardLevel, uint256 _amount, bytes1 _mintedContractChar, uint256 _number) 
    external onlyMinter {
      IRainiCard(nftV1Token).mint(_recipient, _cardId, _cardLevel, _amount, _mintedContractChar, _number);
