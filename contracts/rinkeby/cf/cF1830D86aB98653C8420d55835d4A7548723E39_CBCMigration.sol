...
contract CBCMigration is Ownable {
	 ...
	 * @param nonce unique number for each migrations
	...
	function migrate(
		...
		require(verifyContract.isValidRewardData(msg.sender, amount, nonce, signature) == true, "Transaction is not verified");

		uint totalBalance = oldCBC.balanceOf(msg.sender);
		require(totalBalance >= amount, "User doesn't have enough balance on old CBC contract");

		oldCBC.transferFrom(msg.sender, burnWallet, amount);
