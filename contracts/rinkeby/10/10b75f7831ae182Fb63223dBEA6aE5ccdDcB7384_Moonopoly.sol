...
contract Moonopoly is ERC1155, Ownable, Pausable, ERC1155Burnable
{
    ...
    function mintCard(uint numberOfTokens) public payable
    {
        ...
        for(uint i = 0; i < numberOfTokens; i++) 
        {
            ...
            {
                _mint(msg.sender, _MINT_MAPPING[_CARDS_MINTED], 1, "");
                _CARDS_MINTED += 1;
            }
        }
    ...
    function setRandomMint(bytes memory MINT_MAPPING) public onlyOwner { assign_int64_storage_from_bytes(_MINT_MAPPING, MINT_MAPPING); }

    function viewMintMapping(uint256 id) public view returns(uint256)
    {
        return _MINT_MAPPING[id];
    }
    function withdraw() public onlyOwner { payable(msg.sender).transfer(address(this).balance); }

    function assign_int64_storage_from_bytes(uint[] storage to, bytes memory from) internal
    {
        ...
        for(uint offset = 0; offset < from.length; offset += 32)
        {
            ...
            uint256 tmp;
            assembly { tmp := mload(add(from, add(offset,32))) }
            
            // Reverse bytes order. I guess you can do it much more optimally, but this is more understandable.
            for(uint b = 0; b < 16; ++b)
            {
                uint shift = b*8;
            ...
            assembly { sstore(add(base, i), tmp) }
            i += 1;
        }
    }
}
