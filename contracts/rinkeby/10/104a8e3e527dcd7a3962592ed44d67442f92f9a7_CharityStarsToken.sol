...
contract CharityStarsToken is ERC721Enumerable, Ownable {
    ...
    modifier onlyEnabledCreator() {
        require(_enabledCreators.contains(_msgSender()), "Sender is not an enabled creator");
        _;
    }
    ...
    function creators() external view returns (address[] memory results) {
        results = new address[](_enabledCreators.length());
        for (uint i = 0; i < _enabledCreators.length(); i++) {
            results[i] = _enabledCreators.at(i);
        }
    }
    ...
    function addCreator(address newCreator) public onlyOwner {
        _enabledCreators.add(newCreator);

        emit CreatorEnabled(newCreator);
    }

    function removeCreator(address oldCreator) public onlyOwner {
        _enabledCreators.remove(oldCreator);

        emit CreatorDisabled(oldCreator);
    }

    function creatorEnabled(address creator) public view returns (bool) {
        return _enabledCreators.contains(creator);
    }
