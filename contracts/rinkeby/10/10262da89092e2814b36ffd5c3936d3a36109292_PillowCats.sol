...
contract PillowCats is ERC721URIStorage, Ownable, ReentrancyGuard{
    using Strings for uint256;
    ...
    bool public TogglePublicSale = true;
    bool public ToggleWhitelist = true;
    bool public revealed = false;

    uint256 immutable public PC_MAXSUPPLY = 6000;
    uint256 immutable public PC_PUBLIC = 5800;
    uint256 public constant PC_PER_MINT = 6;

    uint256 public constant WHITELIST_SALE_PRICE = 0.06 ether;
    uint256 public constant PUBLIC_SALE_PRICE = 0.06 ether;
    uint256 public giftMintId = 5801;
    ...
    constructor() ERC721("Phillygang", "PC") {
        setHiddenMetadataUri("ipfs://QmVjMVpBcNJyfn2tuv7Qgi8DzwWFTLfbyXBu7fw6jdcUmR/hidden.json");
    }
    ...
    modifier canMint(uint256 numberOfTokens) {
        require(
            MintId + numberOfTokens -1  <= PC_PUBLIC,
    ...
    function mintGift(
        ...
        nonReentrant
    {
      require(giftMintId + numberOfTokens -1<= PC_MAXSUPPLY);
    ...
    function mintWhitelist(
      ...
      uint256 numberOfTokens
        ...
        nonReentrant
    {
        require(numberOfTokens <= PC_PER_MINT, "EXCEED PC_PER_MINT");
        require(!ToggleWhitelist, "Contract is paused");
        require(!claimed[msg.sender], "NFT is already claimed by this wallet");
        for (uint256 i = 0; i < numberOfTokens; i++) {
    ...
    {
        require(numberOfTokens <= PC_PER_MINT, "EXCEED PC_PER_MINT");
        require(!TogglePublicSale, "Contract is paused");
         for (uint256 i = 0; i < numberOfTokens; i++) {
            _mint(msg.sender, MintId);
            MintId++;
        }
    ...
    function setPausePublicSale(bool _state) public onlyOwner {
    TogglePublicSale = _state;
    }

    function setPauseWhitelist(bool _state) public onlyOwner {
    ToggleWhitelist = _state;
    }
