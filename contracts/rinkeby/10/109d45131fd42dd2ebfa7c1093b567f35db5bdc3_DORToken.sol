...
contract DORToken is ERC721Enumerable, ReentrancyGuard, Pausable, WhitelistAdminRole {
    ...
    enum TokenType {
        NONE,
        SILVER, // 1. mix & match silver grade
        GOLD, // 2. mix & match gold grade
        RAINBOW, // 3. mix & match rainbow grade
        EBIBLE, // 4. daughter ebible
        RANDOM, // 5. silver/gold/rainbow random
        M3D, // 6. 3D mother
        AIRDROPS, // 7. presale airdrops
        DAUGHTER, // 8. daughters
        ALIEN, // 9. daughters of alien
        RARE, // 10. rare daughters
        MOTHER // 11. the mother
    }
    ...
    uint256 ebibleIndexMax = rareIndexMax + 360;
    // mother index
    uint256 nextMotherIndex = ebibleIndexMax + 1;
    uint256 motherIndexMin = ebibleIndexMax + 1;
    uint256 motherIndexMax = ebibleIndexMax + 30;
    // random index (silver)
    uint256 nextRandomIndex = motherIndexMax + 1;
    uint256 randomIndexMin = motherIndexMax + 1;
    uint256 randomIndexMax = motherIndexMax + 150;
    // m3d index
    uint256 nextM3dIndex = randomIndexMax + 1;
    uint256 m3dIndexMin = randomIndexMax + 1;
    uint256 m3dIndexMax = randomIndexMax + 30;
    // airdrop index
    uint256 nextAirdropsIndex = m3dIndexMax + 1;
    uint256 airdropsIndexMin = m3dIndexMax + 1;
    uint256 airdropsIndexMax = m3dIndexMax + 50;
    ////////////////////////////////////////////////////
    // nft index
    ////////////////////////////////////////////////////
    ...
    mapping (address => uint256) public mintCounts; // limit for pre-sale

    bool public ebibleClaimEnabled; // ebible claim switch
    bool public m3dClaimEnabled;  // m3d claim switch
    bool public randomClaimEnabled; // random claim switch
    ...
    function setRandomEnabled(bool flag) public onlyWhitelistAdmin {
        randomClaimEnabled = flag;
    }
    ...
    function getNextRandomId() internal returns (uint256 id) {
        id = nextRandomIndex;
        nextRandomIndex = nextRandomIndex.add(1);
    }
    ...
    function randomLeftover() public view returns (uint256) {
        if (nextRandomIndex > randomIndexMax) {
            return 0;
        }
        return randomIndexMax.sub(nextRandomIndex).add(1);
    }

    function randomCurrentCount() public view returns (uint256) {
        return nextRandomIndex.sub(randomIndexMin);
    }
    ...
    function setClaimEbibleFlag(address account, bool flag) internal {
    ...
    function claimSGR(uint256 tp) public {
        ...
        require(daughterCount >= cnt, "can't claim");
        // set claim flag
    ...
    function _mintRandom(address account, uint256 num) internal {
        require(nextRandomIndex.add(num) <= randomIndexMax.add(1), "num limited");
        for (uint256 i; i < num; i++) {
            // get new token id
            uint256 tokenId = getNextRandomId();
            // mint and set token type
            _safeMint(account, tokenId);
            setType(tokenId, uint256(TokenType.RANDOM));
        }
    }
    ...
    function mint(uint256 num) public payable nonReentrant WhenNotPaused {
        if (whitelistEnabled) { // check whitelist
            ...
            require(mintCounts[_msgSender()].add(num) <= 2, "num limited");
            mintCounts[_msgSender()] =  mintCounts[_msgSender()].add(num);
        }
        // validate
    ...
    function canClaimSGR(address owner) public view returns (bool silver, bool gold, bool rainbow) {
        ...
        if (daughterCount >= 20 && rainbowLeftover() > 0) {
            rainbow = true;
        } else if (daughterCount >= 10 && goldLeftover() > 0) {
            gold = true;
        } else if (daughterCount >= 5 && silverLeftover() > 0) {
            silver = true;
        }
    }
    ...
    function canClaim(address account) public view
        returns
        (bool silver, bool gold, bool rainbow, bool ebible, bool random, bool m3d, bool airdrops) {
        // check silver/gold/rainbow
    ...
    function canClaimRandom(address account) public view returns (bool) {
        if (!randomClaimEnabled) {
            return false;
        }
        ...
        uint256 cnt;
        for (uint256 i = 0; i < tokenTypes.length; i++) {
            if (tokenTypes[i] == uint256(TokenType.RARE) && !getClaimFlag(tokenIds[i])) {
                cnt = cnt.add(1);
            }
        }

        if (cnt >= 3 && randomLeftover() > 0) {
            return true;
        }
        return false;
    }
    ...
    function claimRandom() public {
        address account = _msgSender();
        require(randomClaimEnabled, "not start");
        ...
        require(cnt >= 3, "can't claim random");
    ...
    function claimAirdrops() public {
        ...
        // set claim flags
        setClaimEbibleFlag(account, true);
    ...
    function claim(uint256 tp) public {
        if (tp == uint256(TokenType.SILVER) || tp == uint256(TokenType.GOLD) || tp == uint256(TokenType.RAINBOW)) {
            claimSGR(tp);
        } else if (tp == uint256(TokenType.RANDOM)) {
            claimRandom();
