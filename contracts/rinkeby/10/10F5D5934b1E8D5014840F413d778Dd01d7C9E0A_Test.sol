...
contract Test is Datetime{
    ...
    function computStakingProfit(uint _time)  public view returns (uint profit){
        ...
        uint computYear = uint(20210911)/uint(10000);
        uint computMonth = uint(20210911)/uint(100)%100;
        uint num;
        uint monthSecord;
        uint computEndTime = 20210911;
        for(uint8 i=0;i<=passMonths;i++){
            if(computMonth >12){
                computYear  = computYear + 1;
                computMonth = 1;
            }
            num = 1000000000000000000000*signRatePath[3][computYear*100+computMonth]*1;
    ...
    function text3() public view returns(uint,uint,uint){
        uint num = 1000000000000000000000*signRatePath[3][2021*100+9]*1;
        num = num*dayLen*60*60*24/getDaysInMonth(9,2021);
        num = num/decimals/decimals;
        uint monthSecord = getDaysInMonth(9,2021) - 20210911%100;
        monthSecord = monthSecord*60*60*24/dayLen;
        return (num*monthSecord,num,monthSecord);
    }
    function text5() public view returns(uint,uint,uint){
        uint num = 1000000000000000000000*signRatePath[3][2021*100+10]*1;
