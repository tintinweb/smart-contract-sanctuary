...
contract TomiStaking is Ownable, ReentrancyGuard, Pausable {
    ...
    constructor(address _rewardsDistribution, address _tomiToken, address _tomiPlatform, address _tomiGovernance) {
    ...
    function getRewardRevenueShare(address account)
        ...
        returns (uint256)
    {
        if ((userInfo.length == 0) ||  (_revenueShared == 0)) {
            return 0;
        }

        uint256 sumOfTotal = 0;
        uint256 totalTimeStakedOfUser;

        for (uint256 i = 0; i < userInfo.length; i++) {
            totalTimeStakedOfUser = block.timestamp.sub(_lastStakedTime[userInfo[i]]);
            sumOfTotal = sumOfTotal.add(
                _balances[userInfo[i]].mul(totalTimeStakedOfUser)
            );
        }
    ...
    function updateRevenueShare(uint256 revenueShared) external PlatformAndGovernance {
    ...
    modifier PlatformAndGovernance() {
        require((msg.sender == tomiPlatform) || (msg.sender == tomiGovernance), "TomiStaking: caller is not TomiPlatform");
        _;
    }
