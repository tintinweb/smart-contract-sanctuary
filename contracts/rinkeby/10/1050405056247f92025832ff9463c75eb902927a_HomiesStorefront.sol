...
contract HomiesStorefront is Pausable, Ownable, PaymentSplitter {
    ...
    uint16 _currentPresaleWave = 0;
    string _baseURIValue;
    PresaleWave[] _presaleWaves;

    mapping(address => bool) _hasBoughtPresale;
    ...
    constructor(
        ...
        _presaleStart = _saleStart - 6 * 60 * 60;
    ...
    function hasBoughtPresale(address addr) external view returns(bool) {
        return _hasBoughtPresale[addr];
    }
    ...
    function addPresaleWave(uint16 mintLimit, uint16 totalLimit, bytes32 merkleRoot)
        external
        onlyOwner
    {
    ...
    function startNextWave() external onlyOwner {
        _currentPresaleWave += 1;
    }
    ...
    function mintPresale(uint16 numberOfTokens, bytes32[] calldata merkleProof, uint256 presaleWave)
        external
        payable
        whenNotPaused
    {
        require(presaleHasStarted(), "PRESALE_NOT_STARTED");
        require(_currentPresaleWave >= presaleWave, "PRESALE_WAVE_NOT_STARTED");

        PresaleWave memory wave = _presaleWaves[presaleWave];

        require(
            !_hasBoughtPresale[_msgSender()],
            "PRESALE_ALREADY_PURCHASED"
        );

        require(
            numberOfTokens + token.totalSupply() <= _presaleWaves[_currentPresaleWave].totalLimit,
            "MAX_FOR_PRESALE_WAVE_EXCEEDED"
        );

        require(
            numberOfTokens <= wave.mintLimit,
        ...
        require(
            MerkleProof.verify(merkleProof, wave.merkleRoot, keccak256(abi.encodePacked(_msgSender()))),
            "INVALID_MERKLE_PROOF"
        );

        _hasBoughtPresale[_msgSender()] = true;
        token.mintTokens(numberOfTokens, _msgSender());
    }
}
