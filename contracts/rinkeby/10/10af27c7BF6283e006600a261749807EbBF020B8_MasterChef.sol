...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accQentryPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accQentryPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accQentryPerShare;   // Accumulated SH13LD per share, times 1e12. See below.
    ...
    IERC20 public usdc;
    address public usdcQentryLP = address(0x0); // set after listing.
    ...
    uint256 public QentryPerBlock = 1 ether;
    // Maximum emission rate
    uint256 public maxEmissionRate = 1 ether;
    // Bonus muliplier for early Qentry makers.
    ...
    constructor(
        ...
        address _burnaddr,
        address _feeAddress,
        uint256 _QentryPerBlock,
        uint256 _startBlock
    ) {
        qentry = _qent;
        burnaddr = _burnaddr;
        feeAddress = _feeAddress;
        QentryPerBlock = _QentryPerBlock;
    ...
    function addPool(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, bool _withUpdate) public onlyOwner nonDuplicated(_lpToken) {
        require(_depositFeeBP <= 400, "add: Never more than 4% fees!");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // Update the given pool's qent allocation point and deposit fee. Can only be called by the owner.
    ...
    function updateEmissionIfNeeded() public {
        ...
        if (block.number - lastBlockUpdate > emissionUpdateInterval) {
            ...
            if (QentryBalance > 0) {
                ...
                priceCents = usdc.balanceOf(usdcQentryLP).mul(1e14).div(QentryBalance);
            }

            // Update pools before changing the emission rate
            massUpdatePools();
            uint256 emissionRatePercent = getEmissionRatePercent(priceCents);
            QentryPerBlock = maxEmissionRate.div(100).mul(emissionRatePercent);
        }
    }
    ...
    function getEmissionRatePercent(uint256 QentryPriceCents) public view returns (uint256 epr) {

        if (QentryPriceCents>=topPrice*100){return (1);}
        if (QentryPriceCents<=bottomPrice*100){return (100);}

        uint256 QentryPricePercentOfTop = QentryPriceCents.mul(100).div(topPrice*100);
        int256 curveCoefficient = FixidityLib.reciprocal(int256(curveRate));
        uint256 f1 = uint256(100).sub(QentryPricePercentOfTop);
        int256 p1 = FixidityLib.fromFixed(FixidityLib.multiply(curveCoefficient,int256(f1)));
        int256 p2 = FixidityLib.fromFixed(FixidityLib.divide(100,int256(QentryPricePercentOfTop)));
    ...
    function updateEmissionRate(uint256 _QentryPerBlock) public onlyOwner {
        massUpdatePools();
        QentryPerBlock = _QentryPerBlock;
        emit UpdateEmissionRate(msg.sender, _QentryPerBlock);
    }
    ...
    // For checking prices to link emission rate
    function setUsdcQentryLPAddress(address _address) public onlyOwner {
        usdcQentryLP = _address;
    }
