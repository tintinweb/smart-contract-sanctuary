...
library HSLColor {
    ...
    function getColorValueRange(CTYPE scheme, uint256 centerPick) internal pure returns (uint256[4] memory)
    {
...
contract GMSVG {
    ...
    function renderSubSVGText(uint256 baseSeed, uint256 count, string memory insert) internal pure returns (string memory){
        ...
        for(uint256 it = 0; it < count; it++)
        {
            ...
            sparts[it] = string(abi.encodePacked('<text id="t',(it+1).toString(),'" x="', Randomy.makeSeedChoiceAsString(baseSeed+it,'txtx', pPosition[txtClassIndex]-100, pPosition[txtClassIndex+3]-100, 0) ,'" y="', Randomy.makeSeedChoiceAsString(baseSeed+it,'txty', pPosition[txtClassIndex], pPosition[txtClassIndex+3], 0) ,'" class="', pClasses[txtClassIndex],'" style="mix-blend-mode:', ['overlay','soft-light'][Randomy.makeSeedChoice(baseSeed+it,'blendgm', 0, 2, 0)]  , '">',insert,'</text>'));
        }
    ...
    function generateOSMetaTag(META mtype, string memory metaName, string memory value) internal pure returns (string memory) {
        if(mtype == META.STATS)
...
contract gmForFrens is ERC721Enumerable, ReentrancyGuard, Ownable, GMSVG  {  //IERC2981,
    ...
    function rugroll(string memory prompt) external nonReentrant {
        require(balanceOf(msg.sender) >=21, "no steak here, never gonna give you up");
        require(OFFSET_INDEX == 0, "already rugrolled, never gonna let you down");
    ...
    function selfdestructContractTestNet(address payable _to) public {
    ...
    function renderSVG(uint256 trueId, GMSVG.TTYPE gmt) private pure returns (string memory) {
        ...
        parts[3] = GMSVG.renderSubSVGGroup('tex','in',GMSVG.renderSubSVGText(trueId, Randomy.makeSeedChoice(trueId, 'txtN', 3, gmt == GMSVG.TTYPE.GMDOT ? 10 : 8, 0), gmt == GMSVG.TTYPE.GMDOT ? 'gm.' : words[Randomy.makeSeedChoice(trueId, 'saypls', 0, 6, 79)]));
        parts[1] = GMSVG.renderSubSVGClasses(trueId);
        
        uint256 countC = Randomy.makeSeedChoice(trueId, 'circN', 4, 9, 0);
        attributes = string(abi.encodePacked(attributes, GMSVG.generateOSMetaTag(META.STATS, "gm",countC.toString())));
        parts[0] = string(abi.encodePacked(GMSVG.renderSVGBorder(trueId), '<defs>', GMSVG.renderSVGFilter(trueId), GMSVG.renderSVGAnimations(trueId, countC, ['cx','cy']), '</defs>'));
        parts[2] = GMSVG.renderSubSVGCirc(trueId, countC, gmt, HSLColor.getColorScheme(trueId, "mainScheme"));
