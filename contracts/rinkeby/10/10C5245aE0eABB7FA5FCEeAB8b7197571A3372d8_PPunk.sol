...
contract PPunk is ERC721Enumerable, Ownable {
    ...
    uint256 private publicMintIndex = 250;
     ...
     */
    function addToWhitelist(address[] calldata wallets, uint256 whiteListNum) public onlyOwner  {
     ...
     */
    function removeFromWhitelist(address[] calldata wallets, uint256 whiteListNum)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < wallets.length; i++) {
     ...
     */
    function isWhitelisted(address wallet, uint256 whiteListNum) external view returns (bool) {
        if (whiteListNum == 2) {
    ...
    function mintCHWhiteList(uint256 numberOfTokens) public {
        ...
        require(this.isWhitelisted(msg.sender, 1),"PPunk: You don't hold any NFTs from previous collection");
    ...
    function mintWhiteList(uint256 numberOfTokens)  external payable {
        ...
        require(this.isWhitelisted(msg.sender, 2),"PPunk: You don't hold any NFTs from previous collection");

        require(
            numberOfTokens <= 4,
            "PPunk: Can only mint 4 tokens at a time"
        );
