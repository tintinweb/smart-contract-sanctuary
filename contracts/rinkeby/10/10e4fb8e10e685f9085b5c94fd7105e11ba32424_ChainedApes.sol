...
contract ChainedApes is ERC721Enumerable {
 ...
 |_|  |_|_|_||_\__|_|_||_\__, | |_| \_,_|_||_\__|\__|_\___/_||_/__/
   ...
   */
    ...
    function updateRewards(address _wallet) internal {
        rewards[_wallet] = rewards[_wallet] + (((block.timestamp - lastTime[_wallet]) * (RATE * addressBalance[_wallet])));
    }
    
    function viewClaimableRewards(address _address) external view returns(uint256){
        uint256 time = block.timestamp;
        uint256 pending = ((addressBalance[_address] * RATE * (time - lastTime[_address])));
        return ((rewards[_address] + pending)/19);
    }
    
    function returnRewardsCurrent(address _wallet) external view returns(uint256){
     ...
     */
    function mintApes(uint256 _times) public payable {
