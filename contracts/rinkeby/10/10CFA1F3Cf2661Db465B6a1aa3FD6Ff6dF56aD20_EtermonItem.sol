...
// File: contracts/EtermonProxySupported/ERC721.sol
...
// File: contracts/EtermonProxySupported/Etermon.sol
...
contract Etermon is ERC721, MinterRole {
    ...
    function mintBatchSameToken(
        address to,
        uint256[] memory tokenId
    ) external onlyMinter {
        for (uint256 i; i < tokenId.length; i++) {
            mint(to, tokenId[i]);
            require(_exists(tokenId[i]));
        }
    }

    function mintBatchDifferentToken(
        address[] memory to,
        uint256[] memory tokenId
    ) external onlyMinter {
        require(
            to.length == tokenId.length,
            'ERC721: Arrays length missmatch'
        );
...
// File: contracts/EtermonProxySupported/EtermonItem/EtermonItem.sol
...
contract EtermonItem is Etermon {
    ...
    function itemInitailize(address mintFactory_) external  {
        require(!lock);
        mintFactory = mintFactory_;
        _addMinter(mintFactory_);
        _addMinter(owner());
        etermonInitailize("EtermonItem","ETI");
        lock = true;
    }
}