...
/**
 *  SourceUnit: c:\Users\HP\Desktop\SafeKeep\contracts\SafeKeep.sol
 */
...
contract SafeKeep is Ownable, ReentrancyGuard {
    ...
    struct Vault {
        ...
        mapping(address => mapping(address => uint256)) inheritorTokenAllocations;
    ...
    mapping(address => mapping(uint256 => bool)) inheritorActiveVaults;
    mapping(address => uint256[]) userVaults;
    mapping(address => uint256) ownerVault;
    ...
    function checkBackupAddressAndPing(uint256 _vaultId)
        public
        view
        vaultExists(_vaultId)
        returns (address _backup, uint256 _p)
    {
        Vault storage v = vaultDefaultIndex[_vaultId];
        _backup = v.backup;
        _p = v._lastPing;
    ...
    **/
        ...
        returns (uint256 currentEtherBalance)
    {
        ...
        _ping(_vaultId);
        emit EthDeposited(_vaultId, _amount);
        ...
        returns (address[] memory, uint256[] memory)
    {
        ...
        _ping(_id);
        emit tokensDeposited(_id, tokenDeps, _amounts);
    ...
    function allocateTokens(
        ...
        _ping(_vaultId);
        emit tokenAllocated(_vaultId, tokenAdd, _inheritors, _shares);
    ...
    function allocateEther(
        ...
        _ping(_vaultId);
        emit EthAllocated(_vaultId, _inheritors, _ethShares);
    ...
    function findUintIndex(uint256 _item, uint256[] memory noArray)
        internal
        pure
        returns (uint256 i)
    {
    ...
    function claim(uint256 _vaultId) external nonReentrant {
        ...
        if (v._inheritorWeishares[msg.sender] > 0) {
            ...
            emit ClaimedEth(_vaultId, msg.sender, _toClaim);
        }
