...
/// @title Dice
/// @notice This contract mints Dice NFTs that can be called by anyone.
...
/// @notice there are known limitations of pRNG utilising onchain information, which could be subject to manipulation.
...
contract Dice is ERC721Enumerable, ReentrancyGuard {
    ...
    function rollDice(uint256 _tokenId) public nonReentrant payable {
        ...
        uint256 _diceRoll = (random() % 6) + 1;
        uint256 _rollCount = diceRolls[_tokenId].rollCount + 1;

        roll.rollCount = _rollCount;
        roll.rollValue = _diceRoll;

        emit DiceRolled(_tokenId, _diceRoll, block.timestamp);
    }
