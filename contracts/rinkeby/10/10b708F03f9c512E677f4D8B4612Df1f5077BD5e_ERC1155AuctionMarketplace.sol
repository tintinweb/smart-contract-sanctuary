 ...
 *         Users are able to create auctions for ERC1155 tokens
 ...
 */
contract ERC1155AuctionMarketplace is ERC1155Holder, OnlyNonContractsOrApproved {
  ...
  /**
   * @notice Creates an auction for an ERC1155 token
   *
   ...
   * @param _tokenQty the quantity of the token to be auctioned
   * @param _endsAt when the auction ends
   */
  function createAuction(address _tokenAddress, uint256 _tokenId, uint256 _tokenQty, uint128 _endsAt) external onlyNonContractsOrApproved returns (bytes32) {
    // require(_endsAt >= block.timestamp + 1 days, "ends too early");

    bytes32 id = keccak256(abi.encodePacked(_tokenAddress, _tokenId, _tokenQty, tx.origin, _endsAt, block.timestamp));
   ...
   * @param _id the id of the auction
   */
  function claimAuction(bytes32 _id) external onlyIfAuctionEnded(_id) onlyNonContractsOrApproved {
    ...
    // function to complete.
    if(auction.bidder != address(0) && !auction.bidder.isContract()) {
      IERC1155(auction.tokenAddress).safeTransferFrom(address(this), auction.bidder, auction.tokenId, auction.tokenQty, "");
    }
