...
/**
 * @title Chain Estate Air Drop
 * @dev Contract responsible for the Chain Estate air drop mechanism
 */
contract ChainEstateAirDrop is Ownable {
    ...
    * Sends 10% of user's current balance + more depending how long they have been holding their CHES.
    ...
    function claimAirDrop() public {
        ...
        string memory canClaimRewardsIn = Strings.toString((userTimeStamp + minimumInvestTime - currTimeStamp) / 60 / 60 / 24);
        require(userTimeStamp <= currTimeStamp - minimumInvestTime, string(abi.encodePacked("Too many of your recent investments were made to close to the air drop time. You can claim your rewards in ", canClaimRewardsIn, " days. The air drop will still be open then.")));

        // Initial percentage gained from claiming air drop.
        uint256 initialAmount = CHES.balanceOf(claimAddress) / airDropInitialPercent;
