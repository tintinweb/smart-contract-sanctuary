...
contract PolicyBookFacade is IPolicyBookFacade, AbstractDependant, Initializable {
    ...
    function deployLeverageFundsAfterRebalance(
        ...
        if (leveragePool == ILeveragePortfolio.LeveragePortfolio.USERLEVERAGEPOOL) {
            LUuserLeveragePool[msg.sender] = isLeverage
                ? LUuserLeveragePool[msg.sender].add(deployedAmount)
                : LUuserLeveragePool[msg.sender].sub(deployedAmount);
        } else {
    ...
    function deployVirtualFundsAfterRebalance(uint256 deployedAmount, bool isLeverage)
        external
        override
        onlyLeveragePortfolio
    {
        ...
        VUreinsurnacePool = isLeverage
            ? VUreinsurnacePool.add(deployedAmount)
            : VUreinsurnacePool.sub(deployedAmount);

        emit DeployLeverageFunds(deployedAmount, true);
    }
    ...
    function _deployLeveragedFunds() internal {
        ...
        if (reinsurancePoolMPL > 0) {
            (_isLeverage, _deployedAmount) = reinsurancePool.deployVirtualStableToCoveragePools();

            if (_isLeverage) {
                VUreinsurnacePool = VUreinsurnacePool.add(_deployedAmount);
            } else {
                if (VUreinsurnacePool >= _deployedAmount) {
                    VUreinsurnacePool = VUreinsurnacePool.sub(_deployedAmount);
                }
            }
            ...
            if (_isLeverage) {
                LUreinsurnacePool = LUreinsurnacePool.add(_deployedAmount);
            } else {
                if (LUreinsurnacePool >= _deployedAmount) {
                    LUreinsurnacePool = LUreinsurnacePool.sub(_deployedAmount);
                }
            }
        }
        if (userleveragedMPL > 0) {
            ...
            for (uint256 i = 0; i < _userLeverageArr.length; i++) {
                (_isLeverage, _deployedAmount) = ILeveragePortfolio(_userLeverageArr[i])
                    .deployLeverageStableToCoveragePools(
                    ILeveragePortfolio.LeveragePortfolio.USERLEVERAGEPOOL
                );
                LUuserLeveragePool[_userLeverageArr[i]] = _isLeverage
                    ? LUuserLeveragePool[_userLeverageArr[i]].add(_deployedAmount)
                    : LUuserLeveragePool[_userLeverageArr[i]].sub(_deployedAmount);
        ...
        uint256 _totalLeveragedLiquidity =
            VUreinsurnacePool.add(LUreinsurnacePool).add(_LUuserLeveragePool);
...
interface IPolicyBookAdmin {
    ...
    function setUserLeverageRebalancingThreshold(
        address _LeveragePoolAddress,
        uint256 _newRebalancingThreshold
    ) external;

    function setUserLeveragesetProtocolConstant(
        address _LeveragePoolAddress,
        uint256 _targetUR,
        uint256 _d_ProtocolConstant,
        uint256 _a_ProtocolConstant,
