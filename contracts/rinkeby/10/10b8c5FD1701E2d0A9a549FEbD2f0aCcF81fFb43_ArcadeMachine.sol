...
contract ArcadeMachine is DisplayURISwitchable, ERC721Enumerable, ReentrancyGuard, ERC2981, Ownable {
    ...
    constructor(address arcadeOfficialAddress) ERC721("AMac03", "AMAC03") {
        ...
        onlyOperator
    {
        ...
        require(
            imageURIs.length == modes.length
            && arcadeTokenIds.length == imageURIs.length
        ...
        for(uint256 i = 0; i < arcadeTokenIds.length; i++) {
            ...
            _devMint(targetAddress, tokenId, tokenId, imageURIs[i], modes[i]);
        }
        ...
        onlyOperator
    {
        ...
        for(uint256 i = 0; i < arcadeTokenIds.length; i++) {
            ...
            _devMint(targetAddress, tokenId, arcadeTokenId, imageURIs[i], modes[i]);
            _setReceiver(tokenId, arcade.ownerOf(arcadeTokenId));
            _setRoyaltyPercentage(tokenId, ROYALTY_PCT);
        }
