...
contract Superchief is Pausable, Ownable, ERC721 {
    ...
    mapping(uint256 => ArtStructure) ArtData;
    ...
    event newCardTypeCount(uint256);
    event artSold(address, uint256);
    event mintNewCardEvent(uint256, uint256, bool);
    event setNewPrice(uint256, uint256);
    event setArtStatus(uint256, bool);
    event setNewBaseURI(string);
    ...
    function setController(address _controller) external onlyOwner {
        ...
        emit newController(_controller);
    }

    // Set Card Property
    function setRedeemable(uint256 _Id, bool _status) external onlyAllowed {
        ArtData[_Id].redeemable = _status;
        emit setArtStatus(_Id, _status);
    }

    function setPrice(uint256 _Id, uint256 _newPrice) external onlyAllowed {
        ArtData[_Id].price = _newPrice;
        emit setNewPrice(_Id, _newPrice);
    }
    ...
    // Preset
    function preset40Arts(
        uint256[] memory artIDs,
        uint256[] memory _prices,
        bool[] memory _redeemStatus
    ) external onlyAllowed {
        // Max is 40.
        require(artIDs.length == _prices.length, "art !=price length");
        require(artIDs.length == _redeemStatus.length, "art !=redeem length");
        require(artIDs.length == 40, "not 40");

        for (uint256 j = 0; j < 40; j++) {
            ArtData[artIDs[j]].price = _prices[j];
            ArtData[artIDs[j]].redeemable = _redeemStatus[j];
            emit setNewPrice(artIDs[j], _prices[j]);
            emit setArtStatus(artIDs[j], _redeemStatus[j]);
        }
    }

    // Entry
    function purchaseArt(uint256 _artID) public {
        require(is_sale_on(), "Sale is not running");
        uint256 price = getArtPrice(_artID);
        require(getArtPrice(_artID) > 0, "Art must have a price more than 0. ");
        require(
        ...
        // You can't safeMint the same ID twice. Will throw if _exists.
        _safeMint(msg.sender, _artID);
        emit artSold(msg.sender, _artID);
        totalSold++;
    }

    // Cards View
    function getArtPrice(uint256 _tokenID) public view returns (uint256) {
        return ArtData[_tokenID].price;
    }

    function getArtRedeemStatus(uint256 _tokenID) public view returns (bool) {
        if (_exists(_tokenID)) {
            return ArtData[_tokenID].redeemable;
        }
        return false;
    }
    ...
    // Time Sales
    function endSales() external onlyAllowed {
    ...
    function mintNewCard(
        ...
        require(!_exists(_Id), "ID exists ");
        ArtData[_Id].price = _price;
        ArtData[_Id].redeemable = _redeemable;
        emit mintNewCardEvent(_Id, _price, _redeemable);
    }
    ...
    function setDataFolder(string calldata _baseURI) external onlyAllowed {
        require(!_FuzeBlown, "This data can no longer be changed");
        _setBaseURI(_baseURI);
        emit setNewBaseURI(_baseURI);
    }
    ...
    {
        uint256 artPrice = getArtPrice(_tokenId);
        if (artPrice == 0) {
