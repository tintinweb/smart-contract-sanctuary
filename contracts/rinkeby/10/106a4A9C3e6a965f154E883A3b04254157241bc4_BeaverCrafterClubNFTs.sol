...
contract BeaverCrafterClubNFTs is ERC721Tradable {
    ...
    uint256 public earlySaleSupply;
    ...
    constructor(
        ...
        uint256 _maxSupply,
        uint256 _maxBalance,
        uint256 _earlySaleSupply,
        bytes32 _merkleRoot,
        uint256 _earlySaleStart,
        uint256 _earlySaleEnd
    ) ERC721Tradable("BeaverCrafterClub NFTs", "BCCNFTs", _proxyRegistryAddress) {
        price = _price;
        earlyPrice = _earlyPrice;
        maxSupply = _maxSupply;
        maxBalance = _maxBalance;
        earlySaleSupply = _earlySaleSupply;
        merkleRoot = _merkleRoot;
        earlySaleStart = _earlySaleStart;
        earlySaleEnd = _earlySaleEnd;
    ...
    function governorMint(address _to) public onlyOwner {
        tokenIdCount.increment();
        _mint(_to, tokenIdCount.current());
    }

    /// @notice Mints `tokenId` and transfers it to `to` during Pre-Mint.
    function governorMintMultiple(address _to, uint256 amount) external payable onlyOwner {
        for (uint256 i = 0; i < amount; i++) {
            tokenIdCount.increment();
            super._mint(_to, tokenIdCount.current());
        }
    }

    function isWhitelisted(uint256 index, bytes32[] calldata merkleProof) public view returns (bool) {
    ...
    function vipMint(
        ...
        require(this.totalSupply() < earlySaleSupply, "No More Early Sale token available");
        require(block.timestamp < earlySaleEnd, "Early Sale is closed");
        require(isWhitelisted(index, merkleProof), "Account is not whitelisted");
        for (uint256 i = 0; i < amount; i++) {
            tokenIdCount.increment();
            _mint(_to, tokenIdCount.current());
        }
    ...
    function publicMint(address _to, uint256 amount) external payable {
        require(msg.value >= price * amount, "Incorrect amount sent");
        require(block.timestamp > earlySaleEnd, "Public Minting not opened yet");
        require(balanceOf(_to) < maxBalance, "Account owns too many to mint");
        for (uint256 i = 0; i < amount; i++) {
            tokenIdCount.increment();
            _mint(_to, tokenIdCount.current());
        }
    ...
    // TODO: Differences btw 3 following functions ?
    ...
    function _mint(address _to, uint256 tokenId) internal override {
        require(totalSupply() < maxSupply, "Reached minting limit");
        require(!_to.isContract(), "No automatic minting allowed");
