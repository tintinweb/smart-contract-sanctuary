...
contract ERC721Token is Ownable, ERC721Enumerable, ERC721Burnable {
    ...
    constructor(string memory name_, string memory symbol_, address _mintSigner, string memory _tokenURIPrefix) ERC721(name_, symbol_) {
    ...
    function mintToken(uint256 _tokenId, uint8 sig_v, bytes32 sig_r, bytes32 sig_s) public {
        string memory msgPacked = keccak256(abi.encode(this, _msgSender(), _tokenId)).toString();
        require(msgPacked.recover(sig_v, sig_r, sig_s) == mintSigner, "incorrect mint signature");
        _safeMint(_msgSender(), _tokenId);
    }

    function mintSingleByOwner(uint256 _tokenId) public onlyOwner {
        _safeMint(_msgSender(), _tokenId);
    }

    function mintBatchByOwner(uint256[] memory _tokenIds) public onlyOwner {
