...
/// @title NFT Contract with AccessControl
/// @notice This Contract is used to mint tokens and manage minter and admin roles
/// @dev Inherits the ERC721 and AccessControl from OpenZeppelin
contract NFTMinter is ERC721, AccessControl {
    ...
    /// maps tokenId to token metadata
    mapping(uint256 => TokenExtraInfo) public TokenExtraInfos;

    /// @notice sets token name, symbol and baseUrI on deployment
    /// @dev set deployer as admin and deploys ERC721 token
    /// @param _name Token Name
    /// @param _symbol Token Symbol
    /// @param _baseURI TokenURI base
    constructor(
        string memory _name,
    ...
    ///@notice checks if caller has minter role
    modifier IsTokenMinter {
        require(
            this.hasRole(this.MINTER_ROLE(), msg.sender),
            "MarketPlace: Caller must have MINTER role"
        );
        _;
    }
    ...
    modifier IsAdmin {
        require(
            ...
            "MarketPlace: Caller must have ADMIN role"
        );
        _;
    }
    ...
    ///@notice grants minter role to account
    ///@param _account address to be granted role
    function addMinter(address _account) public IsAdmin {
        grantRole(MINTER_ROLE, _account);
    }

    ///@notice changes baseUri of NFT
    ///@param _baseURI baseUri of token
    ...
    ///@param _tokenDataURI Uri of the token
    ///@param _metaDataURI MetaData Uri of token
    function mintItem(
        address _owner,
        string memory _tokenDataURI,
        string memory _metaDataURI
    ) public IsTokenMinter returns (uint256) {
        _tokenIds.increment();

        TokenExtraInfos[_tokenIds.current()] = TokenExtraInfo({
            metaDataURI: _metaDataURI,
            metaDataHash: getTokenDataHash(_metaDataURI),
            minter: _owner
        });
    ...
    ///@dev gives keccak256 hash of given data
    ///@param _data string data whose hash is to be calculated
    ///@return hash of data
    function getTokenDataHash(string memory _data)
        internal
        pure
        returns (bytes32)
    {
