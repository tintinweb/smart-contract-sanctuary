    ...
    PausableUpgradeable
{
    ...
    function updateWhiteListPublic(address _address, bool _active)
        public
        whenNotPaused
        onlyOwner
    {
    ...
    function setWhiteListLoop(address[] memory addresses) public onlyOwner {
        for (uint256 index = 0; index < addresses.length; index++) {
            updateWhiteListPublic(addresses[index], true);
        }
    }
    ...
    function stake(
        ...
        if (whiteNFT[_nftAdr]) {
            ICryptoPunks(_nftAdr).transferPunk(address(this), _nftId);
        } else {
            require(
