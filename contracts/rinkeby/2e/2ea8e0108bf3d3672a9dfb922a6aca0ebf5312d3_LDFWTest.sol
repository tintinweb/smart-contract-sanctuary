...
contract LDFWTest is ERC721Enumerable, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    function internalMint(address recipient, uint256 qty) private {
        require((totalSupply() + qty) < MAX_SUPPLY + 1, "Requested quantity would exceed total supply");
    ...
    function setMetadataUrl(string calldata newUrl) external onlyOwnerOrAdmin {
        require(!_metadataLocked, "Metadata url is locked");
    ...
    function lockMetadata() external onlyOwner {
        require(!isActive, "Cannot lock the metadata while the contract is still active");
