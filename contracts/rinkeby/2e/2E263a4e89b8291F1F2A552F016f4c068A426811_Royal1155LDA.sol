    ...
    Lockable 
{
    ...
    event TierStatusChanged(uint128 indexed tierID, TierState indexed state);
    ...
    // (tierID) => minting status
    // mapping(uint128 => TierState) private _mintable;
    ...
    */ 
    ...
    function initialize() public initializer {
        __Lockable_init();
        __ERC1155_init("https://royal-io.mypinata.cloud/ipfs/QmSsLe4d86dEmLcAedbUPi9cCUR2LGSzaaoyro9vG6ZyNc/{id}.json");
        
        _contractMetadataURI = "https://royal-io.mypinata.cloud/ipfs/QmdUUfvmBKXnxupEcggq4e247KXSBn5nUZ56H6TFRNiHSS";

        // TODO: May need more initialization
    }
    ...
    function updateContractMetadataURI(string memory newURI) public controllerOnly {
...
enum TierState {
    UNKNOWN_VALUE, // Unintialized value
    DRAFT,
    MINTABLE,
    ACTIVE,
    CANCELED
...
/**
 @dev Adds the ability to restrict function calls to only be from the address that 
 originally deployed the contract via the `controllerOnly` modifier.
 */
abstract contract Controllable is Initializable {
    ...
    function __Controllable_init_unchained() internal initializer {
...
abstract contract Lockable is Controllable {
    function __Lockable_init() internal initializer {
        __Controllable_init();
        __Lockable_init_unchained();
    }

    function __Lockable_init_unchained() internal initializer {
        locked = false;
    }
    ...
    function lock() public virtual controllerOnly {
        locked = true;
    }
}
