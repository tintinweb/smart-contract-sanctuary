...
contract TokensSaleReserver is Ownable, Pausable {
    ...
    IRebelBots public _rebelBotsContract;

    uint _saleStartTs;
    uint _saleEndTs;

    uint256 public _tokenPrice = 320000000000000; // 0.00032 ETH

    uint256 public _plainRbHolderTokensLimit = 100000000000;
    uint256 public _majorRbHoldersTokensLimit = 150000000000;
    uint256 public _rbNumbersPriceThreshold = 5;

    mapping(address => uint) private reservedTokens;
    ...
    function getTokensLimitForRBHolder(address rbHolder) public view returns (uint256) {
        uint256 addressBalance = _rebelBotsContract.balanceOf(rbHolder);
        if (addressBalance < _rbNumbersPriceThreshold) {
            return _plainRbHolderTokensLimit;
        } else {
            return _majorRbHoldersTokensLimit;
        }
    }

    function getMaxTokensBuyLimit(address to) public view returns (uint256) {
        uint256 totalTokensLimit = getTokensLimitForRBHolder(to);
        uint256 reservedTokensNumber = reservedTokens[to];
