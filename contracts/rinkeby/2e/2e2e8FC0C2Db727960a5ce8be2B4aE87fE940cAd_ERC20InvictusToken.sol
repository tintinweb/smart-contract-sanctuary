...
/**
 * Contract for standard Invictus tokens.
 *
 */
contract ERC20InvictusToken is Initializable, ERC20PresetMinterPauserUpgradeable {
     ...
     */
    function requestWithdrawal(uint256 amountTokensToWithdraw) external whenNotPaused onlyWhitelisted {
        address participant = msg.sender;
        require(balanceOf(participant) >= amountTokensToWithdraw, 'Cannot withdraw more than balance held');
    ...
    function claimTokens(ERC20PresetMinterPauserUpgradeable token) external onlyOwner {
    ...
    function addLiquidity(uint256 amount) external {
        ERC20PresetMinterPauserUpgradeable erc20 = ERC20PresetMinterPauserUpgradeable(stableContract);
    ...
    function removeLiquidity(uint256 amount) external onlyOwner {
        ERC20PresetMinterPauserUpgradeable(stableContract).safeTransfer(msg.sender, amount);
    ...
    function processWithdrawals() internal {
        uint256 numberOfWithdrawals = MathUpgradeable.min(withdrawals.length, maxWithdrawalsPerTx);
    ...
    function handleWithdrawal(uint256 index) internal {
        ...
        if (ERC20PresetMinterPauserUpgradeable(stableContract).balanceOf(address(this)) < withdrawValue) {
            mint(participant, tokens);
            emit WithdrawFailed(participant, tokens);
            return;
        }

        ERC20PresetMinterPauserUpgradeable(stableContract).safeTransfer(participant, withdrawValue);
        emit Withdraw(participant, tokens, withdrawValue);
    }

    modifier onlyWhitelisted() {
        require(
            InvictusWhitelist(whitelistContract).hasRole(keccak256('WHITELIST_USER_ROLE'), msg.sender),
            'Must be whitelisted'
        );
        _;
    }
    ...
    modifier onlyMinter() {
        require(hasRole(MINTER_ROLE, msg.sender), 'ERC20InvictusToken: must have minter role to mint');
        _;
    }

    modifier onlyPauser() {
        require(hasRole(PAUSER_ROLE, msg.sender), 'ERC20InvictusToken: must have pauser role to pause');
        _;
    }


    modifier onlyOwner() {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), 'ERC20InvictusToken: must have owner role');
        _;
    }
