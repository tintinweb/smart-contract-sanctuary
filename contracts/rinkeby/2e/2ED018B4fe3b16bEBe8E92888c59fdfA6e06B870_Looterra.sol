...
///@notice Looterra was inspired by Dom Hofmann's Loot project, Sam Mason de Caires's Maps Project, Will Papper the creator of and Abstract Loot.
contract Looterra is ERC721, ERC721Enumerable, ReentrancyGuard, Ownable{
    
    uint256 public lootOwnerPrice = 15000000000000000; //0.015 ETH
    uint256 public publicPrice =    30000000000000000; //0.03 ETH
    address public lootAddress = 0xB52bac7B6BEdEaB22AC91d1a6621e11C8E23B2BE;// 0xFF9C1b15B16263C61d017ee9F65C50e4AE0113D7  CHANGE TO LOOT CONTRACT
    ...
    constructor(string memory baseUri) ERC721("Looterra", "Terra") {  ///change name to check opensea bug
        baseURI = baseUri;
    }
    ...
    function getResourceNode(uint256 tokenId, uint256 resourceIndex) public view returns (string memory){
        ...
        string[2] memory resourceName;
        resourceName[0] = quantity[rand % quantity.length];
        resourceName[1] = resource[rand % resource.length];
        output = string(abi.encodePacked(resourceName[0], " ", resourceName[1], output));
        return output;
    }
    ...
    function publicMint(uint256 tokenId) public payable nonReentrant {   //works
        require(msg.value >= publicPrice, "Ether value sent is not correct");
        require(tokenId > 8000 && tokenId < 9900, "Token ID invalid");
        _safeMint(msg.sender, tokenId);       
        }
    
    function ownerClaim(uint256 tokenId) public nonReentrant onlyOwner { //works
        require(tokenId > 9900 && tokenId < 10002, "Token ID invalid");
        _safeMint(owner(), tokenId);
    }

    function sendEther(address payable recipient) external onlyOwner {  //works 
        recipient.transfer(address(this).balance);
        }
    
    function setPublicPrice(uint256 newPrice) public onlyOwner {  //WORKS!
        publicPrice = newPrice;
        }
    function setlootOwnerPrice(uint256 newPrice) public onlyOwner {// Works!
