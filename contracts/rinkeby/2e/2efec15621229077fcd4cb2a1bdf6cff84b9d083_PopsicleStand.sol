...
contract PopsicleStand is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public lastRewardBlockTime; // Last block number that ICE and Reward token distribution occurs.
    ...
    IERC20 immutable public stakingToken; // Address of Staked token contract.
    ...
    constructor(
        ...
        require(_ice != address(0) && _stakingToken != address(0), "PopsicleStand: Addresses should not be zero");
        ice = IERC20(_ice);
    ...
    function setIcePerSecond(uint256 _icePerSecond) external onlyOwner {
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > lastRewardBlockTime && stakingSupply != 0) {
            uint256 multiplier =
                getMultiplier(lastRewardBlockTime, block.timestamp);
            uint256 iceReward = multiplier * icePerSecond;
            accIcePerShareTemp += iceReward * 1e12 / stakingSupply;
            
        }
        return user.amount * accIcePerShareTemp / 1e12 - user.rewardDebtIce + user.remainingIceTokenReward;
    }

    // Update reward variables of the contract to be up-to-date.
    function updatePool() internal {
        if (block.timestamp <= lastRewardBlockTime) {
            return;
        }
        uint256 stakingSupply = stakingToken.balanceOf(address(this));
        if (stakingSupply == 0) {
        ...
        uint256 multiplier = getMultiplier(lastRewardBlockTime, block.timestamp);
        
        uint256 iceReward = multiplier * icePerSecond;
        accIcePerShare += iceReward * 1e12 / stakingSupply;
