...
// ----------------------------------------------------------------------------
// --- Contract aOXMigration
// ----------------------------------------------------------------------------

contract aOXMigration is Ownable {
    ...
    function migrate(uint256 amount) external onlyInitialized() {
        require(
            aOX.balanceOf(msg.sender) >= amount,
            "amount above user balance"
        );
        require(block.number < swapEndBlock, "swapping of aOX has ended");

        aOX.transferFrom(msg.sender, address(this), amount);
        senderInfo[msg.sender] = senderInfo[msg.sender].add(amount);
        OX.transfer(msg.sender, amount);
    }

    function reclaim() external {
        require(senderInfo[msg.sender] > 0, "user has no aOX to withdraw");
        require(
            block.number > swapEndBlock,
            "aOX swap is still ongoing"
        );

        uint256 amount = senderInfo[msg.sender];
        senderInfo[msg.sender] = 0;
        aOX.transfer(msg.sender, amount);
    }

    function withdraw() external onlyOwner() {
        require(block.number > swapEndBlock, "swapping of aOX has not ended");
        uint256 amount = OX.balanceOf(address(this));
