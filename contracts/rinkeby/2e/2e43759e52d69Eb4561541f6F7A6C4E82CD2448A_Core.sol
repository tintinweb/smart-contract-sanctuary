...
contract Core is ERC1155, AccessControl, ERC1155Pausable {
    ...
    uint256 constant UNQ_FUNGIBLE_TOKEN_ID = 0x1;
    uint256 constant UNIQUETTES_TOKEN_BASE = 0x999;
    ...
    mapping(uint => string) internal idToIPFSHashMapping;
    ...
    function uri(uint256 tokenId) public view virtual override returns (string memory) {
        return string(abi.encodePacked(_baseURI, idToIPFSHashMapping[tokenId]));
    }
    ...
    function approveSubmission(string calldata ipfsHash) public {
        ...
        uint256 newTokenId = UNIQUETTES_TOKEN_BASE + _uniquetteNonce.current();
        ...
        _mint(
            ...
            UNQ_FUNGIBLE_TOKEN_ID,
            _submissionPrize,
            bytes(Strings.toString(newTokenId))
        );

        idToIPFSHashMapping[newTokenId] = ipfsHash;
