    ...
    Ownable
{
    ...
    // uint256 public revealSupply = 1400; // 70% of the reveal
    // uint256 public maxSupply = 2000;
    uint256 public revealSupply = 14;
        ...
        if (offset == 0) {
            return
                bytes(baseURI).length > 0
                    ? string(abi.encodePacked(baseURI, "unrevealed"))
                    : ".json";
        } else {
            uint256 neuronID = tokenId.add(offset) % maxSupply;
            return
                bytes(baseURI).length > 0
                    ? string(abi.encodePacked(baseURI, neuronID.toString()))
                    : ".json";
        }
    }
