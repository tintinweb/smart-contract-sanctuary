...
contract BoredApeYachtClub is ERC721, Ownable {
    ...
    string[] public avatarNames;
    
    function addAvatar(uint256 _index , string memory _avatarName) public payable onlyOwner returns(bool){
        avatarNames[_index] = _avatarName;
    }

    function setAvatarName(uint256 _index , string memory _avatarName) public payable returns(bool){
        require(ownerOf(_index) == msg.sender);
        avatarNames[_index] = _avatarName;
    }
    
    function getAvatarName(uint256 _index) public view returns(string memory){
        return avatarNames[_index];
    }
