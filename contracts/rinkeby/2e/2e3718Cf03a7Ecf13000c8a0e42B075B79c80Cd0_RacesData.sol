...
contract RacesData is Ownable {
    ...
    event QueuesCreation(Race.Struct[] newQueues);
    event DeleteQueue(uint256 indexed theQueueId);
    event UploadRace(Race.Finished race);
    event PlayerEnqueue(uint256 indexed theQueueId, uint256 indexed theHoundId);
    ...
    function deleteQueue(uint256 theQueueId) external onlyOwner {
        (bool success, ) = control.methods.delegatecall(msg.data);
        require(success,error);
    }
