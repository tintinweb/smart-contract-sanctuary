...
contract Market is IMarket, Governed, PeriodTime {
    ...
    uint public override collateralizationRequirement = 1.5e18; 
    ...
    IPositionNFT public zhuPositionNFT;
    ...
    constructor() PeriodTime (PERIOD_LENGTH) {}

    
    function _initHook() internal override {
        zhuPositionNFT = governor.zhuPositionNFT();

        
        require(zhuPositionNFT.mintTo(address(accounting)) == 0);

        IAccounting.DebtPosition memory dp;
        accounting.setPosition(0, _updatePosition(0, dp));

        bytes4[4] memory validUpdates = [
            this.setCollateralizationRequirement.selector,
            this.setTwapDuration.selector,
            this.setMinPositionSize.selector,
            this.setInterestPortionToLenders.selector];
    ...
    function createGenesisPosition(
        IGovernor.GenesisAuth calldata ga
    ) external payable lockProtocol runnable returns (uint64 positionID) {
        require(governor.getIsGenesisPhaseAndRequireAuthIfSo(msg.sender, ga), 'No longer genesis');

        require(!accounting.getParticipatedInMarketGenesis(msg.sender), 'Already participated');
        accounting.setParticipatedInMarketGenesis(msg.sender, true);

        return _createPosition(minPositionSize);
    }

    
    function removeGenesisPosition(uint64 positionID) external lockProtocol runnable beforePhase(2) {
        
        _requireAuthorized(zhuPositionNFT.isApprovedOrOwner(msg.sender, positionID));

        IAccounting.DebtPosition memory position =
            _updatePosition(positionID, accounting.getPosition(positionID));

        accounting.setParticipatedInMarketGenesis(msg.sender, false);

        _adjustPosition(position, positionID, -(position.debt.toInt256()), position.collateral);
    }

    
    function adjustGenesisPositionCollateral(
        uint64 positionID,
        uint collateralDecrease
    ) external payable lockProtocol runnable beforePhase(2) {
    ...
    function createPosition(
        uint initialDebt
    ) external payable lockProtocol runnable atLeastPhase(2) returns (uint64 positionID) {
        return _createPosition(initialDebt);
    }
    ...
    function adjustPosition(
        ...
        int debtChange,
        uint collateralDecrease
    ) external payable lockProtocol runnable atLeastPhase(2) {
    ...
    function claimRewards(uint64 positionID) external lockProtocol notStopped {
        _requireAuthorized(zhuPositionNFT.isApprovedOrOwner(msg.sender, positionID));

        accounting.setPosition(positionID, _updatePosition(positionID, accounting.getPosition(positionID)));
    }
    ...
    function _createPosition(uint initialDebt) internal returns (uint64 positionID) {
        require(msg.value > 0, 'Must add collateral');

        
        positionID = zhuPositionNFT.mintTo(msg.sender);

        IAccounting.DebtPosition memory dp;

        _adjustPosition(_updatePosition(positionID, dp), positionID, initialDebt.toInt256(), 0);

        emit NewPositionCreated(msg.sender, positionID);
    }
    ...
    function _adjustPosition(
        ...
        int collateralChange = collateralIncrease.toInt256();
        if (collateralChange == 0) collateralChange = -(collateralDecrease.toInt256());

        require(debtChange != 0 || collateralChange != 0, 'Noop');

        
        if (debtChange != 0) {
            if (debtChange > 0)  {
                uint debtIncrease = uint(debtChange);
                ...
                require(position.debt > 0, 'No debt to pay back');
                
                require(position.lastBorrowTime.add(MIN_BORROW_TIME) < _currentTime(), 'No flash borrow');
                 
                if (position.debt < uint(-debtChange)) debtChange = -(position.debt.toInt256());
                
                position.debt -= uint(-debtChange);
                
                accounting.decreaseDebt(uint(-debtChange));
            }
        }

        
        if (collateralChange != 0) {
            if (collateralChange > 0) position.collateral = position.collateral.add(uint(collateralChange));
            else position.collateral = position.collateral.sub(uint(-collateralChange));
        }
        ...
        if (debtChange != 0) {
            if (debtChange > 0) zhu.mintTo(msg.sender, uint(debtChange));
            else zhu.burnFrom(msg.sender, uint(-debtChange));
        }

        
        if (collateralChange != 0) {
            if (collateralChange > 0) TransferHelper.safeTransferETH(address(accounting), uint(collateralChange));
            else accounting.sendCollateral(msg.sender, uint(-collateralChange));
        }

        emit PositionAdjusted(positionID, debtChange, collateralChange);
    }

    
    function _requireWellCollateralized(uint debtCount, uint collateralCount) view internal {
        
        IUniswapV3Pool _collateralPool = collateralPool;
        
        if (address(_collateralPool) == address(0)) return;
        ...
        uint price = governor.prices().calculateInstantTwappedPrice(_collateralPool, twapDuration);
    ...
    function _accrueInterestImpl() internal {
        ...
        uint rewardCount = governor.currentDailyRewardCount()
            ._mul(governor.rewards().borrowRewardsPortion())
            .mulDiv(periods, PERIODS_PER_DAY);
        ...
        if (cii.additionalLends > 0) {
            accounting.increaseLentZhu(cii.additionalLends);
            zhu.mintTo(address(accounting), cii.additionalLends);
        }
        if (cii.additionalReserves > 0) zhu.mintTo(address(zhu), cii.additionalReserves);
        if (cii.reducedReserves > 0) zhu.burnReserves(cii.reducedReserves);
    ...
    function _updatePosition(
        ...
        if (rewards > 0) {
            if (positionID == 0) governor.increaseLiquidationAccountRewards(rewards);
            else governor.distributeLiquidityRewards(zhuPositionNFT.ownerOf(positionID), rewards);
        }
    ...
    function setCollateralizationRequirement(uint requirement) external onlyGovernor {
        ...
        emit ParameterUpdated('collateralizationRequirement', requirement);
    }

    
    function setMinPositionSize(uint size) external onlyGovernor {
        minPositionSize = size;
        emit ParameterUpdated('minPositionSize', size);
    }
