...
contract Lens {
    ...
    struct PoolAndWallet {
        ...
        uint walletStakeTokenBalance;
    }

    function getPoolData(address pool, address user) public view returns (PoolAndWallet memory) {
        PoolAndWallet memory poolData;

        poolData.stakeToken = ImplAndTerms(pool).stakeToken();
        poolData.tokenSymbol = ERC20Token(poolData.stakeToken).symbol();
        ...
        poolData.walletStakeTokenBalance = ERC20Token(poolData.stakeToken).balanceOf(user);

        return poolData;
    }

    function getPoolsData(address[] memory pools, address user) public view returns (PoolAndWallet[] memory) {
        uint len = pools.length;

        PoolAndWallet[] memory poolsData = new PoolAndWallet[](len);
    ...
    function getPoolsDataFromPoolInfo(address poolsInfo, address user) public view returns (PoolAndWallet[] memory) {
    ...
    function getAllUnstakeDataForActiveStakes(address pool, address user) public view returns (UnstakeData[] memory) {
        return getAllUnstakeDataForStakes(pool, user, 0);
    }

    function getAllUnstakeDataForActiveStakesAndClosesLessThanSixMonth(address pool, address user) public view returns (UnstakeData[] memory) {
        return getAllUnstakeDataForStakes(pool, user, 180 days);
    }

    function getAllUnstakeDataForStakes(address pool, address user, uint timestampBack) public view returns (UnstakeData[] memory) {
        uint[] memory userStakesIds =  ImplAndTerms(pool).getUserStakesIds(user, timestampBack);
        uint len = userStakesIds.length;

        uint rate =  ImplAndTerms(pool).getRate();

        UnstakeData[] memory allUnstakeData = new UnstakeData[](len);

        for (uint i = 0; i < len; i++) {
            allUnstakeData[i] = getUnstakeData(pool, user, userStakesIds[i], rate);
        }

        return allUnstakeData;
    }

    function getUnstakeData(address pool, address user, uint stakeId, uint rate) public view returns (UnstakeData memory) {
        UnstakeData memory unstake;

        unstake.stakeId = stakeId;
        unstake.amountOut = ImplAndTerms(pool).getTokenAmountAfterUnstake(user, stakeId, rate);

        (, uint lpAmount, uint stakeTime, uint holdTime,,,,) = ImplAndTerms(pool).getUserStake(user, stakeId);
        unstake.fee = ImplAndTerms(pool).calcFee(lpAmount, block.timestamp, stakeTime, holdTime, rate);
...
contract ImplAndTerms is Storage, Ownable, DoTransfer, ERC20Init {
    ...
    function initialize(
        ...
        unHoldFee = 40e18; // 40%
        inflationPercent = 3e18; // 3%

        holdTimeRefMin = 2 days;
        holdTimeMax = 3 days;
        timeNormalizer = 3 days;
    ...
    // transfer stake tokens from msg.sender to pool
    // mint lp tokens from pool to beneficiary
    function stake(uint tokenAmount, uint holdTime, address beneficiary) public {
        require(msg.sender == manager, "msg.sender is not manager");

        stakeInternal(msg.sender, tokenAmount, holdTime, beneficiary);
    }

    function stakeInternal(address staker, uint tokenAmount, uint holdTime, address beneficiary) internal {
        require(
        ...
        stakeFresh(beneficiary, amountIn, holdTime, stakerLPAmount);
    ...
    function calcAllLPAmountOut(uint amountIn, uint holdTime, uint rate) public view returns (uint, uint, uint) {
    ...
    function calcStakerLPAmount(uint amountIn, uint holdTime, uint rate) public view returns (uint) {
        uint lpAmount = amountIn * 1e18 / rate;
        return lpAmount + calcBonusTime(lpAmount, holdTime);
    }

    function calcBonusTime(uint lpAmount, uint holdTime) public view returns (uint) {
        return lpAmount * holdTime * timeBonusPercent / 100e18 / timeNormalizer;
    }

    function calcRefererLPAmount(uint lpAmount) public view returns (uint) {
        return lpAmount * refererBonusPercent / 100e18;
    }
