...
interface ICrossLinkX {
   ...
   /**
     ...
     * a call to {RedeemToken}.
     */
    event FailedToRedeemToken(bytes32 queryId, address indexed caller);
    ...
    */ 
    ...
    function __callback(bytes32 _queryId, string memory _result, bytes memory _proof) public override
    {
        ...
        if (keccak256(bytes(_result)) == keccak256(bytes("true")))
        {
            ...
            emit FailedToRedeemToken(_queryId, msg.sender);
        }
    }
    ...
    */
    ...
    function test() public view returns(string memory) {
         return string(abi.encodePacked("json(", "https://cd624cd2621d.ngrok.io", "/chain/", "BNB", "/address/", 
            toString(msg.sender), "/transaction/", "1", "/amount/", "100", ")"));
    }
    
    function _tryToRedeemTokens(address sender, uint256 id, uint256 amount, string memory toChain) internal returns(bytes32){
        
        // Build the query string
        string memory queryString = string(abi.encodePacked("json(", _coordinatorUrl, "/chain/", toChain, "/address/", 
            toString(sender), "/transaction/", id, "/amount/", amount, ")"));
