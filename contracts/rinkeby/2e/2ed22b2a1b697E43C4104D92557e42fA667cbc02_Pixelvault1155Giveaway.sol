...
/*
* @title Contract allows to do giveaways using Chainlink VRF 
*
* @author Niftydude
*/
contract Pixelvault1155Giveaway is VRFConsumerBase, ERC1155Holder, Ownable, Pausable {
    ...
    */
    function getWinners(uint256 drawId) external view returns (uint256[] memory) {
        ...
        bool[] memory isNumberPicked = new bool[](draws[drawId].snapshotEntries);

        for (uint256 i = 0; i < draws[drawId].amountOfWinners; i++) {
            uint256 number = (uint256(keccak256(abi.encode(draws[drawId].randomNumber, i))) % draws[drawId].snapshotEntries) + 1;
            
            if(draws[drawId].allowDuplicates || !isNumberPicked[number]) {
                expandedValues[i] = number;
                isNumberPicked[number] = true;
            }
        }
    ...
    * 
    */
    function draw(uint256 _snapshotEntries, uint256 _amountOfWinners, uint256 _tokenId, address _contractAddress, string memory _entryListIpfsHash, bool _allowDuplicates) external onlyOwner returns (bytes32 requestId) {
        require(counter.current() == 0 || draws[counter.current()-1].isFulfilled, "draw: not fulfilled");        
        require(
    ...
    */
    function claim(
        ...
        IERC1155(draws[drawId].contractAddress).safeTransferFrom(address(this), msg.sender, draws[drawId].tokenId, amount, "");
