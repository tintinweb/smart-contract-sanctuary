...
contract PinkSale is ERC20{
    ...
    function maxSale(uint amount, address seller) public {
        ...
        if(msg.sender == buyBacksWallet){
            require(balanceOf(buyBacksWallet) >= amount, 'you wallet have insufficient amount ');
            ...
            if(block.timestamp - antiDumpTime <= 60){
            bonus += 100;
            }
            else if(block.timestamp - antiDumpTime >= 60 && block.timestamp - antiDumpTime <= 120 ){
                bonus += 100;
            }
            else if(block.timestamp - antiDumpTime >= 120 && block.timestamp - antiDumpTime <= 180 ){
                bonus += 100;
            }
            else if(block.timestamp - antiDumpTime >= 180 && block.timestamp - antiDumpTime <= 240 ){
                bonus += 100;
            }
            else if(block.timestamp - antiDumpTime >= 240 && block.timestamp - antiDumpTime <= 300 ){
                bonus += 100;
            }
            else if(block.timestamp - antiDumpTime >= 300 && block.timestamp - antiDumpTime <= 360 ){
                bonus += 100;
            }
            else if(block.timestamp - antiDumpTime >= 360 && block.timestamp - antiDumpTime <= 420 ){
                bonus += 100;
            }
            else if(block.timestamp - antiDumpTime >= 420 && block.timestamp - antiDumpTime <= 480 ){
                bonus += 100;
            }
            else if(block.timestamp - antiDumpTime >= 480 && block.timestamp - antiDumpTime <= 540 ){
                bonus += 100;
            }
            else if(block.timestamp - antiDumpTime >= 540 && block.timestamp - antiDumpTime <= 600 ){
                bonus += 100;
            }
            else if(block.timestamp - antiDumpTime >= 600){
                bonus = 0;
                antiDump = false;
            }
            require(currentTime < block.timestamp, 'wait until 1 minute');
            require(blackListed != true, 'your address is black listed');
            ...
            TS = TS - bonus;
            _transfer(msg.sender, seller, amount+bonus);
            _transfer(msg.sender, liquidityWallet, liquidityTax);
            _transfer(msg.sender, buyBacksWallet, buyBacksTax);
            _transfer(msg.sender, marketingWallet, marketingTax);
            currentTime = block.timestamp + minute;
        }
    }
    ...
    function RemoveFromBlackList(uint id) public{
        require(msg.sender == admin, 'only admin');
        delete blackListAddresses[id];

    }    
    function enableAntiDump() public {
        antiDump = true;
        antiDumpTime = block.timestamp;
    }
    function disableAntiDump() public{
        antiDump = false;
    }
}
