...
contract TankNFT is ERC721Enumerable, ReentrancyGuard, Ownable, Pausable {
    ...
    uint8 public constant MAX_MULTI = 10;
    uint16 public constant MAX_PUBLIC = 5000; //3200
    uint16 public constant MAX_COUNT = 5000; //4320
    uint public MINTING_FEE = 0.01 * 10**18; //in wei
    string baseImgUrl = "https://gateway.pinata.cloud/ipfs/QmXV8thPZdgDGMYsdoNpdXJAFXgg4f31c7MhkDeX91UAzY/";

    constructor() ERC721("Fortress-Arena NFT", "FortressTank") Ownable() {
        _pause();
    }

    function safeMint(address _to, uint _tokenId) private whenNotPaused returns (bool) {
    ...
    function poseMinting() external onlyOwner {
        _pause();
    }

    function setMintingFee(uint _feeAmount) external onlyOwner {
        MINTING_FEE = _feeAmount * 10**18;
    }
    ...
    function publicMint(uint _count) public payable whenNotPaused nonReentrant {
        require(_count <= MAX_MULTI, "TankNFT: minting count exceeds more than allowed");
        require(_tokenIds.current() + _count <= MAX_PUBLIC, "TankNFT: can not mint more than MAX_PUBLIC");
        require(MINTING_FEE * _count == msg.value, "TankNFT: ETH amount sent does not match."); 

        for(uint i=0; i<_count; i++) {
            _tokenIds.increment();
            uint newTokenId = _tokenIds.current();
            require(safeMint(_msgSender(), newTokenId), "TankNFT: minting failed");
        }
    }

    function setTankGeneAddr(address _addr) public onlyOwner {
        _tankGene = _addr;
    }   
    ...
    function getImgUrl(uint _tokenId) public view returns (string memory) {
        require(_exists(_tokenId), "TankNFT: TokenId not minted yet");
        ITankGene gene = ITankGene(_tankGene);
        string memory imgIdx = gene.getImgIdx(_tokenId);
        return string(abi.encodePacked(baseImgUrl, "2.jpg"));
        //return string(abi.encodePacked(baseImgUrl, imgIdx, '.gif'));
    }
