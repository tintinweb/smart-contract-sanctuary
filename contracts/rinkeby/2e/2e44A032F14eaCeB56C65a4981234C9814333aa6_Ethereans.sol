...
contract Ethereans is ERC721, Ownable {
    ...
    IERC721                   public EtherCards = IERC721(0xAAD4475343f5150E33d6194270f04e7e5968A2f8);
    IRNG                      public iRnd       = IRNG(0x4E004E0Ab7d7839cA226226E37e6FdeD38b5Ac81);
    ...
    uint256                   public constant earlyAllocation = 1000;


    constructor( address _presigner, IRNG _rnd) ERC721("Ethereans", "ETHRS") {
   ...
   function reserveEthereans() public onlyOwner {
    ...
    function mintEarly() internal  {
        require(earlyIsActive,"Early Access is not active");
        require(cardsTaken[msg.sender] < 2,"You have already claimed two Ethereans");
        require(msg.value >= earlyAccessPrice,"Insufficent payment made");
        require(earlyMinted++ <= earlyAllocation,"All Early Access tokens have been claimed");
        uint mintIndex = super.totalSupply();
        require(mintIndex < maxEthereans, "No remaining Ethereans available");
        cardsTaken[msg.sender]++;
        _safeMint(msg.sender, mintIndex);
    }


    function earlyAccessByCard() public payable {
        require(EtherCards.balanceOf(msg.sender) > 0, "You are not an EC holder");
        require(ECAllocation++ < earlyAllocation,"Ether Cards Quota already reached");
        mintEarly();
    }

    function earlyAccessBySignature( bytes memory signature) public payable {
        require(verify(msg.sender,signature) ,"Unauthorised");
        require(CommunityAllocation++ < earlyAllocation,"Community Quota already reached");
        mintEarly();
    }
