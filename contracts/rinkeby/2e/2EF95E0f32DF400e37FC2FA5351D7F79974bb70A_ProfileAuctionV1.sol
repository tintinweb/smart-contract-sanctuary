    ...
    ReentrancyGuardUpgradeable   
{
    ...
    event ChangedBidURI(address _user, string _old, string _new);
    ...
    */
    ...
    function removeBidHelper(address _user, string memory _profileURI, uint256 _tokens) private {
    ...
    */
        ...
        for (uint256 i = 0; i < bids.length; i++) {
            if (compareString(bids[i]._profileURI, _profileURI)) {
                if (_action == 0) {
                    ...
                    require(_bids[_user][i]._blockMinted == 0, "claim");
                    _bids[_user][i]._profileURI = _optionalURI;

                    emit ChangedBidURI(_user, _profileURI, _optionalURI);
                    return true;
                }
    ...
    function setPublicClaim(uint256 _val) external onlyGovernor {
        publicClaim = _val;
    }
    ...
    */
    function submitProfileBid(
        uint256 _nftTokens,
        string memory _profileURI
    ) external nonReentrant validURI_(_profileURI) {
        require(transferNftTokens(msg.sender, _nftTokens));
        findEffectDeleteArray(msg.sender, _profileURI, 0, _nftTokens, "");
    }

    /**
     @notice allows users to change bid without fully unstaking and restaking
    ...
    */
    ...
    ) external nonReentrant validURI_(_newProfileURI) {
        findEffectDeleteArray(msg.sender, _oldProfileURI, 4, 0, _newProfileURI);
    }
