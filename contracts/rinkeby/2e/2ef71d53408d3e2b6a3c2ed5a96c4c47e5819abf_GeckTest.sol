...
contract GeckTest is ERC721Enumerable, RoyaltiesV2Impl { 
    ...
    function mint(uint256 amount) public payable {
        ...
        for (index = 0; index < amount; index++) {
            if (totalPublicSupply < MAX_PUBLIC_SUPPLY) {
                ...
                if (legendaryDecider == 0) {
                    //This is a legendary
                    _tokenIdToLegendary[index] = true;

                    legendariesMinted++;

                    //It will assign this legendary
                    _tokenIdToLegendaryNumber[index] = legendariesMinted;
            ...
            tokenIdToHash[index][tokenIndex[index]] = hash(index, _to, 0);

            hashToMinted[tokenIdToHash[index][tokenIndex[index]]] = true;
    ...
    function genesisMint(uint256[] memory tokenIds) public {
        ...
        for (index = 0; index < tokenIds.length; index++) {
            require(mintPassContract.ownerOf(tokenIds[index]) == msg.sender, "You do not own this token.");
            require(!isClaimed(tokenIds[index]), "Gecko has already been claimed for this token.");
            claimlist[tokenIds[index]].tokenId = tokenIds[index];
            claimlist[tokenIds[index]].claimed = true;
