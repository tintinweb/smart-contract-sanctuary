...
contract DeFido is Ownable, ERC721, IConditional {
  ...
  constructor() ERC721("DeFido", "DFD") { 
    deployer = msg.sender;
    airdropToWallet(deployer,400);
  }
  ...
  //OKLG Implementation
  //if it has at least 1 token, returns true
  function passesTest(address wallet) external override view returns (bool){
  ...
  function airdropSingleToMultipleWallets(address[] memory walletAddresses) public onlyOwner{
    uint numberOfWallets = walletAddresses.length;
    for(uint i= 0; i<numberOfWallets; i++){
        mintInternal( walletAddresses[i], 1);
    }
  }
  ...
  function airdropToMultipleWallets(address[] memory recipients, uint[] memory quantity) external onlyOwner{
    require(recipients.length == quantity.length, "Data lengths doesn't match!");
    uint totalRequestedMints = 0;
    for(uint i = 0; i< quantity.length ; i++){
      totalRequestedMints += quantity[i];
    }

    require((totalRequestedMints) + totalSupply() < maxSupply,"Minting this many would exceed max cap!");

    for(uint index = 0; index < recipients.length; index++){
      mintInternal(recipients[index], quantity[index]);
    }
  }

  function mintPresale(uint numberOfTokens) external whitelisted payable {
