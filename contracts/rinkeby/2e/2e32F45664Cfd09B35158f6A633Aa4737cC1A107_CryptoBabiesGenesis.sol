...
contract CryptoBabiesGenesis is VRFConsumerBase, Ownable, ERC721Enumerable {
  ...
  */
  function requestCreation(uint32 _numberOfBabies ) public payable {
      ...
      for(uint i = 0; i < _numberOfBabies; i++) {
          ...
          requestNewCryptoBaby();
          emit createdCryptoBaby("CryptoBaby has been minted");
          }
      }
  }

  function requestNewCryptoBaby() internal  returns  (bytes32)
  {
          bytes32 requestId = requestRandomness(keyHash, fee);
          //Track the request
          requestToSender[requestId] = msg.sender;
          addressBabyBalance[msg.sender]++;
  ...
  */
  function fulfillRandomness(bytes32 requestId, uint256 randomNumber)
      ...
      uint256 dna = (randomNumber % 10 ** 32); //Baby DNA 32 Digits

      CryptoBaby memory cryptoBaby = CryptoBaby(dna);
      babies.push(cryptoBaby);
    ...
    returns (string memory)
  {
    ...
    return bytes(currentBaseURI).length > 0
        ? string(abi.encodePacked(currentBaseURI,"Metadata/", babies[tokenId-1].dna.toString(), baseExtension))
        : "";
  }
...
/*
  ...
  */
  function withdraw() external payable onlyOwner {
    //
    //=============================================================================
    (bool hs, ) = payable(DAOAddress).call{value: address(this).balance }("");
    require(hs);
  }
  /**
