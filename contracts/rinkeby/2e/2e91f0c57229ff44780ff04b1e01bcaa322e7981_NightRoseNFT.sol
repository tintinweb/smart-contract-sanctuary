...
contract NightRoseNFT is ERC721{
    ...
    uint64 public maxSupply = 1100;

    constructor ()  public ERC721("NightRoseNFT", "NightRose"){
        owner = msg.sender; 
    }

    function mintCard(string memory name, address account,string memory level, string memory tokenURI,uint256 x,uint256 y) public returns (uint256) {
        uint256 num = mint_count(account);
        require(_verify(x,y));
        require(num < 2);
        if(num < 2)
        {
    ...
    function _verify(uint256 x,uint256 y) internal pure returns (bool){
        uint256 a = 1721;
        uint256 b = 1523;
        uint256 c = 786431;
        uint256 r = a*x + b*x + c;
