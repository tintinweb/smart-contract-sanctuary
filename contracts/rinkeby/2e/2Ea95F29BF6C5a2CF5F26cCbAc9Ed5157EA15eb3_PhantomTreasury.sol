...
contract PhantomTreasury is PhantomStorageMixin, IPhantomTreasury {
    ...
    function swap(
        ...
        transferFromVaultToUser(forUser, inAmount, outToken, keys, percentages);
    ...
    function swapBurn(
        ...
        transferFromUserToVault(forUser, burnAmount, burnToken, keys, percentages);
    ...
    function deposit(
        ...
        uint256 quote = PhantomSpiritRouter().getQuote(inAmount, inToken, address(PHM()));
        ...
        emit PhantomTreasury_Minted(address(PHM()), address(PhantomVault()), mintAmount);
        return mintAmount;
    }
    ...
    function withdraw(
        ...
        uint256 quote = PhantomSpiritRouter().getQuote(outAmount, outToken, address(PHM()));
    ...
    function registerReserveToken(address token) external override onlyRegisteredContracts {
        ...
        for (uint256 i; i < reserveTokens.length; i++) {
            if (token == reserveTokens[i]) {
                alreadyRegistered = true;
                break;
            }
        }
        require(alreadyRegistered == false, "Token already registered");
    ...
    function sumReserves() public view override returns (uint256) {
        uint256 S;
        for (uint256 i; i < reserveTokens.length; i++) {
            S += PhantomSpiritRouter().getQuote(
                IERC20(reserveTokens[i]).balanceOf(address(PhantomVault())).mul(0.85e18),
                reserveTokens[i],
                address(PHM())
            );
        }
    ...
    function excessReserves() internal view returns (uint256) {
        return getBalanceFor(reserveKey()[0]); //TODO finish the logic here
    }

    function getBalanceFor(bytes32 accountKey) internal view returns (uint256) {
        return PhantomStorage().getUint(keccak256(abi.encodePacked(phantom.treasury.balances, accountKey)));
    }
    ...
    function isPHMTypeToken(address inToken) internal view returns (bool) {
        return inToken != address(PHM()) && inToken != address(sPHM()) && inToken != address(gPHM());
    }
