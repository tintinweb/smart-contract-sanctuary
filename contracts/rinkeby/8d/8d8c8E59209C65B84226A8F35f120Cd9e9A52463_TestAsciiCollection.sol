...
contract TestAsciiCollection is ERC721, Ownable {
    ...
    mapping(uint256 => mapping(uint => string)) private _asciiData;
    ...
    function addAsciiArt(uint256 tokenId, string[50] memory asciiData) public onlyOwner {
        for (uint i2 = 0; i2 < 50; i2++) {
            _asciiData[tokenId][i2] = asciiData[i2];
        }
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory finalSvg = string(abi.encodePacked("<svg width='500' height='500' viewBox='0 0 500 500' xmlns='http://www.w3.org/2000/svg'>"));
        finalSvg = string(abi.encodePacked(finalSvg, "<rect width='100%' height='100%' fill='#000'></rect>"));
        for (uint i = 0; i < 50; i++) {
            string memory yString = Strings.toString(i * 11);
            finalSvg = string(abi.encodePacked(finalSvg, "<text x='0' y='", yString, "' width='100%' dominant-baseline='middle' textLength='499' font-size='8px' font-family='monospace' fill='#fff'>"));
            string memory printSymbolsRow = _asciiData[tokenId][i];
            finalSvg = string(abi.encodePacked(finalSvg, printSymbolsRow));
