...
import "./Token2476.sol";


contract Attention2476 is AdminControl, ERC1155Receiver{
    using SafeMath for uint256;
    
    address contractAddress2476 = 0xd2a5bC10698FD955D1Fe6cb468a17809A08fd005;
    
    struct DepositInfo {
        uint256[] amount;       // current staked 2476
    ...
    mapping (string => DepositInfo) public stakesByProject;
    mapping (address => string) public stakesByAdress;
    string[] projects;
    ...
    /**    
    * Set 2476 contract address
    */
    function set2476Contract(address newAddress) public onlyOwner {
    ...
    function depositTokens( uint256 total,string memory project ) external {
        ...
        require(keccak256(abi.encodePacked(stakesByAdress[msg.sender])) == (keccak256(abi.encodePacked(("")))),"Attention2476 : Wallet is already promoting a project.");
    ...
    function withDrawTokens(string memory project) external {
        require(keccak256(abi.encodePacked(stakesByAdress[msg.sender])) != (keccak256(abi.encodePacked(("")))),"Attention2476 : Wallet is NOT promoting a project.");
        require(stakesByProject[project].inUse,"Attention2476 :Project not Found.");
        stakesByAdress[msg.sender]="";
        uint256 value=0;
        for (uint i = 0; i < stakesByProject[project].wallet.length; i++) {
            if(stakesByProject[project].wallet[i]==msg.sender){
                IERC1155(contractAddress2476).safeTransferFrom(address(this),msg.sender, 2476, stakesByProject[project].amount[i],"" );
                emit WithDrawDeposit(msg.sender, stakesByProject[project].amount[i], project);
                stakesByProject[project].wallet[i]=address(0);
                stakesByProject[project].amount[i]=uint256(0);
            }else{
                value+= stakesByProject[project].amount[i];
            }
        }
        if(value==0){ // project is empty, lets remove it.
            delete stakesByProject[project].amount;
            ...
            string memory lastProject = projects[lastIndex];
            // swap index 
            projects[stakesByProject[project].index]=lastProject;
    ...
    function burnDeposit(string memory project) external adminRequired{
        ...
        for (uint i = 0; i < stakesByProject[project].wallet.length; i++) {
            ...
            if (stakesByProject[project].wallet[i]!=address(0)){
                Token2476(contractAddress2476).burn(address(this),2476, stakesByProject[project].amount[i]); 
                stakesByAdress[stakesByProject[project].wallet[i]]="";
                total += stakesByProject[project].amount[i];
            }
        }

        // reset the both arrays
        delete stakesByProject[project].amount;
        delete stakesByProject[project].wallet;
        stakesByProject[project].inUse = false;
        // delete from project array
      ...
      * 
      */
    function tokensInHolding() external view returns (uint){
         return IERC1155(contractAddress2476).balanceOf(address(this),2476);
    }
    /**
     * Retrieve the project name a wallet is promoting.
     */
    function myDeposit() public view returns (string memory project){
         return stakesByAdress[msg.sender];
    }
    /**
     * Retrieve all project names.
     */
    function getProjects() public view returns (string[] memory project){
        return projects;
    }
    
    function getProjectData(string memory project) public view returns (DepositInfo memory){
        return stakesByProject[project];
    }
...
contract Token2476 is ERC1155Supply, Ownable, ERC1155Pausable, ERC1155Burnable{
    ...
    constructor() ERC1155("ipfs://QmYbLhbLYyxuGbGbEFPuKNqmSurvrSaTgykY5xR8hRsuxM") { // TODO change before production!
        preSale(FRANK); // TODO change before production!
    }
    ...
    function preSale(address to) public onlyOwner {
        require(!paused(), "minting is paused");
       _mint(to, TOKEN_ID, NUM_RESERVED_TOKENS, "");
    }
    ...
    */
    ...
    function mint(uint numberOfTokens) external payable{
        ...
        require(totalSupply(TOKEN_ID) + numberOfTokens <= MAX_TOKENS, "2476: Purchase would exceed max supply of tokens!");
    ...
    function withdraw() public onlyOwner {
        ...
        require(payable(FRANK).send(dev));
        require(payable(owner()).send(dev*49));
        emit PaymentReleased(owner(), dev*50);
    }
...
/**
 * Based one AdminControl from manifold.xyz
