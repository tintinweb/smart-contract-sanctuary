...
contract OlyPrincipleDepository is IPrincipleDepository, Ownable {
  ...
  modifier onlyReserveToken( address reserveTokenChallenge_ ) {
    require( isReserveToken[reserveTokenChallenge_], "OlyPrincipleDepository:: Not a registered reserve token" );
    _;
  }
    ...
    internal
  {
    ...
    DepositInfo memory currentDepositorInfo_ = getDepositorInfoForReserveTokenForPrincipleTokenForDepositor[msg.sender][bondPrincipleTokenToDeposit_][reserveToken_];

    uint totalAmountDepositing = currentDepositorInfo_.principleAmount.add(amountToDeposit_);

    uint totalInterestDue_ = IBondingCalculator( getBondingCalculatorForPrincipleTokenForReserveToken[reserveToken_][bondPrincipleTokenToDeposit_] )
          .calculateBondInterest(
            address( treasury ),
            reserveToken_,
            bondPrincipleTokenToDeposit_,
            totalAmountDepositing,
    ...
    getDepositorInfoForReserveTokenForPrincipleTokenForDepositor[msg.sender][bondPrincipleTokenToDeposit_][reserveToken_] = DepositInfo(
      {
        principleAmount: currentDepositorInfo_.principleAmount.add(amountToDeposit_),
