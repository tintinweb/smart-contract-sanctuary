...
contract Fish is ERC721, Ownable {
    ...
    uint public constant MAX_Fishy = 10000;
    ...
    function calculatePrice() public view returns (uint256) {
        ...
        require(totalSupply() < MAX_Fishy, "Sale has already ended");
   ...
   function claimKoala(uint256 numFishy) public payable {
   
        require(totalSupply() < MAX_Fishy, "Sale has already ended");
        require(numFishy > 0, "You can adopt minimum 1 Fishy");
        require(totalSupply().add(numFishy) <= MAX_Fishy, "Exceeds MAX_Fishy");
		
        require(msg.value >= calculatePrice().mul(numFishy), "Ether value sent is below price");

        for (uint i = 0; i < numFishy; i++) {
    ...
    function reserveGiveaway(uint256 numFishy) public onlyOwner {
	
        uint currentSupply = totalSupply();
        require(totalSupply().add(numFishy) <= 42, "Exceeded giveaway");
        		
        uint256 index;
        
        for (index = 0; index < numFishy; index++) {
