...
contract Staking is Ownable {
    ...
    function setRewardRatio(uint256 _numerator, uint256 _denominator)
        public
        onlyOwner
    {
        require(
        ...
        returns (uint256)
    {
        ...
        uint256 inner = SafeMath.mul(
            SafeMath.mul(rewardRate, numerator),
            SafeMath.mul(
                SafeMath.mul(stakedAmount[_addr], numerator),
                blockdiff
            )
        );
        uint256 outer = SafeMath.add(
            summarizedReward[_addr],
            SafeMath.sub(inner, spentAmount[_addr])
        );
        uint256 result = SafeMath.div(outer, denominator);
        return result;
    }
