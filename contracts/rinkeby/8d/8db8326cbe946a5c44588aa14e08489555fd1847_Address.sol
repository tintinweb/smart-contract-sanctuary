...
contract Bags is ERC721Enumerable, ReentrancyGuard, Ownable {
  ...
  string[] private assetEight = [
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: sans-serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="yellow" /><text x="10" y="20" class="base">';
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Bag #', toString(tokenId), '", "description": "Bags are randomized adventure gear generated and stored on chain. The idea takes inspiration from Loot for the Ape In community. Feel free to use Bags in any way you want.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    constructor() ERC721("Bags", "BAGS") Ownable() {}
}