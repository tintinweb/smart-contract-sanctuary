...
contract Sale is Ownable{
    ...
    function _saleToken(address payable from, address payable admin,uint256 tokenId, uint256 amount,uint256 totamount) internal{
        require(totamount> order_place[from][tokenId].price , "Insufficent found");
...
contract ExchangeContract is Sale{
    ...
    function saleToken(address payable from, address payable admin,uint256 tokenId, uint256 amount,uint256 totamount, address token, uint256 _type, uint256 NOFToken) public payable{
       _saleToken(from, admin, tokenId, amount,totamount);
       if(_type == 721){
           NFTBANK721 tok= NFTBANK721(token);
