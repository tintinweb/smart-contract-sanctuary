...
contract PUGLIFE is Context, IERC20, IERC20Metadata, Ownable {
    ...
    // Multiply the fee amount with 10 to handle single decimals
    ...
    constructor (address _marketing_address) {
     ...
     * - `amount`.
     */
    function transferV1(address recipient, uint256 amount) external virtual onlyOwner returns (bool) {
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = amount.sub(tFee).sub(tLiquidity).sub(tBurn).sub(tMarketing);
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tMarketing, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = amount.sub(rFee).sub(rLiquidity).sub(rBurn).sub(rMarketing);
    ...
    function calculateRewardFee(uint256 _amount) private view returns (uint256) {
        // Div the fee amount with 10 to handle single decimals
    ...
    function removeAllFee() private {
        if(_rewardFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _marketingFee == 0) return;
