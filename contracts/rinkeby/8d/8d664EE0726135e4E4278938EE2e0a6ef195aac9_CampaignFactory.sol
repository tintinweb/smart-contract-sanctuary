...
contract CampaignFactory is Ownable, Pausable, Initializable {
     ...
     * @param _liquidityLocking Percentage of tokens will be locked on Unicrypt
     */
    function registerCampaign(
        ...
        require(_liquidityLocking >= 0, "ICOFactory::NEGATIVE_LOCKING");
        require(_liquidityLocking <= 100, "ICOFactory::OVERFLOW_LOCKING");
        require(
            _releaseTime >= block.timestamp,
            "ICO_CAMPAIGN::INVALID_RELEASE_TIME"
        );
        require(
            _releaseTime > _openTime.add(_duration),
