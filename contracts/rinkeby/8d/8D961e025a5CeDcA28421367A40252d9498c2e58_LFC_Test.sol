...
contract LFC_Test is ERC721, Ownable, RandomlyAssigned{
    ...
    mapping(address => uint256) totalLuck;
    mapping(address => mapping(uint256 => uint256)) luck;
    mapping(address => mapping(uint256 => uint256)) CooldownPerToken;
    ...
    {   
      ...
      for(uint256 i = 1; i <= _amount; i++){
        ...
        if(currentSupply==9 || currentSupply==19 || currentSupply==24){
          cost+=0.01 ether;
        }
        NFTid[currentSupply] = id;
        currentSupply++;
      }
    ...
    // FUNCTIONNALITY
    modifier cooldown(uint256 _tokenId){
        require(block.timestamp> CooldownPerToken[msg.sender][_tokenId]);
        _;
    }
    
    modifier hasLFC(uint256 _tokenId){
        require(walletOfOwner(msg.sender).length!=0);
        require(ownerOf(_tokenId) == msg.sender);
        _;
    }
    
    function currentLuck(address _addr)view public returns(uint256){
        return totalLuck[_addr];
    }
    
    function pray(uint256 _tokenId) public cooldown(_tokenId) hasLFC(_tokenId){
        setTimer(30, _tokenId);
        totalLuck[msg.sender] = totalLuck[msg.sender]+50;
    }

    
    //TIMED EVENTS
   function setTimer(uint256 _time, uint256 tokenId) public{
        startTime = block.timestamp;
        CooldownPerToken[msg.sender][tokenId] = startTime+_time;
    }
    
    function timeLeft(uint256 tokenId) public view returns(uint256){
        return CooldownPerToken[msg.sender][tokenId]-block.timestamp;
    }
