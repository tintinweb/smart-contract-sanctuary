...
contract StableUnitDAOaNFT is ERC721, ERC721Enumerable, Pausable, AccessControl {
    ...
    function insertLevelToId(uint256 id, uint256 level) public pure returns (uint256) {
        ...
        return id | ((uint256(level) << 128) & ((1 << 16) - 1));
    }
    ...
    function getLevel(address user) external view returns (uint256) {
        require(balanceOf(user) == 1, "can't determine aNFT level");
        uint256 id = tokenOfOwnerByIndex(user, 0);
        return extractLevelFromId(id);
    }

    function mint(address to, uint256 level) public onlyRole(MINTER_ROLE) {
    ...
    // mint batch of NFTs
    function mintBatch(address[] calldata users, uint256[] calldata levels) external onlyRole(MINTER_ROLE) {
