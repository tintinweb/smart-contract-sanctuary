...
////// /nix/store/8xb41r4qd0cjb63wcrxf1qmfg88p0961-dss-6fd7de0/src/PREMIA.sol
...
contract PREMIA is LibNote {
  ...
  event Transfer(address indexed src, address indexed dst, uint wad);
    ...
    modifier auth {
        require(wards[msg.sender] == 1, "PREMIA/not-authorized");
        _;
    }

    // --- ERC20 Data ---
    string  public constant name     = "Mock PREMIA";
    string  public constant symbol   = "PREMIA";
    ...
    function transferFrom(address src, address dst, uint wad) public returns (bool) {
      ...
      require(balanceOf[src] >= wad, "PREMIA/insufficient-balance");
        if (src != msg.sender && _allowance( src, msg.sender ) != uint(-1)) {
            require(_allowance( src, msg.sender ) >= wad, "PREMIA/insufficient-allowance");
    ...
    function burn(address usr, uint wad) external {
        require(balanceOf[usr] >= wad, "PREMIA/insufficient-balance");
        if (usr != msg.sender && _allowance( usr, msg.sender ) != uint(-1)) {
            require(_allowance( usr, msg.sender ) >= wad, "PREMIA/insufficient-allowance");
                    ...
                    bool allowed, uint8 v, bytes32 r, bytes32 s) external
    {
        ...
        require(holder != address(0), "PREMIA/invalid-address-0");
        require(holder == ecrecover(digest, v, r, s), "PREMIA/invalid-permit");
        require(expiry == 0 || block.timestamp <= expiry, "PREMIA/permit-expired");
        require(nonce == nonces[holder]++, "PREMIA/invalid-nonce");
