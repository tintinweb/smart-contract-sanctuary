...
/*------------------ERC-1594 Implementation--------------------*/
    ...
    //  }
    function issue(address tokenHolder, uint256 value, bytes calldata _data) external override onlyOwner isUserBlackListed(tokenHolder) isUserKyc(tokenHolder)
    {
        ...
        if (_isIssuable && balanceAvailable.div(10 ** 18) >= value)
        {
            ...
            addressTokenHistory[tokenHolder].push(TokenSaleData(block.timestamp,value.mul(10**18),_data,false,currentGoldGramPart));
            orderHistory.push(saleData(block.timestamp,value,_data,currentGoldGramPart,tokenHolder));
    ...
    function redeemFrom(address tokenHolder, uint256 value, bytes calldata _data) external override onlyOwner
    {
        if (_balances[tokenHolder].div(10**18) >= value) {
            _balances[tokenHolder] = _balances[tokenHolder].sub(value.mul(10**18));
           // balanceAvailable = balanceAvailable.add(value);
           
           
            addressTokenHistory[tokenHolder].push(TokenSaleData(block.timestamp,value.mul(10**18),_data,true,0));
...
/*------------------ERC-20 Implementation--------------------*/
    ...
    function mintNumberOfGram(uint gram,address _address) external onlyOwner  //input will be in gram
    {
        ...
        emit Issued(address(0),_address,balanceAvailable.div(10**18),toBytes(gram));
    }
