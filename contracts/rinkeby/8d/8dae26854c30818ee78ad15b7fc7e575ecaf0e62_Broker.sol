...
// File: contracts/BrokerModifiers.sol
...
contract BrokerModifiers is Storage {
    ...
    modifier tokenOwnerOnlly(uint256 tokenID, address _mintableToken) {
        ...
        require(
            MintableToken(_mintableToken).ownerOf(tokenID) == msg.sender,
...
contract Broker is ERC721Holder, BrokerModifiers {
    // events
    event Bid(
        ...
        onlyOwner
    {
        // require(
        //     erc20TokensArray.exists(_erc20Token),
        //     "This ERC20token not available in Array"
        // );
        ...
        activeAuction(tokenID, _mintableToken)
    {
        ...
        //     "Token Not For Sale"
        // );
        // require(block.timestamp < _auction.closingTime, "Auction Time Over!");
        // require(_auction.auctionType != 1, "Auction Not For Bid");
        ...
        activeAuction(tokenID, _mintableToken)
    {
        ...
        uint256 royalitiy = (royalities * _auction.currentBid) / 10000;
        ...
        if (_auction.buyer == true) {
            if (_auction.erc20Token == address(0)) {
                ...
                erc20Token.transfer(creator, royalitiy);
        ...
        flatSaleOnly(tokenID, _mintableToken)
    {
        ...
        //     "Token Not For Sale"
        // );
        // require(_auction.auctionType != 2, "Auction for Bid only!");

        address payable lastOwner2 = _auction.lastOwner;
        uint256 royalities = Token.royalities(tokenID);
        address payable creator = Token.creators(tokenID);
        uint256 royalitiy = (royalities * _auction.buyPrice) / 10000;
        uint256 brokerage_this = (brokerage * _auction.buyPrice) / 10000;
