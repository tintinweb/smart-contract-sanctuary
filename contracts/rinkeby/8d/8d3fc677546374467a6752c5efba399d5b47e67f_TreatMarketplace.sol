...
contract TreatMarketplace {
    ...
    constructor(address payable _treasuryAddress, address _treatMinterAddress) {
      maxId = 0;
    ...
    function Purchase(uint256 orderId, uint256 value) public payable {
        ...
        treasuryAddress.transfer(value.mul(100 - treasuryPercentage).div(100));
        order.seller.transfer(value.mul(treasuryPercentage).div(100));
        //uint256[] memory nftIdArg = [order.nftId];
        //uint256[] memory quantityArg = [order.quantity];
        //treatMinter.safeBatchTransferFrom(order.seller, msg.sender, nftIdArg, quantityArg, "0x0");

        emit OnNftReceived(msg.sender, order.nftId, order.quantity);
        emit OnNftSent(order.seller, order.nftId, order.quantity);
    }

    function transferNfts(address payable _seller, address _receiver, uint256[] memory  _nftIds, uint256[] memory  _quantity) internal {
        treatMinter.safeBatchTransferFrom(_seller, _receiver, _nftIds, _quantity, "0x0");
    }
}