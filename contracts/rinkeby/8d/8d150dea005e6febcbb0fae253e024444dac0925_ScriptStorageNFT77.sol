...
contract ScriptStorageNFT77 is ERC721, Pausable, AccessControl, ERC721Burnable {
    ...
    string internal constant scriptHEAD = '<body onLoad="loadUp"><script>';
...
//returns an alternate, shorter stringized version of the entire token hash.
    ...
    function formatTokenURI(string memory fullString) internal pure returns (string memory) {
        return string(abi.encodePacked("data:application/json;base64,",Base64.encode(bytes(abi.encodePacked('{"name":"Frogs Alive", "description":"mikehauncho", "animation_url":"',fullString,'"}')))));
    }
