...
contract Staking is IStaking, Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 private _executorsNum;

    event Registered(address indexed executor, uint256 timestamp);
    event Unregistered(address indexed executor, uint256 timestamp);
    ...
    function registeredExecutorsNum() external view returns (uint256) {
        return _executorsNum;
    }

    function registerExecutor(address _executor) external onlyOwner {
        require(_executor != address(0), "Invalid executor address");
        require(!_isExecutor[_executor], "Executor already registered");
        _isExecutor[_executor] = true;
        _executorsNum++;
        emit Registered(_executor, block.timestamp);
    }

    function unregisterExecutor(address _executor) external onlyOwner {
        require(
            _isExecutor[_executor] && _executor != address(0),
            "Invalid executor address"
        );
        _isExecutor[_executor] = false;
        _executorsNum--;
        emit Unregistered(_executor, block.timestamp);
    }

    modifier onlyExecutor(address _executor) {
        require(_isExecutor[_executor], "Only executor");
        _;
    }
