...
contract ProductProxy is Proxy {
  ...
  function name() virtual public view returns (bytes32 name_) {
  ...
  function _implementation() virtual override internal view returns (address) {
    ...
    if(OpenZeppelinUpgradesAddress.isContract(factory_))
        return IProxyFactory(factory_).productImplementations(name());
    else
        return address(0);
  }
...
contract InitializableProductProxy is ProductProxy {
   ...
   * This parameter is optional, if no data is given the initialization call to proxied contract will be skipped.
   */
  function initialize(address factory_, bytes32 name_, bytes memory data_) public payable {
...
contract MappingTokenFactory is ContextUpgradeSafe, Configurable, Constants {
    ...
    function createTokenMapped(address token) external payable returns (address tokenMapped) {
        ...
        InitializableProductProxy(payable(tokenMapped)).initialize(address(this), _TokenMapped_, abi.encodeWithSignature('__TokenMapped_init(address)', token));
    ...
    function createMappableToken(string memory name, string memory symbol, uint8 decimals, uint totalSupply) external payable returns (address mappableToken) {
        ...
        InitializableProductProxy(payable(mappableToken)).initialize(address(this), _MappableToken_, abi.encodeWithSignature('__MappableToken_init(address,string,string,uint8,uint256)', _msgSender(), name, symbol, decimals, totalSupply));
    ...
    function _createMappingToken(uint mainChainId, address token, string memory name, string memory symbol, uint8 decimals, uint cap) internal returns (address mappingToken) {
        ...
        InitializableProductProxy(payable(mappingToken)).initialize(address(this), _MappingToken_, abi.encodeWithSignature('__MappingToken_init(uint256,address,address,string,string,uint8,uint256)', mainChainId, token, _msgSender(), name, symbol, decimals, cap));
