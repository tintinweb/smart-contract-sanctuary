...
contract Pupi is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getNftNoun(uint256 tokenId, uint256 offset) public view returns (string memory) {
        return pluck(tokenId, "NFTNOUN", nftNouns, offset);
    }
    ...
    function conditionalFunctionCall(string memory strFunc, uint256 tokenId, uint256 offset) public view returns (string memory returnVal) {
        ...
        if (keccak256(abi.encodePacked(strFunc)) == keccak256(abi.encodePacked("nftNouns"))) {
            return getNftNoun(tokenId, offset);
        }
    }
