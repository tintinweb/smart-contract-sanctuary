...
contract VBITDEXMultiple is BEP1155, Ownable, BEP1155Burnable{
    event Approve(
    ...
    uint256 private valobitdxserviceValue;
    address private Valobitdxaddress;
    address private Wethaddress;
    string private valobitdxType;
    string private wethType;
    ...
    constructor(
        ...
        uint256 _valobitdxserviceValue,
        address _valobitdexaddress,
        ...
        valobitdxserviceValue=_valobitdxserviceValue;
        Valobitdxaddress=_valobitdexaddress;
   ...
   function getValobitdxFee() public view returns (uint256){
       return valobitdxserviceValue;
   }
    function serviceFunction(uint256 _serviceValue) public onlyOwner{
        serviceValue = _serviceValue;
    }
    function getValibitdxAddress() public view returns (address){
        return Valobitdxaddress;
    }
    function getWethaddress() public view returns (address){
        return Wethaddress;
    }
    function setTokenAddress(string memory _valobitdxType,address _valobitdexaddress,string memory _wethType,address _Wethaddress) public onlyOwner{
        valobitdxType=_valobitdxType;
        Valobitdxaddress = _valobitdexaddress;    
        Wethaddress      = _Wethaddress;
        wethType         = _wethType;
    }
    function valobitdxserviceFunction(uint256 _valobitdxserviceValue) public onlyOwner{
        valobitdxserviceValue = _valobitdxserviceValue;
    }
    ...
    function saleWithToken(
        ...
        address tokenAddress
            ...
            _saleToken(from, tokenId, amount, bidtoken,tokenAddress);
    ...
    function saleToken(
        ...
        _saleToken(from, tokenId, amount, "BNB",tokenAss);
        _setApprovalForAll(from, msg.sender,true);
        saleTokenTransfer(from, tokenId, nooftoken);
    }
    ...
    function _saleToken(
        ...
        address tokenAss
         ...
         uint256 fee = serviceValue;
        if(tokenAss == Valobitdxaddress ){
            fee = valobitdxserviceValue;
        }
        uint256 val = pBEPent(amount, fee).add(amount);
        if(keccak256(abi.encodePacked((bidtoken))) == keccak256(abi.encodePacked(("BNB")))){   
            ...
            (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], fee);
            ...
            BEP20 t = BEP20(Valobitdxaddress);
             if(keccak256(abi.encodePacked((bidtoken))) == keccak256(abi.encodePacked((wethType)))){   
            BEP20 t = BEP20(Wethaddress);
            }
    ...
    function acceptBId(string memory bidtoken,address bidaddr, uint256 amount, uint256 tokenId, uint256 NOFToken,address tokenAddress,bool _isbool) public{
        _acceptBId(bidtoken, bidaddr, amount, tokenId, owner(),tokenAddress,_isbool);
    ...
    function _acceptBId(string memory tokenAss,address from, uint256 amount, uint256 tokenId, address admin, address tokenAddress,bool _isbool) internal{
         uint256 fee = serviceValue;
         if(tokenAddress == Valobitdxaddress ){
            fee = valobitdxserviceValue;
        }
       uint256 val = pBEPent(amount, fee).add(amount);
