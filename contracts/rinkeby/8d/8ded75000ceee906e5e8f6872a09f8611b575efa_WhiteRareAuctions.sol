...
contract WhiteRareAuctions is IAuctionHouse, ReentrancyGuard {
    ...
    function _fixedBuy(uint256 auctionId, uint256 amount) auctionExists(auctionId) internal {
         require(
            amount == auctions[auctionId].reservePrice,
        ...
        IERC721(auctions[auctionId].tokenContract).transferFrom(address(this), msg.sender, auctions[auctionId].tokenId);
        ...
        for (uint256 ct = 0; ct < tokenId.length; ct++){
            ...
            for (uint256 cnt = 0; cnt < tokens.length; cnt++){
                ...
                if (tokens[counter] == tokenId[count]){
                    ...
                    _fixedBuy(auctionCount, currAmount);
