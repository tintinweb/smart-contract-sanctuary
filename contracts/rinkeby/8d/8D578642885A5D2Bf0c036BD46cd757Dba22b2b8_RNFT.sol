...
contract RDAO is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 50 * 10**4 * 10**18;
    ...
    constructor(IERC20 _usdtAddr) {
        _rOwned[_msgSender()] = _rTotal;
        _usdtToken = _usdtAddr;
    ...
    function setLpToken(IERC20 _lpAddr) public onlyOwner {
        _lpToken = _lpAddr;
    }
    ...
    function balanceOf(address account) public view override returns (uint256) {
        if (_isExcluded[account])
            return _tOwned[account].mul(priceFactor).div(10**18);
        return
            tokenFromReflection(_rOwned[account]).mul(priceFactor).div(10**18);
    }
    ...
    function rebase() public onlyGuardian {
        uint256 usdtBalance = _usdtToken.balanceOf(address(_lpToken));
        uint256 tokenBalance = balanceOf(address(_lpToken));

        uint256 tokenPrice = usdtBalance.div(tokenBalance);

        if (tokenPrice > 1 * 10**6) {
            priceFactor = priceFactor.mul(10**6 + 2 * 10**4).div(tokenPrice);
        } else if (tokenPrice < 1 * 10**6) {
            priceFactor = priceFactor.mul(10**6 - 2 * 10**4).div(tokenPrice);
        }
    }
}
    ...
    AccessControlEnumerable
{
    ...
    function _updateExcluded(address from, address to) private {
        ...
        if (_balances[vipTokenId][from] == 0 && _rDao.isExcluded(to) == true) {
