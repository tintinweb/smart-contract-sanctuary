...
contract DCBToken is ERC20, DCBWhitelisted {
    using SafeMath for uint256;
...
contract DCBWhitelisted is Context,Ownable {
    ...
    modifier notBlackListed(address from, address to) {
        if(isEnabled){
            require(!isBlackListed[from] && 
                !isBlackListed[to], "Address is blacklisted");
        }
        _;
    }

    function whiteList(address addr, bool value) external onlyOwner {
        isWhitelisted[addr] = value;
        emit WhiteListSet(addr, value);
    }

    function blackList(address addr, bool value) external onlyWhiteListed {
        isBlackListed[addr] = value;
        emit BlackListSet(addr, value);
    }

    function bulkBlackList(address[] calldata addr, bool[] calldata value) external onlyWhiteListed {
    ...
    function setBlackList(bool value) external onlyWhiteListed {
        isEnabled = value;
