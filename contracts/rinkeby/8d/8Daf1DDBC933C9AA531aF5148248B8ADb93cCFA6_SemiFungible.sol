...
contract SemiFungible is ERC1155, AccessControl {
    ...
    function createCollection(uint256 _id, CollectionData memory collectionData)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            collections[_id].percent == 0,
            "This Collection already created"
        );

        collections[_id] = collectionData;
    ...
    function buy(
        ...
        require(collections[_id].available > 0, "Sold Out");
        require(
            collections[_id].available >= _amount,
            "There are no cards on sale as much as the number of cards you will mint."
        );
        require(
        ...
        collections[_id].farm == address(0)
            ? _withToken(_buyer, _id)
            : _withStones(_buyer, _id);

        _mint(_buyer, _id, _amount, "");

        collections[_id].available -= 1;
        collections[_id].sold += 1;

        emit Sold(address(this), _buyer, _id, collections[_id].amount);
    }
    ...
    function _withStones(address _buyer, uint256 _id) private {
        address farmAddress = collections[_id].farm;
        uint256 stones = FarmV2(farmAddress).rewardedStones(_buyer);
        require(
            stones >= collections[_id].amount,
            "You do not have enough points !"
        );
        require(
            FarmV2(farmAddress).payment(_buyer, collections[_id].amount),
            "Payment was unsuccessful"
        );

        emit SoldWithStones(_buyer, collections[_id].amount);
    }
    ...
    function setStarTime(uint256 _tokenId, uint256 _starTime)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        collections[_tokenId].startTime = _starTime;

        emit NewStartTime(_starTime);
    }

    function setEndTime(uint256 _tokenId, uint256 _endTime)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        collections[_tokenId].endTime = _endTime;

        emit NewEndTime(_endTime);
    }

    function setAmount(uint256 _tokenId, uint256 _newAmount)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        collections[_tokenId].amount = _newAmount;

        emit NewUsdAmount(_newAmount);
    }
    ...
    function getCardPrice(uint256 _id) public view returns (uint256) {
        ...
        uint256 result = (collections[_id].amount * (1e44)) /
            (tokenPrice * (1e18));

        return result;
    }

    function _withToken(address _buyer, uint256 _id) private {
        uint256 price = getCardPrice(_id);
        require(
            token.balanceOf(_buyer) >= price,
        ...
        token.transferFrom(_buyer, address(moneyHand), shareAmount);
