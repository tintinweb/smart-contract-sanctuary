...
contract BridgeDev is AccessControl, Pausable, ReentrancyGuard {
  ...
  constructor(address _owner, address _tokenToSwapWithNative) public {
  ...
  function withdraw(address _ethToken, address _ektaToken, address user, uint256 amount, uint256 nonce, bytes calldata signature) external nonReentrant whenNotPaused returns(bool) {
    ...
    require(EthEktaPairs[_ethToken] == _ektaToken, "Token pair does not exist");
    require(amount > 0, "Bridge: Amount cant be zero or negative numbers");
  ...
  function depositTokenToNative(uint256 amount) external nonReentrant whenNotPaused returns(bool) {
    ...
    // transfer token to contract address
    IERC20(tokenToSwapWithNative).safeTransferFrom(msg.sender, address(this), amount);
    
    // update token and amount in user mapping
    tokenToNativeBalance[msg.sender][tokenToSwapWithNative] += amount;
  ...
  function withdrawNativeToToken(address user, uint256 amount, uint256 nonce, bytes calldata signature) external nonReentrant whenNotPaused returns(bool) {
    require(amount > 0, "Bridge: Amount cant be zero or negative numbers");
    require(amount <= tokenToNativeBalance[user][tokenToSwapWithNative], "Bridge: Amount is higher than deposited");
    // check for nonce
    require(processedNonces[msg.sender][nonce] == false, 'Bridge: Transfer already processed');
    // verify signature
    ...
    // transfer token from contract address to user
    IERC20(tokenToSwapWithNative).safeTransfer(user, amount);
    
    // update token and amount in user mapping
    tokenToNativeBalance[user][tokenToSwapWithNative] -= amount;
  ...
  function updateTokenToSwapWithNative(address token) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(token != address(0), "Token address cant be address(0)");
