...
contract DateMinter is Ownable {
  ...
  */
  function mint(address _dst, int256 _dateTimestamp) public payable {
    ...
    // check if not minted
    string memory dateString = getDateStringFromTimestamp(_dateTimestamp);
    require(dateStringToTokenId[dateString] == 0, 'date that corresponds to the timestamp already exists');
