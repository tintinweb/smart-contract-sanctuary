...
import "../interface/ICC.sol";
import "../interface/IClaimable.sol";
// import "hardhat/console.sol";


contract TradeMining is Ownable, IClaimable {

    using SafeMath for uint256;

    ICC public rewardToken;// TradeMining reward token
    IVolumeBook public volumeBook;// Trade Volume recorder
    ...
    mapping (uint256 => uint256) public cycleReward; // the amount of total reward token of each cycle

    event Claim(address indexed user, uint256 startCycle, uint256 endCycle, uint256 amount);
    ...
    constructor(
        ...
        address _volumeBook,
        uint256 _rewardPerCycle
    ) public {
        require(_rewardToken != address(0), "TradeMining: zero address");
        require(_volumeBook != address(0), "TradeMining: zero address");
        rewardToken = ICC(_rewardToken);
    ...
    function setRewardToken(address _newAddress) public onlyOwner {
        require(_newAddress != address(0), "TradeMining: zero address");
        rewardToken = ICC(_newAddress);
    }
    ...
    function pending(address user, uint256 cycleNum) public override view returns (uint256) {
        if (!isClaim[cycleNum][user]) {
            uint256 volume = volumeBook.getUserVolume(user, cycleNum);
                if (volume != 0) {
                    uint256 totalVolume = volumeBook.getTotalTradeVolume(cycleNum);
                    if (totalVolume != 0) {
                        return volume.mul(rewardPerCycle).div(totalVolume);
                    }
                }
        }
        return 0;
    }
    // claim for others do no harm
    function claim(address user, uint256 startCycle, uint256 endCycle) external override {
        require(startCycle <= endCycle);
        uint256 currentCycleNum = volumeBook.currentCycleNum();
        uint256 lastCycleNum = currentCycleNum.sub(1);
        if (endCycle > lastCycleNum) {
            endCycle = lastCycleNum;
        }
        uint256 amount = 0;
        for (uint256 cycleNum = startCycle; cycleNum <= endCycle; cycleNum ++) {
            amount += pending(user, cycleNum);
            isClaim[cycleNum][user] = true;
        }
        
         if (amount > 0 ) {
            rewardToken.mint(user, amount);
        }
        emit Claim(user, startCycle, endCycle, amount);
    }
