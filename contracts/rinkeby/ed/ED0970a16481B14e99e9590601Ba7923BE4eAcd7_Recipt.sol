...
contract Recipt is ERC721 {
    ...
    constructor ()  ERC721 ("Recipt", "RECIPT") public {
        tokenCounter = 0;
    }
    ...
    function writeRecipt(string memory _text) public returns (uint256) {
        require(written == false, "Message already written!");
        Data storage new_data = data[newItemId];
        // uint256 newItemId = tokenCounter;
        new_data.author = msg.sender;
        new_data.date = block.timestamp;
        new_data.text = _text;
        written = true;
        // _safeMint(msg.sender, newItemId);
        // tokenCounter = tokenCounter + 1;
    ...
    function getData(uint256 tokenId) public view returns (address, uint256, string memory) {
        require(written, "Message not yet written!");
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory author = addressToString(data[tokenId].author);
        string memory date = uint256(data[tokenId].date).toString();
        string memory text = data[tokenId].text;
