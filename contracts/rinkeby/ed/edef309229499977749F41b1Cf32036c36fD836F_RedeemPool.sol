...
contract RedeemPool is IRedeemPool, Operator {
    ...
    // rid => reward amount
    mapping(uint256 => uint256) currentReward;
    mapping(uint256 => uint256) lastReward;
    mapping(uint256 => uint256) historyReward;
    ...
    function setSwapRouter(address _swapRouter) public onlyOperator {
        swapRouter = _swapRouter;
    }

    function setBurn(address _burn) public onlyOperator {
        burn = _burn;
    }
    ...
    modifier updateReward(address _director) {
        ...
        seat.rewardEarned = seat
            .deposit
            .mul(snapShot.rewardPerBond)
            .div(1e18)
            .add(seat.rewardEarned);
        seat.deposit = 0;
        seats[_director] = seat;
    ...
    function rewardEarned(address _director) public view returns (uint256) {
        ...
        return
            seat.rewardEarned.add(
                seat.deposit.mul(
                    history[seat.lastSnapshotIndex].rewardPerBond.div(1e18)
                )
            );
    }
    ...
    function migrate(uint256 amount, address to) public onlyOperator {
    ...
    function swap(
        ...
        uint256 amount = IERC20(cash).balanceOf(address(this)) - cashBefore;
