...
// first official GLICPIXXXVER002 remix on-chain generated art nft project by berk aka princesscamel
// @berkozdemir - berkozdemir.com
// https://artglixxx.world/
...
contract ARTGLIXXX is ERC721, Ownable {
    ...
    mapping(uint => uint) public ARTGLIXXX_NEXT_GEN;
    ...
    constructor() ERC721("ARTGLIXXX VOLUME 1","ARTGLIXXXVOL1")  {
    }
    ...
    function tokenHash(uint256 tokenId) public view returns(bytes32){
        ...
        return bytes32(keccak256(abi.encodePacked(address(this), creationDates[tokenId], creators[tokenId], tokenId, ARTGLIXXX_NEXT_GEN[tokenId])));
    }
    ...
    function calculateMint(uint _id) public view returns(uint) {
        return ARTGLIXXX_NEXT_GEN[_id] * BASE_MINT_PRICE;
    }
    ...
    function mintArtGLIXXXX(uint _id) public {
        ...
        if (mintPrice > 0) {
            require(GLIXTOKEN.balanceOf(msg.sender) > mintPrice, "Not enough $GLIX to burn!");
            GLIXTOKEN.burn(msg.sender, mintPrice);       
        }
        uint mintIndex = _id + (TOTAL_GLICPIXXX * ARTGLIXXX_NEXT_GEN[_id]);
        _safeMint(msg.sender, mintIndex);
        creationDates[mintIndex] = block.number;
        creators[mintIndex] = msg.sender;
        if (ARTGLIXXX_NEXT_GEN[_id] == 0){
            updateGlixRewardOnMint(msg.sender);
            balanceGENESIS[msg.sender]++;
        }
        ARTGLIXXX_NEXT_GEN[_id]++;
    }
    ...
    function getGeneration(uint256 tokenId) external view returns (uint256) {
        ...
        return ARTGLIXXX_NEXT_GEN[tokenId] + 1;
    }
    ...
    function getCurrentReward(address _user) public view returns (uint256) {
		...
		if (timerUser > 0)
			return GLIXREWARDS[_user] + (balanceGENESIS[_user] * BASE_YIELD_RATE * (time - timerUser) / 86400);
        else
            return 0;
    }
