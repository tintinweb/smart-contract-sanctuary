...
contract testGameEngine is Ownable,ReentrancyGuard{
    ...
    function stake (uint[] memory tokenId) external {
        ...
        } else if ( stakeConfirmation[tokenId[i]] == false && firstStakeLockPeriod[tokenId[i]]==0 ) {
            uint randomNo = randomNumberGenerated.initiateRandomness(tokenId[i],nftToken.actionTimestamp(tokenId[i])) % 7;
            nftToken.setTimeStamp(tokenId[i]);
            if (randomNo < 2) {randomNo += 2;}
            firstStakeLockPeriod[tokenId[i]] = block.timestamp + randomNo; //convert randomNo from hours to sec
            nftToken.safeTransferFrom(msg.sender, address (this), tokenId[i]);
            stakeTime[tokenId[i]] = block.timestamp;
            isStaked[tokenId[i]] = true;
            tokenToRandomHourInStake[tokenId[i]]= randomNo; //conversion required
            determineAndPush(tokenId[i]);
          }
        }
    }
    ...
    function moveToLast(uint _tokenId) internal {
        (uint8 tokenType,uint8 level,,,) = metadataHandler.getToken(_tokenId);
    ...
    function determineAndPush(uint tokenId) internal {
        ...
        (uint8 tokenType,uint8 tokenLevel,,,) = metadataHandler.getToken(tokenId);
    ...
    function calculateSUP (uint tokenId) internal returns (uint) {
        ...
        if (lastClaimTime == 0) {
            calculatedDuration = (block.timestamp - (stakedTime+tokenToRandomHourInStake[tokenId]));//todo /60*60
    ...
    function onSuccess (uint tokenId) internal returns (bool) {
        (uint8 nftType,uint8 tokenLevel,,,) = metadataHandler.getToken(tokenId);
        require (lastClaim[tokenId] + 12 <= block.timestamp, "Claiming before 12 hours");
        ...
        if(randomNumber<40 && tokenLevel < 5){
            moveToLast(tokenId);
            determineAndPush(tokenId);
           nftToken.restrictedChangeNft(tokenId, nftType, tokenLevel, false, stakeTime[tokenId],lastClaim[tokenId]);
        }
        return false;
    }
    ...
    function onCriticalSuccess (uint tokenId) internal returns (bool) {
        ...
        lastClaim[tokenId] = block.timestamp;
        if (randomNumberGenerated.initiateRandomness(tokenId,nftToken.actionTimestamp(tokenId))%100 < 40 && tokenLevel<5) {
            moveToLast (tokenId);
            
            determineAndPush(tokenId);
            nftToken.restrictedChangeNft(tokenId, nftType, tokenLevel + 1, false, stakeTime[tokenId],lastClaim[tokenId]);
        }
        uint value = howManyTokensCanSteal(nftType);

            for (uint i=0;i < value;i++) {
                uint stolenTokenId = steal(1-nftType,i+1);
                moveToLast(stolenTokenId);
                nftToken.restrictedChangeNft(stolenTokenId, 1-nftType, tokenLevel, false, stakeTime[tokenId],lastClaim[tokenId]);//s->1
                pool[nftType][tokenLevel].push(stolenTokenId);
