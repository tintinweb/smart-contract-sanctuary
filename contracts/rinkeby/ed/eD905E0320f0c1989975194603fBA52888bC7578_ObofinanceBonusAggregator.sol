 ...
 */
contract ObofinanceBEP20 is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;
...
// File: contracts/ObofinanceMoneyPot.sol

/*
* This contract is used to collect sOBO stacking rewards from fee (like swap, deposit on pools or farms)
*/
contract ObofinanceMoneyPot is Ownable {
    ...
    mapping(address => mapping (address => UserInfo)) public sOboHoldersRewardsInfo;
    // user => LastSOboAmountSaved
    mapping (address => uint256) public sOboHoldersInfo;

    address[] public registeredToken; // Should never be too weight !
    mapping (address => bool )  public tokenInitialized;

    address public masterObofinance;
    ...
    uint256 public lastSOboSupply;

    constructor (SOboToken _sObo, address _feeManager, address _masterObofinance, uint256 _startBlock, uint256 _initialUpdateMoneyPotPeriodNbBlocks) public{
        updateMoneyPotPeriodNbBlocks = _initialUpdateMoneyPotPeriodNbBlocks;
        startBlock = _startBlock;
        lastUpdateMoneyPotBlocks = _startBlock;
        sObo = _sObo;
        masterObofinance = _masterObofinance;
    ...
    function _updateTokenPot(address _token) internal {
        ...
        if (block.number >= tokenPot.lastUpdateTokenPotBlocks.add(updateMoneyPotPeriodNbBlocks)){
            if(tokenPot.tokenAmount > 0){
                ...
                tokenPot.accTokenPerShare = tokenPot.accTokenPerShare.add(tokenRewardsPerBlock.mul(multiplier).mul(1e12).div(lastSOboSupply));
            }
...
//                _updateTokenPot(_token);
    ...
    function pendingTokenRewardsAmount(address _token, address _user) external view returns (uint256){
        ...
        if (block.number > lastRewardBlock && lastSOboSupply != 0 && tokenReward > 0) {
            accTokenPerShare = accTokenPerShare.add(
                    tokenReward.mul(getMultiplier(lastRewardBlock, block.number)
                ).mul(1e12).div(lastSOboSupply));
        }
        return (sObo.balanceOf(_user).mul(accTokenPerShare).div(1e12).sub(sOboHoldersRewardsInfo[_token][_user].rewardDept))
                    .add(sOboHoldersRewardsInfo[_token][_user].pending);
    }

    function updateSOboHolder(address _sOboHolder) external {
        uint256 holderPreviousSOboAmount = sOboHoldersInfo[_sOboHolder];
        uint256 holderBalance = sObo.balanceOf(_sOboHolder);
        uint256 length = registeredToken.length;
        for (uint256 index = 0; index < length; ++index) {
            _updateTokenPot(registeredToken[index]);
            TokenPot storage tokenPot = _distributedMoneyPot[registeredToken[index]];
            if(holderPreviousSOboAmount > 0 && tokenPot.accTokenPerShare > 0){
                uint256 pending = holderPreviousSOboAmount.mul(tokenPot.accTokenPerShare).div(1e12).sub(sOboHoldersRewardsInfo[registeredToken[index]][_sOboHolder].rewardDept);
                if(pending > 0) {
                    if (_sOboHolder == masterObofinance) {
                        reserveTokenAmount[registeredToken[index]] = reserveTokenAmount[registeredToken[index]].add(pending);
                    }
                    else {
                        sOboHoldersRewardsInfo[registeredToken[index]][_sOboHolder].pending = sOboHoldersRewardsInfo[registeredToken[index]][_sOboHolder].pending.add(pending);
                    }
                }
            }
            sOboHoldersRewardsInfo[registeredToken[index]][_sOboHolder].rewardDept = holderBalance.mul(tokenPot.accTokenPerShare).div(1e12);
        }
        if (holderPreviousSOboAmount > 0){
            lastSOboSupply = lastSOboSupply.sub(holderPreviousSOboAmount);
        }
        lastSOboSupply = lastSOboSupply.add(holderBalance);
        sOboHoldersInfo[_sOboHolder] = holderBalance;
    }

    function harvestRewards(address _sOboHolder) external {
        uint256 length = registeredToken.length;

        for (uint256 index = 0; index < length; ++index) {
            harvestReward(_sOboHolder, registeredToken[index]);
        }
    }

    function harvestReward(address _sOboHolder, address _token) public {
        uint256 holderBalance = sOboHoldersInfo[_sOboHolder];
        _updateTokenPot(_token);
        TokenPot storage tokenPot = _distributedMoneyPot[_token];
        if(holderBalance > 0 && tokenPot.accTokenPerShare > 0){
            uint256 pending = holderBalance.mul(tokenPot.accTokenPerShare).div(1e12).sub(sOboHoldersRewardsInfo[_token][_sOboHolder].rewardDept);
            if(pending > 0) {
                if (_sOboHolder == masterObofinance) {
                    reserveTokenAmount[_token] = reserveTokenAmount[_token].add(pending);
                }
                else {
                    sOboHoldersRewardsInfo[_token][_sOboHolder].pending = sOboHoldersRewardsInfo[_token][_sOboHolder].pending.add(pending);
                }
            }
        }
        if ( sOboHoldersRewardsInfo[_token][_sOboHolder].pending > 0 ){
            safeTokenTransfer(_token, _sOboHolder, sOboHoldersRewardsInfo[_token][_sOboHolder].pending);
            sOboHoldersRewardsInfo[_token][_sOboHolder].pending = 0;
        }
        sOboHoldersRewardsInfo[_token][_sOboHolder].rewardDept = holderBalance.mul(tokenPot.accTokenPerShare).div(1e12);
    }
    ...
    */
    ...
    function safeTokenTransfer(address _token, address _to, uint256 _amount) internal {
        ...
        require(transferSuccess, "safeSOboTransfer: Transfer failed");
    }
...
// File: contracts/SOboToken.sol

pragma solidity 0.6.12;


// SOboToken with Governance.
contract SOboToken is ObofinanceBEP20("Obofinance ShareObo", "sOBO") {
    using SafeMath for uint256;
    ...
    ObofinanceMoneyPot public moneyPot;
    OboToken public obo;
    bool private _isOboSetup = false;
    bool private _isMoneyPotSetup = false;

    uint256 public immutable SWAP_PENALTY_MAX_PERIOD ; // after 24h penalty of holding sObo. Swap penalty is at the minimum
    uint8 public immutable SWAP_PENALTY_MAX_PER_SOBO ; // 30% => 1 sobo = 0.3 obo

    mapping(address => HolderInfo) public holdersInfo;

    constructor (uint256 swapPenaltyMaxPeriod, uint8 swapPenaltyMaxPerSObo) public{
        SWAP_PENALTY_MAX_PERIOD = swapPenaltyMaxPeriod; // default 28800: after 24h penalty of holding sObo. Swap penalty is at the minimum
        SWAP_PENALTY_MAX_PER_SOBO = swapPenaltyMaxPerSObo; // 30% => 1 sobo = 0.3 obo
    }

    function setupObo(OboToken _obo) external onlyOwner {
        require(!_isOboSetup, "Obo: already setup");
        obo = _obo;
        _isOboSetup = true;
    }

    function setupMoneyPot(ObofinanceMoneyPot _moneyPot) external onlyOwner {
    ...
    /**
     * @dev Returns true if the caller is the obo contract.
     */
    function isObo() internal view returns (bool) {
        return msg.sender == address(obo);
    }

    function swapToObo(uint256 _amount) external {
        require(_amount > 0, "amount 0");
        address _from = msg.sender;
        uint256 oboAmount = _swapOboAmount( _from, _amount);
        ...
        if (holdersInfo[_from].amount > 0) {
            uint256 amountTOboSwapped = getSOboAtMinPenalty(_from);
            if (amountTOboSwapped > _amount){
                amountTOboSwapped = amountTOboSwapped.sub(_amount);
            }
            holdersInfo[_from].amountSwapped = holdersInfo[_from].amountSwapped.add(amountTOboSwapped);
        }
        else{
            holdersInfo[_from].amountSwapped = 0;
        }
        obo.mint(_from, oboAmount);

        if (address(moneyPot) != address(0)) {
            moneyPot.updateSOboHolder(_from);
        }
    }

    /* @notice Preview swap return in obo with _sOboAmount by _holderAddress
    *  this function used by front-end to show how much obo will be retrieve if _holderAddress swap _sOboAmount
    */
    function previewSwapOboExpectedAmount(address _holderAddress, uint256 _sOboAmount) external view returns (uint256 expectedObo){
        return _swapOboAmount( _holderAddress, _sOboAmount);
    }

    function getSOboAtMinPenalty(address _holderAddress) public view returns (uint256){
    ...
    /* @notice Calculate the adjustment for a user if he want to swap _sOboAmount to obo */
    function _swapOboAmount(address _holderAddress, uint256 _sOboAmount) internal view returns (uint256 expectedObo){
        HolderInfo storage holderInfo = holdersInfo[_holderAddress];
        require(holderInfo.amount >= _sOboAmount, "Not enough sOBO");
        if(block.number >= holderInfo.avgTransactionBlock.add(SWAP_PENALTY_MAX_PERIOD)){
            return _sOboAmount;
        }

        uint256 _sOboAtMinPenalty = getSOboAtMinPenalty(_holderAddress);

        if (_sOboAtMinPenalty >= _sOboAmount){
            return _sOboAmount;
        }

        uint256 swapPenaltyPerSObo = SWAP_PENALTY_MAX_PER_SOBO.sub(
            (_sOboAtMinPenalty.div(holderInfo.amount)).mul(SWAP_PENALTY_MAX_PER_SOBO)
        );
        uint256 _sOboWithPenalty = _sOboAmount.sub(_sOboAtMinPenalty);
        uint256 _oboForSOboWithPenalty = _sOboWithPenalty.sub(_sOboWithPenalty.mul(swapPenaltyPerSObo).div(100));
        return _oboForSOboWithPenalty.add(_sOboAtMinPenalty);
    }

    /* @notice Calculate average deposit/withdraw block for _holderAddress */
    function _getAvgTransactionBlock(HolderInfo storage holderInfo, uint256 _sOboAmount, bool _onWithdraw) internal view returns (uint256){
        if (holderInfo.amount == 0) {
            return block.number;
        }
        uint256 transactionBlockWeight;
        if (_onWithdraw) {
            if (holderInfo.amount == _sOboAmount) {
                return 0;
            }
            else {
            ...
            transactionBlockWeight = (holderInfo.amount.mul(holderInfo.avgTransactionBlock).add(block.number.mul(_sOboAmount)));
        }
        return transactionBlockWeight.div(holderInfo.amount.add(_sOboAmount));
    }
    ...
    function mint(address _to, uint256 _amount) external onlyOwner() {
        ...
        if (address(moneyPot) != address(0)) {
            moneyPot.updateSOboHolder(_to);
        }
    }

    /// @dev overrides transfer function to meet tokenomics of OBO
    function _transfer(address _sender, address _recipient, uint256 _amount) internal virtual override {
        ...
        if (_recipient == BURN_ADDRESS) {
            ...
            if (address(moneyPot) != address(0)) {
                moneyPot.updateSOboHolder(_sender);
            }
            ...
            if (address(moneyPot) != address(0)) {
                ...
                if (_sender != _recipient){
                    moneyPot.updateSOboHolder(_recipient);
                }
            }
        }
    ...
    internal
    {
        ...
        // balance of underlying OBOs (not scaled);
...
contract OboToken is ObofinanceBEP20("Obofinance Obo", "OBO") {

    address public constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;
    SOboToken public sObo;
    bool private _isSOboSetup = false;

    /*
     * @dev Throws if called by any account other than the owner or sObo
     */
    modifier onlyOwnerOrSObo() {
        require(isOwner() || isSObo(), "caller is not the owner or sObo");
        _;
    }
    ...
    /**
     * @dev Returns true if the caller is sObo contracts.
     */
    function isSObo() internal view returns (bool) {
        return msg.sender == address(sObo);
    }

    function setupSObo(SOboToken _sObo) external onlyOwner{
        require(!_isSOboSetup, "sObo: already setup");
        sObo = _sObo;
        _isSOboSetup = true;
    }

    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterObofinance).
    function mint(address _to, uint256 _amount) external onlyOwnerOrSObo {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (recipient == BURN_ADDRESS) {
            ...
            require(amount == sendAmount + burnAmount, "OBO::transfer: Burn value invalid");
...
contract ObofinanceBonusAggregator is Ownable{
    using SafeMath for uint256;
    using SafeMath for uint16;

    MasterObofinance master;
    ...
    function setupMaster(MasterObofinance _master) external onlyOwner{
        master = _master;
    }
...
// File: contracts/MasterObofinance.sol

pragma solidity 0.6.12;

// import "@nomiclabs/buidler/console.sol";

// MasterObofinance is the master of OBO AND THO.
// He can make Obo and he is a fair guy.
//
...
contract MasterObofinance is Ownable {
    using SafeMath for uint256;
    using SafeMath for uint16;
    using SafeBEP20 for IBEP20;
    ...
    ObofinanceBonusAggregator public bonusAggregator;
    // The OBO TOKEN!
    OboToken public obo;
    // The SOBO TOKEN!
    SOboToken public sObo;
    ...
    // Initial emission rate: 1 OBO per block.
    uint256 public immutable initialEmissionRate;
    // Minimum emission rate: 0.1 OBO per block.
    ...
    constructor(
        OboToken _obo,
        SOboToken _sobo,
        ObofinanceBonusAggregator _bonusAggregator,
        address _devaddr,
        address _feeAddress,
        ...
        startBlock = _startBlock;
        initialEmissionRate = _oboPerBlock;
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, bool _isSOboRewards, bool _withUpdate) external onlyOwner {
        ...
        poolInfo.push(PoolInfo({
            ...
            isSOboRewards: _isSOboRewards
        }));
    }

    // Update the given pool's OBO allocation point. Can only be called by the owner.
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, bool _isSOboRewards, bool _withUpdate) external onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
        ...
        poolInfo[_pid].isSOboRewards = _isSOboRewards;
    ...
    function pendingObo(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return user.amountWithBonus.mul(accOboPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updateEmissionRate() internal {
        ...
        if(oboPerBlock <= minimumEmissionRate){
            return;
        }
        ...
        uint256 newEmissionRate = oboPerBlock;
        ...
        if (newEmissionRate >= oboPerBlock) {
            return;
        }

        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = oboPerBlock;
        oboPerBlock = newEmissionRate;
    ...
    function updatePool(uint256 _pid) public validatePool(_pid) {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 oboReward = multiplier.mul(oboPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        uint256 devMintAmount = oboReward.div(10);
        obo.mint(devaddr, devMintAmount);
        if (pool.isSOboRewards){
            sObo.mint(address(this), oboReward);
        }
        else{
    ...
    function updateUserBonus(address _user, uint256 _pid, uint256 bonus) external  validatePool(_pid) onlyAggregator{
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amountWithBonus.mul(pool.accOboPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                if(pool.isSOboRewards){
                    safeSOboTransfer(_user, pending);
                }
                else{
                    safeOboTransfer(_user, pending);
                }
            }
        }
        ...
        user.rewardDebt = user.amountWithBonus.mul(pool.accOboPerShare).div(1e12);
    }

    // Deposit LP tokens to MasterObofinance for OBO allocation.
    function deposit(uint256 _pid, uint256 _amount) external validatePool(_pid) {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(obo)) {
    ...
    // Withdraw LP tokens from MasterObofinance.
    function withdraw(uint256 _pid, uint256 _amount) external validatePool(_pid) {
        ...
        if(pending > 0) {
            if(pool.isSOboRewards){
                safeSOboTransfer(msg.sender, pending);
            }
            else{
    ...
    returns(address lpToken, uint256 allocPoint, uint256 lastRewardBlock,
            uint256 accOboPerShare, uint16 depositFeeBP, bool isSOboRewards) {
        return (
            address(poolInfo[_pid].lpToken),
            poolInfo[_pid].allocPoint,
            poolInfo[_pid].lastRewardBlock,
            poolInfo[_pid].accOboPerShare,
            poolInfo[_pid].depositFeeBP,
            poolInfo[_pid].isSOboRewards
        );
    }
    ...
    function safeOboTransfer(address _to, uint256 _amount) internal {
        ...
        bool transferSuccess = false;
        if (_amount > oboBal) {
            transferSuccess = obo.transfer(_to, oboBal);
        } else {
            transferSuccess = obo.transfer(_to, _amount);
        }
        require(transferSuccess, "safeOboTransfer: Transfer failed");
    }

    // Safe sObo transfer function, just in case if rounding error causes pool to not have enough SOBOs.
    function safeSOboTransfer(address _to, uint256 _amount) internal {
        uint256 sOboBal = sObo.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > sOboBal) {
            transferSuccess = sObo.transfer(_to, sOboBal);
        } else {
            transferSuccess = sObo.transfer(_to, _amount);
        }
        require(transferSuccess, "safeSOboTransfer: Transfer failed");
    }
