...
contract MasterChefBrainV2 is Ownable {
...
import "@openzeppelin/contracts/access/Ownable.sol";
import "./BrainiacInterface.sol";

contract BrainiacToken is BrainiacTokenInterface {
...
abstract contract BrainiacTokenInterface is ERC20, Ownable {
    /// @notice Total number of tokens
    uint256 public maxSupply = 1_000_000_000e18; // 1 Billion Braniac

    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (Master Brain).
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying BRAINs (not scaled);
