...
contract MoonOwlToken is Context, IBEP, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using Address for address;
    
    address constant blackholeAddress = 0x000000000000000000000000000000000000dEaD;
    address payable public charityAddress = 0x41f1F4276922e5fa0743b8aEbf5Ee6FAF8613722;
    ...
    IPancakeSwapRouter02 immutable public pancakeSwapRouter;
    ...
    constructor (
        ...
        IPancakeSwapRouter02 _pancakeSwapRouter = IPancakeSwapRouter02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
    ...
    function _swapETHForTokens(
        ...
        // generate the QuickSwap pair path of token -> wBNB
        address[] memory path = new address[](2);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        if (charityFee > 0){
         if (sender != address(this) && liquidityAdded){
               ...
               forCharity = amount.mul(charityFee).div(100);
                    ...
                    if(balanceAfter > balanceBefore)
                        swapTokensForBNBForCharity(balanceAfter.sub(balanceBefore));
            }
          }
        }
    ...
    uint256 public disruptiveCoverageFee = 5 ether; // antiwhale
    ...
    uint256 public rewardThreshold = 5 ether; // 5 BNB
    ...
    function calculateBNBReward(address ofAddress) public view returns (uint256) {	
        ...
        .sub(balanceOf(address(pancakeSwapPair)));	
    ...
    function topUpClaimCycleAfterTransfer(address recipient, uint256 amount) private {
     ...
     function swapTokensForBNBForCharity(
        uint256 tokenAmount
    ) public {
        _approve(address(this), address(pancakeSwapRouter), tokenAmount);
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        !(from == address(this) && to == address(pancakeSwapPair)) // swap 1 time
        ) {
            ...
            _addLiquidity(otherPiece, BNBToBeAddedToLiquidity);

            emit SwapAndLiquify(piece, deltaBalance, otherPiece);
        }
    }
    
    function changeRewardCycleBlock(uint256 _days) external onlyOwner{
        require(_days >= 1);
        require(_days <= 7);
        rewardCycleBlock = _days;
    }
    ...
    function changeCharityFee(uint256 n) external onlyOwner {
        require(n <= 5, "charityFee can not be more than 5%");
        charityFee = n;
    }
    ...
    function activateContract() public onlyOwner {
        ...
        // protocol
        disruptiveCoverageFee = 1000 ether; //1000 BNB
        disruptiveTransferEnabledFrom = block.timestamp;
        setMaxTxPercent(10); // 0.1% per transaction
        liquidityAdded = true;
