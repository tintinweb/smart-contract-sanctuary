...
contract EggToken is ERC20("EGG", "EGG") {
    ...
    event Claim(uint256 orderId, uint256 amount, address userAddress);
    ...
    constructor() public {
        _setupDecimals(18);
        _mint(msg.sender, uint256(1333333).mul(1e18));
        owner = msg.sender;
    }

    function setDev1(address _dev) public onlyOwner {
        dev1 = _dev;
    }

    function setDev2(address _dev) public onlyOwner {
        dev2 = _dev;
    }

    function claim(uint256 orderId, uint256 amount, uint256, bytes memory signature) public returns(address){
        ...
        return signer;
        /*require(signer == dev1 || signer == dev2, "invalid signer");

        transfer(msg.sender, amount);

        claimedOrderId[orderId] = true;

        emit Claim(orderId, amount, msg.sender);*/
    }
