...
contract HypeRockets is ERC1155, Ownable {
    ...
    function setMaxTokensPerMint(uint _maxTokensPerMint) external onlyOwner {
        maxTokensPerMint = _maxTokensPerMint;
    }
    // End Setters region

    // Sale stated
    function startSale(uint _maxTokensPerMint, IERC1155 _ticketsContract) external onlyOwner {
        isSaleStarted = true;
        maxTokensPerMint = _maxTokensPerMint;
    ...
    modifier maxSupplyCheck(uint amount)  {
        ...
        require(amount <= maxTokensPerMint, "You can't mint more than maxTokensPerMint tokens");
        _;
    }
    ...
    function claim(uint amount) external {
        require(isSaleStarted, "Sale is not started");
        ticketsContract.safeTransferFrom(msg.sender, owner(), TICKET_ID, amount, "");
        mintNFTs(amount);
    }

    function mintNFTs(uint amount) internal maxSupplyCheck(amount) {
        for (uint i = 0; i < amount; i++) {
            _mint(msg.sender, rocketsMinted + i + 1, 1, "");
        }
