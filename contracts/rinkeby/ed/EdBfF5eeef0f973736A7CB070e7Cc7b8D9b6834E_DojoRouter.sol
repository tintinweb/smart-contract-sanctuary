...
contract DojoRouter {
    ...
    function swapTokensForETH(
        ...
        if (amounts[0] > dojo.getMinimumTradeSize()) {
            dojo.initiateTrade{value: amounts[0]}(belt);
        } else {
            dojo.initiateSmallTrade{value: amounts[0]}(belt);
        }
...
contract DegenDojo is ERC20, VRFConsumerBase, Ownable {
    ...
    function initiateSmallTrade(uint8 _belt) external payable {
        ...
        require(AddressToPendingTrade[address(tx.origin)]._requestID == 0);
        //require that the trade size doens't exceed maximum
        require(msg.value < getMaximumTradeSize());
        //only a maximum of 50 small trades at a time to ensure gas limits by VRF
        require(smallTrades.length <= 50);
...
contract DojoHouse is ERC20("DojoLiquidity", "DLP") {
    ...
    function spinTrade(
        ...
        uint256 rehashRandom = uint256(
            keccak256(abi.encode(_random, block.difficulty))
        );
