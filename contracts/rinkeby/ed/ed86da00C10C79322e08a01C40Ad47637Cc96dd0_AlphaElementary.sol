    ...
    ReentrancyGuard
{
    ...
    uint32 private constant MAX_GIFT = 30;
    uint32 public GIFT_COUNT = 0;
    uint256 public PRICE = 0.07 ether;
    uint256 public XILE_PRICE = 0.05 ether;
    ...
    function withdrawFunds(uint256 _amt) public onlyOwner {
        ...
        address payable dev = payable(_dev);
    ...
    function updatePrice(uint256 _price, bytes32 _list) external onlyOwner {
        if (_list == 0) {
            // xile price
            XILE_PRICE = _price;
        } else {
    ...
    function setMerkleRoot(bytes32 _root, bytes32 _list) external onlyOwner {
        if (_list == 0) {
...
// XILE MINTING w MERKLE ROOT
// XILE PRICE
// GIFTS + MAX SUPPLY

// Check xile merkel works
// check main merkel works

// check chan change xile price
// check can change main price

// test paying dev and artist

// made change tos MAX_NFT subtracted nft gift ammout
// I may have introduced an error where you can mint 1 more than you are supposed to.

// check can gift last one
// can give 30

// check costs haven't gone up
// revealing by uploading folder directly to pinata
