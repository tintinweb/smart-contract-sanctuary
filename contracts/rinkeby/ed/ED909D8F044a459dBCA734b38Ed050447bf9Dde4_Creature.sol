...
library SafeMath {
    ...
    function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        // Gas optimization: this is cheCreaturer than requiring 'a' not being zero, but the
...
library EnumerableMap {
    ...
    function _set(Map storage map, bytes32 key, bytes32 value) private returns (bool) {
        ...
        if (keyIndex == 0) { // Equivalent to !contains(map, key)
            map._entries.push(MCreaturentry({ _key: key, _value: value }));
    ...
    function _remove(Map storage map, bytes32 key) private returns (bool) {
        ...
        if (keyIndex != 0) { // Equivalent to contains(map, key)
            ...
            MCreaturentry storage lastEntry = map._entries[lastIndex];
    ...
    */
    function _at(Map storage map, uint256 index) private view returns (bytes32, bytes32) {
        ...
        MCreaturentry storage entry = map._entries[index];
...
contract Creature is ERC721, Ownable {
    ...
    */
    function mintCreature(uint numberOfTokens) public payable {
        ...
        require(numberOfTokens <= maxCreaturePurchase, "Can only mint 20 tokens at a time");
