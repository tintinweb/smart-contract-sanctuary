...
contract Yammies is ERC721, Ownable {
    ...
    function reserveYammies(address _to, uint256 _count) public onlyOwner {        
        uint256 supply = totalSupply();
        require(_count > 0 && _count <= reservedAmount, "Not enough reserves left");
        for (uint256 i = 0; i < _count; i++) {
            _safeMint(_to, supply + i);
        }
        reservedAmount = reservedAmount.sub(_count);
    }
