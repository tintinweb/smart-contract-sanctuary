...
contract Market is OwnableContract,MarketReentrancyGuard,Initializable,IMarket{
    ...
    function mintXNftAndOnLent (
        ...
        uint64 minDuration,
        uint256 pricePerSecond
    ) public nonReentrant whenNotPaused onlyApprovedOrOwner(msg.sender,IBaseDoNFT(doNftAddress).getOrignalNftAddress(),oNftId){
    ...
    function offLent(address nftAddress, uint256 nftId) public whenNotPaused onlyApprovedOrOwner(msg.sender,nftAddress,nftId){
    ...
    function makeDeal(address nftAddress,uint256 tokenId,uint256 durationId,uint64 startTime,uint64 endTime) public whenNotPaused nonReentrant payable virtual returns(uint256 tid){
    ...
    function distributePayment(address nftAddress,uint256 nftId,uint64 startTime,uint64 endTime) internal returns (uint256 totalPrice,uint256 leftTotalPrice,uint256 curFee,uint256 curRoyalty){
        ...
        require(msg.value >= totalPrice,"need more");
