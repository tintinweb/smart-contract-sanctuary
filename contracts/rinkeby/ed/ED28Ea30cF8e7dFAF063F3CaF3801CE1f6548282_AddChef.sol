...
contract AddChef is Ownable {
    ...
    function setPoolMultipler(
        ...
        // require((_fromBlock < endBlock) && (_toBlock <= endBlock), "MC: Invalid Multiplier Info");
    ...
    function add(
        ...
        poolInfo.push(
            PoolInfo({
                ...
                accAddPerShare: 0,
                isStopped: false,
                epochMultiplers: _epochMultiplers,
                epochMultiplersValue: _epochMultiplersValue
            })
        );
    }
    ...
    function getMultiplier(
        ...
        */
        ...
        uint256 poolMultiplierLength = pool.epochMultiplers.length;

        uint256 sumOfMultiplier = 0;

        // if(_from < pool.fromBlock){
        //     return 0;
        // }
        ...
        for (uint256 index = 0; index < poolMultiplierLength; index++) {
             if (_from<= pool.epochMultiplers[index] && _to <= pool.epochMultiplers[index]) {
             sumOfMultiplier.add(_to.sub(_from).mul(pool.epochMultiplersValue[index]));
        }
        }
