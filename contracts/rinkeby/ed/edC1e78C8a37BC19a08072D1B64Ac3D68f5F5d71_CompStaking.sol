...
contract CompStaking {
    ...
    address  public compContract= address(0xf63FABd25c528C9888dbeb77ffdD165c1E4f5f85);
    event Staking(address indexed from, uint32 compId,uint32 power,uint256 amount);
...
//            if(block.number<13600000){ //stop time
            //todo
            if(block.number<9164590){ //stop time
  ...
  */
    ...
    function staking(uint32[] memory compIdList , address poolTokenContract) public returns (bool){
        ...
        for(uint8 i=0;i<compIdList.length;i++){
            ...
            uint256 increaceAmoount = _currentStakingAmount(power,poolTokenContract);
            poolInfo.totalPower +=  power;

            stakingAmount+=increaceAmoount;

            emit Staking(msg.sender, compIdList[i],power,increaceAmoount);
        }
  ...
  */
    function redeem(uint32 compId, address poolTokenContract)public returns (bool){
        ...
        erc20.transfer(msg.sender, redeemAmount);  //5% free
