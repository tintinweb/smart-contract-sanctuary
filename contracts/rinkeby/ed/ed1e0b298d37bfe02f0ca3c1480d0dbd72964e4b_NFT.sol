...
contract NFT is ERC721Enumerable, Ownable {
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
        if(onlyWhitelistedAdd == true){
            require(isWhitelisted(msg.sender), "This Address is not whitelisted");
            uint256 ownerTokenCount = balanceOf(msg.sender);
            require(ownerTokenCount + _mintAmount <= maxMintAmountPerWhitelist, "Exceed Minting Limit");
        }
  ...
  function isWhitelisted(address _user) public view returns (bool) {
      for (uint256 i = 0; i < WhitelistedAdd.length; i++){
          if (WhitelistedAdd[i] == _user){
              return true;
          }
      }
      return false;
    ...
    returns (string memory)
  {
   ...
   function mintReserved(uint256 tokensAmount) public onlyOwner {
        require(
              _tokenIdCounter.current() + tokensAmount <= maxSupply,
  ...
  function setOnlyWhitelistedSales(bool _state) public onlyOwner {
    onlyWhitelistedAdd = _state;
  }
  
  function setWhitelistAdd(address[] calldata _users) public onlyOwner {
      delete WhitelistedAdd;
      WhitelistedAdd = _users;
  }
