...
contract AlphaPass is ERC721Enumerable, CollaborativeOwnable {
  using SafeMath for uint256;
  using Address for address;
  using Strings for uint256;

  event MintAlphaPass(address indexed sender, uint256 startWith);
  ...
  uint256 public ethMintPrice = 25000000000000000;
  uint256 public mintedWithEth = 0;
  uint256 public ethMintMax = 250;
  bool public ethMintIsActive = false;

  uint256 public tokenMintPrice = 150 ether;
  uint256 public constant tokenMintPriceStep = 50;
  uint256 public constant tokenMintPriceIncrement = 30 ether;
  uint256 public mintedWithTokens = 0;
  bool public tokenMintIsActive = false;

  mapping(address => bool) private _prepaidList;
  uint256 public reservedForPrepaid = 0;
  bool public prepaidMintIsActive = false;

  constructor() ERC721("Night Owls Alpha Pass", "OWL") {}
  ...
  function prepaidMint() external {
    require(prepaidMintIsActive);
    require(_prepaidList[_msgSender()]);

    uint256 ts = totalSupply();
    emit MintAlphaPass(_msgSender(), ts);
    _safeMint(_msgSender(), ts);

    _prepaidList[_msgSender()] = false;
    reservedForPrepaid--;
  }

  function mintWithEth() external payable {
    require(ethMintIsActive);
    require(remainingForMint() > 0);
    require(msg.value >= ethMintPrice);
    require(mintedWithEth < ethMintMax);
  ...
  function mintWithTokens() external {
    ...
    if (mintedWithTokens % tokenMintPriceStep == 0) {
      tokenMintPrice = tokenMintPrice.add(tokenMintPriceIncrement);
    }
  }

  function isPrepaid(address addr) external view returns (bool) {
  ...
  function setEthMintPrice(uint256 newPrice) external onlyCollaborator {
  ...
  function setEthMintIsActive(bool active) external onlyCollaborator {
    ethMintIsActive = active;
  }

  function reduceEthMintMax(uint256 newMax) external onlyCollaborator {
    require(newMax >= 0 && newMax < ethMintMax);
    require(newMax >= mintedWithEth);
    ethMintMax = newMax;
  }
  ...
  function addToPrepaidList(address[] calldata data) external onlyCollaborator {
    for (uint256 i = 0; i < data.length; i++) {
      bool existing = _prepaidList[data[i]];
      if (!existing) {
        _prepaidList[data[i]] = true;
       reservedForPrepaid++;
      }
    }
  }
