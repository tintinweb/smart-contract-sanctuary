...
contract MetaSafeNFT is ERC721("MetaSafe", "MS"), ERC721Enumerable, Ownable {
  ...
  */
  ...
  uint256 public NFTPrice = 120000000000000;  // 0.00012 ETH
  ...
  mapping(address => uint256) private presaleWhiteListClaimed;
  ...
  */
    ...
    onlyOwner
  {
    for (uint256 i = 0; i < _addresses.length; i++) {
      ...
      */
      presaleWhiteListClaimed[_addresses[i]] > 0 ? presaleWhiteListClaimed[_addresses[i]] : 0;
    }
  }
  ...
  */ 
    ...
    payable
  {
    require(isActive, 'Contract is not active');
    require(isPresaleActive, 'Only whiteing from White List');
    require(presaleWhiteList[msg.sender], 'You are not on the White List');
    ...
    require(totalSupply().add(_numOfTokens).sub(giveawayCount) <= MAX_NFT_PUBLIC, 'Purchase would exceed max public supply of NFTs');
    require(presaleWhiteListClaimed[msg.sender].add(_numOfTokens) <= WHITELIST_MAX_MINT, 'Purchase exceeds max whiteed');
    require(NFTPrice.mul(_numOfTokens) == msg.value, "Ether value sent is not correct");
    
    for (uint256 i = 0; i < _numOfTokens; i++) {
      presaleWhiteListClaimed[msg.sender] += 1;
      _safeMint(msg.sender, totalSupply().sub(giveawayCount));
    }
  }
  ...
  */
   ...
   * URI will be blank untill reveal is not true
   * This method is very important for Opensea
  */
  function tokenURI(
      uint256 _tokenId
  ) 
