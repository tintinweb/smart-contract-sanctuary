...
contract LimitedPhotographies is ERC721, EIP712, ERC721Enumerable {
    ...
    uint16 public constant MAX_SUPPLY = 42;
    ...
    // @notice Represents tokenId to photo map
    ...
    constructor(address payable _owner)
        ERC721("LimitedPhotographies", "LPS")
    ...
    function redeem(address redeemer, Voucher calldata voucher) public payable {
        ...
        require(MAX_SUPPLY >= voucher.tokenId, "All photos minted");
        ...
        // set infromation
        photos[voucher.tokenId].rarity = voucher.rarity;
        photos[voucher.tokenId].message = voucher.message;

        // emit event
        emit Redeemed(redeemer, voucher.tokenId);
    }
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "Voucher(uint256 tokenId,uint256 minPrice,string rarity,string message)"
                        ),
                        voucher.tokenId,
                        voucher.minPrice,
