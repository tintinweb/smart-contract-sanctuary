...
contract Hancom721 is Ownable, ERC721 {
    ...
    uint16 private _shareRateA = 5500;    // B의 수수료율은 (10000 - shareRateA) / 100
    ...
    function mintToken(string memory title, string memory uri, uint16 royalty,
        ...
        //require(tokens[tokenId].minter == address(0));
        ...
        // ERC721 function uri use the value added baseURI and uri
        // -> input value : uri
        ...
        //tokens[tokenId] = Token(true, title, uri, royalty, payable(_msgSender()), payable(_msgSender()));
    ...
    function buyToken(uint256 saleId, uint256 dataId) external payable {
...

        {
            ...
            //(bool success, bytes memory data) = _distributor.delegatecall(
...

        {
            ...
            //safeTransferFrom(sale.seller, _msgSender(), sale.tokenId);
        }
    ...
    function changeSale(uint256 tokenId, uint256 saleId, bool putOnSale, uint16 currencyId, uint256 price, uint256 dataId) external {
        ...
        if (putOnSale) {
            if (!sales[changeSaleId].isPresent) {// create
    ...
    function burn(uint256 tokenId) public {
        require(tokens[tokenId].isPresent);
        // 메세지센더가 토큰 소유자거나 컨트랙트 오너일때만 소각 가능
    ...
    // NOTE: https://github.com/rarible/protocol-contracts/blob/master/tokens/contracts/erc-721/ERC721BurnableUpgradeable.sol
