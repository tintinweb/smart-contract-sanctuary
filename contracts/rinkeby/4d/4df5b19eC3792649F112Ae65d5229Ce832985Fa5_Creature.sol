...
contract Creature is ERC721Tradable {
    ...
    IERC721Enumerable[] private contracts;
    ...
    function getMintable(address user) public view returns (uint256[] memory) {
        uint256[][] memory tokensByKind = new uint256[][](contracts.length);
        uint256 total = 0;
        for (uint256 k = 0; k < contracts.length; k++) {
            tokensByKind[k] = getMintableFor(user, k);
    ...
    function getMintableFor(address user, uint kind) private view returns (uint256[] memory) {
        IERC721Enumerable cont = contracts[kind];
        uint256 count = cont.balanceOf(user);
        uint256[] memory tokens = new uint256[](count);
        for (uint256 i = 0; i < count; i++) {
            try cont.tokenOfOwnerByIndex(user, i) returns (uint256 externTokenId) {
