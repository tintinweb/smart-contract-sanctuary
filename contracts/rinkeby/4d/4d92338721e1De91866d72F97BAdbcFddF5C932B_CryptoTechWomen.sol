...
contract CryptoTechWomen is ERC721, Ownable, ReentrancyGuard {
    ...
    // uint16 constant public maxTotalSupply = 8888;   // Max amount of tokens available for mint in total
    uint16 constant public maxTotalSupply = 50;   // Max amount of tokens available for mint in total
    uint16 constant public preSaleTokenLimit = 4000;    // Max amount of tokens available for mint at the pre sale
    uint8 constant public preSaleLimitPerUser = 3;  // Max amount of tokens available for mint at the pre sale per address
    // uint8 constant public amountForGiveAway = 150;  // Amount of tokens reserved for give away
    uint8 constant public amountForGiveAway = 10;  // Amount of tokens reserved for give away
    ...
    bytes32 immutable public merkleRoot;    // Merkle root for the whitelist
    ...
    event GiveAway(uint256[] ids, address[] addresses);
    ...
    */
    ...
    /*
    * @notice Distrubites specified amount of ETH between 15 members of Royalty Club
    * @dev Only owner can call it
    * @param _amount Amount of ETH to be distributed
    * @param _members Members' addresses array
    */
    function distributeRoyalties(uint256 _amount, address[] memory _members) external onlyOwner {
        uint256 toSend = _amount / 15;
        for (uint256 i; i < 15; i++) {
            _sendETH(payable(_members[i]), toSend);
        }
    }
    ...
    */
    function preSaleMint(uint256 _amount, uint256 _userId, bytes32[] memory _proof) external payable nonReentrant {
        require(preSaleMinted[_msgSender()] == 0, "already registered");
        require(_verify(_leaf(_msgSender(), _userId), _proof), "incorrect proof");
    ...
    */
    function publicSaleMint(uint256 _amount) external payable nonReentrant {
        ...
        uint256 totalAvailable = maxTotalSupply - amountForGiveAway;
        uint256 totalSupplyAfterMint = totalSupply + _amount;
        require(totalSupplyAfterMint <= totalAvailable, "already sold out");
        _buyAndRefund(_amount, publicSalePrice);
        if (totalSupplyAfterMint == totalAvailable) emit SoldOut();
    ...
    */
    function giveaway(address[] memory _accounts, uint256[] memory _ids) external onlyOwner {
        require(_accounts.length == _ids.length, "invalid lengths");
        uint256 maxTotSup = maxTotalSupply;
        require(totalSupply + _ids.length <= maxTotSup, "maxTotalSupply exceeded");
        uint256 totalAvailable = maxTotSup - amountForGiveAway;
        uint256 counter;
        for (; counter < _ids.length; counter++) {
            require(_ids[counter] >= totalAvailable && _ids[counter] < maxTotSup, "id is out of range");   // first ID to be given away is 8888 - 150 = 8738, last ID is 8887                
            _safeMint(_accounts[counter], _ids[counter]);
        }
        totalSupply += uint16(counter);
        emit GiveAway(_ids, _accounts);
    }
    ...
    */
    ...
    /*
    * @dev Checks if a milestone reached, do some logic if yes. 
    * Emits MilestoneReached event
    * @param _totalCost The total cost
    * @param _mintCounter The mint counter 
    */
    function _checkMilestones(uint256 _totalCost, uint256 _mintCounter) private {
        Milestone memory milestone = milestones[0];
        if (_mintCounter >= milestone.triggerId) {
            uint256 toPay = bpsToDonationFund * _totalCost / BPS_BASE;
        ...
        if (
            ...
            _mintCounter >= milestone.triggerId
        ) {
            emit MilestoneReached(milestone.triggerId);
    ...
    */
    function _leaf(address _account, uint256 _userId)
    internal pure returns (bytes32)
    {
        return keccak256(abi.encodePacked(_userId, _account));
    }
