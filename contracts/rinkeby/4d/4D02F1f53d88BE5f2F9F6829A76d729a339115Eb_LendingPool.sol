    ...
    ILendingPool
{
    ...
    function pendingDToken(address _userAddress) external returns(uint256){
        return dao.getPendingDToken(_userAddress);
    }
        ...
        returns (uint256)
    {
        ...
        if (getBlockNumber() > pool.accrualBlockNumber && totalSupply != 0) {
            ...
            accTokenPerShare = accTokenPerShare.add(
                tokenReward.mul(1e18).div(totalSupply)
            );
        }
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            totalBorrow = totalBorrow.add(
                totalBorrow.mul(blockDelta.mul(pool.borrowIndex))
            );
        }
        return totalBorrow.sub(deed.borrow).div(1e18);
    }
    ...
    function updatePool(address _user, address _token) internal {
        ...
        uint256 dividedReward = tokenReward.mul(REWARD_FACTOR).div(BASE_DENOMINATOR); // Divide tokenReward in half
        if (dividedReward > 0) {
