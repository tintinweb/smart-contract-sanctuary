...
contract ICO is Ownable, ReentrancyGuard {
    ...
    function _changeCurrentAllocatedTokens(uint256 amount, uint256 ethForTokens) private {
        if (amount <= current_allocatedTokens) {
            ...
            ethInStep[currentIterationOfICO] = ethInStep[currentIterationOfICO].add(ethForTokens);
        } else {
    ...
    function getRewardAmount() public view returns (uint256 amount) {
        ...
        for (uint256 i = 0; i < currentIterationOfICO; i++) {
            ...
            amount = amount.add((ethInStep[i].div(getTokensPerIteration(i).div(10 ** 10))).mul(usersRewards[msg.sender][i]
            .amountTokens.div(10 ** 10)));
        }
    }
    ...
    allowedInvestor whenIcoStart {
        ...
        uint256 iteration = currentIterationOfICO;
        buy();
        (bool success,) = referral.call{value : bonusAmount}("");
        require(success);
        _subRewardAmount(bonusAmount, iteration);
    }

    function _subRewardAmount(uint256 bonusAmount, uint256 iteration) private {
        ethInStep[iteration] = ethInStep[iteration].sub(bonusAmount);
    }
