...
contract CarOwnership is IFactoryERC721, CarFactory, ERC721Enumerable {
    string _ContractURL = "https://37.194.178.161:45456/api/GlobalMeta";
    mapping(uint256 => Car) public IdToCar; // кэш машин по ID
    uint256 _CurMintId = 1; // текщий ID токена для минта

    constructor() ERC721("MusicRacer", "MRCR") {}
    ...
    /// @dev используется для вычисления адресов через tokenURI(uint256 tokenId), который дописывает ID токена вконец
    function _baseURI() internal view override returns (string memory) {
        return "https://37.194.178.161:45456/api/Cars/";
    }
    ...
    function mint(uint256 _optionId, address _toAddress) external override {
        IdToCar[_CurMintId] = CreateCar(_optionId);
...
struct CarType {
    ...
    /// @notice сколько всего имеется текстур
    uint256 TexturesCount;
    /// @notice все возможные цвета, для каджого типа текстуры
...
/// @title описание одной машины
struct Car {
    /// @notice ID типа машины (больше 0)
    uint256 TypeId;
    /// @notice установленные части машины 0 - часть не задана
    uint256[] Parts;
    /// @notice ID текстуры (может быть 0)
    uint256 TextureId;
...
contract CarFactory is MultyOwned {
    ...
    function CreateCar(uint256 carTypeId) internal returns (Car memory) {
        // ограничитель
        require(carTypeId < CarTypesCount, "incorrect car type id");
        // берем тип машины
        CarType storage carType = CarsTypes[carTypeId];

        // больше определенного количества машины не создаются
        require(
            carType.Count < carType.MaxCount,
            "iall cars in type is minted"
        );

        // генерация ID текстуры
        uint256 textureId = _Random() % carType.TexturesCount;
        // генерация цвета
        uint256 color;
        if (carType.TexturesCount > 0) {
        ...
        for (uint256 i = 0; i < carType.PartTypes.length; ++i) {
            ...
            if (partType.CanBeNull) parts[i] = _Random() % partType.Count;
            else parts[i] = 1 + (_Random() % partType.Count);
        }

        // создаем машину
        Car memory newCar = Car(carTypeId, parts, textureId, color);
...
contract MultyOwned {
    ...
    function SetOwnerRequest(address newOwner) public onlyOwner { // todo доделать так, чтобы можно было назначить цену в эфире, для продажи
        _OwnerRequest[GetOwnerIndex(msg.sender)] = newOwner;
    }

    /// @dev принимает запрос на получение права владения
    function ApplyOwnerRequest() public { // todo сделать платной, чтобы продавать
        for (uint8 i = 0; i < OWNERS_COUNT; ++i)
