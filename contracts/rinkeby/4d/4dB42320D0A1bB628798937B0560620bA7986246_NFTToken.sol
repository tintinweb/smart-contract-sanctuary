...
contract NFTToken is INFTToken, ERC721, Managed {
    ...
    function getIds(
        ...
        uint256 to =
            (_offset.add(_limit)).min(_nftsByUser[_owner].length()).max(
                _offset
            );
    ...
    {
        require(count < totalSupply, "NFTF: the coins ran out");
        count = count + 1;
        _tokenIds.increment();
    ...
    function _beforeTokenTransfer(
        ...
        require(
            hasPermission(_msgSender(), CAN_TRANSFER_NFT) || isTransferOpen,
