...
contract LostAvatars is Ownable, ERC721Enumerable, ERC721URIStorage {
    ...
    uint256 private _mintingCost = 50000000000000000;
    uint256 private constant _reserved = 600;
...
constructor(address initialRoyaltiesReceiver) ERC721("LostAvatars", "LOAV") {
    ...
    {
        require(totalSupply() <= MAX_SUPPLY - _reserved, "All tokens minted");
        require(bytes(hash).length > 0); // dev: Hash can not be empty!
        require(hashes[hash] != 1); // dev: Can't use the same hash twice
        require(msg.value >= _mintingCost,"LostAvatars: Inconsistent amount sent!");
