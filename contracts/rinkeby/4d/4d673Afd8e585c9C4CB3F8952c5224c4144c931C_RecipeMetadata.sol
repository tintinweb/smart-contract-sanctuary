...
import "./ItemMetadata.sol";
import "./MaterialMetadata.sol";
import "./MaterialMetadata.sol";
import "./Ownable.sol";
import "../Recipe.sol";
...
contract RecipeMetadata is MaterialMetadata, ItemMetadata, Ownable {


    constructor() {
    }  

    function setOwner(address _recipe) onlyOwner public {
        transferOwnership(_recipe);
    }
    ...
    string constant legendary1 = 'Light Saber';
    string constant forceCrystals = 'Force Crystals';
    string constant katana = 'Katana';
    string constant holyGauntlets = 'Holy Gauntlets';
    string constant demonHide = 'Demon Hide';
    string constant dragonSkin = 'Dragon Skin';
    string constant silk = 'Silk';
    ...
    uint8 constant rareRunesLength = 6;
    ...
    uint8 constant rareMaterialsLength = 8;
    ...
    function getItemId(uint recipeTokenId) external view returns(uint) {
        uint256 rand = random(string(abi.encodePacked(Strings.toString(recipeTokenId)))) % 8;
        uint256 id;

        if (rand == 0){
            id = itemId(recipeTokenId, ItemComponents.weaponComponents, WEAPON);
        } 
        else if (rand == 1){
            id = itemId(recipeTokenId, ItemComponents.chestComponents, CHEST);
        }
        else if (rand == 2){
            id =itemId(recipeTokenId, ItemComponents.waistComponents, WAIST);
        }
        else if (rand == 3){
            id = itemId(recipeTokenId, ItemComponents.footComponents, FOOT);
        }
        else if (rand == 4){
            id = itemId(recipeTokenId, ItemComponents.headComponents, HEAD);
        }
        else if (rand == 5){
            id = itemId(recipeTokenId, ItemComponents.handComponents, HAND);
        }        
        else if (rand == 6){
            id = itemId(recipeTokenId, ItemComponents.neckComponents, NECK);
        }        
        else {
            id = itemId(recipeTokenId, ItemComponents.ringComponents, RING);
        }

        return id;
    }
    function getRecipeName(uint tokenId, bool vanilla) onlyOwner public view returns(string memory) {
        ...
        if (rand == 0){
            (components, itemType) = ItemTokenId.fromId(itemId(tokenId, ItemComponents.weaponComponents, 0x0));
            fullOutput = tokenName(itemId(tokenId, ItemComponents.weaponComponents, 0x0));
        } 
        else if (rand == 1){
            (components, itemType) = ItemTokenId.fromId(itemId(tokenId, ItemComponents.chestComponents, CHEST));
            fullOutput = tokenName(itemId(tokenId, ItemComponents.chestComponents, CHEST));
        }
        else if (rand == 2){
            (components, itemType) = ItemTokenId.fromId(itemId(tokenId, ItemComponents.waistComponents, WAIST));
            fullOutput = tokenName(itemId(tokenId, ItemComponents.waistComponents, WAIST));
        }
        else if (rand == 3){
            (components, itemType) = ItemTokenId.fromId(itemId(tokenId, ItemComponents.footComponents, FOOT));
            fullOutput = tokenName(itemId(tokenId, ItemComponents.footComponents, FOOT));
        }
        else if (rand == 4){
            (components, itemType) = ItemTokenId.fromId(itemId(tokenId, ItemComponents.headComponents, HEAD));
            fullOutput = tokenName(itemId(tokenId, ItemComponents.headComponents, HEAD));
        }
        else if (rand == 5){
            (components, itemType) = ItemTokenId.fromId(itemId(tokenId, ItemComponents.handComponents, HAND));
            fullOutput = tokenName(itemId(tokenId, ItemComponents.handComponents, HAND));
        }        
        else if (rand == 6){
            (components, itemType) = ItemTokenId.fromId(itemId(tokenId, ItemComponents.neckComponents, NECK));
            fullOutput = tokenName(itemId(tokenId, ItemComponents.neckComponents, NECK));
        }        
        else {
            (components, itemType) = ItemTokenId.fromId(itemId(tokenId, ItemComponents.ringComponents, RING));
            fullOutput = tokenName(itemId(tokenId, ItemComponents.ringComponents, RING));
        }
    ...
    function getRecipeRequirements(uint tokenId) public view onlyOwner returns(RecipeItemNames memory, RecipeItemIds memory) {
        ...
        recipeNames.recipe = getRecipeName(tokenId, true);

        //check if legendary:
        if (compareStrings(recipeNames.recipe, legendary1)) {
            //require specific item
            recipeNames.material = forceCrystals;
            index[0] = 16;
        ...
        else if (compareStrings(recipeNames.recipe, holyGauntlets)){
            //require specific item
        ...
        //check if uncommon
            ...
            //require specific item
            recipeNames.material = demonHide;
            index[0] = 14;
            recipeIds.material = MaterialTokenId.toId(index, MATERIALS);
        }
            ...
            //require specific item
            recipeNames.material = dragonSkin;
            index[0] = 13;
            recipeIds.material = MaterialTokenId.toId(index, MATERIALS);
        }
        ...
        recipeNames.recipe = getRecipeName(tokenId, false);
    ...
    function output(uint256 tokenId) internal view returns (string memory, RecipeItemNames memory) {

        (RecipeItemNames memory recipeNames,) = getRecipeRequirements(tokenId);
        ...
        string memory outputValue = string(abi.encodePacked(parts[0], parts[1], parts[2], parts[3], parts[4], parts[5], parts[6], parts[7]));
        outputValue = string(abi.encodePacked(outputValue, parts[8], parts[9], parts[10], parts[11], parts[12], parts[13], parts[14]));

        return (outputValue, recipeNames);
    }
    ...
    function materialAttributes(uint256 id) internal view returns (string memory) {
        (RecipeItemNames memory recipeNames,) = getRecipeRequirements(id);
        string memory res = string(abi.encodePacked('[', materialTrait("Gem", recipeNames.gem)));
        res = string(abi.encodePacked(res, ", ", materialTrait("Rune", recipeNames.rune)));
        res = string(abi.encodePacked(res, ", ", materialTrait("Material", recipeNames.material)));
        res = string(abi.encodePacked(res, ", ", materialTrait("Charm", recipeNames.charm)));
        res = string(abi.encodePacked(res, ", ", materialTrait("Tool", recipeNames.tool)));
        res = string(abi.encodePacked(res, ", ", materialTrait("Element", recipeNames.element)));
        res = string(abi.encodePacked(res, ", ", materialTrait("Requirement", recipeNames.requirement)));
        res = string(abi.encodePacked(res, ", ", materialTrait("Recipe", getRecipeName(id,true))));
    ...
    function _tokenURI(uint256 tokenId) public view returns (string memory) {
        ...
        (string memory firstPart, ) = output(tokenId);
        string memory json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(

                        '{"name": "Recipe for ', string(getRecipeName(tokenId, false)), '", "description": "Recipe is randomized adventurer gear generated and stored on chain. Stats, images, and other functionality are intentionally omitted for others to interpret. Feel free to use Recipe in any way you want.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(firstPart)), '", ''"attributes": ', materialAttributes(tokenId),'}'))));
...
/// @author Brecht Devos <[email protected]ing.org>
library Base64 {
...
import "./library/RecipeMetadata.sol";
...
contract Recipe is ERC721, ReentrancyGuard, Ownable {
    ...
    constructor(address _recipeMetadata) ERC721("Recipes (for Adventurers)", "Recipe") {
        //for testing
        //lootBagContract = LootBagInterface(_lootBagContract);
        transferOwnership(newOwner);
        recipeMetadata = RecipeMetadata(_recipeMetadata);

    }

    string public PROVENANCE = "";
    uint256 public ownersPrice = 0; //0 ETH
    uint256 public membersPrice = 0; //0 ETH
    ...
    function setMembersPrice(uint256 newPrice) external onlyOwner {
    ...
    function tokenOwner(uint tokenId) private view  {
        require(ownerOf(tokenId) == msg.sender, '!owner');
    }


    function seeRecipeRequirements(uint tokenId) public view returns(RecipeMetadata.RecipeItemNames memory, RecipeMetadata.RecipeItemIds memory){
        tokenOwner(tokenId);
        return recipeMetadata.getRecipeRequirements(tokenId);
    }

    function seeRecipeName(uint tokenId) public view returns(string memory) {
        tokenOwner(tokenId);
        return recipeMetadata.getRecipeName(tokenId, false);
    }

    function isActive() private view {
        require(saleIsActive, "!active");
    }
    
    function enoughEther(uint256 price, uint256 length) private view {
        require(price * length <= msg.value, "!ether");
    }

    function exists(uint256 id) private view {
        require(!_exists(id), "!id");
    }

    function correctId(uint256 id, uint256 a, uint256 b) private pure {
        require(id > a && id < b, "wrong id");
    }


    //Private sale minting (reserved for LootBag owners)
    function mintWithLoot(uint lootBagId) external payable nonReentrant {
        isActive();
        enoughEther(membersPrice, 1);
        require(lootBagContract.ownerOf(lootBagId) == msg.sender, "!owner.");
        exists(lootBagId);

        _safeMint(msg.sender, lootBagId);
    }
    function multiMintWithLoot(uint[] memory lootBagIds) external payable nonReentrant {
        isActive();
        enoughEther(membersPrice, lootBagIds.length);
        
        for (uint i=0; i<lootBagIds.length; i++) {
            require(lootBagContract.ownerOf(lootBagIds[i]) == msg.sender, "!owner.");
            exists(lootBagIds[i]);
    ...
    //Public sale minting
    function mint(uint lootBagId) external payable nonReentrant {
        isActive();
        enoughEther(publicPrice, 1);
        correctId(lootBagId, 8000, 15101);
        exists(lootBagId);

        _safeMint(msg.sender, lootBagId);
    }
    function multiMint(uint[] memory lootBagIds) external payable nonReentrant {
        isActive();
        enoughEther(publicPrice, lootBagIds.length);
        
        for (uint i=0; i<lootBagIds.length; i++) {
            correctId(lootBagIds[i], 8000, 15101);
            exists(lootBagIds[i]);
            _safeMint(msg.sender, lootBagIds[i]);
        }
    }
    ...
    //Owner mint
    function ownerMint(uint lootBagId) external payable nonReentrant onlyOwner {
        enoughEther(ownersPrice, 1);
        correctId(lootBagId, 15100, 16001);
        exists(lootBagId);

        _safeMint(msg.sender, lootBagId);
    }
    function ownerMultiMint(uint[] memory lootBagIds) external payable nonReentrant onlyOwner {
        enoughEther(ownersPrice, lootBagIds.length);
        
        for (uint i=0; i<lootBagIds.length; i++) {
            correctId(lootBagIds[i], 15100, 16001);
            exists(lootBagIds[i]);
            _safeMint(msg.sender, lootBagIds[i]);
        }
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        return recipeMetadata._tokenURI(tokenId);
    }
