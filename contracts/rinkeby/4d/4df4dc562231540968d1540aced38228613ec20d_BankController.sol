...
// SPDX-License-Identifier: MIT
// important:
// 合约采用可升级架构，原先业务逻辑相关字段即便不再使用，也不要删除，否则会升级失败
// 每次添加字段，在最后添加
contract BankController is Exponential, Initializable {
    ...
    uint256 public flashloanFeeBips; // 万分之九，9 for 0.0009
    ...
    function mintCheck(
        ...
        if (supplyCap != 0) {
            ...
            uint256 totalUnderlyingSupply = mulScalarTruncate(
                _exchangeRate,
                totalSupply
            );
            uint256 nextTotalUnderlyingSupply = totalUnderlyingSupply.add(
                amount
            );
            require(
    ...
    {
        (uint256 sumCollaterals, uint256 sumBorrows) = getTotalDepositAndBorrow(
            account
        );
    ...
    struct HealthFactorLocals {
        uint256 sumLiquidity; //sum of 每个币种提供的流动性（以USD计）
        uint256 sumLiquidityPlusThreshold; // sum of 每个币种的流动性*相应的清算阈值
        uint256 sumBorrows; // sum of 每个币种的借款
        uint256 fTokenBalance;
        uint256 borrowBalance;
        uint256 exchangeRate;
        uint256 oraclePrice;
        uint256 liquidationThreshold; // 单个币种的清算阈值
        uint256 liquidity; // 单个币种的流动性（以USD计）
        uint256 liquidityPlusThreshold; // 单个币种的流动性*相应的清算阈值
    }
    ...
    function _calcExchangeUnit(address fToken) internal view returns (uint256) {
        ...
        uint256 underlyingDecimals = tokenDecimals(
