...
abstract contract ArcadeBaseCollection is Ownable, ERC721Enumerable {
    ...
    mapping(string => uint256) public collectionPrice;
     ...
     * @param _gameId - gameId to set the price for
     * @param _price - price for the collection
     */
    function setCollectionPrice(string memory _gameId, uint256 _price)
        public
        onlyAllowed
    {
        collectionPrice[_gameId] = _price;
    }

    /**
     * @dev Get price for collection.
     * @param _gameId - gameId to get the price for
     */
    function _getCollectionPrice(string memory _gameId)
        internal
        view
        returns (uint256)
    {
        return collectionPrice[_gameId];
    }
        ...
        onlyOwner
    {
        ...
        require(_maxIssuance > 0, "Max issuance should be greasater than 0");
    ...
    function _mint(
        ...
        require(
            allowed[_beneficiary]
                ? msg.value >= 0
                : collectionPrice[_gameId] <= msg.value,
    ...
    function balance(address _owner) public view onlyAllowed returns (uint256) {
