...
contract XmasAccessory is ERC1155SupplyUpgradeable, OwnableUpgradeable {
    ...
    // support multiple token to mint
    ...
    function initialize(string memory _baseURI)
        external
        initializer
    {
        __Ownable_init();
    ...
    // mint by customToken
    function mintPublicByCustomToken(
        uint8 _id,
        uint256 _numItem,
        uint256 amount,
        uint8 tokenId
        ...
        require(supportedToken[tokenId].initialized, "unsupported token");
        require(
            amount >=
                (item[_id] * supportedToken[tokenId].conversionRate) / 10**3,
            "amount is below the price"
        );

        IERC20Upgradeable(supportedToken[tokenId].tokenAddress)
            .safeTransferFrom(msg.sender, owner(), amount);
        _mint(msg.sender, _id, _numItem, "");
    }
