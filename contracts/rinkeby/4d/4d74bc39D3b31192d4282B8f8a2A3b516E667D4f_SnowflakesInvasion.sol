...
contract SnowflakesInvasion is IERC1155Receiver {
  ...
  function move(uint256 tokenId,uint8 x,uint8 y) public {
    ...
    require(canMoveToken(msg.sender,tokenId),"Token need to wait to do next movement");
  ...
  function _calculateWin(uint256 tokenId,address target,address targetCollectible,uint256 targetTokenId) private returns(bool){
    ...
    powerP1 = powerP1.add(movementAtackBonusP1);
    powerP2 = powerP2.add(movementAtackBonusP2);
    uint256 basePowerHashAvatars = 10**18;
    uint256 basePowerSnowflakes = 2*10**18;
    bool win;
    if(targetCollectible == address(collectibles[1])){
      win = (basePowerSnowflakes.add(powerP2) < basePowerHashAvatars.add(powerP1));
    } else {
      win = (basePowerSnowflakes.add(powerP1) > basePowerHashAvatars.add(powerP2));
    }
    return(win);
  }
  ...
  function getMovementPoints(address player,uint256 tokenId) public view returns(uint256 x){
    ...
    if((block.number.sub(lastMovement)).div(50) <= 3){
      movementPoints = 3;
    } else {
      movementPoints = (block.number.sub(lastMovement)).div(50);
    }
    if(movementPoints > 10){
      movementPoints = 10;
    }
    
    uint256 extraMovement = _movement.balanceOf(player);
    if(extraMovement > 0){
      extraMovement = extraMovement.div(totalTokens).div(10**18);
    }
  ...
  function canMoveToken(address player,uint256 tokenId) public view returns(bool){
    ...
    uint256 stamina = _stamina.balanceOf(player);
    
    if(stamina > 0){
      stamina = stamina.div(totalTokens).div(10**18);
    }
    if(stamina > 40){
      stamina = 40;
    }
    bool canMove = (blocks >= uint256(50).sub(stamina));
    return(canMove);
  }
  /**
      @dev Handles the receipt of a single ERC1155 token type. This function is
