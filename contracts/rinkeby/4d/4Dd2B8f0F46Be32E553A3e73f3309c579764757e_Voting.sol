...
contract Voting is Ownable {
  ...
  |_______________________________________*/
    ...
    address private governanceContractAddress=address(0);

    uint private winningBar=0;
    mapping (uint => Proposal) proposals;
    uint private numProposals=0;
    uint private ProposePrice=0;
  ...
  |_______________________________________*/
         ...
         function getVoted(uint proposal,address _address) external view returns (bool _voted)
    {
      ...
      function getProposal(string memory _uri) external view returns (uint _proposal)
    {
        _proposal =  URIs[_uri];
    }
