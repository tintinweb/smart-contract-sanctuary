...
contract Creature is ERC721Tradable {
    ...
    uint256 private maxNftSupply;  // TODO: needed?
    ...
    function getMintable() public view returns (uint256[] memory) {
        ...
        for (uint256 k = 0; k < contracts.length; k++) {
            for (uint256 i = 0; i < tokensByKind[k].length; i++) {
                tokens[idx++] = tokensByKind[i][k];
            }
        }
        return tokens;
    }

    function getMintableFor(uint kind) private view returns (uint256[] memory) {
        IERC721Enumerable cont = contracts[kind];
        uint256 count = cont.balanceOf(_msgSender());
        uint256[] memory tokens = new uint256[](count);
        for (uint256 i = 0; i < count; i++) {
            try cont.tokenOfOwnerByIndex(_msgSender(), i) returns (uint256 tokenId) {
                tokens[i] = toTokenId(kind, tokenId);
