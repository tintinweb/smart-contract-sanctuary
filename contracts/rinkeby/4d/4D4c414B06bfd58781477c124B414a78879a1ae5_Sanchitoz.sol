 ...
 */
// TODO change
contract Sanchitoz is ERC721Tradable {
    using Address for address;
    ...
    uint256 public constant roundOneSupply = 30;

    // TODO change
    /// @notice Max number of tokens available during second round
    uint256 public constant roundTwoSupply = 70;

    /// @notice Start of the early sale period (in Unix second)
    // TODO change the timestamp
    uint256 public constant mintStartDate = 1640974639;

    // ============================== Constructor ===================================

    /// @notice Constructor of the NFT contract
    /// Takes as argument the OpenSea contract to manage sells
    constructor(address _proxyRegistryAddress) ERC721Tradable("Sanchitoz", "TOZ", _proxyRegistryAddress) {
    // ) ERC721Tradable("ApeRacingAcademy NFTs", "ARANFTs", _proxyRegistryAddress) {

        tokenIdCount._value = 100;
        metadataTokenURI = "https://aperacingacademy-metadata.herokuapp.com/";
        lock = false;
    }
    ...
    function baseTokenURI() public view override returns (string memory) {
        // TODO change the metadata URL
        return metadataTokenURI;
    }

    // ============================== Public functions ===================================

    function turboBoost(uint256 amount) view internal returns (uint) {
     uint256 globalAmount = amount;
        if (tokenIdCount._value - 100 < roundOneSupply){
            globalAmount = uint(globalAmount * 150 / 100);
        } 

        else if (tokenIdCount._value - 100 < roundTwoSupply){
            globalAmount = uint(globalAmount * 125 / 100);
        } 
    ...
    function mint(uint256 amount) external payable {
        ...
        uint256 globalAmount = turboBoost(amount);
        for (uint256 i = 0; i < globalAmount; i++) {
    ...
    function recoverERC20(address tokenAddress, address to, uint256 amountToRecover) external onlyOwner {
    ...
    function withdraw(address payable to, uint256 amountToRecover) external onlyOwner {
