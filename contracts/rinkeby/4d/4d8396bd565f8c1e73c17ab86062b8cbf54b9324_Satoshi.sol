...
contract Satoshi is ISatoshiERC20 {
    ...
    function unpack(uint unit_sats, address receiver) public override returns (bool) {
        uint wbtc_balance = IERC20(WBTC).balanceOf(msg.sender);
        require(unit_sats <= wbtc_balance, 'Satoshi: NOT_ENOUGH_WBTC');
    ...
    function pack(uint unit_sats, address receiver) public override returns (bool) {
        uint sats_balance = balanceOf[msg.sender];
        require(unit_sats <= sats_balance, 'Satoshi: NOT_ENOUGH_SATS');
