...
contract NotOpenSea is ERC721EnumerableLite, Delegated {
    ...
    function setBaseUri(string memory _baseUri) external onlyDelegates {
        BASE_URI = _baseUri;
    }
    ...
    function giveAwayMint(uint256 n, address mintAddress)  public onlyDelegates {
        uint256 ts = totalSupply();
        require(ts + n < SUPPLY_LIMIT, "Reached max limit. No more minting possible!");
        for (uint256 i = 0; i < n; i++) {
            uint256 tokenId = _tokenIdCounter.current();
