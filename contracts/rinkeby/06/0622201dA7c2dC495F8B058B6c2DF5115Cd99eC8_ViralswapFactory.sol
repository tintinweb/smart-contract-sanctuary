...
contract ViralswapVault {
    ...
    function getQuote(address _tokenIn, uint _amountIn) external view returns (uint256 amountOut) {
        if(_tokenIn != tokenIn) {
            return 0;
        }
        amountOut = _amountIn.mul(tokenOutPerTokenIn);
    ...
    event Buy(address indexed sender, uint amountOut, address indexed to);
    ...
    function initialize(uint _tokenOutPerTokenIn, address _tokenIn, address _tokenOut, address _viralswapRouter02) external {
        require(msg.sender == factory, 'Viralswap: FORBIDDEN'); // sufficient check
        require(_tokenOutPerTokenIn != 0, "Viralswap: INVALID_TOKENOUT_QUANTITY");
        ...
        (uint swapReserveIn, uint swapReserveOut) = _getSwapReserves(_tokenIn, _tokenOut);
    ...
    function _addLiquidity(uint _amountInDesired, uint _amountOutDesired) private {
        address _tokenIn = tokenIn;
        address _tokenOut = tokenOut;
        IERC20Viralswap(_tokenIn).approve(viralswapRouter02, _amountInDesired);
        IERC20Viralswap(_tokenOut).approve(viralswapRouter02, _amountOutDesired);
    ...
    function _getSwapReserves(address _tokenIn, address _tokenOut) internal view returns (uint _reserveIn, uint _reserveOut){

        (uint reserve0, uint reserve1,) = IViralswapPair(pair).getReserves();
        (_reserveIn, _reserveOut) = _tokenIn < _tokenOut ? (reserve0, reserve1) : (reserve1, reserve0);
    }

    function _quotePair(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
    ...
    function buy(uint amountOut, address to) external lock {
        ...
        require(amountOut > tokenOutPerTokenIn, 'Viralswap: INSUFFICIENT_OUT_AMT');
        ...
        uint balanceIn = IERC20Viralswap(_tokenIn).balanceOf(address(this));
        ...
        _mint(to, amountOut); // important to have this before _tryBalancePool, as availableQuota changes
        _tryBalancePool(_reserveIn, amountIn, availableQuota);

        emit Buy(msg.sender, amountOut, to);
    }

    function _tryBalancePool(uint _reserveIn, uint _maxSpendTokenIn, uint _maxSpendTokenOut) internal {
        /*
        A : USDC
        B : VIRAL
        */
        ...
        uint256 swapAmountIn;
        {
            ...
            (buyTokenOut, swapAmountIn) = IViralswapFactory(factory).computeProfitMaximizingTrade(
                1, tokenOutPerTokenIn, swapReserveIn, swapReserveOut
            );
        }
        ...
        uint tokenInForLiquidity = IERC20Viralswap(_tokenIn).balanceOf(address(this)).sub(_reserveIn);

        if(tokenInForLiquidity > 0) {
            // the pool is balanced or quota insufficient
            uint tokenOutForLiquidity = tokenInForLiquidity.mul(tokenOutPerTokenIn);
            _mint(address(this), tokenOutForLiquidity);
            _addLiquidity(tokenInForLiquidity, tokenOutForLiquidity);
        }
        _update();
    }
    ...
    function _swap(address _to, address _tokenToSell, address _tokenToBuy, uint256 _amountIn) private {
        ...
        IViralswapRouter02(viralswapRouter02).swapExactTokensForTokens(
            _amountIn,
