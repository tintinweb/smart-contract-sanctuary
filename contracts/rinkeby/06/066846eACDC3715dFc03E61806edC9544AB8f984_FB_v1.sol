...
contract FB_v1 is ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    mapping(string => address) private _changeableAddresses;
    ...
    function setupMintingRounds() internal {
        uint256 nextMintingAt = 1628702896; // 11 Ağustos 2021 20:28:16 GMT+03:00
        for (uint8 i = 1; i <= 73; i++) {
            _mintingDates[i] = nextMintingAt;
            nextMintingAt += 10 minutes;

            if (i < 53 && (i + 4) % 12 != 0) {
                // mint for marketing & tech
    ...
    function setupLocks() internal {
        _nextUnlockAt = 1628706496; // 11 Ağustos 2021 21:28:16 GMT+03:00
        _lastUnlockAt = _nextUnlockAt + 420 minutes;
        _mint(_presaleLocked, _presaleLockedAmount);
    }

    function checkLockedAmount(address from, uint256 amount) internal {
        // check locked account on every transfer and decrease locked amount if conditions met
        if (from == _presaleLocked && _presaleLockedAmount > 0) {
            // run a while loop to update locked amount
            while (_nextUnlockAt <= block.timestamp && _nextUnlockAt <= _lastUnlockAt) {
                ...
                // increase next unlock timestamp for 30 days
                _nextUnlockAt += 30 minutes;
                _presaleLockedAmount -= unlockAmount;
            }

            // revert transaction if available balance is insufficient
