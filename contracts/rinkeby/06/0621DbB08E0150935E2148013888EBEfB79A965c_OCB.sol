...
contract OCB is ERC721, ERC721Enumerable, Ownable {
        ...
        returns (string memory)
    {
        ...
        uint256 rand = random(string(abi.encodePacked("banana", tokenId)));

        params[0] = (rand / 1) % 100; // age
        params[1] = (rand / 100) % 100; // sticker_type
        params[2] = (rand / 10000) % 100; // sticker_text
        params[3] = (rand / 1000000) % 100; // spots_type
        params[4] = (rand / 100000000) % 100; // is_rainbow
        params[5] = (rand / 10000000000) % 100; // is_double
        ...
        aparts[0] = '[{ "trait_type": "Age", "value": "';
        aparts[1] = string(abi.encodePacked(Util.toStr(params[0])));
        aparts[2] = '"}]';

        string memory strparams = string(
            abi.encodePacked(aparts[0], aparts[1], aparts[2])
        );
        ...
        string memory json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "OnChain Banana", "description": "Beautiful Banana Collective","attributes":',
                        strparams,
