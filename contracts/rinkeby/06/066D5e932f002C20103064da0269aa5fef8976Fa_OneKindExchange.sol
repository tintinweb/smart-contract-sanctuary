...
contract OneKindExchange is Ownable, BidValidator {
    mapping(bytes32 => uint256) public auctionBidLedger;
    mapping(address => bool) private _admins;

    address public wethAddress; // wethAddress should be fixed
    address public exchangeWallet; // owns NFT and receives wETH
    ...
    function exchangeAssets(LibBid.Bid memory userBid) internal {
        ...
        require(auctionBidLedger[bidHash] != 1, "Bid was already processed");

        // TODO Cancelation is only done on database level, TBD Business Decision if we want to offer this Onchain
        require(auctionBidLedger[bidHash] != 99, "Bid was canceled by user");

        // Validate Balances
        require(
            IERC20(wethAddress).allowance(userBid.userWallet, address(this)) >=
                userBid.amount,
            "User has not approved enough balance on WETH Contract"
        );
        require(
            IERC20(wethAddress).balanceOf(userBid.userWallet) >= userBid.amount,
            "Insufficient Funds to fulfill Bid"
        );
        require(
            IERC721(userBid.assetContract).ownerOf(userBid.tokenId) ==
                exchangeWallet,
            "NFT is not owned by Exchange"
        );
        ...
        IERC721(userBid.assetContract).transferFrom(
            exchangeWallet,
            userBid.userWallet,
            userBid.tokenId
        );
    ...
    modifier onlyAdmin() {
        require(
            ...
            "Sender does not have Admin Access"
        );
        _;
    }
}
...
abstract contract BidValidator is EIP712 {
        ...
        view
    {
        ...
        if (signer != userBid.userWallet) {
            revert("userBid signature verification error");
        }
    }
}
