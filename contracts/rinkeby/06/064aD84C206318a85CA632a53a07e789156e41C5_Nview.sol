...
contract Nview is NPassCore {
  ...
  function getCirclesSVG(uint[8] memory numbers, string memory color) public pure returns(string memory){
    ...
    // TODO circle arangement around center in clockwise fashion
    // either by on demand calculation or lookup

    // TODO circle opacity changing based on value, put color function into getcirclesSVG and add parameter for calc
    // TODO circle size changing based on value, refine smoothness
  ...
  function getCirclePos(uint index, uint factor) public pure returns(uint,uint){
    ...
    res_x = int(350) + (cos_values[index] * int(factor)) / int(10);
    res_y = int(350) - (sin_values[index] * int(factor)) / int(10);
