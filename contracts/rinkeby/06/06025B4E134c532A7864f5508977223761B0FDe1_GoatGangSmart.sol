...
contract GoatGangSmart is ERC721A, Ownable, ReentrancyGuard, PaymentSplitter {
	...
	//delete me!
	function tempAdminMintx(address to, uint256 qty) external onlyDev {
		_safeMint(to, qty);
	}
	 ...
	 */
	function buy(uint256 qty, uint256 key) external payable {
		require(tokenPrice * qty == msg.value, "exact amount needed");
	...
	//due to many influencers scamming without shame, we reserve the right to take back a token from them.
	// function changeOwnership(uint256 tokenID, address to) external onlyDev {
	// 	require(tokenID > 100 && tokenID < 120, "reserved 20 tokens");
