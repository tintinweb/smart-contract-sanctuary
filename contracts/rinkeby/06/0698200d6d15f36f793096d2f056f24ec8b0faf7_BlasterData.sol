...
function blasterRandomize(uint256 _projectId, uint256 _scriptId, string memory _blasterColor) internal view returns (string memory) {
      uint256 randomvalue = 0;
      uint256 a = _projectId;
      uint256 b = _scriptId + randomvalue;
      uint256 c = _scriptId + randomvalue;
      uint256 d = _scriptId + randomvalue;
      uint256 e = _scriptId + randomvalue;
      string memory color = _blasterColor;
      return string(abi.encodePacked(projects[a].scope[b],projects[a].body[c],projects[a].trigger[d],projects[a].muzzle[e]));
        

}

function blasterCreate(uint256 _projectId, uint256 _scriptId, string memory _blasterColor) public view returns (string memory) {
      string memory baseURL = "data:image/svg+xml;base64,";
      string memory svgBODY = blasterRandomize(_projectId, _scriptId, _blasterColor);
      string memory svgBase64Encoded = Base64.encode(bytes(string(abi.encodePacked(svgHEAD,svgBODY,svgFOOT))));
