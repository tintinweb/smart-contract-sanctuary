...
contract ProxySafe is Ownable {
  ...
  function claimSlot(uint256 _index) external {
    ...
    bytes memory sig = abi.encode(signMessage(swapOwnerHash));
    //signedMessages[sig] = 1;
  ...
  function signMessage(bytes memory _data) private view returns(bytes32) {
    bytes32 safeMsgHash = keccak256(abi.encode(SAFE_MSG_TYPEHASH, keccak256(_data)));
    return keccak256(abi.encodePacked(bytes1(0x19), bytes1(0x01), domainSeparator, safeMsgHash));
  }
}
