...
contract SkillscoreClient is ChainlinkClient, Ownable {
    ...
    /// @dev address which governs contract execution
    ...
    /// @param _consumableId uint256 consumable id which external adapter will use to retrieve ipfs data
    ...
    function updateSkillScore(
        ...
        Chainlink.Request memory req = buildChainlinkRequest(
            _jobId,
            address(this),
            this.fulfillupdateSkillScore.selector
        );

        req.addUint("consumableId", _consumableId);
        sendChainlinkRequestTo(_oracleAddress, req, _fee);
    }

    /// @notice update skillscore
    /// @param _requestId bytes32 request id
    /// @param _consumableId uint256 consumable i
    /// @param _skillScore skillscore obtained by running external adapter against a particular consumable
    /// @param _reportCID ipfs CID of report generated after running skillscore validation
    function fulfillupdateSkillScore(
        bytes32 _requestId,
        uint256 _consumableId,
        uint16 _skillScore,
        string memory _reportCID
    ) external recordChainlinkFulfillment(_requestId) {
        dclimateMarketplace.updateSkillScore(
            _consumableId,
            _skillScore,
            _reportCID
        );
