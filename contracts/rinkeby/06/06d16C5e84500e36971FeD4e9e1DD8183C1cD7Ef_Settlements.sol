...
// @author zeth ifykyk
// @notice
// This contract is heavily inspired by Dom Hofmann's Loot Project with game design from Sid Meirs Civilisation, DND, Settlers of Catan & Age of Empires.
...
contract Settlements is ERC721, ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory attributes = string(
            abi.encodePacked('[',
            '{ "trait_type": "Size", "value": "', parts[1], '" },',
            '{ "trait_type": "Spirit", "value": "', parts[3], '" },',
            '{ "trait_type": "Age", "value": "', parts[5], '" },',
            '{ "trait_type": "Resource", "value": "', parts[7], '" },',
            '{ "trait_type": "Morale", "value": "', parts[9], '" },',
            '{ "trait_type": "Government", "value": "', parts[11], '" },',
            '{ "trait_type": "Realm", "value": "', parts[13], '" }',
            ']')
        );
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Settlement #', Strings.toString(tokenId), '", "description": "Settlements are a turn based civilisation simulator stored entirely on chain, go forth and conquer.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"', ',"attributes":', attributes, '}'))));
    ...
    function settle(uint256 tokenId) public nonReentrant {
        require(!_exists(tokenId) && tokenId > 0 && tokenId < 9900, "Settlement id is invalid");
