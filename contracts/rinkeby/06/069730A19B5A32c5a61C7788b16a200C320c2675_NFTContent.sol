...
contract NFTContent is ERC721Tradable {
  ...
  mapping(address => uint256) tokenMapper;
  ...
  function contentUri(uint256 _tokenId) public view virtual returns (string memory) {
    require(msg.sender == ownerOf(_tokenId) || msg.sender == delegate || msg.sender == owner());

    return _contentURI;
  }

  function _setContentUri(string memory contentURI_) internal virtual {
    _contentURI = contentURI_;
  }

  function setContentUri(string memory contentURI) external payable {
    _setContentUri(contentURI);
  }

  function metaDataUri() external virtual returns (string memory) {
    string memory base = baseURI();

    return string(abi.encodePacked(base, _metaURI));
  }
  ...
  function purchase() external payable {
    ...
    tokenMapper[msg.sender] = tokenId;

    _setTokenURI(tokenId, _metaURI);
  }

  function getTokenByAddress(address owner) external returns (uint256) {
    return tokenMapper[owner];
  }
...
contract ERC721Tradable is ERC721, CustomAccessControl {
   ...
   */
  function setBaseURI(string memory _baseURI) public onlyOwner payable {
    _setBaseURI(_baseURI);
  }
