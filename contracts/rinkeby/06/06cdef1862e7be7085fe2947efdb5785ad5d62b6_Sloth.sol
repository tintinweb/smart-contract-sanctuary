...
contract Sloth is ERC721, ERC721Enumerable, Ownable {
  ...
  uint256 public revealTimeStamp = block.timestamp+300 ; 
...
//ok
  ...
  function whiteListMintSloths(uint numSloths) public payable {
    ...
    require(totalSupply() + numSloths <= MAX_SUPPLY, 'Sale would exceed max supply');
    require(balanceOf(msg.sender) + numSloths <= maxBalance, 'Sale would exceed max balance');
    require(numSloths <= maxMint, 'Sale would exceed max mint');
