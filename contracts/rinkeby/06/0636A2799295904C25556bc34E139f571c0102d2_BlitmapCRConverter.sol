...
contract BlitmapCRConverter {
    ...
    function tokenNameOf(uint256 tokenId) external view returns (string memory) {
        return makeStringMultipleOf6(IBlitmap(blitmapAddress).tokenNameOf(tokenId));
    }

    function makeStringMultipleOf6(string memory input) private pure returns (string memory) {
