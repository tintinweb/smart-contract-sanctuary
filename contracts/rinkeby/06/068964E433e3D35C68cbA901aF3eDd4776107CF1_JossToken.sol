...
///@title
///@author Joss Duff
///@notice this is a personal project to set up a token for interacting with and displaying messages on my website jossduff.com.  jossToken isn't intended to hold any monetary value
contract JossToken is ERC20 {

  constructor() ERC20("JossToken", "JOS") public {
    _mint(msg.sender, 1000000e18);
  }

  ///@dev each message is an object with a address as the author and the actual message text
  struct Message {
    address author;
    string text;
  }

  ///@dev event for front end to update and display a newly made message
  ///@param author is the address of the wallet writing the message (the wallet calling makeMessage)
  ...
  function makeMessage(string memory _newMessage) public {
    _burn(msg.sender, 1); //burns a users jossToken from their wallet.  Cannot create a message if the user doesn't have a jossToken
  ...
  function freeJoss() public {
    ...
    _mint(msg.sender, 1); //mints a new jossToken to callers address
