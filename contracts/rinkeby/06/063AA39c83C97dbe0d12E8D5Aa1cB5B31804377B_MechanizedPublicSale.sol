...
interface Mechanized {
    ...
    function doMintCheap(uint256, address) external;
}
...
contract MechanizedPublicSale is Ownable{
    ...
    // Flags
    bool public saleIsActive = true; // flag to indicate if minting is possible
    bool public ownerCanModify = true;

     // Token data
    uint256 public MAX_SUPPLY = 700;    // the maximum amount of tokens that can be minted
    uint256 public MINT_PRICE = 0.00000008 ether; //mint price: 0.08 ETH
    uint256 public MAX_MINT_PER_USER = 700; // maximum amount of tokens one address can mint  
    ...
    */
    ...
    /*function mintCheap(uint256 numberOfTokens) public payable {
        require(saleIsActive, "SALE NOT ACTIVE");
        require(_mintsPerAddress[msg.sender].add(numberOfTokens) <= MAX_MINT_PER_USER, "MINTER EXCEEDS MAX TOKENS");
        require(publicSaleMinted.add(numberOfTokens) <= MAX_SUPPLY, "MAX TOKENS ALREADY MINTED");
        require(MINT_PRICE.mul(numberOfTokens) == msg.value, "ETHER SENT NOT CORRECT"); 
        ...
        publicSaleMinted = publicSaleMinted.add(numberOfTokens);
        _mintsPerAddress[msg.sender] = _mintsPerAddress[msg.sender].add(numberOfTokens);
        nft.doMintCheap(numberOfTokens, msg.sender);
    }*/
