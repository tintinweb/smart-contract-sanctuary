...
contract Polkadog_V1_5_ERC20 is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    bool public _enableHourRule = false;
    bool public _enableMaxLimit = false;
    
    uint256 private liquidityFeeToSell = 100 * 10**18;
    uint256 private marketingFeeToSell = 100 * 10**18;
    uint256 public transactionLimit = 500 * 10**18;
    ...
    function enableHourRule(bool enableHourRule) external onlyOwner() {
        _enableHourRule = enableHourRule;
    }

    function enableMaxLimit(bool enableMaxLimit) external onlyOwner() {
        _enableMaxLimit = enableMaxLimit;
    }
    ...
    function setTimeBound(uint256 timeBound) external onlyOwner() {
    ...
    function setMarketingFeeToSell(uint256 limit) external onlyOwner() {
        marketingFeeToSell = limit;
    }
    ...
    function _transfer(
        ...
        if(_enableFee && (from == uniswapV2Pair || to == uniswapV2Pair) && (_isIncludedInFee[from] || _isIncludedInFee[to])){
            ...
            // when hour rule is enabled
            if(_enableHourRule && from != uniswapV2Pair) {
            ...
            // when max transaction limit is enabled
            if(_enableMaxLimit && from != uniswapV2Pair) {
                if(amount >= transactionLimit){
                    _taxIncreased[from] = true;
                    afterLimitFee();
                }
    ...
    function checkTimeBound(address from, uint256 currentTime) internal {      
        if((currentTime - _transactionTime[from]) > _timeBound ){
    ...
    function _swapAndLiquify(address from) internal {
        if(from != uniswapV2Pair && (_marketingFeeBalance >= marketingFeeToSell || _liquidityFeeBalance >= liquidityFeeToSell)) {
            bool initialFeeState = _enableFee;
            // remove fee if initialFeeState was true
            if(initialFeeState) _enableFee = false;

            if(_marketingFeeBalance >= marketingFeeToSell) {
            ...
            if(!inSwapAndLiquify && swapAndLiquifyEnabled && _liquidityFeeBalance >= liquidityFeeToSell) {
