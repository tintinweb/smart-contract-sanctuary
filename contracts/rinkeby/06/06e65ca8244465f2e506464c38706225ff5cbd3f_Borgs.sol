...
contract Borgs is ERC721Enumerable, BorgFactory, NumberUtils{
    ...
    function getAttributesUsedCount(string memory attribute) public view returns(uint256){
        // Return the count of attributes used
        return _borgAttributesUsed[attribute];
    }
    
    function getBorg(uint256 borgId) public view returns(string[] memory image, string[] memory attributes, uint256 parentAId, uint256 parentBId, uint256 childId){
        // Find the borg in question
        Borg memory borg = _borgs[borgId];
