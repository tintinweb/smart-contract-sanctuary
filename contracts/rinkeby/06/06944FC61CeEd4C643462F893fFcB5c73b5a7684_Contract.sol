...
contract Contract is ERC721Enumerable, Ownable {  
    ...
    // Total claimed tokens from the reserve
    uint256 public reserveClaimed = 0;
    ...
    // 
    uint256 public MAX_FOR_SALE = MAX_SUPPLY - reserved;

    // The base link that leads to the jpeg of the token
    string public baseTokenURI;
    

    constructor (string memory newBaseURI) ERC721 ("Contract", "CT") {
    ...
    // Admin minting function to mint reserved tokens for the team, collabs, customs and giveaways
    function mintReserved(uint256 _amount) public onlyOwner {
        ...
        require( reserveClaimed + _amount <= reserved, "Can't claim more than reserve" );
        reserveClaimed += _amount;
        uint256 supply = totalSupply();
        for(uint256 i; i < _amount; i++){
            _safeMint( msg.sender, supply + i );
        }
