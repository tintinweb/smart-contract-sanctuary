...
contract Blindpet {
    ...
    function toy() public {
        require(getAlive(), "no longer with us");
        require(getToy() < 80, "no toys to play");
        lastToyBlock = block.number;
    ...
    function getToy() public view returns (uint256){
        return havetoy + ((block.number - lastToyBlock) / 50);
    }
