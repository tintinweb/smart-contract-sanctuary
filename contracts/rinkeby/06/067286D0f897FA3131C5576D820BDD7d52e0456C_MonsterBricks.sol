...
contract MonsterBricks is ERC1155, ERC1155Burnable, ERC1155Supply, Ownable, Pausable, ReentrancyGuard {
  ...
  mapping(address => uint256) internal lastTokensClaimed;
  mapping(uint256 => string) public brickNames;
  ...
  function uri(uint256 _tokenId) public view override returns (string memory) {
    require(_tokenId <= tokenIds.current(), "URI requested for invalid token");
    return
  ...
  function claimBrick(address _account, uint256 _quantity, bytes32[] calldata _proof) public whenNotPaused nonReentrant {
    ...
    require(lastTokensClaimed[_account] < tokenIds.current(), "Already claimed this token");

    lastTokensClaimed[_account] = tokenIds.current();

    _mint(_account, tokenIds.current(), _quantity, "");
  }
  ...
  function createNewRelease(string memory _name) public onlyOwner {
    require(tokenIds.current() < maxTokenCount, "Cannot create more releases");

    tokenIds.increment();
    brickNames[tokenIds.current()] = _name;
  }
