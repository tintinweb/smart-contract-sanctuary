...
contract TheIndifferentDuck is ERC721Enumerable, PaymentSplitter, Ownable {
    ...
    address[] private _teamAddresses = [
        0x0a1CfAF72740c0edA86799cDFc18C559D7805CAe,
        0xCa9AA72FeA844b937f64bD4013ff9503a27CAc01,
        0xD9031e8Ad43256B553e3CA312402BdF88581cc3B,
        0x0427AA0dd2Cd9a8b8432Cf4755B7Fd5a8084A4e9,
        0xaA0a84cD9aB4570Cbe1590C8e1DE6123b07762A2
    ];
    uint256[] private _teamShares = [20, 20, 20, 20, 20];

    constructor(string memory baseURI)
        ERC721('The Indifferent Duck', 'TID')
        PaymentSplitter(_teamAddresses, _teamShares)
    {
        setBaseURI(baseURI);

        // team gets the first five
        for (uint256 i = 0; i < _teamAddresses.length; i++) {
            _safeMint(_teamAddresses[i], i);
        }
    }
    ...
    function preSaleMintDuck(uint256 numberOfTokens) public payable {
        ...
        require(
            supply.add(numberOfTokens) <= MAX_DUCKS - _reserved,
            'Purchase would exceed max supply of Ducks'
        );
        require(
            msg.value >= _price.mul(numberOfTokens),
    ...
    function withdrawAll() public payable onlyOwner {
        for (uint256 i = 0; i < _teamAddresses.length; i++) {
            release(payable(_teamAddresses[i]));
        }
    }
}
