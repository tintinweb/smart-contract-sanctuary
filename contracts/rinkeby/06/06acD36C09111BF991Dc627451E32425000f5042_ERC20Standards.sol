...
abstract contract Ownable is Context {
   ...
   */
  modifier groupOwner() {
    require(checkOwner(_msgSender()) || owner() == _msgSender(), "GroupOwner: caller is not the owner");
    _;
  }
   ...
   */
  function setOwners(address[] memory _owners) public virtual onlyOwner {
    _setOwners(_owners);
  }

  function _setOwners(address[] memory _owners) private {
    for (uint256 index = 0; index < owners.length; index++) {
      ownerByAddress[owners[index]] = false;
    }
