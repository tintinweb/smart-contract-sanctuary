...
contract AuctionHouse is IAuctionHouse, ReentrancyGuard {
    ...
    nonReentrant
    {
        ...
        // For Media Protocol, ensure that the bid is valid for the current bidShare configuration
    ...
    /**
     * @notice End an auction, finalizing the bid on Media if applicable and paying out the respective parties.
    ...
    function endAuction(uint256 auctionId) external override auctionExists(auctionId) nonReentrant {
        ...
        if(auctions[auctionId].tokenContract == Media) {
            ...
            (bool success, uint256 remainingProfit) = _handleMediaAuctionSettlement(auctionId);
    ...
    function _handleMediaAuctionSettlement(uint256 auctionId) internal returns (bool, uint256) {
        address currency = auctions[auctionId].auctionCurrency == address(0) ? wethAddress : auctions[auctionId].auctionCurrency;
...
library Decimal {
    ...
    function mul(uint256 target, D256 memory d)
        ...
        returns (uint256)
    {
        return getPartial(target, d.value, BASE);
    }
    ...
    function div(uint256 target, D256 memory d)
        ...
        returns (uint256)
    {
        return getPartial(target, BASE, d.value);
    }
}
