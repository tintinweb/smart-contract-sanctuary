...
contract V3Oracle is IOracle {
    ...
    address public V3_FACTORY = 0x1F98431c8aD98523631AE4a59f267346ea31F984;
        ...
        returns (uint256 usdAmount)
    {
        ...
        (address token0, address token1, uint24 fee) = (uniswapPool.token0(), uniswapPool.token1(), uniswapPool.fee());
        (address tokenIn, address tokenOut, uint256 amountIn) = (token0 == USDT || token0 == DAI || token0 == USDC)
    ...
    function checkPairsAndLiquidity(address token) public view override returns (address) {
        address pair;
        uint256 liquidity;
    ...
    function checkWethPairsAndLiquidity(address token) public view override returns (address) {
        address pair;
        uint256 liquidity;
        uint16[3] memory feeTier = [3000, 500, 10000];
        uint256 maxLiquidity;
        ...
        returns (uint256 pilotAmount)
    {
        ...
        uint24 tokenFee = IUniswapV3Pool(wethToken).fee();
    ...
    function getPrice(
        ...
        address pool = PoolAddress.computeAddress(V3_FACTORY, PoolAddress.getPoolKey(tokenA, tokenB, _poolFee));
        (uint32 lastTimeStamp, , , ) = IUniswapV3Pool(pool).observations(0);
        int256 twapTick = OracleLibrary.consult(pool, uint32(block.timestamp) - lastTimeStamp);
