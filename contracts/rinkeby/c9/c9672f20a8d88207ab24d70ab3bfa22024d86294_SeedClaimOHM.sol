...
contract SeedClaimOHM {
    ...
    function claim( uint _amount ) external {
        ...
        require( redeemableFor( msg.sender ) >= OHMToSend, 'Not enough vested' );
        require( info.max.sub( claimed( msg.sender ) ) >= OHMToSend, 'Claimed over max' );

        terms[ msg.sender ].wClaimed = info.wClaimed.add( wOHM.sOHMTowOHM( OHMToSend ) );
    ...
    function redeemableFor( address _address ) public view returns (uint) {
        uint max = circulatingSupply().mul( terms[ _address ].percent ).div( 1e6 );
