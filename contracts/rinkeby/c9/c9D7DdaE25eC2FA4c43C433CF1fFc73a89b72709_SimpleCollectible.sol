...
contract SimpleCollectible is ERC721, Ownable {
    ...
    function buy() payable public returns (uint256)
    {
      ...
      uint256 sentinal = buyerValue;
      uint256 count = 0;

      require(buyerValue > 0, "spend some god damn fucking money");
      require(buyerValue % (5 * 10 ** 17) == 0, "Bro you suck cock at math");

      while(sentinal > 0)
      {
        count = count + 1 * 10 ** 18;
        sentinal = sentinal - 5 * 10 ** 17;
      }
