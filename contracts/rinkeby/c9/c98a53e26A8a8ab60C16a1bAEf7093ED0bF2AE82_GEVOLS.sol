...
contract GEVOLS is ERC721Burnable, Ownable {
    ...
    */
    function addAddressToWhitelist(address[] memory addrs, uint[] memory claimAmounts) onlyOwner public returns(bool success) {
        for(uint i = 0; i < addrs.length; i++) {
            whitelist[addrs[i]].addr = addrs[i];
            whitelist[addrs[i]].claimAmount = claimAmounts[i];
