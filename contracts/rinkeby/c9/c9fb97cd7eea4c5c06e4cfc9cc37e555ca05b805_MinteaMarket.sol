...
contract MinteaNFT is ERC721 {
    using SafeMath for uint256;

    uint256 constant public PercentUnit = 1000;
	uint256 constant public RoyaltyMax = 200; // 20 %
    uint256 constant public RoyaltyMin = 50; // 5 %
    ...
    function addItem(string memory _tokenURI, uint256 royalty) public returns (uint256){
        require(royalty <= RoyaltyMax, "too big royalties");
        require(royalty >= RoyaltyMin, "too small royalties");
    ...
    modifier creatorOnly(uint256 _id) {
        require(Items[_id].creator == _msgSender(), "ERC721Tradable#creatorOnly: ONLY_CREATOR_ALLOWED");
        _;
    }
}

interface IMinteaNFT {
	function initialize(string memory _name, string memory _uri, address creator, bool bPublic) external;	
...
contract MinteaMarket is Ownable, ERC721Holder {
    using SafeMath for uint256;
    ...
    function createCollection(string memory _name, string memory _uri, bool bPublic) public returns(address collection) {
        bytes memory bytecode = type(MinteaNFT).creationCode;
        ...
        IMinteaNFT(collection).initialize(_name, _uri, msg.sender, bPublic);
    ...
    function list(address _collection, uint256 _token_id, uint256 _price) OnlyItemOwner(_collection,_token_id) public {
        ...
        IMinteaNFT nft = IMinteaNFT(_collection);
    ...
    function delist(uint256 _id) external {        
        ...
        IMinteaNFT(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, pairs[_id].token_id);        
    ...
    function buy(uint256 _id) external ItemExists(_id) {
		...
		IMinteaNFT(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, pair.token_id);
    ...
    modifier OnlyItemOwner(address tokenAddress, uint256 tokenId) {
        IMinteaNFT tokenContract = IMinteaNFT(tokenAddress);
