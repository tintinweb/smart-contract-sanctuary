...
contract VectorField is ERC721URIStorage, VRFConsumerBase, Ownable {
    ...
    ERC721("VectorField", "rsNFT")
    {
        ...
        height = 2000;
        width = 1500;
        colors = ["#D5BA90", "#011C2D", "#B42C51", "#F5F7F4", "#D1D0D0", "#000c66", "#7EC8E3"];
    }
 ...
 function withdraw() public payable onlyOwner {
    ...
    function generateSVG(uint256 _randomness) public view returns (string memory finalSvg) {
        ...
        finalSvg = string(abi.encodePacked("<svg xmlns='http://www.w3.org/2000/svg' height='", uint2str(height), "' width='", uint2str(width), "' fill='none' viewBox='500 100 1500 2000'><path d='M500 100h1500v2000H0z'"));
        string memory color = colors[_randomness % colors.length];
        
        finalSvg = string(abi.encodePacked(finalSvg, " fill='", color,"'/>"));
    ...
    function generatePath(uint256 _randomness) public view returns(string memory pathSvg) {
        ...
        uint256 opacity = uint256(keccak256(abi.encode(_randomness + 2))) % 100;
