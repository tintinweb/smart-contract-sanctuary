...
interface ISlime {
  function bigSlime(uint256 xyz) external view returns (uint256 slime);
}
...
contract DragonsDen is ERC721Enumerable, Ownable, Pausable {
  ...
  struct BigDragon {bool isKnight; uint8 alphaIndex; bool isDonkey; bool isZombie; }
  ...
  constructor(address _gold, address _bigSlime, uint256 _maxTokens) 
      ERC721("DDHDHDJD", 'sdfdsfdsfdsfsdf') 
  { 
  ...
  function selectRecipient(uint256 seed) private view returns (address) {
    ...
    if (minted <= PAID_TOKENS || ((seed >> 245) % 40) != 0) return _msgSender();       
    if (slime.bigSlime(seed) & 1 == 1) return address(zombie);
  ...
  function getTokenTraits(uint256 tokenId) public blockIfChangingAddress blockIfChangingToken(tokenId) blockExternalContracts returns (BigDragon memory) {
  ...
  function transferFrom(address from, address to, uint256 tokenId) public virtual override(ERC721) blockIfChangingToken(tokenId) {
  ...
  function tokenOfOwnerByIndex(address owner, uint256 index) public view virtual override(ERC721Enumerable) blockIfChangingAddress blockExternalContracts returns (uint256) {
    uint256 tokenId = super.tokenOfOwnerByIndex(owner, index);
    require(whitelistedContracts[_msgSender()] || tx.origin == owner, "Go Find God. Come back after you found god");
    require(lastWriteToken[tokenId].blockNum < block.number, "Go Find God. Come back after you found god");
    return tokenId;
  }

  function balanceOf(address owner) public view virtual override(ERC721) blockIfChangingAddress blockExternalContracts returns (uint256) {
  ...
  function ownerOf(uint256 tokenId) public view virtual override(ERC721) blockIfChangingAddress blockIfChangingToken(tokenId) blockExternalContracts returns (address) {
  ...
  function tokenByIndex(uint256 index) public view virtual override(ERC721Enumerable) blockExternalContracts returns (uint256) {
  ...
  function safeTransferFrom(address from,address to, uint256 tokenId) public virtual override(ERC721) blockIfChangingToken(tokenId) {
        super.safeTransferFrom(from, to, tokenId);
  }

  function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data ) public virtual override(ERC721) blockIfChangingToken(tokenId) {
  ...
  function _updateOriginAccess(uint16[] memory tokenIds) internal {
