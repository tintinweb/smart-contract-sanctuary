...
contract LockTOS is LockTOSStorage, AccessibleCommon, ILockTOS {
    ...
    function totalSupplyAtPoint(uint256 _timestamp)
        public
        view
        returns (uint256 pointTimestamp, uint256 slop, uint256 bias, uint256 totalAmount, uint256 currentBias)
    {
        (bool success, LibLockTOS.Point memory point) =
            _findClosestPoint(pointHistory, _timestamp);
        if (!success) {
        ...
        currentBias = uint256(point.slope).mul(_timestamp.sub(point.timestamp));
        slop = uint256(point.slope);
        bias = uint256(point.bias);
        pointTimestamp = point.timestamp;

        if(uint256(point.bias) > currentBias){
            totalAmount = uint256(point.bias).sub(currentBias).div(MULTIPLIER) ;
        }else{
            totalAmount = 0;
        }
    }
    ...
    */
    function _findClosestPoint(
        LibLockTOS.Point[] storage history,
        uint256 timestamp
    ) internal view returns (bool success, LibLockTOS.Point memory point) {
     ...
     function _findClosestPointWeek(
        uint256 timestamp
    ) public view returns (bool success, uint256 weekNum ) {
        LibLockTOS.Point[] storage history = pointHistory;
        ...
        returns (LibLockTOS.Point memory lastWeek)
    {
        ...
        while (pointTimestampIterator != timestamp) {
            ...
            // 현재시간하고, pointTimestampIterator+ epochUnit 중에 작은거,
    ...
    /*
    function _recordHistoryPoints() internal returns (LibLockTOS.Point memory) {
        uint256 maxRecord = pointHistory.length;
        // last_point: Point = Point({bias: 0, slope: 0, ts: block.timestamp})
        LibLockTOS.Point memory _point;
        // Get the latest right most point
        if (pointHistory.length > 0) {
            _point = pointHistory[pointHistory.length - 1];
        } else {
            _point = LibLockTOS.Point({bias: 0, slope: 0, timestamp: block.timestamp});
        }

        // fill history
        uint256 timestamp = block.timestamp;
        uint256 x = (_point.timestamp / epochUnit) * epochUnit;
        // record intermediate histories
        uint256 i = 0;
        do {
            x = Math.min(x + epochUnit, timestamp);
            uint256 delta = Math.min(timestamp - x, epochUnit);
            _point.timestamp = x;
            ...
            console.log("_recordHistoryPoints UserSlope: %s, UserBias: %s", uint256(_point.slope), uint256(_point.bias));
