...
import './../libraries/TransferHelper.sol';
import './../libraries/DexSwapOracleLibrary.sol';
import './../libraries/DexSwapLibrary.sol';
import './../libraries/SafeMath.sol';
...
contract OracleCreator {
    ...
    function createOracle(
        ...
        IDexSwapPair sourcePair = IDexSwapPair(pair);
    ...
    function update(uint256 oracleIndex) public {
        ...
        (uint price0Cumulative, uint price1Cumulative, uint32 blockTimestamp) =
            DexSwapOracleLibrary.currentCumulativePrices(address(oracle.pair));
...
library DexSwapOracleLibrary {
    ...
    function currentCumulativePrices(
        ...
        price0Cumulative = IDexSwapPair(pair).price0CumulativeLast();
        price1Cumulative = IDexSwapPair(pair).price1CumulativeLast();

        // if time has elapsed since the last update on the pair, mock the accumulated price values
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = IDexSwapPair(pair).getReserves();
