...
contract DeepWatersVault is IDeepWatersVault, Ownable {
    ...
    function getDepositAPY(address _asset) external view returns (uint256) {
        ...
        if (usersList.length == 0) {
            ...
            for (uint256 j = 0; j < usersList.length; j++) {
                ...
                data.lastUpdate = usersCumulatedLiquidityIndexes[_asset][data.user].lastUpdate; 
                if (data.lastUpdate == 0) {
                    data.lastUpdate = block.timestamp;
                }
                
                data.deltaT = block.timestamp.sub(data.lastUpdate);
        ...
        return data.userCount == 0 ? 0 : data.depositAPY.div(data.userCount);
    }
    ...
    **/
    function getUserAssetCumulatedLiquidityIndex(address _asset, address _user) public view returns (uint256) {
        ...
        return userAssetCumulatedLiquidityIndex == 0 ? uint256(1).mul(10**INDEX_DECIMALS) : userAssetCumulatedLiquidityIndex;
    }
