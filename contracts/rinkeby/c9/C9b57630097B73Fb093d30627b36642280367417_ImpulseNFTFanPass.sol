...
contract ImpulseNFTFanPass is ERC721Enumerable, Ownable {
    ...
    uint256 public nftPassReserve = 100;
    ...
    constructor() ERC721("ImpulseNFT Fan Pass","PASS"){
    ...
    function reserveNFTPass(address _to, uint256 _reserveAmount)
        public
        onlyOwner
    {
        uint256 supply = totalSupply();
        require(
            _reserveAmount > 0 && _reserveAmount <= nftPassReserve,
            "Not enough reserve left for team"
        );
        for (uint256 i = 0; i < _reserveAmount; i++) {
            _safeMint(_to, supply + i);
        }
        nftPassReserve = nftPassReserve.sub(_reserveAmount);
    }

    function mintNFTPass(uint numberOfTokens) public payable {
        ...
        require(totalSupply().add(numberOfTokens) <= MAX_SUPPLY.sub(nftPassReserve), "Purchase would exceed max supply of NFTPasses");
    ...
    function mintNFTPassPresale(uint numberOfTokens) public payable {
        ...
        require(_allowListClaimed[msg.sender] + numberOfTokens <= 3,"Exceeds supply of presale NFTPasses you can mint.");
        require(numberOfTokens > 0, "Number of tokens can not be less than or equal to 0");
        require(NFT_PRICE.mul(numberOfTokens) <= msg.value, "Sent ether value is incorrect");
