...
interface IAvnNftRoyaltyStorage {
  ...
  function getRoyalties(uint256 id) external view returns(Royalty[] memory); // onlyPermitted
}
...
interface IAvnNftListingsV1 {
  ...
  event LogSold(uint256 indexed nftId, address indexed buyer);
...
contract AvnNftListingsV1 is IAvnNftListingsV1, Owned {
    ...
    isNotListed(_nftId)
  {
    ...
    checkProof(keccak256(abi.encode(AUCTION_INSTRUCTION,
        address(this), _nftId, msg.sender, _endTime, _avnOpId, _royalties)), _proof);
    ...
    isListedForSale(_nftId)
  {
    ...
    emit LogSold(_nftId, msg.sender);
