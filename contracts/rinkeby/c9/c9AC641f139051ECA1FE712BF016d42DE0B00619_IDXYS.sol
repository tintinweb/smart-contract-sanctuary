...
contract IDXYS {
  ...
  */
  ...
  struct _Assets{
        ...
        address[] protocols;                                          // adress of protocol interface ex COMP
        bool active;                                                  // Safety. 
     }

     
    mapping(address => mapping(uint256 => bool)) userAccessLevel;     // user => market  => true or false
    ...
    mapping(address => bool) assetRegistered;                         // asset authorized
  ...
  */
    ...
    /// @dev Make sure you have the allowance    
    /// @param _asset  ERC2020 the asset to deposit
    /// @param _amount uint the amount to deposit

  function depositErc(IERC20 _asset, uint256 _amount, uint256 _market)  public  {
    require(userAccessLevel[msg.sender][_market]);                                               // user is registered on market
    require(_amount>0,'Zero amount?');                                                           // canot be Zero
    require(address(_asset) != ETHER,'Use depositEth');                                          // is not ether
    require(registeredAssets[address(_asset)],'Asset Unknown?');                                 // is registered
  //  require(_asset.transferFrom(msg.sender, address(this), _amount),'Available Funds?');       // is transfered   <----------------------- 1 * approval
    balances[msg.sender][address(_asset)] = balances[address(_asset)][msg.sender].add(_amount);  // The balance 
    _Deposits storage _deposit = deposits[depositCount];
    
    _deposit.id = depositCount;
    _deposit.owner = msg.sender;
    _deposit.amount = _amount;
    _deposit.asset = address(_asset);
    _deposit.timestamp = block.timestamp;
    depositCount = depositCount + 1; 

    // _depositToMarket(_market, _asset, _amount);
    
    emit Deposit(address(_asset), msg.sender, _amount);
    }
  ...
  function depositETH(uint _market)  public payable  {
    ...
    require(userAccessLevel[msg.sender][_market]);                                // user is registered on market
    require(registeredAssets[ETHER],'Asset Unknown?');                            // is registered
