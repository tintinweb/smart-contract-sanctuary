...
contract SPCatItem is ERC1155Tradable {
    string private _baseUrl;

    constructor (address _proxyRegisteryAddress, string memory baseUrl)
        ERC1155Tradable(
            "SP Cat Item",
            "SPCI",
            string(abi.encodePacked(baseUrl, "/metadata/cat/{id}")),
            _proxyRegisteryAddress) {
        _baseUrl = baseUrl;
    }

    function contractURI() public view returns (string memory) {
        return string(
            abi.encodePacked(_baseUrl, "/contract/sp-cat-erc1155")
        );
    }

    function setBaseUrl(string memory _newUrl) public onlyOwner {
        _baseUrl = _newUrl;
        setURI(string(abi.encodePacked(_baseUrl, "/metadata/cat/{id}")));
    }
}
