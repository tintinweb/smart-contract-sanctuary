...
contract NFTFactory is FactoryERC721, Ownable {
    ...
    function mint(uint256 _optionId, address _toAddress) override public {
        ...
        _NormalNFTCX nftInstance = _NormalNFTCX(nftAddress);
    ...
    function debugOwner() public view returns (string memory){
        ProxyRegistry proxyRegistry = ProxyRegistry(proxyRegistryAddress);
        bool fir = address(proxyRegistry.proxies(owner())) == _msgSender();
        bool sec = owner() == _msgSender() ;
        address proxoi = address(proxyRegistry.proxies(owner()));
        address sender = _msgSender();
        
        return string(abi.encodePacked(fir, " - ", sec, " - ", (proxoi), " - ",(sender)));
    }
