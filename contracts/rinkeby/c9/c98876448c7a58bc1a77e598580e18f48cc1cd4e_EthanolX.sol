...
contract EthanolX is Ownable, IERC20Metadata {
    ...
    uint256 public weeklyPayoutRewards;
    ...
    function _distributeTax(uint256 _amount) internal returns(uint8) {
        ...
        */
        ...
        weeklyPayoutRewards += _splitedAmount;
    ...
    function weeklyPayout() external {
        require(block.timestamp >= (weeklyPayouts[_msgSender()] + 7 days), "EthanolX: ETH payouts is restricted to once per week");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();

        uint256 _rewards = (weeklyPayoutRewards * calculateDailyCashback(_msgSender())) / 100;
        weeklyPayouts[_msgSender()] = block.timestamp;
        weeklyPayoutRewards -= _rewards;

        // approve uniswapV2Router to swap trade tokens
        ...
        emit WeeklyPayout(_msgSender(), _rewards, block.timestamp);
    }
