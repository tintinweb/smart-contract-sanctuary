...
import "./interfaces/ICoinDeedFactory.sol";
    ...
    ILendingPool
{
    ...
    struct PoolInfo {
        ...
        uint256 borrowIndex;
        uint256 supplyIndex; // tăng mãi mãi
    ...
    modifier onlyDeed(address _address) {
        require(
            coinDeedFactory.isDeed(msg.sender),
            "Coindeed: Caller is not a Deed"
        );
        _;
    }

    function setContractDeedFactoryAddress(address _coinDeedFactoryAddress)
        public
        onlyOwner
    {
        coinDeedFactory = ICoinDeedFactory(_coinDeedFactoryAddress);
    }
    ...
    function claimDToken(address _tokenAddress) external {
        ...
        if (pending > 0) {
            ...
            dao.claimDToken(_tokenAddress, msg.sender, pending);
        }
    ...
    function borrow(address _tokenAddress, uint256 _amount) external override {
        require(
            deedInfo[msg.sender].isValid || coinDeedFactory.isDeed(msg.sender),
...
interface IDao {
    ...
    function claimCoinDeedManagementFee(
...
interface ICoinDeedFactory {
    ...
    */
    ...
    function isDeed(address _address) external view returns(bool);
}
