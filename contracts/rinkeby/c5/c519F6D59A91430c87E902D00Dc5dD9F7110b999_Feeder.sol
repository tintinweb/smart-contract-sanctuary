...
contract Feeder is OwnableUpgradeable, IFeeder {
    ...
    function punish(uint256 punishmentValue) external override onlyRouter {
        require(
            securityDeposite >= punishmentValue,
            "FEED: punishmentValue exceeded!"
        );
        securityDeposite = securityDeposite - punishmentValue;
    }
    ...
    function createAgreement(
        ...
        IERC20(router.getProtocolToken()).transfer(
            address(router),
            jobPenalty + taskResultPenalty
        );
    }
    ...
    function cancelAgreement(uint256 agreementID) external override onlyOwner {
        router.cancelAgreement(agreementID);
    }

    /**
     * @dev permisionless deposite azuro token
     * @param amount of azuro tokens
     */

    function deposite(uint256 amount) external override {
        IERC20(router.getProtocolToken()).transferFrom(
            msg.sender,
            address(this),
            amount
        );
    ...
    /**
     * @dev owner withdraw azuro token
     * @param amount of azuro tokens
     */

    function withdraw(uint256 amount) external override onlyOwner {
        IERC20(router.getProtocolToken()).transfer(owner(), amount);
    }
}
