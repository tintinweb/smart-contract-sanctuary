...
/**
 * @title TheSampleNft contract
 */
contract TestGenies4 is ERC721Enumerable, Ownable {
    ...
    modifier whenSaleActive {
        require(isSaleActive || block.number >= saleStartBlock, "TheSampleNft: Sale is not active");
        _;
    }

    modifier whenPresaleActive {
        require(isPresaleActive || block.number >= presaleStartBlock, "TheSampleNft: Presale is not active");
        require(presaleMinted < MAX_PRESALE_ITEMS, "TheSampleNft: Presale sold out");
        _;
    }

    constructor() ERC721("Test Genies 4", "TGEN4") {
        presaleStartBlock = 9257720;
        saleStartBlock = 9257750;

        MAX_ITEMS = 30;
        MAX_PRESALE_ITEMS = 10;
        COMMUNITY_ITEMS = 10;
    ...
    function mint(uint256 amount) external payable whenSaleActive {
        ...
        require(amount <= maxPerMint, "TheSampleNft: Amount exceeds max per mint");
        require(publicMinted + amount <= PUBLIC_ITEMS, "TheSampleNft: Purchase would exceed cap");
        require(mintPrice * amount <= msg.value, "TheSampleNft: Ether value sent is not correct");
    ...
    function presaleMint(uint256 amount) external payable whenPresaleActive {
        require(amount <= maxPerPresaleMint, "TheSampleNft: Amount exceeds max per presale mint");
        require(presaleMinted + amount <= MAX_PRESALE_ITEMS, "TheSampleNft: Purchase would exceed presale supply cap");
    ...
    function mintForCommunity(address _to, uint256 _numberOfTokens) external onlyOwner {
        require(_to != address(0), "TheSampleNft: Cannot mint to zero address.");
        require(totalSupply() + _numberOfTokens <= MAX_ITEMS, "TheSampleNft: Minting would exceed cap");
        require(communityMinted + _numberOfTokens <= COMMUNITY_ITEMS, "TheSampleNft: Minting would exceed community cap");
