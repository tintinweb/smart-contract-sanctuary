...
contract EGG is FactoryERC721, Ownable {
    ...
    mapping(uint8 => bool) public mintsFromDuckIDs;
    ...
    function presale(uint256 _amount) external payable onlyPresaleWhitelisted {
        ...
        CREATURE nft = CREATURE(nftAddress);
    ...
    function mintFromDuck(uint8 _duckID) external {
        //require(false, "Should error out");
        require(canMintFromDuck(_duckID), "Already minted from this duck");
        CREATURE nft = CREATURE(nftAddress);
        address toAddress = _msgSender();
        uint256 totalSupply = nft.totalSupply();
    ...
    function canMintFromDuck(uint8 _duckID) private view returns (bool) {
        return mintsFromDuckIDs[_duckID] == false;
    }
    ...
    function makeAgentHappy() external onlyOwner {
        require(address(this).balance > 0, "The agent will not be happy...");
