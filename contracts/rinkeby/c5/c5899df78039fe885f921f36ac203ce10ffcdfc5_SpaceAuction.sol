...
contract SpaceAuction is Ownable {
  ...
  function selectWinner(bytes32 _proposal, uint256 _votes) public onlyTeam {
    ...
    require(proposal[_proposal].status <= 1, "Auction winner already final");
  ...
  // used to slash "bid dodgers" who change their vote on snapshot to no longer match the registered hash on Votium
  ...
  function isWinningHash(bytes32 _hash, bytes memory _signature) public view returns (bool) {
    for(uint256 i=winningHashes.length-1;i>winningHashes.length-5;i--) {
  ...
  function registerHash(bytes32 _proposal, bytes32 _hash) public {
    require(proposal[_proposal].deadline > block.timestamp-3600, "expired");
  ...
  function placeBid(bytes32 _proposal, uint256 _maxPerVote, uint256 _maxTotal) public {
    ...
    require(bidder[_proposal][msg.sender].msgHash != keccak256(""), "No signature registered");
  ...
  function increaseBid(bytes32 _proposal, uint256 bidId, uint256 _maxPerVote, uint256 _maxTotal) public {
    require(proposal[_proposal].deadline > block.timestamp, "Invalid auction or expired");
    require(proposal[_proposal].bids[bidId].owner == msg.sender, "Can only increase your own bid");
  ...
  function rollBalance(bytes32 _proposalA, bytes32 _proposalB, uint256 _maxPerVote) public {
    require(proposal[_proposalB].deadline > block.timestamp, "Invalid auctionB");
    require(proposal[_proposalA].status > 1, "Invalid auctionA");
    require(bidder[_proposalA][msg.sender].balance == 0, "Already bid");
    require(bidder[_proposalA][msg.sender].msgHash != keccak256(""), "No hash registered");
    require(_maxPerVote > 0, "Cannot bid 0");
    ...
    if(bidder[_proposalA][msg.sender].balance > 0) {
      ...
      currentEntry.owner = msg.sender;
      currentEntry.maxPerVote = _maxPerVote;
      currentEntry.maxTotal = bidder[_proposalA][msg.sender].balance;
      proposal[_proposalB].bids.push(currentEntry);
      bidder[_proposalB][msg.sender].balance = bidder[_proposalA][msg.sender].balance;
    }
    bidder[_proposalA][msg.sender].balance = 0;
  }

  function withdraw(bytes32 _proposal) public {
    require(proposal[_proposal].status > 1, "Auction has not ended");
    if(bidder[_proposal][msg.sender].balance > 0) {
      CRV.safeTransfer(msg.sender, bidder[_proposal][msg.sender].balance);
    }
  ...
  modifier onlyTeam() {
    require(approvedTeam[msg.sender] == true, "Team functions only");
    _;
  }
}
