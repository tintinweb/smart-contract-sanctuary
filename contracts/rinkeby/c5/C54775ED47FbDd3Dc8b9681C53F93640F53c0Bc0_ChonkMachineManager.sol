...
contract ChonkMachineManager is ReentrancyGuard, Ownable, AccessControl {
    ...
    constructor(address _team, address _nft, address _taiyaki, address _weth) public {
        teamAccount = _team;
        nftAddress = _nft;
...
contract ChonkMachine {
    ...
    constructor(uint256 _machineId, //machine id
        ...
        teamAccount = _administrator;
        _staffAccountSet.add(administrator);

        machineOwner = option[0];
        setupMachineOption(option);

        _salt = uint256(keccak256(abi.encodePacked(_nftToken, _currencyToken, block.timestamp))) % 10000;
    }
