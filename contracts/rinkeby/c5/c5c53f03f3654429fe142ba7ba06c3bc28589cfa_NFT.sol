...
contract NFT is ERC721URIStorage {
    ...
    function batchMintFirstEdition(uint256 amount) external onlyOwner returns (bool) {
        uint256 i = 1;
        while (i <= amount) {
            i++;
            mintFirstEdition();
    ...
    function batchMint(uint256 amount, address recipient) external onlyOwner {
        uint256 i = 1;
        while (i <= amount) {
            i++;
            mint(recipient);
    ...
    function mintAfterTokenTransfer() external {
