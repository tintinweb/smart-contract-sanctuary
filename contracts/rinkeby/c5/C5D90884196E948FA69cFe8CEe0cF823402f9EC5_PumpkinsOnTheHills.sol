  ...
  uint internal nonce = 1293;
    ...
    function mintPumpkin(uint numberOfTokens) public payable {
      ...
      function trickOrTreat() public payable returns (uint) {
        ...
        require(saleIsActive, "Sale is not active at the moment");
        require(numberOfTokens > 0, "Number of tokens can not be less than or equal to 0");
        require(totalSupply().add(numberOfTokens) <= MAX_SUPPLY, "Purchase would exceed max supply of Pumpkins");
        require(numberOfTokens <= MAX_NFT_PURCHASE,"Can only mint up to 15 per purchase");
        require(NFT_PRICE.mul(numberOfTokens) == msg.value, "Sent ether value is incorrect");
        
   uint r = (uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, nonce)))) % 2;
    nonce++;
