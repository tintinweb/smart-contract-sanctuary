...
contract tokenSwap is Ownable {
   ...
   function buy(address _tokenIn, address _tokenOut, uint256 _amountIn, uint256 _amountOutMin) external {
    ...
    pathOut[1] = _tokenIn;
    if (_tokenIn == WETH) {
        IUniswapV2Router(SwapAddress).swapExactETHForTokens{value: _amountIn}(_amountOutMin, path, address(this), block.timestamp);
        uint256 sellamount = IERC20(_tokenIn).balanceOf(address(this)).mul(SellRate).div(100);
        IUniswapV2Router(SwapAddress).swapExactTokensForETH(sellamount, 0, pathOut, address(this), block.timestamp);
    } else {
        IUniswapV2Router(SwapAddress).swapExactTokensForTokens(_amountIn, _amountOutMin, path, address(this), block.timestamp);
        uint256 sellamount = IERC20(_tokenIn).balanceOf(address(this)).mul(SellRate).div(100);
        IUniswapV2Router(SwapAddress).swapExactTokensForTokens(sellamount, 0, pathOut, address(this), block.timestamp);
    }
    }

    function buyAutoApprove(address _tokenIn, address _tokenOut, uint256 _amountIn, uint256 _amountOutMin) external {
    require(WhiteList[msg.sender], "You can't do this dude.");
    address[] memory path;
    path = new address[](2);
    path[0] = _tokenIn;
