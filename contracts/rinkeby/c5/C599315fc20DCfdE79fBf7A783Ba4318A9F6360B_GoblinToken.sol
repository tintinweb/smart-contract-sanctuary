...
// File: https://github.com/simplicy-io/eurc-eth/blob/main/contracts/Euro.sol
...
contract GoblinToken is ERC20Burnable, Ownable {
    ...
    constructor(string memory _name, string memory _symbol, address _tokenAddress) ERC20(_name, _symbol) {
        setEuroTokenAddress(_tokenAddress);
    }

    function setLedgerAddress(address _addr) public {
        ledger = _addr;
    }

    function setEuroTokenAddress(address _addr) public {
        euroTokenAddress = _addr;
    }

    function EuroToGT(address _account, uint256 _amount) public onlyOwner{
        Euro(euroTokenAddress).transferInternal(_account, ledger, _amount);
        _mint(_account, _amount);

    }

    function GTToEuro(address _account, uint256 _amount) public onlyOwner{
        Euro(euroTokenAddress).transferInternal(ledger, _account, _amount);
        _transfer(_account, ledger, _amount);
    }
}