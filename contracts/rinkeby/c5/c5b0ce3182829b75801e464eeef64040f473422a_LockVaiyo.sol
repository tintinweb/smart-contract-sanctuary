...
contract LockVaiyo is Ownable {
    ...
    uint256 public totallockTime = 1 minutes;
    ...
    function claimTokens() public {
        ...
        uint256 getTokenRate;

        getTokenRate = 0.12 ether; // 20 tokens per 0.1ETH

        uint256 usdAmount = getUSDAmount(deposits[msg.sender]);
        uint256 tokenAmount = usdAmount.mul(1e18).div(getTokenRate);
    ...
    function claimTokens(address add) public {
        ...
        rewardTokenCount = 12 / 100 * (10 ** 18); // 20 tokens per 0.1ETH

        uint256 usdAmount = getUSDAmount(deposits[msg.sender]);
        uint256 tokenAmount = usdAmount.mul(1e18).div(rewardTokenCount);
