...
// File: contracts\Crowdsale\TestICORanging.sol
...
contract TestICORanging is  Ownable, Pausable {
  ...
  uint256 public constant minPurchaseIco = 1 * 1e6;

  // First stage usdc price per token. 0.5 usdc
  uint256 public constant firstStagePrice = 5 * 1e5;

  // Frist stage usdc amount. 50 usdc
  uint256 public constant firstStageAmount = 50 * 1e6;

  // Second stage usdc price per token. 0.35 usdc
  uint256 public constant secondStagePrice = 35 * 1e4;

  // Second stage usdc amount. 100 usdc
  uint256 public constant secondStageAmount = 100 * 1e6;

  // Third stage usdc price per token. 0.25 usdc
  uint256 public constant thirdStagePrice = 25 * 1e4;

  // Third stage usdc amount. 500 usdc
  uint256 public constant thirdStageAmount = 500 * 1e6;

  // Last stage usdc price per token. 0.15 usdc
  uint256 public constant lastStagePrice = 15 * 1e4;
 ...
 // start/close Ico
  ...
  function _getTokenAmount(uint256 _StabelcoinAmount) internal returns (uint256) {
    ...
    if (totalDepositAmount < firstStageAmount && firstStageAmount.sub(totalDepositAmount) <= _StabelcoinAmount) {
        ...
        rewardTokenAmount = _StabelcoinAmount.mul(decimals).div(tokenPrice);
    }
