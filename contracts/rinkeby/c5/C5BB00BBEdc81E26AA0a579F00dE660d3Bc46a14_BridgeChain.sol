...
contract BridgeChain is Ownable, Pausable{
  ...
  // 修改手续费
  event UpdateFee(address indexed operator, address indexed token, uint256 oldFee, uint256 newFee);
  // 修改限制
  ...
  // 增加设置
  function addConfig(address _token, uint256[] calldata _supportChains, uint256 _fee, uint256 _singleMin, uint256 _singleMax) external onlyManager returns (bool) {
    require(_token != address(0), "BridgeChain: token is the zero address");
  ...
  // 修改费用
  function updateFee(address _token, uint256 _fee) external onlyManager returns (bool) {
    ...
    emit UpdateFee(msg.sender, _token, bridgeConfig.fee, _fee);
    bridgeConfig.fee = _fee;
    return true;
  }
  // 修改支持
  function updateSupport(address _token, uint256[] calldata _supportChains) external onlyManager returns (bool) {
    require(_supportChains.length > 0, 'BridgeChain: at least one chain is supported');
    BridgeConfig storage bridgeConfig = configMapping[_token];
    require(bridgeConfig.token != address(0), 'BridgeChain: token does not exist');
  ...
  // 修改单笔限额
  function updateSingle(address _token, uint256 _singleMin, uint256 _singleMax) external onlyManager returns (bool) {
    require(_singleMax > _singleMin, 'BridgeChain: singleMax must be greater than singleMin');
    BridgeConfig storage bridgeConfig = configMapping[_token];
    require(bridgeConfig.token != address(0), 'BridgeChain: token does not exist');
  ...
  // 启/停设置
  function updateEnable(address _token, bool _enable) external onlyManager returns (bool) {
    BridgeConfig storage bridgeConfig = configMapping[_token];
    require(bridgeConfig.token != address(0), 'BridgeChain: token does not exist');
