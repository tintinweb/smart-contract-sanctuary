...
interface IStakeShiba
{
   ...
   function ownTokenAddress() external view returns(address);
   function externaltoken() external view returns(bool);
...
contract AIPool {
    ...
    function sendToOnlyExchangeContract() public onlyAdministrator returns(bool)
    {
        ...
        bool externaltoken  = IStakeShiba(stakeshiba).externaltoken();
        address token_Address =  IStakeShiba(stakeshiba).tokenAddress();
        if(!externaltoken)
        {
          token_Address=IStakeShiba(stakeshiba).ownTokenAddress();
        }
