...
abstract contract AbstractLocker is Initializable, OwnableUpgradeable {
    ...
    bytes32 private constant WITHDRAW_TYPEHASH=keccak256(abi.encodePacked(
        "Withdraw(uint256 claimId,uint256 targetChainGuid,address targetLockerAddress,address targetAddress,uint256 amount,uint256 deadline)"
    ));
    bytes32 private constant REFUND_TYPEHASH=keccak256(abi.encodePacked(
        "Refund(uint256 claimId,uint256 sourceChainGuid,address sourceLockerAddress,address sourceAddress,uint256 amount)"
    ));
    bytes32 private DOMAIN_SEPARATOR;
    uint256 private chainGuid;
    ...
    event Deposit(address indexed sender, uint256 indexed targetChainGuid, address targetLockerAddress, address indexed targetAddress, uint256 amount);
    ...
    function initialize(
        ...
        __AbstractLocker_init(_chainGuid, _token, _oracleAddress, _feeAddress, _feeBP);
    }
    ...
    function deposit(
        ...
        // Checks
        require(_targetChainGuid != chainGuid || _targetLockerAddress != address(this));
        ...
        emit Deposit(msg.sender, _targetChainGuid, _targetLockerAddress, _targetAddress, _amount);
    }
    ...
    function withdraw(
        ...
        // Checks
        require((chainGuid == _targetChainGuid) && (address(this) == _targetLockerAddress), 'withdraw: wrong chain');
    ...
    function refund(
        ...
        // Checks
        require((chainGuid == _sourceChainGuid) && (address(this) == _sourceLockerAddress), 'refund: wrong chain');
