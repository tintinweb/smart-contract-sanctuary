...
contract AuctionV3 is ManagerAuction {
	...
	function migrateAuction(
		...
		IAuctionVersion oldContract = IAuctionVersion(_oldContract);
		totalAuctions = oldContract.totalAuctions();
		...
		for (uint256 i = _fromId; i < _toId; i++) {
			...
			) = oldContract.auctions(i);
			// bool isERC721 = IERC721Upgradeable(tokenAddress).supportsInterface(_INTERFACE_ID_ERC721);
			// if (isERC721 && oldContract.versionOnAuction(tokenAddress, tokenId, 1)) {
			...
			for (uint256 j = 0; j < listBidId.length; j++) {
				newAuction.listBidId[j] = listBidId[j];
			}
	...
	function migrateBid(
		...
		IAuctionVersion oldContract = IAuctionVersion(_oldContract);
		totalBidAuctions = oldContract.totalBidAuctions();
