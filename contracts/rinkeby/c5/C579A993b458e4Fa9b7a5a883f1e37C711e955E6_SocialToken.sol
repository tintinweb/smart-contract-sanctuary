...
contract SocialToken is ERC20, Ownable {
    ...
    function buyToken(uint256 amount) public {
        ...
        uint256 tradeActivityFee =
            priceForTokens.mul(tradingSpread).div(MAX_PERCENTAGE);

        require(
            fundingToken.transferFrom(
                msg.sender,
                address(this),
                priceForTokens.add(tradeActivityFee)
            ),
            "Sender does not have enough funding token"
        );

        tradeAccumulatedFee = tradeAccumulatedFee.add(tradeActivityFee);
    ...
    function sellToken(uint256 amount) public {
        ...
        uint256 tradeActivityFee =
            fundingTokensToReturn.mul(tradingSpread).div(MAX_PERCENTAGE);

        fundingToken.transfer(
            msg.sender,
            fundingTokensToReturn.sub(tradeActivityFee)
        );
     ...
     */
    function withdrawSocialToken(uint256 amount) public onlyOwner {
        require(
     ...
     */
    function withdrawFundingToken(uint256 amount) public onlyOwner {
        require(
            tradeAccumulatedFee >= amount,
            "Withdrawl amount of accumulated funding token is not sufficient"
        );
