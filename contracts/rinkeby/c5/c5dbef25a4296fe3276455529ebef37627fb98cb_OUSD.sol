...
contract OUSD is ERC20, Ownable {
    ...
    function collect() external {
        if (terms.interest > 0) {
            ...
            terms.outstanding += terms.interest;
            terms.interest = 0;
        }
    }
    ...
    function _canWithdraw (address depositor, uint256 exchangeRate) internal view returns (uint256) {
        ...
        uint256 locked = info.collateral * (info.borrowed + info.interest) / _maxBorrow(info.collateral, exchangeRate);
    ...
    function currentInterest(address depositor) public view returns (uint256) {
        ...
        uint256 interest = userInfo[depositor].borrowed * terms.IR * blocksPassed / INTEREST_DECIMALS;
        return userInfo[depositor].interest + interest;
    }
    ...
    // collateral value minus debts
    function equity (address depositor) external view returns (uint256) {
        return collateralValue(depositor) - (userInfo[depositor].borrowed + userInfo[depositor].interest);
    }

    // minimum collateral value minus debts before eligible for liquidation
    function minimumEquity (address depositor) external view returns (uint256) {
        return (userInfo[depositor].borrowed + userInfo[depositor].interest) * LIQUIDATION_DECIMALS / terms.LT;
    }
