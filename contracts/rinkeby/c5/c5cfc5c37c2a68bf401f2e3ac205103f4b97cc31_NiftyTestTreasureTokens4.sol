...
contract NiftyTestTreasureTokens4 is ERC721Enum, Ownable, PaymentSplitter, ReentrancyGuard {
    ...
    function setPayout(address[] calldata _addressList, uint[] calldata _shareList) external onlyOwner{
        uint percentage = 100;
        uint totalShares = 0;
        require(_shareList.length == _addressList.length, "Address and quantity length must be equal");
        for(uint i = 0; i < _shareList.length; ++i){
            totalShares += _shareList[i];
        }
        require(percentage == totalShares, "Total payout shares must equal 100");
        delete percentage;
        delete totalShares;
