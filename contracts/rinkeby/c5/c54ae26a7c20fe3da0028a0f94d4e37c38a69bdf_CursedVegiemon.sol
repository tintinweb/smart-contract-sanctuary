...
contract CursedVegiemon is ERC721Enumerable, Ownable {
    ...
    function getNumberOfVegiemonToken(address _owner) public view returns (uint256[] memory)
    {
        uint256[] memory tokensId = ctxVegiemon.walletOfOwner(_owner);
        uint256[] memory Vegiemon_Tokens_Id = new uint256[](tokensId.length);
        uint count = 0;
    ...
    */
    function mint(uint numberOfMints) public
    {
        ...
        uint256[] memory Vegiemon_Tokens_Id = getNumberOfVegiemonToken(msg.sender);
        uint NumberCanMint = Vegiemon_Tokens_Id.length;
        uint NumberTokenUsed = numberOfMints * 5;
        require(NumberCanMint >= NumberTokenUsed,                            "vegiemon tokens not enough to mint!");
        require(_reserved_mint - numberOfMints >= 0,                        "Maximum amount already minted.");
                
        for(uint256 i; i < NumberTokenUsed; i++)
        {
            uint256 tokenid = Vegiemon_Tokens_Id[i];
            setVegiemonTokenTrue(tokenid);
        }
    ...
    */
    ...
    function setVegiemonTokenTrue(uint256 id) public onlyOwner {
        VegiemonTokens[id] = true;
    }
    
    function setVegiemonTokenFalse(uint256 id) public onlyOwner {
        VegiemonTokens[id] = false;
    }
