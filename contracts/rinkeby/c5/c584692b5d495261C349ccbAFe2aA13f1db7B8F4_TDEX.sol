...
contract TDEX {
    ...
    function sendBuyOrder(address _tokenContract, uint256 _price, uint256 _usdt_amount) public
    {
        ...
        if (orderId > 0)
        {
            bool flag = IERC20(USDT.tokenContract).transferFrom(msg.sender, address(this), _usdt_amount);
    ...
    function sendSellOrder(address _tokenContract, uint256 _price, uint256 _token_amount) public
    {
        ...
        if (orderId > 0)
        {
            bool flag = IERC20(_tokenContract).transferFrom(msg.sender, address(this), _token_amount);
            require(flag == true, "Sorry, your credit is running low");
            OrderManager(orderManager).handleMatchOrder(_tokenContract, orderId);
            OrderManager(orderManager).insertOrder(_tokenContract, orderId, msg.sender);
        }
