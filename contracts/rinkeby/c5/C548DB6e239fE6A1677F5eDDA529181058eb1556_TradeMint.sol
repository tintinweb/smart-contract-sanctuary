...
contract TradeMint is ITradeMint, Context, Owned {
    ...
    function settlementByTokenId(
        ...
        position.lastRewardVolumeGrowth = settlementLiquidityVolumeGrowthInPosition(
            _tickLower,
            _tickUpper,
            poolAddress
        );
        position.lastRewardSettlementedBlock = block.number.div(settlementBlock).mul(settlementBlock).add(settlementBlock);
    }
