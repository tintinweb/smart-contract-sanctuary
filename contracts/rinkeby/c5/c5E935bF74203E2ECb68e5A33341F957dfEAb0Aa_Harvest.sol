...
contract Harvest {
  ...
  constructor(IERC20 _dmtToken) public {
    ...
    // 每个区块百分之一能获得的收益额度
    rewardRate = MAXREWARD / 100 / 365 / 24 / 60 / 20;
  ...
  // 获取投资人相关数据
  function selfData() external view returns (uint256, uint256, uint256) {
    Data storage funder = funders[msg.sender];
    
    // 最多领取收益数
    uint256 self_max_reward = MAXREWARD / 100 * funder.fundRate;

    // 未领取的收益数总数
    uint256 self_max_unclaimed = self_max_reward - funder.totalClaimed;

    // 当前可以领取的收益数
    uint256 diffBlock = block.number - funder.lastBlock;
    uint256 self_active_reward = rewardRate * funder.fundRate * diffBlock;
  ...
  function claim() external onlyFunder() {
    ...
    Data storage funder = funders[msg.sender];
    uint256 totalUnclaimed = MAXREWARD / 100 * funder.fundRate - funder.totalClaimed;
    uint256 diffBlock = block.number - funder.lastBlock;
    uint256 amount;

    // 本次领取收益 小于 剩余领取收益总额
    if(amount <= totalUnclaimed){
      amount = rewardRate * funder.fundRate * diffBlock;
    }else{
    ...
    dmtToken.safeTransferFrom(address(this), msg.sender, amount);
