...
contract ImnotArtExhibitions is ERC721Enumerable {
    ...
    function royaltyInfo(uint256 tokenId) external view onlyValidTokenId(tokenId) returns (address receiver, uint256 amount) {
        ...
        if (useRoyaltyContracts) {
            ...
            if (artistAddress == address(0)) {
                royaltyContractAddress = imnotArtPayoutAddress;
            } else {
                royaltyContractAddress = royaltyContractByArtistAddress[artistAddress];
            }

            return (royaltyContractAddress, combinedBpsForSinglePayout * 100);
        }

        return (imnotArtPayoutAddress, combinedBpsForSinglePayout * 100);
    }
}
