...
// File: contracts/mDao.sol

pragma solidity ^0.6.0;

contract mDao is ERC721Burnable,Ownable{
    ...
    event SaleMint(address account, uint256 tokenId,uint256 price);
    ...
    bytes32 public constant MINT_TYPEHASH =
        keccak256(
            "Mint(address account,uint256 amount,uint256 price,uint256 nonce)"
    );


    constructor()  public  ERC721("mDao","mDao") {}
    ...
    //     require(_count <= max,"gt max");
    //     require(nextTokenId + _count <= maxSupply + 1,"max supply exceeded");
    //     require(tokenPrice.mul(_count) == msg.value, "incorrect Ether value");
    ...
    function mint(
        ...
        require(nextTokenId + _amount <= maxSupply + 1,"max supply exceeded");
        
        require(_price.mul(_amount) == msg.value, "incorrect Ether value");
        ...
        for(uint256 i=0;i< _amount;i++){
            _safeMint(msg.sender, nextTokenId+i);
            emit SaleMint(msg.sender,nextTokenId + i,_price);
        }
