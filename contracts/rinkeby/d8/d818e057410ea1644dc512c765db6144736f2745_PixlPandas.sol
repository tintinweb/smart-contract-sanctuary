...
// File: contracts/pixlpandas.sol

/**
 * @title PixlPandas contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract PixlPandas is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public MAX_PIXL_SUPPLY;
    ...
    constructor() ERC721("Pixl Pandas", "PIXL") {
        MAX_PIXL_SUPPLY = 1499;
        REVEAL_TIMESTAMP = block.timestamp;
        mintPrice = 12500000000000000; // 0.125 ETH
        maxToMint = 10;
        saleIsActive = false;
        wallet1 = 0x710a68E9696AaCb80e187b1fBD45031f88Ab08D8;
        wallet2 = 0x710a68E9696AaCb80e187b1fBD45031f88Ab08D8;
    }
     ...
     * Mint PIXL by owner
     */
    function reservePIXL(address _to, uint256 _numberOfTokens) external onlyOwner {
    ...
    * Mints tokens
    */
    function mintPIXL(uint256 numberOfTokens) external payable {
        require(saleIsActive, "Sale must be active to mint");
        require(numberOfTokens <= maxToMint, "Invalid amount to mint per once");
        require(totalSupply().add(numberOfTokens) <= MAX_PIXL_SUPPLY, "Purchase would exceed max supply");
        require(mintPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
        
        for(uint256 i = 0; i < numberOfTokens; i++) {
            uint256 mintIndex = totalSupply();
            if (totalSupply() < MAX_PIXL_SUPPLY) {
    ...
    function setStartingIndex() external {
        ...
        startingIndex = uint256(blockhash(startingIndexBlock)) % MAX_PIXL_SUPPLY;
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number.sub(startingIndexBlock) > 255) {
            startingIndex = uint256(blockhash(block.number - 1)) % MAX_PIXL_SUPPLY;
        }
