...
contract BrainDanceNft is ERC721Enumerable, Ownable, HeroFactory {
    ...
    uint256 public preslaeAddressLimit = 2500;
    ...
    function mint(uint256 sign) public payable {
        require(!bPaused, "Paused");
        require(verifySignature(sign), "permission error");
        if (isPresale()) {
            require(balanceOf(msg.sender) + 1 <= tickets[msg.sender], "max presale error");
        }
    ...
    function buyTicket(uint256 sign) public payable {
        require(!ticketPaused, "paused");
        require(verifySignature(sign), "signature error");
        require(msg.value >= ticketPrice, "Value below price");
        require(tickets[msg.sender] < 2, "count error");

        if (tickets[msg.sender] == 0) {
            require(presaleReservedAddressCount + 1 <= preslaeAddressLimit, "address limit error");
