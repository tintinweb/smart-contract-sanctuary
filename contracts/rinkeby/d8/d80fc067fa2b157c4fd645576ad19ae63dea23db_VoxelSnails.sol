...
contract VoxelSnails is IERC721, Ownable, Functional {
    ...
    constructor() {
        _name = "VX Snails";
        _symbol = "VXSNL";
        _baseURI = "https://slimysnails.io/metadata/vx/";
    ...
    function airDrop(address[] memory voxeldrops) external onlyOwner {
        require(voxeldrops.length + numberMinted < totalTokens);
		
		uint256 mintSeedValue = numberMinted;
		
        for (uint256 i = 0; i <= voxeldrops.length; i++){
		    _safeMint(voxeldrops[i], mintSeedValue + i);
           	numberMinted ++;
        }
