...
contract recommender{
    ...
    Credit_token CT = Credit_token(0x0762cCCe6862D028FAe06a5248abb7Ce9AcaECBA);
...
contract decoffer_V2{
    ...
    Credit_token CT = Credit_token(0x0762cCCe6862D028FAe06a5248abb7Ce9AcaECBA);
    recommender RT = recommender(0x5a02cBbDAa7D9b5b171360CC6682C0FacDB0f69C);
...
contract CT_debt{
    ...
    decoffer_V2 DT = decoffer_V2(0xC635CB786952a1b7fF0a8E3a27ad5e5EbFFE0E64);
    Credit_token CT = Credit_token(0x0762cCCe6862D028FAe06a5248abb7Ce9AcaECBA);
    recommender RT = recommender(0x5a02cBbDAa7D9b5b171360CC6682C0FacDB0f69C);
    ...
    constructor()public{
        ...
        fee_info[1][1] = 100*10**18;  //1-1最小認購枚數
        fee_info[1][2] = 80;          //1-2匯兌比 
        fee_info[1][4] = 500;         //1-4 Actual profit
        fee_info[1][5] = 300;         //1-5收益抽成
    
        
        fee_info[3][1] = 17;         // credit
        fee_info[3][2] = 5;          // credit weight
    ...
    function subscribe()public payable{
        ...
        debt_info[msg.sender][3] = debt_info[msg.sender][3].add(msg.value.mul(fee_info[3][1]).div(1000).mul(fee_info[3][2]));
        debt_info[msg.sender][4] = debt_info[msg.sender][4].add(ct_amount);
        
        uint cost = msg.value.mul(fee_info[3][1]).div(1000);
        DT.input.value(msg.value)(msg.value.sub(cost),0);
        DT.sell_debt(msg.sender,ct_amount);
    ...
    function save_price(uint ct_amount)private{
        ...
        debt_info[msg.sender][2] =debt_info[msg.sender][2].add(price);
    ...
    function claim_profit()public {
        ...
        out_share = DT.get_out_share();
        now_balance = DT.get_balance();
        uint now_price = debt_info[msg.sender][4].mul(now_balance).div(out_share);
        //---先計算儲存CT的目前價格
        ...
        uint fee = actual.mul(fee_info[1][5]).div(1000); 
        uint _claim_profit = actual.sub(fee);
        
        uint ct_amount = profit.mul(out_share).div(now_balance);
        debt_info[msg.sender][4] = debt_info[msg.sender][4].sub(ct_amount);
        debt_info[msg.sender][6] = debt_info[msg.sender][6].add(_claim_profit);
        
        DT.CT_swap_ETH(msg.sender,ct_amount,_claim_profit,profit,fee);
    ...
    function withdraw()public {
        ...
        uint fee = actual.mul(fee_info[1][5]).div(1000); 
        uint total_profit = debt_info[msg.sender][1].add(actual).sub(fee);
        
        if(debt_info[msg.sender][3] >= debt_info[msg.sender][5]){
            uint fine = debt_info[msg.sender][3].sub(debt_info[msg.sender][5]);
            require(total_profit >= fine);
            total_profit = total_profit.sub(fine);
            DT.CT_swap_ETH(msg.sender,debt_info[msg.sender][4],total_profit,total_profit.add(fee),fee);
        }else{
            uint cost = debt_info[msg.sender][1].add(profit);
            DT.CT_swap_ETH(msg.sender,debt_info[msg.sender][4],total_profit,cost,fee);
        }
