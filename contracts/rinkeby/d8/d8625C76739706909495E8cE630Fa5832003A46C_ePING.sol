...
contract ePING is Context, IERC20, Ownable {
   ...
   function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
    ...
    function addAdmin(address newAdmin , bool removeAdmin) external onlyOwner {
        adminAddresses[newAdmin] = removeAdmin;

    }

    // function removeAdmin(address adminAddress) external onlyOwner {
    //     adminAddresses[adminAddress] = false;
    // }
    ...
    function _mint(address recipient, uint256 amount) private {
        ...
        valuesFromGetValues memory s = _getValues(amount, false);
    ...
    function _burn(address senderAddress, uint256 amount) private {
        ...
        _rOwned[senderAddress] = _rOwned[senderAddress].sub(s.rAmount);
