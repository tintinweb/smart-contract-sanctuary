...
//change "name1" and "symbol"
	...
	// get a variable that is the totalFees
	
	uint256 public totalFees = _taxFee + _liquidityFee + _burnFee + _charityFee + _marketingFee;
	...
	// This variable stops people from transferring or approving other contracts during the swap period
	// Once transferEnabled is set to "true", it cannot be toggled back to "false"
	
	bool public transferEnabled = false;
	
	//paste the addresses that can transfer during the swap period
    address public canTransferPawthDuringSwap1 = 0x8EA6cC82148D92F979D34031Bdba60CCD35b0f9e;
    address public canTransferPawthDuringSwap2 = 0x971dB08176bba44e7D5D0733D9f1127684033E47;
    address public canTransferPawthDuringSwap3 = 0xaf284EfD8C649522120a05bfeeB36F2b6e7c970C;
    address public canTransferPawthDuringSwap4 = 0x1C0592cdA2c8F0BED6BA88aB1e7E2E61421c0869;
    address public canTransferPawthDuringSwap5 = 0xaf284EfD8C649522120a05bfeeB36F2b6e7c970C;
    ...
    function _transfer(
        ...
        if (sender != canTransferPawthDuringSwap1 && sender != canTransferPawthDuringSwap2 && sender != canTransferPawthDuringSwap3 && sender != canTransferPawthDuringSwap4 && sender != canTransferPawthDuringSwap5) {
            require(transferEnabled ==true, "Certain transfers are not enabled during the swap period.");
        }
    ...
    function setTaxFee(uint256 fee) external onlyOwner {
        totalFees = (fee + _burnFee + _marketingFee + _liquidityFee + _charityFee);
        require(totalFees <=1000, "You cannot set total fees above 10%");
        _taxFee = fee;
    }
    
    function setBurnFee(uint256 fee) external onlyOwner {
        totalFees = (fee + _taxFee + _marketingFee + _charityFee + _liquidityFee);
        require(totalFees <=1000, "You cannot set total fees above 10%");
        _burnFee = fee;
    }
    
    function setLiquidityFee(uint256 fee) external onlyOwner {
        totalFees = (fee + _burnFee + _taxFee + _marketingFee + _charityFee);
        require(totalFees <=1000, "you cannot set total fees above 10%");
        _liquidityFee = fee;
    }
 
    function setMarketingFee(uint256 fee) external onlyOwner {
        totalFees = (fee + _charityFee + _burnFee + _liquidityFee + _taxFee);
        require(totalFees <=1000, "You cannot set total fees above 10%");
        _marketingFee = fee;
    }
	
	function setCharityFee(uint256 fee) external onlyOwner {
	   totalFees = (fee + _burnFee + _taxFee + _liquidityFee + _marketingFee);
	   require(totalFees <=1000, "You cannot set total fees above 10%");
		_charityFee = fee;
	}
    ...
    function setCanTransferPawthDuringSwap1(address _address) external onlyOwner {
        canTransferPawthDuringSwap1 = _address;
    }
    
    function setCanTransferPawthDuringSwap2(address _address) external onlyOwner {
        canTransferPawthDuringSwap2 = _address;
    }
    
    function setCanTransferPawthDuringSwap3_4_and_5(address _address1, address _address2, address _address3) external onlyOwner {
        canTransferPawthDuringSwap3 = _address1;
        canTransferPawthDuringSwap4 = _address2;
        canTransferPawthDuringSwap5 = _address3;
    }
    
    function revertToOriginalFees() external onlyOwner {
        _charityFee = 200;
        _burnFee = 0;
        _marketingFee = 100;
        _liquidityFee = 100;
