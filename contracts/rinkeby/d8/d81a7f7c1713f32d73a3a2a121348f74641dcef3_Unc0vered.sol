 ...
 */
contract Unc0vered is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    constructor(string memory name, string memory symbol) ERC721(name, symbol) {
        ...
        wallet1 = 0xD5F7d2764c9B45F1F39c7B6C9a4eed76C03BE8B7;
        wallet2 = 0x602211858d2Fa0110bC5841ECCA1E57567D9CC77;
    }
    ...
    function isPresaleLive() public view returns(bool) {
        ...
        if (presaleAllowed && presaleStartTimestamp <= curTimestamp && totalSupply() < maxPresaleSupply) {
            return true;
        }
        return false;
    }
    ...
    function reserveMannequins(address _to, uint256 _numberOfTokens) external onlyOwner {
        uint256 supply = totalSupply();
        require(supply + _numberOfTokens <= maxNftSupply, "Cannot reserve more than max supply");
        uint256 i;
    ...
    function doPresale(uint256 numberOfTokens) external payable {
        ...
        require(isPresaleLive(), "Whitelist sale has not started yet");
        require(presaleWhitelist[_msgSender()], "You are not on the whitelist");
        require(numberOfTokens.add(numOfUser) <= maxPresaleToMint, "Exceeds max whitelist mints allowed per user");
        require(totalSupply() <= maxPresaleSupply, "Exceeds max whitelist supply");
        require(numberOfTokens > 0, "Must mint at least one token");
        require(mintPrice.mul(numberOfTokens) == msg.value, "Ether value sent is incorrect");
    ...
    function doPublic(uint256 numberOfTokens) external payable {
        ...
        require(totalSupply() + numberOfTokens <= maxNftSupply, "Exceeds max supply");
        require(numberOfTokens > 0, "Must mint at least one token");
    ...
    function withdraw() external onlyOwner {
        ...
        uint256 balance2 = balance.mul(30).div(1000);
