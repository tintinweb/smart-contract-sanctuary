...
// File: contracts/bid/ERC1155BidStorage.sol

// contracts/ERC1155BidStorage.sol
...
contract ERC1155BidStorage {
    ...
    mapping(address => mapping(uint256 => mapping(uint256 => Bid))) bidsByToken;
...
// File: contracts/bid/ERC1155Bid.sol
...
contract ERC1155Bid is Ownable, Pausable, ERC1155BidStorage {
    ...
    */
    constructor(address _erc20Token, address _owner) Ownable() Pausable() {
        ERC20Token = ERC20Interface(_erc20Token);
        // Set owner
    ...
    */
        ...
        whenNotPaused()
    {
        ...
        if (bidFeePerMillion > 0) {
            ...
            require(
                ERC20Token.transferFrom(msg.sender, owner(), feeAmount),
    ...
    */
        ...
        returns (bytes4)
    {
        ...
        if (ownerCutPerMillion > 0) {
            ...
            require(
                ERC20Token.transferFrom(bidder, owner(), saleShareAmount),
                "Transfering the cut to the bid contract owner failed"
            );
        }

        // Transfer ERC20Token from bidder to seller
        require(
            ERC20Token.transferFrom(bidder, _from, price.sub(saleShareAmount)),
            "Transfering ERC20Token to nft owner failed"
        );
        ...
        if (inflationPerMillion > 0) {
            ...
            // Transfer share amount to the bid conctract Owner
            ERC20Token.mint(owner(), mintShareAmount);
        }
    ...
    */
    function _requireBidderBalance(address _bidder, uint256 _amount) internal view {
        require(
            ERC20Token.balanceOf(_bidder) >= _amount,
            "Insufficient funds"
        );
        require(
            ERC20Token.allowance(_bidder, address(this)) >= _amount,
            "The contract is not authorized to use erc20 token on bidder behalf"
        );        
    }
}