...
contract RiskPool is IRiskPool, RiskPoolERC20 {
    ...
    function leaveFromPending(address _to) external override onlySSIP {
        uint256 cryptoBalance = IERC20(currency).balanceOf(address(this));
        require(cryptoBalance > 0, "UnoRe: zero uno balance");
        require(balanceOf(_to) >= uint256(withdrawRequestPerUser[_to].pendingAmount), "UnoRe: lp balance overflow");
        uint256 pendingAmountInUno = (uint256(withdrawRequestPerUser[_to].pendingAmount) * lpPriceUno) / 1e18;
        if (pendingAmountInUno < cryptoBalance) {
            TransferHelper.safeTransfer(currency, _to, pendingAmountInUno);
        } else {
            TransferHelper.safeTransfer(currency, _to, cryptoBalance - 1e20);
        }
        _withdrawImplement(_to);
    }

    function cancelWithrawRequest(address _to) external override onlySSIP {
    ...
    function policyClaim(address _to, uint256 _amount) external override onlySSIP {
        uint256 cryptoBalance = IERC20(currency).balanceOf(address(this));
        require(cryptoBalance > 0 && totalSupply() > 0, "UnoRe: zero uno and lp balance");
        if (cryptoBalance > _amount) {
            TransferHelper.safeTransfer(currency, _to, _amount);
            emit LogPolicyClaim(_to, _amount);
        } else {
            TransferHelper.safeTransfer(currency, _to, cryptoBalance - 1e20);
            emit LogPolicyClaim(_to, cryptoBalance - 1e20);
        }
...
contract RiskPoolERC20 is Context, IRiskPoolERC20 {
    ...
    // mapping(address => mapping(uint256 => uint256)) private _userStakedAmountPerBlock;
    // mapping(address => uint256[]) public userStakedBlocks;
    // mapping(uint256 => uint256) public totalStakedAmountPerBlock;

    uint256 public immutable CLAIMABLE_TIME = 120 seconds;
    ...
    function _withdrawImplement(address _user) internal {
        uint128 lastRequestTime = withdrawRequestPerUser[_user].requestTime;
        require(block.timestamp - uint256(lastRequestTime) >= CLAIMABLE_TIME, "UnoRe: no claimable time");
...
interface IRiskPoolERC20 {
    ...
    event UserDeposit(address indexed _user, address indexed _pool, uint256 _depositAmount, uint256 _pendingAmount);

    event BatchDepositRequest(uint256 _implementationTime, uint256 _depositAmount);

    event BatchWithdrawRequest(uint256 _implementationTime, uint256 _withdrawAmount);

    event RequestDeposit(address indexed _user, address indexed _pool, uint256 _depositAmount);

    event UserWithdraw(address indexed user, address indexed _pool, uint256 _withdrawAmount, uint256 _pendingAmount);

    event RequestWithdraw(address indexed _user, address indexed _pool, uint256 _withdrawAmount);

    event WithdrawClaimByUser(address indexed user, address indexed _pool, uint256 _claimAmount);

    event CancelWithdrawClaim(address indexed user, address indexed _pool, uint256 _claimAmount);
}
