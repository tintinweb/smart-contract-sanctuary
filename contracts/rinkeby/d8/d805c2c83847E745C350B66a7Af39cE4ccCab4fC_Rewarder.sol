...
contract Rewarder is IRewarder {
    ...
    address private immutable SSIP;
    uint256 private rewardBalance;

    constructor(address _rewardToken, address _SSIP) {
        rewardToken = _rewardToken;
        SSIP = _SSIP;
    }

    function initialRewardBalance(uint256 _amount) external override onlyMC {
        uint256 unoBalance = IERC20(rewardToken).balanceOf(address(this));
        require(unoBalance > 0, "UnoRe: zero balance");
    ...
    function onUnoReward(
        ...
        require(rewardBalance > 0, "UnoRe: zero rewarder balance");
