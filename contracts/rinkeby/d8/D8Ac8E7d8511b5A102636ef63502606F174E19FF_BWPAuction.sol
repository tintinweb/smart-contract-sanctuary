...
contract BWPAuction is Context, Ownable, IERC721Receiver {
  ...
  function createAuction(address _nft, uint _itemId, uint _price,  uint _start, uint _end) external {
    require(_start < _end, 'Auction: Period is not valid');
    AuctionInfo memory newAuction = AuctionInfo(msg.sender, _nft, _itemId, _price, address(0), block.timestamp, block.timestamp, _start, _end, AuctionStatus.Normal);
  ...
  function bid(uint _auctionId, uint _price) external {
    ...
    // require(bidCount[msg.sender] == 0, 'Auction: Bidder can only win 1 shop at a time');

    require(IERC20(_bwp).transferFrom(msg.sender, address(this), _price), 'Auction: BWP transfer failed');
  ...
  function withdraw(uint amount) external onlyOwner {
    require(IERC20(_bwp).transfer(msg.sender, amount), 'Auction: BWP transfer failed');
