...
contract LinksDAO is ERC721, ReentrancyGuard, Ownable, EIP712Whitelisting {
  ...
  constructor (
    ...
    MAX_STANDARD_SUPPLY = 7272;
    MAX_PREMIUM_SUPPLY = 1818;
    MAX_STANDARD_RESERVED_SUPPLY = 727;
    MAX_PREMIUM_RESERVED_SUPPLY = 181;
    MAX_SUPPLY = MAX_STANDARD_SUPPLY + MAX_PREMIUM_SUPPLY;

    MAX_STANDARD_WHITELIST_SUPPLY = 3636;
    MAX_PREMIUM_WHITELIST_SUPPLY = 909;
  }
  ...
  function mintPremium() public payable nonReentrant {
    require(saleIsActive, "Sale not active");
    require(totalPremiumSupply() < MAX_PREMIUM_SUPPLY - MAX_PREMIUM_RESERVED_SUPPLY, "Exceeds max supply");
    require(msg.value >= PREMIUM_PRICE , "Insufficient payment");

    if (allowedPremiumMintCount(_msgSender()) > 0) {
      updatePremiumMintCount(_msgSender(), 1);
    } else {
      revert("Minting limit exceeded");
    }
  ...
  function mintPremiumWhitelist(bytes calldata signature) public payable requiresWhitelist(signature) nonReentrant {
    require(whitelistSaleIsActive, "Sale not active");
    require(totalPremiumWhitelistMints() < MAX_PREMIUM_WHITELIST_SUPPLY, "Exceeds whitelist supply");
    require(totalPremiumSupply() < MAX_PREMIUM_SUPPLY - MAX_PREMIUM_RESERVED_SUPPLY, "Exceeds max supply");
    require(msg.value >= PREMIUM_PRICE, "Insufficient payment");
  ...
  function baseTokenURI() public view returns (string memory) {
    // TODO: Need to do this for each token type
