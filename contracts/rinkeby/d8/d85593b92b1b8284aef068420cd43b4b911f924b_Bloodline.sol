...
contract Bloodline is ERC721Tradable, ReentrancyGuard {
    ...
    function getChildName(string memory gender, string memory lastName, uint256 rand) internal view returns (string memory) { 
        ...
        string memory firstName = pluck(descendentID, string(abi.encodePacked("FIRSTNAME", toString(rand))), sourceArray);
    ...
    function getChildProfession(uint maleTokenId, uint femaleTokenId, uint childID, uint rand) internal view returns (string memory) {
        ...
        return pluck(childID, string(abi.encodePacked("PROFESSION", toString(maleTokenId), toString(femaleTokenId))), Profession);
    }
    ...
    function isCompatibleWording(string memory build, string memory specialAppearance) internal pure returns (bool) {
        if (keccak256(bytes(build)) == keccak256(bytes("skinny")) && (keccak256(bytes(specialAppearance)) == keccak256(bytes("gorgeous")) || keccak256(bytes(specialAppearance)) == keccak256(bytes("handsome")))) {
            return false;
        }
        return true;
    }
    ...
    function getAppearance(uint256 tokenId) public view existsToken(tokenId) returns (string memory) {
        ...
        if (greatness == 4) {
            ...
            if (isCompatibleWording(appearance, specialAppearance)) 
                return appearance;
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory output = string(abi.encodePacked(parts[0], parts[1], '</text><text x="10" y="40" class="base">', parts[2], '</text><text x="10" y="60" class="base">', parts[3],
                                 '</text><text x="10" y="80" class="base">', parts[4], '</text><text x="10" y="100" class="base">', parts[5], '</text></svg>'));
        // output = string(abi.encodePacked(output, parts[5], '</text><text x="10" y="120" class="base">', parts[6], '</text><text x="10" y="140" class="base">', parts[7], '</text><text x="10" y="160" class="base">'));
        // output = string(abi.encodePacked(output, parts[8], '</text><text x="10" y="180" class="base">', parts[9], '</text><text x="10" y="200" class="base">', parts[10], '</text></svg>'));
