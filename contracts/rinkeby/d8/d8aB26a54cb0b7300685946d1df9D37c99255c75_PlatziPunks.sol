...
contract PlatziPunksDNA {
    ...
    function pseudoRandomDNA(address _minter)
        public
        view
        returns (uint256)
    {
...
contract PlatziPunks is ERC721, ERC721Enumerable, PlatziPunksDNA {
    ...
    constructor(uint256 _max) ERC721("PlatziPunks","PLPKS") {
        maxSupply = _max;
    }
    ...
    function mint() public {
        ...
        tokenDNA[current] = pseudoRandomDNA(msg.sender);
    ...
    function getImageByDNA(uint256 _dna)
        public
        view
        returns (string memory)
    {
        ...
        returns(string memory) 
    {
        ...
        string memory image = getImageByDNA(dna);

        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{"name":"Platzi Punks #',
                tokenId,
                '", "description":"Platzi Punks son avatars random"',
                '"image":"',
                image,
                '"}'
            )
