...
contract METACLAYS is ERC721Enumerable, Ownable, PaymentSplitter {
    ...
    uint256 public constant PRICE = 15 ether / 10000000;
    uint256 public constant MAX_MINT_PER_ADDRESS = 10;
    uint256 public constant MAX_MINT_NOT_WHITELISTED = 5;
    uint256 public constant PREMINE_AMOUNT = 6;
    string public constant BEFORE_REVEAL_URI = "ipfs://lolo";
    address public constant TEAM = 0xd2EcaFaC2812522D2128B8f9cf33ab23e5bd7f9D;

    bytes32 private immutable WHITELIST_ROOT;

    Counters.Counter private _tokenIdCounter;

    bytes32 private raffleRoot;
    mapping(address => bool) public isWhitelisted;
    mapping(address => uint256) public nbOfMETACLAYSMintedBy;
    ...
    ) ERC721("lolo", "lolo") PaymentSplitter(payees, shares) {
        WHITELIST_ROOT = root;

        for (uint256 i = 0; i < PREMINE_AMOUNT; i++) {
    ...
    /// @dev mints `amount` of METACLAYS and transfers it to msg.sender in exchange of the price (amount in ETH).
    ///
    /// Required `proof` a valid merkle proof that msg.sender is whitelisted
    /// Provide an empty proof for the public sale
    ///
    /// Required `quantity` the amount of desired METACLAYS to buy
    ///
    ...
    function buy(
        ...
        uint256 userMintLimit = MAX_MINT_NOT_WHITELISTED;
        ...
        require(
            nbOfMETACLAYSMintedBy[msg.sender] + quantity <= userMintLimit,
    ...
    function startPrivateSale(bytes32 root) external onlyOwner {
        require(!privateSaleStarted, "Raffle whitelist can't be modified");
        raffleRoot = root;
    ...
    function checkWhitelisting(bytes32[] calldata proof, ListType listType) public returns (bool) {
