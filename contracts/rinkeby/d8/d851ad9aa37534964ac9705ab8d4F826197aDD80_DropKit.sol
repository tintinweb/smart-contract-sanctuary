...
contract DropKit is IDropKit, Initializable, AccessControlUpgradeable {
    ...
    function createDropKitCollection(
        ...
        _setupRole(COLLECTION_ROLE, address(dropKit));
    ...
    function addFees(uint256 amount)
        external
        override
        onlyRole(COLLECTION_ROLE)
    {
        _fees[_msgSender()] = _fees[_msgSender()].add(amount);
    }

    function addFeesClaimed(uint256 amount)
        external
        override
        onlyRole(COLLECTION_ROLE)
    {
    ...
    function commission(uint256 amount)
        external
        pure
        override
        returns (uint256)
    {
        return ((_rate * amount) / 10000);
    }
    ...
    OwnableUpgradeable
{
    ...
    function mint(uint256 numberOfTokens) public payable {
        require(_started == true, "Sale must be active");
        ...
        _dropKit.addFees(_dropKit.commission(_price.mul(numberOfTokens)));
        _mintCount[_msgSender()] = _mintCount[_msgSender()].add(numberOfTokens);
        ...
        onlyOwner
    {
        ...
        require(
            _dropKit.commission(_price.mul(numberOfTokens)) == msg.value,
            "Value sent is not correct"
        );
    ...
    function start() public onlyOwner {
        require(_started == false, "Sale is already started");

        _started = true;
    }

    function pause() public onlyOwner {
        require(_started == true, "Sale is already paused");
    ...
    function withdraw() public {
        ...
        AddressUpgradeable.sendValue(payable(owner()), balance - fees);
...
interface IDropKit {
     ...
     */
    function commission(uint256 amount) external pure returns (uint256);
