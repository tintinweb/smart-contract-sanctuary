...
contract Marc is ERC1155, Ownable {
    ...
    constructor() ERC1155("https://storage.googleapis.com/nasa_gemini/{id}") {}
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i <= totalMintedTokens; i++) {
            uint256 owned = balanceOf(_owner, i);
        ...
        uint256 arrayPosition = 0;
        for (uint256 i = 0; i <= count; i++) {
            uint256 owned = balanceOf(_owner, i);
            if (owned > 0) {
                tokensId[arrayPosition] = i;
                arrayPosition++;
            }
        }
    ...
    function flipClaimState() external onlyOwner {
        require(canMint, "This contract is locked forever");
    ...
    function lockNFTClaiming() external onlyOwner {
        canMint = false;
        claimIsActive = false;
    }

    function claimNFT(uint256 amount, address _to) private {
        uint256 mintIndex = totalMintedTokens;
        uint256 newTotalMintedTokens = mintIndex + amount;
        require(newTotalMintedTokens <= MAX_NFTS, "No NFTs left to be claimed");
    ...
    function claim(
        ...
        require(claimIsActive, "Claim must be active to mint NFT");
        require(block.timestamp <= deadline, "The claiming time has passed.");
        require(!isClaimed(index), "NFT already claimed.");
