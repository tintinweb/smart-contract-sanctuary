...
contract BearsDeluxe is ERC721Enumerable, AccessControlEnumerable, Ownable {
    uint32 public constant MAX_SUPPLY = 6900;
    ...
    string private baseUri = "https://ipfs.io/ipfs/QmVoaBP7pdSusPQVaLkfcshaLvaCxsKR5tE3mhXmnDcrdG/";

    /**
     * @dev in case something happens with a token, metadata can be changed by the owner.
     * only emergency
     */
    function setTokenUri(uint16 _tokenId, string calldata _tokenURI) external {
     ...
     */
    function mint(address _owner, uint16 _tokenId) external {
    ...
    function exists(uint16 _tokenId) external view returns (bool) {
    ...
    function _setTokenURI(uint16 tokenId, string calldata _tokenURI) internal virtual {
