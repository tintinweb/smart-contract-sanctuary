...
contract HypeBeasts is HypeBeastCoreERC721, ReentrancyGuard, VRFConsumerBase {
  ...
  uint256 public constant numberOfTraits = 2;
  uint256 internal constant traitWeighting = 1000;
  uint8[numberOfTraits][maxSupply] public mintedTraits;
  ...
  constructor(
    ...
    bytes32[] memory bodyNames,
    uint16[] memory eyesProbabilities,
    bytes32[] memory eyesNames
  )
    ...
    VRFConsumerBase(VRFCoordinator, LinkToken)
  {
    ...
    traitNames[0] = bodyNames;
    traitCategories[0] = "Body";

    traitProbabilities[1] = eyesProbabilities;
    traitNames[1] = eyesNames;
    traitCategories[1] = "Eyes";
  }

  mapping(uint256 => uint256) public hypeBeastsDna;
  ...
  //   require(hypeBeastsDna[_tokenId] != 0, "No DNA yet");
  //
  ...
  //     }
  //     resultString = strConcat(resultString, "\'");
  //     resultString = string(abi.encodePacked(bytes(resultString), traitCategories[i]));
  //     resultString = strConcat(resultString, "\': \'");
  //     resultString = string(abi.encodePacked(bytes(resultString), traitNames[i][mintedTraits[_tokenId - 1][i]]));
  //     resultString = strConcat(resultString, "\'");
  //   }
  //   resultString = strConcat(resultString, "}");
  //   return resultString;
  ...
  function generateHypeBeast(uint256 _quantity, uint256 _deadline) external payable nonReentrant whenNotPaused {
    require(_tokenIds.current() < maxSupply, "Maximum minted");
    require(_tokenIds.current() + _quantity <= maxSupply, "Not enough left");
  ...
  function fulfillRandomness(bytes32 _requestId, uint256 _randomNumber) internal override {
    ...
    uint256[] memory expandedDnaNumbers = new uint256[](numberOfTraits);
    for (uint256 i = 0; i < numberOfTraits; i++) {
      expandedDnaNumbers[i] = uint256(keccak256(abi.encode(_randomNumber, i)));

      emit ModCalc(vrfRequestIds[_requestId], i, expandedDnaNumbers[i]);
    }

    for(uint8 i = 0; i < numberOfTraits; i++) {
      uint256 currentWeight = traitWeighting;
      uint256 dnaModuloWeight = expandedDnaNumbers[i].mod(currentWeight).add(1);
