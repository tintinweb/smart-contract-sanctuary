    ...
    INFModuleWithRoyalties
{
        ...
        returns (string memory)
    {
        ...
        return
            string(
                abi.encodePacked(
                    'data:application/json;utf8,{"name":"Planets - ',
                    tokenId.toString(),
                    '","license":"CC BY-SA 4.0","description":"',
                    getDescription(),
                    '","created_by":"Fabin Rasheed","twitter":"@astraglade","image":"',
                    abi.encodePacked(getBaseImageURI(), tokenId.toString()),
                    '","seed":"',
                    seed.toString(),
                    abi.encodePacked(
                        '","astragladeSeed":"',
                        ...
                        '],"animation_url":"',
                        _renderTokenURI(seed, astragladeSeed, attributes),
                        '"}'
                    )
                )
            );
    ...
    function getBaseRenderURI() public view returns (string memory) {
        if (bytes(_baseRenderURI).length == 0) {
            return 'ar://ywKXSlkqZV0tjFPD1VynCCogM8uRngKcZ9GxlJow4wY/';
        }
    ...
    function getBaseImageURI() public view returns (string memory) {
        if (bytes(_baseImagesURI).length == 0) {
            return 'https://astraglade.beyondnft.io/images/planets/';
        }
    ...
    function claim(uint256[] memory tokenIds) external payable {
        address operator = msg.sender;
        for (uint256 i; i < tokenIds.length; i++) {
            _claim(operator, tokenIds[i]);
        }
    }
    ...
    function _claim(address operator, uint256 tokenId) internal {
        ...
        require(
            owner_ == operator || astraglade.isApprovedForAll(owner_, operator),
            '!NOT_AUTHORIZED!'
        );

        // mint
        INiftyForge721 planets = INiftyForge721(planetsContract);
