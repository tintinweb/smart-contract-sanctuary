...
contract Collection is ERC721Enumerable {
    ...
    /**
     * @dev Returns the `owner`'s address of this collection.
     */
    ...
    /**
     * @dev Returns the price of nft.
     */
    uint256 public price;

    /**
     * @dev Returns the metadata of collection.
     */
    ...
    /**
     * @dev Initializes the contract by setting `owner`, `maxSupply`, `price`,
     * `_uri` and `ERC721`
     */
    constructor(
        address _owner,
        ...
        returns (string memory)
    {
        ...
        return
            (bytes(uri_).length > 0)
                ? string(abi.encodePacked(uri_, _tokenId.toString(), ".json"))
                : "";
    }

    /**
     * @dev Returns all tokenIds of `_owner`.
     */
    ...
    /**
     * @dev Returns price of tokens a/c to `_noOfTokens`.
     */
    function tokensPrice(uint256 _noOfTokens) public view returns (uint256) {
        return _noOfTokens * price;
    }

    /**
     * @dev Mints new nfts to caller.
     *
     * @param _noOfTokens - no of tokens to be minted.
     *
     * Requirements:
     *
     * - max 5 tokens per txn.
     * - ether should be equal to tokensPrice a/c to `_noOfTokens`.
     *
     * Emits a {NFTMinted} and {FundsTransferred} event.
     */
    function mint(uint256 _noOfTokens) external payable {
        uint256 _value = msg.value;
        require(_noOfTokens <= 5, "max 5 per transaction");
        require(_value == tokensPrice(_noOfTokens), "invalid price");
     ...
     *
     * Requirements:
     * - {totalSupply} must not max out {maxSupply}.
     */
    function _supplyValidator(uint256 _noOfTokens) internal view {
        require(totalSupply() + _noOfTokens <= maxSupply, "max limit reached");
    }
    ...
    function _transferFunds(uint256 _value) internal {
        ...
        Address.sendValue(_owner, _value);
        emit FundsTransferred(_owner, _value);
    }
}
