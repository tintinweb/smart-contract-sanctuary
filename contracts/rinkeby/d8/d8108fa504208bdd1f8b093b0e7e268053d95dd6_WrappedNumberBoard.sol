...
/// @author numberboard73, based on original wrapper by William Entriken
contract WrappedNumberBoard is ERC721Enumerable, Ownable {
    ...
    function wrapUnsafe(uint256 theNum) payable external {
        NUMBERBOARD.acceptBuyNowOffer{value: msg.value}(theNum);
        _safeMint(msg.sender, theNum);
    }
    ...
    function getDummyNumber() internal view returns (uint256) {
        ...
        return ~uint256(2**128 - 1) & uint256(blockhash(0));
    }
}