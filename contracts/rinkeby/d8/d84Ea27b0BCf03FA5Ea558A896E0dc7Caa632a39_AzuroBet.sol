...
contract AzuroBet is
    ...
    IAzuroBet
{
    // temporary set it here. TODO: decide its placement
    ...
    modifier OnlyLP() {
        require(lpAddresses[msg.sender], "NFT:OnlyLP");
    ...
    //
    modifier onlyRouter() {
        require(address(router) == msg.sender, "NFT:Only router");
        _;
    }
    ...
    function setLP(address lpAddress_) external override onlyRouter {
        //lpAddress = lpAddress_;
        lpAddresses[lpAddress_] = true;
    }

    function getlastBetNumber() external view override returns (uint256) {
        return lastBetNumber;
    }

    function inclastBetNumber() external override onlyRouter {
        lastBetNumber++;
    }

    function burn(uint256 id) external override onlyRouter {
        super._burn(id);
    }

    function mint(address account, uint256 id) external override onlyRouter {
