...
contract KCLWealth is Ownable {
    ...
    uint public minimumInvestment = 500;
    uint public maximumInvestment = 100000;
    ...
    constructor(address _token) {
        ...
        minimumInvestment = minimumInvestment * 10 ** tokenDecimals;
        maximumInvestment = maximumInvestment * 10 ** tokenDecimals;
        
    }
    
    function buyKCLPoints(uint _amount) public {
        require(!blacklist[msg.sender], "You are blacklisted");
        require(_amount >= minimumInvestment, "Purchase is below minimum");
        require(_amount <= maximumInvestment, "Purchase is above maximum");
        
        token.transferFrom(msg.sender, address(this), _amount);
        
        uint fullTax = getFullTax();
        
        uint taxed = _amount.div(100).mul(100 - fullTax);
        ...
        if (!excludeTax[msg.sender]) {
            ...
            myKCLPoints[msg.sender] = myKCLPoints[msg.sender].add(_amount);
            totalKCLPoints = totalKCLPoints.add(_amount);
            if(!isInvestor[msg.sender]) {
                isInvestor[msg.sender];
                investors.push(msg.sender);
            }
        ...
        uint adminAmt = _amount.div(100).mul(adminTax);
        token.transfer(owner(), adminAmt);

        
        // Distribute to holders
        
        uint holdersAmt = _amount.div(100).mul(adminTax);
        
        uint proportion = holdersAmt / investors.length;
        for (uint i = 0; i < investors.length; i++) {
            myKCLRewards[investors[i]] = myKCLRewards[investors[i]].add(proportion);
        }
        
        
    }
    
    function buyKCLPoints(uint _amount, address _referrer) public {
        require(msg.sender != _referrer, "You can't refer yourself.");
        require(!blacklist[msg.sender], "You are blacklisted");
        require(_amount >= minimumInvestment, "Purchase is below minimum");
        require(_amount <= maximumInvestment, "Purchase is above maximum");
        ...
        uint holdersAmt = _amount.div(100).mul(distributeTax);
        ...
        if (referrer[up] != address(0)) {
            while (referrer[up] != address(0)) {
                ...
                if (counter < 5) {
                    up = referrer[up];
                    if (counter == 2) {
                        uint directTax = _amount.div(100).mul(up2Tax);
                        myKCLReferrals[up] = myKCLReferrals[up].add(directTax);
                    } else if (counter == 3) {
                        uint directTax = _amount.div(100).mul(up3Tax);
                        myKCLReferrals[up] = myKCLReferrals[up].add(directTax);
                    } else {
                        uint directTax = _amount.div(100).mul(up4Tax);
                        myKCLReferrals[up] = myKCLReferrals[up].add(directTax);                 
                    }                    
                } else {
                    break;
            ...
            uint directTax = _amount.div(100).mul(up1Tax);
            myKCLReferrals[up] = myKCLReferrals[up].add(directTax);
        }
    ...
    function withdrawPoints() public {
        ...
        if ((block.timestamp < lastInvestment[msg.sender] + 60 days)) {
            uint taxed = myKCLPoints[msg.sender].div(100).mul(100-earlyWithdrawalPercent);
            token.transfer(msg.sender, taxed);
        } else {
            token.transfer(msg.sender, myKCLPoints[msg.sender]);
        }
    }
    
    function withdrawReferrals() public {
        require(myKCLReferrals[msg.sender] > 0, "You don't have any referral rewards");
        token.transfer(msg.sender, myKCLReferrals[msg.sender]);
    }
    
    function withdrawRewards() public {
        require(myKCLRewards[msg.sender] > 0, "You don't have any rewards");
        token.transfer(msg.sender, myKCLRewards[msg.sender]);
    }
    ...
    function setMinimumInvestment(uint _newMin) public onlyOwner {
        minimumInvestment = _newMin;
    }
    
    function setMaximumInvestment(uint _newMax) public onlyOwner {
        maximumInvestment = _newMax;
    }
