...
contract GhostTownGoblins is ERC721, Ownable {
	struct Whitelist {
		bool isWhitelisted;
		uint16 numMinted;
	}
	...
	modifier requireContractsSet() {
		require(
			address(gold) != address(0) && address(mines) != address(0),
			"Contracts not set"
		);
		_;
	}

	function setContracts(address _mines, address _gold) external onlyOwner {
		mines = IMines(_mines);
		gold = IGold(_gold);
	}
	...
	function mint(uint256 _amount, bool _stake) public payable whenNotPaused {
		...
		for (uint256 i = 0; i < _amount; i++) {
			...
			if (!_stake || recipient != msg.sender) {
				...
				_safeMint(address(mines), tokenId);
				tokenIds[i] = tokenId;
			}
		}
		if (_stake) {
	...
	function selectRecipient() internal returns (address) {
		...
		if (getSomeRandomNumber(goblinsMinted, 100) >= 10) {
	...
	function setGoblinId(uint16 id, bool special) external onlyOwner {
