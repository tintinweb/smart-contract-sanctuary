...
contract BattleDoge is ERC721, VRFConsumerBase, Ownable {
   ...
   function Attack (uint256 tokenId, uint256 _tokenId , uint256 _number)  public returns (uint256 winner)   {
       require(address(msg.sender) == ownerOf(tokenId) && isAllowed[msg.sender], "You are not a fighter or the owner of this token");
   ...
   function getCurrentFighter(uint256 tokenId) public view returns(address){
    ...
    function toggleAllowed(address addr) external onlyOwner {
     ...
     function toggleFighter(address addy) external  {
        isFighter[addy] = !isFighter[addy];
    }
