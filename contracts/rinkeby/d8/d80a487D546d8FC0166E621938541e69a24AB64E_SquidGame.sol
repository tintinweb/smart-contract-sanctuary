...
contract SquidGame is Initializable {
    ...
    event NftPriceChange(address,uint value);
    //Fund address change events
    event FundAddressChange(address operator,address value);
    //nft address change event
    event NftAddressChange(address operator,address value);
    ...
    function mint() public Reentrant {
        ...
        require(
            endtime_ > block.timestamp || block.timestamp - endtime_ > 1800,'lucky draw has endedended'
    ...
    function setSquidNft(address squidNft_) public onlyOwner {
        require(squidNft_ != address(0),'invalid squidNft');
        _squidNft = ISquidGameNft(squidNft_);
        emit NftAddressChange(msg.sender,squidNft_);
    }
