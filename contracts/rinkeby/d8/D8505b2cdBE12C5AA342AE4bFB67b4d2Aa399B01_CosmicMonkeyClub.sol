...
contract CosmicMonkeyClub is ERC721Enumerable, Ownable {
    ...
    uint public startPrice = 3 ether;
    uint public finalPrice = 0.25 ether;
    uint private _increments = 11; // This decreases by .25 eth each time
    ...
    constructor(address _recipient) ERC721("Cosmic Monkey Club", "COSMIC") {
        transferOwnership(0x6A635cd72604AA2a43E7fe4034a0fBd72B544485);
    ...
    function mintPrice() public view returns (uint) {
        require(block.timestamp >= startTimestamp, "Not open yet");
        if (block.timestamp >= finalTimestamp) {
            return finalPrice;
        }

        uint decrements = (_increments * (block.timestamp - startTimestamp)) / (finalTimestamp - startTimestamp);
        uint price = startPrice - (decrements * finalPrice);
        return price;
    }
    ...
    function whitelistMint(uint amount) external payable {
        // Require nonzero amount
        require(amount > 0, "Can't mint zero");
    ...
    function setStartPrice(uint _startPrice) external onlyOwner {
        startPrice = _startPrice;
    }

    function setFinalPrice(uint _finalPrice) external onlyOwner {
        finalPrice = _finalPrice;
    }

    function setIncrements(uint __increments) external onlyOwner {
        _increments = __increments;
    }
    ...
    function whitelistUsers(address[] memory users, uint[] memory amounts) external onlyOwner {
        require(users.length == amounts.length, "Pass arrays of matching amounts");
        for (uint i = 0; i < users.length; i++) {
            whitelistRemaining[users[i]] = amounts[i];
        }
    }
