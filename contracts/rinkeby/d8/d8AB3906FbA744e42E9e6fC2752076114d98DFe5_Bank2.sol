...
contract Bank2 is Ownable, IERC721Receiver, Pauseable {
   ...
   */
    ...
    function setOldTokenInfo(uint256 _tokenId) public {
        require(msg.sender == swapper, "only swpr");
        (bool thief, , , , , , , , uint8 alphaIndex) = game.tokenTraits(_tokenId);

        if (thief) {
            _addThiefToBankWithTime(oldBank.bank(_tokenId).owner, _tokenId, oldBank.bank(_tokenId).value);
        }
        else {
            uint256 packIndex = oldBank.packIndices(_tokenId);

            _addPoliceToPack(oldBank.pack(alphaIndex, packIndex).owner, _tokenId);
        }
    }
