...
contract ERC20EscrowCID is Ownable {
    ...
    mapping(bytes32 => mapping(address => uint256)) public expirations;
    ...
    function deposit(
        ...
        emit Deposited(_cid, address(token), _amount, "deposited");
    }
    ...
    function withdraw(
        ...
        emit Withdrawn(_cid, address(token), _amount, "completed");
    }

    function refund(bytes32 _cid, IERC20 token) public onlyOwner {
        require(
        ...
        emit Withdrawn(_cid, address(token), payment, "refunded");
    }
}
