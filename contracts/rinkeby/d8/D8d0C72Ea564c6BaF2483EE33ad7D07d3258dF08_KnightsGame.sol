...
contract KnightsGame is ERC721Enumerable, Ownable {
    ...
    address GALACTICAPE_ADDR = 0x21402eF8275405CB1F4dA1419680D3c6fA4a4229;

    uint256 public totalKnights;
    uint256 public totalKnightsByGalapeHolder;
    uint256 public totalCount = 10000;
    uint256 public availableForGalapeHolder = 500;

    uint256 public price = 0.05 ether;
    uint256 public specialPrice = 0.03 ether;
    ...
    constructor() ERC721("Knights Game", "KNIGHTS") {
        baseURI = "https://knightgames-dev.vercel.app/api/knights/";
        unRevealedURI = "https://knightgames-dev.vercel.app/api/knights/0";
    }
    ...
    function isHolder(address _wallet) 
        public 
        view 
        returns (bool holder, uint256 galapeBalance) 
    {
        ERC721Enumerable GALAPE = ERC721Enumerable(GALACTICAPE_ADDR);
        uint256 _galapeBalance = GALAPE.balanceOf(_wallet);

        return (_galapeBalance > 0, _galapeBalance);
    }
    ...
    function mintFree(uint8 numberOfTokens) external payable {
     ...
     function mintHolder(uint8 numberOfTokens) external payable {
        require(started, "Not started!");
        (bool _holder, ) = isHolder(msg.sender);
        require(_holder, "GALAPE NOT FOUND!");
        require(numberOfTokens > 0, "Error input!");
        require(numberOfTokens <= maxBatch, "Must mint fewer in each batch!");
        require(
            numberOfTokens <= availableForGalapeHolder, 
            "Sorry, too late! :("
        );
        require(
          totalKnights + numberOfTokens <= totalCount, 
        ...
        availableForGalapeHolder -= numberOfTokens;
        totalKnightsByGalapeHolder += numberOfTokens;
        for (uint256 i = 0; i < numberOfTokens; i++) {
            _safeMint(msg.sender, 1 + totalKnights++);
        }
    }
