...
/**
 *
 * @dev Implementation of the non fungible token (ERC1155).
 * Operated by Safee marketplace.
 */
contract NFT1155 is ERC1155("") {
   ...
   */
	function mint(address to, uint256 id, string memory uri_) public {
    require(
      balanceOf(to, id) == 0,
      "NFT1155: already minted"
    );
  ...
  /**
   *
   * @dev Burn token using token `id` and `account`.
   */
