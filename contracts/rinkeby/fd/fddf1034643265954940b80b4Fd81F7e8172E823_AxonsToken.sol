...
contract AxonsToken is IAxonsToken, ERC20, Ownable {
    ...
    constructor(address _auctionHouse, address _voting) ERC20("Axon", "AXON")
    {
     ...
     * @dev Batch airdrops
     */
    function batchGenesisMint(address[] memory _users, uint256[] memory _amounts) public onlyOwner whenGenesisMintNotLocked {
        uint size = _users.length;
        
        for (uint256 i=0; i< size; i++){
          address user = _users[i];
          uint256 amount = _amounts[i];
		  _genesisMintAmount(user, amount);
        }
    }
