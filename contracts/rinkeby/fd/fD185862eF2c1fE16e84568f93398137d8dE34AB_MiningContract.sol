...
contract MiningContract is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 miningTokenAmount;  // mining token deposit amount in this pool, for calculating APY
    }
    ...
    constructor(address _mining_token, uint256 _mining_start_block, address _devAddr) public {
        ...
        phase1EndBlockNumber = phase1StartBlockNumber.add(blockCountPerDay.mul(14)); 
        phase2EndBlockNumber = phase1EndBlockNumber.add(blockCountPerDay.mul(14));
        phase3EndBlockNumber = phase2EndBlockNumber.add(blockCountPerDay.mul(90)); 
        phase4EndBlockNumber = phase3EndBlockNumber.add(blockCountPerDay.mul(90)); 
        phase5EndBlockNumber = phase4EndBlockNumber.add(blockCountPerDay.mul(90));  
        phase6EndBlockNumber = phase5EndBlockNumber.add(blockCountPerDay.mul(90));

        uint256 phase1TokenMountPerDay = 248000 * 1e18; // phase1: 248000 KIKA per day
    ...
    function add(string memory _poolName, uint256 _allocPoint, address _lpToken,uint256 _miningTokenRatioNumerator, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        miningToken.mint(devAddr, reward.div(9)); // team rewards
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if(_amount > 0) { // for gas saving
            ...
            if(pool.miningTokenRatioNumerator > 0) {
                ...
                pool.miningTokenAmount = pool.miningTokenAmount.add(miningTokenAmount);
            }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public  nonReentrant {
        ...
        if(pool.miningTokenRatioNumerator > 0) {
            ...
            if(pool.miningTokenAmount >= miningTokenAmount) {
                pool.miningTokenAmount = pool.miningTokenAmount.sub(miningTokenAmount);
            }
        }
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        if(pool.lpTokenAmount >= user.amount) {
        ...
        if(user.miningTokenPledgeAmount > 0){
            ...
            if(pool.miningTokenAmount >= user.miningTokenPledgeAmount) {
                pool.miningTokenAmount = pool.miningTokenAmount.sub(user.miningTokenPledgeAmount);
            }
        }
