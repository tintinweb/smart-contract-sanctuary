...
contract GenesisVault is TokenVault, ContractGuard {
    ...
    uint256 totalMultipliedWBTCTokens = 0;
    ...
    function totalStakedValue() public view returns (uint256) {
        return totalSupply().mul(getStakingTokenPrice());
    }
    ...
    function getShareTokenPrice() public view returns (uint256) {
        uint256 totalLPTokensValue = (totalSupply() + totalMultipliedWBTCTokens).mul(getStakingTokenPrice());
        uint256 shareTokenPrice = totalLPTokensValue.div(weeklyEmissions).div(variableReduction);
    ...
    function beginGenesis() onlyOperator onlyOnce public {
        ...
        address pairTo = IOracle(theOracle).pairFor(router.factory(), peg, share);
