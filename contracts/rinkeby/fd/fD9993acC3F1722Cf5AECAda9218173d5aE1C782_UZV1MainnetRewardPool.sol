// contracts/pools/UZV1MainnetRewardPool.sol
...
/**
 * @title UnizenMainnetRewardPool
 * @author Unizen
 * @notice Reward pool for mainnet tokens with linear vesting
 **/
contract UZV1MainnetRewardPool is UZV1BasePool {
    /* === STATE VARIABLES === */
    ...
    function isMainnet() public pure override returns (bool) {
        return true;
    }

    function _safeClaim(address _user, uint256 _amount) internal override returns (uint256) {
        uint256 _realAmount = UZV1BasePool._safeClaim(_user, _amount);
