...
contract MoPAr is Ownable, AccessControlEnumerable, ERC721Enumerable {
    ...
    function getCollection(uint256 collectionId) external view onlyRole(MANAGER_ROLE) returns (bool exists, string memory name, string memory artist, bytes32[] memory signatures, uint256[] memory tokenIds, bool paused, uint128 max, uint128 circulating) {
        ...
        exists = bytes(_collections[collectionId].name).length != 0 && bytes(_collections[collectionId].artist).length > 0 && signatures.length > 0;
    }
