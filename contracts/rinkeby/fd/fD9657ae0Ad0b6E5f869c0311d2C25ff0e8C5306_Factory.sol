...
contract Factory is Context, Ownable, IFactory {
  function setupTickets(uint256[] calldata ids_, uint256[] calldata fees_) external override onlyOwner {
    ...
    for(uint256 i = 0; i < ids_.length; i++) {
      ...
      token.grantRole(token.MANAGER_ROLE(), converter);
  ...
  function setConverter(address converter_) external override onlyOwner {
...
contract TicketToken is ITicketToken, ERC20, AccessControl {
  bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE");
  ...
  modifier onlyManager() {
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()) || hasRole(MANAGER_ROLE, _msgSender()), "TicketToken: caller is not a manager");
    _;
  }
