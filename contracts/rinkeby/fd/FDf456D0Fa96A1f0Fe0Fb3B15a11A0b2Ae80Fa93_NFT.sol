...
contract NFT is ERC721, Ownable {
    ...
    constructor(uint256 airdrop_available, string memory name, string memory symbol, uint256 price, string memory provenance_hash, uint256 max_supply, uint16 networkid) ERC721(name, symbol) {
        ...
        if (_networkid == 1) {
            _baseURI = string(abi.encodePacked("https://ipfs.io/ipfs/", _provenance_hash, "/"));
        } else {
            _baseURI = string(abi.encodePacked("https://ipfs.io/ipfs/", _provenance_hash, "/"));
        }
    ...
    function purchase(uint numberOfTokens) public payable {
        require(totalSupply() >= 3, "First 50 have not been airdropped yet");
        require(numberOfTokens <= 20, "Can only purchase a maximum of 20");
        require(_price.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
        mintToSender(numberOfTokens);
    }

    
    function airdrop(address addresstm) public onlyOwner {
        require(_airdrop_available > 0, "No airdrop tokens available or the airdrop is over");
        _airdrop_available -= 1;
        mintToAddress(1, addresstm);
    }
