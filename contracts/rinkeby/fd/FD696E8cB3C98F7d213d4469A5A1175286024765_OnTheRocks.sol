...
contract OnTheRocks {
  ...
  function safeBuy(uint256 id) public payable {
        ...
        returns (address[] memory , bool[] memory , uint[] memory , uint[] memory )
    {
        ...
        (address owner, bool currentlyForSale, uint price, uint timesSold) = allRocks.rocks(0);
        
        rockOwners[0] = owner;
        rocksCurrentlyForSale[0] = currentlyForSale;
        rocksPrice[0] = price;
        rocksTimesSold[0] = timesSold;
