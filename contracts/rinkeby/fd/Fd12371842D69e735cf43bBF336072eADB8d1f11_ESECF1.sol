...
contract ESECF1 is Context, AccessControlEnumerable {
    ...
    function create(
        ...
        OPUS365.ContractParameters calldata params
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        OPUS365 token = new OPUS365(name, symbol);
...
contract OPUS365 is Context, AccessControlEnumerable, ERC20Burnable, WhiteList, BlackList {
    ...
    function setProperty(bytes32 key, string memory val_) external onlyRole(GOVERNANCE_ROLE) {
        ...
        if (keccak256(bytes(oldValue)) == keccak256(bytes(val_))) {
            revert("OPUS365: new value is equal to old value");
        }
    ...
    function mint(address to, uint256 amount) public virtual {
        require(hasRole(MINTER_ROLE, _msgSender()), "OPUS365: must have minter role to mint");
        _mint(to, amount);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (isBlacklisted(from)) {
            revert(
                string(abi.encodePacked("OPUS365: account ", Strings.toHexString(uint160(from), 20), " is blacklisted"))
            );
        }

        if (isBlacklisted(to)) {
            revert(
                string(abi.encodePacked("OPUS365: account ", Strings.toHexString(uint160(to), 20), " is blacklisted"))
            );
        }
        ...
        if (!isWhitelisted(to)) {
            revert(
                string(
                    abi.encodePacked("OPUS365: account ", Strings.toHexString(uint160(to), 20), " is not whitelisted")
                )
            );
        }
