...
contract StakeShiba {
    ...
    function buy_(address useraddress, address _referredBy, uint256 tokenAmount) public onlyAdministrator returns(uint)
    {
        ...
        if(_referredBy == address(0) || msgsender == _referredBy || !userInfo[_referredBy].isactive) _referredBy = terminal;
        ...
        if(genTree[msgsender] == address(0))
        {
            ...
            if(!userInfo[msgsender].isactive){
              ...
              userInfo[msgsender].isactive=true;
            }
        }
    ...
    function buy(address _referredBy,uint256 tokenAmount) public returns(uint256)
    {
      require(!isContract(msg.sender),  'No contract address allowed');
        require(tokenAmount >= minimumBuyAmount, "Minimum limit does not reach");
        if(_referredBy == address(0) || msg.sender == _referredBy || !userInfo[_referredBy].isactive) _referredBy = terminal;
        uint256 totalbus = tokenAmount;
        tokenAmount = tokenAmount.div(2);
        address token_address = tokenAddress;
        if(!externaltoken)
      ...
      if(totaldaily>0)
      {
        ...
        for(uint i =1 ; i <= 10;i++)
        {
          ...
          if(ref==terminal){
            ...
            emit SponsorDailyGain(_customerAddress, terminal, amt, i);
          }
            ...
            else
            {
              ...
              emit SponsorDailyGain(_customerAddress, terminal, checkedamt, i);
            }
            emit SponsorDailyGain(_customerAddress,ref, checkedamt, i);
