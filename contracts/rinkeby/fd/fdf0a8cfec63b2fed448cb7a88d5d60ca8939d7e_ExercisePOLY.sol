...
contract ExercisePOLY {
    ...
    function getCirculatingSupply() public view returns ( uint ) {
        ...
        uint i = 0;
        while ( i < nonCirculatingAddresses.length ) {
            supply.sub( IERC20( OHM ).balanceOf( nonCirculatingAddresses[i] ) );
            i = i + 1;
        }
        return ( supply );
    }
    ...
    function getPOLYAbleToClaim( address _vester ) public view returns (uint) {
        return ( getCirculatingSupply().mul( percentCanVest[ _vester ] ).mul( 1e9 ).div( 10000 ) ).sub( amountClaimed[ _vester ] );
    }

    function addNonCirculatingAddress( address _address ) public returns ( bool ) {
        require( msg.sender == owner, "Sender is not owner" );
        nonCirculatingAddresses.push( _address );
        return true;
    }
}