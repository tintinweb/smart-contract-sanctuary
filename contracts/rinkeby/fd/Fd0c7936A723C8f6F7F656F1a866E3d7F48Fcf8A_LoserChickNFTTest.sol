...
contract LoserChickNFTTest is OwnableContract, ERC721("LaborChick", "LaborChick"){
    ...
    function tokenOfOwnerPage(address owner, uint256 pageNumber, uint256 pageSize) external view returns (uint256 total, uint256[12] memory chicks){
        total = balanceOf(owner);

        uint256 start = pageNumber * pageSize;

        if (total > start) {
            uint256 countOnPage = pageSize;
