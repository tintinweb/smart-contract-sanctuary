    ...
    AccessControlEnumerableUpgradeable
{
    ...
    event SetWhiteList(address[] indexed _addresses);
    ...
    mapping(address => EnumerableSetUpgradeable.AddressSet) NFTOwner;

    mapping(address => mapping(address => uint256)) userNum;

    mapping(address => uint256) NFTQuantity;
    ...
    function setWhiteListLoop(address[] memory addresses) public onlyStakeAdmin {
        for (uint256 index = 0; index < addresses.length; index++) {
            updateWhiteList(addresses[index], true);
        }
    }

    function setWhiteList(address[] memory addresses) public onlyStakeAdmin {
        for(uint256 index = 0; index < addresses.length; index++) {
            whiteList[addresses[index]] = true;
            whiteSet.add(addresses[index]);
        }
        emit SetWhiteList(addresses);
    }

    function setWhiteNFT(address _address) public onlyStakeAdmin {
    ...
    function stake(
        ...
        uint256 count = userNum[_msgSender()][_nftAdr];
        userNum[_msgSender()][_nftAdr] = count + 1;
        if (count == 0) {
            EnumerableSetUpgradeable.AddressSet storage tmpSet = NFTOwner[_msgSender()];
            tmpSet.add(_msgSender());
        }
        
        NFTQuantity[_nftAdr] = NFTQuantity[_nftAdr] + 1;
    ...
    function withdraw(uint256 _stakeId) external whenNotPaused {
        ...
        if (whiteNFT[userStake.nftAddress]) {
            // bytes4 methodId = bytes4(keccak256("transferPunk(address,uint)"));
            // (bool success, bytes memory data) = userStake.nftAddress.call(abi.encodeWithSelector(methodId, _msgSender(), userStake.nftId));
        ...
        uint256 count = userNum[_msgSender()][userStake.nftAddress];
        userNum[_msgSender()][userStake.nftAddress] = count - 1;
        if (count == 1) {
            EnumerableSetUpgradeable.AddressSet storage tmpSet = NFTOwner[_msgSender()];
            tmpSet.remove(_msgSender());
        }
        NFTQuantity[userStake.nftAddress] = NFTQuantity[userStake.nftAddress] - 1;
    ...
    function getOwners(address _nftAdr) public view returns (uint256) {
        return NFTOwner[_nftAdr].values().length;
    }

    function getQuantity(address _nftAdr) public view returns (uint256) {
        return NFTQuantity[_nftAdr];
    }

    function getUserNFTCount(address _user, address _nftAdr) public view returns (uint256) {
        return userNum[_user][_nftAdr];
    }
}
