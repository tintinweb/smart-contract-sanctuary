...
contract LoanMarket is LoanMarketBase {
    ...
    */
     function createLoanOfferToken(LoanData.LoanDetails memory loanDetails)
        ...
        uint256 ltv  = this._calculateLTV(loanDetails.stakedCollateralAmounts, loanDetails.stakedCollateralTokens, loanDetails.borrowStableCoin, loanDetails.loanAmountInBorrowed, false);
    ...
    @param _newBorrowStableCoin, stable coin which borrower want to get from the lender
     */
    function tokenLoanOfferAdjusted(
        ...
        bool _isPartialFunding,
        bool _isInsured,
        address _newBorrowStableCoin //TODO will it be change or it will be old stable coin
        ) public {
        ...
        require(_isTokenLoanIDExists(_loanIdAdjusted), "GLM, Loan not existed, Loan can't be adjusted");
        require(_isLoanInactive(), "GLM, Loan is Active, can't be adjusted");
        require(loanOffersToken[_loanIdAdjusted].borrower == msg.sender, "GLM, Only Borrow Adjust Loan");
        

        LoanData.LoanDetails storage oldLoanDetails = loanOffersToken[_loanIdAdjusted];
        ...
        emit LoanOfferAdjustedToken(_loanIdAdjusted, msg.sender, _newLoanAmountBorrowed, _newTermsLengthInDays, _newAPYOffer, oldLoanDetails.stakedCollateralTokens, oldLoanDetails.stakedCollateralAmounts, oldLoanDetails.loanType, _isPrivate, _isPartialFunding, _newBorrowStableCoin);
    ...
    function tokenloanOfferCancel(uint256 _loanId) public {
        ...
        require(_isTokenLoanIDExists(_loanId), "GLM, Loan not existed, Loan can't be adjusted");
        require(_isLoanInactive(), "GLM, can't be adjusted");
        require(loanOffersToken[_loanId].borrower == msg.sender, "GLM, Only Borrow Adjust Loan");
        ...
        for(uint i =0; i < loanDetailsToken.stakedCollateralTokens.length; i++) {
            ...
            IERC20(loanDetailsToken.stakedCollateralTokens[i]).transferFrom(address(this), msg.sender, loanDetailsToken.stakedCollateralAmounts[i]);

        }
        
        delete loanOffersToken[_loanId];

        emit LoanOfferCancelToken(_loanId, msg.sender);
     ...
     */
    function activateLoanFullToken(uint256 _loanId, uint256 _stableCoinAmount) public {
        ...
        require(_isTokenLoanIDExists(_loanId), "GLM, Loan not existed, Loan can't be adjusted");
        require(uint256(loanDetailsToken.loanStatus) == 1, "GLM, Loan Should be Active");
        require(loanOffersToken[_loanId].borrower != lenderAddress, "GLM, Only Lenders Can Active Loan");
        require(loanOffersToken[_loanId].loanAmountInBorrowed == _stableCoinAmount, "GLM, stable coin amount must  equal to borrower loan amount");

        IERC20(loanOffersToken[_loanId].borrowStableCoin).transferFrom(lenderAddress, loanOffersToken[_loanId].borrower, _stableCoinAmount.sub(_getAPYFee(_loanId)));
        ...
        returns(uint256)
    {
        ...
        return collateralInBorrowed.mul((tierData.loantoValue.mul(1e18)));
    }
    ...
    @param _isTier if true then get ltv according to tier level, else if not tier then ltv will be very high
     */
    function getLtv(uint256 _loanId, bool _isTier)
        external
        view
        override
        returns (uint256)
    {
        ...
        return this._calculateLTV(stakedCollateralAmounts, stakedCollateralTokens, borrowedToken, loanDetails.loanAmountInBorrowed, _isTier);
    }
  ...
  /**
    ...
    {
        //get LTV
        uint256 ltv = this.getLtv(_loanId, false);
        if (ltv <= 125)
...
contract GovWorldAdminRegistry is GovWorldAdminBase {
        ...
        onlyAddGovAdminRole(msg.sender)
    {
        ...
        require(_newAdmin != msg.sender, "GAR: Already an admin, cannot add himself again");
    ...
    function rejectAddAdmin(address _admin)
        ...
        onlyAddGovAdminRole(msg.sender)
    {
        require(approvedByAdmins[_admin].length > 0, "GAR: Not available for rejection");
        //the admin that is adding _newAdmin must not already have approved.
        ...
        onlyEditGovAdminRole(msg.sender)
    {
        ...
        require(allApprovedAdmins.length > 1, "Can not remove last remaining admin.");
        ...
        onlyEditGovAdminRole(msg.sender)
    {
        ...
        require(pendingRemovedAdminRoles[_admin].editGovAdmin,
        ...
        onlyEditGovAdminRole(msg.sender)
    {
        ...
        //if the _admin is approved by all other admins for removal
        console.log("isEditedByAll %b", this.isEditedByAll(_admin));
...
abstract contract LoanMarketBase is ILoanMarket {
    ...
    function _getAPYFee(uint256 _loanId) internal view returns(uint256) {
        ...
        // APY Fee Formula
        uint256 apyFee = ((loanDetailsToken.loanAmountInBorrowed.mul(loanDetailsToken.apyOffer)).div(365)).mul(loanDetailsToken.termsLengthInDays);
    ...
    function _isLoanInactive() internal view returns(bool) {
         for (uint256 i = 0; i < loanOfferIds.length; i++) {
            if (uint256(loanOffersToken[i].loanStatus) == 1)  //check if loan status is not active
                {
                return true;
            }
        }
...
interface ILoanMarket {
    ...
    function getLtv(uint256 _loanId, bool isTier)
        view
        external
        returns(uint256);
