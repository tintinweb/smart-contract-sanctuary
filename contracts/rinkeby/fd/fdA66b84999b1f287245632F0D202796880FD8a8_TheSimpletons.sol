...
contract TheSimpletons is Context, ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  mapping (address => ClaimStatus) private whitelist;
  mapping(string => bool) private usedNonces;

  string private baseTokenURI;
  string private tempTokenURI;
  ...
  constructor(string memory name, string memory symbol, string memory _tempTokenURI) ERC721(name, symbol) {
    tempTokenURI = _tempTokenURI;
  }
  ...
  function mint(address wallet, uint count, bytes32 hash, bytes memory signature, string memory _nonce) external payable nonReentrant() {
    ...
    require(isSaleActive == true, "Sale must be active");
    require(tx.origin == msg.sender, "Minting unauthorized");
    require(matchAddresSigner(hash, signature), "Direct mint not allowed");
    ...
    require(count > 0 && count <= MAX_PER_TX, "Must mint from 1 to 10 simpleton");
  ...
  function mintPreSale(uint count, bytes32 hash, bytes memory signature, string memory _nonce) external payable nonReentrant() {
    require(isSaleActive == false, "Sale must be inactive");
    require(isPreSaleActive == true, "Presale must be active");
    require(whitelist[msg.sender] == ClaimStatus.Unclaimed, "You are not able to claim again");
    require(whitelist[msg.sender] != ClaimStatus.Claimed, "You already claimed your presale tokens");
    require(tx.origin == msg.sender, "Minting unauthorized");
    require(matchAddresSigner(hash, signature), "Direct mint not allowed");
    require(!usedNonces[_nonce], "Hash used");
    require(hashTransaction(msg.sender, count, _nonce) == hash, "Hash failed");
    require(count <= MAX_PER_TX, "You can only mint 10 max");
    require(balanceOf(msg.sender) + count <= MAX_PER_TX, "Balance exceeds max count");
    require(msg.value >= count * mintPrice, "You didn't send the right amount of eth");

    _mintMultiple(msg.sender, count);
    usedNonces[_nonce] = true;

    if(balanceOf(msg.sender) == MAX_PER_TX) {
      whitelist[msg.sender] = ClaimStatus.Claimed;
    }
  }

  function mintPromo(address wallet, uint count) external onlyOwner {
    require(promoTokens > 0 && count <= promoTokens, "No more promo tokens");

    promoTokens = promoTokens - count;
    _mintMultiple(wallet, count);
  }
  ...
  function tokenURI(uint tokenId) public view virtual override returns (string memory) {
    ...
    return isRevealed ? string(abi.encodePacked(baseTokenURI, uintToBytes(tokenId))) : tempTokenURI;
  }

  function setTempBaseURI(string memory _baseUri) external onlyOwner {
    tempTokenURI = _baseUri;
  }
