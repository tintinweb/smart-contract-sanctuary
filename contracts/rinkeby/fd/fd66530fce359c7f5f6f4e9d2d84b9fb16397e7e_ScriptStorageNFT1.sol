...
contract ScriptStorageNFT1 is ERC721, Pausable, AccessControl, ERC721Burnable {
    using Counters for Counters.Counter;

/*     string internal constant scriptHEAD = '<html><script>';
...
function startMint(address to) public {
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        bytes32 hash = keccak256(abi.encodePacked(keyHash, tokenId, msg.sender, block.number));
...
//    }
    ...
    function formatScript (uint256 tokenId) internal view returns (string memory){
        ...
        string memory hashyString = string(abi.encodePacked("let {'hash':'",hashboy,"'}"));
        string memory baseURL = "data:text/html;base64,";
        string memory uriScript = string(abi.encodePacked(scriptHEAD,hashyString,storedScripts,storedSVG,scriptTAIL));
    ...
    function bytes32ToString(uint256 tokenId) internal view returns (string memory) {
        uint8 i = 0;
        while(i < 32 && tokenIdToHash[tokenId][i] != 0) {
            i++;
        }
        bytes memory bytesArray = new bytes(i);
        for (i = 0; i < 32 && tokenIdToHash[tokenId][i] != 0; i++) {
            bytesArray[i] = tokenIdToHash[tokenId][i];
        }
