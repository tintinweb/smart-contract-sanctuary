...
import "./Registry.sol";
import "./BeforeTokenTransferHook.sol";

contract Token is ERC20 {
    address private _settingsController;
    BeforeTokenTransferHook private _beforeTokenTransferHook;

    modifier onlySettingsController() {
        require(_settingsController == _msgSender());
        _;
    }

    /**
     * @dev Sets the values for {name} and {symbol}, initializes {decimals} with
     * a default value of 3, sets the {hook_} to execute checks before transfer, mint and burn operations.
     * The hook contract can be replaced with a new version by {settingsController_} account.
     */
    constructor(
        string memory name_,
        string memory symbol_,
        BeforeTokenTransferHook hook_,
        address settingsController_
    ) ERC20(name_, symbol_) {
        super._setupDecimals(9);
        _settingsController = settingsController_;
        _beforeTokenTransferHook = hook_;
    }
    ...
    /** @dev Creates `amount` tokens and assigns them to sender's address, increasing
     * the total supply.
     *
    ...
    /**
     * @dev Destroys `amount` tokens from sender's account, reducing the
     * total supply.
     *
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override {
        _beforeTokenTransferHook.execute(from, to, amount);
    }
}
