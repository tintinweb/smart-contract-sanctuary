...
contract Pool is Ownable, ReentrancyGuard, Initializable, IPool {
    ...
    function globalValue() public view returns (uint256, uint256) {
        ...
        uint256 _shareReserveValue = (_shareReserveBalance * _share_price) / PRICE_PRECISION; // Todo: unclaimed_pool_share ?
        return (_collateralReserveValue + _shareReserveValue, _globalDollarValue);
    }

    function getTax(uint256 _tokenAmount) internal returns(uint256) {
        (uint256 _globalReserveValue, uint256 _globalDollarValue) = globalValue();
        uint256 gap = _globalReserveValue / _globalDollarValue;
        uint256 tax = 0;
        if(gap <= 80) {
            tax = 25;
        } else if(gap <= 90) {
