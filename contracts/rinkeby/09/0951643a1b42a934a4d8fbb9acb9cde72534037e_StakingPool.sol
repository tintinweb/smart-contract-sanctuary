...
contract StakingPool is ReentrancyGuard, Pausable, Ownable {
    ...
    constructor(address _apeToken) public {
    ...
    function claimBaby() external nonReentrant whenNotPaused returns(uint256) {
        ...
        _breeding();
        if (birth[female] == 0) {
            apeToken.safeTransferFrom(address(this), msg.sender, babyOf[female][0], 1, "0x0"); // TODO address(this) - who own?
            emit Claimed(msg.sender, babyOf[female][0]);
        } else {
            apeToken.safeTransferFrom(address(this), msg.sender, babyOf[female][1], 1, "0x0");
    ...
    function _breeding() internal {
        if (stakedFemales[msg.sender] == 1 && stakedMales[msg.sender] == 1) {
            breedingEnd[msg.sender] = now + 1 minutes;
        } 
    }
