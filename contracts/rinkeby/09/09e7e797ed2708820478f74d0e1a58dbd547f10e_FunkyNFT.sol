...
contract FunkyNFT is ERC721URIStorage, Ownable{
 
    event MintNFT (address indexed minter, uint256 startWith, uint256 times);
    
    
    uint256 public totalNFT;
    uint256 public totalCount = 7500; //total
    uint256 public maxBatch = 20; //max mint per Tx
    ...
    function mintNFT(uint256 _times) payable public mintEnabled {
        ...
        require(totalNFT + _times <= totalCount, "too much");
        require(msg.value == _times * price, "value error");
        payable(owner()).transfer(msg.value);
        emit MintNFT(_msgSender(), totalNFT+1, _times);
        for(uint256 i=0; i< _times; i++){
            _mint(_msgSender(), 1 + totalNFT++);
        }
    } 
    
    function adminMintGiveaways(address _addr) public onlyOwner {
        require(totalNFT + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit MintNFT(_addr, totalNFT+1, 1);
        _mint(_addr, 1 + totalNFT++);
    }
