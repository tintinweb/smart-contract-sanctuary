    ...
    DSMath
{
    ...
    event MinedCoinage(uint256 curTime, uint256 miningInterval, uint256 miningAmount, uint256 prevTotalSupply , uint256 afterTotalSupply);
    ...
    function miningCoinage() public lock {
        ...
        if (block.timestamp > (coinageLastMintBlockTimetamp + miningIntervalSeconds) ) {
            ...
            if (miningAmount > 0 && prevTotalSupply > 0) {
                ...
                emit MinedCoinage(block.timestamp, miningInterval, miningAmount, prevTotalSupply, afterTotalSupply);
            }
        }
    }

    function burnCoinage(address user, uint256 tokenId, uint256 amount) internal {
        require(amount > 0, "StakeUniswapV3: amount is zero");
        uint256 _amount = amount * (10**9);
        require(
            _amount <= IAutoRefactorCoinageWithTokenId(coinage).balanceOf(tokenId),
            "StakeUniswapV3: coinages's user's balanceOf is insufficent"
        );

        uint256 prevTotalSupply = IAutoRefactorCoinageWithTokenId(coinage).totalSupply();
        IAutoRefactorCoinageWithTokenId(coinage).burn(user, tokenId, _amount);

        uint256 afterTotalSupply = IAutoRefactorCoinageWithTokenId(coinage).totalSupply();
        ...
        emit BurnedCoinage(block.timestamp, tokenId, amount, prevTotalSupply, afterTotalSupply);
    }
        ...
        )
    {
        if(startTime > 0) {
            ...
            if(_depositTokens.liquidity > 0){
                ...
                if(liquidity > 0 ) (balanceOfTokenId, minableAmount) = _canMiningAmountTokenId(tokenId, liquidity);
        ...
        public override
    {
        ...
        burnCoinage(msg.sender, tokenId, minableAmount);
    ...
    function withdraw(uint256 tokenId) external override {
        ...
        burnCoinage(msg.sender, tokenId, balanceOfTokenId);
    ...
    function canMiningAmountTokenId(uint256 tokenId)
        public
        view
        returns (uint256 balanceCoinageOfTokenId, uint256 minableAmount)
    {
        LibUniswapV3Stake.StakeLiquidity storage _depositTokens = depositTokens[tokenId];
        return _canMiningAmountTokenId(tokenId, _depositTokens.liquidity);
    }

    function _canMiningAmountTokenId(uint256 tokenId, uint256 depositAmount)
        internal
        view
        returns (uint256 balanceOfTokenId, uint256 minableAmount)
    {
        uint256 balanceOfTokenIdRay = IAutoRefactorCoinageWithTokenId(coinage).balanceOf(tokenId);
        if (balanceOfTokenIdRay > 0 ) {
            balanceOfTokenId = balanceOfTokenIdRay / (10**9);
            if (balanceOfTokenId > depositAmount){
                minableAmount = balanceOfTokenId - depositAmount;
            }
        }
    }
