...
contract BoringBananasCo is ERC721, Ownable {
    ...
    uint256 public constant bananaPrice = 30000000000000000; // 0.030 ETH

    uint public constant maxBananaPurchase = 20;

    uint256 public constant MAX_BANANAS = 500;

    bool public saleIsActive = false;
    
    mapping(uint => string) public bananaNames;
    
    // Reserve 125 Canvases for team - Giveaways/Prizes etc
    uint public bananaReserve = 3;
    
    event bananaNameChange(address _by, uint _tokenId, string _name);
    
    event licenseisLocked(string _licenseText);

    constructor() ERC721("Canvas by Felt Zine", "CNVS") { }
    ...
    function tokenLicense(uint _id) public view returns(string memory) {
        require(_id < totalSupply(), "CHOOSE A CANVAS WITHIN RANGE");
        return LICENSE_TEXT;
    }
    ...
    function mintBoringBanana(uint numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint Canvas");
        require(numberOfTokens > 0 && numberOfTokens <= maxBananaPurchase, "Can only mint 20 tokens at a time");
        require(totalSupply().add(numberOfTokens) <= MAX_BANANAS, "Purchase would exceed max supply of Canvases");
    ...
    function changeBananaName(uint _tokenId, string memory _name) public {
        require(ownerOf(_tokenId) == msg.sender, "Hey, your wallet doesn't own this Canvas!");
    ...
    function viewBananaName(uint _tokenId) public view returns( string memory ){
        require( _tokenId < totalSupply(), "Choose a Canvas within range" );
