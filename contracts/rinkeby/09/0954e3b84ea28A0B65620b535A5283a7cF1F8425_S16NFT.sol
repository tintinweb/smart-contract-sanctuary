...
contract S16NFT is ERC1155, AccessControl, ERC165Storage, ERC2981ContractWideRoyalties {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    using Strings for uint256;
    
    string _baseTokenURI = "https://s16token/nfts/";
    ...
    uint256 internal tokenCap = 5; //TODO change when deployment // should be 160
    uint256 internal editionCap = 4; //TODO change when deployment // should be 25
    ...
    function updateTokenCap(uint256 _tokenCap) external {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "S16NFT: Caller is not a admin"
        );
        require(_tokenCap > 0, "S16NFT: value error");
        tokenCap = _tokenCap;
    }

    function updateEditionCap(uint256 _editionCap) external {
       require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "S16NFT: Caller is not a admin"
        );
        require(_editionCap > 0, "S16NFT: value error");
        editionCap = _editionCap;
    }
    ...
    function setNewBaseURI(string memory baseURI) public {
        ...
        _setURI(baseURI); //_setURI is been set during  testing
    }
    ...
    function mintEditions(
        ...
        require(tokenIds.length == quantity.length, "S16NFT: tokenIds length must be equal to quantity");
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(
                tokenIds[i] <= tokenCap,
                "S16NFT: Cap reached, maximum 256 NFT mints possible"
            );
            require(quantity[i] > 0, "S16NFT: quantity must be greater than 0");
            require(quantity[i] + mintedEditionsToken[tokenIds[i]] <= editionCap, "S16NFT: quantity exceed limit");
    ...
    function updateIdsEdition(uint256[] memory tokenIds,uint256[] memory quantity) external{
          require(hasRole(MINTER_ROLE, msg.sender), "S16NFT: Caller is not a minter");
            _tokenIds.increment();
           for (uint256 i = 0; i < tokenIds.length; i++) {
            mintedEditionsToken[tokenIds[i]] += quantity[i];
           }
