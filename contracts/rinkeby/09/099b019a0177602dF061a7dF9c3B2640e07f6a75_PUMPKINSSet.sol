...
contract PUMPKINSSet is ERC721Creator {
    string[] pumpkins;
    string[] mouths;
    string[] eyes;
    string[] noses;
    ...
    function addSunglass(string calldata sunglass) external {
        sunglasses.push(sunglass);
    }
    
    function addPumpkin(string calldata pumpkin) external {
        pumpkins.push(pumpkin);
    }
    
    function addMouth(string calldata mouth) external {
        mouths.push(mouth);
    }
    
    function addEye(string calldata eye) external {
        eyes.push(eye);
    }
    ...
    function addAccessory(string calldata accesory) external {
        accessories.push(accesory);
    }
    
    function mint(address to) external returns (uint256) {
        _updateEntropy();
        seeds.push(entropyBase);
        return mintBase(to);
    }
        ...
        returns (string memory)
    {
        ...
        string memory img = generateSvg(seed);
        ...
        return formatTokenURI(imgURI);
    }
    
   function generateSvg(bytes32 seed) public view returns (string memory) {
        string memory svgOpen = "<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 -0.5 88 88' shape-rendering='crispEdges'>";
        string memory pumpkin = pumpkins[uint256(seed) % pumpkins.length];
        string memory sunglass = sunglasses[uint256(seed) % sunglasses.length];
        string memory nose = noses[uint256(seed) % noses.length];
        string memory eye = eyes[uint256(seed) % eyes.length];
        string memory hair = hairs[uint256(seed) % hairs.length];
        string memory accessory = accessories[uint256(seed) % accessories.length];
        string memory mouth = mouths[uint256(seed) % mouths.length];
        
        string memory svgClose = "</svg>";
        return string(abi.encodePacked(svgOpen, pumpkin, mouth, eye, nose, sunglass, hair, accessory, svgClose));
    }
 ...
 */
contract ERC721Creator is ERC721, ERC721CreatorCore {
     ...
     */
    function registerExtension(address extension, string calldata baseURI) external override nonBlacklistRequired(extension) {
     ...
     * @dev See {ICreatorCore-registerExtension}.
     */
    function registerExtension(address extension, string calldata baseURI, bool baseURIIdentical) external override nonBlacklistRequired(extension) {
     ...
     */
    function setBaseTokenURI(string calldata uri) external override {
     ...
     */
    function setTokenURIPrefix(string calldata prefix) external override {
     ...
     * @dev See {ICreatorCore-setTokenURI}.
     */
    function setTokenURI(uint256[] memory tokenIds, string[] calldata uris) external override {
        require(tokenIds.length == uris.length, "Invalid input");
        for (uint i = 0; i < tokenIds.length; i++) {
     ...
     */
    function setMintPermissions(address extension, address permissions) external override {
     ...
     */
    function mintBase(address to) public virtual override nonReentrant returns(uint256) {
        return _mintBase(to, "");
    }
     ...
     * @dev See {IERC721CreatorCore-mintBase}.
     */
    function mintBase(address to, string memory uri) public virtual override nonReentrant returns(uint256) {
