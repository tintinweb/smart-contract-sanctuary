...
contract EthereumPro is Context, IERC20, Ownable {
    ...
    function addBotToBlacklist(address[] memory _botAdr) public onlyOwner {
        for (uint256 j = 0; j < _botAdr.length; j++) {
            require(
                _botAdr[j] != 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D,
                "Can't blacklist Uniswap router."
            );
    ...
    function _transfer(
        ...
        if (sender != owner() && recipient != owner()) {
            ...
            require(
                balanceOf(recipient).add(amount) <= _maxTxAmount,
                "Already bought maxTxAmount, wait till check off"
            );
        }
    ...
    function withdrawToken(
        ...
        require(_ERC20address != address(this), "Can't");
    ...
    function migrateHolders(
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            require(
                balanceOf(msg.sender) - amounts[i] >= 0,
