...
contract MeltToken is DynamicArweaveHash {
    ...
    function _getChangingData() internal view returns(string memory) {
        return string(abi.encodePacked(
            _wrapTrait("Day",_getDay().toString()),
            ',',_wrapTrait("Melted", _getMelted()),
            ',',_wrapTrait("Background Color",_getBackground())
        ));
    }
    ...
    function _getDay() internal view returns(uint256) {
        return ((block.timestamp - _creationTimestamp)/_cadence) % 365;
    }
