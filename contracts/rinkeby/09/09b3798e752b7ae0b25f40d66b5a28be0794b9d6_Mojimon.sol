...
contract Mojimon is ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    bool public isMintingActive = false;
    uint256 public price = 1; //0.000001 ETH //100000000000000000; //0.1 ETH
    uint256 public _maxSupply = 0;
    string internal baseURI;
    address payable internal multisig;
    uint256[11111][6] public pandas;
    bytes private prevHash;
    uint256[] public partQuantites = [5,8,5,10,15,20];
    ...
    function getPanda(uint256 panda) public view returns (uint256 color, uint256 background, uint256 hair, uint256 eyes, uint256 mouth, uint256 neck) {

        return (pandas[panda][0],pandas[panda][1],pandas[panda][2],pandas[panda][3],pandas[panda][4],pandas[panda][5]);
    }
    ...
    function _rng(address sender) internal returns (uint256) {
        ...
        prevHash = abi.encodePacked(seed, block.coinbase, sender);
        return uint256(seed);
    }
    ...
    function startMinting(uint256 endIndex) public onlyOwner {
        require(endIndex > _maxSupply, "endIndex must be greater than the current _maxSupply of Mojimon");
        _maxSupply = endIndex;
        ...
        for(uint256 i = 0; i < quantity; i++) {
            ...
            if (totalSupply() < _maxSupply) {
                ...
                for(uint256 j = 0; j < 6; j++){
                    pandas[mintIndex][j] = random(_seed, partQuantites[j]);
