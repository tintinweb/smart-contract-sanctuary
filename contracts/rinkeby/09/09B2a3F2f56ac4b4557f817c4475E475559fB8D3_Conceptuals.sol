...
contract Conceptuals is ERC721Enumerable, ReentrancyGuard, Ownable {
  uint256 private naturalCount;
  uint256 private syntheticCount;

  bool private allNaturalsClaimed = false;
  bool private releaseAllSynthetics = false;

  uint256 private syntheticUnlockTimestamp;

  constructor() ERC721("Conceptuals", "CONCEPT") {}

  function ownerUpdateAllNaturalsClaimed(bool _claimed) public onlyOwner {
    allNaturalsClaimed = _claimed;
  }

  function ownerUpdateSyntheticUnlockTimestamp(uint256 _timestamp)
    public
    onlyOwner
  {
    syntheticUnlockTimestamp = _timestamp;
  }

  function ownerReleaseAllSynthetics(bool _release) public onlyOwner {
    releaseAllSynthetics = _release;
  }
    ...
    returns (string memory)
  {
    ...
    string[7] memory possesiveAdjectives = ConceptualsParts
    .getPossessiveAdjectives();
    string[3] memory linkingVerbs = ConceptualsParts.getLinkingVerbs();
    string[188] memory targetDomains = ConceptualsParts.getTargetDomains();
    ...
    if (possessiveAdjectiveIndex == 0) {
      return
        string(
          abi.encodePacked(
            Capitalize.capitalizeString(targetDomains[targetDomainIndex]),
            " ",
    ...
    returns (string memory)
  {
    ...
    string[3] memory articles = ConceptualsParts.getArticles();
    string[124] memory sourceDomains = ConceptualsParts.getSourceDomains();
      ...
      bytes(
        string(
          abi.encodePacked(
            ...
            '",',
            '"description": "Conceptuals are a collection of randomly generated conceptual metephors generated and stored entirely on the Ethereum blockchain. These abstract thoughts and ideas can inspire endless interpretations and use cases. Feel free to use Conceptuals in any way you see fit.",',
  ...
  function claimNatural(uint256 _tokenId) public nonReentrant {
    require(_tokenId > 0 && _tokenId < 9001, "Natural token ID invalid");
  ...
  function claimSynthetic(uint256 _tokenId) public nonReentrant {
    require(_tokenId > 10000 && _tokenId < 39001, "Synthetic token ID invalid");

    if (!releaseAllSynthetics) {
    ...
    onlyOwner
  {
    ...
    for (uint256 i; i < _tokenIds.length; i++) {
      ...
      require(tokenId > 9000 && tokenId < 10001);
    ...
    onlyOwner
  {
    ...
    for (uint256 i; i < _tokenIds.length; i++) {
      ...
      require(tokenId > 39000 && tokenId < 40001);
...
/// [MIT License]
/// @title Capitalize
/// @notice Provides a function for capitalizing a given word
/// @author Conceptuals Team <conceptualsproject.io>
library Capitalize {
  function capitalizeString(string memory _string)
    internal
    pure
    returns (string memory)
  {
    ...
    for (uint256 i = 0; i < lowercase.length; i++) {
      if (
        keccak256(abi.encodePacked((lowercase[i]))) ==
        keccak256(abi.encodePacked((uppercase[i])))
      ) {
...

{
  ...
  "libraries": {
    "/contracts/ConceptualsParts.sol": {
      "ConceptualsParts": "0xea6a92ABb25C7EcCeC6edae61ce9ac1d5e4A8CD2"
    }
  },
  "outputSelection": {
    "*": {
