...
contract Pack is ERC1155, Ownable {
  ...
  /// @dev Sets the pack market address.
  function setPackMarket(address _packMarket) external onlyOwner {
    packMarket = _packMarket;
  }
  ...
  * @param rewardTokenUris The URIs for each reward token added to the pack.
  ...
  function createPack(
    ...
    require(packMarket != address(0), "Must set pack market before packs can be created.");
    ...
    // Approve PackMarket to handle pack tokens.
    setApprovalForAll(packMarket, true);
  ...
  * @param packId The ERC1155 tokenId of the pack token being opened.
   */
  function openPack(uint packId) external returns (uint rewardTokenId) {
    require(balanceOf(msg.sender, packId) > 0, "Sender owns no packs of the given packId.");

    // FOR DEMO ONLY -- get random number.
    uint randomNumber = uint(keccak256(abi.encodePacked(msg.sender, _seed, uint(blockhash(block.number - 1)))));
    // Update seed.
    _seed = randomNumber;

    // Get reward token ID.
    rewardTokenId = getRandomReward(packId, randomNumber);

    // Burn 1 pack of `msg.sender` 
    _burn(msg.sender, packId, 1);
    circulatingSupply[packId] -= 1;

    // Mint 1 reward token to `msg.sender`
  ...
  function burn(uint tokenId, uint amount) external {
    // `_burn` makes balance check.
    _burn(msg.sender, tokenId, amount);
  }

  /// @dev See ERC1155 `_burnBatch`
  function burnBatch(uint[] calldata tokenIds, uint[] calldata amounts) external {
    // `_burnBatch` makes balance check.
