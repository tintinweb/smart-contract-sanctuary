...
contract BlootDoggsTest is ERC721Enumerable, ReentrancyGuard, Ownable {
    //Loot Contract
    address public lootAddress = 0x4F8730E0b32B04beaa5757e5aea3aeF970E5B613;
    address public doggAddress =
    0xb2c02e550ad5845831D03aA38155FeE26931d33E;
    
    LootInterface public lootContract = LootInterface(lootAddress);
    LootInterface public doggContract = LootInterface(doggAddress);

    bool private toggle = true;
    bool private blootMintToggle = true;
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        parts[1] = toggle ? getTag(tokenId) : getBreed(tokenId);

        parts[3] = toggle ? getBreed(tokenId) : getTag(tokenId);
        ...
        for (uint i = 1; i < 16; i = i + 2){
            prevY = y;
            y = prevY + ceilDiv(length(parts[i]), maxChars) * 20;
            parts[i] = getBeautyText(parts[i], prevY);
            if(i < 15)
                parts[i + 1] = string(abi.encodePacked('</text><text x="10" y="', toString(y), '" class="base">'));
        }
    ...
    function mintWithDogg(uint256 lootId) public nonReentrant {
        require(limit == 0 || totalSupply() < limit, "Limit reached");
        require(lootId > 0 && lootId <= 20000, "Token ID invalid");
        // require(doggContract.ownerOf(lootId) == msg.sender, "Not the owner of this BlootDogg");
	    uint256 newTokenId = lootId > 8008 || blootMintToggle ? lootId : lootId + 20000;
        _safeMint(_msgSender(), newTokenId);
    }

    function multiMintWithDogg(uint256[] memory lootIds) public nonReentrant {
        require(limit == 0 || totalSupply() + lootIds.length <= limit, "Limit reached");
        for (uint256 i = 0; i < lootIds.length; i++) {
            require(lootIds[i] > 0 && lootIds[i] < 20000, "Token ID invalid");
            require(doggContract.ownerOf(lootIds[i]) == msg.sender, "Not the owner of this BlootDogg");
	        uint256 newTokenId = lootIds[i] > 8008 || blootMintToggle ? lootIds[i] : lootIds[i] + 20000;
            _safeMint(_msgSender(), newTokenId);
        }
    }
    ...
    function getBeautyText(string memory str, uint y) internal view returns (string memory) {
        ...
        for (uint256 i = 0; i < arr.length; i++) {
            if(str_length + length(arr[i]) + 1 >= maxChars) {
                output = string(abi.encodePacked(output, '</text><text x="10" y="', Strings.toString(y + 20 * lines), '" class="base">'));
                lines = lines + 1;
                str_length = 0;
            }
