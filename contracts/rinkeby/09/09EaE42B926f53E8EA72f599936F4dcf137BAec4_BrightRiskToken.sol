...
contract BrightRiskToken is ERC20Upgradeable, OwnableUpgradeable, IBrightRiskToken {
    ...
    /// @notice An event that's emitted when new staking has happened
    ...
    /// @notice Deposits capital into 'external' pool, to be 'staked' on behalf of the user later on, access: ANY
    function deposit(uint256 _amount) external override {
        //TODO Allow only more than min value
    ...
    /// @notice Here is where rewards and unstaked capital comes into, from position controllers, access: CONTROLLER
    ...
    /// @notice Moves capital from 'internal' pool into 'external', access: OPERATOR
    function depositFromInternal(uint256 _amount) external {
    ...
    /// @notice Stake 'external' pool at the dedicated position controller, access: OPERATOR
    // TODO Fix permissions
    function stakeAt(
    ...
    // TODO Fix permissions
    function callUnstakeAt(address _controllerAddress) external {
        uint256 _calledAmount = IPositionController(_controllerAddress).callUnstake();
        //TODO emit event
    }

    // TODO Fix permissions
    function unstakeAt(address _controllerAddress) external {
        uint256 _internalPoolBefore = internalPool;
        uint256 _unstakedAmount = IPositionController(_controllerAddress).unstake();
        require(
            _internalPoolBefore.add(_unstakedAmount) == internalPool,
            "BrightRiskToken: BRI18"
        );
        //TODO emit event
    }
    ...
    */
    ...
    function adjustStreamingFee(FeeState memory _feeSettings) external onlyOwner {
        feeState = _feeSettings;
    }
    ...
    function totalTVL() public view returns (uint256 _tvl) {
        _tvl = totalAtStake().add(externalPool);
    }
...
interface IPriceFeed {
    ...
    function howManyTokenAinB(address tokenA,address tokenB,uint256 amount)  external view returns (uint256);
