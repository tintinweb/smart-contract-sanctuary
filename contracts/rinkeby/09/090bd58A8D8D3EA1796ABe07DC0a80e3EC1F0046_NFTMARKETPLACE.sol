...
contract NFTMARKETPLACE is Ownable, ERC721Token, ERC721Holder {
    using Strings for string;
    using Integers for uint;

    function NFTMARKETPLACE () ERC721Token("NFTMARKETPLACE" ,"NFTMKT") public {
    ...
    function mintDigitalArt(string _ipfsHash, string _name, string _description) public payable {
        require(msg.value == createDigitalArtFee);
        require(ipfsHashToTokenId[_ipfsHash] == 0);

        DigitalArt memory _digitalArt = DigitalArt({ipfsHash: _ipfsHash, publisher: msg.sender, name: _name, description: _description });
    ...
    function addDigitalArtSellingItem(uint256 _tokenId, uint128 _price) public onlyOwnerOf(_tokenId) {
        ...
        SellingItem memory _sellingItem = SellingItem(msg.sender, uint128(_price), digitalArts[_tokenId].ipfsHash, digitalArts[_tokenId].name, digitalArts[_tokenId].description);
    ...
    function purchaseDigitalArtSellingItem(uint256 _tokenId) public payable {
        uint128 oneEther = 1 ether;
        uint128 priceItem = tokenIdToSellingItem[_tokenId].price;
        uint128 priceItemEther = priceItem * oneEther;

        require(tokenIdToSellingItem[_tokenId].seller != address(0));
        require(tokenIdToSellingItem[_tokenId].seller != msg.sender);
        require(priceItemEther == msg.value);

        uint128 feeToOwnerItem = priceItemEther * feeToOwner / 100;
        uint128 priceItemEtherFee = priceItemEther - feeToOwnerItem;

        tokenIdToSellingItem[_tokenId].seller.transfer(priceItemEtherFee);
