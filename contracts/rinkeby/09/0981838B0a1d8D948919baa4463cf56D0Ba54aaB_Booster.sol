...
contract Booster is ERC1155, Ownable, Pausable {
    ...
    IManager public staking;

    mapping(address => bool) public approvedManagers;

    constructor() ERC1155("https://booster-test-api.herokuapp.com/meta/{id}") {
    }
    ...
    function mint(uint16[] calldata unstakedTokens) external whenNotPaused {
        ...
        IManager.Stake[] memory stake = staking.getAccountGoldMiners(msg.sender);
        IManager.Stake[] memory stakePirates = staking.getAccountPirates(msg.sender);
        ...
        for (uint i = 0; i < unstakedTokens.length; i++) {
            ...
            require(token.ownerOf(unstakedTokens[i]) == msg.sender, "Only unstaked tokens");
