...
contract METAdor is ERC1155MintBurn, Ownable {
  using SafeMath for uint256;
  
  address proxyRegistryAddress;
  uint256 private _currentTokenID = 0;
  mapping (address => bool) public isApprovedCreators;
  ...
  /*
    * @dev Require msg.sender to be an approved ntf creator;
  */
  modifier onlyApprovedCreator() {
      require(isApprovedCreators[msg.sender], "METAdor#onlyApprovedCreator ONLY_APPROVED_CREATOR_CAN_MINT");
      _;
  }
  ...
  /**
    ...
    */
  function create(address _initialOwner, uint256 _initialSupply, string calldata _uri, bytes calldata _data) external onlyApprovedCreator returns (uint256) {
  ...
  function setCreator(address _to, uint256[] memory _ids) public {
    require(_to != address(0), "METAdor#setCreator: INVALID_ADDRESS.");
    for (uint256 i = 0; i < _ids.length; i++) {
      uint256 id = _ids[i];
      _setCreator(_to, id);
    }
  ...
  */
  ...
  /*
  * @dev approve an address to mint token
  * @param _newCreator The new address to add to the isApprovedCreators
  */
  function approveCreator(address _newCreator) external onlyOwner {
      isApprovedCreators[_newCreator] = true;
  }
  /*
  * @dev disapprove an address from minting token
  * @param _creator The address to disapprove
  */
  function disapproveCreator(address _creator) external onlyOwner {
      isApprovedCreators[_creator] = false;
  }
}