...
contract DichoStudio0xCarat is ERC721, Ownable, ReentrancyGuard {
    ...
    uint256 private _maxPublicSupply = 888;
    ...
    function craft(uint256 numToMint) external nonReentrant {
        ...
        require(numToMint <= mintsAvailable(), "Supply exceeded");
        require(
    ...
    function correctHue(uint256 hue, uint256 seed) internal pure returns (uint256) {
        if (hue < 35 && hue > 25) {
            hue += ((seed & 0xFF) % 301);
        }
        return hue;
    ...
    function getRingParams(uint256 tokenId) internal view returns (RingParams memory) {
        RingParams memory rp;
        {
            ...
            rp.gemcol_h = correctHue(rSeed % 335, rSeed);
            rSeed = rSeed >> 8;
            ...
            if ((rSeed & 0xFF) < 8) { // 8/255 = ~3% chance of "ethereum" cut 
                rp.cut_idx = 0; 
            } else if ((rSeed & 0xFF) < 59) { // between 8 and 59 / 255 = ~20% chance of round cut
                rp.cut_idx = 1;
            } else if ((rSeed & 0xFF) < 148) { // between 26 and 102 / 255 = ~35% chance of emerald cut 
                rp.cut_idx = 2;
            } else {
            ...
            if (_wearerSeed[tokenId] != _ringSeed[tokenId]) {
                ...
                rp.bgcol_h = correctHue(wSeed % 335, wSeed);
                wSeed = wSeed >> 8; 
    ...
    function getSvg(uint256 tokenId) internal view returns (bytes memory, RingParams memory) {
...

        {
            ...
            if (rp.cut_idx == 0) {
                buf = abi.encodePacked(
                    ...
                    '<stop offset=".27" stop-opacity=".79" stop-color="hsl(',
                    ...
                    '%)" />'
                    '<stop stop-color="#EFC8DD" offset=".35" stop-opacity=".6"/>'
        ...
        {
            uint256 bgcol_l = rp.bgcol_h < 210 ? 35 : rp.bgcol_h < 330 ? 65 : 80;
            buf = abi.encodePacked(
                buf,
                ...
                "%, ",
                toString(rp.bgcol_h < 210 ? 35 : rp.bgcol_h < 330 ? 65 : 80),
                '%)">'
