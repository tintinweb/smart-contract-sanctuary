...
contract TokenVestingV2 is Ownable, ReentrancyGuard, CarefulMath {
  ...
  event CreateStream(
    ...
    function investorStream(address recipient, uint256 deposit)
        public
        returns (uint256){
        uint256 startTime = block.timestamp + 300 seconds;
        uint256 stopTime = block.timestamp + 1500 seconds;
        ...
        require(deposit / 4 == 0, 'deposite can not be div into equal');
        ...
        IERC20(iagonTokenAddress).safeTransferFrom(msg.sender, address(this), deposit);
        emit CreateStream(streamId, msg.sender, recipient, deposit, iagonTokenAddress, startTime, stopTime);
        return streamId;
    }

    function advisorStream(address recipient, uint256 deposit)
        public
        returns (uint256){
        uint256 startTime = block.timestamp + 300 seconds;
        uint256 stopTime = block.timestamp + 1200 seconds;
        require(recipient != address(0x00), "stream to the zero address");
        require(recipient != address(this), "stream to the contract itself");
        require(recipient != msg.sender, "stream to the caller");
        require(deposit > 0, "deposit is zero");
