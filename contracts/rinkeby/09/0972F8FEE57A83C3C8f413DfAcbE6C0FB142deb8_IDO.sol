...
contract IDO is Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) public purchasedBuyerMap;
    ...
    modifier inPurchase(){
        require(msg.sender == tx.origin, "only the EOA address can purchase");
        if (buyerWhiteList[msg.sender]) {
            require(startTime <= block.timestamp + 10 minutes, "startTime <= block.timestamp");
        } else {
    ...
    function addWhiteList(address[] memory _operators) onlyOwner external {
        for (uint256 i = 0; i < _operators.length; i++){
            buyerWhiteList[_operators[i]] = true;
        }
    }

    function removeWhiteList(address[] memory _operators) onlyOwner external {
        for (uint256 i = 0; i < _operators.length; i++){
            buyerWhiteList[_operators[i]] = false;
        }
    }


    function purchase() inPurchase nonReentrant payable external {

        require(currentSupply != 0, "currentSupply is 0, sold out");
        address buyer = msg.sender;
        require(!purchasedBuyer[buyer], "you have bought");

        require(msg.value >= 0.1 ether, "the min amount is 0.1");
        require(msg.value <= 3 ether, "the max amount is 3");

        uint256 amount = msg.value / pricePerShare;

        purchasedBuyer[buyer] = true;
        purchasedBuyerMap[buyer] = amount;
        currentSupply = currentSupply.sub(amount);

        emit Purchase(buyer, amount, pricePerShare);
    }
    ...
    function changeEndTime(uint256 _endTime) onlyOwner external {
        if (_endTime == uint256(0)) {
            _endTime = block.timestamp;
        }
        endTime = _endTime;
    }

    function changeCurrentSupply(uint256 _left) onlyOwner external {
        currentSupply = _left;
    }
