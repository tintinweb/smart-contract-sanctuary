...
contract StakingErc721 is IERC721Receiver, ReentrancyGuard, Pausable, Ownable {
  ...
  mapping(uint256 => bool) public nftTokensMap;
  mapping(address => uint256) public nftAccountBalances;
  mapping(uint256 => mapping(address => uint256)) public nftTokenBalances;

  constructor(address _NFT, address _POINT) public {
    NFT = IERC721(_NFT);
    POINT = IERC20(_POINT);
  }
  ...
  function balanceOf(address account) external view returns (uint256) {
    return nftAccountBalances[account];
  }
  ...
  function changeAddresses(address _POINT, address _NFT) public onlyOwner {
    POINT = IERC20(_POINT);
    NFT = IERC721(_NFT);
  }
  ...
  function stakeInternal(address owner, uint256 tokenId) internal {
    require(nftAccountBalances[owner].add(1) <= maxAmount, "NFT max amount exceeded");

    if(!nftTokensMap[tokenId]) {
      nftTokensMap[tokenId] = true;
    ...
    nftAccountBalances[owner] = nftAccountBalances[owner].add(1);
    nftTokenBalances[tokenId][owner] = 1;
  ...
  function withdrawNFTInternal(uint256 tokenId) internal {
    ...
    nftAccountBalances[msg.sender] = nftAccountBalances[msg.sender].sub(1);
    nftTokenBalances[tokenId][msg.sender] = nftTokenBalances[tokenId][msg.sender].sub(1);
  ...
  function withdraw() public updateReward(msg.sender) {
    ...
    for (uint8 i = 0; i < nftTokens.length; i++) {
      if(nftTokenBalances[nftTokens[i]][msg.sender] > 0) {
        length++;
      }
    }
    ...
    for (uint8 i = 0; i < nftTokens.length; i++) {
      ...
      if(nftTokenBalances[tokenId][msg.sender] > 0) {
        tokenIds[j] = tokenId;
        j++;
      }
    }
  ...
  function getReward() public payable nonReentrant updateReward(msg.sender) {
    ...
    if (reward > 0) {
      ...
      POINT.safeTransfer(msg.sender, reward);
    ...
    updateReward(address(0))
  {
    ...
    uint256 balance = POINT.balanceOf(address(this));
    require(
  ...
  function recoverERC20(address tokenAddress, uint256 tokenAmount)
    ...
    onlyOwner
  {
    require(tokenAddress != address(POINT) && tokenAddress != address(POINT), "Cannot withdraw the staking or rewards tokens");
