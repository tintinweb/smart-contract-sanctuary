...
contract PiRatGame is IPiRatGame, Ownable, ReentrancyGuard, Pausable {
    ...
    function showTotalSupply() public view returns (uint256 totalSupply) {
        totalSupply = (potm.totalPiratsMinted() - potm.totalPiratsBurned());
    }
    ...
    function showWalletOfOwner(address owner) public view returns (uint256[] memory) {
        uint256 tokenCount = potm.balanceOf(owner);
        uint256[] memory tokensId = new uint256[](tokenCount);
        for (uint256 i; i < tokenCount; i++) {
            tokensId[i] = potm.tokenOfOwnerByIndex(owner, i);
        }
        return tokensId;
    }    
