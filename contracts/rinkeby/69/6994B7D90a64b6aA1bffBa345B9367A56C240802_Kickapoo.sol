    ...
    ReentrancyGuard
{
    ...
    function mintToken(uint256 amount)
        ...
        returns (bool)
    {
        require(getSaleMode() == 1 || getSaleMode() == 3, "Sale not available.");
        if (getSaleMode() == 3) {
            require(amount <= maxTokenPerTx, "Transaction limit exceed.");
        ...
        if (getSaleMode() == 1 || getSaleMode() ==3) {
            ...
            if (getSaleMode() == 1) {
                _presaleClaimed[msg.sender] = _presaleClaimed[msg.sender] + amount;
            }
        }
    ...
    function mintReserve(uint256 amount) external onlyOwner {
        require(totalSupply() < maxSupply, "Not enough reserve");
        require(
