...
contract Beholders is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint256 private constant FOUNDERS_RESERVE_AMOUNT = 100;
    uint256 private constant MAX_PUBLIC_BEHOLDERS = MAX_BEHOLDERS - FOUNDERS_RESERVE_AMOUNT;
    ...
    function mintEarlyAccess(uint256 _count) external payable nonReentrant whenEarlyAccessActive returns (uint256, uint256) {
        require(_count != 0, "Invalid Beholder count");
        require(isOnEarlyAccessList[msg.sender], "Address not on Early Access list");
        require(_tokenIds.current() + _count <= MAX_BEHOLDERS, "All Beholder have been minted");
    ...
    function founderMint(uint256 _count) public nonReentrant returns (uint256, uint256) {
        ...
        uint256 firstMintedId = MAX_PUBLIC_BEHOLDERS + _tokenIds.current() + 1;
        for (uint256 i = 0; i < _count; i++) {
            _reservedTokenIds.increment();
            mint(MAX_PUBLIC_BEHOLDERS + _reservedTokenIds.current());
        }
    ...
    function mintPublicSale(uint256 _count) external payable nonReentrant whenPublicSaleActive returns (uint256, uint256) {
        require(_count > 0 && _count <= MAX_PER_ADDRESS, "Invalid Beholders count");
        require(_tokenIds.current() + _count <= MAX_PUBLIC_BEHOLDERS, "All Beholders have been minted");
