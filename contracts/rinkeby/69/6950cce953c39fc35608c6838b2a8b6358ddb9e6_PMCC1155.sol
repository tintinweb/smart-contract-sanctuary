...
// Modified ERC-1155 contract to function more as an ERC721 but with
// improved efficient minting capabilities. Minting only creates NFTS.

contract PMCC1155 is Context, ERC165, IERC2981, IERC1155, IERC1155MetadataURI, ReentrancyGuard, Ownable {
    ...
    // 5% royalty expected
    uint256 private _royaltyFee = 500; // out of 10 000

    // royalty to be paid to specific address
    ...
    // sale stage variable: 0 - founder, 1 - whitelist, 2 - public
    // starts at 0
    uint256 _stage = 0;
    ...
    // Public function to set the base URI
    function setURI(string calldata newuri) public onlyOwner {
        _setURI(newuri);
    }

    // Public functions to get the total supply
    function totalSupply() public view returns (uint256) {
        uint256 tokenId = _tokenIdCounter.current();
        return tokenId;
    }

    // Public function to add list of addresses to whiteList
    ...
    // Mints multiple tokens at the same time
    function mintBatch(
        address to,
        uint256 number,
        bytes memory data
        ...
        require(
            payable(_royaltyReceiver).send(address(this).balance),
            "PMCC: sending fee failed - transaction reverted"
        );
        _mintBatch(to, ids, amount, data);
    }

    // Returns cost of minting a token, which is based on the tokenId range
    ...
    function uri(uint256 tokenId) public view virtual override returns (string memory) {
        if (tokenId < 301) {
            return string(bytes.concat(bytes(_uri), "founder.json"));
        } else if (tokenId < 1301) {
            return string(bytes.concat(bytes(_uri), "diamond.json"));
        } else {
            return string(bytes.concat(bytes(_uri), "platinum.json"));
        }
    }

    // Set the recommended royalty percentage 0-10000
    function setRoyaltyFee(uint256 fee) public onlyOwner {
        require(fee <= 10000, "PMCC: royalty cannot be greater than 100%");
        _royaltyFee = fee;
    }

    // Retrieve recommended royalty payment as per EIP-2981
    ...
    // Change receiving address for royalties
