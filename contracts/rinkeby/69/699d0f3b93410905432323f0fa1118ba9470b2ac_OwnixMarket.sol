...
contract IOwnixMarket {
  ...
  // Bids by token token id => index => bid
...
/**
 * @title Interface for contracts conforming to Ownix ERC2 ERC-20
 */
...
contract OwnixMarket is ERC1155Holder, Ownable, Pausable, IOwnixMarket {
  ...
  function placeBid(
    ...
    Bid memory bid = bids[_tokenId][_index];
 ...
 /**
  ...
  function finishAuction(
    ...
    require(bid.bidder != address(0), "Can't finish Auction without any bids");
    ...
    if (ownerSharePerMillion > 0) {
      // Calculate sale share
      saleShareAmount = bid.price * ownerSharePerMillion / ONE_MILLION;
  ...
  /**
   * @dev Get bid by token id and index
   * @param _tokenId - uint256 of the token id
   * @param _index - token index
   * @return bid
   */
  function getBid(uint256 _tokenId, uint256 _index) external view returns (Bid memory)  {
    return bids[_tokenId][_index];
  }
}