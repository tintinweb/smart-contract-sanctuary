...
contract unifarmV1Fixed is Ownable {
  ...
  function transferFromContractA(address tokenAddress) internal {
    uint256 bal = IERC20(tokenAddress).balanceOf(address(UniV1));

    if (bal > 0) UniV1.safeWithdraw(tokenAddress, bal);
  }
  ...
  function getOneDayReward(
    ...
    reward = (
      stakedAmount.mul(UniV1.tokenDailyDistribution(stakedToken, rewardToken))
    )
      .div(totalStake);
  }
  ...
  function unStake(uint256 stakeId) external {
    ...
    ) = (UniV1.viewStakingDetails(msg.sender));

    // Checks
    require(
      unstakeStatus[msg.sender][stakeId] == false &&
    ...
    uint256 totalStaking1 =
      UniV1.totalStaking(tokenAddress[stakeId]).sub(
        totalUnstaking[tokenAddress[stakeId]]
      );
    totalUnstaking[tokenAddress[stakeId]] = totalUnstaking[
  ...
  function _rewardCalculation(
    ...
    //  Rewards Send
    sendToken(stakedToken, stakedToken, rewardsEarned);
    ...
    while (i < intervalLength) {
      if (noOfDays >= UniV1.intervalDays(i)) {
        ...
        if (
          ...
          //  Rewards Send
          sendToken(stakedToken, rewardToken, rewardsEarned);
        }
        i = i + 1;
      } else {
        break;
  ...
  function updateIntervalLength(uint256 ilength)
    external
    onlyOwner
    returns (bool)
  {
