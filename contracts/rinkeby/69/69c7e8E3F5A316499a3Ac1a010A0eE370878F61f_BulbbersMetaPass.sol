...
contract BulbbersMetaPass is ERC721, Ownable  {
    ...
    uint8 private mintedPerStage = 0;
    ...
    mapping(address => bool) public usedList;
    mapping(address => uint16) public reservedTokens;

    uint8 public usedTokens = 0;
    ...
    function _mintSingleNFT() private {
        require(_totalSupply < MAX_SUPPLY + 1, "Reached maximum count of NFTs!");
    ...
    function mintNFTs(uint8 _count) public payable {
        ...
        require(saleStage < 8, "Sale has been ended!");
        // require(balanceOf(msg.sender) + _count < MAX_PER_ACCOUNT + 1, "Reached maximum limit per account");
        require((_totalSupply + _count < MAX_SUPPLY + 1) && (mintedPerStage + _count < 101), "Not enough NFTs!");
    ...
    function claimReserved() public {
        require(saleStage > 0, "Sale has not been started");
        require(reservedList[msg.sender], "You have no access to free mint!");
        require(saleStage < 7, "Free-mint time is over!");
        require(!usedList[msg.sender], "You have already claimed your Meta-Pass!");
        usedList[msg.sender] = true;
        usedTokens += 1;

        _safeMint(msg.sender, 600 + usedTokens);
    }
    ...
    function setStage(uint8 stage) public onlyOwner {
        ...
        if (stage == 7) {
            mintedPerStage = usedTokens;
            _totalSupply += usedTokens;
        }
    }

    function addWinner(address[] memory winners) public onlyOwner {
        for(uint8 i = 0; i < winners.length; i++) {
            address addr = winners[i];
            require(addr != address(0), "Empty Address");
            require(!reservedList[addr], "Duplicate Address");

            reservedList[addr] = true;
        }
    }
}