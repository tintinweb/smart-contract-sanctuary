 ...
 */
contract Mandox is ERC721, Ownable {
    using SafeMath for uint256;
    using Strings for uint256;
    ...
    */
    function mint(uint256 _count) public payable {
        ...
        if (privateSaleIsActive) {
            ...
            require(whitelist[msg.sender].hasMinted.add(_count) <= MAX_MINT_WHITELIST, "Can only mint 5 while whitelisted");
            require(whitelist[msg.sender].hasMinted <= MAX_MINT_WHITELIST, "Can only mint 5 while whitelisted");
    ...
    function setWhitelistAddr(address[] memory addrs, uint[] memory claimAmounts) public onlyOwner {
        whitelistAddr = addrs;
