...
// File contracts/PachPunksDNA.sol


pragma solidity ^0.8.0;

contract PachPunksDNA{
    ...
    //This pseudorandom function is deterministic and shouldn't be used in production
    ...
    function getAccessorieType(uint256 _dna)
    public
    view
...
contract PachPunks is ERC721, ERC721Enumerable, PachPunksDNA {
    ...
    constructor(uint256 _maxSupply) ERC721("PachPunks", "PACHPKS") {
        maxSupply = _maxSupply;
    } 

    function mint() public {
        uint256 current = _idCounter.current();
        require(current <= maxSupply, "No PachPunks left");
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory){
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "PachPunks #',
                tokenId.toString(),
                '", "description": "This PachPunk is part of a project with the intention of learning about web3", "image": "',
                image,
                '"}'
            )
        );
