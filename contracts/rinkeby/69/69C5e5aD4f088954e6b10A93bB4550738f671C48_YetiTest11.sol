...
contract YetiTest11 is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  bool private communitySaleIsActive = false;
  mapping(uint256 => uint) revealTime;
  address[] private communityAddressBook;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if(communitySaleIsActive) {
        communityPresaleMint();
        communitySaleIsActive = false;
    } else if(supply + _mintAmount < 6){
  ...
  // public
  function communityPresaleMint() private {
        ...
        for (uint i = 0;
            supply + i <= 10;
            i++) {
            require((supply + i) <= 10, "Cannot exceed max presale supply");
            for(uint j = 0; j < communityAddressBook.length; j++){
                if (!_exists(supply + i)) {
                    _safeMint(communityAddressBook[j], supply + i);
