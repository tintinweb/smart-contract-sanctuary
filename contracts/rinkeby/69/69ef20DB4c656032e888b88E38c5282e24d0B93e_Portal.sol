...
contract Portal is RelayRecipientUpgradeable {
  ...
  function metaUnsynthesize(
    ...
    balanceOf[_path[0]] = balanceOf[_path[0]] - _amountIn;
    unsynthesizeStates[_txID] = UnsynthesizeState.Unsynthesized;

    // unlock assets and transfer to portal
    // unlockAssets(_path[0], _amountIn, address(this));

    // swap token2 to token3
    IERC20(_path[0]).approve(_dex, _amountIn);
    (bool success, ) = _dex.call(swapCalldata);

    require(success, "Portal: swap in metaUnsynthesize failed");

    // transfer token3 from portal to router
    ...
    emit BurnCompleted(_txID, address(this), _amountIn, _path[0]);
  }
