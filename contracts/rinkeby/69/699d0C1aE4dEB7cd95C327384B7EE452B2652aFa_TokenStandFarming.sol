...
contract TokenStandFarming is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function stake(uint256 fid, uint256 amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (giftReward > 0) {
                user.rewards = 0;
                TransferHelper.safeTransfer(address(farm.gift), msg.sender, giftReward);
            }
        }
        if (amount > 0) {
    ...
    // Withdraw ERC20 LP token/STAND staked from MasterChef
    function withdraw(uint256 fid, uint256 amount) external nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(farm.accStandPerShare).div(1e12);
        emit Withdrawn(msg.sender, fid, amount, standReward, giftReward);
    }
    ...
    function emergencyWithdraw(uint256 fid) external {
        ...
        TransferHelper.safeTransfer(stakingTokens[fid], address(msg.sender), amount);
    ...
    function transferStandOwnership(address newMasterChef) public restricted {
