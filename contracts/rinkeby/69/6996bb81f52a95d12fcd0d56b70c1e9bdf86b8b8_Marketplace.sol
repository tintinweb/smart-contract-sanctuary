...
contract Marketplace {
    ...
    string memory _name, uint256 _price, address payable _buyer_addr, address payable _seller_addr)
    internal {
        uint256 i;
    ...
    internal view returns(address payable, uint256, uint256) {
    ...
    modifier checker() {
        require(check(seller) && check(buyer));
        _;
    }
    ...
    function Sell(string memory _name, uint256 _price, string memory _unit) checker()
    public payable {
        ...
        insert(PublicBoard, Pseller, payable(msg.sender), _name, _price, payable(msg.sender), payable(msg.sender));
    }
    ...
    public payable {
        ...
        insert(SellerBoard, seller, SellerAddr, _name, GoodsPrice, payable(msg.sender), SellerAddr);
        insert(BuyerBoard, buyer, payable(msg.sender), _name, GoodsPrice, payable(msg.sender), SellerAddr);
