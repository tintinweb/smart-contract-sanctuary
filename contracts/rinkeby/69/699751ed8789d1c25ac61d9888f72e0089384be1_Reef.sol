...
contract ReefController {
    ...
    function startGame() public returns (address _reefAddr) {
...
contract Reef {
    ...
    function stakeFish(uint _index) public returns (bool _success){
        delete inventoryFish[_index];
        return stake(inventoryFish[_index]);
    }
    ...
    function claimRewards() public returns (uint _amountClaimed) {
        ...
        totalDaysPassed += getDaysSinceLastWithdraw();
    ...
    // TODO: test this function
    function getDaysSinceLastWithdraw() public view returns (uint _daysStaked) {
        return uint((block.number - lastBlockClaimed) / blocksPerDaySuperExpedited);
    }

    // TODO: test this function
    function getDaysStakedForFish(Fish memory _fish) public view returns (uint _daysStaked) {
