...
contract PawnMortgage is Pawn
{
    ...
    struct Loan {
        ...
        uint64 loanStartTime; // The block.timestamp when the loan first began (measured in seconds).
        uint32 loanDuration; // The amount of time (measured in seconds) that can elapse before the lender can liquidate the loan and seize the underlying collateral.
        uint32 platformFee;
    ...
    function lend(
        ...
        Loan memory loan = Loan({
            ...
            platformFee: uint32(_platformFee),
    ...
    // Payback - only borrower can call
    function payback(uint256 _loanId) external whenNotPaused nonReentrant {
        ...
        uint256 interestDue = loan.repaymentAmount;
    ...
    function cancelLoanBeforeLoanHasBegun(uint256 _nonce) external {
        require(!_userNonce[msg.sender][_nonce], 'CancelLoanBeforeLoanHasBegun:: Invalid Nonce, Cancelled or Reused or Loand Started');
        _userNonce[msg.sender][_nonce] = true;
    }
    
    function checkWhetherNonceUsedByUserOrNot(address _user, uint256 _nonce) external view returns (bool) {
        return _userNonce[_user][_nonce];
    }
    
    function getPayoffAmount(uint256 _loanId) external view returns (address, uint256) {
    ...
    // Verify is valid Lender sign
    function isLenderSignValid(
        uint256 _loanPrincipalAmount,
        uint256 _repaymentAmount,
        uint256 _nftTokenId,
        uint256 _loanDuration,
    ...
    // Verify is valid Borrower sign
    function isBorrowerSignValid(
        uint256 _nftTokenId,
        uint256 _borrowerNonce,
        address _nftContract,
        address _borrower,
        ...
        if (_borrower == address(0)) {
            ...
            bytes32 messageWithEthSignPrefix = message.toEthSignedMessageHash();
            return (messageWithEthSignPrefix.recover(_borrowerSig) ==
                _borrower);
        }
    }
}
