...
contract JPGRegistry is Ownable, SignedMessage {
    ...
    event CreateSubRegistry(address indexed curator, bytes32 name);
    ...
    event AddNFTToSubRegistry(address indexed curator, bytes32 name, NFT token);
    ...
    function bulkAddToMainRegistry(NFT[] calldata tokens) public {
        ...
        for (uint256 index = 0; index < tokens.length; index++) {
            if (_addToMainRegistry(tokens[index])) {
                ...
                bundleListings[index] = Listing({signedMessage: bytes("0"), nft: tokens[index]});
            }
        }
    ...
    /**
     * @notice Add a token to a subregistry
     * @param subRegistryName The name of the subregistry
     * @param token An instance of an NFT struct
     */
    function addToSubregistry(bytes32 subRegistryName, NFT calldata token) public {
        ...
        if (mainRegistry[token.tokenContract][token.tokenId] || _addToMainRegistry(token)) {
            bool created = subRegistries[msg.sender].subRegistry[subRegistryName].created;
            if (!created) {
                // hasn't been created yet, create it and add to registryNames
                ...
                emit CreateSubRegistry(msg.sender, subRegistryName);
            }
            subRegistries[msg.sender].subRegistry[subRegistryName].nfts[token.tokenContract][token.tokenId] = true;
            emit AddNFTToSubRegistry(msg.sender, subRegistryName, token);
        }
    }

    function bulkAddToSubregistry(bytes32 subRegistryName, NFT[] calldata tokens) public {
        require(curators[msg.sender], "JPGR:ats:Only allowed curators");
        if (!subRegistries[msg.sender].subRegistry[subRegistryName].created) {
            // hasn't been created yet, create it and add to registryNames
            subRegistries[msg.sender].subRegistry[subRegistryName].created = true;
            subRegistries[msg.sender].registryNames.push(subRegistryName);
            emit CreateSubRegistry(msg.sender, subRegistryName);
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            if (mainRegistry[tokens[i].tokenContract][tokens[i].tokenId] || _addToMainRegistry(tokens[i])) {
                subRegistries[msg.sender].subRegistry[subRegistryName].nfts[tokens[i].tokenContract][
                    tokens[i].tokenId
                ] = true;
                emit AddNFTToSubRegistry(msg.sender, subRegistryName, tokens[i]);
            }
        }
    }
    ...
    function getSubRegistries(address curator) public view returns (string[] memory) {
        ...
        for (uint256 i = 0; i < registryNames.length; i++) {
            ...
            registryStrings[ctr] = bytes32ToString(registryNames[i]);
            ctr += 1;
        }
    ...
    function removeFromSubregistry(bytes32 subRegistryName, NFT calldata token) public {
        ...
        subRegistries[msg.sender].subRegistry[subRegistryName].nfts[token.tokenContract][token.tokenId] = false;
    }
    ...
    function inSubRegistry(
        ...
        return
            ...
            subRegistries[curator].subRegistry[subRegistryName].nfts[nft.tokenContract][nft.tokenId];
    }
    ...
    function adminBulkAddToMainRegistry(OwnerBundle[] memory ownerBundles) public onlyOwner {
        for (uint256 bundleIndex = 0; bundleIndex < ownerBundles.length; bundleIndex++) {
            Listing[] memory listings = ownerBundles[bundleIndex].listings;

            for (uint256 listingIndex = 0; listingIndex < listings.length; listingIndex++) {
                if (!_adminAddToMainRegistry(listings[listingIndex])) {
                    delete ownerBundles[bundleIndex].listings[listingIndex];
                }
            }
        }
     ...
     * @param listing An instance of a Listing struct
     */
    function _adminAddToMainRegistry(Listing memory listing) internal returns (bool) {
        try ERC721(listing.nft.tokenContract).ownerOf(listing.nft.tokenId) returns (address owner) {
            address signer =
