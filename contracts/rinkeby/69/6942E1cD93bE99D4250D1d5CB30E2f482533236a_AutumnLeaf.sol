...
contract AutumnLeaf is ERC721Enumerable, Ownable, ReentrancyGuard{
    ...
    string[2] private effect=["normal","glitch"];
    ...
    function getShadowBody(Leaf memory leaf) internal pure returns(string memory){
        ...
        '<feDropShadow dx="-6" dy="11" flood-color="#000000" flood-opacity="0.14" stdDeviation="0"/>'));
            ...
            if(leaf.effect==1){
                bg = string(abi.encodePacked(bg,'<feTurbulence id="E" type="turbulence" baseFrequency="0.02" numOctaves="3" result="noise" seed="0"/>',
                '<feDisplacementMap id="F" in2="noise" in="SourceGraphic" scale="50" xChannelSelector="R" yChannelSelector="G" scale="6"/>','</filter>'));
            }
        return bg;
    }
    ...
    function getSVGImage(Leaf memory leaf,uint256 tokenId) internal view returns(string memory){
        ...
        parts[10] = '</defs>';
        if(leaf.effect==1){
            parts[11] = string(abi.encodePacked('<animate href="#E" attributeName="baseFrequency" dur="2s" values="0.01 0.4;0.0;0.01 0.4;0.0;0.01 0.4" keyTimes="0; 0.25; 0.5; 0.75; 1" fill="freeze" repeatCount="indefinite"/>',
                        '<animate href="#F" attributeName="scale" dur="2s" values="40;6;8;6;8;" keyTimes="0; 0.25; 0.5; 0.75; 1" fill="freeze" repeatCount="indefinite"/>','</svg>'));
        }
        else
            parts[11]='</svg>';
