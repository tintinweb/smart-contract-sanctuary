...
contract PerpetualUpdateLogic is PerpetualUpdateFunctions, IPerpetualUpdateLogic, IFunctionList {
    function updateAMMTargetPoolSize(bytes32 _iPerpetualId) external override {
        PerpetualData storage perpetual = _getPerpetual(_iPerpetualId);
        _updateAMMTargetPoolSize(perpetual);
    }

    function updateFundingAndPricesBefore(uint256 _iPoolIdx) external override {
        uint256 iCurrentTime = block.timestamp;
        _accumulateFundingInPool(_iPoolIdx, iCurrentTime); //perpetualmanager. Was: updateFundingState
        _updateOraclePricesForPool(_iPoolIdx, iCurrentTime); //treasury
    }

    function updateFundingAndPricesAfter(uint256 _iPoolIdx) external override {
        uint256 iCurrentTime = block.timestamp;
        _updateFundingRatesInPool(_iPoolIdx); //perpetualmanager
        _updateTargetDefaultFundSize(_iPoolIdx, iCurrentTime); // treasury
    }

    function getFunctionList() external pure override returns (bytes4[] memory) {
        bytes4[] memory functionList = new bytes4[](3);
        functionList[0] = this.updateAMMTargetPoolSize.selector;
