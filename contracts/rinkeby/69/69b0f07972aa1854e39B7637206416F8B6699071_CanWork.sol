...
contract Escrow is Migratable {
     ...
     */
     ...
     function getPaprPrice() public view returns (uint256 paprPrice) {
       IOracle(paprOracle).consult(escrowToken, 1e18);
       return paprPrice;
    ...
    returns (uint256) {
        ...
        uint256 daiValueInEscrow1 = getPaprPrice();
        uint256 daiValueInEscrow = daiValueInEscrow1.mul(_amount); 
    ...
    returns (uint256) {
        ...
        uint256 totalPayoutCAN = totalPayoutCAN1.div(escrows[_escrowId].totalValueDai);
