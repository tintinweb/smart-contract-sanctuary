...
contract CryptoWrapped is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    /// @dev Can be changed by owner in `setMinimumDonationPrice`.
    uint256 public minimumDonationPrice = 0.01 ether;

    /// @notice Used for quick lookup of an address' donation amount.
    /// @dev Mapping from address to donation amount.
    mapping(address => uint256) public donations;

    /// @notice Used for retrieving list of all donators and donation amounts.
    /// @dev Array of all donator addresses.
    ...
    modifier newMinter() {
        require(balanceOf(msg.sender) == 0, "Only allowed to mint one NFT");
        _;
    }
    ...
    modifier newTokenURI(string memory _tokenURI) {
        require(
            ...
            "Token URI already associated with another token"
        );
        _;
     ...
     * by sending an amount >= minimumDonationPrice. Donators and donations are tracked
        ...
        returns (uint256)
    {
        ...
        if (msg.value > 0) {
            require(
                msg.value >= minimumDonationPrice,
                "Expected minimum donation amount of 0.01 ether"
            );
            isPremium = true;
        }
    ...
    /**
     * @return addresses of all donators
     */
    ...
    function withdraw() external onlyOwner nonReentrant {
        ...
        require(sent, "Owner failed to withdraw ETH from CryptoWrapped");
    }

    /**
     * @notice Used by owner to change minimumDonationPrice
     */
    function setMinimumDonationPrice(uint256 _minimumDonationPrice)
        external
        onlyOwner
        nonReentrant
    {
        minimumDonationPrice = _minimumDonationPrice;
    }
