...
contract Campaign is ReentrancyGuard, Pausable, TierSystem, Blacklist {
    ...
    function setOpenTimeAndRealeaseDateAndVerifyStartTime(
        ...
        require(
            _verifyStartTime >= block.timestamp,
            "ICO_CAMPAIGN::INVALID_VERIFY_START_TIME"
        );
        require(
            _verifyStartTime < _openTime.sub(500), // 1800
            "ICO_CAMPAIGN::INVALID_VERIFY_START_TIME"
        );
        require(
...
contract TierSystem is Ownable {
    ...
    function verifyStake() external {
        ...
        require(
            stakBank.balanceOf(_msgSender()) >= tier3,
    ...
    function isTiered(address _address) public view returns (bool tier) {
        if (block.timestamp >= campaignOpenTime.add(500)) { //1800
            tier = true;
        } else if (block.timestamp >= campaignOpenTime.add(450)) { //1200
            tier =
                userTier[_address].userTier >= 3 &&
                stakBank.balanceOf(_address) >= tier3;
        } else if (block.timestamp >= campaignOpenTime.add(400)) { //600
            tier =
                userTier[_address].userTier >= 2 &&
                stakBank.balanceOf(_address) >= tier2;
        } else if (block.timestamp >= campaignOpenTime) {
            tier =
    ...
    {
        if (userTier[_address].userTier == 1) {
            tier = 1;
            timestamp = campaignOpenTime;
        } else if (userTier[_address].userTier == 2) {
            tier = 2;
            timestamp = campaignOpenTime.add(400); // 500
        } else if (userTier[_address].userTier == 3) {
            tier = 3;
            timestamp = campaignOpenTime.add(450); //1200
        } else if (userTier[_address].userTier == 0) {
            tier = 0;
            timestamp = campaignOpenTime.add(500); //1800
        }
    }
    ...
    function _setupTierSystem(
        ...
        verifyEndTime = _verifyStartTime.add(500); //1800
