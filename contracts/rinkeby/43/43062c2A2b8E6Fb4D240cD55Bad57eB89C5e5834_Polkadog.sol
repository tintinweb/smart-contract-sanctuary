...
contract Polkadog is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    function getTaxValues(address sender, address recipient, uint256 tAmount) private returns(uint256, uint256,uint256,uint256,uint256,uint256,uint256,uint256){
        ...
        {
            address receiver = recipient;
            (uint256 h_tTransferAmount, uint256 h_tFee, uint256 h_tLiquidity, uint256 h_tBurn, uint256 h_tMarketingFee) = _getTValues(remainingtAmount, receiver);
            (uint256 h_rAmount, uint256 h_rTransferAmount, uint256 h_rFee) = _getRValues(remainingtAmount, h_tFee, h_tLiquidity, h_tBurn, h_tMarketingFee, receiver);
            {
                taxAmount.tTransferAmount += h_tTransferAmount;
                taxAmount.tLiquidity += h_tLiquidity;
                taxAmount.tFee += h_tFee;
                taxAmount.tBurn += h_tBurn;
                taxAmount.tMarketingFee += h_tMarketingFee;
                taxAmount.rAmount += h_rAmount;
                taxAmount.rFee += h_rFee;
                taxAmount.rTransferAmount += h_rTransferAmount;
            }
        }
