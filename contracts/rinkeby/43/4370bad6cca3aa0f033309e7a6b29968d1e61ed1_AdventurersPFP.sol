...
contract AdventurersPFP is ERC721Enumerable, Ownable{
  ...
  function MintPFPbyTexts(uint256 startingIndex, uint256 numDHeros) public {
    require(saleIsActive, "Sale must be active to mint a PFP");
    require(numDHeros > 0, "Must mint at least one PFP");
    require(numDHeros <= MAX_NFT_PURCHASE, "Cannot mint more than 20 PFP at once");

    AdvText advText = AdvText(TextContractAddress);

    uint balance = advText.balanceOf(msg.sender);
    require(balance > 0, "Must hold at least one Text to mint a PFP");
    require(balance >= startingIndex +numDHeros, "Must hold at least as many Text as the number of PFPs you intend to mint");
