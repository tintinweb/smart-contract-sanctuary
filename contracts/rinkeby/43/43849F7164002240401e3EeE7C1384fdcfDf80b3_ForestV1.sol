...
contract ForestV1 is Ownable, IERC721Receiver, ReentrancyGuard, Initializable {
    ...
    mapping(uint256 => uint256) public commonIndices;
    mapping(address => Stake[]) public commonStake;

    uint public totalHarvesterStaked;
    uint public totalHunterStaked;
    uint public totalCommonStaked;

    uint public constant MINIMUM_TIME_TO_EXIT = 25 minutes;
    ...
    mapping(uint16 => bool) public isGenesis;
    uint256 public genesisReward = 10000 ether;
    uint256 public commonReward = 100 ether;
    bool public commonAllowed;
    ...
    event CommonClaimed(uint16 tokenId, uint earned, bool unstaked);
    ...
    function setGenesis(uint16[] calldata ids, bool value) external onlyOwner {
        for (uint256 i = 0; i < ids.length; i++) {
            isGenesis[ids[i]] = value;
        }
    }

    function setGenesisReward(uint256 reward) external onlyOwner {
        genesisReward = reward;
    }

    function setCommonReward(uint256 reward) external onlyOwner {
        commonReward = reward;
    }

    function setCommonAllowed(bool status) external onlyOwner {
        commonAllowed = status;
    }
    ...
    function getAccountCommons(address user) external view returns (Stake[] memory) {
        return commonStake[user];
    }
    ...
    function addTokensToStake(address account, uint16[] calldata tokenIds) external whenNotPaused {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            if (!isGenesis[tokenIds[i]]) {
                require(commonAllowed, "Common stake not allowed");
                _stakeCommon(account, tokenIds[i]);
            } else if (metaMoose.isHunter(tokenIds[i])) {
    ...
    function _stakeCommon(address account, uint16 tokenId) internal {
        totalCommonStaked += 1;

        commonIndices[tokenId] = commonStake[account].length;
        commonStake[account].push(Stake({
            tokenId: uint16(tokenId),
            value: block.timestamp
        }));
    ...
    function claimFromStake(uint16[] calldata tokenIds, bool unstake) external whenNotPaused nonReentrant {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            if (!isGenesis[tokenIds[i]]) {
                owed += _claimFromCommon(tokenIds[i], unstake);
            } else if (!metaMoose.isHunter(tokenIds[i])) {
    ...
    function _claimFromHarvester(uint16 tokenId, bool unstake) internal returns (uint owed) {
        ...
        if (stake.value >= stakingEndTimestamp) {
            ...
            owed = ((block.timestamp - stake.value) * genesisReward) / 5 minutes;
        } else {
            owed = ((stakingEndTimestamp - stake.value) * genesisReward) / 5 minutes;
        }
        if (unstake) {
    ...
    function _claimFromCommon(uint16 tokenId, bool unstake) internal returns (uint owed) {
        Stake memory stake = commonStake[msg.sender][commonIndices[tokenId]];
        require(stake.tokenId == tokenId, "This NTF does not belong to address");
        require(!(unstake && block.timestamp - stake.value < MINIMUM_TIME_TO_EXIT), "Need to wait more since last claim");
        ...
        if (stake.value >= stakingEndTimestamp) {
            ...
            owed = ((block.timestamp - stake.value) * commonReward) / 1 days;
        } else {
            owed = ((stakingEndTimestamp - stake.value) * commonReward) / 1 days;
        }
        if (unstake) {
            totalCommonStaked -= 1;

            Stake memory lastStake = commonStake[msg.sender][commonStake[msg.sender].length - 1];
            commonStake[msg.sender][commonIndices[tokenId]] = lastStake;
            commonIndices[lastStake.tokenId] = commonIndices[tokenId];
            commonStake[msg.sender].pop();
            delete commonIndices[tokenId];

            metaMoose.safeTransferFrom(address(this), msg.sender, tokenId, "");
        } else {
            uint80 timestamp = uint80(block.timestamp);

            commonStake[msg.sender][commonIndices[tokenId]] = Stake({
                tokenId: uint16(tokenId),
                value: timestamp
            }); // reset stake
        }

        emit CommonClaimed(tokenId, owed, unstake);
    }
    ...
    function rescue(uint16[] calldata tokenIds) external nonReentrant {
        ...
        for (uint16 i = 0; i < tokenIds.length; i++) {
            ...
            if (!isGenesis[tokenId]) {
                stake = commonStake[msg.sender][commonIndices[tokenId]];
                ...
                emit CommonClaimed(tokenId, 0, true);
            } else if (!metaMoose.isHunter(tokenId)) {
