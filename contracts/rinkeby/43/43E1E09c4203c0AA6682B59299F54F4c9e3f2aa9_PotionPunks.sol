...
contract PotionPunks is ERC721URIStorage, Ownable {
    ...
    address public apeAddress = 0x15d98E8762B2183D8Ad313DE3cb247C9064Dfd5D;
    ...
    function apeHolderFreeMint() payable public apeHolderEnabled {
        if (apeHolderMintCount[msg.sender]==0)  {
            apeHolderMintCount[msg.sender] = 1;
        }
        require(apeHolderMintCount[msg.sender] == 0, "Already claimed");
        require(apeContract.balanceOf(msg.sender) > 0, "Must own a Genesis ape");
        for (uint256 i = 1; i <= apeContract.balanceOf(msg.sender); i++) {
          _mint(_msgSender(), 1 + totalPotionPunks++);
        }
        
    }
 
    function mintPotionPunk(uint256 _times) payable public mintEnabled {
       
        require(_times >0 && _times <= maxBatch, "mint wrong number");
        require(totalPotionPunks + _times <= totalCount, "too much");
        require(msg.value == _times * price, "value error");
        payable(owner()).transfer(msg.value);
        emit MintPotionPunk(_msgSender(), totalPotionPunks+1, _times);
