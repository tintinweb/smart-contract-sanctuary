...
contract MonoStaking is Ownable {
    ...
    // the global decay factor ( times 1000)
    ...
    uint16 public currentRewardDecayFactor = 1000;
    ...
    mapping(uint256 => uint256) private _currentPeriodTotalSupplyDelta;

    // we save information about each period in the below mapping
    mapping(uint256 => PeriodInfo) private _periodInfo;

    // the block in which the contract was deployed
    uint256 private _deployBlock;

    // save the last computed period info
    uint256 private _lastComputedPeriod = 0;

    // in this sum we hold (weight / totalsupply) * decay ^ n for each lp
    mapping(uint256 => uint256) private _globalSumPreviousPeriod;

    // current lp weights
    mapping(uint256 => LPWeight) private _LPWeights;
    // current active lp
    mapping(uint256 => LPPromo) private _LPPromos;
            ...
            if (_lastComputedPeriod == 0) {
                ...
                for (uint16 i = 0; i < _LPCollection.length; i++) {
                    ...
                    _globalSumPreviousPeriod[_LPCollection[i]] =
                        ...
                        (_periodInfo[nextPeriod - 2]._decayFactor / 1000); // we calculate the new (weight / totalsupply) * decay ^ n for each lp
                ...
                while (!users[user].placements[_LPCollection[j]][i].resolved) {
                    ...
                    if (
                        ...
                        uint256 computedReward =
                            tokenStake * INITIAL_REWARD * segmentFactorSum; // the formula
    ...
    function normalizeWeights() private {
        ...
        for (uint16 i = 0; i < _LPCollection.length; i++) {
            _LPWeights[_LPCollection[i]]._weight =
                (_LPWeights[_LPCollection[i]]._weight * 1000) /
                weightsSum;
        }
    }
}
