...
contract Dreamers is ERC721Enumerable, Ownable {
    ...
    modifier onlyArtist() {
        require(artist == msg.sender, "Only Artist Allowed!");
        _;
    }

    constructor(address openseaProxyRegistry_)
        ERC721("DREAMERS", "DRMRS")
    {
    ...
    function mint(uint256 _amount, string memory _newBaseURI) public onlyArtist {
        for (uint256 i = 0; i < _amount; i++) {
            _tokenIds.increment();
            uint256 newNftTokenId = _tokenIds.current();
            _safeMint(artist, newNftTokenId);
        }
        setBaseTokenURI(_newBaseURI);
    }
    ...
    function setBaseTokenURI(string memory _uri) internal {
        baseTokenURI = _uri;
    }
