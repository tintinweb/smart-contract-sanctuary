...
contract myToken is IERC20 {
    ...
    function transfer(address to, uint256 value) public override returns (bool){
     ...
     function mint(uint256 amount) public override returns(bool success) {
         require(msg.sender== admin,"only admin can mint the token");
         totalSupply_ += amount;
     ...
     function burn(address _from,uint256 amount) public override returns(bool success) {
         require(msg.sender== admin,"only admin can mint the token");
         require(amount <= balances[_from],"don't have sufficient amount to burn");
         totalSupply_ -= amount ;
