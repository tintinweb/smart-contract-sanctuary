...
contract sAbachi is IsABI, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgABI public gABI; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked ABI", "sABI", 9) ERC20Permit("Staked ABI") {
    ...
    function setgABI(address _gABI) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gABI) == address(0), "gABI:  gABI already set");
        require(_gABI != address(0), "gABI:  gABI is not a valid contract");
        gABI = IgABI(_gABI);
    }
    ...
    /**
        @notice increases rABI supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sABI of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sABI
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sABI: insufficient balance");
    }
    ...
    // toG converts an sABI balance to gABI terms. gABI is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gABI.balanceTo(amount);
    }

    // fromG converts a gABI balance to sABI terms. sABI is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gABI.balanceFrom(amount);
    }

    // Staking contract holds excess sABI
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gABI.balanceFrom(IERC20(address(gABI)).totalSupply())).add(
