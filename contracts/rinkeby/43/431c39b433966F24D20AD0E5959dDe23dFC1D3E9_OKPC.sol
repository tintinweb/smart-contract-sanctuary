...
contract OKPC is Insightful721, IERC2981, ReentrancyGuard {
  ...
  function _createArt(Art memory artwork) private {
    ...
    if (bytes(artwork.title).length > 14) revert InvalidArtData();
  ...
  function collectArt(
    ...
    uint256 artId,
    bool makeActive
  ) public payable nonReentrant onlyWhenArtExists(artId) {
    ...
    if (_art.artist != msg.sender && msg.value != COLLECT_PRICE)
    ...
    returns (string memory)
  {
    ...
    return
      IOKPCMetadata(metadataAddress).tokenURI(tokenID, insight(tokenID), data);
  }
