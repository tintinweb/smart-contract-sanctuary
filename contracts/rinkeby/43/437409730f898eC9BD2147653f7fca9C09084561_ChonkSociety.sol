...
contract ChonkSociety is ERC721Enumerable, Ownable {
  ...
  function mint(uint256 _mintAmount) external payable {
    ...
    require(_mintAmount <= maxMintAmount, "Exceed max per mint");
    ...
    require(cost * _mintAmount <= msg.value, "Insufficient Eth");
  ...
  function whitelistMint(uint256 _mintAmount) external payable {
    ...
    require(isWhitelisted(msg.sender), "User not on whitelist.");
    require(_mintAmount <= maxMintAmount, "Exceed max per mint");
  ...
  //checks if user is whitelisted, 'view' ensures read-only
  function isWhitelisted(address _user) public view returns(bool) {
    for(uint256 i = 0; i < whitelisted.length; i++) {
      if(_user == whitelisted[i]) {
        return true;
      }
    }
    return false;
