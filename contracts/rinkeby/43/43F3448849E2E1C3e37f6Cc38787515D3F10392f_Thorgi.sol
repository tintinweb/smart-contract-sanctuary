...
contract Thorgi is ERC20Burnable, Ownable {
    ...
    function stakeBones(uint256[] memory tokenIds) public stakingEnabled {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 id = tokenIds[i];
            require(pupIERC721Enumerable.ownerOf(id) == msg.sender && corgiOwner[id] == address(0), "TOKEN_IS_NOT_YOURS");
            pupIERC721Enumerable.transferFrom(msg.sender, address(this), id);
    ...
    function mintNow(uint256 number) public onlyOwner {
    ...
    function unstakeAll() public {
        ...
        for (uint256 i = corgiTokenIds[msg.sender].length; i > 0; i--) {
            ...
            corgiTokenIds[msg.sender].pop();
            corgiOwner[tokenId] = address(0);
        }
        
        for (uint256 i = pupTokenIds[msg.sender].length; i > 0; i--) {
            uint256 tokenId = pupTokenIds[msg.sender][i - 1];

            pupIERC721Enumerable.transferFrom(address(this), msg.sender, tokenId);
            totalRewards += ((block.timestamp - pupTimeStaked[tokenId]) * pupRatePerDay);
            pupTokenIds[msg.sender].pop();
            pupOwner[tokenId] = address(0);
        }
    ...
    function claimFromCorgi(uint256 tokenId) public {
        ...
        _mint(msg.sender, ((block.timestamp - corgTimeStaked[tokenId]) * corgiRatePerDay));
    ...
    function claimFromPup(uint256 tokenId) public {
        ...
        _mint(msg.sender, ((block.timestamp - pupTimeStaked[tokenId]) * pupRatePerDay));
    ...
    function claimAll() public {
        ...
        uint256[] memory vampTokenIds = corgiTokenIds[msg.sender];
        for (uint256 i = 0; i < vampTokenIds.length; i++) {
            uint256 id = vampTokenIds[i];
            require(corgiOwner[id] == msg.sender, "NOT_BURIED_BY_YOU");
            totalRewards += ((block.timestamp - corgTimeStaked[id]) * corgiRatePerDay);
            corgTimeStaked[id] = block.timestamp;
        }
        
        uint256[] memory batTokenIds = pupTokenIds[msg.sender];
        for (uint256 i = 0; i < batTokenIds.length; i++) {
            uint256 id = batTokenIds[i];
            require(pupOwner[id] == msg.sender, "NOT_BURIED_BY_YOU");
            totalRewards += ((block.timestamp - pupTimeStaked[id]) * pupRatePerDay);
            pupTimeStaked[id] = block.timestamp;
        }
    ...
    function checkRewardsFromCorgis(uint256 tokenId) public view returns (uint256) {
        ...
        uint256 secondsStaked = block.timestamp - corgTimeStaked[tokenId];
        return secondsStaked * corgiRatePerDay;
    }
    ...
    function checkRewardsFromPups(uint256 tokenId) public view returns (uint256) {
        ...
        uint256 secondsStaked = block.timestamp - pupTimeStaked[tokenId];
        return secondsStaked * pupRatePerDay;
    }

    function checkAllRewards(address burier) public view returns (uint256) {
        uint256 totalRewards = 0;

        uint256[] memory vampTokenIds = corgiTokenIds[burier];
        for (uint256 i = 0; i < vampTokenIds.length; i++) {
            totalRewards += ((block.timestamp - corgTimeStaked[vampTokenIds[i]]) * corgiRatePerDay);
        }
        
        uint256[] memory batTokenIds = pupTokenIds[burier];
        for (uint256 i = 0; i < batTokenIds.length; i++) {
            totalRewards += ((block.timestamp - pupTimeStaked[batTokenIds[i]]) * pupRatePerDay);
        }
