...
contract PotionLabV2 is ERC1155, Ownable {
    ...
    function getIds(uint256 amount) private returns (uint256[] memory) {
        uint256[] memory ids;
        while (amount > 0) {
            amount--;
            ids[amount] = currentId--;
        }
        return ids;
    }
    ...
    function mint(uint256 amount) external payable {
        ...
        _mintBatch(
            msg.sender,
            getIds(amount),
            ArrayUtils.getFilledArray(amount, 1),
            ""
        );
    }
}
