...
contract solidSwing is owned {
    ...
    event check(uint diff,uint benchMarkValue,uint base,  uint reBaseFactor, uint calc, uint curentPrice);
    function snapShot(uint _spotPrice) public onlySigner returns(bool)
    {
      require(!safeguard, "safe gurard active");
      require(now > snapTime + oneDay, "24 Hr. not reached");
      uint diff;
      if(_spotPrice >= benchMarkValue)
      {
        diff = _spotPrice - benchMarkValue;
        diff = diff * base / benchMarkValue;
        reBaseFactor = benchMarkValue + (benchMarkValue * diff / base);
        currentPrice = benchMarkValue - (benchMarkValue * diff / base);
        emit check(diff,benchMarkValue,base,reBaseFactor,benchMarkValue * diff / base,currentPrice);
      }
      else
      {
        diff =  benchMarkValue - _spotPrice;
        diff = diff * base / benchMarkValue;
        reBaseFactor = benchMarkValue - (benchMarkValue * diff / base);
        currentPrice = benchMarkValue + (benchMarkValue * diff / base);
        emit check(diff,benchMarkValue,base,reBaseFactor,benchMarkValue * diff / base,currentPrice);
