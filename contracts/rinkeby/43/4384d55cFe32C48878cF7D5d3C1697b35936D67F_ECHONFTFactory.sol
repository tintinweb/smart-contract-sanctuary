...
import "./lib/FactoryERC1155Base.sol";

/**
 * @title Creature
 * Creature - a contract for my non-fungible creatures.
 */
contract ECHONFTFactory is FactoryERC1155Base {
    constructor(
        address _proxyRegistryAddress,
        address nftAddress
    )
        FactoryERC1155Base(_proxyRegistryAddress, nftAddress)
    {}
 ...
 * tokens.
 */
contract FactoryERC1155Base is FactoryERC1155, Ownable, ReentrancyGuard {
    using Strings for string;
    using SafeMath for uint256;
    ...
    function changeProxyRegistryAddress(address _proxyRegistryAddress) public onlyOwner {
        // require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), 'Caller is not admin');
        proxyRegistryAddress = _proxyRegistryAddress;
    }
    ...
    function _mint(
        ...
        if (_option < NUM_ITEM_OPTIONS) {
            ...
            // declare token
            ERC1155Tradable _NFTtoken = ERC1155Tradable(nftAddress);

            uint256 _baseType =  _NFTtoken.getCurrentRotationBase(_option);

            // Option is used as a token ID here
            _NFTtoken.mint(_toAddress, _baseType, _data);
    ...
    function _createOrMint(
        ...
        if (!tradable.exists(_id)) {
            ...
            _toArray[0] = _to;
            tradable.createManyForType(_toArray, _type, _data);
        } else {
            tradable.mint(_to, _type, _data);
        }
    }
        ...
        returns (uint256)
    {
        ...
        //     uint256 currentSupply = 0; //lootBox.balanceOf(owner(), _optionId);
        ...
        //     return SUPPLY_PER_TOKEN_ID.sub(0);
        // }
        return 1000000;
    }
