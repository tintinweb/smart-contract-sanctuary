...
contract Bridge{
    ...
    function doubleSendStats(uint256 ID, uint256 ID1) external view returns(address sender, uint256 amount, uint256 fee, address tokenAddressFrom, address sender1, uint256 amount1, uint256 fee1, address tokenAddressFrom1){
        sender = _sendSender[ID];
        amount = _sendAmount[ID];
        fee = _sendFee[ID];
        tokenAddressFrom = _sendTokenAddressFrom[ID];
