...
contract infinityTokensPublic is ERC721, OwnableClone, HasSecondarySaleFees, Saleable {
    ...
    function getMetadata(uint256 tokenId) public view returns (uint256 NFTRef, string memory name, Attributes[] memory attributes, uint256 editionSize, uint256 editionNumber, uint256 evolutionLevel, uint256 remainingRedemptions, bool isActive) {		require(_exists(tokenId), "Token does not exist.");
	...
	function NFTMouldMetadata(uint256 NFTId) public view returns (string memory name, Attributes[] memory attributes, uint256 editionSize, uint256 evolutionLevel, bool isActive) {
