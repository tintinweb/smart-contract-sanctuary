...
contract ZenCatsFactory is FactoryERC721, Ownable {
    ...
    string public baseURI = "https://dev.zencats.io/api/factory/";

    /*
     * Enforce the existence of only 100 OpenSea creatures.
     */
    uint256 ZENCATS_SUPPLY = 100;

    /*
     * Three different options for minting Creatures (basic, premium, and gold).
     */
    uint256 NUM_OPTIONS = 2;
    uint256 SINGLE_ZENCAT_OPTION = 0;
    uint256 MULTIPLE_ZENCATS_OPTION = 1;

    uint256 NUM_ZENCATS_IN_MULTIPLE_ZENCATS_OPTION = 4;
    ...
    function mint(uint256 _optionId, address _toAddress) override public {
        ...
        if (_optionId == SINGLE_ZENCAT_OPTION) {
            zencatContract.mintTo(_toAddress);
        } else if (_optionId == MULTIPLE_ZENCATS_OPTION ) {
            for (
                uint256 i = 0;
                i < NUM_ZENCATS_IN_MULTIPLE_ZENCATS_OPTION;
                i++
            ) {
                zencatContract.mintTo(_toAddress);
            }
    ...
    function canMint(uint256 _optionId) override public view returns (bool) {
        ...
        if (_optionId == SINGLE_ZENCAT_OPTION) {
            ...
            numItemsAllocated = NUM_ZENCATS_IN_MULTIPLE_ZENCATS_OPTION;
        } 
        return zencatSupply < (ZENCATS_SUPPLY - numItemsAllocated);
    }
