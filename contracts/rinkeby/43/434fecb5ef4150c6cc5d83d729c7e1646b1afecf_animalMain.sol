...
contract ERC721 {
  ...
  function balanceOf(address _owner) public view returns (uint256 _balance); //_owner有多少資產
  function ownerOf(uint256 _tokenId) public view returns (address _owner); //輸入tokenid可以知道持有者是誰
  function transfer(address _to, uint256 _tokenId) public; //給指定對象token,但一次只有一個token
...
contract animalMain is  ERC721,Ownable {
  ...
  function transfer(address _to, uint256 _tokenId) public {
    ...
    require(animalToOwner[_tokenId] == _to);
    //增加受贈者的擁有動物數量
    ownerAnimalCount[_to]++;
    //減少轉出者的擁有動物數量
    ownerAnimalCount[animalToOwner[_tokenId]]--;
    //動物所有權轉移
