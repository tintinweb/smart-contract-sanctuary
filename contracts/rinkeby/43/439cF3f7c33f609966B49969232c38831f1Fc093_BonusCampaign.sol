...
contract BonusCampaign is StakingRewards {
    ...
    function stake(uint256 amount) external override nonReentrant whenNotPaused updateReward(msg.sender) {
        revert("Standard staking is not allowed.");
    }

    function withdraw(uint256 amount) public override nonReentrant updateReward(msg.sender) {
        revert("Standard withdraw is not allowed.");
    }

    function exit() external override {
        revert("Standard exit is not allowed.");
    }

    function notifyRewardAmount(uint256 reward) external override onlyRewardsDistribution updateReward(address(0)) {
        revert("Standard notify reward amount is not allowed.");
    }
