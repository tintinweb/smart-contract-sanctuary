...
contract ChrL2 {
    ...
    function withdraw(bytes32 _hash, address payable beneficiary) public {
        Withdraw storage wd = _withdraw[_hash];
        require(wd.beneficiary == beneficiary, "ChrL2: there's no fund for the beneficiary");
        require(wd.block_number <= block.number, "ChrL2: it's not mature enough to withdraw the fund");
        require(wd.isWithdraw == false, "ChrL2: fund was already claimed");
        require(wd.amount > 0 && wd.amount <= _balances[beneficiary][wd.token], "ChrL2: it's not enough amount to withdraw");
        wd.isWithdraw = true;
