...
contract StakingToken is Ownable, ERC20Pausable, Blacklist {
    ...
    uint256 constant totalFreeSupply = 100000000;
    uint8 	constant numDecimals = 4;
    uint256 constant stakingAmount = 1000;
    uint256 constant numPhases = 3;
    uint256 constant baseReward = 10;
    
    uint256 [numPhases] phases = [1625387050, 1628065450, 1630743850];
    uint256 lastRewardStamp;
    
    /*
    phase0 : 1625387050 04.07.2021 08:24 UTC 30%
    phase1 : 1628065450 04.08.2021 08:24 UTC 20%
    phase2 : 1630743850 04.09.2021 08:24 UTC 10%
    */
    ...
    constructor() Ownable() ERC20('StakingToken','STT') public
    {
    ...
    function createSingleStake(uint256 _stake) public
    {
		...
		function getCurrentRate(uint256 tStamp) public view returns(uint256)
		{
		   
			assert(tStamp > phases[0]);
			 
			uint256 phase = 1;
			while ((phases[phase] < tStamp) && (phase < numPhases)) 
				phase.add(1);
			
			return SafeMath.mul(baseReward, numPhases.sub(phase.sub(1)));
		}

		function distributeDailyRewards() public
		{
			require(block.timestamp > (lastRewardStamp + 3600*24), "Can be called only once per day");
			uint256 currentRate = getCurrentRate(block.timestamp);
			
			for (uint256 s = 0; s < stakeholders.length; s++)
				rewards[stakeholders[s]] = rewards[stakeholders[s]].add(SafeMath.mul(SafeMath.div(stakes[stakeholders[s]],100),currentRate));
