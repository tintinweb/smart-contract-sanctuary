...
contract Kaiba is Context, IERC20, IERC20Metadata {
    ...
    bool public dynamic_locks;
    mapping (address => bool) public is_offender;
    mapping (address => uint) public offender_lock;

    uint256 sell_limit_base = (circulating_supply * 1) / 100;
    uint256 tx_limit_base = (circulating_supply * 2) / 100;

    uint256 wallet_limit = (circulating_supply * 3) / 100;
    ...
    function _swapTaxes(uint256 all_taxes) private {
        ...
        marketing_eth = (gain*_marketingFee) / 10;
        growth_eth = (gain*_growthFee) / 10;
        treasury_eth = (gain*_treasuryFee) / 10;
        if (!pairedLiq) {            
         liquidity_eth = (gain*_liquidityFee) / 10;
        }
