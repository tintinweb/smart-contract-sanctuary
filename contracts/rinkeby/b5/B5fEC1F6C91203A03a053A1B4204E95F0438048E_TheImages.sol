...
contract TheImages is ERC721Enumerable, Ownable {
    ...
    function mint(uint256 numberOfTokens) external payable {
        ...
        require(numberOfTokens <= maxToMint, "Can only mint up to 20 tokens");
        require(_publicImages.current() < maxSupply, "Purchase would exceed DRAGONS_PUBLIC");
        require(mintPrice * numberOfTokens <= msg.value, "ETH amount is not sufficient");

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicImages.current();

            if (_publicImages.current() < maxSupply) {
