...
contract Data {
  ...
  function _buildQueueLimit(uint256 _realmId) public returns (uint256) {
    if (queueMod[_realmId] == 0) {
      queueMod[_realmId] = 25;
    }

    uint256 remainder = data[_realmId][2] % queueMod[_realmId];

    if (remainder == 0 && data[_realmId][2] != 0) {
      queueLimit[_realmId]++;

      queueMod[_realmId] += 25;
    }

    return queueLimit[_realmId];
  }
