...
// File contracts/SantiPunksDNA.sol


pragma solidity ^0.8.0;

contract SantiPunksDNA {
    ...
    // This pseudo random function is deterministic a don't be used in production
    ...
    function _getDNASection (uint256 _dna, uint _rightDiscard) 
        internal 
        pure
        returns (uint8)
    {
        return uint8(
            (_dna % (1 * 100 ** (_rightDiscard + ADN_SECTION_SIZE))) / 
            (1 * 10 ** _rightDiscard)
        );
    }
...
contract SantiPunks is ERC721, ERC721Enumerable, SantiPunksDNA {
    ...
    constructor(uint256 _maxSupply) ERC721("SantiPunks", "SPKS") {
        maxSupply = _maxSupply;
    }

    function mint() public {
        uint256 current = _idCounter.current();
        require(current < maxSupply, "No SantiPunks left");
