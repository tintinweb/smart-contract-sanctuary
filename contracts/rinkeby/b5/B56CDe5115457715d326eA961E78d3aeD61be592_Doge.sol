 ...
 * IDoge: game utility of each Doge + ChadVirgin struct
 */
...
interface IDoge {
    // Struct that stores each Doge's traits
    struct ChadVirgin {
        bool isVirgin;
        uint8 hat;
        uint8 eyes;
...
contract Doge is IDoge, ERC721Enumerable, Ownable, Pausable {
    ...
    uint256 public constant MINT_PRICE = 0.042069 ether; // Mint price
    ...
    mapping(uint256 => ChadVirgin) private tokenTraits;       // Mapping from tokenId to a struct containing the token's traits
    mapping(uint256 => uint256) private existingCombinations; // Mapping from hashed(tokenTrait) to tokenId to ensure no duplicates

    
    // 0 - 9 are associated with Chads, 10 - 18 are associated with Virgins
    ...
    IMemeverse public memeverse; // Reference to the Memeverse for choosing random Chad thieves
    MEME public meme;            // Reference to $MEME for burning
    ITraits public traits;     

    uint256 public reserveLimit = 500;

    /**
     * Instantiate Doge contract and rarity registries
     */
    constructor(address _meme, address _traits, uint256 _maxTokens) ERC721("Doge Game", "DOGEGAME") {
        meme = MEME(_meme);
    ...
    /**
     * Minting function - 90% Virgins, 10% Chads
     * The first 20% are free to claim, the remaining cost $MEME
    ...
    function mint(uint256 amount) external payable whenNotPaused {
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            totalMemeCost += mintCost(minted);
            _safeMint(recipient, minted);
        }
        if (totalMemeCost > 0) meme.burn(_msgSender(), totalMemeCost);
    }

    /**
     * the first 20% are paid in ETH
     * the next 20% are 20000 $MEME
     * the next 40% are 40000 $MEME
     * the final 20% are 80000 $MEME
     ...
     */
    function generate(uint256 tokenId, uint256 seed) internal returns (ChadVirgin memory t) {
      t = selectTraits(seed);
     ...
     * the remaining 80% have a 10% chance to be given to a random staked Chad
     * @param seed: a random value to select a recipient from
     * @return the address of the recipient (either the minter or the Chad thief's owner)
     */
    function selectRecipient(uint256 seed) internal view returns (address) {
      ...
      address thief = memeverse.randomChadOwner(seed >> 144);
     ...
     */
    function selectTraits(uint256 seed)internal view returns (ChadVirgin memory t){
        t.isVirgin = (seed & 0xFFFF) % 10 != 0;
        uint8 shift = t.isVirgin ? 0 : 6;
        seed >>= 16;
     ...
     */
    function structToHash(ChadVirgin memory s) internal pure returns (uint256) {
        return
            uint256(
                sha256(
                    abi.encodePacked(
    ...
    */
     ...
     */
    function getTokenTraits(uint256 tokenId) external view override returns (ChadVirgin memory) {
        // To prevent people from minting only Chads,
    ...
    /**
     * Called after deployment so that the contract can get random Chad thieves
        ...
        returns (string memory)
    {
        ...
        // to prevent people from minting only Chads. We reveal the minted batch 
...
contract MEME is ERC20, Ownable {
    ...
    /**
     * Mints $MEME to a recipient
     * @param to: the recipient of the $MEME
     * @param amount: the amount of $MEME to mint
     */
    ...
    /**
     * Burns $MEME from a holder
     * @param from: the holder of the $MEME
     * @param amount: the amount of $MEME to burn
     */
