...
contract DevContract is ERC721Enumerable, Ownable, ReentrancyGuard {
  uint256 private constant MAX_KANJI = 10000;
  uint256 private constant KANJI_PER_TX = 50;
  uint256 private constant KANJI_PRICE = 0.01 ether;

  string public KANJI_PROVENANCE;
  bool private _isPublicSaleActive = false;
  bool private _isPreSaleActive = false;
  string private _baseTokenURI;

  uint256 CHARITY_SHARES = 20;
  uint256 DEV_SHARES = 80;
  address CHARITY_ADDRESS = 0x667143035BAF701A6A5E7333C5D1980d72092040;
  ...
  function withdraw() public onlyOwner {
    ...
    uint256 ownerShare = (totalBalance * DEV_SHARES) / 100;
    require(
      payable(CHARITY_ADDRESS).send(charityShare),
      "Unable to send funds to charity."
    );
    require(
      payable(msg.sender).send(ownerShare),
  ...
  /*
   * A SHA256 hash representing all 8000 Kanji. Will be set once all 8k are born
   */
  function setProvenanceHash(string memory provenanceHash) public onlyOwner {
    KANJI_PROVENANCE = provenanceHash;
  }
  ...
  function mintPublic(uint256 amount) external payable nonReentrant() {
    ...
    require(
      amount > 0 && amount <= KANJI_PER_TX,
      "You can't mint that many kanji"
    );
    require(
      totalSupply() + amount <= MAX_KANJI,
      "Mint would exceed max supply of kanji"
    );
    require(
      msg.value == amount * KANJI_PRICE,
  ...
  /*
   * Sets aside some kanji for the dev team, used for competitions, giveaways and mods memberships
   */
