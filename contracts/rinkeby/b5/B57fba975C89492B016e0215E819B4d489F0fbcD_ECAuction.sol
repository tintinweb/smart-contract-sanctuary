...
contract ECAuction is Ownable {
    ...
    function bid() external payable isActive() {
        ...
        BidStruct storage olderstBid = bid_history[bid_count];

        require(
            msg.value >= olderstBid.value.add(price_increment),
        ...
        if(bid_count > 1) {
            // return old bild value
            payable(olderstBid.sender).transfer(olderstBid.value);
        }
    }
    ...
    function processAuction() external hasEnded notProcessed {

        require( NFTContract.sale_is_over(), "Not until main sale is over");

        processed = true;
        BidStruct storage olderstBid = bid_history[bid_count];
        NFTContract.safeTransferFrom(address(this), olderstBid.sender, 1);
        ...
        AuctionWinningAmountPaid(olderstBid.sender, olderstBid.value);
    }
