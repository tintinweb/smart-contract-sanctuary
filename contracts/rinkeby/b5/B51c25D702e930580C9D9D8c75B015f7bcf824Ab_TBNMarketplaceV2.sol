...
contract TBNMarketplaceV2 is ERC721Holder, AccessControl, ReentrancyGuard {
    ...
    struct Listing {
        ...
        uint256 reservePrice; // min price listing is willing to  sell for
        bool isPriceBound; // if true then price is bound to marketplace
    }

    mapping(uint256 => Listing) public listings; // why is this not an array
    // Get all TBNS for an address
  ...
  */
    function updateCharityWalletAddress(address payable _charityWallet)
        external
        onlyAdmin
    {
        charityWallet = _charityWallet;
  ...
  */
    ...
    @param reservePrice Min amount willing to sell for
    @param priceBound true if price is bound to market price
  */
    /*  
        add param - bool for priceBound
            if above true then paymentAmount should be basis point above or below 10,000
            ie. 9,000 = 90%, 15,000 = 150%, 500,000 = 500%
        add param - uint256 for reservePrice
    ...
    */
    function listTBNTokens(
        ...
        require(
            (basisPointFee + charityBasisPoint) <= 10000,
            "Your charity basisPoint is too high"
        );
  ...
  */
    function buyToken(uint256 listingId) external payable nonReentrant {
        ...
        /*
          ...
          listing.paymentAmount would be basisPoints compared to market price from oracle
          get percentage with listing.paymentAmount.mul(${oraclePrice}).div(10000)
        ...
        */
        ...
        ISimpleOracle currOracle = ISimpleOracle(oracle);

        uint256 fullCost;
        if (listing.isPriceBound) {
            /** Calculation of price
                get reserves of tbn and payment tokens
                tbnReservePrice = WETH reserve / tbnReserve 
                paymentReservePrice = paymentReserve / WETH reserve
                exchangeRate = tbnReservePrice * paymentReservePrice
                fullPrice = exchangeRate * # of tokens in TBN
            */
            (address TBNAddress, uint256 TBNAmount) = token.nftsToTokenData(
                listing.nftTokenId
            );
            (uint256 tbnWETHReserves, uint256 tbnTokenReserves) = currOracle
                .getReservesForTokenPool(TBNAddress);
            (
                uint256 paymentWETHReserves,
                uint256 paymentTokenReserves
            ) = currOracle.getReservesForTokenPool(listing.paymentTokenAddress);
            PriceCalculation memory priceCalc;

            priceCalc.tbnReservePrice = tbnWETHReserves
                .mul(1000000000000000000)
                .div(tbnTokenReserves);

            priceCalc.paymentReservePrice = paymentTokenReserves
                .mul(1000000000000000000)
                .div(paymentWETHReserves);

            priceCalc.exchangePrice = priceCalc.tbnReservePrice.mul(
                priceCalc.paymentReservePrice
            );

            priceCalc.oraclePrice = priceCalc.exchangePrice.mul(TBNAmount).div(
                (1000000000000000000000000000000000000)
            );

            if (priceCalc.oraclePrice > listing.reservePrice) {
                fullCost = listing.paymentAmount.mul(priceCalc.oraclePrice).div(
                        10000
                    );
            } else {
                fullCost = listing.paymentAmount.mul(listing.reservePrice).div(
                    10000
                );
            }
        } else {
            fullCost = listing.paymentAmount;
        }

        address paymentTokenAddress = listing.paymentTokenAddress;

        uint256 payoutToSeller = (
            fullCost.mul((10000 - basisPointFee - listing.charityBasisPoint))
        ).div(10000);
        uint256 payoutToTreasury = (fullCost.mul(basisPointFee)).div(10000);
        uint256 payoutToCharity = (fullCost.mul((listing.charityBasisPoint)))
            .div(10000);
        ...
        if (paymentTokenAddress == address(0)) {
            ...
            payable(charityWallet).transfer(payoutToCharity);
        } else {
