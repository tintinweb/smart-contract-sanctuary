...
contract FrogGame is ERC721Enumerable, Pausable, ReentrancyGuard {
    ...
    uint constant public maxMintPerTx = 5;
    ...
    mapping(uint => creatureType) idToCreatureType;
    mapping(address => uint) callerToLastActionBlock;

    /// @dev Constructor
    constructor() ERC721("FrogGame", "FG") {
        owner=msg.sender;
    }
    ...
    function tokenURI(uint _tokenId) public view override noSameBlockAsAction returns (string memory) {
        ...
        return string(abi.encodePacked(_baseTokenURI, idToCreatureType[_tokenId]==creatureType.FROG?"frog/":"snake/",Strings.toString(_tokenId)));
    }
    ...
    /// @dev Mint new token
    function mint() external payable noCheaters onlyMintAllowed nonReentrant whenNotPaused {
        uint newTokenId = totalSupply();
        address tokenReceiver = msg.sender;

        // Phase 0 - mint for ETH, frogs only, can not be stolen
        if (newTokenId < ETH_MINT_SUPPLY) {
            require(msg.value == (mintCost), "Invalid value!");
            idToCreatureType[newTokenId]=creatureType.FROG;
            _frogsTotal++;
        ...
        // Phase 1-4 - mint for $TADPOLE, frogs and snakes, can be stolen
            ...
            uint random = _randomize(_rand(), "creatureType", newTokenId) % 10000;

            // 10% chance to mint snake
            if (random < 1000) {
                idToCreatureType[newTokenId] = creatureType.SNAKE;
                _snakesTotal++;
            } else {
                idToCreatureType[newTokenId] = creatureType.FROG;
                _frogsTotal++;
            ...
            if (random < 1000) {
                ...
                if (randomSnakeOwner!=nullAddress) {
                    ...
                    idToCreatureType[newTokenId]==creatureType.SNAKE?_snakesStolen++:_frogsStolen++;
                }
            }
    ...
    /// @dev Get creature type by tokenId
    function creatureTypeById(uint tokenId) external view noSameBlockAsAction returns(creatureType) {
        require(_exists(tokenId));
        return idToCreatureType[tokenId];
    }
