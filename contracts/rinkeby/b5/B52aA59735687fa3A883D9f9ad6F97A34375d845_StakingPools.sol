...
contract StakingPools is AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function deposit(uint256 _pid, uint256 _amount, uint256 _amountFlype) public virtual nonReentrant hasPool(_pid) poolRunning(_pid) {
        ...
        for (uint256 i = 0; i < pool.rewardsTokens.length; i++) {
            _updateUserInfo(pool, user, i, poolAmountBefore, poolAmountFlypeBefore, flypeTotalSupply);
        }
    ...
    function _updateUserInfo(
        ...
        if (_amount > 0) {
            ...
            if (pending > 0) {
                ...
                pool.rewardsSupply[_tokenNum] -= pending - user.rewardsDebts[_tokenNum];
            }
        }
        ...
        if(flypePoolInfo[_msgSender()] > 0) _bonusReward = (_reward - _baseReward) * flypePoolInfo[_msgSender()] / flypeTotalSupply;
    ...
    function pendingRewards(uint256 _pid, address _user) external view hasPool(_pid) notZeroAddress(_user) returns (uint256[] memory amounts) {
        ...
        for (uint256 i = 0; i < pool.rewardsTokens.length; i++) {
            ...
            uint256 baseReward = reward * 75 / 100;
            uint256 bonusReward;
            if(flypeTotalSupply > 0) bonusReward = (reward - baseReward) * flypePoolInfo[_msgSender()] / flypeTotalSupply; 
    ...
    function getPoolArraysInfo(uint256 _pid) external hasPool(_pid) view returns (
        address[] memory rewardsTokens, 
        uint256[] memory rewardsSupply, 
        uint256[] memory rewardsPerSecond, 
        uint256[] memory accumulatedPerShare, 
        uint256[] memory accuracy)
    {
        PoolInfo memory pool = poolInfo[_pid];
        return (
            pool.rewardsTokens,
