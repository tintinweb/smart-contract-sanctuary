...
contract Traits is Ownable, ITraits {
  ...
  function drawSVG(uint256 tokenId) public view returns (string memory) {
    ...
    string memory svgString = string(abi.encodePacked(
      ...
      s.isBee ? drawTrait(traitData[1 + shift][s.head]) : drawTrait(traitData[1 + shift][s.senseIndex]),
      s.isBee ? drawTrait(traitData[2 + shift][s.ears]) : '',
      drawTrait(traitData[3 + shift][s.eyes]),
      s.isBee ? drawTrait(traitData[4 + shift][s.nose]) : '',
      drawTrait(traitData[5 + shift][s.mouth]),
      s.isBee ? '' : drawTrait(traitData[6 + shift][s.neck]),
      s.isBee ? drawTrait(traitData[7 + shift][s.feet]) : ''
    ));
  ...
  function compileAttributes(uint256 tokenId) public view returns (string memory) {
    ...
    if (s.isBee) {
      ...
      traits = string(abi.encodePacked(
        ...
        attributeForTypeAndValue(_traitTypes[1], traitData[10][s.senseIndex].name),',',
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    string memory metadata = string(abi.encodePacked(
      ...
      s.isBee ? 'Bee #' : 'Bear #',
      tokenId.toString(),
      '", "description": "Thousands of Bees and Bears compete on a farm in the metaverse. A tempting prize of $HONEY awaits, with deadly high stakes. All the metadata and images are generated and stored 100% on-chain. No IPFS. NO API. Just the Ethereum blockchain.", "image": "data:image/svg+xml;base64,',
