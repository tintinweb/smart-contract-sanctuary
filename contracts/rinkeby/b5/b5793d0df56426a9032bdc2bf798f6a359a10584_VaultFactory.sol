...
contract Vault is ERC721Holder, ERC1155Holder {
    ...
    function _addToPrice(uint256 _amount, uint256 _price) internal {
        ...
        if (priceToCount[_price] > 99 && !prices.contains(_price)) {
            prices.add(_price);
        }
    }
    ...
    function _removeFromPrice(uint256 _amount, uint256 _price) internal {
        priceToCount[_price] -= _amount;
        if (priceToCount[_price] < 100 && prices.contains(_price)) {
...
contract VaultFactory is Ownable, ERC1155Holder {
  ...
  event Mint(address indexed token, uint256 id, uint256 fractionId, address vault, uint256 vaultId);

  constructor(address _settings, address _fnft) {
    settings = _settings;
    fnft = _fnft;
    logic = address(new Vault(_settings));
  }
  ...
  function mint(address _token, uint256 _id) external returns(uint256) {
    ...
    FERC1155(fnft).safeTransferFrom(address(this), msg.sender, fractionId, 10000, bytes("0"));
