...
contract PixelPengs is Ownable, ERC721 {
  ...
  uint256 public constant WHITELIST_MINTS = 501;
  ...
  function claim(
    ...
    bytes32 node = keccak256(abi.encodePacked(index, _address));
    require(MerkleProof.verify(proof, merkleRoot, node), 'Invalid Claims - Not on the whitelist or the proof modified');
    _setClaimed(index);
  }

  function whitelistMint(uint256 index, bytes32[] memory proof) external {
    require(active, 'Token sale is not currently active');
    require(whitelistEnabled, 'Whitelisting must be enabled');

    uint256 supply = _tokenSupply.current();
    require(supply + 1 < WHITELIST_MINTS, 'Whitelist minting is finished!');

    // Claim the whitelist mint
    claim(index, msg.sender, proof);
  ...
  function freeMint(uint256 amount) external {
    require(active, 'Token sale is not currently active');
    require(!whitelistEnabled, 'Whitelist is currently enabled - use whitelistMint instead');
  ...
  function setMerkleProof(bytes32 _merkleProof) external onlyOwner {
