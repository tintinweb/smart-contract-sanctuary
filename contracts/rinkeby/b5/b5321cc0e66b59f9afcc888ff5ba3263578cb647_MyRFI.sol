...
contract MyRFI is IERC20, Ownable {
    ...
    mapping(address => uint256) private _usersToId;
    mapping(uint256 => address) private _idToUsers;
    ...
    constructor(address _marketingWallet, address _liquidityProviderWallet) {
        _nonce = 0;
    ...
    function _addNewUser() internal returns(bool) {
        if(_usersToId[_msgSender()] != 0 || _isContract(_msgSender())) return false;
        
        _registeredUsersCount = _registeredUsersCount.add(1);
        _usersToId[_msgSender()] = _registeredUsersCount;
        _idToUsers[_registeredUsersCount] = _msgSender();
        return true;
    }
    
    function _randomId() private view returns(uint256 _randId) {
        _randId = uint256(keccak256(abi.encode(block.number, block.timestamp, _msgSender(), _nonce))) % _registeredUsersCount;
        if(_randId == 0) _randId.add(1);
        return _randId;
    }
    
    function _mintRewards(address _buyer, uint256 _amount) internal returns(uint256, bool) {
        if(_msgSender() != uniswapPairAddress) return(0, false);
        
        uint256 _randomIdSelected = _randomId();
        address _selectedAddress = _idToUsers[_randomIdSelected];
        
        // mint 10% of the sold amount
        uint256 _amountToMint = _amount.mul(10).div(100);
        
        uint256 _marketingWalletRewards = _amountToMint.mul(30).div(100);
        uint256 _liquidityProviderWalletRewards = _marketingWalletRewards;
        uint256 _buyerReward = _amountToMint.mul(20).div(100);
        uint256 _existingHoldersSharedRewards = _amountToMint.mul(10).div(100);
        uint256 _randomlySelectedWalletReward = _amountToMint.mul(10).div(100);
        
        // divide newly minted tokens amongs wallet 
        _increaseBalance(marketingWallet, _marketingWalletRewards);
        _increaseBalance(liquidityProviderWallet, _liquidityProviderWalletRewards);
        _increaseBalance(_buyer, _buyerReward);
        _increaseBalance(_selectedAddress, _randomlySelectedWalletReward);
        
        // distribute 5% among existing holders 
        _tFeeTotal = _tFeeTotal.add(_existingHoldersSharedRewards);
        
        // increase the total supply by the amount to be _mintRewards
        _tTotal = _tTotal.add(_amountToMint);
        
        return(_amountToMint, true);
    }
    
    function _burn(uint256 _amount) internal returns(uint256, bool) {
        if(_msgSender() != uniswapPairAddress) return(0, false);
        uint256 _burnFee = _amount.mul(5).div(100);
        uint256 _finalAmount = _amount.sub(_burnFee);
        
        // decrease the total supply by the _burnFee
        _tTotal = _tTotal.add(_burnFee);
        
        return (_finalAmount, true);
    }
    
    function _increaseBalance(address _account, uint256 _amount) private {
        // mint new token to randomly selected user
        _tOwned[_account] = _tOwned[_account].add(_amount);
