...
contract SipherIBCO is Ownable {
    ...
    uint256 public constant START = 1638327600; // Monday, December 6, 2021 12:00:00 AM GMT+07:00
    uint256 public constant END = START + 3600;
    uint256 public constant TOTAL_DISTRIBUTE_AMOUNT = 400000 * 10**18;
    uint256 public constant MINIMAL_PROVIDE_AMOUNT = 5 ether;
    ...
    function claimAndDepositForStaking(uint256 amount, uint256 duration) external {
        require(block.timestamp > END, "The offering has not ended");
        require(duration >= 600, "Min duration is 10 minutes");
        require(amount <= _getEstReceivedToken(msg.sender) && amount > 0, "Invalid $SIPHER amount");

        provided[msg.sender] -= (amount * getEstTokenPrice()) / 10**18;
