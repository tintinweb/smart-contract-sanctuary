...
// GO TO LINE 1904 TO SEE WHERE THE GORILLA CONTRACT STARTS
...
contract GreatGenesisGorillas is ERC721, Ownable {
    ...
    string public GORILLA_PROVENANCE = ""; // IPFS URL WILL BE ADDED WHEN GORILLAS ARE ALL SOLD OUT
    
    string public LICENSE_TEXT = ""; // IT IS WHAT IT SAYS
    
    bool licenseLocked = false; // TEAM CAN'T EDIT THE LICENSE AFTER THIS GETS TRUE

    uint256 public constant gorillaPrice = 60000000000000000; // 0.025 ETH

    uint public constant maxGorillaPurchase = 5;

    uint256 public constant MAX_GORILLAS = 2000;

    bool public saleIsActive = false;
    bool public presaleOpen = false;
    
    mapping(uint => string) public gorillaNames;
    ...
    constructor() ERC721("Great Genesis Gorillas", "GGG") { }
    ...
    function setProvenanceHash(string memory provenanceHash) public onlyOwner {
        GORILLA_PROVENANCE = provenanceHash;
    }
    ...
    function tokenLicense(uint _id) public view returns(string memory) {
        require(_id < totalSupply(), "CHOOSE A GORILLA WITHIN RANGE");
        return LICENSE_TEXT;
    }
    ...
    function mintGorilla(uint numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint Gorilla");
        require(numberOfTokens > 0 && numberOfTokens <= maxGorillaPurchase, "Can only mint 5 tokens at a time");
        require(totalSupply().add(numberOfTokens) <= MAX_GORILLAS, "Purchase would exceed max supply of Gorillas");
        require(msg.value >= gorillaPrice.mul(numberOfTokens), "Ether value sent is not correct");
    ...
    function mintGorillaPresale(uint256 _count) public payable {
        require(presaleOpen);
        require(_count <= whitelist[msg.sender]);
        uint256 total = totalSupply();
        require(total + _count <= MAX_GORILLAS, "Max limit");
        require(msg.value >= (_count * gorillaPrice), "Value below price");
    ...
    function preMintGorilla(address[] memory recipients) external onlyOwnerOrTeam {
        ...
        require(total + totalRecipients <= MAX_GORILLAS, "Max limit");
