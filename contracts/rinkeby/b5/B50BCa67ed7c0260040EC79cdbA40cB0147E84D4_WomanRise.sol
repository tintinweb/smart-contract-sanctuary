...
// File: contracts/WomanRise.sol

contract WomanRise is IERC2981, ERC721Enumerable, Ownable {
    ...
    function mint(uint32 _mintAmount, bytes memory _signature) public payable {
        ...
        if (!mintingEnabled) {
            ...
            require(
                addressMintedBalance[msg.sender] + _mintAmount <= 10 &&
                _mintAmount != 0,
    ...
    function isWhitelisted(address _wallet, bytes memory _signature) private pure returns(bool) {
        bytes32 messagehash = keccak256(bytes("whitelist"));
        address signerAddress = messagehash.toEthSignedMessageHash().recover(_signature);
        
        return _wallet == signerAddress;
    }
    ...
    function canClaimAirdrop(
        ...
        bytes32 messageHash = keccak256(abi.encodePacked("airdrop", _addressAirDropNumber.toString()));
        address signerAddress = messageHash.toEthSignedMessageHash().recover(_signature);
