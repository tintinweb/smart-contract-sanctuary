...
contract MoodyMonsteras is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
  ...
  uint256 public constant MAX_ELEMENTS = 2999;
  ...
  function changeName(uint _tokenId, string memory _name) public {
    require(ownerOf(_tokenId) == msg.sender, "Your wallet doesn't own this Moody Monstera!");
    require(sha256(bytes(_name)) != sha256(bytes(moodyMonsteraNames[_tokenId])), "The name is the same");
    moodyMonsteraNames[_tokenId] = _name;
    emit MonsteraNameChange(msg.sender, _tokenId, _name);
  }

  function viewName(uint _tokenId) public view returns( string memory ){
    require( _tokenId < totalSupply(), "No Moody Monstera with that id" );
