...
contract TheGame is Ownable {
    ...
    function setAns(uint256 _question, bytes32 _answer) public onlyOwner {
        answers[_question] = _answer;
    }
    
    function getAns(uint256 _question) internal view returns(bytes32) {
        require(answers[_question] != "", "This answer has not been set, if you expect it to be please contact the game admin");
    ...
    function submitAnswer(uint256 _question, bytes32 _givenAnswer) public returns(bool) {
       bytes32 realAnswer = getAns(_question);
       if (_givenAnswer == realAnswer) {
           emit correctAnswer(msg.sender, _question);
    ...
    function getKeccak(string memory _given) public pure returns(bytes32) {
        return keccak256(abi.encodePacked(_given));
    }
    
    function getAlreadyAnswered() public view returns(uint256) {
        uint256[] memory correct_ = correctAnswers[msg.sender];
