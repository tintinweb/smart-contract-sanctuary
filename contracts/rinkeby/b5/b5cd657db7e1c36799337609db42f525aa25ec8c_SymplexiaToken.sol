...
abstract contract BaseToken  is ERC20, Ownable {
    ...
    uint256 internal _storeFactor = (MAX - (MAX % _tokensSupply))/_tokensSupply;
...
//   This Function is responsible for taking all fee, if 'applyFee' is _Enabled
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, uint256 applyFee) private {
        ...
        _totalBonusDue =  _totalBonusDue.sub(bonusQuota.mul(_storeFactor)); 
        ...
        _totalBonusDue =  _totalBonusDue.add(bonusQuota.mul(_storeFactor)); 
...
//   ======================================
    ...
    function _calcBonus (address account) private view returns (uint256) {
        if ( _isExcludedFromBonus[account] || _totalBonusDue == 0 || _rawBalances[account] == 0 ){
            ...
            return _totalBonusDue.mul(_rawBalances[account]).div(_bonusBalances ().div(_storeFactor));
        }
    }
    ...
    function _bonusBalances () private view returns (uint256) {
        ...
        return  _tokensSupply.sub(expurgedBalance);                 
    }
    ...
    function AccruedBonus() public view returns (uint256) {
        return _totalBonusDue.div(_storeFactor).div(10**_decimals);
    }
...
//   ======================================
    ...
    */
    ...
    function includeInBonus(address account) public onlyOwner() {
        ...
        uint256 _adjustedBonus    = _rawBalances[account].div(_bonusBalances().add(_totalBonusDue));
        uint256 _adjustedBalance = _rawBalances[account].sub(_adjustedBonus);      			// It has a bit distorted cumulative effect 
        for (uint256 i = 0; i < _noBonus.length; i++) {
...
contract SymplexiaToken is  Taxable {
  ...
  //
    ...
    constructor () ERC20 ("DEBUG46", "DBG46") BaseToken (5, 5, 5, 
