...
contract Token is ERC721Enumerable, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    function setMetadataList(uint256[] memory tokenIds, string[] memory metadata) public {
        require(msg.sender == metadataRole());

        require(tokenIds.length == metadata.length);
        for (uint index = 0; index < tokenIds.length; index += 1) {
            _setTokenURI(tokenIds[index], metadata[index]);
        }
    }
