...
contract CryptoPageToken is ERC20, AccessControl {
    ...
    constructor() ERC20("Crypto Page", "PAGE") {
        ...
        PAGE_NFT_BANK = new PageNFTBank(address(PAGE_MINTER));
        PAGE_MINTER_NFT = new PageMinterNFT(address(PAGE_MINTER), address(PAGE_NFT_BANK));
        ...
        PAGE_MINTER.setMinter("BANK_SELL", address(PAGE_NFT_BANK), 1 ** 18, true); // On the price effect amount of comments
    ...
    // ONLY ADMIN
    function removeMinter(string memory _key) public onlyRole(ADMIN_ROLE) {
        PAGE_MINTER.removeMinter(_key);
    }
    function setMinter(string memory _key, address _account, uint256 _pageamount) public onlyRole(ADMIN_ROLE) {
        PAGE_MINTER.setMinter(_key, _account, _pageamount, false);
    }
    function setTreasuryFee(uint256 _percent) public onlyRole(ADMIN_ROLE) {
        PAGE_MINTER.setTreasuryFee(_percent);
    }
    function setTreasuryAddress(address _treasury) public onlyRole(ADMIN_ROLE) {
        PAGE_MINTER.setTreasuryAddress(_treasury);
    }

    // UPDATE MINTER CORE
    /*
    function updateMinterCore(address _mintercore) public onlyRole(ADMIN_ROLE) {
        PAGE_MINTER = PageMinter(_mintercore);
        _setupRole(MINTER_ROLE, _mintercore);
    }
    */
...
contract PageMinter {
    ...
    function mint(string memory _key, address [] memory _to) public {
        require(_keytank[_key], "removeMinter: _key doesn't exists");
    ...
    function mintX(string memory _key, address [] memory _to, uint _multiplier) public {
        require(_keytank[_key], "removeMinter: _key doesn't exists");
    ...
    function removeMinter(string memory _key) public onlyPageToken() {
        require(_keytank[_key], "removeMinter: _key doesn't exists");
        _keytank[_key] = false;
    ...
    function setMinter(string memory _key, address _account, uint256 _pageamount, bool _xmint) public onlyPageToken() {
        if (_keytank[_key]) {
    ...
    // GET FUNCTIONS
    function getMinter(string memory _key) public view returns (
        uint256 id,
        address author,
        uint256 amount,
        bool xmint) {
...
contract PageMinterNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    constructor(address _PAGE_MINTER, address _BANK_ADDRESS) ERC721("Crypto Page NFT", "PAGE-NFT")  {
        PAGE_MINTER = PageMinter(_PAGE_MINTER);
