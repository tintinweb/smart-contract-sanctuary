...
contract RoyaltySplitter is Ownable {
    ...
    function avalibleWithdrawalBalance(address account) public view returns(uint) {
        ...
        if (/* minterToAmountMinted[account] */ amountPerMinter(account) > 0) {
            payment += ((totalReceived * 3) * /* minterToAmountMinted[account] */ amountPerMinter(account)) / 11900;
        }
        if (_tokenBalance(account) > 0) {
            payment += ((totalReceived * 2) * _tokenBalance(account)) / 11900;
        }
    ...
    function _tokenBalance(address account) public view returns(uint) {
        if (account == communityWallet) {
            return (WolfPackContract.balanceOf(account) + (1701 - WolfPackContract.getSupply()));
        } else {
            return WolfPackContract.balanceOf(account);
        }
    }
    ...
    function amountPerMinter(address _minter) public view returns(uint16) {
        ...
        // minterToAmountMinted[_minter] = counter;
        return counter;
    }

    // function syncMinterToAmountMintedMapping(address minterToUpdate) external {
    //     minterToAmountMinted[minterToUpdate] = amountPerMinter(minterToUpdate);
    //     minterToAmountMinted[communityWallet] = amountPerMinter(communityWallet);
    // }
