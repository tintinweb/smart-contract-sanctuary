...
contract SupShop is ERC1155Upgradeable, OwnableUpgradeable {
    ...
    address private frogContract;
    string private baseURI;

    mapping(uint256 => bool) public itemTypeIds;
    mapping(uint256 => uint256) public totalSupplies;
    uint256 public numTypes;

    event SetBaseURI(string indexed _baseURI);

    function mintBatch(uint256[] memory ids, uint256[] memory amounts, address to) external {
    ...
    function totalSupply(uint256 typeId) public view returns (uint256) {
        require(itemTypeIds[typeId], "URI requested for invalid serum type");
        return totalSupplies[typeId];
    }

    function setfrogContractAddress(address frogContractAddress) external onlyOwner {
        frogContract = frogContractAddress;
    }

    function burnItem(uint256 typeId, address burnTokenAddress) external {
        require(msg.sender == frogContract, "Invalid burner address");
    ...
    function addItemTypeId(uint256 itemTypeId) external onlyOwner {
        itemTypeIds[itemTypeId] = true;
        numTypes++;
    }
    ...
    function initialize() initializer public {
        __ERC1155_init("https://api.supducks.com/metadata/");
        __Ownable_init();

        baseURI = "https://api.supducks.com/metadata/";
        itemTypeIds[0] = true;
        itemTypeIds[1] = true;
        numTypes = 2;
    }
...
// WIP
contract SupShopFactory is Initializable, ContextUpgradeable, IFactory, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  using StringsUpgradeable for uint256;

  address public proxyRegistryAddress;
  SupShop public supShop;
  ...
  /**
   * Three different options for minting SupShops (basic, premium, and gold).
   */

  function initialize(address _proxyRegistryAddress, address _supShop) initializer public  {
    __Context_init_unchained();
    __Ownable_init();
    
    baseURI = "https://api.supducks.com/metadata/";
    proxyRegistryAddress = _proxyRegistryAddress;
    supShop = SupShop(_supShop);
  }
  ...
  function mint(uint256 _optionId, address _toAddress, uint256 _amount, bytes calldata _noWarning) external override nonReentrant() {
    require(_isOwnerOrProxy(msg.sender), "must be owner or proxy");
    _noWarning = _noWarning;
    return _mint(_optionId, _toAddress, _amount);
  }
  ...
  function _mint(
    ...
    //SupShop nftContract = SupShop(supShop);
    //uint256 id = optionToTokenID[optionId];
    supShop.mint(_option, _amount, _toAddress);
    //nftContract.setBaseURI("bananas");
    // if (id == 0) {
...
// 
  ...
  function safeTransferFrom(
    ...
    uint256 _optionId,
    uint256 _amount,
    bytes calldata _noWarning
  ) external override {
    require(_isOwnerOrProxy(msg.sender), "must be owner or proxy");
    _noWarning = _noWarning;
    _mint(_optionId, _to, _amount);
  }
  ...
  function _canMint(
    ...
    return _amount > 0 && balanceOf(_fromAddress, _option) >= _amount;
  }
