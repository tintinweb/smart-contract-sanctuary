...
contract Skeletongues is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  function amIOwner(uint256 tokenId) public view returns (bool) {
    ...
    uint256[] memory sgbTokensArray = sgb.tokensOfOwner(msg.sender);
    require(sgbTokensArray.length > 0, "NO_SGB");
    for (uint256 i=0; i<sgbTokensArray.length; i++) {
      if (sgbTokensArray[i] == tokenId) {
        return true;
      }
    }
    return false;
  ...
  function getMintableIds() public view returns (uint256[] memory){
    SadGirlsBarClient sgb = SadGirlsBarClient(_sgbAddress);
    ...
    // array for result
    uint256[] memory mintableIds = new uint256[](_maxBatchSize);
  ...
  function availableToMint() public view returns (uint256[] memory) {
    return getMintableIds();
  }

  function mintMany() external {
    require(isStarted, "NO_START");
    uint256[] memory mintableIds = getMintableIds();
    require(mintableIds.length > 0, "NO_MINT");
    for (uint256 i=0; i<mintableIds.length; i++) {
      //f***ing solidity restrictions to memory arrays (no dynamic array can be returned from function)
      if (mintableIds[i] == 0) {
        break;
      }
...
//    Skeletongues me = Skeletongues(address(this)); // I HATE SOLIDITY @VitalikButerin
//    SadGirlsBarClient sgb = SadGirlsBarClient(_sgbAddress);
//    uint256 supply = sgb.totalSupply();
//    uint256[] memory allTokens = new uint256[](supply);
//    for (uint256 i=0; i<supply; i++) {
//      try me.tokenByIndex(i) returns (uint256 _value) { // I CANT CALL try/catch FOR INTERNAL METHODS. WTF?!
//        if (_value != 0) {
//          allTokens[i] = _value; // I CANT OVERRIDE VISIBILITY OF _allTokens VARIABLE. W.T.F?!?!
//        }
...
//      function getAllTokens() public view returns (uint256[] memory) {
////        require(_allTokens.length > 0, "NO_TOKENS");
//        uint256[] memory allTokens = new uint256[](_allTokens.length);
//        for (uint256 i=0; i<_allTokens.length; i++) {
//          allTokens[i] = _allTokens[i];
//        }
//        return allTokens;
//      }
