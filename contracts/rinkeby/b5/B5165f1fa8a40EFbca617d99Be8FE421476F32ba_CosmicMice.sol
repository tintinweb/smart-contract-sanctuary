...
contract CosmicMice is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
        ...
        if(supply <= 6) {
            require(ownerMintedCount + _mintAmount <= 5, "max NFT per address exceeded for the first 666 ");
        }
        //free first 666 mint
        if(supply >= 6){
