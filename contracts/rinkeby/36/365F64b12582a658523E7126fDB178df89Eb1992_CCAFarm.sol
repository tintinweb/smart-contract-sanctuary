...
contract CCAFarm is CCAToken, Ownable, ERC20Burnable, ERC20Pausable {
  ...
  CCAToken public immutable ccaToken;
  ...
  function stake(uint256 amount) public {
      require(amount > 0 && balanceOf(msg.sender) >= amount, "You cannot stake zero tokens");
      if(isStaking[msg.sender] == true){
          uint256 toTransfer = calculateYieldTotal(msg.sender);
          ccaBalance[msg.sender] += toTransfer;
      }
  ...
  function unstake(uint256 amount) public {
      ...
      IERC20(stakeToken).safeTransfer(msg.sender, balTransfer);
  ...
  function calculateYieldTotal(address user) public view returns(uint256) {
      require(stakingBalance[user] > 0, "No active Stake");
      uint256 time = calculateYieldTime(user) * 10**decimals();
      uint256 rate = 86400;
      uint256 timeRate = time / rate;
      uint256 rawYield = (stakingBalance[user] * timeRate) / 10**decimals();
      return rawYield;
  }
