...
contract JustBackgrounds is ERC721, ERC721Enumerable, Pausable, Ownable {
    ...
    function mintCollectible() external whenNotPaused {
        require(availableSupply() > 0, "GRNDS: Qty exceeds available supply");
        _mintBackground(msg.sender, uint8(_generateRandomNumber(block.difficulty) % _unusedColors.length));
    }

    function reserveCollectibles() external onlyOwner {
        require(!_isReserved, "GRNDS: Tokens already reserved");
        require(availableSupply() >= 10, "GRNDS: Qty exceeds available supply");
     ...
     */
    function _mintBackground(address to, uint8 colorIndex) private {
        uint256 tokenId = totalSupply() + 1;
        _safeMint(to, tokenId);
        _tokenToColorIndex[tokenId] = bytes1(_unusedColors[colorIndex]);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (_checkIfMatch(traits.special, 'Meme')) {
            ...
            byteString = abi.encodePacked(byteString, 'Hand crafted SVG, 520 x 520 pixels",');
        } else if (_checkIfMatch(traits.special, 'Transparent')) {
            byteString = abi.encodePacked(byteString, "**", traits.name, '**\\n\\n');
            byteString = abi.encodePacked(byteString, 'Hand crafted SVG, 1080 x 1080 pixels",');
        } else {
