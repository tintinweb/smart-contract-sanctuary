...
contract Sale is Ownable, AccessControl {
    ...
    /**
     * @dev changeStaking - allowed only at UNKNOWN stage (when sale just created and not started).
     * @param saleID - id of sales list
     */

    function changeStaking(uint256 saleID, address _newStaking) public onlyOwner {
    ...
    function registerSale(uint256 saleID, uint256 paymentAmount) public {
        ...
        require(
            paymentAmount <=
                (sales[saleID].staking.getCurrentStake(msg.sender).mul(1e9).div(sales[saleID].staking.getCurrentTotalStake()))
