...
contract Borgs is CommonObjects, StringUtils, NumberUtils{
    ...
    uint256 _imageSize = 576; // 3x3 for now but should be 576
    ...
    function generateToken() external returns(string[] memory generatedPixals)
    {
        ...
        for(uint256 i=0;i<_layerMetaData.length;i++){
            ...
            LayerItem memory layerItem = layer.layerItems[0];
            generatedPixals = _addLayerToImage(generatedPixals, layerItem.hexPixals);
        }
    ...
    function _addLayerToImage(string[] memory basePixals, string[] memory layerPixals) internal returns(string[] memory newHexPixals)
    {
        require(basePixals.length == layerPixals.length, 'Base pixals are not the same length as the layers pixals');
        newHexPixals = new string[](basePixals.length);
