...
contract DecubateVesting is IDecubateVesting, Ownable, ReentrancyGuard {
    ...
    returns (uint256)
  {
    ...
    if (block.timestamp < vestingPools[_option].cliff) {
      ...
      block.timestamp >= vest.start.add(vest.duration) || whitelist.revoke
    ) {
   ...
   *
   */
  function disable(uint256 _option, address _wallet)
    public
    onlyOwner
    userInWhitelist(_option, _wallet)
  {
    ...
    require(!whitelist.disabled, "User is already disabled");
...
interface IDecubateVesting {
   ...
   *
   */
  event Disabled(address indexed wallet);
