...
contract PudgySerums is VRFConsumerBase, ERC1155, Pausable, Ownable, ReentrancyGuard {
    //Start timestamp
    uint public startTime = 1646089200;

    //Serum mapping
    uint256[] internal serumIds;
    mapping(uint => uint) internal tokenIdToSerum;
    ...
    constructor(address _vrfCoordinator, address _link, bytes32 _keyHash, uint _fee, address _fridge, address _pudgyApes, address _mutantPudgies , string memory _uri) 
        VRFConsumerBase(
            _vrfCoordinator,
            _link
        )
    ...
    //CLAIM
    function claim(uint[] calldata _ids) external whenNotPaused nonReentrant {
        require(isClaimingOpen(), "PudgySerums: Claiming is not open yet!");
        for(uint i = 0; i < _ids.length; i++) {
            require(ownedOrStaked(_ids[i]), "PudgySerums: You don't own or stake this token!");
            require(isClaimed(_ids[i]), "PudgySerums: The allocated serum has already been claimed!");
            pudgyApesUsed[_ids[i]] = true;
            _mint(msg.sender, tokenIdToSerum[_ids[i]], 1, "");
        }
    ...
    function ownedOrStaked( uint256 _tokenId) public view returns (bool) {
        return IERC721(PudgyApes).ownerOf(_tokenId) == msg.sender || FridgeInterface(Fridge).ownedBySender(_tokenId);
    }

    //Allocate serum to pudgys, based on a random seed provided by the VRF coordinator
    function allocate() internal {
        for (uint256 i = 0; i < serumIds.length; i++) {
            ...
            serumIds[i] = temp;
            tokenIdToSerum[i+1] = serumIds[i];
        }
    }

    //Allocate Serums
    function allocateSerums() external onlyOwner returns (bytes32 _requestId) {
        require(LINK.balanceOf(address(this)) >= fee, "Not enough LINK - fill contract with faucet");
        return requestRandomness(keyHash, fee);
    }

    //Set initial serum ids, before randomly allocating
    function setSerumIds() external onlyOwner {
        for (uint i = 0; i < 3; i++) {
            serumIds.push(M3Serum);
        }
        for(uint i = 0; i < 1000; i++) {
            serumIds.push(M2Serum);
        }
        for(uint i = 0; i < 2330; i++) {
            serumIds.push(M1Serum);
        }
    }
