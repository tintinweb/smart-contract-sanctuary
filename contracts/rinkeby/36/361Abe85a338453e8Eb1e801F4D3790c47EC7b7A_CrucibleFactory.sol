...
contract CrucibleFactory is CrucibleTokenDeployer,
		NoDelegateCall, ICrucibleFactory, WithAdmin {
	event CrucibleCreated(address token, address baseToken,
		uint256 feeOnTransferX10000, uint256 feeOnWithdrawX10000);
    mapping(bytes32 => address) private crucible;
	uint64 constant MAX_FEE = 10000;
	address immutable public override router;

	constructor() {
		(router) = abi.decode(IFerrumDeployer(msg.sender).initData(), (address));
	}

    function getCrucible(
		address baseToken,
        uint64 feeOnTransferX10000,
        uint64 feeOnWithdrawX10000) external override view returns (address) {
		return crucible[crucibleKey(baseToken, feeOnTransferX10000, feeOnWithdrawX10000)];
	}
    ...
    function _createCrucible(
	    ...
	    require(feeOnTransferX10000 != 0 || feeOnWithdrawX10000 != 0,
		...
		bytes32 key = crucibleKey(baseToken, feeOnTransferX10000, feeOnWithdrawX10000);
	    ...
	    string memory cName = string(abi.encodePacked("Crucible: ", name, " ", feeOnT, "X", feeOnW));
	    string memory cSymbol = string(abi.encodePacked(symbol, feeOnT, "X", feeOnW));
        token = deploy(
			address(this),
			baseToken,
			feeOnTransferX10000,
			...
			cSymbol);
        crucible[key] = token;
        emit CrucibleCreated(token, baseToken, feeOnTransferX10000, feeOnWithdrawX10000);
    }

	function crucibleKey(address baseToken,
        uint64 feeOnTransferX10000,
        uint64 feeOnWithdrawX10000) private pure returns (bytes32) {
		return keccak256(abi.encodePacked(baseToken, feeOnTransferX10000, feeOnWithdrawX10000));
	}

    function safeName(address token) internal view returns (string memory) {
		(bool succ, bytes memory data) = token.staticcall(abi.encodeWithSignature(("name()")));
		if (succ) {
    ...
    function safeSymbol(address token) internal view returns (string memory) {
		(bool succ, bytes memory data) = token.staticcall(abi.encodeWithSignature(("symbol()")));
...
abstract contract CrucibleTokenDeployer is ICrucibleTokenDeployer {
    ...
    function deploy(
        ...
        token = address(new CrucibleToken{salt: keccak256(
            abi.encode(baseToken, feeOnTransferX10000, feeOnWithdrawX10000))}());
        delete parameters;
    }
}
...
contract CrucibleToken is ERC20, TokenReceivable, ICrucibleToken {
	...
	address /*immutable*/ public override baseToken; // Remocing immutables to allow etherscan verification to work. Hopefully etherscan gives us a solution
	uint64 /*immutable*/ public feeOnTransferX10000;
	uint64 /*immutable*/ public feeOnWithdrawX10000;
	...
	constructor() {
		address token;
		address fac;
		(fac, token, feeOnTransferX10000, feeOnWithdrawX10000, name, symbol) = 
	...
	function overrideFee(address target, OverrideState overrideType, uint64 newFeeX10000)
	...
	function _withdraw(address from, address to, uint256 amount)
	...
	*/
	...
	external virtual {
		...
		require(amount != 0, "CT: amount required");
        uint256 decreasedAllowance = allowance[from][msg.sender]
    ...
    internal virtual override {
		...
		if (sender == td || recipient == td) {
			...
			if (overFrom.over == OverrideState.OverrideOut ||
					overFrom.over == OverrideState.OverrideBoth) {
				feeRatioX10k = overFrom.feeX10000;
				overriden = true;
			}
			if ((overTo.over == OverrideState.OverrideIn ||
	...
	function _doTransfer(address sender, address recipient, uint256 amount)
	internal {
        ERC20._transfer(sender, recipient, amount);
	}

	function tax(address from, address taxDist, uint256 amount)
	internal {
		_doTransfer(from, taxDist, amount);
		IGeneralTaxDistributor(taxDist)
			.distributeTax(address(this));
	} 

    function safeDecimals(address token)
	internal view returns (uint8) {
		(bool succ, bytes memory data) = token.staticcall(abi.encodeWithSignature(("decimals()")));
		if (succ) {
