...
contract ReconRams is ERC721Enum, Ownable, PaymentSplitter {
    ...
    // 1. I dislike the current whitelisting logic. Please suggest any recommended alternatives. Currently looking into Merkle Trees
    // 2. Reentrancy guard needed given the attempted checks-effects-interaction pattern? I observed contracts using it even with checks-effects-interaction, don't understand why atm.
    // 3. "isContract" currently not sufficient to protect against contract-to-contract public mint abuse, looking into alternatives.
    // 4. Using a modified ERC721 taken from here: https://medium.com/coinmonks/the-cannibalization-of-nfts-by-openzeppelin-by-insanely-high-gas-prices-cd2c9a7c1e7
    ...
    ) ERC721P(_name, _symbol) PaymentSplitter(_splitterAddressList, _shareList) {
    ...
    function mint(uint256 _mintAmount) external payable {
        ...
        if (presaleOnlyActive == true) {
            ...
            uint256 senderMintedCount = whitelistAddressMintedBalance[
                msg.sender
            ];
            require(
                senderMintedCount + _mintAmount <= MAX_PRESALE_MINT,
                "Total mints after tx exceeds max mints for this address"
            );
            whitelistAddressMintedBalance[msg.sender] += _mintAmount;
        } else {
            require(
                _mintAmount <= MAX_PUBLIC_SALE_PER_TX,
                "Mint amt greater than max per tx"
            );
        }
    ...
    // COMMENT FOR AUDITOR: The first X (WIP) token ids will be manually arranged in IPFS to be the legendaries in collection
