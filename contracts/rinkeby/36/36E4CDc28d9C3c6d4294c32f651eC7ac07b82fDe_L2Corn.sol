 ...
 */
contract L2Corn is ERC777, IERC777Recipient, ReentrancyGuard, IERC721Receiver, ERC165, ERC721Holder, IArbToken {
    ...
    */
     ...
     */
    uint256 private constant maxGrowthCycle = 268800;
    ...
    */
        ...
        requireFarm(_msgSender(), false) // Ensure LAND is not already in a farm
    {
        ...
        globalCompostedAmount = globalCompostedAmount.add(farm.compostedAmount);
        globalTotalFarms += 1;
        ...
        requireFarm(sourceAddress, true) // Ensure the adress that is being harvested has a farm on the LAND
    {
        ...
        Farm storage farm = farms[sourceAddress]; // Shortcut accessor, pay attention to sourceAddress here
    ...
    function equipCollectible(uint256 tokenID)
        ...
        (string memory _name, uint256 _maxBoostLevel,uint256 _blocksToMaxBoost,uint256 _expiryDate) = nftToken.collectibleTraits(tokenID);
  
        Farm storage farm = farms[_msgSender()]; // Shortcut accessor
        // Increment number of collectibles owned by that address
        farm.numberOfCollectibles += 1;
        // // Add details to Collectibles
        farm.collectibles.push(Collectible(tokenID, _name, _maxBoostLevel, _blocksToMaxBoost, block.number, _expiryDate));
    ...
    function releaseCollectible(uint256 tokenID)
        public
        nonReentrant()
        preventSameBlock(_msgSender())
        ...
        // Search all farms collectibles
        for (uint i=0; i<farm.numberOfCollectibles; i++) {
            // Check for collectible matching tokenID & delete
            if ( farm.collectibles[i].id > tokenID ) {
                delete farm.collectibles[i];
                farm.numberOfCollectibles = farm.numberOfCollectibles.sub(1);
                _success = true;
            }
        }
        require ( _success == true, "Unable to locate tokenID" );
        ...
        returns (uint256 actualMinted)
    {
        ...
        uint256 lastBlockInGrowthCycle = farm.lastHarvestedBlockNumber.add(maxGrowthCycle); // end of growth cycle last allowed block
        ...
        returns (uint256 collectibleMultiplier)
    {
        ...
        (uint256 totalMaxBoostLevel, uint256 totalBlocksToMaxBoost, uint256 finalExpiry) = getCollectibleDetails(targetAddress); 

        // You don't get a boost if the MaxBoostLevel is less than or equal to 1x
        if (totalMaxBoostLevel <= _percentMultiplier) {
            return _percentMultiplier;
        }
        ...
        uint256 boostExtension = totalMaxBoostLevel.sub(_percentMultiplier);
        ...
        collectibleMultiplier = Math.min(totalMaxBoostLevel, blockDiff); // Min 1x, Max depends on the collectible attributes 
    }
    ...
    function getCollectibleDetails(address targetAddress)
        ...
        view
        returns (
            uint256 totalMaxBoostLevel,
            uint256 totalblocksToMaxBoost,
            uint256 finalExpiry
            )
    {
        Farm memory farm = farms[targetAddress]; // Shortcut accessor
        totalMaxBoostLevel = 10000;
        bool noExpiry;
        for (uint i=0; i<farms[targetAddress].numberOfCollectibles; i++) {
            totalMaxBoostLevel += farm.collectibles[i].maxBoostLevel;
            totalblocksToMaxBoost += farm.collectibles[i].blocksToMaxBoost;
            if ( farm.collectibles[i].expiry == 0 ) {
                noExpiry = true;
                finalExpiry = 0;
            }
            if ( farm.collectibles[i].expiry > finalExpiry && noExpiry == false ) { 
                finalExpiry = farm.collectibles[i].expiry;
            }
        }
    }
