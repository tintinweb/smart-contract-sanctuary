...
contract NameTag is ERC721, Ownable, StringUpper, DenyList, Random {
    ...
    uint256 private _limitRandomSupply = 12000;
    ...
    constructor(string memory name_, string memory symbol_, uint256 price_, uint256 metadataFee_, uint8 tokenAmountBuyLimit_) ERC721(name_, symbol_) {
        ...
        waves.push(Wave(3000, 0));
        waves.push(Wave(6000, 0));
        waves.push(Wave(9000, 0));
        waves.push(Wave(_limitRandomSupply, 0));
        waves.push(Wave(type(uint256).max, 0));
    }

    function limitSupply() public view virtual returns (uint256) {
        return _limitRandomSupply;
    }
    ...
    function randomSelling() public view virtual returns (bool) {
        return totalSupply() < limitSupply();
    }
    ...
    function _preValidatePurchase(uint256 tokensAmount) internal view {
        require(tokensAmount <= tokenAmountBuyLimit());
        require(msg.sender != address(0));
        require(msg.value >= price());

        uint256 time = waves[currentWaveIndex()].startTime;
        require(time != 0 && block.timestamp >= time);
    }
    ...
    function setNames(uint256[] memory _tokens, string[] memory _names) public payable returns (bool[] memory) {
        ...
        for (uint index = 0; index < _tokens.length; index += 1) {
            ...
            if (hasName && statuses[index]) {
                require(weiAmount >= metadataFee());
                weiAmount -= metadataFee();

                _setTokenURI(_tokens[index], defaultNamedMetadata());
            }
    ...
    function setMetadata(uint256[] memory _tokens, string[] memory _metadata) public {
    ...
    function buyNamedTokens(string[] memory _names) external payable returns (uint256[] memory) {
        ...
        for (uint index = 0; index < tokensAmount; index += 1) {
            ...
            if (index < _names.length) {
                require(_setName(tokens[index], _names[index]));
                _setTokenURI(tokens[index], defaultNamedMetadata());
            } else {
                _setTokenURI(tokens[index], defaultMetadata());
            }
        }
...
abstract contract Random {
    ...
    function random(uint256 limit) internal virtual returns (uint256) {
