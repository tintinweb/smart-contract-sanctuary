...
contract FacaNFT is FacaBase, VRFConsumerBase {
  ...
  constructor(
    ...
    FacaBase(_tokenName, _tokenSymbol, _mintFee, _maxSupply, _limitMint)
   ...
   */
  function _mintFaca(address _to,uint256 numberToken) internal returns (bool) {
  ...
  function mintFaca(uint256 numberToken)
    public
    payable
    online
    mintable(numberToken)
    returns (bool) {
    return _mintFaca(_msgSender(), numberToken);
  }
  ...
  function airdrop(address _to, uint256 numberToken) public offline onlyOwner {
    _mintFaca(_to, numberToken); // mint for marketing & influencer
  }
}
