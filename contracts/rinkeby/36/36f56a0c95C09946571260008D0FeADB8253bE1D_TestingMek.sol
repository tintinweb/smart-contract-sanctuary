...
contract TestingMek is ERC721Enumerable, Ownable {
    ...
    function mint(uint256 _times) payable public {
        ...
        require(_times >0 && _times <= maxMintAmount, "Must mint fewer in each batch");
        require(maxSupply + _times <= totalCount, "Max supply reached!");
        require(msg.value == _times * price, "Value error, please check price.");
        //payable(owner()).transfer(msg.value);
        emit mintNFT(_msgSender(), maxSupply+1, _times);
        for(uint256 i=0; i< _times; i++){
            _mint(_msgSender(), 1 + maxSupply++);
        }
    }  
