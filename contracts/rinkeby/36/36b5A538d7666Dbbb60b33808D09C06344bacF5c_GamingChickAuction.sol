...
contract GamingChickAuction is OwnableContract, IERC721Receiver, Initializable{
    ...
    function updDebug(bool _debug) external onlyAdmin {
        debug = _debug;
    }
    ...
    function removeWinnerRecords() public onlyAdmin {
        require(batchId >= 1, "Invalid batchId");
        uint256 id = batchId - 1;
        for (uint256 i = 0; i < winnerList[id].length; i++) {
            address addr = winnerList[id][i];
            removeUserInfoByPoolId(0, addr);
            removeUserInfoByPoolId(1, addr);
            removeUserInfoByPoolId(2, addr);
            removeUserInfoByPoolId(3, addr);
        }
    }

    function removeUserInfoByPoolId(uint256 _pid, address _wallet) public onlyAdmin {
    ...
    function updTotalScore() internal {
