...
contract TestMiddleware is IERC721Receiver, Ownable{
    ...
    function setMigrator(address newMigrator) public onlyOwner {
        migrator = newMigrator;
    }

    function getMigrator() public view returns (address) {
        return migrator;
    }
    ...
    function getDepositedItem(uint256 tokenId) public view returns (bool) {
        return depositedItems[tokenId];
    }

    function migrateBatch(uint256[] memory tokenIds,address[] memory to) public {
        require(msg.sender == migrator,"MigrateBatch: caller does not have access to migrate");
        for(uint256 index=0; index<tokenIds.length; index++){
            require(getDepositedItem(tokenIds[index]) == true,"MigrateBatch: Item not deposited to middleware");
            depositedItems[tokenIds[index]] = false;
            nftContract.safeTransfer(address(this), address(this), to[index], tokenIds[index]);
            emit TokenWithDraw(to[index],tokenIds[index]);
        }
    }

    function migrate(uint256 tokenId,address to) public {
        require(msg.sender == migrator,"Migrate: caller does not have access to migrate");
        require(getDepositedItem(tokenId) == true,"Migrate: Item not deposited to middleware");
