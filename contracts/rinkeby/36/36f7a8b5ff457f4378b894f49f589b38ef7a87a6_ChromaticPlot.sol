...
contract ChromaticPlot is IERC721 {
    ...
    constructor(address payable _adminAddress, address _chroma, uint _networkId) {
    ...
    function tokenURI(uint256 _tokenId) external view validNFToken(_tokenId) returns (string memory) {
        return string(abi.encodePacked("https://chromatic.art/api/metadata?type=plot&id=", toString(_tokenId), "&network=", toString(networkId)));
    }
