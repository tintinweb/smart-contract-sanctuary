...
contract FIRST_OPTION is ERC20, Ownable {
	...
	constructor () ERC20("TEST20211119", "TEST", 9, totalFund) Ownable(ownerWallet) {
	...
	function calcTokenReward(address account) public view returns (uint256) {
		...
		if (_dividendPerToken > _dividendCreditedTo[account] && account != ownerWallet) {
	...
	function transfer(address recipient, uint256 amount) override public returns(bool) {
		checkOneYearLock(_msgSender(), recipient);
		checkLockWallet(_msgSender(), recipient);
	...
	function transferFrom(address sender, address recipient, uint256 amount) override public returns(bool) {
		checkOneYearLock(sender, recipient);
		checkLockWallet(sender, recipient);
	...
	function checkLockWallet(address sender, address recipient) private view {
		require(_lockWallet[sender] == false && _lockWallet[recipient] == false, "OPTN: locked tokens cannot be used");
	}

	function checkOneYearLock(address sender, address recipient) private view {
		if (sender == teamWallet || sender == futureDevWallet || recipient == teamWallet || recipient == futureDevWallet) {
