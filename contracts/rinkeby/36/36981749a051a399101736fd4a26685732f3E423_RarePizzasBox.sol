    ...
    IOpenSeaCompatible
{
    ...
    int256 public bitcoinPriceInETH;
     ...
     */
    function updateBitcoinPriceInETH(int256 fallbackValue) public virtual onlyOwner {
        if (_chainlinkBTCETHFeed != address(0)) {
            try AggregatorV3Interface(_chainlinkBTCETHFeed).latestRoundData() returns (
                ...
                int256 old = bitcoinPriceInETH;
                bitcoinPriceInETH = answer;
                emit BTCETHPriceUpdated(old, bitcoinPriceInETH);
                return;
        ...
        if (fallbackValue > 0) {
            int256 old = bitcoinPriceInETH;
            bitcoinPriceInETH = fallbackValue;
            emit BTCETHPriceUpdated(old, bitcoinPriceInETH);
        }
