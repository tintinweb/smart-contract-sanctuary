...
function isOwner(uint _id) public view returns(bool) {
    return msg.sender==ownerOfCar[_id];
  }
...
function changeColor(string memory _color,uint _id) external payable onlyOwner(_id)  {
...
function registerCar(uint _id)external payable  onlyOwner(_id){
