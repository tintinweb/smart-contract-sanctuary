...
interface ILitedexTimelockVIP {
        ...
        function initialize(address _lockToken, uint256 _vestingPeriod) external returns(bool);
...
contract UpgradedLitedexPrivatesale is TimeCrowdsale{
    ...
    constructor(address _tokenAddress, address _pairAddress) public {
        ...
        ILitedexTimelockVIP(timelock).initialize(_tokenAddress, 9);
    ...
    function addRolesTransactor(
        ...
        Param[rolesid].stakedUSD = 0;
        Param[rolesid].token_sold = 0;
        Param[rolesid].total_received = 0;
...
contract LitedexTimelockVVIP is BEP20("Litedex VVIP","LDX-VVIP") {
        ...
        function initialize(address _lockToken, uint256 _vestingPeriod) external returns(bool){
            ...
            for(uint i = 1; i<=vestingPeriod;i++){
                vesting[i].timeUnlock = (i==1) ? unlockedTime : unlockedTime.add(i.mul(30 days));
        ...
        function unlocked(uint256 amount) external whileOpen returns(bool){
            ...
            require(amount > 0 && amount <= maxReedeem, 'Litedex: amount exceeds maximum vesting');
