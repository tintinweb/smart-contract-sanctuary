...
contract ProofOfCultureMinter is Ownable {
  ...
  */
  ...
  /**
  * @dev Gets the price up to 10000 tokens.
  ...
  */
  function getPrice(bool isReservation) public view returns (uint256) {
    ...
    if (isReservation) {
      if (currentSupply == 9999) {
          return 9000000000000000000; // 9999 9 ETH
      } else if (currentSupply >= 9992) {
          return 9000000000000000000; // 9992 - 9998 9 ETH
      ...
      if (currentSupply == 9999) {
          ...
          return 5000000000000000000; // 9992 - 9998 5 ETH
      } else if (currentSupply >= 8493) {
  ...
  */
  ...
  /**
  * @dev Mint up to 20 hashtags at once. We will try our best to mint all but
         ...
         event and this doesn't scale. 20 should be OK, but for anything more, consider the
  ...
  */
  function mint(address _dst, string[] memory _hashtags) public payable {
    ...
    uint mintedCount = 0; 
    string[] memory tmpMintedHashtags = new string[](numberOfNfts);
    for (uint i = 0; i < numberOfNfts; i++) {
      string memory hashtag = _hashtags[i];

      if (hashtagToTokenId[hashtag] != 0) {
      ...
      hashtagToTokenId[hashtag] = tokenId;
      tmpMintedHashtags[mintedCount] = hashtag;
      mintedCount++;
    }

    // recreate array
    string[] memory mintedHashtags = new string[](mintedCount);
    for (uint i = 0; i < mintedCount; i++) {
      mintedHashtags[i] = tmpMintedHashtags[i];
    }
    ...
    emit UpdatedRegistry(hashtagTokenType, mintedHashtags);
  }
  ...
  */
  function redeem(address _dst, string[] memory _hashtags) public payable {
    ...
    // TODO: MIGHT NEED TO EMIT AN EVENT HERE ABOUT HOW MANY RES TOKENS BURNED
    emit UpdatedRegistry(hashtagTokenType, mintedHashtags);
  }
}
