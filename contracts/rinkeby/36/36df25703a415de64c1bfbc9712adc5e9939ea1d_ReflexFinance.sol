...
contract ReflexFinance is IERC20, Ownable {
    ...
    string private _name = "Reflex.Finance";
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        if (_isUniswap[sender] || _isUniswap[recipient]) {
            _transferUniswap(sender, recipient, amount);
        } else {
    ...
    function _getValues(uint256 tAmount, uint256 buySell) private view returns (uint256, uint256, uint256, uint256, uint256) {
    ...
    // @param buySell 0 is for neither buy nor sell, 1 is for buy, 0 is for sell
    function _getTValues(uint256 tAmount, uint256 buySell) private pure returns (uint256, uint256) {
        uint256 tFee = 0;

        if (buySell == 1) { // buy
            tFee = tAmount.mul(2).div(100);
        } else if (buySell == 2) { // sell
