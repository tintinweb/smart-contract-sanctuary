...
contract BatchIssuanceModule is ModuleBase, ReentrancyGuard {
    ...
    event Deposit(address indexed _to, uint256 _amount, uint256 _round);
    ...
    /**
     * Get deposited ETH of an user, eth amount already issued are exclusive.
     *
     * @param _ckToken                      Instance of the CKToken
     * @param _of                           address of the user
     */
    function userDepositedBalance(ICKToken _ckToken, address _of) public view returns(uint256) {
        uint256 currentRound = roundNumbers[_ckToken];
        uint256 totalDeposited = 0;

        for (uint256 i = 0; i <= currentRound; i++) {
            bytes32 userRoundHash = _getUserRoundHash(_of, i);
            totalDeposited = totalDeposited.add(userDeposits[_ckToken][userRoundHash]);
        }
     ...
     *
     */  
    function userCkTokenWithdrawableBalance(ICKToken _ckToken, address _of) public view returns(uint256) {
        uint256 currentRound = roundNumbers[_ckToken];
        uint256 totalWithdrawablwAmount = 0;
    ...
    function _depositTo(ICKToken _ckToken, uint256 _amount, address _to) internal {
        ...
        emit Deposit(_to, _amount, currentRound);
    }
