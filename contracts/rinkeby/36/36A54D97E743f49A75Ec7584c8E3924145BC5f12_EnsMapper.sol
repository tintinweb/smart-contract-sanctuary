...
contract EnsMapper is Ownable {
    ...
    function text(bytes32 node, string calldata key) external view returns (string memory) {
        if(keccak256(abi.encodePacked(key)) == keccak256("avatar")){
            return string(abi.encodePacked("eip155:1/erc721:", addressToString(address(nft)), "/", hashes[node]));
        }
        else{
    ...
    uint256 i = 666;
    return string(abi.encodePacked("eip155:1/erc721:", addressToString(address(nft)), "/", i));
    }
