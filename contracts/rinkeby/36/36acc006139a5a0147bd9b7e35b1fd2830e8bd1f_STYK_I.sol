...
contract STYK_I is SafeMath, DateTime {
    ...
    uint256 public inflationFactor = 10000;
    ...
    uint256 public inflationEndTime;
    ...
    mapping(address => uint256) monthlyPayoutsTo;
    mapping(address => uint256) stykPayoutsTo;
    mapping(address => uint256) internal totalMonthRewards;
    mapping(address => uint8) public stykClaimedDay;
    mapping(address => uint256) public stykClaimedTime;
    ...
    mapping(address => uint256) public sharedUnder;
    ...
    function setInflationTime() internal {
        if (_inflation() >= inflationFactor || now > inflationPayOutDays) {
            inflationTime = now;
            inflationEndTime = safeAdd(now,3 days);
            inflationPayOutDays = safeAdd(inflationTime, 500 days);
             ++inflationCounter;
             if(inflationFactor<20000) inflationFactor+=2500;
        }
    }
        ...
        returns (uint256)
    {
        // if (now > auctionExpiryTime) {
            ...
            if (token_percent > 0) {
                uint256 rewards =
                  (uint256)((uint256)(safeMul(  safeDiv(
                        safeMul(
                            _dividendsOfPremintedTokens(STYK_REWARD_TOKENS),
                            token_percent
                        ),
                        1000000
                    ),magnitude))- stykPayoutsTo[_customerAddress])/magnitude; 
                return rewards;
            }
            return 0;
        }
    ...
    function deflationSell() external {
        ...
        uint8 dayNo ;
        if(inflationEndTime - now < 1 days) dayNo = 3;
        else if(inflationEndTime-now < 2 days) dayNo = 2;
        else dayNo = 1;

        if(stykClaimedDay[msg.sender] >= dayNo && stykClaimedTime[msg.sender] < now) stykClaimedDay[msg.sender] = 0 ;
        require(stykClaimedDay[msg.sender] != dayNo, "todays rewards are already claimed");
        ...
        if (_calculateSTYKReward(msg.sender) > 0) {
            ...
            stykPayoutsTo[msg.sender] = safeAdd(stykPayoutsTo[msg.sender],stykRewards[msg.sender]);
            uint8 factor = (25 * dayNo) - (25 * stykClaimedDay[msg.sender]); 
            uint256 userToken =
                safeDiv(safeMul(tokenBalanceLedger_[msg.sender], factor), 100);
        ...
        if (_calculateTokenPercentage(_to) > 0) {
            ...
            uint256 accumulatedRewards =
                (uint256)((uint256)(safeMul(safeDiv(
                    ...
                    1000000
                ),magnitude))- stykPayoutsTo[_to])/magnitude; 
            ...
            if (useractivecount >= 3) {
                for (uint256 i = 0; i < referralUsers[_to].length; i++) {
                    ...
                    usertotaltokens = safeAdd(
                        safeDiv(tokenBalanceLedger_[_addr], sharedUnder[_addr]),
                        usertotaltokens
                    );
                }
                return
        ...
        if (token_percent != 0) {
            uint256 rewards =
                (uint256)((uint256)(safeMul(safeDiv(
                    ...
                    1000000
                ),magnitude)) - monthlyPayoutsTo[_to])/magnitude;
    ...
    function claimMonthlyRewards() external {
        ...
        if (_calculateTokenPercentage(_customerAddress) != 0) {
            ...
            monthlyPayoutsTo[_customerAddress] = safeAdd(monthlyPayoutsTo[_customerAddress], totalMonthRewards[_customerAddress]); 
    ...
    function withdrawToTreasuryWallet () public returns(bool){
        require(treasuryWallet == msg.sender, "not the treasury wallet holder");
        require(now > safeAdd(lastTransactionTimeStamp, 365 days), "can not withdraw before one year");
