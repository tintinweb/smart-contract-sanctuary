...
contract sDepo is IsDEPO, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgDEPO public gDEPO; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked DEPO", "sDEPO", 18) ERC20Permit("Staked DEPO") {
    ...
    function setgDEPO(address _gDEPO) external {
        require(
            msg.sender == initializer,
            "Initializer:  caller is not initializer"
        );
        require(address(gDEPO) == address(0), "gDEPO:  gDEPO already set");
        require(_gDEPO != address(0), "gDEPO:  gDEPO is not a valid contract");
        gDEPO = IgDEPO(_gDEPO);
    }
    ...
    /**
        @notice increases rDEPO supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sDEPO of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sDEPO
    ...
    // toG converts an sDEPO balance to gDEPO terms. gDEPO is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gDEPO.balanceTo(amount);
    }

    // fromG converts a gDEPO balance to sDEPO terms. sDEPO is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gDEPO.balanceFrom(amount);
    }

    // Staking contract holds excess sDEPO
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply
                .sub(balanceOf(stakingContract))
                .add(gDEPO.balanceFrom(IERC20(address(gDEPO)).totalSupply()))
