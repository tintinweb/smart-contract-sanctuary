...
contract animalMain is  ERC721,Ownable {
  ...
  using SafeMath for uint256;
    string public name_ = "1090337";

  struct animal {
      bytes32 dna; //dna
      uint8 star; //幾星級
      uint16 roletype; //(哪種動物)
      uint8 attribute; //屬性
  }

  animal[] public animals;
  string public symbol_ = "37";
  ...
  function seeAnimalAttribute(uint256 _tokenId) public view returns (uint8 attribute) {
    return animals[_tokenId].attribute;
  }
  ...
  function createAnimal() public payable {
      ...
      //1(鼠) : 2(牛) : 3(虎) : 4(兔) : 5(龍)
      roletype=uint(random()) % 5+1;
      
      //1(水) : 2(火) : 3(木) : 4(金) : 5(土)
      attribute=uint(random()) % 5+1;
      
      //動手玩創意，可以限制每次建立動物需要花費多少ETH
       require(msg.value == 1 wei);
       
      uint id = animals.push(animal(dna, uint8(star), uint8(roletype), uint8(attribute))) - 1;
