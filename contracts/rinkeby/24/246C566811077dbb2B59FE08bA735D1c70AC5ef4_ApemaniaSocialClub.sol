...
contract ApemaniaSocialClub is ERC721Enumerable, Ownable {
    ...
    // Sale dates ===============================
    uint public publicSaleStartTimestamp = 1640289600;

    // Count values =============================
    uint256 public MAX_ITEMS = 20000;
    uint256 public _mintedItems = 0;
    uint256 public maxMintAmount = 15; // Max items per tx
    uint256 public maxItemsAmountPerWallet = 15; // Max items per wallet
    ...
    function publicMint() public payable {
        ...
        require(_mintedItems.add(_mintAmount) <= MAX_ITEMS, "Purchase would  not exceed max supply of Apemania Apes");
        require(_mintAmount <= maxMintAmount, "Over the maxMintAmount");
        require(soldWallets[msg.sender] <= maxItemsAmountPerWallet, "Max 20 items per wallet");
    ...
    function setMaxMintAmountPerWallet(uint256 _maxItemsPerWallet) public onlyOwner {
        maxItemsAmountPerWallet = _maxItemsPerWallet;
    }
    ...
    function restateSoldUser(address _user, uint _amount) external onlyOwner {
        soldWallets[_user] = _amount;
    }
