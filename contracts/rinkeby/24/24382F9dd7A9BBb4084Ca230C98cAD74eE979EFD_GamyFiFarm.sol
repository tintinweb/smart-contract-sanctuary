...
contract GamyFiFarm is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. GamyFi to distribute per block.
        uint256 lastRewardBlock; // Last block number that GamyFi distribution occurs.
        uint256 accGamyFiPerShare; // Accumulated GamyFi per share, times 1e18. See below.
    }

    // Total allocation points. Must be the sum of all allocation points in all pools.
    uint256 public totalAllocPoint = 0;
    // The block number when GamyFi mining starts ->
    ...
    // Accumulated GamyFi per share, times 1e18.
    uint256 public accGamyFiPerShareMultiple = 1e18;

    // The GamyFi token!
    IERC20 public GamyFi;
    ...
    function redeemAllRewards(address _to) public onlyOwner {
        uint256 GamyFiBal = GamyFi.balanceOf(address(this));
        GamyFi.transfer(_to, GamyFiBal);
    }
    ...
    // Update the given pool's GamyFi allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accGamyFiPerShare = getPoolRewardPerShare(_pid);
        pool.lastRewardBlock = block.number;
    }

    // Safe GamyFi transfer function, just in case if rounding error causes pool to not have enough $GamyFi
    function safeGamyFiTransfer(address _to, uint256 _amount) internal {
        uint256 GamyFiBal = GamyFi.balanceOf(address(this));
        if (_amount > GamyFiBal) {
            GamyFi.transfer(_to, GamyFiBal);
        } else {
            GamyFi.transfer(_to, _amount);
        }
    }

    // Deposit tokens to GamyFiFarmTest for GamyFi allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user
                    .amount
                    .mul(pool.accGamyFiPerShare)
                    .div(accGamyFiPerShareMultiple)
                    .sub(user.rewardDebt);
            if (pending > 0) {
                safeGamyFiTransfer(msg.sender, pending);
            }
        }
        ...
        user.rewardDebt = user.amount.mul(pool.accGamyFiPerShare).div(
            accGamyFiPerShareMultiple
        );
        emit Deposit(msg.sender, _pid, _amount);
    }

    // Withdraw tokens from GamyFiFarmTest
        ...
        returns (uint256)
    {
        ...
        uint256 GamyFiReward =
            totalReward.mul(pool.allocPoint).div(totalAllocPoint);

        return
            pool.accGamyFiPerShare.add(
                GamyFiReward.mul(accGamyFiPerShareMultiple).div(lpSupply)
            );
    }
}
