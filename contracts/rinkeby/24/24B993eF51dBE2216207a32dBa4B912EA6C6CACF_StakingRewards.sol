	...
	RewardsDistributionRecipient
{
	...
	function earned(address account) public view returns (uint256) {
		return
			store
				...
				.div(1e18)
				.add(store.getUserGogoRewards(account));
	}
	...
	function getReward() public nonReentrant {
		...
		uint256 reward = store.getUserGogoRewards(msg.sender);
		if (reward > 0) {
			store.setUserGogoRewards(msg.sender, 0);
	...
	function updateReward(address account) public {
		...
		if (account != address(0)) {
			store.setUserGogoRewards(account, earned(account));
	...
	function getRewardFor(address user) external onlyManager {
		updateReward(user);
		uint256 reward = store.getUserGogoRewards(user);
		if (reward > 0) {
			store.setUserGogoRewards(user, 0);
...
contract GovStakingStorage is Ownable {
	...
	function setUserGogoRewards(address user, uint256 amount)
		external
		isAllowed
	{
		UserInfo storage info = userInfo[user];
		info.gogoRewards = amount;
	}
	...
	function removeRewardMultiplier(address user) external isAllowed {
		UserInfo storage info = userInfo[user];
	...
	// views
	function getTotalLockeGogo() public view returns (uint256) {
		return totalLockedGogo;
	}

	function getTotalRewardRates() public view returns (uint256) {
	...
	function getUserInformations(address user)
		external
		view
	...
	function getUserGogoRewards(address user) public view returns (uint256) {
		return userInfo[user].gogoRewards;
	}
