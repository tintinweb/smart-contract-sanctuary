...
contract Sk8ers is ERC721Enumerable, Ownable {
    ...
    uint256 public constant MAX_SK8ERS = 8888;
    uint256 public constant RESERVES = 100;
    uint256 public constant MINT_LIMIT = 20;
    uint256 private _saleTime = 1639026900;
    uint256 private _price = 80000000000000000; // 0.08 eth
    string private _baseTokenURI;

    mapping(address => bool) public _preSaleAddresses;

    event CreateSk8er(uint256 indexed id);

    constructor(string memory baseURI, address[] memory preSaleAddresses) ERC721("Sk8ers", "SK8") {
        setBaseURI(baseURI);
        for(uint256 i = 0; i < preSaleAddresses.length; i++) {
            _preSaleAddresses[preSaleAddresses[i]] = true;
        }
    }

    modifier preSaleIsOpen() {
        require(block.timestamp >= _saleTime - 600, "The Sk8ers presale has not yet started");
    ...
    function addPreSaleAddress(address _address) public onlyOwner {
        _preSaleAddresses[_address] = true;
    }

    function isValidPreSaleAddress(address _address) public view returns (bool) {
        return _preSaleAddresses[_address] == true;
    }

    function preSaleMint(uint256 _count) public payable preSaleIsOpen {
        uint256 total = totalSupply();
        require(isValidPreSaleAddress(msg.sender), "Invalid address");
    ...
    function reserve(uint256 _count) public onlyOwner {
        uint256 total = totalSupply();
        require(block.timestamp < _saleTime - 600, "Presale has already started");
