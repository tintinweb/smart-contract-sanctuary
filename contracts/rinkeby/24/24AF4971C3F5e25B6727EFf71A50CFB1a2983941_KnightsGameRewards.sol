...
contract KnightsGameRewards is EIP712, Ownable {
    ...
    mapping(address => RewardContract) public contracts;
    ...
    event Withdraw(address contractAddress, address owner, uint amount);
    ...
    constructor() EIP712("KnightsGameRewards", "1.0.0") {}

    function claimRewards(address contractAddress, uint amount, bytes calldata signature) external {
        RewardContract storage _contract = contracts[contractAddress];
        require(_contract.active, "token contract is not active");
        require(_signerAddress == recoverAddress(contractAddress, msg.sender, amount, accountNonce(msg.sender), signature), "invalid signature");

        IERC20 rewards = IERC20(contractAddress);
    ...
    function _hash(address contractAddress, address account, uint amount, uint nonce) internal view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256("KnightsGameRewards(address contractAddress,address account,uint256 amount,uint256 nonce)"),
                        contractAddress,
                        account,
                        amount,
                        nonce
    ...
    function recoverAddress(address contractAddress, address account, uint amount, uint nonce, bytes calldata signature) public view returns(address) {
        return ECDSA.recover(_hash(contractAddress, account, amount, nonce), signature);
    }
