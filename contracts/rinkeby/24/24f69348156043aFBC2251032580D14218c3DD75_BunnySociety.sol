...
contract BunnySociety is Ownable, ERC721 {
    ...
    uint256 public maxBunnyMint = 10;
    ...
    address[] public BunnyPresale;


    constructor(
        
    ) ERC721("Bunny Society", "BS") {
        supplyL = 100;
        presaleSupplyL = 10;
        baseURI = "";
    }
    ...
    function sumon_maxBunnyMint(uint256 _mintLimit) external onlyOwner {
        maxBunnyMint = _mintLimit;
    }
    ...
    function populate_PreSaleWhitelist(address[] calldata preSaleWalletAddresses) external onlyOwner {
        delete BunnyPresale;
        BunnyPresale = preSaleWalletAddresses;
  	    return;
  	}
    ...
    function buy(uint nft) external payable {
        ...
        require(nft <= maxBunnyMint, "Too many");
    ...
    function buy_presale(uint nft) external payable {
        ...
        require(nft <= maxBunnyMint, "Too many"); //YES
    ...
    function isWalletInPreSale(address _address) public view returns (bool) {
        for(uint256 i = 0; i < BunnyPresale.length; i++) {
            if (BunnyPresale[i] == _address) {
                return true;
            }
        }
        return false;
    ...
    function sumon(uint256[] memory token_ids) public meetsOwnership(token_ids) {
        require(sumonActive, "Bunny Summonings is not active");
        require(sumonSupply > 0, "No Summon Supply left");
        require(token_ids.length >= sumonCost, "Not enough Bunnys provided");
