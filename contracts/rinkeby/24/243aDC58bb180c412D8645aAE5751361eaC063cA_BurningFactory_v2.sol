...
contract BurningFactory_v2 {
    ...
    function burn(address _tokenAddress, uint256 _amount, bytes32 _salt) public onlyBurner {
        ...
        require(Token_v2(_tokenAddress).balanceOf(burnAddress) >= _amount, 'burn failed as insufficient balance');

        tokenAddress = _tokenAddress;
        amount = _amount;
        recipient = address(0);

        bytes memory bytecode = type(Burning_v2).creationCode;
        assembly {
    ...
    function transfer(address _tokenAddress, address _recipient, uint256 _amount, bytes32 _salt) public onlyBurner {
        address burnAddress = predict(_salt);
        require(Token_v2(_tokenAddress).balanceOf(burnAddress) >= _amount, 'transfer failed as insufficient balance');
    ...
    function predict(bytes32 _salt) public view returns(address){
        return address(uint(keccak256(abi.encodePacked(
            ...
            address(this),
            _salt,
            keccak256(abi.encodePacked(type(Burning_v2).creationCode))
        ))));
    }
}
...
contract Burning_v2 {
    ...
    constructor() public {
        ...
        address token = BurningFactory_v2(factory).tokenAddress();
        uint256 amount = BurningFactory_v2(factory).amount();
        address recipient = BurningFactory_v2(factory).recipient();

        if(recipient == address(0)){
            Token_v2(token).burn(amount);
        } else {
            Token_v2(token).transfer(recipient, amount);
        }
