...
contract NoahArkTreasury is NoahArkAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    constructor(
        ...
        require(_NAC != address(0), "Zero address: NAC");
        NAC = INAC(_NAC);
    ...
    /**
        @notice allow approved address to deposit an asset for NAC
        @param _amount uint
    ...
    function deposit(
        ...
        // mint NAC needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        NAC.mint(msg.sender, send_);
    ...
    /**
        @notice allow approved address to burn NAC for reserves
        @param _amount uint
        @param _token address
     */
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        ...
        uint256 availableDebt = sNAC.balanceOf(msg.sender).sub(sNAC.debtBalances(msg.sender));
        require(value <= availableDebt, "Exceeds debt limit");

        sNAC.changeDebt(value, msg.sender, true);
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        sNAC.changeDebt(value, msg.sender, false);
    ...
    /**
        @notice allow approved address to repay borrowed reserves with NAC
        @param _amount uint
     */
    function repayDebtWithNAC(uint256 _amount) external {
        require(permissions[STATUS.DEBTOR][msg.sender], "Not approved");

        NAC.burnFrom(msg.sender, _amount);

        sNAC.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, address(NAC), _amount, _amount);
    }
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SNAC) {
            // 9
            sNAC = IsNAC(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(NAC.totalSupply().sub(totalDebt));
    }

    /**
        @notice returns NAC valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(NAC)).decimals()).div(10**IERC20Metadata(_token).decimals());
