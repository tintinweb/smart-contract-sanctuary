...
/**
   @title Collection Interface
        This interface provides a skeleton of Collection contract
        Note: The interface supports a feature that one collection can have multiple sub-collections
        However, this feature has not yet released in 'Collection_v1'
*/
interface ICollection {
    ...
    //  Emit when minting a single NFT721 item of a collection
    ...
    //  Emit when minting a batch of NFT721 items of a collection
    ...
    /**
       @notice Update new address of Registry contract
       @dev Caller must be Admin of Spores Network
       @param _newRegistry          Address of new Registry contract
    */
    function updateRegistry(address _newRegistry) external;

    /**
       @notice Add another sub-collection in this collection
       @dev Caller must be Owner
       @param _maxEdition           Cap a number of editions of a sub-collection
       @param _requestId            An identification of a request (BE requirement)
    ...
    */
    ...
    /**
       @notice Mint NFT item to a receiver `to`
       @dev Caller must be Owner
            Requirement: 
                + Signature from Verifier
                + Sub-collection ID must be correct. Collection, in this version, has only one sub-collection only
                + Current minted NFT + newly request <= maxEditions[subcollectionId]
    ...
    /**
       ...
       @dev Caller must be Owner
            Requirement: 
                + Signature from Verifier
                + Sub-collection ID must be correct. Collection, in this version, has only one sub-collection only
                + Current minted NFT + newly request <= maxEdition
                + `_tokenIds` and `_uris` must match respectively
       @param _to                    Receiver Address
...
/**
   @title Collection contract
        This contract supports holding Collection of NFT items
        Note: This version only supports one collection - one sub-collection
        When a contract is deployed, constructor calls SporesRegistry contract to add itself
        Hence, a signature from Verifier is required for Collection creation
        This implementation might be crucial in this version, but could be depricated in future versions
   @dev This contract handles NFT Token using ERC-721 (https://eips.ethereum.org/EIPS/eip-721)
*/
contract Collection is Ownable, ICollection, ERC721URIStorage {
    ...
    //  Admin address of Spores Network
    ...
    //  Current version supports one Sub-collection only in one collection
    mapping(uint256 => SubCollection) public subcollections;

    modifier onlyAdmin() {
        require(_msgSender() == admin, "Collection: Unauthorized");
        _;
    }

    /**
       @notice Initialize Collection Name, Collection Symbol, Max Editions
       @param _admin             Address of Admin of Spores Network
       @param _registry          Address of SporesRegistry contract
       @param _collectionId      Integer number of Collection
       @param _maxEdition        Cap a number of editions of a sub-collection
       @param _requestId         An identification of a request (BE requirement)
       @param _name              Initialized Collection Name
       @param _symbol            Initialized Collection Symbol
       @param _signature         A signature from Verifier
    */
    constructor(
        address _admin,
        ...
        //  Set Sub-collection = 1 and Max Editions of Sub-collection
        subcollectionId++;
        subcollections[subcollectionId].maxEdition = _maxEdition;
        admin = _admin;
        registry = SporesRegistry(_registry);
        registry.addCollectionByConstructor(
            address(this),
            admin,
            collectionId,
            _maxEdition,
    ...
    /**
       ...
       @param _newRegistry          Address of new Registry contract
    */
    function updateRegistry(address _newRegistry) external override onlyAdmin {
        require(_newRegistry != address(0), "Collection: Set zero address");
        registry = SporesRegistry(_newRegistry);
    }

    /**
       @notice Not support in this version
    */
    function addSubCollection(uint256 _maxEdition, uint256 _requestId, bytes calldata _signature)
        external
        override
    {}
    ...
    */
    function mint(
        ...
        //  If `_subcollectionId` not existed, `maxEdition` is likely 0
        (uint256 _collectionId, uint256 _subcollectionId) =
        ...
        require(
            subcollections[_subcollectionId].maxEdition >=
                subcollections[_subcollectionId].mintedAmt + 1,
            "Collection: Reach max edition"
        );

        //  A minting request is required a signature from Verifier
        //  Call to verify `_signature`
        ...
        subcollections[_subcollectionId].mintedAmt++;
        emit CollectionMintSingle(_to, address(this), _tokenId);
    }
    ...
    /**
       ...
       @dev Caller must be Owner
            Requirement: 
                ...
                + Current minted NFT + newly request <= maxEdition
                + `_tokenIds` and `_uris` must match respectively
                + Batch of NFT items should be minted to one sub-collection only
       @param _to                    Receiver Address
       @param _tokenIds              Array of Token IDs
       @param _uris                  Array of Token URIs
       @param _signature             A signature from Verifier 
    */
    function mintBatch(
        ...
        require(
            _amount == _uris.length,
            "Collection: TokenIDs and URIs not match"
        );
        //  If `_subcollectionId` not existed, `maxEdition` is likely 0
        _checkTokenIdRequirement(_tokenIds);
        (uint256 _collectionId, uint256 _subcollectionId) =
            _getCollection(_tokenIds[0]);
        require(
            _collectionId == collectionId,
            "Collection: Invalid collection"
        );
        uint256 _mintedAmt = subcollections[_subcollectionId].mintedAmt;
        require(
            subcollections[_subcollectionId].maxEdition >= _mintedAmt + _amount,
            "Collection: Reach max edition"
        );
        ...
        //  Mint NFT items as requested
        for (uint256 i; i < _amount; i++) {
            _safeMint(_to, _tokenIds[i]);
            _setTokenURI(_tokenIds[i], _uris[i]);
        }
        subcollections[_subcollectionId].mintedAmt = _mintedAmt + _amount;
        emit CollectionMintBatch(_to, address(this), _tokenIds, _amount);
    }

    function _checkTokenIdRequirement(uint256[] calldata _ids) private pure {
        //  TokenId is created following a rule: //  Collection ID + Sub-collection ID + Edition Number
        //  Requirement: Minting request TokenIds should be in one sub-collection only
        //  Go through all `_ids`. If valid, `_collection` and `_subcollection` must remain unchanged
        uint256 slice1 = _ids[0] / 10**12;
        uint256 slice2;
        for (uint256 i = 1; i < _ids.length; i++) {
            slice2 = _ids[i] / 10**12;
            require(slice1 == slice2, "Collection: Invalid TokenIds");
        }
    }

    function _getCollection(uint256 _tokenId)
        private
        pure
        returns (uint256 _collection, uint256 _subcollection)
    {
        //  TokenId is created following this rule:
        //  Collection ID + Sub-collection ID (4 digits) + Edition Number (12 digits)
        //  For example: TokenId = 2300040...088
        //  Collection ID = 23, Sub-collection ID = 4, Edition Number = 88
        _collection = _tokenId / 10**16;
        _subcollection = (_tokenId % 10**16) / 10**12;
    }
}