...
contract MeatToken is ERC20, Ownable {
    ...
    uint256 public ALPHA_EMISSIONS_RATE = 115740700000000000; //115740700000000
    uint256 public BETA_EMISSIONS_RATE  = 57870350000000000; //57870350000000
    uint256 public CLAIM_END_TIME = 1672491599;

    address nullAddress = 0x0000000000000000000000000000000000000000;

    address public alphaWolfAddress;

    //Mapping of wolf to timestamp
    mapping(uint256 => uint256) internal tokenIdToTimeStamp;

    //Mapping of wolf to staker
    mapping(uint256 => address) internal tokenIdToStaker;

    //Mapping of staker to wolf
    mapping(address => uint256[]) internal stakerToTokenIds;

    constructor() ERC20("Meat", "MEAT") {}
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                IERC721(alphaWolfAddress).ownerOf(tokenIds[i]) == msg.sender &&
                    tokenIdToStaker[tokenIds[i]] == nullAddress,
                "Token must be stakable by you!"
            );

            IERC721(alphaWolfAddress).transferFrom(
                msg.sender,
                address(this),
                tokenIds[i]
            );
    ...
    function unstakeAll() public {
        ...
        for (uint256 i = stakerToTokenIds[msg.sender].length; i > 0; i--) {
            ...
            uint256 rate = tokenId <= 5000 ? ALPHA_EMISSIONS_RATE : BETA_EMISSIONS_RATE;
    ...
    function unstakeByIds(uint256[] memory tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            uint256 rate = tokenIds[i] <= 5000 ? ALPHA_EMISSIONS_RATE : BETA_EMISSIONS_RATE;
    ...
    function claimByTokenId(uint256 tokenId) public {
        ...
        _mint(
            msg.sender,
            ((block.timestamp - tokenIdToTimeStamp[tokenId]) * rate));
    ...
    function claimAll() public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            totalRewards =
                totalRewards +
                ((block.timestamp - tokenIdToTimeStamp[tokenIds[i]]) * rate);
    ...
    function setAlphaWolfAddress(address _alphaWolfAddress) public onlyOwner {
        alphaWolfAddress = _alphaWolfAddress;
        return;
    }

    function burn(address tokenOwner, uint256 tokenAmount) external {
        require(msg.sender == alphaWolfAddress, "Not authorized to call this method");
        _burn(tokenOwner, tokenAmount);
    }
}
