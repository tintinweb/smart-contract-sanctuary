...
import "./interfaces/IPamperedPugs.sol";
import "./interfaces/IPamperedPugsTraits.sol";

contract PamperedPugs is IPamperedPugs, ERC721Enumerable, Ownable, Pausable {
    ...
    event PugsMinted(address indexed owner,uint256 indexed tokenId, uint256 blockNum, uint256 timeStamp);
    event PugsStolen(address indexed owner, address indexed previous, uint256 indexed tokenId, uint256 blockNum, uint256 timeStamp);
    event PugsBurned(uint256 indexed tokenId, uint256 blockNum, uint256 timeStamp);
    ...
    mapping(uint256 => PamperedPugsStruct) private tokenTraits;
    ...
    // reference to Pampered Pugs Traits
    IPamperedPugsTraits public pugsTraits;
    ...
    constructor(uint256 _maxTokens) ERC721("PugsNFT", "PamperedPugs") {
    ...
    modifier requireContractsSet() {
        require(address(pugsTraits) != address(0), "Contracts not set");
        _;
    }
    ...
    function setContracts(address _pugsTraits) external onlyOwner {
        pugsTraits = IPamperedPugsTraits(_pugsTraits);
    }
    ...
    */
    function mint(address recipient, uint256 seed) external override whenNotPaused {
        ...
        if(tx.origin != recipient) {
            // Stolen!
            emit PugsStolen(recipient, tx.origin, minted, block.number, block.timestamp);
        }
    ...
    */
    function generate(address recipient, uint256 tokenId, uint256 seed) public returns (PamperedPugsStruct memory t) {
        t = selectTraits(seed);
        tokenTraits[tokenId] = t;
        emit TokenMinted(recipient, tokenId, block.number, block.timestamp);
        emit TestTokenMinted(recipient, tokenId);
        emit PugsMinted(recipient, tokenId, block.number, block.timestamp);
        return t;
    }
    ...
    */
    function selectTraits(uint256 seed) internal view returns (PamperedPugsStruct memory t) {    
        seed >>= 16;
    ...
    */
    function structToHash(PamperedPugsStruct memory s) internal pure returns (uint256) {
        return uint256(keccak256(
            abi.encodePacked(s.body)
        ));
    }
    ...
    */
    function burn(uint256 tokenId) external override whenNotPaused {
        require(admins[_msgSender()], "Only admins can call this");
        _burn(tokenId);
        emit PugsBurned(tokenId, block.number, block.timestamp);
    }
    ...
    */
    ...
    function getTokenTraits(uint256 tokenId) external view override blockIfChangingAddress blockIfChangingToken(tokenId) returns (PamperedPugsStruct memory) {
    ...
    function tokenURI(uint256 tokenId) public view override blockIfChangingAddress blockIfChangingToken(tokenId) returns (string memory) {
        ...
        return pugsTraits.tokenURI(tokenId);
    }
...
interface IPamperedPugs is IERC721Enumerable {
    
    // Pugs NFT struct
    struct PamperedPugsStruct {
        uint8 body;
    }
    ...
    function getTokenTraits(uint256 tokenId) external view returns (PamperedPugsStruct memory);
