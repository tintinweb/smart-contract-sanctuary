...
import "../AssetPermission/XTokenWrapper.sol";
...
contract DOTCManager is IdOTC, AdminFunctions, ReentrancyGuard {
    ...
    constructor(address _xtokenAddress, address _permission) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        xtokenAddress = _xtokenAddress;
        permissionAddress = _permission;
    }

    // Freeze Function new order
    // grant Role  function
    ...
    function makeOffer(
        ...
        public
        allowedAsset(_tokenInAddress)
        allowedAsset(_tokenOutAddress)
        _accountIsTierTwo
        _accountSuspended
        nonReentrant
    ...
    {
        require(
            IERC20(_tokenInAddress).balanceOf(msg.sender) >= (_amountIn + (_amountIn.mul(feeAmount).div(BPSNUMBER))),
        ...
        IEscrow(escrow).setMakerDeposit(currentOfferId);
        safeTransfertokenIn(_tokenInAddress, _amountIn);
        payFee(_tokenInAddress, _amountIn.mul(feeAmount).div(BPSNUMBER));
        emit CreatedOffer(
            currentOfferId,
            msg.sender,
            _tokenInAddress,
    ...
    function setOffer(
        ...
        Offer memory _offer =
            Offer({
                ...
                unitPrice: _amountOut.mul(BPSNUMBER).div(_amountIn),
        ...
        returns (uint256 takenOderId)
    {
        ...
        if (_amount == allOffers[offerId].amountOut) {
            ...
            amountToSend = _amount.mul(BPSNUMBER).div(allOffers[offerId].unitPrice);
            allOffers[offerId].amountOut = allOffers[offerId].amountOut.sub(amountToSend);
            allOffers[offerId].takenAmount = allOffers[offerId].takenAmount.add(_amount);
        }
        ...
        takerOrders[orderId] = Order(offerId, _amount, msg.sender, amountToSend);
        IEscrow(escrow).withdrawDeposit(offerId, orderId);
        safeTransferAsset(allOffers[offerId].tokenOutAddress, allOffers[offerId].maker, msg.sender, _amount);
        emit CreatedOrder(offerId, orderId, _amount, msg.sender, amountToSend);
        return orderId;
    }
     ...
     */
    function cancelOffer(uint256 offerId) public can_cancel(offerId) nonReentrant returns (bool success) {
        Offer memory offer = allOffers[offerId];
        delete allOffers[offerId];
        uint256 _amountToSend = offer.unitPrice.div(offer.amountOut);
        require(_amountToSend > 0, "can not cancel");
        IEscrow(escrow).cancelDeposit(offerId, offer.tokenInAddress, offer.maker, _amountToSend);
        emit CancledOffer(offerId, msg.sender, _amountToSend);
        return true;
    }

    /**
        @dev getOwner returns the address of the maker
        @param offerId uint256 the Id of the order
        @return owner address
     */
    function getOwner(uint256 offerId) external view override returns (address owner) {
     ...
     *   @param offerId uint256
     */
    function freezeXOffer(uint256 offerId) public returns (bool hasfrozen) {
        require(hasRole(dOTC_Admin_ROLE, _msgSender()), "must have dOTC Admin role");
        IEscrow(escrow).freezeOneDeposit(offerId, msg.sender);
     ...
     *  @return Offer[] memory
     */
    function getOffersFromAddress(address account) public view returns (Offer[] memory) {
     ...
     *  @return Offer[] memory
     */
    function getTakenOffersFromAddress(address account) public view returns (Offer[] memory) {
    ...
    function safeTransferAsset(
        ...
        require(IERC20(erc20Token).balanceOf(_from) >= _amount, "Insufficient balance");
    ...
    function isWrappered(address token) internal view returns (bool wrapped) {
        return XTokenWrapper(xtokenAddress).tokenToXToken(token) != address(0);
    }

    /**
     *   @dev Asset is Approve on the Swarm market
     *   @param tokenAddress address
     */
    modifier allowedAsset(address tokenAddress) {
        require(isWrappered(tokenAddress), "Asset not allowed");
        _;
    }
    ...
    */
    ...
    event CancledOffer(uint256 offerId, address canceledBy, uint256 amountToReceive);

    event freezeOffer(uint256 offerId, address freezedBy);
}
...
interface IdOTC {
    ...
    */
    ...
    struct Offer {
        ...
        uint256 unitPrice;
        uint256 amountOut; // take amount ie the amount to be receive by the maker
        OfferType offerType; // can be PARTIAL or FULL
        uint256 takenAmount; // amount taken for partial other
    ...
    function getOwner(uint256 offerId) external view returns (address owner);
...
contract AdminFunctions is AccessControl {
    ...
    uint256 public feeAmount = 3000;
    uint256 internal BPSNUMBER = 10000;
    address internal feeAddress;
    address internal xtokenAddress;
     ...
     * - the caller must have ``role``'s admin role.
     */
    function setdOTCAdmin(address _dOTCAdmin) public {
    ...
    function setEscorwLinker() public isAdmin returns (bool status) {
        IEscrow(escrow).setdOTCAddress(address(this));
        return true;
    }

    function freezeEscrow() public isAdmin returns (bool status) {
        IEscrow(escrow).freezeEscrow(msg.sender);
        return true;
    }

    function setXtokenAddress(address _contractAddress) public isAdmin returns (bool status) {
        xtokenAddress = _contractAddress;
        return true;
    }
 ...
 */
interface IEscrow {
    function setMakerDeposit(uint256 _offerId) external returns (bool);
