...
contract Router is OwnableUpgradeable, IRouter {
     ...
     * @param jobConditionCreations - number of condition creations rest in job
    ...
    /**
     * @dev LP => FEED => agreementID and FEED => LP => agreementID
     */
    mapping(address => mapping(address => uint256)) LPtoFeedAgreement;
    mapping(address => mapping(address => uint256)) FeedtoLPAgreement;

    /**
     * @dev FEED => agreementID
     */
    mapping(address => uint256) FeedtoAgreement;
    ...
    modifier onlyAgreedFeed(
        ...
        require(
            feedtoNumberMap[feedAddress] != 0 &&
                feedRegistry[feedtoNumberMap[feedAddress]].isActive,
            "Agreement: only active feed allowed"
        );
        agreement storage _agreement = agreements[
            LPtoFeedAgreement[lpAddress][feedAddress]
        ];
        require(
            _agreement.timeStop <= block.timestamp &&
                (
                    /* (isCreate && _agreement.jobConditionCreations > 0) || */
                    (isCreate && _agreement.jobConditionResolves > 0)
                ),
    ...
    function registerFeed(
        ...
        require(feedtoNumberMap[feedAddress] == 0, "feed exist");
     ...
     * @param lpRegs lp registration list
    ...
    function createCondition(
        ...
        for (uint256 index = 0; index < lpRegs.length; index++) {
            ...
            if (checkLPtoCoreType(lpRegs[index], coreType)) {
                ILP(lpRegistry[lpRegs[index]].pool).createConditionTest(
                    oracleConditionID,
                    coreType,
                    rates,
                    timestamp,
    ...
    function resolveCondition(
        ...
        for (uint256 index = 0; index < lpRegs.length; index++) {
            ...
            ILP(lpRegistry[lpRegs[index]].pool).resolveConditionTest(
                conditionID,
                outcomeWin
            );
        }
    ...
    function createAgreement(
        ...
        emit ProposalCreated(msg.sender, agreements.length - 1, agreementName);
    }
    ...
    function cancelAgreement(uint256 agreementID) external override {
        require(FeedtoAgreement[msg.sender] == agreementID, "Incorrent ID");
        require(agreements[agreementID].timeStop == 0, "Agreement job in work");
...
interface ILP {
    ...
    function resolveConditionTest(uint256 conditionID_, uint128 outcomeWin_)
        external;
