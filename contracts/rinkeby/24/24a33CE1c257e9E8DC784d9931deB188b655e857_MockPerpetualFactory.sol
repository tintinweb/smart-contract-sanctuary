...
contract MockPerpetualFactory is PerpetualFactory {
    ...
    function createPerpetual(
        ...
        perpetual.fCurrentAMMExposureEMA[0] = ONE_64x64.neg();
        perpetual.fCurrentAMMExposureEMA[1] = ONE_64x64;
        // initial funding
...
contract PerpetualUpdateFunctions is PerpetualRebalanceFunctions {
    ...
    /**
     * @dev     Update the fUnitAccumulatedFunding variable in perpetual.
     ...
     *
     * @param   _perpetual     The reference of perpetual storage.
     * @param   _fTimeElapsed  The elapsed time since last update in same unit
     *                         as FUNDING_INTERVAL_SEC, i.e. seconds in ABDK64.64 format.
     */
    function _accumulateFunding(PerpetualData storage _perpetual, int128 _fTimeElapsed) internal {
        // determine payment in collateral currency for 1 unit of base currency \
        // (e.g. USD payment for 1 BTC for BTCUSD)
        int128 fInterestPaymentLong = _fTimeElapsed.div(FUNDING_INTERVAL_SEC).mul(_perpetual.fCurrentFundingRate);
     ...
     * @param   _iPoolIndex       The pool id. 64x64 fixed point
     * @param   _iCurrentTime     The current timestamp, uint256
     */
    function _accumulateFundingInPool(uint256 _iPoolIndex, uint256 _iCurrentTime) internal {
        LiquidityPoolData storage liquidityPool = liquidityPools[_iPoolIndex];
        if (liquidityPool.iLastFundingTime >= _iCurrentTime) {
            // invalid time
            return;
        }
        // _iCurrentTime>iLastFundingTime, so safe:
        uint256 iTimeElapsed = _iCurrentTime - liquidityPool.iLastFundingTime;
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            _accumulateFunding(perpetual, fTimeElapsed);
        }
        liquidityPool.iLastFundingTime = _iCurrentTime;
    }
     ...
     * @return  The index price for the given perpetual
     */
    function _getInsurancePremiumEMA(PerpetualData storage _perpetual) internal view returns (int128) {
        return _perpetual.state != PerpetualState.NORMAL ? 0 : _perpetual.currentPremiumEMA.fPrice;
    }

    /**
     * Update the funding rate of each perpetual that belongs to the given liquidity pool
     * @param   _iPoolIndex The index for the liquidity pool.
     */
    function _updateFundingRatesInPool(uint256 _iPoolIndex) internal {
    ...
    function _updateFundingRate(PerpetualData storage _perpetual) internal {
        ...
        int128 fPremiumRate = _getInsurancePremiumEMA(_perpetual).div(_getIndexPrice(_perpetual));
        // clamp the rate
        ...
        int128 fBase = _perpetual.fkStarSide>=0 ? BASE_RATE : BASE_RATE.neg();
        fFundingRate = fFundingRate.add(fBase);
        // cap: 90% * (initialmargin - maintenance)
        int128 scale = 0xe666666666666800;
        int128 fCap = scale.mul(_perpetual.fInitialMarginRateAlpha -  _perpetual.fMaintenanceMarginRateAlpha);
        ...
        emit UpdateFundingRate(_perpetual.id, fFundingRate, block.timestamp);
    }
     ...
     */
    function _updateAMMTargetPoolSize(PerpetualData storage _perpetual) internal {
        ...
        _perpetual.fTargetAMMFundSize = _getUpdatedTargetAMMFundSize(_perpetual, ccy);
        ...
        emit UpdateAMMFundTargetSize(_perpetual.id, _perpetual.fTargetAMMFundSize, block.timestamp);
    }

    /**
     * Updates the target size for default fund and AMM pool
     ...
     * @param   _iCurrentTime   Timestamp in seconds
     */
    function _updateTargetDefaultFundSize(uint256 _iPoolIndex, uint256 _iCurrentTime) internal {
        require(_iPoolIndex <= iPoolCount, "pool index out of range");
        LiquidityPoolData storage liquidityPool = liquidityPools[_iPoolIndex];
        int128 fTargetAMMFundSize;
        int128 fTargetDFSize;
        if (_iCurrentTime - liquidityPool.iLastTargetPoolSizeTime > liquidityPool.iTargetPoolSizeUpdateTime) {
            ...
            for (uint256 i = 0; i < length; i++) {
                ...
                // get pool needs for current perpetual
                perpetual.fTargetDFSize = _getTargetDefaultFundSize(perpetual);
                fTargetDFSize = fTargetDFSize.add(perpetual.fTargetDFSize);
            }
            liquidityPool.fTargetDFSize = fTargetDFSize;
            // reset update time
            liquidityPool.iLastTargetPoolSizeTime = _iCurrentTime;
            emit UpdateDefaultFundTargetSize(liquidityPool.id, fTargetDFSize, _iCurrentTime);
        }
    }
     ...
     */
    function _getUpdatedTargetAMMFundSize(PerpetualData storage _perpetual, AMMPerpLogic.CollateralCurrency _ccy) internal view returns (int128) {
        // loop through perpetuals of this pool and update the
        // pool size
        AMMPerpLogic.MarketVariables memory mv;
        mv.fIndexPriceS2 = _perpetual.indexS2PriceData.fPrice;
        ...
        if (_ccy == AMMPerpLogic.CollateralCurrency.BASE) {
            ...
            if (fK == 0 || fLockedIn == 0) {
                ...
                fMStar = _getAMMPerpLogic().getTargetCollateralM2(fK, fLockedIn, mv, _perpetual.fAMMTargetDD);
            }
            ...
            if (fK == 0) {
                ...
                mv.fIndexPriceS3 = _perpetual.indexS3PriceData.fPrice;
                // get target collateral for current AMM exposure
                fMStar = _getAMMPerpLogic().getTargetCollateralM3(fK, fLockedIn, mv, _perpetual.fAMMTargetDD);
            }
        } else {
            ...
            if (fK == 0) {
                ...
                fMStar = _getAMMPerpLogic().getTargetCollateralM1(fK, fLockedIn, mv, _perpetual.fAMMTargetDD);
            }
        }
     ...
     * @param   _perpetual      Reference to perpetual
     */
    function _getTargetDefaultFundSize(PerpetualData storage _perpetual) internal view returns (int128) {
        int128[2] memory fIndexPrices;
        fIndexPrices[0] = _perpetual.indexS2PriceData.fPrice;
        fIndexPrices[1] = _perpetual.indexS3PriceData.fPrice;
        return
            _getAMMPerpLogic().calculateDefaultFundSize(
                _perpetual.fCurrentAMMExposureEMA,
                _perpetual.fCurrentTraderExposureEMA,
                _perpetual.fDFCoverN,
    ...
    function _updateOraclePriceData(OraclePriceData storage _priceData, function() external returns (int128, uint256) priceGetter) internal {
        ...
        if (time >= _priceData.time) {
            _priceData.fPrice = fPrice;
            _priceData.time = time;
        }
    }

    /**
     * Update the oracle price of the perpetual, including the index price and the mark price
    ...
    function _updateSpotPrice(PerpetualData storage perpetual, uint256 _iCurrentTimeSec) internal {
        ...
        if (oraclePriceDataUpdate[perpetual.oracleS2Addr].time != _iCurrentTimeSec) {
            _updateOraclePriceData(perpetual.indexS2PriceData, oracle.getSpotPrice);
            oraclePriceDataUpdate[perpetual.oracleS2Addr] = perpetual.indexS2PriceData;
        } else {
            perpetual.indexS2PriceData = oraclePriceDataUpdate[perpetual.oracleS2Addr];
        }
        if (perpetual.oracleS3Addr != address(0)) {
            if (oraclePriceDataUpdate[perpetual.oracleS3Addr].time != _iCurrentTimeSec) {
                // quanto currency
                oracle = ISpotOracle(perpetual.oracleS3Addr);
                _updateOraclePriceData(perpetual.indexS3PriceData, oracle.getSpotPrice);
                oraclePriceDataUpdate[perpetual.oracleS3Addr] = perpetual.indexS3PriceData;
            } else {
                perpetual.indexS3PriceData = oraclePriceDataUpdate[perpetual.oracleS3Addr];
            }
        }
        _updateMarkPrice(perpetual, _iCurrentTimeSec);
        emit UpdatePrice(
            perpetual.id,
            perpetual.oracleS2Addr,
            perpetual.oracleS3Addr,
            perpetual.indexS2PriceData.fPrice,
            perpetual.indexS2PriceData.time,
            perpetual.indexS3PriceData.fPrice,
            perpetual.indexS3PriceData.time
        );
    }
     ...
     * @param   _iCurrentTimeSec   The current timestamp (block.timestamp)
     */
    function _updateOraclePricesForPool(uint256 _iPoolIndex, uint256 _iCurrentTimeSec) internal {
        require(_iPoolIndex <= iPoolCount, "pool index out of range");
        LiquidityPoolData storage liquidityPool = liquidityPools[_iPoolIndex];

        if (liquidityPool.iPriceUpdateTimeSec >= _iCurrentTimeSec) {
            return;
        }
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            _updateSpotPrice(perpetual, _iCurrentTimeSec);
            if (ISpotOracle(perpetual.oracleS2Addr).isTerminated()) {
                _rebalance(perpetual);
                _setEmergencyState(perpetual);
            }
            if (perpetual.oracleS3Addr != address(0) && ISpotOracle(perpetual.oracleS3Addr).isTerminated()) {
                _rebalance(perpetual);
                _setEmergencyState(perpetual);
            }
        }
        liquidityPool.iPriceUpdateTimeSec = _iCurrentTimeSec;
    }
...
contract PerpetualFactory is PerpetualUpdateFunctions, OracleInterfaceID, IFunctionList, IPerpetualFactory {
    ...
    function _createPerpetual(
        ...
        require(_baseParams[6] > 0 && _baseParams[6] < ONE_64x64 >> 3, "invalid fReferralRebateRate");
        ...
        require(_baseParams[9] >= 0, "invalid incentive-spread");
        ...
        require(!isQuanto || (_defaultFundRiskParams[3] > 0 && _defaultFundRiskParams[3] < ONE_64x64), "invalid fStressReturnS3[1]");
        require(_defaultFundRiskParams[4] > ONE_64x64 && _defaultFundRiskParams[4] < ONE_64x64 << 5, "invalid fDFCoverN");
        ...
        require(_defaultFundRiskParams[8] > 0, "minimal AMM size must be positive");
        require(_defaultFundRiskParams[9] > 0, "minimal trader exposure EMA must be positive");
        require(_defaultFundRiskParams[10] > 0, "minimal AMM exposure EMA must be positive");
        require(_defaultFundRiskParams[11] > ONE_64x64, "maximal trade size bump up must be >1");
        ...
        // oracles
        perpetual.oracleS2Addr = _oracles[0];
        perpetual.oracleS3Addr = _oracles[1];

        // base parameters
        perpetual.fInitialMarginRateAlpha = _baseParams[0];
        perpetual.fMarginRateBeta = _baseParams[1];
        perpetual.fMaintenanceMarginRateAlpha = _baseParams[2];
        perpetual.fInitialMarginRateCap = _baseParams[3];
        perpetual.fTreasuryFeeRate = _baseParams[4];
        perpetual.fPnLPartRate = _baseParams[5];
        perpetual.fReferralRebateRate = _baseParams[6];
        perpetual.fLiquidationPenaltyRate = _baseParams[7];
        perpetual.fMinimalSpread = _baseParams[8];
        perpetual.fIncentiveSpread = _baseParams[9];
        perpetual.fLotSizeBC = _baseParams[10];

        // risk parameters for underlying instruments
        perpetual.fFundingRateClamp = _underlyingRiskParams[0];
        perpetual.fMarkPriceEMALambda = _underlyingRiskParams[1];
        perpetual.fSigma2 = _underlyingRiskParams[2];
        perpetual.fSigma3 = _underlyingRiskParams[3];
        perpetual.fRho23 = _underlyingRiskParams[4];

        // risk parameters for default fund / AMM pool
        perpetual.fStressReturnS2[0] = _defaultFundRiskParams[0];
        perpetual.fStressReturnS2[1] = _defaultFundRiskParams[1];
        perpetual.fStressReturnS3[0] = _defaultFundRiskParams[2];
        perpetual.fStressReturnS3[1] = _defaultFundRiskParams[3];
        perpetual.fDFCoverN = _defaultFundRiskParams[4];
        perpetual.fDFLambda[0] = _defaultFundRiskParams[5];
        perpetual.fDFLambda[1] = _defaultFundRiskParams[6];
        perpetual.fAMMTargetDD = _defaultFundRiskParams[7];
        perpetual.fAMMMinSizeCC = _defaultFundRiskParams[8];
        perpetual.fMinimalTraderExposureEMA = _defaultFundRiskParams[9];
        perpetual.fMinimalAMMExposureEMA = _defaultFundRiskParams[10];
        perpetual.fMaximalTradeSizeBumpUp = _defaultFundRiskParams[11];
        ...
        perpetual.fCurrentTraderExposureEMA = _defaultFundRiskParams[9];
        // same for AMM
        perpetual.fCurrentAMMExposureEMA[0] = _defaultFundRiskParams[10].neg(); //= -fMinimalAMMExposureEMA
        perpetual.fCurrentAMMExposureEMA[1] = _defaultFundRiskParams[10]; //= fMinimalAMMExposureEMA
    ...
    function _activatePerpetual(bytes32 _perpetualId) internal {
        ...
        require(perpetual.state == PerpetualState.INVALID, "perpetual needs to be in invalid state");
    ...
    function getFunctionList() external view override returns (bytes4[] memory) {
