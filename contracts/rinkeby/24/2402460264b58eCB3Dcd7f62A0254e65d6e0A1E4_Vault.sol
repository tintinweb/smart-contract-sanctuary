    ...
    ReentrancyGuard
{
    ...
    constructor(ERC20 _stakedToken)
        ERC20EToken(
            string(abi.encodePacked('EQZ ', _stakedToken.name())),
     ...
     */
    function splitFees(uint256 fee) external onlyFlashLoanProvider {
        uint256 treasuryAmount = getTreasuryAmountToSend(fee);
...
contract FlashLoanFeeProvider is IFlashLoanFeeProvider, Moderable {
    uint256 public treasuryFeePercentage = 20;
    uint256 public flashFeePercentage = 1000;
    ...
    function setTreasuryFeePercentage(uint256 _treasuryFeePercentage) external onlyModerator {
        require(_treasuryFeePercentage <= 100, 'TREASURY_FEE_PERCENTAGE_TO_BIG');
