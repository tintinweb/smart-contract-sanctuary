...
interface IMerkleList {
    function merkleURI() external view returns (string memory);
}

interface IJellyFactory {
    function getContracts() external view returns (address[] memory);
    function getContractsByTemplateId(bytes32 _templateId) external view returns (address[] memory, uint256 size);
}
...
contract JellyHelper is PricesHelper {
    ...
    function getPoolInfo(address _pool) public view returns (Pool memory pool) {
        ...
        if (rewardsContract != address(0)) {
            ...
            for (uint256 i = 0; i < rewardTokens.length; i++) {
                ...
                reward.rewardTokenPrice = getPriceUsdc(token);
        ...
        pool.stakingTokenPrice = getPriceUsdc(stakingToken);
    ...
    function getPools(bytes32 _templateId) public view returns (Pool[] memory) {
        (address[] memory contracts, uint256 size) = jellyFactory.getContractsByTemplateId(_templateId);
        Pool[] memory pools = new Pool[](size);

        for (uint256 i = 0; i < size; i++) {
            pools[i] = getPoolInfo(contracts[i]);
        }
        return pools;
    }
        ...
        returns (UserInfo memory userInfo)
    {
        ...
        userInfo.userId = _user;
        userInfo.pools = userPools;
    }

    function getUserPoolInfo(address _user, address _pool)
        public
        view
        returns (UserPool memory userPool)
    {
        ITokenPool tokenPool = ITokenPool(_pool);
        userPool.poolId = _pool;
        userPool.staked = tokenPool.balanceOf(_user);
        address rewardsContract = tokenPool.rewardsContract();
        if (rewardsContract != address(0)) {
            (
                address[] memory tokens,
            ...
            UserReward[] memory userRewards = new UserReward[](tokens.length);
    ...
    {
        IAirdrop airdrop = IAirdrop(_airdropAddress);
        IMerkleList list = IMerkleList(airdrop.list());
        airdropInfo.airdrop = _airdropAddress;
        airdropInfo.merkleURI = list.merkleURI();
        airdropInfo.rewardToken = getTokenInfo(airdrop.rewardsToken());
    }

    function getAirdrops(bytes32 _templateId) public view returns (AirdropInfo[] memory) {
        (address[] memory contracts, uint256 size) = jellyFactory.getContractsByTemplateId(_templateId);
    ...
    function getRewardInfo(address _reward)
        public
        view
