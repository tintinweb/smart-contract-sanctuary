...
contract EnsMapper is Ownable {
    ...
    function setDomain(string memory label, uint256 token_id) public {
        ...
        require(whitelist[token_id], "token not currently on the whitelist");
    ...
    function resetHash(uint256 token_id) public onlyOwner {
        bytes32 domain = tokenHashmap[token_id];
        require(!ens.recordExists(domain), "sub-domain already exists");
    ...
    function test(address addy, uint256 index) public view returns(uint256){
        return nft2.tokenOfOwnerByIndex(addy, index);
    }

    function getClaimableIds(address addy) public view returns(uint256[] memory){
        uint256 balance = nft.balanceOf(addy);
        uint256[] memory ids = new uint256[](balance);
        uint256 count;
        for(uint256 i; i < balance; i++){
            uint256 id = nft2.tokenOfOwnerByIndex(addy, i);
            if(whitelist[id]){
                ids[count++] = id;
            }
        }
