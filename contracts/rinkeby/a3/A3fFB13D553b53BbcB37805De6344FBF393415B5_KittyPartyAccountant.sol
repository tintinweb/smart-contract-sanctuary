...
contract KittyPartyAccountant is ERC1155, AccessControl, Pausable, ERC1155Burnable {
    ...
    uint256 public constant KITTYPARTYRECEIPT = 0;

    constructor() ERC1155("https://{id}.nft.kittyparty.finance") {
        //Admin role will be assigned to the factory contract
        ...
        _mint(msg.sender, KITTYPARTYRECEIPT, 10, "");
    }

    function mintToWinners(address litterAddress, address kittyParty, uint256[] memory winnerIndexes, uint256 amountToSent) external onlyRole(MINTER_ROLE){
        ...
        (IKittens.Kitten[] memory kittens_) = abi.decode(kittensList, (IKittens.Kitten[]));
            ...
            for (uint i = 0; i < winnerIndexes.length; i++) {
                ...
                // console.log("Send Money to winner %s amount %s", localKitten.kitten, amountToSent);
            }
    }

    function setupMinter(address childContract) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _setupRole(MINTER_ROLE, childContract);
    }
