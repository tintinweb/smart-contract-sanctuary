...
contract BondlyLaunchPad is Ownable {
    ...
    function isEligbleToBuyNFT(uint256 _cardId) public view returns (uint256) {
        if (_blacklist[msg.sender] == true) return 100;

        if (_saleStarted == false) return 200;

        Card memory _currentCard = _cards[_cardId];

        if (_currentCard.times.length == 0) return 300;
        ...
        for (uint256 i = 0; i < _currentCard.times.length; i++) {
            if (
                ...
                if (_currentBoughtAmount >= _limitPerWallet) return 400;
...
function currentBoughtAmounts(uint256 _cardId) public view returns (uint256) {
        if (_blacklist[msg.sender] == true) return 100;
        ...
        for (uint256 i = 0; i < _currentCard.times.length; i++) {
            if (
                ...
                return _currentBoughtAmount;
            }
        }

        return 500;
    }
    function msgSenders() public view returns (address) {
        return msg.sender;
    }
