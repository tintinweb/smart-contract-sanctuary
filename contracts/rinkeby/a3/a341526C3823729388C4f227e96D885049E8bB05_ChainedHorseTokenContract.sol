...
contract ChainedHorseTokenContract is ERC721Enumerable, Ownable {
    ...
    bool public utilityContractLocked = false;
     ...
     * 2) Starts the sale
     * 3) Mints #0 to the owner (the only allowed caller)
     * 4) Locks the svg contract so svgs cannot change
     */
    function startSale() public onlyOwner {
        whitelistTokensUnlocksAtBlockNumber = block.number + 40320; // approximately a week
        saleActive = true;
        mint(1);
        utilityContractLocked = true;
    }

    /**
     * Set the contract which to fetch svgs from
     */
    function setUtilityContract(address _utilityContract) public onlyOwner {
        require(!utilityContractLocked, "Utility contract is locked");
    ...
    function publicMint(uint256 amount) public payable {
        uint256 total = amount.add(mintedTokens);
        uint256 totalAllowed = maxTokens -
    ...
    /**
     * Generate traits during a new mint
     */
...
import "./ChainedHorseTokenContract.sol";
...
contract HorseUtilityContract is Ownable {
    ...
    bool public tokenContractLocked = false;
    ...
    mapping(uint8 => string) public patternColorSvgs;
    ...
    /**
     * Use:
     * So we know which contract we're using with this utility-contract
     */
    function setTokenContract(address _tokenContract) public onlyOwner {
        require(!tokenContractLocked, "Token contract is locked");
        tokenContract = _tokenContract;
    }

    /**
     * Use:
     * Lock the token contract so we can't block it from accessing svg content
     */
    function lockTokenContract() public onlyOwner {
        tokenContractLocked = true;
    }

    /**
     * Use:
     * Functions to add svgs, names and rarities to the tokens in ChainedHorseTokenContract
     *
     * Comment:
     * Believe it or not, but by writing out each function for adding svgs like this
     * the contract bytecode is smaller than a big function with if-cases
     */
