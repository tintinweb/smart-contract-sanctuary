...
// File: contracts/AngryContract.sol
...
contract AngryContract is ReentrancyGuard{
    ...
    uint256 public prePurchaseSupplyPerDay;
    ...
    event PrePurchaseSupplyPerDayChange(uint256 _oldValue, uint256 _newValue);
    ...
    function initialize(address _angryTokenAddr, address _usdtTokenAddr, address _uniswapRouterAddr, address _vitalikButerinAddr, uint256 _startTime) public {
        ...
        prePurchaseSupplyPerDay = 426137000;
    ...
    function setExpectedPriceArgs(uint256 _multipleLo, uint256 _multipleHi, uint256 _floatValLo, uint256 _floatValHi) public onlyExecutor{
        require(_floatValLo <= 100 && _floatValHi <= 100, "Bad Value!");
    ...
    function vitalikButerinWithdraw() public {
        require ( msg.sender == vitalikButerinAddr, "Only for Vitalik Buterin!" );
    ...
    function processPrePurchaseOrder(address _addr, uint256 _orderIdx, uint256 _amountOutMin) nonReentrant private {
        ...
        if( pcInfo.paymentType == 1 ){
            ...
            amount = uniswapRouterV2.swapExactETHForTokens{value:repurchase}(_amountOutMin, path, address(this), block.timestamp )[1];
            IBurnable(angryTokenAddr).burn(amount);
        }else{
            revenueUSDT = revenueUSDT + ( pcInfo.paymentAmount - repurchase );
            path[0] = usdtTokenAddr;
            usdtToken.safeApprove(uniswapRouterAddr,repurchase);
            amount = uniswapRouterV2.swapExactTokensForTokens(repurchase, _amountOutMin, path, address(this), block.timestamp )[1];
            IBurnable(angryTokenAddr).burn(amount);
        }
    ...
    function queryCurrPrePurchaseQuota() public view returns (uint256){
        ...
        return ds * prePurchaseSupplyPerDay * 10 ** angryTokenDecimals;
    }
    ...
    function prePurchase(uint256 _expectedPrice, uint256 _startTime, uint256 _expiredTime, string calldata _invoiceId, uint256 _invoiceExpiredTime, bytes memory _sig) public payable {
        ...
        require( executorList[hash.recover(_sig)], "Unexpected signer!" );
        require( _invoiceExpiredTime >= block.timestamp, "Invoice expired!" );
        require( !prePurchaseInvoiceMapping[_invoiceId], "Invoice exists!" );
        ...
        require( tmpArgs.accountQuota > 0, "Exceed account quota!" );
        ...
        if(msg.value > 0){
            require(tmpArgs.ethPrice > 0, "Invalid ethPrice!");
            ...
            require( _expectedPrice > tmpArgs.ethPrice && _expectedPrice >= lowEthPrice && _expectedPrice <= highEthPrice, "Invalid expectedPrice!" );
            ...
            require(tmpArgs.usdtPrice > 0, "Invalid usdtPrice!");
            ...
            require( _expectedPrice > tmpArgs.usdtPrice && _expectedPrice >= lowUsdtPrice && _expectedPrice <= highUsdtPrice, "Invalid expectedPrice!" );
        ...
        require( (tmpArgs.currAmount + totalPrePurcaseAmount) <= totalQuota, "Exceed daily quota!" );
        require( tmpArgs.currAmount <= tmpArgs.accountQuota, "Exceed account quota!" );
        ...
        emit PrePurchase(msg.sender, purchases.length-1, tmpArgs.currAmount, pcInfo.paymentAmount, pcInfo.price, pcInfo.expectedPrice, pcInfo.startTime, pcInfo.expiredTime, pcInfo.paymentType, pcInfo.status);
    }
    ...
    function completePrePurchaseOrder(uint256 _orderIdx, uint256 _amountOutMin) public {
        require( bStarted, "Not Start!" );
        PrePurchaseInfo[] storage purchases = prePurchaseList[ msg.sender ];
        require( purchases.length > _orderIdx, "Order index out of range!" );
        PrePurchaseInfo storage pcInfo = purchases[ _orderIdx ];
        require( pcInfo.status == 0, "Unexpected order status!" );
        require( pcInfo.expiredTime >= block.timestamp, "Order expired!" );
        uint256 ethPrice = 0;
        uint256 usdtPrice = 0;
        (ethPrice, usdtPrice) = getANBPrice();
        if(pcInfo.paymentType == 1){
            require( pcInfo.expectedPrice <= ethPrice, "Unmet price!");
        }else{
            require( pcInfo.expectedPrice <= usdtPrice, "Unmet price!");
        }
        pcInfo.status = 3;
        processPrePurchaseOrder(msg.sender, _orderIdx, _amountOutMin);
    }
    ...
    function withdrawMiningRewards(string calldata _taskId, uint256 _amount, bytes memory _sig) public {
        ...
        require( executorList[signer], "Unexpected signer!" );
    ...
    function newMiningTask(string calldata _taskId, uint256 _rewardAmount, uint256 _beginTime, uint256 _endTime) public onlyExecutor {
        ...
        require( _rewardAmount <= maxMiningTaskReward * 10 ** angryTokenDecimals, "Amount exceeded!" );
        MiningPoolInfo storage pi = miningPoolInfos[_taskId];
    ...
    function setPrePurchaseSupplyPerDay(uint256 _newValue) public onlyExecutor {
        emit PrePurchaseSupplyPerDayChange(prePurchaseSupplyPerDay, _newValue);
        prePurchaseSupplyPerDay = _newValue;
    }
