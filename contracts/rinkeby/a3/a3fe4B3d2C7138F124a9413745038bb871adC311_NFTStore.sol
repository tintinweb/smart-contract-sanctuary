...
contract NFTStore is EIP712, Ownable {
	...
	// This is address of the lower level raw nft contract that the store is managing - it can be changed on the fly
	NFTContract public nftcontract = NFTContract(0xBAB7dF6D042F4b83b62070b670Da929b62eD22d8);
	...
	*/
	...
	function mintPreSaleSigned(uint256 _amount,uint256 _passAmount, uint8 v, bytes32 r, bytes32 s) external payable preSaleOnTest {
		require(_amount <= mintedPerTxMax, "exceeds max tx");
		require(_amount <= mintedMax - mintedSoFar, "exceeds avail");
		require(price * _amount <= msg.value, "exceeds caller spend");
		vipSignedFinalize(_amount,_passAmount,v,r,s);
		mintedSoFar+=_amount;
		for (uint256 i = 0; i < _amount; i++) {
			nftcontract.mint(msg.sender);
		}
	...
	function vipBruteFinalize(uint256 _amount) internal {
		require(_amount <= mintedMax - mintedSoFar, "exceeds max mint");
...
//		vipBloomFilter.add( bytes32(uint256(uint160(_address)) << 96) );
	...
	function vipSignedFinalize(uint256 _amount, uint256 _passAmount, uint8 v, bytes32 r, bytes32 s) internal {
		require(_amount <= mintedMax - mintedSoFar, "exceeds max mint");
		require(_amount <= mintedVIPMax - mintedSoFar, "exceeds vip avail");
		require(_amount <= _passAmount - mintedPerVIP[msg.sender], "exceeds mint avail for you");
