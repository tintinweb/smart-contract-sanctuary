...
contract CnMGame is Ownable, ReentrancyGuard, Pausable {
  ...
  event RollApplied(address indexed owner, uint256 indexed tokenId);
  event RollDeclined(address indexed owner, uint256 indexed tokenId);
...
*/
  ...
  */
  function rollMouseCommit(uint256 tokenId) external whenNotPaused nonReentrant {
    require(allowCommits, "adding commits disallowed");
    require(tx.origin == _msgSender(), "Only EOA");
    require(!cnmNFT.isCat(tokenId), "affected only for Mouse NFTs");
    ...
    */
    ...
    emit RollApplied(_msgSender(), tokenId);
  }
  ...
  */
  function rollMouseDeclineCommit(uint256 tokenId) external whenNotPaused nonReentrant {
    ...
    emit RollDeclined(_msgSender(), tokenId);
  }
