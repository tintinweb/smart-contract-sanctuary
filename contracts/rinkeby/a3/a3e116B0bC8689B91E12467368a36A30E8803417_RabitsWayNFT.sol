...
contract RabitsWayNFT is ERC721Enumerable, Ownable {
    using Strings for uint256;

    uint256 public maxSupply = 10000;
    uint256 public currentSupply = 1000;
    //TODO: add function for setCurrentSupply() as well

    uint256 public bunnyPrice = 0.08 * 10 ** 18;
    uint256 public rareBunnyPrice = 0.12 * 10 ** 18;
    ...
    constructor(address _contractAddress, address _w1, address _w2, address _w3) ERC721("RabitsWay", "RBW") {
    ...
    function mint(uint256 _mintAmount, bool rarity) public payable {
        ...
        if (_msgSender() != owner()) {
            ...
            if( rarity )
                require(msg.value >= rareBunnyPrice * _mintAmount, "Must send proper eth value to mint rare tokens");
            else
                require(msg.value >= bunnyPrice * _mintAmount, "Must send proper eth value to mint tokens");
        }
    ...
    //sets price of bunny
    function setBunnyPrice(uint256 _price) public onlyOwner() {
        bunnyPrice = _price;
    }

    //sets price of rare bunny
    function setRareBunnyPrice(uint256 _price) public onlyOwner() {
        rareBunnyPrice = _price;
    }
    
    //sets current supply of bunny
    function setCurrentSupply(uint256 _supply) public onlyOwner() {
        require( _supply > currentSupply && _supply <= maxSupply, 'Provide a valid supply i.e. greater than current supply and less than max supply');
    ...
    function withdraw() external onlyOwner {
        ...
        (bool success4, ) = payable(erc20ContractAddress).call{value: (percent * 7)}("");
        require(success4, "Transaction Unsuccessful");
    }
