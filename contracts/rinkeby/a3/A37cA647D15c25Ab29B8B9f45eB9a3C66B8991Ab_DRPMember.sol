...
contract DRPMember is ERC1155Supply, Ownable {
	...
	// all token holders
	mapping(uint256 => HoldersHelper.TokenHolders) tokenHolders;
	...
	*/
	...
	function toggleSalesActive(uint256[] calldata ids) external onlyOwner{
		// check if ok first 
	...
	function toggleWhitelistSalesActive(uint256[] calldata ids) external onlyOwner {
		// check if ok first 
		for (uint16 i = 0; i < ids.length; i++) {
			require(tokens[ids[i]].maxSupply > 0, "Query for non-existent token");
		}
	...
	*/
	...
	function setURI(uint256[] calldata ids, string[] calldata uris) external {
