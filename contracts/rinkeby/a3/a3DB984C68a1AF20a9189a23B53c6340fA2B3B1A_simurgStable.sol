...
contract ColletoralsContract is OwnerContract{
    ...
    function setWBTCAddress(address _address) public isAdmin{
        WBTCOracleAddress = _address;
    }
    function setWETHAddress(address _address) public isAdmin{
        WETHOracleAddress = _address;
    }
    function setLINKAddress(address _address) public isAdmin{
        LINKOracleAddress = _address;
    }
...
contract simurgStable is ERC20, OwnerContract, ColletoralsContract{
    ...
    */
    ...
    constructor() ERC20("Simurg Stable Dollar", "SSD10"){
        redeemTax = 50;
    ...
    function mintNewStableWithETH(address _referral) payable public{
        if (msg.value > 1 wei){
            uint256 mintAmount = getWETHPrice() * msg.value;
            payable(_referral).transfer(((referralPercentage)/10000) * msg.value);
    ...
    function mintNewStableWithLINK(uint _amount, address _referral) public {
        ...
        mint(msg.sender, price * _amount / 100);
        emit Transfer(msg.sender, address(this), _amount);
    }
    ...
    function LINKPrice() external view returns(uint256){
        return (getLINKPrice());
    }
    function WBTCPrice() external view returns(uint256){
        return (getWBTCPrice());
    }
    ...
    function redeemCollateralWBTC(uint _amount) public {
        ...
        uint256 price = getWETHPrice();
        burn(msg.sender, _amount);
        WBTC.transfer(msg.sender, (_amount * price * (10000 - redeemTax)/10000));
        emit Transfer(msg.sender, address(0), _amount);
    }
    ...
    function redeemCollateralLINK(uint _amount) public {
        ...
        uint256 price = getWETHPrice();
        burn(msg.sender, _amount);
        LINK.transfer(msg.sender, (_amount * price * (10000 - redeemTax)/10000));
        emit Transfer(msg.sender, address(0), _amount);
    }
    ...
    function redeemCollateralUSDT(uint _amount) public {
        ...
        uint256 price = getWETHPrice();
        burn(msg.sender, _amount);
        USDT.transfer(msg.sender, (_amount * price * (10000 - redeemTax)/10000));
        emit Transfer(msg.sender, address(0), _amount);
    }
    ...
    function redeemCollateralUSDC(uint _amount) public {
        ...
        uint256 price = getWETHPrice();
        burn(msg.sender, _amount);
        USDC.transfer(msg.sender, (_amount * price * (10000 - redeemTax)/10000));
        emit Transfer(msg.sender, address(0), _amount);
    }
    
    function collateralValue() external view returns(uint256){
        uint256 totalValue = USDC.balanceOf(address(this));
        totalValue += USDT.balanceOf(address(this));
        totalValue += (WETH.balanceOf(address(this)) * getWETHPrice());
        totalValue += (WBTC.balanceOf(address(this)) * getWBTCPrice());
        totalValue += (LINK.balanceOf(address(this)) * getLINKPrice() / 100);
        return totalValue;
    }
