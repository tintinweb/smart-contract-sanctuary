...
contract animalMain is  ERC721,Ownable {
  ...
  enum AnimalType{ None, Tiger, Dog, Lion, Bird}
  
  struct animal {
    bytes32 dna; //DNA
    uint8 star; //幾星級(抽卡的概念)
    AnimalType roletype; //(哪種動物 1老虎  2狗  3獅子  4鳥)
  }
  ...
  function seeAnimalRole(uint256 _tokenId) public view returns (AnimalType roletype) {
  ...
  function createAnimal() public {
      ...
      dna = bytes32(random());
      star = random() % 5;
      roletype = AnimalType(random() % 5);
      
      //動手玩創意，可以限制每次建立動物需要花費多少ETH
       
      uint id = animals.push(animal(dna, uint8(star), roletype)) - 1;
