...
contract FightClub is ERC721Burnable, Ownable {
    ...
    */
    function mintFC(uint256 noOfTokens) external payable {
        ...
        if(totalSupply()>1000) {
            require(minted[msg.sender]+noOfTokens <= 6, "Cannot buy more then 6 nfts per Wallet");
            require(msg.value == (noOfTokens * MINT_PRICE)," Wrong eth value sent");
        }
        else {
            require(minted[msg.sender]+noOfTokens <= 3, "Cannot buy more then 3 free nfts per Wallet");
        }
