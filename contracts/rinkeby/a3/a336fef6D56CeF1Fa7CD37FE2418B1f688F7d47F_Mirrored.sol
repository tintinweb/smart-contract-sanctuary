...
contract Mirrored is ERC721, Ownable, Pausable {
	...
	// Pack these two 128s
	uint128 private PREMINT_PLACEHOLDER = 777; // Placeholder for null premint
	uint128 private maxSupply = 18; // Max supply, accounting for designated premints
	uint256 public listPrice = 2000000000000000;
	// uint256 public listPrice = 200000000000000000; // 0.2 eth initial list price
	uint128 private saleLimitPerUser = 2;
	mapping(address => uint256) public premintReserveMap; // Address paired w token ids for minting
	 ...
	 * compose into matching key-value pairs within artistToken.
	 *
	 * The same approach is taken with premints.
	 *
	 * @param _baseURI list of artist addresses
	 * @param artistTokenIds list of token ids
	 * @param artistAddresses list of artist addresses
	 * @param premintTokenIds list of premint addresses
	 * @param premintAddresses list of reserved premint token ids
	 */
	constructor(
		string memory _baseURI,
		uint256[] memory artistTokenIds,
		address[] memory artistAddresses,
		uint256[] memory premintTokenIds,
		address[] memory premintAddresses
	) ERC721("Mirrored", "Mirrored") {
		require(
			artistAddresses.length == artistTokenIds.length,
			"ARTIST_TOKEN_CLASH"
		);
		require(
			premintTokenIds.length == premintAddresses.length,
			"PREMINT_TOKEN_CLASH"
		);
		...
		for (uint128 i = 0; i < artistAddresses.length; i++) {
			artistTokenMap[artistTokenIds[i]] = artistAddresses[i];
		}

		// Set premint key value map
		for (uint128 i = 0; i < premintAddresses.length; i++) {
			premintReserveMap[premintAddresses[i]] = premintTokenIds[i];
		}
	}
	 ...
	 */
	function addToPremint(address _address, uint128 _reservedTokenId)
		public
		onlyOwner
	{
		premintReserveMap[_address] = _reservedTokenId;
	}
	...
	function removeFromPremint(address _address) public onlyOwner {
		delete premintReserveMap[_address];
	}

	/**
	 * Add a collaborators's address to royalty mapping.
	 */
	function addToArtistToken(address _address, uint128 _tokenId)
		public
		onlyOwner
	{
	...
	function handleSale(uint256 _tokenId) private {
		...
		if (artistTokenMap[_tokenId] != address(0)) {
			pastSales.push(
				Sale({
					artistAddress: artistTokenMap[_tokenId],
	...
	function handleDesignatedMint() private {
		// Make sure that a user cannot mint a token if all tokens have been minted.
		// This excludes users with designated tokens
		require(saleLimitMap[_msgSender()] < saleLimitPerUser);
		require(
			premintReserveMap[_msgSender()] != 0 ||
		...
		if (
			premintReserveMap[_msgSender()] != 0 &&
			premintReserveMap[_msgSender()] != PREMINT_PLACEHOLDER
		) {
			require(
				!_exists(premintReserveMap[_msgSender()]),
				"TOKEN_ALLOCATED"
			);
			handleSale(premintReserveMap[_msgSender()]);
	...
	function premint() public payable whenNotPaused {
		require(premintReserveMap[_msgSender()] != 0, "NOT_PREMINT");
