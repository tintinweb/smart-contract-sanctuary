...
library LinearVestingLibrary {
    ...
    function availableInputAmount(
        ...
        if (block.number < self.cliffEnd || totalAmount == 0) {
    ...
    function _vested(
        ...
        if (self.vestingDuration == 0 || block.number >= self.cliffEnd + self.vestingDuration) {
            return totalAmount - vestedAmount;
        }
        uint passedBlocks = block.number - self.cliffEnd;
        uint available = totalAmount * passedBlocks / self.vestingDuration - vestedAmount;
