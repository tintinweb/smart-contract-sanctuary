...
/* This code should work for most ERC-721 contracts. Please feel free to reuse / alter / copy / ignore :-)
...
contract EnsMapper is Ownable {
    ...
    event RegisterSubdomain(bytes32 indexed node, address indexed registrar, uint256 indexed token_id, string label);
   ...
   function getTokenDomain(uint256 token_id) private view returns(string memory uri){
    ...
    function setDomain(string calldata label, uint256 token_id) public isAuthorised(token_id) {     
        ...
        emit RegisterSubdomain(big_hash, nft.ownerOf(token_id), token_id, label);     
    }
    ...
    function resetHash(uint256 token_id) public isAuthorised(token_id) {
        ...
        if(nextRegisterTimestamp[msg.sender] > block.timestamp && msg.sender != owner()){
            nextRegisterTimestamp[msg.sender] = block.timestamp + (60 * 60 * 24); //1 day cooldown
        }
