...
contract ChainedApes is ERC721Enumerable {
    ...
    uint256 public RATE = 11574070000000;
 ...
 |_|  |_|_|_||_\__|_|_||_\__, | |_| \_,_|_||_\__|\__|_\___/_||_/__/
   ...
   */
    ...
    function getLastTime(address _address) external view returns(uint256){
        return lastTime[_address];
    }

    function getAddressBalance(address _address) external view returns (uint256){
    ...
    function getClaimableRewards(address _address) external view returns(uint256){
		...
		uint256 pending = addressBalance[_address].mul(RATE.mul((time.sub(lastTime[_address]))));
		return rewards[_address] + pending;
    }
    
    function returnRewardsCurrent(address _address) external view returns(uint256){
    ...
    function updateRewards(address _address) internal {
		...
		if (timerFrom > 0)
			rewards[_address] += addressBalance[_address].mul(RATE.mul((time.sub(timerFrom))));
    }
    ...
    function mintApeWithToken() public {        
        ...
        ICigareth(cigarethAddress).burnFrom(msg.sender, currentCigarethCost());
        return mintInternal();
    }
