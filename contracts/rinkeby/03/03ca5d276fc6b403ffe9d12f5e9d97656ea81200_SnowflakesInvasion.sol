...
contract SnowflakesInvasion is IERC1155Receiver {
  ...
  constructor(IERC1155[] memory _collectibles,IERC20[] memory power) public {
  ...
  function atack(uint256 tokenId,uint8 x,uint8 y) private {
    ...
    uint256 totalTokensP2 = players[positions[x][y].owner].tokensId.length;
    for(uint256 i = 0;i < _power.length ; i++){
      powerP1 = powerP1 + _power[i].balanceOf(msg.sender).mul(i + 1).div(totalTokensP1);
      powerP2 = powerP2 + _power[i].balanceOf(positions[x][y].owner).mul(i + 1).div(totalTokensP2);
    }
  ...
  function getTokenPos(address player,uint256 tokenId) public returns(uint8 x,uint8 y){
