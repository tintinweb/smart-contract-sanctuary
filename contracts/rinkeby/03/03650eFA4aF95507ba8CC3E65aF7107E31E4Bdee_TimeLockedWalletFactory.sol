...
*/
    ...
    */
    ...
    function withdraw() public onlyOwner {
        ...
        payable(originalOwner).transfer(address(this).balance);
        Withdrew(originalOwner, address(this).balance);
    }

   function IsOwner() public view returns (bool, address, address, address) {
        return (msg.sender == owner, msg.sender, owner, originalOwner);
    }
    ...
    function withdrawTokens(address _tokenContract) public onlyOwner {
        ...
        token.transfer(originalOwner, tokenBalance);
        WithdrewTokens(_tokenContract, originalOwner, tokenBalance);
    }
...
*/
    ...
    function newTimeLockedWallet(address _originalOwner, uint256 _unlockDate)
        public
        payable
        returns (address wallet)
    {
        // Create new wallet.
        timeLockedWallet = new TimeLockedWallet(msg.sender, address(this), _originalOwner, _unlockDate);
        ...
        Created(wallet, msg.sender, address(this), block.timestamp, _unlockDate, 0);
    }

    // callable by owner only, after specified time, only for Tokens implementing ERC20
    function withdrawTokens(address _tokenContract) public   {
        //require(block.timestamp >= unlockDate, "Cannot withdraw, the wallet is locked...");
        ...
        //token.approve(owner, tokenBalance);
        //token.transferFrom(fromWallet, owner, tokenBalance);
        
        //TimeLockedWallet timeLockedWallet = TimeLockedWallet(payable(fromWallet));
        
        timeLockedWallet.withdrawTokens(_tokenContract);
  ...
  function IsOwner() public view returns (bool, address, address, address) {
        return timeLockedWallet.IsOwner();
    }
   ...
   function getBalanceOf(address wallet, address _tokenContract) public view returns (uint256, uint256, address, address) {
        ...
        return (tokenBalance, ownerBalance, msg.sender, originalOwner);
    }
