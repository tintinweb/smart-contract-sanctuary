...
interface ILido {
    ...
    /**
      ...
      * @return StAVAX Amount of StAVAX generated
      */
    function submit(address _referral) external payable returns (uint256 StAVAX);
    ...
    /**
      ...
      * @param _amount Amount of StAVAX to burn
...
import "../oracle/LidoOracle.sol";
import "../nos/ValidatorRegistry.sol";
import "../Lido.sol";
...
contract LidoTemplateE2E is BaseTemplate {
    ...
    */
    ...
    bytes32 internal constant StAVAX_APP_ID = (
        keccak256(abi.encodePacked(LIDO_PM_NODE, keccak256(abi.encodePacked("stavax")))) // stavax.lido.eth
    );
    ...
    */
        ...
        internal
    {
        ...
        state.operators = ValidatorRegistry(_installNonDefaultApp(state.dao, REGISTRY_APP_ID, initializeData));
...
/**
  ...
  *
  * See the comment of `IValidatorRegistry`.
  *
  * NOTE: the code below assumes moderate amount of node operators, e.g. up to 50.
  */
contract ValidatorRegistry is IValidatorRegistry, IsContract, AragonApp {
    using SafeMath for uint256;
    using SafeMath64 for uint64;
    using UnstructuredStorage for bytes32;
    ...
    bytes32 internal constant SIGNING_KEYS_MAPPING_NAME = keccak256("lido.ValidatorRegistry.signingKeysMappingName");


    /// @dev Node Operator parameters and internal state
    struct NodeOperator {
        bool active;    // a flag indicating if the operator can participate in further staking and reward distribution
        address rewardAddress;  // Ethereum 1 address which receives stavax rewards for this operator
    ...
    bytes32 internal constant TOTAL_OPERATORS_COUNT_POSITION = keccak256("lido.ValidatorRegistry.totalOperatorsCount");

    // @dev Cached number of active operators
    bytes32 internal constant ACTIVE_OPERATORS_COUNT_POSITION = keccak256("lido.ValidatorRegistry.activeOperatorsCount");

    /// @dev link to the Lido contract
    bytes32 internal constant LIDO_POSITION = keccak256("lido.ValidatorRegistry.lido");
    ...
    /**
      ...
      * @param _rewardAddress Ethereum 1 address which receives stAVAX rewards for this operator
...
*/
contract Lido is ILido, IsContract, StAVAX, AragonApp {
    using SafeMath for uint256;
    using SafeMath64 for uint64;
    using UnstructuredStorage for bytes32;
    ...
    bytes32 internal constant VALIDATOR_REGISTRY_POSITION = keccak256("lido.Lido.validatorRegistry");
    bytes32 internal constant NODE_REGISTRY_POSITION = keccak256("lido.Lido.nodeRegistry");
    ...
    */
        ...
        public onlyInit
    {
        ...
        _setNodeRegistry(_nodeRegistry);
    ...
    */
    ...
    /**
    * @notice Deposits buffered ethers to the node from NodeRegistry.
    ...
    */
    ...
    /**
      * @notice Deposits buffered ethers to the node from NodeRegistry, making no more than `_maxDeposits` deposit calls.
      * @dev This function is separated from submit() to reduce the cost of sending funds.
      */
    ...
    /**
      ...
      * @param _amount Amount of StAVAX to withdraw
      * @param _pubkeyHash Receiving address
      */
    ...
    */
    ...
    /**
      * @notice Gets validator registry interface handle
      */
    function getValidatorRegistry() public view returns (IValidatorRegistry) {
        return IValidatorRegistry(VALIDATOR_REGISTRY_POSITION.getStorageAddress());
    }

    /**
      * @notice Gets node registry interface handle
      */
    function getNodeRegistry() public view returns (INodeRegistry) {
        return INodeRegistry(NODE_REGISTRY_POSITION.getStorageAddress());
    }
    ...
    /**
     * @dev Internal function to set validator registry address
     * @param _r registry of node validators
     */
    function _setOperators(IValidatorRegistry _r) internal {
        require(isContract(_r), "NOT_A_CONTRACT");
        VALIDATOR_REGISTRY_POSITION.setStorageAddress(_r);
    }
     ...
     */
    function _setNodeRegistry(INodeRegistry _r) internal {
        require(isContract(_r), "NOT_A_CONTRACT");
        NODE_REGISTRY_POSITION.setStorageAddress(_r);
    }
    ...
    /**
     * @dev Deposits buffered eth to the node from NodeRegistry and assigns chunked deposits to node operators
     */
    ...
    function _ETH2Deposit(uint256 _numDeposits) internal returns (uint256) {
        (bytes memory pubkeys, bytes memory signatures) = getValidatorRegistry().assignNextSigningKeys(_numDeposits);
    ...
    /**
     * @dev Deposits AVAX to the node from NodeRegistry contract
     */
    function _transferToNode() internal {
        // Deposit ETH to the first node from NodeRegistry
        uint256 value = DEPOSIT_SIZE;
        uint256 targetBalance = address(this).balance.sub(value);
        getNodeRegistry().nodes(0).transfer(value);
    ...
    function distributeRewards(uint256 _totalRewards) internal {
        ...
        // StAVAX docs for the explanation of the shares mechanics). The staking rewards fee
        ...
        // shares has became _totalRewards StAVAX tokens more, effectively splitting the reward
    ...
    function _distributeNodeOperatorsReward(uint256 _sharesToDistribute) internal returns (uint256 distributed) {
        (address[] memory recipients, uint256[] memory shares) = getValidatorRegistry().getRewardsDistribution(_sharesToDistribute);
...
interface INodeRegistry {
    ...
    function findNode(address node) external view returns(uint256);
    function addNode(address node) external returns(uint256 nodesLen);
 ...
 * `_getTotalPooledEther` function. `Lido.sol` contract inherits StAVAX and defines
 * the `_getTotalPooledEther` function.
 *
 * StAVAX balances are dynamic and represent the holder's share in the total amount
 ...
 */
contract StAVAX is IERC20, Pausable {
    using SafeMath for uint256;
    using UnstructuredStorage for bytes32;

    /**
     * @dev StAVAX balances are dynamic and are calculated based on the accounts' shares
    ...
    */
     ...
     */
    bytes32 internal constant TOTAL_SHARES_POSITION = keccak256("lido.StAVAX.totalShares");
     ...
     *
     * @dev The sum of all ETH balances in the protocol, equals to the total supply of stAVAX.
     */
...
contract LidoMock is Lido {
    ...
    * @param _r registry of node operators
    */
    function setOperators(IValidatorRegistry _r) public {
        _setOperators(_r);
    }
 ...
 * StAVAX mock version of mintable/burnable/stoppable token.
 */
contract StAVAXMock is StAVAX {
...
contract PoolMock {
    ...
    IValidatorRegistry private operators;

    constructor(address _operators) public {
        operators = IValidatorRegistry(_operators);
    }
