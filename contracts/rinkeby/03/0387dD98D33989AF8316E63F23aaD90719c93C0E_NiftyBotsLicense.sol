...
contract NiftyBotsLicense is ERC721, Ownable, ERC721Burnable, ERC721Enumerable {
    ...
    // token ID to license info
    mapping(uint256 => License) public licenses;

    // tier configuration
    // tier ID to tier config
    mapping(uint256 => TierConfig) public tierConfigs;

    // allows to create custom tiers for addresses
    // this should be treated as a one-off and needs to be cleared
    mapping(uint256 => address) private _tierConfigAddressRestriction;

    // allow list for private minting of a free license
    // address to allowFreeMint flag
    ...
    uint256 seasonDuration = 7890000 seconds; // roughly 3 months
    uint256 currentSeasonStartDate;

    event LicenseMinted(address account, uint256 tokenId);
    event SeasonChanged(uint256 newSeason, uint256 newSeasonStartDate);
    ...
    constructor() ERC721("NiftyBots License", "NBL") {
        ...
        tierConfigs[1] = TierConfig(Tier.FREE, 0 ether, 53 weeks);
        tierConfigs[2] = TierConfig(Tier.SEED, 0.6 ether, 53 weeks);
        tierConfigs[3] = TierConfig(Tier.NIFTY, 1.2 ether, 53 weeks);
    ...
    function addToAllowList(address[] calldata addresses) external onlyOwner {
        require(addresses.length < 11, "Can only add 10 addresses at a time.");
        for (uint256 i; i < addresses.length; i++) {
            require(addresses[i] != address(0), "Can't add the null address");
            _freeMintAllowList[addresses[i]] = true;
        }
    }

    function removeFromAllowList(address[] calldata addresses) external onlyOwner {
        require(addresses.length < 11, "Can only remove 10 addresses at a time.");
        for (uint256 i; i < addresses.length; i++) {
            require(addresses[i] != address(0), "Can't remove the null address");
            _freeMintAllowList[addresses[i]] = false;
        }
    }

    function onAllowList(address _address) external view returns (bool) {
        return _freeMintAllowList[_address];
    }
    ...
    function changeSeason() external onlyOwner returns (uint256 season) {
        _changeSeason();
    ...
    function _changeSeason() internal {
        ...
        emit SeasonChanged(_seasonCounter._value, currentSeasonStartDate);
    }
    ...
    function updateTierConfigs(
        ...
        address[] calldata restrictToAddresses
    ) external onlyOwner {
        for (uint256 i; i < tierIds.length; i++) {
            tierConfigs[tierIds[i]] = TierConfig(tiers[i], weiAmounts[i], validDurations[i]);
            address restrictToAddress = restrictToAddresses[i];
            if (restrictToAddress != address(0)) {
                _tierConfigAddressRestriction[tierIds[i]] = restrictToAddress;
            }
        }
    }
    ...
    function mintLicense(uint256 tierId) external payable {
        address restrictedToAddress = _tierConfigAddressRestriction[tierId];
        bool isRestrictedTier = restrictedToAddress != address(0);

        require(!salePaused || isRestrictedTier, "Public sale is paused.");

        // copy to memory because we might delete the storage tierConfig for the given tierId if restricted to address (one-shot)
        TierConfig memory tierConfig = tierConfigs[tierId];
        require(tierConfig.tier != Tier.NONE, 'This tier does not exist.');

        if (isRestrictedTier) {// custom listing
            require(restrictedToAddress == _msgSender(), 'This tier is restricted.');
            // restricted tiers are one-time use
            delete _tierConfigAddressRestriction[tierId];
            delete tierConfigs[tierId];
        } else if (tierConfig.tier == Tier.FREE) {// free license in unrestricted listing needs to be on allowlist
            // require permission on list
            require(_freeMintAllowList[_msgSender()], 'Address cannot mint a free license.');
            // free mint is one-time use
            delete _freeMintAllowList[_msgSender()];
        }

        // require enough eth to pay for tier
        require(tierConfig.weiAmount <= msg.value, 'ETH amount is not sufficient');
        _performMint(tierConfig.tier, _calculateValidUntil(tierConfig.validDuration), _msgSender());
    }
