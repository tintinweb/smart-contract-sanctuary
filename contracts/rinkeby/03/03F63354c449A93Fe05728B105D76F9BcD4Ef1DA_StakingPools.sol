...
contract StakingPools is AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    struct UserInfo {
        ...
        uint256 amountFlype; // How many Flype tokens the user has provided.
    ...
    struct PoolInfo {
        ...
        uint256 flypeTotalSupply; // Total amount of deposited Flype
        bool paused;
        uint256 deadline;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, uint256 _amountFlype) public virtual nonReentrant hasPool(_pid) poolRunning(_pid) {
        ...
        uint256 poolAmountFlypeBefore = user.amountFlype;
        user.amount += _amount;
        user.amountFlype += _amountFlype;
        poolInfo[_pid].flypeTotalSupply += _amountFlype;
        for (uint256 i = 0; i < pool.rewardsTokens.length; i++) {
            _updateUserInfo(_pid, pool, user, i, poolAmountBefore, poolAmountFlypeBefore, pool.flypeTotalSupply);
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount, uint256 _amountFlype) public virtual nonReentrant poolRunning(_pid) hasPool(_pid) {
        ...
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_msgSender()];
        require((user.amount > 0 && user.amount >= _amount) || (user.amountFlype > 0 && user.amountFlype >= _amountFlype), "Flype::staking:withdraw: wrong amount");
        uint256 poolAmountBefore = user.amount;
        uint256 poolAmountFlypeBefore = user.amountFlype;
        if(_amount == 0) _claimRewards(_pid, _msgSender());
        user.amount -= _amount;
        user.amountFlype -= _amountFlype;        
        poolInfo[_pid].flypeTotalSupply -= _amountFlype;
        if(_amount > 0) // gas savings
        {
            for (uint256 i = 0; i < pool.rewardsTokens.length; i++) {
                _updateUserInfo(_pid, pool, user, i, poolAmountBefore, poolAmountFlypeBefore, pool.flypeTotalSupply);
            }
    ...
    function _updateUserInfo(
        ...
        if (_amount > 0) {
            uint256 reward = (_amount * accumulatedPerShare)/ pool.accuracy[_tokenNum];
            ...
            if (pending > 0) {
                IERC20(pool.rewardsTokens[_tokenNum]).safeTransfer(_msgSender(), pending - user.rewardsDebts[_tokenNum]);
            }
        }
        uint256 _reward = (user.amount * accumulatedPerShare) / pool.accuracy[_tokenNum];
        uint256 _baseReward = _reward * 75 / 100;
        uint256 _bonusReward = 0;
        if(user.amountFlype > 0) _bonusReward = (_reward - _baseReward) * user.amountFlype / poolInfo[_pid].flypeTotalSupply;
        user.rewardsDebts[_tokenNum] = _baseReward + _bonusReward;
    }
    ...
    function pendingRewards(uint256 _pid, address _user) external view hasPool(_pid) notZeroAddress(_user) returns (uint256[] memory amounts) {
        PoolInfo memory pool = poolInfo[_pid];
        ...
        for (uint256 i = 0; i < pool.rewardsTokens.length; i++) {
            ...
            uint256 reward =  (user.amount * accumulatedPerShare) / pool.accuracy[i];
            uint256 baseReward = reward * 75 / 100;
            uint256 bonusReward;
            if(pool.flypeTotalSupply > 0) bonusReward = (reward - baseReward) * user.amountFlype / pool.flypeTotalSupply; 
            amounts[i] = baseReward + bonusReward - rewardsDebts;
        }
    }
    ...
    function userPoolAmountFlype(uint256 _pid, address _user) external notZeroAddress(_user) hasPool(_pid) view returns (uint256) {
        return userInfo[_pid][_user].amountFlype;
    }
    ...
    function _claimRewards(uint256 _pid, address _user) internal {
        ...
        for (uint256 i = 0; i < pool.rewardsTokens.length; i++) {
            amounts[i] = _updateUserInfo(_pid, pool, user, i, user.amount, user.amountFlype, pool.flypeTotalSupply);
        }
