...
contract GMLoot is ERC721Enumerable, Ownable {  
    ...
    // counts the tokens that got merged/burned
    ...
    mapping (uint256 => uint256[]) public mergedTokens;

    // Merge function
    function mergeTokens(uint256 hodlToken, uint256 burnToken) public {
        require(hodlToken != burnToken, "Can't merge the same token");

        // Returns array of MVL token IDs owned by the sender
        uint256[] memory ownedTokens = tokensOfOwner(msg.sender);

        bool ishodlValid = false;
        bool isburnValid = false;

        // Check if token params belongs to sender
        for(uint i = 0; i < ownedTokens.length; i++) {
            if (ownedTokens[i] == hodlToken) {
                ishodlValid = true;
                continue;
            }

            if (ownedTokens[i] == burnToken) {
                isburnValid = true;
                continue;
            }
        }

        require(ishodlValid == true && isburnValid == true, "Requested merge tokens don't belong to sender.");

        // Merge tokens
        // Save merge relation
        uint256[] memory newMergeValue = new uint256[](mergedTokens[hodlToken].length + 1);

        // copy old values in new array
        for (uint256 k = 0; k < mergedTokens[hodlToken].length; k++) {
            newMergeValue[k] = mergedTokens[hodlToken][k];
        }

        // adding the new burned token
        newMergeValue[newMergeValue.length - 1] = burnToken;

        // save new array to hodlToken
        mergedTokens[hodlToken] = newMergeValue;

        // increase burnCount
        burnCount = burnCount + 1;

        // burn the burnToken after merge relation save
        _burn(burnToken);
    }

    function getMergedRelationById(uint256 tokenId) public view returns(uint256[] memory){
        uint256[] memory returnValue = new uint256[](mergedTokens[tokenId].length);

        for (uint256 k = 0; k < mergedTokens[tokenId].length; k++) {
            returnValue[k] = mergedTokens[tokenId][k];
        }
    ...
    function withdrawTeam(uint256 amount) public payable onlyOwner {
        ...
        require(payable(a1).send(percent * 80)); // Pixelord
        require(payable(a2).send(percent * 20)); // Developers
        // TODO: Collection wallet?
        // require(payable(a3).send(percent * ?));
    }
}
