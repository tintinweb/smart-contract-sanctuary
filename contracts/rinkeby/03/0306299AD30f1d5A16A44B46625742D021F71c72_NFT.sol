...
// import "./MyDataConsumerForShib.sol";
// import "./MyDataConsumerForLeash.sol";
...
contract MyDataConsumerForLeash is ConsumerBase, Ownable {
    ...
    // RinkeBy 
    address private constant ROUTER_ADDRESS = address(0x05AB63BeC9CfC3897a20dE62f5f812de10301FDf);

    // RinkeBy
    address private constant XFUND_ADDRESS = address(0x245330351344F9301690D5D8De2A07f5F32e1149);
    ...
    modifier onlyNFTOrOwner() {
        require(msg.sender == NFT || msg.sender == owner(), "Price Can only be fetched by NFT contract or the Owner");
        _;
    }

    constructor() ConsumerBase(ROUTER_ADDRESS, XFUND_ADDRESS) {
        price = 0;
    }

    // Optionally protect with a modifier to limit who can call
    function getData() external onlyNFTOrOwner returns (bytes32) {

        bytes32 _data = 0x4c454153482e4554482e50522e41564900000000000000000000000000000000;  //SHIB.ETH.PR.AVI
...
contract MyDataConsumerForShib is ConsumerBase, Ownable {
    ...
    function getData() external onlyNFTOrOwner returns (bytes32) {
        ...
        bytes32 _data = 0x534849422e4554482e50522e4156490000000000000000000000000000000000;  //SHIB.ETH.PR.AVI
...
contract NFT is ERC721, Ownable {
    ...
    IERC20 public LEASH;
    IERC20 public SHIB;
    MyDataConsumerForLeash public LeashPrice;
    MyDataConsumerForShib public ShibPrice;

    string public TESTNFTS_PROVENANCE = "";
    ...
    uint256 private NAME_CHANGE_PRICE ;
    ...
    constructor(string memory name, string memory symbol,  address _leashToken, address _shibToken, address  _leashpriceToken, address _shibPriceToken) ERC721(name, symbol) {
        LEASH = IERC20(_leashToken);
        SHIB = IERC20(_shibToken);
     ...
     */
    function getNFTPriceForLEASH() public view returns (uint256) {
        ...
        if (currentSupply >= 9000) {
            uint256 num =7 ; 
            return getLeashEthPrice().mul(num).div(10); // 9000 - 1000 0.7 LEASH
        } else if (currentSupply >= 3000) {
            uint256 num =5 ;
            return getLeashEthPrice().mul(num).div(10); // 3000 - 8999 0.5 LEASH
        } else {
            uint256 num =3 ;
            return getLeashEthPrice().mul(num).div(10); // 0 - 2999 0.3 LEASH
        }
    }
    
    function getNFTPriceForETH() public view returns (uint256) {
        require(block.timestamp >= SALE_START_TIMESTAMP, "Sale has not started");
        require(totalSupply() < MAX_NFT_SUPPLY, "Sale has already ended");
        ...
        if (currentSupply >= 8000) {
            return 500000000000000000; // 9000 - 1000 0.5 ETH
        } else if (currentSupply >= 3000) {
            return 300000000000000000; // 3000 - 7999 0.3 ETH
        } else {
            return 100000000000000000; // 0 - 2999 0.1 ETH
        }
    }

    function getNameChangePrice() public returns (uint256) {
        NAME_CHANGE_PRICE = getShibEthPrice();
    ...
    function getShibEthPrice() internal view returns(uint256){
        return ShibPrice.price();
    }
    ...
    /**
     * @dev Changes the name for testNFT tokenId
     */
    function changeName(uint256 tokenId, string memory newName) public {
        ...
        SHIB.transferFrom(msg.sender, address(this), NAME_CHANGE_PRICE);
        ...
        ERC20Burnable(address(SHIB)).burn(NAME_CHANGE_PRICE);
    ...
    */
    function setProvenanceHash(string memory provenanceHash) public onlyOwner {
        TESTNFTS_PROVENANCE = provenanceHash;
    }
    ...
    */
    function mintNFT(uint numberOfNfts, uint256 _value, bool _isETH) public payable {
        ...
        require(numberOfNfts <= CAPPED_NFT_LIMIT_PER_USER, "You may not buy more than 10 NFTs at once");
        require(totalSupply().add(numberOfNfts) <= MAX_NFT_SUPPLY, "Exceeds MAX_NFT_SUPPLY");
        require(IERC721(address(this)).balanceOf(msg.sender) < CAPPED_NFT_LIMIT_PER_USER, "Exceeds CAPPED_NFT_LIMIT_PER_USER");
        ...
        {
            require(getNFTPriceForLEASH().mul(numberOfNfts) == _value, "LEASH value sent is not correct");
            IERC20(LEASH).transferFrom(msg.sender, address(this), _value);
        }
        ...
        {
            require(getNFTPriceForETH().mul(numberOfNfts) == msg.value, "ETH value sent is not correct");
        }
