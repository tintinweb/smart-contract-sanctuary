...
contract Betting is Ownable {
  ...
  using SafeMath for uint;
	...
	uint public referrerFee = 50; //50%
	uint public adminFee = 50; //50%
	...
	function init(address _managerAddress, address _eth_usd_consumer_address, address[] memory supportedTokens, address[] memory chainlink_price_consumer_addresses, bool[] memory _isStablecoin) public {
	...
	function closeGame(uint t, uint gameIndex, address payable winnerAddress) public onlyManager {
		...
		require(!(typeGames[t][gameIndex].amount1 == typeGames[t][gameIndex].amount2 && block.timestamp.sub(typeGames[t][gameIndex].latestBet) < 1 days), "game already closed");
		...
		internalTransfer(winnerAddress, winnerAddress == _game.gamer1 ? _game.gamer2 : _game.gamer1, _game.token1, _game.amount1, totalFee1);
		internalTransfer(winnerAddress, winnerAddress == _game.gamer1 ? _game.gamer2 : _game.gamer1, _game.token2, _game.amount2, totalFee2);
