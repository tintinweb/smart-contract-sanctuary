...
contract BattleContract is VRFConsumerBase, Ownable {
        ...
        returns (bytes32 requestId)
    {
        ...
        requestId = requestRandomness(keyHash, fee, block.timestamp);
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness)
        ...
        override
    {
        uint256 battleId = requestBattleIds[requestId] - 1;
        require(battleId >= 0, "BattleContract: Invalid Chainlink Request");

        battleRandomness[battleId] = randomness;
        battleWinnerIndexes[battleId] = randomness;
        uint256 winnerIndex = randomness % participantsCount[battleId];

        Participant memory participant = participants[battleId][winnerIndex];
        battleHistories[battleId] = participant;

        _burnNFT(battleId, winnerIndex);

        nft_.setNFTLevelUp(participant.nftId);
        nft_.setNFTURI(participant.nftId, battles[battleId].uri);

        emit NewWinner(battleId, participant.nftId, participant.nftOwner);
    }
