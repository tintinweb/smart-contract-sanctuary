...
contract BridgeChain is Ownable, Pausable{
  ...
  // 提取费用日志
  event WithdrawFee(address indexed operator, address indexed receiver, uint256 amount);
  // 检查管理员
  ...
  // 检测单比最大 单比最小
  modifier checkBridgeIn(address _token, uint256 _toChain, uint256 _crossAmount) {
    ...
    require(_crossAmount >= bridgeConfig.singleMin, 'BridgeChain: less than single minimum quantity');
    require(_crossAmount <= bridgeConfig.singleMax, 'BridgeChain: greater than the maximum quantity');
    _;
  }
  ...
  function bridgeBack(address _receiver, address _token, uint256 _amount, uint256 _fee, string calldata _fromTxid) external onlyManager{
    ...
    recordMapping[msg.sender][_token] = record.sub(_amount);
  ...
  // 提取手续费
  function withdrawFee(address _receiver, uint256 _amount) external onlyManager{
    uint256 balance = IERC20(feeToken).universalBalanceOf(address(this));
    require(balance >= _amount, 'BridgeChain: greater than current balance');
    IERC20(feeToken).universalTransfer(_receiver, _amount);
    Fee storage fee = feeMapping[feeToken];
    fee.currentFee = fee.currentFee.sub(_amount);
    emit WithdrawFee(msg.sender, _receiver, _amount);
  }
  // 设置提币权限
