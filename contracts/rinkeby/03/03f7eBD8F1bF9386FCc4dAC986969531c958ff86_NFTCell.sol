...
contract NFTCell is Evolable, Ownable {
    ...
    function batchCreate() external onlyOwner {
        ...
        // create 10k cells with ids
        CellData.Cell memory newCell = ILaboratory(laboratory).createBatch(msg.sender);

        // save 10k
        //        ICellRepository(repository).addBatch(newCell, ids);
        ICellRepository(repository).addBatch(newCell);

        // mint 10k cells
        //        ICellToken(nftToken).mintBatch(msg.sender, ids, amounts, "");
        ICellToken(nftToken).mintBatch(msg.sender);

        emit InitialSupplyMinted(msg.sender, "Minted initial supply of 10k tokens");
    }

    function batchCreateV2() external onlyOwner {

//        uint256 maxCount = 10000;
//        uint256 oneBatch = 100;
//        uint256 iterator = maxCount / oneBatch;
        ...
        //            ICellRepository(repository).addBatch(newCell); // adds 100 cells
        //            ICellToken(nftToken).mintBatch(msg.sender); //  // mints 100 cells
        //            console.log('>>Minted & added: %s tokens', oneBatch);
        //        }
...
//    function addBatch(CellData.Cell memory cell, uint256[] memory _ids) external virtual;
    function addBatch(CellData.Cell memory cell) external virtual;
    /**
...
interface ILaboratory is ISeed {
    ...
    function createBatch(address tokenOwner) external returns (CellData.Cell memory);
