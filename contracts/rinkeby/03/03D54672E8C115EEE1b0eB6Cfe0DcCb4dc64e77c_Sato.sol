...
contract Sato is ERC721, ERC721Enumerable, SatoDNA {
    ...
    constructor(uint256 _maxSupply) ERC721('Sato', 'S4T0'){
        maxSupply = _maxSupply;
    }

    function mint() public {
        uint256 current = _idCounter.current();
        require(current < maxSupply, 'No S4T0 left');
    ...
    // Consultar metadata por id
    // override se aplica porque se reemplaza en la cadena de herencia
    function tokenURI(uint256 tokenId) 
        public 
        view 
        override 
        returns(string memory ){
            // consultar solo los tokens que existen:
            require(
                _exists(tokenId), 
