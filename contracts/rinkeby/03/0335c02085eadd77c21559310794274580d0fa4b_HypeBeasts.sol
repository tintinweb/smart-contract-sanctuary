...
contract HypeBeasts is HypeBeastCoreERC721, ReentrancyGuard, VRFConsumerBase {
    ...
    VRFConsumerBase(VRFCoordinator, LinkToken)
  {
    ...
    traitProbabilities[0] = eyesProbabilities;
    traitNames[0] = eyesNames;
    traitCategories[0] = "Eyes";
  }
  ...
  function traitMetadata(uint256 _tokenId) public view returns (string memory) {
    ...
    for (uint8 i = 0; i < numberOfTraits; i++) {
      ...
      resultString = strConcat(resultString, "\': \'");
      resultString = string(abi.encodePacked(bytes(resultString), traitNames[i][mintedTraits[_tokenId][i]]));
      resultString = strConcat(resultString, "\'");
    }
    resultString = strConcat(resultString, "}");
    return resultString;
    ...
    for(uint8 i = 0; i < numberOfTraits; i++) {
      ...
      for (uint8 j = 0; j < traitProbabilities[i].length; j++) {
        ...
        if (dnaModuloWeight > currentWeight) {
          mintedTraits[vrfRequestIds[_requestId]][i] = j;
          break;
        }
      }
    }
