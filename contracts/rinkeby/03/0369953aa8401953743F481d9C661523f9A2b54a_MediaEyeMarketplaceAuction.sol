...
import "./interfaces/ISubscriptionTier.sol";
import "./interfaces/IMinter.sol";
import "./interfaces/IMarketplaceInfo.sol";
    ...
    ReentrancyGuard
{
    ...
    bytes32 internal immutable _DOMAIN_SEPERATOR;
    bytes32 public constant AUCTION_SIGNATURE_TYPEHASH =
        keccak256(
     ...
     * _newFeeContract: new MediaEyeFee contract
     * _newInfoContract: new MediaEyeMarketplaceInfo contract
     ...
     *
     * Params:
     * _label: auction label
     * _nfts: nfts to auction
     * _auctionPayments: payment methods to accept and initial price/ buy it now (optional)
     * _setRoyalty: if we set royalty (creator only)
     * _royalty: royalty amount to original seller
     * _split: how to split the revenue if any
    ...
    function startAuction(
        ...
        MediaEyeOrders.Nft[] memory _nfts,
        MediaEyeOrders.AuctionPayment[] memory _auctionPayments,
        bool _setRoyalty,
        uint256 _royalty,
        ...
        require(_auctionPayments.length > 0 && _nfts.length > 0, "length");
        require(_endTime > _startTime && _endTime > block.timestamp, "time");
        require(
            _split.splitBasisPoint + _split.charityBasisPoint <= 10000,
            "total payout over 100%"
        );

        if (_nfts.length > 1) {
            ISubscriptionTier.UserSubscription
                memory userSub = ISubscriptionTier(feeContract)
                    .getUserSubscription(msg.sender);

            require(
                userSub.subscriptionTier !=
                    ISubscriptionTier.SubscriptionTier.Unsubscribed &&
        ...
        for (uint256 i = 0; i < _auctionPayments.length; i++) {
            require(
                mediaEyeMarketplaceInfo.isPaymentMethod(
                    _auctionPayments[i].paymentMethod
                ),
                "payment"
            );

            if (_auctionPayments[i].paymentMethod == address(0)) {
                require(
                    _auctionPayments[i].buyItNowPrice > 0,
                    "invalid but now price"
                );
                auctionBuyItNowAmounts[auctionId][
                    _auctionPayments[i].paymentMethod
                ] = _auctionPayments[i].buyItNowPrice;
            } else {
                require(_auctionPayments[i].initialPrice > 0, "invalid price");
                auctionInitialAmounts[auctionId][
                    _auctionPayments[i].paymentMethod
                ] = _auctionPayments[i].initialPrice;
                if (_auctionPayments[i].buyItNowPrice > 0) {
                    auctionBuyItNowAmounts[auctionId][
                        _auctionPayments[i].paymentMethod
                    ] = _auctionPayments[i].buyItNowPrice;
                }
        ...
        if (!_setRoyalty) {
            compareRoyaltyRecipient = mediaEyeMarketplaceInfo
                .getRoyalty(_nfts[0].nftTokenAddress, _nfts[0].nftTokenId)
                .artist;
        }
        ...
        for (uint256 i = 0; i < _nfts.length; i++) {
            ...
            // transfer
            if (_nfts[i].nftTokenType == MediaEyeOrders.NftTokenType.ERC721) {
                require(_nfts[i].nftNumTokens == 1, "ERC721 Amount");
                IERC721(_nfts[i].nftTokenAddress).safeTransferFrom(
                    msg.sender,
                    address(this),
                    _nfts[i].nftTokenId
                );
            } else if (
                _nfts[i].nftTokenType == MediaEyeOrders.NftTokenType.ERC1155
            ) {
                IERC1155(_nfts[i].nftTokenAddress).safeTransferFrom(
                    msg.sender,
                    address(this),
                    _nfts[i].nftTokenId,
                    _nfts[i].nftNumTokens,
                    ""
                );
            }
            auction.nfts.push(_nfts[i]);
        }
        ...
        auction.split = _split;

        auctionIds.add(auctionId);
        _auctionIds.increment();

        emit AuctionCreated(auctions[auctionId], _auctionPayments);
    }
    ...
    function cancelAuction(uint256 _auctionId) external nonReentrant {
        require(auctionIds.contains(_auctionId), "MediaEyeOrders.Auction?");
    ...
    function auctionBuyItNow(
        ...
        require(auctionIds.contains(_auctionId), "null auction");

        uint256 price = auctionBuyItNowAmounts[_auctionId][
    ...
    function _sendPayments(
        ...
        MediaEyeOrders.ListingPayment memory _listingPayment,
        MediaEyeOrders.Split memory _split,
        address _payer
    ) internal {
        ...
        uint256 payoutToTreasury = (_listingPayment.price * basisPointFee) /
            10000;
        ...
        if (royalty.royaltyBasisPoint > 0) {
            payoutToCreator =
                (_listingPayment.price * royalty.royaltyBasisPoint) /
                10000;
        }

        // payout to Charity/sellers
        uint256 remainingPayout = (_listingPayment.price *
        ...
        if (_listingPayment.paymentMethod == address(0)) {
            ...
            IERC20(_listingPayment.paymentMethod).transferFrom(
                _payer,
                treasuryWallet,
                payoutToTreasury
            );
    ...
    function sellerClaimBySig(
        ...
        MediaEyeOrders.Auction memory auction = auctions[
            _auctionSignature.auctionId
        ];

        require(msg.sender == auction.seller, "auctioner");
        require(block.timestamp >= auction.endTime, "endtime");
        ...
        bytes32 digest = keccak256(
            abi.encodePacked("\x19\x01", _DOMAIN_SEPERATOR, structHash)
        );
        require(
            ecrecover(
                digest,
        ...
        _sellClaim(_auctionSignature, auction);
    }
