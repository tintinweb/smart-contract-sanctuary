...
contract ShroomHeads is AccessControl, ReentrancyGuard, ERC1155Supply
{
    ...
    constructor(address _owner, string memory uri_, address[] memory _team, uint256[] memory _shares, address wETH, address _signerAddress) ERC1155(uri_) {
    ...
    */
    function auctionMint(uint256 tokenId, address winningBidder, uint256 winningBid, uint256 ts, bytes memory _signature) external onlyRole(TEAM) {
        require(winningBidder == keccak256(abi.encode("Shroomheads place bid_", tokenId, "_", winningBidder, "_", winningBid, "_", ts, "_", address(this))).toEthSignedMessageHash().recover(_signature), "Bad signature");
