...
// File contracts/ico/Crowdsale.sol
 ...
 * behavior.
 */
contract Crowdsale is Context, Ownable, ReentrancyGuard, Listing {
    ...
    uint256 public _preIcoWeiRaised;

    uint256 public icoSold;
    uint256 public preIcoSold;
    uint256 public _hardCap = 1000 ether;

    // PRE-ICO Start Date (16th August)
    uint256 public _preIcoStartDate = 1629072000;

    // PRE-ICO End Date (27th August)
    uint256 public _preIcoEndDate = 1630047600;

    // PRE-ICO Tokens available (15%)
    uint256 public _preIcoTokens = 75000000000 ether;

    // ICO Start Date (28th August)
    uint256 public _icoStartDate = 1630108800;

    // ICO End Date (28th September)
    uint256 public _icoEndDate = 1632787200;

    // ICO Tokens available (70%)
    uint256 public _icoTokens = 350000000000 ether;
        ...
        checkIcoTime
    {
        ...
        if (stage == CrowdsaleStage.PREICO) {
            _addWhitelist(beneficiary);
            _preIcoWeiRaised = _preIcoWeiRaised.add(msg.value);
            return;
        }
        // require(
        ...
        if ((stage == CrowdsaleStage.ICO) && (icoSold.add(tokens) > _icoTokens))
            revert("ICO_SALE_TOKENS_SOLD_OUT");
    ...
    ) external onlyOwner checkPreIcoTime {
        require(indexes.length == value.length);
        require(whiteListAddresses.length >= indexes.length);
        uint256 len = indexes.length;
        uint256 sum = 0;

        for (uint256 i = 0; i < len; ++i) {
            require(indexes[i] < whiteListAddresses.length);
            require(whiteListAddresses[indexes[i]] != address(0));
            sum = sum.add(value[i]);
        }
        preIcoSold = preIcoSold.add(sum);
        require(preIcoSold < _preIcoTokens, "PREICO_SALE_TOKENS_SOLD_OUT");

        for (uint256 j = 0; j < len; ++j) {
            if (investorsAllocated[whiteListAddresses[indexes[j]]] == 0)
                investorsAllocatedAddresses.push(
                    whiteListAddresses[indexes[j]]
                );
            investorsAllocated[
                whiteListAddresses[indexes[j]]
            ] = investorsAllocated[whiteListAddresses[indexes[j]]].add(
                value[j]
            );
        }
    }
     ...
     */
    function distributeToPrivateInvestors(uint256 count)
        external
        onlyOwner
        checkPreIcoTime
    {
        uint256 addressesLeft = whiteListAddresses.length.sub(
            investorsProcessed
        );
        ...
        for (uint256 i = investorsProcessed; i < top; ++i) {
            _token.transfer(
                ...
                investorsAllocated[whiteListAddresses[i]]
            );
            ++investorsProcessed;
        }
    }
    ...
    function finish() public onlyOwner {
        require(hasEnded(), "Crowdsale: CROWDSALE_NOT_ENDED");
        state = State.Closed;
        uint256 tokenAmount = _preIcoTokens.mul(60).div(100); // 60 %
        uint256 ethAmount = _preIcoWeiRaised.mul(40).div(100); // 40 %
 ...
 * token purchases and the crowdsale will assign them RAT tokens based
 * on a RAT token per ETH rate. Funds collected are forwarded to a wallet
 * as they arrive
 */
contract RatIco is Crowdsale {
    // Constructor
    // ============
    constructor(
        uint256 tokenRate,  
        ...
        address payable walletAddress,
        address router
    ) Crowdsale(tokenRate, crowdsaleToken, walletAddress, router) {}
