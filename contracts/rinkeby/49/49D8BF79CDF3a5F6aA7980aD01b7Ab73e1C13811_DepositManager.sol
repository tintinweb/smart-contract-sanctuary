...
contract DepositManager is BaseGovernanceWithUserUpgradable {
    ...
    EnumerableMapUpgradeable.UintToAddressMap private _beneficiaries ;
     ...
     */
    function getProperties(uint id) public view returns(address beneficiary, uint256 additionTime, uint256 initial, uint256 claimed ) {
        beneficiary  = _beneficiaries.get(id);
     ...
     * @return List of NFTs IDs to remove
     */
    function removeDeposits(bytes calldata data) external onlyLockContract returns(uint[] memory) {
        uint[] memory IDs = _decodeDataToRemove(data);
        lockContract.burnNFTs(IDs);
    ...
    function split(uint originId, uint[] memory splitParts, address[] memory addresses) external onlyLockContract returns(uint[] memory){
        ...
        uint[] memory origin = new uint[](1);
        origin[0] = originId;

        lockContract.burnNFTs(origin);
        _removeID(originId);
    ...
    function _setBeneficiary(
        ...
        internal 
        {
            _beneficiaries.set(_id, _beneficiary);
            _properties[_id] = Property(_additionTime, _amount, _claimed);
    }

    function _removeID(uint _id) internal {
        _beneficiaries.remove(_id);
        delete _properties[_id]; 
    }

    function _decodeDataToRemove(bytes calldata _data) internal view returns(uint[] memory) {
        uint[] memory IDs = abi.decode(_data, (uint[]));
    ...
    function _existIds(uint[] memory IDs) internal view returns(bool) {
        for (uint i; i < IDs.length; i++) {
            if (_beneficiaries.contains(IDs[i])) {
                return true;
            }  
        }
        return false;
