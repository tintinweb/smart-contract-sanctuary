...
contract ReentalToken is ERC20CappedUpgradeable, UUPSUpgradeableByRole {
  ...
  function mint(address to, uint256 amount) external onlyRole(keccak256("MINTER_ROLE")) {
    _mint(to, amount);
  }
  ...
  function burnFrom(address account, uint256 amount) external onlyRole(keccak256("MINTER_ROLE")) {
  ...
  function _swap(address from, address to, uint256 amount) internal onlyRole(keccak256("MINTER_ROLE")) {
    require(from != address(0), "ReentalToken: _from address is the zero address");
    require(to != address(0), "ReentalToken: _to address is the zero address");
  ...
  function extractTokens(address token) public onlyRole(DEFAULT_ADMIN_ROLE) {
      ...
      require(tokenInterface.transfer(_msgSender(), balance), "ReentalToken: extraction failed");
  }

  function extractETH() public onlyRole(DEFAULT_ADMIN_ROLE) {
      require(payable(_msgSender()).send(address(this).balance), "ReentalToken: funds transfer failed");
  }
}
