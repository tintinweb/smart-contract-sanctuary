...
contract UniswapV3Staker is IUniswapV3Staker, Multicall {
    ...
    /// @inheritdoc IUniswapV3Staker
    function isOutOfRange(IncentiveKey memory key, int24 tickLower, int24 tickUpper) public override view returns (bool){
    ...
    function unstakeToken(IncentiveKey memory key, uint256 tokenId) external override {
        ...
        if (block.timestamp < key.endTime) {
            require(isOutOfRange(key, deposit.tickLower, deposit.tickUpper),'UniswapV3Staker:: non out of range');
            require(
...
interface IUniswapV3Staker is IERC721Receiver, IMulticall {
    ...
    function isOutOfRange(IncentiveKey memory key, int24 tickLower, int24 tickUpper) external view returns (bool);
