...
contract Kazoo is ERC721URIStorage, Ownable {
    ...
    function _mint(address _addr, bool _isSponsored) private
    {
        ...
        KazooObject memory object = KazooObject(_addr, kazooId, _mNumber, "<svg></svg>","Unnamed Kazoo", new string[](MAX_NAMES), new uint256[](15), 0, _isSponsored, _isSpecial, new string[](13), new string[](13));
        ...
        //emit we are done
        emit mintedKazoo(kazooId, kazoos[kazooId]); //emit the updated method

        //finally, mint the finished kazoo ending the transaction request
        _safeMint(_addr, kazooId++);
    }

    function rebuildKazoo(uint256 _kazooId) private view {
        ...
        string memory _mColor;

        if( _kaz.masterNumber < RANDOM_NUMBER / 2)
            _mColor = SVGKazoo.getHexCode( RANDOM_NUMBER );
        else
            _mColor = SVGKazoo.getHexCode( _kaz.masterNumber >> 3 );

        _kaz.kazoo = SVGKazoo.buildKazoo(_styles, _kaz.names, SVGKazoo.getHexCode( _kaz.masterNumber >> 2 ), _mColor, _kaz.isSpecial, _kaz.masterNumber);
        _kaz = kazoos[_kazooId];
    }
    ...
    function buildKazooSVG(uint256 _kazooId) private {
        ...
        string memory _staticBorder = SVGKazoo.getHexCode( _kaz.masterNumber >> 1 );
        string memory _mColor;

        if( _kaz.masterNumber < RANDOM_NUMBER / 2)
            _mColor = SVGKazoo.getHexCode( RANDOM_NUMBER );
        else
            _mColor = SVGKazoo.getHexCode( _kaz.masterNumber >> 2 );
        ...
        for(uint256 i = 0; i < 13; i++){
            ...
            _kaz.backgroundColours[i] = SVGKazoo.getHexCode((( _kaz.masterNumber << 1 ) * (i + 7) * 6));
    ...
    function remintKazoo(uint256 _kazooId) public payable {
        ...
        _kaz.masterNumber = randomNumber(RANDOM_NUMBER);
        kazoos[_kazooId] = _kaz;

        buildKazooSVG(_kazooId);
        nameKazoo(_kazooId);
    }
    ...
    function nameKazoo(uint256 _kazooId) private {
        ...
        uint256 _randomNameNumber = randomNumber( _kaz.masterNumber >> 2 );
...
library SVGKazoo {
        ...
        returns (string memory)
    {
        ...
        for (uint256 i = 0; i < _names.length; i++) {
            ...
            if (i != 0) _sp = (stringLength(_names[i - 1]) * 13);

            _cl = _cl + _sp;

            //if the current location + the length of the current name sets us over
            if (_cl + (stringLength(_names[i]) * 13) <= 185 && _cl > 345)
