...
contract FoliaControllerV2 is Ownable {
    ...
    function works(uint256 workId) public view returns (Work memory work) {
      if (workId > foliaController.latestWorkId()) {
        return _works[workId];
      } else {
        (bool exists, bool _paused, uint256 editions, uint256 printed, uint256 price, address payable artist) = foliaController.works(workId);
        return Work({
            exists: exists,
            paused: _paused,
            saleType: SaleType.noID,
            editions: editions,
            printed: printed,
            price: price,
            artist: artist
