...
// YELStaking is a "semifredo of popsicle stand" this contract is created to provide single side farm for IFO of Popsicle Finance.
...
contract YELStaking is Ownable {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "YELStaking: you cant eat that much popsicles");
        updatePool(_pid);
        uint256 pending =
            user.amount * pool.accYelPerShare / 1e12 - user.rewardDebt + user.remainingYelTokenReward;
        user.remainingYelTokenReward = safeRewardTransfer(msg.sender, pending);
