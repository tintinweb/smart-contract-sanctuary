...
contract FrostyStake is Owned {
    
    //initializing safe computations
    using SafeMath for uint;

    //FRST contract address
    address public frosty;
    //total amount of staked frosty
     ...
     * transfers FRST from sender's address into the smart contract
    ...
    function registerAndStake(uint _amount, address _referrer) external onlyUnregistered() whenActive() {
        ...
        require(IERC20(frosty).balanceOf(msg.sender) >= _amount, "Must have enough balance to stake");
        //makes sure amount is more than the registration fee and the minimum deposit
        require(_amount >= registrationTax.add(minimumStakeValue), "Must send at least enough FRST to pay registration fee.");
        //makes sure smart contract transfers FRST from user
        require(IERC20(frosty).transferFrom(msg.sender, address(this), _amount), "Stake failed due to failed amount transfer.");
        ...
        //update the total staked FRST amount in the pool
    ...
    function unstake(uint _amount) external onlyRegistered() {
        ...
        IERC20(frosty).transfer(msg.sender, afterTax);
    ...
    function withdrawEarnings() external returns (bool success) {
        ...
        require((IERC20(frosty).balanceOf(address(this))).sub(totalStaked) >= totalReward, 'Insufficient FRST balance in pool');
        ...
        IERC20(frosty).transfer(msg.sender, totalReward);
        //emit event
    ...
    function rewardPool() external view onlyOwner() returns(uint claimable) {
        return (IERC20(frosty).balanceOf(address(this))).sub(totalStaked);
    }
    ...
    function filter(uint _amount) external onlyOwner returns (bool success) {
        //makes sure _amount is not more than required balance
        require((IERC20(frosty).balanceOf(address(this))).sub(totalStaked) >= _amount, 'Insufficient FRST balance in pool');
        //transfers _amount to _address
        IERC20(frosty).transfer(msg.sender, _amount);
        //emit event
