...
interface IERC721 is IERC165 {
    ...
    function getFee(uint256 id) external view  returns(uint256[] memory);
}
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
  ...
  // mapping for token creator
    ...
    function getFeeRecipients(uint256 id) public view virtual override returns (address payable[] memory) {
        ...
        address payable[] memory recipient_address = new address payable[](_fees.length);
        //uint256[] memory recipient_value;
        for (uint i = 0; i < _fees.length; i++) {
            recipient_address[i] = _fees[i].recipient;
            //recipient_value[i] = _fees[i].value;
        }
        return recipient_address;
    }


    function getFee(uint256 id) public view virtual override returns(uint256[] memory) {
        Fee[] memory _fees = fees[id];
        uint256[] memory value;

        for(uint i = 0; i < _fees.length; i++) {
            value[i] =  _fees[i].value;
        }
        return value;
    }
    ...
    */
    ...
    function _safeMint(address to, uint256 tokenId, Fee[] memory _fees) internal virtual {
        _safeMint(to, tokenId, _fees, "");
    }

    function _safeMint(address to, uint256 tokenId, Fee[] memory _fees, bytes memory _data) internal virtual {
...
contract DEMONft721 is ERC721 {
     ...
     * @param _fees uint256 royalty of the token to be minted.
     */

    function createCollectible(string memory tokenURI, Fee[] memory _fees, Sign memory sign) public returns (uint256) {
        uint256 newItemId = tokenCounter;
        verifySign(tokenURI, sign);
        _safeMint(msg.sender, newItemId, _fees);
