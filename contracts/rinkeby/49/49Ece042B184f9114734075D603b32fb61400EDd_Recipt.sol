...
contract Recipt is 
    ...
    function generateMetadata(uint256 tokenId, bool image) public view returns (string memory) {
        ...
        string memory max = string(abi.encode("/",uint2str(maxSupply)));
        if (!image) {
        ...
        string memory time = string(abi.encodePacked(dt.hour,":",dt.minute));
        string memory mdy = string(abi.encodePacked(dt.month,"/",dt.day,"/",dt.year));
