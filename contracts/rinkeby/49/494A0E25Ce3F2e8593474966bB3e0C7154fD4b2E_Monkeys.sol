...
contract Monkeys is ERC721, ERC721Enumerable, Ownable {
    ...
    FInterface nContract = FInterface(fAddress);
    ...
    function setProvenance(string memory rate) public onlyOwner {
        PROVENANCE = random(string(abi.encodePacked('SFD-Monkeys', rate))) % 7000;
    }
        
    function getProvenance() public view returns (uint256) {
        return PROVENANCE;
    }
    ...
    function mintToken(uint tokenId) public {
        ...
        require(nContract.balanceOf(msg.sender) > 0, "Must own an Frog to mint token");
        require(nContract.ownerOf(tokenId) == msg.sender, "Must own the SFD to mint token");
