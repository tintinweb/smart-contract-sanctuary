...
contract DiggCollection is PoolTokenWrapper, Ownable, Pausable {
	...
	function earned(address account, uint256 pool) public view returns (uint256) {
		...
		// (uint256 mantissa, uint8 exponent) =
		// 	FractionalExponents.power(multiplyer.mul(poolBalance), uint256(1e18), rootN, rootD);
		...
		uint256 rewardRate =
			power((multiplyer.mul(poolBalance).mul(uint256(1e6)) << 127).div(uint256(1e18)), 10000 / 4).div(
				9867703801462435
			);
