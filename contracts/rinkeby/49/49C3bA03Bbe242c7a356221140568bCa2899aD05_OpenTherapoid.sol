...
contract OpenTherapoid is ERC20, Ownable, Pausable {
    ...
    constructor(
        ...
        super._mint(contractOwner, fixedSupply); // Since Total supply 500 Million NOBL
        // Sets the transferThreshold
        ...
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < adminLength; i++) {
            require(_admins[i] != address(0), "Admin can't be address zero");
            require(!isAdmin[_admins[i]], "Already an admin");
            isAdmin[_admins[i]] = true;
            //solhint-disable-next-line not-rely-on-time
            emit LogAddAdmin(_admins[i], block.timestamp);
        }
        ...
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < adminLength; i++) {
            require(_admins[i] != address(0), "Admin can't be address zero");
            require(isAdmin[_admins[i]], "Not an admin");
            isAdmin[_admins[i]] = false;
            //solhint-disable-next-line not-rely-on-time
            emit LogRemoveAdmin(_admins[i], block.timestamp);
        }
    }
    ...
    function bulkTransfer(
        ...
        for (uint256 i = 0; i < recipientsLength; i++) {
            require(
                ...
                "Recipient can't be address zero"
            );
            tokensIssued = totalTokensIssued.add(amounts[i]);
            //solhint-disable-next-line reason-string
            require(
                transferThreshold >= tokensIssued,
                "issueBulkSCN: Threshold exceeds, wait till threshold is updated"
            );
            totalTokensIssued = tokensIssued;
            if (isAdmin[msg.sender]) {
                transferFrom(owner(), recipients[i], amounts[i]);
            } else {
