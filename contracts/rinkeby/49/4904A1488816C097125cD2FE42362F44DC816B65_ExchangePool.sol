...
contract ExchangePool is Ownable, IERC721Receiver {
    ...
    bytes32 public constant CLAIM_TYPEHASH = keccak256("Claim(uint256 ticketId,address user,address tokenContract,uint256 tokenId)");

    event TokenReceived(address operator, address from, uint256 tokenId, address tokenContract, uint256 timestamp);
    event TokenClaimed(uint256 ticketId, address user, address tokenContract, uint256 tokenId, uint256 timestamp);

    constructor() {}

    function claim(uint ticketId, address user, address tokenContract, uint tokenId, uint8 v, bytes32 r, bytes32 s) external {
        bytes32 domainSeparator = keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes(CONTRACT_NAME)), getChainId(), address(this)));
        bytes32 structHash = keccak256(abi.encode(CLAIM_TYPEHASH, ticketId, user, tokenContract, tokenId));
        ...
        emit TokenClaimed(ticketId, user, tokenContract, tokenId, block.timestamp);
    }
    ...
    function onERC721Received(
        ...
        IERC721 token = IERC721(address(bytes20(data)));
