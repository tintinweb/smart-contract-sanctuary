...
import "./interfaces/IBridgeDistributor.sol";
...
contract BridgeDistributor is AbstractDistributor,
    IDistributor,
    Initializable,
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    IClaimingRegistry public claimRegistry;
    ...
    struct PolicyInfo {
        ...
        address[] polBooksArr;
        IPolicyRegistry.PolicyInfo[] policiesInfo;
        IClaimingRegistry.ClaimStatus[] status;
    }

    function __BridgeDistributor_init(
                address _contractsRegistry
        ) public initializer {
        __Ownable_init();
        __ReentrancyGuard_init();
        contractsRegistry    = IContractsRegistry(_contractsRegistry);
        policyRegistry       = IPolicyRegistry( contractsRegistry.getPolicyRegistryContract() );
        policyBookRegistry   = IPolicyBookRegistry( contractsRegistry.getPolicyBookRegistryContract() );
        claimRegistry        = IClaimingRegistry( contractsRegistry.getClaimingRegistryContract() );
        policyQuote          = IPolicyQuote( contractsRegistry.getPolicyQuoteContract() );
    }
    ...
    function getQuote(
        ...
        IPolicyBook policyBook = IPolicyBook(_bridgeProductAddress);

        IDistributor.CoverQuote memory coverQuote;
       (coverQuote.prop1, // totalSeconds, totalPrice
        coverQuote.prop2) = policyBook.getPolicyPrice(_bridgeEpochs,_amountInWei);
        coverQuote.prop3  = policyBook.totalLiquidity();
        coverQuote.prop4  = policyBook.totalCoverTokens();
    ...
    function buyCover (
       ...
       if (IERC20Upgradeable(stblToken).allowance(address(this), address(policyBook)) == uint256(0)) {
        ...
        // buy policyBook
        //TODO Use 'buyPolicyFor' from Bridge V2
        policyBook.buyPolicyFor(msg.sender, _durationSeconds,  _premium);
    }

    function buyCoverInsurace(IDistributor.BuyInsuraceQuote memory buy) external payable override nonReentrant {
            revert("BridgeDistributor: Unsupported operation");
    }
    ...
    function listWithStats(uint8 _offset, uint256 _limitLoop) external view
        ...
        (address[] memory _policyBooks,
         IPolicyBookRegistry.PolicyBookStats[] memory _stats) = policyBookRegistry.listWithStats(_offset, count);

         PolicyCatalog[] memory catalogList;
         PolicyCatalog memory catalog;

         for(uint8 i = 0; i < limit; i++){
               catalog.name = _stats[i].name;
               catalog.insuredContract = _stats[i].insuredContract;
               catalog.maxCapacity = _stats[i].maxCapacity;
               catalog.totalDaiLiquidity = _stats[i].totalDaiLiquidity;
...
interface IBridgeDistributor {
    struct PolicyInfo {
        uint256 policiesCount;
        address[] polBooksArr;
        IPolicyRegistry.PolicyInfo[] policies;
        IClaimingRegistry.ClaimStatus[] status;
    }
    ...
    function getOwnersPolicies(address _userAddr)
        external
        view
        virtual
        returns (uint256);
