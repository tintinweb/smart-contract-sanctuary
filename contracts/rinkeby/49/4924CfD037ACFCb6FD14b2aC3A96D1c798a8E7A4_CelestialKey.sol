...
contract CelestialKey is Auth, Pausable, ERC721("Celestial Key", "KEY") {
  using Strings for uint256;

  /// @notice max free mint supply
  uint256 public constant MAX_SUPPLY_FREE = 444;
  /// @notice max early access supply
  uint256 public constant MAX_SUPPLY_EARLY = 1444;
  /// @notice max total supply
  uint256 public constant MAX_SUPPLY_PUBLIC = 4444;
  ...
  mapping(address => uint256) public earlyAcessMinted;

  /// @notice OpenSea proxy registry.
  OpenSeaProxyRegistry public proxyRegistry;

  constructor(
    string memory unrevealedURI_,
    bytes32 merkleroot1_,
    bytes32 merkleroot2_,
    OpenSeaProxyRegistry proxyRegistry_
  ) {
    ...
    // mint genesis token to the owner
  ...
  function claimFree(bytes32[] memory proof_) external {
    ...
    require(
      msg.sender == owner() || saleState == 2,
      "Sale state is not free mint"
    );
    require(freeMinted[msg.sender] == 0, "User already minted max amount");
    require(supply <= MAX_SUPPLY_FREE, "All free NFT have been minted");
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(proof_, merkleRoot1, leaf), "Invalid proof");
  ...
  function claimEarly(uint256 amount_, bytes32[] memory proof_)
    external
    payable
  {
    // Claim requirements
    uint256 supply = totalSupply();
    require(
      msg.sender == owner() || saleState == 1,
      "Sale state is not early mint"
    );
    require(supply < MAX_SUPPLY_EARLY, "All early NFT have been minted");
    require(
      supply + amount_ <= MAX_SUPPLY_EARLY,
      "Minting would exceed max early supply"
    );
    require(amount_ > 0 && amount_ <= MAX_PER_TX, "Invalid mint amount");
    require(
      earlyAcessMinted[msg.sender] + amount_ <= MAX_PER_TX,
      "User already minted max amount"
    );
    require(
      msg.sender == owner() || msg.value == 0.044 ether * amount_,
      "Invalid ether amount"
    );
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(proof_, merkleRoot2, leaf), "Invalid proof_");

    earlyAcessMinted[msg.sender] += amount_;
  ...
  function claim(uint256 amount_) external payable {
    ...
    require(
      msg.sender == owner() || saleState == 2,
      "Sale state is not public mint"
    );
    require(supply < MAX_SUPPLY_PUBLIC, "All early NFT have been minted");
    require(
      supply + amount_ <= MAX_SUPPLY_PUBLIC,
      "Minting would exceed max public supply"
    );
    require(amount_ > 0 && amount_ <= MAX_PER_TX, "Invalid mint amount");
    require(supply + amount_ <= MAX_SUPPLY_PUBLIC, "Max supply exceeded");
    require(
      msg.sender == owner() || checkPrice() * amount_ == msg.value,
      "Invalid ether amount"
    );
  ...
  /// @notice Check actual token price on public sale based on time elapsed.
  function checkPrice() public view returns (uint256) {
    require(saleState == 2, "Sale state is not public mint");

    uint256 price = 0.5 ether -
      (0.012222 ether * ((block.timestamp - publicSaleStart) / 10 minutes));

    return price > 0.06 ether ? price : 0.06 ether;
  }
  ...
  function setSaleState(uint256 saleState_) external onlyAuthorized {
    ...
    if (saleState_ == 2) publicSaleStart = block.timestamp;
  }

  /// @notice Set merkleRoot1 to `merkleRoot_`.
  function setMerkleRoot1(bytes32 merkleRoot_) external onlyAuthorized {
    merkleRoot1 = merkleRoot_;
  }

  /// @notice Set merkleRoot1 to `merkleRoot_`.
  function setMerkleRoot2(bytes32 merkleRoot_) external onlyAuthorized {
    merkleRoot2 = merkleRoot_;
  }

  /// @notice Set proxyRegistry to `proxyRegistry_`.
  function setProxyRegistry(OpenSeaProxyRegistry proxyRegistry_)
    external
    onlyAuthorized
  {
    proxyRegistry = proxyRegistry_;
  ...
  /// @notice Withdraw ether balance to the owner().
  function withdraw() external onlyAuthorized {
    payable(owner()).transfer(address(this).balance);
  }

  /// @dev Modified for OpenSea pre-approving so users can make truly gasless sales.
    ...
    returns (bool)
  {
    return
      ...
      OpenSeaProxyRegistry(proxyRegistry).proxies(owner) == operator;
  }
