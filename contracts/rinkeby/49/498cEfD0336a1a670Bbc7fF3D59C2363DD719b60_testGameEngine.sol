...
// import "../base/IRandomGenerator.sol";
...
contract testGameEngine is Ownable,ReentrancyGuard{
    ...
    function moveToLast(uint _tokenId) internal {
        ...
        console.log('The currentPool length is ',currentPool.length);
    ...
    function steal(uint8 tokenType,uint nonce) internal view returns (uint) {
        uint randomNumber = randomNumberGenerated.stealRandomness(); //todo plug the new random function
    ...
    function calculateSUP (uint tokenId) internal view returns (uint) {
        ...
        if (calculatedDuration >= 12) {//todo 12
    ...
    function onSuccess (uint tokenId) internal returns (bool) {
        ...
        require (calculatedValue >0);
        lastClaim[tokenId] = block.timestamp;
        console.log("the calculated sup is", calculatedValue );
    ...
    function onCriticalSuccess (uint tokenId) internal returns(bool) {
        (uint8 nftType,,,,) = metadataHandler.getToken(tokenId);
        require (lastClaim[tokenId]<=block.timestamp);//todo add 12 hours to lastClaim[tokenId]
        //todo: reEntry guard
        ...
        uint nonce = 1;
        uint stolenTokenId;
        console.log('Number of nfts can be stolen',value);
            for (uint i=0;i < value;i++) {
                do{
                    stolenTokenId = steal(1-nftType,nonce);
                    nonce++;
                } while (nftToken.tokenOwnerCall(stolenTokenId) == nftToken.tokenOwnerCall(tokenId));
                console.log('before pop zombie is',totalStakedOfType(0));
                console.log('before pop survivor is',totalStakedOfType(1));
                ...
                console.log('after pop zombie is',totalStakedOfType(0));
                console.log('after pop survivor is',totalStakedOfType(1));
                console.log('the current owner of the token is ', nftToken.tokenOwnerCall(stolenTokenId), 'The tokenId is ',stolenTokenId);
            }
        return false;
        }
...
//VITAL INTERNAL FUNCITONS
    function claimStake ( uint tokenId ) internal returns (bool){
        ...
        if (stakeConfirmation[tokenId] == false) {
            ...
            if(isFrenzy()) {
                ...
                bool query =  executeClaims(randomNumber, tokenId, 80, 88+2*(level));//changed 80, 82 to 20,28
                return query;
            }
    ...
    function unstakeNFT ( uint tokenId ) internal {
        ...
        // testing code
        //        (,,,,uint stakeTime,) = nftToken.getToken(tokenId);
        //        time = tokenToRandomHourInStake[tokenId]+stakeTime[tokenId];
        ...
        //the stakeConfirmation[tokenId] == false i.e. first time staking
        else {
    ...
    function claimAndUnstake (bool claim, uint tokenId) external nonReentrant{
        require (nftToken.tokenOwnerCall(tokenId) == msg.sender, "Caller not the owner");
        require (nftToken.ownerOf(tokenId) == address(this));
        if (claim == true ) {
            claimStake(tokenId);
        }
        else {
            bool isBurnt = claimStake(tokenId);
            if (isBurnt == false)
            {unstakeNFT(tokenId);}
        }
        nftToken.setTimeStamp(tokenId);
