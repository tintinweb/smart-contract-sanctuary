...
interface AlinxNFT is IERC721 {
    ...
    function getAlix(uint256) external view returns (Alix memory);
}

contract AlinxMarket {
    ...
    AlinxNFT public alixNFT;
    IAlinxManager public router;
    ...
    constructor(
        ...
        alixNFT = AlinxNFT(_alixNFT);
        router = IAlinxManager(_manager);
    }

    modifier notEgg(uint256 _tokenId) {
        require(alixNFT.getAlix(_tokenId).level != 0, "not evolved");
        _;
    }
    ...
    function placeOrder(uint256 _tokenId, uint256 _price)
        ...
        notEgg(_tokenId)
    {
        require(alixNFT.ownerOf(_tokenId) == msg.sender, "Only owner can sell");
        require(_price > 0, "Cannot sell without fee");
        alixNFT.transferFrom(msg.sender, address(this), _tokenId);
    ...
    function cancelOrder(uint256 _tokenId) public {
        ...
        alixNFT.transferFrom(address(this), msg.sender, _tokenId);
    ...
    function fillOrder(uint256 _tokenId) public {
        ...
        alixToken.transferFrom(msg.sender, router.feeAddress(), feeMarket);
