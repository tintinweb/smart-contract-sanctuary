...
contract bonus is ERC721Enumerable {

/* Variables */
    address payable public beneficiary ;
    uint256 public currentId;
    string public defaultMetadata = "QmS69wM8KTN5qYVhJkZAeYRPgBXYPeup7McAKoG6y4zn5v";
    mapping (address => bool) public hasMinted;
    address[] private _tokenAddrs;
   
    string public baseURI = "ipfs://";
    mapping(uint256 => string) public _tokenURIs;
    string public _contractURI = "QmXqtMKHL5AKQE8VhumF9aH5MeyPuwvd7m9KS5d22GKdhm";

    constructor (address payable _beneficiary, address[] memory _tokens) ERC721 ("PACDAO BONUS", "PAC-B"){
	...
	for(uint _i = 0; _i < _tokenAddrs.length; _i++) {
		...
		if(IERC721( _addr ).balanceOf(msg.sender) > 0) {
			count++;	
		}		
	}
	return count;
...
/* Payable Functions */
	...
	{
		uint _mint_num = userCap(msg.sender);
		require(_mint_num > 0, "No NFT Owned");
		require(hasMinted[msg.sender] == false, "Already Minted");

		for(uint _i = 0; _i < _mint_num; _i++) {
			_mint(msg.sender);
		}
    ...
    function _mint(address _mint_address) private {
	currentId += 1;
	_safeMint(_mint_address, currentId);
    }
    function mintFor(address _mint_address) public payable
    {
...
// No ETH handled in this contract, but recover funds inadvertently wired to the contract
