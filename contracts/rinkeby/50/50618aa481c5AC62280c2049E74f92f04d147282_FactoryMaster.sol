...
contract FactoryMaster {
    ...
    function createChild(Param memory data) external {
        //check input
        require(data.types != Types.none, "you must enter the word 1");
        require(
        ...
        require(
            keccak256(abi.encodePacked((data.symbol))) !=
                keccak256(abi.encodePacked((""))),
            "phai nhap gia tri"
        );

        if (data.types == Types.erc20) {
            //check fee
            if(gasleft() >=1 wei){
                ERC20 child = new ERC20(data.name, data.symbol);
                childrenErc20.push(child);
                emit ChildCreatedERC20(address(child), data.name, data.symbol);
            }
        }
        if (data.types == Types.erc721) {
            //check fee

            ERC721 child = new ERC721(data.name, data.symbol);
            childrenErc721.push(child);
            emit ChildCreatedERC721(address(child), data.name, data.symbol);
        }
        if (data.types == Types.erc2917) {
            require(data._interestsRate >= 0, "gia tri phair lon hon ko");
            //check fee

            ERC2917 child = new ERC2917(data.name, data.symbol, data._interestsRate);
            childrenErc2917.push(child);
            emit ChildCreated2917(address(child), data.name, data.symbol, data._interestsRate);
        }
    }
}
