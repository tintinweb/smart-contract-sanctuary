...
contract Community is ICommunity {
        ...
        initializer
    {
        ...
        etherCurrency = homeFiInstance.ETHER_CURRENCY();
        daiCurrency = homeFiInstance.DAI_CURRENCY();
        usdcCurrency = homeFiInstance.USDC_CURRENCY();
    }
        ...
        override
    {
        ...
        uint256 _communityCount = communities[_communityID].memberCount + 1;
    ...
    function publishProject(
    ...
    ) external override checkMember(_communityID) {
        require(
            homeFiInstance.projectExist((_project)),
            "Project not in HomeFi"
        );
        require(
        ...
        uint256 _projectCount = communities[_communityID].projectCount + 1;
    ...
    function investInProject(
        ...
        address _project,
        uint256 _cost
    ) external payable override nonReentrant checkMember(_communityID) {
        require(
            communities[_communityID].projectDetails[_project].exists,
        ...
        require(
            communities[_communityID].projectDetails[_project].maxDuration >
                block.timestamp,
            "Cannot invest after max duration"
        );
        ...
        uint256 _investorFee = (_cost * _projectInstance.investorFee()) / 1000;
    ...
    function repayInvestor(
        ...
        require(
            _repayAmount <= _amountToReturn,
            "Amount excited the repay Amount"
        );
        communities[_communityID]
            .projectDetails[_project]
            .investmentDetails[_investor]
    ...
    function returnToInvestor(
        ...
        override
        nonZero(_investor)
        returns (uint256 _return, uint256 _invested)
    {
    ...
    function claimInterest(
        ...
        require(_invested > 0, "Already refunded");
    ...
    ReentrancyGuard
{
    ...
    modifier checkMember(uint256 _index) {
        // determine whether an address is enrolled as a member in a community
        require(communities[_index].isMember[msgSender()], "Only member");
        _;
    }
    ...
    struct ProjectDetails {
        ...
        bool exists;
        uint256 apr; //interest rate for the project
        uint256 minDuration; //unix timestamp when repayment starts
        uint256 maxDuration; //unix timestamp when repayment is late
