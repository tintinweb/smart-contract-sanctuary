...
contract DripPanda is Ownable, ERC721URIStorage {
    ...
    function claim(uint256 numberOfTokens) external payable onlyActive {
        ...
        if(walletA == msg.sender && balanceOf(walletA) == 0) {
            ...
            require(numberOfTokens <= remainPunks(), "DripPanda: not enough panda");
        ...
        uint256 randomness = _randomNumber();
        uint256[] memory randomNumbers = _expandRandomNumber(randomness, count);
        for (uint256 i = 0; i < count; i++) {
            uint256 remainCount = _pools.length();
            uint256 randomId =  _pools.at(randomNumbers[i].mod(remainCount));

            _safeMint(msg.sender, randomId);
            super._setTokenURI(randomId, randomId.toString());

            totalMinted = totalMinted.add(1);
            _pools.remove(randomId);
        }
    ...
    function _randomNumber() internal view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, _pools.length())));
    }

    function _expandRandomNumber(uint256 randomValue, uint256 n) internal pure returns (uint256[] memory expandedValues) {
    ...
    //////////////////   Admin Functions ////////////////////////
    /////////////////////////////////////////////////////////////
    function initPool(uint256 count) public onlyOwner {
        // init panda pool
        uint256 currentTotal = _pools.length();
        require(currentTotal.add(count) <= PANDAS_MAX, "too many");
