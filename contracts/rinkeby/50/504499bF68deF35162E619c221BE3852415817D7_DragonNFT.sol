...
contract DragonNFT is ERC721Enumerable, Ownable {
    ...
    uint256 public constant DRAGONS_PUBLIC = 5000;
    uint256 public constant DRAGONS_MAX = DRAGONS_PUBLIC;
    ...
    Counters.Counter private _publicDragons;

    constructor() ERC721("DragonTest", "DRGN") {
    }
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(_publicDragons.current() < DRAGONS_PUBLIC, "Purchase would exceed DRAGONS_PUBLIC");
        require(PRICE * numberOfTokens <= msg.value, "ETH amount is not sufficient");

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicDragons.current();

            if (_publicDragons.current() < DRAGONS_PUBLIC) {
                _publicDragons.increment();
