...
contract xsLocker is IxsLocker, ERC721Enhanced2, /*EIP712,*/ ReentrancyGuard, /*Initializable,*/ Governable {
    // Inherits EIP712 through ERC721Enhanced
    ...
    ***************************************/
    ...
    function delegateeOf(uint256 xsLockID) public view returns (address) {
    ...
    function delegatedRights(address voter) external view override returns (uint256 length) {
        require(
            voter != address(0),
    ...
    function delegatedRightByIndex(address voter, uint256 idx) external view override returns (uint256 xsLockID) {
        require(
            voter != address(0),
            "VotingEscrowLock: delegate query for the zero address"
        );
    ...
    ***************************************/
    ...
    function _beforeTokenTransfer(
        ...
        if(from == address(0x0)) _notify(xsLockID, from, to, Lock(0, 0), lock);
        else if(to == address(0x0)) _notify(xsLockID, from, to, lock, Lock(0, 0));
        else _notify(xsLockID, from, to, lock, lock);
    }
