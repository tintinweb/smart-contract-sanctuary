...
contract TimelockVault is Ownable {
    ...
    */
    function lockUnderVesting(address to_user, uint256 amount, uint256 vesting_seconds, uint256 cliff_seconds) external {
    ...
    * @param cliff_seconds Amount of seconds while tokens would be completely locked.
    */
    function safeLockUnderVesting(address to_user, uint256 amount, uint256 vesting_seconds, uint256 cliff_seconds) external {
        require(cliff_seconds < vesting_seconds, "cliff_seconds < vesting_seconds");
    ...
    /**
    * @notice Add more tokens to the existing (possibly zero) vesting account. Doesn't change cliff or vesting period!
    ...
    */
    ...
    function availableToClaim(address user) public view returns (uint256) {
        ...
        uint64 t = uint32(block.timestamp);

        // if before the cliff period is over - there's nothing vested yet
        if (a.cliff_timestamp < t) return 0;

        // if after the cliff
        uint256 vested;
        if (a.vesting_ends_timestamp < t) {
            // if everything is vested
            vested = a.amount_under_vesting_div1e12 * 1e12;
        }  else {
            // otherwise the amount is proportional to the amount after the cliff before end of vesting
            vested = uint256(a.amount_under_vesting_div1e12) * 1e12 * (t - a.cliff_timestamp) / (a.vesting_ends_timestamp - a.cliff_timestamp);
        }
        // the answer is how much is vested in total minute how much already withdrawn
        return vested - uint256(a.amount_already_withdrawn_div1e12) * 1e12;
    }
    ...
    function rescue(IERC20 token) external onlyOwner {
        ...
        if (token == IERC20(address(0))) {
            ...
            uint256 withdraw_amount = token.balanceOf(address(this));
            if (withdraw_amount > 0) {
                token.safeTransfer(address(msg.sender), withdraw_amount);
            }
        }
    }
