...
contract DistributionSign is AccessControl, ReentrancyGuarded, EIP712 {
    ...
    function validateOrderAuthorization(
        ...
        else if (ecrecover(keccak256(abi.encodePacked(personalSignPrefix, "32", calculatedHashToSign)),v,r,s) == maker) {
            return true;
        }
        ...
        returns (bool)
    {
        ...
        if (order.expirationTime != 0 && order.expirationTime <= block.timestamp)
         {
            return false;
        }
    ...
    function claim(Order memory claimer, Order memory owner, bytes memory signatures) public reentrancyGuard {
        /* CHECKS */
        ...
        require(firstHash != secondHash, "claim: Self-matching orders is prohibited");
        {
        ...
        IERC20(owner.token).transferFrom(owner.maker, owner.target, owner.amount);
    }
