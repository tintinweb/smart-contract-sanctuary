...
contract SpaceAuction is Ownable {
    ...
    uint256 public platformFee = 600;                // 6% platform fee
    ...
    // permanently set address of CRV
      constructor(address _crv, address _stash, address _platform) {
        CRV = IERC20(_crv);
        stash = _stash;
        platform = _platform;
    ...
    function finalize(bytes32 _proposal, uint256 _votes) public onlyTeam {
      ...
      paidTotal -= pfee;
      CRV.safeTransfer(platform, pfee);
      CRV.safeTransfer(stash, paidTotal);
    }
    ...
    function rollBalances(uint256[] memory _proposals, bytes32 _proposal, uint256 _maxPerVote) public {
      require(proposal[_proposal].deadline > block.timestamp, "Invalid auction or expired");
      require(bidder[_proposal][msg.sender].balance == 0, "Already bid");
      require(bidder[_proposal][msg.sender].msgHash != keccak256(""), "No hash registered");
      require(_maxPerVote > 0, "Cannot bid 0");
      uint256 total;
    ...
    function withdraw(uint256[] memory _proposals) public {
      ...
      if(total > 0) {
        CRV.safeTransfer(msg.sender, total);
      }
    }
