...
contract VectorField is ERC721URIStorage, VRFConsumerBase, Ownable {
    ...
    event CreatedVectorField(uint256 indexed tokenId, string tokenURI);
    event CreatedUnfinishedVectorField(uint256 indexed tokenId, uint256 randomNumber);
    event requestedVectorField(bytes32 indexed requestId, uint256 indexed tokenId); 
...
"6424e5"];
 ...
 function withdraw() public payable onlyOwner {
    ...
    function Claim() public payable returns (bytes32 requestId) {
        ...
        emit requestedVectorField(requestId, tokenId);
    }
    ...
    function Mint(uint256 tokenId) public {
        ...
        emit CreatedVectorField(tokenId, svg);
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override {
        ...
        emit CreatedUnfinishedVectorField(tokenId, randomNumber);
    }

    function generateSVG(uint256 _randomness) public view returns (string memory finalSvg) {

        uint256 bgopacity = uint256(keccak256(abi.encode(_randomness + 1))) % 1500;
        finalSvg = string(abi.encodePacked("<svg xmlns='http://www.w3.org/2000/svg' height='", uint2str(height), "' width='", uint2str(width), "' fill='none' viewBox='500 100 1500 2000'><path d='M500 100h1500v2000H0z' fill= '#ffffff'/><path d='M500 100h1500v2000H0z'"));
    ...
    function generatePath(uint256 _randomness) public view returns(string memory pathSvg) {
        ...
        uint256 w = uint256(keccak256(abi.encode(_randomness, height + 2 + 3))) % 2000;
        uint256 h = uint256(keccak256(abi.encode(_randomness, height * 2 + 1))) % width;
        uint256 opacity = uint256(keccak256(abi.encode(_randomness + 2))) % 900;
        ...
        return string(
                abi.encodePacked(
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name":"Field #',TokenID,'", "description":"Vector fields randomly generated and stored immutably on the Ethereum blockchain", "attributes":"", "image":"',imageURI,'"}'
                            )
                        )
                    )
                )
