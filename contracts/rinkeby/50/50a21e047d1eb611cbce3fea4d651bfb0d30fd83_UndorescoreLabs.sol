...
contract UndorescoreLabs is ERC721Enumerable, Ownable {
    using Strings for uint256;
    using SafeMath for uint256;
    using ECDSA for bytes32;

    uint public constant PUBLIC_SUPPLY = 8588;
    uint public constant PRESALE_SUPPLY = 300;
    uint public constant MAX_SUPPLY = PRESALE_SUPPLY + PUBLIC_SUPPLY;
    uint public constant PRICE = 0.05 ether;
    uint public constant MAX_MINT_AMOUNT = 4;
    ...
    uint256 public publicPurchaseLimit = 2;
    string private contractURI;
    string private baseTokenURI = "";
    string public provHash;

    // Our wallet addresses
    address public constant creatorAddress = 0xb74642259EBd54E38f8b6Bdcc98FA23D9c631aaB;
    address private signerAddress = 0xb74642259EBd54E38f8b6Bdcc98FA23D9c631aaB; // tbd
    ...
    constructor(string memory unrevealedBaseURI) ERC721("Underscore Labs", "ULABS") {
    ...
    function mintPublic(uint256 mintQuantity) external payable {
        ...
        require(publicAmountMinted + mintQuantity <= PUBLIC_SUPPLY, "Out of public stock");
        require(publicListPurchases[msg.sender] + mintQuantity <= publicPurchaseLimit, "Public mint limit exceeded");
        require(mintQuantity <= MAX_MINT_AMOUNT, "Exceeded mint limit");
        require(PRICE * mintQuantity <= msg.value, "Insufficient funds"); 
    ...
    function mintPrivate(uint256 mintQuantity) external onlyOwner {
        require(saleLive, "Sale closed");
        require(totalSupply() < MAX_SUPPLY, "Out of stock");
        require(privateAmountMinted + mintQuantity <= PRESALE_SUPPLY, "Out of private stock");
    ...
    function getPublicUnsold() external view returns (uint256) {
