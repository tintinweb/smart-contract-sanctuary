...
contract xSOLACE is /*IxSOLACE,*/ /*ERC20Permit,*/ ReentrancyGuard, Governable {
     ...
     */
    constructor(address governance_, address solace_, uint256 startTime_, uint256 endTime_) Governable(governance_) {
    ...
    ***************************************/
    ...
    function balanceOf(address user) public view returns (uint256 amount) {
        ...
        uint256 pendingRewards = userInfo_.value * accRewardPerShare_ / 1e12 - userInfo_.rewardDebt + userInfo_.unpaidRewards;
        uint256 deposited = userInfo_.value;
        uint256 balance = deposited + pendingRewards;
        // fetch time vars
     ...
     */
    function multiplier(address user) public view returns (uint256 multiplier_) {
        // get farmer information
        UserInfo storage userInfo_ = userInfo[user];
        // fetch time vars
        uint256 timestamp = block.timestamp;
     ...
     */
    function getRewardAmountDistributed(uint256 from, uint256 to) public view returns (uint256 amount) {
        // validate window
    ...
    ***************************************/
    ...
    // TODO. this might take two signatures, one for the deposit and one for the lock
    ...
    ***************************************/
    ...
    function _deposit(address depositor, uint256 amount) internal {
        // get farmer information
        UserInfo storage userInfo_ = userInfo[depositor];
        // accounting
        valueStaked += amount;
    ...
    /**
     * @notice Update farm and transfer a user's rewards.
     * @param user User to process rewards for.
     */
    function _harvest(address user) internal {
        ...
        uint256 unpaidSolace1 = userInfo_.unpaidRewards;
        uint256 unpaidSolace2 = userInfo_.value * accRewardPerShare / 1e12;
        uint256 unpaidSolace3 = unpaidSolace2 + unpaidSolace1 - userInfo_.rewardDebt;
        uint256 transferAmount = Math.min(IERC20(solace).balanceOf(address(this)), unpaidSolace3);
        uint256 unpaidSolace4 = unpaidSolace3 - transferAmount;
        if(unpaidSolace4 != unpaidSolace1) userInfo_.unpaidRewards = unpaidSolace4;
        userInfo_.rewardDebt = unpaidSolace2;
        SafeERC20.safeTransfer(IERC20(solace), user, transferAmount);
        emit Harvested(user);
        // TODO: pull SOLACE from multisig?
    }
