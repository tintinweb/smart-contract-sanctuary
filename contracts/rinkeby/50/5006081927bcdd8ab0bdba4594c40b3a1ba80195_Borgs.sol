...
contract CommonObjects{
    ...
    struct BorgAttribute{
        ...
        bytes16[] colors;
        int8 colorSize;
        mapping (bytes32 => uint32[]) colorPositions;
    }
...
contract Borgs is ERC721Enumerable, ERC721Holder, CommonObjects, NumberUtils{
    ...
    function getBorg(uint256 borgId) public view returns(string memory name, bytes16[] memory image, bytes32[] memory attributes, uint256 parentId1, uint256 parentId2, uint256 childId){
    ...
    function getBorgAttribute(bytes32 borgAttributeName) public view onlyOwner returns (bytes32[] memory imagePixals){
        ...
        for(uint256 i = 0;i<borgAttribute.colors.length;i++){
            bytes32 color = borgAttribute.colors[i];
    ...
    function getListingByTokenId(uint256 tokenId) public view returns(uint256 borgId, uint256 price, address seller, address buyer, uint256 timestamp, bytes16[] memory image){
        // Get listing
    ...
    function getListingByIndex(uint256 index) public view returns(uint256 tokenId, uint256 price, address seller, address buyer, uint256 timestamp, bytes16[] memory image){
        // Get tokenId
    ...
    function getOwnersListingByIndex(address owner, uint256 index) public view returns(uint256 tokenId, uint256 price, address seller, address buyer, uint256 timestamp, bytes16[] memory image){
        // Get tokenId
    ...
    function createBorgAttribute(bytes32 borgAttributeName, bytes16[] calldata colors, uint32[][] memory positions) external onlyOwner editable{
    ...
    function previewBreedBorgs(uint256 borgId1, uint256 borgId2) external view usable returns(bytes16[] memory image, bytes32[] memory attributes){
        // Get the first borg to breed
        Borg storage borg1 = _borgs[borgId1];
        require(borg1.exists, 'Borg 1 doesnt exist');
        require(borg1.descendantId < 1, 'Borg1 already has a descendant');
    ...
    function _combineBorgAttributes(bytes32[] memory borgAttributeNames) internal view returns (bytes16[] memory hexPixals){
        // Init the 
        hexPixals = new bytes16[](IMAGE_SIZE);
        ...
        for(uint256 i=0;i<borgAttributeNames.length;i++){
             ...
             for(uint256 j = 0;j<borgAttribute.colors.length;j++){
                bytes16 color = borgAttribute.colors[j];
                uint32[] memory positions = borgAttribute.colorPositions[color];
