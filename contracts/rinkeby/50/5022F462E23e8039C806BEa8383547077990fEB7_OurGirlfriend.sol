...
contract OurGirlfriend is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    event Wooed(address _wooer, uint256 _newScore);
    ...
    function relationshipLevel(uint256 tokenId) internal view returns (uint256) {
        uint256 relationshipScore = relationshipScores[tokenId];
        if (relationshipScore < 1) {
            return 1;
        } else if (relationshipScore < 2) {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        return string(abi.encodePacked('data:application/json;base64,', Base64.encode(bytes(string(abi.encodePacked('{"name": "Asami (v0.1) #', tokenId, '", "description": "Asami is looking for her soulmate.", "attributes": [ { "trait_type": "Alignment", "value": "', tokenId % 2 == 0 ? 'Light' : 'Dark', '" } ], "image": "https://d3l938e4wlber9.cloudfront.net/ASAMI-000', tokenId, '-' , relationshipLevel(tokenId) , '.svg"}'))))));
    }
}
