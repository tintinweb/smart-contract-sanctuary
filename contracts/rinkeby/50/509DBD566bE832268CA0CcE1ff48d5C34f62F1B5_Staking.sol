...
contract Staking is IStaking, Ownable, ReentrancyGuard, Pausable {
	...
	function stake(uint256 _amount) external override whenNotPaused nonReentrant updateReward(_msgSender()) {
	...
	function withdraw(uint256 _amount) public override nonReentrant updateReward(_msgSender()) {
		...
		require(staked[_msgSender()] >= _amount, "Insufficient staked amount");
	...
	function rewardPerToken() public view override returns (uint256) {
		...
		uint256 accumulatedReward = blocksWithRewardsPassed().mul(rewardPerBlock).mul(PRECISION).div(
			totalPoolStaked
		);
	...
	function earned(address _account) public view override returns (uint256) {
		...
		uint256 newlyAccumulated = staked[_account].mul(rewardsDifference).div(PRECISION);
