...
contract NFT is Ownable, ERC721 {
    ...
    uint256 public constant MAX = 7777; // total amount

    /// @notice price of each NFT for each stage
    uint256 public constant PER_PRICE = 0.05 ether; // presale first stage
    ...
    function mint(uint256 tokenQuantity) external payable {
        require(
            lastTokenId.add(tokenQuantity) <= MAX,
            "Sorry, there's not that many BBG left."
        );

        require(PER_PRICE * tokenQuantity <= msg.value, "Not Enough payments");
        ...
        for (uint256 i = 0; i < tokenQuantity; i++) {
            ...
            emit PublicSale(tokenId, msg.sender, PER_PRICE);
        }
    }
