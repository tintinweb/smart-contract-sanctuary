...
contract MainnetMultiBridgePool is AccessControl, ReentrancyGuard {
  ...
  address private WETH         = Uniswap(UniV2Router).WETH();
  ...
  function getUniV3TokenInEth(address _token, uint256 _amount) 
  ...
  function getUniV2TokenInEth(address _token, uint256 _amount) 
    public view returns (uint256) {
  ...
  function getFee(address _token, uint256 _amount) 
    public view returns (uint256) {
      ...
      uint256 _tokenInEth = getUniV3TokenInEth(_token, _amount);
      if (_tokenInEth == 0) {
        _tokenInEth = getUniV2TokenInEth(_token, _amount);
        if (_tokenInEth == 0) {
