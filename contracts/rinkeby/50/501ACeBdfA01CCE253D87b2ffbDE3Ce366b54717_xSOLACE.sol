...
contract xSOLACE is /*IxSOLACE,*/ ERC20Permit, ReentrancyGuard, Governable {
    ...
    uint256 public totalSolaceDeposited;
    uint256 public checkpointSolaceLocked;
    uint256 public checkpointLockedVotePower;
    uint40 public checkpointTimestamp;
    uint256 public lockAcc; // an accumulator of deposit amount * unlock time

    mapping(uint40 => uint256) public epochEndUnlockAmount;
    mapping(address => uint40) public userUnlockTime;
    mapping(address => uint256) public userDeposit;

    uint40 public constant TIME_GRANULARITY = 1 weeks;
    uint40 public constant MIN_LOCK_DURATION = 1 weeks;
    uint40 public constant MAX_LOCK_DURATION = 4 * 365 days;
    uint256 public constant MAX_LOCK_BONUS_BPS = 10000;
    ...
    constructor(address governance_, address solace_) ERC20("xsolace", "xSOLACE") ERC20Permit("xsolace") Governable(governance_) {
        ...
        solace = solace_;
        checkpointTimestamp = uint40(block.timestamp);
    }
    ...
    ***************************************/
     ...
     */
    function totalSupply() public view virtual override returns (uint256 amount) {
        // fetch vars
        uint256 solaceLocked_ = checkpointSolaceLocked;
        uint256 lockedVotePower_ = checkpointLockedVotePower;
        //uint256 lockAcc_ = lockAcc;
        uint40 nextEpochEnd = ((checkpointTimestamp / TIME_GRANULARITY)  * TIME_GRANULARITY) + TIME_GRANULARITY;
        uint40 timestamp = uint40(block.timestamp);
        // loop over checkpoints
        if(nextEpochEnd < timestamp) {
            uint256 c1 = 1 weeks * MAX_LOCK_BONUS_BPS;
            uint256 c2 = MAX_LOCK_DURATION * MAX_BPS;
            for(; nextEpochEnd < timestamp; nextEpochEnd += TIME_GRANULARITY) {
                uint256 unlockAmount = epochEndUnlockAmount[nextEpochEnd];
                lockedVotePower_ -= (lockedVotePower_ * c1 / c2);
                lockedVotePower_ -= unlockAmount;
                solaceLocked_ -= unlockAmount;
                //lockAcc -= unlockAmount * uint256(nextEpochEnd);
            }
        }
        uint256 solaceUnlocked = totalSolaceDeposited - solaceLocked_;
        //amount += (amount * timeLeft * MAX_LOCK_BONUS_BPS) / (MAX_LOCK_DURATION * MAX_BPS);
        //return 0; // ?
        return solaceUnlocked + lockedVotePower_;
    }

    /**
     * @notice Returns the amount of **SOLACE** that is deposited and locked.
     */
    function totalSolaceLocked() public view returns (uint256 amount) {
        amount = checkpointSolaceLocked;
        // fetch vars
        uint40 checkpoint = (checkpointTimestamp / TIME_GRANULARITY) * TIME_GRANULARITY;
        uint40 timestamp = uint40(block.timestamp);
        // loop over checkpoints
        for(uint40 nextEpochEnd = checkpoint + TIME_GRANULARITY; nextEpochEnd <= timestamp; nextEpochEnd += TIME_GRANULARITY) {
            uint256 unlockAmount = epochEndUnlockAmount[nextEpochEnd];
            amount -= unlockAmount;
        }
        return amount;
    ...
    /**
     * @notice Returns the amount of **SOLACE** that is deposited and unlocked.
     */
    function totalSolaceUnlocked() external view returns (uint256 amount) {
        return totalSolaceDeposited - totalSolaceLocked();
    }

    /**
     * @notice Returns the amount of tokens owned by `account`.
     * @param account Account balance to query.
     * @return amount The amount of **xSOLACE** owned by `account`.
     */
    function balanceOf(address account) public view virtual override returns (uint256 amount) {
        // fetch vars
        uint40 timestamp = uint40(block.timestamp);
        uint40 unlockTime = userUnlockTime[account];
        amount = userDeposit[account];
        // 1:1 if unlocked
        if(unlockTime <= timestamp) return amount;
        // multiplier increases with amount of weeks left in lock
        uint40 timeLeft = unlockTime - timestamp;
        //timeLeft = (timeLeft / TIME_GRANULARITY) * TIME_GRANULARITY;
        amount += (amount * timeLeft * MAX_LOCK_BONUS_BPS) / (MAX_LOCK_DURATION * MAX_BPS);
        return amount;
    }
    ...
    function isLocked(address account) external view returns (bool locked) {
        uint40 timestamp = uint40(block.timestamp);
        uint40 unlockTime = userUnlockTime[account];
        return (unlockTime > timestamp);
    }

    function lockTimeLeft(address account) external view returns (uint40 timeLeft) {
        uint40 timestamp = uint40(block.timestamp);
        uint40 unlockTime = userUnlockTime[account];
        return (unlockTime > timestamp)
    ...
    function pendingRewards(address account) external view returns (uint256 amount) { return 0; }
    ...
    ***************************************/
    ...
    function deposit(uint256 amountSolace) external {
        ...
        // pull solace
        SafeERC20.safeTransferFrom(IERC20(solace), msg.sender, address(this), amountSolace);
        // accounting
        _deposit(msg.sender, amountSolace);
    }
    ...
    function depositFor(address receiver, uint256 amountSolace) external {
        ...
        SafeERC20.safeTransferFrom(IERC20(solace), msg.sender, address(this), amountSolace);
        // accounting
        _deposit(receiver, amountSolace);
    }

    function depositAndLock(uint256 amountSolace, uint40 duration) external {
        // pull solace
        SafeERC20.safeTransferFrom(IERC20(solace), msg.sender, address(this), amountSolace);
        // accounting
        _deposit(msg.sender, amountSolace);
    ...
    function depositSigned(address depositor, uint256 amountSolace, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external {
        // permit
        IERC20Permit(solace).permit(depositor, address(this), amountSolace, deadline, v, r, s);
        // pull solace
        SafeERC20.safeTransferFrom(IERC20(solace), depositor, address(this), amountSolace);
        // accounting
        _deposit(depositor, amountSolace);
    }
     ...
     * Reverts if locked.
     * @param amountSolace The amount of **SOLACE** to withdraw.
     */
    function withdraw(uint256 amountSolace) external {
        _withdraw(amountSolace);
    }

    function lock(uint40 duration) external {
        _lock(msg.sender, duration);
    }
    ...
    ***************************************/
    ...
    function _deposit(address depositor, uint256 amountSolace) internal {
        // global update
        _update();
        // fetch vars
        uint40 timestamp = uint40(block.timestamp);
        uint40 unlockTime = userUnlockTime[depositor];
        // update deposit amount
        totalSolaceDeposited += amountSolace;
        userDeposit[depositor] += amountSolace;
        // update unlock amount
        if(unlockTime > timestamp) {
            checkpointSolaceLocked += amountSolace;
            epochEndUnlockAmount[unlockTime] += amountSolace;
            lockAcc += amountSolace * unlockTime;
        }
    }
     ...
     * @param amountSolace The amount of **SOLACE** to withdraw.
     */
    function _withdraw(uint256 amountSolace) internal {
        // global update
        _update();
        // fetch vars
        uint40 timestamp = uint40(block.timestamp);
        uint40 unlockTime = userUnlockTime[msg.sender];
        // verify unlock
        require(unlockTime <= timestamp, "xSOLACE: locked");
        // update deposit amount
        totalSolaceDeposited -= amountSolace;
        userDeposit[msg.sender] -= amountSolace; // safemath
        // update unlock amount
        if(unlockTime > timestamp) {
            checkpointSolaceLocked += amountSolace;
            epochEndUnlockAmount[unlockTime] += amountSolace;
    ...
    function _lock(address account, uint40 duration) internal {
        ...
        uint40 timestamp = uint40(block.timestamp);
        uint40 newUnlockTime = ((timestamp+duration) / TIME_GRANULARITY) * TIME_GRANULARITY;
        // existing lock
        uint40 unlockTime = userUnlockTime[account]; // existing lock
        if(unlockTime != 0) {
            require(newUnlockTime >= unlockTime, "xSOLACE: existing lock exceeds new lock");
        }
        // new lock
    ...
    function _update() internal {
        // TODO
        uint256 amount = totalSolaceDeposited - checkpointSolaceLocked;
        // fetch vars
        uint40 checkpoint = (checkpointTimestamp / TIME_GRANULARITY) * TIME_GRANULARITY;
        uint40 timestamp = uint40(block.timestamp);
        // loop over checkpoints
    ...
    function _beforeTokenTransfer(
        ...
        uint40 timestamp = uint40(block.timestamp);
        uint40 unlockTime = userUnlockTime[from];
        require(unlockTime <= timestamp, "xSOLACE: locked");
        // TODO: transfer underlying SOLACE
    }
}
