...
import "./ADAMCustodyWalletFactory.sol";

contract ADAMCustodyWallet {
    ...
    constructor() public {
        ...
        bool erc721 = ADAMCustodyWalletFactory(factory).erc721Flag(); 
        address token = ADAMCustodyWalletFactory(factory).tokenAddress();
        address from = ADAMCustodyWalletFactory(factory).fromAddress();
        address to = ADAMCustodyWalletFactory(factory).toAddress();
        uint256 tokenId = ADAMCustodyWalletFactory(factory).tokenId();
        ...
        if(erc721){
            ...
            uint256 amount = ADAMCustodyWalletFactory(factory).amountERC1155();
...
import "@openzeppelin/contracts/token/ERC721/IERC721.sol";
import "@openzeppelin/contracts/token/ERC1155/IERC1155.sol";
import "./ADAMCustodyWallet.sol";
import "./Pauser.sol";

contract ADAMCustodyWalletFactory is Pauser {
    ...
    function predict(bytes32 _salt) public view returns(address){
        return address(uint(keccak256(abi.encodePacked(
            ...
            address(this),
            _salt,
            keccak256(abi.encodePacked(type(ADAMCustodyWallet).creationCode))
        ))));
    }
    ...
    function transferERC721(address _tokenAddress, address _from, address _to, uint256 _tokenId, bytes32 _salt) external whenNotPaused onlyOperator {
        ...
        bytes memory bytecode = type(ADAMCustodyWallet).creationCode;
        assembly {
    ...
    function transferERC1155(address _tokenAddress, address _from, address _to, uint256 _tokenId, uint256 _amount, bytes32 _salt) external whenNotPaused onlyOperator {
        ...
        require(IERC1155(predictAddress).balanceOf(_from, _tokenId) >= _amount, 'transferERC1155: wrong balance');
        ...
        ADAMCustodyWallet custodyWallet = new ADAMCustodyWallet{salt: _salt}();
        require(
