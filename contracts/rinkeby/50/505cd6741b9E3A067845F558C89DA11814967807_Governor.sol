...
contract Governor is 
  ...
  UUPSUpgradeable {
    ...
    ERC721Upgradeable public vetoToken;

    function initialize(ERC20VotesUpgradeable _token, ERC721Upgradeable _vetoToken)
        public initializer
    {
        __Governor_init("Governor");
        __GovernorSettings_init(1, 20, 0);
        __GovernorCountingSimple_init();
        __GovernorVotes_init(_token);
        __GovernorVotesQuorumFraction_init(/* quorum percentage */ 4);
        __Ownable_init();
        __UUPSUpgradeable_init();

        vetoToken = _vetoToken;
    }

    function veto(uint256 proposalId, uint256 tokenId) public {
        vetoToken.safeTransferFrom(msg.sender, address(this), tokenId);
        _countVote(proposalId, msg.sender, 0, token.totalSupply());
    }
