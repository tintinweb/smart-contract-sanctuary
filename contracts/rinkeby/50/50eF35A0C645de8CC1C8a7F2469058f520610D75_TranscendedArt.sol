...
//   ______                                          __         _____         __
//  /_  __/________ _____  _____________  ____  ____/ /__  ____/ /   |  _____/ /_
//   / / / ___/ __ `/ __ \/ ___/ ___/ _ \/ __ \/ __  / _ \/ __  / /| | / ___/ __/
//  / / / /  / /_/ / / / (__  ) /__/  __/ / / / /_/ /  __/ /_/ / ___ |/ /  / /_
// /_/ /_/   \__,_/_/ /_/____/\___/\___/_/ /_/\__,_/\___/\__,_/_/  |_/_/   \__/
...
contract TranscendedArt is ERC721, ReentrancyGuard, Ownable {
    event Mint(uint256 indexed _tokenId);

    struct Project {
        uint256 costToMintWei;
        uint256 startTimestamp;
        uint256 maxMintBatch;
        uint256 invocations;
    ...
    constructor() ERC721("TranscendedArt", "TRNSDART") {
        admin = msg.sender;
        treasuryAddress = msg.sender;
    }

    function mint(uint256 _projectId, uint256 numToMint)
        external
        payable
        nonReentrant
    {
        ...
        require(
            numToMint >= projects[_projectId].maxMintBatch,
            "numToMint too high"
        );
        require(numToMint > 0, "Must mint at least 1");
        require(
            projects[_projectId].invocations + numToMint <=
                projects[_projectId].maxInvocations,
            "exceeded max invocations"
        );
        uint256 costToMint = projects[_projectId].costToMintWei * numToMint;
        require(costToMint <= msg.value, "insufficient ETH");
        batchMint(_projectId, numToMint);
    }

    function adminMint(uint256 _projectId, uint256 numToMint)
        external
        onlyAdmin
        nonReentrant
    {
    ...
    function batchMint(uint256 _projectId, uint256 numToMint) internal {
        uint256 tokenIdToBe = (_projectId * ONE_MILLION) +
            projects[_projectId].invocations;
        for (uint256 i = 0; i < numToMint; i++) {
            _safeMint(msg.sender, tokenIdToBe);
            tokenIdToProjectId[tokenIdToBe] = _projectId;
            emit Mint(tokenIdToBe);
            tokenIdToBe++;
        }
        projects[_projectId].invocations =
            projects[_projectId].invocations +
        ...
        onlyAdmin
    {
        ...
        projects[projectId].costToMintWei = _pricePerTokenInWei;
        projects[projectId].dynamic = _dynamic;
        projects[projectId].maxInvocations = ONE_MILLION;
        projects[projectId].maxMintBatch = 1000;
        if (!_dynamic) {
    ...
    function updateProjectPricePerTokenInWei(
        ...
        projects[_projectId].costToMintWei = _pricePerTokenInWei;
    }

    function updateProjectStartTimestamp(
        uint256 _projectId,
    ...
    function updateProjectMaxMintBatch(
        uint256 _projectId,
        uint256 _maxMintBatch
    ) external onlyAdmin {
        projects[_projectId].maxMintBatch = _maxMintBatch;
    }
    ...
    function updateProjectMaxInvocations(
        ...
        require(_maxInvocations <= ONE_MILLION, "max 1,000,000");
    ...
    function removeProjectLastScript(uint256 _projectId) external onlyAdmin {
        require(
            projects[_projectId].scriptCount > 0,
            "there are no scripts to remove"
        );
        delete projects[_projectId].scripts[projects[_projectId].scriptCount--];
        projects[_projectId].scriptCount = projects[_projectId].scriptCount--;
    }
    ...
    function updateProjectScriptLanguage(
        uint256 _projectId,
        string memory _projectScriptLanguage
    ) external onlyAdmin {
        projects[_projectId].scriptLanguage = _projectScriptLanguage;
    }

    function updateProjectScriptDependency(
        uint256 _projectId,
        string memory _projectScriptDependency
    ) external onlyAdmin {
        projects[_projectId].scriptLanguage = _projectScriptDependency;
    }
    ...
    function toggleProjectIsDynamic(uint256 _projectId) external onlyAdmin {
        require(
    ...
    function clearTokenIpfsImageUri(uint256 _tokenId) external onlyAdmin {
