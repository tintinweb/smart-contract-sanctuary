...
contract Master is Ownable {
    ...
    function initializeWork(address _address) external returns (bool) {
        if (!IJobsRegistry(jobsRegistry).exists(_msgSender()))
            revert Forbidden();
    ...
    function initializeWorkWithRequirements(
        ...
        if (!IJobsRegistry(jobsRegistry).exists(_msgSender()))
            revert Forbidden();
        gasCheckpoint = gasleft();
        uint256 _bonded = IBonder(bonder).bonded(_address);
        if (_bonded == 0 || (_minimumBonded > 0 && _bonded < _minimumBonded))
            return false;
        if (
            _minimumEarned > 0 &&
            IBonder(bonder).earned(_address) < _minimumEarned
        ) return false;
        if (
            _minimumAge > 0 &&
            block.timestamp - IBonder(bonder).activationTimestamp(_address) <
            _minimumAge
        ) return false;
        return true;
    }
