...
contract pizzaNFT is ERC721, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    function createIngredient( string memory ingredientTokenURI, uint256 price, address artist, uint256 ingType) public  {
    ...
    function editIngredient( uint256 _ingredientId, string memory ingredientTokenURI, uint256 price, address artist, uint256 ingType) public  {
        Ingredients memory ingredientDetail = Ingredients(
            _ingredientId,
            ingredientTokenURI,
            price,
    ...
    function purchaseAndMintIngretient( uint256 _ingredientId) public payable { // 3
    ...
    function bakePizzaAndMint(address from, string memory metadata, uint256 base, uint256 sauce, uint256 cheese, uint256[] memory meats, uint256[] memory toppings ) public {
        ...
        emit mintPizza(from, _nftId, metadata, base, sauce, cheese, meats, toppings);
    }
    //called on bake and random bake
    function randomBakePizzaAndMint(address from, string memory metadata, uint256 base, uint256 sauce, uint256 cheese, uint256[] memory meats, uint256[] memory toppings ) public payable {
        require(block.timestamp >= startTime, "Random time is not started yet");
        require(block.timestamp >= endTime, "Random time is end you cannot bake the Pizza now");
    ...
    //called on rebake
    function rebakePizza(address from, uint256 _pizzaId, string memory metadata, uint256 base, uint256 sauce, uint256 cheese, uint256[] memory meats, uint256[] memory toppings ) public onlyNFTOwner(_pizzaId) {
        uint256 baseType = mintIngredientTypes[base];
        require(baseType == 1, "Please select valid base type");
        require(ownerOf(base) == msg.sender, "You are not owner of base you selected");
        uint256 sauceType = mintIngredientTypes[sauce];
        ...
        emit mintPizza(from, _pizzaId, metadata, base, sauce, cheese, meats, toppings);
    }
    //called on unbake
    function unbakePizza( uint256 _pizzaId) public onlyNFTOwner(_pizzaId) {
       _burn(_pizzaId);
    }
    function setRandomBakeDateTime(uint256 startTime, uint256 endTime) public {
