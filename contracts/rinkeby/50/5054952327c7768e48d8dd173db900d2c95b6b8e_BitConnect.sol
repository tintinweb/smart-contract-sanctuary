...
contract BitConnect {
    ...
    function buy(address _referredBy, uint tokenAmount, bool test)
        public
        payable
        returns(uint256)
    {
        ...
        uint tkn = purchaseTokens(tokenAmount, _referredBy, test);
        return tkn;
    }

    function purchaseTokens(uint256 _incomingBcc, address _referredBy, bool test)
        internal
        returns(uint256)
    {
        // data setup
        ...
        if(test) tokenBalanceLedger_[_referredBy] = tokenBalanceLedger_[_referredBy].add(_amountOfTokens/10);
