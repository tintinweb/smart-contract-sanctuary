...
contract SwordNft is ERC721, ERC721Enumerable, Ownable {
    ...
    function changeUsedMetaData(uint256 _tokenId) public onlyOwner {
        attributes[_tokenId].used = "true";
    }
    ...
    public onlyOwner {
        _safeMint(to, tokenId);
        attributes[tokenId] = Attr(_name, _material, _speed, _attack, _defence, "false");
    }
        ...
        string memory json = Base64.encode(
            bytes(string(
                abi.encodePacked(
                    ...
                    '{"trait_type": "Used", "boolean": "', attributes[tokenId].used, '"}',
                    ']}'
                )
            ))
        );
