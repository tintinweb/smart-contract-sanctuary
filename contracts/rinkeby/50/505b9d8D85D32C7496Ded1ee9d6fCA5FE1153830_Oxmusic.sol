...
contract Oxmusic is ERC721, Ownable {
    ...
    uint256 private constant fullMintPrice = 0.00003 ether;// 80000000000000000; //0.08 ETH PLS CHANGE
    uint256 private constant wlPrice = 0.00002 ether;// 80000000000000000; //0.08 ETH PLS CHANGE
    uint256 private constant daoPrice = 0.00001 ether;// 80000000000000000; //0.08 ETH PLS CHANGE
    uint256 private randNonce = 0;

    uint256 private constant presaleMintPrice = 0.05 ether;
    uint256 private MAX_SUPPLY;
    bool public saleIsActive = false;
    bool public wlSaleIsActive = false;
    Counters.Counter public counter;

    uint32[3] private records = [uint32(3), 6, 27];
    uint32[3] private songLengths = [uint32(2), 3, 4];
    ...
    mapping(uint => string) private bgIdToColor;
    ...
    function addToWL(address input) external onlyOwner {
        whiteList[input] = 1;
    }
    ...
    function removeFromWL(address input) external onlyOwner {
        whiteList[input] = 0;
    }

    function removeFromDao(address input) external onlyOwner {
        prefList[input] = 0;
    }
    ...
    function _mintInternal() private  {
        ...
        uint djDiceRoll = uint(random(100));
        uint32 randDJIdx;
        if(djDiceRoll > 0 && djDiceRoll < 18){
            randDJIdx = 4;
        } else if(djDiceRoll < 32){
            randDJIdx = 3;
        } else if(djDiceRoll < 43){
            randDJIdx = 6;
        } else if(djDiceRoll < 48){
            randDJIdx = 5;
        } else if(djDiceRoll < 59){
            randDJIdx = 7;
        } else if(djDiceRoll < 73){
            randDJIdx = 1;
        } else if(djDiceRoll < 86){
            randDJIdx = 0;
        } else {
            randDJIdx = 2;
        }
        uint id = random(360) + 1;
        uint lenDiceRoll = uint(random(10));
        uint recordsDiceRoll = uint(random(10));

        uint32 lenIdx = lenDiceRoll < 5 ? 0 : lenDiceRoll < 9 ? 1 : 2;
        uint32 recordsIdx = recordsDiceRoll < 7 ? 0 : recordsDiceRoll < 9 ? 1 : 2;

        Trait memory t = Trait(mintIndex, songLengths[lenIdx], id, randDJIdx, records[recordsIdx]);

        tokenIdToTrait[mintIndex] = t;

        emit MintDetails(randDJIdx, songLengths[lenIdx], records[recordsIdx], owner(), msg.sender, msg.value, mintIndex);
    ...
    function updateStaticSong(uint256 tokenId, address sender) external {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory nameT = string(abi.encodePacked('{"name": "',  details.name, " #", details.tokenId, '",'));
        ...
        return string(abi.encodePacked('data:application/json,', nameT, details.description, ',"image":  "', 
            details.imgUrl, ',"animation_url": "', details.animationUrl, '","external_url": "https://www.0xmusic.com", "background_color": "', details.hexColor, '" ,', 
                '"attributes": [', 
                djTrait, cycleLengthTrait, ambienceT, recordsTrait));
