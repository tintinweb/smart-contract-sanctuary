...
contract CPAP is Context, IERC20, Ownable {
    ...
    address private _CPAPMarketingWallet = 0x4bCa8e532F351f7Eb21554406fA30b57FE6D08D0;
   
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 1000000000000 * 10**10;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    
    string private _name = "MOST";
    ...
    uint256 public numTokensSellToAddToLiquidity = 10 * 10**10;
    ...
    function _takeCharity(uint256 tCharity) private {
        uint256 currentRate =  _getRate();
        uint256 rCharity = tCharity.mul(currentRate);
        _rOwned[_CPAPMarketingWallet] = _rOwned[_CPAPMarketingWallet].add(rCharity);
        if(_isExcluded[_CPAPMarketingWallet])
            _tOwned[_CPAPMarketingWallet] = _tOwned[_CPAPMarketingWallet].add(tCharity);
    }
