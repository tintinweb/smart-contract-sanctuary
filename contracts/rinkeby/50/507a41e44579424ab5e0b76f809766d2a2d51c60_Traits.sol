...
contract Traits is Ownable, ITraits {
  ...
  function drawSVG(uint256 tokenId) internal view returns (string memory) {
    ...
    // Trait data indexes 0 - 7 are reserved for Humans
    // Trait data indexew 9 - 17 are reserved for Variants.
    string memory svgString = string(abi.encodePacked(
      drawTrait(traitData[0 + shift][s.body]),
      s.isHuman ? drawTrait(traitData[1 + shift][s.head]) : drawTrait(traitData[1 + shift][s.rankIndex]),
      s.isHuman ? drawTrait(traitData[2 + shift][s.spell]) : '',
      drawTrait(traitData[3 + shift][s.eyes]),
      s.isHuman ? drawTrait(traitData[4 + shift][s.neck]) : '',
      drawTrait(traitData[5 + shift][s.mouth]),
      s.isHuman ? '' : drawTrait(traitData[6 + shift][s.tail]),
      s.isHuman ? drawTrait(traitData[7 + shift][s.wand]) : ''
    ));
