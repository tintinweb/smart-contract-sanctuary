...
//    uint256 public oneHourBlock = 240;
    ...
    uint256 public advertisingSpace = 5;
    //每小时成块数
    uint256 public oneHourBlock = 10;
    ...
    //当前竞价中
    mapping(uint256 => Round) private currentBidding;
    ...
    //参与竞拍
    event participateBidding(uint256 indexed adIndex,uint256 indexed epoch,uint256 amount,address biddingAddress);
    //竞拍成功
    event successfulBidding(uint256 indexed adIndex,uint256 indexed epoch,string content,address userAddress);
    //返还失败竞拍
    ...
    function _startBidding(uint256 adIndex) external locationExists(adIndex) onlyOwner{
        ...
        require(currentBidding[currentRound-1].endBlock.add(showBlock) < block.number,"THE_LAST_ROUND_OF_IMPRESSIONS_IS_NOT_OVER_YET");
        epochMap[adIndex] = currentRound;
        Round storage round = currentBidding[currentRound];
    ...
    function _endBidding(uint256 adIndex) external locationExists(adIndex) onlyOwner{
        ...
        //给上一次出价的返利
        uint256 rebate = (adUser.latestBid.sub(lastPrice)).mul(rebatePercentage).div(percentage);

        uint256 teamAmount = (adUser.latestBid.sub(rebate).mul(proportionOfTeam).div(percentage));
        TransferHelper.safeTransfer(pttoken,teamAddress,teamAmount);
        //销毁数量
        uint256 destroyAmount =adUser.latestBid.sub(rebate).sub(teamAmount);
        TransferHelper.safeTransfer(pttoken,blackHoleAddress,destroyAmount);
        emit successfulBidding(adIndex,currentRound,adUser.content,adUser.userAddress);
    }
    ...
    *   竞拍合约
    */
    function _auctionBilling(uint256 adIndex,string memory content) external locationExists(adIndex) biddingLock{
        ...
        emit participateBidding(adIndex,currentRound,thisBid,msgSender);
    }
    ...
    function addBidReward(uint256 adIndex,uint256 currentRound,uint256 bidPrice,address lastAddress) internal {
        ...
        adUser.bidReward += (bidPrice.sub(adUser.latestBid)).mul(rebatePercentage).div(percentage);
    }
    ...
    function _redemption(uint256 epoch,uint256 adIndex) external locationExists(adIndex){
        ...
        Round storage round = currentBidding[epoch];
