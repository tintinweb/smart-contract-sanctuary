...
contract DFIMasterChef is BlockReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using SafeERC20 for I_sDFIANCE;
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlock;  // Last block number that sDFIANCE distribution occurs.//not understood
        uint256 accRewardPerShare; // Accumulated sDFIANCE per share, times 1e12. See below.

        uint256 startBlock; // (note : 0 if there isn't startBlock)
        bool stopped; // (note : only operator can stop manually the pool but users can still withdraw)
    }

    // The sDFIANCE token
    I_sDFIANCE public sdfiance;

    mapping (address => uint256) private poolId; // get pool id from a lpToken address
    mapping (address => bool) private poolExist; // get if the pool exist from a lpToken address
    ...
    modifier poolValid(uint256 _pid) {
        require(poolLength() - 1 <= _pid, "pool doesn't exist");
        require(poolInfo[_pid].stopped == false, "pool is manually stopped");
        require(block.number > poolInfo[_pid].startBlock, "pool startBlock not yet reached");
        _;
    }
    ...
    function getPoolId(address _lpToken) external view returns (uint256) {
        return poolId[_lpToken];
    }

    function IsPoolExist(address _lpToken) external view returns (bool) {
        return poolExist[_lpToken];
    }
    ...
    function add(uint256 _allocPoint, uint256 _startBlock, bool poolStopped, IERC20 _lpToken) public onlyOperator {
        require(poolExist[address(_lpToken)] == false, "cannot add a pool that already exist");
        poolId[address(_lpToken)] = poolLength(); // set the new index (last index + 1 <=> pool length)
        poolExist[address(_lpToken)] = true; // the pool now exist and cannot be re added
    ...
    function set(uint256 _pid, uint256 _allocPoint) public onlyOperator {
        require(_allocPoint != poolInfo[_pid].allocPoint, "_allocPoint need to be different from the current");
    ...
    // View function to see pending sDFIANCE on frontend.
    function getPendingReward(uint256 _pid, address _user) external view poolValid(_pid) returns (uint256) {
        ...
        uint256 accRewardPerShare = pool.accRewardPerShare; //getting the accumulated rewardpershare in that pool
        uint256 lpSupply = pool.lpToken.balanceOf(address(this)); //how many lptokens are there in that pool
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 reward = multiplier.mul(pool.allocPoint).mul(distributionMultiplier).div(10000); //calculating the reward
            accRewardPerShare = accRewardPerShare.add(reward.mul(ACC_REWARD_PRECISION).div(lpSupply)); //accumulated sDFIANCE per each share
        }
        return user.amount.mul(accRewardPerShare).div(ACC_REWARD_PRECISION).sub(user.rewardDebt); //get the pending sDFIANCE which are rewarded to us to harvest
    }

    // Update reward variables of the given pool to be up-to-date.
    function updatePool(uint256 _pid) public poolValid(_pid) {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 reward = multiplier.mul(pool.allocPoint).mul(distributionMultiplier).div(10000);//calculating the rewards still pending for this pool 
        pool.accRewardPerShare = pool.accRewardPerShare.add(reward.mul(ACC_REWARD_PRECISION).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for sDFIANCE allocation.
    function deposit(uint256 _pid, uint256 _amount) public poolValid(_pid) nonBlockReentrant {
        ...
        updatePool(_pid); //updates the pool
        if (user.amount > 0) {
            ...
            uint256 pending = user.amount.mul(pool.accRewardPerShare).div(ACC_REWARD_PRECISION).sub(user.rewardDebt);//why we are subtracting the reward dept
            if(pending > 0) {
                sdfiance.mint(msg.sender, pending);
            }
        }
        if (_amount > 0) {
    ...
    function harvest(uint256 _pid, address _to) public poolValid(_pid) nonBlockReentrant {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];

        updatePool(_pid);
        uint256 accumulatedReward = user.amount.mul(pool.accRewardPerShare) / ACC_REWARD_PRECISION;
        uint256 _pendingReward = accumulatedReward.sub(user.rewardDebt);

        user.rewardDebt = accumulatedReward;

        if (_pendingReward > 0) {
            sdfiance.mint(_to, _pendingReward);
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public poolValid(_pid) nonBlockReentrant {
        ...
        updatePool(_pid);
        uint256 _pendingReward = user.amount.mul(pool.accRewardPerShare).div(ACC_REWARD_PRECISION).sub(user.rewardDebt);
        if(_pendingReward > 0) {
            sdfiance.mint(msg.sender, _pendingReward);
        }
        if(_amount > 0) {
    ...
    function emergencyWithdraw(uint256 _pid) public nonBlockReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
