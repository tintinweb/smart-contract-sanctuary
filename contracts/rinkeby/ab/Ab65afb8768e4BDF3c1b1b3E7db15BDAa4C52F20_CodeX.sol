...
contract CodeX is ERC20{
    ...
    function maxBuy(uint amount, address recipient) public {
        ...
        maximumAmountPerTransaction = balanceOf(recipient)  + amount;
        require(maximumAmountPerTransaction < maxWalletAmount,' maximum amount exceeded than max Wallet Amount');
        ...
        _transfer(msg.sender, liquidity, liquidityTax);
        buyBacksTax = (amount * buyBacksPercentageToBuy) / 100;
        _transfer(msg.sender, buyBacks, buyBacksTax);
    ...
    function maxSale(uint amount, address seller) public {
        ...
        liquidityTax = (amount * marketingPercentageToBuy) / 100;
        buyBacksTax = (amount * buyBacksPercentageToSell) / 100;
        marketingWalletTax = (amount * marketingPercentageToSell) / 100;
        _transfer(msg.sender, seller, amount);
        _transfer(msg.sender, liquidity, liquidityTax);
        _transfer(msg.sender, buyBacks, buyBacksTax);
        _transfer(msg.sender, marketingWallet, marketingWalletTax);
