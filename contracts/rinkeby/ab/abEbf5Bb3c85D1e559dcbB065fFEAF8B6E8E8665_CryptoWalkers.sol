  ...
  ReentrancyGuard
{
  ...
  uint256 public constant RESERVED_WALKERS = 50;
  uint256 public constant WALKER_PRICE = 8E16; // 0.08ETH
  ...
  constructor() ERC721("CryptoWalkers", "Walkers") {
    _state = State.Setup;
    _startingIndex = 8900;
    _tokenUriBase = "https://nl6tehhk3bl0.usemoralis.com/{id}.json";
    _whitelistDetails[0].whitelistType = "Infected Walker";
    _whitelistDetails[0].amountToMint = 1;
    _whitelistDetails[1].whitelistType = "Metakey";
  ...
  function updateUserWhitelistStatus(address user) public {
  ...
  function mintReserve(address reserveAddress) public onlyOwner {
    ...
    for (uint256 i = 0; i < RESERVED_WALKERS; i++) {
      _safeMint(reserveAddress, _nextTokenId);
      _nextTokenId = _nextTokenId.add(1).mod(MAX_WALKERS);
    }
