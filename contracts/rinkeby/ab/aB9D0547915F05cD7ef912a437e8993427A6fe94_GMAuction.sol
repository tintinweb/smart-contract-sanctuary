...
contract GMAuction is Ownable {
    event Bid(uint256 artId, uint256 price, address bidder);
    event AuctionScheduled(uint256 artId, address beneficiary, uint256 startTime, uint256 endTime, uint256 startPrice, uint256 bidStep);
    event AuctionStart(uint256 artId, uint256 startPrice);
    event AuctionSuccess(uint256 artId, uint256 price, address winner);
    event AuctionFailure(uint256 artId);

    uint256 constant DEFAULT_DURATION = 7 days;
    uint256 constant MAX_START_PRICE = 1000 ether;
    uint256 constant MAX_BID_STEP = 50 ether;
    ...
    /**
        ...
        @param startTime_ scheduled auction start time
        @param endTime_ scheduled auction end time
        @param bidStep_ each new bid must be higher then the current highest bid plus bid step
    */
    ...
    */
    function scheduleInitialAuction(
        ...
        nft.transferFrom(msg.sender, address(this), artId_);
        logAuctionScheduled(artId_, auctions[artId_]);
    }
    ...
    function checkAuctionParams(
        ...
        require(startTime_ >= block.timestamp, "GMAuction constructor: start time must be in the future");
        require(endTime_ >= startTime_, "GMAuction constructor: end time must be later than start time");
        require(startPrice_ <= MAX_START_PRICE, "GMAuction constructor: start price too high");
        require(startPrice_ <= MAX_START_PRICE, "GMAuction constructor: bid step too high");
    }

    function logAuctionScheduled(uint256 artId, Auction storage a) private {
        emit AuctionScheduled(artId, a.beneficiary, a.startTime, a.endTime, a.startPrice, a.bidStep);
    }
    ...
    */
    function startAuction(uint256 artId) private {
        if (!started(artId)) {
            emit AuctionStart(artId, auctions[artId].startPrice);
        }
    }
    ...
    function bid(uint256 artId, uint256 amount) public onlyTrader whenActive(artId) {
        ...
        require(amount >= a.highestBid + a.bidStep, "GMAuction: bid amount must >= hightest bid + bid step");
        require(a.highestBidder != msg.sender, "GMAuction: you're the highest bidder already");
        stablecoin.transferFrom(msg.sender, address(this), amount);
        _refundBid(artId);
        a.highestBidder = msg.sender;
        a.highestBid = amount;
        emit Bid(artId, a.highestBid, msg.sender);
    }
    ...
    function completeAuction(uint256 artId) public whenEnded(artId) {
        ...
        if (a.highestBidder != address(0)) {
            _processAuctionSuccess(artId);
        } else {
            _processAuctionFailure(artId);
        }
        markNotScheduled(artId);
    }

    /**
        @dev Process a successful auciton by sending NFT to the highest bidder and stablecoin to its beneficiary
        @param artId ID of the item sold
      */
    function _processAuctionSuccess(uint256 artId) private {
        //        Auction storage a = auctions[artId];
        ...
        emit AuctionSuccess(artId, highestBid, highestBidder);
    }

    /**
        @dev Bid process a failed auction by sending NFT back to the beneficiary and refunding the  highest bid
        @param artId ID of the item sold
      */
    function _processAuctionFailure(uint256 artId) private {
        nft.transferFrom(address(this), auctions[artId].beneficiary, artId);
        _refundBid(artId);
        emit AuctionFailure(artId);
    }
    ...
    modifier onlyTrader {
        _;
        //TODO implement Trader status check
    }
    ...
    */
    function isActive(uint256 artId) public view returns (bool){
        ...
        return auctions[artId].startTime < block.timestamp && auctions[artId].endTime > block.timestamp;
    }

    function isScheduled(uint256 artId) public view returns (bool){
        return auctions[artId].startTime != 0;
    }
