...
contract Amused is Ownable, IERC20Metadata {
    ...
    event AmusedVaultRegistered(address indexed vault, uint256 timestamp);
    event AmusedVaultMint(address indexed vault, uint256 amount, uint256 timestamp);
    ...
    function setVault(address _vault) external onlyOwner {
        require(_isContract(_vault), "Amused: Not a valid contract address");
        AmusedVault = _vault;
        emit AmusedVaultRegistered(_vault, block.timestamp);
    }

    function amusedVaultMint(uint256 _amount) external {
        require(_msgSender()  == AmusedVault, "Amused: Authentication failed");
        _mint(AmusedVault, _amount);
        emit AmusedVaultMint(_msgSender(), _amount, block.timestamp);
    }
