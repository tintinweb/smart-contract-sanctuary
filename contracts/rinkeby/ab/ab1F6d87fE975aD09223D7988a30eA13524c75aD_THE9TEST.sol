...
contract THE9TEST is ERC20, Pausable, AccessControl {
    ...
    function createAmountWithLock(address beneficiary, uint256 amount, uint256 firstReleaseTime, uint256 unlockPercentage, uint256 lockCycleDays)
        public
        whenNotPaused
        onlyRole(ORG_ADMIN_ROLE) 
    {
        assert(_beforeSaveAmountWithLock(beneficiary, firstReleaseTime, unlockPercentage, lockCycleDays));
        
        if (_checkExists(beneficiary)) {
            updateAmountWithLock(beneficiary, amount, firstReleaseTime, unlockPercentage, lockCycleDays);
        } else {
            _getInfo(beneficiary).amount = _calcDecimal(amount);
            _getInfo(beneficiary).releaseTime = firstReleaseTime;
            _getInfo(beneficiary).unlockPercentage = unlockPercentage;
            _getInfo(beneficiary).lockCycleDays = lockCycleDays;
            _getInfo(beneficiary).remainPercent = 100;
            _getInfo(beneficiary).remainAmount = _calcDecimal(amount);
    ...
    function updateAmountWithLock(address beneficiary, uint256 amount, uint256 firstReleaseTime, uint256 unlockPercentage, uint256 lockCycleDays)
        public
        whenNotPaused
        onlyRole(ORG_ADMIN_ROLE) 
    {
        ...
        assert(_beforeSaveAmountWithLock(beneficiary, firstReleaseTime, unlockPercentage, lockCycleDays));
        require(_getInfo(beneficiary).remainPercent >= 100, "account to which revenue was transferred");

        emit UpdateAmountWithLock(beneficiary, amount, firstReleaseTime, _getInfo(beneficiary));
        ...
        onlyRole(ORG_ADMIN_ROLE) 
    {
        ...
        require(_isReleased(beneficiary), "It hasn't been released yet");
        ...
        if (transferableAmount > _getInfo(beneficiary).remainAmount) {
            transferableAmount = _getInfo(beneficiary).remainAmount;
            remainPercent = 0;
        } else {
            remainPercent = SafeMath.sub(remainPercent, beforeInfo.unlockPercentage);
        }
        ...
        _getInfo(beneficiary).amount = beforeInfo.amount;
        _getInfo(beneficiary).releaseTime = _getNextReleaseTime(beneficiary);
        _getInfo(beneficiary).unlockPercentage = beforeInfo.unlockPercentage;
        _getInfo(beneficiary).lockCycleDays = beforeInfo.lockCycleDays;
        _getInfo(beneficiary).remainPercent = remainPercent;
        _getInfo(beneficiary).remainAmount = SafeMath.sub(remainAmount, transferableAmount);
    ...
    function _beforeSaveAmountWithLock(address beneficiary, uint256 firstReleaseTime, uint256 unlockPercentage, uint256 lockCycleDays)
        internal
        view
        returns (bool)
    {
        require(beneficiary != address(0), "beneficiary from the zero address");
        require(unlockPercentage > 0, "percentage cannot be zero");
        require(unlockPercentage <= 100, "percentage cannot exceed 100");
        require(lockCycleDays > 0, "lock cycle days cannot be zero");
        
        if (firstReleaseTime == 0) {
            firstReleaseTime = DEFAULT_RELEASE_TIMESTAMP;
        }
        require(firstReleaseTime > block.timestamp, "first release time is before current time");
    ...
    function _getNextReleaseTime(address beneficiary) 
        ...
        returns(uint256) 
    {
        return SafeMath.add(_getInfo(beneficiary).releaseTime, SafeMath.mul(_getInfo(beneficiary).lockCycleDays, 24*60*60));
    }
        ...
        returns (uint256)
    {
        if (_isReleased(beneficiary)) {
            return SafeMath.div(SafeMath.mul(_getInfo(beneficiary).amount, _getInfo(beneficiary).unlockPercentage), 100);
        } else {
            return 0;
        }
    }
