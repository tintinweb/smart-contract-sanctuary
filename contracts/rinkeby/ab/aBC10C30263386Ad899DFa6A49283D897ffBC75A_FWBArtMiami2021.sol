...
contract FWBArtMiami2021 is ERC1155, Ownable {
    ...
    constructor() public ERC1155("https://gateway.pinata.cloud/ipfs/QmNrGBUUHqVwM8maX1nPdHEfX8i6Dx49zGkeLGp48uLirH/{id}.json") {
    ...
    function withdraw() public onlyOwner {
        ...
        _withdraw(artist1, artistCut);
        _withdraw(artist2, artistCut);
        _withdraw(artist3, artistCut);
        _withdraw(artist4, artistCut);
        _withdraw(artist5, artistCut);
        _withdraw(artist6, artistCut);
        _withdraw(artist7, artistCut);
        _withdraw(artist8, artistCut);
        _withdraw(fwb, address(this).balance);
    }
    ...
    function batchMint(address to, uint[] memory artists, uint[] memory amounts) public onlyOwner {
        for (uint256 i = 0; i < artists.length; i++) {
            uint artist = artists[i];
            uint numberOfTokens = amounts[i];
            require(artist > 0 && artist <= 8, "invalid artist");
            require(numberOfTokens > 0 && numberOfTokens <= 2, "invalid token amount");
            _mint(to, artist, numberOfTokens, "");
            amountMintedPerArtist[artist] += numberOfTokens;
            amountMintedPerWallet[to].mintsPerArtist[artist] += numberOfTokens;
        }
    }
