...
contract ERC20Basic {
    ...
    string public constant name = "NineERC";
...
contract sellNftNine is Pausable{

    using SafeMath for uint256;

    address owner = msg.sender;
    ERC721 public nftAddress;
    ERC20Basic public tokenAddress;
    uint256 public currentPrice;

    event Received(address indexed payer, uint tokenId, uint256 amount);

    constructor(address _nftAddress, address _tokenAddress, uint256 _currentPrice) public {
        require(_nftAddress != address(0) && _nftAddress != address(this));
        require(_tokenAddress != address(0) && _tokenAddress != address(this));
        require(_currentPrice > 0);
        nftAddress = ERC721(_nftAddress);
        tokenAddress = ERC20Basic(_tokenAddress);
    ...
    function purchaseToken(uint256 _tokenId) public payable whenNotPaused {
        ...
        require(tokenAddress.balanceOf(msg.sender) >= currentPrice);

        tokenAddress.transferFrom(msg.sender, owner, currentPrice);
        address tokenSeller = nftAddress.ownerOf(_tokenId);
        nftAddress.safeTransferFrom(tokenSeller, msg.sender, _tokenId);
        _pause();

        emit Received(msg.sender, _tokenId, currentPrice);
    }
