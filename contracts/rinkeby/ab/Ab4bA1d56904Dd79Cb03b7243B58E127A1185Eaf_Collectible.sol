...
contract Collectible is ERC721 {
    ...
    mapping(uint256 => uint256) public tokenidToFund;
    event requestedCollectible(uint256 indexed newTokenId, address requester);
    ...
    function createCollectible() public returns (bytes32){
        ...
        emit requestedCollectible(newTokenId, msg.sender);
    ...
    function fund(uint256 tokenId) external payable {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: caller is not owner no approved");
        tokenidToFund[tokenId] = msg.value;
    }
    function withdraw(uint256 tokenId) external payable {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: caller is not owner no approved");
        payable(msg.sender).transfer(tokenidToFund[tokenId]);
    }
}
