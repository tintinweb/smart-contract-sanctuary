...
contract ICOContract is Ownable {
    ...
    function defineStep() private view returns(uint256) {
        for (uint256 i = 0; i < limits.length - 1; i++) {
            if (limits[i] * rates[i] > gnusSoldAmount) return i;
        }
    ...
    */ 
    ...
    receive () external payable {
        ...
        require(msg.value > 0, "You've sent 0 ether!");
        uint256 tokenAmount = msg.value * rates[defineStep()];
        require(gnusBalance() > tokenAmount, "You've sent too much eth amount");
        sendGNUStoReceiver(msg.sender, tokenAmount);
        emit Received(msg.sender, msg.value);
    }

    // Send GNUS tokens 
    function sendGNUStoReceiver (address to, uint256 amount) internal onlyOwner {
        require(gnusToken.transferFrom(address(this), to, amount) == true, "Could not send GNUS tokens");
    }
    ...
    function withdrawGNUS(address to, uint256 amount) external onlyOwner {
        ...
        require(gnusBalance() > amount, "You've sent too much eth amount");
        require(gnusToken.transferFrom(address(this), to, amount) == true, "Could not send GNUS tokens");
    }
}
