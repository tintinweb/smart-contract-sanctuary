...
contract KoiFishCollective is ERC721Enumerable, Ownable, ReentrancyGuard {
    // Provenance hash for all mutants (Minted, Mutated Koi, AUctions)
    string public constant KOI_PROVENANCE = "";
    
    uint256 public constant PS_MAX_KOI_PURCHASE = 20;
    // Max supply of Minted Koi
    uint256 public PS_MAX_KOI = 10000;
    // Public sale final price - 0.01 ETH
    uint256 public constant PS_KOI_PRICE = 50000000000000000;
    uint256 public maxKoi;
    uint256 public revealTimestamp;

    // Supply of Minted Koi
    uint256 public numKoiMinted;

    // Public sale params
    uint256 public publicSaleDuration;
    uint256 public publicSaleStartTime;
    uint256 public publicSaleKoiStartingPrice;
    ...
    // Starting index for Minted Koi
    uint256 public mintedKoiStartingIndex;

    string private baseURI;

    event KoiPublicSaleStart(
        uint256 indexed _saleDuration,
        uint256 indexed _saleStartTime
    );
    event KoiPublicSalePaused(
        uint256 indexed _currentPrice,
        uint256 indexed _timeElapsed
    );
    event StartingIndicesSet(
        uint256 indexed _mintedKoiStartingIndex
    );
    ...
    modifier startingIndicesNotSet() {
        require(
            mintedKoiStartingIndex == 0,
            "Minted Koi starting index is already set"
        );
        _;
    }
    constructor(
        ...
        onlyOwner
    {
        ...
        publicSaleKoiStartingPrice = saleStartPrice;
        publicSaleStartTime = block.timestamp;
        publicSaleActive = true;
        emit KoiPublicSaleStart(saleDuration, publicSaleStartTime);
    }

    function pausePublicSale() external onlyOwner whenPublicSaleActive {
        uint256 currentSalePrice = PS_KOI_PRICE;
        publicSaleActive = false;
        emit KoiPublicSalePaused(currentSalePrice, getElapsedSaleTime());
    }
    ...
    function mintKoi(uint256 numKoi)
        external
        payable
        whenPublicSaleActive
    {
        require(
            numKoiMinted + numKoi <= PS_MAX_KOI,
            "Minting would exceed max supply"
        );
        require(numKoi > 0, "Must mint at least one Koi");
        require(
            numKoi <= PS_MAX_KOI_PURCHASE,
            "Requested number exceeds maximum"
        );

        uint256 costToMint = PS_KOI_PRICE * numKoi;
        require(costToMint <= msg.value, "Ether value sent is not correct");
        
        if (mintedKoiStartingIndex == 0) {
            collectionStartingIndexBlock = block.number;
        }

        for (uint256 i = 0; i < numKoi; i++) {
            uint256 mintIndex = numKoiMinted;
            if (numKoiMinted < PS_MAX_KOI) {
                numKoiMinted++;
    ...
    function getKoiId(uint256 koiId)
        internal
        pure
        returns (uint256)
    {
    ...
    function setStartingIndices() external startingIndicesNotSet {
        ...
        mintedKoiStartingIndex = calculateStartingIndex(
