...
/**
 * @dev This is a fork of openzeppelin ERC721 contract. The updated part is that we add a new hook
 * `_beforeTokenMint` that is called before minting, and `_beforeTokenTransfer` is no longer called
 * before minting. In this way, web3 can now estimate the gas cost of `_mint` correctly.
 */
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        ...
        // We replaced the original hook `_beforeTokenTransfer` with a new hook `_beforeTokenMint`.
     ...
     *
     * [Updated]
     * Token minting will not call this function, instead call `_beforeTokenMint`, for a better
     * gas estimation for web3.
     */
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual {}

    /// @dev This function is called when minting token.
 ...
 * - added a hook `_beforeTokenMint`, because we're using our updated version of ERC721 contract.
 */
abstract contract ERC721EnumerableSimple is ERC721, IERC721Enumerable {
    ...
    /// @dev Hook that is called before minting. This is a new hook defined in us but not openzepplin
    /// See our updated ERC721 code for a explanation of this hook.
    function _beforeTokenMint(address to, uint256 tokenId) internal virtual override {
    ...
    /// @dev Hook that is called before any token transfer. This includes minting
    /// Nope, this does not include minting. See our updated ERC721 code for a explanation.
    function _beforeTokenTransfer(
        ...
        if (from == address(0)) {
            assert(false); // This will never happen, since minting won't be calling this function
        }
