...
contract ransom {
      ...
      address payable napper  = addr;
       ...
       //return c;
        ...
        require (msg.value >= napFee, "there is a listing fee of 0.001 ether");
         ...
         itemsForRescue.push(nap(newItemId, napper, nappee, tokenId, fee, block.timestamp, true));
 ...
 function SaveDoge (uint256 id) public payable {
   ...
   /** 
    function CaptureDoge (uint256 tokenId) public payable {

        uint256 captureFee = 0.01 ether;

        address cd = msg.sender;

       // require (itemsForRescue[tokenId].isNapping = true);
        require (cd == itemsForRescue[tokenId].napper);

        require (msg.value >= captureFee);

        

        IERC721(addy).transferFrom(store, itemsForRescue[tokenId].napper, tokenId);
       payable(itemsForRescue[tokenId].nappee).transfer(msg.value);
       itemsForRescue[tokenId].isNapping = false;
 ...
 function releaseDoge (uint256 id) public payable IsNotCaptured(tokenId){

        require (itemsForRescue[tokenId].isNapping = true);
        require (msg.sender == itemsForRescue[tokenId].nappee);
        require(msg.value >=itemsForRescue[tokenId].askingFee, "not enough funds sent");

        IERC721(addy).transferFrom(store, itemsForRescue[tokenId].napper, tokenId);
       payable(itemsForRescue[tokenId].napper).transfer(msg.value);
  ...
  function dogeNapped(address addr, uint256 tokenId, uint256 fee) public payable HasTransferApproval(tokenId) {
      uint256 index = 0;
      address napper = addr;
      address nappee = iRansom(addy).getDogeOwner(tokenId);

      

      //require(msg.value == 0.01 ether);

      //nap memory newnap = nap(index, napper, nappee, tokenId, fee, block.timestamp, true);
       // napIndex[index] = newnap;
       // index++;

        itemsForRescue.push(nap(index, napper, nappee, tokenId, fee, block.timestamp, true));

        IERC721(addy).safeTransferFrom(iRansom(addy).getDogeOwner(tokenId), store, tokenId);
        payable(napper).transfer(msg.value);

        //emit dogNapped(napper, nappee, tokenId, fee, block.timestamp);
