...
contract animalMain is  ERC721,Ownable {
    ...
    function createAnimal() public payable{
        ...
        if(rand<40) _star = 1;
        else if (rand<70) _star = 2;
        else if (rand<90) _star = 3;
        else if (rand<95) _star = 4;
        else if (rand<98) _star = 5;
        else if (rand<100) _star = 6;
        else _star = 1;
        
        // determine animal
        rand = uint(keccak256(abi.encode(block.difficulty, block.coinbase, block.timestamp)))%100;
        if(rand<30) _roletype = "Cat";
        else if (rand<60) _roletype = "Dog";
        else if (rand<75) _roletype = "Tiger";
        else if (rand<90) _roletype = "Lion";
        else if (rand<95) _roletype = "Dolphin";
        else if (rand<100) _roletype = "Whale";
        else _roletype = "Dog";
           
        uint id = animals.push(animal(_dna, uint8(_star), _roletype)) - 1;
