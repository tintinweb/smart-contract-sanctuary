...
contract ThievesGang is Context, ERC165, IERC1155, IERC1155MetadataURI, Ownable {
    ...
    uint256 private _max_tokens = 10200;
    ...
    function whitelisted(address _a) public view returns (bool) {
        return whitelist[_a];
    }
    ...
    function initWhitelist(address[] memory addresses) public onlyOwner {
    ...
    function reserve(uint256 number_of_tokens) public onlyOwner {
        require(total_supply + number_of_tokens <= _max_tokens, "Minting amount exceeds maximum supply");
        uint256[] memory ids = new uint256[](number_of_tokens);
        for(uint i = 0; i < number_of_tokens; i++) {
            ids[i] = total_supply + i + 1;
        }
