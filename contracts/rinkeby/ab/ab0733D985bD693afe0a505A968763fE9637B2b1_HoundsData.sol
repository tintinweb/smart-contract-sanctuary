...
contract HoundsData is Ownable, ERC721, ERC721Holder {
    ...
    event NewHound(uint256 indexed houndId, Hound.Default hound);
     ...
     * Admin only
     */
    function adminCreateHound(Hound.Default memory theHound) external onlyOwner {
    ...
    function hound(uint256 theHoundId) external view returns(Hound.Default memory) {
        require(theHoundId < houndId, "23");
        return hounds[theHoundId];
    }

    function getHoundOwner(uint256 theHoundId) external view returns(address) {
        return ownerOf(theHoundId);
    }
