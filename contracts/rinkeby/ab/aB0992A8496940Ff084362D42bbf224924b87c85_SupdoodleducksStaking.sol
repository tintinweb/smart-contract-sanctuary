...
contract SupdoodleducksStaking is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    address public stakingDestinationAddress = 0x2386693cb8c2F38Fa51fE2519d55587165f07704;
    ...
    */
    function withdraw(uint256[] calldata _tokenIds) external nonReentrant() {
        for (uint256 i; i < _tokenIds.length; i++) {
            ...
            tokenofaddr[_tokenIds[i]] = nullAddress;
    ...
    */
    function openLottery(uint time, uint256[] calldata staketokenIds, uint256 ethprice) public onlyOwner {
        ...
        //        for (uint256 i; i < lotterytokenids.length; i++) {
        //            if (lotterytokenids[i] > 0) {
        //                _tokenIds.push(lotterytokenids[i]);
        //            }
        //        }
        _tokenIds = lotterytokenids;
        for (uint256 i; i < _tokenIds.length; i++) {
            seedinput = seedinput + _tokenIds[i] + ethprice;
        }
        seedinput = seedinput + block.timestamp;
        rand = random(string(abi.encodePacked(seedinput.toString())));
        index = rand % _tokenIds.length;
        winnertokenid = _tokenIds[index];
        winner = queryownerbytoken(_tokenIds[index]);
