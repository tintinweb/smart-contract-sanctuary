...
contract RGT is Ownable{
    ...
    function getRandomToken(uint _seed, address _address) public view returns (uint256, address) {
        ...
        return (
            tokenId,
            ERC721(_address).ownerOf(tokenId)
            );
    }

    function gaiveaway(uint _seed, address _address, address _giveawayAddress, uint256 _giveawayTokenId) public payable returns (uint256, address, address, uint256) {
        (uint256 _tokenId, address _winner) = getRandomToken(_seed, _address);
        ERC721(_giveawayAddress).transferFrom(msg.sender, _winner, _giveawayTokenId);
        return (
            _tokenId,
            _winner,
            _giveawayAddress,
