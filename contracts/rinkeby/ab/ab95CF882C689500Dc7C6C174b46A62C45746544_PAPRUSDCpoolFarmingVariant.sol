...
contract PAPRUSDCpoolFarmingVariant is Ownable, ReentrancyGuard {
    ...
    (uint256 amountOut) = IUniswapRouterETH(unirouter).getAmountOut(amount, reserve1, reserve0);
    IUniswapRouterETH(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(amount, amountOut.mul(slippage).div(100), usdcToPRNTRRoute, address(this), block.timestamp);
    }
