...
contract KaijuBreeder is Ownable {
    ...
    function setKaijuAddress(address _address) external onlyOwner {
        kaiju = _address;
        Kaiju = IKaiju(kaiju);
    }
    
    function setRwasteAddress(address _address) external onlyOwner {
        rwaste = _address;
        Rwaste = IRWaste(rwaste);
    }
    ...
    function retrieveKaiju(uint256 _id) external onlyOwner {
    ...
    function claimRwaste() public view onlyOwner {
        Rwaste.claimReward;
    }
    ...
    //@notice most of this will go back to the Kaiju DAO !
    ...
    function breedYou() public {
        ...
        require(Kaiju.balanceGenesis(address(this)) > 0,                                                         "It seems like no Kaiju is available for you to use");
        
        uint256 UserKaijuId = Kaiju.walletOfOwner(msg.sender)[0];
        uint256 BabyId = Kaiju.maxGenCount() + Kaiju.babyCount();
    ...
    //@dev same as above but uses the Rwaste inside the contract and send the baby back to the contract's owner
     function breedMe() public {
        
        require(active,                                                                                         "Sorry, the system is not live at the moment!");
        require(IERC20(rwaste).balanceOf(address(this)) >= 750 ether,                                           "Looks like i don't have enougth Rwaste to do this!");
        require(Kaiju.balanceGenesis(msg.sender) > 0,                                                           "Is your Kaiju in your back pocket ?");
