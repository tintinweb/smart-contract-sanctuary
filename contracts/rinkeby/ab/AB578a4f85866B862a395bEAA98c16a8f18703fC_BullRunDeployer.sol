...
contract BullRunDeployer {
    ...
    constructor(address vault_, bytes32 holdersMerkleRoot, string memory holdersUri, bytes32 whitelistMerkleRoot, string memory whitelistUri) {
        vault = vault_;
        config = address(new BullRunConfig());
        whitelist = address(new BullRunWhitelist(config, whitelistMerkleRoot, whitelistUri));
        token = address(new BullRunToken("BullRun Token", "BULL", 475000 ether, config));
        distributor = address(new BullRunDistributor(holdersMerkleRoot, holdersUri, config));
        dividends = address(new BullRunDividends(config));
        
        IBullRunConfig(config).initialize(token, whitelist, distributor, vault, 0xc778417E063141139Fce010982780140Aa0cD5Ab);
...
contract BullRunConfig is AccessControlUpgradeable {
    ...
    function initialize(address token_, address whitelist_, address distributor_, address vault_, address usd_) public initializer {
        require(deployer == msg.sender, "BullRunConfig: only deployer can execute initializer");
        __AccessControl_init();
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        updateAll(token_, whitelist_, distributor_, vault_, usd_);
    }
    ...
    function updateUsd(address usd_) public onlyRole(DEFAULT_ADMIN_ROLE) {
        usd = usd_;
    }

    // Updates all contracts addresses
    function updateAll(address token_, address whitelist_, address distributor_, address vault_, address usd_) public {
        updateToken(token_);
...
contract BullRunWhitelist is AccessControlProxyPausable {
  ...
  mapping(address=>uint256) public retention;

  event Added(address account, uint256 amount);
  event Removed(address account);
  event RetentionUpdated(address account, uint256 amount);
  event ToggleDisabled(bool state);

  constructor (address config, bytes32 merkleRoot_, string memory uri_) {
    __AccessControlProxyPausable_init(config);
    merkleRoot = merkleRoot_;
    uri = uri_;
  }

  function add(uint256 index, address account, uint256 amount, bytes32[] memory merkleProof) public whenNotPaused {
    require(!authorized[account], "BullRunWhitelist: account already whitelisted");
    bytes32 node = keccak256(abi.encodePacked(index, account, amount));
    require(MerkleProofUpgradeable.verify(merkleProof, merkleRoot, node), "BullRunWhitelist: invalid proof.");
    authorized[account] = true;
    retention[account] = amount;
    emit Added(account, amount);
  }

  function remove(address account) public whenNotPaused onlyRole(DEFAULT_ADMIN_ROLE){
      require(authorized[account], "BullRunWhitelist: account not whitelisted");
  ...
  function whitelisted(address account) public view returns(bool) {
    return disabled ? true : authorized[account];
  }

  function updateRetention(uint256 index, address account, uint256 amount, bytes32[] memory merkleProof) public whenNotPaused onlyRole(DEFAULT_ADMIN_ROLE) {
    bytes32 node = keccak256(abi.encodePacked(index, account, amount));
    require(MerkleProofUpgradeable.verify(merkleProof, merkleRoot, node), "BullRunWhitelist: invalid proof.");
    authorized[account] = true;
    retention[account] = amount;
    emit RetentionUpdated(account, amount);
  }
...
contract BullRunToken is ERC20CappedUpgradeable, AccessControlProxyPausable {
  ...
  constructor(string memory name, string memory symbol, uint256 supply, address config) {
    initialize(name, symbol, supply, config);
  }
...
contract BullRunDistributor is AccessControlProxyPausable {
    ...
    constructor(bytes32 merkleRoot_, string memory uri_, address config_) {
    ...
    function claim(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) external whenNotPaused {
      ...
      IBullRunToken(IBullRunConfig(config).token()).mint(account, amount);
