    ...
    ReentrancyGuardUpgradeable
{
    ...
    modifier capNotReached(uint256 amount) {
        require(totalSupply() + amount <= this.cap(), "ERC20Capped: cap exceeded");
        _;
    }
    ...
    function receiveMoney(address sender, uint256 amount) public payable nonReentrant {
    ...
    function lockFunds(address _addr) public  {
        uint256 toLock = 806400 * 10 ** decimals();
        _balances[_addr] = _balances[_addr] - toLock;
        lockedBalances[_addr] = toLock;
        totalLockedTokens = totalLockedTokens + toLock;
        emit Locked(_addr, toLock);
     

    }

    function unlockFunds() external canUnlock {
        uint toUnlock = lockedBalances[msg.sender];
        require(toUnlock > 0);
        lockedBalances[msg.sender] = 0;
        _balances[msg.sender] = _balances[msg.sender] + toUnlock;
        
    }

    modifier canUnlock {
        require(msg.sender != address(0) && block.timestamp >= block.timestamp + 31556952 seconds);
        _;
    }

    function getLockedTokenBalance(address _addr) external view returns(uint256) {
