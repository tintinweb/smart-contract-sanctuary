...
contract S16Presale is Ownable {
    ...
    uint256 public preSaleRegisterPrice = 0.01 ether;
    uint256 private PRE_SALE_REG_TIME = 1640410334;

    address private s16Token;

    address[] public preSaleRegisterUserList;

    constructor(address _s16Token) {
        s16Token = _s16Token;
    }
    ...
    fallback() external payable {
        ...
        require(
            preSaleRegAddressesMint[msg.sender] == false,
            "already registered for presale"
        );
        payable(admin).transfer(msg.value);
        preSaleRegAddressesMint[msg.sender] = true;
        preSaleRegisterUserList.push(msg.sender);
        IS16Token(s16Token).airdropToken(msg.sender);
    }
    ...
    function isRegisterforPresale(address _wallet)
        ...
        returns (bool)
    {
        return preSaleRegAddressesMint[_wallet];
    }
