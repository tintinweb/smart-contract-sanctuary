...
contract nftBin is ERC721Enumerable, Ownable {
    struct NFT {
        ERC721 nftContract;
        uint256 tokenId;
    }
    ...
    function santaDepositNFT(ERC721 contractaddr, uint256 tokenId) external {
        contractaddr.safeTransferFrom(msg.sender, address(this), tokenId);
        nftCount[msg.sender]++;
    ...
    function mintcertificate() external {
        require(minted[msg.sender] == false, "You already got certificate");
        require(nftCount[msg.sender] > 0, "You should send NFT first");
