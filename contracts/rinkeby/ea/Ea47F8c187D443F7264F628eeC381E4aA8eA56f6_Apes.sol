...
contract Apes is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
  ...
  function listToSell(uint256 _idx, uint256 _price) public {
    ...
    current.onSale = true;
    apesOnSaleList.push(current.id);
    apesOnSaleAmount++;
  ...
  function migrate(uint256 _tokenId) public {
    IOldApe oldApeInstance = IOldApe(address(0xCa00FDc5a1Fe300C703c9351B0d36cBFE0c1A2d4));
    (
      uint256 unUsedIdx,
      uint256 unUsedId,
      uint8 level,
  ...
  uint256[] public apesOnSaleList;
  uint256 public apesOnSaleAmount;

  function addApeAtSellList(uint256 apeId) public onlyOwner {
    apesOnSaleList.push(apeId);
    apesOnSaleAmount++;
  }
}
