...
contract MintPass is ERC1155, ERC1155Burnable, ERC1155Supply, ERC1155Pausable, Ownable {
  ...
  uint256 public constant publicLaunch = 1638478800;
  ...
  function burnFromRedeem(address account, uint256 token, uint256 count) external onlyRedeemer {
  ...
  function mintPassWithAllocation(
    ...
    bytes memory input = abi.encode(msg.sender, allocated);
    bytes32 leaf = keccak256(input);
    if (!proof.verify(_rootHash, leaf)) {
      revert InvalidProof(msg.sender, allocated, leaf, _rootHash);
    }
    ...
    _mint(_msgSender(), 0, count, "minted from presale");
  ...
  function mintPass(uint256 count) external payable onlyAfter(publicLaunch) {
