...
contract ECRegistryV2 is Ownable {
    ...
    uint16 public traitControllerCount;
    ...
    */
    ...
    // traits
    event newTraitEvent(string _name, address _address);
    ...
    function _setTrait(uint16 traitID, uint16 tokenId, bool _value) internal {
        (uint8 byteNum, uint8 bitPos) = getByteAndBit(tokenId);
        if(traits[traitID].traitType == 1 || traits[traitID].traitType == 2) { _value = !_value; }
        if(_value) {
            tokenData[traitID][byteNum] = uint8(tokenData[traitID][byteNum] | 2**bitPos);
        } else {
            tokenData[traitID][byteNum] = uint8(tokenData[traitID][byteNum] & ~(2**bitPos));
        }
        emit tokenTraitChangeEvent(traitID, tokenId, _value);
    }
    ...
    */
    ...
    /*
    * _items = 1250 in order to load 10000 tokens ( 10000 / 8; starting from 0 )
    */
    ...
    function getTokenData(uint16 tokenId) public view returns (uint8[] memory) {
        ...
        while(i < traitCount) {
            if(hasTrait(i, tokenId)) {
                (uint8 byteNum, uint8 bitPos) = getByteAndBit(i);
    ...
    function getByteAndBit(uint16 _offset) public pure returns (uint8 _byte, uint8 _bit)
    {
        // find byte storig our bit
        _byte = uint8(_offset / 8);
