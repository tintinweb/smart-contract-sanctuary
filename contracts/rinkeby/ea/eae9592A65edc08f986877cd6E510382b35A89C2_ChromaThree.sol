    ...
    function makeTokenURI_(uint256 tokenId) internal
    {
        _setTokenURI( tokenId,
            string(abi.encodePacked(
                "https://candy.collect-code.com/api/token/", config_.seriesCode, 
...
library Utils
{
    ...
    function convertArrayToString_uint256(uint256[] memory values) public pure returns (string memory)
    {
        string memory result = "";
        for(uint256 i = 0 ; i < values.length ; i++)
            result = string(abi.encodePacked(result, (i==0?'':','), utoa(values[i])));
        return result;
    }
    ...
    function reduceColors(bytes20 s0, bytes20 s1, bytes20 s2, uint8 colorCount, uint256 offset) public pure returns (uint8[] memory)
    {
        ...
        uint256 sum0 = sum_bytes20(s0);
        uint256 sum1 = sum_bytes20(s1);
        bytes20 hues = lshift_bytes20(s2, sum1 + (offset*8));
        uint256 lh = sum_bytes20(s2) + sum1;
        for(uint8 c = 0 ; c < colorCount ; c++) {
            uint8 h = uint8(hues[c]);
            uint8 s = uint8((uint256(uint8(hues[c+10])) + sum0) % 256);
            uint8 v = uint8((uint256(uint8(hues[c+10])) + sum1) % 256);
            s = map_uint8(s, (64+(5-(colorCount/2))*30), 255);
            v = map_uint8(h|v, (127+(5-(colorCount/2))*30), 255);
