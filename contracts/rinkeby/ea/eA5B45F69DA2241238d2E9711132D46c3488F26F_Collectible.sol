...
contract Collectible is ERC721, Ownable {
    ...
    function mint(uint256 mints) public payable returns (uint256[] memory){
        ...
        require(Washies.balanceOf(msg.sender) >= 2, "Queen only for wallets holding 2 or more Washies!");
        require(balanceOf(msg.sender) < 2, "Limit 2 per wallet.");
        require(balanceOf(msg.sender) + mints <= 2, "Can't mint more than 2 queens per wallet.");
        require(totalSupply + mints <= maxSupply, "Not enough supply.");

        uint256 txPrice = mints*price;
        if (premint &&
            totalSupply + mints <= 500 &&
            !claimedFree[msg.sender]) { 
        ...
        uint256[] memory newTokenIds = new uint[](mints);
