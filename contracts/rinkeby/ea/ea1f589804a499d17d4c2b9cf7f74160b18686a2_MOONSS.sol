...
interface THEMOONMONSTERPunks {
    function tokensOfOwner(address ownwer) external view returns (uint256[] memory);

}

contract MOONSS is ERC20, ERC20Burnable, Ownable {

    address constant public THEMOONMONSTERAddress = 0x2FcCEB9813fEa6015E7583537f500e436a6E450B;
    ...
    constructor() ERC20("Moonss", "MOONSS") {}
    ...
    function setSaleState(bool newState) public onlyOwner {
     ...
     function totalAvailable(address ownwer) public view returns (uint256) {
        uint[] memory punks = THEMOONMONSTERPunks(THEMOONMONSTERAddress).tokensOfOwner(ownwer);
    ...
    function claim() public payable  {
        ...
        uint[] memory punks = THEMOONMONSTERPunks(THEMOONMONSTERAddress).tokensOfOwner(msg.sender);
        uint arrayLength = punks.length;
    ...
    function reservedMoonsss(address to, uint256 amount) public onlyOwner {
         require(claimed + amount <= MAX_CLAIM, "Value exceeds the max limit");
         _mint(to, amount);
         claimed += amount;
    }

    function unclaimedMoonsss() public view onlyOwner returns(uint256) {
         uint256 unclaimed = MAX_CLAIM - claimed;
         return unclaimed;
    }

    function punkMoonsss(uint punkid) public view returns(uint256) {
         uint256 rewards = 0;
         uint256 time = block.timestamp;
         uint256 lastTime = lastUpdate[punkid];
         if (lastTime > 0){
