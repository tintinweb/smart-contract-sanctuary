...
contract TestNekos is ERC721EnumerableSimple, Ownable {
    // Maximum amount of Nekos in existance. Ever.
    uint public constant MAX_NEKO_SUPPLY = 10000;

    // The provenance hash for all Nekos concatenated will be stored here once all Nekos are issued
    string public METADATA_PROVENANCE_HASH = "";

    // Truth.
    string public constant R = "Inspired by Vegeta, not the one in DBZ, my cat.";

    // Time of when the sale can be switched on
    uint private constant SALE_SWITCH_TIMESTAMP = 1615402800; // 2021-03-10T19:00:00.000Z
    ...
    constructor() ERC721("TestNeko", "TESTNEKO") {}
    ...
    function calculatePriceForToken(uint _id) public pure returns (uint) {
        ...
        if (_id >= 9900) {
            return 0.001 ether;   // 9900-10000: 0.001 ETH
        } else if (_id >= 9500) {
            return 0.00064 ether; // 9500-9500:  0.00064 ETH
        } else if (_id >= 7500) {
            return 0.00032 ether; // 7500-9500:  0.00032 ETH
        } else if (_id >= 3500) {
            return 0.00016 ether; // 3500-7500:  0.00016 ETH
        } else if (_id >= 1500) {
            return 0.00008 ether; // 1500-3500:  0.00008 ETH
        } else if (_id >= 500) {
            return 0.00004 ether; // 500-1500:   0.00004 ETH
        } else {
            return 0.00002 ether; // 0 - 500     0.00002 ETH
        }
    }
    ...
    function adoptNekos(uint numNekos) public payable {
        ...
        require(numNekos > 0 && numNekos <= 20, "You can adopt minimum 1, maximum 20 chubbies");
    ...
    function startSale() public onlyOwner {
        require(block.timestamp >= SALE_SWITCH_TIMESTAMP, "Sale isn't ready to start");
        hasSaleStarted = true;
    }
    ...
    function reserveGiveaway(uint numNekos) public onlyOwner {
        ...
        require(currentSupply + numNekos <= 30, "Exceeded giveaway supply");
...
/**
 * @dev Rewriting ERC721Enumerable for sequential token IDs for gas optimization
 * Specifically, replace the array `_allToken`  with a simple uint `_totalSupply`, then rewrite
 * `totalSupply` and `_beforeTokenTransfer`.
 */
abstract contract ERC721EnumerableSimple is ERC721, IERC721Enumerable {
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0)) {
            // Ensure token is minted sequentially
