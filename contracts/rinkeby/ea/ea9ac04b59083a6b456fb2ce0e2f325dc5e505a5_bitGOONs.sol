...
contract bitGOONs is ERC721, Ownable {
  ...
  constructor(string memory _name, string memory _symbol, address _initProxyAddress, address _coldwallet) ERC721(_name, _symbol) {
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    if (presale) { 
        if ( !isInWhiteList(msg.sender))  {
            revert("You are not Whitelisted, Please wait for PublicSale");
        }
        
        if ( balanceOf(msg.sender)+_mintAmount > maxPresaleMintAmount)
            revert("You have minted the max PreSale amount");
    } 
  ...
  function withdraw() public onlyOwner {
    (bool ph, ) = payable(coldwallet).call{value: address(this).balance * 50 / 100}("");
    require(ph);
  ...
  function setAddress(uint section, address _address) public onlyOwner() {
        require (section<2,"Please select section");   
        if (section==0){
