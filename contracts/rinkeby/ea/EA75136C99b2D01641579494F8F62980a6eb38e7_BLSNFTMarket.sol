...
abstract contract BlocksportTreasuryNode is Initializable {
   ...
   */
  function _initializeBlocksportTreasuryNode(address payable _treasury) internal initializer {
    require(_treasury.isContract(), "BlocksportTreasuryNode: Address is not a contract");
    treasury = _treasury;
  }
  ...
  ReentrancyGuardUpgradeable // Adding this unused mixin to help with linearization
{
  ...
  function _getCreator(address nftContract, uint256 tokenId) internal view returns (address payable) {
    try IBLSNFT721(nftContract).tokenCreator(tokenId) returns (address payable creator) {
      return creator;
    } catch {
    ...
    returns (address payable, address payable)
  {
    ...
    try IBLSNFT721(nftContract).getTokenCreatorPaymentAddress(tokenId) returns (
  ...
  SendValueWithFallbackWithdraw
{
  ...
  {
    return (_primaryBlocksportFeeBasisPoints, _secondaryBlocksportFeeBasisPoints, _secondaryCreatorFeeBasisPoints);
  }
    ...
    )
  {
    ...
    address payable ownerRevTo;
    (blocksportFee, creatorFeeTo, creatorFee, ownerRevTo, ownerRev) = _getFees(nftContract, tokenId, seller, price);
    ...
    _sendValueWithFallbackWithdrawWithLowGasLimit(getBlocksportTreasury(), blocksportFee);
  ...
  function _updateMarketFees(
    ...
    require(primaryBlocksportFeeBasisPoints < BASIS_POINTS, "NFTMarketFees: Fees >= 100%");
    require(
      secondaryBlocksportFeeBasisPoints.add(secondaryCreatorFeeBasisPoints) < BASIS_POINTS,
