...
contract Masks is Context, Ownable, ERC165, IMasks, IERC721Metadata {
    ...
    function isMintedBeforeReveal(uint256 index) public view override returns (bool) {
     ...
     function tokenURI(uint256 _tokenId) external view override returns (string memory) {
        require(_tokenId < totalSupply(), "Not Minted");
    ...
    */
     ...
     function mintOneNFT(string memory metaDataURI) public payable {
        require(totalSupply() < MAX_NFT_SUPPLY, "Sale has already ended");
        require(getNFTPrice() == msg.value, "Ether value sent is not correct");

            uint mintIndex = totalSupply();
            _safeMint(msg.sender, mintIndex);
            _tokenURIs[mintIndex] = metaDataURI;
    ...
    function mintNFT(uint256 numberOfNfts) public payable {
        ...
        for (uint i = 0; i < numberOfNfts; i++) {
           ...
           // if (block.timestamp < REVEAL_TIMESTAMP) {
                _mintedBeforeReveal[mintIndex] = false;
           // }
            _safeMint(msg.sender, mintIndex);
        }
