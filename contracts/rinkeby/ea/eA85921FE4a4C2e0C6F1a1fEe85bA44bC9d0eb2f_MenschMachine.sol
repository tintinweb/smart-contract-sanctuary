...
contract MenschMachine is ERC721, Ownable {
    ...
    function getDrop() external view returns (uint256, uint256, uint256) {
        return (drop.blockNumber, drop.limit, drop.priceInGwei);
    }

    // Sets the drop
    function setDrop(uint256 blockNumber, uint256 limit, uint256 priceInGwei) public onlyOwner {
        drop = Drop(blockNumber, limit, priceInGwei);
    }
    ...
    function buy(uint256 blockNumber) public payable {
        require(msg.value >= drop.priceInGwei);
    ...
    function _baseURI() internal view override virtual returns (string memory) {
        return "https://menschmaschine.io/";
    }
}
