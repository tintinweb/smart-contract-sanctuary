...
contract CoordinationToken is Ownable, BondingCurveToken {
    ...
    uint256 immutable phi; // 1e18 = 100%, 1e16 = 1%, 0 = 0%
    SecurityToken immutable securityToken;
    ...
    /// CoordinationTokenSupply = SecurityTokenSupply ^ 2
    ...
    function bondToMint(uint256 amountToBond, uint256 minimalAmountToReceive) external {
        ...
        if(totalSupply() == 0){
            uint256 result;
            uint8 precision;
            // this formula sets (result >> precision) = (amountToBond/1) ^ (MAX_RESERVE_RATIO/reserveRatio)
            (result, precision)  = power(amountToBond, 1, reserveRatio, MAX_RESERVE_RATIO );
            minted = result >> precision;
        ...
        require(securityToken.transferFrom(msg.sender, address(this), amountToBond),
            "CoordinationToken.bondToMint: Impossible to bond this amount of SecurityTokens");
    ...
    function burnToWithdraw(uint256 amountToWithdraw, uint256 minimalAmountToReceive) external {
        ...
        require(calculateCurvedBurnReturn(amountToWithdraw) >= minimalAmountToReceive,
        ...
        require(securityToken.transfer(msg.sender, burned - currentFee),
            "Failed to transfer SecurityTokens");
        _poolBalance = _poolBalance - burned + currentFee;
    ...
    function getSecurityToken() external view returns(address){
...
contract DelphiaEscrow is Ownable {
    ...
    modifier onlyRDOperator() {
        require(owner() == msg.sender || operators[msg.sender] == true,
            "DelphiaEscrow: Only RD operators can distribute rewards");
        _;
    }
    ...
    */
    function withdraw() external {
        ...
        uint256 balance = withdrawBalances[msg.sender];
        withdrawBalances[msg.sender] = 0;
        require(coordinationToken.transfer(msg.sender, balance),
    ...
    function distribute(Receipt[] memory receipts) external onlyRDOperator{
        require(receipts.length <= 200,
            "DelphiaEscrow.distribute: Can distribute 200 receipts at max");
    ...
    function getWithdrawBalance(address user) external view returns(uint256){
    ...
    function getActiveBalance() external view returns(uint256){
