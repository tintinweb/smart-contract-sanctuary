...
contract ERC721Token is Ownable, ERC721Enumerable {
    ...
    function mintTokenSingle(uint256 _tokenId, uint8 sig_v, bytes32 sig_r, bytes32 sig_s) public {
    ...
    function mintTokenBatch(uint256[] memory _tokenIds, uint8[] memory sig_vs, bytes32[] memory sig_rs, bytes32[] memory sig_ss) public {
        require(_tokenIds.length == sig_vs.length, "invalid sig_vs length");
        require(_tokenIds.length == sig_rs.length, "invalid sig_rs length");
        require(_tokenIds.length == sig_ss.length, "invalid sig_ss length");
        for (uint i = 0; i < _tokenIds.length; i++) {
            string memory msgPacked = keccak256(abi.encode(this, _msgSender(), _tokenIds[i])).toString();
            require(msgPacked.recover(sig_vs[i], sig_rs[i], sig_ss[i]) == mintSigner, "incorrect mint signature");
