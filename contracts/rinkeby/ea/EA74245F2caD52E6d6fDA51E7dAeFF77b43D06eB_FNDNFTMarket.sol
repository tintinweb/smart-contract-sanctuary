  ...
  NFTMarketAuction
{
  ...
  mapping(address => bool) private bannedTokens;
  mapping(address => ReserveAuction) private bannedAuction;
  ...
  event TokenBannedByAdmin(address indexed token, string reason);
  event TokenUnbannedByAdmin(address indexed token);
  ...
  modifier notBanned(address nftContract) {
    require(bannedTokens[nftContract], "NFTMarketReserveAuction: This token banned");
    _;
  }
  /**
  ...
  function createReserveAuction(
    ...
    uint256 tokenId,
    uint256 reservePrice
  ) public onlyValidAuctionConfig(reservePrice) nonReentrant notBanned(nftContract){
  ...
  */
  function adminBanToken(address nftContract, uint256 tokenId, string memory reason) public onlyFoundationAdmin {
      ...
      require(!bannedTokens[nftContract], "Token already banned");
      uint256 auctionId = getReserveAuctionIdFor(nftContract, tokenId);

      if(auctionIdToAuction[auctionId].tokenId == tokenId) {
        bannedAuction[nftContract] = auctionIdToAuction[auctionId];
        _adminCancelReserveAuction(auctionId, reason);
        emit ReserveAuctionBannedByAdmin(auctionId, reason);
      }
        
        bannedTokens[nftContract] = true;
        emit TokenBannedByAdmin(nftContract, reason);
  }
 ...
 function adminUnbanToken(address nftContract) public onlyFoundationAdmin {
    require(!bannedTokens[nftContract], "NFTMarketReserveAuction: Token not banned");

    if(bannedAuction[nftContract].tokenId > 0) {
        ReserveAuction memory auction = bannedAuction[nftContract];
        delete bannedAuction[nftContract];
        createReserveAuction(nftContract, auction.tokenId, auction.amount);
        emit ReserveAuctionUnbannedByAdmin(auction.tokenId);
    } 
    
    delete bannedTokens[nftContract];
    emit TokenUnbannedByAdmin(nftContract);
