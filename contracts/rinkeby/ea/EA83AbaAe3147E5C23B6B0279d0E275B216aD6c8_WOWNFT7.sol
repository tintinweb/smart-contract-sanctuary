...
contract WOWNFT7 is ERC721, ERC721Burnable, Ownable {
    ...
    address private _wow; // wow ,token address
    address private _recipient; // 接受USDTaddress
    address private _usdtAddress;// usdt-contract address 
    uint256 private _globalNext; // 全局盲盒
    uint256 private _wizard_of_wow_token_amount;//  zhuoshushi
    ...
    constructor(
        ...
        address recipient_,
        uint256 globalNext_,
        uint256 wizard_of_wow_token_amount_,
        uint256[] memory ids,
        uint256[] memory from,
        uint256[] memory limits,
        uint256[] memory prices
    ) ERC721("WOW-NFT", "WOW-NFT") {
        require(wow_ != address(0), "wow_ cannot be zero address");
        require(usdtAddress_ != address(0), "wow_ cannot be zero address");
        require(recipient_ != address(0), "recipient_ cannot be zero address");
        _wow = wow_;
        _usdtAddress = usdtAddress_;
        _recipient = recipient_;
        _globalNext = globalNext_;
        _wizard_of_wow_token_amount = wizard_of_wow_token_amount_;
    ...
    */
    ...
    function setWizardOfWowTokenAmount(uint wizard_of_wow_token_amount_) public onlyOwner {
        _wizard_of_wow_token_amount = wizard_of_wow_token_amount_;
    }

    function getWizardOfWowTokenAmount() public view returns (uint256) {
        return _wizard_of_wow_token_amount;
    }
    ...
    function mint(address to, uint256 kindID) public  {
        mint(to, kindID, 1);
    }
    ...
    function mint(
        ...
        // miner 挖100 类型的NFT,不需要支出USDT
        // miner 挖101 ,不需要支付WOW token
        if (msg.sender != owner() && !_miners[msg.sender]) {
            if (kindID == 100 ){
                require(
                    IERC20(_usdtAddress).transferFrom(msg.sender, _recipient, kind.price * 1e18),
                    "fails to transfer usdt token"
                );
            } else if (kindID == 101 ){
                // kindID == 101
                Kind memory kindOfWizard = _kinds[100];
                // kind.next < 
                // 1041 - 1000 + 1 = 41
                if  (kindID == 101 ){
                    uint limit_1_200 = (kindOfWizard.next - kindOfWizard.limit + 1) * 200 ;
                    require(kind.next <= (kind.limit+limit_1_200),"not proper limit_1_200" );
                    // 巫师
                    // 巫师师要给对应的 _wow token 
                    require(IERC20(_wow).transferFrom(address(this), msg.sender, _wizard_of_wow_token_amount * 1e18),
