...
interface Ilink {
    ...
    function getLinkInfo() external view returns(string memory symbol_,address token_, address userA_,address userB_, uint256 amountA_,uint256 amountB_,uint256 percentA_,uint256 totalPlan_,uint256 lockDays_, uint256 startTime_, uint256 status);
...
contract LinkInfo is Enum {
    ...
    function _getLinkInfo() internal view returns(
        ...
        return(symbol, token, userA, userB, amountA, amountB, percentA, totalPlan, lockDays, startTime, uint256(status));
    }
}
