  ...
  PausableUpgradeable
{
  ...
  function collectRewardsBatch(
    ...
    require(
      tokenIds.length == periods.length,
      "tokenIds and periods length is different"
    );
    uint256 rewards = 0;
      ...
      if (period.start == 0) {
        if (i == nextClaimedPeriod) {
          ...
          _stakePeriods[i] = StakePeriod(
            lastClaimed.start +
              ((nextClaimedPeriod - _stakes[tokenId].lastClaimedPeriod) *
                _periodLength),
            lastTotalStaked
          );
  ...
  function transferTreasury(uint256 amount, address to) public {
    require(
