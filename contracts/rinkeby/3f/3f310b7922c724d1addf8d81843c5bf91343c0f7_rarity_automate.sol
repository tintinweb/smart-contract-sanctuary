...
contract rarity_automate {
    ...
    function treat_my_own_summoner() external {
        ...
        bool treatement = adventure || claim_mats || level_up || claim_gold;
        require(treatement == true, 'Thank you! But no treatment needed at the moment!');
    }
    
    function bulk_approve(uint256[] calldata _ids) external payable {
        uint count = 0;
        for (uint i = 0; i < _ids.length; i++) {
