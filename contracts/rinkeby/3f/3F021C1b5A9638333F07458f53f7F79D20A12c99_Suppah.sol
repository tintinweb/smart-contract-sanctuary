...
contract Suppah is ERC721, VRFConsumerBase {
    ...
    mapping(uint256 => Experiences) public tokenIdToExperience;
    mapping(bytes32 => address) public requestIdToSender;
    event requestedExperience(bytes32 indexed requestID, address requester);
    event experienceAssigned(uint256 indexed tokenID, Experiences experience);
    ...
    function createExperience() public returns (bytes32) {
        bytes32 requestID = requestRandomness(keyhash, fee);
        requestIdToSender[requestID] = msg.sender;
        emit requestedExperience(requestID, msg.sender);
    }

    // FUNCTION 2: fulfilling randomness, assigning an experience
    function fulfillRandomness(bytes32 requestID, uint256 randomNumber)
        internal
        override
    {
        Experiences experience = Experiences(randomNumber % 2);
        uint256 newTokenID = tokenCounter;
        tokenIdToExperience[newTokenID] = experience;
        emit experienceAssigned(newTokenID, experience);
