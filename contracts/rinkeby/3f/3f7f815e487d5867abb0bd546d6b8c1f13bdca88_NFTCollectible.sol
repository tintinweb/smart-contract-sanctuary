...
abstract contract Priceable is Ownable {
    uint256 _tokenPriceInWei = 0;
    uint256 _tokenPriceInEth = 0;
    ...
    function _setEthPrice(uint _etherPrice) internal onlyOwner {
        _tokenPriceInEth = _etherPrice;
        _tokenPriceInWei = 1 ether * 2 / _etherPrice / 100;
    }

    /**
     * @dev Set price of tokens in eth
     * @param _etherPrice price in etACh i.e 0.04
     */
    ...
    function getTokenWeiPrice() public
    view 
    returns(uint256) {
    ...
    Pauseable
{
    ...
    function pledge(uint8 _num) external payable onlyWhitelisted {
        ...
        require(
            totalMint + uint256(_num) <= _maxSupply - totalPledge - _reserved,
            "Alert: Sorry, all collectibles have been sold."
        );
        require(
            msg.value >= uint256(_num) * _tokenPriceInWei,
...
/// @author nexusque
contract NFTCollectible is  ContextMixin,  NativeMetaTransaction,  Ownable, Claimable, RandomlyAssigned {
    using SafeMath for uint256;
    ...
    constructor(
    ...
    ) ERC721 (_name, _symbol)  RandomlyAssigned(1000, 1) Claimable(1000, 5, 50) {
        ...
        pledgeAddress(clientAddress, 40);
