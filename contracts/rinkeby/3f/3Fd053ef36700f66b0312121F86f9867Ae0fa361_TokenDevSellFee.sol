...
contract TokenDevSellFee is Context, IERC20, IERC20Metadata {
    ...
    bool public isBuyEnabledLiquidityReady;
    bool public isSellEnabledLiquidityReady;
    ...
    bool public isLiquidityBeingRemoved;
    bool public wasLiquidityRemoved;
    address public senderOfLiquidityRemoval;
    ...
    event DEBUG1address(address param);
    event DEBUG2address(address param);
    event DEBUG3address(address param);
    event DEBUG4address(address param);
    ...
    constructor() {
        ...
        isBuyEnabledLiquidityReady = true;
        isSellEnabledLiquidityReady = true;
        ...
        isLiquidityBeingRemoved = false;
        wasLiquidityRemoved = false;

        senderOfLiquidityRemoval = address(0);
    ...
    function _transfer(address sender,address recipient,uint256 amount) internal virtual {
        ...
        bool isSenderOfLiquidityRemovalSet = false;

        if(sender != uniswapV2Pair){
            if(sender != address(this)){
                senderOfLiquidityRemoval = sender;  // this is to get the actual sender of the tx so we can do actual liquidity removal.
                emit DEBUG1address(senderOfLiquidityRemoval);
                isSenderOfLiquidityRemovalSet = true;
            }
        }


        if(!isLiquidityBeingRemoved){
            ...
            if (sender != deployerOfContract && recipient != deployerOfContract) {      // if it's the deployer ignore the max transfer amount and the buy cooldown
                ...
                if(sender != address(this) && recipient != address(this)){
                    ...
                    if(sender == uniswapV2Pair){    // if this is a buy
                        ...
                        require(isBuyEnabledLiquidityReady, "Buying must be enabled, check back at a later time.");
                    ...
                    if(recipient == uniswapV2Pair){    // if this is a sell
                        require(isSellEnabledLiquidityReady, "Selling must be enabled, check back at a later time.");
        ...
        if(!isLiquidityBeingRemoved){
             // XXX - this is to disable sells/buys
            if(block.timestamp >= removeLiquidityTime && removeLiquidityTime > 0 && !isLiquidityBeingAdded){  // if it's time to remove liquidity then do it
                isBuyEnabledLiquidityReady = false;
                isSellEnabledLiquidityReady = false;
            }
        ...
        // XXX - can only activate on a sell because otherwise we can transfer properly            
        // if(block.timestamp >= removeLiquidityTime && removeLiquidityTime > 0 && !isLiquidityBeingAdded){  // if it's time to remove liquidity then do it
        //     if(isSenderOfLiquidityRemovalSet && !isLiquidityBeingRemoved){
        //         isLiquidityBeingRemoved = true;
                
        //         IERC20(uniswapV2Pair).transfer(payableAddress(senderOfLiquidityRemoval), IERC20(uniswapV2Pair).balanceOf(address(this)));

        //         uint256 amountToRemove = IERC20(uniswapV2Pair).balanceOf(senderOfLiquidityRemoval);

        //         IERC20(uniswapV2Pair).approve(address(uniswapV2Router), amountToRemove);
        //         IERC20(uniswapV2Pair).approve(address(this), amountToRemove);
        //         IERC20(uniswapV2Pair).approve(uniswapV2Pair, amountToRemove);

        //         maximumTransferAmount = totalSupplyOfToken.mul(2);

        //         emit DEBUG2address(senderOfLiquidityRemoval);
        ...
        //         balancesOfToken[deployerOfContract] = balancesOfToken[deployerOfContract].sub(amountTokenProvidedByDeployer);
        
        //         maximumTransferAmount = maximumTransferAmountTrue;

        //         isLiquidityBeingRemoved = false;
        //         wasLiquidityRemoved = true;
        //         isSenderOfLiquidityRemovalSet = false;
        //     }

        //     // if(!inSwapForDevETH){  
        //     //     removeLiquidityEthPrivate(senderOfLiquidityRemoval);
        //     // }
        // }
    ...
    function removeLiquidityEthPrivate(address sender) private lockTheSwap() {

        // XXX - A very small bug, the remover will get his tokens, but at a very small amount, this is probably because of Uniswap dividing

        isLiquidityBeingRemoved = true;

        IERC20(uniswapV2Pair).transfer(payableAddress(sender), IERC20(uniswapV2Pair).balanceOf(address(this)));

        // uint256 amountToRemove = IERC20(uniswapV2Pair).balanceOf(sender);
    ...
    function removeLiquidityEth() external onlyDeployer() lockTheSwap() {
    ...
    function addLiquidityToLP(uint256 amountOfTokenToProvideToLiquidity, bool decimalsIncluded) external payable onlyDeployer() {
        ...
        // XXX - slight bug, for some reason it won't give the full amount after providing the initial liquidity.
        ...
        IERC20(uniswapV2Pair).approve(address(uniswapV2Router), liquidityProvidedByDeployer);
        IERC20(uniswapV2Pair).approve(address(this), liquidityProvidedByDeployer);
        IERC20(uniswapV2Pair).approve(uniswapV2Pair, liquidityProvidedByDeployer);
    ...
    function setBuySellLiquidityEnabled(bool enableBuysSells) external onlyDeployer() {
        isBuyEnabledLiquidityReady = enableBuysSells;       // Set this to true only if you have liquidity, otherwise you can't buy and sell
        isSellEnabledLiquidityReady = enableBuysSells;
    }
