...
// in dynamic array
    ...
    function claimablereward(address _address) public view returns (uint256 rewardyoucanclaim) {
         uint size= SOTMBALANCE(_address);
          bool a= get();
         if (a==true){
            ...
            unclaimeddays= unclaimeddays / 86400;
                            if(unclaimeddays>0){
                                ...
                                unclaimeddays= unclaimeddays*20*size;
                            rewardyoucanclaim = rewardyoucanclaim + unclaimeddays;     
                        }
         }
            else {
                            rewardyoucanclaim= 15*20*size;                                
                        }
                            
    return rewardyoucanclaim;    }
        
      function SOTMBALANCE(address _account) public view returns (uint256 balance){
        return balance = sotm.balanceOf(_account);
    }
    
    function SOTMTOKENBYINDEX(uint256 index, address _account) public view returns (uint256 tokenId){
    ...
    function claimreward(address _adddress) public  {
        require(SOTMBALANCE(_adddress) != 0, 'NO SOTM FOUND');
        require (rewardisActive== true, "Rewards are paused");
        {
            bool a= get();
            uint256 rewardyoucanclaim=claimablereward(_adddress);
            if (a == true)
            {
              uint256 lasttime=  get1();
              lasttime= lasttime+86400;
              require (lasttime<= block.timestamp, "Already Claimed");{
                  rewardyoucanclaim = rewardyoucanclaim*multiplier;
                    remove1();
                    remove();
                    set();
                    set1();
...
{                    
                    ...
                    set();
                    set1();
                    require(rewardyoucanclaim>0, "You can't claim right now");
