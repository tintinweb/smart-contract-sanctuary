...
// File: contracts/IYing.sol

pragma solidity ^0.6.2;


/// @title YING stablecoin interface
/// @author Ying Protocol
interface IYing is IERC20 {
    // ----------- Events -----------
...
interface ICore is IPermissions {
    ...
    event YingUpdate(address indexed _ying);
    event FedUpdate(address indexed _fed);
    event GenesisGroupUpdate(address indexed _genesisGroup);
    event FedAllocation(address indexed _to, uint256 _amount);
    ...
    function setYing(address token) external;

    function setFed(address token) external;

    function setGenesisGroup(address _genesisGroup) external;

    function allocateFed(address to, uint256 amount) external;
    ...
    function ying() external view returns (IYing);

    function fed() external view returns (IERC20);
...
interface ICoreRef {
    ...
    function yingBalance() external view returns (uint256);

    function fedBalance() external view returns (uint256);
}
...
abstract contract CoreRef is ICoreRef, Pausable {
    ...
    modifier onlyYing() {
        require(msg.sender == address(ying()), "CoreRef: Caller is not YING");
        _;
    }
    ...
    function ying() public view override returns (IYing) {
        return _core.ying();
    }

    /// @notice address of the Tribe contract referenced by Core
    /// @return IERC20 implementation address
    function fed() public view override returns (IERC20) {
        return _core.fed();
    }

    /// @notice fei balance of contract
    /// @return fei amount held
    function yingBalance() public view override returns (uint256) {
        return ying().balanceOf(address(this));
    }

    /// @notice tribe balance of contract
    /// @return tribe amount held
    function fedBalance() public view override returns (uint256) {
        return fed().balanceOf(address(this));
    }

    function _burnYingHeld() internal {
        ying().burn(yingBalance());
    }

    function _mintYing(uint256 amount) internal {
        ying().mint(address(this), amount);
    }
}

// File: contracts/Ying.sol
...
constructor(address core) public ERC20("Ying USD", "YING") CoreRef(core) {
uint256 chainId;
// solhint-disable-next-line no-inline-assembly
assembly {
chainId := chainid()
...
string public constant name = "Fed";
...
// solhint-disable-next-line const-name-snakecase
uint public totalSupply = 1_000_000e18; // 1 million Fed
...
require(msg.sender == minter, "Fed: only the minter can change the minter address");
...
require(msg.sender == minter, "Fed: only the minter can mint");
require(dst != address(0), "Fed: cannot transfer to the zero address");

// mint the amount
uint96 amount = safe96(rawAmount, "Fed: amount exceeds 96 bits");
uint96 safeSupply = safe96(totalSupply, "Fed: totalSupply exceeds 96 bits");
totalSupply = add96(safeSupply, amount, "Fed: totalSupply exceeds 96 bits");

// transfer the amount to the recipient
balances[dst] = add96(balances[dst], amount, "Fed: transfer amount overflows");
...
amount = safe96(rawAmount, "Fed: amount exceeds 96 bits");
}
...
require(signatory != address(0), "Fed: invalid signature");
require(signatory == owner, "Fed: unauthorized");
// solhint-disable-next-line not-rely-on-time
require(block.timestamp <= deadline, "Fed: signature expired");
...
uint96 newAllowance = sub96(spenderAllowance, amount, "Fed: transfer amount exceeds spender allowance");
...
/// @notice the address of the YING contract
IYing public override ying;

/// @notice the address of the FED contract
IERC20 public override fed;
...
Ying _ying = new Ying(address(this));
_setYing(address(_ying));

Fed _fed = new Fed(address(this), msg.sender);
_setFed(address(_fed));
}

/// @notice sets Fei address to a new address
/// @param token new fei address
function setYing(address token) external override onlyGovernor {
_setYing(token);
}

/// @notice sets Tribe address to a new address
/// @param token new tribe address
function setFed(address token) external override onlyGovernor {
_setFed(token);
}
...
/// @notice sends FED tokens from treasury to an address
/// @param to the address to send TRIBE to
/// @param amount the amount of TRIBE to send
function allocateFed(address to, uint256 amount)
external
override
onlyGovernor
{
IERC20 _fed = fed;
require(
_fed.balanceOf(address(this)) >= amount,
"Core: Not enough Fed"
);

_fed.transfer(to, amount);

emit FedAllocation(to, amount);
}
...
function _setYing(address token) internal {
ying = IYing(token);
emit YingUpdate(token);
}

function _setFed(address token) internal {
fed = IERC20(token);
emit FedUpdate(token);
}
}