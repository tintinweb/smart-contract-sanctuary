...
contract PerlerPunks is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("PerlerPunks", "PRLP") {
        // TODO update
        cryptoPunksAssets = CryptoPunksAssets(0xc271100C2e6379C9A526Fdb4f63a0E4212f5CddF);
    ...
    function setMintPrice(uint256 priceInWei, uint8 maxPerTransaction) external onlyOwner unsealed {
    ...
    function attributesByIndex(uint16 index) external view returns (string memory text) {
        ...
        text = string(abi.encodePacked(text, ", CryptoPunk #", Strings.toString(ogPunks[index])));
    }

    function cryptoPunkImageByIndex(uint16 punkIndex) external view returns (string memory svg) {
        require(punkIndex < MAX_COUNT);
        svg = punkAssetsImageSvg(parseAssets(cryptoPunksData.punkAttributes(punkIndex)));
    }
    ...
    function mintPunk() public payable {
        ...
        punks[clownIndex] = parseAssets(cryptoPunksData.punkAttributes(punkIndex));
        ogPunks[clownIndex] = punkIndex;

        _mint(msg.sender, clownIndex);
    }
    
    function mintPunks(uint8 numPunks) external payable {
        require(numPunks <= MAX_MINT_PER_TRANSACTION);
   ...
   function parseAssets(string memory attributes) internal view returns (bytes memory punkAssets) {
    ...
    function nextIndex() internal returns (uint16) {
        ...
        uint16 previous;
        uint16 next;
        for (uint8 step = 0; step < stepCount; ++stepCount) {
            if (backwards) {
    ...
    string private constant SVG_HEADER = '<svg id="crisp" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMax meet" viewBox="0 0 350 350"><rect x="0" y="0" width="350" height="350" fill="#008080"/>';
        ...
        for (uint y = 0; y < 24; y++) {
            for (uint x = 0; x < 24; x++) {
                ...
                if (uint8(pixels[p + 3]) > 0) {
                    ...
                    svg = string(abi.encodePacked(svg, rectSvg(((y % 2) == 0 ? 17 : 10) + (14 * x), 37 + (12 * y), color)));
                }
            }
        }
    ...
    function rectSvg(uint x, uint y, bytes4 color) internal pure returns (string memory) {
        ...
        return string(abi.encodePacked(
            '<polygon points="', polygonPoints(x, y, false), ',', polygonPoints(x, y, true), '" fill="#', string(buffer), '"/>'));
    }
