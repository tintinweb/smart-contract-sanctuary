...
contract samoDAOTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, sSAMOD }
    ...
    address public sSAMOD;
    uint public sSAMODQueue; // Delays change to sSAMOD address
    ...
    /**
        @notice allow approved address to deposit an asset for samoDAO
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint samoDAO needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( samoDAO ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn samoDAO for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IsamoDAOERC20( samoDAO ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sSAMOD ).balanceOf( msg.sender ); // Can only borrow against sSAMOD held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with samoDAO
        @param _amount uint
     */
    function repayDebtWithsamoDAO( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IsamoDAOERC20( samoDAO ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, samoDAO, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( samoDAO ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( samoDAO ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns samoDAO valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match samoDAO decimals
            value_ = _amount.mul( 10 ** IERC20( samoDAO ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.sSAMOD ) { // 9
            sSAMODQueue = block.number.add( blocksNeededForQueue );
        } else return false;
