...
contract ERC20FixedSupply is ERC20, AccessControl {
  ...
  constructor(
    ...
    if (keccak256(abi.encodePacked(types)) == keccak256('Mintable')) {
  ...
  modifier checkMintable(string memory types) {
      if (keccak256(abi.encodePacked(types)) == keccak256('Mintable')) {
         _;
      }
   }
  ...
  // Mintable
  function mint(address to, uint256 amount) checkMintable(types) public virtual {
    require(
  ...
  function cap() public view virtual checkMintable(types) returns (uint256) {
    return _cap;
  }

  function _mint(address account, uint256 amount) checkMintable(types) internal virtual override {
