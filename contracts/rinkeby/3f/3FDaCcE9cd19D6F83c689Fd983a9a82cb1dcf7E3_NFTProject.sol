...
contract NFTProject is ERC721Enumerable, Ownable{
    ...
    constructor() ERC721("9021 Collective","9021"){
    }

    function set9021ContractAddress(address contractAddress) public onlyOwner {
    ...
    function bet(uint256 useTokenId1) public {
        require(NFTContract != address(0), "9021 contract address need be set");
        ...
        Collective collective = Collective(NFTContract);
        bool result = collective.bet9021(from);
        require(result, "Collective compose failed");
    }

    function compose(uint256 useTokenId1, uint256 useTokenId2, uint256 useTokenId3) public {
        require(NFTContract != address(0), "Collective contract address need be set");
        ...
        Collective collective = Collective(NFTContract);
        bool result = collective.compose9021(from);
        require(result, "Collective compose failed");
    }
...
interface Collective {
    function compose9021(address owner) external returns (bool);
    function bet9021(address owner) external returns (bool);
}
