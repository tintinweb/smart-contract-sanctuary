...
contract Moonpet is Initializable, ERC721HolderUpgradeable, OwnableUpgradeable {
    ...
    function claimMiningRewards(uint256 nftId) external isAllowed(nftId) {
        ...
        require(isPetAlive(nftId), "Your pet is dead, you can't mine");
        require(
    ...
    function buyAccesory(uint256 nftId, uint256 itemId) external {
        require(isPetAlive(nftId), "ded pet");
        ...
        token.mint(address(this), (amount * 90) / 100);
    ...
    function bonk(uint256 _victim, uint256 _attacker)
        ...
        isAllowed(_attacker)
    {
        require(timeUntilStarving[_attacker] >= 4 days, "!time");
        require(timeUntilStarving[_victim] <= 2 days, "!time");

        // only bonking allowed when gas price is cheap?
        require(tx.gasprice <= 20 * 10**9, "!gas high");
        ...
        if (chanceOfAttack > 80) {
            ...
            //the attacker win 81% of time
        } else {
            // the defender win 19%
            // IERC721Upgradeable(_pet.nft).ownerOf(_pet.id); //todo check fix this
            // token.mint(msg.sender, reward);
        }

        // emit Bonk(_attacker, _victim, reward, msg.sender);
    }
    ...
    function fatality(uint256 _deadId, uint256 _tokenId) external {
        //TODO CHECK THIS
        require(
            !isPetAlive(_deadId) && tx.gasprice <= gas * (10**9), //inspired by NFT GAS by 0Xmons
            "The PET has to be starved or gas below ${gas} to claim his points"
        );

        petScore[_tokenId] =
            petScore[_tokenId] +
            (((petScore[_deadId] * (60)) / (100)));

        petScore[_deadId] = 0;

        delete petDetails[_deadId]; //tODO check this not sure
    ...
    function totalPets() external returns (uint256) {
        ...
        )
    {
        ...
        _pet = _nftId;
        _isAlive = this.isPetAlive(_nftId);
    ...
    function hasBeenUsed(address _nftAddress, uint256 _nftId)
        public
        view
        returns (bool)
    {
        return (isNftInTheGame[_nftAddress][_nftId]);
    }
