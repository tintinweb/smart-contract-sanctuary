...
library Address {
     ...
     * but also tran0x3d079b51EA706c9a7A40bc62e9CBF836060984CdSolidity function must be `payable`.
     *
     * _Available since v3.1._
     */
...
contract newOwn is Ownable {

    address potentialOwner;

    event OwnerNominated(address indexed from);
    event OwnerChanged(address indexed from,address newowner);

    function nominatePotentialOwner(address owner) external onlyOwner {
    potentialOwner = owner;
    emit OwnerNominated(owner);
    }
...
contract ChipitToken is Context, IBEP20, newOwn {
