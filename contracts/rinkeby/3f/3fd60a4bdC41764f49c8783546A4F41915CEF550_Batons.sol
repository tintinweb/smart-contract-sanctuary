    ...
    AccessControl
{
    ...
    // TODO(Woojong): Replace `TODO_INSERT_CID` with an actual CID.
    // TODO(Woojong): Test if OpenSea can take IPFS URLs for contract-level
    // metadata. If not, switch to `https://ipfs.io/ipfs/...`.
    string private constant CONTRACT_METADATA_URI =
        "ipfs://TODO_INSERT_CID/contract-metadata.json";

    // Create a new role identifier for the treasurer role.
    bytes32 public constant TREASURER_ROLE = keccak256("TREASURER_ROLE");

    // TODO set these with the right numbers
    uint256 public constant BRONZE_UPGRADE = 0.01 ether;
    uint256 public constant SILVER_UPGRADE = 0.02 ether;
    uint256 public constant GOLD_UPGRADE = 0.03 ether;
    uint256 public constant CHARITY_SALES_PCT = 55; // 55% of paymentsplitter is towards orgs
     ...
     */
    function _mintToCharityGroup(uint256 _charityGroup, address _recipient)
        internal
        returns (uint256)
    {
        require(
            nextMintIdx[_charityGroup] < BATONS_PER_GROUP,
            "Charity Group is sold out"
        );
        uint256 offsetId = nextMintIdx[_charityGroup]++;
        uint256 tokenId = _charityGroup * BATONS_PER_GROUP + offsetId;
        _safeMint(_recipient, tokenId);
        return tokenId;
    }

    /**
     * free reserved batons for handful of advisors (~70ish)
     */
    function mintAdvisor(
        ...
        for (uint256 i = 0; i < _amountToMint; i++) {
            _mintToCharityGroup(_charityGroup, _advisor);
        }
    }

    /**
     * Supporters will have the opportunity to mint two batons
     * during the presale. Once the public sale is live, they'd
     * just have to mint like everyone else.
     * @dev note a user may not be on both lists
     */
    function mintEarlybird(
        ...
        require(sent, "Payment failed");
        for (uint256 i = 0; i < _amountToMint; i++) {
            _mintToCharityGroup(_charityGroup, _supporter);
        }
    }
        ...
        onlyMintableGroup(_charityGroup)
    {
        ...
        require(sent, "Payment failed");
        for (uint256 i = 0; i < _amountToMint; i++) {
            _mintToCharityGroup(_charityGroup, msg.sender);
        }
    }
    ...
    function startPresale() external onlyAdmin {
        ...
        // TODO we may check if earlybirdMerkleRoot != 0 as well.
    ...
    string private baseURI = ""; // an ipfs CID ending in a slash
     ...
     * Upgrades can come from project-wide charitable revenues,
     * specific baton charitable revenues, and from your "team" - ie the
     * other batons that map to the same charity payout
     */
        ...
        returns (uint256 tokenId)
    {
        ...
        tokenId = _mintToCharityGroup(CHARITY_GROUPS, msg.sender);
    ...
    /* TODO: withdrawCoin(address) onlyAdmin*/
}
