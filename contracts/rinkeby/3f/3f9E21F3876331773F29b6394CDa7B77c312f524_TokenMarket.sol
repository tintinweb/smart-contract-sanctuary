    ...
    */
     function createLoanOfferToken(TokenLoanData.LoanDetails memory loanDetails)
        ...
        for(uint256 index =  0 ; index < loanDetails.stakedCollateralAmounts.length ; index ++){
            collatetralInBorrowed+=  this.getAltCoinPriceinStable(loanDetails.borrowStableCoin, loanDetails.stakedCollateralTokens[index], loanDetails.stakedCollateralAmounts[index]);
        }
    ...
    function tokenLoanOfferAdjusted(
        ...
        for(uint256 index =  0 ; index < loanOffersToken[_loanIdAdjusted].stakedCollateralAmounts.length ; index ++){
            collatetralInBorrowed+=  this.getAltCoinPriceinStable(loanOffersToken[_loanIdAdjusted].borrowStableCoin, loanOffersToken[_loanIdAdjusted].stakedCollateralTokens[index], loanOffersToken[_loanIdAdjusted].stakedCollateralAmounts[index]);
        }
        ...
        ) external view override returns(uint256) 
    {
        ...
        if(govPriceConsumerV3.isChainlinFeedEnabled(_altCoin) && govPriceConsumerV3.isChainlinFeedEnabled(_stableCoin)) {
               
                (int collateralChainlinkUsd, uint8 atlCoinDecimals) = govPriceConsumerV3.getLatestUsdPriceFromChainlink(_altCoin);
                uint256 collateralUsd = (uint256(collateralChainlinkUsd) * _collateralAmount).div(atlCoinDecimals); 
                (int priceFromChainLinkinStable, uint8 stableDecimals) = govPriceConsumerV3.getLatestUsdPriceFromChainlink(_stableCoin);
                return collateralAmountinStable.add(collateralUsd.div(uint256(priceFromChainLinkinStable)).mul(stableDecimals));
                
        }
        else {
                return collateralAmountinStable.add(govPriceConsumerV3.getDexTokenPrice(uniswapDexFactory, _stableCoin, _altCoin, _collateralAmount));
            } 
