...
contract Zombies is ERC721URIStorage, Ownable {
    ...
    event MintZombies (address indexed minter, uint256 startWith, uint256 times);
   
    
    uint256 public totalZombies;
    ...
    function mintZombies(uint256 _times) payable public mintEnabled {
       
        require(_times >0 && _times <= maxBatch, "mint wrong number");
        require(totalZombies + _times <= totalCount, "too much");
        require(msg.value == _times * price, "value error");
        payable(owner()).transfer(msg.value);
        emit MintZombies(_msgSender(), totalZombies+1, _times);
        for(uint256 i=0; i< _times; i++){
            _mint(_msgSender(), 1 + totalZombies++);
        }
    } 
    ...
    function adminMintGiveaways(address _addr) public onlyOwner {
        require(totalZombies + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit MintZombies(_addr, totalZombies+1, 1);
        _mint(_addr, 1 + totalZombies++);
    } 
