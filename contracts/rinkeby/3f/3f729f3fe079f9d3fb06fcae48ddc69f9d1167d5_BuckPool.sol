...
contract BUCKStablecoin is ERC20Custom {
    ...
    function refreshCollateralRatio() public {
        ...
        if(global_collateral_ratio < effectiveCollateralRatio()){
            ...
            // if collateral is sufficient and withdraw it from treasury to Pool
            uint256 recollat_possible = (global_collateral_ratio.mul(totalSupply()).sub(totalSupply().mul(effectiveCollateralRatio()))).div(1e6);
...
contract BuckPool {
    ...
    function mintSingleFractionalBUCK(uint256 collateral_amount, uint256 bevy_amount, uint256 BUCK_out_min) external notMintPaused {
        ...
        if(collateral_amount > 0){
            ...
            require(collateral_amount >= collateral_units_precision.add(collateralFractionAmount),"Collateral amount is sufficient");
    
            collateral_token.transferFrom(msg.sender, address(this), collateral_units_precision.add(collateralFractionAmount));
            ...
            require(treasury.getCollateralSupply() >= collateralFractionAmount.add(collateral_units_precision), "Collateral in treasury is sufficient");
            
            treasury.withdraw(collateralFractionAmount.add(collateral_units_precision));
        }
    ...
    function bevyToCollateral(uint256 _bevyAmount) public view returns(uint256 collateral_precision){
        ...
        (uint256 collateral_equivalent_d18) = (BuckPoolLibrary.calcSingleBEVY(input_params, treasury.getCollateralSupply())).div(1e6);
