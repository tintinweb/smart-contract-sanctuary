...
import "../interfaces/IBuilderData.sol";
...
contract Builder is IBuilderData{
    address public _strategy_address;
    address private _owner;
    mapping (address => uint) public _user_deposits;
    address public _prize_pool_address;
    PrizePool private _prize_Pool_Instance;

    modifier isOwner{
        require(msg.sender == _owner, 'Not authorized to call function');
        _;
    }

    constructor(address strategy_address)public{
        _strategy_address = strategy_address;
    }

    function setPrizePoolAddress(address prize_pool_address) public{
        _prize_pool_address = prize_pool_address;
        _prize_Pool_Instance = PrizePool(prize_pool_address);
    }
    ...
    function initializePool(CompoundPrizePoolConfig memory poolConfig, MultipleWinnersBuilder.MultipleWinnersConfig memory prizeStrategy, uint8 decimals) public{
        IBuilder poolInstance = IBuilder(_strategy_address);
        poolInstance.createCompoundMultipleWinners(poolConfig,prizeStrategy, decimals);
        _owner = msg.sender;
    }

    function deposit(address to, uint256 amount, address controlledToken, address referrer, address prizePoolAddress, address tokenAddress) public{
        IERC20Upgradeable(tokenAddress).transferFrom(msg.sender, address(this), amount);
        IERC20Upgradeable(tokenAddress).approve(prizePoolAddress, amount);
        // PrizePool poolInstance = PrizePool(_prize_pool_address);
        _prize_Pool_Instance.depositTo(to, amount, controlledToken, referrer);
        _user_deposits[msg.sender] = amount;
    }

    function calculateEarlyExitFee(address from, address controlledToken, uint256 amount)public returns (uint256 exitFee, uint256 burnedCredit){
        // PrizePool poolInstance = PrizePool(_prize_pool_address);
        _prize_Pool_Instance.calculateEarlyExitFee(from, controlledToken, amount);
    }

    function withdrawInstantlyFrom(address from, uint256 amount, address controlledToken, uint256 maximumExitFee) public returns (uint256 exitFee){
        // PrizePool poolInstance = PrizePool(_prize_pool_address);
        _prize_Pool_Instance.withdrawInstantlyFrom(from, amount, controlledToken, maximumExitFee);
    }
}
...
import "@pooltogether/pooltogether-contracts/contracts/external/compound/CTokenInterface.sol";
import "@pooltogether/yield-source-interface/contracts/IYieldSource.sol";
import "./IBuilderData.sol";

interface IBuilder is IBuilderData{
    // 0xB852Bb662B692146BEC334A9E91C8e2e6e64FCdc

    function createCompoundMultipleWinners( CompoundPrizePoolConfig memory prizePoolConfig, MultipleWinnersBuilder.MultipleWinnersConfig memory prizeStrategyConfig, uint8 decimals ) external returns (CompoundPrizePool) ; 
...
contract Pool{
    ...
    function deposit(address to,uint256 amount,address controlledToken,address referrer) public{
        // IPool poolInstance = IPool(_poolAddress);
        // poolInstance.depositTo(to, amount, controlledToken, referrer);

        _poolAddress.delegatecall(abi.encodeWithSignature("depositTo(address to,uint256 amount,address controlledToken,address referrer)", to, amount, controlledToken, referrer));
    }
}
...
import "@pooltogether/pooltogether-contracts/contracts/token/TokenListenerInterface.sol";
import "@pooltogether/pooltogether-contracts/contracts/token/ControlledTokenInterface.sol";
