...
/** @notice PolkaBridgeFarm this contract is in charge of Stability Dao
 *  Staking and Farming. Stake LP tokens to earn ETH.
...
contract PolkaBridgeFarm is Ownable, ReentrancyGuard {
    ...
    constructor(
        address _lpToken, // IERC20 _lpToken,
        address _polkaBridge,// PolkaBridge _polkaBridge,
        address _devaddr,
        uint256 _PBRPerBlock,
        uint256 _startBlock
    ) {
        polkaBridge = PolkaBridge(_polkaBridge);
        devaddr = _devaddr;
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 blocks = block.number.sub(pool.lastRewardBlock);
        uint256 PBRReward = blocks.mul(PBRPerBlock).mul(pool.allocPoint).div(
            totalAllocPoint
        );
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending = user
                .amount
                .mul(pool.accPBRPerShare)
                .div(1e18)
                .sub(user.rewardDebt);
            if (pending > 0) {
                safePBRTransfer(_pid, msg.sender, pending);
            }
        }
        if (_amount > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accPBRPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safePBRTransfer(_pid, msg.sender, pending);
    ...
    // Stake LP tokens or Other tokens to Pool
    function stake(uint256 _pid, uint256 _amount) public {
        ...
        polkaBridge.mint(msg.sender, _amount);
        emit Deposit(msg.sender, 0, _amount);
    }

    // Withdraw LP tokens or Other staked tokens from STAKING.
    function unstake(uint256 _pid, uint256 _amount) public {
        ...
        polkaBridge.burn(msg.sender, _amount);
    ...
    function safePBRTransfer(
        ...
        polkaBridge.safePBRTransfer(_pid, _to, _amount);
    }
