...
contract RevestPrivateSale is ERC1155, AccessControlEnumerable, Ownable {
    ...
    uint256 public constant UNIT_PRICE = 1E18; //Unit price in Wei – 1 ETH
    uint256 public constant SALE_CAP = 25E20;
    uint256 public constant TOTAL_UNITS = 2500;
    uint256 public constant PER_CAP = 500;
    ...
    address private negative_entropy;
    ...
    *
    */
    constructor(address _negative) ERC1155("https://token-cdn-domain/{id}.json") Ownable() 
    {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(PAUSER_ROLE, _msgSender());

        eth_remaining = SALE_CAP; 
        burner = address(0);
        enableTransfers = false;
        whitelistEnabled = true;
        whitelist[_msgSender()] = true;
        negative_entropy = _negative;
    }
    ...
    receive() external payable {
        //Calculate quantity
        require(eth_remaining > 0, 'The private sale has been completed');
        require(eth_remaining - msg.value >= 0, 'The requested transaction would exceed the private sale cap');
        require(msg.value >= UNIT_PRICE, 'Message lacks adequate value for transaction to succeed');
        require(msg.value >= floor, 'Must be below floor price');
        ...
        if(whitelistEnabled) {
            if(!whitelist[_msgSender()]) {
                IERC721Enumerable entropy = IERC721Enumerable(negative_entropy);
                ...
                if(nft_balance > 0) {
                    ...
                    require(ticketSales[_msgSender()] + msg.value  < ticketCap);
                    //Mark ticket used
                    usedTickets[entropy.tokenOfOwnerByIndex(_msgSender(), 0)] = true;
    ...
    function _beforeTokenTransfer(
        ...
        override {
            ...
            require(from == address(0) || enableTransfers, 'Transfers of this NFT are currently disabled; please check back later');
         }
