...
contract BoredChadsNew is ERC721, Ownable {

  string _name = "Bored Chad";
  string _symbol = "BORCA";
  ...
  uint256 public mintLimitPartners = 3800;
  ...
  //if value == 1, 1 free mint ==> Giga chad
  //if value == 3, 1 free mint == > OG chad
  //if value == 2, discounted mint == > OG Chad

  //partner tokens
  IERC20 private tokenMICE = IERC20(0x9281dd9D11E34126bBbA331112B58660203C3254); // anonymice testnet
  IERC20 private tokenBAYC = IERC20(0xf63476B447aB6610429d3C9d4d0f447716A316c8); // Bored Ape testnet
  IERC20 private tokenKONGZ = IERC20(0x7EA3Cca10668B8346aeC0bf1844A49e995527c8B); // KONGZ
  IERC20 private tokenMAYC = IERC20(0x60E4d786628Fea6478F785A6d7e704777c86a7c6); // Mutant
  IERC20 private tokenHabibz = IERC20(0x98a0227E99E7AF0f1f0D51746211a245c3B859c2); // Habibz

  event Mint(uint256 index, address creator);

  constructor() ERC721(_name, _symbol) {
    setHiddenMetadataUri("ipfs://QmZN6EgTdkdxuQUxRg3Bp9DFK837KuipxktJxj33tcpLXG/hidden.json");
  }
  ...
  function mintChads(uint256 _mintQuantity) public payable mintCompliance(_mintQuantity) {
    if(supply.current() <= 4 ) {
        //free public mint
        _mintChads(msg.sender, _mintQuantity);
    } else if(whitelistRoles[msg.sender] == 1 || whitelistRoles[msg.sender] == 3) {
        _mintForGiga(1);
    } else if(whitelistRoles[msg.sender] == 2) {
        _mintForOG(_mintQuantity);
    }else if( mintLimitPartners > 0 && (tokenMICE.balanceOf(msg.sender) > 0 || tokenBAYC.balanceOf(msg.sender) > 0 
        || tokenMAYC.balanceOf(msg.sender) > 0 || tokenKONGZ.balanceOf(msg.sender) > 0
        || tokenHabibz.balanceOf(msg.sender) > 0 )) {
  ...
  // function mintForRoles(uint256 _mintQuantity) public payable mintCompliance(_mintQuantity) {
  //   if( mintLimitPartners > 0 && (tokenMICE.balanceOf(msg.sender) > 0 || tokenBAYC.balanceOf(msg.sender) > 0 
  //       || tokenMAYC.balanceOf(msg.sender) > 0 || tokenKONGZ.balanceOf(msg.sender) > 0
  //       || tokenHabibz.balanceOf(msg.sender) > 0 )) {
  ...
  function _mintForPartner(uint256 _mintQuantity) internal {
    ...
    require( tokenMICE.balanceOf(msg.sender) > 0 || tokenBAYC.balanceOf(msg.sender) > 0 
        || tokenMAYC.balanceOf(msg.sender) > 0 || tokenKONGZ.balanceOf(msg.sender) > 0
        || tokenHabibz.balanceOf(msg.sender) > 0 
        , "not partner");
  ...
  function _mintForOG(uint256 _mintQuantity) internal {
    require(whitelistRoles[msg.sender] == 2, "not whitelisted for OG");
        //discounted OG chad mint at costPartners
    require(msg.value >= costPartners * _mintQuantity, "Insufficient funds sent!");
    _mintChads(msg.sender, _mintQuantity);
  }

  function _mintForGiga(uint256 _mintQuantity) internal {
  ...
  function whitelistForRoles(address[] memory _users, uint256[] memory _roleCounts) external onlyOwner {
    ...
    // function mintDiscounted(uint256 _mintQuantity) public payable mintCompliance(_mintAmount) {
  //   require(whitelistRoles[msg.sender] >0, "not whitelisted");
  //   if(whitelistRoles[msg.sender] == 1 || whitelistRoles[msg.sender] == 3) {
  //       require(_mintQuantity == 1, "Invalid mint amount!");
  //       _mintChads(msg.sender, 1);
  //       whitelistRoles[msg.sender] = whitelistRoles[msg.sender] - 1;
  //   } else if(whitelistRoles[msg.sender] == 2) {
  //       //discounted OG chad mint at 50%
  //       require(msg.value >= (_cost/2) * _mintQuantity, "Insufficient funds sent!");
  //       _mintChads(msg.sender, _mintQuantity);
  //   }
  // }
