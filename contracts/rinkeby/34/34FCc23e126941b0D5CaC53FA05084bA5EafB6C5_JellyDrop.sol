...
contract JellyDrop is IJellyContract, Documents {
    ...
    /// Admin fees for the JellyFactory.
    ...
    function setRewardsToken(address _addr) external {
        ...
        emit RewardsTokenUpdated(rewardsToken, _addr);
        rewardsToken = _addr;
    }
    ...
    function addRewards(uint256 _rewardAmount) public {
        require(accessControls.hasAdminRole(msg.sender)); // Optional. think through this
        ...
        uint256 jellyFee = tokensAdded * uint256(feePercentage) / PERCENTAGE_PRECISION;
     ...
     * @param _claimExpiry Timestamp when tokens are no longer claimable. For non expiry, set to 0.
     * @param _tokensClaimable Bool to determine if the airdrop is initially claimable.
     */
    function setJellyCustom(uint256 _startTimestamp, uint256 _streamDuration,  uint256 _claimExpiry, bool _tokensClaimable) public  {
        require(accessControls.hasAdminRole(msg.sender), "setJelly: Sender must be admin");
        require(_startTimestamp < 10000000000, "setJelly: enter start unix timestamp in seconds, not miliseconds");
        require(_claimExpiry < 10000000000, "setJelly: enter claim expiry unix timestamp in seconds, not miliseconds");
        require((_startTimestamp < _claimExpiry && _claimExpiry > block.timestamp )|| _claimExpiry == 0, "setJelly: claim expiry incorrect");
        ...
        rewardInfo.claimExpiry = BoringMath.to48(_claimExpiry);
    ...
    function setJellyAirdrop() external  {
        setJellyCustom(block.timestamp, 0, 0, false);
    }
    ...
    function setJellyAirdrip(uint256 _streamDuration) external  {
        setJellyCustom(block.timestamp, _streamDuration, 0, false);
    }
    ...
    function _earnedAmount(
        ...
        if (block.timestamp <= uint256(_rewardInfo.startTimestamp)) {
            return 0;
        }
    ...
    function adminReclaimTokens(address _vault) external {
        ...
        uint256 tokenAmount =  IERC20(rewardsToken).balanceOf(address(this));
        rewardsPaid += tokenAmount;
        rewardInfo.tokensClaimable = false;
        OZIERC20(rewardsToken).safeTransfer(_vault, tokenAmount);
    }
