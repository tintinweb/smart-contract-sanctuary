...
/**
 * @dev Collection of functions related to the address type   Address.sol
 */  
library Address {
...
contract ExchangeCore is ReentrancyGuarded, Ownable {
        ...
        return
            atomicMatch(
                ...
                Order(
                    ...
                    addrs[11],
                    HowToCall(feeMethodsSidesKindsHowToCalls[7]),
                    calldataSell,
    ...
    function atomicMatch(
        ...
        uint256 nftID = uint256(sell.extra);
        address nftaddress = address(sell.target);
        /* Must be matchable. */
        bool rel = ordersCanMatch(buy, sell);
        ...
        emit TestMe(nftaddress,sell.maker,buy.maker, nftID);
        ERC721(nftaddress).safeTransferFrom(sell.maker, buy.maker, nftID);
    ...
    function chargeProtocolFee(
        ...
        ERC20(exchangeToken).transferFrom(from, to, amount);
    }
    ...
    /**
     * @dev Return whether or not two orders can be matched with each other by basic parameters (does not check order signatures / calldata1 or perform static calls)
     * @param buy Buy-side order
     * @param sell Sell-side order
        ...
        returns (uint256)
    {
            ...
            if (sell.feeMethod == FeeMethod.SplitFee) {
                ...
                if (sell.makerRelayerFee > 0) {
                    ...
                    if (sell.paymentToken == address(0)) {
                        ...
                        (bool sent, bytes memory data) = sell.feeRecipient.call{
        ...
        if (sell.paymentToken == address(0)) {
            (bool sent, bytes memory data) = sell.maker.call{value: msg.value}(
                ""
            );

            // sell.maker.send(msg.value);
            // sell.maker.transfer(receiveAmount);
            /* Allow overshoot for variable-price auctions, refund difference. */
            uint256 diff = msg.value;//SafeMath.sub(msg.value, requiredAmount);
            if (diff > 0) {
                // buy.maker.transfer(diff);
                (bool sent, bytes memory data) = buy.maker.call{value: diff}(
                    ""
                );
            }
        }
