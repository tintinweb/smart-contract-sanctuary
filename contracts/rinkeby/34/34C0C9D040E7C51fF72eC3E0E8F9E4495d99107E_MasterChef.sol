...
// File: contracts/bakery/MasterChef.sol

pragma solidity ^0.6.12;

interface IMigratorChef {
    // Perform LP token migration from legacy exchange.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // Migrator must have allowance access to DOUGH LP tokens.
    // EasyBake must mint EXACTLY the same amount of EasyBake LP tokens.
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardTime; // Last time that OVEN distribution occurs.
        uint256 accOvenPerShare; // Accumulated OVEN per share, times 1e12. See below.
    }
    ...
    // Number of blockchains with MasterChef deployed.
    uint256 public chains = 1;
    // OVEN tokens created per day.
    ...
    // The time when OVEN mining begins.
    ...
    function pendingOven(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 ovenReward =
                multiplier.mul(ovenPerSecond).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accOvenPerShare = accOvenPerShare.add(
    ...
    function updatePool(uint256 _pid) public validatePoolByPid(_pid) {
        ...
        uint256 adminFee = ovenReward.mul(1000).div(10650);
        uint256 netReward = ovenReward.sub(adminFee.mul(2));

        oven.mint(team, adminFee); // 1.50 OVEN per block to team (9.375%)
        oven.mint(treasury, adminFee); // 1.50 OVEN per block to treasury (9.375%)

        oven.mint(address(sugar), netReward);
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant validatePoolByPid(_pid) {
        ...
        if (user.amount > 0) {
            ...
            uint256 pending =
                user.amount.mul(pool.accOvenPerShare).div(1e12).sub(
                    user.rewardDebt
                );
            if (pending > 0) {
