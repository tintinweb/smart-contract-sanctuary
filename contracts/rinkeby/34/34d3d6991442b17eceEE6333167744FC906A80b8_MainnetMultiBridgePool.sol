...
contract MainnetMultiBridgePool is AccessControl, ReentrancyGuard {
  ...
  function _getUniV3TokenPrice(address _token) 
  ...
  function _getUniV2TokenPrice(address _token) 
    internal view returns (uint256) {
  ...
  function getFee(address _token, uint256 _amount) 
    public view returns (uint256) {
      ...
      uint256 _tokenToEth = _getUniV3TokenPrice(_token);
      if (_tokenToEth == 0) {
        _tokenToEth = _getUniV2TokenPrice(_token);
        if (_tokenToEth == 0) {
          _tokenToEth = info.tokenToEth;
        }
      }

      return info.fee.add(_amount.div(_tokenToEth).mul(info.percentFee).div(10 ** info.percentFeeDecimals)); 
  }
