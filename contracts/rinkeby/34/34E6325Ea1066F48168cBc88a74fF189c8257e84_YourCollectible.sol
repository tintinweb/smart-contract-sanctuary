...
library MetadataGenerator {
    ...
    function tokenURI(
        ...
        string memory name = string(
            abi.encodePacked("Example SVG ", tokenId.toString())
        );
        string memory description = string(
            abi.encodePacked(
                "this is an example svg with the color: ",
                color.toColor()
            )
        );
        string memory image = Base64.encode(
            bytes(generateSVGofTokenById(owner, tokenId, color, chubbiness))
        );
...
// File @openzeppelin/contracts/utils/Count[email protected]
