...
contract VectorField is ERC721URIStorage, ERC721Enumerable, ReentrancyGuard, VRFConsumerBase, Ownable {
    ...
    ERC721("Vector Field", "VCFD")
    {
        ...
        height = 2000;
        width = 1500;
        colors = ["#e7d2cc", "#b9b7bd", "#868b8e", "#eeede7", "#647c90", "#970c10"];
    }
 ...
 function _beforeTokenTransfer(address from, address to, uint256 tokenId)
   ...
   function withdraw() external onlyOwner {
     ...
     function Claim(uint256 numTokens) external payable virtual nonReentrant {
        require(price * numTokens == msg.value, 'ETH amount is incorrect');
        require(numTokens <= MAX_PER_TX, 'Claim fewer Fields');
        require(balanceOf(_msgSender()) + numTokens <= MAX_PER_ADDRESS, 'Maximum number of Fields claimed');

        _claim(numTokens);
    }
   
    function _claim(uint256 numTokens) private returns (bytes32 requestId) {
        
        require(totalSupply() < MAX_SUPPLY, 'All Fields have been minted');
        require(totalSupply() + numTokens <= MAX_SUPPLY, 'Minting exceeds maximum supply');
        require(numTokens > 0, 'You must mint at least one Field');
    ...
    function Mint(uint256 tokenId) public {
        ...
        require(tokensMinted > tokenId, "TokenId has not been minted yet!");
    ...
    function generateSVG(uint256 _randomness) public view returns (string memory finalSvg) {
        ...
        uint256 bgopacity = uint256(keccak256(abi.encode(_randomness + 1))) % 60;
       ...
       return string(
                abi.encodePacked(
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name":"Field #',tokensMinted,
                        
                                '", "description":"Vector fields randomly generated and stored immutably on the Ethereum blockchain", "attributes":"", "image":"',imageURI,'"}'
                            )
                        )
                    )
                )
