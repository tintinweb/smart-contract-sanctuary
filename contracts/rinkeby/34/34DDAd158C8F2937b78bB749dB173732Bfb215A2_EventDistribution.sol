...
contract EventDistribution is AccessControl {
  ...
  function withdrawByArtist(address _nft) external onlyIfEventClosedAndArtist(_nft) {
    uint256 _artistEthBalance = artistEthBalance[_nft];
    uint256 _artistPointBalance = artistPointBalance[_nft];
    uint256 _artistDropsBalance = artistDropsBalance[_nft];
    uint256 _artistNdrBalance = artistNdrBalance[_nft];
    
    uint256 zero = 0;
    artistEthBalance[_nft] = zero;
    artistPointBalance[_nft] = zero;
    artistDropsBalance[_nft] = zero;
    artistNdrBalance[_nft] = zero;

    (bool success, ) = _msgSender().call{ value: _artistEthBalance }("");
    require(success);
    
    point.safeTransfer(_msgSender(), _artistPointBalance);
    drops.safeTransfer(_msgSender(), _artistDropsBalance);
    ndr.safeTransfer(_msgSender(), _artistNdrBalance);

    emit WithdrawnByArtist(_nft, _msgSender(), _artistEthBalance, _artistPointBalance, _artistDropsBalance, _artistNdrBalance);
  }
