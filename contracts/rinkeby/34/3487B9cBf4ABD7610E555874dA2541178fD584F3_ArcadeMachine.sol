...
contract ArcadeMachine is DisplayURISwitchable, ERC721Enumerable, ReentrancyGuard, ERC2981, Ownable {
    ...
    constructor(address arcadeOfficialAddress) ERC721("ArcMa", "ARC-MA") {
    ...
    modifier inTheReserveRange(uint256 tokenId) {
        require(
    ...
    function _mintReserved(
        ...
        private
        inTheReserveRange(arcadeTokenId)
    {
        require(_hasLength(imageURI), "All images URIs are required.");
        require(arcade.ownerOf(arcadeTokenId) == targetAddress, "Target address must be the Arcade ID owner.");
        ...
        _safeMint(targetAddress, arcadeTokenId);
        totalOriginal += 1;
    }
        ...
        private
    {
        ...
        require(arcadeTokenIds.length == imageURIs.length, "Input arrays must have the same length.");

        for(uint256 i = 0; i < arcadeTokenIds.length; i++) {
            uint256 arcadeTokenId = arcadeTokenIds[i];
            string memory imageURI = imageURIs[i];
            _mintReserved(targetAddress, arcadeTokenId, imageURI);
        }
    }
    ...
    function mintReserved(
        ...
        onlyOperator
    {
        _mintReserved(msg.sender, arcadeTokenIds, imageURIs);
    }

    function devMintReserved(
        address targetAddress,
        uint256[] calldata arcadeTokenIds,
        string[] calldata imageURIs
    )
        ...
        onlyOperator
    {
        _mintReserved(targetAddress, arcadeTokenIds, imageURIs);
    }
    ...
    function listArcadesOwned()
        external
        view
        returns (uint256[] memory tokenIds)
    {
        address owner = msg.sender;
        uint256 balance = arcade.balanceOf(owner);

        if (balance == 0) return new uint256[](0);

        tokenIds = new uint256[](balance);
        for(uint256 i = 0; i < balance; i++) {
            tokenIds[i] = arcade.tokenOfOwnerByIndex(owner, i);
        }
        return tokenIds;
    }

    function listArcadeMachinesOwned()
        external
        view
        returns (uint256[] memory tokenIds)
    {
