...
contract CnM is ICnM, ERC721Enumerable, Ownable, Pausable {
    ...
    event CatStolen(uint256 indexed tokenId);
    event CrazyCatLadyStolen(uint256 indexed tokenId);
    event MouseStolen(uint256 indexed tokenId);
        ...
        if (tx.origin != recipient && recipient != address(tower)) {
            ...
            if (tokenTraits[minted].isCat) {
                if (tokenTraits[minted].isCrazy) {
                    emit CrazyCatLadyStolen(minted);
                } else {
                    emit CatStolen(minted);
                }

            }
            else {
                emit MouseStolen(minted);
            }
        }
