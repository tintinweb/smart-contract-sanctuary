...
contract Generic10k is ERC721, Ownable, PaymentSplitter, VRFConsumerBase {
  ...
  mapping(bytes32 => uint256) private _randomnessRequestIds;
    ...
    )
  {
    ...
    LINK_KEY_HASH = 0x2ed0feb3e7fd2022120aa84fab1945545a9f2ffc9076fd6156fa96eaff4c1311;
  ...
  function mint(uint256 amount) public payable {
    ...
    require(Counters.current(_totalSupply) + amount < MAX_SUPPLY, "Exceeds maximum number of tokens");
    require(msg.value >= MINT_PRICE * amount, "Invalid Ether amount sent");
    require(_saleActive, "Sale is not active");

    for (uint256 i = 0; i < amount; i++) {
      _safeMint(msg.sender, Counters.current(_totalSupply));
      _setGenes(Counters.current(_totalSupply));
      Counters.increment(_totalSupply);
    }
  }

  function _setGenes(uint256 tokenId) internal {
    require(LINK.balanceOf(address(this)) >= LINK_FEE, "Not enough LINK");
    _randomnessRequestIds[requestRandomness(LINK_KEY_HASH, LINK_FEE)] = tokenId;
  }

  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    _genes[_randomnessRequestIds[requestId]] = randomness;
  }

  function genes(uint256 tokenId) public view returns (uint256) {
