...
contract Monster is ERC721 {
    ...
    uint8 constant TOTAL = 30;
    uint8 constant POINT = 9;

    mapping(uint => string) public monster;
    mapping(uint => string) public prefix;
    mapping(uint => uint8) public suffix;

    mapping(uint => uint8) public health_Point;
    mapping(uint => uint8) public physical_damage_point;
    mapping(uint => uint8) public magical_damage_point;
    mapping(uint => uint8) public physical_defence;
    mapping(uint => uint8) public magical_defence;
    mapping(uint => uint8) public dodge;
    mapping(uint => uint8) public hit; 
    mapping(uint => uint8) public critical;
    mapping(uint => uint8) public parry;
    ...
    function mintMonster() private{
        ...
        // uint8[] memory divide_points = new uint8[](POINT-1);
        // uint8 p;
        ...
        // uint8[9] memory basePoints = get_base_points(professions[rand % professions.length]);
    ...
    function get_base_points(uint8 _suffix) public pure returns(uint8[9] memory){
