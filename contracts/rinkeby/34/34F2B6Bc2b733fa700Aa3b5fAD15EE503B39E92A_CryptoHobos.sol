...
contract CryptoHobos is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 public currentTokenPrice = 50000000000000000; //0.05E
    ...
    function mintReservedHobo(address to, uint256 count) public onlyOwner {
        if (to == address(0)) {
            to = msg.sender;
        }
        require(count > 0
    ...
    function reduceSupply(uint256 supply) public onlyOwner {
        require(supply < maxTokens && supply > lastTokenId);
        maxTokens = supply;
    }
}