...
// File: node_modules/openzeppelin-solidity/contracts/token/ERC721/extensions/IERC721Metadata.sol
...
// File: node_modules/openzeppelin-solidity/contracts/token/ERC721/extensions/IERC721Enumerable.sol
...
contract TheImages is ERC721Enumerable, Ownable {
    ...
    Counters.Counter private _publicImages;

    constructor() ERC721("TheImages", "IMG") {
    }
    ...
    function reserveImages(address _to, uint256 _numberOfTokens) external onlyOwner {
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(_publicImages.current() < MAX_SUPPLY, "Purchase would exceed DRAGONS_PUBLIC");
        require(PRICE * numberOfTokens <= msg.value, "ETH amount is not sufficient");

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicImages.current();

            if (_publicImages.current() < MAX_SUPPLY) {
                _publicImages.increment();
