...
contract Pronto is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Pronto", "PRONTO", maxBatchSize_, collectionSize_) {
  ...
  function preSaleMint(uint256 quantity, bytes32[] memory proof) external payable {
    ...
    if (MerkleProof.verify(proof, saleConfig.merkleRoot, keccak256(abi.encodePacked(msg.sender)))) {
