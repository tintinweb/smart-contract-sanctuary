...
contract MedusaToken is ERC721, ERC721Enumerable, Ownable, PaymentSplitter, Pausable, VRFConsumerBase {
    ...
    // a merkle tree that stores addresses with permission to  early minting
    ...
    // a merkle tree that stroes addresses with permission to free mint
    ...
    function _payAndMintNFTs(uint amount) internal {
        require(_price * amount == msg.value, "Medusa: too little eth sent");
        _mintNFTs(amount);
    }

    function _mintNFTs(uint amount) internal whenNotPaused {
    ...
    function devMintNFTs(uint amount) public {
    ...
    function _mintNFT(address _to) internal {
      ...
      if (block.timestamp < REVEAL_TIME) {
          _mintedBeforeReveal[_tokenId] = true;
      }
    ...
    function fulfillRandomness(bytes32, uint256 randomNumber) internal override {
        startingIndex = randomNumber % _tokenCount;
    }
        ...
        returns (string memory)
    {
        ...
        if (startingIndex == 0) {
            return "ipfs://QmWdaX1iJRRn3yeHmn2MJDkoHn7MTNXYkK1f2aY6i5NLvv";
        }
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "ipfs://QmaUV41x5wNBkKQh1Q87rXDLiPPK6RvhH22pjujzZuSHRN/";
    }
