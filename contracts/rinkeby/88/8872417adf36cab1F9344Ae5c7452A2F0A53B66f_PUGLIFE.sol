...
contract PUGLIFE is Context, IERC20, IERC20Metadata, Ownable {
    ...
    constructor (address _marketing_address, address _charity_address) {
        ...
        charity_address = _charity_address;
    ...
    function setRewardFee(uint256 taxFee) external onlyOwner {
        _rewardFee = taxFee;
    }
    ...
    function setMarketingAddress(address _marketing_address) external onlyOwner {
        require(_marketing_address != address(0), "Address cant be zero address");
        marketing_address = _marketing_address;
    }

    function setCharityAddress(address _charity_address) external onlyOwner {
        require(_charity_address != address(0), "Address cant be zero address");
        charity_address = _charity_address;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = amount.sub(tFee).sub(tLiquidity).sub(tBurn).sub(tMarketing).sub(tCharity);
        return (tTransferAmount, tFee, tLiquidity, tBurn, tMarketing, tCharity);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tMarketing, uint256 tCharity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 amount = rAmount;
        uint256 rTransferAmount = amount.sub(rFee).sub(rLiquidity).sub(rBurn).sub(rMarketing).sub(rCharity);
    ...
    function _takeMarketingFee(uint256 tMarketing) private {
        ...
        if(_isExcluded[marketing_address])
            _tOwned[marketing_address] = _tOwned[marketing_address].add(tMarketing);
    }
    ...
    function _takeCharityFee(uint256 tCharity) private {
        ...
        if(_isExcluded[charity_address])
            _tOwned[charity_address] = _tOwned[charity_address].add(rCharity);
    }
    ...
    function removeAllFee() private {
        if(_rewardFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _marketingFee == 0 && _charityFee == 0) return;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tBurn, tMarketing, tCharity, _getRate());
