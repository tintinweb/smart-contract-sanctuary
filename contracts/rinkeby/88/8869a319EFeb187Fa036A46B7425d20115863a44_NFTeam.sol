...
/**
 * @title NFTeam
 * NFTeam - a contract for NFTeam semi-fungible tokens.
 */
contract NFTeam is ERC1155Tradable {
  ...
  ERC1155Tradable(
    ...
    _setBaseMetadataURI("https://nfteam-web.vercel.app/api/card/");
  }

  function contractURI() public view returns (string memory) {
    return "https://nfteam-web.vercel.app/api/contract";
  }
  ...
  function tokenURI(uint256 tokenId) public view returns (string memory) {
    ...
    output = string(abi.encodePacked(output, 
      parts[13], parts[14], parts[15], parts[16], parts[17],
      parts[18], parts[19], parts[20], parts[21], parts[22],
      parts[23], parts[24], parts[25]
    ));

    string memory json = Base64.encode(string(abi.encodePacked('{"name": "NFTeam #', toString(tokenId), '", "description": "The NFTeam Collection by NFTeam for NFTeam by NFTeam generated and stored on chain. Stats, images, and other functionality are intentionally omitted for others to interpret. Feel free to use the NFTeam Collection by NFTeam for NFTeam by NFTeam in any way you want.", "image": "data:image/svg+xml;base64,', Base64.encode(output), '"}')));
