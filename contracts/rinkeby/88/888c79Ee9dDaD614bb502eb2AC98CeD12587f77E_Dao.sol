...
contract Dao is Initializable, OwnableUpgradeable, IDao {
    ...
    mapping(address => TokenOracelInfo) public tokenOracle;
    ...
    function setCoinDeedFactory(address _coinDeedFactory) public onlyOwner {
        coinDeedFactory = ICoinDeedFactory(_coinDeedFactory);
    }
    ...
    modifier onlyDeed() {
        require(coinDeedFactory.isDeed(msg.sender), "CoinDeedDAO: Caller is not a Deed");
        _;
    }
    ...
    function claimCoinDeedManagementFee(
        address _tokenAddress,
        uint256 _amount
    ) external payable override onlyDeed returns (uint256) {
    ...
    function getCoinDeedManagementFee(
        address _tokenAddress,
        uint256 _amount
    ...
    function sendTokenToAddress(address _tokenAddress, address _to, uint256 _amount) public onlyOwner {
        if(_tokenAddress == address(0)) {
            (bool sent, ) = _to.call{value: _amount}(""); // send ETH
            require(sent, "Coindeed: Transfer failed");
        }
        else {
            require(IERC20(_tokenAddress).balanceOf(address(this)) >= _amount, "Coindeed: Transfer amount exceed balance");
            IERC20(_tokenAddress).safeTransfer(_to, _amount); //send token
        }
    }
}
