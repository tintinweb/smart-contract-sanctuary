...
contract Hourglass {
    ...
    modifier antiEarlyWhale(uint256 _amountOfBomb){
        address _customerAddress = msg.sender;
        
        // are we still in the vulnerable phase?
        // if so, enact anti early whale protocol 
        if( onlyAmbassadors && ((totalBombBalance() - _amountOfBomb) <= ambassadorQuota_ )){
            require(
                // is the customer in the ambassador list?
                ambassadors_[_customerAddress] == true &&
                
                // does the customer purchase exceed the max ambassador quota?
                (ambassadorAccumulatedQuota_[_customerAddress] + _amountOfBomb) <= ambassadorMaxPurchase_
                
            );
            
            // updated the accumulated quota    
            ambassadorAccumulatedQuota_[_customerAddress] = SafeMath.add(ambassadorAccumulatedQuota_[_customerAddress], _amountOfBomb);
    ...
    // the bomb token contract
    IERC20 public BOMBTOKEN;
    ...
    */
    function Hourglass(
        IERC20 _BOMBtokenAddress
    )
        public
    {
        // set the bomb token address
        BOMBTOKEN = _BOMBtokenAddress; 
    ...
    /**
     * Converts all incoming bomb to tokens for the caller, and passes down the referral addy (if any)
     */
    function buy(address _referredBy, uint _bombAmount)
        public
        payable
        returns(uint256)
    {
        // check if tokens are approved to the contract
        require(BOMBTOKEN.allowance(msg.sender, address(this)) >= _bombAmount, 'please approve your BOMB tokens first');
        BOMBTOKEN.transferFrom(msg.sender, address(this), _bombAmount);

        purchaseTokens(_bombAmount, _referredBy);
    }
    ...
    /**
     * Method to view the current Bomb stored in the contract
     * Example: totalBombBalance()
     */
    function totalBombBalance()
        public
        view
        returns(uint)
    {
        return BOMBTOKEN.balanceOf(this);
    }
