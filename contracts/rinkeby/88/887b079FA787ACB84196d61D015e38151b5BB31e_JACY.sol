...
contract JACY is Context, IERC20, Ownable {
    ...
    function setFees(uint256 taxFee_, uint256 marketingFee_, uint256 devFeePercentage_, uint256 liquidityFee_,
        uint256 distSharePercentage, uint256 SwapThresholdAmount_) external onlyOwner {
        taxFee = taxFee_;
        marketingFeePercentageage = marketingFee_;
    ...
    function _transfer(
        ...
        if(from != uniswapV2Pair && from != owner() ) {
            require(!botWallets[from] , "bots are not allowed to sell");
            require(!botWallets[to] , "bots are not allowed to sell");
        }
