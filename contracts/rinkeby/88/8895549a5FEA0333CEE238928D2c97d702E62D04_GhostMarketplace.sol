...
contract GhostMarketplace is Ownable {
  ...
  constructor() {
      ghostsContract = IERC721(0xa06Eee6426Ff69695D3ab4ab1eC8f93592030Ab3);
  }
  ...
  modifier ghostOnSale(uint ghostID) {
    require(priceOf[ghostID]!=0);
    _;
  }

  function getAllGhostsOnSale() external view returns(uint[] memory){
  ...
  function purchaseGhost(uint ghostID) external payable ghostOnSale(ghostID) reentrancyGuard {
    ...
    require(ghostsContract.safeTransferFrom(beneficiery,msg.sender,ghostID));
