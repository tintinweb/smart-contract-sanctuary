...
contract Exchange is ExchangeCore {
    ...
    function setArtistPercent(uint256 _artistPercent) public onlyOwner {
        setArtistPercent_(_artistPercent);
    }
...
contract ExchangeCore is ReentrancyGuarded, EIP712, Ownable {
    ...
    bytes32 constant ORDER_TYPEHASH =
        keccak256(
            "Order(address maker,address target,address token,address artist,uint256 price,uint256 tokenId,uint256 listingTime,uint256 expirationTime,uint256 salt)"
        );
    ...
    function setArtistPercent_(uint256 _artistPercent) internal onlyOwner {
    ...
    {
        uint256 artistMul = SafeMath.mul(_price, artistPercent);
        artistShare = SafeMath.div(artistMul, 10**18);

        uint256 treasuryMul = SafeMath.mul(_price, treasuryPercent);
        treasuryShare = SafeMath.div(treasuryMul, 10**18);

        sellerShare = _price - (treasuryShare + artistShare);

        return (artistShare, treasuryShare, sellerShare);
    }
    ...
    function atomicMatch_(
        ...
        IERC721 collection = IERC721(seller.target);
        ...
        require(
            collection.balanceOf(seller.maker) > 0,
        ...
        (
            ...
            uint256 treasuryShare,
            uint256 sellerShare
        ) = calculateShareAmount_(seller.price);

        if (seller.token == address(0)) {
            require(
                buyer.maker.balance >= seller.price,
            ...
            (success, ) = payable(seller.artist).call{value: artistShare}("");
            require(
                success,
                "Revert: There was a problem sending ether to artist"
            );
        } else {
            ...
            token_.transferFrom(buyer.maker, seller.artist, artistShare);
        }

        collection.safeTransferFrom(seller.maker, buyer.maker, seller.tokenId);
