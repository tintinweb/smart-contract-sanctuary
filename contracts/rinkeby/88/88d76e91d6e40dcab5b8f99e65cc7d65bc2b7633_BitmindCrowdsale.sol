...
contract BitmindCrowdsale {
    ...
    function Purchase(uint _amount) public {
        ...
        require(tokenAddress.balanceOf(address(this)) > 0 && tokenAddress.balanceOf(address(this)) >= _amount, "BitmindMsg: INSUFFICIENT BMD");
        require(pairAddress.balanceOf(address(this)) >= _amount, "BitmindMsg: Amount higher than Your Balance");
        
        require(pairAddress.approve(msg.sender, _amount) , 'BitmindMsg: Approve Failed');
