...
contract CartelPunks is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress = payable(0x4Ce7CF51a9Ba9aEDEd85BcA786eF27d03149595D);
    address payable private _DevAddress;

    uint256 public constant CTPUNK_MAX = 10000;
    uint256 public PURCHASE_LIMIT = 20;
    uint256 public PRICE = 10_000_000_000_000_000; // 0.01 ETH
    ...
    Counters.Counter private _publicCTPUNK;

    constructor() ERC721("CartelPunks", "CP") {
    ...
    function gift(address to, uint256 numberOfTokens) external onlyOwner {
        require(
            _publicCTPUNK.current() < CTPUNK_MAX,
            "Purchase would exceed CTPUNK_MAX"
        );

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicCTPUNK.current();

            if (_publicCTPUNK.current() < CTPUNK_MAX) {
                _publicCTPUNK.increment();
                _safeMint(to, tokenId);
            }
        }
    }
