...
// Dependency file: contracts/TokenInfo.sol
...
// Root file: contracts/LPStaking.sol
...
contract LPStaking is ReentrancyGuard, Ownable {
    ...
    event SetDailyOutput(uint256 indexed dailyOutput);
    ...
    mapping(uint256 => bool) public poolIdUsed;
    mapping(uint256 => mapping(address => UserInfo)) public userInfos;

    constructor(address token_) public {
        token = TokenInfo(token_);
    }
    ...
    function setDailyOutput(uint256 dailyOutput_) external onlyOwner {
        dailyOutput = dailyOutput_;
        emit SetDailyOutput(dailyOutput);
    }

    modifier verifyPid(uint256 pid) {
        require(poolIdUsed[pid], "pool does not exist");
        _;
    }

    modifier updateAllPool() {
    ...
    modifier updatePool(PoolInfo storage poolInfo) {
        if (poolInfo.weight > 0) {
            ...
            if (
                poolInfo.totalAmount > 0 &&
                block.timestamp > poolInfo.lastBlockTimeStamp
            ) {
                uint256 pendingShare = poolInfo.reward.mul(1e18).div(
                    poolInfo.totalAmount
                );
                poolInfo.rewardPerShare = poolInfo.rewardPerShare.add(
                    pendingShare
                );
    ...
    modifier updateUserReward(PoolInfo storage poolInfo, address user) {
        ...
        if (userInfo.amount > 0) {
            uint256 debt = userInfo.amount.mul(poolInfo.rewardPerShare).div(
                1e18
            );
    ...
    {
        return userInfos[pid][userAddress];
    }

    function currentAllPoolRewardShare()
        public
        view
        virtual
        returns (uint256 _reward, uint256 _perShare)
    {
        ...
        uint256 preAmount = dailyOutput.mul(time).div(1 days);
        _reward = token.preMint(preAmount);
        ...
        updateUserReward(poolInfos[pid], msg.sender)
    {
        ...
        if (amount > 0) {
            ...
            userInfo.debt = userInfo.amount.mul(poolInfo.rewardPerShare).div(
                1e18
            );
            poolInfo.totalAmount = poolInfo.totalAmount.add(amount);
            address(poolInfo.token).safeTransferFrom(
                msg.sender,
                address(this),
                amount
            );
    ...
    function calculateIncome(uint256 pid, address userAddress)
        external
        view
        virtual
        verifyPid(pid)
        returns (uint256)
    {
        ...
        uint256 poolPendingReward = poolInfo
            .weight
            .mul(_perShare)
            .div(1e18)
            .sub(poolInfo.debt);
        ...
        if (
            ...
            uint256 poolPendingShare = _reward.mul(1e18).div(
                poolInfo.totalAmount
            );
            _perShare = _perShare.add(poolPendingShare);
        }
        uint256 userReward = userInfo.amount.mul(_perShare).div(1e18).sub(
            userInfo.debt
        );
        return userInfo.reward.add(userReward);
    }

    function addPoolInfo(address tokenAddr, uint256 weight)
        external
        virtual
        onlyOwner
        updateAllPool
    {
        require(tokenUsed[tokenAddr] == false, "!Token Used");
        uint256 pid = poolInfos.length;
        uint256 debt = weight.mul(rewardPerShare).div(1e18);
        poolInfos.push(
            PoolInfo(pid, tokenAddr, weight, 0, 0, block.timestamp, debt, 0)
        );
    ...
    function updatePoolInfo(uint256 pid, uint256 weight)
        external
        virtual
        onlyOwner
        verifyPid(pid)
