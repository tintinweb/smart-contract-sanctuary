...
contract WYVERNSBREATH is Context, IERC20, Ownable {
    ...
    //todo : write a setter for maxSupply 10000000
  ...
  */
   ...
   */
    ...
    function setLastUpdate (uint[] memory tokenIds, uint[] memory lastUpdatedDateValues) external onlyOwner {
        require(tokenIds.length == lastUpdatedDateValues.length, "Inconsistent values sent" );
        for (uint i = 0; i < tokenIds.length; i++) {
            uint tokenId = tokenIds[i];
            lastUpdate[tokenId] = lastUpdatedDateValues[i];
        }
    }

    function mint ( uint _amount) external onlyOwner {
        require (maxSupply >= _totalSupply + _amount,"Minting Limit Reached");
