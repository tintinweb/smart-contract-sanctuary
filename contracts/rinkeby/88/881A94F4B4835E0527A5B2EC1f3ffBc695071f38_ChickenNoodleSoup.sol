    ...
    PausableUpgradeable
{
    ...
    function updateBatch(uint16 start, uint16 max) external onlyProxyAdmin {
        for(uint16 tokenId = start; tokenId <= max; tokenId++) {
            ...
            if (available) {
                ...
                chickenNoodle.finalize(tokenId, t, address(0x0));
            }
        }
    }
