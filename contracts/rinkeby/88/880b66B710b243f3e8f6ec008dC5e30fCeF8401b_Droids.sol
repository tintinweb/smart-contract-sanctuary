...
interface Functions {
  function addToPrivate(address[] calldata addresses) external;

  function onPrivate(address addr) external returns (bool);

  function removeFromPrivate(address[] calldata addresses) external;
  
  function PrivateClaimedBy(address owner) external returns (uint256);

  function mintPrivate(uint256 numberOfTokens) external payable;
  ...
  function PrivateActive(bool isPrivateActive) external;
...
contract Droids is ERC721Enumerable, Ownable, Functions, Metadata {
  using Strings for uint256;

  uint256 public constant RESERVE = 2222;
  uint256 public constant PUBLIC = 5555;
  uint256 public constant NFT_MAX = RESERVE + PUBLIC;
  uint256 public constant PURCHASE_LIMIT = 15;
  uint256 public constant PRICE = 0.1 ether;

  bool public isPrivateActive = false;
  ...
  mapping(address => uint256) private _PrivateClaimed;
 
  string private _contractURI = '';
  string private _tokenBaseURI = '';
  string private _tokenRevealedBaseURI = '';

  constructor() ERC721("Droid", "DROID") {}


//Private
  function addToPrivate(address[] calldata addresses) external override onlyOwner {
    for (uint256 i = 0; i < addresses.length; i++) {
      require(addresses[i] != address(0), "Can't add the null address");
      _Private[addresses[i]] = true;
      _PrivateClaimed[addresses[i]] > 0 ? _PrivateClaimed[addresses[i]] : 0;
    }
  }
  
  function onPrivate(address addr) external view override returns (bool) {
    return _Private[addr];
  }
  
  function removeFromPrivate(address[] calldata addresses) external override onlyOwner {
    for (uint256 i = 0; i < addresses.length; i++) {
      require(addresses[i] != address(0), "Can't add the null address");
      _Private[addresses[i]] = false;
    }
  }
  
  function PrivateClaimedBy(address owner) external view override returns (uint256){
    require(owner != address(0), 'Zero address');
    return _PrivateClaimed[owner];
  }


  function mintPrivate(uint256 numberOfTokens) external override payable {


    require(isMasterActive, 'Contract is not active');
    require(isPrivateActive, 'Private is not active');
    require(_Private[msg.sender]);
    require(totalSupply() < NFT_MAX, 'All tokens have been minted');
    require(numberOfTokens <= PrivateMaxMint, 'Cannot purchase this many tokens');
    require(totalPublicSupply + numberOfTokens <= PUBLIC, 'Purchase would exceed max supply');
    require(_PrivateClaimed[msg.sender] + numberOfTokens <= PrivateMaxMint, 'Purchase exceeds max allowed');
...
function mintPublic(uint256 numberOfTokens) external override payable {
  ...
  function PrivateActive(bool _isPrivateActive) external override onlyOwner {
    isPrivateActive = _isPrivateActive;
  }
