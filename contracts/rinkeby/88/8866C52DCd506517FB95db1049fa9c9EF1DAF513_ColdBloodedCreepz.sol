 ...
 * {ERC721Enumerable}.
 */
contract ColdBloodedCreepz is Context, ERC165, IERC721, IERC721Metadata {
    ...
    // Token Supply
    uint public constant _totalSupply = 10900;
    uint public currentSupply = 0;

    // Token Price
    uint256 public constant tokenPrice = 1000000000000000;
    ...
    // Minting Before Reveal

    // TODO SET REVEALDATE
    uint256 public constant revealDate = 1231245923085;
    mapping(uint => bool) private _isMintedBeforeReveal;
    ...
    mapping (address => uint256) public numberOfPurchasedTokens;
    ...
    */
    ...
    function buyTokens(uint tokensNumber) public payable {
        ...
        require((numberOfPurchasedTokens[msg.sender].add(tokensNumber)) <= 10, "cannot buy more then 10 tokens");

        for(uint i = 0; i<tokensNumber; i++) {
            currentSupply++;
            _mint(msg.sender, currentSupply);

            bool isMintedBeforeReveal = block.timestamp < revealDate;
            _isMintedBeforeReveal[currentSupply] = isMintedBeforeReveal;
            numberOfPurchasedTokens[msg.sender] += 1;
        }
        emit tokensMinted(msg.sender, tokensNumber);
    }

    function isTokenMintedBeforeReveal(uint tokenId) public view returns (bool) {
        return _isMintedBeforeReveal[tokenId];
    }
    ...
    function _baseURI() internal view virtual returns (string memory) {
        return "https://cold-blooded-creepz.com/meta/";
    }
