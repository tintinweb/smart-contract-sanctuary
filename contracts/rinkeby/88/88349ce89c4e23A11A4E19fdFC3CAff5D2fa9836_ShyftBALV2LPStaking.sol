...
contract ShyftBALV2LPStaking is Ownable {
  ...
  function updatePairObservation(
    ...
    address pair = UniswapV2Library.pairFor(address(factory), _tokenA, address(usdcToken));
    ...
    if (token0 == _tokenA) {
      ...
      FixedPoint.uq112x112 memory price0 = FixedPoint.uq112x112(uint224((price1Cumulative - observation.price1CumulativeLast) / timeElapsed));
      FixedPoint.uq112x112 memory price1 = FixedPoint.uq112x112(uint224((price0Cumulative - observation.price0CumulativeLast) / timeElapsed));

      observation.price0 = price0.mul(1e8).decode144();
      observation.price1 = price1.mul(1e8).decode144();
      observation.price0CumulativeLast = price1Cumulative;
      observation.price1CumulativeLast = price0Cumulative;
    }
  ...
  function getShyftPrice() public view returns(uint256 shyftPrice) {    
    address pair = UniswapV2Library.pairFor(factory, address(shyftToken), usdcToken);
