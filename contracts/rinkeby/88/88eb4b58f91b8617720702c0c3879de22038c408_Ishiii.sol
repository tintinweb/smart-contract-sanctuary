...
contract Ishiii is ERC721, Ownable {
    ...
    function mintIshihara(uint numberOfTokens) public payable {
     ...
     function changeIshiharaName(uint _tokenId, string memory _name) public {
        require(ownerOf(_tokenId) == msg.sender, "Hey, your wallet doesn't own this ishihara!");
        require(sha256(bytes(_name)) != sha256(bytes(ishiharaNames[_tokenId])), "New name is same as the current one");
        ishiharaNames[_tokenId] = _name;
        
        emit ishiharaNameChange(msg.sender, _tokenId, _name);
