...
contract LitedexInitalOffering is TimeCrowdsale{
    ...
    uint256 private minGetRefId = 1000 * 1e18;
    ...
    function initalize(uint _min, uint _max, uint _cap, uint _price) external onlyOwner returns(bool){
        min = _min;
        max = _max;
        total_cap = _cap;
        price = _price;
    ...
    function purchase(uint256 pid, uint256 amount, bytes3 refId) external onlyWhileOpen returns(bool){
        ...
        if(checkRefId(refId)){
            ...
            if(!binding[_acc][msg.sender].isBind){
                ...
                accounts[_acc].points += 5*1e17;
            }
            
            accounts[_acc].totalReferredUsd += amounts;
            accounts[_acc].points += amounts.div(1000);
        }
