...
contract Auctions is IAuctions, Governed, Time {
    ...
    uint public minBidDelta               = 1.03e18;
    
    uint public minLotSize                = 2500e18;
    
    uint public maxSurplusLotSize         = 20000e18;
    
    uint public maxDeficitLotSize         = 20000e18;
    
    uint public reservesBufferLowerBound  = 0.01e18;
    
    uint public reservesBufferUpperBound  = 0.05e18;

    
    uint64 public maxBatchSize            = 5;
    
    uint64 public extensionPerBid         = 30 minutes;
    
    uint64 public minAuctionDuration      = 24 hours;
    
    uint64 public maxAuctionDuration      = 48 hours;

    
    mapping(uint64 => Auction) public surplusAuction;
    
    mapping(uint64 => Auction) public deficitAuction;

    
    uint64 public surplusAuctionCount;
    
    uint64 public deficitAuctionCount;
    
    uint64 public override latestAuctionCompletionTime;
    
    uint32 public twapDuration = 1 hours;

    
    uint public zhuToBeSold;
    
    uint public zhuToBeBought;
    
    ITCP public tcp;

    
    IUniswapV3Pool public protocolPool;
    ...
    function _initHook() internal override {
        ...
        bytes4[11] memory validUpdates = [
            this.setMinBidDelta.selector,
            this.setMinLotSize.selector,
            this.setMaxSurplusLotSize.selector,
            this.setMaxDeficitLotSize.selector,
            this.setReservesBufferLowerBound.selector,
            this.setReservesBufferUpperBound.selector,
            this.setMaxBatchSize.selector,
            this.setExtensionPerBid.selector,
            this.setMinAuctionDuration.selector,
            this.setMaxAuctionDuration.selector,
    ...
    function completeSetup() external override onlyGovernor {
        protocolPool = governor.protocolPool();
        collateralPool = governor.collateralPool();
    }

    
    
    function bidSurplusAuction(uint64 auctionID, uint128 bid) external lockProtocol notStopped {
        require(surplusAuctionLive(auctionID), 'Auction not live');

        address msgSender = msg.sender;
        Auction memory auction = surplusAuction[auctionID];
        ...
        if (auction.bidder == address(0)) {
            ...
            require(bid > _approximateTCPEquivalent(auction.count) / 10, 'Bid too low');
            burnAmount = bid;
        } else {
            
            require(bid >= auction.bid._mul(minBidDelta), 'Did not out bid');

            
            transferAmount = auction.bid;
            transferTo = auction.bidder;
            burnAmount = bid.sub(auction.bid);
        }

        
        auction.bidder = msgSender;
        auction.bid = bid;
        uint48 endTime = _futureTime(extensionPerBid).toUint48();
        if (auction.endTime < endTime) auction.endTime = endTime;

        
        surplusAuction[auctionID] = auction;

        
        if (transferAmount > 0) TransferHelper.safeTransferFrom(address(tcp), msgSender, transferTo, transferAmount);

        tcp.burnFrom(msgSender, burnAmount);

        emit SurplusAuctionBid(auctionID, msgSender, bid);
    }

    
    function bidDeficitAuction(uint64 auctionID, uint128 bid) external lockProtocol notStopped {
        require(deficitAuctionLive(auctionID), 'Auction not live');

        Auction memory auction = deficitAuction[auctionID];
        address msgSender = msg.sender;
        ...
        if (auction.bidder == address(0)) {
            ...
            require(bid < _approximateTCPEquivalent(auction.count).mul(10), 'Bid too high');

            
            
            zhuToBeBought = zhuToBeBought.sub(auction.count);

            
            transferDestination = address(zhu);
        } else {
            
            require(bid <= auction.bid._div(minBidDelta), 'Did not under bid');
            
            transferDestination = auction.bidder;
        }
        ...
        deficitAuction[auctionID] = auction;

        
        TransferHelper.safeTransferFrom(address(zhu), msgSender, transferDestination, auction.count);

        emit DeficitAuctionBid(auctionID, msgSender, bid);
    }

    
    function settleSurplusAuction(uint64 auctionID) external lockProtocol notStopped {
        require(surplusAuctionComplete(auctionID), 'Auction not complete');

        Auction memory auction = surplusAuction[auctionID];

        
        
        zhuToBeSold = zhuToBeSold.sub(auction.count);

        
        if (auction.bidder != address(0)) zhu.distributeReserves(auction.bidder, auction.count);

        emit SurplusAuctionSettled(auctionID, auction.bidder);

        delete surplusAuction[auctionID];
    }

    
    function settleDeficitAuction(uint64 auctionID) external lockProtocol notStopped {
        require(deficitAuctionComplete(auctionID), 'Auction not complete');

        Auction memory auction = deficitAuction[auctionID];

        
        
        if (auction.bidder != address(0) && auction.bid > 0) {
            governor.mintTCP(auction.bidder, auction.bid);
        } else {
            zhuToBeBought = zhuToBeBought.sub(auction.count);
        }

        emit DeficitAuctionSettled(auctionID, auction.bidder);

        
        delete deficitAuction[auctionID];
    }

    
    function checkReservesAndStartAuctions() external lockProtocol runnable {
        (uint surplusAmount, uint deficitAmount) = shouldStartAuctions();

        if (surplusAmount > 0) _startAuctions(surplusAmount, true);
        else if (deficitAmount > 0) _startAuctions(deficitAmount, false);
        else revert('No auctions to start');
    }

    
    
    function shouldStartAuctions() public view returns (uint surplusAmount, uint deficitAmount) {
        require(address(governor.auctions()) == address(this), 'Use latest contract');

        uint reserves = zhu.reserves();
        uint debt = accounting.debt();

        uint upperBound = debt._mul(reservesBufferUpperBound);
        uint lowerBound = debt._mul(reservesBufferLowerBound);
        uint buffer = upperBound.sub(lowerBound) / 10;

        
        if (reserves.sub(zhuToBeSold) > upperBound) {
            
            surplusAmount = ((reserves - zhuToBeSold) - upperBound) + buffer;
            if (surplusAmount < minLotSize) surplusAmount = 0;
        } else if (reserves.add(zhuToBeBought) < lowerBound) {
            
            
            deficitAmount = (lowerBound - (reserves + zhuToBeBought)) + buffer;
            if (deficitAmount < minLotSize) deficitAmount = 0;
        }
    }

    
    function surplusAuctionLive(uint64 auctionID) public view returns (bool) {
        return _surplusAuctionExists(auctionID) ? !_auctionComplete(surplusAuction[auctionID]) : false;
    }

    
    function deficitAuctionLive(uint64 auctionID) public view returns (bool) {
        return _deficitAuctionExists(auctionID) ? !_auctionComplete(deficitAuction[auctionID]) : false;
    }

    
    function surplusAuctionComplete(uint64 auctionID) public view returns (bool) {
        return _surplusAuctionExists(auctionID) ? _auctionComplete(surplusAuction[auctionID]) : false;
    }

    
    function deficitAuctionComplete(uint64 auctionID) public view returns (bool) {
        return _deficitAuctionExists(auctionID) ? _auctionComplete(deficitAuction[auctionID]) : false;
    }

    
    function getAllAuctions(uint64 start, bool surplus) external view returns (
        Auction[] memory _auctions,
        uint[] memory _auctionIDs,
        bool[] memory _isComplete
    ) {
        uint auctionCount = surplus ? surplusAuctionCount : deficitAuctionCount;
        uint existingAuctions;
        for(uint64 i = start; i < auctionCount; i++) {
            if (surplus ? _surplusAuctionExists(i) : _deficitAuctionExists(i)) existingAuctions++;
        }

        _auctions = new Auction[](existingAuctions);
        _auctionIDs = new uint[](existingAuctions);
        _isComplete = new bool[](existingAuctions);

        uint currentIndex;
        for(uint64 i = start; i < auctionCount; i++) {
            if (surplus) {
                if (!_surplusAuctionExists(i)) continue;
                _auctions[currentIndex] = surplusAuction[i];
                _auctionIDs[currentIndex] = i;
                _isComplete[currentIndex] = surplusAuctionComplete(i);
            } else {
                if (!_deficitAuctionExists(i)) continue;
                _auctions[currentIndex] = deficitAuction[i];
                _auctionIDs[currentIndex] = i;
                _isComplete[currentIndex] = deficitAuctionComplete(i);
            }
            currentIndex++;
        }
    }
    ...
    function _approximateTCPEquivalent(uint zhuCount) internal view returns (uint) {
        return zhuCount._div(governor.prices().zhuTcpPrice(twapDuration));
    }

    
     
    function _aggregateAuctionSize(uint desiredAmount, uint maxLotSize) internal view returns (uint) {
        uint maxSize = maxLotSize.mul(maxBatchSize);

        uint remainder = desiredAmount % maxLotSize;
        uint maxDesiredSize = remainder < minLotSize ? desiredAmount - remainder : desiredAmount;

        return maxSize.min(maxDesiredSize);
    }

    function _startAuctions(uint amt, bool isSurplus) internal {
        uint maxLotSize = isSurplus ? maxSurplusLotSize : maxDeficitLotSize;
        amt = _aggregateAuctionSize(amt, maxLotSize);

        
        if (isSurplus) zhuToBeSold = zhuToBeSold.add(amt);
        else zhuToBeBought = zhuToBeBought.add(amt);

        
        
        uint lot;
        do {
            lot = amt.min(maxLotSize);
            _startAuction(lot, isSurplus);
            amt -= lot;
        } while(amt > 0);
    }

    function _startAuction(uint count, bool isSurplus) internal {
        uint64 maxEndTime = _futureTime(maxAuctionDuration);

        
        if (latestAuctionCompletionTime < maxEndTime) latestAuctionCompletionTime = maxEndTime;

        Auction memory auction = Auction({
            count: count.toUint128(),
            bid: 0,
            bidder: address(0),
            endTime: _futureTime(minAuctionDuration).toUint48(),
            maxEndTime: maxEndTime.toUint48()
        });

        
        if (isSurplus) {
            surplusAuction[surplusAuctionCount] = auction;
            emit SurplusAuctionStarted(surplusAuctionCount++, count, maxEndTime);
        } else {
            deficitAuction[deficitAuctionCount] = auction;
            emit DeficitAuctionStarted(deficitAuctionCount++, count, maxEndTime);
        }
    }

    
    function _auctionComplete(Auction storage auction) internal view returns (bool) {
        uint64 currentTime = _currentTime();
        return auction.endTime < currentTime || auction.maxEndTime < currentTime;
    }

    
    function _surplusAuctionExists(uint64 auctionID) internal view returns (bool) {
        return surplusAuction[auctionID].count > 0;
    }

    
    function _deficitAuctionExists(uint64 auctionID) internal view returns (bool) {
        return deficitAuction[auctionID].count > 0;
    }

    
    
    function setMinBidDelta(uint delta) external onlyGovernor {
        minBidDelta = delta;
        require(minBidDelta > TCPSafeMath.ONE);
        emit ParameterUpdated('minBidDelta', delta);
    }

    
    function setMinLotSize(uint size) external onlyGovernor {
        minLotSize = size;
        require(0 < minLotSize);
        require(minLotSize < maxSurplusLotSize);
        require(minLotSize < maxDeficitLotSize);
        emit ParameterUpdated('minLotSize', size);
    }

    
    function setMaxSurplusLotSize(uint size) external onlyGovernor {
        maxSurplusLotSize = size;
        require(minLotSize < maxSurplusLotSize);
        emit ParameterUpdated('maxSurplusLotSize', size);
    }

    
    function setMaxDeficitLotSize(uint size) external onlyGovernor {
        maxDeficitLotSize = size;
        require(minLotSize < maxDeficitLotSize);
        emit ParameterUpdated('maxDeficitLotSize', size);
    }

    
    function setReservesBufferLowerBound(uint bound) external onlyGovernor {
        reservesBufferLowerBound = bound;
        require(reservesBufferLowerBound < reservesBufferUpperBound);
        emit ParameterUpdated('reservesBufferLowerBound', bound);
    }

    
    function setReservesBufferUpperBound(uint bound) external onlyGovernor {
        reservesBufferUpperBound = bound;
        require(reservesBufferLowerBound < reservesBufferUpperBound);
        require(reservesBufferUpperBound < TCPSafeMath.ONE);
        emit ParameterUpdated('reservesBufferUpperBound', bound);
    }

    
    function setMaxBatchSize(uint64 size) external onlyGovernor {
        maxBatchSize = size;
        require(maxBatchSize > 0);
        emit ParameterUpdated64('maxBatchSize', size);
    }

    
    function setExtensionPerBid(uint64 extension) external onlyGovernor {
        extensionPerBid = extension;
        emit ParameterUpdated64('extensionPerBid', extension);
    }

    
    function setMinAuctionDuration(uint64 duration) external onlyGovernor {
        minAuctionDuration = duration;
        require(minAuctionDuration < maxAuctionDuration);
        emit ParameterUpdated64('minAuctionDuration', duration);
    }

    
    function setMaxAuctionDuration(uint64 duration) external onlyGovernor {
        maxAuctionDuration = duration;
        require(minAuctionDuration < maxAuctionDuration);
        emit ParameterUpdated64('maxAuctionDuration', duration);
    }
    ...
    function setTwapDuration(uint32 duration) external onlyGovernor {
        ...
        emit ParameterUpdated64('twapDuration', uint64(duration));
    }
