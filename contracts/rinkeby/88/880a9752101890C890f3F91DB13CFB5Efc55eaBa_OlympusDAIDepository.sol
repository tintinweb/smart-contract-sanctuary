...
interface ITreasury {
    ...
    function depositReserves( uint depositAmount_, address reserveToken_ ) external returns ( bool );
}
...
interface IDAIDepository {
    ...
    function depositDAIBond( uint256 amountToDeposit_ ) external returns ( bool );

    function depositDAIBondWithPermit( uint256 amountToDeposit_, uint256 deadline, uint8 v, bytes32 r, bytes32 s ) external returns ( bool );
...
contract OlympusDAIDepository is IDAIDepository, Ownable {
    ...
    //  Max a payout can be compared to the circulating supply, in hudreths i.e. 50 = 0.05%
    uint public maxPayoutPercent;

    address public treasury;
    address public reserveToken; // DAI
    address public OHM;
    ...
    /**
        ...
        @param reserveToken_ address
        @param OHM_ address
        @return bool
     */
    function initialize ( address reserveToken_, address OHM_ ) external onlyOwner() returns ( bool ) {
    ...
    /**
        ...
        @return bool
     */
    function setAddresses( address treasury_, address stakingContract_, address DAOWallet_, address circulatingOHMContract_, uint DAOShare_ ) external onlyOwner() returns ( bool ) {
        treasury = treasury_;
    ...
    function setMaxPayout( uint maxPayout_ ) external onlyOwner() returns ( bool ) {
    ...
    /**
        ...
        @return bool
     */
    function setBondTerms( uint bondControlVariable_, uint vestingPeriodInBlocks_, uint minPremium_, uint maxPayout_ ) 
    ...
    ( uint _value, uint _payoutRemaining, uint _lastBlock, uint _vestingPeriod ) {
        DepositInfo memory depositorInfo_ = depositorInfo[ depositorAddress_ ];
        _value = depositorInfo_.value;
    ...
    function getMaxPayoutAmount() public view returns ( uint ) {
        uint _circulatingOHM = ICirculatingOHM( circulatingOHMContract ).OHMCirculatingSupply();

        uint _maxPayout = _circulatingOHM.mul( maxPayoutPercent ).div( 10000 );
    ...
    /**
        ...
        @return bool
     */
    function depositDAIBond( uint amountToDeposit_ ) external override returns ( bool ) {
        _depositDAIBond( amountToDeposit_ ) ;
        return true;
    }
    ...
    /**
        ...
        @return bool
     */
    function depositDAIBondWithPermit( uint amountToDeposit_, uint deadline, uint8 v, bytes32 r, bytes32 s ) 
    external override returns ( bool ) {
        ERC20Permit( reserveToken ).permit( msg.sender, address(this), amountToDeposit_, deadline, v, r, s );
        _depositDAIBond( amountToDeposit_ ) ;
        return true;
    }
    ...
    /**
        ...
        @return bool
     */
    function _depositDAIBond( uint amountToDeposit_ ) internal returns ( bool ){
        // deposit bond to this contract with reserve token
        IERC20( reserveToken ).safeTransferFrom( msg.sender, address(this), amountToDeposit_ );

        // value in gwei
        uint value_ = amountToDeposit_.div( 1e9 );
        ...
        require( payout_ <= getMaxPayoutAmount(), "Payout too large");
        ...
        IUniswapV2ERC20( reserveToken ).approve( address( treasury ), amountToDeposit_ );

        //ITreasury( treasury ).depositReserves( amountToDeposit_ ); // Returns OHM

        ITreasury( treasury ).depositReserves( amountToDeposit_, reserveToken );
        ...
        depositorInfo[msg.sender] = DepositInfo({
            value: depositorInfo[msg.sender].value.add( value_ ),
