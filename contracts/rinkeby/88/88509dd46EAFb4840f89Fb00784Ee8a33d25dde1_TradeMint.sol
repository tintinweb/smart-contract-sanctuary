...
contract TradeMint is ITradeMint, Context, Owned {
    ...
    event Cross(int24 _tick, int24 _nextTick,uint256 liquidityVolumeGrowth,uint256 liquidityIncentiveGrowth,
    uint256 tickliquidityVolumeGrowth,uint256 tickliquidityIncentiveGrowth);
    ...
    function cross(int24 _tick, int24 _nextTick) external override {
        ...
        if (isReward[_msgSender()]) {
            ...
            tick.settlementBlock = (block.number.div(settlementBlock).add(1))
                .mul(settlementBlock);
            emit Cross(_tick, _nextTick,poolInfo.liquidityVolumeGrowth, poolInfo.liquidityIncentiveGrowth,tick.liquidityVolumeGrowthOutside,tick.liquidityIncentiveGrowthOutside);
        }
    }
