...
/*
    ...
    Dev Telegram: @elbrupt

*/

contract Ryu is ERC721, Ownable {
    using Strings for uint256;

    uint256 public constant RYU_PRIVATE = 1000;
    uint256 public constant RYU_PUBLIC = 7888;
    uint256 public constant RYU_MAX = RYU_PRIVATE + RYU_PUBLIC;

    uint256 public RYU_PRICE = 0.09 ether;
    uint256 public constant RYU_PRESALE_PRICE = 0.09 ether;
    uint256 public constant RYU_SALE_PRICE = 0.1 ether;

    uint256 public constant RYU_PER_MINT = 5;
    ...
    constructor() ERC721("Ryu Nation", "RYU") {}
    ...
    function mint(uint256 tokenQuantity) external payable {
        ...
        require(totalSupply < RYU_MAX, "OUT_OF_STOCK");
        require(
            publicAmountMinted + tokenQuantity <= RYU_PUBLIC,
            "EXCEED_PUBLIC"
        );
        require(tokenQuantity <= RYU_PER_MINT, "EXCEED_RYU_PER_MINT");
        require(RYU_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
    ...
    function presaleMint(uint256 tokenQuantity) external payable {
        ...
        require(totalSupply < RYU_MAX, "OUT_OF_STOCK");
        require(
            privateAmountMinted + tokenQuantity <= RYU_PRIVATE,
            "EXCEED_PRIVATE"
        );
        require(
