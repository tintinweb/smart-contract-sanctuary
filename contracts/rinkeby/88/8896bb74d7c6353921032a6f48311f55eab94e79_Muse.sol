...
contract Muse is Context, IERC20, Ownable {
    ...
    event TreasuryUpdated(address treasuryWallet);
    event VaultUpdated(address museVault);
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        _transferMuse(_msgSender(), recipient, amount);
        return true;
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        _transferMuse(sender, recipient, amount);
    ...
    function _transferMuse(address from, address to, uint256 amount) internal virtual {
        ...
        uint256 treasuryVal = amount.mul(feePercentage).div(
            10**(uint256(feeDecimals) + 2)
        );
        uint256 vaultVal = amount.mul(feePercentage).div(
            10**(uint256(feeDecimals) + 2)
        );
        ...
        if(isWhitelisted[from] || isWhitelisted[to]) {
            ...
            uint256 tokensToTransfer = amount.sub(tokensToLock).sub(treasuryVal).sub(vaultVal);
    
            _transfer(from, address(this), tokensToLock);
            _transfer(from, treasuryWallet, treasuryVal);
            _transfer(from, museVault, vaultVal);
    ...
    */
    ...
    function updateTreasury(address _treasuryWallet)
        public
        onlyOwner
    {
        treasuryWallet = _treasuryWallet;
    ...
    function updateVault(address _museVault)
        public
        onlyOwner
    {
        museVault = _museVault;
        emit VaultUpdated(_museVault);
    }
