...
contract GearUniversity is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint256 public GEAR_UNIVESITY_CLASS_SIZE = 12000;
    uint256 public GEAR_LEGACY_APPLICANTS = 7777;
    ...
    function getSwagger(uint256 tokenId) public view returns (uint256) {
        uint256 rand = random(string(abi.encodePacked("SWAGGER", toString(tokenId))));
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[15] = toString(getSwagger(tokenId));
...
//old stuff. 
    ...
    function mint(uint256 tokenId) public payable nonReentrant {
        require(tokenId > 7777 && tokenId < 12000, "Token ID invalid");
    ...
    function multiMint(uint256[] memory tokenIds) public payable nonReentrant {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(tokenIds[i] > 7777 && tokenIds[i] < 12000, "Token ID invalid");
            _safeMint(msg.sender, tokenIds[i]);
        }
    }

    function mintWithGear(uint256 gearId) public payable nonReentrant {
        require(gearId > 0 && gearId <= 7777, "Token ID invalid");
        require(gearContract.ownerOf(gearId) == msg.sender, "Not the owner of this gear");
        _safeMint(_msgSender(), gearId);
    }

    function multiMintWithGear(uint256[] memory gearIds) public payable nonReentrant {
        for (uint256 i = 0; i < gearIds.length; i++) {
            require(gearContract.ownerOf(gearIds[i]) == msg.sender, "Not the owner of this gear");
