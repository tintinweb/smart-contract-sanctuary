...
abstract contract YSLOpt is Ownable, AccessControl {
    ...
    function deposit(
        ...
        uint256 _amount,
        address _referrer
    ) external virtual stratOnly returns (uint256) {
    ...
    function withdraw(uint256 _amount, address _user) external virtual stratOnly returns (uint256) {
    ...
    function collectControllerFee(uint256 _amount) internal virtual returns (uint256 summary);

    function collectOptimisationTax(uint256 _amount) internal virtual returns (uint256 summary);
}
...
contract YSLOptSingleApe is YSLOpt {
    ...
    function collectControllerFee(uint256 _amount) internal override returns (uint256 summary) {
        uint256 fee = (_amount * controllerFee) / 100;
        summary = _amount - fee;
    ...
    function collectOptimisationTax(uint256 _amount) internal override returns (uint256 summary) {
        uint256 tax = _amount / 10;
    ...
    function earn(
        ...
        if (koef == coef_s1) {
            uint256 bonus = ((usersCompUSD * _price) * coef_ref) / 10**18 / 100;
            IsYSL(_sysl).mintPurchased(IReferral(referral).referrals(_user), bonus, 90 days);
        }
        koef = _isAmplified ? coef_s2 : koef;
        uint256 usersIncome = ((usersCompUSD * _price) * koef) / 10**18 / 100;
