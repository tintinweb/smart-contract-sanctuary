...
contract WW is ERC721, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public constant MAX_TOKENS = 44;
    mapping(address => uint256) private minted;
    
    
    constructor() ERC721("WW", "WW") { }
    ...
    function remainingMint(address user) public view returns (uint256) {
        ...
        require(preSaleIsActive || saleIsActive, "Presale and sale aren't active");
      
      return maxTokenPurchase - minted[user];
    }

    
    function whitelistMint(uint numberOfTokens, bytes32[] memory proof) public payable nonReentrant {
      uint256 supply = totalSupply();
      bytes32 leaf = _leaf(msg.sender, numberOfTokens);
      require(preSaleIsActive || msg.sender == owner(), "Pre-sale not started or has ended");
      require(remainingMint(msg.sender) >= numberOfTokens, "You can't mint any more during the presale");
      require(totalSupply().add(numberOfTokens) <= MAX_TOKENS, "Purchase would exceed max supply");
      require(msg.value >= numberOfTokens * tokenPrice, "Ether sent is not correct");
