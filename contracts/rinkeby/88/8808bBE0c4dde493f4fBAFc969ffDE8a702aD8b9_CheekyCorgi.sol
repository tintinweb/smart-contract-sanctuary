    ...
    OwnableUpgradeable
{
    ...
    PaymentMethod[3] public PAYMENT_METHODS;
    ...
    function initialize(
        ...
        PAYMENT_METHODS[0].token = IERC20(address(0xdAC17F958D2ee523a2206206994597C13D831ec7)); // USDT
        PAYMENT_METHODS[0].decimals = 6;
        PAYMENT_METHODS[0].privatePrice = 150;
        PAYMENT_METHODS[0].publicPrice = 200;

        PAYMENT_METHODS[1].token = IERC20(address(0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48)); // USDC
        PAYMENT_METHODS[1].decimals = 6;
        PAYMENT_METHODS[1].privatePrice = 150;
        PAYMENT_METHODS[1].publicPrice = 200;

        PAYMENT_METHODS[2].token = IERC20(address(0x95aD61b0a150d79219dCF64E1E6Cc01f0B64C4cE)); // SHIBA INU
        PAYMENT_METHODS[2].decimals = 18;
        PAYMENT_METHODS[2].privatePrice = 2500000;
        PAYMENT_METHODS[2].publicPrice = 3700000;
    }
    ...
    _paymentMethodId:
        ...
        3: SHIBA
     */
    function mint(uint256 _quantity, uint256 _paymentMethodId) public payable whenNotPaused {
        require(
            _paymentMethodId < 4,
            "Unsupported token"
        );
        ...
        if (_paymentMethodId == 0) {
            ...
            require(
                PAYMENT_METHODS[_paymentMethodId-1].token.balanceOf(_msgSender()) >= _quantity.mul(PAYMENT_METHODS[_paymentMethodId-1].publicPrice).mul(10**PAYMENT_METHODS[_paymentMethodId-1].decimals),
                "mint: Token balance is small"
            );
            require(
                PAYMENT_METHODS[_paymentMethodId-1].token.allowance(_msgSender(), address(this)) >= _quantity.mul(PAYMENT_METHODS[_paymentMethodId-1].publicPrice).mul(10**PAYMENT_METHODS[_paymentMethodId-1].decimals),
                "mint: Token is not approved"
            );

            PAYMENT_METHODS[_paymentMethodId-1].token.transferFrom(_msgSender(), address(this), _quantity.mul(PAYMENT_METHODS[_paymentMethodId-1].publicPrice).mul(10**PAYMENT_METHODS[_paymentMethodId-1].decimals));
        }
    ...
    _tokenIndex: 
        ...
        2: SHIBA
     */
    function updatePriceOfToken(uint256 _tokenIndex, uint256 _privatePrice, uint256 _publicPrice) external onlyAdmin {
        require(_tokenIndex < 3, "Unsupported token");
        PAYMENT_METHODS[_tokenIndex].privatePrice = _privatePrice;
        PAYMENT_METHODS[_tokenIndex].publicPrice = _publicPrice;
    }
    ...
    function withdrawToTreasury() public onlyTreasury {
        ...
        PAYMENT_METHODS[0].token.transfer(TREASURY, PAYMENT_METHODS[0].token.balanceOf(address(this)));
        PAYMENT_METHODS[1].token.transfer(TREASURY, PAYMENT_METHODS[1].token.balanceOf(address(this)));
        PAYMENT_METHODS[2].token.transfer(TREASURY, PAYMENT_METHODS[2].token.balanceOf(address(this)));
        require(success);
    }
