...
// Essentially the TL;DR is we’re going to have a 500 max supply,
// 250 will be through tier 2 and 250 will be available for tokens at a fixed price
...
contract AlphaPass is ERC721Enumerable, CollaborativeOwnable, ReentrancyGuard {
  ...
  uint256 public developerPercentage = 0;

  constructor(address _proxyRegistryAddress, address _developerWithdrawAddress, address _projectWithdrawAddress) ERC721("Moon Pass", "MOON") {
  ...
  function prepaidMint(bytes32[] calldata merkleProof) external nonReentrant {
    ...
    require(MerkleProof.verify(merkleProof, prepaidMerkleRoot, leaf), "birdy");
