...
contract ChicksTestFinal is ERC721, ERC721Enumerable, Ownable {
  
  // Provenance
  string public CHICK_TEST_HASH = "";

  // Price & Supply
  uint256 public constant NFT_PRICE = 10000000000000000; //0.010 ETH
  ...
  constructor(string memory baseURI) ERC721("ChickadeezTestFinal", "CDZTF") {
    setBaseURI(baseURI);
  }
  ...
  function mint(uint256 numNFTs) public payable {
    ...
    for (uint i = 0; i < numNFTs; i++) {
      uint mintIndex = totalSupply(); // +1 so it doesn't start on index 0.
  ...
  function setProvenanceHash(string memory provenanceHash) public onlyOwner {
    CHICK_TEST_HASH = provenanceHash;
  }
}