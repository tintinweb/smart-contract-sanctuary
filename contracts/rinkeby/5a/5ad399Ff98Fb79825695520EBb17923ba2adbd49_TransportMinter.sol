...
contract TransportMinter is Ownable, ContextMixin {
    ...
    function allocateTransports(uint256[] calldata tokenIds, address[] calldata addr) external onlyOwner {
        require(tokenIds.length == addr.length, "TransportMinter: Wrong data format");
        for (uint256 i = 0; i < addr.length; i++) {
            allocatedTransports[tokenIds[i]] = addr[i];
        }
    }

    function redeemTransport(uint256 tokenId) public {
        require(isMinted[tokenId] == false, "TransportMinter: Already Minted");

        address minter = msg.sender;
        require(allocatedTransports[tokenId] == minter, "TransportMinter: Not Allocated");
    ...
    function batchRedeemTransports(uint256[] calldata tokenIds) public {
        uint256 tokenId;
        address minter = msg.sender;
    ...
    function _updateAddresses(address transportAddress)
        external
        onlyOwner
    {
        netvrkTransport = IMintableERC721(transportAddress);
