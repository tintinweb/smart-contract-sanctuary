...
contract Jungle is Ownable, IERC721Receiver {
    ...
    */
    ...
    //Amount of $Banana stolen by Pirate while staked
    mapping(uint256 => uint256) public bananaStolen;

    //Daily $Banana earned by adventurers
    uint256 public adventuringBananaRate = 100 ether;
    //Total number of adventurers staked
    uint256 public totalAdventurersStaked = 0;
    //Percent of $Banana earned by adventurers that is kept
    uint256 public adventurerShare = 50;

    //Percent of $Banana earned by adventurers that is stolen by pirates
    ...
    */
    ...
    function _claim(address owner, uint256 tokenId) internal {
        if (statusById[tokenId] == 1){
            if(piratesStaked.length > 0){
                bananas.mint(owner, getPendingBanana(tokenId).mul(adventurerShare).div(100));
                distributeAmongstPirates(getPendingBanana(tokenId).mul(pirateShare).div(100));
            }
            else {
    ...
    //Distribute stolen $Banana accross all staked pirates
    function distributeAmongstPirates(uint256 amount) internal {
        for(uint256 i = 0; i < piratesStaked.length; i++){
            bananaStolen[piratesStaked[i]] += amount.div(piratesStaked.length);
        }
    }
...
contract Apes is ERC721Enumerable, Ownable {
    ...
    bool public bananaMinting = false;
    ...
    function mintHammieForbanana() public {
        uint256 total = _totalSupply();
        require(total < maxTotalSupply, "No Apes left");
        require(bananaMinting, "Minting with $banana has not begun");
        bananas.burn(msg.sender, getBananaCost(total));
        _mintHammie(msg.sender);
    }

    function getBananaCost(uint256 totalSupply) internal pure returns (uint256 cost){
        if (totalSupply < 5888)
            return 100;
    ...
    //Start/Stop minting hammies for $banana
    function toggleBananaMinting() public onlyOwner {
        bananaMinting = !bananaMinting;
    }
...
contract Bananas is ERC20, Ownable {
    ...
    constructor() ERC20("ban", "bb") {}

    function setApesAddress(address apeAddr) external onlyOwner {
        apesAddress = apeAddr;
    }
    ...
    function burn(address user, uint256 amount) external {
        require(msg.sender == jungleAddress || msg.sender == apesAddress, "Address not authorized");
        _burn(user, amount);
    }
