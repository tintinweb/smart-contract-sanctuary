...
contract ViralswapVault {
    ...
    function getQuoteIn(address _tokenIn, uint256 _amountIn) external view returns (uint256 amountOut) {
        if(_tokenIn != tokenIn) {
            return 0;
        }
    ...
    function getQuoteOut(address _tokenOut, uint256 _amountOut) external view returns (uint256 amountIn) {
