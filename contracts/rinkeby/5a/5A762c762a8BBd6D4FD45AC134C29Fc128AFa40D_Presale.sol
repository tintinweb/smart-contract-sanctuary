...
interface ISaft {
    ...
    struct PurchaserInfo {
        uint256 maximumTokenAllocation; // (1) Maximum number of Tokens to be purchased by the Purchaser
        uint256 maximumPurchaseAmount;  // (2) Maximum purchase amount, for example 50,000 USDT
        IERC20 paymentMethod;           // (3) Payment method, for example USDT
        uint64 paymentDeadline;         // (4) Payment ultimate date.
        uint64 fullVestingTimestamp;    // (5) Vesting period
        uint64 cliffTimestamp;          //     cliff.
...
contract Presale is ISaft, Ownable {
    ...
    function purchase(address purchaser, uint256 payAmount) external payable override {
        ...
        require(p.maximumTokenAllocation != 0, "Purchase was not initialized");
        require(block.timestamp <= p.paymentDeadline, "Payment deadline finished");

        uint256 purchaseAmount = p.maximumTokenAllocation * payAmount / p.maximumPurchaseAmount;
        require(p._tokensBought + purchaseAmount <= p.maximumTokenAllocation, "exceeded the purchase limit");

        if(p.paymentMethod == IERC20(address(0))) {
            require(msg.value == payAmount, "invalid ETH payment amount");
        } else {
        ...
        emit Purchased(purchaser, purchaseAmount);
    }
    ...
    function tokenVested(address purchaser) public view returns (uint256) {
        ...
        uint256 timeSinceCliffSeconds = block.timestamp - p.cliffTimestamp;
    ...
    function claimTokens() external override {
        ...
        require(p._tokensBought > 0, "Nothing to claim");
        require(p.cliffTimestamp < block.timestamp, "Cannot claim tokens before cliff is over");
    ...
    function addPurchaser (
        ...
        require(presale[purchaser].maximumTokenAllocation == 0, "Purchase is already initialized");
        require(maximumTokenAllocation >= 1e18, "Allocation is too small");
        require(vestingPeriodSeconds > cliffSeconds, "Negative vesting");
        token.safeTransferFrom(msg.sender, address(this), maximumTokenAllocation);
    ...
    function increaseAllocation(
        ...
        token.safeTransferFrom(msg.sender, address(this), newMaxTokenAllocation - p.maximumTokenAllocation);
    ...
    function decreaseAllocation(
        ...
        token.safeTransfer(msg.sender, p.maximumTokenAllocation - newMaxTokenAllocation);
    ...
    function adminWithdraw(IERC20 _token) external onlyOwner {
        if (_token == token) {
            require(block.timestamp > endDate, "Presale haven't finish yet");

            uint256 withdrawAmount = token.balanceOf(address(this)) - totalTokensSold;
            if (withdrawAmount > 0) {
    ...
    event Purchased(address account, uint256 stakedAmount);
