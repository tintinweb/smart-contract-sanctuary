...
///_/    \_\___/\__, |\__,_|_|  \__,_| |_____/_/    \_\____/
//               __/ |
//              |___/
...
contract AsgardTreasury is Ownable {
    ...
    address public sAMP;
    uint public sAMPQueue; // Delays change to sAMP address
    ...
    // TODO(DD): below lines should probably be uncomented
    ...
    /**
        @notice allow approved address to deposit an asset for AMP
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint AMP needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( AMP ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn AMP for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IAMPERC20( AMP ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sAMP ).balanceOf( msg.sender ); // Can only borrow against sAMP held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with AMP
        @param _amount uint
     */
    function repayDebtWithAMP( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IAMPERC20( AMP ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, AMP, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( AMP ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( AMP ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns AMP valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match AMP decimals
            value_ = _amount.mul( 10 ** IERC20( AMP ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SAMP ) { // 9
            sAMPQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
