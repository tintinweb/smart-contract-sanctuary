...
contract SundayDrips is ERC721, Ownable {
    ...
    uint256 public chromeAvailable = 1330;
    uint256 public goldAvailable = 667;
    uint256 public trippyAvailable = 225;

    uint256 public startTime;
    uint256 public startPrice = 0.07 ether;

    uint256 private chromeId;
    uint256 private goldId = 1331;
    uint256 private trippyId = 1998;

    mapping(address => uint256) public chromeAmount;
    mapping(address => uint256) public goldAmount;
    mapping(address => uint256) public trippyAmount;
    ...
    function mintChrome() external payable {
        require(_saleIsActive, "Sale not active");
        require(1 <= chromeAvailable, "No chrome left");
        require(chromeAmount[msg.sender] == 0, "Can't mint more");
        require(msg.value == getCurrentPrice(), "Not enough ETH");

        chromeId++;
        _mint(msg.sender, chromeId);
        chromeAvailable.sub(1);
        chromeAmount[msg.sender]++;
    }

    function mintGold() external payable {
        require(_saleIsActive, "Sale not active");
        require(1 <= goldAvailable, "No gold left");
        require(goldAmount[msg.sender] == 0, "Can't mint more");
        require(msg.value == getCurrentPrice(), "Not enough ETH");

        goldId++;
        _mint(msg.sender, goldId);
        goldAvailable.sub(1);
        goldAmount[msg.sender]++;
    }

    function mintTrippy() external payable {
        require(_saleIsActive, "Sale not active");
        require(1 <= trippyAvailable, "No trippy left");
        require(trippyAmount[msg.sender] == 0, "Can't mint more");
        require(msg.value == getCurrentPrice(), "Not enough ETH");

        trippyId++;
        _mint(msg.sender, trippyId);
        trippyAvailable.sub(1);
        trippyAmount[msg.sender]++;
    }
