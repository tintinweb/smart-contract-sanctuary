...
contract BAYCBills is ERC721URIStorage {
  ...
  function genAttributes(uint256 _tokenId) public {
    ...
    uint256 numerOfFriends = genRandomNumber() % 10;
    uint256 numberOfRounds = genRandomNumber() % 6;
    // for each friend
    for (uint256 friend=0; friend<numerOfFriends; friend++) {
      ...
      for (uint256 round=0; round<numberOfRounds; round++) {
        ...
        uint256 beerIndex = genRandomNumber() % 4;
        string memory beerThatRoundByThatFriend = beerTypes[beerIndex - 1];
        ...
        if (numberOfRounds > 4) {
          uint256 randomNumber = genRandomNumber();
          if (randomNumber % 2 == 1) {
        ...
        if (jagerBottleCounter > 0) {
          // for every bottle there's a 20% chance another one gets ordered
          uint256 randomNumber = genRandomNumber();
          if (randomNumber % 5 == 1) {
  ...
  function genBillItem(uint256 _tokenId) public view returns (string memory) {

    // push final number of drinkCounters into array;
    // no of different beers is array.length
    ...
    if (totalIpas > 0) {
      ...
      ipasRow = string(
        abi.encodePacked(
          ...
          uint2str(totalIpas),
          '</text>',
          '<text x="425" y="375" class="t">Draft IPAs</text>',
          '<text x="600" y="375" class="t">',
          uint2str(totalIpas),
          ' ETH</text>'
        )
    ...
    return string(
      abi.encodePacked(lagersRow,ipasRow)
    );

    // uint[] memory billDrinks = [
    //   tokenIdToNumberOfLagers[_tokenId],
    //   tokenIdToNumberOfIPAs[_tokenId],
    //   tokenIdToNumberOfNEIPAs[_tokenId],
    //   tokenIdToNumberOfStouts[_tokenId],
    //   tokenIdToNumberOfJagerbombs[_tokenId],
    //   tokenIdToNumberOfJagerBottles[_tokenId]
    // ]
  ...
  function genRandomNumber() private view returns (uint256) {
    return uint256(
  ...
  // function sortArray(uint[] memory arr, int left, int right) pure {
  //   int i = left;
  //   int j = right;
  //   if (i == j) return;
