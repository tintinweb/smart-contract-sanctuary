...
// File: contracts/TellorAccess.sol
...
contract Oracle is TellorAccess, IOracle {
    
    string internal _collateral;
    string internal _underlyingAsset;
    bool internal _isTerminated;
    bool internal _isMarketClosed;
    uint256 public markRequestId;
    uint256 public indexRequestId;
    
    /**
     * @dev Constructor which sets configuration parameters
     * @param collateral_ is the collateral symbol
     * @param underlyingAsset_ is the underlying asset symbol
     * @param _markRequestId is the requestId of the MarkPrice
     * @param _indexRequestId is the requestId of the IndexPrice
    */
    constructor(string memory collateral_, string memory underlyingAsset_, uint256 _markRequestId, uint256 _indexRequestId) TellorAccess() {
        _collateral = collateral_;
        _underlyingAsset = underlyingAsset_;
        markRequestId = _markRequestId;
        indexRequestId = _indexRequestId;
    }
     ...
     * @dev Mark price.
     */
    function priceTWAPLong() external view override returns (int256 newPrice, uint256 newTimestamp) {
        uint256 _count = getNewValueCountbyRequestId(markRequestId);
        uint256 _time =
            getTimestampbyRequestIDandIndex(markRequestId, _count - 1);
        uint256 _value = retrieveData(markRequestId, _time);
        if (_value > 0) return (int256(_value), _time);
        return (0, _time);
    }
    ...
    function priceTWAPShort() external view override returns (int256 newPrice, uint256 newTimestamp) {
        uint256 _count = getNewValueCountbyRequestId(indexRequestId);
        uint256 _time =
            getTimestampbyRequestIDandIndex(indexRequestId, _count - 1);
        uint256 _value = retrieveData(indexRequestId, _time);
        if (_value > 0) return (int256(_value), _time);
        return (0, _time);
    }
    
    /**
     * @dev Terminate the oracle service
     */
