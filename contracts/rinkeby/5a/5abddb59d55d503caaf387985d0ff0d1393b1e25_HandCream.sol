...
contract HandCream is Ownable, ERC721Enumerable, ReentrancyGuard, ERC721URIStorage {
    ...
    constructor(string memory baseTokenURI) 
        ERC721("Rock Paper Scissors","RPS") {
   ...
   function mint() public payable nonReentrant {
        ...
        require(msg.value >= TOKEN_PRICE , "Ether value sent is unsufficient");
        _mintWithRandomTokenId(msg.sender);
    }
    
    function burn(uint tokenId) public nonReentrant {
        require(ownerOf(tokenId) == _msgSender() , "Token is not owned by sender");
        _burn(tokenId);
        Address.sendValue(payable(_msgSender()),TOKEN_PRICE);
    }
