...
* @author Niftydude
*/
contract AdidasCard is AbstractERC1155Factory, PaymentSplitter  {

    uint256 constant MAX_SUPPLY = 30000;
    uint256 constant MAX_EARLY_ACCESS = 20700;
    uint256 constant MAX_PER_TX = 3;
    uint256 constant MINT_PRICE = 200000000000000000;

    uint256 public cardIdToMint = 1;

    uint256 public purchaseWindowOpens;
    uint256 public purchaseWindowCloses;    
    uint256 public earlyAccessWindowOpens;
    ...
    event BurnedForCard(uint indexed indexToBurn, uint indexed indexToMint, address indexed account, uint amount);
    event Purchased(uint indexed index, address indexed account, uint amount);

    constructor(
        string memory _name, 
        string memory _symbol,  
        address _teamTokenReceiver,
        string memory _uri,
        bytes32 _merkleRoot,
        ...
        _mint(_teamTokenReceiver, 0, 200, "");
    } 
    ...
    * 
    * @param _uri the URI of the metadata folder
    */
    function editURI(string calldata _uri) external onlyOwner {
        _setURI(_uri);    
    } 
    ...
    */
    ...
    /**
    * @notice edit windows for purchasing card
    * 
    ...
    */
    function earlyAccessSale(
        ...
        require(!hasPurchased[msg.sender], "Purchase: only one purchase allowed");

        hasPurchased[msg.sender] = true;
        
        bytes32 node = keccak256(abi.encodePacked(index, msg.sender, uint256(0)));
        require(
    ...
    * 
    * @param cardIdToBurn the token id of the card to burn
    * @param amount the amount to burn
    */
    function burnCardForOther(uint256 cardIdToBurn, uint256 amount) external whenNotPaused {
        require(balanceOf(msg.sender, cardIdToBurn) >= amount, "BurnCardForOther: sender does not own required amount");
        require(amount > 0, "need to burn at least one");
        require(block.timestamp >= burnWindowOpens && block.timestamp <= burnWindowCloses, "BurnCardForOther: window closed");
        require(cardIdToBurn < cardIdToMint, "BurnCardForOther: card cannot be burned");

        burn(msg.sender, cardIdToBurn, amount);
       _mint(msg.sender, cardIdToMint, amount, "");

        emit BurnedForCard(cardIdToBurn, cardIdToMint, msg.sender, amount);       
    }
    ...
    function release(address payable account) public override {
        require(msg.sender == account, "Release: no permission");
