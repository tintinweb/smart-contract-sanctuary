...
function CreateRentalAgreement(string calldata Key, uint256 assetID, uint256 termlength, uint256 cost)public {
...
function RentAsset(string memory key) public payable{
...
uint256 devsplit = (msg.value * rentalTax) / 100;

uint256 remain= msg.value - devsplit;


payable(devWallet).transfer(devsplit );
payable(RentOwner[key]).transfer(remain);
...
function EndRental(string memory key) public {
