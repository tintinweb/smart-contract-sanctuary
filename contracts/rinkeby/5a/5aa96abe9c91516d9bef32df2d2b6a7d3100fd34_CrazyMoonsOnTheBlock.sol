...
contract CrazyMoonsOnTheBlock is ERC721Enumerable, Ownable{
    using SafeMath for uint256;
    using Address for address;
    using Strings for uint256;

    string public CMOTB_PROVENANCE  = ""; 
    uint256 public constant crazymoonPrice = 20000000000000000; // 0.08 ETH
    ...
    constructor() ERC721("CrazyMoonsOnTheBlock","CMOTB"){
    }
    ...
    function mintCrazyMoon(uint numberOfTokens) public payable {
        ...
        require(totalSupply().add(numberOfTokens) <= MAX_SUPPLY, "Purchase would exceed max supply of Moons");
        require(numberOfTokens <= MAX_NFT_PURCHASE,"Can only mint up to 10 per purchase");
        require(crazymoonPrice.mul(numberOfTokens) == msg.value, "Sent ether value is incorrect");
    ...
    function setProvenanceHash(string memory provenanceHash) public onlyOwner {
        CMOTB_PROVENANCE = provenanceHash;
    }
