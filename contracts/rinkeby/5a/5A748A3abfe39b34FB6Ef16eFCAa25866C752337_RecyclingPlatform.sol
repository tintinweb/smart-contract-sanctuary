...
contract RecyclingPlatform is Context, AccessControlEnumerable {
    ...
    uint256 private rawTokenPrice = 18000000000000000000000;

    RawToken private rawToken;
    ...
    constructor(IERC20 recyclingToken, RawToken _rawToken) {
        rcl = recyclingToken;
        rawToken = _rawToken;

        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(BOOKKEEPER_ROLE, _msgSender());
    }

    function setExchangePrice(uint256 newPrice) public {
        require(hasRole(BOOKKEEPER_ROLE, _msgSender()), "RecyclingPlatform: must have bookkeeper role to change price");
    ...
    function setRawTokenPrice(uint256 newPrice) public {
        require(hasRole(BOOKKEEPER_ROLE, _msgSender()), "RecyclingPlatform: must have bookkeeper role to change price");
        rawTokenPrice = newPrice;
    }

    function getRawTokenPrice() public view returns (uint256) {
        return rawTokenPrice;
    }

    function buyRawToken() public {
        bool result = rcl.transferFrom(address(msg.sender), address(this), rawTokenPrice);
        if (result) {
            rawToken.mintToken(address(msg.sender),
                rawTokenPrice,
...
ERC721Burnable {
    ...
    struct ResourceToken {
        ...
        address token_creator; // автор созданного токена ( логин/id пользователя с правами _ROOT)
        bool is_resource_token; //2 основных состояния. false - future, true - resource
        bool sold_to_client; // : {false :true}  false--->true; true-x->false //продан токен или нет
    ...
    function mintToken(address to, uint256 base_price, uint256 deposit_price,
        uint256 interest_rate_future, uint256 interest_rate_resource) public {
    ...
    //    function mintToken(address to, uint256 base_price, uint256 deposit_price,
    //        uint256 interest_rate_future, uint256 interest_rate_resource,
    //string test) public {
    //        uint256 tokenId = _tokenIdTracker.current();
    //
    ////        tokenInfo[tokenId].token_id = tokenId;
    ////        tokenInfo[tokenId].token_creator = tx.origin;
    ////        tokenInfo[tokenId].is_resource_token = false;
    ////        tokenInfo[tokenId].sold_to_client = true;
    ////        tokenInfo[tokenId].purchase_date = block.timestamp;
    ////        tokenInfo[tokenId].base_price = base_price;
    ////        tokenInfo[tokenId].deposit_price = deposit_price;
    ////        tokenInfo[tokenId].benefits_period = 31556926;
    ////        tokenInfo[tokenId].interest_rate_future = interest_rate_future;
    ////        tokenInfo[tokenId].interest_rate_resource = interest_rate_resource;
    //
    //        mint(to);
    //    }

    function getTokenInfo(uint256 tokenId) public view returns (ResourceToken memory){
        ResourceToken memory resourceTokenInfo = tokenInfo[0];
        //        ResourceToken memory resourceTokenInfo = ResourceToken(
        //            tokenId,
        //            tx.origin,
        //            false,
    ...
    function mint(address to) public virtual {
        require(hasRole(MINTER_ROLE, _msgSender()), "RawToken: must have minter role to mint");
