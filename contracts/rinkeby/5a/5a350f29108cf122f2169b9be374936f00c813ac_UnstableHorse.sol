...
library SafeMath {
    ...
    // uint public total_No_Horses = 9000;
    // uint public total_No_HorsesRemaining = total_No_Horses;
    // uint public nextTokenId = 1;
    // uint public farmJucies = 1000;
    // uint public farmJuciesRemaining = farmJucies;
    // uint public maxHorsesMintInOnce = 15;
...
contract UnstableHorse is ERC721URIStorage {
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    // horse own; 
    mapping(uint => address) public horse_Owner;
    mapping(uint => bool) public horse_Own;
    address public owner;
    uint public total_No_Tokens = 20;
    uint public total_No_TokensRemaining = total_No_Tokens;
    uint public total_No_Horses = 15;
    uint public total_No_HorsesRemaining = total_No_Horses;
    uint public nextTokenId = 1;
    uint public maxHorsesMintInOnce = 5;
    uint public price = 1000000000000000000;  // 1 ETH
    bool public sale  =  true;  // false
    uint256 public startingIpfsId = 0;
    uint256 private _lastIpfsId = 0;
    uint256 public start = 1;
    address payable fundWallet = payable(0xdD870fA1b7C4700F2BD7f44238821C26f7392148);  
    string private baseUri = "https://gateway.pinata.cloud/ipfs/QmTyGF89MvPoVEYFFDoUgDrA6H7V9dnLqKfcdD7rnpFYKm/";

 

    // farm juice own;
    mapping(address => uint) public farmJucies_Owner;
    mapping(address => bool) public farmJucies_Own;
    uint private FarmJuice_newTokenId = 1000001;
    uint public farmJucies = 5;
    uint public farmJuciesRemaining = farmJucies;
    uint public farmJuciesTransfered = 0;
    string private FarmJuice_baseUri = "https://gateway.pinata.cloud/ipfs/QmTyGF89MvPoVEYFFDoUgDrA6H7V9dnLqKfcdD7rnpFYKm/";



    // Breed section;
    uint public total_No_HorsesBreed = 0;
    uint public StartIndex_HorsesBreed = 100000;
    ...
    constructor() ERC721("The Unstable Horse", "TUH") {
      owner = msg.sender;
    }
    ...
    function mint(uint numberOfMints) public payable {
        ...
        require(numberOfMints <= maxHorsesMintInOnce, "max 15 horses at once");
		...
		for (uint256 i = 0; i < numberOfMints; i++) {
            ...
            if(total_No_Horses == total_No_HorsesRemaining) {
            ...
            require(horse_Own[nextTokenId] != true, "Mint: Token already exist.");
            string memory tokenURI = string(abi.encodePacked(baseUri, uint2str(_lastIpfsId),".json"));
            _mint(msg.sender, nextTokenId);
            _setTokenURI(nextTokenId, (tokenURI));
            horse_Owner [nextTokenId] = msg.sender;
            horse_Own[nextTokenId] = true;
        ...
        //     uint256 newItemId = _tokenIds.current();
            ...
            // string memory tokenURI = string(abi.encodePacked(baseUri, newItemId,".jpg"));
        ...
        // // fundWallet.transfer(msg.value);
        // total_No_TokensSold +=  numberOfMints;
        // total_No_HorsesSold +=  numberOfMints;
    ...
    function getIpfsIdToMint() public view returns(uint256 _nextIpfsId) {
        require(total_No_HorsesRemaining > 0, "All tokens have been minted");
        
        if(_lastIpfsId == total_No_Horses && nextTokenId < total_No_Horses) {
            _nextIpfsId = start;   // 2
        } else if(nextTokenId <= total_No_Horses) {
    ...
    /* Farm Juice */
    function farmJucie_Mint(address player) public {  // there is no fund transfer because only  owner can mint FARM Juices. 
        require(msg.sender == owner, "Only owner can mint");
        require(farmJuciesRemaining > 0, "All farm juices  transfered."); 
        require(farmJucies_Own[player] == false, "User already own farmJucie"); 
        
        total_No_TokensRemaining--;
        farmJuciesRemaining--;
        farmJuciesTransfered++;
        farmJucies_Owner[player] = FarmJuice_newTokenId;
        farmJucies_Own[player] = true;
    ...
    function update_FarmJuice_limit(uint  add_newTokens) public{
        require(msg.sender == owner);
        farmJucies += add_newTokens; 
        farmJuciesRemaining += add_newTokens;
    }


    /* Breed Horse */
    function Breed(address player, string memory uri, uint token1_ID, uint token2_ID) public {
        require(farmJucies_Own[player] == true, "User should own farm Jucie for Breeding"); 
        ...
        _setTokenURI(NextIndex_HorsesBreed, uri);
    ...
    function updateTokensLimit(uint add_newTokens) public{
      require(msg.sender == owner);
      
      start = total_No_Horses + 1;
      _lastIpfsId = total_No_Horses;
      total_No_Tokens += add_newTokens ;
      total_No_Horses += add_newTokens ;
      total_No_TokensRemaining += add_newTokens;
      total_No_HorsesRemaining = add_newTokens;
