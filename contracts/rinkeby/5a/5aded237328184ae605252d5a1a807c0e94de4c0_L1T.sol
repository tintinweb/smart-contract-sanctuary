...
contract L1T is ERC20, Ownable {
    ...
    uint256 public maxDailySellForInvestors = 100000000000 * 10**9; // 0.01%

    uint256 public ETHRewardsFee = 4;
    uint256 public liquidityFee = 3;
    uint256 public marketingFee = 2;
    uint256 public devFee = 2;
    uint256 public dPrizeFee = 1;
    uint256 public mPrizeFee = 1;
    uint256 public yPrizeFee = 1;

    uint256 public totalFees = ETHRewardsFee.add(liquidityFee).add(marketingFee).add(dPrizeFee).add(mPrizeFee).add(yPrizeFee);

    address public marketingWallet = 0x182aaE027a2187fDCd7937EaF7CB7D68C1a89671;
    address public devWallet = 0xE533A12b64025Eb7eDc2eE5a756d1EBBCb7b23C5;
    address public dPrizeWallet = 0x70cE45C2baE7C0A34Bb0f6D984c041d5F6495984;
    address public mPrizeWallet = 0xB48D5BAc74d5d0558433b91EFAABf7A990C30834;
    address public yPrizeWallet = 0x57A6e785409a116C176327976749e38C61f3A64D;
    ...
    function setMaxDailySellForInvestor(uint256 amount) external onlyOwner{
        maxDailySellForInvestors = amount * 10**9;
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "L1T: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            if(marketingFee > 0 || devFee > 0){
                uint256 marketingTokens = contractTokenBalance.mul(marketingFee + devFee).div(totalFees);
    ...
    function swapAndSendToFee(uint256 tokens) private{
        ...
        if(marketingFee > 0) payable(marketingWallet).transfer(newBalance.div(devFee + marketingFee).mul(marketingFee));
        if(devFee > 0) payable(devWallet).transfer(newBalance.div(devFee + marketingFee).mul(devFee));
    }
