...
// File: contracts/v1-mining/interface/ITokenGameFi.sol
...
// File: contracts/v1-mining/interface/IGameFiPoolsV1.sol
...
interface IGameFiPoolsV1 {

    struct RewardInfo {
        uint256 receiveReward;//receiveReward
        uint256 inviteReward;//inviteReward
    ...
    event UpdatePower(uint256 pool, address lp, uint256 totalPower, address indexed owner, uint256 ownerInvitePower, uint256 ownerPledgePower, address indexed upper1, uint256 upper1InvitePower);
    ...
    function setGameFi(ITokenGameFi) external;

    function setInvite(GameFiInviteV1) external;
    ...
    function addPool(string memory, address, uint256, uint256, uint256, uint256, uint256) external returns (bool);
...
// File: contracts/v1-mining/implement/GameFiPoolsV1.sol
...
contract GameFiPoolsV1 is IGameFiPoolsV1 {
    ...
    ITokenGameFi public gameFi;//gameFi contract
    GameFiInviteV1 public invite;//invite contract
    ...
    //uint256 public constant invite1Reward = 15;//Level 1 invite rewards，15%
    uint256 public rewardTotal = 0;//Total mining bonus

    constructor (ITokenGameFi _gameFi, GameFiInviteV1 _invite) {
    ...
    function setGameFi(ITokenGameFi _gameFi) override external {
        _setGameFi(_gameFi);
    }
    
    function _setGameFi(ITokenGameFi _gameFi) internal {
        require(owner == msg.sender, ErrorCode.FORBIDDEN);
        gameFi = _gameFi;
    }

    function setInvite(GameFiInviteV1 _invite) override external {
        require(owner == msg.sender, ErrorCode.FORBIDDEN);
        invite = _invite;
    }
    ...
    function deposit(uint256 _pool, uint256 _amount) override external {
        ...
        address upper1 = invite.inviteUpper1(msg.sender);

        computeReward(_pool);

        provideReward(_pool, poolInfo.rewardPerShare, poolInfo.lp, msg.sender, upper1);

        addPower(_pool, msg.sender, _amount, upper1);

        setRewardDebt(_pool, poolInfo.rewardPerShare, msg.sender, upper1);
    ...
    function withdraw(uint256 _pool, uint256 _amount) override external {
        ...
        address _upper1 = invite.inviteUpper1(msg.sender);
        ...
        if (0 < _amount) {
            subPower(_pool, msg.sender, _amount, _upper1);
        }

        setRewardDebt(_pool, poolInfo.rewardPerShare, msg.sender, _upper1);
    ...
    function addPower(uint256 _pool, address _user, uint256 _amount, address _upper1) internal {
        PoolInfo storage poolInfo = poolInfos[_pool];
        ...
        if (address(0) != _upper1 && poolInfo.invite1Reward > 0) {
        ...
        emit UpdatePower(_pool, poolInfo.lp, poolInfo.totalPower, _user, userInfo.invitePower, userInfo.pledgePower, _upper1, upper1InvitePower);
    }

    function subPower(uint256 _pool, address _user, uint256 _amount, address _upper1) internal {
        PoolInfo storage poolInfo = poolInfos[_pool];
        UserInfo storage userInfo = pledgeUserInfo[_pool][_user];
    ...
    function provideReward(uint256 _pool, uint256 _rewardPerShare, address _lp, address _user, address _upper1) internal {
        ...
        if (0 < userInfo.pendingReward) {
            gameFi.mint(_user, userInfo.pendingReward);
    ...
    function setRewardDebt(uint256 _pool, uint256 _rewardPerShare, address _user, address _upper1) internal {
        UserInfo storage userInfo = pledgeUserInfo[_pool][_user];
    ...
    function addPool(string memory _name, address _lp, uint256 _startBlock, uint256 _rewardTotal, uint256 _rewardPerBlock, uint256 _multiple, uint256 _invite1Reward) override external returns (bool) {
