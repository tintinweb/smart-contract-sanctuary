...
contract AveragePunks is ERC721EnumerableA, Ownable, PaymentSplitter, ReentrancyGuard {
  ...
  function tokenURI(uint tokenId) external view virtual override returns (string memory) {
    ...
    return bytes(_baseTokenURI).length > 0 ? string(abi.encodePacked(_baseTokenURI, tokenId.toString(), _tokenURISuffix)) : 'ipfs://QmWvpPDSv5RWE2uHv3Dh528LNPAWTShAjR38hL9GGxbFQ9';
  }
}
