    ...
    PaymentSplitter
{
    ...
    uint256 _mintPrice;
    uint256 _mintPriceForTen;
    uint256 _maxPurchaseCount;
    string _baseURIValue;
    uint256 _saleStart;
    ERC721[] _presaleEligibleOwnershipContracts;
    ...
    constructor(
        ...
        address[] memory presaleOwnershipContracts,
        address osStorefront
    ) ERC721("Waifu Warriors", "WW") PaymentSplitter(payees, paymentShares) {
        MAX_SUPPLY = 11111;
        _baseURIValue = baseURIVal_;
        _saleStart = saleStart_;
        _mintPrice = 0.07 ether;
        _maxPurchaseCount = 10;

        for (uint256 i = 0; i < presaleOwnershipContracts.length; i++) {
            _presaleEligibleOwnershipContracts.push(
                ERC721(presaleOwnershipContracts[i])
            );
        }
    ...
    function openseaBalanceOf(uint256 cardId) public view returns (uint256) {
        return opensea.balanceOf(msg.sender, cardId);
    }
        ...
        validatePurchasePrice(numberOfTokens)
    {
        if (!saleHasStarted()) {
            if (presaleHasStarted()) {
                require(
                    callerOwnsPresaleTokens(),
                    "You do not own any pre-sale eligible tokens"
                );
            } else {
    ...
    function mintReserved(uint256 numberOfTokens, address to) public onlyOwner {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            _safeMint(to, initialSupply + i);
        }
    }

    function callerOwnsPresaleTokens() public view returns (bool) {
        for (
            uint256 i = 0;
            i < _presaleEligibleOwnershipContracts.length;
            i++
        ) {
            if (
                _presaleEligibleOwnershipContracts[i].balanceOf(msg.sender) > 0
            ) {
                return true;
            }
        }
