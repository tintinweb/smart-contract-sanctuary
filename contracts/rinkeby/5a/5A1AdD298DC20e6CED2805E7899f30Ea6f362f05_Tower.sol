...
contract Tower is ITower, Ownable, ReentrancyGuard, IERC721Receiver, Pausable {
  ...
  function _addWizardToTower(address account, uint256 tokenId) internal whenNotPaused _updateEarnings {
    ...
    numWizardsStaked += 1;
    _staked[account].wizards.add(tokenId);
  ...
  function _addDragonToFlight(address account, uint256 tokenId) internal {
    ...
    _staked[account].dragons.add(tokenId);
  ...
  function _claimWizardFromTower(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    if (unstake) {
      ...
      numWizardsStaked -= 1;
      _staked[stake.owner].wizards.remove(tokenId);
  ...
  function _claimDragonFromFlight(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    if (unstake) {
      ...
      _staked[stake.owner].dragons.remove(tokenId);
  ...
  function onERC721Received(
    ...
    function getStakedTokens(address account) external view returns(uint256[] memory) {
      Staked storage stakedSet = _staked[account];
      uint256[] memory tokenIds = new uint256[] (stakedSet.wizards.length() + stakedSet.dragons.length());

      for (uint256 i; i < stakedSet.wizards.length(); i++) {
        tokenIds[i] = stakedSet.wizards.at(i);
      }
      for (uint256 i; i < stakedSet.dragons.length(); i++) {
        tokenIds[i + stakedSet.wizards.length()] = stakedSet.dragons.at(i);
      }
