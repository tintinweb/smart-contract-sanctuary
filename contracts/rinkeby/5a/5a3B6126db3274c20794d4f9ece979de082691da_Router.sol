...
contract Router {
    ...
    function _swapOnUniswapV2(
        ...
        if (address(path[0]) == address(0)) {
            ...
            require(path[0].approve(address(pair), amount), "Router: approve has failed");
            // path[0].safeTransferFrom(address(this), address(pair), amount);
            TransferHelper.safeTransferFrom(address(path[0]), address(this), address(pair), amount);
        }
