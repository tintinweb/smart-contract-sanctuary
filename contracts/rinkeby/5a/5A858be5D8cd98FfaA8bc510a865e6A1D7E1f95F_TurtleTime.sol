...
contract TurtleTime is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    uint256 public constant TURTLE_PUBLIC = 6;
    uint256 public constant TURTLE_MAX = TURTLE_PUBLIC;
    uint256 public constant PURCHASE_LIMIT = 2;
    uint256 public allowListMaxMint = 2;
    uint256 public constant PRICE = 30_000_000_000_000_000; // 0.03 ETH
    ...
    Counters.Counter private _publicTURTLE;

    constructor() ERC721("Turtle Time", "TURTLE") {}
    ...
    function minting(address to, uint256 numberOfTokens)
        ...
        onlyOwner 
        {
            ...
            require(
                _publicTURTLE.current() < 10000,
                "Purchase would exceed TURTLE_PUBLIC"
            );
            
            for (uint256 i = 0; i < numberOfTokens; i++) {
                uint256 tokenId = _publicTURTLE.current();
    
                if (_publicTURTLE.current() < TURTLE_PUBLIC) {
                    _publicTURTLE.increment();
                    _safeMint(to, tokenId);
                }
            }
    }
    ...
    function purchaseAllowList(uint256 numberOfTokens) external payable {
        ...
        require(
            _publicTURTLE.current() < TURTLE_PUBLIC,
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(
            ...
            'Each address may only have 10 Turtles'
        );
        require(
            numberOfTokens <= PURCHASE_LIMIT,
            "Can only mint up to 10 tokens"
