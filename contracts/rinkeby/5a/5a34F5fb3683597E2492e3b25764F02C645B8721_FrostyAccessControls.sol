...
contract FrostyAuction is Context, ReentrancyGuard {
    ...
    FrostyGarmentNFT public garmentNft;
    ...
    constructor(FrostyAccessControls _accessControls, FrostyGarmentNFT _garmentNft) public {
    ...
    // TODO may need to take in lister is setup is done from a orchestrating contract
        ...
        require(_endTime > _startTime, "FrostyAuction.createAuction: End time must be greater than start");

        // Check another auction not already flight
        require(
            _getNow() > auctions[_garmentTokenId].endTime,
            "FrostyAuction.createAuction: Cannot create an auction in the middle of another"
        );
    ...
    function placeBid(uint256 _garmentTokenId) external payable nonReentrant {
        ...
        require(auction.endTime > 0, "FrostyAuction.placeBid: Auction does not exist");
    ...
    function resultAuction(uint256 _garmentTokenId) external nonReentrant {
        ...
        require(accessControls.hasAdminRole(_msgSender()), "FrostyAuction.resultAuction: Sender must be admin");

        // Check the auction to see if it can be resulted
        Auction storage auction = auctions[_garmentTokenId];

        // Check the auction real
        require(auction.lister != address(0), "FrostyAuction.resultAuction: Auction does not exist");
        ...
        require(winner != address(0), "FrostyAuction.resultAuction: No one has bid");
    ...
    function cancelAuction(uint256 _garmentTokenId) external nonReentrant {
        // TODO check this should be admin and smart contract role check
        // Admin only resulting function
        require(accessControls.hasAdminRole(_msgSender()), "FrostyAuction.cancelAuction: Sender must be admin");

        // Check valid and not resulted
        Auction storage auction = auctions[_garmentTokenId];

        // Check auction is real
        require(auction.lister != address(0), "FrostyAuction.cancelAuction: Auction does not exist");
    ...
    function updateAuctionReservePrice(uint256 _garmentTokenId, uint256 _reservePrice) external {
        require(accessControls.hasAdminRole(_msgSender()), "FrostyAuction.updateAuctionReservePrice: Sender must be admin");
    ...
    function updateAuctionStartTime(uint256 _garmentTokenId, uint256 _startTime) external {
        require(accessControls.hasAdminRole(_msgSender()), "FrostyAuction.updateAuctionStartTime: Sender must be admin");
    ...
    function updateAuctionEndTime(uint256 _garmentTokenId, uint256 _endTime) external {
        require(accessControls.hasAdminRole(_msgSender()), "FrostyAuction.updateAuctionEndTime: Sender must be admin");
    ...
    function updateAccessControls(FrostyAccessControls _accessControls) external {
        require(accessControls.hasAdminRole(_msgSender()), "FrostyAuction.updateAccessControls: Sender must be admin");
        accessControls = _accessControls;
    }
...
contract FrostyGarmentNFT is ERC721("Frosty", "DTX") {
    ...
    constructor(FrostyAccessControls _accessControls) public {
        accessControls = _accessControls;
    }

    //todo: maybe we can drop mint? just use safe mint under the hood like the genesis
    /**
     @notice Mints a FrostyGarmentNFT but does not check if the beneficiary is a 721 compatible contract
    ...
    function safeMint(address _beneficiary, string calldata _tokenUri, address _designer) external returns (uint256) {
        require(
            accessControls.hasSmartContractRole(_msgSender()) || accessControls.hasMinterRole(_msgSender()),
            "FrostyGarmentNFT.safeMint: Sender must have the minter or contract role"
        );
    ...
    function setTokenURI(uint256 _tokenId, string calldata _tokenUri) external {
        require(accessControls.hasAdminRole(_msgSender()), "FrostyGarmentNFT.setTokenURI: Sender must have the admin role");
    ...
    function assertMintingParamsValid(string calldata _tokenUri, address _designer) pure private {
        require(bytes(_tokenUri).length > 0, "FrostyGarmentNFT.assertMintingParamsValid: Token URI is empty");
        require(_designer != address(0), "FrostyGarmentNFT.assertMintingParamsValid: Designer is zero address");
    }
}
...
contract FrostyMaterials is ERC1155 {
    using SafeMath for uint256;
    ...
    constructor(string memory _name, string memory _symbol, FrostyAccessControls _accessControls) public {
        name = _name;
        symbol = _symbol;
        accessControls = _accessControls;
    ...
    function createStrand(
        ...
        require(accessControls.hasMinterRole(_msgSender()), "FrostyMaterials.createStrand: Sender must be minter");
        require(_initialSupply > 0, "FrostyMaterials.createStrand: No initial supply");
        require(bytes(_uri).length > 0, "FrostyMaterials.createStrand: URI is a blank string");
    ...
    function batchCreateStrands(
        ...
        require(accessControls.hasMinterRole(_msgSender()), "FrostyMaterials.batchCreateStrands: Sender must be minter");
        require(_initialSupplies.length == _uris.length, "FrostyMaterials.batchCreateStrands: Array lengths are invalid");
        require(_initialSupplies.length > 0, "FrostyMaterials.batchCreateStrands: No data supplied in arrays");
        ...
        for(uint i = 0; i < _initialSupplies.length; i++) {
            ...
            require(initialSupply > 0, "FrostyMaterials.batchCreateStrands: No initial supply");

            string memory uri = _uris[i];
            require(bytes(uri).length > 0, "FrostyMaterials.batchCreateStrands: URI is a blank string");
    ...
    function mintStrand(uint256 _strandId, uint256 _amount, address _beneficiary) external {
        require(accessControls.hasMinterRole(_msgSender()), "FrostyMaterials.mintStrand: Sender must be minter");
        require(bytes(tokenUris[_strandId]).length > 0, "FrostyMaterials.mintStrand: Strand does not exist");
        require(_amount > 0, "FrostyMaterials.mintStrand: No amount specified");
    ...
    function batchMintStrands(uint256[] calldata _strandIds, uint256[] calldata _amounts, address _beneficiary) external {
        require(accessControls.hasMinterRole(_msgSender()), "FrostyMaterials.batchMintStrands: Sender must be minter");
        require(_strandIds.length == _amounts.length, "FrostyMaterials.batchMintStrands: Array lengths are invalid");
        require(_strandIds.length > 0, "FrostyMaterials.batchMintStrands: No data supplied in arrays");

        // Check the strands exist and no zero amounts
        for(uint i = 0; i < _strandIds.length; i++) {
            uint256 strandId = _strandIds[i];
            require(bytes(tokenUris[strandId]).length > 0, "FrostyMaterials.batchMintStrands: Strand does not exist");

            uint256 amount = _amounts[i];
            require(amount > 0, "FrostyMaterials.batchMintStrands: Invalid amount");
        }
...
import "../FrostyAuction.sol";

contract FrostyAuctionMock is FrostyAuction {
    uint256 public nowOverride;

    constructor(
        FrostyAccessControls _accessControls,
        FrostyGarmentNFT _garmentNft
    )
    FrostyAuction(_accessControls, _garmentNft)
    public {}
...
contract FrostyGenesisNFTMock is FrostyGenesisNFT {
    uint256 public nowOverride;
    ...
    FrostyGenesisNFT(_accessControls, _fundsMultisig, _genesisStart, _genesisEnd, _tokenURI)
    public {}
