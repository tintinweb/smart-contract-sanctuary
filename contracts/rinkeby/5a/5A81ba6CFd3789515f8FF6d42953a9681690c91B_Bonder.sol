...
contract Bonder is Ownable {
    ...
    // FIXME: using an uint32 is more memory-efficient
    uint256 public bondingTime;
    ...
    function _bond(uint256 _amount) internal {
        Worker storage _worker = worker[_msgSender()];
    ...
    function consolidateBond() external {
        ...
        totalBonded += _worker.bonding;
        _worker.bonded += _worker.bonding;
        _worker.bonding = 0;
    }
    ...
    function cancelBonding() external {
        ...
        _worker.bonding = 0;
        IERC20(nativeToken).safeTransfer(_msgSender(), _refundedAmount);
    }

    function accrueReward(address _address, uint256 _amount) external {
        if (_msgSender() != jobsRegistry) revert Forbidden();
        Worker storage _worker = worker[_address];
        totalBonded += _amount;
        _worker.bonded += _amount;
    ...
    function consolidateUnbonding() external {
        Worker storage _worker = worker[_msgSender()];
        if (_worker.unbonding == 0) revert NothingToWithdraw();
        if (_worker.unbondingTimestamp > block.timestamp)
            revert NothingToConsolidate();
        uint256 _unbonded = _worker.unbonding;
        totalBonded -= _unbonded;
        _worker.bonded -= _unbonded;
        _worker.unbonding = 0;
        if (_worker.bonded == 0) _worker.activationTimestamp = 0;
        IERC20(nativeToken).safeTransfer(_msgSender(), _unbonded);
    }
