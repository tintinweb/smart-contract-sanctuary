...
contract TestNFTRewards is ERC721, Ownable {
    ...
    string public PROVENANCE_HASH = "HASH";
    // PreSale
    ...
    uint256 public COMMUNITY_VAULT_PERCENT = 70;
    ...
    uint256 constant private RewardsDividendRate = 24;
    uint256 constant private RewardsInitialIssuance = 1440;
    uint256 public REWARD_UPGRAD_PRICE = 777000000000000000000;
    event rewardsMint(address indexed user, uint256 reward);
    event rewardsBurn(address indexed user, uint256 reward);
    ...
    */
    function setVaultAdress(address _vault) public onlyOwner {
        ...
        COMMUNITY_VAULT = address (_vault);
    }

    function setVaultPercent(uint256 _percentValue) public onlyOwner {
        COMMUNITY_VAULT_PERCENT = _percentValue;
    }

    function withdraw() external{
        require(msg.sender == owner() || msg.sender == address(devAddress) || msg.sender == address(COMMUNITY_VAULT), "Not team!");
        uint256 balance = address(this).balance;
        require(COMMUNITY_VAULT != address(0), "Vault address not found");
        require(balance > 0, 'Insufficient balance');
        uint256 vaultBalance = balance.mul(COMMUNITY_VAULT_PERCENT).div(100);
        uint256 devBalance = balance.sub(vaultBalance);
        (bool withdrawVault, ) = payable(COMMUNITY_VAULT).call{value: vaultBalance}("");
    ...
    */
    ...
    function setRewardDividend(uint _id, uint256 _amount) public  {
        ...
        emit rewardsBurn(msg.sender, tokenAmount);
    }
    ...
    function claimRewards(uint _id) public returns (uint256) {
        ...
        if (totalSpent > 0) {
            ...
            emit rewardsMint(ownerOf(_id), totalSpent);
        }
        return totalSpent;
    }
    ...
    function claimAllRewards(address _address) public returns (uint256) {
        ...
        if (totalSpent > 0) {
            ...
            emit rewardsMint(_address, totalSpent);
        }
        return totalSpent;
    }
