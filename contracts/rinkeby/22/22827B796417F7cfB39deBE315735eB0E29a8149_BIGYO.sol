...
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
  ...
  // Counters.Counter public bearStolen;
  // Counters.Counter public fisherStolen;
...
interface IWoolf {
  struct SheepWolf {bool isSheep; uint8[14] traitarray; uint8 alphaIndex;}
...
contract BIGYO is IWoolf, ERC721Enumerable, Ownable, Pausable, VRFConsumerBase {
  using Counters for Counters.Counter;
  // using EnumerableSet for EnumerableSet.UintSet; 
  Counters.Counter public fisherMenMinted;
  Counters.Counter public bearsMinted;
  ...
  mapping(uint256 => SheepWolf) public tokenTraits;                       // mapping from tokenId to a struct containing the token's traits
  ...
  IBarn public barn;                                                       // STAKING - reference to the Barn for choosing random Wolf thieves
  IWool public wool;                                                       // TOKEN - reference to $WOOL for burning on mint
  ITraits public traits;                                                   // TRAITS - reference to Traits

  // Team Wallets

  address private project_wallet = 0x77a45d5BD81916901474ef1a162b34D8FAaE1030;
	address private Bear1 = 0x06e8198A5a4AB3E5F4B13DdC9e5c2FCDDD4f8838;
  ...
  address private Bear4 = 0x11360F0c5552443b33720a44408aba01a809905e;
  ...
  uint256 internal vrfcooldown = 10000;
  Counters.Counter public vrfReqd;




  constructor(address _wool, uint256 _maxTokens, address _vrfCoordinator, address _link) 
      ERC721("SALTEST", 'SALTEST') 
  ...
  function mint(uint256 amount, bool stake) external payable whenNotPaused {
    ...
    uint256 totalWoolCost = 0;                                                          // $SALMON Cost to mint. 0 is Gen0
    ...
    if (totalWoolCost > 0) wool.burn(msgSender, totalWoolCost);
    if (stake) barn.addManyToBarnAndPack(msgSender, tokenIds);
  }
  ...
  function generate(uint256 tokenId, uint256 seed) internal returns (SheepWolf memory t) {
    getRandomChainlink();
    t = selectTraits(seed);
    if (existingCombinations[structToHash(t.isSheep, t.traitarray, t.alphaIndex)] == 0) {
      tokenTraits[tokenId] = t;
      existingCombinations[structToHash(t.isSheep, t.traitarray, t.alphaIndex)] = tokenId;
      return t;
    }
  ...
  function selectTraits(uint256 seed) internal view returns (SheepWolf memory t) {    
    ...
    uint8 shift = t.isSheep ? 0 : 9;                                          // 0 if its a sheep, 9 if its Wolf
    ...
    if (t.isSheep) {
      ...
      // uint8[] memory t.traitarray = new uint8[](14); // this is fine
      ...
      // t.traitarray = t.traitarray;
      seed >>= 16;
      t.alphaIndex = selectTrait(uint16(seed & 0xFFFF), 4 + shift);
  ...
  function structToHash(bool isSheep, uint8[14] memory traitarray, uint8 alphaIndex) internal pure returns (uint256) {
    return uint256(bytes32(abi.encodePacked(isSheep, traitarray, alphaIndex)));
  }

  // Select who the NFT goes to --- The first 20% (ETH purchases) go to the minter & the remaining 80% have a 10% chance to be given to a random staked wolf
  ...
  // Set Interfaces
  function setInit(address _barn, address erc20Address, address _traits ) public onlyOwner {
    barn = IBarn(_barn);
    wool = IWool(erc20Address);
    // wool = IERC20(_wool);
  ...
  function getRandomChainlink() internal returns (bytes32 requestId) {
    ...
    if (vrfReqd.current() % vrfcooldown != 0) {
