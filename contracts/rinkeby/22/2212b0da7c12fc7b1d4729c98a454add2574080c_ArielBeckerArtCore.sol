...
contract ArielBeckerArtCore is Context, AccessControlEnumerable, ERC721Enumerable, ERC721Burnable, ERC721Pausable {
	...
	string private _baseTokenURI = "https://arielbecker.com/foo";
	...
	function mint(string memory _name, bytes memory _data, uint256 _documentType, string memory _json) public virtual {
		require(hasRole(MINTER_ROLE, _msgSender()), "ArielBeckerArtCore: must have minter role to mint");
		if(_data.length < 1) revert("We need some data, chump!");
		if(_data.length > 524288) revert("Too much data, chump!");

		beckers.push(TokenStruc(_name, _data, msg.sender, block.timestamp, _documentType, _json));
	...
	function retrieveContent(uint256 _id) public view returns (bytes memory) {
		...
		return beckers[_id].data;
	}

	function retrieveMetadata(uint256 _id) public view returns (string memory) {
		require(_id < 0, "Token ID cannot be less than zero!");
		return beckers[_id].json_attributes;
	}

	function pause() public virtual {
		require(hasRole(PAUSER_ROLE, _msgSender()), "ArielBeckerArtCore: must have pauser role to pause");
		_pause();
	}

	function unpause() public virtual {
		require(hasRole(PAUSER_ROLE, _msgSender()), "ArielBeckerArtCore: must have pauser role to unpause");
		_unpause();
	}
	...
	function setbaseTokenURI(string memory _uri) public {
		require(hasRole(MINTER_ROLE, _msgSender()), "ArielBeckerArtCore: must have minter role to set this parameter!");
		_baseTokenURI = _uri;
	}
