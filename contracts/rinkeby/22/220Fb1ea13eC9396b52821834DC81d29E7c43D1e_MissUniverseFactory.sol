...
import "./MissUniverse.sol";

contract MissUniverseFactory is Ownable {
    address[] public allMisses;

    event NewMissUniverse(
        string name,
        string symbol,
        address missAny,
        IERC20 token,
    ...
    function createMissUniverse(
        ...
        uint256 saleEndTime,
        address owner
    ) external onlyOwner returns (address newMiss) {
        bytes memory bytecode = type(MissUniverse).creationCode;
        bytes32 salt = keccak256(
            abi.encodePacked(allMisses.length, name, symbol)
        );

        assembly {
            newMiss := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        MissUniverse(newMiss).initialize(
            name,
            symbol,
            baseURI,
            saleToken,
        ...
        allMisses.push(address(newMiss));

        emit NewMissUniverse(name, symbol, newMiss, saleToken, price, maxNft);

        return address(newMiss);
    }
}
...
import "./interfaces/IMissFactory.sol";

contract MissUniverse is
    ERC721,
    ERC721Enumerable,
    ERC721URIStorage,
    Pausable,
    Ownable
{
    ...
    constructor() ERC721("", "") {
        factory = IMissFactory(msg.sender);
    }
    ...
    function initialize(
        ...
        require(saleEndTime_ > 0, "Invalid end time");
        _name = name_;
        _symbol = symbol_;
...
interface IMissFactory {
    function createMissUniverse() external returns (address pair);
}
