...
// File: contracts/pool/3rd/Pool.sol
...
contract Pool is Ownable, Pausable{
    ...
    constructor(address _lpt, address _rewardToken, uint _rewardRate) public {
        lpt = _lpt;
    ...
    function pendingReward(address _user) external view returns(uint) {
        ...
        if (block.timestamp > lastUpdateTime && totalSupply != 0) {
            ...
            rewardPerToken = rewardPerToken.add(rewardAdd.mul(1e18).div(totalSupply));
        }
        ...
        if (user.amount > 0) {
            pending = pending.add(user.amount.mul(rewardPerToken).div(1e18).sub(user.rewardDebt));
        }
        return pending;
    }
    ...
    function updatePool() public {
        ...
        uint rewardAdd = rewardRate.mul(block.timestamp.sub(lastUpdateTime));
        rewardPerTokenStored = rewardPerTokenStored.add(rewardAdd.mul(1e18).div(totalSupply));
        lastUpdateTime = block.timestamp;
    }
    ...
    function deposit(uint _amount) public whenNotPaused {
        ...
        if (_amount > 0) {
            IERC20(lpt).safeTransferFrom(msg.sender, address(this), _amount);
            totalSupply = totalSupply.add(_amount);
            user.amount = user.amount.add(_amount);
        }

        user.rewardDebt = user.amount.mul(rewardPerTokenStored).div(1e18);
    ...
    function withdraw(uint _amount) external {
        ...
        if (_amount > 0) {
            ...
            IERC20(lpt).safeTransfer(msg.sender, _amount);
        }
        user.rewardDebt = user.amount.mul(rewardPerTokenStored).div(1e18);
    ...
    function _getReward(bool _f) internal {
        ...
        uint pending = user.rewardPending;
        if (user.amount > 0) {
            pending = pending.add(user.amount.mul(rewardPerTokenStored).div(1e18).sub(user.rewardDebt));
        }
    ...
    function claimUnsupportToken(address _token, address _to, uint _amount) external onlyOwner {
        require(_token != rewardToken && _token != lpt, "!safe token");
