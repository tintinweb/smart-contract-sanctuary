...
contract POLKADOG is Context, IERC20, IERC20Metadata, Pausable, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (block.timestamp <= (_deploy_timstamp + _max_days)) {
            require(amount <= (_initial_total_supply * (_transaction_max / 10**2 )) / 10**2, "ERC20: Transfer amount exceeds max amount");
            require(recipientBalance + amount <= (_initial_total_supply * (_transaction_max / 10**2)) / 10**2, "ERC20: Recipient amount exceeds max amount");
        }
    ...
    function _transferStandard(address sender_address, address recipient_address, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tFee, uint256 burn_amount) = _getTValues(tAmount);
        uint256 currentRate =  _getRate();
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = totalAmount - tFee - burn_amount;
        return (tTransferAmount, tFee, burn_amount);
    }
