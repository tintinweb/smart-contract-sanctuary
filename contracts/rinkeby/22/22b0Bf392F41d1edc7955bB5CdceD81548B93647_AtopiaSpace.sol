...
contract AtopiaSpace is ERC721, Ownable {
	...
	constructor(address _atopia) ERC721("Atopia Center", "ATPCENTER") {
	...
	function addCenter(address center) external onlyOwner {
	...
	function enter(uint256 tokenId, uint256 centerId) external onlyTokenOwner(tokenId) returns (uint256 life) {
		life = centers[centerId - 1].enter(tokenId);
		atopia.setLife(tokenId, life);
	}

	function exit(uint256 tokenId, uint256 centerId) external onlyTokenOwner(tokenId) returns (uint256 grown) {
		grown = centers[centerId - 1].exit(tokenId);
		atopia.exitCenter(tokenId, address(centers[centerId - 1]), grown);
	}
	...
	function work(
		...
		if (package > 0) {
			uint256 life = centers[centerId - 1].work(tokenId, package);
			atopia.setLife(tokenId, life);
		} else {
			centers[centerId - 1].work(tokenId, package);
			atopia.setLife(tokenId, 0);
		}
	}
