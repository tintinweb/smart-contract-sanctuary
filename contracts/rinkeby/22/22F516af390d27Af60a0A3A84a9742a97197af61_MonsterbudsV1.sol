...
abstract contract MonsterBudHolders is OwnableUpgradeable{
    ...
    function addPPPUserStatus(address[485] calldata _pppUser, bool _status) onlyOwner external {
        for(uint i = 0; i < 485; i++){
...
contract MonsterbudsV1 is MonsterBudHolders, ERC721URIStorageUpgradeable, IMonsterBuds {
    ...
    function initialize() public initializer {
        ...
        feeMargin = 5; // 5% fee of total amount
        tokenValue = 70874000000000000; // tokenvalue 0.070000
        feeSKTWallet = payable(0x4a1fD5328A255aF50e63c9f978141a46beF8880C); // SKT fee wallet
        SmartContractCommunity = payable(0x391571d2adabDd73eb257061374474a88626F88A); // community wallet
    }
    ...
    */
    ...
    function buyCollectiable(uint256 token_id) external override payable returns(bool){
        ...
        payable(feeSKTWallet).transfer(feeAmount); // transfer 2.5% ethers of msg.value to skt fee wallet
        payable(SmartContractCommunity).transfer(feeAmount); // transfer 2.5% ethers of msg.value to commuinty
     ...
     * - `req_token_id` must be pass.
     * - `accept_token_id` must be pass.
     * - `breed_req_id` must be pass.
     *
     * Returns
     * - new token count.
     *
