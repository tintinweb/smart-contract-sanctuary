...
contract HarbergerAsset is ERC721URIStorage, ReentrancyGuard {
   ...
   * `prevListingTimestamp` Timestamp of the previous listing price set by the current owner
   ...
   * - `prevListingTimestamp` must be greater than 24 hours from current time.
  ...
  function listAssetForSaleInWei(uint64 _tokenId, uint256 _priceAmount) public validToken(_tokenId) onlyOwner(_tokenId) {
    ...
    /* require(assets[_tokenId].prevListingTimestamp <= block.timestamp || assets[_tokenId].totalDepositAmount == 0, "You must wait 24 hours before relisting the asset"); */
    require(
    ...
    assets[_tokenId].prevListingTimestamp = block.timestamp;
