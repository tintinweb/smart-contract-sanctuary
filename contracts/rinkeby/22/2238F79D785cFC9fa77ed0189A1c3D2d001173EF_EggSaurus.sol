...
interface IImage {
  function generateSVG(uint256 id, uint256 saurusHat, uint256 saurusCollar) external view returns(string memory);
...
contract EggSaurus is ERC721, ERC721Enumerable, Ownable {
  ...
  uint256 private constant _CLEAN_UP_PRICE = 50 * (10 ** 18);
  uint256 private constant _REVIVE_PRICE = 500 * (10 ** 18);
  uint256 private constant _HAT_PRICE = 1000 * (10 ** 18);
  uint256 private constant _COLLAR_PRICE = 60 * (10 ** 18);
  ...
  address public devAddress = 0x1cEa7d8Cf909FF8B08D9c69c7f6c9fb010C9a78b;//あとで
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
      ...
      string memory json = Base64.encode(bytes(string(abi.encodePacked(
        ...
        '","image_data":"',
        bytes(svgData),
        '","attributes": [{"display_type": "number","trait_type":"Age","value":',
        uint2str(returnAge(tokenId)),
        '},{"display_type":"number","trait_type":"Virus","value:"',
        uint2str(returnPoopNum(tokenId)),
        '}]}'
      ))));
  ...
  function cleanUp(uint256 tokenId) external {
    ...
    ICoin(_coinAddress).transferFrom(msg.sender, address(this), _CLEAN_UP_PRICE);

    _cleanedUp[tokenId] = block.timestamp;
    ICoin(_coinAddress).burn(_CLEAN_UP_PRICE);
  }
  ...
  function buyHat(uint256 tokenId) external {
    ...
    ICoin(_coinAddress).transferFrom(msg.sender, address(this), _HAT_PRICE);

    _hat[tokenId] = block.timestamp;

    ICoin(_coinAddress).burn(_HAT_PRICE);
  }
  ...
  function whitelistMint(uint256 nftNum, uint256 totalAllocation, bytes32 leaf, bytes32[] memory proof) external payable {
    ...
    if(!whitelistUsed[msg.sender]){        
        ...
        //require(verify(merkleRoot, leaf, proof), "invalid");
  ...
  function returnImage(uint256 tokenId) public view returns (string memory) {
    ...
    return IImage(_imageAddress).generateSVG((_randomNum - 1).mul(1000).add(tokenId), _hat[tokenId], _collar[tokenId]);
  }
...
//
  ...
  function makeRandomNum() external onlyOwner{
    ...
    _randomNum = block.timestamp % 3 + 1;//あとで
  }
  ...
  function setMerkleRootAndSaleTime(bytes32 merkleRoot_, uint256 whitelistSaleStart_, uint256 saleStart_) public onlyOwner {
