...
contract Traits is Ownable, ITraits {
  ...
  function drawSVG(uint256 tokenId) internal view returns (string memory) {
    ...
    uint8 shift = s.isCat ? 0 : 8;

    // Trait data indexes 0 - 7 are reserved for Cats
    // Trait data indexes 8 - 15 are reserved for Mouse.
    string memory svgString = string(abi.encodePacked(
      ...
      drawTrait(traitData[4 + shift][s.neck]),
      drawTrait(traitData[5 + shift][s.mouth]),
      drawTrait(traitData[6 + shift][s.tail]),
      drawTrait(traitData[7 + shift][s.wand])
    ));
  ...
  function compileAttributes(uint256 tokenId) internal view returns (string memory) {
    ...
    if (s.isCat) {
      traits = string(abi.encodePacked(
        ...
        attributeForTypeAndValue(_traitTypes[6], traitData[6][s.tail].name),',',
        attributeForTypeAndValue(_traitTypes[7], traitData[7][s.wand].name),','
      ));
    } else {
      traits = string(abi.encodePacked(
        attributeForTypeAndValue(_traitTypes[0], traitData[8][s.body].name),',',
        attributeForTypeAndValue(_traitTypes[1], traitData[9][s.head].name),',',
        attributeForTypeAndValue(_traitTypes[2], traitData[10][s.spell].name),',',
        attributeForTypeAndValue(_traitTypes[3], traitData[11][s.eyes].name),',',
        attributeForTypeAndValue(_traitTypes[4], traitData[12][s.neck].name),',',
        attributeForTypeAndValue(_traitTypes[5], traitData[13][s.mouth].name),',',
        attributeForTypeAndValue(_traitTypes[6], traitData[14][s.tail].name),',',
        attributeForTypeAndValue(_traitTypes[7], traitData[15][s.wand].name),','
      ));
    }
    return string(abi.encodePacked(
      '[',
