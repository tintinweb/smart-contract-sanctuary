...
interface ITokensFarm {
    ...
    function withdrawBondAllocation(address _beneficiary) external;
    function bond() external;
    function bond(uint256 _amount) external;
        function initializeBond(
            address _bondingToken,
            uint256 _totalBondThreshold,
            address _pricingContract,
            uint256 _bondFeePercent,
            address _daoAddress,
            bool _isEtherBond
    ) external;
...
// File: BondsFarmFactory.sol

contract BondsFarmFactory is Initializable, Ownable {
    using SafeMath for uint256;
        ...
        onlyOwner
    {
     ...
     {
         ITokensFarm(_bondAddress).initializeBond(
            _bondingToken,
            _totalBondThreshold,
            _pricingContract,
            _bondFeePercent,
    ...
    function withdrawBondAllocation(
        address farmAddress,
        address _beneficiary
    )
        external
        onlyOwner
    {
        ...
        ITokensFarm tokensFarm = ITokensFarm(farmAddress);
        tokensFarm.withdrawBondAllocation(_beneficiary);
    }
