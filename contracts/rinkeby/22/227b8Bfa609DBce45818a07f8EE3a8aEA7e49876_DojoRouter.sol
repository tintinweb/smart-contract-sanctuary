...
contract DojoRouter {
    ...
    function swapTokensForETH(
        ...
        dojo.initiateTrade{value: amounts[0]}(belt);
...
contract DegenDojo is ERC20, VRFConsumerBase, Ownable {
    ...
    //winners list where [0] = biggest winner, [1] = most recent win
    ...
    function initiateSetHouse(address _house) public onlyOwner {
        //hosue can only be updated after 86,400 blocks (~3 days on BSC)
    ...
    function setHouse() public onlyOwner {
        ...
        require(block.number >= nextHouse.changeBlock);
    ...
    function setRewardsRate(uint256 rewardRate) public onlyOwner {
        ...
        require(rewardRate < rewardsPerBlock);
    ...
    function _spinJackpot(
        ...
        if (winnings > winners[0].amount) {
            winners[0] = Winner(msg.sender, winnings);
        }
        //update the recent winner
        winners[1] = Winner(msg.sender, winnings);
    }
}
...
contract DojoHouse is ERC20("DojoLiquidity", "DLP") {
     ...
     * For whitebelt = 50% 0.5x, 25% 1.24x, 13% 1.4x, 8% 1.65x, 4% 2x
     * For bluebelt  = 67% 0.5x, 22% 1.5x , 7%  2x  , 3% 3.3x , 1% 5x
     ...
     * If they win, depending on type, user will get back either 2x,3x,4x
     * House edge is always 4-5% with a portion going as follows:
     *  -1% fee given to Treasury
     *  -1% fee given to DojoBar
     */
