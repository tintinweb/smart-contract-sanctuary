...
contract nft is ERC721Enumerable, Ownable {
    ...
    uint public constant mintCost_ = 0.03 ether;
    ...
    constructor() ERC721('Assholios', 'ASS') {
        ...
        _currentBaseURI = 'ipfs://QmRig42S2G9ZQXm9hgyDXfd6Mqe9qQ76NcVEYjbFogTxDs/';
    ...
    */
    ...
    function mint(uint256 number) public payable {
        ...
        require(totalSupply() + number <= maxTotalTokens - (maxReservedMints - reservedMints), "Not enough NFTs left to mint..");
        require(msg.value >= mintCost() * number, 'Insufficient Funds to mint this number of NFTs!');
        require(mintsPerAddress[msg.sender] + number <= maxMints, 'Not allowed to mint this number of NFTs, max amount per address is 3!');
    ...
    function wlMint(uint256 number, uint8 _v, bytes32 _r, bytes32 _s) onlyValidAccess(_v,  _r, _s) public payable {
        require(saleState() != State.NoSale, "Sale in not open yet!");
        require(totalSupply() + number <= maxTotalTokens - (maxReservedMints - reservedMints), "Not enough NFTs left to mint..");
        require(msg.value >= mintCost() * number, 'Insufficient Funds to mint this number of NFTs!');
        require(mintsPerAddress[msg.sender] + number <= maxMints, 'Not allowed to mint this number of NFTs, max amount per address is 3!');
    ...
    function reservedMintCreator(uint256 number) public onlyOwner {
        require(saleState() != State.NoSale, "Sale in not open yet!");
        require(reservedMints + number <= maxReservedMints, "Not enough Reserved NFTs for Creator left to mint..");
        for (uint256 i = 0; i < number; i++) {
            uint256 tid = tokenId();
            _safeMint(msg.sender, tid);
            mintsPerAddress[msg.sender] += 1;
    ...
    //to add more NFTs to the collection
    function addToSupply(uint number) public onlyOwner {
        maxTotalTokens += number;
    }
