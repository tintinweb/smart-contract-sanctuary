...
contract AutoStake is ERC20, Ownable {
    ...
    bool public feeEnabled = false; // should be true after adding liquidity
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if(feeEnabled && !feeless[sender] && !feeless[recipient])
            amountToTransfer = amount.sub(fee);
        ...
        handleStakeTransfer(sender, recipient, amount, amountToTransfer);
        
        emit Transfer(sender, recipient, amount);
    }
    
    function handleStakeTransfer(address from, address to, uint256 fromAmount, uint256 toAmount) internal {
        if(!rewardless[from]) {
            vault.unstake(from,fromAmount); // remove stake, this will also withdraw all rewards
            vault.withdraw(to); // withdraw any rewards of recipient
        }
        if(!rewardless[to]) {
            vault.stake(to,toAmount); // transfer stake
        }
    }
