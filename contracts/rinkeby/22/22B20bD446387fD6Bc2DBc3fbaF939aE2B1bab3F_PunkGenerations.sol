...
contract PunkGenerations {
    ...
    function breedPunkAssets(uint16 fatherIndex, uint16 motherIndex) internal returns (bytes memory childAssets) {
        ...
        byte punkType = byte((info.isMale ? 1 : 5) + uint8(low + (random() % (high + 1 - low))));
    ...
    function mintChildPunks(uint16 fatherIndex, uint16 motherIndex) external {
        ...
        require(child1Mapping[fatherIndex] == 0, "Father already has minted children");
        require(child1Mapping[motherIndex] == 0, "Mother already has minted children");
    ...
    function colorFromGeneration(uint16 index) internal pure returns (bytes4) {
        uint32[9] memory colors = [
            0x00000000, 0x9CB5FEFF, 0x39D27DFF, 
            0xFC9144FF, 0xFD9DCBFF, 0xFA3C6AFF, 
            0xA070DDFF, 0xFAD121FF, 0xDEC5A8FF];
        return bytes4(colors[index < colors.length ? index : colors.length - 1]);
    }

    string private constant SVG_HEADER = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" version="1.2" viewBox="0 0 24 24">';
