...
contract FeistyPrint is ERC721Enumerable, Ownable, Pausable {
  ...
  function printMultiple(uint256 count) public whenNotPaused {
  ...
  function redeemMultiple(uint256 count) public whenNotPaused {
    require(count <= 10, "FeistyPrint: max 10 prints");
    uint256 tokenCount = balanceOf(_msgSender());
    require(tokenCount > 0, "FeistyPrint: sender doesn't have a print");
    require(tokenCount <= 10, "FeistyPrint: sender doesn't have enough prints");

    for (uint i = 0; i < count; i++) {
      uint256 tokenToRedeem = tokenOfOwnerByIndex(_msgSender(), tokenCount - 1 - i);
      _burn(tokenToRedeem);
      _token.transfer(_msgSender(), _tokenPrices[tokenToRedeem]);
    }
  }
  // -- Functions -- //
}
