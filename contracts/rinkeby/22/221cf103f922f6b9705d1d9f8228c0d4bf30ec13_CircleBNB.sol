...
contract CircleBNB {
    ...
    uint256 public DONATE_MIN_AMOUNT = 0.05 ether;
    ...
    function donate(address referrer) public payable {



        require(!isContract(msg.sender) && msg.sender == tx.origin);
        require(msg.value >= DONATE_MIN_AMOUNT,'Min invesment');
        ...
        totalDonated = totalDonated.add(msgValue);
    ...
    function update_donate(uint256 _amount) external {

        require(msg.sender == admin, 'permission denied!');
        DONATE_MIN_AMOUNT = _amount;
    }
    ...
    function contractInfo() view external returns(uint256 _total_users, uint256 _total_deposited, uint256 _total_withdraw) {
        return (totalUsers, totalDonated, totalWithdrawn);
    }
