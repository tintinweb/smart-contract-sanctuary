...
contract SLNFT is ContextMixin, ERC721Enumerable, NativeMetaTransaction, Ownable {
    ...
    uint8 private _preMintSupply = 10;

    address proxyRegistryAddress;
    uint256 private _currentTokenId = 0;
    uint256 _mintFee = 60000000000000000;
    
    bool _saleActive = false;
    uint16 _maxSupply = 100;
    
    uint8 _maxMintQuantity = 10;
    
    string _name = "TEST NFT";
    string _symbol = "TSLNFT";
    string _baseTokenURI = "https://creatures-api.opensea.io/api/creature/";
    
    uint8 _maxWhitelistQuantity = 2;
    ...
    */
    ...
    function mintTo(address _to, uint8 quantity) public payable {
        ...
        require(quantity > 0 && quantity <= _maxMintQuantity, "Quantity exceeds minting maximum!");
        ...
        if(!_saleActive && _whitelist[_to].isWhitelisted) {
            require(_whitelist[_to].mintCount < _maxWhitelistQuantity, "Already minted all whitelist spots");
        }
    ...
    */
    ...
    /**
     * @dev transfers contract funds to desired address
     */
    function withdrawFunds(uint256 _amount) public onlyOwner {
        require(balance() >= _amount, "Not enough funds");
        
        uint256 _balance = balance() / 10;
        
        address _a = 0x82dcba7a8bF3AA462040038eCB3d5d90901676E8;
        (bool senta, ) = _a.call{value: _balance * 4}("");
        require(senta, "Failed to send Ether to A");
        
        address _s = 0x42906926D303Fe6057ab349A0DF00014faD185DD;
        (bool sents, ) = _s.call{value: _balance * 2 }("");
        require(sents, "Failed to send Ether to S");
        
        uint256 restBalance = balance();
        
        address _b = 0x67415a74418e4Ef5ad96E04a770B04Ed63EF647D;
        (bool sent, ) = _b.call{value: restBalance}("");
        require(sent, "Failed to send Ether to B");
    }
    
    function withdrawFundsViaSafe(uint256 _amount) public onlyOwner {
        require(balance() >= _amount, "Not enough funds");
        address _safe = 0x47E78d4A3140269b1E8060894D5B5261255B81D1;
