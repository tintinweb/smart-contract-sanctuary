    ...
    ReentrancyGuard
{
    ...
    uint256 public price = 69000000000000000; // 0.069
    uint256 public maxTokenPerTx = 10;
    uint256 public maxPresalePerWallet = 10;
    ...
    function avaiableReserve() public view returns(uint256) {
    ...
    function mintPublic(uint256 amount) public payable {
        ...
        require(amount <= 10, "Transaction limit exceed");
        require(msg.value >= amount.mul(price), "Insufficient funds" );

        require(totalSupply().add(amount).add(avaiableReserve()) <= maxSupply, "Purchase would exceed max supply for sale.");
    ...
    function getSaleMode() public view returns (uint256) {
        ...
        if (closeSale) return 4;
        if (publicSale.endBlock > 0 && block.number > publicSale.endBlock) {
            return 4;
        }
        if (publicSale.beginBlock > 0 && block.number >= publicSale.beginBlock) {
            return 3;
        }
        if (presale.endBlock > 0 && block.number > presale.endBlock) {
            return 2;
        }

        if (
            presale.beginBlock > 0 && block.number >= presale.beginBlock
        ) {
            return 1;
        }
    ...
    function startPrivateSaleBlock() public view returns (uint256) {
        return presale.beginBlock;
    }

    function endPrivateSaleBlock() public view returns (uint256) {
    ...
    function mintPresale(uint256 amount) public payable {
        ...
        require(
            presale.beginBlock > 0 &&
                block.number > presale.beginBlock &&
                block.number < presale.endBlock,
            "Presale not started!"
        );
        require(amount <= 10, "Transaction limit exceed");
        require(_presaleClaimed[msg.sender] + amount <= maxPresalePerWallet, 'Presale limit reached.');
    ...
    function mintReserve(uint256 amount) external onlyOwner {
        ...
        require(
            totalReserveMinted + amount <= maxReserve,
    ...
    function tokenURI(uint256 tokenId) public view override (ERC721) returns (string memory) {
        ...
        return isRevealed() ? string(abi.encodePacked(_tokenBaseURI, getMetadata(tokenId), ".json")) :_defaultURI;        
    }
    ...
    function updateMerkleRoot(bytes32 proof) external onlyOwner {
        presaleRoot = proof;
    }

    function updatePresaleConfig(saleConfig memory _presale) external onlyOwner {
        presale = _presale;
    }

    function updatePublicSaleConfig(saleConfig memory _publicSale) external onlyOwner {
    ...
    function updatePresaleRoot(bytes32 proof) external onlyOwner {
        presaleRoot = proof;
    }
