...
contract CoolShit is Ownable, ERC721Enumerable {
    ...
    uint constant public MAX_NUM_OF_COOL_SHIT_NFTS = 1001;
    uint256 constant public COOL_SHIT_PRICE = 0.02 ether;

    uint public teamReserve = 301;
    ...
    function reserveHewerTags(address _to, uint amount) public onlyOwner {
        ...
        require(total.add(amount) < MAX_NUM_OF_COOL_SHIT_NFTS, "Cannot exceed max number of tags");

        for (uint i = 1;
            i <= amount;
            i++) {
            require((total + i) < MAX_NUM_OF_COOL_SHIT_NFTS, "Cannot exceed max supply");
    ...
    function mintCoolShit(uint numOfTags) public payable {
        ...
        require(total.add(numOfTags) < MAX_NUM_OF_COOL_SHIT_NFTS, "Cannot exceed max number of tags");
