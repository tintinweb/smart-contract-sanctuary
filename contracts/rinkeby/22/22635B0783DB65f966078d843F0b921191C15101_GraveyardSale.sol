...
contract RBAC is Ownable {
  ...
  Roles.Role private preminters;


  function addPreminter(address _newPreminter) external onlyOwner{
    preminters.add(_newPreminter);
  }

  modifier onlyPreminter()
  {
    require(preminters.has(msg.sender) == true, "Must be eligible for premint");
    _;
  }
}
    ...
    IERC721Enumerable
{
    ...
    /**
     * @dev Returns whether `spender` is allowed to manage `tokenId` using FLACKO.
     *
     * Requirements:
     *
     * - `tokenId` must exist.
...
/**
 * @title Skulls contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */

contract GraveyardSale is ERC721, Ownable, RBAC {
  using SafeMath for uint256;

  bool public canSetURI = true; // one way toggle that allows use for hardSetTokenURI
  // use ONLY in the case that community asks for this functionality to be removed
  // this cannot be undone.

  bool public saleIsActive = false; // current status of sale, default is inactive
  bool public premintIsActive = false; // curent status of premint, default is inactive
  ...
  uint256 public NFTReserve = 50; // Reserve amount of NFTs [COLIN]
  uint256 public constant maxNFTPurchase = 15; // max no. of tokens that can be bought at a time
  uint256 public constant WALLET_CAP = 30; // max no. of tokens a wallet can own before losing the ability to mint
  uint256 public constant maxPremintPurchase = 3; // max no. of tokens that can be bought or owned during premint
  uint256 public constant MAX_NFTS = 4200; // max total supply of NFTs 4200
  uint256 public constant PREMINT_SUPPLY = 420; // 420
  uint256 public premintReserve = 420; //420
  address[420] private premintList;
  constructor() ERC721('Graveyard Sale', 'SKULL') {}

  // sets canSetURI to false. USE ONLY IF COMMUNITY ASKS FOR FUNCTIONALITY TO BE REMOVED
  function cannotSetURI() public onlyOwner
  {
    canSetURI = false;
  }

  // set token URI in the event webhost fails: use with ipfs json
  function hardSetTokenURI(uint256 tokenId, string memory tokenURI) public onlyOwner
  {
   ...
   */
  function reserveNFTs(address _to, string memory tokenURI) public onlyOwner {
  ...
  function mintNFT(uint256 numberOfTokens) public payable {
    ...
    require(balanceOf(msg.sender).add(numberOfTokens) <= WALLET_CAP);
    /**
  ...
  function premint(uint256 _toPremint) external onlyPreminter() payable
  {

    uint256 supply = totalSupply();
    // Checks that there is sufficient reserve supply of tokens for the transaction
    require(
      _toPremint > 0 && _toPremint <= premintReserve,
      'Not enough reserve left'
    );
    require((balanceOf(msg.sender) + _toPremint) <= maxPremintPurchase, "Limit of 3 NFTs during presale");
    require(premintIsActive, 'Premint must be active to mint NFT');
    // Checks that requested number of tokens is positive and not more than the max purchase limit
    require(
      _toPremint > 0 && _toPremint <= maxPremintPurchase,
      'Exceeded max token purchase'
    );
    // Checks that final total supply of tokens (if sale occurs) would not exceed the max supply limit
    require(
      totalSupply().add(_toPremint) <= PREMINT_SUPPLY,
      'Purchase would exceed max supply of NFTs'
    );
    /**
         Checks that the buyer sends at least the total price of the transaction (ETH), calculated as number
        ...
        */
    require(
      msg.value >= NFTPrice.mul(_toPremint),
      'ETH value sent is not correct'
    );

    for (uint256 i = 0; i < _toPremint; i++) {
      uint256 mintIndex = totalSupply();
      if (totalSupply() < MAX_NFTS) {
        _safeMint(msg.sender, mintIndex);
      }
    ...
    premintReserve = premintReserve.sub(_toPremint);
  }
}