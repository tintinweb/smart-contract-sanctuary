...
contract AbstractSale is Ownable {
    ...
    function prepareMessage(address token, uint256 tokenId, uint256 bid, uint256 sellerFee, uint256 nonce) internal pure returns (string memory) {
...
contract AuctionERC1155SaleNonceHolder is OwnableOperatorRole {
...
contract Auction1155 is Ownable, AbstractSale {
    using StringLibrary for string;
    ...
    AuctionERC1155SaleNonceHolder public nonceHolder;

    constructor(TransferProxy _transferProxy, AuctionERC20TransferProxy _erc20TransferProxy, AuctionERC1155SaleNonceHolder _nonceHolder, address payable beneficiary, address _systemSigner) AbstractSale(beneficiary) public {
    ...
    function closeAuction(Asset memory asset, Trade memory trade, BuyerSig memory buyerSig, SystemSig memory systemSig) public payable {
        ...
        uint256 sellerBalance = IERC1155(asset.token).balanceOf(msg.sender, asset.assetTokenId);
        require(sellerBalance >= trade.noOfTokens, "Seller don't have enough balance to trade");
        
        // Check if the Auction reached the reservedPrice
        require(trade.offeredPrice >= trade.reservedPrice, "Reserved Price haven't met");
        
        // Get nonce
        uint256 nonce = nonceHolder.getNonce(asset.token, asset.assetTokenId, msg.sender);
        ...
        // Transfer NFT
        transferERC1155NFT(IERC1155(asset.token), msg.sender, trade.buyer, asset.assetTokenId, trade.noOfTokens);
        ...
        emitAuction(asset, trade, trade.buyer, nonce + 1, trade.noOfTokens);
    }
    ...
    function emitAuction(Asset memory asset, Trade memory trade, address buyer, uint256 nonce, uint256 noOfTokens) internal {
        emit AuctionClosed(address(asset.token), asset.assetTokenId, msg.sender, buyer, address(trade.erc20Token), trade.offeredPrice, nonce, noOfTokens);
    }
    
    function getCreatorFee(address erc1155Adddress, uint256 tokenId) internal view returns (uint256) {
        uint256[] memory fees = ERC1155TokenInter(erc1155Adddress).getFeeBps(tokenId);
        return fees[0];
    }
    
    function getCreator(address erc1155Adddress, uint256 tokenId) internal view returns (address) {
        address[] memory creators = ERC1155TokenInter(erc1155Adddress).getFeeRecipients(tokenId);
        return creators[0];
    }
    ...
    function transferERC1155NFT(IERC1155 token, address seller, address buyer, uint256 tokenId, uint256 tokens) internal {
        transferProxy.erc1155safeTransferFrom(token, seller, buyer, tokenId, tokens, EMPTY);
    }
    
    function verifyBidderSign(Asset memory asset, Trade memory trade, BuyerSig memory buyerSig, uint256 nonce) internal pure {
        string memory messageSigned = prepareMessage(address(asset.token), asset.assetTokenId, trade.offeredPrice, trade.noOfTokens, trade.sellerFee, nonce);
    ...
    function verifySystemSign(Asset memory asset, Trade memory trade, SystemSig memory systemSig, uint256 nonce) internal view {
        address isSystemSigner = StringLibrary.getAddress(abi.encodePacked(IERC1155(asset.token), asset.assetTokenId, msg.sender, trade.buyer, IERC20(trade.erc20Token), trade.offeredPrice, trade.noOfTokens, trade.auctionStart, trade.auctionEnd, nonce), systemSig.v, systemSig.r, systemSig.s);
    ...
    function cancelAuction(address token, uint256 tokenId) public payable {
        uint nonce = nonceHolder.getNonce(token, tokenId, msg.sender);
        nonceHolder.setNonce(token, tokenId, msg.sender, nonce + 1);
        emit CancelAuction(token, tokenId, msg.sender, nonce + 1);
    }

    function prepareMessage(address token, uint256 tokenId, uint256 bid, uint256 tokens, uint256 sellerFee, uint256 nonce) internal pure returns (string memory) {
        return prepareMessage(token, tokenId, bid, sellerFee, nonce).append(". tokens: ", tokens.toString());
    }
}