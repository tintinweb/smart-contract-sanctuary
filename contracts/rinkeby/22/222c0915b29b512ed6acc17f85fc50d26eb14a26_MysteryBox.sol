...
contract MysteryBox is OwnableUpgradeable {
        ...
        external
    {
        ...
        require(IERC721(nft_address).isApprovedForAll(msg.sender, address(this)), "nft not approved");
        ...
        payable
    {
        ...
        uint256 total;
        if (box.sell_all) {
            total = IERC721(box.nft_address).balanceOf(box.creator);
        }
        else {
        ...
        if (box.sell_all) {
            for (uint256 i = 0; i < amount; i++) {
                ...
                uint256 token_id = IERC721Enumerable(nft_address).tokenOfOwnerByIndex(box.creator, token_index);
                IERC721(nft_address).safeTransferFrom(box.creator, msg.sender, token_id);
                box.purchased_nft[msg.sender].push(token_index);
                total--;
            }
        }
        else {
            ...
            for (uint256 i = 0; i < amount; i++) {
                rand = uint256(keccak256(abi.encodePacked(rand, i)));
                uint256 token_index = rand % total;
                // token_id = IERC721(nft_address).tokenOfOwnerByIndex(box.creator, token_index);
                IERC721(nft_address).safeTransferFrom(box.creator, recipient, token_id);
                total--;
            }
            */
        }
        {
            ...
            if (payment_token_address == address(0)) {
                ...
                if ((_number_of_nft > amount) && (eth_to_refund > 0)) {
    ...
    function claim_payment(uint256[] calldata box_ids) external {
    ...
    function get_box_info(uint256 box_id)
        external
        view
        returns (
            address creator,
        ...
        )
    {
        ...
        if (box.sell_all) {
            remaining = IERC721(box.nft_address).balanceOf(creator);
        }
        else {
    ...
    function get_purchased_nft(uint256 box_id, address customer)
        external
        view
    ...
    function get_nft_for_sale(uint256 box_id)
        external 
        view
        returns(uint256[] memory nft_id_list)
    {
        Box storage box = box_by_id[box_id];
        nft_id_list = box.nft_id_list;
    }
        ...
        returns(bool)
    {
        for (uint256 i= 0; i < nft_token_id_list.length; i ++){
            address owner = IERC721(nft_address).ownerOf(nft_token_id_list[i]);
            if (owner != msg.sender){
                return false;
            }
        }
