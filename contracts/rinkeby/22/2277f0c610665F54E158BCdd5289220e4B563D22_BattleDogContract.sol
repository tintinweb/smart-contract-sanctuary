...
contract BattleDogContract is ERC721Enumerable, Ownable {
  ...
  uint256 public witheListMaxSupply = 2000;
  uint256 public maxMintAmount = 2;
  uint256 public WhiteListMaxMintAmount = 1;

  bool public deploy = false;
  ...
  constructor(string memory _initBaseURI, string memory _initNotRevealedUri, address [] memory _users) 
  ERC721("BattleDog", "BD") {
    setBaseURI(_initBaseURI);
    setNotRevealedURI(_initNotRevealedUri);
    setWhitelistUsers(_users);
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
        if(onlyWhitelisted) {
            ...
            require(supply + _mintAmount <= witheListMaxSupply, "max NFT limit exceeded");
            whiteListAddressMintedBalance[msg.sender] = whiteListAddressMintedBalance[msg.sender] + _mintAmount;
        }
        require(deploy, "the contract is paused");
        require(checkMintBalnce(msg.sender,_mintAmount));
  ...
  function checkMintBalnce(address _user, uint256 _mintAmount) public view returns (bool) {
    if(onlyWhitelisted == true && deploy == false){
...
function ownerMint() public payable onlyOwner{
  ...
  function tokenURI(uint256 tokenId)public view virtual override returns (string memory)
  {
    ...
    return bytes(currentBaseURI).length > 0
        ? string(abi.encodePacked(currentBaseURI, tokenId.toString(), ".jason"))
        : "";
  }
