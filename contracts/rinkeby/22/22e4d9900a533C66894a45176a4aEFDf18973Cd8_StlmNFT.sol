...
contract StlmNFT is StarlERC721("Starlink Living Module", "STLM") {
    ...
    /**
     @notice Upgrades a Stlm, increases apy
     @dev Only the owner
     @param _tokenId the token ID to burn
     */
    function upgrade(uint256 _tokenId, uint8 _bonusApy) public upgradeNotPaused {
        ...
        require(_stlmInfo.st_apr.add(_bonusApy) <= MAX_APR, "Exceeds maximum apr");
        require(_stlmInfo.st_launchPrice > 0, "Primary sale price is not set yet");

        starlToken.safeTransfer(vault, _stlmInfo.st_launchPrice.mul(_bonusApy).div(100));
        _stlmInfo.st_apr = _stlmInfo.st_apr + _bonusApy;
        emit StlmInfoUpdated(_tokenId, _stlmInfo.st_launchTime, _stlmInfo.st_launchPrice, _stlmInfo.st_apr);
    }
    ...
    function _setPrimarySalePrice(uint256 _tokenId, uint256 _salePrice) internal {
        ...
        emit StlmInfoUpdated(_tokenId, _stlmInfo.st_launchTime, _salePrice, _stlmInfo.st_apr);
    }
    ...
    function _setStlmLaunchTime(uint256 _tokenId, uint256 _timestamp) internal {
        ...
        emit StlmInfoUpdated(_tokenId, _timestamp, _stlmInfo.st_launchPrice, _stlmInfo.st_apr);
    }
