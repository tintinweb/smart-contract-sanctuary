...
contract CrazyRewards is ERC721Enumerable, Ownable {
	//call up functions
	using Strings for uint256;

	//global variables
	uint256 public constant PRESALE_AMT = 10;
	uint256 public constant PUBLIC_AMT = 40;
	uint256 public constant MAX_AMT = PRESALE_AMT + PUBLIC_AMT;
	...
	uint256 public constant EARLYBIRD_MINT_AMT = 10;
	...
	address private _withdraw = 0xCEEB16254718a8534742aEd8Ed9a3A492Ab13B4a;
    ...
    bool public earlybirdLive;
	bool public saleLive;
	bool public locked;

	constructor() ERC721('CrazyRewards', 'CRW') {
	...
	function buy(uint256 tokenQuantity) external payable {
		...
		require(!earlybirdLive, 'ONLY_EARLYBIRD');
		
		// After first 1000 sold use regular price
		if (totalSupply() < EARLYBIRD_MINT_AMT) {
		...
		for (uint256 i = 0; i < tokenQuantity; i++) {
        	...
        	giftValue = (supply + 1) % 5 == 0 ? address(this).balance * 10 / 100 :  0;
	...
	function presaleBuy(uint256 tokenQuantity) external payable {
		...
		require(totalSupply() < MAX_AMT, 'OUT_OF_STOCK');
		require(
			privateAmountMinted + tokenQuantity <= PRESALE_AMT,
			'EXCEED_PRIVATE'
		);
		require(
