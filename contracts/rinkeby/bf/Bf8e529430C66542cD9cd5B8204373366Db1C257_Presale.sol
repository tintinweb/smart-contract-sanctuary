...
contract Presale {
    ...
    uint256 public constant MAX_ALLOC_STRIP = 2e8 * 10e18; // 200,000,000 STRIP is the max allocation for each presale buyer
    uint256 public constant MAX_ALLOC_WEI = 5e17; // 0.5 ETH is the max allocation for each presale buyer
    uint256 public constant RATE = MAX_ALLOC_WEI / MAX_ALLOC_STRIP; // Fixed Rate
    uint256 public constant IDS = 100e27; // Total StripToken amount for presale : 100b
    ...
    modifier whileOnGoing() {
        ...
        require(isPresaleStarted == true, "Presale has ended or paused");
        _;
    }
    ...
    /********************** Internal ***********************/
    /// Get the StripToken amount of vesting contract
    ...
    function startPresale() external whileDeposited onlyOwner {
        require(isPresaleStarted == false, "StartPresale: Presale has already started!");
    ...
    /// Pause the ongoing presale by emergency
    ...
    /// After presale ends, we withdraw funds to the multiSig admin
    function withdrawRemainingFunds() external whileFinished onlyOwner returns (uint256) {
        ...
        multiSigAdmin.transfer(weiBalance);
       
        return weiBalance;
    }

    /// After presale ends, we withdraw unsold StripToken to multisig.
    ...
    /// Receive Wei from the presale buyers.
    function deposit(address sender) external payable whileOnGoing returns (uint256) {
        ...
        require(address(vestingContract) != address(0x00), "Withdraw: Set vesting contract!");
        uint256 vestingPlan = vestingContract.getVestingPlan(sender);
        require(vestingPlan == 0, "Deposit: sender should not be allowed to participate in presale");
        ...
        uint256 newStripAmount = weiAmount.div(RATE);
        ...
        vestingContract.updateRecipient(sender, recipients[sender].amountStrip, 0);

        require(weiAmount > 0, "Deposit: No ETH balance to withdraw");
        multiSigAdmin.transfer(weiAmount);
...
interface IVesting {
    ...
    function updateRecipient(address, uint256, uint256) external;
    function addRecipient(address, uint256, uint256) external;
    function addRecipients(address[] memory, uint256[] memory, uint256[] memory) external;
