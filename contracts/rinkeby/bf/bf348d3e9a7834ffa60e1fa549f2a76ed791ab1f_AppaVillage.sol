...
contract AppaVillage is ERC721Enumerable, Ownable {
    ...
    event AddedWhitelist(address[] _addresses);
    event ClaimedWhitelist(address _address, uint _tokenId);


    constructor(string memory baseURI) ERC721("Appa Test NFT2", "APT2")  {
        setBaseURI(baseURI);
    ...
    function addWhitelist(address[] calldata _addresses) public onlyOwner {
        ...
        emit AddedWhitelist(_addresses);
    }
    ...
    function claimWhitelist() public payable {
        ...
        require(balanceOf(msg.sender) < 2, 'only 2 nfts per address');
        uint256 tokenId = totalSupply() + 1;
        getAppa(1);
        emit ClaimedWhitelist(msg.sender, tokenId);
    }
