...
contract Zapper is Ownable, IZapper, ContractGuard {
    ...
    function userRestakedAmount(address _address) external override view returns (uint256) {
    ...
    function zapTokenToLPFromPool(address _sender, address _from, uint amount, address _to) external override onlyOneBlock {
        require(msg.sender == UltraStaking, "You aint the pool contract, ser");
        // We receive funds from the pool        
        IERC20(_from).safeTransferFrom(msg.sender, address(this), amount);

        // We define variables used for registering amounts and do swaps
        address sender = _sender;
        // We reset the temporary mapping
        ...
        // WAGMI, ULTRA, BUSD to create WAGMI-BUSD or ULTRA-BUSD 
        if (_from == token0 || _from == token1) {
            // Swap half amount for other
            address other = _from == token0 ? token1 : token0;
            uint sellAmount = amount.div(2);
            _swap(_from, sellAmount, other, address(this));
            // Record balances and add liquidity
            uint token0Amount = IERC20(token0).balanceOf(address (this));
            uint token1Amount = IERC20(token1).balanceOf(address (this));
            ROUTER.addLiquidity(token0, token1, token0Amount, token1Amount, 0, 0, address(this), block.timestamp + 600);

            // Recprd bal of LP, send it back to the pool contract
            uint256 lpbal = pair.balanceOf(address(this));
            restakedAmount[sender] = restakedAmount[sender].add(lpbal);
            IERC20(_to).transfer(address(UltraStaking), lpbal);
        } 
    }
