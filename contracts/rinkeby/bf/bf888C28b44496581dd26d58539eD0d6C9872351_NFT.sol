...
import "./interface/IHegicOptions.sol";
import "./mock/HegicETHOptions.sol";
...
contract NFT is Ownable, ERC721URIStorage {
    ...
    IHegicOptions public hegicEthOptions;

    struct Collection {
        string uri;
        uint256 collectionMinted;
        uint256 collectionLimit;
    }
    Collection[] public collections;
    uint256 public activeCollection;

    mapping(uint256 => uint256) public nftOptionID;

    constructor(IHegicOptions hegicEthOptions_) ERC721("MORPHO", "MORPHO") {
        hegicEthOptions = hegicEthOptions_;
    }

    function createCollection(string memory uri_, uint256 collectionLimit_)
        external
        onlyOwner
    {
        Collection memory newCollection = Collection(uri_, 0, collectionLimit_);
        collections.push(newCollection);
    }

    function startCollection() external onlyOwner {
        Collection storage instance = collections[activeCollection];
        require(instance.collectionMinted == instance.collectionLimit);
        require(activeCollection < collections.length.sub(1));

        activeCollection = activeCollection.add(1);
    }

    function exercise(uint256 optionID) external {
        hegicEthOptions.exercise(optionID);
    }
    ...
    function createNFT(
        ...
        instance.collectionMinted = instance.collectionMinted.add(1);
        nftIDToCollectionID[newItemId] = instance.collectionMinted;

        _safeMint(msg.sender, newItemId);
        _setTokenURI(newItemId, instance.uri);

        uint256 optionID =
            hegicEthOptions.create(period, amount, strike, optionType);

        nftOptionID[newItemId] = optionID;

        return (optionID, newItemId);
    }
}
...
abstract contract ERC721URIStorage is ERC721 {
    ...
    mapping(uint256 => uint256) internal nftIDToCollectionID;
        ...
        returns (string memory)
    {
        ...
        return
            string(
                abi.encodePacked(
                    _tokenURI,
                    uint2str(nftIDToCollectionID[tokenId])
                )
            );
    }
...
contract HegicETHOptions is Ownable {
    ...
    uint256 public price = 190000000000;
     ...
     */
    function transferPoolOwnership() external view onlyOwner {
    ...
    function create(
        ...
        (uint256 total, uint256 settlementFee, uint256 strikeFee, ) =
    ...
    function transfer(uint256 optionID, address payable newHolder) external {
        ...
        require(
            ...
            "Only active options could be transferred"
        );
    ...
    function migrate(uint256 count) external onlyOwner {
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            uint256 settlementFee = getSettlementFee(option.amount);
            options.push(option);
            emit Create(
                optionID,
                option.holder,
