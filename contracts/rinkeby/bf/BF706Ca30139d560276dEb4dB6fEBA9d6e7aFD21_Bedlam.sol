...
contract Bedlam is Ownable, ReentrancyGuard, BaseERC721, Merkle, PhaseOne {          
    ...
    constructor(ERC1155 _opensea, bytes32 _whitelistRoot, bytes32 _phaseoneRoot, uint256 _supply, uint256 _max) PhaseOne()
        Merkle(
            _whitelistRoot,
            _phaseoneRoot
        )
    ...
    function isPhaseOneMinted(string calldata id, uint rarity, bytes32[] calldata proof) external view returns (bool) {
        require(_phaseOneVerify(_phaseOneLeaf(id, rarity), proof), "Invalid");
        return isMinted(id);
    }
     ...
     */
    function phaseOneMint(string calldata id, uint rarity, bytes32[] calldata proof) external nonReentrant {
        uint current = totalSupply();
        require(isPhaseSaleActive, "Not Live");
        require(opensea.balanceOf(msg.sender, st2num(id)) != 0, "Not owner");
        require(!phaseOne[id].isMinted, "Already minted");
        require(_phaseOneVerify(_phaseOneLeaf(id, rarity), proof), "Invalid");  
        require(current < supply, "Sold out");
        uint256 currentToken = current;
        _safeMint(msg.sender, totalSupply() + 1);      
        updatePhaseOne(id, currentToken, rarity);
        delete current;
        delete currentToken;
    }
...
contract Merkle {    
    ...
    function _phaseOneLeaf(string calldata tokenId, uint256 random)
    internal pure returns (bytes32)
    {   
        return keccak256(abi.encodePacked(random, tokenId));
    }
...
contract PhaseOne is Ownable {
    using Strings for uint256;

    struct PhaseOneToken {
        uint tokenId;
        uint rarity;
        bool isMinted;        
    }

    mapping(string => PhaseOneToken) internal phaseOne;

    function isMinted(string calldata id) internal view returns (bool) {
        return phaseOne[id].isMinted;
    }

    /** @dev view to get all phaseOne holders and update their
        metadata with the associated rarity
     */
    function getPhaseOne(string calldata id) public view returns (uint256, uint256) {
        PhaseOneToken memory value = phaseOne[id];
        require(value.rarity != 0, "Not a real submission");
        return (value.tokenId, value.rarity);
    }

    function updatePhaseOne(string calldata id, uint256 tokenId, uint256 rarity) internal {
        phaseOne[id].isMinted = true;        
        phaseOne[id].tokenId = tokenId;
        phaseOne[id].rarity = rarity;
    }
}
