...
contract CloneFactory is Ownable {
  ...
  function addImplementation(address _implementation) public onlyOwner {
    require(templates[_implementation] == address(0), "Implementation should not already exists");

    (bool _isContract, address _template) = IContractProbe(contractProbe).probe(_implementation);

    require(_isContract, "Implementation should be a Contract");
  ...
  function setDefaultTemplate(address _template) external onlyOwner {
    require(templates[_template] != address(0), "Template should be listed");
    require(templates[_template] == _template, "Template should not be a Clone");
