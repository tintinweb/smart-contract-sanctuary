...
contract AddressRecord {
    ...
    modifier logicAuth(address logicAddr) {
        ...
        require(
            RegistryInterface(registry).logic(logicAddr),
            "logic-not-authorised"
        );
        _;
    }

    modifier logicAuths(address[] memory logicAddrs) {
        for (uint256 i = 0; i < logicAddrs.length; i++) {
            require(
                logicAddrs[i] != address(0),
                "logic-proxy-address-required"
            );
            require(
                RegistryInterface(registry).logic(logicAddrs[i]),
                "logic-not-authorised"
            );
        }
        _;
...
contract UserWallet is UserAuth, UserNote {
    event LogExecute(address target, uint256 srcNum, uint256 sessionNum);
    ...
    {
        response = _execute(_target, _data, _src, _session);
    }

    function executeMulti(
        address[] memory _targets, // modules addresses
        bytes[] memory _data,
        uint256 _src,
        uint256 _session
    )
        ...
        returns (bytes[] memory returnData)
    {
        ...
        for (uint256 i = 0; i < _targets.length; i++) {
            returnData[i] = _execute(_targets[i], _data[i], _src, _session);
        }
    }
}
...
contract AddressRegistry {
    ...
    function setAddress(string memory _name, address _userAddress)
        public
        isAdmin
    {
...
contract WalletRegistry is LogicRegistry {
    ...
    function record(address _currentOwner, address _nextOwner) public {
        require(
            msg.sender == address(proxies[_currentOwner]),
            "invalid-proxy-or-owner"
        );
        require(
            proxies[_nextOwner] == UserWallet(0),
            "multiple-proxy-per-user-not-allowed"
        );
