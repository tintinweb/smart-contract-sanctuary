...
contract Staking is Ownable {
    ...
    event Operation(address indexed investor, uint256 amount, string action);

    constructor(AVV _avv) {
        avv = AVV(_avv);
    }
    ...
    function stake(uint256 _amount) public {
        ...
        avv.transferFrom(msg.sender, address(this), _amount);
    ...
    function unstake(uint256 _amount) public {
        ...
        avv.transfer(address(this), _amount);
    ...
    function _claimReward(address payable _stakeholder) internal returns (uint256 reward) {
        ...
        if (reward > 0) {
            ...
            rewards[_stakeholder] = rewards[_stakeholder] + reward;

            emit Operation(msg.sender, reward, "claim");
        }
    }

    function _safeTransfer(address payable recipient, uint256 _amount) private returns (uint256) {
        uint256 amt = _amount;
    ...
    function pendingReward(address _stakeholder) public view returns (uint256) {
        uint256 secondsPassed = block.timestamp - stakes[_stakeholder].checkpoint;
        return ((((stakes[_stakeholder].amount * bonusRateNumerator) / bonusRateDenominator) * secondsPassed) / 1 days);
    }
    ...
    function initInvest(address payable _target) public onlyOwner {
