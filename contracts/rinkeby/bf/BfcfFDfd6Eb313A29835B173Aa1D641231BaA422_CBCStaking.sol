...
contract CBCStaking is Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) private _baseRates;
    ...
    constructor(
      ...
      address _signer
        ...
        // TO BE REMOVED PRIOR DEPLOY
        // _stakers[msg.sender].lastClaimed = block.timestamp - 1000;
        // _stakers[msg.sender].currentYield = 15000;
        // _stakers[msg.sender].accumulatedAmount = 10000;
        // _stakers[msg.sender].lastCheckpoint = block.timestamp - 10000;
    }
    ...
    function deposit(
      ...
      // _validateTokensNumber(contractType, tokenIds.length);
    ...
    function getCurrentReward(address staker) public view returns (uint256) {
      ...
      if (user.lastCheckpoint < acceleratedYield && block.timestamp < acceleratedYield) {
        return (block.timestamp - user.lastCheckpoint) * user.currentYield / SECONDS_IN_DAY * 5;
      }
      if (user.lastCheckpoint < acceleratedYield && block.timestamp > acceleratedYield) {
        uint256 currentReward;
        currentReward += (acceleratedYield - user.lastCheckpoint) * user.currentYield / SECONDS_IN_DAY * 5;
