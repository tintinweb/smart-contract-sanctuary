...
contract lotterytest is ERC721, Ownable, ReentrancyGuard, VRFConsumerBase {
  ...
  function buyTicket(uint amount) external payable nonReentrant {
    ...
    require(thisSeasonsTicketsByAddress[currentSeason][msg.sender] + amount <= maxTicketsPerAddress, "You bought the maximum amount of tickets per season already.");
    
    thisSeasonsTicketsByAddress[currentSeason][msg.sender] += amount;
  ...
  function getWinnerAddress(uint randomVal) private returns (address){
    uint currentSeason = getCurrentSeason();
    uint selectedId = (randomVal % getCurrentSeasonTicketCount()) + 1;
    uint winnerTokenId = seasonToParticipantsTokenIds[currentSeason][selectedId];
