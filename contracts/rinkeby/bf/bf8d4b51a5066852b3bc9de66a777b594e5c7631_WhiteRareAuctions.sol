...
contract WhiteRareAuctions is IAuctionHouse, ReentrancyGuard {
   ...
   Records[] records;
    ...
    function setRecord (
      ...
      uint256 _bulkAuctionId
      ) internal {
        records.push(Records({
          bulkAuctionCreator : msg.sender,
          tokens:_tokens,
          bulkAuctionId:_bulkAuctionId
        }));
   }
   
   
   function getBulkTokens(uint256 _bulkIndex) public view returns (uint256[] memory _tokenIds) {
      Records storage record = records[_bulkIndex];
      return (record.tokens);
   }
   
   function getAuctionStatus(uint256 _auctionId) public view returns(bool) {
       if(auctions[_auctionId].amount != 0){
           return true;
       }else{
           return false;
       }
   ...
   function getAuctionPrice(uint256 _auctionId) public view returns(uint256) {
       return(auctions[_auctionId].reservePrice);
   }
   
   function getCummilativeAuctionPrice(uint256[] memory bulkAuctions) public view returns(uint256) {
       uint256 price = 0;
       for(uint256 i = 0; i < bulkAuctions.length; i++){
           price = price + getAuctionPrice(bulkAuctions[i]);
       }
       return price;
   }
   
   function getAuctionCurrentNumber() public view returns (uint256) {
       return _auctionIdTracker.current();
   }
    ...
    function createBulkAuc(
        ...
        for (uint256 ct = 0; ct < tokenId.length; ct++){
            createFixedSaleAuction(tokenId[ct], tokenContract, duration, reservePrice, curator, curatorFeePercentage, auctionCurrency);
        }
        setRecord(tokenId, bulkAucId);
    ...
    function fixedBuyInternal(uint256 auctionId, uint256 amount)
    internal
    auctionExists(auctionId)
    nonReentrant
    {
    ...
    function fixedBulkBuy (uint256[] calldata _fixedAuctionIds, uint256 amount) 
      external  
      payable 
    {
        require(amount == getCummilativeAuctionPrice(_fixedAuctionIds));
        for(uint256 cd = 0; cd < _fixedAuctionIds.length; cd++){
            uint256 auctionId = _fixedAuctionIds[cd];
            uint256 auctionPrice = getAuctionPrice(auctionId);
            fixedBuyInternal(auctionId, auctionPrice);
            
        }
        
    }
    
    function fixedBulkBuyInternal (uint256[] calldata _fixedAuctionIds, uint256 amount) 
      external  
      payable 
    {
        require(amount == getCummilativeAuctionPrice(_fixedAuctionIds));
        for(uint256 cd = 0; cd < _fixedAuctionIds.length; cd++){
            uint256 auctionId = _fixedAuctionIds[cd];
            uint256 auctionPrice = getAuctionPrice(auctionId);
            this.fixedBuy(auctionId, auctionPrice);
        }
