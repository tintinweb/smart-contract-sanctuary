...
contract MappingToken is Permit, ERC20CappedUpgradeSafe, MappingBase {
	function __MappingToken_init(uint mainChainId_, address token_, address creator_, string memory name_, string memory symbol_, uint cap_) external initializer {
        __Context_init_unchained();
		__ERC20_init_unchained(name_, symbol_);
...
contract TokenMappedProxy is InitializableProductProxy, Constants {
    function productName() override public pure returns (bytes32) {
        return _TokenMapped_;
    }
}

contract MappableTokenProxy is InitializableProductProxy, Constants {
    function productName() override public pure returns (bytes32) {
        return _MappableToken_;
    }
}

contract MappingTokenProxy is InitializableProductProxy, Constants {
    function productName() override public pure returns (bytes32) {
        return _MappingToken_;
    }
}
...
contract MappingTokenFactory is ContextUpgradeSafe, Configurable, Constants {
    ...
    function increaseAuthQuotas(address mappingTokenMapped, address[] memory signatorys, uint[] memory increments) virtual external returns (uint[] memory quotas) {
    ...
    function decreaseAuthQuotas(address mappingTokenMapped, address[] memory signatorys, uint[] memory decrements) virtual external returns (uint[] memory quotas) {
    ...
    function createTokenMapped(address token) external payable returns (address tokenMapped) {
        ...
        bytes memory bytecode = type(TokenMappedProxy).creationCode;
        assembly {
            tokenMapped := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        InitializableProductProxy(payable(tokenMapped)).initialize(address(this), abi.encodeWithSignature('__TokenMapped_init(address)', token));
    ...
    function createMappableToken(string memory name, string memory symbol, uint8 decimals, uint totalSupply) external payable returns (address mappableToken) {
        ...
        bytes memory bytecode = type(MappableTokenProxy).creationCode;
        assembly {
            mappableToken := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        InitializableProductProxy(payable(mappableToken)).initialize(address(this), abi.encodeWithSignature('__MappableToken_init(address,string,string,uint8,uint256)', _msgSender(), name, symbol, decimals, totalSupply));
    ...
    function _createMappingToken(uint mainChainId, address token, string memory name, string memory symbol, uint8 decimals, uint cap) internal returns (address mappingToken) {
        _chargeFee();
        address tokenOrCreator = (token == address(0)) ? _msgSender() : token;
        ...
        bytes memory bytecode = type(MappingTokenProxy).creationCode;
        assembly {
            mappingToken := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        InitializableProductProxy(payable(mappingToken)).initialize(address(this), abi.encodeWithSignature('__MappingToken_init(uint256,address,address,string,string,uint256)', mainChainId, token, _msgSender(), name, symbol, decimals, cap));
        
        mappingTokens[mainChainId][tokenOrCreator] = mappingToken;
        emit CreateMappingToken(mainChainId, token, _msgSender(), name, symbol, decimals, cap, mappingToken);
    }
    event CreateMappingToken(uint mainChainId, address indexed token, address indexed creator, string name, string symbol, uint8 decimals, uint cap, address indexed mappableToken);
    
    function createMappingToken(uint mainChainId, address token, string memory name, string memory symbol, uint8 decimals, uint cap) public payable governance returns (address mappingToken) {
        return _createMappingToken(mainChainId, token, name, symbol, decimals, cap);
    }
    ...
    function createMappingToken(uint mainChainId, address token, string memory name, string memory symbol, uint8 decimals, uint cap, Signature[] memory signatures) public payable returns (address mappingToken) {
        ...
        for(uint i=0; i<N; i++) {
            ...
            require(signatory == signatures[i].signatory, "unauthorized");
            _decreaseAuthCount(signatures[i].signatory, 1);
            emit AuthorizeCreate(_msgSender(), mainChainId, token, name, symbol, decimals, cap, signatory);
        }
        return _createMappingToken(mainChainId, token, name, symbol, decimals, cap);
    }
    event AuthorizeCreate(address indexed creator, uint mainChainId, address indexed token, string name, string symbol, uint8 decimals, uint cap, address indexed signatory);
