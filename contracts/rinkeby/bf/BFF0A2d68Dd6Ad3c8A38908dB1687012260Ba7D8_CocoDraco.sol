...
contract CocoDraco is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    uint256 public teamTracker;
    uint256 public publicTracker;
    uint256 public rareTracker;

    Gem public gem;

    uint256 public constant MAX_ELEMENTS = 8888;
    uint256 public constant WHITELIST_SALE = 2100;
    uint256 public constant PUBLIC_SALE = 6000;
    uint256 public constant TEAM_SUPPLY = 888;
    uint256 public constant MAX_BLACK_GOLD_DRACO = 2000;
 
    uint256 public constant PRICE = 3 * 1e16;
    uint256 public constant RARE_PRICE = 1 * 1e18;
    // uint256 public constant RARE_PRICE = 120 * 1e18;
    uint256 public constant MAX_BY_MINT = 20;
    uint256 public constant WHITELIST_START = 1627588800; // Thu Jul 29 2021 20:00:00 GMT+0000
    ...
    function devMint(address _to, uint256 _count) public onlyOwner {
        uint256 total = teamTracker;
        require(total + _count <= TEAM_SUPPLY, "Max limit");
        require(_count <= MAX_BY_MINT, "Exceeds number");

        for (uint256 i = 0; i < _count; i++) {
            _mintAnElement(_to, publicTracker);
            publicTracker += 1;
            teamTracker += 1;
        }
    }
    ...
    function mint(address _to, uint256 _count) public payable saleIsOpen {
        uint256 total = publicTracker;
        
        require(total + _count <= PUBLIC_SALE, "Max limit");
        require(_count <= MAX_BY_MINT, "Exceeds number");
        require(msg.value >= price(_count), "Value below price");
        if(onlyWhitelist){
            require(whitelistAddresses[msg.sender] > 0, "Not whitelisted");
            require(whitelistAddresses[msg.sender] - _count >= 0, "Can't mint more than 3");
    ...
    function rareMint(address _to, uint256 _count) public saleIsOpen {
        uint256 total = rareTracker;
        require(total + _count <= MAX_BLACK_GOLD_DRACO, "Max limit");
        require(_count <= MAX_BY_MINT, "Exceeds number");
        require(gem.balanceOf(msg.sender) >= rarePrice(_count), "Not enough GEM balance");
    ...
    function rarePrice(uint256 _count) public pure returns (uint256) {
        return RARE_PRICE * _count;
    }
...
contract Gem is ERC20Burnable, Ownable {
    ...
    uint256 public constant EMISSION_RATE = uint256(40 * 10**18) / 86400;
    // 40 GEM / Draco / 86400
