...
contract OurGirlfriend is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 private mintRevenue;
    uint256 private creatorShareClaimed;

    function startARelationship() payable public nonReentrant { 
        require(msg.value == 0.07 ether, "It costs 0.07 ETH to start a relationship");
        require(totalSupply() < 7777, "The GF is at max social capacity.");
        // Our GF is exclusive once she has found her soulmate.
        require(soulmate == address(0), "The GF has found her soulmate.");
        relationshipScores[totalSupply()] = 1;
        _safeMint(_msgSender(), totalSupply());

        mintRevenue += 0.07 ether;
    }

    function woo(uint256 tokenId) payable public nonReentrant { 
        ...
        require(lastWooed + 1 hours < block.timestamp, "The GF is tired.");
        require(soulmate == address(0), "The GF has found her soulmate.");
    ...
    function howIsSheFeeling(uint256 tokenId) public view returns (string memory) {
        if (lastWooed + 1 hours < block.timestamp) {
            return "Tired.";
        } else {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "', parts[1], '", "description": "Our GF. She loves all of us. Only one of us can be her soulmate.", "attributes": [ { "trait_type": "Step 1", "value": "', questActions[0], '" }, { "trait_type": "Step 2", "value": "', questActions[1], '" }, { "trait_type": "Step 3", "value": "', questActions[2], '" }, { "trait_type": "Step 4", "value": "', questActions[3], '" } ], "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
