...
contract CGChef is Ownable {
    ...
    function setPoolMultiplier(
        ...
        require(_fromBlock < _toBlock, "CG Staking: _fromBlock Should be less than _toBlock");
    ...
    function addPool(
        ...
        require(_fromBlock < _toBlock, "CG Staking: _fromBlock Should be less than _toBlock");
        require(address(_lpToken) != address(0), "CG Staking: _lpToken should not be address zero");
    ...
    function getMultiplier(
        ...
        // if (poolMultipliers.length == 0) return to.sub(_from);

        // PoolMultiplier storage poolMulti = poolMultipliers[_poolId];
        if (to <= pool.toBlock) {
            return to.sub(_from).mul(pool.actualMultiplier);
        } else if (_from >= pool.toBlock) {
            return to.sub(_from);
        } else {
            return pool.toBlock.sub(_from).mul(pool.actualMultiplier).add(to.sub(pool.toBlock));
        }
    }
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accTokensPerShare = accTokensPerShare.add(stakingReward.mul(1e12).div(lpSupply));
        }
