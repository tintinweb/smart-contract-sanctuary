...
contract Lend is ILend, Governed {
    using SafeMath for uint256;
    using TCPSafeMath for uint256;
    using TCPSafeCast for uint256;

    
    ILendZhu public lendZhu;
    
    uint8 internal constant STANDARD_DECIMALS = 18;
    
    
    uint public maxRatioOfTotalDebtAllowedPerBlock = 0.5e18;

    
    
    
    
    mapping(IERC20 => bool) public validToken;
    
    mapping(IERC20 => bool) public currentToken;

    struct OneToOneMintedZhuByBlock {
        uint64 blockNumber;
        uint192 count;
    }

    
    OneToOneMintedZhuByBlock public oneToOneMintedZhuByBlock;

    modifier valid(IERC20 token) {
        _requireValidToken(validToken[token]);
        _;
    }

    modifier current(IERC20 token) {
        _requireValidToken(currentToken[token]);
        _;
    }

    function _initHook() internal override {
       lendZhu = governor.lendZhu();

        validUpdate[this.setMaxRatioOfTotalDebtAllowedPerBlock.selector] = true;
    }

    function completeSetup() external override onlyGovernor {
        IUniswapV3Pool[] memory referencePools = governor.getReferencePools();
        for(uint i = 0; i < referencePools.length; i++) _addToken(referencePools[i]);
    }

    
    
    function lend(uint zhuCount) external lockProtocol runnable {
        _requireEffect(zhuCount > 0);
        address msgSender = msg.sender;

        
        
        governor.market().accrueInterest();

        
        uint lendTokenCount = zhuCount._mul(_lendTokenExchangeRate());

        accounting.increaseLentZhu(zhuCount);

        
        
        TransferHelper.safeTransferFrom(address(zhu), msgSender, address(accounting), zhuCount);
        lendZhu.mintTo(msgSender, lendTokenCount);

        emit Lend(msgSender, zhuCount, lendTokenCount);
    }
    ...
    function unlend(uint lendTokenCount) external lockProtocol notStopped {
        ...
        uint zhuCount = valueOfLendTokensInZhu(lendTokenCount);

        
        lendZhu.burnFrom(msgSender, lendTokenCount);
        accounting.sendLentZhu(msgSender, zhuCount);

        emit Unlend(msgSender, zhuCount, lendTokenCount);
    }

    
    function mintZhu(
        IERC20 tokenToDeposit,
        uint zhuToMint
    ) external lockProtocol runnable beforePhase(3) current(tokenToDeposit) {
        _requireEffect(zhuToMint > 0);

        _updateMintedZhuByBlock(zhuToMint);
        ...
        TransferHelper.safeTransferFrom(
            ...
            msgSender,
            address(accounting),
            _normalizeToNativeDecimals(tokenToDeposit, zhuToMint));

        zhu.mintTo(msgSender, zhuToMint);

        emit MintZhu(msgSender, address(tokenToDeposit), zhuToMint);
    }

    
    function returnZhu(
        IERC20 tokenToRecieve,
        uint zhuToReturn
    ) external lockProtocol notStopped valid(tokenToRecieve) {
        _requireEffect(zhuToReturn > 0);

        address msgSender = msg.sender;

        zhu.burnFrom(msgSender, zhuToReturn);

        accounting.sendOneToOneBackedTokens(
            tokenToRecieve,
            msgSender,
            _normalizeToNativeDecimals(tokenToRecieve, zhuToReturn));

        emit ReturnZhu(msgSender, address(tokenToRecieve), zhuToReturn);
    }

    
    
    function valueOfLendTokensInZhu(uint lendTokenCount) public view returns (uint) {
        return lendTokenCount._div(_lendTokenExchangeRate());
    }

    
    function countTokensAvailable(IERC20 token) external view valid(token) returns (uint) {
        return token.balanceOf(address(accounting));
    }

    
    
    function _updateMintedZhuByBlock(uint countIncrease) internal {
        uint total = oneToOneMintedZhuByBlock.blockNumber < block.number
            ? countIncrease
            : countIncrease.add(oneToOneMintedZhuByBlock.count);

        require(total <= accounting.debt()._mul(maxRatioOfTotalDebtAllowedPerBlock), 'No flash minting');

        oneToOneMintedZhuByBlock = OneToOneMintedZhuByBlock({
            blockNumber: block.number.toUint64(),
            count: total.toUint192()
        });
    }

    
    function _lendTokenExchangeRate() internal view returns (uint) {
        uint lends = accounting.lentZhu();
        uint totalLendZhu = lendZhu.totalSupply();
        return (totalLendZhu == 0 || lends == 0) ? TCPSafeMath.ONE : totalLendZhu._div(lends);
    }
    ...
    function _requireValidToken(bool _valid) internal pure {
        require(_valid, 'Invalid token');
    }

    
    function _addToken(IUniswapV3Pool pool) internal {
        IERC20 tokenB = _findTokenB(pool);
        currentToken[tokenB] = true;
        validToken[tokenB] = true;
    }

    
    function _removeToken(IUniswapV3Pool pool) internal {
        IERC20 tokenB = _findTokenB(pool);
        currentToken[tokenB] = false;
    }

    
    function _findTokenB(IUniswapV3Pool pool) internal view returns (IERC20){
        address _zhu = pool.token0();
        address tokenB = pool.token1();
        if (_zhu != address(zhu)) {
            (tokenB, _zhu) = (_zhu, tokenB);
            require(_zhu == address(zhu), 'Missing zhu');
        }
        return IERC20(tokenB);
    }

    
    function _normalizeToNativeDecimals(
        IERC20 token,
        uint countE18
    ) internal view returns (uint adjustedCount) {
        uint8 tokenDecimals = ERC20Decimals(address(token)).decimals();

        if (tokenDecimals == STANDARD_DECIMALS) return countE18;

        return tokenDecimals > STANDARD_DECIMALS
            ? countE18.mul(10**(tokenDecimals - STANDARD_DECIMALS))
            : countE18 / (10**(STANDARD_DECIMALS - tokenDecimals));
    }
    ...
    function setMaxRatioOfTotalDebtAllowedPerBlock(uint ratio) external onlyGovernor {
        maxRatioOfTotalDebtAllowedPerBlock = ratio;
        emit ParameterUpdated('maxRatioOfTotalDebtAllowedPerBlock', ratio);
    }
