...
contract SnowflakesInvasion {

  ERC1155[] public collectibles;
  uint256 public totalPlayers;
  uint8 private _maxXY;
  ...
  constructor(ERC1155[] memory _collectibles) public {
  ...
  function respawn(address collectible,uint256 tokenId,uint8 x,uint8 y) public tokenAllowed(collectible) {
    ...
    if(collectible == address(collectibles[0])){
      ...
      players[msg.sender].x = uint8(2000);
      players[msg.sender].y = uint8(2000);
    }
  ...
  function move(address collectible,uint256 tokenId,uint8 x,uint8 y) public {
    ...
    require(players[msg.sender].collectible == collectible &&
            players[msg.sender].tokenId == tokenId &&
            players[msg.sender].lastMovement.add(50) >= block.number &&
            players[msg.sender].x.add(10) <= players[msg.sender].x &&
            players[msg.sender].x.sub(10) <= players[msg.sender].x &&
            players[msg.sender].y.add(10) <= players[msg.sender].y &&
            players[msg.sender].y.sub(10) <= players[msg.sender].y &&
            x <= _maxXY &&
            y <= _maxXY,
            "Not allowed to move token");
    require(positions[x][y].owner == address(0));
