...
contract LoserChickNFT is Ownable, ERC721("ShriekingChick", "ShriekingChick"){
    ...
    constructor(uint256 totalAmount_) public{
       totalAmount = totalAmount_;

       _setBaseURI("https://www.baidu.com/");
    }
     ...
     */
    function getAccessoriesInfo(address owner, uint256 index, bool flag) public{
        uint256 tokenId = _mintChick(owner);
        string memory tokenUri = tokenURI(tokenId);
            
        ChickAttribute memory chickAttribute;
        chickAttribute.tokenURI = tokenUri;
        if(index == 0 && flag){
            chickAttribute.contractAddr = address(this);
        }else{
            uint256 expressionIndex = uint256(keccak256(abi.encodePacked(block.number, now, msg.sender))) % 10;
            uint256 hatIndex = uint256(keccak256(abi.encodePacked(block.number, now, msg.sender))) % 7;
            uint256 leftHandOrnamentsIndex = uint256(keccak256(abi.encodePacked(block.number, now, msg.sender))) % 11;
            uint256 rightHandOrnamentsIndex = uint256(keccak256(abi.encodePacked(block.number, now, msg.sender))) % 11;
            uint256 shoesIndex = uint256(keccak256(abi.encodePacked(block.number, now, msg.sender))) % 10;
            uint256 bodyOrnamentsIndex = uint256(keccak256(abi.encodePacked(block.number, now, msg.sender))) % 11;
            ...
            uint256 isSuitRandom = uint256(keccak256(abi.encodePacked(block.number, now, msg.sender))) % 2;
            if(isSuitRandom == 0){
                uint256 suitIndex = uint256(keccak256(abi.encodePacked(block.number, now, msg.sender))) % 3;
                chickAttribute.suit = suit[suitIndex];
            }else if(isSuitRandom == 1){
                uint256 jacketIndex = uint256(keccak256(abi.encodePacked(block.number, now, msg.sender))) % 7;
                uint256 trousersIndex = uint256(keccak256(abi.encodePacked(block.number, now, msg.sender))) % 8;
