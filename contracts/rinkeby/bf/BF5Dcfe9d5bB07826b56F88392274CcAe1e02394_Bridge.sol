...
contract Bridge is Trustable {
    ...
    function returnAmount(uint256 _am) public returns (uint256) {
    uint paprTaxAmount = _am * IPapr(papr).getfeesTaxRate(_countTimesOverCents());
    ...
    function create(uint16 tokenId, uint amount, uint8 destination, address target) external {
        ...
        uint paprTaxAmount = amount * IPapr(papr).getfeesTaxRate(_countTimesOverCents());
