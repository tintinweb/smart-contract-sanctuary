...
contract Disputes is IDisputes {
    ...
    function raiseDispute(bytes calldata _data, bytes calldata _signature)
        ...
        returns (uint256)
    {
        address signer = recoverKey(keccak256(_data), _signature, 0);
        (
            address _project,
            uint256 _taskID,
            uint8 _actionType,
        ...
        eventsInstance.disputeRaised(disputeCount);
        disputeCount++;
        ...
        resolvable(_disputeID)
    {
        ...
        assertMember(_project, _taskID, msgSender());
        dispute.attachments[dispute.attachmentsCount] = _attachment;
        dispute.attachmentsCount++;
        eventsInstance.disputeAttachmentAdded(_disputeID, _attachment);
    }

    function attachMultipleDocuments(
        uint256 _disputeID,
        bytes[] calldata _attachments
    ) external override resolvable(_disputeID) {
        Dispute storage dispute = disputes[_disputeID];
        address _project = dispute.project;
        uint256 _taskID = dispute.taskID;
        assertMember(_project, _taskID, msgSender());
        for (uint8 i = 0; i < _attachments.length; i++) {
            dispute.attachments[dispute.attachmentsCount] = _attachments[i];
            dispute.attachmentsCount++;
            eventsInstance.disputeAttachmentAdded(_disputeID, _attachments[i]);
        }
    }
    ...
    function resolveDispute(
        ...
        bool _ratify
    ) external override onlyAdmin resolvable(_disputeID) {
        disputes[_disputeID].judgement = _judgement;
        if (_ratify) {
        ...
        eventsInstance.disputeResolved(_disputeID, _ratify);
    }
    ...
    function resolveHandler(uint256 _disputeID) internal override {
        Dispute storage dispute = disputes[_disputeID];
        require(dispute.status == Status.Active, "Disputes::!Active");
        if (dispute.actionType == ActionType.PhaseAdd) {
            executePhaseAdd(dispute.project, dispute.actionData);
        } else if (dispute.actionType == ActionType.PhaseChange) {
            executePhaseChange(dispute.project, dispute.actionData);
        } else if (dispute.actionType == ActionType.PhasePay) {
            executePhasePay(dispute.project, dispute.actionData);
        } else if (dispute.actionType == ActionType.TaskAdd) {
        ...
        } else if (dispute.actionType == ActionType.TaskPay) {
    ...
    function executePhaseAdd(address _project, bytes memory _actionData)
        ...
        override
    {
        IProject(_project).addPhasesGC(_actionData, bytes(""));
    }
    ...
    function executePhaseChange(address _project, bytes memory _actionData)
        ...
        override
    {
        IProject(_project).changeCostGC(_actionData, bytes(""));
    }
    ...
    function executePhasePay(address _project, bytes memory _actionData)
        ...
        override
    {
        uint256 _phase = abi.decode(_actionData, (uint256));
        IProject(_project).releaseFeeContractor(_phase);
    }
    ...
    {
        return disputes[_disputeID].attachments[_index];
    }
}
    ...
    SignatureDecoder
{
    ...
    struct Dispute {
        ...
        address by; // user who raised the dispute
     ...
     *   - const types = ["address", "uint256", "uint256", "bytes", "bytes", "bytes"]
    ...
    SignatureDecoder
{
     ...
     */
    function _inviteSC(uint256 _task, address _sc) internal virtual;
     ...
     *   - const types = ["address", "uint256", "bytes", "bytes", "bytes"]
     * @param _signature bytes - hash of _data signed by the address raising dispute
     */
    function raiseDispute(bytes calldata _data, bytes calldata _signature)
        external
...
abstract contract IEvents is Initializable {
    ...
    event DisputeAttachmentAdded(uint256 indexed _disputeID, bytes _attachment);
