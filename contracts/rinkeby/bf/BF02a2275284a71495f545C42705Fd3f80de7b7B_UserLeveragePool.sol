...
contract UserLeveragePool is AbstractLeveragePortfolio, IUserLeveragePool, ERC20PermitUpgradeable {
    ...
    function forceUpdateBMICoverStakingRewardMultiplier() public override {
        ...
        for (uint256 i = 0; i < leveragedCoveragePools.length(); i++) {
            ...
            _leverageProvided = poolsDeployedAmount[policyBookAddress].div(totalLiquidity);
    ...
    AbstractDependant
{
    ...
    function _calcM(uint256 poolUR) internal view returns (uint256) {
        ...
        uint256 _multiplier =
            Math.min(
                d_ProtocolConstant.mul(PRECISION).div(_ur).mul(PRECISION).div(a_ProtocolConstant),
