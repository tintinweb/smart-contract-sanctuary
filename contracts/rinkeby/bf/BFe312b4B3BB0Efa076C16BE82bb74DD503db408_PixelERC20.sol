...
contract PixelERC20 is ERC20, Ownable {
    ...
    uint256 public amountLeftForReserve = 135000 ether;
    ...
    function numInitialRewardsUnclaimed(address user) public view returns(uint256) {
        uint256[] memory ownersTokens = PixelTigers.tokenGenesisOfOwner(user);
    ...
    function claimReward() external {
        ...
        uint256[] memory ownersTokens = PixelTigers.tokenGenesisOfOwner(msg.sender);
        uint256 numTigersUnclaimed = 0;
        for(uint256 i; i < ownersTokens.length; i++){
            uint256 tokenId = ownersTokens[i];
            if(tigerClaimedInitialReward[tokenId] == false){
        ...
        _mint(msg.sender, 20 ether * numTigersUnclaimed + pendingReward(msg.sender) + storeRewards[msg.sender]);
    ...
    function totalTokensClaimable(address user) external view returns(uint256) {    
        if (numInitialRewardsUnclaimed(user) > 0){
            return pendingReward(user) + numInitialRewardsUnclaimed(user)* 20 ether + storeRewards[user];
        } else{
