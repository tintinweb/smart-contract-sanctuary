...
contract PoolFarming is Ownable {
    ...
    function deposit(uint8 _pid, uint256 _amount) public {
        ...
        user.loss = getPending(_pid, msg.sender);
    ...
    function withdraw(uint8 _pid) public {
        ...
        pool.accPerShare = getAccPerShare(_pid);
        if (pool.balance > 0 && user.rewards > 0) {
            pool.accPerShare = pool.accPerShare.add(user.rewards.mul(1e12).div(pool.balance));
        }
        pool.lastBlock = getLastBlockNumber();
    ...
    function getReward(uint256 _pid, address _user) public view returns(uint256) {
        ...
        return calcReward(user.rewards.add(pending), user.date);
    }

    function calcReward(uint256 amount, uint256 date) internal view returns(uint256) {
    ...
    function getUser(uint256 _pid, address _user) public view returns(
        uint256 balance,
        uint256 claimable,
        uint256 pending,
        uint256 lockDate,
        ...
        pending = user.rewards.add(getPending(_pid, _user));
        ...
        nextUnlock = calcReward(pending, lockDate - 2 weeks).sub(claimable);
    }
    ...
    function withdrawUnusedPaper() public onlyOwner {
        uint256 amount = paperToken.balanceOf(address(this)).sub(pools[paperPool].balance);
        if (amount > 0) {
            paperToken.transfer(
                msg.sender,
                amount
