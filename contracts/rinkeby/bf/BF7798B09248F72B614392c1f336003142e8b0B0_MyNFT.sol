...
contract MyNFT is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public constant TOTAL_PRESALE_SUPPLY = 50;

    uint256 public constant RESERVED_TOKENS = 5;
    uint256 public constant MAX_PRESALE_MINT_PER_TX = 10;
    uint256 public constant MAX_PUBLIC_MINT_PER_TX = 10;

    uint256 public constant PRICE_GIVEAWAY = 0 ether;
    uint256 public constant PRICE_PRE_SALE = 0.0001 ether;
    uint256 public constant PRICE_PUBLIC_SALE = 0.0002 ether;
    ...
    function setGiveawayAllowedList(
    ...
    function getGiveawayAvailableToMintByAddress(address addr)
        external
        view
        returns (uint256)
    {
    ...
    function presaleMint(uint256 numberOfTokens) public payable {
        ...
        require(
            PRICE_PRE_SALE * numberOfTokens <= msg.value,
            "Ether value sent is not correct/enough."
        );

        if (numberOfTokens >= 5) {
            numberOfTokens += 2;
        } else if (numberOfTokens == 3 || numberOfTokens == 4) {
            numberOfTokens += 1;
        }
    ...
    function giveawayMint(uint256 numberOfTokens) external payable {
        ...
        require(
            numberOfTokens <= _giveawayAllowedList[msg.sender],
            "No sufficient available tokens to claim."
        );
        require(
            currentTotalSupply + numberOfTokens <= MAX_SUPPLY,
            "This claim will exceed max possible number of tokens."
        );
        require(
            PRICE_GIVEAWAY * numberOfTokens <= msg.value,
            "Ether value sent is not correct/enough."
        );
