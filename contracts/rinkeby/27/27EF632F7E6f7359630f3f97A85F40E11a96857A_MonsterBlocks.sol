...
contract MonsterBlocks is MonsterBlockCoreERC721, VRFConsumerBase {
  ...
  function fulfillRandomness(bytes32 _requestId, uint256 _randomNumber) internal override {
    for (uint256 k = 0; k < vrfRequestIds[_requestId].length; k++) {
      monsterBlocksDna[vrfRequestIds[_requestId][k]] = _randomNumber;

      for (uint256 i = 0; i < numberOfTraits; i++) {
        uint256 currentWeight = traitWeighting;
        uint256 dnaModuloWeight = (uint256(keccak256(abi.encode(_randomNumber, i * k))) % currentWeight) + 1;
