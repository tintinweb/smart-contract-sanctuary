...
contract FlurksMarket is ReentrancyGuard, Pausable, Ownable {
    ...
    function flurkNoLongerForSale(uint flurkIndex) public nonReentrant() {
        if (flurkIndex >= 1260) revert('token index not valid');
    ...
    function offerFlurkForSale(uint flurkIndex, uint minSalePriceInWei) public whenNotPaused nonReentrant()  {
        if (flurkIndex >= 1260) revert('token index not valid');
        if (flurksContract.ownerOf(flurkIndex) != msg.sender) revert('you are not the owner of this token');
        uint stoneRoyaltiesTotal = minSalePriceInWei / 20;
        uint scatterRoyaltiesTotal = minSalePriceInWei / 40;
        flurksOfferedForSale[flurkIndex] = Offer(true, flurkIndex, msg.sender, minSalePriceInWei + stoneRoyaltiesTotal + scatterRoyaltiesTotal, address(0x0));
        emit FlurkOffered(flurkIndex, minSalePriceInWei + stoneRoyaltiesTotal + scatterRoyaltiesTotal, address(0x0));
    }
    ...
    function offerFlurkForSaleToAddress(uint flurkIndex, uint minSalePriceInWei, address toAddress) public whenNotPaused nonReentrant() {
        if (flurkIndex >= 1260) revert();
        if (flurksContract.ownerOf(flurkIndex) != msg.sender) revert('you are not the owner of this token');
        uint stoneRoyaltiesTotal = minSalePriceInWei / 20;
        uint scatterRoyaltiesTotal = minSalePriceInWei / 40;
        flurksOfferedForSale[flurkIndex] = Offer(true, flurkIndex, msg.sender, minSalePriceInWei + stoneRoyaltiesTotal + scatterRoyaltiesTotal, toAddress);
    ...
    function buyFlurk(uint flurkIndex) payable public whenNotPaused nonReentrant() {
        ...
        if (seller != flurksContract.ownerOf(flurkIndex)) revert('seller no longer owner of flurk'); // Seller no longer owner of flurk
         // Andrew starting
        uint256 stoneRoyaltiesTotal = msg.value / 20;
        uint256 scatterRoyaltiesTotal = msg.value / 40;
        payable(address(0x0020D49D4941643043E57E72f28Dc6b0bD80FAdc)).transfer(stoneRoyaltiesTotal);
        payable(address(0x0020D49D4941643043E57E72f28Dc6b0bD80FAdc)).transfer(scatterRoyaltiesTotal);
        uint256 newValue = msg.value - stoneRoyaltiesTotal - scatterRoyaltiesTotal;
        flurksOfferedForSale[flurkIndex] = Offer(false, flurkIndex, msg.sender, 0, address(0x0));
        flurksContract.safeTransferFrom(seller, msg.sender, flurkIndex);
        pendingWithdrawals[seller] += newValue;
        // Andrew ending

        emit FlurkBought(flurkIndex, newValue, seller, msg.sender);
