...
import './Interfaces/ISyncxColors.sol';

/** 
AIR NFT Rental Contract
Supports all ERC721
- Version 1 implements Sync x Colors minting and recoloring functionality only using THE_COLORS primitive NFTs
- Version 2 (future) to provide general flash lending interface supporting any supported erc721/1155
*/
contract AirNFT is
  Initializable,
  OwnableUpgradeable,
...
{
  //Public declarations
  uint128 public platformFeeRate;
  mapping(address => uint128) public minimumRoyalty;

  //Private declarations
  address private constant TREASURY =
    address(0x263853ef2C3Dd98a986799aB72E3b78334EB88cb);

  uint256 private platformAccruals;

  struct StakedNFTStruct {
    uint128 rentalFee;
    uint128 allTimeLends;
    uint256 accruals;
    uint256 allTimeAccruals;
  }

  mapping(uint256 => StakedNFTStruct) private stakedNFT; // Staked NFTs mapped to Receipt tokens (tokenIds limited to uint16 (131071))
  mapping(address => uint256) private airSupport; // Supported ERC721 NFTs are mapped to an index
  uint256 private currentIndex; // Track current index for adding support for additional NFTs

  //* Note: Variables newly declared in Future contract
  //  upgrades MUST be located below existing declarations
  // ----------------------------------------*/

  /**
   * TODO: Used for testing only. To be removed.
  */ 
  function setColorAddress(address colorAddress) public {
    airSupport[colorAddress] = 0x100000000;
    minimumRoyalty[colorAddress] = 0.003 ether;
  }
  ...
  function initialize() public initializer {
    ...
    address THE_COLORS = address(0x3C4CfA9540c7aeacBbB81532Eb99D5E870105CA9);
   ...
   * @param erc721Address Address of supported ERC721 NFT
  ...
  function addStaked(
    ...
    uint128[] calldata rentalFees
  ) external whenNotPaused {
    uint256 _nftIndex = airSupport[erc721Address];
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      ...
      require(rentalFees[i] >= minimumRoyalty[erc721Address], 'Require Fee >= minimumRoyalty');
      require(rentalFees[i] <= 10 ether, 'Require Fee <= 10 ether');

      stakedNFT[_nftIndex + tokenIds[i]].rentalFee = rentalFees[i];
   ...
   * @param claimRoyalties value of true will also claim royalties for those tokens removed from staking
   */
  function removeStaked(
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      require(
        ownerOf(_nftIndex + tokenIds[i]) == msg.sender,
        'Staking receipt not found'
      );
   ...
   * @param mintAmount Address of ERC721 NFT
   * @param tokenIds tokenIds of COLORS NFT to apply to mint
   * TODO: Compare gas cost between implementing non-renentry versus the overflow check with safemath
   */
  function mintSyncsWithRentedTokens(
    uint256 mintAmount,
    uint16[] calldata tokenIds
  ) external payable whenNotPaused {
    require(mintAmount <= 10, 'Max mint is 10');
    require(tokenIds.length <= 3, 'Num COLORS must be <=3');

    // In future this function will be moved out of the lender contract.
    uint256 mintPrice = 0.05 ether;
    address SYNCXCOLORS = address(0x6224a53929612A1C8cB7d57dccedDa090d15CD44); // ROP
    // address SYNCXCOLORS = address(0x6F69141C0419B1D94C29bD5972F99213C2CE7b92); // Mainet
//     address SYNCXCOLORS = address(0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512); // local

    // Update accrued royalties
    StakedNFTStruct memory rentalNFT;
    uint256 totalRentalFee;
    uint256 _nftIndex = 0x100000000; // Avoid an MLoad here since the Colors contract is predetermined to be within slot 1
    for (uint256 i = 0; i < tokenIds.length; i++) {
      require(_exists(_nftIndex + tokenIds[i]), 'COLORS tokenId unavailable');

      rentalNFT = stakedNFT[_nftIndex + tokenIds[i]];
      stakedNFT[_nftIndex + tokenIds[i]].allTimeAccruals =
        rentalNFT.allTimeAccruals +
        rentalNFT.rentalFee *
        mintAmount;
      stakedNFT[_nftIndex + tokenIds[i]].accruals =
        rentalNFT.accruals +
        rentalNFT.rentalFee *
        mintAmount;
      stakedNFT[_nftIndex + tokenIds[i]].allTimeLends =
        rentalNFT.allTimeLends +
        uint128(mintAmount);
      totalRentalFee += rentalNFT.rentalFee * mintAmount;
    }
    platformAccruals +=
      (totalRentalFee * platformFeeRate) /
      1000 -
      totalRentalFee;

    require(
      msg.value ==
        (mintAmount * mintPrice) + ((totalRentalFee * platformFeeRate) / 1000),
      'Insufficient funds.'
    );
...
//    address SYNCXCOLORS = address(0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512); // local
    ...
    uint256 _nftIndex = 0x100000000; // Avoid an MLoad here since the Colors contract is predetermined to be in slot 1
    ...
    // Verify staked colors; update storage
    StakedNFTStruct memory rentalNFT;
    uint256 totalRentalFee;
    for (uint256 i = 0; i < colorsTokenIds.length; i++) {
      require(
        _exists(_nftIndex + colorsTokenIds[i]),
        'COLORS unavailable for rent'
      );

      rentalNFT = stakedNFT[_nftIndex + colorsTokenIds[i]];

      stakedNFT[_nftIndex + colorsTokenIds[i]].allTimeAccruals =
        rentalNFT.allTimeAccruals +
        rentalNFT.rentalFee *
        tokenIds.length;
      stakedNFT[_nftIndex + colorsTokenIds[i]].accruals =
        rentalNFT.accruals +
        rentalNFT.rentalFee *
        tokenIds.length;
      stakedNFT[_nftIndex + colorsTokenIds[i]].allTimeLends =
        rentalNFT.allTimeLends +
        uint128(tokenIds.length);
      totalRentalFee += rentalNFT.rentalFee * tokenIds.length;
    }
    platformAccruals +=
      (totalRentalFee * platformFeeRate) /
      1000 -
   ...
   * @param erc721Address Nft address
   */
  function claimAllContract(address erc721Address) public whenNotPaused {
    uint256 _nftIndex = airSupport[erc721Address];
    require(_nftIndex != 0, 'NFT unsupported');
    ...
    for (uint256 i = 0; i < bal; i++) {
      ...
      if (receiptTokenId & 0xFFFF00000000 == _nftIndex) {
        royalties += stakedNFT[receiptTokenId].accruals;
        stakedNFT[receiptTokenId].accruals = 0;
      }
    }
    _claim(royalties);
  }
   ...
   * @param tokenIds tokenIds staked within the contract
   */
  function claim(address erc721Address, uint16[] calldata tokenIds)
    public
    whenNotPaused
  {
    uint256 _nftIndex = airSupport[erc721Address];
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      ...
      royalties += stakedNFT[_nftIndex + tokenIds[i]].accruals;
      stakedNFT[_nftIndex + tokenIds[i]].accruals = 0;
    }
    _claim(royalties);
  }
   ...
   */
  function getRentalCost(address erc721Address, uint16[] calldata tokenIds)
    public
    view
    returns (uint256)
  {
    ...
    uint256 rentalFee = 0;
    for (uint256 i = 0; i < tokenIds.length; i++) {
      rentalFee += stakedNFT[_nftIndex + tokenIds[i]].rentalFee;
    }
  ...
  {
    require(airSupport[erc721Address] != 0, 'NFT unsupported');
    require(
  ...
  /**
   * @dev Returns ERC721 tokenIds staked by calling address (based on receipts held)
   * @param erc721Address Address of ERC721 NFT
   * @param stakerAddress Address of staker
   * @return Array of tokenIds
   */
  function getStakedByAddress(address erc721Address, address stakerAddress)
    public
    view
    returns (uint256[] memory)
  {
    ...
    uint256[] memory stakedTrimmed = new uint256[](index);
    for (uint256 i = 0; i < index; i++) {
      stakedTrimmed[i] = staked[i];
    }
    return stakedTrimmed;
  }
   ...
   * @return Array of tokenIds
   */
  function getStaked(address erc721Address)
    public
    view
    returns (uint256[] memory)
  {
  ...
  function getAccruals(address stakerAddress) public view returns (uint256) {
    ...
    uint256 bal = balanceOf(stakerAddress);
    for (uint256 i = 0; i < bal; i++) {
      royalties += stakedNFT[tokenOfOwnerByIndex(stakerAddress, i)].accruals;
    }
    return royalties;
  }
   ...
   * @return Array of StakedNFTStruct structs
   */
  function stakedNFTData(address erc721Address, uint256[] calldata tokenIds)
    public
    view
    returns (StakedNFTStruct[] memory)
  {
    uint256 _nftIndex = airSupport[erc721Address];
    require(_nftIndex != 0, 'NFT unsupported');

    StakedNFTStruct[] memory stakedStructs = new StakedNFTStruct[](
      tokenIds.length
    );
    
    for (uint256 i = 0; i < tokenIds.length; i++) {
      stakedStructs[i] = stakedNFT[_nftIndex + tokenIds[i]];
    }
   ...
   */
  function updatePlatformFeeRate(uint128 feeRate) public onlyOwner {
  ...
  function withdrawPlatformAccruals() external onlyOwner {
    uint256 balance = platformAccruals;
    platformAccruals = 0;

    bool sent;
    (sent, ) = payable(TREASURY).call{value: balance}('');
    require(sent);
  }
  ...
  function getPlatformAccruals() external view onlyOwner returns (uint256) {
    return platformAccruals;
  }

  /**
   * @dev Adds ERC721 to supported staking NFTs
   * @param erc721Address Address of ERC721 NFT
   */
  function addSupportedNFT(address erc721Address, uint128 _minimumRoyalty) public onlyOwner {
    require(airSupport[erc721Address] == 0, 'NFT already supported');
    currentIndex += 0x100000000;
    airSupport[erc721Address] = currentIndex;
    minimumRoyalty[erc721Address] = _minimumRoyalty;
  }
  function setMinimumRoyalty(address erc721Address, uint128 _minimumRoyalty) public onlyOwner {
    require(airSupport[erc721Address] != 0, 'NFT unsupported');
    minimumRoyalty[erc721Address] = _minimumRoyalty;
  }
}
