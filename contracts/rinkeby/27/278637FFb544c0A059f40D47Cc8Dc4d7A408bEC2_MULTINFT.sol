...
contract MULTINFT is ERC1155, Ownable{
    ...
    function transferItemFromBox(
        ...
        require(getAvailableBox() >= amount, "Box Sold out");
    ...
    function getAvailableBox() public view returns (uint256){
        uint total = 0;
        for(uint i=0;i<availableBox.length;i++){
            total = total.add(availableBox[i]);
        }
        return total;
    }
    function randomOfBox() public view returns (uint256){
        ...
        uint256 _rand =  uint256(keccak256(abi.encodePacked(block.difficulty,block.timestamp,getAvailableBox()))).mod(getAvailableBox());
