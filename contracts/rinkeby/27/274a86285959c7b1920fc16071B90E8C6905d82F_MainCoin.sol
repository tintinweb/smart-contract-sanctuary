...
contract MainCoin is Ownable, IERC20 {
    string private _name = "MainCoin";
    ...
    uint256 public mainCoinPriceLevel;
    ...
    constructor (
        ...
        uint256 _mainCoinPriceLevel
        ) {
            baseToken = _baseToken;
            //replace _mainCoin with the address (this) will not work,
            //because there is no pool with this token yet
            //mainCoin = address(this);
            mainCoin = _mainCoin;
            holoswapFactory = _holoswapFactory;
            priceAdapterAddress = _priceAdapterAddress;
            mainCoinPriceLevel = _mainCoinPriceLevel;
    ...
    modifier onlyCrowdsale() {
        require(crowdsale == msg.sender, '!crowdsaleContract');
        _;
    }

    function setMainCoinPriceLevel(uint256 _mainCoinPriceLevel) external onlyOwner {
        mainCoinPriceLevel = _mainCoinPriceLevel;
    }
    ...
    function setCrowdsaleAddress(address _crowdsale) external onlyOwner {
        if (isExcludedFromFee(crowdsale)) {
            _isExcludedFromFee[crowdsale] = false;
        }
    ...
    function _getPrice() public view returns (uint256 poolPrice, uint256 priceLevel){
        poolPrice = IPriceAdapter(priceAdapterAddress).getPrice(baseToken, /* address(this), */mainCoin, holoswapFactory);
        return (poolPrice, mainCoinPriceLevel);
    }

    function _getPercentage() public view returns (uint256 precentage) {
        (uint256 poolPrice, uint256 _mainCoinPriceLevel)= _getPrice();
        if (poolPrice >= _mainCoinPriceLevel) {
            return 5;
        } else if (poolPrice < _mainCoinPriceLevel) {
            return 5 + (((_mainCoinPriceLevel - poolPrice) * 100) / ( _mainCoinPriceLevel));
        }     
    }
    ...
    function mint(address account, uint256 tAmount) external onlyCrowdsale {
        (uint256 rSupply, uint256 tSupply) = _getCurrentSupply();
        uint256 currentRate =  _getRate();
        uint256 rAmount = tAmount * currentRate;
    ...
    function burn(address account, uint256 tAmount) external onlyCrowdsale {
        (uint256 rSupply, uint256 tSupply) = _getCurrentSupply();
        uint256 currentRate =  _getRate();
        uint256 rAmount = tAmount * currentRate;
        _tTotal = tSupply - tAmount;
