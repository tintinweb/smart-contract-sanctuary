...
contract Fren is ERC721Tradable {
    ...
    function initialize(
        ...
        ERC721Tradable.initialize(_name, _symbol, _proxyRegistryAddress, _splitterAddress);
        maxByMint = 10;
        maxSupply = 8888;
    ...
    */
    ...
    function mintFren(uint numberOfTokens) external payable {
        require(saleIsActive, "Sale not active");
        require(numberOfTokens <= maxByMint, "Max mint reached");
        require(totalMint() + numberOfTokens <= maxSupply, "Max supply reached");
        require(getCurrentPrice() * numberOfTokens <= msg.value, "Eth value incorrect");
    ...
    function getCurrentPrice() view public returns (uint){
        if((block.timestamp - START) >= 3 hours){
            return 0.01 ether;
        }else{
            return (3 hours - (block.timestamp - START)) * 0.99 ether / 3 hours + .01 ether;
        }
    }

    function pauseSale() external onlyOwner {
        require(saleIsActive == true, "already paused");
        saleIsActive = false;
    }

    function startSale() external onlyOwner {
        require(saleIsActive == false, "already started");
 ...
 */
abstract contract ERC721Tradable is ERC721EnumerableUpgradeable, OwnableUpgradeable, ContextMixin, NativeMetaTransaction {
    ...
    function mintOwner(address _to) public onlyOwner {
        _mintTo(_to);
    }
