...
*/
contract TaskManager is IIterableFunctions, StorageAccessor, FeeManager, EscrowManager {
    ...
    string constant ETH_INSUFFICIENT_REASON = "TM: Attempted to deposit a different amount of ETH than declared";
    ...
    function createTask(
        ...
        require(taskParams.amount > 0, "TM: zero amount");
        bool isEth = false;
        //checks
        uint256 tax = calculateTax(taskParams.isHyveCommission, isEth, taskParams.amount, taskParams.tokenAddress);

        if (msg.value > 0) {
            require(taskParams.tokenAddress == address(0), "TM: ETH paid, but token address included");
            isEth = true;
            ...
            require(
                ...
                "TM: payment in Hyve, commission in something else");
            if (taskParams.isHyveCommission) {
        ...
        require(taskParams.deadline > block.timestamp, "TM: deadline in the past");
        ...
        depositToEscrow(taskId, _msgSender(), IERC20Upgradeable(taskParams.tokenAddress), taskParams.amount);
    ...
    function cancelTask(uint256 taskId) external {
        ...
        require(task.creator != address(0), "TM: non-existent task");
        ...
        redeemDepositFromEscrow(taskId);
    ...
    function submitSolution(uint256 applicationId, string memory solutionUri, string memory solutionHash) external {
        require(applicationId <= getTaskStorage().getLastApplicationId(), "TM: nonexistent application");
    ...
    function acceptSolution(uint256 solutionId) external {
        require(solutionId <= getTaskStorage().getLastSolutionId(), "TM: nonexistent solution");
        TaskStorage.Solution memory solution = getTaskStorage().getSolution(solutionId);
        TaskStorage.Application memory application = getTaskStorage().getApplication(solution.applicationId);
        TaskStorage.Task memory task = getTaskStorage().getTask(application.taskId);
        require(_msgSender() == task.creator, NOT_CREATOR_REASON);
        getTaskStorage().acceptSolution(solutionId);
        payBeneficiary(application.taskId, application.worker);
...
abstract contract EscrowManager is IAddressManager, ERC20Sender {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using SafeMath for uint256;

    function depositToEscrow(uint256 taskId, address user, IERC20Upgradeable token, uint256 amount) internal {
        IEscrow(getConfigStorage().getEscrow()).createDeposit(address(this), taskId, user, address(token), amount);
    }

    function redeemDepositFromEscrow(uint256 taskId) internal {
        IEscrow(getConfigStorage().getEscrow()).redeemDeposit(taskId);
    }

    function payBeneficiary(uint256 taskId, address user) internal {
        IEscrow(getConfigStorage().getEscrow()).setBeneficiary(taskId, user);
        IEscrow(getConfigStorage().getEscrow()).executePayment(taskId);
    }
}
...
interface IEscrow {
    ...
    function createDeposit(address mediator, uint256 taskId, address from, address tokenAddress, uint256 amount) external;
...
contract Escrow is IEscrow, AccessLevel, ERC20Taker {
    ...
    function createDeposit(address mediator, uint256 taskId, address from, address tokenAddress, uint256 amount) override external {
    ...
    function executePayment(uint256 taskId) override external {
        ...
        transferDeposit(taskId, _deposits[taskId].beneficiary);
    }
    ...
    function redeemDeposit(uint256 taskId) override external {
        require(depositExistsForTask(taskId), "Escrow: No deposit has been created for given task id!");

        transferDeposit(taskId, _deposits[taskId].from);
    }

    function transferDeposit(uint256 taskId, address beneficiary) private {
        uint256 amount = _deposits[taskId].initialDeposit;
