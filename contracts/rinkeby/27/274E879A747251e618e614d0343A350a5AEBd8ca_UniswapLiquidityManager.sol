...
contract UniswapLiquidityManager is PeripheryPayments, IUniswapLiquidityManager {
    ...
    function toggleFeesInPilot(address pool) external onlyGovernance {
        _feesInPilot[pool] = !_feesInPilot[pool];
    }
    ...
    /// @param premiumStatus_ Status of the pool for premium
    function setReadjustPremiumForPools(
        ...
        readjustPremium[pool] = ReadjustPremium({
            premiumStatus: premiumStatus_,
            premium: premium_
        });
    ...
    function readjustFrequencyStatus(address pool) public returns (bool status) {
        if (block.timestamp - _readjustFrequency[pool].timestamp > 900) {
            // change for mainnet
            _readjustFrequency[pool].counter = 0;
            _readjustFrequency[pool].status = false;
        }
        status = _readjustFrequency[pool].status;
    }
    ...
    function readjustLiquidity(
        ...
        _readjustFrequency[b.poolAddress].timestamp = block.timestamp;
        ...
        _readjustFrequency[b.poolAddress].counter += 1;
        if (_readjustFrequency[b.poolAddress].counter == 2)
            _readjustFrequency[b.poolAddress].status = true;
        ...
        if (
            ...
            readjustPremium[b.poolAddress].premiumStatus
        ) {
            ...
            _mintPilot(
                msg.sender,
                b.pilotAmount.add(readjustPremium[b.poolAddress].premium)
            );
        }
        ...
        )
    {
        ...
        if (position.totalLiquidity > 0) {
            ...
            return IULMState(_ulmState).getTotalAmounts(_pool, address(this));
        }
    }
        ...
        )
    {
        ...
        if (a.token0 == WETH || a.token1 == WETH) {
            ...
            if (
                _feesInPilot[params.pool] &&
                params.pilotToken &&
                ethTokenPair != address(0)
            ) {
