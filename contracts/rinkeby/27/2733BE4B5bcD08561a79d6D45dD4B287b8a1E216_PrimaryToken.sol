...
contract PrimaryToken is ERC721, ERC721Enumerable, Pausable, Ownable, ERC721Burnable, VRFConsumerBase {
  ...
  address public immutable lAddress;
  address public immutable ibAddress;
  ...
  ERC721("PrimaryToken", "PT")
  VRFConsumerBase(vrfCoordinator_, link_) {
    lAddress = specialMintAddresses_[0];
    ibAddress = specialMintAddresses_[1];
    developerAddress = specialMintAddresses_[2];
  ...
  function getRandomNumber() public onlyOwner returns (bytes32 requestId) {
    require(!_randomnessHasBeenSet, "Randomness has already been set.");
  ...
  // TODO: ABSOLUTELY MUST REMOVE BEFORE DEPLOY
  function setRandomnessManually_FOR_TESTING(uint256 randomness_) public onlyOwner {
    _randomness = randomness_;
    _randomnessHasBeenSet = true;
  }

  // A withdraw function to avoid locking ERC20 tokens in the contract forever.
  // Tokens can only be withdrawn to the owner.
  ...
  // All tokens other than tokens 0, 1, and 2 are minted using the random offset.
  ...
  function mint(address to_) public onlyOwner whenNotPaused {
    ...
    require(mintedSupply() < maxSupply, "Max supply has already been minted");

    uint256 indexToMint = (mintedSupply() + _randomness) % maxSupply;

    if (indexToMint == 0) {
      require(to_ == lAddress, "Token 0 reserved");
    } else if (indexToMint == 1) {
      require(to_ == ibAddress, "Token 1 reserved");
    } else if (indexToMint == 2) {
      require(to_ == lAddress, "Token 2 reserved");
    } else if (mintedDeveloperAllocationCounter() < developerAllocation) {
      // The first developerAllocation tokens (excluding tokens 0, 1, and 2) must be given to developerAddress.
      // These tokens and all others are subject to the random offset.
      require(to_ == developerAddress, "First batch of tokens is allocated to developer");
