...
contract ApeTraders is ERC721, Ownable {
    ...
    function mint(uint256 count) external payable {
        ...
        require(_tokenIds.current() < _maxSupply, "Sale ended");
        require(count > 0 && count <= MAX_NFT_PURCHASE, "Exceeds quota of mint");
    ...
    function mintPresale(uint256 count) public payable {
        require(_paused, "Public Minting is currently active");
        require(_presaleStarted, "Presale is currently not active");
    ...
    function reserve(uint256 count) public onlyOwner {
        require(_tokenIds.current() < 7777, "Max tokens reached");
        require(count > 0 , "Count cannot be zero");
        for (uint256 i = 0; i < count; i++) {
            _tokenIds.increment();
