  ...
  PresaleList
{
  ...
  uint16 private constant MINT_LIMIT = 20;

  uint16 private tokensGiven;
  uint16 private constant TOKENS_PARTNER = 378;
  ...
  function setPresaleIsActiveState(bool _presaleIsActive) public onlyOwner {
    presaleIsActive = _presaleIsActive;
  }

  function setSaleIsActiveState(bool _saleIsActive) public onlyOwner {
  ...
  // function presaleMint(bytes32[] memory merkleProof, uint16 _count)
  function presaleMint(uint16 _count) external payable {
    ...
    require(!presaleMinted[_msgSender()], "presaleMinted"); // address can only mint once
    // require(isOnList(merkleRoots[0], merkleProof, _msgSender()), "presaleList");

    presaleMinted[_msgSender()] = true;
    for (uint16 i = 0; i < _count; i++) {
      _mint(_msgSender(), ++ts);
    }
  }

  // function partnerMint(bytes32[] memory merkleProof, uint16 _count) external {
  //   uint256 ts = totalSupply();
  //   require(partnerGiveawayIsActive, "partnerGiveawayIsActive");
  //   require(_count <= presaleCounts[1], "TRANSACTION_LIMIT");
  //   require(ts + _count <= MAX_SUPPLY, "MAX_SUPPLY");
  //   require(!presaleMinted[_msgSender()], "presaleMinted"); // address can only mint once
  //   require(isOnList(merkleRoots[1], merkleProof, _msgSender()), "presaleList");
  ...
  function mintGiveaway(uint16 _count) public onlyOwner {
    uint256 ts = totalSupply();
    require(
      tokensGiven + _count <= (MAX_SUPPLY - TOKENS_PARTNER),
      "tokensGiven"
    );
    require(ts + _count <= MAX_SUPPLY, "MAX_SUPPLY");
    for (uint16 i = 0; i < _count; i++) {
