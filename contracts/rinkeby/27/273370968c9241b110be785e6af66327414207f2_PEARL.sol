...
- ✔️ Implemented Dynamic Sell Logic
    ...
    Team: Angelica Tresvalles, Leonard Bangco, Kriz Resurreccion, Edward Nguyen  
...
contract PEARL is Context, IERC20, Ownable { // Nominal name
    ...
    uint256 private _teamFee = _teamCutPct.add(_marketingCutPct).add(_charityCutPct).add(_liquidityCutPct);
    ...
    uint256 public _liquidityCutPct = 2; // NEW LQ cut
    ...
    uint256 public minimumContractTokenBalanceToSwap = 60000000 * 10**9;   // 0.06% of total supply for both LQ and ETH distribution
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isAdmin[from] && !_isAdmin[from]) {
            ...
            if (!inSwap && swapEnabled && to == uniswapV2Pair) { // Dynamic Selling Logic
                ...
                if(firstSell[from] + (60 seconds) < block.timestamp) {
                    sellNumber[from] = 0;
                }
                if (sellNumber[from] == 0) { // Cooldown Timings
                    ...
                    _teamFee = 13;
                    sellNumber[from]++;
                    sellCooldown[from] = block.timestamp + (60 seconds); //from initial buy +4 hours
                }
                else if (sellNumber[from] == 2) {
                    _teamFee = 16;
                    sellNumber[from]++;
                    sellCooldown[from] = block.timestamp + (60 seconds); //from initial buy +8 hours
                }
                else if (sellNumber[from] == 3) {
                    _teamFee = 19;
                    sellNumber[from]++;
                    sellCooldown[from] = firstSell[from] + (60 seconds); //from initial buy, 24 hours then resets to 60 seconds
                }
            }
