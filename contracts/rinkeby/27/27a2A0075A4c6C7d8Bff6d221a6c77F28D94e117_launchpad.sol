...
interface Token {
    ...
    function getStableTokenPrice() external returns (uint256 price) ;
}
...
modifier  set_status(){
 ...
 function get_qsd_price() public set_status returns (uint qsd_price){
     //qsd_price 
	  Token qsd_token=Token(qsd_token_address);
	  uint256 qsd_price= 0.5*10**18;// qsd_token.getStableTokenPrice()
	  return qsd_price;
}
