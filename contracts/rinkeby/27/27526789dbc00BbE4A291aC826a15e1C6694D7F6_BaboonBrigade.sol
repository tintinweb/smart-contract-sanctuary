...
contract BaboonBrigade is Ownable, ERC721 {
     ...
     */
    function premint(address to, uint256 count) external onlyOwner {
        require(to != address(0), "Cannot premint to 0x0");
        require(count > 0, "No need to mint 0 or less");
        require(
            count + lastTokenId + 1 < BBG_MAX,
            "Sorry, there is not that many BBG left."
        );

        _premint(to, lastTokenId + 1, count);
        lastTokenId += count;
    }
