...
contract AFCharacter is AFRoles{
  ...
  enum Rarities { RARE, SUPERRARE, EPIC, LEGENDARY, MYTHIC }

  // All available characters currently registered in the contract
  Character[] public allCharactersEver;

  // The characters that currently have availability
  uint256[] public availableCharacters;

  // The total number of cards that are still available for minting
  uint256 availableCardCount;
  ...
  // Used to return character IDs in functions that require a transaction (ex. makeCharacter)
  event CharacterId(uint256 id);
  ...
  // TODO: lock off access to this function
  function makeCharacter(
    ...
    Character memory newCharacter = Character(name, rarity, scarcity, 0, artist, collection, series, power,defense,speed,intelligence,magic);

    allCharactersEver.push(newCharacter);
    uint256 newCharacterId = allCharactersEver.length - 1;
    availableCharacters.push(newCharacterId);
    availableCardCount += scarcity;
    emit CharacterId(newCharacterId);
  }
  ...
  function decrementAvailableCharacterCount () private {
  ...
  function takeRandomCharacter() external onlyContract returns (uint256) {
    ...
    require(arrayCount > 0, "Attempted to select a random character, but no characters are available");
    ...
    require(characterID >=0 && characterID < allCharactersEver.length, 
    "Attempted to mint a card with a non-existent character");

    incrementCharacterSupply(characterID);
    decrementAvailableCharacterCount();
  ...
  // Gets the number of remaining cards that can be minted
  function getAvailableCardCount() view external returns (uint256){
...
abstract contract AFRoles is AccessControlEnumerable{
    modifier onlyEditor {
        require(hasRole(EDITOR_ROLE, msg.sender), "Caller is not an editor");
        _;
    }

    modifier onlyContract{
        require(hasRole(CONTRACT_ROLE, msg.sender), "Caller is not a contract");
        _;
    }
    ...
    function setRole(bytes32 role, address user) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || hasRole(ROLE_MANAGER_ROLE, msg.sender), "Caller is not admin nor role manager");
        
        _setupRole(role, user);
    }

    function batchAddPresaleRole(address[] memory addresses) external{
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || hasRole(ROLE_MANAGER_ROLE, msg.sender), "Caller is not admin nor role manager");

        for(uint32 i = 0; i < addresses.length; i++){
            _setupRole(PRESALE_ROLE ,addresses[i]);
        }
    }
}
