...
contract PlayerStoxFactory {
    ...
    constructor(address playerStoxImpl_, address curveWallet_, address adminWallet_) {
    ...
    function addSupportForToken(address token_) external onlyAdmin {
        require(!supportedTokens.contains(token_), "Token already exist.");
        supportedTokens.add(address(token_));
    }

    function removeSupportForToken(address token_) external onlyAdmin {
    ...
    function createPlayerStox(string memory name_, string memory symbol_, uint8 decimals_, uint256 totalSupply_) external onlyAdmin {
        PlayerStoxTokenUpgradableProxy playerStox = new PlayerStoxTokenUpgradableProxy(playerStoxImplementation);
        IPlayerStoxToken(address(playerStox)).initialize(name_, symbol_, decimals_, totalSupply_);
    ...
    function userWithdraw(address token_, address user_, uint256 amount_) external {
        ...
        require(supportedTokens.contains(token_), "Token not supported.");
        IERC20 token = IERC20(token_);
        require(token.allowance(curveWallet, address(this)) >= amount_, "Not enough allowance");
        token.transferFrom(curveWallet, user_, amount_); //replace with safeERC20
    }
    ...
    function userDeposit(address token_, address user_, uint256 amount_) external {
        ...
        token.transferFrom(user_, curveWallet, amount_); //replace with safeERC20
    }
