...
///
/// @title Contract that contains all active pools
///
/// @dev This contract is central repository for ParibusStakeContractV3, it's used by frontend, and deployer of
///      ParibusStakeContractV3 should call ParibusStakeManager.addContract(...) after deploying it.
///
contract ParibusStakeManager is Ownable {
    address[] public contracts;

    ///
    /// @dev Get contract length.
    ///
    ...
    ///
    /// @dev Get contract array. This is used by frontend
    ///
    ...
    ///
    /// @dev Add contract to internal array
    ///
    ...
    ///
    /// @dev Remove contract from internal array. Because of how solidity handles array, we don't want to guarantee
    ///      that order of elements will be preserved. So assume that order of contract will change after calling this
    ///      method.
    ///
    /// @param _contract Address of contract
    ///
    ...
    ///
    /// @dev Check if _contract is listed inside internal array.
    ///
    function hasContract(ParibusStakeContractV3 _contract) external view returns (bool) {
        for (uint256 it = 0; it < contracts.length; ++it) {
            if (address(_contract) == contracts[it]) {
                return true;
            }
