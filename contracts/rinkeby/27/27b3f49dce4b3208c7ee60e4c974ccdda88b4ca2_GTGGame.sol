...
interface IGTG {
	function mint(address recipient, uint16 tokenId) external;
	...
	function totalGoblinsMinted() external view returns (uint16);
}
...
contract GTGGame is IGTGGame, Ownable, ReentrancyGuard {
	struct Whitelist {
		bool isWhitelisted;
		uint16 numMinted;
	}
	...
	uint16 private constant MAX_PRESALE_MINTS = 3;
	...
	constructor() {
		...
		// phasePrice[1] = 0.06969 ether;
		//testing
		phasePrice[1] = 0.0001 ether;
		...
		onlyOwner
	{
		...
		if (phase == 1) {
			...
			internalAddTokens(40001, 50000);
		}
	}

	function mint(uint256 _amount, bool _stake)
		public
		payable
		whenNotPaused
		nonReentrant
	{
		...
		uint16 minted = GTG.getTotalMinted();
		uint256 maxTokens = GTG.getMaxTokens();
		require(minted + _amount <= maxTokens, "All tokens minted");
		require(
		...
		if (phase == 1) {
			if (hasPublicSaleStarted) {
				...
				require(
					_whitelistAddresses[_msgSender()].numMinted + _amount <=
						MAX_PRESALE_MINTS,
					"too many mints"
				);
				_whitelistAddresses[_msgSender()].numMinted += uint16(_amount);
			}
		} else {
			require(msg.value == 0, "Now minting is done via Gold");
			totalGoldCost = mintPrice(_amount);
			require(
				gold.balanceOf(msg.sender) >= totalGoldCost,
				"Not enough Gold"
			);
		}
		...
		for (uint256 i = 0; i < _amount; i++) {
			...
			if (!_stake || recipient != msg.sender) {
				GTG.mint(recipient, tokenId);
			} else {
				GTG.mint(address(mines), tokenId);
				tokenIds[i] = tokenId;
			}
		}
		if (_stake) {
			mines.addTokensToStake(msg.sender, tokenIds);
		}
	}

	function selectRecipient() internal returns (address) {
		if (phase == 1) {
			return msg.sender; // During ETH sale there is no chance to steal NFTs
		}

		// 10% chance to steal NFTs
		if (getSomeRandomNumber(GTG.totalGoblinsMinted(), 100) >= 10) {
			return msg.sender; // 90%
		}

		address thief = mines.randomGoblinOwner();
	...
	function checkIfWhitelisted(address addy)
		external
		view
		returns (Whitelist memory)
	{
		return _whitelistAddresses[addy];
	}
	...
	function getTokenToBeMinted() private returns (uint16) {
		uint256 random = getSomeRandomNumber(
	...
	function changeRandomSource(uint16 _id, address _address)
		external
		onlyOwner
	{
