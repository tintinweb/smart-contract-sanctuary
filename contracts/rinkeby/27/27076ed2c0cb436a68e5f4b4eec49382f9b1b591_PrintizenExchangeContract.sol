...
contract PrintizenExchangeContract is Sale{
    ...
    function transferOwnershipForColle(address newOwner, address token721, address token1155) public onlyOwner {
        require(newOwner != address(0), "Ownable: new owner is the zero address");
        Printizen721 tok= Printizen721(token721);
        Printizen1155 tok1155= Printizen1155(token1155);
    ...
    function mint(address token ,string memory tokenuri, uint256 value, uint256 tokenId, uint256 royal, uint256 _type, uint256 supply) public{
       ...
       if(_type == 721){
           Printizen721 tok= Printizen721(token);
           ...
           if(value != 0){
            ...
            Printizen1155 tok = Printizen1155(token);
    ...
    function swapfee(uint256 amount) internal{
        address payable admin = address(uint160(owner()));
        admin.transfer(amount);
        
    }
    function tokenTransfer(address to,uint256 tokenId, address token, uint256 amount, uint256 _type, uint256 NOFToken) public payable{
