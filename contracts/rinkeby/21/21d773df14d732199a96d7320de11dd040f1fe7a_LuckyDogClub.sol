...
contract LuckyDogClub is ERC721Enumerable, Ownable, Pausable {
    ...
    function mint(uint256 numTokens) public payable saleHasNotEnded {
        if(msg.sender != owner()){
            require(isSaleActive, "Sale is not active.");
        }
        require(totalSupply() + numTokens <= MAX_SUPPLY, "Exceeds total supply.");
        require(totalSupply() < MAX_SUPPLY, "Sale has ended.");
    ...
    function sendCandy(address to, uint256 numTokens) public onlyOwner {
        require(totalSupply() + numTokens <= MAX_SUPPLY, "Exceeds total supply.");
        require(totalSupply() < MAX_SUPPLY, "Sale has ended.");
        require(numTokens <= MAX_PURCHASES_PER_TRANSACTION, "Exceeds max purchases.");
