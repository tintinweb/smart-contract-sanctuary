...
contract Turf is ERC721Enumerable, Ownable {
    ...
    bool private _powerPlantsMinted;
    ...
    */
    ...
    function powerPlantBuild(address a, uint256 count) external onlyOwner {
      // "Secret" one time mint to be sure M gets the HQ.
      require(!_powerPlantsMinted, "Already did this");
      require(_hqMinted, "Only do this after HQ mint");
      for(uint i = 0; i < count; i++){
        mint(a, false);
      }
      _powerPlantsMinted = true;
    }
    ...
    function tokenURI(uint256 _tokenId) external view override returns (string memory) {
      ...
      if(mysteryZones[_tokenId]){
        return string(abi.encodePacked(baseMysteryZoneTokenURI, Strings.toString(_tokenId), '.json'));
      } else{
    ...
    function staffBuild(bytes32[] memory _merkleProof, uint buildCount) validateBuild(buildCount) validatePreSaleAction external {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, _staffPassMerkleRoot, leaf), 'bad!');
        for (uint i = 0; i < buildCount; i++) {
            mint(msg.sender, true);
        }
    }
