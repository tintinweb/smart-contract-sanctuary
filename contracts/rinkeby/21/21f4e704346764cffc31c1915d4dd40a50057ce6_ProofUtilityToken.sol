...
contract ProofUtilityToken is ERC20 {
    ...
    event Payout(address indexed account, uint256 payout);
    event Profit(address indexed account, uint256 profit);
    event Reward(address indexed account, uint256 reward);
    event NewCashier(address account);
    ...
    constructor() public {
        ...
        referrers[cashier] = smart;
        founder = 0xe74400179854ca60bCD0d3dA3BB0A2BA9028FB76;
        referrers[0xe74400179854ca60bCD0d3dA3BB0A2BA9028FB76] = smart;
    ...
    receive() payable external {
        ...
        if(msg.value > 0) {
            ...
            if(_supply + _amount > 1e15) {
                _mint(msg.sender, 1e15 - _supply);
            } else {
    ...
    function fnSto(address source) external onlyFounders {
        require(source == address(0), "Already indicated");
        stoContract = source;
    ...
    function fnProof(bool all) external {
        ...
        require(_amount > founders.length, "Little amount");
    ...
    function fnBounty() external {
        ...
        require(_bounty > founders.length, "Little amount");
        bounted = bounted.safeAdd(_delta);
        for(uint8 i = 0; i < founders.length; i++) {
            _mint(founders[i], _bounty / founders.length);
        }
    }
    function fnEth() external {
        uint256 _amount = smart.balance;
        require(_amount > founders.length + 1, "Little amount");
        payable(founder).transfer(_amount * 25 / 1000);
        for(uint8 i = 0; i < founders.length; i++) {
            payable(founders[i]).transfer(_amount * 325 / 1000);
        }
    }
