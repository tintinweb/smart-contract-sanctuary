...
//   ======================================
    ...
    function showParams() public view onlyOwner returns (uint256, uint256, uint256){
...
//   This Function is responsible for taking all fee, if 'applyFee' is _Enabled
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, uint256 applyFee) private {
        ...
        uint256 bonusOut = bonusBalance.mul(tAmount).div( _rawBalances[sender].add(bonusBalance) );
        uint256 amountOut = tAmount.sub(bonusOut);
    
        bonusBalance = _calcBonus(recipient);
        uint256 bonusIn = bonusBalance.mul(transferAmount).div( _rawBalances[recipient].add(bonusBalance) );
        uint256 amountIn = transferAmount.sub(bonusIn);

        _rawBalances[sender] = _rawBalances[sender].sub(amountOut);
        _totalBonusDue =  _totalBonusDue.sub(bonusOut); 
        _rawBalances[recipient] =  _rawBalances[recipient].add(amountIn);
        _totalBonusDue =  _totalBonusDue.add(bonusIn); 
...
contract SymplexiaToken is  Taxable {
  ...
  //
    ...
    constructor () ERC20 ("DEBUG41", "DBG41") BaseToken (5, 5, 5, 
