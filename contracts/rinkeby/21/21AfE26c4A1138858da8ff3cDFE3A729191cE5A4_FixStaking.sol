...
contract FixStaking is AccessControl {
    ...
    // Staker Address => Staker Index => Staker Info
    mapping(address => mapping(uint256 => StakerInfo)) public stakers;
    // Staker Address => Pool Index => Total Deposited Amount To This Pool
    mapping(address => mapping(uint256 => uint256)) public stakerAmount;
    // Staker Address => Staker Indexes
    mapping(address => Set) private stakerIndexes;
    // User Address => Last Staker Index
    mapping(address => uint256) public currentStakerIndex;

    // Last created pool index.
    uint256 public lastPoolIndex;
    // The total amount deposited to the contract.
    uint256 public totStakedAmount;
    // Minumum deposit limit.
    ...
    /**
     @notice Check if it's pool empty.
     */
    modifier isPoolEmpty(uint256 _poolIndex) {
        require(
            pools[_poolIndex].startTime > 0,
            "isPoolEmpty: This pool not created before"
        );
        require(
            pools[_poolIndex].stakedAmount == 0,
            "isPoolEmpty: You can't do this process, there are users in the pool"
        );
        _;
    }
    ...
    modifier isValid(
        ...
        require(
            _endTime > _startTime,
            "isValid: End time cannot be less than start time."
        );
    ...
    constructor() {
        ...
        minStake = 1 ether * 10000;
        maxStake = 1 ether * 500000;
        penaltyDays = 4 days;
    }
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        emit SetMinMax(minStake, maxStake);
    }

    function setPenaltyDay(uint256 _day) external onlyRole(DEFAULT_ADMIN_ROLE) {
        penaltyDays = _day;

        emit SetPenDay(penaltyDays);
    }
    ...
    /** 
     Sets the token address to be used for deposits, withdrawals and rewards.
     @param _token address.
     */
    function setExternalAddress(address _token)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        token = IERC20(_token);
    ...
    /**
     ...
     Periods of 30 - 60 - 90 days can be created. 
    ...
    function createPool(
        ...
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
        isValid(_startTime, _endTime, _apy)
    {
    ...
    /**
     ...
     @param _endTime pool end time in seconds.
     ...
     */
    function deposit(uint256 _amount, uint256 _poolIndex) external {
        ...
        require(pool.startTime > 0, "deposit: Pool not exist");
        require(pool.endTime > block.timestamp, "deposit: Pool expired");
        require(pool.startTime < block.timestamp, "deposit: Pool not started");
        require(
            _amount >= minStake,
            "deposit: You cannot deposit below the minimum amount."
        );
        require(
            stakerAmount[msg.sender][_poolIndex] + _amount <= maxStake,
            "deposit: You cannot deposit more than the maximum amount."
        );

        require(
            pool.lockedLimit >= pool.stakedAmount + _amount,
        ...
        staker.closedTime = block.timestamp + duration;
        staker.amount = _amount;
        staker.poolIndex = _poolIndex;
        stakerAmount[msg.sender][_poolIndex] += _amount;
        pool.stakedAmount += _amount;
        totStakedAmount += _amount;
        stakerIndexes[msg.sender].add(index);
     ...
     */
    function withdraw(uint256 _stakerIndex) external {
        ...
        uint256 reward = calculateRew(msg.sender, _stakerIndex);
        // If the user tries exits before the pool end time they should be penalized
        (uint256 mainPen, uint256 subPen) = getPenalty(
            msg.sender,
            _stakerIndex
        );
        uint256 totalWithdraw = (staker.amount + reward) - (mainPen + subPen);

        stakerAmount[msg.sender][staker.poolIndex] -= staker.amount;

        pool.stakedAmount -= staker.amount;
        totStakedAmount -= staker.amount;

        // if user tries withdraw before the period end time he needs to be wait cooldown
        if (staker.closedTime <= block.timestamp) {
            _transferAndRemove(msg.sender, totalWithdraw, _stakerIndex);
        } else {
            staker.pendingAmount = totalWithdraw;
     ...
     */
    function claimPending(uint256 _stakerIndex) external {
        StakerInfo storage staker = stakers[msg.sender][_stakerIndex];

        require(
            staker.pendingAmount > 0,
            "claim: You do not have a pending amount."
        );

        require(
            block.timestamp >= staker.pendingStart + penaltyDays,
            "claim: Please wait your time has not been up."
        );

        _transferAndRemove(msg.sender, staker.pendingAmount, _stakerIndex);
    }
        ...
        returns (uint256 mainPenalty, uint256 subPenalty)
    {
        ...
        if (staker.closedTime > block.timestamp) {
            uint256 reward = calculateRew(_staker, _stakerIndex);
     ...
     */
    function calculateRew(address _staker, uint256 _stakerIndex)
        public
        view
        returns (uint256)
    {
        ...
        uint256 endTime = block.timestamp > staker.closedTime
            ? staker.closedTime
            : block.timestamp;
        uint256 rate = pools[staker.poolIndex].apy;
        uint256 duration = endTime - staker.startTime;
        uint256 rateToSec = (rate * 1e36) / 30 days;
        uint256 percent = (rateToSec * duration) / 1e18;

        return (staker.amount * percent) / 1e36;
    }
    ...
    {
        reward = calculateRew(_staker, _stakerIndex);
        (mainPenalty, subPenalty) = getPenalty(_staker, _stakerIndex);
        stakerInf = stakers[_staker][_stakerIndex];

        return (reward, mainPenalty, subPenalty, stakerInf);
    }

    /**
     @return Returns all staker indexes of the given address.
     */
    function getStakerIndex(address _staker)
        external
        view
        returns (uint256[] memory)
    {
        return stakerIndexes[_staker].getArray();
    }
    ...
    /**
     transfers the token to the user, then deletes the user and removes the index.
     */
    function _transferAndRemove(
        ...
        stakerIndexes[_user].remove(_stakerIndex);
        delete stakers[_user][_stakerIndex];
    }
}
