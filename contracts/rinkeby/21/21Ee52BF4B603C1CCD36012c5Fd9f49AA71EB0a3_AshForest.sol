...
contract AshForest is AdminControl, ICreatorExtensionTokenURI, ILazyDelivery, ILazyDeliveryMetadata {
    ...
    function deliver(address, uint256 listingId, uint256, address, uint256, uint256) external override returns(uint256) {
        require(listingId == _listingId);
        return IERC721CreatorCore(_creator).mintExtension(msg.sender);
    }
