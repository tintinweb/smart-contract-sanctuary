...
contract Neopets is Context, ERC165, Ownable, INeopets, IERC721Metadata {
    ...
    uint public constant NAME_CHANGE_PRICE = 180 * (10 ** 18);

    uint public constant MAX_TOKENS_SUPPLY = 20500;

    uint private constant MAX_RESERVED_TOKENS = 500;
    ...
    mapping (uint => string) private _tokenName;

    mapping (string => bool) private _nameReserved;

    mapping (uint => bool) private _mintedBeforeReveal;

    mapping (address => mapping (address => bool)) private _operatorApprovals;

    event NameChange (uint indexed index, string newName);

    constructor(string memory name_, string memory symbol_, address nnrAddress) {
        _name = name_;
        _symbol = symbol_;
    ...
    function revealTimestamp() public view override returns (uint) {
        require(_saleStart != 0, "Sale is not started");

        return _saleStart + 3 * 86400;
    }

    function reserveTokens(uint tokenAmount) public onlyOwner {
        require(tokenAmount > 0, "Token amount cannot be 0");
        require(_totalReservedTokens + tokenAmount <= MAX_RESERVED_TOKENS, "Reserve limit is exceeded");

        for (uint i; i < tokenAmount; i++) {
            uint mintIndex = totalSupply();
            _safeMint(_msgSender(), mintIndex);
            _totalReservedTokens++;
        }
    }

    function getPriceByIndex(uint tokenIndex) private view returns (uint) {
        if (tokenIndex >= _totalReservedTokens) {
            return 100000000000000000;
        } else if (tokenIndex >= 3000 + _totalReservedTokens) {
            return 200000000000000000;
        } else if (tokenIndex >= 7000 + _totalReservedTokens) {
            return 250000000000000000;
        } else if (tokenIndex >= 11000 + _totalReservedTokens) {
            return 300000000000000000;
        } else if (tokenIndex >= 15000 + _totalReservedTokens) {
            return 400000000000000000;
        } else if (tokenIndex >= 18000 + _totalReservedTokens) {
            return 500000000000000000;
        } else if (tokenIndex >= 19000 + _totalReservedTokens) {
            return 750000000000000000;
        } else if (tokenIndex >= 19750 + _totalReservedTokens) {
    ...
    function isMintedBeforeReveal(uint tokenIndex) public view override returns (bool) {
        return _mintedBeforeReveal[tokenIndex];
    }

    function mint(uint tokenAmount) public payable {
        require(_saleStart != 0, "Sale is not started");
        require(tokenAmount > 0, "Tokens amount cannot be 0");
        require(totalSupply().add(tokenAmount) <= MAX_TOKENS_SUPPLY, "Purchase would exceed max supply");
        require(tokenAmount <= MAX_TOKENS_MINT_AT_ONCE, "Can only mint 20 tokens at a time");

        uint price = getPrice(tokenAmount);
    ...
    function tokenNameByIndex(uint tokenIndex) public view returns (string memory) {
        return _tokenName[tokenIndex];
    }
    ...
    function validateName(string memory str) private pure returns (bool){
        ...
        for(uint i; i < b.length; i++){
            ...
            if(!(char >= 0x30 && char <= 0x39) && !(char >= 0x41 && char <= 0x5A) && !(char >= 0x61 && char <= 0x7A) && !(char == 0x20)) return false;
    ...
    function toggleReserveName(string memory str, bool isReserved) internal {
        _nameReserved[toLower(str)] = isReserved;
    }

    function changeName(uint tokenIndex, string memory newName) public {
        address owner = ownerOf(tokenIndex);

        require(_msgSender() == owner, "Caller is not the owner");
        require(validateName(newName), "New name is not valid");
        require(sha256(bytes(newName)) != sha256(bytes(_tokenName[tokenIndex])), "New name cannot be same as previous one");
        require(!isNameReserved(newName), "Name is reserved");

        IERC20(_nnrAddress).transferFrom(_msgSender(), address(this), NAME_CHANGE_PRICE);
        IERC20Burnable(_nnrAddress).burn(NAME_CHANGE_PRICE);

        if (bytes(_tokenName[tokenIndex]).length > 0) {
            toggleReserveName(_tokenName[tokenIndex], false);
        }

        toggleReserveName(newName, true);
        _tokenName[tokenIndex] = newName;

        emit NameChange(tokenIndex, newName);
    }
