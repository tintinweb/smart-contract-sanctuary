...
contract XcuteMaster is Ownable {
    ...
    function worker(address _address) external view returns (bool) {
        return IXcuteStaker(staker).staked(_address) > 0;
    }
    ...
    function initializeWorkWithRequirements(
        ...
        gasCheckpoint = gasleft();
        uint256 _staked = IXcuteStaker(staker).staked(_address);
        if (_staked == 0 || (_minimumStaked > 0 && _staked < _minimumStaked))
            return false;
        if (
            _minimumEarned > 0 &&
    ...
    function finalizeWork(address _worker) external {
        if (IXcuteStaker(staker).staked(_worker) == 0) revert InvalidWorker();
