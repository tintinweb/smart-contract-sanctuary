...
contract TokenSeller{
	...
	function setProxyInfoForChainLink(address[] memory _proxies) external {
		...
		for (uint256 i = 0; i < _proxies.length; i++) {
			priceFeed[i] = AggregatorV3Interface(_proxies[i]);
		}
		proxies = _proxies;
	}
	function getPriceFromChainLink() public view returns (uint256[] memory) {
		require(tokenOwner == address(msg.sender), 'getPriceFromChainLink : not an owner');
		require(proxies.length > 0, 'getProxyInfoForChainLink : length must be greater than 0');
