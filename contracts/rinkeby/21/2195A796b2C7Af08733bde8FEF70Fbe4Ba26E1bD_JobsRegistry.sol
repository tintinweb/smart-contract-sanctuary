...
contract JobsRegistry is IJobsRegistry, Ownable {
    ...
    function addJob(
        ...
        emit AddJob(_job, _owner, _contentHash);
    }
    ...
    function upgradeJob(
        ...
        if (bytes(_newContentHash).length == 0) revert InvalidContentHash();
        if (msg.sender != owner()) revert Forbidden();
        ...
        _newJobFromStorage.contentHash = _newContentHash;
        ...
        emit UpgradeJob(_oldJob, _newJob, _newContentHash);
    }
    ...
    function _addCredit(
        ...
        _jobFromStorage.credits.add(_token, _amount - _fee, false);
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        IERC20(_token).safeTransfer(feeReceiver, _fee);
        emit AddCredit(_job, _token, _amount);
    }
    ...
    function jobsOfOwner(address _owner) external view returns (Job[] memory) {
        ...
        Job[] memory _jobsOfOwner = new Job[](_jobsAmount);
        for (uint256 _i = 0; _i < _jobsAmount; _i++)
            _jobsOfOwner[_i] = job[_jobAddresses[_i]];
        return _jobsOfOwner;
    }
...
interface IJobsRegistry {
    ...
    event AddJob(address job, address owner, string contentHash);
    event UpgradeJob(address oldJob, address newJob, string newContentHash);
    event RemoveJob(address job);
    event AddCredit(address job, address token, uint256 amount);
    event RemoveCredit(address job, address token, uint256 amount);
