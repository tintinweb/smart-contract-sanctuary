...
contract NavyONE is Ownable, fundTokenized{
    ...
    function getTokenPrice (uint256 tokenIndex, uint256 eth2usd) public view returns(uint256)  { //should be internal
    ...
    function getTotalValue () public view returns(uint256)  {
        ...
        for (uint256 i = 0; i < valuedTokens.length; i ++){
            ...
            if (balance > 0){
                tokenValue = getTokenBalance(i).mul(getTokenPrice(i, eth2usd)).mul(BASE + valuedTokens[i].premium);
            }else{
                tokenValue = 0;
            }
    ...
    function calInvestValue (uint256 tokenIndex, uint256 amount) public view returns(uint256)  { 
