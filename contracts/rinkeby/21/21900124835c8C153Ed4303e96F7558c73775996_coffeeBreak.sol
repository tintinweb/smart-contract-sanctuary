...
contract coffeeBreak is Ownable {
    ...
    uint constant VERSION = 5;
    bool started;
    uint startTime;
    address judge;
    address coffeeCoinRegistery = 0xEbe45e73Dcad7e96f504091A7D8b71215111f2A4;
    uint coffeePayment = 10000000000000000000;

    uint minMembers = 0; // minimum team count
    uint maxMembers = 36; 
    uint minRounds = 20; // min rounds to claim prize

    uint round; 
    uint deadline; // deadline timestamp
    uint interval;
    uint gracePeriod; // 15 min
    ...
    function createTeam(address[] memory _members, string memory _name) public {
        ...
        teams[numTeams].capchas.push(capcha({
...
// JUDGE FUNCTIONS
    ...
    function setCoffeePayment(uint _newPaymentAmount) public {
        coffeePayment = _newPaymentAmount;
    }

    function setMinRounds(uint _minRounds) public {
...
// VIEW FUNCTIONS
    ...
    function getCoffeePayment() public view returns (uint) {
        require (msg.sender == judge, "not judge");
        return coffeePayment;
    }

    function getMinRounds() public view returns (uint) {
        require (msg.sender == judge, "not judge");
        return minRounds;
    }
