...
contract WaultVenusVaultBNB is ERC20, Ownable {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 _accWaultPerShare = accWaultPerShare;
        uint256 lpSupply = balance();
        if (block.number > lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier();
            uint256 waultReward = multiplier.mul(rewardPerBlock);
            _accWaultPerShare = _accWaultPerShare.add(waultReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(_accWaultPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function safeTransferReward(address to, uint256 value) internal {
        IUniswapRouter(unirouter).swapExactTokensForTokens(value, 0, wbnbToWaultRoute, to, block.timestamp.add(600));
    }
    ...
    function harvest() public {
        ...
        safeTransferReward(address(msg.sender), rewardAsWault);
        earn();
    }
