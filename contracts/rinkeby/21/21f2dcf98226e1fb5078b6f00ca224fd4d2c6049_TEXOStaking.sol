...
contract TEXOStaking is Ownable {
    ...
    mapping(address => StakeHolder) internal stakeholdersMap;

    constructor(string memory stakeTokenName) public {
        stakeToken = stakeTokenName;
    }

    modifier onlyStaker(address anyAddress) {
        if (stakeholdersMap[anyAddress].owner != address(0)) {
            _;
        } else {
            return;
        }
    ...
    modifier onlyGuest(address anyAddress) {
        if (stakeholdersMap[anyAddress].owner == address(0)) {
            _;
        } else {
            return;
    ...
    modifier mustBeStaker(address anyAddress) {
        require(stakeholdersMap[anyAddress].owner != address(0), "Only a staker allowed");
        _;
    }
     ...
     */
    function addNewStakeholder(address stakeholderAddress, uint256 stakeAmount) internal onlyGuest(stakeholderAddress) {
     ...
     */
    function removeStakeholder(address stakeholderAddress) internal onlyStaker(stakeholderAddress) {
     ...
     */
    function calculateReward(address stakeHolderAddresses) public onlyStaker(stakeHolderAddresses) view returns (uint256) {
