...
contract Cells is ERC721Cells {
    ...
    function mintCells(uint amount) public payable {
        ...
        require(amount >= minMintable, "Minimum quantity too low");        
        require(amount <= MAX_CELLS_PURCHASE, "Can only mint 25 cells at a time");
        require(mintedSupply + amount <= maxMintable, "Purchase would exceed max mintable Cells");
        require(totalSupply() + amount <= MAX_CELLS, "Purchase would exceed max Cells supply");
