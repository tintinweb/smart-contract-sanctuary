...
contract FullOnchainTest is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function pick(uint256 tokenId, string[] memory sourceArray) internal pure returns (string memory) {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "#', toString(tokenId),'Limited Edition Otaku Coin', '", "description": "Spread Awareness of your Favorite Anime Around the World!", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    uint256 _tmpNumber = value;
        ...
        while (_tmpNumber != 0) {
            digits -= 1;
            buffer[digits] = bytes1(uint8(48 + uint256(_tmpNumber % 10)));
            _tmpNumber /= 10;
        }
