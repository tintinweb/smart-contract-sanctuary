...
contract PunksMarket is Ownable, Pausable , ReentrancyGuard{
    ...
    */
    ...
    /**
    * Returns offer, bid and owner data for ALL punks.
    */

    function getAllPunks() external view returns (Punk[] memory) {
        Punk[] memory punks = new Punk[](TOTAL_PUNKS);
        for (uint i = 0; i < TOTAL_PUNKS; i++) {
            address owner = punkContract.punkIndexToAddress(i);
            bool wrapper = false;
            if (owner==address(this)){
                owner = punksWrapperContract.ownerOf(i);
                wrapper = true;
            }
            punks[i]=Punk(wrapper,owner,punkBids[i],punksOfferedForSale[i]);
        }
        return punks;
    }
    ...
    */
    function getWrappedPunksForAddress(address user) external view returns(uint256[] memory) {
        ...
        uint256 i=0;
        for (i=0; i<TOTAL_PUNKS; i++) {
            if ( punksWrapperContract.ownerOf(i) == user ) {
                punks[j] = i;
                j++;
            }
        }
