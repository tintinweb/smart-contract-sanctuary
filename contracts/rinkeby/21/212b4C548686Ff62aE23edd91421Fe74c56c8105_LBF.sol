...
contract LBF is ERC721, Ownable {
    using Strings for uint256;
    ...
    uint256 public constant MAX_MINT_AMOUNT = 18;
    uint256 public constant WHITELIST_MAX_MINT_AMOUNT = 6;
    uint256 public whitelist_cost = 0.02 ether;
    uint256 public cost = 0.025 ether;
    uint256 public marketingSupply = 12;

    Counters.Counter private _tokenSupply;

    event NewLBFMinted(address sender, uint256 tokenId);
    ...
    modifier mustPassChecks(uint256 _mintAmount) {
        ...
        require(
            msg.value >=
                (isOnlyWhitelisted ? whitelist_cost : cost) * _mintAmount ||
                msg.sender == owner(),
            "Insufficient funds"
        );
        _;
    ...
    function _processMint(uint256 _mintAmount) internal {
        for (uint256 i = 1; i <= _mintAmount; i++) {
            ...
            emit NewLBFMinted(msg.sender, id);
            _tokenSupply.increment();
        }
    }
    ...
    function withdraw() public payable onlyOwner {
        ...
        //Owner
        (bool successA, ) = payable(owner()).call{value: (balance / 100) * 87}(
            ""
        );
        //Dev
        (bool successB, ) = payable(0x51D8f89Fae2e82c4bAcF6Ec270f36624bc9C6D1E)
            .call{value: (balance / 100) * 13}("");
        require(successA && successB, "Withdraw failed");
