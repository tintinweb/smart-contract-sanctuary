...
contract Presale {
    ...
    struct PresaleBuyer {
        ...
        uint256 amountFast; // Rewards token that needs to be vested.
    }
    ...
    uint256 public constant IDS = 100e27; // Total FastToken amount for presale : 100b
    ...
    uint256 public soldFastAmount;

    IFastToken public fastToken; // Rewards Token : Token for distribution as rewards.
    ...
    modifier whileDeposited() {
        require(getDepositedFast() >= IDS, "Deposit enough Fast tokens to the vesting contract first!");
        _;
    }

    constructor(address _fastToken, address payable _multiSigAdmin) {
        owner = msg.sender;
    ...
    /********************** Internal ***********************/
    /// Get the FastToken amount of vesting contract

    function getDepositedFast() internal view returns (uint256) {
        address addrVesting = address(vestingContract);
        return fastToken.balanceOf(addrVesting);
    }

    function getUnsoldFast() internal view returns (uint256) {
        uint256 totalDepositedFast = getDepositedFast();
        return totalDepositedFast.sub(soldFastAmount);
    }

    /********************** External ***********************/
    function remainingFast() external view returns (uint256) {
        return getUnsoldFast();
    }
    ...
    function setFastTokenAddress(address _fastToken) external onlyOwner {
        require (_fastToken != address(0x00));
        fastToken = IFastToken(_fastToken);
    }
    ...
    /// After presale ends, we withdraw unsold FastToken to multisig.
    function withdrawUnsoldFastToken() external whileFinished onlyOwner returns (uint256) {
        require(multiSigAdmin != address(0x00), "Withdraw: Project Owner address hasn't been set!");
        require(address(vestingContract) != address(0x00), "Withdraw: Set vesting contract!");

        uint256 unsoldFast = getUnsoldFast();

        require(
            fastToken.transferFrom(address(vestingContract), multiSigAdmin, unsoldFast),
            "Withdraw: can't withdraw Fast tokens"
        );
    ...
    function deposit(address sender) external payable whileOnGoing returns (uint256) {
        ...
        uint256 newFastAmount = weiAmount.div(RATE);

        recipients[sender].amountDepositedWei = newDepositedWei;
        soldFastAmount = soldFastAmount.add(newFastAmount);

        recipients[sender].amountFast = recipients[sender].amountFast.add(newFastAmount);
        vestingContract.updateRecipient(sender, recipients[sender].amountFast, 0);

        require(weiAmount > 0, "Deposit: No ETH balance to withdraw");
        multiSigAdmin.transfer(weiAmount);

        return recipients[sender].amountFast;
    }
}
