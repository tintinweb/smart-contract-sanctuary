...
import 'hardhat/console.sol'; // TODO: remove for deploy

// import contract utils
import './RandomlyAssigned.sol';

contract Human is ERC721, ERC1155Holder, Ownable, RandomlyAssigned {
	using Strings for uint256; // TODO: should this be removed?

	/*
	 * Private Variables
	 */
	// TODO: update `NUMBER_OF_GENESIS_HUMANS_PLUS_HONORARIES` to reflect number of final honoraries, or set it via constructor
	uint256 private constant NUMBER_OF_GENESIS_HUMANS = 229; // there are 229 Humans in the Genesis Collection
	uint256 private constant NUMBER_OF_RESERVED_HUMANS = 33; // TODO: change this
	uint256 private constant MAX_HUMANS_SUPPLY = 1500; // collection total (including genesis)
	uint256 private constant MAX_TEAM_HUMANS = 69; // reserved for the team and marketing
	uint256 private constant MAX_MINTS_PER_ADDRESS = 4; // max total mints (incl. presale)
	...
	uint256 public reservedTokenCount;

	mapping(address => MintTypes) public _addressToMints;

	// TODO: remove before deploying, this is just for testing we don't go over the upper/lower bounds for random token assignment
	/*
	 * TESTING ONLY
	 */
	...
	constructor(string memory uri, address adminSigner)
		ERC721('Mebges', 'ABCD') // TODO: Update this
		RandomlyAssigned(
			MAX_HUMANS_SUPPLY,
	...
	// TODO: test that tokenCount() returns the number minted (not claimed by genesis)
	...
	function claimAuthorTokens(
		...
		require(_isVerifiedCoupon(digest, coupon), 'Invalid coupon');
		require(
			count + _addressToMints[msg.sender]._numberOfAuthorMintsByAddress <=
				allotted,
			'Exceeds number of earned Tokens'
		);
		_addressToMints[msg.sender]._numberOfAuthorMintsByAddress += count;
		...
		validateEthPayment(count)
	{
		...
		require(
			count + _addressToMints[msg.sender]._numberOfMintsByAddress <=
		...
		_addressToMints[msg.sender]._numberOfMintsByAddress += count;
	...
	/*
		FOR TESTING ONLY: 
	*/
	function testERC1155Data(bytes memory data, uint256[] calldata openseaIds)
		external
		onlyOwner
	{
		(uint256[] memory genesisIds, Coupon[] memory coupons) = abi.decode(
			data,
			(uint256[], Coupon[])
		);

		// uint256[] memory openseaIds = new uint256[](2);
		// openseaIds[
		// 	0
		// ] = 91134134745631553866546332671427385652842381040538843676297470613474590588929;
		// openseaIds[
		// 	1
		// ] = 91134134745631553866546332671427385652842381040538843676297470614574102216705;

		for (uint256 i; i < genesisIds.length; i++) {
			console.log('id : ', genesisIds[i]);
			console.log('opensea id: ', openseaIds[i]);
			bytes32 digest = keccak256(
				abi.encode(CouponType.Genesis, genesisIds[i], openseaIds[i])
			);
			require(_isVerifiedCoupon(digest, coupons[i]), 'Invalid coupon');
		}
	}

	/// forward a single token submission to the batch receive method
	/// @dev wraps the args in arrays and forwards them to the batch receive method
	function onERC1155Received(
		...
		values[0] = value;
		uint256[] memory genesisIds = new uint256[](1);
		genesisIds[0] = genesisId;
		Coupon[] memory coupons = new Coupon[](1);
		coupons[0] = coupon;
		...
		onERC1155BatchReceived(
			...
			ids,
			values,
			abi.encode(genesisIds, coupons)
		);
		return this.onERC1155Received.selector;
	}

	/// Override the batch receive method to mint genesis tokens
	/// @dev only mints tokens if sent from `_openseaContractAddress` and `data contains the necessary coupon
	/// @param from address of sender
	/// @param ids array of openseaContract token IDs
	/// @param data ecoded data containing coupon and genesis token IDs
	function onERC1155BatchReceived(
		...
		for (uint256 i; i < ids.length; i++) {
			bytes32 digest = keccak256(
				abi.encode(CouponType.Genesis, genesisIds[i], ids[i])
			);
			require(_isVerifiedCoupon(digest, coupons[i]), 'Invalid coupon');
			_claimReservedToken(from, genesisIds[i]);
		}
	...
	// TODO: remove this prior to deployment
	/*
	 * TESTING ONLY ( REMOVE BEFORE DEPLOYING )
	 * This functions only purpose is to test that token IDs outside of the lower or upper bounds are minted
	 */
	function mintRandomTestTokens(uint256 count)
		external
		ensureAvailabilityFor(count)
		onlyOwner
	{
		require(
			tokenCount() + count <= MAX_HUMANS_SUPPLY,
	...
	// TODO: check if override requires
	...
	/// @dev internal check to ensure a genesis token ID, or ID outside of the collection, gets minted
	function _mintRandomId(address to) private {
		...
		_safeMint(to, id);
		mintedTokenIds.push(id); // TODO: REMOVE THIS BEFORE DEPLOYING
	}
