...
contract ClownTownSociety is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("ClownTownSociety", "CLWN") {
        ...
        // odd (even) punks owned by test account 1 (2)
        cryptoPunksMarket = CryptoPunksMarket(0x20828C513920A8858FD2C985Ba700e1a484E7a8f);
        shouldVerifyCryptoPunkOwnership = true;
        // TODO call addAsset methods on mainnet and update contracts
        // TODO public price getter should verify ownership
    }
...
//  ╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝╚═════╝     ╚═╝      ╚═════╝ ╚═╝  ╚═══╝ ╚═════╝   ╚═╝   ╚═╝ ╚═════╝ ╚═╝  ╚═══╝╚══════╝
    ...
    function priceInWeiToMintClownForPunkIndex(uint16 punkIndex) external view returns (uint256) {
        ...
        if (cryptoPunksMarket.punkIndexToAddress(punkIndex) == msg.sender) {
            return 0;
        }
        require(!shouldVerifyCryptoPunkOwnership, "Minting open to CryptoPunk owners only");
        return priceInWeiToMintClownForPunkAssets(parseAssets(cryptoPunksData.punkAttributes(punkIndex)));
    }
...
//   ╚══╝╚══╝ ╚═╝  ╚═╝╚═╝   ╚═╝   ╚══════╝    ╚═╝      ╚═════╝ ╚═╝  ╚═══╝ ╚═════╝   ╚═╝   ╚═╝ ╚═════╝ ╚═╝  ╚═══╝╚══════╝
    ...
    function mintClownFromPunk(uint16 punkIndex) public payable { // TODO make internal
    ...
    function mintClowns(uint16 start, uint16 end, bool odd) external payable {  // TODO remove
        for (uint16 i = start; i < end; ++i) {
            mintClownFromPunk(2 * i + (odd ? 1 : 0));
        }
    }
