 ...
 */
contract TellerOptions is ERC721Upgradeable, IERC721ReceiverUpgradeable, IERC1155ReceiverUpgradeable {
    ...
    modifier onlyOwner(uint256 optionId) {
        require(msg.sender == ownerOf(optionId), "TellerOptions: not owner");
        _;
    }

    /**
     * @notice Initializes the TellerOption contract as an ERC721 token.
     */
    function initialize() external initializer {
        __ERC721_init("TellerOptions", "TOPT");
    }
    ...
    function contractURI() external view returns (string memory) {
        return "https://ipfs.io/ipfs/QmUxey6RzLrXmrQnMM7JCLhV9LmAVU6goFT2J8QfFsobYr";
    }

    /**
     * @notice The token metadata URI.
     * @param tokenId Token ID to get URI for.
     * @return uri_ the token URI hash
     */
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory uri_) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");

        TokenBundle storage bundle = _bundles[tokenId];
        if (bundle.types[0] == TokenType.ERC721) {
            uri_ = IERC721Metadata(bundle.addresses[0]).tokenURI(bundle.ids[0]);
        } else if (bundle.types[0] == TokenType.ERC1155) {
            uri_ = IERC1155MetadataURI(bundle.addresses[0]).uri(bundle.ids[0]);
        }
    }

    /**
     * @notice Creates a new option for a bundle of tokens and mints an TellerOption NFT.
    ...
    function createOption(
        ...
        require(duration >= ONE_DAY && duration <= ONE_YEAR, "TellerOptions: invalid duration");
        ...
        Option storage opt = options[optionId_];
        ...
        _mint(msg.sender, optionId_);

        emit OptionCreated(optionId_, msg.sender);
    }
     ...
     *  - {msg.sender} must be the option owner
     */
    function cancelOption(uint256 optionId) public onlyOwner(optionId) {
        Option storage opt = options[optionId];

        require(opt.status < OptionStatus.exercised, "TellerOptions: option closed");
    ...
    function expireOption(uint256 optionId) public {
        ...
        require(
            ...
            "TellerOptions: status not filled"
        );
        require(
            block.timestamp >= (opt.startTime + opt.duration),
            "TellerOptions: not expired"
        );
    ...
    function fillOption(uint256 optionId) public payable {
        ...
        require(_exists(optionId), "TellerOptions: non-existent");
        require(
            opt.status == OptionStatus.undefined,
            "TellerOptions: already filled"
        );
        require(
            msg.value == opt.buyoutPriceWei,
            "TellerOptions: incorrect ether value"
        );

        // Start options contract
        opt.optionFiller = msg.sender;
        opt.status = OptionStatus.filled;
        ...
        if (opt.incentiveAmountWei > 0) {
            payable(msg.sender).transfer(opt.incentiveAmountWei);
        }
     ...
     *  - must not be expired
     */
    function exerciseOption(uint256 optionId) public onlyOwner(optionId) {
        Option storage opt = options[optionId];
    ...
    function _transferToken(TokenType tokenType, bytes memory details, bytes memory transferData) internal {
        ...
        if (tokenType == TokenType.ERC721) {
            require(amount == 1, "TellerOptions: 721 amount not 1");
    ...
    function _transferTokenBundleIn(TokenBundle calldata bundle) internal {
        ...
        require(
            ...
            "TellerOptions: bundle lengths mismatch"
        );

        for (uint256 i; i < bundle.types.length; i++) {
            bytes memory data =
                abi.encode(msg.sender, address(this), bundle.addresses[i], bundle.ids[i], bundle.amounts[i]);
    ...
    function _verifyNFTData(bytes calldata data) private view {
        ...
        require(abi.decode(data, (bytes32)) == _encodeTokenTransferData(), "TellerOptions: invalid deposit");
    }
