...
contract EyesTest3 is ERC721Enumerable, Ownable {
  using Strings for uint256;
...
//   mapping(address => uint256) public giveawayMintedBalance;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
        ...
        if(giveawayList(msg.sender) ){
            require(_mintAmount <= nftGiveawayLimit, "max giveaway NFT per address exceeded");
  ...
  function addGiveawayUsers(address[] calldata _users) public onlyOwner {
    for (uint i = 0; i < _users.length; i++) {
  ...
  function removeGiveawayUsers(address[] calldata _users) public onlyOwner {
    for (uint i = 0; i < _users.length; i++) {
      giveawayAddresses[_users[i]] = false;
    }
  }
