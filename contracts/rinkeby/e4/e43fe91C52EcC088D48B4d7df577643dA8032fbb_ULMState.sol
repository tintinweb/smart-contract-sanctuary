...
contract ULMState is IULMState, BlockTimestamp {
    ...
    function calculateLiquidity(
        ...
        IUniswapV3Pool pair = IUniswapV3Pool(_poolAddress);
    ...
    {
        IUniswapV3Pool _pair = IUniswapV3Pool(_poolAddress);
        uint128 liquidity = calculateLiquidity(_poolAddress, _tickLower, _tickUpper);

        (uint160 sqrtPriceX96, , , , , , ) = _pair.slot0();
        ...
        liquidityRatio = (amount1 * 1e18) / amount0;
    }
    ...
    function checkLiquidityAmounts(
        ...
        uint256 amount1,
        bytes calldata data
    ) external override returns (uint256 amount0In, uint256 amount1In) {
        ...
        (int24 baseTickUpper, int24 baseTickLower, , , , ) = IUniStrategy(uniStrategy).getTicks(pool);
        (, , uint256 ratio) = getLiquidityRatios(pool, baseTickUpper, baseTickLower);
        uint256 inputRatio = (amount1 * 1e18) / amount0;
        (amount0In, amount1In) = ratio == inputRatio
            ? (amount0 - 1, amount1)
            : (amount0, amount1);
    }
