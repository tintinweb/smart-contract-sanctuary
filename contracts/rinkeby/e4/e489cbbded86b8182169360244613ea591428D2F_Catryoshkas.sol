...
contract Catryoshkas is ERC721, ERC721Enumerable, Ownables {
    ...
    // cats limits
    uint256 private constant _BIG_CATS = 100;
    uint256 private constant _MINI_CATS = 200;
     
    // uint[200] private _specialCats; 
    ...
    modifier checkMintingSupplyAvailable(uint256 numberOfTokens_) {
        ...
        require(totalSupply().add(numberOfTokens_) <= _MINTED_SUPPLY, "Exceeds max minted supply");
        _;
    ...
    function _bindSpecials(uint256[] memory specials_) internal {

        for (uint i=0; i< specials_.length; i++) {
             
            _specialCatsBreed[specials_[i]] = false;
