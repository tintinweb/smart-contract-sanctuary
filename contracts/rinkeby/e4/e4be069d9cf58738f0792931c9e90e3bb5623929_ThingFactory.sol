...
contract Thing is ERC721Full, ERC721MetadataMintable, ERC721Burnable, Ownable {
  ...
  enum TokenState { Pending, ForSale, Sold, Transferred, Burned }
  ...
  mapping(string => bool) metaIdExists;
  ...
  function batchMint(address to, uint256 amountToMint, string memory metaId, uint256 setPrice, bool isForSale) public onlyMinter {
    ...
    if(!metaIdExists[metaId])
     {  
         metaIds.push(metaId);
         metaIdExists[metaId] = true;
     }
  }

    
 function getMetaIds() public view returns (string[] memory) {
    return metaIds;
  }
  ...
  function batchBurn(uint256[] memory tokenIds) public onlyMinter {
    for (uint256 i = 0; i < tokenIds.length; i++) {
      _burn(tokenIds[i]);
	  items[tokenIds[i]].state = TokenState.Burned;
    }
...
contract ThingFactory {
    ...
    function launchStore(
      ...
      string memory uri
        ...
        emit StoreLaunch(newStore, msg.sender, name, symbol);
        return newStore;
    }
