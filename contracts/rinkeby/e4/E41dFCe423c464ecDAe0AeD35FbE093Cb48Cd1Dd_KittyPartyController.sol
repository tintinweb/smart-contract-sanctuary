...
contract KittyPartyController is KittyPartyStateTransition, OwnableUpgradeable{
    ...
    function changeState() public timedTransitions() onlyOwner returns(bool success){
        return true;
    }
...
enum KittyPartyStages {
    ...
    modifier timedTransitions() {
        ...
        if (stage == KittyPartyStages.Collection && block.timestamp >= creationTime + 100 seconds) {
            nextStage();
        }
        if (stage == KittyPartyStages.Staking && block.timestamp >= creationTime + durationInDays * 100 seconds) {
            nextStage();
        }
        _;
    }
