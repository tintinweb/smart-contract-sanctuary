...
contract DotcMain is owned {
	...
	require(buy_amount.mul(buy_price).div(10**decimal)>=adMaps[sell_ad_id].low_limit && buy_amount.mul(buy_price).div(10**decimal)<=adMaps[sell_ad_id].high_limit, "Limitation is not met.");
	...
	//for test
	uint256 order_id = total_order_num.add(1);
