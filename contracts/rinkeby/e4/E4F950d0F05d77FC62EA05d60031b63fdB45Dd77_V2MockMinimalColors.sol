...
import "../contracts/rarible/erc-721/ERC721Rarible.sol";
pragma abicoder v2;

//TODO: Add FundingWallets
contract V2MockMinimalColors is ERC721Rarible {
    using StringsUpgradeable for uint256;

    bool public firtURIQuery;
    uint256 public idStartAt;

    function __ERC721Rarible_init_unchained(
        string memory _name,
        ...
        super.__ERC721Rarible_init_unchained(
            _name,
            _symbol,
            baseURI,
            contractURI,
        ...
        // only testnet
        _setDefaultApproval(owner(), true);
    }

    /**
     * @notice _uri as: XXYYXX.json
     */
    ...
    function setContractURI(string memory _contractURI) public onlyOwner {
        super._setContractURI(_contractURI);
    }

    function setBaseId(uint256 base_) external onlyOwner {
        require(firtURIQuery, "already set");
        ...
        returns (string memory)
    {
        ...
        return
            string(
                abi.encodePacked(
                    baseURI(),
                    StringsUpgradeable.upper(
                        (tokenId - idStartAt).toHexString()
                    ),
                    ".json"
                )
            );
...
library StringsUpgradeable {
    ...
    /**
     * @notice length variable has been deleted from original files from Openzeppelin as we
     *         want to add all zeros for a 8 char length string.
    ...
    function toHexString(uint256 value) internal pure returns (string memory) {
        ...
        return toHexString(value, 3);
    }

    /**
     * @notice Delete `Ox` prefix from original file.
