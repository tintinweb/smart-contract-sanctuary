...
contract GenerateBadge is ERC721, VRFConsumerBase {
    ...
    function issueNFT(uint256 userProvidedSeed, string memory name, address userAddress) public {
        generateNFT(userProvidedSeed, name, userAddress);
        setTokenURI(badges.length, badgeUrls[name]);
    }

    function generateNFT(uint256 userProvidedSeed, string memory name, address userAddress) public returns (bytes32) {
