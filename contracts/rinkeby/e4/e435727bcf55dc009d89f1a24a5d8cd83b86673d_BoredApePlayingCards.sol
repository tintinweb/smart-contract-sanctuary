...
contract BoredApePlayingCards is ERC721 {
    ...
    mapping(address => uint) public _maxMintedPerWallet;
    

    constructor() ERC721("BoredApePlayingCards", "BAPC") {
        owner = msg.sender;
    }
    ...
    function addToWhitelist(address whitelisted) external onlyOwner {
        _whitelisted[whitelisted] = !_whitelisted[whitelisted];
    }
    ...
    function setMaxMintPerWallet(uint _maxMintsPerWallet) external onlyOwner {
    ...
    function buySmallPack() external payable {
        require(saleStatus, "Sale not active!");
        require(_maxMintedPerWallet[msg.sender] + 5 <= maxMintsPerWallet, "Exceeds mint limit!");
        require(counter + 5 < maxSupply, "Not enough tokens!");
        uint balance = IERC721(token).balanceOf(msg.sender);
        if(balance > 0 || _whitelisted[msg.sender]){
            require(msg.value == (whitelistPrice * 90 / 100) * 5, "Incorrect value!");
            for(uint8 i=0; i<5; i++){
                _safeMint(msg.sender, counter++);
            }
            _maxMintedPerWallet[msg.sender]+= 5;
        }else{ 
            uint price = (salePrice * 90 / 100) * 5;
    ...
    function buyBigPack() external payable {
        require(saleStatus, "Sale not active!");
        require(_maxMintedPerWallet[msg.sender] + 10 <= maxMintsPerWallet, "Exceeds mint limit!");
        require(counter + 10 < maxSupply, "Not enough tokens!");
        uint balance = IERC721(token).balanceOf(msg.sender);
        if(balance > 0 || _whitelisted[msg.sender]){
            uint price = (whitelistPrice * 80 / 100) * 5;
            require(msg.value == price, "Incorrect value!");
