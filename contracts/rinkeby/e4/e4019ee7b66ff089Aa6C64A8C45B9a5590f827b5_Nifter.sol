...
contract Nifter is ERC1155, Ownable, INifter {
    ...
    /**
     * @dev Gets the is Buy Now Enable
     * @param _tokenId uint256 ID of the token
     * @param _owner address of the token owner
     */
    function getIsBuyNowEnable(uint256 _tokenId, address _owner)
    external
    view 
    returns (bool){
        return tokenOwnerInfo[_tokenId][_owner].isBuyNowEnable;
    }
        ...
        for(uint i = _startIndex; i < _endIndex; i++){
            ...
            for(uint j = 0; j < owners.length; j++) {
                ...
                tokenOwnerInfo[tokenId][owner] = TokenOwnerInfo(
                    oldContract.getIsBuyNowEnable(tokenId, owner),
    ...
    function setIsBuyNowEnable(bool _isBuyNowEnable, uint256 _tokenId) external onlyTokenOwner(_tokenId) {
        tokenOwnerInfo[_tokenId][msg.sender].isBuyNowEnable = _isBuyNowEnable;
    }
     ...
     */
    function setMarketAddresses(address _marketAuction, address _marketplaceSettings) external onlyOwner {
     ...
     */
    function addNewToken(uint256 _tokenAmount, bool _isBuyNowEnable, bool _isForSale, uint8 _priceType) public {
        uint256 tokenId = _createToken(msg.sender, _tokenAmount, _isBuyNowEnable, _isForSale, 0, _priceType);
     ...
     * @param _priceType 0 is for fixed, 1 is for Auction Time bound, 2 is for Auction Infiniite
     */
    function addNewTokenWithId(uint256 _tokenAmount, bool _isBuyNowEnable, bool _isForSale, uint256 _tokenId, uint8 _priceType) public {
        uint256 tokenId = _createTokenWithId(msg.sender, _tokenAmount, _isBuyNowEnable, _isForSale, 0, _priceType, _tokenId);
     ...
     */
    function addNewTokenAndSetThePrice(uint256 _tokenAmount, bool _isBuyNowEnable, bool _isForSale, uint256 _price, uint8 _priceType, uint256 _startTime, uint256 _endTime) public {
        uint256 tokenId = getTokenIdAvailable();
        addNewTokenAndSetThePriceWithId(_tokenAmount, _isBuyNowEnable, _isForSale, _price, _priceType, _startTime, _endTime, tokenId);
    }
     ...
     * @param _tokenId uint256 ID of the token.
     */
    function addNewTokenAndSetThePriceWithId(uint256 _tokenAmount, bool _isBuyNowEnable, bool _isForSale, uint256 _price, uint8 _priceType, uint256 _startTime, uint256 _endTime, uint256 _tokenId) public {
        uint256 tokenId = _createTokenWithId(msg.sender, _tokenAmount, _isBuyNowEnable, _isForSale, _price, _priceType, _tokenId);
        if(_priceType == 0) {
            marketAuction.setSalePrice(tokenId, _price, msg.sender);
        } if(_priceType == 1){
            marketAuction.setInitialBidPriceWithRange(_price, _startTime, _endTime, msg.sender, tokenId);
        }
        emit AddNewToken(msg.sender, tokenId);
    }
     ...
     * @param _priceType 0 is for fixed, 1 is for Auction Time bound, 2 is for Auction Infiniite
     */
    function _createToken(address _creator, uint256 _tokenAmount, bool _isBuyNowEnable, bool _isForSale, uint256 _price, uint8 _priceType) internal returns (uint256) {
        uint256 newId = getTokenIdAvailable();
        return _createTokenWithId(_creator, _tokenAmount, _isBuyNowEnable, _isForSale, _price, _priceType, newId);
    }
     ...
     * @param _tokenId uint256 token id
     */
    function _createTokenWithId(address _creator, uint256 _tokenAmount, bool _isBuyNowEnable, bool _isForSale, uint256 _price, uint8 _priceType, uint256 _tokenId) internal returns (uint256) {
