...
contract SmolBrain is MinterControl, ERC721Enumerable {
    ...
    function mintFemale(address _to) external onlyMinter {
        emit SmolBrainMint(_to, _femaleTokenIdTracker.current());

        _safeMint(_to, _femaleTokenIdTracker.current());
        _femaleTokenIdTracker.increment();
    }

    function mintMale(address _to) external onlyMinter {
        // Offset token Ids by the number of females
        uint256 _tokenId = _maleTokenIdTracker.current() + 6711;
