...
contract Aftrmrkt is ERC1155, Ownable, VRFConsumerBase {
    ...
    // pack limit
    uint256 public PremiumPack = 5;
    uint256 public FreshPack = 5;
    uint256 public BasePack = 5;
    ...
    string public baseURI = "ipfs://QmdnRRLGUAJDzgcKHTd4HPFT8gH9wPzR6gRxCSEQgBKy6q";
    ...
    function isWhitelisted(address _address) public view returns (bool) {
        for (uint256 i = 0; i < whitelistAddresses.length; i++) {
            if (_address == whitelistAddresses[i]) return true;
        }
        return false;
    }
    ...
    function buyPresalePremiumPack() external payable {
        ...
        require(
            ...
            "Aftmrkt::buyPresalePremiumPack: Only for whitelisted users."
        );
        ...
        require(
            msg.value >= PremiumPackFee || _msgSender() == owner(),
    ...
    function buyPresaleFreshPack() external payable {
        ...
        require(
            isWhitelisted(_msgSender()),
            "Aftmrkt::buyPresaleFreshPack: Only for whitelisted users."
        );
        require(preSale, "Aftmrkt::preCheck: presale is not active");
        require(
            FreshPack > 0,
        ...
        require(
            msg.value >= FreshPackFee || _msgSender() == owner(),
            "Aftmrkt::buyPremiumPack: Insufficient amount."
        );
        bytes32 requestId = requestRandomness(keyHash, fee);
    ...
    function buyPresaleBasePack() external payable {
        ...
        require(
            isWhitelisted(_msgSender()),
            "Aftmrkt::buyPresaleBasePack: Only for whitelisted users."
        );
        require(preSale, "Aftmrkt::preCheck: presale is not active");
        require(
            BasePack > 0,
        ...
        require(
            msg.value >= BasePackFee || _msgSender() == owner(),
            "Aftmrkt::buyPremiumPack: Insufficient amount."
        );
        bytes32 requestId = requestRandomness(keyHash, fee);
