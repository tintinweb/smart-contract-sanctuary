...
contract StonersRock is ERC721Burnable, Ownable {
    ...
    address public constant community = 0xB58Fb5372e9Aa0C86c3B281179c05Af3bB7a181b;     // stonersrock.eth
    ...
    mapping (bytes32=> uint256) private requestByToken;
    event RockMinted(address minter, uint256 tokenId, string tokenUri);

    // Constructor
    constructor(string memory _baseUri) ERC721("Stoners", "ROCK") {
        baseUri = _baseUri;
        _tokenIds._value = maxReserveSupply;
    }

    /*
    *   Check if the sale has started yet.
    */
    ...
    */
    ...
    /*
    *   Mint reserved rock and transfer to another address.
    */
    function giveAwayRock(address receiver) public onlyOwner {
        uint256 reserved = _reserveIds.current();
        require(reserved.add(1) <= maxReserveSupply, "Exceeding max supply!");
        _reserveIds.increment();
        uint256 tokenId = _reserveIds.current();
    ...
    *   Owner setters.
    */
    function setRoyaltyContract(address payable _splitContract) public onlyOwner {
    ...
    *   Money management.
    */
    function withdrawCommunity(uint256 amount) public onlyOwner {
        require(payable(community).send(amount));
    }
    ...
    function withdrawRoyalties() public onlyOwner {
        StonersSplit(splitContract).release();
    }
