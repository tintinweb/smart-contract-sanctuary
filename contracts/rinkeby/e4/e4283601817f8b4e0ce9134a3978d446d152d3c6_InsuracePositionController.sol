...
contract InsuracePositionController is AbstractController {
    ...
    */
     ...
     */
    //FIXME Kiril: how do we plan to use it? Is this must be called first and then withdrawRewards() ?
    // How can we track time?
    ...
    function apy() external view override returns (uint256 apy) {
        //TODO
        // is if same what _caclulatePoolAPY does?
        // note: return value is expected as % with with 10**5 precision
        apy = _caclulatePoolAPY(_stakingAsset).div(PERCENTAGE_100);
    }
     ...
     */
    function _caclulateRewardsPerBlock(address _tokenAddress)
        internal
        view
        returns (uint256 _rewards)
    {
        ...
        return poolWeight.div(totalPoolWeight).mul(rewardsPB);
    }
     ...
     */
    function _caclulatePoolAPY(address stakingtokenAddress) internal view returns (uint256) {
        // N = The number of blocks mined in a year.
        uint256 averageBlockTime = 15; // 15 Avrg ETH network - TODO: get it from realtime oracle
        uint256 blocksMined = SECONDS_IN_THE_YEAR.div(averageBlockTime);

        // R = RWRD per Block
        uint256 data = _caclulateRewardsPerBlock(stakingtokenAddress);
        uint256 rewards = abi.decode(abi.encodePacked(data), (uint256));

        // P _stakingAsset = Price of 1 token _stakingAsset to base investment token
        uint256 stakingAssetPrice = _priceFeed.howManyTokensAinB(
            address(base),
            address(_stakingAsset),
            1 ether
        );

        // P = Price of 1 staking token to base from UniswapV2
        uint256 stakingTokenPrice = _priceFeed.howManyTokensAinB(address(base), _stakingAsset, 1);

        // V = Pool TVL
        uint256 poolTVL = _stakersPool.stakedAmountPT(_stakingAsset);

        uint256 apyDividend = blocksMined.mul(rewards).mul(stakingTokenPrice);
        uint256 apyDivisor = poolTVL.mul(stakingTokenPrice);

        return apyDividend.div(apyDivisor);
    }
