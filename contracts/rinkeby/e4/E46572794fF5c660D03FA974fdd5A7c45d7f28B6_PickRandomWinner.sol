...
contract PickRandomWinner is VRFConsumerBase, Ownable {
    ...
    constructor(bytes32 keyHash, uint256 fee, address coordinator, address linkToken) public VRFConsumerBase(coordinator, linkToken) {
		privKeyHash = keyHash; 
		privFee = fee; 
	}    
    ...
    function requestWinner(string memory urlHash, uint256 totalSigners) public {
		require(LINK.balanceOf(address(this)) >= privFee, "Not enough Link to request random value!"); 
        require(users[urlHash] == 0, "Winner for document already chosen!");
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        // Fire an event here and listen to this event 
    ...
    function getDocumentWinner(string memory urlHash) public view returns (Document memory) {
        require(users[urlHash] != 0, "Winner request has not been made!");
        require(requestStatus[urlHash] == PICK_COMPLETED, "Winner request is in progress!");
