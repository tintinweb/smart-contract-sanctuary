...
contract fiveMinutes is Context, Ownable, IERC20, IERC20Metadata, Additions {
    ...
    uint256 buyTime = launchTime + 3300;
    uint256 sellTime = launchTime;
    uint32 secsInHour = 3600;
    ...
    function _transfer(
        ...
        if((_currentTime <= buyTime && _currentTime > sellTime) && (recipient != address(0) || sender != address(0))){
            require(sender == _router, "5Minutes: it's pump time! Buy more.");
        }
        
        if(_currentTime > buyTime){
            buyTime += secsInHour;
            sellTime += secsInHour;
        }
