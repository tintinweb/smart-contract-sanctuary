...
contract BattleContract is Ownable {
    ...
    struct Winner {
        ...
        bool isPaid; // Did winner claim GFX
        bool isClaimed; // Did winner claim NFT
    }
    ...
    modifier _isWinner(uint256 _battleId) {
        ...
        require(winner.nftOwner == msg.sender, "You are not winner");
        _;
    }

    modifier _isClaimedGFX(uint256 _battleId) {
        Winner memory winner = winners[_battleId];
        require(!winner.isPaid, "Already paid award");
        _;
    }
        ...
        _canParticipate(_battleId, _nftId)
    {
        ...
        nft_.participateBattle(_battleId, participant.nftOwner);
    ...
    function claimNFT(uint256 _battleId) external _isWinner(_battleId) {
        Winner memory winner = winners[_battleId];
        require(!winner.isClaimed, "Already claimed NFT");
        ...
        )
    {
        ...
        return (battle.name, battle.uri, battle.award, battle.level);
    }
...
interface INFTContract {
    ...
    function participateBattle(uint256 _battleId, address _owner) external;
}
