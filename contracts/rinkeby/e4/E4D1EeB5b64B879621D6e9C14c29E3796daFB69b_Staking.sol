...
contract Staking is Deposit,ReentrancyGuard{
    ...
    function withdraw (address _withdrawer,uint _amount,address _coin,uint timestamp) public onlyOwner{
        _safeExtraction(_withdrawer,_coin,_amount);
        emit AddWithdraw(_withdrawer,_amount,_coin,timestamp);
        totalWithdrawCount[_coin] = totalWithdrawCount[_coin].add(_amount);
    }

    // 计算收益
    function computStakingProfit(uint _id,uint timestamp)  public view returns (uint){
        uint profit = 0;
        (uint passMonths,uint curDate) = passMonthCount(_id,timestamp);
        uint num;
        uint monthSecord;
    ...
    function passMonthCount(uint _id,uint timestamp) public view returns (uint passMonths,uint curDate) {
        // 判断当前日期是否大于结束日期 如果大于结束日期按结束日期计算
        if(getYmd(timestamp) >= signDeposit[_id].endTime){
            curDate = signDeposit[_id].endTime;
        }else{
            // curDate = getYmd(block.timestamp);
            curDate = getYmd(timestamp);
    ...
    function extractionProfit(uint _cycleId,uint _depositId,uint timestamp) external nonReentrant existCycle(_cycleId){
        // require(!signDeposit[_depositId].extractStatu);
        uint curTime = getYmd(timestamp);
        uint profit = computStakingProfit(_depositId,timestamp);
        
        // 发放收益
        _safeExtraction(msg.sender,signCycle[_cycleId].outAddress,profit);
        emit AddExtract(msg.sender,profit,timestamp,_cycleId,signCycle[_cycleId].outAddress);
    ...
    function extractionCapital(uint _cycleId,uint _depositId,uint timestamp) external nonReentrant checkUser(_depositId) existCycle(_cycleId){
        require(!signDeposit[_depositId].extractStatu,'Extracted');
        uint _amount;
        if(getYmd(timestamp) >= signDeposit[_depositId].endTime){
        ...
        emit AddExtract(msg.sender,_amount,timestamp,_cycleId,signCycle[_cycleId].inAddress);
        ...
        signDeposit[_depositId].endTime = getYmd(timestamp);
    }
