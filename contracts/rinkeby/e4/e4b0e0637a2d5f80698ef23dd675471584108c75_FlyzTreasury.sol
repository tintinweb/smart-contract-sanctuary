...
contract FlyzTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SFLZ }
    ...
    address public sFLZ;
    uint public sFLZQueue; // Delays change to sFLZ address
    ...
    uint256 _blocksNeededForQueue
        ...
        // isLiquidityToken[ _FLZDAI ] = true;
        // liquidityTokens.push( _FLZDAI );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for FLZ
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint FLZ needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( FLZ ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn FLZ for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IFLZERC20( FLZ ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sFLZ ).balanceOf( msg.sender ); // Can only borrow against sFLZ held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with FLZ
        @param _amount uint
     */
    function repayDebtWithFLZ( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IFLZERC20( FLZ ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, FLZ, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( FLZ ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( FLZ ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns FLZ valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match FLZ decimals
            // we want 1 FLZ = 0.001 reserve token
            value_ = 1000 * _amount.mul( 10 ** IERC20( FLZ ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SFLZ ) { // 9
            sFLZQueue = block.number.add( blocksNeededForQueue );
        } else return false;
