...
contract RaffleWorld is Ownable, VRFConsumerBase {
    ...
    event SetRaffleWinnerStructure(
        address indexed user,
        uint256 indexed _raffleId,
        uint256 _index,
        uint256 _percentage
    );
    event AddRaffleWinner(
        uint256 indexed _raffleId,
    ...
    modifier beforeRaffleStart(uint256 _raffleId) {
        require(
            ...
            "RaffleWorld: you cannot update raffle params after it has started!"
        );
        _;
    }

    modifier beforeWithdraw(uint256 _raffleId) {
        if (
            keccak256(bytes(raffles[_raffleId].status)) ==
            keccak256(bytes("ended"))
        ) {} else if (raffles[_raffleId].canceled == true) {}
        _;
    }
    ...
    modifier checkTicketsAcquisition(uint256 _raffleId, uint256 _ticketsNo) {
        ...
        require(
            raffleEntries[_raffleId].length.add(_ticketsNo) <=
                raffles[_raffleId].ticketNumber,
    ...
    mapping(address => mapping(uint256 => uint256[])) raffleEntriesPositionById;
    ...
    */
    function addRaffle(
        ...
        require(
            ...
            "RaffleWorld: RaffleType or RaffleStartDate incorrect"
        );

        //check if the contract has enough link tokens to add the raffle
        uint256 requiredLinkAmount = activeRaffles.mul(requiredRaffleLink);
        require(
            requiredLinkAmount <= linkToken.balanceOf(address(this)),
            "RaffleWorld: There is not enough LINK Tokens to add a new raffle"
        );
        
        //check if the owner has allowed enough raffle tokens for the prize pool
        IERC20 raffleToken = IERC20(_tokenContract);
        require(
        ...
        //add the raffle to the coresponding array and set the index 
         if (
            ...
            rafflesIndices[raffles.length - 1].raffles_by_type_index = bronze_raffles.length - 1;
        } else if (
            keccak256(bytes(raffles[raffles.length - 1].raffleType)) ==
            keccak256(bytes("silver"))
        ) {
            silver_raffles.push(raffles.length - 1);
            rafflesIndices[raffles.length - 1].raffles_by_type_index = silver_raffles.length - 1; 
        } else if (
            keccak256(bytes(raffles[raffles.length - 1].raffleType)) ==
            keccak256(bytes("gold"))
        ) {
            gold_raffles.push(raffles.length - 1);
            rafflesIndices[raffles.length - 1].raffles_by_type_index = gold_raffles.length - 1;
        } else {
            diamond_raffles.push(raffles.length - 1);
            rafflesIndices[raffles.length - 1].raffles_by_type_index = diamond_raffles.length - 1;
        }
    ...
    */
    function _activateRaffle(uint256 _raffleId) internal {
        ...
        rafflesIndices[_raffleId].active_raffles_index = active_raffles.length - 1;
        ...
        if (
            ...
            rafflesIndices[_raffleId].active_raffles_by_type_index = active_bronze_raffles.length - 1;
        } else if (
            keccak256(bytes(raffles[_raffleId].raffleType)) ==
            keccak256(bytes("silver"))
        ) {
            active_silver_raffles.push(_raffleId);
            rafflesIndices[_raffleId].active_raffles_by_type_index = active_silver_raffles.length - 1;
        } else if (
            keccak256(bytes(raffles[_raffleId].raffleType)) ==
            keccak256(bytes("gold"))
        ) {
            active_gold_raffles.push(_raffleId);
            rafflesIndices[_raffleId].active_raffles_by_type_index = active_gold_raffles.length - 1;
        } else {
            active_diamond_raffles.push(_raffleId);
            rafflesIndices[_raffleId].active_raffles_by_type_index = active_diamond_raffles.length - 1;
        }
    ...
    */
    function _cancelRaffle(uint256 _raffleId) internal {
        ...
        active_raffles[rafflesIndices[_raffleId].active_raffles_index] = active_raffles[rafflesIndices[active_raffles[active_raffles.length - 1]].active_raffles_index];
        rafflesIndices[active_raffles[active_raffles.length - 1]].active_raffles_index = rafflesIndices[_raffleId].active_raffles_index;
        ...
        if (
            ...
            active_bronze_raffles[rafflesIndices[_raffleId].active_raffles_by_type_index] = active_bronze_raffles[rafflesIndices[active_bronze_raffles.length - 1].active_raffles_by_type_index];
            rafflesIndices[active_bronze_raffles.length - 1].active_raffles_by_type_index = rafflesIndices[_raffleId].active_raffles_by_type_index; 
            active_bronze_raffles.pop();
        } else if (
            keccak256(bytes(raffles[_raffleId].raffleType)) ==
            keccak256(bytes("silver"))
        ) {
            active_silver_raffles[rafflesIndices[_raffleId].active_raffles_by_type_index] = active_silver_raffles[rafflesIndices[active_silver_raffles.length - 1].active_raffles_by_type_index];
            rafflesIndices[active_silver_raffles.length - 1].active_raffles_by_type_index = rafflesIndices[_raffleId].active_raffles_by_type_index; 
            active_silver_raffles.pop();
        } else if (
            keccak256(bytes(raffles[_raffleId].raffleType)) ==
            keccak256(bytes("gold"))
        ) {
            active_gold_raffles[rafflesIndices[_raffleId].active_raffles_by_type_index] = active_gold_raffles[rafflesIndices[active_gold_raffles.length - 1].active_raffles_by_type_index];
            rafflesIndices[active_gold_raffles.length - 1].active_raffles_by_type_index = rafflesIndices[_raffleId].active_raffles_by_type_index; 
            active_gold_raffles.pop();
        } else {
            active_diamond_raffles[rafflesIndices[_raffleId].active_raffles_by_type_index] = active_diamond_raffles[rafflesIndices[active_diamond_raffles.length - 1].active_raffles_by_type_index];
            rafflesIndices[active_diamond_raffles.length - 1].active_raffles_by_type_index = rafflesIndices[_raffleId].active_raffles_by_type_index; 
    ...
    function setRaffleDiscountLevels(
        ...
        for (uint256 i = 0; i < discountLevels[_raffleId].length; i++) {
            if (discountLevels[_raffleId][i].ticketNumber == _ticketsNo) {
    ...
    function _addRaffleWinner(
        ...
        emit AddRaffleWinner(_raffleId, winnerStructure[_raffleId][_winIndex].winnerAddress, winnerStructure[_raffleId][_winIndex].percentage);
    }

    function removeRaffleWinner(uint256 _raffleId, uint256 _winIndex)
        public
        onlyOwner
    {
        winnerStructure[_raffleId][_winIndex] = winnerStructure[_raffleId][
    ...
    function _decideRaffle(bytes32 _requestId) internal {
        ...
        console.log(_raffleId, random, entries);
        if (entries != 0) {
            for (uint256 i = 0; i < winnerStructure[_raffleId].length; i++) {
    ...
    function subscribeToRaffle(
        ...
        uint256 _ticketsNo,
        address _refferedBy
    ) external checkTicketsAcquisition(_raffleId, _ticketsNo) {
        uint256 ticketsValue = _getTicketsValue(_raffleId, _ticketsNo);
        IERC20 raffleToken = IERC20(raffles[_raffleId].tokenContract);

        require(
            raffleToken.allowance(_msgSender(), address(this)) >= ticketsValue,
            "RaffleWorld: you don't have necessary founds!"
        );
        ...
        if (
            raffleEntries[_raffleId].length == raffles[_raffleId].ticketNumber
        ) {
    ...
    function giveTickets(
        ...
        emit SubscribeToRaffle(_to, _raffleId, _ticketsNo, _msgSender());
    ...
    function withDrawSubscription(uint256 _raffleId)
        public
        beforeWithdraw(_raffleId)
    {
        uint256 ticketsPrice = raffleEntriesPositionById[_msgSender()][_raffleId].length;
        ticketsPrice = ticketsPrice.mul(raffles[_raffleId].ticketPrice);
        IERC20 raffleToken = IERC20(raffles[_raffleId].tokenContract);
        raffleToken.transfer(_msgSender(), ticketsPrice);
        
        while (raffleEntriesPositionById[_msgSender()][_raffleId].length != 0) {
            raffleEntries[_raffleId][
                raffleEntriesPositionById[_msgSender()][_raffleId][0]
            ] = raffleEntries[_raffleId][raffleEntries[_raffleId].length - 1];
    ...
    {
        IERC20 raffleToken = IERC20(raffles[_raffleId].tokenContract);
        uint256 ownerFunds = raffles[_raffleId].ticketPrice.mul(raffles[_raffleId].ticketNumber).sub(raffles[_raffleId].prizeAmount);
        require(
            keccak256(bytes(raffles[_raffleId].status)) ==
                        keccak256(bytes("ended")),
