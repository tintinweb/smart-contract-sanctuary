...
contract Ernests is ERC721, Ownable, IMintedBeforeReveal {

    // This is the original provenance record of all Ernests in existence at the time.
    ...
    uint256 public constant SALE_START_TIMESTAMP = 1616611245;

    // Time after which the Ernests are randomized and revealed (10 days from initial launch).
    uint256 public constant REVEAL_TIMESTAMP = SALE_START_TIMESTAMP + (86400 * 10);

    // Maximum amount of Ernests in existance. Ever.
    ...
    // Mapping from token ID to whether the Ernest was minted before reveal.
    ...
    /**
    * @dev Returns if the Ernest was minted before reveal phase. This could come in handy later.
    */
    ...
    /**
    * @dev Gets current Ernest price based on current supply.
    */
    function getErnestMaxAmount() public view returns (uint256) {
        require(block.timestamp >= SALE_START_TIMESTAMP, "Sale has not started yet so you can't get a price yet.");
        require(totalSupply() < MAX_PIXL_SUPPLY, "Sale has already ended, no more Ernests left to sell.");
    ...
    * @dev Gets current Ernest price based on current supply.
    */
    function getErnestPrice() public view returns (uint256) {
        require(block.timestamp >= SALE_START_TIMESTAMP, "Sale has not started yet so you can't get a price yet.");
        require(totalSupply() < MAX_PIXL_SUPPLY, "Sale has already ended, no more Ernests left to sell.");
    ...
    /**
    * @dev Mints yourself a Ernest. Or more. You do you.
    */
    function mintAErnest(uint256 numberOfErnests) public payable {
        // Some exceptions that need to be handled.
        require(totalSupply() < MAX_PIXL_SUPPLY, "Sale has already ended.");
        require(numberOfErnests > 0, "You cannot mint 0 Ernests.");
        require(numberOfErnests <= getErnestMaxAmount(), "You are not allowed to buy this many Ernests at once in this price tier.");
        require(SafeMath.add(totalSupply(), numberOfErnests) <= MAX_PIXL_SUPPLY, "Exceeds maximum Ernest supply. Please try to mint less Ernests.");
        require(SafeMath.mul(getErnestPrice(), numberOfErnests) == msg.value, "Amount of Ether sent is not correct.");

        // Mint the amount of provided Ernests.
        for (uint i = 0; i < numberOfErnests; i++) {
