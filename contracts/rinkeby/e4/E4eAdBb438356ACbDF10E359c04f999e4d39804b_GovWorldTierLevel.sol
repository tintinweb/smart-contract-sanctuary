    ...
    */
        ...
        returns (NFTTierData memory nftTierData)
    {
        ...
        if(nftTierLevels[nftTierLevelsKeys[0]].isTraditional) {
                maxLTVFromNFTTier = tierLevels[nftTierLevels[nftTierLevelsKeys[0]].traditionalTier].loantoValue;
        } else {
                maxLTVFromNFTTier = spTierLevels[nftTierLevels[nftTierLevelsKeys[0]].nftTier].ltv;
        }
        ...
        for (uint256 i = 1; i < nftTierLevelsKeys.length; i++) {
            ...
            if (IERC721(nftTierLevelsKeys[i]).balanceOf(_wallet) > 0) {
                ...
                if(nftTierLevels[nftTierLevelsKeys[i]].isTraditional) {
                   currentLoanToValue = tierLevels[nftTierLevels[nftTierLevelsKeys[i]].traditionalTier].loantoValue;
                } else {
                    currentLoanToValue = spTierLevels[nftTierLevels[nftTierLevelsKeys[i]].nftTier].ltv;
                }
