...
contract Card is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        parts[1] = getCommonMonster(tokenId, 1);

        parts[2] = '</text><text x="10" y="40" class="base">';

        parts[3] = getCommonMonster(tokenId, 3);

        parts[4] = '</text><text x="10" y="60" class="base">';

        parts[5] = getCommonMonster(tokenId, 5);

        parts[6] = '</text><text x="10" y="80" class="base">';

        parts[7] = getCommonMonster(tokenId, 7);

        parts[8] = '</text><text x="10" y="100" class="base">';

        parts[9] = getCommonMonster(tokenId, 9);

        parts[10] = '</text><text x="10" y="120" class="base">';

        parts[11] = getCommonMonster(tokenId, 11);

        parts[12] = '</text><text x="10" y="140" class="base">';

        parts[13] = getCommonMonster(tokenId, 13);

        parts[14] = '</text><text x="10" y="160" class="base">';

        parts[15] = getCommonMonster(tokenId, 15);
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Deck #',
                        toString(tokenId),
                        '", "description": "Cards is a collectable game based on crypto primatives which can be unbundled", "image": "data:image/svg+xml;base64,',
