...
contract NameTag is ERC721Enumerable, Ownable, StringUpper, DenyList {
    ...
    function _getTokenAmount(uint256 _weiAmount) internal returns (uint256) {
        if (currentWaveIndex() < waves.length - 1) {
            ...
            uint256 toNextLimitAmount = currentLimit() - totalSupply();
    ...
    function _setName(uint256 tokenId, string memory _name) internal returns (bool) {
        require(msg.sender != address(0));
        ...
        return _changeTokenName(tokenId, _name);
    }
    ...
    NameTag private _token;

    function setTokenAddress(NameTag token_) external onlyOwner {
    ...
    function migrate(uint16 from, uint16 to) external onlyOwner returns (bool) {
        string memory name;
        bytes memory b;
        for (uint16 i = from; i <= to; i += 1) {
            _safeMint(_token.ownerOf(i), i);
            name = _token.getTokenName(i);
            b = bytes(name);
            if (b.length > 0) {
                _setName(i, name);
            }
        ...
        while(totalSupply >= currentLimit()) {
            _currentWaveIndex += 1;
        }
