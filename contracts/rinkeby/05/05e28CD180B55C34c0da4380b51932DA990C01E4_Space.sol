...
contract Space {
    ...
    function test_arctan(uint256 val) public pure returns (uint256) {
        return Arctan.arctan(val);
    }

    function test_cos(uint16 val) public pure returns (int) {
        return Trigonometry.cos(val);
    }

    function test_sin(uint16 val) public pure returns (int) {
