...
contract Rocks721 is Context, ERC721, Ownable {
    ...
    //Input array of addresses to whitelist - using gift mint instead
    function setWhitelistAddress(address[] memory users) public onlyOwner {  //, uint256[][] memory givenGiftTokens
    ...
    function mintGifts(address[] memory recipients, uint256 [][] memory tokenIds) public onlyOwner {
        for(uint256 i = 0; i < recipients.length; i ++){  
            //add recipients to whitelist
            whitelisted[recipients[i]] = 100;
            //mint all tokens to recipients
            for(uint256 r = 0; r < tokenIds[i].length; r ++){
                _safeMint( recipients[i], tokenIds[i][r] );
            }
        }
    }
