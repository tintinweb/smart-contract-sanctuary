...
import "./token/Dream.sol";

contract Vesting is Ownable {
    using SafeERC20 for Dream;
    ...
    function addInvestorBatch(
        ...
        VestingType[] memory _vTypes
    ) external onlyOwner {
        require(
            _tokensAllotments.length == _investors.length,
            "array lengths should be the same"
        );
        require(
            _initialUnlockAmounts.length == _tokensAllotments.length,
            "array lengths should be the same"
        );
        require(
            _recurrences.length == _initialUnlockAmounts.length,
            "array lengths should be the same"
        );
        require(
            _afterCliffUnlockAmounts.length == _recurrences.length,
            "array lengths should be the same"
        );
        require(
            _cliffDays.length == _afterCliffUnlockAmounts.length,
            "array lengths should be the same"
        );
        require(
            _vTypes.length == _cliffDays.length,
            "array lengths should be the same"
        );
    ...
    function withdrawTokens() external onlyInvestor initialized {
        ...
        // token.safeTransfer(msg.sender, tokensAvailable);
    ...
    function recoverToken(address _token, uint256 amount) external onlyOwner {
        Dream(_token).safeTransfer(msg.sender, amount);
