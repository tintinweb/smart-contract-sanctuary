 ...
 * @title TheOtherNFT
 */
contract TheOtherNFT is ERC721Enumerable, Ownable {
    using SafeMath for uint256;

    string public baseTokenURI = "";
    string public notRevealedURI = "";
    string public baseExtension = ".json";
    uint256 public maxSupply = 13; // 2^8
    uint256 public OtherCost = 0.0496 ether; // 2^8(1/2^0+1/2^1+1/2^2+1/2^3+1/2^4)/10^(2^2)
    uint256 public OtherSpecialCost = 0.0384 ether; // 2^8(1/2^0+1/2^1)/10^(2^2)
    uint256 public totalMint = 0;
    uint8 public maxPerWallet = 4; // 2^8(1/2^0+1/2^1+1/2^4)/10^2
    bool public revealed = false;
    mapping(address => uint8) public minted;
    address public copycatAddress = 0xA72E1b90244153fcab56E1b1dF4ECE94d37c4219;
    ...
    /**
     * @dev Mint _num of Others for COPYCAT holders (4 max per wallet)
     * @param _numOfOthers Quantity to mint
     */
    function mintForCOPYCAT(uint8 _numOfOthers) public payable {
        require(
            totalMint + uint256(_numOfOthers) <= maxSupply,
            "Sorry, all Others have been sold."
        );
        require(
            CopycatContract.balanceOf(msg.sender) >= 1, "You need to have at least 1 COPYCAT!"
        );
        require(
            _numOfOthers + minted[msg.sender] <= maxPerWallet,
            "Max 4 per wallet!"
        );
        require(
            msg.value >= uint256(_numOfOthers) * OtherSpecialCost,
            "Not enough ETH..."
        );
        require(
        ...
        for (uint8 i = 0; i < _numOfOthers; i++) {
            _safeMint(msg.sender, ++totalMint);
        }
        minted[msg.sender] += _numOfOthers;
    }


    /**
     * @dev Mint _num of Others (4 max per wallet)
     * @param _numOfOthers Quantity to mint
     */
    function mint(uint8 _numOfOthers) public payable {
        require(
            totalMint + uint256(_numOfOthers) <= maxSupply,
            "Sorry, all Others have been sold."
        );
        if (msg.sender != owner()) {
            ...
            require(
                msg.value >= uint256(_numOfOthers) * OtherCost,
                "Not enough ETH..."
            );
        }
        require(
    ...
    /**
     * @dev get the ids of the Other owned by _owner
    ...
    function tokenURI(uint256 _tokenId)
        ...
        returns (string memory)
    {
        require(_exists(_tokenId), "This Other doesn't exist!");
        if (revealed == false) {
