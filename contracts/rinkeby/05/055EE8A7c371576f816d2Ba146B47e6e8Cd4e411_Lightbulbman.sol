...
contract Lightbulbman is ERC721Enumerable, Ownable, ILightbulbman {
    ...
    bool public whitelistBuyOnce = true;
    ...
    function setWhitelistBuyOnce(bool _whitelistBuyOnce) external onlyOwner  {
    ...
    function giveAway(address _luckyWalletAddress, uint256 _amount) external onlyOwner  {
        require(totalSupply() < supplyLimit, "Not enough tokens left.");
               for (uint i = 0; i < _amount; i++) {
                    uint mintIndex = totalSupply();
                    _safeMint(_luckyWalletAddress, mintIndex);
        }
    }
    ...
    function tokenURI(uint256 _tokenId) public view override returns(string memory) {
        ...
        uint tokenIndex = (_tokenId + startingIndex)%supplyLimit+1;
