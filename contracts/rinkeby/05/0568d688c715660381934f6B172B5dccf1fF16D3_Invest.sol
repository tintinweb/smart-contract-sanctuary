...
contract Invest is PoolsData {
    ...
    mapping(uint256 => Investor) public yourAllocation;
        ...
        validateSender()
    {
        ...
        if(yourAllocation[_PoolId].InvestorAddress == msg.sender){
            yourAllocation[_PoolId] = Investor(_PoolId, msg.sender, SafeMath.add(yourAllocation[_PoolId].MainCoin,msg.value),
            block.timestamp, SafeMath.add(yourAllocation[_PoolId].TotalTokens,Tokens));
        }else{
            yourAllocation[_PoolId] = Investor(_PoolId, msg.sender, msg.value, block.timestamp, Tokens);
        }
        ...
        validateSender()
    {
        ...
        if(yourAllocation[_PoolId].InvestorAddress == msg.sender){
            yourAllocation[_PoolId] = Investor(_PoolId, msg.sender, SafeMath.add(yourAllocation[_PoolId].MainCoin,_Amount),
            block.timestamp, SafeMath.add(yourAllocation[_PoolId].TotalTokens,Tokens));
        }else{
            yourAllocation[_PoolId] = Investor(_PoolId, msg.sender, _Amount, block.timestamp, Tokens);
        }
