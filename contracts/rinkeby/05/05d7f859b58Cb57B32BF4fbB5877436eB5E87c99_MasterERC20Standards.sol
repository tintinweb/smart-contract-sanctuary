...
contract ERC20Standards is ERC20 {
  ...
  modifier checkMintable() {
    for (uint index = 0; index < types.length; index++) {
      if (keccak256(abi.encodePacked(types[index])) == keccak256(abi.encodePacked("Mintable"))) {
        _;
        break;
      }
    }
  ...
  modifier checkBurnable() {
    for (uint index = 0; index < types.length; index++) {
      if (keccak256(abi.encodePacked(types[index])) == keccak256(abi.encodePacked("Burnable"))) {
        _;
        break;
      }
    }
  ...
  modifier checkGovernanace() {
    for (uint index = 0; index < types.length; index++) {
      if (keccak256(abi.encodePacked(types[index])) == keccak256(abi.encodePacked("Governance"))) {
        _;
        break;
      }
    }
  ...
  function mint(address to, uint256 amount) public payable virtual checkMintable {
    // require(
  ...
  function burnFrom(address account, uint256 amount) public virtual checkBurnable {
    // require(
    //   keccak256(abi.encodePacked(types)) ==
    //     keccak256(abi.encodePacked("Mintable"))
    // );
