...
contract CityParks is ERC721Enumerable, Ownable {
    ...
    function getMetadata(uint256 tokenId) public view returns (string memory) {
        ...
        CityParkUtils.Art memory artInfo = cityParks[tokenId];
        return string(abi.encodePacked(
            CityParkUtils._wrapTrait("Trees", artInfo.numTrees.toString()),
            ',',CityParkUtils._wrapTrait("UFO", CityParkUtils._boolToString(artInfo.hasUFO)),
            ',',CityParkUtils._wrapTrait("Sun", CityParkUtils._boolToString(artInfo.hasSun)),
            ',',CityParkUtils._wrapTrait("Fence", CityParkUtils._boolToString(artInfo.hasFence)),
            ',',CityParkUtils._wrapTrait("Bricks", CityParkUtils._boolToString(artInfo.hasBricks))
        ));
    }
    ...
    function _generateImage(uint256 mintNum) private view returns (string memory) {
        ...
        string memory bricks = CityParkArt._generateAllBricks(artData);
        string memory fences = CityParkArt._generateAllFences(artData);
        string memory rug = CityParkArt._generateRug(artData);

        return string(abi.encodePacked(CityParkUtils._generateHeader(artData.randomDifficulty,artData), CityParkUtils._borderRect, rug, trees, artData.hasUFO ? ufo : sun, artData.hasFence ? fences : bricks, CityParkUtils._imageFooter));
    }
...
library CityParkArt {
    ...
    function _generateAllBricks(CityParkUtils.Art memory artData) public pure returns (string memory) {
        ...
        for (uint i = 0; i < numBrickStructures; i++) {
            string memory localBricks = _generateBricks(artData, i);
    ...
    function _generateBricks(CityParkUtils.Art memory artData, uint rand) public pure returns (string memory) {
    ...
    function _generateFence(CityParkUtils.Art memory artData, uint rand) public pure returns (string memory) {
        string memory fence = '';
    ...
    function _generateAllFences(CityParkUtils.Art memory artData) public pure returns (string memory) {
        ...
        for (uint i = 0; i < numFenceStructures; i++) {
            string memory localFences = _generateFence(artData, i);
            bool xPos =  CityParkUtils.seededRandom(0,2,i*i+69,artData) > 1;
            bool yPos =  CityParkUtils.seededRandom(0,2,i*i+420,artData) > 1;
            uint randX = CityParkUtils.seededRandom(0,300,i*i+888,artData);
            uint randY = CityParkUtils.seededRandom(0,300,i*i+777,artData);
