...
contract FixedPricePool is Base {
    ...
    /* struct CurrentState{
        ...
        uint startTime;
        uint endTime;
        uint minimumFillPercentage;
        uint minimumFill;
        uint fundRaised;
        uint tokenAmountSold;
        uint tokenAmountLeft;
        uint minimumOrderSize;
        uint price;
        uint[] maximumAllocation;
        uint totalWithdrawnAmount;
        uint maximumFunding;
        uint stakingTokenReward;
        address stakingToken;
        bool RemainingStakingTokenWithdrawnByPlatformAdmin;
        uint claimLockDuration;
        uint claimAvailableTimestamp;
        uint[] minimumStakeTiers;
    ...
    /* function getCurrentState1() view public returns (address, uint, uint, uint, uint,
    ...
    function getCurrentState2() view public returns (uint[] memory, uint, uint, uint, address,
                                                    bool, uint, uint, uint[] memory, address, address) {
        return (
            maximumAllocation,
            ...
            RemainingStakingTokenWithdrawnByPlatformAdmin,
            claimLockDuration,
            claimAvailableTimestamp,
            minimumStakeTiers,
            address(stakingContract),
            address(whitelistContract)
        );
    ...
    /* function getCurrentState() view public returns (CurrentState memory) {
    ...
    function getCurrentState() view public returns (uint[] memory, uint[] memory, uint[15] memory, address[4] memory, bool) {
        uint[15] memory otherUints = [tokenAmountToSell, startTime, endTime, minimumFillPercentage, minimumFill, fundRaised, tokenAmountSold, tokenAmountLeft, minimumOrderSize, price, totalWithdrawnAmount, maximumFunding, stakingTokenReward,claimLockDuration, claimAvailableTimestamp];
        address[4] memory addresses = [address(token), address(stakingToken), address(stakingContract), address(whitelistContract)];
        return (maximumAllocation, minimumStakeTiers, otherUints, addresses, RemainingStakingTokenWithdrawnByPlatformAdmin);
