...
contract testCompany {
    ...
    function random() private view returns(uint){
        uint rand= uint(keccak256(abi.encodePacked(block.timestamp,block.difficulty)))%10000;
        return rand;
        }
    // --------------Random function-----------------------
    // --------------Company function----------------------
    ...
    function changeCompanyStatus(uint _companyId, uint _adminID, uint _status) public{
        ...
        require(addresstoId[msg.sender]==_adminID, 'Wrong ID');
        require(companyInfoMapping[_companyId].adminId == _adminID, 'You are not allowed');
