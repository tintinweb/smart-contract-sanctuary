...
contract StarrToken is Context, IERC20, Ownable {
    ...
    // Owner cannot transfer out this contract's token from this smart contract
    ...
    function setWalletBanStatus(address user, bool banned) external onlyOwner {
        if (banned) {
            require(
                1618531200 + 3 days > block.timestamp,
                'Owner cannot longer ban wallets'
            );
