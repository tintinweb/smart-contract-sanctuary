...
contract BullAlphaEth is Pausable, AlphaTokenEth {
    ...
    /**
     * @dev Set the address treasury.
     */
    constructor(address _treasury, address _stableTokenAddress) {
        require(_treasury != address(0), "Alpha: treasury is the zero address");
        require(
   ...
   // }
    ...
    function _deposit(uint256 _amount) internal {
        ...
        TVL = TVL + _amount;
        uint256 _amountAlpha = convertToAlpha(_amount);
    ...
    function _requestWithdrawal(uint256 _amount) internal {
        ...
        uint256 stableValueAfterFees = convertFromAlpha(_amountAlpha);  
    ...
    //function sendFees() external onlyOwner {
      //  require(CLAIMABLE_FEE > 0, "Alpha: fees is zero");
       // CLAIMABLE_FEE = 0;
      //  emit SendFees(treasury, CLAIMABLE_FEE);
   // }

    function convertToAlpha(uint256 _amount) internal view returns (uint256) {
        uint256 _alphaAmount = (_amount *
            (COEFF_SCALE_DECIMALS + RATE_TO_ALPHA)) / COEFF_SCALE_DECIMALS;
        return _alphaAmount;
    }

    function convertFromAlpha(uint256 _amount) internal view returns (uint256) {
