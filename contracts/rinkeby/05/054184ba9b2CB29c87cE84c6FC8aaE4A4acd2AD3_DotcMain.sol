...
contract DotcMain is owned {
	...
	if(side == 1)
	{
		...
		require(ad.status == 3,"is done");
		require(block.timestamp-ad.place_time >= frozen_span, "is releasable");
	...
	{
		require(isReleasable(account, order_id),"is releasable");
	...
	require(tokens.length>0 && tokens.length == values.length, "Withdraw check");
