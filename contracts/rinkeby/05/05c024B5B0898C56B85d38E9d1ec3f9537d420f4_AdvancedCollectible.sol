...
contract AdvancedCollectible is ERC721, VRFConsumerBase {
    ...
    uint8 DOOM_ENUM_LENGTH = 4;
    mapping(uint256 => DOOM) public tokenIdToDoom;

    mapping(bytes32 => address) public requestIdToSender;

    event requestedCollectible(bytes32 indexed requestId, address requester);
    event doomGuyAssigned(uint256 indexed tokenId, DOOM guy);
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber)
        ...
        override
    {
        DOOM doom = DOOM(randomNumber % DOOM_ENUM_LENGTH);
        uint256 newTokenId = tokenCounter++;
        tokenIdToDoom[newTokenId] = doom;
        emit doomGuyAssigned(newTokenId, doom);
