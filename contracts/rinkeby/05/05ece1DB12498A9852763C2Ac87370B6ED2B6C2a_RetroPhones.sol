...
contract RetroPhones is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public mintPrice = 75000000000000000; // 0.075 ETH;
    uint256 public mintPriceAllowlist = 55000000000000000; // 0.055 ETH;
    ...
    constructor(address _randomNumberProvider, address _allowlist) ERC721("RetroPhones", "RTP") {
    ...
    function mintRetroPhones(uint256 amount) external payable whenSaleIsActive {
        ...
        require(balanceOf(msg.sender) + amount <= maxPerWallet, "RetroPhones: Amount exceeds max per wallet");
        require(_tokenIdCounter.current() + amount <= MAX_RETROPHONES, "RetroPhones: Purchase would exceed cap");

        if (allowlist.allowlist(msg.sender)) {
    ...
    function randomNumberToOffset() external onlyOwner {
        ...
        require(PROVENANCE_HASH != 0, "RetroPhones: PROVENANCE_HASH needs to be set");
        OFFSET_VALUE = IRandom(randomNumberProvider).asRange(PROVENANCE_HASH, 1, 10000);
    }
...
contract Allowlist is Ownable {
  ...
  function addAddressToAllowlist(address _addr) public returns(bool success) {
    require(memberCount < maxLength, "RetroPhonesAllowlist: Allowlist is full");
  ...
  function removeSelfFromAllowlist() public returns(bool success) {
    require(allowlist[msg.sender], "RetroPhonesAllowlist: Not on the list");
    memberCount = memberCount - 1;
    allowlist[msg.sender] = false;
    AllowlistedAddressRemoved(msg.sender);
    success = true;
  }
}
