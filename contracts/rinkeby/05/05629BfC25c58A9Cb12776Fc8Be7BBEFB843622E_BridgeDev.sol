...
contract BridgeDev is AccessControl, Pausable, ReentrancyGuard {
  ...
  function deposit(address _ethToken, uint256 _amount) external nonReentrant whenNotPaused returns(bool) {
    ...
    require(_amount <= maxBridgeAmount, "Amount cant be greater than maxBridgeAmount");
    // check for time bound
    require(bridgeTxTime[msg.sender] + bridgeTimeBound >= block.timestamp, "Next transaction should be after the time bound");
  ...
  function withdraw(address _ethToken, address _user, uint256 _amount) external nonReentrant whenNotPaused returns(bool) {
    ...
    address ektaToken = EthEktaPairs[_ethToken];
    emit TokenWithdrawn(_ethToken, ektaToken, _amount, _user);
    return true;
  }
  ...
  function updatebridgeTimeBound(uint256 timeBound) external onlyRole(UPDATER_ROLE){
    require(timeBound > 60, "Time bound cant be less than 1 minute");
