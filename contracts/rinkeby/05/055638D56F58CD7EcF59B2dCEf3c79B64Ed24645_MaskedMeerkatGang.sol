	...
	Ownable 
{
    ...
    uint256 public totalMeerkatSupply = 0;
    uint256 public presaleTime = 1633141341; 
    uint256 public presaleEndTime = 1633156161;
    
    // time where general sale occurs
    uint256 public pledgeTime = 1633156161; 
    
    // mint time should be the same as general time
    uint256 public mintTime = 1633156521; 
    ...
    // available supply is the amount available for general sale which should 6667
    ...
    address dev = 0xaBC03008D7A596E3867Bd936009d912D345503Ec;
    ...
    */
    function giveAway() external onlyOwner() {
        for (uint256 i = 1; i <= reservedGiveaways; i++) {
            _safeMint(dev, totalMeerkatSupply + i);
            _incrementTokenId(); 
        }
    ...
    */
    function presale(uint8 _num) external payable onlyWhitelisted {
        ...
        require(
            (_num + presaleNumOfPlayer[msg.sender] + claimed[msg.sender]) <= 5,
    ...
    /**
     * @dev 6667 meerkats available for the general sale
     */
    ...
    function mint(uint8 _num) external payable {
        ...
        require(
            _num <= 5,
            "Up to 5 Meerkats can be minted in a transaction"
        );
        require(
            msg.value == uint256(_num) * price,
            "You need to pay the exact price!"
