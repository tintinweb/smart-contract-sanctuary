...
contract decoffer_V2{
    ...
    Credit_token CT = Credit_token(0x9AE69fec5Bd9a24103FC711f70320204230183e0);
    ...
    function withdraw()public {
        ...
        CT.transferFrom(msg.sender,address(0x0),coffer_info[msg.sender][4]);
...
contract CT_debt{
    ...
    decoffer_V2 DT = decoffer_V2(0x0077DF36FF453fb2e9c3e56Bbda41dcE397AfAa5);
    Credit_token CT = Credit_token(0x9AE69fec5Bd9a24103FC711f70320204230183e0);
    ...
    mapping(uint=>mapping(uint=>uint))public fee_info;
    mapping(address=>mapping(uint=>uint))public debt_info; //1認購支出 2認購枚樹 3.解約時間
    ...
    function Subscription()public payable{
        ...
        uint eth_amount = msg.value.sub(msg.value.mul(fee_info[1][1]).div(1000));
        uint ct_amount = eth_amount.mul(out_share).div(DT_balance);
        
        require(CT.balanceOf(address(this)) >= ct_amount);
        debt_info[msg.sender][1] = debt_info[msg.sender][1].add(msg.value);
        debt_info[msg.sender][2] = debt_info[msg.sender][2].add(ct_amount);
        debt_info[msg.sender][3] = now.add(fee_info[2][1]);
        
        
        CT.transfer(msg.sender,ct_amount);
        DT.input.value(msg.value)();
        
    }
    
    
    function exchange()public{
        require(CT.balanceOf(msg.sender) >=  debt_info[1][2]);
        require(debt_info[msg.sender][3] <= now);
        
        uint DT_balance = DT.get_balance();
        uint out_share = DT.get_out_share();
        
        uint debt_price = debt_info[msg.sender][2].mul(DT_balance).div(out_share); //目前CT的價格
        debt_price = debt_price.sub(debt_price.mul(fee_info[1][2]).div(1000)); //目前CT價增加成數
   
        DT.CT_swap_ETH(debt_info[1][2],debt_price,msg.sender);
        reset();
...
//----------------view-----------------------------
    ...
    function get_ct_balance()public view returns(uint){
