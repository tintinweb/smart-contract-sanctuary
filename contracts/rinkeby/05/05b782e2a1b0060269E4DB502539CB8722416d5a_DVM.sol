...
contract DVM is ERC20 {
     ...
     *  Event
     *
     * 廠商幫用戶維修物品時觸發該event，該event會在後台有歷史紀錄 讓廠商和顧客看到資料被登記
     */
...
struct Vehicle {
    address owner;
    string vMake; //廠牌 Audi
    string vModel; //款式 A4 Avant 35 TFSI
    string vListed; //掛牌日期 2021/02/25
    string vTransmission; //變速箱  CVT 8速手自排
    string vColor; //顏色 白銀色
    string vEnergy; //能源種類 : 汽油
    string vExhaust; // 排氣cc數 1798
    int256 vMile;
    int256 previousOwner;
    int256 Accidents;
}
