...
contract Nobodyv2 is ERC721, Ownable {
  ...
  function airdropNobody(address[] memory _to, uint256 _numNobody ) public onlyOwner {        
      for (uint256 i = 0; i < _to.length; i++) {
      _mintnobody(_to[i], _numNobody);
      }
  }

  function airdropNobodylist(address[] memory _to) public onlyOwner {
     for (uint256 i = 0; i < _to.length; i++) {
         uint256 newItemId = _tokenIds.current();
        _tokenIds.increment();
