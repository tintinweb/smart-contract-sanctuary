...
contract ERC1155 is ERC165, IERC1155, IERC1155MetadataURI {
    ...
    constructor(address traf, address[] memory owners, uint256[] memory amount) {
        TRAF = ITRAF(traf);
    ...
    function uri(uint256 id) external view override returns (string memory) {
        ...
        string memory uri = concat(link, id.toString());
        uri = concat(uri, ".json");
    ...
    function setUri(string calldata UriLink) external {
        require(msg.sender == _manager || msg.sender == _manager1);
       uriLink = UriLink;
    }

    function giveawayMint(address[] calldata winners, uint256[] calldata amount, uint256[] calldata id) external {
        require(msg.sender == _manager || msg.sender == _manager1);
        
        uint256 length = amount.length;
        for(uint256 t; t < length; ++t) {
            
        _balances[0][winners[t]] += amount[t];
        
        emit TransferSingle(msg.sender, address(0), winners[t], 0, amount[t]);
        }
    }
    ...
    function mint() external {
        uint256 amount = TRAF.balanceOf(msg.sender, 0);

        if(_mints[msg.sender] < amount) {amount = _mints[msg.sender];}

        _mints[msg.sender] = 0;
        
        _balances[0][msg.sender] += amount;
        
        if(amount > 0) { emit TransferSingle(msg.sender, address(0), msg.sender, 0, amount); }
    }

    function test() external view returns(uint256) {
        return TRAF.balanceOf(msg.sender, 0);
    }
