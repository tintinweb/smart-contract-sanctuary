...
contract Vesting {
    ...
    noGrantExistsForUser(_recipient)
    {
        ...
        uint256 amountVestedPerMonth = _amount.div(_vestingDuration);
    ...
    function calculateGrantClaim(address _recipient) public view returns (uint256, uint256) {
        ...
        uint256 elapsedTime = now.sub(tokenGrant.startTime);
        ...
        if (elapsedMonths >= tokenGrant.vestingDuration) {
            ...
            uint256 amountVestedPerMonth = tokenGrant.amount.div(tokenGrant.vestingDuration);
