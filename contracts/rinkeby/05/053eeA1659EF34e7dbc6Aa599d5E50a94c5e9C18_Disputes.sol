...
contract Disputes is IDisputes {
        ...
        returns (uint256)
    {
        ...
        disputeCount++;
        eventsInstance.disputeRaised(disputeCount - 1);
    ...
    function attachMultipleDocuments(
        ...
        uint256 _taskID = dispute.taskID;
        assertMember(_project, _taskID, msgSender());
        uint256 _attachmentCount = dispute.attachmentsCount;
        dispute.attachmentsCount += _attachments.length;
        for (uint8 i = 0; i < _attachments.length; i++) {
            dispute.attachments[_attachmentCount] = _attachments[i];
            eventsInstance.disputeAttachmentAdded(
                _disputeID,
                msgSender(),
                _attachmentCount,
    ...
    function executePhaseAdd(address _project, bytes memory _actionData)
        ...
        override
    {
        IProjectContract(_project).addPhasesGC(_actionData, bytes(""));
    }
    ...
    function executePhaseChange(address _project, bytes memory _actionData)
        ...
        override
    {
        IProjectContract(_project).changeCostGC(_actionData, bytes(""));
    }
        ...
        override
    {
        ...
        IProjectContract(_project).releaseFeeContractor(_phase);
    }
