...
contract BoostingChef is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 constant public EPS = 10000;
    uint256 public maxBoostRate = 20;
    ...
    function calculateShare(uint256 boostRate, uint256 lpAmount, uint256 boostAmount) public pure returns (uint256) {
        return boostAmount.mul(boostRate).div(EPS).add(lpAmount);
    }

    function deposit(uint256 _pid, uint256 _lpAmount, uint256 _boostAmount) public notPause {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (_lpAmount > 0) {
            ...
            // action
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _lpAmount);
        ...
        if (_boostAmount > 0) {
            ...
            // action
            boostToken.safeTransferFrom(address(msg.sender), address(this), _boostAmount);
        }
        ...
        emit Deposit(msg.sender, _pid, _lpAmount, _boostAmount);
    }

     // Withdraw LP tokens from MasterChef.
    function withdraw(uint256 _pid, uint256 _lpAmount, uint256 _boostAmount) public notPause {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.lpAmount >= _lpAmount, "Withdraw: insufficient LP balance");
