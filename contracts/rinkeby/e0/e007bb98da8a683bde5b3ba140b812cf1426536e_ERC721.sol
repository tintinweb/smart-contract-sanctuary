...
contract WCAGTest is ERC721Enumerable,ERC2981Collection  {
    ...
    mapping(uint256 => bool) editionCheck;
    ...
    constructor(string memory _tokenName, string memory _tokenSymbol) ERC721(_tokenName, _tokenSymbol)  
    {
        ...
        //openseas proxy registry Project Wyern Proxy
  ...
  function gateKeeperOneToggleAdminChange(bool toggle_bool)  onlyGateKeeperOne public
    ...
    function addEdition(string memory _editionName, uint256 _editionLimit,uint256 _price)  onlyWhitelisted public
    {

        uint256 editionId = nextEditionId;
        editions[editionId].name = _editionName;
        editions[editionId].editionLimit = _editionLimit;
        editions[editionId].editionPrice=_price;
        editionCheck[editionId]=true;
        //
    ...
    function updateEditionprice(uint256 _editionId, uint256 _price) onlyWhitelisted public 
    {
        editions[_editionId].editionPrice = _price;
    }
    ...
    function galleryMint(address _to, uint256 _editionId, bool _freeze, string memory _ipfsHash) onlyWhitelisted external returns (uint256 _tokenId) 
    {
        require(editionCheck[_editionId],"E23!");
        ...
        require(gateKeeperOneAppointed, "E19!");
        require(gateKeeperTwoAppointed, "E20!");
    ...
    function publicSaleMint(address _to, uint256 _editionId, bool _freeze, string memory _ipfsHash) public payable returns (uint256 _tokenId) 
    {
        require(editionCheck[_editionId],"E23!");
        require(msg.value >=  editions[_editionId].editionPrice, "E24!");
        require(editions[_editionId].editionPrice>0, "E25!");
        require(editions[_editionId].minted.add(1) <= editions[_editionId].editionLimit, "E15!");
        require(!editions[_editionId].paused , "E16!");
        require(gateKeeperOneAllowMinting, "E17!");
        require(gateKeeperTwoAllowMinting, "E18!");
    ...
    function editionTokenInfo(uint256 _editionId) view public returns (uint256 minted, uint256 editionLimit, bool paused, bool locked,uint256 price) 
    {
        ...
        price=editions[_editionId].editionPrice;
    ...
    //per token conventions-ask Shao
    //add receive and withdraw, max supply, frozen issue
    //is ipfs link enough
