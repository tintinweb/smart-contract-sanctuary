...
contract CryptoRugsAlpha is ERC721, Ownable {
    using SafeMath for uint256;
    using Strings for uint256;
    uint public constant MAX_RUGS = 1024;
    uint256 public constant RUGS_PRICE = 10000000000000000; // 0.01 ETH
    ...
    address public developerAddress = 0x9ac1EA6429AE959D4B1AFFea2078085e967B867c;
    address public artistAddress = 0x9ac1EA6429AE959D4B1AFFea2078085e967B867c;
    address public constant ADVISOR_ADDRESS = 0x9ac1EA6429AE959D4B1AFFea2078085e967B867c;
    string public script;
    string public scriptType = "p5js";
    bool public saleStarted = false;
    mapping (uint256 => uint256) public creationDates;

    constructor() ERC721("CryptoRugsAlpha","RUGS")  {
        setBaseURI("https://api.cryptorugs.xyz/woc/json/");
    }
   ...
   function mint(uint256 rugsNumber) public payable {
        require(saleStarted, "Sale has not started yet");
        require(totalSupply() < MAX_RUGS, "Sale has already ended");
        require(rugsNumber > 0 && rugsNumber <= 20, "Rugs number must be between 1 and 20");
        require(totalSupply().add(rugsNumber) <= MAX_RUGS, "Exceeds MAX_RUGS");
        require(msg.value >= RUGS_PRICE.mul(rugsNumber), "Ether value sent is below the price");

        for (uint i = 0; i < rugsNumber; i++) {
   ...
   function reserve(uint256 rugsNumber, address toAddress) public onlyOwner {
        require(totalSupply() < MAX_RUGS, "There are no rugs left");
        require(totalSupply().add(rugsNumber) <= MAX_RUGS, "Exceeds MAX_RUGS");
