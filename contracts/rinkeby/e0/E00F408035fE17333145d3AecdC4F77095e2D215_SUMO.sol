...
contract SUMO is ERC721, ERC721Enumerable, Ownable {
    ...
    function setTokenURI() external onlyOwner {
        string memory currentBaseURI = _baseURI();
        for ( uint i = 0; i < MAX_TOKENS ; i ++ ) {
            _tokenUri[i] = bytes(currentBaseURI).length > 0
                ? string(abi.encodePacked(currentBaseURI, ( i ).toString(), baseExtension))
                : "";
        }
        isSetedURI = true;
    }
