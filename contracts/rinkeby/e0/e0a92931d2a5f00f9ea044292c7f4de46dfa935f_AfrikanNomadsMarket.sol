...
interface AfrikanNomads is IERC721 {
...
contract AfrikanNomadsMarket is ReentrancyGuard, Pausable, Ownable {
    using SafeMath for uint;

    AfrikanNomads africanNomadsContract;     // instance of the CryptoPhunks contract

    uint256 platformFee = 25;
    uint256 artistFee = 75;
    ...
    constructor(address initialAddress) {
        AfrikanNomads(initialAddress).balanceOf(address(this));
        africanNomadsContract = AfrikanNomads(initialAddress);
    }
    ...
    function setAfrikanNomadsContract(address newContract) public onlyOwner {
      africanNomadsContract = AfrikanNomads(newContract);
    }
    ...
    function removeFromSale(uint index) public nonReentrant() {
        if (index >= 10000) revert('token index not valid');
