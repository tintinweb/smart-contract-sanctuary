 ...
 */
contract UserContract is NFTokenMetadata, Ownable, Escrow {
    /**
...
contract Escrow {
    ...
    // Withdraw all ethers
    function withdrawAll(address payable to) public returns (bool success) {
        require(balances[to] != 0, NO_BALANCE);
        to.transfer(balances[to]);
        emit LogTransfer(address(this), to, balances[to]);
        balances[to] = 0;
        return true;
    }
