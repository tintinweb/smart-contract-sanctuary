...
abstract contract CustomMintRenameWithEth is CustomMintRename {
    ...
    function updateRenameFeeEth(uint256 renameFeeInWei) public {
        require(ownerAddress == _msgSender(), "Must be owner");
        ...
        payable
    {
        ...
        require(checkLength(newName, MAX_LENTH), "Name too long");
        ...
        private
    {
        ...
        uint256 feePlusRemainder = feeCollectorFeeAmount + remainderOfAllocation;

        ownerAddress.transfer(feePlusRemainder);
    }
    ...
    function _getRenameFeesAvailableToClaimEth(
        uint32 jNumber
...
abstract contract CustomMintRename is Context, ERC721Pausable {
    ...
    uint256 constant internal PRECISION_MULTIPLIER  = 1e18;
    ...
    // Credit some guy on stack overflow
    function checkLength(string memory str, uint256 maxLength)
        public
        pure
        returns (bool underMaxLength)
    {
        uint256 i = 0;
        uint256 length = 0;
...
contract MyERC721Token is Ownable, CustomMintRenameWithEth, CustomMintRenameWithERC20 {
    constructor(string memory baseURI) public ERC721("MichaelJackson", "MKR") {
        ownerAddress = _msgSender();

        // MINT the 0th eth J to the owner
        _safeMint(ownerAddress, uint256(0));
...
abstract contract CustomMintRenameWithERC20 is CustomMintRename {
    ...
    function upsertNewErc20Token(
        ...
        require(ownerAddress == _msgSender(), "Must be owner");
        uint256 convertedRenameFee = uint256(10)**decimals * renameFee;
        ...
        for (uint256 i = 0; i < supportedErc20Contracts.length; i++) {
            if (
                ...
                supportedErc20Contracts[i].renameFee = convertedRenameFee;
                return;
            }
        }
        ...
        public
    {
        ...
        _safeTransferFrom(erc20ContractAddress, minter, thisContractAddress, cost);
        // Sweep fee from contract to owner
        uint256 fee = CostUtils.getMintFee(jNumber, decimals);
        _safeTransfer(erc20ContractAddress, ownerAddress, fee);
    ...
    function _allocateAndSweepRenameFee(
        ...
        // Transfer total cost from minter to our contract
        uint256 costOfRename = _getRenameFeeForContractAddress(erc20ContractAddress);
        _safeTransferFrom(erc20ContractAddress, ownerOfNft, address(this), costOfRename);
        ...
        _safeTransfer(erc20ContractAddress, ownerAddress, feePlusRemainder);
    }
        ...
        public
    {
        ...
        uint256 ownerFeeAmount = _getRenameFeesAvailableToClaimERC20(
            erc20ContractAddress,
            jNumber
        );
    ...
    function getRenameFeesAvailableToClaimERC20(
        ...
        return _getRenameFeesAvailableToClaimERC20(erc20ContractAddress, jNumber);
    }
    ...
    function _safeTransferFrom(
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), "Failed to transferFrom");
    }
    ...
    function _safeTransfer(
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'Failed to transfer');
    }
...
contract MyERC20Token is ERC20PresetMinterPauser {

    constructor(string memory name, string memory symbol) public ERC20PresetMinterPauser(name, symbol) {
    }


    function mintForJ(uint32 jNumber, address erc721ContractAddress) public {
        uint256 cost = (jNumber * 110 / 100) * (10**18);
        mint(0x70997970C51812dc3A010C7d01b50e0d17dc79C8, cost);
        _approve(0x70997970C51812dc3A010C7d01b50e0d17dc79C8, erc721ContractAddress, cost);
    }
}
