...
contract MaxNugzSupply is ERC721, Ownable, IMintedBeforeReveal {
    ...
    uint256 public constant MAX_NUGGZ_SUPPLY = 24;
    ...
    function getMonstroMaxAmount() public view returns (uint256) {
        ...
        require(totalSupply() < MAX_NUGGZ_SUPPLY, "Sale has already ended and all sold out, no more left to sell.");
    ...
    function getMonstroPrice() public view returns (uint256) {
        require(block.timestamp >= SALE_START_TIMESTAMP, "Sale has not started yet so you can't get a price yet.");
        require(totalSupply() < MAX_NUGGZ_SUPPLY, "Sale has already ended, no more Monsters left to sell.");
    ...
    function mintMonster(uint256 numberOfMonstro) public payable {
        ...
        require(totalSupply() < MAX_NUGGZ_SUPPLY, "Sale has already ended.");
        ...
        require(SafeMath.add(totalSupply(), numberOfMonstro) <= MAX_NUGGZ_SUPPLY, "Exceeds maximum Monster supply of 8,000. Please try to mint less Monsters.");
        ...
        if (startingIndexBlock == 0 && (totalSupply() == MAX_NUGGZ_SUPPLY || block.timestamp >= REVEAL_TIMESTAMP)) {
    ...
    */
    function finalizeStartingIndex() public {
        ...
        startingIndex = uint(blockhash(startingIndexBlock)) % MAX_NUGGZ_SUPPLY;

        if (SafeMath.sub(block.number, startingIndexBlock) > 255) {
            startingIndex = uint(blockhash(block.number-1)) % MAX_NUGGZ_SUPPLY;
        }
