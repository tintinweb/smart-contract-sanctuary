...
contract HashWord is ERC721Enumerable, Ownable, ReentrancyGuard {
    using Strings for uint256;

    uint256   public constant _maxSupply        = 10000;
    uint256   public constant _maxMintCount     = 7;
    uint256   public constant _maxPresaleCount  = 2;
    uint256   public _mintPrice                 = 0.07 ether;           //70000000000000000 wei
    ...
    mapping(string => bool) public _wordList;
    mapping(uint256 => string)public _wordNum;

    constructor(address signer, address multiSig) ERC721("Test", "TST") {
      _signer = signer;
      _multiSig = multiSig;
    }
    ...
    function MintWord (uint256 mintCount) internal {
      uint256 s = totalSupply();
      require(mintCount > 0, "count <= zero");
      require(s + mintCount <= _maxSupply, "Max Out");
    ...
    function split() internal {
      ...
      ( sent, ) = payable(_multiSig).call{value: share}("");
    ...
    function presaleMint(uint256 tokenCount, bytes memory signature) public payable {
      ...
      require(_presaleIsActive, "Pre-Sale not active");
      require(verify(msg.sender, signature), "Not on Presale List");
    ...
    function publicMint(uint256 tokenCount, string memory word) public payable {
      require(_publicSaleIsActive, "Public Sale not active");
      require(!(_presaleIsActive), "Pre-Sale is active");
      require(tokenCount <= _maxMintCount, "Only 7 tokens can be minted per TX");
      require(_mintPrice * tokenCount == msg.value, "Incorrect Eth");
      require(!_wordList[word], "Word Taken");

      _wordList[word] = true;
      _wordNum[totalSupply()] = word;
    ...
    function genSVG(uint256 tokenId) internal view returns (string memory) {
      ...
      parts[0] = "<svg width='500' height='500' viewBox='0 0 350 350' style='background-color:black' xmlns='http://www.w3.org/2000/svg'><text x='50%' y='50%' dominant-baseline='middle' text-anchor='middle' fill='white'>";
      parts[1] = _wordNum[tokenId];
      parts[2] = "</text></svg>";

      string memory baseURL = "data:image/svg+xml;base64,";
      string memory svgBase64Encoded = Base64.encode(bytes(string(abi.encodePacked(parts[0], parts[1], parts[2]))));
      return string(abi.encodePacked(baseURL,svgBase64Encoded));
    }

    function tokenURI(uint256 tokenId) override public view returns (string memory) {
      require(_exists(tokenId), 'Token does not exist');
      string memory svgData = genSVG(tokenId);
