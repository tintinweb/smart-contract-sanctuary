...
contract LiquidityPoolLocker is ILiquidityPoolLocker {
    ...
    function lockLP(address _owner, address _token) external override {
        uint256 balance = IERC20(_token).balanceOf(_owner);
        IERC20(_token).transferFrom(msg.sender, _token, balance);

        releaseDate = block.timestamp + 365 days;
        owner = _owner;
        token = _token;
    }
