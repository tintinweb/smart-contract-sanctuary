 ...
 */
contract ArgentModule is
  BaseModule,
  RelayerManager,
  SecurityManager,
  TransactionManager
{
  bytes32 public constant NAME = 'ArgentModule';
  ...
  {
    require(
      registry.isRegisteredModule(_module),
      'AM: module is not registered'
    );
    ...
    returns (uint256, OwnerSignature)
  {
    ...
    if (
      ...
      methodId ==
      TransactionManager.multiCallWithGuardiansAndStartSession.selector ||
...
abstract contract RelayerManager is BaseModule, SimpleOracle {
   ...
   * @param _wallet The target wallet.
   */
  function getSession(address _wallet)
    external
    view
    returns (address key, uint64 expires)
  {
  ...
  function checkAndUpdateUniqueness(
    ...
    if (
      ...
      (ownerSignatureRequirement == OwnerSignature.Required ||
        ownerSignatureRequirement == OwnerSignature.Session)
    ) {
  ...
  function refund(
    ...
    if (
      _gasPrice > 0 &&
      (_option == OwnerSignature.Required || _option == OwnerSignature.Session)
    ) {
      ...
      if (_requiredSignatures == 1 && _option == OwnerSignature.Required) {
        ...
        if (
          ...
          uint256 whitelistAfter = userWhitelist.getWhitelist(
            _wallet,
            refundAddress
          );
          require(
            whitelistAfter > 0 && whitelistAfter < block.timestamp,
            'RM: refund not authorised'
          );
        }
      }
      uint256 refundAmount;
...
abstract contract TransactionManager is BaseModule {
   ...
   * @param _wallet The target wallet.
   */
  function clearSession(address _wallet)
    external
    onlyWalletOwnerOrSelf(_wallet)
    onlyWhenUnlocked(_wallet)
  {
   ...
   */
  function removeFromWhitelist(address _wallet, address _target)
    external
    onlyWalletOwnerOrSelf(_wallet)
    onlyWhenUnlocked(_wallet)
  {
