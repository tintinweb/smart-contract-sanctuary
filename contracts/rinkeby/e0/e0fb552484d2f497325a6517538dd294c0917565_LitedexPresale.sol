...
contract LitedexPresale is TimeCrowdsale{
    ...
    function initialize(
        ...
        crowdsaleInfo[_id].totalCap = initialCap;
        crowdsaleInfo[_id].price = initialPrice;
        crowdsaleList.push(crowdsaleList.length.add(1));
    ...
    function purchase(uint256 amount) external onlyWhileOpen returns(bool){
        ...
        if(cappable == false) {
            ...
            pair.transferFrom(msg.sender, address(this), tokenReached.div(10 ** (_tokendecimals.sub(_pairdecimals))));
        }
     ...
     */
    function getEstimateToken(uint256 _amount) internal view returns(uint256) {
        ...
        if(cappable == false){
            estimateToken = _amount.mul(10 ** tokendecimal).div(price);
        }else{
