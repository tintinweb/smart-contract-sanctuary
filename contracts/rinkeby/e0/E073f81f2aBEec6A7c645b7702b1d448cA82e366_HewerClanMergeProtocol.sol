...
contract HewerClanMergeProtocol is Ownable, ERC721Enumerable {
    ...
    mapping(string => mapping(uint256 => MergedEntry)) collectionNftTokenIdToMergeState;
    mapping(uint256 => MergedEntry) tagTokenIdToMergeState;
    ...
    constructor(string memory uri, address hewerClanContractAddress) ERC721("HewerClanMergeProtocol", "HCMP") {
        setBaseURI(uri);
    ...
    function mintMergeRequest(uint256 tagTokenId, uint256 tokenId, string memory collectionName) public onlyRegisteredCollection(collectionName) {
        ...
        tagTokenIdToMergeState[tagTokenId] = entry;
        collectionNftTokenIdToMergeState[collectionName][tokenId] = entry;
    ...
    function mergeRequest(uint256 mergedTokenId, uint256 tagTokenId, uint256 collectionTokenId, string memory collectionName) public onlyOwnerOf(mergedTokenId) {
    ...
    function tagIsMerged(uint256 tagTokenId) public view returns (bool) {
        return tagTokenIdToMergeState[tagTokenId].merged;
    }

    function collectionNftIsMerged(string memory collectionName, uint256 tokenId) public view returns (bool) {
        return collectionNftTokenIdToMergeState[collectionName][tokenId].merged;
    }
    ...
    function addCollection(string memory name, address _addr) public onlyOwner {
        ...
        //        require(IERC721(_addr).supportsInterface(type(Collection).interfaceId), "Given address does not support the required interface");
