...
contract CyblocPresale {
  ...
  struct Package {
      ...
      uint256 maxGold;
      uint256 maxPlatium;
      uint256 maxLegendary;
      uint16[5] classProbabilities;
  }

  Package public packNormal = Package({
          packType: 0,
          quantity: 3000,
          price: 0.00001 ether,
          maxBronze: 6997,
          ...
          maxPlatium: 6,
          maxLegendary: 1,
          classProbabilities: [6997, 2113, 183, 6, 1]
        });

  Package public packRare = Package({
          packType: 1,
          quantity: 300,
          price: 0.00004 ether,
          maxBronze: 130,
          ...
          maxPlatium: 12,
          maxLegendary: 2,
          classProbabilities: [130, 380, 343, 12, 2]
        });

  Package public packEpic = Package({
          packType: 2,
          quantity: 30,
          price: 0.00006 ether,
          maxBronze: 0,
          ...
          maxPlatium: 53,
          maxLegendary: 6,
          classProbabilities: [uint16(0), uint16(0), 31, 53, 6]
        });                
  ...
  // mapping (uint8 => uint256) public totalBronzeOpened;
  // mapping (uint8 => uint256) public totalSilverOpened;
  // mapping (uint8 => uint256) public totalGoldOpened;
  // mapping (uint8 => uint256) public totalPlatiumOpened;
  // mapping (uint8 => uint256) public totalLegendaryOpened;

  // pack type > Cybloc classs > value
  mapping (uint8 => mapping (uint256 => uint256)) public totalCyblocOpenedInPack;
  mapping (address => mapping (uint8 => uint256)) public packsPurchased;
  mapping (address => mapping (uint8 => uint256)) public packsOpened;
    ...
    returns (uint256 totalPrice)
  {
    ...
    totalPrice = totalPrice.add(_cyblocPackPrice(packNormal.packType, normalQuantity));

    totalPrice = totalPrice.add(_cyblocPackPrice(packRare.packType, rareQuantity));

    totalPrice = totalPrice.add(_cyblocPackPrice(packEpic.packType, epicQuantity));
  }
    ...
    payable
  {
    ...
    if (normalQuantity > 0) {
      packsPurchased[msg.sender][packNormal.packType] = packsPurchased[msg.sender][packNormal.packType].add(normalQuantity);
      totalPacksPurchased[packNormal.packType] = totalPacksPurchased[packNormal.packType].add(normalQuantity);
    ...
    if (rareQuantity > 0) {
      packsPurchased[msg.sender][packRare.packType] = packsPurchased[msg.sender][packRare.packType].add(rareQuantity);
      totalPacksPurchased[packRare.packType] = totalPacksPurchased[packRare.packType].add(rareQuantity);
      emit CyblocPackPurchased(
              msg.sender,
              packRare.packType,
              rareQuantity
    ...
    if (epicQuantity > 0) {
      packsPurchased[msg.sender][packEpic.packType] = packsPurchased[msg.sender][packEpic.packType].add(epicQuantity);
      totalPacksPurchased[packEpic.packType] = totalPacksPurchased[packEpic.packType].add(epicQuantity);
      emit CyblocPackPurchased(
              msg.sender,
              packEpic.packType,
              epicQuantity
  ...
  function _pickGenes(uint8 packType, uint256 _initialSeed) public returns (uint256) {
    uint256 _class;
    uint256 _seed;
    uint256 _genes;   
    _seed = _randomNumber(_initialSeed);
    _class = _pickRandomClass(packType, _seed);
    _genes = _generateGenes(_class, _seed);
 
    return _genes;
  }

  function _pickRandomClass(uint8 packType, uint256 _seed) private returns (uint256) {
    Package memory packInfo;
    if (packType == 0) {
      packInfo = packNormal;
    } else if (packType == 1){
      packInfo = packRare;
    ...
    uint256 totalCyblocInPack = packInfo.maxBronze.add(
                                packInfo.maxSilver).add(
                                packInfo.maxGold).add(
                                packInfo.maxPlatium).add(
                                packInfo.maxLegendary);
                              
    uint256 value = _seed.mod(totalCyblocInPack - totalCyblocOpened);
    // Start at top class (length - 1)
    // skip bronze (0), we default to it
    for (uint256 i = packInfo.classProbabilities.length - 1; i > 0; i--) {
      uint256 probability = packInfo.classProbabilities[i];
      uint256 cyblocOpenedInPack = totalCyblocOpenedInPack[packType][i];
      if (value < probability && cyblocOpenedInPack < probability) {
        totalCyblocOpened++;
        totalCyblocOpenedInPack[packType][i] = cyblocOpenedInPack.add(1);
        return i;
      } else {
  ...
  function _generateGenes(uint256 _class, uint256 _seed) private view returns (uint256) {
    uint256 _genes;   
    bytes memory _classBytes = abi.encodePacked(_class);
    uint256 _genesTmp = _seed;   
    uint256 _length;
    ...
    bytes memory _genesBytes = new bytes(_classBytes.length + _length + 5);
    uint256 _i = _genesBytes.length - 6;

    _genesTmp = _seed;

    do {
      _genesBytes[_i--] = bytes1(uint8(48 + _genesTmp % 10));
      _genesTmp /= 10;
    } while (_genesTmp > 0);

    for (_i = 0; _i < _classBytes.length; _i++) {
      _genesBytes[_i] = _classBytes[_i];
    }

    _genes = toUint256(_genesBytes);
    return _genes;
  }    
