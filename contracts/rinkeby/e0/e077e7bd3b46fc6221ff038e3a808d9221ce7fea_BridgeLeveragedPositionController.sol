...
import "../../interfaces/bridge/IUserLeveragePool.sol";
import "../../interfaces/bridge/IRewardsGenerator.sol";
import "../../interfaces/bridge/IShieldMining.sol";
import "../../interfaces/bridge/IBMICoverStaking.sol";
import "../../interfaces/bridge/ILeveragePortfolio.sol";
import "./DecimalsConverter.sol";
import "../../interfaces/bridge/IBMICoverStakingView.sol";
...
contract BridgeLeveragedPositionController is AbstractController, IERC1155Receiver {
    ...
    function maxUnstake() public view override returns (uint256) {
        if (staking()) {
            return bmiCoverStaking.stakedByNFT(_nftId);
        } else {
            return leveragedPortfolio.getAvailableBMIXWithdrawableAmount(address(this));
        }
    }
    ...
    function stake(uint256 _amount) external override onlyIndex {
        ...
        uint256 _stblAmount = _swapTokenForToken(_amount, address(base), address(stblToken));
        // STBL -> bmiX -> stake
        ...
        _nftId = bmiCoverStaking.tokenOfOwnerByIndex(address(this), 0);
    ...
    function callUnstake() external override onlyIndex returns (uint256) {
        ...
        uint256 _bmiXAmount = requestWithdrawal();
    ...
    // It might require to withdraw it later again
    function requestWithdrawal() public returns (uint256 _bmiXAmount) {
    ...
    function unstake() external override onlyIndex returns (uint256) {
        ...
        _checkTetherApprovals(address(stblToken), address(_uniswapRouter), _amountStbl);
        uint256 _baseAmount = _swapTokenForToken(_amountStbl, address(stblToken), address(base));
    ...
    function withdrawRewards() external override {
        ...
        //swap BMI into base
        uint256 _baseAmount = _sellBMI(_rewards);

        //deposit base into index
        index.depositInternal(_baseAmount);
    }

    function _sellBMI(uint256 _amount) internal returns (uint256 _baseAmount) {
        _checkApprovals(bmiToken, address(_uniswapRouter), _amount);
        _baseAmount = _swapTokenForToken(_amount, address(bmiToken), address(base));
    ...
    function outstandingRewards() public view override returns (uint256) {
        return bmiCoverStaking.getSlashedBMIProfit(_nftId);
    }

    function outstandingRewardsWithoutSlashing() public view returns (uint256) {
        return bmiCoverStaking.getBMIProfit(_nftId);
    }
    ...
    function netWorth() external view override returns (uint256 _worth) {
        ...
        uint256 _worthStbl;
        if (staking()) {
            uint256 _bmiRewards = outstandingRewards();
            uint256 _rewardsInBase = _priceFeed.howManyTokensAinB(
                address(base),
                address(bmiToken),
                _bmiRewards
            );
            uint256 _bmiXStaked = bmiCoverStaking.stakedByNFT(_nftId);
            _worthStbl = leveragedPortfolio.convertBMIXToSTBL(_bmiXStaked).add(_rewardsInBase);
        } else {
            //or unstaking
            (uint256 _unstakingBMIX, ) = unstakingInfo();
            _worthStbl = leveragedPortfolio.convertBMIXToSTBL(_unstakingBMIX);
        }
        _worth = _priceFeed.howManyTokensAinB(address(base), address(stblToken), _worthStbl);
    }
    ...
    function apy() external view override returns (uint256) {
        ...
        return
            leveragedPortfolio.getAPY().add(
                bmiCoverStakingView.getPolicyBookAPY(address(leveragedPortfolio))
            );
    }
...
interface IUserLeveragePool {
    ...
    //BU Additions
    function stblToken() external view returns (address);

    function rewardsGenerator() external view returns (address);

    function shieldMining() external view returns (address);

    function bmiCoverStaking() external view returns (address);

    function bmiCoverStakingView() external view returns (address);
...
interface IBMICoverStaking {
    ...
    //BU Additions
    function bmiToken() external view returns (address);
...
abstract contract AbstractController is OwnableUpgradeable, IPositionController {
    ...
    function _calculateRewards() internal view returns (uint256 _rewards, uint256 _feeAmount) {
        _rewards = outstandingRewards();
        //calculate fee
        ...
        _rewards = _rewards.sub(_feeAmount);
    }
