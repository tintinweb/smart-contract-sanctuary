...
contract MiddlePunks is Initializable, OwnableUpgradeable, ERC721EnumerableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  uint256 public constant MAX_MINTABLE_AT_ONCE = 200;
  ...
  function maxNumFreeMidPunks() public view virtual returns (uint256) {
    // how many MidPunks that will cost 0.
    // the number The Dark Lords give away will be (10000 - however some free Goblin Master MidPunks are minted)
    return 10000;
  }
  ...
  function mint(uint256 _numToMint) public payable nonReentrant() {
    ...
    if (isFreeSaleOn) {
      require(
        totalSupply < maxNumFreeMidPunks(),
      ...
      require(
        ...
        "All MidPunks have been mint already."
      );
      require(
        totalSupply + _numToMint <= numTotalMidPunks(),
        "There aren't this many MidPunks left."
      );
      uint256 costForMintingMidPunks = getCostForMintingMidPunks(_numToMint);
      require(
        msg.value >= costForMintingMidPunks,
        "Too little sent, please send more eth."
      );
      if (msg.value > costForMintingMidPunks) {
        payable(msg.sender).transfer(msg.value - costForMintingMidPunks);
      }
    }
  ...
  function getCostForMintingMidPunk(uint256 _numMinted)
    private
    view
    returns (uint256)
  {
  ...
  function getCostForMintingMidPunks(uint256 _numToMint)
    public
    view
    returns (uint256)
  {
    ...
    for (_index; _index < _numToMint; _index++) {
      _cost += getCostForMintingMidPunk(totalSupply() + _index);
    }
    return _cost;
  }
  ...
  function ownerMint(uint256 _numToMint) public onlyOwner {
    ...
    require(
      ...
      "Devs have already minted all available dev MidPunks."
    );
