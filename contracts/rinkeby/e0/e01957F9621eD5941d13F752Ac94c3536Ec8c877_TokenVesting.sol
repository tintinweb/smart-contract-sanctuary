...
contract TokenVesting is Context,Ownable{
    ...
    function upDataRelease(bool isUpDataCreateTimestamp) private returns(uint256,uint256) {
      ...
      if(releaseDay >= 1 && tokenVestUsers[msg.sender]._lockUpAmount != 0){
        ...
        if(indexReleaseDeltaunlockReleaseAmount > tokenVestUsers[msg.sender]._lockUpAmount.sub(tokenVestUsers[msg.sender]._unlockAmount)){
           indexReleaseDeltaunlockReleaseAmount = tokenVestUsers[msg.sender]._lockUpAmount.sub(tokenVestUsers[msg.sender]._unlockAmount);
        }
