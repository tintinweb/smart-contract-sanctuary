...
import "./facades/IFeeDistributor.sol";
...
contract PowerLiquidVault is Ownable {
  ...
  function purchaseLPFor(address beneficiary) public payable lock {
      ...
      require(msg.value > 0, "PowerLiquidVault: ETH required to mint OptionsMarket LP");
      ...
      uint osmTokensRequired;

      if (address(config.osmToken) < address(config.weth)) {
          osmTokensRequired = config.uniswapRouter.quote(
              exchangeValue,
              reserve2,
              reserve1
          );
      ...
      require(
          balance >= osmTokensRequired,
          "PowerLiquidVault: insufficient OptionsMarket tokens in PowerLiquidVault"
      );
  ...
  //send ETH to match with OptionsMarket tokens in PowerLiquidVault
  ...
  function claimLP() public {
      ...
      require(
          ...
          "revert PowerLiquidVault: LP still locked."
      );
      next++;
  ...
  function buyPressure(uint amount) external {
      ...
      address[] memory path = new address[](2);
            ...
            path[1] = address(config.osmToken);
