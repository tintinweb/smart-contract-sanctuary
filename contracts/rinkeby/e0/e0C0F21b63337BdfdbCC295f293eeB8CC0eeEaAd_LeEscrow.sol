...
contract LeEscrow is Ownable, ReentrancyGuard {
    ...
    bool public onlyTreasuryWithdrawalEnabled = true; 
    ...
    function _deposit(address _user, uint256 _amount, uint256 _tokenID) internal{
        ...
        token.transferFrom(_user, treasury, _amount.mul(10**ti.decimal));
    ...
    function _withdraw(address _user, uint256 _amount, uint256 _tokenID) internal{
        ...
        TokenInfo memory ti = listOfTokenInfo[_tokenID];
        IERC20 token = IERC20(ti.tokenAddress);
        require(userBalances[_user][_tokenID] >= _amount.mul((ti.rates.div(1e6))), "LeEscrow: Balance Less than withdrawn amount");
        userBalances[_user][_tokenID] = userBalances[_user][_tokenID].sub(_amount.div((ti.rates.div(1e6))));
        
        token.transferFrom(address(this), _user, _amount.mul(10**ti.decimal));
    ...
    function withdrawToTreasury(uint256 _tokenId) public onlyOwner {
        ...
        token.transferFrom(address(this),treasury, balance);
