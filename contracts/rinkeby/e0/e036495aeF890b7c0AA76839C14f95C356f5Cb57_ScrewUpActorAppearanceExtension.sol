...
contract ScrewUpActorAppearanceExtension is ScrewUpInGameActorExtension,IScrewUpActorAppearanceExtension {
    ...
    //Fire when update status definition.
    event OnPartsPresetUpdated(uint256 presetId,uint256 partId,uint256 partValue);

    event OnPartsPresetNameUpdated(uint256 presetId,string partName);

    //Fire when Actor appearance updated
    event OnActorAppearanceUpdated(uint256 indexed actorId,uint256 _id,uint256 _value);
    ...
    function definePartPreset(string memory presetName,uint256[] memory partIds,uint256[] memory partValues) external onlyOwner {
        ...
        for(uint256 i = 0; i < partIds.length; i++){
            _presets[newPresetId][partIds[i]] = partValues[i];
            emit OnPartsPresetUpdated(newPresetId,partIds[i],partValues[i]);
        }
    ...
    function updatePresetValues(uint256 presetId,uint256[] memory partIds,uint256[] memory partValues)  external onlyOwner{
        require(isPartsPresetDefined(presetId),"Id and Name length mismatch");
        for(uint256 i = 0; i < partIds.length; i++){
            _presets[presetId][partIds[i]] = partValues[i];
            emit OnPartsPresetUpdated(presetId,partIds[i],partValues[i]);
        }
    }
    function getPartPresetDefinition() external view virtual override returns (FPresetDeinition[] memory){
        ...
        for(uint256 pId = 1; pId <= lastPresetId; pId++){
            uint256[] memory _preset_partIds = _getPresetPartIds(pId);
            FPresetDeinition memory _definition;
            _definition.Id = pId;
            _definition.Name = _preset_names[pId];
            _definition.Values = new FPartValue[](_preset_partIds.length);
             for(uint256 pt = 0; pt <  _preset_partIds.length; pt++)
                _definition.Values[pt] = FPartValue(_preset_partIds[pt], _presets[pId][_preset_partIds[pt]]);
    ...
    function _applyActorAppearanceWithPreset(uint256 actorId,uint256 presetId) internal{
        uint256[] memory _pIds = _getPresetPartIds(presetId);
        for(uint256 i = 0; i < _pIds.length; i++){
           uint256 _preset = _presets[presetId][_pIds[i]];
           _actor_appearances[actorId][_pIds[i]] = _preset;
           emit OnActorAppearanceUpdated(actorId,_pIds[i],_preset);
        }
    }
    function _applyActorAppearance(uint256 actorId,uint256[] memory partIds,uint256[] memory partValues) internal{
        for (uint256 i = 1; i <= partIds.length; i++) {
            if(_part_defined[partIds[i]]){
                _actor_appearances[actorId][partIds[i]] = partValues[i];
                emit OnActorAppearanceUpdated(actorId,partIds[i],partValues[i]);
            }
        }
    }
    function _getPresetPartIds(uint256 presetId) internal view returns(uint256[] memory){
        uint256 partCount = 0;
        uint256 lastPartId = _partIds.current();
        for(uint256 sId= 1; sId<= lastPartId; sId++){
...
interface IScrewUpActorAppearanceExtension{
    ...
    function getPartPresetDefinition() external view returns (FPresetDeinition[] memory);
