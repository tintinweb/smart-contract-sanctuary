...
contract pizzaNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    function createPizzaIngretients(address wallet_address, string[] memory tokenURI) public  onlyOwner{
        _itemIds.increment();
   ...
   function addMedia(string memory _name, string memory _mediaHash) public returns (bool success) {
        
        MediaAsset memory currentMedia;
        currentMedia.id = counter;
        currentMedia.name = _name;
        currentMedia.author = msg.sender;
        currentMedia.createDate = block.timestamp;
