...
contract BridgePool is SigCheckable, WithAdmin {
	...
	mapping(uint256=>address) public allowedTargets;
	...
	function allowTarget(uint256 chainId, address targetToken) external onlyAdmin() {
		allowedTargets[chainId] = targetToken;
	}

	function disallowTarget(uint256 chainId) external onlyAdmin() {
		delete allowedTargets[chainId];
	}
    ...
    function _swap(address from, address token, uint256 amount, uint256 targetNetwork,
		...
		require(allowedTargets[targetNetwork] == targetToken, "BP: target not allowed");
