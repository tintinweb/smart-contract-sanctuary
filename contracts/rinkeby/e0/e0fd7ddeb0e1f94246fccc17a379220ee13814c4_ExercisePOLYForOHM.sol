...
contract ExercisePOLYForOHM {
    ...
    function setMaxAllowedToClaim(address _vester, uint _amountCanClaim ) external returns ( bool ) {
        require( msg.sender == owner, "Sender is not owner" );
        require( _amountCanClaim > maxAllowedToClaim[ _vester ], "cannot lower amount claimable" );
    ...
    // vesting rate is in hundreths i.e. 50 = 0.5%
    function setVestingRate( address _vester, uint _rate) external returns ( bool ) {
        require( msg.sender == owner, "Sender is not owner" );
        require( _rate > percentCanVest[ _vester ], "cannot lower vesting rate" );
