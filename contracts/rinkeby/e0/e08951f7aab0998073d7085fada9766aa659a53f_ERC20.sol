...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    event BattleCompleted(uint16 _fromId, uint16 _toId, bool _outcome);
     ...
     */
    function getRandomNFTId(uint16 _Id) internal view returns (uint16) {
        ...
        {
            randomNFTId = uint(keccak256(abi.encodePacked(block.timestamp, _seed))).mod(IFA(_nftAddress).totalSupply());
            if(randomNFTId == _Id)
            {
                _seed = _seed.add(1);
            }
