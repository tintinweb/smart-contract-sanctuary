...
contract AbachiBondDepositoryV2 is IBondDepository, NoteKeeper {
/* ======== DEPENDENCIES ======== */
...
/* ======== CONSTRUCTOR ======== */
  ...
  constructor(
    IAbachiAuthority _authority,
    IERC20 _abi,
    IgABI _gabi,
    IStaking _staking,
    ITreasury _treasury
  ) NoteKeeper(_authority, _abi, _gabi, _staking, _treasury) {
    // save gas for users by bulk approving stake() transactions
    _abi.approve(address(_staking), 1e45);
  }
...
/* ======== DEPOSIT ======== */
   ...
   * @return payout_     the amount of gABI due
  ...
  function deposit(
     ...
     * price = quote tokens : abi (i.e. 42069 DAI : ABI)
     *
     * 1e18 = ABI decimals (9) + price decimals (9)
     */
     ...
     *
     * this is either the number of ABI that the market can sell
    ...
    // purchased, and how much ABI has been sold
  ...
  function _tune(uint256 _id, uint48 _time) internal {
    ...
    if (_time >= meta.lastTune + meta.tuneInterval) {
      ...
      // abi decimals (9) + price decimals (9)
       ...
       * is 10,000 ABI. max payout would be 1,000 ABI (10,000 * 1 / 10).
       */
...
/* ======== CREATE ======== */
   ...
   * @param _market      [capacity (in ABI or quote), initial price / ABI (9 decimals), debt buffer (3 decimals)]
  ...
  function create(
     ...
     *
     * 1e18 = abi decimals (9) + initial price decimals (9)
     */
     ...
     * interval. for example, if capacity is 1,000 ABI, there are 10 days to conclusion,
     * and the preferred deposit interval is 1 day, max payout would be 100 ABI.
     */
    ...
    emit CreateMarket(id_, address(abi), address(_quoteToken), _market[1]);
  }
...
/* ======== EXTERNAL VIEW ======== */
   ...
   * @param _id          ID of market
   * @return             price for market in ABI decimals
   *
   ...
   * @param _id          ID of market
   * @return             amount of ABI to be paid in ABI decimals
   *
   * @dev 1e18 = abi decimals (9) + market price decimals (9)
   */
   ...
   * @param _id          ID of market
   * @return             current debt for market in ABI decimals
   */
   ...
   * @param _id          ID of market
   * @return             control variable for market in ABI decimals
   */
...
abstract contract NoteKeeper is INoteKeeper, FrontEndRewarder {
  ...
  constructor (
    ...
    IStaking _staking,
    ITreasury _treasury
  ) FrontEndRewarder(_authority, _abi) {
    gABI = _gabi;
    staking = _staking;
    treasury = _treasury;
  }
...
/* ========== ADD ========== */
   ...
   * @param _payout      the amount of ABI due to the user
  ...
  function addNote(
    ...
    // note that only the payout gets staked (front end rewards are in ABI)
...
/* ========== REDEEM ========== */
   ...
   * @param _sendgABI    send payout as gABI or sABI
   * @return payout_     sum of payout sent, in gABI
   */
  function redeem(address _user, uint256[] memory _indexes, bool _sendgABI) public override returns (uint256 payout_) {
    ...
    if (_sendgABI) {
      gABI.transfer(_user, payout_); // send payout as gABI
    } else {
      staking.unwrap(_user, payout_); // unwrap and send payout as sABI
    }
  }
   ...
   * @param _sendgABI    send payout as gABI or sABI
   * @return             sum of payout sent, in gABI
   */
  function redeemAll(address _user, bool _sendgABI) external override returns (uint256) {
    return redeem(_user, indexesFor(_user), _sendgABI);
  }
...
/* ========== VIEW ========== */
   ...
   * @return payout_     the payout due, in gABI
...
interface IBondDepository {
  ...
  struct Market {
    ...
    bool capacityInQuote; // capacity limit is in payment token (true) or in ABI (false, default)
  ...
  struct Terms {
    ...
    uint64 maxDebt; // 9 decimal debt maximum in ABI
  }
...
abstract contract FrontEndRewarder is AbachiAccessControlled {
  ...
  IERC20 internal immutable abi; // reward token

  constructor(
    IAbachiAuthority _authority,
    IERC20 _abi
  ) AbachiAccessControlled(_authority) {
    abi = _abi;
  }
  ...
  function getReward() external {
    ...
    abi.transfer(msg.sender, reward);
  }
...
interface IgABI is IERC20 {
  ...
  function initialize( address _staking, address _sABI ) external;
}
...
interface INoteKeeper {
  ...
  struct Note {
    uint256 payout; // gABI remaining to be paid
  ...
  function redeem(address _user, uint256[] memory _indexes, bool _sendgABI) external returns (uint256);
  function redeemAll(address _user, bool _sendgABI) external returns (uint256);
