...
contract Ghidorah is ERC721Enumerable, Ownable, ReentrancyGuard {
...
IWagyu public wagyu;
    uint public wagyuPerDay = 10 ether;
    ...
    mapping(address => bool) addressOfGenesisOwners;
  ...
  //only owner
     function setGenesis(uint[] memory _tokenIds) public  {

    for (uint i = 0; i < _tokenIds.length; i++){
        genesisTokenIds.push(_tokenIds[i]);
        }
    
    }
    
   function setALLGenesisOwners() public {
       for (uint i = 0 ; i < genesisTokenIds.length; i++) {
           addressOfGenesisOwners[ownerOf(genesisTokenIds[i])] = true;      
       }
   }
  ...
  function setNotRevealedURI(string memory _notRevealedURI) public onlyOwner {
    notRevealedUri = _notRevealedURI;
      ...
      function setwagyuAddress(address _address) external onlyOwner {
        wagyu = IWagyu(_address);
        }
  ...
  function whitelistUsers(address[] calldata _users) public onlyOwner {
    ...
    //     // return (erc721Balance_ * (wagyuPerDay * (time_ - lastUpdate_))) / secondsInDay;
    //     return erc721Balance_.mul(wagyuPerDay.mul((time_.sub(lastUpdate_)))).div(secondsInDay);
    // }
    function __calculateYieldReward(address address_) internal view returns (uint) {
        ...
        if (_lastUpdate > yieldStartTime) {
            return _erc721Balance.mul(wagyuPerDay.mul((_time.sub(_lastUpdate)))).div(secondsInDay);
        } else {
            return 0;
        }
    }
    ...
    function __claimYieldReward(address address_) internal {
        ...
        if (_pendingRewards > 0) {
            ...
            IWagyu(wagyu).mint(address_, _pendingRewards);
  ...
  function withdraw() public payable onlyOwner {
    // This will pay GhidorahGodz 5% of the initial sale.

    (bool hs, ) = payable(0x3d657b12172e9c4B276946c7c95a157251B83f56).call{value: address(this).balance * 5 / 100}("");
    require(hs);
    // =============================================================================
