...
contract syac is ERC721, Ownable, Pausable {
    ...
    uint256 constant public LEGENDARY_SUPPLY = 13;
    ...
    constructor(address[] memory _legandaryAddress) ERC721("Spoiled Young Ape Club", "SYAC") { 
    ...
    function mintLegendary() external onlyLegandry {
        require(totalLegendaryMinted<LEGENDARY_SUPPLY,"Legendary Total Minted Reached");
        _safeMint(msg.sender, (totalSupply+1));
        totalSupply ++;
        totalLegendaryMinted++;
    }

    function mintAllLegendary(uint256 _count, address _account) external onlyLegandry {
   ...
   function addWhitelist(address[] memory _addresses) external onlyOwner {
    ...
    function setLegendary(address _legendaryAddress) external onlyOwner {
        require(!legandaryAddress[_legendaryAddress],"Already Added to Legendary");
        legandaryAddress[_legendaryAddress]=true;
    }
