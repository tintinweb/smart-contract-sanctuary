...
contract NNX is Ownable, ERC721, IMerkleDistributor {
    ...
    uint256 public reserveLimit = 20;
    ...
    constructor(string memory _baseURI, bytes32 _merkleRoot) ERC721("N N X", "NNX") {
        baseURI = _baseURI;
    ...
    function setReserveLimit(uint256 _reserveLimit) external onlyOwner {
    ...
    function airdrop(address _luckyAddr) external onlyOwner {
        _batchMint(_luckyAddr, 1);
    ...
    function mint(uint256 _num) external payable {
        require(totalSupply.add(_num) <= maxTotalSupply.sub(reserveLimit), "Sold out.");
        require(block.timestamp >= saleStartTime, "Sale is not active.");
        require(_num <= mintLimit, "Too many tokens for one transaction.");
    ...
    function claim(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) external override {
        ...
        reserveLimit = reserveLimit.sub(amount);
