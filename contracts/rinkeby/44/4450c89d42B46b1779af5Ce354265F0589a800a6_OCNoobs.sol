...
contract OCNoobs is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    uint256 public constant maxTokensPerTransaction = 30;
    uint256 public tokenPrice = 0.01 ether;
    uint256 public constant maxNumber = 732;
    Counters.Counter private _tokenIdCounter;

     constructor() ERC721("OCNoobs", "Noobs") {
         claim();
    }
    
    // function addManyColorsToPalette(string[] calldata newColors) external onlyOwner {
    //     require(palettes.length + newColors.length <= 256, 'Palettes can only hold 256 colors');
    //     for (uint256 i = 0; i < newColors.length; i++) {
    //         palettes.push(newColors[i]);
    //     }
    // }

    function tokenURI(uint256 tokenId) public pure override returns (string memory){

    string memory svg = NoobGenerator.getnoobForSeed(Strings.toString(tokenId * 3210));
    string memory aparts = '[{ "trait_type": "type", "value": "1"}]';
       
    string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "OnChain Noobs", "description": "Beautiful views, completely generated OnChain","attributes":', aparts, ', "image": "data:image/svg+xml;base64,', Base64.encode(bytes(svg)),'"}'))));
    ...
    function claim() public {
        require(_tokenIdCounter.current() < 500);
        ...
        returns (string memory)
    {
       ...
       string memory head =
        ...
        string memory svg = string(
            abi.encodePacked(
                '<svg width="100%" height="100%" viewBox="0 0 640 640" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" presevAspectRatio="xMidYMid meet"><defs>',
