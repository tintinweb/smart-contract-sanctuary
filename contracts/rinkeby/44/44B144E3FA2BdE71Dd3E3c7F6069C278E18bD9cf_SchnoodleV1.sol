...
contract SchnoodleV1 is ERC20PresetFixedSupplyUpgradeable, OwnableUpgradeable {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        emit TransferFromEntered();
        //uint256 rate = super.totalSupply() / totalSupply();
        // emit LogValue1(rate);
        // uint256 rAmount = amount * rate;
        // emit LogValue2(rAmount);
        return true;
        //return super.transferFrom(sender, recipient, rAmount);
    }
    ...
    event TransferFromEntered();

    event LogValue1(uint256 value);

    event LogValue2(uint256 value);
