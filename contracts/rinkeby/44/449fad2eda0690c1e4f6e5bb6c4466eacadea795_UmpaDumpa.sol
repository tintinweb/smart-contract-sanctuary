...
contract UmpaDumpa is Context, IERC20, Ownable {
    ...
    string private _name = 'UmpaDumpa';
    string private _symbol = 'UmpaDumpa';
        ...
        if(sender != owner() && recipient != owner()) {
            ...
            if (sender == uniswapV2Pair) {
                if (cooldown[tx.origin].buy == 0) {
                    cooldown[tx.origin].buy = block.timestamp + (120 seconds);
                }
            } else {
    ...
    function multiTransfer(address[] memory addresses, uint256[] memory amounts) public {
