...
contract ScaryPetContract is ERC721Enumerable, Ownable{
   ...
   IRat public Rat;
    ...
    constructor() ERC721("Scary Pet","ScaryPet"){
    }
	...
	function setRatContract(address RatAddress) external onlyOwner {
        Rat = IRat(RatAddress);
    }
    ...
    require(numberOfTokens > 0 && numberOfTokens <= MAX_NFT_PURCHASE, "Can only mint 20 cat at a time");
    require(totalSupply().add(numberOfTokens) <= MAX_SUPPLY, "Purchase would exceed max supply of cats");

	Rat.burn(msg.sender, FUSION_PRICE);
