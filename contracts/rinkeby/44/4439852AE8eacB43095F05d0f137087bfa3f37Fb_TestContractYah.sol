...
contract TestContractYah is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    mapping(uint256 => BlockData[]) public stackData;
    mapping(uint256 => address) private owners;
    mapping(address => uint) private blocksMinted;
    
    uint256 public initialSupply = 10000;
    uint256 private _reserved = 100;
    uint256 private _price = 0.05 ether;
    uint256 private _maxMintPerTransaction = 10;
    ...
    uint public numOfFullStacks;
    
    bool public _paused = true;
    
    constructor() ERC721("TestContractYah", "TCY") {}
    ...
    {
        require(msg.value >= _price * quantity, "Payment amount invalid. Cost = Price * Quantity");
        require(quantity <= _maxMintPerTransaction, "You can only mint up to 10 blocks per transaction");
        require(_tokenIdCounter.current() + quantity <= initialSupply - _reserved, "Exceeds maximum supply" );
        
        for (uint256 i=0; i<quantity; i++)
        {
            uint256 colorId = random(7, _tokenIdCounter.current());
            ...
            blocksMinted[msg.sender] += 1;
        }
    }
    
    function giftBlocksTo(address recipient, uint256 quantity) Secure public onlyOwner
    {
        require(quantity <= _reserved, "Exceeds reserved supply");
    ...
    function stackManyBlocks(uint256[] memory blockIds) public
    {
        require(blockIds.length > 1 && blockIds.length < 8, "To stack you must provide 2 to 7 tokenIds");
    ...
    function getAttributes(uint256 tokenId) private view returns (string memory)
    {
        ...
        string[7] memory colorStrings;
        bool[7] memory alreadyClaimed;
        uint pointer;
        for (uint i=0; i<stackData[tokenId].length; i++)
        {
            ...
            if (alreadyClaimed[color] == false)
            {
                alreadyClaimed[color] = true;
        ...
        string memory height = string(abi.encodePacked(
            ...
            string(abi.encodePacked('"value": "', uint2str(stackData[tokenId].length), '"')),
            '}'
            ));
        
        string memory attributeString = string(abi.encodePacked('"attributes": [', parts[0], parts[1], parts[2], parts[3], parts[4], parts[5], parts[6], height, ']'));
    ...
    function getStackString(uint id) public view returns (string memory)
    {
        ...
        {
            result = string(abi.encodePacked(result, uint2str(stackData[id][i].color)));
        }
    ...
    function withdrawAll(address recipient) public payable onlyOwner {
