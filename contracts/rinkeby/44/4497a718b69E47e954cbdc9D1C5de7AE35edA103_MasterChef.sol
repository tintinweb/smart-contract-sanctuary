...
contract MasterChef is BlockReentrancyGuard{
    ...
    constructor(address _sDFIANCE, uint256 _distributionMultiplier) public {
        ...
        maximumDistributionMultiplier = 20000; // max mint mulitplier is at the start at x2
    }
    ...
    function addPool(address _lpPair, uint256 _tokenDistributionPerBlock, uint256 _startBlock) external onlyOperator poolNotExist(_lpPair) {         
        ...
        poolInfo[_lpPair].sDFIANCESupply = 1; // initialize to avoid div by 0
    }
    ...
    function activatePool(address _lpPair) external onlyOperator { 
        poolInfo[_lpPair].stopped = false; 
        poolInfo[_lpPair].lastUpdateBlock = block.number;
    }

    function desactivatePool(address _lpPair) external onlyOperator { 
        poolInfo[_lpPair].stopped = true; 
    }
    ...
    function updateDistributionRatio(address _lpPair) private {
        ...
        uint256 _newReward = block.number.sub(_pool.lastUpdateBlock).mul(_pool.tokenDistributionPerBlock.mul(distributionMultiplier).div(10000));
    ...
    function getRealDistributionRatio(address _lpPair) public view poolValid(_lpPair) returns (uint256) {
        uint256 _newReward = block.number.sub(poolInfo[_lpPair].lastUpdateBlock).mul(poolInfo[_lpPair].tokenDistributionPerBlock.mul(distributionMultiplier).div(10000));
    ...
    function deposit(address _lpPair, uint _lpAmount) external poolValid(_lpPair) nonBlockReentrant {
        ...
        // add new lp deposited to the supply
        ...
        uint256 virtualCollateralAmount = _lpAmount.div(poolInfo[_lpPair].lpSupply.sub(_lpAmount)).mul(poolInfo[_lpPair].sDFIANCESupply);
        poolInfo[_lpPair].sDFIANCESupply = poolInfo[_lpPair].sDFIANCESupply.add(virtualCollateralAmount);

        // get the distribution ratio (sDFIANCE / lpSupply)
        uint actualDistributionRatio = getRealDistributionRatio(_lpPair);

        // update user entry distribution ratio
        if (lastLpAmount == 0) {
            ...
            // calculate the average entry distribution ratio
    ...
    function withdraw(address _lpPair, uint _lpAmount) external poolValid(_lpPair) nonBlockReentrant {
        ...
        uint256 virtualCollateralAmount = _lpAmount.mul(actualDistributionRatio);
    ...
    function getPendingReward(address _lpPair, address _account) public view poolValid(_lpPair) returns (uint256) {
        uint reward = getRealDistributionRatio(_lpPair).sub(poolInfo[_lpPair].farmerInfo[_account].distributionRatio).mul(poolInfo[_lpPair].farmerInfo[_account].lpAmount); // multiply "reward token per lp" with the number of lp and divide to keep the result in wei
        return reward;
    }
    ...
    function _harvestReward(address _lpPair, address _from) poolValid(_lpPair) private {
