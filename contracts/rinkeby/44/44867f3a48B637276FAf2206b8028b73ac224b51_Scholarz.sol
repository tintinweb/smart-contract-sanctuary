...
contract Scholarz is ERC721EnumerableUpgradeable, ERC721PausableUpgradeable, OwnableUpgradeable {
  ...
  uint public constant MAX_GENESIS_AMOUNT = 2500; // CHANGE THIS TO 2500
  uint public constant MAX_UNIQUE_AMOUNT = 25; // CHANGE THIS TO 25
  ...
  mapping(uint => uint) private uniqueToken;
  ...
  function getTraits(uint tokenId) public view returns (uint[9] memory) {
    ...
    if (tokenSeed[tokenId] != 0) {
      ...
      if (uniqueToken[tokenId] == 0) {
        return _genTraits[tokenId];
      } else {
        uint[9] memory genes;
        genes[0] = 0;
        for (uint i = 1; i < 9; i++) {
          genes[i] = uniqueToken[tokenId];
        }
        return genes;
      }
    }
