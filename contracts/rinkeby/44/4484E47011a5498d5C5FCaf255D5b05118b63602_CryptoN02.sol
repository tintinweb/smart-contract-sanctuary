...
contract CryptoN02 is ERC721, Ownable, IMintedBeforeReveal {
    ...
    function getMonstroPrice() public view returns (uint256) {
        require(block.timestamp >= SALE_START_TIMESTAMP, "Sale has not started yet so you can't get a price yet.");
        require(totalSupply() < MAX_SUPPLY, "Sale has already ended, no more Monsters left to sell.");
    ...
    function mintMonster(uint256 numberOfMonstro) public payable {
        ...
        require(SafeMath.add(totalSupply(), numberOfMonstro) <= MAX_SUPPLY, "Exceeds maximum Monster supply of 8,000. Please try to mint less Monsters.");
