...
contract BattleRoyale is Ownable {
  ...
  function addToBattleQueue(address payable _battle) external payable onlyOwner returns(bool) {
    return battleQueue.push(_battle);
  }

  function getBattleQueue() external view onlyOwner returns (address payable[] memory) {
    return battleQueue.getAll();
  }

  function name() external view returns (string memory) {
    return "NFTBattleRoyale Arena";
  }
  ...
  returns(
    ...
    for (uint i = 0; i < battleQueue.size(); i++) {
      if (i >= battleQueue.size()) {
        break;
      }
      address payable nftAddress = battleQueue.atIndex(i);
      Royale royale = Royale(nftAddress);
      uint256 timestamp = royale.getTimestamp();
      uint256 intervalTime = royale.getIntervalTime();

      if (royale.getBattleStateInt() == 1
   ...
   */
   function performUpkeep(bytes calldata performData) external {
     ...
     royale.setTimestamp(block.timestamp);
     // execute random elimination using VRF
     royale.executeRandomElimination(1);
     // If battle state has ended, remove from queue
     if (royale.getBattleStateInt() == 2) {
       battleQueue.remove(nftAddress);
     }
   }

   function removeFromQueue(address payable nftAddress) external onlyOwner returns(address payable[] memory) {
     battleQueue.remove(nftAddress);
     return battleQueue.getAll();
   }
...
contract ERC721Tradable is ERC721, Ownable {
   ...
   */
  function getCurrentBalance() external onlyOwner returns (uint256) {
    uint256 balance = address(this).balance;
    return balance;
  }
  /**
...
contract Royale is ERC721Tradable, VRFConsumerBase {
  using AddressArray for AddressArray.Addresses;
   ...
   */
  function getCurrentLinkBalance() external onlyOwner returns (uint256) {
    return LINK.balanceOf(address(this));
  }
  /*
   * Update Chainlink Fee
   */
  function setChainlinkFee(uint256 _fee) external onlyOwner payable {
    fee = _fee * 10**18;
  }
  /*
   * Mint NFTs
  ...
  function purchase(uint256 units) external payable {
    ...
    require(purchasers.getIndex(msg.sender) < 0);
  ...
  /*
   * Method to update interval time
   * @param {[type]} uint256 [description]
   */
  function setIntervalTime(uint256 _interval) external payable onlyOwner {
   ...
   */
  function setDefaultTokenURI(string memory _tokenUri) external onlyOwner payable {
   ...
   * @param string IPFS meta-data uri
   */
  function setPrizeTokenURI(string memory _tokenUri) external onlyOwner payable {
  ...
  function setTimestamp(uint256 _timestamp) external payable {
    require(msg.sender == battleContract || msg.sender == owner());
  ...
  */
  function executeRandomElimination(uint256 userProvidedSeed) public returns (bytes32 requestId) {
    require(msg.sender == battleContract || msg.sender == owner());
  ...
  function fulfillRandomness(bytes32 _requestId, uint256 _randomNumber) internal override {
    ...
    if (inPlay.size() > 1) {
      ...
      uint256 index = _randomNumber % inPlay.size();
...
library Uint256Array {
   ...
   * @param element the element to add in the array
   */
  function push(Uint256s storage self, uint256 element) internal {
    if (!exists(self, element)) {
      self._items.push(element);
    }
  }
