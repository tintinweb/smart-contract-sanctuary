...
contract LiquidityMining is Ownable, ReentrancyGuard {
        ...
        checkStart
    {
        ...
        if (reward > 0) {
            totalUserRewards[msg.sender] += reward;
    ...
    function getTotalUserRewards(address account)  external view returns (uint256) {
        return totalUserRewards[account] + rewards[account];
    }
