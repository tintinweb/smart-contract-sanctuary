...
import "./sales/ExternalAuthSales.sol";
...
contract InfinityFactory is Ownable {
    ...
    address public clonableExternalAuthSalesAddress;
    ...
    event NewExternalAuthSalesCreated(address indexed owner, address contractAddress);
    ...
    function setClonableExternalAuthSalesAddress(address _clonableExternalAuthSalesAddress) public onlyOwner {
        clonableExternalAuthSalesAddress = _clonableExternalAuthSalesAddress;
    }
    ...
    function createExternalAuthSalesContract(string memory name ) public returns (address) {
        address factoryAddress = whatIsMyAddress();
        address clone = Clones.clone(clonableExternalAuthSalesAddress);
        ExternalAuthSales cloneContract = ExternalAuthSales(clone);
        cloneContract.init(name, factoryAddress);
        cloneContract.transferOwnership(msg.sender);
        emit NewExternalAuthSalesCreated(msg.sender, clone);
        return clone;
    }
