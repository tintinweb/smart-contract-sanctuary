...
contract Lightbulbman is ERC721Enumerable, Ownable, ILightbulbman {
    ...
    function setPrerevealURI(string memory _preRevealURI) external onlyOwner {
        preRevealURI = _preRevealURI;
        emit PrerevealURIChanged(_preRevealURI);
    }
    ...
    function gift(address _walletAddress, uint256 _amount) external onlyOwner  {
        require(totalSupply() < supplyLimit, "Not enough tokens left.");
               for (uint i = 0; i < _amount; i++) {
                    uint mintIndex = totalSupply();
                    _safeMint(_walletAddress, mintIndex);
        }
    }
    ...
    function tokenURI(uint256 _tokenId) public view override returns(string memory) {
        ...
        return (bytes(baseURI).length > 0 && tokenIndex > 0) ? 
            string(abi.encodePacked(baseURI, tokenIndex.toString())) : string(abi.encodePacked(preRevealURI));
    }
...
interface ILightbulbman {
    ...
    event PrerevealURIChanged(string _preRevealURI);
    ...
    event MaxTokensPerWalletChanged(uint256 _maxTokensPerWallet);
