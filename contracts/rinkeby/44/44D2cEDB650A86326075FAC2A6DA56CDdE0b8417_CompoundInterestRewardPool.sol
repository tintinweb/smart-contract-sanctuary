...
interface ICompoundProxyUserTemplate {
    function init( address _op, bytes16 _lendingId, address _user, address _rewardComp ) external;
    ...
    function recycle(address _asset,address _treasuryFund, address _underlyingToken) external;
...
// File: contracts/compound/CompoundPoolFactory.sol
...
contract CompoundPoolFactory {
    ...
    function CreateCompoundRewardPool(address rewardToken, address op)
        public
        returns (address)
    {
        CompoundRewardPool pool = new CompoundRewardPool(rewardToken, op);

        return address(pool);
    }

    function CreateCompoundInterestRewardPool(address rewardToken, address op)
        public
        returns (address)
    {
        CompoundInterestRewardPool pool = new CompoundInterestRewardPool(
            rewardToken,
            op
        );
