 ...
 * @title ERC20Migrator
 * @author Ruban
 * @notice You can use this contract for only waifer migration
 ...
 * ```
 */
contract WaiferMigrator is Initializable, OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  using SafeERC20Upgradeable for IERC20Upgradeable;

  /// Address of the old token contract
  IERC20Upgradeable private _legacyToken;
   ...
   */
  function initialize(address legacyToken, address newToken) public initializer {
    _legacyToken = IERC20Upgradeable(legacyToken);
    _newToken    = IERC20Upgradeable(newToken);
  }

  function _authorizeUpgrade(address) internal override onlyOwner {}

  /**
   * @dev Returns the legacy token address that is being migrated.
   */
  function getlegacyToken() external view returns (IERC20Upgradeable) {
    return _legacyToken;
  }

  /**
   * @dev Returns the new token address to which we are migrating.
   */
  function getnewToken() external view returns (IERC20Upgradeable) {
    return _newToken;
  }

  /**
   * @dev Setting new token address
   * @param newToken the token that will be minted
   */
  function setNewToken(address newToken) external onlyOwner {
     _newToken    = IERC20Upgradeable(newToken);
  }

  /**
   * @dev Setting legacy token address
   * @param legacyToken the token that will be minted
   */
  function setLegacyToken(address legacyToken) external onlyOwner {
     _legacyToken = IERC20Upgradeable(legacyToken);
  }
  ...
  function migrateAll(address account) external {
    ...
    uint256 amount = MathUpgradeable.min(balance, allowance);
  ...
  * total supply.
  */
  function removeToken(uint256 amount) external onlyOwner whenNotPaused returns(bool) {
      _removeToken(amount);
      return true;
  }
  ...
  */
  function _removeToken(uint256 amount) internal virtual {
    require(address(this) != address(0), "Address cant be zero address");
		// require amount greter than 0
		require(amount > 0, "amount cannot be 0");
    IERC20Upgradeable tokenContract = IERC20Upgradeable(address(this));
    require(tokenContract.balanceOf(address(this)) > amount, "removeToken: Remove token amount exceeds token balance");
