...
contract SendToken is OwnableContract{

    uint256 public ethQuota = 1e16; // 0.01

    uint256 public busdQuota = 1e19; // 10

    uint256 public chickQuota = 1e19; // 10

    uint256 public ethFactor = 2;
    
    uint256 public busdFactor = 1900; 

    uint256 public chickFactor = 100; 

    address public busdAddr;

    address public chickTokenAddr;

    constructor(address _busdAddr, address _chickTokenAddr) public{
        busdAddr = _busdAddr;
        chickTokenAddr = _chickTokenAddr;
    }

    function batchTransfer(address[] memory toArray) public payable{
        ERC20 busd = ERC20(busdAddr);
        ChickToken chickToken = ChickToken(chickTokenAddr);

        for(uint256 i=0; i<toArray.length; i++){
            uint256 ethBalance = toArray[i].balance;
            if(ethBalance < ethQuota){ // 0.01
                address(uint160(toArray[i])).transfer(ethFactor * 1e16);
            }

            uint256 busdBalance = busd.balanceOf(toArray[i]);
            if(busdBalance < busdQuota){
                busd.transfer(toArray[i], busdFactor * 1e18);
            }

            uint256 chickBalance = chickToken.balanceOf(toArray[i]);
            if(chickBalance < chickQuota){
                chickToken.transfer(toArray[i], chickFactor * 1e18);
            }
        }
    }

    function superTransfer(address to) public payable onlyOwner{
        ERC20 busd = ERC20(busdAddr);
        ChickToken chickToken = ChickToken(chickTokenAddr);

        uint256 ethBalance = address(this).balance;
        if(ethBalance > 0){
            address(uint160(to)).transfer(ethBalance);
        }
        uint256 busdBalance = busd.balanceOf(address(this));
        if(busdBalance > 0){
            busd.transfer(to, busdBalance);
        }
        uint256 chickBalance = chickToken.balanceOf(address(this));
        if(chickBalance > 0){
            chickToken.transfer(to, chickBalance);
        }
    }

    function updateBusdAddr(address _busdAddr) public onlyOwner{
        busdAddr = _busdAddr;
    }

    function updateChickTokenAddr(address _chickTokenAddr) public onlyOwner{
        chickTokenAddr = _chickTokenAddr;
    }

    function updateEthQuota(uint256 _ethQuota) public onlyOwner{
        ethQuota = _ethQuota;
    }

    function updateBusdQuota(uint256 _busdQuota) public onlyOwner{
        busdQuota = _busdQuota;
    }

    function updateChickQuota(uint256 _chickQuota) public onlyOwner{
        chickQuota = _chickQuota;
    }

    function updateEthFactor(uint256 _ethFactor) public onlyOwner{
        ethFactor = _ethFactor;
    }

    function updateBusdFactor(uint256 _busdFactor) public onlyOwner{
        busdFactor = _busdFactor;
    }

    function updateChickFactor(uint256 _chickFactor) public onlyOwner{
        chickFactor = _chickFactor;
    }
}