...
contract Game is Ownable, IERC721Receiver, IERC1155Receiver, ReentrancyGuard {
    ...
    */
    ...
    function getAttackPlayerReward(uint256 _opponent)
        public
        view
        returns (uint256 _tokenAmont, uint256 _timeRemoved)
    {
        uint256 ageOfLongestLiving =
            (block.timestamp - timeWenBorn[longestLivingId] + 1 days) / 1 days; //TODO check this as not sure this correct
        uint256 ageOfOpponent =
            (block.timestamp - timeWenBorn[_opponent] + 1 days) / 1 days;

        return (
            ageOfOpponent * 1 ether, //TODO this, for now it's just the time opponent lived in days * ether
            ageOfLongestLiving //TODO
        );
    }
    ...
    function buyPowerUp(uint256 _id, uint256 _amount) public payable {
        require(isAlive(_id), "!alive");
        require(_amount > 1 * 10**18); //min one token
        ...
        payable
    {
        ...
        require(!gameEnded, "ended");
        require(
            msg.value >= attackPrice[_attackId] && attackPrice[_attackId] > 0,
            "!money"
        );

        require(balanceOf(msg.sender) >= 1 || playersAlive == 1, "!pets");

        (uint256 tokens, uint256 timeToRemove) =
            getAttackPlayerReward(_opponent);

        // time to decrease is timeToRemove + the specific attack tod decrease.
        // they pay more eth for bigger attacks..
        // maybe time to remove should be a constant per attack, it incentivizes to use the attack as much as possible and grow the pot? ....like if i want to take down someone with 3 days tod, I just attack 10 times, etc.

        uint256 timeToDecrease = timeToRemove + attackTodDecrease[_attackId];

        //can't kil someone with attacks
        require(timeUntilDeath(_opponent) > timeToDecrease, "!kill");

        tokensToClaim[msg.sender] += tokens;

        // here if they have over 4 hours of tod we just remove timeToRemove
        // If they have less than 4 hours of tod we add 4 hours so attacks can do good.
        timeWenDed[_opponent] = timeUntilDeath(_opponent) > 4 hours
            ? timeWenDed[_opponent] - timeToDecrease // TODO it means timetodecrease can't be > 4 hours otherwise you can kill in one time
    ...
    function kill(uint256 _opponentId) public payable nonReentrant {
