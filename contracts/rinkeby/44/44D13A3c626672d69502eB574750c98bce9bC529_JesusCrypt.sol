 ...
 */
abstract contract ERC721Tradeable is
    ContextMixin,
    ERC721Enumerable,
...
/*
TESTING JESUSCRYPT - DO NOT USE IT
NFT Collection
Web: https://jesuscrypt.com
...
contract JesusCrypt is ERC721Tradeable {
    ...
    uint256 public offerPrice = 0.05 ether;
    uint256 public offerSupply = 5;
    ...
    function mint(uint256 count) public payable {
        ...
        uint256 priceToMint = getActualPrice();
        require(!paused(), "JesusCrypt: Minting is paused!");
        require(count > 0, "JesusCrypt: Must mint atleast 1 JesusCrypt!");
        require(
            count <= 20,
            "JesusCrypt: Can only mint 20 JesusCrypt at a time!"
        );
        require(
            supply + count < maxSupply - reserved,
            "JesusCrypt: Mint will exceeded supply!"
        );
        require(
            msg.value >= priceToMint * count,
        ...
        if (msg.value > priceToMint * count) {
            payable(msg.sender).transfer(msg.value - priceToMint * count);
        }
    }

    function giveAway(address to, uint256 count) external onlyOwner {
        require(count <= reserved, "JesusCrypt: Reserve is empty!");
