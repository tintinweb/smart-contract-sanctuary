...
//! TODO: Gas Efficient way to handle all data
...
contract OKG is Ownable, Pausable, Blacklistable, Claimable {
	...
	event OmegaKeyRegistered(address indexed Owner, uint256 indexed OmegaKey);
	...
	event OmegaKeyRestored(address[] indexed oldOwners, uint256 indexed OmegaKey);
	...
	modifier isUnregistered(uint256 _keyId) {
	 ...
	 */
	function register(uint256 _keyId) public
		whenNotGameOver()
		whenNotPaused()
		isReleased(_keyId)
		isUnregistered(_keyId)
	{
		IERC721 _token = IERC721(address(nftContract));
		require(_token.ownerOf(_keyId) == _msgSender(), "VOXO: You are not the owner of this KeyVoxo");

		// register Player as an owner of this Omega Key
		omegaKeys[_keyId].owners.push(_msgSender());
		// add Omega Key to a Player's Omega Key Collection
		if (players[_msgSender()].active) {
			players[_msgSender()].keyids.push(_keyId);
		} else {
	 ...
	 * @param eth_claim The ETH amount the Reward's winner will be awarded
	 */
	function claim(uint256 eth_claim) public whenNotGameOver() whenNotPaused() {
		require(players[_msgSender()].active, "VOXO: Player does not exist");
		require(((eth_claim == uint256(16)) || (eth_claim == uint256(33)) || (eth_claim == uint256(66)) || (eth_claim == uint256(135))), "VOXO: Reward for this ETH amount does not exist");

		// The Reward specifics
		uint256 eth_reward;
		uint256 no_keys_required;
		// Tier Bronze
		if (eth_claim == uint256(16)) {
			...
			eth_reward = 16 ether;
			no_keys_required = 3;
			removeOmegaKeys(no_keys_required);
			isBronzeRewardClaimed = true;
		}
		// Tier Silver
		if (eth_claim == uint256(33))  {
		...
		if (eth_claim == uint256(135)) {
			...
			eth_reward = 135 ether;
			no_keys_required = 8;
			removeOmegaKeys(no_keys_required);
			isDiamondRewardClaimed = true;
		}
		// Send the ETH Reward to the Player's address
		claimValues(address(0), _msgSender(), eth_reward);

		emit RewardClaimed(_msgSender(), no_keys_required, eth_reward);
	}
	...
	function removeOmegaKeys(uint256 _keys) private {
		for (uint i = 0; i < _keys; i++) {
			...
			omegaKeys[keyToRemove].owners = new address[](0);
			...
			for (uint j = 0; j < allOwners.length; j++) {
				...
				//! This logic is necessary to prevent eliminate keyIds from
				//! the array, when the array is empty. this occurs when a
				//! Player has registered an Omega Key, and this Key is removed
				//! for another Player to use for claim a reward.
	...
	/**
	 * @dev Method to revert a removed Omega Key
	...
	//!       Because is this occur after to the Winner Claim the Reward
	function restoreOmegaKey(uint256 _keyId) public
		onlyOwner()
		whenNotGameOver()
		whenNotPaused()
	{
		require ((omegaKeys[_keyId].removed && !omegaKeys[_keyId].active), "VOXO: Omega Key is not removed");
		//! TODO: I believe this currently does nothing,as the array of owners
		//        is always reset to zero items in removeOmegaKeys
		address[] memory oldOwner = omegaKeys[_keyId].owners;
		OmegaKey memory key = OmegaKey({
			active: true,
			removed: false,
			keyId: _keyId,
		...
		omegaKeys[_keyId] = key;
		emit OmegaKeyRestored(oldOwner, _keyId);
	}
	 ...
	 * @return True if at least one Reward is available, False if not
	 */
	 ...
	 */
	function isRegistered(uint256 _keyId) internal view isReleased(_keyId) returns (bool registered) {
		registered = false;
		for (uint i = 0; i < omegaKeys[_keyId].owners.length; i++) {
			if (omegaKeys[_keyId].owners[i] == _msgSender()) {
				registered = true;
			}
		}
		return registered;
