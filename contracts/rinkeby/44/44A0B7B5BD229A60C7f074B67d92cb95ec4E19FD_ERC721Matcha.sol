...
---------------------- [ WPSmartContracts.com ] ----------------------
    ...
    |  Flavors
    |
    |  >  Mochi: Fully featured ERC-721 Token
    |  >  Matcha: Buy, sell and auction ERC-721 Marketplace
    |
...
contract ERC721Mochi is ERC721, ERC721Enumerable, ERC721Metadata, ERC721Mintable, ERC721MetadataMintable {
     ...
     * @return A boolean that indicates if the operation was successful.
     */
    function autoMint(address to) public onlyMinter returns (bool) {
        do {
            autoTokenId++;
...
contract ERC721Matcha is ERC721Mochi, ReentrancyGuard {
    ...
    // Mapping from token ID to sell price in Ether or to bid price, according to _isAuction
    mapping(uint256 => uint256) public sellBidPrice;

    // Mapping from token ID to auction is opened
    mapping(uint256 => bool) public isAuction;

    // Mapping payment address for tokenId 
    mapping(uint256 => address payable) private _wallets;

    event Sale(uint256 tokenId, address from, address to, uint256 value);
    ...
    */
    ...
    constructor(address payable _admin, uint256 _commissionRate, string memory name, string memory symbol, bool _anyoneCanMint) public 
        ERC721Mochi(name, symbol, _anyoneCanMint) {
        admin = _admin;
    ...
    function sell(uint256 tokenId, uint256 price, address payable wallet) public {
        ...
        require(!isAuction[tokenId], "ERC721Matcha: Cannot sell an item which has an active auction");
        ...
        if (price>0) {
            ...
            address ga = getApproved(tokenId);
    ...
    function buy(uint256 tokenId) public payable nonReentrant {
        ...
        // is on sale
        require(!isAuction[tokenId] && sellBidPrice[tokenId]>0, "ERC721Matcha: The collectible is not for sale");
        ...
        // to owner
        _wallets[tokenId].transfer(amount4owner);

        // to admin
        admin.transfer(amount4admin);
