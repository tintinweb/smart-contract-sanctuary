 ...
 */
contract RidingRug is Context, Ownable, ERC721Enumerable {
    ...
    uint256 public constant MAX_NFT_SUPPLY = 895;
    ...
    // Token name
    string private _name = "Riding Rug NFT";

    // Token symbol
    string private _symbol = "RRNFT";

    // Name change token address
    address private _rETHAddress;

    //set number of nfts minted in at once
    uint256 private numberOfNfts = 1;
    /*
     ...
     */
    constructor (address rETHAddress) ERC721(_name, _symbol) {
        _rETHAddress = rETHAddress;
    ...
    function getNFTPrice() public view returns (uint256) {
        ...
        if (currentSupply >= 890) {
            return 100000000000000000000; // 890 - 895 100 ETH
        } else if (currentSupply >= 850) {
            return 3000000000000000000; // 850 - 889 3.0 ETH
        } else if (currentSupply >= 700) {
            return 1700000000000000000; // 700  - 849 1.7 ETH
        } else if (currentSupply >= 500) {
            return 900000000000000000; // 500 - 699 0.9 ETH
        } else if (currentSupply >= 300) {
            return 500000000000000000; // 300 - 499 0.5 ETH
        } else if (currentSupply >= 150) {
            return 300000000000000000; // 150 - 299 0.3 ETH
        } else {
            return 100000000000000000; // 0 - 149 0.1 ETH 
        }
    }
    ...
    */
     ...
     */
    // function _safeMint(address to, uint256 tokenId, bytes memory _data) internal  override virtual {
    //     _mint(to, tokenId);
    ...
    //     private override virtual returns (bool)
    // {
    //     if (!to.isContract()) {
    //         return true;
    //     }
    ...
    // Set numberOfNfts (nfts minted in at once) public function to be called by owner of the contract
    function setNumberOfNfts(uint256 _numberOfNfts) public onlyOwner {
