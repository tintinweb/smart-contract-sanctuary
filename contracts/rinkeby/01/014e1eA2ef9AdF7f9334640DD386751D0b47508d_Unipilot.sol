...
contract Unipilot is ERC721Permit, IUnipilot {
    ...
    address private constant PILOT = 0x6351ee103fAEdb80E1432D878026C45c47EB7d73;
...
contract UniswapLiquidityManager is PeripheryPayments, IUniswapLiquidityManager {
    ...
    {
        (address token0, address token1, uint24 fee, , , , ) = getPoolDetails(pair);
        (address tokenIn, address tokenOut, uint256 amountIn) = (token0 == WETH9)
            ? (token1, token0, amount)
            : (token0, token1, amount);
    ...
    function checkPoolLiquidity(
        ...
        if (wethToken0Pair != address(0) && wethToken1Pair != address(0)) {
            ...
            address wethUSDPair = checkPairsAndLiquidity(WETH9);
    ...
    function _distributeFeesInPilot(
        ...
        uint256 tokenAmount = getPilotAmountForTokens(
            _token0,
            _token1,
            _tokensOwed0,
            _tokensOwed1
    ...
    function distributeFeesForETH(DistributeFeesParams memory params) public override {
        ...
        (address tokenAlt, uint256 altAmount, address tokenWeth, uint256 wethAmount) = token0 == WETH
        ...
        if (params.pilotToken && ethTokenPair != address(0)) {
            uint256 tokenAmount = getPilotAmountWethPair(tokenAlt, altAmount, wethAmount);
...
abstract contract PeripheryPayments is IPeripheryPayments {
    ...
    address internal constant PILOT = 0x6351ee103fAEdb80E1432D878026C45c47EB7d73;
