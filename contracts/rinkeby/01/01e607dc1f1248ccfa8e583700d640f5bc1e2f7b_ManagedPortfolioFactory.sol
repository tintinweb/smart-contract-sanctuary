...
interface IProtocolConfig {
    function protocolFee() external view returns (BP);
...
interface IManagedPortfolio is IPortfolio {
    ...
    function managerFee() external view returns (BP);
...
abstract contract InitializableManageable is UUPSUpgradeable, Manageable, Initializable {
    ...
    function initialize(address _manager) internal initializer {
        require(!Address.isContract(address(this)), "InitializableManageable: Initialization must be performed during deployment");
        _setManager(_manager);
    }
...
import {ERC1967Proxy} from "ProxyWrapper.sol";
import {BP} from "BP.sol";
...
contract ManagedPortfolioFactory is InitializableManageable {
    ...
    function createPortfolio(
        ...
        IManagedPortfolio newPortfolio = IManagedPortfolio(address(new ERC1967Proxy(address(portfolioImplementation), initCalldata)));
