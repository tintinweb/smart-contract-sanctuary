...
contract SoccerStats is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getBalance(uint256 tokenId) public pure returns (string memory) {
        return pluck(tokenId, "Balance");
    }
    ...
    function tokenURI(uint256 tokenId) override public pure returns (string memory) {
        ...
        parts[9] = getBalance(tokenId);

        parts[10] = '</text><text x="10" y="120" class="base">';

        parts[11] = getTeamwork(tokenId);
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Sheet #', toString(tokenId), '", "description": "Soccer Stats are randomized soccer player stats generated and stored on chain. Feel free to use Soccer Stats in any way you want.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    constructor() ERC721("Soccer Stat", "SOCCERSTAT") Ownable() {}
}
