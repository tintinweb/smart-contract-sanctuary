...
contract NftRewards is Ownable {
    ...
    uint256 public totalTokens = 200000000000000000000;
    ...
    function getTokensEpoch() public view returns(uint256){
        ...
        if (now >= 1617740700) {
            return totalTokens.mul(55).div(100);
        }
        if (now >= 1617740400) {
    ...
    function addBalances(address[] calldata holders, uint256[] calldata amounts) external onlyOwner returns(uint256 count){
        count = holders.length;
        require(count == amounts.length, "Holders length and amounts length are not the same");
    ...
    function withdraw() external {
        require(IERC20(paper).balanceOf(address(this)) >= _balances[msg.sender], "Error: Not enough tokens");
