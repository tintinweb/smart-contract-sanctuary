...
contract RickRainbows is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721("Rick Rainbows", "RICK") {
        _owner = msg.sender;
        _baseTokenUri = "https://fuckbubbles.io/api/metadata/";
        _baseTokenExtension = '';
    }


    function mintRickRainbow(uint256 numberOfBots) public payable nonReentrant {
        require(totalSupply() < MAX_RICK_SUPPLY, "Sale has ended");
        require(numberOfBots > 0, "Cannot buy zero Rick Rainbows");
        
        require(
            (totalSupply() + numberOfBots) <= MAX_RICK_SUPPLY,
            "Exceeds MAX_RICK_SUPPLY"
        );
        require(
            (PRICE * numberOfBots) == msg.value,
            "ETH sent is not correct"
        );

        for (uint256 i = 0; i < numberOfBots; i++) {
