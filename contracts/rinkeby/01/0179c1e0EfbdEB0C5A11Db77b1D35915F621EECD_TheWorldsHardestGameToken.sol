...
contract TheWorldsHardestGameToken is ERC721, Ownable {
    ...
    constructor(string memory name_, string memory symbol_, uint256 price_, uint8 tokenAmountBuyLimit_, uint256 limitSupply_) ERC721(name_, symbol_) {
        _price = price_;
    ...
    function _preValidatePurchase() internal view {
        ...
        require(msg.value >= price(), "TWHGT: Insufficient funds");
        require(totalSupply() < limitSupply(), "TWHGT: All tokens sold");
    }
    ...
    function buyTokens() external payable returns (uint256[] memory) {
        ...
        uint256 limitAmount = limitSupply().sub(totalSupply());

        uint256 tokensAmount = amount < limitAmount ? amount : limitAmount;
        require(tokensAmount <= tokenAmountBuyLimit(), "TWHGT: Limited amount of tokens");
