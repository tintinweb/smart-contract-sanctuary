...
contract MultiStake is Ownable {
    ...
    // uint256 public stakedBalanceA;
    // uint256 public stakedBalanceB;
    ...
    function changeCap(address tokenAddress, uint256 amount) external onlyOwner {
        poolCap[tokenAddress] = amount;
    }
        ...
        )
    {
        if (hasStaked[user][tokenAddress]) {
            return (
                ...
                // deposits[user][tokenAddress].tokenAddress,
        ...
        returns (bool)
    {
        ...
        uint256 poolRemaining = poolCap[tokenAddress].sub(stakedTotalA);
    ...
    function _stake(
        ...
        if (token == tokenAddressA) {
            // stakedBalanceA = stakedBalanceA.add(amount);
            stakedTotalA = stakedTotalA.add(amount);
        } else {
            // stakedBalanceB = stakedBalanceB.add(amount);
        ...
        returns (bool)
    {
        ...
        // address token = deposits[from][token].tokenAddress;
        ...
        returns (uint256, uint256)
    {
        ...
        if (token == tokenAddressA) {
            ...
            uint256 test =
                payOut[tokenAddressB].mul(1000000).div(payOut[tokenAddressA]);
            rewardB = rewardA.mul(conversionAtoB).mul(test).div(10**12);
        } else {
            rewardB = amount.sub(amount1);
            uint256 test =
                payOut[tokenAddressA].mul(1000000).div(payOut[tokenAddressB]);
