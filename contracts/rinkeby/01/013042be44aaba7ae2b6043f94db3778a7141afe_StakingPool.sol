...
contract StakingPool is Context, Ownable, ReentrancyGuard {
    ...
    function getRewards(address account) external view returns (uint256) {
        if (block.timestamp > stakeTimes[account].add(maturityDays.mul(interval))) {
            return stakes[account].mul(poolApy).mul(maturityDays).div(36000);
        }
    ...
    */
        ...
        _realAddress(_msgSender())
    {
        ...
        if (block.timestamp > stakeTimes[staker].add(maturityDays.mul(interval))) {
            uint256 reward = amount.mul(poolApy).mul(maturityDays).div(36000);
