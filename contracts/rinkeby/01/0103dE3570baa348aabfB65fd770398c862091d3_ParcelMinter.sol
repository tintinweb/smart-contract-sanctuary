...
contract ParcelMinter is Ownable, ContextMixin {
    mapping(uint256 => address) public allocatedParcels;
    ...
    function allocateLands(uint256[] calldata tokenIds, address[] calldata addr) external onlyOwner {
        require(tokenIds.length == addr.length, "ParcelMinter: Wrong data format");
        for (uint256 i = 0; i < addr.length; i++) {
            allocatedParcels[tokenIds[i]] = addr[i];
        }
    }
    ...
    function redeemParcels(uint256 tokenId) public {
        ...
        require(allocatedParcels[tokenId] == minter, "ParcelMinter: Not allocated");
