...
contract PUGLIFE is Context, IERC20, IERC20Metadata, Ownable {
    ...
    constructor (IERC20 _legacyToken, address _marketing_address, address _charity_address) {
    ...
    function balanceOf(address account) public view virtual override returns (uint256) {
        if (_isExcluded[account]) {
            if (!migratedBalances[account]) return _tOwned[account] + legacyToken.balanceOf(account);
            return _tOwned[account];
        } else {
            if (!migratedBalances[account]) return tokenFromReflection(_rOwned[account] + (legacyToken.balanceOf(account) * _getRate()));
            return tokenFromReflection(_rOwned[account]);
        }
    }

    function oldBalance(address account) public view virtual returns (uint256) {
        return legacyToken.balanceOf(account);
    }

    function isMigratedBalances(address account) public view virtual returns (bool) {
