...
contract AuroraRoundTwo  is Ownable {
    ...
    constructor( address _tokenUsdt ,address _tokenAur,  address _addressReceive, uint _usdtDecimal, uint _aurDecimal) public {
		tokenUsdt = IERC20(_tokenUsdt); // Token USDT
		usdtDecimal = _usdtDecimal;
		aurDecimal = _aurDecimal;
	...
	function buyTokenAur(address _sponsor, uint _balance) public {
		...
		if (totalUsdt[sponsor] >= theLowestQuantityBuyToGetBonus) {
			uint bonus = _balance.mul(5).mul(uint(10**aurDecimal)).div(100).div(uint(price));
			tokenAur.mint(sponsor, bonus);
		}
		uint receiveAur = _balance.mul(uint(10**aurDecimal)).div(uint(price));
