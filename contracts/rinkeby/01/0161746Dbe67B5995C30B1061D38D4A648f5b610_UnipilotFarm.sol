...
contract UnipilotFarm is IUnipilotFarm, ReentrancyGuard, IERC721Receiver {
    ...
    function getGlobalReward(
        ...
        uint256 tvl = IUnipilotFarmV1(deprecated).poolInfo(pool).totalLockedLiquidity;
        uint256 temp = FullMath.mulDiv(rewardPerBlock, multiplier, 1e18);
        uint256 globalReward = FullMath
        ...
        )
    {
        ...
        if (userState.boosterActive) {
            uint256 _pilotReward = (userReward.mul(userState.liquidity)).div(1e18);
            uint256 boostedReward = (_pilotReward.mul(userState.booster)).div(1e18);
            pilotReward = _pilotReward.add((boostedReward));
        } else {
    ...
    function toggleBooster(uint256 tokenId) external {
        require(msg.sender == staking, "Only staking contract Call this");
        userInfo[tokenId].boosterActive = !userInfo[tokenId].boosterActive;
    }

    function updateUserBooster(uint256 _tokenId, uint256 _booster) external {
        require(msg.sender == staking, "Only staking contract Call this");
        require(userInfo[_tokenId].boosterActive, "Booster not active");
        emit UserBooster(_tokenId, userInfo[_tokenId].booster = _booster);
    }

    function setStake(address _staking) external onlyGovernance {
        emit Stake(staking, staking = _staking);
    }

    function getPoolInfo(address pool) external view override returns (uint256 tvl) {
        tvl = IUnipilotFarmV1(deprecated).poolInfo(pool).totalLockedLiquidity;
    }
}
...
interface IUnipilotFarm {
    ...
    function getPoolInfo(address pool) external view returns (uint256 tvl);
}
