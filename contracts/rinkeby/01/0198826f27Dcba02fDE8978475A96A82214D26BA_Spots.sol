...
contract Spots is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 spotIndex) internal override(ERC721, ERC721Enumerable) {
        super._beforeTokenTransfer(from, to, spotIndex);
    }

    function _burn(uint256 spotIndex) internal override(ERC721, ERC721URIStorage) {
    ...
    function _transfer(address from, address to, uint256 spotIndex) internal virtual override {
        super._transfer(from, to, spotIndex);
        unbondNFT(spotIndex);
    }
    
    function defaultURI(uint spotIndex) public view returns(string memory) {
        return concat(prefix, uint2str(spotIndex), ".json", "", "");  
    }
    
    
    function tokenURI(uint256 spotIndex) public view override(ERC721, ERC721URIStorage) returns (string memory) {
        if (!bonded(spotIndex)) {
    ...
    function _unbondNFT(uint spotIndex) private {
        require(bonded(spotIndex), "Spot already unbonded");
    ...
    function setOwnerLink(uint spotIndex, string memory link) public {
        require(ownerOf(spotIndex) == msg.sender, "Caller is not owner of spot");
    ...
    function setBondedNFTCreatorLink(uint spotIndex, string memory link) public {
        require(bondedTokenOwner(spotIndex) == msg.sender, "Caller is not owner of bonded asset");
    ...
    // Only owner (todo: removed onlyOwner to make testing simpler, add back in when finished)
    ...
    function setTokenURI(uint spotIndex, string memory uri) public { //onlyOwner {
        _setTokenURI(spotIndex, uri);
    }
    
    function setDefaultPrefixURI(string memory uri) public { //onlyOwner {
        prefix = uri;
    }
    
    // When bonded NFT's are transferred, they must be unbonded by force
    function forceUnbondNFT(uint spotIndex) public { //onlyOwner {
        _unbondNFT(spotIndex);
    }
    
    function forceUnbondNFTs(uint[] memory spotIndexes) public { //onlyOwner {
    ...
    //For overriding
    function setBondedNFTModelURI(uint spotIndex, string memory uri) public { //onlyOwner
        require(bonded(spotIndex), "Spot not bonded");
    ...
    function bonded(uint spotIndex) public view returns(bool) {
        return _bondedSpots[spotIndex];
    }
    
    function bondedNFTContractAddress(uint spotIndex) public view returns(address) {
        require(bonded(spotIndex), "Spot not bonded");
        return _bondedNFTContractAddresses[spotIndex];
    }
    
    function bondedNFTTokenId(uint spotIndex) public view returns(uint) {
        require(bonded(spotIndex), "Spot not bonded");
    ...
    function bondedTokenIndices() public view returns(uint[] memory) {
    ...
    function bondedTokenAddresses() public view returns(address[] memory)
    {
    ...
    function anyOwnersChanged() public view returns(bool) {
        uint[] memory indices = bondedTokenIndices();
        ...
        for (uint i = 0; i < indices.length; i++) {
            ...
            if (bondedTokenOwnerChanged(nftContract, nftIndex, spotIndex))
            {
                return true;
            }
        }
    ...
    function numberBondedTokensChanged() public view returns(uint) {
        uint[] memory indices = bondedTokenIndices();
        uint changed = 0;
    ...
    function changedBondedTokensIndices() public view returns(uint[] memory) {
        uint[] memory indices = bondedTokenIndices();
    ...
    function bondedTokenOwnerChanged(address nftContract, uint nftIndex, uint spotIndex) public view returns(bool) {
        require(bonded(spotIndex), "Spot index not bonded");
        return IERC721(nftContract).ownerOf(nftIndex) != ownerOf(spotIndex);
    }
    
    function bondedNFTMetadataURI(uint spotIndex) public view returns (string memory) {
        require(bonded(spotIndex), "Token not bonded");
        address nftContract = _bondedNFTContractAddresses[spotIndex];
        uint nftIndex = _bondedNFTTokenIds[spotIndex];
    ...
    function bondedTokenOwner(uint spotIndex) public view returns(address) {
        require(bonded(spotIndex), "Token not bonded");
        address nftContract = _bondedNFTContractAddresses[spotIndex];
        uint nftIndex = _bondedNFTTokenIds[spotIndex];

        return genericNFTOwner(nftContract, nftIndex);
    }
    ...
    function genericNFTOwner(address nftContract, uint tokenIndex) public view returns(address) {
         return IERC721(nftContract).ownerOf(tokenIndex);
    }
    
    //Lookup token URI for any NFT
    function genericTokenURI(address nftContract, uint tokenIndex) public view returns(string memory) {
