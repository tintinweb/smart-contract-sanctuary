...
contract DAOPrivate is Ownable, ReentrancyGuard, SignatureVerifyer {
    ...
    function createEpoch(uint256 round, bool shift) public {
      if(shift){
        currentEpoch[round]++;
      }
      uint256 curEpochNum = currentEpoch[round];
      Epoch memory epoch;
      roundEpochs[round].push(epoch);
      // roundEpochs[round][curEpochNum] = Epoch({
      //   fullShare: 0,
      //   users: new uint[](0)
      // });
    }
    ...
    function getEpochInfo(uint256 round, uint256 epoch) public view returns (uint256[] memory epochUsers, uint256 fullShare){
      return (
