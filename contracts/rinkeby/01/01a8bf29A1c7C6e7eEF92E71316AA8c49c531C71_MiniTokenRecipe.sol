...
contract MiniTokenRecipe {
    ...
    */
        ...
        returns (address)
    {
        ...
        IJellyContract(mini_token).initContract(abi.encode(_name, _symbol, _tokenAdmin, _initialTokenDestination, _initialSupply, _supplyCap)); 

        emit MiniTokenDeployed(mini_token, _tokenAdmin, _supplyCap);
        return mini_token;
    }

    /// @notice allows for the recovery of ETH sent to contract
    function wrapEth()
        external
    {
        require(
            accessControls.hasAdminRole(msg.sender),
            "wrapEth: Sender must be admin"
        );
        // Deposit the contract's total ETH balance.
