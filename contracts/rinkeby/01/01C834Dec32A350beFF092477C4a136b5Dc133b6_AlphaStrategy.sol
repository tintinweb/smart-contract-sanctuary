...
contract AlphaStrategy is Pausable, AlphaToken {
    ...
    uint256 public TOTAL_ALPHA_SUPPLY = 1;
    ...
    modifier canCancel() {
        require(CAN_CANCEL == true, "Alpha: Cancel feature is not available");
        _;
    }
    ...
    // deposits
    function finalizeDeposits(address[] calldata _users, uint256 _timestamp) external onlyManager {
        ...
        for (uint256 i = 0; i < _users.length; i++) {
            ...
            _depositAlpha = ((_amountStable - _feeStable) * amountScaleDecimals * COEFF_SCALE_DECIMALS) / ALPHA_PRICE;
        ...
        _stableToken.transfer(MANAGER, _feeStableTotal);
       
    }

    // withdrawals
    function finalizeWithdrawals(address[] calldata _users, uint256 _timestamp) external onlyManager {
        require(_users.length > 0, "Alpha: no users provided ");
        ...
        for (uint256 i = 0; i < _users.length; i++) {
            ...
            _amountStable = (_amountLP *  ALPHA_PRICE) / (COEFF_SCALE_DECIMALS * amountScaleDecimals);

            _stableToken.transfer(_users[i], _amountStable);
    ...
    function cancelDepositRequest() external whenNotPaused canCancel {
        ...
        _stableToken.transfer(msg.sender, _amount);
