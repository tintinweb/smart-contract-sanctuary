    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(address => bool) override public whitelistedFeeTokens;
    ...
    event RemoveTokenFromFeeWhitelist(
        address indexed admin,
    ...
    {
        MsgReceiver(payable(_persona)).updateRelayer(_relayer);
    ...
    /// @param _token token address
    function removeFeeToken(address _token)
        external
        onlyOwner
        validAddress(_token)
    {
...
interface IMsgReceiverFactory {
    function retrievePersona(address _user) external returns (address);
...
contract MsgReceiver is Ownable {
    ...
    mapping(bytes32 => bool) forwarded;
    ...
    constructor(address _user, address _relayer, address _msgReceiverFactory)
        validAddress(_user)
        validAddress(_relayer)
    ...
    /// @notice foward a function call to another smart contract
    ...
    /// @notice foward a function call to another smart contract, approve
    ...
    function saveEth(address _receiver, uint256 _amount)
        external
        onlyUser(msg.sender)
        ...
        validAddress(_receiver)
    {
        ...
        IERC721(_nftContract).transferFrom(address(this), _receiver, _nftId);
    ...
    {
        IERC20(_token).safeIncreaseAllowance(_target, _amount);
    }

    /// @notice updates the relayer address
    /// @param _relayer new relayer address
    function updateRelayer(address _relayer)
        external
        validAddress(_relayer)
        onlyOwner
    {
    ...
    modifier onlyRelayer(address _addr) {
        require(_addr == relayer, "Unauthorized - not the relayer");
        _;
    }
