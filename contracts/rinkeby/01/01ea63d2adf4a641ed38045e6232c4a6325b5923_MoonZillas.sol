...
contract MoonZillas is ERC721Enumerable, Ownable {
  ...
  uint256 private nftPerDevTeamLimit = 50; // Max NFT per each dev team
  uint256 public reserved = 200; // Max NFT available to promote project
  uint public publicSale = 1640707200; // December 28th 12PM EST
  bool public paused = false;
  address[] public whitelistedAddresses = [
      0x159848B6b4aC7f87Fdd7B988605e25A59a479D54  // Designer Address
  ];
  ...
  */
  function mint(uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner() && !isWhitelisted(msg.sender)) {
      ...
      require(supply + _mintAmount <= maxSupply && reserved >= _mintAmount); // Prevent minting +10.000
      require(ownerTokenCount <= nftPerDevTeamLimit, "Sale exceed maximum for staff wallet"); // Fair minting
    }
  ...
  /**
  * Mint by owner for promotional purposes
  */
  function mintPromotional(address _to, uint256 _numberOfTokens) external onlyOwner {
  ...
  /**
  * Check whitelisted users for free minting
  */
