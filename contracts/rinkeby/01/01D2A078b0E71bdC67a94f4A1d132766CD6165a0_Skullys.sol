...
contract Skullys is ERC721, ReentrancyGuard, Ownable {
    ...
    uint256 public constant PRESALE_LIMIT_PER_ADDRESS = 1;
    ...
    uint256 public constant RESERVED_SUPPLY = 15;
    ...
    function mint(uint256 numberOfSkullys) public payable nonReentrant {
        ...
        if (mintPassRequired || block.timestamp <= SALE_START_TIMESTAMP) {
            ...
            if(earlyMintPass.balanceOf(_msgSender()) == 0) {
                ...
                updateMintCount(_msgSender(), PRESALE_LIMIT_PER_ADDRESS);

                /** Each regular mint pass holder gets 1 Skully during pre-sale **/
                ...
                /** Each early mint pass holder gets 2 Skullys **/
    ...
    /**
     * Allows the 5 founders to mint their custom, 1/1 skullys first
     **/
    ...
    /**
     * Reset total supply so that we can re-use the same contract for future drops
     * This function ensures that the counter has reached the TOTAL_SUPPLY first to prevent us
     * from cutting off supply of an active drop
     **/
    function resetTotalSupply() external onlyOwner {
        require(
            totalSupply() == TOTAL_SUPPLY,
            "Current collection not sold out yet"
        );
