...
contract xRooStaking {
    ...
    function _claimContractNFTXRewards() internal {
        uint256 currentRewards = NFTXRewardToken.balanceOf(address(this));
        uint256 dividend = NFTXRewardDistributionToken.dividendOf(
            address(this)
        );
        if (dividend == 0) return;

        NFTXLPStaking.claimRewards(vaultId);
        require(
            NFTXRewardToken.balanceOf(address(this)) ==
        ...
        )
    {
        ...
        uint256 initialRewardToken = NFTXRewardToken.balanceOf(address(this));
        ...
        uint256 newTokens = NFTXRewardToken.balanceOf(address(this)) -
            initialRewardToken;
        ...
        )
    {
        ...
        NFTXLPStaking.deposit(vaultId, liquidity); // DEPOSIT IN NFTX
        ...
        if (amountToken < _amount) {
            NFTXRewardToken.transfer(_user, _amount - amountToken);
        }
    ...
    function _claimRewards(address _user) internal {
        ...
        if (rewards.NFTXRewardToken > 0) {
            ...
            NFTXRewardToken.transfer(_user, rewards.NFTXRewardToken);
        }
    ...
    function _removeLiquidity(
        ...
        NFTXLPStaking.withdraw(vaultId, amount); // gives us <amount> SLP
