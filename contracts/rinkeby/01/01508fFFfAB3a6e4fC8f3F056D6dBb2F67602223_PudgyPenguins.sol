...
contract PudgyPenguins is ERC721Burnable, ERC721Pausable, RandomlyAssigned, WithSaleStart, WithContractMetaData, WithIPFSMetaData {
    using SafeMath for uint256;

    uint256 public constant PRICE = 0.075 ether; //3 * 10**16;
    uint256 public constant MAX_BY_MINT_PUBLIC = 3;
    uint256 public constant MAX_BY_MINT_PRIVATE = 3; // max one ticket per wallet in private sale
    address[] public creatorAddresses = [0x47fF8E0b9447C92C7df9051a009fA32EFd69A8A9, 0xEc251619367c0e897417D761B7c18BFeF568EC74];
    Ticket public privateSaleTicket;
    uint8 public constant TICKET_ID = 0;
    mapping(address => uint256) public amountMintedPrivate; // to restrict wallets with multiple tickets to mint more than x NFTs
    mapping(address => uint256) public amountMintedPublic; // to restrict wallets with multiple tickets to mint more than x NFTs


    event CreatePenguin(uint256 indexed id);
    constructor(Ticket ticket, uint256 maxElements, uint256 publicSaleStartTime, string memory contractUri, string memory cid)
    ERC721("PudgyPenguins", "PPG")
    WithSaleStart(publicSaleStartTime)
    WithContractMetaData(contractUri)
    WithIPFSMetaData(cid)
    RandomlyAssigned(maxElements, 0) // Max. 1k NFTs available; Start counting from 0
    {
        privateSaleTicket = ticket;
        pause(true);
    }
    ...
    function mint(address _to, uint256 _count) public payable saleIsOpen {
        ...
        require(total <= totalSupply(), "Sale end");
        bool isPublicSale = saleStarted();
        if (isPublicSale) {
            amountMintedPublic[_msgSender()] = amountMintedPublic[_msgSender()].add(_count);
        } else {
            amountMintedPrivate[_msgSender()] = amountMintedPrivate[_msgSender()].add(_count);
        }


        require((!isPublicSale && amountMintedPrivate[_msgSender()] <= MAX_BY_MINT_PRIVATE) || (isPublicSale && amountMintedPublic[_msgSender()] <= MAX_BY_MINT_PUBLIC), "Exceeds max mint allowance");
        require(msg.value >= price(_count), "Value below price");

        uint256 requiredTicketAmount = _count.div(MAX_BY_MINT_PRIVATE);
        if (_count.mod(MAX_BY_MINT_PRIVATE) > 0) {
            requiredTicketAmount = requiredTicketAmount.add(1);
            //TODO: review & refactor???
        }

        // e.g. 3 to mint, 3 mintables per ticket
        require(isPublicSale || privateSaleTicket.balanceOf(_msgSender(), TICKET_ID) >= requiredTicketAmount, "Not enough tickets");

        for (uint256 i = 0; i < _count; i++) {
            _mintAnElement(_to);
        }
        if (!isPublicSale) {
            // TODO: Add approve in DApp for people in private sale! --> setApproveAll
            privateSaleTicket.burn(_msgSender(), TICKET_ID, requiredTicketAmount);
        }
    }
    ...
    //TODO, remove this function after testing
    //TODO, remove this function after testing
    //function mintAsOwner(address _to, uint256 _count) public onlyOwner { //TODO, remove this function after testing
    //    for (uint256 i = 0; i < _count; i++) {
    //        _mintAnElement(_to);
    //    }
    //}
    ...
    function withdrawMintProfits() external onlyOwner {
        uint256 balance = address(this).balance;
        require(balance > 0);

        uint creatorAddressesCount = creatorAddresses.length;
        for (uint i = 0; i < creatorAddresses.length; i++) {
            _widthdraw(creatorAddresses[i], balance.div(creatorAddressesCount));
        }
    }
...
abstract contract WithIPFSMetaData is ERC721, Ownable {
    ...
    event MetadataRevealed(string cid);
    event MetadataFinalized(string cid);
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked(
                _baseURI(), "/", tokenId.toString(), ".json" // "/metadata.json"
            ));
    }
    ...
    // Needed so that we can host temporary metadata for the unrevealed NFTs so that you see more than blank NFTs on OpenSea
    function revealNFTs(string memory _cid) external onlyOwner {
        require(!isFinalized, "Already finalized"); // NFT metadata cannot be changed after they are finalized
        _setCID(_cid);
        emit MetadataRevealed(_cid);
    }

    // Needed so that we can host temporary metadata for the unrevealed NFTs so that you see more than blank NFTs on OpenSea
    function freezeMetadata() external onlyOwner {
        require(!isFinalized, "Already finalized"); // NFT metadata cannot be changed after they are finalized
        isFinalized = true; // this function can only be called once
        emit MetadataFinalized("");
    }
}
...
contract Ticket is ERC1155Burnable, Ownable {
    using SafeMath for uint256;

    uint256 public constant TICKET = 0;
    address[] public whitelist; // to restrict wallets with multiple tickets to mint more than x NFTs


    constructor(string memory baseURI, uint256 amount) ERC1155(baseURI) {
        _mint(_msgSender(), TICKET, amount, "");
    }

    function hasTickets(address _wallet) external view returns (bool) {
        return 0 < balanceOf(_wallet, TICKET);
    }
    ...
    function batchTransferToMany(address[] memory _beneficiaries, uint8 _amount) external {
        require(_beneficiaries.length.mul(_amount) <= balanceOf(_msgSender(), TICKET), "Not enough tickets");

        for (uint256 i = 0; i < _beneficiaries.length; i++) {
            safeTransferFrom(_msgSender(), _beneficiaries[i], TICKET, _amount, "");
        }
    }

    function batchMintToMany(address[] memory _beneficiaries, uint8 _amount) external onlyOwner {
        //require(_beneficiaries.length.mul(_amount) <= balanceOf(_msgSender(), TICKET), "Not enough tickets");
        for (uint256 i = 0; i < _beneficiaries.length; i++) {
            _mint(_beneficiaries[i], TICKET, _amount, "");
            //safeTransferFrom(_msgSender(), _beneficiaries[i], TICKET, _amount, "");
        }
    }

    function editWhitelist(address[] memory _beneficiaries) external onlyOwner {
        whitelist = _beneficiaries;
    }
}
