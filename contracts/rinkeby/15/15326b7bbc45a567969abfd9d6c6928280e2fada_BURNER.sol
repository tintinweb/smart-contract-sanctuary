...
contract BURNER is Context, AccessControlEnumerable, ERC721Enumerable, ERC721Burnable, ERC721Pausable {
	...
	function _getTier(uint256 _tokenId) private pure returns (uint256) {
		if(_tokenId % 6 == 0) {
			return 3;
		}
	...
	function _getGasTier(uint gasCost) private view returns (uint8) {
		uint8 tier = 0;
		for(uint8 i = 1; i <= 8; i++) {
	...
	function getRenderData(uint16 tokenIndex, uint tokenTier, uint prevGas, string memory prevDay, string memory currentDay, string memory prevSetup) public view returns (uint, bytes memory, string memory) {

		require(_exists(tokenIndex), "Not minted yet!");
		require(tokenTier >= 0, "Tier out of bounds!");
		require(tokenTier < 4, "Tier out of bounds!");

		uint currentGas = block.basefee;
		bytes memory currentSetup;
		bytes32 seed = keccak256(abi.encodePacked(blockhash(block.number - 1), tokenIndex));
		bool boolReset = false;
		if(keccak256(abi.encode(prevDay)) != keccak256(abi.encode(currentDay))) {
			boolReset = true;
		}

		for(uint16 i = 1; i <= 7; i++) {
			if(_getGasTier(currentGas) == i) {
				if(bytes(prevSetup)[i] == 0) {
					currentSetup[i - 1] = seed[i - 1];
				}
				else {
					if(_getGasTier(currentGas) < _getGasTier(prevGas)) {
						currentSetup[i - 1] = 0;
					}
					else {
						if(boolReset) {
							...
							currentSetup[i - 1] = bytes(prevSetup)[i - 1];
						}
					}
				}
			}
		...
		for(uint16 i = 8; i <= 9; i++) {
			if(tokenTier == 1) {
				currentSetup[i - 1] = 0;
			}
			else if(tokenTier == 2 && i == 8) {
				if(_getGasTier(prevGas) != _getGasTier(currentGas)) {
					currentSetup[i - 1] = seed[i - 1];
				}
				else {
					currentSetup[i - 1] = 0;
		...
		return(currentGas, currentSetup, _imagesPath);
	}
