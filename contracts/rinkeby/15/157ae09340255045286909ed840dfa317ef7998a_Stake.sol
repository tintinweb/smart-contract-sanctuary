...
contract Stake is Owned {
   ...
   function FIRSTSTAKE(address refferal, uint256 tokens)  external{
    ...
    function CalculateAmountFreeze(address user,uint256 tokens) public view returns (uint256)
    {
        ...
        //look at the need for this later on
       
        owing =  (percal.mul(daysLockCount)).add(owing);
    ...
    function LOCKWITHDRAW(uint256 tokens) external {
        ...
        uint256 owing = 0;
        uint256 _UntakingFee = (onePercent(tokens).mul(UnstakingFee)).div(10);
        msg.sender.transfer(tokens.sub(_UntakingFee));
        
        BFTFee = BFTFee.add(_UntakingFee);
        owing = CalculateAmountFreeze(msg.sender,stakers[msg.sender].lockTokens);
        if(owing > 0)
        {
          stakers[msg.sender].freezeReward = (stakers[msg.sender].freezeReward).add(owing);   
        }
