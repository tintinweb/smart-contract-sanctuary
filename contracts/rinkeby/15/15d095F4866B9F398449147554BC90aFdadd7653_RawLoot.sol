...
contract RawLoot is ERC721Enumerable, ReentrancyGuard, Ownable {
  ...
  function publicMint(uint256 tokenId) external payable nonReentrant {
    ...
    require(tokenId >= 75000 && tokenId < block.number.div(75), "Token Id must between 50000 and the latest block divided by 100");
  ...
  function mintWithLoot(uint256 lootId, uint256 tokenId) external nonReentrant {
    require(lootContract.ownerOf(lootId) == msg.sender || temporalLootContract.ownerOf(tokenId) == msg.sender || SVBLootContract.ownerOf(tokenId) == msg.sender, "You must own Loot or TemporalLoot");

    require(tokenId >= 0 && tokenId < 75000, "Token Id must between 0 and 100000");
        ...
        bytes(
          string(
            abi.encodePacked(
              '{"name": "RawLoot #',
              _toString(tokenId),
              '", "description": "RawLoot: Collect the right combination of materials to creat ForgedLoot", "image": "data:image/svg+xml;base64,',
  ...
  constructor() ERC721("RawLoot", "RWLOOT") Ownable() {}
}
