...
contract PunkstersV2 is ERC721, Ownable {
    ...
    function mint(uint256 tokenId0, uint256 tokenId1, address otherToken) external payable{
        ...
        bool free = canMintForFree(tokenId0);
        if (free) {
            freeMintPerAddress[msg.sender]++;
            freeMintPerId[tokenId0]++;
        } else {
            require(msg.value == mintingPrice, "not enough sent");
        }
    ...
    function canMintForFree(uint256 tokenId0) public view returns(bool){
        if(freeMintPerAddress[msg.sender] == 4) return false;
        return tokenId0 > 9999 ? freeMintPerId[tokenId0] < 1 : freeMintPerId[tokenId0] < 4;
    }
}
