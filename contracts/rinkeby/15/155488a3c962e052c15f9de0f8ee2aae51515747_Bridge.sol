...
contract Bridge{
    ...
    function OrdersCount() external view returns(uint256 SendOrdersCount, uint256 ReceiveOrdersCount, uint256 RequestTokenOrdersCount, uint256 AddTokenOrdersCount){
    ...
    function sendOrderData(uint256 startingIndex, uint256 finalIndex) external view returns(address[] memory receiver, uint256[] memory amount, uint256[] memory fee, uint256[] memory claimingFee, address[] memory tokenAddressFrom){
        uint256 space = finalIndex-startingIndex;
        receiver = new address[](space);
        amount = new uint256[](space);
        fee = new uint256[](space);
        claimingFee = new uint256[](space);
        tokenAddressFrom = new address[](space);
        
        uint256 index;
        for(uint256 t = startingIndex; t < finalIndex; ++t){
            receiver[index] = _sendreceiver[t];
            amount[index] = _sendAmount[t];
            fee[index] = _sendFee[t];
            claimingFee[index] = _sendClaimingFee[t];
            tokenAddressFrom[index] = _sendTokenAddressFrom[t];
            ++index;
        }
    }
    
    function sendOrderSender(uint256 startingIndex, uint256 finalIndex) external view returns(address[] memory sender){
        uint256 space = finalIndex-startingIndex;
        sender = new address[](space);
        
        uint256 index = 0;
        for(uint256 t = startingIndex; t < finalIndex; ++t){
            sender[index] = _sendSender[t];
            ++index;
        }
    }
    
    function addTokenData(uint256 startingIndex, uint256 finalIndex) external view returns(address[] memory owner, address[] memory main, address[] memory parallel, uint256[] memory fee){
        uint256 space = finalIndex-startingIndex;
        owner = new address[](space);
        main = new address[](space);
        parallel = new address[](space);
        fee = new uint256[](space);
        
        uint256 index;
        for(uint256 t = startingIndex; t < finalIndex; ++t){
            owner[index] = _owner[t];
            main[index] = _addTokenMainContract[t];
            parallel[index] = _addTokenParallelContract[t];
            fee[index] = _addTokenFee[t];
            ++index;
        }
    }
    ...
    function addTokenProcessed(address owner, address main, address parallel, uint256 fee) external view returns(bool signed){
        return _addTokenProcessed[_addTokenIndex[owner][main][parallel][fee]];
    }
