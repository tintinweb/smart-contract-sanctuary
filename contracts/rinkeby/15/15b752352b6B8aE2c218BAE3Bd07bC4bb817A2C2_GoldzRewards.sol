...
contract GoldzRewards is EIP712, Ownable {
    ...
    mapping(address => uint) accountToNonce;
    ...
    function claim(uint amount, uint nonce, bytes calldata signature) external {
        require(accountToNonce[msg.sender] == nonce, "invalid nonce lol");
        require(_signerAddress == recoverAddress(msg.sender, amount, nonce, signature), "invalid signature");

        goldz.transfer(msg.sender, amount);

        accountToNonce[msg.sender]++;

        emit Withdraw(amount, nonce, msg.sender);
    }
     ...
     function _hash(address account, uint amount, uint nonce) internal view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256("Withdraw(uint256 amount,address account,uint nonce)"),
                        amount,
                        account,
                        nonce
                    )
