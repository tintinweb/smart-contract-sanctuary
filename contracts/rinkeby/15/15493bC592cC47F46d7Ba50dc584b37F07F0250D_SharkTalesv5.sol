...
contract SharkTalesv5 is Context, IERC20, Ownable {
    ...
    string private _name = "Shark Tales v5";
    string private _symbol = "SHv5";
    ...
    function initContract() external onlyOwner() {
        ...
        _teamDevAddress = payable(0xCC05ff09a65FC9Fde89Ddbd40D47d06b5fbc9310);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (overMinTokenBalance && !currentlySwapping && from != uniswapV2Pair) {
    ...
    function swapAndRedirectEthFees(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalRedirectTokenFee = _devFee.add(_liquidityFee); // 5
        uint256 devTokenBalance = contractTokenBalance.mul(_devFee).div(totalRedirectTokenFee); // 1000 * 5  / 5 == 1000
        
        // split the liquidity balance into halves/ 0
        ...
        // swap tokens for ETH
        swapTokensForEth(devTokenBalance);
        
        uint256 newBalance = address(this).balance.sub(initialBalance);
        
        if(newBalance > 0) {
            // rebalance ETH fees proportionally to half the liquidity
            uint256 totalRedirectEthFee = _devFee; /// 2.5
            ...
            emit OnSwapAndRedirectEthFees(contractTokenBalance, devEthBalance);
        }
    }
