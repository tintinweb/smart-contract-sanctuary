...
contract Planeto is ERC721, Pausable, PaymentSplitter, Ownable {
    ...
    constructor(address[] memory payees, uint256[] memory shares_) ERC721("Planeto", "PLA") PaymentSplitter(payees, shares_) {}
    ...
    function mint(uint256 num) public payable whenNotPaused {
        ...
        require(num > 0 && num < 11, "Amount not valid" );
        require(supply + num < _supplyLimit - _reserved, "Exceeds Planet supply" );
    ...
    function betaMint(bytes32[] calldata proof) public whenNotPaused {
        address account = msg.sender;

        require(MerkleProof.verify(proof, _root, _leaf(account)), "Invalid proof");
        require(balanceOf(account) == 0, "Minted before");

        _tokenSupply.increment();
        _safeMint(account, _tokenSupply.current());
    }
    ...
    function merge(uint256 tokenIdA, uint256 tokenIdB) public {
        require(ownerOf(tokenIdA) == _msgSender() , "Orbito: not owner of tokenIdA");
        require(ownerOf(tokenIdB) == _msgSender() , "Orbito: not owner of tokenIdB");
