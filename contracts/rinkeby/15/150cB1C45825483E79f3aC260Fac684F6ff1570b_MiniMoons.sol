...
contract MiniMoons is Context, ERC1155Pausable, Ownable, VRFConsumerBase {
    ...
    uint256[] internal weights = [10, 10, 10, 10, 10, 10, 10, 10, 10, 10];

    mapping(bytes32 => address) public requestIdToSender;
    mapping(uint256 => uint256) public tokenSupply; // Supply of tokens
    ...
    constructor(
        ...
        address _VRFCoordinator,
        bytes32 _keyhash
    ) ERC1155(_uri) VRFConsumerBase(_VRFCoordinator, _chainLinkTokenAddress) {
    ...
    // Events
    event TokenMinted(address indexed to, uint256 id);
    event BatchMinted(address indexed to, uint256[] ids);
    event TokenRequested(bytes32 requestId);
    ...
    function mint() public payable {
        ...
        drawToken(_msgSender());
    }

    // Find a new Moon from the rarity index
    function drawToken(address _requester) internal {
        require(
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber)
        ...
        override
    {
        // Get Token Owner from requestIndex
        address tokenOwner = requestIdToSender[requestId];

        // Pluck tokenId from weights using true random Number
        uint256 tokenId = pluck(randomNumber % BASIS_POINTS);

        // Mint token
        super._mint(tokenOwner, tokenId, 1, _msgData());

        // Emit Minted event
        emit TokenMinted(tokenOwner, tokenId);

        // Add supply to specific tokenSupply index
        tokenSupply[tokenId] = tokenSupply[tokenId] + 1;

        // Add count to totalSupply
        totalMints = totalMints + 1;
    }

    // Loop through and get weighted index as the tokenID
    function pluck(uint256 target) internal virtual returns (uint256) {
     ...
     * @param _uri New URL of token's URI
     */
