...
contract RedBerryToken is BEP20 {
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(
            _burnRate <= 100,
            "REDB::updateBurnRate: Burn rate must not exceed the maximum rate."
        );
...
// Berries with Governance.
contract Berries is BEP20("Berries Token", "BERRY") {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    ...
    // The RED BERRY TOKEN!
    RedBerryToken public redberrys;

    constructor(RedBerryToken _redberrys) {
        redberrys = _redberrys;
    }
    ...
    function safeRedBerryTransfer(address _to, uint256 _amount)
        ...
        onlyOwner
    {
        uint256 redberrysBal = redberrys.balanceOf(address(this));
        if (_amount > redberrysBal) {
            redberrys.transfer(_to, redberrysBal);
        } else {
            redberrys.transfer(_to, _amount);
        }
    }
...
contract MasterChef is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 redBerryReward = multiplier
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        redberrys.mint(devaddr, redBerryReward.div(10));
        redberrys.mint(address(berry), redBerryReward);
    ...
    function safeRedBerryTransfer(address _to, uint256 _amount) internal {
        uint256 RedBerryBal = berry.balanceOf(address(this));
        if (_amount > RedBerryBal) {
            berry.transfer(_to, RedBerryBal);
        } else {
