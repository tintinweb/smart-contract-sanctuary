    ...
    AccessControlUpgradeable
{
    ...
    mapping(address => mapping(bytes32 => uint256))
        public tierTotalClaimableToken;
    mapping(address => mapping(bytes32 => mapping(uint256 => uint256)))
        public tierTotalClaimableChapter;
    ...
    function setClaimableRole(
        address[] memory claimableAddress,
        uint256[] memory _role,
        uint256[] memory _chapter,
        uint256[] memory _tokenAmount
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            (claimableAddress.length == _tokenAmount.length),
            "Invalid Data"
        );
        for (uint256 i = 0; i < claimableAddress.length; i++) {
            ...
            if (tiers[_role[i]] == TIER3_ROLE) {
                tierTotalClaimableToken[claimableAddress[i]][
                    tiers[_role[i]]
                ] = tierTotalClaimableToken[claimableAddress[i]][
                    tiers[_role[i]]
                ].add(_tokenAmount[i]);
                if (
                    tierTotalClaimableChapter[claimableAddress[i]][
                        tiers[_role[i]]
                    ][_chapter[i]] > 0
                ) {
                    tierTotalClaimableToken[claimableAddress[i]][
    ...
    function claimAllNFT() external whenNotPaused nonReentrant {
        require(
            teamNFTStatus != false,
            "Can not claim before team NFT distribution"
        );
        ...
        for (uint256 i = 0; i < tiers.length; i++) {
            if (
                tiers[i] == TIER3_ROLE &&
                tierTotalClaimableToken[msg.sender][tiers[i]] > 0
            ) {
                for (uint256 j = 0; j < totalChapters; j++) {
                    uint256 _chapterMintCount = tierTotalClaimableChapter[
                        msg.sender
                    ][tiers[i]][j];
                    if (_chapterMintCount > 0) {
                        uint256 _chapterCurrentIndex = tierTotalSupply[
                            tiers[i.sub(1)]
                        ].add(uint256(100).div(totalChapters).mul(j)).add(
                                chapterMintCount[tiers[i]][j]
                            );
                        uint256 _chapterMaxLimit = tierTotalSupply[
                            tiers[i.sub(1)]
                        ].sub(1).add(
                                uint256(100).div(totalChapters).mul(j.add(1))
                            );
                        if (j < totalChapters.sub(2)) {
                            require(
                                chapterMintCount[tiers[i]][j] <= 7 &&
                                    _chapterCurrentIndex.add(
                                        _chapterMintCount.sub(1)
                                    ) <=
                                    _chapterMaxLimit,
                                "Chapter NFT already claimed"
                            );
                        } else {
                            if (j == 13) {
                                _chapterCurrentIndex = _chapterCurrentIndex.add(
                                        1
                                    );
                        ...
                        _nftAmount = _claimNFT(
                            _msgSender(),
                            tiers[i],
                            _chapterCurrentIndex,
                            _chapterMintCount,
                            j
                        );
                    }
                ...
                if (tierTotalClaimableToken[msg.sender][tiers[i]] > 0) {
                    _nftAmount = _claimNFT(
                        _msgSender(),
                        tiers[i],
                        tierTotalClaimed[tiers[i]],
                        tierTotalClaimableToken[msg.sender][tiers[i]],
                        0
                    );
                }
            }
    ...
    function _claimNFT(
        address _claimable,
        bytes32 _role,
        uint256 _tierCurrentIndex,
        uint256 _tierTotalClaimable,
        uint256 _chapter
    ) internal returns (uint256 _nftAmount) {
        require(tierTotalSupply[_role] > 0, "Role not added");
        require(
            tierTotalClaimed[_role].add(_tierTotalClaimable) <=
                tierTotalSupply[_role],
        ...
        for (uint256 i = 0; i < _tierTotalClaimable; i++) {
            ...
            if (teamNFTStatus == true) {
                tierTotalClaimableToken[_claimable][
                    _role
                ] = tierTotalClaimableToken[_claimable][_role].sub(1);
            }
            a = _role;
            _nftAmount = _nftAmount.add(1);
            if (_role == TIER3_ROLE) {
                chapterMintCount[_role][_chapter] = chapterMintCount[_role][
                    _chapter
                ].add(1);
                if (teamNFTStatus == true) {
                    tierTotalClaimableChapter[_claimable][_role][
                        _chapter
                    ] = tierTotalClaimableChapter[_claimable][_role][_chapter]
                        .sub(1);
                }
            }
        }
    ...
    function claimAllTeamNFT(
        ...
        for (uint256 i = 0; i < claimableAddress.length; i++) {
            if (tiers[_role[i]] == TIER3_ROLE) {
                uint256 _chapterCurrentIndex = tierTotalSupply[
                    tiers[_role[i].sub(1)]
                ].add(uint256(100).div(totalChapters).mul(_chapter[i])).add(
                        chapterMintCount[tiers[_role[i]]][_chapter[i]]
                    );
                uint256 _chapterMaxLimit = tierTotalSupply[
                    tiers[_role[i].sub(1)]
                ].sub(1).add(
                        uint256(100).div(totalChapters).mul(_chapter[i].add(1))
                    );
                if (_chapter[i] < totalChapters.sub(2)) {
                    require(
                        chapterMintCount[tiers[_role[i]]][_chapter[i]] <= 7 &&
                            _chapterCurrentIndex <= _chapterMaxLimit,
                        "Chapter NFT already claimed"
                    );
                } else {
                    if (_chapter[i] == 13) {
                    ...
                    require(
                        chapterMintCount[tiers[_role[i]]][_chapter[i]] <= 8 &&
                            _chapterCurrentIndex <= _chapterMaxLimit,
                        "Chapter NFT already claimed"
                    );
                }
                ...
                _claimNFT(
                    claimableAddress[i],
                    tiers[_role[i]],
                    tierTotalClaimed[tiers[_role[i]]],
                    1,
                    0
                );
            }
