...
import '../libraries/houndsParams/Constructor.sol';
import '../libraries/houndsParams/Updatable.sol';
import '../libraries/hound/Hound.sol';
import '../libraries/payments/Transferrer.sol';
import '../libraries/payments/Withdrawer.sol';
...
contract HoundsData is Ownable, ERC721, ERC721Holder {
    ...
    mapping(uint256 => Hound.Default) hounds;
    string error = "Failed to delegatecall";
    event NewHounds(uint256 indexed houndIdStart, uint256 indexed houndIdFinish, Hound.Default[] hound);
    event NewHound(uint256 indexed houndIdStart, Hound.Default hound);
    event EditHound(uint256 indexed houndId, Hound.Default hound);
    ...
    /**
     * DIIMIIM:
     * - params: the constructor parameters structure
     * - name: the name of the ERC721 contract
     * - symbol: the symbol of the ERC721 contract
     * - theBirthCost: the ETH cost to mint a hound
     * - theBreedCost: the ETH cost to breed hounds
     * - incubator: the address of the incubator controller
     * - incubatorBirth: the birth method from the incubator controller, displayed as raw function signature, e.g: "birth(uint256)"
     * - incubatorBreed: the breed method from the incubator controller, displayed as raw function signature, e.g: "breed(uint256)"
     */
    constructor(
    ...
    function setGlobalParameters(
        HoundsUpdatable.Struct memory input
    ) external onlyOwner {
        (bool success, ) = input.methods.delegatecall(
            abi.encodeWithSignature(
                "setGlobalParameters((address[],address,address,address,uint256,uint256,uint256,bool[]))",
                input
            )
        );
     ...
     * Admin only
     */
    function adminCreateHound(Hound.Default[] memory theHounds) external onlyOwner {
        (bool success, ) = control.methods.delegatecall(
            abi.encodeWithSignature(
                "adminCreateHound(((uint64,uint64,uint64,uint64),(uint256,uint256,uint32,uint32,uint32),(uint256,uint256,uint256,bool),(uint32[78],uint256,uint32[78],uint32[78]),string,string,bool,bool)[])",
                theHounds
            )
        );
        require(success,error);
    ...
    function updateHoundStamina(uint256 theHoundId, uint32 staminaToConsume) external payable {
        (bool success, ) = control.methods.delegatecall(
            abi.encodeWithSignature(
                "updateHoundStamina(uint256,uint32)",
                theHoundId,staminaToConsume
            )
        );
        require(success,error);
    }
    ...
    function putHoundForBreed(uint256 _hound, uint256 fee, bool status) external {
        (bool success, ) = control.methods.delegatecall(
            abi.encodeWithSignature(
                "putHoundForBreed(uint256,uint256,bool)",
                _hound,fee,status
            )
        );
        require(success,error);
...
library Withdrawer {
	 ...
	 */
	function withdraw(address currency, uint256 quantity) internal {
		if ( currency == address(0) )
		    require(payable(msg.sender).send(quantity), "14");
		else
		    require(IERC20(currency).transferFrom(address(this),msg.sender,quantity), "15");
	}
...
pragma solidity <=0.8.10;
import '../libraries/hound/Hound.sol';
import '../libraries/incubatorParams/Constructor.sol';


interface IIncubatorData {
    function setGlobalParameters(IncubatorConstructor.Struct memory input) external;
    function breedHounds(uint32[78] memory hound1, uint32[78] memory hound2) external view returns(Hound.Default memory);
}
