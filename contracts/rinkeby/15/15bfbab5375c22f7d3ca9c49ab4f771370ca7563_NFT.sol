    ...
    AccessControlUpgradeable
{
    ...
    function setClaimableRole(address[] memory claimableAddress, bytes32[] memory _role, uint256[] memory _chapter, uint256[] memory _tokenAmount)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require((claimableAddress.length == _tokenAmount.length), "Invalid Data");
        for (uint256 i = 0; i < claimableAddress.length; i++) {
            require(tierTotalSupply[_role[i]] > 0, "Role not added");
            _setupRole(_role[i], claimableAddress[i]);
            tierTotalClaimableToken[claimableAddress[i]][_role[i]] = tierTotalClaimableToken[claimableAddress[i]][_role[i]].add(_tokenAmount[i]);
            if(_role[i] == TIER3_ROLE) {
                tierTotalClaimableChapter[claimableAddress[i]][_role[i]][_chapter[i]] = tierTotalClaimableChapter[claimableAddress[i]][_role[i]][_chapter[i]].add(_tokenAmount[i]);
            }
        }
    }
        ...
        for(uint256 i=0; i<tiers.length; i++) {
            if(tiers[i] == TIER3_ROLE && tierTotalClaimableToken[msg.sender][tiers[i]] > 0) {
                for(uint256 j=0; j<totalChapters; j++) {
                    ...
                    uint256 _chapterCurrentIndex = tierTotalClaimed[tiers[i]].add(uint256(100).div(totalChapters).mul(j)).add(chapterMintCount[tiers[i]][j]);
                    uint256 _chapterMaxLimit = tierTotalClaimed[tiers[i]].sub(1).add(uint256(100).div(totalChapters).mul(j.add(1)));
                    if(i < totalChapters.sub(2)) {
                        require(chapterMintCount[tiers[i]][j] <= 7 && _chapterCurrentIndex <= _chapterMaxLimit, "All chapter NFT claimed");
                    } else {
                        _chapterMaxLimit = _chapterMaxLimit.add(1);
                        require(chapterMintCount[tiers[i]][j] <= 8 && _chapterCurrentIndex <= _chapterMaxLimit, "All chapter NFT claimed");
                    }
    ...
    function _claimNFT(address _claimable, bytes32 _role, uint _tierCurrentIndex, uint256 _tierTotalClaimable, uint256 _chapter) internal returns(uint256 _nftAmount) {
        ...
        for(uint256 i=0; i<_tierTotalClaimable; i++) {
            ...
            tierTotalClaimableToken[_claimable][_role] = tierTotalClaimableToken[_claimable][tiers[i]].sub(1);
            _nftAmount = _nftAmount.add(1);
            if(_role == TIER3_ROLE) {
                chapterMintCount[_role][_chapter] = chapterMintCount[_role][_chapter].add(1);
                tierTotalClaimableChapter[_claimable][_role][_chapter].sub(1);
            }
        }
        return _nftAmount;
    }

    function claimAllTeamNFT(address[] memory claimableAddress, bytes32[] memory _role, uint256[] memory _chapter) external whenNotPaused nonReentrant {
        require(claimableAddress.length == _role.length, "Invalid Data");
        require(teamNFTStatus != true, "Team NFT Already Claimed");
        require(totalSupply() < MAX_SUPPLY, "Max supply limit reached");
        for(uint256 i=0; i<claimableAddress.length; i++) {
            if(_role[i] == TIER3_ROLE) {
                uint256 _chapterCurrentIndex = tierTotalClaimed[_role[i]].add(uint256(100).div(totalChapters).mul(_chapter[i])).add(chapterMintCount[_role[i]][_chapter[i]]);
                uint256 _chapterMaxLimit = tierTotalClaimed[_role[i]].sub(1).add(uint256(100).div(totalChapters).mul(_chapter[i].add(1)));
                if(i < totalChapters.sub(2)) {
                    require(chapterMintCount[_role[i]][_chapter[i]] <= 7 && _chapterCurrentIndex <= _chapterMaxLimit, "All chapter NFT claimed");
                } else {
                    _chapterMaxLimit = _chapterMaxLimit.add(1);
                    require(chapterMintCount[_role[i]][_chapter[i]] <= 8 && _chapterCurrentIndex <= _chapterMaxLimit, "All chapter NFT claimed");
                }
                _claimNFT(claimableAddress[i], _role[i], _chapterCurrentIndex, 1, _chapter[i]);
            } else {
                _claimNFT(claimableAddress[i], _role[i], tierTotalClaimed[_role[i]], 1, 0);
            }
        }
        teamNFTStatus = true;
    }
