...
contract RightwayToken is ERC721Enumerable, Saleable, Ownable {
  constructor(
    string memory name,
  ...
  function setDropRoyalties( address payable newRoyaltyAddress, uint256 newRoyaltyBps ) public onlyOwner unsealed {
    royaltyAddress = newRoyaltyAddress;
    royaltyBps = newRoyaltyBps;
  }

  function setApis( string calldata newContentApi, string calldata newMetadataApi ) public onlyOwner {
    contentApi = newContentApi;
    metadataApi = string(abi.encodePacked(newMetadataApi, '/', addressToString(address(this)), '/'));
  }

  function addDropContentLibraries( RightwayDecoder.DropContentLibrary[] memory contentLibraries ) public onlyOwner unsealed {
    for (uint idx = 0; idx < contentLibraries.length; idx++) {
      drop.contentLibraries.push(contentLibraries[idx]);
    }
  }

  function addDropContent( bytes32[] calldata content, uint offset ) public onlyOwner unsealed {
    uint idx = 0;
  ...
  function addDropStringData( bytes32[] calldata stringData, uint offset) public onlyOwner unsealed {
    uint idx = 0;
  ...
  function addDropSentences( bytes32[] calldata sentences, uint offset) public onlyOwner unsealed {
    uint idx = 0;
  ...
  function addDropAttributes( bytes32[] calldata attributes, uint offset) public onlyOwner unsealed {
    uint idx = 0;
  ...
  function addDropTemplates( bytes32[] calldata templates, uint offset) public onlyOwner unsealed {
    uint idx = 0;
  ...
  function addDropEditions( bytes32[] calldata editions, uint offset) public onlyOwner unsealed {
    uint idx = 0;
  ...
  function addDropTokens( bytes32[] calldata tokens, uint offset, uint length) public onlyOwner unsealed {
    uint idx = 0;
  ...
  function addTokenContent( uint256[] memory tokens, uint16 contentIdx) public onlyOwner unsealed {
    for (uint idx = 0; idx < tokens.length; idx++) {
      stateByToken[tokens[idx]].additionalContent.push(contentIdx);
    }
  }
  ...
  function mint(address to, uint256 tokenId, uint16 attributesStart, uint16 attributesLength) public onlyOwner {
  ...
  function mintBatch(address to, uint256[] calldata tokenIds, uint16 attributesStart, uint16 attributesLength) public onlyOwner {
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
      return interfaceId == _INTERFACE_ID_FEES || ERC721Enumerable.supportsInterface(interfaceId);
  }
