...
interface IBasicVault {
  ...
  function isVault(address _target) external view returns (bool);
}
...
interface IStrategy {
  ...
  function getProportions(uint256 _vaultId)
    external
    view
    returns (uint256, uint256);
}
...
abstract contract BasePairLPStrategy is OndoRegistryClient, IStrategy {
  ...
  /**
   * @notice The ratio of senior to junior assets at the time the Vault was created.
   */
  function getProportions(uint256 _vaultId)
    external
    view
    ...
    returns (uint256, uint256)
  {
    ...
    return (vault_.senior.balanceOf(pool), vault_.junior.balanceOf(pool));
  }
...
contract UniswapStrategy is BasePairLPStrategy {
    ...
    returns (uint256 seniorInvested, uint256 juniorInvested)
  {
    ...
    vault_.seniorExcess = _totalSenior - seniorInvested;
    vault_.juniorExcess = _totalJunior - juniorInvested;
