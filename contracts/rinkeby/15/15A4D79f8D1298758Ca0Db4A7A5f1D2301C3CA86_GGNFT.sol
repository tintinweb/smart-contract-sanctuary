...
// Part: OpenZeppelin/openz[email protected]/IERC721Receiver
...
contract GGNFT is ERC721("Gnarly Gnomes", "GG"), Ownable {
    using SafeMath for uint256;
    ...
    uint256 public constant maxGnomePurchase = 20;

    uint256 public constant MAX_GNOMES = 9999;

    uint256 public constant gnomePrice = 50000000000000000; //0.08 ETH
    ...
    function mintGnome(uint256 numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint!");
        require(
            numberOfTokens <= maxGnomePurchase,
            "Can only mint 20 tokens at a time"
        );
        require(
            totalSupply().add(numberOfTokens) <= MAX_GNOMES,
            "Purchase would exceed max supply!"
        );
        require(
            gnomePrice.mul(numberOfTokens) <= msg.value,
            "Ether value sent is not correct"
        );

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 mintIndex = totalSupply() + 1;
            if (totalSupply() < MAX_GNOMES) {
