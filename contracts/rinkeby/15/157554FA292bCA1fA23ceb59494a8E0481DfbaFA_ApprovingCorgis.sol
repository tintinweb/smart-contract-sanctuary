...
contract ApprovingBone is ERC721Enumerable, Ownable {
    ...
    ApprovingCorgis public creator;
    ...
    constructor(address owner) ERC721("Approving Bone", "ACBONE")  {
        ...
        creator = ApprovingCorgis(msg.sender);
    }

    modifier onlyCreator() {
        require(address(creator) == msg.sender, "Only the parent ApprovingCorgis contract can perform this action");
        _;
    }
    /**
     * mint Corgis
     */
    function mintBone() public payable {
        uint256 supply = totalSupply();
        require(boneMintActive, "Minting Approving Bone Is Not Yet Active");
        require(isAddressReserved(msg.sender), "You Are Not In The List");
    ...
    function toggleBoneMintActive() public onlyCreator {
        boneMintActive = !boneMintActive;
    }

    function changeMaxBones(uint256 _maxBones) public onlyCreator {
        maxBones = _maxBones;
    }

    function addReservationAddress(address _ogAddress) public onlyCreator {
        require(!isAddressReserved(_ogAddress));
        earlyAccessAddresses[_ogAddress] = true;
    }

    function addMultipleAddresses(address[] memory _ogAddress) public onlyCreator {
        for (uint256 i = 0; i <= _ogAddress.length; i++) {
            
            require(!isAddressReserved(_ogAddress[i]));
...
contract ApprovingCorgis is ERC721Enumerable, Ownable {
    ...
    address public communityVault = 0xc4B4245f9aD53A5F8D99A5eE7fE1D45F196E404c;
    address public charityFundWallet = 0x49f8Bbf2f2576F76f2BDd1A58dc26a4258492188;
    ...
    constructor(string memory tokenBaseUri, address boneContractAddress) ERC721("Approving Corgis", "ACORGIS")  {
        ...
        // founding team gets the first 99 corgis
        reserveCorgis();
        // Approving Bone Contract
     ...
     */
    function reserveCorgis() public onlyOwner {        
        uint supply = totalSupply();
        uint i;
        for (i = 0; i < team.add(reserved); i++) {
            _safeMint(msg.sender, supply + i);
        }
    }

    function earlyAccessMint(uint256 numberOfTokens) public payable {
        uint256 supply = totalSupply();
        uint256 tokenCount = approvingBoneContract.balanceOf(msg.sender);

        require(earlyAccessIsActive, "Early Access Sale is not active yet");
        require(tokenCount > 0, "You don't have a mint pass");
        require(msg.value >= price.mul(numberOfTokens), "Payment is Insufficient");
        require(numberOfTokens <= maxEarlyAccessPurchase, "You can mint a maximum of 5 Corgis");
        require(supply.add(numberOfTokens) <= maxEarlyAccess, "Exceeds maximum Corgis supply" );
    ...
    function mintCorgis(uint256 numberOfTokens) public payable {
        ...
        require(msg.value >= price.mul(numberOfTokens), "Payment is Insufficient");
        require(numberOfTokens <= maxPublicPurchase, "You can adopt a maximum of 20 Corgis");
        require(supply.add(numberOfTokens) <= maxCorgis, "Exceeds maximum Corgis supply" );
