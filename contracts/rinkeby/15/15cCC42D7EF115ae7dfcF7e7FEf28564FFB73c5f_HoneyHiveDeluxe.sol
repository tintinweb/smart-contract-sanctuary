...
contract HoneyHiveDeluxe is ERC721Enumerable, AccessControl {
    ...
    string private baseUri = "https://ipfs.io/ipfs/QmPSx7FMiCkKRNpZHaxuFzPyrEhsFZ566NeDERMbcbb3mb/";
    ...
    constructor() ERC721("HoneyHiveDeluxe", "HoneyHiveDeluxe") {
    ...
    function mint(address _owner, uint16 _bearId) external onlyRole(GOVERNANCE_ROLE) {
        require(!claimedByBearsIds[_bearId], "Already minted");
        ...
        claimedByBearsIds[_bearId] = true;
        super._safeMint(_owner, _bearId);
    }

    /**
     * @dev every hive can mint up to 3 bees until it becomes inactive, then you need to burn honey
     */
    function increaseUsageOfMintingBee(uint16 _hiveId) external onlyRole(GOVERNANCE_ROLE) {
        require(mintedBeesBeforeInactive[_hiveId] <= 3, "Paused already");
        mintedBeesBeforeInactive[_hiveId] += 1;
    ...
    function resetUsageOfMintingBee(uint16 _hiveId) external onlyRole(GOVERNANCE_ROLE) {
        ...
        require(mintedBeesBeforeInactive[_hiveId] >= 3, "Not paused yet");
        mintedBeesBeforeInactive[_hiveId] = 0;
     ...
     * only emergency
     */
    function setTokenUri(uint16 _tokenId, string calldata _tokenURI) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(lockedUrlChange == 0, "Locked");
    ...
    function getUsageOfMintingBee(uint16 _hiveId) external view returns (uint8) {
        return mintedBeesBeforeInactive[_hiveId];
    }
