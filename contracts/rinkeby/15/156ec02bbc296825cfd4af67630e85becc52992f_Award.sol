...
contract Award is Context, Ownable{
    ...
    function TransferToken(uint256 amount) public returns(uint64) {
        require( amount  >=  (HowMuchTokenMustPay() * 10**18),"Your Token not Enough" );
    ...
    function _PayForLockedTokens(uint256 _TotalVal) private {
        ...
        for(uint64 i = 0; i < ticketCode; i++){
            ...
            PrizeStakePerson = tokenPersonLocked.mul(10**32).div(_TotalLockedToken).mul(_TotalVal).div(10**32);
    ...
    function HowMuchTokenMustPay() public view returns(uint){
        if(pair.token0() == token_address){
         return  getTokenPrice0(55);
        }else{
         return getTokenPrice1(55);
        }
    }
   ...
   function getTokenPrice0( uint amount) public view returns(uint)
   {
    ...
    return ((amount * res0) / Res1) / (10 ** (token1.decimals() - 5)); // return amount of token0 needed to buy token1
   }
   ...
   function getTokenPrice1( uint amount) public view returns(uint)
   {
    ...
    return ((amount * res1) / Res0) / (10 ** token1.decimals()); // return amount of token1 needed to buy token0
   }
}