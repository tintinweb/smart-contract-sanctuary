...
//File UniveETHBridge.sol

contract UniveETHBridge is Ownable{
    ...
    address public tokenAddress = 0xa1Ebb2B45194C415Dad4D4dCe91eD61f042fef96; //UNIVE token address on Ethereum chain
    ...
    // allows the owner to withdraw ETH and other tokens
    ...
    function getTransactionStatus(uint256 id) public view returns (uint256, address, uint256, address){
        return (txs[id].txId, txs[id].wallet, txs[id].amount, txs[id].toWallet);
    }
    function getClaimStatus(uint256 id) public view returns (uint256){
        return claimed[id];
    }
}