...
contract Dogeman is Context, IERC20, Ownable {
    ...
    address payable private _marketingWallet = 0x8262f2fAb095650206F3E643A7C73A62AB4B2F17;
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        uint256 currentRate =  _getRate();
        uint256 tBurn = tLiquidity.mul(_burnFee).div(_burnFee.add(_marketingFee).add(_liquidityFee));
        tLiquidity = tLiquidity.sub(tBurn);
        uint256 rBurn = tBurn.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        _rOwned[_dead] = _rOwned[_dead].add(rBurn);
        _tOwned[_dead] = _tOwned[_dead].add(tBurn);
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee.add(_burnFee).add(_marketingFee)).div(
            100
        );
    }

    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 && _marketingFee == 0 && _liquidityFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 marketingTokenBalance = contractTokenBalance.mul(_marketingFee).div(_marketingFee.add(_liquidityFee));
        ...
        // BNB to liquify is 1/6 of the BNB that has already been liquified
        uint256 bnbToLiquify = bnbSwapped.mul(tokenBalanceToLiquifyAsBNB).div(tokensToSwapToBNB);
        ...
        // The remaining BNB balance is to be donated to the marketing wallet and charity wallet
        uint256 marketingBNBToDonate = bnbSwapped.sub(bnbToLiquify);

        // Transfer the BNB to the marketing and charity wallet
        _marketingWallet.transfer(marketingBNBToDonate);
