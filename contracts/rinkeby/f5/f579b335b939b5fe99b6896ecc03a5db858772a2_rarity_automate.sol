...
contract rarity_automate {
    ...
    ERC721 constant _erc721 = ERC721(0xd99d686F75Dfd6884a822b436eFf8CD0783A3889);
    ...
    function bulk_summon(uint class, uint count) external payable{
        require(count < 10, 'Max 10 per call');
        uint256[] memory summoned = new uint256[](count);
        
        for (uint i = 0; i < count; i++) {
            uint summonerId = _rm.next_summoner();
            _rm.summon(class);
            _erc721.transferFrom(address(this), msg.sender, summonerId);
