...
contract AccessRestriction is
    ...
    IAccessRestriction
{
    bytes32 public constant MINTII_CONTRACT_ROLE =
        keccak256("MINTII_CONTRACT_ROLE");
    ...
    /**
     * @dev check if given address is Mintii contract
     * @param _address input address
     */
    function ifMintiiContract(address _address) external view override {
        require(isMintiiContract(_address), "caller is not mintii contract");
    }
    ...
    /**
     * @dev check if given address has Mintii contract role
     * @param _address input address
     * @return if given address has Mintii contract role
     */
    function isMintiiContract(address _address)
        public
        view
        override
        returns (bool)
    {
        return hasRole(MINTII_CONTRACT_ROLE, _address);
    }
...
interface IAccessRestriction is IAccessControlUpgradeable {
     ...
     * @param _address input address
     */
    function ifMintiiContract(address _address) external view;
     ...
     * @return if given address has Mintii contract role
     */
    function isMintiiContract(address _address) external view returns (bool);
