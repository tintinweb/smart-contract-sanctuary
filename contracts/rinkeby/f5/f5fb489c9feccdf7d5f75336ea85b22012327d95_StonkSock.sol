...
contract StonkSock is IERC721Metadata, ERC721Enumerable, Ownable, ERC721Pausable {
    ...
    uint256 public constant MINT_LIMIT = 2021;
    ...
    constructor(string memory baseURI) ERC721("StonkSock", "SSOK") {
        setBaseURI(baseURI);
        presaleLive = false;
        saleLive = false;
    }
    ...
    function isPresaleClaimed(address owner) external view returns (uint256){
    ...
    modifier saleIsLive {
        ...
        if (_msgSender() != owner()) {
            require(saleLive == true, "Mint Limit Reached");
        }
    ...
    function presaleMint(address _to, uint256 _count) public payable {
        ...
        require(total + _count < PRESAL_LIMIT, "Max limit");
    ...
    function mint(address _to, uint256 _count) public payable saleIsLive {
        require(_totalSupply() + _count < MINT_LIMIT, "There are not enough Tokens remaining for this Request");
        require(_count + balanceOf(_to) <= MAX_PER_WALLET, "Exceeds number");
        require(msg.value >= price(_count), "Insufficient Funds For Mint");
