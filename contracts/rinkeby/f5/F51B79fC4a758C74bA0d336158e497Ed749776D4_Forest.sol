...
contract Forest is Ownable, IERC721Receiver, Pausable {
	...
	modifier _updateEarnings() {
		if (totalWoolEarned < MAXIMUM_GLOBAL_WOOL) {
			totalWoolEarned += ((block.timestamp - lastClaimTimestamp) * totalApeStaked * DAILY_WOOL_RATE) / 1 days;
			lastClaimTimestamp = block.timestamp;
		}
		_;
	}

	function getStakeList(address _address) external view returns (Stake[] memory) {
		uint256[] memory stakeIds = stakeMap[_address];
		Stake[] memory stakeList = new Stake[](stakeIds.length);
		uint8 index = 0;
		for (uint256 i = 0; i < stakeIds.length; i++) {
			uint256 tokenId = stakeIds[i];
			if (isApe(tokenId)) {
				Stake memory stake = forest[tokenId];
				stakeList[index] = stake;
			} else {
