...
contract ShyftBALV2LPStaking is Ownable {
  ...
  function updatePairObservation(
    ...
    address pair = UniswapV2Library.pairFor(factory, _tokenA, usdcToken);
  ...
  function getPrice(
    ...
    (address token0,) = UniswapV2Library.sortTokens(_tokenA, usdcToken);
    uint256 usdcPrice = getTokenUSDPrice(usdcProxy);

    if (_tokenA == token0) {
      priceA = observation.price0.mul(usdcPrice.div(1e8));
    } else {
      priceA = observation.price1.mul(usdcPrice.div(1e8));
    }
