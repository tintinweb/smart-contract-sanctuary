...
contract PolicyBookFacade is IPolicyBookFacade, AbstractDependant, Initializable {
    ...
    // premiums (under safe price model)
    EnumerableSet.UintSet internal _safePricingModelPremiums;
    // premiums (under risk price model)
    EnumerableSet.UintSet internal _riskPricingModelPremiums;
    // premiums (under safe price model) => list of URs
    mapping(uint256 => EnumerableSet.UintSet) internal _safePricingModelURs;
    // premiums (under risk price model) => list of URs
    mapping(uint256 => EnumerableSet.UintSet) internal _riskPricingModelURs;
    ...
    function _buyPolicy(
        ...
        (uint256 _premium, uint256 pricePercentage) =
        ...
        uint256 utilizationRatioPercentage =
            ((policyBook.totalCoverTokens().add(_coverTokens)).mul(PERCENTAGE_100)).div(
                policyBook.totalLiquidity().add(totalLeveragedLiquidity)
            );

        if (safePricingModel) {
            _safePricingModelPremiums.add(pricePercentage);
            _safePricingModelURs[pricePercentage].add(utilizationRatioPercentage);
        } else {
            _riskPricingModelPremiums.add(pricePercentage);
            _riskPricingModelURs[pricePercentage].add(utilizationRatioPercentage);
        }
    ...
    /// @dev The Minimum utilization ratio is the highest utilization ratio under
    /// which we have a minimum coverage price under the current pricing model.
    function getSafePricingModelMINUR() external view override returns (uint256) {
        uint256 _smallestPremium;
        uint256 _highestUR;

        for (uint256 i = 0; i < _safePricingModelPremiums.length(); i++) {
            if (_safePricingModelPremiums.at(i) > _smallestPremium) {
                _smallestPremium = _safePricingModelPremiums.at(i);
            }
        }
        ...
        for (uint256 i = 0; i < _safePricingModelURs[_smallestPremium].length(); i++) {
            if (_safePricingModelURs[_smallestPremium].at(i) > _highestUR) {
                _highestUR = _safePricingModelURs[_smallestPremium].at(i);
            }
        }
        return _highestUR;
    }
    ...
    /// @dev The Minimum utilization ratio is the highest utilization ratio under
    /// which we have a minimum coverage price under the current pricing model.
    function getRiskPricingModelMINUR() external view override returns (uint256) {
        uint256 _smallestPremium;
        uint256 _highestUR;

        for (uint256 i = 0; i < _riskPricingModelPremiums.length(); i++) {
            if (_riskPricingModelPremiums.at(i) > _smallestPremium) {
                _smallestPremium = _riskPricingModelPremiums.at(i);
            }
        }
        ...
        for (uint256 i = 0; i < _riskPricingModelURs[_smallestPremium].length(); i++) {
            if (_riskPricingModelURs[_smallestPremium].at(i) > _highestUR) {
                _highestUR = _riskPricingModelURs[_smallestPremium].at(i);
            }
        }
        return _highestUR;
    }
