...
// File: contracts/dispersals/bonds/interfaces/IPrincipleDepository.sol
...
// File: contracts/dispersals/profits/interfaces/IStaking.sol
...
contract Vault is ITreasury, Ownable {
  ...
  //IEpochCalculator public getEpochCalculator;
  ...
  address public override principleDepository;
  ...
  modifier onlyPayment() {
    ...
    //require( getPaymentAddressForReserveToken[reserveToken_] == msg.sender, "Vault:: msg.sender"  );
    _;
  }

  // *** Functions that set for Principle Depository *** \\

  function addReserveTokenForPrincipleDepository( address newReserveToken_, bool isERC2612_ ) external override onlyOwner() returns ( bool ) {
    
    IPrincipleDepository( principleDepository ).addReserveToken( newReserveToken_, isERC2612_ );
    return true;
  }

  function addPrincipleTokenForPrincipleDepository( address newPrincipleToken_, bool isERC2612_ ) external override onlyOwner() returns ( bool ) {
    
    IPrincipleDepository( principleDepository ).addPrincipleToken( newPrincipleToken_, isERC2612_ );
    return true;
  }

  function removePrincipleTokenForPrincipleDepository( address oldPrincipleToken_ ) external override onlyOwner() returns ( bool ) {
    
    IPrincipleDepository( principleDepository ).removePrincipleToken( oldPrincipleToken_ );
    return true;
  }

  function addBondTermForPrincipleDepository( address reserveToken_, address bondPrincipleToken_, uint256 bondScalingFactor_, uint256 bondingPeriodInBlocks_ ) external override onlyOwner() returns ( bool ) {
    
    IPrincipleDepository( principleDepository ).addBondTerm( reserveToken_, bondPrincipleToken_, bondScalingFactor_, bondingPeriodInBlocks_ );
    return true;
  }

  function removeBondTermForPrincipleDepository( address reserveToken_, address bondPrincipleToRemove_ ) external override onlyOwner() returns ( bool ) {
   
    IPrincipleDepository( principleDepository ).removeBondTerm( reserveToken_, bondPrincipleToRemove_ );
    return true;
  }

  function removeBondingCalculatorForPrincipleTokenForPrincipleDepository( address reserveToken_, address principleToken_ ) external override onlyOwner() returns ( bool ) {
   
    IPrincipleDepository( principleDepository ).removeBondingCalculatorForPrincipleToken( reserveToken_, principleToken_);
    return true;
  }

  // *** Functions that set for Principle Depository *** \\

  function setPrincipleDepository( address principleDepository_ ) external override onlyOwner() returns ( bool ) {
    principleDepository = principleDepository_;
  ...
  //function setEpochCalculator( address newEpochCalculator_ ) external onlyOwner() returns ( bool ) {
   // getEpochCalculator = IEpochCalculator( newEpochCalculator_ );
   // return true;
 // }
  ...
  // function ownerDeposit( address principleTokenAddress_, address reserveTokenAddress_, address tokenAddressToDeposit_ uint amount_ ) external onlyReserveToken( reserveTokenAddress_ ) onlyPrincipleToken( principleTokenAddress_ ) onlyOwner() {
  //   require( isPrincipleToken[tokenAddressToDeposit_] == true || isReserveToken[tokenAddressToDeposit_] == true, 'Not depositing principle or reserve token');
    
  //   IERC20( tokenAddressToDeposit_ ).safeTransferFrom( msg.sender, address(this), amount_ );

  //   _updateIntrinsicValue(principleTokenAddress_, reserveTokenAddress_ );
...
//   // function _calcPendingProfit( address reserveToken_ ) internal returns( uint pendingProfit_ ) {
//   //   uint currentIntrinsicValue_ = getIntrinsicValueOfReserveToken[reserveToken_];
//   //   uint reserveTokenTotalSupply_ = IERC20( reserveToken_ ).totalSupply();
//   //   pendingProfit_ = FixedPoint.fraction( currentIntrinsicValue_, reserveTokenTotalSupply_ ).decode();
//   // }

//   // function distributeProfit( address reserveToken_ ) external onlyPayment( msg.sender ) returns ( bool ) {
//   //   if( getEpochCalculator.isEpochElapsed() ) {
//   //      IERC20Mintable( reserveToken_ ).mint( msg.sender, _calcPendingProfit( address reserveToken_ );
//   //      IPayment( getPaymentAddressForReserveToken[reserveToken_] ).distributeOLYProfits();
//   //   }
//   // }

//   /*************************************************************************************************************************************************/

//   // mapping( address => mapping( address => uint ) ) public getEpochProfitOfProceedsTokenOfReserveToken;

  
//   // mapping( address => address ) public getIntrinsivValueCalculatorForReserveToken;
//   // mapping( address => address ) public getReserveTokenMigratorForReserveToken;


//   // mapping( address => uint ) public getAmountOfReserveToken;
// }
