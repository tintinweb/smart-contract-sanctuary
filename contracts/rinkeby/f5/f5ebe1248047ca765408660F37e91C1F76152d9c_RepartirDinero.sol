...
contract RepartirDinero {
    ...
    function agregarUsuario(string memory _nombre, address _address) public {
        getAddress[_nombre] = _address;
        User memory newUser = User(_nombre, _address, 0);
        users.push(newUser);
    }
    
    function conocerDireccion(string memory _nombre) public view returns(address) {
    ...
    function obtenerBalance(address _token) public view returns(uint){
        IERC20 paymentToken = IERC20(_token);
        return paymentToken.balanceOf(owner);
    }
    
    function agregarPunto(address _address)public {
        for(uint i = 0; i < users.length; i++){
            if(users[i].billetera == _address){
                users[i].puntos += 1;
            }
        }
    }
    ...
    function transferERC20(IERC20 _token, address _to, uint _amount) public soloPropietario {
        ...
        require(_amount <= erc20balance, "Insuffienct founds");
    ...
    function realizarPago() public soloPropietario view returns(uint){
        require(address(owner).balance > 0 , "Al parecer no hay fondos :c");
        //Variable
        uint puntosTotales;
        uint dineroTotal = address(owner).balance;
        uint partesIguales;
        for(uint i = 0; i < users.length; i++){
            puntosTotales+=users[i].puntos;
        }
        
        partesIguales = dineroTotal / puntosTotales;
