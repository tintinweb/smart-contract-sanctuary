...
contract animalMain is  ERC721,Ownable {
  ...
  using SafeMath for uint256;
    string public name_ = "Bagajong"; // 8+9
  ...
  function seeAnimalRole(uint256 _tokenId) public view returns (uint16 roletype) {
    ...
    function seeAnimalAge(uint256 _tokenId) public view returns (uint8 age) {
    return animals[_tokenId].age;
  }
  ...
  function approve(address _to, uint256 _tokenId) public {
    require(animalToOwner[_tokenId] == msg.sender); //判定持有人
  ...
  function createAnimal() public payable {
      ...
      //使用亂數來產生DNA, 星級, 動物種類
      dna = random();
      uint age = uint(random()) % 100;
      uint ram = uint(random()) % 100;
      if (ram == 89){ // 8+9混混霸主
          star = 6;
      }
      if (ram < 40){ // 國中屁孩 高中生小孩
          star = 1;
      } else if (ram < 65){ //社會大學
          star = 2;
      } else if (ram < 85){ //八家將舞者
          star = 3;
      } else if (ram < 95 && ram != 89){ // 已有前科
          star = 4;
      } else { //更生人
          star = 5;
      }
      ...
      uint id = animals.push(animal(dna, uint8(star), uint8(roletype), uint8(age))) - 1;
