...
contract IMNOTBRICKS is Ownable, ERC721Enumerable, RoyaltiesV2 {
    ...
    uint256 public maxInvocations = 312;
    uint256 public mintPriceInWei = 3120000000000000000; // 3.12 ETH
    ...
    mapping(address => uint) private addressToMintCount;
    ...
    modifier maxMintCheck() {
        require(addressToMintCount[msg.sender].add(1) <= maxPerAddress, "Max mint limit reached.");
        _;
    }
    ...
    // Constructor
    // ---
    constructor() ERC721("imnotArt Founder Bricks", "IMNOTBRICKS") {
        isAdmin[msg.sender] = true;

        // Contract URI @TODO(iolson): Change this before going live.
        contractUri = 'contract-uri';
        metadataBaseUrl = 'metadata-base-url/';
        ...
        // imnotArt Asset
        uint256 tokenId = invocations;
        _mint(gnosisSafe, tokenId);
        addressToMintCount[gnosisSafe] = addressToMintCount[gnosisSafe].add(1);
    ...
    // Minting
    // ---
    function mint() public payable onlyActive onlyNonPaused maxMintCheck returns (uint256) {
        if (presale) {
            require(isPresaleWallet[msg.sender], "Wallet is not part of pre-sale.");
        }
        ...
        if (refund > 0) {
            ...
            emit Payment(msg.sender, refund);
        }
    ...
    function addPresaleWallet(address presaleWallet) public onlyAdmin {
        require(!active, "Cannot alter pre-sale wallets after active.");
        isPresaleWallet[presaleWallet] = true;
    }

    function removePresaleWallet(address presaleWallet) public onlyAdmin {
        require((msg.sender != presaleWallet), "Cannot remove self.");
        require(!active, "Cannot alter pre-sale wallets after active.");
    ...
    function updateMetadataBaseUrl(string memory newMetadataBaseUrl) public onlyAdmin {
    ...
    function tokenURI(uint256 tokenId) public view override virtual onlyValidTokenId(tokenId) returns (string memory) {
        return StringsUtil.concat(metadataBaseUrl, StringsUtil.uint2str(tokenId));
    }
