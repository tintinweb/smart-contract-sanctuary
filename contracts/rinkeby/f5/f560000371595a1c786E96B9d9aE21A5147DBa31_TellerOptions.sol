 ...
 */
contract TellerOptions is ERC721, IERC721ReceiverUpgradeable {
    ...
    event OptionFilled(uint256 optionId);
    event OptionCancelled(uint256 optionId);
    ...
    constructor() ERC721("TellerOptions", "TOPT") public { }
    ...
    function createOption(
        ...
        require(duration >= ONE_MONTH && duration <= ONE_YEAR);

        
        bytes memory tokenData = abi.encode(keccak256(abi.encodePacked( msg.sender, nftContractAddress, nftTokenId, block.number  )));
    ...
    function getOptionOwner(uint256 optionId) public view returns (address) {
        return ownerOf(optionId);
    }

    /*
        Used by the option owner to retrieve the escrowed NFT 
    */
    function cancelOption(uint256 optionId) public {
        require(
            msg.sender == getOptionOwner(optionId),
        ...
        require(
            originalStatus == OptionStatus.created ||
                originalStatus == OptionStatus.filled
        );

        //return the NFT to the option owner
        IERC721(options[optionId].nftContractAddress).safeTransferFrom(
            address(this),
            getOptionOwner(optionId),
            options[optionId].escrowedTokenId
        );

        //return any buyout ether to the filler [if exists]
        if (originalStatus == OptionStatus.filled) {
            payable(options[optionId].optionFiller).transfer(
                options[optionId].buyoutPriceWei
            );
        }
    ...
    function expireOption(uint256 optionId) public {
        ...
        require(
            block.timestamp >= (options[optionId].startTime + options[optionId].duration),
            "not expired"
        );
    ...
    function fillOption(uint256 optionId) public payable {
        require(
            options[optionId].status == OptionStatus.created,
            "incorrect state"
        );
        require(
            msg.value == options[optionId].buyoutPriceWei,
            "incorrect ether value to escrow"
        );
    ...
    /*
        Used by the option owner to retrieve the escrowed ETH 
    */
    function exerciseOption(uint256 optionId) public {
        ...
        payable( getOptionOwner(optionId) ).transfer(
            options[optionId].buyoutPriceWei
        );
    ...
    function onERC721Received(
        ...
        bytes32 tokenData = keccak256(abi.encodePacked( from,  msg.sender, tokenId,  block.number  ));

        require( abi.decode(data,(bytes32)) == tokenData );
