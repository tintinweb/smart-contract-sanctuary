...
contract boosterz is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    returns (string memory)
  {
    ...
    function mintNFT(address _to ,uint256 _mintAmount) public payable {
        ...
        if (msg.sender != owner()){
        require(msg.value >= price, "Price is lower than its value!");
        }
    ...
    function _setTotalNftSupply(uint256 _newTotalNft) private {
        totalCount = _newTotalNft;
    }
    
    function _setNewPrice(uint256 _newPrice) private {
        price = _newPrice;
    }
    ...
    function changePrice(uint256 _newPrice) public onlyOwner {
        _setNewPrice(_newPrice);
    }
    
    function changeTotalNFTSupply(uint256 _newTotal) public onlyOwner {
        _setTotalNftSupply(_newTotal);
    }
    
    function changeAllSettings(uint256 _newPrice, string memory _newURI, uint256 _newMaxSupply) public onlyOwner {
        _setBaseURI(_newURI);
        _setNewPrice(_newPrice);
        _setTotalNftSupply(_newMaxSupply);
    }
