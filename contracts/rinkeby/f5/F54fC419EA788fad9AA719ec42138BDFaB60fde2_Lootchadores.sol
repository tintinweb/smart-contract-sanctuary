...
contract Lootchadores is ERC721Enumerable, Ownable, ERC165Storage, PaymentSplitter {
    ...
    function getName(uint256 tokenId) internal view returns (string memory) {
        ...
        return bytes(name).length == bytes("").length ? string(abi.encodePacked("Lootchadore #", tokenId.toString())) : name;
    }
    ...
    function renderAttributes(uint256 _tokenId) internal view returns (bytes memory) {
        uint256 tokenId = luchaAbilites[_tokenId];
        return
            abi.encodePacked(
    ...
    function extractValue(string memory ability) internal pure returns (string memory) {
        ...
        strings.slice memory number = _ability.beyond(" ".toSlice());
