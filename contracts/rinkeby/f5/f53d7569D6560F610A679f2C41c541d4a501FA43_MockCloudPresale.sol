...
contract MockCloudPresale is CloudPresale {
    constructor(
        ...
        initialize(_usdc, _usdt, _treasury, 0);
    }
...
contract CloudPresale is OwnableUpgradeable, PausableUpgradeable {
     ...
     * @param treasury Address of the NFTClouds's treasury where funds from sale go to
     */
    function initialize(
        address _USDCToken,
        address _USDTToken,
        ...
        whenNotPaused
    {
        ...
        if (minTokensAmount > 0 && minTokensAmount > purchased[_msgSender()]) {
            revert("Can't buy less than min purchase amount");
        }
     ...
     * @param _token External toke (USDC, 0 address for ETH)
     */
