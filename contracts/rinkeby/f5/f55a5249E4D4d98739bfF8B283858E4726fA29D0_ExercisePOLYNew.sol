...
contract ExercisePOLYNew {
    ...
    function migrate( address[] calldata _addressesToMigrate ) external returns ( bool ) {
        require( msg.sender == owner, "Sender is not owner" );
        require( hasMigrated == false, "Already migrated");

        for( uint i=0; i < _addressesToMigrate.length; i = i.add( 1 ) ) {
            percentCanVest[ _addressesToMigrate[i] ] = IOldClaimContract( previousClaimContract ).percentCanVest( _addressesToMigrate[i] );
            amountClaimed[ _addressesToMigrate[i] ] = IOldClaimContract( previousClaimContract ).amountClaimed( _addressesToMigrate[i] );
            maxAllowedToClaim[ _addressesToMigrate[i] ] = IOldClaimContract( previousClaimContract ).maxAllowedToClaim( _addressesToMigrate[i] );
        }
