...
contract HeritageModule is Module {
    ...
    function setup(address[] memory _heirs, uint256[] memory _percentages, uint256 _threshold, uint256 _timelapse)
        public
    {
        _checkConfigParams(_heirs, _percentages, _threshold);
        setManager();
        _reconfigure(_heirs, _percentages, _threshold, _timelapse);
    }

    /// @dev Setup function sets initial storage of contract.
    /// @param _heirs List of heirs' addresses.
    /// @param _threshold Required number of heirs to confirm replacement.
    function reconfigure(address[] memory _heirs, uint256[] memory _percentages, uint256 _threshold, uint256 _timelapse)
        public
        authorized
    {
        _checkConfigParams(_heirs, _percentages, _threshold);
    ...
    /// @param _heirs List of heirs' addresses.
    /// @param _threshold Required number of heirs to confirm replacement.
    function _reconfigure(address[] memory _heirs, uint256[] memory _percentages, uint256 _threshold, uint256 _timelapse)
        internal
    {
        // Set allowed heirs.
    ...
    function askForInherit(address[] memory _erc20Addresses)
        public
        onlyHeir
    {
    ...
    /// @dev Returns time left before comfirmRecoverAccess
    /// @return uint time left before comfirmRecoverAccess.
    function getERCBalance(address _ercAddress)
        public
        view
        returns (uint)
    {
        IERC20 token = IERC20(_ercAddress);
        return token.balanceOf(address(manager));
    }
}
