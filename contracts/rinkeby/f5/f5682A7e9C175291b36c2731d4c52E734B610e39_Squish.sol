...
contract Squish is IERC721 {
    ...
    // test this, see if it works without the function 
    function _initializeArray(bytes32[] storage _arr) internal onlyDeployer() {
        for (uint i = 0; i <= _arr.length; i++) {
            ipfsHashes.push(_arr[i]);
        }
    }
    //////////////////////////
    //// ERC 721 ERC 165  ////
    ...
    function tokenURI(uint256 _tokenId) external view validNFToken(_tokenId) returns (string memory) {
        return string(abi.encodePacked("ipfs://", bytes32ToString(ipfsHashes[_tokenId - 1])));
    }
