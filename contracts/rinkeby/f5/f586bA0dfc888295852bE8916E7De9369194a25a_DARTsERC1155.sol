...
import {RightUtils} from "./lib/RightUtils.sol";
import "./DARTsERC2981.sol";

contract DARTsERC1155 is ERC1155, AccessControl, ERC1155Supply, DARTsERC2981Base {
    bytes32 public constant URI_SETTER_ROLE = keccak256("URI_SETTER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant ADD_MEMBER_ROLE = keccak256("ADD_MEMBER_ROLE");

    using Counters for Counters.Counter;
    using RightUtils for RightUtils.Right;
    ...
    constructor(string memory _URI) ERC1155(_URI) {
        ...
        setURI(_URI);
        Rights[0] = RightUtils.createNew(false);
    }
    ...
    function create(address[] memory owners, address[] memory creators, address[] memory sponsors, address[] memory thanks)
    external onlyRole(MINTER_ROLE) returns (uint) {
        require(owners[0] == msg.sender, "N C");
        _tokenIds.increment();
        uint newItemId = _tokenIds.current();
        RightUtils.royaltyTable memory RT = RightUtils.royaltyTable(0, 10000, 0, 0, 0);

        _mint(owners[0], newItemId, 1, "");

        _grantRole(ADD_MEMBER_ROLE, owners[0]);

        Rights[newItemId] = RightUtils.Right(
            newItemId, 0,
            ...
            uri(newItemId), true, 0, RT, "");
        defaultRoyaltyTable(newItemId);
        return newItemId;
    }

    function clone(uint formerId, uint _initSupply, uint8 _editType) external onlyRole(MINTER_ROLE) returns (uint) {
        ...
        require(_editType < 2, "ED");

        uint8 SELL_TYPE = 1;
        uint initSupply = 1;
        bool cloneable = true;

        if (_editType == SELL_TYPE) {
        ...
        RightUtils.royaltyTable memory RT;
        uint32[] memory zero;
        Rights[newItemId] = RightUtils.Right(
            newItemId, formerId,
    ...
    public onlyRole(ADD_MEMBER_ROLE) returns (uint){
        ...
        RightUtils.Right memory tempright = getItem(_tokenId);
        uint32[] memory memberspercent = _shares;

        if (msg.sender == tempright.owners[0]) {
            require(balanceOf(msg.sender, _tokenId) != 0, "CD");
            tempright.owners.concat(members);
            tempright.ownerspercent.concat(memberspercent);
        } else if (msg.sender == tempright.creators[0]) {
            require(hasRole(ADD_MEMBER_ROLE, tempright.creators[0]), "NCA");
            tempright.creators.concat(members);
            tempright.creatorpercent.concat(memberspercent);
        } else if (msg.sender == tempright.sponsors[0]) {
            require(hasRole(ADD_MEMBER_ROLE, tempright.sponsors[0]), "NSA");
            tempright.sponsors.concat(members);
            tempright.sponsorpercent.concat(memberspercent);
        } else if (msg.sender == tempright.thanks[0]) {
            require(hasRole(ADD_MEMBER_ROLE, tempright.thanks[0]), "NTA");
            tempright.thanks.concat(members);
            tempright.thankpercent.concat(memberspercent);
        }
    ...
    function setRoyaltyPercent(uint _tokenId, address[] memory members, uint32[] memory percentages) external view onlyRole(ADD_MEMBER_ROLE) {
        require(exists(_tokenId), "NT");

        RightUtils.Right memory tempright = getItem(_tokenId);
        if (msg.sender == tempright.owners[0]) {
            require(balanceOf(msg.sender, _tokenId) != 0, "CD");
            require(tempright.owners.length == percentages.length);
            tempright.owners.concat(members);
        } else if (msg.sender == tempright.creators[0]) {
            require(hasRole(ADD_MEMBER_ROLE, tempright.creators[0]), "NCA");
            tempright.creators.concat(members);
    ...
    function getItem(uint _tokenId) public view returns (RightUtils.Right memory) {
    ...
    public view returns (RightUtils.RoyaltyInfo memory){
        // require(balanceOf(msg.sender, _tokenId) != 0, "can deal with owned assets");
        // require(exists(_tokenId), "not supported tokenID");

        uint256 tokenId = _tokenId;
        uint256 salePrice = _salePrice;

        (uint _formerID, uint _royaltycount) = royaltycount_ID(tokenId);

        address[] memory _recepient = new address[](_royaltycount);
        uint256[] memory _royaltyAmount = new uint256[](_royaltycount);
        RightUtils.status[] memory _shareflag = new RightUtils.status[](_royaltycount);

        uint index = 0;
        RightUtils.Right memory tempright = Rights[tokenId];

        uint j;
        if (tempright.owners.length == 1) {
            _recepient[index] = tempright.owners[0];
            _shareflag[index] = RightUtils.status.Owners;
            _royaltyAmount[index] = tempright.royalty_table.holdershare;
            index++;
        } else if (tempright.owners.length > 1) {
            for (j = 0; j < tempright.owners.length; j++) {
                _recepient[index] = tempright.owners[j];
                _shareflag[index] = RightUtils.status.Owners;
                _royaltyAmount[index] = tempright.ownerspercent[j] * tempright.royalty_table.holdershare / 10000;
                index++;
            }
        }
        if (tempright.creators.length > 0) {
            for (j = 0; j < tempright.creators.length; j++) {
                _recepient[index] = tempright.creators[j];
                _shareflag[index] = RightUtils.status.Creators;
                _royaltyAmount[index] = tempright.creatorpercent[j] * tempright.royalty_table.creatorshare / 10000;
                index++;
            }
        }
        if (tempright.sponsors.length > 0) {
            for (j = 0; j < tempright.sponsors.length; j++) {
                _recepient[index] = tempright.sponsors[j];
                _shareflag[index] = RightUtils.status.Sponsors;
                _royaltyAmount[index] = tempright.sponsorpercent[j] * tempright.royalty_table.sponsorshare / 10000;
                index++;
            }
        }
        if (tempright.thanks.length > 0) {
            for (j = 0; j < tempright.thanks.length; j++) {
                _recepient[index] = tempright.thanks[j];
                _shareflag[index] = RightUtils.status.Thanks;
                _royaltyAmount[index] = tempright.thankpercent[j] * tempright.royalty_table.thanksshare / 10000;
                index++;
            }
        }
        uint _price = tempright.royalty_table.Royalty * salePrice / 10000;

        RightUtils.RoyaltyInfo memory tempRoyaltyInfo =
        RightUtils.RoyaltyInfo(_recepient, _royaltyAmount, _shareflag, _formerID, _price);

        return (tempRoyaltyInfo);
    }
}
...
library RightUtils {
    ...
    function createNew(bool cloneable) internal pure returns (Right memory) {
        uint32[] memory zero;
        return Right(0, 0,
            new address[](0), zero, new address[](0), zero,
            new address[](0), zero, new address[](0), zero,
            '', cloneable, 0, royaltyTable(0, 100, 0, 0, 0), ''
        );
    }
}
...
abstract contract DARTsERC2981Base {
    using UintArrayUtils for uint32[];
    using RightUtils for RightUtils.Right;

    mapping(uint => RightUtils.Right) internal Rights;

    function defaultRoyaltyTable(uint _tokenIds) internal returns (RightUtils.royaltyTable memory){
        /*
           Percentage decimal is -2: 0 ~ 10000
         Table; Roylaty, Owner, Creator,  Sponsor,  Thanks;
        */
        uint32 creatorpercent = 0;
        uint32 sponsorpercent = 0;
        uint32 thankpercent = 0;
        uint32 royalty = 0;
        if (Rights[_tokenIds].creators.length != 0) {
            creatorpercent = 1500;
            Rights[_tokenIds].creatorpercent = UintArrayUtils.defaultSetRoyaltyForMember(Rights[_tokenIds].creators.length);
        }
        if (Rights[_tokenIds].sponsors.length != 0) {
            sponsorpercent = 1000;
            Rights[_tokenIds].sponsorpercent = UintArrayUtils.defaultSetRoyaltyForMember(Rights[_tokenIds].sponsors.length);
        }
        if (Rights[_tokenIds].thanks.length != 0) {
            thankpercent = 1000;
            Rights[_tokenIds].thankpercent = UintArrayUtils.defaultSetRoyaltyForMember(Rights[_tokenIds].thanks.length);
        }
        if (Rights[_tokenIds].order != 0) {royalty = 5000;}

        uint32 holderspercent = 10000 - creatorpercent - sponsorpercent - thankpercent - royalty;
        Rights[_tokenIds].ownerspercent = UintArrayUtils.defaultSetRoyaltyForMember(Rights[_tokenIds].owners.length);

        return Rights[_tokenIds].royalty_table = RightUtils.royaltyTable(royalty, holderspercent, creatorpercent, sponsorpercent, thankpercent);
    }


    // memory allocation purpose, if ID inputs, output formerID , number of owners, current order returns
    function royaltycount_ID(uint256 tokenId) public view virtual returns (uint, uint){
        return (
            Rights[tokenId].formerId,
            Rights[tokenId].owners.length + Rights[tokenId].creators.length + Rights[tokenId].sponsors.length + Rights[tokenId].thanks.length
        );
    }

    // memory allocation purpose, if ID inputs, output formerID , number of owners, current order returns
    function royaltyID_order(uint256 tokenId) internal view virtual returns (uint, uint){
        return (Rights[tokenId].formerId, Rights[tokenId].order);
    }
}
