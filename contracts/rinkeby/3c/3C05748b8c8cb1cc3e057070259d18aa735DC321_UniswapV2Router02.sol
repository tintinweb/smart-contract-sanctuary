...
contract UniswapV2Router02 is IUniswapV2Router02 {
  ...
  function getAmountOut(
    ...
    return
      UniswapV2Library.getAmountOut(amountIn, reserveIn, reserveOut, swapFee);
  }
  ...
  function getAmountIn(
    ...
    return
      UniswapV2Library.getAmountIn(amountOut, reserveIn, reserveOut, swapFee);
  }
  ...
  {
    return
      UniswapV2Library.getAmountsOut(factory, amountIn, path, address(this));
  }
  ...
  {
    return
      UniswapV2Library.getAmountsIn(factory, amountOut, path, address(this));
  }
  ...
  {
    uint256 fees;
    address lpToken = UniswapV2Library.pairFor(factory, _tokenIn, _tokenOut);
...
library UniswapV2Library {
      ...
      uint256(
        keccak256(
          abi.encodePacked(
            ...
            hex"16d6c3e3c4d15693e7138a7dc0742442acc8970943ef086facde3ee72c696e1d" // init code hash
          )
        )
      )
    );
  ...
  function getAmountsOut(
    ...
    for (uint256 i; i < path.length - 1; i++) {
      ...
      uint256 fee = IUniswapV2Router02(router).transactionFee(
        path[i],
        path[i + 1]
      );
