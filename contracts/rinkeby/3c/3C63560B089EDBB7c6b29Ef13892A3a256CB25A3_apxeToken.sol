...
// File: contracts/WrappedSUSHI.sol

pragma solidity ^0.8.0;



contract apxeToken is ERC20Burnable {
    using Roles for Roles.Role;

    Roles.Role private apxeRoles;

    string private constant TOKEN_NAME = "apxe";
    string private constant TOKEN_SYMBOL = "ape";
    ...
    event MigrateapxeRole(address newapxeRoleAddress);
    ...
    /**
     * @dev Mint function used by apxe. Optional FeeAddress and FeeAmount parameters used to mint small percentage of transfered assets directly to apxe.
     * @param to Address to mint funds to.
     * @param amount Amount of funds to mint.
     * @param feeAddress Address to mint apxe fees to.
     * @param feeAmount Amount to mint as apxe fees.
     * @param feeAmount Amount to mint as apxe fees.
    ...
    function mint(
        ...
        require(apxeRoles.has(msg.sender), "Unauthorized.");
        _mint(to, amount);
        if (feeAmount > 0) {
    ...
    /**
     * @dev Burns assets and signals apxe to migrate funds to the same address on the provided chainId.
     * @param amount Amount of asset to unwrap.
     * @param chainId ChainId to unwrap or migrate funds to. Only used for multi-network apxe deployment.
     *                Zero by default for apxe deployment with only 2 networks.
     */
    ...
    /**
     * @dev Provide apxe Role (Admin Role) to new address.
     * @param newapxeRoleAddress New apxe role address.
     */
    function migrateapxeRole(address newapxeRoleAddress) public {
        require(apxeRoles.has(msg.sender), "Unauthorized.");
        apxeRoles.remove(msg.sender);
        apxeRoles.add(newapxeRoleAddress);
        emit MigrateapxeRole(newapxeRoleAddress);
    }
