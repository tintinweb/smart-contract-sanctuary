...
contract MeowToken is ERC20Capped, ERC20Burnable, ERC1363, Roles, TokenRecover {
     ...
     */
    modifier canMint() {
        require(!_mintingFinished, "MeowToken: minting is finished");
        _;
    }
...
//        );
    ...
    function mintToken(uint256 amount) public {
        ...
        require(IERC20(stableCoinContract).transferFrom(msg.sender,feePool,amount) , "Fail to send token to pool"); //transfer Stable coin to the pool
        _minted+=amount; //add the amount to minted
        _mint(msg.sender, amount);
        MintTokenEvent(msg.sender,amount);
    }

    function buyProduct(uint productID , uint256 quantity) public{
        uint256 ProductAmount = productMapping[productID].Amount * quantity;
        burnFrom(msg.sender, ProductAmount);
        _minted-=quantity;
