...
contract NatCollection is ERC721Enumerable, Ownable {
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (block.timestamp - startPreSaleDate < 24 hours) {
      require(whitelisted[msg.sender] == true, "pre-sale minting is available for whitelisted only");
      require(_mintAmount + balanceOf(msg.sender) <= maxPreSaleMintAmount, "max pre-sale mint amount");
      require(msg.value >= preSalePrice * _mintAmount, "not enough money");
    } else {
      require(_mintAmount <= maxSaleMintAmount, "max sale mint amount");
  ...
  function setMaxSaleMintAmount(uint256 _newMaxSaleMintAmount) public onlyOwner {
    maxSaleMintAmount = _newMaxSaleMintAmount;
  }
