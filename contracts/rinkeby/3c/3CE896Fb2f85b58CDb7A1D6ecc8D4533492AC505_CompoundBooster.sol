...
interface ICompoundProxyUserTemplate {
    ...
    function repayBorrowBySelf(address _asset, address _liquidatePool,address _underlyingToken) external payable;
...
contract CompoundProxyUserTemplate {
    ...
    ) public payable onlyInited onlyOp {
        autoClaimComp(_asset);

        uint256 borrows = borrowBalanceCurrent(_asset);

        if (_underlyingToken != address(0)) {
            IERC20(_underlyingToken).safeApprove(_asset, 0);
            IERC20(_underlyingToken).safeApprove(_asset, borrows);
...
contract CompoundBooster {
    ...
    function demoInit() public {
        init(
            0x89ca530c084598E23c0DE2BB21EA1CD05924B4b4,
    ...
    function deposit(
        ...
        ICompRewardPool(pool.rewardCompPool).stakeFor(msg.sender, mintToken);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public returns (bool) {
        ...
        ICompRewardPool(pool.rewardCompPool).withdrawFor(msg.sender, _amount);
    ...
    function _repayBorrow(
        ...
        if (isErc20) {
            ...
            if (bal > 0) {
                ...
                payable(address(0)).transfer(lendflareDeposterReward);
    ...
    function liquidate(bytes16 _lendingId) public payable returns (address) {
        LendingInfo storage lendingInfo = lendingInfos[_lendingId];
        PoolInfo memory pool = poolInfo[lendingInfo.pid];
        ...
        ICompoundProxyUserTemplate(lendingInfo.proxyUser).repayBorrowBySelf{
            value: msg.value
        }(pool.lpToken, veLendFlareReward, lendingInfo.underlyToken);
    ...
    function getLendingInfos(bytes16 _lendingId)
        public
        view
