...
contract RocketComponent is ERC721, Ownable {
     ...
     */
    function addEarlyAccess(address[] memory _addresses)
        external
        onlyOwnerOrAdmin
    {
     ...
     */
    function removeEarlyAccess(address[] memory _addresses)
        external
        onlyOwnerOrAdmin
    {
        for (uint256 i; i < _addresses.length; i++) {
...
contract RocketFactory is ERC721, Ownable {
    ...
    constructor() ERC721("Tom Sachs Rocket Factory", "TSRF") {
        utils = new Utils();
    }
        ...
        returns (RocketView memory)
    {
        ...
        bool perfectRocket = utils.compareStrings(nose.brand, body.brand) &&
            utils.compareStrings(body.brand, tail.brand);
        ...
        string memory slug = utils.toSlug(
            string(abi.encodePacked(nosePath, "-", bodyPath, "-", tailPath))
        );
