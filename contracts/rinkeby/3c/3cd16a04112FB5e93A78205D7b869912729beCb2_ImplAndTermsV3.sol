...
contract ImplAndTermsV3 is Storage, Ownable, DoTransfer, ERC20Init {
    ...
    function stake(uint tokenAmount) public {
        stakeInternal(msg.sender, tokenAmount, 0, address(0), address(0));
    }

    function stake(uint tokenAmount, uint holdTime) public {
        stakeInternal(msg.sender, tokenAmount, holdTime, address(0), address(0));
    }

    function stake(uint tokenAmount, uint holdTime, address referer) public {
        stakeInternal(msg.sender, tokenAmount, holdTime, referer, address(0));
    }

    function stake(uint tokenAmount, uint holdTime, address referer, address beneficiary) public {
        stakeInternal(msg.sender, tokenAmount, holdTime, referer, beneficiary);
    }

    function stakeInternal(address staker, uint tokenAmount, uint holdTime, address referer, address beneficiary) internal {
        require(
            referer != staker,
