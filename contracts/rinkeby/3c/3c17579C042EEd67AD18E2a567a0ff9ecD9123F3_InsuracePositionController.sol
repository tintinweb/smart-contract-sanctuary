...
contract InsuracePositionController is AbstractController {
    ...
    function __InsuracePositionController_init(
        ...
        IERC20Internal(_lpToken).approve(
            address(address(_stakersPool)),
            PreciseUnitMath.MAX_UINT_256
        );
    }
    ...
    function stake(uint256 _amount) external override onlyIndex returns (uint256) {
        ...
        require(
            _amount >= _stakingController.minStakeAmtPT(_stakingAsset),
            "InsuracePositionController: IPC1"
        );
        require(
            _amount >= _stakingController.minStakeAmtPT(_stakingAsset),
            "InsuracePositionController: IPC2"
        );
        ...
        require(lptokenBalance >= 0, "InsuracePositionController: IPC4");

        // get amount from controller
        _stakes = _stakes.add(investmentAmount);
    ...
    */
    function callUnstake() external override onlyIndex returns (uint256) {
        require(
            _stakingController.minUnstakeAmtPT(_stakingAsset) <= _stakes &&
                _stakingController.maxUnstakeAmtPT(_stakingAsset) >= _stakes,
            "InsuracePositionController: IPC5"
        );
        require(_stakes != 0, "stakes 0");
        _stakingController.proposeUnstake(_stakes, _stakingAsset);
        return _stakes;
    }

    function unstake() external override onlyIndex returns (uint256) {
        require(_stakes > 0, "InsuracePositionController: IPC6");

        uint256 burnableAmtPH = ILPToken(_lpToken).burnableAmtPH(address(this));
        uint256 lptokenBalance = IERC20Internal(_lpToken).balanceOf(address(this));

        // require(lptokenBalance > 0 && lptokenBalance <= burnableAmtPH, " IPC:6 - B:1");

        // withdraw after unlocking on callUnstake
        _stakingController.withdrawTokens(_stakes, _stakingAsset);

        uint256 _unstakedAmount = IERC20Internal(_stakingAsset).balanceOf(address(this));
        require( _unstakedAmount > 0, "InsuracePositionController: IPC8" );

        // Withdrawal fee
        uint256 withdrawalFee = _stakingController.withdrawFeePT(_stakingAsset);

        // Convert to base
        uint256 baseAmount = _swapAssets(_unstakedAmount, address(base),_stakingAsset);

        // Return to index
        base.safeTransfer(address(index), baseAmount.sub(withdrawalFee));

        setUnstakingState();

        // Subtract fee?
        return baseAmount.sub(withdrawalFee);
    }

    function withdrawRewards() external override {
        uint256 _rewards = 0;
        require(_rewards > 0, "InsuracePositionController: IPC9");

        require(
            IERC20Internal(_insurToken).balanceOf(address(this)) >= _rewards,
        ...
        require(_rewards >= _fee, "InsuracePositionController: IPC10");
        ...
        require(_baseAmount != 0, "InsuracePositionController: IPC11");
    ...
    function outstandingRewards() public view override returns (uint256) {
        return
            _stakersPool.showHarvestRewards(
                address(this),
    ...
    function maxUnstake() external view override returns (uint256 _maxUnstake) {
        _maxUnstake = _stakingController.maxUnstakeAmtPT(_stakingAsset);
    }
