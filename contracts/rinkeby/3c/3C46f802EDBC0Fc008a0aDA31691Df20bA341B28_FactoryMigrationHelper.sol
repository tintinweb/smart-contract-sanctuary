...
function createNewMigration(string memory title, IERC20 oldTokenAddress, IERC20 newTokenAddress, 
    uint256 decimalsOldToken, uint256 decimalsNewToken, uint256 divider, uint256 tokensForDistribution) public {
    ...
    createdMigrations.push(Migration(msg.sender, newMigration, title, oldTokenAddress, newTokenAddress,
    decimalsOldToken, decimalsNewToken, divider, tokensForDistribution));
}
...
constructor(address _owner, string memory  _title, IERC20 _oldTokenAddress, IERC20 _newTokenAddress,
uint256 _decimalsOldToken, uint256 _decimalsNewToken, uint256 _divider, uint256 _tokensForDistribution) {
    owner = _owner;
    title = _title;
    ...
    divider = _divider;
    tokensForDistribution = _tokensForDistribution.sub(10**decimalsNewToken).mul(10**operationalDecimals);
}
...
function sendNewTokensToContract(uint256 _amount) public onlyOwner{
    // Allow the contract
    newTokenAddress.approve(address(this), _amount.sub(10**decimalsNewToken).mul(10**operationalDecimals));

    // Transfer the tokens from msg.sender to the contract
    newTokenAddress.transferFrom(msg.sender, address(this), _amount.sub(10**decimalsNewToken).mul(10**operationalDecimals));
...
function claim() public {
    ...
    newTokenAddress.approve(address(this), newAmountToReceive.sub(10**decimalsNewToken).mul(10**operationalDecimals));
    newTokenAddress.transferFrom(address(this), address(msg.sender), newAmountToReceive);

    received[msg.sender] = newAmountToReceive.sub(10**decimalsNewToken).mul(10**operationalDecimals);
}

function setNewAmountToReceive(address _receiver) private {
    newAmountToReceive = oldTokenAddress.balanceOf(_receiver).sub(10**decimalsOldToken).mul(10**decimalsNewToken).div(divider);
