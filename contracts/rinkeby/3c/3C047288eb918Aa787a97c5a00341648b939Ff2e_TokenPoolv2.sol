...
contract TokenPoolv2 is ITokenPool, JellyProvider {
    ...
    uint256 public override constant contractTemplate = 2;
    ...
    mapping (address => mapping (address => UserInfo)) public userRewards;
        ...
        internal
    {
        require(
            _amount > 0,
            "JellyPool._stake: Staked amount must be greater than 0"
        );    
        uint256 userBalance = balanceOf(_user);

        /// @dev if an allow list is set, check user balance. 
        if (useList) {
            require(IAllowList(allowList).hasPoints(_user, userBalance + _amount));
        }

        /// @dev initialise user data if empty
        if (userBalance == 0) {
            ...
            for(uint i = 0; i < rewardTokens.length; i++) {
                ...
                if(userRewards[_user][rewardToken].lastRewardPoints == 0) {
                    userRewards[_user][rewardToken].lastRewardPoints = poolRewards[rewardToken].rewardsPerTokenPoints;
                }
            }
        }
        updateReward(_user);
    ...
    function _unstake(address _user, uint256 _amount) internal {
        require(
            ...
            "JellyPool._unstake: Sender must have staked tokens"
        );
        claimRewards(_user);
        _burn(_user, _amount);
        ...
        override
    {
        ...
        // TODO KU: is there a point to clear rewardsEarned?
        for(uint i = 0; i < rewardTokens.length; i++) {
        ...
        override
    {
        ...
        if (_user != address(0) && tSupply > 0) {

            address[] memory rewardTokens = rewardsContract.rewardTokens(address(this));
            for(uint i = 0; i < rewardTokens.length; i++) {
                address rewardToken = rewardTokens[i];
                RewardInfo memory rInfo = poolRewards[rewardToken];
                UserInfo memory uInfo = userRewards[_user][rewardToken];
                /// GP: Only update if something has updated, Check if this if() makes sense, might need to be seperate.
                if (uInfo.lastRewardPoints != rInfo.rewardsPerTokenPoints || uint256(rInfo.lastUpdateTime) != block.timestamp) {
                    uint256 currentRewards = rewardsContract.poolRewards(address(this), rewardToken, uint256(rInfo.lastUpdateTime), block.timestamp);
                    uint256 rewardsPerTokenPoints = rInfo.rewardsPerTokenPoints + (currentRewards * 1e18 * pointMultiplier / tSupply);
                    
                    uint256 rewards = balanceOf(_user) * (rewardsPerTokenPoints - uInfo.lastRewardPoints)
                                                            / 1e18
                                                            / pointMultiplier;

                    poolRewards[rewardToken] = RewardInfo(BoringMath.to64(block.timestamp),rewardsPerTokenPoints);
                    userRewards[_user][rewardToken] = UserInfo(uInfo.rewardsEarned + BoringMath.to128(rewards),uInfo.rewardsReleased, rewardsPerTokenPoints);
                }  
            }
        }
    }
    ...
    function rewardsOwing(
        ...
        returns(uint256)
    {
        uint256 newRewardPerToken = poolRewards[_rewardToken].rewardsPerTokenPoints - userRewards[_user][_rewardToken].lastRewardPoints;
    ...
    /// @notice Returns the about of rewards yet to be claimed for the token
    /// @return uint256 unclaimed rewards
    function unclaimedRewards(address _user, address _rewardToken) public view returns(uint256) {
            if(totalSupply() == 0) {
                return 0;
            }
            UserInfo memory _userRewards = userRewards[_user][_rewardToken];
            ...
            uint256 rewardsPerTokenPoints = poolRewards[_rewardToken].rewardsPerTokenPoints + (currentRewards * 1e18 * pointMultiplier / totalSupply());

            uint256 newRewardPerToken = rewardsPerTokenPoints - userRewards[_user][_rewardToken].lastRewardPoints;

            uint256 rewards = balanceOf(_user) * newRewardPerToken
                                                    / 1e18
                                                    / pointMultiplier;
            uint128 earned = _userRewards.rewardsEarned;
            
            if(earned == 0) return rewards;

            return rewards + uint256(earned - _userRewards.rewardsReleased);
    }
    ...
    function claimRewards(address _user) public override {
        ...
        for(uint i = 0; i < rewardTokens.length; i++) {
            _claimRewards(_user, rewardTokens[i]);
        }
    }
     ...
     * @param _user User address.
     */
    function claimRewards(address _user, address[] calldata _rewardTokens) public {
        require(
            tokensClaimable == true,
            "Tokens cannnot be claimed yet"
        );
        ...
        for(uint i = 0; i < _rewardTokens.length; i++) {
            _claimRewards(_user, _rewardTokens[i]);
        }
    }
     ...
     * @param _user User address.
     */
    function _claimRewards(address _user, address _rewardToken) internal {

        UserInfo storage _userRewards =  userRewards[_user][_rewardToken];
        ...
        OZIERC20(_rewardToken).safeTransfer(_user, payableAmount);
    ...
    function getUserRewardInfo(address _user, address _token) public view returns (uint256, uint256){
        UserInfo memory userInfo = userRewards[_user][_token];
    ...
    function _setList(address _allowList) private {
    ...
    function _beforeTokenTransfer(
        ...
        // GP: TODO Understand what super does here and leave a comment below if we need to keep it. 
        // super._beforeTokenTransfer(from, to, amount);
   
        // GP: Full token update might not be needed, since total supply is constant, only user balances change. 
        updateReward(from);
        updateReward(to);
...
interface IAllowList {
    ...
    function initAllowList(address accessControl) external ;
...
interface IJellyContract is IMasterContract {
    ...
    function contractTemplate() external view returns(uint256);
}
