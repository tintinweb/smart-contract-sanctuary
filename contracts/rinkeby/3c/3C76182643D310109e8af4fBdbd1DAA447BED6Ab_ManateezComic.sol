...
contract AlgaeToken is ERC20("Algae", "ALGAE") {
	using SafeMath for uint256;
	...
	uint256 constant public END = 1637871681; // CHANGE THIS BEFORE PROD

	mapping(address => uint256) public rewards;
	mapping(address => uint256) public lastUpdate;

	IAlgae public  ManateezComic;

	event RewardPaid(address indexed user, uint256 reward);

	constructor(address _comic) public{
		ManateezComic = IAlgae(_comic);
	}
	...
	function updateRewardOnMint(address _user, uint256 _amount) external {
		require(msg.sender == address(ManateezComic), "Can't call this");
		uint256 time = min(block.timestamp, END);
		uint256 timerUser = lastUpdate[_user];
		if (timerUser > 0)
			rewards[_user] = rewards[_user].add(ManateezComic.balanceOG(_user).mul(BASE_RATE.mul((time.sub(timerUser)))).div(60)
	...
	function updateReward(address _from, address _to, uint256 _tokenId) external {
		require(msg.sender == address(ManateezComic));
		if (_tokenId < 10001) {
			uint256 time = min(block.timestamp, END);
			uint256 timerFrom = lastUpdate[_from];
			if (timerFrom > 0)
				rewards[_from] += ManateezComic.balanceOG(_from).mul(BASE_RATE.mul((time.sub(timerFrom)))).div(60);
			if (timerFrom != END)
				lastUpdate[_from] = time;
			if (_to != address(0)) {
				uint256 timerTo = lastUpdate[_to];
				if (timerTo > 0)
					rewards[_to] += ManateezComic.balanceOG(_to).mul(BASE_RATE.mul((time.sub(timerTo)))).div(60);
				if (timerTo != END)
					lastUpdate[_to] = time;
			}
		}
	...
	function getTotalClaimable(address _user) external view returns(uint256) {
		uint256 time = min(block.timestamp, END);
		uint256 pending = ManateezComic.balanceOG(_user).mul(BASE_RATE.mul((time.sub(lastUpdate[_user])))).div(60);
		return rewards[_user] + pending;
	}
}

// File contracts/ManateezComic.sol

/**
 * @title Mighty Manateez Comic Issue #1 contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract ManateezComic is ERC721, Ownable {
    using SafeMath for uint256;
    
    uint256 public constant ComicPrice = 10000000000000000; // 0.01 ETH
    
    uint256 public MAX_COMICS = 10000;
    bool public isLive = false;
    bool public canBurn = false;

    mapping(uint256 => ManateezComic) public comic;
    ...
    constructor() ERC721("Manateez Comic Issue #1", "MMCOMIC") {
    ...
    function mintComics(uint numberOfTokens) public payable {
        ...
        require(ComicPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");

         for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply();
            if (totalSupply() < MAX_COMICS) {
                _safeMint(msg.sender, mintIndex);
                algaeToken.updateRewardOnMint(msg.sender, 1);
    ...
    function getReward() external {
		algaeToken.updateReward(msg.sender, address(0), 0);
		algaeToken.getReward(msg.sender);
	}

	function transferFrom(address from, address to, uint256 tokenId) public override {
		algaeToken.updateReward(from, to, tokenId);
		if (tokenId < 10001)
		{
			balanceOG[from]--;
			balanceOG[to]++;
