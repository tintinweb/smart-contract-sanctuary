...
contract TierOneFighters is ERC721 {
    ...
    /**
     * @dev getter function for Launch
     */
    function getLaunchStatus() external view returns(bool) {
        return launched;
    }
    ...
    /**
     * @dev adds tokens to the max supply  
     */
    function addToTokenMaxSupply(uint _amountToAdd) external onlyOwner {
        require(_amountToAdd % 8 == 0);
        totalSupply += _amountToAdd;
    }

    /**
     * @dev getter function for max supply of tier one tokens
     */
    function getMaxTokenSupply() external view returns(uint) {
        return totalSupply;
    }
     ...
     * @dev makes sure that eth r is paid before minting
     */
