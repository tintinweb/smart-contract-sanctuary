...
import "./Essence.sol";
 ...
 contract Dracoverse is ERC721, Ownable {
    ...
    constructor() ERC721("Dracoverse", "DRACO") { 
    }

    function adminMint() public onlyOwner {
        require(totalSupply + TEAM_SUPPLY <= maxDraco,      "You have missed your chance");
    ...
    function freeMint(uint256 numberOfMints, bytes32[] calldata _merkleProof) public {
        uint256 total = freeWhitelist[msg.sender] + numberOfMints;
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "Invalid proof");
        require(presaleActive || saleActive,                        "A sale period must be active to mint");
        require(MAX_FREE >= total,                                  "Can't mint more than reserved");
        require(totalSupply + numberOfMints <= maxDraco,            "Claim would exceed max supply of tokens");
        freeWhitelist[msg.sender] = total;
    ...
    function presaleMint(uint256 numberOfMints, bytes32[] calldata _merkleProof) public payable {
        uint256 total = presaleWhitelist[msg.sender] + numberOfMints;
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRoot, leaf),             "Invalid proof");
        require(presaleActive || saleActive,                                    "A sale period must be active to mint");
        require(MAX_WHITELIST >= total,                                         "Can't mint more than reserved");
        require(totalSupply + numberOfMints + freeMintCount <= maxDraco,        "Purchase would exceed max supply of tokens");
    ...
    function editEssence(Essence _essence) public onlyOwner {
        essence = _essence;
    }
    ...
    function claim(uint256[] calldata tokenIds) public {
        ...
        essence.mintToken(msg.sender, total);
    }    
    ...
    function freeCount(bytes32[] calldata _merkleProof, address _address) external view returns(uint256 count) {
        bytes32 leaf = keccak256(abi.encodePacked(_address));
        if(MerkleProof.verify(_merkleProof, merkleRoot, leaf)) {
            return MAX_FREE - freeWhitelist[_address];
        } else {
            return 0;
        }
    }
