...
contract Random is VRFConsumerBase, Ownable {
    ...
    modifier onlyAdmin() {
        require(owner() == _msgSender() || admins[_msgSender()] || admins[tx.origin], "You are not an admin!");
        _;
    }

    function random(uint256 seed) external view onlyAdmin returns (uint256) {
     ...
     * Requests randomness
     */
    function refreshRandom() external onlyAdmin returns (bytes32 requestId) {
        if (randomNumber != 0) {
          if (cooldown == 0) {
            return 0x000;
          }

          if (counter.current() < cooldown) {
            counter.increment();
            return 0x000;
          }
          ...
          require(LINK.balanceOf(address(this)) >= fee, "Not enough link to generate random number!");
        }
    ...
    function setCooldown(uint256 cd) external onlyAdmin {
      require(cd != cooldown, "Cooldown is the same");
      cooldown = cd;
    }
