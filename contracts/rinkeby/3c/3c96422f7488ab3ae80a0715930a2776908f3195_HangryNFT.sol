...
contract HangryNFT is ERC721PresetMinterPauserAutoId, Ownable {
    ...
    address payable platformAddress = payable(0x1568976f91979a5263C297D50a9f3F3e7F892E3D);
    ...
    uint256 public constant totalTokenToMint = 20;
    uint256 public mintedTokens;
    uint256 public startingIpfsId;
    uint256 public howManyToMint = 1;
    uint256 public nftPerAddressLimit = 3;
    uint256 private lastIPFSID;
    uint256[] public excludedNumbers;
    uint256 public constant ADMIN_MINT = 4;
    ...
    modifier adminOnly() {
        require(hasRole(ADMIN_ROLE, msg.sender),"HangryNFT: caller is not an admin!");
        _;
    }
    ...
    //Main Token Function - Payable
    function createTokens(uint256 _howMany) external payable {
        require(!paused(), "HangryNFT: Contract paused, no NFTs can be minted right now.");
        require(_howMany > 0,"HangryNFT: minimum 1 tokens need to be minted!");
        require(_howMany <= tokensRemainingToBeMinted(),"HangryNFT: purchase amount is greater than the token available!");
        require(_howMany <= howManyToMint,"HangryNFT: max tokens you can mint at once exceeded!, reduce your amount of tokens you want to mint");
        ...
        if(msg.sender != owner()){
            ...
            require(ownerMintedCount + _howMany <= nftPerAddressLimit,"NFTs Per Address during mint is limited to allow fair purchases.");
        }
        
        require(msg.value >= price.mul(_howMany),"HangryNFT: insufficient ETH to mint! Try minting less NFTs");
    ...
    //Internal Function that triggers the actual minting of tokens
    function _mintToken(address to) private {
        if (mintedTokens == 0) {
            lastIPFSID = getRandom(1,totalTokenToMint,uint256(uint160(address(_msgSender()))) + 1);
            startingIpfsId = lastIPFSID;
        } else {
            lastIPFSID = getIpfsIdToMint();
        }
        mintedTokens++;
        require(!_exists(mintedTokens),"HangryNFT: one of these tokens already exists!");
        _safeMint(to, mintedTokens);
        _setTokenURI(mintedTokens, lastIPFSID.toString());
    }

    //Mint Tokens via Admin Function - Used for Private Sale & Competition Winners Token Distribution
    function mintTokenAdmin(uint256 _howMany, address to)
        external
        adminOnly
    {
        require(_howMany > 0,"HangryNFT: minimum 1 tokens need to be minted!");
        require(_howMany <= 100, "YourNFToken: max 100 tokens at once!");
        require(getAdminRemainingMint() > 0,"HangryNFT: All admin tokens minted");
        require(_howMany <= getAdminRemainingMint(), "HangryNFT: Token should be less than remaining amount to mint");
        if (mintedTokens == 0) {
            lastIPFSID = getRandom(1,totalTokenToMint,uint256(uint160(address(_msgSender()))) + 1);
            startingIpfsId = lastIPFSID;
        } else {
    ...
    function getIpfsIdToMint() private view returns (uint256 _nextIpfsId) {
        require(!isAllTokenMinted(),"HangryNFT: all tokens have been minted!");
    ...
    function setBaseURI(string memory baseURI_) public onlyOwner {
        require(locked == false,"ERC721Metadata: BaseURI Now Locked. No further changes permitted");
    ...
    function setNotRevealedURI(string memory _initNotRevealedUri) public onlyOwner {
        require(locked == false,"ERC721Metadata: NotRevealedURI Now Locked. No further changes permitted");
        notRevealedURI = _initNotRevealedUri;
    }

    //Intercept Token URI to show notRevealed Image until Reveal is triggered
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory){
        require(_exists(tokenId),"ERC721Metadata: URI query for nonexistent token");
    ...
    //Set and Remove Whitelist Addresses
    ...
    //Set How many permitted to mint per transaction
    function setHowMany(uint256 _howMany) public onlyOwner {
        howManyToMint = _howMany;
    }
    
    //Set Token Per Address Limit During Presale
    ...
    //Withdraw funds if every it gets stuck in contract to admin wallet
