...
contract SimpleCollectible is ERC721, Ownable {
    ...
    function createCollectible() public payable returns (uint256) {
        ...
        tokenCounter++;
        artistBalance += (msg.value * 95) / 100;
        blueOceanBalance += msg.value - artistBalance;
        return tokenId;
    }

    function artistWithdraw() public payable onlyOwner {
        msg.sender.transfer(artistBalance);
        artistBalance = 0;
    }

    function blueOceanWithdraw() public payable onlyBlueOcean {
        msg.sender.transfer(blueOceanBalance);
        blueOceanBalance = 0;
    }

    modifier onlyBlueOcean() {
        require(msg.sender == blueOcean);
        _;
    }
}