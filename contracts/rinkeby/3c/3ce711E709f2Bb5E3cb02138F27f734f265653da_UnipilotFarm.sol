...
contract UnipilotFarm is IUnipilotFarm, ReentrancyGuard, IERC721Receiver {
    ...
    //mapping poolAltInfo add => PoolAltInfo struct
    ...
    //PILOT
    address internal constant PILOT_TOKEN = 0x18F10404cC1414e6ACBfEA16CCC97D9ed303e98C;
    ...
    function updatePilotPerBlock(uint256 _value) external override onlyGovernance {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            if (poolWhitelist[pools[i]]) {
                ...
                updatedPools.push(pools[i]);
            }
        }

        emit UpdatePilotPerBlock(updatedPools, pilotPerBlock, pilotPerBlock = _value);
    }
        ...
        returns (bool)
    {
        ...
        // (, address pool, uint256 liquidity, , , , ) = IUniswapLiquidityManager(ulm).userPositions(_tokenId);

        // (, , , , , , , , , , uint256 totalLiquidity,,,,,) = IUniswapLiquidityManager(ulm).poolPositions(pool);
        ...
        // uint256 liquidity = positions.pool;
        // address pool = positions.pool;
    ...
    function updateAltToken(address _pool, address _altToken) external {
    ...
    /// @param _pools The number of address of pools from ULM
    function initializer(address[] memory _pools) public override onlyGovernance {
        for (uint256 i = 0; i < _pools.length; i++) {
            if (
    ...
    function getGlobalReward(
        ...
        globalReward = FullMath
            .mulDiv(blockDifference.mul(temp), 1e18, poolInfo[pool].totalLockedLiquidity)
            .add(_globalReward);
    }
        ...
        )
    {
        ...
        DirectTo status = DirectTo.GRforPilot;

        if (isFarmingActive) {
            globalReward = checkLimit(_tokenId, status);

            if (poolState.isAltActive) {
                status = DirectTo.GRforAlt;
                globalAltReward = checkLimit(_tokenId, status);
            } else {
        ...
        pilotReward = (userReward.mul(userState.liquidity)).div(1e18);
        uint256 userAltReward = globalAltReward.sub(userState.altReward);
        altReward = (userAltReward.mul(userState.liquidity)).div(1e18);
    }
    ...
    function insertPool(address _pool) internal {
    ...
    function altDeposit(address _pool, uint256 _tokenId) internal {
...
interface IUnipilotFarm {
    ...
    event UpdateUnipilot(address oldAddress, address newAddress, uint256 time);

    event UpdatePilotPerBlock(address[] pool, uint256 old, uint256 updated);
