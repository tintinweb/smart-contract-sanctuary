...
contract Coof1155 is PermitControl, ERC165, IERC1155, IERC1155MetadataURI {
  ...
  */
  mapping (address => uint256) public coofed;
  ...
  */
  event Infected(address indexed _host, uint256 indexed _id);
  ...
  */
  ...
  /**
  */
  function _infect(address _coofed) internal {
    uint256 roll = uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, _msgSender(), _coofed)));
    if((roll % 10) < rNaught){
      ++variant;
    }
    ...
    this.mintBatch(_coofed, _asSingletonArray(id), _asSingletonArray(uint256(1)), "");
    emit Infected(_coofed, id);
  }
