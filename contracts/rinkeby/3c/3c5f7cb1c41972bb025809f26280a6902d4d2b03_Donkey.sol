...
// GO TO LINE 1889 TO SEE WHERE THE DONKEY CONTRACT STARTS
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (_startingIndex > 0) {
            return string(abi.encodePacked(base, ( (tokenId + _startingIndex) % 10000 ).toString()));
        }
    ...
    */
    ...
    function getTokenGen(uint256 tokenId) public view virtual returns (uint256) {
        return _tokenGen[tokenId];
    }

    function setTokenGen(uint256 tokenId, uint256 gen) public {
        _tokenGen[tokenId] = gen;
    }
    
    function increaseTokenGen(uint256 tokenId) public {
...
contract Donkey is ERC721, Ownable {
    ...
    // sell ending price
    uint256 public constant donkeyMinPrice = 25000000000000000; // 0.025 ETH
    
    uint256 public publicSaleDonkeyStartingPrice;
    ...
    // ############################# constructor #############################
    constructor() ERC721("Donkeys", "DNK") { }
    ...
    function validateSign(address accountAddress, bytes memory sign) internal pure returns (bool) {
        ...
        onlyOwner
    {
        ...
        publicSaleDonkeyStartingPrice = saleStartPrice;
    ...
    function getMintPrice() public view whenPublicSaleActive returns (uint256) {
        ...
        if (elapsed >= publicSaleDuration) {
            return donkeyMinPrice;
        } else {
            uint256 currentPrice = ((publicSaleDuration - elapsed) *
                publicSaleDonkeyStartingPrice) / publicSaleDuration;
            return
                currentPrice > donkeyMinPrice
                    ? currentPrice
                    : donkeyMinPrice;
        }
    }
    ...
    function mintDonkey(uint numberOfTokens, bytes memory sign) public payable {
        require(publicSaleIsActive, "Sale must be active to mint Donkey");
        ...
        bool isValidSign = validateSign(msg.sender, sign);
        require(isValidSign, "signature is not valid");
