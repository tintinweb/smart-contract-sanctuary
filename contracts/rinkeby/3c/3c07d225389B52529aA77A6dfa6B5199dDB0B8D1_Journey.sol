...
contract Journey is ERC721Holder, Ownable {
    ...
    function lordsIssurance() public view returns (uint256) {
        return lordsPerRealm;
    }

    function updateRealmsAddress(address _new) external onlyOwner {
        realmsAddress = _new;
    }

    function updateLordsAddress(address _new) external onlyOwner {
        lordsToken = _new;
    }
        ...
        returns (uint256 lords)
    {
        ...
        for (uint256 i = weekStaked[_player]; i < _week(); i++) {
            totalClaimable =
                totalClaimable +
