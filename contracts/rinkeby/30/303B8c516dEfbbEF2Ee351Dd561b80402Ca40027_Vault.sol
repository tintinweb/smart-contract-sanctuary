...
contract Vault is IERC20, ReentrancyGuard, Ownable {
	...
	function withdraw(uint256 amount, address recipient) external nonReentrant notLocked returns (uint256) {
		...
		if (amount == type(uint256).max) {
			...
			amount = (shares * totalSupply) / currentSupply;
		} else shares = (amount * currentSupply) / totalSupply; // calculate real share
		...
		token.transfer(recipient, (amount * 10**token.decimals()) / (10**decimals));
