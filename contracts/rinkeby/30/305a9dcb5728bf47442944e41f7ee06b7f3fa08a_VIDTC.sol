...
contract VIDTC is Pausable, IERC165, IERC721, IERC721Metadata, IERC721Enumerable {
	...
	function verifyToken(uint256 tokenID, string calldata hash) external view returns (bool) {
		return (keccak256(abi.encodePacked((idToHash[tokenID]))) == keccak256(abi.encodePacked((hash))));
	}

	function verifyToken(address nftContract, uint256 tokenID, string calldata hash) external view returns (bool) {
        return (Verifications[nftContract][tokenID].verified && keccak256(abi.encodePacked((Verifications[nftContract][tokenID].hash))) == keccak256(abi.encodePacked((hash))));
	}
	...
	function verifyHash(string calldata hash) external view returns (uint256) {
		return hashToId[hash];
	}

	function createMyNFT(bytes calldata data) public whenNotPaused returns (uint256) { 
		require(data.length == 64,"C1 - Invalid hash provided");
		require(verifiedPublishers[msg.sender],"C2 - Unverified publisher address");
		string memory hash = string(data);
		require(hashToId[hash] == 0,"C3 - NFT exists already");
	...
	function createMyNFT(string calldata hash) public whenNotPaused returns (uint256) { 
		require(bytes(hash).length == 64,"C1 - Invalid hash provided");
		require(verifiedPublishers[msg.sender],"C2 - Unverified publisher address");
		require(hashToId[hash] == 0,"C3 - NFT exists already");
	...
	function createNFT(bytes calldata data) public whenNotPaused onlyTrusted returns (uint256) { 
		require(data.length == 64,"CP1 - Invalid hash provided");
		string memory hash = string(data);
		require(hashToId[hash] == 0,"CP2 - NFT exists already");
	...
	function createNFT(string memory hash) public whenNotPaused onlyTrusted returns (uint256) { 
		require(bytes(hash).length == 64,"CP1 - Invalid hash provided");
		require(hashToId[hash] == 0,"CP2 - NFT exists already");
	...
	function requestVerification(address nftContract, uint256 tokenID, string memory hash) public whenNotPaused payable returns (bool) {
	...
	function approveVerification(address nftContract, uint256 tokenID, address originalCreator, string memory hash) public onlyController returns (bool) {
	    Verifications[nftContract][tokenID] = Verification({
            hash: hash,
            verified: true,
	...
	function deployNFT(address Publisher, bytes calldata data) onlyController public returns (uint256) { 
