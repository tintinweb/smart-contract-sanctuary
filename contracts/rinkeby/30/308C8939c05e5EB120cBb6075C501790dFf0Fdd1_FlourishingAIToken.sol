...
abstract contract ERC20Vestable is ERC20Burnable, AccessControl {
    ...
    // ===========================================================
    // === Adminstrators's config TimeLock transferring functions.
    // ===========================================================

    // Set a batch of addresses as whitelists.
    function setWhiteList(address[] memory whiteListAddresses) public {
        ...
        for (uint i = 0; i < whiteListAddresses.length; i++) {
          ...
          whiteLists[whiteListAddresses[i]] = true;
        }
    }
        ...
        returns (bool)
    {
        if(endTimeLock != 0 && block.timestamp <= endTimeLock) {
            require(whiteLists[_msgSender()], "sender isn't allowed to transfer until lock time end");
        }
    ...
    {
        if(endTimeLock != 0 && block.timestamp <= endTimeLock) {
            require(whiteLists[from], "sender isn't allowed to transfer until lock time end");
        }
