...
contract ReciprocalDrop {
    ...
    function initializeDrop(uint supply, address[] memory valuedCompliantCollections, string memory baseUri, string memory name, string memory description, address minterProxy) public returns(uint) {
        ...
        for (uint i = 0; i < valuedCompliantCollections.length; i++) {
            if (drop.usedCollections[valuedCompliantCollections[i]] == false) {
                drop.valuedCompliantCollections.push(valuedCompliantCollections[i]);
                drop.usedCollections[valuedCompliantCollections[i]] = true;
            }
        }
    ...
    function getNftBalances(uint dropId, address owner) public view returns(string[] memory, uint[] memory) {
        uint[] memory balances = new uint[](getDrop(dropId).valuedCompliantCollections.length);
        string[] memory names = new string[](getDrop(dropId).valuedCompliantCollections.length);

        for (uint i = 0; i < getDrop(dropId).valuedCompliantCollections.length; i++) {
            uint balance = InterfaceErc721(getDrop(dropId).valuedCompliantCollections[i]).balanceOf(owner);
            string memory name = InterfaceErc721(getDrop(dropId).valuedCompliantCollections[i]).name();
    ...
    function executeProxyMint(address valuedContractAddress, address owner, uint tokenId, uint dropId) public payable returns(uint) {
        require(isValued(valuedContractAddress, dropId), "This is not a valued collection by this drop.");
        require(verifiedNftOwner(valuedContractAddress, owner, tokenId), "You must be an owner to reciprocal mint.");
        require(isUsedNft(getAddressUintHashed(valuedContractAddress, tokenId), dropId) == false, "This NFT was already used.");
        (bool success, uint proxyTokenId) = IProxyMint(valuedContractAddress).proxyMint{value:msg.value}(owner);
    ...
    function getValuedCollections(uint dropId) public view returns (address[] memory) {
        return getDrop(dropId).valuedCompliantCollections;
    }
