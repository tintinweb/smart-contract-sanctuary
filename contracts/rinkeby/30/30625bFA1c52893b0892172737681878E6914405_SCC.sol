...
contract SCC is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("testCard", "tCC") {
    }
    ...
    // mint for whitelist
    function mintWhiteList(uint256 level) external {
        Card memory card = cardList[level];
        require(bytes(card.tokenURI).length != 0, "card not exist");
        require(whiteList[level][msg.sender], "not in whitelist");
        _mintCard(level, 1, msg.sender);
        whiteList[level][msg.sender] = false;
    }
    ...
    function addWhiteList(uint256 level, address[] calldata list) public onlyOwner {
        require(bytes(cardList[level].tokenURI).length != 0, "card not exist");
        for(uint256 i = 0; i < list.length; i++) {
            whiteList[level][list[i]] = true;
        }
    }

    function removeFromList(uint256 level, address[] calldata list) public onlyOwner {
        require(bytes(cardList[level].tokenURI).length != 0, "card not exist");
        for(uint256 i = 0; i < list.length; i++) {
            whiteList[level][list[i]] = false;
        }
    }

    function isWhiteListed(uint256 level, address addr) public view returns (bool) {
        return whiteList[level][addr];
    }

    function isWhiteListedAll(address addr) public view returns (bool[] memory) {
        bool[] memory cards = new bool[](cardNum);
        for(uint256 i = 0; i < cardNum; i++) {
            cards[i] = whiteList[i][addr];
        }
        return cards;
    }
