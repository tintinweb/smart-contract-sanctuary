...
interface IDelfyStaking {
    ...
    event Claimed(address indexed pid, address indexed sender, uint256 amount);
...
contract DelfyStakingPool is IDelfyStaking, Ownable, ReentrancyGuard {
    ...
    function updatePool(address _pid) internal {
        ...
        returns (uint256)
    {
        ...
        uint256 lpSupply = IERC20(_pool.lpToken).balanceOf(address(this));
    ...
    function deposit(
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                ...
                if (_withdrawRewards) {
                    safeDelfyTransfer(_msgSender(), pending);
    ...
    function claim(address pid) external override nonReentrant {
        ...
        if (pending > 0 || user.pendingReward > 0) {
            ...
            emit Claimed(pid, _msgSender(), pending);
            user.pendingReward = 0;
        }
        user.rewardDebt = user.amount.mul(_pool.accDelfyPerShare).div(1e12);
    }
