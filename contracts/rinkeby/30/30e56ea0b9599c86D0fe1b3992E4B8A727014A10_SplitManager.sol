...
import "./interfaces/ILock.sol";
  ...
  * @dev 
  */
contract SplitManager is BaseGovernanceWithUserUpgradable {
    uint public initialLockedPart;
    ...
    function __SplitManager_init(ILock _lock, uint256 _initialLockedPart, address governanceAddress) internal initializer {
        __BaseGovernanceWithUser_init(governanceAddress);
        __SplitManager_init_unchained(_lock, _initialLockedPart);
    }

    function __SplitManager_init_unchained(ILock _lock, uint256 _initialLockedPart) internal initializer {
     ...
     */
    function getLockedPart(uint ID) view public returns(uint) {
        if(_isStoraged[ID]){
    ...
    function _checkSplitPart(uint[] memory _splitParts) pure internal returns(bool) {
        uint splitPartsSum;
