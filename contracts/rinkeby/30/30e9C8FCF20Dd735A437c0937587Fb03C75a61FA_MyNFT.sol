...
contract MyNFT is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public constant TOTAL_PRESALE_SUPPLY = 15;

    uint256 public constant RESERVED_TOKENS = 5;
    uint256 public constant MAX_PRESALE_MINT_PER_TX = 3;
    uint256 public constant MAX_PUBLIC_MINT_PER_TX = 3;
    ...
    function presaleMint(uint256 numberOfTokens) public payable {
        ...
        uint256 finalMintableTokensNumber = 0;

        for (uint256 i = 0; i < numberOfTokens; i++) {
            finalMintableTokensNumber =
                finalMintableTokensNumber +
                getRandomNumberOfMints(2);
        }

        if (TOTAL_PRESALE_SUPPLY - currentTotalSupply < finalMintableTokensNumber) {
            finalMintableTokensNumber = TOTAL_PRESALE_SUPPLY - currentTotalSupply;
        }

        for (uint256 i = 0; i < finalMintableTokensNumber; i++) {
    ...
    function publicSaleMint(uint256 numberOfTokens) public payable {
        ...
        if (MAX_SUPPLY - currentTotalSupply < finalMintableTokensNumber) {
            finalMintableTokensNumber = MAX_SUPPLY - currentTotalSupply;
        }
    ...
    function getRandomNumberOfMints(uint256 number)
        public
        view
        returns (uint256)
    {
