...
contract FishGame is ERC1155, Ownable, ReentrancyGuard{
    ...
    constructor(address _fishermanAddress) ERC1155("https://game.example/api/item/{id}.json") {
        fisherman = IERC721(_fishermanAddress);
        // RandomnessEngine = IVRF(_randomGenAddress);
    }
    ...
    mapping (uint => uint) public level;// LEVEL IS +1 to what is shown. i.e. if it shows from mapping ,level as 3 its level 4
    mapping (uint=> uint) lastClaim;
    mapping (uint => uint) lastClaimFishWorm;
    uint16[]  legendaryFishermanPool;
    mapping(uint=>uint) public goodFishCounter;
    // mapping (uint=>mapping(uint=>uint)) actionTimeStamp;
    ...
    function LevelUpCalculation(uint tokenId) internal view returns(uint) {
        if(isLegendary[tokenId]== true){
            return 10+10*level[tokenId]; //Divide by 100 for testing
        }
        else {
            return 6+2*level[tokenId]; // Divide by 100 for testing
        }
    }
    ...
    function claimWorm(uint[] memory fishermen) external nonReentrant{ 
        for(uint i=0; i<fishermen.length; i++){
            if(lastClaim[fishermen[i]] == 0) { // First Claim
                ...
                require(fisherman.ownerOf(fishermen[i]) == _msgSender(), "Caller doesn't own the token(s)");
                _mint(_msgSender(), 2, calculateWorm(fishermen[i])*1 ether, "");
                lastClaim[fishermen[i]] = block.timestamp;
            }
        }
    }
    ...
    function calculateWorm(uint tokenId) internal view returns(uint){
        if(isLegendary[tokenId] == true){
            return 50*(level[tokenId]+1);
        }
        else{
            return 10*(level[tokenId]+1);
        }
    }
    ...
    function fishingPrice(uint tokenId) internal view returns (uint) {
        if(isLegendary[tokenId] == true){
            return (10-2*level[tokenId]);
        }
        else{
            return (15-3*level[tokenId]);
        }
    }
    ...
    function fish(uint tokenId) external nonReentrant{
        ...
        // uint rand = RandomnessEngine.initiateRandomness(tokenId, actionTimeStamp[2][tokenId])%100;
        uint rand = randomNumberGenerator()%100;
        // actionTimeStamp[2][tokenId] = block.timestamp;
        if(isLegendary[tokenId] == true){
            if(rand<50+10*level[tokenId]){
                _mint(_msgSender(), 1, 1, "");
                goodFishCounter[tokenId]++;
                // actionTimeStamp[1][tokenId] = block.timestamp;
                emit FishStatus("Good fish");
            }
            else{
                emit FishStatus("Bad fish");
                autoCompost(tokenId, 0);
            }
        }
        else{
            if(rand<15*(level[tokenId]+1)){
                _mint(_msgSender(), 1, 1, "");
                goodFishCounter[tokenId]++;
                // actionTimeStamp[1][tokenId] = block.timestamp;
                emit FishStatus("Good fish");
            }
            else if(rand<15*(level[tokenId]+1)+35-5*(level[tokenId])){
                emit FishStatus("Bad fish");
                autoCompost(tokenId, 1);
            }
            else{
    ...
    function autoCompost(uint tokenId,uint fisherType) internal{
        // uint rand = RandomnessEngine.initiateRandomness(tokenId, actionTimeStamp[2][tokenId])%100;
        uint rand = randomNumberGenerator()%100;
        // actionTimeStamp[2][tokenId] = block.timestamp;
        if(fisherType == 1){
            if(rand< 40+10*level[tokenId]){
    ...
    function calculateWormOnGoodFish(uint tokenId) internal view returns(uint){
        ...
        uint goodFishCount = goodFishCounter[tokenId];
        require(goodFishCount > 0, "No good fish yet");
        if(isLegendary[tokenId] == true){
    ...
    function Cook(uint tokenId) external nonReentrant{
        require(fisherman.ownerOf(tokenId) == _msgSender(),"Caller doesn't own the token(s)");
        uint goodFish = goodFishCounter[tokenId];
        require(goodFish>0, "No good fish remains");
        // uint rand = RandomnessEngine.initiateRandomness(tokenId, actionTimeStamp[3][tokenId])%100;
        uint rand = randomNumberGenerator()%100;
        // actionTimeStamp[3][tokenId] = block.timestamp;
        if(isLegendary[tokenId] == true){
            if(rand< 40){
                _burn(_msgSender(), 1, 1);
                goodFishCounter[tokenId]--;
                _mint(_msgSender(), 3, 700 ether,"");
                // actionTimeStamp[3][tokenId] = block.timestamp;
                emit FishStatus("Caviar is cooked");
            }
            else if(rand<75){
                _burn(_msgSender(), 1, 1);
                goodFishCounter[tokenId]--;
                _mint(_msgSender(), 3, 200 ether,"");
                // actionTimeStamp[3][tokenId] = block.timestamp;
                emit FishStatus("Sushi is cooked");
            }
            else{
                _burn(_msgSender(), 1, 1);
                goodFishCounter[tokenId]--;
                _mint(_msgSender(), 3, 75 ether, "");
                // actionTimeStamp[3][tokenId] = block.timestamp;
                emit FishStatus("Tuna Sandwich is cooked");
            }
        }
        else{
            if(rand<10){
                ...
                _burn(_msgSender(), 1, 1);
                goodFishCounter[tokenId]--;
                _mint(_msgSender(), 3, 25 ether, "");
                // actionTimeStamp[3][tokenId] = block.timestamp;
                emit FishStatus("Soup is cooked");
            }
        }
    }
}