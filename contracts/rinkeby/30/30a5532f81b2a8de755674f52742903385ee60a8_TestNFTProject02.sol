...
// Following the recent worldwide pandemic, emerging reports suggest that several token species have begun exhibiting strange characteristics. Our research team located across the globe has commenced efforts to study and document these unusual phenomena.
...
// The fate of the world's tokens depends on it.

// from our website (https://boringtokens.co)

// BoringTokensCo is a community-centered enterprise focussed on preserving our research about the emerging reports that several token species have begun exhibiting strange characteristics following the recent worldwide pandemic. 
...
contract TestNFTProject02 is ERC721, Ownable {
    ...
    // Variables
    uint private tokenReserve = 150;    // Reserved for the team and for giveaways/promotions
    uint256 private tokenPrice = 33000000000000000; // 0.033 ETH
    bool public saleIsActive = false;

    // Withdraw Addresses
    address private creatorAddress = 0xE6C4a47A90Dc0C2C8962440C94f59dec2a40cf73;
    address private partnerAddress = 0x14aC5a4EDC8bea89470403F89EDa4e5d6a003088;

    // Withdraw Allocation
    uint256 private creatorShare = 75;
    uint256 private partnerShare = 25;
    ...
    constructor() ERC721("Fun Faces", "FFUN") { }

    // Make sale Active/Inactive
    function flipSaleState(bool val) public onlyOwner {
        saleIsActive = val;
    }
    ...
    function withdrawShares() public payable onlyOwner {
        uint256 balance = address(this).balance;
        require(balance > 0, "There is nothing to withdraw");

        uint256 creatorShareBalance = balance * (creatorShare / 100);
        uint256 partnerShareBalance = balance * (partnerShare / 100);

        widthdraw(creatorAddress, creatorShareBalance);
        widthdraw(partnerAddress, partnerShareBalance);
    }

    function widthdraw(address withdrawAddress, uint256 amount) private {
