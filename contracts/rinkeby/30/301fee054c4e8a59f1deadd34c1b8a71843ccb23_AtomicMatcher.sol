...
contract AtomicMatcher {
  ...
  address public feeTakerAddress3 = 0x2D045410f002A95EFcEE67759A92518fA3FcE677;
  ...
  function atomicMatch(Order memory order, bytes memory buyerSignature, bytes memory sellerSignature) public {
  ...
  function distributeFunds(Order memory order) internal {
    ...
    uint256 creatorFee = (order.creator == address(0)) ? 0 : orderFullPrice / 10; // creator
    uint256 makerFee = (order.creator == address(0)) ? orderFullPrice / 90 : orderFullPrice / 80; // maker
    uint256 fee1 = orderFullPrice * 3 / 100; // fee 1
    uint256 fee2 = orderFullPrice * 3 / 100; // fee 2
    uint256 fee3 = orderFullPrice * 4 / 100; // fee 3 
