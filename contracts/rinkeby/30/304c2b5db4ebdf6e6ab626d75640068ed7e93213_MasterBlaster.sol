...
contract MasterBlaster is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    address public _charityWalletAddress = 0xC55EcEc6BaBB76a383eC6cD25327012603Cd515A;     // TODO - charity wallet here
    address public _devWalletAddress = 0xfE2ecA6A55F2EB09dfbff5A9fe1c1299102D9555;     // TODO - dev wallet here
    address public _deadWalletAddress = 0x000000000000000000000000000000000000dEaD;     // TODO - dead wallet here
    ...
    uint256 private _otherFee = 6;
    uint256 private _previousOtherFee = 6;
    uint256 public _burnRate = 1;
    uint256 private _previousBurnRate = 1;
    uint256 public _charityFee = 5;
    uint256 private _previouscharityFee = 5;
    ...
    function setBurnFeePercent(uint256 burnRate) external onlyOwner() {
        _burnRate = burnRate;
        _otherFee = _charityFee.add(_devFee).add(_burnRate);
    }
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair) {         
            ...
            _rOwned[_deadWalletAddress] = _rOwned[_deadWalletAddress].add(rBurn);
            _tTotal = _tTotal.sub(tBurn);
            _rTotal = (MAX - (MAX % _tTotal));
            if(_isExcluded[_deadWalletAddress])
                _tOwned[_deadWalletAddress] = _tOwned[_deadWalletAddress].add(tBurn);
        }
