...
contract AIDIMULTICOLL  is ERC1155, Ownable, ERC1155Burnable{
    event Approve(
    ...
    address public WethAddress = 0x6Ad21c4C2e6704f3a3cd8E46E4A9Ce945057d0c6;
    address public Aidiaddress = 0xFf7831662a12d987870eEDcB1eCfe7125Ac1c42f;
    ...
    function getWETHTokenFee() public view returns (uint256) {
        return wethtokenfee;
    }
    function SetserviceFunction(uint256 _serviceValue) public onlyOwner{
        serviceValue = _serviceValue;
    }
     function SetTokenService(uint256 _tokenfee) public onlyOwner {
        tokenfee = _tokenfee;
    }

    function SetWETHTokenService(uint256 _wethtokenfee) public onlyOwner {
        wethtokenfee = _wethtokenfee;
    }
    ...
    function saleToken(
        ...
        require(amount == order_place[from][tokenId].price.mul(nooftoken) , "Amount is Different for the Orderplace");
        checker[tokenId][from] = true;
        _saleToken(from, tokenId, amount, 0x6aF3317a545D1977003e7fDE666d5DAbbCa01CE5);
        saleTokenTransfer(from, tokenId, nooftoken);
    }
    ...
    function _saleToken(
        ...
        if(bidtoken==WethAddress){
             ERC20 t = ERC20(bidtoken);
            uint256 approveValue = t.allowance(msg.sender, address(this));
            require( approveValue >= val, "Approval Value is Different");
            (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], wethtokenfee);
            require( approveValue >= _adminfee.add(roy.add(netamount)), "Approve Value is Different for Admin Fee");
        ...
        else if (bidtoken==Aidiaddress){
            ERC20 t = ERC20(bidtoken);
            uint256 approveValue = t.allowance(msg.sender, address(this));
            require( approveValue >= val, "Approval Value is Different");
            (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], tokenfee);
            require( approveValue >= _adminfee.add(roy.add(netamount)), "Approve Value is Different for Admin Fee");
            t.transferFrom(msg.sender, owner(), _adminfee);
            t.transferFrom(msg.sender,_creator[tokenId],roy);
            t.transferFrom(msg.sender,from,netamount);
        }else{
            require( msg.value == val, "Insufficient Balance is Msg Value");
    ...
    function acceptBId( address tokenAss,address bidaddr, uint256 amount, uint256 tokenId, uint256 NOFToken) public{
        _acceptBId(tokenAss, bidaddr, amount, tokenId, owner());
    ...
    function _acceptBId( address tokenAss,address from, uint256 amount, uint256 tokenId, address admin) internal{


        uint256 val = pBEPent(amount, tokenfee).add(amount);
        if (tokenAss == WethAddress) {
            val = pBEPent(amount, wethtokenfee).add(amount);
        }

        ERC20 t = ERC20(tokenAss);
        uint256 approveValue = t.allowance(from, address(this));
        require( approveValue >= val, "Approve Value Error");
        require(_balances[tokenId][msg.sender] > 0, "Is Not a Owner");
        (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], tokenfee);
        require( approveValue >= _adminfee.add(roy.add(netamount)), "Approve value mismatches with Admin Value");
