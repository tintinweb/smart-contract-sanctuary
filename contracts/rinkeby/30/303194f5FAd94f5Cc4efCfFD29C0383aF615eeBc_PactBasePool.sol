...
contract PactBasePool is Whitelist, TxStorage {
    ...
    function priceUpdate( uint price) public onlyGovernance {
        _price = price;
    }
	...
	function calcPriceEthUdtPact(uint amountInEth) public view returns (uint) {
        uint UsdtEthPrice = IUniswapOracle(_oracleAddress).consultAB(1e18);
        uint detailUsdtEthPrice = uint(1e18).div(UsdtEthPrice);
        uint detailUsdtPactPrice = uint(1e18).div(_price);
        return detailUsdtPactPrice.mul(amountInEth.div(detailUsdtEthPrice));
	}

	function calcPricePactUdtEth(uint amountInPact) public view returns (uint) {
        uint UsdtEthPrice = IUniswapOracle(_oracleAddress).consultAB(1e18);
        uint detailUsdtPactPrice = uint(1e18).div(_price);
        uint detailUsdtEthPrice = uint(1e18).div(UsdtEthPrice);
        return detailUsdtEthPrice.mul(amountInPact.div(detailUsdtPactPrice));    
	}
