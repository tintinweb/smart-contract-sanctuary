...
contract TestNFT is ERC721, Ownable {
    ...
    uint256 public TokenPrice = 10000000000000000; // 0.010 ETH
    uint256 public constant maxTokenPurchase = 30;
    uint256 public MAX_Token = 10000;
    bool public saleIsActive = false;

    constructor() ERC721("TestNFT", "AOS") {}
    ...
    function reserveToken(uint256 reserveAmount) public onlyOwner {
    ...
    function airdropToken(uint256 numToken, address recipient) public onlyOwner {
        require(totalSupply().add(numToken) <= MAX_Token, 'Sale would exceed max supply');
        
        uint256 mintIndex = totalSupply();
        uint256 i;
        for (i = 0; i < numToken; i++) {
            if (mintIndex + i < MAX_Token) {
                _safeMint(recipient, mintIndex + i);
            }
        }
      }
    
     function airdropTokenToMany(address[] memory recipients) external onlyOwner {
        require(totalSupply().add(recipients.length) <= MAX_Token, 'Sale would exceed max supply');
    ...
    function mintToken(uint256 numberOfTokens) public payable {
        ...
        require(
            totalSupply().add(numberOfTokens) <= MAX_Token,
