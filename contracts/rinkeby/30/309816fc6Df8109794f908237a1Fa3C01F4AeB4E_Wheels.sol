...
contract Wheels is ERC721 {
	...
	mapping (string => bool) mintedTokenUris;
		...
		returns (uint256) 
	{
		...
		require(mintedTokenUris[uri] == false);
		passOnEth(msg.value);
		...
		returns (uint256) 
	{
		...
		require(mintedTokenUris[uri] == false);
		require(mintedTokenUris[uriTwo] == false);
		passOnEth(msg.value);
		...
		returns (uint256) 
	{
		...
		require(mintedTokenUris[uri] == false);
		require(mintedTokenUris[uriTwo] == false);
		require(mintedTokenUris[uriThree] == false);
		require(mintedTokenUris[uriFour] == false);
		passOnEth(msg.value);
		...
		mintedTokenUris[uriThree] = true;

		_tokenIds.increment();
		newItemId = _tokenIds.current();
		_mint(to,newItemId);
		_setTokenURI(newItemId, uriFour);
		mintedTokenUris[uriFour] = true;
	...
	function mintMisteryToken(address to, string memory uri) 
		public 
		virtual 
		returns (uint256) 
	{
		require(tokensAvailable == 0,"Can not mint the mistery wheel yet.");
		require(to == destinationAddress, "Can not mint mistery wheel." );
		require(destinationAddress == msg.sender, "You are not allowed to mint the mistery token.");
		require(mintedTokenUris[uri] == false);
