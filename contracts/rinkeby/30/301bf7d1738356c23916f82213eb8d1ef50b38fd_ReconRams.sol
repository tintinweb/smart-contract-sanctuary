...
contract ReconRams is ERC721Enum, Ownable, PaymentSplitter {
    ...
    // 1. Current whitelisting implementation seems bad. Whitelisting 2000+ users potentially costly using mapping. Suggested alternatives?
    // 2  Took an ERC721 derivative implementation called "ERC721P" from another contract. Looks to be a trimmed version of ERC721, saves 0.1 ETH in deployment cost. Valid?
    // 3. Reentrancy guard needed given attempted checks-effects-interaction pattern? I observed contracts using it even with checks-effects-interaction, don't understand why.
    // 4. Removed withdraw() fxn given usage of payment splitter.

    using Strings for uint256;

    uint256 constant public MAX_SUPPLY = 20;
    uint256 constant public MAX_PRESALE_MINT = 2;
    uint256 constant public MAX_PUBLIC_SALE_PER_TX = 3;
    ...
    // COMMENT FOR AUDITOR: The first ten token ids will be manually arranged in IPFS to be the 10 legendaries in collection
    //                      Dev to mint legendaries after deployment.
    ...
    function toggleOnlyWhitelist(bool _state) public onlyOwner {
