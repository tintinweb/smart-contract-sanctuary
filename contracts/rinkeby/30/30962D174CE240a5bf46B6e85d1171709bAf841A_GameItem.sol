...
function append(string memory a, string memory b, string memory c) internal pure returns (string memory) {
    ...
    function mintAll(string memory tokenURI1, string memory tokenURI2, uint256 nb) public{
        for(uint i = 0; i< nb; i++){
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
    
        string memory tokenuri = append(tokenURI1, toString(newItemId), tokenURI2);
