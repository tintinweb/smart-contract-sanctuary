...
contract Nft is ERC721Enumerable, Ownable {
    ...
    constructor(uint256 _maxNftCount, address payable _nftManagerAddress, uint256 _currentPrice, address payable _artistAddress, uint256 _royaltyPercent, string memory nftName, string memory nftSymbol) ERC721(nftName, nftSymbol) {
        ...
        setNewRoyalty(2, nftWallet, nftPercent);
    }
    ...
    function nftBuy() payable public {
    ...
    function _payRoyalties(uint256 _royaltyIndex, uint256 _value) private {
        ...
        if(_royaltyIndex == 1) {
            emit nftRoyaltyPaid(royaltyRecipients[_royaltyIndex], paymentAmount);
        }
    }

    function nftList(uint256 _id, uint256 _price) public {
    ...
    function nftSwap(uint256 _id) payable public {
        require(_swapMap.inserted[_id] == true, "NFT: NFT not available for swap");
        require(_swapMap.price[_id] > 0, "NFT: NFT not available for swap");
        require(msg.value >= _swapMap.price[_id], "NFT: Insufficient money sent for swap");
    ...
    function nftManagerPerformSwap(address _recipient, uint256 _tokenID) public {
        require(msg.sender == nftManagerAddress);
    ...
    function getTradePrice(uint256 _id) public view returns (uint256) {
        if (!_swapMap.inserted[_id]) {
            return 0;
        }
...
contract NftManager is Ownable {
    ...
    function swap(address _recipient, uint256 _tokenID) public {
        require(approvedContracts[msg.sender] == true, "NftManager: Run setApprovedContract for NFT Contract");
