...
contract PreSale is Context {
    ...
    mapping (address => uint) boughtCommon;
    mapping (address => uint) boughtRare;
    mapping (address => uint) boughtLegendary;

    address[] buyersCommon;
    address[] buyersRare;
    address[] buyersLegendary;

    constructor(uint256 _costCommon, uint256 _limitCommon, uint256 _costRare, uint256 _limitRare, uint256 _costLegendary, uint256 _limitLegendary) {
        owner = _msgSender();
        setParams(_costCommon, _limitCommon, _costRare, _limitRare, _costLegendary, _limitLegendary);
    }
    ...
    function setParams(uint256 _costCommon, uint256 _limitCommon, uint256 _costRare, uint256 _limitRare, uint256 _costLegendary, uint256 _limitLegendary) public onlyOwner {
    ...
    receive() external payable {
        if (msg.value == costCommon) {
            if (boughtCommon[msg.sender] == 0) {
                buyersCommon.push(msg.sender);
            }
            boughtCommon[msg.sender] ++;
            boughtCommonCount ++;
            require(boughtCommonCount <= limitCommon);
        } else if (msg.value == costRare) {
            if (boughtRare[msg.sender] == 0) {
                buyersRare.push(msg.sender);
            }
            boughtRare[msg.sender] ++;
            boughtRareCount ++;
            require(boughtRareCount <= limitRare);
        } else if (msg.value == costLegendary) {
            if (boughtLegendary[msg.sender] == 0) {
                buyersLegendary.push(msg.sender);
            }
            boughtLegendary[msg.sender] ++;
            boughtLegendaryCount ++;
            require(boughtLegendaryCount <= limitLegendary);
        } else {
