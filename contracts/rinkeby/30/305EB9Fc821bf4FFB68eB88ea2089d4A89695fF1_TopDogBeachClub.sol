...
contract TopDogBeachClub is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 private constant MAX_DOGS = 7999; // because totalSupply() starts at 0
    ...
    function selling() external view returns (bool status) {
        return _isPublicSaleActive || _isPreSaleActive;
    }

    /*
    * A SHA256 hash representing all 10000 dogs. Will be set once all 10k are born
    */
    ...
    /*
    * Message Pixel Lion "WOOF WOOF" on Discord
    */
    ...
    /*
    * Used by the TDBC website to tracker remaining supply
    */
    function getMintableCount() public view returns (uint256) {
        uint256 dogSupply = totalSupply();

        if (dogSupply >= MAX_DOGS) {
            return 0;
        } else {
            return _isPreSaleActive ? 1 : 10;
        }
    }
