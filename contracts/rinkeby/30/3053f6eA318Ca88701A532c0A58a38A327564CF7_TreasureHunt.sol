...
contract TreasureHunt is BaseContract {
    ...
    DependentContract private _depContract = DependentContract(0xB08423117cBEACCDC8378CC7262349161a32046e);
    ...
    function claimReward(bytes memory signature) external whenNotPaused {
        ...
        require(_depContract.balanceOf(msg.sender) > 0, "You don't have enough tokens");

        bytes32 digest = generateDigest(msg.sender);
        address puzzleKey = _recoverSigner(digest, signature);

        require(key == puzzleKey, "Incorrect answer");
