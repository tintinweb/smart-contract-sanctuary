...
contract GOATZ_V2 is ERC721, Ownable {
    ...
    function _setTokenURI(uint256 tokenId, uint256 _tokenURI) public onlyOwner {
        require(
            _exists(tokenId),
    ...
    constructor() ERC721("GOATz", "GOATZ") {}
    ...
    function _mintGoat(address _to) private {
        ...
        _setTokenURI(purchasedGoatz, _lastIpfsId);
    }

    // RESERVE FUNCTIONS
    function gift(address[] calldata to) external onlyOwner {
        require(
            to.length <= goatzRemainingToBeMinted(),
