...
contract GoldenGrannyPresaleToken is ERC1155PresetMinterPauser, Ownable {
    ...
    constructor() ERC1155PresetMinterPauser("https://staging.goldengrannys.com/api/presale-metadatas/{id}-metadatas.json") {
    }

    function mintPublicFlag(uint256 tokenId) public whenNotPaused {
        require (
            allowedAddresses.contains(msg.sender),
            "Not allowed to mint"
        );
    ...
    function _beforeTokenTransfer(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            if (from != address(0x0) && balanceOf(from, ids[i]) == 0) {
                // If the seller doesn't own any new tokens
    ...
    function isOwner() public view returns (bool) {
        return allOwners.contains(msg.sender);
    }
