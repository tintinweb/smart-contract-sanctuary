    ...
    ReentrancyGuard
{
    ...
    address internal constant ORACLE = 0x8ECbc9df72f77Ab246475cEd505945925146F7F5; // create set
    address private constant INDEX_FUND = 0xaBfF3ac119e83824df568C698441cB9AB843e07c;
    address internal constant UNI_STRATEGY = 0x58474D9c8eC0fb7A45b8B8D5193e89056F70f37a; //create set
    ...
    function _addLiquidityInManager(AddLiquidityManagerParams memory params) private returns (uint256 amount0Base, uint256 amount1Base, uint256 amount0Range, uint256 amount1Range) {
        ...
        if (position.totalLiquidity > 0) {
            ...
            if (position.baseTickLower != 0 && position.baseTickUpper != 0) {
                ...
                {
                    uint256 amount0 = params.amount0Desired.sub(tokenDetails.amount0Added);
                    uint256 amount1 = params.amount1Desired.sub(tokenDetails.amount1Added);

                    if (amount0 == 0 && amount1 == 0) amount0 = 1;
    ...
    function _distributeFeesInPilot(
    ...
    ) private returns (uint256 _index0, uint256 _index1, uint256 _pilotAmount) {
        ...
        if (_token0 == WETH || _token1 == WETH) {
            _pilotAmount = oracle.getPilotAmountWethPair(
                _token1,
                _tokensOwed1,
                _tokensOwed0
            );
            unipilot.mintPilot(_recipient, _pilotAmount);
            ...
            _pilotAmount = oracle.getPilotAmountForTokens(
                _token0,
                _token1,
                _tokensOwed0,
                _tokensOwed1
    ...
    function _transferLiquidity(
        ...
        if (_token0 == WETH || _token1 == WETH) {
            ...
            if (wethToken) {
                ...
                if (altAmount > 0) TransferHelper.safeTransfer(tokenAlt, _recipient, altAmount);
            }
        } else {
            if (amount0Removed > 0)
