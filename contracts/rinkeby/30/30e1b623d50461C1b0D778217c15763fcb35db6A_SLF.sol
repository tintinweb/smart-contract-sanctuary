...
// File: Quest/mintNFT.sol

/**
 *Submitted for verification at Etherscan.io on 2021-03-22
*/

// File: browser/SLF_flat.sol


// File: browser/QuestCrypto/Strings.sol
...
function _toLower(string memory str) public pure returns (string memory) {
    ...
    /*-----------------------To convert a string into bytes 32-------------------*/
...
// File: browser/QuestCrypto/EnumerableSet.sol
...
// File: browser/QuestCrypto/Address.sol
...
// File: browser/QuestCrypto/SafeMath.sol
...
// File: browser/QuestCrypto/IERC165.sol
...
// File: browser/QuestCrypto/IERC721.sol
...
// File: browser/QuestCrypto/ERC165.sol
...
// File: browser/QuestCrypto/IERC721Receiver.sol
...
// File: browser/QuestCrypto/IERC721Enumerable.sol
...
// File: browser/QuestCrypto/IERC721Metadata.sol
...
// File: browser/QuestCrypto/Context.sol
...
// File: browser/QuestCrypto/ERC721.sol
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
     ...
     */
    constructor (string memory name_, string memory symbol_,string memory _baseuri) public {
        _name = name_;
        _symbol = symbol_;
        _baseURI = _baseuri;
    ...
    */
    ...
    function _safeMint(address to, uint256 tokenId, bytes memory _data) internal virtual {
        _mint(to, tokenId);
        _setTokenURI(tokenId,string(_data));
...
contract mintNFT is ERC721{
    ...
    constructor() ERC721("Quest","QST","13.82.217.131:3001/") public{
       contractOwner = msg.sender;
    }
    
    
    /*Only SLF contract can call this function when the transaction is signed*/
    function mint(address to_,uint256 id_,string memory token_uri) public onlyOwner{
         _safeMint(to_,id_,bytes(token_uri));
    }
    
    function changeBaseURI(string memory new_base_uri) public onlyOwner{
        _setBaseURI(new_base_uri);
    }
...
interface AuctionContract{
     function CheckBidderIdentity(string memory,address,string memory) external returns(bool);
     function Enlist_Auction(string memory,uint,uint,uint,uint,uint,string memory) external returns(bool);
     function End_Auction(string memory,string memory) external returns(bool,uint,uint);
}
contract SLC{
    ...
    constructor(string memory _name,
                ...
                address receiver,
                address _AUCTION_DEPLOYED_ADDRESS) public {
        name = _name;
        symbol = _symbol;
        decimals = _decimals;
        owner = receiver;
        _owners[receiver] = true;
        balances[receiver] = _initialSupply;
        totalSupply=_initialSupply;
        AUCTION_DEPLOYED_ADDRESS = _AUCTION_DEPLOYED_ADDRESS;
        emit Transfer(address(0),receiver,_initialSupply);
    }
    ...
    /*EndAuction*/
    function EndAuction(string memory uid,string memory property_id) public onlyOwner returns(bool,uint,uint){

        (bool success,uint owner_money,uint treasury_money) = AuctionContract(AUCTION_DEPLOYED_ADDRESS).End_Auction(uid,property_id);
    ...
    function GET_AUCTION_WIN_SLC(string memory auctionID,address Treasury_Admin,string memory property_id) public returns(bool){
        bool success = AuctionContract(AUCTION_DEPLOYED_ADDRESS).CheckBidderIdentity(auctionID,msg.sender,property_id);
...
library SafeMaths {
    ...
    //     require(_owners[msg.sender],"caller is not SLF contract or Owner");
    //     _;
    // }
...
interface Auction{
     function Store_SLC_Contract_Address(address,string memory) external returns(bool);
}
...
contract SLF{
    ...
    mapping(string => address) public PROPERTY_ERC20_ADDRESS;
    ...
    constructor() public{
        ...
        MINTNFT_DEPLOYED_ADDRESS = address(nft);
    }
    ...
    /*Store Auction Contract Deployed Address*/
    function SET_AUCTION_DEPLOYED_ADDRESS(address _address) public onlyOwner returns(bool){
        AUCTION_DEPLOYED_ADDRESS = _address;
        return true;
    }
    ...
    function ListProperty_details(uint _origValue,
                                  ...
                                  string memory propertyID) public onlyOwner returns(bool){
        require(MINTNFT_DEPLOYED_ADDRESS != address(0),"MINT NFT ADDRESS IS ZERO");
        require(AUCTION_DEPLOYED_ADDRESS != address(0),"AUCTION ADDRESS IS ZERO");
        SLC s = new SLC("SLC","S",18,_origValue,msg.sender,AUCTION_DEPLOYED_ADDRESS);
        PROPERTY_ERC20_ADDRESS[propertyID] = address(s);
        require(Auction(AUCTION_DEPLOYED_ADDRESS).Store_SLC_Contract_Address(address(s),propertyID),"SLC address not stored");
        ++TOKEN_ID;
        mintNFTcontract(MINTNFT_DEPLOYED_ADDRESS).mint(msg.sender,TOKEN_ID,token_uri);
