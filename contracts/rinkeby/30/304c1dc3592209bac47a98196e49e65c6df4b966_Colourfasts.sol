...
contract Colourfasts is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function generateHash(uint8 tokenId) internal returns (bytes10) {
      // bytes10 scheme is r/g/b/background/filter/pattern/transform/shape
    ...
    function getDNASeed(bytes10 DNA, uint8 tokenId) internal pure returns (uint64) {
    ...
    function generateSvg(uint8 tokenId) public view returns (string memory) {
        return string(abi.encodePacked(
