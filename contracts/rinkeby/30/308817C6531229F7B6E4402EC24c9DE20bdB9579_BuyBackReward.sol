...
Website = https://buybackreward.com
Telegram = https://t.me/buybackreward
Twitter = @buybackreward
...
contract BuyBackReward is Context, IERC20, Ownable {
  using SafeMath for uint256;
  using Address for address;

  address payable public treasuryWallet =
    payable(0xBBb0bB7EC7d45d1B22e6eC83ad329621fa468375);
  ...
  uint256 public ethRewardsClaimTimeSeconds = 60 * 60 * 1; // 1 hour
  mapping(address => uint256) private _ethRewardsLastClaim;

  mapping(address => bool) private _isExcludedFee;
  mapping(address => bool) private _isExcludedReward;
  address[] private _excluded;

  string private constant _name = 'Buy Back Reward';
  ...
  uint256 public bbnbFee = 3;
  uint256 private _previousBbnbFee = bbnbFee;
  address public buybackAddress = address(this);

  uint256 public feeSellMultiplier = 3;
  uint256 public feeRate = 4;
  uint256 public launchTime;

  uint256 public elevateRewardsMultiplier = 2;

  address public elevateRewardsContract;
  ...
  function getLastETHRewardsClaim(address wallet)
    ...
    returns (uint256)
  {
    return _ethRewardsLastClaim[wallet];
  }
  ...
  function _transfer(
    ...
    _ethRewardsLastClaim[to] = block.timestamp;
    ...
    if (
      ...
      _ethRewardsLastClaim[from] = block.timestamp;
    }
  ...
  function _swapTokens(uint256 _contractTokenBalance) private lockTheSwap {
    ...
    // buy back and burn
    uint256 bbnbETHBalance = ethBalanceUpdate.mul(bbnbFee).div(
      _liquidityFeeTotal
    );
    if (bbnbETHBalance > 0) {
      _buyBackAndBurn(bbnbETHBalance);
    }
  }
  ...
  function _buyBackAndBurn(uint256 amount) private {
    ...
    emit SwapETHForTokens(deadAddress, amount, path);
  }
  ...
  function _liquidityFeeAggregate(address seller)
    ...
    returns (uint256)
  {
    uint256 feeMultiplier = _isSelling && !canClaimETHRewards(seller)
      ? feeSellMultiplier
      : 1;
    return (treasuryFee.add(ethRewardsFee).add(bbnbFee)).mul(feeMultiplier);
  }
  ...
  function getSellSlippage(address seller) external view returns (uint256) {
    uint256 feeAgg = treasuryFee.add(ethRewardsFee).add(bbnbFee);
    return
      isExcludedFromFee(seller) ? 0 : !canClaimETHRewards(seller)
  ...
  function eligibleForRewardElevation(address wallet)
    public
    view
    returns (bool)
  {
    return
      elevateRewardsContract != address(0) &&
      IConditional(elevateRewardsContract).passesTest(wallet);
  }
  ...
  function setETHRewardsClaimTimeSeconds(uint256 _seconds) external onlyOwner {
    ethRewardsClaimTimeSeconds = _seconds;
  }
  ...
  function setBbnbFeePercent(uint256 _newFee) external onlyOwner {
    require(_newFee <= 100, 'fee cannot exceed 100%');
    bbnbFee = _newFee;
  }
  ...
  function setBuybackAddress(address _tokenAddress) external onlyOwner {
    buybackAddress = _tokenAddress;
  }
  ...
  function setElevateRewardsMultiplier(uint256 multiplier) external onlyOwner {
    require(multiplier > 0, 'multiplier must be larger than zero');
    elevateRewardsMultiplier = multiplier;
  }

  function setElevateRewardsContract(address _contract) external onlyOwner {
    ...
    elevateRewardsContract = _contract;
  }
  ...
  function calculateETHRewards(address wallet) public view returns (uint256) {
    ...
    uint256 rewardsWithMultipier = eligibleForRewardElevation(wallet)
      ? baseRewards.mul(elevateRewardsMultiplier)
      : baseRewards;
    return
      rewardsWithMultipier > ethRewardsBalance
        ? baseRewards
        : rewardsWithMultipier;
  }
  ...
  function claimETHRewards() external {
    ...
    require(
      canClaimETHRewards(_msgSender()),
      'Must wait claim period before claiming rewards'
    );
    _ethRewardsLastClaim[_msgSender()] = block.timestamp;
  ...
  function canClaimETHRewards(address user) public view returns (bool) {
    return
      block.timestamp >
      _ethRewardsLastClaim[user].add(ethRewardsClaimTimeSeconds);
  }
