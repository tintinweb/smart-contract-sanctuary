...
contract Factory is Ownable {
  ...
  function createNFT(string memory _uri) external virtual {
    MNFT _nft = new MNFT(_uri);
    projects[projectId].nftAddress = address(_nft);
    projects[projectId].nft = _nft;
    projects[projectId].projectOwner = _msgSender();
    emit NewProject(_msgSender(), address(_nft), projectId, block.timestamp);
    projectId++;
  }

  function mintNFT(uint64 amount, uint256 _projectId) external virtual {
    ...
    projects[_projectId].nft.mint(_msgSender(), tokenId, amount);
    tokenId++;
  }
  ...
  function sellNFT(
    ...
    require(_projectId <= projectId, "Factory: Invalid Project Id");
    require(
      projects[_projectId].nft.balanceOf(_msgSender(), _tokenID) >= amount,
      "Factory: Not much NFTs yet"
    );
    projects[_projectId].token[_tokenID][_msgSender()].price = value;
    projects[_projectId].token[_tokenID][_msgSender()].forSale = amount;
    emit SellNFT(_msgSender(), _projectId, _tokenID, value, amount);
  }
  ...
  function buyNFT(
    ...
    _nft.transferFrom(seller, _msgSender(), _tokenID, amount);
  ...
  function mintBatchNFT(
    ...
    require(_projectId <= projectId, "Factory: Invalid Project Id");
    projects[_projectId].nft.mintBatch(_msgSender(), ids, amounts);
  }
}
