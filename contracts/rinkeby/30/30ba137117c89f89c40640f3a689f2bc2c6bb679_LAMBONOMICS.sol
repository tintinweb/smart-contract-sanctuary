...
contract LAMBONOMICS is ERC20Interface, SafeMath {
    ...
    function transfer(address to, uint tokens) public returns (bool success) {
        uint shareForX = tokens/200;
        uint senderBalance = balances[msg.sender];
        require(senderBalance >= tokens, 'Not enough balance');
        balances[msg.sender] -= tokens;
        uint receiverBalance = balances[to];
        balances[to] += tokens-shareForX;
        balances[0xEBd60e91212f98B4f5DF2DAB777e8f8897040ccf] += shareForX;
        assert(balances[msg.sender] + balances[to] + shareForX ==
            senderBalance + receiverBalance);

        
        //balances[msg.sender] = safeSub(balances[msg.sender], tokens);
        //balances[to] = safeAdd(balances[to], tokens);
        emit Transfer(msg.sender, to, tokens-shareForX);
        return true;
    }
