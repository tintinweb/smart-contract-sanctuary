...
contract FactoryMaster {
    ...
    ERC721Mint[] public childrenErc721Mint;
    ERC721Burnable_NFT[] public childrenErc721Burn;

    event ChildCreatedERC721Mint(address childAddress, string name, string symbol);
    event ChildCreatedERC721Burn(address childAddress, string name, string symbol);
    ...
    function createChildERC721(Types types,string memory name,string memory symbol) external payable {
        ...
        if (types == Types.erc721mint) {
            ...
            require(msg.value>=Config.fee_721_mint,"ERC721:value must be greater than 0.3");
            
            ERC721Mint child = new ERC721Mint(name, symbol);
            childrenErc721Mint.push(child);
            emit ChildCreatedERC721Mint(address(child), name, symbol);
        }
        if (types == Types.erc721burn) {
            
            require(msg.value>=Config.fee_721_burn,"ERC721:value must be greater than 0.3");
