...
contract WhiteRareAuctions is IAuctionHouse, ReentrancyGuard {
   ...
   function getAuctionCurrentNumber() public view returns (uint256) {
    ...
    function fixedBulkBuyInternal (uint256[] calldata _fixedAuctionIds, uint256 _totalValue) 
      external  
      payable 
    {
        // require(_totalValue == getCummilativeAuctionPrice(_fixedAuctionIds), "Must send reservePrice");
        for(uint256 cd = 0; cd < _fixedAuctionIds.length; cd++){
            uint256 auctionId = _fixedAuctionIds[cd];
            uint256 amount = getAuctionPrice(auctionId);
            // starts here
            uint256 curatorFee = 0;
            uint256 tokenOwnerProfit = auctions[auctionId].amount;
