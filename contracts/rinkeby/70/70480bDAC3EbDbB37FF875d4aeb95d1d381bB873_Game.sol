...
contract Game is Ownable {
    ...
    event Purchase(address indexed _from, uint _value);
    ...
    event rewards(uint _amount, address indexed _winner);
    event rewardERC20Token(address indexed _tokenAddress, uint _value, address indexed _winner);
    ...
    function reward(address payable _winner, uint256 _amount) public onlyOwner {
        _winner.transfer(_amount);
        emit rewards(_amount, _winner);
    }

    function tokenReward(address _tokenContract, uint256 _amount, address payable _winner) public onlyOwner {
        IERC20 tokenContract = IERC20(_tokenContract);
        
        tokenContract.transfer(_winner, _amount);
        emit rewardERC20Token(_tokenContract, _amount, _winner);
    }
}
