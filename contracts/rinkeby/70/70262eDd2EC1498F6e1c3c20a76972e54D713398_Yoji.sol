...
contract Yoji is Ownable, ReentrancyGuard {
    ...
    // Customized
    uint256 public TOKEN_PRICE = 30000000000000000; // 0.03 ETH ToDo: To be set
    uint256 public MIX_FEE = 30000000000000000; // 0.03 ETH ToDo: To be set
    ...
    uint256 private constant MUTATION_RATE = 2; // ToDo: 2%
    ...
    function setMixFee(uint256 _fee) public onlyOwner returns (bool) {
        MIX_FEE = _fee;
        return true;
    }
    ...
    function buy(uint256 _seed) public payable nonReentrant limitedSaleable returns (bool) {
        require(msg.value >= TOKEN_PRICE, 'Customized: msg.value is less than TOKEN_PRICE');
        uint256 tokenId = seedToTokenId(_seed);
        _mint(msg.sender, tokenId);
    ...
    function checkTokenId(uint256 _tokenId) public view returns (address _owner, string memory _chars) {
    ...
    function checkSeed(uint256 _seed) public view returns (uint256 _tokenId, address _owner, string memory _chars) {
        _tokenId = seedToTokenId(_seed);
        (_owner, _chars) = checkTokenId(_tokenId); 
    }

    function mix(uint256 _tokenId1, uint256 _tokenId2) public payable returns (bool) {
        require(ownerOf(_tokenId1) == msg.sender, 'Customized: caller is not owner of tokenId1');
        require(ownerOf(_tokenId2) == msg.sender, 'Customized: caller is not owner of tokenId2');
        require(msg.value >= MIX_FEE, 'Customized: msg.value is less than MIX_FEE');

        uint256 childTokenId = getChildTokenId(_tokenId1, _tokenId2, nonce);

        _mint(msg.sender, childTokenId);

        // refund
        if(msg.value > MIX_FEE) {
            payable(msg.sender).transfer(msg.value - MIX_FEE);
        }
    ...
    // Customized (private)
    function random(uint256 _nonce, uint256 _modulo) private view returns (uint256) { // ToDo: make it private
        uint256 seed = uint256(keccak256(abi.encodePacked(msg.sender, _nonce, blockhash(block.number), externalSeeds())));
        return seed % _modulo;
    }

    function getRandomTokenId() private view returns (uint256) { // ToDo: make it private
        uint256 seed = uint256(keccak256(abi.encodePacked(msg.sender, nonce, blockhash(block.number), externalSeeds())));
        return seedToTokenId(seed);
    }
    ...
    function seedToTokenId(uint256 _seed) private pure returns (uint256) { // ToDo: make it private
        (uint256 char1, uint256 char2, uint256 char3, uint256 char4) = tokenIdToArray(_seed);
        uint256 index1 = char1 % NUM_CHARS;
        uint256 index2 = char2 % NUM_CHARS;
        uint256 index3 = char3 % NUM_CHARS;
        uint256 index4 = char4 % NUM_CHARS;
        return index1 * 0x0000000000000001000000000000000000000000000000000000000000000000
             + index2 * 0x0000000000000000000000000000000100000000000000000000000000000000
             + index3 * 0x0000000000000000000000000000000000000000000000010000000000000000
             + index4;
    }

    function tokenIdToArray(uint256 _tokenId) private pure returns (uint256, uint256, uint256, uint256) { // ToDo: make it private
        uint256 char4 = _tokenId & MASK;
        uint256 char3 = (_tokenId / 0x0000000000000000000000000000000000000000000000010000000000000000) & MASK;
        uint256 char2 = (_tokenId / 0x0000000000000000000000000000000100000000000000000000000000000000) & MASK;
        uint256 char1 = (_tokenId / 0x0000000000000001000000000000000000000000000000000000000000000000) & MASK;
    ...
    function getChildTokenId(uint256 _tokenId1, uint256 _tokenId2, uint256 _nonce) private view returns (uint256) { // ToDo: make it private
        uint256[] memory child = new uint256[](4);
        uint256[] memory paternal = new uint256[](4);
        uint256[] memory maternal = new uint256[](4);
        
        (maternal[0], maternal[1], maternal[2], maternal[3]) = tokenIdToArray(_tokenId1);
        (paternal[0], paternal[1], paternal[2], paternal[3]) = tokenIdToArray(_tokenId2);
        
        uint256 localNonce = _nonce;
        uint256 paternalCount = 0;
        for(uint256 i = 0; i < 4; i++) {
            uint256 seed = random(localNonce, 100);
            localNonce++;
            if(seed < MUTATION_RATE) {
                // mutate
                child[i] = random(localNonce, NUM_CHARS);
                localNonce++;
            }else if(seed < (100 - MUTATION_RATE) / 2 + 1) {
                // paternal
                child[i] = paternal[i];
                paternalCount++;
            }else{
        ...
        if(paternalCount == 4) child[3] = maternal[3];

        return child[0] * 0x0000000000000001000000000000000000000000000000000000000000000000
             + child[1] * 0x0000000000000000000000000000000100000000000000000000000000000000
             + child[2] * 0x0000000000000000000000000000000000000000000000010000000000000000
             + child[3];
    }
