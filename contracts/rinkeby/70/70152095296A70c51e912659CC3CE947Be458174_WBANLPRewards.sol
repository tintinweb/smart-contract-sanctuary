...
contract WBANLPRewards is ERC1155PresetMinterPauserUpgradeable, OwnableUpgradeable, ContextMixin {
    ...
    function initializeWithOpenSeaProxy(string memory uri, address _openSeaProxyRegistryAddress) public virtual initializer {
    ...
    function resolveFarmAndLevelToTokenId(Farm farm, Level level) internal returns (uint256) {
        uint256 farmID = uint(farm);
        uint256 levelID = uint(level);
    ...
    function claimGoldenNFT(uint256 levelID) external {
        ...
        require(balanceOf(_msgSender(), resolveFarmAndLevelToTokenId(Farm.WBAN_STAKING, Level(levelID))) > 0, "Missing NFT for wBAN staking farm");
        require(balanceOf(_msgSender(), resolveFarmAndLevelToTokenId(Farm.WBAN_BNB, Level(levelID))) > 0, "Missing NFT for wBAN-BNB farm");
        require(balanceOf(_msgSender(), resolveFarmAndLevelToTokenId(Farm.WBAN_BUSD, Level(levelID))) > 0, "Missing NFT for wBAN-BUSD farm");
