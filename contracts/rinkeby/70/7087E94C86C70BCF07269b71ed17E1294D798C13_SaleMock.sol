...
contract Sale is Ownable, AccessControl {
    ...
    function withdraw(uint256 saleID) public {
        ...
        if (sales[saleID].receivedPayments > 0) {
            sales[saleID].token.transfer(msg.sender, sales[saleID].tokenAmount);
        }
    ...
    function registerStake(uint256 saleID) public {
        ...
        require(sales[saleID].staking.getCurrentStake(msg.sender) > 0, "Sale:have no stake!");
    ...
    function registerSale(uint256 saleID, uint256 paymentAmount) public {
        ...
        require(
            salePayments[saleID][msg.sender].add(paymentAmount) <=
                (
                    sales[saleID].staking.getCurrentStake(msg.sender).mul(1e9).div(
                        sales[saleID].staking.getCurrentTotalStake()
                    )
                )
    ...
    function getSaleBalanceRest(uint256 saleID, address userID) public view returns (uint256 balanceRest, uint256 payments) {
        balanceRest = 
            sales[saleID].staking.getCurrentStake(userID)
            .mul(1e9)
            .div(sales[saleID].staking.getCurrentTotalStake());
        payments = salePayments[saleID][userID];
        return (
            balanceRest
            .mul(sales[saleID].tokenAmount)
            .mul(sales[saleID].tokenPrice)
            .div(1e12).sub(salePayments[saleID][userID]),
            payments
        );
    }
    ...
    function claim(uint256 saleID) public {
        ...
        if (purchasedTokens > 0) {
            ...
            sales[saleID].tokenAmount -= purchasedTokens;
        }
    }
...
contract SaleMock is Sale {
    stages public mockStage;

    constructor(address _stakingContract) public Sale(_stakingContract) {
        grantRole(keccak256("CREATE_SALE_ROLE"), msg.sender);
    }

    function getSalestage(uint256 saleId) public view override returns (stages) {
        return mockStage;
    }

    function setMockStage(uint8 _stage) public {
        mockStage = stages(_stage);
    }

    function setSaleDates(uint256 _saleId, uint256[6] calldata _dates) external {
        saleRecord storage sr = sales[_saleId];
        sr.dates = _dates;
    }
}