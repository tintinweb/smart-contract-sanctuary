...
contract NFT is ERC721, PaymentSplitter {
  ...
  uint public constant presalePrice = 0.1 ether; // TODO set final value
  uint public constant startPriceDA = 0.3 ether; // TODO set final value
  uint public constant endPriceDA = 0.1 ether; // TODO set final value
  ...
  uint public presaleStartDate = block.timestamp; // TODO change to correct date
  uint public saleStartDate = block.timestamp + 1 minutes; // TODO change to correct duration
  ...
  function isSaleOpen() public view returns(bool){
    ...
    && block.timestamp <= saleStartDate + 12 minutes){ // TODO change to correct duration
      return true;
    }
    else{
      return false;
  ...
  function mintableSupply() public view returns(uint){
  ...
  function calculateTokenPriceDA() public view returns(uint){
    ...
    uint elapsedIntervalsDA = (block.timestamp - saleStartDate) / 6;
    if (elapsedIntervalsDA >= 72){
      return endPriceDA;
    }
    else {
      return startPriceDA - (0.0028 ether * elapsedIntervalsDA);          //TODO set final value
    }
  }
  ...
  function mintSale(uint amount) external payable onlyEOA {
    ...
    require(mintedTokensByAddress[msg.sender] + amount <= maxMintsPerAddressSale, "Exceeds MAX amount per address");
  ...
  function mintWhitelist(uint amount, bytes32[] calldata proof) external payable onlyEOA {
    require(isPresaleOpen(), "Presale not open");
    require(mintedTokensByAddress[msg.sender] + amount <= maxMintsPerAddressPresale, "already minted max amount");
    require(verifyWhitelist(proof), "not whitelisted");
    require(totalSupply() + amount <= mintableSupply(), "Max Supply reached");
    require(msg.value == presalePrice * amount , "Incorrect presalePrice sent");
