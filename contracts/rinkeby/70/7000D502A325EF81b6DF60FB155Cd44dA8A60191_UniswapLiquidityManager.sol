...
contract UniswapLiquidityManager is PeripheryPayments, IUniswapLiquidityManager {
    ...
    function shouldReadjust(address pool) public view returns (bool) {
    ...
    {
        // here
        (, pool, liquidity, , , , ) = unipilot.positions(tokenId);
        (fee0, fee1, amount0, amount1, totalLiquidity) = getTotalAmounts(pool);
        (token0, token1, fee, , , , currentTick) = getPoolDetails(pool);
        rebase = shouldReadjust(pool);
    }
    ...
    function _distributeFeesInTokens(
        ...
        if (_token0 == WETH || _token1 == WETH) {
            ...
            if (_index1 > 0) TransferHelper.safeTransfer(_token1, INDEX_FUND, _index1);
        } else {
            if (_tokensOwed0 > 0) {
    ...
    function removeLiquidityWithFees(
        ...
        if (paySender) {
            if (token0 == WETH || token1 == WETH) {
                (address tokenAlt, uint256 amountAlt, uint256 amountWeth) = token0 == WETH
                    ? (token1, amount1Removed, amount0Removed)
                    : (token0, amount0Removed, amount1Removed);
                TransferHelper.safeTransfer(tokenAlt, recipient, amountAlt);
                IWETH9(WETH).withdraw(amountWeth);
                TransferHelper.safeTransferETH(recipient, amountWeth);
            } else {
                TransferHelper.safeTransfer(token0, recipient, amount0Removed);
                TransferHelper.safeTransfer(token1, recipient, amount1Removed);
            }
