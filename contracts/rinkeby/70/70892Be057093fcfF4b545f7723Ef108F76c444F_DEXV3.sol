...
contract DEXV3 is Initializable, OwnableUpgradeable {
    /// Import library
    ...
    address[] private listUserCommission;
    mapping(address => bool) private hasCommission;
    uint256 public summaryCommission;
    bool private affiliateFunctionStatus;
    ...
    function buy() public payable {
        ...
        uint256 usdBnbPrice = uint256(getLatestPrice());
        ...
        emit Bought(amountRate);
    }

    function etherOfDex() external view returns (uint256) {
    ...
    function checkReturnValue(uint256 _value, bool _isCoin) public view returns (uint256) {
    ...
    function buyWithAffiliate(address _affilliateAddress) public payable activeAffiliate {
        ...
        if (custodianContract.isStaking(_affilliateAddress) == true && msg.sender != _affilliateAddress) {
            /// check is affilliate allow

            if (hasCommission[_affilliateAddress] == false) {
                listUserCommission.push(_affilliateAddress);
            }

            userCommission[_affilliateAddress] = userCommission[_affilliateAddress].add(amountRate / commissionRate);
            summaryCommission = summaryCommission.add(amountRate / commissionRate);
    ...
    function withdrawCommission(uint256 _amount) public {
        require(userCommission[msg.sender] >= _amount, 'balance not enough');

        userCommission[msg.sender] = userCommission[msg.sender].sub(_amount);
        summaryCommission = summaryCommission.sub(_amount);
    ...
    function exportUserCommission() public view returns (address[] memory) {
        return listUserCommission;
    }
    ...
    function checkPriceFeed() public view returns (uint256) {
    ...
    function getSummaryCommission() public view returns(uint256){
