...
contract FarmingStorage is FarmingInterface {
    ...
    // The budget of address from which the award is paid
    uint256 rewardAddressBudget;
}
...
contract MintFarming is FarmingStorage, Ownable, Initializable {
    ...
    function enterStaking(uint256 _amount) public {
        ...
        updatePool(ERC20Detailed(address(rewardToken)));
        uint256 decimals = ERC20Detailed(address(rewardToken)).decimals();
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accRewardPerShare).div(10 ** decimals).sub(user.rewardDebt);
            if (pending > 0) {
                _reward(_msgSender(), pending);
    ...
    function setRewardAddress(address target) public onlyOwner {
        require(target != address(0), "You can't set zero address");
        rewardAddress = target;
    }

    // Sets reward and coins to address. Can be called only by the owner.
    function setRewardAddress(address target, uint256 _amount) public onlyOwner {
        require(target != address(0), "You can't set zero address");
        rewardAddress = target;
        rewardAddressBudget = _amount;
    }
}
