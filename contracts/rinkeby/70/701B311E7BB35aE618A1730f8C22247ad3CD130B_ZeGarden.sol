...
contract ZeGarden is ERC721, Ownable, VRFConsumerBase, PaymentSplitter {
  ...
  uint8 public PRESALE_MAX_MINT = 3;
  ...
  function setWhitelist(bool allow, address[] calldata gardeners) public onlyOwner {
    uint8 amount = allow ? PRESALE_MAX_MINT : 0;

    for (uint256 i = 0; i < gardeners.length; i++) {
      whitelist[gardeners[i]] = amount;
    }
  }
  ...
  function presaleMint(uint256 amount) public payable {
    require(amount <= whitelist[msg.sender], "Exceeds remaining whitelist balance");
    require(presaleActive,                   "Presale is not active");

    _paidMint(amount);
    whitelist[msg.sender] -= uint8(amount);
  }
  ...
  function _paidMint(uint256 amount) internal {
