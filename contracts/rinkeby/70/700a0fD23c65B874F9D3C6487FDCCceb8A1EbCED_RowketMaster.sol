...
import "./RowketNFT.sol";

contract RowketMaster is ERC721Holder, Ownable, Pausable {

    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using Address for address;

    RowketNFT public rowketNFT;
    address public rowketToken;
    ...
    constructor(
        address _rowketToken,
        address _rowketNFT,
        address _feeAddress,
        uint256 _fee
    ) public {
        rowketToken = _rowketToken;
        rowketNFT = RowketNFT(_rowketNFT);
        feeAddress = _feeAddress;
        fee = _fee;
        emit FeeAddressTransferred(address(0), feeAddress);
    }
    ...
    function updateFee(uint256 _fee) public onlyOwner {
        emit UpdateFee(msg.sender, fee, _fee);
        fee = _fee;
    }

    function setFeeAddr(address _feeAddress) external {
        require(msg.sender == feeAddress, 'RowketMaster: FORBIDDEN');
        feeAddress = _feeAddress;
        emit FeeAddressTransferred(msg.sender, feeAddress);
    }

    function mint() public payable {
        require(msg.value == fee, 'RowketMaster: mint, amount too small');
        uint256 feeAmount = msg.value.mul(fee).div(100);
        IERC20(rowketToken).safeTransferFrom(address(msg.sender), feeAddress, feeAmount);
        rowketNFT.mint(address(msg.sender), mintNFTCount);
        mintNFTCount++;
    }
...
contract RowketNFT is ERC721Pausable, AccessControl, Ownable {
    ...
    constructor() public ERC721('Rowket NFT', 'RNFT') {
