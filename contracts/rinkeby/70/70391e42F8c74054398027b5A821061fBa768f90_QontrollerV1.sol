...
contract QollateralManager is IQollateralManager {
  ...
  constructor(address _qontrollerAddress) {
...
contract QontrollerV1 is IQontrollerV1 {
  ...
  function _createFixedRateLoanMarket(
    ...
    FixedRateLoanMarket market = new FixedRateLoanMarket(
                                                         qollateralManager,
    ...
    fixedRateLoanMarkets[principalTokenAddress][maturity] = address(market);

    // Enable newly-created `FixedRateLoanMarket`
    enabledMarkets[address(market)] = true;

    // Emit the event
    emit CreateFixedRateLoanMarket(address(market), principalTokenAddress, maturity);
