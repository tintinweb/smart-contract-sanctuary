...
// File: contracts/interface/ICashPool.sol
...
interface ICashPool {
    ...
    function distributePool(uint256 amount) external;
}
...
// File: contracts/abstract/IRewardDistributionRecipient.sol
...
contract Pool is ERC20, IRewardDistributionRecipient {
    ...
    uint256 public stipulationRewardRate;
    uint256 public lastUpdateTime;
    uint256 public stipulationLastUpdateTime;
    uint256 public rewardPerTokenStored;
    address public cashPoolAddress;
    ...
    constructor(
        address stakeToken_,
        address cashPoolAddress_,
        uint256 startTime_,
        uint256 stipulationRewardRate_,
        string memory name_,
        ...
        stipulationRewardRate = stipulationRewardRate_;
        cashPoolAddress = cashPoolAddress_;
        rewardDistribution = cashPoolAddress_;
        _decimals = decimals_;
        if (block.timestamp > startTime_) {
            stipulationLastUpdateTime = block.timestamp;
        }
    ...
    function rewardPerToken() public view returns (uint256) {
        ...
        uint256 rewardPerTokenStoredA = block.timestamp.sub(stipulationLastUpdateTime).mul(stipulationRewardRate).mul(1e18).div(totalSupply());

        uint256 rewardPerTokenStoredB = lastTimeRewardApplicable().sub(lastUpdateTime).mul(rewardRate).mul(1e18).div(totalSupply());

        return rewardPerTokenStored.add(rewardPerTokenStoredA).add(rewardPerTokenStoredB);
    ...
    function withdraw(uint256 amount) public checkStart updateReward(msg.sender) {
        ...
        require(block.timestamp >= unfrozenStakeTime(msg.sender), "Pool: Cannot withdrawal during freezing");
    ...
    function getReward() public checkStart updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            ICashPool(cashPoolAddress).sendRewards(msg.sender, reward);
    ...
    function setFrozenStakingTime(uint256 frozenStakingTime_) external onlyOwner {
        frozenStakingTime = frozenStakingTime_;
    }

    function setStipulationRewardRate(uint256 stipulationRewardRate_) external updateReward(address(0)) onlyOwner {
        stipulationRewardRate = stipulationRewardRate_;
    }
