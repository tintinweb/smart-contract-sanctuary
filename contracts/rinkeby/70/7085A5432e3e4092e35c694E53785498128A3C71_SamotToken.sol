...
contract SamotToken is ERC20, ERC721Holder, Ownable {
   ...
   constructor(string memory _name, string memory _symbol, address _contract, address _wallet) ERC20(_name, _symbol) {
      wallet = _wallet;
      nft = SamotNFT(_contract);
      uint256 balance = initialSupply.mul(1).div(100);
      _mint(wallet, balance.mul(1000000000000000000));
      _mint(msg.sender, initialSupply.sub(balance).mul(1000000000000000000));
   }
    ...
    function mint(uint256 numberOfTokens) public payable {
      ...
      if (preSaleIsActive) {
          ...
          require(numberOfTokens <= maxToMint, "Exceeds per wallet limit for presale.");
          require(balanceOf(msg.sender) <= maxToMint, "Exceeds per wallet limit for presale.");
          require(nft.balanceOf(msg.sender) > 0, "You must own at least one NFT to participate in the pre-sale.");
      } else {
          require(mintPrice.mul(numberOfTokens) <= msg.value, "ETH sent is incorrect.");
          require(balanceOf(msg.sender) <= maxToMint, "Exceeds per wallet limit for public sale.");
      }
      _mint(msg.sender, numberOfTokens.mul(1000000000000000000));
    }

    function isStaked(uint256 _tokenId) public view returns (bool) {
      return staked[_tokenId].exists == true;
    }

    function ownerOf(uint256 _tokenId) public virtual view returns (address) {
        return staked[_tokenId].owner;
    }
   ...
   function stakeNFTs(uint256[] memory tokenIds) public {
       for(uint i = 0; i < tokenIds.length; i++) {
          ...
          require(nft.ownerOf(tokenIds[i]) == msg.sender, "You do not own this NFT.");
          require(nft.isApprovedForAll(msg.sender, address(this)), "Contract is not approved to transfer your NFT.");
          nft.safeTransferFrom(msg.sender, address(this), tokenIds[i]);
          if(stakes[msg.sender] == 0) addStakeholder(msg.sender);
          stakes[msg.sender] = stakes[msg.sender].add(1);
          staked[tokenIds[i]].owner = msg.sender;
          staked[tokenIds[i]].exists = true;
          staked[tokenIds[i]].start = block.timestamp;
        }
   }

   function unstakeNFTs(uint256[] memory tokenIds) public {
      for(uint i = 0; i < tokenIds.length; i++) {
        require(stakes[msg.sender] > 0, "You don't have any NFTs staked.");
        require(ownerOf(tokenIds[i]) == msg.sender, "You do not own this NFT.");
        nft.safeTransferFrom(address(this), msg.sender, tokenIds[i]);
        stakes[msg.sender] = stakes[msg.sender].sub(1);
   ...
   function totalStakingRewards() public view returns(uint256) {
    ...
    function calculateReward(address _stakeholder) public view returns (uint256) {
      return stakes[_stakeholder] * stakingReward;
    }
