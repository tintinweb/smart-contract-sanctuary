...
contract RainVesting is Ownable {
    ...
    function addBeneficiaries(
        address[] memory _beneficiaryes,
        uint256[] memory _tokenAmounts
    ) public onlyOwner {
        require(
            _beneficiaryes.length == _tokenAmounts.length,
            "Invalid params"
        );

        for (uint256 i = 0; i < _beneficiaryes.length; i++) {
        ...
        private
    {
        require(
            block.timestamp < lockupTime ||
                (lockupTime == start && block.timestamp < 2 days),
            "Invalid timing"
        );
        require(
