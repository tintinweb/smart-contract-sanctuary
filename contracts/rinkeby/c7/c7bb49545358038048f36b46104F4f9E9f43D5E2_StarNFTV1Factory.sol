...
//import "./StarNFTV2Cap.sol";


contract StarNFTV1Factory is Ownable {

    event CreateStarNFT(address newStarNFT);
    event UpdateSpaceStation(address oldSpaceStation, address newSpaceStation);
    event UpdateBaseURI(string oldURI, string newURI);

    address public spaceStation;
    string public baseURI;

    constructor(address _spaceStation, string memory _baseURI) {
        spaceStation = _spaceStation;
        baseURI = _baseURI;
    }

    function createStarNFTV1(address owner) external returns (address nft) {
        ...
        StarNFTV1(nft).addMinter(spaceStation);
        StarNFTV1(nft).setURI(string(abi.encodePacked(baseURI, "0x", toAsciiString(nft), "/")));
        StarNFTV1(nft).transferOwnership(owner);
        emit CreateStarNFT(nft);
    }
...
//        starNFT.addMinter(spaceStation);
//        starNFT.setURI(string(abi.encodePacked(baseURI, "0x", toAsciiString(address(starNFT)), "/")));
//        starNFT.transferOwnership(owner);
//        stars.push(starNFT);
//        emit CreateStarNFT(starNFT);
//        return starNFT;
//    }

    function updateSpaceStation(address newSpaceStation) external onlyOwner {
        emit UpdateSpaceStation(spaceStation, newSpaceStation);
        spaceStation = newSpaceStation;
    }

    function updateBaseURI(string calldata newBaseURI) external onlyOwner {
        emit UpdateBaseURI(baseURI, newBaseURI);
        baseURI = newBaseURI;
    }
