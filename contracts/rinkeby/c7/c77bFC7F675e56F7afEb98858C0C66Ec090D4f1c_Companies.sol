...
// Create Company
    ...
    function removeIndex(uint _companyID, uint arrayIndex) private {
        if (arrayIndex >= companies[_companyID].members.length) return;
        for (uint i = arrayIndex; i<companies[_companyID].members.length-1; i++){
            companies[_companyID].members[i] = companies[_companyID].members[i+1];
        }
        delete companies[_companyID].members[companies[_companyID].members.length-1];
        companies[_companyID].members.pop;
    }
