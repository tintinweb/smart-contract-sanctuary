...
contract TokenSale is Ownable {
    ...
    function getParticipantsDataInRange(uint256 from, uint256 to) external view returns (address[] memory, uint256[] memory) {
        require(from <= to, "Incorrect range");
        require(to < _participants.length(), "Incorrect range");

        address[] memory pAddresses = new address[](to - from + 1);
        uint256[] memory pBalances = new uint256[](to - from + 1);

        for (uint256 i = 0; i < to - from + 1; i++) {
            address pAddress = _participants.at(i + from);
            pAddresses[i] = pAddress;
            pBalances[i] = balances[pAddress];
        }

        return (pAddresses, pBalances);
    }
