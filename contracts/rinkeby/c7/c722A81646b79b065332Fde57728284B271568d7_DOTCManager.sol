...
contract DOTCManager is IdOTC, AdminFunctions, ReentrancyGuard {
        ...
        returns (uint256 takenOderId)
    {
        ...
        if (_amount == allOffers[offerId].amountOut) {
            ...
            allOffers[offerId].amountOut -= amountToReceiveByTaker;
            allOffers[offerId].takenAmount += _amount;
        }
        ...
        payFee(allOffers[offerId].tokenOutAddress, _amount.mul(feeAmount).div(BPSNUMBER));
        safeTransferAsset(
     ...
     *   @param offerId uint256
     */
    function adminRemoveOffer(uint256 offerId) public returns (bool hasRemoved) {
        require(hasRole(dOTC_Admin_ROLE, _msgSender()), "must have dOTC Admin role");
        delete allOffers[offerId];
        IEscrow(escrow).removeOffer(offerId, msg.sender);
     ...
     */
    function unFreezeXOffer(uint256 offerId) public returns (bool hasUnfrozen) {
        require(hasRole(dOTC_Admin_ROLE, _msgSender()), "must have dOTC Admin role");
        IEscrow(escrow).unFreezeOneDeposit(offerId, msg.sender);
    ...
    */
    ...
    event unFreezeOffer(uint256 offerId, address unFreezedBy);

    event AdminRemoveOffer(uint256 offerId, address freezedBy);
}
...
contract AdminFunctions is AccessControl {
    ...
    function unFreezeEscrow() public isAdmin returns (bool status) {
        IEscrow(escrow).unFreezeEscrow(msg.sender);
        return true;
    }
