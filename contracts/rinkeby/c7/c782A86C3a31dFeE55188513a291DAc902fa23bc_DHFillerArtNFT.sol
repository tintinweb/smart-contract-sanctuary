...
contract DHFillerArtNFT is ERC721Enumerable, Ownable {
    ...
    uint256 public constant FRE_SUPPLY = 5;
    ...
    function mint(uint256 amountToMint) public payable {
        ...
        require(totalSupply < TOTAL_SUPPLY, "All arts have been minted.");
        ...
        require(balanceOf(msg.sender).add(amountToMint) <= mintLimit, string(abi.encodePacked("You have already minted ", mintLimit.toString(), " arts.")));

        if (totalSupply > FRE_SUPPLY) {
    ...
    function ownerMint(uint256 amountToMint) public onlyOwner {
        require(totalSupply() < TOTAL_SUPPLY, "All arts have been minted.");
        require(totalSupply() + amountToMint <= TOTAL_SUPPLY, "The amount of arts you are trying to mint exceeds the max supply.");
