    ...
    BlockTimestamp
{
    ...
    function removeLiquidityWithFees(
        ...
        if (paySender) {
            if (amount0Removed > 0) TransferHelper.safeTransfer(token0, recipient, amount0Removed);
            if (amount1Removed > 0) TransferHelper.safeTransfer(token1, recipient, amount1Removed);
...
contract Unipilot is IUnipilot, ERC721Permit, PeripheryPayments {
    ...
    function withdrawForETH(
        ...
        // );
        liquidityManager.withdrawForETH(wethToken, pilotToken, recipient, tokenId, liquidity, data);
    }
    ...
    function collectFeesForWETHPairs(
        ...
        //     "UNIPILOT:: INVALID_EXCHANGE"
        // );
            liquidityManager.collectFeesForWETHPairs(pilotToken, wethToken, recipient, tokenId, data);
    }
