 ...
 */
// FIXME: find a way to recognize AND tokens
    ...
    ReentrancyGuardUpgradeable
{
    ...
    event Redeem(uint256 burned, Collateral[] redeemed);
    ...
    function initialize(
        ...
        if (_oracles.length == 0) revert NoOracles();
        ...
        for (uint256 _i = 0; _i < _oracles.length; _i++) {
            Oracle memory _oracle = _oracles[_i];
            _totalWeight += _oracle.weight;
            oracles[_oracle.addrezz] = FinalizableOracle({
                lowerBound: _oracle.lowerBound,
                higherBound: _oracle.higherBound,
                finalProgress: 0,
                weight: _oracle.weight,
                finalized: false
            });
        }
        if (_totalWeight == 0) revert InconsistentWeights();

        // TODO: does this fail if the additional data bytes are empty?
        (bytes32 _erc20Name, bytes32 _erc20Symbol, uint256 _erc20Supply) = abi
            .decode(_data, (bytes32, bytes32, uint256));
        __ERC20_init(
        ...
        creator = _creator;
        toBeFinalized = uint16(_oracles.length);
    ...
    function finalize(uint256 _result) external override nonReentrant {
        ...
        if (_result < _oracle.lowerBound || _result == INVALID_ANSWER) {
            ...
            // negative result, give back all the collateral to the creator
            if (andRelationship) {
                ...
                // collateral tied to the failed condition
                for (uint256 _i = 0; _i < collaterals.length; _i++) {
                    Collateral storage _collateral = collaterals[_i];
    ...
    function redeem() external override nonReentrant {
        ...
        Collateral[] memory _redeemedCollateral = new Collateral[](
            collaterals.length
        );
        for (uint256 _i = 0; _i < collaterals.length; _i++) {
            Collateral storage _collateral = collaterals[_i];
            // FIXME: can total supply be 0?
            ...
            _redeemedCollateral[_i] = Collateral({
                token: _collateral.token,
                amount: _redeemableAmount
            });
        }
        emit Redeem(_kpiTokenBalance, _redeemedCollateral);
    }
}
