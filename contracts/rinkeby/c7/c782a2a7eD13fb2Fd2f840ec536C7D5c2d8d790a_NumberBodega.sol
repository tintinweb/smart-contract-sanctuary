...
// File: NumberBodega.sol

contract NumberBodega is ERC721, VRFConsumerBase {
    uint256 public tokenCounter;
    // add other things
    mapping(bytes32 => address) public requestIdToSender;
    mapping(bytes32 => string) public requestIdToTokenURI;
    mapping(uint256 => NumberType) public tokenIdToNumberType;
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override {
        address numberOwner = requestIdToSender[requestId];
        string memory tokenURI = requestIdToTokenURI[requestId];
        uint256 newItemId = tokenCounter;
        _safeMint(numberOwner, newItemId);
        _setTokenURI(newItemId, tokenURI);
        NumberType numberType = NumberType(randomNumber % 3);
        tokenIdToNumberType[newItemId] = numberType;
