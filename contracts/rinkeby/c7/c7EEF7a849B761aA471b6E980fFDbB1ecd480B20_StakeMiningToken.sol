...
contract StakeMiningToken is Context, Ownable {
    ...
    function deposit(uint stakeAmount) public {
        ...
        usersToken[msg.sender] = UserDeopsit({
            tokenBalanceOf: stakeAmount + usersToken[msg.sender].tokenBalanceOf,
            t: block.timestamp
        });
    }
    ...
    function withdraw(uint amount, address[] memory users, uint[] memory amounts, uint nonce, bytes memory signature) external {
        ...
        usersToken[msg.sender].tokenBalanceOf -= amount;
        stakeToken.transfer(msg.sender, amount);

        for (uint256 index = 0; index < users.length; index++) {
            stakeToken.transfer(users[index], amounts[index]);
        }
    }
