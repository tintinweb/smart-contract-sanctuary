...
/**
 * @title SadgeBadge contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract SadgeBadge is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public MAX_BADGE_SUPPLY;
    ...
    constructor() ERC721("Sadge Badge", "BADGE") {
        MAX_BADGE_SUPPLY = 10000;
        REVEAL_TIMESTAMP = block.timestamp + (86400 * 7);
        mintPrice = 30000000000000000; // 0.03 ETH
        maxToMint = 30;
        saleIsActive = false;
        wallet1 = 0x97080d27b483F380779c8bEFfFB2FCF222a2b5a7;
        wallet2 = 0xf4Ebb631F347009fb6701ea8BBaCDB11Bf7ccBe4;
    }
    ...
    /**
     * Mint Badges by owner
     */
    function reserveBadges(address _to, uint256 _numberOfTokens) external onlyOwner {
    ...
    * Mints tokens
    */
    function mintBadges(uint256 numberOfTokens) external payable {
        require(saleIsActive, "Sale must be active to mint");
        require(numberOfTokens <= maxToMint, "Invalid amount to mint per once");
        require(totalSupply().add(numberOfTokens) <= MAX_BADGE_SUPPLY, "Purchase would exceed max supply");
        require(mintPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
        
        for(uint256 i = 0; i < numberOfTokens; i++) {
            uint256 mintIndex = totalSupply();
            if (totalSupply() < MAX_BADGE_SUPPLY) {
        ...
        if (startingIndexBlock == 0 && (totalSupply() == MAX_BADGE_SUPPLY || block.timestamp >= REVEAL_TIMESTAMP)) {
    ...
    function setStartingIndex() external {
        ...
        startingIndex = uint256(blockhash(startingIndexBlock)) % MAX_BADGE_SUPPLY;
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number.sub(startingIndexBlock) > 255) {
            startingIndex = uint256(blockhash(block.number - 1)) % MAX_BADGE_SUPPLY;
        }
