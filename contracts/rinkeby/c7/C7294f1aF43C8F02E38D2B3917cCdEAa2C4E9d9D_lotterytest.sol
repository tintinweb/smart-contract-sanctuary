...
contract lotterytest is ERC721, Ownable, ReentrancyGuard, VRFConsumerBase {
  ...
  function buyTicket() external payable nonReentrant {
    require(thisSeasonsTicketsByAddress[getCurrentSeason()][msg.sender] + 1 <= maxTicketsPerAddress, "you bought the maximum amount of tickets per season already");
    require(!drawingInProgress, "wait until lottery drawing is over to enter next season");
    require(
      msg.value >= ticketPrice,
      "Pay the correct Ticket Price."
    );
    require(
      getCurrentSeasonTicketCount() + 1 <= maxTicketsPerSeason,
      "maximum amount of tickets sold for this season, come back next week"
    );
    thisSeasonsTicketsByAddress[getCurrentSeason()][msg.sender] += 1;
    mintTicket();
  }

  function mintTicket() private {
    _safeMint(msg.sender, _tokenIds.current());
    seasontickets.push(_tokenIds.current());
    _tokenIds.increment();
  }
  ...
  function getWinnerAddress(uint randomVal) private returns (address){
    uint winnerTokenId = seasontickets[randomVal % getCurrentSeasonTicketCount()];
    address winnerAddress = ownerOf(winnerTokenId);
    emit LotteryWon(getCurrentSeason(), winnerAddress, winnerTokenId);
    return winnerAddress;
  }

  // sends 80% of pot to winner
  // 20 % to treasury
  function payWinnings(address winner) private {
    (bool d, ) = payable(owner()).call{value: address(this).balance * 20 / 100}("");
