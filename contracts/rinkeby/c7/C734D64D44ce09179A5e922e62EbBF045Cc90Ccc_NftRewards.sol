...
contract NftRewards is Ownable {
    ...
    function addBalances(address[] calldata holders, uint256[] calldata amounts) external onlyOwner returns(uint256 count){
        ...
        require(count == amounts.length, "Error: Holders length and amounts length are not the same");
    ...
    function withdraw() external {
        require(_balances[msg.sender] > 0 && IERC20(paper).balanceOf(address(this)) >= _balances[msg.sender], "Error: Not enough tokens");
