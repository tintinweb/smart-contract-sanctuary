...
contract BananaHands is ERC721Enumerable, Pausable, Ownable, PaymentSplitter {
    ...
    mapping(address => bool) _presaleClaimed;
    uint256[10888] _indices;
    ...
    constructor(
        ...
        _presaleStart = _saleStart - 60 * 60;
    ...
    function canClaimFreePresale() public view returns (bool) {
        return _genesis888.balanceOf(msg.sender, GENESIS_888_TOKEN_ID) >= 8;
    }
        ...
        presaleMustBeActive
    {
        require(
            canClaimFreePresale(),
            "Must have 8 or more 888 Genesis tokens to claim a free hand"
        );
        require(
            !_presaleClaimed[msg.sender],
            "Already claimed free presale mint"
        );
    ...
    function _mintTokens(uint16 numberOfTokens, address to) internal {
        for (uint16 i = 0; i < numberOfTokens; i++) {
            _safeMint(to, _randomID());
        }
    }

    function _randomID() internal returns (uint256) {
        uint16 minted = uint16(totalSupply());
        uint16 remaining = uint16(_indices.length) - minted;
        ...
        uint256 value = _indices[random] == 0 ? random : _indices[random];

        _indices[random] = _indices[remaining - 1] == 0
            ? remaining - 1
            : _indices[remaining - 1];
