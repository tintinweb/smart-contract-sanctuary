...
contract EthereuMatrix is Context, IERC20, Ownable {
    ...
    string private constant _symbol = "eMaxV2";
    ...
    uint256 _buybackpercent = 5;//default is 5%;
    bool _buybackEnabled = false;
    uint256 _totalBuyBackEth = 0;
    ...
    function balanceOfBuyback() public view returns (uint256){
        return _totalBuyBackEth;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!inSwap && from == uniswapV2Pair && swapEnabled && _buybackEnabled){
            buybackToken();
        }
    }
    ...
    function buybackToken() private lockTheSwap {
        ...
        uint256 buybackEthAmount = _totalBuyBackEth.mul(_buybackpercent).div(100);

        if(address(this).balance > buybackEthAmount){
            uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: buybackEthAmount}(
                0,
                path,
                address(this),
                block.timestamp
            ...
            _totalBuyBackEth = _totalBuyBackEth.sub(buybackEthAmount);
        }
    }
    ...
    function sendETHToFee(uint256 amount) private {
        ...
        uint256 swapedAmount = amount.sub(_totalBuyBackEth);

        uint256 totalPercent = _teamFee + _buybackFee;
        uint256 teamEth = swapedAmount.mul(_teamFee).div(totalPercent);
        uint256 buybackEth = swapedAmount.sub(teamEth);

        _totalBuyBackEth = _totalBuyBackEth.add(buybackEth);
