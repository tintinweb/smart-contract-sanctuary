...
/**
 * @notice Access Controls contract for the DArtFlex Platform
 */
 ...
 */
contract DArtFlexNFT is ERC721("DArtFlex NFT", "DFN") {

    // @notice event emitted upon construction of this contract, used to bootstrap external indexers
    event DArtFlexNFTContractDeployed();

    // @notice event emitted when base UIR is updated
    event DArtFlexBaseUriUpdate(
        string _tokenUri
    );

    // @notice event emitted when token URI is updated
    event DArtFlexTokenUriUpdate(
    ...
    /**
     @notice Constructor
     @param _accessControls Address of the DArtFlexNFT access control contract
     */
    constructor(AccessControls _accessControls) public {
        accessControls = _accessControls;
        emit DArtFlexNFTContractDeployed();
    }

    /**
     @notice Mints a DArtFlexNFT AND when minting to a contract checks if the beneficiary is a 721 compatible
     ...
     @param _postCreator Creator of Digital Art
    ...
    function mint(address _beneficiary, string calldata _tokenUri, address _postCreator) external returns (uint256) {
        require(
            ...
            "DArtFlexNFT.mint: Sender must have the admin or minter role"
        );
        // Valid args
        ...
        emit DArtFlexNFTMinted(tokenId, _postCreator, _beneficiary, _tokenUri, "1 of 1");

        return tokenId;
    }

    /**
     @notice Burns a DArtFlexNFT
    ...
    function burn(uint256 _tokenId) external {
        ...
        require(
            ...
            "DArtFlexNFT.burn: Only garment owner or approved"
        );
    ...
    function setBaseURI(string memory baseURI_) external {
        require(
            ...
            "DArtFlexNFT.mint: Sender must have the admin or contract role"
        );
        _setBaseURI(baseURI_);
        emit DArtFlexBaseUriUpdate(baseURI_);
    }
    ...
    function setTokenURI(uint256 _tokenId, string calldata _tokenUri) external {
        ...
        emit DArtFlexTokenUriUpdate(_tokenId, _tokenUri);
    }
    ...
    function setPrimarySalePrice(uint256 _tokenId, uint256 _salePrice) external {
        ...
        require(_exists(_tokenId), "DArtFlexNFT.setPrimarySalePrice: Token does not exist");
        require(_salePrice > 0, "DArtFlexNFT.setPrimarySalePrice: Invalid sale price");
    ...
    function updateAccessControls(AccessControls _accessControls) external {
        require(accessControls.hasAdminRole(_msgSender()), "DArtFlexNFT.updateAccessControls: Sender must be admin");
        accessControls = _accessControls;
    }
    /////////////////
    // View Methods /
    ...
    function _assertMintingParamsValid(string calldata _tokenUri, address _postCreator) pure internal {
        require(bytes(_tokenUri).length > 0, "DArtFlexNFT._assertMintingParamsValid: Token URI is empty");
        require(_postCreator != address(0), "DArtFlexNFT._assertMintingParamsValid: Post createErrors is zero address");
    }
}
