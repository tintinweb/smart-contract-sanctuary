...
contract BasicRewarder is IJellyRewards {
   ...
   // mapping(uint256 => uint256) currentPoolTemplateId;
    ...
    constructor(
        ...
        rewardInfo.startTime = BoringMath.to64(_startTime);
        rewardInfo.lastRewardTime = BoringMath.to64(_lastRewardTime);
    }
        ...
        external
    {
        ...
        rewardsPerSecond = amounts[0] * POINT_MULTIPLIER
                                        / SECONDS_PER_PERIOD
                                        / POINT_MULTIPLIER;
    }
        ...
        returns(bool)
    {
        ...
        if (block.timestamp <= uint256(info.lastRewardTime)) {
            return false;
        }

        /// @dev check that the rewards have started
        if(block.timestamp <= uint256(info.startTime)) {
            info.lastRewardTime = BoringMath.to64(block.timestamp);
            return false;
        }
    ...
    function totalRewards() external override view returns (uint256) {
        ...
        for (uint256 i = 0; i < uint256(rewardInfo.poolCount); i++) {
            poolRewardsCount += poolRewards(i, uint256(rewardInfo.lastRewardTime), block.timestamp);
        }
    ...
    function poolRewards(uint256 _poolId, uint256 _from, uint256 _to) public override view returns (uint256 rewards) {
        ...
        address tokenPool = tokenPoolTemplate[_poolId];
        return rewardsPerSecond * (_to - _from);
    }
    ...
    {
        rewards = poolRewards(poolId, uint256(rewardInfo.lastRewardTime), block.timestamp);
        if ( rewards > 0 ) {
