...
contract Token is LucaInterface, LucaTokenStorage {
    ...
    modifier onlyOwner(){
         require(msg.sender == _owner, "LUCA: Caller not owner");
        _;
    }
    ...
    function initializeToken(string memory name_, string memory symbol_, uint8 decimals_, uint256 initTotalSupply) internal unInitialized {
        ...
        _owner = msg.sender;
        _fragmentBalances[msg.sender] = _fragment;
        _setRebaser(msg.sender);
        _setMinter(msg.sender);
        _setReceiver(msg.sender);
    }
    ...
    function setRebaser(address rebaser_) override external onlyOwner{
