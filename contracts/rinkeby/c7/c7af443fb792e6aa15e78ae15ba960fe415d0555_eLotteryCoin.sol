...
contract eLotteryCoin is Context, IBEP20, Ownable {	
    ...
    address[] private _excluded;
   	...
   	address payable private _devWallet = payable (0x07d92411765DE1448dBc49984768eE8f1A28CCF0);
    ...
    uint public _lottoDrawCount = 0;
   	...
   	uint256 private _minLottoBalance = 1 * 10**9;
    bool newDraw;
   	
   	
    uint256 private constant MAX = ~uint256(0);	
    uint256 private _tTotal = 10**12 * 10**9; // 1T  	   
    uint256 private _rTotal = (MAX - (MAX % _tTotal));	
    
    uint256 private _tFeeTotal;	
    string private _name = "eLottery";	
    ...
    uint256 private numTokensSellToAddToLiquidity = 500 * 10**7 * 10**9;	
    uint256 public lotteryThreshold = 10 * 10**12 * 10**9;	
    // anti whale
    bool    public _isAntiWhaleEnabled = true;
    uint256 public _antiWhaleThreshold = 1 * 10**10 * 10**9;
    ...
    function _transfer(	
        ...
        */
        ...
        uint256 cTime = block.timestamp.mod(1 days);
        bool isPastTwelve = cTime.add(12 hours) >= 1 days && newDraw;
        //bool overMinLottoBalance = lottoBalance >= lotteryThreshold;
        if (cTime.add(12 hours) < 1 days) { newDraw = true; }
        if (
            isPastTwelve && !inSwapAndLiquify && !inLotteryDraw && lottoEnabled
        ) {
            drawLotto(lottoBalance.div(2));
            newDraw = false;
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {	
        ...
        uint256 burnAmt = amount.mul(3).div(100);    
