...
contract Stake is Ownable {
    ...
    mapping(address => uint256) private totalUserDepst;
    ...
    function addFarmAddress(address _farm) external onlyOwner {
        farm = IFarm(_farm);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (farm.userExist(msg.sender)) {
            farm.consolidateExternal(msg.sender);
            farm.registerLp(msg.sender);
        }
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accERC20PerShare).div(1e36);
        totalStaked = totalStaked.add(_amount);
        totalUserDepst[msg.sender] += _amount;
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        totalUserDepst[msg.sender] -= _amount;
        if (totalUserDepst[msg.sender] == 0) {
            consUnRegister(msg.sender);
        }
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        totalUserDepst[msg.sender] -= user.amount;
    ...
    function userExist(address user) external view returns (bool) {
        return totalUserDepst[user] > 0 ? true : false;
    }

    function consUnRegister(address staker) internal {
        if (farm.userExist(staker)) {
            farm.consolidateExternal(staker);
            farm.unRegisterLp(staker);
        } else {
            farm.unRegisterLp(staker);
        }
    }
