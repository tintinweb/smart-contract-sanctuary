...
contract ChainlinkPriceOracleProxy is Ownable, PriceOracle {
        ...
        returns (uint256)
    {
        ...
        uint chainLinkDecimals = AggregatorV3Interface(config.chainlinkAggregatorAddress).decimals();
        ...
        if (config.chainlinkPriceBase == 1) {
            underlyingPrice = uint256(chainlinkPrice).mul(10**config.underlyingTokenDecimals).div(10**chainLinkDecimals);
        } else if (config.chainlinkPriceBase == 2) {
            (, int256 ethPriceInUsd, , , ) = AggregatorV3Interface(
                ethUsdChainlinkAggregatorAddress
            )
                .latestRoundData();
            uint chainLinkDecimals_ethusd = AggregatorV3Interface(ethUsdChainlinkAggregatorAddress).decimals();
            require(ethPriceInUsd > 0, "ETH price invalid");

            underlyingPrice = uint256(chainlinkPrice)
                .mul(uint256(ethPriceInUsd))
                .mul(10**config.underlyingTokenDecimals)
                .div(10**chainLinkDecimals_ethusd)
                .div(10**chainLinkDecimals);
        } else {
     ...
     * @dev Implements the PriceOracle interface for Utopia v2.
     ...
     */
    function getUnderlyingPrice(UToken cToken)
        public
        view
        returns (uint256)
    {
