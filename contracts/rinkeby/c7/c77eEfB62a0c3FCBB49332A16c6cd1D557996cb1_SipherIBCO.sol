...
contract SipherIBCO is Ownable {
    ...
    event Claimed(address indexed account, uint256 userShare, uint256 sipherAmount);
    event Deposited(address indexed account, uint256 amount);

    uint256 public constant START = 1637845200;
    uint256 public constant END = START + 3 days;
    uint256 public constant TOTAL_DISTRIBUTE_AMOUNT = 4000000 * 10**18;
    uint256 public constant MINIMAL_PROVIDE_AMOUNT = 40 ether;
    ...
    function deposit() external payable {
        ...
        require(
            SIPHER.balanceOf(address(this)) == TOTAL_DISTRIBUTE_AMOUNT,
            "Not enough Sipher Token in contract balance yet"
        );

        totalProvided += msg.value;
        provided[msg.sender] += msg.value;
        
        if (provided[msg.sender] > accumulated[msg.sender]){
            accumulated[msg.sender] += provided[msg.sender] - accumulated[msg.sender];
        }
     ...
     */
    function getUserDeposited(address _userAddress) external view returns (uint256) {
        return provided[_userAddress];
    }
     ...
     * @dev TODO
     */
    function getProviderMaxWithdrawCap(uint256 userAccumulated) external pure returns (uint256) {
        return _withdrawCap(userAccumulated);
    }
    
    /**
     * @dev TODO
     */
    function getProviderMinWithdrawCap(uint256 userAccumulated) external pure returns (uint256) {
     ...
     * Requirements:
     * - The offering must have been already ended.
     * - Amount to withdraw must be smaller than amount deposited
     */
    function withdraw(uint256 amount) external {
        require(block.timestamp > START && block.timestamp < END, "Only Withdrawable during the Offering duration");
        require(provided[msg.sender] > 0 && provided[msg.sender] >= amount, "Not enough balance");

        uint256 userAccumulated = accumulated[msg.sender];
        uint256 userWithdrawableAmount = _withdrawCap(userAccumulated);
        
        require(amount <= userWithdrawableAmount, "Maximum withdraw cap reached");
        require(
            provided[msg.sender] >= userAccumulated + amount - userWithdrawableAmount ,
            "Minimum withdraw cap reached"
        );
     ...
     */
    function getEstTokenPrice() external view returns (uint) {
        return Math.max(totalProvided, MINIMAL_PROVIDE_AMOUNT) / TOTAL_DISTRIBUTE_AMOUNT;
    }

    /**
     * @dev Get estimated amount of SIPHER token an address will receive
     */
    function getEstReceivedToken() public view returns (uint) {
    ...
    function withdrawSaleFunds() external onlyOwner {
        ...
        require(address(this).balance != 0, "Funds have been withdrawn");
    ...
    function withdrawUnclaimedSIPHER() external onlyOwner {
        ...
        require(SIPHER.balanceOf(address(this)) != 0, "No funds to withdraw");
