...
contract MultiSigOTC is Ownable, Pausable, ReentrancyGuard {
    ...
    function initiateTokenTransfer(address _tokenAddress, uint256 _amount, address  _toAddress) public onlyApprover{
    ...
    function initiateETHTransfer(uint256 _amount, address payable _toPayableAddress) public onlyApprover{
        require(typeOfTransfer == 0, "MultiSigOTC: There is already a pending Transaction");
    ...
    function initiateAddApprover(address  _toAddress) public onlyApprover{
        require(typeOfTransfer == 0, "MultiSigOTC: There is already a pending Transaction");
        typeOfTransfer = 3;
        toAddress = _toAddress;
        uint32 startTime = SafeMath32.fromUint(now);
    ...
    function initiateRemoveApprover(address  _toAddress) public onlyApprover{
        require(typeOfTransfer == 0, "MultiSigOTC: There is already a pending Transaction");
        typeOfTransfer = 4;
        toAddress = _toAddress;
        uint32 startTime = SafeMath32.fromUint(now);
    ...
    function cancelTransaction() public onlyApprover {
