...
contract Pass is ERC721Enumerable, Ownable {
    ...
    uint256 public PASS_PUBLIC = 1900;
    uint256 public PASS_MAX = PASS_GIFT + PASS_PUBLIC;
    uint256 public PASS_PRICE = .04 ether;
    uint256 public constant PASS_PER_MINT = 5;
    string private _tokenBaseURI = "https://skuxxverse.vercel.app/api/";
    uint256 public giftedAmount;
    uint256 public publicAmountMinted;
    uint256 public privateAmountMinted;
    bool public saleLive;
    constructor() ERC721("GGG", "GGG") { }
    ...
    function reserveGiveaway(uint256 numDice) public onlyOwner {
        uint currentSupply = totalSupply();
        require(numDice > 0 && numDice <= 46, "Max 46 to reserve at once");
        require(totalSupply().add(numDice) <= PASS_MAX, "Exceeds PASS_MAX");
        require(saleLive == false, "Sale has already started");
        uint256 index;
        for (index = 0; index < numDice; index++) {
            _safeMint(0xA6e172C01e08DB32c073f48150147a15558AeC12, currentSupply + index + 1);
        }
    }
    function gift(address[] calldata receivers) external onlyOwner {
        require(totalSupply() + receivers.length <= PASS_MAX, "MAX_MINT");
        require(giftedAmount + receivers.length <= PASS_GIFT, "GIFTS_EMPTY");
