...
contract MultipleDropsPublicSale is Ownable, IERC165 {
    ...
    function deposit() public payable {
        ...
        uint256 randomStartIndex = randomAvailableTokenIndex();
        sendRandomNft(randomStartIndex);
        if (amount > 1) {
            if (randomStartIndex > _availableTokens.length) {
                sendRandomNft(_availableTokens.length - 1);
            } else {
                sendRandomNft(randomStartIndex);
            }
        }
