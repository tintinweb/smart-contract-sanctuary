...
import "./BatchCounters.sol";

contract JaxBlast is ERC721URIStorage, Ownable {
  ...
  function batchMint(address to, string memory tokenURI, uint256 count) public returns (uint256) {
    uint256 startId = _tokenIds.current();
    _tokenIds.increment(count);
