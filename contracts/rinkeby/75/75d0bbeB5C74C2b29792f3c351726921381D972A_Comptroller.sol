...
contract Comptroller is ComptrollerV4Storage, ComptrollerInterfaceG2, ComptrollerErrorReporter, ExponentialNoError {
    ...
    function getDAWAddress() public view returns (address) {
        return 0x5a21F2DAdf0b55bAe3f9Fa9231F2dBB45e07A561;
    }
...
contract YAI is LibNote {
    // --- Auth ---
    mapping (address => uint) public wards;
    function rely(address guy) external note auth { wards[guy] = 1; }
    function deny(address guy) external note auth { wards[guy] = 0; }
    modifier auth {
        require(wards[msg.sender] == 1, "YAI/not-authorized");
        _;
    }

    // --- BEP20 Data ---
    string  public constant name     = "YAI Stablecoin";
    string  public constant symbol   = "YAI";
    ...
    function add(uint x, uint y) internal pure returns (uint z) {
        require((z = x + y) >= x, "YAI math error");
    }
    ...
    function transferFrom(address src, address dst, uint wad)
        public returns (bool)
    {
        require(balanceOf[src] >= wad, "YAI/insufficient-balance");
        if (src != msg.sender && allowance[src][msg.sender] != uint(-1)) {
            require(allowance[src][msg.sender] >= wad, "YAI/insufficient-allowance");
    ...
    function burn(address usr, uint wad) external {
        require(balanceOf[usr] >= wad, "YAI/insufficient-balance");
        if (usr != msg.sender && allowance[usr][msg.sender] != uint(-1)) {
            require(allowance[usr][msg.sender] >= wad, "YAI/insufficient-allowance");
                    ...
                    bool allowed, uint8 v, bytes32 r, bytes32 s) external
    {
        ...
        require(holder != address(0), "YAI/invalid-address-0");
        require(holder == ecrecover(digest, v, r, s), "YAI/invalid-permit");
        require(expiry == 0 || now <= expiry, "YAI/permit-expired");
        require(nonce == nonces[holder]++, "YAI/invalid-nonce");
...
contract DAW is Tokenlock {
    /// @notice BEP-20 token name for this token
    string public constant name = "Deswap";

    /// @notice BEP-20 token symbol for this token
    string public constant symbol = "DAW";

    /// @notice BEP-20 token decimals for this token
    uint8 public constant decimals = 18;

    /// @notice Total number of tokens in circulation
    uint public constant totalSupply = 30000000e18; // 30 million DAW
    ...
    /**
     * @notice Construct a new DAW token
    ...
    function approve(address spender, uint rawAmount) external validLock returns (bool) {
        ...
        if (rawAmount == uint(-1)) {
            ...
            amount = safe96(rawAmount, "DAW::approve: amount exceeds 96 bits");
        }
    ...
    function transfer(address dst, uint rawAmount) external validLock returns (bool) {
        uint96 amount = safe96(rawAmount, "DAW::transfer: amount exceeds 96 bits");
    ...
    function transferFrom(address src, address dst, uint rawAmount) external validLock returns (bool) {
        ...
        uint96 amount = safe96(rawAmount, "DAW::approve: amount exceeds 96 bits");

        if (spender != src && spenderAllowance != uint96(-1)) {
            uint96 newAllowance = sub96(spenderAllowance, amount, "DAW::transferFrom: transfer amount exceeds spender allowance");
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public validLock {
        ...
        require(signatory != address(0), "DAW::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "DAW::delegateBySig: invalid nonce");
        require(now <= expiry, "DAW::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) public view returns (uint96) {
        require(blockNumber < block.number, "DAW::getPriorVotes: not yet determined");
    ...
    function _transferTokens(address src, address dst, uint96 amount) internal {
        require(src != address(0), "DAW::_transferTokens: cannot transfer from the zero address");
        require(dst != address(0), "DAW::_transferTokens: cannot transfer to the zero address");

        balances[src] = sub96(balances[src], amount, "DAW::_transferTokens: transfer amount exceeds balance");
        balances[dst] = add96(balances[dst], amount, "DAW::_transferTokens: transfer amount overflows");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint96 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "DAW::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "DAW::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
      uint32 blockNumber = safe32(block.number, "DAW::_writeCheckpoint: block number exceeds 32 bits");
 ...
 * DTokens should reference this contract as their comptroller.
 */
...
contract PriceOracle {
    ...
    /**
      ...
      *  Zero means the price is unayailable.
      */
...
contract ComptrollerErrorReporter {
    ...
    enum FailureInfo {
        ...
        SET_YAI_MINT_RATE_CHECK,
        SET_YAICONTROLLER_OWNER_CHECK,
        SET_MINTED_YAI_REJECTION,
...
contract TokenErrorReporter {
    ...
    enum FailureInfo {
        ...
        REDUCE_RESERVES_CASH_NOT_AYAILABLE,
        ...
        REPAY_YAI_COMPTROLLER_REJECTION,
        REPAY_YAI_FRESHNESS_CHECK,
        YAI_MINT_EXCHANGE_CALCULATION_FAILED,
...
contract YAIControllerErrorReporter {
    enum Error {
        NO_ERROR,
        UNAUTHORIZED,
        REJECTION,
    ...
    enum FailureInfo {
        ...
        YAI_MINT_REJECTION,
        YAI_BURN_REJECTION,
        YAI_LIQUIDATE_ACCRUE_BORROW_INTEREST_FAILED,
        YAI_LIQUIDATE_ACCRUE_COLLATERAL_INTEREST_FAILED,
        YAI_LIQUIDATE_COLLATERAL_FRESHNESS_CHECK,
        YAI_LIQUIDATE_COMPTROLLER_REJECTION,
        YAI_LIQUIDATE_COMPTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED,
        YAI_LIQUIDATE_CLOSE_AMOUNT_IS_UINT_MAX,
        YAI_LIQUIDATE_CLOSE_AMOUNT_IS_ZERO,
        YAI_LIQUIDATE_FRESHNESS_CHECK,
        YAI_LIQUIDATE_LIQUIDATOR_IS_BORROWER,
        YAI_LIQUIDATE_REPAY_BORROW_FRESH_FAILED,
        YAI_LIQUIDATE_SEIZE_BALANCE_INCREMENT_FAILED,
        YAI_LIQUIDATE_SEIZE_BALANCE_DECREMENT_FAILED,
        YAI_LIQUIDATE_SEIZE_COMPTROLLER_REJECTION,
        YAI_LIQUIDATE_SEIZE_LIQUIDATOR_IS_BORROWER,
        YAI_LIQUIDATE_SEIZE_TOO_MUCH,
...
/**
 * @title Deswap's DToken Contract
...
contract DToken is DTokenInterface, Exponential, TokenErrorReporter {
    ...
    function transferTokens(address spender, address src, address dst, uint tokens) internal returns (uint) {
        ...
        (mathErr, srdTokensNew) = subUInt(accountTokens[src], tokens);
        if (mathErr != MathError.NO_ERROR) {
        ...
        accountTokens[src] = srdTokensNew;
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint, uint) {
         ...
         *  Note: The dToken must handle variations between BEP-20 and BNB underlying.
    ...
    function borrowFresh(address payable borrower, uint borrowAmount) internal returns (uint) {
        ...
        if (getCashPrior() < borrowAmount) {
            return fail(Error.TOKEN_INSUFFICIENT_CASH, FailureInfo.BORROW_CASH_NOT_AYAILABLE);
        }
    ...
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, DTokenInterface dTokenCollateral) internal returns (uint, uint) {
        ...
        (uint amountSeizeError, uint seizeTokens) = comptroller.liquidateCalculateSeizeTokens(address(this), address(dTokenCollateral), actualRepayAmount);
    ...
    function _reduceReservesFresh(uint reduceAmount) internal returns (uint) {
        ...
        if (getCashPrior() < reduceAmount) {
            return fail(Error.TOKEN_INSUFFICIENT_CASH, FailureInfo.REDUCE_RESERVES_CASH_NOT_AYAILABLE);
        }
...
/**
  * @title Deswap's InterestRateModel Interface
  * @author Deswap
  */
