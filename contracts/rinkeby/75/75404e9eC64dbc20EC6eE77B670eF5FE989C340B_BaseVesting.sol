...
contract BaseVesting is Ownable {
    ...
    constructor(
        ...
        require(
            ...
            "You can't set new start date as past time"
        );
    ...
    function updateContractDate(uint256 newStartTimeStamp) internal {
        ...
        require(
            block.timestamp <= newStartTimeStamp,
            "You can't set new start date as past time"
        );
    ...
    function setInitialTimestamp(uint256 newTimeStamp) public onlyOwner {
        require(
            isInitTimeStampCall == false,
            "You already called this function"
        );
    ...
    function _getRewardBalance(
        ...
        uint256 amountAvailable = mintedToken
            .mul(vestingAvailablePercentage)
            .div(TOTAL_PERCENTAGE);
        uint256 rewardToPay = amountAvailable.mul(percentageOfToken).div(
            TOTAL_PERCENTAGE
        );
        return rewardToPay;
    }

    function _calculateAvailablePercentage(AllocationType allocation)
        internal
        view
        returns (uint256)
    {
        ...
        if (currentTimeStamp < startDate) {
            ...
            startDate <= currentTimeStamp && currentTimeStamp < firstRelease
        ) {
            return tgePercentage;
        } else if (
            firstRelease <= currentTimeStamp &&
            currentTimeStamp < vestingTimeEnd
        ) {
            uint256 numOfPeriods = currentTimeStamp.sub(firstRelease).div(
                PERIOD
            );
            uint256 perioPercentage = (TOTAL_PERCENTAGE.sub(tgePercentage)).div(
                100
            );
