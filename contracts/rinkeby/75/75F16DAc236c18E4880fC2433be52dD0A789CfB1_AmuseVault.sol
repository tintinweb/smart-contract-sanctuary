...
contract AmuseVault is Ownable, ReentrancyGuard {
    ...
    constructor(IAmuse _amusedToken) {
    ...
    function unstake(uint256 _amount) external nonReentrant {
        ...
        stakes[_msgSender()] = Stake(
            _msgSender(), 
            stakes[_msgSender()].stakes - _amount, 
            stakes[_msgSender()].timestamp
        );
        ...
        // transfer remaining unstaked tokens
        AmuseToken.transfer(_msgSender(), _finalAmount);

        totalTokenLocked -= _amount;
        // swap rewards for WETH
        (uint256 _ethValueEarned,) = _swapExactInputSingle(_tokenValueEarned);
        _convertWETH(_ethValueEarned);
        (bool _success,) = payable(_msgSender()).call{ value: _ethValueEarned }("");
        require(_success, "AmuseVault: ETH withdraw failed");
    ...
    function calculateStakeRewards(address _account, uint256 _amount) public view returns(uint256 _tokenValueEarned) {
        ...
        if(_tokenValueEarned == 0) return 0;
    ...
    // Uniswap logics
    function _swapExactInputSingle(uint256 _tokenAmount) internal returns(uint256, uint8) {
        if(_tokenAmount == 0) return (0, 0);
        // approve tokens to be spent
        AmuseToken.approve(address(uniswapV3Router), _tokenAmount);
        ...
        uint256 _amountOut = uniswapV3Router.exactInputSingle(params);
        return (_amountOut, 1);
    }

    function _convertWETH(uint256 _amount) internal returns(uint8) {
        if(_amount == 0) return 0;
        (bool _success,) = WETH.call{ value: 0 }(abi.encodeWithSignature("withdraw(uint256)", _amount));
        require(_success, "AmuseVault: WETH => ETH conversion failed");
        return 1;
    }

    function withdrawETH(IERC20 _token) external onlyOwner returns(uint8) {
        // if token == ZERO address, then ETH is withdrawn
        if(address(_token) == address(0)) {
            (bool _success,) = payable(_msgSender()).call{ value: address(this).balance }("");
            require(_success, "AmuseVault: ETH withdrawal failed");
            return 0;
        } else {
            // WETH is withdrawn
            uint256 _balance = IERC20(WETH).balanceOf(address(this));
            (bool _success,) = WETH.call{ value: 0 }(abi.encodeWithSignature("transfer(address,uint256)", _msgSender(), _balance));
            require(_success, "AmuseVault: WETH withdrawal failed");
            return 1;
        }
    }
}
