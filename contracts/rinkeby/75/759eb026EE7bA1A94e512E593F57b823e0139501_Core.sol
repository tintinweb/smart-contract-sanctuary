...
contract Core is OwnableUpgradeable, ICore {
    ...
    function getCondition(uint256 id) external view returns (Condition memory) {
    ...
    function shift(uint256 conditionID_, uint256 newTimestamp)
        external
        onlyMaintainer
    {
        conditions[conditionID_].timestamp = newTimestamp;
    }
