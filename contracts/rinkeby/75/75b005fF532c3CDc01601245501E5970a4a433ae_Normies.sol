...
contract NormieTraits {
    ...
    string private constant ACCESSORY = "Accessory";
    string private constant ACCESSORY_ONE = "AccessoryOne";
    string private constant ACCESSORY_TWO = "AccessoryTwo";
    string private constant ACCESSORY_THREE = "AccessoryThree";
    ...
    constructor() {
        ...
        traitRarity[0] = [2500, 2500, 2500, 2500];
        traitRarity[1] = [400, 400, 800, 800, 800, 1000, 1000, 1400, 1400, 2000];
        traitRarity[2] = [800, 800, 1200, 1200, 1500, 1500, 1500, 1500];
        traitRarity[3] = [200, 200, 900, 900, 1950, 1950, 1950, 1950];
        traitRarity[4] = [200, 600, 600, 600, 1000, 1000, 1000, 1600, 1600, 1800];
        traitRarity[5] = [200, 400, 1000, 1200, 1200, 2000, 2000, 2000];
        traitRarity[6] = [100, 500, 500, 1500, 1500, 2950, 2950];
        traitRarity[7] = [100,500,500,1000,7900];	
        traitRarity[8] = [100,700,1600,2000,5600];
        traitRarity[9] = [100,500,1500,7900];
    }
    ...
    returns (NormieLib.Normie memory, uint256) {
        ...
        if (encodedTraitType == keccak256(abi.encodePacked(SHOES))){
            trait = getTraitByID(normie.shoes);
            normie.shoes = emptyTrait.traitID;
        } else if (encodedTraitType == keccak256(abi.encodePacked(PANTS))) {
            trait = getTraitByID(normie.pants);
            normie.pants = emptyTrait.traitID;
        } else if (encodedTraitType == keccak256(abi.encodePacked(TORSO))) {
            trait = getTraitByID(normie.torso);
            normie.torso = emptyTrait.traitID;
        } else if (encodedTraitType == keccak256(abi.encodePacked(SKIN_TYPE))
                    && equipAfter) {
            trait = getTraitByID(normie.skinType);
            normie.skinType = emptyTrait.traitID;
        } else if (encodedTraitType == keccak256(abi.encodePacked(MOUTH))) {
            trait = getTraitByID(normie.mouth);
            normie.mouth = emptyTrait.traitID;
        } else if (encodedTraitType == keccak256(abi.encodePacked(EYES))) {
            trait = getTraitByID(normie.eyes);
            normie.eyes = emptyTrait.traitID;
        } else if (encodedTraitType == keccak256(abi.encodePacked(HAIR))) {
            trait = getTraitByID(normie.hair);
            normie.hair = emptyTrait.traitID;
        } else if (encodedTraitType == keccak256(abi.encodePacked(ACCESSORY_ONE))) {
            trait = getTraitByID(normie.accessoryOne);
            normie.accessoryOne = emptyTrait.traitID;
        } else if (encodedTraitType == keccak256(abi.encodePacked(ACCESSORY_TWO))) {
            trait = getTraitByID(normie.accessoryTwo);
            normie.accessoryTwo = emptyTrait.traitID;
        } else if (encodedTraitType == keccak256(abi.encodePacked(ACCESSORY_THREE))) {
            trait = getTraitByID(normie.accessoryThree);
            normie.accessoryThree = emptyTrait.traitID;
        } else {
            revert();
        }
    ...
    returns (NormieLib.Normie memory) {
        ...
        bytes32 encodedTraitType = keccak256(abi.encodePacked(traitType));
        bytes32 newTraitEncoded = keccak256(abi.encodePacked(newTrait.traitType));

        if (newTraitEncoded == keccak256(abi.encodePacked(ACCESSORY))){
            require(encodedTraitType == keccak256(abi.encodePacked(ACCESSORY_ONE))
                || encodedTraitType == keccak256(abi.encodePacked(ACCESSORY_TWO))
                || encodedTraitType == keccak256(abi.encodePacked(ACCESSORY_THREE)), "Accessory mismatch");
        } else {
        ...
        if (encodedTraitType == keccak256(abi.encodePacked(SHOES))){
            ...
            normie.accessoryOne = newTrait.traitID;
        } else if (encodedTraitType == keccak256(abi.encodePacked(ACCESSORY_TWO))) {
            normie.accessoryTwo = newTrait.traitID;
        } else if (encodedTraitType == keccak256(abi.encodePacked(ACCESSORY_THREE))) {
            normie.accessoryThree = newTrait.traitID;
        } else {
            revert();
        }
        return normie;
    ...
    function getNormieTraitObject(NormieLib.Normie memory normie, uint256 traitIndex) private view returns (NormieLib.Trait memory) {
            if (traitIndex == 0) {
                ...
                return getTraitByID(normie.accessoryOne);
            } else if (traitIndex == 8) {
                return getTraitByID(normie.accessoryTwo);
            } else {
                return getTraitByID(normie.accessoryThree);
            }
    }
    ...
    function getOptionalTraitsString(NormieLib.Normie memory normie) private view returns (string memory) {
        return string(abi.encodePacked(
                '","Accessory One":"', getTraitByID(normie.accessoryOne).traitName,
                '","Accessory Two":"', getTraitByID(normie.accessoryTwo).traitName,
                '","Accessory Three":"', getTraitByID(normie.accessoryThree).traitName,
                '"}'
            )
        );
    }
    ...
    function editTrait(uint256 traitID, NormieLib.Trait memory newTraitData) external onlyOwner {
        NormieLib.Trait memory _trait = getTraitByID(traitID);
...
contract Normies is ERC1155, ERC1155Supply {
    ...
    function preMintCheck(uint256 amount) private view {
        ...
        require(amount > 0 && amount <= 100, "Mint01");
