...
contract HarbourElection is AdminRole {
  ...
  function _addVote(
    ...
    Vote existing = _voteByProposalVoter[proposalIndex][voter];
    if (existing != Vote.Null) {
      _voterListByProposal[proposalIndex].push(voter);
    }
    _voteByProposalVoter[proposalIndex][voter] = vote;
  }

  function submitVote(uint256 proposalIndex, bool yes) public {
    _addVote(proposalIndex, yes, msg.sender);
  }

  function processProposal(uint256 proposalIndex) public returns (bool) {
    require(getProposalResult(proposalIndex) == Vote.Null, 'already_processed');
  ...
  function getProposalVote(uint256 proposalIndex, address voter)
    ...
    returns (Vote)
  {
    return _voteByProposalVoter[proposalIndex][voter];
  }

  function getProposalResult(uint256 proposalIndex) public view returns (Vote) {
    return _resultByProposal[proposalIndex];
  }
  ...
  function getProposalVoterCount(uint256 proposalIndex)
    ...
    returns (uint256)
  {
    return _voterListByProposal[proposalIndex].length;
  }

  function getProposalVoterList(uint256 proposalIndex)
    public
    view
    returns (address[] memory)
  {
    return _voterListByProposal[proposalIndex];
  }
}

contract HarbourElectionTesterV2 is HarbourElection {
  ...
  function testAddSingle(
    ...
    _addVote(proposalIndex, yes, voter);
  }
  ...
  function testAddMulti(
    ...
    for (uint160 i = 0; i < count; i++) {
      _addVote(proposalIndex, yes, address(startAddress + i));
    }
  }
}