...
contract AvatarNFT is ERC721, ERC721Enumerable, DNA {
    ...
    constructor(uint256 _maxSupply) ERC721("AvatarNFT", "AVTNFT") {
        maxSupply = _maxSupply;
    }

    function mint() public {
        uint256 current = _idCounter.current();
        require(current < maxSupply, "No AvatarNFT left :(");
        ...
        returns (string memory)
    {
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                 '{ "name": "Avatar NFT #',
                tokenId.toString(),
                '", "description": "Avatar NFT are randomized Avataaars stored on chain to teach DApp development on Avatars", "image": "',
                image,
                '"}'
            )
        );
