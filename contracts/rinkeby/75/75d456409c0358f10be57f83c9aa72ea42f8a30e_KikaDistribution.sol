...
// File: contracts/KikaDistribution.sol
...
// Distribution fee
contract KikaDistribution is OwnableContract {
    using SafeMath for uint256;
    ...
    address private deadAddress = 0x0000000000000000000000000000000000001024;

    // Allocation points assigned to KikaBar
    uint256 public barAllocPoint = 7;
    // Allocation points assigned to deflation kika
    uint256 public deflationAllocPoint = 3;
    // Total allocation points. Must be the sum of all allocation points.
    uint256 public totalAllocPoint = 0;

    uint256 public totalBarDistributionAmount = 0;
    uint256 public totalDeflationDistributionAmount = 0;
    ...
    constructor(address _bar, address _kika) public {
        bar = _bar;
        kika = _kika;

        totalAllocPoint = barAllocPoint.add(deflationAllocPoint);
    }

    // distribute reword
    function distribute() external onlyAdmin() {
        uint256 totalAmount = IERC20(kika).balanceOf(address(this));
        if (totalAmount < 1e18) {
            return;
        }

        // distribute to KikaBar
        uint256 amountBar = totalAmount.mul(barAllocPoint).div(totalAllocPoint);
        IERC20(kika).safeTransfer(bar, amountBar);
        totalBarDistributionAmount = totalBarDistributionAmount.add(amountBar);
        // distribute to deflation
        uint256 amountDeflation = totalAmount.mul(deflationAllocPoint).div(totalAllocPoint);
        IERC20(kika).safeTransfer(deadAddress, amountDeflation);
        totalDeflationDistributionAmount = totalDeflationDistributionAmount.add(amountDeflation);

        emit Distribution(bar, amountBar, amountDeflation);
    }
