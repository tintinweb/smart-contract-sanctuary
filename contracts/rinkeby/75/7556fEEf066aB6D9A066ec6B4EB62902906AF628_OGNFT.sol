...
import "./interfaces/IWingNFT.sol";
    ...
    IMasterBaristaCallback
{
    ...
    mapping(uint256 => uint256) public override wingNFTToCategory;

    mapping(uint256 => EnumerableSetUpgradeable.UintSet)
        private _categoryToWingNFTList;
    ...
    event SetTokenURI(
        uint256 indexed tokenId,
        string indexed prevURI,
        string indexed currentURI
    );
    ...
    event Pause();
    event Unpause();
    event SetOgOwnerToken(
        uint256 indexed categoryId,
        address indexed ogOwnerToken
    );
    event Harvest(
    ...
    modifier withOGOwnerToken(uint256 _tokenId) {
        require(
            address(ogOwnerToken[wingNFTToCategory[_tokenId]]) != address(0),
    ...
    modifier onlyExistingCategoryId(uint256 _categoryId) {
        require(
            _categoryIds.current() >= _categoryId,
    ...
    function categoryToWingNFTList(uint256 _categoryId)
        external
        view
        override
        ...
        returns (uint256[] memory)
    {
        uint256[] memory tokenIds = new uint256[](
            _categoryToWingNFTList[_categoryId].length()
        );
        for (
            uint256 i = 0;
            i < _categoryToWingNFTList[_categoryId].length();
            i++
        ) {
            tokenIds[i] = _categoryToWingNFTList[_categoryId].at(i);
        }
        return tokenIds;
    }
    ...
    function addCategoryInfo(string memory _name, string memory _uri)
        external
        onlyGovernance
    {
        ...
        returns (string memory)
    {
        ...
        if (
            bytes(categoryInfo[wingNFTToCategory[_tokenId]].categoryURI)
                .length > 0
        ) {
            return
                string(
                    abi.encodePacked(
                        base,
                        categoryInfo[wingNFTToCategory[_tokenId]].categoryURI
                    )
                );
        }
    ...
    {
        uint256 categoryIdToBeRemovedFrom = wingNFTToCategory[_tokenId];
        wingNFTToCategory[_tokenId] = _newCategoryId;
        require(
            _categoryToWingNFTList[categoryIdToBeRemovedFrom].remove(_tokenId),
            "OGNFT::updateTokenCategory::tokenId not found"
        );
        require(
            _categoryToWingNFTList[_newCategoryId].add(_tokenId),
            "OGNFT::updateTokenCategory::duplicated tokenId"
        );
     ...
     */
    function getWingNameOfTokenId(uint256 _tokenId)
        external
        view
        override
        returns (string memory)
        ...
        returns (uint256)
    {
        ...
        _tokenIds.increment();
        wingNFTToCategory[newId] = _categoryId;
        require(
            _categoryToWingNFTList[_categoryId].add(newId),
            "OGNFT::mint::duplicated tokenId"
        );
     ...
     */
    function setWingName(uint256 _tokenId, string calldata _name)
        external
        onlyGovernance
    {
    ...
    function _withdrawFromMasterBarista(uint256 _categoryId, uint256 _shares)
        internal
    {
        if (_shares == 0) return;
        masterBarista.withdraw(
            _msgSender(),
            address(ogOwnerToken[_categoryId]),
            _shares
        );
    }
    ...
    /// @param _categoryId specified category Id
    function _harvestFromMasterBarista(address _user, uint256 _categoryId)
        internal
    {
        address stakeToken = address(ogOwnerToken[_categoryId]);
        (uint256 userStakeAmount, , , ) = masterBarista.userInfo(
            stakeToken,
            _user
        );
        ...
        emit Harvest(
            _user,
            _categoryId,
            wing.balanceOf(_user).sub(beforeReward)
        );
    }
    ...
    function _stake(uint256 _tokenId, address _for) internal {
        uint256 categoryId = wingNFTToCategory[_tokenId];
        ...
        IERC20Upgradeable(address(stakeToken)).safeApprove(
            address(masterBarista),
            1 ether
        );
    ...
    function harvest(uint256 _categoryId) external whenNotPaused nonReentrant {
        require(
            address(ogOwnerToken[_categoryId]) != address(0),
            "OGNFT::harvest:: og owner token not set"
        );
    ...
    function harvest(uint256[] calldata _categoryIdParams)
        external
        whenNotPaused
        nonReentrant
    {
        for (uint256 i = 0; i < _categoryIdParams.length; i++) {
            require(
                address(ogOwnerToken[_categoryIdParams[i]]) != address(0),
                "OGNFT::harvest:: og owner token not set"
            );
    ...
    /// @dev currently, the caller will be a master barista calling before a wing lock
...
interface IBoosterConfig {
    ...
    function stakeTokenAllowance(address stakingToken)
        external
        view
        returns (bool);
    ...
    IERC721EnumerableUpgradeable
{
    ...
    function wingNames(uint256 tokenId) external view returns (string calldata);
    ...
    function wingNFTToCategory(uint256 tokenId) external view returns (uint256);

    function categoryToWingNFTList(uint256 categoryId)
        external
        view
        returns (uint256[] memory);
    ...
    function categoryURI(uint256 categoryId)
        external
        view
        returns (string memory);

    function getWingNameOfTokenId(uint256 tokenId)
        external
        view
        returns (string memory);
