...
import "./opensea-creatures/contracts/IFactoryERC1155.sol";
//import "./interfaces/FactoryERC1155.sol";
import "./WarpedApeYachtClub.sol";

contract WarpedApeYachtClubFactory is FactoryERC1155, AuthorizedList, ERC165, ReentrancyGuard {
    using Strings for string;
    using SafeMath for uint256;

    address public proxyRegistryAddress;
    address public nftAddress;
    string internal constant baseMetadataURI = "ipfs://"; // TODO - Change
     ...
     */
    uint256 constant SUPPLY_PER_NFT = 1;
    uint256 constant SUPPLY_PER_TOKEN = UINT256_MAX;

    uint256 public mintedNFTs;
    uint256 public totalNFTs;

    mapping(uint256 => string) private idMetadataLocation;
    mapping(uint256 => bool) public isReserved;
    mapping(uint256 => address) private reservedFor;
    ...
    constructor(address _proxyRegistryAddress, address _nftAddress) {
        ...
        authorizeCaller(_proxyRegistryAddress, true);

        idMetadataLocation[0] = "bafkreigawndlh5r4s3caea53yqvkxnozjynuit6ekqnnryjhsnv6miifs4";  // TODO - Remove after testing
        idMetadataLocation[1] = "bafkreigawndlh5r4s3caea53yqvkxnozjynuit6ekqnnryjhsnv6miifs4";
        idMetadataLocation[2] = "bafkreigawndlh5r4s3caea53yqvkxnozjynuit6ekqnnryjhsnv6miifs4";
        idMetadataLocation[3] = "bafkreigawndlh5r4s3caea53yqvkxnozjynuit6ekqnnryjhsnv6miifs4";
        totalNFTs = 4; // TODO - Remove after testing
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC165) returns (bool) {
        return interfaceId == type(FactoryERC1155).interfaceId || super.supportsInterface(interfaceId);
    }
    ...
    function uri(uint256 _optionId) override public pure returns (string memory) {
        if (_optionId == 0){
            return string("ipfs://bafkreieyrheyns4jqmtzb4gtkyb7dfxckwti6vdzgidvada4cqeenujiiq");
        }

//    return string("ipfs://");
        return string(
            abi.encodePacked(baseMetadataURI, "bafkreieyrheyns4jqmtzb4gtkyb7dfxckwti6vdzgidvada4cqeenujiiq"
//                Strings.toString(_optionId)
    ...
    */
    ...
    function safeTransferFrom(address, address _to, uint256 _optionId, uint256 _amount, bytes calldata _data) external nonReentrant() {
...
//    }
    ...
    function mint(uint256 _optionId, address _toAddress, uint256 _amount, bytes calldata _data) override external nonReentrant() {
     ...
     */
    function _mint(uint256 _option, address _toAddress, uint256 _amount, bytes memory _data) internal {
        require(_canMint(_msgSender(), _option, _amount), "CANNOT_MINT_MORE");
        if (_option < NUM_OPTIONS) {
            require(_isOwnerOrProxy(_msgSender()), "Caller cannot mint");
            uint256 option = _option - NUM_OPTIONS;
            // LootBoxes are not premined, so we need to create or mint them.
            // lootBoxOption is used as a token ID here.
            _createOrMint(nftAddress, _toAddress, mintedNFTs, _amount, _data);
        } else {
     ...
     */
    function _createOrMint(address _erc1155Address, address _to, uint256 _id, uint256 _amount, bytes memory _data) internal {
        WarpedApeYachtClub NFT = WarpedApeYachtClub(_erc1155Address);
        // Lazily create the token
        if (!NFT.exists(_id)) {
            NFT.create(_to, _id, _amount, idMetadataLocation[_id], _data);
            mintedNFTs++;

        } else {
            NFT.mint(_to, _id, _amount, _data);
        }
    }
    ...
    function balanceOf(address _owner, uint256 _optionId) override public view returns (uint256){
        if (!_isOwnerOrProxy(_owner)) {
            // Only the factory's owner or owner's proxy can have supply
            return 0;
        }
        if (_optionId < NUM_OPTIONS) {
            ...
            uint256 currentSupply = WAYC.balanceOf(owner(), _optionId);
            return currentSupply;
//        } else {
//            // We explicitly calculate the token ID here
//            uint256 tokenId = (_optionId - NUM_OPTIONS);
//            WarpedApeYachtClub WAYC = WarpedApeYachtClub(nftAddress);
////            uint256 currentSupply = WAYC.totalSupply(tokenId);
//            uint256 currentSupply = WAYC.totalSupply();
//            // We can mint up to a balance of SUPPLY_PER_NFT
//            return SUPPLY_PER_NFT.sub(currentSupply);
//        }
        }
        return 0; // TODO - Update
    }

    function _canMint(address _fromAddress, uint256 _optionId, uint256 _amount) internal view returns (bool) {
        return _amount > 0 && mintedNFTs + _amount < totalNFTs;
    }
    ...
    // TODO
    function addApe(string memory _uri) external authorized {
        idMetadataLocation[totalNFTs] = _uri;
        totalNFTs++;
    }

    function addApes(string[] memory _uri) external authorized {

    }

    function reserveApe(uint256 id, address whitelistedAddress) external authorized {

    }

    function reserveApes(uint256[] memory id, address[] memory whitelistedAddress) external authorized {
...
import "./opensea-creatures/contracts/ERC1155Tradable.sol";

contract WarpedApeYachtClub is AuthorizedList, ERC1155Tradable {
    using Strings for string;
    using SafeMath for uint256;
...
//    address public proxyRegistryAddress;
    ...
    uint256 public NFT_SUPPLY = 1;
    mapping(address => uint256) public apeHoldings;
    mapping(uint256 => mapping(address => uint256)) private apeBalances;
    mapping(uint256 => uint256) public maxSupply;
//    mapping(uint256 => string) customUri;

//    mapping(uint256 => address) public creators;

    uint256 private _apesMinted;

    constructor(string memory _name, string memory _symbol, string memory _baseNftUri, string memory _contractUri, address _proxyRegistryAddress)
                ERC1155Tradable(_name, _symbol, _baseNftUri, _proxyRegistryAddress){
//        name = _name;
//        symbol = _symbol;
...
//        _initializeEIP712(name);
    ...
    */
    function create(
        ...
        string memory _uri,
        bytes memory _data
    ) public override authorized returns (uint256) {
        require(!_exists(_id), "token _id already exists");

        creators[_id] = _msgSender();

        if (bytes(_uri).length > 0) {
            string memory createdURI = _uri;
            customUri[_id] = createdURI;
            emit URI(createdURI, _id);
        }
        maxSupply[_id] = NFT_SUPPLY;
    ...
    */
    function mint(
        ...
        uint256 _quantity,
        bytes memory _data
    ) override virtual public authorized {
    ...
    */
    ...
    function _safeTransferFrom(
        ...
        apeHoldings[from] -= amount;
        apeBalances[id][to] += amount;

        if(apeHoldings[to] > MAX_APE_HOLDINGS){
            revert("Account cannot hold this many NFTs");
        }
    }
    ...
    function _safeBatchTransferFrom(
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            apeBalances[ids[i]][to] += amounts[i];
            totalSent += amounts[i];
        }

        apeHoldings[to] += totalSent;
        apeHoldings[from] -= totalSent;
    ...
    function _mintBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            apeBalances[ids[i]][to] += amounts[i];
            numMinted++;
            if(tokenSupply[ids[i]] > maxSupply[ids[i]]){
                revert("Token mint exceeds maximum");
            }
        }

        apeHoldings[to] += numMinted;

        if(apeHoldings[to] > MAX_APE_HOLDINGS){
            revert("Account cannot hold this many NFTs");
        }
        _apesMinted += numMinted;
    }

    /**
    * @dev Ape Overrides for naming conventions
    */
