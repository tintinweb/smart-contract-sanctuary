...
/* when publishing set:
- provenance
- owner payment contract
- sale start timestamp
- time LENGTH (seconds in day * 10)
- make sure tokens burn after reveal! (aka they can never be minted)
- pausable
- owner functions
- make withdraw a public function (or set up access roles)
X metadata link
X correct name & symbol
X withdraw function
*/
...
contract PvlaceContract is ERC721, Ownable {
    ...
    address payable OWNER_PAYMENT_CONTRACT =
        0xcC2Ee13961507e09D9bc0D5A9483927C217960e1;

    // Public variables
    uint256 public constant SALE_START_TIMESTAMP = 1618249800;

    // Tokens revealed after 14 days!
    uint256 public constant REVEAL_TIMESTAMP =
        SALE_START_TIMESTAMP + (3600);
    ...
    function getNFTMaxAmount() public view returns (uint256) {
        ...
        if (currentSupply >= 176) {
            ...
            return 7; // Tokens 111 - 175 can only be bought 7 at a time
        } else if (currentSupply >= 51) {
            return 5; // Tokens 51 - 110 can only be bought 5 at a time
        } else {
            return 3; // First 50 tokens can only be bought 3 at a time
        }
    }
    ...
    function getNFTPrice() public view returns (uint256) {
        ...
        if (currentSupply >= 350) {
            ...
            return 200000000000000000; // 51 - 110 0.2 ETH
        } else {
    ...
    /**
     * @dev Withdraw ether from this contract (Anyone can call this but it goes to the Owner's Wallets.)
    */
