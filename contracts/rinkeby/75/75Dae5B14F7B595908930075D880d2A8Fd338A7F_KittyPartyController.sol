...
enum KittyPartyStages {
    ...
    modifier timedTransitions() {
        if (stage == KittyPartyStages.InitialCollection && (block.timestamp >= (lastStageTime + (activityDuration * 1 seconds)))) {
           nextStage(2);
        }
        if (stage == KittyPartyStages.Collection && block.timestamp >= (lastStageTime + (activityDuration * 1 seconds))) {
            nextStage(1);
        }
        if (stage == KittyPartyStages.Staking && block.timestamp >= (lastStageTime + (durationInDays * 1 seconds))) {
            nextStage(1);
        }
        if (stage == KittyPartyStages.Payout && block.timestamp >= (lastStageTime + (durationInDays * 1 seconds)) && (numberOfRounds >= currentRound)) {
