...
// File: contracts/Flattened-StakingUbe-v0.5.7.sol
...
---------------------- [ WPSmartContracts.com ] ----------------------
    ...
    |  Flavor
    |
    |  >  Ube: Fully featured ERC-20 Staking contract with Minimum 
    |          Lock Time, APY and Minimum Deposit
    |
 ...
 */
contract ERC20Staking is Owner, ReentrancyGuard {
    ...
    // limit the amount of tokens to stake
    ...
    constructor(address _owner, uint8 _apy, uint256 _mst, uint8 _penalty, uint256 _minimum, uint256 _limit, ERC20 _token) Owner(_owner) public {
        token = _token;
    ...
    function modifyLimit(uint256 _newVal) external isOwner {
        limit = _newVal;
    }
    
    function modifyMinimum(uint256 _newVal) external isOwner {
        minimum = _newVal;
    }
    ...
    function checkAvailableLimit(address _account) public view returns(uint256){
        require(msg.sender==_account || msg.sender==getOwner(), "You dont have permission");
