...
contract GRPSBeneficiaryTokenVesting2 is Ownable {
  ...
  mapping (address => uint256) public startOfLastReleasedPeriod;
  ...
  modifier onInit {
    require(!init, "This action cannot be performed again");
    _;
  }
  ...
  function addBeneficiaries(address[] memory _beneficiaryes, uint256[] memory _tokenAmounts) 
    onlyOwner
    onInit
  public {
    ...
    require(totalAmount <= token.balanceOf(address(this)), "Invalid token amount");
    init = true;
  }
  ...
  function addBeneficiary(address _beneficiary, uint256 _tokenAmount) private {
    ...
    startOfLastReleasedPeriod[_beneficiary] = lockupTime;
  ...
  function claimTokens() onlyBeneficiaries public {
    ...
    if (unreleased > 0) {
      released += unreleased;
      startOfLastReleasedPeriod[msg.sender] = floorToPeriod(block.timestamp);
  ...
  function releasableAmount() private view returns (uint256) {
    if (block.timestamp < lockupTime) {
      ...
      if(block.timestamp < startOfLastReleasedPeriod[msg.sender]) return 0;
      ...
      if(block.timestamp >= start + duration) {
        return tokenAmounts[msg.sender] - releasedAmount[msg.sender];
      } else {
        periodsPassed = (block.timestamp - startOfLastReleasedPeriod[msg.sender]) / period;
      }

      if (periodsPassed > 0) {
        result += tokenAmounts[msg.sender] * (periodsPassed * percent) / BP;
      } 
  ...
  function floorToPeriod(uint timestamp) private view returns(uint){
    if(timestamp < lockupTime){
      return 0;
    }
    if(period == 0){
      return timestamp; //nothing to round
    }
    
    uint timeAfterLockup = timestamp - lockupTime;
    // remove %period to get a timeAfterLockup that is multiple of period
    // and then readd lockupTime
    return timeAfterLockup - (timeAfterLockup % period) + lockupTime;
  }
}
