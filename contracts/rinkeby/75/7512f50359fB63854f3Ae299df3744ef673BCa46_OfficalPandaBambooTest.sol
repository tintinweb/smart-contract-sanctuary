...
contract OfficalPandaBambooTest is ERC721, Ownable, ReentrancyGuard {
    using Counters for Counters.Counter;

    uint256 public MAX_TEAM_RESERVES_POSTSALE = 100;
    //allows for 100 tokens to remain for team reserved to be minted after public sale
    uint256 public constant MAX_PUBLIC_TOKENS = 9900;
    //limits pre and public sale totals to 9900 leaving 100 for team reserves
    uint256 public constant MAX_TOKENS_TOTAL_SUPPLY = 10000;
    uint256 public MAX_PRESALE_TOKENS_PER_ADDRESS = 5;
    uint256 public PRESALE_TOKEN_PRICE = 0.07 ether;
    ...
    function presaleMint(uint256 quantity, bytes32[] calldata PresaleMerkleProof) external payable nonReentrant {
        ...
        require(totalSupply() + quantity <= MAX_PUBLIC_TOKENS, "minting this many would exceed supply");
    ...
    function mint(uint256 quantity) external payable nonReentrant {
        ...
        require(totalSupply() + quantity <= MAX_PUBLIC_TOKENS, "invalid quantity: would exceed max supply");
    ...
    function teamReservesPostsaleMint(uint256 quantity) public onlyOwner {
        require(quantity <= MAX_TEAM_RESERVES_POSTSALE, "Can't reserve more than set amount" );
        MAX_TEAM_RESERVES_POSTSALE -= quantity;
        require(totalSupply() + quantity <= MAX_TOKENS_TOTAL_SUPPLY, "invalid quantity: would exceed max supply");
        _minter(msg.sender, quantity);
    }
