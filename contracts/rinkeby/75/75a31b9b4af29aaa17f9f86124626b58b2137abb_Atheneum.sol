...
contract Atheneum is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    string private _metadataURI = "https://theAtheneum.net/api/tokenInfo/";
    ...
    // Token Id to poem
    mapping(uint256 => string) private _tokenIdToPoem;
    ...
    function tokenIdForName(string memory _topic)
        external
        view
        returns (uint256)
    {
        return _nameToTokenId[_topic];
    }

    function tokenIdForPoem(uint256 tokenId)
        external
        view
        returns (string memory)
    {
        return _tokenIdToPoem[tokenId];
    }
    ...
    function _getPurchasePrice() public view returns(uint256) {
        if (_tokenSupply == 0) {
            return 0;
        } else if(_tokenSupply >= 1 && _tokenSupply <= 2) {
            return 10000000000000000 wei;
        } else if(_tokenSupply >= 3 && _tokenSupply <= 4) {
    ...
    function _mint(
        ...
        // _namePurchases[_topic] = true;
        // _nameToTokenId[_topic] = _newTokenId;
        // _tokenIdToName[_newTokenId] = _topic;
        // _tokenIdToPoem[_newTokenId] = _poem;
    ...
    function mint(
        ...
        require(msg.value >= _getPurchasePrice(), "Insufficient message value");
    ...
    function ownerMint(
        ...
        //     "That named piece has already been purchased"
        // );
        uint256 _newTokenId = _mint(_owner
                                    // , 
                                    // _topic, 
                                    // _poem
                                );
        ...
        returns (string memory)
    {
        ...
        string memory poem = _tokenIdToPoem[tokenId];

        string memory output = string(abi.encodePacked(poem));
        string memory packed = string(abi.encodePacked('{"name": "', topic , '", "description": "A torn page, stolen from a book within the Atheneum. https://theatheneum.net", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'));
