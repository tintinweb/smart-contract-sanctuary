...
contract BaboonBrigade is ERC721Enumerable, Ownable {
    ...
    event PreSale(uint256 tokenId, address sender, uint256 amount);
    event PrivateSale(uint256 tokenId, address sender, uint256 amount);
    ...
    function preSale(uint256 tokenQuantity) external payable {
        ...
        if (_state == State.PrivatePreSaleFirst) {
            ...
            require(
                BBG_PRICE * tokenQuantity <= msg.value,
                "Not Enough payments"
            );
