...
contract exchangeNFT {
    ...
    function setPrice(uint256 _tokenId, uint256 tokenPrice) external {
        require ( msg.sender == IaunToken721(erc721).ownerOf(_tokenId) );
    ...
    function buyNFT(uint256 _tokenId) external {
        ...
        require( tokenPrice <= IaunToken(erc20).balanceOf(recipient) );
        address owner = IaunToken721(erc721).ownerOf(_tokenId);
        
        IaunToken(erc20).transferFrom(recipient, owner, tokenPrice);
        IaunToken721(erc721).transferFrom(owner, recipient, _tokenId);
