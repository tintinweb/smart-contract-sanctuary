...
contract AdAuction is Ownable {
    ...
    //参与竞拍
    event participateBidding(uint256 indexed adIndex,uint256 indexed epoch,uint256 amount,address biddingAddress,string image,string description,string name,string category,uint256 lastBlock);
    //竞拍成功
    event successfulBidding(uint256 indexed adIndex,uint256 indexed epoch,address userAddress,uint256 lastBlock);
    //结束竞拍
...
//        }
        if(adPrice[adIndex]> 0){
            ...
            emit successfulBidding(adIndex,currentRound,adUser.userAddress,adUser.lastBlock);
            adPrice[adIndex] = 0;
        }
    ...
    */
    function _auctionBilling(uint256 adIndex,string memory image,string memory description,string memory name,string memory category) external locationExists(adIndex) biddingLock{
        ...
        //同一区块不能连续竞价
        require(adUser.lastBlock < block.number,"NO_CONTINUOUS_BIDDING_FOR_THE_SAME_BLOCK");
        if(adUser.amount <= 0){
        ...
        adUser.lastBlock = block.number;
        adUser.amount = adUser.amount.add(thisBid);
        if(round.lastUserAmount > 0){
        ...
        emit participateBidding(adIndex,currentRound,thisBid,msgSender,image,description,name,category,adUser.lastBlock);
    }
