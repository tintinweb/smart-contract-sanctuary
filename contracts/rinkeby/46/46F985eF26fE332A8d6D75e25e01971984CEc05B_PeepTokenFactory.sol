...
contract PeepTokenFactory is Ownable {
	...
	PeepToken public immutable peepToken;
	...
	constructor(address peepTokenAddr) {
		peepToken = PeepToken(peepTokenAddr);
	}
	 ...
	 */
	function reservedMint(uint256 amount) public {
		require(
			amount <= reservedAmount[msg.sender],
			'PeepTokenFactory: exceeded reserved amount'
		);
		peepToken.doMint(msg.sender, amount);
