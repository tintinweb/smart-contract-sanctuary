...
contract ProductTokenHighBase is ProductTokenCore {
   ...
   * @param _coolTime cool time is a interval which user should wait when sell after buy.
   *
  */
  function initialize(
    string memory _name,
  ...
  */
  function buy(uint256 maxPrice_) external virtual whenNotPaused nonReentrant {
    ...
    if(coolTime > 0) {
      coolTimes[_msgSender()] = now256() + coolTime;
    }
  }
  ...
  */
  function sell(uint32 amount_) external virtual whenNotPaused nonReentrant {
    ...
    require(now256() > coolTimes[_msgSender()], "wait to cool down");
...
contract ProductTokenCore is ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  /// @dev cool time is a interval which user should wait when sell after buy
  ...
  */
  ...
  /**
  * @dev a list stored cool time of every user
  */
  mapping(address => uint256) public coolTimes;
  ...
  /**
    * @dev Fired in updateCoolTime()
    *
    * @param sender an address which performed an operation, usually owner
    * @param coolTime a timestamp which restrict user to wait when sell
    */
  event UpdateCoolTime(address indexed sender, uint256 coolTime);
  ...
  */
  function _buy(uint256 _deposit) internal virtual returns (uint256 change)
  {
    ...
    // 75% of fee, is for platform
    _updatePlatformFee(fee * (FEE_RATE_IN_BUY - FEE_MULTIPLIER) / FEE_RATE_IN_BUY);
  ...
  */
  ...
  function updateCoolTime(uint256 time_) public virtual onlyOwner {
    coolTime = time_;
    emit UpdateCoolTime(_msgSender(), time_);
  }
  ...
  */
  ...
  /**
    ...
    *
    * @dev Additionally to the parent smart contract, add cool time limitation
    * @dev user will not able to transfer within cool time (for more see buy, sell, transferFrom)
    */
  ...
  /**
    ...
    *
    * @dev Additionally to the parent smart contract, add cool time limitation
    * @dev user will not able to transferFrom within cool time (for more see buy, sell, transfer)
    */
  function transferFrom(
    ...
    require(now256() > coolTimes[sender], "wait to cool down");
