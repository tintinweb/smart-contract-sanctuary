...
contract ayfyifg is ERC721PresetMinterPauserAutoId, VRFConsumerBase, Data  {
    ...
    address public tokenAddress = 0xdbfb96a79efFEA8Ce8D4abc8B177ab03fef2C450; // change this to ;
    ...
    bool internal hatchingEnabled = true;
    ...
    event InitialHatch(uint256 tokenId, uint256 createdAt, address tokenOwner, uint256 color, uint256 randomness);
    event Hatched(uint256 tokenId, uint256 createdAt, address tokenOwner, uint256 color);
    ...
    constructor(
        ...
        ERC721PresetMinterPauserAutoId("geagea", "geagea", "ipfs://") 
    {
    ...
    function tokenURI(uint256 id) public view virtual override returns (string memory) {
        Device memory device = unit[id];
        
        string memory name = device.name;
        string memory NAME_IPFS = nameIPFS[keccak256(abi.encode(name))];
        
        if (growthStage(id) == DeviceState.Egg) return IPFS(EGG_IPFS);
        if (growthStage(id) == DeviceState.Gen1) return IPFS(NAME_IPFS);
        if (growthStage(id) == DeviceState.Gen2) return IPFS(NAME_IPFS);
        if (growthStage(id) == DeviceState.Gen3) return IPFS(NAME_IPFS);
        if (growthStage(id) == DeviceState.Gen4) return IPFS(NAME_IPFS);
        if (growthStage(id) == DeviceState.Gen5) return IPFS(NAME_IPFS);
        if (growthStage(id) == DeviceState.Gen6) return IPFS(NAME_IPFS);
        if (growthStage(id) == DeviceState.Gen7) return IPFS(NAME_IPFS);
        if (growthStage(id) == DeviceState.Gen8) return IPFS(NAME_IPFS);
        if (growthStage(id) == DeviceState.Gen9) return IPFS(NAME_IPFS);
        if (growthStage(id) == DeviceState.Gen10) return IPFS(NAME_IPFS);
        return IPFS(DEAD_IPFS);
    }
    
    // return string for TokenURI
    function IPFS(string memory _IPFS) internal view returns (string memory) {
        return string(abi.encodePacked(_baseURI(), _IPFS));
    }
    ...
    function setMetadata() public onlyOwner {
        for (uint256 index = 0; index < names.length; index++) {
            nameIPFS[keccak256(abi.encode(names[index]))] = metadata[index];
        }
    }
    ...
    // change connected token address
    ...
    // batch minter with custom pass amount and color
    function batchMint(address[] memory recipients, uint256 _availableFreeDays, uint256 _color) public onlyOwner {
        uint256 i = 0;
    ...
    function hatch(uint256 tokenId) public {
        ...
        if (device.color > 0) {
            ...
            if (device.color == 2) {
                device.name = names[10];
            }
            if (device.color == 3) {
                device.name = names[20];
            }
            if (device.color == 4) {
                device.name = names[30];
            }
            if (device.color == 5) {
                device.name = names[40];
            }
            
            device.generation = 1;
            
            emit Hatched(tokenId, block.timestamp, ownerOf(tokenId), device.color);
            return;
        }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        emit InitialHatch(tokenId, device.createdAt, ownerOf(tokenId), device.color, randomness);
    }
    ...
    function growthStage(uint256 tokenId) public view returns (DeviceState) {
        ...
        if (device.feedLevel > fullCycles) return findGeneration(device.generation);
    ...
    function findGeneration(uint256 level) internal pure returns (DeviceState generationLevel_){
        if (level == 1) return DeviceState.Gen1;
        if (level == 2) return DeviceState.Gen2;
        if (level == 3) return DeviceState.Gen3;
        if (level == 4) return DeviceState.Gen4;
        if (level == 5) return DeviceState.Gen5;
        if (level == 6) return DeviceState.Gen6;
        if (level == 7) return DeviceState.Gen7;
        if (level == 8) return DeviceState.Gen8;
        if (level == 9) return DeviceState.Gen9;
        if (level >= 10) return DeviceState.Gen10;
    }
            ...
            if (device.generation < 10) {
                for (uint i = 1; i < 10; i++) {
                    if (device.generation == i) {
                        ...
                        if (device.color == 1) {
                            device.name = names[i.add(1)];
                        }
                        if (device.color == 2) {
                            device.name = names[(i.add(10)).add(1)];
                        }
                        if (device.color == 3) {
                            device.name = names[(i.add(20)).add(1)];
                        }
                        if (device.color == 4) {
                            device.name = names[(i.add(30)).add(1)];
                        }
                        if (device.color == 5) {
                            device.name = names[(i.add(40)).add(1)];
                        }
                        // add to local rewards counters
                        device.tokensToRevive = device.tokensToRevive.add(amount);
                        device.rewards = device.rewards.add(amount);
    ...
    function prestige(uint256 tokenId) public {
        ...
        uint256 elapsed = block.timestamp - device.createdAt;
        uint256 fullCycles = uint256(elapsed).div(ONE_DAY);
        // require(keccak256(bytes(device.name)) != keccak256(bytes(nameList[40])), "g1");
    ...
    function getReward(uint256 tokenId) public {
        ...
        uint256 tempAmount = device.rewards;
        uint256 amount;
    ...
    // get internal data
    function internalData() public view returns (uint256 pendingRewards_, uint256 paidRewards_, bool mintEnabled_, bool hatchingEnabled_) {
        return (pendingRewards, paidRewards,mintEnabled, hatchingEnabled);
    }
    ...
    // feed and play time decentralized data
    function getTimeData(uint256 tokenId) public view returns (
        ...
        if (device.createdAt != MAX_TIMESTAMP && alive(tokenId)) {
            ...
            uint256 feedWindow = ONE_HOUR.mul(device.generation);
            
            if (block.timestamp.sub(device.lastPlayTime) > ONE_HOUR.add(device.prestige.mul(ONE_HOUR))) { // in play
                elapsed = block.timestamp.sub(device.lastPlayTime);
                if (device.feedLevel > fullCycles) { // not in a feed
                    return (
                        true, 0, ONE_HOUR.add(device.prestige.mul(ONE_HOUR)), device.generation,  
                        false, device.feedLevel.mul(ONE_DAY).sub(feedElapsedTime), feedWindow 
                    );
                }
                if (device.feedLevel == fullCycles && modulo < feedWindow) { // in feeding window
                    return (
                        true, 0, ONE_HOUR.add(device.prestige.mul(ONE_HOUR)), device.generation, 
                        true, 0,  feedWindow.sub(modulo)
                    );
                }
            } else { // not in play
                elapsed = block.timestamp.sub(device.lastPlayTime);
                if (device.feedLevel > fullCycles) {  // not in a feed
                    return (
                        false, ONE_HOUR.add(device.prestige.mul(ONE_HOUR)).sub(elapsed), ONE_HOUR.add(device.prestige.mul(ONE_HOUR)), device.generation, 
                        false, device.feedLevel.mul(ONE_DAY).sub(feedElapsedTime), feedWindow
                    );
                }
                if (device.feedLevel == fullCycles && modulo < feedWindow) { // in feeding window
...
contract Data is Ownable {
    ...
    string[50] public names;
    string[50] public metadata;
    
    function updateMetadataIPFS(string[50] memory _metadata) public onlyOwner {
        metadata = _metadata;
    }
    
    function updateNamesIPFS(string[50] memory _names) public onlyOwner {
        names = _names;
    }
    
    function updateEggIPFS(string memory _EGG_IPFS) public onlyOwner {
        EGG_IPFS = _EGG_IPFS;
    }
    
    function updateDeadIPFS(string memory _DEAD_IPFS) public onlyOwner {
    ...
    /* ERROR MESSAGESS FROM REQUIREMENTS
        ...
        !e = not egg
        !f = must feed
        !g = must prestige to continue or a egg
        ...
        !x = not alive
        x! = alive
        !11 = not generation 11
        a0 = must enter amount
