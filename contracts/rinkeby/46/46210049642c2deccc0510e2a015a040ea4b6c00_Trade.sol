...
interface IBEP1155 is IBEP165 {
    ...
    /**
        ...
        @param _data    Additional data with no specified format, MUST be sent unaltered in call to `onBEP1155Received` on `_to`
    */
    ...
    /**
        ...
        @param _data    Additional data with no specified format, MUST be sent unaltered in call to the `BEP1155TokenReceiver` hook(s) on `_to`
    */
...
contract TransferProxy {
    ...
    function bep721safeTransferFrom(IBEP721 token, address from, address to, uint256 tokenId) external  {
        token.safeTransferFrom(from, to, tokenId);
    }

    function bep1155safeTransferFrom(IBEP1155 token, address from, address to, uint256 id, uint256 value, bytes calldata data) external  {
        token.safeTransferFrom(from, to, id, value, data);
    }
    
    function bep20safeTransferFrom(IBEP20 token, address from, address to, uint256 value) external  {
...
contract Trade {
    ...
    enum BuyingAssetType {BEP1155, BEP721}
    ...
    function getFees(uint paymentAmt, BuyingAssetType buyingAssetType, address buyingAssetAddress, uint tokenId) internal view returns(Fee memory){
        ...
        if(buyingAssetType == BuyingAssetType.BEP721) {
            royaltyPermille = ((IBEP721(buyingAssetAddress).royaltyFee(tokenId)));
            tokenCreator = ((IBEP721(buyingAssetAddress).getCreator(tokenId)));
        }
        if(buyingAssetType == BuyingAssetType.BEP1155)  {
            royaltyPermille = ((IBEP1155(buyingAssetAddress).royaltyFee(tokenId)));
            tokenCreator = ((IBEP1155(buyingAssetAddress).getCreator(tokenId)));
        }
    ...
    function tradeAsset(Order memory order, Fee memory fee) internal virtual {
        if(order.nftType == BuyingAssetType.BEP721) {
            transferProxy.bep721safeTransferFrom(IBEP721(order.nftAddress), order.seller, order.buyer, order.tokenId);
        }
        if(order.nftType == BuyingAssetType.BEP1155)  {
            transferProxy.bep1155safeTransferFrom(IBEP1155(order.nftAddress), order.seller, order.buyer, order.tokenId, order.qty, ""); 
        }
        if(fee.platformFee > 0) {
            transferProxy.bep20safeTransferFrom(IBEP20(order.bep20Address), order.buyer, owner, fee.platformFee);
        }
        if(fee.royaltyFee > 0) {
            transferProxy.bep20safeTransferFrom(IBEP20(order.bep20Address), order.buyer, fee.tokenCreator, fee.royaltyFee);
        }
        transferProxy.bep20safeTransferFrom(IBEP20(order.bep20Address), order.buyer, order.seller, fee.assetFee);
    }
    ...
    function buyAsset(Order memory order, Sign memory sign) public returns(bool) {
        ...
        verifySellerSign(order.seller, order.tokenId, order.unitPrice, order.bep20Address, order.nftAddress, sign);
    ...
    function executeBid(Order memory order, Sign memory sign) public returns(bool) {
        Fee memory fee = getFees(order.amount, order.nftType, order.nftAddress, order.tokenId);
        verifyBuyerSign(order.buyer, order.tokenId, order.amount, order.bep20Address, order.nftAddress, order.qty, sign);
