...
contract GuardianKongzToken is ERC20, Ownable {
    iGuardianKongz public GuardianKongz;

    event rewardClaim(address indexed _from, uint256 _amount);
    event rewardBurn(address indexed _from, uint256 _amount);

    bool public rewardPaused = false;

    uint256 constant public GEN_LEG_BASE = 50 ether;
    uint256 constant public GEN_BASE = 30 ether;
    uint256 constant public LEG_BASE = 10 ether;
    uint256 constant public ORI_BASE = 2 ether;
    ...
    constructor(address _gurdianKongzAddress, uint256 _start) ERC20("gkongz", "GKONGZ") {
        GuardianKongz = iGuardianKongz(_gurdianKongzAddress);
        START_TIME = _start;
    }
    ...
    function claimReward() external {
        ...
        uint256[] memory KongzId = GuardianKongz.walletOfOwner(msg.sender);
    ...
    function burn(uint256 _amount) external {
        ...
        emit rewardBurn(msg.sender, _amount);
    }
