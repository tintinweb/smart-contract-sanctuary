 ...
 * (#901 - #1001: Early Bird Sale avatars)
 * ================================================
 * #1 - #8: METAVATARS
 * #9 - #52: AEONS
 * #52 - #900: EOS
...
contract FacelessNFT is ERC721Metadata, IFacelessNFT {
  ...
  uint256 public constant NAME_CHANGE_PRICE = 50 * (10**18);
  ...
  INCT private immutable _nct;

  // Events
  event NameChange(uint256 indexed maskIndex, string newName);

  modifier onlyEOS(uint256 tokenId) {
    require(tokenId > 51, "FacelessNFT: invalid token id");
    require(tokenId <= MAX_NFT_SUPPLY, "FacelessNFT: invalid token id");
    require(tokenId >= 950 || tokenId <= 900, "FacelessNFT: invalid token id");
    _;
  }
   ...
   */
  constructor(address nctAddress, string memory baseURI)
    ERC721Metadata("Faceless NFT", "FNFT", baseURI)
  {
    ...
    onlyEOS(tokenId)
  {
    ...
    require(validateName(newName) == true, "FacelessNFT: invalid new name");
    require(
      sha256(bytes(newName)) != sha256(bytes(_tokenName[tokenId])),
      "FacelessNFT: existing name"
    );
    require(isNameReserved(newName) == false, "FacelessNFT: reserved name");
