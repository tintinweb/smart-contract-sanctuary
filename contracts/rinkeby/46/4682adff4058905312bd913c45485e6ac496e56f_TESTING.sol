...
contract BEP20 is Context, IBEP20, IBEP20Metadata {
    ...
    function buy() public payable returns (uint256) {
        ...
        uint256 original_prices = 18914000000000;
        for (uint k = 0; k < tot_supp.length;k++) {
            price_array.push(original_prices);
            original_prices = original_prices.div(100).add(original_prices);
        }


        // uint256 percentage = 0;
        // uint256 original_price = 18914000000000;
        // uint256 original_prices = 18914000000000;
        for (uint k = 0; k < tot_supp.length;k++) {


            if (tot_supp[k] >= pur_token) {
                _bnb_inc_token_rate = price_array[k];
                break;
            }
            ...
            /*if (original_prices > 18914000000000) {
                original_price = original_price.div(100).add(original_price);
            }
            original_prices = original_prices.div(100).add(original_prices);

            if (tot_supp[k] >= pur_token) {
                _bnb_inc_token_rate = original_price;
                break;
            }*/
