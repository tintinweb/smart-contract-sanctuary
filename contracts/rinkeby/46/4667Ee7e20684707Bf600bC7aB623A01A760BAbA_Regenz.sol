...
contract Regenz is ERC721Burnable, Ownable {
  ...
  constructor(address _claimEthAddress) public ERC721("Regenz", "REGENZ") {
  ...
  */
  ...
  function claimETH() public onlyOwner {
    address payable payableAddress = payable(address(claimEthAddress));
    payableAddress.transfer(address(this).balance);
    // payable(owner()).transfer(address(claimEthAddress).balance);
  }
