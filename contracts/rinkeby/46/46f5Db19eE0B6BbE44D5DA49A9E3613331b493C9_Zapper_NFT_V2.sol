...
contract SignatureVerifier_V2 {
    ...
    function verify(
        ...
        bytes32 messageHash = getMessageHash(account, volts, nonce);
...
contract Zapper_NFT_V2 is ERC1155, Ownable, SignatureVerifier_V2 {
    ...
    //Mapping from address to Volt balance
    ...
    event ClaimVolts(address account, uint256 voltsRecieved, uint256 nonce);

    //Emitted when `account` burns Volts
    event BurnVolts(address account, uint256 voltsBurned);

    //Emitted when `account` mints one or more NFTs by spending Volts
    event Mint(address account, uint256 voltsSpent);

    //Emitted when `account` redeems Volts by burning one or more NFTs
    event Redeem(address account, uint256 voltsRecieved);

    //Emitted when `account` crafts one or more of the same NFT
    event Craft(address account, uint256 craftID);

    //Emitted when `account` crafts multiple different NFTs
    event CraftBatch(address account, uint256[] craftIDs);
    ...
    function add(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            uint256 newId = ids[i];
            require(exists[newId] == false, "ID already exists");
    ...
    ///TODO: Remove!!
    function distributeVolts(
        address operator,
        uint256 quantity,
        bool shouldBurn
    ) external onlyOwner {
        if (shouldBurn) {
            voltBalance[operator] -= quantity;
            voltSupply -= quantity;
            return;
        }
        voltBalance[operator] += quantity;
     ...
     * @param signature The signature granding msg.sender the volts
     */
    ...
    {
        require(
            verify(msg.sender, voltsEarned, nonces[msg.sender]++, signature),
            "Invalid Signature"
        );
        voltBalance[msg.sender] += voltsEarned;
        voltSupply += voltsEarned;
    ...
    function burnVolts(uint256 voltsBurned) external pausable {
        require(
            voltBalance[msg.sender] >= voltsBurned,
            "Burn amount exceeds balance"
        );

        voltBalance[msg.sender] -= voltsBurned;

        voltSupply -= voltsBurned;
    ...
    function mint(uint256 id, uint256 quantity) external pausable {
        ...
        require(cost[id] > 0, "Cost not set");

        uint256 voltsSpent = cost[id] * quantity;
        require(
            voltBalance[msg.sender] >= voltsSpent,
            "Insufficient Volt balance"
        );
        voltBalance[msg.sender] -= voltsSpent;
        voltSupply -= voltsSpent;
        ...
        pausable
    {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            require(cost[ids[i]] > 0, "Cost not set");

            voltsSpent += cost[ids[i]];
    ...
    function redeem(uint256 id, uint256 quantity) external pausable {
        ...
        voltBalance[msg.sender] += voltsRecieved;
        voltSupply += voltsRecieved;
    ...
    function craft(uint256 id, uint256 quantity) external pausable {
        ...
        tokenSupply[id] -= quantity;
        tokenSupply[craftID] += craftQuantity;
        ...
        pausable
    {
        ...
        for (uint256 i = 0; i < quantities.length; i++) {
            ...
            craftIds[i] = craftID;
            tokenSupply[ids[i]] -= quantities[i];
            tokenSupply[craftIds[i]] += craftQuantities[i];
        }
    ...
    /**
     * @dev Updates the redeem modifier which exchanges an NFT for Volts
     */
