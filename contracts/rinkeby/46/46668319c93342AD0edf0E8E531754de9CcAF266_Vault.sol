...
contract Vault is BaseContract {
  ...
  address public uinswapV3QuoterAddress;
  ...
  DepositableERC20 weth = DepositableERC20(wethAddress);
  ...
  constructor(
    address payable _uinswapV3QuoterAddress,
    address payable _uinswapV3RouterAddress,
    address payable _chainLinkETHUSDAddress,
    address payable _daiAddress,
    address payable _wethAddress
  ) {
    console.log('Deploying Vault Version:', version);
    uinswapV3QuoterAddress = _uinswapV3QuoterAddress;
    uinswapV3RouterAddress = _uinswapV3RouterAddress;
  ...
  function getTotalBalance() public view returns(uint) {
    ...
    uint ethUSD = ethBalance * ethPrice; // assumes both assets have 18 decimals
    uint totalBalance = ethUSD + wethUSD + daiBalance;
    return totalBalance;
  }
  ...
  function updateEthPriceChainlink() public returns(uint) {
    uint256 chainLinkEthPrice = EACAggregatorProxy(chainLinkETHUSDAddress).latestAnswer();
  ...
  function buyWeth(uint amountUSD) internal {
    ...
    require(dai.approve(address(uinswapV3RouterAddress), amountIn), 'DAI approve failed');
  ...
  function sellWeth(uint amountUSD) internal {
    ...
    uint160 sqrtPriceLimitX96 = 0;
    require(weth.approve(address(uinswapV3RouterAddress), amountOut), 'WETH approve failed');
  ...
  function annualDividend() public onlyOwner {
  ...
  function closeAccount() public onlyOwner {
    uint daiBalance = getDaiBalance();
    if (daiBalance > 0) {
      dai.safeTransfer(msg.sender, daiBalance);
    }
    uint wethBalance = getWethBalance();
    if (wethBalance > 0) {
