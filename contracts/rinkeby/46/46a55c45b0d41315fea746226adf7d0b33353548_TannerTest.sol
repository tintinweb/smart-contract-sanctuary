...
contract TannerTest is ERC721, ERC721Enumerable, Ownable {
   ...
   IYolk public Yolk;
    ...
    event successfulMint(address minter, uint256 amount);
...
//test if changing cost works with the ether there.
  ...
  //can't add original metadata makes it too big... add delete function  
 //add original metadata in native initial contract   
    ...
    function initiateMetadata1() public onlyOwner {
        for (uint256 i = 1; i<= 1000; i++){
            string memory temp = string(abi.encodePacked("chicken",i));
            _commonMetadata.push(temp);
        }
    }
     function initiateMetadata2() public onlyOwner {
        for (uint256 i = 1001; i<= 2000; i++){
            string memory temp = string(abi.encodePacked("chicken",i));
            _commonMetadata.push(temp);
        }
    }
     function initiateMetadata3() public onlyOwner {
        for (uint256 i = 2001; i<= 3000; i++){
            string memory temp = string(abi.encodePacked("chicken",i));
            _commonMetadata.push(temp);
        }
    }
     function initiateMetadata4() public onlyOwner {
        for (uint256 i = 3001; i<= 4000; i++){
            string memory temp = string(abi.encodePacked("chicken",i));
            _commonMetadata.push(temp);
        }
    }
     function initiateMetadata5() public onlyOwner {
        for (uint256 i = 4001; i<= 5000; i++){
            string memory temp = string(abi.encodePacked("chicken",i));
            _commonMetadata.push(temp);
        }
    }
     function initiateMetadata6() public onlyOwner {
        for (uint256 i = 5001; i<= 6000; i++){
            string memory temp = string(abi.encodePacked("chicken",i));
            _commonMetadata.push(temp);
        }
    }
     function initiateMetadata7() public onlyOwner {
        for (uint256 i = 6001; i<= 7000; i++){
            string memory temp = string(abi.encodePacked("dragpn",i));
            _rareMetadata.push(temp);
        }
    }
    function initiateMetadata8() public onlyOwner {
        for (uint256 i = 7001; i<= 8000; i++){
            string memory temp = string(abi.encodePacked("dragpn",i));
            _rareMetadata.push(temp);
        }
    }
    function initiateMetadata9() public onlyOwner {
        for (uint256 i = 8001; i<= 9000; i++){
            string memory temp = string(abi.encodePacked("dragpn",i));
            _rareMetadata.push(temp);
        }
    }
    function initiateMetadata10() public onlyOwner {
        for (uint256 i = 9001; i<= 10000; i++){
            string memory temp = string(abi.encodePacked("phoenix",i));
            _legendaryMetadata.push(temp);
        }
    }
    ...
    function commonMetadataLength () external view returns(uint256){
        return _commonMetadata.length;
    }


    function rareMetadataLength () external view returns(uint256){
        return _rareMetadata.length;
    }

     function legendaryMetadataLength () external view returns(uint256){
        return _legendaryMetadata.length;
    }
    ...
    function mintAllowList(uint8 numberOfTokens) external payable {
        ...
        emit successfulMint(msg.sender, numberOfTokens);
    }
    ...
    function reserveEggs(address to, uint256 n) public onlyOwner {
