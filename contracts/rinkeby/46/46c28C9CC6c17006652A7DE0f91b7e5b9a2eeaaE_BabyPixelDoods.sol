...
contract BabyPixelDoods is ERC721A, Ownable {
    ...
    constructor() ERC721A("BabyPixelDoods", "BPD", MAX_MINT_PER_TX, MAX_SUPPLY) {
    }
    ...
    modifier isPaymentSufficient(uint256 amount) {
        if (totalSupply() + amount > 15 && totalSupply() < 15){
            require(PRICE * (totalSupply() + amount - 15) <= msg.value, "There was not enough/extra ETH transferred to mint an NFT.");
        } else if (totalSupply() + amount > 15 && totalSupply() > 15){
            require(PRICE * amount <= msg.value, "There was not enough/extra ETH transferred to mint an NFT.");
        }
        _;
    }
