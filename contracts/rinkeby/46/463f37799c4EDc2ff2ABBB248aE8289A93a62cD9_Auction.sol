...
contract Auction is Ownable, Pausable {
  ...
  constructor(
    ...
    for(uint256 i = 0; i < _numberOfAuctions; i++) {
      ...
      _auctionStatus[i] = AuctionStatus(false, false);
    }
    pause();
  }
  ...
  // Refunds the losing bidders.
  // If you send too little ether, one or more of the transfers will fail and revert.
  // If you send too much ether, the function will revert at the end.
  function refundBidders(address payable[] calldata bidders) external payable onlyOwner whenPaused whenAuctionEnded {
    ...
    require(msg.value == totalRefundAmount, "The amount of ether sent doesn't match the amount to refund.");
  }
  ...
  // Handles receiving ether to the contract. Reject all direct payments to the contract. Bids must be placed using the placeBid function.
  receive() external payable {
    revert("This contract can't be paid directly.");
  }
}
