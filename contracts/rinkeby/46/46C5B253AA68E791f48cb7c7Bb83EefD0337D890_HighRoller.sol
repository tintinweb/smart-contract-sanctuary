...
contract HighRoller is ERC721Enumerable, Ownable, ReentrancyGuard{
    using SafeMath for uint256;
    using Counters for Counters.Counter;

    // constants
    uint256 constant MAX_ELEMENTS = 7777;
    uint256[] public PURCHASE_LIMIT = [8, 5];
    uint256[] public STAGE_PRICE = [0.0777 ether, 0.085 ether, 0.098 ether];
    ...
    bytes32 public friendMerkleRoot;
    bytes32 public discordMerkleRoot;
    ...
    function setPurchaseLimit(uint256 _case, uint256 _limit) public onlyOwner{
        PURCHASE_LIMIT[_case] = _limit;
    }

    function setFriendMerkleRoot(bytes32 merkleRoot) external onlyOwner {
        friendMerkleRoot = merkleRoot;
    }

    function setDiscordMerkleRoot(bytes32 merkleRoot) external onlyOwner {
        discordMerkleRoot = merkleRoot;
    }

    function ownerSale(uint256 numberOfTokens) external onlyOwner {
    ...
    function checkCategory(bytes32[] calldata merkleProof) external view returns(uint256){
        uint256 category = 0;
    ...
    function friendSale(bytes32[] calldata merkleProof, uint256 numberOfTokens) external payable isValidMerkleProof(merkleProof, friendMerkleRoot) nonReentrant {
    ...
    function discordSale(bytes32[] calldata merkleProof, uint256 numberOfTokens) external payable isValidMerkleProof(merkleProof, discordMerkleRoot) nonReentrant {
        require(!MINTING_PAUSED, "Minting is not active");
        require(CURRENT_STAGE == STAGE.PRE_SALE, "Current stage should be PRE_SALE");
        require(totalSupply() < MAX_ELEMENTS, 'All tokens have been minted');
        require(totalSupply() + numberOfTokens < MAX_ELEMENTS, 'Purchase would exceed max supply');
    ...
    function publicMint(uint256 numberOfTokens) external payable {
        ...
        require(totalSupply() < MAX_ELEMENTS, 'All tokens have been minted');
        require(totalSupply() + numberOfTokens < MAX_ELEMENTS, 'Purchase would exceed max supply');
        require(STAGE_PRICE[2] * numberOfTokens <= msg.value, 'ETH amount is not sufficient');
