...
/**
 * @notice Frst Governance Token = ERC20 + mint + burn.
 * @author blockchainerr
 */

// SPDX-License-Identifier: GPLv2
contract FRST is Context, IERC20  {
    ...
    constructor(
        ...
        string memory name_,
        uint8 decimals_,
        FrostyAccessControls accessControls_,
        address tokenOwner,
    ...
    function setCap(uint _cap, bool _freezeCap) external  {
        require(
            accessControls.hasAdminRole(_msgSender()),
            "FRST.setCap: Sender must be admin"
        );
    ...
    function mint(address tokenOwner, uint tokens) external returns (bool success) {
        require(
            accessControls.hasMinterRole(_msgSender()),
            "FRST.mint: Sender must have permission to mint"
        );
...
import "../FrostyAccessControls.sol";
import "./IFrostyMaterials.sol";
import "./IFrostyGarmentNFT.sol";

/**
 * @notice Collection contract for Frosty NFTs
 */
contract FrostyGarmentCollection is Context, ReentrancyGuard, IERC721Receiver, ERC1155Receiver {
    using SafeMath for uint256;
    using Address for address payable;

    /// @notice Event emitted only on construction. To be used by indexers
    event FrostyGarmentCollectionContractDeployed();
    ...
    IFrostyMaterials public materials;
    ...
    /**
     @param _accessControls Address of the Frosty access control contract
     @param _garmentNft Garment NFT token address
     */
    constructor(
        FrostyAccessControls _accessControls,
        IFrostyGarmentNFT _garmentNft,
        IFrostyMaterials _materials
    ) public {
        require(address(_accessControls) != address(0), "FrostyGarmentCollection: Invalid Access Controls");
        require(address(_garmentNft) != address(0), "FrostyGarmentCollection: Invalid NFT");
        require(address(_materials) != address(0), "FrostyGarmentCollection: Invalid Child ERC1155 address");
        accessControls = _accessControls;
        garmentNft = _garmentNft;
        materials = _materials;

        emit FrostyGarmentCollectionContractDeployed();
    }
    ...
    function mintCollection(
        ...
        require(
            ...
            "FrostyGarmentCollection.mintCollection: Sender must have the minter or contract role"
        );

        require(
            _amount <= maxGarmentsPerCollection,
            "FrostyGarmentCollection.mintCollection: Amount cannot exceed maxGarmentsPerCollection"
        );
    ...
    function updateMaxGarmentsPerCollection(uint256 _maxGarmentsPerCollection) external {
        require(accessControls.hasAdminRole(_msgSender()), "FrostyGarmentCollection.updateMaxGarmentsPerCollection: Sender must be admin");
     ...
     */
    function updateAccessControls(FrostyAccessControls _accessControls) external {
        require(accessControls.hasAdminRole(_msgSender()), "FrostyGarmentCollection.updateAccessControls: Sender must be admin");
        accessControls = _accessControls;
    }
...
interface IFrostyMaterials is IERC1155 {
...
import "@openzeppelin/contracts/GSN/Context.sol";
import "@openzeppelin/contracts/utils/ReentrancyGuard.sol";
import "./FrostyGarmentNFT.sol";
import "./FrostyMaterials.sol";
import "../FrostyAccessControls.sol";

/**
 * @title Frosty Garment Factory
 ...
 */
contract FrostyGarmentFactory is Context, ReentrancyGuard {
    ...
    FrostyGarmentNFT public garmentToken;

    // @notice the child ERC1155 strand tokens
    FrostyMaterials public materials;

    // @notice access controls
    FrostyAccessControls public accessControls;

    constructor(
        FrostyGarmentNFT _garmentToken,
        FrostyMaterials _materials,
        FrostyAccessControls _accessControls
    ) public {
    ...
    function createNewChild(string calldata _uri) external returns (uint256 childTokenId) {
        require(
            accessControls.hasMinterRole(_msgSender()),
            "FrostyGarmentFactory.createNewChild: Sender must be minter"
        );
    ...
    function createNewChildren(string[] calldata _uris) external returns (uint256[] memory childTokenIds) {
        require(
            accessControls.hasMinterRole(_msgSender()),
            "FrostyGarmentFactory.createNewChildren: Sender must be minter"
        );
    ...
    function createNewChildrenWithBalances(
        ...
        require(
            accessControls.hasMinterRole(_msgSender()),
            "FrostyGarmentFactory.createNewChildrenWithBalances: Sender must be minter"
        );
    ...
    function createNewChildrenWithBalanceAndGarment(
        ...
        require(
            accessControls.hasMinterRole(_msgSender()),
            "FrostyGarmentFactory.createNewChildrenWithBalanceAndGarment: Sender must be minter"
        );
    ...
    function mintParentWithChildren(
        ...
        require(
            accessControls.hasMinterRole(_msgSender()),
            "FrostyGarmentFactory.mintParentWithChildren: Sender must be minter"
        );
        // Generate parent 721 token
    ...
    function mintParentWithoutChildren(
        ...
        require(
            accessControls.hasMinterRole(_msgSender()),
            "FrostyGarmentFactory.mintParentWithoutChildren: Sender must be minter"
        );
...
/**
 * @title Frosty Garment NFT a.k.a. parent NFTs
 * @dev Issues ERC-721 tokens as well as being able to hold child 1155 tokens
 */
contract FrostyGarmentNFT is ERC721("FrostyNFT", "FSTA"), ERC1155Receiver, IERC998ERC1155TopDown {

    // @notice event emitted upon construction of this contract, used to bootstrap external indexers
    event FrostyGarmentNFTContractDeployed();

    // @notice event emitted when token URI is updated
    event FrostyGarmentTokenUriUpdate(
    ...
    /**
     @param _accessControls Address of the Frosty access control contract
     @param _childContract ERC1155 the Frosty child NFT contract
     */
    constructor(FrostyAccessControls _accessControls, ERC1155 _childContract) public {
        accessControls = _accessControls;
        childContract = _childContract;
        emit FrostyGarmentNFTContractDeployed();
    }

    /**
     @notice Mints a FrostyGarmentNFT AND when minting to a contract checks if the beneficiary is a 721 compatible
    ...
    function mint(address _beneficiary, string calldata _tokenUri, address _designer) external returns (uint256) {
        require(
            ...
            "FrostyGarmentNFT.mint: Sender must have the minter or contract role"
        );
    ...
    /**
     @notice Burns a FrostyGarmentNFT, releasing any composed 1155 tokens held by the token itself
    ...
    function burn(uint256 _tokenId) external {
        ...
        require(
            ...
            "FrostyGarmentNFT.burn: Only garment owner or approved"
        );
    ...
    function _validateReceiverParams(uint256 _receiverTokenId, address _operator, address _from) internal view {
        ...
        // We only accept children from the Frosty child contract
    ...
    function setTokenURI(uint256 _tokenId, string calldata _tokenUri) external {
        require(
            ...
            "FrostyGarmentNFT.setTokenURI: Sender must be an authorised contract or admin"
        );
        _setTokenURI(_tokenId, _tokenUri);
        emit FrostyGarmentTokenUriUpdate(_tokenId, _tokenUri);
    }
    ...
    function setPrimarySalePrice(uint256 _tokenId, uint256 _salePrice) external {
        require(
            accessControls.hasSmartContractRole(_msgSender()) || accessControls.hasAdminRole(_msgSender()),
            "FrostyGarmentNFT.setPrimarySalePrice: Sender must be an authorised contract or admin"
        );
        require(_exists(_tokenId), "FrostyGarmentNFT.setPrimarySalePrice: Token does not exist");
        require(_salePrice > 0, "FrostyGarmentNFT.setPrimarySalePrice: Invalid sale price");
     ...
     */
    function updateAccessControls(FrostyAccessControls _accessControls) external {
        require(accessControls.hasAdminRole(_msgSender()), "FrostyGarmentNFT.updateAccessControls: Sender must be admin");
        accessControls = _accessControls;
    }
    ...
    function updateMaxChildrenPerToken(uint256 _maxChildrenPerToken) external {
        require(accessControls.hasAdminRole(_msgSender()), "FrostyGarmentNFT.updateMaxChildrenPerToken: Sender must be admin");
    ...
    function _assertMintingParamsValid(string calldata _tokenUri, address _designer) pure internal {
        require(bytes(_tokenUri).length > 0, "FrostyGarmentNFT._assertMintingParamsValid: Token URI is empty");
        require(_designer != address(0), "FrostyGarmentNFT._assertMintingParamsValid: Designer is zero address");
    }
}
...
contract FrostyMaterials is ERC1155Burnable {
    ...
    function createChild(string calldata _uri) external returns (uint256 id) {
        require(
            ...
            "FrostyMaterials.createChild: Sender must be smart contract"
        );

        require(bytes(_uri).length > 0, "FrostyMaterials.createChild: URI is a blank string");
    ...
    function batchCreateChildren(string[] calldata _uris) external returns (uint256[] memory tokenIds) {
        require(
            accessControls.hasSmartContractRole(_msgSender()),
            "FrostyMaterials.batchCreateChildren: Sender must be smart contract"
        );

        require(_uris.length > 0, "FrostyMaterials.batchCreateChildren: No data supplied in array");

        uint256 urisLength = _uris.length;
        tokenIds = new uint256[](urisLength);
        for (uint256 i = 0; i < urisLength; i++) {
            string memory uri = _uris[i];
            require(bytes(uri).length > 0, "FrostyMaterials.batchCreateChildren: URI is a blank string");
            tokenIdPointer = tokenIdPointer.add(1);
    ...
    function mintChild(uint256 _childTokenId, uint256 _amount, address _beneficiary, bytes calldata _data) external {
        require(
            accessControls.hasSmartContractRole(_msgSender()),
            "FrostyMaterials.mintChild: Sender must be smart contract"
        );

        require(bytes(tokenUris[_childTokenId]).length > 0, "FrostyMaterials.mintChild: Strand does not exist");
        require(_amount > 0, "FrostyMaterials.mintChild: No amount specified");
    ...
    function batchMintChildren(
        ...
        require(
            accessControls.hasSmartContractRole(_msgSender()),
            "FrostyMaterials.batchMintChildren: Sender must be smart contract"
        );

        require(_childTokenIds.length == _amounts.length, "FrostyMaterials.batchMintChildren: Array lengths are invalid");
        require(_childTokenIds.length > 0, "FrostyMaterials.batchMintChildren: No data supplied in arrays");

        // Check the strands exist and no zero amounts
        for (uint256 i = 0; i < _childTokenIds.length; i++) {
            uint256 strandId = _childTokenIds[i];
            require(bytes(tokenUris[strandId]).length > 0, "FrostyMaterials.batchMintChildren: Strand does not exist");

            uint256 amount = _amounts[i];
            require(amount > 0, "FrostyMaterials.batchMintChildren: Invalid amount");
        }
    ...
    function updateAccessControls(FrostyAccessControls _accessControls) external {
        require(
            accessControls.hasAdminRole(_msgSender()),
            "FrostyMaterials.updateAccessControls: Sender must be admin"
        );

        require(
            address(_accessControls) != address(0),
            "FrostyMaterials.updateAccessControls: New access controls cannot be ZERO address"
        );
...
contract UniswapPairOracle_FRST_WETH is UniswapOracleExample {
...
contract UniswapV2ERC20 {
    ...
    string public constant name = 'WswapSwap LP Token';
