...
contract RainiLpv3StakingPool is AccessControl, ReentrancyGuard, ERC721Holder {
  ...
  uint256 constant public RAINI_REWARD_PER_TOKEN_DECIMALS = 1000000000;
  ...
  mapping(address => uint32[8]) internal stakedNFTs;
  ...
  constructor(address _rainiLpNFT, address _rainiToken, address _token1, address _v2Pool) {
  ...
  function getCurrentBonus(address _owner) 
    public view returns(uint256) {
      ...
      uint256 duration = block.timestamp.sub(_accountRewardVars.lastUpdated);
  ...
  function setBonus(uint256 _maxBonus, uint256 _bonusDuration)
    external onlyOwner {
      maxBonus = _maxBonus.mul(BONUS_DECIMALS);
  ...
  function stake(uint32 _tokenId)
    external nonReentrant {
      ...
      require(token0 == token0Address, "RainiLpv3StakingPool: token0 != token0Address");
      require(token1 == token1Address, "RainiLpv3StakingPool: token1 != token1Address");      
      ...
      uint32[8] memory nfts = stakedNFTs[_msgSender()];
      bool wasAdded = false;
      ...
      require(wasAdded, "Max NFTs staked");

      totalSupply = totalSupply.add(liquidity);
      uint128 currentStake = _accountVars.staked;
      _accountVars.staked = currentStake + liquidity;
      _accountRewardVars.lastBonus = uint40(_accountRewardVars.lastBonus * currentStake / (currentStake + liquidity));
  ...
  function withdraw(uint256 _tokenId)
    external nonReentrant balanceUpdate(_msgSender()) {
      ...
      _accountVars.staked = _accountVars.staked - liquidity;
  ...
  function buyUnicorn(uint256 _amount) 
    external payable {
      ...
      require(msg.value.mul(unicornToEth) >= _amount, "RainiLpv3StakingPool: not enougth eth");
  ...
  function rainiRewardPerToken() public view returns (uint256) {
    ...
    return ((_generalRewardVars.rainiRewardPerTokenStored + lastTimeRewardApplicable() - _generalRewardVars.lastUpdateTime) * _generalRewardVars.rainiRewardRate * RAINI_REWARD_DECIMALS * RAINI_REWARD_PER_TOKEN_DECIMALS) / totalSupply;
  }
  ...
  function rainiEarned(address account) public view returns (uint256) {
    ...
    uint256 calculatedEarned = (_accountVars.staked * (rainiRewardPerToken() - _accountRewardVars.rainiRewardPerTokenPaid)) / (RAINI_REWARD_PER_TOKEN_DECIMALS * RAINI_REWARD_DECIMALS) + _accountRewardVars.rainiRewards;
  ...
  function addRainiRewardPool(uint256 _amount, uint256 _duration)
    external onlyOwner nonReentrant balanceUpdate(address(0)) {
      ...
      require(_generalRewardVars.periodFinish < block.timestamp, "Cannot create new pool while old pool active");

      rainiToken.safeTransferFrom(_msgSender(), address(this), _amount);
      _generalRewardVars.rainiRewardRate = uint64(_amount / _duration);
      _generalRewardVars.periodFinish = uint32(block.timestamp + _duration);
      _generalRewardVars.lastUpdateTime = uint32(block.timestamp);
      generalRewardVars = _generalRewardVars;
  }

  // function removeReward() external onlyOwner nonReentrant {
  //   if (periodFinish > block.timestamp) {
  //     periodFinish = block.timestamp;
  //   }
  //   rainiToken.transfer(_msgSender(), rainiToken.balanceOf(address(this)));
  // }
