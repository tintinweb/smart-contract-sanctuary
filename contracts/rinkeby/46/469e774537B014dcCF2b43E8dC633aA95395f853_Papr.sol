...
contract Papr is ERC20Burnable, DoubleOperator {
    ...
    // mapping(address => uint256) public excludedUnirouter;
    ...
    constructor() public ERC20('PAPR', 'PAPR') {
        _mint(msg.sender, 35000 * 10**18);
        // Set the array of tax rate according to the price
        // Dont't forget to change Index 0 to 30 after injecting liquidity
    ...
    function addExcludedAddresses(address newAddress) external onlyDoubleOperator returns(bool) {
    ...
    function removeExcludedAddresses(address oldAddress) external onlyDoubleOperator returns(bool) {
    ...
    function setfeesTaxRate(uint256 index, uint256 fees) external onlyDoubleOperator {
        require(fees <= 45);
    ...
    function setpaprOracleAddress(address newOracle) external onlyDoubleOperator {
        paprOracle = newOracle;
    }
    // Reference the PAPR address to itself as an argument for the Oracle
    function setpaprAddress(address newPaprAddress) external onlyDoubleOperator {
        papr = newPaprAddress;
    }

    function setInjectorAddress(address newInjector) external onlyDoubleOperator {
        injector = newInjector;
    }

    function setUnirouter(address newUnirouter) external onlyDoubleOperator {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        uint256 paprPrice = getPaprPrice();
        bool excludedAddresses = isExcludedAddresses(sender);

        if (excludedAddresses == true) {
    ...
    function _transferWithFees(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        uint256 feesAmountHalf = feesAmount.div(2);

        burnFrom(sender, feesAmountHalf);
        _transfer(sender, injector, feesAmountHalf);
...
contract DoubleOperator is Context, DoubleOwnable {
    ...
    function setSecondaryOperator(address newOperator_) external onlyPrimaryOwner {
        ...
        require(
            _secondaryOperator == address(0),
            'operator: secondary operator is already assigned'
        );
