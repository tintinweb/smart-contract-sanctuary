...
contract animalMain is  ERC721,Ownable {
  ...
  event Create(uint _tokenId, bytes32 dna,uint8 star, uint16 roletype, uint16 attr);
  ...
  function seeAnimalAttr(uint256 _tokenId) public view returns (uint16 attr) {
    return animals[_tokenId].attr;
  }
  ...
  function createAnimal() public payable {
      ...
      uint id = animals.push(animal(dna, uint8(star), uint8(roletype), uint8(attr))) - 1;
