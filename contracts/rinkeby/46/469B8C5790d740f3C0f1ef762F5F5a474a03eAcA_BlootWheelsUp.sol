...
contract BlootWheelsUp is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    mapping(uint256 => bytes32) private _randomHashes;
    ...
    function getWave(uint256 tokenId) public view returns (string memory) {
        ...
        string[4] memory fill;

        for (uint j = 0; j < 4; j++)
        {
            fill[j] = string(abi.encodePacked(getColor(tokenId * 10 * j), 'b2'));
        }
        // string memory fill1 = rand == 0 ? 'url(#wave-pattern)' : string(abi.encodePacked(getColor(tokenId), 'b2'));
        // string memory fill2 = rand == 1 ? 'url(#wave-pattern)' : string(abi.encodePacked(getColor(tokenId * 10), '80'));
        // string memory fill3 = rand == 2 ? 'url(#wave-pattern)' : string(abi.encodePacked(getColor(tokenId * 20), '4c'));
        // string memory fill4 = rand == 3 ? 'url(#wave-pattern)' : string(abi.encodePacked(getColor(tokenId * 30), ',66'));
        ...
        wave = string(abi.encodePacked(wave, '</defs><g class="parallax"><defs><pattern id="wave-pattern" x="0" y="0" patternUnits="userSpaceOnUse" width="5%" height="5%" viewBox="0 0 10 10"><rect width="100%" height="100%" fill="', fill[rand],'"></rect>', pattern, '</pattern></defs>'));
        fill[rand] = 'url(#wave-pattern)';
        wave = string(abi.encodePacked(wave, '<use xlink:href="#gentle-wave1" x="48" y="1" fill="', fill[0], '"/><use xlink:href="#gentle-wave2" x="48" y="3" fill="', fill[1], '" /><use xlink:href="#gentle-wave3" x="48" y="5" fill="', fill[2], '" /><use xlink:href="#gentle-wave4" x="48" y="7" fill="', fill[3], '" /></g></svg>'));
        return wave;
    }
    ...
    function getDurationFromTime() public view returns (string[4] memory) {
        ...
        for(uint i = 0; i < 4; i++)
        {
            ...
            durations[i] = string(abi.encodePacked(toString(3 + i), '.', toString((curTime / 100) % 10)));
        }
        return durations;
    }
    ...
    function pluck(
        ...
        uint256 rand = random(string(abi.encodePacked(keyPrefix, _randomHashes[tokenId], toString(tokenId))));
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_randomHashes[tokenId] != bytes32(0), "not minted");
        string[18] memory parts;
        string[4] memory durations = getDurationFromTime();
        string memory leftRight = getDirection(tokenId);
        string memory shimmer = '<defs><linearGradient id="shimmer" x1="0%" y1="0%" x2="100%" y2="0%"><stop stop-color="gold"></stop><stop stop-color="yellow"><animate attributeName="offset" values=".05;.95;-0.4" dur="4s" begin="0" repeatCount="indefinite"/></stop><stop stop-color="gold"><animate attributeName="offset" values="0;2;0" dur="4s" begin="0" repeatCount="indefinite"/></stop></linearGradient></defs>';
