...
contract NFTMining is ERC1155Holder, Ownable {
    ...
    function harvest(uint256 minerId) external {
        ...
        uint256 reward = (block.timestamp - pool[minerId].start) *
            pool[minerId].rate;
    ...
    function _setRate(uint256 tokenId, uint256 rate) internal {
        idsToRates[tokenId] = rate;
        avaliableIdsCounter += 1;
    }

    function getAllUserMiners(address user) public view onlyOwner {
        _getAllUserMiners(user);
    }

    function _getAllUserMiners(address user)
        internal
        view
        returns (
        ...
        )
    {
        ...
        for (uint256 miner = 0; miner < minersCounter; miner++) {
            ...
            harvestedLastTime[miner] = pool[users[msg.sender].minersIds[miner]]
                .lastHarvest;
