...
contract Folk006 is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    string[] private motivation = [
        ...
        "raise a family",
        "reconcile",
        "retreive a stolen item",
        "rob you",
        "sail around the world",
        "save someone",
        "seduce someone",
        "sel you a treasure map",
...
//                output = string(abi.encodePacked('"', name[0], ' ', name[1], '" ', output, " +1"));
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="black" /><text x="20" y="30" class="base">The ';

        parts[1] = getAdj1(tokenId);

        parts[2] = ' ';

        parts[3] = getJob1(tokenId);

        parts[4] = ' wants to ';

        parts[5] = getMot1(tokenId);

        parts[6] = '</text><text x="20" y="60" class="base">The ';

        parts[7] = getAdj2(tokenId);

        parts[8] = ' ';

        parts[9] = getJob2(tokenId);

        parts[10] = ' wants to ';

        parts[11] = getMot2(tokenId);

        parts[12] = '</text><text x="20" y="90" class="base">The ';

        parts[13] = getAdj3(tokenId);

        parts[14] = ' ';

        parts[15] = getJob3(tokenId);

        parts[16] = ' wants to ';

        parts[17] = getMot3(tokenId);

        parts[18] = '</text><text x="20" y="120" class="base">The ';

        parts[19] = getAdj4(tokenId);

        parts[20] = ' ';

        parts[21] = getJob4(tokenId);

        parts[22] = ' wants to ';

        parts[23] = getMot4(tokenId);

        parts[24] = '</text><text x="20" y="150" class="base">The ';

        parts[25] = getAdj5(tokenId);

        parts[26] = ' ';

        parts[27] = getJob5(tokenId);

        parts[28] = ' wants to ';

        parts[29] = getMot5(tokenId);
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Folk006:', toString(tokenId), '", "description": "Folk generates random people you might meet on an adventure", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    constructor() ERC721("Folk006", "FOLK006") Ownable() {}
}
