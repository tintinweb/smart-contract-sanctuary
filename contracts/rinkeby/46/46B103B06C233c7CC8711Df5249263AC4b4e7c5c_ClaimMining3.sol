...
contract ClaimMining3 is OwnableContract, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor(address _mining_token, uint256 _mining_start_block) public {
        ...
        uint256 phase1EndBlockNumber =  phase1StartBlockNumber.add(blockCountPerDay.mul(28));
        phaseEndBlockNumberArray.push(phase1EndBlockNumber); 

        uint256 phase1PerBlockReword = 61 * 1e16; // 0.61 Claim per block
        phasePerBlockRewordArray.push(phase1PerBlockReword);
    }
    ...
    function updateBlockReword(uint256 _phaseIndex, uint256 _phasePerBlockReword) public onlyOwner {
        require(_phaseIndex < phasePerBlockRewordArray.length, "invalid _phaseIndex");
        phasePerBlockRewordArray[_phaseIndex] = _phasePerBlockReword;
    }
