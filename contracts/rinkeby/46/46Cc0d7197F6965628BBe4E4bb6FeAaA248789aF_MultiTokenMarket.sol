...
import { IERC1155, IERC165 } from "@openzeppelin/contracts/token/ERC1155/IERC1155.sol";
import { ERC1155Receiver } from "@openzeppelin/contracts/token/ERC1155/utils/ERC1155Receiver.sol";
import { SafeERC20 } from "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import { IMarketMaker, IMtMarketMaker } from "./IMarketMaker.sol";
import { TokenMarket } from "./TokenMarket.sol";

/**
 * @title An open auction house, enabling collectors and curators to run their own auctions
 */
contract MultiTokenMarket is ERC1155Receiver, TokenMarket, IMtMarketMaker {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    bytes4 constant private ERC1155_INTERFACE_ID = 0xd9b67a26; // 1155 interface id

    constructor(address currency, uint16 fee) {
      feeExemptCurrencyAddress = currency;
      ownerFeePercentage = fee;
      emit MarketFeeSet(address(this), feeExemptCurrencyAddress, fee);
    }
    ...
    function createMarket(address tokenContract, uint256 tokenId, uint256 tokenCount, Stakeholder[] calldata stakeholders)
      ...
      nonReentrant
      returns (uint256) {
        require(IERC165(tokenContract).supportsInterface(ERC1155_INTERFACE_ID), "tokenContract does not support ERC1155 interface");
        require(IERC1155(tokenContract).isApprovedForAll(msg.sender, address(this)), "This contract must be approved for token id");
        require(IERC1155(tokenContract).balanceOf(msg.sender, tokenId) >= tokenCount, "Caller must own sufficient token");
        return _createMarket(tokenContract, tokenId, tokenCount, msg.sender, stakeholders);
    }

    function createBid(uint256 marketId, address currency, uint256 amount, uint256 tokenCount)
      external
      override
      marketExists(marketId)
      nonReentrant {
          require(markets[marketId].tokenCount >= tokenCount, "Bid must not exceed tokens in market");
          _validateIncomingBid(amount, currency);

          // If there is an ask, and if the bid matches the ask, then finalize the auction
          if (markets[marketId].ask.amount > 0  && markets[marketId].ask.currency != address(0) && currency == markets[marketId].ask.currency && amount >= markets[marketId].ask.amount * tokenCount) {
            _finalizeTransfer(marketId, currency, msg.sender, amount, tokenCount);
            return;
          }

          Bid memory bid = Bid({ amount: amount, currency: currency, bidder: msg.sender, tokenCount: tokenCount });
    ...
    function transferToken(address tokenContract, address owner, address buyer, uint256 tokenId, uint256 tokenCount)
      internal
      override
      returns(bool) {
        try IERC1155(tokenContract).safeTransferFrom(owner, buyer, tokenId, tokenCount, "") {
          return true;
        } catch {
          return false;
        }
...
interface IMarketMaker {
    ...
    function setFee(address currency, uint16 feePercentage)
      external;
    ...
    event MarketStakeholderAdded(uint256 indexed marketId, address tokenContract, uint256 tokenId, address indexed stakeholder, uint16 indexed stakePercentage);
    event MarketFeeSet(address indexed marketContract, address indexed currencyAddress, uint16 indexed feePercentage);
}
 ...
 * @title An open auction house, enabling collectors and curators to run their own auctions
 */
abstract contract TokenMarket is IMarketMaker, ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using Counters for Counters.Counter;

    mapping(uint256 => IMarketMaker.Market) public markets;
    mapping(bytes32 => bool) public marketExistence;

    Counters.Counter private _marketIdTracker;
    uint16 public ownerFeePercentage;
    address public feeExemptCurrencyAddress;
    ...
    modifier isMarketOwner(uint256 marketId, address sender) {
    ...
    function setFee(address currency, uint16 feePercentage)
      ...
      feeExemptCurrencyAddress = currency;
      ownerFeePercentage = feePercentage;
      emit MarketFeeSet(address(this), feeExemptCurrencyAddress, ownerFeePercentage);
    }

    function _createMarket(address tokenContract, uint256 tokenId, uint256 tokenCount, address tokenOwner, Stakeholder[] calldata stakeholders)
      internal
      returns (uint256) {
        bytes32 hashed = keccak256(abi.encodePacked(tokenContract, tokenId, tokenOwner));
        require(marketExistence[hashed] == false, "Market for token already exists");
        uint256 marketId = _marketIdTracker.current();
        markets[marketId].existenceHash = hashed;
        ...
        markets[marketId].tokenCount = tokenCount;
        _marketIdTracker.increment();
        marketExistence[hashed] = true;

        emit MarketCreated(marketId, tokenContract, tokenId, tokenCount, tokenOwner);

        uint16 totalPct = ownerFeePercentage;
        for (uint i = 0; i < stakeholders.length; i++) {
          markets[marketId].stakeholders.push(stakeholders[i]);
          totalPct = SafeCast.toUint16(SafeMath.add(uint256(totalPct), uint256(stakeholders[i].stakePercentage)));
          emit MarketStakeholderAdded(marketId, tokenContract, tokenId, stakeholders[i].stakeholder, stakeholders[i].stakePercentage);
        }
        require(totalPct < 10000, "Invalid stake percentages");
    ...
    function removeMarket(uint256 marketId)
      ...
      override
      marketExists(marketId)
      isMarketOwner(marketId, msg.sender)
      nonReentrant {
    ...
    function removeBid(uint256 marketId, uint256 bidIndex)
      ...
      bidExists(marketId, bidIndex)
      {
        ...
        emit BidRemoved(marketId, markets[marketId].tokenContract, markets[marketId].tokenId, bidIndex, bid.tokenCount, bid.currency, bid.amount, bid.bidder);        // future code golf check -- check Results. Commented code takes 100 more gas
    ...
    function _removeMarket(uint256 marketId)
      internal
      {
        ...
        delete marketExistence[markets[marketId].existenceHash];
    ...
    function acceptBid(uint256 marketId, uint256 bidIndex)
      ...
      nonReentrant {
        ...
        Bid memory bid = markets[marketId].bids[bidIndex];
        _finalizeTransfer(marketId, bid.currency, bid.bidder, bid.amount, bid.tokenCount);
      }

    function _finalizeTransfer(uint256 marketId, address currency, address buyer, uint256 amount, uint256 tokenCount)
      internal
      {
        require(markets[marketId].tokenCount >= tokenCount, "Bid must not exceed tokens in market");
        ...
        markets[marketId].tokenCount = SafeMath.sub(uint256(markets[marketId].tokenCount), uint256(tokenCount));
        bool transferSuccess = transferToken(market.tokenContract, market.tokenOwner, buyer, market.tokenId, tokenCount);
        if (!transferSuccess) {
            _removeMarket(marketId);
            return;
        }
        ...
        if (currency != feeExemptCurrencyAddress) {
          uint256 cut = SafeMath.div(SafeMath.mul(amount, ownerFeePercentage), 10000);
          currencyToken.transfer(owner(), cut);
          tokenOwnerProfit = SafeMath.sub(tokenOwnerProfit, cut);
        }

        for (uint i = 0; i < market.stakeholders.length; i++) {
          uint256 cut = SafeMath.div(SafeMath.mul(amount, market.stakeholders[i].stakePercentage), 10000);
          currencyToken.transfer(market.stakeholders[i].stakeholder, cut);
          tokenOwnerProfit = SafeMath.sub(tokenOwnerProfit, cut);
        }

        currencyToken.transfer(market.tokenOwner, tokenOwnerProfit);
        emit TransferFinalized(marketId, market.tokenContract, market.tokenId, tokenCount, currency, amount, buyer);
        if (markets[marketId].tokenCount == 0) {
          _removeMarket(marketId);
        }
      }
    ...
    function _validateIncomingBid(uint256 amount, address currency) internal view {
        IERC20 currencyToken = IERC20(currency);
        require(currencyToken.balanceOf(msg.sender) >= amount && amount > 0, "Caller's token balance is insufficient for bid");
        require(currencyToken.allowance(msg.sender, address(this)) >= amount, "Caller must approve market for currency");
    }

    function _validateCurrency(address currency) internal view {
      IERC20 currencyToken = IERC20(currency);
      require(currencyToken.balanceOf(msg.sender) >=0, "Currency must be a valid ERC-20 contract");
      require(currencyToken.totalSupply() >= 0, "Currency must be a valid ERC-20 contract");
    }
    ...
    function transferToken(address tokenContract, address owner, address buyer, uint256 tokenId, uint256 tokenCount) internal virtual returns(bool);
}
