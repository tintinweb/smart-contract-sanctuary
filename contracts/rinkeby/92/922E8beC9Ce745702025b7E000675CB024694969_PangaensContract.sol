...
// File: PangaensContract.sol

/**
 * @dev Contract to mint the Pangaens NFT collection.
 *
 * The contract allows user to mint during both presale and the main sale
 *
 */
contract PangaensContract is ERC721Enumerable, Ownable, ReentrancyGuard {
    using Strings for uint256;
    using ECDSA for bytes32;

    uint256 public cost = 0.075 ether;
    uint256 public maxSupply = 8900;
    uint256 public devSupply = 325;
    ...
    mapping (address => bool) private presaleClaimed; // Presale mint can only claim once.
    address private preSaleSigner = 0x93ED7537De1f640684c7895926489dD4f659475A;
    ...
    function mintSale(
        ...
        //require(!isPresaleActive, "Presale in progress");
        require(isSaleActive, "Sale is not active");
        require(_mintAmount > 0, "Must mint at least 1 NFT");
        require(_mintAmount <= maxMintAmount, "Mint count exceeds limit for sale");
        require((cost * _mintAmount) <= msg.value, "ETH sent does not match required payment");
    ...
    function mintPresale(
        ...
        require(!isSaleActive, "Cannot mint while main sale is active");
        require(_mintAmount > 0, "Must mint at least 1 NFT");
    ...
    function _mintToUser(address recipient, uint256 _mintAmount, uint256 maxMintSupply) private {
        ...
        require(totalSupply() + _mintAmount <= maxMintSupply, "Cannot mint more than available");
    ...
    external onlyOwner {
        ...
        // Mint
        _mintToUser(msg.sender, _mintAmount, devSupply);
    }
  ...
  function setMaxMintAmount(uint256 _newmaxMintAmount) public onlyOwner() {
   ...
   function setPresaleMaxMint(uint256 _newmaxMintAmount) public onlyOwner() {
    ...
    function getPresaleClaimed(address _user) external view returns (bool) {
     ...
     // Allows to enable/disable minting of presale
