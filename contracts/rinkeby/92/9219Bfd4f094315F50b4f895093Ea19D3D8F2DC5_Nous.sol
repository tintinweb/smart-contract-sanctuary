...
contract Nous is ERC721, ERC721Enumerable, ERC721Burnable, PaymentSplitter, Ownable, VRFConsumerBase {
  ...
  uint256 private LINK_FEE = 0.1 * 10 ** 18; // 0.1 LINK;
  bytes32 private LINK_KEY_HASH = 0x2ed0feb3e7fd2022120aa84fab1945545a9f2ffc9076fd6156fa96eaff4c1311;
  ...
  function tokenOffset() public view returns (uint256) {
    require(_saleActive, "Offset has not been generated");
  ...
  function initializeSale() public onlyOwner {
    require(!_saleActive, "Sale is already initialized");
  ...
  function _mintAmount(uint256 amount, address to) internal {
    require(Counters.current(_tokenIds) + amount < MAX_SUPPLY, "Exceeds maximum number of tokens");

    for (uint256 i = 0; i < amount; i++) {
      _safeMint(to, Counters.current(_tokenIds));
