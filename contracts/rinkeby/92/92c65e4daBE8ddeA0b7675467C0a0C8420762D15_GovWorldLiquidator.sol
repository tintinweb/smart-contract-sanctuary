...
contract GovWorldLiquidator is GovLiquidatorBase {
    ...
    mapping(address => mapping(address => uint)) private liquidatedSUNTokenbalances;

    event WithdrawNetworkCoin(address walletAddress, uint256 withdrawAmount);

    event WithdrawToken(address tokenAddress, address walletAddress,uint256 withdrawAmount);
    ...
    function liquidateLoan(uint256 _loanId) public override onlyTokenMarket {
        ...
        if(lenderDetails.autoSell  == true) {
             ...
             for(uint256 i  = 0 ; i  <  loanDetails.stakedCollateralTokens.length ; i++){
                ...
                IUniswapSwapInterface swapInterface = IUniswapSwapInterface(IGovPriceConsumer(govPriceConsumer).getSwapInterface(loanDetails.stakedCollateralTokens[i]));
            ...
            uint256 thresholdFeeinStable = ((loanDetails.loanAmountInBorrowed.mul(govProtocolRegistry.getThresholdPercentage())).div(10000));
            ...
            for(uint256 i  = 0 ; i  <  loanDetails.stakedCollateralTokens.length ; i++){
                ...
                if(govClaimToken.isClaimToken(claimToken)) {
                    ...
                    liquidatedSUNTokenbalances[lenderDetails.lender][loanDetails.stakedCollateralTokens[i]] += loanDetails.stakedCollateralAmounts[i];
                } else {
                    priceofCollateral = govPriceConsumer.getAltCoinPriceinStable(loanDetails.borrowStableCoin, loanDetails.stakedCollateralTokens[i], loanDetails.stakedCollateralAmounts[i]);
                ...
                if (collateralAmountinStable <= loanDetails.loanAmountInBorrowed) {
                    ...
                    uint256 exceedAltcoinValue = govPriceConsumer.getAltCoinPriceinStable(loanDetails.stakedCollateralTokens[i], loanDetails.borrowStableCoin, collateralAmountinStable.sub(loanDetails.loanAmountInBorrowed));
                    uint256 collateralToLender = loanDetails.stakedCollateralAmounts[i].sub(exceedAltcoinValue);
                    IERC20(loanDetails.stakedCollateralTokens[i]).transfer(lenderDetails.lender, collateralToLender);
                    break;
                    }
                }
            }
    ...
    function getLtv(uint256 _loanId)
        ...
        return govPriceConsumer.calculateLTV(stakedCollateralAmounts, stakedCollateralTokens, borrowedToken, loanDetails.loanAmountInBorrowed.sub(loanDetails.paybackAmount));
    }
    ...
    function getTotalPaybackAmount(uint256 _loanId) external returns (uint256, uint256, uint256) {

        TokenLoanData.LoanDetails memory loanDetails = govTokenMarket.getLoanOffersToken(_loanId);
        
        uint256 loanTermLengthPassedInDays = (block.timestamp.sub(govTokenMarket.getActivatedLoanDetails(_loanId).activationLoanTimeStamp)).div(86400);

        // require(loanTermLengthPassedInDays <= loanDetails.termsLengthInDays, "GLM: Loan already paybacked or liquidated."); //TODO uncomment before mainnet deployment
       
        uint256 apyFeeOriginal = govTokenMarket.getAPYFee(loanDetails);
        uint256 earnedAPYFee = ((loanDetails.loanAmountInBorrowed.mul(loanDetails.apyOffer).div(10000)).div(365)).mul(loanTermLengthPassedInDays);
        ...
        (loanDetails.loanAmountInBorrowed.add(earnedAPYFee), earnedAPYFee, unEarnedAPYFee);  
    }
    ...
    function fullLoanPaybackEarly(uint256 _loanId) internal {
        ...
        (uint256 finalPaybackAmounttoLender,uint256 earnedAPYFee,) = this.getTotalPaybackAmount(_loanId);
        uint256 stableCoinFee = govTokenMarket.getStableCoinAPYFeeinContract(loanDetails.borrowStableCoin);
        stableCoinFee = govTokenMarket.getStableCoinAPYFeeinContract(loanDetails.borrowStableCoin).sub(earnedAPYFee);
        
        //first transferring the payback amount from borrower to the Gov Token Market
        IERC20(loanDetails.borrowStableCoin).transferFrom(loanDetails.borrower, address(this), (loanDetails.loanAmountInBorrowed.sub(loanDetails.paybackAmount)));
        ...
        for(uint256 i  = 0 ; i  <  loanDetails.stakedCollateralTokens.length ; i++){
           ...
           if(market.isSP && loanDetails.isMintSp[i]) {
                gtoken.burnFrom(loanDetails.borrower, loanDetails.stakedCollateralAmounts[i]);
            }
        }

        loanDetails.paybackAmount = finalPaybackAmounttoLender;
        loanDetails.loanStatus = TokenLoanData.LoanStatus.CLOSED;
        // emit FullTokensLoanPaybacked(_loanId, msg.sender, lenderDetails.lender, loanDetails.loanAmountInBorrowed.sub(loanDetails.paybackAmount),   TokenLoanData.LoanStatus.CLOSED, earnedAPYFee);
    }
     ...
     */
    function payback(uint256 _loanId, uint256 _paybackAmount) public override onlyTokenMarket {
    
        TokenLoanData.LoanDetails memory loanDetails = govTokenMarket.getLoanOffersToken(_loanId);
        TokenLoanData.LenderDetails memory lenderDetails = govTokenMarket.getActivatedLoanDetails(_loanId);

        require(loanDetails.loanType == TokenLoanData.LoanType.SINGLE_TOKEN 
        ...
        require(loanDetails.loanStatus == TokenLoanData.LoanStatus.ACTIVE, "GLM, not active");
        require(_paybackAmount > 0  && _paybackAmount <= loanDetails.loanAmountInBorrowed, "GLM: Invalid Loan Amount");
        ...
        {   
            ...
            uint256 newLtv  = IGovPriceConsumer(govPriceConsumer).calculateLTV(loanDetails.stakedCollateralAmounts, loanDetails.stakedCollateralTokens, loanDetails.borrowStableCoin, remainingLoanAmount);
            ...
            loanDetails.paybackAmount = loanDetails.paybackAmount.add(_paybackAmount);
            loanDetails.loanStatus = TokenLoanData.LoanStatus.ACTIVE;
            // emit PartialTokensLoanPaybacked(loanId,  msg.sender, lenderDetails.lender, _paybackAmount);
        }     
    }

    function getLenderSUNTokenBalances(address _lender, address _sunToken) public view returns(uint256) {
