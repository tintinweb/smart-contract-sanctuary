...
interface StandaloneNft721 is IERC721 {
...
contract Raini721Functions is AccessControl, ReentrancyGuard {
  ...
  constructor(address _nftContractAddress, uint256 _maxMintsPerTx, address payable _feeRecipient, address _contractOwner, address _verifier) {
    ...
    nftContract = StandaloneNft721(_nftContractAddress);
  ...
  function setNftContract(address _nftContract)
    external onlyOwner {
      nftContract = StandaloneNft721(_nftContract);
  }
  ...
  function mint(
    ...
    (_locals.success, ) = feeRecipient.call{ value: msg.value - _locals.amountEthToWithdraw }(""); // pay fees
