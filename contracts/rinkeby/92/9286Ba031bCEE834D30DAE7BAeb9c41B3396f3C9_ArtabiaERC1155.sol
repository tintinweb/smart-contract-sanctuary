...
contract ArtabiaERC1155 is GloballyApprovedOperatorsERC1155, IERC2981 {
  ...
  function mint(uint256 _tokenId, uint256 _tokenQty, uint8 _royaltyPercentage) public onlyNonContracts {
    require(_royaltyPercentage <= 30, "royalty percentage too big");
  ...
  function mintAndCreateOrder(uint256 _tokenId, uint256 _tokenQty, uint8 _royaltyPercentage, uint256 _price) external onlyNonContracts  returns (bytes32) {
    mint(_tokenId, _tokenQty, _royaltyPercentage);

    bytes32 id = orderMarketplace.createOrder(address(this), _tokenId, _tokenQty, _price);
  ...
  function mintAndCreateListing(uint256 _tokenId, uint256 _tokenQty, uint8 _royaltyPercentage) external onlyNonContracts  returns (bytes32) {
    mint(_tokenId, _tokenQty, _royaltyPercentage);

    bytes32 id = listingMarketplace.createListing(address(this), _tokenId, _tokenQty);
  ...
  function mintAndCreateAuction(uint256 _tokenId, uint256 _tokenQty, uint8 _royaltyPercentage, uint128 _endsAt) external onlyNonContracts  returns (bytes32) {
    mint(_tokenId, _tokenQty, _royaltyPercentage);

    bytes32 id = auctionMarketplace.createAuction(address(this), _tokenId, _tokenQty, _endsAt);
