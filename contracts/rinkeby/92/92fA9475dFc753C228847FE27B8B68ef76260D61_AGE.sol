...
contract AGE is ERC721, ERC721Enumerable, Pausable, Ownable {
    ...
    //Address => tokenIDs
    mapping(address => uint[]) private mintRecords;
    //Stage => merkle proof
    mapping(uint => bytes32) private preMintRoot;
    ...
    uint256 public constant AGE_PRICE = 0.1 ether; //0.1 ETH

    uint256 public constant AGE_PREPRICE = 0.088 ether; //0.088 ETH


    uint public constant MAX_AGE_PURCHASE = 20;

    uint256 public MAX_AGES = 10000;

    Vault private vault;

    address private hostAddress;

    constructor(string memory name, string memory symbol, address host) ERC721(name, symbol) {
        hostAddress = host;
    }
    ...
    function burn(uint tokenId) external {
        require(tx.origin == ownerOf(tokenId), "Only vault can burn token.");
    ...
    function ownerMint(uint numberOfTokens, address to) public onlyOwner payable {
        ...
        require(totalSupply() + numberOfTokens <= MAX_AGES, "Out of max ages");
        require(AGE_PRICE * numberOfTokens / 10 <= msg.value, "Not enough Ether to mint.");

        uint vaultsValue = msg.value / 10;
        for (uint i = 0; i < numberOfTokens; i++) {
            _safeMint(to, _tokenIdCounter.current());
            vault.initGuardianVault{value : vaultsValue}(_tokenIdCounter.current(), vaultsValue);
    ...
    */
    function mintAge(uint numberOfTokens) public payable whenNotPaused {
        ...
        for (uint i = 0; i < numberOfTokens; i++) {
            ...
            vault.initGuardianVault{value : vaultValue}(_tokenIdCounter.current(), vaultValue);
            mintRecords[_msgSender()].push(_tokenIdCounter.current());
            _tokenIdCounter.increment();
        }
    }
    ...
    function preMintAge(
        ...
        payable(hostAddress).transfer(msg.value - vaultValue * numberOfTokens);
