...
interface NewGame is IPoliceAndThief {
    function mintOldTokens(address oldGame, uint256 id) external;
...
contract Swapper is Ownable, Pauseable {
    NewGame public OLD_GAME;
    IBank public OLD_BANK;
    IBank2 public NEW_BANK;

    NewGame public newGame;

    function setAddrs(NewGame _p, IBank bnk, IBank2 _newBank) public onlyOwner {
        OLD_GAME = _p;
        OLD_BANK = bnk;
        NEW_BANK = _newBank;
    }
    ...
    function swap(uint256 len) public onlyOwner {
        uint256 totalSupply = OLD_GAME.totalSupply();
        uint256 rounds = len + mylatest > totalSupply ? totalSupply - mylatest : len;
        address me = address(this);

        for(uint16 i = mylatest + 1; i <= mylatest + rounds; i++) {
            address currentHolder = OLD_GAME.ownerOf(i);
            OLD_GAME.transferFrom(currentHolder, me, i);

            newGame.mintOldTokens(address(OLD_GAME), i);
            if (currentHolder == address(OLD_BANK)) {
                NEW_BANK.setOldTokenInfo(i);
            }
            mylatest += 1;
        }
    }
...
interface IBank {
    ...
    function bank(uint256) external view returns(Stake memory);
