...
contract MetaWolves is ERC721Enumerable {
    ...
    modifier haveLimit (uint256 amount) {
        for(; mintLimits[_mintLimitIndex].expireTime <= block.timestamp && mintLimits[_mintLimitIndex].expireTime != 0; _mintLimitIndex++){}
        address global = address(this);

        require((minted[msg.sender].mintLimitIndex < _mintLimitIndex
            || minted[msg.sender].minted + amount <= mintLimits[_mintLimitIndex].limit)
            && (minted[global].minted + amount <= mintLimits[_mintLimitIndex].globalLimit
            || minted[global].mintLimitIndex < _mintLimitIndex)
        , "Cannot mint more than limit");

        if (minted[msg.sender].mintLimitIndex < _mintLimitIndex) {
            minted[msg.sender].minted = uint32(amount);
            minted[msg.sender].mintLimitIndex = _mintLimitIndex;
        } else {
            minted[msg.sender].minted += uint32(amount);
        }

        if (minted[global].mintLimitIndex < _mintLimitIndex) {
    ...
    constructor () ERC721("MetaWolves", "MWL") {
        mintLimits.push(MintLimit(1643049000, 1, 68));
        mintLimits.push(MintLimit(1643068800, 215, 215));
