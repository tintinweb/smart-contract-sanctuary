...
contract Market is Expend {
    ...
    event SetProxy(address _proxy,bool _open);
    event CancelTrade(bytes[] signs);

    constructor (address _owner,uint _fee,address _platformCurrency,address _wallet,address _coin) Expend(_owner,_fee,_platformCurrency,_wallet,_coin){}

    // 设置代理中心地址
    function setProxyModel(address _proxyModel,bool _open) public onlyOwner {
        emit SetProxy(_proxyModel,_open);
        model[_proxyModel] = _open;
    }
    // 提币
    ...
    function getMsgHash(Order memory o,uint _who) internal view returns (bytes32){
      bytes32 msghash;
      if(_who == 1){
    ...
    function trade(
      ...
      if(us[4] == 1){
        ...
        newCreateor = TradeRecord[o.art][o.id].creator;
        _erc20Transfer(msg.sender,o.token,o.price,o.gooder,newCreateor,signID.belong,signID.nonce);
        _nTransfer(msg.sender,o.nft,o.art,newCreateor,signID.belong,o.id,signID.nonce);
        ...
        require(o.token != address(0),"We won't support it");
        // 签名检查
        _checkPass(o,ss,2);
        _checkPass(b,bs,1);
        NFTExchange memory signID = TradeRecord[o.art][o.id];
        ...
        newCreateor = TradeRecord[o.art][o.id].creator;
        // erc20或eth转账
        _erc20Transfer(b.doer,o.token,b.price,b.gooder,newCreateor,signID.belong,signID.nonce);
        // nft转账
        _nTransfer(b.doer,o.nft,o.art,newCreateor,signID.belong,o.id,signID.nonce);
        // 记录交易后的所属关系
        TradeRecord[o.art][o.id].nonce = signID.nonce + 1;
    ...
    function _checkPass(Order memory order,bytes[] memory signs,uint _who) internal view {
      require(order.end/1000 >= block.timestamp && Address.isContract(order.art) && order.create/1000 < block.timestamp && order.sale/1000 < block.timestamp && !forbidSignature[signs[0]], "Illegal parameter !");
      if(order.token != address(0)){
        require(Address.isContract(order.token),"Illegal parameter !");
      }
      ...
      if(_who == 2){
        ...
        require(signer == order.doer);
      }else{
        revert("Illegal parameter !");
      }
    }
}