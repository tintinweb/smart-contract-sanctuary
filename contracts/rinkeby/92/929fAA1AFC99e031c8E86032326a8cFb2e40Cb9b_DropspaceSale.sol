...
contract DropspaceSale is IDropspaceSale, ERC721, Ownable {
    ...
    function setPresaleTime(uint256 _presaleTime) external override onlyOwner {
    ...
    function setSaleTime(uint256 _saleTime) external override onlyOwner {
        saleTime = _saleTime;
    ...
    function toggleSaleActive() external onlyOwner override {
        if (block.timestamp >= saleTime) {
            saleTime = MAX;
            emit ToggleSaleState(false);
        } else {
            saleTime = 0;
            emit ToggleSaleState(true);
        }
    }

    function toggleWhitelistSaleActive() external onlyOwner override {
        if (block.timestamp >= whitelistSaleTime) {
            whitelistSaleTime = MAX;
            emit ToggleWhitelistSaleState(false);
        } else {
            whitelistSaleTime = 0;
            emit ToggleWhitelistSaleState(true);
        }
    }
    ...
    function togglePresaleActive() external onlyOwner override {
        if (block.timestamp >= presaleTime) {
            presaleTime = MAX;
            emit TogglePresaleState(false);
        } else {
            presaleTime = 0;
            emit TogglePresaleState(true);
        }
    }
