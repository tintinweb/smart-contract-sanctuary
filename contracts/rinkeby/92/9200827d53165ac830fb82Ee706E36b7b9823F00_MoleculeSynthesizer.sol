...
contract MoleculeSynthesizer is MoleculeScripter, ERC721 {
  ...
  mapping (string => uint) elementToId;
  ...
  event NewMolecule(uint moleculeId, string formula, string InChIKey, string name, uint16 scriptId);

  /// @notice stores molecule information
  /// @param formula is in InChI (international chemical Identifier) format
  /// @param InChIKey is a unique hash for each molecule
  ...
  /// @notice InChIKeys to tokenIDs
  mapping (string => uint) private keys;

  /// @notice ensures that each molecule can only exist once per script
  modifier moleculeChecker (uint16 _scriptId, string memory _InChIKey) {
    if (keys[_InChIKey] > 0) {
      require(molecules[keys[_InChIKey]-1].scriptId != _scriptId, "molecule already minted");
    }
    _;
  }
  ...
  function _createMolecule(
    ...
    string memory _InChIKey,
    string memory _name,
    uint16 _scriptId
    ) internal moleculeChecker(_scriptId, _InChIKey) mintableScript(_scriptId) returns (uint) {
          uint id = _scriptId * 100000 + scripts[_scriptId].currentSupply;
          _safeMint(msg.sender, id);
          molecules[id] = Molecule(_formula, _InChIKey, _name, _scriptId);
          keys[_InChIKey] = id+1;
          scripts[_scriptId].currentSupply++;
          emit NewMolecule(id, _formula, _InChIKey, _name, _scriptId);
          return id;
      }
  ...
  function chemPolice(
    ...
    string memory _name) notSealed(molecules[_moleculeId].scriptId) onlyOwner external {
      ...
      wrongMolecule.InChIKey = _InChIKey;
  ...
  function getPrice(uint _scriptId) view public returns(uint) {
    uint duration = scripts[_scriptId].saleDuration * 1 hours;
  ...
  function mintMolecule(
    ...
    uint _numberOfElements
      ...
      _createMolecule(_formula, _InChIKey, _name, _scriptId);
  }
...
contract MoleculeScripter is Ownable {
  ...
  function updateScriptPrice(uint _scriptId, uint8 _saleDuration, uint64 _startPrice, uint64 _endPrice) notLocked(_scriptId) onlyScriptCreator(_scriptId) external {
