...
interface IBase1155 {
    function mint(address creator, string memory tokenURI, uint256 fee) external returns(uint256);
}
...
contract TransferProxy {
    function erc721mint(IBase721 token, address creator, string memory tokenURI, uint256 fee) external  {
        token.createCollectible( creator, tokenURI, fee);
    }

    function erc1155mint(IBase1155 token, address creator, string memory tokenURI, uint256 fee) external  {
...
contract Trade {
    ...
    function verifyOwnerSign(address nftAddress, address caller, string memory tokenURI, Sign memory sign) internal view {
        bytes32 hash = keccak256(abi.encodePacked(nftAddress, caller, tokenURI));
        require(signer == getSigner(hash, sign), "Owner sign verification failed");
    }
    ...
    function mint(address nftAddress, BuyingAssetType nftType, string memory tokenURI, uint256 fee, Sign memory sign) public payable returns(bool) {
        require(msg.value == mintingFee, "Minting: Values must be equal to mintingFee");
        verifyOwnerSign(nftAddress, msg.sender, tokenURI, sign);
        payout(owner, msg.value);
        if(nftType == BuyingAssetType.ERC721) {
            transferProxy.erc721mint(IBase721(nftAddress) ,msg.sender, tokenURI, fee);
        }
        if(nftType == BuyingAssetType.ERC1155) {
            transferProxy.erc1155mint(IBase1155(nftAddress) ,msg.sender, tokenURI, fee);
        }
        return true;
    }
