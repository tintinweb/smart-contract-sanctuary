...
contract Pixls is ERC721, Ownable, IMintedBeforeReveal {
    ...
    uint256 public constant SALE_START_TIMESTAMP = 1616700623;

    // Time after which the Pixls are randomized and revealed (1 hour from initial launch).
    uint256 public constant REVEAL_TIMESTAMP = SALE_START_TIMESTAMP + (3600 * 1);

    // Maximum amount of Pixls in existance. Ever.
    uint256 public constant MAX_PIXL_SUPPLY = 310;
    ...
    */
    function getPixlMaxAmount() public view returns (uint256) {
        ...
        if (currentSupply >= 310) {
            return 50; // After 310, do it per 50.
        }
        else if (currentSupply >= 75) {
            return 50; // From 75 to 310 - per 50.
        } 
        else {
            return 20; // First 75 can only be bought per 20.
        }
    }
    ...
    */
    function getPixlPrice() public view returns (uint256) {
        ...
        if (currentSupply >= 300) {
            return 1000000000000000000; // 300 - 310 1 ETH
        } else if (currentSupply >= 200) {
            return 300000000000000000; // 200-300 0.3 ETH
        } else if (currentSupply >= 150) {
            return 200000000000000000; // 150-200 0.2 ETH
        } else if (currentSupply >= 100) {
            return 100000000000000000; // 100 - 150 0.1 ETH
        } else if (currentSupply >= 75) {
            return 40000000000000000; // 75 - 100 0.04 ETH
        } else {
            return 20000000000000000; // 0 - 74 0.02 ETH 
        }
    }
