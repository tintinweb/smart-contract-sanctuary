...
contract PaceArtFactory is Ownable {
    ...
    ProxyRegistry private _proxyRegistryAddress;
    ...
    constructor(PaceArtStore _impl, ProxyRegistry _registry) {
        impl = _impl;
        _proxyRegistryAddress = _registry;
    }

    function setProxyRegistry(ProxyRegistry _registry) external onlyOwner {
        require(address(_registry) != address(_proxyRegistryAddress), "PaceArtFactory::SAME REGISTRY ADDRESS");
        emit CollectionRegistrySettled(address(_proxyRegistryAddress), address(_registry));
        _proxyRegistryAddress = _registry;
    }
    ...
    function newCollection(
        ...
        PaceArtStore(newCollection).initialize(_name, _symbol, _tokenURI, address(_proxyRegistryAddress));
 ...
 */
contract PaceArtStore is RoyaltiesV2Impl, ERC721Upgradeable {
    using SafeMath for uint256;
