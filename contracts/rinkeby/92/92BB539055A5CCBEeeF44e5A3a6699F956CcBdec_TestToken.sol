...
contract TestToken is ERC721Enumerable, ERC721URIStorage {
    ...
    {
        string memory encodedClaim = Base64.encode(
            Strings.toString(claim / 1e18)
        );
        string memory output = string(
            abi.encodePacked(
                '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="black" /><text x="10" y="20" class="base">Hello</text><text x="10" y="40" class="base">World</text></svg>'
            )
        );
        string memory encodedOutput = Base64.encode(output);
