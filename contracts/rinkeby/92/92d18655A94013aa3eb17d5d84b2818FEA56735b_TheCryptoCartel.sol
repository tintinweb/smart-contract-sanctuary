...
contract TheCryptoCartel is ERC721Enumerable, AccessControl {
    ...
    string private BASE_URI = "https://gateway.pinata.cloud/ipfs/QmcJYqxXyyB5obzkXbjTc7kZvfSTXJQCA8f2GgS6muutfQ/";
    ...
    uint8 MINT_LIMIT = 6;
    uint8 WHITELIST_TRANSACTION_LIMIT = 10; // Limit 10

    uint16 SUPPLY_LIMIT = 3500;
    ...
    function mint(uint256 numberOfTokens, bytes32[] calldata proof, bytes32 merkleRoot) public payable {
        ...
        if(whitelistEnabled) {
            require(balanceOf(msg.sender) + 1 < 11, "You have crossed the limit for whitelist" );
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            ...
            //_setTokenURI(tokenId, string(abi.encodePacked(BASE_URI, Strings.toString(tokenId), ".json")));
            
        }
    }

    function giveAwayMint(uint256 numberOfTokens, address mintAddress)  public onlyRole(DEFAULT_ADMIN_ROLE){
        uint256 ts = totalSupply();

        require(ts + numberOfTokens <= SUPPLY_LIMIT, "Reached max limit. No more minting possible!");
        require(_giveawayCounter.current() + numberOfTokens < 101, "max limit for giveaway reached");
    ...
    function giveAwayMint2(address[] memory mintAddress)  public onlyRole(DEFAULT_ADMIN_ROLE){
        uint256 ts = totalSupply();
        uint256 len = mintAddress.length;

        require(ts + len <= SUPPLY_LIMIT, "Reached max limit. No more minting possible!");
        require(_giveawayCounter.current() + len < 101, "max limit for giveaway reached");
        
        for (uint256 i = 0; i < mintAddress.length; i++) {
            uint256 tokenId = _tokenIdCounter.current();
            _tokenIdCounter.increment();
            _giveawayCounter.increment();
            _safeMint(mintAddress[i], tokenId);
            //_setTokenURI(tokenId, string(abi.encodePacked(BASE_URI, Strings.toString(tokenId), ".json")));
