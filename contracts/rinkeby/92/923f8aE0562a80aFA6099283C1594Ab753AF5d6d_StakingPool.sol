...
import "./WAYTokenWrapper.sol";

contract Constant {
	uint public constant ZOOM_WAY = 10 ** 18;
	uint public constant ZOOM_PUMPKIN = 10 ** 18;
}

contract StakingPool is WAYTokenWrapper, Constant {
	ERC1155Tradable public wayNFTs;
	...
	constructor(ERC1155Tradable _wayNFT, IERC20 _wayToken) public WAYTokenWrapper(_wayToken) {
		wayNFTs = _wayNFT;
		whiteListAdder[msg.sender] = true;
	}

	function setWAYNFT(ERC1155Tradable _wayNFT) public onlyOwner {
		wayNFTs = _wayNFT;
	}

	function changeStakingAddress(ERC1155Tradable _adder) external onlyOwner {
		wayNFTs = _adder;
	}
	...
	// 1000 WAY staked will earn 10 pumpkins perday
	...
	// stake visibility is public as overriding WAYTokenWrapper's stake() function
	...
	function redeem(uint256 card) public updateReward(msg.sender) {
		...
		require(wayNFTs.totalSupply(card) < wayNFTs.maxSupply(card), "Max cards minted");
		pumpkins[msg.sender] = pumpkins[msg.sender].sub(cards[card]);
		wayNFTs.mint(msg.sender, card, 1, "");
...
contract WAYTokenWrapper is Ownable {
	using SafeMath for uint256;
	IERC20 public WAYToken;

	constructor(IERC20 _wayToken) public {
		WAYToken = IERC20(_wayToken);
	}
	...
	function stake(uint256 amount) public {
		...
		WAYToken.transferFrom(msg.sender, address(this), amount);
	}

	function withdraw(uint256 amount) public {
		_totalSupply = _totalSupply.sub(amount);
		_balances[msg.sender] = _balances[msg.sender].sub(amount);
		WAYToken.transfer(msg.sender, amount);
	}

	function setWAYToken(IERC20 _wayToken) public onlyOwner {
		WAYToken = _wayToken;
	}
}
