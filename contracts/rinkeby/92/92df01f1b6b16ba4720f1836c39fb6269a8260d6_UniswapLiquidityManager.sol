...
contract UniswapLiquidityManager is PeripheryPayments, IUniswapLiquidityManager {
    ...
    /// @param readjustSwapStatus_ Status of the pool for swapping of new tokens during readjust
    function setReadjustPremiumForPools(
        address pool,
        uint256 premium_,
        uint256 gasPriceLimit_,
        bool premiumStatus_,
        bool readjustSwapStatus_
    ) external onlyGovernance {
        readjustPremium[pool] = ReadjustPremium({
            readjustSwapStatus: readjustSwapStatus_,
    ...
    function readjustLiquidity(
        ...
        if (
            readjustPremium[b.poolAddress].readjustSwapStatus &&
            (b.amount0 == 0 || b.amount1 == 0)
        ) {
    ...
    {
        return IOracle(_oracle).checkWethPairsAndLiquidity(token);
    }
