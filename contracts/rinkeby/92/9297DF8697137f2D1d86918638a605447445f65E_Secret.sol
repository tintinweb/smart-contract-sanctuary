...
contract Secret is ERC721Enumerable, Ownable {
    ...
    uint256 public constant PRICE = 0.000833 ether; // 0.0833

    uint16 public constant MAX_GIFT_TOKENS = 32;
    uint16 public constant MAX_PRESALE_AMOUNT = 300;
    uint16 public constant MAX_PUBLIC_SALE_TOKENS = 8000;
    uint16 public constant TOTAL_SUPPLY =
        MAX_GIFT_TOKENS + MAX_PRESALE_AMOUNT + MAX_PUBLIC_SALE_TOKENS;
    ...
    function mint(
        ...
        require(
            publicAmount + quantity <= MAX_PUBLIC_SALE_TOKENS,
            "EXCEED_PUBLIC_LIMIT"
        );
    ...
    function mintPresale(uint256 quantity) external payable {
        ...
        require(totalSupply() < TOTAL_SUPPLY, "OUT_OF_STOCK");
        require(
            presaleAmount + quantity <= MAX_PRESALE_AMOUNT,
            "EXCEED_PRESALE_LIMIT"
        );
        require(
            presalePurchases[msg.sender] + quantity <= presalePurchaseLimit,
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        ...
        require(
            giftedAmount + receivers.length <= MAX_GIFT_TOKENS,
            "GIFTS_EMPTY"
        );
    ...
    function setStartingIndexInternal() private {
        if (block.number - startingIndexBlock > 255) {
            ...
            startingIndex = uint256(blockhash(block.number - 1)) % TOTAL_SUPPLY;
        } else {
            startingIndex =
        ...
        if (startingIndex == 0) {
            startingIndex = (presaleAmount + 1) % TOTAL_SUPPLY;
        }
    }
    ...
    function addToPresale(address[] calldata entries) external onlyOwner {
