...
contract BarrelyNaked is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public _saleStartUTS;

    // Maximum token supply
    uint256 public _MAX_SUPPLY;

    // Minting price per token
    uint256 public _mintPrice = 0.04 ether;

    // Sets maximum possible purchases per transaction
    uint256 public _maxPurchaseCount = 20;

    // Tracks number of free minted tokens
    uint256 public _numberMintedFree = 0;

    // Minting start offset
    uint256 public _freeMintStartOffsetUTS = 0;

    // Minting end offset
    uint256 public _freeMintEndOffsetUTS = 86400; 
    ...
    function mintTokens(uint256 numberOfTokens_) public payable checkSupplyAvailable(numberOfTokens_) {
        ...
        require(_mintPrice.mul(numberOfTokens_) == msg.value, "Ether value for request isn't valid");
        require(numberOfTokens_ <= _maxPurchaseCount, "Purchase would exceed the minting limit per transation");
    ...
    function claimFreeToken() public checkSupplyAvailable(1) {
        ...
        require(hasFreeMintPeriodStart(), "The free minting period hasn't started");
        require(hasFreeMintPeriodEnd(), "Free mint period has ended");
        require(!_addressesUsedFreeMint[msg.sender], "Free token has already been claimed for this address");
