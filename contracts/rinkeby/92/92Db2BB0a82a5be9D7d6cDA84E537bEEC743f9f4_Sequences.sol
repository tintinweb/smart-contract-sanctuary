...
contract Sequences is ERC721, ERC721URIStorage, Ownable{
    ...
    uint256 constant MAX_SEQUENCES = 3; //deploy: 4
    uint256 constant SEQUENCE_ROWS = 4; 
    uint256 constant STEPS_PER_BAR = 4;
    uint256 constant BARS_PER_SEQUENCE = 4; //deploy 4
    uint256 constant WIDTH = STEPS_PER_BAR * BARS_PER_SEQUENCE;
    uint256 constant HEIGHT = (SEQUENCE_ROWS * MAX_SEQUENCES);
    //uint256 constant HEIGHT = (SEQUENCE_ROWS * MAX_SEQUENCES) + (MAX_SEQUENCES + 1);
    int256 constant HALF_WIDTH = int(WIDTH) / 2;
    int256 constant HALF_HEIGHT = int(HEIGHT) / 2;
    ...
    function getRow(uint256 row, uint256 id) internal view returns (bytes memory) {
        ...
        uint256 rand = random(string(abi.encodePacked( Strings.toString(row),Strings.toString(id)))) + 5;
        
        uint256 length = (rand % (STEPS_PER_BAR * BARS_PER_SEQUENCE / 2)) + 1;
        //could make this a silence variable too
        ...
        for (uint j = 0; j < WIDTH; j++) {
            ...
            if(length > 0) {
                //consideration: allow long length sequences but insert more random silence
    ...
    function drawSequence(uint256 id) public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Sequence #', Strings.toString(id), '", "description": "Sequences are randomly generated beat sequences stored and rendered on chain. Build players for your sequence and remix with others.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(svgAccumulator)),'"}'))));
        return string(abi.encodePacked('data:application/json;base64,', json));
    }
    
    function readSequence(uint256 id) public view returns (string memory) {
        uint256 rowWidth = WIDTH + 3; //add 3 bytes for EOL sequence
        bytes memory output = new bytes(HEIGHT * (WIDTH + 3) + 30);
        uint256 c;
        for (c = 0; c < 30; c++) {
            output[c] = prefix[c];
