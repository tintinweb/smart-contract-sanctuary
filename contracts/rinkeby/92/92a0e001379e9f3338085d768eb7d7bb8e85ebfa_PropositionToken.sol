...
/**
 * This is Dimenschen Propositions
 * Special thanks to those who have helped me to reach this point
 */
...
contract PropositionToken is ERC721Enumerable, Ownable {
    ...
    event Mint(address indexed _from, uint indexed _id, string _proposition);
    ...
    // public facing proposition validity check
    function isPropositionValid( string memory _proposition ) public view returns(string memory) {
        string memory transformedProp = toLower(_proposition);
        string memory message = "Proposition can be created";
        if (bytes(transformedProp).length > 350) {
            message = "Proposition is over 350 characters";
        } else if (doesPropositionExist(transformedProp) == true) {
            message = "Proposition is not unique";
        } else if (validateString(transformedProp) == false) {
    ...
    function mint( string memory _proposition ) public payable {
        ...
        string memory transformedProp = toLower(_proposition);
        require(validateString(transformedProp) == true, "Proposition is invalid");
        propositionToId[transformedProp] = true;
        idToProposition[id] = transformedProp;

        _mint(msg.sender, id);
        emit Mint(msg.sender, id, transformedProp);
    }
    
    /**
     * @dev Check if the Proposition string is valid 
     * No leading or trailing spaces
     * No continous spaces
     * abcdefghijklmnopqrstuvwxyz0123456789 ',-.:;#$%()*+/=
     */
    function validateString(string memory str) public view returns (bool){
        string memory transformedProp = toLower(str);
        bytes memory b = bytes(transformedProp);
        if (b.length < 1) return false; // cannot be short than 1 character
        if (b.length > 350) return false; // cannot be longer than 350 characters
        if (doesPropositionExist(transformedProp) == true) return false; // proposition must be unique 
        ...
        for(uint i; i<b.length; i++){
            ...
            if (
                ...
                !(char >= 0x23 && char <= 0x25) && //#$%
                !(char >= 0x27 && char <= 0x2F) && //'()*+,-./
                !(char >= 0x30 && char <= 0x39) && //0123456789
                !(char >= 0x61 && char <= 0x7A) //abcdefghijklmnopqrstuvwxyz
            ) 
                return false;
            lastChar = char;
        }
    ...
    function getImageCode(string memory _proposition, uint256 tokenId) public pure returns (uint) {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (tokenId < 100) {
            ...
            tokenEpoch = epochs[4];
        } else if (tokenId >= 9750 && tokenId < 10000) {
            tokenEpoch = epochs[5];
        } else {
            tokenEpoch = epochs[6];
        }
        ...
        uint imageCode = getImageCode(_proposition, tokenId);
        // start svg
        ...
        jsonMeta = string(abi.encodePacked(jsonMeta, ' "attributes": [{ "trait_type": "Epoch", "value": "',tokenEpoch,'" }, { "trait_type": "Image DNA", "value": "',toString(imageCode),'" }, { "trait_type": "Sky", "value": "',skyColors[getIndexes(imageCode, 0)],'" }, { "trait_type": "Planet", "value": "',planetColors[getIndexes(imageCode, 1)],'" }, { "trait_type": "Heavenly Body", "value": "',heavenlyBodyColors[getIndexes(imageCode, 2)],'" }, { "trait_type": "Heavenly Reflection", "value": "',heavenlyReflectionColors[getIndexes(imageCode, 3)],'" }]}'));
