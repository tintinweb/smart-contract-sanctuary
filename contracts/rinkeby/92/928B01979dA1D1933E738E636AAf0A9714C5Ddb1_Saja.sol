...
contract Saja is Context, IERC20, Ownable {
    ...
    address[] private cradleRewardAddresses;
    ...
    function getTctTierSettings() public view returns (uint256, uint256, uint256, uint256)  {
        return (_tctCubTier, _tctLionTier, _tctLionessTier, _tctAlphaTier);
    }
    ...
    function setRewardRates(uint256 cradleRate, uint256 cubRate, uint256 lionessRate, uint256 lionRate, uint256 alphaLionRate) external onlyOwner() {
        _cradleRate = cradleRate;
    ...
    function distributeTokens(uint256 cradleAmount, uint256 cubAmount, uint256 lionessAmount, uint256 lionAmount, uint256 alphaLionAmount) private {
        ...
        for (uint256 i = 0; i < _holders.length; i++) {
            ...
            cradleRewardAddresses.push(holderAddress);
        }
        ...
        cradleAmount = cradleAmount.add(unusedRewards);
        addTokensToAddresses(cradleRewardAddresses, cradleAmount);
    }
