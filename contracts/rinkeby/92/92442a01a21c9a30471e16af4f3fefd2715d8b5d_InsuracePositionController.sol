...
contract InsuracePositionController is AbstractController {
    ...
    function lpTokenInfo() public view returns(
        uint256 rewardDebtOf,
        uint256 burnWeightPH,
        uint256 pendingBurnAmtPH,
        uint256 burnableAmtPH,
        uint256 burnableAmtOf
    ){
        rewardDebtOf = ILPToken(lpToken).rewardDebtOf(address(this));
        burnWeightPH = ILPToken(lpToken).burnWeightPH(address(this));
        pendingBurnAmtPH = ILPToken(lpToken).pendingBurnAmtPH(address(this));
        burnableAmtPH = ILPToken(lpToken).burnableAmtPH(address(this));
        burnableAmtOf = ILPToken(lpToken).burnableAmtOf(address(this));
    }
}
