...
contract SocialToken is ERC20 {
     ...
     */
    function curveIntegral(uint256 upperBound, uint256 lowerBound) internal returns (uint256) {
        uint256 multiplier;
        uint256 integral;
        ...
        if (ammType == AMMType.QUADRATIC) {
            ...
            if (upperBound**(TWO_DECIMAL) > lowerBound**TWO_DECIMAL) {
                integral = upperBound**THREE_DECIMAL - lowerBound**THREE_DECIMAL;
            } else {
                integral = lowerBound**THREE_DECIMAL - upperBound**THREE_DECIMAL;
            }
            return multiplier.mul(integral).div(THREE_DECIMAL);
        }
        if (ammType == AMMType.EXPONENTIAL) {
            multiplier = targetPrice.mul(eD ** targetSupply).div(eN ** targetSupply);
            if (eN**upperBound.div(eD**upperBound) > eN**lowerBound.div(eD**lowerBound)) {
                integral = eN**upperBound.div(eD**upperBound) - eN**lowerBound.div(eD**lowerBound);
            } else {
                integral = eN**lowerBound.div(eD**lowerBound) - eN**upperBound.div(eD**upperBound);
            }
    ...
    function rewardForBurn(uint256 numTokens) public returns(uint256){
        ...
        return fundingAmount.mul(100).sub(tradingSpread).div(100);
    }
    ...
    function buyToken(uint256 numTokens) public {
        ...
        require(fundingToken.transferFrom(msg.sender, address(this), priceForTokens));
