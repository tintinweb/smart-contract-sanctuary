...
contract NoBrainers is ERC721("No Brainers", "NBRN"), ERC721Enumerable, Ownable, Pausable {
    using SafeMath for uint256;
    using Strings for uint256;

    uint256 public constant RESERVE_NFT = 100;
    uint256 public constant PUBLIC_NFT = 5000;
    ...
    uint256 public constant WHITELIST_MAX_MINT = 4;
    ...
    constructor(uint256 _PRESALE_START_TIMESTAMP) {
        PRESALE_START_TIMESTAMP = _PRESALE_START_TIMESTAMP;
        WHITELIST_START_TIMESTAMP = _PRESALE_START_TIMESTAMP.add(1 days);
        SALE_START_TIMESTAMP = WHITELIST_START_TIMESTAMP.add(30 minutes);

        for(uint i = 0; i < RESERVE_NFT; i++) {
    ...
    //Function addToWhiteList to add whitelisted addresses
    ...
    function mint(uint256 _numOfTokens) public payable whenNotPaused {
        require(block.timestamp >=  SALE_START_TIMESTAMP, "Sale Not Started");
        require(totalSupply().add(_numOfTokens) <= MAX_NFT, "Purchase would exceed max NFTs");
        require(claimed[msg.sender].add(_numOfTokens) <= PURCHASE_LIMIT, "Above Purchase Limit");
        ...
        claimed[msg.sender] = claimed[msg.sender].add(_numOfTokens);
    }
    
    // Function to mint new NFTs during the presale
    function mintDuringPresale(uint256 _numOfTokens) public payable whenNotPaused {
        require(block.timestamp >=  PRESALE_START_TIMESTAMP && block.timestamp < WHITELIST_START_TIMESTAMP, 
            "Purchase in Presale");
        require(presaleAccess[msg.sender] == true, "Presale Access Denied");
        require(totalSupply().add(_numOfTokens) <= MAX_NFT, 'Purchase would exceed max NFTs');
        require(presaleClaimed[msg.sender].add(_numOfTokens) <= WHITELIST_MAX_MINT, "Above Presale Purchase Limit");
        require(NFT_PRICE.mul(_numOfTokens) == msg.value, "Invalid Amount");
    ...
    // Function to mint new NFTs during the whitelist
    function mintDuringWhitelist(uint256 _numOfTokens) public payable whenNotPaused {
        require(block.timestamp >=  WHITELIST_START_TIMESTAMP && block.timestamp < SALE_START_TIMESTAMP, 
            "Purchase in Whitelist");
        require(whiteList[msg.sender] == true, "Not Whitelisted");
        require(totalSupply().add(_numOfTokens) <= MAX_NFT, 'Purchase would exceed max NFTs');
        require(whiteListClaimed[msg.sender].add(_numOfTokens) <= PRESALE_MAX_MINT, "Above Whitelist Purchase Limit");
        require(NFT_PRICE.mul(_numOfTokens) == msg.value, "Invalid Amount");
    ...
    function giveaway(address[] memory _to, uint256[] memory _tokenIds) external onlyOwner {
    ...
    // Function for support interface    
