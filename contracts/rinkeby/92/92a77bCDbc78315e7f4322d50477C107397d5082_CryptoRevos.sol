/**  
 SPDX-License-Identifier: GPL-3.0
   ...
   _____                  _        _____                     
  / ____|                | |      |  __ \                    
 | |     _ __ _   _ _ __ | |_ ___ | |__) |_____   _____  ___ 
 | |    | '__| | | | '_ \| __/ _ \|  _  // _ \ \ / / _ \/ __|
 | |____| |  | |_| | |_) | || (_) | | \ \  __/\ V / (_) \__ \
  \_____|_|   \__, | .__/ \__\___/|_|  \_\___| \_/ \___/|___/
               __/ | |                                       
              |___/|_|                      
...
contract CryptoRevos is ERC721A, ReentrancyGuard, Ownable, EIP712Whitelisting {
    ...
    uint256 public maxMintCountPerTxn;
    ...
    constructor(
        ...
        ERC721A("CryptoRevos", "CR", _maxBatchSize, _collectionSize)
        EIP712Whitelisting()
    {
        customBaseURI = _customBaseURI;
        splitter = new PaymentSplitter(_payees, _shares);
        price = 0.4 ether;
        maxMintCountPerTxn = _maxBatchSize;
        maxSupply = _collectionSize;
        maxReserveSupply = 200;
        maxWhitelistSupply = 1500;
    }
    ...
    function mint(uint256 _count) public payable nonReentrant {
        ...
        require(
            totalTokenSupply() + _count - 1 < maxSupply - maxReserveSupply,
            "Exceeds max supply"
        );
        ...
        for (uint256 i = 0; i < _count; i++) {
            ...
            _safeMint(_msgSender(), maxReserveSupply + totalTokenSupply());
        }
    ...
    function mintReserve(uint256 _count) external onlyOwner {
        require(_count > 0, "Cannot mint 0 tokens");
        require(
            totalReserveSupply() + _count - 1 < maxReserveSupply,
            "Exceeds max supply"
        );
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < _count; i++) {
            reserveSupplyCounter.increment();
            _safeMint(_account, totalReserveSupply());
        }
    }
        ...
        nonReentrant
    {
        ...
        require(
            totalWhitelistMints() + _count - 1 < maxWhitelistSupply,
            "Exceeds whitelist supply"
        );
        require(
            totalTokenSupply() < maxSupply - maxReserveSupply + _count - 1,
            "Exceeds max supply"
        );
        require(_count - 1 < maxMintCountPerTxn, "Trying to mint too many");
        // require(msg.value >= price * _count, "Insufficient payment");
    ...
    constructor() {
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256(bytes("Non-fungible Golfers Association")),
                keccak256(bytes("1")),
                block.chainid,
                address(this)
            )
