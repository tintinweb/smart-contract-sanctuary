...
contract ERC721Full is ERC721, ERC721Enumerable, ERC721Metadata {
    string name   = "SANDBOX LANDS";
    string symbol = "SANDBOX";
...
contract sandbox is ERC721Full, Ownable, IERC721Receiver {
    using SafeMath for uint256;
    
    
    string public tokenURIPrefix = "http://3.129.57.187/";
    
    IERC721   public SANDBOX721token;
    ...
    function setElfin721(address _SANDBOX721token) public onlyOwner {
        SANDBOX721token = IERC721(_SANDBOX721token);
    }
    ...
    function batchMint(address _to, uint256[] memory _tokenId) public onlyMinter {
        for (uint i = 1; i < _tokenId.length; i++) {
    ...
    function batchMint2(address _to, uint256 _tokenIdstart, uint256 _tokenIdend) public onlyMinter {
        while(_tokenIdstart < _tokenIdend) {
            _tokenIdstart = _tokenIdstart + 1;
            _mint(_to, _tokenIdend);
            emit minted(_to, _tokenIdend);
        }
    }
