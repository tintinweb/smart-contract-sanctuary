...
contract CollateralAndMint is LandOracleProxy {
    ...
    // When users deposit, the borrow limit increases.
    // When users withdraw the borrow limit decreases.
    ...
    function CalculateCollateralUse(
        ...
        // 0 + 9418640572678363 *
        if (_mint > 0) {
            CollateralUsedEth[msg.sender] =
                CollateralUsedEth[msg.sender] +
                ((_mint * 1e18) / landPerEthPrice);
            CalculateBorrowLimit();
        }
        if (_burn > 0) {
            CollateralUsedEth[msg.sender] =
                CollateralUsedEth[msg.sender] -
                ((_burn * 1e18) / landPerEthPrice);
            CalculateBorrowLimit();
        }
    }
    ...
    function CalculateBorrowLimit() private {
        // 1 - ((0.4 / 100 * 60) + 0.4)
        CurrentBorrowLimitEth[msg.sender] =
            CollateralBalance[msg.sender] -
            (((CollateralBalance[msg.sender] / 10000) *
                collateralRequirementPercent) + CollateralUsedEth[msg.sender]);
    }
    ...
    function MintAsset(uint256 _amount) public {
        ...
        require(
            // token price is 0.03 per one ETH
            (TokenPricePerEth * CurrentBorrowLimitEth[msg.sender]) / 1e18 >=
                _amount,
    ...
    function WithdrawCollateral(uint256 _amount) public {
        ...
        require(
            _amount <=
                CollateralBalance[msg.sender] -
                    (CollateralUsedEth[msg.sender] * 10000) /
