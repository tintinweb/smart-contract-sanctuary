...
contract MyToken is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable,  OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable, VRFConsumerBase {
    ...
    mapping(uint => uint256) private _dWalletSnapshotTotal;
    mapping(address => uint) private _dWalletSnapshotLastClaimed;
    ...
    function playNumberdome(uint256 luckyPicks) public {
        ...
        _transfer(_msgSender(), nWallet, 1000 * 10 ** decimals());
    ...
    function playGridlock(uint256[] memory tokensPlaced) public {
	   ...
	   for (uint i=0; i<tokensPlaced.length; i++) {
	        _gridlockEntries[_msgSender()][gridlockCurrentID][i] = _gridlockEntries[_msgSender()][gridlockCurrentID][i] + tokensPlaced[i];
    ...
    function checkDividendsOwed() public view returns(uint256){
        
        uint lastClaimedID = _dWalletSnapshotLastClaimed[_msgSender()];
        ...
        for (uint i= lastClaimedID; i<currentID; i++) {
            ...
            uint256 balanceAt = balanceOfAt(_msgSender(),i-1);
            uint256 eligbleSupplyAt = totalSupplyAt(i-1) - balanceOfAt(uniswapV2Pair, i-1) - balanceOfAt(owner(), i-1) - balanceOfAt(tWallet, i-1);
            
            if (balanceAt > 0) {
                dividendOwed = dividendOwed + _dWalletSnapshotTotal[i-1].mul(balanceAt).div(eligbleSupplyAt);
            }
        }
    ...
    function claimAllDividends() public {
        ...
        uint256 dividendOwed = checkDividendsOwed();
        
        require(dividendOwed > 0);
        
        _transfer(dWallet, _msgSender(), dividendOwed);
        
        _dWalletSnapshotLastClaimed[_msgSender()] = _currentSnapshotId.current();
