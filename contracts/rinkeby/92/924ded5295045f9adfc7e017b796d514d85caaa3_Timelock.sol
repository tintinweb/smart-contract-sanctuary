...
contract Offering is Configurable {
	...
	function offer() external {
		...
		require(now <= getConfig(_time_, _timeOfferEnd_), 'EXPIRED');
		...
		uint volume = quota.mul(getConfig(_ratio_, getConfig(_isSeed_, msg.sender)));
		_setConfig(_volume_, msg.sender, volume);
		_setConfig(_volume_, address(0), volume.add(getConfig(_volume_, address(0))));
	}
	
	function settleRemain() external {
	    require(now > getConfig(_time_, _timeOfferEnd_), 'It is not time');
	    token.safeTransfer(address(config[_public_]), token.balanceOf(address(this)).add(getConfig(_unlocked_, address(0))).sub(getConfig(_volume_, address(0))));
	}
...
contract BLACK is MappableToken {
	function __BLACK_init(address creator_, address factory_, address mine_, address investor_, address fund_, address team_, address contribution_) external initializer {
        __Context_init_unchained();
		__ERC20_init_unchained("AiSwap.io Governance Token", "Ai");
		__MappableToken_init_unchained(factory_, creator_);
		__BLACK_init_unchained(mine_, investor_, fund_, team_, contribution_);
	}
	
	function __BLACK_init_unchained(address mine_, address investor_, address fund_, address team_, address contribution_) public initializer {
		_mint(mine_,            80_000_000 * 10 ** uint256(decimals()));
		_mint(investor_,         6_000_000 * 10 ** uint256(decimals()));
		_mint(fund_,             7_000_000 * 10 ** uint256(decimals()));
		_mint(team_,             5_000_000 * 10 ** uint256(decimals()));
		_mint(contribution_,     2_000_000 * 10 ** uint256(decimals()));
	}
}