    ...
    */
    function _findClosestPoint(
        ...
        */
        ...
        if (history[left].timestamp <= timestamp) {
            return (true, history[left]);
        }
        return (false, point);
     ...
     function _findClosestPointWeek(
        uint256 timestamp
    ) public view returns (bool success, uint256 weekNum , uint256 pointHistoryLength) {
        ...
        */
        ...
        if (history[left].timestamp <= timestamp) {
            return (true, left, history.length);
        }
        return (false, 0, 0 );
    }
