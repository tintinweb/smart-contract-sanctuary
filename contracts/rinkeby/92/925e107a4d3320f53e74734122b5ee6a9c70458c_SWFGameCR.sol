...
// File: SWF_GAME/interfaces/ITraits.sol
...
contract SWFGameCR is ISafezoneGame, Ownable, ReentrancyGuard, Pausable {
  ...
  // max $SAFE cost 
  uint256 private maxSafeCost = 72000 ether;
  ...
  // reference to the Safezone for choosing random Variant thieves
  ISafezone public safezone;
  // reference to $SAFE for burning on mint
  ...
  modifier requireContractsSet() {
      require(address(safeToken) != address(0) && address(traits) != address(0) 
        && address(swfNFT) != address(0) && address(safezone) != address(0) && address(alter) != address(0)
        , "Contracts not set");
      _;
  }

  function setContracts(address _safe, address _traits, address _swf, address _safezone, address _alter) external onlyOwner {
    safeToken = ISafe(_safe);
    traits = ITraits(_traits);
    swfNFT = ISWF(_swf);
    safezone = ISafezone(_safezone);
  ...
  /** Initiate the start of a mint. This action burns $SAFE, as the intent of committing is that you cannot back out once you've started.
  ...
  function mintCommit(uint256 amount, bool stake) external whenNotPaused nonReentrant {
    ...
    uint16 minted = swfNFT.minted();
    uint256 maxTokens = swfNFT.getMaxTokens();
    require(minted + pendingMintAmt + amount <= maxTokens, "All tokens minted");
    require(amount > 0 && amount <= 10, "Invalid mint amount");

    uint256 totalSafeCost = 0;
    // Loop through the amount of 
    for (uint i = 1; i <= amount; i++) {
      totalSafeCost += mintCost(minted + pendingMintAmt + i, maxTokens);
    }
    if (totalSafeCost > 0) {
      safeToken.burn(_msgSender(), totalSafeCost);
  ...
  function reveal(address addr) internal {
    ...
    for (uint k = 0; k < commit.amount; k++) {
      ...
      if(recipient != addr && alter.balanceOf(addr, treasureChestTypeId) > 0) {
        ...
        //  a variant will prefer a treasure chest over it
        if(seed & 1 == 1) {
      ...
      if (!commit.stake || recipient != addr) {
        swfNFT.mint(recipient, seed);
      } else {
        swfNFT.mint(address(safezone), seed);
        tokenIdsToStake[k] = minted;
      }
    }
    swfNFT.updateOriginAccess(tokenIds);
    if(commit.stake) {
      safezone.addManyToSafezoneAndFlight(addr, tokenIdsToStake);
    }
    delete _mintCommits[addr][commitIdCur];
    delete _pendingCommitId[addr];
  ...
  function payTribute(uint256 safeAmt) external whenNotPaused nonReentrant {
    require(tx.origin == _msgSender(), "Only EOA");
    uint16 minted = swfNFT.minted();
    uint256 maxTokens = swfNFT.getMaxTokens();
    uint256 safeMintCost = mintCost(minted, maxTokens);
    require(safeMintCost > 0, "Sacrificial alter currently closed");
    require(safeAmt >= safeMintCost, "Not enough safe given");
    safeToken.burn(_msgSender(), safeAmt);
    if(safeAmt < safeMintCost * 2) {
  ...
  function makeTreasureChests(uint16 qty) external whenNotPaused {
    ...
    // $SAFE exchange amount handled within alter contract
    // Will fail if sender doesn't have enough $SAFE
  ...
  function sacrifice(uint256 tokenId, uint256 safeAmt) external whenNotPaused nonReentrant {
    require(tx.origin == _msgSender(), "Only EOA");
    ...
    ISWF.HumansVariants memory nft = swfNFT.getTokenTraits(tokenId);
    uint16 minted = swfNFT.minted();
    uint256 maxTokens = swfNFT.getMaxTokens();
    uint256 safeMintCost = mintCost(minted, maxTokens);
    require(safeMintCost > 0, "Sacrificial alter currently closed");
    if(nft.isHuman) {
      // Human sacrifice requires 3x $SAFE curve
      require(safeAmt >= safeMintCost * 3, "not enough safe provided");
      safeToken.burn(_msgSender(), safeAmt);
      ...
      // Variant sacrifice requires 4x $SAFE curve
      require(safeAmt >= safeMintCost * 4, "not enough safe provided");
      safeToken.burn(_msgSender(), safeAmt);
      // This will check if origin is the owner of the token
      swfNFT.burn(tokenId);
   ...
   * the remaining 80% have a 10% chance to be given to a random staked variant
   * @param seed a random value to select a recipient from
   * @return the address of the recipient (either the minter or the Variant thief's owner)
   */
  function selectRecipient(uint256 seed) internal view returns (address) {
    if (((seed >> 245) % 10) != 0) return _msgSender(); // top 10 bits haven't been used
    address thief = safezone.randomVariantOwner(seed >> 144); // 144 bits reserved for trait selection
  ...
  function setMaxSafeCost(uint256 _amount) external requireContractsSet onlyOwner {
    maxSafeCost = _amount;
  } 
