...
contract EnsMapper is Ownable {
    ...
    function testHash(string memory label, string memory domain) public pure returns (bytes32 big_hash) {
            bytes32 encoded_label = keccak256(abi.encodePacked(label));
            bytes32 encoded_hash = keccak256(abi.encodePacked(domain));
            big_hash = keccak256(abi.encodePacked(encoded_hash, encoded_label));
    }
