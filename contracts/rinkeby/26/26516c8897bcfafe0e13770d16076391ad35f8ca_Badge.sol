...
contract Badge is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint256 private costUserTotalSupply = 0;

    uint256 public maxTotalSupply = 7999;

    uint256 public addOne1 = 0;

    uint256 public addOne2 = 0;

    uint256 public addOne3 = 0;

    uint256 public addOne4 = 0;

    uint[] usedTokenIdArr;

    //Loot Contract 0xFF9C1b15B16263C61d017ee9F65C50e4AE0113D7
    address public lootAddress = 0x60b6f91e3AEdfEB5FA9040597670F7a1DfAB1713;
    LootInterface lootContract = LootInterface(lootAddress);

    //Whitelist Contract 0xF2A8F7ebD1e32D823534ebCAb91883C509E9fe1b
    address public whitelistAddress = 0xF2A8F7ebD1e32D823534ebCAb91883C509E9fe1b;
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[11] = getTrait1(tokenId);

        parts[12] = '  ';

        parts[13] = attrsIndex[tokenId].addOne2 == 1 ? '+1' : '';

        parts[14] = '</text><text x="10" y="120" class="base">';

        parts[15] = getTrait2(tokenId);

        parts[16] = '  ';

        parts[17] = attrsIndex[tokenId].addOne3 == 1 ? '+1' : '';

        parts[18] = '</text><text x="10" y="140" class="base">';

        parts[19] = getTrait3(tokenId);

        parts[20] = '  ';

        parts[21] = attrsIndex[tokenId].addOne4 == 1 ? '+1' : '';
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Badge #', toString(tokenId), '", "description": "Badge is a seed, a seed account for a PVP Metaverse Game.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
...
//    }
    ...
    function randLevel(uint8 num) public payable returns (uint8) {
        uint8 maxNum = 0;
        if (num <= maxNum && addOne4 < maxTotalSupply / 100 *  1)
        {
            addOne4++;
            return 4;
        }
        maxNum += 3;
        if (num <= maxNum && addOne3 < maxTotalSupply / 100 * 3)
        {
            addOne3++;
            return 3;
        }
        maxNum += 5;
        if (num <= maxNum && addOne2 < maxTotalSupply / 100 * 5)
        {
            addOne2++;
            return 2;
        }
        maxNum += 10;
        if (num <= maxNum && addOne1 < maxTotalSupply / 100 * 10)
        {
            addOne1++;
            return 1;
        }
    ...
    function randAddOneArr() public payable returns (uint8[] memory){
    ...
    {
        return whitelistContract.checkExistWhitelist(addr);
    }
    ...
    function checkTokenIdAvailable(uint256 tokenId,address addr) public view returns (bool)
    {
        ...
        if(result)
        {
            if(tokenId>=1 && tokenId<=999 && !whitelistContract.checkExistWhitelist(addr))//whitelist
            {
                result=false;
            }
    ...
    function claimWithWhitelist(uint256 tokenId, uint256 genderId, uint256 raceId, uint256 elementId, uint256 strNum, uint256 staNum, uint256 agiNum, uint256 chaNum, uint256 intNum, uint256 shiNum) public nonReentrant {
        ...
        require(whitelistContract.checkExistWhitelist(_msgSender()), "Not in whitelist");
        whitelistContract.updateWhitelist(_msgSender());
