...
// todo public private vars, comments 
...
contract Cats is ERC721, Ownable {
    ...
    //purchase multiple cats at once
    function purchaseCats(uint256 _howMany) public payable {
        require(_howMany > 0, "Minimum 1 tokens need to be minted");
        require(
            _howMany <= catsRemainingToBeMinted(),
            "Purchase amount is greater than the token available"
        );
        require(isSaleActive, "Sale is not active");
        require(_howMany <= 20, "max 20 cats at once");
    ...
    function _mintCat(address _to) private {
        if (purchasedCats == 0) {
            _lastIpfsId = random(
                1,
                totalTokenToMint,
        ...
        purchasedCats++;
        require(!_exists(purchasedCats), "Mint: Token already exist.");
        _mint(_to, purchasedCats);
        _setTokenURI(purchasedCats, _lastIpfsId);
    }

    // RESERVE FUNCTIONS
    function gift(address[] calldata to) external onlyOwner {
        require(
            to.length <= catsRemainingToBeMinted(),
    ...
    function catsRemainingToBeMinted() public view returns (uint256) {
        return totalTokenToMint - purchasedCats;
    }

    function isAllTokenMinted() public view returns (bool) {
        return purchasedCats == totalTokenToMint;
    }

    function getIpfsIdToMint() public view returns (uint256 _nextIpfsId) {
        require(!isAllTokenMinted(), "All tokens have been minted");
        if (
            _lastIpfsId == totalTokenToMint && purchasedCats < totalTokenToMint
        ) {
            _nextIpfsId = 1;
        } else if (purchasedCats < totalTokenToMint) {
