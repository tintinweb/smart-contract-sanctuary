...
contract StarlightTemple is Initializable, StarlightTempleContracts {
    ...
    function setTempleSettings(
        uint256 _chanceLegionMax1Constellation,
        uint256 _chanceLegionMaxAllConstellations,
        uint8 _maxConstellationRank,
        uint256 _starlightTreasureId,
        uint256[] calldata _currentRankToNeededStarlight)
    external onlyAdminOrOwner
    {
        require(_chanceLegionMax1Constellation + _chanceLegionMaxAllConstellations <= 100000, "Bad odds");
        require(_currentRankToNeededStarlight.length == _maxConstellationRank, "Starlight length must match max");
        chanceLegionMax1Constellation = _chanceLegionMax1Constellation;
        chanceLegionMaxAllConstellations = _chanceLegionMaxAllConstellations;
        maxConstellationRank = _maxConstellationRank;
        starlightTreasureId = _starlightTreasureId;
    ...
    function onLegionCreated(uint256 _tokenId, uint256 _randomNumber) external onlyAdminOrOwner whenNotPaused {
        uint256 _resultForConstellation = _randomNumber % 100000;

        if(_resultForConstellation < chanceLegionMax1Constellation) {

            uint256 _numConstellations = numberOfConstellations();

            Constellation _randomConstellation = Constellation(_randomNumber % _numConstellations);

            legionMetadataStore.increaseConstellationRank(_tokenId, _randomConstellation, maxConstellationRank);
        } else if(_resultForConstellation < chanceLegionMax1Constellation + chanceLegionMaxAllConstellations) {
            Constellation[6] memory _allConstellations = allConstellations();
            for(uint256 i = 0; i < _allConstellations.length; i++) {
                legionMetadataStore.increaseConstellationRank(_tokenId, _allConstellations[i], maxConstellationRank);
            }
        }
    }
    ...
    onlyEOA
    whenNotPaused {
        require(starlightTreasureId > 0, "Bad starlight ID");
        ...
        treasure.safeTransferFrom(msg.sender, address(treasury), starlightTreasureId, _cost, "");
    }
...
abstract contract StarlightTempleState is Initializable, IStarlightTemple, ERC1155HolderUpgradeable, AdminableUpgradeable {
    ...
    // Chance out of 100,000. Chance on summon/pilgrimage.
    uint256 public chanceLegionMax1Constellation;
    uint256 public chanceLegionMaxAllConstellations;

    uint8 public maxConstellationRank;
    uint256[] public currentRankToNeededStarlight;
    uint256 public starlightTreasureId;

    function __StarlightTempleState_init() internal initializer {
        AdminableUpgradeable.__Adminable_init();
        ERC1155HolderUpgradeable.__ERC1155Holder_init();

        chanceLegionMax1Constellation = 10;
        chanceLegionMaxAllConstellations = 1;
...
interface IStarlightTemple {
    // Occurs when a legion (721) is created via summoning or a pilgrimage. This will run the
    // odds on if they start with 0, 1, or all constellations maxed out.
    // Admin only.
    function onLegionCreated(uint256 _tokenId, uint256 _randomNumber) external;
}
