...
contract PickRandomWinner is VRFConsumerBase, Ownable {
    ...
    event RequestWinner(string urlHash, bytes32 requestId);
    event ReturnWinner(string urlHash, uint256 result);
    event VerificationAddresses(address recoveredSig, address verifier);
    ...
    constructor(bytes32 keyHash, uint256 fee, address coordinator, address linkToken, address signedVerifier) VRFConsumerBase(coordinator, linkToken) {
		privKeyHash = keyHash; 
		privFee = fee; 
    ...
    function requestWinnerSigningHash(uint256 blockExpiry, uint256 documentId, uint256 totalSigners) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(blockExpiry, documentId, totalSigners));
    }

    // Set this to internal once done testing
    function verify(bytes memory signature, uint256 blockExpiry, uint256 documentId, uint256 totalSigners) public returns (bool) {
        require(block.number < blockExpiry, "Signature has expired!");

        bytes32 signingHash = requestWinnerSigningHash(blockExpiry, documentId, totalSigners).toEthSignedMessageHash();
        address recoveredSig = ECDSA.recover(signingHash, signature);

        emit VerificationAddresses(recoveredSig, privSignedVerifier);
    ...
    function requestWinner(bytes memory signature, uint256 blockExpiry, uint256 documentId, string memory urlHash, uint256 totalSigners) public {
        require(verify(signature, blockExpiry, documentId, totalSigners) == true, "Verification for signer failed");
        ...
        emit RequestWinner(urlHash, requestId);
    }
