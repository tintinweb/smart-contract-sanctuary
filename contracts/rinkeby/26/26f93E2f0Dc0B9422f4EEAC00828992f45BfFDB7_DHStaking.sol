...
contract DHStaking is ERC721, ERC721URIStorage, Ownable {
    ...
    IShowBiz _showBiz = IShowBiz(0x74D5BB5A1e10EF0d2C3f62357de9183ed0A7af78);
    ...
    mapping(address => mapping(uint => StakedToken)) localTokenIdToStakedToken;
    mapping(address => mapping(address => EnumerableSet.UintSet)) addressToStakedTokensSet;
    mapping(address => mapping(uint => uint)) tokenIdToClaimedRewards;
    mapping(address => mapping(uint => uint)) tokenIdToLocalTokenId;
    ...
    constructor() ERC721("Staked DeadHeads", "sDEAD") {
        contracts[0xC3F78f50222b3CAB2eD2F73D07Ed28131d1EBAA3] = PartnerContract(
            true,
            IERC721(0xC3F78f50222b3CAB2eD2F73D07Ed28131d1EBAA3),
            "",
            1 ether
        );
    }
    ...
    function onERC721Received(address operator, address from, uint256 tokenId, bytes calldata) external returns(bytes4) {
        uint localTokenId = tokenIdToLocalTokenId[operator][tokenId];
        require(localTokenIdToStakedToken[operator][localTokenId].owner == from, "token must be staked over stake method");
        return bytes4(keccak256("onERC721Received(address,address,uint256,bytes)"));
    }
    
    function stake(address contractAddress, uint tokenId, uint months) external {
        ...
        localTokenIdToStakedToken[contractAddress][_tokenIdCounter.current()] = StakedToken({
            tokenId: tokenId,
            stakedAt: block.timestamp - 7 days + 15, // TODO: remove -7 days + 15
            endsAt: endsAt,
    ...
    function unclaimedRewards(uint tokenId) public view returns (uint) {
        PartnerContract storage _contract = contracts[localTokenIdToContract[tokenId]];
        
        StakedToken storage stakedToken = localTokenIdToStakedToken[localTokenIdToContract[tokenId]][tokenId];
        ...
        return (rewardsUntilNow > rewardLimit ? rewardLimit : rewardsUntilNow)
            * _contract.stakeReward * stakedToken.rewardMultiplier / 100 / 4 - tokenIdToClaimedRewards[localTokenIdToContract[tokenId]][tokenId];
    }
    
    function claimTokenRewards(uint tokenId) public {
        require(localTokenIdToStakedToken[localTokenIdToContract[tokenId]][tokenId].owner == msg.sender, "caller did not stake this token");
        
        uint _unclaimedRewards = unclaimedRewards(tokenId);
        
        if (_unclaimedRewards > 0) {
            _showBiz.mint(msg.sender, _unclaimedRewards);
            tokenIdToClaimedRewards[localTokenIdToContract[tokenId]][tokenId] += _unclaimedRewards;
    ...
    function unstake(uint tokenId) external {
        ...
        delete localTokenIdToStakedToken[localTokenIdToContract[tokenId]][tokenId];
        addressToStakedTokensSet[localTokenIdToContract[tokenId]][msg.sender].remove(tokenId);
    ...
    function tokenURI(uint256 localTokenId) public view override(ERC721, ERC721URIStorage) returns (string memory) {
        ...
        PartnerContract storage _contract = contracts[localTokenIdToContract[localTokenId]];
        uint tokenId = localTokenIdToStakedToken[localTokenIdToContract[localTokenId]][localTokenId].tokenId;
        return string(abi.encodePacked(_contract.baseURI, tokenId));
    }
    
    function addContract(address contractAddress, string calldata baseURI, uint stakeReward) public onlyOwner {
    ...
    function updateContract(address contractAddress, bool active, uint stakeReward) public onlyOwner {
        contracts[contractAddress].active = active;
        contracts[contractAddress].stakeReward = stakeReward;
    }
