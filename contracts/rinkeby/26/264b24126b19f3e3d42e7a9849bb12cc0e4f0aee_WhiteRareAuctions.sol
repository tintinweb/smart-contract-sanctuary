...
// File contracts/interfaces/IAuctionHouse.sol
 ...
 */
contract WhiteRareAuctions is IAuctionHouse, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // The address of the WhiteRareNFT  to use via this contract
    address public whiterarenft;
    ...
    mapping(uint256 => uint256[]) public bulkAuc;
    
    mapping(uint256 => IAuctionHouse.bulkAucts) public indexer;
    ...
    Counters.Counter private _bulkIdTracker;
     ...
     * Constructor
     */
    constructor(address _whiterarenft, address _weth) public {
        require(
            IERC165(_whiterarenft).supportsInterface(interfaceId),
            "Doesn't support NFT interface"
        );
        whiterarenft = _whiterarenft;
        wethAddress = _weth;
    ...
    function getCurrentBulkID() public view returns(uint256){
        return _bulkIdTracker.current();
    }
    ...
    function createBulkAuc(
        ...
        address payable curator,
        uint8 curatorFeePercentage,
        address auctionCurrency) public  payable returns (uint256){
        uint256 bulkAucId = _bulkIdTracker.current();
        uint256 auctId = 0;
        uint256 tok = 0;
        for (uint256 ct = 0; ct < tokenId.length; ct++){
            auctId = createFixedSaleAuction(tokenId[ct], tokenContract, duration, reservePrice, curator, curatorFeePercentage, auctionCurrency);
            bulkAuc[ct].push(auctId);
            
            tok = tokenId[ct];
            
            indexer[bulkAucId].auctionIds.push(auctId);
            indexer[bulkAucId].tokenIds.push(tok);
            
           size += 1;
        }
        
        indexer[bulkAucId].bulkAuctionId = bulkAucId;
        indexer[bulkAucId].status = true;
         
        _bulkIdTracker.increment();
    ...
    nonReentrant
    {
        ...
        // For WhiteRareNFT, ensure that the bid is valid for the current bidShare configuration
        if(auctions[auctionId].tokenContract == whiterarenft) {
            require(
                IMarket(IMediaExtended(whiterarenft).marketContract()).isValidBid(
    ...
    /**
     * @notice End an auction, finalizing the bid on WhiteRareNFT if applicable and paying out the respective parties.
    ...
    function endAuction(uint256 auctionId) external override auctionExists(auctionId) nonReentrant {
        ...
        if(auctions[auctionId].tokenContract == whiterarenft) {
            // If the auction is running on whiterare, settle it on the protocol
            (bool success, uint256 remainingProfit) = _handleWhiteRareAuctionSettlement(auctionId);
    ...
    function _handleFixedSale(address to, uint256 amount, address currency) internal {
        if(currency == address(0)){
    ...
    function _handleWhiteRareAuctionSettlement(uint256 auctionId) internal returns (bool, uint256) {
        address currency = auctions[auctionId].auctionCurrency == address(0) ? wethAddress : auctions[auctionId].auctionCurrency;
        ...
        IERC20(currency).approve(IMediaExtended(whiterarenft).marketContract(), bid.amount);
        IMedia(whiterarenft).setBid(auctions[auctionId].tokenId, bid);
        uint256 beforeBalance = IERC20(currency).balanceOf(address(this));
        try IMedia(whiterarenft).acceptBid(auctions[auctionId].tokenId, bid) {} catch {
            // If the underlying NFT transfer here fails, we should cancel the auction and refund the winner
            IMediaExtended(whiterarenft).removeBid(auctions[auctionId].tokenId);
            return (false, 0);
        }
    ...
    function createFixedSaleAuction(
        uint256 tokenId,
        address tokenContract,
        uint256 duration,
        uint256 reservePrice,
    ...
    function fixedBuy(uint256 auctionId, uint256 amount)
    external
    payable
    auctionExists(auctionId)
    nonReentrant
    {
        ...
        curatorFee = tokenOwnerProfit.mul(auctions[auctionId].curatorFeePercentage).div(100);
        tokenOwnerProfit = tokenOwnerProfit.sub(curatorFee);
        _handleFixedSale(msg.sender, amount, auctions[auctionId].auctionCurrency);
        IERC721(auctions[auctionId].tokenContract).safeTransferFrom(address(this), msg.sender, auctions[auctionId].tokenId);
     ...
     function fixedBulkBuy(
        uint256 bulkAuctionId,
        uint256[] calldata tokenId,
        uint256[] calldata amount
      ) external  payable {
         require(indexer[bulkAuctionId].status == true, "The bulk auction doesn't exist");
         require(indexer[bulkAuctionId].tokenIds.length >= tokenId.length, "The tokens exceed maximum available in the bulk sale auction");
         require(tokenId.length == amount.length, "The token array must match the length of the amount array");
         
        uint256[] memory tokens = indexer[bulkAuctionId].tokenIds;
        uint256[] memory aucts = indexer[bulkAuctionId].auctionIds;
        ...
        for (uint256 ct = 0; ct < tokenId.length; ct++){
            count = ct;
            currAmount = amount[count];
            for (uint256 cnt = 0; cnt < tokens.length; cnt++){
                counter = cnt;
                if (tokens[counter] == tokenId[count]){
                    auctionCount = aucts[counter];
                    this.fixedBuy(auctionCount, currAmount);
                    delete indexer[bulkAuctionId].tokenIds[counter];
                    delete indexer[bulkAuctionId].auctionIds[counter];
   ...
   function availableBulkAuctionTokens(uint256 bulkAuctionId) public view returns(uint256[] memory){
