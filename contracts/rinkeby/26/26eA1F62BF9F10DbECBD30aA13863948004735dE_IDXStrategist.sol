...
contract IDXStrategist
{
    ...
    function UpdateVaultValue(address vaultAddress) public {
        COMPVault memory vault = vaults[vaultsIds[vaultAddress]];
        vault.logic.updateRate();
    }
...
/**
        IDX Digital Labs Strategist Vault.
        Author: Ian Decentralize
 */
    ...
    AccessControl
{
    ...
    function updateRate() public {
        ...
        if (debt){
            CErc20(borrowedCollateral).borrowBalanceCurrent(address(this)); 
       }
    }
    ...
    function getFees() external {
        ...
        if(farming == ETHER){
            CEther cToken = CEther(collateral);
            cToken.transfer(strategist, collaterals[strategist]);
        }else{
              CErc20 cToken = CErc20(collateral);  
              cToken.transfer(strategist, collaterals[strategist]);
        }       
