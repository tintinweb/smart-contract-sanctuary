...
import "../contracts/MainchainGatewayStorage.sol";
import "../contracts/Proxy.sol";

contract MainchainGatewayProxy is Proxy, MainchainGatewayStorage {
    constructor(address _proxyFor, address _registry) Proxy(_proxyFor) {
...
abstract contract MainchainGatewayStorage is ProxyStorage, Pausable {
...
abstract contract Proxy is ProxyStorage {
    constructor(address _proxyFor) {
        setProxyFor(_proxyFor);
    }
...
abstract contract ProxyStorage is Ownable {
    address public proxyFor;

    event ProxyUpdated(address indexed _oldContract, address indexed _newContract);

    function setProxyFor(address _newContract) internal onlyOwner {
        address oldContract = proxyFor;
        proxyFor = _newContract;

        emit ProxyUpdated(oldContract, _newContract);
    }
}
...
contract Registry is Ownable {
    ...
    function mapToken(address _mainchainToken, address _sidechainToken, uint32 _standard) external onlyOwner {
    ...
    {
        delete mainchainMap[_mainchainToken];
        delete sidechainMap[_sidechainToken];
    }
