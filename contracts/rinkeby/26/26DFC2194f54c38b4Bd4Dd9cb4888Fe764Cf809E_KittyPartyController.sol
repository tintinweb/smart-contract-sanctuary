...
contract KittyPartyController is KittyPartyStateTransition, IKittenPartyInit {
    ...
    function applyWinnerStrategy() public timedTransitions() atStage(KittyPartyStages.Payout){
        ...
        console.log("The amount to split amongst amountToSendKreator and DAO %i", profit);
        ...
        console.log("Proceeding to send balance to winners %i", amountToSend);
    ...
    function mintValue(address mintTo, uint256 amount) private {
        ...
        (bool success,) = address(accountantContract).call(payload);
                    console.log("calling account contract status %b", success);
...
enum KittyPartyStages {
    ...
    uint public lastStageTime;
    uint8 public durationInDays;
    uint256 public currentRound = 1;
    uint256 public activityDuration;
    ...
    modifier timedTransitions() {
        if (stage == KittyPartyStages.InitialCollection && (block.timestamp >= (lastStageTime + (activityDuration * 1 minutes)))) {
           nextStage(2);
        }
        if (stage == KittyPartyStages.Collection && block.timestamp >= (lastStageTime + (activityDuration * 1 minutes))) {
            nextStage(1);
        }
        if (stage == KittyPartyStages.Staking && block.timestamp >= (lastStageTime + (durationInDays * 1 minutes))) {
            nextStage(1);
        }
        if (stage == KittyPartyStages.Payout && block.timestamp >= (lastStageTime + (durationInDays * 1 minutes)) && (numberOfRounds >= currentRound)) {
...
//Check if rounds have been force ended or if rounds are finished and then close else go back to Collection phase
    function nextStage(uint8 jumpTo) internal {
