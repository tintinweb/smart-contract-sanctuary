...
contract TOKordinator is Ownable, ReentrancyGuard, Multicall {
    ...
    // IUniswapV3Factory internal uniswapV3;
    ...
    function multiSwap(
        ...
        IERC20 destToken = IERC20(
            path[lastNonZeroIndex][path[lastNonZeroIndex].length - 1]
        );
    ...
    function formatPath(IERC20[] memory path)
        public
        view
        returns (IERC20[] memory realPath)
    {
    ...
    function swapOnUniswapV3(
        ...
        if (inputIsETH) {
            require(
                msg.value >= amountIn,
                "TOKordinator: value does not enough"
            );
        }
