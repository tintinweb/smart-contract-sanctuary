...
contract Dog is ERC721, ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function preserveMint(uint numDogs, address to) public onlyOwner {
    require(totalSupply() + numDogs <= tierSupply, 'Preserve mint would exceed tier supply');
    require(totalSupply() + numDogs <= MAX_SUPPLY, 'Preserve mint would exceed max supply');
    _mintDog(numDogs, to);
  ...
  function getDogByOwner(address _owner) public view returns (uint256[] memory) {
  ...
  function mintDogs(uint numDogs) public payable {
    require(_isSaleActive, 'Sale must be active to mint Dogs');
    require(totalSupply() + numDogs <= tierSupply, 'Sale would exceed tier supply');
    require(totalSupply() + numDogs <= MAX_SUPPLY, 'Sale would exceed max supply');
    require(balanceOf(msg.sender) + numDogs <= maxBalance, 'Sale would exceed max balance');
    require(numDogs <= maxMint, 'Sale would exceed max mint');
    require(numDogs * mintPrice <= msg.value, 'Not enough ether sent');
    _mintDog(numDogs, msg.sender);
    emit TokenMinted(totalSupply());
  }

  function whiteListMintDogs(uint numDogs) public payable {
    require(_isWhiteListSaleActive, 'Sale must be active to mint Dogs');
    require(totalSupply() + numDogs <= tierSupply, 'Sale would exceed tier supply');
    require(totalSupply() + numDogs <= MAX_SUPPLY, 'Sale would exceed max supply');
    require(balanceOf(msg.sender) + numDogs <= maxBalance, 'Sale would exceed max balance');
    require(numDogs <= maxMint, 'Sale would exceed max mint');
    ...
    require(numDogs * price <= msg.value, 'Not enough ether sent');
    _mintDog(numDogs, msg.sender);
    emit TokenMinted(totalSupply());
  }

  function auctionMintDog(uint numDogs) public payable {
    require(_isAuctionActive, 'Auction must be active to mint Dogs');
    require(block.timestamp >= auctionStartTime, 'Auction not start');
    require(totalSupply() + numDogs <= tierSupply, 'Auction would exceed tier supply');
    require(totalSupply() + numDogs <= MAX_SUPPLY, 'Auction would exceed max supply');
    require(balanceOf(msg.sender) + numDogs <= maxBalance, 'Auction would exceed max balance');
    require(numDogs <= maxMint, 'Auction would exceed max mint');
    require(numDogs * getAuctionPrice() <= msg.value, 'Not enough ether sent');
    _mintDog(numDogs, msg.sender);
    emit TokenMinted(totalSupply());
  }

  function _mintDog(uint256 numDogs, address recipient) internal {
    uint256 supply = totalSupply();
    for (uint256 i = 0; i < numDogs; i++) {
