...
contract BBAAEEDD is ERC721, ERC721Enumerable, Ownable, ERC721Burnable {

    using Counters for Counters.Counter;
    using Strings for uint256;

    Counters.Counter private _tokenIdCounter;
    uint256 private  tokenPrice = 800000000000000; // 0.0008 ETH
    uint private constant maxTokenPurchase = 5;
    uint256 public maxSupply = 30;
    uint256 public maxAirdropAmount = 5;
    ...
    address main2Addr = 0x63387DdB4000b6B3fbf534AE8480585d66F49365;
    
    // WhiteLists for presale.
    mapping(address => uint256) public whiteList;

    // Array for random minting
    uint256[] public fullArray = new uint256[](maxSupply);
    uint256[] public randomArray; 
    uint256[] public airdropArray = new uint256[](maxAirdropAmount);
    
    uint public airdropCount;
 
    constructor(string memory baseURI, string memory _notrevealURI) ERC721("BBAAEEDD", "BBAAEEDD") { 
    ...
    function addWhiteList(address[] calldata addresses, uint8 numAllowedToMint) public onlyOwner {
    ...
    function addAirdropList(uint256[] calldata indexes) public onlyOwner {
        ...
        airdropAmount = indexes.length;
        for (uint256 i = 0; i < airdropAmount; i++) {
            airdropArray[i] = indexes[i];
            fullArray[indexes[i] - 1] = 1; 
        }

        randomArray = new uint256[](maxSupply - airdropAmount);
    ...
    function startPresale() external onlyOwner{
        require(presaleActive == false, "Presale is already started");
        presaleActive = true;
    }

    function stopPresale() external onlyOwner{
        require(presaleActive == true, "Presale is already stopped");
        presaleActive = false;
    }
    ...
    function stopSale() external onlyOwner{
        require(saleIsActive == true, "Sale is already stopped");
        saleIsActive = false;
    }
    ...
    function airdropMint() external onlyOwner{
        for(uint256 i = 0; i < airdropArray.length; i++)
        {
            _tokenIdCounter.increment();
    ...
    function withdraw() external onlyOwner {
        uint256 balance = address(this).balance;
        require(balance > 0);
        payable(mainAddr).transfer((balance * 975) / 1000); //97.5%
        payable(main2Addr).transfer((balance * 25) / 1000); //2.5%
