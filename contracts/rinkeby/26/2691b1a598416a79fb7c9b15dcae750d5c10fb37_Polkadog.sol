...
contract Polkadog is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    uint256 public _iTotal = 10 * 10**18;
    uint256 public _rTotal = (MAX - (MAX % _tTotal));
    uint256 public _riTotal = (MAX - (MAX % _iTotal));
    ...
    constructor () {
        ...
        emit Transfer(address(0), _msgSender(), _iTotal);
    }
     ...
     * @dev See {IERC20-balanceOf}.
     */
    function bal(address account) public view virtual returns (uint256) {
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) external view returns(uint256) {
        require(tAmount <= _iTotal, "Amount must be less than supply");
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        _maxTxAmount = _iTotal.mul(maxTxPercent).div(
            10**2
        );
    }
    ...
    function _getCurrentSupply() private view returns(uint256, uint256) {
        uint256 rSupply = _riTotal;
        uint256 tSupply = _iTotal;      
        for (uint256 i = 0; i < _excluded.length; i++) {
            if (_rOwned[_excluded[i]] > rSupply || _tOwned[_excluded[i]] > tSupply) return (_riTotal, _iTotal);
            rSupply = rSupply.sub(_rOwned[_excluded[i]]);
            tSupply = tSupply.sub(_tOwned[_excluded[i]]);
        }
        if (rSupply < _riTotal.div(_iTotal)) return (_riTotal, _iTotal);
