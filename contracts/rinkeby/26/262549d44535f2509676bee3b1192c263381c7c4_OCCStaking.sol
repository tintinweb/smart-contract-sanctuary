...
// File: contracts/OCCStaking.sol
pragma solidity ^0.7.0;




contract OCCStaking is Ownable, ReentrancyGuard {
    ...
    constructor(address _OCC, uint _unstakingFeeRatio, address _assetManager, address _owner, uint emissionStart, uint firstCheckPoint, uint _rewardPerSecond) Ownable(_owner) {
        OCC = IERC20(_OCC);
    ...
    function removeStake(uint stake, uint maximumFee) public nonReentrant updateReward(msg.sender) {
        ...
        if (unstakingFee > 0) {
            totalCollectedFee = totalCollectedFee.add(unstakingFee);
        }
    ...
    function changeUnstakingFeeRatio(uint newUnstakingFeeRatio) public onlyOwner {
        require(newUnstakingFeeRatio <= unstakingFeeDenominator, "OCCStaking: invalid unstaking fee.");
    ...
    function collectFee(address feeDestination, uint amount) public {
        require(msg.sender == assetManager, "OCCStaking: not authorized to collect fee.");
        require(amount <= totalCollectedFee, "OCCStaking: not enough collected fee.");
        totalCollectedFee = totalCollectedFee.sub(amount);
