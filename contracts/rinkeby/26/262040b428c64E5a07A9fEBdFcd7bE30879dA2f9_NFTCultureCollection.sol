...
contract NFTCultureCollectionBase is ERC1155Supply, Ownable {
    ...
    address private nftCult;
    address private nftCultForgeComponents;
    ...
    constructor(
        ...
        nftCult = __nftCult;
        nftCultForgeComponents = __nftCultForgeComponents;
    ...
    function getClaimsRemaining(uint256 nftcultTokenId)
        ...
        returns (uint256)
    {
        uint256 forgeComponent = INFTCultForgeComponents(nftCultForgeComponents)
            .getMapping(INFTCult_V1_Thin(nftCult).tokenURI(nftcultTokenId));

        return
            _cultTokenClaimYield[forgeComponent] -
        ...
        returns (uint256)
    {
        ...
        return
            _cultTokenEmberYield[forgeComponent] -
            _cultTokenEmbersReceived[nftcultTokenId];
    }

    function setMintingState(bool __claimingActive, uint256 __pricePerToken, uint256 __emberPieceId)
        external
        onlyOwner
    {
    ...
    {
        require(
            balanceOf(msg.sender, pieceId) == 0,
            'Cant add to collection'
        );
    ...
    function claimPiecesInCollection(
        ...
        require(
            INFTCult_V1_Thin(nftCult).ownerOf(nftcultTokenId) == msg.sender,
