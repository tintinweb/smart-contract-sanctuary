...
contract ERC721Sale is ReentrancyGuard {
    ...
    function buy(
        ...
        HasPrice hasPrice = HasPrice(address(token));
        uint256 tokenPrice = hasPrice.getPrice(tokenId);

        ikonicTokenContract.transferFrom(msg.sender, address(this), tokenPrice);
        
        uint256 price = tokenPrice.mul(10000).div(serviceFeeProxy.getBuyServiceFeeBps(msg.sender).add(10000));
    ...
    function transferEther(
        ...
        uint256 price = tokenPrice.mul(10000).div(buyerServiceFeeBps.add(10000));
        uint256 sellerServiceFee = price.mul(sellerServiceFeeBps).div(10000);
        uint256 ownerValue = price.sub(sellerServiceFee);
        uint256 sumFee;

        if (token.supportsInterface(_INTERFACE_ID_FEES)) {
            HasSecondarySaleFees withFees = HasSecondarySaleFees(address(token));
            address[] memory recipients = withFees.getFeeRecipients(tokenId);
            ...
            for (uint256 i = 0; i < fees.length; i++) {
                ...
                ikonicTokenContract.transfer(recipients[i], current);
                sumFee = sumFee.add(current);
            }
        }
        ownerValue = ownerValue.sub(sumFee);
        ikonicTokenContract.transfer(serviceFeeRecipient, sellerServiceFee.add(tokenPrice.sub(price)));
        ikonicTokenContract.transfer(owner, ownerValue);
    }
