    ...
    Ownable
{
    ...
    function getVotedBuyPrice(uint punkIndex) public view returns(uint) {
    ...
    function voteSellPrice(uint256 punkIndex, uint256 price) public {
        ...
        require(senderBalance > 0, "balance is 0");
        ...
        if (vote.price > 0) {
            prePriceSum.priceWeightSum = prePriceSum.priceWeightSum.sub(
                vote.price.mul(senderBalance)
            );
            prePriceSum.weightSum = prePriceSum.weightSum.sub(senderBalance);
        }
        ...
        if (price > 0) {
            ...
            newPriceSum.priceWeightSum = newPriceSum.priceWeightSum.add(
                price.mul(senderBalance)
            );
            newPriceSum.weightSum = newPriceSum.weightSum.add(senderBalance);
        }
    }

    function getVotedSellPrice(uint punkIndex) public view returns(uint) {
