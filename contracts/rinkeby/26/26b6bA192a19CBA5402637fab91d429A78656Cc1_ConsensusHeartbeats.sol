...
// SPDX-License-Identifier: MIT 
    ...
    constructor() ERC721("Con Her2", "CH") {}
            
    using SafeMath for uint256;
    // price of a CH
        uint256 public constant heartbeatPrice = 330000000000000000; // 0.33 ETH

    // max number of CH to purchase
        uint public constant MAX_HEARTBEAT_PURCHASE = 1;
    
    // max number of CH overall
        uint256 public constant MAX_HEARTBEATS_MAIN = 313;

    // event for setting block information
        event tokenInformation (uint256 indexed nameIndex, string blockInformation);        
    ...
    // reconstructed code
        ...
        function setTokenInformation(uint256 _tokenId, string calldata _currName) public {
            address owner = ownerOf(_tokenId);
            require(msg.sender == owner, "not the owner"); 
            heartbeatBlockInformation[_tokenId] = _currName; 
            emit tokenInformation(_tokenId, _currName); // user can set any name. // block_totgas_avggas_tottx_freqshum_freqsmain
        }
