...
struct Camps {
    ...
    uint256 public REGEN_TIME = 24 seconds; //CHANGE!!
    uint256 public HEALTHBONUS_TIME = 8 minutes;
    
    uint[6] public DISTRIBUTION = [0,40,25,20,10,5];
...
function initialize(address _elfcontract) public onlyOwner {
    ...
    require(!initialized, "Contract instance has already been initialized");
      ...
      camps[1] = Camps({baseRewards: 20, creatureCount: 3000, creatureHealth: 8,  expPoints:3,  minLevel:1, itemDrop: 100, weaponDrop: 100, spare: 0});
      camps[2] = Camps({baseRewards: 20, creatureCount: 3000, creatureHealth: 16, expPoints:3,  minLevel:1, itemDrop: 100, weaponDrop: 100, spare: 0});
      camps[3] = Camps({baseRewards: 20, creatureCount: 3000, creatureHealth: 32, expPoints:3,  minLevel:1, itemDrop: 100, weaponDrop: 100, spare: 0});
...
returns(uint256 level, uint256 rewards, uint timestamp){
  ...
  require((DISTRIBUTION[_sector] * camp.creatureCount / 100) > 0);
  
  camps[_campId].creatureCount = camp.creatureCount - 1;

  rewards = camp.baseRewards + (_level.mul(75).div(1000)) + (_sector.mul(250).div(100));
  rewards = rewards.mul(1 ether);

  uint healthPoints = _level.div(3).add(2);
  uint attackPoints;

  if(_useItem){
         attackPoints = _inventory == 1 ? _attackPoints.mul(2)   : _attackPoints;
         healthPoints = _inventory == 2 ? healthPoints.mul(2)    : healthPoints; 
         rewards      = _inventory == 3 ? rewards.mul(2)         : rewards;
         level        = _inventory == 4 ? _level.add(2)          : _level;
         healthPoints = _inventory == 5 ? healthPoints.add(300)  : healthPoints; 
         attackPoints = _inventory == 6 ? _attackPoints.mul(3)   : _attackPoints;
  }

  level = level < MAX_LEVEL ? level.add(uint256(camp.expPoints).div(3)) : level;

                             
  uint256 creatureHealth =  _sector * camp.creatureHealth * _campId;

  healthPoints = healthPoints > 0 ? healthPoints.mul(HEALTHBONUS_TIME) : 0;
  uint attackTime = creatureHealth.div(attackPoints);
  
  attackTime = attackTime > 0 ? attackTime.mul(TIME_CONSTANT) : 1;
  
  uint regenTime = REGEN_TIME > healthPoints ? timestamp.add(REGEN_TIME).sub(healthPoints) : 0;
    
    if(regenTime !=0){
      timestamp = _sentinelClass == 0 ? regenTime.div(3).mul(2) : _sentinelClass == 1 ? regenTime.div(4).mul(3) : regenTime;
    }
        
    timestamp = regenTime.add(block.timestamp).add(attackTime);
...
function addCamp(uint256 id, uint16 baseRewards_, uint16 creatureCount_, uint16 expPoints_, uint16 creatureHealth_, uint16 minLevel_, uint16 itemDrop_, uint16 weaponDrop_) external onlyOwner     
    {
...
function rollWeapons(uint256 level_, uint256 sectorIndex_) public view returns (uint256 newWeaponTier, uint256 newWeapon) {

   uint256 rand  = _rand();

   //select higher of sector or level tier
...
function rollItems(uint256 level_) public view returns (uint256 newInventory) {
    
    uint256 rand  = _rand();
    uint256 levelTier = level_.div(20);
...
function getToken(uint256 character) internal pure returns(Token memory token) {
    ...
    token.campaign       = (uint8(character>>208));
...
function _setElf(
                address owner, uint256 timestamp, uint256 action, uint256 campaign, 
