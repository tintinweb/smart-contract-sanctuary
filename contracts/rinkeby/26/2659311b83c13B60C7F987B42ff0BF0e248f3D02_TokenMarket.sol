...
contract TokenMarket is TokenMarketBase {
    ...
    */
    ...
    function activateLoan(uint256[] memory loanIds, uint256[] memory stableCoinAmounts, bool[] memory _autoSell) public {
        ...
        uint platformFee = loanOffersToken[loanIds[i]].loanAmountInBorrowed.mul(govWorldProtocolRegistry.getGovPlatformFee()).div(100);
    ...
    function getTotalPaybackAmount(uint256 _loanId) external view returns (uint256, uint256, uint256) {
        ...
        uint256 loanTermLengthPassedInDays = (block.timestamp.sub(activatedLoanOffersFull[_loanId].activationLoanTimeStamp)).div(86400);
        ...
        uint256 earnedAPYFee = ((loanDetails.loanAmountInBorrowed.mul(loanDetails.apyOffer).div(100)).div(365)).mul(loanTermLengthPassedInDays);
        uint256 unEarnedAPYFee = apyFeeOriginal.sub(earnedAPYFee); //working here
    ...
    function fullLoanPaybackEarly(uint256 _loanId) internal {
        ...
        (uint256 finalPaybackAmounttoLender,,) = this.getTotalPaybackAmount(_loanId);
        stableCoinAPYFeeFromToken[address(this)] = stableCoinAPYFeeFromToken[address(this)].sub(finalPaybackAmounttoLender);
        ...
        IERC20(loanOffersToken[_loanId].borrowStableCoin).transfer(activatedLoanOffersFull[_loanId].lender, finalPaybackAmounttoLender);
    ...
    //         uint256 autosellFeeinStable = this.getautosellAPYFee(loanDetails.loanAmountInBorrowed, govWorldProtocolRegistry.getAutosellPercentage(), loanDetails.termsLengthInDays);
    ...
    //         uint256 thresholdFee = govWorldProtocolRegistry.getThresholdPercentage();
    ...
    //             uint256 priceofCollateral = this.getAltCoinPriceinStable(loanDetails.borrowStableCoin, loanDetails.stakedCollateralTokens[i], loanDetails.stakedCollateralAmounts[i]);
    ...
    //                 uint256 lastAltcoinValue = this.getAltCoinPriceinStable(loanDetails.stakedCollateralTokens[i], loanDetails.borrowStableCoin, collateralAmountinStable.sub(loanDetails.loanAmountInBorrowed));
    //                 IERC20(loanDetails.stakedCollateralTokens[i]).transfer(lenderDetails.lender, lastAltcoinValue);
    //                 break;
    //                 }
...
abstract contract TokenMarketBase is ITokenMarket {
    ...
    function getautosellAPYFee(uint loanAmount, uint autosellAPY, uint256 loanterminDays) external pure override returns(uint256) {
        // APY Fee Formula
        return ((loanAmount.mul(autosellAPY).div(100)).div(365)).mul(loanterminDays);  
    }  
}
