 ...
 */
contract EchoRFI is RFIBase {

    constructor(
        uint256 launchStartTime,
        uint256 launchVolumeLimitDuration
    ) RFIBase (launchStartTime, launchVolumeLimitDuration) {}
}
...
contract RFIBase is AccessControl, IERC20, VolumeLimiter {
    ...
    // // minimum time interval between fee adjustments by community (set to 15 min for testing)
    uint256 private percentChangeTimespan = 60 * 15;//60 * 60 * 24 * 7 * 4 * 3; // 3 months in seconds

    // will be 3% donation ,2% reflect, 1% burn initially
    uint256 public _redistFee = 0;
    ...
    address payable public charityAddress = payable(0x75311b096Bce494E7441c2A69AdAf33e8905C12B);
    ...
    uint256 private _minTokenExchangeBalance = 12 * 10**4 * 10**18;
    ...
    function setRedistFeePercent(uint256 redistFee) external {
        ...
        require(block.timestamp >= _redistFeeLastChangeDate);//, '[0]'); // Attempting to change value before enough time elapsed
        ...
        _redistFeeLastChangeDate = _redistFeeLastChangeDate.add(percentChangeTimespan);
    ...
    function setBurnFeePercent(uint256 burnFee) external {
        ...
        require(block.timestamp >= _burnFeeLastChangeDate);//, '[T0]'); // Attempting to change value before enough time elapsed
        ...
        _burnFeeLastChangeDate = _burnFeeLastChangeDate.add(percentChangeTimespan);
        _burnFee = burnFee;
    }
    ...
    function setDonationFeePercent(uint256 donationFee) external {
        ...
        require(block.timestamp >= _donationFeeLastChangeDate); //, '[T0]' Attempting to change value before enough time elapsed
        ...
        _donationFeeLastChangeDate = _donationFeeLastChangeDate.add(percentChangeTimespan);
