...
contract Controller is ControllerStorage, ControllerInterface, ControllerErrorReporter, Exponential {
    ...
    function mintAllowed(address pToken, address minter, uint mintAmount) external override returns (uint) {
        ...
        distributeSupplierPie(pToken, minter);
    ...
    function redeemAllowed(address pToken, address redeemer, uint redeemTokens) external override returns (uint) {
        ...
        // Keep the flywheel moving
        updatePieSupplyIndex(pToken);
        distributeSupplierPie(pToken, redeemer);
    ...
    function borrowAllowed(address pToken, address borrower, uint borrowAmount) external override returns (uint) {
        ...
        distributeBorrowerPie(pToken, borrower, borrowIndex);
    ...
    function seizeAllowed(
        ...
        distributeSupplierPie(pTokenCollateral, borrower);
        distributeSupplierPie(pTokenCollateral, liquidator);
    ...
    function transferAllowed(
        ...
        // Keep the flywheel moving
        updatePieSupplyIndex(pToken);
        distributeSupplierPie(pToken, src);
        distributeSupplierPie(pToken, dst);
     ...
     */
    function distributeSupplierPie(address pToken, address supplier) internal {
        PieMarketState storage supplyState = pieSupplyState[pToken];
        ...
        pieAccrued[supplier] = supplierAccrued;
    ...
    function distributeBorrowerPie(
        ...
        if (borrowerIndex.mantissa > 0) {
            ...
            pieAccrued[borrower] = borrowerAccrued;
        ...
        for (uint i = 0; i < pTokens.length; i++) {
            ...
            if (borrowers == true) {
                ...
                for (uint j = 0; j < holders.length; j++) {
                    distributeBorrowerPie(pToken, holders[j], borrowIndex);
                    pieAccrued[holders[j]] = grantPieInternal(holders[j], pieAccrued[holders[j]]);
                }
            }
            if (suppliers == true) {
                updatePieSupplyIndex(pToken);
                for (uint j = 0; j < holders.length; j++) {
                    distributeSupplierPie(pToken, holders[j]);
                    pieAccrued[holders[j]] = grantPieInternal(holders[j], pieAccrued[holders[j]]);
                }
            }
        }
     ...
     * @param amount The amount of PIE to (possibly) transfer
     * @return The amount of PIE which was NOT transferred to the user
     */
    function grantPieInternal(address user, uint amount) internal returns (uint) {
        EIP20Interface pie = EIP20Interface(getPieAddress());
        uint pieRemaining = pie.balanceOf(address(this));
        if (amount > 0 && amount <= pieRemaining) {
            pie.transfer(user, amount);
            return 0;
        }
        return amount;
    }
