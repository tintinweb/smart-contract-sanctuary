...
contract EchoBase is AccessControl, IERC20, VolumeLimiter {
    ...
    // Set the maximum amount that can be exchanged in a single transfer
    uint256 public _maxTxAmount = 12 * 10**8 * 10**18;

    // Set the maximum amount that can be liquidated from the contract in the uniswap pool in a single transfer
    uint256 public _maxTxLiquidationAmount = _maxTxAmount.div(10);

    // Set the minimum amount of tokens that can be liquidated in the 
    // uniswap pool in a single call to transfer => 10M
    ...
    // update the maximum amount of tokens that the token contract can swap
