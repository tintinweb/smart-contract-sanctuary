...
//        whitelist[0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266] = Library.data({mintCount: 0, exist: true});
    }
    ...
    function daBoss(uint256 id) public ownerOfOrc(id) {
        ...
        if (boss.lvlProgress == 0) {
            ...
            uint8 item = uint8(((seed % 29) ** 2) / 29);

            uint8 helm = uint8(((_randomize(seed, "HELM", id) % 10) ** 2) / 10);
            uint8 mainhand = uint8(((_randomize(seed, "MAINHAND", id) % 10) ** 2) / 10);
            uint8 offhand = uint8(((_randomize(seed, "OFFHAND", id) % 10) ** 2) / 10);
            ...
            bosses[bossSlot] = Orc({body : 0, helm : helm, mainhand : mainhand, offhand : offhand, level : 0, lvlProgress : uint32(((helm + mainhand + offhand) / (29 + 10 + 10)) * 100), zugModifier : 0});
