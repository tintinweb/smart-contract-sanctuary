...
contract MintdropzERC721 is Ownable, ERC721Enumerable {
    ...
    function mintNFT(uint256 numberOfTokens) public payable {
        uint256 mintdropzPrice = 30000000000000000; // each token price is 0.03 ETH
        ...
        require(totalSupply().add(numberOfTokens) <= MAX_MINTDROPZ, "Purchase would exceed max supply of Mintdropz");
        require(msg.value >= mintdropzPrice.mul(numberOfTokens), "Ether value sent is not correct");

        uint256 restAmount = msg.value - mintdropzPrice.mul(numberOfTokens);
