...
contract Marketplace {
    ...
    mapping(uint256 => MarketNft) private _idToMarketNft;

    constructor(address xsroAddress) {
        _token = SarahRO(xsroAddress);
    }

    function setNftAddress(address nftAddress_) public {
        _nft = SRO721(nftAddress_);
    }

    // create sale function
    function createSale(uint256 id) public {
        address owner = _nft.ownerOf(id);
        require(msg.sender == owner, "Markerplace: you must be the owner of this nft");
        _idToMarketNft[id] = MarketNft(id, msg.sender);
        _nft.transferFrom(msg.sender, address(this), id); // use safeTransferFrom
    }

    function removeSale(uint256 id) public {
        address owner = _idToMarketNft[id].seller;
        require(msg.sender == owner, "Markerplace: you must be the owner of this nft");
        _nft.safeTransferFrom(address(this), owner, id);
    }

    // buy function
    function buyNft(uint256 id) public {
        uint256 price = _nft.getNftById(id).price;
        require(_token.balanceOf(msg.sender) >= price, "Marketplace: not enough SRO");
        address owner = _idToMarketNft[id].seller;
        _token.transferFrom(msg.sender, owner, price);
    ...
    // test
    function royal(uint256 id) public view returns (uint8) {
        return _nft.getNftById(id).royalties;
    }

    function description(uint256 id) public view returns (string memory) {
        return _nft.getNftById(id).description;
    }
}
...
contract SRO721 is ERC721, ERC721Enumerable, ERC721URIStorage {
    ...
    function create(
        ...
        _nfts[currentId] = Nft(_msgSender(), timestamp, royalties_, price_, title_, description_);
        emit Created(_msgSender(), currentId);
        return currentId;
    }

    // function owner change price
