...
contract Booster is IBooster, AccessControl {
  ...
  function initialize(address admin, address rewardHandler_) external {
    ...
    rewardDefinitions.push(RewardDefinition(15552000, 1875000000000000000));

    // Reward definition: 90 days / 130% APR
    rewardDefinitions.push(RewardDefinition(7776000, 1325000000000000000));

    // Reward definition: 30 days / 100% APR
    rewardDefinitions.push(RewardDefinition(2592000, 1083333333333333333));
  }
    ...
    if (currentLock.end == 0) {
      ...
      for (uint256 i = 0; i < rewardDefinitions.length; ++i) {
        if (lockPeriod >= rewardDefinitions[i].length) {
          uint256 originalAmount = currentLock.totalAmount;
          currentLock.totalAmount = (
            currentLock.totalAmount.mul(rewardDefinitions[i].rate)
          ).div(1E18);
          currentLock.end = ts + rewardDefinitions[i].length;
          currentLock.rate = rewardDefinitions[i].rate;
          currentLock.rateLength = rewardDefinitions[i].length;
          currentLock.last = ts;

          rewardsProvided.add(currentLock.totalAmount.sub(originalAmount));
          _verifyRewardsProvided();
          emit Lock(recipient, originalAmount, currentLock.totalAmount);
  ...
  function transferTo(address token, address recipient) external onlyAdmin {
    require(recipient != address(0), 'burn not allowed');
  ...
  function setRewardDefinition(
    ...
    // Validate input
    require(durations.length == rewards.length, 'B: Length mismatch');

    delete (rewardDefinitions);
    for (uint256 i = 0; i < durations.length; ++i) {
      require(i == 0 || durations[i] > durations[i - 1], 'B: Wrong sorting');
      rewardDefinitions.push(RewardDefinition(durations[i], rewards[i]));
    }
  }
  ...
  function _addMore(
    ...
    uint256 timedRate = ((lock_.rate.sub(1E18)).mul(lock_.end.sub(ts))).div(
      lock_.rateLength
    );
    uint256 rewardedAmount = (amount.mul(timedRate.add(1E18))).div(1E18);
    lock_.totalAmount = lock_.totalAmount.add(rewardedAmount);
    rewardsProvided.add(rewardedAmount.sub(amount));
    _verifyRewardsProvided();
    emit Add(recipient, amount, rewardedAmount);
  }
