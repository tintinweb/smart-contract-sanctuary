...
//        }
    ...
    function calcSLTKAmountForEth(uint256 amountEthToPay) public view returns (int256) {
        uint256  studentsAmount = 35;
        ( , int256 ethPrice, , , ) = AggregatorV3Interface(aggregator_ETH_USD).latestRoundData();
        uint256 decimals = uint256(AggregatorV3Interface(aggregator_ETH_USD).decimals());
        uint256 amountSLTKToBuy = (amountEthToPay * uint256(ethPrice)) / 10 ** decimals / studentsAmount;
