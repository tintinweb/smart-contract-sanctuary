...
contract WhitelistVerify is Ownable, Administrable {
  ...
  function getClaimableAmount(address addr, bytes32[] memory proof)
    external
    view
    returns (uint256)
  {
    return claimableAmount(addr, proof);
  }
  ...
  function checkAddress(address addr, bytes32[] memory proof) private view returns (bool) {
    require(
      ...
      'Addreess is not whitelisted'
    );
    return true;
  }

  function claimableAmount(address addr, bytes32[] memory proof) private view returns (uint256) {
    if (checkAddress(addr, proof)) {
  ...
  function canClaim(
    ...
    require(claimableAmount(addr, proof) >= amount, 'Too many tokens to claim');
    return true;
  }

  function addClaimed(address addr, uint256 amount) external onlyAdmin {
  ...
  function setMaxToClaim(uint256 max) external onlyOwner {
    maxToClaim = max;
  }
}
