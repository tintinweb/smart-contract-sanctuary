...
contract Oracle {
    ...
    mapping(address => uint256) public cummulativeMaticPrice;
    mapping(address => uint32) public tokenToTimestampLast;
    mapping(address => uint256) public cummulativeAveragePriceReserve;
    mapping(address => uint256) public cummulativeMaticPriceReserve;
    ...
    function setValues(address token) public {
        ...
        if (pool != address(0)) {
            if (MATIC < token) {
                ...
                cummulativeMaticPriceReserve[token] = IUniswapV2Pair(pool)
        ...
        returns (uint256 maticPerToken)
    {
        ...
        maticPerToken = _calculate(
            cummulativeAveragePrice[token],
            cummulativeMaticPriceReserve[token],
            timeElapsed,
            token
        );
    }
