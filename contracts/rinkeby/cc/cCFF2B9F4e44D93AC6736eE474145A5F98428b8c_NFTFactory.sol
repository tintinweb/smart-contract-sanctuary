...
import "../interfaces/factorys/INFTFactory.sol";
import "../interfaces/tokens/IKAISHIToken.sol";
import "../interfaces/tokens/INFTToken.sol";
import "../management/Managed.sol";

contract NFTFactory is INFTFactory, Managed {
    using EnumerableSet for EnumerableSet.AddressSet;
    using SafeERC20 for IKAISHIToken;
    ...
    function getMemberships()
        ...
        view
        override
        returns (address[] memory nftAddress, NFTInfo[] memory nftsInfo)
    {
        ...
        )
    {
        ...
        for (uint256 index = 0; index < length; index++) {
            ...
            count[index] = INFTToken(nftAddress).balanceOf(_addr);
        }
        highest = _getHighest(_addr);
    }

    function getHighest(address _addr)
        external
        view
        override
        returns (address)
    {
        return _getHighest(_addr);
    }
        ...
        returns (uint256)
    {
        ...
        return
            info.basePrice +
            (info.totalSupply - info.remainingSupply) *
            info.priceModifier;
    }
    ...
    {
        require(
            _nftMembershipAddress.remove(_nftAddress),
            "NFTFactory: not found"
        );
        delete nftInfo[_nftAddress];
        emit RemoveTier(_nftAddress);
    }

    function setRemainingSupply(address _nft, uint256 _amount)
        external
        override
        requirePermission(CAN_SET_REMAINING_SUPPLY)
    {
        nftInfo[_nft].remainingSupply = _amount;
    }
    ...
    function addNewTier(
        ...
        require(
            ...
            "NFTFactory: Can't equal zero address"
        );
        require(_basePrice > 0, "NFTFactory: Can't be 0");
        require(_isInfinity || _totalSupply > 0, "NFTFactory: Can't be 0");
        require(
            _nftMembershipAddress.add(_nftAddress),
            "NFTFactory: already exists"
        );
    ...
    function setBasePrice(address _nft, uint256 _amount)
        external
        override
        requirePermission(ROLE_ADMIN)
    {
        require(_amount > 0, "NFTF: can't be 0");
        NFTInfo storage info = nftInfo[_nft];
        info.basePrice = _amount;
    }

    function setPriceModifier(address _nft, uint256 _amount)
        external
        override
        requirePermission(ROLE_ADMIN)
    {
        NFTInfo storage info = nftInfo[_nft];
        info.priceModifier = _amount;
    }

    function setPrerequisiteNFT(address _nft, address _prerequisteNft)
        external
        override
        requirePermission(ROLE_ADMIN)
    {
        NFTInfo storage info = nftInfo[_nft];
        info.prerequisiteNFT = _prerequisteNft;
    }

    function buy(address _nft, uint256 _amount) external override {
        require(_amount > 0, "NFTFactory: amount can't be zero");

        NFTInfo storage info = nftInfo[_nft];

        require(info.totalSupply > 0, "NFTFactory: This NFT is not supported");
        require(
            info.remainingSupply >= _amount,
            "NFTFactory: not enough NFT for sale"
        );

        if (info.prerequisiteNFT != address(0)) {
            require(
                INFTToken(info.prerequisiteNFT).balanceOf(msg.sender) >=
                    _amount,
                "NFTFactory: There is not enough previous tokens"
            );
        }

        uint256 price = calculatePrice(_nft, _amount);

        require(
            kaishiToken.balanceOf(msg.sender) >= price,
            "NFTFactory: Not enough Kaishi tokens to buy"
        );

        uint256 percentage = price / 2;
        kaishiToken.burnFrom(msg.sender, price - percentage);
        kaishiToken.safeTransferFrom(msg.sender, tresuary, percentage);

        if (info.prerequisiteNFT != address(0)) {
            INFTToken nftIn = INFTToken(info.prerequisiteNFT);
            uint256[] memory ids = nftIn.getIds(msg.sender, 0, _amount);
        ...
        for (uint256 i = 0; i < _amount; i++) {
            INFTToken(_nft).mint(msg.sender);
        }

        emit Buy(msg.sender, _nft, _amount);
    }

    function calculatePrice(address _nft, uint256 _amount)
        public
        view
        returns (uint256)
    {
    ...
    function _getHighest(address _addr) internal view returns (address) {
        address nft;
        uint256 maxPrice;
        for (
            uint256 index = 0;
            index < _nftMembershipAddress.length();
            index++
        ) {
            address addr = _nftMembershipAddress.at(index);
            if (
                maxPrice < nftInfo[addr].basePrice &&
                IERC20(addr).balanceOf(_addr) > 0
            ) {
                maxPrice = nftInfo[addr].basePrice;
                nft = _nftMembershipAddress.at(index);
            }
        }
        return nft;
    }
...
interface INFTFactory {
    ...
    event AddTier(address _nft);
    event RemoveTier(address _nft);
    ...
    function getUserInfo(address addr_)
        external
        view
        returns (
            address[] memory nft,
    ...
    function getHighest(address addr_) external view returns (address);

    function getSupportNFTAddress() external view returns (address[] memory);

    function getCurrentPrice(address nftAddress_)
        external
        view
        returns (uint256);
    ...
    function buy(address nft_, uint256 amount_) external;

    function setPrerequisiteNFT(address _nft, address _prerequisteNft) external;

    function setRemainingSupply(address _nft, uint256 _amount) external;

    function setPriceModifier(address _nft, uint256 _amount) external;

    function setBasePrice(address _nft, uint256 _amount) external;
}
