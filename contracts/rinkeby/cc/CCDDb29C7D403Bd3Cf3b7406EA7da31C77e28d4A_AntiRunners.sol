...
contract AntiRunners is ERC721, Ownable, ReentrancyGuard {
    ChainRunners public chainRunners;
    ...
    uint256 private antiRunnersTeamWithdrawalAmount;
    uint256 private chainRunnersTeamWithdrawalAmount;
    address private chainRunnersTeamAddress;

    constructor(address payable _chainRunnersAddress, address payable _chainRunnersTeamAddress) ERC721("Anti Runners", "ANTI") {
        chainRunners = ChainRunners(_chainRunnersAddress);
        chainRunnersTeamAddress = _chainRunnersTeamAddress;
    }
    ...
    function mintRunners(uint256 _count) external payable nonReentrant returns (uint256, uint256) {
    ...
    function mintRunnerZero() external {
        ...
        require(chainRunners.getDna(0) != 0, "Chain Runner 0 does not exist");
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        return renderer.tokenURI(_tokenId, getDna(_tokenId));
    }
    ...
    function tokenURIForSeed(uint256 _tokenId, uint256 seed) public view virtual returns (string memory) {
        ...
        IAntiRunnersRenderer renderer = IAntiRunnersRenderer(renderingContractAddress);
        return renderer.tokenURI(_tokenId, seed);
    }

    function getDna(uint256 _tokenId) public view returns (uint256) {
        return chainRunners.getDna(_tokenId);
    }

    receive() external payable {}

    function withdraw() public nonReentrant {
        require(msg.sender == owner() || msg.sender == chainRunnersTeamAddress, "Not allowed.");

        uint256 total = address(this).balance + antiRunnersTeamWithdrawalAmount + chainRunnersTeamWithdrawalAmount;

        uint256 amount;
        if (msg.sender == owner()) {
            amount = total * 4 / 5 - antiRunnersTeamWithdrawalAmount;
            antiRunnersTeamWithdrawalAmount += amount;
        } else {
            amount = total * 1 / 5 - chainRunnersTeamWithdrawalAmount;
            chainRunnersTeamWithdrawalAmount += amount;
        }
