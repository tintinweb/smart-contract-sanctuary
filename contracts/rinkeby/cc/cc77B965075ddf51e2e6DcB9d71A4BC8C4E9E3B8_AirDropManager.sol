...
import "./managers/ProjectTokenURIManager/ProjectTokenURIManager.sol";
    ...
    PausableUpgradeable
{
    ...
    /// @dev AirDropId => AirDropInfo
    mapping(uint256 => AirDropInfo) public airDropInfos;
    /// @dev user => (airDropId => status)
    mapping(address => mapping(uint256 => bool)) public claimed;
    /// @dev airDropId => (position => edition)
    ...
    event AirDropCreated(uint256 indexed airDropId, AirDropInfo airDropInfo);
    event AirDropUpdated(uint256 indexed airDropId, AirDropInfo airDropInfo);
    event AirDropBaseURIUpdated(uint256 indexed airDropId, string baseURI);
    event Mint(
    ...
    modifier onlyValidConfig(AirDropInfo calldata airDropInfo) {
        require(
            airDropInfo.endTime == 0 || airDropInfo.endTime > airDropInfo.startTime,
            "endTime should > startTime or = 0"
        );
        require(airDropInfo.totalAmount > 0, "bad totalAmount");
        require(airDropInfo.claimedAmount == 0, "bad claimedAmount");
        require(address(airDropInfo.project).isContract(), "bad project address");
        require(bytes(airDropInfo.baseURI).length > 0, "bad baseURI");
        _;
    }

    /// @dev Ensure that the function is called before the sale start
    modifier beforeStart(AirDropInfo calldata airDropInfo) {
        require(block.timestamp < airDropInfo.startTime, "not allow after start");
        _;
    }
    ...
    /// @notice admin setup a air drop artwork
    function createAirDrop(AirDropInfo calldata _airDropInfo) external onlyOwner onlyValidConfig(_airDropInfo) {
        uint256 airDropId = _getNextAndIncrementId();
        airDropInfos[airDropId] = _airDropInfo;
        emit AirDropCreated(airDropId, _airDropInfo);
    }

    /// @notice admin can update air drop info if not started already
    function updateAirDrop(uint256 _airDropId, AirDropInfo calldata newInfo)
        external
        onlyValidConfig(newInfo)
        onlyOwner
        beforeStart(newInfo)
    {
        airDropInfos[_airDropId] = newInfo;
        emit AirDropUpdated(_airDropId, newInfo);
    }

    function updateAirDropURI(uint256 _airDropId, string calldata _baseURI) external onlyOwner {
        airDropInfos[_airDropId].baseURI = _baseURI;
        emit AirDropBaseURIUpdated(_airDropId, _baseURI);
    }

    function redeem(uint256 _airDropId, bytes32[] calldata proof) external nonReentrant whenNotPaused {
        AirDropInfo storage airDropInfo = airDropInfos[_airDropId];
        require(airDropInfo.startTime <= block.timestamp, "airDrop: not started yet");
        require(airDropInfo.endTime == 0 || airDropInfo.endTime >= block.timestamp, "airDrop: already ended");
        require(airDropInfo.claimedAmount <= airDropInfo.totalAmount, "exceed max amount");
        require(!claimed[_msgSender()][_airDropId], "user cannot mint more");
        require(
            MerkleProofUpgradeable.verify(proof, airDropInfo.root, getLeaf(_msgSender(), _airDropId)),
            "Invalid merkle proof"
        );
        claimed[_msgSender()][_airDropId] = true;
        uint64 randomPrintEdition = _getPrintEdition(_airDropId, airDropInfo.totalAmount, airDropInfo.claimedAmount);
        airDropInfo.claimedAmount++;
        uint256 tokenId = airDropInfo.project.managerMint(_msgSender(), "");
        projectTokenIdToEditionInfo[address(airDropInfo.project)][tokenId] = EditionInfo({
            airDropId: _airDropId,
            printEdition: randomPrintEdition
        });
        emit Mint(_airDropId, _msgSender(), address(airDropInfo.project), tokenId, randomPrintEdition);
    }

    function getLeaf(address account, uint256 _airDropId) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(account, _airDropId));
    }
    ...
    function tokenURI(address project, uint256 tokenId) public view override returns (string memory) {
        ...
        require(editionInfo.printEdition > 0, "ProjectAirDropManager: nonexistent token");
        return
            string(
                abi.encodePacked(
                    airDropInfos[editionInfo.airDropId].baseURI,
    ...
    function _getPrintEdition(
        ...
        mapping(uint64 => uint64) storage tokenMatrix = randomMatrix[_airDropId];
