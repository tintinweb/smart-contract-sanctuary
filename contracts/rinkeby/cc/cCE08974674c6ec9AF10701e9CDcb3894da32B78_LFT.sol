...
contract LFT is Ownable, ERC721 {
    ...
    uint256 _tokensAvailableToSell;
    uint256 _tokensAvailablePerMint;
    ...
    function getTier1TokenIds(uint256 offset, uint256 limit) external view returns (uint256[] memory) {
        ...
        uint256 maxLimit;
        if (totalSupplyCount >= (offset + limit)) {
            maxLimit = limit;
        } else if (offset > totalSupplyCount) {
            maxLimit = 0;
        } else {
            maxLimit = totalSupplyCount - offset;
        }
        ...
        for (uint256 i = offset + 1; i <= offset + maxLimit; i++) {
            if (_tier1Tokens[i] == true) {
                res[counter] = i;
                counter++;
            }
        }
    ...
    function getTokenAddressMap(uint256 offset, uint256 limit) external view returns (uint256[] memory, address[] memory) {
        uint256 totalSupplyCount = totalSupply();
        uint256 iterLimit = totalSupplyCount >= (offset + limit) ? offset + limit : totalSupplyCount;

        uint256 maxLimit;
        if (offset > totalSupplyCount) {
            maxLimit = 0;
        } else {
            maxLimit = iterLimit - offset;
        }
        ...
        uint256 counter = 0;
        for (uint256 i = offset; i < iterLimit; i++) {
            addresses[counter] = ownerOf(i+1);
            tokenIds[counter] = i+1;
    ...
    function getTokenListByAddress(address owner, uint256 offset, uint256 limit) external view returns (uint256[] memory) {
        require(balanceOf(owner) != 0, "Empty balance");
    ...
    function tier1balanceOf(address owner, uint256 offset, uint256 limit) external view returns (uint256) {
        ...
        for (uint256 i = offset + 1; i <= offset + maxLimit; i++) {
            if (res >= addressTokensCount) {
                break;
            }

            if (ownerOf(i) == owner && _tier1Tokens[i] == true) {
                res++;
            }
        }
    ...
    function tier2balanceOf(address owner, uint256 offset, uint256 limit) external view returns (uint256) {
        ...
        for (uint256 i = offset + 1; i <= offset + maxLimit; i++) {
            ...
            if (ownerOf(i) == owner && _tier1Tokens[i] == false) {
                res++;
            }
        }
