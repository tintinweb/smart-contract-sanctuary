...
contract Vesting is DSMath {
  ...
  event GrantAdded(address indexed recipient, uint256 indexed startTime, uint128 indexed amount, uint16 vestingDuration, uint16 vestingCliff, uint16 claimFrequency);
  event GrantRemoved(address indexed recipient, uint128 amountVested, uint128 amountNotVested);
  event GrantTokensClaimed(address indexed recipient, uint128 amountClaimed);
  ...
  function addTokenGrant(address _recipient, uint256 _startTime, uint128 _amount, uint16 _vestingDuration, uint16 _vestingCliff, uint16 _claimFrequency) public 
  onlyColonyMultiSig
  noGrantExistsForUser(_recipient)
  {
    require(_vestingCliff > 0, "colony-token-zero-vesting-cliff");
    require(_claimFrequency > 0, "colony-token-zero-claim-frequency");
    ...
    emit GrantAdded(_recipient, grant.startTime, _amount, _vestingDuration, _vestingCliff, _claimFrequency);
  }
  ...
  function calculateGrantClaim(address _recipient) public view returns (uint16, uint128) {
    ...
    // Duration is start from the cliff time
    elapsedMonths -= tokenGrant.vestingCliff;
    ...
    if (elapsedMonths >= tokenGrant.vestingDuration) {
      ...
      // See if month elapsed is according to every month claimed
      // ie. 10 months elapsed, with once every 4 month being claimed.
      // So months that available for being claimed is 8 (not 10)
      uint16 modulo = monthsVested % tokenGrant.claimFrequency;
      if (modulo > 0) {
        monthsVested -= modulo;
      }
