...
// Version: 0.2
contract CycloTurtlesV02 is ERC721URIStorage, Ownable { 
    ...
    VITSaleConfig public vitSaleConfig = VITSaleConfig(80000000000000000, false, 2, 0);
    PresaleConfig public presaleConfig = PresaleConfig(100000000000000000, false, 5, 0);
    SaleConfig public saleConfig = SaleConfig(150000000000000000, false, 10);

    uint256 public maxTotalSupply = 7777;
    uint public maxGiftSupply = 155;
    ...
    constructor() ERC721("TestNFT", "TEST") {} // "CycloTurtles", "TURTLES"
        ...
        onlyOwner
    {
        ...
        for (uint i = 0; i < totalSupplyCount; i++) {
            _setTokenURI(i, baseURI);
        }
    }
    ...
    function isOnList (address _address, string memory _list) external view returns (bool) {
        if (keccak256(abi.encodePacked(_list)) == keccak256(abi.encodePacked("VIT"))) {
            return _vitList[_address];
        } else if (keccak256(abi.encodePacked(_list)) == keccak256(abi.encodePacked("presale"))) {
    ...
    {
        require(
            totalSupplyCount + _addresses.length <= maxTotalSupply,
        ...
        for (uint i = 0; i < _addresses.length; i++) {
            ...
            _safeMint(_addresses[i], _newTokenId);
            _setTokenURI(_newTokenId, baseURI);
            _giftClaimed[_addresses[i]] += 1;
            totalSupplyCount += 1;
            giftCount += 1;
            emit Mint(_addresses[i], 1, "Gift");
        }
    }
    ...
    function vitMint(uint256 _amount) external payable {
        ...
        require(
            ...
            "CycloTurtles: Can only mint 2 per wallet"
        );
        require(
    ...
    function presaleMint (uint256 _amount) external payable {
        ...
        require(
            ...
            "CycloTurtles: Can only mint 2 tokens"
        );
        require(
    ...
    function saleMint(uint256 _amount) external payable {
        ...
        require(
            _amount <= 5,
            "CycloTurtles: Can only mint 5 tokens per txn"
        );
        require(
            _saleClaimed[msg.sender] + _amount < saleConfig.maxWallet,
            "CycloTurtles: Can only mint 10 tokens per wallet"
        );
        require(
