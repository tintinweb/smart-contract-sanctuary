...
contract VechTokenContract is ERC721Enumerable, Ownable, ERC721Pausable {
    ...
    uint256 public preSaleTokensPerAddress = 5;
    ...
    // for promotions
    uint256 public promotionReservedTokens = 150;
    ...
    // cap the OG whitelist at 300
    uint256 public constant maxOGAddresses = 300;

    // track how many tokens every og has minted
    mapping(address => uint8) public ogMints;

    // count the amount of og addresses
    uint256 public ogAddressCount;

    // count the reserved number of reserved og tokens
    uint256 public ogReservedTokenCount;
    ...
    // TODO: addresses below are for testing purposes only
    address devAddress = 0x0C7a1AE154717fdA46A8cc52913C63572FDb66e8; // developer
    address cmAddress = 0x65fF8Fd15C2c7f510fa942E00c6E5c09f81f7085; // TODO: use real address // community manager
    address mcAddress = 0xccf8312a1314517316F2D96dadF68b3263A8Bf46; // TODO: use real address // Minters Collective
    address vechAddress = 0x7e929da5B4283c4881B58c5F83C4C2Fc7C78fC34; // TODO: use real address // vech founders
     ...
     * First token is free
     */
    function ogSaleMint(uint8 amount)
        public
        payable
        ...
        amountWithinMaxTokens(amount)
    {
        if (ogMints[msg.sender] > 0) {
            ...
            require(
                tokenPrice.mul(amount - 1) <= msg.value,
                "Not enough ether to mint"
            );
            ogReservedTokenCount += 1;
        }
        mint(amount);
     ...
     */
    function preSaleMint(uint8 amount)
        public
        payable
     ...
     */
    function reservedMint(uint8 amount, address to) public {
        require(
            mintedReservedTokenCount.add(amount) <=
                promotionReservedTokens + teamReservedTokens,
    ...
    // set minting price
    function setMintPrice(uint256 _tokenPrice) public onlyOwner {
    ...
    function addOGAddresses(address[] memory newOGMembers) public onlyOwner {
        require(
            ogAddressCount.add(newOGMembers.length) < maxOGAddresses,
            "Too many og addresses"
        );
        for (uint256 i = 0; i < newOGMembers.length; i++) {
            ogAddress[newOGMembers[i]] = true;
        }
    }

    // remove addresses from og-list
    function removeOGAddresses(address[] memory ogMembers) public onlyOwner {
        for (uint256 i = 0; i < ogMembers.length; i++) {
            ogAddress[ogMembers[i]] = false;
        }
    }
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < newWhitelistMembers.length; i++) {
            whitelistedAddress[newWhitelistMembers[i]] = true;
        }
    }

    // remove addresses from whitelist
    function removeWhitelistAddresses(address[] memory whitelistMembers)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < whitelistMembers.length; i++) {
            whitelistedAddress[whitelistMembers[i]] = false;
        }
    }
    ...
    function revealTokens() public onlyOwner {
        ...
        require(
            reveals == 0 || totalSupply() == maxTokens,
        ...
        partialReveals[reveals] = PartialReveal(
            seed,
            totalSupply().sub(revealedTokens)
        );

        reveals++;
        revealedTokens = totalSupply();
    }

    // withdraws funds
    function withdraw() public onlyOwner {
        uint256 amountToDev = (address(this).balance * 100) / 1000; // 10%
        uint256 amountToMC = (address(this).balance * 200) / 1000; // 20%
        uint256 amountToCM = (address(this).balance * 10) / 1000; // 1%

        sendEth(mcAddress, amountToMC);
        sendEth(devAddress, amountToDev);
        sendEth(cmAddress, amountToCM);

        uint256 amountToFounders = address(this).balance; // The rest
        sendEth(vechAddress, amountToFounders);
    }
        ...
        returns (string memory)
    {
        ...
        if (reveals == 1 || tokenId < partialReveals[0].tokenCount) {
            ...
            vechId = (partialReveals[0].seed + tokenId) % maxTokens;
        ...
        return string(abi.encodePacked(base, uint2str(vechId % maxTokens)));
    }

    // returns: the amount of reserved tokens
    function reservedTokens() public view returns (uint256) {
        // ogAddressCount = amount of og addresses ~ around 55
        // ogReservedTokenCount removes an og reservation when ogs mint their free token
        if (publicSaleIsActive) {
            return promotionReservedTokens + teamReservedTokens;
        }
        return
            promotionReservedTokens +
            teamReservedTokens +
            ogAddressCount -
            ogReservedTokenCount;
    }
    ...
    modifier correctPayment(uint8 amount) {
        require(
            tokenPrice.mul(amount) <= msg.value,
            "Not enough ether to mint"
        );
    ...
    modifier whitelisted(uint8 amount) {
        require(whitelistedAddress[msg.sender], "Address not whitelisted");
        require(
    ...
    modifier ogListed(uint8 amount) {
        require(ogAddress[msg.sender], "Address not whitelisted");
        require(
    ...
    modifier amountWithinMaxTokens(uint8 amount) {
        require(
            totalSupply().add(amount) <= maxTokens.sub(reservedTokens()),
