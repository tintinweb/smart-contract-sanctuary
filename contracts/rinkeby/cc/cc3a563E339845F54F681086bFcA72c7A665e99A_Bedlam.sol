...
contract Bedlam is Ownable, ReentrancyGuard, BaseERC721, Merkle {          
    ...
    /**
        @dev phase one holders can mint for free and are assigned SOP 
        based on rarity defined offchain. Phase one tokens cannot be
        used more than once and are defined in opensea.
     */
    function claim(uint totalAllocated, bytes32[] calldata proof) external nonReentrant {
        uint current = totalSupply();
        bool isClaimed = claimed[msg.sender];        
        require(isPhaseSaleActive, "Not Live");
        require(!isClaimed, "Cannot claim again");
        require(_phaseOneVerify(_phaseOneLeaf(msg.sender, totalAllocated), proof), "Invalid"); 
        require(current + totalAllocated <= supply, "Sold out");
        for(uint256 i; i < totalAllocated; i++) {
     ...
     */
    function submitSymbols(uint256[] memory ids, uint16[5][] memory value) external nonReentrant {        
        require(ids.length == balanceOf(msg.sender), "Must submit all");
        for(uint i; i < ids.length; i++) {
            require(ownerOf(i) == msg.sender, "Not owner");
            require(!submissions[i].isSubmitted, "Cannot resubmit");
            Submission memory submission;
            submission.isSubmitted = true;
            submission.value = value[i];
    ...
    /** @dev view to scrape submissions for art generation  */
    function getSubmission(uint id) external view returns (uint16[5] memory) {        
        return (submissions[id].value);
    }

    function isClaimed(address sender) external view returns (bool) {
...
contract BaseERC721 is ERC721Enum, Ownable, ReentrancyGuard, PaymentSplitter {
    using SafeMath for uint256;
    ...
    address[] _addresses = [
        ...
        0x91C0E67119ffaFe8d5e274A05cc10509F40A82F7
    ];
    ...
    ) ERC721(name, symbol) PaymentSplitter(_addresses, _shares){
        supply = _supply;
        max = _max;
        price = _price;
        baseUri = _baseUri;
   ...
   function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    function _callMint(uint256 count) internal {
        ...
        require(total < supply, "Sold out");
        if(msg.sender != teamWallet) {
            require(price.mul(count) == msg.value, "Incorrect Eth");
            require(mintCount[msg.sender].add(count) <= max, "Max Mint");        
        }
