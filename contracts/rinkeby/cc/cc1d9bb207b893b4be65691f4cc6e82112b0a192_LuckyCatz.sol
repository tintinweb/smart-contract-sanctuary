...
contract LuckyCatz     is ERC721 
{
    ...
    uint256 private     totalTokens        = 7777;//30;//7777;
    uint256 private     totalMintedTokens  = 0;
    uint256 private     maxMintPerWallet   = 7;
    
    uint256 private		fullRawShare       = 10000;	        // This is 100.00% without the DOT
    
    string  private     baseURI = 'https://ipfs.io/ipfs/QmNbyTEKdHj9xEGot6NAXYU8kMgyZwrCLc38giE92BM9Dt/';
    ...
    mapping(uint256 => bool)    private a5;

    uint16[]            availableTokens;

    
    constructor() ERC721("CTZ10","CETEZED Token")       // temporary Symbol and title
    {
    ...
    function    getMintCount(address walletAddress) external view returns(uint256)
    {
        return walletMintCounts[walletAddress];
    }
    ...
    function    mint() external payable callerIsUser 
    {
        ...
        {
            require(salesDate<block.timestamp, "Sales did not start yet");
    ...
    {
        require(msg.value == salesPrice * quantity,                          "Not enough Ether to claim the tokens");
        require(walletMintCounts[msg.sender] + quantity <= maxMintPerWallet, "You cannot claim more tokens");
