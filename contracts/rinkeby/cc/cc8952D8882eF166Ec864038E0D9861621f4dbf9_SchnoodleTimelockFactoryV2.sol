...
contract SchnoodleTimelockFactoryV2 {
    address immutable schnoodleTimelock;

    constructor() {
        schnoodleTimelock = address(new SchnoodleTimelockV2());
    }
    ...
    function create(IERC20Upgradeable token, address beneficiary, uint256 releaseTime) external returns (address) {
        ...
        SchnoodleTimelockV2(clone).initialize(token, beneficiary, releaseTime);
        return clone;
    }
}
