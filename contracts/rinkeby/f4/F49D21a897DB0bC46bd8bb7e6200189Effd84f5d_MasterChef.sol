...
contract RedBerryToken is BEP20 {
    ...
    // Addresses that excluded from tranferRate
    mapping(address => bool) private _excludedFromTransferRate;
    ...
    function _transfer(
        ...
        if (
            ...
            transferTaxRate == 0 ||
            isExcludedFromTransferRate(sender) ||
            isExcludedFromTransferRate(recipient)
        ) {
     ...
     */
    function isExcludedFromTransferRate(address _account)
        public
        view
        returns (bool)
    {
        return _excludedFromTransferRate[_account];
    }

    /**
     * @dev Exclude or include an address from TransferRate.
     * Can only be called by the current operator.
     */
    function setExcludedFromTransferRate(address _account, bool _excluded)
        public
        onlyOperator
    {
        _excludedFromTransferRate[_account] = _excluded;
    }
...
contract MasterChef is Ownable {
    ...
    function deposit(
        ...
        if (user.amount > 0) {
            ...
            if (referarBalance > 0) {
                userReferalClaimedAmount[_pid][msg.sender] = userReferalClaimedAmount[_pid][msg.sender].add(referarBalance);
