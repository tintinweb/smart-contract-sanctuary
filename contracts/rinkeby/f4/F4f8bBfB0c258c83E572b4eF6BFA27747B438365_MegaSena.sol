...
contract MegaSena is VRFConsumerBase {
    ...
    function contains(uint256 n, uint256[] memory arr) public pure returns (bool){
    ...
    function findInsertIndex(uint256 n, uint256[] memory arr) public pure returns (uint256){
        for(int i = int(arr.length - 1); i >= 0; i--){
            if(n > arr[uint(i)] && arr[uint(i)] != 0){
                return uint(i + 1);
            }
        }
        return 0;
     ...
     */
    function insertAt(uint256[] memory arr, uint256 at, uint256 n) public pure returns (uint256[] memory shiftedArr){
        shiftedArr = new uint256[](arr.length);
        ...
        for(uint i = at + 1; i < arr.length; i++){
    ...
    function expand(uint256 randomValue, uint256 n) public{
        ...
        while(inserted < n){
            uint256 value = uint256(keccak256(abi.encode(randomValue, j))) % 61 + 1;
            if(contains(value, expandedValues)){
                ...
                uint256 at = findInsertIndex(value, expandedValues);
