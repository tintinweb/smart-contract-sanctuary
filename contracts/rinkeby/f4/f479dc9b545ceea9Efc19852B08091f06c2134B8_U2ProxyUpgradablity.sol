...
contract U2ProxyUpgradablity is Ownable {
  ...
  mapping(address => uint256) public remainingRewards;
  ...
  function getOneDayReward(
    ...
    uint256 dailyReward = remainingRewards[tokenAddress].div(6).div(90);
    return dailyReward.mul(stakedAmount.div(totalStake));
  }
  ...
  function sendToken(
    ...
    if (tokenAddress != address(0)) {
      require(
        remainingRewards[tokenAddress] >= amount,
        "SEND : Insufficient Balance"
      );
      remainingRewards[tokenAddress] = remainingRewards[tokenAddress].sub(
        amount
      );
      // Transfer of rewards
  ...
  function setRemainingRewards(
    address[] memory tokenAddresses,
    uint256[] memory rewards
  ) external onlyOwner {
    require(tokenAddresses.length == rewards.length, "invalid array size");
    for (uint256 v = 0; v < tokenAddresses.length; v++) {
      require(tokenAddresses[v] != address(0), "invalid address");
      require(rewards[v] > 0, "invalid reward");
      remainingRewards[tokenAddresses[v]] = rewards[v];
    }
  }
  ...
  function _rewardCalculation(
    ...
    rewardsEarned = noOfDays.mul(
      getOneDayReward(stakedToken, stakedAmount, totalStake)
    );
      ...
      if (noOfDays >= intervalDays[i]) {
        ...
        if (
          ...
          rewardsEarned = balDays.mul(
            getOneDayReward(rewardToken, stakedAmount, totalStake)
          );
          //  Rewards Send
