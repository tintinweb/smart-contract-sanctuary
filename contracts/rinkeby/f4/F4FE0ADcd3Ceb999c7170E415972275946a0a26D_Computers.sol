...
contract Computers is ERC721Enumerable, Ownable {

    using Strings for uint256;

    string _baseTokenURI;
    uint256 private BUYABLE = 20;
    uint256 private PRICE = 1 * 10**15; //0.03 ETH;
    uint public constant MAX_ENTRIES = 10;
    ...
    function mint() external payable {
		...
		require(msg.value >= PRICE, "Computer: incorrect price");
        uint256 tokenCount = balanceOf(msg.sender);
		require(tokenCount + 1 <= BUYABLE, "Computer: Buyable amount has been reached");

		_mint(msg.sender, _getNewId(sold + 1));
		sold ++;
    }
