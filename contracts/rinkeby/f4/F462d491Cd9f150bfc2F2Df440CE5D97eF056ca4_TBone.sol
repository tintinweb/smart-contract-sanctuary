...
contract TBone is ERC20Upgradeable, AccessControlUpgradeable {
  ...
  function calculateTokensSinceLastClaim(uint256 _token, uint256 _date, bool _genesis) public view returns (uint256) {
    ...
    uint256 tokens = ratioPerDay * ((_date - lastClaimedDate) / DAY_IN_SECONDS);
  ...
  function changeName(uint256 _tokenId, string memory _newName, bool _genesis) public returns (bool) {
    require(bytes(_newName).length > 0, "Name can't be empty");
    require(nameReserve[_newName] != true, "Name is already taken");

    return _changeName(_tokenId, _newName, _genesis ? genesisPugs : unleashedPugs);
  }

  function _changeName(uint256 _tokenId, string memory _newName, address _collection) private returns (bool) {
