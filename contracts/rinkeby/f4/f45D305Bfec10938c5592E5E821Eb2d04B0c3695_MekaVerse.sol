...
contract MekaVerse is ERC721Enumerable, EIP712, Ownable {
    ...
    bytes32 public constant TYPEHASH = keccak256("MintReq(address wallet,uint256 tokensId,uint256 timestamp)");

    event PauseEvent(bool pause);
    event welcomeToMekaVerse(uint256 indexed id);

    constructor(string memory baseURI) ERC721("MekaVerse", "MEKA") EIP712("MekaVerse", "1"){
        setBaseURI(baseURI);
    }
    ...
    function signatureWallet(address wallet, uint256[] memory _tokensId, uint256 _timestamp, bytes memory _signature) public view returns (address){
        ...
        for(uint256 i = 0; i < _tokensId.length; i++){
            tokensId += _tokensId[i] * ((MAX_ELEMENTS + 1) * (i + 1));
        }

        bytes32 digest = _hashTypedDataV4(
            keccak256(abi.encode(TYPEHASH, wallet, tokensId, _timestamp))
        );
    ...
    function mintUnsoldTokens(uint256[] memory _tokensId) public onlyOwner {
        ...
        require(PAUSE, "Pause is not active");
