...
contract PresaleContract is Ownable, ReentrancyGuard {
    ...
    event NewReward(uint reward);
    
    constructor(address _rewardAddress, address _verifyAddress) public {
    ...
    {   
        if(userPercentage[msg.sender] == 0) {
            bytes32 message = prefixed(keccak256(abi.encode(address(this), _tokenAddress, msg.sender, _percentage, _nonce)));
            require(verify(message, signature), "invalid signature");
            userPercentage[msg.sender] = _percentage;
        }

        _claim(msg.sender);
    }
    
    function claimWithoutSig() external nonReentrant() {
        _claim(msg.sender);
    }
    
    function _claim(address _userAddress) internal {
        uint userReward = calculateReward(_userAddress);
        require(userReward > 0, "nothing to claim");

        userLastUnclaimedReward[_userAddress] = rewards.length;
        rewardToken.safeTransfer(_userAddress, userReward);

        emit Transferred(_userAddress, userReward);
    }
    
    function setVerifyAddress(address newVerifyAddress) public onlyOwner {
    ...
    function calculateReward(address _userAddress) public view returns(uint) {
        ...
        uint res;
        for(uint i = userLastUnclaimedReward[_userAddress]; i < rewards.length; i++) {
            res = res.add(rewards[i]);
        }

        return res.mul(userPercentage[_userAddress]).div(1e18);
    }

    function addReward(uint _rewardAmount) external onlyOwner {
        rewards.push(_rewardAmount);
        emit NewReward(_rewardAmount);
    }
