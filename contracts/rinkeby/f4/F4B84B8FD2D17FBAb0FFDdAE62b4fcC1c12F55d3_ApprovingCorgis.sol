...
contract ApprovingBone is ERC721Enumerable, Ownable {
     ...
     * mint Bones
     */
    function mintSpecialBones(uint256 numberOfTokens) public payable onlyCreator {
        uint256 supply = totalSupply();
        require(supply <= maxBones, "Exceeds maximum Corgis supply" );
...
contract ApprovingCorgis is ERC721Enumerable, Ownable {
    ...
    function earlyAccessMint(uint256 numberOfTokens) public payable {
        ...
        uint256 corgiCount = balanceOf(msg.sender);

        require(earlyAccessIsActive, "Early Access Sale is not active yet");
        require(boneCount > 0, "You don't have a mint pass");
        require(corgiCount.add(numberOfTokens) <= maxEarlyAccessPurchase, "Max 5 Corgis per account");
