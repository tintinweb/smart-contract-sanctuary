    ...
    Pausable
{
    ...
    function secondaryRoyaltyCancel(
        Orders.SecondaryRoyalty memory secondaryRoyalty
    ) external nonReentrant {
        require(
            secondaryRoyalty.owner == msg.sender ||
                owners[msg.sender] ||
                adminSigner == msg.sender,
            "ANIFTY: FORBIDDEN"
        );
        bytes32 hash = secondaryRoyalty.hash();
        isCancelled[hash] = true;

        emit SecondaryRoyaltyCancel(hash);
    }
    ...
    function askUpdate(
        ...
        emit Update(prevHash, hash, askOrder);
    }
...
interface IBaseExchange {
    event SecondaryRoyaltyCancel(bytes32 indexed hash);
