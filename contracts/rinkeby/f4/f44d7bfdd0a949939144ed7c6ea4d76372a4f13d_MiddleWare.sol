...
contract MiddleWare{
    ...
    function getDclOperatorByDoNft(address nftAddr, uint256 nftId) public  view returns (address operator) {
        ...
        (uint256 oid,,,,) = doNft.getDoNftInfo(nftId);
        address oNftAddr = doNft.getOrignalNftAddress();
    ...
    function getDoLandOrDoEstInfo(address nftAddr, uint256 nftId, address marketAddr) public view  
        returns (address owner , address operator, uint64 startTime, uint64 endTime, bool isOnLent, uint64 minDuration, uint64 maxEndTime, uint256 pricePerSecond) {
        ...
        (,startTime, endTime)  = doNft.getDuration(nftId, 0);
                         
        IMarket market = IMarket(marketAddr);
         
        isOnLent = market.isOnLent(nftAddr, nftId);

        if(isOnLent) {
           IMarket.Lending memory lenting  = market.getLent(nftAddr, nftId);
