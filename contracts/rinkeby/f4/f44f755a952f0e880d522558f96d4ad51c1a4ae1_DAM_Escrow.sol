...
interface IERC721 {
        function mint(uint numTokens) external;
...
contract DAM_Escrow is UUPSUpgradeable {
        ...
        override
    {
        ...
        function addAssayer(uint marketID) public {
                ...
                tokenMarketID[IERC721(damToken).tokenID()] = marketID;
                mintingNewAssayerCost[marketID] *= 2;
                IERC721(damToken).mint(1);
        }
        ...
        function mint(uint numTokens, uint marketID) public onlyOwner(marketID){
                ...
                IERC721(damToken).mint(numTokens);
        }

        function commitAssay(uint _auditID, bytes32 saltedCommit, uint _marketAddressID, uint _tokenID)
        auditNotExpired(_auditID)
        public {
                if(_marketAddressID == 0) require(IERC721(damToken).ownerOf(_tokenID)==msg.sender && audits[_auditID].marketID==tokenMarketID[_tokenID],"16"); // _marketAddressID = 0 in cases when assayers are only from local market and not from "otherMarkets"
                //else require(ERC721Upgradeable(otherMarkets[audits[_auditID].marketID][_marketAddressID-1]).ownerOf(_tokenID)==msg.sender,"17");
                else require(IERC721(damToken).ownerOf(_tokenID)==msg.sender && tokenMarketID[_tokenID]==otherMarkets[audits[_auditID].marketID][_marketAddressID-1],"17");
        ...
        modifier trueAssayer(uint _auditID, uint assayID) {
                if(audits[_auditID].commits[assayID].marketAddressID==0) require(IERC721(damToken).ownerOf(audits[_auditID].commits[assayID].tokenID)==msg.sender,"19");
                //else require(ERC721Upgradeable(]).ownerOf(audits[_auditID].commits[assayID].tokenID)==msg.sender,"20");
                else require(IERC721(damToken).ownerOf(audits[_auditID].commits[assayID].tokenID)==msg.sender && tokenMarketID[audits[_auditID].commits[assayID].tokenID]==otherMarkets[audits[_auditID].marketID][audits[_auditID].commits[assayID].marketAddressID-1],"20");
                _;
        }
