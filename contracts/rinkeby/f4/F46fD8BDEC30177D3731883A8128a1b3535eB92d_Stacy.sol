...
contract Stacy is Ownable, ERC721, IStacy {
    ...
    // @inheritdoc IStacy
    uint256 public immutable override preSaleStartTimestamp = 1633536000;

    // @inheritdoc IStacy
    uint256 public immutable override saleStartTimestamp = 1633708800;

    //  @inheritdoc IStacy
    uint256 public immutable override preSalePrice = 0.005 ether;
    ...
    //  @inheritdoc IStacy
    address public immutable override chad = 0x82Ed4d970944B84ab640abdC82fc485EFa2a03CC;
    ...
    constructor(string memory contractURI_, string memory baseURI_) ERC721("Te", "TST") {
    ...
    function mintPreSale(uint256[] calldata chadIds) external payable override {
        ...
        if (block.timestamp < preSaleStartTimestamp)
            revert SaleNotStarted(block.timestamp, preSaleStartTimestamp);
        if (block.timestamp >= saleStartTimestamp)
            revert SaleEnded(block.timestamp, saleStartTimestamp);
        // solhint-enable not-rely-on-time

        uint256 length = chadIds.length;
        if (msg.value < preSalePrice * length)
            revert NotEnoughEther(msg.value, preSalePrice * length);
        ...
        _safeMintMultiple(_msgSender(), length);
    }
...
interface IStacy is IERC721 {
    /**
     * @notice Mints number of tokens equal to the length of passed array
     * @param chadIds IDs of Chad NFTs owned by `_msgSender()`
     *
     * Requirements:
     *
     * - `chadIds` should not be previously used in this function call
     * - `block.timestamp` must be greater than the presale start timestamp
    ...
    /**
     * @notice Returns the timestamp of presale (Wednesday, 6 October 2021, 16:00 UTC).
     */
    ...
    /**
     * @notice Returns pre-mint price of each token (0.005 ETH)
     */
