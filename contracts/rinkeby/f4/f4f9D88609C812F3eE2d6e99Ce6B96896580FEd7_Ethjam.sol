...
contract Ethjam is ERC721 {
    ...
    mapping (uint /*timestamp*/ => int32[]) internal priceDataETH;
    mapping (uint /*timestamp*/ => int32[]) internal priceDataBTC;
    ...
    function mint(uint fromTimestamp) public returns (uint256) {
        ...
        int32[] memory ethData = priceDataETH[fromTimestamp];
        int32[] memory btcData = priceDataBTC[fromTimestamp];
        if (ethData.length == 0) {
            int32[] memory ethPrices = fetcher.fetchCoinPriceData(fromTimestamp, Fetcher.Coin.ETH);
            priceDataETH[fromTimestamp] = ethPrices;
        }
        if (btcData.length == 0) {
            int32[] memory btcPrices = fetcher.fetchCoinPriceData(fromTimestamp, Fetcher.Coin.BTC);
            priceDataBTC[fromTimestamp] = btcPrices;
        }
    ...
    function getPriceDataForCoin(uint256 tokenId, Fetcher.Coin coin) internal view returns (int32[] memory) {
        int32[] memory prices;
        if (tokenId == 0) {
            prices = fetcher.fetchCoinPriceData(0, coin);
        } else {
            uint256 fromTimestamp = fromTimestamps[tokenId];
            prices = (coin == Fetcher.Coin.ETH) ? priceDataETH[fromTimestamp] : priceDataBTC[fromTimestamp];
        }
        return prices;
    }
    ...
    function getAllDataForNumDays(uint256 startTimestamp, uint256 numDays) external view returns (DayData[] memory) {
        ...
        for (uint256 i = 0; i < tokenIdTracker.current(); i++) {
            uint fromTimestamp = fromTimestamps[i + 1];
            uint256[] memory tokenIds = tokenIdsForDate[fromTimestamp];
            string memory ethData = Encoding.encode(priceDataETH[fromTimestamp]);
            string memory btcData = Encoding.encode(priceDataBTC[fromTimestamp]);
            string memory bgHue = Encoding.uint2str(bgHueForPriceData(ethData));
            dayData[numDays + i] = DayData(fromTimestamp, tokenIds, ethData, btcData, bgHue);
        }
        return dayData;
    }
    ...
    function getAllDataForDay(uint256 fromTimestamp) public view returns (DayData memory) {
        ...
        uint256[] memory tokenIds = tokenIdsForDate[fromTimestamp];
        string memory ethDataString;
        string memory btcDataString;
        {
            int32[] memory ethData = priceDataETH[fromTimestamp];
            int32[] memory btcData = priceDataBTC[fromTimestamp];
            if (ethData.length == 0) {
                ethData = fetcher.fetchCoinPriceData(fromTimestamp, Fetcher.Coin.ETH);
            }
            if (btcData.length == 0) {
                btcData = fetcher.fetchCoinPriceData(fromTimestamp, Fetcher.Coin.BTC);
            }
            ethDataString = Encoding.encode(ethData);
            btcDataString = Encoding.encode(btcData);
        }
...
contract Fetcher {
    ...
    */
    ...
    function guessSearchRoundsForTimestamp(Coin coin, uint256 fromTime, uint80 daysToFetch) internal view returns (uint80 firstRoundToSearch, uint80 numRoundsToSearch) {
        ...
        uint80 lhRound  = priceFeed.oldestRound;
        uint256 lhTime = priceFeed.oldestTimestamp;
