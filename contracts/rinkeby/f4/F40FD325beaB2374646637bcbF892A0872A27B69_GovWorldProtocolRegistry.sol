...
contract GovWorldProtocolRegistry is GovWorldProtocolBase {
    ...
    uint256 lenderUnearndedAPYPercentage = 20;
    uint256 govUnearndedAPYPercentage = 80;
    ...
    modifier onlyEditAPYPerRole(address admin) {
        require(
            govAdminRegistry.isEditAPYPerAccess(admin) == true,
            "GovProtocolRegistry: No admin right to update or remove Strategic Partner"
        );
        _;
    ...
    */
    function addTokens(address[] memory _tokenAddress, Market[] memory marketData)
        external
        override
        onlyAddTokenRole(msg.sender)
    {
        require(_tokenAddress.length == marketData.length, "GPL: Token Address Length must match Market Data");
        require(tokenMarket != address(0), "Set Token Market First");
        for (uint256 i = 0; i < _tokenAddress.length; i++) {
              //checking Token Contract have not already added
           require(!this.isTokenApproved(_tokenAddress[i]), "GPL: already added Token Contract");
             _addToken(_tokenAddress[i], marketData[i]);
            emit TokensAdded(_tokenAddress[i], marketData[i]);
        }
    }
    ...
    {
        require(_tokenAddress.length == _marketData.length, "GPL: Token Address Length must match Market Data");
    ...
    function removetokens(address[] memory _removeTokenAddress)
        ...
        onlyEditTokenRole(msg.sender)
    {
        for(uint i = 0; i < _removeTokenAddress.length; i++) {
            require(
        ...
        _removeToken(_getIndex(_removeTokenAddress[i], allapprovedTokenContracts));
    ...
    */
     ...
     */
    function setUnearnedAPYPercentageForLender(uint _percentage) public onlyEditAPYPerRole(msg.sender) {
        require(_percentage <= 50, "GPL: Gov Percentage should lower than 50%");
        lenderUnearndedAPYPercentage = _percentage;
    }

      /**
    @dev set the percentage of the unearned APY Fee to the Gov Lend Market Contract
    @param _percentage percentage which goes to lender
     */
    function setUnearnedAPYPercentageForGovLoanMarket(uint _percentage) public onlyEditAPYPerRole(msg.sender) {
        require(_percentage <= 80, "GPL: Gov Percentage should lower than 80%");
        govUnearndedAPYPercentage = _percentage;
    }

    function getUnearnedAPYPercentageForLender() public view override returns(uint256) {
        return lenderUnearndedAPYPercentage;
    }

    function getUnearnedAPYPercentageForGovLoanMarket() public view override returns(uint256) {
         return govUnearndedAPYPercentage;
    }

    function getTokenMarket() external view override  returns(address[] memory) {
        return allapprovedTokenContracts;
    }
...
abstract contract GovWorldProtocolBase is IGovWorldProtocolRegistry, Ownable{
    ...
    function setTokenMarketAddress(address _tokenMarket) external onlyOwner  { //onlyOwner modifier
        require(_tokenMarket != address(0), "Token Market Empty");
        ...
        internal
    {
        ...
        if(marketData.isSP){
            require(_tokenAddress == marketData.gToken, "GPL: gtoken must equal token address");
            ...
            marketData.gToken = address(new GToken(gTokenName, gTokenSymbol, _tokenAddress)); //TODO gtoken decimals?
            GToken(marketData.gToken).transferOwnership(tokenMarket);
     ...
     */
    function _removeToken(uint index) internal {
        for (uint256 i = index; i < allapprovedTokenContracts.length.sub(1); i++) {
