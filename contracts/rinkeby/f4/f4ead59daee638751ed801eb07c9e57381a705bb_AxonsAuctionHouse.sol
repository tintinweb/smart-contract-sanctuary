...
contract AxonsAuctionHouse is IAxonsAuctionHouse, PausableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function createBid(uint256 axonId, uint256 amount) external override nonReentrant {
        ...
        if (lastBidder != address(0)) {
            IERC20(axonToken).transferFrom(address(this), lastBidder, _auction.amount);
        }
        
        // We must check the balance that was actually transferred to the auction,
        // as some tokens impose a transfer fee and would not actually transfer the
        // full amount to the market, resulting in potentally locked funds
        IERC20 token = IERC20(axonToken);
    ...
    function _createAuction() internal {
        try axons.mint() returns (uint256 axonId) {
    ...
    function _settleAuction() internal {
        ...
        if (_auction.amount > 0) {
            IERC20(axonToken).transferFrom(address(this), 0x000000000000000000000000000000000000dEaD, _auction.amount);
        }
