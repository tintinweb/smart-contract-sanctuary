...
contract RideHoldingV2 is IRideHoldingV2, ReentrancyGuard {
     ...
     */
    function depositTokensV2(bytes32 _key, uint256 _amount) external override {
     ...
     */
    function withdrawTokensV2(bytes32 _key, uint256 _amount)
        external
        override
        nonReentrant
    {
    ...
    function getHoldingV2(address _user, bytes32 _key)
        external
        view
        override
        returns (uint256)
...
interface IRideHoldingV2 {
    ...
    function depositTokensV2(bytes32 _key, uint256 _amount) external;

    event TokensRemoved(address indexed sender, uint256 amount);

    function withdrawTokensV2(bytes32 _key, uint256 _amount) external;
