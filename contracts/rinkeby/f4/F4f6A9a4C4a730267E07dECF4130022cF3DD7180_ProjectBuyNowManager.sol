...
import "../ProjectTokenURIManager/ProjectTokenURIManager.sol";
    ...
    UUPSUpgradeable
{
    ...
    modifier onlyValidConfig(BuyNowInfo calldata buyNowInfo) {
        require(
            buyNowInfo.endTime == 0 ||
                (buyNowInfo.endTime > buyNowInfo.startTime && buyNowInfo.endTime > buyNowInfo.publicStartTime),
            "endTime should > startTime or = 0"
        );
        require(
            buyNowInfo.publicStartTime == 0 || buyNowInfo.publicStartTime > buyNowInfo.startTime,
            "publicStartTime should > startTime or = 0"
        );
    ...
    function tokenURI(address project, uint256 tokenId) public view override returns (string memory) {
        ...
        require(editionInfo.printEdition > 0, "ProjectBuyNowManager: nonexistent token");
    ...
    function buyNow(
        ...
        // 4. buy
        _mintToken(_buyNowId, buyNowInfo);
    }

    /// @notice public sale NFT token of specific artwork.
    /// @param  _buyNowId uint256 the id of the buyNow artwork
    function publicSale(uint256 _buyNowId) external payable onlyEOA nonReentrant whenNotPaused {
        // 1. check status first, saving gas if is sold out or over
        BuyNowInfo storage buyNowInfo = buyNowInfos[_buyNowId];
        require(buyNowInfo.edition > 0, "ProjectBuyNowManager: not on sale");
        require(buyNowInfo.edition > saleCounts[_buyNowId], "ProjectBuyNowManager: sold out");
        require(
            buyNowInfo.publicStartTime > 0 && buyNowInfo.publicStartTime <= block.timestamp,
            "ProjectBuyNowManager: public sale not started yet"
        );
        require(
            buyNowInfo.endTime == 0 || buyNowInfo.endTime >= block.timestamp,
            "ProjectBuyNowManager: already ended"
    ...
    function _mintToken(uint256 _buyNowId, BuyNowInfo storage buyNowInfo) internal {
        // 1. get random print edition
        ...
        // 2. mint token to the buyer, uri is empty because it's controlled by #ProjectBuyNowManager.tokenURI function
        uint256 tokenId = buyNowInfo.project.managerMint(_msgSender(), "");
        // 3. record edition info
