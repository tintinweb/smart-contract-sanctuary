...
contract GalaXYKats is ERC721Enumerable, Ownable {
    ...
    function mintPreSale(bytes32[] memory proof) public payable {
        ...
        require(
            totalSupply().add(1) <= PUBLIC_SALE_FIRST_TOKEN_ID,
            "Purchase would exceed max presale quota"
        );
        require(
    ...
    function mintKats(uint256 numberOfTokens) public payable {
        require(saleIsActive, "Sale is not active at the moment");
        require(
            block.timestamp > publicSaleStartTime,
            "public sale not active yet"
        );
        require(
            numberOfTokens > 0,
        ...
        require(
            PUBLIC_SALE_NFT_PRICE.mul(numberOfTokens) == msg.value,
            "Sent ether value is incorrect"
        );
