...
contract LootChestCollectible is BDERC1155Tradable, ERC1155Holder {
    ...
    uint constant CHANCE_BONUS_10       = 200; //2% - Bronze,       4% - Silver,    8% - Gold,      16% - Diamond
    uint constant CHANCE_BONUS_25       = 100; //1% - Bronze,       2% - Silver,    4% - Gold,      8% - Diamond
    uint constant CHANCE_BONUS_50       =  50; //0.5% - Bronze,     1% - Silver,    2% - Gold,      4% - Diamond
    uint constant CHANCE_BONUS_WARRIOR  =  10; //0.1% - Bronze,     0.2% - Silver,  0.4% - Gold,    0.8% - Diamond
    uint constant CHANCE_BONUS_REGION   =   1; //0.01% - Bronze,    0.02% - Silver, 0.04% - Gold,   0.16% - Diamond   
    ...
    uint8 constant RECENT_SALES_TO_TRACK = 32;              //Max of last 32 sales
    ...
    */
    ...
    function onERC1155Received(
        ...
        uint payOutAmount = getBuyBackPrice(_amount);
    ...
    function open(ChestType chestType, uint quantity) public ownersOnly(uint(chestType)) returns (uint fameReward, uint warriors, uint regions) {
        ...
        if(balance >= quantity){
            ...
            for(uint r=0;r<regions;r++){
                ...
                //First we mint:
                uint regionID = RegionContract.trustedCreateRegion();
    ...
    *@return Random roll from 0-10000 (random percentage to 2 decimal places)
    */
    function roll() internal view returns (uint) {
        uint randomUint = uint(keccak256(abi.encodePacked(block.timestamp,blockhash(block.number))));
        return randomUint % 10000;
    }
