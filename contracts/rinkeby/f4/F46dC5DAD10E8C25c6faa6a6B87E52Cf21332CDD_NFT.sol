...
contract NFT is ERC721, PaymentSplitter {
  ...
  uint public saleStartDate = block.timestamp + 1 days; // TODO change to correct duration
  ...
  function isSaleOpen() public view returns(bool){
    ...
    && block.timestamp <= saleStartDate + 122 days){ // TODO change to correct duration
      return true;
    }
    else{
      return false;
  ...
  function mintDogeWhitelist(uint amount, bytes32[] calldata proof) external payable onlyEOA {
    require(isPresaleOpen(), "Presale not open");
    require(mintedTokensByAddress[msg.sender] + amount <= maxMintsPerAddressPresale, "already minted max amount");
