...
supports_interface: HashMap[bytes32, bool]
ERC165_INTERFACE_ID: constant(Bytes[4]) = 0x01ffc9a7
ERC721_INTERFACE_ID: constant(Bytes[4]) = 0x80ac58cd
ERC721METADATA_INTERFACE_ID: constant(Bytes[4]) = 0x5b5e139f
...
def __init__():
    ...
    """
    self.supports_interface[convert(ERC165_INTERFACE_ID, bytes32)] = True
    self.supports_interface[convert(ERC721_INTERFACE_ID, bytes32)] = True
    self.supports_interface[convert(ERC721METADATA_INTERFACE_ID, bytes32)] = True
...
def supportsInterface(_interfaceID: Bytes[4]) -> bool:
    return self.supports_interface[convert(_interfaceID, bytes32)]
...
def tokenURI(_token_id: uint256) -> String[69]:
    ...
    """
    assert self.owner_of[_token_id] != ZERO_ADDRESS, "ERC721Metadata: URI query for nonexistent token"
...
def getApproved(_tokenId: uint256) -> address:
    ...
    """
    assert self.owner_of[_tokenId] != ZERO_ADDRESS, "ERC721: approved query for nonexistent token"
...
def _transfer_from(_from: address, _to: address, _token_id: uint256, _sender: address):
    ...
    """
    assert self._isApprovedOrOwner(_sender, _token_id), "ERC721: transfer caller is not owner nor approved"
    assert _to != ZERO_ADDRESS, "ERC721: transfer to the zero address"
...
def _check_on_erc721_received(_from: address, _to: address, _token_id: uint256, _data: Bytes[1024]):
    if _to.is_contract:
        ...
        assert returnValue == convert(method_id("onERC721Received(address,address,uint256,bytes)"), bytes32), "ERC721: non ERC721Receiver implementer"
...
def approve(_approved: address, _token_id: uint256):
    ...
    assert _approved != owner, "ERC721: approve to current owner"
...
def setApprovalForAll(_operator: address, _approved: bool):
    ...
    """
    assert _operator != msg.sender, "ERC721: approve to caller"
