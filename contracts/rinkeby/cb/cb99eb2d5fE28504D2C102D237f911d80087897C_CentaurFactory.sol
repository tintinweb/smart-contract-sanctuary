...
contract CentaurFactory is ICentaurFactory, Ownable {
    ...
    constructor(address _poolLogic, address _cloneFactory, address _WETH) public {
        ...
        settlementContract.transferOwnership(msg.sender);
    ...
    function setRouter(address _router) external onlyOwner override {
        router = _router;
    }
}
...
contract CentaurPool is Initializable, CentaurLPToken {
    ...
    function mint(address to) external lock depositAllowed returns (uint liquidity) {
    ...
    function burn(address to) external lock withdrawAllowed returns (uint amount) {
