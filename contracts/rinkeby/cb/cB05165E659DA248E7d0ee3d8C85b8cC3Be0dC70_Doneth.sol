...
contract Doneth is Ownable {
    ...
    function getMemberAtKey(uint key) public view returns(address) {
    ...
    function getContractInfo() public view returns(string memory name, address owner,  uint256 genesisBlockNumber,  uint256 totalShares,  uint256 totalWithdrawn) {
        return (string(name), owner, genesisBlockNumber, totalShares, totalWithdrawn);
    }
    
    function returnMember(address _address) public view onlyExisting(_address) returns(bool admin, uint256 shares, uint256 withdrawn, string memory memberName) {
      Member memory m = members[_address];
      return (m.admin, m.shares, m.withdrawn, m.memberName);
    }

    function checkERC20Balance(address token) public view returns(uint256) {
    ...
    function addMember(address who, uint256 shares, bool admin, string memory memberName) public onlyAdmin() {
    ...
    function updateMember(address who, uint256 shares, bool isAdmin, string memory name) public onlyAdmin() {
        if (keccak256(abi.encode(members[who].memberName)) != keccak256(abi.encode(name))) changeMemberName(who, name);
    ...
    function changeMemberName(address who, string memory newName) public onlyExisting(who) {
        if (msg.sender != who && msg.sender != owner && !members[msg.sender].admin) revert();
        if (bytes(newName).length > 21) revert();
        emit ChangeMemberName(who, members[who].memberName, newName);
        members[who].memberName = newName;
    }

    function changeAdminPrivilege(address who, bool newValue) public onlyAdmin() {
        emit ChangePrivilege(who, members[who].admin, newValue);
        members[who].admin = newValue; 
    }

    // Only admins and owners can change the contract name
    function changeContractName(string memory newName) public onlyAdmin() {
        if (bytes(newName).length > 21) revert();
        emit ChangeContractName(name, newName);
        name = newName;
    }
    ...
    function changeSharedExpenseAllocation(uint256 newAllocation) public onlyOwner() {
        ...
        if (newAllocation.sub(sharedExpenseWithdrawn) > address(this).balance) revert();

        emit ChangeSharedExpense(address(this).balance, sharedExpense, newAllocation);
    ...
    function addShare(address who, uint256 amount) public onlyAdmin() onlyExisting(who) {
        ...
        emit AddShare(who, amount, members[who].shares);
    }
    ...
    function removeShare(address who, uint256 amount) public onlyAdmin() onlyExisting(who) {
        ...
        emit RemoveShare(who, amount, members[who].shares);
    }
    ...
    function withdraw(uint256 amount) public onlyExisting(msg.sender) {
        ...
        emit Withdraw(msg.sender, amount, totalWithdrawn);
    }
    ...
    function withdrawToken(uint256 amount, address token) public onlyExisting(msg.sender) {
        ...
        emit TokenWithdraw(msg.sender, amount, token, tokens[token].totalWithdrawn);
    }
    ...
    function withdrawSharedExpense(uint256 amount, address payable to) public onlyAdmin() {
        if (amount > calculateTotalExpenseWithdrawableAmount()) revert();
        
        sharedExpenseWithdrawn = sharedExpenseWithdrawn.add(amount);
        to.transfer(amount);
        emit WithdrawSharedExpense(msg.sender, to, amount, sharedExpenseWithdrawn);
    }
    ...
    function calculateTotalWithdrawableAmount(address who) public view onlyExisting(who) returns (uint256) {
        // Total balance to calculate share from = 
        // contract balance + totalWithdrawn - sharedExpense + sharedExpenseWithdrawn
        uint256 balanceSum = address(this).balance.add(totalWithdrawn);
    ...
    function calculateTotalWithdrawableTokenAmount(address who, address token) public view returns(uint256) {
    ...
    function calculateTotalExpenseWithdrawableAmount() public view returns(uint256) {
        return sharedExpense.sub(sharedExpenseWithdrawn);
    }

    // Used for testing
    function delegatePercent(uint256 a, uint256 b, uint256 c) public view returns (uint256) {
