...
contract SGDToken is ERC20,ERC20Burnable,AccessControl{
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
    uint256 public  maxTotalSupply = 700;

    constructor(address admin,address admin2) ERC20 ("SGDToken", "SGDT") {
        _setupRole(ADMIN_ROLE,admin);
        _setupRole(ADMIN_ROLE,admin2);
    }
    
    function checkAdmin(address msgUser) public view returns(bool){
         require(hasRole(ADMIN_ROLE, msgUser), "Caller is not a admin");
         return true;
    }
    
    function setMaxTotalSupply(uint256 amount) public{
    ...
    function supply (uint256 amount) public{
        require(hasRole(ADMIN_ROLE, msg.sender), "Caller is not a admin");
        uint256 totalSupplyNum = totalSupply();
        require(totalSupplyNum+amount <= maxTotalSupply,"Exceed total supply limit");
...
contract SGDTAirdrop {
    ...
    // SGDToken sgdToken = SGDToken(address(0xd1371fd923f51c1f260f2716ffa3763340b94ea3));
    ...
    // function airDrop(address[] memory walletAddresses) public { 
