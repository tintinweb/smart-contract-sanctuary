...
contract AuctionERC1155 {
    ...
    function ERC1155FixedAuction(
        ...
        // NFTContract.safeTransferFrom(myAuction.owner, to, tokenId);(msg.sender, myAuction.productId);
				uint256 serviceFeePercent =getServiceFeePercent(_listingType);

				uint256 serviceFeeAndComission = msg.value.mul(serviceFeePercent.add(commission)).div(10**4);
    ...
    */
        ...
        returns (History[] memory)
    {
		...
		function getServiceFeePercent(uint256 listingType) public view returns (uint256) {
        if(listingType == 1){
					return 250;
				} 
				return 2000;
