...
contract CollectionManager is ICollectionManager {
  ...
  function createCollection(string memory name, string memory symbol) external {
    ERC721 newCollection = new ERC721(name, symbol);
  ...
  function mint(address collectionAddress, uint256 tokenId) external {
    ERC721(collectionAddress)._safeMint(msg.sender, tokenId);
  }
