...
contract MoneyPot {
    ...
    // money pot internal variables : 
    uint256 public accRewardPerShare;
    uint256 public rewardableShare; // total number of share that are allowed to earn rewards

    mapping (address => bool) public addressUnrewardable; // mapping of all address that are excluded from money pot reward distribution
    ...
    mapping (address => uint256) public usersHoldingBonus; // each user have his actual milestone (to avoid error if milestones are edit)

    mapping (address => uint256) public usersShareBonusFromPapr; // user actual bonus from papr
    mapping (address => uint256) public usersShareBonusFromHoldingTime; // user actual bonus from holding time
    ...
    constructor (address _sDFIANCE, address _rewardToken, address _feeManager, address _papr, uint256 _PAPR_STACKED_REWARD_PERCENT, uint256 _MAX_PAPER_AMOUNT_PER_SHARE_PERCENT, uint256 _BONUS_SHARE_PER_PAPER) public {
        sDFIANCE = _sDFIANCE;
        ...
        PAPR_STACKED_REWARD_PERCENT = _PAPR_STACKED_REWARD_PERCENT;
    ...
    * -if newMilestoneAvailable = false, the actual milestone is the best that the user can get, if it = true, there is a better milestone that the user can get
    * -percentBonus indicate the percentBonus of the better milestone (if there is one, else, its at 0)
    */
    function getUpdateMilestone(address _account) public returns (bool, uint) {
        ...
        for (uint i=0; i < milestones.length; i++) {
            if (holdingTime >= milestones[i].minimumHoldingHours) { // if the milestone [i] is low enough for the user ..
                if (userPercentBonus < milestones[i].percentBonus) {
                    return (true, milestones[i].percentBonus); // set milestone [i] to his
                }
            }
        }
    ...
    // return (bool : if true, the user has a milestone, if false he hasn't) (uint : id of the new milestone)
    function getRealMilestone(address _account) public returns (bool, uint) {
        uint256 holdingTime = getAvgSwapTimeInHours(_account);

        for (uint i=0; i < milestones.length; i++) {
            if (holdingTime >= milestones[i].minimumHoldingHours) { // if the milestone [i] is low enough for the user ..
                return (true, i); // set milestone [i] to his
            }
        }
    ...
    // function called by the user to move up a milestone if there is a higher one available
    function updateMilestone() public {
        _updateMilestone(msg.sender);
    }

    // function called automatically by the sDFIANCE contract when a user -mint, burn or transfer- token to update bonus to the real milestone 
    function updateMilestoneOf(address _of) public only_sDFIANCE {
        _updateMilestone(_of);
    }

    // update _account milestone to the real one
    function _updateMilestone(address _account) private {
        uint256 shareAmount = userInfo[_account].amount;
        (bool isAvalaible, uint256 milestoneId) = getRealMilestone(_account);

        uint256 newShareHoldingBonus = (isAvalaible) ? milestones[milestoneId].percentBonus.mul(shareAmount).div(10000) : 0;

        if (newShareHoldingBonus != usersShareBonusFromHoldingTime[_account]) {
            uint256 newTotalAmount = shareAmount.sub(usersShareBonusFromHoldingTime[_account]).add(newShareHoldingBonus);

            usersShareBonusFromHoldingTime[_account] = newShareHoldingBonus;
    ...
    */
    function updatePaprBonusShare(address _account) external {
        require(addressUnrewardable[_account] == false);

        uint256 shareAmount = userInfo[_account].amount;
        uint256 paprAmount = IERC20(papr).balanceOf(_account);
        ...
        uint256 newSharePaprBonus = BONUS_SHARE_PER_PAPER.mul(paprAmountBonus).div(10000);

        if (newSharePaprBonus != usersShareBonusFromPapr[_account]) {
            uint256 newTotalAmount = shareAmount.sub(usersShareBonusFromPapr[_account]).add(newSharePaprBonus);

            usersShareBonusFromPapr[_account] = newSharePaprBonus;
    ...
    // Update reward variables of the pot with new reward is _amount (note : called privately when new reward Token are deposited)
    function updatePot(uint256 _amount) private {

        uint256 shareSupply = rewardableShare; // take the number of shares that are allowed to earn rewards 
        accRewardPerShare = accRewardPerShare.add(_amount.mul(ACC_REWARD_PRECISION).div(shareSupply)); // update moneyPot accumulated reward
    }
    ...
    function _lessHolder(address _from, uint256 _amount) private {
        ...
        if (addressUnrewardable[_from] == false) {
            ...
            //subtracting the amount from the number of lptokens user had staked
            if(_amount > 0) {
    ...
    function transferHolder(address _from, address _to, uint256 _amount) external only_sDFIANCE {
        ...
        // if the _from address is excluded from the money pot, 0 update is required for it
        if (addressUnrewardable[_from] == false) {

            // harvest the "_from" reward
            _harvestReward(_from);

            _fromUser.amount -= _amount;
        ...
        // if the _to address is excluded from the money pot, 0 update is required for it
        if (addressUnrewardable[_to] == false) {

            // harvest the "_to" reward
            _harvestReward(_to);

            _toUser.amount += _amount;
    ...
    function getPendingReward(address _account) public view returns (uint256) {
        ...
        uint256 _pendingReward = user.amount.mul(accRewardPerShare).div(ACC_REWARD_PRECISION).sub(user.rewardDebt); // get basic reward
