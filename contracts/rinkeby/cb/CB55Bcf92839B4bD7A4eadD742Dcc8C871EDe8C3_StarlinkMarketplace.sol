...
contract StarlinkMarketplace is Ownable, ReentrancyGuard {
    ...
    function updateListing(
        ...
        listedItem.isEtherBased = _isEtherBased;
        emit ItemUpdated(_msgSender(), _nftAddress, _tokenId, _newPrice, _isEtherBased);
    }
    ...
    function buyItem(
        ...
        if (listedItem.isEtherBased) {
            ...
            uint256 feeAmount = payAmount.mul(platformFee).div(1e3);
            _approveHelper(erc20Token, address(this), uint256(~0));
            erc20Token.safeTransferFrom(_msgSender(), feeReceipient, feeAmount);
            erc20Token.safeTransferFrom(_msgSender(), _owner, payAmount.sub(feeAmount));
        }
        ...
        emit ItemSold(_owner, _msgSender(), _nftAddress, _tokenId, listedItem.quantity, msg.value.div(listedItem.quantity), listedItem.isEtherBased);
    ...
    function createOffer(
        ...
        emit OfferCreated(_msgSender(), _nftAddress, _tokenId, _quantity, _pricePerItem, _deadline, _isEtherBased);
    }
    ...
    function acceptOffer(
        ...
        if (offer.isEtherBased) {
            (bool feeTransferSuccess,) = feeReceipient.call{value : feeAmount}("");
            require(feeTransferSuccess, "StarlinkMarketplace: Fee transfer failed");
            (bool ownerTransferSuccess,) = _msgSender().call{value : price.sub(feeAmount)}("");
            require(ownerTransferSuccess, "StarlinkMarketplace: Owner transfer failed");
        } else {
            erc20Token.safeTransferFrom(_creator, feeReceipient, feeAmount);
            erc20Token.safeTransferFrom(_creator, _msgSender(), price.sub(feeAmount));
        }
        ...
        emit ItemSold(_msgSender(), _creator, _nftAddress, _tokenId, offer.quantity, offer.pricePerItem, offer.isEtherBased);
