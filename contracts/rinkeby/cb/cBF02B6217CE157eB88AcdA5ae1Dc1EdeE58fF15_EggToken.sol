...
contract EggToken is ERC20("EGG", "EGG"), Ownable{
    ...
    uint256 public addUpClaim;
    uint256 public addUpBurn;
    uint256 public inCirculation;
    uint256 public awaitMining;
    ...
    function getUserClaimCountPerDay(uint256 _dayIndex) public view returns(uint256){
        return userClaimCountPerDay[_dayIndex][msg.sender];
    }
    ...
    function claim(uint256 orderId, uint256 floatAmount, bytes memory signature) public returns(address){
        ...
        updateTime();
        uint256 maxAmountPerUserPerDay = userClaimCountPerDay[dayIndex][msg.sender];
        require(maxAmountPerUserPerDay + floatAmount <= perUserPerDayLimit, 'Maximum single day limit exceeded！');
        uint256 maxAmountMarketPerDay = marketClaimCountPerDay[dayIndex];
        require(maxAmountMarketPerDay + floatAmount <= marketPerDayLimit, 'It has exceeded the maximum market quota！');
        ...
        userClaimCountPerDay[dayIndex][msg.sender] = maxAmountPerUserPerDay + floatAmount;
        marketClaimCountPerDay[dayIndex] = maxAmountMarketPerDay + floatAmount;

        addUpClaim += floatAmount;
        inCirculation += floatAmount;
        awaitMining = MAX_TOTAL_SUPPLY - addUpClaim * 1e18;
    ...
    function mint(address _to, uint256 _amount) internal{
        ...
        if(totalSupply ==  MAX_TOTAL_SUPPLY){
            return;
        }else if(totalSupply.add(_amount) <= MAX_TOTAL_SUPPLY){
            _mint(_to, _amount);
        }else{
    ...
    function burn(uint256 amount) public{
        ...
        updateTime();
        burnAmountPerDay[dayIndex] = burnAmountPerDay[dayIndex] + amount;
        inCirculation = addUpClaim - amount;
    }

    function setPerUserPerDayLimit(uint _perUserPerDayLimit) public onlyOwner {
        perUserPerDayLimit = _perUserPerDayLimit;
    }

    function setMarketPerDayLimit(uint _marketPerDayLimit) public onlyOwner {
