...
contract TronicRacer is ERC721, Ownable, WithIPFSMetaData, WithContractMetaData, WithWithdrawals {
    ...
    // In seconds. Note: subtract 1 second when initializing to reduce gas when comparing
    ...
    function mintWithMiniRacer(uint256 amount, uint256[] memory tokenIds) external payable {
        require(miniRacerSaleStarted(), "Mini Racer MOGO has not started");

        require(totalSupply() + amount + tokenIds.length <= MAX_SUPPLY, "Racers are sold out!");
        require(amount > 0, "You must mint at least one.");
        require(amount >= tokenIds.length, "You can only redeem as many as you purchase.");
        ...
        for (uint256 t = 0; t < tokenIds.length; t++) {
            require(_ownsMiniRacer(tokenIds[t]) == true, "You must own the mini racer token provided.");
            require(miniRacerAvailableToRedeem(tokenIds[t]) == true, "One of the mini racers was already redeemed.");
            redeemedMiniRacers[tokenIds[t]] = 1;
            _mintAndIncrement();
        }
