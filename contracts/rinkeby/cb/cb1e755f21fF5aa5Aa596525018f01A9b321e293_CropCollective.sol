...
contract CropCollective is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {    
    ...
    uint public royalityOwnerFeePercent = 5;
    ...
    function setFee(uint256 _teamFeePercent, uint256 _charityFeePercent, uint256 _royalityOwnerFeePercent, uint256 _royalityTeamFeePercent, uint256 _totalFeeUnitPercent) external onlyOwner{
        charityFeePercent = _charityFeePercent;
        teamFeePercent = _teamFeePercent;
        royalityOwnerFeePercent = _royalityOwnerFeePercent;
    ...
    function mint(uint256 amount) public payable {
        ...
        require(totalSupply() + amount <= MaxSupply, 'max public sale amount is limited.');        
        for (uint256 i = 0; i < amount; i++) {
            uint256 tokenId = randomPickId();
            internalMint(msg.sender, tokenId);
        }
