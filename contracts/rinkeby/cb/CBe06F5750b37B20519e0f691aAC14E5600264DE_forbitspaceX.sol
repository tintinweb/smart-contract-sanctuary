...
contract forbitspaceX is Payment {
	...
	{
		(address tokenIn, address tokenOut, uint amountInTotal, address recipient, SwapParam[] calldata sParams) = (
			aParam.tokenIn,
			aParam.tokenOut,
			aParam.amountInTotal,
			aParam.recipient,
			aParam.sParams
		);
...
abstract contract Payment is StorageUpgradeable {
	...
	function balanceOf(address token) internal view returns (uint bal) {
		...
		bal = IERC20Upgradeable(token).balanceOf(address(this));
	}
