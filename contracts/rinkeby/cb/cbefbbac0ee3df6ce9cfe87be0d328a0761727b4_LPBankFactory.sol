...
import "./LPBank.sol";
import "./interfaces/ILPBank.sol";
import "./libraries/Initializable.sol";
import "./libraries/Pausable.sol";
import "./libraries/SafeMath.sol";

contract LPBankFactory is Pausable, Initializable {
    using SafeMath for uint256;

    // Array of created Pool Address
    address[] public allLPBanks;
    // Mapping from User token. From tokens to array of created LPBank for token
    mapping(address => mapping(IERC20 => address[])) public getLPBanks;
    ...
    /**
     * @notice Get the number of all created pool
     * @return Return number of created pool
     */
    function allLPBanksLength() public view returns (uint256) {
        return allLPBanks.length;
    }

    /**
     * @notice Get the created pool by token address
     * @dev User can retrieve their created pool by address of tokens
     * @param _creator Address of created pool user
     * @param _lpToken Address of LP token want to query
     * @return Created LPBank Address
     */
    function getCreatedLPBanksByToken(address _creator, IERC20 _lpToken)
        public
        view
        returns (address[] memory)
    {
        return getLPBanks[_creator][_lpToken];
    }

    /**
     * @notice Retrieve number of pool created for specific token
     * @param _creator Address of created campaign user
     * @param _lpToken Address of LP token want to query
     * @return Return number of created LPBank
     */
    function getCreatedLPBanksLengthByToken(address _creator, IERC20 _lpToken)
        public
        view
        returns (uint256)
    {
        return getLPBanks[_creator][_lpToken].length;
    }

    /**
     * @notice Register Pool
     * @dev To register, you MUST have an ERC20 token
     * @param _name String name of new LPBank
     * @param _lpToken address of ERC20 LP token users will deposit
     * @param _rewardToken address of ERC20 token to distribute as rewards
     */
    function registerLPBank(
        string memory _name,
        IERC20 _lpToken,
        IERC20 _rewardToken
    ) external whenNotPaused onlyOwner returns (address pool) {
        require(_lpToken != IERC20(address(0)), "LPBankFactory::ZERO_ADDRESS");
        require(
            _rewardToken != IERC20(address(0)),
            "LPBankFactory::ZERO_ADDRESS"
        );
        bytes memory tempNameString = bytes(_name);
        require(tempNameString.length != 0, "ICOFactory::NO_NAME");

        bytes memory bytecode = type(LPBank).creationCode;
        uint256 tokenIndex = getCreatedLPBanksLengthByToken(
            _msgSender(),
            _lpToken
        );
        bytes32 salt = keccak256(
            abi.encodePacked(_msgSender(), _lpToken, tokenIndex)
        );
        assembly {
        ...
        ILPBank(pool).initialize(_name, _lpToken, _rewardToken);
        getLPBanks[_msgSender()][_lpToken].push(pool);
        allLPBanks.push(pool);
...
contract LPBank is ReentrancyGuard, Pausable {
    using SafeMath for uint256;
    ...
    // User reward per token paid mapping
    ...
    event PoolCreated(string name, address lpToken, address rewardToken);
    event RewardAdded(uint256 reward);
    event Staked(address indexed user, uint256 amount);
    event Composed(address indexed user, uint256 amount);
     ...
     * @param _lpToken Address of the token being staked
    ...
    function initialize(
        ...
        emit PoolCreated(name, address(lpToken), address(rewardToken));
    }
    ...
    /**
     * @notice Owner can set the LP token contract(IERC20)
     * @param _token Address of the new LP token
     */
    function setLPTokenTokenContract(IERC20 _token) external onlyOwner {
        lpToken = _token;
    }
