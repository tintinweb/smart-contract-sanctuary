...
contract ViralswapVault {
    ...
    function initialize(uint _tokenOutPerTokenIn, address _tokenIn, address _tokenOut, address _viralswapRouter02) external {
        ...
        // todo : check pair exists
        (uint swapReserveIn, uint swapReserveOut) = _getReserves(_tokenIn, _tokenOut);
        // todo : uppercase error
        require(swapReserveIn > 0 && swapReserveOut > 0, "Viralswap: No liquidity in pool");
    }
    ...
    function _getReserves(address _tokenIn, address _tokenOut) internal view returns (uint _reserveIn, uint _reserveOut){
    ...
    function buy(uint amountOut, address to) external lock {
        ...
        if(amountIn > amountInExpected) {
            _safeTransfer(_tokenIn, to, amountIn.sub(amountInExpected));
        }

        (uint swapReserveIn, uint swapReserveOut) = _getReserves(_tokenIn, _tokenOut);
        uint amountOutOptimal = _quote(amountInExpected, swapReserveIn, swapReserveOut);
