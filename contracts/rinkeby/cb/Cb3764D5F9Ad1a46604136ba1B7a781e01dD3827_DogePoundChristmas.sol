...
contract DogePoundChristmas is ERC721Burnable {
    ...
    address private wallet = 0x2075252c9e00381FCDDdB0f0967B6Cb7892ca745;
    address private admin = 0xe2f071Fa6a421c6Bd7DF3635cC001c569182B950;

    string public constant CONTRACT_NAME = "Dogepound Christmas Contract";
    bytes32 public constant DOMAIN_TYPEHASH = keccak256("EIP712Domain(string name,uint256 chainId,address verifyingContract)");
    bytes32 public constant MINT_TYPEHASH = keccak256("Claim(address user)");
    ...
    * Set mintLimit flag
    */
    function changePresaleStatus(bool _mintLimit) external onlyOwner {
    ...
    * Set whitelist count
    */
    function setSaleStatus(uint256 _whitelistCount) external onlyOwner {
    ...
    */
    function claimDPC(uint8 v, bytes32 r, bytes32 s) external {
        require(tx.origin == msg.sender, "Only EOA");
        require(!whitelistClaim[msg.sender], "Already claimed");
        require(currentMintCount < MAX_DPC_SUPPLY, "Exceed max supply");

        bytes32 domainSeparator = keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes(CONTRACT_NAME)), getChainId(), address(this)));
        bytes32 structHash = keccak256(abi.encode(MINT_TYPEHASH, msg.sender));
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, structHash));
        address signatory = ecrecover(digest, v, r, s);
        require(signatory == admin, "Invalid signatory");

        _safeMint(msg.sender, currentMintCount);
        currentMintCount = currentMintCount.add(1);
