...
contract HewerClanMergeProtocol is Ownable, ERC721Enumerable {
    ...
    function getOwnerTagTokenIdsAvailableForMerge(address _owner) public view returns (uint [] memory) {
        ...
        for (uint i = 0; i < balanceWithoutMergedTokens; i++) {
            if (allTokenIds[i] != - 1) {
                tokenIds[i] = hewerClan.tokenOfOwnerByIndex(_owner, uint(allTokenIds[i]));
            }
        }
    ...
    function getOwnerNftTokenIdsAvailableForMerge(address _owner, string memory _name) public view returns (uint [] memory) {
        if (collectionIsRegistered(_name)) {
            ...
            for (uint i = 0; i < balanceWithoutMergedTokens; i++) {
                if (allTokenIds[i] != - 1) {
                    tokenIds[i] = collection.tokenOfOwnerByIndex(_owner, uint(allTokenIds[i]));
                }
            }
    ...
    function burn(uint256 tokenId) public {
        ...
        delete tagTokenIdToMergeState[entry.tagTokenId];
        delete collectionNftTokenIdToMergeState[entry.collectionName][entry.tokenId];
