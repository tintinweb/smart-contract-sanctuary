...
contract DPSTest is Ownable, ERC721Enumerable {
  ...
  mapping(uint256 => Attributes) attributesPerToken;
  ...
  function mint(address _owner, uint256 _tokenId) external onlyOwner {
    ...
    attributesPerToken[_tokenId] = Attributes(generateRandom(1), generateRandom(2), generateRandom(3));
  ...
  function getAttributesByToken(uint256 _tokenId) external view returns (Attributes memory) {
    return attributesPerToken[_tokenId];
  }
  ...
  function generateRandom(uint16 nonce) internal view returns (uint16) {
    uint16 rand = uint16(uint256(keccak256(abi.encodePacked(msg.sender, block.difficulty, nonce, block.timestamp))));
    return (rand % 150) + 150;
  }
