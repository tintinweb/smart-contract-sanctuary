    ...
    ReentrancyGuard
{
    ...
    function rebalance(
        ...
        require(msg.sender == strategy || msg.sender == governance, "strategy || governance");
    ...
    //function rebalanceTokens() external {
        //uint256 balance0 = getBalance0();
        //uint256 balance1 = getBalance1();
        //uint128 liquidity = _liquidityForAmounts(baseLower, baseUpper, balance0, balance1);
        //require(liquidity > 0, "liquidity");
        //(uint256 amount0, uint256 amount1) = _amountsForLiquidity(baseLower, baseUpper, liquidity);
        //require(amount0 != amount1, 'amount');

        //int256 swapAmount = int256(balance0.sub(amount0));
        //bool zeroForOne = swapAmount > 0;
        //pool.swap(
            //address(this),
            //zeroForOne,
        ...
        //require(balance0 >= amount0, 'balance0>=amount0');
        //require(balance1 >= amount1, 'balance1>=amount1');
    //}
        ...
        )
    {
        ...
        reward0 = reward0 + feesToVault0;
        reward1 = reward1 + feesToVault1;
    ...
    function emergencyWithdraw() external onlyGovernance {
        (uint128 limitLiquidity, , , , ) = _position(baseLower, baseUpper);
        pool.burn(baseLower, baseUpper, limitLiquidity);
        pool.collect(address(this), baseLower, baseUpper, type(uint128).max, type(uint128).max);
    ...
    function emergencyBurnPositions() external onlyGovernance {
        (uint128 limitLiquidity, , , , ) = _position(baseLower, baseUpper);
        pool.burn(baseLower, baseUpper, limitLiquidity);
        pool.collect(address(this), baseLower, baseUpper, type(uint128).max, type(uint128).max);
    }

    function emergencyTransferToGovernance() external onlyGovernance {
        token0.safeTransfer(governance, token0.balanceOf(address(this)));
        token1.safeTransfer(governance, token1.balanceOf(address(this)));
        accruedProtocolFees0 = 0;
        accruedProtocolFees1 = 0;
    ...
    function resetReward() external onlyGovernance {
        reward0 = 0;
        reward1 = 0;
    }
