...
contract TheDoolniz     is ERC721 
{
    ...
    modifier requiredPresalesDate() 
    {
        require(presalesDate != 0 && presalesDate <= block.timestamp, "Presales did start yet");
        require(presalesDate < salesDate,                             "Presales cannot be after the sales date");
        _;
    }
    modifier requiredSalesDate() 
    {
        require(salesDate != 0 && salesDate <= block.timestamp, "Sales did start yet");
        _;
    }
    ...
    { 
        ...
        uint256 cut;                // a share amount from >0 t 100
    }
...
*/
    ...
    //-------------------------------------------------------------------
    function    getTokenIndex(uint16 idx) public view returns(uint16)
    {
        return tokenIdsMap[idx];
    }
    ...
    function    setPartners(TPartners[] memory partnerList) external onlyOwner
    {
        ...
        for (i=0; i<partnerList.length; i++)        // no problem, on ajout
        {
