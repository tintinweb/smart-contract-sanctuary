...
contract PresaleContract is Ownable, ReentrancyGuard {
    ...
    event Transferred(address indexed tokenAddress, address indexed to, uint indexed amount, uint nonce);
    event WithdrawnLostTokens(address indexed owner, address tokenAddress, uint amountWithdrawn);
    
    constructor(address _verifyAddress) public {
    ...
    {
        bytes32 message = prefixed(keccak256(abi.encode(address(this), _tokenAddress, msg.sender, _amount, _nonce)));
        ...
        emit Transferred(_tokenAddress, msg.sender, _amount, _nonce);
    }
