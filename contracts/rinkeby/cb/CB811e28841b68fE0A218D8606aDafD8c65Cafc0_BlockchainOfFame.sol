...
contract BlockchainOfFame is ERC721Enumerable, Ownable {
    ...
    address public creator1Address;
    address public creator2Address;
    ...
    function totalToken() public view returns (uint256) {
     ...
     function mintBof(uint numberOfTokens) public payable {
        require(saleIsOpen, "Sale must be active to mint NFT");
        require(numberOfTokens > 0 && numberOfTokens <= maxBofPurchase, "Can only mint 10 tokens at a time");
        require(msg.value == PRICE.mul(numberOfTokens), "Ether value sent is not correct");
        require(max_mints_per_address[msg.sender].add(numberOfTokens) <= 20,"Max 20 mints per wallet allowed");
    ...
    function withdrawAll() public onlyOwner {
        ...
        require(balance > 0);
        _widthdraw(creator2Address, balance.mul(40).div(100));
    ...
    function whitelistedMints(uint numberOfTokens, bytes32[] calldata merkleProof ) payable external  {
        ...
        require(numberOfTokens > 0 && numberOfTokens <= 10, "Can only mint 10 tokens at a time");
        require(msg.value == PRICE.mul(numberOfTokens), "Ether value sent is not correct");
        require(max_mints_per_address[msg.sender].add(numberOfTokens) <= 20,"Max 20 mints allowed per whitelisted wallet");
