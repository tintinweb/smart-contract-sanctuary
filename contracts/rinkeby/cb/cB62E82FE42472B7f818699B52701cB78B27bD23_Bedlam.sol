...
contract Bedlam is Ownable, ReentrancyGuard, BaseERC721, Merkle, PhaseOne {          
     ...
     */
    function phaseOneMint(uint id, uint rarity, bytes32[] calldata proof) external nonReentrant {
        uint current = totalSupply();
        require(isPhaseSaleActive, "Not Live");
        require(opensea.balanceOf(msg.sender, id) != 0, "Not owner");
...
contract Merkle {    
    ...
    function _phaseOneLeaf(uint256 tokenId, uint256 random)
    internal pure returns (bytes32)
    {   
        return keccak256(abi.encodePacked(random, tokenId.toString()));
    }
...
contract PhaseOne is Ownable {
    ...
    mapping(uint256 => PhaseOneToken) internal phaseOne;
    ...
    function updatePhaseOne(uint256 id, uint256 tokenId, uint256 rarity) internal {
