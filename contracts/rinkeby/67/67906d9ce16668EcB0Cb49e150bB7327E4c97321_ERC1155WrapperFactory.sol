...
contract ERC1155WrapperFactory is UpgradeableBeacon {
    ...
    mapping(address => bool) public marketWrapped;
    ...
    function createWrappers(
        ...
        IMarketMaker _marketMaker = IMarketMaker(_marketMakerAddress);
        bytes32[] memory _conditionIds = _marketMaker.conditionIds();
        address _collateralToken = _marketMaker.collateralToken();
        uint256 _nameSymbolIndex = 0;
        for (uint256 _i = 0; _i < _conditionIds.length; _i++) {
            uint256 _outcomesCount =
                conditionalTokens.getOutcomeSlotCount(_conditionIds[_i]);
            for (uint256 _j = 0; _j < _outcomesCount; _j++) {
                ...
                deployWrapper(
                    _symbols[_nameSymbolIndex],
                    _names[_nameSymbolIndex],
        ...
        marketWrapped[_marketMakerAddress] = true;
    }
...
interface IConditionalTokens {
    function getOutcomeSlotCount(bytes32 _conditionId)
        external
        view
        returns (uint256);
    ...
    function getPositionId(address _collateralToken, bytes32 _collectionId)
        external
        pure
        returns (uint256);
}
...
interface IMarketMaker {
    ...
    function conditionIds() external view returns (bytes32[] memory);
}
