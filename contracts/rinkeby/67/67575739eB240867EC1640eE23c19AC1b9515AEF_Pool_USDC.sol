...
library ArthPoolLibrary {
        ...
        )
    {
        ...
        uint256 arthValueToMint = collateralValue.mul(collateralRatio).div(1e18);
        ...
        )
    {
        ...
        return (arthAmount.add(arthxValueNeeded).mul(1e18).div(collateralPriceGMU)
        );
    }
...
interface IARTHPool {
    ...
    function mint(uint256 collateralAmount, uint256 arthOutMin, uint256 cid)
        external
       returns (uint256);

    function redeem(uint256 arthAmount, uint256 collateralOutMin, uint256 cid)
        external;
...
contract TestArthPool is AccessControl, IARTHPool {
    ...
    mapping (uint256 => mapping (address => amountsRecord)) public amountsbycid;
    ...
    struct CollateralInfo {
        IERC20 collateralAddress;           // Address of the collateral
    }
    ...
    //uint256 private constant _COLLATERAL_RATIO_MIN = 1e18 + 1; // 100.0001 in 1e18 precision.
    uint256 private constant _COLLATERAL_RATIO_PRECISION = 1e18;
    ...
    uint256 public pcr;
    uint256 public mockUSDCPrice = 1e18;
    ...
    constructor(
        ...
        collateralInfo.push(CollateralInfo({
            collateralAddress: _COLLATERAL
        }));
    }

// View functions for collaterals // 


    // colalteral ids
    function collateralLength() external view returns (uint256) {
        return collateralInfo.length;
    }

// Set functions //

    function setCollPrice(uint256 _newPrice) public {
    mockUSDCPrice = _newPrice;

    }

    function addCollateral(IERC20 _collateralAddress) public onlyByOwnerOrGovernance {
        collateralInfo.push(CollateralInfo({
...
// End Set functions //
    ...
    function mint(
        ...
        CollateralInfo storage collat = collateralInfo[cid];
        uint256 collateralAmountAfterFees = (collateralAmount.mul(uint256(1e18).sub(_arthController.getMintingFee()))).div(1e18);
        uint256 collateralAmountD18 = collateralAmountAfterFees;
        ...
        require(
            (collat.collateralAddress.balanceOf(address(this)))
        ...
        require(
            collat.collateralAddress.balanceOf(msg.sender) >= collateralAmount,
            'ArthPool: balance < required'
        );
        require(
            collat.collateralAddress.transferFrom(
                msg.sender,
                address(this),
                collateralAmount
            ),
        ...
        amountsRecord storage users = amountsbycid[cid][msg.sender];
        users.amount = users.amount.add(collateralAmountD18);
        users.id = msg.sender;
    ...
    function calcCr(uint256 arthAmount,uint256 _cr) public view returns (uint256) {
        ...
        uint256 arthAmountPrecision = arthAmount;
        uint256 collateralRatio = uint256(cr);
    ...
    function redeem(
        ...
        if (collateralNeeded <=  users.amount) {
            
            require(
            collateralNeeded <=
                collat.collateralAddress.balanceOf(address(this)).sub(
        ...
        //todo add per-collateral amounts
       ...
       lastRedeemed[msg.sender] = block.number;
        ...
        users.amount = users.amount.sub(fullAmount);
        IERC20Burnable(_arthContractAddress).burn(compute);
            
        } else {
         redeemNotMinter(compute, collateralOutMin, cid); 
    ...
    function redeemNotMinter(uint256 _arthAmount, uint256 _collateralOutMin, uint256 cid) public {
    ...
    require(collateralNeeded <= collat.collateralAddress.balanceOf(address(this)).sub(unclaimedPoolCollateral),'ARTHPool: Not enough collateral in pool');
    ...
    function collectRedemption(uint256 cid) external override {
        require(
        ...
        if (sendCollateral)
            require(
                collat.collateralAddress.transfer(msg.sender, CollateralAmount),
        ...
        returns (uint256)
    {
        uint256 stabilityFeeInARTH =
            amount.mul(_arthController.getStabilityFee()).div(1e18);

        // ARTH is redeemed at 1$.
        return (
            stabilityFeeInARTH.mul(1e18).div(_arthController.getMAHAPrice())
        );
    }
...

{
  ...
  "libraries": {
    "/var/www/html/boltfork/arthcoin-v2/contracts/Arth/Pools/ArthPoolLibrary.sol": {
      "ArthPoolLibrary": "0x301Fe93aB1CB65A205D333495f148CE9aE425991"
    }
  },
  "outputSelection": {
    "*": {
