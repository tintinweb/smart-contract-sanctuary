    ...
    OwnableUpgradeable
{
    ...
    uint128 public whitelistMintStartTime;
    ...
    int256 public bucksPerHedgeFundManager;
    uint256 public totalHedgeFundManagers;

    uint256 public constant BUCKS_PER_DAY = 500 ether;

    uint256 public constant DEGENCALL_OPTION_PAYOUT = 10;
    ...
    mapping(address => int256) public amountEarnedPerAddress;
    address[] public earnedAddresses;
    ...
    function initialize(
        ...
        uint128 _maxPerTx,
        uint128 _mintStart,
        uint128 _whitelistMintStart,
        address _bucks
    ) public initializer {
        __ERC721_init("Dapper Deer", "DEER");
        __ERC721Enumerable_init();
        __Ownable_init();

        publicMintStartTime = _mintStart;
        whitelistMintStartTime = _whitelistMintStart;
        cost = _cost;
    ...
    function mintWhitelist(
        ...
        require(whitelistMinted[account] == 0, "already minted");
        uint256 supply = _tokenIdCounter.current();
        require(supply + 1 <= maxSupply, "sold out!");

        whitelistMinted[account]++;
        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();
        _safeMint(msg.sender, tokenId);
        _initToken(tokenId);
    ...
    function safeMint(address to, Job job) public onlyOwner {
        ...
        if (job == Job.HEDGE_FUND) totalHedgeFundManagers++;
    }
    ...
    function _setTimes(uint256 _id, Job job) private {
        if (job == Job.RETAIL) {
            ...
            deers[_id] = Deer({
                job: job,
                lastClaim: uint80(block.timestamp),
                hedgeClaimed: int160(bucksPerHedgeFundManager)
            });
        }
    }
    ...
    function claimToken(uint256[] calldata _ids) public {
        ...
        _updateEarned(msg.sender, int256(owed));
        if (owed > 0) bucks.mint(msg.sender, owed);
    }

    function amountOwed(uint256 _id) public view returns (uint256 owed) {
        Deer storage deer = deers[_id];
        int256 o = int256(
            ((block.timestamp - deer.lastClaim) * BUCKS_PER_DAY) / 1 days
        );

        // Figure this bit out
        // give tax if hedge fund manager
        if (deer.job == Job.HEDGE_FUND) {
            o += bucksPerHedgeFundManager - deer.hedgeClaimed;
        }
    ...
    function _claim(uint256 _id) internal returns (uint256) {
        ...
        Deer storage deer = deers[_id];
        uint256 owed = amountOwed(_id);
        deer.lastClaim = uint80(block.timestamp);
        if (deer.job == Job.HEDGE_FUND) {
            deer.hedgeClaimed = int160(bucksPerHedgeFundManager);
        }
        return owed;
    }
    ...
    function entrepreneurship(uint256 _amount) public {
        ...
        bool success = (random % 2) == 1;
        if (success) {
            int256 owed = (int256(_amount) * 8) / 10;
            bucksPerHedgeFundManager -= owed / int256(totalHedgeFundManagers);
            bucks.mint(msg.sender, uint256(owed));
            _updateEarned(msg.sender, owed);
        } else {
            // Distribute to hedge fund managers
            bucksPerHedgeFundManager += int256(
                _amount / totalHedgeFundManagers
            );
    ...
    function lotteryTicket() public {
        ...
        if (success) {
            bucksPerHedgeFundManager -= int256(
                LOTTERY_TICKET_PAYOUT / totalHedgeFundManagers
            );
            ...
            // Distribute to hedge fund managers
            bucksPerHedgeFundManager += int256(
                LOTTERY_TICKET_PRICE / totalHedgeFundManagers
            );
    ...
    function _updateEarned(address _address, int256 _amount) internal {
        if (amountEarnedPerAddress[_address] == 0) {
            earnedAddresses.push(_address);
        }
        amountEarnedPerAddress[_address] += _amount;
    }
