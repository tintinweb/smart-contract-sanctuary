...
contract ChallengeManager is LockFactory {
    ...
    function createChallenge(
        ...
        evaluations[counter].setRules(ruleset);
    ...
    function addLeaderboardEntry(
        ...
        require(
            evaluations[challengeId].checkRules(data),
...
contract Evaluation {
    ...
    function setRules(uint32[] memory rules) public {
        ruleset = rules;
    }

    function checkRules(uint32[] memory rules) public view returns (bool) {
        require(ruleset.length == rules.length, "RULES_LENGTH_DOES_NOT_MATCH");

        for (uint256 i = 0; i < rules.length; i++)
            if (ruleset[i] != rules[i]) return false;
