...
contract RepToken is ChainlinkClient {
    StockAPI public stockAPI;
    address private oracle;

    constructor(StockAPI _stockAPI) public {
        stockAPI = _stockAPI;
    ...
    function sendRequest() external {
        stockAPI.requestVolumeData(address(this), this.fulfill.selector);
    }
        ...
        returns (uint256)
    {
        ...
        uint256 mintt;
        bool decimalsExist;
        for (uint256 i = 0; i < bresult.length; i = i + 1) {
            if ((uint8(bresult[i]) >= 48) && (uint8(bresult[i]) <= 57)) {
                if (decimalsExist) {
                    if (_decimals == 0) break;
                    else _decimals = _decimals - 1;
                }
                mintt = mintt * 10;
                mintt = mintt + (uint8(bresult[i]) - 48);
            } else if (uint8(bresult[i]) == 46) decimalsExist = true;
        }
        if (_decimals > 0) mintt = mintt + (10**_decimals);
        return mintt;
    }
     ...
     */
    function fulfill(bytes32 _requestId, bytes32 _close)
        public
...
contract StockAPI is ChainlinkClient {
     ...
     */
    function requestVolumeData(address _cbContract, bytes4 _cbFunction)
        public
        returns (bytes32 requestId)
    {
        ...
        request.add(
            "get",
            "https://api.twelvedata.com/time_series?symbol=DAI&exchange=XETR&start_date=2021-07-06%2010:00&end_date=2021-07-06%2010:00&interval=1min&apikey=d8f072b5b5314d29b71c1ff807cf4109"
        );
