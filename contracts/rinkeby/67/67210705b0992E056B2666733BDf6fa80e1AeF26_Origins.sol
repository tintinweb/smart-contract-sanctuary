...
contract Origins is ERC721Enumerable, ReentrancyGuard, Ownable {
  ...
  function getColor(uint256 salt) internal view returns (string memory) {
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    string memory atrrOutput = makeAttributeParts(getCity(), getProvince(), getRealm());
    string memory json =
      _encode(
        bytes(
          string(
  ...
  function getCity() public view returns (string memory) {
    return _pluck1(cities, 1);
  }

  function getProvince() public view returns (string memory) {
    return _pluck1(provinces, 2);
  }

  function getRealm() public view returns (string memory) {
    return _pluck1(realms, 3);
  }

  function _pluck1(string[] memory array, uint256 salt) internal view returns (string memory) {
    uint256 rand = random(salt);
