    ...
    DSMath
{
        ...
        )
    {
        ...
        if(stakeStartTime > 0 && expectBlocktimestamp > coinageLastMintBlockTimetamp ){
            ...
            if(expectTime > _depositTokens.claimedTime && expectTime > _depositTokens.startTime ){
                if (_depositTokens.claimedTime > 0){
        ...
        )
    {
        ...
        if(stakeStartTime > 0  && expectBlocktimestamp > coinageLastMintBlockTimetamp){
            ...
            if(addIntervalTime > 0 ){
                uint256 miningPerSecond_ = IIStake2Vault(vault).miningPerSecond();
                uint256 addAmountCoinage = addIntervalTime.mul(miningPerSecond_);
                afterTotalCoinage = currentTotalCoinage.add(addAmountCoinage.mul(10**9));
                uint256 factor_ = IAutoRefactorCoinageWithTokenId(coinage).factor();
                uint256 infactor = _calcNewFactor(currentTotalCoinage, afterTotalCoinage, factor_);
                ...
                uint256 afterBalanceTokenId_ = applyCoinageFactor(balance, refactoredCount, f, count);
        ...
        )
    {
        ...
        if(stakeStartTime > 0 && expectBlocktimestamp > coinageLastMintBlockTimetamp){
            ...
            if ( uint256(liquidity) > 0
                && afterBalanceTokenId > uint256(liquidity).mul(10**9) ){
                minableAmountRay = afterBalanceTokenId.sub(uint256(liquidity).mul(10**9));
                minableAmount = minableAmountRay.div(10**9);
            }
