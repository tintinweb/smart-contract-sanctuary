...
contract Auction{
    ...
    constructor(address escrow_account,IERC20 DAIcontractAddress) public{
    ...
    function saveBid(string memory auctionID,uint bid_amount) public isAuctionNotSuspended returns(bool){
        Auction_Details memory a = auction[auctionID];
        ...
        //Bidders Transfer DAIs in Escrow Account
        if(i == temp.length){
            //Bidder not present in auction array
            require(DaiInstance.balanceOf(Bidder_Address) >= (bid_amount));
            require(DaiInstance.transferFrom(msg.sender,Escrow_Address,bid_amount),"DAI TRANSFER FAIL");
    ...
    function End_Auction(string memory auction_id) public onlySLCcontract isAuctionNotSuspended returns(bool,uint property_owner_money,uint treasury_money){
