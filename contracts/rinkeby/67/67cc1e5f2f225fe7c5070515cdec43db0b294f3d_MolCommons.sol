...
contract GAMMA { // Γ - mv - NFT - mkt - γ
    ...
    function purchase(uint256 tokenId) payable external {
        ...
        require(success, "!transfer GAMMA to gRoyalties");
        
        uint256 payout = sales[tokenId].ethPrice.sub(r);
        (success, ) = ownerOf[tokenId].call{value: payout}("");
        require(success, "!transfer GAMMA to owner of NFT");
...
contract MolCommons {
	...
	function approveContract(address _contract) public onlyOrganizers {
