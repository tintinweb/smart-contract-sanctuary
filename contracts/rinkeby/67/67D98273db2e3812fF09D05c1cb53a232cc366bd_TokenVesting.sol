...
contract TokenVesting is Ownable {
    ...
    constructor (
        ...
        for (uint256 i = 0; i < beneficiaries.length; i++) {
            ...
            uint256 remaining = allocated[i].sub(initial);
            _beneficiaries.push(investor);
            _initialRelease[investor] = initial;
            _allocated[investor] = remaining;
            _totalAllocation = _totalAllocation.add(allocated[i]);
        }
        // solhint-disable-next-line max-line-length
    ...
    function allocation(address investor) public view returns (uint256) {
        return _allocated[investor].add(_initialRelease[investor]);
    }
