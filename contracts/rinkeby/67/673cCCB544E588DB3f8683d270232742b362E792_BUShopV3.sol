...
// File: contracts/interface/IBUFactory.sol
...
// File: contracts/interface/IBUFactoryV2.sol
...
// File: contracts/interface/IPreSoldierFactory.sol
...
contract BUShopV3 is Ownable, IERC721Receiver {
    ...
    mapping(uint256 => mapping(address => BgPayType)) public _bgPayTypes;
    ...
    function removeBgPayTypes(uint256 bgId, address payToken) external onlyOwner checkPayToken(bgId, payToken) {
        ...
        for(uint256 idx = 0; idx < length; idx ++){
            if(tokens[idx] == payToken){
                delete _battleGrounds[bgId].payTokens[idx];
                break;
            }
        }
    }
    ...
    function setBUFactory(address _newFactory) external onlyOwner {
        _buFactory = IBUFactory(_newFactory);
    }

    function mintForCampaign(uint256 ruleId, uint256 category, uint256 nftType, uint256 level, uint256 count, address[] calldata receivers) external onlyOwner {
        bool flag = receivers.length == count;
        require(receivers.length == 0 || flag, "SOVI: count error");
        for (uint256 idx = 0; idx < count; idx ++) {
            _buFactory.mint(flag ? receivers[idx] : msg.sender, ruleId, category, nftType, level);
        }
    }
