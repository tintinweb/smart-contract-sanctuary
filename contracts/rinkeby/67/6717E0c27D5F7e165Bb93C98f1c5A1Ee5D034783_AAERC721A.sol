...
contract AAERC721A is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public immutable amountForCommunity;
    ...
    constructor(
        ...
        uint256 amountForCommunity_,
        uint256 amountForSale_
    ) ERC721A("AA TEST", "AA", maxBatchSize_, collectionSize_) {
        maxPerAddressDuringMint = maxBatchSize_;
        amountForCommunity = amountForCommunity_;
        amountForSale = amountForSale_;
        require(
            amountForCommunity_ + amountForSale_ <= collectionSize_,
    ...
    function communityMint(uint256 quantity) external onlyOwner {
        require(
            totalSupply() + quantity <= amountForCommunity,
            "too many already minted before dev mint"
        );
        require(
            quantity % maxBatchSize == 0,
            "can only mint a multiple of the maxatchSize"
        );
