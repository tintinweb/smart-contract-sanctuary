...
contract Liquidity_v8 is Ownable {
    ...
    function _calculate(address from) private view returns (uint256) {
        ...
        for (uint64 i = userIndex; i < index; i++) {
                ...
                interest = amount.mul(rates[i].newInterestRate).mul(time).div(315360000000); //replace with (_lockduration * 10000)
                amount += interest;
        ...
        // if (depositTime < endTime) {
            ...
            interest = time
                ...
                .div(315360000000); //replace with (lockduration * 10000)
