...
contract GangWars is ERC1155, Ownable {
    ...
    SaleRound[] saleRounds;
    bool isMintStarted = true;
    uint32 userMintLimitPerRound = 5;

    mapping(uint32 => string) uris; // round id => uri
    mapping(uint32 => uint32) public cardRound; // card id => round id
    mapping(uint32 => uint32) cardsSold; // round id => quantity
    ...
    function mint(bytes32 hash, uint8 _v, bytes32 _r, bytes32 _s) payable public {
        ...
        uint32 times = uint32(msg.value / saleRounds[currentRound].price);
        ...
        uint[] memory ids = new uint[](times);
        uint[] memory amounts = new uint[](times);
        uint32 j = 0;
        while (times > 0) {
            idCounter++;
            ids[j] = idCounter;
    ...
    function getSaleRounds() external view returns (uint[] memory, uint32[] memory, uint32[] memory, bool[] memory, uint32[] memory) {
        uint[] memory prices = new uint[](saleRounds.length);
        uint32[] memory quantities = new uint32[](saleRounds.length);
        uint32[] memory timestamps = new uint32[](saleRounds.length);
        bool[] memory whitelists = new bool[](saleRounds.length);
        uint32[] memory sold = new uint32[](saleRounds.length);

        for (uint32 i = 0; i < saleRounds.length; i++) {
            prices[i] = saleRounds[i].price;
            quantities[i] = saleRounds[i].quantity;
            timestamps[i] = saleRounds[i].timestamp;
            whitelists[i] = saleRounds[i].whitelist;
            sold[i] = cardsSold[i];
        }

        return (prices, quantities, timestamps, whitelists, sold);
    }
