...
contract VotingApp {
    ...
    //     public
    //     view
    //     returns (PresidentialCandidates[] memory)
    // {
        ...
        returns (string[] memory, uint256[] memory)
    {
        if (presidentialCandidates.length > 0) {
            string[] memory candidateNames = new string[](
                presidentialCandidates.length
            );
            uint256[] memory candidateNumber = new uint256[](
                presidentialCandidates.length
            );
            for (uint256 i = 0; i < presidentialCandidates.length; i++) {
                candidateNames[i] = presidentialCandidates[i].name;
                candidateNumber[i] = presidentialCandidates[i].votingCount;
            }
            return (candidateNames, candidateNumber);
        } else {
            string[] memory candidateNames = new string[](
                presidentialCandidates.length
            );
            return (candidateNames, new uint256[](0));
        }
    }
}