...
contract NounsToken is INounsToken, Ownable, ERC721Enumerable {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        // TODO: we need to parse the data that's returned by descriptor.genericDataURI
        // and overlay a hat on the image, then encode and send back
        // string memory svgData = descriptor.generateSVGImage(seeds[tokenId]);
        // Now decode base64 encoded SVG image
        string memory svgData = Base64.decode(descriptor.generateSVGImage(seeds[tokenId]));
        return svgData;
    }
