...
contract TEST_NBRL is ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  // ERC721 params
  string private _name = "Nature Barrels";
  string private _symbol = "NTBLS";
  string private _baseTokenURI = "";

  // Withdraw address
  address public withdraw_address = address(0); //TODO
  uint256 public available = 0;

  // Collection params
  uint256 public TOTAL_SUPPLY = 50;
  uint256 public PRICE = 1 ether;
  uint256[6] public MAX_PER_STATUS = [0, 10];
  ...
  event ChangedAvailabe(uint256 available);
  ...
  modifier qtyValidation(uint256 _qty) {
    ...
    require(_qty <= MAX_PER_STATUS[uint256(status)], "Exceeded the max amount of mintable NFT");
    require(total <= TOTAL_SUPPLY, "Collection is sold out");
    require(_qty <= available, "Not enough NFTs available");
  ...
  function publicMint(uint256 _qty) external payable nonReentrant qtyValidation(_qty){
  ...
  function getStatus() external view returns(string memory status_, uint qty_, uint price_, string memory msg_, uint256 available_){
    ...
    if(status == MintStatus.CLOSED){
        return ("CLOSED", MAX_PER_STATUS[uint256(status)], PRICE, "Minting is closed", public_supply);
    } else if(available == 0){
      return ("SOLD OUT", 0, PRICE, "Sold out", public_supply);
    } else {
        return ("PUBLIC", max_mintable, PRICE, "Public sale", public_supply);
    }
  }
  ...
  function setAvailable(uint8 _available) external onlyOwner {

    uint256 total_supply=  totalSupply();
    uint256 public_supply = TOTAL_SUPPLY - total_supply;
    require(_available <= public_supply, "Mint status must be between 0 and 1");
    available = _available;
    emit ChangedAvailabe(_available);
  }
