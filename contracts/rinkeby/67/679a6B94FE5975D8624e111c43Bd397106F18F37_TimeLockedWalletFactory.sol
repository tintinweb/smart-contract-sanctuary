...
contract TimeLockedWalletFactory {
    ...
    /*
view (in older complier versions called constant) denotes that 
this is a function that doesn’t change the blockchain state 
and can be therefore called for free, without spending any gas.
*/
        ...
        returns (address wallet)
    {
        ...
        Created(wallet, msg.sender, _owner, block.timestamp, _unlockDate, 0);
    }

    // callable by owner only, after specified time, only for Tokens implementing ERC20
    function withdrawTokens(address fromWallet, address _tokenContract) public view onlyOwner  {
        require(block.timestamp >= unlockDate);

        //wallet needs to be one of the wallets it created

        ERC20 token = ERC20(_tokenContract);
        
        uint256 tokenBalance = token.balanceOf(fromWallet);
        //token.transfer(owner, tokenBalance);
        //WithdrewTokens(_tokenContract, msg.sender, tokenBalance);
   ...
   function getBalanceOf(address wallet, address _tokenContract) public view returns (uint256, uint256) {
        ERC20 token = ERC20(_tokenContract);
        
        uint256 tokenBalance = token.balanceOf(wallet);
        uint256 ownerBalance = token.balanceOf(owner);
        
        return (tokenBalance, ownerBalance);
    }
