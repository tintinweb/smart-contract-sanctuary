...
interface OwlTokenInterfaceForNightOwls {
...
contract NightOwls is ERC721Enumerable, CollaborativeOwnable {
  ...
  address public owlToken = address(0);
  ...
  function mint(uint256 quantity) public payable {
    require(mintIsActive, "M1");
    require(quantity > 0, "M2");
    require(quantity <= mintLimit, "M3");
    require(quantity <= remainingForMint(), "M4");
    require(mintPrice.mul(quantity) <= msg.value, "M5");
  ...
  function prepaidMint() public {
    require(prepaidMintIsActive, "P1");

    uint256 quantity = _prepaidList[_msgSender()];
    require(quantity > 0, "P2");

    uint256 ts = totalSupply();
    require(ts.add(quantity) <= maxSupply.sub(reservedCount), "P3");
  ...
  function prepayBalanceOf(address addr) public view returns (uint256) {
  ...
  function transferFrom(address from, address to, uint256 tokenId) public override {
    if (owlToken != address(0)) {
      OwlTokenInterfaceForNightOwls(owlToken).onNightOwlTransfer(from, to);
    }
  ...
  function airDrop(address to, uint quantity) external onlyCollaborator {
    ...
    require(quantity > 0, "A2");
    require(quantity <= remainingForMint(), "A3");
  ...
  function reservedMint(address to, uint256 quantity) external onlyCollaborator {
    require(to != address(0), "R1");
    require(quantity > 0 && quantity <= reservedCount, "R2");
  ...
  function addToPrepaidList(address[] memory data) external onlyCollaborator {
  ...
  function removeFromPrepaidList(address[] memory data) external onlyCollaborator {
    for (uint256 i = 0; i < data.length; i++) {
      uint256 existing = _prepaidList[data[i]];
