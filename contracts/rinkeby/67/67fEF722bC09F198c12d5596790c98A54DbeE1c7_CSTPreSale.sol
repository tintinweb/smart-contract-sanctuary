...
contract CSTPreSale is Ownable {
    using SafeMath for uint256;
    ...
    mapping(address => bool) boughtCST;
    ...
    function getAllotmentPerBuyer() public view returns (uint256) {
        return IERC20(CSTP).balanceOf(address(this)).div(totalWhiteListed);
    }

    function purchaseCSTP(uint256 _amountDAI) external returns (bool) {
        require(saleStarted == true, "Not started");
        require(whiteListed[msg.sender] == true, "Not whitelisted"); 
        require(boughtCST[msg.sender] == false, "Already participated");
        require(block.timestamp < endOfSale, "Sale over");

        boughtCST[msg.sender] = true;
        ...
        IERC20(CSTP).safeTransfer(msg.sender, _purchaseAmount);

        return true;
    }

    function sendRemainingCSTP(address _sendCSTPTo)
        external
        onlyOwner()
        returns (bool)
    {
        ...
        IERC20(CSTP).safeTransfer(
            _sendCSTPTo,
            IERC20(CSTP).balanceOf(address(this))
        );
