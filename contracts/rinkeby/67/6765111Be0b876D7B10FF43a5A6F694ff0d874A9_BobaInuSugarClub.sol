...
contract BobaInuSugarClub is Ownable, ERC721 {
    ...
    Counters.Counter private _exclusive_token_id;
    ...
    string public exclusiveBaseURI;
    ...
    constructor(string memory _baseURI, string memory _exclusiveBaseURI) ERC721("BobaInuSugarClub", "BISC") {
        baseURI = _baseURI;
        exclusiveBaseURI = _exclusiveBaseURI;
    }

    function reserve(uint256 quantity) public onlyOwner {
        require(_exclusive_token_id.current() + quantity <= 30, "Reserving too many");
        for (uint256 i = 1; i <= quantity; i++) {
            _exclusive_token_id.increment();
            _safeMint(owner(), _token_id.current());
            id_to_uri[_token_id.current()] = string(abi.encodePacked(exclusiveBaseURI, _exclusive_token_id.current().toString(), ".json"));
        }
    }
    ...
    function claim(uint256 quantity) public payable {
        if (msg.sender != owner()) {
            ...
            require(totalSupply() + quantity <= maxSupply, "Mint amount exceeds supply");
    ...
    function ownedTokens() public view returns(uint256[] memory) {
        uint256 latest_id = _token_id.current();
        uint256[] memory ids = new uint256[](balanceOf(msg.sender));
        uint256 t = 0;
        for (uint256 i = 1; i <= latest_id; i++) {
    ...
    function totalSupply() public view returns(uint256) {
        return _token_id.current() + _exclusive_token_id.current();
    }
    ...
    function setMaxSupply(uint256 quantity) public onlyOwner {
        require(quantity <= 8000, "Must not go above 8000 tokens");
        maxSupply = quantity;
    }
}
