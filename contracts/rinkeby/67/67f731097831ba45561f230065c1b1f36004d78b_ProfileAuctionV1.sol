    ...
    OwnableUpgradeable
{
    ...
    address internal nftErc20Contract;        // contract of NftCash
    address internal nftProfile;
    uint256 public publicClaim;             // 0 = false, >= 1 = true
    uint256 public staticFee;               // static fee for claiming profile
    ...
    address internal coldWallet;            // for fees
    ...
    function initialize(
        ...
        staticFee = 10000000000000000000000; // 10,000 NFT.com tokens
        publicClaim = 0;
    ...
    */
    function findEffectDeleteArray(
        ...
        for (uint256 i = 0; i < bids.length; i++) {
            if (compareString(bids[i]._profileURI, _profileURI)) {
                if (_action == 0) {
                    ...
                    emit UpdateBid(_user, _profileURI, _nftTokens, _bids[_user][i]._nftTokens.add(_nftTokens));

                    _bids[_user][i]._nftTokens = _bids[_user][i]._nftTokens.add(_nftTokens);
                    return true;
                } else if (_action == 1) {
                    require(_bids[_user][i]._blockMinted == 0);
                    ...
                    emit RemoveBid(
                        _user,
                        _profileURI,
                        _bids[_user][i]._nftTokens.mul(9950).div(10000)
                    );
                    ...
                    payable(_user).transfer(profileFee);
                } else if (_action == 2) {
        ...
        if (_action == 0) { // submitBidFor
            require(msg.value == profileFee, "!ETH"); // only required on first bid
            bids.push(Bid(
                _nftTokens,
                0,
                _profileURI,
        ...
        } else { // remove bid, mint for, claim profile not found...
            require(false, "!exist");
        }
    ...
    function setPublicClaim(uint256 _val) external onlyRole(GOVERNANCE_ROLE) {
        publicClaim = _val;
    }
    ...
    */
    function submitProfileBid(
        uint256 _nftTokens,
        string memory _profileURI
    ) external payable nonReentrant validURI_(_profileURI) {
        require(transferNftTokens(msg.sender, _nftTokens));
        findEffectDeleteArray(msg.sender, _profileURI, 0, _nftTokens);
    }
    ...
    */
    function removeProfileBid(
        ...
        findEffectDeleteArray(msg.sender, _profileURI, 1, 0);
    }
    ...
    */
    function mintProfileFor(
        address _buyer,
        string memory _profileURI
    ) external onlyRole(MINTER_ROLE) nonReentrant validURI_(_profileURI) {
        findEffectDeleteArray(_buyer, _profileURI, 2, 0);
    ...
    */
    ...
    function claimProfile(
        ...
        findEffectDeleteArray(msg.sender, _profileURI, 3, 0);
    }
    ...
    */
    function redeemProfile(uint256 tokenId) external {
        require(INftProfile(nftProfile).profileDetails(tokenId)._blockMinted != 0, "invalid or unclaimed profile");

        require(block.number >= 
            INftProfile(nftProfile).profileDetails(tokenId)._blockMinted.add(
                INftProfile(nftProfile).profileDetails(tokenId)._blockWait), "block wait not met");

        // transfer profile to here
        IERC721EnumerableUpgradeable(nftProfile).transferFrom(msg.sender, address(this), tokenId);

        uint256 amount = INftProfile(nftProfile).profileDetails(tokenId)._nftTokens.mul(9950).div(10000);
        ...
        IERC20Upgradeable(nftErc20Contract).transfer(
            coldWallet,
            INftProfile(nftProfile).profileDetails(tokenId)._nftTokens.mul(50).div(10000)
        );
