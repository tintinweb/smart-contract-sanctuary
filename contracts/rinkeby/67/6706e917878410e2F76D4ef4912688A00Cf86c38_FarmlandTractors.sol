...
import "./interfaces/iCropV2.sol";

contract FarmlandTractors is ERC721, Ownable, ReentrancyGuard, FarmlandCollectible {
    constructor() ERC721("Farmland Tractors", "TRACTOR") public {
        pause(true);
    }
    
    iCropV2 private farm;
    FarmlandCollectible private farmers;
        ...
        nonReentrant 
    {
        ...
        require( farmers.balanceOf(msg.sender)>0 || 
                farm.getFarmCollectibleTotalOfType(msg.sender, CollectibleType.Farmer)>0,   "You need a farmer to purchase a tractor");
        ...
        override 
    {
        ...
        collectibleTrait.maxBoostLevel = randMod(15000).add(5000); // Set max boost level pseudo randomly between 50% and 150%
        uint256 collectibleExpires = randMod(100);
        if (collectibleExpires > 5) { // 50% of the collectibles expire
            collectibleTrait.expiryDate = now.add(90 days).add(randMod(730 days)); // pseudo random between 90 days & 2 years
        }
    }

    function initialize(string memory baseURI, uint256 reserved, uint256 maxSupply, uint256 price, address payable farmersAddress, address cornAddress) 
        public
        override
        onlyOwner 
    {
        ...
        _price = price;
        farm = iCropV2(cornAddress);
        farmers = FarmlandCollectible(farmersAddress);
        _initialized = true;
    }
}
...
//    function tokensReceived(address operator,address from,address to,uint256 amount,bytes calldata,bytes calldata) external;
...
// GETTERS
    ...
    function getGlobalDetails() external view returns (uint256 globalTotalFarms, uint256 globalRatio, uint256 globalAverageRatio, uint256 globalAllocatedAmount, uint256 globalCompostedAmount);
    function getConstantDetails() external view returns (uint256 maxCompostBoost, uint256 maxMaturityBoost, uint256 maxGrowthCycle, uint256 maxGrowthCycleWithFarmer, uint256 maxMaturityCollectibleBoost);
}
