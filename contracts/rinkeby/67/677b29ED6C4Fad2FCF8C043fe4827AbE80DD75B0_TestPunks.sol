...
contract TestPunks is  Ownable, ERC721Enumerable {

    uint public constant MAX_SPUNKS = 2077;
    bool public hasSaleStarted = false;
    mapping (address => uint) private _TestPunksV1Owners;
    string private _baseTokenURI;
    string private _baseContractURI;

    constructor(string memory baseTokenURI, string memory baseContractURI) ERC721("TestPunks V2","TPUNKS")  {
   ...
   function getTestPunk(uint256 numTestPunks) public payable {
        require(totalSupply() < MAX_SPUNKS, "Sale has already ended");
        require(numTestPunks > 0 && numTestPunks <= 20, "You can mint minimum 1, maximum 20 TestPunks");
        require(totalSupply()+numTestPunks <= MAX_SPUNKS, "Exceeds MAX_SPUNKS");
        require(msg.value >= calculatePrice() * numTestPunks, "Ether value sent is below the price");

        for (uint i = 0; i < numTestPunks; i++) {
    ...
    function reserveGiveaway(uint256 numTestPunks) public onlyOwner {
        uint currentSupply = totalSupply();
        require(totalSupply() + numTestPunks <= 130, "Exceeded giveaway supply");
        require(hasSaleStarted == false, "Sale has already started");
        uint256 index;
        // Reserved for TestPunks V1 holders airdrop, DystoFactory Upgrade and giveaways
        for (index = 0; index < numTestPunks; index++) {
   ...
   function claimTestPunk() public {
     ...
     require(_TestPunksV1Owners[msg.sender] > 0, "Not owner");
     address _to = msg.sender;
     uint _NumberToClaim = _TestPunksV1Owners[_to];
     for(uint i = 0; i < _NumberToClaim; i++){
       _safeMint(_to, totalSupply());
     }
     _TestPunksV1Owners[_to]=0;
   }
