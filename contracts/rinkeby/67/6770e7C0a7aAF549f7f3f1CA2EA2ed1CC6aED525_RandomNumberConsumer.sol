...
interface IRandomNumberGenerator {
    ...
    function viewRandomResult() external view returns (uint);
...
function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    ...
    function viewRandomResult() external view override returns(uint) {
        return randomResult;
    }
     function viewRandomResult2() public view  returns(uint[] memory) {
        return randomArray;
    }
}
interface IERC165 {
...
contract pizzaNFT is ERC721, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    uint256 totalprice) public {
        uint _randomId = randomGenerator.viewRandomResult();
