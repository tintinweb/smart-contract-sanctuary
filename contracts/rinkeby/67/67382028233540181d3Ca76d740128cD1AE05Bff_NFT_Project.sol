...
contract NFT_Project is ERC721Burnable, Ownable, ReentrancyGuard{
   ...
   event minted (uint256 _NftId, string msg);
    ...
    function MINT_NFT(uint256 index, string memory _TokenURI) 
    public payable nonReentrant returns (uint256) {
         require(msg.value > 0, "amount need to be gretaer than 0");
        Treasury.transfer(msg.value);
      return( _mintNft(msg.sender, index, _TokenURI));
        
    }
    


    function _mintNft(address creator, uint256 index, string memory _TokenURI) 
    internal 
    returns (uint256) 
    {
