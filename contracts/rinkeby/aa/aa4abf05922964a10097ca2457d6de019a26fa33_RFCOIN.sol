...
contract RFCOIN is ERC20, Ownable {
 ...
 function swapAndDividend(uint256 tokens) private lockTheSwap {
        uint256 liqTokens = tokens.mul(LIQUIDITY_FEE).div(TOTAL_FEES);
        uint256 dividendTokens = tokens.sub(liqTokens);

        uint256 initialBalance = address(this).balance;

        uint256 toSwapForEth = liqTokens.add(dividendTokens);
        
        swapTokensForEth(toSwapForEth); 

        uint256 fromSwap = address(this).balance.sub(initialBalance);
        uint256 dividendBalance = fromSwap.mul(dividendTokens).div(toSwapForEth);
        
        uint256 dividends = dividendBalance;
        ...
        _devAndMarketingAddress.transfer(fromSwap.sub(dividendBalance));
        
        emit SwapAndSendToDev(tokens, (fromSwap.sub(dividendBalance)));
