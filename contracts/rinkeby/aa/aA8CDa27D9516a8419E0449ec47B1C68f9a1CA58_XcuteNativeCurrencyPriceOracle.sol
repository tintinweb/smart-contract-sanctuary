...
abstract contract Job is Context {
    ...
    modifier needsExecution() {
        if (!IXcuteMaster(XCUTE_MASTER).initializeWork(_msgSender()))
            revert NotAWorker();
        _;
        IXcuteMaster(XCUTE_MASTER).finalizeWork(_msgSender());
    }
    ...
    modifier needsExecutionWithRequirements(
        ...
        if (
            !IXcuteMaster(XCUTE_MASTER).initializeWorkWithRequirements(
                _msgSender(),
                _minimumStaked,
                _minimumEarned,
                _minimumAge
