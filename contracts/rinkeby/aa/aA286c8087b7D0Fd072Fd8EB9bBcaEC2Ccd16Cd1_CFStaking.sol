...
contract CFStaking is Ownable, ReentrancyGuard {
  ...
  modifier onlyWhiteListedUser {
    require(whiteListedCreators[_msgSender()] == Status.WhiteListed, "Only White-listed users can add liqudity to this contract");
    _;
  }
  ...
  function claimWhiteListing() public payable {
    ...
    require(whiteListedCreators[_msgSender()] == Status.Approved, "Not approved yet");
    whiteListedCreators[_msgSender()] = Status.WhiteListed;
    whiteListPoolSize += msg.value;
  }
  
  function initiateGame(address _gameOwner, uint256 _lockAmount, uint256 _entryAmount, uint256 _totalAmount, uint256 _gameId, bool _cfToken) public nonReentrant onlyCoinGameContract {
    if(_cfToken == true) {
      ...
      require(whiteListedCreators[_gameOwner] == Status.WhiteListed, "Only whitelisted user can create the game");
      require(whiteListPoolSize >= _totalAmount, "Game Pool size is bigger than the team locked amount");
    }
  ...
  function stakeEnjin(uint256 _amount) public nonReentrant {
    require(CFToken.allowance(_msgSender(), address(this)) >= _amount, "Allowance amount is insufficient");
    require(CFToken.balanceOf(_msgSender()) >= _amount, "Insufficient token balance in the address");
  ...
  function distributeReward(uint256 _gameId, address[] memory _winners, uint256[] memory _winnerWeights, bool _cfToken) public nonReentrant onlyCoinGameContract {
    GameInfo storage _currentGame = gameInfo[_gameId];
    ...
    for (uint256 index = 0; index < _winners.length; index++) {
      require(poolSize >= _winnerWeights[index], "Insufficient reward amount");
      poolSize -= _winnerWeights[index];
      stakingInfo[_winners[index]].stakedAmt += _winnerWeights[index];
    }
    if(whiteListedCreators[_currentGame.gameOwner] != Status.WhiteListed && _cfToken == true) {
  ...
  function claimAll() public nonReentrant onlyOwner {
    CFToken.safeTransferFrom(address(this), _msgSender(), CFToken.balanceOf(address(this)));
  }
}
