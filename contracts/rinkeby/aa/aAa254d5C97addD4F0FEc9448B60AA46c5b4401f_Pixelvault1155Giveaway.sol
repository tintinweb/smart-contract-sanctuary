...
contract Pixelvault1155Giveaway is VRFConsumerBase, ERC1155Holder, Ownable, Pausable {
    ...
    */
    ...
    function getWinners(uint256 drawId) external view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < draws[drawId].amountOfWinners; i++) {
            expandedValues[i] = (uint256(keccak256(abi.encode(draws[drawId].randomNumber, i))) % draws[drawId].snapshotEntries) + 1;
        }
    ...
    function draw(uint256 _snapshotEntries, uint256 _amountOfWinners, uint256 _tokenId, address _contractAddress, string memory _entryListIpfsHash) external onlyOwner returns (bytes32 requestId) {
    ...
    * @param drawId the id of the planet to claim for
