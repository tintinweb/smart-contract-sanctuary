...
contract GamingChick is ERC721Enumerable, OwnableContract{
    ...
    function tokenOfOwnerPage(address owner, uint256 from, uint256 to) external view returns (uint256, uint256[] memory){
        uint256 total = balanceOf(owner);
        require(from < to && (to - from) <= 100 && (to - from) <= total, "from and to is error.");
        uint256[] memory tokenIds = new uint256[](to - from);
        uint256 count = 0;
