...
contract DeepWatersLending is ReentrancyGuard {
    ...
    **/
    function borrow(
        ...
        nonReentrant
    {
        _borrow(_asset, msg.sender, _amount, isStableRateBorrow);
    }
        ...
        internal
    {
        ...
        if (isStableRateBorrow) {
            data.newAverageStableInterestRate = 
                ...
                add(data.currentBorrowBalance.add(data.linearInterest).mul(vault.getUserBorrowAverageStableInterestRate(_asset, _user))).
    ...
    **/
        ...
        nonReentrant
    {
        ...
        uint256 borrowBalance = vault.getUserBorrowBalance(_asset, msg.sender).
                                  add(vault.getUserBorrowCurrentLinearInterest(_asset, msg.sender));
        
        require(borrowBalance > 0, "The user has no borrow");
        
        bool newIsStableRateBorrow = !vault.isUserStableRateBorrow(_asset, msg.sender);
        
        repayAssetCall(_asset, msg.sender, borrowBalance);
        
        _borrow(_asset, msg.sender, borrowBalance, newIsStableRateBorrow);
    }
