...
contract Woolf is IWoolf, ERC721Enumerable, Ownable, Pausable {
    ...
    // 2 AVAX
    uint256 public constant MINT_PRICE = 60000000000000000;
    ...
    function testRandom(uint256 num) external whenNotPaused onlyOwner returns (uint256 seed) {
        seed = entropy.random(num);
        return seed;
    }

    function testgenerate(uint256 num, uint256 seed) external whenNotPaused onlyOwner returns (SheepWolf memory) {
        generate(num, seed);
        _safeMint(owner(), minted);
        return tokenTraits[num];
