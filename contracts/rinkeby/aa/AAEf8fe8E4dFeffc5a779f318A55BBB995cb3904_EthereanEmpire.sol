...
/// @notice Creates a new empire if minimum etherean balance in wallet is met
...
/// @dev Claim function is only available for wallets who have created an empire and have maintained minimum etherean balance requirement at time of claim. The claim amount is determined by: total elapsed time (since last claim) x base rewards x multiplier bonus. Users can claim past the END_REWARDS timestamp if pending rewards are available.
