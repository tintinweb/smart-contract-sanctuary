...
contract CryptoCriminals is ERC721, Ownable, ReentrancyGuard, PaymentSplitter {
  ...
  uint public constant recruitingTimeWindow = 3 minutes;

  mapping (uint => uint) public lastRecruitTimeOfTokenID;
  mapping (uint => uint) public pointsOfToken;
  
  constructor() ERC721("CryptoCriminalsTest", "CCt") PaymentSplitter(_team, _teamShares) {
  }
  ...
  function pseudoRandomPointAssignment(uint[] memory tokenIds) private view returns (uint){
    uint result;
    for(uint i = 0; i < tokenIds.length; i++){
      result = uint(keccak256(abi.encodePacked(result ,tokenIds[i])));
    }
    result = uint(keccak256(abi.encodePacked(result, msg.sender.balance, block.timestamp)));
    return (result % 3) + 1;
  }
