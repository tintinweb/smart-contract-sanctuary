...
contract PriceDiscovery is Ownable {
    ...
    uint[] public bonusCoefficient 
        = [3000000, 2000000, 1800000, 1600000, 1500000, 1300000, 1200000, 1000000, 1000000];
    ...
    function deposit(uint amount, Token token, bytes32 signature) external discoveryCurrent supportedToken(token){
        //TODO check referral code
        uint256 usdDeposited = _deposit(amount, token);

        //if (hasReferrer(_msgSender())) {
            // TODO add referral bonus
            // TODO emit refarral deposit event
            // TODO: totalBUSDwithBonusesCollected += referral bonus
        //}
    }
    ...
    function sYSLwithdraw() external discoveryFinished {
        ...
        if (amountDeposited > 0) {
            ...
            uint256 sYSLamount = amountDeposited / getDiscoveredPrice();
    ...
    function getCurrentStage() public view returns(Stage _stage) {
        if (block.timestamp < privateSaleStartTime || block.timestamp > priceDiscoveryStartTime + PRICE_DISCOVERY_DURATION) {
            _stage =  Stage.Closed;
        }
