...
contract SKBMain is ERC721Enumerable, Ownable {
    ...
    // Events
    event OnNameChange   (uint256 indexed tokenId, string newName);
    event StatusChange (uint256 indexed tokenId, string newStatus);
    ...
    mapping (uint256 => string) private _tokenStatus;
    ...
    constructor(uint256 _maxSupply, uint256 _reserveCount) ERC721("Schrodinger's Cat", "SKBS") {
        require(_reserveCount <= _maxSupply, "Schrodinger's Cat: reserve count out of range");
    ...
    function reserveTokens(address recipient, uint256 count) external onlyOwner {
        ...
        require(_nextTokenId + count <= maxSupply, "Schrodinger's Cat: max supply exceeded");

        require(tokensReserved + count <= reserveCount, "Schrodinger's Cat: max reserve count exceeded");
    ...
    function setUpPresale(
        ...
        // Check params
        require(whitelistSigner != address(0), "Schrodinger's Cat: zero address");
        require(_startTime > 0 && _endTime > _startTime, "Schrodinger's Cat: invalid time range");
    ...
    //     require(_presaleConfig.whitelistSigner != address(0), "Schrodinger's Cat: presale not configured");

    //     require(treasury != address(0), "Schrodinger's Cat: treasury not set");
    //     require(tokenPrice > 0, "Schrodinger's Cat: token price not set");
    //     require(count > 0, "Schrodinger's Cat: invalid count");
    //     require(block.timestamp >= _presaleConfig.startTime, "Schrodinger's Cat: presale not started");
    //     require(block.timestamp < _presaleConfig.endTime, "Schrodinger's Cat: presale ended");

    //     require(_nextTokenId + count <= maxSupply, "Schrodinger's Cat: max supply exceeded");
    //     require(tokenPrice * count == msg.value, "Schrodinger's Cat: incorrect Ether value");
    ...
    function mintTokens(uint256 count) external payable {
        ...
        require(_saleConfig.startTime > 0, "Schrodinger's Cat: sale not configured");

        require(treasury != address(0), "Schrodinger's Cat: treasury not set");
        require(tokenPrice > 0, "Schrodinger's Cat: token price not set");
        require(count > 0, "Schrodinger's Cat: invalid count");
        require(block.timestamp >= _saleConfig.startTime, "Schrodinger's Cat: sale not started");
    ...
    function burn(uint256 tokenId) external {
        require(isBurnEnabled, "Schrodinger's Cat: burning disabled");
        require(_isApprovedOrOwner(msg.sender, tokenId), "Schrodinger's Cat: burn caller is not owner nor approved");
        _burn(tokenId);
    }
    ...
    /**
     * @dev Returns status of the NFT at index.
     */
    ...
    /**
     * @dev Changes the status for Cat tokenId
     */
    function lookCat(uint256 tokenId) public {
        address owner = ownerOf(tokenId);

        require(_msgSender() == owner, "ERC721: caller is not the owner");

        _tokenStatus[tokenId] = "dead";
        emit StatusChange(tokenId, _tokenStatus[tokenId]);
    }
    
    /**
     * @dev Changes the name for Cat tokenId
     */
    function NameChange(uint256 tokenId, string memory newName) public {
        address owner = ownerOf(tokenId);
        ...
        emit OnNameChange(tokenId, newName);
    }
