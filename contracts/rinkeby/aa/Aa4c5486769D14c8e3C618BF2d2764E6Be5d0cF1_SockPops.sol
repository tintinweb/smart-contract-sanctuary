...
contract SockPops is Ownable, ERC721 {
  ...
  uint256 public maxFreeMintSupply = 333;

  Counters.Counter private _tokenIdCounter;
  Counters.Counter public _freeClaimCounter;
  ...
  function freeMint(uint amount) external {
    ...
    require(_freeClaimCounter.current() + amount <= maxFreeMintSupply,"This tx exceeds max free limit!");

    mintFreeInternal(msg.sender, amount);
  ...
  function mintFreeInternal(address wallet, uint amount) internal {
    require(amount <= 3,"Max 3 free mints per Transaction!");
    uint256 minted = freeMintedByAddress[wallet];
    require(minted + amount <= 3, "This would pass the max free cap mint for this address");
  ...
  function withdrawAll() public onlyOwner {
    ...
    artist.transfer(balance*3/10);
    teamLead.transfer(balance*3/10);
