  ...
  Recoverable
{
    ...
    returns (uint64 newRound)
  {
    ...
    // Likely, we'd have upgrade the contract by this point.
    round++;
 ...
 */
contract MintingCeremony is Windowed, Recoverable, ReentrancyGuard {
  using SafeMath for uint256;
  using SafeDecimalMath for uint256;
  using SafeERC20 for IERC20;
  using SafeERC20 for ISupplyControlledERC20;
  ...
  function commit(
    ...
    uint256 floatOutMin
  ) external nonReentrant inWindow {
    // External calls occur here, but trusted
    uint256 floatOutFromPrice = _floatOut(underlyingIn);
    uint256 floatOutFromAllowance = allowance(recipient);

    uint256 floatOut = Math.min(floatOutFromPrice, floatOutFromAllowance);
    ...
    IERC20(basket.underlying()).safeTransferFrom(
      msg.sender,
      address(basket),
      underlyingIn
    );
   ...
   */
  function mint() external afterWindow {
  ...
  function _floatOut(uint256 wethIn) internal view returns (uint256) {
...
abstract contract LockInPool is BasePool {
  ...
  function getStorageAt(bytes32 slot) public returns (uint256 ret) {
      bytes32 s = slot;
      assembly {
          ret := sload(s)
      }
