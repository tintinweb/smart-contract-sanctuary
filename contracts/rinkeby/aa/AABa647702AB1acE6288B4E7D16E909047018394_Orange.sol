...
contract Orange is ERC721EnumerableAlt, Ownable {
    ...
    */
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        uint lastTokenRange = 0;
        RevealStruct memory reveal;
        ...
        for(uint i = 0; i < revealList.length; i++) {
            ...
            lastTokenRange = reveal.tokenRange;
        ...
        uint difference = reveal.tokenRange - lastTokenRange;

        uint moddedId = (tokenId + reveal.shift) % difference;

        return string(abi.encodePacked(uri, moddedId.toString()));
    ...
    function revealTokens(string memory _uri, uint128 _tokenRange) external onlyOwner {
        ...
        if(RevealList.length > 0) {
            lastTokenRevealed = RevealList[RevealList.length - 1].tokenRange;
        }

        require(_tokenRange > lastTokenRevealed, "Token range needs to be larger than the last revealed");

        require(_tokenRange <= maxSupply, "Cant reveal more than max supply");

        uint difference = _tokenRange - lastTokenRevealed;

        uint128 shift = uint128(uint(keccak256(abi.encodePacked(_msgSender(), block.timestamp, block.difficulty))) % difference);
