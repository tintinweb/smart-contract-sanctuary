...
contract Dobercity is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    uint256 public constant AMOUNT_RESERVED = 200;
    ...
    address payable[] private _devWallets = [payable(address(0xfA528fBAA85E8be1e72968aC283EAaaf5809C5a1))];

    bool private _devsWithdrawn = false;
    ...
    function mintDober(address human, uint256 amountOfDobers) public nonReentrant payable virtual returns (uint256) {
     ...
     function withdrawAllEth(address payable payee) public virtual onlyOwner {
        require(_devsWithdrawn, "The Developer Withdraw has not happened yet.");
        payee.sendValue(address(this).balance);
    }

    function withdrawDevEth() public virtual onlyOwner{
        for(uint i = 0; i < _devWallets.length; i++) {
            _devWallets[i].transfer(address(this).balance / 10);
        }
        _devsWithdrawn = true;
    }
