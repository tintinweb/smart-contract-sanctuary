...
import "./interfaces/ISaberedToothShark.sol";

contract MysticalWhale is ERC721Enumerable, Ownable {
  ...
  function areOwned(uint256[] memory tokenIds) public view returns (bool out) {
    ...
    for (uint256 i = 0; i < tokenIds.length; i++){
      if (STS.ownerOf(tokenIds[i]) != msg.sender){
        out = false;
      }
    }
  }
...
interface ISaberedToothShark is IERC721Enumerable{
