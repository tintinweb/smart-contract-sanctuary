 ...
 * @dev see https://eips.ethereum.org/EIPS/eip-721
 */
contract CAMA is Context, CustomOwnable, ERC165, IERC721, IERC721Metadata {//change the name at the end
    ...
    */
    ...
    function batchTransfer(address to, uint96 size) public virtual {
        require(_msgSender() == owner() || this.isApprovedForAll(owner(), _msgSender()), "CAMA: transfer caller is not owner nor approved");
        require(to != owner(), "CAMA: Can't transfer batch to own address");
        _batchTransfer(owner(), to, size);
    }

    function batchTransferFrom(address from, address to, uint[] memory ids) public virtual {
        require(from == _msgSender() || this.isApprovedForAll(owner(), _msgSender()), "CAMA: transfer caller is not owner nor approved");
        require(to != owner(), "CAMA: Can't transfer batch to own address");
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
     ...
     function _batchTransfer(address from, address to, uint96 size) internal returns (uint) {
        ...
        emit ConsecutiveTransfer(start, end, owner(), to);
