...
contract EliteForces is ERC721Enumerable, Ownable {
    ...
    constructor(uint256 _maxSupply, uint256 _reserveCount)
        ERC721("Elite Forces - testv1 Token", "EFTV1")
    {
        require(
            _reserveCount <= _maxSupply,
            "Elite Forces - testv1: reserve count out of range"
        );
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256(bytes("Elite Forces - testv1")),
                keccak256(bytes("1")),
                chainId,
                address(this)
            )
        ...
        onlyOwner
    {
        require(
            recipient != address(0),
            "Elite Forces - testv1: zero address"
        );

        // Gas optimization
        uint256 _nextTokenId = nextTokenId;

        require(count > 0, "Elite Forces - testv1: invalid count");
        require(
            _nextTokenId + count <= maxSupply,
            "Elite Forces - testv1: max supply exceeded"
        );

        require(
            tokensReserved + count <= reserveCount,
            "Elite Forces - testv1: max reserve count exceeded"
        );
    ...
    function reserveTokens(address[] memory recipients) external onlyOwner {
        ...
        require(recipients.length > 0, "Elite Forces - testv1: invalid count");
        require(
    ...
    function setUpPresale(
        ...
        require(
            ...
            "Elite Forces - testv1: invalid time range"
        );
    ...
    function setUpSale(
        ...
        require(
            ...
            "Elite Forces - testv1: zero amount"
        );
        require(
            _startTime > 0 &&
    ...
    function mintPresaleTokens(
        ...
        require(
            ...
            "Elite Forces - testv1: presale not configured"
        );

        require(
            treasury != address(0),
            "Elite Forces - testv1: treasury not set"
        );
        require(
            presaleTokenPrice > 0,
            "Elite Forces - testv1: token price not set"
        );
        require(count > 0, "Elite Forces - testv1: invalid count");
        require(
            block.timestamp >= _presaleConfig.startTime,
            "Elite Forces - testv1: presale not started"
        );
        require(
            block.timestamp < _presaleConfig.endTime,
            "Elite Forces - testv1: presale ended"
        );
        ...
        require(
            ...
            "Elite Forces - testv1: incorrect Ether value"
        );
        ...
        require(
            ...
            "Elite Forces - testv1: invalid signature"
        );
        require(
            presaleBoughtCounts[msg.sender] + count <= maxCount,
            "Elite Forces - testv1: presale max count exceeded"
        );
    ...
    function mintTokens(uint256 count) external payable {
        ...
        require(
            ...
            "Elite Forces - testv1: sale not configured"
        );
        ...
        require(_tokenPrice > 0, "Elite Forces - testv1: token price not set");
        require(count > 0, "Elite Forces - testv1: invalid count");
        require(
            block.timestamp >= _saleConfig.startTime,
            "Elite Forces - testv1: sale not started"
        );
        ...
        require(
            ...
            "Elite Forces - testv1: max count per tx exceeded"
        );
        require(
            _nextTokenId + count <= maxSupply,
            "Elite Forces - testv1: max supply exceeded"
        ...
        require(
            ...
            "Elite Forces - testv1: unsufficient Ether sent"
        );
    ...
    function burn(uint256 tokenId) external {
        require(isBurnEnabled, "Elite Forces - testv1: burning disabled");
        require(
            _isApprovedOrOwner(msg.sender, tokenId),
            "Elite Forces - testv1: burn caller is not owner nor approved"
        );
        _burn(tokenId);
    }
