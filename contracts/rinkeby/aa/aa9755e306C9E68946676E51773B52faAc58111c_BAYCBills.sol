...
contract BAYCBills is ERC721URIStorage {
  ...
  function genAttributes(uint256 _tokenId) public {
    ...
    uint256 randomNumber = randrange(100000, 900000);
    console.log('--------------------');
    ...
    for (uint256 friend=0; friend<numerOfFriends; friend++) {
      ...
      for (uint256 round=0; round<numberOfRounds; round++) {
        ...
        string memory beerThatRoundByThatFriend = beerTypes[
          unbiasModulo(loopRN, 4)
        ];

        if (isSameString(beerThatRoundByThatFriend, "Draft Lager")) {
          // push into orderOfBeers array when beer type is ordered for first time
          if (lagerCounter == 0) {
            tokenIdToOrderOfBeers[_tokenId].push(0); // "Draft Beer"
          }
        ...
        if (isSameString(beerThatRoundByThatFriend, "Draft IPA")) {
          if (ipaCounter == 0) {
            tokenIdToOrderOfBeers[_tokenId].push(1); // "Draft IPA"
          }
          ipaCounter = ipaCounter + 1;
        }
        if (isSameString(beerThatRoundByThatFriend, "Draft NEIPA")) {
          if (neipaCounter == 0) {
            tokenIdToOrderOfBeers[_tokenId].push(2); // "Draft NEIPA"
          }
