...
interface ITeamWallet {
    function swapTokens(uint256 _tokenAmount) external;
}

contract Pika is ERC20, Owned {
    uint256 public fee; // fixed
    uint256 public minSupply; // variable
    address public beneficiary; // variable
    // uint256 public minBeneficiaryAmount; // variable
    bool public feesEnabled; // variable
    mapping(address => bool) public isExcludedFromFee; // variable
    ...
    constructor() ERC20("PIKA", "PIKA") {
        ...
        // minBeneficiaryAmount = 5000000000 ether;
    ...
    function _transfer(
        ...
        uint256 transferFee = calculateFee(amount);
        super._transfer(sender, beneficiary, transferFee);
        super._transfer(sender, recipient, amount - transferFee);

        uint256 beneficiaryBalance = balanceOf(beneficiary);
        // if (beneficiaryBalance > minBeneficiaryAmount) {
        // result of the token swap is not checked, a failed swap should not fail the transaction
        beneficiary.call(
            abi.encodeWithSignature(
                "swapTokens(uint256 _tokenAmount)",
                beneficiaryBalance
            )
        );
        // }
