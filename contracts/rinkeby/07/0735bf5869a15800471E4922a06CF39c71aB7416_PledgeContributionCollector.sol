...
error CapOverflow();
error NonCancelledContribution();
error AlreadyRecovered();

/**
 * @title PledgeContributionCollector
 * @dev PledgeContributionCollector contract
 * @author Federico Luzzi - <[email protected]>
 * SPDX-License-Identifier: GPL-3.0
 */
contract PledgeContributionCollector is Initializable, OwnableUpgradeable {
    ...
    function updateCap(uint256 _newCap) external onlyOwner {
        if (state != State.ACTIVE) revert InactiveContribution();
        if (_newCap < contribution.amount) revert InvalidContributionCap();
    ...
    function contribute(
        ...
        if (contribution.amount + _amount > contribution.cap)
            revert CapOverflow();
    ...
    function recover() external {
        if (state != State.CANCELED) revert NonCancelledContribution();
