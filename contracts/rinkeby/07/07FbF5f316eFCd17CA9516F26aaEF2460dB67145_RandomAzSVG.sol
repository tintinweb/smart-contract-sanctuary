...
contract RandomAzSVG is ERC721URIStorage, VRFConsumerBase {
    bytes32 public keyHash;
    uint256 public fee;
    uint256 public tokenCounter;

    // SVG parameters
    // NOTE: make this immutable to save on gas...?
    ...
    // NOTE: Good practise whenever using mapping that you emit an event
    ...
    function create() public returns (bytes32 _requestId) {
        ...
        // get a random number and generate some random SVG code
        // base64 encode the SVG code
        // get the tokenURI and mint the NFT
        // to get a random number need to look outside the blockchain to get one
        // use chainlink
    }

    // Chainlink will return it to this function
    function fulfillRandomness(bytes32 _requestId, uint256 _randomNumber)
        internal
        override
    {
        // There is an issue
        // Chainlink VRF has a max gas of 200,000 gas
        ...
        returns (string memory pathSvg)
    {
        ...
        for (uint256 i = 0; i < numberOfPathCommands; i++) {
            ...
            string memory pathCommand = generatePathCommands(newRNG);
    ...
    function generatePathCommands(uint256 _randomNumber)
        public
        view
