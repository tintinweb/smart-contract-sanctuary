...
contract ChipmunksAndFriends is ERC721Enumerable, Ownable {
    ...
    constructor(uint256 _maxSupply, uint256 _reserveCount, address payable _treasury) ERC721("Chipmunks & Friends", "CHIPMUNK") {
    ...
    function mintPresaleTokens(
        ...
        require(privateSaleIsActive, "ChipmunksAndFriends: presale not active");
        ...
        // !!! @TODO:This is not needed anymore when signature works.
        require(presaleBoughtCounts[msg.sender] + count <= maxCount, "ChipmunksAndFriends: presale max count exceeded");
    ...
    function test(bytes calldata signature, uint256 maxCount) external view returns (address encodedAddress) {
        // Verify EIP-712 signature
        bytes32 digest = keccak256(
            abi.encodePacked("\x19\x01", DOMAIN_SEPARATOR, keccak256(abi.encode(PRESALE_TYPEHASH, msg.sender, maxCount)))
        );
