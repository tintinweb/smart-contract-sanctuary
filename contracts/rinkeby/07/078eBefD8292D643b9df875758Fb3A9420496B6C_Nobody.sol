...
// File: contracts\NobodyFactory.sol
...
contract Nobody is ERC721Enumerable, NobodyFactory, Ownable {
  ...
  address public thedudesaddress =0x942662EEc4ba246Ee9229627e2B9c5a954DB8892; //testnet address
  theDudes thedudescontract = theDudes(thedudesaddress);  
    
  address public creator = 0x264fFE58b1a497FEbE7A45D9D70B5Ca9b7Cf27C8; //testnet address

  constructor (uint _maxNobody, uint _maxNobodyPerPurchase ) ERC721("nobody", "nb") {
  ...
  function _claim(address _to, string memory thedudesdna) internal {
      uint256 mintIndex = totalSupply();
          if (totalSupply() < maxNobody) {
                string memory nobodyid = string(abi.encodePacked(thedudesdna , '0'));
