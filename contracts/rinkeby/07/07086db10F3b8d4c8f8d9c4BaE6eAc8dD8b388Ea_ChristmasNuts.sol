...
contract ChristmasNuts is ERC721, Ownable {
    ...
    constructor(address _proxyRegistryAddress) ERC721("ChristmasNuts", "CHN") {
    ...
    function whitelistMint(uint256 amount) external {
        ...
        require(alreadyMinted[msg.sender] + amount <= whitelist[msg.sender], "User already minted all allowance");
    ...
    function mintTripleMerkle(bytes32[] calldata _merkleProof) external {
        ...
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, quadMerkleRoot, leaf), "User not whitelisted for 3 mints");
    ...
    function mintDoubleMerkle(bytes32[] calldata _merkleProof) external {
        ...
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, quadMerkleRoot, leaf), "User not whitelisted for 2 mints");
    ...
    function mintSingle(bytes32[] calldata _merkleProof) external {
        ...
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, quadMerkleRoot, leaf), "User not whitelisted for 1 mints");
        // todo merkle check
