...
contract trailersNFT is Ownable, ERC721{
    ...
    //mapping(uint256 => bool) public lotStatement;
    ...
    function mintableLength() external view onlyOwner returns (uint256){
        return mintableTokens.length;
    }
    
    function RNG(uint256 lot) external view onlyOwner returns (uint256){
        return lotTracker[lot];
    }
    
    function mintableItem(uint256 mintableKey) external view onlyOwner returns (uint256){
        return mintableTokens[mintableKey];
    }
