...
contract Merge is ERC721, ERC721Metadata {
    ...
    uint256 private constant CLASS_MULTIPLIER = 100 * 1000 * 1000; // 100 million
    ...
    function ensureValidClass(uint256 class) private pure {
        require(
            MIN_CLASS_INCL <= class && class <= MAX_CLASS_INCL,
            "Merge: Class must be [1, 4]."
        );
    }
    ...
    function ensureValidMass(uint256 mass) private pure {
        require(
            MIN_MASS_INCL <= mass && mass < MAX_MASS_EXCL,
            "Merge: Mass must be [1, 100m - 1)."
        );
    }
    ...
    event MassUpdate(
        ...
        uint256 indexed tokenIdPersist,
        uint256 mass
    );
    ...
    modifier onlyValidSender() {
        require(
            INiftyRegistry(_registry).isValidNiftySender(_msgSender()),
            "Merge: Invalid msg.sender"
        );
        _;
    }
    ...
    function merge(uint256 tokenIdRcvr, uint256 tokenIdSndr)
        external
        onlyValidWhitelist
        notFrozen
        returns (uint256 tokenIdDead)
    {
        ...
        require(
            owner == ownerOf(tokenIdSndr),
            "Merge: Illegal argument disparate owner."
        );
        require(
            _msgSender() == owner,
            "ERC721: msg.sender is not token owner."
        );
    ...
    function _transfer(
        ...
        require(
            ...
            "Merge: transfer attempt to blacklist address"
        );
        ...
        returns (uint256 tokenIdDead)
    {
        require(
            ...
            "Merge: Illegal argument identical tokenId."
        );
        ...
        onlyPak
    {
        require(
            address_ != _omnibus,
            "Merge: Illegal argument address_ is _omnibus."
        );
    ...
    function whitelistUpdate(address address_, bool status) external onlyPak {
        if (address_ == _omnibus) {
            require(
                status != false,
                "Merge: Illegal argument _omnibus can't be removed."
            );
        }
        ...
        if (status == false) {
            require(
                ...
                "Merge: Address with more than one token can't be removed."
            );
        }
        ...
        while (valueIx < values_.length) {
            if (isSentinelMass(values_[valueIx])) {
                ...
                (
                    ...
                    /* uint256 class */
                    uint256 mass
                ) = decodeClassAndMass(values_[valueIx]);
    ...
    function batchSetMergeCountFromSnapshot(
        uint256[] calldata tokenIds_,
        uint256[] calldata mergeCounts_
    ) external onlyValidSender {
        require(!_mintingFinalized, "Merge: Minting is finalized.");
    ...
    function encodeClassAndMass(uint256 class, uint256 mass)
        public
        pure
        returns (uint256)
    {
    ...
    function decodeClassAndMass(uint256 value)
        public
        pure
...
import {ABDKMath64x64} from "./utils/ABDKMath64x64.sol";
import {Base64} from "./utils/Base64.sol";
import {Roots} from "./utils/Roots.sol";
