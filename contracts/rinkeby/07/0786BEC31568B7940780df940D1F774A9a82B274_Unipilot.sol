...
contract Unipilot is IUnipilot, ERC721Permit, PeripheryPayments {
        ...
        returns(uint128 baseLiquidity, uint128 rangeLiquidity, uint256 mintedTokenId)
    {
        ...
        (depositVars.totalAmount0, depositVars.totalAmount1, depositVars.totalLiquidity) = IHandler(
            params.exchangeAddress
        ).getReserves(params.token0, params.token1, data);

        (depositVars.shares, depositVars.amount0, depositVars.amount1) = getSharesAndAmounts(
        ...
        (baseLiquidity, rangeLiquidity, mintedTokenId) = IHandler(params.exchangeAddress).deposit(
            params.token0,
            params.token1,
            _msgSender(),
            depositVars.amount0,
