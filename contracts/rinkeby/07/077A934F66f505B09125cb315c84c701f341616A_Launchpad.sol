...
contract Launchpad is Ownable {
    ...
    address[] private goldParticipants;
    address[] private silverParticipants;
    address[] private bronzeParticipants;
    address[] private publicParticipants;
    ...
    event EWithdrawBalance(address _sender, uint256 _balance);
    ...
    // function updateParticipantsBalances() internal {}
        ...
        )
    {
        ...
        return (
            ...
            whitelistWallet.allocationBalance,
            whitelistWallet.rewardBalance,
    ...
    function getTotalParticipants() public view returns (uint256) {
        return
            goldParticipants.length +
            silverParticipants.length +
            bronzeParticipants.length;
    }
    ...
    function registerIDO() external {
        ...
        if (userBalance > GOLD_THRESHOLD) {
            goldParticipants.push(_address);
            ...
            silverParticipants.push(_address);
            Whitelist memory _whitelist = Whitelist(
                _address,
                true,
                0,
            ...
            bronzeParticipants.push(_address);
            Whitelist memory _whitelist = Whitelist(
                _address,
                true,
                0,
            ...
            publicParticipants.push(_address);
            Whitelist memory _whitelist = Whitelist(
                _address,
                true,
                0,
    ...
    function getClaimableBalance() public view returns (uint256) {
        ...
        Whitelist memory whitelistSnapshot = whitelist[redeemer];

        uint256 totalWeight = (goldParticipants.length * GOLD_WEIGHT) +
            ((silverParticipants.length * SILVER_WEIGHT)) +
            (bronzeParticipants.length * BRONZE_WEIGHT);

        uint256 totalTokens = getTotalTokens();
        uint256 tokenAllocationPerTier = totalTokens / totalWeight;
        whitelistSnapshot.allocationBalance = tokenAllocationPerTier;
    ...
    function calculateAmount(uint256 acceptedAmount)
        ...
        returns (uint256)
    {
        uint256 tokenDecimal = 10**(ERC20(_token).decimals());
    ...
    function buyTokens() external payable {
        ...
        require(
            ...
            "Cannot participate. You did not register."
        );

        uint256 claimableBalance = getClaimableBalance();
        require(
            claimableBalance >= acceptedAmount,
            "Amount is greater than reward balance"
        );

        uint256 totalAmount = (tokenRate * acceptedAmount) /
            10**(ERC20(_token).decimals());
        require(totalAmount > 0, "You already bought all your allocation.");

        whitelistSnapshot.allocationBalance = claimableBalance - acceptedAmount;
    ...
    function redeemTokens() external {
        ...
        // Update total redeem
        totalRedeemed += whitelistWallet.rewardBalance;
    ...
    function withdrawBalance() external onlyOwner {
        ...
        // Emit event
        emit EWithdrawBalance(msg.sender, balance);
    }
