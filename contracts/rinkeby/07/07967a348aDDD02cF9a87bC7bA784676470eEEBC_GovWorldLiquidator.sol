    ...
    function liquidateLoan(uint256 _loanId) public override onlyTokenMarket {
        ...
        if(lenderDetails.autoSell  == true) {
             ...
             for(uint256 i  = 0 ; i  <  loanDetails.stakedCollateralTokens.length ; i++){
                ...
                //transfer the swap stable coins to the token market contract address.
                swapInterface.swapExactTokensForTokensSupportingFeeOnTransferTokens(amountIn, amountOut, path, _tokenMarket, block.timestamp + 5 minutes);
            }
            ...
            IERC20(loanDetails.borrowStableCoin).approve(address(this), finalAmountToLender);
            IERC20(loanDetails.borrowStableCoin).transferFrom(_tokenMarket,lenderDetails.lender, finalAmountToLender);
            ...
            for(uint256 i  = 0 ; i  <  loanDetails.stakedCollateralTokens.length ; i++){
                ...
                if (collateralAmountinStable <= loanDetails.loanAmountInBorrowed) {
                    IERC20(loanDetails.borrowStableCoin).approve(address(this), loanDetails.stakedCollateralAmounts[i]);
                    IERC20(loanDetails.borrowStableCoin).transferFrom(_tokenMarket,lenderDetails.lender, loanDetails.stakedCollateralAmounts[i]);
                    }
                    ...
                    IERC20(loanDetails.borrowStableCoin).approve(address(this),collateralToLender);
                    IERC20(loanDetails.borrowStableCoin).transferFrom(_tokenMarket,lenderDetails.lender, collateralToLender);
                    break;
                    }
                }
            ...
            loanDetails.loanStatus = TokenLoanData.LoanStatus.LIQUIDATED;
            IERC20(loanDetails.borrowStableCoin).approve(address(this),lenderAmountinStable);
            IERC20(loanDetails.borrowStableCoin).transferFrom(_tokenMarket, lenderDetails.lender, lenderAmountinStable);
