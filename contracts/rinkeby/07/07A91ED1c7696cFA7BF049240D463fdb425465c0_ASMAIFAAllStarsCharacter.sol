...
contract ASMAIFAAllStarsCharacter is ERC721Enumerable, WhitelistTimelock {
  ...
  function reserve(
    ...
    require(reserveIds.length == hashes.length, 'Invalid hashes.');
    require(reserveIds.length == recipients.length, 'Invalid recipients.');

    for (uint256 i = 0; i < reserveIds.length; i++) {
      uint256 newItemId = reserveIds[i];

      require(!reservedTokenIds[newItemId], 'Token already been reserved.');
      ...
      reservedTokenIds[newItemId] = true;
