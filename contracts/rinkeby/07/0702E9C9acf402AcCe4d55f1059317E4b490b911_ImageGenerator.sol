...
contract ImageGenerator is ERC721Enumerable, Ownable, Pausable {
    ...
    constructor(string memory name_, string memory symbol_, string memory baseURI_,
        uint256 price_, uint8 tokenAmountBuyLimit_, uint256 limitSupply_
    ...
    function _preValidatePurchase() internal view {
        ...
        require(msg.value >= price(), "IG: Insufficient funds");
        require(totalSupply() < limitSupply(), "IG: All tokens sold");
    }
    ...
    function buyTokens() external payable whenNotPaused returns (uint256[] memory) {
        _preValidatePurchase();
        uint256 amount = msg.value / price();
        uint256 limitAmount = limitSupply() - totalSupply();

        uint256 tokensAmount = amount < limitAmount ? amount : limitAmount;
        require(tokensAmount <= tokenAmountBuyLimit(), "IG: Limited amount of tokens");
