...
contract OnChainGenArtist is ERC721Enumerable, Ownable {
  ...
  constructor() ERC721("OnChainGenArtist", "ONCHAINGENARTIST") {
    nextProjectId = nextProjectId + 1; // start project id at 1
  }

  function mint(uint256 _projectId, uint numberOfTokens) external payable {
    require(projects[_projectId].isSaleActive, "Sale must be active to mint tokens");
    require(numberOfTokens <= MAX_PUBLIC_MINT, "Exceeded max token purchase");
    require(totalSupply() + numberOfTokens <= projects[_projectId].maxInvocations, "Purchase would exceed max tokens");
    require(projects[_projectId].pricePerTokenInWei * numberOfTokens <= msg.value, "Ether value sent is not correct");

    for (uint i = 0; i < numberOfTokens; i++) {
      _internalMint(msg.sender, _projectId);
    }
  }

  function _internalMint(address _to, uint256 _projectId) internal {
    ...
    bytes32 seed = keccak256(abi.encodePacked(tokenId, _to, block.number, block.difficulty));
    tokenSeeds[tokenId].push(seed);
    _safeMint(_to, tokenId);
    tokenIdToProjectId[tokenId] = _projectId;
    emit MintGenArt(_to, tokenId, _projectId, projects[_projectId].invocations);
  }

  function addProject(uint256 _pricePerTokenInWei, uint256 _mintVolume, string memory _name) external onlyOwner {
  ...
  function setProjectScript(uint256 _projectId, uint256 _scriptId, string memory _script) external onlyOwner {
  ...
  function projectDetails(uint256 _projectId) view public returns (string memory projectName, uint256 pricePerTokenInWei, uint256 invocations, uint256 maxInvocations, bool isSaleActive) {
  ...
  function showTokenSeeds(uint _tokenId) public view returns (bytes32[] memory){
