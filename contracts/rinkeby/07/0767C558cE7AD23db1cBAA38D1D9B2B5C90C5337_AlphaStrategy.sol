...
contract AlphaStrategy is Pausable, AlphaToken {
    ...
    uint256 constant COEFF_SCALE_DECIMALS_F = 1e2;
    uint256 constant COEFF_SCALE_DECIMALS_M = 1e6;
    uint256 constant COEFF_SCALE_DECIMALS_W = 1e18;
    ...
    uint256 public TOTAL_ALPHA_SUPPLY = 10000;
    ...
    function getTVL() public view returns (uint256) {
        return (ALPHA_PRICE * TOTAL_ALPHA_SUPPLY) / (COEFF_SCALE_DECIMALS * COEFF_SCALE_DECIMALS);
    }
    ...
    function calculateProfitFees() external onlyManager {
        require(PROFIT_FEES == 0, "Alpha: profit fees pending minting");
        PROFIT_FEES = ((TOTAL_ALPHA_SUPPLY * (ALPHA_PRICE - ALPHA_PRICE_WAVG) * PROFIT_FEE_RATE) / (COEFF_SCALE_DECIMALS * COEFF_SCALE_DECIMALS * COEFF_SCALE_DECIMALS_F)) * COEFF_SCALE_DECIMALS_W;
    ...
    function calculateManagementFees() external onlyManager {
        require(MANAGEMENT_FEES == 0, "Alpha: management fees pending minting");
        MANAGEMENT_FEES = ((TOTAL_ALPHA_SUPPLY * MANAGEMENT_FEE_RATE) / (COEFF_SCALE_DECIMALS * COEFF_SCALE_DECIMALS_M)) * COEFF_SCALE_DECIMALS_W;
    }
