...
contract HUMBLNFT is ERC721URIStorage, Ownable {
    ...
    function mintNewId(address owner, string memory URI) public returns (uint256) {
        _tokenIds.increment();
    ...
    event returnTokenIds(uint256 id);

    function mintNewCollection(address owner, string memory URI, uint256 quantity) public {
        for(uint count = 1; count <= quantity ; count++) {
           _tokenIds.increment();
           uint256 newTokenId = _tokenIds.current();
           _mint(owner, newTokenId);
           _setTokenURI(newTokenId, URI);
           emit returnTokenIds(newTokenId);
        }
    }
}