...
contract NFTProject is ERC721, Ownable {
    ...
    string internal preRevealTokenURI = "";
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
      return bytes(_baseURI()).length > 0 ? super.tokenURI(tokenId) : preRevealTokenURI;
    }

    function getTokenScore(uint256 tokenId) external view returns (uint256 score) {
      require(scoresAddress != address(0x0), "scores address is not set");
      return IScores(scoresAddress).getTokenScore(tokenId);
    }
    ...
    function _mintNFT(uint256 amount, bytes32 signatureR, bytes32 signatureVS, bool stake) internal {
        ...
        if (stake) {
          NFTStaking(vaultAddress).stakeDuringMint(msg.sender, tokens);
        }
    }
    ...
    {
        require(tokenId <= MAX_BLOCKS, "ERC721Enumerable: owner index out of bounds");
...
contract NFTStaking is Ownable, IERC721Receiver {
  ...
  constructor(NFTProject _nft, Token _token) { 
    nft = _nft;
    token = _token;
  }
  ...
  function claimForAddress(address account, uint24[] calldata tokenIds) external {
  ...
  function _claim(address account, uint24[] calldata tokenIds, bool _unstake) internal {
    uint256 tokenId;
    uint256 score;
    uint256 earned = 0;
