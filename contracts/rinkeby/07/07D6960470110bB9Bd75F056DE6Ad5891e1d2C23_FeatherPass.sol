...
contract FeatherPass is ERC721, Ownable, ReentrancyGuard {
    ...
    // FAKE ADDRESSES
    address r1 = 0x4355b692c9Cd853D0E09EDB74cC1AEF80d595f8e;
    address r2 = 0x4355b692c9Cd853D0E09EDB74cC1AEF80d595f8e;
    address r3 = 0x4355b692c9Cd853D0E09EDB74cC1AEF80d595f8e;
    address r4 = 0x4355b692c9Cd853D0E09EDB74cC1AEF80d595f8e;
    address r5 = 0x4355b692c9Cd853D0E09EDB74cC1AEF80d595f8e;

    mapping(address => uint256) public whitelist;

    constructor(address[] memory addresses, uint256[] memory mintAmounts) ERC721("FeatherPass", "FTHRS") {
    ...
    function mint(uint256 numberOfTokens) public payable {
        ...
        uint256 newSupply = totalSupply().add(numberOfTokens);
        require(newSupply <= numberAvailable, "Exceeds max available.");
        require(newSupply <= MAX_SUPPLY, "Exceeds max supply.");
        require(msg.value == featherPrice.mul(numberOfTokens), "Wrong ETH value sent.");
    ...
    function claim(uint256 numberOfTokens) public {
        ...
        uint256 numRemaining = getWhitelistBalance(msg.sender);
        require(numRemaining > 0, "You have no feathers to claim");
        require(numberOfTokens <= numRemaining, "Unable to claim that many feathers.");
        
        // Update the number of feathers claimed for the sender address
        whitelist[msg.sender] = whitelist[msg.sender].sub(numberOfTokens);
    ...
    function withdrawBalance() public onlyOwner {
        ...
        payable(r3).transfer((balance * 6) / 100);  // 6%
        payable(r4).transfer((balance * 6) / 100);  // 6%
        payable(r5).transfer((balance * 6) / 100);  // 6%
        payable(r1).transfer(address(this).balance); // Transfer remaining balance to treasury

        // payable(msg.sender).transfer(address(this).balance); // Old function
    }
