...
contract TestTP is ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    
    mapping(uint256 => uint256) private assignOrders;
    uint256 public punksRemainingToAssign = 0;
    uint public constant MAX_TP = 20;
	string _baseTokenURI;
    uint256 public constant DNAPrice = 10000000000000000; // 0.01 ETH
	bool public paused = false;
    // Truth.　
    constructor() ERC721("DNAPunk", "DPUNK")  {
        punksRemainingToAssign = 20;
    }

    modifier saleIsOpen{
        require(totalSupply() < MAX_TP, "Sale end");
        _;
    }

    function mintMyTP(uint numberOfTokens) public payable saleIsOpen {
        require(!paused, "Sale is Paused");
        require(punksRemainingToAssign > 0, "No remainig punk");
        require(totalSupply() + numberOfTokens <= MAX_TP, "Purchase would exceed max supply of DNA");
        require(totalSupply() < MAX_TP, "Sale end");
        require(numberOfTokens <= 2, "Can only mint 2 tokens at a tim");
        require(DNAPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
