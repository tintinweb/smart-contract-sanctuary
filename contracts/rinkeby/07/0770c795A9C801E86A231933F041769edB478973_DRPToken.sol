...
import "../artifacts/base64-sol/base64.sol";
...
contract DRPToken is ERC721Enumerable, Ownable {
    ...
    uint8[] public reserved = [3,13,14,15,19,20,22,24,25,32,36,38,39,42,47,50,52,53,54,56,57,58,70,71,76,77,78,81,86,92,93,97,98,99,102,104,109,114,115,119,120,124,125,127,128,135,145,148,149,150];
    ...
    function approveClaim(uint8 tokenId) public onlyOwner{
        ...
        require(!_exists(tokenId), "Cannot approve already claimed token");
    ...
    function removeApproval(uint8 tokenId) public onlyOwner{
        require(!_exists(tokenId), "Cannot remove approval on existing token");
        require(tokenId > 0 && tokenId <= MAX_SUPPLY, "Edition number must be between 1 and 150");
