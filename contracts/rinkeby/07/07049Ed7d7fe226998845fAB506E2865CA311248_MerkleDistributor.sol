...
contract MerkleDistributor is Ownable {
     ...
     */
    function withdrawRewards(address rewardCurrency, uint256 amount)
        external
        onlyOwner
    {
    ...
    function claimMulti(Claim[] memory claims) external {
        ...
        for (uint256 i = 0; i < claimCount; i++) {
            ...
            if (
                ...
                address(merkleWindows[claims[nextI].windowIndex].rewardToken) !=
                currentRewardToken
                // Next claim reward token is different than current one.
            ) {
                IERC20(currentRewardToken).safeTransfer(
                    _claim.account,
                    batchedAmount
                );
                batchedAmount = 0;
    ...
    function claim(Claim memory _claim) public {
        _verifyAndMarkClaimed(_claim);
        merkleWindows[_claim.windowIndex].rewardToken.safeTransfer(
            _claim.account,
            _claim.amount
        );
    }
     ...
     */
    function isClaimed(uint256 windowIndex, uint256 accountIndex)
        public
        view
        returns (bool)
    {
    ...
    function verifyClaim(Claim memory _claim) public view returns (bool valid) {
        ...
        return
            MerkleProof.verify(
                _claim.merkleProof,
                merkleWindows[_claim.windowIndex].merkleRoot,
                leaf
            );
    }
    ...
    function _setWindow(
        ...
        window.rewardToken.safeTransferFrom(
            msg.sender,
            address(this),
            rewardsDeposited
        );
    ...
    function _verifyAndMarkClaimed(Claim memory _claim) private {
        ...
        require(
            !isClaimed(_claim.windowIndex, _claim.accountIndex),
