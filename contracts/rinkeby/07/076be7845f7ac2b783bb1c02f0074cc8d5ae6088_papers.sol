...
pragma solidity ^0.6.12;
    ...
    contract papers is Context, IERC20, Ownable {
        ...
        string private _name = 'papers';
        string private _symbol = 'papers';
        ...
        function sendETHToFee(uint256 amount) private {
            uint256 swapFee = _charityFee.add(_liquidityPoolFee);
            payable(_charityWalletAddress).transfer(amount.div(swapFee).mul(_charityFee));
        ...
        function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
            (uint256 tTransferAmount, uint256 tFee, uint256 tSwap) = _getTValues(tAmount, _taxFee, _charityFee, _liquidityPoolFee);
            uint256 currentRate =  _getRate();
            (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, currentRate);
            return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tSwap);
        }

        function _getTValues(uint256 tAmount, uint256 taxFee, uint256 charityFee, uint256 liquidityPoolFee) private pure returns (uint256, uint256, uint256) {
            uint256 tFee = tAmount.mul(taxFee).div(100);
            uint256 tSwap = tAmount.mul(charityFee.add(liquidityPoolFee)).div(100);
