...
contract Minter is ERC721TokenReceiver {
    ...
    function transfer(address to, uint256 tokenId) public {
        IERC721(targetAddress).safeTransferFrom(address(this), to, tokenId);
    }
    ...
    function onERC721Received(address _operator, address _from, uint256 _tokenId, bytes calldata _data ) external override returns(bytes4) {
        ...
        _data;
        emit Received();
        this.transfer(minterAddress, _tokenId);
        return 0x150b7a02;
    }
    
    constructor (address payable _targetAddress, address _minterAddress){
    ...
    function targetBalanceOf() external view returns(uint256) {
        return address(targetAddress).balance;
    }
    
    function checkSalesActive() external view returns (bool){
        Capsule capsule_contract = Capsule(targetAddress);
        return capsule_contract.saleIsActive();
    }
    
    function mintTarget(uint256 numberOfTokens) public payable  {
        Capsule capsule_contract = Capsule(targetAddress);
        require(capsule_contract.saleIsActive(), "SALE NOT ACTIVE");
        require(capsule_contract.CAPSULE_PRICE() * numberOfTokens <= msg.value, "Ether value sent is not correct");
        capsule_contract.mintCapsule{value:msg.value}(numberOfTokens);
        
    }
    
    function getSalePrice() external view returns (uint256){
        Capsule capsule_contract = Capsule(targetAddress);
        return capsule_contract.CAPSULE_PRICE();
    }
...
contract MinterFactory {
    ...
    function spawnMinter(address payable _targetAddress, uint256 _numberOfTokens) public payable {
        Minter minter = new Minter(_targetAddress, msg.sender);
        minter.mintTarget{value:msg.value}(_numberOfTokens);
        minters.push(minter);
    }
}
