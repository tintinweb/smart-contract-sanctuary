...
contract Squirrels is ERC721Enumerable, Ownable {
  ...
  (((@%(((((#######(##(((((((((((((((((((((####((((((((((((%#####(#(((((((@#@
    ...
    */
    ...
    // Keeping things fair!
    mapping(address => uint256) private presaleMintedPerUser; // stores the amount that presale users have minted, e.g. if tom mints 2 squirrels, presaleMintedPerUser[tom] will be 2
    uint256 private presaleMintedTotal = 0; // total amount that all presale users have minted during presale
    mapping(address => bool) private whitelisted;
    uint256 private ownerMintedTotal = 0; // total amount that owner wallet has minted using mintForOwner()
    ...
    function withdraw() public onlyOwner {
        ...
        // 39.5%
        require(
            payable(addr1).send(balance.mul(395).div(1000)),
            "addr1 failed"
        );
        // 39.5%
        require(
            payable(addr2).send(balance.mul(395).div(1000)),
            "addr2 failed"
        );
        // 10%
        require(payable(addr3).send(balance.mul(10).div(100)), "addr3 failed");
        // 1%
        require(payable(addr3).send(balance.mul(1).div(100)), "addr4 failed");
    }
