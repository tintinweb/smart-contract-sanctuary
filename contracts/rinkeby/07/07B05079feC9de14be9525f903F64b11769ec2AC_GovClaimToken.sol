...
contract GovClaimToken is Ownable, IGovClaimToken {
    ...
    function configureProtocolRegistry(address _govProtocolRegistry)
        public
        onlyOwner
    {
        require(
            _govProtocolRegistry != address(0),
            "GovClaimToken: null address"
        );
        govProtocolRegistry = IGovWorldProtocolRegistry(_govProtocolRegistry);
    }

    //modifier: only sp admin wallet can add claim tokens
    modifier onlySPWalletAdmin(
        address _tokenAddress,
        address _spWalletAddress
    ) {
        require(
            govProtocolRegistry.isAddedSPWallet(
                _tokenAddress,
                _spWalletAddress
            ),
            "GovClaimToken: only sp admin wallet allowed"
        );
        _;
    }
