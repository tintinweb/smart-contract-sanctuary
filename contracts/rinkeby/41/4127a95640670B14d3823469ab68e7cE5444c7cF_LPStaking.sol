...
contract LPStaking is ILPStaking, Staking {
	...
	uint256 public constant BLOCKS_PER_YEAR = 2_354_250;
	...
	function getAPY() external view override returns (uint256 APY) {
		...
		APY = rewardPerBlock.mul(BLOCKS_PER_YEAR).mul(PERCENTAGE_100).div(
			totalStaked.add(DECIMALS18).mul(reserveLACE.mul(2).mul(10**20).div(totalSupply))
		);
	}
}
