...
contract First is ERC721,ReentrancyGuard {
  ...
  bytes[32] public t1; //trait 1
  //pointers of strings in t1
  ...
  function calc_pointers(uint16 cid) public  {
    uint16 k=0;
    p1[cid][k] = 0;
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    uint i = 0;
    uint e = 0;    
    uint ll = 28; //max length of each line
...
//      console.log("i:%d e:%d len: %d",i,e,b.length);
    ...
    /*
    string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "First NFT #', toString(tokenId), '", "description": "First satirical generative NFT about firsts.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    */
    ...
    function claim(uint256 tokenId) public nonReentrant {
        require(tokenId > 0 && tokenId < 4500, "Token ID invalid");
        _safeMint(_msgSender(), tokenId);
    }
    
    function ownerClaim(uint256 tokenId) public nonReentrant requireAdmin {
        require(tokenId >= 4500 && tokenId < 5001, "Token ID invalid");
        _safeMint(admin_address, tokenId);
    }
