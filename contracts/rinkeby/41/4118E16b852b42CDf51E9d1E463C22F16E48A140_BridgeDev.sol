...
contract BridgeDev is AccessControl, Pausable, ReentrancyGuard {
  ...
  mapping(address => mapping(address => uint256)) public tokenToNativeBalance;
  ...
  function deposit(address _ethToken, uint256 amount) external nonReentrant whenNotPaused returns(bool) {
    ...
    IERC20(_ethToken).safeTransferFrom(msg.sender, address(this), amount);
    
    // update token and amount in user mapping
    tokenBalance[msg.sender][_ethToken] += amount;
    address ektaToken = EthEktaPairs[_ethToken];
    emit TokenDeposited(_ethToken, ektaToken, amount, msg.sender);
    return true;
  }

  function withdraw(address _ethToken, address user, uint256 amount, uint256 nonce, bytes calldata signature) external nonReentrant whenNotPaused returns(bool) {
    require(_ethToken != address(0), "Bridge: Token cant be zero address");
    require(amount > 0, "Bridge: Amount cant be zero or negative numbers");
    require(amount <= tokenBalance[user][_ethToken], "Bridge: Amount is higher than staked");
    // verify signature
    require(_verify(abi.encodePacked(user, amount, nonce, "ETH_WITHDRAW"), signature), "Bridge: Invalid signature");
    // check for nonce
    ...
    IERC20(_ethToken).safeTransfer(user, amount);
    
    // update token and amount in user mapping
    tokenBalance[user][_ethToken] -= amount;
  ...
  function depositTokenToNative(address _ethToken, uint256 amount) external nonReentrant whenNotPaused returns(bool) {
    require(_ethToken != address(0), "Bridge: Token cant be zero address");
    require(amount > 0, "Bridge: Amount cant be zero or negative numbers");
    ...
    // update token and amount in user mapping
    tokenToNativeBalance[msg.sender][_ethToken] += amount;
    emit DepositedTokenToNative(_ethToken, amount, msg.sender);
    return true;
  }

  function withdrawNativeToToken(address _ethToken, address user, uint256 amount, uint256 nonce, bytes calldata signature) external nonReentrant whenNotPaused returns(bool) {
    require(_ethToken != address(0), "Bridge: Token cant be zero address");
    require(amount > 0, "Bridge: Amount cant be zero or negative numbers");
    require(amount <= tokenToNativeBalance[user][_ethToken], "Bridge: Amount is higher than deposited");
    // verify signature
    ...
    // update token and amount in user mapping
    tokenToNativeBalance[user][_ethToken] -= amount;

    // update nonce
    processedNonces[msg.sender][nonce] = true;
    
    emit WithdrawnNativeToToken(_ethToken, amount, user);
    return true;
  }
