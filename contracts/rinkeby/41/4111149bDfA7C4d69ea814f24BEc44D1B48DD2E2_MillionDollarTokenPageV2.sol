...
contract MillionDollarTokenPageV2 is Initializable, ERC721Upgradeable, PausableUpgradeable, OwnableUpgradeable, UUPSUpgradeable, IERC721ReceiverUpgradeable {
    ...
    function initialize(uint16 _totalMintLimit, uint16 _singleMintLimit, uint16 _ownershipMintLimit, uint256 _mintPrice, string memory _metadataBaseURI, string memory _defaultContentBaseURI, address _original, uint256[] memory _tokenIdsToMigrate) public virtual initializer {
        __MillionDollarTokenPageV2_init(_totalMintLimit, _singleMintLimit, _ownershipMintLimit, _mintPrice, _metadataBaseURI, _defaultContentBaseURI, _original, _tokenIdsToMigrate);
    }

    function __MillionDollarTokenPageV2_init(uint16 _totalMintLimit, uint16 _singleMintLimit, uint16 _ownershipMintLimit, uint256 _mintPrice, string memory _metadataBaseURI, string memory _defaultContentBaseURI, address _original, uint256[] memory _tokenIdsToMigrate) public onlyInitializing {
        __ERC721_init("MillionDollarTokenPage", "\u22A1");
        __Pausable_init();
        __Ownable_init();
        __UUPSUpgradeable_init();
        __MillionDollarTokenPageV2_init_unchained(_totalMintLimit, _singleMintLimit, _ownershipMintLimit, _mintPrice, _metadataBaseURI, _defaultContentBaseURI, _original, _tokenIdsToMigrate);
    }

    function __MillionDollarTokenPageV2_init_unchained(uint16 _totalMintLimit, uint16 _singleMintLimit, uint16 _ownershipMintLimit, uint256 _mintPrice, string memory _metadataBaseURI, string memory _defaultContentBaseURI, address _original, uint256[] memory _tokenIdsToMigrate) public onlyInitializing {
        canMintCenter = false;
        ...
        for (uint8 tokenIdIndex = 0; tokenIdIndex < _tokenIdsToMigrate.length; tokenIdIndex++) {
            _owners[_tokenIdsToMigrate[tokenIdIndex]] = address(original);
    ...
    function _setTokenContentURI(uint256 tokenId, string memory contentURI) internal {
        require(!paused(), "MDTP: cannot set content URI while paused");
    ...
    function tokenContentURI(uint256 tokenId) public view returns (string memory) {
        ...
        return string(abi.encodePacked(defaultContentBaseURI, StringsUpgradeable.toString(tokenId), ".json"));
    }
    ...
    function mintTokenTo(address receiver, uint256 tokenId) public payable {
        ...
        require(balanceOf(receiver) + 1 <= ownershipMintLimit, "MDTP: reached current ownership limit");
    ...
    function mintTokenGroupTo(address receiver, uint256 tokenId, uint8 width, uint8 height) public payable {
        ...
        require(balanceOf(receiver) + quantity <= ownershipMintLimit, "MDTP: reached current ownership limit");
        ...
        _mintTokenGroup(receiver, tokenId, width, height);
    }

    function _mintTokenGroup(address receiver, uint256 tokenId, uint8 width, uint8 height) internal onlyValidGroup(tokenId, width, height) {
        for (uint8 y = 0; y < height; y++) {
            for (uint8 x = 0; x < width; x++) {
                uint256 innerTokenId = tokenId + (ROW_COUNT * y) + x;
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override {
        ...
        require(!paused(), "MDTP: cannot transfer while paused");
        if (to != from) {
            if (from == address(0)) {
    ...
    function onERC721Received(address operator, address from, uint256 tokenId, bytes calldata) external override returns (bytes4) {
        require(operator == address(original), "MDTP: cannot accept token from contract");
        ...
        this.transferFrom(address(this), from, tokenId);
