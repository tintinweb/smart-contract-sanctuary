    ...
    */
    ...
    function activateLoan(uint256[] memory loanIds, uint256[] memory stableCoinAmounts, bool[] memory _autoSell) public {
        ...
        for(uint256 k  = 0 ; k  <  loanOffersToken[loanIds[i]].stakedCollateralTokens.length; k++){
            uint256 allowance =  IERC20(loanOffersToken[loanIds[i]].stakedCollateralTokens[k]).allowance(loanOffersToken[loanIds[i]].borrower, address(this));
            require( allowance >= loanOffersToken[loanIds[i]].stakedCollateralAmounts[k],"GLM: Transfer amount exceeds allowance.");
            
            IERC20(loanOffersToken[loanIds[i]].stakedCollateralTokens[k]).transferFrom(loanOffersToken[loanIds[i]].borrower, address(this), loanOffersToken[loanIds[i]].stakedCollateralAmounts[k]);

            Market memory  market = govWorldProtocolRegistry.getSingleApproveToken(loanOffersToken[loanIds[i]].stakedCollateralTokens[k]);
    ...
    function getTotalPaybackAmount(uint256 _loanId) external view override returns (uint256, uint256, uint256) {
    ...
    function fullLoanPaybackEarly(uint256 _loanId) internal {
        ...
        stableCoinAPYFeeFromToken[loanOffersToken[_loanId].borrowStableCoin] = stableCoinAPYFeeFromToken[loanOffersToken[_loanId].borrowStableCoin].sub(earnedAPYFee);
    ...
    function liquidateLoan(uint256 _loanId) public onlyLiquidatorRole(msg.sender) {
        govWorldLiquidator.liquidateLoan(_loanId);
    }
  ...
  /**
    ...
    */
    ...
    function swapCollateralTokens(
        ...
        address[] memory path,
        address to,
        uint256 deadline) external override onlyLiquidatorContract {
     ...
     */
    function transferCollateral(address _collateralToken, address _to, uint256 _amount) external override onlyLiquidatorContract {
