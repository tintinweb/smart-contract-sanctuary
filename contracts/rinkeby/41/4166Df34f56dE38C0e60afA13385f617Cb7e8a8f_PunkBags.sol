...
contract PunkBags is ERC721, Ownable
{
    ...
    uint256 price= 50000000000000000;
    ...
    uint256 public alienPublicPrice=2000000000000000000; //2.0 ETH

    constructor () public ERC721("PunkBags", "PB") {}
    ...
    function mint(uint256 punkId) public payable  {
        ...
        require(!_exists(punkId),"Already Minted!");
        if (punkId==635 || punkId ==2890 || punkId == 3100 ||punkId == 3443 ||punkId == 5822 || punkId ==5905 ||punkId ==  6089 ||punkId ==   7523 ||punkId == 7804 )
        {
            require(publicPrice <= alienPublicPrice, "Ether value sent is not correct for Aliens!");
        }
        _safeMint(msg.sender, punkId);
    }
    ...
    function setAlienPrice(uint256 newPrice) public onlyOwner {
        alienPublicPrice = newPrice;
    }
    ...
    function tokenURI(uint256 __tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Punk Bag #', tokenId.toString(), '", "description": "Punk Bags are gear for your punks stored on-chain. This collection is derived entirely from on-chain Cryptopunks metadata.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
