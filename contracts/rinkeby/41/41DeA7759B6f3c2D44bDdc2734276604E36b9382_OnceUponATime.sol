...
contract OnceUponATime is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    RobotosContract private ROB;

    constructor(address dependentContractAddress)
        ERC721("OnceUponATime", "OUAT")
    {
        ROB = RobotosContract(dependentContractAddress);
    }
        ...
        returns (string memory)
    {
        ...
        parts[
            0
        ] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.font { fill: black; font-family: sans-serif; font-size: 12px; }</style><rect width="100%" height="100%" fill="#cfe0e5" /><foreignObject x="10" y="20" width="200" height="350" class="font">';
    ...
    function robotoClaim(uint256 tokenId) public nonReentrant {
        require(tokenId > 4700 && tokenId < 4800, "Token ID invalid");
        uint256 ownedRobotos = ROB.balanceOf(_msgSender());
        require(
            ownedRobotos > 0,
