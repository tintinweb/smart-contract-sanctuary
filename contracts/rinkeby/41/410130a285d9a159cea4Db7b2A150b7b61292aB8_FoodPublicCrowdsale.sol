...
contract FoodPublicCrowdsale is TimedCrowdsale {
    ...
    function buyTokens(uint amount) external onlyWhileOpen nonReentrant {
        ...
        require(amount > 0, "FoodCrowdSale: purchasable amount is 0");
    ...
    function claim() external nonReentrant {
        require(hasClosed(), "FoodCrowdSale: not closed");
        require(!claimed[msg.sender], "FoodCrowdSale: already claimed");

        uint tokenAmount = getTokenAmount(purchasedAddresses[msg.sender]);
        require(tokenAmount > 0, "FoodCrowdSale: not purchased");

        require(address(token) != address(0), "FoodCrowdSale: token not set");
