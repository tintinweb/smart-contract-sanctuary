...
contract ValidatorCloneFactory {
    ...
    constructor() {
        ...
        ID_VALIDATOR_MASTER_COPY = address(
            new IdValidator(new Jukebox.ERC721Grouping[](0))
        );
    }
    ...
    function deployIdValidatorClone(
        Jukebox.ERC721Grouping[] memory erc721Groupings
...
contract Jukebox is IERC721Receiver, ReentrancyGuard {
    event OrderCreated(
        address validator,
...
contract IdValidator is IERC721Validator, Ownable {
    event IdsSet(Jukebox.ERC721Grouping[] tokenGroupings, bool set);

    mapping(address => mapping(uint256 => bool)) public acceptedTokens;

    constructor(Jukebox.ERC721Grouping[] memory tokenGroupings) {
    ...
    function initialize(
        address _owner,
        Jukebox.ERC721Grouping[] memory tokenGroupings
    ) public {
        require(owner() == address(0));
        _transferOwnership(_owner);
        _setIds(tokenGroupings, true);
    ...
    function setIds(Jukebox.ERC721Grouping[] memory tokenGroupings, bool set)
        public
        onlyOwner
    {
    ...
    function _setIds(Jukebox.ERC721Grouping[] memory tokenGroupings, bool set)
        internal
    {
        for (uint256 i = 0; i < tokenGroupings.length; i++) {
            Jukebox.ERC721Grouping memory grouping = tokenGroupings[i];
