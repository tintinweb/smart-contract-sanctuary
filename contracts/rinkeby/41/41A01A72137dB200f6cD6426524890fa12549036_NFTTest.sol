...
contract NFTTest is ERC721URIStorage, Ownable {
    ...
    uint256 public maxItems                       = 1500;      // Maximum items in the collection
    uint256 public price                          = 0.01 ether; // Price for minting one NFT
    uint256 public totalSupply;                                // number of NFTs minted thus far
    string  internal __extensionURI               = "";        // Extension of the URI

    bool    public isActive                       = true;      // If the mint is active
    uint256 public maxMintsPerWallet              = 10;         // Maximum number of mint per wallet
    ...
    **/
    ...
    function setMaxMintsPerWallet(uint _max) public onlyOwner{
        maxMintsPerWallet = _max;
    }

    function mint(uint tokenNum) public payable {
        require(isActive, "minting not activated");
        require(totalSupply + tokenNum <= maxItems, "would exceed max supply");
        require(raffleAmountMinted[msg.sender] + tokenNum <= maxMintsPerWallet, "too many tokens mint per wallet");
        ...
        raffleAmountMinted[msg.sender] += tokenNum;
