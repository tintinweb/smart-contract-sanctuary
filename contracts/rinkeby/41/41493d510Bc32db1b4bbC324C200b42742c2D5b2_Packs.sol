...
contract Packs is IPacks, ERC721, ReentrancyGuard {
  ...
  function saleStarted(uint256 cID) public view returns (uint256) {
    LibPackStorage.Storage storage ds = LibPackStorage.packStorage();

    uint256 saleStartTime = ds.collection[cID].saleStartTime;
    if (block.timestamp < (saleStartTime - ds.collection[cID].mintPassDuration)) {
      return 0;
    }
