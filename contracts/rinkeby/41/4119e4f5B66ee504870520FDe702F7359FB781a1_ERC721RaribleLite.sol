...
import "./ERC721LazyLite.sol";
import "../HasContractURI.sol";

abstract contract ERC721BaseLite is
    OwnableUpgradeable,
...
abstract contract ERC721LazyLite is ERC721Lazy, OwnableUpgradeable {
    // tokenId => creators
    mapping(uint256 => address[]) private creators;

    function __ERC721LazyLite_init_unchained() internal initializer {
        __ERC721Lazy_init_unchained();
    }

    function _mintByOperator(uint256 tokenId, LibPart.Part[] calldata _parts)
        internal
    {
        _mint(owner(), tokenId);
        _setTokenURI(tokenId, string(abi.encodePacked(baseURI(), tokenId)));
        _saveFees(tokenId, _parts);
        creators[tokenId] = [_parts[0].account];
    }
...
import "./ERC721BaseLite.sol";
import "@rarible/royalties/contracts/LibPart.sol";

contract ERC721RaribleLite is ERC721BaseLite {
    address private _operator;

    function __ERC721RaribleLite_init(
        string memory _name,
        ...
        _setBaseURI(baseURI);
        __ERC721LazyLite_init_unchained();
    ...
    function mintByOperator(uint256 tokenId, LibPart.Part[] calldata _parts)
        external
    {
        require(
            _msgSender() == _operator || _msgSender() == owner(),
            "ERC721RaribleLite: sender is not operator"
        );
        _mintByOperator(tokenId, _parts);
    }
}
