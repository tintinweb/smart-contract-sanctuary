...
contract UnipilotFarm is IUnipilotFarm, ReentrancyGuard, IERC721Receiver {
    ...
    function getGlobalReward(
        ...
        if (backwardCompatible) {
            tvl =
                IUnipilotFarmV1(deprecated).poolInfo(pool).totalLockedLiquidity +
        ...
        )
    {
        ...
        if (userState.boosterActive) {
            uint256 boostedReward = (_reward.mul(userState.booster)).div(1e18);
    ...
    function toggleBooster(uint256 tokenId) public onlyStake {
    ...
    //TESTING FUNCTION BELOW REMOVE ON DEPLOY
