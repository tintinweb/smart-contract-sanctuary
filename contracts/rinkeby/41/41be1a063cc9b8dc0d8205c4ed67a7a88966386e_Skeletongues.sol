...
contract Skeletongues is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  function isExistsInSgb(uint256 tokenId) public view returns (bool) {
  ...
  function isCanMintOne(uint256 tokenId) public view returns (bool) {
    if (isExistsInSgb(tokenId) && !_exists(tokenId)) {
      return true;
    }
    return false;
  }
  ...
  function mintOne(uint256 tokenId) external {
    ...
    require(isExistsInSgb(tokenId), "NO_OWN");
