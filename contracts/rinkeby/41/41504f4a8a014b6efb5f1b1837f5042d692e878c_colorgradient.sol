...
/*        return string(
    ...
    function _rng(uint256 tokenId) internal returns(uint256) {
        ...
        if (rand_counter % 2 == 0){
            return uint256(keccak256(abi.encodePacked(block.timestamp,toString(tokenId),counter)));}
        else{
          return uint256(keccak256(abi.encodePacked(block.timestamp + tokenId, toString(tokenId), counter)));}
    }
