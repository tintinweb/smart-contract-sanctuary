...
contract CrossLinkX is usingProvable, IERC20, ICrossLinkX, IERC20Metadata, Ownable {
    ...
    string private _coordinatorUrl;
    ...
    */ 
    function moveTokens(uint256 amount, string memory toChain, string memory addressTo) public returns (bool){
        ...
        Transaction[] storage transactions = _movedChainTransactions[_toLower(addressTo)];
    ...
    */ 
    ...
    function doesAddressOwnMovedTokenTransaction(string memory potentialOwnersAddress, uint256 transactionId, uint256 amount, string memory toChain) public view returns(bool){
        ...
        Transaction[] memory transactions = _movedChainTransactions[_toLower(potentialOwnersAddress)];
    ...
    */
    function _isFeeValid(uint256 value) internal returns(bool){
        // Checks fee has been met
        uint256 provableFee = provable_getPrice("URL");
        
        // Check fee is less than or equal to amount supplied
        return provableFee <= value;
    }
    ...
    function _toLower(string memory str) public pure returns (string memory) {
		...
		for (uint i = 0; i < bStr.length; i++) {
			// Uppercase character...
			if ((bStr[i] >= byte(uint8(65))) && (bStr[i] <= byte(uint8(90)))) {
    ...
    function _tryToRedeemTokens(address sender, uint256 id, uint256 amount, string memory fromChain) internal returns(bytes32){
        ...
        bytes memory queryBytes =abi.encodePacked("json(", _coordinatorUrl, "/token/event/from/", fromChain , "/to/", _currentChain , "/address/0x", 
            toAsciiString(sender), "/transaction/", uintToString(id) , "/amount/", uintToString(amount), ").validated");
