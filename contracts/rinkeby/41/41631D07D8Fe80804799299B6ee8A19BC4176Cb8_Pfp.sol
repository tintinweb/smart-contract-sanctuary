...
contract Pfp is ERC721, ERC721Enumerable, Ownable {
	using Strings for uint256;

	event Withdraw(address indexed operator);
	event UpdateFee(address indexed operator, uint256 fee);
	...
	// @notice Represents fee.
	uint256 fee = 10000000000000000;
	// @notice Represents mapping
	mapping(address => bool) claimed;

	constructor() ERC721("Pfp", "PFP") {}

	/// @notice Supports ERC721, ERC721Enumerable interface.
	...
	/// @notice Override _basURI().
	function _baseURI() internal view override returns (string memory) {
		return baseURI;
	}

	/// @notice Override _beforeTokenTransfer() for ERC721Enumerable.
	...
	/// @notice Updates fee
	/// @dev Will revert if not owner.
	/// @param newFee A new fee.
	function setFee(uint256 newFee) external onlyOwner {
		fee = newFee;
		emit UpdateFee(msg.sender, fee);
	}
    ...
    */
	...
	/// @notice Updates osRegistryAddress
	/// @dev Will revert if not enough fee or already minted.
	/// @param tokenId image hash
	function mint(uint256 tokenId) external payable {
		require(!_exists(tokenId) && !claimed[msg.sender], "Already Minted");
