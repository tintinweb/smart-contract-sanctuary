...
contract YieldFarm {
    ...
    constructor(
        ...
        for (uint256 i = 0; i < poolTokenAddresses.length; i++) {
            ...
            poolTokens.push(TokenDetails(addr, IERC20Metadata(addr).decimals()));
        }
    ...
    function _getPoolSize(uint128 epochId) internal view returns (uint256) {
        ...
        for (uint256 i = 0; i < poolTokens.length; i++) {
            totalPoolSize = totalPoolSize + staking.getEpochPoolSize(poolTokens[i].addr, stakingEpochId) * 10 ** (18 - poolTokens[i].decimals);
        }
    ...
    function _getUserBalancePerEpoch(address userAddress, uint128 epochId) internal view returns (uint256){
        ...
        for (uint256 i = 0; i < poolTokens.length; i++) {
            totalUserBalance = totalUserBalance + staking.getEpochUserBalance(userAddress, poolTokens[i].addr, stakingEpochId) * 10 ** (18 - poolTokens[i].decimals);
        }
