...
function _orderPlace(address from, uint256 tokenId, uint256 _price) internal{
 ...
 function _changePrice(uint256 value, uint256 tokenId) internal{
    ...
    function mint(uint256 value,uint256 tokenId, uint256 royal, uint256 nooftoken) public {
        ...
        totalQuantity[tokenId] = nooftoken;
       _orderPlace(msg.sender, tokenId, value);
    }


    function saleToken(address payable from, uint256 tokenId, uint256 amount, uint256 NOFToken, string memory tokenAss) public{
        _saleToken(from, tokenId, amount, tokenAss);
        saleTokenTransfer(from, tokenId, NOFToken);
    }
     ...
     function saleTokenTransfer(address payable from, uint256 tokenId,  uint256 NOFToken) internal{
       
            tokenTrans(tokenId, from, msg.sender, NOFToken);
    ...
    function tokenTrans(uint256 tokenId,address from, address to, uint256 NOFToken) internal{
            ...
            newsafeTransferFrom(from, to, tokenId, NOFToken);
            balances[tokenId][from] = balances[tokenId][from] - NOFToken;
            balances[tokenId][to] = balances[tokenId][to] + NOFToken;
         
    }

     function newsafeTransferFrom(address _from, address _to, uint256 _id,uint256  _value) internal{
