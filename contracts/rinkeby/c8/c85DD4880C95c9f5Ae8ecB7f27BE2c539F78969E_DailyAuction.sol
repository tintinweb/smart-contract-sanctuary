...
contract DailyAuction is AccessControl {
    ...
    constructor(CommonCalendar _assetToken, CalendarSteward _calendarToken) {
        ...
        beneficiary = payable(0x613ea163982868DecEf2D9C1A1cbE23e43186549);
    ...
    function mintDay(uint256 year, uint8 month, uint8 dayOfMonth, uint256 _newPrice, uint256 _deposit, string memory dayName
        ...
        if(auctionedDays[targetDay].winner == msg.sender) {
            uint256 remainingValueForDeposit = msg.value;
            calendarToken.listNewToken(msg.sender, month, dayOfMonth, _newPrice, _deposit, dayName, remainingValueForDeposit);
    ...
    function forceMintDay(uint256 _year, uint256 _month, uint256 _day, uint256 _newPrice, uint256 _deposit, string memory dayName) public payable onlyRole(ADMIN_ROLE) returns(uint256) {
        uint targetDay = getDateFromTimestamp(_year, _month, _day);
        ...
        uint256 remainingValueForDeposit = msg.value;
        calendarToken.listNewToken(bidAddress, uint8(_month), uint8(_day), _newPrice, _deposit, dayName, remainingValueForDeposit);
    }
...
contract CalendarSteward {
    ...
    */
    ...
    function listNewToken(
        address to, uint8 month, uint8 dayOfMonth, uint256 _newPrice, uint256 _deposit, string memory dayName, uint256 remainingValueForDeposit
        ...
        if(to != owner) {
            buy(to, tokenID, month, dayOfMonth, _newPrice, _deposit, dayName, remainingValueForDeposit);   
        }
    }
        ...
        collectPatronageAddress(to)
    {
        ...
        deposit[to] = deposit[to].add(depositValue);
