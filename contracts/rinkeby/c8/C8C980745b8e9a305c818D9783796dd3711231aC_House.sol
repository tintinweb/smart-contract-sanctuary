...
contract House is IHouse, ERC721Enumerable, Ownable, Pausable {
    ...
    event ShakeMinted(address indexed owner,uint256 indexed tokenId);
    event RanchMinted(address indexed owner,uint256 indexed tokenId);
    event MansionMinted(address indexed owner,uint256 indexed tokenId);

    event ShakeStolen(address indexed owner,uint256 indexed tokenId);
    event RanchStolen(address indexed owner,uint256 indexed tokenId);
    event MansionStolen(address indexed owner,uint256 indexed tokenId);

    event ShakeBurned(uint256 indexed tokenId);
    event RanchBurned(uint256 indexed tokenId);
    event MansionBurned(uint256 indexed tokenId);

    event ShakeStaked(address indexed owner,uint256 indexed tokenId);
    event RanchStaked(address indexed owner,uint256 indexed tokenId);
    event MansionStaked(address indexed owner,uint256 indexed tokenId);

    event ShakeUnStaked(address indexed owner,uint256 indexed tokenId);
    event RanchUnStaked(address indexed owner,uint256 indexed tokenId);
    event MansionUnStaked(address indexed owner,uint256 indexed tokenId);
    ...
    // 0 is associated with Shake, 1 is associated with Ranch, 2 is associated with Mansion
    ...
    // reference to the Tower contract to allow transfers to it without any approval
    ITower public tower;
    ...
    modifier requireContractsSet() {
        require(address(houseTraits) != address(0) && address(tower) != address(0) && address(randomizer) != address(0), "Contracts not set");
        _;
    }
    ...
    function setContracts(address _houseTraits, address _tower, address _rand) external onlyOwner {
    ...
    function mint(address recipient, uint256 seed) external override whenNotPaused {
        ...
        if(tx.origin != recipient && recipient != address(tower)) {
            ...
            if(tokenTraits[minted].roll == 0) {
                emit ShakeStolen(recipient, minted);
            } else if(tokenTraits[minted].roll == 1) {
                emit RanchStolen(recipient, minted);
            } else {
                emit MansionStolen(recipient, minted);
            }
        }
    ...
    */
    function generate(address recipient, uint256 tokenId, uint256 seed, LastWrite memory lw) internal returns (HouseStruct memory t) {
        t = selectTraits(seed);
        t.roll = 0;
        if (existingCombinations[structToHash(t)] == 0) {
            ...
            if(t.roll == 0) {
                emit ShakeMinted(recipient, tokenId);
            } else if(t.roll == 1) {
                emit RanchMinted(recipient, tokenId);
            } else {
                emit MansionMinted(recipient, tokenId);
            }
            return t;
        }
    ...
    */
    function selectTraits(uint256 seed) internal view returns (HouseStruct memory t) {    
        // Shake & Ranch & Mansion percet is 70% & 25% & 5%
    ...
    */
    function burn(uint256 tokenId) external override whenNotPaused {
        ...
        if(tokenTraits[tokenId].roll == 2) {
            emit MansionBurned(tokenId);
        } else if (tokenTraits[tokenId].roll == 1) {
            emit RanchBurned(tokenId);
        } else {
            emit ShakeBurned(tokenId);
        }
    }
    ...
    /**
    * checks if a token is a Shake
    * @param tokenId the ID of the token to check
    * @return bool - whether or not a token is a Shake
    */
    function isShake(uint256 tokenId) external view override blockIfChangingToken(tokenId) returns (bool) {
    ...
    */
    function emitShakeStakedEvent(address owner, uint256 tokenId) external override whenNotPaused {
        emit ShakeStaked(owner, tokenId);
    }
    ...
    */
    function emitRanchStakedEvent(address owner, uint256 tokenId) external override whenNotPaused {
        emit RanchStaked(owner, tokenId);
    }
    ...
    */
    function emitMansionStakedEvent(address owner, uint256 tokenId) external override whenNotPaused {
        emit MansionStaked(owner, tokenId);
    }
    ...
    * @param tokenId the ID of the token stacked
    */
    function emitShakeUnStakedEvent(address owner, uint256 tokenId) external override whenNotPaused {
        emit ShakeUnStaked(owner, tokenId);
    }
    ...
    */
    function emitRanchUnStakedEvent(address owner, uint256 tokenId) external override whenNotPaused {
        emit RanchUnStaked(owner, tokenId);
    }
    ...
    */
    function emitMansionUnStakedEvent(address owner, uint256 tokenId) external override whenNotPaused {
        emit MansionUnStaked(owner, tokenId);
    }
}
...
interface IHouse is IERC721Enumerable {
    ...
    function emitShakeStakedEvent(address owner, uint256 tokenId) external;
    ...
    function emitShakeUnStakedEvent(address owner, uint256 tokenId) external;
