...
contract Auction{
    ...
    function saveBid(string memory auctionID,uint bid_amount) public returns(bool){
        ...
        //Important :=> Bidder has to give Approval from frontend to this contract address amount = simple amount *10^18
        if(i == temp.length){
            //_ofBidder not present in auction array
            require(usdcInstance.balanceOf(Bidder_Address) >= (bid_amount));
            require(usdcInstance.transferFrom(msg.sender,Escrow_Address,bid_amount),"USDC TRANSFER FAIL");
