...
contract AGE is ERC721, Pausable, Ownable {
    ...
    */
    function mintAge(uint numberOfTokens) public payable whenNotPaused {
        ...
        payable(address(guardVault)).transfer(msg.value / 10);
        for (uint i = 0; i < numberOfTokens; i++) {
            _safeMint(msg.sender, _totalSupply);
            guardVault.initGuardianVault(_totalSupply, vaultValue);
            mintRecords[_msgSender()][0].push(_totalSupply);
            _totalSupply += 1;
        }
    }
    ...
    function preMintAge(
        ...
        payable(address(guardVault)).transfer(vaultValue * numberOfTokens);

        for (uint i = 0; i < numberOfTokens; i++) {
            _safeMint(_msgSender(), _totalSupply);
            guardVault.initGuardianVault(_totalSupply, vaultValue);
            mintRecords[_msgSender()][preStage].push(_totalSupply);
            _totalSupply += 1;
        }
    }
    ...
    */
    function reserveAge(uint numberOfTokens, address to) public onlyOwner payable {
        ...
        payable(address(guardVault)).transfer(msg.value);
        for (uint i = 0; i < numberOfTokens; i++) {
            _safeMint(to, _totalSupply);
            guardVault.initGuardianVault(_totalSupply, vaultsValue);
            _totalSupply += 1;
        }
    }
