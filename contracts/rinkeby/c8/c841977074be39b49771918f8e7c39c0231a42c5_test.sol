...
contract test is ERC721, ERC721Enumerable, Ownable {
	...
	string public DINOS_PROVENANCE = ""; // IPFS PROVENANCE
	string public LICENSE_TEXT = "";
	bool licenseLocked = false;
	
    uint256 public dinosPrice = 49000000000000000; 
    uint256 public constant maxDinosPurchase = 20;
    uint256 public constant MAX_DINOS = 9999;
    uint256 public dinosReserve = 50; 
    uint256 public dinosRsrvMntd;
    ...
    constructor() ERC721("pd", "d") {_safeMint(msg.sender, 0);}
    ...
    function mintDinos(uint256 numberOfTokens) external payable {
        require(mintIsActive, "Minting is not active");
		if (whitelistEnabled)
                require(whitelist[_msgSender()], "Not whitelisted");
        require(numberOfTokens > 0, "Must mint atleast 1 DINOS");
        require(totalSupply() + numberOfTokens <= MAX_DINOS, "Exceeds max supply of DINOS");
        require(numberOfTokens <= maxDinosPurchase, "Can mint 20 DINOS at a time");
        uint256 costToMint = dinosPrice * numberOfTokens;
        require(msg.value >= costToMint, "Ether value sent is LESS");
        for (uint256 i = 0; i < numberOfTokens; i++) {
                _safeMint(msg.sender, totalSupply().add(1));
        }
    }

    function mintRDinos(address _to, uint256 _reserveAmount)
        external
        onlyOwner
    {
        require(_reserveAmount > 0, "Must reserve atleast 1 DINOS"); 
        require(_reserveAmount <= dinosReserve, "Not enough reserve DINOS left");
        for (uint256 i = 0; i < _reserveAmount; i++) {
            _safeMint(_to, totalSupply().add(1));
        }
        dinosRsrvMntd = dinosRsrvMntd.add(_reserveAmount);
        dinosReserve = dinosReserve.sub(_reserveAmount);
    }
	...
	function setProvenanceHash(string memory provenanceHash) public onlyOwner {
        DINOS_PROVENANCE = provenanceHash;
    }
    ...
    function setDinosPrice(uint256 newPrice) public onlyOwner {
        dinosPrice = newPrice;
    }
