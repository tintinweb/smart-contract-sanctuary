...
/// @title Obscura's Curated collection
/// @author Emmy
contract ObscuraCurated is ERC721Enumerable, AccessControlEnumerable, IERC2981 {
    ...
    constructor(address admin, address payable obscuraAddress)
        ERC721("ObscuraCurated", "OBSCURACURATED")
    {
    ...
    function setIsRevealed(uint256 projectId, bool isRevealed)
        external
        onlyRole(MODERATOR_ROLE)
    {
        require(
            projects[projectId].random == 0,
            "Random can only be set once."
        );
        projects[projectId].isRevealed = isRevealed;
        projects[projectId].random = uint256(blockhash(block.number - 3)) / 2;

        emit SetIsRevealedEvent(msg.sender, projectId, isRevealed);
    }
    ...
    function updateAllowedPassId(uint256 projectId, uint256 passId)
        external
        onlyRole(MODERATOR_ROLE)
    {
        projects[projectId].allowedPassId = passId;
    }
    ...
    function createProject(
        ...
        require(allowedPassId > 0, "Pass ID missing.");
    ...
    {
        uint256 circulatingTokens = projects[projectId].circulatingTokens += 1;
        require(projects[projectId].maxTokens > 0, "Project doesn't exist.");
        require(
            projects[projectId].isRandomlyGenerated == true,
            "This project is randomly generated."
        );
        require(projects[projectId].active == true, "Public sale is not open");
        require(
            circulatingTokens <= projects[projectId].maxTokens,
            "All Curated tokens have been minted"
        );
        uint256 tokenId = (projectId * DIVIDER) + (circulatingTokens);
    ...
    function mintToBySelect(
        ...
        uint256 circulatingTokens = projects[projectId].circulatingTokens += 1;
        require(
            projects[projectId].maxTokens > 0,
            "Project must exist before you can mint."
        );
        require(
            tokenId > 0 && projects[projectId].isRandomlyGenerated == false,
            "You must select a tokenId for this project."
        );
        require(
            tokenId <= (projects[projectId].maxTokens),
            "Invalid token ID selected"
        );
        require(projects[projectId].active == true, "Public sale is not open");
        require(
            circulatingTokens <= projects[projectId].maxTokens,
    ...
    {
        if (bytes(tokenIdToCID[tokenId]).length > 0) {
            return string(abi.encodePacked(tokenIdToCID[tokenId]));
        }
        ...
        if (
            bytes(projects[projectId].cid).length > 0 &&
            !project.isRandomlyGenerated
        ) {
            return
                string(
                    abi.encodePacked(
                        "https://arweave.net/",
                        projects[projectId].cid,
                        "/",
                        tokenId.toString()
                    )
                );
        ...
        if (
            ...
            uint256 shuffledId = (tokenId + project.random / 2) %
                project.maxTokens;
            shuffledId += 1;

            if (bytes(projects[projectId].cid).length > 0) {
                return
                    string(
                        abi.encodePacked(
                            "https://arweave.net/",
    ...
    function getAllowedPassId(uint256 projectId)
        external
        view
        returns (uint256 allowedPassId)
    {
        return projects[projectId].allowedPassId;
    }
    ...
    function getProjectMaxTokens(uint256 projectId)
        external
        view
        returns (uint256 maxTokens)
    {
        return projects[projectId].maxTokens;
    }
