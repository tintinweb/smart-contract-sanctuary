...
contract SimpsCity is ERC721Enumerable, Ownable {
    ...
    // function pushUsedRand(uint256 i) public{
    //     used_rand.push(i);
    // }

    // function getUsedRand() public view returns(uint256[] memory){
    //     uint256[] memory b = new uint256[](used_rand.length);
    //     for (uint i=0; i < used_rand.length; i++) {
    //         b[i] = used_rand[i];
    //     }
    //     return b;
    // }

    // function getUsedTokenID() public view returns(uint16[] memory){
    //     uint16[] memory b = new uint16[](used_tokenId.length);
    //     for (uint i=0; i < used_tokenId.length; i++) {
    //         b[i] = used_tokenId[i];
    //     }
    //     return b;
    // }
    ...
    function mint(uint _amount, bool _stake) public payable whenNotPaused{
        ...
        require(_amount > 0 && _amount <= MINT_PER_TX_LIMIT, "Invalid mint amount");
        require(_availableTokens.length > 0, "All tokens for this Phase are already sold");
        //require( phase >= 1, "Public mint not started.");
        uint totalPennyCost = 0;
        
        if (phase == 0||phase == 1) {
            //require(getAuctionAmount(_amount) == msg.value, "Invalid payment amount");
    ...
    function withdraw(address to) external onlyOwner {
        ...
        //payable(address(0x3c2B492c397bd5eE0bE3d43DE777A42d9Ff22515)).transfer(share);
