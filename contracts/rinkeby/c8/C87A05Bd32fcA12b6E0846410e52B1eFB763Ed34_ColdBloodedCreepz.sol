 ...
 * {ERC721Enumerable}.
 */
contract ColdBloodedCreepz is Context, CERC721,VRFConsumerBase, Ownable  {
  ...
  uint256 public constant MAX_PER_WHITELISTED_WALLET = 10;
  ...
  uint256 public ROYALTY_SIZE = 750; // 7.5%
  ...
  address public lizardsStatsContract;
  ...
  constructor(address _royaltyAddress, address _signerAddress) 
  CERC721("Cold Blooded Creepz", "CBC")
  VRFConsumerBase(
  ...
  function buyCreepz(uint256 tokensNumber, bytes calldata signature) public payable {
      ...
      if (!isLimitExempt[_msgSender()]) {
        uint256 currentLimit = whitelistOnly ? MAX_PER_WHITELISTED_WALLET : MAX_PER_WALLET;
        require(_mintedPerAddress[_msgSender()].add(tokensNumber) <= currentLimit, "You have hit the max Creepz per wallet");
      }
      require(totalSupply().add(tokensNumber) <= MAX_SUPPLY, "You tried to mint more than the max allowed");

      if (_msgSender() != owner()) {
        require(tokensNumber.mul(tokenPrice) == msg.value,
      ...
      emit tokensMinted(msg.sender, tokensNumber);
    }
    ...
    function limitExempt(address addressToExempt, bool shouldExempt) public onlyOwner {
      isLimitExempt[addressToExempt] = shouldExempt;
    }
    ...
    function setProvenanceHash(string memory provenanceHash) public onlyOwner {
      ...
      emit provenanceSet(PROVENANCE_HASH);
    }
    
    function setStatsContract(address statsContract) public onlyOwner {
      lizardsStatsContract = statsContract;
    }
 ...
 * {ERC721Enumerable}.
 */
abstract contract CERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = CERC721.ownerOf(tokenId);
    ...
    function _transfer(
        ...
        require(CERC721.ownerOf(tokenId) == from, "ERC721: transfer of token that is not own");
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(CERC721.ownerOf(tokenId), to, tokenId);
    }
