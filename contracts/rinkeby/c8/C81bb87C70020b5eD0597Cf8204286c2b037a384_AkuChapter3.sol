...
contract AkuChapter3 is AdminControl, ICreatorExtensionTokenURI, ReentrancyGuard {
  ...
  function activate(uint16 total, address nifty_omnibus_wallet) external adminRequired {
  ...
  function mintNifty(uint16 niftyType, uint16 count) external adminRequired nonReentrant {
  ...
  function _generateURI(uint256 tokenId) private view returns(string memory) {
    ...
    for (uint i = 0; i < _uriParts.length; i++) {
      if (_checkTag(_uriParts[i], _EDITION_TAG)) {
        byteString = abi.encodePacked(byteString, uint256(_tokenEdition[tokenId]).toString());
      } else if (_checkTag(_uriParts[i], _TOTAL_TAG)) {
        byteString = abi.encodePacked(byteString, uint256(_total).toString());
      } else {
