...
contract PIKA is Context,Owned,  ERC20 {
    ...
    address oldPika = 0x2E7b4bCAE956B81E391Ca89d07d5ae69D5ee412f;
    uint256 oldPika_amount; 

    
    
    //uint256 private minamountTakenOut = 100000 *10**9 * 10 **9;  
    uint256 private minamountTakenOut = 1000 *10**9 * 10 **9;
    ...
    function transferFrom(address _from,address _to,uint256 _amount) public override  returns (bool success) {
        ...
        uint256 _Fee = calPercentofToken(_amount,_taxFeepercent);
        _taxFee +=  _Fee;
        ...
        emit Transfer(_from, _to, _amount-_Fee);
        uint256 currentAllowance = allowed[_from][_msgSender()];
        require(currentAllowance >= _amount, "ERC20: transfer amount exceeds allowance");
        _approve(_from, _msgSender(), currentAllowance - _amount);
    }
    ...
    function transferFromPIKA(address _from,address _to,uint256 _amount) public  returns (uint256 burnedPercent) {
    ...
    {
        percent= calPercentofToken(_amount,1200);
         _burn(_from,percent);
    }else
        percent = 0;
    ...
    function transferFromThunder(address _from,address _to,uint256 _amount) public  returns (uint256 burnedPercent) {
    ...
    if(totalSupply >= MinimumSupply)
    {
        percent= calPercentofToken(_amount,2500);
         _burn(_from,percent);
    }else
        percent = 0;
    ...
    function thunderFunc(address _from,uint256 _amount) public returns(uint256 burnedPercent){
           
           if(totalSupply >= MinimumSupply)
                {  
                    uint256 percent = calPercentofToken(_amount,2500);
                    _burn(_from,percent);
                    return percent;
                }
                else 
    ...
    function viewMinExtractAmt() public view onlyOwner() returns(uint256){
    ...
    function updateFee(uint256 percent) public onlyOwner(){
        _taxFeepercent = percent;
    } 
          ...
          uint256 day = time.div(86400);
...

            {
             //0-10 B they get 5%, 10-100 they get 2.5% and 100 B + they get 1%
                ...
                {
                    uint256 extra = calSwapToken(tokens,10);
                    PIKA(address(this)).transfer(_msgSender(), tokens.add(extra));
                }
     ...
     function calPercentofToken(uint256 _tokens, uint256 cust) internal virtual returns (uint256) {
        uint256 custPercentofTokens = _tokens.mul(cust).div(100 * 10**uint(3));
   ...
   function calSwapToken(uint256 _tokens, uint256 cust) internal virtual returns (uint256) {
     ...
     function viewMinSupply()public view onlyOwner()  returns(uint256) 
    {
