...
// contract TestContract is ERC721, Ownable, PaymentSplitter {
contract TestContract is ERC721, Ownable {
    using SafeMath for uint256;
    using Strings for uint256;

    // wallets
    // TODO: update to actual wallets
    address public marketingWallet = 0xD39b723259db6AEd5ee383dfC35d60974474837F;
    ...
    // Sale variables
    //TODO: update amount before deploy
    uint16 public totalTokens = 200;
    // uint16 public totalTokens = 7777;
    ...
    // Burn variables
    //TODO: update amount before deploy
    uint16 public totalBurnTokens = 25;
    //uint16 public totalBurnTokens = 2500;
    // uint16 public maxBurnPerWallet = 5;
    // counter
    uint16 private _totalBurnSupply = 0; // start with zero

    // metadata URIs
    string public contractURI; // initially set at deploy
    string public baseURI; // initially set at deploy
    string public notRevealedURI; // initially set at deploy
    string public baseExtension = ".json";

    // Mapping Presale Listed Addresses
    ...
    /**
     * @dev Returns the total number of tokens in burned
     */
    ...
    /**
     * @dev Returns the total number of tokens in minted
     */
        ...
        onlyAllowValidCountAndActiveSale(_mintAmount)
    {
        ...
        require(publicSale, "Not started");
        require(
     ...
     * @dev Presale mint
     */
    function presaleMint(uint16 _mintAmount)
        public
        payable
        onlyAllowValidCountAndActiveSale(_mintAmount)
    {
        require(presaleListed[_msgSender()], "Not listed");
        ...
        returns (uint256[] memory)
    {
        ...
        // console.log("balanceOf(", _owner, ")", ownerTokenCount);
    ...
    */
    ...
    /**
     * @dev Updates the max mint amount; just in case
     */
    ...
    /**
     * @dev Updates the presale max tokens per wallet; just in case
     */
    function setMaxTokensPerWalletPresale(uint16 _newMaxTokensPerWalletPresale)
        public
        onlyOwner
    {
        maxTokensPerWalletPresale = _newMaxTokensPerWalletPresale;
    }

    /**
     * @dev Updates the max tokens per wallet; just in case
     */
    function setMaxTokensPerWallet(uint16 _newMaxTokensPerWallet)
        public
        onlyOwner
    {
        maxTokensPerWallet = _newMaxTokensPerWallet;
    }

    /**
     * @dev Updates the public sale cost; just in case
     */
    function setCostPublicSale(uint256 _newCost) public onlyOwner {
        costPublicSale = _newCost;
    }

    /**
     * @dev Updates the presale cost; just in case
     */
    function setCostPresale(uint256 _newCostPresale) public onlyOwner {
        costPresale = _newCostPresale;
    }

    /**
     * @dev Updates the max allowable burns
     */
    function setTotalBurnTokens(uint16 _newTotalBurnTokens) public onlyOwner {
        totalBurnTokens = _newTotalBurnTokens;
    }

    /**
     * @dev Updates the contract uri to metadata; just in case
     */
    ...
    /**
     * @dev Updates the base uri to metadata; just in case
     */
    ...
    /**
     * @dev Updates the not revealed uri to metadata; just in case
     */
    ...
    /**
     * @dev Updates the metadata base extension; just in case
     */
    ...
    /**
     * @dev Presale List - Batch add addresses
     */
    function batchPresaleList(address[] memory _walletList) external onlyOwner {
        for (uint256 i = 0; i < _walletList.length; i++) {
            presaleListed[_walletList[i]] = true;
        }
    }

    /**
     * @dev Presale List - Remove single address
     */
    function removePresaleListUser(address _user) public onlyOwner {
