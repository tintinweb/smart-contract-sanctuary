...
contract CnMCheddar is ICnM, ERC721Enumerable, Ownable, Pausable {
    ...
    function mintCats(address recipient, uint256 seed) external whenNotPaused {
        minted++;
        CatMouse memory t;
        t.isCat = true;
        uint8 shift = 0;
        ...
        emit TokenMinted(recipient, minted, block.number, block.timestamp);
        if (t.isCat) {
            if (t.isCrazy) {
                emit CrazyCatLadyMinted(recipient, minted, block.number, block.timestamp);
            } else {
                emit CatMinted(recipient, minted, block.number, block.timestamp);
            }
        } else {
            emit MouseMinted(recipient, minted, block.number, block.timestamp);
        }
