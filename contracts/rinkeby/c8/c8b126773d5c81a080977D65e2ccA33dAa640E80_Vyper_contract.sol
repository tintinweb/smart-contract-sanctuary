...
@external
def setStaking(_staking: address, _poolId: uint256):
    """
    @notice
        Used to change the address of `staking`.

        This may only be called by governance.
    @param _staking The new staking address to use.
    @param _poolId The new poolId address to use.
    """
    assert msg.sender == self.governance
    ...
    self.poolId = _poolId
    ERC20(self).approve(self.staking, MAX_UINT256)
