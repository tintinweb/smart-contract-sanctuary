...
interface ISudopop {
    function capCount(address _user) external view returns (uint256);
}

library BubbleMaker {
    function getRandomHSL(uint256 uniqueRand) internal view returns (string memory) {
    ...
    function getTraits(string memory name, uint256 poured, uint256 fizz) public view returns (string memory) {
    ...
    function traits(uint256 poured, uint256 fizz) internal view returns (string memory) {
        uint max = 25;
    ...
    // RANDOM STUFF
    function random(uint256 min, uint256 max, uint256 seed) public view returns (uint256) {
        return min + (seed % (max - min + 1));
    }

    function shakeItUpThatllDO(uint256 tokenId) public view returns (uint256) {
        uint256 jeneral = gremblosCoolRandomFunctionThatIsBerryCoolAndGood(tokenId);
        return random(0,25, jeneral);
    }

    function gremblosCoolRandomFunctionThatIsBerryCoolAndGood(uint256 tokenID) public view returns (uint256) {
        return  uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, tokenID)));
    }

    function biggerAndBetterThanGemblosBYAPPSUS(uint256 tokenId) public view returns (uint256[2] memory magicArray) {
        uint256 jeneral = gremblosCoolRandomFunctionThatIsBerryCoolAndGood(tokenId);
    ...
    function getFillHeight(uint256 poured) internal view returns (string memory) {
    ...
    function getBg(uint256 randSeed) internal view returns (string memory) {
    ...
    function getGradient(uint256 randSeed) internal view returns (string memory) {
    ...
    function getTransform(uint256 poured) internal view returns (string memory) {
        return
            string(
                abi.encodePacked(
    ...
    **/
    function getWaves(uint256 randSeed, uint256 poured) internal view returns (string memory) {
    ...
    function getScale(uint256 randSeed) internal view returns (string memory) {
    ...
    function getDuration(uint256 randSeed) internal view returns (string memory) {
        uint256 randonNum_1 = randSeed / 30;
        uint256 randonNum_2 = randSeed / 35;
        return
            string(
    ...
    function getStartTime(uint256 randSeed) internal view returns (string memory) {
    ...
    function getPathFromList(uint256 randSeed) internal view returns (string memory) {
    ...
    */
    function getTransformTranslateX(uint256 randSeed) public view returns (string memory) {
        uint256 randomNum = randSeed / 10;
        return string(abi.encodePacked(
    ...
    function makeBubbles(uint256 num, uint256 seed) public view returns (string memory) {
        string memory out;
    ...
    function makeSVG(uint256 tokenId, uint256 poured, uint256 fizz, uint256 seed) public view returns (string memory) {        
...
contract BottleCaps is ERC20("Bottle Caps", "CAPS") {
    ...
    ISudopop public popContract;

    event RewardPaid(address indexed user, uint256 reward);

    constructor(address _pop) {
        popContract = ISudopop(_pop);
    }
    ...
    function updateRewardOnMint(address _user, uint256 _amount) external {
        // must be the pop contract
        require(msg.sender == address(popContract), "Can't call this");
    ...
    function updateReward(
        ...
        require(msg.sender == address(popContract));
        if (_tokenId < 1001) {
            uint256 time = min(block.timestamp, END);
            uint256 timerFrom = lastUpdate[_from];
            if (timerFrom > 0)
                rewards[_from] += popContract
                    .capCount(_from)
...
contract ERC721Namable is ERC721, Ownable {
    ...
    function changeName(uint256 _tokenId, string memory _name) public virtual {
        address owner = ownerOf(_tokenId);
        require(msg.sender == owner, "ERC721 caller is not the owner");
        require(validateName(_name) == true, "Not a valid name");
        require(isNameReserved(_name) == false, "Name already reserved");

        // toggle previously owned names
        if (bytes(_tokenName[_tokenId]).length > 0) {
            toggleReserveName(_tokenName[_tokenId], false);
        }
...
contract Pseudopop is ERC721Namable, ERC721Enumerable {
    ...
    address public constant gremblo =
        0x06F6327f7F5E09cba31cE98C74071c786112F22d;
    ...
    mapping(address => uint256) public capCount;
    ...
    constructor() ERC721Namable("PseudoPop", "SUDOPOP") {
        _owner = msg.sender;
        maxSupply = 50;
        maxPerTx = 1;
    }
    ...
    function getReward() external {
        bottleCaps.updateReward(msg.sender, address(0), 0);
        bottleCaps.getReward(msg.sender);
    }
    ...
    function transferFrom(
        ...
        bottleCaps.updateReward(from, to, tokenId);
        if (tokenId < 1001) {
            capCount[from]--;
            capCount[to]++;
        }
        ...
        returns (string memory)
    {
        ...
        string memory output = BubbleMaker.makeSVG(tokenId, soda.poured, soda.fizz, soda.magicNum);
        ...
        string memory json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        "{",
                        BubbleMaker.getTraits(super.tokenNameByIndex(tokenId), soda.poured, soda.fizz),
    ...
    function ygmi(uint256 num) public payable {
        ...
        for (uint256 i = 0; i < num; i++) {
            ...
            capCount[msg.sender]++;
            sodas[_currentToken] = Soda(block.timestamp, seedFizz[1], seedFizz[0]);
    ...
    function getSVG(uint256 tokenId, uint256 poured, uint256 fizz, uint256 magicNum) public view returns (string memory) {
        return BubbleMaker.makeSVG(tokenId, poured, fizz, magicNum);
    }
}
