...
contract HoneyX is Ownable, IERC20, IERC20Metadata {
    ...
    function _init1() public onlyOwner {
        ...
        for (uint8 i = 0; i < devAddresses.length; i++) {
            ...
            _transfer(address(this), devAddresses[i], amountSentFirst);
        }
    ...
    function _transfer(
        ...
        bool buy = recipient == _pairAddress;
        bool sell = sender == _pairAddress;
    ...
    function _sendTaxToLiquidity(uint256 amount) private {
        _transfer(address(this), address(_liquidityTaxAddress), amount);
    }
    ...
    function claimTokenForDev() public {
        ...
        _transfer(address(this), _msgSender(), amountClaimable);
    }
    ...
    function airdrop(
        ...
        for (uint256 i = 0; i < airdropAddresses.length; i++) {
            ...
            _transfer(address(this), airdropAddresses[i], amounts[i]);
    ...
    // function createLiquidityFromTax(uint256 amount) public onlyOwner {
    //     require(_liquidityCreatedByThisContract >= amount, "not enough amount");
    //     _disableTax = true;
    //     _transfer(address(this), owner(), amount);
    //     _liquidityCreatedByThisContract =
    //         _liquidityCreatedByThisContract -
    //         amount;
    //     _disableTax = false;
    // }

    // function _createLiquidityFromTax() private {
    //     if (_liquidityCreatedByThisContract > 0) {
    //         _disableTax = 1;
    ...
    //             _liquidityCreatedByThisContract / 2,
    //             1,
    //             path,
    //             address(this),
