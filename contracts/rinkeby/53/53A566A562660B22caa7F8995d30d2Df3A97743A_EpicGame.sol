...
contract EpicGame is ERC721 {
    event NewCharacter(address sender, uint256 tokenId, uint256 characterIndex);
    event AttackComplete(uint256 bossHp, uint256 characterHp);
    ...
    function mintNFT(uint256 _index) external {
        uint256 itemId = _tokenIds.current();

        _safeMint(msg.sender, itemId);

        nftHolderAttributes[itemId] = Character({
            index: _index,
            name: characters[_index].name,
            imageURI: characters[_index].imageURI,
            hp: characters[_index].hp,
            maxHp: characters[_index].maxHp,
            attackDamage: characters[_index].attackDamage
        });
        ...
        emit NewCharacter(msg.sender, itemId, _index);
    }
    ...
    {
        Character memory charAttrs = nftHolderAttributes[_tokenId];
        ...
        string memory json = Base64.encode(
            abi.encodePacked(
                ...
                strTokenId,
                '",',
                '"description": "This is an NFT that lets people play in the Epic Game!",'
                '"image": "',
                charAttrs.imageURI,
                '",',
                '"attributes": [',
    ...
    function attackBoss() public {
        ...
        Character storage character = nftHolderAttributes[playerTokenId];
        ...
        require(
            ...
            string(
                abi.encodePacked(
                    "Unable to attack. Your hero, ",
                    character.name,
                    ", has no remaining health."
                )
            )
        );

        console.log(
            "\n%s is getting ready to attack. They have %s HP and %s AD.\n",
            character.name,
            character.hp,
        ...
        if (boss.hp < character.attackDamage) {
            boss.hp = 0;
            console.log(
                "%s attacked %s, leaving them with 0 HP. They've been defeated!\n",
                character.name,
                boss.name
            );
        } else {
            ...
            console.log(
                "%s attacked %s, leaving them with %s HP.\n",
                character.name,
                boss.name,
                boss.hp
            );
