...
contract BearXNFTStaking is OwnableUpgradeable{
    ...
    function initialize() public initializer{
        __Ownable_init();
        BearXNFTAddress = 0xf61fd4F15fc9d3EeAa248a1cfA7DA90b07047585;
        ROOTxTokenAddress = 0xDc97F24099Ad53b229AcbFE8b2903B07d4Dc39C8;
        SROOTxTokenAddress = 0xe47aB7f4C9B69F98AfaC89Ca93aa48b1aCB418c1;
        totalStakes = 0; 
        MaxSROOTXrate = 100*10**18;
        MinSROOTXrate = 50*10**18;
        MaxRate = 11050*10**18;
    ...
    function unStake(uint256[] memory _ids) public isOnlyStaker{
        ...
        if(sumofgenesisbear >= 5){
            ...
            for(uint256 i = 0; i < stakes[msg.sender].length; i++) {
                if(isGenesisBear(stakes[msg.sender][i].nft_id) && calDay(stakes[msg.sender][i].claimedDate) < 1){
                    flag = false;
                }
            }
            if(flag && stakes[msg.sender].length != 0){
                uint256 claimAmount =  10;
                require(IERC20Upgradeable(ROOTxTokenAddress).balanceOf(address(this)) >= claimAmount * (10 ** 18), "ROOT: staking pool has insufficient funds");
                IERC20Upgradeable(ROOTxTokenAddress).transfer(msg.sender, claimAmount * (10 ** 18));
            }
            for(uint256 i = 0; i < stakes[msg.sender].length; i ++){
                
                uint256 dd = calDay(stakes[msg.sender][i].claimedDate);
                if(isSpecialBear(stakes[msg.sender][i].nft_id)) {
                    claimAmountOfSROOTxToken = 200;
                    _amountIn = 200;
                }
                else {
                    claimAmountOfSROOTxToken = dd *  MaxSROOTXrate / 2;
                     _amountIn = dd * MaxSROOTXrate / 2 ;
                }
               
                uint256 _amountOutMin = 0;
                if(calDay(stakes[msg.sender][i].claimedDate) <= 3)
                    continue;
                IERC20Upgradeable(SROOTxTokenAddress).approve(UNISWAP_V2_ROUTER, _amountIn);
                ...
                require(IERC20Upgradeable(SROOTxTokenAddress).balanceOf(address(this)) >= claimAmountOfSROOTxToken, "staking pool has insufficient funds");
                IERC20Upgradeable(SROOTxTokenAddress).transfer(msg.sender, claimAmountOfSROOTxToken);
             ...
             for(uint256 i = 0; i < stakes[msg.sender].length; i ++){
                if(isGenesisBear(stakes[msg.sender][i].nft_id) ) {
                    uint256 dd = calDay(stakes[msg.sender][i].claimedDate);
                    uint256 claimAmountOfROOTxToken = dd * 10;
        ...
        if(sumofgenesisbear >= 5){
            ...
            for(uint256 i = 0; i < stakes[msg.sender].length; i ++){
                if(isGenesisBear(stakes[msg.sender][i].nft_id) ) {
                    uint256 dd = calDay(stakes[msg.sender][i].claimedDate);
                    uint256 claimAmountOfROOTxToken = dd * 10;
                    finalOfROOTxToken = claimAmountOfROOTxToken;
                    require(IERC20Upgradeable(ROOTxTokenAddress).balanceOf(address(this)) >= claimAmountOfROOTxToken * (10 ** 18), "staking pool has insufficient funds");
                    IERC20Upgradeable(ROOTxTokenAddress).transfer(msg.sender, claimAmountOfROOTxToken * (10 ** 18));
                    stakes[msg.sender][i].claimedDate = block.timestamp;
                }
    ...
    function claimOfSROOTxToken() external isOnlyStaker returns(uint256, bool){
        ...
        bool status = false;
        (uint256 sumofgenesisbear,) = getGenesisBear(msg.sender);
         (uint256 sumofspecialbear,) = getSpecialBear(msg.sender);
        sumofgenesisbear = sumofgenesisbear.add(sumofspecialbear);

        bool flag = true;
        if(sumofgenesisbear >= 5){
            for(uint256 i = 0; i < stakes[msg.sender].length; i++) {
                if(!isClaimable(stakes[msg.sender][i].nft_id) && calDay(stakes[msg.sender][i].claimedDate) < 1){
                    flag = false;
                }
            }
            if(flag && stakes[msg.sender].length != 0){
                for(uint256 i = 0; i < stakes[msg.sender].length; i ++){
                    if(!isClaimable(stakes[msg.sender][i].nft_id)) continue;
                    uint256 dd = calDay(stakes[msg.sender][i].claimedDate);
                    if(isSpecialBear(stakes[msg.sender][i].nft_id)) {
                        claimAmountOfSROOTxToken = claimAmountOfSROOTxToken.add(200);
                    }    
                    else {
                        claimAmountOfSROOTxToken =claimAmountOfSROOTxToken.add( dd *  MaxSROOTXrate / 2);
                    }

                    finalOfSROOTxToken =finalOfSROOTxToken.add( claimAmountOfSROOTxToken);
                    if(calDay(stakes[msg.sender][i].claimedDate) <= 3)
                        continue;
                    status = true;
                    require(IERC20Upgradeable(SROOTxTokenAddress).balanceOf(address(this)) >= claimAmountOfSROOTxToken, "staking pool has insufficient funds");
         ...
         if(sumofgenesisbear >= 5){
            ...
            if(flag && stakes[msg.sender].length != 0){
                for(uint256 i = 0; i < stakes[msg.sender].length; i ++){
                    ...
                    uint256 dd = calDay(stakes[msg.sender][i].claimedDate);
                    if(isSpecialBear(stakes[msg.sender][i].nft_id)) {
                        _amountIn = _amountIn.add(200);
                    }
                    else {

                    _amountIn =_amountIn.add(dd * MaxSROOTXrate / 2);
                    }
                    uint256 _amountOutMin = 0;
    ...
    function claimALL() external isOnlyStaker returns(uint256, uint256, uint256){
        uint256 finalOfROOTxToken = 0;
        uint256 finalOfSROOTxToken = 0;
        uint256 finalOfWETH = 0;
        (uint256 sumofgenesisbear,) = getGenesisBear(msg.sender);
        if(sumofgenesisbear >= 5){
            ...
            if(flag && stakes[msg.sender].length != 0){
                uint256 claimAmount =  10;
                finalOfROOTxToken += claimAmount;
                require(IERC20Upgradeable(ROOTxTokenAddress).balanceOf(address(this)) >= finalOfROOTxToken * (10 ** 18), "ROOTxTokenAddrres: staking pool has insufficient funds");
                IERC20Upgradeable(ROOTxTokenAddress).transfer(msg.sender, finalOfROOTxToken * (10 ** 18));
            ...
            for(uint256 i = 0; i < stakes[msg.sender].length; i ++){
                ...
                uint256 dd = calDay(stakes[msg.sender][i].claimedDate);
                uint256 claimAmountOfSROOTxToken = dd *  MaxSROOTXrate / 2;
                finalOfSROOTxToken += claimAmountOfSROOTxToken;
                uint256 _amountIn = finalOfSROOTxToken;
                uint256 _amountOutMin = 0;
                ...
                finalOfWETH += _amountOutMin;
                require(IERC20Upgradeable(SROOTxTokenAddress).balanceOf(address(this)) >= claimAmountOfSROOTxToken, "SROOTxToken: staking pool has insufficient funds");
                IERC20Upgradeable(SROOTxTokenAddress).transfer(msg.sender, claimAmountOfSROOTxToken);
                stakes[msg.sender][i].claimedDate = block.timestamp;
            }
        ...
        return(finalOfROOTxToken, finalOfSROOTxToken, finalOfWETH);
    }
    ...
    function claimOf(address _addr) public view returns(uint256, uint256,uint256) {
        ...
        (uint256 sumofspecialbear,) = getSpecialBear(_addr);
        sumofgenesisbear = sumofgenesisbear.add(sumofspecialbear);

        if(sumofgenesisbear >= 5){
            bool flag = true;
            for(uint256 i = 0; i < stakes[_addr].length; i++) {
                uint256 temp = calDay(stakes[_addr][i].claimedDate);
                if(isClaimable(stakes[_addr][i].nft_id) &&  temp < 1){
                    flag = false;
                }
            }
            ...
            for(uint256 i = 0; i < stakes[_addr].length; i ++){
                ...
                if(isSpecialBear(stakes[_addr][i].nft_id)) {
                    ...
                    claimAmountOfSROOTxToken = claimAmountOfSROOTxToken.add(dd * MaxSROOTXrate / 2);
                    _amountIn = _amountIn.add(claimAmountOfSROOTxToken);
                }
                
                if(_amountIn == 0) continue;
                uint256 _amountOutMin = getAmountOutMin(SROOTxTokenAddress, WETH, _amountIn);
                claimAmountOfWETH += _amountOutMin;
            }
   
        }
        else {
             for(uint256 i = 0; i < stakes[_addr].length; i ++){
                if(isClaimable(stakes[_addr][i].nft_id) ) {
                    uint256 dd = calDay(stakes[_addr][i].claimedDate);
                    claimAmountOfROOTxToken += dd * 10;
                }
            }
        }
    ...
    function isSpecialBear(uint256 _id) internal pure returns(bool) {
        bool returned;
        if(_id >= 10000000000000 && _id <= 10000000000005){
            returned = true;
        }
        return returned;
    }
