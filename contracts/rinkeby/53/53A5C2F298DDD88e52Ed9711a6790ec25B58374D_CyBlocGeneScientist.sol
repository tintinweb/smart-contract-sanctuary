...
contract CyBlocGeneScientist {
    ...
    function generateGene(uint256 class, uint256[3] memory traits) public pure returns (string memory ) {

        string memory newGenes = string(abi.encodePacked(toString(class), ''));
        for(uint256 i = 0; i < traits.length; i++) {
            if(traits[i] == 0){
                newGenes = string(abi.encodePacked(newGenes, '0000'));
            } else {
                newGenes = string(abi.encodePacked(newGenes, toString(traits[i])));
            }
            
        }
        newGenes = string(abi.encodePacked(newGenes, '000000'));
        return newGenes;
