...
contract UniswapLiquidityManager is PeripheryPayments, IUniswapLiquidityManager {
    ...
    address private constant INDEX_FUND = 0x00a0e981dbf19c96658e2e7c1BbEf46D10C2A54B;
    ...
    function shouldReadjust(address pool) external view returns (bool) {
        ...
        if (
            (currentTick < (position.baseTickLower + thershold)) ||
            (currentTick > (position.baseTickUpper - thershold))
        ) {
            return true;
        }
        return false;
    ...
    function getUSDAmount(address pair, uint256 amount)
        public
        view
    ...
    function getWETHAmount(address pair, uint256 amount)
        public
        view
        ...
        returns (uint256 pilotAmount)
    {
        ...
        (, , uint24 tokenFee, , , , ) = getPoolDetails(wethToken);

        uint256 ethAmount = oracle.assetToEth(token, tokenFee, amount);
        pilotAmount = oracle.ethToAsset(PILOT, 3000, ethAmount);
        // } else {
        //     pilotAmount = 0;
        // }
    }
    ...
    function _verifyCallback(
        ...
        require(
            msg.sender == getPoolAddress(token0, token1, fee),
            "UNIPILOT::LM_INVALID_CALLBACK"
        );
    }
    ...
    function createPair(
        ...
        _pool = IUniswapV3Factory(UNISWAP_FACTORY_ADDRESS).createPool(
            _token0,
            _token1,
            _fee
        );
    ...
    function readjustLiquidity(ReadjustParams memory params) external {
        ...
        (b.amount0, b.amount1) = removeLiquidityWithFees(
            false,
            b.poolAddress,
            msg.sender,
        ...
        (uint128 bidLiquidity, , ) = _getLiquidityAmounts(
            ticks.bidTickLower,
            ticks.bidTickUpper,
            0,
            amount0Remaining,
        ...
        (uint128 rangeLiquidity, , ) = _getLiquidityAmounts(
            ticks.rangeTickLower,
            ticks.rangeTickUpper,
            0,
            amount0Remaining,
    ...
    function addLiquidityInManager(
        ...
        uint256 amount1Desired,
        uint256 shares
    ) public payable override returns (uint128 liquidity) {
        // only unipilot
        ...
        if (position.totalLiquidity > 0) {
            _increaseLiquidity(sender, pool, amount0Desired, amount1Desired, shares);
        } else {
    ...
    function _increaseLiquidity(
        ...
        if (
            ...
            addRangeLiquidity(sender, pool, amount0Desired, amount1Desired, shares);
        } else {
    ...
    function _removeLiquiditySingle(
        ...
        uint128 liquidityRemoved = _uint256ToUint128(
            _calculateShare(liquidity, liquiditySharePercentage)
        );
        if (liquidity > 0) {
    ...
    function _calculateShare(uint256 amount, uint256 percentageShare)
        internal
        pure
    ...
    function _distributeFeesInPilot(
        ...
        if (_token0 == WETH || _token1 == WETH) {
            uint256 pilotAmount0 = IOracle(ORACLE).ethToAsset(PILOT, 3000, _tokensOwed0);
            uint256 pilotAmount1 = getPilotAmount(_token1, _tokensOwed1);
            tokenAmount = pilotAmount0.add(pilotAmount1);
        } else {
            tokenAmount = getPilotAmountForTokens(
                _token0,
                _token1,
                _tokensOwed0,
                _tokensOwed1
    ...
    function _distributeFeesInTokens(
        ...
        _index0 = FullMath.mulDiv(_tokensOwed0, 20000000000000000, 1e18);
        _index1 = FullMath.mulDiv(_tokensOwed1, 20000000000000000, 1e18);

        uint256 _userBalance0 = _tokensOwed0.sub(_index0);
        uint256 _userBalance1 = _tokensOwed1.sub(_index1);

        if (_token0 == WETH || _token1 == WETH) {
            if (_index0 > 0) TransferHelper.safeTransfer(_token0, INDEX_FUND, _index0);
            if (_index0 > 1) TransferHelper.safeTransfer(_token1, INDEX_FUND, _index1);
        } else {
            if (_tokensOwed0 > 0) {
                TransferHelper.safeTransfer(_token0, _recipient, _userBalance0);
                TransferHelper.safeTransfer(_token0, INDEX_FUND, _index0);
            }
            if (_tokensOwed1 > 0) {
                TransferHelper.safeTransfer(_token1, _recipient, _userBalance1);
                TransferHelper.safeTransfer(_token1, INDEX_FUND, _index1);
            }   
        }
    }
        ...
        if (token0 == WETH || token1 == WETH) {
            (
                ...
                uint256 wethAmount
                    ...
                    ? (token1, _tokensOwed1, token0, _tokensOwed0)
                    : (token0, _tokensOwed0, token1, _tokensOwed1);

            address ethTokenPair = checkWethPairsAndLiquidity(tokenAlt);

            if (pilotToken && ethTokenPair != address(0)) {
            ...
            if (
                pilotToken && ethToken0Pair != address(0) && ethToken1Pair != address(0)
            ) {
                (indexAmount0, indexAmount1) = _distributeFeesInPilot(
                    recipient,
                    token0,
        ...
        )
    {
        ...
        tokensOwed0 = FullMath.mulDiv(
            position.feeGrowthGlobal0.sub(feeGrowth0),
            userLiquidity,
            1e18
        );
        tokensOwed1 = FullMath.mulDiv(
            position.feeGrowthGlobal1.sub(feeGrowth1),
            userLiquidity,
            1e18
        );
...
interface IUnipilot is IUnipilotEvents {
    ...
    function collectForETH(address recipient, uint256 tokenId) external payable;
...
contract Unipilot is ERC721Permit, IUnipilot {
    ...
    constructor(address liquidityManagerAddress)
    ...
    // vulnerable
    function checkUserFees(uint256 tokenId)
        external
        returns (uint256 fees0, uint256 fees1)
    {
        ...
        (uint256 tokensOwed0, uint256 tokensOwed1, , ) = liquidityManager
            .getTokensOwedAmount(
                position.pool,
                position.liquidity,
                position.feeGrowth0,
        ...
        returns (uint256 mintedTokenId)
    {
        require(
            ...
            "UNIPILOT::INSUFFICIENT_AMOUNTS"
        );
        address pool = liquidityManager.getPoolAddress(
            params.token0,
            params.token1,
            params.fee
        );

        (uint256 shares, uint256 amount0, uint256 amount1) = getSharesAndAmounts(
            pool,
            params.amount0Desired,
            params.amount1Desired
        );
        ...
        if (msg.value > 0)
            TransferHelper.safeTransferETH(address(liquidityManager), msg.value);
        liquidityManager.addLiquidityInManager(
            msg.sender,
            pool,
            amount0,
            amount1,
        ...
        if (tokenId > 0) {
            ...
            position.tokensOwed0 += FullMath.mulDiv(
                feeGrowthGlobal0 - position.feeGrowth0,
                position.liquidity,
                1e18
            );
            position.tokensOwed1 += FullMath.mulDiv(
                feeGrowthGlobal1 - position.feeGrowth1,
                position.liquidity,
                1e18
            );
    ...
    function withdraw(
        ...
        liquidityManager.removeLiquidityWithFees(
            true,
            position.pool,
            recipient,
            liquidity
    ...
    function withdrawForETH(
        ...
        (address _token0, address _token1, , , , , ) = liquidityManager.getPoolDetails(
            _positions[tokenId].pool
        );
        withdraw(false, recipient, tokenId, liquidity);
        address token = _token0 == WETH ? _token1 : _token0;
        IPeripheryPayments(address(liquidityManager)).unwrapWETH9(0, msg.sender);
        IPeripheryPayments(address(liquidityManager)).sweepToken(token, 0, msg.sender);
    }

    function collectForETH(address recipient, uint256 tokenId)
        external
        payable
        override
        isAuthorizedForToken(tokenId)
    {
        ...
        collect(false, recipient, tokenId);
        address token = _token0 == WETH ? _token1 : _token0;
        IPeripheryPayments(address(liquidityManager)).unwrapWETH9(0, msg.sender);
        IPeripheryPayments(address(liquidityManager)).sweepToken(token, 0, msg.sender);
    }
