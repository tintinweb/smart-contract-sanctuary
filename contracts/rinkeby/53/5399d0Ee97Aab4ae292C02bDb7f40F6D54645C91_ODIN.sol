...
contract ODIN is Context, IERC20, Ownable {
    ...
    function swapTokensForEthUser(uint256 tokenAmount, address payable winner, address payable pool) private lockTheSwap{
        ...
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        _approve(jackpot, address(this), tokenAmount);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee, uint256 _taxFee, uint256 _teamFee) private {
        ...
        if(sender != uniswapV2Pair && sender != address(uniswapV2Router) && !whitelist[sender]){
            Users[sender]._balanceDividend = Users[recipient]._balanceDividend.sub(amountT.mul(10**9).div(_totalSupply));
        }
    ...
    function GetTier(address addr) public view returns(bytes32){
        ...
        if(holdtime > (0 hours)){
            ...
            if(holdtime > (4 hours)){
                tier = 'VIKINGR';
                if(holdtime > (12 hours)){
                    tier = 'JARL';
                    if(holdtime > (24 hours)){
                        tier = 'KING';
                        if(holdtime > (45 hours)){
                            tier = 'GOD';
                        }
