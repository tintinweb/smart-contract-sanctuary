...
contract Farm is AccessControl, ReentrancyGuard {
    ...
    function initialize(
        ...
        require(rewardTokens[_rewardToken].initialized == false,"already initialized");
  ...
  function rewardedPoints(address staker) public view returns (uint256) {
        ...
        return st.points + ((st.amount * earnPerSec) * _seconds) / 1e18;
    }
    ...
    function withdraw(uint256 amount) public {
        ...
        uint256 len = tokenAddresses.length;
        for (uint256 i = 0; i < len; i++) {
             uint256 reward = rewardTokens[tokenAddresses[i]].rewards[msg.sender];
             if(reward > 0){getReward(tokenAddresses[i]);}
