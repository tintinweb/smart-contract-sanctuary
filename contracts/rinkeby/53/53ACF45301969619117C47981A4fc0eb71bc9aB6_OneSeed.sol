...
contract OneSeed is Context, IERC20, Ownable {
    ...
    function stakeReferred(uint256 amount, address ref) public {
    ...
    function lockStake(uint256 time) public OnlyActiveStaker {
    ...
    function extendStakeLock(uint256 time) public OnlyActiveStaker {
    ...
    function getStakingMember(address a) public view returns (uint256, uint256, bool, bool, uint256, uint256, bool){
        return (
    ...
    function rewardAllStaking(uint256 maxPerCall) public {
        // function should not be runnning if no one has joined staking yet
        ...
        // calculate the indexes based on maxPerCall for this function call
        // Option 1: When the number of possible rewards is higher than the actual participating staker, then the length will be taken
        // Option 2: When the number of possible rewards is lower than the actual participating staker it will start at the last eIndex
        // from the last function call, if there is an overflow, the function will be called again starting from index 0 to meet the maxPerCall
        // This means that all participants are paid out one after the other. // GAS - Fee - Loop problem
        ...
        for (uint256 i = sIndex; i < eIndex; i++) {
            // only pay stakers which stake more than minimum amount
            if (_stakeholders[_stakeMapping[i]].amount >= _minimumStakingAmount) {
                ...
                // in case the function is not called for more than 24 hours, it will multiply the reward depending on last reward
                // since it's an int division only dividers of 24 hours are counted
                // remaining time will be subtracted to the lastReward day to balance the time
                if (nDays >= 1) {

                    _stakeholders[_stakeMapping[i]].lastReward = now.sub(diff.mod(_rewardTime));

                    // pay staker regular interest if stake not locked
                    ...
                    if (_stakeholders[_stakeMapping[i]].locked) {
                        if (_stakeholders[_stakeMapping[i]].lockedUntil > now) {
                            ...
                            // unlock stake since lockedUntil is in the past
                    ...
                    // calc absolute reward in tokens
                    uint256 reward = _stakeholders[_stakeMapping[i]].amount.mul(interest).mul(nDays).div(_returnDivisor);

                    // when auto compound is active and staking amount is less than _maxStakingAmount and global max staking amount not yet reached
                    // then stake the reward otherwise add reward to balance although isAutoCompound is true
                    if (
                    ...
                    // get the team reward , if staker is not referred then the referral reward will be added to the team
                    uint256 tr = _teamReward;

                    // pay the referral if there is any
                    if (isReferred(_stakeholders[_stakeMapping[i]].holder)) {
                        ...
                        // push object to public and private list for history
                    ...
                    // add objects to public and private list for history
        ...
        // 1 call function again if there is overflow
        // 2 reset index if maxPerCall is higher than count people in staking
        // 3 keep on with the next stack if count people > maxPerCall and index also higher than eIndex
        if (indexOverflow > 0) {
