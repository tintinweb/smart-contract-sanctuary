...
contract YSLOptSinglePancake is YSLOpt {
    ...
    function transferOut(
        ...
        IStrategySwap(stratSwap).swapLPToBusd(fee, pancakeSwap, path, lpToken);
        ...
        IERC20(BUSD).approve(pancakeSwap, amountUSDAfter - amountUSDBefore);
        IERC20(_ysl).approve(pancakeSwap, emission);
        ...
        IStrategySwap(stratSwap).swapLPToBusd(_amount, pancakeSwap, path, lpToken);
    ...
    function transferIn(address _user, uint256 _amount) public stratOnly returns (uint256) {
        ...
        IStrategySwap(stratSwap).swapBusdToLP(_amount, pancakeSwap, path, BUSD);
    ...
    function collectControllerFee(uint256 _amount) internal override returns (uint256 summary) {
        ...
        _path[1] = WBNB;
        IERC20(BUSD).approve(pancakeSwap, amountUSDAfter - amountUSDBefore);
        IPancakeRouter02(pancakeSwap).swapExactTokensForETH(
            amountUSDAfter - amountUSDBefore,
            1,
            _path,
            feeAddress,
    ...
    function collectOptimisationTax(uint256 _amount) internal override returns (uint256 summary) {
        ...
        IStrategySwap(stratSwap).swapLPToBusd(tax, pancakeSwap, path, lpToken);
        uint256 amountUSDAfter = IERC20(BUSD).balanceOf(address(this));
    ...
    function compound() external {
        ...
        IPancakeMaster(pancakeMaster).deposit(pancakeId, lpToken.balanceOf(address(this)));
        IERC20(BANANA).approve(pancakeSwap, IERC20(CAKE).balanceOf(address(this)));
        ...
        IStrategySwap(stratSwap).swapBusdToLP(IERC20(BUSD).balanceOf(address(this)), pancakeSwap, path, BUSD);
    }
    ...
    function earn(
        ...
        uint256 usersCompUSD = IStrategySwap(stratSwap).swapLPToBusd(usersComp, pancakeSwap, path, lpToken);
    ...
    function getUserDepositedUSD(address _user) public view returns (uint256) {
        return IStrategySwap(stratSwap).getBusdAmount(userDeposited[_user], path, pancakeSwap);
    }

    function getTotalDepositedUSD() public view returns (uint256) {
        return IStrategySwap(stratSwap).getBusdAmount(totalDeposited, path, pancakeSwap);
    }
}
...
import "../YSLfarm/YSLOptSinglePancake.sol";

contract MockYSLOptSinglePancake is YSLOptSinglePancake {
    constructor(
        address _adapter,
        uint256 _pancakeId,
        address[] memory _path,
