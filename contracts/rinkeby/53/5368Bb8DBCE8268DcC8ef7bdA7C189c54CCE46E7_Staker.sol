...
contract Staker {
  ...
  modifier notCompleted() {
    ...
    require(!completed, 'NOT COMPLETED YET');
    _;
  }
  ...
  function execute() external notCompleted {
    require(block.timestamp <= deadline, 'DEADLINE HAS BEEN MET');
    require(address(this).balance >= threshold, 'REQUIREMENTS NOT MET');

    (bool success, ) = address(exampleExternalContract).call{value: address(this).balance}(abi.encodeWithSignature('complete()'));
    require(success, 'call to exampleExternalContract.complete failed');
  }

  // if the `threshold` was not met, allow everyone to call a `withdraw()` function
  function withdraw(address _address) external payable notCompleted {
