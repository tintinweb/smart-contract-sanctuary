...
contract  SOFTIK  is ERC721Enumerable, Ownable
{
    ...
    constructor(string memory baseURI) ERC721("SOFTIK", "SFT")  {
        setBaseURI(baseURI);
    }
    ...
    function getPrice(uint256 _quantity) public view returns (uint256) {
     ...
     function addWhiteList(address whiteAddress)public onlyOwner {
        ...
        function preSaleMint(uint chosenAmount) public payable{
        
        require(totalSupply()+chosenAmount<=1650-reserve,"Quantity must be lesser then MaxSupply");
