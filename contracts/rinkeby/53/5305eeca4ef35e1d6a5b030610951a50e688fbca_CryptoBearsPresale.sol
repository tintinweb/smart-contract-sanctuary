...
// File contracts/CryptoBearsPresale.sol
...
contract CryptoBearsPresale is ERC721Enumerable, Ownable {
  using Strings for uint256;
  using EnumerableSet for EnumerableSet.AddressSet;
  ...
  function isWhitelisted(address _user) public view returns (bool) {
      ...
      if (_whitelistedAddresses.contains(_user)) {
          return true;
      }
  ...
  function whitelistUsers(address[] calldata _users) public onlyOwner {
      for (uint256 i; i < _users.length; i++) {
        if (!_whitelistedAddresses.contains(_users[i])) {
            _whitelistedAddresses.add(_users[i]);
        }
      }   
  }
