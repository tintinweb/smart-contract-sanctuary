...
contract IntelliLinker is AccessControl {
	 ...
	 *
	 * @dev Feature FEATURE_LINKING_ENABLED must be enabled
	 *      as a prerequisite for `link()` function to succeed
	 */
	uint32 public constant FEATURE_LINKING_ENABLED = 0x0000_0001;

	/**
	 * @notice Enables iNFT unlinking (destruction)
	 *
	 * @dev Feature FEATURE_UNLINKING_ENABLED must be enabled
	 *      for the `unlink()` and `unlinkNFT()` functions to succeed
	 */
	uint32 public constant FEATURE_UNLINKING_ENABLED = 0x0000_0002;
	...
	function link(uint96 personalityId, address targetContract, uint256 targetId) public {
		...
		require(isFeatureEnabled(FEATURE_LINKING_ENABLED), "linking is disabled");
	...
	function unlink(uint256 iNftId) public {
		...
		require(isFeatureEnabled(FEATURE_UNLINKING_ENABLED), "unlinking is disabled");
