...
contract CORXLPFarmR {
    ...
    uint256 public constant k = 47520; // 10% (base APY unit) APY == x0.000,274 per day = x0.000,000,047,520 block in Rinkeby (at 5766 blocks/day)
    ...
    function getPending(uint256 _pid, address _user) public view returns (uint256) {
        return getPending(_pid, _user, uint32(block.number));
    }
    ...
    function stake(uint256 _pid, uint256 _amount) public {
        ...
        Program storage _p = programs[_pid];
        require(_p.start <= block.number && block.number < _p.finish);
    ...
    function infoBundle(address _user) public view returns (Program[] memory pp, Stake[] memory ss) {
        pp = programs;
        ss = new Stake[](pp.length);
        for (uint256 i = 0; i < pp.length; i++) ss[i] = stakeOf[i][_user];
    }
