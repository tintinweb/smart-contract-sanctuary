...
contract DappRegistry is IAuthoriser {
    ...
    returns (bool isEnabled)
  {
    ...
    return
      (((registries >> _registryId) & 1) > 0) == (_registryId > 0); /* "is bit set for regId?" */ /* "not Argent registry?" */
  }
  ...
  function toggleRegistry(uint8 _registryId, bool _enabled) external {
    ...
    bool current = (((registries >> _registryId) & 1) > 0) == (_registryId > 0); /* "is bit set for regId?" */ /* "not Argent registry?" */
    if (current != _enabled) {
      enabledRegistryIds[msg.sender] = bytes32(
        registries ^ (uint256(1) << _registryId)
      ); // toggle [_registryId]^th bit
   ...
   */
  function changeOwner(uint8 _registryId, address _newRegistryOwner)
    external
    onlyOwner(_registryId)
  {
   ...
   */
  function requestTimelockChange(uint64 _newTimelockPeriod)
    external
    onlyOwner(0)
  {
  ...
  function confirmTimelockChange() external {
    ...
    require(
      timelockPeriodChangeAfter > 0 &&
        timelockPeriodChangeAfter <= block.timestamp,
      "DR: can't (yet) change timelock"
    );
   ...
   */
  function getAuthorisation(uint8 _registryId, address _dapp)
    external
    view
    returns (address filter, uint64 validAfter)
  {
    uint256 auth = uint256(authorisations[_registryId][_dapp]);
   ...
   */
  function removeDapp(uint8 _registryId, address _dapp)
    external
    onlyOwner(_registryId)
  {
    require(
  ...
  function requestFilterUpdate(
    ...
    pendingFilterUpdates[_registryId][_dapp] = bytes32(
      (uint256(uint160(_filter)) << 64) | validAfter
    );
