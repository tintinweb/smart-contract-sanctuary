...
abstract contract RefferalContract{
    function getRefferer(address _user) external virtual view returns(address);
    function isRefferBonusPaid(address _user) view external virtual returns(bool);
    function updateUserDetails(address _user,uint amount) external virtual;
    function getRefferals(address _user) external virtual view returns(address[] memory);
}
...
contract Trade {
    ...
    uint8 private refferalFeePermile;
    TransferProxy public transferProxy;
    RefferalContract public refferalContract;
    ...
    constructor (uint8 _buyerFee, uint8 _sellerFee,uint8 _referralFee, TransferProxy _transferProxy,RefferalContract _refferalContract) {
        buyerFeePermille = _buyerFee;
        sellerFeePermille = _sellerFee;
        refferalFeePermile = _referralFee;
        transferProxy = _transferProxy;
        refferalContract = _refferalContract;
        owner = msg.sender;
    }
    ...
    function getFees(uint paymentAmt, BuyingAssetType buyingAssetType, address buyingAssetAddress, uint tokenId) internal view returns(Fee memory){
        ...
        address referrer = refferalContract.getRefferer(msg.sender);
        refferalFee = !(refferalContract.isRefferBonusPaid(msg.sender)) ? price.mul(refferalFeePermile).div(1000)
                                                       : 0;
        assetFee = price.sub(royaltyFee).sub(sellerFee).sub(refferalFee);
        return Fee(platformFee, assetFee, royaltyFee,refferalFee, price, tokenCreator,referrer);
    }
    ...
    function tradeAsset(Order memory order, Fee memory fee) internal virtual {
        ...
        if(fee.referralFee > 0){
            transferProxy.erc20safeTransferFrom(IERC20(order.erc20Address), order.seller, fee.refferer, fee.referralFee);
            refferalContract.updateUserDetails(order.seller, fee.referralFee);
        }
