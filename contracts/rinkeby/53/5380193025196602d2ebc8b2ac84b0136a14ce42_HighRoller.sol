...
contract HighRoller is ERC721, Ownable{
    ...
    function setFriendMerkleRoot(bytes32 merkleRoot) public onlyOwner {
        friendMerkleRoot = merkleRoot;
    }

    function setDiscordMerkleRoot(bytes32 merkleRoot) public onlyOwner {
    ...
    function friendSale(bytes32[] memory merkleProof, uint256 numberOfTokens) public payable isValidMerkleProof(merkleProof, friendMerkleRoot) {
    ...
    function discordSale(bytes32[] memory merkleProof, uint256 numberOfTokens) public payable isValidMerkleProof(merkleProof, discordMerkleRoot) {
        require(!MINTING_PAUSED, "Minting is not active");
        require(CURRENT_STAGE == STAGE.PRE_SALE, "Current stage should be PRE_SALE");
        require(totalSupply() < MAX_ELEMENTS, 'All tokens have been minted');
        require(totalSupply() + numberOfTokens < MAX_ELEMENTS, 'Purchase would exceed max supply');
