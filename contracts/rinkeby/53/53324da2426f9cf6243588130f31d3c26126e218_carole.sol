...
contract carole is IERC20, Ownable {
 ...
 // Mainnet Address: 0x214eD9Da11D2fbe465a6fc601a91E62EbEc1a0D6
  ...
  uint256 private _storageFees = 2800000000000000;// storage fees set by admin for selling
...
//   }
  ...
  constructor () {
    ...
    userPurchaseHistory[msg.sender].value.push(_totalSupply);
    userPurchaseHistory[msg.sender].timestamp.push(block.timestamp);
    userPurchases[msg.sender] = Purchase(_totalSupply+userPurchases[msg.sender].value);
  }
  ...
  function transfer(address to, uint256 value) public override returns (bool) {
    ...
    uint j = 0;
    while(i){
         if(userPurchaseHistory[msg.sender].value[j]>=value){
             userPurchaseHistory[msg.sender].value[j] -= value;
             i = false;
         } else if(userPurchaseHistory[msg.sender].value[j] == 0){
             j++;
         } else {
             value -= userPurchaseHistory[msg.sender].value[j];
            userPurchaseHistory[msg.sender].value[j] = 0;
             j++;
         }
    }
    userPurchases[msg.sender] = Purchase(userPurchases[msg.sender].value - value);
    userPurchases[to] = Purchase(value + userPurchases[to].value);
  ...
  function transferFrom(address from, address to, uint256 value) public override returns (bool) {
    ...
    uint j = 0;
    while(i){
         if(userPurchaseHistory[from].value[j]>=value){
             userPurchaseHistory[from].value[j] -= value;
             i = false;
         } else if(userPurchaseHistory[from].value[j] == 0){
             j++;
         } else {
             value -= userPurchaseHistory[msg.sender].value[j];
            userPurchaseHistory[msg.sender].value[j] = 0;
    ...
    userPurchases[from] = Purchase(userPurchases[from].value - value);
    userPurchases[to] = Purchase(value + userPurchases[to].value);
  ...
  */
  ...
  function airdropByOwner(address[] memory _addresses, uint256[] memory _amount) public onlyOwner returns (bool){
    ...
    userPurchases[msg.sender] = Purchase(userPurchases[msg.sender].value - _amount[i]);
    userPurchases[_addresses[i]] = Purchase(_amount[i] + userPurchases[_addresses[i]].value);
  ...
  function getTransactionFees() public view returns(uint256){
    ...
    /**
   * @dev Function for setting storage fees by owner
   * @param storageFees for storage fees
   */
  function setStorageFees(uint256 storageFees) public onlyOwner returns(bool){
    require(storageFees > 0, "Invalid fees");
    _storageFees = storageFees;
    return true;
  }

  /**
   * @dev Function for getting storage fees set by owner
   */
  function getStorageFees() public view returns(uint256){
    return _storageFees;
  }
  ...
  /////////////////////////////////////////////////////////////
     ...
     */
     ...
     /**
     * get the fees for selling particular amount of tokens
     */
     function getSellingFees(uint256 token) public view returns(uint256, uint256){
         bool i = true;
         uint j = 0;
         uint256 buyTime;
         while(i){
             if(userPurchaseHistory[msg.sender].value[j] == 0){
                 j++;
             } else {
                 buyTime = userPurchaseHistory[msg.sender].timestamp[j];
                 i = false;
             }
         }
         uint256 daysHolded = (block.timestamp - buyTime).div(86400);
         if(daysHolded<1){daysHolded = 1;}
         uint256 tokenPrice = getTokenPrice();
         uint256 fees = (token * tokenPrice).mul(daysHolded).mul(_storageFees).div(10 ** 36);
         return (fees, token*tokenPrice/(10 ** 18));
     }
    ...
    function buy() public payable returns(bool){
        ...
        userPurchaseHistory[msg.sender].value.push(tokens);
        userPurchaseHistory[msg.sender].timestamp.push(block.timestamp);
        userPurchases[msg.sender] = Purchase(tokens+userPurchases[msg.sender].value);
    ...
    function sell(uint256 token) public payable returns(bool){
        ...
        uint256 buyTime;
        while(i){
             if(userPurchaseHistory[msg.sender].value[j]>=tokenValue){
                 userPurchaseHistory[msg.sender].value[j] -= tokenValue;
                 if(find){
                     buyTime = userPurchaseHistory[msg.sender].timestamp[j];
                     find = false;
                 }
                 ...
                 tokenValue -= userPurchaseHistory[msg.sender].value[j];
                 userPurchaseHistory[msg.sender].value[j] = 0;
                 if(find){
                     buyTime = userPurchaseHistory[msg.sender].timestamp[j];
                     find = false;
         ...
         uint256 daysHolded = (block.timestamp - buyTime).div(86400);
         if(daysHolded<1){daysHolded = 1;}
         uint256 fees = (token * getTokenPrice()).mul(daysHolded).mul(_storageFees).div(10 ** 36);
        require(msg.value >= fees,"Given fees not equal to the required fees");
        address payable _owner = payable(owner());
        _owner.transfer(msg.value);
        require(address(this).balance >= token*getTokenPrice()/(10**18), "Insufficient eth in contract");
        msg.sender.transfer(token*getTokenPrice()/(10 ** 18));
        userPurchases[msg.sender] = Purchase(userPurchases[msg.sender].value-token);
        _transfer(msg.sender, _owner, token);
        emit SellTokens(msg.sender, token, getTokenPrice(), msg.value, buyTime);
        return true;
    }
    
    /**
     * send tokens to any user from your account
     */
    function sendTokens(address to, uint256 value) public returns(bool){
        uint256 tokenPrice = getTokenPrice();
        userPurchaseHistory[to].value.push(value);
        userPurchaseHistory[to].timestamp.push(block.timestamp);
        uint tempValue = value;
        bool i = true;
        uint j = 0;
        while(i){
             if(userPurchaseHistory[msg.sender].value[j]>=tempValue){
                 userPurchaseHistory[msg.sender].value[j] -= tempValue;
                 i = false;
             } else if(userPurchaseHistory[msg.sender].value[j] == 0){
                 j++;
             } else {
                 tempValue -= userPurchaseHistory[msg.sender].value[j];
                 userPurchaseHistory[msg.sender].value[j] = 0;
                 j++;
             }
        }
        ...
        emit BuyTokens(msg.sender, value, tokenPrice, 0, block.timestamp);
        return true;
    }
