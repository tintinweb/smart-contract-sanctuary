...
contract ADC_Dev is ERC721URIStorage, Ownable {
...
/*****  *****/
	...
	mapping (string => uint256) public tokenMap; mapping (uint256 => string) public phraseMap;
	modifier openSale() {require(saleOpen,'Sale is closed, baby!');_;}

/*****  *****/
	constructor() ERC721("ADC Test","ADC1722") {tokenMap["tom"]=11237; tokenMap["mrhouse"]=10013; 
	tokenMap["domainking"]=10301; tokenMap["phattest"]=10458; tokenMap["help"]=11379;} 

/*****  *****/ 
	function mint_normal(string memory phrase) public payable openSale returns (uint256) {
		...
		require(_tokenIds.current() < tokenSupply, 'Sorry folks: sold out!');
...
/*****  *****/
	function mint_special(string memory phrase) public onlyOwner returns (uint256) {
		require(_validatePhrase(phrase), 'Validity.');require(bytes(phrase).length < 11, 'Length.');
		require(tokenMap[phrase] == 0, 'Taken.'); require(_tokenIds_special.current() < 10, 'All claimed!');
		uint256 tokenID = tokenSupply - _tokenIds_special.current();
		_tokenIds_special.increment(); _safeMint(msg.sender, tokenID);
...
/***** 'onlyOwner' methods for administration *****/
	...
	function get_phrase(uint256 tokenID) public view onlyOwner returns (string memory) {
		require(tokenID <= _tokenIds.current(), "Error: tokenID wasn't minted."); return phraseMap[tokenID];}
