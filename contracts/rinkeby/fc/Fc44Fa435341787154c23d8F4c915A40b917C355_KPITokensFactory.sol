...
import "@xcute/contracts/interfaces/IMaster.sol";
...
contract KPITokensFactory is Ownable, IKPITokensFactory {
    ...
    function createToken(
        ...
        KpiTokenCreationOracle[] calldata _oracles,
        ...
        for (uint256 _i = 0; _i < _collaterals.length; _i++) {
            ...
            uint256 _fee = (_collateral.amount * fee) / BASE;
            IERC20(_collateral.token).safeTransferFrom(
                msg.sender,
                address(this),
                _collateral.amount + _fee
            );
            IERC20(_collateral.token).safeTransfer(feeReceiver, _fee);
        ...
        uint256 _jobsFunding = 0;
        OracleCreationData[] memory _oraclesData = new OracleCreationData[](
            _oracles.length
        );
        Oracle[] memory _kpiTokenOracles = new Oracle[](_oracles.length);
        for (uint256 _i = 0; _i < _oracles.length; _i++) {
            KpiTokenCreationOracle memory _oracle = _oracles[_i];
            if (_oracle.higherBound <= _oracle.lowerBound)
                revert InvalidOracleBounds();
            if (_oracle.weight == 0) revert InvalidOracleWeights();
            _jobsFunding += _oracle.jobFunding;
            _oraclesData[_i] = OracleCreationData({
                template: _oracle.template,
                initializationData: _oracle.initializationData,
                jobFunding: _oracle.jobFunding
            });
            _kpiTokenOracles[_i] = Oracle({
                addrezz: address(0), // populated later
        ...
        if (_jobsFunding > 0)
            IERC20(workersToken).safeTransferFrom(
                msg.sender,
                oraclesManager,
                _jobsFunding
            );

        address[] memory _oracleAddresses = IOraclesManager(oraclesManager)
            .instantiate(_instance, _oraclesData);
        for (uint256 _i = 0; _i < _oracleAddresses.length; _i++)
            _kpiTokenOracles[_i].addrezz = _oracleAddresses[_i];
