...
contract ClashOfDragonzTF2 is ERC721Enumerable, ERC721Burnable, ERC721Pausable, Ownable {
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    uint public constant WHITELIST_PRICE = 0.05 ether;
    uint public constant PUBLIC_PRICE = 0.07 ether;
    uint public constant PUBLIC_PRICE_OF_3 = 0.18 ether;
    uint public constant PUBLIC_PRICE_OF_5 = 0.25 ether;
    uint public constant PRESALES_PRICE = 0.06 ether;
    uint public NUMBER_OF_RESERVED_DRAGONZ;
    ...
    bytes32 public merkleRoot = 0x04c499693aa22f89c2453fbd3c3cac1629f6676fd92fb582c84f39ce6cdd9910;
    mapping(address => bool) whitelistedAddresses;

    // One URI to rule them all
    string public baseTokenURI;

    constructor(string memory baseURI) ERC721("ClashOfDragonzTF2", "COFDTF2")
    {
        setBaseURI(baseURI);
    }
    ...
    // Overrides because we import ERC721 multiple times via derivates
    ...
    // Function to freeze the contract, when it's frozen, it's cold and you can't heat it up anymore
    modifier unfrozen() {
    ...
    modifier mintRequirementsWhitelist(uint256 count) {
        require((currentIndex + count) <= MAX_SUPPLY, "Not enough Dragonz left!");
        require(msg.value >= WHITELIST_PRICE.mul(count), "Not enough ether to purchase dragons when whitelisted");
        _;
    }
    ...
    modifier mintRequirementsSale(uint256 count) {
        ...
        if (count == 1) {
            ...
            require(msg.value >= price.mul(1), "Not enough ether to purchase the Dragonz.");
        } else if (count == 2) {
            // Public sale - 2 NFT - 0.14
            require(msg.value >= price.mul(2), "Not enough ether to purchase the Dragonz.");
        } else if (count == 3) {
            // You pay 0.18
            require(msg.value >= PUBLIC_PRICE_OF_3.mul(1), "Not enough ether to purchase the Dragonz.");
        } else if (count == 4) {
            ...
            require(msg.value >= (price.mul(1) + PUBLIC_PRICE_OF_3.mul(1)), "Not enough ether to purchase the Dragonz.");
        } else if (count == 5) {
            // You pay 0.25; the discounted price for 5
            require(msg.value >= PUBLIC_PRICE_OF_5.mul(1), "Not enough ether to purchase the Dragonz.");
        } else if (count > 5) {
            // They if you want discounts, buy them in batches of 5 otherwise you pay full price :)
            if (count % 5 == 0) {
                // Good you want dem discounts
                require(msg.value >= PUBLIC_PRICE_OF_5.mul(count), "Not enough ether to purchase the Dragonz.");
            } else {
                // You're weird but ok here we go
                require(msg.value >= price.mul(count), "Not enough ether to purchase the Dragonz.");
            }
        }
     ...
     */
    function reserveNFTs(uint256 numberToReserve) public onlyOwner unfrozen {
        uint totalMinted = currentIndex;
        require(totalMinted.add(numberToReserve) < MAX_SUPPLY, "There are not enough NFTs remaining to reserve");
        _internalMintMultiple(numberToReserve);
     ...
     */
    function revealNFTs(string memory _metadataURI) public onlyOwner unfrozen {
     ...
     */
    function togglePublicSaleActive() public unfrozen {
     ...
     */
    function togglePrivateSaleActive() public unfrozen {
     ...
     */
    function setBaseURI(string memory _baseTokenURI) public unfrozen onlyOwner {
     ...
     */
    function freezeAll() public onlyOwner unfrozen {
        _frozen = true;
    }
     ...
     */
    function theBurn(uint[] memory tokenIds) public onlyOwner unfrozen {
    ...
    {
        _internalMintMultiple(count);
    }

    /**
     * @notice Release the kra- dragon eggs :)
     */
    function mintDragons(uint256 count) public payable unfrozen mintRequirementsSale(count) {
        _internalMintMultiple(count);
    }

    function _internalMintMultiple(uint256 count) private {
     ...
     */
    function withdraw(uint256 howMuch) public payable onlyOwner {
        uint balance = address(this).balance;
        require(balance > 0, "No ether left to withdraw");
        require(howMuch > balance, "Not enough left to withdraw");
        (bool success, ) = (msg.sender).call{value: howMuch}("");
