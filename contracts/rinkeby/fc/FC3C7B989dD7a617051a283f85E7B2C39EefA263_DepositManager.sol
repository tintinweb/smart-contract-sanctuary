...
contract DepositManager is BaseGovernanceWithUserUpgradable {
    ...
    function initialize(ILock _lock) initializer public {
        __DepositManager_init(_lock);
    }

    function __DepositManager_init(ILock _lock) initializer internal {
        __BaseGovernanceWithUser_init();
        __DepositManager_init_unchained(_lock);
    }

    function __DepositManager_init_unchained(ILock _lock) initializer internal {
        lockContract = _lock;
     ...
     */
    function removeDeposits(bytes calldata data) external onlyLockContract returns(uint) {
        uint[] memory IDs = _decodeDataToRemove(data);
        uint refundAmount = _getData(IDs);
     ...
     */
    function split(uint originId, uint[] memory splitParts, address[] memory addresses) external onlyLockContract {
    ...
    function _getData(uint[] memory _IDs) internal view returns(uint) {
        uint _refundAmount;
        for(uint i; i < _IDs.length; i++) {
            (, , uint initial, uint claimed) = getProperties(_IDs[i]);
           _refundAmount += (initial - claimed);
        }
...
interface ILock is IERC721Upgradeable{
    ...
    function canAddOrRemove() external view returns (bool);
