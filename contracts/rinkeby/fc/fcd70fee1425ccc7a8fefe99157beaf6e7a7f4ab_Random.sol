...
contract Random is Ownable{
    ...
    function getSomeRandomNumber(uint _seed, uint _limit) public view returns (uint256) {
        uint extra = 0;
    ...
    function getRandomToken(uint _seed, address _address) external view returns (uint256) {
        uint256 totalSupply = ERC721(_address).totalSupply();
        uint256 index = getSomeRandomNumber(_seed, totalSupply);
        uint256 tokenId = ERC721(_address).tokenByIndex(index);
        return tokenId;
    }
