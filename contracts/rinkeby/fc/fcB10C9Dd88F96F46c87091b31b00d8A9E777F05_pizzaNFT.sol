...
interface IERC165 {
    ...
    function bakePizzaAndMint(string memory metadata, uint256 base, uint256 sauce, uint256 cheese, uint256[] memory meats, uint256[] memory toppings ) public {
        ...
        for(uint256 x = 0; x < meats.length; x++) {
            if(meats[x] > 0) {
                ...
                increaseIngredientCount(meats[x]);
            }
        }
        for(uint256 x = 0; x < toppings.length; x++) {
            if(toppings[x] > 0) {
                ...
                increaseIngredientCount(toppings[x]);
            }
        }
        totalPizzas+=1;
    ...
    function increaseIngredientCount(uint256 ingredientId) internal {
        uint256 ingCountUsed = 0;
        ingCountUsed = ingredientUsedCount[ingredientId]+1;
