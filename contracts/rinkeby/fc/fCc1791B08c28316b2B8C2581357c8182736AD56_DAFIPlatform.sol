...
contract DAFIPlatform is Ownable{
    ...
    function getdToken(string memory _type, uint256 _balance) public payable {
        require (msg.value == 1500000000000000,"Fee amount is not valid");
    ...
    function getDAFIToken(address _beneficiary) public view returns(uint256 totalAssetVal, uint256 USDValOfdLINK, uint256 USDValOfdBTC, uint256 USDValOfdETH, uint256 USDValOfdSNX, uint256 DAFIToken){
        (USDValOfdLINK, USDValOfdBTC, USDValOfdETH, USDValOfdSNX, totalAssetVal) = DAFIContract.getDAFIToken(_beneficiary);
