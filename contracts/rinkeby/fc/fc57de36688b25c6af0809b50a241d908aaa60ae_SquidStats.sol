...
contract SquidStats is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    string[] private occupation = [
        ...
        "chauffeur",
        "gambling addict",
        "derivatives investor",
        "secret billionaire",
        "foreign blue collar worker ",
        "single Mother",
        "police Officer",
        "doctor",
        "glass-maker",
    ...
    function getDexteriy(uint256 tokenId) public view returns (string memory) {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: #f8057f; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="black" /><text x="10" y="20" class="base"> Gender: ';
        
        parts[1] = getGender(tokenId);

        parts[2] = '</text><text x="10" y="40" class="base">Strength: ';

        parts[3] = getStrength(tokenId);

        parts[4] = '</text><text x="10" y="60" class="base">Dexterity: ';

        parts[5] = getDexteriy(tokenId);

        parts[6] = '</text><text x="10" y="80" class="base">Luck: ';

        parts[7] = getLuck(tokenId);

        parts[8] = '</text><text x="10" y="100" class="base">Vitality: ';

        parts[9] = getVitality(tokenId);

        parts[10] = '</text><text x="10" y="120" class="base">Occupation: ';
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Character #', toString(tokenId), '", "description": "Squidgame : Stats is randomized player stats and characteristics based on the Netflix Series that is generated and stored on chain. Functionality is obvious for the Squidverse, but its direct usage is intentionally omitted for interpretation. Feel free to apply Squidgame: Stats to any part of the Squidverse.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
        output = string(abi.encodePacked('data:application/json;base64,', json));

        return output;
    }

    function mintSquid(uint256 numberOfTokens) external payable {
    ...
    constructor() ERC721("SquidStats", "SQUIDSTATS") Ownable() {}
}
