...
contract Fade is ERC721, Ownable {
    ...
    constructor() ERC721("Fade", "FADE") {
        ...
        _imageParts.push("<linearGradient id='g' x1='0%' x2='0%' y1='1%' y2='100%'>");
        _imageParts.push("<stop offset='0%' stop-color='hsl(");
        _imageParts.push(_COLOR1_TAG);
        _imageParts.push(",");
        ...
        _imageParts.push(_COLOR2_TAG);
        _imageParts.push(",");
        _imageParts.push(_SATURATION2_TAG);
        _imageParts.push("%,");
        _imageParts.push(_LIGHTNESS2_TAG);
        _imageParts.push("%)' />");
        _imageParts.push("<stop offset='100%' stop-color='hsl(0,0%,15%)' />");
    ...
    function _generateImage(uint256 milliDistance, int128 completion, int128 c1curve, int128 c2curve, int128 randHue, int128 randOffset) private view returns (string memory radius) {
        ...
        for (uint i = 0; i < _imageParts.length; i++) {
            ...
            } else if (_checkTag(_imageParts[i], _COLOR1_TAG)) {
                byteString = abi.encodePacked(byteString, _color1String(completion, randHue));
            ...
            } else if (_checkTag(_imageParts[i], _COLOR2_TAG)) {
                byteString = abi.encodePacked(byteString, _color2String(completion, randHue, randOffset));
    ...
    function _color1String(int128 completion, int128 randHue) private pure returns (string memory) {
    ...
    function _color2String(int128 completion, int128 randHue, int128 randOffset) private pure returns (string memory) {
