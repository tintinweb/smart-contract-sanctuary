...
contract ValidatorCloneFactory {
    ...
    constructor() {
        ...
        ID_VALIDATOR_MASTER_COPY = address(
            new IdValidator(new AlbumERC721Swapper.ERC721Grouping[](0))
        );
    }
    ...
    function deployIdValidatorClone(
        AlbumERC721Swapper.ERC721Grouping[] memory erc721Groupings
...
contract AlbumERC721Swapper is IERC721Receiver, ReentrancyGuard {
    event OrderCreated(
        address validator,
...
contract IdValidator is IERC721Validator, Ownable {
    event IdsSet(AlbumERC721Swapper.ERC721Grouping[] tokenGroupings, bool set);

    mapping(address => mapping(uint256 => bool)) public acceptedTokens;

    constructor(AlbumERC721Swapper.ERC721Grouping[] memory tokenGroupings) {
    ...
    function initialize(
        address _owner,
        AlbumERC721Swapper.ERC721Grouping[] memory tokenGroupings
    ) public {
        require(owner() == address(0));
        _transferOwnership(_owner);
        _setIds(tokenGroupings, true);
    ...
    function _setIds(
        ...
        for (uint256 i = 0; i < tokenGroupings.length; i++) {
            AlbumERC721Swapper.ERC721Grouping memory grouping = tokenGroupings[
                i
            ];
