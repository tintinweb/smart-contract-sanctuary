...
contract NotOpenSea is ERC721Enumerable, AccessControl {
    ...
    function mint(uint256 n) public payable {
        ...
        require(n < MINT_LIMIT, "Only 5 mints per transaction allowed!");
    ...
    function giveAwayMint(uint256 n, address mintAddress)  public onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 ts = totalSupply();
        require(ts + n < SUPPLY_LIMIT, "Reached max limit. No more minting possible!");
        for (uint256 i = 0; i < n; i++) {
            uint256 tokenId = _tokenIdCounter.current();
    ...
    function withdraw() public onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        payable(saviour).transfer(balance);
    }
