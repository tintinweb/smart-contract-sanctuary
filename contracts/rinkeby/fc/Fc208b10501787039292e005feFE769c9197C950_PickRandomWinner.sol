...
contract PickRandomWinner is VRFConsumerBase, Ownable {
    ...
    function requestWinnerSigningHash(uint256 blockExpiry, uint256 documentId, string memory urlHash, uint256 totalSigners) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(blockExpiry, documentId, urlHash, totalSigners));
    }

    // Set this to internal once done testing
    function verify(bytes memory signature, uint256 blockExpiry, uint256 documentId, string memory urlHash, uint256 totalSigners) public returns (bool) {
        require(block.number < blockExpiry, "Signature has expired!");

        bytes32 signingHash = requestWinnerSigningHash(blockExpiry, documentId, urlHash, totalSigners).toEthSignedMessageHash();
    ...
    function requestWinner(bytes memory signature, uint256 blockExpiry, uint256 documentId, string memory urlHash, uint256 totalSigners) public {
        require(verify(signature, blockExpiry, documentId, urlHash, totalSigners) == true, "Verification for signer failed");
