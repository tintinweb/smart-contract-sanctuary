...
contract GiraffeTower is ERC721PresetMinterPauserAutoId {
    ...
    uint public stotalSupply = 100;
    uint public maxItemsPerPreSale = 5; // Mutable by owner
    uint public preSaleMaxItems = 300;
    uint public mintPrice = 0.05 ether;
    uint public maxItemsPerTx = 5; // Mutable by owner
    uint public adoptionsCount = 800;
    uint public OgsCount = 100;
    ...
    constructor()
        ERC721PresetMinterPauserAutoId(
            "Giraffe Tower",
            "GT",
            "https://ipfs.io/ipfs/QmRt2wr65DnVt8KMnYDYEcybhq7kcdX4S37MfLYpTSTvPY/"
        )
    {
    ...
    function _ownerRoyalty() public view returns (uint) {
        return ownerRoyalty;
    }

    receive() external payable {
        uint tt = msg.value/100;
        totalDividends += tt;
        uint ot = msg.value - tt;
        ownerRoyalty += ot;
    }

    function withdrawReward(uint256 tokenId) external{
        require(ownerOf(tokenId) == msg.sender && tokenId <= 100 , "WR:Invalid");
         uint total =  (totalDividends - tokenRound[tokenId]) / OgsCount;
         ...
         uint totalClaim;
          for(uint256 i; i < _tokensOwned.length; i++){
            if(_tokensOwned[i] <= 100){
                totalClaim += (totalDividends - tokenRound[_tokensOwned[i]]) / OgsCount;
    ...
    function withdrawRoyalty() external ownerOnly{
     ...
     function preSale() external payable {
        ...
        require(preSaleCount + amount <= preSaleMaxItems, "preSale: Surpasses cap");
        require(amount <= preMints[msg.sender], "preSale: Amount greater than allocation");
        preSaleCount += amount;
    ...
    function adoptionMint() external {
        ...
        require(OgsCount + adoptionsCount + preSaleCount + amount <= maxToken, "mintWithoutValidation: Sold out");
        ...
        require(OgsCount + adoptionsCount + preSaleCount + amount <= maxToken, "publicMint: Surpasses cap");
        _mintWithoutValidation(msg.sender, amount);
    }
        ...
        returns (string memory)
    {
        ...
        return
            string(abi.encodePacked(gateway,base, Strings.toString(tokenId), ".json"));
    }
    ...
    function setMaxItemsPerTx(uint _maxItemsPerTx) external ownerOnly {
        maxItemsPerTx = _maxItemsPerTx;
    }

    function setMaxItemsPerPreSale(uint _maxItemsPerPreSale) external ownerOnly {
    ...
    function setAdoptions(uint256[] memory adoptions) external ownerOnly {
        for(uint i = 0; i < adoptions.length; i++) {
    ...
    function ogbulkMint(uint256[] memory pick) external ownerOnly {
   ...
   function getOgs() public view returns(address[] memory){
       address[] memory ogs = new address[](100);
   ...
   function getGenesisAddress() public view returns(address[] memory){
       address[] memory ga = new address[](10000);
   ...
   function walletOfOwner(address _owner) public view returns(uint256[] memory) {
    ...
    function rewardBalance(uint256 tokenId)  public view returns(uint){
    //    require(ownerOf(tokenId) == msg.sender && tokenId < 100 , "WR:Invalid");
        uint total =  (totalDividends - tokenRound[tokenId]) / OgsCount;
      return total;
    }

    function adoptionsOfOwner(address _owner) public view returns(uint256[] memory) {
        uint256 tokenCount = balanceOf(_owner);
