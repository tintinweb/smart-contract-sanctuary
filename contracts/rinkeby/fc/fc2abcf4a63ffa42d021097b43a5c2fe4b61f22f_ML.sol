...
interface IERC20 {
     ...
     * allowance.
     *
     * Returns a boolean value indicating whLINKer the operation succeeded.
     *
     * Emits a {Transfer} event.
     */
...
abstract contract Context {
    ...
    function _msgData() internal view virtual returns (bytes memory) {
        this; // silence state mutability warning without generating bytecode - see https://github.com/LINKereum/solidity/issues/2691
        return msg.data;
    }
}
...
library Address {
     ...
     *
     * https://eips.LINKereum.org/EIPS/eip-1884[EIP1884] increases the gas cost
     ...
     * Requirements:
     *
     * - the calling contract must have an LINK balance of at least `value`.
...
interface IsushiSwapRouter01 {
    ...
    function addLiquidityETH(
        ...
        address to,
        uint deadline
    ) external payable returns (uint amountToken, uint amountLINK, uint liquidity);
...
interface IsushiSwapRouter02 is IsushiSwapRouter01 {
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        ...
        address to,
        uint deadline
    ) external returns (uint amountLINK);
...
contract ML is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    IsushiSwapRouter02 immutable public sushiSwapRouter;
    ...
    constructor (
        ...
        IsushiSwapRouter02 _sushiSwapRouter = IsushiSwapRouter02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
        // Create a pancake pair for this new token
        sushiSwapPair = IPancakeSwapFactory(_sushiSwapRouter.factory())
        .createPair(address(this), _sushiSwapRouter.WETH());
    ...
    function _calculateLINKReward(
        uint256 currentBalance,
        uint256 currentLINKPool,
        uint256 winningDoubleRewardPercentage,
        uint256 totalSupply
    ) public view returns (uint256) {
        uint256 LINKPool = currentLINKPool;
        ...
        uint256 reward = LINKPool.mul(multiplier).mul(currentBalance).div(100).div(totalSupply);
    ...
    function _swapTokensForLINK(
     ...
     function _swapETHForLINK(
        ...
        // make the swap
        sushiSwapRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value : amount} (
            0, // accept any amount of LINK
            path,
            address(this),
            block.timestamp + 360
    ...
    function _swapETHForTokens(
        ...
        // generate the QuickSwap pair path of token -> wLINK
        address[] memory path = new address[](2);
        path[0] = sushiSwapRouter.WETH();
        path[1] = address(this);

        // make the swap
        sushiSwapRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: ethAmount}(
            0, // accept any amount of LINK
            path,
            address(recipient),
            block.timestamp + 360
    ...
    function _addLiquidity(
     ...
     function changeLinkAddress(address addr) external onlyOwner {
         LINK = addr;
     }
    ...
    function removeBlacklistAbility() external onlyOwner {
        canBlacklist = false;
    }
    
    function blacklist(address addr, bool _bool) external onlyOwner {
        require(canBlacklist, "Owner can not blacklist anymore");
        blacklisted[addr] = _bool;
    }
    ...
    function _transfer(
        ...
        if (block.timestamp < antiBotTimestamp){
            if (from == sushiSwapPair){
                require(balanceOf(to).add(amount) <=  _tTotal.mul(75).div(
            10 ** 4), "Anti bot");
            }
        }
    ...
    address public LINK = 0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984;
    ...
    uint256 public _maxTxAmount = _tTotal; // should be 0.15% percent per transaction, will be set again at activateContract() function
    ...
    function claimLINKReward() isHuman nonReentrant public {	
        require(nextAvailableClaimDate[msg.sender] <= block.timestamp, 'Error: next available not reached');	
        require(balanceOf(msg.sender) >= 0, 'Error: must own WO to claim reward');	
        uint256 reward = calculateLINKReward(msg.sender);	
       
        // update rewardCycleBlock	
        nextAvailableClaimDate[msg.sender] = block.timestamp + getRewardCycleBlock();
        IERC20(LINK).transfer(msg.sender, reward);
        emit ClaimLINKSuccessfully(msg.sender, reward, nextAvailableClaimDate[msg.sender]);	
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        !(from == address(this) && to == address(sushiSwapPair)) // swap 1 time
        ) {
            // only sell for minTokenNumberToSell, decouple from _maxTxAmount
            contractTokenBalance = minTokenNumberToSell;


            uint256 pooledLINK = contractTokenBalance.div(2);
            uint256 piece = contractTokenBalance.sub(pooledLINK).div(2);


            
            _swapTokensForETH(piece.add(pooledLINK));
            
            uint256 forLINK =  address(this).balance.sub(address(this).balance.mul(222).div(1000)); 
            
            _swapETHForLINK(forLINK);

            uint256 forLiq = address(this).balance;
            // add liquidity to pancake
            _addLiquidity(piece, forLiq);

            emit SwapAndLiquify(piece, forLiq, piece);
        }
    }
    ...
    function activateContract() public onlyOwner {
        ...
        // protocol
        disruptiveCoverageFee = 10 ether; 
        disruptiveTransferEnabledFrom = block.timestamp;
        setMaxTxPercent(100); // 1% per transaction
        liquidityAdded = true;
        setSwapAndLiquifyEnabled(true);
        antiBotTimestamp = block.timestamp.add(5 minutes);
        moonMissionStarted = true;

        
    }
    
       function activateContractTest() public onlyOwner {
        // reward claim
        disableEasyRewardFrom = block.timestamp + 1 weeks;
        rewardCycleBlock = 3 days;
