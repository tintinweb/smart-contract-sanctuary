...
abstract contract TSTToken is ERC20 {
...
contract AutoFarmV2 is Ownable, ReentrancyGuard {
    ...
    address public TST = 0x19a1417784D8Ab59D3669E7ED338e0B432E45714;
    ...
    returns (uint256)
    {
        if (IERC20(TST).totalSupply() >= AUTOMaxSupply) {
            return 0;
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        TSTToken(TST).mint(address(this), AUTOReward);
    ...
    function safeAUTOTransfer(address _to, uint256 _AUTOAmt) internal {
        uint256 AUTOBal = IERC20(TST).balanceOf(address(this));
        if (_AUTOAmt > AUTOBal) {
            IERC20(TST).transfer(_to, AUTOBal);
        } else {
            IERC20(TST).transfer(_to, _AUTOAmt);
        }
    }
