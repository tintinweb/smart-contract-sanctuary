...
contract GovWorldLiquidator is GovLiquidatorBase {
    ...
    function initialize(
        ...
        marketRegistry = ITokenMarketRegistry(_marketRegistry);
    }
    ...
    function configureTokenMarket(address _tokenMarketAddress) external {
        ...
        require(
            _tokenMarket == address(0),
            "GL: Media Contract Alredy Configured!"
        );
        _tokenMarket = _tokenMarketAddress;
        govTokenMarket = ITokenMarket(_tokenMarket);
    }

    //modifier: only super admin allowed to add and remove liquidators
    ...
    function _liquidateCollateralAutoSellOn(uint256 _loanId) internal {
        ...
        govTokenMarket.updateLoanStatusonLiquidation(_loanId);
        ...
        onlyLiquidatorRole(msg.sender)
    {
        ...
        require(this.isLiquidationPending(_loanId));
    ...
    function fullLoanPaybackEarly(uint256 _loanId) internal {
        ...
        stableCoinFee =
            govTokenMarket.getStableCoinAPYFeeinContract(
                loanDetails.borrowStableCoin
            ) -
            earnedAPYFee;
        ...
        govTokenMarket.updateLoanStatusonPayback(_loanId);
...
abstract contract GovLiquidatorBase is OwnableUpgradeable, IGovLiquidator {
