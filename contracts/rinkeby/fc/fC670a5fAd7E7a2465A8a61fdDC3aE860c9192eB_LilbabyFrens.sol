...
// File: contracts/LilbabyFrens.sol
...
contract LilbabyFrens is ERC721, Ownable {
    using Strings for uint256;
    ...
    event NewLilbabyFrenMinted(address sender, uint256 tokenId);
    ...
    function _processMint(uint256 _mintAmount) internal {
        for (uint256 i = 1; i <= _mintAmount; i++) {
            ...
            emit NewLilbabyFrenMinted(msg.sender, id);
            _tokenSupply.increment();
        }
    }
    ...
    function airdrop(address[] calldata _users) public payable onlyOwner {
        ...
        for (uint256 i = 0; i < _users.length; i++) {
            ...
            emit NewLilbabyFrenMinted(_users[i], id);
            _tokenSupply.increment();
            marketingSupply--;
        }
    }
