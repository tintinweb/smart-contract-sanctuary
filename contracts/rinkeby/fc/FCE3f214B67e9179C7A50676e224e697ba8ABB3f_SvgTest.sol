...
contract SvgTest is ERC721Enumerable, Ownable {
		...
		return
			string(
				abi.encodePacked(
					...
					'   <style>     path { stroke: green;       animation: test 5s infinite;      }          @keyframes test {  to {    stroke: red;  }}  </style>',
					generatePaths(),
					'</svg>'
				)
			);
	...
	function generatePaths() private pure returns (string memory result) {
		for (uint8 i = 0; i < 100; i++) {
			uint256 scale = 1 + i * 3;
			uint256 rotation = 1 + i * 3;

			result = string(abi.encodePacked(result, generatePath(scale, rotation)));
		}
	}

	function generatePath(uint256 scale, uint256 rotation) private pure returns (string memory) {
		return
			string(
				abi.encodePacked(
					'<path d="M29,60.125c57.6456,0 116.23006,3 169,3" stroke-width="calc(2 / ',scale,')" style="transform: rotate(',rotation,'deg) scale(',scale,');"></path>',
					'<path d="M29,60.125c57.6456,0 116.23006,3 169,3" stroke-width="calc(2 / ',scale,')" style="transform: rotate(',rotation,'deg) scale(',scale,');"></path>',
					'<path d="M29,60.125c57.6456,0 116.23006,3 169,3" stroke-width="calc(2 / ',scale,')" style="transform: rotate(',rotation,'deg) scale(',scale,');"></path>',
					'<path d="M29,60.125c57.6456,0 116.23006,3 169,3" stroke-width="calc(2 / ',scale,')" style="transform: rotate(',rotation,'deg) scale(',scale,');"></path>'
				)
