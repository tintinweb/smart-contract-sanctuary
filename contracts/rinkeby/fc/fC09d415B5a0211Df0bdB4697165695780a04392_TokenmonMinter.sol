...
contract EvolutionContract is IEvolutionContract {
  function isEvolvingActive() external override view returns (bool) {
      return true;
  }
  
  function isEvolutionValid(uint256[3] memory) external override returns (bool) {
      return true;
  }
  ...
  function getEvolutionPrice() external override view returns (uint256) {
      return 0 ether;
  }
}
...
contract TokenmonMinter is ERC721Enumerable, PaymentSplitter, Ownable {
  ...
  string private _baseUri = "https://ipfs.io/ipfs/QmQRwoP3U7a1UMpBtYDBMACaFWXNDVr1rRHL9aTN2zg9wM/";
  ...
  event Evolve(address _to, uint256[3] _burnedTokens);
  ...
  function mint(uint256 amount) public payable {
    ...
    require(amount <= 20, "TokenmonMinter: must mint fewer than 30");
  ...
  function toggleMinting() public onlyOwner {
    if (_firstEvoTokenId == 0) {
      REVEAL_TIMESTAMP = block.timestamp + 3 days;
    }
  ...
  function evolve(uint256[3] memory _tokensToBurn) public payable {
    ...
    emit Evolve(msg.sender, _tokensToBurn);
  }
}
