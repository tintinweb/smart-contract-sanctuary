...
contract CloneFactory is Ownable {
    ...
    function setTemplate(address _template) public onlyOwner {
        ...
        implementations.push(_template);

        emit NewImplementation(getVersion(), _template, msg.sender, true);
    }
    ...
    function clone() external returns (address _clone) {
        _clone = Clones.clone(getTemplate());
        addClone(_clone, getVersion());
    }
     ...
     */
    function addClone(address _clone, uint256 _version) public onlyOwner {
        implementations.push(_clone);

        emit NewImplementation(_version, _clone, msg.sender, true);
    }
