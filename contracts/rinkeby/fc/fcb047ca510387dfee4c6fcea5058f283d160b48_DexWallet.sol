...
contract DexWallet is IDexWallet {
    ...
    // withdraw id -> bool
    mapping(uint256 => bool) private withdraws;
    ...
    function withdraw(
        ...
        // can only withdraw once identified by _withdrawId:
        require(withdraws[_withdrawId] == false, "Already withdrawed");
        withdraws[_withdrawId] = true;
        if (_token == ETH) {
            (bool sent, bytes memory d) = _withdrawTo.call{value: _amount}("");
            require(sent, "Failed to withdraw Ether");
        } else {
            require(tokens[_token] != 0, "Token cannot be withdrawed");
            bool sent = IErc(_token).transfer(_withdrawTo, _amount);
            require(sent, "Failed to withdraw ERC");
        }
    ...
    function emergencyTransfer(address _token) public onlyOwner {
        ...
        if (_token == ETH) {
            ...
            bool sent = erc.transfer(owner, amount);
            require(sent, "Failed to emergency transfer ERC");
        }
    }
}