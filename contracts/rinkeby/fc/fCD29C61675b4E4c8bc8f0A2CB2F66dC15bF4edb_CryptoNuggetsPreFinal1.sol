...
contract CryptoNuggetsPreFinal1 is ERC721, Ownable, IMintedBeforeReveal {
    ...
    function getMonstroPrice() public view returns (uint256) {
        require(block.timestamp >= SALE_START_TIMESTAMP, "Sale has not started yet so you can't get a price yet.");
        require(totalSupply() < MAX_NUGGET_SUPPLY, "Sale has already ended, no more Monsters left to sell.");
    ...
    function mintNugget(uint256 numberOfMonstro) public payable {
        ...
        require(numberOfMonstro <= getMonstroMaxAmount(), "You are not allowed to buy this many items at once. The limit is 10.");
        require(SafeMath.add(totalSupply(), numberOfMonstro) <= MAX_NUGGET_SUPPLY, "Exceeds maximum supply of 3,000. Please try to mint less.");
    ...
    */
      function reserveGiveaway(uint256 numMonstro) public onlyOwner {
        uint currentSupply = totalSupply();
        require(totalSupply() + numMonstro <= 10, "10 mints for sale giveaways");
        uint256 index;
