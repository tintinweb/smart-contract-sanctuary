...
contract CryptoFlyz is ERC721, Ownable, PaymentSplitter {
    ...
    function mintWithToad(uint256 nftId) external payable {
    ...
    function multiMintWithnft(uint256 [] memory nftIds) external payable {
        require(mintIsActive, "CryptoFlyz must be active to mint");
        ...
        for (uint i=0; i< nftIds.length; i++) {
            ...
            if (nftIds[i] >= 1000000) {
                ...
                newID = SafeMath.add(newID, 6969);
                require(!_exists(newID), "One of these Toadz has already been used.");
                _safeMint(msg.sender, newID);
                tokenCount++;
            } else {
                require(!_exists(nftIds[i]), "One of these Toadz has already been used.");
    ...
    function mintSpecial(uint256 [] memory specialId) external onlyOwner {        
        ...
        for (uint256 i = 0; i < specialId.length; i++) {
            ...
            _safeMint(msg.sender,specialId[i]);
        }
    }
