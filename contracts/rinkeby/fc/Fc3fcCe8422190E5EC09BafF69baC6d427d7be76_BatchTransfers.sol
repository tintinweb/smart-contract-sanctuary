...
contract BatchTransfers is Ownable{
    ...
    function transfer(
        ...
        require(lastProcessedIndex >= -1 && int256(addrs.length) > lastProcessedIndex + 1,
            "BABYBILZ: last proccessed > -1 and last address not reached");
    	...
    	while(gasUsed < gas && iterations < addrs.length) {
    	    lastProcessedIndex++;
    		if(lastProcessedIndex >= int256(addrs.length)) break;
