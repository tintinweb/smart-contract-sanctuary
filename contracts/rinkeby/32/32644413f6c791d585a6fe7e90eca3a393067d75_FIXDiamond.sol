...
contract FIXDiamond is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 private constant PERCENTAGE_MULTIPLICATOR = 100000; // 10000
    ...
    constructor (uint256 _ownerSupply, address marketing)  {
        _decimals = 18;
        _totalSupply = 100_000_000 * uint(10) ** _decimals;
        require (_ownerSupply < _totalSupply, "Owner supply must be lower than total supply");
        _name = "ProFIXone Diamond Token";
        _symbol = "FixDiamond";
        _price = 1_000_000; 
        _growthRate = 603; 
        _balances[address(this)] = _totalSupply.sub(_ownerSupply).sub(1_000_000);
        _balances[marketing] = _balances[marketing].add(1_000_000);
...
/*
    function calculatePrice() public view returns (uint256) {
        ...
        if (block.timestamp > _lastUpdate) {
            i = uint256((_lastUpdate.sub(_firstUpdate).sub(ST_10_00_MSK)).div(SECONDS_PER_WEEK).add(uint256(1)).sub(_zeros.length));
            ...
            if (_growthRate_after > 0) {
                i = uint256((block.timestamp.sub(_lastUpdate).sub(ST_10_00_MSK)).div(SECONDS_PER_WEEK));
                for (uint256 x = 0; x < i; x++) {
                    newPrice = newPrice.mul(PERCENTAGE_MULTIPLICATOR.add(_growthRate_after)).div(PERCENTAGE_MULTIPLICATOR);
                }
            }
        } else {
            i = uint256((block.timestamp.sub(_firstUpdate).sub(ST_10_00_MSK)).div(SECONDS_PER_WEEK)).add(uint256(1));
