...
contract TestNFTs is ERC721Tradable {
    ...
    function baseTokenURI() public pure override returns (string memory) {
        ...
        return "https://gateway.pinata.cloud/ipfs/QmNMuz5UT7ruWaicijhFvrxj72JTwy5eEnkFbBQpZN2U45/";
    }

    // ============================== Public functions ===================================

    // function turboBoost() internal returns (uint):
    //  uint256 globalAmount = amount;
    //     if (tokenIdCount.current() < roundOneSupply){
    //         globalAmount = uint(globalAmount * 1.5);
    //     } 

    //     else if (tokenIdCount.current() < roundTwoSupply){
    //         globalAmount = uint(globalAmount * 1.25);
    //     } 
    ...
    function governorMintMultiple(address[] memory to) external onlyOwner {
        for (uint256 i = 0; i < to.length; i++) {
            ...
            // tokenIdCount.increment();
            // _mint(to, tokenIdCount.current());
            // Might be economic : https://ethereum.stackexchange.com/questions/19380/external-vs-public-best-practices
        }
    }
