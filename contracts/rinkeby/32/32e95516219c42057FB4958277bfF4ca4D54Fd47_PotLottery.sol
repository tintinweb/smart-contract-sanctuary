...
contract PotLottery {
    ...
    uint256 public winningPoint; //to be deleted later
    uint256[] public winning_point_during_processing; //this is to be deleted later, it stores the value of the winning point through out the process

    function determineWinner() public returns (address) {
        uint256 randomNumber = fullFillRandomness();
        winningPoint = randomNumber % totalPotUsdValue;
        uint256 winning_point = winningPoint;
        address pot_winner;
