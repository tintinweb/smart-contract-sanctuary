    ...
    ContextMixin
{
    ...
    function setAvatarMinter(address _minterAddress) external onlyOwner {
    ...
    function _beforeTokenTransfer(
        ...
        if (
            avatarMinter.allocatedAvatars(tokenId) != address(0) &&
            avatarMinter.allocatedAvatars(tokenId) != auctionAddress &&
            from != address(0)
        ) {
            require(
                block.timestamp > (startDate + 17 days),
                "RootAvatar: Locked for 17 days"
            );
        }
    }
}
