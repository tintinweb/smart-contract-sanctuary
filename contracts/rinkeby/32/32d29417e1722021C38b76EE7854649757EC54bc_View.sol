...
contract View {
    ...
    constructor(address _stakingManager, address _escrowPool) {
    ...
    function fetchData(address _account) external view returns (Data memory result) {
        uint256 rewardPerSecond = stakingManager.rewardPerSecond();
        uint256 lastDistribution = stakingManager.lastDistribution();
        result.pendingRewards = rewardPerSecond * (block.timestamp - lastDistribution);

        result.totalWeight = stakingManager.totalWeight();

        StakingManager.Pool[] memory pools = stakingManager.getPools();
