...
contract SkeletonCrew is ERC721Enumerable, pixiedust, Ownable, recovery , ReentrancyGuard, IERC777Recipient {
    ...
    function mintFreePresaleWithEtherCards(uint256[] memory tokenIds) external {
        ...
        require(tfec <= _maxFreeEC, "Not enough EC allocation remaining");
    ...
    function _mintPayable(uint256 numberOfCards, address recipient, uint256 price) internal {
        ...
        (bool sent, ) = _wallet.call{value: msg.value}(""); // don't use send or xfer (gas)
        require(sent, "Failed to send Ether");
    }

    // Starts at ONE or ZERO ?
    // Assuming 1
    ...
    function tokensReceived(
        ...
        bytes calldata
          ...
          uint256 supply = totalSupply();
            ...
            require(supply + tokenCount <= _maxSupply, "Exceeds Total Mint amount");

            for (uint j = 0; j < tokenCount; j++) {
                _mint(from,supply+j+1);
            }

            IERC777(_DUST).send(_wallet, amount, bytes("Dust Minting"));
    ...
    function process(uint256 random, bytes32 reqID) external {
        ...
        _randomCL = (random << 1) >> 1; // set msb to zero
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory) {
        ...
        return _randomReceived ?
            string(abi.encodePacked(
                revealedBaseURI, 
                ((tokenId + _randomCL) % totalSupply()).toString())
             ) :
        _tokenPreRevealURI;
    }
...
contract pixiedust {
    ...
    // jsons/girls_robots_drogons_2.json

    string                   constant _name   = "GirlsRobotsDragons Sale";
    string                   constant _symbol = "GRDS";
    ...
    uint256                  constant _maxSupply = 5000;

    address payable          constant _wallet = payable(0xEC1a9407B39696c0c85CDc0CdfE36fa2E7063617);

    uint256                  constant _fullPrice = 100000000000000000;
    uint256                  constant _discountPrice = 50000000000000000;
    uint256                  constant _communityPrice = 0;

    bool                     constant _timedPresale  = false;
    uint256                  constant _presaleStart = 1636102800;  // 
    uint256                  constant _presaleEnd = 1637398800;      // 

    bool                     constant _timedSale = true;
    uint256                  constant _saleStart = 1636102800;        // Fri, 05 Nov 2021 09:00
    uint256                  constant _saleEnd = 1638262800;            // Tue, 30 Nov 2021 09:00

    bool                     constant _dustMintAvailable = false;
    uint256                  constant _dustPrice = 560000000000000000000;
    ...
    // DISCOUNTS
    uint256                  constant _maxDiscount = 1000;
    uint256                           _totalDiscount;

    // per address limits
    uint256                  constant _freePerAddress = 0;
    uint256                  constant _discountedPerAddress = 10;

    string                   constant _tokenPreRevealURI = "https://ether-cards.mypinata.cloud/ipfs/QmaiRXTLg3KyVjpgKp947Ydab2wPgQG7xybVyWrb3M6teW";

    address           constant public _signer = 0x0000000000000000000000000000000000000000;
