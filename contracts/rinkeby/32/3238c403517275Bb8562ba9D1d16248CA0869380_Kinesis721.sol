...
contract Kinesis721 is ERC721Enumerable, Ownable {
    ...
    struct Project {
        ...
        mapping(address => bool) isValidatedBuyer;
        mapping(address => uint256) numMintedByWallet;
    ...
    function mint(uint256 _projectId, uint256 amountToMint) public payable {
        ...
        if (projects[_projectId].maxPerWallet > 0) {
            require(projects[_projectId].numMintedByWallet[msg.sender] + amountToMint <= projects[_projectId].maxPerWallet, "You have reached the max number of mints for this wallet");
        }
        if (projects[_projectId].privateSaleIsActive) {
            require(projects[_projectId].isValidatedBuyer[msg.sender], "User is not whitelisted to buy from private sale");
        }
        projects[_projectId].numMintedByWallet[msg.sender] = projects[_projectId].numMintedByWallet[msg.sender] + amountToMint;
    ...
    function addValidatedBuyer(uint256 _projectId, address _address) public onlyAdmin {
        projects[_projectId].isValidatedBuyer[_address] = true;
    }

    function removeValidatedBuyer(uint256 _projectId, address _address) public onlyAdmin {
        projects[_projectId].isValidatedBuyer[_address] = false;
    }
    ...
    function toggleProjectIsPrivateSale(uint256 _projectId) public onlyArtist(_projectId) {
        projects[_projectId].privateSaleIsActive = !projects[_projectId].privateSaleIsActive;
    }

    function updateProjectWalletMintLimit(uint256 _projectId, uint256 limit) public onlyArtist(_projectId) {
        require(limit >= 0, "Limit must be greater than or equal to 0");
        projects[_projectId].maxPerWallet = limit;
    }
    ...
    function addProject(uint256 _pricePerTokenInWei, bool _dynamic) public onlyWhitelisted {
        ...
        projects[projectId].isValidatedBuyer[msg.sender] = true;
        if (!_dynamic) {
    ...
    function projectTokenInfo(uint256 _projectId) view public returns (address artistAddress, uint256 pricePerTokenInWei, uint256 invocations, uint256 maxInvocations, bool active, address additionalPayee, uint256 additionalPayeePercentage, bool privateSaleActive, uint256 maxPerWallet) {
        ...
        maxPerWallet = projects[_projectId].maxPerWallet;
    }
