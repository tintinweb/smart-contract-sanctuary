...
contract YieldBones is IBone20, ERC20 {
	...
	function updateRewardOnMint(address _user) external override {
		...
		rewards[_user] = rewards[_user].add(IERC721(dinoContract).balanceOf(_user).mul(BASE_RATE.mul((time.sub(timerUser)))).div(86400));
	...
	function updateReward(address _from, address _to) external override{
		...
		if (timerFrom > 0)
				rewards[_from] += IERC721(dinoContract).balanceOf(_from).mul(BASE_RATE.mul((time.sub(timerFrom)))).div(86400);
		if (timerFrom != END)
				lastUpdate[_from] = time;
		if (_to != address(0)) {
			uint256 timerTo = lastUpdate[_to];
			if (timerTo > 0)
					rewards[_to] += IERC721(dinoContract).balanceOf(_to).mul(BASE_RATE.mul((time.sub(timerTo)))).div(86400);
			if (timerTo != END)
					lastUpdate[_to] = time;
		}
	}
	...
	function getTotalClaimable(address _user) external override view returns(uint256) {
		uint256 time = min(block.timestamp, END);
		uint256 pending = IERC721(dinoContract).balanceOf(_user).mul(BASE_RATE.mul((time.sub(lastUpdate[_user])))).div(86400);
