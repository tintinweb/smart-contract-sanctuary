...
contract animalMain is  ERC721,Ownable 
{
    ...
    struct animal {
      ...
      function transfer(address _to, uint256 _tokenId) public {
        //TO DO 請使用require判斷要轉的動物id是不是要轉送出去的人的
      ...
      function createAnimal() public payable{
          //動手玩創意，可以限制每次建立動物需要花費多少ETH
           require(msg.value == 0.01 ether, "轉一次 0.01 eth");
      ...
      // DNA
      function GenerateDNA(uint8 star, uint16 roletype)private view returns(bytes32) 
      {
          return keccak256(abi.encodePacked(star, roletype, block.timestamp ));
      }
}