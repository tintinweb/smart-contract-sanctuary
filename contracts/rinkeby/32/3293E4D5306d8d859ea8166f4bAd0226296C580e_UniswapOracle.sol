...
/// @title generic oracle interface for Ying
interface IOracle {
    // ----------- Events -----------
    ...
    function update() external returns (uint256,uint32,uint256,uint256,uint256,uint256,uint256);
...
// File: contracts/interfaces/IUniswapOracle.sol
...
contract UniswapOracle is Permissions, IUniswapOracle {
    using Decimal for Decimal.D256;
    using SafeMathCopy for uint256;

    /// @notice uniswap 交易对实例
    ...
    uint256 public override duration = 5 minutes;
    ...
    /// @param token0 Uniswap Pair token0
    /// @param token1 Uniswap Pair token1
    ...
    function update() external override returns (uint256,uint32,uint256,uint256,uint256,uint256,uint256) {
        (
        ...
        uint256 currentCumulative =
            _getCumulative(price0Cumulative, price1Cumulative);
        ...
        return (twap.asUint256(),priorTimestamp,priorCumulative,deltaCumulative,deltaCumulative,FIXED_POINT_GRANULARITY,duration);
    }

    /// @notice determine if read value is stale 判断是否过期
