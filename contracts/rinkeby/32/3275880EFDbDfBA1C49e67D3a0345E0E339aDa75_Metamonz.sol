...
contract Metamonz is ERC721Enumerable, Ownable, AccessControl {
    ...
    string private _baseTokenURI_electric = "";
    ...
    function mint_batch_1(uint256 num, uint256 base) public payable whenBatch1Open() {
        ...
        require(num <= BATCH_1_MAX_MINT,                                "METAMONZ: You can only mint 3 TESTNFTs");
        require(balanceOf(msg.sender) + num <= MAX_AMOUNT_PER_WALLET,   "METAMONZ: You can only hold 18 TESTNFTs per wallet");
        require(BATCH_1_MINTED + num <= BATCH_1_MAX_MINT,               "METAMONZ: Exceeds maximum TESTNFTs for Batch 1");
    ...
    function mint_batch_2(uint256 num, uint256 base) public payable whenBatch1Open() whenBatch2Open() batch2Allowed(msg.sender) {
        require(base >= 1 && base <= 4,                                 "METAMONZ: Base must be 1, 2, 3 or 4");
        require(num <= BATCH_2_MAX_MINT,                                "METAMONZ: You can only mint 6 TESTNFTs");
        require(balanceOf(msg.sender) + num <= MAX_AMOUNT_PER_WALLET,   "METAMONZ: You can only hold 18 TESTNFTs per wallet");
        require(BATCH_2_MINTED + num <= BATCH_2_MAX_MINT,               "METAMONZ: Exceeds maximum TESTNFTs for Batch 2");
    ...
    function mint_batch_3(uint256 num, uint256 base) public payable whenBatch1Open() whenBatch2Open() whenBatch3Open() {
        require(base >= 1 && base <= 4,                                 "METAMONZ: Base must be 1, 2, 3 or 4");
        require(num <= BATCH_3_MAX_MINT,                                "METAMONZ: You can only mint 9 TESTNFTs");
        require(balanceOf(msg.sender) + num <= MAX_AMOUNT_PER_WALLET,   "METAMONZ: You can only hold 18 TESTNFTs per wallet");
        require(BATCH_3_MINTED + num <= BATCH_3_MAX_MINT,               "METAMONZ: Exceeds maximum TESTNFTs for Batch 3");
    ...
    // GET BASE OF NFT
    // BASE: 1 = FIRE, 2 = WATER, 3 = DARK, 4 = ELECTRIC
    ...
    // SET BASE URI
    function setBaseURI(string memory fire, string memory water, string memory dark, string memory electric) public onlyOwner {
        _baseTokenURI_fire = fire;
        _baseTokenURI_water = water;
        _baseTokenURI_dark = dark;
        _baseTokenURI_electric = electric;
    }
    ...
    // GET BASE URI ELECTRIC
    function getBaseURIElectric() public view returns (string memory) {
        return _baseTokenURI_electric;
    }
