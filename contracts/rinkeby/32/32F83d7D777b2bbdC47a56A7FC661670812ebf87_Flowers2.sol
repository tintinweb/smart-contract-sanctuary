...
interface FlowersInterface {
    function tokenURI(uint256 tokenId, address sender)
        external
        view
        returns (string memory);
}

contract Flowers2 is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    FlowersInterface private flowersFactory;

    constructor(address _flowersFactory) ERC721("Flowers", "FLWRS") Ownable() {
        flowersFactory = FlowersInterface(_flowersFactory);
    }
    ...
    function tokenURI(uint256 tokenId)
        ...
        returns (string memory)
    {
        return flowersFactory.tokenURI(tokenId, _msgSender());
    }

    function addToWhitelist(uint256 _claimAmount, address[] calldata entries)
        external
        onlyOwner
    {
    ...
    function whitelistInfoFor(address _addr)
        public
        view
        returns (
            bool isWhiteListed,
            uint256 numHasMinted,
            uint256 allottedMints
        )
    {
    ...
    function mintedInfoFor(address _addr)
        public
        view
        returns (uint256 numHasMinted)
    {
    ...
    function mint(address destination, uint256 amountOfTokens) private {
        ...
        require(
            totalSupply() + amountOfTokens <= maxSupply,
            "Minting would exceed max supply"
        );
        require(
            amountOfTokens <= maxMint,
            "Cannot purchase this many tokens in a transaction"
        );
        require(amountOfTokens > 0, "Must mint at least one token");
        require(
            _mintPerAddress[msg.sender] + amountOfTokens <= maxPerAddress,
        ...
        if (privateSaleIsActive) {
            require(
                ...
                "Buyer not whitelisted for this private sale"
            );
            require(
                _whiteListPurchases[msg.sender] + amountOfTokens <=
                    _whiteListLimit[msg.sender],
                "Cannot exceed allotted presale mint count"
            );
            _whiteListPurchases[msg.sender] =
                _whiteListPurchases[msg.sender] +
                amountOfTokens;
        }
    ...
    function mintForFriend(address walletAddress, uint256 amountOfTokens)
        public
        payable
        virtual
    {
