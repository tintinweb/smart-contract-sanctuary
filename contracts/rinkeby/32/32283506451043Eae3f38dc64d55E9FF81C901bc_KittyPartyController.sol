...
enum KittyPartyStages {
    ...
    modifier timedTransitions() {
        if (stage == KittyPartyStages.InitialCollection && block.timestamp >= creationTime + 10 hours) {
           nextStage();
           nextStage();// we do a double jump here
        }
        if (stage == KittyPartyStages.Collection && block.timestamp >= creationTime + 10 hours) {
            nextStage();
        }
        if (stage == KittyPartyStages.Staking && block.timestamp >= creationTime + durationInDays * 10 hours) {
            nextStage();
        }
        _;
    }
