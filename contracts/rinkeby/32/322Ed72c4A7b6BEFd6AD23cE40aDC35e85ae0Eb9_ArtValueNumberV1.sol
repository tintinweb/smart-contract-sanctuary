    ...
    OwnableUpgradeable
{
    ...
    **********/
    ...
    function setTokenNumber(uint256 tokenId, uint256 newTokenNumber) public {
        ...
        require(!isAssigned(newTokenNumber), "ArtValueNumber: cannot set token number to already assigned number");
    ...
    function unsetTokenNumber(uint256 tokenId) public {
        ...
        require(hasNumberAssigned(tokenId), "ArtValueNumber: cannot unset number of token that has no number assigned");
    ...
    *****************/
    ...
    function metadataHash(uint256 tokenId) public view returns (uint256) {
        require(_exists(tokenId), "ArtValueNumber: cannot get metadata hash of nonexistent token");

        return _tokenMetadataHashMap[tokenId];
    }

    function setMetadataHash(uint256 tokenId, uint256 _hash) public {
        require(_exists(tokenId), "ArtValueNumber: cannot set metadata hash of nonexistent token");
        require(hasRole(METADATA_HASH_SETTER_ROLE, _msgSender()) || _isApprovedOrOwner(_msgSender(), tokenId), "ArtValueNumber: must have metadata_hash_setter role or be approved or owner to set token metadata hash");
