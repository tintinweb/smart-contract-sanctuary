...
contract Torum is ERC20Interface, SafeMath, Ownable {
    ...
    function transferToBridge(address _offChainToAddress, uint256 _tokenAmount) public returns (bool success) {
        ...
        uint256 _actualTokenAmount;
        uint256 _actualTransactionFee;
        
        if(isFeePercentage) {
            _actualTransactionFee = safeMul(_tokenAmount, safeDiv(transactionFee, 100));
        } else {
            _actualTransactionFee = safeSub(_tokenAmount, transactionFee);
        }
        _actualTokenAmount = safeSub(_tokenAmount, _actualTransactionFee);
        
        balances[onChainBridgeAddress] = safeAdd(balances[onChainBridgeAddress], _actualTokenAmount);
        balances[feeVaultAddress] = safeAdd(balances[feeVaultAddress], _actualTransactionFee);
        
        createMetaData(_offChainToAddress, _actualTokenAmount);
        emit Transfer(msg.sender, onChainBridgeAddress, _actualTokenAmount);
        return true;
    }
