        ...
        override
    {
        ...
        for (uint256 i = 1; i <= __numberOfFlavors; i++) {
            if (__lastBitEnabled) {
                ...
                _setFlavorUri(flavorIdx, defaultFlavorURI);
                _setFlavorUri(flavorIdx + 1, defaultFlavorURI);
            } else {
                // No fuss numbering.
                _setFlavorUri(i * 100, defaultFlavorURI);
            }
        }
    }
...
abstract contract NFTCBase is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function reserveCultTokens(uint256 count) external onlyOwner {
        require(0 < count && count <= MAX_MINT_PER_TRANS, "Invalid count");

        _mintCultTokens(_msgSender(), count);
    }

    function sendCultTokens(address[] memory friends, uint256 count)
        external
        onlyOwner
    {
        require(0 < count && count <= MAX_MINT_PER_TRANS, "Invalid count");
    ...
    function forgeCultTokens(uint256[] memory ownerTokenIndexes, uint256 opCode)
        external
        nonReentrant
    {
        require(
    ...
    function setFlavorURI(
        ...
        _setFlavorUri(flavorIndex, flavorUri);
    }
    ...
    function setFlavorURIBulk(
        ...
        for (idx = 0; idx < flavorIndex.length; idx++) {
            if (extendedFlavor[idx]) {
                require(
                    bytes(_flavorURIs[flavorIndex[idx]]).length == 0,
                    "Cant overwrite"
                );
            } else {
                require(
            ...
            _setFlavorUri(flavorIndex[idx], flavorUri[idx]);
        }
    }
     ...
     *
     * Thanks to all NFT Culture community members.
     */
    ...
    function _setFlavorUri(uint256 flavorIndex, string memory flavorUri)
        internal
    {
        _flavorURIs[flavorIndex] = flavorUri;
    }

    function _mintCultTokens(address minter, uint256 count) private {
