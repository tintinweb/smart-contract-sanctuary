...
pragma solidity ^0.8.0;
contract CARSJAREDNFT is ERC721Enumerable, Ownable, ReentrancyGuard {
    using Counters for Counters.Counter;
    using Strings for uint256;

    uint256 public constant TOKENS_PUBLIC = 10000;
    uint256 public constant maxSupply = TOKENS_PUBLIC;
    ...
    constructor() ERC721("CARSJAREDNFT", "CJNFT") {
    ...
    {
        require(
            _publicTotalTokens.current() < TOKENS_PUBLIC,
            "Purchase would exceed TOKENS_PUBLIC"
        );

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicTotalTokens.current();

            if (_publicTotalTokens.current() < TOKENS_PUBLIC) {
