...
contract IDOTest is IDO {
    ...
    function testSetTimestamps(uint256 start, uint256 end, uint256 claim) external {
        startTime = start;
        endTime = end;
        claimTime = claim;
    }
...
contract IDO is VestedClaim, Whitelisted {
    ...
    ERC20 public immutable USDTAddress;
    ERC20 public immutable USDCAddress;
    ...
    function updateClaimTimeStamp(uint256 _claimTime) external onlyOwner {
        require(block.timestamp < claimTime, "claim has been started");
        require(_claimTime > endTime, "claim has been started");
        claimTime = _claimTime;
    }

    function invest(ERC20 paymentToken, uint256 depositedAmount)
        external
        ...
        checkTimespan
    {
        ...
        emit Invested(msg.sender, depositedAmount, rewardTokenAmount);
    }
    ...
    function withdrawPaymentToken(ERC20 tokenAddress, uint256 amount)
        external
        onlyOwner
    {
        require(tokenAddress != rewardToken, "Cannot withdraw reward token");
    ...
    function withdrawUnclaimedToken() external onlyOwner {
        require(block.timestamp.sub(claimTime) > 720 days);
...
contract VestedClaim is BaseClaim {
        ...
        returns (uint256)
    {
        ...
        if (timeSinceClaim <= 90 days) {
            ...
            uint256 vestable = _totalAmount.mul(80).div(100);
...
contract Whitelisted is Ownable {
    ...
    modifier onlyWhitelisted(address addr) {
        require(whitelisted[addr] == true, "Address has not been whitelisted");
        _;
    }
...
contract BaseClaim is Ownable {
    ...
    modifier onlyWithRewards(address addr) {
        require(userInfo[addr].reward > 0, "Address has not invested");
        _;
    }
