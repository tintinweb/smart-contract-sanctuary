...
contract Dogo is ERC721Enumerable, Ownable, ReentrancyGuard {
	...
	uint256 public constant Dogo_MAX_AMOUNT = 5000;
	uint256 public Dogo_PER_TX = 15;
	uint256 public Dogo_PRICE = 0.08 ether;
	address public founder = 0xDFB53efC8F6a42e67B9C2f1cf8961c66b31284e0;
	...
	function mintPublic(uint256 amount) external payable nonReentrant() {
		...
		require(msg.value == amount * Dogo_PRICE, "You didn't send the right amount of eth");
	...
	function mintPresaleMemberWithAmount(uint256 amount) public payable nonReentrant() {
		...
		require(totalSupply() + amount <= Dogo_MAX_AMOUNT, "Mint would exceed max supply of tokens");
		require(mintedPresaleCount + amount <= Dogo_MAX_AMOUNT, "Mint would exceed max pre-sale tokens");
		require(msg.value == amount*Dogo_PRICE, "You didn't send the right amount of eth");
	...
	function setEntityBirthdaysPrice(uint256 newPrice) public onlyOwner {
		...
		emit SetNewValue("Dogo_PRICE", Dogo_PRICE, newPrice);
		Dogo_PRICE = newPrice;
	}
