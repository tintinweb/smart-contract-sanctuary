...
contract NFT is ERC721, Ownable, RandomlyAssigned {
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(_mintAmount <= maxMintAmount, "max mint amount ssionssion exceeded");
    ...
    if (msg.sender != owner()) {
        if(onlyWhitelisted == true) {
            ...
            require(ownerMintedCount + _mintAmount <= nftPerAddressLimitPresale, "NFT per address limit reached");
        }
        else{
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(ownerMintedCount + _mintAmount <= nftPerAddressLimit, "NFT per address limit reached");
        }
        //console.log("attempting to mint");
        //console.log("cost ", (cost * _mintAmount).toString());
        //console.log("msg.value ", (msg.value).toString());
