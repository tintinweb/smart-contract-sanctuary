...
contract SipherNFTSale is ISipherNFTSale, Whitelist {
  ...
  uint256 public constant SALE_PUBLIC_STARTING_PRICE = 1000000000000000000; //1 ether
  uint256 public constant SALE_BASE_PRICE = 100000000000000000; // 0.1 ether
  uint256 public constant SALE_AUCTION_PRICE_LEVEL = 80000000000000000; //0.08 ether
  uint32 public constant SALE_AUCTION_TIME_INTERVAL = 180; //5 minutes
  ...
  function getElapsedPublicSaleTime() public view returns (uint256) {
    ...
    if (_blockTimestamp() < config.publicTime){
      ...
      return 
      (config.publicTime <= timestamp) && (timestamp < config.endTime)
      ? timestamp - config.publicTime 
      : config.endTime - config.publicTime;
    }
  }

  function getPublicSaleCurrentPrice() public view returns (uint256) {
    uint256 elapsed = getElapsedPublicSaleTime();
    uint256 reducePrice = ((elapsed / SALE_AUCTION_TIME_INTERVAL )*SALE_AUCTION_PRICE_LEVEL);
    if(reducePrice >= SALE_PUBLIC_STARTING_PRICE ) {
      uint256 currentPrice = SALE_BASE_PRICE;
      return currentPrice;
    } else {
      uint256 currentPrice = SALE_PUBLIC_STARTING_PRICE - reducePrice;
      return
   ...
   */
  function buy(uint32 amount,uint32 cap, bytes32[] memory proofs) external payable override {
    ...
    _validateAndUpdateWithBuyAmount(buyer, amount, cap, proofs);
  ...
  *       - can buy up to MAX_OWNER_BOUGHT_INITIAL before endTime with price = 0
   ...
   */
  function _validateAndUpdateWithBuyAmount(address buyer, uint32 amount, uint32 cap, bytes32[] memory proofs) internal {
    SaleConfig memory config = _saleConfig;
    ...
    if (config.whitelistTime <= timestamp && timestamp < config.publicTime) {
      ...
      require(isWhitelistedAddress(buyer, cap, proofs) && whitelistedMerkelRoot != bytes32(0), 'SipherNFTSale: only whitelisted buyer');
      ...
      require(
        _userRecord[buyer].whitelistBought + amount <= cap,
...
contract Whitelist is IWhitelist, Ownable {
    ...
    function isWhitelistedAddress(address buyer, uint32 cap, bytes32[] memory proofs)
        public
        view
        override
        returns (bool)
    {
        ...
        bytes32 computedHash = keccak256(abi.encode(buyer,cap));
...
interface IWhitelist {
  ...
  function isWhitelistedAddress(address buyer, uint32 cap, bytes32[] memory proofs) external view returns (bool);
...
interface ISipherNFTSale {
   ...
   *   The total sold tokens should be capped to maxSupply
   */
  function buy(uint32 amount, uint32 cap, bytes32[] memory proofs) external payable;
