...
contract FarmingLens is Ownable {
    ...
    function getBalanceOf(address _token, address _account) external
        returns(uint balance, uint pendingReward) {
        FarmingInterface.UserInfo memory info = farming.userInfo(_token, _account);
        balance = info.amount;
        pendingReward = farming.getPendingReward(ERC20Detailed(_token), _account);
    }
    ...
    function getBalanceAll(address[] calldata _tokens, address _account) external
        ...
        for (uint i = 0; i < _tokens.length; i ++) {
            (uint balance, uint pendingReward) = this.getBalanceOf(_tokens[i], _account);
            balances[i].balance = balance;
            balances[i].pendingReward = pendingReward;
        }

        return balances;
    }

    function getMatadataOf(address _token) external
    ...
    function getMatadataAll(address[] calldata _tokens) external
        returns(Pool[] memory) {
        Pool[] memory pools = new Pool[](_tokens.length);
        for (uint i = 0; i < _tokens.length; i ++) {
            pools[i] = this.getMatadataOf(_tokens[i]);
        }
