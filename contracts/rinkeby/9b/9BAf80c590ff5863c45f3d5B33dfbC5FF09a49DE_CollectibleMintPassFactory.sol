...
contract CollectibleMintPassFactory is AbstractMintPassFactory  {
    ...
    struct MintPass {
        ...
        uint256 redeemableQuantity;
        string ipfsMetadataHash;
        address redeemableContract;
        uint256 overrideSupplyRemaining;
        bool overrideSupply;
    ...
    constructor(
        ...
        _contractURI = "ipfs://QmddQSpPiF6AJr5PuXdW4BQ5sqYZuFLG9dYLTaV3cuuGZG";
    }
    ...
    function addMintPass(
        ...
        uint256 _redeemableQuantity,
        bool _overrideSupply,
        uint256 _overrideSupplyRemaining
        ...
        mp.redeemableQuantity = _redeemableQuantity;
        mp.overrideSupplyRemaining = _overrideSupplyRemaining;
        mp.overrideSupply = _overrideSupply;
        mpCounter.increment();
    ...
    function editMintPass(
        ...
        mintPasses[_mpIndex].redeemableQuantity = _redeemableQuantity;

    }   

     function editOverrideSupplyRemaining(
        uint256 _overrideSupplyRemaining, 
        uint256 _mpIndex
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        mintPasses[_mpIndex].overrideSupplyRemaining = _overrideSupplyRemaining;
    } 

    function editOverrideSupply(
        bool _overrideSupply, 
        uint256 _mpIndex
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        mintPasses[_mpIndex].overrideSupply = _overrideSupply;
    } 

    function editRedeemableQuantity(
        uint256 _redeemableQuantity, 
        uint256 _mpIndex
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        mintPasses[_mpIndex].redeemableQuantity = _redeemableQuantity;
    ...
    function claim(
        ...
        uint256 numMint = mintPasses[mpIndex].redeemableQuantity * numPasses;

        if(mintPasses[mpIndex].overrideSupply){
            mintPasses[mpIndex].overrideSupplyRemaining -= numPasses;
        }
        
        _mint(msg.sender, 0, numMint, "");

        emit Claimed(mpIndex, msg.sender, numMint);
    }
    ...
    function claimMultiple(
        ...
        for (uint i=0; i< mpIndexs.length; i++) {
            ...
            numMint += mintPasses[mpIndexs[i]].redeemableQuantity * numPasses[i];
            if(mintPasses[mpIndexs[i]].overrideSupply){
                mintPasses[mpIndexs[i]].overrideSupplyRemaining -= numPasses[i];
            }
        }
    ...
    function isValidClaim( uint256 numPasses,
       ...
       require(
        ...
        if(mintPasses[mpIndex].overrideSupply){
             isValid = mintPasses[mpIndex].overrideSupplyRemaining - numPasses >= 0;
        }

        require(
            isValid,
            "Bulk: purchase would exceed max supply" 
        ); 
    ...
    function isSaleOpen(uint256 mpIndex) public view returns (bool) {
        if(!paused() && block.timestamp > mintPasses[mpIndex].windowOpens && block.timestamp < mintPasses[mpIndex].windowCloses && mintPasses[mpIndex].saleIsOpen){
            return true;
        }
        else{
            return false;
