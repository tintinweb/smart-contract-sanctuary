...
contract SuperGeisha is ERC721A, PaymentSplitter, Ownable {
    ...
    // Tracks redeem for claim
    mapping(address => uint256) private claimsLeft;
    ...
    uint256 private saleMaxPerWallet;
    ...
    ) ERC721A("SuperGeisha", "SG", 10) PaymentSplitter(payees_, shares_) {
        price = price_;
    ...
    function claim(uint256 quantity) public {
        ...
        require(claimsLeft[_msgSender()] - quantity > 0, "Exceeded Max Claim");

        claimsLeft[_msgSender()] = claimsLeft[_msgSender()] - quantity;
    ...
    function claimableQuantity(address address_) public view returns (uint256) {
        return claimsLeft[address_];
    }
    ...
    function setClaimsList(
        address[] calldata addresses,
        uint256[] calldata quantities
    ) external onlyOwner {
        require(addresses.length == quantities.length, "Invalid Length");

        for (uint256 i = 0; i < addresses.length; i++) {
            claimsLeft[addresses[i]] = quantities[i];
        }
    }
