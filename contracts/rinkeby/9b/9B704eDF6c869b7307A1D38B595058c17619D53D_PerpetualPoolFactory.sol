...
interface IPerpetualPoolFactory {
    function initializePerpetualFactory(address _shareTokenFactory) external;
...
contract PerpetualPoolFactory is PerpetualUpdateFunctions, OracleInterfaceID, IFunctionList, IPerpetualPoolFactory {
    ...
    function initializePerpetualFactory(address _shareTokenFactory) external override onlyOwner {
        require(address(shareTokenFactory) == address(0), "already initialized");
    ...
    function createLiquidityPool(
        ...
        require(_iTargetPoolSizeUpdateTime >= 30 && _iTargetPoolSizeUpdateTime <= 1 weeks, "invalid iTargetPoolSizeUpdateTime");
        
        require(_iPnLparticipantWithdrawalPeriod > 0 && _iPnLparticipantWithdrawalPeriod <= 1 weeks, "invalid iPnLparticipantWithdrawalPeriod");
        require(
     ...
     */
    function runLiquidityPool(uint256 _liqPoolID) external override onlyOwner {
        ...
        _updateOraclePricesForPool(_liqPoolID, block.timestamp);
    }
    ...
    function setTreasury(uint256 _poolId, address _treasury) external override onlyOwner {
    ...
    function getFunctionList() external view override returns (bytes4[] memory) {
        ...
        functionList[0] = this.initializePerpetualFactory.selector;
