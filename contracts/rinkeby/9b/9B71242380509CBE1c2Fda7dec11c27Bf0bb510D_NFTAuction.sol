...
contract NFTAuction is Context, ReentrancyGuard {
    ...
    function placeBid(uint256 _tokenId) external payable nonReentrant whenNotPaused {
        ...
        (uint256 tokenId, address creator, address owner, uint256 price, string memory tokenUri) = marketTradingNft.getNFTDetailByTokenId(_tokenId);
        emit BidPlaced(_tokenId, creator, owner, price, tokenUri, _msgSender(), bidAmount);
    }
