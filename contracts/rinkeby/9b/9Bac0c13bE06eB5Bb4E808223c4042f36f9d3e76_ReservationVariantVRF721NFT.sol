...
contract ReservationVariantVRF721NFT is ERC721, Ownable {
  ...
  function purchaseReservation(uint256 quantity) external payable {
    require(block.timestamp >= ticketingStartTimeUnix, "ReservationVariantVRF721NFT::purchaseReservation: ticketing period has not started");
    require(block.timestamp <= ticketingEndTimeUnix, "ReservationVariantVRF721NFT::purchaseReservation: ticketing period has ended");
    require((msg.value) == (ticketingPriceWei * quantity), "ReservationVariantVRF721NFT::purchaseReservation: incorrect ETH value provided");
    require(quantity <= singleOrderLimit, "ReservationVariantVRF721NFT::purchaseReservation: quantity exceeds max per transaction");
    require((_ticketIds.current() + quantity) <= supplyLimit, "ReservationVariantVRF721NFT::purchaseReservation: would cause total supply to exceed max supply");

    // We increment first because we want our first ticket & reservation ID to have an ID of 1 instead of 0
    // (makes wrapping from max -> min slightly easier)
    for(uint256 i = 0; i < quantity; i++) {
      _ticketIds.increment();
      uint256 newReservationId = _ticketIds.current();
      buyerToReservationIds[msg.sender].push(newReservationId);
      ticketIdToBuyer[newReservationId] = msg.sender;
    }
  }
  ...
  function mint(uint256[] memory _mintReservationIds) external {
    ...
    uint256[] memory ticketIdsMemory = buyerToReservationIds[msg.sender];
    require(ticketIdsMemory.length > 0, "ReservationVariantVRF721NFT::mint: msg.sender has not purchased any tickets");
    uint256 buyerToRedeemedReservationCountMemory = buyerToRedeemedReservationCount[msg.sender];
    require(buyerToRedeemedReservationCountMemory < ticketIdsMemory.length, "ReservationVariantVRF721NFT::mint: all msg.sender tickets redeemed");
    uint256 ticketSupply = _ticketIds.current();
    for(uint256 i = 0; i < _mintReservationIds.length; i++) {
      require(ticketIdToBuyer[_mintReservationIds[i]] == msg.sender, "ReservationVariantVRF721NFT::mint: mintId not assigned to msg.sender");
      ...
      if((_mintReservationIds[i] + randomOffset) <= ticketSupply) {
        // e.g. with a randomOffset of 2, and a ticketSupply of 10, and a reservationId of 5: offsetTokenId = 7 (5 + 2)
        offsetTokenId = _mintReservationIds[i] + randomOffset;
      } else {
        // e.g. with a randomOffset of 2, and a ticketSupply of 10, and a reservationId of 9: offsetTokenId = 1 (wraps around from 9 -> 1: (2 - (10 - 9)))
        offsetTokenId = (randomOffset - (ticketSupply - _mintReservationIds[i]));
      }
  ...
  function initiateRandomDistribution() external {
    require(block.timestamp > ticketingEndTimeUnix, "ReservationVariantVRF721NFT::mint: ticketing period has not ended");
    uint256 ticketSupply = _ticketIds.current();
    require(ticketSupply > 0, "ReservationVariantVRF721NFT::beginReveal: ticket supply must be more than 0");
