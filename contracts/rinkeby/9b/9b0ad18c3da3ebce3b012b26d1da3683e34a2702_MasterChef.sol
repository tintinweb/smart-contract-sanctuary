...
contract MasterChef is UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 public startUnlockBlock;      // start to unlock since this time (after 3 months)
    ...
    //_currentBlock 배포할때 빼고 block.number 로 대채해야함 
    ...
    function initialize(
        ...
        startUnlockBlock = startBlock + 30 days / blockTime;
    }
    ...
    function unlockAvailable() public view returns(uint256) {
        if(_currentBlock < startUnlockBlock) return 0;

        LockedInfo storage user = lockedInfo[msg.sender];
        if(user.lockedAmount <= 0) return 0;

        uint256 pastMonth = (_currentBlock - startUnlockBlock) * blockTime / 30 days;
    ...
    function unlock() external nonReentrant {
        require(_currentBlock < startUnlockBlock, "No available to unlock");
        ...
        user.unlockedCount = (_currentBlock - startUnlockBlock) * blockTime / 30 days;
    }
    ...
    function updateStartUnlockBlock(uint256 _block) external onlyOwner {
        require(_currentBlock < startUnlockBlock);
        require(_block > _currentBlock, "time should be greater than current block number");
        startUnlockBlock = _block;
    }
