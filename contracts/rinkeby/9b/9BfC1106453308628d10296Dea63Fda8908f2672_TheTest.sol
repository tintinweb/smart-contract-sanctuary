 ...
 */
 ...
 // Cambiar SPDX-License-Identifier

 /**
  * TODO: check commented lines
  * TODO: check comment values
  * TODO: check supportsInterface function
  * TODO: comment public sale smart contract
  * TODO: check for console.log()'s DONE
  * TODO: lower the mint-per-transaction amount DONE
  */
...
contract TheTest is ERC721, ERC721Enumerable, Ownable {

    // Whitelist Merkle Root
    bytes32 public whitelistMerkleroot;

    // Mapping from owner to testies minted in pre-sale
    mapping(address => uint256) private _mintedPresale;

    // Mapping from owner to testies minted in public sale
    mapping(address => uint256) private _mintedPublicsale;
    
    // Testy price for public sale
    uint256 public constant TestySalePrice = 0.09 ether; // Should we use ether or wei?

    // Testy price for public sale
    uint256 public constant TestyPresalePrice = 0.07 ether; // Should we use ether or wei?
    
    // Max testies quantity a user can mint at a time in pre-sale
    uint256 public constant MAX_PRESALE_MINT = 5;

    // Max testies quantity a user can mint at a time in publicsale
    uint256 public constant MAX_PUBLIC_MINT = 10;

    // Numbers of testies the team will keep
    uint256 public constant TEAM_testies = 222;

    // Numbers of testies the team will keep
    uint256 private constant AIRDROP_testies = 16;
    
    // Max TNWG supply
    uint256 public MAX_testies;

    // Max TNWG presale supply
    uint256 public MAX_PRESALE_testies = 1947;
    ...
    // Indicates if team already got testies.
    bool public aretestiesReserved = false;

    // Original provenance
    string public originalProvenance;

    // Post-deterministic shuffle provenance
    string public finalProvenance;

    // Metadata base URI
    string public baseURI = ""; // TODO THIS
    ...
    constructor(string memory name, string memory symbol, uint256 maxNftSupply) ERC721(name, symbol) {
        MAX_testies = maxNftSupply;
        reserveAirdroptesties(AIRDROP_testies);
    }

    /**
    * Mints public sale testies.
    * @param numberOfTokens Number of tokens to be minted.
    */
    function mint(uint numberOfTokens) public payable {
        require(isSaleActive, "Sale has not started yet.");
        require(_mintedPublicsale[msg.sender] + numberOfTokens <= MAX_PUBLIC_MINT, "Can only mint 10 testies per wallet in public sale CONTRACT");
        require(totalSupply() + numberOfTokens <= MAX_testies, "Purchase would exceed max supply of testies");
        require(TestySalePrice * numberOfTokens <= msg.value, "Ether value sent is not enough");
        
        for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply();
            if (totalSupply() < MAX_testies) {
    ...
    /**
     * Mints pre-sale testies if isPresaleActive is true.
    ...
    external payable
    {
        ...
        require(_mintedPresale[msg.sender] + numberOfTokens <= MAX_PRESALE_MINT, "Can only mint 5 testies per wallet in presale");
        require(totalSupply() + numberOfTokens <= MAX_PRESALE_testies, "Purchase would exceed max supply of presale testies");
        require(TestyPresalePrice * numberOfTokens <= msg.value, "Ether value sent is not enough");
        
        for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply();
            if (totalSupply() < MAX_PRESALE_testies) {
    ...
    /**
     * Set testies aside for the Aidrop.
     * Only called once by the constructor
     * @param numberOfTokens Number of tokens to be minted.
     */
    function reserveAirdroptesties(uint numberOfTokens) internal{        
    ...
    /**
     * Set testies aside for the team.
     * Only can be called once after sale is active.
     * @param numberOfTokens Number of tokens to be minted.
     */
    function reserveTeamtesties(uint256 numberOfTokens) public onlyOwner{
        uint256 supply = totalSupply();
        require(!aretestiesReserved, "Already reserved some testies for the team.");
        require(supply + TEAM_testies <= MAX_testies, "Purchase would exceed max supply of testies");    

        for (uint256 i = 0; i < numberOfTokens; i++) {
            _safeMint(msg.sender, supply + i);
        }

        aretestiesReserved = true;
    }
    ...
    /**
     * Sets nex max supply
     * @param newMaxSupply New MAX_testies value
     * New value should be lower than totalSupply()
     * New value should be lower than MAX_testies
     */
    function setMaxSupply(uint newMaxSupply) public onlyOwner{
        ...
        require(newMaxSupply < MAX_testies, "New max supply is greater than MAX_testies.");
        MAX_testies = newMaxSupply;
    }
