...
contract Mizuki is ERC721, ERC721Enumerable, VRFConsumerBase, Ownable {
    ...
    uint256 private _totalAdvisorShare;
    address[] private _advisors;
    mapping(address => uint256) private _advisorShares;

    mapping(uint256 => uint256) public metadata;

    constructor()
        ERC721("Chic Mizuki", "Mizuki")
        VRFConsumerBase(
    ...
    // Helper to list all the Mizukis of a wallet
    ...
    function addAdvisor(address _receiver, uint256 _share) external onlyOwner {
        require(
            _totalAdvisorShare.add(_share) < 10000,
            "Total share exceeds 100%"
        );

        _totalAdvisorShare = _totalAdvisorShare.add(_share);

        if (_advisorShares[_receiver] == 0) {
            _advisors.push(_receiver);
        }

        _advisorShares[_receiver] = _share;
    }
    ...
    function withdraw() public onlyOwner {
        ...
        // Send eth to advisors
        for (i = 0; i < _advisors.length; i++) {
            payable(_advisors[i]).transfer(
                totalBalance.mul(_advisorShares[_advisors[i]]).div(10000)
            );
        }
    ...
    function fulfillRandomness(bytes32, uint256 randomness) internal override {
        randomStyle = randomness;
    }

    function getRandomStyle(uint256 n)
        public
        view
        returns (uint256[] memory expandedValues)
    {
        ...
        for (uint256 i = 0; i < n; i++) {
            expandedValues[i] = uint256(keccak256(abi.encode(randomStyle, i)));
        }
