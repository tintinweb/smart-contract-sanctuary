...
// File: contracts/BNFYStakingNFT.sol
...
contract BNFYStakingNFT is Ownable, ERC721 {
    using SafeMath for uint256;
    ...
    constructor() Ownable() ERC721("BNFY Staking NFT", "BNFYNFT") public {}

    // Will mint BNFY NFT when a user stakes
    ...
    function burn(uint256 _token) external onlyPlatform() {
        require(!safeguard);
        require(_exists(_token), "ERC721Metadata: Burn of nonexistent token");
        _burn(_token);
    }
    
    function setTokenURI(uint256 tokenId, string memory _tokenURI) external onlyPlatform() {
         require(!safeguard);
     ...
     * or to the token ID if {tokenURI} is empty.
     */
    function setBaseURI(string memory baseURI_) external onlyPlatform() {
        require(!safeguard);
