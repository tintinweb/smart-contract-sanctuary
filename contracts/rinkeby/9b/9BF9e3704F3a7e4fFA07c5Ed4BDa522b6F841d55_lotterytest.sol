...
contract lotterytest is ERC721, Ownable, ReentrancyGuard, VRFConsumerBase {
  ...
  uint public seasonStartDate = block.timestamp;
  uint public timeWindow = 3 seconds;
  uint public maxTicketsPerSeason = 200;
  uint public maxTicketsPerAddress = 200;
  uint public maxTicketsPerTransaction = 10;
  mapping(uint => mapping(address =>uint)) public thisSeasonsTicketsByAddress;
  mapping(uint => uint) public seasonOfTicket;
  mapping(uint => uint[]) public seasonToParticipantsTokenIds;
  //uint[] public thisSeasonsTickets;
  bool public drawingInProgress;

  // address of the last drawn lottery winner
  address public lastWinner;
  
  // events for progress information
  ...
  event TimeWindowChanged(uint newTime);
  ...
  function buyTicket(uint amount) external payable nonReentrant {
    require(!drawingInProgress, "Wait until lottery drawing is over to enter next season.");
    require(msg.value == ticketPrice * amount,"Not the correct ticket price.");
    require(getCurrentSeasonTicketCount() + amount <= maxTicketsPerSeason,"Maximum amount of tickets sold for this season, come back next Season.");
    require(amount <= maxTicketsPerTransaction, "Trying to buy more than the maximum tickets per transaction.");
    require(thisSeasonsTicketsByAddress[getCurrentSeason()][msg.sender] + amount <= maxTicketsPerAddress, "You bought the maximum amount of tickets per season already.");
    
    thisSeasonsTicketsByAddress[getCurrentSeason()][msg.sender] += amount;
  ...
  function mintTicket() private {
    ...
    seasonOfTicket[id] = getCurrentSeason();
    _safeMint(msg.sender, id);
    //thisSeasonsTickets.push(id);
    seasonToParticipantsTokenIds[seasonOfTicket[id]].push(id);
    _tokenIds.increment();
  }

  // Initiates the Lottery Winner Drawing, by sending a request to Chainlink VRF, to provide a random number
  function requestLotteryWinnerDrawing() external onlyOwner nonReentrant {
    require(!drawingInProgress, "already drawing a winner for this seasons lottery");
    require( block.timestamp >= seasonStartDate + timeWindow, "too early to draw new winner");
    require( getCurrentSeasonTicketCount() > 1, "wait for more participants");
    drawingInProgress = true;
    bytes32 drawingRequestId = getRandomNumber();
    emit LotteryDrawingRequested(drawingRequestId, getCurrentSeason(), getCurrentSeasonTicketCount(), getCurrentPot());
  }

  // sends request for random number to Chainlink VRF node along with fee
  function getRandomNumber() private returns (bytes32 requestId) {
    require(LINK.balanceOf(address(this)) >= fee, "Not enough LINK - fill contract with LINK token or faucet");
  ...
  function getWinnerAddress(uint randomVal) private returns (address){
    uint winnerTokenId = seasonToParticipantsTokenIds[getCurrentSeason()][randomVal % getCurrentSeasonTicketCount()];
    address winnerAddress = ownerOf(winnerTokenId);
    emit LotteryWon(getCurrentSeason(), winnerAddress, winnerTokenId);
    return winnerAddress;
  }

  // sends 80% of pot to winner, 20 % to treasury
  function payWinnings(address winner) private {
    (bool d, ) = payable(0x7207952e02c360fcD7C0b8521affE5e89F9d1B7f).call{value: address(this).balance * 20 / 100}("");
    require(d, "treasury transaction failed");
    uint winnings = address(this).balance;
    (bool s,) = payable(winner).call{value: winnings}("");
    require(s, "winner transaction failed");
    emit LotteryPaid(getCurrentSeason(), winner, winnings);
  }

  function resetAndStartNextSeason() private {
    _season.increment();
    seasonStartDate = block.timestamp;
    drawingInProgress = false;
    emit NextSeasonStarted(getCurrentSeason(), seasonStartDate);
  }
  ...
  function changeTimeWindow(uint newTimeWindow) external onlyOwner {
    timeWindow = newTimeWindow;
    emit TimeWindowChanged(newTimeWindow);
  }
  ...
  function getCurrentSeasonTicketCount() public view returns (uint){
    return seasonToParticipantsTokenIds[getCurrentSeason()].length;
  }

  function getCurrentSeasonTickets() public view returns(uint[] memory){
    return seasonToParticipantsTokenIds[getCurrentSeason()];
  }
