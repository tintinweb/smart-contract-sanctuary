...
contract Concoles is ERC721Enumerable, Ownable {
    
    uint256 private currentTokenId = 1;

    uint256 MAX_SUPPLY = 10000;
    string public baseTokenURI;
    uint256 public startTime = 1635116400; //  Oct 25th 00:00  ??
    uint256 cost = 4e16;
    
    
    
    // testing only - delete //
    string _name = "consoles";
    string _symbol = "CSL";
    string _uri = "test";
    ...
    function mint(uint256 _num) public payable {
        ...
        require(_num > 0 && _num <= 20); // mint limits
        require(msg.value >= _num * cost); // mint cost
        
        for (uint256 i = 0; i < _num; i++) {
            _mint(msg.sender, currentTokenId);
            emit TokenBought(currentTokenId);
            currentTokenId++;
        }
    }
    ...
    function bulkMintTo(address[] calldata _to) external onlyOwner {
        for (uint i=0; i<_to.length; i++) {
            _mint(_to[i], currentTokenId);
            currentTokenId++;
        }
