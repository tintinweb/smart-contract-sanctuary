...
contract Poker is ERC721, ReentrancyGuard, Ownable {
    ...
    function getCard(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "Player Card:", cards, suits);
    }
    
    function getDealerCard(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "Dealer Card:", cards, suits);
    }
    ...
    function pluck(
        ...
        string memory output =  string(
                abi.encodePacked(prefixes[rand % prefixes.length], " ", sourceArray[rand % sourceArray.length]));
