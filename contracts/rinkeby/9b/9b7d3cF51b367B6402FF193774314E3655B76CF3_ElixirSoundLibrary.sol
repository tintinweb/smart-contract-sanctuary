...
contract ElixirSoundLibrary is ERC721 {
    ...
    mapping(uint => SoundToken) soundTokens;
    ...
    function mintSound(string memory name, string memory tokenURI, uint price) public {
        uint tokenId = tokenCount;
        SoundToken memory newSound;
        newSound.tokenId = tokenId;
        newSound.name = name;
        newSound.tokenURI = tokenURI;
        newSound.price = price;
        newSound.creator = payable(msg.sender);
        soundTokens[tokenId] = newSound;
        creatorToSound[msg.sender].push(tokenId);    
        _safeMint(msg.sender, tokenId);
        tokenCount++;        

        emit SoundTokenCreated(tokenId, name, tokenURI, price, msg.sender);
    }
    
    function addLicensee(uint tokenId) public payable {
        uint price = soundTokens[tokenId].price;
        uint commission = price / 50;
        address creator = soundTokens[tokenId].creator;
        require(msg.sender != creator, "Licensee cannot be the creator.");
        require(msg.value == price+commission, "Please submit the correct amount of ether");
        soundTokens[tokenId].licensees.push(msg.sender);
    ...
    function getSound(uint tokenId) public view returns (SoundToken memory){
        return soundTokens[tokenId];
    }
