...
contract WalletSimple  {
  // Events
    event ForwardDeposited(address executer, uint256 value, bytes data);
    event Deposited(address from, uint256 value, bytes data);
    event ForwardDepositedToken(address tokenContractAddress, address executer, uint256 value, bytes data);
  // Public fields
  ...
  fallback() external payable {
     ...
     executeDepositEvent(msg.sender, msg.value, address(this), msg.data);
    if (msg.value > 0) {
  ...
  function flushTokens(address tokenContractAddress) public {
    ...
    TransferHelper.safeTransfer(
     ...
     emit ForwardDepositedToken(tokenContractAddress, msg.sender, forwarderBalance, msg.data);
  }
   /**
