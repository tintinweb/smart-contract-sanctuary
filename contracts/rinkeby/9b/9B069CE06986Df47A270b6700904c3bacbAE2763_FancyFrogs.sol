...
contract FancyFrogs is ERC721, Ownable {
    
    using SafeMath for uint256;

    uint256 public constant MAX_TOKENS = 100; //10 000
    uint256 public constant MAX_TOKENS_PER_PURCHASE = 20;
    
    uint private nonce = 0;
    uint private tokenCounter = 0;
    uint[MAX_TOKENS] private indices;

    uint256 private price = 65000000000000000; // 0.065 Ether

    bool public isSaleActive = false;
    bool public URIset = false;

    constructor() ERC721("Fancy Frogs", "FancyFrogs") {}
    ...
    */
    function randomIndex() internal returns (uint) {
      uint totalSize = MAX_TOKENS - tokenCounter;
    ...
    function reserveTokens(address _to, uint256 _reserveAmount) external onlyOwner {
      require(isSaleActive == false, "Sale has already started");

      uint i;
      uint id;

      for(i = 0; i < _reserveAmount; i++) {
        id = randomIndex();
        _safeMint(_to, id);
    ...
    function mint(uint256 _count) external payable {
      ...
      require(_count > 0, "Mint at least 1 token");
      require(_count <= MAX_TOKENS_PER_PURCHASE, "Trying to mint too many tokens in a single transaction");
      require(tokenCounter + _count <= MAX_TOKENS, "Exceeds maximum tokens available for purchase");
      require(msg.value >= price.mul(_count), "Ether value sent too low");
    ...
    function activateSale() external onlyOwner {
      require(URIset, "URI hasn't been set yet");
      isSaleActive = true;
    }
