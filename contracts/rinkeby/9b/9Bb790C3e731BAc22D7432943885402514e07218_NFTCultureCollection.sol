 ...
 */
contract NFTCultureCollection is NFTCultureCollectionBase {
    constructor()
        NFTCultureCollectionBase(
            'The Gallery by NFT Culture',
            'https://gateway.pinata.cloud/ipfs/QmNXynSoUjRENXPTrtfLJVCVZkbfNkzL62DbeqhELJV9nY/{id}.json',
            0,
            0xd4d44c1DA605F1AbC33EA5B4b2C905A5dDCE5790, // The NFTCult contract on rinkeby.
            0x1A33e9Ab1c9e3fA2Da1c0b4a7DcDc5855eD9dFcC  // The NFTCultForgeComponents contract on rinkeby.
        )
    {
 ...
 * airdrip pieces.
 */
contract NFTCultureCollectionBase is ERC1155Supply, Ownable {
    uint256 public constant EMBER_SUPPLY = 10**18;
    ...
    INFTCult_V1_Thin private nftCult;
    INFTCultForgeComponents private nftCultForgeComponents;

    // For future extension
    mapping(uint256 => uint256) private _emberLookup;
    mapping(uint256 => uint256) private _extendedYields;

    // Format: [64 bit claims consumed][64 bit claims yield][64 bit ember received][64 bit ember yield]
    mapping(uint256 => uint256) private _cache;

    mapping(uint256 => uint256) private _cultTokenClaimsConsumed;
    mapping(uint256 => uint256) private _cultTokenEmbersReceived;

    function _addExtendedYield(
        uint256 forgeComponent,
        uint256 claimYield,
        uint256 emberYield
    ) internal {
        _extendedYields[forgeComponent] = claimYield << (128 + emberYield);
    }

    function _getForgeComponent(uint256 nftcultTokenId)
        internal
        view
        returns (uint256)
    {
        return
            nftCultForgeComponents.getMapping(nftCult.tokenURI(nftcultTokenId));
    }

    function _getForgeComponentYield(uint256 forgeComponent)
        internal
        view
        returns (uint256)
    {
        // These are hardcoded for gas efficiency.
        if (forgeComponent == 201) {
            return defaultYield;
        }

        if (forgeComponent == 202) {
            return defaultYield;
        }

        if (forgeComponent == 203) {
            return twotoneYield;
        }

        if (forgeComponent == 301) {
            return defaultYield;
        }

        if (forgeComponent == 302) {
            return defaultYield;
        }

        if (forgeComponent == 303) {
            return twotoneYield;
        }

        if (forgeComponent == 401) {
            return defaultYield;
        }

        if (forgeComponent == 402) {
            return defaultYield;
        }

        if (forgeComponent == 403) {
            return twotoneYield;
        }

        if (forgeComponent == 997) {
            return ffspecialYield;
        }

        if (forgeComponent == 901) {
            return justiceYield;
        }

        if (forgeComponent == 902) {
            return justiceYield;
        }

        if (forgeComponent == 903) {
            return twotoneJusticeYield;
        }

        // fallback to any extended values here.
        return _extendedYields[forgeComponent];
    }
    ...
    constructor(
        ...
        nftCult = INFTCult_V1_Thin(__nftCult);
        nftCultForgeComponents = INFTCultForgeComponents(
    ...
    {
        // Uninitialized
        if (_cache[nftcultTokenId] == 0) {
            return
                _getForgeComponentYield(_getForgeComponent(nftcultTokenId)) >>
                128;
        }

        // Initialized
        uint256 tokenStats = _cache[nftcultTokenId];
        return (uint64(tokenStats >> 128) - tokenStats) >> 192;
    }
        ...
        returns (uint256)
    {
        ...
        // Initialized
        uint256 tokenStats = uint128(_cache[nftcultTokenId]);
        return (uint64(tokenStats) - tokenStats) >> 64;
    }
    ...
    function addForgeComponentClaimables(
        uint256 forgeComponent,
        uint256 claimYield,
        uint256 emberYield
    ) external onlyOwner {
        _addExtendedYield(forgeComponent, claimYield, emberYield);
    }
    ...
    function claimPiecesInCollection(
        ...
        uint256 pieceId,
        uint256 count,
        uint256 forgeComponentId
    ) external payable {
        require(exists(pieceId), 'Invalid piece to claim');
        //require(_emberLookup[pieceId] == 0, 'Cant claim that type');
        require(
            nftCult.ownerOf(nftcultTokenId) == msg.sender,
            'Cult token not owned'
        );

        uint256 yieldBitMap = _cache[nftcultTokenId];
        if (yieldBitMap == 0) {
            // We havent seen this token before, so we must look up.
            require(
                _getForgeComponent(nftcultTokenId) == forgeComponentId,
                'Invalid forge component Id'
            );

            yieldBitMap = _getForgeComponentYield(forgeComponentId);
        }

        uint64 claimYield = uint64(yieldBitMap >> 128);
        uint64 claimsConsumed = uint64(yieldBitMap >> 192);
        ...
        _cultTokenClaimsConsumed[nftcultTokenId] += count;

        //uint64 emberYield;
        //uint64 emberConsumed;
        // A fun bonus.
        // uint256 embersReceivedFromCultToken = _cultTokenEmbersReceived[
        //     nftcultTokenId
        // ];
        // uint256 emberYield = _cultTokenEmberYield[forgeComponentId];
        // if (embersReceivedFromCultToken < emberYield) {
        //     _safeTransferFrom(
        //         owner,
        //         msg.sender,
        //         emberPieceId,
        //         emberYield - embersReceivedFromCultToken,
        //         ''
        //     );
        //     _cultTokenEmbersReceived[nftcultTokenId] = emberYield;
        // }
    }
}
