...
function multiSend(address tokenairdrop, address[]calldata recipients, uint256[]calldata amounts) external onlyOwner {
    ...
    TransferHelper.safeTransferFrom(tokenairdrop,address(this),recipients[i], amounts[i]);  
    }
}


function batch(address tokenAddr,address[] memory toAddr,uint256  value)public onlyOwner  {
        IERC20 erc20 = IERC20(tokenAddr);
        for (uint256 i = 0; i < toAddr.length; i++) {
            !erc20.transfer(toAddr[i], value) ;
        }
