...
import "./burnNRedeem/ERC721RedeemBase.sol";

/**
 * Aoki Redeem
 */
contract mockExtension2 is
    ...
    ICreatorExtensionTokenURI
{
    ...
    string private _endpoint =
        "https://client-metadata.ether.cards/api/aoki/FightingGravity/";
    ...
    constructor(address _creator)
        ...
        ERC721RedeemBase(_creator, 1, max)
    {
        creator = _creator;
    }
    ...
    {
        return
            interfaceId == type(ERC721RedeemBase).interfaceId ||
    ...
    function mintTo(address _to, uint16 _count) public {
        IERC721CreatorCore(creator).mintExtensionBatch(_to, _count);
        totalCount += _count;
    }

    function tokenURI(address __creator, uint256 tokenId)
        public
        view
        override
        returns (string memory)
    {
        require(creator == __creator, "Invalid token");
        return
            string(
                abi.encodePacked(
                    _endpoint,
                    uint256(tokenId % totalCount).toString()
                )
            );
    }
}
