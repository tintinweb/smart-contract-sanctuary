...
contract CMFarming is Ownable {
    ...
    constructor (address tokenAddress_, address rewardTokenAddress_) {
        require(tokenAddress_ != address(0), "CMFarming: 0 address");
        require(tokenAddress_ != address(0), "CMFarming: 0 reward address");
        tokenAddress = tokenAddress_;
        rewardTokenAddress = rewardTokenAddress_;
        admin = msg.sender;
        withdrawer = msg.sender;
        minStake = 1000*10**18;
        lockupPeriod = 86400*7*12;
    }
...
//        require(startTime_ > block.timestamp, "SEA-01");
    ...
    function withdraw(address staker, uint256 amount, uint256 amountReward)
        external
        _realAddress(staker)
        returns (bool)
    {
        require(msg.sender == withdrawer,"CMFarming: Not Withdrawer");
        require(amount >= 0 && amountReward > 0, "CMFarming: Wrong withdrawal amount");
        return _withdraw(staker, amount, amountReward);
    }

    function _withdraw(address _staker, uint256 _amount, uint256 _amountReward)
        private
        returns (bool)
    {
        bool withdrawn = _payDirect(_staker, _amount, tokenAddress);
        bool claimed = _payDirect(_staker, _amountReward, rewardTokenAddress);
        require(withdrawn && claimed, "CMFarming: withdrawing error");
        emit Withdrawn(_staker, _amount, _amountReward, block.timestamp);
        return true;
    }
    ...
    modifier _realAddress(address addr) {
        require(addr != address(0), "CMFarming: zero address");
        _;
    }

    modifier _positive(uint256 amount) {
        require(amount >= 0, "CMFarming: negative amount");
        _;
    }
    ...
    modifier _hasAllowance(address allower, uint256 amount) {
        ...
        require(
            amount <= ourAllowance,
            "CMFarming: Not enough allowance"
        );
        _;
    }
