...
contract BasicToken is ERC20, Ownable {
    ...
    constructor() payable ERC20("Basic", "BSC") {
...
contract NWToken is ERC721, ERC721Enumerable, Ownable {
...
contract ProofOfExistence {
    ...
    function notarize(string memory document) public {
        string memory proof = proofFor(document);
        storeProof(proof);
    }

    // helper function to get a document's sha256
    function proofFor(string memory document) private pure returns (string memory) {
        return bytes32ToString(sha256(abi.encodePacked(document)));
    }
    ...
    //string can be converted to bytes with bytes(<string>)
