...
contract Moartroller is MoartrollerV6Storage, MoartrollerInterface, MoartrollerErrorReporter, ExponentialNoError, Versionable {
    ...
    // Custom initializer
    function initialize(LiquidityMathModelInterface mathModel, LiquidationModelInterface lqdModel) public {
...
abstract contract MToken is MTokenInterface, Exponential, TokenErrorReporter, MTokenStorage {
    ...
    function getRealBorrowIndex() public view returns (uint) {
        ...
        require(mathErr == MathError.NO_ERROR, "could not calculate simpleInterestFactor");

        (mathErr, borrowIndexNew) = mulScalarTruncateAddUInt(simpleInterestFactor, borrowIndexPrior, borrowIndexPrior);
        require(mathErr == MathError.NO_ERROR, "could not calculate new borrowIndex");
    ...
    function borrowForInternal(address payable borrower, uint borrowAmount) internal nonReentrant returns (uint) {
        require(moartroller.isPrivilegedAddress(msg.sender), "Insufficient permission");
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted borrow failed
            return fail(Error(error), FailureInfo.BORROW_ACCRUE_INTEREST_FAILED);
