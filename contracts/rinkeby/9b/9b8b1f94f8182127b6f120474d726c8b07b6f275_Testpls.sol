...
contract Testpls is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   TestplsDividendTracker public dividendTracker;

    address public liquidityWallet;

    uint256 public constant MAX_SELL_TRANSACTION_AMOUNT = 10000000 * (10**18);

    uint256 private ETH_REWARDS_FEE = 10;
    ...
    function activate() public onlyOwner {
        require(!tradingEnabled, "Testpls: Trading is already enabled");
        _swapEnabled = true;
        tradingEnabled = true;
    }
    ...
    constructor() ERC20("Test Please", "Testpls") {
        dividendTracker = new TestplsDividendTracker();
    ...
    receive() external payable {
     ...
     function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Testpls: The Uniswap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Testpls: Automated market maker pair is already set to that value");
    ...
    function excludeFromFees(address account) public onlyOwner {
        require(!_isExcludedFromFees[account], "Testpls: Account is already excluded from fees");
    ...
    function updateFees(uint256 _ETH_REWARDS_FEE, uint256 _LIQUIDITY_FEE) external onlyOwner {
        require(_ETH_REWARDS_FEE < 15, "Rewards Fee must be less than 15");
        require(_LIQUIDITY_FEE < 10, "DEV Fee must be less than 10");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        ...
        require(newValue != gasForProcessing, "Testpls: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if (!tradingIsEnabled) {
            ...
            if(!openForPresale){
                require(canTransferBeforeTradingIsEnabled[from], "Testpls: This account cannot send tokens until trading is enabled");
            }
        }
    ...
    function swapAndSendToDev(uint256 tokens) private {
        ...
        address payable _devAndMarketingAddress = payable(0xe9fc872Ca90536c15e525C310920b789C5aC169D);
...
contract TestplsDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Testpls_Dividend_Tracker", "Testpls_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "Testpls_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "Testpls_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Testpls contract.");
    }
    ...
    function updateGasForTransfer(uint256 newGasForTransfer) external onlyOwner {
        require(newGasForTransfer != gasForTransfer, "Testpls_Dividend_Tracker: Cannot update gasForTransfer to same value");
    ...
    function updatDividendTrackerToken(address tokenAddress) external onlyOwner {
        require(tokenAddress != dividendToken, "Testpls_Dividend_Tracker: Cannot update dividendToken to same value");
        dividendToken = tokenAddress;
    }

    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "Testpls_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Testpls_Dividend_Tracker: Cannot update claimWait to same value");
