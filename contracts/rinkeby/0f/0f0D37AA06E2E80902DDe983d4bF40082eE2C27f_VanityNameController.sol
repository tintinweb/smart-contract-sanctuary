...
contract VanityNameController is OwnableAndPausable {
    ...
    function buy(string memory vanityName) public payable {
        _isNotPaused();
        require(!_exists(vanityName), "ERC721: token already minted");
        require(msg.value == _buyFeeETH, "ETH sent are not enough to create the token.");
