    ...
    ReentrancyGuardUpgradeable   
{
    ...
    // _merge = 0 => new order, 1 => merged existing order
    event ChangedBidURI(address _user, string _old, string _new, uint256 _merge);
    ...
    function initialize(
        ...
        blockWait = 1228540;                // roughly 6 months
    ...
    */
    ...
    function deleteBid(uint256 index, address user) private {
        Bid[] storage bids = _bids[user];
        if (index != bids.length - 1) bids[index] = bids[bids.length - 1];
        bids.pop();
    }
    ...
    */
    function redeemProfile(uint256 tokenId) external {
        ...
        INftToken(nftErc20Contract).burn(INftProfile(nftProfile).profileDetails(tokenId)._nftTokens.mul(50).div(10000));
