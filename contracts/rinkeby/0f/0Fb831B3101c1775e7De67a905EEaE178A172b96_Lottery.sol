...
contract PriceFeed {
    ...
    // amended (who/20210923) - parameterize price feed contract address
    // note the address can be a true price feed contract or a local mock
...
contract Lottery is VRFConsumerBase, Ownable {
    ...
    uint8 internal constant USD_ENTRY_FEE = 50;
    ...
    function enter() public payable {
        require(state == LOTTERY_STATE.OPEN, "Lottery is yet to start!");
        require(msg.value >= getEntranceFee(), "Fund is less than minimum");
    ...
    function getEntranceFee() public view returns (uint256) {
        ...
        uint256 usdEntryFee = USD_ENTRY_FEE * precision;
        uint256 price = getPrice();
        return (usdEntryFee * precision) / price;
    }

    function startLottery() public onlyOwner {
        require(state == LOTTERY_STATE.CLOSED, "Cannot start a lottery yet!");
        state = LOTTERY_STATE.OPEN;
    }

    function endLottery() public onlyOwner {
        require(state == LOTTERY_STATE.OPEN, "Lottery is not opened yet!");
        state = LOTTERY_STATE.WIP;
        requestId = requestRandomness(keyHash, fee);
        emit RequestedRandomness(requestId);
    }

    // internal - only allows the vrfCoordinator to call back - addressing security concern
    function fulfillRandomness(bytes32 _requestId, uint256 _randomness)
        internal
        override
    {
        require(state == LOTTERY_STATE.WIP, "Not yet in calculating winner!");
        require(_randomness > 0, "No random number is generated!");
