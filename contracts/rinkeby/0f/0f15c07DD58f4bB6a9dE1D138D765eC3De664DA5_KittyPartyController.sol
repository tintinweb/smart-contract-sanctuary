...
contract KittyPartyController is KittyPartyStateTransition, Ownable, Pausable{
    ...
    ///@dev These functions will be called by oracle/backend or anyone can call them
    ...
    function applyWinnerStrategy() public timedTransitions() atStage(KittyPartyStages.Payout){
        ...
        for (uint16 i = 0; i < kittens.getLength(); i++) {
            bytes memory payload = abi.encodeWithSignature("mint(address,uint256,uint256,bytes)", kittens.getValueAt(i), 0, amountInDAIPerRound * kittens.getLength(), "");
            // bytes memory payload = abi.encodeWithSignature("mint(address,uint256,uint256,bytes)", kittens.getValueAt(i), 0, winAmount, "");
            (bool success,) = address(accountantContract).call(payload);
            require(success, "Not able to mint receipts to kittens");
        }
    }
    ...
    function approveRoundDeposits() public kittenExists(msg.sender) timedTransitions() atStage(KittyPartyStages.Collection) whenNotPaused() returns(bool){
        ...
        require(dai.approve(address(msg.sender), amountInDAIPerRound), 'Please approve');
...
abstract contract Ownable is Context {
    ...
    function set(address _newowner) internal {
        //this can only be called during cloning when the owner is the zeroth address
...
enum KittyPartyStages {
    ...
    modifier timedTransitions() {
        if (stage == KittyPartyStages.InitialCollection && block.timestamp >= creationTime + 10 seconds) {
           nextStage();
           nextStage();// we do a double jump here
        }
        if (stage == KittyPartyStages.Collection && block.timestamp >= creationTime + 10 seconds) {
            nextStage();
        }
        if (stage == KittyPartyStages.Staking && block.timestamp >= creationTime + durationInDays * 10 seconds) {
            nextStage();
        }
        _;
    }
