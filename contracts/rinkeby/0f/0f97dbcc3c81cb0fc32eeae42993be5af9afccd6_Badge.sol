...
contract Badge is ERC1155, BaseContract{
  ...
  // such as
    ...
    constructor(string memory UriPrefix_, string memory contractURI_) ERC1155(tokenUri) {
    ...
    function createBadge(
        ...
        for (uint256 i = 0; i < numberOfCopies; i++) {
            ...
            badgeExpiry[ids[i]] = _matureTime;
            badgeExpiry[ids[i]] += block.timestamp;
            unchecked {
                tokenCounter += 1;
            }
            mapTokenUri[ids[i]] = URIs[i];
    ...
    function applyBadge(uint256[] memory badgeIds) external onlyStudent {
        for (uint256 i = 0; i < badgeIds.length; i++) {
            mapBadgeUser[badgeIds[i]].push(msg.sender);
            mapUserBadge[msg.sender].enumAdd(badgeIds[i]);
            require(block.timestamp <= badgeExpiry[badgeIds[i]],
            "Error: badge expired");
        }
        ...
        onlySponsor
    {
        ...
        for (uint256 i = 0; i < badgeIds.length; i++) {
            ...
            coolingTime[badgeIds[i]] += block.timestamp;
     ...
     function resetBadgeExpiry(uint256 _reMaturedTime, uint256[] memory badgeIds)external payable onlySponsor{
        ContractManagerInterface manager = ContractManagerInterface(
            managerAddress
        );
        address getsAddress = manager.getAddress("gets");
