...
contract Marketplace {
    ...
    function SignUp(string memory _id) public {
        Pseller.register(payable(msg.sender), _id);
        seller.register(payable(msg.sender), _id);
        buyer.register(payable(msg.sender), _id);
    }
    ...
    function FindUserID(address payable _addr) public view returns(string memory) {
        return(seller.findID(_addr));
    }
    
    function FindUserAddr(string memory _id) public view returns(address) {
        return(seller.findAddr(_id));
    }
    
    function FindUserRec(address payable _addr) public view returns(uint256) {
        uint256 _PosRec;
        uint256 _TradeRec;
        uint256 _StdRec;
        (_PosRec, _TradeRec, _StdRec) = seller.checkRec(_addr);
    ...
    function Deal(string memory _name, uint256 _count) checker() public payable {
        ...
        BuyerBoard.pay(buyer, payable(msg.sender), _count, 1, owner);
        BuyerBoard.pay(seller, payable(msg.sender), _count, 3, owner);
    ...
    function PosFeedback(string memory _name, uint256 _count) checker() public payable {
        ...
        BuyerBoard.pay(buyer, _g.buyer_addr, _count, 1, owner);
        BuyerBoard.pay(seller, _g.buyer_addr, _count, 3, owner);
