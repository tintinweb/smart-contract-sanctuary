...
contract ICO is AccessControl {
    ...
    function contribute(uint256 _amount) external onlyRole(WHITELISTED) {
        ...
        require(totalAccumulated + _amount <= GOAL, "Contribution exceeds goal");

        // must be one of the 3 options
        require(_amount == 100 ether || _amount == 400 ether || _amount == 800 ether, "Incorrect amount");
        require(addressToContribution[msg.sender] + _amount <= LIMIT_PER_USER, "Max limit reached");
    ...
    function withdraw() external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        require(block.timestamp >= startTime + duration || totalAccumulated == GOAL, "ICO in progress");
        BUSD.transfer(msg.sender, totalAccumulated);
    }

    function whitelist(address[] memory _whitelist) external onlyRole(DEFAULT_ADMIN_ROLE) {
