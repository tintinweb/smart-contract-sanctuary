...
contract NewBuff is Context, IERC20, Ownable {
    ...
    function requestNewCoinWithOldBuff(address _oldBuffWallet, address referralWallet) external {
        ...
        if(referralWallet != address(0)) {
            ...
            _mint(_oldBuffWallet, (oldBuffBalance.sub(affiliateAmount)).mul(currentRate));
            _mint(referralWallet, affiliateAmount.mul(currentRate));
        }
        else {
            _mint(_oldBuffWallet, oldBuffBalance.mul(currentRate));
        }

        _burn(address(this), oldBuffBalance.mul(currentRate));
    ...
    function setShoppingCart(address cartAddress) external onlyOwner returns (bool) {
        ...
        _mint(cartAddress, cartAmount.mul(currentRate));
        _burn(address(this), cartAmount.mul(currentRate));
    ...
    function setRewardAddress(address rewardAddress) external onlyOwner returns (bool) {
        ...
        _mint(rewardAddress, burnAmount.mul(currentRate));
        _burn(address(this), burnAmount.mul(currentRate));
        _approve(rewardAddress, owner(), burnAmount.mul(currentRate));
    ...
    function mintDev(Minting[] calldata mintings) external onlyOwner returns (bool) {
        ...
        for(uint i = 0; i < mintings.length; i++) {
            ...
            _mint(recipient, amount.mul(currentRate));
            _burn(address(this), amount.mul(currentRate));
