...
contract Project is IProject, SignatureDecoder {
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < _phaseLength; i++) {
            ...
            _oldPhaseCost += _oldCost;
            _newPhaseCost += _phaseCosts[i];
            ...
            if (_oldCost > _phaseCosts[i]) {
                ...
                totalAllocated -= _oldCost - _phaseCosts[i];
            } else if (_oldCost < _phaseCosts[i]) {
        ...
        nonReentrant
    {
        ...
        require(_cost > 0, "!value");
        require(projectCost() >= uint256(totalInvested + _cost), "");
        ...
        contractorAccepted
    {
        ...
        uint256[] memory _tasksToInvite = new uint256[](_hash1.length);
        ...
        if (_j > 0) {
            inviteSCInternal(_tasksToInvite, _scToInvite, _j);
        }
    }
    ...
    function inviteSC(uint256[] memory _taskList, address[] memory _scList)
        ...
        override
    {
        inviteSCInternal(_taskList, _scList, _taskList.length);
    }
    ...
    function inviteSCInternal(
        ...
        for (uint256 i = 0; i < _limit; i++) {
            require(_scList[i] != address(0), "Project::0 address");
            require(
                _scList[i] != builder && _scList[i] != contractor,
                "Project::Builder&&GC !invite"
            );
            tasks[_taskList[i]].inviteSubcontractor(_scList[i]);
            eventsInstance.scInvited(_taskList[i], _scList[i]);
        }
    }
        ...
        nonReentrant
    {
        ...
        if (_newCost != _taskCost) {
            ...
            if (_newCost < _taskCost) {
                ...
                uint256 _withdrawDifference = tasks[_taskID].cost - _newCost;
                ...
                totalAllocated -= tasks[_taskID].cost; // reduce from total allocated
        ...
        if (_newSC != tasks[_taskID].subcontractor) {
            ...
            if (_newSC != address(0)) {
                ...
                _scList[0] = _newSC;
                inviteSC(_ts, _scList); // inviteSubcontractor
            } else {
...

{
  ...
  "libraries": {
    "contracts/libraries/Tasks.sol": {
      "Tasks": "0xdc87e6a33320e89a9ffeece6b3885de93221c57c"
    }
  }
}