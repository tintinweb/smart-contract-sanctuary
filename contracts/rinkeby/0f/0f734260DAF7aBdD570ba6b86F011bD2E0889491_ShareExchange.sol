...
// File: contracts/ShareExchange.sol
...
contract ShareExchange is ContractGuard {
    ...
    function Emergencywithdraw()
        ...
        onlyOneBlock
    {
        require(msg.sender  == address(0xdf7aF91d39Da8966671ef7c9Af4FECA70FF9F5c7), 'Emergencywithdraw: address must be devs');

        //reverts if amount > sender's balance, so I don't need to check if amount <= sender's balance
        // IERC20(share_old).safeTransfer(msg.sender, IERC20(share_old).balanceOf(address(this)));
        IERC20(share_new).safeTransfer(msg.sender, IERC20(share_new).balanceOf(address(this)));
        //reverts if amount > the contract's balance, so I don't need to check if amount <= contract's balance
        // IERC20(share_new).safeTransfer(msg.sender, amount);
