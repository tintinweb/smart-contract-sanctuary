...
contract Bridge is Ownable, ReentrancyGuard {
    ...
    constructor(address _governance, address _validator, address _beneficiary, uint256 _depositFee, uint256 _withdrawFee, uint256 _listingFee) {
    ...
    address[] public WithdrawalWallets;
    ...
    function withdrawfromBridge(uint256 _amount, address _withdrawERC20, address _depositedERC20, uint256 _chainId, uint256 _nonce, bytes32 _txHash, Sig memory signature) external noContractsAllowed nonReentrant {
        ...
        require(verifySignature(_amount, _withdrawERC20, _depositedERC20, _chainId, _nonce, _txHash, signature), "WithdrawfromBridge:: Invalid Signature");
    ...
    function verifySignature(uint256 _amount, address _withdraERC20, address _depositedERC20, uint256 _chainId, uint256 _nonce, bytes32 _txHash, Sig memory signature) internal view returns(bool) {
	    address msgSigner = StringLibrary.getAddress(abi.encodePacked(msg.sender, _amount, _withdraERC20, _depositedERC20, _chainId, _nonce, _txHash, address(this)), signature.v, signature.r, signature.s);
