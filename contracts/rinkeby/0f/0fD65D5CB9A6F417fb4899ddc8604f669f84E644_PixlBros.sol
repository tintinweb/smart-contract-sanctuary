...
contract PixlBros is ERC721Tradable {
    ...
    string private _contractURI = "https://pixlbros.com/api/contract";
...
abstract contract ERC721Tradable is ContextMixin, ERC721Enumerable, NativeMetaTransaction, Ownable {
    ...
    function ipfsTokenURI() virtual public view returns (string memory);

    function tokenURI(uint256 _tokenId) override public view returns (string memory) {
        require(_tokenId < 8205, "ERC721Metadata: URI query for nonexistent token");

        string memory base = baseTokenURI();

        // If there is no base URI, return the token URI.
        if (bytes(base).length == 0) {
            return string(abi.encodePacked(ipfsTokenURI(), Strings.toString(_tokenId)));
        }
