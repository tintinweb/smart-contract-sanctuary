...
interface InscribeInterface {
    ...
    /*
    * @dev Similar to the function above, without passing in an optional URI.
    */
     ...
     */
    function addInscriptionWithSig(address nftAddress, uint256 tokenId, address inscriber, uint256 nonce, uint256 baseUriId, string memory optionalURI, bytes memory sig) external;
    
    /* @dev Similar to the function above without passing in an optional URI.
    * The hash to be signed consists of (nftAddress, tokenId, inscriber, nonce, optionalURI, thisContractAddress)
    */
    function addInscriptionWithSig(address nftAddress, uint256 tokenId, address inscriber, uint256 nonce, uint256 baseUriId, bytes memory sig) external;  
...
contract Inscribe is InscribeApproval, InscribeInterface {
    ...
    function addInscriptionWithSig(
        ...
        bytes32 message = keccak256(abi.encodePacked(nftAddress, tokenId, inscriber, nonce, baseUriId, this)).toEthSignedMessageHash();
        
        // Verifies the signature
        require(ECDSA.recover(message, sig) == inscriber, "Address does not match signature");
        
        _addInscription(nftAddress, tokenId, inscriber, baseUriId, "");
    }
    ...
    function addInscriptionWithSig(
        ...
        // Recreate signed message 
        bytes32 message = keccak256(abi.encodePacked(nftAddress, tokenId, inscriber, nonce, baseUriId, optionalURI, this)).toEthSignedMessageHash();
    ...
    */
    function _addInscription(address nftAddress, uint256 tokenId, address inscriber, uint256 baseUriId, string memory optionalURI) private {
        ...
        if (bytes(optionalURI).length > 0) {
            require(_supportsOptionalUri(baseUriId), "Base URI does not support optional URI");
