...
import "hardhat/console.sol";
    ...
    //TODO : When user sell his token what will do? "PENDING"
...
contract IBGV0 is IIBGEvents, CustomOwnable, ReentrancyGuard {
    ...
    uint public constant SERVICE_START_TIME = 1619522998;//TODO: check before deploying

    uint private constant DECIMAL_FACTOR = 10**18;
    uint private constant SECONDS_IN_DAY = 10;//change this to 86400
    uint private constant IBG_TO_USDT = 1 * DECIMAL_FACTOR;//TODO: confirm the initial conversion rate
    ...
    uint private constant MAX_CURRENT_PLAN = DECIMAL_FACTOR ** 2;

    uint private constant MAX_ELIGIBLE_REFERRER_WALKS = 25;


    //TODO: make list for all the values that should be dynamic
    uint ibgPercentage;
    uint bgbfPercentage;
    uint maxPacks;
    uint maxPlans;
    ...
    uint increaseIBGAfterLimit;
    uint increaseRateIBG;
    uint public totalIBGDistributed;
    uint yieldPercentage;
    ...
    function initialize(address _governance, address _ibg, address _usdt, address _root, address _treasuryWallet) external {
        ...
        increaseRateIBG = 25 * 10**16;//TODO: change 25 to 10
        increaseIBGAfterLimit = 100000 * DECIMAL_FACTOR;
        maxPacks = MAX_PACKS;
        maxPlans = MAX_PLANS;
    ...
    // To change IBG and BGBF percentage of package amount
    ...
    function registerAdmin(address _user, address _referrer, uint _pack, uint _plan, uint stakingPeriod) external onlyOwner onlyBeforeLaunch {
        _register(_user, _referrer, _pack, _plan, stakingPeriod, true);
    }

    function register(address _referrer, uint _pack, uint _plan, uint stakingPeriod) external {
    ...
    function investmentAdmin(address _user, uint _pack, uint _plan, uint stakingPeriod) external onlyOwner onlyBeforeLaunch {
        require(_pack >= 1 && _pack <= maxPacks, 'IBGV0: Invalid Pack');
        require(_plan >= 1 && _plan <= maxPlans, 'IBGV0: Invalid Plan');
        require(planActiveStatus[_plan], 'IBGV0: Plan is not active yet');
        require(stakingPeriod >= 1 && stakingPeriod <= 3, 'IBGV0: Invalid staking period');
        ...
        uint value = packPrice[_pack];
        _investment(_user, _pack, _plan, value, stakingPeriod, true);
    }

    /*
    Investment on the basis of selected package.
    */

    function investment(uint _pack, uint _plan, uint stakingPeriod) public {
        require(_pack >= 1 && _pack <= maxPacks, 'IBGV0: Invalid Pack');
        require(_plan >= 1 && _plan <= maxPlans, 'IBGV0: Invalid Plan');
        require(planActiveStatus[_plan], 'IBGV0: Plan is not active yet');
        require(stakingPeriod >= 1 && stakingPeriod <= 3, 'IBGV0: Invalid staking period');
    ...
    function _investment(address _user, uint _pack, uint _plan, uint currentPackAmount, uint stakingPeriod, bool isAdmin) private {
        ...
        if(_plan == 1) {
            ...
            if (isAdmin) {
                iBGAmount = currentPackAmount;
            } else {
                iBGAmount = ((currentPackAmount).mul(DECIMAL_FACTOR)).div(IBGToUsdt);
    ...
    /*
    Send Referral to Direct referral address
    For IBG :- Direct Referral earnings will be calculated from 20% of the package amount
    For BGBF :- Direct Referral earnings will be calculated from 10% of the package amount
    receiver :- Address of the receiver
    amount :- Amount send to referrerAddress
    */

    function iBGYieldCommission(address user, address reciever, uint currentPlan, uint yieldDistributionAmount) private {
        uint level = 1;
        TransferHelper.safeTransfer(address(ibg), treasuryWallet, (yieldDistributionAmount.mul(20)).div(100));

        while(level <= 5) {
            uint amount = (yieldDistributionAmount.mul(yieldMatchingRate[level]).div(100));
            if(reciever == address(0)) {
                // Check who will recieve income when address is zero.
    ...
    function commisionTransfer(address _from, address _receiver, uint currentPlan, uint distributeAmount, uint _basePercentage) private {
        TransferHelper.safeTransfer(address(usdt), treasuryWallet, (distributeAmount.mul(10)).div(100));
        uint level = 1;
        while(level <= 3) {
            uint amount;
    ...
    function withdrawYield(bool onlyYield) public {
        // calculateYieldAndInvestment(msg.sender);
        _withdrawIBGYield(msg.sender, users[msg.sender].investmentCount, onlyYield);
    }
    //ToDo withdraw investmnt and decrease rate;
    function _withdrawIBGYield(address user, uint cycles, bool onlyYield) private {
        ...
        if(totalInvestment > 0 && !onlyYield) {
            ...
            demotePack(user, (totalInvestment.mul(IBGToUsdt)).div(DECIMAL_FACTOR));
        }
        ...
        TransferHelper.safeTransfer(address(ibg), treasuryWallet, ((totalYield).mul(5)).div(100));
    ...
    function demotePack(address user, uint investment) private {
        if(users[user].currentPlan >= investment) {
            users[user].currentPlan = users[user].currentPlan.sub(investment);
        } else {
    ...
    function calculateIBGTokenPrice() private {
        ...
        if(newValue > lastRate) {
            ...
            IBGToUsdt = (1 * DECIMAL_FACTOR).add(newValue);
        }
    }
    ...
    function updatePack(uint packNumber, uint price) external onlyOwner returns(uint, uint) {
    ...
    function updatePlan(uint planNumber, string calldata planName) external onlyOwner returns(uint, string memory) {
    ...
    function updateStakingTime(uint timeIndex, uint timeValue) external onlyOwner returns(uint, uint) {
    ...
    function updateYieldRate(uint timeInDays, uint yieldValue) external onlyOwner returns(uint, uint) {
    ...
    function updateBGBFToken(address _token) external onlyOwner {
        require(_token != address(0), "IBGV0: Invalid Token Address");
        bgbf = IERC20(_token);
    }

    function updatePlanStatus(uint plan, bool status) external onlyOwner {
    ...
    function updateTreasuryWallet(address _treasuryWallet) external onlyOwner {
        require(_treasuryWallet != address(0), 'IBGV0: Invalid Address');
        require(_treasuryWallet != treasuryWallet, "IBGV0: Wallet is same as previous");
        treasuryWallet = _treasuryWallet;
    }

    function walletBalance() external onlyOwner {
