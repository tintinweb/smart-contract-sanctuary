...
contract SecretMintPass is ERC721, Ownable {
    bytes32 immutable public root;

    address secretProject;

    // baseURI
    string private _uri;

    constructor(bytes32 merkleroot)
    ERC721("Secret mint pass", "SMP")
    {
        root = merkleroot;
    }
    ...
    function setSecretProject(address secretProjectAddress) 
    external onlyOwner {
        secretProject = secretProjectAddress;
    }

    function burn(uint256 tokenId)
    external 
    {
        require(msg.sender == secretProject);
        _burn(tokenId);
    }
