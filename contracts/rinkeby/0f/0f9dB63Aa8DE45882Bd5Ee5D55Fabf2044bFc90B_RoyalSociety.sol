...
contract RoyalSociety is ERC721Enumerable, Ownable {
    ...
    uint256 public constant RESERVES = 100; // Saved for the team for promotional purposes

    uint256 private _saleTime = 1626483600; // https://www.epochconverter.com/
    uint256 private _price = 80000000000000000; // This is currently .08 eth
    uint256 private _maxPerTx = 21; // This value will be one higher than the true max
    ...
    function setMaxPerTransaction(uint256 maxPerTx) public onlyOwner {
        _maxPerTx = maxPerTx;
    }
    ...
    function getReserves() public onlyOwner {
        ...
        require(
            totalSupply == 0,
            "The reserves can only be saved when trigger the mint of Token #0 - #100."
        );
        require(
    ...
    function mint(uint256 _count) public payable {
        ...
        require(
            _count < _maxPerTx,
            "Exceeds the max token per transaction limit."
        );
        require(
        ...
        require(
            block.timestamp >= _saleTime,
            "The Royal Society sale has not yet started."
        );
