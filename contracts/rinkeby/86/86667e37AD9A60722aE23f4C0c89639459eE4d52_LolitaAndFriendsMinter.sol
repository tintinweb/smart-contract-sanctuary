...
import "./ILolitaAndFriendsToken.sol";

contract LolitaAndFriendsMinter is Ownable, ReentrancyGuard {
    ...
    // ======== Cost =========
    uint256 public constant TOKEN_COST = 0.01 ether;

    // ======== Sale status =========
    bool public saleIsActive = false;
    uint256 public preSaleStart; // Whitelist minting starts
    ...
    function mintPublic(uint amount) public payable nonReentrant {
        ...
        require(supply + amount <= maxTokens - reservedTokens, "Exceeds max supply!");
        require(msg.value >= TOKEN_COST * amount, "Invalid Eth value sent!");

        lolitaFriendsToken.mint(amount, msg.sender);
        addressToMintCount[msg.sender] += amount;
    }

    function reserveTeamTokens(address _to, uint256 _reserveAmount) public onlyOwner {        
        require(_reserveAmount <= reservedTokens, "Not enough reserves");

        lolitaFriendsToken.mint(_reserveAmount, _to);
        reservedTokens = reservedTokens - _reserveAmount;
    }
...
// SPDX-License-Identifier: MIT
/// @title Interface for LolitaAndFriendsToken

pragma solidity 0.8.9;

import "@openzeppelin/contracts/token/ERC721/extensions/ERC721Enumerable.sol";

abstract contract ILolitaAndFriendsToken is ERC721Enumerable {
