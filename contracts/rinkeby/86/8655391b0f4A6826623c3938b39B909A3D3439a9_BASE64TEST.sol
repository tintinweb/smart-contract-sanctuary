...
contract BASE64TEST is ERC721URIStorage {
    uint256 public tokenCounter;
    event CreatedSVGNFT(uint256 indexed tokenId, string tokenURI);

    constructor() ERC721("BASE64 TEST", "BASE64") {
        tokenCounter = 0;
    }

    function create(string memory _data) public {
        _safeMint(msg.sender, tokenCounter);
        string memory imageURI = _data;
        ...
        return string(abi.encodePacked(
            ...
            Base64.encode(
                bytes(abi.encodePacked(
                    '{"name": "BASE64 TEST", "description": "testing different data formats encoded in base64 on OpenSea", "attributes": "", "image": "',
                    _imageURI,
                    '"}'
                    )
                )
