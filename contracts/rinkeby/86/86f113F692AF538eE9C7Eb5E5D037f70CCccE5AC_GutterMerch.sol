...
contract GutterMerch is ERC1155, Ownable, Pausable, ReentrancyGuard {
	...
	uint256 public maxSupply = 1000; //the max initial supply

	uint256 public itemPrice = 50000000000000000; //0.05 ETH

	//nftID = type + size. eg: 1 - shirt, size xxxl = 5 = 15

	event CAction(uint256 petID, uint256 value, uint256 actionID, string payload);
	event Redeemed(address indexed from, uint256 id, string uuid);

	string public _baseURI = "https://xxxxxx.s3.us-east-2.amazonaws.com/j/";
	...
	function mint(uint256 itemID) external payable nonReentrant whenNotPaused {
