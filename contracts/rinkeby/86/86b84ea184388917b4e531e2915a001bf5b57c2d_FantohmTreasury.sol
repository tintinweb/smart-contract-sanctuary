...
contract FantohmTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SHEC }
    ...
    address public sHEC;
    uint public sHECQueue; // Delays change to sHEC address
    ...
    /**
        @notice allow approved address to deposit an asset for HEC
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint HEC needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( HEC ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn HEC for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IHECERC20( HEC ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sHEC ).balanceOf( msg.sender ); // Can only borrow against sHEC held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with HEC
        @param _amount uint
     */
    function repayDebtWithHEC( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IHECERC20( HEC ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, HEC, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( HEC ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( HEC ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns HEC valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match HEC decimals
            value_ = _amount.mul( 10 ** IERC20( HEC ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SHEC ) { // 9
            sHECQueue = block.number.add( blocksNeededForQueue );
        } else return false;
