...
// Deployer will be the owner of this contract
contract NFTShopFactoryV1 is INFTShopFactoryV1, Ownable, FundManager {
    ...
    // in V1 one address can have one shop
    ...
    function ownerShopExists(address _shopOwner)
        ...
        returns (bool)
    {
        require(_shopOwner != address(0), "caller zero address");
    ...
    // destruct must be called in shop
...
interface INFTShopFactoryV1 {
    struct Shop {
        address owner;
        address nftAddr;
        address ftAddr; // zero address means bnb
        string uri;
    }
...
contract FundManager is IFundManager, ReentrancyGuard {
    ...
    uint256 public override feePercent = 4 * 10**18; // 4%
    ...
    receive() external payable {
        // forward if any funds received from fallback
    ...
    // distribute fee and remaining amount to respective addresses
    ...
    // function to transfer funds to wallet and dev a/c to respective percentages
    function _transferFees(
        address _from,
        uint256 _amount,
        address _ftAddr
    ) internal virtual {
        uint256 walletShare = (_amount * 96) / 100; // 96% -> wallet addr
        uint256 devShare = (_amount * 4) / 100; // 4%  -> dev addr

        _transferFunds(_from, wallet, walletShare, _ftAddr);
        _transferFunds(_from, dev, devShare, _ftAddr);
    }

    // transfers `_amount` from `_from` to `_to`
...
contract NFTShopV1 is INFTShopV1, ERC721URIStorage, FundManager {
    ...
    function buyNFT(uint256 _tokenId) public payable override isAllowed {
