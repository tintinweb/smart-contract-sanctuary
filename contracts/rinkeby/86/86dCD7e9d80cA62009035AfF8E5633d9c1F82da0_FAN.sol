...
/**
 * @dev The ERC20 FAN DAO token.
 */
contract FAN is OwnableUpgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable {
  using SafeERC20Upgradeable for ERC20VotesUpgradeable;

  /**
   * @dev `FAN` contract constructor that mints entire supply supply
   * for a given recipient.

   BOOKMARK set the token name three places in these two lines

   */
  function initialize(address recipient, uint256 totalSupply)
    public
    initializer
  {
    __ERC20_init("FAN", "FAN");
    __ERC20Permit_init("FAN");
