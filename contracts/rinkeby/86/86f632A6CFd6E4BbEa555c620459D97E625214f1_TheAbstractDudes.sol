...
contract TheAbstractDudes is ERC721Enumerable, Ownable {
  ...
  function claimableOf(address _owner) public view returns (uint256[] memory) {
    uint256[] memory tokenIds = theDudes.tokensOfOwner(_owner);
    uint256[] memory claimableTokenIds = new uint256[](tokenIds.length);
    uint256 index = 0;
    for (uint256 i = 0; i < tokenIds.length; i++) {
      uint256 tokenId = tokenIds[i];
      if (theDudes.ownerOf(tokenId) == _owner) {
        if (!claimedTokenIds[tokenId]) {
          claimableTokenIds[index] = tokenId;
          index++;
        }
      }
    }
