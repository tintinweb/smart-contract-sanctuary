...
contract HippyGhostsERC721 is OwnableUpgradeable, ERC721EnumerableUpgradeable {
    ...
    function _mintInSequenceToAddress(address destination, uint256 numberOfTokens) internal {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            ...
            while (_exists(_nextMintIndex)) {
                // skip tokenId minted in mintTokenWithSignature
                _nextMintIndex = _nextMintIndex + 1;
            }
    ...
    function mintTokenWithSignature(
        ...
        SignatureVerification.requireValidSignature(
            abi.encodePacked(msg.sender, tokenId, valueInWei, mintKey, this),
            signature,
            _verificationAddress
        );
        ...
        require(totalSupply() < maxSupply, "no available supply");
    ...
    function mintForPresale(address destination) external onlyOwner {
        require(totalSupply() == 0, "mintForPresale should be run at the beginning");
