...
contract CanWorkJob is Escrow {
    ...
    event OnCreateJob(address indexed dapp, bytes32 indexed jobId, address employer, address indexed provider, uint256 totalCosts);
    ...
    function createJob(bytes32 _jobId, address _employer, address _provider, uint256 _totalCosts) 
    public 
    returns (bool) {
        require(_jobId[0] != 0);
        ...
        jobs[_jobId].employer = _employer;
        jobs[_jobId].provider = _provider;
        jobs[_jobId].status = JobStatus.New;
        jobs[_jobId].amount = _totalCosts;
        jobs[_jobId].escrowId = createEscrow(_employer, _provider, _totalCosts);

        emit OnCreateJob(dApp, _jobId, _employer, _provider, _totalCosts);
...
contract Escrow is Migratable {
    ...
    function createEscrow(address _employer, address _provider, uint256 _amount) 
    internal 
    returns (uint256) {
        require(_employer != address(0) && _provider != address(0) && _amount > 0, "Must be a valid addresses and non zero amounts");
        ...
        emit OnCreateEscrow(escrowDapp, _employer, _provider, _amount, daiValueInEscrow);
