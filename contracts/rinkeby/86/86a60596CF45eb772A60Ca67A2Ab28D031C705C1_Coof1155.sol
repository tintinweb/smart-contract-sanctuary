  ...
  function _generateSVGDefs() private view returns (string memory svgDef) {
    ...
    for(uint256 i = 0; i < 6; ++i){
      ...
      subDefs = string(abi.encodePacked(
        ...
        subIndex.toString(),
        '" >',
        '<animate attributeName="d" values="m240,230l0,0 0,0;m190,190l30,-17 30,17;m110,60l0,0 0,0;" dur="12s" repeatCount="indefinite" begin="p1.begin+',
        (subIndex * 2).toString(),
        's"/>',
    ...
    for(uint256 r = 0; r < 6; ++r){
      for(uint256 i = 0; i < 11; ++i){
        ...
        if((i+1) % 2 != 0){
          ...
          //((i+1) % 2 != 0) ? '<use xlink:href="#r1" filter="url(#f1)"/>' : ''
        }else{
          last = string(abi.encodePacked(
            '<use xlink:href="#r',
            r.toString(),
            '" transform="rotate(330 160 160)" filter="url(#f1)"/>'
          ));
        }
