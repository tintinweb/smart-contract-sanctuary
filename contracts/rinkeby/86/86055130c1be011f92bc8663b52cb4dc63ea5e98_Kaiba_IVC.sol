...
contract Kaiba_IVC {
    ...
    struct SVTLiquidity {  // This struct defines the types of liquidity and the relative properties
        ...
        uint256 liq_mode; // 1: Direct pair, 2: LP token (on token_1), 3: Synthetic
    ...
    function get_svt_liquidity(uint256 svt_id) public view returns (bool, bool, address, address, uint256, uint256, uint256, address, address) {
    ...
    function operate_issue_svt(uint256 qty, string memory name, string memory ticker, uint256[] memory taxes) public returns (uint256) {
        require(kaiba.balanceOf(msg.sender) > 1000000000000);
        kaiba.approve(address(this), 1000000000000);
        kaiba.transferFrom(msg.sender, address(this), 1000000000000);

        uint256 newAddress = svt_last_id+1;
        SVT_address[newAddress].name = name;
        SVT_address[newAddress].ticker = ticker;
        SVT_address[newAddress].totalSupply = qty;
        SVT_address[newAddress].circulatingSupply = qty;
        SVT_address[newAddress].fees = taxes;
        SVT_address[newAddress].tokenOwner = msg.sender;
        SVT_address[newAddress].balance[msg.sender] = qty;
    ...
    function operate_svt_to_native(uint256 svt_id, uint256 qty, address receiver, bool simulation) public returns (uint256) {
    ...
    function operate_native_to_svt(uint256 svt_id, address receiver, uint256 qty, bool simulation) public returns (uint256){
        /// @notice Sanity checks
        require(SVT_address[svt_id].deployed, "SVT token does not exist");
        uint256 liq_index = SVT_address[svt_id].SVT_Liquidity_storage;
        require(SVT_Liquidity_index[liq_index].active, "SVT liquidity does not exist");
    ...
    function operate_svt_to_svt(uint256 svt_id_1, uint256 svt_id_2, uint256 qty, address receiver) public {
        require(get_svt_address_balance(msg.sender, svt_id_1) >= qty, "Balance is too low");
        require(SVT_address[svt_id_2].deployed, "SVT 2 does not exist");
    }
    
    function operate_svt_to_kaiba(uint256 svt_id, uint256 qty, address receiver) public {
        require(get_svt_address_balance(msg.sender, svt_id) >= qty, "Balance is too low");
    }

    function operate_kaiba_to_svt(uint256 svt_id, uint256 qty, address receiver) public {
        require(kaiba.balanceOf(msg.sender) >= qty, "Not enough Kaiba");
    }

    /////////////////////////////////////////////////////// Utility functions

    function operate_ivc_get_amount_out(uint256 to_deposit, uint256 to_deposit_liq, uint256 to_withdraw_liq) internal pure returns (uint256 out_qty) {
    ...
    function create_svt_pair(address to_bridge, address to_pair,  uint256 qty_1, uint256 qty_2) public onlyAuth(to_bridge) {
        ...
        from_token.approve(address(this), qty_1);
        from_pair.approve(address(this), qty_2);
    ...
    /// @param toUnBridge ERC20 address
    /// @param qty how much to unbridge

    function _unbridgeToken(address toUnBridge, uint256 qty) public {
        ERC20 from_token = ERC20(toUnBridge);
        // Security and uniqueness checks
        require(imported[toUnBridge], "This token is not imported");
        uint256 unbridge_id = imported_id[toUnBridge];
        ...
        from_token.approve(address(this), qty);
        from_token.transferFrom(address(this), msg.sender, qty);
