...
contract DemoTradeMarket is Ownable {
    ...
    function buyToken(
        ...
        address tokenOwner = _seller;
        // Encode isApprovedForAll Function and parameters
        bytes memory data =
        ...
        bytes memory result =
            AddressUtils.functionDelegateCall(_originalContractAddress, data);
        // Check if the Trade market is operator
        bool isOperator = abi.decode(result, (bool));
        ...
        // Delegate Call
        AddressUtils.functionDelegateCall(
            _originalContractAddress,
            transferData
        );
        emit LogPutItemOnSale(_buyer, _seller, _token, _price);
        // Returns true
        return true;
    }
    ...
    ) external view returns (bytes memory success) {
        bytes memory data =
            abi.encodeWithSignature(
