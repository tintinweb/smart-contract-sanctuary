...
contract TestNFT is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  uint256 public constant MINT_PRICE_PER_TOKEN = 0.1 ether;
  ...
  {
    uint256 currentTotalSupply = totalSupply();
    require(currentTotalSupply + numberOfTokens <= MAX_SUPPLY, "Mint amount would exceed max token supply");
    require(isSaleActive, "Sale is not active");
    require(numberOfTokens <= MAX_MINT_AMOUNT, "This exceeds the max mint amount");
    require(
      MINT_PRICE_PER_TOKEN * numberOfTokens <= msg.value,
      "Invalid ETH value"
    );
