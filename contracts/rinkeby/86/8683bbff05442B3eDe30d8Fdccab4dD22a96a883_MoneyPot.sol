...
contract MoneyPot is BlockReentrancyGuard {
    ...
    constructor (address _sDFIANCE, address _rewardToken, address _feeManager, address _papr, uint256 _MAX_PAPER_AMOUNT_PER_SHARE_PERCENT, uint256 _BONUS_SHARE_PER_PAPER) public {
        sDFIANCE = _sDFIANCE;
    ...
    */
    function getUpdateMilestone(address _account) public view returns (bool, uint) {
        ...
        for (uint i=0; i < milestones.length; i++) {
            if (holdingTime >= milestones[i].minimumHoldingHours) { // if the milestone [i] is low enough for the user ..
                if (userPercentBonus < milestones[i].percentBonus) {
                    _a = true;
                    _b = milestones[i].percentBonus; // set milestone [i] to his
                }
            }
        }
    ...
    function getRealMilestone(address _account) public view returns (bool, uint) {
        uint256 holdingTime = getAvgSwapTimeInHours(_account);
        bool _a;
        uint _b;
    ...
    function _updateMilestone(address _account) private {
        ...
        if (newShareHoldingBonus != usersShareBonusFromHoldingTime[_account]) {
            uint256 newTotalAmount = userInfo[_account].amount.sub(usersShareBonusFromHoldingTime[_account]).add(newShareHoldingBonus);
    ...
    */
    function updatePaprBonusShare(address _account) external {
        ...
        if (newSharePaprBonus != usersShareBonusFromPapr[_account]) {
            uint256 newTotalAmount = userInfo[_account].amount.sub(usersShareBonusFromPapr[_account]).add(newSharePaprBonus);
