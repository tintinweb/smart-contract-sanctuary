...
contract FlokiBull is Context, IERC20 {
    ...
    bool private allowedPresaleExclusion = false;
    ...
    //Hourly fee Added by Alpha
    uint256 public firstHourFeeMultiplier;
    uint256 public oneToFiveHourFeeMultiplier;
    uint256 public fiveToTwentyFourHourFeeMultiplier;
    mapping (address=>uint256) public buyTimestampOfAddress;
    address public ammContractAddress;
    ...
    // PCS ROUTER
    address private _routerAddress = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;//0x10ED43C718714eb63d5aA57B78B54704E256024E;

    address public burnAddress = 0x000000000000000000000000000000000000dEaD;
    address payable private _marketingWallet = payable(0xE7e68B6fC21d683DEcbC33d7Be297aD148F0248a);
...
//===============================================================================================================
    ...
    function setFirstHourFeeMultiplier(uint256 mult) public onlyOwner{
        firstHourFeeMultiplier=mult;
    }
    function setOneToFiveHourFeeMultiplier(uint256 mult) public onlyOwner{
        oneToFiveHourFeeMultiplier=mult;
    }
    function setFiveToTwentyFourHourFeeMultiplier(uint256 mult) public onlyOwner{
        fiveToTwentyFourHourFeeMultiplier = mult;
    }
...
//===============================================================================================================
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        uint256 hourlySellFeeMultiplier=uint256(1);
        if (from==ammContractAddress){
            buyTimestampOfAddress[to]=block.timestamp;
        }
        if (to==ammContractAddress){
            uint256 timeElapsed=block.timestamp  - buyTimestampOfAddress[from];
            if (timeElapsed < 1 hours){
                hourlySellFeeMultiplier=firstHourFeeMultiplier;
            }
            else if (timeElapsed>= 1 hours && timeElapsed < 5 hours){
                hourlySellFeeMultiplier = oneToFiveHourFeeMultiplier;
            }
            else if( timeElapsed>=5 hours && timeElapsed<24 hours){
                hourlySellFeeMultiplier=fiveToTwentyFourHourFeeMultiplier;
            }
        }
        ...
        return _finalizeTransfer(from, to, amount, takeFee, hourlySellFeeMultiplier);
    }
    ...
    function _finalizeTransfer(address from, address to, uint256 tAmount, bool takeFee, uint256 hourlySellFeeMultiplier) private returns (bool) {
        if (sniperProtection){
        ...
        ExtraValues memory values = _getValues(tAmount, takeFee, hourlySellFeeMultiplier);
    ...
    function _getValues(uint256 tAmount, bool takeFee, uint256 hourlySellFeeMultiplier) private view returns (ExtraValues memory) {
        ...
        if(takeFee) {
            ...
            values.rFee = values.tFee * currentRate * hourlySellFeeMultiplier;
        } else {
            values.tFee = 0;
            values.tLiquidity = 0;
