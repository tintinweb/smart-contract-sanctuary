...
contract HoneyFarmQueen is Ownable, ReentrancyGuard {
    ...
    uint256 public REWARDS_COOLDOWN = 86400; //one day

    uint256 public HIVE_CLAIM_COOLDOWN = 86400; //one day
    ...
    function claimBearsHoney(uint16[] calldata _bearsIds) external nonReentrant {
        ...
        for (uint16 i = 0; i < _bearsIds.length; i++) {
            ...
            if (lastReward > 0 && lastReward > STARTING_POINT) {
                ...
                epochsToReward = (block.timestamp - lastReward) / REWARDS_COOLDOWN;
            } else {
                // if no rewards claimed so far, then he gets rewards from when the rewards started.
                epochsToReward = (block.timestamp - STARTING_POINT) / REWARDS_COOLDOWN;
            }
    ...
    function claimBeesHoney(uint16[] calldata _beesIds) external nonReentrant {
        ...
        for (uint16 i = 0; i < _beesIds.length; i++) {
            ...
            if (lastReward > 0 && lastReward > STARTING_POINT) {
                // solhint-disable-next-line
                //we get whole numbers for example if someone claims after 1 round and a half, he should be rewarded for 1 round.
                epochsToReward = uint16((block.timestamp - lastReward) / REWARDS_COOLDOWN);
            } else {
                // if no rewards claimed so far, then he gets rewards from when the rewards started.
                epochsToReward = uint16((block.timestamp - STARTING_POINT) / REWARDS_COOLDOWN);
            }
    ...
    function mintBee(uint16 _hiveId) external nonReentrant {
        ...
        require(lastClaimedBeePerHive[_hiveId] < block.timestamp - HIVE_CLAIM_COOLDOWN, "Mint bee cooldown");
    ...
    function setHoneyRewardCoolDown(uint256 _cooldown) external onlyOwner {
        REWARDS_COOLDOWN = _cooldown;
    }

    function setHiveClaimCoolDown(uint256 _cooldown) external onlyOwner {
        HIVE_CLAIM_COOLDOWN = _cooldown;
    }
