...
contract USDTTEST is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 10_000_000_000_000 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = 'USDT Test';
    string private _symbol = 'USDTTEST';
    uint8 private _decimals = 9;

    uint256 private _taxFee = 0; // 0% reflection fee for every holder
    ...
    function _setMarketingFee(uint256 marketingFee) external onlyOwner() {
        require(marketingFee >= 1 && marketingFee <= 99, 'marketingFee should be in 1 - 11');
        _marketingFee = marketingFee;
    }

    function _setLiquidityFee(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee >= 1 && liquidityFee <= 99, 'liquidityFee should be in 1 - 11');
