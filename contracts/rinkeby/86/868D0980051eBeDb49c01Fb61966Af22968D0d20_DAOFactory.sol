...
contract ACL is Initializable {
    ...
    function grant(
        ...
        address _who,
        bytes32 _role
    ) external auth(address(this), ROOT_ROLE) {
    ...
    function freeze(address _where, bytes32 _role)
        external
        auth(address(this), ROOT_ROLE)
    {
        ...
        auth(address(this), ROOT_ROLE)
    {
        for (uint256 i = 0; i < items.length; i++) {
            ...
            if (item.op == ACLData.BulkOp.Grant)
                _grant(_where, item.who, item.role);
            else if (item.op == ACLData.BulkOp.Revoke)
                _revoke(_where, item.who, item.role);
            else if (item.op == ACLData.BulkOp.Freeze)
                _freeze(_where, item.role);
        }
    }
    ...
    function isFrozen(address _where, bytes32 _role)
        public
        view
        returns (bool)
    {
    ...
    function _checkRole(
        ...
        address accessFlagOrAclOracle = authPermissions[
            permissionHash(_where, _who, _role)
        ];
    ...
    function freezeHash(address _where, bytes32 _role)
        internal
        pure
        returns (bytes32)
    {
...
abstract contract GovernancePrimitive is Component {
    bytes32 public constant CREATE_PRIMITIVE_START_ROLE =
        keccak256('CREATE_PRIMITIVE_START_ROLE');
    bytes32 public constant PRIMITIVE_EXECUTE_ROLE =
        keccak256('PRIMITIVE_EXECUTE_ROLE');

    string internal constant ERROR_EXECUTION_STATE_WRONG =
        'ERROR_EXECUTION_STATE_WRONG';
    string internal constant ERROR_NOT_ALLOWED_TO_EXECUTE =
        'ERROR_NOT_ALLOWED_TO_EXECUTE';
    string internal constant ERROR_NOT_ALLOWED_TO_START =
    ...
    struct Execution {
        // A execution contains the process to execute, the proposal passed by the user, and the state of the execution.
        uint256 id;
    ...
    event GovernancePrimitiveStarted(
        uint256 indexed executionId,
        Execution execution
    );
    event GovernancePrimitiveExecuted(
        uint256 indexed executionId,
        Execution execution
    );
    ...
    function start(
        Processes.Process calldata process,
        ...
        returns (uint256 executionId)
    {
        ...
        emit GovernancePrimitiveStarted(executionId, execution);
    ...
    function execute(uint256 executionId)
        public
        executionExist(executionId)
        authP(PRIMITIVE_EXECUTE_ROLE)
    {
        ...
        emit GovernancePrimitiveExecuted(executionId, execution);
    }

    /// @dev Internal helper and abstraction to get a execution struct.
    /// @param executionId The id of the execution struct.
    function _getExecution(uint256 executionId)
        internal
        view
        returns (Execution storage execution)
    {
...
abstract contract VotingGovernancePrimitive is GovernancePrimitive {
    event VotedOnGovernancePrimitive(Execution execution, bytes data, uint256 executionId);
...
contract DAO is IDAO, Initializable, UUPSUpgradeable, ACL {
    ...
    // TODO: perhaps NewProposal need some indexed such as porcess.name
    event NewProposal(GovernancePrimitive.Proposal proposal, Processes.Process process, address submitter, uint256 executionId);
...
contract Permissions is UpgradableComponent {
    ...
    event RoleSet(string indexed role, Permission permission);
...
contract Processes is UpgradableComponent {
    bytes32 public constant PROCESSES_START_ROLE =
        keccak256('PROCESSES_START_ROLE');
    bytes32 public constant PROCESSES_SET_ROLE =
        keccak256('PROCESSES_SET_ROLE');

    event ProcessStarted(
        uint256 indexed executionId,
        GovernancePrimitive.Proposal proposal
    );
    event ProcessSet(string name, Process process);
        ...
        )
    {
        ...
        // require(
        //     checkActions(proposal.actions, process.allowedActions),
        //     'Not allowed action!'
        // );

        executionId = GovernancePrimitive(process.governancePrimitive).start(
            process,
            proposal
        );

        emit ProcessStarted(executionId, proposal);
    ...
    function checkActions(
        Executor.Action[] calldata actions,
        AllowedActions[] memory allowedActions
        ...
        for (uint256 i = 0; i < actionsLength; i++) {
            // FOR EVERY PROPOSAL ACTION
            Executor.Action calldata action = actions[i];
            for (uint256 k = 0; k < allowedActionsLength; k++) {
                // FOR EVERY ALLOWED CONTRACT
                AllowedActions memory allowedAction = allowedActions[k];
                if (action.to == allowedAction.to) {
                    // CONTRACT MATCHED
                    uint256 methodsLength = allowedAction.methods.length;
                    for (uint256 y = 0; y < methodsLength; y++) {
                        // CHECK FOR EVERY ALLOWD METHOD OF A CONTRACT
                        if (
                            bytes4(action.data[:4]) == allowedAction.methods[y]
                        ) {
                            // METHOD FOUND STOP SEARCHING
                            allowed = true;
                            break;
                        } else {
                            // METHOD NOT FOUND
