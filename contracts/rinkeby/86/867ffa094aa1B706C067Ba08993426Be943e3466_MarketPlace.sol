...
contract ENCNFT is ERC721, Ownable {
    ...
    function transferTokenWithEncryption(
        ...
        safeTransferFrom(ownerToken, _to, _tokenId, _data);
...
contract MarketPlace is Ownable {
    ...
    ENCNFT nftExample = ENCNFT(
        0x017B48535979B015687B26E37c9361145CB19147
        );
    struct BuyerData {
    ...
    function makeBet(uint256 idToken, string memory buyerPublicKey, address buyerAddress) public payable {
        require(msg.value != 0, "Rate must be more 0");
         uint256 buyerIndex = getBuyerIndex(idToken, buyerAddress);
         if(buyersBoard[idToken].length > 0 && buyersBoard[idToken][buyerIndex].buyerAddress == buyerAddress) {
    ...
    function getBuyersById(uint256 idToken) public view  returns(BuyerData[] memory){
    ...
    function acceptRateAndTransferToken(uint256 idToken, address buyerAddress,  bytes memory data) public {
        uint256 buyerIndex = getBuyerIndex(idToken, buyerAddress);
        uint256 sellerIndex = getSellerIndex(idToken, msg.sender);
        nftExample.transferTokenWithEncryption(buyerAddress, idToken, data, msg.sender);
