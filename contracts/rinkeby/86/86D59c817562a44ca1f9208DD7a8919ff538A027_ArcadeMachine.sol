...
contract ArcadeMachine is DisplayURISwitchable, ERC721Enumerable, ReentrancyGuard, ERC2981, Ownable {
    ...
    constructor(address arcadeOfficialAddress) ERC721("ArcMa2", "ARC-MA2") {
    ...
    function devMintOriginalBatch(
        ...
        onlyOperator
    {
        require(targets.length == tokenIds.length);

        for(uint256 i = 0; i < targets.length; i++) {
            _devMintOriginal(targets[i], tokenIds[i], metadataURIs[i], modes[i]);
        }
    }

    function _devMintOriginal(
        address targetAddress,
        uint256[] memory arcadeTokenIds,
        string[] memory imageMetadataURIs,
        bool[] memory modes
    )
        private
    {
    ...
    function devMintOriginal(
        ...
        onlyOperator
    {
        _devMintOriginal(targetAddress, arcadeTokenIds, imageMetadataURIs, modes);
    }
    ...
    function setImageMetadataURI(uint256 tokenId, string memory metadataURI) public {
...
contract Arcade is DisplayURISwitchable, ERC721Enumerable, AccessControl, Ownable {
    ...
    constructor(uint256 maxPublic, uint256 maxTemporaryPublic, uint256 maxReserved, uint256 startingReservedID, uint256 maxPerAddress, address[] memory whitelistAddresses) ERC721("Arc23", "ARC23") {
