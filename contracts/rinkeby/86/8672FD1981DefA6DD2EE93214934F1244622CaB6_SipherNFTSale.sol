...
contract SipherNFTSale is ISipherNFTSale, Whitelist {
  ...
  constructor(
    ...
    uint64 _publicTime,
    uint64 _endTime,
    uint64 _autionDuration,
    uint64 _autionTimeInterval,
    uint64 _maxSupply
  ...
  )   
  {
    ...
    _saleConfig = SaleConfig({
      ...
      endTime: _endTime,
      autionDuration: _autionDuration,
      autionTimeInterval:_autionTimeInterval,
  ...
  function getElapsedSaleTime() internal view returns (uint256) {
    return 
      _saleConfig.autionDuration + _saleConfig.publicTime  > block.timestamp
      ? block.timestamp - _saleConfig.publicTime 
      : 0;
  }

  function getCurrentPrice() public view returns (uint256) {
    uint256 elapsed = getElapsedSaleTime();
    if (elapsed >= _saleConfig.autionDuration) {
        return SALE_BASE_PRICE;
    } else {
      uint256 currentPrice = ((_saleConfig.autionDuration / _saleConfig.autionTimeInterval) - (elapsed / _saleConfig.autionTimeInterval)) *((SALE_PUBLIC_STARTING_PRICE - SALE_BASE_PRICE) * _saleConfig.autionTimeInterval / _saleConfig.autionDuration);
      return
  ...
  *       - can buy up to MAX_OWNER_BOUGHT_INITIAL before endTime with price = 0
  ...
  function _validateAndUpdateWithBuyAmount(address buyer, uint64 amount, bytes32[] memory proofs) internal {
    ...
    if (config.whitelistTime <= timestamp && timestamp < config.publicTime) {
      ...
      require(totalPaid == amount * currentPrice, 'SipherNFTSale: invalid paid value');
      require(
...
interface ISipherNFTSale {
  struct SaleConfig {
    ...
    uint64 autionDuration;      // duration of the aution   
    uint64 autionTimeInterval;  // the time interval between the price levels
  }
