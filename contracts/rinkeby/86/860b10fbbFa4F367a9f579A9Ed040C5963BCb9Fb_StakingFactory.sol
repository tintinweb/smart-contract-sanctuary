...
contract Staking {
    ...
    function calculateRewardsView(address _wallet, uint256 _noOfTokens) external view returns (uint256) {
        uint256 userShareInPool = (_noOfTokens.mul(100)).div(stakingPool);
        return StakeHolders[_wallet].rewards.add((userShareInPool.mul(stakingPoolRewards)).div(100));
    }
    ...
    function withdraw(uint256 _noOfTokens) external onlyManager {
        require(FXFTOKENADDRESS != stakeToken, "Withdraw:: Can not withdraw FXF tokens");
