...
contract WSBToken is ERC721Enumerable, IERC721Receiver, ReentrancyGuard, Ownable {
  ...
  uint16 public constant MAX_RAFFLE = 30000;
  uint16 public constant MAX_TOKEN = 14900;
  uint16 public constant MAX_DEV_CLAIM = 15000;
  ...
  function claimRaffle(uint8 amount) external nonReentrant {
    ...
    require(address(this).balance >= PRESALE_PRICE.mul(amount), "ClaimRaffle: Insufficient funds for refund.");
    uint8 balance = uint8(presaleContract.balanceOf(msg.sender)); // save gas
    require(balance > 0, "ClaimRaffle: Need pre-mint to claim.");
    amount = amount < balance ? amount : balance; // is amount editable? save some gas
  ...
  // to deploy when MAX_RAFFLE < MAX_TOKEN
  function claimToken(uint256 amount) external payable nonReentrant {
  ...
  function swapFrom(address owner, uint256 tokenId) external onlyOwner {
    require(tx.origin == msg.sender, "Swap: Can not be called using a contract.");
    require(swapActive, "Swap: Swap is not active.");
    require(ERC721.ownerOf(tokenId) == address(this), "Swap: This token is not swappable.");
