...
contract AWToken is Context, IERC20Metadata, Ownable { 
     ...
     */
    function newBattleFAtest(uint16 _fromId, uint16 _toId) internal returns(bool){
     ...
     */
    function battleTest(uint16 _fromId, uint nonce) public returns (bool){
        ...
        bool battleResult;
        if (battleWithAddons)
            battleResult = newBattleFAtest(_fromId, _toId);
        else
     ...
     */
    function battleAlltest(uint16[] memory tokenIds) public {
        uint _toMint;
        bool outcome;
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            {   
                if(nftBattleCount[tokenIndex] <  IFA(_nftAddress).getStamina(tokenIndex))
                {
                    ...
                    {    
                        outcome = battleTest(tokenIndex, k);
                        if (outcome)
                            _toMint += WIN_REWARD;
                        else
                            _toMint += LOSE_REWARD;
