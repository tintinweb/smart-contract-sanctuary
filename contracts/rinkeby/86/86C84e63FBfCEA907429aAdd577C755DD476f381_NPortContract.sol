...
contract NPortContract is  ERC721PresetMinterPauserAutoId,Ownable, EIP712, RoyaltiesV2Impl {
...
event RedeemAndMint(uint256 _tokenId, address _from, address _to, uint256 _soldPrice, uint256 _nportCut, string _tokenUri);
    ...
    external payable
    {
        ...
        require(msg.sender != signer, "May not mint your own token");
        ...
        require(unlistedSignatures[signature] == false, "Signature has expired");
        //mint can only be called from within this contract
        mint(signer, tokenuri);
        //$25
        //Add to used signatures so this signature can't be reused
        //$11
        IERC721 tokenContract = IERC721(address(this));

        //tokenContract.approve(msg.sender,_tokenIdTracker.current() - 1);

        //need to do current -1 becaues mint increments the counter
        uint256 countBeforeMint = _tokenIdTracker.current() - 1;
        tokenContract.safeTransferFrom(signer, msg.sender,countBeforeMint);
        ...
        //$25
        _internalSetRoyalties(countBeforeMint, payable(signer), 1000);
        ...
        //$13
        //Shortening the URI will cause this to be cheaper
        unlistedSignatures[signature] = true;

        //$2
        emit RedeemAndMint(countBeforeMint,signer,msg.sender, msg.value, cut, tokenuri);
...
event Unlist(bytes _signature, address _from);
    ...
    external
    {
        ...
        require(signer == msg.sender, "Must own token to unlist");
        //require signer is owner of token id
        unlistedSignatures[signature] = true;


        emit Unlist(signature,msg.sender);
    ...
    function powerMint(address signer,string memory tokenuri)public onlyOwner{
    ...
    function mint(address to, string memory tokenuri) private {
