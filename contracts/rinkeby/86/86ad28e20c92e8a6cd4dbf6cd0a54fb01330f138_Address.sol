...
// CRYPTO DINOS - NOW YOU REACHED THE INTERESTING PART OF THE CODE.
// It's been a journey but we are here, enjoy!


pragma solidity ^0.8.0;

contract CryptoDinos is Ownable, ERC721Enumerable, ReentrancyGuard {
  ...
  uint256 private _price = 0.05 ether; // 30000000000000000
  
  string public punkcontractURI;
  
  constructor() ERC721("CryptoDinos", "DINOS") {}

  // for wd
  address oaf = 0x3d1C6628DFc1Ca40B26751150D18675d13016573; 
  address quack = 0x092Ef31f64Bb62cABE00036c2362DbafB47E53a5;

  uint256[10000] private _availableTokens;
  uint256 private _numAvailableTokens = 10000;
  uint256 private _numFreeRollsGiven = 0;

  mapping(address => uint256) public freeRollDinos;

  uint256 private _lastTokenIdMintedInInitialSet = 10000;

  function numTotalDinos() public view virtual returns (uint256) {
    return 10000;
  }

  function freeRollMint() public nonReentrant() {
    require(freeRollDinos[msg.sender] > 0, "You don't have any free rolls!");
    uint256 toMint = freeRollDinos[msg.sender];
    freeRollDinos[msg.sender] = 0;
    uint256 remaining = numTotalDinos() - totalSupply();
    if (toMint > remaining) {
      toMint = remaining;
    }
    _mint(toMint);
  ...
  function getNumFreeRollDinos(address owner) public view returns (uint256) {
    return freeRollDinos[owner];
  }
  ...
  function mint(uint256 _numToMint) public payable nonReentrant() {
    ...
    require(
      totalSupply + _numToMint <= numTotalDinos(),
      "There aren't this many dinos left."
    );
    uint256 costForMintingDinos = _price * _numToMint;
    require(
      msg.value >= costForMintingDinos,
      "Too little sent, please send more eth."
    );
    if (msg.value > costForMintingDinos) {
      payable(msg.sender).transfer(msg.value - costForMintingDinos);
    }
  ...
  function getCostForMintingDinos(uint256 _numToMint)
    public
    view
    returns (uint256)
  {
    require(
      totalSupply() + _numToMint <= numTotalDinos(),
      "There are not this many dinos left."
    );
    require(
      _numToMint <= MAX_MINTABLE_AT_ONCE,
      "You cannot mint that many dinos."
    );
    return _numToMint * _price;  
  }

  function getDinosBelongingToOwner(address _owner)
    external
    view
    returns (uint256[] memory)
  {
    uint256 numDinos = balanceOf(_owner);
    if (numDinos == 0) {
      return new uint256[](0);
    } else {
      uint256[] memory result = new uint256[](numDinos);
      for (uint256 i = 0; i < numDinos; i++) {
  ...
  function giveFreeRoll(address receiver, uint256 numRolls) public onlyOwner {
    ...
    require(freeRollDinos[receiver] + numRolls < 21, "Cannot exceed 20 unused free rolls!");
    uint256 freeRolls = freeRollDinos[receiver];
    freeRollDinos[receiver] = freeRolls + numRolls;
