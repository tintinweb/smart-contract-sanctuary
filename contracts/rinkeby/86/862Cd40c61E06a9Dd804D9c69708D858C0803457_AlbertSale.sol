...
import {IAlbertKit} from '../interface/IAlbertKit.sol';
import {INFTSale} from '../interface/INFTSale.sol';
import {Whitelist} from './Whitelist.sol';


contract AlbertSale is INFTSale, Whitelist {

  using Address for address;

  // at initial launch, the owner can buy up to 299 tokens
  uint64 public constant MAX_OWNER_BOUGHT_INITIAL = 299;
  uint64 public constant CAP_PER_WHITELISTED_ADDRESS = 5;
  uint64 public constant CAP_PER_ADDRESS = 5;
  uint256 public constant SALE_PRICE = 5e16; // 0.05 ether
  address public treasury;

//   bytes32 public override merkleRoot; // store the merkle root data for verification purpose

  IAlbertKit internal nft;
...
//   event SetMerkleRoot(bytes32 merkelRoot);
  ...
  function withdrawSaleFunds(address payable recipient, uint256 amount) external onlyOwner {
    ...
    require(success, 'AlbertNFTSale: withdraw funds failed');
  }
...
//       'AlbertNFTSale: only update before whitelist buy time'
//     );
//     require(_root != bytes32(0), 'AlbertNFTSale: invalid root');
//     require(merkleRoot == bytes32(0), 'AlbertNFTSale: already set merkle root');
//     merkleRoot = _root;
...
//   }
  ...
  function buy(uint64 amount) external payable override {
    ...
    require(!buyer.isContract() || buyer == owner(), 'AlbertNFTSale: only EOA or owner');
    // require(merkleRoot != bytes32(0), 'AlbertNFTSale: merkle root is not set yet');

    _validateAndUpdateWithBuyAmount(buyer, amount);

    nft.mint(amount, buyer);
  }
...
//     require(_blockTimestamp() > _saleConfig.endTime, 'AlbertNFTSale: sale not ended');

//     address sender = msg.sender;
//     require(!sender.isContract() || sender == owner(), 'AlbertNFTSale: only EOA or owner');
...
//   }
  ...
  /**
   * @dev Update public sale time by the owner only
   */
  function updatePublicTime(uint64 _publicTime) external onlyOwner {
  ...
  *       - can buy up to MAX_OWNER_BOUGHT_INITIAL before endTime with price = 0
  ...
  function _validateAndUpdateWithBuyAmount(address buyer, uint64 amount) internal {
    ...
    require(
      ...
      'AlbertNFTSale: max supply reached'
    );
    ...
    if (buyer == owner) {
      ...
      if (timestamp <= config.endTime) {
        require(
          ...
          'AlbertNFTSale: max owner initial reached'
        );
      }
    ...
    require(
      _saleRecord.totalSold + amount <= _saleConfig.maxSupply - MAX_OWNER_BOUGHT_INITIAL,
      'AlbertNFTSale: max supply reached'
    );
    require(config.whitelistTime <= timestamp, 'AlbertNFTSale: not started');
    require(timestamp <= config.endTime, 'AlbertNFTSale: already ended');

    if (config.whitelistTime <= timestamp && timestamp < config.publicTime) {
      // only whitelisted can buy at this period
      require(isWhitelistedAddress(buyer), 'AlbertNFTSale: only whitelisted buyer');
      // whitelisted address can buy up to CAP_PER_WHITELISTED_ADDRESS token
      require(totalPaid >= amount * SALE_PRICE, 'AlbertNFTSale: invalid paid value');
      require(
        _userRecord[buyer].whitelistBought + amount <= CAP_PER_WHITELISTED_ADDRESS,
        'AlbertNFTSale: whitelisted cap reached'
      );

      //never leave money in the contract
      payable(treasury).transfer(totalPaid);
    ...
    if (config.publicTime <= timestamp && timestamp < config.endTime) {
      ...
      require(
        ...
        _userRecord[buyer].publicBought  + amount <= CAP_PER_ADDRESS,
        'AlbertNFTSale: normal cap reached'
      );
      //never leave money in the contract
      payable(treasury).transfer(totalPaid);
...
interface IAlbertKit is IERC721Enumerable {
//   /**
...
//   function rollStartIndex() external;
  ...
  /**
   * @dev Call to mint new tokens, only by Sales Minter
   *  Can mint up to MAX_SUPPLY tokens
...
// import {IAlbertKit} from './IAlbertKit.sol';
...
interface INFTSale {
  ...
  // function nft() external view returns (IAlbertKit);
}
