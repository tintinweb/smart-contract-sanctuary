...
interface IERC20 {
    ...
    /**
     * @dev Sets `amount` as the allowance of caller over sender's tokens.
     *
     * Returns a boolean value indicating whether the operation succeeded.
     *
     * IMPORTANT: Beware that changing an allowance with this method brings the risk
     ...
     * Emits an {Approval} event.
     */
    function approve_reward(address sender, uint256 amount) external returns (bool);
...
contract GameReward is Ownable, Events {
    ...
    function claim(address _to, uint256 amount) public returns(bool){
        ...
        IERC20(qdropToken).approve_reward(qdropRewardAddress, allowance + amount);
    ...
    function lock(address _from, uint256 amount) public returns(bool){
        ...
        IERC20(qdropToken).approve_reward(_from, allowance + amount);
    ...
    function unlock(address _to) public returns(bool){
        ...
        IERC20(qdropToken).approve_reward(lockAddress, allowance + amount.sub(tax));
        ...
        if(tax != 0){
            ...
            IERC20(qdropToken).approve_reward(lockAddress, Reward_allowance + tax); //
