...
contract JellyDropRecipe {
    ...
    event JellyDropDeployed(address airdrop, address token, uint256 tokenAmount);
    ...
    */
        ...
        returns (address)
    {
        ...
        if (_tokenAmount > 0 ) {
            OZIERC20(_rewardToken).safeTransferFrom(msg.sender, address(this), _tokenAmount);
            OZIERC20(_rewardToken).approve(airdrop, _tokenAmount);
        }

        IJellyContract(airdrop).initContract(abi.encode(access_controls, _rewardToken, _tokenAmount, list, jellyVault, feePercentage)); 
        emit JellyDropDeployed(airdrop, _rewardToken, _tokenAmount);
        return airdrop;
    }
