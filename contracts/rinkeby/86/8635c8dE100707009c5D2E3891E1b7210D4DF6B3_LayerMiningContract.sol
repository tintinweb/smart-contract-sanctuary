...
contract LayerMiningContract is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    IERC20 public layerToken; //  layer TOKEN
    ...
    constructor(address _mining_token, uint256 _mining_start_block) public {
        layerToken = IERC20(_mining_token);
        ...
        uint256 phase1TokenMountPerDay = 761904 * 1e18; // phase1: 761904 LAYER per day
    ...
    function add(string memory _poolName, uint256 _allocPoint, address _lpToken, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function safeMiningTokenTransfer(address _to, uint256 _amount) internal {
        uint256 bal = layerToken.balanceOf(address(this));
        require(bal >= _amount, "LayerMiningContract' balance is not enough.");
        layerToken.safeTransfer(_to, _amount);
    }
}