    ...
    SuperMerkleAccess
{
  ...
  */
    function mintFromPool(
        ...
        bool whiteListed;
        // TODO: line671 needs to be updated from Apes' WL fix (< allowance for multiple mints)
    ...
    function sellingHelper(
        ...
        if (_whiteListPrice) {
            ...
            if (accessList.token == address(0)) {
                ...
                require(
                    IERC20(accessList.token).balanceOf(_msgSender()) >= price,
                    "0x1C"
                );
            ...
            if (sellingPair.assetType == DFStorage.AssetType.Ether) {
                uint256 price = ((sellingPair.startPrice -
                    sellingPair.finalPrice) *
                    ((pools[_id].config.endTime - block.timestamp))) /
                    (pools[_id].config.endTime - pools[_id].config.startTime);
                if (price < sellingPair.finalPrice)
                    price = sellingPair.finalPrice;
                uint256 etherPrice = price * _amount;
                ...
                require(success, "0x0C");
                uint256 diff = msg.value - etherPrice;
                if (diff > 0) {
                ...
                if (price < sellingPair.finalPrice)
                    price = sellingPair.finalPrice;
                uint256 tokenPrice = price * _amount;
                require(
                    IERC20(sellingPair.asset).balanceOf(_msgSender()) >=
                        tokenPrice,
                    "0x1C"
                );
    ...
    function currentPrice(
        ...
        uint256 price = ((startPrice - finalPrice) *
            ((pools[_id].config.endTime - block.timestamp))) /
            (pools[_id].config.endTime - pools[_id].config.startTime);
