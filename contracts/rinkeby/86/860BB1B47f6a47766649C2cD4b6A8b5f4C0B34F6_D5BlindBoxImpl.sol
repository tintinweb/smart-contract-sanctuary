...
import "./interfaces/ID5BlindBoxV1.sol";
import "./interfaces/ID5BoxRandom.sol";
...
contract D5BlindBoxImpl is ID5BlindBoxV1, ERC721Upgradeable, ERC721EnumerableUpgradeable, AccessControlUpgradeable,
PausableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    mapping(uint256 => uint32) private _tokenAccountMapping;
    ...
    function initialize(uint256 _boxPrice, string memory _name, string memory _symbol, string memory _initBaseURI, address _proxyRegistryAddress) public initializer {
        __Ownable_init();
        __Pausable_init();
        ...
        // whiteListAddress = _wihiteListAddress;
        proxyRegistryAddress = _proxyRegistryAddress;
    }

    /* ================ UTIL FUNCTIONS ================ */
    function _pickLocation(uint256 index) internal returns (uint32) {
        uint256 lastIndex = _userAccounts.length - 1;

        uint32 lastAccount = _userAccounts[lastIndex];
        uint32 pickAccount = _userAccounts[index];
        _userAccounts[index] = lastAccount;
        _userAccounts.pop();
    ...
    {
        require(startIndex < _userAccounts.length, "startIndex cannot over stockLocationsLength");
        require(length > 0, "length cannot equal 0");
        uint256 endIndex = MathUpgradeable.min(startIndex + length, _userAccounts.length);
        uint256 realLength = endIndex - startIndex;
        uint256[] memory tmpUserAccounts = new uint256[](realLength);
        for (uint256 idx = 0; idx < realLength; ++idx) {
            tmpUserAccounts[idx] = _userAccounts[startIndex.add(idx)];
        }
     ...
     */
    function tokenDetail(uint256 tokenId) public view override returns (uint32) {
        require(_exists(tokenId), "Token not exists");
        return _tokenAccountMapping[tokenId];
    }
    ...
    function tokenURI(uint256 tokenId) public view override(ID5BlindBoxV1, ERC721Upgradeable) returns (string memory) {
    ...
    function isBought(address accountAddr) internal view returns (bool) {
        return paidBoxes[accountAddr] >= 1;
    }
     ...
     */
    function buy(uint256 quality) external payable override whenNotPaused nonReentrant {
        _notContract();
        ...
        //just buy once
        require(!isBought(_msgSender()),"have bought");
    
        // if(pawnsStock() <= IWhitelist(whiteListAddress).countValidMember()){
        //     require(IWhitelist(whiteListAddress).isMember(_msgSender()),"blind box sold out");
        // }
        
        // if(IWhitelist(whiteListAddress).isMember(_msgSender())){
        //     IWhitelist(whiteListAddress).invalidMember(_msgSender());
        // }
    ...
    function boxesAward(uint256 quality, address receiver) internal {
        ...
        uint256 seed = ID5BoxRandom(randomGeneratorAddress).generateSignature(salt);
        for (uint256 i = 0; i < quality; ++i) {
            seed = seed.add(totalSupply());
            uint256 randomIdx = uint256(keccak256(abi.encodePacked(seed))).mod(_userAccounts.length);
            uint32  tmpUserAccount = _pickLocation(randomIdx);
            uint256 newId = _awardItem(receiver);
            _tokenAccountMapping[newId] = tmpUserAccount;
    ...
    function batchSetPawnLocationStock(uint32[] memory userAccounts) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
        for (uint16 locationIdx = 0; locationIdx < userAccounts.length; ++locationIdx) {
            uint32 userAccount = userAccounts[locationIdx];
            _userAccounts.push(userAccount);
        }
    }
    ...
    function setShare(address _funderAddress) external override {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
    ...
    function withdraw(uint256 amount) external override {
        ...
        payable(funderAddress).transfer(amount);
    }

    // function setWhitelist(address _whitelistAddress) external override {
    //     require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
...
interface ID5BlindBoxV1 {
    
    struct Product {
        uint16 locationId;
        uint32 userAccount; // properties
    }
    ...
    /* ================ VIEWS ================ */
    function tokenDetail(uint256 tokenId) external view returns (uint32);
    ...
    /* ================ TRANSACTIONS ================ */
    function buy(uint256 quality) external payable;
    ...
    function setShare(address _funderAddress) external;
