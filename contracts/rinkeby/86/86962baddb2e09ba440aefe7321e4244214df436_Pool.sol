...
contract Pool {
    ...
    function initialize(
        ...
        require(
            _liquidityPercentage >= 51 && _liquidityPercentage <= 100,
    ...
    function finish() external onlyOwner {
        ...
        uint256 liquidityBnb = (totalRaised * liquidityPercentage) / 100;
        uint256 liquidityToken = (liquidityBnb * liquidityListingRate) / 1e18;

        uint256 remainingBnb = address(this).balance - liquidityBnb;
        uint256 remainingToken =
            IERC20(token).balanceOf(address(this)) - liquidityToken;
