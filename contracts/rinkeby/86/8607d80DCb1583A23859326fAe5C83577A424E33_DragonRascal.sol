    ...
    Ownable
{
    ...
    constructor(string memory _baseURI) ERC721("Dragon Rascals", "DRAGON") {
        balanceLimit = 15;
        royaltyRate = 500; // 5%
        price = 100000000000000; // 0.0001 ether
        tokenLimit = 8888;
        baseURI = _baseURI;
        suffixURI = ".json";
    ...
    function mint(address to) external payable {
        require(totalSupply() < tokenLimit, "Sold out");
        require(balanceOf(to) < balanceLimit, "Exceeded balance limit");
        require(msg.value >= price, "Insufficient funds");
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
        _setRoyalties(tokenId, payable(owner()), royaltyRate);
