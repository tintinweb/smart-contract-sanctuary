...
contract Waifu is ERC721, Ownable, ERC721URIStorage, ERC721Burnable {
	...
	uint256 rareRampSpeed;
	uint256 commonRampSpeed;

	uint256 rareIDOffset;
	uint256 commonIDOffset;

	uint256 maxRareSupply;
	uint256 maxCommonSupply;
	...
	Counters.Counter private rareIDCounter;
	...
	constructor(Config memory _config) ERC721("Waifu", "WFU") {
		base = _config.base;
		rareIDOffset = _config.rareIDOffset;
		commonIDOffset = _config.commonIDOffset;
		maxRareSupply = _config.maxRareSupply;
		maxCommonSupply = _config.maxCommonSupply;
		maxCommonMintPerTx = _config.maxCommonMintPerTx;
		rareRampSpeed = _config.rareRampSpeed;
	...
	// toggleSale enables and disables purchase by the public
	function toggleSale() public onlyOwner {
		saleActive = !saleActive;
		emit onSaleToggle(saleActive);
	}
	...
	function commonSupplyCapReached() public view returns (bool) {
		return commonIDCounter.current() == maxCommonSupply;
	}

	// rareSupplyCapReached returns true when rare token supply is exhausted
	function rareSupplyCapReached() public view returns (bool) {
		return rareIDCounter.current() == maxRareSupply;
	}
	...
	function getRareUnitPrice() public view returns (uint256) {
		return rareBasePrice + rareIDCounter.current() * rareRampSpeed;
	}
	...
	modifier IsRareID(uint256 id) {
		require(
			id >= rareIDOffset && id < rareIDOffset + maxRareSupply,
	...
	modifier CommonSupplyCapNotReached() {
		require(
			commonIDCounter.current() != maxCommonSupply,
			"Supply cap reached"
		);
		_;
	}
	...
	modifier NotCommonOvermint(uint256 amount) {
		require(
			commonIDCounter.current() + amount <= maxCommonSupply,
	...
	modifier NotRareOvermint(uint256 fromID, uint256 amount) {
		require(
			fromID + amount <= maxRareSupply,
			"Minting this many will overflow into the common ID allocation"
		);
		_;
	}

	// RareSupplyCapNotReached verifies non-exhaustion of rare tokens
	modifier RareSupplyCapNotReached() {
		require(rareIDCounter.current() != maxRareSupply, "Supply cap reached");
		_;
	}

	// SaleActive verifies public sale is active
	modifier SaleActive() {
	...
	function devRareMint(
		...
		onlyOwner
		MoreThanZero(amount)
		RareSupplyCapNotReached
		NotRareOvermint(fromID, amount)
	{
	...
	// rareMint is used to mint rare tokens
	function rareMint(uint256 id)
		public
		payable
		RareSupplyCapNotReached
		SaleActive
	...
	function _rareMint(address to, uint256 id) internal {
		...
		rareIDCounter.increment();
		if (rareIDCounter.current() == maxRareSupply) {
			emit onRareSupplyCapReached();
		}
	}
	...
	function _commonMint(address to) internal {
		_safeMint(to, commonIDOffset + commonIDCounter.current());
		emit onCommonMint(commonIDCounter.current());
		commonIDCounter.increment();
		if (commonIDCounter.current() == maxCommonSupply) {
	...
	// onSaleToggle event to track activated and deactivated sale events
	event onSaleToggle(bool active);

	// onCommonSupplyCapReached event to track supply cap events
	event onCommonSupplyCapReached();

	// onRareSupplyCapReached event to track supply cap events
	event onRareSupplyCapReached();
}
