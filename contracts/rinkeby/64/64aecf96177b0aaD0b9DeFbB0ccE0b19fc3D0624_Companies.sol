...
contract Companies {
    ...
    // Events
    event CreatedCompany(uint[] members, string indexed companyName, uint indexed companyID, address indexed  _wallet);
    event LeftCompany(uint indexed member, string indexed companyName, address indexed _wallet);
    event JoinedCompany(uint[] members, string companyName, address indexed _wallet);
    event CreateOffer(uint members, uint[] indexed candidate, string indexed companyName, address indexed _wallet);
    event AcceptOffer(uint indexed member, string indexed companyName, address indexed _wallet);
    event ClaimAll(address indexed member, uint amount, address indexed _wallet);
...
// Payments
    ...
    function claimAll() public returns (uint){
        ...
        RegularsERC20Contract(regularsERC20_Address).mint(msg.sender, totalPay * DECIMALS);
        emit ClaimAll(msg.sender, totalPay, msg.sender);
