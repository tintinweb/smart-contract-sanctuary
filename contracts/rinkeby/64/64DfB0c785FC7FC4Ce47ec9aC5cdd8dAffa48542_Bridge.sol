...
contract Bridge is Ownable, ReentrancyGuard {
    ...
    event Deposit(address indexed account, uint256 amount, uint256 chainId, uint256 blocknumber, uint256 timestamp, uint256 id);
    event Withdraw(address indexed account, uint256 amount, uint256 id, uint256 chainId);
    ...
    function deposit(uint256 _amount) external noContractsAllowed nonReentrant {
        ...
        emit Deposit(msg.sender, _amount, chainId, block.number, block.timestamp, lastDepositIndex);
    }
    ...
    function withdraw(uint256 _amount, uint256 _chainId, uint256 _nonce, bytes32 _txHash, uint8 _v, bytes32 _r, bytes32 _s) external noContractsAllowed nonReentrant {
        ...
        emit Withdraw(msg.sender, _amount, _nonce, chainId);
    }
