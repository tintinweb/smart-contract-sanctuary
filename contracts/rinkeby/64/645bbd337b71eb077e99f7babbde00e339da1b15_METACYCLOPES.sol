...
contract  METACYCLOPES  is ERC721Enumerable, Ownable
{
    using SafeMath for uint256;
    uint public whiteListCounter;
    uint maxSupply=50;
    uint private tokenId=1;
    bool public isPreSaleFounderPaused=true;
    bool public isPreSaleWhitelistPaused=true;
    bool public isPaused=true;
    uint private publicMintCounter=3;
    uint256 public founderPrice = 0.05 ether;
    ...
    struct FounderAddresses{     
        address userAddress;
        uint counter;
    }

    mapping(address => SpecificAddresses) public _whiteList;
    mapping(address => FounderAddresses) public _founderList;
    ...
    constructor(string memory baseURI) ERC721("Metacyclop", "MC")  {
        setBaseURI(baseURI);
    }
    ...
    function flipPreSalePauseStatusWhiteList() public onlyOwner {
        isPreSaleWhitelistPaused = !isPreSaleWhitelistPaused;
    }
    function flipPreSalePauseStatusFounderList() public onlyOwner {
        isPreSaleFounderPaused = !isPreSaleFounderPaused;
    }
    function addFounderListBundle(address[] memory founderAddress) public onlyOwner {
            for (uint i = 0; i < founderAddress.length; i++)
            {
                require(!_addressExist[founderAddress[i]],"Address already Exist");
                _founderList[founderAddress[i]] = FounderAddresses({
                    userAddress :founderAddress[i],
                    counter: 2
                });
                _addressExist[founderAddress[i]]=true;
            }
    }
    ...
    function getFounderPrice(uint256 _quantity) public view returns (uint256) {
        return _quantity*founderPrice;
    }
    ...
    function preSalemintWhiteList(uint quantity) public payable {
        require(isPreSaleWhitelistPaused == false, "Sale is not active at the moment");
        ...
        require(whiteListPrice.mul(quantity) == msg.value, "Sent ether value is incorrect");
    ...
    function preSalemintFounderList(uint quantity) public payable {
        require(isPreSaleFounderPaused == false, "Sale is not active at the moment");
        require(totalSupply()+quantity<=maxSupply,"Quantity is greater than remaining Supply");
        require(_addressExist[msg.sender]==true,"Address not Found in whitelist");
        require(founderPrice.mul(quantity) == msg.value, "Sent ether value is incorrect");
        SpecificAddresses storage myaddress = _whiteList[msg.sender];
        require(myaddress.counter>=quantity,"The Amount Selected is Greater than the Remaining Amount of the Token at this Address");
        for (uint256 i; i < quantity; i++) {
            _safeMint(msg.sender, totalsupply());
    ...
    function mint(uint quantity) public payable {
        ...
        require(totalSupply()+quantity<=maxSupply,"Quantity is greater than remaining Supply");
        require(publicPrice.mul(quantity) == msg.value, "Sent ether value is incorrect");
