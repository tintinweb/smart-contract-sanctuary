...
contract BananaHands is ERC721Enumerable, Pausable, Ownable, PaymentSplitter {
    ...
    mapping(address => bool) _discountApplied;
    ...
    function canAccessPresale() public view returns (bool) {
        return _bonesAndBananas.balanceOf(msg.sender) > 0;
    }
    ...
    function mintPrice(uint256 numberOfTokens) public view returns (uint256) {
        uint256 price = _mintPrice.mul(numberOfTokens);

        if (_discountApplied[msg.sender]) {
            return price;
        }

        uint256 discountUnits = _bonesAndBananas.balanceOf(msg.sender);
        ...
        return price.sub(_mintPrice.div(10).mul(discountUnits));
    }
        ...
        validatePurchasePrice(numberOfTokens)
    {
        ...
        _discountApplied[msg.sender] = true;
        ...
        validatePurchasePrice(numberOfTokens)
    {
        ...
        require(
            canAccessPresale(),
            "Must own a Banana to participate in presale"
        );
        _discountApplied[msg.sender] = true;
