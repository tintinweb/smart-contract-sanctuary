...
contract AcidGanApes is ERC721Enumerable, Ownable {
    ...
    modifier canPresaleMint() {
        ...
        require(isPresaleAllowed, 'Presale not allowed');
        _;
    }

    modifier canMint() {
        require(block.timestamp >= saleStart, 'Sale not started');
        require(isSaleAllowed, 'Sale not allowed');
        _;
    }

    function presaleMint(uint256 quantity) public payable canPresaleMint {
        require(quantity > 0 && quantity <= presaleAllowance[_msgSender()], 'Quantity must be x > 0 and sender allowance');
    ...
    function mint(uint256 quantity) public payable canMint {
    ...
    function setSaleAllowed(bool isSaleAllowed_) external onlyOwner {
        isSaleAllowed = isSaleAllowed_;
    }
    ...
    function setPresaleAllowed(bool isPresaleAllowed_) external onlyOwner {
        isPresaleAllowed = isPresaleAllowed_;
    }

    function setPresaleAllowance(address addr, uint256 quantity) public onlyOwner {
        presaleAllowance[addr] = quantity;
    }

    function setPresaleAllowanceBulk(address[] memory addrs, uint256 [] memory quantities) public onlyOwner {
