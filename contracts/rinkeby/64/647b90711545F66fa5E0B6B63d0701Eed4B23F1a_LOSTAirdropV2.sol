...
contract LOSTAirdropV2 is Ownable, Pausable {
    ...
    function claim(address _beneficiary, uint256 _glitch) public {
        require(tlgcmc.ownerOf(_glitch) == _beneficiary, "LOSTAirdrop: Beneficiary is not owner of Glitch");
        require(hasClaimed[_glitch] == false, "LOSTAirdrop: Reward already claimed for Glitch");
        require(lost.balanceOf(address(this)) >= airdropPerComic, "LOSTAirdrop: Not enough rewards left.");
    ...
    function claimAll(address _beneficiary) public {
        uint256 ownedComics = tlgcmc.balanceOf(_beneficiary);
        require(ownedComics > 0, "LOSTAirdrop: Beneficiary is does not own any Glitches");
