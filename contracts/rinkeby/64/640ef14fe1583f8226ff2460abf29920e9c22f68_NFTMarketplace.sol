...
contract NFTMarketplace is Pausable, FeeManager, ERC721Holder {
    ...
    function safeExecuteOrder(address _tokenAddress, uint256 _tokenId) public whenNotPaused payable{
        ...
        _executeOrder(order.orderId, msg.sender,  _tokenAddress,  _tokenId);
    ...
    function _createOrder(address _tokenAddress, uint256 _tokenId, uint256 _askingPrice, uint256 _expiryTime) internal   {
        ...
        function _executeOrder(bytes32 _orderId, address _buyer, address _tokenAddress, uint256 _tokenId) internal {
    ...
    function tokensOfOwner(address _owner) public view returns (uint256[] memory)
    {
        ...
        for (uint256 index = 0; index < count; index++) {
            result[index] = nftToken.tokenOfOwnerByIndex(_owner, index);
        }
        return result;
    }
