...
contract Escrow is Migratable {
    ...
    //function getPaprTaxRate() public view returns (uint256) {
     // return IPapr(escrowToken).getfeesTaxRate(_countTimesOverCents());
   // }
    ...
    returns (uint256) {
        ...
        uint256 _computeAmount = _amount.div(10);
        uint256 _amountAfterBurn = _amount.sub(_computeAmount);
        require(escrowToken.transferFrom(_employer, address(this), _amountAfterBurn), "Client must have authorisation and balance to transfer PAPR");
        ...
        emit OnCreateEscrow(escrowDapp, _employer, _provider, _amountAfterBurn, daiValueInEscrow);
