...
contract GenTickets is ERC1155, ERC1155Receiver {
    ...
    function redeemTicket(address _to, uint _id, uint _amount) public { //address, 18, 1

        uint tier = _id.mod(numTicketTypes); //18 mod 4 = 2
        GenTicket memory ticketType = genTickets[tier]; //pink
        ...
        // Tranche past cliff
        uint tranche = _id.div(numTicketTypes); //18/4=4
        uint actualTranche;
        if (tranche <= ticketType.cliffTranches){ //4 <= 5
            actualTranche = 0; //0
        } else{
        ...
        require(tranche < ticketType.totalTranches, "GenTickets: Ticket has redeemed all tokens"); // 4 < 5-1
        ...
        uint availableToClaim = (ticketType.ticketSize).div((ticketType.totalTranches).sub(ticketType.cliffTranches)); // 5/(5-1) = 1
        uint averageTokensAvailableToClaim = availableToClaim.mul(_amount); //1
        ...
        if (eligibleTraches.mul(averageTokensAvailableToClaim) > redeemedTokens){
            tokens = eligibleTraches.mul(averageTokensAvailableToClaim).sub(redeemedTokens); //4
        } else {
            tokens =0;
        }
