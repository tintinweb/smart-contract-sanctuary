...
contract OrderBookDEX is ReentrancyGuard {
    ...
    */
    ...
    function getQuoteMarketSell(ERC20 token2sell, uint256 amount2sell, ERC20 token2get) public view
        returns (uint256 ret_fee, uint256 ret_rewards, uint256 ret_total) {
        ...
        for (uint i = 0; 
            ...
            how_much_to_spend = amount2sell*10**token2get.decimals();
    ...
    function getQuoteMarketBuy(ERC20 token2get, uint256 amount2sell, ERC20 token2sell) public view 
        returns (uint256 ret_fee, uint256 ret_rewards, uint256 ret_total) {
        ...
        for (uint i = 0; 
             i < open_limit_sell_orders[address_token2get][address_token2sell].length && how_much_to_spend>0; i++
        ) {
            ...
            if (users_balances[address_token2get][user] >= trade_amount1 && users_balances[address_token2sell][msg.sender] >= trade_amount2) {
                ...
                ret_rewards += getReward(ERC20(address(token2get)), trade_amount1);
                ret_total += trade_amount2;
                ret_fee += theCommission(trade_amount2);
    ...
    function getQuoteLimitBuy(ERC20 token2get, uint256 amount2sell, ERC20 token2sell, uint256 price_buy) public view
        returns (uint256 ret_fee, uint256 ret_rewards, uint256 ret_total) {

        uint256 how_much_to_spend;

        how_much_to_spend = amount2sell.mul(price_buy).div(10**token2sell.decimals());
        ...
        for (uint i = 0; i < open_limit_sell_orders[address_token2get][address_token2sell].length && how_much_to_spend>0; i++) {

            if (open_limit_sell_orders[address_token2get][address_token2sell][i].price <= price_buy) {
                ...
                if (users_balances[address_token2get][user] >= trade_amount1 && users_balances[address_token2sell][msg.sender] >= trade_amount2) {
                    ...
                    how_much_to_spend = how_much_to_spend.sub(trade_amount2);

                    amount2sell = amount2sell.sub(trade_amount1);
