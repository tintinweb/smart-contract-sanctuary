...
/**
 * @title XcuteMaster
 * @dev XcuteMaster contract
 * @author Federico Luzzi - <[email protected]>
 * SPDX-License-Identifier: GPL-3.0
 */
contract XcuteMaster is Ownable {
    ...
    constructor(
        ...
        if (_staker == address(0)) revert ZeroAddressStaker();
    ...
    function addFreeCredit(address _job, uint256 _amount) external {
        ...
        IXCUTE(xcute).mint(address(this), _amount);
    }
    ...
    function addCredit(address _job, uint256 _amount) external {
        ...
        IERC20(xcute).safeTransfer(owner(), _fee);
    }
    ...
    function isWorkerWithRequirements(
        ...
        gasCheckpoint = gasleft();
        if (!worker[_address]) return false;
        if (
            _minimumStaked > 0 &&
            IXcuteStaker(staker).staked(_address) < _minimumStaked
        ) return false;
        if (
            _minimumEarned > 0 &&
            IXcuteStaker(staker).earned(_address) < _minimumEarned
        ) return false;
        if (
            _minimumAge > 0 &&
            block.timestamp -
                IXcuteStaker(staker).activationTimestamp(_address) <
            _minimumAge
        ) return false;
        return true;
    }
    ...
    function worked(address _worker) external {
        if (!worker[_worker]) revert InvalidWorker();
        Job storage _jobFromStorage = job[_msgSender()];
        if (!_jobFromStorage.exists) revert NonExistentJob();
        uint256 _gasUsed = gasCheckpoint - gasleft();
        uint256 _workCost = IXcuteWorkEvaluator(workEvaluator).evaluateCost(
            _worker,
            _gasUsed
        );
        if (_jobFromStorage.credit < _workCost) revert NotEnoughCredit();
        _jobFromStorage.credit -= _workCost;
        IXcuteStaker(staker).stake(_workCost);
    }
}
