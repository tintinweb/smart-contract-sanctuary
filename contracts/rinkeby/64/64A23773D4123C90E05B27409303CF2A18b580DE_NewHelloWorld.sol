...
contract NewHelloWorld is Ownable, ReentrancyGuard {
    ...
    uint private constant MAXPALATFORMFEE = 10000;
    uint constant public MAX_VALIDATOR_COUNT = 50;
    ...
    modifier validRequirement(uint validatorCount, uint _required) {
        require(validatorCount <= MAX_VALIDATOR_COUNT
            && _required <= validatorCount
            && _required != 0
            && validatorCount != 0);
        _;
    }

    function MultiSigWallet(address[] memory _validator, uint _required) internal validRequirement(_validator.length, _required) {
        for (uint i=0; i<_validator.length; i++) {
            require(!isValidator[_validator[i]] && _validator[i] != address(0));
    ...
    function addvalidator(address validator) external onlyGovernance validatorDoesNotExist(validator) notNull(validator) validRequirement(validators.length + 1, required) {
        isValidator[validator] = true;
        validators.push(validator);
        emit ValidatorAddition(validator);
    }

    function removeValidator(address validator) external onlyGovernance validatorExists(validator){
    ...
    function changeRequirement(uint _required) internal validRequirement(validators.length, _required) {
        required = _required;
        emit RequirementChange(_required);
    }

    constructor(address _governance, address[] memory _validators, uint256 _required, address _beneficiary, uint256 _depositFee, uint256 _withdrawFee, uint256 _listingFee, uint256 _foreignChainId) {
        governance = _governance;
        MultiSigWallet(_validators, _required);
    ...
    event Deposited(address indexed account, uint256 amount, address homeERC20, address foreignERC20, uint256 homeChainId, uint256 foreignChainId, uint256 blocknumber, uint256 timestamp, uint256 id);
    event Withdrawal(address indexed account, uint256 amount, address withdrawERC20, address depositedERC20, uint256 id, uint256 homeChainId, uint256 foreignChainId);
    event BridgePairCreated(address indexed account, address homeERC20, uint256 homeChainId, address foreignERC20, uint256 foreignChainId, address listedBy, uint256 blocknumber, uint256 blockTimestamp, uint256 tokenIndex);
    ...
    function createBridgePair(address _homeERC20, address _foreignERC20) external noContractsAllowed nonReentrant payable {
        ...
        emit BridgePairCreated(msg.sender, _homeERC20, homeChainId, _foreignERC20, foreignChainId, msg.sender, block.number, block.timestamp, lastPairIndex);
    }
    ...
    function depositToBridge(address _homeERC20, uint256 _amount) external noContractsAllowed nonReentrant {
        ...
        updateDeposit(lastDepositIndex, _amount, _homeERC20, trustedTokens[_homeERC20]._foreignERC20, block.number, block.timestamp);
        emit Deposited(msg.sender, _amount, _homeERC20, trustedTokens[_homeERC20]._foreignERC20, homeChainId, foreignChainId, block.number, block.timestamp, lastDepositIndex);
    }
    
    function withdrawfromBridge(uint256 _amount, address _withdrawERC20, address _depositedERC20, uint256[2] memory _chainID, uint256 _nonce, bytes32 _txHash, Sig[] memory signature, address[] memory _validators) external noContractsAllowed nonReentrant {
        ...
        require(homeChainId == _chainID[0], "WithdrawfromBridge:: Invalid Home chainId!");
        require(foreignChainId == _chainID[1], "WithdrawfromBridge:: Invalid Foreign chainId!");
        require(!claimedWithdrawalsByOtherChainDepositId[_nonce], "WithdrawfromBridge:: Already Withdrawn!");
        require(!(_validators.hasDuplicate()), "WithdrawfromBridge:: Duplicate Validator Found");
        for(uint256 i=0; i< signature.length; i++) {
            require(verifySignature(_amount, _withdrawERC20, _depositedERC20, homeChainId, foreignChainId, _nonce, _txHash, signature[i], _validators[i]), "WithdrawfromBridge:: Invalid Signature");   
        }
        withdrawTokensAndPlatformFee(_withdrawERC20, _amount);
        claimedWithdrawalsByOtherChainDepositId[_nonce] = true;
        updateWithdraw(_nonce, _amount, _withdrawERC20, _depositedERC20, _txHash);
        emit Withdrawal(msg.sender, _amount, _withdrawERC20, _depositedERC20, _nonce, homeChainId, foreignChainId);
    }

    function depositTokensAndPlatformFee(address _homeERC20, uint256 _amount) internal {
        uint256 platformFee = _amount.mul(depositFee).div(MAXPALATFORMFEE);
        if(depositFee > 0) {
    ...
    function withdrawTokensAndPlatformFee(address _withdrawERC20, uint256 _amount) internal {
        uint256 platformFee = _amount.mul(withdrawFee).div(MAXPALATFORMFEE);
        if(withdrawFee > 0) {
    ...
    function updatePair(uint256 _index, address _homeERC20, address _foreignERC20) internal {
        ...
        trustedTokens[_homeERC20]._homeChainId = homeChainId;
        trustedTokens[_homeERC20]._foreignChainId = foreignChainId;
    ...
    function updateDeposit(uint256 _index, uint256 _amount, address _homeERC20, address _foreignERC20, uint256 _depositedBlock, uint256 _depositedTimeStamp) internal {
    ...
    function updateWithdraw(uint256 _nonce, uint256 _amount, address _withdrawERC20, address _depositedERC20, bytes32 _txHash) internal {
    ...
    function verifySignature(uint256 _amount, address _withdraERC20, address _depositedERC20, uint256 _homeChainId, uint256 _foreignChainId, uint256 _nonce, bytes32 _txHash, Sig memory signature, address _validator) internal view returns (bool) {
        address msgSigner = StringLibrary.getAddress(abi.encodePacked(msg.sender, _amount, _withdraERC20, _depositedERC20, _homeChainId, _foreignChainId, _nonce, _txHash, address(this)), signature.v, signature.r, signature.s);
    ...
    function updateDepositFee(uint256 _newDepositFee) external onlyGovernance {
        require(_newDepositFee <= MAXPALATFORMFEE, "UpdateWithdrawFee:: Invalid withdraw fee ");
    ...
    function updateWithdrawFee(uint256 _newWithdrawFee) external onlyGovernance{
        require(_newWithdrawFee <= MAXPALATFORMFEE, "UpdateWithdrawFee:: Invalid withdraw fee ");
    ...
    function updateRewardToken(address _newRewardToken) external onlyGovernance {
        require(_newRewardToken != address(0), "UpdateRewardToken:: Renounce Reward Token");
        rewardToken = _newRewardToken;
    }

    function renounceRewardToken() external onlyGovernance {
    ...
    function validatorSignMessage(address _userWallet, uint256 _amount, address _withdraERC20, address _depositedERC20, uint256 _nonce, bytes32 _txHash) external view returns(bytes memory) {
        return abi.encodePacked(_userWallet, _amount, _withdraERC20, _depositedERC20, homeChainId, foreignChainId, _nonce, _txHash, address(this));
    }
}
