...
contract ASCII is ERC721Enumerable, Ownable, Functions, Metadata {
  ...
  function mintFreeAscii() external  {
    ...
    require(_freeMintTotal[msg.sender] <= PURCHASE_LIMIT_FREE,'Already maxed out');
    require(balanceOf(msg.sender) <= PURCHASE_LIMIT_FREE,'Mint less');
  ...
  function mintPaidAscii_max4_01EthEach(uint256 hash_ASC) external payable  {
    ...
    require(_paidMintTotal[msg.sender] <= PURCHASE_LIMIT_PAID,'Already maxed out');
    require(balanceOf(msg.sender) <= PURCHASE_LIMIT_PAID,'Too many in wallet');
