...
contract TestDAO is ERC721A, Ownable, PaymentSplitter, ReentrancyGuard {
    uint256 public immutable MAX_PER_ADDRESS;
    uint256 public constant PRICE = 0.069 ether;
    uint256 public immutable MAX_SUPPLY;
    uint256 public constant MAX_WHITELIST_TOKENS_PER_USER = 5;
    ...
    constructor(
        ...
        ERC721A("TestDAO", "TD", maxBatchSize_)
    {
        MAX_SUPPLY = 10_000;
        ...
        callerIsUser
    {
        require(
            ...
            "Whitelist sale not open"
        );
        require(
            count <= MAX_WHITELIST_TOKENS_PER_USER,
            "Exceeds max whitelist token limit"
        );
        require(PRICE * count == msg.value, "Incorrect ETH value sent");
        require(
            claimed[msg.sender] < MAX_WHITELIST_TOKENS_PER_USER,
            "Already minted max whitelist allocation"
        );
    ...
    function setWhitelistSaleStartTime(uint256 _whitelistStartTime)
        ...
        onlyOwner
    {
        whitelistSaleStartTime = _whitelistStartTime;
    }
