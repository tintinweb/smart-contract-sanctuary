...
contract testNftFactory is Ownable, ERC721{
       ...
       require(tokenType < 2,"Invalid type");
        ...
        for (uint i=0; i< tokenAmount; i++) {
            if (stake) {
                tokenId_.increment();
                _safeMint(address(game), tokenId_.current());
                _addTokenToOwnerEnumeration(msg.sender,tokenId_.current());
                metadataHandler.addMetadata(1,tokenType);
                game.alertStake(tokenId_.current());
            }
            else {
    ...
    function _addTokenToOwnerEnumeration(address to, uint256 tokenId) private {
        uint count = 0;
        while(_ownedTokens[to][count] != 0){
            count++;
        }
        _ownedTokens[to][count] = tokenId;
        _ownedTokensIndex[tokenId] = count;
    }

    function getAllUserTokens() external view returns(uint[] memory){

        uint count = 0;
        while(_ownedTokens[msg.sender][count] != 0){
            count++;
        }
        uint[] memory tokens = new uint[](count);

        for (uint i=0;i<count;i++){
            tokens[i] = _ownedTokens[msg.sender][i];
        }
        return tokens;
    ...
    function _removeTokenFromOwnerEnumeration(address from, uint256 tokenId) private {
        ...
        uint count = 0;
        while(_ownedTokens[from][count] != 0){
            count++;
        }
        uint256 lastTokenIndex = count - 1;
