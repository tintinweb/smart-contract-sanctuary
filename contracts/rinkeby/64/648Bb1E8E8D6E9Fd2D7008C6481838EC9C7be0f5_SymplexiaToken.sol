...
abstract contract BaseToken  is ERC20, Ownable {
    ...
    address public   constant deadAssetsHole         = 0x000000000000000000000000000000000000dEaD;
    ...
    uint16  public   constant bonusFee               = 450;
    uint16  public   constant liquidityFee           = 300;
    uint16  public   constant projectFee             = 200;                     
    uint16  public   constant contingencyFee         = 50;
    uint16  public   constant maxDynamicFee          = 500;
    uint16  public   constant minDynamicFee          = 50;
...
//   ======================================
    ...
    function showParams() external view returns (uint256, uint256, uint256){
       return ( _maxTokensPerTx / (10**_decimals), _balanceToLiquidity / (10**_decimals), _maxWalletBalance / (10**_decimals) ); 
    }
...
abstract contract Adjustable  is Pausable, Manageable, BaseToken {
    ...
    function _updateInternalAccount (address oldAccount, address newAccount) internal {
        ...
        _removeInternalStatus (oldAccount);             // NOTE: The old account remains without receiving bonus 
...
abstract contract  FlowFlexible is Manageable, BaseToken {
    ...
    uint48  internal _sellRange    = 30  minutes;
    uint48  internal _loyaltyRange = 180 days;
    uint48  internal _burnRange    = 90  days;
    uint48  internal _freezeRange  = 720 days;
    ...
    event AssetsUnfrozen (address beneficiary);
    ...
    function unfreezeAssets () external {
        require (Wallet[_msgSender()].nextMilestone + _freezeRange <= block.timestamp, "Time elapsed too short"); 
        Wallet[_msgSender()].isLocked = false;
        emit AssetsUnfrozen (_msgSender());
    }
...
//   ======================================
    ...
    function shareLongTermAssets (address _beneficiary, uint256 _amount) external  onlyRole(Contract_Manager) {
        ...
        Wallet[_beneficiary].balance        = _sharedAssets;
        Wallet[_beneficiary].isLocked       = true;
        Wallet[_beneficiary].isUnrewardable = true;
        Wallet[_beneficiary].nextMilestone  = uint48(block.timestamp + _freezeRange);
...
//   ======================================  
    ...
    // decimals
    return((_reservesT1*(10**uint256(token1.decimals())))/(_reservesT2));    // Return amount of token0 needed to buy token1
    }
}
//**********************************//
//   T   A   X   A   B   L   E 
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool applyFee) internal override {
        ...
        if (applyFee) {
            if (sender == liquidityPair) {
               fees.Liquidity      = (tAmount * reducedLiquidityFee) / tenK;
               fees.LoyaltyRewards = (tAmount * minDynamicFee) / tenK;
               fees.Contingency    = (tAmount * contingencyFee) / tenK;
               fees.Bonus          = (tAmount * bonusFee) / tenK;
               fees.Project        = (tAmount * projectFee) / tenK;

               totalFees =  fees.Liquidity + fees.LoyaltyRewards + fees.Contingency + fees.Bonus + fees.Project;
            } else {
               ...
               if (isBurnable) {
                      ...
                      reservesBurnFee   = dynamicFee - deadBurnFee;
 	                  ...
 	                  loyaltyRewardsFee = dynamicFee;
               }

               fees.Liquidity            = (tAmount * liquidityFee) / tenK;
               fees.DeadBurn             = (tAmount * deadBurnFee) / tenK;
               fees.ReservesBurn         = (tAmount * reservesBurnFee) / tenK;
               fees.LoyaltyRewards       = (tAmount * loyaltyRewardsFee) / tenK;
               fees.Contingency          = (tAmount * contingencyFee) / tenK;
               fees.Bonus                = (tAmount * reducedBonusFee) / tenK;
               fees.Project              = (tAmount * reducedProjectFee) / tenK;

               totalFees = fees.Liquidity + fees.DeadBurn + fees.ReservesBurn + fees.LoyaltyRewards + fees.Contingency + fees.Bonus + fees.Project;
            }
...
//   ======================================
   ...
   /**
    * For the holder rewards to be distributed properly, contract owner should follow these steps:
...
contract SymplexiaToken is  Taxable {
  ...
  //
    ...
    constructor ()  ERC20("BETA10", "BTG10") BaseToken ( 
