...
// dummy data for testing
        ...
        myMembers3[1] = 4551;
        myMembers3[2] = 5675;
        createCompany("Test LLC", myMembers3);
    }
...
// Create Company
    ...
    function leaveCompany(uint[] memory _members, uint _companyID) public {
        ...
        for (uint m = 0; m < _members.length; m++) {
            for (uint i = 0; i < companies[companyIDs[m]].members.length; i++) {
                if (companies[companyIDs[m]].members[i] == _members[m]) {
                    delete companies[_companyID].members[m];
                    companyIDs[_members[m]] = 0; 
                    emit LeftCompany(_members[m], companies[companyIDs[m]].name);
                    break;
                }
            }
        }
