...
contract eLoot is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    /**
     * @dev random
     * 这里随机的时候增加用户自己的地址,和时间戳,还有tokenId, 这样每次得到的就是不同的随机数
     */
    function random(string memory input) public view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(_msgSender(), block.timestamp, input)));
    }

    function getFirst() public pure returns (string memory output) {
        output = '0x123456789';
    }
    ...
    function _getAllElement(uint256 tokenId) internal view returns (string[8] memory output)  {
        ...
        // 选择8里面一个元素
        rand = random(tokenId.toString());
        output[6] = transition_element[rand % transition_element.length];
        ...
        output[7] = is9_and10 == 1 ?
             ...
             transition_element[random(tokenId.toString()) % transition_element.length];
       
        return output;
    }

    /*** 生成 8 个元素里 2-7 位的两个方法 ***/
    // 返回一个从 1-7 组里各取一个元素组成的长度为 7 的数组
    // function arrayFrom1To7(uint256 tokenId) internal pure returns (string[7] memory) {
    //     uint256 rand = random(tokenId.toString());
    // }

    // 从 1-7 组随机取一个，每次都要返回不同
    // function get1FromArray () internal pure returns (string memory) {
    // }
    /*** 生成 8 个元素里 2-7 位的两个方法结束 ***/

    // 生成 8 个元素里最后一个
    // function getLastEle(uint256 tokenId) internal view returns (string memory output) {
    //     // 因为上面有取第transition_element,为了不跟上面取一样的，简单加个 x 用来生成rand
    //     uint256 rand = random(string(abi.encodePacked("x", tokenId.toString())));
    //     output = transition_element[rand % transition_element.length];
    // }
