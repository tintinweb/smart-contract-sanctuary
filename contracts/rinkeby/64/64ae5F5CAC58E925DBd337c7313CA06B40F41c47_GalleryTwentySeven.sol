...
contract GalleryTwentySeven is Ownable, ReentrancyGuard {
    ...
    uint64 private _ownerShare;
    uint64 private _daoShare;
    ...
    event Bid(uint256 indexed punkScapeId, uint256 indexed bid, address indexed from);
    event Claim(uint256 indexed punkScapeId, uint256 tokenId);
    ...
    function getAuction (uint256 punkScapeId)
        public view
        returns (address latestBidder, uint128 latestBid, uint64 endTimestamp, bool settled)
    {
        Auction memory auction = _auctions[punkScapeId];

        return (auction.latestBidder, auction.latestBid, auction.endTimestamp, auction.settled);
    }
    ...
    ) external payable
    {
        ...
        require(endTimestamp >= uint64(block.timestamp), "Auction has ended."); // solhint-disable-line not-rely-on-time
        ...
        _auctions[punkScapeId] = Auction(_msgSender(), uint128(msg.value), endTimestamp, false);

        emit Bid(punkScapeId, msg.value, _msgSender());
    }

    function bid (
        uint256 punkScapeId
        // TODO: add style, haiku?
    )
        external payable
        nonReentrant
    {
        ...
        require(block.timestamp <= auction.endTimestamp, "Auction has ended."); // solhint-disable-line not-rely-on-time
        ...
        uint64 _now = uint64(block.timestamp); // solhint-disable-line not-rely-on-time
        ...
        emit Bid(punkScapeId, bidValue, bidder);
    }
    ...
    function claim (
        ...
        address scapeOwner = ERC721(_punkScapes).ownerOf(punkScapeId);
        ...
        // Pay OG scape owner and PS
        uint256 ownerShare = auction.latestBid / 2;
        payable(scapeOwner).transfer(ownerShare);
        payable(owner()).transfer(auction.latestBid - ownerShare);

        // End the auction
        auction.settled = true;
        emit Claim(punkScapeId, tokenId);
    }
