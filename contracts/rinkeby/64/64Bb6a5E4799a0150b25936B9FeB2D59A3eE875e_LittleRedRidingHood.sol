...
/// @author: West Coast NFT

import "@manifoldxyz/creator-core-solidity/contracts/core/IERC721CreatorCore.sol";
import "@manifoldxyz/creator-core-solidity/contracts/extensions/ICreatorExtensionTokenURI.sol";
import "@manifoldxyz/creator-core-extensions-solidity/contracts/redeem/ERC721/ERC721BurnRedeem.sol";
 ...
 */
contract LittleRedRidingHood is ERC721BurnRedeem, ICreatorExtensionTokenURI {

    uint16 public constant MAX_PUBLIC_MINT = 15;
    uint256 public constant PRICE_PER_TOKEN = 0.05 ether;
    uint16 constant REDEMPTION_RATE = 3;
    uint16 constant REDEMPTION_MAX = 300;
    ...
    DateTimeAPI dt = DateTimeAPI(0x92482Ba45A4D2186DafB486b322C6d0B88410FE7);
    ...
    string constant BASE_URI = "ipfs://QmfUQvSdBoDcR2Wfhhw8PQwJAM1DBtzU1y1YNSMiaBPDH1";

    enum WeatherCondition { Sun, Clouds, Wind, Rain, Fog, Snow }
    enum Season { Spring, Summer, Fall, Winter }
    enum Hemisphere { Northern, Southern }

    mapping(WeatherCondition => string) private weatherConditionStrings;
    ...
    mapping(uint256 => int) private offsets;
    mapping(uint256 => Hemisphere) private hemispheres;

    constructor(address creator) ERC721BurnRedeem(creator, REDEMPTION_RATE, REDEMPTION_MAX) {

        weatherConditionStrings[WeatherCondition.Sun] = "Sun";
        weatherConditionStrings[WeatherCondition.Clouds] = "Clouds";
        weatherConditionStrings[WeatherCondition.Wind] = "Wind";
        weatherConditionStrings[WeatherCondition.Rain] = "Rain";
        weatherConditionStrings[WeatherCondition.Fog] = "Fog";
        weatherConditionStrings[WeatherCondition.Snow] = "Snow";

        seasonStrings[Season.Spring] = "Spring";
        seasonStrings[Season.Summer] = "Summer";
        seasonStrings[Season.Fall] = "Fall";
        seasonStrings[Season.Winter] = "Winter";

        hemisphereStrings[Hemisphere.Northern] = "Northern";
        hemisphereStrings[Hemisphere.Southern] = "Southern";

        weights[Season.Spring][WeatherCondition.Sun] = 5;
        weights[Season.Spring][WeatherCondition.Clouds] = 4;
        weights[Season.Spring][WeatherCondition.Wind] = 5;
        weights[Season.Spring][WeatherCondition.Rain] = 5;
        weights[Season.Spring][WeatherCondition.Fog] = 2;
        weights[Season.Spring][WeatherCondition.Snow] = 0;
        
        weights[Season.Summer][WeatherCondition.Sun] = 4;
        weights[Season.Summer][WeatherCondition.Clouds] = 3;
        weights[Season.Summer][WeatherCondition.Wind] = 2;
        weights[Season.Summer][WeatherCondition.Rain] = 1;
        weights[Season.Summer][WeatherCondition.Fog] = 1;
        weights[Season.Summer][WeatherCondition.Snow] = 0;

        weights[Season.Fall][WeatherCondition.Wind] = 1;
        weights[Season.Winter][WeatherCondition.Snow] = 1;

    }

    function setOffset(uint256 tokenId, int offset) public {
        require(msg.sender == IERC721(_creator).ownerOf(tokenId), "Must be the token owner.");
    ...
    function currentSeason(uint256 tokenId) public view returns(Season) {
        return monthToSeason(hemispheres[tokenId], dt.getMonth(block.timestamp));
    }

    function currentWeather(uint256 tokenId) public view returns(WeatherCondition) {
        uint ts = block.timestamp;
        uint8 month = dt.getMonth(ts);
        uint8 day = dt.getDay(ts);
        uint16 year = dt.getYear(ts);
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721BurnRedeem, IERC165) returns (bool) {
        return interfaceId == type(ICreatorExtensionTokenURI).interfaceId || ERC721BurnRedeem.supportsInterface(interfaceId);
    }

    function setBurnState(bool _active) external onlyOwner {
    ...
    function purchase(uint256 amount) external payable nonReentrant {
        require(amount <= redemptionRemaining(), "Too many requested");
        ...
        for (uint i = 0; i < amount; i++) {
            super._mintRedemption(msg.sender);
        }
    }
    ...
    function imageIndex(Season season, WeatherCondition weatherCondition) public pure returns(uint256) {
        return uint(season) * NUM_WEATHER_CONDITIONS + uint(weatherCondition);
    }

    function getImageURL(uint256 tokenId) public view returns(string memory) {
        return string(abi.encodePacked(BASE_URI,
                                       "/",
                                       imageIndex(currentSeason(tokenId),
                                                  currentWeather(tokenId)).toString(),
                                       ".jpg"));
    }
    ...
    function getDescription() internal pure returns(string memory) {
        return "Little Red Riding Hood through the four seasons";
    }
    ...
    function _getChangingData(uint256 tokenId) internal view returns(string memory) {
        return string(abi.encodePacked(
            _wrapTrait("Weather", weatherConditionStrings[currentWeather(tokenId)]),
            ',',_wrapTrait("Season", seasonStrings[currentSeason(tokenId)]),
            ',',_wrapTrait("Hour Offset",
                           string(abi.encodePacked(offsets[tokenId] < 0 ? '-' : '',
                                                   uint(offsets[tokenId]).toString()))),
