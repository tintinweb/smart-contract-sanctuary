...
contract AlexNFT is ERC1155Tradable {
    ...
    function _trade(uint _tokenId, uint _amount, address payable _owner, uint256 _price, bool _isagent, uint256 agentcommission) internal {
        ...
        // 2.5% commission cut
        uint _commissionValue = 0;
        uint _sellerValue = 0;
        uint _clientValue = agentcommission;
        uint _platformValue = (_price * _platformCommission / 100) * _amount;
        
        if(_isagent){
            _commissionValue = (_price * 25 / 100) * _amount ;
            _clientValue = (_price * 10 / 100) * _amount;
            _sellerValue = (_price * _amount) - (_commissionValue + _clientValue + _platformValue);
        }
        else{
            _commissionValue = (_price * 25 / 100) * _amount ;
            _sellerValue = (_price * _amount) - (_commissionValue + _platformValue);
        }
