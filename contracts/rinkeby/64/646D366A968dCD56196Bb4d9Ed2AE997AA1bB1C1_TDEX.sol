...
contract TDEX {
    ...
    event Deposit(address indexed tokenContract, uint256 orderId, address indexed user, uint8 action, uint256 usdtAmount, uint256 tokenAmount);
    ...
    function sendBuyOrder(address _tokenContract, uint256 _price, uint256 _usdt_amount) external returns (uint256 orderId)
    {
        Token memory token = tokenMap.data[_tokenContract];
        ...
        orderId = OrderManager(orderManager).createOrder(_tokenContract, msg.sender, _price, 0, _usdt_amount, 0);
        if (orderId > 0)
        {
        ...
        emit Deposit(_tokenContract, orderId, msg.sender, 0, _usdt_amount, 0);
    ...
    function sendSellOrder(address _tokenContract, uint256 _price, uint256 _token_amount) external returns (uint256 orderId)
    {
        ...
        orderId = OrderManager(orderManager).createOrder(_tokenContract, msg.sender, _price, _token_amount, 0, 1);
        if (orderId > 0)
        {
            OrderManager(orderManager).handleMatchOrder(_tokenContract, orderId);
            OrderManager(orderManager).insertOrder(_tokenContract, orderId, msg.sender);
        }
        emit Deposit(_tokenContract, orderId, msg.sender, 1, 0, _token_amount);
