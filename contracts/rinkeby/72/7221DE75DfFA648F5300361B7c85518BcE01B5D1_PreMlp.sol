...
contract PreMlp is Ownable {
    ...
    function takePreMlp(
        ...
        pair.approve(address(popMarketplace), pendingPreMlp.liquidity);
        pendingPreMlp.token0.approve(
            address(popMarketplace),
            pendingPreMlp.bonusToken0
        );
        pendingPreMlp.token1.approve(
            address(popMarketplace),
    ...
    function releaseLiquidity(uint256 _PreMlpId) external {
        ...
        pair.transfer(pendingPreMlp.maker, liquidity / 2);
        pair.transfer(pendingPreMlp.taker, liquidity / 2);
        ...
        returns (uint256)
    {
        ...
        uint256 amountMin0 =
            _preMlp.amountToken0.sub(_preMlp.amountToken0.mul(10) / 1000);
        uint256 amountMin1 =
            _preMlp.amountToken1.sub(_preMlp.amountToken1.mul(10) / 1000);
