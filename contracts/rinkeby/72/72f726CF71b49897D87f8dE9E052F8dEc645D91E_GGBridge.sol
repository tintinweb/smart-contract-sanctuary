...
contract GGBridge is EIP712, Ownable {
    ...
    mapping(address => TokenType) public tokensType;
    ...
    event AddSuportedToken(address indexed token);
    ...
    constructor(
        ...
        TokenType[] memory tokensType_
    ) EIP712("GGBridge", "v1") {
        _signer = signer_;
        for (uint256 i = 0; i < tokensToSupport_.length; i++) {
            supportedTokens.add(tokensToSupport_[i]);
            tokensType[tokensToSupport_[i]] = tokensType_[i];
            emit AddSuportedToken(tokensToSupport_[i]);
        }
    }
    ...
    function adminWithdraw(address tokenAddress_) public onlyOwner {
        uint256 tokenBalance;

        if (!supportedTokens.contains(tokenAddress_)) {
            tokenBalance = tokenAddress_ == NATIVE
                ? address(this).balance
                : IERC20(tokenAddress_).balanceOf(address(this));
        }

        require(tokenBalance > 0, "Withdraw not allowed for supported token");

        if (tokenAddress_ == NATIVE) {
            payable(msg.sender).sendValue(tokenBalance);
        } else {
            IERC20(tokenAddress_).safeTransfer(msg.sender, tokenBalance);
        }
        emit LogWithdrawToken(msg.sender, tokenAddress_, tokenBalance);
    }

    function addSupportedToken(address _tokenAddress, TokenType tokenType_)
        external
        onlyOwner
        returns (bool success)
    {
        tokensType[_tokenAddress] = tokenType_;
        emit AddSuportedToken(_tokenAddress);
        return supportedTokens.add(_tokenAddress);
    }
    ...
    function removeSupportedToken(address _tokenAddress)
        ...
        returns (bool success)
    {
        delete tokensType[_tokenAddress];
        emit RemoveSupportedToken(_tokenAddress);
    ...
    function deposite(
        ...
        if (tokenAddress_ != NATIVE) {
            if (tokensType[tokenAddress_] == TokenType.ERC20_MINT_BURN_V2) {
                IERC20Mintable(tokenAddress_).burnFrom(msg.sender, amount);
            } else if (tokensType[tokenAddress_] == TokenType.ERC20_MINT_BURN) {
                IERC20Mintable(tokenAddress_).burn(msg.sender, amount);
            } else {
    ...
    function withdraw(
        ...
        if (tokenAddress_ == NATIVE) {
            ...
            if (TokenType.ERC20 == tokensType[tokenAddress_]) {
