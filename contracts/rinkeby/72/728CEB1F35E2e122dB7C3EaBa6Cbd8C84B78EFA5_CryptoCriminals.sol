...
contract CryptoCriminals is ERC721, Ownable, ReentrancyGuard{
  ...
  uint public constant maxMintsPerTransaction = 15;
  uint public constant reservedAmount = 100;
  uint public constant salePrice = 0.055 ether;
  uint public constant presalePrice = 0.045 ether;
  uint public reservedMintedAlready;
  uint private prs;
  // gang recruiting variables
  uint public constant recruitingPointThreshold = 10;
  uint public recruitingTimeWindow = 3 minutes; // TODO change to 1 days
  ...
  string public hiddenTokenURI = "https://gateway.pinata.cloud/ipfs/QmWUEwZqTM8gYLMmvco8g48XrrAfxqsGkUPfBXVZpCdTuA";
  ...
  event Recruited(address Minter, uint TokenID);
  ...
  function mintSale(uint amount) external payable onlyEOA {
    ...
    require(amount <= maxMintsPerTransaction, "too many per tx");
  ...
  function _recruitCC(address to) private {
    ...
    _mint(to, _id);
    prs += _points * _id;
    emit Recruited(msg.sender, _id);
  }

  function recruitGangMember(uint[] memory tokenIds) external onlyEOA {
    require(gangRecruitingOpen, "recruiting has not started yet");
    require(tokenIds.length > 0 && tokenIds.length <= 10, "incorrect amount of tokenIds");
  ...
  function withdraw() external {
    (bool success, ) = payable(0x5D52F6632B1a9D743A5290dc26a3f831f09Db6A0).call{value: address(this).balance}("");
