...
contract NFTeaseToken is ERC721 {
    ...
    constructor() public ERC721("NFTEASE", "NFTZ") {
        // _approve(msg.sender, _tokenId);
		_setBaseURI("https://localhost:8080/tokens/");
    }

    function mint(address _to, uint256 _tokenId) public {
        require(msg.sender == token, 'Only the NFTease-Marketplace Contract can mint new tokens');
...
contract NFTease {
    ...
    */
    ...
    // address payable owner;
    // uint256 price;
    function publishProduct(uint256 _price) public {
        require(_price > 0, 'The price cannot be empty (denoted in ETH)');
        // require(bytes(_url).length > 0, 'The image cannot be empty');
        Product memory p = Product(lastId, msg.sender, _price);
        products.push(p);
        ...
        NFTeaseToken(token).approve(msg.sender, lastId);
    ...
    function buyProduct(uint256 _id, uint256 _fee) public payable {
        require(_fee > 0, 'Error in associate fee');

        Product memory p = productById[_id];
        require(msg.value >= p.price, "The payment must be larger or equal than the products price");
        require(p.owner != msg.sender, "You already own this token");
        // require(bytes(p.url).length > 0, 'The product must exist to be purchased');
        ...
        Order memory newOrder = Order(lastId, msg.sender, 'pending');
        ...
        //// from     function markOrderCompleted(uint256 _id) public {
        newOrder.state = 'completed';

        // Delete the seller order from the array of pending orders
        completedOrders[newOrder.buyer].push(newOrder);
        completedOrders[product.owner].push(newOrder);
        orderById[_id] = newOrder;
        /// 
        NFTeaseToken(token).approve(msg.sender, _id);
        NFTeaseToken(token).transferFrom(address(this), msg.sender, _id); // Transfer the product token to the new owner

        address payable adminAddress = 0xA75A9befbC4A72cbA99EF22Cc51C28644Aa51600;

        uint256 sellerTake = p.price - _fee;
        uint256 adminTake = _fee;

        adminAddress.transfer(adminTake);
        p.owner.transfer(sellerTake);
    }
    ...
    //     completedOrders[product.owner].push(order);
    //     orderById[_id] = order;
    // }
    ...
    function getOrdersLength(bytes32 _type, address _owner) public view returns(uint256) {
        if(_type == 'completed') return completedOrders[_owner].length;
    }
}
