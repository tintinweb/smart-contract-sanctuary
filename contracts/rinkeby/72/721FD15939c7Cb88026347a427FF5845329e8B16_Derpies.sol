...
contract BearsContract {
    function ownerOf(uint256 tokenId) public view virtual returns (address) {}
}

contract Derpies is Delegated, ERC721EnumerableB, PaymentSplitter {
  using Strings for uint;

  BearsContract bearsContract;

  uint public MAX_SUPPLY = 6969;

  bool public isActive   = false;
  bool public isClaim    = false;
  uint public maxOrder   = 2;
  uint public price      = 0.02 ether;
  uint public reservedLeft = 100;
  ...
  mapping(address => uint[]) private _balances;
   ...
   address[] private addressList = [0x13d86B7a637B9378d3646FA50De24e4e8fd78393];
  uint[] private shareList = [100];

  mapping(uint => bool) private bearsUsedList;

  constructor(address bearsAddress)
    Delegated()
    ERC721B("DERPIES", "DERP")
    PaymentSplitter(addressList, shareList)
  {bearsContract = BearsContract(bearsAddress);}
  ...
  function mint( uint quantity ) external payable {
    ...
    function Claim (uint[] calldata bearsIds) external {
        uint256 supply = totalSupply();
        require(isClaim, 'Presale not active');

        for (uint i = 0; i < bearsIds.length; i++) {
            require(bearsContract.ownerOf(bearsIds[i]) == msg.sender, "Not a Bears owner");
            require(bearsUsedList[bearsIds[i]] == false, 'Bearsalready used');
        }

        require(supply + bearsIds.length <= MAX_SUPPLY - reservedLeft, 'Exceeds MAX_SUPPLY');

        for (uint256 i = 0; i < bearsIds.length; i++) {
            // Start index at 1
            bearsUsedList[bearsIds[i]] = true;
            _safeMint(msg.sender, supply + i + 1);
        }
         reservedLeft -= bearsIds.length;
    }
  ...
  function setClaim(bool newState) external onlyDelegates{
    isClaim = newState;
  }
  ...
  function isBearsClaimed(uint bearsId) public view returns (bool) {
        require(bearsId < 1690 && bearsId > 0, "Invalid Bears id");
        return bearsUsedList[bearsId];
    }
