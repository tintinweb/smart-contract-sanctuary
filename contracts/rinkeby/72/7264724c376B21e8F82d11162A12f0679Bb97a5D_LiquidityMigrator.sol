...
contract LiquidityMigrator is ILiquidityMigrator, IERC721Receiver, Context {
    ...
    function migrateV3(MigrateParams calldata params, bytes calldata data) external {
        MigrateVars memory localVars;
        (uint24 fee, ) = abi.decode(data, (uint24, uint256));
        address pair = IUniswapV3Factory(UNISWAP_FACTORY).getPool(params.token0, params.token1, fee);
        INonfungiblePositionManager positionManager = INonfungiblePositionManager(
            POSITION_MANAGER
        );
        positionManager.safeTransferFrom(_msgSender(), address(this), params.uniswapTokenId);
        ...
        TransferHelper.safeApprove(params.token0, params.unipilotAddress, localVars.amount0V3);
        TransferHelper.safeApprove(params.token1, params.unipilotAddress, localVars.amount1V3);
        ...
        if (localVars.amount0V3 < amount0Total) {
            TransferHelper.safeApprove(params.token0, params.unipilotAddress, 0);

            uint256 refund0 = amount0Total - localVars.amount0V3;
        ...
        if (localVars.amount1V3 < amount1Total) {
            TransferHelper.safeApprove(params.token1, params.unipilotAddress, 0);

            uint256 refund1 = amount1Total - localVars.amount1V3;
