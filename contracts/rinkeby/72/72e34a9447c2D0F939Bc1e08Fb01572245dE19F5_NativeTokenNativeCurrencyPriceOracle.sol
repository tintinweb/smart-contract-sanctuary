...
abstract contract Job is Context {
    ...
    modifier needsExecution() {
        if (!IMaster(MASTER).initializeWork(_msgSender())) revert NotAWorker();
        _;
...
interface IMaster {
    ...
    function worker(address _worker) external view returns (bool);

    function initializeWork(address _worker) external returns (bool);
