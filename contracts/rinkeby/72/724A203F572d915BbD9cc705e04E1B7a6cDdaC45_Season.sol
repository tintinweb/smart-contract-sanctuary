...
contract Season is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint256 public packSize = 15;

    bool public seedSet = false;
    string public randomSeed;
    ...
    function _randomSeed() internal view returns (string memory) {
        return randomSeed;
    }

    function setRandomSeed(string memory seed) public onlyOwner {
        seedSet = true;
        randomSeed = seed;
    }

    function getCardTitle(uint256 tokenId, uint256 offset) public view returns (string memory) {
        require(seedSet == true, "Cards not revealed yet.");
        require(offset < 15, "only 15 card titles per pack");
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (seedSet == false) {
            output = string(
                abi.encodePacked(
                    "<svg width='350' height='350' viewBox='0 0 350 350' fill='none' xmlns='http://www.w3.org/2000/svg'> <rect width='350' height='350' fill='url(#paint0_linear)'/> <rect width='318' height='318' transform='translate(16 16)' fill='black'/> <rect width='318' height='318' transform='translate(16 16)' fill='black'/> <text fill='white' xml:space='preserve' style='white-space: pre;' font-family='Georgia' font-size='12' letter-spacing='0em'>",
                    "</text><text fill='white' xml:space='preserve' style='white-space: pre;' font-family='Georgia' font-size='12' font-weight='bold' letter-spacing='0em'><tspan x='32' y='62.1865'>EXPANSION PACK</tspan></text> <text fill='#CE9EFF' xml:space='preserve' style='white-space: pre;' font-family='Georgia' font-size='16' font-weight='bold' letter-spacing='0.16em'><tspan x='32' y='43.582'>15 ADVENTURE CARDS</tspan></text> <defs> <linearGradient id='paint0_linear' x1='175' y1='350' x2='175' y2='0' gradientUnits='userSpaceOnUse'> <stop stop-color='#850EFF'/> <stop offset='1' stop-color='#320065'/> </linearGradient> </defs></svg>"
                )
            );
            ...
            json = Base64.encode(
                bytes(
                    string(
                        abi.encodePacked(
                            '{"name": "Adventure Cards Expansion Packs", "description": "Cards is an on chain, collectable card game, based on crypto primitives. Each Expansion Pack is 15 cards, a mix of rarities and types. The packs can be unbundled and used however you like.", "image": "data:image/svg+xml;base64,',
        ...
        for (uint256 i = 0; i < packSize; i = i + 1) {
            uint256 offset = (i + 1) * 16 + 73;
            output = string(
                abi.encodePacked(
                    output,
                    "<tspan x='29' y='",
    ...
    constructor() ERC721("Pack", "PACK") Ownable() {}
}
