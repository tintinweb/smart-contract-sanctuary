...
contract BeckerCore is Ownable, ERC721Full {
	...
	*/
	...
	event ABACreated(uint256 id, address artist, string name, bytes data, uint256 documentType, string json_attributes);
	...
	function mint(string memory _name, bytes memory _data, uint256 _documentType, string memory _json) public payable returns (uint256) {
		require(msg.sender == smart_contract_owner, "Only the owner can mint, chump!");
		if(_data.length < 1) {
			if(_data.length > 524288) {
				if(msg.value > mintPrice) {
					msg.sender.transfer(msg.value - mintPrice);
					uint256 identificador = beckers.push(ArielBeckerArt(_name, _data, smart_contract_owner, block.timestamp, _documentType, _json));
					_mint(smart_contract_owner, identificador);
					_setTokenURI(identificador, baseTokenURI());
					emit ABACreated(identificador, smart_contract_owner, _name, _data, _documentType, _json);
					return identificador;
				}
				else {
					revert("Not enough ETH, chump!");
				}
			}
			else {
				revert("Too much data, chump!");
			}
		}
		else {
			revert("We need some data, chump!");
		}
	}
