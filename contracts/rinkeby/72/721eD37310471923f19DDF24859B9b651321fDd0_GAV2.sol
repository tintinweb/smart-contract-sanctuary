...
contract GAV2 is ERC20, Ownable, Editor {
    using SafeMath for uint256;
    ...
    constructor(address payable _vaultAddress, uint256 _liquidityFee, uint256 _vaultFee) ERC20("GAV2", "GAv2") {
    ...
    function excludeFromFees(address account, bool excluded) public OwnerOrEditor{
        require(_isExcludedFromFees[account] != excluded, "GAv2: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;
        emit ExcludeFromFees(account, excluded);
    }
	
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GAv2: The Uniswap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
	
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "GAv2: Automated market maker pair is already set to that value");
	...
	function updateVaultFee(uint256 newFee) public OwnerOrEditor {
        require(newFee >= 0 && newFee <= 10000, "GAv2: vaultFee fee must be between 0 and 10000");
        vaultFee = newFee;
    }
	
	function updateLiquidityFee(uint256 newFee) public OwnerOrEditor{
        require(newFee >= 0 && newFee <= 10000, "GAv2: Liquidity fee must be between 0 and 10000");
        liquidityFee = newFee;
    }
