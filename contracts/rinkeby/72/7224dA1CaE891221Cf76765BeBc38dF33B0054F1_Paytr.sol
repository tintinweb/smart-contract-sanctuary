...
contract Paytr{
   ...
   constructor() {
        ...
        returns (bool)
    {
        ...
        Invoice memory invoiceData = Invoice(amount, msg.sender, supplier, dueDate, transactionDate, paymentReference);
    ...
    function transferAndSupplyErc20ToCompound (
        ...
        transactionDate = block.timestamp;
        dueDate = block.timestamp + (dueDate * 1 seconds);
        InvoiceErc20 memory invoiceDataErc20 = InvoiceErc20(amount, msg.sender, supplier, dueDate, transactionDate, paymentReference, _erc20Contract);
    ...
    function createDueListAndPay() public payable {
        ...
        for (uint i = 0; i < invoices.length; i++) {
             ...
             if (block.timestamp >= invoices[i].dueDate) {
                ...
                dueInvoice memory dueList = dueInvoice(amount, sender, supplier, dueDate, transactionDate, paymentReference);
    ...
    function createDueListAndPayErc20() public payable {
        ...
        for (uint i = 0; i < invoicesErc20.length; i++) {
             ...
             if (block.timestamp >= invoicesErc20[i].dueDate) {
                ...
                dueInvoiceErc20 memory dueListErc20 = dueInvoiceErc20(amount, sender, supplier, dueDate, transactionDate, paymentReference, tokenAddress);
