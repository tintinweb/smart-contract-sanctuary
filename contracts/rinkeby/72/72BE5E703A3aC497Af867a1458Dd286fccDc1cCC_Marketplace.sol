...
contract Marketplace is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    // = REMOVAL OR REORDER OR CHANGING THE TYPE =
    // =       OF VARIABLES WILL RESULT          =
     ...
     */
    function createMarketSale(address nftContract, uint256 itemId, address to) external payable nonReentrant whenNotPaused {
     ...
     */
    function setListingPrice(uint256 _listingPrice) external onlyOwner whenNotPaused {
        listingPrice = _listingPrice;
    }
    ...
    function safePullERC20(address erc20) external onlyOwner {
        IERC20Upgradeable(erc20).safeTransfer(msg.sender, IERC20Upgradeable(erc20).balanceOf(address(this)));
    }
