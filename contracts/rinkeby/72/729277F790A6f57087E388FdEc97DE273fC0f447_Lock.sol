...
contract Lock is BaseGovernanceWithUserUpgradable, ERC721EnumerableUpgradeable {
    ...
    function _initializeBeneficiaries(bytes memory _data,  uint256 _lockedAmount) internal initializer {
    ...
    function getInfoBySingleID(uint256 id) view external returns (address owner, uint256 lockedAmount, uint256 claimable, uint256 startLock, uint256 endLock){
        owner = ownerOf(id);
        (, uint256 startLock, uint256 initAmount, uint256 claimed) = depositManager.getProperties(id);
        endLock = schedule.endLockTime();
        lockedAmount = initAmount - claimed;
        claimable = schedule.unlockedAmount(startLock, initAmount) - claimed;
    ...
    function getInfoByManyIDs(uint256[] memory ids) view external returns(address[] memory owners, uint256[] memory lockedAmount, uint256[] memory claimable) {
        ...
        for(uint256 i; i < length; i++) {
            (, uint256 startLock, uint256 initAmount, uint256 claimed) = depositManager.getProperties(ids[i]);
            owners[i] = ownerOf(ids[i]);
            lockedAmount[i] = initAmount - claimed;
            claimable[i] = schedule.unlockedAmount(startLock, initAmount) - claimed;
    ...
    function removeBeneficiaries(bytes calldata data) external onlyBeneficiaryManager {
        ...
        uint[] memory IDs = depositManager.removeDeposits(data);
        for (uint256 i = 0; i < IDs.length; i++) {
            (, uint256 additionTime, uint256 initialAmount, uint256 claimed) =  depositManager.getProperties(IDs[i]);
            uint256 unlockedAmount = schedule.unlockedAmount(additionTime, initialAmount) - claimed;
            require(unlockedAmount != 0, "ERROR: Not enought free tokens - Schedule");
            tokenERC20.safeTransfer(ownerOf(IDs[i]), unlockedAmount);
            tokenERC20.safeTransfer(_msgSender(), (initialAmount - unlockedAmount));
        }
    }
    ...
    * @dev The data to proof the ownership is only require if the NFT is not minted yet
    ...
    function claimUnlocked(uint256 nftId, bytes calldata ownershipProof) external {
        (bool _success, uint256 _ID) = _isApprovedOrOwner(nftId, _msgSender(), ownershipProof);
        require(_success, "ERROR: You are not the owner or are approved for this NFT");
        (, uint256 additionTime, uint256 initialAmount, uint256 claimed) =  depositManager.getProperties(_ID);
        uint256 unlockedAmount = schedule.unlockedAmount(additionTime, initialAmount) - claimed;
        depositManager.updateClaimedAmount(_ID, unlockedAmount);
     ...
     * @dev The data to proof the ownership is only require if the NFT is not minted yet
     */
    function split(uint256 originId, uint256[] memory splitParts, address[] memory addresses, bytes calldata ownershipProof) external {
        (bool _success, uint256 _ID) = _isApprovedOrOwner(originId, _msgSender(), ownershipProof);
        require(_success, "ERROR: You are not the owner or are approved for this NFT");
        uint256[] memory newIDs = depositManager.split(_ID, splitParts, addresses);
     ...
     * @dev This function can be called only for DepositManager address
     * @param IDs List of NFT IDs to be burned
     */
    function burnNFTs(uint256[] memory IDs) external onlyDepositManager {
        for(uint256 i; i < IDs.length; i++) {
            _burn(IDs[i]);
        }
    }

    function _isApprovedOrOwner(uint256 _nftId, address _beneficiary, bytes calldata _data) internal returns(bool, uint256) {
        if(_data.length == 0) {
