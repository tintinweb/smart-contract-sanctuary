...
contract FortubeDao is ERC20Upgradeable {
    ...
    // event MaxSponsorOutRatioChanged(uint oldRatio, uint newRatio);
    ...
    // constructor(uint256 _slope, address _forToken)
    //     public
    //     ERC20("FortubeDao", "FDAO")
    // {
    //     slope = _slope;
    //     forToken = _forToken;
    //     burnDelay = 2 days;
    ...
    function initialize(
        ...
        sellDelay = 2 days;
        sellWindow = 1 days;
        // maxSponsorOutRatio = 40;
        operator = msg.sender;
        multiSig = _multiSig;
    }

    // remember to init this
    function init(uint8 digits) external {
        ...
        onlyMultiSig
    {
        ...
        // require(amount <= totalFor.mul(maxSponsorOutRatio).div(100), "Too much");
        require(sponsorTo != address(0), "Invalid address");

        totalFor = totalFor.sub(amount);
        console.log("totalFor now is ", totalFor);
        sync();
        assert(slope > 0);
        ...
        returns (uint256 daoAmount)
    {
        // int256 radicand =
        //     int256(
        //         _amount.mul(2).mul(1e18).div(slope).add(
        //             totalSupply().mul(totalSupply()).div(1e18)
        //         )
        //     );
        // daoAmount = uint256(
        //     uint256(fixidity.root_n(radicand, 2)).sub(totalSupply())
        // );
        uint a = slope.mul(totalSupply()).div(1e18).add(intercept);
        a = a.mul(a).div(1e18);
        int radicand = int(a.add(slope.mul(_amount).mul(2).div(1e18)));
        uint b = uint256(fixidity.root_n(radicand, 2)).sub(slope.mul(totalSupply()).div(1e18).add(intercept));
    ...
    {
        // uint256 minuend = totalSupply().mul(_sellAmount).mul(slope).div(1e36);
        // uint256 subtractor =
        //     _sellAmount.mul(_sellAmount).mul(slope).div(2).div(1e36);
        // amount = minuend.sub(subtractor);
        uint a = slope.mul(totalSupply()).div(1e18).sub(slope.mul(_sellAmount).div(2).div(1e18)).add(intercept);
    ...
    function calcNewSlope(uint256 _totalFor, uint256 supply)
        ...
        returns (uint256)
    {
        // uint256 dividend = _totalFor.mul(2);
        // uint256 divisor = supply.mul(supply).div(1e18);
        // return dividend.mul(1e18).div(divisor);
        console.log("_totalFor is ", _totalFor);
        console.log("supply is ", supply);
        console.log("first is ", _totalFor.mul(2).mul(1e18).div(supply));
        console.log("second is ", intercept.mul(2));
        uint dividend = _totalFor.mul(2).mul(1e18).div(supply).sub(intercept.mul(2));
        return dividend.mul(1e18).div(supply);
    }

    ///@notice 获取管理员最大可提取的数量
    function calcMaxSponsorOut() public view returns (uint) {
        return totalSupply().mul(totalSupply()).mul(slope).div(1e18).div(1e18).div(2);
    }
    ...
    // function setMaxSponsorOutRatio(uint _maxSponsorOutRatio) external onlyOperator {
    //     uint oldRatio = _maxSponsorOutRatio;
    //     maxSponsorOutRatio = _maxSponsorOutRatio;

    //     emit MaxSponsorOutRatioChanged(oldRatio, maxSponsorOutRatio);
    // }
