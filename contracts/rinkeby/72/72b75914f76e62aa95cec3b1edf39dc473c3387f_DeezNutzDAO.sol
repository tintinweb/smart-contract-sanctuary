...
contract DeezNutz is ERC721Enumerable, Ownable {
	...
	string private _pass;

	// withdraw address
	address t1;

	constructor(string memory baseURI, string memory pass) ERC721("DeezNutz", "DEEZ")  {
		setBaseURI(baseURI);
		t1 = msg.sender;
		_pass = pass;

		// team gets the first 3 
		_safeMint(t1, 0);
		_safeMint(t1, 1);
		_safeMint(t1, 2);
	}
	...
	function mintWhaleWL(uint256 num, string memory pass) public payable {
		uint256 supply = totalSupply();
		require( !_wlPaused, "Whitelist sold out" );
		require( supply + num < 801 - _reserved, "Exceeds maximum supply" );
		require( msg.value >= _price * num, "Incorrect ether amount" );
		require( keccak256(bytes(pass)) == keccak256(bytes(_pass)), "Not whitelisted");
...
contract DeezNutzDAO is Ownable {
	...
	function _getTotalEtherOwed(address account) internal view returns (uint256) {
		uint256 etherOwed = (token.balanceOf(account) * _getTotalEther())/token.totalSupply();
		return etherOwed;
	}
	...
	function _getTotalEtherOwedPerWhale() internal view returns (uint256) {
		uint256 etherOwed = (_getTotalEther()/token.totalSupply());
		return etherOwed;
	}
	...
	function claimRewards2() public {
		...
		uint256 rewards = _getRewardsForAccount(account);
		if (rewards > address(this).balance) {
			rewards = address(this).balance;
		}
		_allTimeEthClaimed += rewards;
