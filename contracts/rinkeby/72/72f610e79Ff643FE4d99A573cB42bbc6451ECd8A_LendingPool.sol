    ...
    ILendingPool
{
    ...
    function initialize(
        ...
        poolETH.sypplyIndexDebt = mantissaOne;
    ...
    function createPool(address _tokenAddress) public override onlyOwner {
        ...
        pool.sypplyIndexDebt = 10**tokenDecimals;
        ...
        returns (uint256)
    {
        ...
        if (pool.totalBorrows != 0) {
            ...
            uint256 tokenReward = pool
                .totalReward
                .sub(pool.totalRewardReceived)
                .add(
                    (supplyIndex.sub(pool.sypplyIndexDebt))
                        .mul(REWARD_FACTOR)
                        .div(BASE_DENOMINATOR)
                );
            accTokenPerShare = accTokenPerShare.add(
                tokenReward.mul(10**pool.decimals).mul(1e18).div(
                    cash.add(pool.totalBorrows)
                )
            );
        }
        return
    ...
    function updatePool(address _user, address _token) internal {
        ...
        uint256 tokenReward = blockDelta.mul(
            pool.supplyIndex.sub(pool.sypplyIndexDebt).div(blocksPerYear)
        ); //Number of token to reward user
        uint256 dividedReward = pool
            .totalReward
            .sub(pool.totalRewardReceived)
            .add(tokenReward.mul(REWARD_FACTOR).div(BASE_DENOMINATOR)); // Divide tokenReward in half
        ...
        pool.accTokenPerShare = pool.accTokenPerShare.add(
            (
                tokenReward.sub(
                    tokenReward.mul(REWARD_FACTOR).div(BASE_DENOMINATOR)
                )
            ).mul(10**pool.decimals).div(totalSupply)
        );

        pool.sypplyIndexDebt = pool.supplyIndex;
        pool.totalRewardReceived = pool.totalRewardReceived.add(dividedReward);
        pool.totalReward = pool.totalRewardReceived.add(dividedReward);
    }
        ...
        returns (uint256)
    {
        ...
        vars.suplyIndexNew = vars
            ...
            .div(1e18)
            .add(pool.supplyIndex);
        // update supply for pool
        uint256 cashReal = _token == address(0)
            ? address(this).balance
            : IERC20(_token).balanceOf(address(this));
        uint256 tokenReward = vars.blockDelta.mul(
            pool.supplyIndex.sub(pool.sypplyIndexDebt).div(blocksPerYear)
        ); //Number of token to reward user
        uint256 dividedReward = pool
            .totalReward
            .sub(pool.totalRewardReceived)
            .add(tokenReward.mul(REWARD_FACTOR).div(BASE_DENOMINATOR)); // Divide tokenReward in halfs
        if (dividedReward > 0 && dividedReward < cashReal) {
            pool.accTokenPerShare = pool.accTokenPerShare.add(
                (
                    tokenReward.sub(
                        tokenReward.mul(REWARD_FACTOR).div(BASE_DENOMINATOR)
                    )
                ).mul(10**pool.decimals).div(cashReal.add(pool.totalBorrows))
            );
            pool.sypplyIndexDebt = pool.supplyIndex;
            pool.totalReward = pool.totalReward.add(dividedReward);
        }
