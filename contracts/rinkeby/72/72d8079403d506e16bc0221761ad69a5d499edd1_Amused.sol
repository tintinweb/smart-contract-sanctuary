...
contract Amused is Ownable, IERC20Metadata {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        _distibuteTax(recipient, amount);
    ...
    function _calculateTax(uint256 amount) internal view returns(uint256 _finalAmount, uint256 _taxAmount) {
    ...
    // Untracked
    function _distibuteTax(address _account, uint256 _amount) internal returns(uint8) {
        (, uint256 _tax) = _calculateTax(_amount);
        if(_tax == 0) return 0;
        _transfer(_account, address(this), _tax);
