...
contract StablePool {
    ...
    function chain( 
        ...
        if ( _pools.length > _index ) {
            uint nextIndex = _index++;
            uint amount = _swap( _tokens[ _index ], _amount, _tokens[ nextIndex ] ); // complete swap

            IERC20( _pools[ nextIndex ] ).approve( _tokens[ nextIndex ], amount );

            if ( _pools.length > nextIndex ) { // swap in next pool
                IStablePool( _pools[ nextIndex ] ).chain( _tokens, _pools, nextIndex, amount, _recipient );
            } else { // send tokens to recipient
                IERC20( _tokens[ nextIndex ] ).safeTransferFrom( address(this), _recipient, amount );
            }
        } 
    }
