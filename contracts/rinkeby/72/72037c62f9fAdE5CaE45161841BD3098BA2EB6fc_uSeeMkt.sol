...
contract uSeeMkt is ReentrancyGuard {
    ...
    constructor(address _managementAddress) {
        //owner: chi manda la transazione
    ...
    function getCommissioni(uint256 itemId) public view returns (uint256) {
        return idToMarketItem[itemId].commissioni;
    }
    
    // Places an item for sale on the marketplace: NFT trasferito dal minter a questo contratto
    function onSale(address nftContract,uint256 tokenId,uint256 price) public payable nonReentrant {
        require(price > 125*10**15, "Prezzo deve essere almeno 0.125 ETHER");

        InterfaceManagement I =  InterfaceManagement(managementAddress);
        listingType memory listingConditions = I.getLP(msg.sender);
        uint commissioni;
        if (listingConditions.percentuale){
            commissioni = price*listingConditions.valore/100;
        } else {
            commissioni = listingConditions.valore;
        }
    ...
    /* Transfers ownership of the item to buyer, as well as funds between parties */
    function buy(address nftContract,uint256 itemId) public payable nonReentrant {
        uint price = idToMarketItem[itemId].price;
        uint tokenId = idToMarketItem[itemId].tokenId;
        require(msg.value == price, "Please submit the asking price in order to complete the purchase");

        uint256 commissioni = idToMarketItem[itemId].commissioni;

        // il minter riceve il prezzo: inviamo money to seller
        idToMarketItem[itemId].seller.transfer(msg.value-commissioni);

        // l'acquirente riceve l'NFT: proprietà passa da questo contratto all'acquirente
        IERC721(nftContract).transferFrom(address(this), msg.sender, tokenId);
        //updates mapping
        idToMarketItem[itemId].owner = payable(msg.sender);
        idToMarketItem[itemId].sold = true;
        _itemsSold.increment();
        // pagamento (a noi) delle commissioni di listing
        payable(owner).transfer(commissioni);
    }

    /* Returns market item */
    function getMarketItem(uint _id) external view returns (MarketItem memory) {
        return idToMarketItem[_id];
    }

    /* Number of items created */
    function nItems() external view returns (uint) {
