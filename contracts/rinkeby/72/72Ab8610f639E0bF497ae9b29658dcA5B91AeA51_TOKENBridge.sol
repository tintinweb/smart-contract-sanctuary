...
contract TOKENBridge is ReentrancyGuard, Context, Ownable {
  ...
  uint256 private maxTransaction = 100000000000000000000000;  // 100K
  ...
  function setMaxTransaction(uint256 _maxTransaction) external onlyOwner nonReentrant {
  ...
  function exchangeWSGToken(uint256 _amt) external payable nonReentrant {
    ...
    if(_amt > maxTransaction) {
      require(WSG_TOKEN.transferFrom(msg.sender, address(this), maxTransaction), "Unable to transfer funds");
      emit ExchangeWSG(msg.sender, maxTransaction, msg.value);
    } else {
  ...
  function exchangeGASPAYToken(uint256 _amt) external payable nonReentrant {
    ...
    feeAddress.transfer(msg.value);
    if(_amt > maxTransaction) {
      require(GASPAY_TOKEN.transferFrom(msg.sender, address(this), maxTransaction), "Unable to transfer funds");
      emit ExchangeGASPAY(msg.sender, maxTransaction, msg.value);
    } else {
  ...
  function exchangeGASGToken(uint256 _amt) external payable nonReentrant {
    ...
    feeAddress.transfer(msg.value);
    if(_amt > maxTransaction) {
      require(GASG_TOKEN.transferFrom(msg.sender, address(this), maxTransaction), "Unable to transfer funds");
      emit ExchangeGASG(msg.sender, maxTransaction, msg.value);
    } else {
