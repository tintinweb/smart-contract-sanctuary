...
contract cows is Ownable, ERC721 {
    ...
    constructor() ERC721("Crunchy Cows", "CCows"){
   ...
   function update_presale(uint one, uint two, uint three, uint total)external onlyOwner{
        pre_one_package=one;
        pre_two_package=two;
   ...
   function buy_presale(uint _count) public payable{
     require(totalSupply + _count <= presale_total,"Required number of Tokens are not available");
 ...
 function _mintcows(uint numberOfTokens, address sender) internal {
        ...
        for(uint i = 0; i < numberOfTokens; i++)
            _safeMint(sender, totalSupply + 1 + i);
