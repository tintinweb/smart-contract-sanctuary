...
contract Sorbettiere is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accIcePerShare; // Accumulated SUSHIs per share, times 1e12. See below.
    }
    
    IERC20 public ice; // The ICE TOKEN!!
    
    uint256 public icePerSecond; // Ice tokens created per second.
    ...
    uint256 public startTime; // The timestamp when ICE mining starts.
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && stakingSupply != 0) {
            uint256 multiplier =
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 iceReward =
            multiplier * icePerSecond * pool.allocPoint / totalAllocPoint;
        pool.accIcePerShare += iceReward * 1e12 / stakingSupply;
        pool.lastRewardTime = block.timestamp;
    }
