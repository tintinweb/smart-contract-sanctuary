...
contract FlokiInuNFTRewardPickUp is Ownable {
    ...
    mapping (bytes8 => Reward) private _rewards;
    ...
    function hasReward(address user) public view returns (bool) {
        return _rewards[_encode(user)] != Reward.None;
    }
    ...
    function addBatch(Reward reward, address[] calldata batch) external onlyOwner {
        for (uint256 i = 0; i < batch.length; i++) {
            _rewards[_encode(batch[i])] = reward;
        }
    }
