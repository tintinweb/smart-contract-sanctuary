...
contract CurseWords is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    string private rarestCurseWord = "*****";
    ...
    function getCurseWord(uint256 tokenId) public view returns (string memory) {
        return multipluck(tokenId, "CURSE", curseSuiteTM);
    }

    function getSecondaryCurseWord(uint256 tokenId) public view returns (string memory) {
        return multipluck(tokenId, "AUXCURSE", curseSuiteTM);
    }
    ...
    function getMainSize(uint256 tokenId) public view returns (string memory) {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        //     // getMainSize(tokenId),
        //     "px; } .aux{fill: #",
        //     getSecondaryColor(tokenId),
        //     '; font-family: arial; font-size: 10',
        //     // getAuxSize(tokenId),
        //     'px}</style><rect width="100%" height="100%" fill="black" />'
        // ));
        
        // parts[1] = string(abi.encodePacked('<text x="', randomX, '" y="20" class="base">'));

        // parts[2] = getCurseWord(tokenId);

        // parts[3] = string(abi.encodePacked('</text><text x="10" y="', randomY, '" class="aux">'));

        // parts[4] = getSecondaryCurseWord(tokenId);

        // parts[5] = '</text></svg>';
        
        parts[0] = string(abi.encodePacked(
            '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: #EF476F',
            "; font-family: serif; font-size: 14px; } .aux{fill: #118AB2",
            '; font-family: arial; font-size: 10px}</style><rect width="100%" height="100%" fill="black" />'
        ));

        parts[1] = string(abi.encodePacked('<text x="', uint2str(randomX),'" y="20" class="base">'));
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Curse Word #', toString(tokenId), '", "description": "Curse Word #', toString(tokenId), '", "description": "Curse Words are randomly generated, minimally artistic curse words generated on-chain. They do nothing...?", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    function claim(uint256 tokenId) public nonReentrant {
        require(tokenId > 0 && tokenId < 1608, "Token ID invalid");
        _safeMint(_msgSender(), tokenId);
    }
    
    function ownerClaim(uint256 tokenId) public nonReentrant onlyOwner {
        require(tokenId > 1609 && tokenId < 1696, "Token ID invalid");
        _safeMint(owner(), tokenId);
    }
    
    function ownerClaimAll() public nonReentrant onlyOwner {
        for (uint i = 1610 ; i < 1696 ; i++) {
            _safeMint(owner(), i);
        }
    }
    ...
    constructor() ERC721("Curse Words", "CURSE") Ownable() {}
}
