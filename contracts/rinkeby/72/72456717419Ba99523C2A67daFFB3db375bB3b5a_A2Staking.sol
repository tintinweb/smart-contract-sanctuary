...
contract A2Staking is Context, Ownable, ReentrancyGuard {
    ...
    event Staked(uint256 indexed pool, address indexed beneficiary, uint256 stake, uint256 stakeAmount, uint256 duration);
    event Unstaked(uint256 indexed pool, address indexed beneficiary, uint256 stake, uint256 unstakeAmount, uint256 penalty);
    ...
    function addPool(IERC20 token, uint256 cliff, uint256 duration, uint256 penalty) external onlyOwner returns(uint256) {
        ...
        emit PoolAdded(poolId, address(token));
        return poolId;
    }
    ...
    returns(uint256[] memory stakeStarts, uint256[] memory stakeAmounts, uint256[] memory penalties) {
        ...
        for(uint256 i=0; i<userStakes.length; i++) {
            ...
            if (penaltieForFullUnstake == 0) {
                ...
                penalties[i] = penaltieForFullUnstake.div(stakee.amount);
            }
        }
    }

    function allUserStakesAndPenalties(address beneficiary) external view
    returns(uint256[] memory stakePools, uint256[] memory stakeStarts, uint256[] memory stakeAmounts, uint256[] memory penalties) {
        uint256 totalStakes;
        ...
        for(uint256 p=0; p<pools.length; p++){
            ...
            for(uint256 i=0; i<userStakes.length; i++) {
                ...
                if (penaltieForFullUnstake == 0) {
                    ...
                    penalties[idx] = penaltieForFullUnstake.div(stakee.amount);
                }
                idx++;
            }
        }
    ...
    function stakeInternal(address beneficiary, uint256 poolId, uint256 amount) internal {
        ...
        emit Staked(poolId, beneficiary, stakeId, amount, pool.duration);
    }
    ...
    function unstakeExactStakeInternal(address beneficiary, uint256 poolId, uint256 stakeId, uint256 unstakeAmount) internal {
        ...
        emit Unstaked(poolId, beneficiary, stakeId, unstakeAmount, penaltyAmount);
    ...
    function unstakeExactStakesInternal(address beneficiary, uint256 poolId, uint256[] memory stakeIds, uint256[] memory amounts) internal {
        ...
        for (uint256 i=0; i<stakeIds.length; i++) {
            ...
            emit Unstaked(poolId, beneficiary, stakeIds[i], amounts[i], penaltyAmount);
    ...
    function calculateUnstakePenalty(Pool storage pool, Stake storage stakee, uint256 unstakeAmount) internal view returns(uint256) {
        ...
        uint256 penaltyTimePassed = timePassed - pool.cliff;
        uint256 penaltyTimePeriod = pool.duration.sub(pool.cliff); 
        // penaltyTimePeriod != 0 because if pool.duration == pool.cliff, then one of conditions above will be true
        return unstakeAmount.mul(pool.penalty).mul(penaltyTimePassed).div(penaltyTimePeriod).div(EXP);
    }
