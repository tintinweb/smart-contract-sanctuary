...
contract Testcities is ERC721, Ownable, IMintedBeforeReveal {
    ...
    uint256 public constant INSTANT_REVEAL = 1626026400;
    ...
    function mintAMonstro(uint256 numberOfMonstro) public payable {
        // Some exceptions that need to be handled.
        require(saleActive, "Sale has not started yet");
        ...
        for (uint i = 0; i < numberOfMonstro; i++) {
            ...
            if (block.timestamp < INSTANT_REVEAL) {
                _mintedBeforeReveal[mintIndex] = true;
            }
            _safeMint(msg.sender, mintIndex);
        }

        if (startingIndexBlock == 0 && (totalSupply() == MAX_MONSTRO_SUPPLY || block.timestamp >= INSTANT_REVEAL)) {
