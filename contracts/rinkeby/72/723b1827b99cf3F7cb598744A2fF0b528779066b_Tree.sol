...
contract Tree is ERC721Enumerable, ReentrancyGuard, Ownable {
  ...
  function grow(address friend, uint256 index) public payable {
    uint256 tokenId = tokenOfOwnerByIndex(friend, index);
    require(msg.value >= (price * ((growth[tokenId])**6) + 1), 'PRICE_NOT_MET');
    growth[tokenId] += 1;
  }
