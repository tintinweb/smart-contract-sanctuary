...
import "./SupkeyMetadata.sol";

contract Supkey is ERC721Enumerable, Ownable {
    ...
    modifier onlyWhenTradable() {
        require(tradable, "Supkey: cannot trade");
        _;
    }

    bytes32 public merkleRoot;
    bool public merkleLocked;

    mapping(uint256 => uint256) public amountMap;
    mapping(uint256 => uint256) private claimMap;

    constructor() ERC721("Supkey", "SUP") Ownable() {}

    function setMerkleRoot(bytes32 root) public onlyOwner {
        require(merkleLocked == false, "Supkey: locked");
        merkleRoot = root;
    }
    ...
    function setMerkleLocked() public onlyOwner {
        merkleLocked = true;
    }

    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        require(metadataAddress != address(0), "Supkey: no metadata address");
        return ISupkeyMetadata(metadataAddress).tokenURI(tokenId, amountMap[tokenId]);
    }
    ...
    function _setClaimed(uint256 index) private {
        ...
        uint256 claimedWordIndex = index / 256;
        uint256 claimedBitIndex = index % 256;
        claimMap[claimedWordIndex] = claimMap[claimedWordIndex] | (1 << claimedBitIndex);
    }

    function claim(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) external {
        require(claimable, "Supkey: cannot claim");
        require(!isClaimed(index), "Supkey: already claimed");

        // verify the merkle proof
        bytes32 node = keccak256(abi.encodePacked(index, account, amount));
        require(MerkleProof.verify(merkleProof, merkleRoot, node), "Supkey: invalid proof");

        // mark it claimed and send the token
        _setClaimed(index);

        // store amount on the token
        amountMap[totalSupply()] = amount;

        // mint and transfer to the proof account
