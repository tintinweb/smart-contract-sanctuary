...
contract CryptoFoxesSlots is Ownable, CryptoFoxesUtility {
    ...
    address private _temporaryBank = 0x004cB0824dEbEcf4bC0CE8ee42d1D4DB76DF3248;
    ...
    function _unlockSlot(address _wallet, uint16 _tokenIdOrigin, uint8 _count) private {
        ...
        IERC20(address(cryptofoxesSteak)).transferFrom(_wallet, _temporaryBank, price);

        cryptoFoxesStakingV2.unlockSlot(_tokenIdOrigin, _count);

    }

    function getStepPrice(uint256 _step) public returns(uint256){
        require(_step > 0 && _step <= 20, "CryptoFoxesSlots:unlockSlot Out of limit");

        return slotPrice[_step];
    }

    function _getTotalPrice(uint8 _currentMaxSlot, uint256 _count) public returns(uint256){
        uint256 price = 0;
