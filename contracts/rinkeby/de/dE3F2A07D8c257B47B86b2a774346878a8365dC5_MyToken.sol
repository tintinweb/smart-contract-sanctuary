...
contract MyToken is ERC721 {
    ...
    modifier notFrozen() {
        require(_frozen == false, "This function can not be called anymore");
    ...
    bool public _pause = true; // Reflects whether the contract mint function is paused
    bool private _frozen = false;
    uint256 public totalTokens = 10;
    uint256 public mintPrice = 0.01 ether;
    string public baseURI = "ipfs://not-set-yet";

    constructor() ERC721("MyToken", "MYTKN") {}
     ...
     */
    function setTotalTokens(uint16 _totalTokens) external onlyOwner notFrozen {
     ...
     */
    function setBaseURI(string memory _uri) external onlyOwner notFrozen {
        baseURI = _uri;
    }

    /**
     * @dev Sets the status of the mint
     */
    function pauseMint(bool val) public onlyOwner notFrozen {
        _pause = val;
    }
     ...
     */
    function devMint(address _address, uint16 amount) external onlyOwner notFrozen {
        require(getAvailableTokens() >= amount, "No tokens left to be minted");

        uint256[] memory tokenIds = new uint256[](amount);

        for (uint256 i; i < amount; i++) {
            tokenIds[i] = currentSupply.current();
     ...
     * @dev Mint a token
     */
    function mint(uint8 amount) external payable callerIsUser notFrozen {
        require(!_pause, "Mint has not started or is paused right now");
    ...
    function getAvailableTokens() public view returns (uint256) {
        return totalTokens - currentSupply.current();
    }
