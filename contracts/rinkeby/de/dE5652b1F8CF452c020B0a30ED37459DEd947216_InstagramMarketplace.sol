...
import "./NFT/IInstagramNFT.sol";
/**
 * @notice Marketplace contract for Instagram NFTs
 */
contract InstagramMarketplace is Context, ReentrancyGuard {
    using SafeMath for uint256;
    using Address for address payable;
    /// @notice Event emitted only on construction. To be used by indexers
    event InstagramMarketplaceContractDeployed();
    event PauseToggled(
        bool isPaused
    );
    event OfferCreated(
    ...
    /// @notice Instagram NFT - the only NFT that can be offered in this contract
    ...
    constructor(
        InstagramAccessControls _accessControls,
        IInstagramNFT _instagramNFT,
        address payable _platformFeeRecipient
    ) public {
        require(address(_accessControls) != address(0), "InstagramMarketplace: Invalid Access Controls");
        require(address(_instagramNFT) != address(0), "InstagramMarketplace: Invalid NFT");
        require(_platformFeeRecipient != address(0), "InstagramMarketplace: Invalid Platform Fee Recipient");
        accessControls = _accessControls;
        instagramNFT = _instagramNFT;
        platformFeeRecipient = _platformFeeRecipient;

        emit InstagramMarketplaceContractDeployed();
    }
    /**
     @notice Creates a new offer for a given Instagram NFT
    ...
    function createOffer(
        ...
        require(
            ...
            "InstagramMarketplace.createOffer: Sender must have the minter or admin role"
        );
        // Ensure the token ID does exists
        require(instagramNFT.exists(_tokenId), "InstagramMarketplace.createOffer: TokenID does not exist");
        // Check owner of the token ID is the owner and approved
        require(
            instagramNFT.ownerOf(_tokenId) == _msgSender() && instagramNFT.isApproved(_tokenId, address(this)),
            "InstagramNFTAuction.createOffer: Not owner and or contract not approved"
        );
        _createOffer(
            _tokenId,
            _primarySalePrice,
     ...
     */
    function buyOffer(uint256 _tokenId) external payable nonReentrant whenNotPaused {
        // Check the offers to see if this is a valid
        require(_msgSender().isContract() == false, "InstagramMarketplace.buyOffer: No contracts permitted");

        Offer storage offer = offers[_tokenId];
        uint256 maxShare = 1000;

        // Ensure this contract is still approved to move the token
        require(instagramNFT.isApproved(_tokenId, address(this)), "InstagramMarketplace.buyOffer: offer not approved");
        require(_getNow() >= offer.startTime && _getNow() <= offer.endTime, "InstagramMarketplace.buyOffer: Purchase outside of the offer window");
        require(msg.value >= offer.primarySalePrice, "InstagramMarketplace.buyOffer: Failed to supply funds");

        // Send platform fee in ETH to the platform fee recipient
        uint256 platformFeeInETH = offer.primarySalePrice.mul(platformFee).div(maxShare);
        (bool platformTransferSuccess,) = platformFeeRecipient.call{value : platformFeeInETH}("");
        require(platformTransferSuccess, "InstagramMarketplace.buyOffer: Failed to send platform fee");

        uint256 creatorFeeInETH = 0;
        // Send creator fee in ETH if seller is not creator of NFT.
        if(instagramNFT.postCreators(_tokenId) != instagramNFT.ownerOf(_tokenId))
        {
            creatorFeeInETH = offer.primarySalePrice.mul(creatorFee).div(maxShare);
            (bool creatorTransferSuccess,) = instagramNFT.postCreators(_tokenId).call{value : creatorFeeInETH}("");
            require(creatorTransferSuccess, "InstagramMarketplace.buyOffer: Failed to send creator fee");
        }
        // Send remaining to seller in ETH
        (bool sellerTransferSuccess,) = instagramNFT.ownerOf(_tokenId).call{value : offer.primarySalePrice.sub(platformFeeInETH).sub(creatorFeeInETH)}("");
        require(sellerTransferSuccess, "InstagramMarketplace.buyOffer: Failed to send the seller their royalties");

        // Record the primary sale price for the garment
        instagramNFT.setPrimarySalePrice(_tokenId, offer.primarySalePrice);
        // Transfer the token to the purchaser
        instagramNFT.safeTransferFrom(instagramNFT.ownerOf(_tokenId), msg.sender, _tokenId);
        emit OfferPurchased(_tokenId, _msgSender(), offer.primarySalePrice);
    }
    /**
    ...
    function cancelOffer(uint256 _tokenId) external nonReentrant {
        ...
        require(
            ...
            "InstagramMarketplace.cancelOffer: Sender must be admin or minter contract"
        );
        // Check valid and not resulted
        Offer storage offer = offers[_tokenId];
        require(offer.primarySalePrice != 0, "InstagramMarketplace.cancelOffer: Offer does not exist");
        require(_getNow() <= offer.endTime, "InstagramMarketplace.cancelOffer: Offer already closed");
        // Remove offer
    ...
    function toggleIsPaused() external {
        require(accessControls.hasAdminRole(_msgSender()), "InstagramMarketplace.toggleIsPaused: Sender must be admin");
        isPaused = !isPaused;
    ...
    function updateMarketplacePlatformFee(uint256 _platformFee) external {
        require(accessControls.hasAdminRole(_msgSender()), "InstagramMarketplace.updateMarketplacePlatformFee: Sender must be admin");
    ...
    function updateMarketplaceCreatorFee(uint256 _creatorFee) external {
        require(accessControls.hasAdminRole(_msgSender()), "InstagramMarketplace.updateMarketplaceCreatorFee: Sender must be admin");
    ...
    function updateOfferPrimarySalePrice(uint256 _tokenId, uint256 _primarySalePrice) external {
        require(accessControls.hasAdminRole(_msgSender()), "InstagramMarketplace.updateOfferPrimarySalePrice: Sender must be admin");
    ...
    function updateAccessControls(InstagramAccessControls _accessControls) external {
        require(
            ...
            "InstagramMarketplace.updateAccessControls: Sender must be admin"
        );
        require(address(_accessControls) != address(0), "InstagramMarketplace.updateAccessControls: Zero Address");
        accessControls = _accessControls;
    ...
    function updatePlatformFeeRecipient(address payable _platformFeeRecipient) external {
        require(
            accessControls.hasAdminRole(_msgSender()),
            "InstagramMarketplace.updatePlatformFeeRecipient: Sender must be admin"
        );
        require(_platformFeeRecipient != address(0), "InstagramMarketplace.updatePlatformFeeRecipient: Zero address");
        platformFeeRecipient = _platformFeeRecipient;
    ...
    /**
     ...
     @param _tokenId Token ID of the garment being offered
     */
    ...
    function _createOffer(
        ...
        require(offers[_tokenId].startTime == 0, "InstagramMarketplace.createOffer: Cannot duplicate current offer");
        // Setup the new offer
    ...
    function reclaimETH() external {
        require(
            accessControls.hasAdminRole(_msgSender()),
            "InstagramMarketplace.reclaimETH: Sender must be admin"
        );
