   ...
   { 
    ...
    function buyObjectsFromUser(uint ObjectsId) public payable 
    {
       ...
       // token.mint(prevOwner,msg.sender,1); 
        ...
        {
          if (upForSaleList[j] == ObjectsId)
		  {
		  delete upForSaleList[j];
		  delete upForSaleListPrice[j];
		  }
    ...
    function putSaleRequest(uint ObjectsId, uint salePrice) public payable
    {
        ...
        forSaleObjectsList[ObjectsId].index = upForSaleList.push(ObjectsId);
    ...
    function withdrawSaleRequest(uint ObjectsId) public
    {
        require (!isContractPaused);
        ...
        // the Objects in question is still up for sale
        require(ObjectsAgainstId[ObjectsId].upForSale==true);

        // change the Objects state to not be on sale
        ObjectsAgainstId[ObjectsId].upForSale=false;
        ObjectsAgainstId[ObjectsId].priceForSale=0;

        // remove the Objects from sale list
        for (uint i=0;i<upForSaleList.length;i++)
        {
            if (upForSaleList[i]==ObjectsId)
			{
			 delete upForSaleList[i];   
			 delete upForSaleListPrice[i];   
			}
    ...
    function showMyObjectsBalance() public view returns (uint256 tokenBalance) 
    {
