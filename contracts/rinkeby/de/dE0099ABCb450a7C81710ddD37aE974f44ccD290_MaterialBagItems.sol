    ...
    function tokenURI(uint256 tokenId) public view returns (string memory) {
        ...
        string memory json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        '{ "name": "', MaterialMetadata.materialTokenName(tokenId),'", ', 
                        '"description" : ', '"Material Bag Items lets you unbundle your Material Bags into individual ERC1155 NFTs or rebundle items into their original Material Bags.", ',
                        '"image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '", ' 
                        '"attributes": ', MaterialMetadata.materialAttributes(tokenId),
                        '}'
                    )
                )
            )
