...
abstract contract ERC20VestableInTimestamp is ERC20Burnable, AccessControl {
    ...
    function inActiveSchedule(uint256 _id) internal {
        uint256 index;
        for (uint i = 0; i < allActiveSchedules.length; i++) {
        ...
        for (uint i = index; i < allActiveSchedules.length; i++) {
            allActiveSchedules[index] = allActiveSchedules[allActiveSchedules.length - 1];
        }
        ...
        onlyGrantor
    {
        ...
        for (uint256 i = 0; i < _whiteListAddresses.length; i++) {
            require(!whiteLists[_whiteListAddresses[i]], "already whitelist");
    ...
    function updateVestingSchedule(
        ...
        // Make sure the duration values are in harmony with 1 (both should be an exact multiple of 1).
        require(
            _duration % 1 == 0 && _cliffDuration % 1 == 0,
        ...
        emit VestingScheduleUpdated(_id, _name, _isActive, _startDay, _cliffDuration, _duration);
    }

    /**
     * @dev This operation permanently establishes the vesting schedule in the beneficiary's account.
     *
     * @param _vestingLocation = Account into which to store the vesting schedule.
    ...
    function _applyVestingSchedule(
        ...
        require(userVestingAmountInSchedule[_vestingLocation][_vestingSchedule] == 0, "already apply vesting schedule");
        require(
            vestingSchedules[_vestingSchedule].isActive == true,
            "vesting schedule is not active"
        );
        userVestingAmountInSchedule[_vestingLocation][_vestingSchedule] = _vestingAmount;
    }

    function _applyMultiVestingSchedule(
        address _vestingLocation,
        uint256[] calldata _vestingSchedules,
        uint256[] calldata _vestingAmounts
    ) internal {
        require(_vestingSchedules.length >= 1, "empty vesting schedule");
        require(_vestingSchedules.length == _vestingAmounts.length, "invalid length");
        for (uint i = 0; i < _vestingSchedules.length; i++) {
            _applyVestingSchedule(_vestingLocation, _vestingSchedules[i], _vestingAmounts[i]);
        }
    }
     ...
     * @param _vestingSchedules = Vesting schedule ID.
     * @param _vestingAmounts = The amount of user will be vested.
     */
    function applyMultiVestingSchedule(
        ...
        require(_beneficiaries.length == _vestingSchedules.length, "invalid length");
        require(_vestingSchedules.length == _vestingAmounts.length, "invalid length");

        for (uint256 i = 0; i < _beneficiaries.length; i++) {
            _applyMultiVestingSchedule(
                _beneficiaries[i],
                _vestingSchedules[i],
                _vestingAmounts[i]
            );
    ...
    function getAllScheduleOfUser(address _grantedHolder) public view returns (uint256[] memory userActiveSchedules) {
        uint256 index = 0;
        uint256[] memory schedules = new uint256[](allActiveSchedules.length);
        for (uint i = 0; i < allActiveSchedules.length; i++) {
            if (userVestingAmountInSchedule[_grantedHolder][allActiveSchedules[i]] > 0) {
    ...
    /**
     * @dev Determines the amount of token that have not vested for 1 schedule in the give account.
     *
     * notVestAmount = vestingAmount * (endDate - onDate)/(endDate - startDate)
     *
     * @param _grantedHolder = The account to check
     * @param _onDayOrToday = The day to check, in seconds since the UNIX epoch.
     * Pass `0` if indicate TODAY.
     */
    function _getNotVestedAmount(address _grantedHolder, uint256 _vestingSchedule, uint256 _onDayOrToday)
        internal
        view
        returns (uint256)
    {
        uint256 userVestingAmount = userVestingAmountInSchedule[_grantedHolder][_vestingSchedule];
        ...
        // Otherwise a fractional amount is vested.
            ...
            uint256 vested =
                userVestingAmount * (daysVested / vesting.duration);
            return userVestingAmount - vested;
        }
    }

    /**
     * @dev Determines the all amount of token that have not vested in multi schedule in the give account.
     *
     * notVestAmount = vestingAmount * (endDate - onDate)/(endDate - startDate)
     *
     * @param _grantedHolder = The account to check
     ...
     * Pass `0` if indicate TODAY.
     */
    function _getNotVestedAmountForAllSchedule(address _grantedHolder, uint256 _onDayOrToday)
        internal
        view
        returns (uint256 notVestedAmount)
    {
        uint256[] memory userSchedules = getAllScheduleOfUser(_grantedHolder);
        if (userSchedules.length == 0) return uint256(0);

        for (uint i = 0; i < userSchedules.length; i++) {
            uint256 userVestingAmount = userVestingAmountInSchedule[_grantedHolder][userSchedules[i]];
            if (userVestingAmount == 0) return uint256(0);
            ...
            if (
                ...
                // None are vested (all are not vested)
                notVestedAmount = notVestedAmount + userVestingAmount;
            }
            // If after end of cliff + vesting, then the not vested amount is zero (all are vested).
            else if (
                onDay >=
            ...
            // Otherwise a fractional amount is vested.
                ...
                uint256 vested =
                    userVestingAmount * daysVested / vesting.duration;
                notVestedAmount = notVestedAmount + (userVestingAmount - vested);
            }
        }
    }
     ...
     */
    function _getAvailableAmount(address _grantedHolder, uint256 _onDay)
        internal
        view
        returns (uint256)
    {
        uint256 totalTokens = balanceOf(_grantedHolder);
        uint256 vested =
            totalTokens - _getNotVestedAmountForAllSchedule(_grantedHolder, _onDay);
        return vested;
    }

    function vestingForAccountAsOf(address _grantedHolder, uint256 _onDayOrToday)
        public
        view
        onlyGrantorOrSelf(_grantedHolder)
        returns (VestingForAccount[] memory userVestingInfo)
    {
        ...
        for (uint i = 0; i < userSchedules.length; i++) {
            ...
            VestingSchedule storage vesting =
                vestingSchedules[userSchedules[i]];
            uint256 notVestedAmount =
                _getNotVestedAmount(_grantedHolder, userSchedules[i], _onDayOrToday);
