...
contract NomoLeague is OwnableUpgradeable, INomoLeague {
    ...
    uint256 public gameDuration = 7 days;

    /// @notice Duration of staking period in one game
    uint256 public stakingDuration = 6 days;
    ...
    function initialize(
        ...
        router = router_;
        nft = INomoNFT(router.getNft());
        rewardToken = IERC20(router.getRewardToken());
     ...
     * @return Total reward
     */
    function totalRewardOf(address account) public view override returns (uint256) {
        ...
        int256 currentCorrections = _magnifiedRewardCorrections[account] -
            SafeCast.toInt256(_rewardPerPointAfterGame[players[account].currentGame] * players[account].pendingPoints);

        uint256 accumulatedReward = SafeCast.toUint256(
            SafeCast.toInt256(currentActivePoints * _magnifiedRewardPerPoint) + currentCorrections
        ) / _magnitude;
     ...
     */
    function nextGame(uint256 totalReward) external override onlyOwner {
        if (lastGameId != 0) {
     ...
     */
    function stakeToken(address account, uint256 tokenId) external override onlyRouter {
        _movePendingPoints(account);
        ...
        if (block.timestamp > lastGameStart + stakingDuration || lastGameStart == 0) {
            ...
            _magnifiedRewardCorrections[account] -= SafeCast.toInt256(_magnifiedRewardPerPoint * points);
        }
     ...
     */
    function unstakeToken(address account, uint256 tokenId) external override onlyRouter {
        _movePendingPoints(account);
        ...
        if (_tokenPendingAtGame[tokenId] == lastGameId) {
            ...
            _magnifiedRewardCorrections[account] += SafeCast.toInt256(_magnifiedRewardPerPoint * tokenPoints[tokenId]);
        }
     ...
     */
    function updatePoints(address account, uint256 tokenId) external override onlyRouter {
        ...
        if (_tokenPendingAtGame[tokenId] == lastGameId) {
            ...
            _magnifiedRewardCorrections[account] += (SafeCast.toInt256(_magnifiedRewardPerPoint * oldPoints) -
                SafeCast.toInt256(_magnifiedRewardPerPoint * newPoints));
    ...
    function _finishGame(uint256 totalReward) private {
        require(
            block.timestamp >= lastGameStart + gameDuration,
    ...
    function _movePendingPoints(address account) private {
        if (players[account].currentGame != lastGameId) {
            ...
            _magnifiedRewardCorrections[account] -= SafeCast.toInt256(
