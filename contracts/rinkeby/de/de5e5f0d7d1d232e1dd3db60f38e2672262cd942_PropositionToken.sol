...
contract PropositionToken is ERC721Enumerable, Ownable {
    ...
    function testSiblingFunction( string memory _proposition ) public view returns(bool) {
        if (siblings.length == 0) return false;
        bool doesPropExist = SIBLINGS(siblings[0]).doesPropositionExist(_proposition);
        return doesPropExist;
    }
    
    function testByteCode( string memory str ) public pure returns(bytes memory) {
        return bytes(str);
    }
    ...
    function calculateSpacesAtEndOfString(bytes memory _str) public pure returns (uint) {
        // pass in 4 space string and count space checking logic
        ...
        while (recentCharIsASpace) {
            uint index = _str.length - endIndex;
            if (uint8(_str[index]) == 32) {
                endStringSpaces++;
                endIndex++;
            } else {
                recentCharIsASpace = false;
            }
            if (_str.length == endStringSpaces) {
                recentCharIsASpace = false;
            }
        }
    ...
    */
    function transformString(string memory str) public pure returns (string memory){
        ...
        // if string is only spaces then return single string
