...
contract WhiteRareAuctions is IAuctionHouse, ReentrancyGuard {
   ...
   function getAuctionCurrentNumber() public view returns (uint256) {
    ...
    function fixedBulkBuy (uint256[] calldata _blukFixedAuctionIds, uint256 amount) 
      external  
      payable 
    {
        uint256 auctionPrice = 0;
        for(uint256 i = 0; i< _blukFixedAuctionIds.length;i++){
                auctionPrice = auctionPrice + getAuctionPrice(_blukFixedAuctionIds[i]);
        }
        require(amount == auctionPrice);
        for(uint256 cd = 0; cd < _blukFixedAuctionIds.length; cd++){
            fixedBuyInternal(_blukFixedAuctionIds[cd], getAuctionPrice(_blukFixedAuctionIds[cd]));
        }
