...
contract UniswapLiquidityManager is PeripheryPayments, IUniswapLiquidityManager {
    ...
    using LiquidityPositions for LiquidityPosition;
    using UserPositions for Position;
    ...
    // mapping(address => ReadjustFrequency) private _readjustFrequency;
    ...
    // mapping(address => bool) private _feesInPilot;

    /// @dev keeps track of pool readjust rewards & swapping per readjust
    // mapping(address => ReadjustPremium) private readjustPremium;

    /// @dev Extra PILOT tokens could be minted into a wallet address.
    /// By default, this feature should be off but can be toggled using governance.
    PilotSustainabilityFund private _pilotSustainabilityFund;
    ...
    constructor(
        ...
        (_oracle, _ulmState, _indexFund, _uniStrategy, _unipilot) = (
            oracle_,
            ulmState_,
            indexFund_,
            uniStrategy_,
        ...
        _gasPriceLimit = 1900226581; // 250 gwei
        _userPilotPercentage = 98;
    }

    // function getPremiumStatusForPools(address pool)
    //     external
    //     view
    //     returns (bool feesInPilot, bool premiumForReadjust)
    // {
    //     return (_feesInPilot[pool], readjustPremium[pool].premiumStatus);
    // }

    /// @dev Blacklist/Whitelist pool for recieving fees in PILOT
    function toggleFeesInPilot(address pool) external onlyGovernance {
        liquidityPositions[pool].feesInPilot = !liquidityPositions[pool].feesInPilot;
    }
    ...
    /// @param gasPriceLimit_ Maximum gas price limit for readjust of vault
    function setReadjustPremiumForPools(
        address pool,
        uint256 premium_,
        uint256 gasPriceLimit_
    ) external onlyGovernance {
        liquidityPositions[pool].rebasePremium = premium_;
        _gasPriceLimit = gasPriceLimit_;
    }

    /// @dev Sets the new details for unipilot sustainability fund
    /// @param recipient Address where the minted PILOT tokens will be recieved
    /// @param pilotPercentage Percentage of minting PILOT amount w.r.t user collected fees
    /// @param userPilotPercentage Percentage of
    /// @param status Status of minting PILOT token for unipilot sustainability fund
    function setPilotProtocolDetails(
        // false
        address recipient,
        uint8 pilotPercentage,
        uint8 userPilotPercentage,
        bool status
    ) external onlyGovernance {
        _pilotSustainabilityFund = PilotSustainabilityFund({
            recipient: recipient,
            pilotPercentage: pilotPercentage,
            status: status
        });
        _userPilotPercentage = userPilotPercentage;
    }
        ...
        returns (uint256 fees0, uint256 fees1)
    {
        ...
        (uint256 tokensOwed0, uint256 tokensOwed1, , ) = position.getTokensOwedAmount(
            lp.feeGrowthGlobal0,
            lp.feeGrowthGlobal1
        );
        ...
        )
    {
        ...
        b.pool = _getPoolAddress(token0, token1, b.fee);
        ...
        if (b.tokenId > 0) {
            ...
            emit Deposited(b.pool, b.tokenId, amount0Desired, amount1Desired, shares); // change
        } else {
            (mintedTokenId) = IUnipilot(_unipilot).mintUnipilotNFT(sender);
        ...
        (b.dust0, b.dust1) = _checkDustAmount(
            b.pool,
        ...
        (amount0Added, amount1Added) = (
            b.amount0Base.add(b.amount0Range.add(b.dust0)),
            b.amount1Base.add(b.amount1Range.add(b.dust1))
        );
    }
    ...
    function readjustLiquidity(
        ...
        require(IULMState(_ulmState).shouldReadjust(b.poolAddress, address(this)));
        ...
        if ((b.amount0 == 0 || b.amount1 == 0)) {
            ...
            b.exactSqrtPriceImpact =
                (b.sqrtPriceX96 * (IUniStrategy(_uniStrategy).pricethreshold() / 2)) /
                1e6;
            ...
            b.amountIn = FullMath.mulDiv(
                b.amountIn,
                IUniStrategy(_uniStrategy).swapPercentage(),
                100
            );
        ...
        (, , uint256 amount0, uint256 amount1, ) = updatePositionTotalAmounts(
            b.poolAddress
        );

        if (
            IOracle(_oracle).checkPoolValidation(token0, token1, amount0, amount1) &&
            position.rebasePremium > 0
        ) {
            require(tx.gasprice <= _gasPriceLimit);
        ...
        )
    {
        ...
        if (position.totalLiquidity > 0) {
            return position.getTotalAmounts(_pool);
        }
    }
    ...
    function _isUnipilot() private view {
        require(msg.sender == _unipilot);
    }

    function _isGovernance() private view {
        require(msg.sender == IUnipilot(_unipilot).governance());
    }
    ...
    {
        return IOracle(_oracle).getTokenWethPairWithMaxLiquidity(token);
    }
    ...
    {
        return IULMState(_ulmState).getPoolDetails(pool);
    }

    // function _getTokensOwedAmount(
    ...
    // {
    //     return
    //         IULMState(_ulmState).getTokensOwedAmount(
    //             liquidityManager,
    //             pool,
    //             userLiquidity,
    //             feeGrowth0,
    ...
    function _mintPilot(address recipient, uint256 amount) private {
        IUnipilot(_unipilot).mintPilot(recipient, amount);
    }
    ...
    function _mintPilotForUser(address _recipient, uint256 _pilotAmount) private {
        if (_pilotAmount > 0) {
            ...
            if (_pilotSustainabilityFund.status)
                _mintPilot(
                    _pilotSustainabilityFund.recipient,
                    FullMath.mulDiv(
                        _pilotAmount,
                        _pilotSustainabilityFund.pilotPercentage,
                        100
                    )
                );
        }
    ...
    {
        return IUniStrategy(_uniStrategy).getTicks(pool);
    }
    ...
    function _checkDustAmount(
        ...
        uint256 amount0Desired,
        uint256 amount1Desired
    ) private returns (uint256 dust0, uint256 dust1) {
        // hur
        LiquidityPosition storage poolPosition = liquidityPositions[pool];
        dust0 = amount0Desired.sub(amount0Added);
        dust1 = amount1Desired.sub(amount1Added);
    ...
    function _addRangeLiquidity(
        ...
        (b.token0, b.token1, b.fee, , , , ) = _getPoolDetails(pool);
        ...
        )
    {
        ...
        (a.token0, a.token1, a.fee, , , , a.currentTick) = IULMState(_ulmState)
            .getPoolDetails(pool);
        ...
        )
    {
        ...
        if (_token0 == WETH || _token1 == WETH) {
            uint256 _pilotAmountInitial = IOracle(_oracle).getPilotAmountWethPair(
                _token1,
                _tokensOwed1,
                _tokensOwed0
            );
            ...
            _pilotAmount = FullMath.mulDiv(
                _pilotAmountInitial,
                _userPilotPercentage,
                100
            );
            _mintPilotForUser(_recipient, _pilotAmount);

            if (_tokensOwed0 > 0)
                TransferHelper.safeTransfer(_token0, _indexFund, _tokensOwed0);
            if (_tokensOwed1 > 0)
                TransferHelper.safeTransfer(_token1, _indexFund, _tokensOwed1);
        } else {
            // if the incoming pair is alt pair then compute the amount
            // of PILOT w.r.t both alt tokens amount
            uint256 _pilotAmountInitial = IOracle(_oracle).getPilotAmountForTokens(
                _token0,
                _token1,
                _tokensOwed0,
                _tokensOwed1
        ...
        (_index0, _index1) = _sortTokenAmounts(
            _token0,
            _token1,
            _tokensOwed0,
            _tokensOwed1
    ...
    function _distributeFeesInTokens(
        ...
        ) = IULMState(_ulmState).getUserAndIndexShares(_tokensOwed0, _tokensOwed1);
        ...
        if (_tokensOwed0 > 0) {
            ...
            TransferHelper.safeTransfer(_token0, _indexFund, _indexAmount0);
        }
        if (_tokensOwed1 > 0) {
            TransferHelper.safeTransfer(_token1, _recipient, _userBalance1);
            TransferHelper.safeTransfer(_token1, _indexFund, _indexAmount1);
        }

        if (_pilotSustainabilityFund.status) {
            if (_tokensOwed0 > 0 || _tokensOwed1 > 0) {
                uint256 pilotAmount = IOracle(_oracle).pilotAmountForFees(
                    _token0,
                    _token1,
                    _tokensOwed0,
                    _tokensOwed1,
                ...
                _mintPilot(_pilotSustainabilityFund.recipient, pilotAmount);
            }
        }
        (_index0, _index1) = _sortTokenAmounts(
            _token0,
        ...
        )
    {
        ...
        (a.token0, a.token1, , , , , ) = _getPoolDetails(params.pool);
        ...
        (
            ...
            a.feeGrowthGlobal0,
            a.feeGrowthGlobal1
        ) = userPosition.getTokensOwedAmount(
    ...
    function _verifyCallback(
        ...
        require(msg.sender == _getPoolAddress(token0, token1, fee));
    }
...
library LiquidityPositions {
        ...
        )
    {
        ...
        fee0 = self.fees0.add(localVars.baseFees0.add(localVars.rangeFees0));
        fee1 = self.fees1.add(localVars.baseFees1.add(localVars.rangeFees1));
    }
}
...
library UserPositions {
    ...
    function getTokensOwedAmount(
        IUniswapLiquidityManager.Position memory self,
        uint256 feeGrowthGlobal0,
        uint256 feeGrowthGlobal1
    )
        internal
        ...
        )
    {
        tokensOwed0 = FullMath.mulDiv(
            feeGrowthGlobal0.sub(self.feeGrowth0),
            self.liquidity,
            FixedPoint128.Q128
        );
        tokensOwed1 = FullMath.mulDiv(
            feeGrowthGlobal1.sub(self.feeGrowth1),
            self.liquidity,
            FixedPoint128.Q128
        );

        _feeGrowthGlobal0 = feeGrowthGlobal0;
        _feeGrowthGlobal1 = feeGrowthGlobal1;
    }
}
