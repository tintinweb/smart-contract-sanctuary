...
// File: SWF_GAME/SacrificialAlter.sol
...
contract SacrificialAlter is ISacrificialAlter, ERC1155, Ownable, Pausable {
    ...
    modifier requireContractsSet() {
        require(address(safeToken) != address(0), "Contracts not set");
        _;
    }

    function setContracts(address _safe) external onlyOwner {
    ...
    */
    function mint(uint256 typeId, uint16 qty, address recipient) external override whenNotPaused {
        ...
        if(typeInfo[typeId].safeExchangeAmt > 0) {
            // If the ERC1155 is swapped for $SAFE, transfer the SAFE to this contract in case the swap back is desired.
            // NOTE: This will fail if the origin doesn't have the required amount of $SAFE
            safeToken.transferFrom(tx.origin, address(this), typeInfo[typeId].safeExchangeAmt * qty);
        }
    ...
    */
    function burn(uint256 typeId, uint16 qty, address burnFrom) external override whenNotPaused {
        require(admins[_msgSender()], "Only admins can call this");
        if(typeInfo[typeId].safeExchangeAmt > 0) {
            // If the ERC1155 was swapped from $SAFE, transfer the SAFE from this contract back to whoever owns this token now.
            safeToken.transferFrom(address(this), tx.origin, typeInfo[typeId].safeExchangeAmt * qty);
        }
    ...
    function setExchangeAmt(uint256 typeId, uint256 exchangeAmt) external onlyOwner {
        ...
        typeInfo[typeId].safeExchangeAmt = exchangeAmt;
    }
    ...
    */
        ...
        returns (string memory)
    {
        ...
        string memory metadata = string(abi.encodePacked(
            '{"name": "',
            img.name,
            '", "description": "Mysterious items spawned from the Sacrificial Alter of the SafeZone. Fabled to hold magical properties, only Act 1 humans will know the truth in the following acts. All the metadata and images are generated and stored 100% on-chain. No IPFS. NO API. Just the Ethereum blockchain.", "image": "data:image/svg+xml;base64,',
