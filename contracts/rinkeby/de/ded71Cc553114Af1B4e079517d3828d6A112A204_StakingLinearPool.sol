...
contract LinearPool is Initializable, OwnableUpgradeable {
    ...
    private linearPendingWithdrawals;

    // The minimum lock time. Users who stake in the flexible pool will be affected by this
    uint256 public linearMinlockTime;
     ...
     * @param _poolId id of the pool
     */
    function linearTotalStaking(uint256 _poolId)
    external
    view
     ...
     * @param _lockTime the duration users need to wait before being able to withdraw and claim the rewards.
     * @param _delayTime the duration users need to wait to receive the principal amount, after unstaking from the pool.
     ...
     * @param _minlockTime the minimum lock time
     */
    function linearSetMinLockTime(uint256 _minlockTime) external onlyOwner {
        require(_minlockTime > 0);
        linearMinlockTime = _minlockTime;
    }
    ...
    linearValidatePoolById(_poolId)
    {
        ...
        if (pool.cap > 0) {
            require(
                pool.totalStaking + _amount <= pool.cap,
                "LinearStakingPool: pool is already full"
            );
        }
    ...
    linearValidatePoolById(_poolId)
    {
        ...
        uint256 lockTime = pool.lockTime > 0
        ? pool.lockTime
        : linearMinlockTime;

        require(
            block.timestamp >= stakingData.joinTime + lockTime,
        ...
        if (stakingData.reward > 0) {
            ...
            emit LinearRewardsHarvested(_poolId, account, stakingData.reward);
        ...
        pending.applicableAt = block.timestamp + pool.delayTime;
    }
    ...
    returns (uint256 reward)
    {
        ...
        if (
            pool.lockTime > 0 &&
            stakingData.joinTime + pool.lockTime < block.timestamp
        ) {
            endTime = stakingData.joinTime + pool.lockTime;
        }
    ...
    linearValidatePoolById(_poolId)
    {
        ...
        linearAcceptedToken.safeTransfer(account, stakingData.balance);
        emit LinearEmergencyWithdraw(_poolId, account, stakingData.balance);
