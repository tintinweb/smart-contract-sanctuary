...
contract RandomSVG is ERC721URIStorage, Ownable {
    ...
    constructor()
    ERC721("RandomSVG", "rsNFT") // (name, token_symbol)
    {
        randNonce = 0;
        tokenCounter = 0;
        //        keyHash = _keyhash;
    ...
    function randMod32() public returns (bytes32)
    {
        randNonce++;
        bytes32 output = keccak256(abi.encodePacked(msg.sender, randNonce));
        return output;
    }
    ...
    function finishMint(uint256 tokenId) public {
        // check if its been minted and a random number is returned
        ...
        require(tokenIdToRandomNumber[tokenId] > 0, "Random number still not generated!");
        // generate random SVG code
        uint256 randomNumber = tokenIdToRandomNumber[tokenId];
        string memory svg = generateSVG(randomNumber);
        // turn SVG into and image URI
        string memory imageURI = svgToImageURI(svg);
        // user that imageURI to format into a tokenURI
