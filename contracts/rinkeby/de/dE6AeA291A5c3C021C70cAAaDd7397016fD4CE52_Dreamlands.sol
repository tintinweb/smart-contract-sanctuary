...
contract Dreamlands is ERC721, Ownable, Pausable {
    uint256 public constant MAX_SUPPLY = 10000;
    uint256 public constant MINT_PRICE = 0.05 ether;
    uint256 public constant REFUND_PERIOD = 100 days;
    ...
    uint256 public refundPeriodStarted;
    uint256 public balanceAtRefundStarted;
    uint256 public refundQuarter = 0;

    string baseURI;

    constructor() ERC721("Dreamlands", "DL") { }
    ...
    function refund(uint256 _tokenId) external {
        require(_refundAddress != address(0), "REFUND_ADDRESS_NOT_SET");
        require(address(this).balance >= MINT_PRICE, "EMPTY_BALANCE");
        require(ownerOf(_tokenId) == msg.sender, "NOT_TOKEN_OWNER");
        require(refundPeriodStarted != 0, "REFUND_PERIOD_NOT_STARTED");
        require(block.timestamp < refundPeriodStarted + REFUND_PERIOD, "REFUND_PERIOD_OVER");
        _transfer(msg.sender, _refundAddress, _tokenId);
    ...
    function setRefundAddress(address _refundAddr) external onlyOwner {
        require(_refundAddr != address(0), "REFUND_ADDRESS_ZERO");
        require(_refundAddress != _refundAddr, "REFUND_ADDRESS_ALREADY_SET");

        _refundAddress = _refundAddr;
    }

    function setRefundPeriodStarted() external onlyOwner {
        require(refundPeriodStarted == 0, "REFUND_PERIOD_ALREADY_STARTED");
        require(_refundAddress != address(0), "REFUND_ADDRESS_NOT_SET");
        refundPeriodStarted = block.timestamp;
        balanceAtRefundStarted = address(this).balance;

        // Immediately withdraw 25%
        payable(_refundAddress).transfer(balanceAtRefundStarted / 4);
    }
    ...
    function withdraw() external onlyOwner {
        ...
        require(_refundAddress != address(0), "REFUND_ADDRESS_NOT_SET");
        // Past Refund Period
        if (block.timestamp >= refundPeriodStarted + REFUND_PERIOD) {
            payable(_refundAddress).transfer(address(this).balance);
        } else {
            uint256 quarter = (block.timestamp - refundPeriodStarted) / (REFUND_PERIOD / 3);
            if (quarter > refundQuarter) {
                payable(_refundAddress).transfer(balanceAtRefundStarted / 4);
                refundQuarter += 1;
            }
        }
