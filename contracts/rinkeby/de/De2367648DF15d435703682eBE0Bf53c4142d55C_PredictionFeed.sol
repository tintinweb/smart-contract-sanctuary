...
contract PredictionFeed is Ownable, ChainlinkClient {
    ...
    modifier isValidTimes(uint256 _startTime, uint256 _endTime) {
        require(_startTime > block.timestamp, "PredictionFeed: Start time is already gone");
        require(_endTime > block.timestamp, "PredictionFeed: End time is already gone");
        require(_endTime > _startTime + READY_TIME, "PredictionFeed: Wrong end time");
        _;
    }
    ...
    ) external onlyOwner isValidPoolType(_poolType) isValidTimes(_startTime, _endTime) returns (uint256) {
        IPool memory pool;
