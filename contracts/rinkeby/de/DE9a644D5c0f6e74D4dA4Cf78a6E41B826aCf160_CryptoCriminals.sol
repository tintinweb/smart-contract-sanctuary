...
contract CryptoCriminals is ERC721, Ownable, ReentrancyGuard, PaymentSplitter {
  ...
  uint public constant maxGangRecruitingSupply = 11111;
  uint public constant maxMintsPerTransaction = 9;
  ...
  uint public recruitingTimeWindow = 3 minutes;

  mapping (uint => uint) public lastRecruitTimeOfTokenID;
  mapping (uint => uint) public pointsOfToken;
  
  constructor() ERC721("CryptoCriminals", "CC") PaymentSplitter(_team, _teamShares) {
  }
  ...
  function addToPointList(uint[] memory tokenIds, uint[] memory amount) external onlyOwner {
    for(uint256 i = 0; i < tokenIds.length; i++) {
      pointsOfToken[tokenIds[i]] = amount[i];
    }
  }
  ...
  function mintSale(uint amount) external payable onlyEOA {
    ...
    require(totalSupply() + amount <= maxMintingSupply - (reservedAmount - reservedMintedAlready), "max supply reached");
  ...
  function mintReserved(address receiver, uint amount) external onlyOwner {
    require(totalSupply() + amount <= maxMintingSupply, "max reached");
  ...
  function _mintCC(address to, uint amount) private {
    for(uint i = 0; i < amount; i++){
      ...
      _mint(to, _id);
      if(gangRecruitingOpen){
        pointsOfToken[_id] = pseudoRandomPointAssignmentSingleId(_id);
      }
      _tokenIds.increment();
    }
  }
  ...
  function recruitGangMember(uint[] memory tokenIds) external onlyEOA {
    ...
    require(totalSupply() + 1 <= maxMintingSupply + maxGangRecruitingSupply, "max supply recruited");
    require(tokensOwnershipValid(tokenIds),"not owner of Tokens");
    require(tokenPointsCanRecruit(tokenIds), "sum of token points not enough");
    require(tokenTimeCanRecruit(tokenIds),"tokenIDs not yet ready to recruit again");

    setTokensRecruitingTimer(tokenIds); 
    _mintCC(msg.sender, 1); 
  }

  function pseudoRandomPointAssignmentSingleId(uint tokenId) private view returns (uint){
    uint result = uint(keccak256(abi.encodePacked(tokenId, msg.sender.balance,  block.timestamp)));
    return (result % 3) + 1;
  }

  // Recruiting TokenId validation
  function tokensOwnershipValid(uint[] memory tokenIds) public view returns(bool){
  ...
  function tokenPointsCanRecruit(uint[] memory tokenIds) public view returns(bool){
    uint sum;
    for(uint i = 0; i < tokenIds.length; i++){
  ...
  function tokenTimeCanRecruit(uint[] memory tokenIds) public view returns(bool){
