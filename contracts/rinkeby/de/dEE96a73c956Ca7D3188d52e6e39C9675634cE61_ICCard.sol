...
contract ICCard is ERC721, Ownable {
    using SafeMath for uint256;
    uint public constant MAX_CARDS = 100000;
    uint public constant MAX_CARDS_PER_ACCOUNT = 5;
    ...
    constructor(string memory baseURI, uint _cardPrice, address _treasureAddress) ERC721("Investors Club Card", "ICCARD") {
        setBaseURI(baseURI);
        cardPrice = _cardPrice;
        treasureAddress = _treasureAddress;
    }
    
    function buyCard(uint numCards) public payable {
        buyCard(numCards, msg.sender);
    }
    
    function buyCard(uint numCards, address receiver) public payable {
        require(hasSaleStarted, "sale hasn't started");
        require(balanceOf(receiver).add(numCards) <= MAX_CARDS_PER_ACCOUNT, "account already have enouth cards");
        require(totalSupply().add(numCards) <= MAX_CARDS, "the sale was sold out");
        require(msg.value >= cardPrice.mul(numCards) || msg.sender == owner(), "ether value sent is below the price");
    ...
    function renameCardRequest(uint tokenId, string memory cardName) public {
        require(msg.sender == ownerOf(tokenId), "sender does not have the current card");
        emit CardRenameRequested(tokenId, cardName);
    }
    
    function renameCard(uint tokenId, string memory cardName) external onlyOwner {
        cardNames[tokenId] = cardName;
        emit CardRenamed(tokenId, cardName);
    }
