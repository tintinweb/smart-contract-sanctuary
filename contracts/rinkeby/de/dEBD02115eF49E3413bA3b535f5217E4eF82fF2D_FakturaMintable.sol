...
UUPSUpgradeable
{
    ...
    function safeMint(address to) public payable limited {
        ...
        _setTokenURI(_tokenIdCounter.current(), string(abi.encodePacked(metadataURI, StringsUpgradeable.toString(_tokenMetadata[0].id), ".json")));
        _tokenMetadata[0].counter.increment();
        if(_tokenMetadata[0].counter.current() == _tokenMetadata[0].amount) delete _tokenMetadata[0];
    ...
    function safeBatchMint(address to, uint256 amount) public payable limited {
        ...
        for (uint256 i = 0; i < amount; i++) {
            _safeMint(to, _tokenIdCounter.current());
            _setTokenURI(_tokenIdCounter.current(), string(abi.encodePacked(metadataURI, StringsUpgradeable.toString(_tokenMetadata[i].id), ".json")));
            _tokenMetadata[i].counter.increment();
            if(_tokenMetadata[i].counter.current() == _tokenMetadata[i].amount) delete _tokenMetadata[i];
            _tokenIdCounter.increment();
            mintCnt[msg.sender]++;
        }
    }
