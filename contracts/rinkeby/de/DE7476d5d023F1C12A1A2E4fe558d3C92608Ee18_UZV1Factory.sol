...
contract UZV1Factory is IUZV1Factory, UZV1ProAccess {
    ...
    mapping(uint8 => address) public _basePools;
    ...
    event Test(address proxy, address impl);
    function createNewPoolProxy(uint8 _poolType, address _proxy, address _implement) external { 
        address implement = _implement;
        address payable proxyContract =
            payable(ClonesUpgradeable.clone(_proxy));
        nProxy initializing = nProxy(proxyContract);
        address newPoolImplement = ClonesUpgradeable.clone(implement);
        initializing.upgradeTo(newPoolImplement);
        IUZV1RewardPool newPool = IUZV1RewardPool(proxyContract);
        newPool.initialize(router, address(_membershipToken));
        emit Test(proxyContract, newPoolImplement);
        //  newPoolProxy = new (implement, abi.encodeWithSelector(IUZV1RewardPool(address(0)).initialize.selector, name, symbol, initialSupply, owner));
    }
     ...
     * @param name - optional name of pool
        ...
        returns (address)
    {
        ...
        require(
            _basePools[poolType] != address(0),
            "INVALID_POOL_TYPE"
        );

        // get base reward contract for the desired pool type
        address _basePool = _basePools[poolType];
        // address of cloned contract
        address _newPoolAddress = _basePool.clone();
        // new pool instance
        IUZV1RewardPool _newPool = IUZV1RewardPool(_newPoolAddress);
        // init the new pool and assign router
        _newPool.initialize(router, address(_membershipToken));
        // Init the PoolInfo
