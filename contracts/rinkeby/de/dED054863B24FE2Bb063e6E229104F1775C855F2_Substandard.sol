...
contract Substandard is ERC721, Ownable {
    ...
    uint256 public constant nftPrice = 0; //No cost in ETH aside from transaction fee
    ...
    //Need any events on the contract calls for testing purposes? 42:25 in PC tutorial

    //Need to determine where these inbound constructor arguments are being populated in BAYC if static, or possibly as part of a contract call?
    ...
    //Sets some Substandard NFTs aside
    //Is totalSupply() defined in a separate contract? seems to be called by multiple places but not really defined in this contract. Looks like it may be stock and standard in the ERC721 contract.
    //Probably could define an ID rather than just minting them only to owner
    ...
    //Mints Substandard NFTs
    // Aligned to create collectible function in PC tutorial - 1:18 in tutorial
    function mintNFT(uint numberOfTokens) public payable {
        ...
        for(uint i = 0; i < numberOfTokens; i++) {
            ...
            if (totalSupply() < MAX_NFTS) {
                _safeMint(msg.sender, mintIndex); //(owner, item id) for arguments. mintIndex set from incrementing totalSupply
            }
        }
    ...
    //Question - where to set tokenUri on a given item? Separate contract?
