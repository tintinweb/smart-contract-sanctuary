...
contract ERC721CollectionImplementation is ERC721CollectionBase, AdminControlUpgradeable {
     ...
     * Initializer
     */
    function initialize(address creator, uint16 maxTokens_, uint16 purchaseLimit_, uint256 tokenPrice_, address signingAddress) public initializer {
        __Ownable_init();
...
abstract contract ERC721CollectionBase is ReentrancyGuard, CreatorExtension, IERC721Collection, ICreatorExtensionTokenURI {
    ...
    function purchase(uint16 amount, bytes32 message, bytes calldata signature, string calldata nonce) external payable virtual override nonReentrant {
        ...
        require(amount <= tokensRemaining() && amount <= (purchaseLimit-_addressMintCount[msg.sender]), "Too many requested");
    ...
    function state() external override view returns(CollectionState memory) {
        return CollectionState(maxTokens, purchaseLimit, purchaseLimit-_addressMintCount[msg.sender], tokenPrice, maxTokens-tokenCount, active, startTime, endTime, presaleInterval);
    }
