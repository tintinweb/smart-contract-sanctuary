...
contract LPDebtFacility is Ownable {
    ...
    function open (
        ...
        uint ohmAmount = IERC20(OHM).balanceOf(address(global.lpToken));
        uint ohmieAmount = OHMIE.balanceOf(address(global.lpToken));

        OHMIE.safeTransferFrom(msg.sender, address(this), ohmieAmount); // transfer paired token

        _borrow(ohmAmount); // leverage sOHM for OHM to pool
        emit Check(1);

        //(ohmAdded, ohmieAdded, liquidity) = _openPosition(ohmAmount, ohmieAmount);
        //emit Check(3);
    ...
    function close (uint lp) external returns (uint ohmRemoved, uint ohmieRemoved) {
        _update();
        emit Check(0);

        (ohmRemoved, ohmieRemoved) = _closePosition(lp);
        emit Check(2);

        _settle(lp, ohmRemoved);
        emit Check(4);
    ...
    function _borrow (uint amount) internal {
        ...
        require(global.debt <= global.ceiling, "Debt ceiling hit");
    ...
    function _openPosition(uint ohmAmount, uint ohmieAmount) internal returns (
        uint ohmAdded,
        uint ohmieAdded,
        uint liquidity
    ) {
        IERC20(OHM).approve(address(router), ohmAmount);
        OHMIE.approve(address(router), ohmieAmount);

        (ohmAdded, ohmieAdded, liquidity) = // add liquidity
            router.addLiquidity(OHM, address(OHMIE), ohmAmount, 0, ohmieAmount, 0, address(this), block.number);
        ...
        _returnExcess(ohmAdded, ohmAmount, ohmieAdded, ohmieAmount); // return overflow
    }

    // removes liquidity
    function _closePosition (uint liquidity) internal returns (uint ohmRemoved, uint ohmieRemoved) {
        uint lp = userInfo[msg.sender].lp;
        require(lp >= liquidity, "withdraw: not good");

        global.lpToken.approve(address(router), liquidity); // remove liquidity
        (ohmRemoved, ohmieRemoved) = router.removeLiquidity(OHM, address(OHMIE), liquidity, 0, 0, address(this), block.number);
        emit Check(1);

        userInfo[msg.sender].lp = lp.sub(liquidity);
        global.lp = global.lp.sub(liquidity);
    }
