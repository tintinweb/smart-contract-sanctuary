    ...
    function Calculate_reward(address Add) public view returns(uint256)
     {
        ...
        for(uint256 i = 0 ; i < Tokenid[Add].length ; i++){
            if(Tokenid[Add][i] > 0)
            {
              RewardToken += ((block.timestamp - (stakingTime[Add][Tokenid[Add][i]])).div(20))*10**18;
            }
     }
     return RewardToken;
     }
...
//User have to pass tokenID to unstake token
    function unstake(uint256 _tokenId)  external 
        ...
        require(stakingTime[msg.sender][_tokenId] <=  block.timestamp+20 seconds," please withdraw after 48 hours");
