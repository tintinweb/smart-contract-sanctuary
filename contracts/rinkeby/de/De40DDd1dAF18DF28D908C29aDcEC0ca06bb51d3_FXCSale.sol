...
contract FXCSale is Delegated, PaymentSplitter {
  ...
  ContractBalance[2] public results;
  ...
  address[] public sourceContracts;
  ...
  function mintTo(uint[] calldata quantity, address[] calldata recipient) external payable onlyDelegates {
    ...
    for(uint i = 0; i < recipient.length; ++i) {
      for(uint j = 0; j < quantity[i]; ++j) {
        tokenContract.mint(recipient[i]);
      }
    }
  }
  ...
  //public
  function checkHoldings(address _addr) public returns(ContractBalance[2] memory) {
      for (uint i = 0; i < 2; i++) {
