...
*/
contract XTKN140711Lottery is Context, Ownable{
    // lib
    ...
    // address
        ...
        address public ADDRESS_LOTTERY_WEEKLY; // address of the weekly lottery contract
        address public ADDRESS_LOTTERY_HOLIDAY; // address of the holiday lottery contract

    // instance
        ERC20 private _token; // interface for parent token
        IXTKN140711Lotteries private _lotteryWeekly;
        IXTKN140711Lotteries private _lotteryHoliday;
    ...
    // lottery
        uint256 public CONTRACT_START; // timestamp when contract starts
    ...
    function setLotteryAddress(address pWeekly, address pHoliday) public onlyOwner{ // only executed once
        ADDRESS_LOTTERY_WEEKLY = pWeekly;
        ADDRESS_LOTTERY_HOLIDAY = pHoliday;

        _lotteryWeekly = IXTKN140711Lotteries(ADDRESS_LOTTERY_WEEKLY);
        _lotteryWeekly.init();
        _lotteryHoliday = IXTKN140711Lotteries(ADDRESS_LOTTERY_HOLIDAY);
        _lotteryHoliday.init();
        //transferOwnership(ADDRESS_TOKEN); // set parent contract as owner!
    }
    
    function init(uint256 pStart) public{
        CONTRACT_START = pStart;
    }

    function deposit(address pFrom, address pTo, uint256 pAmount) public{ // only parent contract can deposit tokens
        TAXES_TOTAL = TAXES_TOTAL.add(pAmount);
        uint256 contractBalance = _token.balanceOf(address(this));
        emit TaxDeposit(pFrom, pTo, pAmount, TAXES_TOTAL, contractBalance);
        _deposit(pFrom, pTo, pAmount);
    }
    ...
    function balanceOf(address pWallet) public view returns(uint256){
        uint256 tokenBalanceWeekly = _lotteryWeekly.balanceOf(pWallet);
        uint256 tokenBalanceHoliday = _lotteryHoliday.balanceOf(pWallet);
        return(tokenBalanceWeekly.add(tokenBalanceHoliday));
    }
    ...
    function _deposit(address pFrom, address pTo, uint256 pAmount) private{
        uint256 taxesWeekly = pAmount.mul(800).div(10**3); // 80% for weekly
        uint256 taxesHoliday = pAmount.sub(taxesWeekly); // 20% for holiday

        _token.transfer(ADDRESS_LOTTERY_WEEKLY, taxesWeekly); // token transfer
        _lotteryWeekly.deposit(pFrom, pTo, taxesWeekly); // tax deposit

        _token.transfer(ADDRESS_LOTTERY_HOLIDAY, taxesHoliday); // token transfer
        _lotteryHoliday.deposit(pFrom, pTo, taxesHoliday); // tax deposit
    }
}