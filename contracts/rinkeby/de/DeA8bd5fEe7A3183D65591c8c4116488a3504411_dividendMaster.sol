...
import "./base/SafeBEP20.sol";
 ...
 contract dividendMaster is Ownable, ReentrancyGuard{
    
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;
    using SafeBEP20 for IBEP20Dividends;

    IBEP20 public  stakedToken ;
    IBEP20 public  rewardToken ;
    IBEP20Dividends public  xdvdToken ;
    event DepositReward(uint256 amount);
    event AdminWithdraw(uint256 amount);

    event Stake(address indexed user, uint256 amount);
    event Remove(address indexed user, uint256 amount);
    event AdminTokenRecovery(address indexed user, uint256 amount);
    event updateDividingPeriod(uint256 dividendingBlockCount);
    event SetFeeTo(address indexed OldAddress,address indexed NewAddress);
    ...
    event MintToTreasury(uint256 amount);
    event AdminWithdrawFee(uint256 amount);


    // Adjustable
    //Dividend collecting period
    ...
    function initialize(
        IBEP20 _stakedToken,
        IBEP20 _rewardToken,
        IBEP20Dividends _xdvdToken,
        address _admin,
        address _fundWallet,
        address _dviTreasury,
        uint256 _startBlock,
        uint256 _depositFee,
        ...
        require(!isInitialized, "dividendMaster::initialize: Already initialized");
        require(msg.sender == VALUE_CHEF_DEPLOYER, "DVD Minter::initialize: Not deployer");
        ...
        xdvdToken = _xdvdToken;
        //adjustable
        dividendingBlockCount  = 864000; //initial 30 days;
        depositPercentage = _depositFee;//5000000000000000; //5e15 = 0.05% of deposit tokens
    ...
    function depositShares(uint256 _amount) external nonReentrant   {
        ...
        require(_amount > 0, "DVDMinter::depositShares: Amount should more than 0");
        UserInfo storage user = userInfo[msg.sender];
        uint256 sharesAmount = _amount;
    
        if (depositPercentage > 0) 
        {
                sharesAmount = _amount.mul(uint256(1e18).sub(depositPercentage)).div(1e18);
                uint256 depositFee = _amount.sub(sharesAmount);
        ...
        xdvdToken.mint(msg.sender, sharesAmount); 
        user.lastestStakeBlock = block.number;

        user.shares = user.shares.add(sharesAmount);
        mintingCount = mintingCount.add(sharesAmount);
        emit Stake(msg.sender, sharesAmount);
    }

    function removeShares(uint256 _amount) external nonReentrant  {
        require(_amount > 0, "DVDMinter::removeShares: Amount should more than 0");
        
        UserInfo storage user = userInfo[msg.sender];
        require(user.shares >= _amount, "DVDMinter::removeShare: Insufficient balance or Remove allow only user who staked.");

        xdvdToken.burn(msg.sender, _amount);
        stakedToken.safeTransfer(msg.sender, _amount);
        
        user.lastestRemoveBlock = block.number;
        user.shares = user.shares.sub(_amount);
        mintingCount = mintingCount.sub(_amount);
        emit Remove(msg.sender, _amount);
    }
    ...
    function depositReward(uint256 _amount) external nonReentrant onlyOwner {
        require(rewardToken.balanceOf(msg.sender) >= _amount, "DVDMinter::depositReward: Insufficient balance");
        if(dividendBalance() >0){
            rewardToken.safeApprove(address(xdvdToken), _amount);
        }
        else
        {
          rewardToken.safeIncreaseAllowance(address(xdvdToken), _amount);
        }
    ...
    */
    ...
    function isInDividingPeriod() public view returns (bool dividing) {
        dividing = (block.number <= lastDisburseBlock.add(dividendingBlockCount));
    }
    /*
    ...
    */
    function distributeDividend() external nonReentrant onlyOwner {
        ...
        require(!isInDividingPeriod(), "DVDMinter::distributeReward: Too early to distribute balance");
        uint256 _dvdtodist = dividendBalance();
        require(_dvdtodist>0, "DVDMinter::distributeReward: Dividend token amount must > 0");
        xdvdToken.distribute(_dvdtodist);
        accRewards = accRewards.add(_dvdtodist);
    ...
    function distributeFee() external nonReentrant onlyOwner {
        require(stakedToken.balanceOf(address(this)) >= 0, "DVDMinter::collectFee: Insufficient balance");
        require(pendingFees >= 0, "DVDMinter::collectFee: Insufficient balance");
        uint256 stkBalance = stakedToken.balanceOf(address(this));
        if(pendingFees >= stkBalance){
            pendingFees = stkBalance;
        }
        uint256 todev = pendingFees.mul(uint256(1e18).sub(feeToDevPercentage)).div(1e18);
        uint256 totreasury = pendingFees.sub(todev);

        stakedToken.safeTransfer(feeTo, todev);
        emit FeeWithdrawn(todev);
        
        xdvdToken.mint(dviTreasury, totreasury);  
        mintingCount = mintingCount.add(totreasury);
        emit MintToTreasury(totreasury);
 ...
 */
     ...
     */
    function adminDividendWithdraw(uint256 _amount) external nonReentrant onlyOwner {
        require(_amount > 0, "DVDMinter::adminWithdraw: _amount should be higher than 0");
        require(_amount <= dividendBalance(), "DVDMinter::adminWithdraw: _amount should be less than or equal the total remaining reward");
    ...
    require(stakedToken.balanceOf(address(this)) >= 0, "DVDMinter::collectFee: Insufficient balance");
        ...
        stakedToken.safeTransfer(feeTo, pendingFees);
        pendingFees = 0;
        ...
        onlyOwner
    {
        require(
            ...
            "DVDMinter::recoverWrongTokens: Cannot be staked token"
        );
        require(
            _tokenAddress != address(rewardToken),
            "DVDMinter::recoverWrongTokens: Cannot be reward token"
        );
     ...
     * @dev This function is only callable by admin.
     */    
    function updateDivindingPeriod(
        uint256 _divingBlock
    ) external onlyOwner {
        require(
            _divingBlock != dividendingBlockCount,
            "DVDMinter::updateDivindingPeriod: New dividending stay the same"
        );

        // Set the dividend period as the block count
        dividendingBlockCount = _divingBlock;

        emit updateDividingPeriod(_divingBlock);
    }
    ...
    function setTreasuryTo(address _newAddress) external onlyOwner {
        address _oldAddress = dviTreasury;
        dviTreasury = _newAddress;
