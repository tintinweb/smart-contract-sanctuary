...
contract CrucibleRouter is MultiSigCheckable, HasTaxDistributor {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    string constant public NAME = "FERRUM_CRUCIBLE_ROUTER";
    ...
    bytes32 constant SET_OPEN_CAP =
        keccak256("SetOpenCap(address crucible,uint256 cap,uint64 expiry)");
	function setOpenCap(
		address crucible,
		uint256 cap,
		uint64 expiry,
	...
	external onlyAdmin expiryRange(expiry) {
        ...
        bytes32 message = keccak256(abi.encode(SET_OPEN_CAP, crucible, cap, expiry));
        verifyUniqueMessageDigest(
			message, expectedGid(crucible, expectedGroupId), multiSignature);
		openCaps[crucible] = cap;
	}

    bytes32 constant DEPOSIT_METHOD =
        keccak256("Deposit(address to,address crucible,uint256 amount,bytes32 salt,uint64 expiry)");
	function deposit(
		address to,
		address crucible,
		uint256 amount,
		...
		uint16 expectedGroupId,
		bytes memory multiSignature)
	external expiryRange(expiry) returns (uint256) {
        bytes32 message = keccak256(abi.encode(
			DEPOSIT_METHOD, to, crucible, amount, salt, expiry));
        verifyUniqueSalt(
			message, salt, expectedGid(crucible, expectedGroupId), multiSignature);

		address token = ICrucibleToken(crucible).baseToken();
		SafeAmount.safeTransferFrom(token,  msg.sender, crucible, amount);
	...
	struct Amounts { uint base; uint pair; bool isWeth; }
	...
	function depositAddLiquidityStake(
		...
		bytes memory multiSignature)
	external {
		verifyDepositSig(to, crucible, pairToken,
			baseAmount, pairAmount, ammRouter, stake, expiry, expectedGroupId, multiSignature);
		pairAmount = SafeAmount.safeTransferFrom(pairToken, msg.sender, address(this), pairAmount);
		address pool;
		{
			address baseToken;
			(baseToken, baseAmount) = _depositToken(crucible, baseAmount);
			pool = IUniswapV2Factory(IUniswapV2Router01(ammRouter).factory()).getPair(pairToken, baseToken);
		}
		Amounts memory amounts = Amounts({
			base: baseAmount,
			pair: pairAmount,
			isWeth: false});
		_depositAddLiquidity(to, crucible, pairToken, amounts, IUniswapV2Router01(ammRouter), deadline);
		IStakeFor(stake).stakeFor(pool, to);
	}
	...
	external payable {
		...
		verifyDepositSig(to, crucible, weth, baseAmount, msg.value, ammRouter, stake, expiry,
			expectedGroupId, multiSignature);
		...
		Amounts memory amounts = Amounts({
			...
			pair: msg.value,
			isWeth: true});
		_depositAddLiquidity(to, crucible, weth, amounts, IUniswapV2Router01(ammRouter), deadline);
		address pool;
		IStakeFor(stake).stakeFor(pool, to);
	}
	...
	function stakeFor(
		...
		IStakeFor(stake).stakeFor(token, to);
	}

	function _depositToken(address crucible, uint256 amount) private returns (address, uint256) {
		address token = ICrucibleToken(crucible).baseToken();
		SafeAmount.safeTransferFrom(token,  msg.sender, crucible, amount);
		return (token, ICrucibleToken(crucible).deposit(address(this)));
	}

	function _depositAddLiquidity(
		address to,
		address crucible,
		address pairToken,
	...
	internal {
		...
		(uint amountA, uint amountB,) = ammRouter.addLiquidity(
			crucible,
			pairToken,
			amounts.base,
			amounts.pair,
		...
		if (pairLeft != 0) {
			if (amounts.isWeth) {
				...
				SafeAmount.safeTransferETH(msg.sender, pairLeft); // refund dust eth, if any
			} else {
				IERC20(pairToken).transfer(msg.sender, pairLeft);
			}
		}
	}

	bytes32 DEPOSIT_ADD_LIQUIDITY_STAKE_METHOD = keccak256(
		"DepositAddLiquidityStake(address to,address crucible,address pairToken,uint256 baseAmount,uint256 pairAmount,address ammRouter,address stake,uint64  expiry)");
	...
	private expiryRange(expiry) {
        bytes32 message = keccak256(abi.encode(
			DEPOSIT_ADD_LIQUIDITY_STAKE_METHOD, to, crucible, pairToken, baseAmount, pairAmount, ammRouter, stake, expiry));
        verifyUniqueMessageDigest(message, expectedGid(crucible, expectedGroupId), multiSignature);
	}

    bytes32 constant OVERRIDE_FEE_METHOD =
        keccak256("OverrideFee(address crucible,address target,uint8 overrideType,uint64 newFeeX10000,uint64 expiry)");
	...
	external expiryRange(expiry) {
        bytes32 message = keccak256(abi.encode(
			OVERRIDE_FEE_METHOD, crucible, target, uint8(overrideType), newFeeX10000, expiry));
        verifyUniqueMessageDigest(
			message, expectedGid(crucible, expectedGroupId), multiSignature);
		ICrucibleToken(crucible).overrideFee(target, overrideType, newFeeX10000);
	}
	...
	function expectedGid(address crucible, uint16 expected)
	private view returns (uint16 gid) {
		gid = expected;
		require(expected < 256 || delegatedGroupIds[crucible] == expected, "CR: bad groupId");
...
abstract contract MultiSigCheckable is WithAdmin, EIP712 {
    ...
    function verifyUniqueMessageDigest(
        ...
        bytes memory multiSignature) internal {
		require(multiSignature.length != 0, "MSC: multiSignature required");
        (bytes32 salt, bool result) = tryVerify(message, expectedGroupId, multiSignature);
		require(result, "MSC: Invalid signature");
    ...
    */
    function tryVerify(
		...
		for (uint i=0; i < signatures.length; i++) {
			...
			require(expectedGroupId == 0 || q.groupId ==  expectedGroupId,
	...
	modifier governanceGroupId(uint16 expectedGroupId) {
		require(expectedGroupId < 256, "MSC: must be governance");
		_;
	}
}
...
interface IStakeFor {
    function stakeFor(address token, address staker) external returns (uint256);
}
...
abstract contract CrucibleTokenDeployer is ICrucibleTokenDeployer {
    ...
    function deploy(
        ...
        string memory symbol
				...
				(bytes32 n1, bytes32 n2, bytes32 n3) = StringLib.strToB32(name);
				(bytes32 s1, bytes32 s2,) = StringLib.strToB32(symbol);
...
contract CrucibleToken is ERC20, TokenReceivable, ICrucibleToken {
	...
	address immutable public override baseToken;
	uint64 immutable public feeOnTransferX10000;
	uint64 immutable public feeOnWithdrawX10000;
	bytes32 name1; bytes32 name2; bytes32 name3;
	bytes32 symbol1; bytes32 symbol2;
	...
	constructor() {
		address token;
		address fac;
		(fac, token, feeOnTransferX10000, feeOnWithdrawX10000, name1, name2, name3, symbol1, symbol2) = 
	...
	function name() external override view returns (string memory) {
		return StringLib.b32ToStr(name1, name2, name3, 64);
	}

	function symbol() external override view returns (string memory) {
		return StringLib.b32ToStr(symbol1, symbol2, bytes32(0), 32);
	}
...
interface ICrucibleTokenDeployer {
    function parameters() external returns
		(address, address, uint64, uint64, bytes32, bytes32, bytes32, bytes32, bytes32);
}
