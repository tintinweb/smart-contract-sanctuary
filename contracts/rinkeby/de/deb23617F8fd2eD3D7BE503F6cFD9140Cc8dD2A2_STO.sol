...
contract STO is Ownable {
    ...
    uint256 public tokenDecimalFactor;
    ...
    mapping(uint256 => PropertyInfo) public propertyMapping;
    ...
    event propertyValueUpdate(uint256 _id, uint256 _propertyValue);
    event buyProperty(
        uint256 _id,
        uint256 _tokenAmount,
    ...
    function whitelistAddress(
        ...
        emit userWhitelisted(_userRole, _userAddress, _status);
    }
    ...
    ) external onlyOwner checkWhiteList(_to) {
        require(whitelistMapping[_to].userRole == 2, "Investor required");
        creatorCounter++;
    ...
    function buy(uint256 _id, uint256 _usdtTokenAmount)
        external
        ...
        validId(_id)
    {
        ...
        uint256 _propertyTokenAmount = (
            usdtAmountInDollar.mul(propertyMapping[_id].totalSupply)
        ).div(propertyMapping[_id].propertyValue);
        ...
        if (!isAccreditedUser) {
            require(
                ...
                "Amount not available for non accredited user."
            );
        }
        require(
        ...
        if (creatorCoinInstance.balanceOf(msg.sender, _id) > 0) {
            require(
        ...
        validId(_id)
    {
        ...
        emit propertyValueUpdate(_id, _propertyValue);
    }
    ...
    function claimIncentive(uint256 _id) external {
        ...
        uint256 _incentiveAmount = (
            creatorCoinInstance.balanceOf(msg.sender, _id).mul(
        ...
        emit incentiveInfo(claimCount, _incentiveAmount, _id, msg.sender);
    }
    ...
    {
        require(!isOfferCreated[msg.sender][_id], "Offer already created");
        require(
    ...
    function acceptOffer(
        ...
        Offer memory currentOffer = idToOfferMapping[_id][_offerId];
        require(currentOffer.isActive, "Inactive Offer");
        require(
            currentOffer.offerCreator != msg.sender,
    ...
    modifier checkWhiteList(address _address) {
        require(
            whitelistMapping[_address].isWhitelisted,
    ...
    modifier validTokenAndUsdtAmount(
        ...
        require(_usdtTokenAmount > 0, "Invalid usdt amount");
        _;
    }
    ...
    function updateCreatorCoinAddress(address _creatorCoinAddress)
        external
        onlyOwner
    {
        creatorCoinAddress = _creatorCoinAddress;
