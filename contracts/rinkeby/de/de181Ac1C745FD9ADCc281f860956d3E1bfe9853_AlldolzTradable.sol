...
contract Alldolz is ERC721Enumerable, Ownable, ERC721Pausable {
    uint256 public constant SALE_PRICE = 0.068 ether;
    uint256 public constant PRESALE_PRICE = 0.068 ether;
    ...
    uint256 public constant PRESALE_MINT_LIMIT = 5;
    ...
    event TokenMinted(uint256 indexed id);
    ...
    function mintReserve(address _to, uint256 _amount) public onlyOwner {
        ...
        require(total + _amount <= MAX_SUPPLY, "Max limit");
    ...
    function mint (address _to, uint256 _amount) public payable {
        ...
        require(msg.value >= SALE_PRICE * _amount, "Value below price");
    ...
    function presaleMint(address _to, uint256 _amount) public payable {
        ...
        require(_presaleListClaimed[_to] + _amount <= PRESALE_MINT_LIMIT, 'Purchase exceeds max allowed');
        require(_amount <= PRESALE_MINT_LIMIT, "Exceeds number");
        require(msg.value >= PRESALE_PRICE * _amount, "Value below price");
...
contract AlldolzTradable is ContextMixin, Alldolz, NativeMetaTransaction {
    address proxyRegistryAddress;

    constructor(address _proxyRegistryAddress, string memory _name, string memory _symbol, string memory _baseURI) Alldolz(_name, _symbol, _baseURI) {
