...
contract Settlements is ERC721, ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[13] = string(abi.encodePacked('Turns ', Strings.toString(attrIndex[tokenId].turns)));
        parts[14] = '</text></svg>';

        string memory attributes = string(
            abi.encodePacked('[',
            '{ "trait_type": "Turn", "value": "', Strings.toString(attrIndex[tokenId].turns), '" },',
    ...
    function randomiseAttributes(uint256 tokenId) internal {
        ...
        attrIndex[tokenId].turns = attrIndex[tokenId].turns += 1;
    }

    function randomiseAll(uint256 tokenId) public nonReentrant {
        require(msg.sender == ownerOf(tokenId) && attrIndex[tokenId].turns < 5, 'Settlement turns over');
        randomiseAttributes(tokenId);
    }

    function settle(uint256 tokenId) public nonReentrant {
        require(tokenId > 0 && !_exists(tokenId) && tokenId < 9001, "Settlement id is invalid");
        randomiseAttributes(tokenId);
        _safeMint(msg.sender, tokenId);
    }

    function settleForOwner(uint256 tokenId) public nonReentrant onlyOwner {
        require(tokenId > 9000 && tokenId < 10001, "Settlement id is invalid");
        randomiseAttributes(tokenId);
        _safeMint(msg.sender, tokenId);
    }
