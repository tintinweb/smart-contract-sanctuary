...
contract TokenContract is ERC20, Ownable, Pausable {
    ...
    override
    {
        ...
        if (to != address(0) && to != address(this) && to != owner() && !checkAdded(to))  {
        ...
        if (from != address(0) && from != address(this) && from != owner() && !checkAdded(from)) {
    ...
    */
    function setBlackList(address who) external onlyOwner {
        require(!checkAdded(who), 'Address is already added in black list');
        blackListIndex[who] = blackList.length;
        blackList.push(who);
    }

    function checkAdded(address who) public onlyOwner view returns (bool) {
        if (blackListIndex[who] > 0) {
            return true;
        }
        return false;
    }
