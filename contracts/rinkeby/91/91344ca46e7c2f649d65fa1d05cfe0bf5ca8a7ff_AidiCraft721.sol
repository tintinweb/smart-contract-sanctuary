...
contract AidiCraft721 is
    ...
    ERC721Burnable
 {
    ...
    IInterfaceAidiFee aidifeecon = IInterfaceAidiFee(0x711C17C792C451b3ac40A0274cB4F28ebF5B9ED3);
    ...
    constructor(
        ...
        addTokenType("WETH",0x6Ad21c4C2e6704f3a3cd8E46E4A9Ce945057d0c6,2500000000000000000);//todo change to WETH mainnet
        addTokenType("VERSE",0xac6b61387277f3d150a4900bc320D2F793FB3B55,0);//todo change to Verse mainnet
    }
    ...
    function _acceptBId(string memory tokenAss,address from, address admin, uint256 amount, uint256 tokenId) internal{
        ...
        require( approveValue >= val, "Approved value mismatch!");
        require(balances[tokenId][msg.sender] > 0, "You should be the token owner");
        (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], dyntokfee);
    ...
    function changePrice(uint256 value, uint256 tokenId) public{
        require( balances[tokenId][msg.sender] > 0, "You should be the token owner");
        require( value < order_place[msg.sender][tokenId].price, "Price should be greater than the previous price");
    ...
    function burnToken(uint256 id,address from) public{
        require( balances[id][from] == 1, "You should be the token owner and must have sufficient balance!");
            burn(id);
