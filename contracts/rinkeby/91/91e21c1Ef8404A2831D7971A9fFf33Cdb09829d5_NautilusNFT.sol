...
*/
contract NautilusNFT is ERC721Enumerable, Ownable, ReentrancyGuard{
    ...
    uint8 public constant TOTAL_LIMIT = 5;
    ...
    mapping(address => bool) isWithelisted;
    ...
    function mint() public payable nonReentrant{
        ...
        require(transactionsForAddress[msg.sender].length < TOTAL_LIMIT,"Cannot buy more than limit");
        require(msg.value > 0,"Cannot buy 0");
        require(totalPublicSupply + 1 <= MAX_SUPPLY,"All has been purchased");
    ...
    function _mintToken(address _minter, uint256 _amount) private{
        ...
        uint256 rand = getRandom(totalPublicSupply);

        totalPublicSupply += 1;
        _safeMint(_minter, totalPublicSupply);
        ...
        // Store NFT Metadata
        nfts[totalPublicSupply] = NFT(totalPublicSupply,getRandomType(rand),getRandomLevel(rand));
    ...
    function isWhitelisted(address _AddressToCheck) public view returns(bool){
        return isWithelisted[_AddressToCheck];
    }

    /* MARKETING */

    //Allows to airdrop token for marketing 
    function marketingAirdrop(address _receiver) external onlyOwner {
      require(airdroped < MAX_AIRDROP, 'Max marketing airdrop reached');
      require(totalPublicSupply < MAX_SUPPLY, "Collection Sold out");

      airdroped++;

      _mintToken(_receiver, 0);
    }
    ...
    // Allows to cancel sale if something goes wrong
    ...
    function claimSale() public nonReentrant{
        require(tx.origin == msg.sender, "Contracts not allowed");
        require(!isActive,"Sale is not inactive");
    ...
    function getRandomLevel(uint256 _seed) internal pure returns (uint8){
        //33% luck on each levels
        return uint8(uint256(keccak256(abi.encode(_seed, 2))) % 3)+1;  
    }
