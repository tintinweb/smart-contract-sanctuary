...
contract NFT4Play is ERC721, Authorizable {
    ...
    constructor(string memory _baseTokenURI) public ERC721("NFT4Play", "4Play") {
    ...
    function mint(string memory _faction, address _to) public onlyAuthorized() {
        totalSupply+=1;
        tokenFaction[totalSupply] = _faction;
        _holderTokens[_to].add(totalSupply);
        _mint(_to,totalSupply);
    }

    function batchMint(string memory _faction, address _to, uint256 _totalToken) external onlyAuthorized() {
        require(_totalToken != 0, "NFT4Play: number of nfts to mint should be non-zero");
