...
contract CannabisMarket is AccessControl {
    using Counters for Counters.Counter;
    IERC20 ERC20;
    Counters.Counter private _marketIdCounter;
    uint256 fee = 10000 wei;
    uint256 rateFee = 5;
    address _wallet;
    ...
    event PlaceItem(address item, uint256 price, uint256 marketId, uint256 tokenId);
    ...
    function _getItemInfo(uint256 marketId) public view returns(bool, Item memory) {
        ...
        return (false, Item(address(0), address(0), address(0), 0, 0, 0, 0, false));
    }
    function placeItem(
        address item,
        uint256 tokenId,
        ...
        require(price > 0, "Price must grater than 0");
        require(expiration > block.timestamp + 1 days, "Expiration must grater than 1 days");
        ...
        emit PlaceItem(item, price, marketId, tokenId);
    }
    function buyItem(uint256 marketId) public onlyExistItem(marketId){
        (, Item memory itemData) = _getItemInfo(marketId);
        require(ERC20.balanceOf(msg.sender) >= itemData._price, "Balance is not enough");
        require(itemData._buyer == address(0), "Item is already sold");
        require(msg.sender != itemData._owner, "You already owned this item");
        require(IERC721(itemData._item).ownerOf(itemData._tokenId) == itemData._owner, "Seller does not own this item (ERC721)");
        uint feePrice = itemData._price * rateFee / 100;

        ERC20.transferFrom(msg.sender, itemData._owner, itemData._price - feePrice);
        ERC20.transferFrom(msg.sender, itemData._owner, feePrice);
    ...
    function cancelPlacement(uint256 marketId) public onlyExistItem(marketId) returns(bool){
        (, Item memory itemData) = _getItemInfo(marketId);
    ...
    function makeOffer(uint256 marketId, uint256 price, uint256 expiration) public onlyExistItem(marketId){
        require(block.timestamp >= expiration, "Offer is expired");
        (, Item memory itemData) = _getItemInfo(marketId);
        require(itemData._price <= ERC20.balanceOf(msg.sender), "Balance is not enough");
        require(itemData._buyer == address(0), "Item is already sold");
    ...
    function getOfferById(address owner, uint256 offerId) public view returns(Offer memory){
        return offers[owner][offerId];
    }
    function acceptOffer(uint256 marketId, uint256 offerId)  public onlyExistItem(marketId){
        (, Item memory itemData) = _getItemInfo(marketId);
        require(itemData._buyer == address(0), "Item is already sold");
        Offer memory offerData = getOfferById(msg.sender, offerId);
        require(offerData._active, "Item is not active");
        ...
        require(ERC20.balanceOf(offerData._buyer) >= offerData._price, "Balance buyer is not enough");
        
        ERC20.transferFrom(offerData._buyer, itemData._owner, offerData._price - offerData._price * rateFee / 100);
        ERC20.transferFrom(offerData._buyer, _wallet, offerData._price * rateFee / 100);
    ...
    function cancelOffer(uint256 marketId, uint256 offerId) public onlyExistItem(marketId){
        (, Item memory itemData) = _getItemInfo(marketId);
        require(offers[itemData._owner].length > offerId, "Invalid offerId");
        require(offers[itemData._owner][offerId]._buyer == msg.sender, "You can't cancel this offer");
        require(ERC20.balanceOf(msg.sender) >= fee, "Balance is not enough to pay fee");
        ERC20.transferFrom(msg.sender, _wallet, fee);

        offers[itemData._owner][offerId]._active = false;
    }
    function setFee(uint256 _fee, uint256 _rateFee) public onlyRole(DEFAULT_ADMIN_ROLE){
        require(rateFee < 100, "Rate fee is incorrect");
        fee = _fee;
        rateFee = _rateFee;
    }   
    ...
    function getMarketId(address item, address owner, uint256 tokenId, bool status) public view returns(bool, uint256){
        for(uint i = 0; i < items.length; i++){
            if(items[i]._available == status && items[i]._owner == owner && items[i]._tokenId == tokenId && items[i]._item == item){
