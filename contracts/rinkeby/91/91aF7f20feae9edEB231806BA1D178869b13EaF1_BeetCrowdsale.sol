...
contract BeetCrowdsale is CappedCrowdsale {
  ...
  BeetTokenVesting private _vesting;
  ...
  modifier onlyWhileOpen() {
    require(isOpen(), "BeetCrowdsale: not open");
    _;
  }

  constructor(address wallet_, address token_, address[] memory collaterals_, BeetTokenVesting vesting_)
  ...
  function setDirectWithdrawPercentage(uint256 percentage_)
    ...
    onlyAdmin
  {
    require(!isClosed(), "BeetCrowdsale::setDirectWithdrawPercentage: sales is already closed");
    require(percentage_ >= 0 && percentage_ <= 100, "BeetCrowdsale::setDirectWithdrawPercentage: withdraw percentage should be >= 0 and <= 100");
  ...
  function openSale()
    ...
    onlyAdmin
  {
    require(_state == State.None, "BeetCrowdsale::openSale: sales is already open or closed");
    _state = State.Open;
  ...
  function closeSale()
    ...
    onlyAdmin
  {
    require(isOpen(), "BeetCrowdsale::closeSale: sales is already closed or not open");
    _state = State.Closed;
  ...
  function addRound(uint256 price_, uint256 totalSupply_, uint8 pool_)
    ...
    onlyAdmin
  {
    require(!isClosed(), "BeetCrowdsale::addRound: sales is already closed");
  ...
  function updateRound(uint256 index_, uint256 price_, uint256 totalSupply_, uint8 pool_) 
    ...
    onlyAdmin
  {
    require(_rounds[index_].defined, "BeetCrowdsale::updateRound: no round with provided index");
    require(_rounds[index_].state != State.Closed, "BeetCrowdsale::updateRound: round is already closed");
    require(!isClosed(), "BeetCrowdsale::updateRound: sales is already closed");
  ...
  function openRound(uint256 index_)
    ...
    onlyAdmin 
  {
    require(isOpen(), "BeetCrowdsale::openRound: sales is not open yet");
    require(_rounds[index_].defined, "BeetCrowdsale::openRound: no round with provided index");
    require(_rounds[index_].state == State.None, "BeetCrowdsale::openRound: round is already open or closed");
  ...
  function closeRound(uint256 index_)
    ...
    onlyAdmin 
  {
    require(isOpen(), "BeetCrowdsale::closeRound: sales is not open yet");
    require(_rounds[index_].defined, "BeetCrowdsale::closeRound: no round with provided index");
    require(_rounds[index_].state == State.Open, "BeetCrowdsale::closeRound: round is not open");
  ...
  function withdrawTokens()
    external
  {
    require(isClosed(), "BeetCrowdsale::withdrawTokens: sales is not closed yet");
  ...
  function _preValidatePurchase(address beneficiary_, address collateral_, uint256 amount_)
    ...
    onlyWhileOpen
  {
    require(_state == State.Open, "BeetCrowdsale::_preValidatePurchase: sales is not open yet");
    require(_rounds[_activeRound].state == State.Open, "BeetCrowdsale::_preValidatePurchase: sales round is not open yet");
    require(
      _rounds[_activeRound].totalSupply >= _rounds[_activeRound].investment + _getTokenAmount(collateral_, amount_),
      "BeetCrowdsale::_preValidatePurchase: exceeded round total supply"
    );
  ...
  function _processPurchase(address beneficiary_, uint256 tokenAmount_)
    ...
    override
  {
    require(tokenBalance() >= (_tokensSold + tokenAmount_) - _tokensWithdrawn, "BeetCrowdsale::_processPurchase: not enough tokens to buy");
    ...
    onlyAdmin
  {
    if (address(getToken()) == token_) {
      ...
      require(tokensAfterAdminWithdrawal >= tokensLocked, "BeetCrowdsale::recoverERC20: cannot withdraw already sold tokens");
    }
