...
/*
 * The contact enables the storage of erc20 tokens under the linear time-vesting with the cliff time-lock.
...
contract TimelockVault is AccessControl {
    ...
    */
    function addBalance(uint256 amount, address to_user) public {
        ...
        SU_DAO.safeTransferFrom(msg.sender, address(this), (amount / 1e12) * 1e12);
        accounts[to_user].amount_under_vesting_div1e12 = accounts[to_user].amount_under_vesting_div1e12 + uint64(amount / 1e12);
    }

    /**
    * @notice Balance of user of all tokens under vesting remaining after withdraws.
    */
    function balanceOf(address user) external view returns (uint256) {
        return uint256(accounts[user].amount_under_vesting_div1e12 - accounts[user].amount_already_withdrawn_div1e12) * 1e12;
    }
    ...
    function claim() external {
        ...
        SU_DAO.safeTransfer(msg.sender, (claim_amount / 1e12) * 1e12);
    }
    ...
    */
    function rescue(IERC20 token) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(token != SU_DAO, "No allowed to rescue this token");
...
// some NFT gives an option to acquire suDAO tokens
contract TokenDistributorForNftHolders_v2 is Ownable {
    using SafeERC20 for IERC20;
    ...
    struct DistributionInfo {
        uint256 maximumRewardAllocation;// Maximum number of suDAO Tokens to acquire
    ...
    function participate(uint256 distributionId, uint256 donationAmount) payable external {
        ...
        require(TIMELOCK_VAULT.balanceOf(msg.sender) + rewardAmount <= distribution.maximumRewardAllocation,
    ...
    function getMaximumDonationAmount(uint256 distributionId, address user) view external returns(uint256) {
        DistributionInfo storage distribution = distributions[distributionId];
        return distribution.maximumRewardAllocation - TIMELOCK_VAULT.balanceOf(user);
    }
