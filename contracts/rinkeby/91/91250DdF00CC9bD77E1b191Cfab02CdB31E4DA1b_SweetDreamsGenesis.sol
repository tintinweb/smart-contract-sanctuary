...
contract SweetDreamsGenesis is ERC721Enumerable, Ownable, VRFConsumerBase {
    ...
    uint16 public constant MAX_DREAMS_RESERVED = 20;
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        // [0;3[ -> revealed from the beginning for bids
        if (tokenId < 3) {
            return string(abi.encodePacked(_baseURI(), Strings.toString(tokenId)));
        }
        // [3;MAX_SUPPLY[ -> hidden until reveal
        if (rng1 == 0 || rng2 == 0) {
            return string(abi.encodePacked(_baseURI(), "placeholder"));
        }
        // [3;MAX_DREAMS_RESERVED[ -> reserved tokens
