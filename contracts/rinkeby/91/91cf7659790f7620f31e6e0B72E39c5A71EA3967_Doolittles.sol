...
contract Doolittles is ERC721Enumerable, CollaborativeOwnable, ReentrancyGuard {
  ...
  uint256 public whitelistMintPrice = 33000000000000000;
  uint16 public whitelistMintLimit = 2;
  bool public whitelistMintIsActive = false;

  address public withdrawAddress1 = address(0);
  address public withdrawAddress2 = address(0);
  address public withdrawAddress3 = address(0);

  constructor(address _proxyRegistryAddress, address _withdrawAddress1, address _withdrawAddress2, address _withdrawAddress3) ERC721("Doolittles", "DOOLITTLES") {
    proxyRegistryAddress = _proxyRegistryAddress;

    withdrawAddress1 = _withdrawAddress1;
    withdrawAddress2 = _withdrawAddress2;
    withdrawAddress3 = _withdrawAddress3;
  }
  ...
  function withdraw() external onlyCollaborator nonReentrant {
    ...
    uint256 cut = balance.mul(33).div(100);

    payable(withdrawAddress1).transfer(cut);
    balance = balance.sub(cut);

    payable(withdrawAddress2).transfer(cut);
    balance = balance.sub(cut);

    payable(withdrawAddress3).transfer(balance);
  }
}
