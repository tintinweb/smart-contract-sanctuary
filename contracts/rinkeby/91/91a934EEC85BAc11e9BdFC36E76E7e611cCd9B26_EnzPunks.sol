...
//ESTA LIBRERIA TRANSFORMA UN BYTE → BASE 64
...
// File contracts/EnzPunksDNA.sol


pragma solidity ^0.8.0;

contract EnzPunksDNA{ //Lo usaremos para que sus funciones se hereden
    ...
    // This pesudo random function is deterministic and shouldn't be use in production
    function deterministicPseudoRandomDNA(uint256 _tokenId, address _minter) public pure returns(uint256){
      ...
      bytes32 hashedParams = keccak256(encodedParams); //Genera el hash a partir del dato encodeado
     //keccak256 es una funcion deterministica de hashes ( hashes → funcion matematica que su resultado es dificil que vuelva a suceder )
      return uint256(hashedParams);
    }
    // Get attributes

    uint8 constant ADN_SECTION_SIZE = 2;
    function _getDNASection (uint256 _dna, uint8 _rightDiscard) internal pure returns(uint8){ //_rightDiscard →(lo que queremos descartar)
...
// File contracts/EnzPunks.sol


pragma solidity ^0.8.0;
contract EnzPunks is ERC721, ERC721Enumerable, EnzPunksDNA{
  
  using Strings for uint256;
  using Counters for Counters.Counter;
  Counters.Counter private _idCounter;
  uint256 public maxSupply;
  mapping (uint256 => uint256) public tokenDNA; //TOKEN ID → ADN
  // El constructor necesita dos parámetros, el nombre y la sigla del NFT.
  constructor(uint256 _maxSupply) ERC721("EnzPunks", "PLPKS"){
    maxSupply = _maxSupply; //definimos la cantidad de NFTs
  }

  function mint() public {
    uint256 current = _idCounter.current(); //viene directo desde openzepelin
    require(current < maxSupply, "No EnzPunks left :c");
    tokenDNA[current] = deterministicPseudoRandomDNA(current, msg.sender);
    _safeMint(msg.sender, current); //Se almacena el dueño en una de las funciones de openZepelin
  ...
  function imageByteDNA(uint _dna) public view returns (string memory){
    string memory baseURI = _baseURI();
    string memory paramsURI = _paramsURI(_dna);
    return string(abi.encodePacked(baseURI, paramsURI));
  }

  //consultar la metada de un contrato ↓ debemos sobreeescribir esta function pq ya la estamos heredando
  function tokenURI(uint256 tokenId) public view override returns(string memory){
    require(_exists(tokenId),"ERC721 Metada: URI query for nonecistent token");//_existent, dado un token ID nos indica si existe o no
    uint256 dna = tokenDNA[tokenId];
    string memory image = imageByteDNA(dna);
    string memory jsonUri = Base64.encode(abi.encodePacked(
          '{ "name": "EnzPunks #',
           ...
           image,
           '"}'
    )); //Concatena strings para retornar en formato bytes → string
    
    return string(abi.encodePacked("data:application/json;base64,", jsonUri));
  }

  //Override requerid (sobreescribir funciones)
