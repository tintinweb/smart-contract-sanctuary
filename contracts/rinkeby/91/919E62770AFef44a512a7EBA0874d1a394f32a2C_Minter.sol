...
contract Minter is AccessControlEnumerableUpgradeable, Provenance, PaymentSplitterUpgradeable {
        ...
        payable
    {
        ...
        uint256 userBalance = IBaseToken(projectTokenNeeded).getUserBalance(msg.sender);
        ...
        for (uint i = 0; i < userBalance; i++) {
          if(!tokenWhitelist[projectTokenNeeded][IBaseToken(projectTokenNeeded).getTokenOfUserByIndex(msg.sender, i)]){
            unusedTokenCount += 1;
            tokenWhitelist[projectTokenNeeded][IBaseToken(projectTokenNeeded).getTokenOfUserByIndex(msg.sender, i)] = true;
    ...
    function getTokenWhitelistStatus(address projectAddress, uint256 index) public view returns(bool){
      uint256 userBalance = IBaseToken(projectAddress).getUserBalance(msg.sender);
      require(userBalance > 0, "You do not own any NFTs from the project needed for the whitelist");
      bool status = tokenWhitelist[projectAddress][IBaseToken(projectAddress).getTokenOfUserByIndex(msg.sender, index)];
      return status;
    }

    function getWhitelistTokens(address projectAddress) public view returns(uint256){
      require(
          IRegistry(registry).getProjectStatus(tokenContract) == IRegistry.ProjectStatus.WhitelistByToken,
          "Minter: whitelist by token is not active"
      );
      ...
      if(userBalance > 0){
        for (uint i = 0; i < userBalance; i++) {
          if(!tokenWhitelist[projectAddress][IBaseToken(projectAddress).getTokenOfUserByIndex(msg.sender, i)]){
