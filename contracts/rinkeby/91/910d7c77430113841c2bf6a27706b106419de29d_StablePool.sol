...
contract StablePool {
    ...
    function swapThrough( 
        ...
        IERC20( _firstToken ).safeTransferFrom( _from, address(this), _amount );

        amount_ = _swap( _firstToken, _amount, _secondToken );

        IERC20( _secondToken ).approve( _to, amount_ );
    }
    ...
    function maxOrderSize( address _firstToken, address _secondToken ) public view returns ( uint ) {
        return maxCanAdd( _firstToken ).add( maxCanRemove( _secondToken ) );
    }
