...
contract DNA {
    ...
    function _getDNASection (uint256 _dna, uint8 _rightDiscard) internal pure returns (uint8) {
        return uint8((_dna % (10 ** (_rightDiscard + ADN_SECTION_SIZE))) / (10 ** _rightDiscard));
    }
...
contract MyNfts is ERC721, ERC721Enumerable, DNA, Ownable {
    ...
    constructor(uint256 _maxSupply) ERC721("MyNft", "VDC") {
    ...
    function mint(address _to) public {
        ...
        require(current < maxSupply, "No quedan MyNfts disponibles" );

        tokenDNA[current] = deterministicPseudoRandomDNA(current, _to);

        _safeMint(_to, current);
    ...
    function _paramsURI(uint256 _dna) internal view returns (string memory) {
        ...
        return string(abi.encodePacked(params,
            "&graphicType=",
            getGraphicType(_dna),
            "&mouthType=",
            getMouthType(_dna),
    ...
    function tokenURI(uint256 tokenId) public view override returns(string memory) {
        require(_exists(tokenId), "ERC721 Metada: URI query for nonexistent token");

        uint256 dna = tokenDNA[tokenId];
        string memory image = imageByDNA(dna);

        string memory jsonURI1 = string(
            abi.encodePacked(
                '{"name" : "Princess # ',
                tokenId.toString(),
                '", "description" : "el mejor NFT que puedes tener es MyNFT" , "image" : "',
                image,
                '", "attributes" : [{"trait_type": "Accessorie Type", "value": "',
        ...
        string memory jsonURI2= string(
         ...
         string memory jsonURI = Base64.encode(
             abi.encodePacked(jsonURI1,jsonURI2)
         );
...
// File contracts/PrincessFactory.sol
...
contract PrincessFactory is FactoryERC721, Ownable {
    ...
    string public baseURI = "https://creatures-api.opensea.io/api/factory/0";


    /*
     * Enforce the existence of only 10 Princess.
     */
    uint256 Princess_SUPPLY = 10;

    constructor() {
        //proxyRegistryAddress = _proxyRegistryAddress;
        nftAddress = address(0x4F99c1365b06D67F4Bd4F622f764F34F72F604f2);
    }

    function name() override external pure returns (string memory) {
        return "princessNft Item Sale";
    }
    ...
    function mint(uint256 _optionId, address _toAddress) override public {
        ...
        MyNfts princessNft = MyNfts(nftAddress);
        princessNft.mint(_toAddress);
    }
    ...
    function canMint(uint256 _optionId) override public view returns (bool) {
        ...
        MyNfts princessNft = MyNfts(nftAddress);
        uint256 princessSupply = princessNft.totalSupply2();
        if (_optionId == 0) {
        uint256 numItemsAllocated = 1;
        return princessSupply < (Princess_SUPPLY - numItemsAllocated);
        }
        return false;
    }
