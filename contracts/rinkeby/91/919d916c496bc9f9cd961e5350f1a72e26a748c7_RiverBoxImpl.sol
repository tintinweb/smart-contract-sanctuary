    ...
    UUPSUpgradeable
{
    ...
    function buy(uint256 quality, address referrer) public payable override whenNotPaused notContract nonReentrant {
        ...
        boxsAward(promotion, _msgSender());
    ...
    function boxsAward(uint256 quality, address receiver) internal {
    ...
    function batchAirDrop(address[] memory receivers, uint256[] memory qualities) public override {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
        ...
        for (uint256 i = 0; i < qualities.length; ++i) {
            require(qualities[i] <= pawnsStock(), "blind box sold out");
            boxsAward(qualities[i], receivers[i]);
        }
    }
    ...
    function debugMint(uint16 locationId, address receiver) public {
        ...
        Product memory newItem = Product(locationId, 0, signature, new uint256[](0));
        uint256 newId = _awardItem(receiver);
        _tokenDetailsMapping[newId] = newItem;
        emit BoxAwarded(receiver, newId, block.timestamp);
    }
