...
contract RocketFuel is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
    // this purely for driving the UI
    struct MintContext {
    ...
    function setMaxPublicMint(uint256 _newMaxPublicMint) public onlyOwner {
        maxPublicMint = _newMaxPublicMint;
    }

    function setPricePerToken(uint256 _newPricePerToken) public onlyOwner {
    ...
    function setAllowList(address[] calldata _addresses, uint8 _numAllowedToMint) external onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            _allowList[_addresses[i]] = _numAllowedToMint;
        }
    }
    ...
    function mintAllowList(uint8 _numberOfTokens) external payable {
        uint256 tokenId = tokenIdCounter.current();
        require(isAllowListActive, "Allow list is not active");
        require(_numberOfTokens <= _allowList[msg.sender], "Exceeded max available to purchase");
        require(tokenId + _numberOfTokens <= MAX_SUPPLY, "Purchase would exceed max tokens");
        require(pricePerToken * _numberOfTokens <= msg.value, "Ether value sent is not correct");
    ...
    function mintTokens(uint256 _numberOfTokens) public payable {
        ...
        require(_numberOfTokens <= maxPublicMint, "Exceeded max token purchase");
        require(pricePerToken * _numberOfTokens <= msg.value, "Ether value sent is not correct");
