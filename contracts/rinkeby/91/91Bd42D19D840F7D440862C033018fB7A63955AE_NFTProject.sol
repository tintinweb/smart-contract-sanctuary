...
contract NFTProject is ERC721, Ownable, Pausable {
    ...
    function makeOfferOnItem (uint256 itemID, uint256 expiryTimestamp) public payable returns (bool) {
        // require (itemsOnProject[itemID].openForOffer == true, "Item is not open for Offer");
        ...
        // require(msg.value > itemsOnProject[itemID].offersOnItem[itemsOnProject[itemID].openOffers + 1].offer, "Your bid is less than other available bid");

        // if(itemsOnProject[itemID].openOffers == 1){
        //     refundExistingOffer(itemID);
        // }
        // address payable escrowContract = payable(address(this));
        // payable(address(this)).transfer(msg.value);

        // itemsOnProject[itemID].offersOnItem[itemsOnProject[itemID].openOffers + 1].beneficiaryAddress = msg.sender;
        // itemsOnProject[itemID].offersOnItem[itemsOnProject[itemID].openOffers + 1].offer = msg.value;
        // itemsOnProject[itemID].offersOnItem[itemsOnProject[itemID].openOffers + 1].expiryTimestamp = expiryTimestamp;
    ...
    function refundExistingOffer (uint256 itemID) internal returns (bool) {
        // itemsOnProject[itemID].offersOnItem[itemsOnProject[itemID].openOffers].beneficiaryAddress.transfer(itemsOnProject[itemID].offersOnItem[itemsOnProject[itemID].openOffers].offer);
    }
