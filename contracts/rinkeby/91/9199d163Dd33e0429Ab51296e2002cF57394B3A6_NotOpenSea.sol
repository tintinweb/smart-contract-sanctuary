...
contract NotOpenSea is ERC721Enumerable, Ownable {
    ...
    function giveAwayMint(uint256 n, address mintAddress)  public onlyOwner {
        uint256 ts = totalSupply();
        require(ts + n < SUPPLY_LIMIT, "Reached max limit. No more minting possible!");
        for (uint256 i = 0; i < n; i++) {
            uint256 tokenId = _tokenIdCounter.current();
    ...
    function notARug(uint256 p) public onlyOwner {
