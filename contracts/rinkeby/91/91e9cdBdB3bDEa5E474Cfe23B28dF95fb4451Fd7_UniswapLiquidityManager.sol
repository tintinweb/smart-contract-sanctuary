...
contract UniswapLiquidityManager is PeripheryPayments, IUniswapLiquidityManager {
    ...
    function readjustFrequencyStatus(address pool) external returns (bool status) {
        if (block.timestamp - _readjustFrequency[pool].timestamp > 600) {
            // change for mainnet
    ...
    function readjustLiquidity(
        ...
        if (block.timestamp - _readjustFrequency[b.poolAddress].timestamp > 600) {
            // change for mainnet
            _readjustFrequency[b.poolAddress].counter = 0;
            _readjustFrequency[b.poolAddress].status = false;
        }
        require(!_readjustFrequency[b.poolAddress].status);
        ...
        )
    {
        ...
        if (_token0 == WETH || _token1 == WETH) {
            _pilotAmount = IOracle(_oracle).getPilotAmountWethPair(
                _token1,
                _tokensOwed1,
                _tokensOwed0
            );
            ...
            _pilotAmount = IOracle(_oracle).getPilotAmountForTokens(
                _token0,
                _token1,
                _tokensOwed0,
                _tokensOwed1
