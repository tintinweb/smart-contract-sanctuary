...
contract faFactory is Ownable, ERC721, ERC721Enumerable {
    ...
    function _generateRandomLuck(uint _input) public view returns (uint32) {
        ...
        uint randLuck = uint(keccak256(abi.encodePacked(block.timestamp + 1 days, msg.sender, _randNonce))).mod(11);
    ...
    function battlePoints(uint16 _id) external view returns (uint32) {
        ...
        uint _luckResult = uint(keccak256(abi.encodePacked(block.timestamp + 1 days, msg.sender))).mod(faArray[_id].Luck);
