...
contract KingPadProjectCoin is
  OwnableUpgradeable,
  EmergencyWithdraw,
  ...
  EIP712Upgradeable
{
   ...
   */
  function __KingPadProjectCoin_init() public initializer {
    __Ownable_init();
  ...
  function buyTokens(
    ...
    // Auto whitelist
    if (whitelistSnapshot.wallet == address(0) && (!isPublicSale_ || block.timestamp <= publicTimeHolder)) {
      if (_useValidator) {
