...
import "../masterFox/FoxToken.sol";

interface IMigratorChef {
    // Perform LP token migration from legacy UniswapV2 to FoxSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // FoxSwap must mint EXACTLY the same amount of FoxSwap LP tokens or
...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accFoxPerShare; // Accumulated FOXs per share, times 1e12. See below.
    }

    // The FOX TOKEN!
    FoxToken public fox;

    // Block number when bonus FOX period ends.
    ...
    // Update the given pool's FOX allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    function pendingFox(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 foxReward = multiplier
                ...
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accFoxPerShare = accFoxPerShare.add(
                foxReward.mul(1e12).div(lpSupply)
            );
        }
        return
            user.amount.mul(accFoxPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // Safe fox transfer function, just in case if rounding error causes pool to not have enough FOXs.
...
// FoxToken with Governance.
contract FoxToken is ERC20("FoxToken", "FOX"), Ownable {
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "FOX::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "FOX::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "FOX::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "FOX::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying FOXs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "FOX::_writeCheckpoint: block number exceeds 32 bits"
        );
