...
contract SellingController is SellingControllerStorage {
    ...
    constructor(
        ...
        address maintainer
    ) SellingControllerStorage(defaultCardCID, phases, vault, maintainer) {}

    /// @notice The price of cards changes according to cards being sold.
    ...
    receive() external payable {
        require(!isSellingStopped(), "Card acquisition has been stopped!");
        if (whitelistEnabled) {
            require(getWhitelistEntry(msg.sender).approved, "Address is not on the whitelist!");
        }
        receiveAndMint();
    }

    /// @notice Transfer ownership to the gameControllers address and call the `startGame` method.
    function startGameFalse(address gameController) external onlyOwner {
        require(isSellingStopped(), "Game can only start once selling has been stopped!");
        require(
            IERC165(gameController).supportsInterface(type(IDnaProvider).interfaceId),
            "The game controller must implement `IDnaProvider`!"
        );
        nftToken.transferOwnership(gameController);
    ...
    /// @notice The price is determined by the current phase.
    /// @notice Can buy multiple NFTs at once.
    ...
    function receiveAndMint() internal {
        Phase storage currentPhase = _phases[phaseIndex];
        require(msg.value >= currentPhase.price, "Not enough funds!");

        uint256 excessWei = msg.value % currentPhase.price;
        require(excessWei == 0, "Leftover wei when buying cards, aborting!");

        uint256 countToMint = msg.value / currentPhase.price;
        uint256 currentTokenSupply = nftToken.totalSupply();
        uint256 newTokenSupply = currentTokenSupply + countToMint;
        require(
            newTokenSupply <= _phases[_phases.length - 1].threshold, // NEVER exceed the threshold for the last phase!
        ...
        if (newTokenSupply < currentPhase.threshold) return;

        // check if we sold last token
        if (newTokenSupply == _phases[_phases.length - 1].threshold) {
            stopSelling();
            return;
        }
...
abstract contract SellingControllerStorage is TokenRecoverable, ERC165 {
    ...
    // NOTE: The whitelist is "APPEND ONLY", once approved, an address cannot be removed.
    // This decision has been made to keep the gas usage for the contract lower.
    mapping(address => WhitelistEntry) internal _whitelistedAddresses;

    // --- Slot start ---
    address payable internal _vault; // 20 bytes -> 160bits
    uint32 public createdAt; // 32 bits
    ...
    constructor(
        ...
        nftToken = new NFT(defaultCardCID);
        _vault = vault;
    ...
    /// @param newWhitelistBatch Addresses to add.
    function addBatchToWhitelist(address[] calldata newWhitelistBatch, uint248 allowance) external onlyMaintainer {
        for (uint256 i = 0; i < newWhitelistBatch.length; i++) {
            _whitelistedAddresses[newWhitelistBatch[i]] = WhitelistEntry(true, allowance);
        }
    ...
    function getWhitelistEntry(address toCheck) public view returns (WhitelistEntry memory) {
        return _whitelistedAddresses[toCheck];
    }
    ...
    /// @param toDecrement the whitelisted address on which's allowance must be decremented.
    /// @param decrementBy The amount of cards that must be decremented from the whitelisted address.
    function whitelistDecreaseAllowance(address toDecrement, uint248 decrementBy) internal {
        // Make sure that a user is not buying more cards than initially permitted!
        require(_whitelistedAddresses[toDecrement].allowance >= decrementBy, "Cannot buy more cards than permitted!");
        _whitelistedAddresses[toDecrement].allowance -= decrementBy;
    }
}
...
contract NFT is ERC721URIStorage, ERC721Enumerable, Ownable {
    ...
    constructor(string memory defaultCardCID) ERC721("SatoshiQuest", "SQG") Ownable() {
    ...
    function mintBatch(uint256 amountToMint, address newTokenOwner) external onlyOwner {
    ...
    /// @notice Destroy a single token.
    /// @dev only owner can call this method.
    /// @param itemId The token ID to destroy.
    function burn(uint256 itemId) external onlyOwner {
        _burn(itemId);
    }
    ...
    function updateURI(uint256 itemId, string calldata cid) external onlyOwner {
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721, ERC721URIStorage) returns (string memory) {
        ...
        if (IERC165(this.owner()).supportsInterface(type(IDnaProvider).interfaceId)) {
            // Check if the card has been revealed (dna will not be 0)
            uint256 dna = IDnaProvider(this.owner()).getDna(tokenId);
            if (dna > 0) {
