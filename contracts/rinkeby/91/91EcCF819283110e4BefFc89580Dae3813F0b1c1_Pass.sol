...
contract Pass is ERC721Enumerable, Ownable {
    ...
    function reserveGiveaway(uint256 numPassGift, uint256 numPassCharity) public onlyOwner {
        uint currentSupply = totalSupply();
        require(numPassGift > 0 && numPassGift <= 20, "Max 20 to reserve at once");
        require(numPassCharity > 0 && numPassCharity <= 30, "Max 30 to reserve at once");
        require(saleLive == false, "Sale has already started");
        uint256 index;
        for (index = 0; index < numPassGift; index++) {
            _safeMint(0x21077b095626b71A2722071453F9E994468Db81C, currentSupply + index + 1);
        }
        for (index = 0; index < numPassCharity; index++) {
