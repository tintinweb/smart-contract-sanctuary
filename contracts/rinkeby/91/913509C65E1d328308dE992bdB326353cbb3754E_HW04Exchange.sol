...
uint256 public test2;
    ...
    function buySerhiiTokens() public payable {
        uint256  studentsAmount = 400; //just to make SLTK2 price about 10 usd
        ...
        uint256 amountSLTKToBuy = (msg.value * uint256(ethPrice)) / 10 ** decimals / studentsAmount;

        if(SerhiiToken.balanceOf(address(this)) < amountSLTKToBuy){
            (bool sent,) = msg.sender.call{value: msg.value}("Not enough SLTK tokens");
            return;
        }
    ...
    function calcForEth() public payable returns (int256) {
        ...
        uint256 amountSLTKToBuy = (uint256(msg.value) * uint256(ethPriceInUSD)) / 10000000;

        test = amountSLTKToBuy;
        test2 = ethPriceInUSD;

        return int256(amountSLTKToBuy);
    }
    ...
    function buySerhiiTokensForDAI(uint256 amountDaiToPay) public {
        ...
        uint256 decimals = uint256(AggregatorV3Interface(aggregator_DAI_USD).decimals());
        uint256 amountSLTKToBuy = (amountDaiToPay * uint256(daiPrice)) / 10 ** decimals / 10; //just to make SLTK2 price about 10 usd

        if(SerhiiToken.balanceOf(address(this)) < amountSLTKToBuy){
            (bool sent,) = msg.sender.call("Not enough SLTK tokens");
            return;
        }
...
//        require(allowance >= amountToBuy, "Check the token allowance please");
        ...
        DAIToken.transferFrom(msg.sender, address(this), amountDaiToPay);
        SerhiiToken.transfer(msg.sender, amountSLTKToBuy);
    }

    function calcSLTKAmountForDai(uint256 amountDaiToPay) public view returns (int256) {
        ( , int256 daiPrice, , , ) = AggregatorV3Interface(aggregator_DAI_USD).latestRoundData();
        uint256 decimals = uint256(AggregatorV3Interface(aggregator_DAI_USD).decimals());
        uint256 amountSLTKToBuy = (amountDaiToPay * uint256(daiPrice)) / 10 ** decimals / 10;
