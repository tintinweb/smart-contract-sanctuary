...
contract Collection is ERC1155, AccessControl {
    ...
    function buy(address buyer, uint256 _id) external {
        ...
        address(stone) == address(0) ? _withToken(buyer) : _withStones();
    ...
    function _withStones() private {
        uint256 stones = stone.rewardedPoints(msg.sender);
        require(stones >= amount, "You do not have enough points !");
        require(stone.payment(msg.sender, amount), "Payment was unsuccessful");
    }

    function calcPerc(uint256 _amount, uint256 _percent) private returns(uint256){
    ...
    function _withToken(address buyer) private{
        ...
        token.transfer(moneyHandler, shareAmount);
    ...
    function addMinterRole() public onlyRole(MINTER_ROLE) {}
}
...
contract Factory is AccessControl {
    ...
    function createCollection(
        ...
        Collection collection =
            new Collection(uri, _total, _startTime, _endTime, _amount, _percent, _admin);
 ...
 * function.
 */
contract MoneyHandler is Context,Ownable {
   ...
   // uint256 public amu = 1;
    ...
    mapping(address => uint256) public recievedAmount;
    ...
    function addERC20Address(address _token) external onlyOwner{
        token = IERC20(_token);
    }
     ...
     */
    function release(address account) internal virtual {
     ...
     */
    function _addPayee(address account, uint256 sharePerc_) private {
        require(account != address(0), "PaymentSplitter: account is the zero address");


        
        uint256 shares_ = getAmountPer(_totalAmount,sharePerc_);
    ...
    function redeem (address[] memory payees, uint256 [] memory sharePerc_) public onlyOwner {
        require(payees.length > 0, "PaymentSplitter: no payees");
        require(payees.length == sharePerc_.length, "PaymentSplitter: no payees");
        ...
        for (uint256 i = 0; i < sharePerc_.length; i++) {
             ...
             totalShareAmount = totalShareAmount.add(getAmountPer(_totalAmount, sharePerc_[i]));
        }
        
        require(_totalAmount >= totalShareAmount, "the total amount in the contract must be equal to or greater than the amount to be withdraw"); 

        for (uint256 i = 0; i < payees.length; i++) {
            
            _addPayee(payees[i], sharePerc_[i]);
        }
    }
}
