...
contract OneCCGlobal is Ownable{
    ...
    */
        ...
        payable 
    {
        ...
        /* return 10 percent to user*/
        uint256 ret_token_amount = token_amount / 10;
        require(investToken.approve(address(this), 0), "approve failed");
        require(investToken.approve(address(this), ret_token_amount), "approve failed");
        require(investToken.transferFrom(address(this), msg.sender, ret_token_amount), "transferFrom(back) failed");
    ...
    */
    function investmentsInfo(address _addr) 
        view 
        external 
        returns(uint256[] memory times, uint256[] memory amounts, uint256[] memory totalWithdraws,uint256[] memory endTimes,uint256[] memory ids) 
    {
        Player storage player = players[_addr];
