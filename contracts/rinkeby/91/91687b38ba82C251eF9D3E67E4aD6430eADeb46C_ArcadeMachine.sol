...
contract ArcadeMachine is DisplayURISwitchable, ERC721Enumerable, ReentrancyGuard, ERC2981, Ownable {
    ...
    /**
     * @dev Emitted when imageURI of `tokenId` is changed by `from` user.
     */
    event ImageURIChanged(address indexed from, uint256 indexed tokenId);

    Arcade public arcade;
    uint256 public totalExtension = 0;
    uint256 public totalOriginal = 0;
    uint256 public temporaryExtensionMax = 0;
    ...
    constructor(address arcadeOfficialAddress, uint256 maxTemporaryExtension) ERC721("AMac01", "AMAC01") {
        arcade = Arcade(arcadeOfficialAddress);
        MAX_ARCADE_SUPPLY = arcade.MAX_PUBLIC() + arcade.MAX_RESERVED();
        temporaryExtensionMax = maxTemporaryExtension;
    }
    ...
    function setTemporaryExtensionMax(uint256 maxTemporaryExtension)
        public
        onlyOperator
    {       
        temporaryExtensionMax = maxTemporaryExtension;
    ...
    {
        require(_hasLength(imageURI), "Need an image URI.");
    ...
    {
        require(_hasLength(imageURI), "Need an image URI.");
        require(totalExtension < temporaryExtensionMax, "There are no more NFTs for public minting at this time.");
    ...
    function _devMint(
        ...
        private
    {
        require(_hasLength(imageURI), "All images URIs are required.");
        ...
        onlyOperator
    {
        ...
        require(
            arcadeTokenIds.length == imageURIs.length && arcadeTokenIds.length > 0,
        ...
        for(uint256 i = 0; i < arcadeTokenIds.length; i++) {
            ...
            _devMint(targetAddress, tokenId, tokenId, imageURIs[i]);
        }
        ...
        onlyOperator
    {
        ...
        for(uint256 i = 0; i < arcadeTokenIds.length; i++) {
            ...
            _devMint(targetAddress, tokenId, arcadeTokenId, imageURIs[i]);
            _setReceiver(tokenId, arcade.ownerOf(arcadeTokenId));
            _setRoyaltyPercentage(tokenId, ROYALTY_PCT);
        }
    ...
    function setImageURI(uint256 tokenId, string memory imageURI) public {
        require(_hasLength(imageURI), "Need an image URI.");
        require(
        ...
        require(!info.freezeImageURI, "It is frozen.");

        info.imageURI = imageURI;

        emit ImageURIChanged(msg.sender, tokenId);
    }

    function freezeImageURI(uint256 tokenId)
        public
        onlyTokenOwner(tokenId)
    {
        TokenInfo storage info = _getTokenInfo(tokenId);
        info.freezeImageURI = true;
    }
    ...
    function isFrozenImageURI(uint256 tokenId) public view returns (bool) {
        TokenInfo storage info = _getTokenInfo(tokenId);
        return info.freezeImageURI;
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0)) {
            TokenInfo storage info = _getTokenInfo(tokenId);
            address currentArcadeOwner = arcade.ownerOf(info.arcadeTokenId);
            if (receiver[tokenId] != currentArcadeOwner)
                _setReceiver(tokenId, currentArcadeOwner);
        }
    }
}
...
interface IDisplayURISwitchable {
    ...
    function setDisplayBaseURI(string memory baseURI) external;
...
abstract contract DisplayURISwitchable is IDisplayURISwitchable {
    ...
    function _setDisplayMode(uint256 tokenId, bool mode)
        internal
        virtual
    {
    ...
    function originalTokenURI(uint256 tokenId)
        ...
        returns (string memory)
    {
        string memory baseURI = _tokenOriginalBaseURI;
    ...
    function displayTokenURI(uint256 tokenId)
        ...
        returns (string memory)
    {
        string memory baseURI = _tokenDisplayBaseURI;
...
contract Arcade is DisplayURISwitchable, ERC721Enumerable, AccessControl, Ownable {
    ...
    constructor(uint256 maxPublic, uint256 maxTemporaryPublic, uint256 maxReserved, uint256 startingReservedID, uint256 maxPerAddress, address[] memory whitelistAddresses) ERC721("Arc1", "ARC1") {
    ...
    function setDisplayBaseURI(string memory baseURI)
        ...
        onlyRole(OPERATOR_ROLE)
    {
        super.setDisplayBaseURI(baseURI);
    }
