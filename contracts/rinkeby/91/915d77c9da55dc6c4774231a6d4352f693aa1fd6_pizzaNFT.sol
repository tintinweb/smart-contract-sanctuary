...
interface IERC165 {
    ...
    function bakePizzaAndMint(string memory metadata, uint256 base, uint256 sauce, uint256 cheese, uint256[] memory meats, uint256[] memory toppings ) public {
        ...
        require(cheeseType == 3 || cheese == 0, "Please select valid cheese type");
        ...
        for(uint256 x = 0; x < meats.length; x++) {
            ...
            require(meatType == 4 || meats[x] == 0, "Please select valid meat type");
            require(getOnwer(meats[x]) == msg.sender || meats[x] == 0, "You are not owner of meet you selected");
        }
        uint256 toppingType = 0;
        for(uint256 x = 0; x < toppings.length; x++) {
            toppingType = mintIngredientTypes[toppings[x]];
            require(toppingType == 5 || toppings[x] == 0, "Please select valid topping type");
        ...
        UserIngredients memory userIngredientDetails;
        if(base > 0) {
            userIngredientDetails = userIngredientsList[base];
            userIngredientDetails.isUsed = true;
            userIngredientsList[base] = userIngredientDetails;
        }
        if(sauce > 0) {
            userIngredientDetails = userIngredientsList[sauce];
            userIngredientDetails.isUsed = true;
            userIngredientsList[sauce] = userIngredientDetails;
        }
        if(cheese > 0) {
            userIngredientDetails = userIngredientsList[cheese];
            userIngredientDetails.isUsed = true;
            userIngredientsList[cheese] = userIngredientDetails;
        }
        for(uint256 x = 0; x < meats.length; x++) {
            if(meats[x] > 0) {
                userIngredientDetails = userIngredientsList[meats[x]];
                userIngredientDetails.isUsed = true;
                userIngredientsList[meats[x]] = userIngredientDetails;
            }
        }
        for(uint256 x = 0; x < toppings.length; x++) {
            if(toppings[x] > 0) {
                userIngredientDetails = userIngredientsList[toppings[x]];
                userIngredientDetails.isUsed = true;
                userIngredientsList[toppings[x]] = userIngredientDetails;
            }
        }
        emit mintPizza(_nftId);
        // emit mintPizza(msg.sender, _nftId, metadata, base, sauce, cheese, meats, toppings);
    }
    ...
    //     require(baseType == 1 || base == 0, "Invalid base type");
    //     uint256 sauceType = ingredientsList[sauce].ingType;
    //     require(sauceType == 2 || sauce == 0, "Invalid sauce type");
    //     uint256 cheeseType = ingredientsList[cheese].ingType;
    //     require(cheeseType == 3 || cheese == 0, "Invalid cheese type");
    //     uint256 meatType = 0;
    //     for(uint256 x = 0; x < meats.length; x++) {
    //         meatType = ingredientsList[meats[x]].ingType;
    //         require(meatType == 4 || meats[x] == 0, "Invalid meat type");
    //     }
    //     uint256 toppingType = 0;
    //     for(uint256 x = 0; x < toppings.length; x++) {
    //         toppingType = ingredientsList[toppings[x]].ingType;
    //         require(toppingType == 5 || toppings[x] == 0, "Invalid topping type");
    //     }
    //     uint256[] memory tops;
    //     uint256[] memory mets;
    ...
    //         pizzasResponse.meets = metasIngs;
    //     }
