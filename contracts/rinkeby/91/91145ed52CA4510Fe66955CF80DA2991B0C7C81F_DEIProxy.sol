...
contract DEIProxy is Ownable {
	...
	uint public collateral_ratio_error_rate = 999000;
	...
	function Nativecoin2DEI(
		...
		global_collateral_ratio = global_collateral_ratio * collateral_ratio_error_rate / 1e6;
		...
		IERC20(deus_address).transfer(msg.sender, IERC20(deus_address).balanceOf(address(this)));
	...
	*/
	function collateral2DEI(
		...
		if(global_collateral_ratio == 1000000) {
			...
			require(collateral_needed_amount + collateral_for_deus_amount < proxy_input.max_amount_in, "PROXY: INSUFFICIENT_INPUT_AMOUNT");
	...
	function getAmountsForMint(
		...
		amount_out = amount_out * 1e6 / (1e6 - minting_fee);
		...
		collateral_for_deus_amount = IUniswapV2Router02(uniswap_router).getAmountsIn(deus_amount, collateral2deus_path)[0];
