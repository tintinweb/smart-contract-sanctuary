...
pragma experimental ABIEncoderV2;   // :notlikethis:
...
contract TWENTYTENNFTS is ERC721, Ownable {
    ...
    function bigArrayOfImages() public view returns (string[] memory) {
        string[] memory images = new string[](MAX_NFT);
        for(uint i=0; i < MAX_NFT; i++) {
            string memory x = tokenURI(i);
            images[i] = string(abi.encodePacked(x));
        }
        return images;
    }
        ...
        returns (string memory)
    {
        if (mintedTokens[_tokenId] == false) {
            ...
            return string(abi.encodePacked(base, "eyo.png"));
        }
    ...
    function mintNFT(uint256 numberOfTokens, uint256[] memory tokenIds)
        public
        payable
    {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            require(mintedTokens[tokenIds[i]] == false, "lolseriously?");
