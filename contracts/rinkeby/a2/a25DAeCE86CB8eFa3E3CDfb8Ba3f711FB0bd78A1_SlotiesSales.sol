...
contract SlotiesSales is Ownable {
     ...
     */
    uint256 public constant maxSloties = 20; // 10000
   ...
   bool internal claimed = false;
    ...
    /**
        Pre sale
     */
    uint256 public preSaleNewlyMintedSloties = 0; //newlyMintedURSWithPass
    uint256 public constant maxPreSaleSupply = 10; // 2500
    uint256 public constant preSaleMaxSloties = 3; //maxURSPerPass
    uint256 public constant preSalePrice = 0.008 ether; // 0.08 ether
    mapping(address => uint256) public mintedSlotiesOf; //mintedURSOf

    mapping(address => uint256) public whiteListAddressToNonce;
    ...
    /**
        Ticket
     */
    uint256 public constant ticketPrice = 0.016 ether; // 0.16 ether
    ...
    function preSaleMint(uint256 _amount, uint256 nonce, bytes memory signature) external payable whenPreSaleStarted {
        require(preSaleNewlyMintedSloties < maxPreSaleSupply, "All presale NFTs have been minted");
        uint256 mintedSloties = mintedSlotiesOf[msg.sender];
        require(_amount.add(mintedSloties) <= preSaleMaxSloties, "mint amount exceeds maximum");
        require(_amount > 0, "Need to mint more than 0");        

        // check valid signature
        require(whiteListAddressToNonce[msg.sender] == nonce, "INCORRECT NONCE");
        ...
        whiteListAddressToNonce[msg.sender] = whiteListAddressToNonce[msg.sender].add(1);
        ...
        mintedSlotiesOf[msg.sender] = mintedSloties + _amount;
        preSaleNewlyMintedSloties += _amount;

        // Refund changes
        uint256 changes = msg.value - totalPrice;
        emit PreSaleMint(msg.sender, _amount, changes);
    ...
    function runRaffle(uint256 _raffleNumber) external onlyOwner {
        ...
        uint256 remainingSloties = maxSloties - preSaleNewlyMintedSloties;
    ...
    function withdraw(address payable _to) external onlyOwner {
        ...
        require(
             maxSloties <= totalTickets + preSaleNewlyMintedSloties,
            "Not enough ethers are collected"
        );

        uint256 withdrawalAmount = ticketPrice * maxSloties;
