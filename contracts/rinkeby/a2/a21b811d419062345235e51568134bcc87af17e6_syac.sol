...
contract syac is ERC721, Ownable, Pausable {
    ...
    uint256 constant public LEGENDARY_SUPPLY = 12;
    mapping(address=>uint) public totalWhiteListedMinted;
        ...
        payable
    {
        ...
        require(totalWhiteListedMinted[msg.sender]<=3,"Max NFT limit reached");
        require(isWhitelisted[msg.sender],"Address isn't Whitelisted");
        for (uint256 i = 1; i <= _count; i++) {
            _safeMint(msg.sender, totalSupply + i);
        }
        totalSupply += _count;
        totalWhiteListedMinted[msg.sender]+=_count;
    }
