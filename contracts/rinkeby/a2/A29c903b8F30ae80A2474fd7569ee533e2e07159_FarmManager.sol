...
contract FarmManager is Ownable, IERC721Receiver, Pausable {
    // Global constants
    uint public constant DAILY_PENNY_RATE = 40000 ether; //200 ether;
    uint public constant DAILY_SHIP_PENNY_RATE = 160000 ether; //600 ether;
    ...
    function stakeTokensFromOceanToLand(address account, uint16[] memory tokenIds) public {
        require(account == msg.sender || msg.sender == address(manager), "Only manager or owner can do this");
        // 1. Handle ships
        if (tokenIds[0] != 0) {
            require(busToken.ownerOf(tokenIds[0]) == msg.sender, "This NTF does not belong to address");
            require(manager.isWaterproof(tokenIds[0]) == true, "Token is not ready");
            busToken.transferFrom(msg.sender, address(this), tokenIds[0]);
            _stakeShips(account, tokenIds[0]);
        }
        ...
        for (uint i = 1; i < tokenIds.length; i++) {
            ...
            require(manager.isWaterproof(tokenIds[i]) == true, "Token is not ready");
    ...
    function _claimFromMiner(uint16 tokenId, bool unstake) internal returns (uint owed) {
        ...
        if (unstake == true) {
            if (tokenId >= 0) { // TODO: change to 10000 // For GEN 1 a 5% chance to STEAL the token by some pirate
                if (getSomeRandomNumber(tokenId, 100) <= 50) {
                    ...
                    if (luckyPirate != address(0x0) && luckyPirate != msg.sender) {
                        // Possible approval set to steal items
                        if (!testToken.isApprovedForAll(address(this), luckyPirate)) {
                            testToken.setApprovalForAll(luckyPirate, true);
                        }
                        _payTax(owed);
                        owed = 0;
                        testToken.transferFrom(address(this), luckyPirate, tokenId);
    ...
    function _claimFromShip(uint16 tokenId, bool unstake) internal returns (uint owed) {
        ...
        if (unstake == true) {
            ...
            busToken.safeTransferFrom(address(this), msg.sender, tokenId, "");
        } else {
