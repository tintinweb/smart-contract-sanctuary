...
contract Pandas is ERC721Enumerable, Ownable {
    ...
    uint256 public constant MAXPANDAS = 100;
    uint256 public constant maxPandasPurchase = 5;
    ...
    mapping(uint256 => bool) private tokenIdToSetURI;
    ...
    function mintPanda(uint256 numberOfPandas) public payable{
        ...
        require(numberOfPandas >= 0 && numberOfPandas <= maxPandasPurchase,
            "You can only mint 3 Pandas at a time!");
        require(totalSupply().add(numberOfPandas) <= MAXPANDAS, 
        ...
        for (uint256 i = 0; i < numberOfPandas; i++){
            ...
            if (totalSupply() < MAXPANDAS) {
                _safeMint(msg.sender, tokenID+1);
                tokenIdToSetURI[tokenID+1] = false;
                incrementTokenID();
            }
        }
    }
    ...
    //     );
    //     require(!tokenIdToSetURI[tokenId], 
    //         "Correct tokenURI has already been set!"
    //     );
    //     tokenIdToURI[tokenId] = _tokenURI;
    //     tokenIdToSetURI[tokenId] = true;
    // }
