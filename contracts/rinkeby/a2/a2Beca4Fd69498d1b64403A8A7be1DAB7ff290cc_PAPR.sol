...
contract PAPR is ERC20Burnable, DoubleOperator {
    ...
    function mint(address recipient_, uint256 amount_) public onlySecondaryOperator returns (bool) {
        ...
        if (updateDFIMasterChef) dfiMasterChefContract.updatePaprAmount(recipient_); // verify that the user still holds enough papr to stay in DFIANCE master chef pools
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        if (updateDFIMasterChef) {
            ...
            dfiMasterChefContract.updatePaprAmount(sender);
            dfiMasterChefContract.updatePaprAmount(recipient);
        }
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        if (updateDFIMasterChef) {
            // verify that the user still holds enough papr to stay in DFIANCE master chef pools
            dfiMasterChefContract.updatePaprAmount(_msgSender());
            dfiMasterChefContract.updatePaprAmount(recipient);
        }
    ...
    function burn(uint256 amount) public virtual override {
        ...
        if (updateDFIMasterChef) dfiMasterChefContract.updatePaprAmount(_msgSender()); // verify that the user still holds enough papr to stay in DFIANCE master chef pools
    }
...
interface IDFIMasterChef {
    function updatePaprAmount(address _of) external;
}
