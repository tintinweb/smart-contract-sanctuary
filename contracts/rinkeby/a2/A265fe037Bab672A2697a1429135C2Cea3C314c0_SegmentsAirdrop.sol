...
contract SegmentsAirdrop is Ownable {
    ...
    // bytes32 private constant _PERMIT_TYPEHASH = keccak256('claim(uint256 nonce,address owner,uint256[] calldata tokens,uint256 pieceAmount');
    bytes32 private constant _PERMIT_TYPEHASH = 0x1d772b20c39fda800de6c9898c1fbce1aae50e9cfb3cc21047235eb39431eaed;
    ...
    constructor (address _millionPiecesSigner, IPiece _pieceToken, IMillionPieces _pieceNft) public {
        ...
        _DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256(bytes('SegmentsAirdrop')),
                keccak256(bytes('1')),
                chainId,
                address(this)
            )
    ...
    function claim(
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                '\x19\x01',
                _DOMAIN_SEPARATOR,
                keccak256(abi.encode(_PERMIT_TYPEHASH, nonce, owner, tokens, pieceAmount))
            )
        );

        address recoveredAddress = ecrecover(digest, v, r, s);

        require(recoveredAddress != address(0) && recoveredAddress == millionPiecesSigner, 'claim: Invalid signature!');
