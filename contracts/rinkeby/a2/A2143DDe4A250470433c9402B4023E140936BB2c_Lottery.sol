...
contract Lottery is VRFConsumerBase, Ownable {
    ...
    uint256 public entryFeeUsd = 50;
    uint256 entryFeeUsdDecimals = 0;
    uint256 weiConversionFactor = 10 ** 18;
    ...
    function getEthPrice()
            public
            view
            returns(uint priceUsd, uint decimals) {
        (,int256 price,,,) = priceFeed.latestRoundData();
        uint _decimals = priceFeed.decimals();
        return (uint(price), _decimals);
    }

    function getCurrentWeiPerUsd()
            public
            view
            returns(uint weiPerUsd) {
    ...
    function convertUsdToWei(uint usd, uint decimals)
            public
            view
            returns(uint gwei) {
    ...
    function getEntranceFee()
            ...
            view
            returns(uint256) {
        return convertUsdToWei(entryFeeUsd, entryFeeUsdDecimals);
    }

    function startLottery() public onlyOwner {
        require(
            lottery_state == LOTTERY_STATE.CLOSED,
            "Can't start a new lottery from this state"
        );
        lottery_state = LOTTERY_STATE.OPEN;
    }

    function endLottery() public onlyOwner {
//        // pseudorandom - don't use this for a production system that
//        // requires real randomness!
//        uint256(
//            keccak256(
//                abi.encodePacked(
//                    nonce,              // predictable
...
//        ) % players.length;
        ...
        // sends request to chainlink node for random value,
        // and later the chainlink node calls back to `fulfillRandomness`
        bytes32 requestId = requestRandomness(keyhash, linkFee);
        emit RequestedRandomness(requestId);
    }

    // abstract vrfCoordinator callback that gets called when requestRandomness
    // gets fulfilled by chainlink node
    function fulfillRandomness(bytes32 _requestId, uint256 _randomness)
            internal
            override {
        require(
            lottery_state == LOTTERY_STATE.CALCULATING_WINNER,
            "Cannot fulfillRandomness, invalid lottery state."
        );
        require(
            _randomness > 0,
            "Randomness not acquired yet, try again later"
        );
