...
contract AdvancedCollectible is ERC721, VRFConsumerBase {
    ...
    enum DungeonType{Dungeon}

    mapping(bytes32 => address) public requestIdToSender;
    mapping(bytes32 => string) public requestIdToTokenURI;
    mapping(uint256 => DungeonType) public tokenIdToDungeonType;
    mapping(bytes32 => uint256) public requestIDToTokenId;
    event requestedCollectible(bytes32 indexed requestId);

    constructor(address _VRFCoordinator, address _LinkToken, bytes32 _keyhash) public
    VRFConsumerBase(_VRFCoordinator, _LinkToken)
    ERC721("Dungeons", "DUNGEON")
    {
        keyHash = _keyhash;
    ...
    function fulfillRandomness(bytes32 requestid, uint256 randomNumber) internal override{
        address dungeonOwner = requestIdToSender[requestid];
        string memory tokenURI = requestIdToTokenURI[requestid];    
        uint256 newItemId = tokenCounter;
        _safeMint(dungeonOwner, newItemId);
        _setTokenURI(newItemId, tokenURI);
        DungeonType dungeontype = DungeonType(randomNumber % 1);
        tokenIdToDungeonType[newItemId] = dungeontype;
