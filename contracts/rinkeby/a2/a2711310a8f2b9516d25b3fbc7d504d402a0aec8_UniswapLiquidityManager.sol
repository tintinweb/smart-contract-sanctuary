...
contract UniswapLiquidityManager is PeripheryPayments, IUniswapLiquidityManager {
    ...
    constructor(UnipilotProtocolDetails memory params) {
    ...
    /// @param premium_ Additional premium of a user as an incentive for optimization of vaults.
    function setPoolIncentives(address pool, uint256 premium_) external onlyGovernance {
        LiquidityPosition storage lp = liquidityPositions[pool];
        lp.feesInPilot = !lp.feesInPilot;
        lp.rebasePremium = premium_;
    }
    ...
    function uniswapV3MintCallback(
        ...
        require(sender == currentPool);
        (address token0, address token1, , , , , , ) = getPoolDetails(sender);
        if (amount0Owed > 0) pay(token0, address(this), sender, amount0Owed);
        if (amount1Owed > 0) pay(token1, address(this), sender, amount1Owed);
    }
    ...
    function uniswapV3SwapCallback(
        ...
        require(sender == currentPool);
        (address token0, address token1, , , , , , ) = getPoolDetails(sender);
        if (amount0Delta > 0) pay(token0, address(this), sender, uint256(amount0Delta));
        if (amount1Delta > 0) pay(token1, address(this), sender, uint256(amount1Delta));
    }
    ...
    function shouldReadjust(
        ...
        int24 baseTickLower,
        int24 baseTickUpper
    ) private view returns (bool readjust) {
    ...
    function readjustLiquidity(
        ...
        (uint256 amount0, uint256 amount1, ) = updatePositionTotalAmounts(b.poolAddress);

        if (
            IOracle(unipilotProtocolDetails.oracle).checkPoolValidation(
                token0,
                token1,
                amount0,
                amount1
            ) && position.rebasePremium > 0
        ) {
            ...
            _mintPilot(msg.sender, b.pilotAmount.add(position.rebasePremium));
        }
    ...
    function _getWethPairsAndLiquidity(address token)
        private
        view
        returns (address pool)
    {
        return
            IOracle(unipilotProtocolDetails.oracle).getTokenWethPairWithMaxLiquidity(
                token
            );
    }
    ...
    function _mintPilotForUser(address _recipient, uint256 _pilotAmount) private {
        if (_pilotAmount > 0) {
            _mintPilot(_recipient, _pilotAmount);

            if (unipilotProtocolDetails.sfStatus)
                _mintPilot(
                    unipilotProtocolDetails.sfRecipient,
                    FullMath.mulDiv(
                        _pilotAmount,
                        unipilotProtocolDetails.sfPilotPercentage,
                        100
                    )
                );
        }
    ...
    function _distributeFeesInPilot(
        ...
        if (_token0 == WETH || _token1 == WETH) {
            uint256 _pilotAmountInitial = IOracle(unipilotProtocolDetails.oracle)
                .getPilotAmountWethPair(_token1, _tokensOwed1, _tokensOwed0);
            ...
            _mintPilotForUser(_recipient, _pilotAmount);
            ...
            uint256 _pilotAmountInitial = IOracle(unipilotProtocolDetails.oracle)
                .getPilotAmountForTokens(_token0, _token1, _tokensOwed0, _tokensOwed1);
    ...
    function _distributeFeesInTokens(
        ...
        if (unipilotProtocolDetails.sfStatus) {
            if (_tokensOwed0 > 0 || _tokensOwed1 > 0) {
                uint256 pilotAmount = IOracle(unipilotProtocolDetails.oracle)
                    .pilotAmountForFees(
                        _token0,
                        _token1,
                        _tokensOwed0,
                ...
                _mintPilot(unipilotProtocolDetails.sfRecipient, pilotAmount);
            }
        }
    }
        ...
        )
    {
        ...
        if (a.token0 == WETH || a.token1 == WETH) {
            ...
            address ethTokenPair = _getWethPairsAndLiquidity(tokenAlt);

            if (position.feesInPilot && params.pilotToken && ethTokenPair != address(0)) {
            ...
            address ethToken0Pair = _getWethPairsAndLiquidity(a.token0);
            address ethToken1Pair = _getWethPairsAndLiquidity(a.token1);
...
interface IOracle {
    event GovernanceUpdated(address oldGovernance, address newGovernance);
    event ULMStateUpdated(address oldUlmState, address newUlmState);
    ...
    function getTokenStablePairWithMaxLiquidity(address token)
        external
        view
        returns (address);
