...
function getArtist(uint256 id) view external returns(address);
...
contract OpenMarketplace is Ownable, Pausable, IMarketplace, ERC721Holder {
    ...
    function cancelOrder(address _nftAddress, uint256 _assetId) public whenNotPaused {
     ...
     function getc(uint256 idd) view public returns(address){
       return iemillionnft.getArtist(idd);        
     }
    /**
    ...
    function safePlaceBid(address _nftAddress, uint256 _assetId, uint256 _expiresAt) payable
        public whenNotPaused {
    ...
    function acceptBid(address _nftAddress, uint256 _assetId, uint256 _priceInWei) public whenNotPaused {
        ...
        if(order.seller == getc(_assetId) ){
          IWETH(WETH).transfer(order.seller,(bid.price).mul(500).div(1000));
         IWETH(WETH).transfer(address(this),(bid.price).mul(500).div(1000));
        }
        else{
         IWETH(WETH).transfer(order.seller,(bid.price).mul(9500).div(10000));
         IWETH(WETH).transfer(getc(_assetId),(bid.price).mul(375).div(10000));
         IWETH(WETH).transfer(address(this),(bid.price).mul(125).div(10000));
