...
contract GovWorldTierLevel is IGovWorldTierLevel, Ownable {
    ...
    @dev this function returns the bytes32 name of the maximum govholding tier level
     */
    function maxTierLevelbyNFTAddresses(address[] memory nftContracts, address _wallet) public view returns (TierData memory tierData) {
        
        uint256 maxFromNFT = tierLevels[tierLevelbyNFTContract[nftContracts[0]]].govHoldings;
        uint256 maxIndexNFT = 0;

        uint256 maxFromAllTierLevel = tierLevels[allTierLevelKeys[0]].govHoldings;
        uint256 maxIndexAllTierLevel = 0;

        for (uint256 i = 0; i < nftContracts.length; i++) {
            uint256 userNFTBalance = IERC721(nftContracts[i]).balanceOf(_wallet);
            require(userNFTBalance > 0, "GTL: NFT Balance Error");
        ...
        for (uint256 i = 0; i < allTierLevelKeys.length; i++) {
            if (tierLevels[allTierLevelKeys[i]].govHoldings > maxFromAllTierLevel) {
                maxIndexAllTierLevel = i;
                maxFromAllTierLevel = tierLevels[allTierLevelKeys[i]].govHoldings;
            }
        }

        if(tierLevels[tierLevelbyNFTContract[nftContracts[maxIndexNFT]]].govHoldings > tierLevels[allTierLevelKeys[maxIndexAllTierLevel]].govHoldings){
            return tierLevels[tierLevelbyNFTContract[nftContracts[maxIndexNFT]]];
        } else {
            revert("GTL: some or one NFT not assigned any tier level");
        }
    ...
    @param _nftContracts addresses to get highest tier level data by GOV TOKEN BALANCE || GOV NFT BALANCE
    @param _userWallet to check balance of GOV TOKEN & NFT contracts
     */
    //TODO tierData for holding NFT set by the super admin
    function getTierDatabyGovBalanceorNFTAddress(address[] memory _nftContracts, address _userWallet)
        public
        override
        view
        returns (TierData memory _tierData)
    {
        ...
        //TODO check on both Govtoken holding and NFT contract balance or not?
        uint256 userGovBalance = IERC20(govToken).balanceOf(_userWallet)+IERC20(govGovToken).balanceOf(_userWallet);
        TierData memory maxTierDataNFT = maxTierLevelbyNFTAddresses(_nftContracts, _userWallet);
        
        for (uint256 i = 1; i < allTierLevelKeys.length; i++) {
            if(userGovBalance > maxTierDataNFT.govHoldings){
                if ((userGovBalance >= 
                    tierLevels[allTierLevelKeys[i.sub(1)]].govHoldings) &&
                (userGovBalance < tierLevels[allTierLevelKeys[i]].govHoldings)) {
                return tierLevels[allTierLevelKeys[i.sub(1)]];
                ...
                (userGovBalance >=
                tierLevels[allTierLevelKeys[allTierLevelKeys.length.sub(1)]].govHoldings) {
                return tierLevels[allTierLevelKeys[allTierLevelKeys.length.sub(1)]];
                }
            }
            else
            {
            return maxTierDataNFT;
            }
        }
    }
