    ...
    BlockTimestamp
{
    ...
    address public override ulmState; // create set
    address public override uniStrategy; //create set

    address internal oracle; // create set
    ...
    uint256 internal constant PREMIUM = 2e18;
    ...
    constructor(
        ...
        address unipilotAddress_
    ) {
        (oracle, ulmState, indexFund, uniStrategy, unipilot) = (oracle_, ulmState_, indexFund_, uniStrategy_, unipilotAddress_);
    }

    // function updateCoreAddresses(address oracle_, address ulmState_, address indexFund_, address uniStrategy_) external onlyGovernance {
    ...
    //     (oracle, ulmState, indexFund, uniStrategy) = (oracle_, ulmState_, indexFund_, uniStrategy_);
    // }
    ...
    function checkUserAmounts(
        address token0,
        address token1,
        uint256 amount0,
        uint256 amount1,
        bytes calldata data
    ) external returns (uint256 amount0In, uint256 amount1In) {
        (uint24 fee, ) = abi.decode(data, (uint24, uint256));
        address pool = getPoolAddress(token0, token1, fee);
        // @dev these are the base ticks which will be set for base liquidity but we are fetching 
        // it before because we need to calculate the expected ratios in which user tokens will be added in base position
        (int24 baseTickLower, int24 baseTickUpper, , , , ) = IUniStrategy(uniStrategy).getTicks(pool);
        return
            IULMState(ulmState).checkLiquidityAmounts(
                // token0,
                // token1,
                // address(this),
                pool,
...
interface IUniswapLiquidityManager is IULMEvents {
    ...
    function ulmState() external view returns (address);

    function uniStrategy() external view returns (address);
...
interface IULMState {
    ...
    function checkLiquidityAmounts(
        address pool,
        uint256 amount0,
        uint256 amount1,
        int24 baseTickLower,
