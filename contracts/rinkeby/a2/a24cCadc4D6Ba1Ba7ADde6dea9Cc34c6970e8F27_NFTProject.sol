...
contract NFTProject is ERC721, Ownable, Pausable {
    ...
    constructor(string memory name, string memory symbol, address creatorWalletAddress) ERC721(name, symbol) {
    ...
    function claimItemAfterAuction(uint256 itemID) public returns (bool) {
        require (msg.sender == itemsOnProject[itemID].offersOnItem[itemsOnProject[itemID].openOffers.current()].beneficiaryAddress, "You are not the new owner");
