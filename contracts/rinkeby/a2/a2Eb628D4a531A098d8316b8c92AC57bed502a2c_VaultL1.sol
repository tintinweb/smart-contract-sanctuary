...
contract VaultL1 is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    validAmount(_amount)
    {
        sendAssetToBridge(arbitrumVaultAddress, _token, _amount, _destination);
    }
    ...
    validAmount(_amount)
    {
        sendAssetToBridge(polygonVaultAddress, _token, _amount, _destination);
    }

    /// @notice Internal function that execute the bridge deposit logic
    function sendAssetToBridge(address _bridgeAddress, address _token, uint256 _amount, address _destination) private {
        require(getTokenBalance(_token) >= _amount, "Not enough liquidity");
        SafeERC20.safeApprove(IERC20(_token), _bridgeAddress, _amount);
        IVaultBase(_bridgeAddress).depositERC20ForAddress(_amount, _token, _destination);
    ...
    /// @notice ERC677 token standard support
    /// @param _sender address of the user that send the tokens
    /// @param _amount amount of tokens user send
    /// @dev message comes from token contract so msg.sender is the address of the token
    function onTokenTransfer(address _sender, uint256 _amount, bytes memory)
    public
    whenNotPaused
    validAmount(_amount)
    onlySupportedToken(msg.sender)
    notOverMaxCap(msg.sender, _amount)
    returns (bool)
    {
        return _deposit(msg.sender, _amount, _sender);
    }
...
interface IVaultBase {
    ...
    function depositERC20ForAddress(uint256 amount, address tokenAddress, address destination) external;
}
