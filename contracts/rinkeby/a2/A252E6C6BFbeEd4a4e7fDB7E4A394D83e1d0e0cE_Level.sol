...
contract Level is ERC721, Ownable, ReentrancyGuard {
    using Strings for uint256;

    mapping(uint256 => uint256) private _tokenValues;
    
    uint256 public constant COLLECTION_SIZE = 9998;
    uint256 public constant MAX_MINT = 20;
    uint256 public constant MINT_COST = 0.01 * 10**18; //0.01 ETH
    uint256 internal constant STARTING_LVL = 1;

    uint256 internal mintIndex = 0;
    uint256 internal additionMintIndex = COLLECTION_SIZE+1;

    string private baseURI = 'ipfs://QmWLFHiLWVRLx8kwPMtxm1zQh8aTmpqBDVuLB4FiD4VYcD/'; //TODO: Set this to default

    constructor() ERC721("Level", "LVL"){}
    ...
    function levelUpAndBurn(uint256 tokenId1, uint256 tokenId2) external nonReentrant 
    {
        ...
        require(ownerOf(tokenId1) == msg.sender, "Must own the level you're trying to burn");
        require(ownerOf(tokenId2) == msg.sender, "Must own the level you're trying to burn");
