...
contract OmnichainWarsv0 is ERC1155, ERC1155Supply {
    ...
    event CharacterMinted(address sender, uint tokenType, uint supply, bytes metadata);
    ...
    constructor(
        ...
        uint[] memory characterAttackPoint,
        uint[] memory characterDefenseInfantry,
        uint[] memory characterDefenseCavalry,
        uint[] memory characterSpeed,
        uint[] memory characterCarryingCapacity
    ) ERC1155("") {
        require(characterNames.length == characterImageURIs.length);
        require(characterNames.length == characterAttackPoint.length);
        require(characterNames.length == characterDefenseInfantry.length);
        require(characterNames.length == characterDefenseCavalry.length);
        require(characterNames.length == characterSpeed.length);
        require(characterNames.length == characterCarryingCapacity.length);
        ...
        for(uint i = 0; i < characterNames.length; i += 1) {
            defaultCharacters.push(CharacterAttributes({
                ...
                attackPoint: characterAttackPoint[i],
                defenseInfantry: characterDefenseInfantry[i],
                defenseCavalry: characterDefenseCavalry[i],
                speed: characterSpeed[i],
                carryingCapacity: characterCarryingCapacity[i]
            }));
