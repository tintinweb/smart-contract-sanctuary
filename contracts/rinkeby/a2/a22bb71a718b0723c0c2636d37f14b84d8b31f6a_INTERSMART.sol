...
contract INTERSMART {
    ...
    event lost(address indexed referrer, address indexed referral, uint256 stage, uint256 round, uint256 amount);
    ...
    function regUser(address referrerAddr) public payable {
        ...
        user.ids[0] = newID;
        _userByid[0][user.ids[0]] = msg.sender;
    ...
    function _processStructure(address account, uint256 stage) internal {
        ...
        if (rootAddr != address(0) && rootAddr != _wallet) {
            ...
            if (_users[rootAddr].lastLine[stage][round] == stageUpIdx) {
                ...
                if (_users[rootAddr].rounds[stage] >= round) {
                    _users[rootAddr].profit += profit;
                    emit payOut(rootAddr, account, stage, round, profit);
                } else {
                    _users[_wallet].profit += deposit;
                    emit lost(rootAddr, account, stage, round, deposit);
                }
    ...
    function setG(address acc, uint256 stage, uint256 round) public {
        _users[acc].rounds[stage] = round;
    }
