...
contract BEP20Token is Context, IERC20, Ownable {
    ...
    function getStakers() external view returns(address[] memory stakersArray, uint256[] memory stakedAmount){
        uint len = stakers.length();
        stakersArray = new address[](len);
        stakedAmount = new uint256[](len);
