...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    function getUserToBadges(address user,uint256 badgeId) public view returns (bool) {
...
contract Badge is ERC1155, BaseContract {
    ...
    function createBadge(
        ...
        uint256 amounts = numberOfCopies;
        uint256  ids = numberOfCopies;
            ids = tokenCounter;
            _badgePrize[ids] = _priceAmount / numberOfCopies;
            _priceAmount = _badgePrize[ids] * numberOfCopies;
            badgeExpiry[ids] = block.timestamp + _matureTime;
            unchecked {
                tokenCounter += 1;
            }   
    ...
    function transferBadge(
        ...
        for (uint256 i = 0; i < badgeIds.length; i++) {
            require(
             ...
             require(balanceOf(msg.sender, badgeIds[i]) != 0,"All Token are already assigned");
       
            // require( 
            //     isBadgeAssigned[badgeIds[i]] == 0,
            //     "ERROR: badge already assigned"
            // );
            amounts[i] = 1;
          //  coolingTime[badgeIds[i]] = _claimedTime;
        
            tokenClaimedTime[msg.sender] = coolingTime[badgeIds[i]] + block.timestamp;
            isBadgeAssigned[badgeIds[i]] = 1;
            require(
                block.timestamp <= tokenClaimedTime[msg.sender],
                "Error : TimePeriod has expired"
            );
        }
    ...
    function resetBadgeExpiry(uint256 _reMaturedTime, uint256[] memory badgeIds)
        external
        payable
        onlySponsor
    {
        ...
        for (uint256 i = 0; i < badgeIds.length; i++) {
            badgeExpiry[badgeIds[i]] = _reMaturedTime;
            badgeExpiry[badgeIds[i]] += block.timestamp;
    ...
    function claimBadgePrize(uint256 badgeId) external onlyStudent {
        ...
        require(
            block.timestamp >= coolingTime[badgeId],
