...
contract Polkadog is Context, IERC20, IERC20Metadata, Ownable, Pausable {
            ...
            if(amount >= transactionLimit){
                ...
                if(_totalTransactions[from] > 0){
                    ...
                    for(uint256 i =_totalTransactions[from]; i>=0 ; i--){
                        ...
                        if((currentTime - _transactionTime[from][i]) <= _perDayTimeBound ){
    ...
    function addTx(address from, uint256 index) public {
        _transactionTime[from][index] = block.timestamp;
        _totalTransactions[from]++;
    }

    function deleteTx(address from, uint256 index) public {
        delete _transactionTime[from][index];
    }
