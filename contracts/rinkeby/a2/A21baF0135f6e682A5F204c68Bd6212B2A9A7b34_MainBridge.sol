    ...
    function unlockTokens (address _requester, uint _bridgedAmount, bytes32 _sideDepositHash) onlyGateway external {
        ...
        if (bridgeFee > 0) {
            if (autoLiquidateForGasEnabled) {
                ...
                uint256 autoLiqAmount = bridgeFee.sub(autoLiqFee);
                
                mainToken.transfer(address(this), autoLiqAmount);
                uint256 bridgeBalance = mainToken.balanceOf(address(this));
                
                if (bridgeBalance >= minAutoLiqAmount) {
                ...
                mainToken.transfer(bridgeFeeCollector, bridgeFee.sub(autoLiqAmount));
            } else {
