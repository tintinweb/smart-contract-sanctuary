...
contract Raffle is Ownable, IERC721Receiver, ERC165, ERC721Holder {
    ...
    function newRaffle(uint _endTimestamp, address contractAddress) external onlyOwner {
        raffleVersion++;
	...
	function draw() external {
		...
		tokenContract.safeTransferFrom(address(this), winner, 1);
    ...
    function recoverGas(uint32 _version, uint[] calldata _tokenIDs) external onlyOwner {
        ...
        for (uint i=0; i < _tokenIDs.length; i++) {
            bytes32 key = keccak256(abi.encodePacked(raffleVersion, _tokenIDs[i]));
