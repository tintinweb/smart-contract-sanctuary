...
contract ChainlinkHolders is ChainlinkClient, Ownable{
    ...
    constructor() {
    	...
    	oracle = 0x228641CA8c3D7d27D045Ee0fbc9C0D9a7Ae41679; // oracle address
    	jobId = "50ed676a95834aa1a251ac956c1d5445"; //job id api adapter
    	fee = 0; 
        //string memory url = "https://rinkeby.etherscan.io/token/generic-tokenholders2?a=0x06f7246f009F6f0372C481AD6cf60f3CDe31DCe2&s=0&p=GY5ISRPT71E9XJ3XIVVYFE8YV48CD1EAMG.etherscan.io/token/generic-tokenholders2?a=0x06f7246f009F6f0372C481AD6cf60f3CDe31DCe2&s=0&p=GY5ISRPT71E9XJ3XIVVYFE8YV48CD1EAMG";
    }
    
    function holdersCheckTime(uint256 minBalance, uint256 number) external onlyOwner returns (bytes32 requestId){
    	Chainlink.Request memory req = buildChainlinkRequest(jobId, address(this), this.updateHolders.selector);
    	req.addUint("minBalance", minBalance);
        req.addUint("number", number);
    	requestId = sendChainlinkRequestTo(oracle, req, fee);
    	return requestId;
    }

    function updateHolders(bytes32 _requestId1, address[] calldata _holders) external recordChainlinkFulfillment(_requestId1) {
        delete holders;
        require(_holders.length > 0, "No holders are recieved");
        for(uint i = 0; i < _holders.length; i++){
            holders.push(_holders[i]);
        }
    }

    function viewHolders() view external returns(address[] memory){
        return holders;
    }
