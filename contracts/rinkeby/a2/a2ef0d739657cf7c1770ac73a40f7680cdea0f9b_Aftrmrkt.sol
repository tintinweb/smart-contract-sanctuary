...
contract Aftrmrkt is ERC1155, Ownable, VRFConsumerBase {
    ...
    // Base pack Limit
    uint256 public BPFreshCardLimit = 449;
    uint256 public BPCommonCardLimit = 9379;

    // Pack Fees
    uint256 public PremiumPackFee = 0.5 ether;
    uint256 public FreshPackFee = 0.2 ether;
    uint256 public BasePackFee = 0.04 ether;
    ...
    constructor() ERC1155("ipfs://linkhere/") VRFConsumerBase(VRFCoordinator, LinkToken) {
	{
            elite.add(4);
            supplyOf[4] = 50;
		...
		address beneficiary = 0x760c5A41b67BE0b8E208Da61c9654d5aad1e92f2;
    ...
    function buyPresaleBasePack(uint256 count, bytes32[] calldata merkleProof) external payable {
	    bytes32 node = keccak256(abi.encodePacked(msg.sender));
        require(!paused, "the contract is paused");
        require(preSale, "Aftmrkt::preCheck: presale is not active");
        require(
        ...
        require(
            msg.value >= BasePackFee.mul(count),
            "Aftmrkt::buyPremiumPack: Insufficient amount."
        );
    ...
    function buyBasePack(uint256 count) external payable {
        require(!paused, "the contract is paused");
        require(mainSale, "Aftmrkt::preCheck: Sale is not active");
        require(
            BasePack >= count,
	...
	function mintBrainReservedNFT(address beneficiary) external onlyOwner {
	   require(!paused, "the contract is paused");
	...
	function mintBreedingNFT(address beneficiary1, address beneficiary2, address beneficiary3, address beneficiary4, address beneficiary5, address beneficiary6, address beneficiary7, address beneficiary8, address beneficiary9) external onlyOwner {
	   require(!paused, "the contract is paused");
	   require(!breedingMinted, "already Minted");
	  _mint(beneficiary1, 31, 1, "");
	  _mint(beneficiary2, 32, 1, "");
	  _mint(beneficiary3, 33, 1, "");
	  _mint(beneficiary4, 34, 1, "");
	  _mint(beneficiary5, 35, 1, "");
	  _mint(beneficiary6, 36, 1, "");
	  _mint(beneficiary7, 37, 1, "");
	  _mint(beneficiary8, 38, 1, "");
	  _mint(beneficiary9, 39, 1, "");
	}
}