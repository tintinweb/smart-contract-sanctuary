...
contract AdventureGold is Context, Ownable, ERC20 {
    ...
    */
    ...
    IERC721Enumerable[] contracts = [lootContract, cdbcontract, toadContract];
    ...
    function claimById(uint256 tokenId, uint256 contractID) external {
        ...
        require(
            _msgSender() == (contracts[contractID]).ownerOf(tokenId),
            "MUST_OWN_TOKEN_ID"
        );
    ...
    function claimAllForOwner(uint256 contractID) external {
        uint256 tokenBalanceOwner = (contracts[contractID]).balanceOf(
            _msgSender()
        );
        ...
        for (uint256 i = 0; i < tokenBalanceOwner; i++) {
            ...
            // the _claim() function
            _claim(
                (contracts[contractID]).tokenOfOwnerByIndex(_msgSender(), i),
                _msgSender()
            );
        }
    }
