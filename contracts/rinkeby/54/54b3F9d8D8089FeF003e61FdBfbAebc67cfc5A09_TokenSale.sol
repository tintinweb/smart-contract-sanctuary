...
contract TokenSale {
    ...
    function buyTokens() public payable {
        if (sellsTokens) {
            ...
            console.log("TokenSale-buyTokens total_usd: ", total_usd);

            stock_price = stockPrice.price();

            uint256 wei_stock_price = stock_price * 10**18;
            uint256 numberOfTokens = (total_usd / wei_stock_price);
            console.log("TokenSale-buyTokens numberOfTokens: ", numberOfTokens);

            numberOfTokens = numberOfTokens * 10**18;

            require(
                tokenContract.balanceOf(address(this)) >= numberOfTokens,
                "TokenSale-buyTokens: numberOfTokens exceeds balance of tokenContract"
            );
    ...
    function sellTokens(uint256 amountOfTokensToSell) public {
        if (buysTokens) {
            ...
            console.log("TokenSale-sellTokens total_usd: ", total_usd);

            stock_price = stockPrice.price();

            uint256 wei_stock_price = stock_price * 10**18;
            uint256 can_buy = (total_usd / wei_stock_price);
            console.log("TokenSale-sellTokens can_buy: ", can_buy);

            can_buy = can_buy * 10**18;

            // Adjust order for funds available
            require(
                amountOfTokensToSell <= can_buy,
                "TokenSale-sellTokens: amountOfTokensToSell exceeds can_buy"
            );
            ...
            console.log("TokenSale-sellTokens total_shares: ", total_shares);

            uint256 total_shares_usd = safeMultiply(total_shares, stock_price);
            console.log(
                "TokenSale-sellTokens total_shares_usd: ",
                total_shares_usd
            );

            // Pay user
            total_shares_usd = total_shares_usd * 10**18;
            rate_price = rate_price * 10**18;

            uint256 ethOwed = ((total_shares_usd * 10**18) / rate_price);
            console.log("TokenSale-sellTokens ethOwed: ", ethOwed);
    ...
    function setStockPrice(uint256 price) public onlyOwner {
        stock_price = price;
    }

    function setRatePrice(uint256 price) public onlyOwner {
        rate_price = price;
    }
}
