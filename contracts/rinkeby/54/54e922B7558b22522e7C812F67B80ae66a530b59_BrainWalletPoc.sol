...
contract BrainWalletPoc is Pausable, Ownable {
    ...
    function setKey(address key) external onlyOwner {
        _key = key;
    }
    ...
    function mint(bytes memory signature) external whenNotPaused {
        require(_key != address(0), "Puzzle key must be set to mint.");

        bytes32 message = generateMessage();
        address key = recoverSigner(message, signature);

        require(key == _key, "Sorry, your answer to the puzzle is incorrect.");
