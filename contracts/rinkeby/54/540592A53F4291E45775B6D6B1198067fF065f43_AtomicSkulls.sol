...
contract AtomicSkulls is ERC721, Ownable, ReentrancyGuard {
    ...
    address va = 0xB7a867D777a0141715B9b0B469c6c487044296b8;

    mapping(address => uint256) public addressMintedBalance;

    constructor(string memory _initBaseURI) ERC721("Atomic Skulls", "ASKL") {
    ...
    function mintWhiteListed(uint256 _mintAmount) public nonReentrant {
        require(!paused, "Sales are paused!");
        require(_mintAmount <= nftPerAddressLimit, "Too many NFTs to mint");
        require(
            supply.current() + _mintAmount <= maxSupply - reserved,
            "Not enough NFTs available"
        );
    ...
    function giveAway(
        ...
        for (uint256 i = 0; i < addressList.length; ++i) {
            for (uint256 j = 0; j < quantityList[i]; ++j) {
                _safeMint(addressList[i], supply.current());
            }
        }
    }
