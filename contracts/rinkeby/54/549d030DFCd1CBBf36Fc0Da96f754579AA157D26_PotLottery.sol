...
contract PotLottery {
    ...
    function setAccountToRecieveFees(address _accountToRecieveFee)
        public
        onlyAdmin
    {
    ...
    uint256[] entriesUsdValue;
    address[] lastPotEntriesAddress;
    uint256[] lastPotEntriesUsdValue;
    address private LAST_POT_WINNER;
    bool calculatingWinner = false;
    address accountToReceivePotFees;
    ...
    function calculateWinner() public {
        ...
        */
        ...
        if ((potStartTime + potDuration) <= block.timestamp) {
            ...
            lastPotEntriesAddress = entriesAddress;
            lastPotEntriesUsdValue = entriesUsdValue;

            uint256 winningPoint = randomNumber % totalPotUsdValue;

            for (
                uint256 index = 0;
                index < lastPotEntriesAddress.length;
                index++
            ) {
                winningPoint -= lastPotEntriesUsdValue[index];
                if (winningPoint <= 0) {
                    //That means that the winner has been found here
                    LAST_POT_WINNER = lastPotEntriesAddress[index];
                    break;
                }
            }
            ...
            for (
                uint256 index = 0;
                index < tokensInPotNames.length - 1;
                index++
            ) {
                if (
            ...
            uint256 amountToPayAsFees = ((tokenLatestPriceFeed[
                tokenWithThehighestUsdValue
            ] * tokenWhiteList[tokenWithThehighestUsdValue].tokenDecimal) /
                (percentageFee * totalPotUsdValue)) <=
                tokenTotalEntry[tokenWithThehighestUsdValue]
                ? ((tokenLatestPriceFeed[tokenWithThehighestUsdValue] *
                    tokenWhiteList[tokenWithThehighestUsdValue].tokenDecimal) /
                    (percentageFee * totalPotUsdValue))
                : tokenTotalEntry[tokenWithThehighestUsdValue];
            payAccount(
            ...
            for (uint256 index = 0; index < tokensInPotNames.length; index++) {
                if (
                    keccak256(bytes(tokensInPotNames[index])) !=
                    keccak256(bytes(tokenWithThehighestUsdValue))
                ) {
                    payAccount(
                    ...
                    if (tokenTotalEntry[tokenWithThehighestUsdValue] > 0) {
                        payAccount(
                            tokensInPotNames[index],
                            LAST_POT_WINNER,
                            tokenTotalEntry[tokenWithThehighestUsdValue]
    ...
    function startNewPot() internal {
        if (
            potStartTime + potDuration >= block.timestamp && calculatingWinner
        ) {
            ...
            delete lastPotEntriesAddress;
            delete lastPotEntriesUsdValue;
    ...
    function payAccount(
        ...
        if (keccak256(bytes(_tokenName)) == keccak256(bytes("PULSE"))) {
            payable(_accountToPay).transfer(_tokenValues);
        } else {
            IPRC20 token = IPRC20(tokenWhiteList[_tokenName].tokenAddress);
            require(
                token.transferFrom(msg.sender, address(this), _tokenValues),
