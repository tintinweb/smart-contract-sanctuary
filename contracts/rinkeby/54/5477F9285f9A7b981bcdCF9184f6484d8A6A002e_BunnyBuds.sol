...
contract BunnyBuds is ERC721Enumerable, Ownable, Accountable {
    ...
    uint256 private constant MAX_KOALA_SUPPLY = 2201;       // One too high -- 2000 total
    uint256 private constant MAX_WHITELIST_SUPPLY = 3701;   // One too high -- 1500 total
    uint256 private constant RESERVES = 201;                // One too high

    uint256 public koalaSaleTime = 1633542194;          // NEED SETTING
    uint256 public whitelistSaleTime = 1633542204;      // Tuesday, October 11th, 2021 at 6:00 PM EST https://www.epochconverter.com/
    uint256 public publicSaleTime = 1633628591;
    ...
    modifier koalaIsValid() {
        require(koalas.balanceOf(msg.sender) > 0, "Wallet does not contain a KIA.");
        require(koalaSaleTime < block.timestamp, "KIA presale not started.");
        _;
    }

    modifier whitelistIsValid() {
        require(addressWhitelisted[msg.sender], "Not whitelisted.");
        require(whitelistSaleTime < block.timestamp, "Whitelist presale not started.");
        _;
    }
    ...
    function setKoalaSaleTime(uint256 _time) public onlyOwner {
        koalaSaleTime = _time;
    }

    function setWhitelistSaleTime(uint256 _time) public onlyOwner {
        whitelistSaleTime = _time;
    }
...
abstract contract Accountable is Context {
    ...
    function getSplitBalance() external view returns (uint256) {
        return splitAmounts[msg.sender];
    }
    ...
    function withdrawSplit(uint256 amount) external hasSplits {
        require(amount > 0, "Withdrawals of 0 cannot take place.");
        require(
            splitAmounts[msg.sender] >= amount,
