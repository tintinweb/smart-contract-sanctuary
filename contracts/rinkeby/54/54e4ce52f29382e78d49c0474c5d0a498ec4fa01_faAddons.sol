...
contract faAddons is Ownable, ERC721, ERC721Enumerable {
    ...
    function newBattlePoints(uint16 _fromId, uint16 _toId) external view returns (uint32, uint32,uint32, uint32,uint32, uint32) {
        ...
        fromLife = IFA(_nftAddress).getLife(_fromId);
        fromArmour = IFA(_nftAddress).getArmour(_fromId);
        fromAttack = IFA(_nftAddress).getAttack(_fromId);
        fromDefence = IFA(_nftAddress).getDefence(_fromId);
        fromMagic = IFA(_nftAddress).getMagic(_fromId);
        fromLuck = IFA(_nftAddress).getLuck(_fromId);
       ...
       */
        ...
        */  
        ...
        return (fromLife, fromArmour, fromAttack, fromDefence, fromMagic, fromLuck);
