...
contract CompoundProtocol is IDefiProtocol, OwnableUpgradeable, AbstractDependant {
    ...
    function deposit(uint256 amount) external override onlyYieldGenerator {
        ...
        uint256 _currentApproval = stablecoin.allowance(address(this), address(cToken));
        uint256 _newApproval = _currentApproval.add(amount);
        _setOrIncreaseTetherAllowance(address(cToken), _newApproval);
