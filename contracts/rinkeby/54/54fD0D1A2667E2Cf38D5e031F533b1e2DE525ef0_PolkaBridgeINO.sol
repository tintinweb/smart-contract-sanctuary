...
contract PolkaBridgeINO is ERC1155, Ownable, ReentrancyGuard {
    ...
    mapping(address => User) private users; // userId -> User
   ...
   // Check if user IsWhitelist
        ...
        nonReentrant
    {
        ...
        if(!purchasecheck[packageId][msg.sender]){
            ...
            users[msg.sender].PurchasedPackageIds.push(packageId);
        }
    ...
    function getPurchasedPackageIds(address user_) public view
        returns ( uint[] memory )
    {        
        return users[user_].PurchasedPackageIds;
    }
