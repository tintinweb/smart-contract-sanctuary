    ...
    ILendingPool
{
        ...
        returns (uint256)
    {
        ...
        return
            dao.exchangRewardToken(
                ...
                user
                    .amount
                    .mul(supplyIndex.sub(user.supplyClaimIndex))
                    .div(10**pool.decimals)
                    .mul(REWARD_FACTOR)
                    .div(BASE_DENOMINATOR)
    ...
    function claimDToken(address _tokenAddress) external {
        ...
        uint256 rewardToken = user
            .amount
            .mul(pool.supplyIndex.sub(user.supplyClaimIndex))
            .div(10**pool.decimals)
            .mul(REWARD_FACTOR)
            .div(BASE_DENOMINATOR)
            .add(user.pendingDtokenDebt);
        ...
        user.supplyClaimIndex = pool.supplyIndex;
    }
        ...
        nonReentrant
    {
        ...
        if (userAsset.amount > 0) {
            ...
            // calculate pending Dtoken
            pending = userAsset
                .amount
                .mul(pool.supplyIndex.sub(userAsset.supplyClaimIndex))
                .div(10**pool.decimals)
                .mul(REWARD_FACTOR)
                .div(BASE_DENOMINATOR);
            userAsset.pendingDtokenDebt = userAsset.pendingDtokenDebt.add(
                pending
            );
        }
        if (_amount > 0) {
        ...
        userAsset.supplyClaimIndex = pool.supplyIndex;
