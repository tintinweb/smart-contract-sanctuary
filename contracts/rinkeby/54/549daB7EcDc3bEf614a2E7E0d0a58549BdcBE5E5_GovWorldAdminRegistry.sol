...
contract GovWorldAdminRegistry is GovWorldAdminBase {
        ...
        onlyAddGovAdminRole(msg.sender)
    {
        ...
        emit NewAdminApproved(_newAdmin, msg.sender, pendingAddedAdminRoles[_newAdmin]);
    ...
    {
        //the admin that is removing _admin must not already have approved.
        require(_notEdited(_admin, msg.sender),"GAR: Admin already give edit right to this admin. ");
        ...
        onlyEditGovAdminRole(msg.sender)
    {
        ...
        if (this.isEditedByAll(_admin)) {
            ...
            emit EditAdminApproved(_admin, msg.sender, pendingEditAdminRoles[_admin]);
        }
    }
...
contract GovWorldAdminBase is Ownable {
    ...
    event NewAdminApprovedForPending(address indexed _newAdmin, address indexed _approvedBy);
    event NewAdminApproved(address indexed _newADmin, address indexed _admin, AdminAccess _adminAccess);
    event AdminRemoved(address indexed _admin);
    event RemoveAdminApprovedForPending(address indexed _removedAdmin);
    event RemoveAdminApproved(address indexed _admin,address indexed _removedBy);
    event EditAdminApprovedForPending(address indexed _editAdmin, address indexed _approvedBy);
    event EditAdminApproved(address indexed _admin,address indexed _editedBy, AdminAccess _adminAccess);
    event AdminEdited(address indexed _admin);
        ...
        internal
    {
        ...
        emit NewAdminApprovedForPending(_newAdmin, msg.sender);
    }
    ...
    function _makePendingForRemove(address _admin) internal {
        ...
        emit RemoveAdminApprovedForPending(_admin);
    }
    ...
    function _makePendingForEdit(address _admin, AdminAccess memory _newAccess) internal {
        ...
        emit EditAdminApprovedForPending(_admin, msg.sender);
    }
