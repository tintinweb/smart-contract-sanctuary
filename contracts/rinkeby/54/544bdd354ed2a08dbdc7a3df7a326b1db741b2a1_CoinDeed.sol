...
contract CoinDeed is ICoinDeed, AccessControl {
    ...
    // Buyer buys in. *amount* is ignored if tokenA is ETH, instead relying on msg.value
    ...
    function sell() onlyInState(DeedState.OPEN) external override {
        ...
        coinDeedAddressesProvider.sell(pair, executionTime);
        ICoinDeedFactory(coinDeedAddressesProvider.coinDeedFactory()).emitStateChanged(state);
    }
    ...
    function executeRiskMitigation() external payable override onlyInState(DeedState.OPEN) {
        ...
        if(pair.tokenA == address(0x00)) {
            ...
            IERC20(pair.tokenA).transferFrom(msg.sender, address(this), buyAmount - discount);
        }
...
library CoinDeedAddressesProviderUtils {
    ...
    function readyCheck(
        ...
        if (totalStake >= (amounts[1] * stakingMultiplier / BASE_DENOMINATOR)) {
    ...
    function _buyWithLeverageAndDeposit(
        ...
        // Borrow the token A
        ILendingPool lendingPool = ILendingPool(coinDeedAddressesProvider.lendingPool());
        lendingPool.borrow(pair.tokenA, buyIn * (deedParameters.leverage - 1));
        // Try to sell through a wholesale first
        uint256 amountSent;
        (amountSent, amountReceived) = _wholesaleSwap(
            coinDeedAddressesProvider, 
            pair, 
            wholesaleId, 
    ...
    function _wholesaleSwap(
        ...
        if (wholesaleId != 0) {
            ...
            if (pair.tokenA == address(0x00)) {
                wholesaleFactory.executeWholesale{value : amount}(wholesaleId, amount);
            } else {
                IERC20(pair.tokenA).safeApprove(address(wholesaleFactory), amount);
                wholesaleFactory.executeWholesale(wholesaleId, amount);
            }
        }
        return (wholesale.receivedAmount, wholesale.soldAmount);
    }

    /** Swaps on uniswap.
      * TODO: Add front running protection by making sure that the amount received isn't less than
      * a certain percentage away from the feed registry price*/
    function _dexSwap(
        ICoinDeedAddressesProvider coinDeedAddressesProvider,
        address tokenIn,
        address tokenOut,
        ...
        if (amountIn > 0) {
            if (tokenIn == address(0x00)) {
                return _swapEthToToken(coinDeedAddressesProvider, amountIn, tokenOut);
            } else if (tokenOut == address(0x00)){
                return _swapTokenToEth(coinDeedAddressesProvider, amountIn, tokenIn);
            } else {
                return _swapTokenToToken(coinDeedAddressesProvider, tokenIn, amountIn, tokenOut);
            }
        }
        return 0;
    }
