...
/**
 * @notice Marketplace contract for Market Trading NFTs
 */
contract NFTMarketplace is Context, ReentrancyGuard {
    ...
    /// @notice MarketTrading NFT - the only NFT that can be offered in this contract
    ...
    constructor(
        ...
        require(address(_MarketTradingNFT) != address(0), "NFTMarketplace: Invalid NFT");
    ...
    /**
     @notice Creates a new offer for a given MarketTrading NFT
    ...
    function createOffer(
        ...
        require(MarketTradingNFT.exists(_tokenId), "NFTMarketplace.createOffer: TokenID does not exist");
        ...
        require(
            ...
            "MarketTradingNFTAuction.createOffer: Not owner and or contract not approved"
        );
        _createOffer(
            _tokenId,
            _primarySalePrice,
    ...
    function confirmOffer(uint256 _tokenId) external payable nonReentrant whenNotPaused {
        ...
        require(MarketTradingNFT.isApproved(_tokenId, address(this)), "NFTMarketplace.confirmOffer: offer not approved");
        ...
        if(MarketTradingNFT.postCreators(_tokenId) != MarketTradingNFT.ownerOf(_tokenId))
        {
            creatorFeeInETH = bidValue.mul(creatorFee).div(maxShare);
            (bool creatorTransferSuccess,) = MarketTradingNFT.postCreators(_tokenId).call{value : creatorFeeInETH}("");
            require(creatorTransferSuccess, "NFTMarketplace.confirmOffer: Failed to send creator fee");
        }
        // Send remaining to seller in ETH
        (bool sellerTransferSuccess,) = MarketTradingNFT.ownerOf(_tokenId).call{value : bidValue.sub(platformFeeInETH).sub(creatorFeeInETH)}("");
        ...
        MarketTradingNFT.setPrimarySalePrice(_tokenId, bidValue);

        // Transfer the token to the purchaser
        MarketTradingNFT.safeTransferFrom(MarketTradingNFT.ownerOf(_tokenId), msg.sender, _tokenId);
