...
contract Farm is AccessControl, ReentrancyGuard {
    ...
    function rewardedPoints(address staker) public view returns (uint256) {
        ...
        st.lpStake == true ? earnPerSec = (earnPerSec * 2) : earnPerSec;
    ...
    function consolidateExternal(address staker)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        uint256 points = rewardedPoints(staker);
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        consolidate(sender);
        if (stake.userExist(sender)) {
            stakers[sender].lpStake = true;
        }

        total = total + amount;
        stakers[sender].amount = stakers[sender].amount + amount;
        stakers[sender].isExist = true;
    ...
    function addStakeAddress(address _stakeAddress)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function userExist(address staker) external view returns (bool) {
        return stakers[staker].isExist;
    }

    function registerLp(address staker) external onlyRole(DEFAULT_ADMIN_ROLE) {
        stakers[staker].lpStake = true;
    }

    function unRegisterLp(address staker)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        stakers[staker].lpStake = false;
    }
}
