...
contract NobiToadz is ERC721, Ownable, PaymentSplitter {
    ...
    constructor() public ERC721("NobiToadz", "NTOADZ") {}
    ...
    function mintSpecial(uint256 [] memory devIDs) external onlyOwner {
        for (uint256 i = 0; i < devIDs.length; i++) {
            _safeMint(msg.sender, devIDs[i]);
        }
    }
    
    // if we wanted all lands to be the same
    function mint(uint256 quantity) external payable {
        require(totalSupply() + quantity <= maxTokens - 39, "minting this many would exceed supply");
