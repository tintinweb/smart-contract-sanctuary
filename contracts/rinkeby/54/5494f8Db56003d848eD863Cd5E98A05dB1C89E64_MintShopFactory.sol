...
contract MintShopFactory is ReentrancyGuard {
  ...
  function createShop(
    ...
    shopRecords[_farm].push(newShopAddress);
    farmMap[newShopAddress] = _farm;
    emit ShopCreated(newShopAddress, _owner);
  ...
  /**
    ...
    @param _farm The address of the shop deployer
    @return The number of MintShops added for a given address.
  */
  function getShopCount(address _farm) external view returns (uint256) {
    return shopRecords[_farm].length;
  }
