...
contract PUNK_EMOJIS is ERC721, Ownable {
    ...
    uint public constant MAX_PUNK_EMOJIS = 9999;
    ...
    constructor() ERC721("Punk Emojis","PUNK_EMOJIS")  {
        setBaseURI("https://punk-emojis.com/api/");
    }
    
    function getNFTPrice() public view returns (uint256) {
        require(hasSaleStarted == true, "Sale hasn't started yet");
        require(totalSupply() < MAX_PUNK_EMOJIS, "Sale has already ended");
    ...
    function mint(uint256 numPunkEmojis) public payable{
        require(hasSaleStarted == true, "Sale hasn't started yet");
        require(totalSupply() < MAX_PUNK_EMOJIS, "Sale has already ended");
        require(numPunkEmojis > 0 && numPunkEmojis <= 20, "You can buy minimum 1, maximum 20 Punk Emojis");
        require(totalSupply().add(numPunkEmojis) <= MAX_PUNK_EMOJIS, "Exceeds max Punk Emojis");
        require(getNFTPrice().mul(numPunkEmojis) == msg.value, "Ether value sent is not correct");
      
        for (uint i = 0; i < numPunkEmojis; i++){
            uint mintIndex = totalSupply();
            _safeMint(msg.sender, mintIndex);
        }
    }
    
    function reservePunkEmojis(uint256 numPunkEmojis) public onlyOwner {
        require(totalSupply().add(numPunkEmojis) <= MAX_PUNK_EMOJIS, 'Sale would exceed max supply');
    ...
    function airdropPunkEmoji(uint256 numPunkEmojis, address recipient) public onlyOwner {
        require(totalSupply().add(numPunkEmojis) <= MAX_PUNK_EMOJIS, 'Sale would exceed max supply');
    ...
    function airdropPunkEmojiToMany(uint256 numPunkEmojis, address[] memory recipients) external onlyOwner {
        require(totalSupply().add(recipients.length * numPunkEmojis) <= MAX_PUNK_EMOJIS, 'Sale would exceed max supply');
        
        for (uint256 i = 0; i < recipients.length; i++) {
            airdropPunkEmoji(numPunkEmojis, recipients[i]);
        }
    }
