...
// MasterChef is the master of PolkaBridge. He can make PolkaBridge and he is a fair guy.
//
...
contract PolkaBridgeFarm is Ownable, ReentrancyGuard {
    ...
    function getMultiplier(uint256 _pid)
        public
        view
        returns (uint256)
    {
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.lastRewardBlock = block.number;
        pool.lastPoolReward = pool.lastPoolReward.add(changePoolReward);
    }

    // Deposit LP tokens to MasterChef for PBR allocation.
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            temptAccPBRPerShare = pool.accPBRPerShare.add((getChangePoolReward(_pid, totalMultiplier).mul(1e18).div(lpSupply)));
        }

        uint256 pending =(user.amount.mul(temptAccPBRPerShare).sub(user.rewardDebt.mul(1e18))).div(1e18);
    ...
    function avgRewardPerBlock(uint256 _pid) public view returns (uint256) {
        uint256 totalMultiplier = countTotalMultiplier();
        uint256 changePoolReward = getChangePoolReward(_pid, totalMultiplier);
        uint256 totalReward = poolInfo[_pid]
                .totalRewardClaimed
