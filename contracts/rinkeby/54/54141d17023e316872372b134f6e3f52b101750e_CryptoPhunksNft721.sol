...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    uint256 MAX_MINTABLE_AT_ONCE = 50;
  ...
  // mapping for token creator
    ...
    uint[10000] private totalphunks;

    uint256 public _Availablephunks = 10000;
    ...
    */
    ...
    function _safeMint(address to,uint256 _num_to_mint) internal virtual {
        _safeMint(to, _num_to_mint);
    }

    function _safeMint(address to,uint256 _num_to_mint, bytes memory _data) internal virtual {
        mint(to, _num_to_mint);
        require(_checkOnERC721Received(address(0), to, _num_to_mint, _data), "ERC721: transfer to non ERC721Receiver implementer");
    }

    function mint(address to, uint256 _num_to_mint) internal virtual {
        require(to != address(0), "ERC721: mint to the zero address");
        require(_num_to_mint <= MAX_MINTABLE_AT_ONCE, "exceeded the limit");

        for (uint i = 0; i<_num_to_mint; i++){
            uint256 newtokenid = useRandomAvailableToken(_num_to_mint,i);
        ...
        // here we have to make the changes
    ...
    uint256 randomNum =
      ...
      // random index = random num % currently available phunks
      
      uint256 randomIndex = randomNum % _Availablephunks;
      
      uint256 valueatindex = totalphunks[randomIndex];
      ...
      if(valueatindex == 0) {
          ...
          //require(!_exists(valueatindex) ,"phunk has already minted");
          if(!_exists(valueatindex)){
              result = valueatindex;
          }else {
              result = _Availablephunks - 1;
          }
          
      }
      
      uint256 lastindex = _Availablephunks - 1;
      
      if(randomIndex != lastindex){
          uint lastvalarray = totalphunks[lastindex];
          if(lastvalarray == 0) {
              totalphunks[randomIndex] = lastindex;
          }else {
              totalphunks[randomIndex] = lastvalarray;
          }
      }
...
contract CryptoPhunksNft721 is ERC721 {
    uint256 public tokenCounter;
    address public owner;

    string public image_hash = '0x1112345654334344';
    ...
    constructor() ERC721("BAFCryptoPhunks", "BAFPHUNK") { }

         function ttotalphunks() public pure returns(uint256) {
         return 10000;
     }
    
    //uint256 _Availablephunks = 10000;
    ...
    //     return newItemId;
    // }
        function createCollectible(uint256 _num_to_mint) public {
        _safeMint(msg.sender, _num_to_mint);
    }
