...
contract Reward is Ownable {
    ...
    mapping(uint256 => uint256) private _tClaimed;
    ...
    function claimToken(uint256 tokenId) public returns (bool) {
        ...
        uint256 amount = _tBalance - _tClaimed[tokenId];

        if (amount > 0) {

            _tClaimed[tokenId] += amount;
    ...
    function claimTokens(uint256[] calldata tokenIds) public returns (bool) {
        require(_enableClaimToken,  "function temporarily disabled"); //FIXME
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 amount = _tBalance - _tClaimed[tokenIds[i]];

            amounts[i]              = amount;
            totalAmount            += amount;
            _tClaimed[tokenIds[i]] += amount;
        }
        ...
        if (totalAmount > 0) {
            ...
            if (!success) {
                ...
                // the sum is required since payout are coming async
                for (uint i = 0; i < tokenIds.length; i++) {
                    _tClaimed[tokenIds[i]] -= amounts[i];
                }
                return false;
            }
        }

        emit ClaimedAll(_msgSender(), totalAmount); //FIXME
    ...
    function claimAll() public returns (bool) {
        ...
        if (numTokens > 0) {
            ...
            for (uint256 i = 0; i < numTokens; i++) {
                ...
                uint256 amount = _tBalance - _tClaimed[id];
    ...
    */
    function balance() public view returns (uint256) {
        ...
        for (uint256 i = 0; i < numTokens; i++) {
            totalAmount += (_tBalance - _tClaimed[_nft.tokenOfOwnerByIndex(_msgSender(), i)]);
        }
    ...
    function balanceOf(uint256 tokenId) public view returns (uint256) {
        return (_tBalance - _tClaimed[tokenId]);
    }
