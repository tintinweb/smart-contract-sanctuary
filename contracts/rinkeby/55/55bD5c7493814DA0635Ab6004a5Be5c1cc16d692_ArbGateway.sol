...
contract ArbGateway is Ownable {
    ...
    //******************************************************
    constructor(address hbContractAddress, address nomContract, uint8 layer)
    {
        ownableInit();
    ...
    function teleportAndLock(uint16[] memory tokenIds, uint256 maxBaseFee, uint256 maxGas, uint256 maxPriority) public payable{
        ...
        uint256 nftTransferFees = (maxGas * maxPriority) + maxBaseFee;
        uint256 ethToDeposit = msg.value - (nftTransferFees * tokenIds.length);
        
        _inbox.depositEth{value: ethToDeposit}(maxBaseFee);
        //_inbox.createRetryableTicketNoRefundAliasRewrite{value: ethToDeposit}(
        //    msg.sender, ethToDeposit, maxBaseFee, msg.sender, msg.sender, 0, 0, '0x');
