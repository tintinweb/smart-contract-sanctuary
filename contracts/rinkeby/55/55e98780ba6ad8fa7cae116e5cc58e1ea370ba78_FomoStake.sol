...
contract FomoStake {
        ...
        )
    {
        ...
        finish = block.timestamp.add(plans[plan].time.mul(TIME_STEP)).sub(
            getDecreaseDays(plans[plan].time, block.timestamp)
        );
    }
    ...
    function getDecreaseDays(uint256 planTime, uint256 from)
        public
        view
        returns (uint256)
    {
        return
            planTime.sub(
                from.sub(LAUNCH_TIME).div(TIME_STEP).mul(DECREASE_DAY_STEP)
            );
    }
