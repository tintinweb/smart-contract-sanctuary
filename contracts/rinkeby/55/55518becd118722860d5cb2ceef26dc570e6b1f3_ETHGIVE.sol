...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public dividendToken = 0xDf032Bc4B9dC2782Bb09352007D4C57B75160B15;
...
contract ETHGIVE is ERC20, Ownable {
    using SafeMath for uint256;
   ...
   ETHGIVEDividendTracker public dividendTracker;
    ...
    address private _dividendToken = 0xDf032Bc4B9dC2782Bb09352007D4C57B75160B15;
    ...
    function activate() public onlyOwner {
        require(!tradingEnabled, "ETHGIVE : Trading is already enabled");
        _swapEnabled = true;
        tradingEnabled = true;
    }
    ...
    constructor() ERC20("Eth Give", "ETHGIVE") {
        dividendTracker = new ETHGIVEDividendTracker();
    ...
    receive() external payable {
      ...
      function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ETHGIVE : The Uniswap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ETHGIVE : Automated market maker pair is already set to that value");
    ...
    function excludeFromFees(address account) public onlyOwner {
        require(!_isExcludedFromFees[account], "ETHGIVE: Account is already excluded from fees");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        ...
        require(newValue != gasForProcessing, "ETHGIVE: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if (!tradingIsEnabled) {
            ...
            if(!openForPresale){
                require(canTransferBeforeTradingIsEnabled[from], "ETHGIVE : This account cannot send tokens until trading is enabled");
            }
        }
 ...
 function swapAndDividend(uint256 tokens) private lockTheSwap {
        uint256 tokenBalance = balanceOf(address(this));
        swapTokensForEth(tokenBalance);
        
        uint256 ethBalance = address(this).balance;
        uint256 devPortion = ethBalance.mul(LIQUIDITY_FEE).div(TOTAL_FEES);
        address payable _devAndMarketingAddress = payable(0x8df44F74B73C7F31fD107c22DaF30AB51C7549a3);
        _devAndMarketingAddress.transfer(devPortion);
        ...
        emit SwapAndSendToDev(tokens, devPortion);
        
    }
}

contract ETHGIVEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    event DividendClaimed(address indexed account, uint256 amount, bool indexed automatic);

    constructor() DividendPayingToken("ETHGIVE_Dividend_Tracker", "ETHGIVE_Dividend_Tracker") {
    	claimWait = 300;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "ETHGIVE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "ETHGIVE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ETHGIVE contract.");
    }
    ...
    function excludeFromDividends(address account) external onlyOwner {
      ...
      function updateGasForTransfer(uint256 newGasForTransfer) external onlyOwner {
        require(newGasForTransfer != gasForTransfer, "ETHGIVE_Dividend_Tracker: Cannot update gasForTransfer to same value");
        emit GasForTransferUpdated(newGasForTransfer, gasForTransfer);
        gasForTransfer = newGasForTransfer;
    }

    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 30 && newClaimWait <= 86400, "ETHGIVE_Dividend_Tracker: claimWait must be updated to between 1 minute and 24 hours");
        require(newClaimWait != claimWait, "ETHGIVE_Dividend_Tracker: Cannot update claimWait to same value");
    ...
    function processAccount(address payable account, bool automatic) public onlyOwner returns (bool) {
    	...
    	if(amount > 0) {
            ...
            emit DividendClaimed(account, amount, automatic);
    		return true;
    	}
