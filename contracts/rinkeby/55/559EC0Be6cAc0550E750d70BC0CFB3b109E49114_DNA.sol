...
contract DNA is ERC721, VRFConsumerBase {
    ...
    event requestedDNA(bytes32 indexed requestId); 
    ...
    function createDNA(string memory tokenURI) 
        public returns (bytes32){
            bytes32 requestId = requestRandomness(keyHash, fee);
            requestIdToSender[requestId] = msg.sender;
            requestIdToTokenURI[requestId] = tokenURI;
            emit requestedDNA(requestId);
    }

    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override {
        address DNAOwner = requestIdToSender[requestId];
        string memory tokenURI = requestIdToTokenURI[requestId];
        uint256 newItemId = tokenCounter;
        _safeMint(DNAOwner, newItemId);
        _setTokenURI(newItemId, tokenURI);
        // bytes32 breed = Breed(randomNumber % 3); 
        tokenIdToDNA[newItemId] = randomNumber;
