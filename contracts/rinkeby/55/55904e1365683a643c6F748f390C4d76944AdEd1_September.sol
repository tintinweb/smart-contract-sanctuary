...
contract September is ERC721, Ownable {
    ...
    constructor() ERC721("September", "SEP") {}
    ...
    function mint(uint256 qty) external payable {
        ...
        if (presaleActive) {
            require(nonce + qty <= presaleTotalSupply, "Presale is sold out");
        }
        require(qty <= maxTx && qty > 0, "Qty of mints not allowed");
        require(
            qty + nonce + presaleTotalSupply <= totalSupply,
    ...
    function withdrawTeam() public payable onlyOwner {
        ...
        require(payable(a1).send((balance * 10) / 100));
        require(payable(a2).send((balance * 25) / 100));
        require(payable(a3).send((balance * 25) / 100));
        require(payable(a4).send((balance * 40) / 100));
    }
