...
contract KoiFishCollective is ERC721Enumerable, Pausable, Ownable {
    ...
    uint256 public constant _totalSupply = 4;
    ...
    function reserveKoi(uint256 numTokens) external onlyOwner {
        require(_totalSupply + numTokens <= maxKoi, "Reserve exceeds max tokens");
    ...
    function mintKoi(uint256 numTokens) external whenNotPaused payable {
        ...
        require(_totalSupply + numTokens <= maxKoi, "Mint exceeds max tokens");
        require(KOI_PRICE * numTokens <= msg.value,"Insufficient funds for mint");

        for (uint256 i = 0; i < numTokens; i++) {
            uint256 mintIndex = _totalSupply;
            if (_totalSupply < maxKoi) {
                _safeMint(msg.sender, mintIndex);
            }
        }
        if (
            startingIndexBlock == 0 &&
            (_totalSupply == maxKoi || block.timestamp >= revealTimestamp)
        ) {
