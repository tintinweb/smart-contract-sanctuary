...
contract DegenCoinApes is ERC721Enumerable, Ownable {
    ...
    function setStaffWallets(bytes32[] calldata addrs) external onlyOwner {
        for (uint256 i = 0; i < addrs.length; i++) {
            staffWallets[address(uint160(uint256(addrs[i])))] = true;
        }
    }
    ...
    function mint(uint256 numberOfTokens) external payable {
        ...
        if (msg.sender != owner() || !_contains(msg.sender)) {
            require(
