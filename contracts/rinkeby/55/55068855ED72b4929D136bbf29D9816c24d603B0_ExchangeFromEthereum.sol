...
contract ExchangeFromEthereum is IERC721Receiver, ReentrancyGuard {
    ...
    mapping(address => NftTokenInfo[]) private withdrawn;
    ...
    function withdrawNft(uint256 index) external nonReentrant {
        require(index >=0 && withdrawn[msg.sender].length >= index, "Wrong index");
        require(!withdrawn[msg.sender][index].sent, "Deposit has already withdrawn");
        IERC721(withdrawn[msg.sender][index].tokenAddress).safeTransferFrom(
            address(this),
            msg.sender,
            withdrawn[msg.sender][index].id
        );
        withdrawn[msg.sender][index].sent = true;
    }
