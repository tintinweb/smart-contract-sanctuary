...
contract POC is ERC721EnumerableB, Ownable, PaymentSplitter, ReentrancyGuard {
  ...
  function mintPreSale(uint count, bytes32 hash, bytes memory signature) external payable nonReentrant() {
    ...
    require(hashTransaction(msg.sender, count) == hash, "Hash failed");
