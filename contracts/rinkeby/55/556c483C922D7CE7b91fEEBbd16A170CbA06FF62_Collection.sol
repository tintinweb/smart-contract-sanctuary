...
contract Collection is Ownable, ReentrancyGuard {
    ...
    uint256 lastRNG = block.difficulty; // Replace when VRF is available on arbitrum
    Dropper dropper;
    ...
    // TODO Move this into constructor and have Dropper.sol deploy this contract as a child
    ...
    function buyPacks(uint256 _packId, uint256 _quantity) external payable mintLock {
        ...
        //emit packsPurchased(msg.sender, _packId, _quantity);
    ...
    function openPacks(
        ...
        for (uint packNumber = 0; packNumber < _packQuantity; packNumber++) 
        {
            ...
            //emit packOpened(msg.sender, momentIds);
    ...
    function getSubId(uint256 _momentId, uint256 _rarityId, uint256 _seed) internal returns (uint256) {
        ...
        //TODO update dropper.momentMinted OR calculate mint range based on total minted 
        usedIds[_momentId][subId] = true;
        dropper.setMoment(uri, (totalMinted + 1), mintRange, _momentId);
        return subId;
    }

    // TODO THIS IS INSECURE! Replace with LINK VRF or better on chain alternative
