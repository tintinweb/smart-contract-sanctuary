...
// Part: OpenZeppelin/[email protected]/IERC1271
...
// Part: OpenZeppelin/[email protected]/SignatureChecker
...
contract GMMinter is Ownable {
    //    string constant prefix = hex"19";//Ethereum Signed Message:\n";
    string constant prefix = "\x19Ethereum Signed Message:\n140";
    string constant delimiter = "\n";

    GreatMastersNFT public nft; //TODO use interface
    GMAuction public auction; //TODO use interface
    address public minter;
    ...
    /**
       ...
       @param nft_ nft contract defining items traded
       @param minter_ minter address, normally owned by a GM moderator
     */
    constructor(
    ...
    function unprefixedMessage(string calldata artwork, string calldata document, string calldata details) public view returns (string memory) {
        return string(abi.encodePacked(artwork, delimiter, document, delimiter, details));
    }

    function combineMessage(string calldata artwork, string calldata document, string calldata details) public view returns (string memory) {
        return string(abi.encodePacked(prefix, artwork, delimiter, document, delimiter, details));
    }

    function hashMessage(string calldata artwork, string calldata document, string calldata details) public view returns (bytes32) {
        return keccak256(abi.encodePacked(prefix, artwork, delimiter, document, delimiter, details));
    }

    function verify(string calldata artwork, string calldata document, string calldata details, address signer, bytes calldata signature) public view returns (bool) {
    ...
    function mintToAuction(
        ...
        uint256 startPrice_
    ) public onlyMinter returns (uint256 artId){
        require(verify(artwork, document, details, signer, signature), "GMMinter: only verified signatures can result in minting artwork tokens");
    ...
    function setAuction(GMAuction auction_) public onlyOwner {
        auction = auction_;
    }

    modifier onlyMinter {
        require(msg.sender == minter, "GMMinter: action is allowed only to the minter");
        _;
    }
