...
contract RedBerryToken is BEP20 {
        ...
        onlyOperator
    {
        ...
        if (_excluded == true) {
            ...
            for (uint256 i = 0; i < excludedFromAntiwhale.length; i++) {
                if (_account == excludedFromAntiwhale[i]) {
                    for (
                        uint256 j = i;
                        j < excludedFromAntiwhale.length - 1;
                        j++
                    ) {
                        excludedFromAntiwhale[j] = excludedFromAntiwhale[j + 1];
                    }
                    excludedFromAntiwhale.pop();
                }
            }
        }
    }
        ...
        onlyOperator
    {
        ...
        if (_excluded == true) {
            ...
            for (uint256 i = 0; i < excludedFromTaxRate.length; i++) {
                if (_account == excludedFromTaxRate[i]) {
                    for (
                        uint256 j = i;
                        j < excludedFromTaxRate.length - 1;
                        j++
                    ) {
                        excludedFromTaxRate[j] = excludedFromTaxRate[j + 1];
                    }
                    excludedFromTaxRate.pop();
                }
            }
        }
    }
...
contract MasterChef is Ownable {
    ...
    uint256 public constant MAX_HARVEST_LOCKUP_TIME = 691200;
    ...
    function deposit(
        ...
        if (user.amount > 0) {
            uint256 redBerryReward = getMultiplier(
                pool.lastRewardBlock,
                block.number
            ).mul(redBerryPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            uint256 _pendingAmount = 0;
            uint256 pending = pendingRedBerry(_pid, msg.sender).add(
                redBerryReward.div(10)
            );
