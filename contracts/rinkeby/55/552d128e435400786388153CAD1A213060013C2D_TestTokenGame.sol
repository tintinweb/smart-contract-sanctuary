...
contract TestTokenGame is Ownable, ERC20 {
    using SafeMath for uint256;

    mapping(address => bool) whales;
    mapping(address => bool) bossCallers;
    mapping(address => bool) gamers;
    address public farmer;

    uint256 public maxSupply = 10**9 * 10**18;
    uint256 public capGame = maxSupply.div(100).mul(20);
    uint256 public capFarm = maxSupply.div(100).mul(20);
    ...
    constructor(string memory name, string memory symbol) ERC20(name, symbol) {
        _mint(_msgSender(), maxSupply.sub(capGame).sub(capFarm));
    ...
    function win(address winner, uint256 reward) external onlyGamer {
        require(capGame > 0, "Over cap game");
        require(winner != address(0), "0x is not accepted here");
        require(reward > 0, "not accept 0 value");

        if (capGame > reward) {
            capGame = capGame.sub(reward);
            _mint(winner, reward);
        } else {
            _mint(winner, capGame);
            capGame = 0;
        }
    }
