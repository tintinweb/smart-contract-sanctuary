...
contract TestStakingRewardV2 is OwnableTokenAccessControl, IStakingReward {
    ...
    uint256 public constant stakingRewardRate = 30 ether / uint256(1 days);
    ...
    function didStakeTokens(address account, uint16[] calldata tokenIds) external override onlyStaking {
        ...
        uint256 claim = _claimDataOf(account);
    ...
    function _claimDataOf(address account) private view returns (uint256 claim) {   
        claim = _claims[account];
        if (claim == 0) {
            uint256 timestamp = stakingRewardStartTimestamp;
            if (timestamp > 0) {
                uint stakedCount = 0;

                uint16[] memory stakedTokens = IStaking(STAKING_ADDRESS).stakedTokensOf(account);
                uint length = stakedTokens.length;
                for (uint i=0; i<length; i++) {
                    if (IStaking(STAKING_ADDRESS).timestampOf(stakedTokens[i]) <= timestamp) {
                        stakedCount++;
                    }
                }

                if (stakedCount > 0) {
                    claim = (timestamp << 14) | stakedCount;
                }
            }
        }
    }
