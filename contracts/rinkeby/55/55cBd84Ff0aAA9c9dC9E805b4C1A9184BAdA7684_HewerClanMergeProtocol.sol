...
contract HewerClanMergeProtocol is Ownable, ERC721Enumerable {
    ...
    modifier onlyRegisteredCollection(string memory name) {
        require(collectionNameToCollectionEntry[name].registered, "Unregistered collection");
        _;
    }
    ...
    function flipUnmergeAllowedState() public onlyOwner {
    ...
    function burn(uint256 tokenId) public {
        require(unmergeAllowed, "Burning merged tag required unmerge option to be allowed");

        _burn(tokenId);

        MergedEntry memory entry = tokenIdToMergedEntry[tokenId];

        emit TagUnmerged(entry.tagTokenId, entry.tokenId, entry.collectionName, tokenId);

        tagTokenIdToMergeState[entry.tagTokenId].merged = false;
        collectionNftTokenIdToMergeState[entry.collectionName][entry.tokenId].merged = false;
        delete tokenIdToMergedEntry[tokenId];
    }
}
