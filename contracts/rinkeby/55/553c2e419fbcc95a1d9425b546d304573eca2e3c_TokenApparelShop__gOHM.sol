...
contract TokenApparelShop__gOHM is AccessControl {
  using SafeMath for uint256;

  bytes32 public constant IYK_MANAGER_ROLE = keccak256('IYK_MANAGER_ROLE');
  bytes32 public constant OLYMPUSDAO_MINTER_ROLE = keccak256('OLYMPUSDAO_MINTER_ROLE');
  ...
  // Sale parameters
  uint256 public price; // Padded price in OHM (ex. 1000000000 for 1 OHM)
  bool public isSaleActive;

  event Purchase(address recipient, Size size, uint256 quantity, uint256 gOHMprice);
  ...
  function buy(uint256 quantity, Size size) public {
    ...
    // Transfer funds
    uint256 gOHMprice = getgOHMPrice();
    gOHM.transferFrom(msg.sender, address(this), gOHMprice * quantity);

    soldAmount[size] += quantity;

    emit Purchase(msg.sender, size, quantity, gOHMprice);
  }

  function mintBatch(uint256[] memory quantities) public onlyRole(OLYMPUSDAO_MINTER_ROLE) {
    ...
    for (uint256 i = 0; i < 6; ++i) {
      if (quantities[i] > 0) {
        ...
        emit Purchase(msg.sender, Size(i), quantities[i], 0);
      }
    }
  }
  ...
  // Views
  function getgOHMPrice() public view returns (uint256) {
    return gOHM.balanceTo(price);
  }

  function getRemainingForSale() public view returns (uint256[6] memory) {
    uint256[6] memory numRemaining;

    for (uint256 i = 0; i < 6; ++i) {
      numRemaining[i] = maxAmount[Size(i)] - soldAmount[Size(i)];
    }
