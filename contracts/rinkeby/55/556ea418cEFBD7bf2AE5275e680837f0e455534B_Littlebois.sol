...
contract Littlebois is ERC721Enumerable, Ownable{
    using SafeMath for uint256;
    using Address for address;
    using Strings for uint256;


    uint256 public constant NFT_PRICE = 30000000000000000; // 0.05 ETH
    uint public constant MAX_NFT_PURCHASE = 40;
    ...
    constructor() ERC721("Little Bois","LBOI"){
    }
    ...
    function mint(uint numberOfTokensMax40) public payable {
        require(saleIsActive, "Sale is not active at the moment");
        require(numberOfTokensMax40 > 0, "Number of tokens can not be less than or equal to 0");
        require(totalSupply().add(numberOfTokensMax40) <= MAX_SUPPLY, "Purchase would exceed max supply");
        require(numberOfTokensMax40 <= MAX_NFT_PURCHASE,"Can only mint up to 10 per purchase");
        require(NFT_PRICE.mul(numberOfTokensMax40) == msg.value, "Sent ether value is incorrect");

        for (uint i = 0; i < numberOfTokensMax40; i++) {
