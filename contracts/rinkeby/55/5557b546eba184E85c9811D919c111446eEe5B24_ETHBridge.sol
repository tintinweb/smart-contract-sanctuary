...
contract ETHBridge is Ownable {
    ...
    function portBack(bytes calldata _signature, string memory _portTxHash, address _bepToken, uint _amount) external {
        ...
        address sigg = ECDSA.recover(message, _signature);

        emit Data(chainId, address(this), _bepToken, _amount, msg.sender, _portTxHash);
        emit Loda(sigg, signer);
        require(ECDSA.recover(message, _signature) == signer, "invalid signature");
