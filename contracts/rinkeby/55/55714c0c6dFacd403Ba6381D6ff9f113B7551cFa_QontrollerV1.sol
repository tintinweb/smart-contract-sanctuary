...
contract FixedRateLoanMarket is ERC20{
  ...
  /// @notice Address of the `CollateralManager`
  ...
  /// signature => filled
  mapping(bytes => uint) public quoteFill;
  ...
  function _createFixedRateLoan(
    ...
    // Effects: Keep track of total borrows by an account.
    ...
    accountBorrows[borrower] += principalPlusInterest;

    // Mint the full `principalPlusInterest` amount to the lender, redeemable
    // at maturity
...
contract QollateralManager {

  using SafeMath for uint;

  /// @notice Address of the `Qontroller`
  QontrollerV1 public qontroller;
  ...
  mapping(address => address[]) public iterableAccountAssets;
  ...
  constructor(address _qontrollerAddress) public {
    qontroller = QontrollerV1(_qontrollerAddress);
  }
  ...
  function depositCollateral(address tokenAddress, uint amount) external {
    ...
    QTypes.Asset memory asset = qontroller.getAsset(tokenAddress);
    ...
    if(!accountAssets[tokenAddress][msg.sender]){
      iterableAccountAssets[msg.sender].push(tokenAddress);
      accountAssets[tokenAddress][msg.sender] = true;
    }
  ...
  function _getTotalCollateralValue(
    ...
    for(uint i=0; i<iterableAccountAssets[account].length; i++){

      // Get the token address in i'th slot of `iterableAccountAssets[account]`
      address tokenAddress = iterableAccountAssets[account][i];
...
import "./FixedRateLoanMarket.sol";
...
contract QontrollerV1 {
  ...
  address public qollateralManager;
  ...
  mapping(address => mapping(uint => address)) public fixedRateLoanMarkets;
  ...
  constructor() public {
    ...
    // Instantiate `QollateralManager`
    QollateralManager qm = new QollateralManager(address(this));
    
    // Store the address of `QollateralManager`
    qollateralManager = address(qm);
  }
  ...
  function _addAsset(
    ...
    QTypes.Asset memory asset = QTypes.Asset(false, _oracleFeed, _riskFactor, maturities);
  ...
  function _createFixedRateLoanMarket(
    ...
    require(assets[principalTokenAddress].isEnabled, "unsupported asset");
    
    // Check that this market hasn't already been instantiated before
    require(fixedRateLoanMarkets[principalTokenAddress][maturity] == address(0),
            "market already exists"
            );

    // Add the maturity as enabled to the corresponding Asset
    QTypes.Asset storage asset = assets[principalTokenAddress];
    asset.maturities.push(maturity);

    // Instantiate `FixedRateLoanMarket` contract
    FixedRateLoanMarket frlm = new FixedRateLoanMarket(
                                                       qollateralManager,
                                                       principalTokenAddress,
                                                       maturity,
                                                       name,
    ...
    fixedRateLoanMarkets[principalTokenAddress][maturity] = address(frlm);
  ...
  /** PUBLIC INTERFACE **/
  function getAsset(address tokenAddress) public view returns(QTypes.Asset memory) {
    return assets[tokenAddress];
  }
  
  function getFixedRateLoanMarket(
                                  address principalTokenAddress,
                                  uint maturity
                                  ) public view returns(address){
    return fixedRateLoanMarkets[principalTokenAddress][maturity];
  }
