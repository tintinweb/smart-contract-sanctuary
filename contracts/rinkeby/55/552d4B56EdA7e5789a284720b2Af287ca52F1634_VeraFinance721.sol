...
contract VeraFinance721 is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    ERC721PresetMinterPauserAutoId _mortgageToken;
    ...
    event NftListed(uint256 id, address nftContract, uint256 nftId, uint256 price, uint256 downPayment, uint256 duration, uint256 paymentPeriod, bytes32 status, address sellerAddress, uint8 payment);
    ...
    constructor(uint16 fee_) ReentrancyGuard() { 
        ...
        // receipt token
        _mortgageToken = new ERC721PresetMinterPauserAutoId(
            "ETH Finance Token",
            "EFC",
    ...
    function setFee(uint16 fee) external onlyOwner { //cannot set fee more than 1000 pt
    ...
    function listNft(
        ...
        emit NftListed(_tradeCounter, nftContract, nftId, price, downPayment, duration, paymentPeriod, "Open", msg.sender, paymentCurrency);
        _tradeCounter += 1;
    }
    ...
    function payDownpayment(
        ...
        require(
            total == amount,
            "NftFinance: unsufficient amount send"
        );
    ...
    function sendPeriodicPayment(
        ...
        if (
            ...
            revert(
                "NftFinance: duration ended"
            );
        }
        require(
            msg.sender == idTobuyer[id],
            "NftFinance: buyer only"
        );

        if (block.timestamp > nft.nextPayment) { 
            revert(
                "NftFinance: periodic payment missed"
            );
        }
        ...
        if (_amountPayed[id] >= (nft.price + fee)) { 
            revert(
                "NftFinance: amount paid. claim nft now"
            );
        }
    ...
    function ownerClaimNft(uint256 id) nonReentrant() external {
        NftFinance storage nft = _idToNftFinance[id];
        require(
            _isDownPaymentPayed[id] == true,
        ...
        token.safeTransferFrom(address(this), payable(nft.sellerAddress), nft.nftId);
    ...
    function claimNft(uint256 id) external {
        require(msg.sender == idTobuyer[id], "NftFinance: you can't claim this Nft");
        NftFinance storage nft = _idToNftFinance[id];
        uint256 fee = calculateFee(nft.price);
        require(_amountPayed[id] == (nft.price + fee), "NftFinance: you have not payed full payment yet"
        );
        IERC721 token = IERC721(nft.nftContract);
    ...
    function idToNftFinance(uint128 id) external view returns(NftFinance memory) {
        return _idToNftFinance[id];
    }

    function setPaymentToken(uint8 index, address currencyAddress) external onlyOwner {
        require(
