...
contract STO is Ownable {
    ...
    // mapping to map offerId to the offers
    mapping(uint=>Offer) idToOfferMapping;
    ...
    // a method for users to create an offer
    function createOffer(uint256 _id,uint256 _tokenAmount, uint256 _usdtTokenAmount) public {
        require(_id<=creatorCounter,"Invalid id");
        require(_tokenAmount>0,"Invalid token amount");
        require(_usdtTokenAmount>0,"Invalid usdt token amount");
        require(creatorCoinInstance.balanceOf(msg.sender,_id)>=_tokenAmount,"User has insufficient property tokens");
        require(IERC20(usdtContractAddress).balanceOf(msg.sender)>=_usdtTokenAmount,"User has insufficient usdt tokens");
        require(!isOfferCreated[msg.sender][_id],"User has already created offer for this property");
        ...
        userOfferIdMapping[msg.sender][_id]=offerCount;
        idToOfferMapping[offerCount] = newOffer;
    }


    // function to edit the offer
    function editOffer(uint256 _offerId,uint256 _usdtTokenAmount,uint256 _tokenAmount)  external  {
               // get the current offer
        Offer memory currentOffer = idToOfferMapping[_offerId];
        // check that offer is active 
        require(currentOffer.offerCreator==msg.sender,"Only owner creator can edit offers");
        require(_offerId<=offerCount,"Invalid offerId");
        require(isOfferCreated[msg.sender][currentOffer.tokenId],"You cannot edit inactive offers");
        require(_tokenAmount>0,"Invalid token amount");
        require(_usdtTokenAmount>0,"Invalid usdt token amount");
        require(creatorCoinInstance.balanceOf(msg.sender,currentOffer.tokenId)>=_tokenAmount,"User has insufficient property tokens");
        require(IERC20(usdtContractAddress).balanceOf(msg.sender)>=_usdtTokenAmount,"User has insufficient usdt tokens");
        ...
        idToOfferMapping[_offerId]=currentOffer;

    }

    // function to accept the offer
    function acceptOffer(uint256 _id,uint256 _offerId) external {
