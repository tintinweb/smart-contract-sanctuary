...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function pendingGive(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 giveReward = multiplier.mul(givePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accGivePerShare = accGivePerShare.add(giveReward.mul(1e18).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accGivePerShare = pool.accGivePerShare.add(giveReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accGivePerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
