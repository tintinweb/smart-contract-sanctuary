...
interface ICasino{
    function randomMafiosoOwner() external returns (address);
    function addTokensToStake(address account, uint16[] calldata tokenIds) external;
}

contract DegenGamblers is ERC721, Ownable {
    uint public MAX_TOKENS = 50000;//Minting new gamblers turns off after this, gen0 + gen1 combined

    //Setup various counters
    uint public tokensMinted = 0;
    uint16 public phase = 1;
    uint16 public mafiosoStolen = 0;
    uint16 public gamblerStolen = 0;
    uint16 public mafiosoMinted = 0;

    bool private _paused = true; //Start the game paused

    mapping(uint16 => uint) public phasePrice;//Mapping from phase to a price for that phase of the sale

    ICasino public casino;//Reference to the Casino, a "game" contract
    ICHIPS public chips;//Reference to chip contract

    string private _apiURI = "https://gold-hunt.herokuapp.com/token/";//_baseURI
    mapping(uint16 => bool) private _isMafioso;//Keeps tracks of which tokens are mafiosos uint16=65535
    ...
    constructor() ERC721("Degen Gamblers", "DEGENS") {
        //Mint some for the homies
    ...
    function mint(uint _amount, bool _stake) public payable whenNotPaused {
        ...
        uint totalChipsCost = 0;
        if (phase == 1) {
            // Paid mint
            require(mintPrice(_amount) == msg.value, "Invalid payment amount");
        } else {
            // Mint via CHIPS token burn
            require(msg.value == 0, "Now minting is done via CHIPS");
            totalChipsCost = mintPrice(_amount);
            require(chips.balanceOf(msg.sender) >= totalChipsCost, "Not enough CHIPS");
        }

        if (totalChipsCost > 0) {
            chips.burn(msg.sender, totalChipsCost);
        }
        ...
        for (uint i = 0; i < _amount; i++) {
            ...
            if (isMafioso(tokenId)) {
                mafiosoMinted += 1;
            }

            if (recipient != msg.sender) {
                isMafioso(tokenId) ? mafiosoStolen += 1 : gamblerStolen += 1;
            ...
            if (!_stake || recipient != msg.sender) {
                ...
                _safeMint(address(casino), tokenId);
                tokenIds[i] = tokenId;
            }
        }
        if (_stake) {
            casino.addTokensToStake(msg.sender, tokenIds);
        }
    }
    ...
    function selectRecipient() internal returns (address) {
        ...
        if (getSomeRandomNumber(mafiosoMinted, 100) >= 10) {
            return msg.sender; // 90%
        }

        address thief = casino.randomMafiosoOwner();
    ...
    function isMafioso(uint16 id) public view returns (bool) {
        return _isMafioso[id];
    }
    ...
    function setMafiosoId(uint16 id, bool special) external onlyOwner {
        _isMafioso[id] = special;
    }

    function setMafiosoIds(uint16[] calldata ids) external onlyOwner {
        for (uint i = 0; i < ids.length; i++) {
            _isMafioso[ids[i]] = true;
        }
    }

    function setCasino(address _island) external onlyOwner {
        casino = ICasino(_island);
    }
