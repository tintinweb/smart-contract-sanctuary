...
import './ProjectPupCultBase.sol';

/**
 * @title ProjectPupCult wrapper contract
 */
contract ProjectPupCult is ProjectPupCultBase {
    constructor()
        ProjectPupCultBase(
            "ProjectPupCult",
            "PPC",
            2222,
            "https://gateway.pinata.cloud/ipfs/QmNRTNPf5czhKcmq9R4pjU61nT9GaiM6EG6hVjASdqN1fs/" //bad ipfs
        )
    {
...
import "@nftculture/nftc-contract-library/contracts/external/ERC721A.sol";
import "@nftculture/nftc-open-contracts/contracts/utility/BooleanPacking.sol";
import "@nftculture/nftc-open-contracts/contracts/utility/MultiIncrementer.sol";
...
/**
 * @title ProjectPupCultBase
 */
abstract contract ProjectPupCultBase is
    ERC721A,
    Ownable,
    ReentrancyGuard,
    PaymentSplitter
{
    using MultiIncrementer for MultiIncrementer.Counter;
    ...
    uint256 public mintControlFlags;
    uint256 private constant COL_CLAIMING_ACTIVE = 1;
    uint256 private constant COL_PRESALE_ACTIVE = 2;
    uint256 private constant COL_MINTING_ACTIVE = 3;

    MultiIncrementer.Counter private _coinMintCounter;

    address[] private addresses = [
        //hardhat wallets for now.
        0xdF3e18d64BC6A983f673Ab319CCaE4f1a57C7097,
        0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266
    ];

    uint256[] private stonks = [85, 15];
    ...
    constructor(
        ...
        ERC721A(__name, __symbol, __maxNftsForSale)
        PaymentSplitter(addresses, stonks)
    {
        baseURI = __baseURI;
    ...
    function setMintingState(
        ...
        tempControlFlags = tempControlFlags.setBoolean(COL_CLAIMING_ACTIVE, __claimingActive);
        tempControlFlags = tempControlFlags.setBoolean(COL_PRESALE_ACTIVE, __presaleActive);
        tempControlFlags = tempControlFlags.setBoolean(COL_MINTING_ACTIVE, __mintingActive);
    ...
    function isClaimingActive() external view returns (bool) {
        return mintControlFlags.getBoolean(COL_CLAIMING_ACTIVE);
    }

    function isPresaleActive() external view returns (bool) {
        return mintControlFlags.getBoolean(COL_PRESALE_ACTIVE);
    }

    function isMintingActive() external view returns (bool) {
        return mintControlFlags.getBoolean(COL_MINTING_ACTIVE);
    }
    ...
    //TBD: not sure this is actually needed with paymentsplitter. might just be a good fallback.
    ...
    /**
     * Purchase bound by terms & conditions of project.
     */
    function mintPCTokens(uint256 count) external payable nonReentrant {
        require(mintControlFlags.getBoolean(COL_MINTING_ACTIVE), "Not started");
        require(0 < count && count <= MAX_MINT_PER_TRANS, "Invalid count");
        require(mintPricePerNft * count == msg.value, "Invalid price");

        _mintPCTokens(_msgSender(), count);
    }

    function reservePCTokens(address[] memory friends, uint256 count)
        external
        onlyOwner
    {
        require(0 < count && count <= MAX_MINT_PER_TRANS, "Invalid count");
        ...
        for (idx = 0; idx < friends.length; idx++) {
            _mintPCTokens(friends[idx], count);
        }
    }
    ...
    function _mintPCTokens(address minter, uint256 count) internal {
        ...
        _coinMintCounter.increment(count);
