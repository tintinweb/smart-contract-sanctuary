...
contract Marketplace2 {
  ...
  // event itemDelete(uint256 id, address tokenAddress, address owner);
  ...
  function delItemFromMarket(address tokenAddress, uint256 id)
    external
    HasTransferApproval(tokenAddress)
  {
    // require(activeItems[tokenAddress][id] == true, "Item is not in market");
    // require(msg.sender == itemsForSale[id].seller);
    activeItems[tokenAddress][itemsForSale[id].tokenId] = false;
    delete itemsForSale[id];
    // emit itemDelete(tokenId, tokenAddress, msg.sender);
  }
  ...
  function buyItem(uint256 id)
    ...
    ItemExists(id)
    IsForSale(id)
    HasTransferApproval(itemsForSale[id].tokenAddress)
  {
