    ...
    */
    ...
    function addVesting(address _beneficiary, uint256 _amount) public onlyOwner {
        ...
        vestings[_beneficiary] = Vesting({
            _endDate: block.timestamp+(60*60*20),
            _startDate:block.timestamp,
            _cliffDate: block.timestamp+(60*60*10),
            _amount: _amount,
            _remaining:_amount,
            _nextDate: block.timestamp+(60*60*10),
            _revoked:false
        });
    ...
    */
    function releaseTokens(address _beneficiary) public {
        ...
        if (now < vesting._cliffDate || now<vesting._nextDate) {
          ...
          vesting._nextDate=vesting._nextDate.add(60*60);
        }
    ...
    { 
        ...
        token.safeTransfer(_owner,remainingBalance);
        emit TokensRemoved(remainingBalance);
    }
