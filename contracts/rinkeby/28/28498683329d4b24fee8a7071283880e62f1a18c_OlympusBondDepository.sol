...
contract OlympusBondDepository is Ownable {
    ...
    function setBondTerm( 
        uint controlVariable_, // Scaling variable for bond price
        uint vestingTerm_, // Length in blocks for bond to vest
        uint minimumPrice_, // Lowest price a bond can be purchased
        uint maxPayout_, // Maximum amount (as % of circ supply) a bond can pay out
        uint DAOShare_ // Amount of profits DAO takes
    ...
    /**
        @notice sets incremental adjustment to BCV
    ...
    function deposit( 
        ...
        uint price = bondPriceInUSD(); // Stored in bond info

        require( maxPrice_ >= _bondPrice(), "Slippage limit: more than max price" ); // slippage protection

        uint value;
        if( isLiquidityBond ) { // convert amount to value of LP at risk-free value
            value = IBondCalculator( bondCalculator ).valuation( principle, amount_ ); 
        } else { // convert amount to OHM decimals
            value = amount_.mul( 10 ** IERC20( OHM ).decimals() ).div( 10 ** IERC20( principle ).decimals() ); 
        }
        uint payout = payoutFor( value ); // compute how much of the OHM minted from principle goes to the bonder

        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 OHM
        require( payout <= maxPayout(), "Bond too large"); // size protection needed because bonds have no slippage
        ...
        // Deposit principle to mint OHM
        IERC20( principle ).approve( address( treasury ), amount_ );
        ITreasury( treasury ).deposit( amount_, principle, profit );
        ...
        bondInfo[ depositor_ ] = Bond({
            valueRemaining: bondInfo[ depositor_ ].valueRemaining.add( value ), // add on to previous amount if exists
            payoutRemaining: bondInfo[ depositor_ ].payoutRemaining.add( payout ), // add on to previous amount if exists
    ...
    /**
        ...
        @return _price uint
     */
    function _bondPrice() internal returns ( uint _price ) {
        _price = terms.controlVariable.mul( _calcDebtRatio() ).add( 1000000000 ).div( 1e7 );
        if ( _price < terms.minimumPrice ) {
            _price = terms.minimumPrice;        
    ...
    /**
        @notice calculate current bond price without a minimum
        @return _price uint
     */
    function pureBondPrice() external view returns ( uint _price ) {
        _price = terms.controlVariable.mul( _calcDebtRatio() ).add( 1000000000 ).div( 1e7 );
    }
    ...
    /**
        ...
        @return _price uint
     */
    function bondPriceInUSD() public view returns ( uint _price ) {
        if( isLiquidityBond ) {
            _price = bondPrice().mul( IBondCalculator( bondCalculator ).markdown( principle ) ).div( 1e2 );
        } else {
            _price = bondPrice().mul( 10 ** IERC20( principle ).decimals() ).div( 100 );
        }
    }
