...
contract CollateralAndMint is LandOracleProxy, Ownable {
    using SafeMath for uint256;
    address public oracleAddress;
    address public CompAddress;
    address public PROTOCOL_TOKEN_ADR;
    uint256 public collateralRequirementPercent;
    uint256 public CETHCurrentBalanace;

    constructor(
        SyntheticToken _token,
        LandOracleProxy _price,
        uint256 _collateralrequirementpercent,
        address _oracleaddress,
        address _compeaddress,
        address _protocoltokenadr,
        address _uniswap
    ) public {
        token = _token;
        landPerEthPriceIndex = _price;
        collateralRequirementPercent = _collateralrequirementpercent;
        oracleAddress = _oracleaddress;
        CompAddress = _compeaddress;
        PROTOCOL_TOKEN_ADR = _protocoltokenadr;
    ...
    LandOracleProxy public landPerEthPriceIndex;
    ...
    // When we will recieve money back
    receive() external payable {}

    function CollateralizeEth() public payable {
        CalculateCollateralBalance(msg.sender, msg.value, 0);
        ICompProxy(CompAddress).mint{value: msg.value}();
        uint256 CETHNewBalance =
            ICompProxy(CompAddress).balanceOf(address(this));
        CETH[msg.sender] = CETHNewBalance.add(CETHCurrentBalanace).sub(
            CETHCurrentBalanace
        );
        CETHCurrentBalanace = CETHNewBalance;
    }

    function LandIndexPrice() public view returns (uint256) {
        uint256 landPerEthPrice = landPerEthPriceIndex.LandPrice(oracleAddress);
        return landPerEthPrice;
    }

    // Adjust Collateral Balance Based on Deposits and withdraws.
    function CalculateCollateralBalance(
        address _address,
        uint256 _deposit,
        uint256 _withdraw
    ) private {
    ...
    function MintAsset(uint256 _amount) public {
        uint256 TokenPricePerEth =
            landPerEthPriceIndex.LandPrice(oracleAddress);
        require(
    ...
    function BurnAsset(uint256 _amount) public {
        require(CollateralUsedEth[msg.sender] > 0);
    ...
    function prepWithdraw(uint256 _amount) public {
        // Calculate the portion of the withdraw.
        uint256 withdrawPortion = _amount;
        // Pass the correct number for redeem.
        CETH[msg.sender] = CETH[msg.sender].sub(withdrawPortion);
        CETHCurrentBalanace = CETHCurrentBalanace.sub(withdrawPortion);
    ...
    function WithdrawCollateral(uint256 _amount) public {
        ...
        // Require that Farm has enough Ether
        require(
            address(this).balance >= _amount,
            "Not enough money on thet protocol"
        );
        CalculateCollateralBalance(msg.sender, 0, _amount);
...
// Calls the function that gives the last price for Land Index Token per Eth
contract LandOracleProxy {
    function LandPrice(address _called) external view returns (uint256) {
        return ILandEthPriceOracle(_called).LandIndexTokenPerEth();
    }
}
