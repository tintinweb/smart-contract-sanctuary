...
// File: contracts/NewRocket.sol


pragma solidity ^0.8.0;


contract NewRocket is ERC721, Ownable, RandomlyAssigned {
  using Strings for uint256;
  using AddrArrayLib for AddrArrayLib.Addresses;

  
  // List of whitelisted addresses which can mint tokens
  AddrArrayLib.Addresses whitelistedUsers;
  ...
  uint256 public maxMintPerTransction = 2;
  uint256 public nftLimitPerAddress = 5;
  ...
  ) ERC721("NewRocket", "NROCKET") RandomlyAssigned(50,1) {
    mint(2);
  ...
  function mint (uint256 _mintAmount) public payable {
      require(tokenCount() + _mintAmount <= totalSupply(), "YOU CAN'T MINT MORE THAN MAXIMUM SUPPLY");
      ...
      require(_mintAmount > 0, "MINTING AMOUNT SHOULD BE GREATER THAN 0");
      
    if (msg.sender != owner()) {  
        require(!paused, "MINTING IS PAUSED");
        require(_mintAmount <= maxMintPerTransction, "YOU CAN'T MINT MORE THAN MAX AMOUNT PER TRANSCTION");
  ...
  function whitelistUser(address _user) public onlyOwner {
     whitelistedUsers.pushAddress(_user);
  }
 
  function removeWhitelistUser(address _user) public onlyOwner {
     whitelistedUsers.removeAddress(_user);
  }
  ...
  function reducePrice() public onlyOwner() {
     require((cost - 0.0001 ether) >= minCost, "YOU CAN'T REDUCE THE PRICE ANY MORE");
     cost -= 0.0001 ether;
  }

  function setMaxMintPerTransction(uint256 _amoount) public onlyOwner {
    maxMintPerTransction = _amoount;
  }
  ...
  function whitelistUsers(address[] calldata _users) public onlyOwner {
    ...
    for(uint256 i = 0; i < _users.length; i++) {
        whitelistedUsers.pushAddress(_users[i]);
    }
  }
    
  function isWhitelisted(address _user) public view returns (bool) {
    for(uint256 i = 0; i < whitelistedUsers.size(); i++) {
       if (whitelistedUsers.exists(_user)) {
    	  return true;
        }
    }
