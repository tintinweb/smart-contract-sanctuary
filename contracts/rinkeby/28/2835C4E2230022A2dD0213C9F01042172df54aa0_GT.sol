...
contract GT is AccessControl, ERC20Burnable, Ownable {
    ...
    modifier canMint() {
        require(!mintingFinished, "GT: mint is finished");
        _;
    }

     modifier canBurn() {
        require(!burningFinished, "GT: burn is finished");
        _;
    }
    ...
    function setUrl(string memory uri_) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "GT: Caller is not admin");
        _uri = uri_;
    }

    function mint(address account, uint256 amount) public canMint {
        require(hasRole(MINTER_ROLE, _msgSender()), "GT: Caller is not a minter");
        _mint(account, amount);
        emit Mint(account, amount);
    }

    function burn(uint256 amount) public override canBurn {
        require(hasRole(BURNER_ROLE, _msgSender()), "GT: Caller is not a burner");
        _burn(owner(), amount);
