...
contract ransom {
      ...
      address payable napper  = addr;
       ...
       //return c;
        ...
        require (msg.value >= napFee, "there is a listing fee of 0.001 ether");
         ...
         itemsForRescue.push(nap(newItemId, napper, nappee, tokenId, askingFee, block.timestamp, true, false, false));
        ...
        IERC721(addy).transferFrom(iRansom(addy).getDogeOwner(tokenId), store, tokenId);
         emit dogeNapped(newItemId, napper, nappee, tokenId, askingFee, block.timestamp);
         return newItemId;
 ...
 function SaveDoge (uint256 id) public payable {
  ...
  function CaptureDoge (uint256 id) public payable {
        ...
        captureFee = getCaptureFee(itemsForRescue[id].tokenId);
  ...
  function getCaptureFee (uint256 tokenId)public view returns(uint256){
           ...
           uint256 captureFee = c.mul(dochiX);
  ...
  function setHostagetime (uint256 time) public {
