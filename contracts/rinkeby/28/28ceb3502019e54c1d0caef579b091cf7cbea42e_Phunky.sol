...
contract Phunky is ERC20 {
    ...
    CryptoPhunk private immutable cryptophunk;
    ...
    mapping(uint256 => uint256) private claimedBlock;
    mapping(uint256 => uint256) private claimedPhunky;

    constructor(
        string memory name, 
        string memory symbol,
        address _cryptophunk,
        uint256 _initialBlock,
        uint256 _halfAfterBlock,
        uint256 _perBlockGrant,
        ...
        cryptophunk = CryptoPhunk(_cryptophunk);
        initialBlock = _initialBlock;
        daoTreasury = _daoTreasury;
        creators = _creators;
        halfAfterBlock = _halfAfterBlock;
        perBlockGrant = _perBlockGrant;
    }
    ...
    function claim(
        ...
        require(
            block.number > claimedBlock[phunkId], 
            "wait"
        );
        ...
        claimedBlock[phunkId] = block.number;
        claimedTotal += amount;
