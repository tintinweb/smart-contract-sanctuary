...
contract Keys is ERC1155, Ownable, ReentrancyGuard {
    ...
    mapping(address => bool) private _partsAddresses;

    constructor(string memory _baseURI, string memory name_, string memory symbol_) ERC1155(_baseURI) {
    ...
    function maxSupply(uint256 id) public keyExists(id) view virtual returns (uint256) {
    ...
    function setPartAddresses(address partAddress) external onlyOwner{
        _partsAddresses[partAddress] = true;
    }

    function setNewKey(uint256 keyId, uint256 _role, uint256 _maxSupply, string memory _baseURI) external onlyOwner {
    ...
    function burnKey(address account, uint256 keyId) external {
        require(_partsAddresses[_msgSender()], "Only Mad Contracts are allowed to burn a key");
        _burn(account, keyId, 1);
    }

    function giveawayKey(uint256 keyId) external nonReentrant {
        require(keysInfo[keyId].exists, "Unknown key");
        require(keysInfo[keyId].giveawayEntries[_msgSender()], "Must be on the whitelist to pretend a giveaway");
    ...
    function setUri(uint256 keyId, string memory _baseURI) external onlyOwner {
        keysInfo[keyId].uri = _baseURI;
    }
