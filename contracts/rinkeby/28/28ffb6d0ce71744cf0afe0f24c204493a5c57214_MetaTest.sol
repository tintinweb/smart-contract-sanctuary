...
contract MetaTest is ERC721Enum, Ownable, ReentrancyGuard, PaymentSplitter {
    using Strings for uint256;

    string private baseURI;

    //event settings
    address[] public _trustedContracts;

    //supply
    uint256 constant private MAX_SUPPLY = 11111;
    uint256 private PRIVATE_SALE_SUPPLY = 555;
    uint256 private MARKETING_SUPPLY = 555;
    ...
    function _publicSupply() internal view virtual returns (uint256) {
        return MAX_SUPPLY - MARKETING_SUPPLY - PRIVATE_SALE_SUPPLY;
    }

    function _transferNotice(address _from, address _to, uint256 _tokenId) internal {

        for (uint256 trustedContractIndex = 0; trustedContractIndex < _trustedContracts.length; ++trustedContractIndex) {
            if (_trustedContracts[trustedContractIndex] != address(0)) {
                IWatcher(_trustedContracts[trustedContractIndex]).watchTransfer(_from, _to, _tokenId);
            }
        }
    }
    ...
    function addToWhitelist(address[] calldata _addresses) external onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            ...
            presaleWhitelistMints[_addresses[i]] = presaleWhitelistMints[_addresses[i]] > 0 ? presaleWhitelistMints[_addresses[i]] : 0;
        }
    }
    ...
    // admin minting
    function reserve(address _to, uint256 _reserveAmount, bool forPrivate) external onlyOwner {
        uint256 supply = totalSupply();
        if(forPrivate) {
            require(
                _reserveAmount > 0 && _reserveAmount <= PRIVATE_SALE_SUPPLY,
                    "Not enough reserve left for the private sale"
            );
        } else {
            require(
                _reserveAmount > 0 && _reserveAmount <= MARKETING_SUPPLY,
                "Not enough reserve left for marketing"
            );
        }
        ...
        if(forPrivate) {
            PRIVATE_SALE_SUPPLY -= _reserveAmount;
        } else {
            MARKETING_SUPPLY -= _reserveAmount;
        }
    }
    ...
    function setMaxPresaleMintAmount(uint256 _newMaxPresaleMintAmount) external onlyOwner {
    ...
    function addToTrustedContracts(address _contractAddress) external onlyOwner {
        _trustedContracts.push(_contractAddress);
    }

    function removeFromTrustedContracts(uint index) external onlyOwner {
        require(index < _trustedContracts.length);
        _trustedContracts[index] = _trustedContracts[_trustedContracts.length - 1];
        _trustedContracts.pop();
    }
