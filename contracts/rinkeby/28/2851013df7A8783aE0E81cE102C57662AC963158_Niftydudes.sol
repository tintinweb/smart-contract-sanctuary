...
contract Niftydudes is ERC721, Ownable, Pausable {
    ...
    uint[] private lowerBounds = [710, 694, 689, 529, 275, 168, 146, 100, 651, 549, 422, 367, 218];
    uint[] private upperBounds = [727, 709, 693, 548, 366, 217, 167, 145, 688, 650, 528, 421, 274];
    ...
    function storeNewDude(uint traitCombi) private {
        ...
        if(newDudeId%2==0 && msg.sender != owner()) {
    ...
    function setSkillActivationBlock(uint block) external onlyOwner {
        skillActivationBlock = block;
    }
    ...
    function detachTrait(uint sistaid, uint[] memory traitsToDetach) external returns (bool) {
        ...
        for (uint i=0; i < traitsToDetach.length; i++) {
            ...
            require(traitsToDetach[i] < lowerBounds[0] || traitsToDetach[i] > upperBounds[0]);
        ...
        for (uint i=0; i < traitsToDetach.length; i++) {
            traitsContract.mint(traitsToDetach[i], msg.sender);
        }
    ...
    function assignDetachedTrait(uint sistaid, uint[] memory traitid, uint[] memory positionAfter) external returns (uint) {
        ...
        for (uint k=0; k < traitid.length; k++) {
            ...
            while(true) {
                ...
                ((temp >= lower && temp <= upper && !singleTrait) && (positionAfter[k] == 0 || temp == positionAfter[k]))) {
