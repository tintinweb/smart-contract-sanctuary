...
//    mapping(uint256 => Trait[]) public traitTypes;
    ...
    // TODO: need to flip so most rare ones goes to the end, hence the mutation pool is at the end of this table. 

    constructor() ERC721("cMyZombie", "ZOMBIE") {
        _owner = msg.sender;

        //Declare all the rarity tiers

        // TODO: szinkronba hozni a graveyard es zombi traitek sorrendjet
        ...
        //Hat
        TIERS[0] = [100, 200, 800, 800, 1200, 2000, 4900];
        //whiskerss
 ...
 |_|  |_|_|_||_\__|_|_||_\__, | |_| \_,_|_||_\__|\__|_\___/_||_/__/
   ...
   */
    ...
    function hash(
        ...
        //The last 8 digits are random, the first is 0, due to the cat not being burned.
        string memory currentHash = "0";
        ...
        for (uint8 i = 0; i < 8; i++) {
            ...
            currentHash = string(
                abi.encodePacked(
                    currentHash,
                    ZombieCatsLibrary.rarityGen(_randinput, i, TIERS)
                )
            );
        }
    ...
    // FIXME: change to final price
    /**
     * @dev Returns the current miece cost of mint.
     */
    function currentMutateCost() public pure returns (uint256)  {
        return 2500000000000000000; // 2.5 MIECE 
    }
    ...
    function mintZombie(address _account, string memory _hash) public {
        require(msg.sender == graveyardAddress, "Only the graveyard contract can call this function");

        mintInternal(_account, _hash);        
    }
    ...
    function mintReserve(uint256 _times) public onlyOwner {
        for(uint256 i=0; i< _times; i++){
            ...
            mintInternal(msg.sender,  hash(_totalSupply, msg.sender, 0) );
        }        
    }
    ...
    function mutateZombie(uint256 _tokenId) public {
        require(ownerOf(_tokenId) == msg.sender, "You must own this zombie to mutate");

        IMiece(mieceAddress).burnFrom(msg.sender, currentMutateCost());
    ...
    function burnZombie(uint256 _tokenId) public {
        uint256 _totalSupply = totalSupply();
        require(ownerOf(_tokenId) == msg.sender, "You must own this zombie to burn");
...
|     | \      / |  |  ||     ||  .  \    |  |   |     ||  |  \     | |  |   |  ||     ||  |  |\    |
 \___/   \_/\_/  |__|__||_____||__|\_|    |__|    \__,_||__|__|\____| |__|  |____|\___/ |__|__| \___|
    ...
    */
    ...
    /**
     * @dev Sets the cats adddress
     * @param _catsAddress The cats address
     */
    ...
    /**
     * @dev Sets the graveyards address
     * @param _graveyardAddress The graveyard address
     */

    function setGraveyardAddress(address _graveyardAddress) public onlyOwner {
