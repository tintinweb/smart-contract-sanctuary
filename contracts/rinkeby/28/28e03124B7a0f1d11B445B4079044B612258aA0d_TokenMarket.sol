    ...
    */
     function createLoan(TokenLoanData.LoanDetails memory loanDetails)
        ...
        for(uint256 i  = 0 ; i  <  loanDetails.stakedCollateralTokens.length ; i++){
            require(IGovWorldProtocolRegistry(govWorldProtocolRegistry).isTokenApproved(loanDetails.stakedCollateralTokens[i]),"GLM: One or more tokens not approved.");
    ...
    function loanAdjusted(
        ...
        for(uint256 index =  0 ; index < loanOffersToken[_loanIdAdjusted].stakedCollateralAmounts.length ; index ++){
            collatetralInBorrowed = collatetralInBorrowed.add(IGovPriceConsumer(govPriceConsumer).getAltCoinPriceinStable(loanOffersToken[_loanIdAdjusted].borrowStableCoin, loanOffersToken[_loanIdAdjusted].stakedCollateralTokens[index], loanOffersToken[_loanIdAdjusted].stakedCollateralAmounts[index]));
        }

        uint256 ltv  = IGovPriceConsumer(govPriceConsumer).calculateLTV(loanOffersToken[_loanIdAdjusted].stakedCollateralAmounts, loanOffersToken[_loanIdAdjusted].stakedCollateralTokens, loanOffersToken[_loanIdAdjusted].borrowStableCoin, _newLoanAmountBorrowed);
        uint256 maxLtv = IGovWorldTierLevel(govWorldTierLevel).getMaxLtv(collatetralInBorrowed, msg.sender);
        ...
        loanOffersToken[_loanIdAdjusted] = TokenLoanData.LoanDetails(
            ...
            loanOffersToken[_loanIdAdjusted].isMintSp
    ...
    function activateLoan(uint256[] memory loanIds, uint256[] memory stableCoinAmounts, bool[] memory _autoSell) public {
        ...
        for(uint256 k  = 0 ; k  <  loanOffersToken[loanIds[i]].stakedCollateralTokens.length; k++){
            uint256 allowance =  IERC20(loanOffersToken[loanIds[i]].stakedCollateralTokens[k]).allowance(loanOffersToken[loanIds[i]].borrower, govWorldLiquidator); //TODO change approval address in testing file
        ...
        //TODO below line, admin fee will be removed asked by client 
