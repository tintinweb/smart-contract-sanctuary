...
contract PerpLib {
	...
	function getSlot(uint strike) external returns (uint) {
	...
	function getStrike(uint ident) external returns (uint) {
	...
	function getTwapOI(uint timeElapsed, uint prevDecayTwapOI, uint oiDecayPerSecond, uint currentOI) external returns (uint) {
		uint total = 1e18;
