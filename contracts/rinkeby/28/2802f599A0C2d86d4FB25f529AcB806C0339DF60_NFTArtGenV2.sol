...
contract NFTArtGenV2 is ERC1155, Ownable {
    string public name;
    ...
    function _mint(address to, uint32 count) internal {
        ...
        require(position + count <= size, "Mint sold out");
        ...
        onlyOwner
    {
        for (uint32 i = 0; i < to.length; i++) {
            require(amountToMint[i] + position <= size, "Limit reached");
            position = position + amountToMint[i];
