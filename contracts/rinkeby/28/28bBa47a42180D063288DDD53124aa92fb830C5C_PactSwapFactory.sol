...
contract PactSwapERC20 is IUniswapV2ERC20 {
...
import './PactSwapPair.sol';
import "../vendors/contracts/access/GovernanceOwnable.sol";
import "../vendors/contracts/access/RequiredTokenValidator.sol";
import '../vendors/interfaces/IUniswapV2Factory.sol';
import "../vendors/interfaces/IWithIncentivesPool.sol";

contract PactSwapFactory is IUniswapV2Factory, IWithIncentivesPool, GovernanceOwnable, RequiredTokenValidator {
    ...
    constructor(address pact_, address governor_, address incentivesPool_) GovernanceOwnable(governor_) RequiredTokenValidator(pact_) public {
    ...
    function pairCodeHash() external pure returns (bytes32) {
        return keccak256(type(PactSwapPair).creationCode);
    }

    function createPair(address tokenA, address tokenB)
        external override onlyGovernance() correctPairByTokens(tokenA, tokenB)
        returns (address pair)
    {
        ...
        bytes memory bytecode = type(PactSwapPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        PactSwapPair(pair).initialize(token0, token1);
...
contract PactSwapPair is IUniswapV2Pair, PactSwapERC20 {
    using SafeMath  for uint;
...
import "../../interfaces/IRequiredTokenValidator.sol";

abstract contract RequiredTokenValidator is IRequiredTokenValidator {
    address private _requiredTokenAddress;

    constructor (address requiredToken_) public {
        _requiredTokenAddress = requiredToken_;
    }

    function requiredToken() public view override returns (address) {
        return _requiredTokenAddress;
    }

    modifier correctPairByTokens(address tokenA, address tokenB) {
        require(
            _requiredTokenAddress == tokenA || _requiredTokenAddress == tokenB,
            "PairWithRequiredTokenValidator::correctPairByTokens - incorrect tokens"
        );
        _;
    }
}
...
import "./IRequiredTokenValidator.sol";
import "./IWithIncentivesPool.sol";

interface IPactSwapFactory is IUniswapV2Factory, IWithIncentivesPool, IGovernanceOwnable, IRequiredTokenValidator {
}
...
interface IRequiredTokenValidator {
