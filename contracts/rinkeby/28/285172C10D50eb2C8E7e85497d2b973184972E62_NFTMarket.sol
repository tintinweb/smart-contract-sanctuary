...
contract NFTMarket is ReentrancyGuard{
    ...
    uint256 private m_nListingPrice = 100000 * 10**9; //
    ...
    function buyingResellMarketFee(uint256 itemId) public view returns(uint256) {
        uint256 price = idToMarketItem[itemId].price;
        uint256 market_fee = price * 10 / 100; // 10%
        return market_fee;
    }

    function buyingResellCreatorFee(uint256 itemId) public view returns(uint256) {
        uint256 price = idToMarketItem[itemId].price;
        uint256 creator_fee = price * 5 / 100; // 5%
        return creator_fee;
    }

    function buyingResellPrice(uint256 itemId) public view returns(uint256) {
        uint256 price = idToMarketItem[itemId].price;
        return price - buyingResellMarketFee(itemId) - buyingResellCreatorFee(itemId);
    }
