...
contract MrEverybody is ERC721, ERC721Enumerable,ERC721URIStorage, Ownable {
   using Counters for Counters.Counter;
    ...
    uint256 public _next48hour = 600;           // 172800 48 hours
    uint256 public _next24hour = 300;           // 86400 24 hour
    
    uint256 public _privateSalePrice =  1E15;   // 6E16 0.06 ETH per NFT
    uint256 public _publicSalePrice =  2E15;   //8E16 0.08 ETH per NFT
    ...
    function mint(uint8 _quantityToMint) public payable {
       ...
       require(_quantityToMint <= 50, 'can mint only 5 at a time');
       ...
       require(sale_started == true,"Sale Not Started");
        ...
        if(block.timestamp > (_startDate + _next48hour + _next24hour)) {
             require(msg.value == (_publicSalePrice * _quantityToMint),"Ether submitted does not match current price 0.08 ETH per token");
             for (uint8 i = 0; i < _quantityToMint; i++) {
             _tokenIds.increment();
             uint256 newItemId = _tokenIds.current();
             emit ReturnEvent(newItemId);
