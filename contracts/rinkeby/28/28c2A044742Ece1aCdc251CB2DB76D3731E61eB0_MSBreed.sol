...
import "./BreedProxyAdmin.sol";

contract MSBreed is BreedProxyAdmin {
    ...
    function getServerAddress() private view onlyCreator returns (address) {
    ...
    function checkBreed(
        ...
        require(maxBreedLimit > 0 ,"MSBreed: maxBreedLimit is not set");
        require(serverAddress != address(0), "MSBreed: serverAddress is not set");
    ...
    function chargeTokens(address _nftContract, uint256 tokenQuantity)
        internal
    {
        require(charges[_nftContract].length > 0, "MSBreed: Charges not set");
        for (uint256 i = 0; i < charges[_nftContract].length; i++) {
            if (charges[_nftContract][i].token == address(0)) {
                ...
                require(
                    IERC20Upgradeable(charges[_nftContract][i].token).allowance(
                        _msgSender(),
                        address(this)
                    ) >= charges[_nftContract][i].price * tokenQuantity,
                    "MSBreed: Approved insufficient ERC20 tokens"
                );
                IERC20Upgradeable(charges[_nftContract][i].token).safeTransferFrom(
                    _msgSender(),
                    address(this),
                    charges[_nftContract][i].price * tokenQuantity
                );
