...
contract CapsToken is ERC20("CAPS", "$CAPS") {
    using SafeMath for uint256;
    ...
    ICaps public OGMilkCaps;

    event RewardPaid(address indexed user, uint256 reward);

    constructor(address _milkcaps) public{
        OGMilkCaps = ICaps(_milkcaps);
    }
    ...
    function updateRewardOnMint(address _user, uint256 _amount) external {
        require(msg.sender == address(OGMilkCaps), "Can't call this");
        uint256 time = min(block.timestamp, END);
        uint256 timerUser = lastUpdate[_user];
        if (timerUser > 0)
            rewards[_user] = rewards[_user].add(OGMilkCaps.balanceOG(_user).mul(BASE_RATE.mul((time.sub(timerUser)))).div(60)
    ...
    function updateReward(address _from, address _to, uint256 _tokenId) external {
        require(msg.sender == address(OGMilkCaps));
        if (_tokenId < 10001) {
            uint256 time = min(block.timestamp, END);
            uint256 timerFrom = lastUpdate[_from];
            if (timerFrom > 0)
                rewards[_from] += OGMilkCaps.balanceOG(_from).mul(BASE_RATE.mul((time.sub(timerFrom)))).div(60);
            if (timerFrom != END)
                lastUpdate[_from] = time;
            if (_to != address(0)) {
                uint256 timerTo = lastUpdate[_to];
                if (timerTo > 0)
                    rewards[_to] += OGMilkCaps.balanceOG(_to).mul(BASE_RATE.mul((time.sub(timerTo)))).div(60);
                if (timerTo != END)
                    lastUpdate[_to] = time;
            }
        }
    ...
    function getTotalClaimable(address _user) external view returns(uint256) {
        uint256 time = min(block.timestamp, END);
        uint256 pending = OGMilkCaps.balanceOG(_user).mul(BASE_RATE.mul((time.sub(lastUpdate[_user])))).div(60);
...
/**
 * @title OGMilkCaps contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract OGMilkCaps is ERC721, Ownable {
    using SafeMath for uint256;

    uint256 public PogzPrice = .4 ether;
    uint256 public MAX_POGZ = 7094;
    ...
    mapping(address => uint) public freePogzCount;
    ...
    mapping(uint256 => OGMilkCaps) public ogmilkcaps;
    mapping(address => uint256) public balanceOG;

    CapsToken public capsToken;

    constructor() ERC721("OGMilkCaps", "OGMILKCAPS") {
    ...
    function reservePogzForTeam(uint amount, address to) public onlyOwner {
    ...
    function reservePogzForMarketing(uint amount, address to) public onlyOwner {
    ...
    function setCapsToken(address _caps) external onlyOwner {
        capsToken = CapsToken(_caps);
    }

    function mintPogz(uint numberOfTokens) public payable {
        ...
        require(totalSupply().add(numberOfTokens) <= MAX_POGZ, "Purchase would exceed max supply of pogz");
        require(PogzPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");


        for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply();
            if (totalSupply() < MAX_POGZ) {
                _safeMint(msg.sender, mintIndex);
                capsToken.updateRewardOnMint(msg.sender, 1);
    ...
    function getReward() external {
        capsToken.updateReward(msg.sender, address(0), 0);
        capsToken.getReward(msg.sender);
    }

    function transferFrom(address from, address to, uint256 tokenId) public override {
        capsToken.updateReward(from, to, tokenId);
        if (tokenId < 10001)
        {
            balanceOG[from]--;
            balanceOG[to]++;
