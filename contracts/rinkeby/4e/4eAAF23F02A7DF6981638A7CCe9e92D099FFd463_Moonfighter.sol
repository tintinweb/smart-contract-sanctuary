...
contract Moonfighter is ERC721Enumerable, Ownable {
    ...
    uint256 public constant MAXGASMASK = 5000;
    uint256 public constant freeMints = 20;
    uint256 public constant maxFreeMintsPerWallet = 10;
    uint256 public reservedGasmasks = 110;
    ...
    mapping (address => bool) private freeMintsCyberKongz;
    ...
    function mintNFT(uint256 numberOfGasmasks) public payable {
        ...
        require(totalSupply().add(numberOfGasmasks) <= MAXGASMASK - reservedGasmasks,
            "Hold up! You would buy more Gasmasks than available...");

        if(totalSupply() >= maxFreeMintsPerWallet){
        ...
        for (uint256 i = 0; i < numberOfGasmasks; i++){
            ...
            if (totalSupply() < MAXGASMASK - reservedGasmasks) {
    ...
    function mintCyberkongzs() public {
        ...
        require(totalSupply().add(1) <= MAXGASMASK - reservedGasmasks, 
            "Hold up! You would buy more Gasmasks than available...");
    ...
    function giveAway(address _to, uint256 _amount) external onlyOwner() {
        require( _amount <= reservedGasmasks, "Exceeds reserved Gasmask supply" );
