...
contract ZOMZOMS is ERC721Enumerable, Ownable {
    ...
    uint256 private presaleprice = 0.015 ether;
    ...
    function Mint(uint256 num) public payable {
        ...
        require( num < 11,                              "You can adopt a maximum of 10" );
        require( supply + num <= 6666 - _reserved,      "Exceeds maximum supply" );
    ...
    function whiteListMint() public {
        //MINT FUNCTION FOR whitelisted USERS
        uint256 supply = totalSupply();
    ...
    function MintZom(address _to, uint256 _amount) external onlyOwner() {
    ...
    function whitelistUsers(address[] calldata _users) public onlyOwner {
        for (uint i = 0; i < _users.length; i++) {
            if (isWhitelisted(_users[i])==false) {
    ...
    function presaleUsers(address[] calldata _users) public onlyOwner {
        for (uint i = 0; i < _users.length; i++) {
            if (isPreListed(_users[i])==false) {
                presaleAddresses.push(_users[i]);
                presaleAddresses.push(_users[i]);
                //LIST IS STORED TWICE SO THAT USER CAN MINT 2 times
            }
    ...
    function withdrawAll() public onlyOwner {
        address t1 = 0x5E0ec4b465656391a947B6C51F725dcc1F57C340;
