...
contract LAZYDOG is ERC721Enumerable, Ownable {
    uint public constant MAX_DOG = 10000;
	string _baseTokenURI;
	bool public paused = true;


    // Truth.　
    constructor(string memory baseURI) ERC721("LAZYDOG", "LZD")  {
        setBaseURI(baseURI);
    }

    modifier saleIsOpen{
        require(totalSupply() < MAX_DOG, "Sale end");
        _;
    }

    function mintMyDog(address _to, uint _count) public payable saleIsOpen {
        if(msg.sender != owner()){
            require(!paused, "Pause");
        }
        require(totalSupply() + _count <= MAX_DOG, "Max limit");
        require(totalSupply() < MAX_DOG, "Sale end");
