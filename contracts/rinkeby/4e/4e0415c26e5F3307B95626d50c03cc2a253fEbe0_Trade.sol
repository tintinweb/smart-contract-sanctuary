...
contract Trade {
    ...
    event ServiceFeeEnabledUpdated(bool enabled, uint8 secondarySellerFee, uint8 secondaryBuyerFee);
    ...
    function getFees(uint paymentAmt, BuyingAssetType buyingAssetType, address buyingAssetAddress, uint tokenId) internal returns(Fee memory){
        address tokenCreator;
        uint platformFee;
        uint royaltyFee;
        uint assetFee;
        uint royaltyPermille;
        if(_exculdedFromFee[buyingAssetAddress][tokenId] && !serviceFee) {
            removeAllFee(false);
        }
        else if(_exculdedFromFee[buyingAssetAddress][tokenId] && serviceFee) {
            removeAllFee(true);
        }
    ...
    function removeAllFee(bool _secondaryFee) private {
       
        if(_secondaryFee) {
             if(sellerFeePermille == secondarySellerFee && buyerFeePermille == secondaryBuyerFee) return;
            ...
            if(sellerFeePermille == 0 && buyerFeePermille == 0) return;
    ...
    function setServiceFeeEnabled (bool _enabled, uint8 _buyerfee, uint8 _sellerFee) public onlyOwner returns(bool) {
        serviceFee = _enabled;
        secondaryBuyerFee = _buyerfee;
        secondarySellerFee = _sellerFee;
        emit ServiceFeeEnabledUpdated(_enabled, secondarySellerFee, secondaryBuyerFee);
        return true;
    }
}