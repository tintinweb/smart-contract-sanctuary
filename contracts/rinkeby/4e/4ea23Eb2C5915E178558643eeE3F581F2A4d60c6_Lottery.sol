...
contract Lottery is VRFConsumerBase, Ownable {
    ...
    function participate() public payable {
        //participate to the lottery with min entrance fee
        require(lottery_state == LOTTERY_STATE.OPEN, 'Lottery not Open');
        uint256 value = msg.value;
        require(value >= getFee(), "Entrance fee below the requirement (50$)!");
    ...
    function activateLottery(bool _active) public view returns(bool){
        //admin active or desactivate the lottery
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        require(lottery_state == LOTTERY_STATE.CALCULATION_WINNER, "Lottery state is not 'CALCULATION_WINNER'");
        require(randomness>0, "Random number not found");
        uint256 randomindex = randomness % players.length;
        recentWinner  = players[randomindex];
        recentWinner.transfer(address(this).balance);
        // Reset
        players = new address payable[](0);

        // can be deleted, to view the random number 
        randomNumber = randomness;
    }

    function withdraw() public payable{
        //admin can withdraw fund
    }
}