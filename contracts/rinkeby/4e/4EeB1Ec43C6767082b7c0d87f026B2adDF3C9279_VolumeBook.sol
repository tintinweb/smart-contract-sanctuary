...
interface ILottery {
      ...
      function claimOnce(address user, uint256 cycleNum) external;
}
...
// File contracts/volumeMining/VolumeBook.sol
...
contract VolumeBook is Ownable, IVolumeBook {
    ...
    address public router; // Use for recording volume only
    address public updater; // Use for updating cycleNum only
    ...
    function addCycleNum() override external {
        ...
        currentCycleNum = currentCycleNum.add(1);
        lastUpdateTime = block.timestamp;
    }
    ...
    function _addVolume(address user, uint256 volume) internal {
        ...
        if (address(lottery) != address(0)) {
            lottery.addSlotVolume(user, volume);
        }
