...
contract CircleGame is ERC1155, Ownable, ERC1155Burnable, ERC1155Supply {
    ...
    uint256 endingPotBalance; //total ending pot balance
    uint256 endingClaimablePotBalance; //total ending pot balance * 90%, used in claiming formula
    uint256 adjustedTokenTotal; //total supply of all circles when game ends, adjusted for tier multipliers
    ...
    function endGame() public {
        ...
        adjustedTokenTotal = 
            totalSupply(0) + 
            totalSupply(1).div(10).mul(11) + 
            totalSupply(2).div(10).mul(12) + 
            totalSupply(3).div(10).mul(13) + 
            totalSupply(4).div(10).mul(14) + 
            totalSupply(5).div(10).mul(15);
    ...
    function claimStake(uint[] memory numberOfTokens) public {
        ...
        for (uint i = 0; i < 6; i++) {
            ...
            playerAdjustedTokens.add(numberOfTokens[i].div(10).mul(10 + i));
            
            _burn(msg.sender, i, numberOfTokens[i]);
        }
        
        uint reward = playerAdjustedTokens.div(adjustedTokenTotal).mul(endingClaimablePotBalance);
