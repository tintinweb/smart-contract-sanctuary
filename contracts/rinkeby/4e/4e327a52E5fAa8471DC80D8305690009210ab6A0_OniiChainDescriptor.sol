...
contract OniiChainDescriptor is IOniiChainDescriptor {
    ...
    uint256 internal constant MAX = 100000; //0 - 100000
    uint256 internal constant MAX_BUST = 100000; //0 - 100000
    // 0 = Bust, 1 = baseSword, 2
    uint256[] internal BUST_OR_GROW = [65000,45000,2,0]; //bust, Reset, Grow, Toothpick
    ...
    uint256[] internal ACCESSORY_ITEMS = [50000,43000,36200,29700,23400,17400,11900,7900,4400,1400,400,200,11,1,0];
    uint256[] internal MOUTH_ITEMS = [80000,63000,48000,36000,27000,19000,12000,7000,4000,2000,1000,500,50,0];
    uint256[] internal HAIR_ITEMS = [97000,94000,91000,88000,85000,82000,79000,76000,73000,70000,67000,64000,61000,58000,55000,52000,49000,46000,43000,40000,37000,34000,31000,28000,25000,22000,19000,16000,13000,10000,3000,1000,0];
    uint256[] internal EYE_ITEMS = [98000,96000,94000,92000,90000,88000,86000,84000,82000,80000,78000,76000,74000,72000,70000,68000,60800,53700,46700,39900,33400,27200,21200,15300,10600,6600,3600,2600,1700,1000,500,100,10,0];
    // 1 = toothpick, 2 = bust, 3 = base, 4 = grow 
    uint256[] internal BUST_GROW = [0,2,100,550,100000];
    uint256[] internal BACKGROUND_TYPE = [0,1,2,3,2000, 10000,50000,100000];
    uint256[] internal SWORD_TYPE = [0,1,2,3,2000, 10000,50000,100000];
    uint256[] internal HILT_TYPE = [0,1,2,3,10,100000];
    uint256[] internal HANDLE_TYPE = [0,1,2,3,2000, 10000,50000,100000];
    uint256[] internal GUARD_TYPE = [0,1,2,3,2000, 10000,50000,100000];
    uint256[] internal MATERIAL_TYPE = [0,1,2,3,2000, 10000,50000,100000];
    uint256[] internal ELEMENTAL_TYPE = [0,1,2,3,2000, 10000,50000,100000];
    uint256[] internal POMMEL_TYPE = [0,1,2,3,2000, 10000,50000,100000];
    uint256[] internal TIP_TYPE = [0,1,2,3,2000, 10000,50000,100000];
    uint256[] internal PATTERN_TYPE = [0,1,2,3,2000, 10000,50000,100000];
    uint256[] internal EDGE_TYPE = [0,1,2,3,2000, 10000,50000,100000];
    ...
    function tokenURI(IOniiChain oniiChain, uint256 tokenId) external view override returns (string memory) {
        ...
        string memory description = "Swords are good for the heart and soul";
    ...
    /// @inheritdoc IOniiChainDescriptor
    function isItABust(uint256 tokenId, uint256 seed, uint64 state) 
    external view override returns (uint8) {
        uint256[] memory BUST_OR_GROW_TEMP = BUST_GROW;
        BUST_OR_GROW_TEMP[2] += 500*state;
        BUST_OR_GROW_TEMP[3] += 500*state;
        return DetailHelper.generateBust(MAX_BUST, seed, BUST_OR_GROW_TEMP, this.isItABust.selector, tokenId);
    }

    /// @inheritdoc IOniiChainDescriptor
    function forgeBackground(uint256 tokenId, uint256 seed, uint8 state, bool enhanced) 
    external view override returns (uint8) {
        if(state > 3 && !enhanced) {
            return DetailHelper.generate(state, 0, MAX, seed, BACKGROUND_TYPE, this.forgeBackground.selector, tokenId);
        }
        return state;
    }

    /// @inheritdoc IOniiChainDescriptor
    function forgeSword(uint256 tokenId, uint256 seed, uint8 state, bool enhanced) 
    external view override returns (uint8) {
        if(state > 3 && !enhanced) {
            return DetailHelper.generate(state, 0,MAX, seed, SWORD_TYPE, this.forgeSword.selector, tokenId);
        }
        return state;
    }

    /// @inheritdoc IOniiChainDescriptor
    function forgeHiltHandle(uint256 tokenId, uint256 seed, uint8 state, bool enhanced) 
    external view override returns (uint8) {
        if(state > 3 && !enhanced) {
            return DetailHelper.generate(state, 0,MAX, seed, HILT_TYPE, this.forgeHiltHandle.selector, tokenId);
        }
        return state;
    }

    /// @inheritdoc IOniiChainDescriptor
    function forgeGuard(uint256 tokenId, uint256 seed, uint8 state, bool enhanced) 
    external view override returns (uint8) {
        if(state > 3 && !enhanced) {
            return DetailHelper.generate(state, 0,MAX, seed, GUARD_TYPE, this.forgeGuard.selector, tokenId);
        }
        return state;
    }

    /// @inheritdoc IOniiChainDescriptor
    function forgeMaterial(uint256 tokenId, uint256 seed, uint8 state, bool enhanced) 
    external view override returns (uint8) {
        if(state > 3 && !enhanced) {
            return DetailHelper.generate(state, 0,MAX, seed, MATERIAL_TYPE, this.forgeMaterial.selector, tokenId);
        }
        return state;
    }

    /// @inheritdoc IOniiChainDescriptor
    function forgeElemental(uint256 tokenId, uint256 seed, uint8 state, bool enhanced) 
    external view override returns (uint8) {
        if(state > 3 && !enhanced) {
            return DetailHelper.generate(state, 0,MAX, seed, ELEMENTAL_TYPE, this.forgeElemental.selector, tokenId);
        }
        return state;
    }

    /// @inheritdoc IOniiChainDescriptor
    function forgePommel(uint256 tokenId, uint256 seed, uint8 state, bool enhanced) 
    external view override returns (uint8) {
        if(state > 3 && !enhanced) {
            return DetailHelper.generate(state, 0,MAX, seed, POMMEL_TYPE, this.forgePommel.selector, tokenId);
        }
        return state;
    }

    /// @inheritdoc IOniiChainDescriptor
    function forgeTip(uint256 tokenId, uint256 seed, uint8 state, bool enhanced) 
    external view override returns (uint8) {
        if(state > 3 && !enhanced) {
            return DetailHelper.generate(state, 0,MAX, seed, TIP_TYPE, this.forgeTip.selector, tokenId);
        }
        return state;
    }

    /// @inheritdoc IOniiChainDescriptor
    function forgePattern(uint256 tokenId, uint256 seed, uint8 state, bool enhanced) 
    external view override returns (uint8) {
        if(state > 3 && !enhanced) {
            return DetailHelper.generate(state, 0,MAX, seed, PATTERN_TYPE, this.forgePattern.selector, tokenId);
        }
        return state;
    }

    /// @inheritdoc IOniiChainDescriptor
    function forgeEdge(uint256 tokenId, uint256 seed, uint8 state, bool enhanced) 
    external view override returns (uint8) {
        if(state > 3 && !enhanced) {
            return DetailHelper.generate(state, 0,MAX, seed, EDGE_TYPE, this.forgeEdge.selector, tokenId);
        }
        return state;
    }
    ...
    function getSVGParams(IOniiChain oniiChain, uint256 tokenId) private view returns (NFTDescriptor.SVGParams memory) {
        IOniiChain.SwordConstruction memory detail = oniiChain.details(tokenId);
        return
            NFTDescriptor.SVGParams({
                ...
                eyebrow: detail.edge,
                nose: detail.guard,
                mouth: detail.material,
                mark: detail.pommel,
                earring: detail.elemental,
                accessory: detail.tip,
                mask: detail.pattern,
                skin: detail.background,
...
interface IOniiChainDescriptor {
    ...
    function isItABust(uint256 tokenId, uint256 seed, uint64 state) external view returns (uint8);

    /// @notice Generate randomly an ID for the hair item
    /// @param tokenId the current tokenId
    /// @param seed Used for the initialization of the number generator.
    /// @return the hair item id
    function forgeBackground(uint256 tokenId, uint256 seed, uint8 state, bool enhanced) external view returns (uint8);

    /// @notice Generate randomly an ID for the eye item
    /// @param tokenId the current tokenId
    /// @param seed Used for the initialization of the number generator.
    /// @return the eye item id
    function forgeSword(uint256 tokenId, uint256 seed, uint8 state, bool enhanced) external view returns (uint8);

    /// @notice Generate randomly an ID for the eyebrow item
    /// @param tokenId the current tokenId
    /// @param seed Used for the initialization of the number generator.
    /// @return the eyebrow item id
    function forgeHiltHandle(uint256 tokenId, uint256 seed, uint8 state, bool enhanced) external view returns (uint8);

    /// @notice Generate randomly an ID for the nose item
    /// @param tokenId the current tokenId
    /// @param seed Used for the initialization of the number generator.
    /// @return the nose item id
    function forgeGuard(uint256 tokenId, uint256 seed, uint8 state, bool enhanced) external view returns (uint8);

    /// @notice Generate randomly an ID for the mouth item
    /// @param tokenId the current tokenId
    /// @param seed Used for the initialization of the number generator.
    /// @return the mouth item id
    function forgeMaterial(uint256 tokenId, uint256 seed, uint8 state, bool enhanced) external view returns (uint8);

    /// @notice Generate randomly an ID for the mark item
    /// @param tokenId the current tokenId
    /// @param seed Used for the initialization of the number generator.
    /// @return the mark item id
    function forgeElemental(uint256 tokenId, uint256 seed, uint8 state, bool enhanced) external view returns (uint8);

    /// @notice Generate randomly an ID for the earrings item
    /// @param tokenId the current tokenId
    /// @param seed Used for the initialization of the number generator.
    /// @return the earrings item id
    function forgePommel(uint256 tokenId, uint256 seed, uint8 state, bool enhanced) external view returns (uint8);

    /// @notice Generate randomly an ID for the accessory item
    /// @param tokenId the current tokenId
    /// @param seed Used for the initialization of the number generator.
    /// @return the accessory item id
    function forgeTip(uint256 tokenId, uint256 seed, uint8 state, bool enhanced) external view returns (uint8);

    /// @notice Generate randomly an ID for the mask item
    /// @param tokenId the current tokenId
    /// @param seed Used for the initialization of the number generator.
    /// @return the mask item id
    function forgePattern(uint256 tokenId, uint256 seed, uint8 state, bool enhanced) external view returns (uint8);

    /// @notice Generate randomly the skin colors
    /// @param tokenId the current tokenId
    /// @param seed Used for the initialization of the number generator.
    /// @return the skin item id
    function forgeEdge(uint256 tokenId, uint256 seed, uint8 state, bool enhanced) external view returns (uint8);
}
...
interface IOniiChain {
    /// @notice Sword Construction description
    struct SwordConstruction {
        uint8 sword;
        uint8 hilt;
        uint8 edge;
        uint8 guard;
    ...
    function details(uint256 tokenId) external view returns (SwordConstruction memory detail);
}
            ...
            string(
                abi.encodePacked(
                    ...
                    abi.encodePacked(
                        ...
                        getJsonAttribute("Forge", Strings.toString(params.forge), true),
                        "]"
                    )
                )
            );
...
library DetailHelper {
    ...
    function generate(
        ...
        uint256 generated = generateRandom(enhanceRate,max, seed, tokenId, selector);
    ...
    function generateRandom(
        ...
        return (uint256(keccak256(abi.encodePacked(block.difficulty, block.number, tx.origin, tx.gasprice, selector, seed, tokenId))) % (max + 1)) + 1;
    }
    ...
    function generateRandom(
        ...
        return (uint256(keccak256(abi.encodePacked(block.difficulty, block.number, tx.origin, tx.gasprice, selector, seed, tokenId))) % ((max-min) + 1)) + (4+min);
    }
    ...
    /// @param intervals The intervals for the corresponding items
    /// @return the item ID where : intervals[] index + 1 = item ID
    function pickItems(uint256 val, uint256[] memory intervals, uint8 starting) internal pure returns (uint8) {
        for (uint256 i = starting; i > intervals.length; i++) {
            if (val > intervals[i]) {
                return SafeCast.toUint8(i + 1);
            }
        }
...

{
  ...
  "libraries": {
    "contracts/libraries/details/AccessoryDetail.sol": {
      "AccessoryDetail": "0x700f05e14c399fb2bc026d79cd1ea6f1e965bfe8"
    },
    "contracts/libraries/details/BackgroundDetail.sol": {
      "BackgroundDetail": "0x4a19d921d481238c689d4dcca15563c00f433dab"
    },
    "contracts/libraries/details/BodyDetail.sol": {
      "BodyDetail": "0xb1965f76f0ee9f9d2aaccc56c16929e87f344371"
    },
    "contracts/libraries/details/EarringsDetail.sol": {
      "EarringsDetail": "0x7c0c8a6bb0253a065996599eb224eeae504736e2"
    },
    "contracts/libraries/details/MaskDetail.sol": {
      "MaskDetail": "0x8e29fabb49cd32d1eb12c4c0f9620cd866264014"
    },
    "contracts/libraries/details/EyebrowDetail.sol": {
      "EyebrowDetail": "0x9defd926e7c0a488f7efc1950c97a7c9e279fd26"
    },
    "contracts/libraries/details/EyesDetail.sol": {
      "EyesDetail": "0xa3e74c8c5adc4ae1b2f2aeaea2f0d3515754ea9b"
    },
    "contracts/libraries/details/HairDetail.sol": {
      "HairDetail": "0xb404a726b36a825a515186b07774ba437aedff21"
    },
    "contracts/libraries/details/MouthDetail.sol": {
      "MouthDetail": "0x8fa172076d0e0aec1e567097bdc33e64ce3e0103"
    },
    "contracts/libraries/details/NoseDetail.sol": {
      "NoseDetail": "0x55c02cfddca730fbd7d58765b5640a1c639aae43"
    },
    "contracts/libraries/details/MarkDetail.sol": {
      "MarkDetail": "0xc52cc3ed38dd0eab08f79599193e84a47ef305c3"
    }
  }
}