...
contract BullAlphaEth is Pausable, AlphaTokenEth {
    ...
    uint256 NAV = 0; // net assets value in USD
    ...
    enum State{ NONE, PENDING, READY}
    struct Pending {
        State state;
        uint256 fee;
        uint256 amountAlpha;
        uint256 amountAlphaNet;
    ...
    event LogChangeVault(address indexed _vault);
    ...
    */
    ...
    function getAlphaTokenValue() public view returns(uint256) {
        return NAV / totalSupply() * COEFF_SCALE_WEI;
    }

    function getTVL() public view returns(uint256) {
        return getAlphaTokenValue() * totalSupply() * COEFF_SCALE_WEI;
    }
    ...
    */
    ...
    function setVault(address _vault) external onlyOwner {
        require(
            _vault != address(0),
            "Alpha: vault address is the zero address"
        );
        vault = _vault;
        emit LogChangeVault(_vault);
    }
    ...
    function setNav(uint256 _nav) external {
        require(msg.sender == manager, "Alpha: unauthorized");
        NAV = COEFF_SCALE_WEI * _nav;
        emit LogChangeNav(NAV);
    }

    function setLockupPeriod(uint256 _period) external {
        require(msg.sender == manager, "Alpha: unauthorized");
        lockupPeriod = _period;
        emit LogChangeLockupPeriod(_period);
    }
    ...
    function setStateToReady(address[] calldata _users) external {
       require(
            msg.sender == manager,
            "Alpha: caller is not the manager"
        );
        uint256 _index;
        for (uint256 i=0; i<_users.length; i++){
             pendingWithdrawal[_users[i]].state = State.READY;
             _index = find(_users[i]);
             UsersOnPendingWithdrawal[_index] = UsersOnPendingWithdrawal[UsersOnPendingWithdrawal.length -1];
    ...
    function mintFees() external {
        ...
        _mint(msg.sender, CLAIMABLE_FEE);
        CLAIMABLE_FEE = 0;
    }
    ...
    function _deposit(uint256 _amount) internal {
        ...
        uint256 alphaTokenValue = getAlphaTokenValue();

        deposits[msg.sender].amount = deposits[msg.sender].amount + _amount; // origin stables amount
        deposits[msg.sender].start = block.timestamp; // new deposit = new start
        deposits[msg.sender].watermark = alphaTokenValue; // new deposit = new watermark

        uint256 _amountAlpha = alphaTokenValue * _amount;
    ...
    function _requestWithdrawal(uint256 _amount) internal {
        ...
        uint256 depositStart = deposits[msg.sender].start;
        require(
            block.timestamp - depositStart >= lockupPeriod,
        ...
        // fees
        uint256 _totalManagementFee = calculateManagementFeeRate(
            deposits[msg.sender].amount,
            depositStart
        );
        uint256 _managementFee = _totalManagementFee * _amount;
        uint256 _netAmountAlpha = _amount - _managementFee;
        // uint256 _profitFee = calculateProfitFees(_netAmountAlpha);
        _netAmountAlpha = _netAmountAlpha - 0; // profit fee instead of 0
        uint256 stableValueAfterFees = _netAmountAlpha / alphaTokenValue;

        pendingWithdrawal[msg.sender].fee = _managementFee + 0; // profit fee instead of 0
        pendingWithdrawal[msg.sender].amountAlpha = _amount;
        pendingWithdrawal[msg.sender].amountAlphaNet = _netAmountAlpha;
        pendingWithdrawal[msg.sender].amountStable = stableValueAfterFees;
        pendingWithdrawal[msg.sender].state = State.PENDING;

        totalPendingWithdrawalAmount = totalPendingWithdrawalAmount + stableValueAfterFees;

        UsersOnPendingWithdrawal.push(msg.sender);
        
        emit LogWithdrawRequested(msg.sender, stableValueAfterFees);
    ...
    function _withdraw() internal {
        ...
        uint256 _fee = _pendingWithdrawal.fee;
        uint256 _amountLP = _pendingWithdrawal.amountAlpha;
        uint256 _amountAlphaNet = _pendingWithdrawal.amountAlphaNet;
        uint256 _amountStable = _pendingWithdrawal.amountStable;
        State _state = _pendingWithdrawal.state;

        require(_state == State.READY, " Alpha: requested amount not yet ready");
        require(
        ...
        CLAIMABLE_FEE = CLAIMABLE_FEE + _fee;

        pendingWithdrawal[msg.sender].fee = 0;
        pendingWithdrawal[msg.sender].amountAlpha = 0;
        pendingWithdrawal[msg.sender].amountAlphaNet = 0;
        pendingWithdrawal[msg.sender].amountStable = 0;
        ...
        _burn(address(this), _amountAlphaNet);
    ...
    function calculateManagementFeeRate(uint256 _amount, uint256 _start)
        internal
        view
        returns (uint256)
    {
    ...
    //     uint256 _amountStableAfter = convertFromAlpha(_amountAlpha);
    //     if (_amountStableAfter >= _amountStableBefore) {
    //         _profitFee =
    //             (convertToAlpha(_amountStableAfter - _amountStableBefore) *
    //                 PROFIT_FEE_RATE) /
    //             COEFF_SCALE_DECIMALS;
    //     }
