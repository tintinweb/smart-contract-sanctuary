...
contract LarvaLads is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getSVG(LarvaObject memory larvaLad) internal view returns (string memory) {
        ...
        string memory output = string(abi.encodePacked(parts[0], parts[1], parts[2], parts[3], parts[4], parts[8], parts[10], parts[11], parts[12], parts[13], parts[5]));
                      output = string(abi.encodePacked(output, parts[4], parts[7], parts[9]));
