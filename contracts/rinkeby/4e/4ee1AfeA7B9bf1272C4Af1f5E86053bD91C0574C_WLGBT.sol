...
contract WLGBT is ERC721, Ownable, Pausable {
    ...
    uint256 public FREE_MINT_MAX_PER_ADDR = 1;
    uint256 public TOTAL_MINT_MAX_PER_ADDR = 10;
    ...
    function publicMint(uint256 number) external payable whenNotPaused{
        require(number >= 1, "Number is 0");
        if (public_minted < FREE_MINT_TOKENS) {
            require(balanceOf(msg.sender) + number <= TOTAL_MINT_MAX_PER_ADDR, "Beyond max number of free tokens");
            require(public_minted + number <= FREE_MINT_TOKENS, "Free tokens sold out");
        }else{
    ...
    function reservedMint(address[] calldata receivers) external onlyOwner {
        ...
        for (uint256 i = 0; i < receivers.length; i++) {
            _safeMint(receivers[i], _tokenIds.current()+1);
            _tokenIds.increment();
            reserved_minted++;
        }
    }
