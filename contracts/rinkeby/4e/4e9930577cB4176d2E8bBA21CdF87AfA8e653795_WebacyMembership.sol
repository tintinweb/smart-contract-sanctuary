...
contract WebacyMembership is Ownable {
    ...
    address public webacyBusinessAddress;
    uint256 public membershipAmount = 0.4 ether;
    ...
    function payMembership() external payable {
        require(webacyBusinessAddress != address(0), "Webacy address is not setted");
        require(
        ...
        proxyFactory.createProxyContract(msg.sender, webacyBusinessAddress);
    ...
    function setWebacyBusinessAddress(address _address) external onlyOwner {
        webacyBusinessAddress = _address;
    }

    function setMembershipAmount(uint256 _amount) external onlyOwner {
        membershipAmount = _amount;
    }

    // For testing only
    function unpayMembership(address _address) external onlyOwner {
        membershipPaid[_address] = false;
    }
}
