...
// this allows for multi-IPFS bundle contracts.
...
contract EaselyBiddableCollection is ERC721Enumerable, Ownable, IEaselyContractInitializer {    
    ...
    // If a tokenId is already minted or set to be a claimable then it is locked
    ...
    event TokenClaimable(uint256 _tokenId, uint256 _claimTokenId);
    // Event that is emitted when a tokenId is claimed by the token owner
    event TokenClaimed(address _claimer, uint256 _tokenId, uint256 _claimedTokenId);
    ...
    function createAuction(
        ...
        require(!_validAuction(_tokenIdToAuction[tokenId]), "Cannot already be on auction");
        require(ownerOf(tokenId) == _msgSender(), "Must own token to create auction");
    ...
    function settleAuction(uint256 tokenId) external {
        ...
        _transfer(ownerOf(tokenId), auction.topBidder, tokenId);
    }
    ...
    function bid(uint256 tokenId) external payable {
        ...
        require(msgValue > auction.price + auction.minBidIncrement, "Bid is too small");
