 ...
 *  - a 5% (customizable) fee on transfer such that:
 *    - 3% is redistributed to all eligible token holders
 *    - 1% is awarded to the team wallet (aka the default referrer)
 *    - 1% is awarded to the referrer of the sender address (fallbacks to the default referrer)
 *
...
contract Viral is IERC20, Ownable {
    ...
    function updateFee(uint256 _totalFee, uint256 _referralFee, uint256 _viralFee) external onlyOwner() {
        require(_totalFee <= 5, "VIRAL: Total fee too high");
        require(_referralFee.add(_viralFee) <= _totalFee, "VIRAL: Individual fee too high");
    ...
    function _disburse(uint256 rAmount) private {
        _totalDividendPoints = _totalDividendPoints.add(rAmount.mul(pointMultiplier).div(_totalSupply.sub(_excludedFromRewardSupply)));
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (!_isExcludedFromFee[sender]) {
            ...
            */
            ...
            uint256 dAmount = fAmount.sub(rAmount).sub(vAmount);

            _updateAccount(defaultReferrer_);
            address rReferrer = referrerOf(recipient);
            if (rReferrer != defaultReferrer_ && rReferrer != sender) {
