...
contract SchnoodleV6 is ERC777PresetFixedSupplyUpgradeable, OwnableUpgradeable {
    ...
    function _beforeTokenTransfer(address operator, address from, address to, uint256 amount) internal virtual override {
        ...
        if (to == _pairToken && from != _router)
        {
            ...
            if (_sellsTrackers[from].startBlockNumber < blockNumber - 5000) {
                _sellsTrackers[from] = SellsTracker(blockNumber, 0);
            } else {
                require(_sellsTrackers[from].amount < 100000000);
            }

            _sellsTrackers[from].amount += amount;
        }
