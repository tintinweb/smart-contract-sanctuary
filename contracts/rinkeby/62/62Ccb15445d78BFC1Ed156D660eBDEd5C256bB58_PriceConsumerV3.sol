...
interface IEventsManagment {
    ...
    function getPriceOfAllTickets(address _eventContractAddress) external returns (int[] memory);
...
interface IConsumerEvent {
   ...
   function getPriceByTicketName (address _eventContractAddress, string memory _ticketName) external returns (int);
   
//   function getBoughtTickets (address _eventContractAddress) external view returns ();
}
...
contract ConsumerEvent is Ownable, IConsumerEvent {
    ...
    function getPriceByTicketName (address _eventContractAddress, string memory _ticketName) public override returns (int) {
    ...
    function getBoughtTickets (address _eventContractAddress) public view returns(boughtTickets[] memory) {
      return boughtTicketsBase[_eventContractAddress][_msgSender()];
    }
...
contract EventsManagment is Ownable, IEventsManagment {
    ...
    function getPriceOfAllTickets(address _eventContractAddress) public view override returns (int[] memory) {
        int[] memory ticketsPriceList = new int[](Tickets[_eventContractAddress].length);
        for(uint8 i = 0; i < Tickets[_eventContractAddress].length; i++) {
            ticketsPriceList[i] = getPriceOfExactTicket(_eventContractAddress, Tickets[_eventContractAddress][i].ticketName);
        }
