...
import './../interface/IStakeActions.sol';
import './../ERC20/custom/WIGL.sol';
import './../ERC721/custom/WiggleWorldRinkeby.sol';


contract WiggleWorldStaking is IERC721Receiver, IStakeActions, Ownable {
    constructor() {
      tokenContract = WIGL(0x960e70a84acEecd71b2D137970757F5F353396f0);
      nftContract = WiggleWorldRinkeby(0x9A1E359A9cF912792C77394a132bC057dE139a89);
    }
    ...
    uint256 public constant DAILY_TOKEN_MINT_RATE = 10000 ether;
    uint256 public constant MINIMUM_TO_EXIT = 2 days;
    uint256 public constant EARLY_UNSTAKE_TAX = 20;
    uint256 public constant MAXIMUM_GLOBAL_TOKEN_SUPPLY = 2400000000 ether;

    uint256 public totalTokensMinted;
    uint256 private taxFromLastClaim;
    ...
    modifier _updateTokenMinted() {
        if (totalTokensMinted < MAXIMUM_GLOBAL_TOKEN_SUPPLY) {
            totalTokensMinted +=
                ((block.timestamp - lastClaimTimestamp) * numNftStaked * DAILY_TOKEN_MINT_RATE) /
                1 days;
            totalTokensMinted += taxFromLastClaim;
            taxFromLastClaim = 0;
            lastClaimTimestamp = block.timestamp;
        } else {
            totalTokensMinted = MAXIMUM_GLOBAL_TOKEN_SUPPLY;
        }
        _;
    }
    ...
    WiggleWorldRinkeby public nftContract;

    function setNftContractAddress(address _nftContract) external onlyOwner {
        nftContract = WiggleWorldRinkeby(_nftContract);
    }
    ...
    function onERC721Received(
        ...
        require(from == address(0x0), 'Cant call this directly!');
        return 0x150b7a02;
    }

    function stakeMultiple(address _owner, uint256[] calldata _tokenIds)
        external
        _updateTokenMinted
    {
        // Owner has the option to disable staking
        // Note that disable staking does not affect the stakers
        // ability to claim their NFTs and tokens!
        if (stakingEnabled) {
            for (uint256 i = 0; i < _tokenIds.length; i++) {
                require(nftContract.ownerOf(_tokenIds[i]) == _owner, 'Not your NFT.');
                nftContract.transferFrom(_owner, address(this), _tokenIds[i]);
                stakedTokenOwners[i] = Stake({
                    owner: _msgSender(),
                    tokenId: i,
    ...
    function claimMultiple(
        ...
        bool _unstake
    ) external _updateTokenMinted {
        uint256 owed = 0;
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            owed += calculateAmountOwed(_tokenIds[i], _unstake);
        }
        if (owed == 0) return;
        tokenContract.mint(_owner, owed);
    }

    function calculateAmountOwed(uint256 _tokenId, bool _unstake) internal returns (uint256 owed) {
        Stake memory stake = stakedTokenOwners[_tokenId];
        // Call requires here rather than for loop so we only have to create Stake one time
        require(stake.owner == _msgSender(), 'You didnt stake that NFT.');
        require(!(block.timestamp - stake.timestamp < MINIMUM_TO_EXIT), 'You cant unstake yet.');
        if (totalTokensMinted < MAXIMUM_GLOBAL_TOKEN_SUPPLY) {
            owed = ((block.timestamp - stake.timestamp) * DAILY_TOKEN_MINT_RATE) / 1 days;
        } else if (stake.timestamp > lastClaimTimestamp) {
            owed = 0;
        } else {
            // Take what you earned before minting stopped
            owed = ((lastClaimTimestamp - stake.timestamp) * DAILY_TOKEN_MINT_RATE) / 1 days;
        }
        if (_unstake) {
            // Apply early unstake tax
            owed = (owed * (100 - EARLY_UNSTAKE_TAX)) / 100;
            // Add to tax pool
            taxFromLastClaim += ((owed * (EARLY_UNSTAKE_TAX)) / 100);
            // Return NFT
            nftContract.safeTransferFrom(address(this), _msgSender(), _tokenId, '');
            delete stakedTokenOwners[_tokenId];
            numNftStaked -= 1;
        } else {
            // Reset stake
            stakedTokenOwners[_tokenId] = Stake({
                owner: _msgSender(),
                tokenId: _tokenId,
                timestamp: uint256(block.timestamp)
            });
...
// Used to add NFTs to staking contract (ERC721 Receiver)
interface IStakeActions {
    function stakeMultiple(address _owner, uint256[] calldata tokenIds) external;
...
import './../base/CoreERC721.sol';

contract WiggleWorldRinkeby is CoreERC721 {
    uint256 public maxPossibleSupply = 1000000;

    constructor() CoreERC721('WiggleWorldRinkeby', 'WIGL', 10000) {}

    uint256 private price = 0.04 ether;

    function validMintOption(uint256 mintOption) internal pure override returns (bool) {
        if (mintOption < 20) {
            return true;
        }
        return false;
    ...
    function getPrice(uint256 quantity) internal view override returns (uint256) {
    ...
    function setMaxSupply(uint256 _max) external onlyOwner {
        require(_max < maxPossibleSupply, 'Max possible supply is 1M.');
        totalSupply = _max;
        if (_max > current()) {
            totalSupply = _max;
        } else {
