...
contract UnitedPunks is ERC721, Ownable {
    
    using SafeMath for uint256;
    uint public constant MAX_UNITEDPUNKS = 13666;
    bool public hasSaleStarted = false;
    uint public constant UNITEDPUNKS_PRICE = 30000000000000000;

    string public METADATA_PROVENANCE_HASH = "";
    
    constructor() ERC721("UnitedPunks","UP")  { }
   ...
   function mintUNITEDPunks(uint256 numPunks) public payable {
        require(totalSupply() < MAX_UNITEDPUNKS, "Sale has already ended");
        require(numPunks > 0 && numPunks <= 20, "You can adopt minimum 1, maximum 20 United Punks");
        require(totalSupply().add(numPunks) <= MAX_UNITEDPUNKS, "Exceeds MAX_UNITEDPUNKS");
        require(msg.value >= UNITEDPUNKS_PRICE.mul(numPunks), "Ether value sent is below the price");
    ...
    function giveGift(address _address) public onlyOwner {
        uint mintIndex = totalSupply();
