...
contract AllocationPool is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 poolReward = (multiplier * allocRewardPerBlock * pool.allocPoint) / totalAllocPoint;
            accRewardPerShare = accRewardPerShare + ((poolReward * ACCUMULATED_MULTIPLIER) / lpSupply);
        }
        return
        user.pendingReward + (((user.amount * accRewardPerShare) / ACCUMULATED_MULTIPLIER) - user.rewardDebt);
    }
     ...
     */
    function allocClaimAll(uint256[] memory _pids)
    external
    nonReentrant
    {
