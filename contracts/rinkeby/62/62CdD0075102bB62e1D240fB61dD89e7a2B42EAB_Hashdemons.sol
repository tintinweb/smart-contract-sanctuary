...
contract Hashdemons is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("Hashdemons", "HDR") { }
    ...
    function airdropTokens(address[] calldata tokenDistribution) external onlyOwner {
        for(uint i = 0; i < tokenDistribution.length; i++) {
            uint mintIndex = totalSupply();
            if (mintIndex < MAX_TOKENS) {
                _safeMint(tokenDistribution[i], mintIndex);
            }
        }
    }
    ...
    function mintToken(uint numberOfTokens) external  {
