...
contract Widget is ERC721, ERC721Enumerable, Ownable {
    ...
    function svg(uint tokenId) public view returns (string memory) {
        ...
        string memory b = Base64.encode(getSVG(tokenId));
    ...
    function getGreet(uint nowh) internal view returns (string memory)
    {
        if (nowh > 5 && nowh <= 10) {
            return string("gm");
    ...
    function getSVG(uint tokenId) internal view returns (bytes memory)
    {
        uint hr = 24;
        uint offseth = 1;
        ...
        string memory a = string(
            '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: black; font-family: consolas; font-size: 40px; }</style><rect width="100%" height="100%" fill="black" opacity="');
            
        string memory c = string('%"/><text x="10" y="40" class="base">');
        string memory b = uint2str(opacity);
        string memory d = getGreet(nowh);

        string memory e = string('</text><text x="10" y="90" class="base">Oslo</text><text x="10" y="140" class="base">wagmi</text></svg>');

        // console.log("nowh: %d", nowh);
        // console.log("greet: %s", getGreet(nowh));
