...
bytes32 constant SUPERUSER = keccak256("SUPERUSER");

abstract contract AdminControl is AccessControl {
    modifier onlyAdmin() {
        require(hasRole(ADMIN, msg.sender), "AdminControl: caller is not an admin");
        _;
    }

    modifier onlySuperUser() {
        require(hasRole(SUPERUSER, msg.sender), "AdminControl: caller is not a superuser");
        _;
    }
}

contract Cans is ERC721Enumerable, ERC721Burnable, ReentrancyGuard, AdminControl {
    ...
    mapping(uint => Tier) public canTier;
    ...
    function getSeries(uint tokenId) public pure returns (uint) {
        /* require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token"); */
        return tokenId / ONE_MILLION;
    }

    function setMaxSupply(uint seriesMaxSupply_) public onlyAdmin {
        require(seriesMaxSupply_ < ONE_MILLION, "Cannot exceed 1,000,000");
        seriesMaxSupply[currentSeries()] = seriesMaxSupply_;
    }

    function createSeries(uint seriesMaxSupply_) external onlyAdmin {
        setMaxSupply(seriesMaxSupply_);
        seriesCount++;
    }

    function setCanTier(uint[] calldata ids, Tier _tier) external onlyAdmin {
        for (uint i; i < ids.length; i++) {
            // should we check that the ids are in the current series?
            canTier[ids[i]] = _tier;
        }
    }

    function setSeriesCount(uint seriesCount_) external onlyAdmin {
        require(seriesCount_ < seriesCount, "Invalid series number");
        seriesCount = seriesCount_;
    }

    function setPricePerToken(uint pricePerToken_) external onlyAdmin {
        pricePerToken = pricePerToken_;
    }

    function setMaxPublicMint(uint maxPublicMint_) external onlyAdmin {
        maxPublicMint = maxPublicMint_;
    }

    function setMaxAllowListMint(uint maxAllowListMint_) external onlyAdmin {
        maxAllowListMint = maxAllowListMint_;
    }

    function setAllowListActive(bool allowListActive_) external onlyAdmin {
        allowListActive = allowListActive_;
    }

    function setAllowList(bytes32 merkleRoot_) external onlyAdmin {
    ...
    function setMysteryTokenURI(string memory mysteryTokenURI_) external onlyAdmin() {
        _mysteryTokenURI = mysteryTokenURI_;
    }

    function setRevealed(bool revealed_) external onlyAdmin() {
    ...
    function devMint(uint n) external onlyAdmin {
        _internalMint(n);
    }

    function setSaleActive(bool saleActive_) external onlyAdmin {
    ...
    function withdraw() external onlySuperUser {
