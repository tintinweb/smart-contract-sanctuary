...
// File: contracts-test/interfaces/PhoenixTokenInterface.sol

// SPDX-License-Identifier: MIT
pragma solidity ^0.6.0;

interface PhoenixTokenInterface {
...
contract OnchainAggregator is AggregatorV2V3Interface, Owned {
  ...
  PhoenixTokenInterface public phoenixToken;
   ...
   * reserve of RESERVE_ROUNDS * oracleCount() PHOENIX earmarked for payment to
   ...
   * @param _phoenix The address of the token
   * @param _paymentAmount The amount paid of PHOENIX paid to each oracle per submission, in wei (units of 10⁻¹⁸ PHOENIX)
  ...
  constructor(
    ...
    phoenixToken = PhoenixTokenInterface(_phoenix);
  ...
  /**
   * @notice recalculate the amount of PHOENIX available for payouts
   */
    ...
    public
  {
    ...
    uint256 nowAvailable = phoenixToken.balanceOf(address(this)).sub(funds.allocated);
  ...
  /**
   * @notice query the available amount of PHOENIX for an oracle to withdraw
   */
  ...
  /**
   * @notice transfers the oracle's PHOENIX to another address. Can only be called
   * by the oracle's admin.
   * @param _oracle is the oracle whose PHOENIX is transferred
   * @param _recipient is the address to send the PHOENIX to
   * @param _amount is the amount of PHOENIX to send
   */
    ...
    external
  {
    ...
    // Safe to downcast _amount because the total amount of PHOENIX is less than 2^128.
    ...
    assert(phoenixToken.transfer(_recipient, uint256(amount)));
  }

  /**
   * @notice transfers the owner's PHOENIX to another address
   * @param _recipient is the address to send the PHOENIX to
   * @param _amount is the amount of PHOENIX to send
   */
    ...
    onlyOwner()
  {
    ...
    require(phoenixToken.transfer(_recipient, _amount), "token transfer failed");
  ...
  /**
   * @notice called through PHOENIX's transferAndCall to update available funds
