...
contract RabbitNFT is ERC1155, SignerRole {
    ...
    event ItemAdded(uint256 id, uint256 supply, string uri);
    ...
    function initialize(string memory _name, string memory _uri, address creator, bool bPublic) external {
        require(isFactory(_msgSender()), "Only for factory");
        _setURI(_uri);
        name = _name;
        owner = creator;
        isPublic = bPublic;
    ...
    */
    function addItem(uint256 supply, uint256 _fee, string memory _uri) public returns (uint256) {
        ...
        if (bytes(_uri).length > 0) {
            customUri[items] = _uri;
            emit URI(_uri, items);
        }
        emit ItemAdded(items, supply, _uri);
        return items;
    }
    /**
...
// File: contracts\RabbitMarket.sol
...
contract RabbitMarket is Ownable {
	...
	uint256 public totalEarning; /* Total ETH amount */
    ...
    constructor () {
        string memory rinkebyWETH = "0xc778417E063141139Fce010982780140Aa0cD5Ab";
        // string mainnetWETH = "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2";
        address wETHAddress = parseAddr(rinkebyWETH);
        rabbitToken = IERC20(wETHAddress);
    }

	function initialize(address _nftAddress, uint256 _fee, address _feeAddress, string memory _name, string memory _uri) external onlyOwner {
		rabbitNFT = IRabbitNFT(_nftAddress);
		feeAddress = _feeAddress;
		swapFee = _fee;
		collections.push(_nftAddress);
		collectionCreators[_nftAddress] = msg.sender;
		emit CollectionCreated(_nftAddress, msg.sender, _name, _uri, true); //"RabbitNFT","https://domain.com/api/item/"
	}

    function setNFTAddress(address _address) external onlyOwner {
		require(_address != address(0x0), "invalid address");
        rabbitNFT = IRabbitNFT(_address);
    }
	...
	function createCollection(string memory _name, string memory _uri, bool bPublic) public returns(address collection) {
		bytes memory bytecode = type(RabbitNFT).creationCode;
        ...
        IRabbitNFT(collection).initialize(_name, _uri, msg.sender, bPublic);
    ...
    function list(address _collection, uint256 _token_id, uint256 _amount, uint256 _price, bool _bMint) public {
		...
		IRabbitNFT nft = IRabbitNFT(_collection);
		if(_bMint) {
		...
		items[currentItemId].creator = nft.creatorOfItem(_token_id);
	...
	function delist(uint256 _id) external {
		...
		IRabbitNFT(items[_id].collection).safeTransferFrom(address(this), items[_id].owner, items[_id].token_id, items[_id].balance, "delist from Daniel Marketplace");
		items[_id].balance = 0;
	...
	function addItems(uint256 _id, uint256 _amount) external {
		...
		IRabbitNFT(items[_id].collection).safeTransferFrom(msg.sender, address(this), items[_id].token_id, _amount, "add items to Daniel Marketplace");
	...
	function removeItems(uint256 _id, uint256 _amount) external {
		...
		IRabbitNFT(items[_id].collection).safeTransferFrom(address(this), msg.sender, items[_id].token_id, _amount, "remove items from Daniel Marketplace");
    ...
    function buy(uint256 _id, uint256 _amount) external {
		...
		uint256 danielAmount = item.price.mul(_amount);

		// transfer Daniel token to admin
		if(swapFee > 0) {
			require(rabbitToken.transferFrom(msg.sender, feeAddress, danielAmount.mul(swapFee).div(PERCENTS_DIVIDER)), "failed to transfer admin fee");
		}
		// transfer Daniel token to creator
		if(item.creatorFee > 0) {
			require(rabbitToken.transferFrom(msg.sender, item.creator, danielAmount.mul(item.creatorFee).div(PERCENTS_DIVIDER)), "failed to transfer creator fee");
		}
		// transfer Daniel token to owner
		uint256 ownerPercent = PERCENTS_DIVIDER.sub(swapFee).sub(item.creatorFee);
		require(rabbitToken.transferFrom(msg.sender, item.owner, danielAmount.mul(ownerPercent).div(PERCENTS_DIVIDER)), "failed to transfer to owner");

		// transfer NFT token to buyer
		IRabbitNFT(items[_id].collection).safeTransferFrom(address(this), msg.sender, item.token_id, _amount, "buy from Daniel Marketplace");
		...
		totalEarning = totalEarning.add(danielAmount);
