 ...
 * {ERC721Enumerable}.
 */
contract Rangers is Context, ERC165, IERC721, IERC721Metadata {
    using Address for address;
    using Strings for uint256;
    using SafeMath for uint256;

    // Token name
    string private _name = 'Rangers';

    // Token symbol
    string private _symbol = 'RANG';
    ...
    // Token Price
    uint256 public constant tokenPrice = 60000000000000000;
    ...
    */
    ...
    function whiteListed(address buyer) internal view returns (bool) {
      address[] memory addresses = _approvedPreSaleAddresses;
      for(uint i = 0; i<addresses.length; i++) {
        if (buyer == addresses[i]) { return true; }
      }
      return false;
    }
    ...
    function buyTokens(uint tokensNumber) public payable {
        ...
        if (preSaleActive) {
          require(whiteListed(msg.sender), 'Your wallet is not whitelisted for presale');
          require(balanceOf(msg.sender).add(tokensNumber) <= 3, 'You cannot purchase more then 3 tokens on presale');
        }
    ...
    function setPreSaleAddresses(address[] memory addresses) public onlyOwner {
      require(addresses.length > 0, 'array length < 1');
      _approvedPreSaleAddresses = addresses;
    }
    ...
    function _baseURI() internal view virtual returns (string memory) {
        return "https://rangersnft.com/meta/";
    }
