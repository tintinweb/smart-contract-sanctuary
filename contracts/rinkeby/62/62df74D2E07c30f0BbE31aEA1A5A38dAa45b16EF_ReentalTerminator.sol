...
contract ReentalTerminator is UUPSUpgradeableByRole {
  ...
  function terminate(address token, uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE) {
    require(IReentalFactory(IReentalDeployer(config).get(keccak256("REENTAL_FACTORY"))).isReentalToken(token), "ReentalTerminator: token NOT listed");
    amounts[token] += amount;
  }

  function claim(address token, address account) public {
    require(IReentalFactory(IReentalDeployer(config).get(keccak256("REENTAL_FACTORY"))).isReentalToken(token), "ReentalTerminator: token NOT listed");
    uint256 totalAmount = amounts[token];
    require(totalAmount > 0, "ReentalTerminator: there is nothing to claim");
    IReentalToken tokenInterface = IReentalToken(token);
    uint256 balance = tokenInterface.balanceOf(account);
    require(balance > 0, "ReentalTerminator: balance of holder is zero");
    uint256 amount = totalAmount * balance / tokenInterface.totalSupply();
    amounts[token] -= amount;
    address vault = IReentalDeployer(config).get(keccak256("REENTAL_VAULT"));
    tokenInterface.burnFrom(account, balance);
    require(IERC20(IReentalDeployer(config).get(keccak256("USDT"))).transferFrom(vault, account, amount));
  ...
  function extractTokens(address token) public onlyRole(DEFAULT_ADMIN_ROLE) {
      ...
      require(tokenInterface.transfer(_msgSender(), balance), "ReentalTerminator: extraction failed");
  }

  function extractETH() public onlyRole(DEFAULT_ADMIN_ROLE) {
      require(payable(_msgSender()).send(address(this).balance), "ReentalTerminator: funds transfer failed");
  }
}
