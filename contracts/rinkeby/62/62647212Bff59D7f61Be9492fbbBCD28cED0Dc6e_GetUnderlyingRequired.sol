...
contract GetUnderlyingRequired {
    function getUnderlyingRequired(IHifiPool hifiPool, uint256 hTokenOut) external view returns (uint256 underlyingRequired) {
        uint256 normalizedUnderlyingReserves = hifiPool.getNormalizedUnderlyingReserves();
        uint256 hTokenReserves = hifiPool.hToken().balanceOf(address(hifiPool));
