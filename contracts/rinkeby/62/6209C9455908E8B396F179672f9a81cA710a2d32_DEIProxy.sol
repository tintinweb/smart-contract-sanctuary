...
contract DEIProxy is Ownable {
	...
	function Nativecoin2DEI(
		...
		uint amount_out = getAmountOutNativecoin2DEI(msg.value, proxy_input.deus_price, proxy_input.path);
		require(amount_out >= proxy_input.min_amount_out, "PROXY: INSUFFICIENT_OUTPUT_AMOUNT");
		...
		if(global_collateral_ratio == 1000000) {
			...
			(uint collateral_needed_amount, uint collateral_for_deus_amount) = getCollateralAmountIn2DEUSForMintDEI(global_collateral_ratio, collateral_amount, proxy_input.deus_price);

			amounts_out = IUniswapV2Router02(uniswap_router).swapExactTokensForTokens(
																	collateral_for_deus_amount,
																	1,
																	collateral2deus_path,
																	address(this),
																	deadline
	...
	function ERC202DEI(
		...
		uint amount_out = getAmountOutERC202DEI(proxy_input.amount_in, proxy_input.deus_price, proxy_input.path);
		require(amount_out >= proxy_input.min_amount_out, "PROXY: INSUFFICIENT_OUTPUT_AMOUNT");

		require(proxy_input.path[proxy_input.path.length - 1] != collateral_address && proxy_input.path.length >= 2, "PROXY: WRONG_PATH");

		IERC20(proxy_input.path[0]).transferFrom(msg.sender, address(this), proxy_input.amount_in);

		uint[] memory amounts_out = IUniswapV2Router02(uniswap_router).swapExactTokensForTokens(proxy_input.amount_in, 1, proxy_input.path, address(this), deadline);
		uint collateral_amount = amounts_out[amounts_out.length - 1];
		...
		emit Buy(proxy_input.path[0], proxy_input.amount_in, dei_amount, global_collateral_ratio);
	}

	function collateral2DEI(
		ProxyInput memory proxy_input
	) external returns (uint dei_amount) {
		uint amount_out = getAmountOutCollateral2DEI(proxy_input.amount_in, proxy_input.deus_price);
		require(amount_out >= proxy_input.min_amount_out, "PROXY: INSUFFICIENT_OUTPUT_AMOUNT");

		IERC20(collateral_address).transferFrom(msg.sender, address(this), proxy_input.amount_in);
	
		uint global_collateral_ratio = IDEIStablecoin(dei_address).global_collateral_ratio();
		if(global_collateral_ratio == 1000000) {
			dei_amount = IDEIPool(pool_collateral).mint1t1DEI(proxy_input.amount_in, proxy_input.collateral_price, proxy_input.expire_block, proxy_input.sigs);
		} else if(global_collateral_ratio == 0) {
			uint[] memory amounts_out = IUniswapV2Router02(uniswap_router).swapExactTokensForTokens(
																	proxy_input.amount_in,
																	1,
																	collateral2deus_path,
																	address(this),
																	deadline
			...
			(uint collateral_needed_amount, uint collateral_for_deus_amount) = getCollateralAmountIn2DEUSForMintDEI(global_collateral_ratio, proxy_input.amount_in, proxy_input.deus_price);
		...
		dei_amount = IERC20(dei_address).balanceOf(address(this));
		IERC20(dei_address).transfer(msg.sender, dei_amount);
	
		emit Buy(collateral_address, proxy_input.amount_in, dei_amount, global_collateral_ratio);
	}
	...
	function DEUS2DEI(
		...
		uint amount_out = getAmountOutDEUS2DEI(deus_amount, proxy_input.deus_price);
		require(amount_out > proxy_input.min_amount_out, "PROXY: INSUFFICIENT_OUTPUT_AMOUNT");
	...
	// function calcMintFractionalDEI(MintFD_Params memory params) public pure returns (uint256) {
	...
	// 	uint deus_amounts;
	// 	collateral_amount *= collateral_missing_decimals_d18;
	// 	uint init_collateral_amount = collateral_amount;
	// 	while (collateral_amount > 0 && deus_amounts < calcMintFractionalDEI(MintFD_Params(deus_price, 1e6, init_collateral_amount, global_collateral_ratio))) {
	// 		uint collateral_for_swap = collateral_amount * (1e6 - global_collateral_ratio) / 1e6;
	// 		collateral_to_deus += collateral_for_swap;
	// 		uint[] memory amounts_out = IUniswapV2Router02(uniswap_router).getAmountsOut(collateral_for_swap / collateral_missing_decimals_d18, collateral2deus_path);
	// 		uint deus_amount = amounts_out[amounts_out.length - 1];
	// 		deus_amounts += deus_amount;
	// 		uint deus_to_collateral = (deus_amount * deus_price) / (1e6);
	// 		uint collateral_needed = deus_to_collateral * global_collateral_ratio / (1e6 - global_collateral_ratio);
	// 		collateral_amount -= (collateral_for_swap + collateral_needed);
	// 	}
	// 	return collateral_to_deus / collateral_missing_decimals_d18;
	// }
	...
	function getCollateralAmountIn2DEUSForMintDEI(
		...
		uint collateral_amount,
		uint deus_price
	) public view returns (uint collateral_needed_amount, uint collateral_for_deus_amount) {
		uint times = while_times;
		while (times > 0 && collateral_amount > 0) {
			uint collateral_for_swap = collateral_amount * (1e6 - global_collateral_ratio) / 1e6;
			collateral_for_deus_amount += collateral_for_swap;
			...
			uint collateral_needed = deus_to_collateral * global_collateral_ratio / (1e6 - global_collateral_ratio);
			collateral_needed_amount += collateral_needed;
			collateral_amount -= (collateral_for_swap + collateral_needed);
	...
	function getAmountOutCollateral2DEI(
		...
		if(global_collateral_ratio == 1000000) {
			...
			(uint collateral_needed_amount, uint collateral_for_deus_amount) = getCollateralAmountIn2DEUSForMintDEI(global_collateral_ratio, collateral_amount, deus_price);
			...
			uint deus_amount = amounts_out[amounts_out.length - 1];
			dei_amount = ((collateral_needed_amount) * collateral_missing_decimals_d18) + (deus_amount * deus_price / 1e6);
		}
		uint minting_fee = IDEIPool(pool_collateral).minting_fee();
		dei_amount = (dei_amount * (uint(1e6) - minting_fee)) / 1e6;
	}
