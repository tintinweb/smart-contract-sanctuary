...
contract DogeClub is ERC721, Ownable, RandomlyAssigned {
  using Strings for uint256;

  uint256 public currentSupply = 0;
  string public baseURI = "";  
  uint256 public cost = 0.04 ether; 
  uint256 public maxSupply = 10; // Needs to be updated to 10,000 for mainnet
  ...
  constructor()
    ERC721("DOGE CLUB", "DC") 
    // Creating the pool to fill tokenArray, RandomlyAssigned(**total supply, **start from)
    RandomlyAssigned(maxSupply - ownerMintAmount, ownerMintAmount + 1) {
        // Building an array of semi random token order 
        for (uint256 i = 1; i <= maxSupply - ownerMintAmount; i++) {
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    for (uint256 i = 1; i <= _mintAmount; i++) {
        uint256 arrayIndex = currentSupply - ownerMintAmount; 
        _safeMint(msg.sender, tokenArray[arrayIndex]);
        currentSupply++;
    }
  }
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    string memory currentBaseURI = baseURI; // Unsure if this not being callable could be a result from an bigger issue, the _baseURI is no longer callable. 
    return bytes(currentBaseURI).length > 0
        ? string(abi.encodePacked(currentBaseURI, tokenId.toString(), ".json"))
        : "";
  }
 
  // Making sure a baseURI is set before minting starts
  function isBaseURISet() public view returns (bool) {
    return keccak256(abi.encodePacked(baseURI)) != keccak256(abi.encodePacked(""));
  }

  // Mint function for the owner to mint the first 3 tokens in order 
  function ownerMint() public onlyOwner {
    require(isBaseURISet());

    for (uint256 i = 1; i <= ownerMintAmount; i++) {
