...
contract PetToken is RawToken {
    ...
    uint256 benefits_period = 31556926; // ~365.24 day = 1 year = 31556926 sec
    ...
    uint256 tokensAlreadySoldF = 0;
    uint256 tokensAlreadySoldR = 0;

    // TODO: 10% годовых
    //
    //в момент продажи
    //
    //сколько дней он владел за каждый день влдения
    //сумму в РЦЛ
    //
    // TODO: покупка нескольких токенов
    ...
    function setPurchaseRules(
        ...
        require(hasRole(MINTER_ROLE, _msgSender()), "PetToken: must have minter role to change price");
    ...
    function getTotalTokens() public override returns (uint256, uint256, uint256, uint256){
        return (tokensToSaleF, tokensToSaleR, tokensAlreadySoldF, tokensAlreadySoldR);
    }

    function incrementAmountToSell(uint256 totalTokens, bool isResource) public override {
        require(hasRole(MINTER_ROLE, _msgSender()), "PetToken: must have minter role to change total tokens");
        if (isResource) {
    ...
    function decrementAmountToSell(uint256 totalTokens, bool isResource) public override {
        require(hasRole(MINTER_ROLE, _msgSender()), "PetToken: must have minter role to change total tokens");
        if (isResource) {
            require(tokensToSaleR > 0, 'tokens to sale is 0');
            tokensToSaleR -= totalTokens;
        } else {
            require(tokensToSaleF > 0, 'tokens to sale is 0');
            tokensToSaleF -= totalTokens;
        }
    }
    ...
    function alreadyToSold(uint256 totalTokens, bool isResource) public override {
        ...
        if (isResource) {
            tokensAlreadySoldR += totalTokens;
        } else {
            tokensAlreadySoldF += totalTokens;
        }
    }
}
...
ERC721Burnable {
    ...
    function getTotalTokens() public virtual returns (uint256, uint256, uint256, uint256);
