...
contract SwampVerse is SwampVerseERC721 {
  ...
  event MintSwampCreature(uint256 indexed tokenId);
  event Release3D(uint256 indexed tokenId);

  uint256 constant public UPDATE_NAME_PRICE = 100 ether;
  uint256 constant public UPDATE_DESCRIPTION_PRICE = 100 ether;
  uint256 constant public MINT_SWAMP_CREATURE_PRICE = 450 ether;
  uint256 constant public RELEASE_3D_PRICE = 800 ether;

  ICroakens public Croakens;
  mapping(uint256 => SwamperInfo) public swamperInfo;
  mapping(uint256 => bool) public released3D;
  ...
  function mintSwampCreature()
    public
    nonReentrant
  {
    Croakens.burn(msg.sender, MINT_SWAMP_CREATURE_PRICE);

    // TODO: implement
    // emit MintSwampCreature(tokenId);
  }

  function release3D(uint256 tokenId)
    public
    onlyTokenOwner(tokenId)
  {
    require(!released3D[tokenId], "3D already released");

    Croakens.burn(msg.sender, RELEASE_3D_PRICE);
    released3D[tokenId] = true;
    emit Release3D(tokenId);
  }
...
contract SwampVerseERC721 is ERC721, Ownable, ReentrancyGuard, PaymentSplitter, Whitelisted {
  using Address for address;
  ...
  uint256 internal _presaleStartTime;
  ...
  uint256 constant public MAX_WHITELIST = 7500;

  string public PROVENANCE_HASH; // keccak-256
  address public OFFSET_PROXY;
  ...
  {
    OFFSET_PROXY = offsetProxy;
    _baseTokenURI = baseTokenURI;
    _payees = payees;
  }

  modifier onlyOffsetProxy() {
    require(msg.sender == OFFSET_PROXY, "Caller is not the offset proxy");
    _;
  }
  ...
  function setTokenOffset(uint256 randomness) public onlyOffsetProxy {
  ...
  function startPresale() public onlyOwner {
    require(_presaleStartTime == 0, "Presale has already been started");

    _presaleStartTime = block.timestamp;
  }

  function _presaleEnded() internal view returns (bool) {
    return block.timestamp >= _presaleStartTime + 1 days
    || _presaleMinted == MAX_WHITELIST;
  }
    ...
    isValidWhitelistSignature(signature)
  {
    ...
    require(_presaleStartTime != 0 && !_presaleEnded(),            "Presale has not started");
  ...
  function mint(uint256 amount) public payable nonReentrant {
    require(msg.value == MINT_PRICE * amount,          "Invalid Ether amount sent");
    require(_presaleStartTime != 0 && _presaleEnded(), "Presale has not ended");
