...
contract ToyApe is ERC721Enumerable, Ownable {
    ...
    // Token Detail
    mapping(uint256 => ToyApeDetail) private _toyApeDetail;
    ...
    function mintTokenId(uint256 tokenId) public onlyOwner {
        ...
        uint256 first_encounter = block.timestamp;
        _safeMint(msg.sender, tokenId);
        _toyApeDetail[tokenId] = ToyApeDetail(first_encounter);
        emit TokenMinted(tokenId, msg.sender, first_encounter);
    }
     ...
     * Mint DPandas
     */
    function mintToyApe(uint256 numberOfTokens) public payable {
        ...
        require(
            ...
            "Purchase would exceed max supply of DPandas"
        );
        require(
     ...
     */
    function getToyApeDetail(uint256 tokenId)
        public
        view
        returns (ToyApeDetail memory detail)
    {
        require(_exists(tokenId), "Token was not minted");

        return _toyApeDetail[tokenId];
    }
