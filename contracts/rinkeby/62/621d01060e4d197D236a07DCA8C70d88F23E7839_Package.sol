...
contract Package {
    ...
    function buy() public returns (uint256) {
        ...
        require(token.balanceOf(msg.sender) >= price, "Your balance is insufficient");
        ...
        require(random_num > 0, "All package have been sold out");
    	...
    	if (random_num <= first) {
    		first = first.sub(1);
    		user_prize.first_num = user_prize.first_num.add(1);
            return 1;
    	}
    	else if (random_num > first && random_num <= (first + second) ) {
    		second = second.sub(1);
    		user_prize.second_num = user_prize.second_num.add(1);
            return 2;
    	}
    	else if (random_num > second && random_num <= (first + second + third)) {
    		third = third.sub(1);
    		user_prize.third_num = user_prize.third_num.add(1);
            return 3;
    	}
    	else if (random_num > third && random_num <= (first + second + third + fourth)) {
    		fourth = fourth.sub(1);
    		user_prize.fourth_num = user_prize.fourth_num.add(1);
            return 4;
    	}
    }
