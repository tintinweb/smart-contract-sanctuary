...
contract MasterChef is Ownable {
    ...
    function getCurrentPerBlock() public view returns (uint256) {
        uint256 calcTreePerBlock = treePerBlock;
        uint256 calcTime = startTime + 1 minutes;
        uint256 num = 1;

        for (calcTime; calcTime < now; calcTime + 1 minutes) {
            num = num.div(50).mul(49);
        }
        
        calcTreePerBlock = treePerBlock.mul(num);
        
        return calcTreePerBlock;
    }
}