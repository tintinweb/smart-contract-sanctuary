...
contract SaberMountain is ERC721Enumerable, Ownable {
  ...
  /* Define */
    ...
    /* CHANGE CONTRACT METADATA AT BOTTOM OF PAGE */
      ...
      bytes32 public merkleRoot = 0x0a6bb41751ef037b9610011b3a591e6954bf48cad2d8bd88ea7ab83aa549b310;
    ...
    function publicMint(uint _qty) external payable publicMintingOpen {
      ...
      require(_qty <= maxItemsPerWLUser, "You can mint a maximum of 4 monkeys only");
    ...
    function whitelistMint(uint _qty, bytes32[] memory _merkleProof ) external payable whitelistMintingOpen {
      // Require nonzero amount
      bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
      require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "Your Proof is invalid");
      require(_qty > 0, "Sale Quantity must be greater than zero");
      require(_qty <= maxItemsPerWLUser, "You can mint a maximum of 4 monkeys only");
      // Check proper amount sent
...
/* require(finalTimestamp >= block.timestamp, "The mint has already closed");
    ...
    function whitelistUsers(address[] memory _users) external onlyOwner {
      require(_users.length > 0, "You havent entered any addresses");
      for (uint i = 0; i < _users.length; i++) {
        whitelistRemaining[_users[i]] = maxItemsPerWLUser;
      }
    }
