...
contract MadParrotCrew is ERC721Enumerable, Ownable {
    ...
    function whitelistMint(uint256 count, bytes32[] calldata _merkleProof)
        public
        payable
    {
        uint256 totalSupply = _owners.length;
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(whitelistSaleActive, "whitelist minting is inactive");
    ...
    function publicMint(uint256 count) public payable {
        uint256 totalSupply = _owners.length;
        require(!whitelistSaleActive, "whitelist minting is active");
        require(publicSaleActive, "public sale is inactive");
        require(totalSupply + count < maxSupply, "Exceeds max supply.");
    ...
    function setBaseURI(string memory _newBaseURI) public onlyOwner {
        require(!revealed, "cannot change URI post-reveal");
        baseURI = _newBaseURI;
    }
    ...
    //allows for tokens to be released in stages, but caps the total possibly supply to 10,000
    function setMaxSupply(uint256 _newMaxSupply) external onlyOwner {
        require(_newMaxSupply <= 10000, "Maximum tokens cannot exceed 10000");
