...
contract CrazyClown is Metadata, NFTExtension {
  ...
  // Public sale details
  uint256 public constant publicSaleSupply = 8000;
  ...
  uint256 public preSaleSupply = 1600;
  uint256 public preSalePrice = 0.04 ether;
  uint256 public preSaleMintLimit = 5; // Presale limit per wallet
  ...
  uint256 private reservedSupply = 96;
  ...
  function mint(uint256 _nbTokens, bool allowStaking) external payable override {
    ...
    if (!_publicSaleStarted && preSaleLive) {
      ...
      require(preSaleAmountMinted.current() + _nbTokens <= preSaleSupply, 'Exceeded presale supply limit');
  ...
  function setPreSaleSupply(uint256 _newpreSaleSupply) external onlyOwner {
    require(!_publicSaleStarted || !preSaleLive, 'Sale has already started');
