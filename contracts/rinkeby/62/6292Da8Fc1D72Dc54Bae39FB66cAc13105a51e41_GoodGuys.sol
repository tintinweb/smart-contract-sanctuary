...
contract GoodGuys is ERC721Enumerable, Ownable {
    ...
    error SoldOut();
    error ZeroNFTsRequested();
    error BuyLimitExceeded(uint256 packLimit, uint256 requested);
    ...
    error AmountExceedsBalance(uint256 balance, uint256 requested);
    error ETHTransferFailed();
    error UnknownTokenId(uint256 tokenId);
    error SaleNotOn();
    error ClaimPeriodHasEnded();
    error ClaimPeriodHasntEnded();
    error DuplicateClaim(uint256 ratId);
    error RatNotOwned(uint256 ratId);
    error OutOfClaims();
    error ZeroClaimCount();
    error ClaimReserveExceeded();
    error ImpossibleClaim(uint256 ratId);
    error TokenURILockedErr();

    // TODO: set provenance
    string public constant GG_PROVENANCE = "";
    ...
    uint256 private constant RECLAIM_RESERVE = 300;
    ...
    function mintGGs(uint256 numberOfGGs) public payable {
        _enforceSaleStarted();
        if (totalSupply() >= MAX_GG_SUPPLY) revert SoldOut();
        if (numberOfGGs == 0) revert ZeroNFTsRequested();
        if (numberOfGGs > GG_PACK_LIMIT)
            revert BuyLimitExceeded(GG_PACK_LIMIT, numberOfGGs);
        if (GG_PRICE * numberOfGGs != msg.value)
            revert InvalidETHAmount(GG_PRICE * numberOfGGs, msg.value);
        if (
            (MAX_GG_SUPPLY - ratClaimsReserve) -
                (totalSupply() - ratClaimsCounter) <
            numberOfGGs
        ) revert MintableSupplyExceeded();
    ...
    function reveal() public onlyOwner {
        if (startingIndex != 0) revert StartingIndexAlreadySet();
    ...
    function withdraw(uint256 amount) public onlyOwner {
        ...
        if (amount > address(this).balance)
            revert AmountExceedsBalance(address(this).balance, amount);
    ...
    function _enforceSaleStarted() internal view {
        if (!saleStarted) revert SaleNotOn();
    }

    function _enforceClaimPeriod(bool on) internal view {
        if (on) {
            if (!claimPeriodInProgress()) revert ClaimPeriodHasEnded();
        } else {
            if (claimPeriodInProgress()) revert ClaimPeriodHasntEnded();
        }
    }
    ...
    function _straightMintGG(address to, uint256 claimedRatId) internal {
        ...
        if (claimedRatId < MAX_RAT_SUPPLY) {
            ...
            if (ratClaims[claimedRatId]) revert DuplicateClaim(claimedRatId);
            // enforce ownership
            if (to != MDR.ownerOf(claimedRatId))
                revert RatNotOwned(claimedRatId);
    ...
    function _nextUnclaimedRatId() internal view returns (uint256 result) {
        ...
        if (result == 0) revert OutOfClaims();
    }

    function _enforceClaimConditions(uint256 count) internal view {
        if (count == 0) revert ZeroClaimCount();
        if (ratClaimsReserve < (ratClaimsCounter + count))
            revert ClaimReserveExceeded();
    }
        ...
        for (uint256 t = 0; t < tokens; t++) {
            if (MDR.tokenOfOwnerByIndex(msg.sender, t) == from) {
                for (uint256 f = t; f < (t + count); f++) {
                    ...
                    if (ratClaims[tokenId]) revert ImpossibleClaim(tokenId);
