...
contract blahtest2 is ERC721A, Ownable {
    ...
    constructor() ERC721A("blahtest2", "blahtest2", MAX_MINT_PER_TX) {}
    ...
    modifier isPaymentSufficient(uint256 quantity) {
        if (totalSupply() + quantity > 15 && totalSupply() < 15) {
            require(
                PRICE * (totalSupply() + quantity - 15) <= msg.value,
                "There was not enough/extra ETH transferred to mint an NFT."
            );
        } else if (totalSupply() + quantity > 15 && totalSupply() > 15) {
            require(
