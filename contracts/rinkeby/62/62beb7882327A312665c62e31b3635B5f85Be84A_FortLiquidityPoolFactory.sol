...
contract FortLiquidityPoolFactory is Ownable {
  ...
  constructor(
    ...
    totalAllocationPoint = totalAllocationPoint.add(5000);
  }
  ...
  function updatePool(uint256 _pid) public {
    ...
    uint256 fortReward = multiplier.mul(fortPerBlock).mul(pool.allocationPoint).div(totalAllocationPoint);
    pool.accumulatedFortPerShare = pool.accumulatedFortPerShare.add(fortReward.mul(1e12).div(lpSupply));
    pool.lastRewardBlock = block.number;
  }
  ...
  function pendingFort(uint256 _pid, address _user) external view returns (uint256) {
    PoolInfo storage pool = poolInfo[_pid];
    ...
    if (block.number > pool.lastRewardBlock && lpSupply != 0) {
      ...
      uint256 fortReward = multiplierBonus.mul(fortPerBlock).mul(pool.allocationPoint).div(totalAllocationPoint);
      accumulatedFortPerShare = accumulatedFortPerShare.add(fortReward.mul(1e12).div(lpSupply));
    }
    return (user.amountOfLpToken.mul(accumulatedFortPerShare).sub(user.rewardDebt)).div(1e12);
  }
  ...
  function deposit(uint256 _pid, uint256 _amount) public {
    ...
    require(_pid != 0 && _pid != 1, 'Invalid pool id!');
    PoolInfo storage pool = poolInfo[_pid];
    UserInfo storage user = userInfo[_pid][msg.sender];
    updatePool(_pid);
    if (user.amountOfLpToken > 0) {
      uint256 pending = (user.amountOfLpToken.mul(pool.accumulatedFortPerShare).sub(user.rewardDebt)).div(1e12);
      if(pending > 0) {
    ...
    user.rewardDebt = user.amountOfLpToken.mul(pool.accumulatedFortPerShare);
    ...
    if (user.amountOfLpToken > 0) {
      ...
      if(pending > 0) {
        if (block.number > endBlock) {
          bar.safeFortTransfer(msg.sender, pending.add(user.lock));
        } else {
  ...
  function depositEth() public payable {
    ...
    console.log(msg.sender, "depositEth", msg.value, "Factory");
    IFETH(FETH).deposit{ value: msg.value }();
    assert(IFETH(FETH).transfer(address(this), msg.value));
    user.amountOfLpToken = user.amountOfLpToken.add(msg.value);
    user.rewardDebt = user.amountOfLpToken.mul(pool.accumulatedFortPerShare);
  ...
  function withdrawEth(uint256 _amount) public {
    console.log(msg.sender, "withdrawEth", _amount, "Factory");
    PoolInfo storage pool = poolInfo[1];
    UserInfo storage user = userInfo[1][msg.sender];
    require(user.amountOfLpToken >= _amount, "Not enough funds!");
    updatePool(1);
    ...
    user.amountOfLpToken = user.amountOfLpToken.sub(_amount);
    user.rewardDebt = user.amountOfLpToken.mul(pool.accumulatedFortPerShare);
    IFETH(FETH).transfer(msg.sender, _amount);
  ...
  function _safeFortTransfer(address _to, uint256 _amount) internal {
    uint256 fortBalance = fortToken.balanceOf(address(this));
    if (_amount > fortBalance) {
      fortToken.transfer(_to, fortBalance);
    } else {
      fortToken.transfer(_to, _amount);
    }
  }

  function _safeFortBurn(uint256 _amount) internal {
    uint256 fortBalance = fortToken.balanceOf(address(this));
    if (_amount > fortBalance) {
      fortToken.burn(fortBalance);
    } else {
      fortToken.burn(_amount);
    }
  }
}
...
contract FortToken is
  ERC20,
  ERC20Burnable
{
  uint256 TOTAL = 21000000 * 10**uint256(18);
  constructor() ERC20('FortToken', 'FORT') public {
  ...
  */
    ...
    external
  {
    ...
    require(signatory != address(0), "FORT::delegateBySig: invalid signature");
    require(nonce == nonces[signatory]++, "FORT::delegateBySig: invalid nonce");
  ...
  function getPriorVotes(address account, uint blockNumber)
    ...
    returns (uint256)
  {
    require(blockNumber < block.number, "FORT::getPriorVotes: not yet determined");
  ...
  function _writeCheckpoint(
    ...
    internal
  {
    uint32 blockNumber = safe32(block.number, "FORT::_writeCheckpoint: block number exceeds 32 bits");
...
contract Bar is Ownable, ERC20('SyrupBar Token', 'SYRUP') {
