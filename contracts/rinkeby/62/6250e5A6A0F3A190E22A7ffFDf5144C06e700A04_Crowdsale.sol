...
contract Crowdsale is Ownable {
    ...
    uint256 public twoMonths = 60 minutes;
    ...
    function buyTokens(uint amount, address _paymentToken) public {
        ...
        require(block.timestamp < startDate + twoMonths, "Presale is over");

        uint usd_amount = amount / (uint(10) ** (IERC20Metadata(_paymentToken).decimals()));
        require(usd_amount >= 2000 && usd_amount <= 20000, "Wrong payment amount");
    ...
    function claim() public {
        require(block.timestamp >= startDate + twoMonths, 'Two months did not pass since the creation of the contract');
    ...
    function unlockedTokens(address _address) public view returns(uint256) {
        ...
        uint256 monthsPassed = ((block.timestamp - (startDate + twoMonths)) / oneMonth) + 1;
