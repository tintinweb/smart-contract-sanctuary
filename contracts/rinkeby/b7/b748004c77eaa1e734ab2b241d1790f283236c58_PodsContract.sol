...
// File contracts/hypepods.sol

pragma solidity ^0.8.4;


// 
// Hype Mansion Club | HypePods Contract
// 
contract HypePods is ERC721, Ownable {
    string internal baseUri;

    uint256 public cost = 0.07777 ether;
    ...
    address private author = 0x94222E234ac1a4a3c5D50b46072087eAE029Ff7B;
    ...
    function setCommission(uint32 _commision) public onlyOwner {
    ...
    modifier preMintChecks(uint32 count) {
        require(count > 0, "Mint value of at least 1 is required.");
        require(count < maxPerMint + 1, "Max mint reached.");
        require(msg.value >= cost * count, "Not enough funds.");
        require(supply + count < totalSupply + 1, "Mint has sold out.");
        require(
...
contract PodsContract is HypePods {
    constructor(
        string memory _uri,
        string memory _name,
        string memory _symbol,
        ...
        uint256 _cost,
        bool _open
    ) HypePods(_uri, _name, _symbol, _totalSupply, _cost, _open) {}
    ...
    {
        require(_tokenId <= supply, "Pods not minted yet");
