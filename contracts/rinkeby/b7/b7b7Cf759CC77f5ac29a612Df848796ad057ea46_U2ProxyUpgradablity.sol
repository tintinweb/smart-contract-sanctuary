...
contract U2ProxyUpgradablity is Ownable {
  ...
  function getOneDayReward(
    ...
    uint256 poolShare = stakedAmount.div(totalStake);
    return dailyReward.mul(poolShare);
  }
  ...
  function getRewards(address tokenAddress) public view returns (uint256) {
    uint256 totalStaking = getTotalStaking(tokenAddress);
    uint256 contractBalance = IERC20(tokenAddress).balanceOf(address(this));
    (, uint256 rewards) = contractBalance.trySub(totalStaking);
    return rewards;
  }

  function deriveDailyRewards(uint256 rewards) public view returns (uint256) {
    uint256 rewardPerPoolShare = rewards.div(6);
    return rewardPerPoolShare.div(u2.stakeDuration());
  }
  ...
  function _rewardCalculation(
    ...
    uint256 availableReward = getRewards(stakedToken);
    uint256 dailyRewards = deriveDailyRewards(availableReward);

    rewardsEarned = noOfDays.mul(
      getOneDayReward(dailyRewards, stakedAmount, totalStake)
    );
    ...
    while (i < intervalDays.length) {
      if (noOfDays >= intervalDays[i]) {
        ...
        uint256 availableRewardForOther = getRewards(rewardToken);
        uint256 dailyRewardsForOther = deriveDailyRewards(
          availableRewardForOther
        );
        ...
        if (
          ...
          rewardsEarned = balDays.mul(
            getOneDayReward(dailyRewardsForOther, stakedAmount, totalStake)
          );
          //  Rewards Send
