...
contract Bobos is ERC721, Ownable {
    ...
    uint256 _presaleprice = 0.0375 ether;
    uint256 tokenSupply = 6600; // 6599 Bobos
    uint256 presaleSupply = 2500; // 2500 Bobos
    ...
    constructor() ERC721("Bobos", "BBs") {
        setBaseURI("https://nftscreen.art/metadata/");
    ...
    function mintPresale(uint256 mintCount, bytes32[] memory proof) external payable {
        ...
        require((_tokenIdCounter.current() + mintCount) <= presaleSupply, "Presale supply exceeded");
        require(_verify(_leaf(msg.sender), proof), "Invalid merkle proof");
        uint256 ownerMintedCount = addressMintedBalance[msg.sender];
        require(ownerMintedCount + mintCount <= _perWalletPresaleLimit, "max Bobos per wallet exceeded");
    ...
    function mintPublic(uint256 mintCount) external payable {
        ...
        uint256 ownerMintedCount = addressMintedBalance[msg.sender];
        require(ownerMintedCount + mintCount <= _perWalletLimit, "max Bobos per wallet exceeded");
...
// Change root of the tree
    ...
    function mintOwnerPublic(address addr, uint256 mintCount) external onlyOwner {
        require((_tokenIdCounter.current() + mintCount) <= tokenSupply, "Sold out! No more Bobos are available");
        mint(addr,mintCount);
    }

    function mintOwnerPresale(address addr, uint256 mintCount) external onlyOwner {
        require((_tokenIdCounter.current() + mintCount) <= presaleSupply, "Bobo presale supply exceeded");
        mint(addr,mintCount);
    }    
}
