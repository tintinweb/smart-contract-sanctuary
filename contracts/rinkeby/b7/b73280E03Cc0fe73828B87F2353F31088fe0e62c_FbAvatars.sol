...
contract FbAvatars is ERC721Enumerable, Ownable, AccessControl {
    ...
    function mint(uint256 num) public payable whenMintNotPaused(){
        ...
        require( tokenCount + num <= 10,                                           "FbAvatars: You can mint a maximum of 13 NFTs per wallet" );
        require( supply + num <= TOTAL_NUMBER_OF_TOKENS - giveaway_reserved,       "FbAvatars: Exceeds maximum supply" );
        require( msg.value >= PRICE * num,                                         "FbAvatars: Ether sent is less than PRICE * num" );
    ...
    function giveAway(uint256 num, uint8 n) external onlyRole(WHITE_LIST_ROLE) {
        require(giveaway_reserved >= n, "FbAvatars: Exceeds giveaway reserved supply" );
        giveaway_reserved -= n;
        uint256 supply = totalSupply();
        for(uint256 i; i < num; i++){
          _safeMint( msg.sender, supply + i);
