...
interface IMedia {
    ...
    function authorized(address _address) external returns(bool);
...
contract AuctionHouse is IAuctionHouse, ReentrancyGuard {
    ...
    function endAuction(uint256 auctionId) external override auctionExists(auctionId) nonReentrant {
        ...
        if(IMedia(media).owner() == auctions[auctionId].tokenOwner || IMedia(media).authorized(auctions[auctionId].tokenOwner)){
