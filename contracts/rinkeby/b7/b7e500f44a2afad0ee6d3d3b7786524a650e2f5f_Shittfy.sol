...
contract TradeableERC721Token is ERC721Full, Ownable {
  ...
  string public LP_POOL_BONUS;
  uint256 public XTM_LOCKED;
  bytes32[] public IPFS_HASH;
  string[] public metadataDetails;
  ERC20 public token;
  ...
  constructor(string memory _name, string memory _symbol, uint256 _supply, address _proxyRegistryAddress, string memory _LP_POOL_BONUS, uint256 _XTM_LOCKED, address _ERC20_) ERC721Full(_name, _symbol, _supply) public {
    proxyRegistryAddress = _proxyRegistryAddress;
    tsupply = _supply.sub(1);
    LP_POOL_BONUS = _LP_POOL_BONUS;
    XTM_LOCKED = SafeMath.div(_XTM_LOCKED,_supply);
    token = ERC20(_ERC20_);
  }
  ...
  function mintTo(address _to, bytes32 _IPFS_HASH, string memory _metadataDetails) public onlyOwner {
    if(_currentTokenId <= tsupply){
        uint256 newTokenId = _getNextTokenId();
        ERC721.approve(_to, newTokenId);
  ...
  function burn(uint256 _tokenidentiy) public {
      ...
      token.transfer(msg.sender, XTM_LOCKED);
  ...
  /**
    ...
    * @return 
    */
  function _getDetails(uint256 _tokenidentiy) public view returns(bytes32 _IPFS_HASH, string memory _metadataDetails) {
  ...
  /**
    ...
    * @return nothing
    */
  function _setDetails(uint256 _tokenidentiy, bytes32 _IPFS_HASH, string memory _metadataDetails) public onlyOwner {
...
// File: contracts/Shittfy.sol

/**
 * @title Collectable Asset
 * Collectable Asset - A contract for easily creating custom assets for your collection. 
 */
contract Shittfy is TradeableERC721Token, ReentrancyGuard {
  ...
  constructor(
    ...
    string memory _lp_pool_bonus,
    uint256 _xtm_locked,
    address _ERC20
    
  ) TradeableERC721Token(_name, _symbol, _supply, _proxyRegistryAddress, _lp_pool_bonus, _xtm_locked, _ERC20) public {
