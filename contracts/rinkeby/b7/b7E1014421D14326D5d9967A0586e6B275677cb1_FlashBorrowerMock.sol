...
contract FlashBorrowerMock is IERC3156FlashBorrower {
    ...
    function flashBorrow(address token, uint256 amount) public {
        ...
        address vaultAddress = vaultFactory.tokenToVault(token);
        uint256 _allowance = stakedToken.allowance(address(this), vaultAddress);
        uint256 _fee = lender.flashFee(token, amount);
        uint256 _repayment = amount + _fee;
        stakedToken.approve(vaultAddress, _allowance + _repayment);
