...
contract Driplet is ERC721Tradable {
    using localStrings for string;
    using SafeMath for uint256;

    string public contractMeta;

    constructor(address _proxyRegistryAddress) ERC721Tradable("Driplet", "DRIPLET", _proxyRegistryAddress){}
    ...
    function drip(address _recipient) public onlyOwner{
      uint256 nextId = totalSupply().add(1);
      string memory index = localStrings.strConcat(localStrings.uint2str(nextId), ".json");
      mintTo(_recipient);
      _setTokenURI(nextId, index);
    }
}
