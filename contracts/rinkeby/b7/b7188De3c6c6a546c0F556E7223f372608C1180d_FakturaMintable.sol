...
UUPSUpgradeable
{
    ...
    modifier restricted(uint256 amount) {
    ...
    function safeMint(address to) public payable {
        ...
        _setTokenURI(_tokenIdCounter.current(), string(abi.encodePacked(metadataURI, _tokenMetadata[0].id, ".json")));
    ...
    function safeBatchMint(address to, uint256 amount) public payable restricted(amount) {
        if(mintPrice > 0) {
            require(mintPrice == msg.value * amount, "Mint price is not correct.");
            _payout();
        }
        //Suffle Metadata
        _shuffle();
        ...
        for (uint256 i = 0; i < amount; i++) {
            _safeMint(to, _tokenIdCounter.current());
            _setTokenURI(_tokenIdCounter.current(), string(abi.encodePacked(metadataURI, _tokenMetadata[i].id, ".json")));
