...
contract TinyKingdoms is ERC721Enumerable,ReentrancyGuard,Ownable {
    ...
    function getThemeIndex (uint256 tokenId) internal pure returns (uint256){
        ...
        if      (rand <115){themeIndex = 0;} 
        else if (rand <230){themeIndex = 1;} 
        else if (rand <345){themeIndex = 2;} 
        else if (rand <460){themeIndex = 3;} 
        else if (rand <575){themeIndex = 4;} 
        else if (rand <690){themeIndex = 5;} 
        else if (rand <805){themeIndex = 6;} 
        else if (rand <930){themeIndex = 7;} 
        else if (rand <990){themeIndex = 8;} 
        else {themeIndex = 9;}
        return themeIndex;
    }
    ...
    function getKingdomOutput (uint256 tokenId) public view returns (string memory){
      ...
      output = string(abi.encodePacked(output,'<text class="place" x="113" y="',Strings.toString(y),'">',line,'</text>'));
      if (y > 500) break;
    ...
    function getFlagSVG(TinyFlag memory flag, uint256 tokenId) internal view returns (string memory){
        ...
        // parts[9]=' <polygon class="shadow" points="112.5 365.5 112.5 87.92 108 97 108 370 490 370 494.67 365.5 112.5 365.5" style="filter: url(#displacementFilter)"/> <text class="place" transform="translate(113 448.72)">'; testing new outputs
