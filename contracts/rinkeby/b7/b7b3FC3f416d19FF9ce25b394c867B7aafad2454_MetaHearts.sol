...
contract MetaHearts is ERC721, ERC721Enumerable, ERC721Burnable, Ownable, Pausable {
    ...
    uint256 public constant MAX_ALLOW_LIST = 1000;

    uint8 public constant MAX_PRESALE_MINT = 4;
    uint8 public constant MAX_MINT = 20;

    uint256 public step_limit = 1000;
    uint256 public allowPrice = 1 * 10**16;
    uint256 public heartPrice = 3 * 10**16;
    ...
    event CreateHeart(uint256 indexed id);
    event HeartPriceAllowChanged(uint256 price);
    event HeartPriceChanged(uint256 price);
    event CurrentLimitChanged(uint256 limit);

    constructor() ERC721("TestMH", "TESTMHRT") {
    }
    ...
    function mint(address _to, uint8 _count) external payable {
    ...
    function mintAllow(address _to, uint8 _count) external payable {
        uint256 total = _totalSupply();
        require(preSaleIsActive, "Pre sale must be active");
        require(total + _count <= MAX_ELEMENTS, "Max limit");
        require(_count <= MAX_MINT, "Exceeds number");
        require(total + _count <= step_limit, "Current stage limit");
    ...
    function selfAddToPresaleAllow(address _address) public payable {
        ...
        _allowList[_address] = MAX_PRESALE_MINT;
    }
    ...
    function setPriceAllow(uint256 _price) external onlyOwner {
        ...
        allowPrice = _price;
        emit HeartPriceAllowChanged(_price);
    }
    ...
    function setCurrentLimit(uint256 _limit) external onlyOwner {
        ...
        step_limit = _limit;
        emit CurrentLimitChanged(_limit);
    }
