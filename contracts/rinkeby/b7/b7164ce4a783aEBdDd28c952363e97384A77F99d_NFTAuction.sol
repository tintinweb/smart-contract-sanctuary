...
contract NFTAuction is Context, ReentrancyGuard {
    ...
    /// @notice global first platform fee, assumed to always be to 1 decimal place i.e. 50 = 5.0%
    uint256 public firstPlatformFee = 50;

    /// @notice global send sale platform fee, assumed to always be to 1 decimal place i.e. 25 = 2.5%
    uint256 public secondPlatformFee = 25;
    ...
    function resultAuction(string memory _igUrl, address _seller) external nonReentrant {
        ...
        if (auctionDetail.tokenId == 0)
        {
            ...
            platformFeeInETH = winningBid.mul(firstPlatformFee).div(maxShare);
        }
        else {
            ...
            platformFeeInETH = winningBid.mul(secondPlatformFee).div(maxShare);
    ...
    function _refundHighestBidder(address payable _currentHighestBidder, uint256 _currentHighestBid) private {
        ...
        // Work out platform fee from above reserve amount
        uint256 platformFeeInETH = _currentHighestBid.mul(firstPlatformFee).div(maxShare);
