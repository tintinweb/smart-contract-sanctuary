...
contract RoyaltySplitterFactory {
    ...
    function createRoyaltySplitter(address[] memory participants, uint256[] memory cuts, string memory name) external {
        RoyaltySplitter royaltySplitter = new RoyaltySplitter();
        royaltySplitter.initiate(participants, cuts, name);
...
interface IRoyaltySplitter {
    function initiate(address[] memory participants, uint256[] memory cuts, string memory _name) external;
...
contract RoyaltySplitter is IRoyaltySplitter, IERC165, ReentrancyGuard {
    ...
    function initiate(address[] memory participants, uint256[] memory cuts, string memory _name) override external {
        name = _name;
        ...
        for (uint256 i; i < participants.length; i++) {
            participantsCut.push(ParticipantCut(participants[i], cuts[i], ERC165Checker.supportsInterface(participants[i], type(IRoyaltySplitter).interfaceId)));
            sum += cuts[i];
