...
contract PopReward is Ownable, IRewardManager {
    ...
    function getPoolSupply(address pool) public view override returns(uint) {
        return poolInfo[pidByAddress[pool]].lpSupply;
    }

    function getUserAmount(address pool, address user) public view override returns(uint) {
    ...
    function _notifyDeposit(address _account, PoolInfo storage pool, UserInfo storage user, uint256 _amount) private {
        if (user.amount > 0) {
            uint256 pending =
    ...
    function notifyWithdraw(address _account, uint256 _amount) public override {
        ...
        UserInfo storage user = userInfo[_pid][_account];
        updatePool(_pid);
        _notifyWithdraw(_account, pool, user, _amount);
        emit Withdraw(_account, _pid, _amount);

    }

    function _notifyWithdraw(address _account, PoolInfo storage pool, UserInfo storage user, uint256 _amount) private {
    ...
    function withdrawPop(uint256 _amount) public {
        UserInfo storage user = popStaker[msg.sender];
        updatePopVault();
        _notifyWithdraw(msg.sender, popVault, user, _amount);
        pop.transfer(msg.sender, _amount);
    ...
    function setPopVaultStartBlock(
        uint256 _startBlock
    ) external onlyOwner {
...
contract PopToken is ERC20("POP", "POP"), IMintableERC20, Ownable  {
