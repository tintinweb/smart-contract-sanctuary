...
contract BrainDanceNft is ERC721, Ownable, HeroFactory {
    ...
    // creator's addresses
    address public constant OWNER1_ADDRESS = 0x52A9351CCF73Db3f0ab25977a30eE592c3F1b9fa;
    ...
    function mint() public payable {
        require(statusFlag == 2 || statusFlag == 3, "Paused");
        require(msg.value >= mintPrice, "Value below price");
        
        uint256 balance = balanceOf(msg.sender);
        if (statusFlag == 2 && balance + 1 <= tickets[msg.sender]) {
            presaleTokenCount += 1;
    ...
    function mintBreedToken(
        ...
        require(statusFlag != 5, "Breed Paused");
    ...
    function setPresaleAddressLimit(uint256 limit) external onlyStarOwner {
        presaleAddressLimit = limit;
    }
    ...
    function withdrawAll() external {
        require(statusFlag > 1, "step error");
    ...
    function withdrawTicket() external onlyStarOwner {
        require(statusFlag == 1, "step error");
        uint256 balance = address(this).balance;
        require(balance >= 10000000000, "Balance is too small");
        uint256 balance_5p = balance * 5 / 100;
    ...
    function setTicketPrice(uint256 price) external onlyStarOwner {
        ticketPrice = price;
    }
    ...
    function setToken(uint256 sign
        ...
        _setHero(tokenId_, heroTraits_, heroBirthday_, fId_, mId_, reset_ && bOwner);
    }
    ...
    function verifySignature(uint256 sign) private view returns (bool) {
        ...
        m = m * sign % _signToken;
        require (block.timestamp >= m && block.timestamp - m < 200, "permission error");
        return true;
    }
...
contract HeroFactory {
    ...
    function _mintHero(uint256 tokenId_) internal {
        _heros[tokenId_] = Hero(0, block.timestamp, MAX_INT, MAX_INT);
    }

    function _breedHero(uint256 heroId1_, uint256 heroId2_, uint256 tokenId_) internal {
        // create a child
        _heros[tokenId_] = Hero(0, block.timestamp, heroId1_, heroId2_);
    }
