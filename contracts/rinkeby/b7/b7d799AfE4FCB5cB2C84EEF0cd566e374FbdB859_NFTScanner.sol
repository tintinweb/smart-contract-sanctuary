...
contract NFTScanner is Context {
        ...
        for (
            ...
            if (_nftTokenIds.length != 0) {
                for (uint256 i = 0; i < _nftTokenIds.length; i++) {
                    ...
                    ogNFTStakingInfo[current] = OGNFT({
                        nftAddress: address(ogNFT),
                        nftCategoryId: _nftCategoryId,
                        nftTokenId: _nftTokenId,
                        isStaking: true
    ...
    function getOGTokenInfo(
        address _nftAddress,
        uint256 _nftCategoryId,
        uint256 _nftTokenId,
        address _user
    ) external returns (OGTokenInfo memory) {
        address _owner = ogNFT.ownerOf(_nftTokenId);
        address _approvedAddress = ogNFT.getApproved(_nftTokenId);
        ...
        return
            OGTokenInfo({
                ...
                isApproved: _approvedAddress == address(ogNFT),
                isStakingIn: _owner == address(ogNFT)
            });
    }
