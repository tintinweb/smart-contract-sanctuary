...
contract TestNFT is ERC721Burnable, Ownable {
    ...
    // This is the provenance record of all NFT artwork in existence
    ...
    function setOffsetIndexBlock() internal {
        ...
        */
        if (offsetIndexBlock == 0 && (totalSupply() >= MAX_SUPPLY || block.timestamp >= REVEAL_TIMESTAMP)) {
    ...
    */
    function mintNFT(uint256 numberOfNfts) public isWhaleProtection(numberOfNfts) payable {
        ...
        require(totalSupply().add(numberOfNfts) <= MAX_SUPPLY - reserved, "over the supply");
    ...
    function communityReserve(address _whitelist) public {
        ...
        require(info.MAX_SUPPLY > 0, "not whitelisted");
        require(reserved > 0 && info.totalSupply < info.MAX_SUPPLY, "fully adopted");
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if ((totalSupply() >= MAX_SUPPLY || block.timestamp >= REVEAL_TIMESTAMP)) {
    ...
    function setWhitelist(address[] calldata _whitelist, uint256[] calldata _maxSupply) public onlyOwner {
        uint256 len = _whitelist.length;
        require(_maxSupply.length == len, "bad length");
        for (uint256 idx = 0; idx < len; idx++) {
            whitelist[_whitelist[idx]] = Whitelist({
                MAX_SUPPLY: uint256(_maxSupply[idx]),
