...
contract SquidMetaGame is Initializable {
   ...
   //Destroy a transaction event
    ...
    //Price Change Event
    event PriceChange(address operator,uint price_);

    event Extract(address operator,uint nonce_);
    ...
    function extract() public{
        uint _extractnum = extractnum ++;
        SquidPieces.transferFrom(msg.sender, _recipientAddress, price);
        emit Extract(msg.sender, _extractnum);
    }
    ...
    function withdraw(
        ...
        require(SquidPieces.balanceOf(address(this)) >= amount, "stake insufficient");
