...
interface IERC21 {
     ...
     * @dev Gets fee
    */
    function gasData(string memory chain) external view returns (GasData memory);
    ...
    */ 
    ...
    /**
     * @dev Set the required gas limit for cross transfer
     */
    function setGasData(string memory chain, uint256 gasLimit, uint256 gasPrice) external returns (bool);
...
contract ERC21 is Context, IERC21, IERC21Metadata, Ownable, RecoverableToken, WithdrawableToken {
    ...
    mapping (string => GasData) private _gasData;
     ...
     * @dev Gets gas limit
    */
    function gasData(string memory chain) public view virtual override returns (GasData memory) {
        return _gasData[chain];
    }
    ...
    */
    function getFee(string memory chain, uint256 value) public view returns(int){
         // Get the price of contract to other chain
        int price = getPrice(chain);
        
        // Get the gas data to estimate price with
        GasData memory data = _gasData[chain];
        
        // Calculate amount to cover fee
        int fee = price * int(data.gasLimit) * int(data.gasPrice);
    ...
    * @dev Gets a fee for the movememnt to another chain
    */
    function test(string memory chain, uint256 value) public view returns(int){
        // Get the price of contract to other chain
        return getPrice(chain);
    }
    
    /*
    * @dev Gets a price from price feed
    */
    function getPrice(string memory chain) internal view returns (int) {
        (
            uint80 roundID, 
            int price,
            uint startedAt,
            uint timeStamp,
            uint80 answeredInRound
        ) = _priceFeeds[chain].latestRoundData();
        
        return price;
    }
    
    /*
    * @dev Updates a price feed
    */
    function updatePriceFeed(string memory chain, address contractAddress) public onlyOwner{
        _priceFeeds[chain] = AggregatorV3Interface(contractAddress);
    }
    ...
    */ 
    function moveToken(uint256 amount, string memory chain) public virtual payable override returns (bool){
        ...
        require(_gasData[chain].gasLimit == 0, "Chain is not supported");
    ...
    */ 
    ...
    /**
     * @dev Set the gas data required for cross transfer
     */
    function setGasData(string memory chain, uint256 gasLimit, uint256 gasPrice) public virtual override onlyOwner returns (bool) {
        _gasData[chain] = GasData(gasLimit, gasPrice);
        return true;
    }
