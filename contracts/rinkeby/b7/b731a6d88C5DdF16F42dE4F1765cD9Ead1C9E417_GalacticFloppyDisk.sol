...
contract GalacticFloppyDisk is Ownable, ERC721Enumerable {
  ...
  string[] private boringSoftware = [
    ...
    "Telegard", "Turbo Assembler", "XTree", "GALACT~1.EXE"
  ];
  ...
  mapping (uint256 => colorSet) private colors;
  mapping(uint256 => bytes32) private genes;
  mapping(uint256 => string) private infectedDisks;
  mapping(uint256 => bool) private insecureDisks;
  mapping(uint256 => uint8) private wackyPatterns;
  mapping(uint256 => BoringSoftware[]) private initialSoftware;

  mapping(uint256 => NFTContract) private contracts;
  string[] private viruses = [
  ...
  {
    console.log("NFT Name:", _NFT_NAME, "- Symbol:", _NFT_SYMBOL);
    console.log("Supply:", _AVAILABLE_SUPPLY, "- MAX PER:", _MAX_PER_ADDRESS);
  ...
  function flipSaleState() public onlyOwner {
    console.log("Setting sale state to", !saleActive);
  ...
  function setNFTContract(address contractAddress, string memory contractName, string memory softwareName) public onlyOwner {
    console.log("Adding a new NFT contract %s at %s will supply %s", contractName, contractAddress, softwareName);
  ...
  function mint() external {
    ...
    uint256 _newAmountMinted = _currentTokenCount.add(1);
    uint256 _walletOwns = balanceOf(_msgSender());
    uint256 _newWalletOwns = _walletOwns.add(1);

    console.log(_currentTokenCount, "floppies have already been minted.");
    console.log("We can only mint", AVAILABLE_SUPPLY, "floppies.");
    console.log("If we mint this floppy, there will be", _newAmountMinted, "floppies minted.");
    console.log("Can we mint?", _newAmountMinted <= AVAILABLE_SUPPLY);
    console.log("This address has minted", _walletOwns, "floppies");
    ...
    _safeMint(msg.sender, _currentTokenCount);
    _tokenCounter.increment();

    console.log("There are now", _newAmountMinted, "floppies minted");

    // Generate 32 bytes of "random" data by hashing some things
    genes[_currentTokenCount] = keccak256(abi.encodePacked(
      block.timestamp,
      block.difficulty,
      _currentTokenCount,
      _msgSender(),
    ...
    colors[_currentTokenCount] = colorSet({
      color1: bytes3(
        bytes2(genes[_currentTokenCount][0]) |
        bytes2(genes[_currentTokenCount][1]) >> 8 |
        bytes3(genes[_currentTokenCount][2]) >> 16
      ),
      color2: bytes3(
        bytes2(genes[_currentTokenCount][3]) |
        bytes2(genes[_currentTokenCount][4]) >> 8 |
        bytes3(genes[_currentTokenCount][5]) >> 16
      ),
      color3: bytes3(
        bytes2(genes[_currentTokenCount][6]) |
        bytes2(genes[_currentTokenCount][7]) >> 8 |
        bytes3(genes[_currentTokenCount][8]) >> 16
      ),
      color4: bytes3(
        bytes2(genes[_currentTokenCount][9]) |
        bytes2(genes[_currentTokenCount][10]) >> 8 |
        bytes3(genes[_currentTokenCount][11]) >> 16
      ),
      color5: bytes3(
        bytes2(genes[_currentTokenCount][12]) |
        bytes2(genes[_currentTokenCount][13]) >> 8 |
        bytes3(genes[_currentTokenCount][14]) >> 16
      )
    });

    // Determine if disk is infected
    uint16 infectedDisk = uint16(
      uint8(genes[_currentTokenCount][15]) |
      uint16(uint8(genes[_currentTokenCount][16])) << 8
    );

    uint16 insecureDisk = uint16(
      uint8(genes[_currentTokenCount][17]) |
      uint16(uint8(genes[_currentTokenCount][18])) << 8
    );

    uint16 wackyPattern = uint16(
      uint8(genes[_currentTokenCount][19]) |
      uint16(uint8(genes[_currentTokenCount][20])) << 8
    );

    uint boringSoftwares = (uint8(
      genes[_currentTokenCount][21]
    ) % 2) + 1; // How many boring softwares are ALWAYS on the disk.

    // Pick some rando boring software
    for (uint256 i = 0; i < boringSoftwares; i++) {
      uint256 boringIdx = (uint8(genes[_currentTokenCount][22 + i])) % boringSoftware.length;
      // Max 12 disks - same software always has the same # of disks
      uint8 diskCount = (uint8(boringIdx) % 11) + 1;
      // Disk 1-12
      uint8 diskNumber = (uint8(genes[_currentTokenCount][23 + i]) % diskCount)+ 1;
      initialSoftware[_currentTokenCount].push(BoringSoftware({
        idx: boringIdx,
        diskCount: diskCount,
    ...
    // value can range from 0-65534
    if (wackyPattern > WACKY_THRESHHOLD) {
      wackyPatterns[_currentTokenCount] = uint8(wackyPattern % 3) + 1;
    }

    // value can range from 0-65534
    if (infectedDisk > INFECTED_THRESHHOLD) {
      string memory virusName = viruses[infectedDisk % viruses.length];
      infectedDisks[_currentTokenCount] = virusName;
    }

    // value can range from 0-65534
    if (insecureDisk > INSECURE_THRESHHOLD) {
      insecureDisks[_currentTokenCount] = true;
    }
    emit FloppyMinted(_msgSender(), _currentTokenCount);
  }
  ...
  function tokenURI(uint256 id) public view override returns (string memory) {
      ...
      colorSet memory tokenColors = colors[id];
      FileListing memory files = getFiles(owner, id);
      string memory traits = getTraits(tokenColors, id, files);

      uint8 wackyPattern = wackyPatterns[id];
      string memory name = string(abi.encodePacked('Galactic Floppy #', id.toString()));
      string memory license = '';
      string memory description = string(abi.encodePacked('This Floppy Is Radical! ', license));
      string memory image =Base64.encode(bytes(SVGBuilder.generateSVGofToken(tokenColors, files, wackyPattern)));
  ...
  function getTraits(colorSet memory tokenColors, uint256 tokenId, FileListing memory files) internal view returns (string memory) {
    string memory infectedWith = infectedDisks[tokenId];
    bool isInsecure = insecureDisks[tokenId];
    uint256 wackyPattern = wackyPatterns[tokenId];
    ...
    for (uint256 i = 0; i < files.boringSoftwares.length; i++) {
      if (files.boringSoftwares[i] > 0) {
        console.log("Getting boring file %s - %s", i, files.boringSoftwares[i]);
        traits = string(abi.encodePacked(
          ...
          '", "value": "',
          boringSoftware[files.boringSoftwares[i] - 1],
          '"}'
        ));
      }
    }
    ...
    for (uint256 i = 0; i < files.specialSoftwares.length; i++) {
      if (files.specialSoftwares[i] > 0) {
        traits = string(abi.encodePacked(
          ...
          '", "value": "',
          coolSoftware[files.specialSoftwares[i] - 1].softwareName,
          '"}'
        ));
      }
    }
    ...
    console.log("Traits are %s", traits);
    return traits;
  }

  function getFiles(address owner, uint256 tokenId) internal view returns (FileListing memory) {
    uint256 fileCount;
    uint256 yPos;
    console.log("There are %s configured external contracts to check", coolSoftware.length);
    FileListing memory fl;

    for (uint256 i = 0; i < initialSoftware[tokenId].length; i++) {
      console.log("Adding in baked in software for %s", tokenId);
      ...
      fl.textElements = string(abi.encodePacked(
        ...
        yPos.toString(),
        "'>* ",
        boringSoftware[initialSoftware[tokenId][i].idx],
        " disk ",
        initialSoftware[tokenId][i].diskNumber.toString(),
        "/",
        initialSoftware[tokenId][i].diskCount.toString(),
        "</text>"
      ));
      fl.boringSoftwares[i] = initialSoftware[tokenId][i].idx + 1;
      fileCount++;
    }
    ...
    for (uint256 i = 0; i < coolSoftware.length; i++) {
      console.log("Checking ownership of %s token", coolSoftware[i].contractName);
      uint256 ownerBalance = coolSoftware[i].contractInterface.balanceOf(owner);
      if (ownerBalance > 0) {
        if (fileCount < 7) {
          console.log("Wallet %s owns %s %s tokens", owner, ownerBalance.toString(), coolSoftware[i].contractName);
          yPos = fileCount * 23 + 32;
          fl.textElements = string(abi.encodePacked(
            fl.textElements,
            "<text class='t' x='49' y='",
          ...
          fl.specialSoftwares[i] = i + 1;
          fileCount++;
        } else {
          console.log("The disk is full!");
        }
      } else {
        console.log("Wallet %s doesn't own a %s NFT", owner, coolSoftware[i].contractName);
      }
    }
...
struct FileListing {
  string textElements;
  uint256[10] boringSoftwares;
  uint256[10] specialSoftwares;
}
