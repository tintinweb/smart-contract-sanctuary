...
interface OwlTokenInterfaceForAlphaPass {
  function onAlphaPassMint(address addr, uint256 amount) external;
} 
...
contract AlphaPass is ERC721Enumerable, CollaborativeOwnable {
  ...
  uint256 public mintPrice = 112 ether;
  ...
  function mint() external {
    require(mintIsActive, "unactive");

    uint256 ts = totalSupply();
    require(ts < maxSupply, "supply");

    OwlTokenInterfaceForAlphaPass(owlToken).onAlphaPassMint(_msgSender(), mintPrice);
  ...
  function addToWhitelist(address[] memory data) external onlyCollaborator {
    for (uint256 i = 0; i < data.length; i++) {
      _whiteList[data[i]] = true;
    }
  }

  function removeFromWhitelist(address[] memory data) external onlyCollaborator {
    for (uint256 i = 0; i < data.length; i++) {
      _whiteList[data[i]] = false;
    }
  }
}
