...
contract PropositionToken is ERC721Enumerable, Ownable {
    ...
    string[10] public heavenlyBodySecondaryColors = [
        '#ffd700', // light gold
        '#20d1cc', // med aqua
        '#20d186', // med green
        '#62b4f7', // light blue
    ...
    */
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        // string memory heavenlyBodySecondary = heavenlyBodySecondaryColors[getIndexes(imageCode, 3)];
        if (imageCode % 2 == 0) {
            heavenlyBody = bodyColors[getIndexes(imageCode, 2)];
        } else {
            heavenlyBody = heavenlyBodySecondaryColors[getIndexes(imageCode, 2)];
        }

        // if groundColor === white then make text black
        string memory textColor = '#ffffff';
        if (keccak256(abi.encodePacked(bodyColors[getIndexes(imageCode, 1)])) == keccak256(abi.encodePacked(textColor))) {
            textColor = skyColors[getIndexes(imageCode, 0)];
        }

        // start svg
        string memory output = string(abi.encodePacked('<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><defs><linearGradient id="rectGradient" gradientTransform="rotate(90)"><stop offset="69%"  stop-color="',skyColors[getIndexes(imageCode, 0)],'" /><stop offset="79%" stop-color="',bodyColors[getIndexes(imageCode, 1)],'" /></linearGradient><linearGradient id="circleGradient" gradientTransform="rotate(315)"><stop offset="10%" stop-color="',heavenlyBody,'" /><stop offset="90%" stop-color="',heavenlyBodySecondaryColors[getIndexes(imageCode, 3)],'" /></linearGradient></defs><style>.title { fill: ',textColor,'; font-family: Liberation Mono; font-size: 22px; } .base { fill: white; font-family: Liberation Mono; font-size: 18px; font-weight: 300; }</style><rect width="100%" height="100%" fill="url(#rectGradient)" /><circle cx="275" cy="150" r="50" fill="url(#circleGradient)" />'));
        ...
        string memory jsonMeta = string(abi.encodePacked('{"name": "Proposition ',toString(tokenId),': ',_proposition,'", "description": "Beliefs are not private. Beliefs are not free.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '",'));
        jsonMeta = string(abi.encodePacked(jsonMeta, ' "attributes": [{ "trait_type": "Image DNA", "value": "',toString(imageCode),'" }, { "trait_type": "Sky Color", "value": "',skyColors[getIndexes(imageCode, 0)],'" }, { "trait_type": "Ground Color", "value": "',bodyColors[getIndexes(imageCode, 1)],'" }, { "trait_type": "Heavenly Body Color", "value": "',heavenlyBody,'" }, { "trait_type": "Heavenly Body Secondary Color", "value": "',heavenlyBodySecondaryColors[getIndexes(imageCode, 3)],'" }]}'));
