...
contract IDXYS is Ownable{
   ...
   // mapping(address => uint256) public tier;                                    // user => tier
    ...
    /// @notice Return the balance of eanc vault for an asset
    /// @dev Can be called on each vault.
    /// @param assetId id of the asset

  function poolBalance(uint256 assetId)public view returns(uint256){
      _Assets memory asset = assets[assetId];
      uint256 _poolBalance;
      uint256 i;
      for(i==0;i<vaultCount;i++){
           _Vaults memory vault = idxVault[i];
           IDXVault Currentvault = IDXVault(vault.addr); 
           _poolBalance.add(Currentvault.balance(asset.addr));
      }
      return _poolBalance;
  }
