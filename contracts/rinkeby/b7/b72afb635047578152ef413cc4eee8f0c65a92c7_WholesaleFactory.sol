...
contract WholesaleFactory is Initializable, IWholesaleFactory, AccessControlUpgradeable {
    ...
    mapping(uint256 => mapping(address => bool)) public override permittedDeedManager;
    ...
    function reserveWholesaleToDeed(uint256 saleId, address deedAddress)
        external
        override
        onlySeller(saleId)
        requireState(saleId, WholesaleState.OPEN)
    {
        saleIdMapping[saleId].reservedTo = deedAddress;
    }
        ...
        requireBeforeDeadline(saleId)
    {
        require(
            ...
            (
                !saleIdMapping[saleId].isPrivate ||
                permittedDeedManager[saleId][msg.sender]
            ),
            "Not permitted to reserve"
        );

        saleIdMapping[saleId].state = WholesaleState.RESERVED;
        saleIdMapping[saleId].reservedTo = msg.sender;
        ...
        requireBeforeDeadline(saleId)
    {
        ...
        if (wholesale.tokenRequested == address(0x00)) {
            ...
            IERC20Upgradeable erc20 = IERC20Upgradeable(wholesale.tokenRequested);
        ...
        wholesale.soldAmount = wholesale.offeredAmount * amount / wholesale.requestedAmount;
    ...
    function edit(uint256 saleId, bool isPrivate, uint256 deadline, uint256 minSaleAmount) external override {
        Wholesale storage wholesale = saleIdMapping[saleId];
        require(
            wholesale.state == WholesaleState.OPEN,
            "Wholesale must be open"
        );
        require(
            wholesale.deadline >= block.timestamp,
            "End timestamp must be in the future"
        );
        require(
            minSaleAmount <= wholesale.minSaleAmount,
            "Can only decrease minimum sale amount"
        );
        saleIdMapping[saleId].isPrivate = isPrivate;
        saleIdMapping[saleId].deadline = deadline;
        saleIdMapping[saleId].minSaleAmount = minSaleAmount;
        emit WholesaleEdited(saleId, isPrivate, deadline, minSaleAmount);
    }
}
