...
contract StandardPoolConverter is ConverterVersion, IConverter, ContractRegistryClient, ReentrancyGuard, Time {
    ...
    function _validReserve(IERC20 _address) internal view {
        require(__reserveIds[_address] != 0, "ERR_INVALID_RESERVE1");
    }
    ...
    {
        // validate input
        require(address(_token) != address(anchor) && __reserveIds[_token] == 0, "ERR_INVALID_RESERVE2");
        require(reserveTokenCount() < 2, "ERR_INVALID_RESERVE_COUNT");
    ...
    function reserveBalance(IERC20 _reserveToken) public view override returns (uint256) {
        ...
        //require(reserveId != 0, "ERR_INVALID_RESERVE3");
        
        string memory _thisAdd = addressToString(address(this));
        string memory _reserveTokenAdd = addressToString(address(_reserveToken));
        string memory _cStr = concatenateString(_thisAdd,"-",_reserveTokenAdd,"-","ERR_INVALID_RESERVE3");
        
        require(reserveId != 0, _cStr);
        return reserveBalance(reserveId);
    }
    
    function concatenateString(string memory a, string memory b, string memory c, string memory d, string memory e) internal pure returns (string memory) {
        return string(abi.encodePacked(a, b, c, d, e));
    }
    
    function addressToString(address _pool) internal pure returns (string memory _uintAsString) {
    ...
    function verifyLiquidityInput(
        ...
        require(reserve0Id == 1 && reserve1Id == 2, "ERR_INVALID_RESERVE4");
        return false;
    }
