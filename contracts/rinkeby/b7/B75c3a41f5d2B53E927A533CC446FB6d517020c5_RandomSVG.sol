...
contract RandomSVG is ERC721URIStorage, VRFConsumerBase {
    ...
    uint256 public currentNumberOfPaths;
    ...
    function transferFrom(
        ...
        if (currentNumberOfPaths < maxNumberOfPaths) {
    ...
    function fulfillRandomness(bytes32 _requestId, uint256 _randomNumber)
        ...
        override
    {
        // the chainlink VRF has a max gas of 200,000 gas
        // our gas would 2M
        ...
        returns (string memory finalSvg)
    {
        ...
        for (uint256 i = 0; i < currentNumberOfPaths; i++) {
            // we get a new random number for each path
            string memory pathSvg = generatePath(_randomness);
        ...
        returns (string memory pathSvg)
    {
        ...
        pathSvg = string(
            abi.encodePacked(pathSvg, "' fill='black' stroke='", color, "'/>")
        );
    }
