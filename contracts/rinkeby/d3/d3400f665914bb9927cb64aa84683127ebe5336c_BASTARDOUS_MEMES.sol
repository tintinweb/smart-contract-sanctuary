...
contract BASTARDOUS_MEMES is ERC721, Ownable {
    
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    // using SafeMath for uint256;
    bool public mintingisOn;
    ...
    mapping (uint256 => Royalty) public _royaltyForToken;
    ...
    // event IPFSHASH(uint256 _id, string  _hash);
    
    constructor() ERC721("BASTARDOUS MEMEZ","BASTARDMEMEZ")  {
    ...
    modifier isWhitelisted{
        ...
        require(_whitelist[address(msg.sender)] == true, "Address not whitelisted");
        _;
    ...
    function admin_mint_bastardous_meme(address _address, string memory _hash, uint256 _fee) public onlyOwner {
        
        _tokenIds.increment();

        uint mintId = _tokenIds.current();
        
        _safeMint(address(_address), mintId);
        _royaltyForToken[mintId] = Royalty(address(_address), _fee);
        _setTokenURI(mintId, _hash);
        
    }
    
    function mint_bastardous_meme(string memory _hash, uint256 _fee)  public isWhitelisted {
        ...
        _royaltyForToken[mintId] = Royalty(address(msg.sender), _fee);
        _setTokenURI(mintId, _hash);
        
    }
    
    function burn_bastardous_meme(uint256 _id) public {
        _burn(_id);
        delete _royaltyForToken[_id];
        delete Bazaar[_id];
    }
    ...
    function buy(uint256 _tokenId) public payable {
        ...
        if (Bazaar[_tokenId].price >= 0) {
            ...
            Royalty memory _royaltyData = _royaltyForToken[_tokenId];
            
            address payable feeReceiver = payable(_royaltyData.minter);
            
            uint256 fee = SafeMath.div(SafeMath.mul(msg.value, _royaltyData.feeAmount), 100);
