...
contract CharactersForCyberpunksTEST is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    Counters.Counter private _creatorSupplyTracker;

    // Loot (for Cyberpunks) Contract
    // Temporarily overriding this to a different address for testing purposes
    address public cyberpunkAddress = 0x9929b3F3599edBae3A06E31fc64F75027c8A3E1D;
    ...
    function _creatorSupply() internal view returns (uint256) {
        return _creatorSupplyTracker.current();
    }
    ...
    function ownerClaim() public nonReentrant onlyOwner {
        require(_creatorSupply() < 50, "No more owner claims");
        require(totalSupply() < TOTAL_MINTS);
        uint256 index = _totalSupply();
        _totalSupplyTracker.increment();
        _creatorSupplyTracker.increment();
        _safeMint(_msgSender(), index);
    }
    ...
    constructor() ERC721("Characters (for Cyberpunks) TEST", "CYBRCHRTEST") Ownable() {}
}
