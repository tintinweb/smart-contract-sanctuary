...
contract lotterytest is ERC721, Ownable, VRFConsumerBase {
  ...
  // lottery variables
  address public treasury = 0x5D52F6632B1a9D743A5290dc26a3f831f09Db6A0;
  ...
  // events for progress information logging
  ...
  function buyTicket(uint amount) external payable {
    require(!drawingInProgress, "lottery drawing in progress");
    require(msg.value == ticketPrice * amount,"incorrect ticket price");
    require(amount <= maxTicketsPerTransaction, "max tickets per transaction");
    ...
    for(uint i = 0; i < amount; i++){
      mintTicket(currentSeason);
    }
  }

  function mintTicket(uint _currentSeason) private {
  ...
  function requestLotteryWinnerDrawing() external onlyOwner {
    require(!drawingInProgress, "drawing in progress");
    require( block.timestamp >= seasonStartDate + timeWindow, "too early to draw");
    uint currentSeason = getCurrentSeason();
  ...
  function tokenURI(uint256 tokenId) override public view returns (string memory) {
    ...
    if (_exists(tokenId)){
      ...
      if(bytes(seasonToBaseURI[tokensSeason]).length != 0){
        _tokenURI = string(abi.encodePacked(seasonToBaseURI[tokensSeason], tokenId.toString(), baseExtension));
      }
      else {
        _tokenURI = string(abi.encodePacked(baseTicketURI, tokenId.toString(), baseExtension));
      }
    }
    return _tokenURI;
  }
  ...
  function changeBaseExtension(string memory newBaseExtension) external onlyOwner {
  ...
  // emergency control in case drawing process gets stuck on VRF side 
  function switchDrawingInProgressState() external onlyOwner {
