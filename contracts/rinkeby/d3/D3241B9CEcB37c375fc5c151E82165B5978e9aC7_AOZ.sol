...
contract AOZ is ERC721, ReentrancyGuard, Ownable {
    ...
    uint256 public constant attainableTokenNumLimit = 5;

    uint256 public constant MAX_SALE_AOZS = 210;
    ...
    uint256 private _payableDerivedIdCounter = 7259;
    ...
    function mintAOZ(uint256 numberOfTokens) public payable nonReentrant {
        ...
        require(numberOfTokens <= attainableTokenNumLimit, "Can only mint 5 tokens at a time");
        require(numberOfTokens <= tokenIdsOnSale.length, "Purchase would exceed max supply of aozs");
        require(aozPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");

        for (uint256 i = 0; i < numberOfTokens; i++) {
            _safeMint(buyer, _getTokenIdToBeMinted(tokenIdsOnSale.length));
        }
    }
    ...
    function setTokenIdsOnSales(uint256 startId, uint256 endId) public onlyOwner {
        ...
        // sanity check
        require(startId <= endId, "StartId is larger than endId");
    ...
    for (uint256 i = 0; i < tokenIdsLen; i++) {
            ...
            require(ownerOf(superClassTokenId) == msg.sender, "wrong ownership");
            require(derivedAmount > 0, "derive nothing");
    ...
    // Pay to derive token with 0.01eth/per token
    function payToDerive(uint256 amountToDerive) public payable nonReentrant {
        address receiver = msg.sender;
        require(!receiver.isContract(), "derivation only to EOA");
        require(deriveIsActive, "not active to derive AOZ");
        require(amountToDerive <= attainableTokenNumLimit, "Can only derive 5 tokens at a time");
        ...
        require(
            ...
            "Derive would exceed max supply of aozs"
        );
