...
contract GuardianKongz is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint256 public presaleMintPrice = 50000000000000000; // 0.05 ETH

    address public constant SIGNATURE_ADDRESS = 0xD044776a0F28793890982489F4884eC3ED26De60;
    ...
    function mint(uint256 numberOfTokens) public payable nonReentrant  {
        ...
        require(numberOfTokens <= 2, "Can only mint up to 2 per transaction.");
    ...
    function presaleMint(bytes memory sig, uint256 numberOfTokens) public payable nonReentrant  {
        ...
        require(isValidData(msg.sender, sig));
    ...
    //verify signature
    function isValidData(address _walletAddress, bytes memory sig) public pure returns(bool){
        bytes32 message = keccak256(abi.encodePacked(_walletAddress));
        return (recoverSigner(message, sig) == SIGNATURE_ADDRESS);
    }
