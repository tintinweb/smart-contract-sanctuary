...
contract SimpleCollectible is ERC721, Ownable {
    ...
    string internal __baseURI =
        "https://jonathan-melnik.github.io/tvfa-web/images/andrews/";
    uint256 mintValue = 0.01 * 1e18;
    uint256 public artistBalance = 0;
    uint256 public blueOceanBalance = 0;
    address public blueOcean;
    ...
    function createCollectible() public payable returns (uint256) {
        ...
        tokenCounter++;
        uint256 artistAdd = (msg.value * 95) / 100;
        artistBalance += artistAdd;
        blueOceanBalance += msg.value - artistAdd;
        return tokenId;
    }

    function artistWithdraw() public onlyOwner {
        msg.sender.transfer(artistBalance);
        artistBalance = 0;
    }

    function blueOceanWithdraw() public onlyBlueOcean {
        msg.sender.transfer(blueOceanBalance);
        blueOceanBalance = 0;
    }

    modifier onlyBlueOcean() {
        require(msg.sender == blueOcean, "Caller is not BlueOcean");
        _;
    }
}