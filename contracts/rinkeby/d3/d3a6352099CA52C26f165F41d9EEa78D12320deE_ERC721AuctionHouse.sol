...
import "./base/OriginalContentERC721.sol";
...
contract ERC721AuctionHouse is IERC721AuctionHouse, Ownable, ReentrancyGuard {
    ...
    returns (uint256)
  {
    ...
    address tokenOwner = OriginalContentERC721(tokenContract).ownerOf(tokenId);
    require(
      msg.sender == OriginalContentERC721(tokenContract).getApproved(tokenId) ||
    ...
    OriginalContentERC721(tokenContract).transferFrom(
      tokenOwner,
      address(this),
      tokenId
    );
    ...
    nonReentrant
  {
    ...
    try
      OriginalContentERC721(auctions[auctionId].tokenContract).safeTransferFrom(
        address(this),
        msg.sender,
        auctions[auctionId].tokenId
      )
    ...
    if (serviceWallet != address(0)) {
      ...
      (minterWallet, royaltyFee, royaltyData) = OriginalContentERC721(
        auctions[auctionId].tokenContract
