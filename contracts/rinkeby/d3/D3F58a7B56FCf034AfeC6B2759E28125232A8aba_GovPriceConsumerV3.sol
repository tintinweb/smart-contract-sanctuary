    ...
    */
    ...
    function getDexTokenPrice(
        ...
        if (pair.token0() == _stable) {
            ...
            uint256 resD = res0 * (10**token0Decimals);//1e8
            return (_amount.mul(resD.div(res1))).div(token1Decimals); // return amount of alt needed to buy 1 stable

        } else {

            uint256 resD = res1 * (10**token1Decimals);
            return (_amount.mul(resD.div(res0))).div(token0Decimals); // return amount of stable needed to buy 1 altcoin
        }
    }
