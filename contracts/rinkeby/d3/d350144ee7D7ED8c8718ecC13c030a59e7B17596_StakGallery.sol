...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    */
    ...
    /**
     * @dev Expansion: Returns all tokens by token owner.
     */
    function tokensOfOwner(address owner) public view returns (uint256[] memory) {
        ...
        for (uint i = 0; i < totalTokens; i++) {
            if (_holderTokens[owner].contains(i)) {
                resultCount++;  // step 1 - determine the result count
            }
        }

        // uint256[resultCount] memory indexes; // step 2 - create the fixed-length array
        uint256[] memory indexes = new uint256[](resultCount);
        uint256 j = 0;
        
        for (uint i=0; i < totalTokens; i++) {
            if (_holderTokens[owner].contains(i)) {
                indexes[j] = _holderTokens[owner].at(i);  // step 3 - fill the array
                j++;
            }  
        }
        return indexes;
