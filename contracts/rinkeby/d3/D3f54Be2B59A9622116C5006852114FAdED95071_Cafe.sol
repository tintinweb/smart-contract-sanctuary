...
contract Cafe is ERC1155 {
    ...
    constructor (string memory uri_, uint32 _timescale) public ERC1155(uri_) {
        _setURI(uri_);
...
/** @dev total stats
 */ 
    uint256 public oldestStarterId;
    uint32 public oldestStarterAge =0;
    uint256 public topScore =0;
    address public leader;
    uint256 public flourSupply = 0;
    uint256 public mostUsedRecipe;
    uint256 public topRecipeScore;

    uint256 public startersCreated = 0;
    uint256 public itemsBaked = 0;
    uint256 public recipesPublished = 0;

/** @dev Credits the user 100 FLOUR every 24 hours if submitted
 */
    function grindFlour() public {
        require(isAutoFeeding[msg.sender]==false,"You are already in the bakery, either wait or turn auto feed off!");
        require(grindCooldown[msg.sender]<block.timestamp,"You did this already today!");
        _mint(msg.sender,flourId, 100, "");
        flourSupply += 100;
...
/** @dev Burns 50 FLOUR and resets the time of death for your token*/
    function feedStarter(uint256 tokenId) public returns (bool) {
        ...
        uint32 currentAge = uint32(block.timestamp) - Sourdoughs[tokenId]._creationTime;

        if(currentAge > oldestStarterAge){
            oldestStarterAge = currentAge;
            oldestStarterId = tokenId;
        }
...
/** @dev Bakes and publishes a recipe that does not yet exist */
    function publishNewRecipe(string memory name, uint256 flourRequired, uint32[] memory ages, uint256[] memory starterBatch) public returns (bool) {
        require(ages.length <=10, "You can only have up to 10 ingredients!");
        Recipe memory recipe = Recipe(name,flourRequired, ages,msg.sender,0,0);
        ...
        if (BakedGoods[recipesPublished]._timesBaked> topRecipeScore){
            topRecipeScore = BakedGoods[recipesPublished]._timesBaked;
            mostUsedRecipe = recipesPublished;
        }
...
/** @dev Takes the id of an existing recipe and a batch of ingredients.  Burns the batch and the required flour, mints an item with the recipe Id */
    function bakeExistingRecipe(uint256 recipeId, uint256[] memory starterBatch) public returns (bool) {
        ...
        itemsBaked+=1;
        if( BakedGoods[recipeId]._timesBaked > topRecipeScore){
            topRecipeScore = BakedGoods[recipeId]._timesBaked;
...
/** @dev Burns an item, credits the player with a score equivalent to the total age of all ingredients */
    function eatBakedGood(uint256 recipeId, uint256 amount) public returns (bool){
        ...
        uint32[] memory ages = BakedGoods[recipeId]._ages;
        uint32 total;
        ...
        if (totalScore[msg.sender] > topScore){
            leader = msg.sender;
            topScore = totalScore[msg.sender];
        }
