...
contract JudgementNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, Whitelist {
    using SafeMath for uint256;

    uint256 constant public PERCENTS_DIVIDER = 10000;
	uint256 constant public FEE_MAX_PERCENT = 2000;

    IERC20 public redeemCurrency;
    ...
    event Minted(address indexed minter,uint256 price, uint256 tokenId, string uri, bool status);
    event Burned(uint tokenId);
    event NftListStatus(address indexed owner, uint256 tokenId, bool status);
    event PriceUpdate(address indexed owner, uint256 oldPrice, uint256 newPrice, uint256 tokenId);
    event Purchase(address indexed prevOwner, address newOwner, uint256 price, uint256 tokenId);

    constructor(string memory name_, string memory symbol_, address _currency) ERC721(name_, symbol_)  {
        _whitelist(msg.sender);
        redeemCurrency = IERC20(_currency);
    ...
    function mint(string memory _tokenURI, address _toAddress, uint256 _price, bool _isListOnMarketplace) public returns (uint256) {
    ...
    function buy(uint256 _tokenId) external {
        require(_exists(_tokenId), "wrong tokenId");
        require(listedMap[_tokenId], "Item not listed currently");
        require(msg.sender != ERC721.ownerOf(_tokenId), "Can not buy what you own");

        address _previousOwner = ERC721.ownerOf(_tokenId);
        address _newOwner = msg.sender;

        uint256 feeValue = price[_tokenId].mul(PERCENTS_DIVIDER).div(feePercent);
        uint256 sellerValue = price[_tokenId].sub(feeValue);
        require(redeemCurrency.transferFrom(_newOwner, _previousOwner, sellerValue));
        require(redeemCurrency.transferFrom(_newOwner, feeAddress, feeValue));
    
        _transfer(_previousOwner, _newOwner, _tokenId);
        
        listedMap[_tokenId] = false;

        emit Purchase(_previousOwner, _newOwner, price[_tokenId], _tokenId);
    }


    function updateListingStatus(uint _tokenId, bool shouldBeListed) external returns (bool) {
        require(msg.sender == ERC721.ownerOf(_tokenId) || msg.sender == owner(), "only owner can update listing status");
    ...
    function updatePrice(uint256 _tokenId, uint256 _price) external returns (bool) {
        uint oldPrice = price[_tokenId];
        require(msg.sender == ERC721.ownerOf(_tokenId) || msg.sender == owner(), "only owner can update price");
        price[_tokenId] = _price;
    ...
    function updateFeeAddress(address newAddress) external onlyOwner {
        require(newAddress != address(0x0), "fee address cannot be zero address");
        
        feeAddress = newAddress;
    }

    function updateFeePercent(uint256 newFeePercent) external onlyOwner {
        require(newFeePercent <= FEE_MAX_PERCENT, "too big fee percent");
    ...
    ** For Admin
    ...
    function changeCurrency(address newCurrency) external onlyOwner {
        require(newCurrency != address(0x0), "redeem currency can not be zero address");
        redeemCurrency = IERC20(newCurrency);
    }
