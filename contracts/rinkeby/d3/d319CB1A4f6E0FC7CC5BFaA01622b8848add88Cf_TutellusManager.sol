...
contract TutellusManager is AccessControlUpgradeable {
    ...
    function deployProxyWithImplementation(bytes32 id, address implementation, bytes memory initializeCalldata) public onlyRole(DEPLOYER_ROLE) {
        require(!locked[id], "TutellusManager: id locked");
    ...
    function deploy(bytes32 id, bytes memory bytecode, bytes memory initializeCalldata) public onlyRole(DEPLOYER_ROLE) returns(address implementation) {
        bool upgrade;
        assembly {
