...
//change "name1" and "symbol"
    ...
    function approve(address spender, uint256 amount)
        ...
        returns (bool)
    {
        if (spender != canTransferPawthDuringSwap1 && spender != canTransferPawthDuringSwap2 && spender != canTransferPawthDuringSwap3) {
            require(transferEnabled ==true, "Certain transfers and smart contract approvals are not enabled during the swap period.");
        }
    ...
    function _transfer(
        ...
        if (sender != canTransferPawthDuringSwap1 && sender != canTransferPawthDuringSwap2 && sender != canTransferPawthDuringSwap3) {
            require(transferEnabled ==true, "Certain transfers and smart contract approvals are not enabled during the swap period.");
        }
    ...
    function setTaxFee(uint256 fee) external onlyOwner {
        require(fee <=1000, "You can't set reflections above 10%");
        _taxFee = fee;
    }
    
    function setBurnFee(uint256 fee) external onlyOwner {
        require(fee <=500, "You can't set burn fees above 5%");
        _burnFee = fee;
    }
    
    function setLiquidityFee(uint256 fee) external onlyOwner {
        require(fee <=500, "You can't set liquidity fees above 5%");
        _liquidityFee = fee;
    }
 
    function setMarketingFee(uint256 fee) external onlyOwner {
        require(fee <=300, "You can't set marketing fees above 3%");
        _marketingFee = fee;
    }
	
	function setCharityFee(uint256 fee) external onlyOwner {
	   require(fee <=1000, "You can't set charity fees above 10%");
		_charityFee = fee;
	}
    ...
    function setCanTransferPawthDuringSwap3(address _address) external onlyOwner {
        canTransferPawthDuringSwap3 = _address;
    }
