...
contract ERC721Base is HasSecondarySaleFees, ERC721, HasContractURI, HasTokenURI, ERC721Enumerable {
    ...
    function _mint(address to, uint256 tokenId, address payable account, uint256 amount) internal {
        require(account != address(0x0), "Recipient should be present");
        require(amount != 0, "Fee value should be positive");
        _mint(to, tokenId);
        Fee memory _fees = Fee(account,amount);
        address[] memory recipients = new address[](1);
        uint[] memory bps = new uint[](1);
        for (uint i = 0; i < 1; i++) {
            fees[tokenId].push(_fees);
...
contract MintableToken is Ownable, IERC721, IERC721Metadata, ERC721Burnable, ERC721Base {
    ...
    function mint(uint256 tokenId,  address payable account, uint256 amount, string memory tokenURI) public {
        _mint(msg.sender, tokenId, account, amount);
