...
contract HeroesCommon1155 is ERC1155Supply, Ownable {

    struct Partners {
        uint256 limit;
        uint256 nftMinted;
    }
    uint256 public NFT_SEQ = 1;
    ...
    constructor()
       ERC1155("https://dev.nftstars.app/backend/api/v1/nfts/metadata/{address}/{id}")  {
        ...
        _mint(owner(),MAGIC_BOX_TOKENID, 1000, bytes('0'));
    ...
    function _mintHero(address to) internal {
        require((NFT_SEQ + 1) < MAX_TOTAL_SUPPLY, "No more common heroes");
        NFT_SEQ++;
        _mint(to, NFT_SEQ, 1, bytes('0'));
    }
