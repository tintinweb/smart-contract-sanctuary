...
contract ClusterToken is ERC20Pausable, AccessControl, IClusterToken {
    ...
    uint256 public DUST_AMOUNT = 1e12;
    ...
    event ClusterWithdrawnToAcc(address clusterAddress, address farmer, uint256 clusterAmount, uint256[] underlyingsAmounts, address[] tokens);
    ...
    event ClusterRefundedFromAcc(address clusterAddress, address farmer, uint256 clusterAmount, uint256[] underlyingsAmounts, address[] tokens);
    ...
    event ProportionsOptimized(address clusterAddress, address delegate, uint256[] updatedShares);
    ...
    modifier onlyAdapter() {
        require(hasRole(ADAPTER_ROLE, _msgSender()), "Unauthorized");
        _;
    }
    ...
    constructor(
        ...
        _setupRole(ADAPTER_ROLE, _msgSender());
    ...
    /// @notice sets ETH dust amount allowed to be sent to the user
    /// @param _dust New dust value
    function setDust(uint256 _dust) external onlyAdmin {
        DUST_AMOUNT = _dust;
    }

    /// @notice withdraws all ETH stucked or kept as dust
    function withdrawDust() external onlyAdmin {
        _payInEth(address(this).balance);
    }
    ...
    /// @param coverDhvWithEth Should DHV to cover commission be purchased during the call,
    /// rather then transferred from the caller's address.
    function disassemble(uint256 _clusterAmount, bool coverDhvWithEth) external payable override whenNotPaused checkCooldownPeriod checkBuyer {
    ...
    function assembleByAdapter(uint256 _clusterAmount) external override onlyAdapter {
        uint256[] memory underlyingAmounts = getUnderlyingsAmountsFromClusterAmount(_clusterAmount);
    ...
    /// @param _clusterAmount Cluster token amount to be minted.
    function disassembleByAdapter(uint256 _clusterAmount) external override onlyAdapter {
        uint256[] memory underlyingAmounts = getUnderlyingsAmountsFromClusterAmount(_clusterAmount);
        ...
        _burn(msg.sender, _clusterAmount);
    ...
    function withdrawToAccumulation(
        ...
        emit ClusterWithdrawnToAcc(address(this), _msgSender(), _clusterAmount, _amounts, _tokens);
    }
    ...
    function refundFromAccumulation(
        ...
        emit ClusterRefundedFromAcc(address(this), _msgSender(), _clusterAmount, _amounts, _tokens);
    }
    ...
    function getUnderlyingsAmountsFromClusterAmount(uint256 _clusterAmount) public view override returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < underlyings.length; i++) {
            ...
            uint8 decimals = IERC20Extend(underlyings[i]).decimals();
            if (decimals < 18) {
