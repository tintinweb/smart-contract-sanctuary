    ...
    ReentrancyGuard
{
    ...
    address internal constant ORACLE = 0xaf494E541e5E705075AF72CFf732E7658C2A18Ac;
    address private constant INDEX_FUND = 0xf868A987e1909a77A542beBf433B6F1bB1D76955;
    ...
    /// @notice Get the pool's balance of token0, token1 with total liquidity of unipilot positions
    ...
    function createPair(
        ...
        _pool = IUniswapV3Factory(UNISWAP_FACTORY).createPool(
            _token0,
            _token1,
            _fee
        );
    ...
    function deposit(
        ...
        uint256 shares,
        bytes memory data
    ) external payable override onlyUnipilot returns (uint256 mintedTokenId) {
        ...
        _addLiquidityInManager(sender, pool, amount0Desired, amount1Desired, shares);
    ...
    function withdraw(
        ...
        (c.amount0Removed, c.amount1Removed) = _removeLiquidityWithFees(
            false,
            position.pool,
            liquidity
        );
    ...
    function readjustLiquidity(
        ...
        (b.amount0, b.amount1) = _removeLiquidityWithFees(
            true,
            b.poolAddress,
    ...
    /// @notice Returns the status that pool needs rebasing or not
    ...
    function _increaseLiquidity(
        ...
        if (
            ...
            _addRangeLiquidity(sender, pool, amount0Desired, amount1Desired, shares);
        } else {
        ...
        if (position.totalLiquidity > 0) {
            ...
            if (position.baseTickLower != 0 && position.baseTickUpper != 0) {
                ...
                );
                {
                    ...
                    (tokenDetails.bidLiquidity, , ) = LiquidityReserves.getLiquidityAmounts(
                        ticks.bidTickLower,
                        ticks.bidTickUpper,
                        0,
                        amount0,
                    ...
                    (tokenDetails.rangeLiquidity, , ) = LiquidityReserves.getLiquidityAmounts(
                        ticks.rangeTickLower,
                        ticks.rangeTickUpper,
                        0,
                        amount0,
    ...
    function _distributeFeesInPilot(
        ...
        // if the incoming pair is weth pair then we will compute the amount
        ...
        if (_token0 == WETH || _token1 == WETH) {
            ...
            // if the incoming pair is alt pair then we will compute the amount
    ...
    function _removeLiquidityWithFees(
        ...
        // @dev reseting the position to initial state to calculate the amounts correctly from getSharesAndAmounts
        if(position.totalLiquidity == 0) {
...
interface IUniswapLiquidityManager is IULMEvents {
    ...
    function deposit(
        ...
        uint256 shares,
        bytes memory data
    ) external payable returns (uint256 mintedTokenId);
...
abstract contract PeripheryPayments {
    ...
    address internal constant PILOT = 0xf52834E190F0Db661160a939542c74F5620422d2;
