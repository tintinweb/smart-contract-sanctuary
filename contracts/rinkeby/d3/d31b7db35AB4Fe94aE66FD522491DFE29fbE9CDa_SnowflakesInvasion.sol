...
contract SnowflakesInvasion is IERC1155Receiver {
  ...
  event PlayerLeft(address player,address collectible,uint256 tokenId);
  event PlayerJoined(address player,address collectible,uint256 tokenId);


  constructor(ERC1155[] memory _collectibles,IERC20 link) public {
  ...
  function respawn(address collectible,uint256 tokenId) public tokenAllowed(collectible) {
    ...
    ERC1155(collectible).safeTransferFrom(msg.sender,address(this),tokenId,1,'');
    if(collectible == address(collectibles[0])){
      players[msg.sender].x = uint8(0);
      players[msg.sender].y = uint8(0);
    } else {
      players[msg.sender].x = uint8(50);
      players[msg.sender].y = uint8(50);
    }
    ...
    players[msg.sender].lastMovement = block.number;
    emit PlayerJoined(msg.sender,collectible,tokenId);
  }

  function stopPlaying(address collectible,uint256 tokenId) public {
    require(players[msg.sender].playing == true,
            "Not playing");
    require(players[msg.sender].collectible == collectible &&
            players[msg.sender].tokenId == tokenId,
            "Not allowed to withdraw token from contract");
    ERC1155(players[msg.sender].collectible).safeTransferFrom(address(this),msg.sender,tokenId,1,'');
    delete players[msg.sender];
    totalPlayers = totalPlayers.sub(1);
    emit PlayerLeft(msg.sender,collectible,tokenId);
  }

  function move(address collectible,uint256 tokenId,uint8 x,uint8 y) public {
    require(players[msg.sender].playing == true,
            "Not playing");
    require(players[msg.sender].collectible == collectible &&
            players[msg.sender].tokenId == tokenId,
            ...
            /*
    require(players[msg.sender].x.add(10) >= x &&
            players[msg.sender].x.sub(10) <= x &&
            players[msg.sender].y.add(10) >= y &&
            players[msg.sender].y.sub(10) <= y &&
            x <= uint8(100) &&
            y <= uint8(100) &&
            x >= uint8(0) &&
            y >= uint8(0),
            ...
            */
    require(players[msg.sender].lastMovement.add(50) <= block.number,"Token need to wait 50 blocks since the last movement/respawn done");
    emit PlayerMoved(msg.sender,collectible,tokenId,x,y);
    if(positions[x][y].owner != address(0)){
      ...
      // battle //
      uint256 powerP1 = _chainlink.balanceOf(msg.sender);
      uint256 powerP2 = _chainlink.balanceOf(positions[x][y].owner);
      ...
      if(win){
        ERC1155(players[positions[x][y].owner].collectible).safeTransferFrom(address(this),positions[x][y].owner,tokenId,1,'');
        emit PlayerLeft(positions[x][y].owner,players[positions[x][y].owner].collectible,players[positions[x][y].owner].tokenId);
        delete players[positions[x][y].owner];
        totalPlayers = totalPlayers.sub(1);
      } else {
        ERC1155(players[msg.sender].collectible).safeTransferFrom(address(this),msg.sender,tokenId,1,'');
        emit PlayerLeft(msg.sender,players[msg.sender].collectible,players[msg.sender].tokenId);
        delete players[msg.sender];
        totalPlayers = totalPlayers.sub(1);
      }
    } else {
      positions[x][y].owner = msg.sender;
      positions[players[msg.sender].x][players[msg.sender].y].owner = address(0);
