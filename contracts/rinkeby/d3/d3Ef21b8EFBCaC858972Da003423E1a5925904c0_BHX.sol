...
contract BHX is ERC20 {
    ...
    // 参数1: 接受方地址
    // 参数2: 交易的数量
    // 参数3: owner签名的signature值
    function backendTransferBhx(address _to, uint256 _value, bytes memory _signature) public onlyFlag returns (bool success) {
        require(_to != address(0), "Zero address error");
        ...
        bytes32 hash = keccak256(abi.encodePacked(msg.sender, _to, _value));
    ...
    // 参数2: 交易的数量
    // 参数3: owner签名的signature值
    function backendTransferUsdt(address _to, uint256 _value, bytes memory _signature) public onlyFlag returns (bool success2) {
        require(_to != address(0), "Zero address error");
        // 创建usdt的合约对象
