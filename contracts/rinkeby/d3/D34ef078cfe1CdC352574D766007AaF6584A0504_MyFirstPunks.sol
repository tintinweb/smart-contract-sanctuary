...
contract MyFirstPunks is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    // Punk Contract
    address public punkAddress = 0xb47e3cd837dDF8e4c57F05d70Ab865de6e193BBB;
    PunkInterface punkContract = PunkInterface(punkAddress);

    // Used for running mints through mapping past minted numbers
    ...
    uint256 public constant PRICE = 25 * 10**15;
    uint256 public constant PUNKPRICE = 25 * 10**15;
    ...
    uint public punkReserveMax = 21;
    uint public preMintedPunks = 0;

    event CreateFirstPunk(uint256 indexed id);

    constructor(string memory baseURI) ERC721("MyFirstPunks", "MFP") {
    ...
    function mintWithPunk(uint punkId) public payable saleIsOpen {
        ...
        require(punkContract.ownerOf(punkId) == msg.sender, "Not the owner of this punk");
    ...
    // Minting by team
    function preMintList(address[] memory recipients,uint256[] memory punkIds) external onlyOwnerOrTeam {

        require( recipients.length == punkIds.length, "different list count" );
    ...
    function _mintElementWithSkip(address _to, uint256 _id) private {
        ...
        // did we just mint our starting number? if so, create a new start point
        if (_id == nextMintID) {
            nextMintID = next;
        }
