...
interface iGenerativeMasks {
    ...
    function revealDate() external view returns (uint256);
...
contract GenerativeMasks is iGenerativeMasks, ERC721, HasSecondarySaleFees, Ownable {
    ...
    function revealDate() external override view returns (uint256) {
        return _revealDate;
    }
    ...
    function buy(uint256 amount) external payable override {
        ...
        for (uint256 i = 0; i < amount; i++) {
            require(nextTokenId < NFT_SUPPLY_AMOUNT, "All tokens are minted");
    ...
    function finalizeStartingIndex() external override {
        ...
        startingIndex = uint256(blockhash(startingIndexBlock)) % NFT_SUPPLY_AMOUNT;
        if ((block.number - startingIndexBlock) > 255) {
            startingIndex = uint256(blockhash(block.number - 1)) % NFT_SUPPLY_AMOUNT;
        }
        if (startingIndex == 0) {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        uint256 metadataId = (tokenId + startingIndex) % NFT_SUPPLY_AMOUNT;
