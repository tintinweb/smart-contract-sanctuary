...
contract LeagueManager {
    ...
    mapping(string => address[]) public rolesToMembers;
    mapping(address => string) public membersToRoles;
    mapping(address => uint256) public membersToTokenIds;

    LeagueBadge public leagueBadge;

    event LeagueAdded(string _league);
    event LeagueRemoved(string _league);
    ...
    function addLeague(string memory _league) public {
        ...
        require(!leagueIsPresent, "addLeague: league already exists");

        leagues.push(_league);

        emit LeagueAdded(_league);
    }

    function removeLeague(string memory _league) public {
        (bool leagueIsPresent, int256 leaguePosition) = leagueExists(_league);
        require(
            leagueIsPresent && leaguePosition >= 0,
            "removeLeague: league does not exist"
        );

        for (
            uint256 index = uint256(leaguePosition);
            index < leagues.length - 1;
            index++
        ) {
            leagues[index] = leagues[index + 1];
        }

        removeAllRoles(_league);
        leagues.pop();

        emit LeagueRemoved(_league);
    }
    ...
    function removeAllRoles(string memory _league) internal {
        for (
            uint256 index = 0;
            index < leaguesToRoles[_league].length;
            index++
        ) {
            removeRole(_league, leaguesToRoles[_league][index]);
        }
    }
