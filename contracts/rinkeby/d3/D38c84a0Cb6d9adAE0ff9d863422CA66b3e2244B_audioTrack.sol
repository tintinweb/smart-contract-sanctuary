...
contract audioTrack is ERC721Tradable {
    ...
    address payable _tWallet; 
    address constant _dWallet = 0x47cf32288Afffd890fa1cb3F6ce8faEB9C9F86d9;
    ...
    uint public MAX_NFT_PURCHASE;
    uint256 private NFT_PRICE_PRE_SALE;
    uint256 public MAX_SUPPLY;

    //  * Reserve tokens for the team and community giveaways
    uint private RESERVED_TOTAL;//200;
    ...
    constructor(address teamWallet, string memory name, string memory symbol, uint _reserve, uint256 _maxSupply, uint _maxPurchase, uint256 _price_, uint256 _pricePresale,
    uint256 royalty, address _proxyRegistryAddress_
    ) ERC721Tradable(name, symbol, _proxyRegistryAddress_) {
        /**
         * Start counting tokens with a reserved shift
         */
        _tokenIdCounter._value = _reserve;
        RESERVED_TOTAL = _reserve;
        _tWallet = payable(teamWallet);
        _royaltyRecipient = payable(_dWallet);
        _royaltyBps = royalty;

        MAX_SUPPLY = _maxSupply;
        //_governance = msg.sender;
        NFT_PRICE = _price_; // = 65000000000000000; // 0.065 ETH
        MAX_NFT_PURCHASE = _maxPurchase;// = 15; 
        NFT_PRICE_PRE_SALE = _pricePresale;// = 55000000000000000; // 0.055 ETH for pre-sale
    }

    modifier onlyAuthorized {
        require( msg.sender == _dWallet || msg.sender == _tWallet || msg.sender == owner(), "only Authorized people only");
        _;
    }
    ...
    function setBaseURI(string memory baseURI_) external onlyAuthorized {
    ...
    function setTeamWallet(address payable wallet) public onlyAuthorized {
        _tWallet = wallet;
    }
    ...
    function _mintGeneric(
        ...
        if (isPreSaleActive) {
            ...
            CURRENT_TOKENS_NUMBER_LIMIT = MAX_NFT_PURCHASE;
        }
