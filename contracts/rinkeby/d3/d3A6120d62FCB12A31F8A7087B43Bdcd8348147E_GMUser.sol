...
/**
  ...
  */
contract GMUser is IGMUser, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    /**
        @notice Controlling user status and permissions
    ...
    function _authorizeUpgrade(address upgrader) internal override {
//        require(hasRole(DEFAULT_ADMIN_ROLE, upgrader));
    }
    ...
    function isAgentOf(address agent, address user) external view override returns (bool) {
        return hasRole(AGENT, user) && agents[user] == agent;
    }

    function setAgentOf(address agent, address user) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _checkRole(AGENT, agent);
        require(!_isAgentOf(agent, user), "GMUser: the user has already this exact agent assigned");
        agents[user] = agent;
    }
    ...
    function revokeCreator(address user) external {
        return revokeRole(CREATOR, user);
    }
