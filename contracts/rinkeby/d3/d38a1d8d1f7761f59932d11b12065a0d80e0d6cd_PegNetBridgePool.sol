...
contract PegNetBridgePool {
    ...
    function claimPayment(uint256 amount, uint256 nonce, bytes memory signature, uint256 tokenType) public {
        ...
        if (tokenType == 1) { // PegNet Token
            require(tokenOfPegNet.balanceOf(owner) >= amount, "No enough PegNet tokens in BridgePool");
            ...
            require(tokenOfpUSD.balanceOf(owner) >= amount, "No enough pUSD tokens in BridgePool");
