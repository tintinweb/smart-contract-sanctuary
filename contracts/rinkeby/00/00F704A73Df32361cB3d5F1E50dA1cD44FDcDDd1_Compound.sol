...
contract Compound is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        uint256 stakeBlock; // blocknumber of user deposit
    }

    IERC20 public stakedToken; // Gate token
    ...
    uint256 public constant lockPeriod = 5760;
    ...
    /**
     * @notice Constructor
     * @param _stakedToken: Cake token contract
    ...
    function maxSafeApprove() external onlyOwner {
        // Infinite approve
        stakedToken.safeApprove(address(staking), type(uint256).max);
    }
    ...
    function harvest() public {
        (uint256 amount, , , ) = staking.stakes(address(this), 0);
        staking.autoCompUnstake(amount);
        _earn();
    ...
    function withdraw(uint256 _shares) public {
        ...
        require(
            user.stakeBlock + lockPeriod < block.number,
            "Compound::lockPeriod don't passed"
        );
        ...
        if (bal < currentAmount) {
            ...
            staking.autoCompUnstake(balWithdraw);
        ...
        stakedToken.safeTransfer(msg.sender, currentAmount);
    ...
    function balanceOf() public returns (uint256) {
        uint256 amount;
        if(staking.getUserStakes(address(this)).length > 0){
            (amount, , , ) = staking.stakes(address(this), 0);
        }
        return stakedToken.balanceOf(address(this)) + amount;
    }
...
interface IStaking {
    ...
    function stakes(address, uint256) external returns (uint256, uint256, uint256, uint256);
    function claim() external;
    function unstake(uint256, uint256) external;
    function getReward(uint256, uint256)
        external
        view
        returns (uint256);
