...
contract SmolWingedTurtlez is ERC721, Ownable {
    ...
    mapping(uint256 => Hash) internal tokenIdHash;
    mapping(uint256 => Coordinates) internal pupils;
    mapping(uint256 => Coordinates[]) internal wingTypes;
    mapping(uint256 => Color[]) internal colorPalettes;

    string[] private wingTypeValues = ['Regular', 'Long', 'Tall', 'Spiky', 'Ruffled', 'Loose', 'Sparkly', 'Claw'];

    string[] private pupilValues = ['Focused', 'Reserved', 'Positive', 'Suspect'];

    uint16[][3] traitWeights;
    ...
    function setWingType(uint48 wingTypeIndex, Coordinates[3] memory coordinates) private onlyOwner {
        for (uint8 i = 0; i < coordinates.length; i++) {
            wingTypes[wingTypeIndex].push(coordinates[i]);
        }
    }

    function setColorPalette(uint48 colorPaletteIndex, Color[8] memory colors) private onlyOwner {
    ...
    constructor() ERC721('Smol Winged Turtlez', 'SWT') {
        ...
        // Wing color
        traitWeights[1] = [6900, 69];

        // Tongue
        traitWeights[2] = [5000, 1969];
    ...
    function weightedRarityGenerator(uint48 pseudoRandomNumber, uint8 trait) internal view returns (uint16) {
        uint16 lowerBound = 0;
    ...
    function createTokenIdHash(uint256 tokenId) public view returns (Hash memory) {
        ...
        return
            Hash({
                backgroundColor: uint48(uint48(pseudoRandomBase) % 8),
                wingType: weightedRarityGenerator(uint48(uint48(pseudoRandomBase >> 0) % 6969), 0),
                wingColor: weightedRarityGenerator(uint48(uint48(pseudoRandomBase >> 48) % 6969), 1),
                bandana: uint48(uint48(pseudoRandomBase >> 96) % 8),
                boots: uint48(uint48(pseudoRandomBase >> 144) % 8),
                pupil: uint48(uint48(pseudoRandomBase >> 192) % 4),
                tongue: weightedRarityGenerator(uint48(uint48(pseudoRandomBase >> 240) % 6969), 2) == 1
            });
    }

    function getTokenIdHash(uint256 tokenId) public view returns (Hash memory) {
        return tokenIdHash[tokenId];
    }
    ...
    function getTokenIdHashSvg(Hash memory hash) public view returns (string memory svg) {
        svg = string(
            abi.encodePacked(
                ...
                "<rect fill='",
                colorPalettes[1][hash.boots].hexCode,
                "' height='1' width='2' x='7' y='15' />",
                "<rect fill='",
                colorPalettes[1][hash.boots].hexCode,
                "' height='1' width='2' x='13' y='15' />",
                "<rect fill='",
                colorPalettes[1][hash.bandana].hexCode,
                "' height='1' width='1' x='14' y='8' />",
                "<rect fill='",
                colorPalettes[1][hash.bandana].hexCode,
    ...
    function getTokenIdHashMetadata(Hash memory hash) public view returns (string memory metadata) {
        metadata = string(
            abi.encodePacked(
                ...
                colorPalettes[1][hash.boots].name,
                '"}',
                '{"trait_type":"Bandana", "value":"',
                colorPalettes[1][hash.bandana].name,
                '"}',
                        ...
                        bytes(
                            string(
                                abi.encodePacked(
                                    ...
                                    '", "description": "Smol Winged Turtlez may or may not be a great investment opportunity.", "image": "data:image/svg+xml;base64,',
    ...
    function mint(uint256 numberOfTokens) public payable {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            ...
            tokenIdHash[tokenId] = createTokenIdHash(tokenId);
