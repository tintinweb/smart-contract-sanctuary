...
import "./NFTBracket.sol";
import "./MMOracle.sol";
import "./MMRewardPool.sol";
...
// MMRewardPool
// MarchMadness
// set march madness contract in rewardPool
// set march madness contract in nftBracket

contract MarchMadness is Ownable, Pausable, ReentrancyGuard {
    using SafeMath for uint256;

    NFTBracket public nftBracket;
    MMOracle public mmOracle;
    MMRewardPool public mmRewardPool;

    uint256 public nftPrice;
    uint256 public nftUpdatePrice;

    struct WinningBracket {
        uint16 score;
        uint256 reward;
        uint256 tokenId;
    }

    WinningBracket[] public winningBrackets;

    bool public disasterState;

    event LogBracketMinted(address triggeredBy, uint256 tokenId);
    event LogBracketUpdated(address triggeredBy, uint256 tokenId);
    event LogRewardClaimed(
        address triggeredBy,
        uint256 tokenId,
        uint256 reward
    );
    event LogBracketScoreUpdated(uint256 tokenId, uint16 score, address owner);
    event LogBracketScoresUpdated(address triggeredBy);
    event LogDisasterStateSet(address triggeredBy, bool disasterState);

    modifier onlyWithTokenUri(string memory tokenUri) {
        require(bytes(tokenUri).length != 0, "MM: Only with TokenUri");
        _;
    }

    modifier onlyWithCorrectLength(uint16[] memory bracket) {
        require(
            bracket.length == mmOracle.bracketLength(),
            "MM: Invalid bracket length"
        );
        _;
    }

    constructor(
        NFTBracket _nftBracket,
        MMOracle _mmOracle,
        MMRewardPool _mmRewardPool,
        uint256 _nftPrice
    ) {
        nftBracket = _nftBracket;
        mmOracle = _mmOracle;
        nftPrice = _nftPrice;
        nftUpdatePrice = nftPrice.div(10); // Todo not a final price for update.
        mmRewardPool = _mmRewardPool;
        disasterState = false;
    }

    /**
     * @dev Mints an NFT from the nftBracket contract and updates the reward pool adding nftPrice to it.
     * @param bracket array of all the predictions as raw data that the user has committed. Will be used to generate hash against the current bracket and stored in the nftBracket
     * @param tokenUri ipfs hash where the raw data will be stored.

     * Requirements:
     *
     * - `tokenUri` must be passed.
     * - `bracket` must be filled with 67 elements.
        ...
        payable
    {
        ...
        uint256 tokenId = nftBracket.mint(msg.sender, bracketHash, tokenUri);
     ...
     * @param bracket array of all the predictions as raw data that the user has committed. Will be used to generate hash against the current bracket and overwrite the previously generated
     * @param tokenUri ipfs hash where the raw data will be stored.
    ...
    {
        require(
            block.timestamp < mmOracle.tournamentStart(),
            "MM: Tournament already started"
        );
        require(msg.value == nftUpdatePrice, "MM: Invalid update price");

        bytes32 bracketHash = keccak256(abi.encodePacked(bracket));
        nftBracket.update(msg.sender, tokenId, bracketHash, tokenUri);

        mmRewardPool.updateRewardPool{value: msg.value}(msg.value);

        emit LogBracketUpdated(msg.sender, tokenId);
    }
     ...
     * Requirements:
     *
     * - MarchMadness contract must be paused
     * - `block.timestamp` must be after the tournament has finished.
     * - `disasterState` must be activated
     */
    function claimOnDisaster(uint256 tokenId) external whenPaused nonReentrant {
        require(disasterState, "MM: Not in disaster state");

        _claim(tokenId);
    }

    /**
     * @dev Internal Function for claiming funds from the mmRewardPool. Claims a reward if the NFT bracket is among the winners of the tournament. 
     * @param tokenId id which is recorded as a winner in the contract

     * Requirements:
     *
     * - `block.timestamp` must be after the tournament has finished.
     * - `tokenId` must be in the `winningBrackets` array
     * Emits a {LogRewardClaimed} event.
     */
    function _claim(uint256 tokenId) private {
        require(
            block.timestamp > mmOracle.tournamentEnd(),
            "MM: Tournament has not ended"
        );

        address owner = nftBracket.ownerOf(tokenId);
        bool canClaim = false;
        uint256 reward;

        for (uint256 i = 0; i < winningBrackets.length; i++) {
            if (winningBrackets[i].tokenId == tokenId) {
                canClaim = true;
                reward = winningBrackets[i].reward;
                break;
            }
        }

        require(canClaim, "MM: Caller is not among winners");

        mmRewardPool.withdraw(owner, reward);

        emit LogRewardClaimed(owner, tokenId, reward);
    }
     ...
     * emits a {LogBracketScoreUpdated} event for every tokenId when the score is updated with his current owner.
    ...
    function updateBracketScores(
        ...
        //todo implement - that's a pseudo code

        for (uint256 i = 0; i < tokenIds.length; i++) {
            bytes32 currHash = keccak256(abi.encodePacked(brackets[i]));
            bytes32 storedHash = nftBracket.tokenIdTokenHash(tokenIds[i]);
            require(currHash == storedHash, "MM: Not a valid bracket");
            uint16 score = mmOracle.calcBracketPoints(brackets[i]);
            address owner = nftBracket.ownerOf(tokenIds[i]);

            emit LogBracketScoreUpdated(tokenIds[i], score, owner);

            for (uint256 k = winningBrackets.length - 1; k >= 0; k--) {
                if (score > winningBrackets[k].score) {
                    winningBrackets[k] = WinningBracket({
                        score: score,
                        tokenId: tokenIds[i],
                        reward: 0 // todo check how rewards are spread across the winners and how many are they
                    });
                }
            }
        }
    ...
    {
        return mmOracle.getBracketPotential(bracket);
    }

    /**
     * @dev Security function. If something bad happens, this is a mechanism we'd use to prevent further exploits / malicious actions. Once the `disaster` state is activated there is no way back. Current tokenIds stored as winners would still be able to claim their rewards via `claimOnDisaster` function
     * Requirements:
     *
    ...
    function setDisasterState() external onlyOwner whenPaused {
        disasterState = true;
        emit LogDisasterStateSet(msg.sender, disasterState);
    }
}
...
contract MMOracle is Ownable {
    ...
    uint256 public immutable tournamentStart;
    uint256 public immutable tournamentEnd;
    ...
    modifier onlyMatchingLengths(uint16[] memory bracket) {
        require(
            bracket.length == bracketLength,
            "MMOracle: lengths don't match"
        );
        _;
    }
     ...
     * Requirements:
     *
     * `bracket` length should be equal to `bracketResults` length
     */
        ...
        returns (uint16)
    {
        ...
        uint8 startIndex = roundIndexes[round];
        uint8 roundWinners = tournamentStage / roundIndex;

        for (uint8 i = 0; i < roundWinners; i++) {
            if (bracket[startIndex + i] == bracketResults[startIndex + i]) {
                bracketPoints += roundIndex;
            }
        }
    ...
    {
        //pseudo code
        //TODO We should take into consideration that there might be 2 teams which are going to compete with one another. Only one of them should get points for a win
        for (uint256 i = 0; i < bracketResults.length; i++) {
            if (bracket[i] == bracketResults[i]) {
                bracketPoints += bracketResults[i] * roundIndex;
            }
        }
    }
}
