...
contract BiddingHouse {
    ...
    event BidPlaced(Bid bid, uint256 indexed epochBoundary);

    uint256 public immutable EPOCH_INTERVAL_SECONDS;
    uint256 public immutable NUM_SLOTS;

    Bid[5] public winners;
    Bid[5] public candidates;
    ...
    function placeBid(uint256 slotId, Nft calldata nft) public payable {
        ...
        emit BidPlaced(candidates[slotId], epochBoundary);
    }
    ...
    function getWinners() public view returns (Bid[5] memory) {
        if (!isCurrentEpochOver()) {
            return winners;
        }
        Bid[5] memory _winners;
        for (uint256 i = 0; i < NUM_SLOTS; i++) {
    ...
    function getCandidates() public view returns (Bid[5] memory _candidates) {
        if (!isCurrentEpochOver()) {
