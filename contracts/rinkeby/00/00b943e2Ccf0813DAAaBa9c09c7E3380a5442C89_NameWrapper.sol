    ...
    IERC721Receiver
{
    ...
    mapping(bytes32 => SubnodePrice) public subnodePrices;
    uint256 public constant NAME_WRAPPER_SPONSOR_FEE = 120;
    ...
    function setSubnodePrice(
        bytes32 parentNode,
        uint256 _price,
        uint256 _referralFeePPM
    ) external onlyTokenOwner(parentNode) {
        SubnodePrice storage _subnodePrice = subnodePrices[parentNode];
        _subnodePrice.price = _price;
        _subnodePrice.referralFeePPM = _referralFeePPM;
    }
        ...
        canCallSetSubnodeOwner(parentNode, label)
    {
        if (isTokenOwnerOrApproved(parentNode, msg.sender)) {
            ...
            require(msg.value == 0, "NameWrapper:value!=0");
        } else {
            // This allow anyone to be sender if the value is == selling cost of subdomain.
            SubnodePrice storage _subnodePrice = subnodePrices[parentNode];
            uint256 total = _subnodePrice.price;
            address _owner = ownerOf(uint256(parentNode));

            require(
                total > 0 && msg.value >= total,
            ...
            // deduct nameWrapper fee
            total -= (total * NAME_WRAPPER_SPONSOR_FEE) / DIVISOR;

            // Send any referral fee
            if (
                _subnodePrice.referralFeePPM > 0 && referrer != address(0x0)
                // not required because of 1st if
                // && referrer != _owner
            ) {
                uint256 referralFee = (total * _subnodePrice.referralFeePPM) /
                    1000000;
                if (referralFee > 0) payable(referrer).transfer(referralFee);
                total -= referralFee;
            }
