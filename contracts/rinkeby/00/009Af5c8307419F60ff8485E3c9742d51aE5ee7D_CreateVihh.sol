...
//contract beckLoot is ERC721Enumerable, ReentrancyGuard, Ownable {
//    constructor() ERC721("Neo Tokyo: Identities", "NEOTI") Ownable() {



contract CreateVihh is Ownable, ReentrancyGuard, ERC721 {
    ...
    event mint(uint256 _tokenId, string _functionName, string _vihh);

    constructor() public ERC721("Vihh", "VIHH") {
        nextTokenId = 5;
        saleIsActive = false;
        publicSaleIsActive = false;
        defaultURI = "notyetavailable";
        maxTokenSupply = 15;
//        baseURIOverrided = "";
    }
//    function currentMintPrice() public view returns(uint256)
//    {
//        return _checkCurrentMintingPrice(nextTokenId);
//    }

    function getCurrentPrice() public view returns (uint64){
        return _checkCurrentMintingPrice();
    }
    function _checkCurrentMintingPrice() internal view returns (uint64){
        if (nextTokenId < 3) {
            return 0;
        }
        if (3 <= nextTokenId && nextTokenId < 5) {
    ...
    function _validateMintRequirements() internal { // uint price
        ...
        uint64 currentPrice = _checkCurrentMintingPrice();
        require(
    ...
    function mintPublicRandom() public payable nonReentrant {
        ...
        emit mint(nextTokenId, "publicRandomMint", "NA");
        nextTokenId ++;
        _setNextTokenId();
    }

    function mintPublicSpecific(string memory chosenVihh) public payable nonReentrant {
        require(publicSaleIsActive, "Public minting is currently closed.");
        _validateMintRequirements();
        _safeMint(this.owner(), nextTokenId);
        emit mint(nextTokenId, "publicSpecificMint", chosenVihh);
        nextTokenId ++;
        _setNextTokenId();
    }

    function mintWhitelist(string memory chosenVihh, string memory password) public payable nonReentrant {
        require(_validateWhitelistPassword(password), "Your password is not valid.");
        _validateMintRequirements();
        _safeMint(_msgSender(), nextTokenId);
        emit mint(nextTokenId, "whitelistMint", chosenVihh);
        nextTokenId ++;
        _setNextTokenId();
    }
    ...
    function mintSpecialTokens(uint startId) public onlyOwner {
       for (uint _tokenId = startId; _tokenId < nextTokenId; _tokenId++) {
           if (specialTokensIds[_tokenId]) {
               ...
               emit mint(nextTokenId, "specialMint", "NA");
           }
       }
    }
    ...
    function setWhitelistPasswords(bytes32[] memory _passwords) public onlyOwner {
       for (uint i = 0; i < _passwords.length; i++)
        {
            bytes32 _password = _passwords[i];
            whitelistPasswords[_password] = true;
        }
    }
    ...
    /**
     * @notice Withdraw eth out of the contract
     */
