...
contract Minter is AccessControlEnumerableUpgradeable, Provenance, PaymentSplitterUpgradeable {
    ...
    uint public maxBlockPurchase;      // Max that may be purchased per block by one address.
    ...
    function signedMint(uint256 numberOfTokens, bytes memory signature, bytes32 nonce) public payable {
        ...
        bool signatureIsValid = SignatureCheckerUpgradeable.isValidSignatureNow(
            mintSigner,
            hashTransaction(msg.sender, numberOfTokens, nonce),
            signature
        );
        ...
        maxPurchaseBehavior(numberOfTokens);
    ...
    function maxPurchaseBehavior(uint256 numberOfTokens) internal {
        // These two incur sstore each.
    ...
    function sharedMintBehavior(uint256 numberOfTokens)
        internal
    {
        require(numberOfTokens <= maxBlockPurchase, "Minter: maxBlockPurchase exceeded");
        require(numberOfTokens > 0, "Minter: numberOfTokens is 0");
        uint256 price = IRegistry(registry).getProjectPrice(tokenContract);
        require(price != 0, "Minter: price not set in registry");
        require(price * numberOfTokens == msg.value, "Minter: Sent ether value is incorrect");
