...
contract Vault is BaseContract {
  ...
  DepositableERC20 weth = DepositableERC20(payable(wethAddress));
  ...
  function updateEthPriceUniswap() public payable returns(uint) {
    uint ethPriceRaw = quoter.quoteExactOutputSingle(daiAddress,wethAddress,3000,100000,0);
    ethPrice = ethPriceRaw / 100000;
    return ethPrice;
  }

  function updateEthPriceChainlink() public payable returns(uint) {
  ...
  function rebalance() public payable onlyOwner {
  ...
  function annualDividend() public payable onlyOwner {
  ...
  function wrapEth() public payable onlyOwner  {
