...
contract Planet is ERC721, ERC721URIStorage, Pausable, Ownable {
    using Counters for Counters.Counter;
    using SafeMath for uint256;

    string private _baseURIPrefix = "https://frozen-sands-24247.herokuapp.com/";

    uint256 private constant maxTokensPerTransaction = 1;
    uint256 private tokenPrice = 50000000000000000; //0.05 ETH
    uint256 public constant nftsNumber = 2;
    uint256 private constant nftsPublicNumber = 2;

    Counters.Counter private _tokenIdCounter;

    event Giveaway(address to, uint256 tokensNumber);

    constructor() ERC721("GARY", "GB") {
        ...
        returns (uint256[] memory)
    {
        ...
        uint256 counter = 0;
        for (uint256 i = 1; i < nftsNumber; i++) {
            if (ownerOf(i) == _owner) {
                result[counter] = i;
                counter++;
            }
    ...
    function buyPlanets(uint256 tokensNumber) public payable whenNotPaused {
