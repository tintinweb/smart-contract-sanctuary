...
contract RandomSVG is ERC721URIStorage, VRFConsumerBase {
    ...
    uint256 constant MAX_PATHS = 10;
    uint256 constant MAX_COMMANDS = 5;
    uint256 constant SIZE = 500;
    string[] PATH_COMMANDS = ["M", "L"];
    ...
    event requestedRandomSVG(bytes32 indexed request, uint256 indexed tokenId);
    ...
    function finishMint(uint256 _tokenId) public {
        ...
        require(tokenIdToRandomNumber[_tokenId] > 0, "invalid random number");
    ...
    function _generateSVG(uint256 _randomNumber)
        public
        view
        returns (string memory finalSvg)
    {
        uint256 numberOfPaths = (_randomNumber % MAX_PATHS) + 1;
        finalSvg = string(
            abi.encodePacked(
    ...
    function _generatePath(uint256 _randomNumber)
        public
        view
        returns (string memory pathSvg)
    {
        uint256 pathCommands = (_randomNumber % MAX_COMMANDS) + 1;
        pathSvg = "<path d='";
        for (uint256 i = 0; i < pathCommands; i++) {
            uint256 newRNG = uint256(
                keccak256(abi.encode(_randomNumber, MAX_PATHS + i))
            );
            string memory pathCommand = _generatePathCommand(newRNG);
    ...
    function _generatePathCommand(uint256 _randomNumber)
        public
        view
        returns (string memory _pathCommand)
    {
        _pathCommand = PATH_COMMANDS[_randomNumber % PATH_COMMANDS.length];
        uint256 parameterOne = uint256(
            keccak256(abi.encode(_randomNumber, MAX_PATHS * 2))
        ) % SIZE;
        uint256 parameterTwo = uint256(
            keccak256(abi.encode(_randomNumber, MAX_PATHS * 3))
        ) % SIZE;
    ...
    function _formatTokenURI(string memory _imageURI)
        internal
        pure
