...
contract CarsToken is ICarsToken, ERC721, Ownable {
  ...
  // address of who can mint (set to Auction House)
  ...
  constructor(address _traits, address _minter, uint256 _maxTokens) ERC721("CarDAO", 'CARD') { 
  ...
  /** EXTERNAL */
  function mint() external override payable onlyMinter returns (uint256) {
    // require(tx.origin == _msgSender(), "Only EOA wallets allowed.");
    require(_msgSender() == minter, "Only auction house can mint");
    ...
    emit CarMinted(minted);
    return minted;
  }

  function burn(uint256 _carId) external override onlyMinter () {
    _burn(_carId);
    emit CarBurned(_carId);
  }
