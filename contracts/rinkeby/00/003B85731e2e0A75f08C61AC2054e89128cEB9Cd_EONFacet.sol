...
contract EONFacet {
    ...
    function transfer(address _to, uint256 _value) public returns (bool success) {
        ...
        require(frombalances >= _value, "EON: Not enough EON to transfer");
    ...
    function addApprovedContract(address _contract) external {
        ...
        require(s.approvedContractIndexes[_contract] == 0, "EONFacet: Approved contract already exists");
    ...
    function removeApprovedContract(address _contract) external {
        ...
        require(index > 0, "EONFacet: Approved contract does not exist");
    ...
    function increaseAllowance(address _spender, uint256 _value) external returns (bool success) {
        ...
        require(newAllowance >= l_allowance, "EONFacet: Allowance increase overflowed");
    ...
    function decreaseAllowance(address _spender, uint256 _value) external returns (bool success) {
        uint256 l_allowance = s.allowances[msg.sender][_spender];
        require(l_allowance >= _value, "EONFacet: Allowance decreased below 0");
        l_allowance -= _value;
    ...
    function mint(address _to, uint256 value) external {
        require(s.isMinter[msg.sender], "EONFacet: Thou shall not mint");
        _mint(_to, value);
    }

    function _mint(address _to, uint256 value) internal {
        uint256 maxSupply = 2000000000 * 1 ether;
        require(s.totalSupply++ <= maxSupply, "EONFacet: MAX SUPPLY REACHED, PLANET IS DRY");
        s.balances[_to] += value;
        s.totalSupply += uint96(value);
        emit Transfer(address(0), _to, value);
    }

    function burn(address _from, uint256 value) external {
        s.balances[_from] -= value;
        emit Transfer(_from, address(0), value);
    }

    function setMinter(address minter, bool status) external {
        LibDiamond.enforceIsContractOwner();
        s.isMinter[minter] = status;
    }
}
