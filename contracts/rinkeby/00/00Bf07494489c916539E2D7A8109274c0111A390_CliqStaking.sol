...
contract CliqStaking {
    ...
    // function to set withdraw time for BNB and token
    function setWithdrawTime(uint256 withdrawTime)
        external
        onlyOwner
        returns (bool)
    {
        require(
            withdrawTime != 0,
            "Zero Time not Supported, Please Try Again!!!"
        );
        _withdrawTime = withdrawTime * 1 days;
        //1296000
        return true;
    }

    // function to get withdraw time for Token and BNB
    ...
    modifier tokenStakeCheck(uint256 tokens, uint256 timePeriod) {
        ...
        require(
            timePeriod == 30 || timePeriod == 60 || timePeriod == 90,
    ...
    function withdrawStakedTokens(uint256 stakingId) public returns (bool) {
        ...
        if (_tokenTotalDays[stakingId] == 30) {
            require(
                now >= _tokenStakingStartTime[stakingId] + _withdrawTime,
        ...
        } else if (_tokenTotalDays[stakingId] == 60) {
            require(
                now >= _tokenStakingStartTime[stakingId] + _withdrawTime.mul(2),
        ...
        } else if (_tokenTotalDays[stakingId] == 90) {
            require(
                now >= _tokenStakingStartTime[stakingId] + _withdrawTime.mul(3),
    ...
    function getBNBRewardDetailsByStakingId(uint256 id)
        ...
        returns (uint256)
    {
        if (_bnbTotalDays[id] == 30) {
            return ((_usersBNB[id] * 10) / 100);
        } else if (_bnbTotalDays[id] == 60) {
            return ((_usersBNB[id] * 20) / 100);
        } else if (_bnbTotalDays[id] == 90) {
            return ((_usersBNB[id] * 30) / 100);
        } else {
            return 0;
        }
    }
        ...
        returns (uint256)
    {
        if (_bnbStakingEndTime[id] > now) {
            if (_bnbTotalDays[id] == 30) {
                return ((_usersBNB[id] * 3) / 100);
            } else if (_bnbTotalDays[id] == 60) {
                return ((_usersBNB[id] * 6) / 100);
            } else if (_bnbTotalDays[id] == 90) {
                return ((_usersBNB[id] * 9) / 100);
            } else {
                return 0;
            }
        } else {
    ...
    function withdrawStakedBNB(uint256 stakingId) public returns (bool) {
        ...
        if (_bnbTotalDays[stakingId] == 30) {
            require(
                now >= _bnbStakingStartTime[stakingId] + _withdrawTime,
        ...
        } else if (_bnbTotalDays[stakingId] == 60) {
            require(
                now >= _bnbStakingStartTime[stakingId] + _withdrawTime.mul(2),
        ...
        } else if (_bnbTotalDays[stakingId] == 90) {
            require(
                now >= _bnbStakingStartTime[stakingId] + _withdrawTime.mul(3),
