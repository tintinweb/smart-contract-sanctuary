...
contract UniswapV2ERC20 is IUniswapV2ERC20 {
    ...
    string public constant name = 'PolkaBridge AMM V1';
    string public constant symbol = 'PBRAMM-V1';
...
contract UniswapV2Pair is IUniswapV2Pair, UniswapV2ERC20 {
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'PolkaBridge AMM V1: OVERFLOW');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, address polkaTreasury) external lock {
        ...
        { // scope for _token{0,1}, avoids stack too deep errors
            ...
            IUniswapV2Callee(to).uniswapV2Call(msg.sender, amount0Out, amount1Out, new bytes(0));
            balance0 = IERC20(_token0).balanceOf(address(this));
            balance1 = IERC20(_token1).balanceOf(address(this));
        }
        ...
        {
            if(amount0In > 0) {
                _safeTransfer(token0, polkaTreasury, amount0In.div(5));
                balance0 = balance0 - balance0.div(5);
            }
            if(amount1In > 0) {
                _safeTransfer(token1, polkaTreasury, amount1In.div(5));
                balance1 = balance1 - balance1.div(5);
            }
        }
