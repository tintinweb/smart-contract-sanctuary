...
contract ExchangeNFT is IERC1155Receiver, Ownable {
    ...
    function exchangeNFT(address nftContractAddress, uint256 tokenId, uint256 value) public {
        ...
        nft.safeTransferFrom(msg.sender, address(this), tokenId, value, "");

        IERC1155 nft_exchange = IERC1155(contractAddress);
        nft_exchange.safeTransferFrom(address(this), msg.sender, tokenId, value, "");
        tokens--;
    }

    function exchangeCitizenNFT(address nftContractAddress, uint256[] calldata id) public view returns (uint256[] memory) {
        IERC1155 nft = IERC1155(nftContractAddress);
        address[] memory account = new address[](1);
        account[0] = msg.sender;
