...
contract NFTStaking is ERC721, ERC721Enumerable {
    ...
    uint256 constant tokenRewardPerMonth = 100 ether;
    ...
    */
    function stake(uint256 tokenId, uint256 months) public {
        ...
        uint256 unlockTimestamp = block.timestamp + 31 days * months;
        locks[tokenId] = NFTLock(msg.sender, unlockTimestamp);

        // Mint the reward
        uint256 tokenAmount = tokenRewardPerMonth * months;
        token.mintReward(msg.sender, tokenRewardPerMonth * months);
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (nftLock.unlockTimestamp < block.timestamp) {
            parts[1] = string(
                abi.encodePacked(
                    "Owner: ",
                    abi.encodePacked(nftLock.owner),
                    " - Staked until: ",
                    nftLock.unlockTimestamp.toString()
                )
            );
        } else {
