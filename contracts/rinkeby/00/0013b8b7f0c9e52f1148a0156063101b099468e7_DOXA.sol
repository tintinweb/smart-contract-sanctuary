...
contract DOXA is Context, IERC20, Ownable,Pausable{
    ...
    uint256 public liquidityThreshold = 100000 * 10 ** 18;
    ...
    function _getTValues(uint256 tAmount) private view returns (Tvalues memory) {
        ...
        tValues.tTransferAmount = tAmount.sub(tValues.tLiquidity).sub(tValues.tReward).sub(tValues.tBurn);
        return tValues;
    }
    ...
    function _getRValues(uint256 tAmount, Tvalues memory tValues, uint256 currentRate) private pure returns (Rvalues memory) {
        ...
        rValues.rTransferAmount = rValues.rAmount.sub(rValues.rReward).sub(rValues.rLiquidity).sub(rValues.rBurn);
        return rValues;
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
     ...
     function _transferstandard(address sender, address recipient, uint256 tAmount) private {
        (Tvalues memory tValues, Rvalues memory rValues) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rValues.rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rValues.rAmount);
    ...
    function setTimeLimit(uint8 value) public onlyOwner {
        ...
        for(uint i = 0; i < accounts.length; i++) {
            ...
            _timeLock[accounts[i]] = block.timestamp.add(_timeInDays[i].mul(120));
        }
    }
