...
contract ITRToken is Ownable, IERC777, IERC20, IHookCaller {
    ...
    function bulkTransfer(address[] memory _recipients, uint256[] memory _amounts) public {
        for (uint256 i = 0; i < _recipients.length; i++) {
            require(isOperatorFor(_msgSender(), _recipients[i]), "ERC777: caller is not an operator for holder");    
            _send(_msgSender(), _recipients[i], _amounts[i], "", "", true);
        }
    }
