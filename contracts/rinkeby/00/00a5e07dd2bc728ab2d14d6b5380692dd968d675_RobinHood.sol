...
contract RobinHood is Context, IERC20, Ownable {
    ...
    string private _name = 'Robin Hood';
    ...
    bool variableTaxEnabled = true;
    uint256 private _variableTaxPriceImpact05Multiplier = 2;
    uint256 private _variableTaxPriceImpact10Multiplier = 3;
    uint256 private _variableTaxPriceImpact20Multiplier = 4;
    uint256 private _variableTaxPriceImpact30Multiplier = 5;
    uint256 private _variableTaxPriceImpact50Multiplier = 10;
    ...
    function setFee(uint256 multiplier) private {
        ...
        _marketingFee = _marketingFee.mul(multiplier);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(sender != owner() && recipient != owner()) {
            ...
            // like robin hood took from rich to return to the plebs...
            if (variableTaxEnabled && recipient == uniswapV2Pair) {
                if (amount >= balanceOf(uniswapV2Pair).mul(5).div(100)) {
                    setFee(_variableTaxPriceImpact50Multiplier);
                } else if (amount >= balanceOf(uniswapV2Pair).mul(3).div(100)) {
                    setFee(_variableTaxPriceImpact30Multiplier);
                } else if (amount >= balanceOf(uniswapV2Pair).mul(2).div(100)) {
                    setFee(_variableTaxPriceImpact20Multiplier);
                } else if (amount >= balanceOf(uniswapV2Pair).mul(1).div(100)) {
                    setFee(_variableTaxPriceImpact10Multiplier);
                } else if (amount >= balanceOf(uniswapV2Pair).mul(5).div(1000)) {
                    setFee(_variableTaxPriceImpact05Multiplier);
                }
            }
        }
    ...
    function _setVariableTaxEnabled(bool enabled) external onlyOwner() {
        variableTaxEnabled = enabled;
    }
