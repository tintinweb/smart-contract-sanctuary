...
contract Trade {
    ...
    function tradeAsset(Order memory order, Fee memory fee, bytes memory data) internal virtual {
        ...
        if(order.nftType == BuyingAssetType.ERC1155)  {
            transferProxy.erc1155safeTransferFrom(IERC1155(order.nftAddress), order.seller, order.buyer, order.tokenId, order.qty, data); 
        }
    ...
    function buyAsset(Order memory order, Sign memory sign, bytes memory data) public returns(bool) {
        ...
        tradeAsset(order, fee, data);
        emit BuyAsset(order.seller , order.tokenId, order.qty, msg.sender);
        return true;
    }

    function executeBid(Order memory order, Sign memory sign, bytes memory data) public returns(bool) {
        Fee memory fee = getFees(order.amount, order.nftType, order.nftAddress, order.tokenId);
