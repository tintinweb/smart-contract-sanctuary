...
contract Borgs is CommonObjects, QuickSort{
    ...
    function generateToken() external returns(string[] memory hexPixals)
    {
        ...
        for(uint256 i=0;i<_layerMetaData.length;i++){
            ...
            for(uint256 i=0;i<layer.layerItemSize;i++){
                LayerItem memory layerItemToAdd = layer.layerItems[i];
                items[i] = layerItemToAdd;
            }
        ...
        string memory strPixals = toString(hexPixals);
        emit Pixals(strPixals);
        _tokens.push(Token(true, strPixals));
    }
    
    function toString(string[] memory pixals) public view returns(string memory value){
        if(pixals.length > 0)
        {
            value = pixals[0];
        }
        ...
        for(uint256 i=1;i<pixals.length;i++){
             bytes memory queryBytes = abi.encodePacked(value, ',', pixals[i]);
             value = string(queryBytes);
        }
        return value;
    }
    ...
    function addLayerItem(string memory layerName, uint256 chance, string memory hexImage) external{
        ...
        uint256 position = layer.layerItemSize + 1;
