...
contract Operate is Ownable {
    ...
    function edit_user(uint256 _user_id, address _account_address, string[] memory _fname_lname, string memory _email, uint256 _phone_number, string[] memory _gen_age_birth, string memory _residential_address, string memory _ip_address, string memory _location_coordinates, string memory _curr_date) public{
        require(msg.sender == id_to_user[_user_id].account_address, "User is not authorized!");

        if(_account_address != address(0)){
            id_to_user[_user_id].account_address = _account_address;
        }
        if(bytes(_fname_lname[0]).length != 0){
            id_to_user[_user_id].first_name = _fname_lname[0];
        }
        if(bytes(_fname_lname[1]).length != 0){
            id_to_user[_user_id].last_name = _fname_lname[1];
        }
        if(bytes(_email).length != 0){
            id_to_user[_user_id].email = _email;
        }
        if(bytes(_gen_age_birth[0]).length != 0){
            id_to_user[_user_id].gender = _gen_age_birth[0];
        }
        if(bytes(_gen_age_birth[1]).length != 0){
            id_to_user[_user_id].age = _gen_age_birth[1];
        }
        if(bytes(_gen_age_birth[2]).length != 0){
            id_to_user[_user_id].birth_date = _gen_age_birth[2];
        }
        if(_phone_number != 0){
            id_to_user[_user_id].phone_number = _phone_number;
        }
        if(bytes(_residential_address).length != 0){
