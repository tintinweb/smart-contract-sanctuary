...
contract DepoV1BondDepository is DepoAccessControlled {
    ...
    IERC20 public immutable DEPO; // token given as payment for bond
    address public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints DEPO when receives principle
    ...
    struct Bond {
        uint256 payout; // DEPO remaining to be paid
    ...
    function deposit(
        ...
        uint256 value = treasury.tokenValue(principle, _amount);
        uint256 payout = payoutFor(value); // payout to bonder is computed

        require(payout >= 10000000, "Bond too small"); // must be > 0.01 DEPO ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) DEPO
         */
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            DEPO.transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            DEPO.approve(address(staking), _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return DEPO.totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    /**
     *  @notice calculate current ratio of debt to DEPO supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(DEPO).totalSupply();
    ...
    /**
     *  @notice calculate amount of DEPO available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or DEPO) to the DAO
     *  @return bool
     */
    function recoverLostToken(address _token) external returns (bool) {
        require(_token != address(DEPO));
