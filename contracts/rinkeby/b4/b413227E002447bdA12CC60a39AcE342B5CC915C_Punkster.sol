...
contract PairedNFT is ERC721 {
    ...
    IERC721 immutable token0;
    IERC721 immutable token1;
...
contract Punkster is PairedNFT, IERC721Receiver {
    ...
    constructor(address _head, address _body, string memory _baseURI)  PairedNFT(_head, _body, "Punkster", "PNK", _baseURI) {
        head = _head;
        body = _body;
    }
    ...
    function onERC721Received(address, address from, uint256 tokenId, bytes calldata data) external override virtual returns (bytes4) {
        ...
        otherToken.transferFrom(from, address(this), tokenId);
