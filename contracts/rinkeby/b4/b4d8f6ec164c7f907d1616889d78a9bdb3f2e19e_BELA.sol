...
contract BELA is Context, IERC20, Ownable {
    ...
    string private _name = "ForBXXXXXX";
    string private _symbol = "FXXB";
    ...
    address payable public charityAddress = 0x0690BddB1e37897C23022A38e8C92715aC0a35af; // Charity Wallet Address
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _takeLpAndMarketingAndLPFee(tValues.tLpReward,tValues.tMarketing,tValues.tLPFee);
    ...
    function _getTValues(uint256 tAmount) private view returns (TValues memory values) {
        ...
        values.tLPFee = calculateLPFee(tAmount);
        values.tTransferAmount = tAmount.sub(values.tFee).sub(values.tBurn).sub(values.tLpReward).sub(values.tMarketing).sub(values.tLPFee);
    }
    ...
    function _getRValues(uint256 tAmount, TValues memory tValues) private view returns (RValues memory values) {
        ...
        values.rMarketing = tValues.tMarketing.mul(values.rate);
        values.rLPFee = tValues.tLPFee.mul(values.rate);
        values.rTransferAmount = values.rAmount.sub(values.rFee).sub(values.rBurn).sub(values.rLpReward).sub(values.rMarketing).sub(values.rLPFee);
    }
   ...
   function _takeLpAndMarketingAndLPFee(uint256 tLiquidity,uint256 tMarketing, uint256 tLPFee) private {
        uint256 currentRate =  _getRate();
        ...
        _rOwned[owner()] = _rOwned[owner()].add(rMarketing);
        if(_isExcluded[owner()])
            _tOwned[owner()] = _tOwned[owner()].add(tMarketing);
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
       ...
       function TransferCharityETH(address payable recipient, uint256 amount) private {
        ...
        _LPFee = _LPFee.add(address(this).balance);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 && _lpRewardFee == 0 && _MarketingFee == 0) return;
