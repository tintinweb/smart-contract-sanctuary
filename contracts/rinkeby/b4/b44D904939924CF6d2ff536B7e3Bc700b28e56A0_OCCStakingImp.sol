...
contract OCCStakingImp is Ownable, ReentrancyGuard {
    ...
    mapping(address => StakeTransfer) public stakeTransferRequests;
    ...
    function makeTransferRequest(address _recipient, uint _amount) public {
        stakeTransferRequests[msg.sender] = StakeTransfer(_recipient, _amount);
    }

    function performTransferStake(address _requester, address _recipient, uint _amount) public {
        require(msg.sender == assetManager, "OCCStakingImp: not authorized to transfer stake.");
        StakeTransfer storage stakeTransfer = stakeTransferRequests[_requester];
        require(stakeTransfer.recipient == _recipient, "OCCStakingImp: recipient does not match.");
        require(stakeTransfer.amount == _amount, "OCCStakingImp: amount does not match.");
        require(_amount <= stakes[_requester], "OCCStakingImp: not enough stake to transfer");
        _updateReward(_requester);
        _updateReward(_recipient);
        stakes[_requester] = stakes[_requester].sub(_amount);
        stakes[_recipient] = stakes[_recipient].add(_amount);
        emit TransferStake(_requester, _recipient, _amount);
    }

    function changeAssetManager(address newAssetManager) public onlyOwner {
