...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    */
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
        ...
        uint256 currentEvolutionLevel = evolutionLevelMemory[tokenId];
        if (currentEvolutionLevel == 5) {
        setEvolutionLevel(tokenId, 1);
        }
        else {
        evolveNFT(tokenId);
        }
    ...
    function evolveNFT(uint256 NFTId) private {
    ...
    function setEvolutionLevel(uint256 NFTId, uint256 newEvolutionLevel) private {
        evolutionLevelMemory[NFTId] = newEvolutionLevel;
        emit EvolvedNFT(NFTId, newEvolutionLevel);
    }   
...
contract infinityTokensPublic is ERC721, Ownable, HasSecondarySaleFees {
	using SafeMath for uint256;    
	...
	mapping(uint256 => string) staticMetadataMemory;
	mapping(uint256 => string) dynamicMetadataMemory;
	...
	mapping(uint256 => bool) isRedeemedMemory;
	...
	constructor(string memory _name, string memory _symbol, address factoryAddress, address contractCreator) ERC721(_name, _symbol) public {
	updateURI(string(abi.encodePacked("https://infinitytokens.azurewebsites.net/api/HttpTrigger?artContract=",address(this),"&id=")));
	NFTIndex = 1;
	fileIPFSReferenceURL = "https://ipfs.infura.io/ipfs/";
    fileArweaveReferenceURL = "https://arweave.net/";
    updateMultiURI("https://joyworldmulti.azurewebsites.net/api/HttpTrigger?id=", "&artworkIndex=");
    factoryContract = factoryAddress;
    artistWalletAddress = contractCreator;
	}  

	event NewNFTMouldCreated(uint256 NFTIndex, string artworkHashIPFS, string artworkHashArweave, string staticMetadata, string dynamicMetadata, string artworkType, uint256 editionSize, uint256 price);
	...
	event RedeemNFT(uint256 tokenId);
	...
	function createNFTMould(
		...
		staticMetadataMemory[NFTIndex] = staticMetadata;
		dynamicMetadataMemory[NFTIndex] = dynamicMetadata;
		...
		priceMemory[NFTIndex] = price;
		
		uint256 tokenId = totalSupply() + 1;
		
		if (createWithProof == true) {
		    artworkNFTReference[tokenId] = NFTIndex;
		    editionNumberMemory[tokenId] = 0;
		    _safeMint(msg.sender, tokenId);
		    totalCreated[NFTIndex] = 0;
		    totalMinted[NFTIndex] = 0;
		    isRedeemedMemory[tokenId] = false;
		}
		else {
		    totalCreated[NFTIndex] = 0;
		    totalMinted[NFTIndex] = 0;
		...
		emit NewNFTMouldCreated(NFTIndex, artworkHashIPFS, artworkHashArweave, staticMetadata, dynamicMetadata, artworkType, editionSize, price);
    ...
    function redeemNFT(uint256 tokenId) public {
        ...
        require(isRedeemedMemory[tokenId] == false, "Token has already been redeemed");
        isRedeemedMemory[tokenId] = true;
        
        emit RedeemNFT(tokenId);
    }

    function ownerMint(uint256 NFTId, uint256 amountToMint) public onlyOwner {
        require(mintingActive[NFTId] == true, "Mint not active");
        require(totalMinted[NFTId] + amountToMint <= editionSizeMemory[NFTId]);
        
        for(uint256 i=totalMinted[NFTId]; i<amountToMint + totalMinted[NFTId]; i++) {
            uint256 tokenId = totalSupply() + 1;
        ...
        totalMinted[NFTId] = totalMinted[NFTId] + amountToMint;
	...
	function NFTMachineFor(uint256 NFTId, address _recipient) public payable {
		require(mintingActive[NFTId] == true, "Mint not active");
		uint256 editionId = totalMinted[NFTId] + 1;
		require(editionId <= editionSizeMemory[NFTId], "Cannot mint more");
		require(msg.value == priceMemory[NFTId]);
	...
	function getMetadata(uint256 tokenId) public view returns (uint256 NFTRef, string memory staticMetadata, string memory dynamicMetadata, uint256 editionSize, uint256 editionNumber, uint256 evolutionLevel, bool isRedeemed, bool isActive, uint256 price) {
		require(_exists(tokenId), "Token does not exist.");
		NFTRef = artworkNFTReference[tokenId];
		
		staticMetadata = staticMetadataMemory[NFTRef];
		dynamicMetadata = dynamicMetadataMemory[NFTRef];
		editionSize = editionSizeMemory[NFTRef];
		editionNumber = editionNumberMemory[tokenId];
		evolutionLevel = evolutionLevelMemory[NFTRef];
        isRedeemed = isRedeemedMemory[tokenId];
		isActive = mintingActive[NFTRef];
		price = priceMemory[NFTRef];
	}
	...
	function NFTMouldMetadata(uint256 NFTId) public view returns (string memory staticMetadata, string memory dynamicMetadata, uint256 editionSize, uint256 evolutionLevel, bool isActive, uint256 price) {
		staticMetadata = staticMetadataMemory[NFTId];
		dynamicMetadata = dynamicMetadataMemory[NFTId];
		editionSize = editionSizeMemory[NFTId];
		
		evolutionLevel = evolutionLevelMemory[NFTId];
		
		isActive = mintingActive[NFTId];
		price = priceMemory[NFTId];
	}
	...
	function updateMultiURI(string memory newURI1, string memory newURI2) public onlyOwner {
        multiURI1 = newURI1;
        multiURI2 = newURI2;
    }
    
    function multiURI(uint256 tokenId, uint256 artworkIndex) external view returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        return string(abi.encodePacked(multiURI1, integerToString(tokenId), multiURI2, integerToString(artworkIndex)));
    }
