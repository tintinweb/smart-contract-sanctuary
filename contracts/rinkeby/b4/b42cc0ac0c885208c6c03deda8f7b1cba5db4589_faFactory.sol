...
contract faFactory is Ownable, ERC721, ERC721Enumerable {
   ...
   /* function battleFA(uint16 _fromId, uint16 _toId) external {
    ...
    function getNFTinfo(uint16 _id) public view returns (uint32 [7] memory) {
        uint32 [7] memory _statsArray = [faArray[_id].dna,faArray[_id].Life,faArray[_id].Armour,faArray[_id].Attack,faArray[_id].Defence,faArray[_id].Magic,faArray[_id].Stamina];
        return _statsArray;
    }
