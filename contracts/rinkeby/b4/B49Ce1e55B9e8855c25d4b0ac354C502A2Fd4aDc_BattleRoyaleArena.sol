...
contract BattleRoyale is ERC721Tradable {
  ...
  )
  {
    ...
    artist = _artist;
    feeRate = _feeRate;
    transferOwnership(_arenaContract);
  }
  /*
   * Mint NFTs
   */
  ...
  function setIntervalTime(uint256 _intervalTime) external onlyOwner payable returns (uint256) {
   ...
   */
  function autoStartOn(bool _autoStart) external payable onlyOwner {
    autoStart = _autoStart;
  }
  /*
   * Toggle auto-start on or off
   */
  function autoPayoutOn(bool _autoPayout) external payable onlyOwner {
    autoPayout = _autoPayout;
  }
  /*
   * Set Fee Rate - aggreed rate the contract takes from the artist for initial sale
   */
  function setFeeRate(uint256 _feeRate) external onlyOwner payable {
    feeRate = _feeRate;
  }
  /*
   * setUnitsPerTransaction
   */
  function setUnitsPerTransaction(uint256 _units) external onlyOwner payable {
    unitsPerTransaction = _units;
  }
  /*
   * setMaxSupply
   ...
   * set artist
   */
  function setArtist(address payable _artist) external onlyOwner payable {
    artist = _artist;
  }
  /*
  ...
  function notifyGameEnded() internal {
    BattleRoyaleArena arena = BattleRoyaleArena(payable(owner()));
   ...
   * payout artist
   */
  function executePayout() public onlyOwner payable {
    executeAutoPayout();
  }
...
contract BattleRoyaleArena is Ownable, VRFConsumerBase {
  using AddressArray for AddressArray.Addresses;
   ...
   */
  function getCurrentBalance() external onlyOwner view returns (uint256) {
    uint256 balance = address(this).balance;
    return balance;
  }
  /*
   ...
   */
  function withdrawLink(uint256 amount) external onlyOwner returns (bool){
   ...
   */
  function getCurrentLinkBalance() external onlyOwner view returns (uint256) {
    return LINK.balanceOf(address(this));
  }

  /* ===== Battle Royale Arena Methods ===== */
  function addToBattleQueue(address payable _nftAddress) external payable onlyOwner returns(bool) {
  ...
  function removeFromQueue(address payable nftAddress) external onlyOwner payable returns(address payable[] memory) {
  ...
  /* ===== Battle Royale Contract Methods ===== */
  function setPrice(address payable _nftAddress, uint256 _price) external payable onlyOwner {
  ...
  function beginBattle(address payable _nftAddress) public onlyOwner {
    require(_nftAddress != address(0));
    BattleRoyale battle = BattleRoyale(_nftAddress);
   ...
   * setUnitsPerTransaction
   */
  function setUnitsPerTransaction(address payable _nftAddress, uint256 _units) external onlyOwner payable {
    require(_nftAddress != address(0));
    BattleRoyale battle = BattleRoyale(_nftAddress);
   ...
   * setMaxSupply
   */
  function setMaxSupply(address payable _nftAddress, uint256 _supply) external onlyOwner payable {
    require(_nftAddress != address(0));
    BattleRoyale battle = BattleRoyale(_nftAddress);
   ...
   * @param string IPFS meta-data uri
   */
  function setDefaultTokenURI(address payable _nftAddress, string memory _tokenUri) external onlyOwner payable {
    require(_nftAddress != address(0));
    BattleRoyale battle = BattleRoyale(_nftAddress);
   ...
   * @param string IPFS meta-data uri
   */
  function setPrizeTokenURI(address payable _nftAddress, string memory _tokenUri) external onlyOwner payable {
    require(_nftAddress != address(0));
    BattleRoyale battle = BattleRoyale(_nftAddress);
   ...
   * @param string IPFS meta-data uri
   */
  function setIntervalTime(address payable _nftAddress, uint256 _intervalTime) external onlyOwner payable {
    require(_nftAddress != address(0));
    BattleRoyale battle = BattleRoyale(_nftAddress);
   ...
   * Get Current ETH Balance from contract
   */
  function getCurrentBalanceInNFTContract(address payable _nftAddress) external view onlyOwner returns (uint256) {
    require(_nftAddress != address(0));
    BattleRoyale battle = BattleRoyale(_nftAddress);

    return battle.getCurrentBalance();
  }

  function withdrawBalanceInNFTContract(address payable _nftAddress, uint256 amount) external onlyOwner payable {
    require(_nftAddress != address(0));
    BattleRoyale battle = BattleRoyale(_nftAddress);
  ...
  function transferBattleContractOwnership(address payable _nftAddress, address _newOwner) external onlyOwner payable {
    require(_nftAddress != address(0));
    BattleRoyale battle = BattleRoyale(_nftAddress);

    battle.transferOwnership(_newOwner);
  }
  /*
   * Toggle auto-start on or off
   */
  function autoStartOn(address payable _nftAddress, bool _autoStart) external payable onlyOwner {
    require(_nftAddress != address(0));
    BattleRoyale battle = BattleRoyale(_nftAddress);
   ...
   * Toggle auto-start on or off
   */
  function autoPayoutOn(address payable _nftAddress, bool _autoPayout) external payable onlyOwner {
    require(_nftAddress != address(0));
    BattleRoyale battle = BattleRoyale(_nftAddress);
   ...
   * Set Fee Rate - aggreed rate the contract takes from the artist for initial sale
   */
  function setFeeRate(address payable _nftAddress, uint256 _feeRate) external onlyOwner payable {
    require(_nftAddress != address(0));
    BattleRoyale battle = BattleRoyale(_nftAddress);
  ...
  function executePayout(address payable _nftAddress) external onlyOwner payable {
    require(_nftAddress != address(0));
    BattleRoyale battle = BattleRoyale(_nftAddress);
  ...
  returns(
    ...
    for (uint i = 0; i < battleQueue.size(); i++) {
      ...
      uint256 timestamp = battle.getTimestamp();
      uint256 intervalTime = battle.getIntervalTime();
   ...
   */
    ...
    * TEST FUNCTIONS
    */
   function testBattleElimination() external onlyOwner payable returns (bool) {
     for (uint i = 0; i < battleQueue.size(); i++) {
       if (i >= battleQueue.size()) {
         return false;
       }
       ...
       if (battle.getBattleStateInt() == 1
         && (block.timestamp >= timestamp + (intervalTime * 1 minutes))) {
         // execute upkeep
         return catchTestBattleElimination(addressToBytes(nftAddress));
       }
     }
     return false;
   }

   function catchTestBattleElimination(bytes memory performData) internal returns (bool) {
     address payable nftAddress = bytesToAddress(performData);
...
contract ERC721Tradable is ERC721, Ownable {
   ...
   * Get Current ETH Balance from contract
   */
  function getCurrentBalance() external view onlyOwner returns (uint256) {
    uint256 balance = address(this).balance;
    return balance;
  }
  /**
