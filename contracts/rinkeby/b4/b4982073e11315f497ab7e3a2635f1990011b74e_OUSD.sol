...
contract OUSD is ERC20, Ownable {
    ...
    function deposit (
        ...
        uint256 collateral;
        if (staked) {
            require(allowStaked, "Only gOHM accepted");
    ...
    function initialize(uint256[] calldata inputs) external onlyOwner {
        require(terms.outstanding == 0, "Already initialized");
        terms.LT = inputs[0];
        terms.LI = inputs[1];
        terms.LTV = inputs[2];
        terms.CF = inputs[3];
