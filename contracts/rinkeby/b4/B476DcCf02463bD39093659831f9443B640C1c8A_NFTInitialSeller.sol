...
/**
 * Early Bird Sale: 5 METAVATARS, 44 AEONS, 51 EOS
 * Second Sale: 8 METAVATARS, 44 AEONS, 849 EOS
 *
...
contract NFTInitialSeller is Ownable {
  ...
  uint256 public constant METAVATARS_MINT_PRICE = 0.5 ether;
  uint256 public constant AEONS_MINT_PRICE = 0.3 ether;
  uint256 public constant EOS_MINT_PRICE = 0.19 ether;
  ...
  SaleStep private _currentSale = SaleStep.EarlyBirdSale;
    ...
    earlyBirdSalePeriod
  {
    ...
    require(nftIndex <= pendingCount - 901, "NFTInitialSeller: too high index");
    uint16 tokenId = _popPendingAtIndex(nftIndex + 901);
    require(
  ...
  function _getMintPrice(uint16 tokenId) internal pure returns (uint256) {
    require(tokenId > 901, "NFTInitialSeller: low token id");
    if (tokenId <= 906) return METAVATARS_MINT_PRICE;
    if (tokenId <= 950) return AEONS_MINT_PRICE;
    if (tokenId <= 1001) return EOS_MINT_PRICE;
  ...
  function _popPendingAtIndex(uint16 _index) internal returns (uint16) {
    ...
    if (_index != pendingCount) {
      uint16 lastPendingId = _getPendingAtIndex(pendingCount);
