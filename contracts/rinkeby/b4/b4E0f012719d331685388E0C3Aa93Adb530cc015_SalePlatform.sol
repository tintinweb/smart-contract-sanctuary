...
contract SalePlatform is ContinuousDutchAuction, ReentrancyGuard {
    ...
    event Purchased(uint256 dropId, uint256 tokenId, address to);

    mapping (uint => Sale) public sales;
    mapping (uint => MPClaim) public mpClaims;
    uint256 public artistPercentage; //10000 * percentage
    IQuantumArt public quantum;
    IQuantumMintPass public mintpass;

    address payable private quantumTreasury;
    ...
    constructor(
        ...
        quantumTreasury = treasury;
        artistPercentage = 8000; //default 80% for artist
    }
    ...
    function createSale(uint256 dropId, uint256 price, uint256 limit) onlyRole(MANAGER_ROLE) public {
        sales[dropId] = Sale({price:price, limit:limit, active:false});
    }

    function createMPClaim(uint256 dropId, uint256 mpId, uint256 price) onlyRole(MANAGER_ROLE) public {
        mpClaims[dropId] = MPClaim({mpId:mpId, price:price, active:false});
    }

    function setArtistPercentage(uint256 percentage) onlyRole(MANAGER_ROLE) public {
    ...
    function payout(address artist, uint256 amount) internal {
        uint256 payout_ = (amount*artistPercentage)/10000;
        Address.sendValue(payable(artist), payout_);
        Address.sendValue(quantumTreasury, amount - payout_);
    }
    ...
    */
    ...
    function claimWithMintPass(uint256 dropId, uint256 amount) nonReentrant payable public {
        ...
        if (msg.value > 0) payout(quantum.getArtist(dropId), msg.value);
    }
}
