...
contract Drrt is Context, ERC165, Ownable, IERC721Metadata {
    ...
    uint public constant TOKEN_PRICE = 7000000000000000;
    ...
    function mint(uint tokenAmount) public payable {
        ...
        require(msg.value >= TOKEN_PRICE, "Ether value sent is not correct");
    ...
    function getTokenURI(uint tokenIndex) public view returns (string memory) {
    ...
    function _burn(uint tokenIndex) internal virtual {
        address owner = ownerOf(tokenIndex);

        _beforeTokenTransfer(owner, address(0), tokenIndex);
        _approve(address(0), tokenIndex);
        _holderTokens[owner].remove(tokenIndex);
        _tokenOwners.remove(tokenIndex);

        emit Transfer(owner, address(0), tokenIndex);
    }
