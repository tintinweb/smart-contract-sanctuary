...
// File: contracts\PocketPuppies.sol
...
/**
 * @title PocketPuppies contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract PocketPuppies is ERC721, Ownable {
    using SafeMath for uint256;

    uint256 public constant basePrice = 80000000000000000; //0.08 ETH

    uint256 public constant maxPurchase = 10;

    uint256 public maxPuppies = 10000;

    bool public saleIsActive = false;

    constructor() public ERC721("PocketPuppies", "POP") {
        //TODO change this and check how to use this
        setBaseURI("TODO-change-this");
    }
    ...
    function getTokensForAddress(address fromAddress)
        external
        view
    ...
    /**
     * Get ids with address for a batch size of 100 tokens
     *
     */
    function getTokenAddressesForBatch(uint256 batch)
        external
        view
        returns (address[] memory)
    {
        uint256 fullBatchesAvailable = totalSupply() / 100;
        uint256 arraySize = 100;
        if (totalSupply() < 100 ) {
            arraySize = totalSupply();
        }
        
        if (batch == fullBatchesAvailable + 1) {
            arraySize = totalSupply() - fullBatchesAvailable * 100;
        }

        require(batch <= fullBatchesAvailable + 1, "provided batch is too high");

        address[] memory result = new address[](arraySize);
        uint256 startIndex = (batch - 1) * 100;
        uint256 endIndex = startIndex + arraySize;
        uint256 resultIndex = 0;
    ...
    /*
     * Set new maximum supply
     * only owner
     */
    function setMaxSupply(uint256 _maxPuppies) external onlyOwner {
        maxPuppies = _maxPuppies;
    }

    /**
     * Mint Puppy to address
     * only owner
     */
    ...
    function mint(uint256 numberOfTokens) public payable {
        ...
        require(
            totalSupply().add(numberOfTokens) <= maxPuppies,
            "Purchase would exceed max supply"
        );
        require(
            basePrice.mul(numberOfTokens) <= msg.value,
            "Ether value sent is not correct"
        );

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 mintIndex = totalSupply();
            if (totalSupply() < maxPuppies) {
