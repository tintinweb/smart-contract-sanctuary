...
contract Hashcards is Ownable, ReentrancyGuard, IERC721Receiver, ERC721("Hashcards", "HCT") {
    ...
    function mintCardDelegate(uint256 _noOfCards, address _receiver) nonReentrant saleStarted nextPhase public {
        ...
        if(currentPhase == Phase.ONE) {
            ...
            for (uint i = 0; i < _noOfCards; i++) {
                uint mintIndex = totalSupply();
                _safeMint(_receiver, mintIndex, abi.encodePacked(mintIndex));
            }
        }
    ...
    function finalizeStartingIndex() saleStarted public {
        ...
        startingIndex = uint(keccak256(abi.encodePacked(blockhash(startingIndexBlock), keccak256(abi.encodePacked(block.timestamp)))))%MAX_NFT_SUPPLY_PHASE_THREE;
        
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number.sub(startingIndexBlock) > 255) {
            startingIndex = uint(keccak256(abi.encodePacked(blockhash(block.number-1), keccak256(abi.encodePacked(block.timestamp)))))%MAX_NFT_SUPPLY_PHASE_THREE;
        }
