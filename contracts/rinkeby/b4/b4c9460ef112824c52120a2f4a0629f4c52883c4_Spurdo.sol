...
contract Spurdo is ERC721Metadata {
  ...
  uint256 public constant MINT_PRICE = 10000000000000000 wei;
  ...
  constructor(
    string memory baseURI_,
    address admin_
  ) ERC721Metadata("AVAX Spurdos", "SPURDO", baseURI_) {
    _admin = admin_;
  }
  ...
  function purchase(uint256 numberOfNfts) external payable {
    require(mintIsOpen == true, "Minting is closed.");
    require(pendingCount > 0, "AVAX Spurdos: All minted");
    require(numberOfNfts > 0, "AVAX Spurdos: numberOfNfts cannot be 0");
    require(
      numberOfNfts <= 20,
      "AVAX Spurdos: You may not buy more than 20 NFTs at once"
    );
    require(
      totalSupply().add(numberOfNfts) <= MAX_NFT_SUPPLY,
      "AVAX Spurdos: sale already ended"
    );
    require(
      MINT_PRICE.mul(numberOfNfts) == msg.value,
      "AVAX Spurdos: invalid ether value"
    );
  ...
  function claimReward(uint256 tokenId) public {
    require(
      ...
      "AVAX Spurdos: Only token owner can claim rewards"
    );
  ...
  function _randomMint(address _to) internal returns (uint256) {
    require(totalSupply() < MAX_NFT_SUPPLY, "AVAX Spurdos: max supply reached");
