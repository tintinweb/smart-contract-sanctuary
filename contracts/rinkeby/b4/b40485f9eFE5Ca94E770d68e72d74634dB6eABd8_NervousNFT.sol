...
contract NervousNFT is ERC721, ERC721Enumerable, PaymentSplitter, Ownable {
    ...
    function onAllowList() public view returns (bool) {
    ...
    function mintAllowList(uint256 numTokens) external payable {
        uint256 _totalSupply = totalSupply();
        require(allowListEnabled, "Allow list is not active");

        require(
            numTokens <= allowList[msg.sender],
            "Exceeded max available to purchase"
        );
        require(
            _totalSupply + numTokens <= MAX_TOKENS,
            "Purchase would exceed max tokens"
        );
        require(
            tokenPrice * numTokens <= msg.value,
            "Ether value sent is not correct"
        );

        allowList[msg.sender] -= numTokens;
    ...
    function mint(uint256 numTokens) public payable {
        ...
        require(
            SafeMath.add(totalSupply(), 1) <= MAX_TOKENS,
