...
contract Pandas is ERC721URIStorage, Ownable {
    ...
    modifier canWhitelistMint() {
        require(whiteListStart, "Whitelist mint has not started");
        _; 
    }
    ...
    function mintPandas(uint256 _times) payable public mintEnabled {
        ...
        require(totalPandas + _times <= totalCount, "too much");
        require(msg.value == _times * price, "value error");
        payable(owner()).transfer(msg.value);
        emit MintPandas(_msgSender(), totalPandas+1, _times);
        for(uint256 i=0; i< _times; i++){
            _mint(_msgSender(), 1 + totalPandas++);
        }
    } 
    ...
    function whitelistMint(uint _times) payable public canWhitelistMint {
        ...
        require(totalPandas + _times <= presaleMax, "Mint amount will exceed total presale amount.");
    ...
    function adminMintGiveaways(address _addr) public onlyOwner {
        require(totalPandas + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit MintPandas(_addr, totalPandas+1, 1);
        _mint(_addr, 1 + totalPandas++);
    } 
