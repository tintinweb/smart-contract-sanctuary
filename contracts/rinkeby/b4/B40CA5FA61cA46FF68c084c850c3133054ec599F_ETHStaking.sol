...
contract ETHStaking is IETHStaking, Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) public stakeNums;
    ...
    /*********** GETTERS ***********/
    function getApyPerSec() public view override returns (uint256) {
        return (apy * 1e18) / 31536000;
    }
        ...
        returns (uint256)
    {
        ...
        // console.log(
        //     "calculateSecs(): %s",
        //     calculateSecs(block.timestamp, _user.lastUpdated)
        // );

        return
            (((calculateSecs(block.timestamp, _user.lastUpdated) *
                (apy * 1e18)) / 31536000) * _user.amount) / 1e36;
    }
    ...
    /*********** ACTIONS ***********/
    function withdrawContractFunds(uint256 _amount)
        public
        override
        onlyOwner()
    {
    ...
    function stake() public payable override {
        ...
        uint256 _stakeNums = stakeNums[_msgSender()];
        uint256 _stakeNum;
    ...
    /*********** INTERNAL FUNCTIONS ***********/
    function calculateSecs(uint256 _to, uint256 _from)
        internal
        pure
        returns (uint256)
    {
    ...
    function _updateUserInfo(
        ...
        if (_actionType == ActionType.Unstake) {
            ...
            totalValueLocked = totalValueLocked - _amount;
            user.amount = 0;
            user.rewardPaid = user.rewardPaid + _reward;
        }
    }
}
...
interface IETHStaking {
    ...
    /*********** GETTERS ***********/
    function getApyPerSec() external view returns (uint256);
