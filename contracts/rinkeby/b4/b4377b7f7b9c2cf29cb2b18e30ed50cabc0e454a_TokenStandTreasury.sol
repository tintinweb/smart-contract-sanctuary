    ...
    ReentrancyGuardUpgradeable
{
    ...
    function transferGiffRewardToRef(
        address _ref,
        uint256 _amount,
        address _giff
    ) external onlyFarming {
        UserInfo storage user = userInfo[_ref];
        user.giffs[_giff] = user.giffs[_giff].add(_amount);
    }

    function claimFarming(uint256 _amount, address[] calldata _giffs) external {
        ...
        uint256[] storage amountGiffs;
        if (_giffs.length > 0) {
            for (uint256 i = 0; i < _giffs.length; i++) {
                if (user.giffs[_giffs[i]] > 0) {
                    amountGiffs.push(uint256(user.giffs[_giffs[i]]));
                    TransferHelper.safeTransfer(
                        _giffs[i],
                        msg.sender,
                        user.giffs[_giffs[i]]
                    );
                    user.giffs[_giffs[i]] = 0;
                } else {
                    amountGiffs.push(0);
                }
            }
        }
        emit ClaimFarming(msg.sender, _amount, _giffs, amountGiffs);
    }
    ...
    function pendingGiff(address _ref, address _giff)
        external
        view
        returns (uint256 pending)
    {
        UserInfo storage user = userInfo[_ref];
        return user.giffs[_giff];
    }
}
