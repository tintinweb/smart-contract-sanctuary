...
contract OraclesController is Ownable {
    ...
    /** @dev Registers a user as an Oracle for the chosen event. Before being able to register the user must
      ...
      * @param winnerVote uint256 of who they voted as winning
    */
        ...
        external
    {
        ...
        // }
        uint256 winner = events.getWinner(eventId);

        mvuRewardPool = oracles.getTotalOracleStake(eventId) - oracles.getStakeForOutcome(eventId, winner);

        uint256 twoPercentRewardPool = 2 * events.getTotalAmountResolvedWithoutOracles(eventId);
        twoPercentRewardPool /= 100;
        uint256 threePercentRewardPool = 3 * (events.getTotalAmountBet(eventId) - events.getTotalAmountResolvedWithoutOracles(eventId));
        threePercentRewardPool /= 100;
        uint256 totalRewardPool = (threePercentRewardPool/12) + (threePercentRewardPool/3) + (twoPercentRewardPool/8);
        uint256 stakePercentage = 100000 * oracles.getMvuStake(eventId, msg.sender);
    ...
    function transferTokensToMevu (address oracle, uint256 mvuStake) internal {
        mvuToken.transferFrom(oracle, address(mevu), mvuStake);
    }

    function withdraw (uint256 mvu) external {
