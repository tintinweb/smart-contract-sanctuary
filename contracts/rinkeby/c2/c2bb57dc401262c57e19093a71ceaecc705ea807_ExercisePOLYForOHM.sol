...
contract ExercisePOLYForOHM {
    ...
    function _setTerm(address _vester, uint _amountCanClaim, uint _rate ) internal {
    ...
    function setTerm(address _vester, uint _amountCanClaim, uint _rate ) external returns ( bool ) {
        require( msg.sender == owner, "Sender is not owner" );
        _setTerm( _vester, _amountCanClaim, _rate );
        return true;
    }
    
    function setTerms(address[] calldata _vesters, uint[] calldata _amountCanClaim, uint[] calldata _rate ) external returns ( bool ) {
        require( msg.sender == owner, "Sender is not owner" );
        
        for( uint256 i; _vesters.length > i; i++ ) {
            _setTerm( _vesters[i], _amountCanClaim[i], _rate[i] );
        }
