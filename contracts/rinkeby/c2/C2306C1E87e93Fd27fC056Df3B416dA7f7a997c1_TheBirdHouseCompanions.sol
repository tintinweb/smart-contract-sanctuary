...
abstract contract TheBirdHouseInterface {
...
contract TheBirdHouseCompanions is ERC721Enumerable, Ownable {
    /*
...
*/
    ...
    TheBirdHouseInterface private TheBirdHouseContract;

    //uint256's
    uint256 public maxSupply = 6000;
    uint256 public saleStartTime = 1628787600;
    ...
    constructor(address theBirdHouseAddress)
        ERC721("TheBirdHouseCompanions", "TBHC")
    {
        TheBirdHouseAddress = theBirdHouseAddress;
        TheBirdHouseContract = TheBirdHouseInterface(theBirdHouseAddress);
    }
...
*/
    ...
    function burnBirds(uint256[] memory birdIds) public {
        require(block.timestamp >= saleStartTime, "Sale must have started!");
        require(
        ...
        for (uint256 i = 0; i < birdIds.length; i++) {
            ...
            uint256 newCompanionId = birdId + 6000;

            require(newCompanionId < maxSupply * 2, "Companion ID too high!");
            require(birdId < maxSupply, "birdId too high!");
            require(
    ...
    function claimCompanionsById(uint256[] memory companionIds) public {
        require(block.timestamp >= saleStartTime, "Sale must have started!");
        require(
            companionIds.length > 0 && companionIds.length <= 50,
            "Can only claim between 0 and 50 birds!"
        );

        for (uint256 i = 0; i < companionIds.length; i++) {
            uint256 companionId = companionIds[i];

            require(companionId < maxSupply, "CompanionId too high!");
            require(
                TheBirdHouseContract.ownerOf(companionId) == msg.sender,
                "You must own the same bird Id!"
            );

            if (!_exists(companionId)) {
                //This doesn't exist, so mint it to them!
                _safeMint(msg.sender, companionId);
            }
        }
    }
...
|_|_\___/_/ \_\___/  |_|  \___/|_|\_|\___| |_| |___\___/|_|\_|___/
    */
        ...
        returns (uint256[] memory)
    {
        ...
        uint256[] memory _tokens = new uint256[](tokenCount);
