...
contract SeedGeneration is Ownable, Pausable, VRFConsumerBase {
    ...
    function callChainLink() internal returns (bytes32 requestId) {
    ...
    function Random(uint256 _someNumber) external returns (uint256 rnd) {
        require(whitelistedContracts[msg.sender], "You're not allowed to call this function");
        bytes32 requestId = callChainLink();
        rnd = uint256(keccak256(abi.encodePacked(requestId,tx.origin, blockhash(block.number - 1), block.timestamp, _someNumber, randomNumber)));
    ...
    function lastRequestID() external view onlyOwner returns (bytes32 requestId )  {
        requestId =  lastReqID;
    }
