...
contract LifeInWeeksNFT is ERC721Tradable {
  ...
  uint[MAX_SUPPLY] private _tokenData;
  ...
  function getRect(uint x, uint y) private  pure returns (string memory) {
    string memory rect = string(abi.encodePacked("<rect x='",uintToString(x),"' y='",uintToString(y),"' width='8' height='8'/>"));
    return rect;
  }

  function getSVG(uint birthyear) public pure returns (string memory) {

    uint currentyear = 2022; // TODO: get current year
    uint week = 13; // TODO: get week of year [0,52]

    string memory weekStr = uintToString(week);
  ...
  function tokenURI(uint256 tokenId) override(ERC721) public view returns (string memory) {
      ...
      uint birthyear = _tokenData[tokenId-1];
      ...
      string memory json = Base64.encode(
          bytes(string(
              abi.encodePacked(
                  ...
                  '"description": "100% on-chain dynamic display of your entire life in weeks, starting from ',birthyearStr,'. The rows are the years and the columns are the weeks (52 weeks per year). This generative SVG updates weekly.",',
   ...
   */
  function mint(uint birthyear) public payable {
  ...
  function reserve(uint birthyear) public onlyOwner {
    uint256 ts = totalSupply();
    uint numberOfTokens = 1;
