...
// STATE VARIABLES
    ...
    /// @dev Stores a mapping for all the whitelisted contracts
    mapping(address => Whitelist) public whitelisted;

    /// @dev True if an item has a whitelist
    mapping(uint256 => bool) public hasWhitelist;

// MODIFIERS

    /// @dev Checks whether an item is active & whitelisted
    /// @param itemID identifies the type of asset
...
// ADMIN FUNCTIONS
        ...
        onlyOwner
    {
        ...
        hasWhitelist[whitelist.itemID] = true;                                           // Sets as true to confirm item has a whitelist
        emit AddressWhitelisted(_msgSender(), block.number, whitelistAddress);           // Write an event to the chain
    }

    /// @dev Remove an address from the whitelist, by deactivating it
    /// @param whitelistAddress address to delist
    ...
    /// @dev Flips the whitelist check on an off
    /// @param itemID item to delist / relist
    function flipWhiteList(uint256 itemID)
        external
        onlyOwner
    {
        hasWhitelist[itemID] = !hasWhitelist[itemID];                                    // Flips the whitelist check variable
        emit AddressRemovedFromWhitelist(_msgSender(), block.number, address(0));        // Write an event to the chain
    }
...
abstract contract FarmlandItemDetails is ERC1155, FarmlandItemWhitelist, ReentrancyGuard {
...
// MODIFIERS       
    ...
    /// @param itemID identifies the type of asset
    modifier whenMintingActive(uint256 itemID) {
        ItemDetails memory item = items[itemID];    // Shortcut accessor
        require(item.mintingActive,                 "Minting not started");
        _;
    }
...
// EVENTS
    ...
    event ItemAdded(uint256 itemID, string name, string imageUrl, uint256 price, uint256 maxSupply, ItemType itemType, IERC20 paymentContract, bool mintingActive);
...
// USER FUNCTIONS
    ...
    function mintItem(uint256 itemID, uint256 amount, address recipient)
        ...
        whenMintingActive(itemID)
    {
        require(itemID < totalItems,                                                            "No itemID");
        ItemDetails storage item = items[itemID];                                               // Shortcut accessor
        require(item.totalSupply + amount <= items[itemID].maxSupply,                          "Out of stock");
        item.totalSupply += amount;                                                            // Increment amount
...
// ADMIN FUNCTIONS
    ...
    function reserveItem(uint256 itemID, uint256 amount) public onlyOwner {
        require(itemID < totalItems,                                "No itemID");
        ItemDetails storage item = items[itemID];                   // Shortcut accessor
        require(item.totalSupply + amount <= item.maxSupply,       "Not enough supply");
        item.totalSupply += amount;                                // Increment amount
        ...
        onlyOwner 
    {
        ...
        require(itemType != ItemType.Payable && paymentAddress != address(0),                           "Payable Items require a payment contract");
        uint256 newItemId = totalItems;                                                                 // Get the next item id
        ItemDetails memory item = ItemDetails(price, maxSupply, 0, itemType, itemName, imageUrl,
        ...
        emit ItemAdded(newItemId, itemName, imageUrl, price, maxSupply, 
    ...
    {
        require(itemID < totalItems,                            "No itemID");
        require(itemType != ItemType.Payable &&
                paymentAddress != address(0),                   "Payable Items require a payment contract");
    ...
    /// @param itemID identifies the type of asset
    function flipMintActive(uint256 itemID) external onlyOwner {
        ItemDetails storage item = items[itemID];                          // Shortcut accessor
        item.mintingActive = !item.mintingActive;                 // Flip true to false & vice versus
    }

    /// @dev Allows the owner to start & stop all minting
    function startOrStopMinting(bool value) external onlyOwner {
    ...
    /// @dev Allows the owner to withdraw the payment from the contract for an item
    /// @param itemID Identifies the type of asset
...
// GETTERS
    ...
    /// @param itemID Identifies the type of asset
    function mintingActive(uint256 itemID) public view returns(bool) {
        ItemDetails memory item = items[itemID];    // Shortcut accessor
        return item.mintingActive;           // Return if minting has started 
    }

    /// @dev Returns a list of all items
    function listItems() public view returns (ItemDetails[] memory _items) {
         uint256 total = totalItems;                  // Store totalItems into a local variable to save gas
