...
contract NFT is ERC721, Ownable {
    ...
    string public hiddenTokenURI = 'ipfs://QmcJ1e56cvXF5HqXcPDEqiZkWggxfiDvCv8ceU4ydEMECN';
    uint256 public maxSupply = 9999;
    uint256 public maxForSale = 9999;
    uint256 public whitelistStart = 1643652000; // 1/31/2022 6pm GMT
    uint256 public whitelistMaxMint = 2;
    uint256 public whitelistPrice = 0.06 ether;
    uint256 public presaleStart = 1643738400; // 2/1/2022 6pm GMT
    uint256 public presaleMaxMint = 2;
    uint256 public presalePrice = 0.07 ether;
    uint256 public publicStart = 1645380000; // 2/2/2022 6pm GMT
    ...
    external payable
    underMintLimit(quantity, whitelistMaxMint)
    validSignature(signature, 1)
    whitelistActive()
    {
        _mintSale(quantity, whitelistPrice);
    }

    function presaleMint(bytes memory signature, uint256 quantity)
    external payable
    underMintLimit(quantity, presaleMaxMint)
    validSignature(signature, 2)
    presaleActive()
    {
        _mintSale(quantity, presalePrice);
    }

    function publicMint(uint256 quantity)
    external payable
    underMintLimit(quantity, publicMaxMint)
    publicActive()
    {
        _mintSale(quantity, publicPrice);
    }

    function adminMint(address to, uint256 quantity) external onlyOwner {
        for(uint256 i = 0; i < quantity; i++) {
            require(_tokenIdTracker.current() < maxSupply, 'MINT: Supply is too low');
            _tokenIdTracker.increment();
            _safeMint(to, _tokenIdTracker.current());
        }
    }

    function _mintSale(uint256 quantity, uint256 price) internal {
        require(msg.value == quantity * price, 'MINT: VAlue is incorrect');
        for(uint256 i = 0; i < quantity; i++) {
            require(_tokenIdTracker.current() < maxForSale, 'MINT: supply is too low');
            _tokenIdTracker.increment();
    ...
    // MODIFIERS
    modifier underMintLimit(uint256 quantity, uint256 limit) {
        require(ERC721.balanceOf(msg.sender) + quantity <= limit, 'MINT: Quantity is too high');
        _;
    }
    ...
    modifier whitelistActive() {
        require(block.timestamp >= whitelistStart && block.timestamp < presaleStart, 'MINT: Whitelist is not active');
        _;
    }

    modifier presaleActive() {
        require(block.timestamp >= presaleStart && block.timestamp < publicStart, 'MINT: Presale is not active');
        _;
    }
