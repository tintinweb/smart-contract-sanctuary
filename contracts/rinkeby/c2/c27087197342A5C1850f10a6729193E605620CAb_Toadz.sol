...
contract Toadz is ERC721, PaymentSplitter, Ownable {
    ...
    //Dev mint legendary tokens
    function mintLegendary(address [] memory recipients, uint256 [] memory legendId) 
        external 
        onlyOwner 
    {        
        require (!devMintLocked,"Dev Mint Permanently Locked");
        for (uint256 i = 0; i < recipients.length; i++) {
            require (legendId[i]!=0);
            _safeMint(recipients[i],legendId[i]*1000000);
        }
    }
