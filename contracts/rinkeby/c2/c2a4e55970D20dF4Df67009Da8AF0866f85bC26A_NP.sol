...
import "./Interfaces/INPT20.sol";
import "@chainlink/contracts/src/v0.8/ChainlinkClient.sol";
import "@openzeppelin/contracts/access/Ownable.sol";


contract NP is ChainlinkClient, Ownable {
  using Chainlink for Chainlink.Request;

  // the only allowed NFT in this pool
  IERC721 public NFT;

  // the proof of ownership token for staked NFTs
  ...
  // maximum percentage of the NFT that can be staked
  uint8 public maxCollateralization;

  // Liquidity pool NPT/MGH // NPT/USDC
  address public LP;
  ...
  event NFTstaked(address indexed staker, address NFT, uint256 ID, uint256 indexed mintAmount);
  event NFTunstaked(address indexed staker, address NFT, uint256 ID, uint256 indexed burnAmount);
  ...
  function requestNFTvalue(uint256 _payment, string memory _jobId, uint256[] calldata _IDs) external {

    string memory idArray;

    for (uint256 i = 0; i < _IDs.length; i++) {
      idArray = append3String(idArray, uintToString(_IDs[i]), ";");
    }

    Chainlink.Request memory req = buildChainlinkRequest(stringToBytes32(_jobId), address(this), this.fulfillRequest.selector);
    req.add("idArray", idArray);
    bytes32 _requestId = sendChainlinkRequest(req, _payment);
 ...
 // TODO  maybe it is better to get the price already in NPT?
  function fulfillRequest(bytes32 _requestID, uint256[] calldata _price) external recordChainlinkFulfillment(_requestID) {
    require(_price.length == IDsbyRequest[_requestID].length, "lengths do not match");

    for (uint256 i=0; i < _price.length; i++) {
      NFTinfo[IDsbyRequest[_requestID][i]].price = _price[i];
      NFTinfo[IDsbyRequest[_requestID][i]].time = block.timestamp;
    }
  }

  /// @param  _IDs is an array of the NFTs that the user wants to stake
  /// @param  _collateralizations[] is an array of percentages that corresponds to the _ID array
  ...
  function stakeNFTs(uint8[] calldata _collateralizations, uint256[] calldata _IDs) external {
    /// TODO  maybe require that this contract is operator for msg.sender so there are no approve issues
    require(NFT.isApprovedForAll(msg.sender, address(this)), "set this as operator");
    require(_collateralizations.length == _IDs.length, "lengths do not match");

    uint256 priceNPT = 1;                                                                                     //FIX THIS

    for (uint i=0; i < _IDs.length; i++) {
      uint256 mintAmount = priceNPT * _collateralizations[i] * NFTinfo[_IDs[i]].price / 100;
      _stakeNFT(_IDs[i], _collateralizations[i], mintAmount);
    }
  }

  function _stakeNFT(uint256 _ID, uint8 _collateralization, uint256 _mintAmount) internal {
    if(
      // price and collaterization should be non zero, the price must not be deprecated and the staker has to be owner
      _collateralization != 0 &&
      NFTinfo[_ID].price != 0 &&
      NFTinfo[_ID].time + allowedDelay > block.timestamp &&
      NFT.ownerOf(_ID) == msg.sender
    ){
      NFTinfo[_ID].collateralization = _collateralization;
      NFT.safeTransferFrom(msg.sender, address(this), _ID);

      //mint the amount of NPT to the staker
      NPT.mint(msg.sender, _mintAmount);

      // mint the S-NFT token to the staker as proof of ownership
      claimToken.mint(msg.sender, _ID);
      emit NFTstaked(
        msg.sender, address(NFT), _ID, _mintAmount
      );
    }
  }

  function unstakeNFTs(uint256[] calldata _IDs) external {
    //get the NPT per USD value
    uint256 priceNPT = 1;                                                                             //FIX THIS
    for (uint i=0; i < _IDs.length; i++) {
      //calculate the amount of NPT to be burned
      require(claimToken.ownerOf(_IDs[i]) == msg.sender, "only the owner can unstake");
      uint256 burnAmount = priceNPT * NFTinfo[_IDs[i]].price * NFTinfo[_IDs[i]].collateralization / 100;
      _unstakeNFT(_IDs[i], burnAmount);
    }
  }

  ///
  /// @param  _ID is the  NFT to be unstaked
  ...
  ///
  function _unstakeNFT(uint256 _ID, uint256 _burnAmount) internal {
    if(
      NFTinfo[_ID].time + allowedDelay > block.timestamp &&
      //check here to prevent the whole txn from reverting in case there is an insufficient amount
      NPT.balanceOf(msg.sender) >= _burnAmount
    ) {
      NPT.burnFrom(msg.sender, _burnAmount);
      claimToken.burn(_ID);
      NFT.safeTransferFrom(address(this), msg.sender, _ID);
      delete NFTinfo[_ID];
      emit NFTunstaked(
        msg.sender, address(NFT), _ID, _burnAmount
      );
    }
  }

  ///////////  setParamsOfPool (DAO Governance connected here)  //////////////////

  function setMaxCollateralization(uint256 _maxCollateralization) external onlyOwner {
    _maxCollateralization = _maxCollateralization;
  }
  ...
  function setAllowedDelay(uint256 delayInHours) external onlyOwner {
    allowedDelay = delayInHours*3600;
  }
  ...
  function getNptPriceInUSD() public view returns(uint256){
  ...
  constructor(
    ...
    setChainlinkOracle(0xBBd7B41Cd3432ca08664923528CAEcaFb0Cc4472);
    NPT = INPT(_NPT);
    NFT = IERC721(_NFT);
    claimToken = IclaimToken(_claimToken);
    allowedDelay = _allowedDelay*3600;
  }
  ...
  function append3String(string memory a, string memory b, string memory c) internal pure returns (string memory) {
...
interface IclaimToken is IERC721 {
