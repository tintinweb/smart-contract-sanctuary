...
contract Fridge is Ownable, IERC721Receiver, Pausable {
    ...
    struct Asset {
        mapping (address => uint[]) tokensStakedByAddress;
    ...
    function putInFrontOfFridge(uint assetIndex, uint[] calldata tokenIds) external whenNotPaused {
        for(uint i = 0; i < tokenIds.length; i++) {
            ...
            fridge[assetIndex].tokensStakedByAddress[_msgSender()].push(tokenIds[i]);
        }
    }
    ...
    function removeFromFridge(uint[] calldata tokenIds, bool unstake, uint assetIndex) external whenNotPaused  {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            if(unstake) {
                ...
                for (uint j = 0; j < fridge[assetIndex].tokensStakedByAddress[_msgSender()].length; j++) {
                    if(fridge[assetIndex].tokensStakedByAddress[_msgSender()][j] == tokenIds[i]) {
                        fridge[assetIndex].tokensStakedByAddress[_msgSender()][j] = fridge[assetIndex].tokensStakedByAddress[_msgSender()][fridge[assetIndex].tokensStakedByAddress[_msgSender()].length - 1];
                        fridge[assetIndex].tokensStakedByAddress[_msgSender()].pop();
                    }
                }
            } else {
                fridge[assetIndex].assets[tokenIds[i]] = Stake({
    ...
    function calculateRewards(address wallet, uint assetIndex)
        ...
        returns (uint)
    {
        uint[] memory tokenIds = fridge[assetIndex].tokensStakedByAddress[wallet];
        uint owed = 0;
        for (uint i = 0; i < tokenIds.length; i++) {
            Stake memory stake = fridge[assetIndex].assets[tokenIds[i]];
            //Minutes for testing purposes, TODO change to days
    ...
    function tokensStaked(address wallet, uint assetIndex) public view returns (uint[] memory tokenIds) {
        return fridge[assetIndex].tokensStakedByAddress[wallet];
    }
}
