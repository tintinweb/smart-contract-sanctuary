...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function tokenURI(uint256 _tokenId)
        ...
        returns (string memory)
    {
        require(bytes(_baseURI).length != 0, "BaseURI has not been set yet");
        require(
            _exists(_tokenId),
...
contract Mushrooms is ERC721Enumerable, Ownable {
    ...
    uint256 public constant MAX_NFT_SUPPLY = 120;

    uint256 public constant reserveMushrooms = 20;

    uint256 internal constant _tokenPrice = 40000000000000000;

    uint256 internal constant _reserveIndexStart = 100;
    ...
    string public constant FinalProofHash = "";

    // Assets metadata containing all the attribute names and their IPFS hashes
    string public constant AttributesHash = "";

    // IPFS HASH containing all the token metadata in one json
    string public constant METADATA_IPFS_HASH = "";

    // Sale starts when the function "getReserves()" is called, which gives the owner 50 RANDOM Munks for the team and marketing purposes
    ...
    function _getRandomId() internal returns (uint256) {
        uint256 id = uint256(
            keccak256(
                abi.encodePacked(
                    block.timestamp + block.difficulty + _randomOffset
                )
            )
        ) % MAX_NFT_SUPPLY;
    ...
    function mintShrooms(uint256 numberOfTokens) public payable {
        ...
        require(
            _currentTokenId < _reserveIndexStart,
            "Purchase would exceed max supply of public Mushrooms"
        );
        require(
            msg.value >= _tokenPrice * numberOfTokens,
    ...
    function getReserves() public onlyOwner {
        require(
            reservesTaken != true,
            "Magic Mushrooms: Reserves already taken!"
        );

        for (uint256 i = 0; i < reserveMushrooms; i++) {
            uint256 id = _reserveIndexStart + i;
            if (totalSupply() < MAX_NFT_SUPPLY) {
