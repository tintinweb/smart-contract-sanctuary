...
contract DogParkLottery is ReentrancyGuard, ILottery, Ownable {
    ...
    modifier onlyOwnerOrInjector() {
        require((msg.sender == owner()) || (msg.sender == funderAddress), "Not owner or injector");
        _;
    }
    ...
    event LotteryTokenSet(IERC20 indexed newToken, IERC20 indexed oldToken);

     /// @notice Constructor
     /// @param _lotteryTokenAddress: address of the lottery token
    ...
    /// @notice buy into the current lottery
    /// @param _lotteryId the lotteryId`
    /// @param _numbers: array of ticket numbers between 1,000,000 and 1,999,999
    function buyTickets(uint256 _lotteryId, uint32[] calldata _numbers)
    external
    override
    notContract
    nonReentrant
    {
        ...
        require(_numbers.length <= maxNumberTicketsPerBuyOrClaim, "Too many tickets");

        require(_lotteries[_lotteryId].status == Status.Open, "Lottery is not open");
        require(block.timestamp < _lotteries[_lotteryId].endTime, "Lottery is over");

        // Calculate number of lottery to this contract
        ...
        // Transfer lotteryToken tokens to this contract
        lotteryToken.safeTransferFrom(address(msg.sender), address(this), amountToTransfer);
        ...
        emit TicketsPurchase(msg.sender, _lotteryId, _numbers.length);
    }
    ...
    function claimTickets(
        ...
        // Initializes the treasuryToTransfer
        ...
        lotteryToken.safeTransfer(msg.sender, rewardToTransfer);
    ...
    /// @param _lotteryId: lottery id
    /// @param _autoFund: re adds funds into next lottery (vs. withdrawing all)
    function drawLottery(uint256 _lotteryId, bool _autoFund)
    external
    override
    onlyOperator
    nonReentrant
    {
        ...
        ((_lotteries[_lotteryId].amountCollected) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in lottery for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                // A. No lottery to distribute, they are added to the amount to withdraw to treasury address
            } else {
        ...
        // Transfer lottery to treasury address
    ...
    /// @notice add funds to lottery
    /// @param _lotteryId: lottery id
    /// @param _amount: amount to inject in lottery token
    function addFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");
    ...
    /// @param _priceTicket: price of a ticket in current token
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(lotteryToken), "Cannot be lottery token");
     ...
     /// @notice Set lottery price ticket upper/lower limit
     /// @param _minPriceTicket: minimum price of a ticket in lottery
     /// @param _maxPriceTicket: maximum price of a ticket in lottery
    ...
    /// @notice Set new lottery token
    function setLotteryToken(IERC20 _newToken) external onlyOwner {
        IERC20 oldToken = lotteryToken;
        lotteryToken = _newToken;

        emit LotteryTokenSet(_newToken, oldToken);
    }

     /// @notice Set operator, treasury, and injector addresses
     /// @param _operatorAddress: address of the operator
     /// @param _treasuryAddress: address of the treasury
     /// @param _funderAddress: address of the injector
    function setOperatorAndTreasuryAndInjectorAddresses(
        ...
        require(_funderAddress != address(0), "Cannot be zero address");

        operatorAddress = _operatorAddress;
        treasuryAddress = _treasuryAddress;
        funderAddress = _funderAddress;

        emit NewOperatorAndTreasuryAndInjectorAddresses(_operatorAddress, _treasuryAddress, _funderAddress);
    }
...
interface ILottery {
    /// @dev buy into the current lottery
    /// @param _lotteryId the lotteryId`
    /// @param _numbers: array of ticket numbers between 1,000,000 and 1,999,999
    function buyTickets(uint256 _lotteryId, uint32[] calldata _numbers) external;

    /// @dev Claim a set of winning tickets for a lottery
    /// @param _lotteryId: lottery id
    /// @param _ticketIds: array of ticket ids
    /// @param _brackets: array of brackets for the ticket ids
    function claimTickets(
    ...
    function closeLottery(uint256 _lotteryId) external;
     ...
     /// @param _lotteryId: lottery id
     /// @param _autoFund: re adds funds into next lottery (vs. withdrawing all)
    function drawLottery(uint256 _lotteryId, bool _autoFund) external;

     /// @dev add funds to lottery
     /// @param _lotteryId: lottery id
     /// @param _amount: amount to inject in lottery token
    function addFunds(uint256 _lotteryId, uint256 _amount) external;
