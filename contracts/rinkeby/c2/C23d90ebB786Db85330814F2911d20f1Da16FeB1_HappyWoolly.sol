...
contract HappyWoolly is ERC721Burnable, Ownable {
     ...
     */
    constructor (uint256 _block, string memory baseURI) ERC721("HappyWoolly", "WOOLLY") {
    ...
    modifier isWhaleProtection(uint256 num) {
        require((whaleProtection && (block.timestamp.sub(coolDown[msg.sender]) >= 7 minutes)) || whaleProtection == false, "cooldown");
        require((whaleProtection && balanceOf(msg.sender).add(num) <= 7) || whaleProtection == false, "max 7");
        _;
    }
    ...
    */
    ...
    function changeName(uint256 tokenId, string memory newName, string memory _tag, uint256 _price) public {
        ...
        require((block.timestamp - changeNameDuration[msg.sender] > duration) || _price >= nameChangePrice, "condition not matched");
