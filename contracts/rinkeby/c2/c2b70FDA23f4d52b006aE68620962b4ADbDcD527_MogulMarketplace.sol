...
contract MogulMarketplace is ERC1155Holder, AccessControl, ReentrancyGuard {
  ...
  uint256 public feeBasisPoint; //4 decimals
   ...
   * indices: indeces of IDs
   */
  function getListingAtIndices(uint256[] memory indices)
    external
    view
   ...
   * indices: token IDs
   */
  function getCommissionInfoAtIndices(
    address NFTAddress,
    uint256[] memory indices
  ) external view returns (TokenCommissionInfo[] memory) {
    TokenCommissionInfo[] memory output =
      new TokenCommissionInfo[](indices.length);
    for (uint256 i = 0; i < indices.length; i++) {
      output[i] = commissions[NFTAddress][indices[i]];
    }
    return output;
  }
    ...
    nonReentrant()
  {
    ...
    if (listing.isStarsListing) {
      ...
      stars.safeTransferFrom(
        msg.sender,
        commissions[listing.tokenAddress][listing.tokenId].artistAddress,
        commission
      );
      ...
      commissions[listing.tokenAddress][listing.tokenId].artistAddress.transfer(
        commission
      );
  ...
  function placeBid(
    ...
    if (isStarsBid) {
      ...
      stars.safeTransfer(auction.highestBid.bidder, auction.highestBid.amount);
  ...
  function claimAuction(uint256 auctionId) external nonReentrant() {
    ...
    if (auction.isStarsAuction) {
      ...
      stars.safeTransfer(
        commissions[auction.tokenAddress][auction.tokenId].artistAddress,
        commission
      );
      ...
      auction.seller.transfer(auction.highestBid.amount - fee - commission);
      commissions[auction.tokenAddress][auction.tokenId].artistAddress.transfer(
        commission
      );
  ...
  function setfeeBasisPoint(uint256 _feeBasisPoint) external onlyAdmin {
