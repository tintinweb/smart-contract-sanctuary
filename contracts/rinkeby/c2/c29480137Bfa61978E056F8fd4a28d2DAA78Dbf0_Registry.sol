...
contract Registry {
    ...
    //A read-only function that allows anyone to check all the CIDS uploaded by a specific address
    function getAddressCids(address _owner) public view returns (string[] memory) { 
        string[] memory addressCids;
        uint256 i = 0;
        for (uint j = 0; j < cids.length; j++) {
            string memory cid = cids[j];
            if (entries[cid].owner == _owner) {
                addressCids[i] = cid;
                i++;
            }
        }
    ...
    //A read-only function that allows the owner to remove a specific CID
    function deleteRegistry(string memory _cid) public {
        require(msg.sender == entries[_cid].owner);
        delete entries[_cid];
    }
}