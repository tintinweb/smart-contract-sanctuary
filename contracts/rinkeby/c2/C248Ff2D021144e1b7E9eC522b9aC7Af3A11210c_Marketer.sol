...
contract Directory is Context, AccessControlEnumerable, ERC721Enumerable, ERC721Pausable, ReentrancyGuard {
    ...
    event UniquetteSubmitted(address indexed submitter, bytes32 indexed hash, uint256 collateral);
    event UniquetteApproved(address approver, address indexed submitter, bytes32 indexed hash, uint256 indexed tokenId);
    event UniquetteRejected(address approver, address indexed submitter, bytes32 indexed hash);
    ...
    event UniquettePutForSale(address indexed operator, address indexed seller, uint256 indexed tokenId, bytes32 hash, uint256 price);
    event UniquetteTakeOffFromSale(address indexed operator, address indexed seller, uint256 indexed tokenId, bytes32 hash);
    ...
    mapping(uint256 => bytes32) internal _idToHashMapping;
    mapping(bytes32 => Uniquette) internal _uniquettes;
    ...
    // Unique functions
    //
    function uniquetteHashById(uint256 tokenId) public view virtual returns (bytes32) {
    ...
    function uniquetteGetByHash(bytes32 hash) public view virtual returns (Uniquette memory) {
    ...
    function uniquetteSubmit(bytes32 hash, uint256 metadataVersion) public payable nonReentrant {
    ...
    function uniquetteApprove(bytes32 hash) isGovernor() public nonReentrant {
    ...
    function uniquetteReject(bytes32 hash) isGovernor() public nonReentrant {
        require(_uniquettes[hash].author != address(0), "Directory: submission not found");
        require(_uniquettes[hash].status == UniquetteStatus.PendingApproval, "Directory: submission not pending approval");
    ...
    function uniquetteForSale(uint256 tokenId, uint256 price) public virtual nonReentrant {
        ...
        bytes32 hash = _idToHashMapping[tokenId];
...
contract Vault is Common, ReentrancyGuard, AccessControl, IERC721Receiver, PaymentRecipient {
    ...
    function uniquetteLiquidate(uint256 tokenId, address payable beneficiary) public virtual nonReentrant {
        ...
        bytes32 hash = _directory.uniquetteHashById(tokenId);
