...
contract DelegateContract is Ownable, IERC721Receiver, ERC1155Receiver
{
    ...
    mapping(address => mapping(uint256 => BidDetail)) public bidDetails;
    mapping(address => mapping(uint256 => SaleItem)) public instantBuyDetails;
    mapping(address => mapping(uint256 => Bid[])) public bids;  //address of the NFT to the ID to the array of Bids
   ...
   function instantBuy(address _nftTokenAddress, TokenType _tokenType, uint256 _tokenId, bytes memory _data, uint256 _quantity) public 
   {
       ...
       require(saleItem.quantity >= _quantity, "Quantity must be less than or equal to the quantity offered for sale");
   ...
   {
       BidDetail
        memory bid = bidDetails[_nftTokenAddress][_tokenId];
       SaleItem memory saleItem = instantBuyDetails[_nftTokenAddress][_tokenId];
   ...
   function placeBid(address _nftTokenAddress, uint256 _currentBid, uint256  _tokenId, string memory _userId, uint256 _quantity) public 
   {
       BidDetail memory bidDetail = bidDetails[_nftTokenAddress][_tokenId];
       Bid[] storage bids = bids[_nftTokenAddress][_tokenId];
       address _previousBidder = bidDetail.highestBidder;
       uint256 _previousBid = bidDetail.highestBid;
       Bid memory bid = Bid(msg.sender, _currentBid, _quantity);


       require(_quantity <= bidDetail.quantity, "The quantity must be less than or equal to the total quantity offered for sale");
       require(bidDetail.isOpen == true, "This asset must be offered for sale before bids can be placed");
       //require(_currentBid > bidDetail.highestBid, "current bid must Me more than previous bid");
       
       IERC20 erc20Contract = IERC20(bidDetail.erc20TokenAddress);
    ...
    //    }
       bidDetail.highestBidder = msg.sender;
       bidDetail.highestBid = _currentBid;
       bidDetail.bidsPlaced++;
       bidDetails[_nftTokenAddress][_tokenId] =bidDetail;
       bids.push(bid);

       emit BidPlaced(bidDetail.erc20TokenAddress, _nftTokenAddress, _previousBidder, msg.sender, _previousBid, _currentBid, _tokenId, _userId);
   }
   
   function closeBid(address _nftTokenAddress, uint256 _tokenId, TokenType _tokenType, bytes memory _data) public 
   {
      BidDetail memory bid = bidDetails[_nftTokenAddress][_tokenId];
      ...
      bid = BidDetail(address(0), 0, false, 0, address(0), address(0), PricingType.Undefined, 0, 0, 0);
   ...
   function closeOrder(address _nftTokenAddress, uint256 _tokenId, TokenType _tokenType, string memory _userId) public
   {
       ...
       bidDetails[_nftTokenAddress][_tokenId] =BidDetail(address(0), 0, false, 0, address(0), address(0), PricingType.Undefined, 0, 0, 0);
