...
contract FounderBoards is ERC2981Base, ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("FounderBoards", "FNDBRD") {
    ...
    function mint(uint256 numberOfTokens) public payable {
        ...
        require(numberOfTokens <= boards.length, "That's more than we've got");
        require(msg.value >= price.mul(numberOfTokens), "Insufficient funds");
        require(
            (claimedPerWallet[msg.sender] + numberOfTokens) <= 8,
        ...
        if (presaleIsActive) {
            require(
                ...
                "You're either in or you're out, and you are out"
            );
        }

        for (uint16 i = 0; i < numberOfTokens; i++) {
            uint256 randBoard = getRandom(boards);
            _safeMint(msg.sender, randBoard);
        }
        claimedPerWallet[msg.sender] += numberOfTokens;
    }
    ...
    function getRarity(uint256 boardId) public pure returns (string memory) {
        if (boardId <= 8) return "gold";
        else if (boardId <= 96) return "silver";
        else if (boardId <= 984) return "bronze";
        else return "wood";
    }

    function hasRarity(address wallet, string memory rarity)
        public
        view
        returns (bool)
    {
        ...
        uint16 index;
        for (index = 0; index < tokenCount; index++) {
            string memory tokenRarity = getRarity(tokens[index]);
            if (
                keccak256(abi.encodePacked(tokenRarity)) ==
