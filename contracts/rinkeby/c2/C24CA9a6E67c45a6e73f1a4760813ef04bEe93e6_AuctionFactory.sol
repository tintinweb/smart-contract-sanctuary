...
contract AuctionFactory is IAuctionFactory, ERC721Holder, Ownable {
        ...
        onlyValidBidAmount(msg.value)
    {
        ...
        if (bidderCurrentBalance == 0) {
            ...
            if (auction.numberOfBids >= auction.numberOfSlots && isWinningBid(auctionId, msg.value)) {
                addBid(auctionId, msg.sender, msg.value);
        ...
        if (bidderCurrentBalance > 0) {
            ...
            if (auction.numberOfBids < auction.numberOfSlots && msg.value > auction.bidBalance[previousBidder]) {
                updateBid(auctionId, msg.sender, bidderCurrentBalance.add(msg.value));   
            }

            if (auction.numberOfBids >= auction.numberOfSlots && isWinningBid(auctionId, bidderCurrentBalance) && msg.value > auction.bidBalance[previousBidder]) {
                updateBid(auctionId, msg.sender, bidderCurrentBalance.add(msg.value)); 
                if (auction.endTime.sub(block.timestamp) < auction.resetTimer) {
                    extendAuction(auctionId);
                }
        ...
        returns (bool)
    {
        ...
        uint256 _amount = auction.bidBalance[_recipient];

        require(_amount > 0, "You have 0 deposited");
        require(!isWinningBid(_auctionId, _amount), "Cannot withdraw winning bid!");

        removeBid(_auctionId, _recipient);
    ...
    function addBid(uint256 auctionId, address bidder, uint256 bid) internal {
        ...
        emit LogBidSubmitted(bidder, auctionId, bid, auctions[auctionId].bidBalance[bidder], block.timestamp);
    }
    ...
    function updateBid(uint256 auctionId, address bidder, uint256 newValue) internal {
        require(auctions[auctionId].nextBidders[bidder] != address(0));
        address previousBidder = _findPreviousBidder(auctionId, bidder);
    ...
    function getTopBidders(uint256 auctionId, uint256 n) public view returns(address[] memory) {
    ...
    function isWinningBid(uint256 auctionId, uint256 bid) public view returns(bool) {
    ...
    function _verifyIndex(uint256 auctionId, address previousBidder, uint256 newValue, address nextBidder)
        internal
        view
        returns (bool)
    {
        return (previousBidder == GUARD || auctions[auctionId].bidBalance[previousBidder] >= newValue) &&
    ...
    function _findIndex(uint256 auctionId, uint256 newValue) internal view returns(address) {
        ...
        while (true) {
            if (_verifyIndex(auctionId, addressToInsertAfter, newValue, auctions[auctionId].nextBidders[addressToInsertAfter]))
                return addressToInsertAfter;
            addressToInsertAfter = auctions[auctionId].nextBidders[addressToInsertAfter];
        }
    }

    function _isPreviousBidder(uint256 auctionId, address bidder, address previousBidder) internal view returns(bool) {
        return auctions[auctionId].nextBidders[previousBidder] == bidder;
    }

    function _findPreviousBidder(uint256 auctionId, address bidder) internal view returns(address) {
