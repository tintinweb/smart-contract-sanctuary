...
contract CTFFarm is Ownable, ReentrancyGuard {
    ...
    uint256 constant timeToChangeRewards = 30 days;
    ...
    function updatePool(uint256 _pid) public validatePool(_pid) {
        ...
        pool.accCTFPerShare = pool.accCTFPerShare.add(ctfReward.mul(1e18).div(tokenSupply));
        pool.lastRewardBlock = block.number;
    ...
    function updateCTFPerBlock(uint256 rate) external onlyDev {
        require(rate > 0, "Zero");
        CTFPerBlock = rate;
    }
}