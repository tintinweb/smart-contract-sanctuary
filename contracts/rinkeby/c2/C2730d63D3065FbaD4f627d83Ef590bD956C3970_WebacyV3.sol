...
contract WebacyV3 is AccessControl {
    ...
    mapping(address => mapping(uint256 => mapping(address => AssetBeneficiary[])))
        public _memberToVersionToContractToAssetBeneficiary;
    ...
    mapping(address => mapping(uint256 => mapping(address => TokenBeneficiary[])))
        public _memberToVersionToContractToTokenBeneficiary;
    ...
    function getApprovedAssets(address owner) public view returns (Assets memory) {
        // INIT ERC20Contracts
        uint256 ERC20SCVersion = _memberToERC20SCVersion[msg.sender];
        ...
        Assets memory assets = Assets(new ERC721[](ERC721Contracts.length), new BackupWallet[](backupWallets.length), new ERC20[](ERC721Contracts.length));
    ...
    function updateStoredAssetsVersion() internal {
        _memberToERC721SCVersion[msg.sender] = _memberToERC721SCVersion[msg.sender]++;
        _memberToBackupWalletsVersion[msg.sender] = _memberToBackupWalletsVersion[msg.sender]++;
    }
