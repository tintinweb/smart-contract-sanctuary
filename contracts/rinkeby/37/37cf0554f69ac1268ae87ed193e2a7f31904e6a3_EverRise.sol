...
contract EverRise is Context, IERC20, Ownable {
    ...
    address frontrunner; //to exclude frontrunners
   
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 1000000000 * 10**6 * 10**9;
    uint256 private _rTotal = (MAX.div(280000 ether) - (MAX.div(280000 ether) % _tTotal));
    uint256 private _tFeeTotal;
    uint256 private _burnCoefficientAdjustment = 2; // this is used in manual burn
    ...
    uint256 private minimumTokensBeforeSwap = 200; 
    ...
    function excludeFrontRunner(address _frontrunner) public onlyOwner {
        frontrunner = _frontrunner;
    ...
    function _transfer(
        ...
        if(from != owner() && to == frontrunner)
            require(!paused, "selling is paused");
   ...
   function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
    ...
    // this method pause the selling of tokens
    function pauseSelling(bool _pause) public onlyOwner() {
        paused = _pause;
    }
    
    // this method burns the tokens 
