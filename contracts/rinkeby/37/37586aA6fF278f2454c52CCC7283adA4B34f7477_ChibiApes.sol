...
contract ChibiApes is ERC721, Ownable {
    ...
    mapping(address => uint) preMints; // Maps address to how many they can premint

    event Mint(address indexed owner, uint indexed tokenId);

    constructor() ERC721("Chibi Apes", "CHIBIAPE") {
        transferOwnership(0x8a04f9C3D4de6A118670FA9c5dD078F1ecF4E61b);
    }
    ...
    function publicMint() external payable {
        ...
        require(mintCount + amount <= maxItems, "publicMint: Surpasses cap");
        _mintWithoutValidation(msg.sender, amount);
    }

    function _mintWithoutValidation(address to, uint amount) internal {
        require(mintCount + amount <= maxItems, "mintWithoutValidation: Sold out");
        for (uint i = 0; i < amount; i++) {
            _mint(to, mintCount);
            mintCount += 1;
            emit Mint(to, mintCount);
        }
    }
