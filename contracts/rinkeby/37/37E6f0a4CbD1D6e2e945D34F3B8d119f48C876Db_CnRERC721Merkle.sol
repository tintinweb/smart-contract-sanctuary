...
contract CnRERC721Merkle is ERC721, Ownable {
    ...
    uint256 public constant MAX_MINT = 2001;
    uint256 public PRICE = 0.75 ether;
    uint256 public MAX_RESERVE = 48;
    ...
    constructor(bytes32 initialRoot) ERC721("BlaBla", "Bla") {
    ...
    function onWhiteList(address addr,bytes32[] calldata _merkleProof) external view returns (bool) {
    ...
    function buyNFT(bytes32[] calldata _merkleProof) external payable {
        ...
        // Check if totalSupply was reached
        require(totalSupply() < MAX_MINT, "All tokens have been minted");

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));

        require(
            isWhiteListActive ? MerkleProof.verify(_merkleProof,merkleRoot,leaf) : true,
            "You are not on the White List"
        );
        // We should check the value is more than 0, if not the other condition will fail
        require(
            msg.value > 0 && msg.value % PRICE == 0,
            "Amount must be a multiple of price"
        );
        // Calculating amount based on sent value and price
        uint256 amount = msg.value / PRICE;
        require(
            amount >= 1 && amount <= purchaseLimit,
            "Amount should be at least 1"
        );
        // Don't need to check if _claimed[msg.sender] exists, because it will always return 0 as non-setted value, removing addition to save gas
        // We can't check on a simple +1, because at first time you will be able more than 1 if understand how the contract works
        require(
            (_claimed[msg.sender] + amount) <= purchaseLimit,
            "Purchase exceeds purchase limit"
        );
        // Check if adding specified amount we don't reach the max public supply
        uint256 reached = amount + _tokenIdCounter.current();
        require(
            reached <= (MAX_MINT - MAX_RESERVE),
            "Purchase would exceed public supply"
        );
        // Claimed counter should me dynamic
        _claimed[msg.sender] += amount;
        // Public supply counter should me dynamic
        totalPublicSupply += amount;
        // Creating a loop to mint the dynamic amount of nfts
        for (uint256 i = 0; i < amount; i++) {
            _tokenIdCounter.increment();
            uint256 newTokenId = _tokenIdCounter.current();
            // Minting NFT to sender
    ...
    function gift(address to) external onlyOwner {
        require(totalSupply() < MAX_MINT, "All tokens have been minted");
        // Need to check if reserve is not reached
        require(_gifted.length < MAX_RESERVE, "Max reserve reached");
        // Minting NFT to provided address
        _tokenIdCounter.increment();
        uint256 newTokenId = _tokenIdCounter.current();
    ...
    function setPurchaseLimit(uint256 newLimit) external onlyOwner {
        require(
            // Purchase limit must be greater than zero, not lower
    ...
    function setMaxReserve(uint256 newReserve) external onlyOwner {
        require(
            newReserve < MAX_RESERVE,
            "New reserve must be less than old reserve"
        );
    ...
    function redeem(uint256 _tokenId) external returns (bool) {
        ...
        // Redeeming must be a mapping, because just one address should be able to redeem
        require(_redeemed[_tokenId] == address(0), "You redeemed the NFT yet");
