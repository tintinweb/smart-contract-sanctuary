...
contract Souvenir is Ownable, ERC721, IRaribleRoyaltiesV2 {
    ...
    uint256 public lastTokenId = 999;
    ...
    modifier maxMintCheck() {
        require(addressToMintCount[_msgSender()].add(1) <= maxPerAddress, "Max mint limit reached.");
        _;
    }
    ...
    constructor(address _royaltyContractAddress) ERC721("Brendan Fernandes Souvenir", "SOUVENIR") {
        ...
        // Defaults
        contractUri = 'https://gateway.pinata.cloud/ipfs/QmNwSWX2yjuWvWATAFjxwuasCSPfyioB7PiSLQWZfQF6gi'; // @TODO(iolson): Update for Production
        metadataBaseUri = 'https://gateway.pinata.cloud/ipfs/QmRiDW8KevFE119gXq6VJfxxefc6g3wtxLBaGyki9eio42/'; // @TODO(iolson): Update Metadata Base URI

        artistPayoutAddress = address(0xC16f1eDAbfAEa8981F2Ba8D61401f7a76B1fDa50); // @todo(iolson): Update this.
        imnortArtPayoutAddress = address(0x12b66baFc99D351f7e24874B3e52B1889641D3f3);
    ...
    function mint() public payable onlyActive onlyNonPaused maxMintCheck returns (uint256) {
        ...
        require(nextTokenId <= lastTokenId, "Must not exceed max invocations.");
        require(msg.value >= mintPriceInWei, "Must send minimum value.");

        uint256 tokenId = nextTokenId;
        _mint(_msgSender(), tokenId);

        // Keep track of how many mints a given address has done.
        addressToMintCount[_msgSender()] = addressToMintCount[_msgSender()].add(1);
    ...
    // @author Ian Olson
    function updateLastTokenId(uint256 newMaxInvocations) public onlyAdmin {
        require(!active, "Cannot change max invocations after active.");
        lastTokenId = newMaxInvocations;
    }
