...
contract PrimaDonna is IERC721, Ownable, Functional {
    ...
    constructor() {
        _name = "PrimaDonnas";
        _symbol = "PD";
        _baseURI = "https://primadonnanft.art/metadata/";
        _hideTokens = true;
        
        _totalTokens = 10001;  //+1 to set up for numbering from 1-10,000 instead of 0-9999
        _numberMinted = 1;     //  Placeholder for the program, will show 0 from outside.
        
        price = 50 * (10 ** 15); // Replace leading value with price in finney
        maxPerTxn = 5;
        whiteListMax = 1;
        maxPerWallet = 20;
        reservedTokens = 1000; // uncomment to reserve tokens for the owner or other purposes
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return  interfaceId == type(IERC721).interfaceId ||
                ...
                interfaceId == PrimaDonna.onERC721Received.selector;
    }
    ...
    function withdraw() external onlyOwner {
        ...
        address founder = payable(0xfD27Db9B7e9598F542aA4Bc39AF413D89a02A01d);
        address dev =     payable(0x2496286BDB820d40C402802F828ae265b244188A);
        
        bool success;
        (success, ) = founder.call{value: ((sendAmount * 90)/100)}("");
        require(success, "Transaction Unsuccessful");
        
        (success, ) = dev.call{value: ((sendAmount * 10)/100)}("");
        require(success, "Transaction Unsuccessful");
    }
    
    function ownerMint(address _to, uint256 qty) external onlyOwner {
        require((_numberMinted + qty) > _numberMinted, "Math overflow error");
        require((_numberMinted + qty) < _totalTokens, "Cannot fill order");
        
        uint256 mintSeedValue = _numberMinted; //Store the starting value of the mint batch
        ...
        for(uint256 i = 0; i < qty; i++) {
            ...
            _numberMinted ++;  //reservedTokens can be reset, _numberMinted can not
        }
    }
    ...
    function airDrop(address[] memory _to) external onlyOwner {
        ...
        uint256 mintSeedValue = _numberMinted;
        if (reservedTokens >= qty) {
            reservedTokens -= qty;
        } else {
            reservedTokens = 0;
    ...
    function mint(uint256 qty) external payable reentryLock {
        ...
        require(qty <= maxPerTxn, "Mint: Above Transaction Threshold!");
        require((qty + reservedTokens + _numberMinted) < _totalTokens, "Mint: Not enough avaialability");
        require((_mintTracker[_msgSender()] + qty) <= maxPerWallet, "Mint: Max tokens per wallet exceeded");
        
        // extends minting availability for approved wallets during pre-sale
        if (!mintActive) {
            require((qty + _numberMinted) < reservedTokens, "Mint: Not enough reserved"); //reserved tokens in this case is for pre-mint
            require((_mintTracker[_msgSender()] + qty) <= whiteListMax, "Mint: Cannot mint more at this time");
    ...
    function setWhiteListMax(uint256 maxValue) external onlyOwner {
        whiteListMax = maxValue;
    }
    ...
    function totalSupply() external view returns (uint256) {
        return (_numberMinted - 1); //stupid bs for etherscan's call
    }
