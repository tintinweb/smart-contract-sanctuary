...
contract Cyborgs is ERC721Enumerable, Pausable {
    struct Trait {
        uint256 index_1;
        uint256 index_2;
    }
    ...
    constructor(address payable _addresssContract) ERC721("Cyborgs", "CBC") {
        require(
            _addresssContract != address(0),
            "Cyborgs Billionaire Club: address contract is Zero address"
        );
        billionaireAddress = payable(_addresssContract);
        billionaireContract = IERC721(billionaireAddress);
    }
    ...
    function mergeNFT(
        ...
        require(
            ...
            "Cyborgs Billionaire Club: Caller is not the owner of tokens"
        );
        require(
            ismerged[_token_1] == false,
            "Cyborgs Billionaire Club: Token is already merged"
        );
    ...
    function burn(uint256 _id) external {
        require(isBurnEnabled, "Cyborgs Billionaire Club: burning disabled");
        require(
            _isApprovedOrOwner(msg.sender, _id),
            "Cyborgs Billionaire Club: burn caller is not owner nor approved"
        );
        _burn(_id);
    }
}
