...
contract RoyaltyDistributor is Ownable {
    ...
    function accumulated(address token, uint256 tokenId)
        public
        view
        returns (uint256)
    {
        return
            (totalReceived(token) / TOTAL_SHARES) -
            totalClaimedOf[tokenId][token];
    }

    function claim(address token, uint256[] memory tokenIds)
        external
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(
                IERC721(nft).ownerOf(tokenId) == _msgSender(),
        ...
        require(totalClaimQty != 0, "No accumulated rewards");
        totalReleased[token] += totalClaimQty;

        if (token == address(0)) {
            _sendEth(msg.sender, totalClaimQty);
        } else {
            _sendErc20(token, msg.sender, totalClaimQty);
        }
