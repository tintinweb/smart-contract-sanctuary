...
import "./RainbowRewardInterface.sol";
...
contract Rainbow_RB_Protocol {
    ...
    struct TokenPool {
        ...
        uint256 tokenPoolAssetsTotal;
        uint256 compensateTokenTotal;
        uint256 insuredCountTotal;
        uint256 insureFeesTotal;

        uint256 tokenAmountCurrent;
        uint256 tokenPoolAssetsCurrent;
        uint256 insuredCountCurrent; 
        uint256 insureFeesCurrent;
    ...
    */
    ...
    function deposit(
        ...
        uint256 licenseFees = 0;
        {
            ...
            if(payUsdts > _payUsdts){
                ...
                RainbowReward(rainbowRewardAddress).increaseIncome(_payUsdts);
...

        {
                ...
                pool.tokenAmountTotal = pool.tokenAmountTotal.add(_tokenAmount);
                pool.tokenPoolAssetsTotal = pool.tokenPoolAssetsTotal.add(totalAssets);
                pool.insuredCountTotal = pool.insuredCountTotal.add(1);
                pool.insureFeesTotal = pool.insureFeesTotal.add(licenseFees);

                pool.tokenAmountCurrent = pool.tokenAmountCurrent.add(_tokenAmount);
                pool.tokenPoolAssetsCurrent = pool.tokenPoolAssetsCurrent.add(totalAssets);
                pool.insuredCountCurrent = pool.insuredCountCurrent.add(1);
                pool.insureFeesCurrent = pool.insureFeesCurrent.add(licenseFees);
    ...
    function distributionTokenPool(uint256 _tokenID) private{
        tokenPools[_tokenID] = TokenPool({
            tokenID: _tokenID,
            tokenAmountTotal: 0,
            tokenPoolAssetsTotal: 0,
            compensateTokenTotal: 0,
            insuredCountTotal: 0,
            insureFeesTotal: 0,
            tokenAmountCurrent: 0,
            tokenPoolAssetsCurrent: 0,
            insuredCountCurrent: 0,
            insureFeesCurrent: 0
        });
    }
    ...
    function withdrawAssets(uint256 _tokenType) external{
        ...
        if(settleAmount == 0){
            ...
            pool.insuredCountCurrent = pool.insuredCountCurrent.sub(1);
            pool.tokenAmountCurrent = pool.tokenAmountCurrent.sub(user.tokenAmount);
            pool.tokenPoolAssetsCurrent = pool.tokenPoolAssetsCurrent.sub(user.userTotalAssets);
            pool.insureFeesCurrent = pool.insureFeesCurrent.sub(user.insureFees);
            user.tokenAmount = 0;
    ...
    // Get token pool Info
    function getTokenPoolInfo(uint256 _tokenType) public view returns (uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256)
    {
        TokenPool storage pool = tokenPools[_tokenType];
        return (pool.tokenAmountTotal,
                pool.tokenPoolAssetsTotal,
                pool.compensateTokenTotal,
                pool.insuredCountTotal,
                pool.insureFeesTotal,
                pool.tokenAmountCurrent,
                pool.tokenPoolAssetsCurrent,
                pool.insuredCountCurrent,
                pool.insureFeesCurrent
                );
    }
    ...
    function getPlatformInfo_Current() public view returns (uint256,uint256,uint256){
        ...
        for(uint i = 0;i<(dataStatistics.tokenTotal);i++){
            ...
            if(pool.tokenAmountCurrent >0){
                totalAssets = pool.tokenPoolAssetsCurrent.add(totalAssets);
            }
        }
