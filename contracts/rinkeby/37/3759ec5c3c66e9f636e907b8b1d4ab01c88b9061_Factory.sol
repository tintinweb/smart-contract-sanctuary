...
contract ERC20 is SafeMath{
    ...
    function mint(uint256 amount) {
        balanceOf[tx.origin] += amount;              // Give the creator all initial tokens
...
contract Factory {
    ...
    function tap() public {
        for(uint8 i = 0; i < tokens.length; i += 1) {
            ...
            ERC20(tokens[i]).mint(100 * (10 ** decimals));
        }
    }

    function create(string tokenName, string tokenSymbol, uint8 decimalUnits) {
        ERC20 t = new ERC20(
            100 * (10**decimalUnits),
            tokenName,
            decimalUnits,
