 ...
 */
contract BuyBackAndBurnV1 is MyPausableUpgradeable{
    ...
    function setBurnToken(IERC20 token) external {         
        require(hasRole(MANAGE_CONTRACTS_ROLE, _msgSender()), "BuyBackAndBurnV1: must have MANAGE_CONTRACTS_ROLE role to execute this function");                           
     ...
     */
    function buyBackAndBurnERC20(IERC20 collectedToken) external whenNotPaused {
        // check input parameter
        require(address(collectedToken) != address(0), "BuyBackAndBurnV1: invalid token address");

        // check how much of the given token is ready for burn
        uint256 amount = collectedERC20ToBurn[address(collectedToken)];
        require(amount > 0, "BuyBackAndBurnV1: no tokens to burn");

        // swap collected tokens to burn tokens using external DEX
        uint256 burnAmount = router.tradeERC20(collectedToken, burnToken, amount);

        // burn the swapped tokens
        ERC20Burnable(address(burnToken)).burn(burnAmount);

        // update records - set collected tokens to 0
        collectedERC20ToBurn[address(collectedToken)] = 0;
     ...
     * @dev emits event BoughtBackAndBurned()
     */
    function buyBackAndBurnNativeToken() external whenNotPaused {
        ...
        uint256 burnAmount = router.tradeNativeTokenForERC20(burnToken, amount);
     ...
     * @dev emits event TokensAdded
     */
    function depositNativeToken(uint256 amount) external payable whenNotPaused {
        //check input parameter
        require(amount > 0, "BuyBackAndBurnV1: amount cannot be 0");
...
interface IRouter {
    function tradeERC20(IERC20 collectedToken, IERC20 burnToken, uint256 amount) external returns (uint256 tokensReceived);
    function tradeNativeTokenForERC20(IERC20 output, uint256 inputAmount) external returns (uint256);
