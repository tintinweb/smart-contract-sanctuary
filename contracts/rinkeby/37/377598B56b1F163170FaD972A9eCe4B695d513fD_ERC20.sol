...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    uint256 public marketingFeeToSell = 100 * 10**18;
     ...
     */
    function updateLiquidityFeeToSell(uint256 _liquidityFeeToSell) public onlyOwner {
     ...
     */
    function updateMarketingFeeToSell(uint256 _marketingFeeToSell) public onlyOwner {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(enableMaxTransactionAmount && recipient == uniswapV2Pair) require(amount.div(10**18) <= calculateMaxTransactionAmount(), "Sell is above transaction limit");
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) internal {
        ...
        if(takeFee && recipient == uniswapV2Pair) {
            takeLiqFee = amount.div(10**18) >= calculateTradeLimitForLiqFee();
    ...
    function calculateMaxTransactionAmount() internal view returns(uint256) {
        ...
        return reserve0.mul(transactionLimit).div(10**2).div(10**18);
    }

    function calculateTradeLimitForLiqFee() internal view returns(uint256) {
        (uint256 reserve0,,) = IUniswapV2Pair(uniswapV2Pair).getReserves();
        return reserve0.mul(tradeLimitForLiqFee).div(10**2).div(10**18);
    }
