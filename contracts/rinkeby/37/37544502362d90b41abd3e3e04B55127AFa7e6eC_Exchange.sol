...
contract Exchange is Ownable, Pausable {
    ...
    function buyWithWei(uint256 _amountTokens) external payable whenNotPaused {
        ...
        require(
            _amountTokens <= balToken,
            "contract doesn't have enough balance"
        );

        //how much would it pay for them
        uint256 requiredAmount = (priceForTokenInWei * _amountTokens) /
            (10**18);
        require(
            requiredAmount == msg.value,
            "you must send the exact ETH amount"
        );
    ...
    function buyWithUSD(uint256 _amountTokens) external whenNotPaused {
        ...
        //how much would it pay for them
        uint256 usdRequiredAmount = (priceForTokenInUSD * _amountTokens) /
            (10**18);
    ...
    function setPriceForTokenInWei(uint256 _newPriceWei) public {
        require(msg.sender == priceOracle, "only oracle can update");
        priceForTokenInWei = _newPriceWei;
    }

    //sets the price of 1 token in USD (with decimals). eg: 1 SPI =  $200 => _newPriceUSD = 200*10**6
    function setPriceForTokenInUSD(uint256 _newPriceUSD) public {
        require(msg.sender == priceOracle, "only oracle can update");
