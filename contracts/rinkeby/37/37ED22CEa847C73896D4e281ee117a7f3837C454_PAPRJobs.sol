...
interface IPapr {
function getPaprPrice() external view returns(uint256);
...
contract Escrow is Migratable {
    ...
    address constant private developper = address(0xB8830331Ec4e3840F89Ca51A3Cc7c29312d6F3d1);
    ...
    event OnCreateEscrow(address indexed dapp, address indexed client, address indexed provider, uint256 amount, uint256 paprLPBalance, uint256 priceperfullShare);
    ...
    isInitializer("Escrow", "0.1.0") {
        ...
        if (lpToken0 != address(escrowToken)) {
             IUniswapRouterETH(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(wbnbHalf, 0, escrowTokentoPAPRroute, address(this), now);
        }

        if (lpToken1 != address(escrowToken)) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(wbnbHalf, 0, escrowTokentoBUSDroute, address(this), now);
        }
    ...
    returns (uint256) {
        ...
        uint256 pricePerFullShare = IPAPRBUSDVault(vault).getPricePerFullShare();
        
        uint256 id = ++escrowId;
        EscrowRecord storage escrow = escrows[id];
        escrow.id = id;
        escrow.LPBal = paprLPBalance;
        escrow.priceperfullshare = pricePerFullShare;
        ...
        emit OnCreateEscrow(escrowDapp, _client, _provider, _amount, paprLPBalance, pricePerFullShare);
    ...
    returns (bool) {
        ...
        uint256 payToDevAmount = escrows[_escrowId].payoutAmount.mul(10).div(1000);
        uint256 payToDappAmount = escrows[_escrowId].payoutAmount.mul(10).div(1000);
        if(payToDappAmount > 0){
            escrows[_escrowId].paidToDappAmount = payToDappAmount;
            //bnb/2 ==> buyback paprgov + burn
            IERC20(paprgov).transfer(escrowDapp, payToDappAmount);
            IERC20(paprgov).transfer(developper, payToDevAmount);
        }

        uint256 totalFees = payToDappAmount.add(payToDevAmount);
        uint256 providerPayoutCan = escrows[_escrowId].payoutAmount.sub(totalFees);
        escrows[_escrowId].paidToProviderAmount = providerPayoutCan;

        IERC20(paprgov).transfer(escrows[_escrowId].provider, escrows[_escrowId].paidToProviderAmount);
    ...
    //     escrows[_escrowId].status = EscrowStatus.Completed; 
    //     //escrows[_escrowId].closedAt = block.number;
    //     uint256 initialAmount = escrows[_escrowId].amount.div(IPapr(paprgov).getPaprPrice());
    //     uint256 totalProfits = getTotalPayoutPAPR(_escrowId).sub(initialAmount);
    //     escrows[_escrowId].payoutAmount = getTotalPayoutPAPR(_escrowId);

    //     uint256 payToDappAmount = escrows[_escrowId].payoutAmount.mul(20).div(1000);
    //     uint256 payToDevAmount = escrows[_escrowId].payoutAmount.mul(10).div(1000);
    //     uint256 totalFees = payToDevAmount.add(payToDappAmount);

    //     if(totalProfits > 0){
    //         escrows[_escrowId].paidToDappAmount = totalFees;
    //         //bnb/2 ==> buyback paprgov + burn
    //         IERC20(paprgov).transfer(escrowDapp, payToDappAmount);
    //         IERC20(paprgov).transfer(developper, payToDevAmount);
    //     }
    ...
    function returnCompute(uint _escrowId) public view returns (uint256) {
        uint256 toWithdraw = escrows[_escrowId].priceperfullshare;
        return(toWithdraw);
        // uint256 LPdeposited = escrows[_escrowId].LPBal;
        // uint256 computeWithdraw = LPdeposited.mul(10 ** 18).div(toWithdraw);
    }

    // function tryBreak(uint _escrowId) public {
    // uint256 toWithdraw = escrows[_escrowId].priceperfullshare;
    // uint256 LPdeposited = escrows[_escrowId].LPBal;
    // uint256 computeWithdraw = LPdeposited.mul(10 ** 18).div(toWithdraw);
    // IPAPRBUSDVault(vault).withdraw(computeWithdraw);
    // IZapper(Zapper).breakLP(want, IERC20(want).balanceOf(address(this)));
    // }
    ...
    returns (uint256) {
        ...
        escrows[_escrowId].paprOut = ERC20(paprgov).balanceOf(address(this));
        uint256 totalPayoutPAPR = escrows[_escrowId].paprOut;
    ...
    returns (bool) {
        ...
        IERC20(paprgov).transfer(escrows[_escrowId].client, escrows[_escrowId].paidToClientAmount);
    ...
    returns (bool) {
        ...
        uint256 payToDappAmount = escrows[_escrowId].payoutAmount.mul(9).div(1000);
        uint256 payToDevAmount = escrows[_escrowId].payoutAmount.mul(1).div(1000);
        if (payToDappAmount > 0){
            escrows[_escrowId].paidToDappAmount = payToDappAmount;
            IERC20(paprgov).transfer(developper, payToDevAmount);
            IERC20(paprgov).transfer(escrowDapp, payToDappAmount);
        ...
        if (_payToArbiterPercentage > 0) {
            ...
            IERC20(paprgov).transfer(_arbiter, escrows[_escrowId].paidToArbiterAmount);
        }                
     ...
     //uint256 closedAt) 
      {      
        ...
        return (
            ...
            escrows[_escrowId].paprOut,
            escrows[_escrowId].LPBal,
            escrows[_escrowId].priceperfullshare,
