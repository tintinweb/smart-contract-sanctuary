...
/*
interface IJag {
    function jagRandomizer(uint256 jagID, uint256 buckID) external returns(uint256);
    function jaguarTokenURI(uint256 jagID) external view returns(string memory);
}

interface IBuck {
    function buckRandomizer(uint256 buckID, uint256 treeID) external returns(uint256); 
...
contract Ecozsystem is Ecoz {
    ...
    function totalSupply() public view returns(uint256) {
        return jagSupply + buckSupply + treeSupply;
    }
