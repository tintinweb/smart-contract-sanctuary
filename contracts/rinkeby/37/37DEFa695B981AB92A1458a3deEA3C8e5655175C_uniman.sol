...
contract uniman is ERC721Enumerable, Ownable {
    ...
    // Give away
    uint256 private initialGiveawayAmount;
    ...
    constructor(string memory baseURI, address[] memory giveAwayAddresses)
        ERC721("uniman", "uniman")
    {
        setBaseURI(baseURI);
        ...
        // Mint to awesome uniman supporting humans who helped us early
        initialGiveawayAmount = giveAwayAddresses.length;
        mintTokenToAddresses(giveAwayAddresses);
    }
    ...
    function claimPresale(uint256 numberOfTokens) external payable onlyPresale {
        ...
        require(
            totalSupply() + numberOfTokens <=
                PRESALE_UNIMAN + PRIVATE_UNIMAN + initialGiveawayAmount,
            PRESALE_SOLD_OUT
        );
    ...
    function mintTokenToAddresses(address[] memory addresses) public onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
