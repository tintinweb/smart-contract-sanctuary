...
contract HailstoneTestnet is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function getJsonImage(uint _tokenId) internal pure returns (string memory) {
        return string(abi.encodePacked(',"image": "data:image/svg+xml;base64,', Base64.encode(bytes(HailImageTestnet.createSvgImage(_tokenId))), '"'));
    }
    ...
    function contractURI() public view returns (string memory output) {
        string memory json = Base64.encode(bytes(string(abi.encodePacked(
            '{'
                '"name": "HAILSTONE", '
                '"description": "HAILSTONE TESTNET is a work in solidity celebrating the Collatz conjecture.", '
