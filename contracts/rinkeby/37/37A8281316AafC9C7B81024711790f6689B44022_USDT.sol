...
contract USDT is owned {
    ...
    uint256 private _totalSupply = 200000000 * (10**_decimals);         //200 million tokens
    uint256 constant public maxSupply = 21000000  * (10**_decimals);    //210 million tokens
    address public companyaddress = address(0);
    ...
    function changeCompany(address _companyaddress) onlyOwner public returns (bool success) {
        require(!safeguard);
 ...
 /*************************************/
    /*  Section for User blacklisting    */
    /*************************************/
    bool public blacklistingStatus;
    mapping (address => bool) public blacklisted;


    function changeblacklistingStatus() onlyOwner public{
        if (blacklistingStatus == false){
            blacklistingStatus = true;
        }
        else{
           blacklistingStatus = false;
        }
    }

    /**
     * Blacklist any user address - only Owner can do this
     *
     * It will add user address in whitelisted mapping
     */
    function blacklistUser(address userAddress) onlyOwner public{
        require(blacklistingStatus == false);
        require(userAddress != address(0));
        blacklisted[userAddress] = true;
    }

    /**
     * Blacklist Many user address at once - only Owner can do this
     * It will require maximum of 150 addresses to prevent block gas limit max-out and DoS attack
     * It will add user address in whitelisted mapping
     */
    function blacklistManyUsers(address[] memory userAddresses) onlyOwner public{
        require(blacklistingStatus == false);
        uint256 addressCount = userAddresses.length;
        require(addressCount <= 150,"Too many addresses");
        for(uint256 i = 0; i < addressCount; i++){
            blacklisted[userAddresses[i]] = true;
        }
    }


    function whitelistUser(address userAddress) onlyOwner public{
        require(blacklistingStatus == true);
        require(userAddress != address(0));
        blacklisted[userAddress] = false;
    }
    ...
    function whitelistManyUsers(address[] memory userAddresses) onlyOwner public{
        ...
        require(addressCount <= 150,"Too many addresses");
        for(uint256 i = 0; i < addressCount; i++){
            blacklisted[userAddresses[i]] = false;
        }
    }
