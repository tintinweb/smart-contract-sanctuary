...
contract TokenVesting is Context,Ownable{
    ...
    // uint256 public awardLowAmount = 5000e4;
    // uint256 public awardMiddleAmount = 10000e4;
    // uint256 public awardHighAmount = 20000e4;
    ...
    // @notice rate of release 0.5%/1Days
    uint256 public releaseRate = 0.001e18;
    ...
    function lock(uint256 _amountIn) public returns (bool){
      ...
      totalLock = totalLock.add(_amountIn);
    ...
    function withdrawal() public returns(uint256){
      ...
      upDataRelease(false);
      uint amountOut = tokenVestUsers[msg.sender]._waitingWithdrawalAmount.add(tokenVestUsers[msg.sender]._awardAmount);
      ...
      totalLock = totalLock.sub(tokenVestUsers[msg.sender]._waitingWithdrawalAmount);
    ...
    function upDataRelease(bool isUpDataCreateTimestamp) private returns(uint256,uint256) {
      ...
      if(releaseDay >= 1 && tokenVestUsers[msg.sender]._lockAmount != 0){
        indexReleaseDeltaunlockReleaseAmount = FixedPoint.multiplyUintByMantissa(tokenVestUsers[msg.sender]._lockAmount,releaseDay.mul(releaseRate));
      ...
      if(newAwardSeconds >= awardCycle && tokenVestUsers[msg.sender]._lockAmount != 0){
        if(tokenVestUsers[msg.sender]._lockAmount >= awardHighAmount){
          indexReleaseDeltaunlockAwardAmount =  FixedPoint.multiplyUintByMantissa(tokenVestUsers[msg.sender]._lockAmount,awardHighRate);
        }else if(tokenVestUsers[msg.sender]._lockAmount >= awardMiddleAmount){
          indexReleaseDeltaunlockAwardAmount =  FixedPoint.multiplyUintByMantissa(tokenVestUsers[msg.sender]._lockAmount,awardMiddleRate);
        }else if(tokenVestUsers[msg.sender]._lockAmount >= awardLowAmount){
          indexReleaseDeltaunlockAwardAmount =  FixedPoint.multiplyUintByMantissa(tokenVestUsers[msg.sender]._lockAmount,awardLowRate);
        }
