...
contract WhiteRareAuctions is IAuctionHouse, ReentrancyGuard {
   ...
   function getAuctionCurrentNumber() public view returns (uint256) {
    ...
    function fixedTrashBuy (uint256 auctionId, uint256 amount) 
      internal  
    {
        _handleFixedSale(msg.sender, amount, auctions[auctionId].auctionCurrency);
        _cancelAuction(auctionId);
    ...
    function FixedBulkBuy(uint256[] calldata _fixedAuc, uint256 amount) external payable {
        uint256 price = amount.div(_fixedAuc.length);
        for(uint256 i = 0; i < _fixedAuc.length; i++){
            uint256 auctionId = _fixedAuc[i];
            fixedTrashBuy(auctionId, price);
        }
    }
