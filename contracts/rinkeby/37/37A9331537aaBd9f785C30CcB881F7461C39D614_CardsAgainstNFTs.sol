...
contract CardsAgainstNFTs is ERC721, ERC721URIStorage, Ownable {
        ...
        returns (string memory)
    {
        ...
        uint maxLines = 7;
        uint startOfLine = 0;
        uint verticalSpacing = 14;
        while (maxLines > 0) {
            for (uint endOfLine = startOfLine + maxWidth; endOfLine >= startOfLine; endOfLine--) {
                if (endOfLine >= input.length) {
                    continue;
                }
                if (input[endOfLine] == ' ' || endOfLine == input.length - 1) {
                    // sol to i is the latest line
                    string memory current = sliceStringBytes(startOfLine, endOfLine + 1, input, 20 + (7 - maxLines) * verticalSpacing);
                    output = string(abi.encodePacked(output, current));
                    startOfLine = endOfLine + 1; 
                    break;
                }
            }
            if (startOfLine >= input.length) {
                break;
            }
            maxLines -= 1;
        }
