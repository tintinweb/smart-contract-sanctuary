...
contract House is IHouse, ERC721Enumerable, Ownable, Pausable {
    ...
    event ShackStolen(address indexed owner,uint256 indexed tokenId, uint256 blockNum, uint256 timeStamp);
    event RanchStolen(address indexed owner,uint256 indexed tokenId, uint256 blockNum, uint256 timeStamp);
    event MansionStolen(address indexed owner,uint256 indexed tokenId, uint256 blockNum, uint256 timeStamp);
    ...
    function mint(address recipient, uint256 seed) external override whenNotPaused {
        ...
        if(tx.origin != recipient && recipient != address(habitat)) {
            ...
            if(tokenTraits[minted].roll == 0) {
                emit ShackStolen(recipient, minted, block.number, block.timestamp);
            } else if(tokenTraits[minted].roll == 1) {
                emit RanchStolen(recipient, minted, block.number, block.timestamp);
            } else {
                emit MansionStolen(recipient, minted, block.number, block.timestamp);
            }
        }
