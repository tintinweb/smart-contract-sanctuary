...
import "../../manifold/creator-core/core/ERC721CreatorCore.sol";

/**
 * @dev ERC721Creator implementation
 */
contract ERC721CreatorImplementation is
    ...
    ERC721CreatorCore
{
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721Upgradeable, ERC721CreatorCore, AdminControlUpgradeable)
        returns (bool)
    {
        return
...
abstract contract ERC721CreatorCore is CreatorCore, IERC721CreatorCore {
        ...
        internal
    {
        ...
        require(
            permissions == address(0x0) ||
                ERC165Checker.supportsInterface(
                    permissions,
                    type(IERC721CreatorMintPermissions).interfaceId
                ),
            "Invalid address"
        );
    ...
    function _checkMintPermissions(address to, uint256 tokenId) internal {
        if (_extensionPermissions[msg.sender] != address(0x0)) {
            IERC721CreatorMintPermissions(_extensionPermissions[msg.sender])
                .approveMint(msg.sender, to, tokenId);
        }
    }
    ...
    function _postBurn(address owner, uint256 tokenId) internal virtual {
        ...
        if (_tokensExtension[tokenId] != address(this)) {
            if (
                ...
                IERC721CreatorExtensionBurnable(_tokensExtension[tokenId])
        ...
        if (_extensionApproveTransfers[_tokensExtension[tokenId]]) {
            require(
                IERC721CreatorExtensionApproveTransfer(
                    _tokensExtension[tokenId]
                ).approveTransfer(from, to, tokenId),
