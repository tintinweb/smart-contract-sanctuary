...
contract STO is Ownable {
    ...
    function acceptOffer(
        ...
        uint256 requiredUsdtTokenAmount = _calculateRequiredUsdtTokenAmount(
            _id,
            _offerId,
            _tokenAmount
        );
    ...
    // a method to calculate how much usdt tokens are needed while accepting offer in secondary market
    function _calculateRequiredUsdtTokenAmount(
        uint256 _id,
        uint256 _offerId,
        uint256 _tokenAmount
    ) public view validId(_id) validOfferId(_id, _offerId) returns (uint256) {
        Offer memory currentOffer = idToOfferMapping[_id][_offerId];
