...
contract LotoNFT is ERC721, ERC721Burnable, Ownable, VRFConsumerBase, ReentrancyGuard {
    ...
    function distributeRewards() internal {
        for (uint i = 0; i < _winnerTokens.length; i++) {
            ...
            if (i < tier1WinnersQuantity) {
                share = tier1Share.mul(1 ** 25).div(tier1WinnersQuantity);
            } else if (i < tier1WinnersQuantity + tier2WinnersQuantity) {
                share = tier2Share.mul(1 ** 25).div(tier2WinnersQuantity);
            } else if (i < tier1WinnersQuantity + tier2WinnersQuantity + tier3WinnersQuantity) {
                share = tier3Share.mul(1 ** 25).div(tier3WinnersQuantity);
            }

            accountRewards[winner] += finalAmount.mul(share).div(100).div(1 ** 25);
        }
    }
