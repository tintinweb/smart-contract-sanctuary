...
contract EtchedNFT1155 is ERC1155, ERC165Storage, Ownable, IERC2981 {
  ...
  mapping(uint256 => uint256) private _caps;
  mapping(address => bool) private minters;

  event TokenURI(uint256 indexed id, string indexed tokenUri, uint256 amount, address creator);
  event RoyaltyRateSet(uint256 indexed rate, uint256 indexed scale);
  event MinterUpdated(address indexed minter, bool indexed canMint, string userIdentifier);
  ...
  /// @notice returns next available NFT index
  /// @dev This is useful in figuring out whitelisting tokenIds
  /// @return the next available index
  function getNextOpenId() external view returns(uint256) {
  ...
  /// @param userIdentifier human readable string of creator

  function setMinter(address minter, bool canMint, string calldata userIdentifier) external onlyOwner {
  ...
  function mint(
    ...
    require(
      _totalSupply[id] + amount <= _caps[id] || _caps[id] == 0,
      "mint exceeds cap"
    );
    _mint(to, id, amount, "");
    ...
    _caps[id]= amount;
    emit TokenURI(id, tokenUri, amount, creator);
  }
  ...
  function mintBatch(
    ...
    for(uint256 i = 0; i < amounts.length; i++) {
      ...
      ids[i] = tokenId;
      _caps[ids[i]]= amounts[i];
      _creators[ids[i]] = msg.sender;
    }
    _mintBatch(to, ids, amounts, "");
    for (uint256 i = 0; i < amounts.length; i++) {
      _tokenURIs[i] = uris[i];
      emit TokenURI(ids[i], uris[i], amounts[i], msg.sender);
    }
  }

  /// @notice Bespoke function for airdropping the HDA NFT
  /// @custom:warning does not perform safety checks!
  /// @dev Cycles through an array of addresses and mints through the first 450 tokenIds
  /// @param to an array of addresses to send the HDA NFT to
  function airdrop(address[] memory to, uint256[] memory amount) external onlyOwner {
    ...
    require(total <= 450, "cannot airdrop >450 NFTs");
    require(len == amount.length, "arrays must have equal length");
    for (uint256 i = 0; i < len; i++) {
      uint256 amt = amount[i];
      address _to = to[i];
      _totalSupply[1] += amt;
      this.mint(msg.sender, _to, amt, _tokenURIs[1]);
      emit TransferSingle(owner(), address(0), _to, 1, amt);
    }
  }
  ...
  function _exists(uint256 id) internal view returns(bool) {
    return _totalSupply[id] > 0 || _creators[id] != address(0) || keccak256(abi.encodePacked(_tokenURIs[id])) != "";
  }
}
