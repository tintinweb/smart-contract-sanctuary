...
import "./ILoot.sol";
import "./LootMonCard.sol";
import "./LootMonUtil.sol";
import "./Base64.sol";
...
contract LootMonster is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory _weapons = getWeapon(tokenId);
        ...
        parts[3] = LootMonCard.card(_element, _type, _horns, _wings, _weapons);
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Monster No #', toString(tokenId), '", "description": "LootMonster is the cloning project of LOOT, for random monsters generated and stored on chain. plus drop 1 item from loot bag!. Stats, images, and other functionality are intentionally omitted for others to interpret. Feel free to use Loot in any way you want.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
...
library LootMonCard {
    ...
    function card (string memory _element, string memory _type, string memory _horns, string memory _wings, string memory _weapons ) public pure returns(string memory) {
        return string(abi.encodePacked('<text x="105" y="60" class="hd" fill="url(#gradient)">', _element,'</text><text x="110" y="52" class="md">', _type,'</text><text x="114" y="95" class="base">- ', _horns,'</text><text x="114" y="115" class="base">- ', _wings,'</text><text x="114" y="135" class="base">- ', _weapons,'</text>'));
    }
