...
contract Origins is ERC721Enumerable, ReentrancyGuard, Ownable {
  ...
  uint256[] private roleColorMatch = [4, 14, 29, 48, 71, 99];

  function random(uint256 a, uint256 b) internal pure returns (uint256) {
    return uint256(keccak256(abi.encodePacked(a, b)));
  }

  //@return 1~6
  function getColor(uint256 tokenId) internal view returns (uint256) {
    uint256 rand = random(block.difficulty, tokenId);
  ...
  function getCity(uint256 tokenId) public view returns (string memory) {
    return _pluck1(tokenId, cities);
  }

  function getProvince(uint256 tokenId) public view returns (string memory) {
    return _pluck1(tokenId, provinces);
  }

  function getKingdom(uint256 tokenId) public view returns (string memory) {
    return _pluck1(tokenId, kingdoms);
  }
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    parts[
      0
    ] = '<jxml version="1.0" encoding="UTF-8"?><svg width="750px" height="750px" viewBox="0 0 750 750" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"><rect fill="#231D39" x="0" y="0" width="750" height="750"></rect><text id="Male-Demons-Chaotic" font-family="Georgia" font-size="28" font-weight="normal" line-spacing="44" fill="#FFFFFF"><tspan x="60" y="212">';
    parts[1] = toString(getColor(tokenId));
    parts[2] = '">';
    parts[3] = getCity(tokenId);
    parts[4] = '</tspan><tspan x="60" y="256" fill="';
    parts[5] = toString(getColor(tokenId));
    parts[6] = '">';
    parts[7] = getProvince(tokenId);
    parts[8] = '</tspan><tspan x="60" y="300" fill="';
    parts[9] = toString(getColor(tokenId));
    parts[10] = '">';
    parts[11] = getKingdom(tokenId);
    parts[12] = "</tspan></text>";
    parts[13] = "</g></svg>";
    ...
    string memory atrrOutput = makeAttributeParts(getCity(tokenId), getProvince(tokenId), getKingdom(tokenId));
    string memory json =
      _encode(
        bytes(
          string(
  ...
  function makeAttributeParts(
    string memory city,
    string memory province,
    string memory kingdom
    ...
    attrParts[4] = '" }, { "trait_type": "Kingdom", "value": "';
    attrParts[5] = kingdom;
    attrParts[6] = '" }]';

    string memory atrrOutput =
      string(abi.encodePacked(attrParts[0], attrParts[1], attrParts[2], attrParts[3], attrParts[4], attrParts[5], attrParts[6]));

    return atrrOutput;
  }

  function _pluck1(uint256 tokenId, string[] memory array) internal view returns (string memory) {
    uint256 rand = random(block.difficulty, tokenId);
