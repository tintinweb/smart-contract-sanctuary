...
contract Factory is Ifactory, FactoryStorage{
    ...
    function initialize(address _luca, address _wluca, address _trader, address _weth, address _collector) external noInit{
       owner = msg.sender;
       luca = _luca;
       wluca = _wluca;
       weth =_weth;
    ...
    function _createLink() internal returns(Link){
         Link link = new Link(address(this), luca, wluca, trader, weth);
         totalLink++;
