...
contract colorgradient is ERC721Enumerable, Ownable,ReentrancyGuard {
    ...
    */
    constructor() ERC721("The Color_Gradient", "GRADCOLORS") {}
    ...
    uint256 private Max_Items = 4000;
    ...
    function generateSVGImage(string memory hexString1,string memory hexstring2) public pure returns(string memory){
        string[7] memory parts;
        
        parts[0] = '<svg width="200" height="200" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg"><rect width="200" height="200" fill="url(#paint0_linear)"/>';
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721) returns (string memory) {
        require(_hexColors1[tokenId] > 0, "ERC721Metadata: URI query for nonexistent token");
        require(_hexColors2[tokenId] > 0, "ERC721Metadata: URI query for nonexistent token");
    ...
    function mintgradients(address add,uint256 numOftokens) public nonReentrant {
        require(totalSupply() + numOftokens <= Max_Items);
        uint256 index;
        address adds = add;
        for(uint32 i=0;i < numOftokens; i++){
            index = totalSupply();
            _safeMint(adds,index);
    ...
    function generateRandomHexColor(uint256 tokenId) internal returns (uint32,uint32) {
        uint32 hexColorA = uint32(_rng() % 16777215);
        uint32 hexColorB = uint32(_rng() % 12998277);

        while (existingHexColorsA[hexColorA] || existingHexColorsA[hexColorB]) {
          if(existingHexColorsA[hexColorA]){
            hexColorA = uint32(uint256(hexColorA + block.timestamp * tokenId) % 16777215);}
          else{
            hexColorB = uint32(uint256(hexColorB + block.timestamp * tokenId) % 16777215);} 
        }

        existingHexColorsA[hexColorA] = true;
        existingHexColorsA[hexColorB] = true;
        if(_hexColors1[tokenId] == 0){
            _hexColors1[tokenId] = hexColorA;
        }
        if(_hexColors2[tokenId] == 0){
            _hexColors2[tokenId] = hexColorB;
        }
        return (hexColorA,hexColorB);
    }
