    ...
    BlockTimestamp
{
    ...
    address internal constant ORACLE = 0x0604ADf9e13DFD3d2eF3Eb0F994885A8D9849234;
    ...
    address internal constant ULM_STATE = 0x5c1E2F8790bd1FEa331569BAB144bD6272D873A4;
    ...
    function _increaseLiquidity(
        ...
        (a.token0, a.token1, a.fee, , , , a.currentTick) = IULMState(ULM_STATE).getPoolDetails(pool);
    ...
    function addRangeLiquidity(
        ...
        // hre
        (address token0, address token1, uint24 fee, , , , ) = IULMState(ULM_STATE).getPoolDetails(pool);
        LiquidityPosition storage position = liquidityPositions[pool];
    ...
    function _updatePosition(
        ...
        IUniswapV3Pool uniswapPool = IUniswapV3Pool(pool);
        (uint128 liquidity, , , , ) = LiquidityReserves.position(address(this), tickLower, tickUpper, uniswapPool);
        if (liquidity > 0) {
    ...
    function updateUserFees(uint256 tokenId, uint256 tokensOwed0, uint256 tokensOwed1, uint256 feeGrowthGlobal0, uint256 feeGrowthGlobal1) private {
    ...
    //     updateUserFees(tokenId, tokensOwed0, tokensOwed1, feeGrowthGlobal0, feeGrowthGlobal1);
    ...
    function distributeFees(
        ...
        uint256 _tokensOwed0,
        uint256 _tokensOwed1
    ) public returns (uint256 indexAmount0, uint256 indexAmount1) {
        // only unipilot
        LiquidityPosition storage position = liquidityPositions[pool];
        (address token0, address token1, , , , , ) = IULMState(ULM_STATE).getPoolDetails(pool);
    ...
    function withdraw(
        ...
        (uint256 indexAmount0, uint256 indexAmount1) = distributeFees(
            pilotToken,
            position.pool,
            recipient,
            position.tokensOwed0,
    ...
    //     (address token0, address token1, , , , , ) = IULMState(ULM_STATE).getPoolDetails(params.pool);
    //     (
    //         address tokenAlt,
    //         uint256 altAmount,
    //         address tokenWeth,
    ...
    //         uint256 tokenAmount = oracle.getPilotAmountWethPair(tokenAlt, altAmount, wethAmount);
    ...
    //             ) = IULMState(ULM_STATE).getUserAndIndexShares(params._tokensOwed0, params._tokensOwed1);
...
contract Unipilot is IUnipilot, ERC721Permit, PeripheryPayments {
    ...
    function collect(
        ...
        // );
        liquidityManager.collect(pilotToken, recipient, tokenId, data);
    }

    function mintUnipilotNFT(address sender) external override returns(uint256 mintedTokenId) { // only ULM
    ...
    function withdraw(
        ...
        //     "UNIPILOT:: EXCHANGE_NOT_WHITELISTED"
        // );
        liquidityManager.withdraw(pilotToken, recipient, tokenId, liquidity, data);
    }
    ...
    //     );
    //     IHandler(exchangeAddress).withdrawForETH(wethToken, pilotToken, data);
    // }
    ...
    //         "UNIPILOT:: INVALID_EXCHANGE"
    //     );
    //     liquidityManager.collectFeesForWETHPairs(pilotToken, wethToken, data);
    // }
