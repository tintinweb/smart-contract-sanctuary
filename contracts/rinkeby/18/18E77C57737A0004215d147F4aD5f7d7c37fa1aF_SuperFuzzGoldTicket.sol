...
contract SuperFuzzGoldTicket is Ownable, ERC721URIStorage, ERC721Enumerable {
    ...
    bool public isSaleActive = DEBUG == true;
    ...
    // using the array as the how many param
    function setManyWhiteList(address[] memory _addr)
        external 
        onlyOwner
        returns (address[] memory)
    {
        ...
        emit WhiteListMutated(_whiteList, msg.sender);
        return _whiteList;
    }

    function setWhiteList(address[] memory _addrs) external onlyOwner {
        whiteList = _addrs;
        emit WhiteListMutated(_addrs, msg.sender);
    }

    function addToWhiteList(address _addr) external onlyOwner {
        whiteList.push(_addr);
        emit WhiteListMutated(whiteList, msg.sender);
    }
    ...
    function addToAirDropList(address _to) internal {
        console.log("Added this address to airDrop List", _to);
