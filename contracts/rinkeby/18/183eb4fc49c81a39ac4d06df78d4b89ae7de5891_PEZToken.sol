...
contract Stakable   is Ownable,UsageGuard
{
    ...
    //----------------------------------------------------------------------
    function    nftStake(string memory collectionName, uint256 tokenId) external callerIsAContract returns(bool)  
    {
        ////// AJOUTER MECANISME POUR QUE SEUL UN CONTRAT PUISSE APPELLER CECI
        
        bool    isAllowedContract = allowedNftContractAddresses[msg.sender];
        
        require(isAllowedContract==true, "This contract is not allowed");
        
        return nftStakeEx(collectionName, tokenId, false, msg.sender, 0,0, false);
    }
    //----------------------------------------------------------------------
    ...
    //----------------------------------------------------------------------
    function    listNftStakeRewards(string memory collectionName, uint256[] memory tokenIds) external view returns(TNftReward[] memory)
    {
        return listNftStakeRewardsEx(collectionName, tokenIds);
    }
    //----------------------------------------------------------------------
    function    listNftStakeRewardsEx(string memory collectionName, uint256[] memory tokenIds) internal view returns(TNftReward[] memory)
    {
        TStakableNftCollectionInfo memory nft = stakableNftCollections[collectionName];
        ...
        for (uint i=0; i<n; i++)
        {
            ...
            if (INftToken(nft.collectionAddress).isItTheNftOwner(msg.sender, tokenId))
            {
                ...
                rewards[i] = TNftReward( tokenId, reward );
            }
        }
...
interface   IPEZToken
{
    function claimNftRewards(string memory collectionName, uint256[] memory tokenIds) external;
}*/
...
contract    PEZToken    is  Context, IERC20, IERC20Metadata , Ownable, ReentrancyGuard, Stakable//, IPEZToken
{
    ...
    function    claimNftRewardsEx(string memory collectionName, address userWallet, uint256[] memory tokenIds) internal returns(bool)
    {
        ...
        TNftReward[] memory rewards = listNftStakeRewardsEx(collectionName, tokenIds);
    ...
    {
        return this.closeNftStakingEx(collectionName, tokenId, msg.sender);
    }
    //----------------------------------------------------------------------
    function    closeNftStakingEx(string memory collectionName, uint256 tokenId, address userWallet) external returns(bool)
    {
    ...
    {
        //----- d'abord il faut fermer le privilege de staking de l'ancien proprio (=from)
