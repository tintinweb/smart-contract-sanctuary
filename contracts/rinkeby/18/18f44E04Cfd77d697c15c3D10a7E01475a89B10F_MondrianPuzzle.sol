 ...
 */
contract MondrianPuzzle is ERC721, ERC721Enumerable, Ownable {
    ...
    uint16 public constant YEARLY_SUPPLY = 1024;
    uint16 public constant MAX_SUPPLY = 5120;
    uint8 public constant YEARLY_FREE_SUPPLY = 5;
    Counters.Counter private tokenIdCounter;
    mapping(uint256 => Counters.Counter) private yearlyCounts;
    mapping(uint256 => Counters.Counter) private yearlyFreebies;
    

    function performMint(address destination) private  {
        require(currentYearSupply() <= YEARLY_SUPPLY, "Yearly quota reached!");
        require(totalSupply() < MAX_SUPPLY, "Reached max supply. It is no longer posible to mint new puzzles.");
        uint256 tokenId = tokenIdCounter.current();
        _safeMint(destination, tokenId);

        uint256 year = getCurrentYear();
        yearlyCounts[year].increment();        
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        uint8 rectsWanted = uint8( (PuzzleGenerator.random(tokenId) % 10) + 6); //6-16
        string memory encodedJSON =  PuzzleGenerator.generateMetadata(tokenId, rectsWanted);
    ...
    function currentYearSupply() public view returns (uint256) {
        return yearlyCounts[getCurrentYear()].current();
    }    
    
    function currentYearFreeSupply() public view returns (uint256) {
        return yearlyFreebies[getCurrentYear()].current();
    }  

    function availableFreebies() public view returns (uint256) {
        return YEARLY_FREE_SUPPLY - yearlyFreebies[getCurrentYear()].current();
    }  

    function mintForSelf() public payable virtual {
        require(msg.value >= price, "PRICE_NOT_MET");
        performMint(msg.sender);
    }

    function mintForOther(address walletAddress) public payable virtual {
        require(msg.value >= price, "PRICE_NOT_MET");
        performMint(walletAddress);
    }

    function mintForFree() public virtual {
        require(yearlyFreebies[getCurrentYear()].current() < YEARLY_FREE_SUPPLY, "No more freebies this year.");
        yearlyFreebies[getCurrentYear()].increment(); 
        performMint(msg.sender);
    }

    function mint(address destination) public onlyOwner {
        performMint(destination);
    }
