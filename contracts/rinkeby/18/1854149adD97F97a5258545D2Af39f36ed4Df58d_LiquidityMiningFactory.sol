...
contract LiquidityMiningFactory is ILiquidityMiningFactory, Managed, EIP712 {
    ...
    function createLiquidityMining(
        ...
        if (!hasPermission(msg.sender, ROLE_ADMIN)) {
            bytes32 structHash =
                keccak256(
                    abi.encode(
                        _CONTAINER_TYPEHASE,
            ...
            address registeredSigner =
                management.contractRegistry(ADDRESS_SIGNER);

            require(signer == registeredSigner, "LiquidityFactory: invalid signature");
        }
        ...
        LiquidityMiningStaking stake =
            new LiquidityMiningStaking(
                address(management),
                _stakingName,
                _stakedToken,
                _startBlock,
        ...
        IStakingRegister(management.contractRegistry(CONTRACT_STAKING_REGISTER))
            .add(msg.sender, address(stake), true);
    }
