...
contract EBMarketAuctions is ERC721Holder, ERC1155Holder, Ownable, ReentrancyGuard, TokensHandler {
    uint public id;
    uint public fee = 20;
    enum Status {
        LISTED,
    ...
    event AuctionCancelled(uint indexed id, Auction auction);
    event NewBid(uint indexed id, Auction auction);
    event AuctionFinished(uint id, Auction auction);
    ...
    function create(address[] calldata _nftAddresses,uint[] calldata _nftIds,uint32[] calldata _nftTypes,uint _valability,uint _price,uint _tax, address _currency) external {
        require(_price > 0 && _tax > 0 && _valability > 0 && _tax * 2 < _price, "Input values are not valid");
    ...
    function cancel(uint auctionId) external isAccesible(auctionId) nonReentrant {
        ...
        emit AuctionCancelled(auctionId,auctions[auctionId]);
    }
    ...
    function bid(uint auctionId) external payable isAccesible(auctionId) nonReentrant {
        ...
        emit NewBid(auctionId,auctions[auctionId]);
    }
    ...
    function finish(uint auctionId) external nonReentrant {
        ...
        emit AuctionFinished(auctionId,auctions[auctionId]);
    }
