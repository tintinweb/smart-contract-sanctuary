...
contract Phunky is ERC20 {
    ...
    function claimable(
        ...
        if (initialBlock + halfAfterBlock > currentBlock) {
            ...
            // where constant is halfAfterBlock * perBlockGrant
            // common ratio is 1/2
            // n terms are splits
            // overflow will not be an issue for about 100 years of claiming, 
            // after that negative subtraction will cause this function to fail
            claimableTokens = (halfAfterBlock * perBlockGrant * 2 * (2**splits - 1)) / 2**splits;
            // claimable tokens claimable after the last split
