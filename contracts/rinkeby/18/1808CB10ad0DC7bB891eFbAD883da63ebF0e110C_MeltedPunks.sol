...
contract MeltedPunks is ERC721Enumerable, Ownable {
    ...
    string currentContractURI = "ipfs";
    string baseURI;
    bool baseURIChangeable = true;

    using Strings for uint256;

    constructor(string memory initialBaseURI) ERC721("MeltedPunks", "MP") {
    ...
    */
    ...
    function mintPunk(uint256 tokenId) public payable returns (uint256) {
        require(msg.value >= currentPrice, "Must send enough ETH.");
        require(
            tokenId >= 0 && tokenId < 10000,
            "Token ID must be between -1 and 10000"
        );
