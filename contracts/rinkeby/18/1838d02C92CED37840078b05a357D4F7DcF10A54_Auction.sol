...
contract Auction is Ownable, ReentrancyGuard {
    ...
    modifier whenSaleActive() {
        require(saleActive, "BAU: Sale is not active");
        _;
    }

    modifier whenSalePaused() {
        require(!saleActive, "BAU: Sale is not paused");
        _;
    }
    ...
    function setTokenAddress(IERC721Enumerable token_) public virtual onlyOwner {
        require(collection.length() == 0, "BAU: The collection have to be empty");
        _token = token_;
    }
    ...
    function startSale(
        ...
        require(saleStartPrice_ < saleFinalPrice_,  "BAU: Start price have to be less than final price");
    ...
    function remainingSaleTime() public view returns (uint256) {
        require(saleStartTime > 0, "BAU: Public sale hasn't started yet");
    ...
    function _preValidatePurchase(uint256 tokensAmount) internal view {
        ...
        require(tokensAmount > 0, "BAU: Need buy at least one token");
        require(tokensAmount <= tokenAmountBuyLimit(), "BAU: Limited amount of tokens in transaction");
        require(tokensAmount <= remainingTokensAmount(), "BAU: Limited amount of remaining tokens");
        require(remainingSaleTime() > 0, "BAU: Sale is over");
    }
    ...
    function buyTokens(uint256 tokensAmount) external payable whenSaleActive nonReentrant returns (uint256[] memory) {
        ...
        require(costToMint <= msg.value, "BAU: Insufficient funds");
