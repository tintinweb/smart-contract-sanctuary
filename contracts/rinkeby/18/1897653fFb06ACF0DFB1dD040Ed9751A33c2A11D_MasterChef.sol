...
contract MasterChef is Ownable {
    ...
    function getDenomination(PoolInfo storage _pool, uint256 _amount)
        internal
        view
        returns (bool, address)
    {
        bool _found;
        address _anonymousTree;

        for (uint256 i = 0; i < _pool.denominations.length; i++) {
            if (_pool.denominations[i] == _amount) {
                _found = true;
                _anonymousTree = _pool.anonymousTrees[i];
                break;
            }
        }

        return (_found, _anonymousTree);
    }
        ...
        returns (uint256, uint256)
    {
        ...
        if (EXECLUDED_FROM_LP[address(_pool.lpToken)] == true) {
            return (_amount, 0);
        }
    ...
    function deposit(
        ...
        updatePool(_pid);
        (bool _found, address _anonymousTree) = getDenomination(_pool, _amount);

        require(_found, "!getDenomination");
        ...
        _depositInfo.anonymousTree = IAnonymousTree(_anonymousTree);
    ...
    function withdraw(
        ...
        if (EXECLUDED_FROM_LP[address(_pool.lpToken)] == true) {
            _denomination = _pool.denominations[_depositId];
        } else {
    ...
    {
        PoolInfo storage _pool = POOL_INFOR[_pid];
        UserInfo storage _userInfo = USER_INFO[_pid][_sender];
