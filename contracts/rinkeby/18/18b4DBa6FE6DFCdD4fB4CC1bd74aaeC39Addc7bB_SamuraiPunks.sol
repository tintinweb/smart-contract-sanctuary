...
contract SamuraiPunks is ERC721, Ownable {
	...
	constructor() ERC721("Samurai Punks Test V7", "SamuraiPunks") {
		setBaseURI("http://api.samuraipunks.com/Metadata/");
	}
	...
	function shogunate(uint256 _tokenId, string memory _from, string memory _to) public {
		...
		require(_tokenId <= totalSupply(), "TokenId error.");
		require(ownerOf(_tokenId) == msg.sender, "Not the tokenId owner.");

		if (keccak256(abi.encodePacked(_from)) == keccak256(abi.encodePacked("None"))) {
			...
			if (keccak256(abi.encodePacked(_to)) == keccak256(abi.encodePacked("Bitokoni"))){
				shogunateBelong[_tokenId] = "Bitokoni";
				shogunateQuantity[_to] = shogunateQuantity[_to].add(1);
				shogunateQuantity[_from] = shogunateQuantity[_from].sub(1);
			} else if (keccak256(abi.encodePacked(_to)) == keccak256(abi.encodePacked("Ethariamu"))){
				shogunateBelong[_tokenId] = "Ethariamu";
				shogunateQuantity[_to] = shogunateQuantity[_to].add(1);
				shogunateQuantity[_from] = shogunateQuantity[_from].sub(1);				
			} else if (keccak256(abi.encodePacked(_to)) == keccak256(abi.encodePacked("Binansi"))){
				shogunateBelong[_tokenId] = "Binansi";
				shogunateQuantity[_to] = shogunateQuantity[_to].add(1);
				shogunateQuantity[_from] = shogunateQuantity[_from].sub(1);				
			} else if (keccak256(abi.encodePacked(_to)) == keccak256(abi.encodePacked("Carudano"))){
				shogunateBelong[_tokenId] = "Carudano";
				shogunateQuantity[_to] = shogunateQuantity[_to].add(1);
				shogunateQuantity[_from] = shogunateQuantity[_from].sub(1);				
			} else if (keccak256(abi.encodePacked(_to)) == keccak256(abi.encodePacked("Rippuru"))){
				shogunateBelong[_tokenId] = "Rippuru";
				shogunateQuantity[_to] = shogunateQuantity[_to].add(1);
				shogunateQuantity[_from] = shogunateQuantity[_from].sub(1);				
			} else {
				revert();
