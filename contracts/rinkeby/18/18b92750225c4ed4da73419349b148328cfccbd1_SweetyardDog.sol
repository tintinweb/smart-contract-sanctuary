...
contract SweetyardDog is ERC721URIStorage, Ownable {
    ...
    // uint256 public total = 8888 * 2 + 1;
    // uint256 private bombId = 99999;
    uint256 private constant Supply = 8888;
    uint256 private constant TOKEN_LIMIT = 8688;
    uint256 private constant Once_Max_Mint = 10;
    ...
    function mint(uint256 _quantity) external payable returns (uint256[] memory) {
        ...
        require(_quantity <= Once_Max_Mint,"error: max quantity 10");
        ...
        if(isStart == true || whiteList[msg.sender] == true){
            ...
            uint256 proj = total.sub(tech);
            
            (bool sent1, bytes memory data1) = payable(TECH_ADDRESS).call{
                value: tech
            }("");
            require(sent1, "Failed to send Eth");
            (bool sent2, bytes memory data2) = payable(address(this)).call{
                value: proj
            }("");
