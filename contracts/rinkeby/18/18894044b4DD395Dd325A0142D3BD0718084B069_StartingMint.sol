...
contract StartingMint is Ownable {
    ...
    uint256 public claimLimitCount = 1;
    ...
    function claim() public payable onlyEOA {
        ...
        require(claimCount[msg.sender] < claimLimitCount, "no quota");
        //pay eth
    ...
    function _claim(address _receiveAddress) internal {
        ...
        claimCount[_receiveAddress] = claimCount[_receiveAddress].add(1);
    }

    function _mintLoot(address _receiveAddress) internal {
        bytes32 seed = keccak256(
            abi.encode(
                claimCount[_receiveAddress],
                _receiveAddress,
                block.number
            )
        );
    ...
    //max balance per account
    function setLimitCount(uint256 _limitCount) public onlyOwner {
        claimLimitCount = _limitCount;
    }
