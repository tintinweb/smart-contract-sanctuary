...
contract PersonalData is Ownable {
    ...
    mapping(uint256 => DataTypesUsed) dataTypesUsed;
    ...
    function checkDataTypeExistence(string memory _dt) private view returns (bool) {
        for (uint256 i = 0; i < dataTypes.retrieveDataTypes().length; i++) {
            if (keccak256(bytes(_dt)) == keccak256(bytes(dataTypes.retrieveDataTypes()[i]))) {
                return true;
            }
        }
        return false;
    ...
    function checkDataTypeUsed(string memory _dt) private view returns (bool) {
        for (uint256 i = 0; i < dataTypesCounter; i++) {
            if (keccak256(bytes(_dt)) == keccak256(bytes(dataTypesUsed[i].dataType))) {
                return false;
            }
        }
        return true;
