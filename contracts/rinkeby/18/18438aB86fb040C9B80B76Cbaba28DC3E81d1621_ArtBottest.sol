...
contract ArtBottest is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    uint256 private tokenPrice = 0.001 * 10 ** 18; //0.001
    ...
    function buyArt(uint numberOfTokens, uint256 eth_paid) public payable {
        require(saleIsActive, "Sale is not active");
        require(numberOfTokens <= maxTokensPerTransaction, "Exceeds max number of Art in one transaction");
        require(totalSupply().add(numberOfTokens) <= nftsPublicNumber, "Purchase would exceed max supply of Art");
        uint256 orderPrice = tokenPrice.mul(numberOfTokens);
        ...
        for (i; i < numberOfTokens; i++) {
            ...
            if (totalSupply() < nftsPublicNumber) {
