...
import { ITeller, IDepository, IBondingCalculator } from "../external/OlympusV2Interfaces.sol";
import { MockERC20 } from "../mocks/MockERC20.sol";
import { IERC20 } from "@openzeppelin/contracts/token/ERC20/IERC20.sol";

contract MockBondDepository is IDepository {
    MockERC20 public immutable mockDAI;

    IBondingCalculator public immutable mockCalculator;

    ITeller public immutable mockTeller;

    constructor(MockERC20 _mockDAI, IBondingCalculator _mockCalculator, ITeller _mockTeller) {
        mockDAI = _mockDAI;
        mockCalculator = _mockCalculator;
        mockTeller = _mockTeller;
    }

    function bonds(uint256 _BID) external override view returns(Bond memory bond) {
        Terms memory term = Terms({
            controlVariable: 100,
            fixedTerm: true,
            vestingTerm: 10,
            expiration: 0,
            conclusion: uint256(type(uint64).max),
            minimumPrice: 1,
            maxPayout: 1000e9,
            maxDebt: 10e6
        });
        return Bond({
            principal: IERC20(address(mockDAI)),
            calculator: address(mockCalculator),
            terms: term,
            termsSet: true,
            capacity: 10000000e9,
            capacityIsPayout: true,
            totalDebt: 1000e9,
    ...
    function deposit(
        ...
        uint256 _BID,
        address _feo
    ) external override returns(uint256 payout, uint256 index) {
        mockDAI.transferFrom(msg.sender, address(this), _amount);
        mockDAI.burn(_amount);

        index = mockTeller.newBond(msg.sender, address(mockDAI), _amount, 1000e9, block.timestamp + 5 days, _feo);
        payout = 1000e9;
    }
}
