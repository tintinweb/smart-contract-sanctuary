...
contract BridgeETH is Ownable, ReentrancyGuard {
    ...
    function withdraw(uint amount, uint chainId, uint id, uint8 v, bytes32 r, bytes32 s) external noContractsAllowed nonReentrant {
        require(chainId == CHAIN_ID, "invalid chainId!");
        require(!claimedWithdrawalsByOtherChainDepositId[id], "already withdrawn!");
        require(verify(msg.sender, amount, chainId, id, v, r, s), "invalid signature!");
    ...
    /// signature methods.
	function verify(address account, uint amount, uint chainId, uint id, uint8 v, bytes32 r, bytes32 s) internal view returns(bool) {
		bytes32 message = prefixed(keccak256(abi.encode(account, amount, chainId, id, address(this))));
        return (recoverSigner(message, v, r, s) == verifyAddress);
	}
