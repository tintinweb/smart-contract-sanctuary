...
contract NobodySea is ERC721, Ownable {
  ...
  constructor (uint _maxNobody, uint _maxNobodyPerPurchase ) ERC721("nobodysea", "nobodysea") {
  ...
  function presalemint(uint256 _numNobody ) public payable {
    ...
    _minttokens(msg.sender, _numNobody);
  ...
  function reserveToken(address _to, uint256 _count) public onlyOwner {
     _minttokens(_to, _count);
  }

  function awardToken(address[] memory _to) public onlyOwner {
     for (uint256 i = 0; i < _to.length; i++) {
        uint256 newItemId = _tokenIds.current();
        _tokenIds.increment();
