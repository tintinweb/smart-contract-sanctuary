...
contract BattleDoge is ERC721, VRFConsumerBase, Ownable {
   ...
   function Attack (uint256 tokenId, uint256 _tokenId , uint256 _number)  public  Gladiator returns (uint256 winner)   {
   ...
   function getCurrentFighter(uint256 tokenId) public view returns(address){
    ...
    function toggleAllowed(address addr) external onlyOwner {
     ...
     function toggleFighter(address addr) external  {
        isFighter[addr] = !isFighter[addr];
    }
