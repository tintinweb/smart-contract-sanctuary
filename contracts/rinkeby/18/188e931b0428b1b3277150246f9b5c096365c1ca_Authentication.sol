...
contract Authentication {
                              ...
                              string memory _password,string memory _cnic) public returns (bool) 
    {
        ...
        if(keccak256(abi.encodePacked(_name)) == keccak256(abi.encodePacked("")) )
            ...
            if(keccak256(abi.encodePacked(_password)) == keccak256(abi.encodePacked("")) )
            {
                return false;
            }
            if(keccak256(abi.encodePacked(_cnic)) == keccak256(abi.encodePacked("")) )
            {
                return false;
            }
            require(_address != address(0) ,"User address is not Entered!");
