...
contract VechTokenContract is ERC721A, Ownable, Pausable {
    ...
    constructor() ERC721A("VECH", "VECH", 100) {}
    ...
    {
        require(
            tokenId < mintedPublicTokens || tokenId >= maxPublicMintableTokens,
        ...
        return
            string(
                abi.encodePacked(
                    base,
                    uint2str(vechId % maxPublicMintableTokens)
                )
            );
    }
    ...
    modifier amountWithinMaxTokens(uint8 amount) {
        require(
            mintedPublicTokens.add(amount) <=
                maxPublicMintableTokens.sub(reservedTokens()),
  ...
  IERC721Enumerable
{
  ...
  function _mint(address to, uint256 quantity, uint256 startTokenId) internal {
