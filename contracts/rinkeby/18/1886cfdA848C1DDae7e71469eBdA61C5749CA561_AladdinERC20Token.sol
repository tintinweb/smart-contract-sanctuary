...
contract AladdinERC20Token is Divine {
  ...
  using SafeERC20 for IERC20;
    ...
    uint public timeForQuilifiedBuyer = 300; //seconds 5mins [3600]
    uint public PresalePrice = 100; // 1 BNB = PresalePrice Aladdin in stage 1;
    uint public S1 = 4000; // stage1
    uint public S2 = 10000;// stage2
    uint public S3 = 50000;// stage3
    uint public _floorbuyAmount = 1*1e16; // 0.01 BNB
    uint public _upperbuyAmount =  10*1e18; // 10 BNB
    mapping( address => bool ) public isQuilifiedPreBuyer;
    // buyer can only buy once in the timeForQuilifiedBuyer period
    mapping( address => uint ) public buyerStage; 

    constructor() Divine("Aladdin", "ALD", 9) {
    }

    function addQuilifiedPreBuyer( address _QuilifiedPreBuyer ) external onlyOwner() returns ( bool ) {
        _addQuilifiedPreBuyer( _QuilifiedPreBuyer );
        return isQuilifiedPreBuyer[_QuilifiedPreBuyer];
    }

    function addQuilifiedPreBuyers( address[] calldata _QuilifiedPreBuyers ) external onlyOwner() returns ( bool ) {

        for( uint256 iteration_; _QuilifiedPreBuyers.length > iteration_; iteration_++ ) {
            _addQuilifiedPreBuyer( _QuilifiedPreBuyers[iteration_] );
        }
        return true;
    }

    function _addQuilifiedPreBuyer( address _QuilifiedPreBuyer ) internal {
        isQuilifiedPreBuyer[_QuilifiedPreBuyer] = true;
    }

    function StartPresale () external onlyOwner() {
        startTime = block.timestamp;
        presaleOpen = true;
        require(_vault!=address(0),"Should set vault before start");
        _mint(_vault,S1.mul(1e9).mul(80).div(100));
    }
    ...
    function buy() payable public returns(uint) {
        ...
        require(msg.value >= _floorbuyAmount && msg.value <= _upperbuyAmount ,"Transaction recovery");
        
        // in the timeForQuilifiedBuyer period, restrict buyers and one can only buy once
        if ( block.timestamp-startTime<timeForQuilifiedBuyer ){
            require( isQuilifiedPreBuyer[msg.sender] == true, "unqulified buyers should buy latter" );
            require( buyerStage[msg.sender] == 0,"Buyer already buy yet");
        }

        // calc payout and mint to users
        uint _payAmount = msg.value;
        uint _tokenPayoutAmount = CalTokenPayoutAmount(_payAmount);
        _mint(msg.sender, _tokenPayoutAmount);

        //record buyer stage
        uint _stage = GetStage();
        buyerStage[msg.sender] = _stage;
        return _tokenPayoutAmount;
    }

    function CalTokenPayoutAmount(uint _payAmount) internal view returns(uint) {
        uint _calTokenPayoutAmount;
        uint _stage1Amount = _payAmount.mul(PresalePrice).mul(1e9).div(1e18); // decimals is different
        uint _stage = GetStage();
        if (_stage == 1){
            _calTokenPayoutAmount = _stage1Amount;
        }else if (_stage == 2){
            _calTokenPayoutAmount = _stage1Amount.mul(75).div(100); 
        }else if (_stage == 3){
            _calTokenPayoutAmount = _stage1Amount.mul(60).div(100); 
        }else if (_stage == 4){
            _calTokenPayoutAmount = _stage1Amount.mul(55).div(100);
        }
        return _calTokenPayoutAmount;    
    }
    
    function GetStage() public view returns(uint) {
        uint _stage;
        uint _totalSupply = totalSupply().div(1e9); // decimal = 9
        if (_totalSupply<S1){
            _stage = 1;
        }else if(_totalSupply<S2){
            _stage = 2;
        }else if(_totalSupply<S3){
            _stage = 3;
        }else {
            _stage = 4;
        }
    ...
    // exist ALD presale and payback
    function existAndPayback (uint _amount) external payable {
        require(msg.value == 2e16,"exist fee is 0.02 BNB");
        transfer(address(this), _amount);
        // get buyers' bnb back
        uint _buyStageRecord = buyerStage[msg.sender];
        uint _paybackAmount;
        if ( _buyStageRecord == 1) {
            _paybackAmount = _amount.div(PresalePrice).mul(1e9); //mul(1e9)=1e18/1e9
        } else if( _buyStageRecord == 2) {
            _paybackAmount = _amount.div(PresalePrice).mul(100).div(75).mul(1e9);
        } else if( _buyStageRecord == 3) {
            _paybackAmount = _amount.div(PresalePrice).mul(100).div(60).mul(1e9);
        } else if( _buyStageRecord == 4) {
            _paybackAmount = _amount.div(PresalePrice).mul(100).div(55).mul(1e9);
        }
        msg.sender.transfer(_paybackAmount);
    }
    ...
    /**
     *  @notice allow anyone to send lost tokens to the vault
     *  @return bool
     */
    function recoverLostToken( address _token ) external returns ( bool ) {
        require(_vault!=address(0),"Vault is not set yet");
