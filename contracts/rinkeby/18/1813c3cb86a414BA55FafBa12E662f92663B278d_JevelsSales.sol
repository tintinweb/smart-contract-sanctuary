...
contract JevelsSales is ERC1155TokenReceiver {
    address authorizer;
    mapping(address => uint256) authorizationIds;
    ...
    function transfer(uint8 v_, bytes32 r_, bytes32 s_, Authorization calldata authorization_) public {
        ...
        require(authorization_.authorizationId == authorizationIds[authorization_.to] + 1, "Authorization ID is not valid");

        ERC1155(authorization_.tokenContract).safeTransferFrom(address(this), authorization_.to, authorization_.id, authorization_.amount, authorization_.data);

        authorizationIds[authorization_.to] += 1;
    }
    ...
    function batchTransfer(uint8 v_, bytes32 r_, bytes32 s_, AuthorizationBatch calldata authorization_) public {
        ...
        ERC1155(authorization_.tokenContract).safeBatchTransferFrom(address(this), authorization_.to, authorization_.ids, authorization_.amounts, authorization_.data);

        authorizationIds[authorization_.to] += 1;
    }

    function validAuthorizationId(address user_) public view returns (uint256){
        return authorizationIds[user_] + 1;
    }

    function setAuthorizer(address authorizer_) public {
