...
abstract contract NPassCore is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint256 public priceForNHoldersInWei;
    uint256 public priceForOpenMintInWei;
...
contract Potenza is NPassCore {
    ...
    constructor (string memory _name, string memory _symbol, address _n, string memory _baseImageUri) NPassCore(_name, _symbol, IN(_n), false, 2000, 1000, 0, 0) {
        builders = Builders(0x06C244a9BaFBC96E609A8DF32A07178552C7295a,0xc1D10c9d08CD1600fCaB60d9cA7cC09601870fE5,0x06C244a9BaFBC96E609A8DF32A07178552C7295a);
        baseImageUri = _baseImageUri;
    }

    function switchOpenSale() public onlyOwner() {
        publicSale = !publicSale;
    }

    function switchColorBurn() public onlyOwner() {
        colorBurn = !colorBurn;
    }

    function setPriceOpenMint(uint256 price) public onlyOwner() {
        priceForOpenMintInWei = price;
    }

    function setPriceNHolders(uint256 price) public onlyOwner() {
        priceForNHoldersInWei = price;
    }

    function mint(uint256 _tokenId) public payable override virtual nonReentrant {
        require(_tokenId > 0 && _tokenId <= 8888, "Token ID invalid");
        require(msg.value == priceForOpenMintInWei, "Incorrect price");
        require(publicSale, "Public sale is not active");
        _safeMint(msg.sender, _tokenId);
    }

    function burnForColor(uint256 _tokenId1, uint256 _tokenId2) public nonReentrant {
        require(_tokenId1 != _tokenId2, "TokenIds cannot be equal");
        require(_tokenId1 > 0 && _tokenId1 <= 8888 && _tokenId2 > 0 && _tokenId2 <= 8888, "Token ID invalid");
        require(colorBurn, "Burn for color is not active");
        require(n.ownerOf(_tokenId1) == msg.sender || n.ownerOf(_tokenId2) == msg.sender, "Not owner of at least 1 N token");

        uint256 rarest = getRarestTokenId(_tokenId1,_tokenId2);
    ...
    function getRarestTokenId(uint256 _tokenId1,uint256 _tokenId2) private returns(uint256) {
        uint256 score1 = getRarityScore(_tokenId1);
        uint256 score2 = getRarityScore(_tokenId2);
        if(score1 >= score2) {
            return _tokenId1;
        }
        return _tokenId2;
    ...
    function getRarityScore(uint256 _tokenId) private view returns(uint256) {
        uint[3] memory highestFrequency = getHighestFrequency(_tokenId);
        uint256 maxSequence = getMaxSequence(_tokenId);
        uint256 sum = getSum(_tokenId);
        uint256 sumScore = 0;
        if(sum <= 20 || sum > 70) {
            sumScore = 10;
        } else {
        ...
        uint256 score = (3*highestFrequency[0])+(2*highestFrequency[1])+highestFrequency[2]+(maxSequence*maxSequence)+sumScore;
        score = score * 2;
        return score;
    }

    function getMaxSequence(uint256 tokenId) private view returns(uint256) {
        uint256 count = 1;
        uint256 highest = 1;
    ...
    function getSum(uint256 tokenId) private view returns(uint256) {
    ...
    function getHighestFrequency(uint256 tokenId) private view returns(uint256[3] memory) {
        uint256[9] memory set = getSet(tokenId);
    ...
    function getSetSize(uint256 tokenId) private view returns(uint256) {
        uint256[9] memory set = getSet(tokenId);
        return set[1]+set[2]+set[3]+set[4]+set[5]+set[6]+set[7]+set[8];
    }

    function getSet(uint256 tokenId) private view returns(uint256[9] memory) {
        uint256[9] memory set;
    ...
    function getColor(uint256 tokenId) private pure returns(bool) {
        if(tokenId > 10000) {
            return true;
        }
        return false;
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Potenza #',
                        toString(tokenId),
                        '", "description": "First generative art project by Gavin Potenza", "image": "',
                        getBaseImageUri(),toString(tokenId),
                        '.jpg", "attributes": [{"trait_type": "Rarity Score", "value": "',
                        toString(getRarityScore(tokenId)),
    ...
    function getBaseImageUri() private view returns(string memory) {
        return baseImageUri;
    }

    function withdrawAll() external onlyOwner {
        uint256 reward1 = address(this).balance*45/100;
        uint256 reward2 = address(this).balance*10/100;
        payable(builders.archethect).transfer(reward1);
        payable(builders.potenza).transfer(reward1);
        payable(builders.nTreasury).transfer(reward2);
    }
