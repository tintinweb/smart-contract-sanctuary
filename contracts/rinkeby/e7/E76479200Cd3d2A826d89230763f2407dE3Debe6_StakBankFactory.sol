...
contract StakBankFactory is Ownable, Pausable {
    ...
    function registerStakBank(
        ...
        uint256 tokenIndex =
            getCreatedStakBanksLengthByToken(msg.sender, _stakingToken);
        bytes32 salt =
            keccak256(abi.encodePacked(msg.sender, _stakingToken, tokenIndex));
        assembly {
            pool := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IStakBank(pool).initialize(_name, _stakingToken, _rewardToken);
