    ...
    UUPSUpgradeable
{
    ...
    // initialize enumerable for the previously minted token of v2
    bool public enumerableInitialized;
    ...
    function initialize(
        ...
        // initialize enumerable for the previously minted token of v2
        enumerableInitialized = false;
        initializeTokensToEnumerable();
    }
    ...
    // --------------- add previous minted tokens to Enumerable datasource, can only called by owner once -------
    function initializeTokensToEnumerable() public onlyOwner {
        require(!enumerableInitialized, "already initialized!");
        for (uint32 tokenId = 1; tokenId <= 100; tokenId++) {
            _addTokenToAllTokensEnumeration(tokenId);
            address owner = ERC721Upgradeable.ownerOf(tokenId);
            // handle address with two or more tokens, should loop only once for most owners
            for (uint32 j = 0; j < 100; j++) {
                if (_ownedTokens[owner][j] == 0) {
                    _ownedTokens[owner][j] = tokenId;
                    _ownedTokensIndex[tokenId] = j;
                    break;
                }
            }
        }
        enumerableInitialized = true;
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(IERC165Upgradeable, ERC2981Upgradeable, ERC721Upgradeable)
        returns (bool)
    {
