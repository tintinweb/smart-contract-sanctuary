...
/*
   ...
   .:ddooolc;'..                          .            dKo;,'.
	...
	A narrative NFT adventure world ...
...
contract BadWolf is Context, ERC721Enumerable, ERC721Burnable, Ownable, ReentrancyGuard {
  address private duchessBetsy;
  address private jase;
  address private daweed;
  address private superMakeBelieve;

  string public BAD_WOLF_PROVENANCE = "provenance hash";
  string private _baseTokenURI;

  bool private _isSaleActive = false;
  bool private _baseURISet = false;
  ...
  uint public promoTokens = 30;
  uint[MAX_BW_SUPPLY] private badWolves;
  uint private nonce = 0;

  mapping (uint256 => uint256) private _badWolfBirthDate;

  constructor(string memory name, string memory symbol, address _duchessBetsy, address _jase, address _daweed, address _superMakeBelieve) ERC721(name, symbol) {
		duchessBetsy = _duchessBetsy;
		jase = _jase;
		daweed = _daweed;
		superMakeBelieve = _superMakeBelieve;
  }
  ...
  function mintBadWolf(uint _count) external payable nonReentrant() {
	...
	// can only mint 30 promo tokens max
  ...
  function tokenURI(uint tokenId) public view virtual override returns (string memory) {
    ...
    return _baseURISet == true ? string(abi.encodePacked(_baseTokenURI, uintToBytes(tokenId))) : "ipfs://QmeFn8XrwvkmmvVtAAHM1vxDT97bLpH6bvEWa1LvvUguUs";
  }
