...
contract SipherNFTSale is ISipherNFTSale, Whitelist {
    ...
    function getPublicSaleCurrentPrice()
        public
        view
        returns (uint256 currentPrice)
    {
        ...
        if (timestamp < _saleConfig.publicTime) {
            ...
            timestamp >= _saleConfig.publicTime &&
            timestamp < _saleConfig.publicEndTime
        ) {
            uint256 i = 0;
            while (
                (_saleConfig.publicTime + i * REDUCE_PRICE_INTERVAL) <=
                timestamp &&
                i < 17
            ) {
                i++;
    ...
    function updateSaleConfigTime(
        ...
        require(
            _publicTime >= _saleConfig.publicTime,
            "SipherNFTSale: Invalid sale time input"
        );
