...
contract Leveling is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function mint(uint num) public nonReentrant
    {
    ...
    function mintReserved() external nonReentrant onlyOwner
    {
        require(mintIndex < MAX_RESERVED, "Can only reserve a max of 20 tokens");
        
        for (uint256 i = 0; i < MAX_RESERVED; i++) {
            uint256 index = i+mintIndex;
            _safeMint(msg.sender, index);
            _setTokenValue(index, STARTING_LVL);
        }

        mintIndex += MAX_RESERVED;
    }
