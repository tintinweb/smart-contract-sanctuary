...
contract ERC1155CollectionImplementation is ERC1155Upgradeable, ERC1155CollectionBase, AdminControlUpgradeable
{
     ...
     * Initializer
     */
    function initialize(uint16 tokenMax_, uint256 tokenPrice_, uint16 transactionLimit_, uint16 purchaseLimit_, uint256 presaleTokenPrice_, uint16 presalePurchaseLimit_, address signingAddress_) public initializer {
        __ERC1155_init("");
        __Ownable_init();
        _initialize(tokenMax_, tokenPrice_, transactionLimit_, purchaseLimit_, presaleTokenPrice_, presalePurchaseLimit_, signingAddress_);
    }

    /**
     * @dev See {IERC165-supportsInterface}.
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC1155Upgradeable, AdminControlUpgradeable) returns (bool) {
        return
...
abstract contract ERC1155CollectionBase is CollectionBase, IERC1155Collection {
     ...
     * Initializer
     */
    function _initialize(uint16 tokenMax_, uint256 tokenPrice_, uint16 transactionLimit_, uint16 purchaseLimit_, uint256 presaleTokenPrice_, uint16 presalePurchaseLimit_, address signingAddress_) internal {
    ...
    function purchase(uint16 amount, bytes32 message, bytes calldata signature, string calldata nonce) external virtual override payable {
        ...
        if (block.timestamp - startTime < presaleInterval) {
            require(amount <= (presalePurchaseLimit - balance), "Too many requested");
    ...
    function state() external override view returns (CollectionState memory) {
        return CollectionState(tokenMax, tokenRemaining(), tokenPrice, transactionLimit, purchaseLimit, presaleTokenPrice, presalePurchaseLimit, uint16(balanceOf(msg.sender)), active, startTime, endTime, presaleInterval);
    }
    ...
    function _validateTokenTransferability(address from) internal view {
        require(tokenRemaining() == 0 || (active && block.timestamp >= endTime) || from == address(0), "Transfer locked until sale ends");
    }
