...
contract TradeFloor is Context, AccessControl, ERC1155PresetMinterPauser {
  ...
  function safeTransferFrom(
    ...
    if ((tokenId >> 64) == 0) _relinkOwner(from, to, uint64(tokenId));
  ...
  function burnBatch(
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      ...
      if ((tokenId >> 64) == 0) {
        ...
        IMinterCallback(minter).onBurn(account, tokenId, values[i]);
      }
    }
    // Unstake if required
    if (numStakes > 0) {
      ...
      for (uint256 i = 0; i < tokenIds.length; ++i) {
        uint256 tokenId = tokenIds[i];
        if ((tokenId >> 64) == 0) unstakeIds[--numStakes] = tokenId;
      }
      _unstake(unstakeIds);
    }
  }
  ...
  function _onTokensReceived(
    ...
    _mintBatch(from, tokenIds, amounts, '');
  }

  /**
   * @dev Transfer ownership of the SFT back to the user
   */
  function _unstake(uint256[] memory tokenIds) private {
