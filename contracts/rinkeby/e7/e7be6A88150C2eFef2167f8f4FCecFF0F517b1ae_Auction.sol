...
contract Auction{
    ...
    function End_Auction(string memory auction_id,string memory property_id) external isAuctionNotSuspended returns(bool,uint,uint){
    ...
    public isAuctionNotSuspended onlyOwner isAuctionEnded(auctionID){
        ...
        claimBidTokens[BidTokenAddress][auctionID][Bid_Token_Claimers[i]] = claim_amount[i]*(10**18);
        }
    }
    ...
    function CheckBidderIdentity(string calldata auctionID,address Bidder_Address,string memory property_id) external isAuctionEnded(auctionID) returns(bool){
        require(Auction_Success[auctionID],"Auction Failed");
        require(SLC_contract_address[msg.sender][property_id],"Check Bidder Identity Error");
        address[] memory temp = auction[auctionID].bidders;
        uint i;
        for(i=0;i<temp.length;i++){
            if(temp[i] == Bidder_Address)
