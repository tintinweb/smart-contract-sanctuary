...
contract MyGovernor is Governor, GovernorCompatibilityBravo, GovernorVotesComp, GovernorTimelockCompound {
    constructor(ERC20VotesComp _token, ICompoundTimelock _timelock)
        Governor("MyGovernor") // initializes the governor contract with a name
        GovernorVotesComp(_token) // sets our token as the votes manager
        GovernorTimelockCompound(_timelock) // sets a coumpound style Timelock for our contract
    {}

    // Sets the amount of time a proposal needs to wait after creation to become active and allow voting
		function votingDelay() public pure override returns (uint256) {
        return 1; // 1 block
    }

		// Sets the voting period for our proposals
    function votingPeriod() public pure override returns (uint256) {
        return 45818; // 1 week
    }
		
		// Sets the amount of votes need for the proposal to succeded
    function quorum(uint256 blockNumber) public pure override returns (uint256) {
        return 4;
    }

		// Sets the amount of votes a proposer needs to have in order to create a proposal
