...
contract JJCatNotice is ERC721URIStorage, Pausable, Ownable {
    ...
    constructor() ERC721("JJCatNotice", "JJCN") {}

    function mint(address user, uint256 tokenId, string memory tokenMessage)
    public onlyOwner
    returns (uint256)

    {
        _mint(user, tokenId);
        _setTokenURI(tokenId, string(abi.encodePacked('data:application/json;base64,', tokenMessage)));
        return tokenId;
    }
