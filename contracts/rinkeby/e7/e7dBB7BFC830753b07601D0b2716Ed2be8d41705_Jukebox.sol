...
contract Jukebox is Ownable, IJukebox, IERC721Receiver {
    event OrderCreated(
        Order order,
        uint256 maxFills,
    ...
    error JukeboxMissingMinterRoleError();
        ...
        returns (bytes32 salt)
    {
        ...
        if (!order.erc20.hasRole(order.erc20.MINTER_ROLE(), address(this))) {
            revert JukeboxMissingMinterRoleError();
        }
        salt = hashOrder(order);
        if (orders[salt] != 0) {
            revert OrderAlreadyExistsError();
        }

        // Add new order
        orders[salt] = maxFills;
        emit OrderCreated(order, maxFills, address(order.erc20), salt);
    }
    ...
    function onERC721Received(
        ...
        if (order.expirationTime <= block.timestamp || orders[salt] == 0) {
            revert InvalidOrderError();
        }
        unchecked {
            // Checked above that this shouldn't underflow
            orders[salt] -= 1;
        }
    ...
    /// @param order Struct entailing the order to be filled
