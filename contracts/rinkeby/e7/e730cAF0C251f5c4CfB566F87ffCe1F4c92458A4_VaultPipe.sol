...
contract Pipe is Vault {
    ...
    function depositFundsIntoVault() public {
        ...
        address underlyingTokenAddress = ISuperToken(acceptedToken).getUnderlyingToken();

        // approve token spend
        ISuperToken(underlyingTokenAddress).approve(_vaultAddress(), amount);
    ...
    function _withdraw(int96 _flowRate, address _user) internal returns (uint256) {
        ...
        if (availableVaultWithdrawAmount > 0) {
            ...
            // approve vault token spend
            ISuperToken(_vaultAddress()).approve(_vaultAddress(), availableVaultWithdrawAmount);
