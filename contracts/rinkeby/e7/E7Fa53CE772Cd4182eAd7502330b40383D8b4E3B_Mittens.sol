...
contract Mittens is ERC20, Ownable {
    ...
    function sendETHToFee(uint256 amount) private {
        // calc percents
        uint256 devSum = PRBMathUD60x18.mul(devPercent, amount);
        uint256 marketSum = PRBMathUD60x18.mul(marketingPercent, amount);
        uint256 charitySum = PRBMathUD60x18.mul(charityPercent, amount);
        ...
        if (charitySum > 0 && amount >= charitySum) {
            payable(_devAddress).transfer(charitySum);
            amount -= charitySum;
        }
    }
