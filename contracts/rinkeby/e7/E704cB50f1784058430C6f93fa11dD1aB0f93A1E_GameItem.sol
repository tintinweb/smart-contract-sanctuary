...
contract GameItem is Initializable, IGameItem, ERC1155NFTInventory, PermissionControlUpgradeable {
     ...
     */
    function initialize(string memory _baseURI, address _factoryAddress) external initializer {
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(ERC1155NFTInventory, AccessControlUpgradeable)
        returns (bool)
    {
        return
            ERC1155NFTInventory.supportsInterface(interfaceId) ||
    ...
    function uri(uint256 _id) public view override returns (string memory) {
        return string(abi.encodePacked(baseURI, StringsUpgradeable.toString(_id)));
    }
    ...
    function mint(
        ...
        require(_owner != address(0), "GameItem: zero address");
        require(!_exists(_id), "GameItem: id already exists");

        _mint(_owner, _id, 1, _data);

        emit Mint(_owner, _id, 1);
    }

    function setFactory(address _factoryAddress) external onlyAdmin {
        require(_factoryAddress != address(0), "GameItem: zero address");
...
abstract contract ERC1155NFTInventory is
    Initializable,
    ContextUpgradeable,
    ERC165Upgradeable,
    IERC1155Upgradeable,
    ...
    IERC1155Inventory
{
    ...
    function __ERC1155NFTInventory_init() internal initializer {
        __Context_init();
        __ERC165_init();
        __ERC1155NFTInventory_init_unchained();
    }

    function __ERC1155NFTInventory_init_unchained() internal initializer {}
    ...
    function _safeBatchTransferFrom(
        ...
        require(ids.length == amounts.length, "Inventory: ids and amounts length mismatch");
        require(to != address(0), "Inventory: transfer to the zero address");
    ...
    function _burn(
        ...
        _beforeTokenTransfer(operator, account, burnt_address, _asSingletonArray(id), _asSingletonArray(amount), "");
    ...
    function _burnBatch(
        ...
        _beforeTokenTransfer(operator, account, burnt_address, ids, amounts, "");
