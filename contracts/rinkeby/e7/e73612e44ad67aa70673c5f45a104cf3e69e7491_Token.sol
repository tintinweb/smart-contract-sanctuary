...
contract Whitelist is ValidAddress, AccessControl{
...
/**
 * @title  Token contract based on ERC20 protocol with extension of pausable and whitelisted modifiers
 */
...
contract Token is ERC20Pausable, ERC20Capped, ERC20Burnable, Whitelist {

  
    event logPayment(address indexed paymentAddress, uint amount, uint date);
    event LogControllerSet(address user);

    /**
    * @notice To keep track of profit payments which were issued due to investment in this token
     */
    struct Payment {
    ...
    Payment[] public payments;  // list of profits payments


    /**
     * @notice Constructor of the token contract
     * @param name The complete name of the token:
     * @param symbol The abbreviation of the token, to be searched for on exchange
     * @param account account which will be set as an admin
     * @param cap The max cap of the token supply
     */
    constructor(
        string memory name,
    ...
    /**
    * @notice retrieve whitelisted status of investor from whitelist location and return its status
    * @param user for which whitelisted status is being checked
     */
    modifier onlyWhitelisted(address user) {

        require(isWhitelisted(user), "Token:onlyWhitelisted - user is not whitelisted");
        _;
    }

    // modifier onlyAdmin() {

    //     require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Token:onlyAdmin - user is not an admin");
    //     _;
    // }

    // function setController(address user) public onlyAdmin() {

    //     require(user != address(0), "Token:setController - User account can't be 0");

    //     grantRole(CONTROLLER_ROLE, user);
    //     LogControllerSet(user);
    // }


    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual override(ERC20Pausable, ERC20, ERC20Capped) {
        super._beforeTokenTransfer(from, to, amount);
    }

    /**
    * @notice register profit payment when such is executed
    * @param paymentAddress profit payment contract address
    * @param amount total amount of profits sent to profit contract
     */
    function registerPayment(address paymentAddress, uint amount) public isController()  {
        ...
        payments[index].date = block.timestamp;
        payments[index].amount = amount;
        payments[index].paymentContractAddress = paymentAddress;
        emit logPayment(paymentAddress, amount, block.timestamp);
    }
    ...
    function mint(address to, uint256 amount) public isController() whenNotPaused()  onlyWhitelisted(to) returns (bool) {       
        require(to != address(0), "Token:mint - Recipient address can't be 0");        
        require(amount !=0, "Token:mint - Amount to mint can't be 0");
        _mint(to, amount);
        return true;
    }

    /**
    * @notice to return number of profit payments
    * @return number of profit payments
     */
    ...
    /**
     * @notice Pausable allowing on transfer to whitelisted users.
    ...
    function transfer(address to, uint256 value) 
        ...
        returns (bool)
    {
        // require(!this.paused(), "Token-Transfer : The token is paused.");
    ...
    /// @notice Overwrite parent implementation to add locked verification and notSelf modifiers
    function decreaseAllowance(address spender, uint subtractedValue)   override public
        onlyWhitelisted(spender)
        returns (bool)  
    {
