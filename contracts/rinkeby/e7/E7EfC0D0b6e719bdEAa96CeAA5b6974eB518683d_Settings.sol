...
interface ISettings {
    ...
    function assetWhitelisted(address asset) external returns (bool);
}
...
contract Settings is Ownable, ISettings {
    ...
    /// @notice the mapping which shows the asset whitelisted
    mapping(address => bool) public override assetWhitelisted;
    ...
    event AssetWhitelisted(address _asset, bool _whitelisted);
    ...
    function whitelistAsset(address asset) external onlyOwner {
        require(asset != address(0), "asset token address cannot be 0");
        require(!assetWhitelisted[asset], "already whitelisted");

        emit AssetWhitelisted(asset, true);

        assetWhitelisted[asset] = true;
    }

    function blacklistAsset(address asset) external onlyOwner {
        require(asset != address(0), "asset token address cannot be 0");
        require(assetWhitelisted[asset], "was not whitelisted");

        emit AssetWhitelisted(asset, false);

        assetWhitelisted[asset] = false;
    }
}
