...
contract TestERC20 is Ownable, ERC20 {
    ...
    function setContestStartDate(uint startDate) external onlyOwner {
        contestsStartDate = startDate;
    }

    function setContestDuration(uint _contestsDuration) external onlyOwner {
    ...
    function getTokens() external payable {
        require(block.timestamp > contestsStartDate + contestsDuration);
    ...
    function availableToClaim(address account) public view returns(bool) {
        ...
        if ((lastClaim[account] - contestsStartDate) / 1 days < (block.timestamp - contestsStartDate) / 1 days){
            return true;
        }
        return false;
    }
