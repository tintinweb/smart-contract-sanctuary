...
contract MoreOrLessVote is ERC721Enumerable, Ownable {
    ...
    function getMetadata(uint256 tokenId) public view returns (string memory) {
        string memory metadata = string(abi.encodePacked(
            voteInfos[tokenId].imageMetadata,
            ',',MoreOrLessArt._wrapTrait("Token Vote", votesEnumerated[voteInfos[tokenId].vote]),
    ...
    function _generateImage(uint256 mintNum) private returns (string memory) {
        uint256 whichShape = MoreOrLessArt.seededRandom(0, 4, seeds[msg.sender], voteInfos[mintNum].nonce);
        voteInfos[mintNum].nonce++;
        uint numRects = MoreOrLessArt.seededRandom(1,10, seeds[msg.sender], voteInfos[mintNum].nonce);
        voteInfos[mintNum].nonce++;
        uint numCircles = MoreOrLessArt.seededRandom(1,10, seeds[msg.sender], voteInfos[mintNum].nonce);
        voteInfos[mintNum].nonce++;
        uint numTriangles = MoreOrLessArt.seededRandom(1,10, seeds[msg.sender], voteInfos[mintNum].nonce);
        voteInfos[mintNum].nonce++;
        uint numLines = MoreOrLessArt.seededRandom(1,10,seeds[msg.sender], voteInfos[mintNum].nonce);
        voteInfos[mintNum].nonce++;
        ...
        if (whichShape == 0) {
            voteInfos[mintNum].imageMetadata = string(abi.encodePacked(
                voteInfos[mintNum].imageMetadata,
...

{
  ...
  "libraries": {
    "contracts/MoreOrLessArt.sol": {
      "MoreOrLessArt": "0x9413ee5fe7974ddaf3f63d26806d928578a5405a"
    }
  }
}