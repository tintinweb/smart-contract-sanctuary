...
contract GameToken is IGameToken, ERC20, Ownable {
  ...
  constructor() ERC20('GAME_TOKEN', 'GT') {}
  ...
  /**
   * mints $GT to a recipient
   * @param to the recipient of the $GT
   * @param amount the amount of $GT to mint
   */
  ...
  /**
   * burns $GT from a holder
   * @param from the holder of the $GT
   * @param amount the amount of $GT to burn
   */
  ...
  function transferFrom(
    ...
    address recipient,
    uint256 amount
  ) public virtual override(ERC20, IGameToken) disallowIfStateIsChanging returns (bool) {
    // NICE TRY MOUSE demon
    ...
    // If it's not an admin entity (game contract, temple, etc)
    // The entity will need to be given permission to transfer these funds
    // For instance, someone can't just make a contract and siphon $GT from every account
