...
contract DevVested is Ownable {
  ...
  constructor(address _beneficiary, uint256 _start, uint256 _cliff, uint256 _duration, bool _revocable) public {
    require(_beneficiary != address(0), "no address allowed");
  ...
  function release(IERC20 token) public {
    require(msg.sender == beneficiary, "Only beneficiary can call release");

    uint256 unreleased = releasableAmount(token);

    require(unreleased > 0);

    released[token] = released[token] + unreleased;
