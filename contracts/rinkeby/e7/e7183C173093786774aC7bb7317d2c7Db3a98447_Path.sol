...
/**
 Path Dao
 */
contract Path is ERC721URIStorage, VRFConsumerBase, Ownable {
  ...
  uint16 public MAX_WHITELIST_MINT = 3;

  string public baseURI;
  string public baseContractURI;
  address[] private whitelistedAddress;
  mapping(address => uint256) public whitelistMinted;
  mapping(uint256 => uint256) public tokenIdAttr;
  ...
  event PathBuilt(uint256 indexed tokenId, uint256 attr, address indexed owner);
  event Transformed(uint256 indexed tokenId, uint256 attr, string sentence, address indexed owner);
  ...
  function whitelistMintPath(uint256 quantity) public payable {
    ...
    for (uint256 i = 0; i < quantity; i++) {
      ...
      tokenIdAttr[mintIndex] = uint256(keccak256(abi.encode(randomNumber, mintIndex + block.number, block.timestamp, msg.sender))) % (10 ** 32);
  ...
  function getPathAttr(uint256 tokenId) public view returns (uint256) {
  ...
  function getRemainingTransforms(uint256 _tokenId) public view returns (uint256) {
    return remainingTransforms[_tokenId];
  }

  function getNumberTransforms(uint256 _tokenId) public view returns (uint256) {
    return transformNumber[_tokenId];
  }

  function getSentence(uint256 _tokenId) public view returns (string memory) {
    return tokenIdSentence[_tokenId];
  }
}
