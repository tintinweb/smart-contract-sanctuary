...
contract ShieldMining is IShieldMining, OwnableUpgradeable, ReentrancyGuard, AbstractDependant {
    ...
    mapping(address => EnumerableSet.UintSet) internal lastBlocksWithReward;
    ...
    function rewardPerToken(address _policyBook) public view override returns (uint256) {
        ...
        uint256 accumulatedReward =
            blocksWithRewardsPassed(_policyBook)
                .mul(getRewardPerBlock(_policyBook))
                .mul(DECIMALS18)
                .div(totalPoolStaked);
    ...
    function fillShieldMining(
        ...
        uint256 _blocksAmount = _duration.mul(BLOCKS_PER_DAY);
    ...
    function recoverNonLockedRewardTokens(address _policyBook) public onlyOwner {
        ...
        uint256 tokenBalance = shieldMiningInfo[_policyBook].rewardsToken.balanceOf(address(this));
        ...
        )
    {
        ...
        _rewardPerBlock = getRewardPerBlock(_policyBook);
    ...
    function getDepositList(
        ...
        for (uint256 i = _offset; i < to; i++) {
            ...
            _depositsList[i].rewardPerBlock = smd.rewardPerBlock;
        ...
        returns (uint256 _lastBlockWithReward)
    {
        ...
        _lastBlockWithReward = _startingBlock.add(_blocksAmount).sub(1);
        ...
        lastBlocksWithReward[_policyBook].add(_lastBlockWithReward);
    }
    ...
    function _updateReward(
        ...
        if (block.number > shieldMiningInfo[_policyBook].nearestLastBlocksWithReward) {
            shieldMiningInfo[_policyBook].lastUpdateBlock = shieldMiningInfo[_policyBook]
                .nearestLastBlocksWithReward;
        }
    ...
    function getRewardPerBlock(address _policyBook) public view returns (uint256 _rewardPerBlock) {
        uint256 _lastUpdateBlock = shieldMiningInfo[_policyBook].lastUpdateBlock;

        for (uint256 i = 0; i < lastBlocksWithReward[_policyBook].length(); i++) {
            uint256 _lastBlockWithReward = lastBlocksWithReward[_policyBook].at(i);
            if (_lastBlockWithReward > _lastUpdateBlock) {
    ...
    function _updateNearestLastBlocksWithReward(address _policyBook) internal {
        ...
        for (uint256 i = 0; i < lastBlocksWithReward[_policyBook].length(); i++) {
            _lastBlock = lastBlocksWithReward[_policyBook].at(i);
            if (_lastBlock <= _nearestLastBlocksWithReward) {
                _nearestLastBlocksWithReward = _lastBlock;
                if (_lastUpdateBlock >= _nearestLastBlocksWithReward) {
                    _nearestLastBlocksWithReward = _lastBlockWithReward;
                }
            }
        }
