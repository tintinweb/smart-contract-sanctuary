...
contract NFT is ERC721, ERC721URIStorage  {
    ...
    function mint(address _recipient, string memory _uri, uint256 _price)
        public
        returns (uint256)
    {
        ...
        tokenCreators[newId] = msg.sender;
        
        setTokenPrice(newId, _price);
        _tokenIds.increment();
    ...
    function setTokenPrice(uint256 _tokenId, uint256 _price)
        internal
    {
        require(tokenCreators[_tokenId] == msg.sender, "You are not the owner of this asset.");
        tokenPrices[_tokenId] = _price;
        
    }
    
    function getTokenPrice(uint _tokenId)
        public
        view
        returns (uint)
    {
    ...
    {
        require(_exists(_tokenId), "This asset has not been minted yet.");
        require(msg.value == tokenPrices[_tokenId], "Transfer the required price of the asset.");
        payable(tokenCreators[_tokenId]).transfer(msg.value);
        _transfer(tokenCreators[_tokenId], msg.sender, _tokenId);
