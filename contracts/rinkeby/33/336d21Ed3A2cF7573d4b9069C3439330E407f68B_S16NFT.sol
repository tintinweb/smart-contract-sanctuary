...
contract S16NFT is ERC1155, AccessControl, Ownable {
    ...
    uint256 internal tokenCap = 256;
    uint256 internal editionCap = 16;
    ...
    function configureS16Dist(address _s16DistAddress) external onlyOwner {
        require(_s16DistAddress != address(0), "S16NFT: null address");
    ...
    function mintEditions(
        ...
        // TODO will remove below line if not needed admin minting
        // require(hasRole(MINTER_ROLE, _msgSender()) || hasRole(S16DIST_ROLE, s16DistAddress), "Caller is not a minter");
        //this check will ensure only the S16Distributor contract can call this function
        require(hasRole(S16DIST_ROLE, s16DistAddress), "Caller is not a minter");
        require(_to != address(0), "S16NFT: mint to the zero address");
        require(_tokenIds.length == _quantity.length, "S16NFT: tokenIds length must be equal to quantity");
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(
                isNFTMinted[_tokenIds[i]] == false,
                "S16NFT: token already minted"
            );
            require(
                _tokenIds[i] <= tokenCap,
                "S16NFT: Cap reached, maximum 256 NFT mints possible"
            );
            require(_quantity[i] > 0, "S16NFT: quantity must be greater than 0");
            require(_quantity[i] + mintedEditionsToken[_tokenIds[i]] <= editionCap, "S16NFT: quantity exceed limit");
            
            mintedEditionsToken[_tokenIds[i]] += _quantity[i];

            if (mintedEditionsToken[_tokenIds[i]] == editionCap) {
                isNFTMinted[_tokenIds[i]] = true;
            } else {
                isNFTMinted[_tokenIds[i]] = false;
            }
        }

        _mintBatch(_to, _tokenIds, _quantity, "");
