...
contract OwnixMarket is ERC1155Holder, Ownable, Pausable, IOwnixMarket {
   ...
   */
  function listNFT(uint256 _tokenId, uint256 _amount, uint256 _reservePrice, uint256 _duration) external whenNotPaused() {
   ...
   */
  function unlistNFT(uint256 _tokenId, uint256 _index) external whenNotPaused() {
   ...
   */
  function changeReservePrice(uint256 _tokenId, uint256 _index, uint256 _reservePrice) external whenNotPaused() {
   ...
   */
  function placeBid(uint256 _tokenId, uint256 _index, uint256 _price) external whenNotPaused() {
  ...
  */
  function finishAuction(uint256 _tokenId, uint256 _index) external whenNotPaused() {
    ...
    if (royaltiesPerMillion > 0 && ownixNFTToken.getFirstTimeOwner(auction.tokenId) != auction.nftOwner) {
      ...
      require(
        ownixToken.transfer(ownixNFTToken.getFirstTimeOwner(auction.tokenId), royaltiesAmount),
        "Transfering the royalties failed"
      );
    }
