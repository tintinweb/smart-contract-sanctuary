...
contract Kinary is ChainlinkClient {
    ...
    //TODO create harvest and withdraw
    ...
    function addUpdateAllowedTokens(address token, uint totalTokenSupply, uint rate) public returns (bool) {
        require(isOwner(msg.sender));
    ...
    function addRate(address token, uint rate) 
    public {
        require(isOwner(msg.sender));
        if (allowedTokens[token] != 0) {
            rates[token] = rate;
    ...
    function addDeposit(address payable owner, uint _amount, uint timePeriod, address token) 
    public {
        if (allowedTokens[token] != 0) {
            //TODO dev only
            require(timePeriod >= 0 && timePeriod <= 360);
            IERC20(token).transferFrom(msg.sender, address(this), _amount);
            deposits.push(
           Deposit({owner: owner, amount: _amount, depositTimestamp: now, timePeriod: timePeriod, harvested: 0, withdrawed: false, token: token}));
    ...
    function harvest(uint256 _depositIndex) public returns (bool) {
        ...
        uint256 rate = getRate(deposit.token) * (deposit.timePeriod / 12);
        if (rate == 0) return false;
        uint lockPeriod = ((deposit.timePeriod.mul(4)) * 1 weeks);
        uint destinationPeriod = deposit.depositTimestamp + lockPeriod;
        require(now > destinationPeriod);
        uint progress = (now) / destinationPeriod;
        uint amount = (deposit.amount.mul(rate).mul(progress)) - deposit.harvested;
     ...
     function withdraw(uint _amount, uint256 _depositIndex) public returns (bool) {
        if (_depositIndex == 0) return false;
        require(deposits[_depositIndex].withdrawed == false);
        Deposit storage deposit = deposits[_depositIndex];
        address token = deposit.token;
        require(deposit.owner == msg.sender);
        uint lockPeriod = ((deposit.timePeriod * 4) * 1 weeks);
        uint destinationPeriod = deposit.depositTimestamp + lockPeriod;
        require(now > destinationPeriod);
        IERC20(deposit.token).transferFrom(address(this), msg.sender, _amount);
...
//assemble the given address bytecode. If bytecode exists then the _addr is a contract.
  ...
  //function that is called when transaction target is an address
    ...
    function transferFromContract(address _to, uint _value) private returns (bool success) {
        ...
        balances[central] = balanceOf(central).sub(_value);
        balances[_to] = balanceOf(_to).add(_value);
