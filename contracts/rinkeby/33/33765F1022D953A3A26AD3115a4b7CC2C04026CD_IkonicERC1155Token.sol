...
contract IkonicERC1155Token is ERC1155Base {
    ...
    */
    ...
    function mint(uint256 _tokenId, Fee[] memory _fees, uint256 _supply, string memory _uri, uint256 _price) external {
        _mint(_tokenId, _fees, _supply, _uri, _price);
    }
...
abstract contract HasPrice {

    constructor() {
        // _registerInterface(_INTERFACE_ID_HAS_PRICE);
    }

    function getPrice(uint256 id) public virtual view returns (uint256);
}
...
abstract contract ERC1155Base is
    ...
    function getFeeRecipients(uint256 id) public override view returns (address[] memory) {
    ...
    function getPrice(uint256 id) public view override returns (uint256) {
        return price[id];
    }

    // Creates a new token type and assings _initialSupply to minter
    function _mint(uint256 _id, Fee[] memory _fees, uint256 _supply, string memory _uri, uint256 _price) internal {
        require(
