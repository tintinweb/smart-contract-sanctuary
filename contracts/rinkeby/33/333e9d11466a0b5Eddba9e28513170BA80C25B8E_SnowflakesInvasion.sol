...
contract SnowflakesInvasion is IERC1155Receiver {
  ...
  function getMovementPoints(address player,uint256 tokenId) public returns(uint8 x){
  ...
  function canMoveToken(address player,uint256 tokenId) public returns(bool){
    uint256 lastMovement = players[player].lastMovement[tokenId];
    uint256 blocks = 0;
    if(lastMovement > 0){
