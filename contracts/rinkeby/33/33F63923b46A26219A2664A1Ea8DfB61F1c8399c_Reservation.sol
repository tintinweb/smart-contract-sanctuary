...
contract Reservation is IReservation, AccessRestriction {
    ...
    function isTimeAvailable(bytes32 unitKey, uint256 time)
        public
        view
        returns (bool)
    {
        ...
        for (i = 0; i < reservationList.length; i++) {
            if (
                reservationStructs[reservationList[i]].unitKey == unitKey &&
    ...
    function createReservation(
        ...
        require(isTimeAvailable(unitKey, startTime), "TIME_NOT_AVAILABLE");
