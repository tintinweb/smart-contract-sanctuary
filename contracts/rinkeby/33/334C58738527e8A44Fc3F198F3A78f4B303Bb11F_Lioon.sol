...
contract Lioon is ILioon, ERC721Enumerable, Ownable, Pausable {
    ...
    // mint price
    uint256 public constant MINT_PRICE = .0420 ether;
    ...
    mapping(uint256 => bool) public _takenZebraIds;
    mapping(uint256 => bool) public _takenLionIds;
    ...
    */
    function mint(uint256 amount, bool stake) external payable whenNotPaused {
        ...
        console.log("past step 1", amount, stake);

        uint256 totalStripesCost = 0;
        uint16[] memory tokenIds = stake ? new uint16[](amount) : new uint16[](0);
        uint256 seed;

        console.log("past step 2");
        ...
        for (uint i = 0; i < amount; i++) {
            ...
            if (!stake || recipient != _msgSender()) {
                ...
                _safeMint(address(safari), minted);
                tokenIds[i] = minted;
            }
    ...
    */
    function generate(uint256 tokenId, uint256 seed) internal returns (TokenMetadata memory t) {
        ...
        uint256 random2 = random(seed + random1);

        newData.isLion = (random1 % 2 == 0);
