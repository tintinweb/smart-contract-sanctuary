...
contract TriallToken is ERC20 {
    uint256 constant TOTAL_SUPPLY = 175 * (10**6) * (10**18);
...
contract Vesting is EIP712, Ownable {
    ...
    TriallToken private _rewardToken;
    ...
    {
        uint256 remaining = _totalSupply
        .sub(convertToToken(_totalDeposited))
        .mul(_tokenPrice)
        .div(10**18);

        if (deposited[_addr] == 0) {
            maxAvailAllocation = Math.min(_maxAllocation, remaining);
            minAvailAllocation = _minAllocation;
        } else {
            maxAvailAllocation = Math.min(
                _maxAllocation.sub(deposited[_addr]),
                remaining
            );
        ...
        returns (uint256 lockedBalance, uint256 unlockedBalance)
    {
        ...
        if (!_isVestingStarted() || block.timestamp < startDate) {
            return (tokenBalance, 0);
        }

        uint256 unlock = _calculateUnlock(_addr);
        return (tokenBalance.sub(unlock).sub(rewardsPaid[_addr]), unlock);
    }
    ...
    function initializeToken(uint256 tokenPrice_, uint256 totalSypply_)
        ...
        onlyOwner
    {
        require(!_initialized, "Is was initialized before");
        require(totalSypply_ > 0 && tokenPrice_ > 0, "Incorrect amount");

        _tokenPrice = tokenPrice_;
        _totalSupply = totalSypply_;
        _rewardToken.transferFrom(msg.sender, address(this), totalSypply_);
        _initialized = true;
    }
    ...
    function addDepositeAmount(
        ...
        uint256 remainingAllocation = _totalSupply.sub(
            convertToToken(_totalDeposited)
        );

        for (uint256 index = 0; index < _addrArr.length; index++) {
            remainingAllocation = remainingAllocation.sub(
                convertToToken(_amountArr[index]),
                "Not enough allocation"
            );
            deposited[_addrArr[index]] = deposited[_addrArr[index]].add(
                _amountArr[index]
            );
            _totalDeposited = _totalDeposited.add(_amountArr[index]);
        }
    ...
    function withdrawFunds() external onlyOwner {
        ...
        bool success = _depositToken.transfer(msg.sender, balance);
        require(success, "Failed transfer");

        uint256 unSoldToken = _totalSupply.sub(convertToToken(_totalDeposited));
    ...
    function harvest() external {
        ...
        bool success = _rewardToken.transfer(msg.sender, amountToTransfer);
        require(success, "Failed transfer");

        rewardsPaid[msg.sender] = rewardsPaid[msg.sender].add(amountToTransfer);
    ...
    function convertToToken(uint256 _amount) public view returns (uint256) {
        return _amount.mul(10**18).div(_tokenPrice);
    }

    function _calculateUnlock(address _addr) internal view returns (uint256) {
        uint256 tokenAmount = convertToToken(deposited[_addr]);
        uint256 initialUnlockAmount = tokenAmount.mul(_initialPercentage).div(
            10**20
        );

        uint256 passeMonth = Math.min(
            block.timestamp.sub(startDate).div(30 days),
            _countPeriodOfVesting
        );
        ...
        return
            tokenAmount
                ...
                .mul(passeMonth)
                .div(_countPeriodOfVesting)
                .add(initialUnlockAmount)
                .sub(rewardsPaid[_addr]);
    }
    ...
    function _isValidAmount(uint256 _amount) internal view returns (bool) {
        ...
        uint256 remainingAmount = Math.min(
            _maxAllocation.sub(depositAmount),
            convertToToken(_totalSupply).sub(_totalDeposited)
        );
        return
