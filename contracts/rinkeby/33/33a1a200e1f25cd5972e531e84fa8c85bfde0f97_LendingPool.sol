    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public priceDToken; // 2usd

    struct UserAssetInfo {
        uint256 totalDeposit; // How many tokens the lender has provided
        uint256 rewardDebt;
        //
        ...
        //   1. The pool's `accDTokenPerShare` (and `lastRewardBlock`) gets updated.
    ...
    function createPool(
        ...
        pool.isCreated = true;
        pool.borrowIndex = mantissaOne;
        pool.supplyIndex = mantissaOne;
        pool.accrualBlockNumber = block.number;

        emit PoolAdded(_tokenAddress, tokenDecimals);
    }

    function addNewDeed (address _address) internal {
        _addNewDeed(_address);
    }

    function _addNewDeed (address _address) private {
        deedInfo[_address].isValid = true;
    }

    function removeExpireDeed (address _address) internal {
        require(deedInfo[_address].isValid, "Coindeed: This deed doesn't exist");
        _removeExpireDeed(_address);
    }

    function _removeExpireDeed (address _address) private {
        ...
        returns (uint256)
    {
        ...
        if (getBlockNumber() > pool.accrualBlockNumber && totalSupply != 0) {
            ...
            uint256 dtokenReward = blockDelta.mul(pool.supplyIndex);
            uint256 dTokenRewardEx = getDtokenExchange(_token, dtokenReward);
            accDTokenPerShare = accDTokenPerShare.add(
                dTokenRewardEx.mul(1e18).div(totalSupply)
            );
        }
        return
            user.totalDeposit.mul(accDTokenPerShare).div(1e18).sub(
                user.rewardDebt
            );
    }
        ...
        nonReentrant
    {
        ...
        if (_amount > 0) {
            if (_tokenAddress == address(0)) {
                ...
                userAsset.totalDeposit = userAsset.totalDeposit.add(msg.value);
            } else {
                require(
                    pool.isCreated,
                    "Coindeed: this token is not supported"
                );
                userAsset.totalDeposit = userAsset.totalDeposit.add(_amount);
        ...
        userAsset.rewardDebt = userAsset.totalDeposit.mul(pool.accDTokenPerShare).div(1e18);
    ...
    function borrow(address _tokenAddress, uint256 _amount) external {
        require(deedInfo[msg.sender].isValid, "Coindeed: Only deed can borrow from pool");
        PoolInfo storage pool = poolInfo[_tokenAddress];
        ...
        // exchangeRate tokenBorrow and check totalColllateral
        ...
        deed.totalBorrow = deed.totalBorrow.add(amountEx);
        pool.totalBorrows = pool.totalBorrows.add(amountEx);
    ...
    function repay(address _tokenAddress, uint256 _amount)
        ...
        nonReentrant
    {
        require(deedInfo[msg.sender].isValid, "Coindeed: Only deed can repay token back to pool");
        require(deedInfo[msg.sender].totalBorrow > 0, "Coindeed: This deed doesn't have any loan from pool");
        PoolInfo storage pool = poolInfo[_tokenAddress];
        require(pool.isCreated, "Coindeed: this token is not supported");
        DeedInfo storage deed = deedInfo[msg.sender];
        // Re-caculate the accure interest
        ...
        deed.totalBorrow = result;
        require(deed.totalBorrow >= amountEx, "Coindeed: exceed borrow amount");
        ...
        deed.totalBorrow = deed.totalBorrow.sub(amountEx);
        pool.totalBorrows = pool.totalBorrows.sub(amountEx);
    ...
    function withdraw(address _tokenAddress, uint256 _amount) external {
        ...
        require(
            userAsset.totalDeposit >= _amount,
        ...
        if (_amount > 0) {
            if (_tokenAddress == address(0)) {
                userAsset.totalDeposit = userAsset.totalDeposit.sub(_amount);
    ...
    function updatePool(address _token) internal {
        ...
        if (dTokenReward > 0) {
            // exchange dTokenReward
            uint256 dTokenRewardEx = getDtokenExchange(_token, dTokenReward);
            // TODO: mint dToken for operator
            dToken.mint(address(this), dTokenRewardEx);
        ...
        returns (uint256)
    {
        ...
        uint256 rewardDebtEx = reward
            .mul(10**(POOL_DECIMALS.sub(pool.decimals)))
            .mul(tokenPrice)
            .div(10**pool.oracleDecimals);

        return
            rewardDebtEx.div(10**pool.oracleDecimals).div(priceDToken).mul(
                10**dToken.decimals()
            );
    }
        ...
        returns (uint256)
    {
        ...
        uint256 supplyRateMantissa = getSupplyRate(
            cashPrior,
            pool.totalBorrows,
            pool.totalReserves,
         ...
         *  simpleInteresSupplyFactor = supplyRate * blockDelta
         *  supplyIndexNew = simpleInteresSupplyFactor * supplyIndex + supplyIndex
         */

        vars.simpleInterestFactor = borrowRateMantissa.mul(vars.blockDelta);
        vars.interestAccumulated = vars.simpleInterestFactor.mul(
            pool.totalBorrows
        );
        vars.simpleInteresSupplyFactor = supplyRateMantissa.mul(
            vars.blockDelta
        );
...
interface ILendingPool{
    ...
    function _addNewDeed (address _address) external;

    function removeExpireDeed (address _address) external;

    function _removeExpireDeed (address _address) external;
     ...
     * @return The quantity of underlying tokens owned by this contract
     */
    function getCashPrior(address _token) external view returns (uint256);

    function borrowBalanceStoredexternal(address account, uint256 borrowIndex)
        external
        returns (uint8, uint256);

    function updatePool(address _token) external;
    ...
    function safeDTokenTransfer(address _to, uint256 _amount) external;
}
