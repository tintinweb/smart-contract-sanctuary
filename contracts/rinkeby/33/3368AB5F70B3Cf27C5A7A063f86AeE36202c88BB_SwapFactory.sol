...
contract SwapFactory is ISwapFactory, BaseSwapFactory {
    ...
    function deploy(IERC20 tokenA, IERC20 tokenB, uint256 amountMaxA, uint256 amountMaxB, uint256 amountMinA, uint256 amountMinB) public payable returns (Swap pool) {
        require(tokenA != tokenB, "Factory: not support same tokens");
        (IERC20 token1, IERC20 token2) = sortTokens(tokenA, tokenB);
        (uint256 amount1Max, uint256 amount2Max) = token1 == tokenA ? (amountMaxA, amountMaxB) : (amountMaxB, amountMaxA);
        (uint256 amount1Min, uint256 amount2Min) = token1 == tokenA ? (amountMinA, amountMinB) : (amountMinB, amountMinA);
        ...
        pool.deposit([amount1Max, amount2Max], [amount1Min, amount2Min]);
...
interface ISwap {
    ...
    ) external payable returns (uint256 failSupply, uint256[2] memory receivedAmounts);
}
