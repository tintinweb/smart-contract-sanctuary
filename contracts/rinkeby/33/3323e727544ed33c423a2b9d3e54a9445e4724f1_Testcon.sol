...
contract Testcon is Context, IERC20, Ownable {
    ...
    uint256 private _buybackAndTeamAndMarketingFee = 10;
    
    uint256 private _previousRedistributionFee = _redistributionFee;
    uint256 private _previousbuybackAndTeamAndMarketingFee = _buybackAndTeamAndMarketingFee;
    ...
    function openTrading() external onlyOwner() {
        buybackAndTeamFundEnabled = true;
        launchTime = block.timestamp;
        tradingOpen = true;
    }

    function removeAllFees() private {
        if (_redistributionFee == 0 && _buybackAndTeamAndMarketingFee == 0) return;
        _redistributionFee = 0;
        _buybackAndTeamAndMarketingFee = 0;
    }

    function restoreAllFees() private {
        _redistributionFee = 5;
        _buybackAndTeamAndMarketingFee = 10;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (sender == _uniswapV2Pair && recipient != address(_uniswapV2Router) && !_isExcludedFromFee[recipient]) { //a buy
            ...
            _buybackAndTeamAndMarketingFee = 6;
        }
        ...
        if (!inSwap && buybackAndTeamFundEnabled && recipient == _uniswapV2Pair) {
            ...
            if (contractETHBalance > uint256(1 * 10**16)) {//to do change
                transferFundsAndBuybackTokens(contractETHBalance.div(5));
            }
        }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        (uint256 tTransferAmount, uint256 tFee, uint256 tBuyBackAndTeamFees) = _getTValues(tAmount, _redistributionFee, _buybackAndTeamAndMarketingFee);
        uint256 currentRate =  _getRate();
