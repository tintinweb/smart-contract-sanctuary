...
contract TankNFT is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint8 public constant MAX_PRE_MULTI = 2;
    uint8 public constant MAX_PUBLIC_MULTI = 10;
    ...
    uint public MINTING_FEE = 0.01 * 10**18; //in wei
    ...
    function increaseMaxPublicId(uint16 _increase) external onlyOwner {
        require(MAX_PUBLIC_ID + _increase <= MAX_COUNT, "TankNFT: can not set more than MAX_COUNT");
        MAX_PUBLIC_ID += _increase;
    }
    ...
    function getHash(address this_, address sender_, string memory msg_, string memory count_) 
        public pure 
        returns (bytes32) {
        return keccak256(abi.encodePacked(this_, sender_, msg_, count_));
    }
    ...
    function freeClaim(uint8 v, bytes32 r, bytes32 s) external nonReentrant {
        ...
        require(freeClaims[_msgSender()] == 0, "Already claimed");

        bytes32 hash = getHash(address(this), _msgSender(), "freeClaim", "1");
        require(verifySig(hash, v, r, s), "Signer's sig does not match ");

        _publicTokenId += 1;
        freeClaims[_msgSender()] = 1;
        require(safeMint(_msgSender(), _publicTokenId), "TankNFT: minting failed");
    }
    ...
    function preMint(uint8 v, bytes32 r, bytes32 s, uint8 _count) external payable nonReentrant {
        ...
        bytes32 hash = getHash(address(this), _msgSender(), "preMint", toString(_count));
        require(verifySig(hash, v, r, s), "Signer's sig does not match ");
    ...
    function tokenURI(uint _tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Fotress-Arena NFT# ', toString(_tokenId), '", "attributes": ', attrs,', "description": "', description, '", "image": "', imgUrl, '"}'))));
