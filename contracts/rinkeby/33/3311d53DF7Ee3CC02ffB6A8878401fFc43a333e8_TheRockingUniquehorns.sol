...
contract TheRockingUniquehorns is ERC721Enumerable, Ownable {
    ...
    string _baseTokenURI = "https://s3.eu-central-1.amazonaws.com/api.rockinguniquehorns.com/public/";
    string private _contractURI = "https://rockinguniquehorns.com/contract/metadata";

    uint256 MAX_SUPPLY = 9990;

    // actual max + 1 so we dont have to check for equals
    ...
    uint256 public constant RETURN_PERIOD_START = 1636132504;   // FOR TESTING PURPOSES: November 5th 6:xxxx pm UTC
    ...
    mapping(uint256 => TRUData) public foundingBandMembers;
    ...
    function fire(uint256[] memory tokenIds) external nonReentrant {
        ...
        for(uint256 i; i < tokenIds.length; i++){
            ...
            if(ownerOf(tokenId) == msg.sender && foundingBandMembers[tokenId].refundable) {
                //Transfer back to TRU to pool
                _transfer(msg.sender, _returnPoolAddress, tokenId);
                _returnedCounter.increment();
                totalRefund += foundingBandMembers[tokenId].mintPrice;
            }
        }
    ...
    */
    ...
    function pauseWhitelist(bool state) external onlyOwner {
        _whitelistPaused = state;
    }
    ...
    */
    ...
    function isEligibleForRefund(uint256 tokenId) public view returns (bool) {
        return foundingBandMembers[tokenId].refundable;
    }

    function getRefundValue(uint256 tokenId) public view returns (uint256) {
        return foundingBandMembers[tokenId].mintPrice;
    }
    ...
    function _mintInternal(address to, uint256 amount, bool refundable) private {
        ...
        for(uint256 i; i < amount; i++){
            ...
            foundingBandMembers[id].discoveredBy = to;
            foundingBandMembers[id].mintPrice = _price;
            foundingBandMembers[id].refundable = refundable;
            if(refundable) {
