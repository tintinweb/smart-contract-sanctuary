...
contract solidSwing is owned {
    ...
    function getMyRebaseDip() public returns (bool)
    {
        ...
        if(currentPrice < benchMarkValue &&  sA > 0 && sellTime[msg.sender] >= snapTime - oneDay )
        {
            ...
            mintToken(msg.sender, (sA * rB * base / 100) / base);       // makes the transfers
            emit getMyRebaseDipEv(msg.sender, (sA * rB * base / 100) / base);  
        }
        return true;
    }

    function viewMyRebaseDip(address _user, uint test) public view returns (uint)
    {
        uint sA = sellAmount[_user];
if(test<1) return sA; 
if(test<2) return benchMarkValue - currentPrice; 
if(test<3) return sellTime[msg.sender] - (snapTime - oneDay);
        if(currentPrice < benchMarkValue &&  sA > 0 && sellTime[msg.sender] >= snapTime - oneDay)
        {
            uint rB = (benchMarkValue - currentPrice) * base / benchMarkValue;
if(test<4) return rB; 
            rB =  (sA * rB * base / 100) / base; 
if(test<5) return rB;
            //return rB;
        }
        return 555;
