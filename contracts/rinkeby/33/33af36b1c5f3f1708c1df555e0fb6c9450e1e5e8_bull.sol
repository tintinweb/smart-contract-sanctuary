...
contract bull is ERC721Enumerable, Ownable {
  ...
  mapping(address => uint8) public allocatefree;
  ...
  // public
   function mint(uint256 _mintAmount) public payable {
    ...
    function freeMint() external {
        require(totalSupply() + allocatefree[msg.sender] <= maxSupply, "Not enough tokens left to mint");
        for (uint256 i = 0; i < allocatefree[msg.sender]; i++) {
            uint mintIndex = randomIndex();
            _safeMint(msg.sender, mintIndex);
            allocatefree[msg.sender]--;
        }
    }
