...
contract Marketplace {
    ...
    public payable {
        Goods storage _g = BuyerBoard.board[msg.sender][_count];
        require(keccak256(abi.encodePacked(_g.name)) == keccak256(abi.encodePacked(_name)) && _g.flag == false);
        feedback(seller, _g.seller_addr, _g.price, _g.buyerAllow);
    ...
    public payable {
        Goods storage _g = BuyerBoard.board[msg.sender][_count];
        require((keccak256(abi.encodePacked(_g.name)) == keccak256(abi.encodePacked(_name))) && ((_g.flag == true) && (block.timestamp >= (_g.time + (10 days)))));
        _g.sellerAllow = true;
    ...
    public payable {
        ...
        require((keccak256(abi.encodePacked(_g.name)) == keccak256(abi.encodePacked(_name))) && ((_g.flag == true) || (block.timestamp >= (_g.time + (7 days)))));
