...
contract Items is ERC1155, Ownable, ERC1155Burnable  {
    ...
    address public tokenStorage = address(0x89e379F11913b8A5599dAbB6c59D13181608f886);
...
function openChest(uint256 _chestId,uint256 amount) public payable  returns(bool success)  {
     ...
     require(permissions[_chestId] != 0 );
    ...
    _mint(_msgSender(), permissions[_chestId], 1,"");

    permissions[_chestId] = 0;
    return true;
   }
    ...
    function setPermissions(uint256[] memory ids) public onlyOwner {
        for (uint256 i; i < ids.length; i++) {
         permissions.push(ids[i]);
        }
    }

    function changePermission(uint256 index,uint256 value) public onlyOwner {
