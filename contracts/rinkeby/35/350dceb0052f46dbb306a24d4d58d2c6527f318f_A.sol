...
contract A is ERC721A, Ownable, ReentrancyGuard {
    ...
    ) ERC721A("A", "A", _maxBatchSize, _collectionSize) {
        baseURI = initBaseURI;
        _signer = signer;
    ...
    modifier chekcSupply(uint256 amount) {
        _checkSupply(amount);
        _;
    }
    ...
    function reserve(address recipient, uint256 amount) external onlyOwner chekcSupply(amount){
    ...
    ) external payable onlyEOA chekcSupply(amount){
    ...
    function mint() external payable onlyEOA chekcSupply(1){
        require(
            !publicMinted[msg.sender],
            ": The wallet has already minted during public sale."
        );
