...
contract Hallows is ERC721, Ownable {
    ...
    constructor(string memory _notRevealedURI, address[] memory _team, uint256[] memory _shares) ERC721("Hallows", "HOL") { 
    ...
    function mint(uint numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint HOL");
        require(numberOfTokens > 0 && numberOfTokens <= maxPurchase, "Can only mint 20 tokens at a time");
        require(totalSupply().add(numberOfTokens) <= MAX - reserve, "Purchase would exceed max supply of HOL");
        require(msg.value >= price.mul(numberOfTokens), "Ether value sent is not correct");        
        for(uint i = 0; i < numberOfTokens; i++) {                  
            if (totalSupply() < MAX - reserve) {
                _tokenIds.increment();  
    ...
    function mintReserve(uint256 n) public onlyOwner {
        require(n > 0 && n <= reserve, "Purchase would exceed reserve supply of HOL");
        for (uint i = 0; i < n; i++) {
            if (totalSupply() < MAX) {
                _tokenIds.increment();
                uint mintIndex = totalSupply(); 
