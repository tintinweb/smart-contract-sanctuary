...
contract TestERC20 is Ownable, ERC20 {
    ...
    function addPrivilegedAccounts(address account, bool active) external onlyOwner {
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        if (block.timestamp > contestsStartDate && block.timestamp < contestsStartDate + contestsDuration) {
            require(privilegedAccounts[msg.sender] || privilegedAccounts[recipient], "Not allowed transfer");
        }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        if (block.timestamp > contestsStartDate && block.timestamp < contestsStartDate + contestsDuration) {
            require(privilegedAccounts[sender] || privilegedAccounts[recipient], "Not allowed transfer");
        }
