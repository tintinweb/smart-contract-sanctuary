...
contract POTMTraits is Ownable, IPOTMTraits {
  ...
  function drawSVG(uint256 tokenId) internal view returns (string memory) {
    ...
    IPOTM.CrewCaptain memory s = potm.getTokenTraits(tokenId);
    if (s.isCrew) {
    ...
    svgString = string(abi.encodePacked(
      drawTrait(traitData[6][s.body]),
      drawTrait(traitData[7][s.clothes]),
      drawTrait(traitData[8][s.face]),
      drawTrait(traitData[9][s.mouth]),
      drawTrait(traitData[10][s.eyes]),
      drawTrait(traitData[11][s.head])
    ));
  }
  ...
  function compileAttributes(uint256 tokenId) internal view returns (string memory) {
    ...
    if (s.isCrew) {
      ...
      traits = string(abi.encodePacked(
        attributeForTypeAndValue(_traitTypes[0], traitData[6][s.body].name),',',
        attributeForTypeAndValue(_traitTypes[1], traitData[7][s.clothes].name),',',
        attributeForTypeAndValue(_traitTypes[2], traitData[8][s.face].name),',',
        attributeForTypeAndValue(_traitTypes[3], traitData[9][s.mouth].name),',',
        attributeForTypeAndValue(_traitTypes[4], traitData[10][s.eyes].name),',',
        attributeForTypeAndValue(_traitTypes[5], traitData[11][s.head].name),','
      ));
    }
    ...
    return string(abi.encodePacked(
      ...
      tokenId <= potm.getPaidTokens() ? '"Gen 0"' : '"Gen 1"',
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    require(_msgSender() == address(potm), "hmmmm what doing?");
    IPOTM.CrewCaptain memory s = potm.getTokenTraits(tokenId);
    ...
    string memory metadata = string(abi.encodePacked(
      ...
      s.isCrew ? 'Crew #' : 'Captain #',
      tokenId.toString(),
      '", "description": "Thousands of Captains and their trusty Crew set sail on a voyage accross the metaverse. A treasure of $BOOTY awaits, but danger and rough seas lay ahead. All the metadata and images are generated and stored 100% on-chain. No IPFS. NO API. Just the Ethereum blockchain.", "image": "data:image/svg+xml;base64,',
