...
contract Alexxeca {
        ...
        for (uint256 i = 1; i <= loopLimit; i++) {
            if (
                ...
                if (users[_directRef].currentLevel >= _level) {
                    ...
                    lostEth[_directRef][_level] = lostEth[_directRef][_level].add(amount);
                }
            }
    ...
    function viewUpline(address _userAddress)
        external
        view
        returns (address directRefer, address upline1, address upline2, address upline3)

    {
        address ref = userList[1];
        directRefer = userList[users[_userAddress].directReferral];
        if(directRefer == ref){
            return (directRefer, upline1, upline2, upline3);
        }
               
        upline1 = userList[users[_userAddress].referrerID];
        if(upline1 == ref){
            return (directRefer, upline1, upline2, upline3);
        }else if(upline1 == directRefer){
            upline1 = userList[users[upline1].referrerID];
        }

        upline2 = userList[users[upline1].referrerID];
        if(upline2 == ref){
            return (directRefer, upline1, upline2, upline3);
        }else if(upline2 == directRefer){
            upline2 = userList[users[upline2].referrerID];
        }

        upline3 = userList[users[upline2].referrerID];
        if(upline3 == ref){
            return (directRefer, upline1, upline2, upline3);
        }else if(upline3 == directRefer){
            upline3 = userList[users[upline3].referrerID];
        }
