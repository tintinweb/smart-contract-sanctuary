...
contract BarrelyNaked is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 private _numberMintedFree;
    
    // Minting start offset
    uint256 private _freeMintStartOffsetUTS;
    ...
    // Minting end offset
    uint256 private _freeMintEndOffsetUTS = 86400; 
    ...
    function getFreeMintPeriodStarting() public view returns(uint256) {

        return _saleStartUTS.sub(_freeMintStartOffsetUTS);

    }

    function getFreeMintPeriodRemaining() public view returns(uint256) {

        return _saleStartUTS.add(_freeMintEndOffsetUTS);
    ...
    function hasFreeMintPeriodStart() private view returns(bool) {

        return getFreeMintPeriodStarting() <= block.timestamp;

    }

    function hasFreeMintPeriodEnd() private view returns(bool) {

        return getFreeMintPeriodRemaining() > block.timestamp;

    }

    function isFreeMintPeriodActive() public view returns(bool) {

        return (hasFreeMintPeriodStart() && hasFreeMintPeriodEnd());
    ...
    function mintTokens(uint256 numberOfTokens_) public payable checkSupplyAvailable(numberOfTokens_) {
        ...
        require(_mintPrice.mul(numberOfTokens_) == msg.value, "Invalid ether value");
    ...
    function claimFreeToken() public checkSupplyAvailable(1) {
        ...
        require(hasFreeMintPeriodStart(), "Free minting period hasn't started");
        require(hasFreeMintPeriodEnd(), "Free mint period has ended");
        require(!_addressesUsedFreeMint[msg.sender], "Free token claimed");
