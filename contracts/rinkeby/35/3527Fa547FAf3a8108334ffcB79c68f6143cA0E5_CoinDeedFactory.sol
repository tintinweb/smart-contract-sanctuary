...
contract CoinDeedFactory is ICoinDeedFactory, AccessControl {
    ...
    constructor(address deedTokenAddress, address _wholesaleFactoryAddress, address _lendingPoolAddress, address _coinDeedDeployerAddress, uint256 _platformFee) {
    ...
    function createDeed(
        ...
        BrokerConfig calldata brokerConfig
    ) external override {
        require(
        ...
        require(
            executionTime.buyTimestamp > block.number,
            "ExecuteBuyTimestamp must be in future."
        );
        require(
            executionTime.sellTimestamp > executionTime.buyTimestamp,
            "ExecuteSellTimestamp must be later than executeBuyTimestamp."
        );
        require(
...
//        ILendingPool.PoolInfo memory poolInfo = ILendingPool(lendingPoolAddress).poolInfo(pair.tokenA);
//
//        require(poolInfo.isCreated, "LendingPool does not exist.");
        ...
        ILendingPool(lendingPoolAddress).addNewDeed(coinDeedAddress);
