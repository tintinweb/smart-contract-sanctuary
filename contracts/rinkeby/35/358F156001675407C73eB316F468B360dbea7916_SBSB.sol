...
contract SBSB is ERC721Enumerable, Ownable 
{
    using Strings for string;

    uint public constant MAX_TOKENS = 8888;
    uint public constant NUMBER_RESERVED_TOKENS = 50;
    uint256 public constant PRICE = 1*10**17; //0.05
    uint256 public constant PRE_SALE_PRICE = 1*10**17; //0.03
    ...
    constructor() ERC721("SBSB", "SBFC") {}

    function mintToken(uint256 amount) external payable
    {
        require(msg.sender == tx.origin, "No transaction from smart contracts!");
        require(saleIsActive, "Sale must be active to mint");
        require(amount > 0 && amount <= 20, "Max 20 shakes per transaction");
