...
contract ETHSphynxSwapAgentImpl is Context {
    ...
    event SphynxSwapPairRegister(address indexed sponsor,address indexed erc20Addr, string name, string symbol, uint8 decimals);
    ...
    function registerSwapPairToBSC(address erc20Addr) external returns (bool) {
       require(!registeredERC20[erc20Addr], "already registered");
        ...
        emit SphynxSwapPairRegister(msg.sender, erc20Addr, name, symbol, decimals);
        return true;
    }
    ...
    function fillBSC2ETHSwap(bytes32 bscTxHash, address erc20Addr, address toAddress, uint256 amount) onlyOwner external returns (bool) {
        ...
        IERC20(erc20Addr).safeTransferFrom(owner, toAddress, amount);
