...
contract MuseCrowdsale is CappedCrowdsale, TimedCrowdsale, CapperRole {
...
constructor (
    ...
    {
        POE = _poe;
        POEBuffer = _poeBuffer;
         _individualDefaultCap = individualCap;
    }

    /**
     * @dev Override this modifer to do nothing. We will instead re-implement it with the POE pass also included
     */
    modifier onlyWhileOpen {
        _;
    }
    ...
    function _preValidatePurchase(address beneficiary, uint256 weiAmount) internal view {
        ...
        require(isOpen() || 
            (IERC20(POE).balanceOf(msg.sender) > 0 && 
                block.timestamp < openingTime() + POEBuffer && 
                    !hasClosed()),
            "TimedCrowdsale: not open for caller");
    }
