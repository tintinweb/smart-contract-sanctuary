  ...
  */
    ...
    function _withdrawFromComp(address _asset,address _cToken,uint256 _amount) internal {
        ...
        if(_asset == ETHER){                                                                  // THIS IS WHERE YOU MAKE MONEY
           ...
           // require(pBalanceB.sub(pBalanceA) == pTokenAmount, 'Balances Mismatch');          // require that the amount of ptoken withdraw reflect the balance after tx
            ...
            require(balanceAfter.sub(balanceBefore) == pTokenAmount, 'Balances Mismatch');          // require that the amount of ptoken withdraw reflect the balance after tx
            uint256 userReturn = balanceAfter.sub(balanceBefore);                                     // the amount of dai going back to the user
    ...
    function _Computation(address _cToken, uint256 _amount, uint256 pTokenAmount) internal view returns (uint256){
            ...
            uint256 price = getCompRate(_cToken, _amount);
                                                        // price in collateral for the amount the user wants to withdraw
           ...
           return (pTokenAmount.add(netProfit)); 
