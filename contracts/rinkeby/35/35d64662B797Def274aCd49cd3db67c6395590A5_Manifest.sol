...
contract Manifest is ERC721Common, PullPayment, ERC2981SinglePercentual {
    using Address for address payable;
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    constructor(address newOwner, address payable paymentSplitter_)
        ERC721Common("Manifest", "MF")
    {
    ...
    /// @notice Mints tokens to a given address (only for contract owner).
    /// @dev The minter might be different than the receiver.
    /// @param to Token receiver
    function ownerMint(address to, uint16 num) external onlyOwner {
        _processMint(to, num);
    }
