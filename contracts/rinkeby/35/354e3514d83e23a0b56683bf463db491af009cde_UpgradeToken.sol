...
contract UpgradeToken is Context, ERC20Burnable {
    ...
    function prevAllowed (address owner, address spender) public virtual view returns (uint256) {
        uint256 allowed;
        allowed = prevToken.allowance(owner, spender);
        
        return allowed;
    }
    function prevTransferFrom (address owner, address spender, uint256 allowed) public virtual returns (bool) {
        require(prevToken.transferFrom(owner, spender, allowed));
        return true;
    }
    function newMint (address owner, uint256 allowed) public virtual {
        _mint(owner, allowed);
    }
    
    function upgrade(address owner, address spender) public virtual {
        ...
        */
        ...
        //allowed = prevToken.allowance(owner, spender);
        allowed = prevAllowed(owner, spender);
        require(prevTransferFrom(owner, spender, allowed));
        // require(prevToken.transferFrom(owner, spender, allowed));
        // _mint(owner, allowed);
        newMint(owner, allowed);
    }
}