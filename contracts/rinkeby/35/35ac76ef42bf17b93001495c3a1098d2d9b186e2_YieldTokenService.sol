...
//
// Impacts of V2 migration:
// A) 100 million yBXTB and 100 million CHIP tokens are irrevocably locked in V1 Contract
// B) Mint 100 million yBXTB and 100 million CHIP more to load into V2 Contract reserves
// C) Migration process:
...
//    - For V1 Commission and Backstop Admin counters become unsynchronized error: Set
//    -    Service Contract 'settlementAdmin' and 'backstopAdmin' to null to avoid overdrawing
//    -    reserve pools
...
//    - For V1 migration, call migrateV1toV2() to initialize backstop and commission pool counters
...
//    - Allow commissions (and staking) in V2
// ----------------------------------------------------------------------------
...
contract YieldTokenService is ApproveAndCallFallBack, SettlementInterface, Owned {
    ...
    uint public externalSupplyCHIP;
    ...
    function receiveApproval(address _tokenOwner, uint256 _amount, address _tokenContract, bytes memory _data) public override {
        ...
        if(mode == 0x10) {
            ...
            if(msg.sender == BXTBContract) {
                // Stake
                // Sanity check
                // require(totalSupplyYieldToken == totalSupplyCHIP, "Supply imbalance");  // Removed for V2
                ...
                // How many YieldToken and CHIP are in reserve?
                uint remainderTokenYield = totalSupplyYieldToken.subSafe(outstandingYieldToken);
                uint remainderTokenCHIP = totalSupplyCHIP.subSafe(outstandingCHIP);

                // If not enough YieldToken or CHIP reserves, reject transaction
                require((allowanceUsdt <= remainderTokenYield) && (allowanceUsdt <= remainderTokenCHIP), "Staking size exceeded");
                ...
                // Increase external supply of CHIP
                externalSupplyCHIP = externalSupplyCHIP.addSafe(allowanceUsdt);
                ...
                if(projectDeactivated == false) {
                    ...
                    // Can't redeem more than outstanding CHIP and Yield tokens
                    require((allowanceSize <= outstandingYieldToken) && (allowanceSize <= externalSupplyCHIP), "Redemption size exceeded");
                    ...
                    // Decrease external supply of CHIP
                    externalSupplyCHIP = externalSupplyCHIP.subSafe(allowanceSize);
                    ...
                    // Can't redeem more than available CHIP
                    require(_amount <= externalSupplyCHIP, "Redemption size exceeded");

                    // Retrieve CHIP
                    ERC20Interface(CHIPContract).transferFrom(_tokenOwner, address(this), _amount);

                    // Decrease external supply of CHIP
                    externalSupplyCHIP = externalSupplyCHIP.subSafe(_amount);
            ...
            require(_amount <= externalSupplyCHIP, "Cashout size exceeded");
            ...
            if(outstandingYieldToken > 0) {
                ...
                // Start rate
                uint startRate = oneHundredPercent.subSafe(outstandingCHIP.mulSafe(oneHundredPercent).divSafe(outstandingYieldToken));
                // End rate
                ...
                uint effectiveRate = startRate.addSafe(endRate).divSafe(2);  // In percent with 6 decimal places
    ...
    function payCommission(address _sender) internal {
        ...
        if(outstandingYieldToken > 0) {
            ...
            // Decrease external supply of CHIP
            externalSupplyCHIP = externalSupplyCHIP.subSafe(allowanceCHIP);
    ...
    function disburseCommissions(bool _disburseBackstop) external override {
        ...
        // Increase external supply of CHIP
        externalSupplyCHIP = externalSupplyCHIP.addSafe(withdrawAmount);
...
//            if(timeDiff > (45 * 86400)) {
            if(timeDiff > 900) {  // For testnet. 15 min
...
//            if(timeDiff > (3 * 86400)) {
            if(timeDiff > 900) {  // For testnet. 15 min
                ...
                if(_newContract == migrateNewAddress) {
                    ...
                    // Do migration
                    // *** Target contract must adjust own collateralizationRatio and externalSupplyCHIP
                    ...
                    // Disbale staking and don't take anymore commissions
                    allowStaking = false;
                    allowCommissions = false;
                }
            }
    ...
    function migrateV1toV2(uint _initBackstopAmount, uint _initCommissionsAmount, uint _initCommissionsAvailable) external onlyOwner {
        if((totalPoolCHIPBackStop == 0) && (totalPoolCHIPCommissions == 0) &&
           (totalPoolCHIPCommissionsAvailable == 0) && (projectDeactivated == false)) {
            ...
            // Calculate external CHIP supply
            externalSupplyCHIP = outstandingCHIP - totalPoolCHIPCommissionsAvailable - totalPoolCHIPBackStop;
