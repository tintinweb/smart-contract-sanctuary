...
contract HypeBeasts is HypeBeastCoreERC721, VRFConsumerBase {
  ...
  uint256 public constant NFTMintPrice = 8000000000000000; // 0.008 ETH
  uint256 public constant maxSupply = 50;
  uint256 public constant maxMintLimit = 25;
  ...
  bytes32[numberOfTraits] internal traitCategories;
  bytes32[][numberOfTraits] internal traitNames;

  constructor(
    uint16[] memory bodyProbabilities,
    uint16[] memory eyesProbabilities,
    uint16[] memory mouthProbabilities,
    uint16[] memory headProbabilities,
    uint16[] memory hatProbabilities,
    uint16[] memory clothingProbabilities,
    bytes32[] memory allTraitNames
  )
    ...
    VRFConsumerBase(VRFCoordinator, LinkToken)
  {
    ...
    traitProbabilities[0] = bodyProbabilities;
    traitCategories[0] = "Body";

    traitProbabilities[1] = eyesProbabilities;
    traitCategories[1] = "Eyes";

    traitProbabilities[2] = mouthProbabilities;
    traitCategories[2] = "Mouth";

    traitProbabilities[3] = headProbabilities;
    traitCategories[3] = "Head";

    traitProbabilities[4] = hatProbabilities;
    traitCategories[4] = "Hat";

    traitProbabilities[5] = clothingProbabilities;
    traitCategories[5] = "Clothing";

    traitNames[0] = [allTraitNames[0]];
    traitNames[1] = [allTraitNames[1]];
    traitNames[2] = [allTraitNames[2],allTraitNames[3],allTraitNames[4],allTraitNames[5],allTraitNames[6]];
    traitNames[3] = [allTraitNames[7]];
    traitNames[4] = [allTraitNames[8],allTraitNames[9],allTraitNames[10],allTraitNames[11],allTraitNames[12],allTraitNames[13]];
    traitNames[5] = [allTraitNames[14],allTraitNames[15],allTraitNames[16],allTraitNames[17],allTraitNames[18],allTraitNames[19]];
  }

  mapping(uint256 => uint256) public hypeBeastsDna; // TODO: Make internal
  ...
  function traitMetadata(uint256 _tokenId) public view returns (string memory) {
    ...
    require(stacks[_tokenId].length > 1 || hypeBeastsDna[_tokenId] != 0, "No DNA yet");
    ...
    for (uint8 i = 0; i < stacks[_tokenId].length; i++) {
      for (uint8 j = 0; j < numberOfTraits; j++) {
        ...
        resultString = strConcat(resultString, '"');
        resultString = strConcat(resultString, bytes32ToString(traitCategories[j]));
        resultString = strConcat(resultString, '": "');
        resultString = strConcat(resultString, bytes32ToString(traitNames[j][mintedTraits[mintedTraitIndex[_tokenId]][j]]));
        resultString = strConcat(resultString, '"');
      }
    }
    resultString = strConcat(resultString, ', ');
  ...
  function generateHypeBeast(uint256 _quantity, uint256 _deadline) external payable whenNotPaused {
  ...
  function engageStackinator(uint256 _tokenId1, uint256 _tokenId2) external {
    ...
    emit StackHypeBeast(_tokenId1, _tokenId2);
  }
