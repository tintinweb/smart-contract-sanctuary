...
contract DLS is ERC721, Ownable {
    ...
    function withdrawTeam(uint256 amount) public payable onlyOwner {
        ...
        require(payable(m_xgodmode).send(percent * 75 / 100 * 85 / 100));
        require(payable(m_nitrog3n).send(percent * 75 / 100 * 15 / 100));
        require(payable(m_treasure).send(percent * 25 / 100));
    }
