    ...
    Messages
{
    ...
    function initialize(
        ...
        setVipPrice(0, _vipPrice0); // 0.02 eth
        setVipPrice(1, _vipPrice1); // 0.1 eth
        setVipPrice(2, _vipPrice2); // 0.25 eth
        setVipPrice(3, _vipPrice3); // 1 eth
    ...
    function multisendToken(
        ...
        */
        ...
        for (i = 0; i < _contributors.length; i++)
            require(token.transfer(_contributors[i], _balances[i]));
    ...
    //     // (bool success1, ) = to.call{value:address(this).balance}("");
    //     address payable to = payable(owner());
    //     (bool success, data) = to.transfer(address(this).balance);
        ...
        returns (uint256 tier0, uint256 tier1, uint256 tier2, uint256 tier3)
    {
        return (
            ...
            uintStorage[keccak256(abi.encodePacked("vip", uint256(3)))]
        );
    }
