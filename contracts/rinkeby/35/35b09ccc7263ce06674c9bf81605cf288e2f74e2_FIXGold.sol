...
contract FIXGold is IERC20, Ownable {
    ...
    function calculatePrice() public view returns (uint256) {
        if (_isStarted == false || _firstUpdate > block.timestamp) {
            return _price;
        }
        uint256 i;
        ...
        if (block.timestamp > _lastUpdate) {
            ...
            i = uint256((block.timestamp.sub(_firstUpdate)).div(SECONDS_PER_WEEK)).add(uint256(1));
            for (uint8 x = 0; x < _zeros.length; x++) {
                if (_zeros[x] >= _firstUpdate && _zeros[x] <= block.timestamp) {
                    i = i.sub(uint256(1));
                }
            }
            for (uint256 x = 0; x < i; x++) {
    ...
    function calculatePriceOnDate(uint256 __price, uint256 _date) public view returns (uint256, uint256) {
        
        uint256 i;
        uint256 skip=0;
        uint256 newPrice = __price;
        if (_date <= MSK_2021_09_04_10_00 ) return (0,__price);
        
        if (_date > _lastUpdate) {
            i = uint256((_lastUpdate.sub(_firstUpdate)).div(SECONDS_PER_WEEK).add(uint256(1)));
            for (uint8 x = 0; x < _zeros.length; x++) {
                if (_zeros[x] <= i) {
                    skip = skip.add(uint256(1));
            ...
            if (_growthRate_after > 0) {
                i = uint256((_date.sub(_lastUpdate)).div(SECONDS_PER_WEEK));
                for (uint256 x = 0; x < i; x++) {
                    newPrice = newPrice.mul(PERCENTAGE_MULTIPLICATOR.add(_growthRate_after)).div(PERCENTAGE_MULTIPLICATOR);
                }
            }
        } else {
            i = uint256((_date.sub(MSK_2021_09_04_10_00)).div(SECONDS_PER_WEEK)).add(uint256(1));
            for (uint8 x = 0; x < _zeros.length; x++) {
                if (_zeros[x] <= i) {
                    skip = skip.add(uint256(1));
                }
    ...
    function startContract(uint256 firstUpdate, uint256 lastUpdate, uint256 [] memory zeros) external onlyOwner {
        //uint256 [] calldata zeros=[18,33,70,84,123,135,175,190,226,240];
        //start date - 1630490401
        //end date - 1789207201
