...
contract TESTFISH is ERC721Enumerable, Ownable {
    using Strings for uint;
    using ECDSA for bytes32;

    uint public constant FISH_PRESALE = 1800; //presale
    uint public constant FISH_WALLET_PRESALE_LIMIT = 2; //FISHperwalletInWholeDuration
    uint public constant FISH_WALLET_PUBLICSALE_LIMIT = 2; //FISHperwalletInWholeDuration
    uint public constant FISH_MAX_COUNT =  3333; //maxSupply
    ...
    uint public constant FISH_MAINSALE_PRICE = 0 ether; //priceInMainSale
    uint public constant FISH_PRESALE_PRICE = 0 ether; //priceInPreSale
    ...
    function buy(uint tokenQuantity) external payable {
        ...
        require (tokenQuantity <= FISH_WALLET_PUBLICSALE_LIMIT);
        require (saleListPurchase[msg.sender] + tokenQuantity <= FISH_WALLET_PUBLICSALE_LIMIT, "MAX MINT COMPLETE");
        require (totalSupply() + tokenQuantity <= (FISH_MAX_COUNT - preSaleAmountMinted), "EXCEED_MAX");
    ...
    function presaleBuy( bytes memory signature, uint tokenQuantity) external payable {
        require (presaleLive, "PRESALE_CLOSED");//FISH_WALLET_PRESALE_LIMIT
        require(matchAddresSigner(signature), "DIRECT_MINT_DISALLOWED");
        require(preSaleAmountMinted + tokenQuantity <= FISH_PRESALE, "EXCEED_PRIVATE");
        require(presalerListPurchases[msg.sender] + tokenQuantity <= FISH_WALLET_PRESALE_LIMIT, "EXCEED_ALLOC");
        require(FISH_MAINSALE_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
