...
contract PerpLib {
	...
	function getSlot(uint strike) external view returns (uint) {
	...
	function getStrike(uint ident) external view returns (uint) {
	...
	function getTwapOI(uint timeElapsed, uint prevDecayTwapOI, uint oiDecayPerSecond, uint currentOI) external view returns (uint) {
		uint total = 1e18;
