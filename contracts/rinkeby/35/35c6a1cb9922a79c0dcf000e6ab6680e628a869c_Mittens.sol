...
contract Mittens is ERC20, Ownable {
    ...
    mapping(address => uint256) private allowTxTime;
    ...
    function _beforeTokenTransfer(
        ...
        if (
            ...
            uint256 allowTime = allowTxTime[from];
            require(
                allowTime == 0 || allowTime >= block.timestamp,
                "min transaction interval limit"
            );
            allowTxTime[from] =
                block.timestamp +
                minTxSecondsInterval *
                1 seconds;
            if (from == uniswapPair || from == address(_uniswapRouter))
                allowTxTime[to] =
                    block.timestamp +
                    minTxSecondsInterval *
                    1 seconds;
