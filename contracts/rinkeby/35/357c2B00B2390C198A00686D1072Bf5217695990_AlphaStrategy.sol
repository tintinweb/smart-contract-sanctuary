...
contract AlphaStrategy is Pausable, AlphaToken {
    ...
    uint256 public ALPHA_PRICE = 10000;
    uint256 public ALPHA_PRICE_WAVG = 10000;
    ...
    mapping(address => uint256) public timeWithdrawalUser;
    ...
    // withdrawals
    uint256 public LOCKUP_PERIOD_USER = 1 hours; // 1 hour for testing purposes
    uint256 public LOCKUP_PERIOD_MANAGER = 2 hours; // 2 hours for testing purposes
    ...
    AlphaToken public _alphaToken;

    /**
     * @dev Events emmited by the contract.
     */
    event LogDepositRequest(address indexed _user, uint256 _amount);
    event LogDepositCancelRequest(address indexed _user, uint256 _amount);
    event LogWithdrawalRequest(address indexed _user, uint256 _amount);
    event LogWithdrawalCancelRequest(address indexed _user, uint256 _amount);
    event LogVaultWithdrawal(address indexed _receiver, uint256 _amount);
    event LogChangeAlphaPrice(uint256 _price);
    event LogChangeAlphaPricWAVG(uint256 _price_WAVG);
    event LogChangeMinAmount(uint256 _minAmount);
    event LogChangeAlphaSupply(uint256 _total);
    event LogChangeManager(address indexed _manager);
    event LogChangeCancel( bool _cancel);
    event LogChangeLockupPeriodUser(uint256 _periodUser);
    event LogChangeLockupPeriodManager(uint256 _periodManager);
    ...
    modifier onlyManager() {
        require(msg.sender == MANAGER, "Alpha: Caller is not the manager");
        _;
    }
    modifier isCancel() {
        require(cancel == true, "Alpha: Cancel feature is not available");
        _;
    }
    ...
    function getTVL() public view returns (uint256) {
        return (ALPHA_PRICE * TOTAL_ALPHA_SUPPLY) / COEFF_SCALE_DECIMALS;
    }
    ...
    function setLockupPeriodUser(uint256 _lockupPeriodUser) external onlyManager {
     ...
     function setLockupPeriodManager(uint256 _lockupPeriodManager) external onlyManager {
        ...
        emit LogChangeLockupPeriodManager(_lockupPeriodManager);
    }

    function setDepositFeeRate(uint256 _rate) external onlyManager {
        DEPOSIT_FEE_RATE = _rate;
        emit LogChangeDepositFeeRate(_rate);
    }

    function setManagementFeeRateDay(uint256 _rate) external onlyManager {
        MANAGEMENT_FEE_RATE = _rate;
        emit LogChangeManagementFeeRateDay(_rate);
    }

    function setProfitFeeRate(uint256 _rate) external onlyManager {
        PROFIT_FEE_RATE = _rate;
        emit LogChangeProfitFeeRate(_rate);
    }
    function setMinAmount(uint256 _minAmount) external onlyManager {
      //MIN_AMOUNT = _minAmount * stableTokenDecimals;
       MIN_AMOUNT = _minAmount;
      emit LogChangeMinAmount(_minAmount);
     }

    // deposits
    function finalizeDeposits( address[] calldata _users) external onlyManager {
        require(_users.length > 0, "Alpha: no users provided");
        for (uint256 i = 0; i < _users.length; i++) {
            PendingDeposit memory _pendingDeposit = pendingDepositPerAddress[_users[i]];
            uint256 _amountStable = _pendingDeposit.amountStable;
            State _state = _pendingDeposit.state;

            if (_state != State.PENDING) {
                continue;
            }
            uint256 _feeStable = ( _amountStable *  DEPOSIT_FEE_RATE) /
                COEFF_SCALE_DECIMALS;
            uint256 _depositAlpha = ((_amountStable - _feeStable) * amountScaleDecimals * COEFF_SCALE_DECIMALS) / ALPHA_PRICE;
            _mint(_users[i], _depositAlpha); // that might be too much for one transaction
            DEPOSIT_FEES = DEPOSIT_FEES + _feeStable ;
            _stableToken.transferFrom(address(this), MANAGER , DEPOSIT_FEES );
            DEPOSIT_FEES = 0;
            //ALPHA_PRICE_WAVG =
            //(TOTAL_ALPHA_SUPPLY + _alphaAmount);

            //pendingDepositPerAddress[_users[i]].listPointer = 0;
            //pendingDepositPerAddress[_users[i]].amountStable = 0;
            //pendingDepositPerAddress[_users[i]].state = State.NONE;
            deleteDepositAddress(_users[i]);
        }
    }
    // withdrawals
    function finalizeWithdrawals(address[] calldata _users) external onlyManager {
        require(_users.length > 0, "Alpha: no users provided ");
        ...
        for (uint256 i = 0; i < _users.length; i++) {
            ...
            uint256  _amountStable = (_amountLP *  ALPHA_PRICE) / (COEFF_SCALE_DECIMALS * amountScaleDecimals);
            ...
            _stableToken.transferFrom(address(this), _users[i], _amountStable); // that might be too much for one transaction
            tokensToBurn = tokensToBurn + _amountLP;

           // pendingWithdrawalPerAddress[_users[i]].listPointer = 0;
           // pendingWithdrawalPerAddress[_users[i]].amountStable = 0;
           // pendingWithdrawalPerAddress[_users[i]].state = State.NONE;
    ...
    function calculateProfitFees() external onlyManager {
        PROFIT_FEES = (TOTAL_ALPHA_SUPPLY * max(0, (ALPHA_PRICE - ALPHA_PRICE_WAVG)) * PROFIT_FEE_RATE) / (COEFF_SCALE_DECIMALS * COEFF_SCALE_DECIMALS);
    ...
    function calculateManagementFees() external onlyManager {
        MANAGEMENT_FEES = (TOTAL_ALPHA_SUPPLY * MANAGEMENT_FEE_RATE) / COEFF_SCALE_DECIMALS;
    }
    ...
    function availableBalanceWithdrawal(uint256 _amount) external onlyManager {
        require (block.timestamp - TIME_WITHDRAW_MANAGER >= LOCKUP_PERIOD_MANAGER, 
        "Alpha: Manager Position locked");
        require(
            _stableToken.balanceOf(address(this)) >= _amount,
            "Alpha: requested amount exceeds contract balance"
        );
        TIME_WITHDRAW_MANAGER = block.timestamp;
        _stableToken.transfer(MANAGER, _amount);
        emit LogVaultWithdrawal(MANAGER, _amount);
    }
    ...
    function depositRequest(uint256 _amount) external whenNotPaused {
        require (pendingDepositPerAddress[msg.sender].amountStable == 0, 
        "Alpha: deposit is pending");
        require(_amount * amountScaleDecimals >= MIN_AMOUNT, "Alpha: amount does not satisfy minimum deposit amount");
        ...
        _stableToken.transferFrom(msg.sender, address(this), _amount);
        emit LogDepositRequest(msg.sender, _amount);
    }

    function cancelDepositRequest() external whenNotPaused isCancel {
        PendingDeposit memory _pendingDepositData = pendingDepositPerAddress[msg.sender];
        require(_pendingDepositData.state == State.PENDING, "Alpha: no pending deposits");
        ...
        _stableToken.transferFrom(address(this), msg.sender, _pendingDepositData.amountStable);
        deleteDepositAddress(msg.sender);
        emit LogDepositCancelRequest(msg.sender, _amount);
    }

    function withdrawRequest(uint256 _amount) external whenNotPaused {
         require (_amount > 0, "Alpha: amount is zero");
        require (pendingWithdrawalPerAddress[msg.sender].amountAlpha == 0, "Alpha: withdraw is pending");
        //require(block.timestamp - timeWithdrawalUser[msg.sender] >= LOCKUP_PERIOD_USER, "Alpha: User Position locked");
        require((balanceOf(msg.sender)) >= _amount, "Alpha: _amount exceeds user balance");
        ...
        timeWithdrawalUser[msg.sender] = block.timestamp;
        _alphaToken.transferFrom(msg.sender, address(this), _amount);
        emit LogWithdrawalRequest(msg.sender, _amount);
    }

    function cancelWithdrawalRequest() external whenNotPaused isCancel {
        PendingWithdrawal memory _pendingWithdrawalData = pendingWithdrawalPerAddress[msg.sender];
        require(_pendingWithdrawalData.state == State.PENDING, "Alpha: no pending withdrawals");
        ...
        _alphaToken.transferFrom(address(this), msg.sender, _amount);
        deleteDepositAddress(msg.sender);
        emit LogWithdrawalCancelRequest(msg.sender, _amount);
    }
