...
contract HashNFT is ERC721Enumerable, Ownable {
    ...
    function setMinIdForMinting(uint _minIdForMinting) public onlyOwner {
        minIdForMinting = _minIdForMinting;
    }

    function setTokensForMinting(uint _tokensForMinting) public onlyOwner {
        tokensForMinting = _tokensForMinting;
    }

    function setTotalTokens(uint _totalTokens) public onlyOwner {
        totalTokens = _totalTokens;
    }

    function setMinTokensPurchased(uint _minTokensPurchased) public onlyOwner {
        minTokensPurchased = _minTokensPurchased;
    }
    ...
    function _checkCrowdsaleParticipation(address _customerAddress) internal view returns (bool) {
        ...
        while (true) {
            try crowdSaleContract.sales(i) returns (
