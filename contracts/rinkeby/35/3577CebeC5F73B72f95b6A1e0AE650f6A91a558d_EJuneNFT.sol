 ...
 */
contract EJuneNFT is EGovernanceBase, ERC1155Burnable {

    // mapping tokenID For URIs
    mapping (uint256 => string) private _tokenURIs;

    // mapping tokenID For amount
    mapping (uint256 => uint256) private _tokenAmount;

    uint[] private _mintedTokens;
    mapping (uint256 => bool) public mintedTokensMap;

    event URIChanged(uint256 indexed id, address setter);

    /**
     * @dev Grants `DEFAULT_ADMIN_ROLE`, `JUNE_MINTER_ROLE` to the account that
     * deploys the contract.
     */
    constructor(address _governanceAdress, string memory baseUri) EGovernanceBase(_governanceAdress) ERC1155(baseUri) {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(JUNE_MINTER_ROLE, _msgSender());
    }

    function tokenAmount(uint256 id) public view returns(uint256){
        return _tokenAmount[id];
    }

    function mintedTokens() public view returns(uint256[] memory ids){
        return _mintedTokens;
    }

    function updateBaseUri(string memory baseUri) public onlyAdminPermission{
        _setURI(baseUri);
    }

    function updateUri(uint256 id, string memory metadataURI) public onlyJuneMinterPermission{
        require(mintedTokensMap[id], "token type tokenId not existed!");
        _tokenURIs[id] = metadataURI;
        emit URIChanged(id, _msgSender());
    }
    ...
    function tokensOwned(address _owner) public view returns (uint256[] memory ids, uint256[] memory amounts) {
        uint256 numTokens = _mintedTokens.length;

        uint256[] memory tempIds = new uint256[](numTokens);
        uint256[] memory tempAmounts = new uint256[](numTokens);

        uint256 count;
        for (uint256 i = 0; i < numTokens; i++) {
            uint256 tokenId = _mintedTokens[i];
            if (balanceOf(_owner, tokenId) > 0) {
                tempAmounts[count] = balanceOf(_owner, tokenId);
        ...
        uint256[] memory coverAmounts = new uint256[](count);
        uint256[] memory coverIds = new uint256[](count);

        for (uint256 i = 0; i < count; i++) {
            coverAmounts[i] = tempAmounts[i];
            coverIds[i] = tempIds[i];
        }

        return (coverIds, coverAmounts);
    }

    function tokensSummarize() public view returns (uint256[] memory ids, uint256[] memory amounts) {
        uint256 numTokens = _mintedTokens.length;
        ...
        for (uint256 i = 0; i < numTokens; i++) {            
            uint256 tokenId = _mintedTokens[i];        
            tempAmounts[i] = tokenAmount(tokenId);
            tempIds[i] = tokenId;
        }

        return (tempIds, tempAmounts);
    }

    /**
     * @dev Creates `amount` of new tokens type `id` for `to`.
     *
     *
     * Requirements:
     * - the caller must have the `JUNE_MINTER_ROLE`.
     */
    function mintNew(address to, uint256 id, uint256 amount, string memory metadataURI, bytes memory data) public virtual onlyJuneMinterPermission{
        require(_tokenAmount[id] == 0 && !mintedTokensMap[id], "token type tokenId existed!");      
        
        _tokenURIs[id] = metadataURI;
        _tokenAmount[id] += amount;

        mintedTokensMap[id] = true;
        _mintedTokens.push(id);

        _mint(to, id, amount, data);
    }

    /**
     * @dev Creates `amount` new tokens for `to`, of existed token type `id`.
     *
     *
     * Requirements:
     *
     * - the caller must have the `JUNE_MINTER_ROLE`.
     */
    function mint(address to, uint256 id, uint256 amount, bytes memory data) public virtual onlyJuneMinterPermission{
        require(mintedTokensMap[id], "token type tokenId not existed!");
        _tokenAmount[id] += amount;
        _mint(to, id, amount, data);       
    }
     ...
     */
    function mintBatch(address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) public virtual onlyJuneMinterPermission{
        for (uint256 i = 0; i < ids.length; i++) {
            uint id = ids[i];
            require(mintedTokensMap[id], "token type tokenId not existed!");
            _tokenAmount[ids[i]] += amounts[i];
        }
    ...
    function burn(address account, uint256 id, uint256 amount) public virtual override{
        ...
        uint256 currentAmount = _tokenAmount[id];
        // require(currentAmount >= amount, "ERC1155: burn amount exceeds current total amount");

        unchecked {
            _tokenAmount[id] = currentAmount - amount;
        }               
    }
    ...
    function burnBatch(address account, uint256[] memory ids, uint256[] memory amounts) public virtual override{           
        ...
        for (uint i = 0; i < ids.length; i++) {
            uint256 currentAmount = _tokenAmount[ids[i]];
            // require(currentAmount >= amounts[i], "ERC1155: burn amount exceeds current total amount");
            
            unchecked {
                _tokenAmount[ids[i]] = currentAmount - amounts[i];
            }
        }
    }

    function transferToGatewayOracle(address from, uint256 id, uint256 amount, bytes memory data) public onlyFromJuneGatewayOracleAddress{
        _safeTransferFrom(from, governance.juneGatewayOracleAddress(), id, amount, data);
    }
