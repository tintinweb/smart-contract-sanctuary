...
contract OlympusBondDepository is Policy {
    ...
    function depositWithPermit( 
        ...
        ERC20Permit( LP ).permit( msg.sender, address(this), amount_, deadline, v, r, s );
        _deposit( amount_, maxPremium_, depositor_ ) ;
        return true;
    }
    ...
    function _deposit( 
        ...
        // slippage protection
        require( maxPrice_ >= bondPrice(), "More than max price" );
        ...
        uint underflowPadding = value.div( 1000000 ); // protection against minor round errors
        uint profit = value.sub( payout ).sub( daoProfit ).sub( underflowPadding );
    ...
    function redeem() external returns ( bool ) {        
        ...
        uint percentVested = _calculatePercentVested( msg.sender );
     ...
     */
    function percentVestedFor( address depositor_ ) external view returns ( uint _percentVested ) {
    ...
    function pendingPayoutFor( address depositor_ ) external view returns ( uint _pendingPayout ) {
        uint percentVested = _calculatePercentVested( depositor_ );
