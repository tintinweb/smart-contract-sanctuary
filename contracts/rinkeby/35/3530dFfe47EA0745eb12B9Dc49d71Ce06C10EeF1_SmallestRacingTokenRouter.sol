...
import './SmallestRacingTokenCreateFactory.sol';
...
contract SmallestRacingTokenRouter is ReentrancyGuard,Ownable,IRacingTokenRouter {
    ...
    Race private race;
    mapping(uint32 => Race) raceFiter;
    mapping(address => RaceTicket) raceTicket; 
    ...
    uint256 public raceTokenTotalSupply = 1000000000000;
    ...
    address public racingTokenFactory;
    address private uniswapV2Pair;
    uint32 public currentRaceId;
    ...
    function calculatePlatformFee(uint256 _amount) external override view returns(uint256) {
        return _amount.mul(platformFee).div(
            10**2
        );
    } 

    function approveRace(address raceTokenAddress) external override onlyOwner {
        IERC20(raceTokenAddress).approve(uniswapV2RouterAddress, type(uint).max);
    }

    function approveRemoveRace() external override onlyOwner {
         uint256 liquidty = IERC20(uniswapV2Pair).balanceOf(address(this));    
         IERC20(uniswapV2Pair).approve(uniswapV2RouterAddress,liquidty);
    }
    function startRace() external override onlyOwner returns(address) {
       require(
          (raceFiter[currentRaceId].status == Status.Claimable),
       "The race has already started"); 

       Race storage newRace = race;
       newRace.status = Status.Open;
       newRace.raceId = currentRaceId;
       newRace.startTime = block.timestamp;
       newRace.participationFeeAmountEther = minAndMaxParticipationFeeAmount;
       newRace.winnerNumber = 0; 
       ...
       currentRaceTokenAddress = IRacingTokenCreateFactory(racingTokenFactory).startRacingToken(
           ...
           currentRaceId, totalParticipants);
    ...
    function joinRace(uint32 _raceId,address player,string memory nickName,uint32[] calldata _raceTicket) external override payable nonReentrant {
        require(address(this).balance <= maxDepositAmount,"Limit filled"); 
        require(msg.value == minAndMaxParticipationFeeAmount,"Not enough");
        require(_raceTicket.length != 10, "No race ticket specified");
        require(raceTicket[player].player == player,"You can only participate in this race once.");                
        RaceTicket storage newRacePlayer = raceTicket[player];
        newRacePlayer.raceId=_raceId;
        newRacePlayer.nickName=nickName;
        newRacePlayer.raceTicketNumber=_raceTicket;  
    }
    ...
    function setRacingTokenFactoryAddress(address newAddress) external override onlyOwner {
        emit UpdateRacingTokenFactoryAddress(newAddress, racingTokenFactory);
        racingTokenFactory=newAddress;
    }
...
interface IRacingTokenRouter {
    ...
    struct Race {  
        ...
        uint32 winnerNumber;
        RaceTicket raceTicket;                       
    }
    
    struct RaceTicket {
        uint32[] raceTicketNumber;
        address player;
        string nickName;
        uint32 raceId;
    }
    ...
    function approveRace(address raceTokenAddress) external;

    function approveRemoveRace() external;

    function startRace() external returns(address);
    /**
     * @notice Buy race tickets for the current race
     * @param _raceId: raceId
     * @param _raceTicket: Random 10-digit numbers between 0 and 9
     * @dev Callable by users
     */
    function joinRace(uint32 _raceId,address player,string memory nickName,uint32[] calldata _raceTicket) external payable;

    function setUniswapV2Router(address newAddress) external;

    function setRacingTokenFactoryAddress(address newAddress) external;
