...
contract ZKTTMint is Ownable {
    ...
    event Mint(address indexed from, address indexed to, uint256 value, string memo);
    ...
    function batchMint(address[] calldata tos, uint[] calldata amounts, string calldata memo) public onlyOwner returns(bool){
        require(tos.length > 0, "ZKTTMint: The length is zero");
        require(tos.length == amounts.length, "ZKTTMint: Unequal length");
    ...
    function _mint(address to, uint amount, string calldata memo) internal {
        require(to != address(0), "ZKTTMint: mint to the zero address");
        require(token.balanceOf(address(this)) >= amount, "ZKTTMint: amount exceeds balance");
