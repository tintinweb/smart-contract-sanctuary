...
contract nft_drops is ERC721, Ownable, VRFConsumerBase {
    ...
    // constructor() ERC721("NFTDrops", "DROPS") {
    constructor()
    ...
    function drawingLots(address contractAddr, uint256 startTokenId)
        external
        onlyOwner
    {
        ...
        */
        address selected = nft.ownerOf(0);

        for (
            uint256 i = startTokenId;
            i < startTokenId + nft.balanceOf(owner());
            i++
        ) {
            nft.safeTransferFrom(msg.sender, selected, i);
        }
    }
