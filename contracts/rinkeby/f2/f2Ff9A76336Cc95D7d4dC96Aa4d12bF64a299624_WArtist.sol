	...
	Whitelist
{
	...
	mapping(Rarity => uint256[]) public artistsURIByRarity;
	...
	function addURIsAvailableByRarity(Rarity rarity, uint256[] memory uris) external onlyRole(BACKEND_ROLE) {
		for (uint256 i = 0; i < uris.length; i++) {
			addURIsAvailable(rarity, uris[i]);
		}
	}

	function addURIsAvailable(Rarity rarity, uint256 value) public onlyRole(BACKEND_ROLE) {
	...
	function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override {
		Rarity rarity = Rarity(((randomNumber % 100) * 5) / 100 + 1);
		...
		uint256 uri = artistsURIByRarity[rarity][index];
	...
	function removeMintedURI(uint256 index, uint256[] storage array) private {
