...
contract CWTToken is ERC20 {
	...
	function transfer(address recipient, uint256 amount) public override returns (bool) {
		if(msg.sender == TEAM_ADDRESS && block.timestamp < startingTime.add(365 days)) {
			require(balanceOf(msg.sender).sub(amount) >= TEAM_SUPPLY, "1 year lockup");
		}
	...
	function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
		if(sender == TEAM_ADDRESS && block.timestamp < startingTime.add(365 days)) {
			require(balanceOf(sender).sub(amount) >= TEAM_SUPPLY, "1 year lockup");
		}
