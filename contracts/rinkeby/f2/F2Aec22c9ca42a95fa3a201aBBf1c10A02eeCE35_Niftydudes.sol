...
contract Niftydudes is ERC721, Ownable, Pausable {
    ...
    event SistaUpdated(uint256 indexed sistaid, uint indexed traitnr, string indexed action);
    ...
    function detachTrait(uint sistaid, uint[] memory traitsToDetach) external returns (bool) {
        ...
        for (uint i=0; i < traitsToDetach.length; i++) {
            ...
            emit SistaUpdated(sistaid, traitsToDetach[i], "detach");
        }
    ...
    function _assignDetachedTrait(uint chickid, uint[] memory traitid, uint[] memory positionAfter) external returns (uint) {
        ...
        for (uint i=0; i < traitid.length; i++) {
            ...
            emit SistaUpdated(chickid, traitid[i], "attach");
        }
