...
contract TokenProtected is Context, IERC20, Ownable, LGEWhitelisted {
    ...
    */
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(!_applyLGEBlacklist(sender, recipient)){
            return;
        }
...
contract LGEWhitelisted is Context {
    ...
    address private _lgeBlockAddress;
    ...
    event BlackListEnabled(uint256 indexed blockNumber, address indexed blockAddress);
    ...
    function checkBlackList(address sender)
        public view
        returns (bool)
    {
        return _blacklist[sender]>0;
    }
    ...
    returns (bool){
        ...
        if (sender == _lgeBlockAddress && _lgeBlockNumber > 0) {
            if(block.number - _lgeBlockNumber <= 3){
                //block forever
                _blacklist[recipient] = block.timestamp;
                emit BlackListAdd(recipient);
                return false;
            }
        }
        return true;
    ...
    function _applyLGEWhitelist(
        ...
        if (_lgeTimestamp == 0 && _msgSender() == _lgePairAddress && amount > 0){
            _lgeTimestamp = block.timestamp;
            _lgeBlockNumber = block.number;
            _lgeBlockAddress = recipient;
            emit BlackListEnabled(_lgeBlockNumber, _lgeBlockAddress);
        }
