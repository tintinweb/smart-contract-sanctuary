...
contract qazipolo is ERC721, Ownable {
    
    using SafeMath for uint256;

    string public QP_PROVENANCE = "";
    ...
    uint256 public constant qpPrice = 20000000000000000;
    uint public constant maxQPPurchase = 3;
    uint256 public constant MAX_QP = 20;
    uint public qpReserve = 5;
    ...
    constructor() ERC721("QaziPolo NFT", "QZP") { }
    ...
    function reserveQP(address _to, uint256 _reserveAmount) public onlyOwner {        
        ...
        require(_reserveAmount > 0 && _reserveAmount <= qpReserve, "Not enough reserve");
        for (uint i = 0; i < _reserveAmount; i++) {
            _safeMint(_to, supply + i);
        }
        qpReserve = qpReserve.sub(_reserveAmount);
    }


    function setProvenanceHash(string memory provenanceHash) public onlyOwner {
        QP_PROVENANCE = provenanceHash;
    }
    ...
    function tokensOfOwner(address _owner) external view returns(uint256[] memory ) {
     ...
     function mintQP(uint numberOfTokens) public payable {
        ...
        require(numberOfTokens > 0 && numberOfTokens <= maxQPPurchase, "Can only mint 3 tokens at a time");
        require(msg.value == qpPrice.mul(numberOfTokens), "Ether value sent is not correct");
        require(max_mints_per_address[msg.sender].add(numberOfTokens) <= 3,"Max 3 mints per wallet allowed");
        
        for(uint i = 0; i < numberOfTokens; i++) {
            if (totalSupply() < MAX_QP) {
                _safeMint(msg.sender, totalSupply());
                max_mints_per_address[msg.sender] = max_mints_per_address[msg.sender].add(1);
            } else {
               saleIsActive = !saleIsActive;
                msg.sender.transfer(numberOfTokens.sub(i).mul(qpPrice));
                break;
            }
        }
    }
   ...
   // to set the merkle proof
    ...
    function whitelistedMints( bytes32[] calldata merkleProof ) payable external  {
        ...
        require(msg.value == qpPrice,"Exactly 0.02 ETH is needed for mint" );
        require(totalSupply().add(1) <= MAX_QP, "Mint would exceed max supply of Cupcats");
