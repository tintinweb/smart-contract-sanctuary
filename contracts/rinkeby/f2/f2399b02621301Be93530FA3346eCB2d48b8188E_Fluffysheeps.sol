...
contract Fluffysheeps is ERC721,Ownable{
  ...
  uint256 public _maxTotalFluffySheeps = 96;
  uint256 public _currentTotalFluffySheeps = 0;
  uint256 public _fluffysheepPrice = 0.04 ether;
  bool _isActiveSale = false;
  bool _isActivePreSale = false;
  ...
  function mintFluffySheepsPresale(uint256 totalSheeps) public payable{
    require(_isPresaleAddress[_msgSender()], "not_presale_address");
    require( totalSheeps > 0, "invalid_total_sheeps");
    require( _currentTotalFluffySheeps + totalSheeps <= _maxTotalFluffySheeps, "not_enought_sheeps");
  ...
  function setPresaleAddressList(address[] memory _addresses) public onlyOwner{
    for(uint i = 0 ; i < _addresses.length ; i++){
      _isPresaleAddress[_addresses[i]] = true;
    }
  }
  ...
  function _calculateDiscount(uint totalSheeps) private view returns (uint256){
    ...
    }else if(totalSheeps >= 100){
      return totalSheeps * _fluffysheepPrice * 20 / 100;
    }else if(totalSheeps >= 1000){
      return totalSheeps * _fluffysheepPrice * 30 / 100;
    }else{
      return 0;
    }
  } 
