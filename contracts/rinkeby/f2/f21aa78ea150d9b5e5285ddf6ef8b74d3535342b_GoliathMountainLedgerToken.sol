...
9 DECIMALS 
TOTAL TAX 15%
MARKETING 10% to owners wallet
LIQUIDITY 5%
DISTRIBUTION TO HOLDERS 0%

*/

contract GoliathMountainLedgerToken is Context, IERC20, Ownable {
    ...
    address private _ownerWalletAddress = 0x183D79274118660a3B727D638929336766f5205c; // Owner Address
    ...
    string private _name = "Goliath Mountain Ledger Token";
    string private _symbol = "GMLT";
    uint8 private _decimals = 18;
    
    uint256 public _taxFee = 0;
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public fundingAmt = 1;
    
    uint256 public _ownerFee = fundingAmt.div(2);
    ...
    function setmarketingFeePercent(uint256 ownerFee) external onlyOwner() {
        _ownerFee = ownerFee;
    }
    ...
    function _takemarketing(uint256 tmarketing) private {
        uint256 currentRate =  _getRate();
        uint256 rmarketing = tmarketing.mul(currentRate);
        _rOwned[_ownerWalletAddress] = _rOwned[_ownerWalletAddress].add(rmarketing);
        if(_isExcluded[_ownerWalletAddress])
            _tOwned[_ownerWalletAddress] = _tOwned[_ownerWalletAddress].add(tmarketing);
    }
...
/** No minting and no Burning 
  ...
  function ChangeownerWalletAddress(address chownadd) public onlyOwner {
      _ownerWalletAddress = chownadd; 
  }*/
