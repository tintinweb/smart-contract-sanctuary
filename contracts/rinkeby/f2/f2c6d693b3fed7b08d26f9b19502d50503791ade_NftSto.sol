...
contract NftSto is Authorizable {
    ...
    function mintNft(address _to, string memory _symbol, string memory _name, string memory _icon) external onlyAuthorized returns (uint256) {
        require(_symbolMap[_symbol] == 0,"symbol exist");
        uint tokenId = totalSupply;
        _symbolMap[_symbol]= tokenId+1;
        string[] memory _files;
        Nft memory n = Nft({symbol:_symbol,name:_name,icon:_icon,files:_files});
        nftList.push(n);
...
//    function setFiles(uint _tokenId, string memory _files) external onlyAuthorized  {
