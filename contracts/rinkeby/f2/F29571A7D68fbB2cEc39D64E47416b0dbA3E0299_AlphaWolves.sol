...
contract AlphaWolves is ERC721Enumerable, Ownable {
    ...
    uint256 public maxWolfSupply = 10000; // 10000
    uint256 public maxAlphaWolfSupply = 5000; // Max Alpha Wolf Count aka 5000
    uint256 public betaWolfCount;
    uint256 public alphaWolfCount;
    ...
    modifier onlyWolfOwner() {
        require(balanceGenesis[msg.sender] > 0, "You do not have any wolves to interact with");
        _;
    }

    constructor() ERC721("AlphaWolvesNFT", "ALPHAWOLVESNFT") { }
    ...
    function clone() public onlyWolfOwner() { 
        uint256 clonePrice = getClonePrice();
        uint256 meatTokensInWallet = IERC20(stakeAddress).balanceOf(msg.sender);
        ...
        uint256 betaWolfId = maxAlphaWolfSupply + betaWolfCount + 1;
        _safeMint(msg.sender, betaWolfId);
        betaWolfCount++;
    }
    ...
    function mintPresale(uint256 numberOfMints) public payable {
        ...
        require(alphaWolfCount.add(numberOfMints) <= maxAlphaWolfSupply,   "Purchase would exceed max supply of Genesis AlphaWolves");
        ...
        for(uint256 i; i < numberOfMints; i++){
            _safeMint(msg.sender, alphaWolfCount + 1);
   ...
   function mint(uint256 numberOfMints) public payable {
        ...
        require(alphaWolfCount.add(numberOfMints) <= maxAlphaWolfSupply,   "Purchase would exceed max supply of Genesis KaijuKingz");
        require(price.mul(numberOfMints) == msg.value,      "Ether value sent is not correct");
        for(uint256 i; i < numberOfMints; i++) {
            _safeMint(msg.sender, alphaWolfCount + i);
            balanceGenesis[msg.sender]++;
            alphaWolfCount++;
        }
    }
