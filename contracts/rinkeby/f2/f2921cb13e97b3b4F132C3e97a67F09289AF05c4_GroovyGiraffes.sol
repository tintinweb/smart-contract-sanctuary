    ...
    Ownable
{
    ...
    uint256 public NFT_price = 0.05 ether; //250 matic 

    uint256 public startTime = block.timestamp;
    string _name = "Groovy Giraffes NFT";
    string _symbol = "GG";
    
    uint256 reserve_for_owner = 1000;
    uint256 reserved_claimed_By_owner = 0;
    ...
    function mintReserved(uint256 num, address _to) public onlyOwner {
        require(_currentTokenId + num < MAX_SUPPLY, "Max Supply Reached");
        require(reserved_claimed_By_owner < reserve_for_owner, "All Claimed");
        require(num <= 20, "Max 20 Allowed.");
        for(uint256 i=0; i<num; i++){
            uint256 newTokenId = _getNextTokenId();
            _mint(_to, newTokenId);
            _incrementTokenId();
            reserved_claimed_By_owner = reserved_claimed_By_owner.add(1);
        }
