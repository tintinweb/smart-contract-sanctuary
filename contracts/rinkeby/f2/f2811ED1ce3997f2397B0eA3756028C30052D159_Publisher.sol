...
contract Publisher is Ownable, AccessControl, Pausable {
    ...
    /// @param metadataCID string metadataCID of the consumable
    /// @param metadata string metadata of the consumable which includes cipher ( encrypted symmetric key with provider publicKey)
    function publish(
        uint256 _providerId,
        uint256 price,
        ...
        if (price != 0) {
            require(
                price >= minConsumablePrice,
                "Consumable Price must be greater than minimum Provider price"
            );
        }
        numConsumables++;
    ...
    function updateSkillScore(
        ...
        emit SkillScoreUpdated(_consumableId, _skillScore, _reportCID);
    }
    ...
    /// @notice event emitted after validation script is run to assign skillscore to consumable
