...
contract ThievesGang is Context, ERC165, IERC1155, IERC1155MetadataURI, Ownable {
    ...
    uint256 public token_price = 30000000000000000; // 0.03 ETH
    uint256 public max_tokens = 8080;
    ...
    mapping(address => uint256) private burned;
    
    address private a1;
    address private a2;
    address private a3;

    constructor(string memory uri_, address _a1, address _a2, address _a3) {
        _uri = uri_;
        a1 = _a1;
        a2 = _a2;
        a3 = _a3;
    ...
    function totalBurned(address _a) public view returns (uint256) {
        return burned[_a];
    }
    
    function mint(uint256 number_of_tokens) public payable {
        require(SALE_STATE, "Hiring is not available");
        require(total_supply + number_of_tokens <= max_tokens, "Minting amount exceeds maximum supply");
        require(msg.value >= number_of_tokens * token_price, "Ether value sent is not correct");
    ...
    function withdraw(uint256 amount) public onlyOwner {
        ...
        uint256 value = amount.div(3);
        payable(a1).transfer(value);
        payable(a2).transfer(value);
        payable(a3).transfer(value);
    }
    
    function uri(uint256 tokenId) public view override returns (string memory) {
        require(tokenId <= total_supply, "URI query for nonexistent token");
