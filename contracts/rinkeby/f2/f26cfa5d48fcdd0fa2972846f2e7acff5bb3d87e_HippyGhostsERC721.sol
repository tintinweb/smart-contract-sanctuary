...
contract HippyGhostsERC721 is OwnableUpgradeable, ERC721EnumerableUpgradeable, IERC2981Upgradeable {
    ...
    /**
     * @dev next mint index for _mintInSequence
     */
    uint256 private _nextMintIndex;
    ...
    function setNextMintIndex(uint256 nextMintIndex_) external onlyOwner {
        _nextMintIndex = nextMintIndex_;
    }
    ...
    function _mintInSequenceToAddress(address destination, uint256 numberOfTokens) internal {
        ...
        require(totalSupply() + numberOfTokens <= maxSupply, "numberOfTokens exceeds available supply");
        for (uint256 i = 0; i < numberOfTokens; i++) {
            // tokenId will start from 1
            _nextMintIndex = _nextMintIndex + 1;
            while (_exists(_nextMintIndex)) {
                // skip tokenId minted in mintMultipleTokensWithSignature
                _nextMintIndex = _nextMintIndex + 1;
            }
            if (totalSupply() < maxSupply) {
                _safeMint(destination, _nextMintIndex);
            }
        }
    }

    function mint(uint256 numberOfTokens) public payable {
        require(state == ContractState.PUBLIC, "public minting is disabled");
    ...
    function mintWithSignature(
        ...
        require(_verificationAddress != address(0), "verification address not set");
    ...
    function mintMultipleTokensWithSignature(
        ...
        require(valueInWei == msg.value, "payable does match require amount");
        require(_claimedMintKeys[mintKey] == false, "mintKey already claimed");
        require(totalSupply() + tokenIds.length <= maxSupply, "numberof tokenIds exceeds available supply");
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            // _safeMint will check _exists(tokenId)
