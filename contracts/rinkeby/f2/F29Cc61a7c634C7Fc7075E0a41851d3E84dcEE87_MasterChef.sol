...
contract MasterChef is Ownable {
    ...
    PoolInfo[] public POOL_INFOR;

    mapping(uint256 => mapping(address => UserInfo)) internal USER_INFO;
    mapping(address => User) public USERS;
    mapping(address => bool) public EXECLUDED_FROM_LP;
    ...
    event Deposit(address sender, uint256 depositAmount, uint256 bonusForTors);
    ...
    // function demoAdd() public {
    //     // weth =  0xc778417e063141139fce010982780140aa0cd5ab
    //     uint256 _allocPoint = 100;
    //     address _lpToken = 0xc778417E063141139Fce010982780140Aa0cD5Ab;
    //     uint16 _depositFeeBP = 0;
    //     uint256 _harvestInterval = 0;
    //     uint256[] memory _denominations = new uint256[](2);
    //     bool _withUpdate = true;

    //     _denominations[0] = 0.1 ether;
    //     _denominations[1] = 0.5 ether;

    //     address[] memory _anonymousTrees = new address[](1);

    //     _anonymousTrees[0] = 0x2Cd85e40103A4B15bf78FC01F70606A0Aa41824e;
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 _lpSupply = _pool.lpToken.balanceOf(address(this));
        ...
        returns (uint256, uint256)
    {
        ...
        if (!EXECLUDED_FROM_LP[address(_pool.lpToken)]) {
            return (_amount, 0);
        }
    ...
    function deposit(
        ...
        UserInfo storage _userInfo = USER_INFO[_pid][msg.sender];
        ...
        emit Deposit(msg.sender, _amount, _bonusForTors);
    }
    ...
    function bindNote(
        ...
        DepositInfo storage _depositInfo =
            USER_INFO[_pid][msg.sender].depositInfos[_depositId];

        require(_depositInfo.at > 0, "!at");
        require(_depositInfo.invalid == false, "!invalid");

        (uint256 _tors, uint256 _bonusForTors) =
            getReward(_pid, _depositId, msg.sender);
        ...
        if (user.referrer != address(0)) {
            ...
            for (uint256 i = 0; i < 3; i++) {
                if (upline != address(0)) {
                    ...
                    safeTorTransfer(upline, _torBonus);
        ...
        if (_depositInfo.at.add(BURN_BONUS_DAYS) <= block.timestamp) {
            safeTorTransfer(msg.sender, _bonusForTors);
        } else {
            TOR_BURN_TOTAL = TOR_BURN_TOTAL.add(_bonusForTors);

            safeTorTransfer(BURN_ADDRESS, _bonusForTors);
        }

        safeTorTransfer(msg.sender, _tors);
    ...
    function withdraw(
        ...
        _depositInfo.anonymousTree.unBind(
            _proof,
            _root,
            _nullifierHash,
            _recipient,
        ...
        uint256 _denomination = _pool.denominations[_depositId];

        if (!EXECLUDED_FROM_LP[address(_pool.lpToken)]) {
            _denomination = _denomination.mul(LIQUIDITY_PERCENT).div(
                PERCENTS_DIVIDER
            );
        }
    ...
    function getReward(
        ...
        if (_depositInfo.at.add(BURN_BONUS_DAYS) <= block.timestamp) {
            return (_tors, _depositInfo.bonusForTors);
        }
