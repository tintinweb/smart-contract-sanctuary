...
contract dust_for_punkz is Ownable, dust_redeemer, IRNGrequestor,IERC777Recipient, ReentrancyGuard {
    ...
    function tokensReceived(
        ...
        console.log("token",db.token,"vault",db.vault);
        ...
        waiting[hash] = DustBusterPro(db.name,db.vault,db.token,0,from, db.handler,pos);
        bytes memory data;
    ...
    function redeem(DustBusterPro memory general) external override {
    ...
    function process(uint256 rand, bytes32 requestId) external override {
        ...
        dust_redeemer(dbp.handler).redeem(dbp);
    }
}
...
interface dust_redeemer {
    function redeem(DustBusterPro memory general) external;
