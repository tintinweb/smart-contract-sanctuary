...
contract Lottery is VRFConsumerBase, Ownable {
    ...
    // definisco gli stati della lotteria, anche se li scrivo a lettere Ã¨ come se stessi scrivendo stato 0,1,2
    enum LOTTERY_STATE {
        OPEN,
        CLOSE,
        CALCULATING_WINNER
    ...
    constructor(
        ...
        // quando deployo il contratto la lotteria Ã¨ chiusa
    ...
    function getEntranceFee() public view returns (uint256) {
        // prendo quanto Ã¨ il valore in dollari di 1 ether
        (, int256 price, , , ) = ethUsdPriceFeed.latestRoundData();
        // qui non fo * 18 ma *10 perchÃ¨ dal contratto torna di suo un numero da 8 decimali
        uint256 adjustedPrice = uint256(price) * 10**10;
        uint256 costToEnter = (usdEntryFee * 10**18) / adjustedPrice;
        return costToEnter;
    }

    //solo l'autore puÃ² far partire o chiudere la lotteria
    function startLottery() public onlyOwner {
        // non avvio la lotteria se non Ã¨ chiusa in primo luogo
        require(
            lottery_state == LOTTERY_STATE.CLOSE,
            "La lotteria Ã¨ gia aperta/sta calcolando il vincitore"
        );
        lottery_state = LOTTERY_STATE.OPEN;
    }
    ...
    // la rendo internal cosi solo il vrf puÃ² chaiamre questa funzione, ovverride Ã¨ un opzione che sovrascrive il contenuto della funzione nel contratto d'appartenenza
        ...
        override
    {
        require(
            lottery_state == LOTTERY_STATE.CALCULATING_WINNER,
            "Non Ã¨ ancora stato deciso il vincitore"
        );
        require(
            _randomness > 0,
            "il vrf Ã¨ ubriaco e non ha ancora dato indietro il numero random"
        );
        // con questa operazione il modulo tornerÃ  sempre un indice possibile per i posti dell'array players
