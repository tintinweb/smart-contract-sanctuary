...
FairToken Cake – A Unique Smart Contract with Reward Distribution

Telegram: https://t.me/fairtokencake
Website: https://fairtokencake.com/

Initial TOKENOMICS:
Buy Tax: 17%
Sell Tax: 25%
HPS Tax: 45% on any sell

Holder Protection Shield (HPS): This feature is to protect YOU, so you can spend some precious time with your family
on the weekends or during holidays and before or after big announcements.
If the Holder Protection Shield is activated – Sell Tax will be increased to 45%
Always check our telegram to see whether HPS is active or not!

!!! Warning: If you are looking for the quick 100X you better turn around, this project is not for you !!!
...
contract HFToken is ERC20, AccessControl, Pausable {
    // CONFIG START

    uint256 denominator = 10000;
    
    // TOKEN
    string tokenName = "Have Fun Token";
    string tokenSymbol = "HFT";
    uint256 tokenTotalSupply = 100_000_000 * (10**18);
    
    // ADRESSES
    address devWallet = 0xf0E2014Df61664C26400cbc4E38EE3482B2713D0;
    address marketingWallet = 0x8F203850D87c804b58041438d7e6B0D04bC5387F;
    address communityWallet = 0x75e3CF0E8454F0D5fdFF10F2847A7d825c124aE9;
    address router02 = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;

    // BUY TAX
    uint256 public devTaxBuy = 150;
    uint256 public marketingTaxBuy = 250;
    uint256 public communityTaxBuy = 0;
    uint256 public redistributeTaxBuy = 300;

    // SELL TAX
    uint256 public devTaxSell = 150;
    uint256 public marketingTaxSell = 225;
    uint256 public communityTaxSell = 225;
    uint256 public redistributeTaxSell = 500;

    uint256 public maxTxAmount = 25_000_000 * 10**18 + 1;
    uint256 public maxWalletAmount = 25_000_000 * 10**18 + 1;

    // REDISTRIBUTION
    address public token = 0xF962AF6a99C54d777B20BF5e7C832763E8D7611E;
            ...
            if(!isExcluded[sender] && !isExcluded[recipient]) {
                ...
                if(sender == address(_UniswapV2Pair) || recipient == address(_UniswapV2Pair)) {
                    if(sender == address(_UniswapV2Pair)) {
                        ...
                        require(sender != address(_UniswapV2Pair) || (sender == address(_UniswapV2Pair) && block.number >= nextBuyBlock[recipient]), "HFT: Cooldown");
                    ...
                    require(isExcludedFromBotProtection[recipient] || balanceOf(recipient) + amount <= maxWalletAmount, "HFT: Max wallet amount");                
                }
            }
