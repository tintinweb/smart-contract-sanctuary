...
contract Vault is ITreasury, Ownable {
  ...
  function initialize(
    ...
    address uniswapV2CompatFactory_
    ...
    getPrincipleToken = IUniswapV2Factory( uniswapV2CompatFactory_ ).createPair( getManagedToken, getReserveToken );
    isPrincipleToken[getPrincipleToken] = true;
    isInitialized = true;
    return true;
  }

  function depositIntoLiquidity( uint reserveTokenAmountToList_, uint managedTokenAmountToList_ ) external onlyOwner() returns ( bool ) {
    address reserveToken_ = getReserveToken;
    address principleToken_ = getPrincipleToken;
    IERC20( reserveToken_ ).safeTransferFrom( msg.sender, address( this ), managedTokenAmountToList_ );
    IERC20( reserveToken_ ).safeTransferFrom( msg.sender, principleToken_, reserveTokenAmountToList_ );
    IERC20Mintable( getManagedToken ).mint( principleToken_, managedTokenAmountToList_ );
    IUniswapV2Pair( principleToken_ ).mint( daoWallet );
    return true;
  }
  ...
  function incurDebt( uint principieTokenAmountDeposited_, uint bondScalingValue_ ) external override onlyPayment() returns ( bool ) {
    getDebtAmountDue = getDebtAmountDue.add(
        IBondingCalculator( getBondingCalculator )
          .calculateBondInterest( address(this), getPrincipleToken, principieTokenAmountDeposited_, bondScalingValue_ ).div( 1e9 )
      );
    return true;
  }
    ...
    returns ( bool )
  {
    ...
    uint debt =  getDebtAmountDue;

    require( debt >= interestDue_, "Vault: msg.sender is trying to collect more interest then there is outstanding debt." );

    IERC20(getPrincipleToken).safeTransferFrom(msg.sender, address(this), principleAmount_);

    getDebtAmountDue = debt.sub( interestDue_ );
    
    IERC20Mintable( getManagedToken ).mint( depositor_, interestDue_ );

    uint riskFreeValue_ = IBondingCalculator( getBondingCalculator ).principleValuation( getPrincipleToken, principleAmount_ ).div( 1e9 );
 ...
 function ownerDepositReserveToken( uint amount_ ) external onlyOwner() returns ( bool ) {
  ...
  function depositPrinciple( uint depositAmount_ ) external onlyOwner() returns ( bool ) {
    address principleToken = getPrincipleToken;
    IERC20( principleToken ).safeTransferFrom( msg.sender, address(this), depositAmount_ );
    IERC20Mintable( getManagedToken ).mint( stakingContract, IBondingCalculator( getBondingCalculator ).principleValuation( principleToken, depositAmount_ ) );
    return true;
  }
