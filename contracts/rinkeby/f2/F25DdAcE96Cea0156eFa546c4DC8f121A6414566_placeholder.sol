...
contract placeholder is Ownable, ERC721Burnable {
    // Setup imports
    ...
    uint256 public constant PUBLIC_MINT_PRICE = 1000000000000000; // 0.04 ETH
    uint256 public constant MAX_SUPPLY = 100;
    bool public paused = true;
    address payable[1] royaltyRecipients;

    mapping (uint => uint256) public bloxOfRegret;
    mapping (uint => uint256) public ethOfRegret;

    constructor(
        address payable[1] memory _royaltyRecipients
    ) public ERC721("placeholder", "ph") {
        ...
        returns (uint256)
    {
        ...
        require(msg.value >= PUBLIC_MINT_PRICE, "The price is .01 ETH or more.");

        //Mint the token and set URI
        _safeMint(recipient, nextId);
        bloxOfRegret[nextId] = block.number;
        ethOfRegret[nextId] = msg.value;
    ...
    function getBloxOfRegret(uint256 tokenId) internal view returns (uint256) {
        return bloxOfRegret[tokenId];
    }

    function getEthOfRegret(uint256 tokenId) internal view returns (uint256) {
        return ethOfRegret[tokenId];
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        parts[1] = getBloxOfRegret(tokenId).toString();
        parts[2] = "\n";
        parts[3] = getEthOfRegret(tokenId).toString();
        parts[4] = " wei";
        parts[5] = "</foreignObject></svg>";

        string memory output = string(
            abi.encodePacked(parts[0], parts[1], parts[2], parts[3], parts[4], parts[5])
        );

        string memory boughtBlock = string(abi.encodePacked(parts[1]));
        string memory paidEth = string(abi.encodePacked(parts[3], parts[4]));
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        boughtBlock,
                        '", "description": "',
                        paidEth, " was paid for this block number",
