...
contract NFTClaimRecipe {
    ...
    bytes32 public constant TEMPLATE_ID = keccak256("NFT_CLAIM_RECIPE");

    bytes32 public constant POOL_ID = keccak256("NFT_CLAIM");
    bytes32 public constant REWARDER_ID = keccak256("BASIC_REWARDER");
    bytes32 public constant ACCESS_ID = keccak256("ACCESS_CONTROLS");

    event NFTClaimDeployed(address token, address pool, address rewarder);
    ...
    */
    ...
    /** 
     * @dev prepare NFT Claim recipe
     *   
    */
    function prepareNFTClaim(
        address _admin,
        address _token,
        ...
        external payable
    {
        ...
        address pool = jellyFactory.deployContract{value: msg.value}(
            POOL_ID,
            _integratorFeeAccount, 
            "");
        IJellyContract(pool).initContract(abi.encode(_token, access_controls)); 

        address rewarder = jellyFactory.deployContract{value: msg.value}(
            REWARDER_ID,
            _integratorFeeAccount, 
            "");

        IJellyContract(rewarder).initContract(abi.encode(access_controls)); 

        uint256 jellyFee = _tokenAmount * uint256(feePercentage) / 1000;
        uint256 claimAmount = _tokenAmount - jellyFee;

        if (jellyFee > 0) {
            OZIERC20(_token).safeTransferFrom(msg.sender, jellyVault, jellyFee);
        }
        IJellyPool(pool).setRewardsContract(rewarder);
        IJellyRewarder(rewarder).setVault(_vault);
        IJellyRewarder(rewarder).addPoolContract(pool, 10000);

        IJellyRewarder(rewarder).addRewardsToPool(
            pool,
            _token,
            _start_time,
            _duration,
        ...
        IJellyAccessControls(access_controls).addAdminRole(_admin);
        IJellyAccessControls(access_controls).removeAdminRole(address(this));

        emit NFTClaimDeployed(_token, pool, rewarder);
    }
