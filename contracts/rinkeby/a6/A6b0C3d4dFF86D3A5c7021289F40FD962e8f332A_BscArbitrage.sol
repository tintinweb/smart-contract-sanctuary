...
contract BscArbitrage is Ownable {
    ...
    function swapStableToBasic(uint256 amount) public onlyOwner returns (uint256) {
        require(amount <= stables.balanceOf(address(this)), "Swap amount exceeds basic balance");
        require(stables.approve(pancakeRouterAddress, amount));
    ...
    function swapBasicToStable(uint256 amount) public onlyOwner returns (uint256) {
        require(amount <= basics.balanceOf(address(this)), "Swap amount exceeds stable balance");
        require(basics.approve(pancakeRouterAddress, amount));
    ...
    function setpancakeRouterAddress(address _pancakeRouterAddress) public onlyOwner
    {
     ...
     function getPathBasicToStable() private view returns (address[] memory) {
    ...
    function getPathStableToBasic() private view returns (address[] memory) {
        address[] memory path = new address[](2);
