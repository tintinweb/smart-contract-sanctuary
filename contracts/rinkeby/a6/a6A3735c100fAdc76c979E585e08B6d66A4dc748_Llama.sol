...
contract Llama is ERC721, ERC721Enumerable, ERC721URIStorage, BaseOpenSea, Pausable, Ownable, ReentrancyGuard, VRFConsumerBase {
    ...
    constructor(uint256 maxPurchase, uint256 maxTokens, string memory contractURI_, address openSeaProxyRegistry) VRFConsumerBase(
        ...
        _unitCost = 10000000000000000; //start price - 0.01 ether
    ...
    function drawLotto() public onlyOwner nonReentrant{
        ...
        require(address(this).balance >= CurrentJackpotInWei.div(6), "Not enough eth to pay jackpot");
        
        
        NextLottoDraw = block.timestamp.add(2419200); //next draw must be 28 days after the preveious one
        lottoTime(TotalJackpotInWei.div(6));
        randomResult = 0;
    ...
    function lottoTime(uint256 jackpot) private {
        CurrentJackpotInWei.sub(jackpot);
   ...
   function currentWinners(uint256 n, uint256 maxNumber) public view returns (uint256[] memory expandedValues) {
    ...
    function mintLlama(uint256 numberToMint) public payable nonReentrant {
        ...
        do {
            ...
            uint256 jp = _unitCost.mul(i).div(10); //fixed 10% jackpot cut
            TotalJackpotInWei = TotalJackpotInWei.add(jp);
            CurrentJackpotInWei = CurrentJackpotInWei.add(jp);
            
            if (numberToMint.sub(i) > 0){
                //return any overspent funds for the last buyer
                payable(msg.sender).transfer(numberToMint.sub(i).mul(_unitCost));
            }
    }
	...
	function withdraw() public onlyOwner {
		...
		require(balance.sub(CurrentJackpotInWei) > 0, '"I\'ve got nothing left!!" - Ian Beale 2014');
