...
contract PerpetualLiquidator is PerpetualRebalanceFunctions, IFunctionList, IPerpetualLiquidator {
    ...
    function liquidateByAMM(
        ...
        address _liquidatorAddr,
        address _traderAddr
    ) external virtual override returns (int128 fLiquidatedAmount) {
        PerpetualData storage perpetual = _getPerpetual(_perpetualIndex);
        require(!_isMaintenanceMarginSafe(perpetual, _traderAddr), "trader is safe");

        fLiquidatedAmount = _getPositionAmountToLiquidate(perpetual, _traderAddr).neg();
        // we liquidate at mark-price
        int128 fDeltaLockedValue = fLiquidatedAmount.mul(_getPerpetualMarkPrice(perpetual));
        //CC stands for Collateral Currency
        int128 fDeltaLockedValueCC = fDeltaLockedValue.div(_getCollateralToQuoteConversionMultiplier(perpetual));
        int128 fTraderPos = _getTraderPosition(perpetual, _traderAddr);
        // partially or fully close the trader's position:
        _getTradeLogic().executeTrade(perpetual.id, _traderAddr, fTraderPos, fLiquidatedAmount, fDeltaLockedValue, true);
        ...
        int128 fRemainingMargin = _getAvailableMargin(perpetual, _traderAddr, false);
        if (fRemainingMargin < 0) {
            fRemainingMargin = 0;
        }
        // calculate penalty in collateral currency
        int128 fPenaltyCC = fDeltaLockedValueCC.abs().mul(perpetual.fLiquidationPenaltyRate);

        int128 fPenaltyToTrader = fPenaltyCC;
        if (fPenaltyCC > fRemainingMargin) {
            // not enough margin balance to pay entire penalty
            // get missing funds from default fund
            int128 fDefaultFundWithdrawal = fPenaltyCC.sub(fRemainingMargin);
            // make sure default fund has enough cash
            if (liqPool.fDefaultFundCashCC < fDefaultFundWithdrawal) {
                fDefaultFundWithdrawal = liqPool.fDefaultFundCashCC;
                // We have not enough funds in the default fund to pay fee.
                // Emergency state for the whole liquidity pool
                _setLiqPoolEmergencyState(liqPool);
            }
            liqPool.fDefaultFundCashCC = liqPool.fDefaultFundCashCC.sub(fDefaultFundWithdrawal);

            fPenaltyToTrader = fRemainingMargin;
        }
        _updateTraderMargin(perpetual, _traderAddr, fPenaltyToTrader.neg());
        // pay liquidator
        address mgnTokenAddr = liqPool.marginTokenAddress;
        _transferFromVaultToUser(mgnTokenAddr, _liquidatorAddr, fPenaltyCC);
        _getUpdateLogic().updateAMMTargetPoolSize(perpetual.id);
        if (fRemainingMargin.sub(fPenaltyCC) > 0) {
            // regular fees
            _getTradeLogic().distributeFees(perpetual.id, _traderAddr, address(0), fDeltaLockedValueCC, false);
        }
        _rebalanceAMMFundSize(perpetual);
        ...
        emit Liquidate(perpetual.id, _liquidatorAddr, _traderAddr, fLiquidatedAmount, block.timestamp);
    
    }

    /**
     * Rebalance the AMM fund from the default fund after liquidation.
     * We only rebalance 75% up to AMM pool funding/target funding
     * @param _perpetual  The perpetual object
     */
    function _rebalanceAMMFundSize(PerpetualData storage _perpetual) internal {
        int128 targetCollateral = _perpetual.fTargetAMMFundSize;
        if (targetCollateral < _perpetual.fAMMFundCashCC) {
            // target already fulfilled
            return;
        }
        // draw funds in relation to available size.
        // if default fund is funded x%<1, we withdraw at most x%*.75
        LiquidityPoolData storage liqPool = _getLiquidityPoolFromPerpetual(_perpetual.id);
        int128 fTotalFundedRatio = liqPool.fDefaultFundCashCC.div(liqPool.fTargetDFSize);
        if (fTotalFundedRatio > ONE_64x64) {
            fTotalFundedRatio = ONE_64x64;
        }
        int128 maxAmount = THREEQUARTER_64x64.mul(fTotalFundedRatio).mul(liqPool.fDefaultFundCashCC);
        int128 gap = targetCollateral.sub(_perpetual.fAMMFundCashCC);
        if (gap > maxAmount) {
            // shrink gap payment
            gap = maxAmount;
        }
        // send funds from default fund to AMM pool
        liqPool.fDefaultFundCashCC = liqPool.fDefaultFundCashCC.sub(gap);
        _perpetual.fAMMFundCashCC = _perpetual.fAMMFundCashCC.add(gap);
        // update total AMMFundCash saved in liq-pool
        liqPool.fAMMFundCashCC = liqPool.fAMMFundCashCC.add(gap);
     ...
     * First, get current margin balance in base currency and
     * calculate the required initial margin for this amount.
     * Second, liquidation size is the current position minus
    ...
    function _getPositionAmountToLiquidate(PerpetualData storage _perpetual, address _traderAddr) internal view returns (int128) {
        // require(!_isMaintenanceMarginSafe(_perpetual, _traderAddr), "this method is to be called only if trader needs to be liquidated");
        // get the current margin (quote currency)
        int128 fMarginQC = _getMarginBalance(_perpetual, _traderAddr);
        // convert to base currency at mark price
        int128 fPx = _getBaseToQuoteConversionMultiplier(_perpetual, true);
        int128 fMarginBC = fMarginQC.div(fPx);
        ...
        /*--------
            calculation: after partial liquidation the margin must be large
             enough for the position, hence:
            newpos * marginrate = margin - (oldpos-newpos) * fTotalFeeRate
            newpos * (a+b*newpos) = margin - (oldpos-newpos) * fTotalFeeRate
            
            --> solve for newpos is quadratic equation
            --> we simplify and liquidate up to the maximal margin rate
            newpos = (margin - oldpos*fTotalFeeRate)/(marginrateCap-fTotalFeeRate)
        --------*/
        // adjust for liquidation penalty and fees
        bool is_short = fTraderPos < 0;
        fTraderPos = fTraderPos.abs();
        int128 fTotalFeeRate = _perpetual.fLiquidationPenaltyRate.add(_perpetual.fTreasuryFeeRate).add(_perpetual.fPnLPartRate);
        int128 fFeeBC = fTraderPos.mul(fTotalFeeRate);
        int128 fNewPos = fMarginBC.sub(fFeeBC).div(_perpetual.fInitialMarginRateCap.sub(fTotalFeeRate));
        int128 fPosToLiquidate = fTraderPos.sub(fNewPos);
        if (fNewPos < _perpetual.fLotSizeBC) {
            // remainder position after liquidation is too small, liquidate all of it
            fPosToLiquidate = fTraderPos;
        }
        if (is_short) {
            fPosToLiquidate = fPosToLiquidate.neg();
        }
        return fPosToLiquidate;
    }

    /**
     * Check if the trader is maintenance margin safe in the perpetual, which means
     * margin >= maintenance margin, 0 if position = 0
     * @param _perpetual The perpetual object
     * @param _traderAddr The address of the trader
     * @return boolean, true if the trader is maintenance margin safe in the perpetual
     */
    function _isMaintenanceMarginSafe(PerpetualData memory _perpetual, address _traderAddr) internal view returns (bool) {
        int128 threshold = _getTraderPosition(_perpetual, _traderAddr) == 0 ? 0 : _getMaintenanceMargin(_perpetual, _traderAddr);
        return (_getMarginBalance(_perpetual, _traderAddr) >= threshold);
    }
    ...
    function _getBaseToQuoteConversionMultiplier(PerpetualData storage _perpetual, bool _isMarkPriceRequest) internal view returns (int128) {
        if (_isMarkPriceRequest) {
            ...
            return _perpetual.state != PerpetualState.NORMAL ? _perpetual.settlementS2PriceData.fPrice : _perpetual.indexS2PriceData.fPrice;
        }
    }
