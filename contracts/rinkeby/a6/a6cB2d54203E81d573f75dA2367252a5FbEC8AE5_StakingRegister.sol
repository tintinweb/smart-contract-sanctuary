...
contract StakingRegister is IStakingRegister, Managed {
    ...
    function list(uint256 _offset, uint256 _limit)
        ...
        returns (address[] memory result)
    {
        uint256 to = (_offset + _limit).min(stakes.length());
    ...
    function listByUser(
        ...
        uint256 to = (_offset + _limit).min(set.length());
    ...
    function add(
        ...
        require(stakes.add(_stakingAddress), "Register: Alreade exists");
        require(
            ownerStakes[_owner].add(_stakingAddress),
            "Register: Alreade exists"
        );
    ...
    {
        require(stakes.remove(_stakingAddress), "Register: Not found");
        require(
            ownerStakes[_owner].remove(_stakingAddress),
            "Register: Not found"
        );
    ...
    function setDefaultDepositFeePercentage(uint256 _amount)
        ...
        requirePermission(ROLE_ADMIN)
    {
        require(_amount < MAX_FEE_PERCENTAGE, "Register: incorrect input");
    ...
    function _setRewardSetting(
        ...
        for (uint256 i = 0; i < _rewardTokens.length; i++) {
            uint256 needTransfer = calculateNumberTokensNeedTransfer(
                _stakingAddress,
                _rewardTokens[i],
                _rewardPerBlock[i]
            );
            ...
            if (_rewardTokens[i].supportsInterface(InterfaceId_ERC721)) {
                ...
                IERC20(_rewardTokens[i]).safeTransferFrom(
                    ...
                    DecimalsConverter.convertFrom18(
                        needTransfer,
                        IERC20Metadata(_rewardTokens[i]).decimals()
                    )
                );
            }
        }
    ...
    function calculateNumberTokensNeedTransfer(
        ...
        uint256 requiredAmount = (rewardEndBlock - startBlock) *
            (newRewardPerBlock - staking.getRewardsPerBlockInfo(rewardToken));

        uint256 balance = 0;

        if (rewardToken.supportsInterface(InterfaceId_ERC721)) {
            balance =
                IERC721(rewardToken).balanceOf(stakingAddress) *
                DECIMALS18 -
                staking.getMustBePaid(rewardToken);
        } else {
            balance =
                IERC20(rewardToken).balanceOf(stakingAddress) -
                (staking.getMustBePaid(rewardToken));
        }
    ...
    function _addDurartion(
        ...
        for (uint256 i = 0; i < tokensRewards.length; i++) {
            if (rewardPerBlock[i] > 0) {
                if (!tokensRewards[i].supportsInterface(InterfaceId_ERC721)) {
                    uint256 transferAmount = rewardPerBlock[i] * _blockAmount;
                    ...
                    IERC20(tokensRewards[i]).safeTransferFrom(
                        ...
                        DecimalsConverter.convertFrom18(
                            transferAmount,
                            IERC20Metadata(tokensRewards[i]).decimals()
                        )
                    );
                }
            }
        ...
        staking.setRewardEndBlock(endBlock + _blockAmount);
    ...
    function _calculateAndTransferNFT(
        ...
        for (uint256 i = 0; i < _rewardTokenAddress.length; i++) {
            uint256 rewardPerBlock = IStaking(_stakingAddress)
            .getRewardsPerBlockInfo(_rewardTokenAddress[i]);

            uint256 transferAmount = (rewardPerBlock * _blockAmount) /
                DECIMALS18 +
                1;
