...
contract MultiChainCapital is Context, IERC20, Ownable {
  ...
  uint256 private _tTotal = 3000000000000 * 10**9;
  ...
  {
    uint256 finalTaxFee = isSelling ? taxFee * _sellTaxMultiplier : taxFee;
    uint256 finalTeamFee = isSelling ? teamFee * _sellTaxMultiplier : teamFee;

    uint256 tFee = tAmount.mul(finalTaxFee).div(100);
    uint256 tTeam = tAmount.mul(finalTeamFee).div(100);
  ...
  function _setSellTaxMultiplier(uint8 sellTaxMultiplier) external onlyOwner {
    require(sellTaxMultiplier <= 10, 'sellTaxMultiplier should be in 0 - 10');
    _sellTaxMultiplier = sellTaxMultiplier;
  }
