...
import '../utils/SaleKindInterface.sol';
...
contract ExchangeCore is ReentrancyGuard, Ownable {
    ...
    uint256 public minimumMakerProtocolFee = 250;
    ...
    struct OrderApproveBytes {
        bytes calldatas;
        bytes replacementPattern;
        bytes staticExtradata;
    }
    
    event OrderApprovedPartOne    (bytes32 indexed hash, OrderApprovePartOne orderApprovePartOne);
    event OrderApprovedPartTwo    (bytes32 indexed hash, OrderApprovePartTwo orderApprovePartTwo);
     ...
     * @param calldatas Calldatas (appended to extradata)
    ...
    function staticCall(address target, bytes memory calldatas, bytes memory extradata)
        ...
        returns (bool result)
    {
        bytes memory combined = new bytes(SafeMath.add(calldatas.length, extradata.length));
        for (uint256 i = 0; i < extradata.length; i++) {
            combined[i] = extradata[i];
        }
        for (uint256 j = 0; j < calldatas.length; j++) {
            combined[j + extradata.length] = calldatas[j];
        }
        assembly {
    ...
    function hashOrderPartOne(Order memory order)
        ...
        returns (bytes32)
    {
        return keccak256(abi.encodePacked(order.exchange, order.maker, order.taker, order.makerRelayerFee, order.takerRelayerFee, order.makerProtocolFee, order.takerProtocolFee, order.feeRecipient, order.feeMethod, order.side, order.saleKind, order.target, order.howToCall));
    }
    ...
    function hashOrderPartTwo(Order memory order)
        ...
        returns (bytes32)
    {
        return keccak256(abi.encodePacked(order.calldatas, order.replacementPattern, order.staticTarget, order.staticExtradata, order.paymentToken, order.basePrice, order.extra, order.listingTime, order.expirationTime, order.salt));
    }
        ...
        returns (bytes32)
    {
        ...
        bytes32 hash = keccak256(abi.encodePacked(prefix, hashOrderPartOne(order), hashOrderPartTwo(order)));
        return hash;
    }
        ...
        public
    {
        ...
        {
            emit OrderApprovedPartOne(hash, OrderApprovePartOne(order.exchange, order.maker, order.taker, order.makerRelayerFee, order.takerRelayerFee, order.makerProtocolFee, order.takerProtocolFee, order.feeRecipient, order.feeMethod, order.side, order.saleKind, order.target));
        }
        {   
            emit OrderApprovedPartTwo(hash, OrderApprovePartTwo(order.howToCall, order.staticTarget, order.paymentToken, order.basePrice, order.extra, order.listingTime, order.expirationTime, order.salt, orderbookInclusionDesired));
        }
    }
        ...
        returns (uint)
    {
        ...
        if (price > 0 && address(sell.paymentToken) != address(0)) {
            transferTokens(address(sell.paymentToken), buy.maker, sell.maker, price);
        }
        ...
        if (sell.feeRecipient != address(0)) {
            ...
            if (sell.feeMethod == FeeMethod.SplitFee) {
                ...
                if (sell.makerRelayerFee > 0) {
                    ...
                    if (address(sell.paymentToken) == address(0)) {
                        ...
                        transferTokens(address(sell.paymentToken), sell.maker, sell.feeRecipient, makerRelayerFee);
                    }
                }
                ...
                if (sell.takerRelayerFee > 0) {
                    ...
                    if (address(sell.paymentToken) == address(0)) {
                        ...
                        transferTokens(address(sell.paymentToken), buy.maker, sell.feeRecipient, takerRelayerFee);
                    }
                }
                ...
                if (sell.makerProtocolFee > 0) {
                    ...
                    if (address(sell.paymentToken) == address(0)) {
                        ...
                        transferTokens(address(sell.paymentToken), sell.maker, protocolFeeRecipient, makerProtocolFee);
                    }
                }
                ...
                if (sell.takerProtocolFee > 0) {
                    ...
                    if (address(sell.paymentToken) == address(0)) {
                        ...
                        transferTokens(address(sell.paymentToken), buy.maker, protocolFeeRecipient, takerProtocolFee);
                    }
                }
            ...
            if (sell.feeMethod == FeeMethod.SplitFee) {
                ...
                if (buy.makerRelayerFee > 0) {
                    ...
                    transferTokens(address(sell.paymentToken), buy.maker, buy.feeRecipient, makerRelayerFee);
                }

                if (buy.takerRelayerFee > 0) {
                    uint256 takerRelayerFee = SafeMath.div(SafeMath.mul(buy.takerRelayerFee, price), INVERSE_BASIS_POINT);
                    transferTokens(address(sell.paymentToken), sell.maker, buy.feeRecipient, takerRelayerFee);
                }

                if (buy.makerProtocolFee > 0) {
                    uint256 makerProtocolFee = SafeMath.div(SafeMath.mul(buy.makerProtocolFee, price), INVERSE_BASIS_POINT);
                    transferTokens(address(sell.paymentToken), buy.maker, protocolFeeRecipient, makerProtocolFee);
                }

                if (buy.takerProtocolFee > 0) {
                    uint256 takerProtocolFee = SafeMath.div(SafeMath.mul(buy.takerProtocolFee, price), INVERSE_BASIS_POINT);
                    transferTokens(address(sell.paymentToken), sell.maker, protocolFeeRecipient, takerProtocolFee);
                }
 ...
 * @dev Just for logging event (for site DB)
 */
