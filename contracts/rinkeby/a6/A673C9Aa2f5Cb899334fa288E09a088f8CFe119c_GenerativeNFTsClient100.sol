...
// NFT owner owns the copyright for this coin

contract GenerativeNFTsClient100 is Ownable, ERC721Enumerable, PaymentSplitter {

    uint public MAX_GEN100 = 500; //
    uint public GEN100_PRICE = 0.04 ether;
    ...
    //GenerativeNFTs Client 100 Release Time -
    uint public publicSale = 1638213080;   // Using Test Time for Rinkeby UTC 
    ...
    constructor(address[] memory distro, uint[] memory distro_shares, address[] memory teamclaim)
        ERC721("GenerativeNFTs Client 100", "GEN100")
        PaymentSplitter(distro, distro_shares)
    {
        ...
        // teamclaimNFT
        accounts[teamclaim[0]] = Account( 5, 0, 0, true); //
        accounts[teamclaim[1]] = Account( 5, 0, 0, true); //
        accounts[teamclaim[2]] = Account( 1, 0, 0, true); //
        accounts[teamclaim[3]] = Account( 1, 0, 0, true); //
        accounts[teamclaim[4]] = Account( 1, 0, 0, true); //
        accounts[teamclaim[5]] = Account( 1, 0, 0, true); //
        accounts[teamclaim[6]] = Account( 1, 0, 0, true); //
    ...
    function increaseReserved(uint _increaseReservedBy, address _addr) external onlyOwner {
        require(_reserved + totalSupply() + _increaseReservedBy <= MAX_GEN100, "This would exceed the max supply");
    ...
    function claimReserved(uint _amount) external onlyAdmin {
        ...
        require(totalSupply() + _amount <= MAX_GEN100, "You would exceed the max supply limit");
    ...
    function airDropNFT(address[] memory _addr) external onlyOwner {
        require(totalSupply() + _addr.length <= (MAX_GEN100 - _reserved), "You would exceed the airdrop limit");
    ...
    function mint(uint _amount) external payable noReentrant {
        ...
        require(totalSupply() + _amount <= (MAX_GEN100 - _reserved), "Purchase would exceed max supply of GEN100");
        require(msg.value >= (GEN100_PRICE * _amount), "Ether value sent is not correct");
