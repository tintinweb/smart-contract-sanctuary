...
contract Depictives is ERC721, Ownable, ReentrancyGuard {
    ...
    uint16 private constant MAXWELL_MINTS = 3;
    uint16 private constant APOSTLE_MINTS = 3;
    uint16 private constant AUGURY_MINTS = 5;
    uint16 private constant FIRST_ALLOW_LIST_MINTS = 2;
    uint16 private constant SECOND_ALLOW_LIST_MINTS = 1;

    uint256 private constant MAX_MULTI_MINT_AMOUNT = 5;
    uint256 private constant PRESALE_PRICE = 0.14 ether;
    uint256 private constant PUBLIC_PRICE = 0.15 ether;
    ...
    mapping(address => bool) firstAllowList;
    mapping(address => bool) secondAllowList;
    ...
    function setFirstAllowList(address[] calldata addresses) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            firstAllowList[addresses[i]] = true;
        }
    }

    function setSecondAllowList(address[] calldata addresses) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            secondAllowList[addresses[i]] = true;
        }
    }
    ...
    function _preSaleEligible(address addr, uint256 amount) internal view returns (bool) {
        ...
        if (maxwells.balanceOf(addr) > 0) {
            return true;
        }

        if (firstAllowList[addr]) {
            return true;
        }

        if (secondAllowList[addr]) {
            return true;
        }
    ...
    function _mintLimit(address addr) internal view returns (uint256) {
        uint256 numberOfMints = 0;
        if (apostleHolders[addr]) {
            numberOfMints += APOSTLE_FREE_MINTS + APOSTLE_MINTS;
        }

        if (auguryHolders[addr]) {
            numberOfMints += AUGURY_FREE_MINTS + AUGURY_MINTS;
        }

        uint256 maxwellBalance = maxwells.balanceOf(addr);
        if (apostleHolders[addr] && maxwellBalance > 1) {
            numberOfMints += (maxwellBalance - 1) * MAXWELL_MINTS;
        } else {
            numberOfMints += maxwellBalance * MAXWELL_MINTS;
        }

        if (firstAllowList[addr]) {
            numberOfMints += FIRST_ALLOW_LIST_MINTS;
        }

        if (secondAllowList[addr]) {
            numberOfMints += SECOND_ALLOW_LIST_MINTS;
        }

        return numberOfMints - balanceOf(addr);
    }
    ...
    function freeMint() public nonReentrant {
        require(_isPreSaleActive(), "SALE_NOT_ACTIVE");
    ...
    function mint(uint256 numberOfMints) public payable nonReentrant {
        ...
        require(_isPublicSaleActive() || _isPreSaleActive() && preSaleEligible, "SALE_NOT_ACTIVE");
        require(_isPublicSaleActive() && numberOfMints <= MAX_MULTI_MINT_AMOUNT ||
            _isPreSaleActive() && numberOfMints <= _mintLimit(msg.sender), "TOO_LARGE_PER_TX");
    ...
    function price() public view returns (uint256) {
        return _isPublicSaleActive() ? PUBLIC_PRICE : PRESALE_PRICE;
    }
}
