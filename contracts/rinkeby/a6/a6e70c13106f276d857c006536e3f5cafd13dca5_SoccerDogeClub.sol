    ...
    Ownable
{
    ...
    function random(uint256 a) public pure returns (uint) {
        return (uint(keccak256(abi.encodePacked(a,"sdnlkfedsajkhjk23278779829798djh"))));
    }
    ...
    function _getUri(uint256 _tokenId) internal pure returns (string memory) {
        return
            string(
                abi.encodePacked(URI_BASE, Strings.toString(random(_tokenId)),".txt"));
    }
