...
contract MoreOrLessVote is ERC721Enumerable, Ownable {
    ...
    uint public maxSupply = 20; // add 1 (so 1000) to account for mint 0
    ...
    Vote[21] public voteInfos; // add 2, so 1001 (token 0 and token 1000)
    MoreOrLessArt.Art[21] public artInfos; // add 2, so 1001 (token 0 and token 1000)
    ...
    function sealVote(address _to) public onlyOwner {
        require(totalSupply() == 20, "Voting must be complete.");
        uint256 mintNum = 20;
        _safeMint(_to, mintNum);
        voteInfos[20].votedBy = msg.sender;
    }
...
library MoreOrLessArt {
    ...
    function getColorPalette(uint256 seed, Art memory artData) private view returns(string[5] memory) {
    ...
    function _generateHeader(uint256 seed, Art memory artData) internal view returns (string memory) {
    ...
    function _generateCircles(Art memory artData) internal view returns (string memory) {
    ...
    function _generateSLines(uint256 seed, Art memory artData) internal view returns (string memory) {
      return string(abi.encodePacked(
        " S",
    ...
    function _generateLines(Art memory artData) internal view returns (string memory) {
        string memory lines = '';
        string[5] memory colorPalette = getColorPalette(artData.randomSeed, artData);
    ...
    function getTrianglePoints(uint256 seed, Art memory artData) private view returns (string memory) {
        return string(abi.encodePacked(
            seededRandom(0, 1000, seed + 1, artData).toString(),
            ",",
            seededRandom(0, 1000, seed + 2, artData).toString(),
    ...
    function _generateTriangles(Art memory artData) internal view returns (string memory) {
    ...
    function _generateRectangles(Art memory artData) internal view returns (string memory) {
