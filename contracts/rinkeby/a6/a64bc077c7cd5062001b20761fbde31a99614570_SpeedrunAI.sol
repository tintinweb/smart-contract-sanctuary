...
contract SpeedrunAI is ERC721Tradable {
		...
		returns (string memory)
	{
		...
		return string(abi.encodePacked(IPFS_BASE, placeholderURI, '.json'));
	}
	...
	/**
	 * @dev Sets the sales active up to the given series
	 * @notice This is exclusive
	 */
	function setSaleToSeries(uint256 newSaleToSeries) external onlyOwner {
		saleToSeries = newSaleToSeries;
	}
	...
	{
		require(
			amount <= MAX_TOKEN_PER_MINT,
			'SpeedrunAI: Too many tokens minted'
		);
