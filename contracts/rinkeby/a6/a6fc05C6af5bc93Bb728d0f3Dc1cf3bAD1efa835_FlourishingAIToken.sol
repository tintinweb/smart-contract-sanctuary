...
abstract contract ERC20VestableInTimestamp is ERC20Burnable, AccessControl {
    ...
    struct VestingSchedule {
        ...
        uint256 startDay;       // Timestamp of vesting schedule beginning.
    ...
    function inActiveSchedule(uint256 _id) internal {
        ...
        for (uint256 i = index; i < allActiveSchedules.length; i++) {
    ...
    function setWhiteList(address[] memory _whiteListAddresses, bool _status)
        public
        onlyGrantor
    {
        ...
        for (uint256 i = 0; i < _whiteListAddresses.length; i++) {
            require(
                ...
                "whitelist status is not change"
            );
            whiteLists[_whiteListAddresses[i]] = _status;
        }
    }
    ...
    function setEndTimeLock(uint256 _endTimeLock) public onlyGrantor {
        require(_endTimeLock > block.timestamp, "end lock time past");
    ...
    function updateVestingSchedule(
        ...
        vestingSchedule.scheduleName = _name;
    ...
    function _applyVestingSchedule(
        ...
        require(_vestingSchedule != 0, "invalid vesting schedule type");
        require(userVestingAmountInSchedule[_beneficiary][_vestingSchedule] == 0, "already applied vesting schedule");
        require(
            vestingSchedules[_vestingSchedule].isActive == true,
    ...
    function getAllSchedulesOfUser(address _beneficiary)
        public
        view
    ...
    function _getNotVestedAmount(
        ...
        // If after end of cliff + vesting, then the not vested amount is zero (all are vested).
            ...
            onDay >=
            vesting.startDay + (vesting.cliffDuration + vesting.duration)
        ) {
        ...
        // Otherwise a fractional amount is vested.
            ...
            uint256 daysVested = onDay - (vesting.startDay + vesting.cliffDuration);
    ...
    function _getNotVestedAmountForAllSchedules(
        ...
        uint256[] memory userSchedules = getAllSchedulesOfUser(_beneficiary);
