...
contract MysteryToken2 is ERC721 {
    ...
    uint16 public totalTokens = 10;
    uint16 public totalSupply = 0;
    string private baseURI = "ipfs:/UNSET/";
    bool public _paused = true; // making the variable public so that people can see if the sale is live or not
    bool private isFrozen = false;
    address public slicesContract = 0x000000000000000000000000000000000000dEaD;

    mapping(address => uint8) public mintedTokensPerWallet;
    mapping(uint16 => uint16) private tokenMatrix;

    constructor() ERC721("MysteryToken2", "MYST2") {
     ...
     */
    function devMint(address _address, uint16 amount) external onlyOwner {
        ...
        for (uint256 i; i < amount; i++) {
            tokenIds[i] = tmpTotalMintedTokens;
    ...
    function contractMint(address _address) external {
        require(msg.sender == slicesContract, "Only the other contract can mint...");
        require(getAvailableTokens() >= 1, "No tokens left to be minted...");

        uint16 tmpTotalMintedTokens = totalSupply;

        totalSupply += 1;

        _mint(_address, tmpTotalMintedTokens);
    }
     ...
     */
    function setSlicesContract(address _slicesContract) external onlyOwner {
        slicesContract = _slicesContract;
    }
