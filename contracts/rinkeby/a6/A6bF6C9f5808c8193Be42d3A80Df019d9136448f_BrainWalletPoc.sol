...
abstract contract PuzzleReward {
    function mint(address to) public virtual returns (uint256);
}

contract BrainWalletPoc is Pausable, Ownable {
    using ECDSA for bytes32;

    // opuzzle solver winner
    address public winner;

    // brainwallet address generated from puzzle answer
    address public key;
    ...
    function mint(bytes memory signature)
        ...
        whenNotPaused
    {
        // require(winner == address(0), "Puzzle has already been solved");
        require(key != address(0), "Puzzle key must be set to mint");

        bytes32 digest = generateDigest(msg.sender);
        address puzzleKey = recoverSigner(digest, signature);

        require(
            key == puzzleKey,
            "Sorry, your answer to the puzzle is incorrect."
        );
