...
// 灏嗘墜缁垂杞寲鎴恥sdt锛岃浆绉诲埌寮€鍙戝洟闃熷湴鍧€ 

contract MintDev is Ownable {
    ...
    // dev profit address
    address public immutable devTo;
    ...
    // F1 - F10: OK
    // C1 - C24: OK
    // All safeTransfer, _swap, _toUSDT, _convertStep: X1 - X5: OK
    function _convertStep(
        ...
        if (token0 == token1) {
            ...
            if (token0 == usdt) {
                IERC20(usdt).safeTransfer(devTo, amount);
                usdtOut = amount;
            } else if (token0 == weth) {
            ...
            // eg. USDT - ETH
            IERC20(usdt).safeTransfer(devTo, amount0);
            usdtOut = _toUSDT(token1, amount1).add(amount0);
        } else if (token1 == usdt) {
            // eg. DAI - USDT
            IERC20(usdt).safeTransfer(devTo, amount1);
    ...
    {
        // X1 - X5: OK
        amountOut = _swap(token, usdt, amountIn, devTo);
    }
}