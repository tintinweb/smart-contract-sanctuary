...
contract NFT is ERC721, PaymentSplitter {
  ...
  uint public constant presalePrice = 0.01 ether; // TODO set final value
  uint public constant startPriceDA = 0.018 ether; // TODO set final value
  uint public constant endPriceDA = 0.011 ether; // TODO set final value
  uint public constant reservedAmount = 50; 
  uint public constant maxMintsPerAddressPresale = 1;
  uint public constant maxMintsPerAddressSale = 10;
  uint public reservedMintedAlready;
  uint public dWC;

  uint public presaleStartDate = block.timestamp;//block.timestamp; // TODO change to correct date
  uint public saleStartDate = presaleStartDate + 1 minutes ; //block.timestamp + 1 seconds; // TODO change to correct duration
  ...
  function calculateTokenPriceDA() public view returns(uint) {
    ...
    uint elapsedIntervalsDA = (block.timestamp - saleStartDate) / 60; //TODO set final time value
    if (elapsedIntervalsDA >=  7){
      return endPriceDA;
    }
    else {
      return startPriceDA - (0.001 ether * elapsedIntervalsDA);   //TODO set final value
    }
  }
  ...
  function isSaleOpen() public view returns(bool){
    ...
    && block.timestamp <= saleStartDate + 1 days){ // TODO change to correct duration
      return true;
    }
    else{
      return false;
  ...
  function setDogeMerkleRoot(bytes32 root) external onlyTeam {
