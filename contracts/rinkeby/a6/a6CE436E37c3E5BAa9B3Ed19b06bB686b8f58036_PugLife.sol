...
contract PugLife is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    uint256 public liquidityFeeToSell = 10 * 10**18;
    uint256 public marketingFeeToSell = 10 * 10**18;
    ...
    function _swapAndLiquify(address from) private {
        ...
        if (
            from != uniswapV2Pair && (marketingFeeBalance >= marketingFeeToSell || liquidityFeeBalance >= liquidityFeeToSell)
        ) {
            bool initialFeeState = _enableFee;
            // remove fee if initialFeeState was true
            if(initialFeeState) _enableFee = false;

            if(contractTokenBalance >= marketingFeeToSell && marketingFeeBalance >= marketingFeeToSell) {
                transferToFeeWallet(marketing_address, marketingFeeBalance);
            }

            //add liquidity
            if(contractTokenBalance >= liquidityFeeBalance && !inSwapAndLiquify && swapAndLiquifyEnabled && liquidityFeeBalance >= liquidityFeeToSell) {
