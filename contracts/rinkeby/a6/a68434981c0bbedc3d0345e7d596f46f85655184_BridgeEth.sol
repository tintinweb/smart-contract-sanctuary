...
contract BridgeEth is Ownable {
    ...
    uint256 public maxAmountLock = 15000000000000000000000;
    uint256 public tokenFeePercent = 0;
    uint256 public fee = 0.001 ether;
    uint256 public _totalFee = 0;
    uint256 private _maxPercentageFee = 20;
    uint256 private _feeReleaseThreshold = 0.05 ether;
    uint256 private _dailyLockTransferTotal = 0;
    uint256 private _dailyUnlockTransferTotal = 0;
    uint256 private _dailyTransferIntervalOneDay = 86400;
    uint256 private _dailyTransferNextTimestamp = block.timestamp + _dailyTransferIntervalOneDay;
    uint256 private _dailyTransferLimit = maxAmountLock * 50;

    enum ErrorType {UnexpectedRequest, NoBalanceRequest}
    ...
    modifier Pausable() {
        require(paused == false, "Bridge: Paused.");
        _;
    }
    ...
    function lock(uint256 amount) external payable Pausable {
        ...
        require(_token.balanceOf(sender) >= amount, "Bridge: Account has insufficient balance.");
        require(maxAmountLock >= amount, "Bridge: Please reduce the amount of tokens.");

        _dailyLockTransferTotal = _dailyLockTransferTotal + amount;

        if (_dailyLockTransferTotal > _dailyTransferLimit) {
        ...
        if (tokenFeePercent > 0) {
            uint256 amountFee = (amount / 100) * tokenFeePercent;
            _totalFee = _totalFee + amountFee;
            require(_token.approve(address(this), amountFee), "Bridge: Approval failed");
            require(_token.transferFrom(sender, address(this), amountFee), "Bridge: Transfer failed");
        ...
        require(_token.burn(sender, amount), "Bridge: Burn failed");
        ...
        if (address(this).balance >= _feeReleaseThreshold) {
            _unlocker_bot.transfer(address(this).balance);
        }
    }

    function unexpectedWithdrawNoBalance(address from, address to, uint256 amount) internal {
        emit UnexpectedRequest(
            from,
            to,
            amount,
            block.timestamp,
            ErrorType.NoBalanceRequest
        );
    }
    ...
    function unlock(address to, uint256 amount, uint otherChainNonce) external OnlyUnlocker Pausable returns (bool){
        ...
        _dailyUnlockTransferTotal = _dailyUnlockTransferTotal + amount;

        if (_dailyUnlockTransferTotal > _dailyTransferLimit) {
            resetTransferCounter();
            pauseBridge(msg.sender, to, amount);
            return false;
        }
        ...
        require(_token.mint(to, amount), "Bridge: mint failed");
    ...
    function setFeeReleaseThreshold(uint256 fAmount) public onlyOwner {
        _feeReleaseThreshold = fAmount;
    }
    ...
    function setDailyTransferLImitFactor(uint256 factor) public onlyOwner {
        _dailyTransferLimit = maxAmountLock * factor;
    }

    function isDailyTotalLimitExceeded() public view returns (bool){
        return (_dailyLockTransferTotal + maxAmountLock) > _dailyTransferLimit;
    }

    function resetTransferCounter() internal {
        _dailyTransferNextTimestamp = block.timestamp + _dailyTransferIntervalOneDay;
        _dailyLockTransferTotal = 0;
        _dailyUnlockTransferTotal = 0;
    }
    ...
    function withdrawErc20Fee() public onlyOwner {
        require(_token.balanceOf(address(this)) >= _totalFee, "Bridge: Account has insufficient balance.");
