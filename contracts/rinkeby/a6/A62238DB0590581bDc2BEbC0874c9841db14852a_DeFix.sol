...
//    address USDT = 0xdAC17F958D2ee523a2206206994597C13D831ec7;
    address USDT = 0xc778417E063141139Fce010982780140Aa0cD5Ab;
    ...
    /*
                    uint256 balanceInTokensBefore = IERC20(shares[i].tokenAddress).balanceOf(address(this));
                    uint256 bnbReceived = swapTokensForBnb(shares[i].tokenAddress, tokensToSell);
                    swappedTokens = balanceInTokensBefore - IERC20(shares[i].tokenAddress).balanceOf(address(this));
        /**
         * @notice Swaps passed tokens for BNB using Pancakeswap router and returns
         * actual amount received.
         */
    function swapTokensForBnb(address tokenAddress, uint256 tokenAmount) internal swapping pcsInitialized returns (uint256, uint256) {
    ...
    function calculateTokensPriceInUSDT(address tokenAddress, uint256 tokenAmount) internal swapping pcsInitialized returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = tokenAddress;
        path[1] = USDT;
...
interface IDeFiX {
    ...
    function getInvested() external view returns (uint256[] memory, uint256[] memory);
    ...
    // покупка и продажа
    function investIn(uint256 _projectId) external payable returns (uint256); // {from: account, value: value}
    function withdraw(uint256 _projectId, uint256 _percent) external returns (uint256);
}
...
contract DeFix is IDeFiX, Uniswap {
    ...
    function createProject(
        ...
        projects[projectCount].minInvestInBnb = _minInvestInBnb;

        uint256 totalPoints;
        for (uint256 i = 0; i < _addrList.length; i++) {
            require(_addrList[i] != USDT, 'USDT can not be added to index');
    ...
    function getInvested() external view override returns (uint256[] memory, uint256[] memory) {
    ...
    function getWithdrawable(uint256 _projectId, address _sender) external view override returns (uint256) {
        ...
        for (uint256 i = 0; i < shares.length; i++) {
            ...
            if (router.WETH() != shares[i].tokenAddress) {
                address[] memory path = new address[](2);
                path[0] = shares[i].tokenAddress;
                path[1] = router.WETH();
                availableToWithdraw += router.getAmountsOut(investedAmount, path)[1];
            } else {
                availableToWithdraw += investedAmount;
            }
        }
        return availableToWithdraw;
    }

    function investIn(uint256 _projectId) external payable override returns (uint256) {
        Share[] memory shares = projects[_projectId].tokens;
        uint256 investedInUSDT;
        uint256 oneSharePart = msg.value / projects[_projectId].totalPoints;
        uint256 tokenReceived;
        for (uint256 i = 0; i < shares.length; i++) {
            uint256 tokensToBuy = oneSharePart * shares[i].point;
            tokenReceived = swapBnbForTokens(shares[i].tokenAddress, tokensToBuy);
    ...
    function withdraw(uint256 _projectId, uint256 _percent) external override returns (uint256) {
        Share[] memory shares = projects[_projectId].tokens;
...
//https://eth-mainnet.alchemyapi.io/v2/30UAJfYxx3apD2NTAHPiaGCdC8dehf6s
