...
contract IDOPool is IPool, Managed {
    ...
    function setKYC(bool value_)
        external
        override
        canCallOnlyRegisteredContract(CONTRACT_POOL_REGISTER)
    {
        isKYC = value_;
    }
    ...
    function startVesting() external override requirePermission(ROLE_ADMIN) {
    ...
    function emergencyFunction() external override requirePermission(ROLE_ADMIN) {
        uint256 balance = address(this).balance;
...
interface IPool {
    ...
    function getVestingInfo() external returns (
            bool withoutVesting,
            uint256 delay,
            uint256 availiableInStart,
            uint256 percentPerBlock,
    ...
    function getRewardsTokenInfo() external returns (RewardTokenInfo[] memory rewardsInfo);

    function getTokenPriceInfo() external returns (address[] memory tokens, uint256[] memory pricePerToken);
