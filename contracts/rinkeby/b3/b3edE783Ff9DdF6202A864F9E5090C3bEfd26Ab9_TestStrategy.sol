...
contract TestStrategy is StratX2 {
  
    constructor(
        address[] memory _addresses,
        uint256 _pid,
        bool _isCAKEStaking,// false
        bool _isSameAssetDeposit,//f
        bool _isTENFIComp,//f
        address[] memory _earnedToTENFIPath,//lp1 token second -> lp2, 3->lp3 0x85d1dF7cd242feD99D79D9731bc07af8F3b5c4dE(lp1)
        address[] memory _earnedToToken0Path,//same as tenfi path 
        address[] memory _earnedToToken1Path,//same as above 
        address[] memory _token0ToEarnedPath,// lp1
        address[] memory _token1ToEarnedPath//  bnb
    )  {
        
        wbnbAddress = _addresses[0];// wbnb
        govAddress = _addresses[1]; //0xd4f6cb0c1fe07407b7098ac7fe4265f3b2ae61f2// timelock
        TENFIFarm = _addresses[2]; // we have that
        TENFIAddress = _addresses[3]; // we have this too

        wantAddress = _addresses[4]; // pair address
        token0Address = _addresses[5];// lp1
        token1Address = _addresses[6];// bnb
        earnedAddress = _addresses[7];//reward token 

        farmContractAddress = _addresses[8];// given to use
        pid = _pid;
        isCAKEStaking = _isCAKEStaking;
        isSameAssetDeposit = _isSameAssetDeposit;
        isTENFIComp = _isTENFIComp;
        
        uniRouterAddress = _addresses[9];// uniswap router address
        ...
        TENGas_Fee = 80;
        rewardsAddress = _addresses[10];//self address, user address
        TENBurn = 300;
        buyBackAddress = _addresses[11];//self addrress (doubt raise)* default dead addrress 1431
        entranceFeeFactor = 9990;
        withdrawFeeFactor = 10000;
        transferOwnership(TENFIFarm);
    }
}
//alloc point  random value :100