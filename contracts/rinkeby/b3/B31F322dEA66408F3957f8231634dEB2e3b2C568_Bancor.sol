...
// File: original_contracts/lib/bancor/IBancorNetworkPathFinder.sol
...
contract Bancor is IExchange, AdapterStorage {
    using SafeMath for uint256;
  ...
  function initialize(bytes calldata data) external override {
        ...
        returns (uint256)
    {
        ...
        IERC20[] memory path = IBancorNetworkPathFinder(lData.pathFinder).generatePath(
            address(fromToken) == Utils.ethAddress() ? IERC20(lData.etherToken) : fromToken,
            address(toToken) == Utils.ethAddress() ? IERC20(lData.etherToken) : toToken
        );
        address bancorNetwork = IContractRegistry(lData.contractRegistry).addressOf(
          BANCOR_NETWORK
        );
    ...
    function getKey() public override pure returns(bytes32) {
      return keccak256(abi.encodePacked("Bancor", "1.0.0"));
    }
