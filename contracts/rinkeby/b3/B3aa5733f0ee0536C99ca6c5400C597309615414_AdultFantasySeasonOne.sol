    ...
    IERC2981
{
    ...
    function purchaseTokenMintPass(
        ...
        require(
            isValidSignedHex(randomHexes[0], signedRandomHexes[0]),
        ...
        mintMultiple(1, randomHexes, msg.sender);
    }
    ...
    function purchaseToken(
        ...
        for (uint256 index = 0; index < signedRandomHexes.length; index++) {
            bytes32 randomHex = randomHexes[index];
            bytes memory signedRandomHex = signedRandomHexes[index];
            require(
                isValidSignedHex(randomHex, signedRandomHex),
                "Random Hex is invalid"
            );
            mintCounts[msg.sender]++;
        }

        mintMultiple(purchaseCount, randomHexes, msg.sender);
    }
    ...
    function batchPurchaseReservedTokenMintPass(
        ...
        require(
            randomHexes.length == signatures.length,
        ...
        mintMultiple(guids.length, randomHexes, _addr);
    }
    ...
    function isValidSignedHex(bytes32 randomHex, bytes memory signedRandomHex)
        public
        view
        returns (bool)
    {
        bytes32 hash = keccak256(
            abi.encodePacked(randomHex, contractIdentifier, mintCounts[msg.sender], msg.sender)
        );
    ...
    function mintMultiple(
        ...
        for (uint256 index = 0; index < quantity; index++) {
            ...
            MintedCard storage card = mintedCards[_id];
            card.characterID = characterID;
            card.specialSauceCode = randomHexes[index];
