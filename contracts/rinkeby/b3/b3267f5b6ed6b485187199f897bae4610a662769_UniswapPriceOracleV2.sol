...
contract UniswapConfig {
    enum PriceSource {
        ...
        REPORTER_ETH,   /// 2: implies the price is set by the reporter, not support now
...
contract UniswapPriceOracleV2 is UniswapConfig {
    ...
    constructor(uint usdtBaseUnit_, uint anchorPeriod_,
    ...
    UniswapConfig(gTokens_, underlyings_, symbolHashs_, baseUints_, priceSources_, fixedPrices_, uniswapMarkets_,
        ...
        for (uint i = 0; i < gTokens_.length; i++) {
            ...
            if (config.priceSource == PriceSource.REPORTER_ETH || config.priceSource == PriceSource.REPORTER_USDT) {
    ...
    function priceInternal(TokenConfig memory config) internal view returns (uint) {
        if (config.priceSource == PriceSource.REPORTER_ETH || config.priceSource == PriceSource.REPORTER_USDT) return prices[config.symbolHash];
    ...
    */
    ...
    /*
        ...
        else if(config.priceSource == PriceSource.REPORTER_USDT) {
