...
contract Bridge is Ownable, ReentrancyGuard {
    ...
    constructor(uint256 _chainId, address _tokenAddress, address _verifierAddress) public {
        chainId = _chainId;
    ...
    mapping (bytes32 => WithdrawalWallet) public depositHash;
    ...
    function setVerifyAddress(address _newVerifierAddress) public {
        require(governance == msg.sender, "Unauthorized Action");
    ...
    function deposit(uint256 _amount) external noContractsAllowed nonReentrant {
        ...
        depositWallets[msg.sender]._index = lastDepositIndex;
        depositWallets[msg.sender]._amount = _amount;
        depositWallets[msg.sender]._chainId = chainId;
        depositWallets[msg.sender]._address = msg.sender;
        depositWallets[msg.sender]._depositedBlock = block.number;
        depositWallets[msg.sender]._depositedTimeStamp = block.timestamp;
    ...
    function withdraw(uint256 _amount, uint256 _chainId, uint256 _nonce, bytes32 _txHash, uint8 _v, bytes32 _r, bytes32 _s) external noContractsAllowed nonReentrant {
        ...
        withdrawalWallets[msg.sender]._address = msg.sender;
	...
	function checkDeposit(address _address) public view returns (uint256, uint256, uint256, address, uint256, uint256) {
	...
	function checkWithdrawal(address _address) public view returns (uint256, uint256, uint256, address, bytes32, bool) {
