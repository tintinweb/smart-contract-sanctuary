...
contract BoxV2 is Box {
    ...
    function setName(string memory _name) public {
        daname = _name;
        emit NameChanged(daname);
    }

    function approveTask(string memory _taskName) public {
        AlxApproval memory newApproval = AlxApproval({
    ...
    function getApprovedTasks() public view returns (AlxApproval[] memory) {
        return approvals;
    }
...
contract Box is ERC721Upgradeable {
    uint256 private _value;

    function initialize(uint256 value)  initializer public {
        __ERC721_init("My Box Collectible", "BOX");
        _value = value;
    }
