...
contract RestrictedWindow is Ownable {
    // Windows can have a configurable grace period up to 7 days.
...
contract PVComicBurn is
    ERC721("PixelVault Founder Token", "PVFT"),
    ERC721Enumerable,
    ReentrancyGuard,
    RestrictedWindow
{
    ...
    function swap(uint256[] calldata tokenIds) external nonReentrant inWindow {
        ...
        for (uint256 i; i < count; i++) {
            ...
            // ComicMinter.burningEnabled must be true; toggleBurningEnabled() to set.
            // This contract must be approved either by token or for all tokens.
