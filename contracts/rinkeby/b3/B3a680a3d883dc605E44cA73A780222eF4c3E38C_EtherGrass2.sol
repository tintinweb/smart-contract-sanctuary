...
contract EtherGrass2 is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
  ...
  uint256[50] private ETHERGRASS_R_IDS = [
  ...
  function preSaleMint(address _to, uint256 _count) public payable {
    ...
    uint256 total = _totalSupply();
    uint256 preSaleMintAmountAllowedLeft = preSaleMintAmountAllowed(msgSender);
    uint256 preSaleMintAmountUsed = _presaleAddressMintCount[msgSender];

    require(preSaleMintAmountAllowedLeft > 0, "No More Premint Items Available");
    require(_count <= preSaleMintAmountAllowedLeft, "Too Many Premint Items Requested");
    require(total + _count <= MAX_ELEMENTS, "Not Enough Available");
    require(total <= MAX_ELEMENTS, "Sold Out");
    require(_count <= MAX_BY_MINT, "Mint Amount Exceeded");
    require(msg.value >= price(_count), "Value Below Price");
  ...
  function ethergrassOwned(address _address) public view returns (uint256) {
    ...
    uint256[] memory ethergrassIds = new uint256[](50);
