...
contract FlurksMarket is ReentrancyGuard, Pausable, Ownable {
    ...
    address stoneRinkebyAddress = 0x0020D49D4941643043E57E72f28Dc6b0bD80FAdc;
    address scatterRinkebyAddress = 0xA8843925178A3159f8e4e38CE19687c24Ad67fc2;
    ...
    function buyFlurk(uint flurkIndex) payable public whenNotPaused nonReentrant() {
        ...
        // SCATTER CUSTOM CODE START
        uint stoneRoyaltiesTotal = msg.value / 20;
        uint scatterRoyaltiesTotal = msg.value / 40;
        pendingWithdrawals[stoneRinkebyAddress] += stoneRoyaltiesTotal;
        pendingWithdrawals[scatterRinkebyAddress] += scatterRoyaltiesTotal;
        uint newValue = msg.value - stoneRoyaltiesTotal - scatterRoyaltiesTotal;
        // SCATTER CUSTOM CODE END (We just use newValue instead of msg.value)
        flurksOfferedForSale[flurkIndex] = Offer(false, flurkIndex, msg.sender, 0, address(0x0));
    ...
    function enterBidForFlurk(uint flurkIndex) payable public whenNotPaused nonReentrant() {
        ...
        uint newValue = msg.value - stoneRoyaltiesTotal - scatterRoyaltiesTotal;
        // SCATTER CUSTOM CODE END (We just use newValue instead of msg.value)
        flurkBids[flurkIndex] = Bid(true, flurkIndex, msg.sender, newValue);
        emit FlurkBidEntered(flurkIndex, newValue, msg.sender);
    }
