...
// File: contracts/Interface/ISortedTroves.sol
...
// File: contracts/Interface/ILiquityBase.sol
...
// File: contracts/Interface/IStabilityPool.sol
 ...
 *
 * --- MERU ISSUANCE TO STABILITY POOL DEPOSITORS ---
 *
 * An MERU issuance event occurs at every deposit operation, and every liquidation.
 *
 * Each deposit is tagged with the address of the front end through which it was made.
 *
 * All deposits earn a share of the issued MERU in proportion to the deposit as a share of total deposits. The MERU earned
 * by a given deposit, is split between the depositor and the front end through which the deposit was made, based on the front end's kickbackRate.
 *
 * Please see the system Readme for an overview:
 * https://github.com/liquity/dev/blob/main/README.md#MERU-issuance-to-stability-providers
 */
interface IStabilityPool {
    ...
    event StabilityPoolMUSDBalanceUpdated(uint _newBalance);
    ...
    event ETHGainWithdrawn(address indexed _depositor, uint _ETH, uint _MUSDLoss);
    event MERUPaidToDepositor(address indexed _depositor, uint _MERU);
    event MERUPaidToFrontEnd(address indexed _frontEnd, uint _MERU);
     ...
     * - _amount is not zero
     * ---
     * - Triggers a MERU issuance, based on time passed since the last issuance. The MERU issuance is shared between *all* depositors and front ends
     * - Tags the deposit with the provided front end tag param, if it's a new deposit
     * - Sends depositor's accumulated gains (MERU, ETH) to depositor
     * - Sends the tagged front end's accumulated MERU gains to the tagged front end
     ...
     * - Sends all depositor's accumulated gains (MERU, ETH) to depositor
     * - Sends the tagged front end's accumulated MERU gains to the tagged front end
     ...
     * ---
     * - Triggers a MERU issuance, based on time passed since the last issuance. The MERU issuance is shared between *all* depositors and front ends
     * - Sends all depositor's MERU gain to  depositor
     * - Sends all tagged front end's MERU gain to the tagged front end
    ...
    /*
     * Calculate the MERU gain earned by a deposit since its last snapshots were taken.
     * If not tagged with a front end, the depositor gets a 100% cut of what their deposit earned.
     * Otherwise, their cut of the deposit's earnings is equal to the kickbackRate, set by the front end through
     * which they made their deposit.
     */
    function getDepositorMERUGain(address _depositor) external view returns (uint);

    /*
     * Return the MERU gain earned by the front end.
     */
    function getFrontEndMERUGain(address _frontEnd) external view returns (uint);

    /*
     * Return the user's compounded deposit.
     */
    function getCompoundedMUSDDeposit(address _depositor) external view returns (uint);
...
// File: contracts/Interface/IMUSD.sol


interface IMUSD is IERC20, IERC2612 { 
    ...
    event MUSDTokenBalanceUpdated(address _user, uint _amount);
...
// File: contracts/Interface/IMERUToken.sol


interface IMERUToken is IERC20, IERC2612 { 
   
    // --- Events ---
    
    event CommunityIssuanceAddressSet(address _communityIssuanceAddress);
    event MERUStakingAddressSet(address _MERUStakingAddress);
    event LockupContractFactoryAddressSet(address _lockupContractFactoryAddress);

    // --- Functions ---
    
    function sendToMERUStaking(address _sender, uint256 _amount) external;
...
// File: contracts/Interface/IMERUStaking.sol

interface IMERUStaking {

    // --- Events --
    
    event MERUTokenAddressSet(address _MERUTokenAddress);
    ...
    event TotalMERUStakedUpdated(uint _totalMERUStaked);
    ...
    function stake(uint _MERUamount) external;

    function unstake(uint _MERUamount) external;

    function increaseF_ETH(uint _ETHFee) external; 

    function increaseF_MUSD(uint _MERUFee) external;  
...
// File: contracts/Interface/ITroveManager.sol
...
interface ITroveManager is ILiquityBase {
    ...
    event MERUTokenAddressChanged(address _MERUTokenAddress);
    event MERUStakingAddressChanged(address _MERUStakingAddress);

    event Liquidation(uint _liquidatedDebt, uint _liquidatedColl, uint _collGasCompensation, uint _MUSDGasCompensation);
    event Redemption(uint _attemptedMUSDAmount, uint _actualMUSDAmount, uint _ETHSent, uint _ETHFee);
    ...
    event LTermsUpdated(uint _L_ETH, uint _L_MUSDDebt);
    event TroveSnapshotsUpdated(uint _L_ETH, uint _L_MUSDDebt);
    ...
    function setAddresses(
        ...
        address _sortedTrovesAddress,
        address _MERUTokenAddress,
        address _MERUStakingAddress
    ) external;

    function stabilityPool() external view returns (IStabilityPool);
    function musdToken() external view returns (IMUSD);
    function MERUToken() external view returns (IMERUToken);
    function MERUStaking() external view returns (IMERUStaking);
    ...
    function getPendingMUSDDebtReward(address _borrower) external view returns (uint);
    ...
    function getBorrowingFee(uint MUSDDebt) external view returns (uint);
...
// File: contracts/Interface/IBorrowerOperations.sol
...
interface IBorrowerOperations {
    ...
    function openTrove(uint _maxFee, uint _MUSDAmount, address _upperHint, address _lowerHint) external payable;
    ...
    function withdrawMUSD(uint _maxFee, uint _amount, address _upperHint, address _lowerHint) external;

    function repayMUSD(uint _amount, address _upperHint, address _lowerHint) external;
