...
contract InsuracePositionController is AbstractController {
    ...
    function withdrawRewards() external override {
        (uint256 _fullRewards,  )= _rewardController.getRewardInfo();

        (uint256 _rewards, uint256 _fee) = _calculateRewards();

        _rewardController.withdrawReward(_fullRewards);

        require(
            IERC20(insurToken).balanceOf(address(this)) == _fullRewards,
    ...
    function outstandingRewards() public view override returns (uint256) {
        return  _stakersPool.stkRewardsPerAPerLPT(lpToken,address(this));
    ...
    function apy() external view override returns (uint256 _apy) {
        //TODO
        // is if same what _calculatePoolAPY does?
        // note: return value is expected as % with with 10**5 precision
        _apy = _calculatePoolAPY(stakingAsset).div(PERCENTAGE_100);
    }
     ...
     */
    function _calculateRewardsPerBlock(address _tokenAddress)
        internal
        view
     ...
     */
    function _calculatePoolAPY(address stakingtokenAddress) internal view returns (uint256) {
        ...
        // R = RWRD per Block
        uint256 data = _calculateRewardsPerBlock(stakingtokenAddress);
        uint256 rewards = abi.decode(abi.encodePacked(data), (uint256));

        // P stakingAsset = Price of 1 token stakingAsset to base investment token
        ...
        uint256 stakingTokenPrice = _priceFeed.howManyTokensAinB(address(base), stakingAsset, 1);

        // V = Pool TVL
        uint256 poolTVL = _stakersPool.stakedAmountPT(stakingAsset);
    ...
    function rewardsInfo()
        public
        view
        returns (uint256 withdrawableReward, uint256 vestingReward)
    {
        (withdrawableReward, vestingReward) = _rewardController.getRewardInfo();
    }
    ...
    function _stakedAmount() internal view returns(uint256 _stakedAmount){
        uint256 _stakedAmountPT = _stakersPool.stakedAmountPT(stakingAsset);
        uint256 _lpTotalSupply  = IERC20(lpToken).totalSupply();

        uint256 _lpTokenStakedRatioAmt = _lpTotalSupply.div(_stakedAmountPT);
    }
...
interface IStakersPoolV2 {
    ...
    function stkRewardsPerAPerLPT(address _lpToken, address _account) external view returns (uint256 _stkRewards);
    
    function harvestedRewardsPerAPerLPT(address _lpToken,address _account) external view returns (uint256 _harvestedRewards);
