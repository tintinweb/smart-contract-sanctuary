...
contract EscrowV1 is EscrowContract, AmendablePreSigned, WithPreSignedMilestones {
    ...
    function executeSettlement(bytes32 _cid, uint16 _index) external {
        ...
        if (_index > MILESTONE_INDEX_BASE) {
            ...
            if (_ruling == RULE_PAYER_WON || _ruling == RULE_PAYEE_WON) {
                _distributeByProportion(_cid, _mid, disputer.ARBITER(), _available, _refundedPercent, _releasedPercent, _m);
            }
        } else {
     ...
     */
    function _distributeByProportion(
        bytes32 _cid,
        bytes32 _mid,
        address _arbiter,
        uint256 _available,
...
abstract contract WithMilestones is ContractContext, MilestoneContext {
     ...
     */
    function _fundMilestone(bytes32 _cid, bytes32 _mid, Milestone memory _m, uint _amountToFund) internal returns(bool) {
        uint _fundedAmount = _m.fundedAmount;
        require(_amountToFund > 0 && _m.amount >= (_fundedAmount + _amountToFund), ERROR_FUNDED);
        require(_m.paymentToken.transferFrom(msg.sender, address(this), _amountToFund), ERROR_FUNDING);

        if (_m.treasury != address(this)) {
            require(_m.paymentToken.approve(_m.treasury, _amountToFund), ERROR_FUNDING);
...
/**
 * @dev Collection of functions related to the address type (taken from OZ)
 */
library Address {
    /**
