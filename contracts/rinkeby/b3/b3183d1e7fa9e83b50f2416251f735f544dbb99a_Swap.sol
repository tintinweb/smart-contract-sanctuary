...
contract Swap {
    ...
    function buy(address token, uint256 amountIn, uint256 slippage, uint256 deadline) external onlyOwner {
        ...
        router.swapExactTokensForTokens(amountIn, amountOut * (1000 - slippage) / 1000, path, address(this), deadline);
    }

    function sell(address token, uint256 slippage, uint256 deadline) external onlyOwner {
