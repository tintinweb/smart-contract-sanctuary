...
contract LightsOut is ERC721, Ownable {
    using Strings for uint256;
    ...
    uint256 public constant MAX_TOTAL_MINT_AMOUNT = 200;
    uint256 public constant MAX_MINT_AMOUNT = 100;
    uint256 public constant WHITELIST_MAX_MINT_AMOUNT = 5;
    ...
    event NewLightsOutMinted(address sender, uint256 tokenId);
    ...
    modifier mustPassChecks(uint256 _mintAmount) {
        ...
        require(
            !isPaused &&
                ...
                supply + _mintAmount <= MAX_SUPPLY - marketingSupply,
            isPaused
                ? "PS0"
                : (
                    _mintAmount == 0
                        ? "M0"
                        : (_mintAmount > MAX_MINT_AMOUNT ? "M2" : "M3")
                )
        );
        ...
        if (msg.sender != owner()) {
            if (isOnlyWhitelisted == true) {
                require(
                    ...
                    _mintAmount > WHITELIST_MAX_MINT_AMOUNT ? "WL2" : "WL3"
                );
            }

            require(
                ownerMintedCount + _mintAmount <= MAX_TOTAL_MINT_AMOUNT &&
    ...
    function _processMint(uint256 _mintAmount) internal {
        for (uint256 i = 1; i <= _mintAmount; i++) {
            ...
            emit NewLightsOutMinted(msg.sender, id);
            supplyCounter.increment();
        }
    }
    ...
    {
        require(isOnlyWhitelisted == false || msg.sender == owner(), "M4");
        ...
        mustPassChecks(_mintAmount)
    {
        require(
            ...
            !isOnlyWhitelisted ? "WL1" : "WL0"
        );
        _processMint(_mintAmount);
    }

    function setIsRevealed(bool _status) public onlyOwner {
        if (_status) require(bytes(baseURI).length > 0, "BU0");
        isRevealed = _status;
    }
    ...
    function airdrop(address[] calldata _users) public payable onlyOwner {
        ...
        require(
            marketingSupply > 0 &&
                ...
                _users.length > 0 &&
                (supply + _users.length - 1 < MAX_SUPPLY - marketingSupply),
            marketingSupply <= 0
                ? "AD0"
                : (
                    _users.length > marketingSupply
                        ? "AD1"
                        : (_users.length == 0 ? "AD2" : "AD3")
                )
        );
        ...
        for (uint256 i = 0; i < _users.length; i++) {
            ...
            emit NewLightsOutMinted(_users[i], id);
            supplyCounter.increment();
            marketingSupply--;
        }
    }
