...
/**
 * @title DealCollectWallet contract
 * @dev Deal contract without distribution
 */
contract DealCollectWallet is DealBase {
    ...
    function __DealCollectWallet_init(
        IVerifier _verifier,
        ILocker _locker,
    ...
    /// @param _wallet wallet for emission (can be another chain)
    /// @param _signature signature of request
    function pay(uint256 _amount, string memory _wallet, bytes memory _signature) payable external nonReentrant isActive whenNotPaused {
        ...
        emit TokensDebt(msg.sender, _wallet, _amount, tokenAmount);
    }
    ...
    function getTokenAmount(uint256 _paymentAmount) public view returns (uint256) {
        return _paymentAmount.mul(1e18).div(tokenPrice); // decimals=18
    }

    /// @dev Update max distributed token amount
