...
contract HashSale is Ownable {
	...
	event NewSale(uint indexed saleId, address indexed customerAddress, uint payAmount, uint tokenAmount, uint8 stage, uint stageSales, bool isSaleFinished, string referral);
	...
	function buyTokens(bytes calldata _agreementSignature, string calldata _referral) payable public {
	    require(!isSaleFinished(), "The sale is over");
		...
		emit NewSale(saleId, msg.sender, msg.value, tokenAmount, currentStage, stages[currentStage].tokensSold, isFinished, _referral);
	}
