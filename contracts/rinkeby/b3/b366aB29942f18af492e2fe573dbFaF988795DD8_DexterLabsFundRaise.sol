...
contract DexterLabsFundRaise is ERC1155 {
    ...
    mapping(uint256 => address) tokenCreatorMapping;
    ...
    constructor() ERC1155("https://cdn.dexterlabsfundraise.com/") {
    ...
    function getNFTTokenMapping(uint256 tokenNumber) public pure returns(uint256) {
        return tokenNumber + 1;
    }

    function getTokenNFTMapping(uint256 tokenNumber) public pure returns(uint256) {
        return tokenNumber - 1;
    }
    ...
    function setURI(string memory url) public onlyGovernance {
        _setURI(url);
    }

    function startAuction(uint256 tokenNumber) public onlyGovernance {
    ...
    function pauseAuction(uint256 tokenNumber) public onlyGovernance {
        require(auctionState[tokenNumber] == AuctionState.RUNNING);
        auctionState[tokenNumber] = AuctionState.PAUSED;
    }

    function unpauseAuction(uint256 tokenNumber) public onlyGovernance {
        require(auctionState[tokenNumber] == AuctionState.PAUSED);
        auctionState[tokenNumber] = AuctionState.RUNNING;
    }

    function endAuction(uint256 tokenNumber) public onlyGovernance {
        require(auctionState[tokenNumber] == AuctionState.RUNNING);
    ...
    function uri(uint256 tokenNumber)  public view override returns (string memory) {
        string memory url = super.uri(tokenNumber);
        return string(abi.encodePacked(url, tokenNumber));
    }
    ...
    function totalSupply(uint256 _id) public view returns (uint256) {
        if (_id % 2 == 0) {
            return totalAmount[getTokenNFTMapping(_id)];
        } else {
            return 1;
        }
    }

    function fractionalise(uint256 amount, uint256 _listingPrice, uint256 ownershipAmount, address creator) public returns(uint256) {
        require(ownershipAmount < amount);
        ownershipAmount *= 10e18;
        amount *= 10e18;
        uint256 tokenNumber = createToken(governance);
        tokenCreatorMapping[tokenNumber] = creator;
        totalAmount[tokenNumber] = amount;
        listingPrice[tokenNumber] = _listingPrice * 10e14;
        createTokenBatch(creator, ownershipAmount);
        uint256 governanceAmount = (amount / 1000) * (25);
        createTokenBatch(governance, governanceAmount);
        balaceTokensForSale[tokenNumber] = amount - governanceAmount - ownershipAmount;
    ...
    function ownershipOfToken(address holder, uint256 tokenNumber) public view returns(uint256) {
        return balanceOf(holder, getNFTTokenMapping(tokenNumber));
    }
    ...
    function amnountRequired(uint256 tokenNumber, uint256 amount) public view returns(uint256) {
        ...
        return _listingPrice * (amount / 10e18);
    }

    function purchaseShare(uint256 tokenNumber, uint256 amount, address to) public payable {
        amount *= 10e18;
        ...
        uint256 unUsedValue = msg.value - amount * _listingPrice;
        _mint(to, getNFTTokenMapping(tokenNumber), numberTokens, "");
        balaceTokensForSale[tokenNumber] = balaceTokensForSale[tokenNumber] - numberTokens;
        payable(msg.sender).transfer(unUsedValue);
    }

    function getLastBidAmount(uint256 tokenNumber) public view returns(uint256) {
        return bidPrice[tokenNumber];
    }

    function bid(uint256 tokenNumber, address bidder) public payable {
        ...
        bidWinner[tokenNumber] = bidder;
    }

    function withdraw(uint256 tokenNumber, address to) public payable {
        require(auctionState[tokenNumber] == AuctionState.ENDED);
        uint256 balance = balanceOf(to, getNFTTokenMapping(tokenNumber));
        uint256 unitPrice = (bidPrice[tokenNumber] * balance) / totalAmount[tokenNumber];
        payable(msg.sender).transfer(unitPrice);
        _burn(to, getNFTTokenMapping(tokenNumber), balance);
    }
}
