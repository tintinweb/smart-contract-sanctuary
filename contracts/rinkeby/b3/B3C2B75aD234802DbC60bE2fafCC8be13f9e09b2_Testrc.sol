...
contract Testrc is ERC721, Ownable {
using Strings for uint256;
  ...
  uint256 public MINT_PRICE = 0.099 ether;
  uint256 public MAX_SUPPLY = 10000;
  uint public INCREASED_MAX_TOKEN_ID = MAX_SUPPLY + 2;
...
function mintPresale(uint256 numberOfTokens) public payable {
        require(presaleWhitelist[msg.sender],                         "No claim available for this address");
...
function mint(uint numberOfTokens) 
    public 
    payable
...
function editPresaleList(address[] calldata presaleAddresses) public onlyOwner {
        for(uint256 i; i < presaleAddresses.length; i++){
            presaleWhitelist[presaleAddresses[i]] = true;
        }
