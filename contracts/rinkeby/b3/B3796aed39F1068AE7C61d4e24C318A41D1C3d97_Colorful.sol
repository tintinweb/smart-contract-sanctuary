...
contract Colorful is ERC721, ERC721Enumerable, Ownable {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (bytes(base).length == 0) {
            return MyStrings.toString(tokenId);
        }

        return string(abi.encodePacked(base, MyStrings.toString(tokenId), '.json'));
    }
