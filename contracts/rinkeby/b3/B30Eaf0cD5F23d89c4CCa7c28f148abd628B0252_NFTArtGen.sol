...
contract NFTArtGen is ERC1155, Ownable {
    ...
    function mint(uint32 count) external payable {
        require(count > 0, "Mint at last one.");
    ...
    function airdrop(address[] calldata to, uint32[] calldata amountToMint)
        public
        onlyOwner
    {
        for (uint32 i = 0; i < to.length; i++) {
            require(amountToMint[i] + supply <= totalSupply, "Limit reached");
            supply = supply + amountToMint[i];
            _mint(to[i], amountToMint[i]);
        }
    }
