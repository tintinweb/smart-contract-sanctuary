...
contract HAYC__ is ERC721URIStorage, Ownable {

    event MintHAYC (address indexed minter, uint256 startWith, uint256 times);
   
    
    uint256 public totalHAYC;
    uint256 public totalCount = 5555; 
    uint256 public totalFreeCount = 1010;
    ...
    function mintHAYC(uint256 _times) payable public mintEnabled {
        ...
        require(totalHAYC + _times <= totalCount, "Too many HAYC");
        require(msg.value == _times * getPrice(), "Incorrect Value");
        payable(owner()).transfer(msg.value);
        emit MintHAYC(_msgSender(), totalHAYC+1, _times);
        for(uint256 i=0; i< _times; i++){
            _mint(_msgSender(), 1 + totalHAYC++);
        }
    } 
    ...
    function adminMintToAddress(address _addr) public onlyOwner {
        require(totalHAYC + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit MintHAYC(_addr, totalHAYC+1, 1);
        _mint(_addr, 1 + totalHAYC++);
    } 
