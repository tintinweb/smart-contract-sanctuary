...
contract SimpsOffice is Ownable, IERC721Receiver {
    ...
    uint public constant DAILY_GOLD_RATE = 20000 ether;
    ...
    uint public constant MERCY_JACKPOT_PAY_PERIOD = 6 hours;
    uint public constant MERCY_JACKPOT_PAYER_COUNT = 10;
    ...
    function _claimFromSimp(uint16 tokenId, bool unstake) internal returns (uint owed) {
        ...
        if (totalLoveEarned < ALL_LOVE_IN_THE_UNIVERSE) {
            owed = ((block.timestamp - stake.value) * DAILY_GOLD_RATE) / PAY_PERIOD;
        } else if (stake.value > lastClaimTimestamp) {
            owed = 0; // $LLOVE production stopped already
        } else {
            owed = ((lastClaimTimestamp - stake.value) * DAILY_GOLD_RATE) / PAY_PERIOD; // stop earning additional $LLOVE if it's all been earned
        }
        ...
        if(endExtraTimeStamp>0){
            ...
            if(block.timestamp<= startExtraTimestamp){
                ...
                owed = owed+((block.timestamp-startExtraTimestamp)* DAILY_GOLD_RATE)*extraPercentage/100 / PAY_PERIOD;
            }else if(stake.value>=startExtraTimestamp && block.timestamp<=endExtraTimeStamp){
                owed = owed+ ((block.timestamp - stake.value)* DAILY_GOLD_RATE)*extraPercentage/100/ PAY_PERIOD;
            }else if(stake.value<=startExtraTimestamp && block.timestamp>=endExtraTimeStamp){
    ...
    function rescue(uint16[] calldata tokenIds) external {
        ...
        for (uint16 i = 0; i < tokenIds.length; i++) {
            tokenId = tokenIds[i];
            if (!simpsCity.isQueen(tokenId)) {
                stake = simpStake[msg.sender][simpIndices[tokenId]];
                ...
                emit SimpClaimed(tokenId, 0, true);
            } else {
                stake = queenStake[msg.sender][queenIndices[tokenId]];
                ...
                emit QueenClaimed(tokenId, 0, true);
            }
        }
    }
    ...
    modifier _updateEarnings() {
        if (totalLoveEarned < ALL_LOVE_IN_THE_UNIVERSE) {
            totalLoveEarned += ((block.timestamp - lastClaimTimestamp) * totalSimpStaked * DAILY_GOLD_RATE) / PAY_PERIOD;
