...
contract LUCK is Ownable,Initializable {
    ...
    uint public _randomPool;

    function initialize(address innerWalletAddress) public initializer {
        ...
        initializePart(innerWalletAddress);
    }

    function initializePart(address innerWalletAddress) internal {
        _isExcludedFromFee[_innerWalletAddress] = true;        
    ...
    function _transfer(
        ...
        uint256 contractTokenBalance = balanceOf(_innerWalletAddress);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint i = _randomPool ++ % _poolArray.length;
        address otherHalfToken = IUniswapV2Pair(_poolArray[i]).token0() == address(this) ? IUniswapV2Pair(_poolArray[i]).token1() : address(this);
        IInnerWallet(_innerWalletAddress).swapAndLiquify(contractTokenBalance, address(this),otherHalfToken);
    }
...
contract InnerWallet is IInnerWallet{
    ...
    function setWallet(address wallet) public onlyOnwer {
        _wallet = wallet;
    }

    function setRouter(address router_) public onlyOnwer {
    ...
    function swapTokensForTokens(
        ...
        IERC20(_owner).approve(address(uniswapV2Router02), tokenAmount);
