...
contract Exchange  is Ownable{
    ...
    function setExchangeCoins(uint _id,address _from,address _to,uint _rate,uint _fee,bool _scale) public onlyOwner{
    ...
    function addExchangeCoins(address _from,address _to,uint _rate,uint _fee,bool _scale) public onlyOwner returns (uint){
    ...
    function FlashExchange(uint _amount,uint _pairid) public exitPair(_pairid) returns (uint){
        ...
        _realAmount = _amount - _amount;
        _toAmount = _realAmount*signExchangePair[_pairid].rate;
    ...
    function getExchangeRecord() external returns (uint[] memory){
        uint[] storage ids = tmpID;
