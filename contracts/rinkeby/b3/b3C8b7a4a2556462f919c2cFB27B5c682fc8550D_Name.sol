...
contract Name is ERC721, ContextMixin, NativeMetaTransaction, Ownable {
    address internal creator;
    // indicate if name has been minted or not
    mapping(string => bool) internal canonicalNames;
    // Map of names by token_id
    mapping(uint256 => string) internal lookupNames;
    // Map of token_id by names
    mapping(string => uint256) internal names;
    // Count of token_it
    uint256 internal topToken;

    constructor() ERC721("Cryptovoxels Names", "NAME") {
        creator = msg.sender;
        topToken = 0;
    }
    ...
    function _isNameValid(string memory str) internal pure returns (bool) {
        ...
        if ((uint8(char) == 95) || (uint8(char) == 45)) {
            return false;
        }
     ...
     * @param _name    Amount of tokens to mint
     */
    function mint(
        address _to,
        string memory _name,
    ...
    function isNameMinted(string memory _name) public view returns (bool) {
        if (!canonicalNames[_toLower(_name)]) {
            return false;
        }
        return true;
    }
    ...
    {
        // Use 0x207Fa8Df3a17D96Ca7EA4f2893fcdCb78a304101 as the whitelisted address for ERC1155's.
     ...
     */
    function _mintBatch(address[] memory tos, string[] memory _names)
        public
        onlyOwner
    {
        require(
            tos.length == _names.length,
            "ERC721: address and _names length mismatch"
        );
        for (uint256 i = 0; i < _names.length; i++) {
            require(tos[i] != address(0), "ERC721: mint to the zero address");
            mint(tos[i], _names[i], "");
        }
        address operator = _msgSender();

        emit MintBatch(operator, tos, _names);
    }
