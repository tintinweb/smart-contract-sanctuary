...
contract WLSC is ERC721Enumerable, Ownable, IERC721Receiver {
    using EnumerableSet for EnumerableSet.UintSet;
    
    uint256 public constant MINT_PRICE = .06 ether;

    uint256 public MAX_TOKENS = 3333;
    uint256 public TX_LIMIT = 10;
    ...
    bool public canWLMint = false;
    bytes32 public merkleRoot;
    mapping(address => bool) hasMinted;

    mapping(address => EnumerableSet.UintSet) private _deposits;
    mapping(uint256 => uint256) public _depositBlocks;

    WL public TOKEN;
    uint256 public RATE = 1562500000000;

    bool public canStake = false;

    constructor(address _token) ERC721("The Whitelisted Social Club", "WLSC") {
        TOKEN = WL(_token);
    }

    function mint(uint256 amount, bool stake) external payable {
        require(canMint || msg.sender == owner(), "Minting not live");
        require(tx.origin == _msgSender(), "Only EOA");
        require(minted + amount <= MAX_TOKENS, "All tokens minted");
        require(amount > 0 && amount <= TX_LIMIT, "Invalid mint amount");
        ...
        for (uint i = 0; i < amount; i++) {
            ...
            if (!stake) {
                ...
                _deposits[msg.sender].add(minted);
                _depositBlocks[minted] = block.number;
            }
        }
    }
    ...
    function mintWhitelist(address account, bytes32[] calldata proof) public payable {
        require(canWLMint || msg.sender == owner(), "Whitelist Mint Must Be Active");
        require(_verify(_leaf(account), proof), "Invalid proof");
        require(hasMinted[account] == false, "Already Minted Your Whitelist");
    ...
    function deposit(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            _depositBlocks[tokenIds[i]] = block.number;
        }
    }
    ...
    function calculateRewards(address account) public view returns (uint256){
        ...
        for (uint256 i; i < depositSet.length(); i++) {
            ...
            rewards = rewards + (RATE * _depositBlocks[tokenId]);
        }
    ...
    function claimRewards() public {
        ...
        for (uint256 i; i < numOfDepositedTokens; i++) {
            ...
            _depositBlocks[tokenId] = block.number;
        }
    ...
    function changeCanMint() external onlyOwner(){
        canMint = !canMint;
    }

    function changeCanMintWhitelist() external onlyOwner(){
        canMint = !canMint;
    }
...
contract WL is ERC20, Ownable {

    mapping(address => bool) controllers;
    
    constructor() ERC20("WL", "WL") { }
