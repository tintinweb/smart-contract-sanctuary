...
contract Ammo is ERC20, ERC20Burnable, Pausable, Ownable, ERC20Permit {
    constructor() ERC20("TEst Ammo", "TAMMO") ERC20Permit("TAMMO") {
...
mapping (address => uint) public arr;
    ...
    uint256 mintprice= 70000000000000000 wei;

    address www= 0x0BD0fFa4e2e3E0c53bAdca0905354c112887c68A;
    bool public rewardisActive = false;
    bool private purchaseactive = false;
    sotmcontract sotm= sotmcontract(www);
    address iio;
    bool internal ismintallowed= true;
    uint256 internal INDEX= 0;
    uint256 public currentids;
    mapping (uint256 => uint256) public tokenid;
    uint256 claimreward = block.timestamp;
    uint256 start= block.timestamp;
    uint256 public currentidsrari;
...
mapping (uint256 => bool) public dotokenexist;
 ...
 function setRari(uint256[] memory rarity, uint256 start1) external {
...
// in dynamic array
     ...
     function SetMintisActive() external onlyOwner {
    ...
    function flipBuyingstatus() external onlyOwner {
    ...
    function SOTMBALANCeE() internal view returns (uint256 balance){
     ...
     function ceilDiv(uint256 a, uint256 b) internal pure returns (uint256) {
        ...
        function claimablereward(address _address) external view returns (uint256){
         ...
         uint256 currentid= sotm.tokenOfOwnerByIndex(_address, i);
         uint256 currentidsrai=getrarirty(currentid);
         bool a= gettoken(currentids);

         
         if (a==true){
            uint256 lasttime=  gettokentime(currentids);
            uint256 totalreward= block.timestamp-lasttime;
            totalreward= totalreward / 70;
                                   if(totalreward>0){

         
                         if (currentidsrari<=49 && currentidsrari>=0){
                                totalreward= totalreward*38;
                            rewardclaim = rewardclaim + totalreward;
                                
                                
                        }
                         if (currentidsrari<=250 && currentidsrari>=50){
                                totalreward= totalreward*30;
                            rewardclaim = rewardclaim + totalreward;
                        }
                         if (currentidsrari<=750 && currentidsrari>=251){
                            totalreward= totalreward*25;
                            rewardclaim = rewardclaim + totalreward;
                        }  
                         if (currentidsrari<=1500 && currentidsrari>=751){
                            totalreward= totalreward*21;
                            rewardclaim = rewardclaim + totalreward;
                             
                         }  
                         if (currentidsrari<=2500 && currentidsrari>=1501){
                            totalreward= totalreward*18;
                            rewardclaim = rewardclaim + totalreward;
                             
                         }  
                         if (currentidsrari<=3500 && currentidsrari>=2501){
                            totalreward= totalreward*15;
                            rewardclaim = rewardclaim + totalreward;
                             
                         }  
                         if (currentidsrari<=5000 && currentidsrari>=3501){
                                totalreward= totalreward*12;
                            rewardclaim = rewardclaim + totalreward;
                             
                         }
                         if (currentidsrari<=6500 && currentidsrari>=5001){
                            totalreward= totalreward*11;
                            rewardclaim = rewardclaim + totalreward;
                ...
                uint256 totalreward= block.timestamp-start;
                totalreward= totalreward/70;
               if (currentidsrari<=49 && currentidsrari>=0){
                                totalreward= totalreward*38;
                            rewardclaim = rewardclaim + totalreward;
    ...
    function SOTMTOKENBYINDEX(address _address) internal {
         uint size= SOTMBALANCeE();
          for (uint256 i=0;i<size; i++)
          {
              
         currentids= sotm.tokenOfOwnerByIndex(_address, i);
         currentidsrari=getrarirty(currentids);
         bool a= gettoken(currentids);
         ...
         if (a==true){
            ...
            totalreward= totalreward / 70;
                         ...
                         if (currentidsrari<=49 && currentidsrari>=0){
                                totalreward= totalreward*38;
                            rewardyoucanclaim = rewardyoucanclaim + totalreward;
    ...
    function getrarirty(uint256 idtoken) public view returns (uint256 rari){
        return rari = tokenid[idtoken] ;
    }


    function SOTMTOKENBYINDEXe(uint256 index, address _account) external view returns (uint256 tokenId){
    ...
    function claimrewardd(address _adddress) public payable  {
        require(SOTMBALANCeE() != 0, 'NO SOTM FOUND');
        {
            bool a= get();
            SOTMTOKENBYINDEX(_adddress);
            if (a == true)
            {
              uint256 lasttime=  get1();
              lasttime= lasttime+70;
...
{                    
    ...
    function buy(uint256 numberOfTokens) public payable {
        require(purchaseactive== true, "Purchase is not allowed");
