...
contract SoulyCuratedMinting is Ownable {
    ...
    address private _tokenDestination;
    ...
    bytes32 private constant MintTypeHash = keccak256(bytes("Mint(address creator,bytes32 assetHash,address curator)"));
    bytes32 private _domainSeparator;

    constructor(ISouly tokenContract, address tokenDestination) Ownable() {
    ...
    function hash(MintType memory mint_) internal pure returns (bytes32) {
        return keccak256(abi.encode(
            MintTypeHash, mint_.creator, mint_.assetHash, mint_.curator
        ));
    }

    function mint(bytes32 assetHash, address curator, bytes memory signature) external {
        require(_mintingAllowance[curator] > 0, "Ninu");
        _mintingAllowance[curator] = _mintingAllowance[curator] - 1;
        bytes32 msgHash = keccak256(abi.encodePacked(
            "\x19\x01",
            _domainSeparator,
            hash(
                MintType(msg.sender, assetHash, curator)
            )
        ));
        require(msgHash.recover(signature) == curator, "Ninu Ninu");
        require(!_minted[assetHash], "Ninu Ninu Ninu");
        _minted[assetHash] = true;
        _tokenContract.mint(payable(msg.sender), _tokenDestination, assetHash);
    }
