...
contract FactoryMasterERC721Mint is Context {
  ...
  function createChildERC721(
    ...
    emit ChildCreatedERC721Mint(address(child), name, symbol, contractURI);
...
contract FactoryMasterERC721Burn is Context {
  ...
  function createChildERC721(
    ...
    emit ChildCreatedERC721Burn(address(child), name, symbol, contractURI);
    withdraw();
  }
...
contract ERC721Burnable is Context, ERC721Mint {
  constructor(
    ...
    string memory contractURI,
    string[] memory metadata
  ) ERC721Mint(owner, metadata) ERC721(name, symbol, contractURI) {}
...
abstract contract ERC721Mint is Ownable, ERC721URIStorage {
    ...
    returns (uint256)
  {
    ...
    for (uint256 i = 0; i < metadataURI.length; i++) {
       _tokenIds.increment();
