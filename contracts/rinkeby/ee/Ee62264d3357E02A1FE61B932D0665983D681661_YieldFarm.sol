...
contract YieldFarm {
    ...
    function massHarvest() external returns (uint256) {
        ...
        require(totalDistributedValue > 0, "Nothing to harvest");
    ...
    function harvest(uint128 epochId) external returns (uint256) {
        ...
        if (userReward > 0) {
            _dmod.safeTransferFrom(_communityVault, msg.sender, userReward);
        }
