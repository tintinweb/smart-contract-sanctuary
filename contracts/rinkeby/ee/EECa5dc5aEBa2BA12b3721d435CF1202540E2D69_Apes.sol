...
contract Apes is ERC721Enumerable, Ownable {
    ...
    uint256 public constant maxPublicSupply = 10000;
    uint256 public constant maxTotalSupply = 30000;
    ...
    address public constant dev1Address = 0x9678AEc495f5f7Ec11f3DB7bCa41723c7b7D7e99;
    address public constant dev2Address = 0x8E673B16b98c0d8118439160CF06bA5a559c17De;
    ...
    function getBananasCost(uint256 totalSupply) internal pure returns (uint256 cost){
        if (totalSupply < 12000)
            return 100;
        else if (totalSupply < 14000)
            return 200;
        else if (totalSupply < 16000)
            return 400;
        else if (totalSupply < 18000)
            return 800;
         else if (totalSupply < 20000)
            return 1000;
         else if (totalSupply < 22000)
            return 1200;
        else if (totalSupply < 24000)
            return 1400;
        else if (totalSupply < 26000)
            return 1600;
         else if (totalSupply < 28000)
            return 1800;
        else if (totalSupply < 30000)
            return 2000;
    }
...
contract Jungle is Ownable, IERC721Receiver {
    ...
    */
    ...
    //Amount of $Banana stolen by GreedyApe while staked
    mapping(uint256 => uint256) public bananaStolen;

    //Daily $Banana earned by HungryApes
    uint256 public adventuringBananaRate = 100 ether;
    //Total number of hungryApes staked
    uint256 public totalHungryApesStaked = 0;
    //Percent of $Banana earned by hungryApes that is kept
    uint256 public hungryApeShare = 50;

    //Percent of $Banana earned by hungryApes that is stolen by Greedys
    ...
    //Store tokenIds of all mutants staked
    uint256[] public mutantsStaked;
    ...
    */
    ...
    function unstakeManyApes(uint256[] calldata ids) external {
        for(uint256 i = 0; i < ids.length; i++){
            require(ownerById[ids[i]] == msg.sender, "Not your Ape");
            ...
            if (statusById[ids[i]] == 1){
                ...
                for (uint256 j = 0; j < greedyApesStaked.length; j++){
                    if (greedyApesStaked[j] == ids[i]){
                        greedyApesStaked[j] = greedyApesStaked[greedyApesStaked.length-1];
                ...
                for (uint256 j = 0; j < mutantsStaked.length; j++){
                    if (mutantsStaked[j] == ids[i]){
                        mutantsStaked[j] = mutantsStaked[mutantsStaked.length-1];
                        mutantsStaked.pop();
                    }
                }
            }
            ...
            apes.safeTransferFrom(address(this), ownerById[ids[i]], ids[i]);
            statusById[ids[i]] = 0;
        }
    }
    ...
    function _claim(address owner, uint256 tokenId) internal {
        if (statusById[tokenId] == 1){
            if(greedyApesStaked.length > 0){
                bananas.mint(owner, getPendingBanana(tokenId).mul(hungryApeShare).div(100));
                distributeAmongstGreedyApes(getPendingBanana(tokenId).mul(greedyApeShare).div(100));
            }
            else {
                bananas.mint(owner, getPendingBanana(tokenId));
            }
        }
        else if (statusById[tokenId] == 2){
            uint256 roll = randomIntInRange(tokenId, 100);
            if(roll > chanceGreedyApeGetsLost || mutantsStaked.length == 0){
                bananas.mint(owner, bananaStolen[tokenId]);
                bananaStolen[tokenId] = 0;
            } else{
    ...
    //Passive earning of $Banana, 100 $Banana per day
    function getPendingBanana(uint256 id) internal view returns(uint256) {
        return (block.timestamp - timeStaked[id]) * 100 ether / 1 days;
    }

    //Distribute stolen $Banana accross all staked greedyApes
    function distributeAmongstGreedyApes(uint256 amount) internal {
         uint256 roll = randomIntInRange(amount, greedyApesStaked.length);
        bananaStolen[greedyApesStaked[roll]] += amount;
    }
    ...
    //Return new owner of lost greedyApe from current mutants
    function getNewOwnerForGreedyApe(uint256 seed, uint256 tokenId) internal{
        uint256 roll = randomIntInRange(seed, mutantsStaked.length);
        emit ApeStolen(ownerById[tokenId], ownerById[mutantsStaked[roll]], tokenId);
        ownerById[tokenId] = ownerById[mutantsStaked[roll]];
        bananas.mint(ownerById[tokenId], bananaStolen[tokenId]);
        bananaStolen[tokenId] = 0;
    }

    function getTotalMutantsStaked() public view returns (uint256) {
        return mutantsStaked.length;
    }
    ...
    //Set address for $Banana
    function setBananasAddress(address bananaAddr) external onlyOwner {
        bananas = Bananas(bananaAddr);
    }
