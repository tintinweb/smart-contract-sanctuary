...
contract JPGRegistry is Ownable, SignedMessage {
    using SafeMath for uint256;

    event CuratorAdded(address curator);
    event TokensListed(NFT[] tokens);
    event TokensListedSubregistry(NFT[] tokens, address curator);
    event TokenListedForSale(NFT token, uint256 price, address curator);
    ...
    struct SubRegistry {
        ...
        mapping(address => mapping(uint256 => bool)) nfts;
    }
    ...
    mapping(address => SubRegistry) internal subRegistry;
    ...
    function bulkAddToMainRegistry(NFT[] calldata tokens) public {
        ...
        for (uint256 index = 0; index < tokens.length; index++) {
            if (_addToMainRegistry(tokens[index])) {
                listedTokens[index] = tokens[index];
            }
        }

        emit TokensListed(listedTokens);
    }

    /**
     * @notice Public method to list non-fungible token(s) on a subregistry
     * @dev permissionless listing from dApp - array of tokens (can be one). Will
     * ensure token is owned by caller or exists on main registry.
     * @param tokens An array of NFT struct instances
     */
    function bulkAddToSubregistry(NFT[] calldata tokens) public {
        require(curators[msg.sender], "JPGR:bats:Only allowed curators");

        NFT[] memory listedTokens = new NFT[](tokens.length);

        for (uint256 index = 0; index < tokens.length; index++) {
            if (_addToSubregistry(tokens[index], msg.sender)) {
                listedTokens[index] = tokens[index];
            }
        }

        emit TokensListedSubregistry(listedTokens, msg.sender);
    }
    ...
    function listForSale(
        ...
        emit TokenListedForSale(token, price, curator);
    }
    ...
    function getPrice(
        ...
        require(inSubRegistry(token, curator), "JPGR:gp:Token not curated by curator");
        ...
        uint256 totalPrice = sellerTake.add(curatorTake);
    ...
    function buyFixedPrice(
        ...
        ListingPrice memory price = getPrice(token, curator, owner);
        ...
        priceList[owner][token.tokenContract][token.tokenId] = InternalPrice({sellerTake: 0, curator: address(0)});
    ...
    function calculateCuratorTake(uint256 price, address curator) internal view returns (uint256) {
        return price.mul(subRegistry[curator].feePercentage).div(100);
    }

    /**
     * @notice Called internally from public bulk add to MainRegistry.
     * @param token An instance of an NFT struct
     */
    function _addToMainRegistry(NFT calldata token) internal returns (bool) {
        try ERC721(token.tokenContract).ownerOf(token.tokenId) returns (address owner) {
            if (owner == msg.sender) {
                mainRegistry[token.tokenContract][token.tokenId] = true;
                return true;
            }
            return false;
        } catch {
    ...
    /**
     * @notice Called internally to add token to subregistry
     * @param token An instance of an NFT struct
     * @param curator Adress of curator - usually msg.sender
     */
    function _addToSubregistry(NFT calldata token, address curator) internal returns (bool) {
        if (mainRegistry[token.tokenContract][token.tokenId] || _addToMainRegistry(token)) {
            subRegistry[curator].nfts[token.tokenContract][token.tokenId] = true;
            return true;
        }
     ...
     * @param token An instance of an NFT struct
     */
    function removeFromSubregistry(NFT calldata token) public {
        require(curators[msg.sender], "JPGR:rfs:Only allowed curators");
        subRegistry[msg.sender].nfts[token.tokenContract][token.tokenId] = false;
    }
     ...
     */
    function inSubRegistry(NFT calldata nft, address curator) public view returns (bool) {
        return
            mainRegistry[nft.tokenContract][nft.tokenId] && subRegistry[curator].nfts[nft.tokenContract][nft.tokenId];
    }
    ...
    function allowCurator(address curator) public onlyOwner {
        ...
        subRegistry[curator].feePercentage = Constants.CURATOR_TAKE_PER_100;
     ...
     * @param feePercentage An instance of an NFT struct
     */
    function setCuratorFee(uint16 feePercentage, address curator) public onlyOwner {
        ...
        subRegistry[curator].feePercentage = feePercentage;
    }
     ...
     * @param listings[] An array of Listing struct instances
     */
    function adminBulkAddToMainRegistry(Listing[] calldata listings) public onlyOwner {
        NFT[] memory listedTokens = new NFT[](listings.length);

        for (uint256 index = 0; index < listings.length; index++) {
            if (_adminAddToMainRegistry(listings[index])) {
                listedTokens[index] = listings[index].nft;
            }
        }
    ...
    function bulkRemoveFromMainRegistry(NFT[] calldata tokens) public onlyOwner {
        for (uint256 index = 0; index < tokens.length; index++) {
            _removeFromMainRegistry(tokens[index]);
        }
    }

    /**
     * @notice Called internally to add to ProtocolRegistry
     * @param listing An instance of a Listing struct
     */
    function _adminAddToMainRegistry(Listing memory listing) internal returns (bool success) {
        try ERC721(listing.nft.tokenContract).ownerOf(listing.nft.tokenId) returns (address owner) {
            address signer =
                SignedMessage.getSigner(listing.nft.tokenContract, listing.nft.tokenId, listing.signedMessage);

            if (owner == signer) {
                mainRegistry[listing.nft.tokenContract][listing.nft.tokenId] = true;
                return true;
            }
            return false;
        } catch {
...
library Constants {
    ...
    uint16 internal constant CURATOR_TAKE_PER_100 = 2;
}
