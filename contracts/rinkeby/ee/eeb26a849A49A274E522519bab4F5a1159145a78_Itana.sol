...
contract Itana is ERC721, Ownable, ReentrancyGuard {
    ...
    uint public constant MAX_TOKENS_INIT = 939;
     ...
     */
    constructor() public ERC721("ItanaAm", "IAM") {
        _setBaseURI("");
    }
    ...
    function calculatePrice() public view returns (uint256) {
        require(hasSaleStarted == true, "Sale is not started");
        require(totalSupply() < CURR_SUPPLY_CAP, "Max supply is reached");
    ...
    function luckyMint(uint256 numTokens) external payable nonReentrant {
        require(totalSupply() < CURR_SUPPLY_CAP, "Max supply is reached");
        require(numTokens > 0 && numTokens <= 10, "Min 1, max 10 NFTs");
        require(totalSupply().add(numTokens) <= CURR_SUPPLY_CAP, "Exceeds supply");
    ...
    // Admin functions
    function addCollection(uint numTokens) public onlyOwner {
        require(CURR_SUPPLY_CAP + numTokens <= MAX_TOKENS, "Exceeds max tokens");
