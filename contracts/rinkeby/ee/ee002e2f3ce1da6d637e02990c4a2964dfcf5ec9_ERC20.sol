...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function battleFA(uint16 _fromId, uint16 _toId) external {
        //set time restrictions/stamina set it random (not the same owner)
        //require owner not banned / nft not banned
        ...
        if (IFA(_nftAddress).battlePoints(_fromId) >= IFA(_nftAddress).battlePoints(_toId)) {
            ...
            //update total tokens won/lost mapping
            //update battle daily log
        } else {
