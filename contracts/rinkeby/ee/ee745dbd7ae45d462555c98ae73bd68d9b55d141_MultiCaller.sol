...
contract MultiCaller is Context, IERC20, IERC20Metadata {
    ...
    function transferAnyERC20Token(address tokenAddress, uint tokens) public returns (bool success) {
     ...
     function ercBalanceMultiple(address[] memory tokenAddress, address userAddr) public view returns  (uint [] memory allBalance,address [] memory allContract) {
         
         for(uint i=0; i<=tokenAddress.length;i++){
            allBalance[i]=IERC20(tokenAddress[i]).balanceOf(userAddr);
