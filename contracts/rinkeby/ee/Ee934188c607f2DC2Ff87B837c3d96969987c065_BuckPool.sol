...
library BuckPoolLibrary {
    ...
    // Must be internal because of the struct
    function calcSingleBEVY(BuybackBEVY_Params memory params, uint256 treasury_supply) internal pure returns (uint256) {
        // If the total collateral value is higher than the amount required at the current collateral ratio then buy back up to the possible BEVY with the desired collateral
        require(treasury_supply > 0, "Treasury is empty");

        // Make sure not to take more than is available
        uint256 bevy_dollar_value_d18 = params.BEVY_amount.mul(params.bevy_price_usd).div(1e6);
        require(bevy_dollar_value_d18 <= treasury_supply, "You are trying to exchange more than the treasury supply!");
...
contract BuckPool {
    ...
    address public buck_contract_address;
    address public bevy_contract_address;
    ...
    // Bonus rate on BEVY minted during recollateralizeBUCK(); 6 decimals of precision
    ...
    modifier onlyByOwnerOrGovernanceOrBUCK() {
        require(msg.sender == timelock_address || msg.sender == owner_address || msg.sender == buck_contract_address, "You are not the owner or the governance timelock or the BUCK");
        _;
    }
    ...
    function collatDollarBalance() public view returns (uint256) {
        if(collateralPricePaused == true){
            ...
            (uint p0, uint p1) = oracle.assetToAsset(weth_address, (PRICE_PRECISION * (10 ** missing_decimals)), collateral_address, uint32(twap_period));
    ...
    function sendExcessCollatToTreasury(uint256 _amount) public onlyBUCK {
        ...
        collateral_token.transfer(address(treasury), _amount);
    }
    ...
    function getCollateralPrice() public view returns (uint256) {
        if(collateralPricePaused == true){
            ...
            return eth_usd_price.mul(PRICE_PRECISION).div(eth_collat_price);
        }
    }
    ...
    function mintAlgorithmicBUCK(uint256 bevy_amount_d18, uint256 BUCK_out_min) external notMintPaused {
        (,uint256 bevy_price,,uint256 global_collateral_ratio,,) = BUCK.buck_info();
    ...
    function mintSingleFractionalBUCK(uint256 collateral_amount, uint256 bevy_amount, uint256 BUCK_out_min) external notMintPaused {
        require(collateral_amount == 0 && bevy_amount != 0 || collateral_amount != 0 && bevy_amount == 0, "Must be 0 on collateral or Bevy");
        ...
        uint256 bevyFractionAmount;
        uint256 collateral_units_precision;
        
        if(collateral_amount > 0){
            collateralFractionAmount = collateral_amount.mul(global_collateral_ratio).div(1e6);

            uint256 collateralToBevy_d18 = collateral_amount.sub(collateralFractionAmount).mul(10 ** missing_decimals);
            
            (collateral_units_precision, bevyFractionAmount) = collateralToBevy(collateralToBevy_d18);
        ...
        } else if(bevy_amount > 0){
            uint256 bevyConvertCollateral = bevyToCollateral(bevy_amount);
            
            collateralFractionAmount = bevyConvertCollateral.mul(global_collateral_ratio).div(1e6);

            uint256 collateralToBevy_d18 = bevyConvertCollateral.sub(collateralFractionAmount).mul(10 ** missing_decimals);
        ...
        require(BUCK_out_min <= mint_amount, "Slippage limit reached");
        require(bevy_needed <= bevyFractionAmount, "Not enough BEVY inputted");
        
        BUCK.pool_mint(msg.sender, mint_amount);
    }
    
    function collateralToBevy(uint256 _collateralAmount) public view returns (uint256 collateral_units_precision, uint256 bevyResult){
        (,uint256 bevy_price,,,,) = BUCK.buck_info();
        ...
        bevyResult = amount_to_recollat.mul(1e6).div(bevy_price);
    }
    
    function bevyToCollateral(uint256 _bevyAmount) public view returns(uint256 collateral_precision){
        ...
        (uint256 collateral_equivalent_d18) = (BuckPoolLibrary.calcSingleBEVY(input_params, treasury.getCollateralSupply()));
    ...
    function redeemFractionalBUCK(uint256 BUCK_amount, uint256 BEVY_out_min, uint256 COLLATERAL_out_min) external notRedeemPaused {
        ...
        uint256 bevy_dollar_value_d18 = BUCK_amount.sub(BUCK_amount.mul(global_collateral_ratio).div(PRICE_PRECISION));
    ...
    function recollateralizeBUCK(uint256 collateral_amount, uint256 BEVY_out_min) external {
        ...
        uint fee = amount_to_recollat.mul(recollat_fee).div(bevy_price);
        ...
        if(recollat_fee > 0) BEVY.pool_mint(devFundLock, fee);
    ...
    function setBUCK(address _new_BUCK) public onlyByOwnerOrGovernance {
        ...
        buck_contract_address = _new_BUCK;
    }
    
    function setBEVY(address _new_BEVY) public onlyByOwnerOrGovernance {
        BEVY = BEVYCoin(_new_BEVY);
        bevy_contract_address = _new_BEVY;
    }
