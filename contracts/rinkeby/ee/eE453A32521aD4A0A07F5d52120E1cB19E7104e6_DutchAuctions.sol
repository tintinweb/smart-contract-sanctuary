...
contract DutchAuctions is OwnableClone, RevenueSplit, SalesHistory {
    ...
    event ListingPurchased( uint256 indexed listingId, address buyer, address recipient, uint256 price );
    ...
    function bid(uint256 listingId, address _recipient, address payable _changeAddress) public payable {
        ...
        postSale(listing.tokenAddress, listing.offeringId, msg.sender, _recipient, currentPrice, block.timestamp);

        emit ListingPurchased(listingId, msg.sender, _recipient, currentPrice);
    ...
    function getAllListings() public view returns (OutputListing [] memory listings) {
        uint256 numListings = currentListingIds.length();
        listings = new OutputListing [](numListings);
        for(uint256 idx = 0; idx < numListings; idx++) {
            uint256 lid = currentListingIds.at(idx);
            listings[idx].listingId    = lid;
            listings[idx].tokenAddress = listingsById[lid].tokenAddress;
            listings[idx].offeringId   = listingsById[lid].offeringId;
            listings[idx].startPrice   = listingsById[lid].startPrice;
            listings[idx].startTime    = listingsById[lid].startTime;
            listings[idx].endPrice     = listingsById[lid].endPrice;
            listings[idx].endTime      = listingsById[lid].endTime;
        }
    }
}
