...
contract RoyaltyManagement {
    ...
    mapping(address => uint256) public artistsBalance;
    ...
    function fund() public payable {
        ...
        for (
            uint256 artistIntex = 0;
            artistIntex < artists.length;
            artistIntex++
        ) {
            address artist = artists[artistIntex];
            artistsBalance[artist] =
                (msg.value * addressToPercentage[artist]) /
                100;
        }
        //amount += msg.value;
    ...
    modifier onlyArtists() {
        require(artistsCheck[msg.sender]);
        _;
    }

    modifier onlyValidBalance() {
        require(artistsBalance[msg.sender] >= 0);
        _;
    }
