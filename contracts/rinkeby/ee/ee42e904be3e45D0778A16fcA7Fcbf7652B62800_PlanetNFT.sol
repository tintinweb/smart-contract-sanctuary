...
contract PlanetNFT is ERC721Tradable {
    ...
    function mint_internal() internal{
        mintTo(_msgSender());
    ...
    function create_random(uint256 tokenId,
        ...
        newPlanet.history=uint64(randomNum(912413254, newPlanet.randomness, block.timestamp));
        newPlanet.distance=uint64(randomNum(92425353446787, newPlanet.randomness, block.timestamp));
...
//        int16 positionY=int16(600-size)-int16(uint16(randomNum((600-size)*2, seedfinal+17, block.timestamp)))-100;
    ...
    function baseTokenURI() public pure virtual override returns (string memory) {
        return "https://xrgap.com/planetnft";
    }
...
*/
    ...
    function _getJsonAttributes(
        ...
        ERC721MetadataAttribute[]
            memory metadataAttributes = new ERC721MetadataAttribute[](6);
        ...
        returns (uint256)
    {
        ...
        for (uint16 i = 0; i < planets.length; i++) {
            ...
            sizeScore = (radius * 100) / 50;

            chance =
                chance +
                uint256(
                    ((uint256(sizeScore) * 400) / patternThresholds[pattern]) /
                        (i + 1)
                );
