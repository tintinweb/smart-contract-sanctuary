...
library RouterLib {
                    ...
                    uint160(
                        uint256(
                            keccak256(
                                ...
                                hex'4d61c27ebaf0e1010adaf57886e26867c34ef19d76f6fb2eca6c60a4efa888cd' // init code hash
                                )
                            )
                        )
                    )
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, uint256 fee, uint256 precision) internal pure returns (uint amountOut) {
        ...
        uint amountInWithFee = amountIn * (precision - fee);
        uint numerator = amountInWithFee * reserveOut;
        uint denominator = reserveIn * precision + amountInWithFee;
        amountOut = numerator / denominator;
    }

    // given an output amount of an asset and pair reserves, returns a required input amount of the other asset
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, uint256 fee, uint256 precision) internal pure returns (uint amountIn) {
        ...
        uint numerator = (reserveIn * amountOut) * precision;
        uint denominator = (reserveOut - amountOut) * (precision - fee);
