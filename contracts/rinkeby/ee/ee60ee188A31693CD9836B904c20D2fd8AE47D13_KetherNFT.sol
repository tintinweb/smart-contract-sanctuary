...
contract KetherNFT is ERC721Enumerable, Ownable {
  ...
  function encodeFlashEscrow(uint _idx) public view returns (bytes memory) {
    return abi.encodePacked(
      type(FlashEscrow).creationCode,
      abi.encode(address(instance), encodeFlashEscrowPayload(_idx)));
  }

  function encodeFlashEscrowPayload(uint _idx) public view returns (bytes memory) {
    return abi.encodeWithSignature("setAdOwner(uint256,address)", _idx, address(this));
  }

  function flashCreationCode() public view returns (bytes memory) {
    return type(FlashEscrow).creationCode;
  }
  ...
  function precompute(uint _idx, address _owner) public view returns (bytes32 salt, address predictedAddress) {
    ...
    bytes memory bytecode = encodeFlashEscrow(_idx);
  ...
  function wrap(uint _idx, address _owner) external {
    ...
    new FlashEscrow{salt: salt}(address(instance), encodeFlashEscrowPayload(_idx));
