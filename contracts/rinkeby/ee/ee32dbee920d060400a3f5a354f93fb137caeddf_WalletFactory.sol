...
contract WalletFactory is CloneFactory {
    ...
    event Deposited(address from, uint256 value, address to, bytes data);
  ...
  constructor(address _implementationAddress,address _coldWalletAddress, address _owner1, address _owner2, address _owner3) {
    ...
    function executeDepositEvent(address _from, uint256 _value, address _to, bytes memory _data)external {
        require(isWalletOrNot[msg.sender] == true);
        emit Deposited(_from, _value, _to, _data);
    }
