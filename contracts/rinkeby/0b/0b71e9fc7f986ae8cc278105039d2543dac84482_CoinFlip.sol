...
contract CoinFlip is VRFConsumerBase {
    ...
    // burn fee, max 100%
    uint16 public constant MAXIMUM_BURN_FEE = 10000;
    // 5% burn fee
    uint16 public burnFee = 500;
    // limit amount of bet
    ...
    function bet(uint256 playerBet, uint256 amount, uint256 seed) public returns (bool) {
        //each bet has unique id
        bets[betId] = Bet(betId, playerBet, seed, amount, msg.sender);
    ...
    function verdict(uint256 random) public onlyVFRC {
        ...
        for(uint256 i=lastBetId; i<betId; i++){
            //reset amount for current user
            uint256 winAmount = 0;
            uint256 burnAmount = 0;
            uint256 transferAmount = 0;
            //if user wins, then receives 2x of their betting amount
            if((random >= half && bets[i].playerBet == 1) || (random < half && bets[i].playerBet == 0)){
                burnAmount = bets[i].amount.mul(burnFee).div(MAXIMUM_BURN_FEE);
                winAmount = bets[i].amount.sub(burnAmount);
                transferAmount = winAmount.add(bets[i].amount);
                
                token.safeTransfer(BURN_ADDRESS, burnAmount);
                token.safeTransfer(bets[i].player, transferAmount);

                totalBetAmount += bets[i].amount;
                totalBurnAmount += burnAmount;
                totalWinAmount += winAmount;
            }
            emit Result(bets[i].id, bets[i].playerBet, bets[i].seed, bets[i].amount, bets[i].player, winAmount, random, block.timestamp);
        }
