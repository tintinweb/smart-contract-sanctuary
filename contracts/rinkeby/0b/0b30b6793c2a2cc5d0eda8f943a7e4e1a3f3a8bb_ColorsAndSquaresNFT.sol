...
contract ERC721CappedSale is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function mint(uint256 mintAmount) public payable nonReentrant {
        require(isSaleStarted(), "ERC721CappedSale: sale is not started");
...
contract ColorsAndSquaresNFT is ERC721CappedSale {

    string private constant INIT__NFT_NAME = 'Colors And Squares';
    string private constant INIT__NFT_SYMBOL = 'COSQ';
    uint256 private constant INIT__EA_TIME = 10 hours;
    //uint256 private constant INIT__STARTTIME = block.timestamp+INIT__EA_TIME;
    uint256 private constant INIT__CAP = 200;
    uint256 private constant INIT__MAX_BUY_AMOUNT = 20;
    uint256 private constant INIT__PRICE_PER_TOKEN = 8 * 10**16;

    // update that later
    address payable private constant INIT__WALLET = payable(0xe8488930236ea1373E5F58A7389C056F93a59c61);
    IERC721 private constant INIT_EA_NFT = IERC721(0x90A0BB75b4fC9D7F5A094E0A5c34272137a0c7B9);
    ...
    string private _tokenBaseURI = 'ipfs://QmYH4uMdjnDQ8GvHE9jvN7sxREtxM5yHANB9hgMmtvHg51/';

    constructor(
    ) ERC721CappedSale(
        INIT__NFT_NAME,
        INIT__NFT_SYMBOL,
        block.timestamp+INIT__EA_TIME,  // update that later
        INIT__CAP,
    ...
    )
    {
        ...
        _earlyAdopterTime = INIT__EA_TIME;
    }
    ...
    function earlyAdopterMint(uint256 mintAmount, uint256 earlyAdopterTokenId) public payable nonReentrant {
        require(isEarlyAdopterTime(), "EarlyAdopter: not early adopter time"); // @dev changed this line
        require(!isSalePaused(), "ERC721CappedSale: sale is paused");
        require(mintAmount > 0, "ERC721CappedSale: buy at least 1 NFT");
        require(mintAmount <= _maxBuyAmount, "ERC721CappedSale: its not allowed to buy this much");
        require(mintAmount <= nftsAvailable(), "ERC721CappedSale: not enough NFTs available");
    ...
    function isEarlyAdopterTime() public view returns(bool) {
        return (block.timestamp < _starttime) && (block.timestamp > (_starttime - _earlyAdopterTime));
    }
