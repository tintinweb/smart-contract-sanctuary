...
contract MysteryBox is OwnableUpgradeable {
    ...
    function get_nft_for_sale(uint256 box_id, uint256 cursor, uint256 amount)
        external 
        view
        returns(uint256[] memory nft_id_list)
    {
        ...
        if (box.sell_all) {
            for (uint256 i = 0; i < amount; i++) {
                ...
                nft_id_list[i] = IERC721Enumerable(box.nft_address).tokenOfOwnerByIndex(box.creator, token_index);
            }
        }
        else {
            for (uint256 i = 0; i < amount; i++) {
