  ...
  ReentrancyGuard
{
  ...
  constructor() ERC721("NFT721", "NFT721") {
    ...
    femaleProbabilities[uint256(Slot.hat)] = [ 237, 0, 378, 332, 347, 0, 146, 237, 409, 404, 0, 227, 436, 378, 0, 0, 414, 227, 434, 424, 434, 175, 0, 242, 139, 244, 229, 0, 378, 0, 388, 370, 381, 228, 229, 141, 228, 0, 370, 441, 350];
    ...
    maleProbabilities[uint256(Slot.hat)] = [ 505, 75, 0, 0, 369, 433, 155, 242, 434, 0, 512, 317, 463, 0, 692, 442, 440, 483, 463, 451, 231, 0, 517, 0, 0, 0, 346, 281, 231, 512, 0, 0, 0, 486, 0, 0, 196, 251, 0, 470, 0];
  ...
  event Zunk( address indexed user,  uint256 serialId,  string tokenRepresentation, bool isReroll);
  ...
  function _mintNoChecks() internal {
  ...
  function validateSlotToRerollValid( uint256 slotToReroll, Attributes memory attributes ) internal pure {
  ...
  function rerollZunk(uint256 serialId, uint256 slotToReroll) public payable {
    ...
    string memory rerolledZunk = _rerollZunk( isFemale, serialId, zunkSkin, zunkGender, attributes, slotToReroll, zunk );

    require(!isInvalidZunk(rerolledZunk), "Could not reroll into a valid zunk, please try again.");
  ...
  function _rerollZunk( bool isFemale, uint256 serialId, uint256 zunkSkin, uint256 zunkGender, Attributes memory attributes, uint256 slotToReroll, string memory zunk) internal view returns (string memory) {
    return
      _rerollZunk( isFemale, serialId, zunkSkin, zunkGender, attributes, slotToReroll, zunk, true );
  }

  function _rerollZunk( bool isFemale, uint256 serialId, uint256 zunkSkin, uint256 zunkGender, Attributes memory attributes, uint256 slotToReroll, string memory zunk, bool throwIfUnableToReroll) internal view returns (string memory) {
    ...
    if (slotToReroll == uint256(Slot.accessories)) {
      rerolledAttribute = rerollZunkSlot( isFemale, serialId, Slot.accessories, attributes.accessories, zunk, throwIfUnableToReroll);
      ...
      rerolledAttribute = rerollZunkSlot( isFemale, serialId, Slot.beard, attributes.beard, zunk, throwIfUnableToReroll );
      if (rerolledAttribute == NO_REROLL_OPTIONS) {
        return NO_OPTIONS;
      }
      rerolledAttributes.beard = rerolledAttribute;
    } else if (slotToReroll == uint256(Slot.ear)) {
      rerolledAttribute = rerollZunkSlot( isFemale, serialId, Slot.ear, attributes.ear, zunk, throwIfUnableToReroll );
      if (rerolledAttribute == NO_REROLL_OPTIONS) {
        return NO_OPTIONS;
      }
      rerolledAttributes.ear = rerolledAttribute;
    } else if (slotToReroll == uint256(Slot.eyes)) {
      rerolledAttribute = rerollZunkSlot( isFemale, serialId,Slot.eyes, attributes.eyes, zunk, throwIfUnableToReroll );
      if (rerolledAttribute == NO_REROLL_OPTIONS) {
        return NO_OPTIONS;
      }
      rerolledAttributes.eyes = rerolledAttribute;
    } else if (slotToReroll == uint256(Slot.hat)) {
      rerolledAttribute = rerollZunkSlot( isFemale, serialId, Slot.hat, attributes.hat, zunk, throwIfUnableToReroll );
      if (rerolledAttribute == NO_REROLL_OPTIONS) {
        return NO_OPTIONS;
      }
      rerolledAttributes.hat = rerolledAttribute;
    } else if (slotToReroll == uint256(Slot.lips)) {
      rerolledAttribute = rerollZunkSlot( isFemale, serialId, Slot.lips, attributes.lips, zunk, throwIfUnableToReroll );
      if (rerolledAttribute == NO_REROLL_OPTIONS) {
        return NO_OPTIONS;
      }
      rerolledAttributes.lips = rerolledAttribute;
    } else if (slotToReroll == uint256(Slot.neck)) {
      rerolledAttribute = rerollZunkSlot( isFemale, serialId, Slot.neck, attributes.neck, zunk, throwIfUnableToReroll );
      if (rerolledAttribute == NO_REROLL_OPTIONS) {
        return NO_OPTIONS;
      }
      rerolledAttributes.neck = rerolledAttribute;
    } else {
      rerolledAttribute = rerollZunkSlot(isFemale, serialId, Slot.smoke, attributes.smoke, zunk, throwIfUnableToReroll );
      if (rerolledAttribute == NO_REROLL_OPTIONS) {
        return NO_OPTIONS;
      }
  ...
  function rerollZunkSlot( bool isFemale, uint256 serialId, Slot slot, uint256 attribute, string memory zunk, bool throwIfUnableToReroll) internal view returns (uint256) {
    uint256 slotAsUint = uint256(slot);
    uint256[] storage attributeProbabilities = getAttributeProbabilities( isFemale, slotAsUint);
    uint256 probabilitiesLength = attributeProbabilities.length;

    string[] memory rerollCandidates = getRerollCandidates( slot, zunk, probabilitiesLength );

    uint256[] memory adjustedProbabilities = getAdjustedProbabilities( isFemale, slot, attribute, probabilitiesLength, rerollCandidates);
    ...
    return pickRandomFromMemory( adjustedProbabilities, serialId, uint256(slot), attribute );
  }

  function getAdjustedProbabilities( bool isFemale, Slot slot, uint256 attribute, uint256 probabilitiesLength, string[] memory rerollCandidates) internal view returns (uint256[] memory) {
  ...
  function getRerollCandidates( Slot slot, string memory zunk, uint256 probabilitiesLength ) internal pure returns (string[] memory) {
  ...
  function getValidSlotsForReroll(Attributes memory attributes) internal pure returns (uint256[] memory)
  {
  ...
  function generateZunk(uint256 serialId) internal view returns (string memory)
  {
    ...
    uint256 zunkGender = pickRandomFromStorage(genderProbabilities, serialId, 1, 2);

    bool isFemale = isZunkFemale(zunkGender);

    Attributes memory attributes = getAttributes(isFemale, serialId);
    string memory generatedZunk = createZunkStringRepresentation( zunkSkin, zunkGender, attributes);

    if (isInvalidZunk(generatedZunk) || unableToRerollZunk(generatedZunk)) {
      uint256[] memory validSlots = getValidSlotsForReroll(attributes);
      uint256 randomSlotIndex = getRandomNumber( zunkGender, zunkSkin, serialId) % validSlots.length;

      generatedZunk = _rerollZunk( isFemale, serialId, zunkSkin, zunkGender, attributes, validSlots[randomSlotIndex], generatedZunk, false );
    }
    return generatedZunk;
  }

  function convertSlotsToString(Attributes memory attributes) internal pure returns (AttributesAsString memory)
  {
    return
      AttributesAsString(
  ...
  function getAttributes(bool isFemale, uint256 serialId) internal view returns (Attributes memory)
  {
    return
      Attributes(
  ...
  function maybeGetAttribute(bool isFemale, Slot slot, uint256 serialId) internal view returns (uint256) {
    uint256 slotAsUint = uint256(slot);
    ...
    bool isSelected = isSlotSelected( slot, isFemale, serialId, slotAsUint, length);

    if (!isSelected) {
      return 99;
    } else {
      uint256 attribute = pickRandomFromStorage( attributeProbabilities, serialId, slotAsUint, length );
      return attribute;
    }
  }

  // prospective slots are 0-7
  function isSlotSelected(Slot prospectiveSlot, bool isFemale, uint256 seed1, uint256 seed2, uint256 seed3 ) internal view returns (bool) {
  ...
  function pickRandomFromStorage( uint256[] storage attributeProbabilities, uint256 seed1, uint256 seed2, uint256 seed3) internal view returns (uint256) {
  ...
  function pickRandomFromMemory( uint256[] memory attributeProbabilities, uint256 seed1, uint256 seed2, uint256 seed3) internal view returns (uint256) {
    // mix up ordering of seed to generate diff random number from the random number in isSlotSelected
    uint256 randomNumber = getRandomNumber(seed3, seed2, seed1);
    uint256 sum = 0;
    for (uint256 i = 0; i < attributeProbabilities.length; i++) {
  ...
  function getRandomNumber( uint256 seed1, uint256 seed2, uint256 seed3 ) public view returns (uint256) {
    uint256 randomNum = uint256( keccak256( abi.encode( msg.sender, tx.gasprice, block.number, block.timestamp, blockhash(block.number - 1), seed1, seed2, seed3 )));
  ...
  function createZunkStringRepresentation( uint256 zunkSkin, uint256 zunkGender, Attributes memory attribues ) internal pure returns (string memory) {
