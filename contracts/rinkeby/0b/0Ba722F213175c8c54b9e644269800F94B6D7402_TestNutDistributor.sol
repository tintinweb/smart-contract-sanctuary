...
contract Nutmeg is Initializable, Governable, INutmeg {
    ...
    function stake(address token, uint tranche, uint principal)
        ...
        require(stk.status == StakeStatus.Uninitialized || stk.status == StakeStatus.Open, 'cannot stake');
    ...
    function borrow(address baseToken, address collToken, uint baseAmt, uint borrowAmt)
        ...
        IERC20Upgradeable(pool.baseToken).safeTransferFrom(position.owner, address(this), baseAmt);
    ...
    function openPosition( IAdapter adapter, address baseToken, address collToken, uint baseAmt, uint borrowAmt ) external {
        uint balance = IERC20Upgradeable(nut).balanceOf(msg.sender);
...
contract TestNutDistributor is NutDistributor {
    using SafeMath for uint;
    function initialize ( address nutAddr, address _governor, address oracleAddr, uint blocks_per_epoch
    ) external initializer {
        initialize(nutAddr, _governor, oracleAddr);
...
contract NutDistributor is Governable, INutDistributor {
    ...
    function initialize(address nutAddr, address _governor, address oracleAddr) public initializer{
...
contract MockCERC20 is ICERC20, ERC20 {
    ...
    constructor(address tokenAddr) ERC20("MockCERC20", "MCERC20") {
        token = tokenAddr;
...
contract CompoundAdapter is Governable, IAdapter {
    ...
    constructor(INutmeg nutmegAddr, address governorAddr) {
        nutmeg = nutmegAddr;
        __Governable__init(governorAddr);
    }
    ...
    function openPosition(address baseToken, address collToken, uint baseAmt, uint borrowAmt)
        ...
        require(mintAmt > 0, 'opnPos: zero mnt');
    ...
    function _doMint(INutmeg.Position memory pos, uint amount) internal returns(uint, uint) {
        ...
        IERC20(pos.baseToken).safeApprove(pos.collToken, 0);
        IERC20(pos.baseToken).safeApprove(pos.collToken, amount);
