...
// File: contracts\CryptoFroggies.sol

pragma solidity ^0.8.2;



contract CryptoFroggies is ERC721Enumerable,  Ownable {
     
    uint256 public nftPrice = 50000000000000000; // 0.05 ETH

    // Only 10 gauntlets can be purchased per transaction.
    uint public constant maxNumPurchase = 20;

    // Only 10,000 total froggies.
   ...
   // Reserved for the team, customs, giveaways, collabs and so on.
    ...
    constructor() ERC721("Crypto Froggies", "Frog") {
    ...
    function reserveTokens(uint number) public onlyOwner { 
        ...
        require(supply + number <= MAX_TOKENS, "Reservation would exceed max supply");  
        uint i;
        for (i = 0; i < number; i++) {
    ...
    function _checkRegularSale(uint numberOfTokens) pure internal {
        require(numberOfTokens <= maxNumPurchase, "Can only mint 20 tokens at a time");
    }
    /**
    * Mints an Crypto Froggie
    */
    function mintFroggie(uint numberOfTokens) public payable {
        ...
        require(nftPrice * numberOfTokens <= msg.value, "Ether value sent is not correct");
