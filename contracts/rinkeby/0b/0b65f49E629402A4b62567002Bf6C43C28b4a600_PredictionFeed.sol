...
contract PredictionFeed is Ownable, ChainlinkClient {
    ...
    mapping(PoolType => AggregatorV2V3Interface) priceFeeds;
    address public oracleAddress_;
    bytes32 public jobId_;
    ...
    function initializeAggregators(
        ...
        priceFeeds[PoolType.BTC_USDT] = AggregatorV2V3Interface(_btcAddress);
        priceFeeds[PoolType.ETH_USDT] = AggregatorV2V3Interface(_ethAddress);
        priceFeeds[PoolType.BNB_USDT] = AggregatorV2V3Interface(_bnbAddress);
    }

    /**
     * Initialize Chainlink Addresses
     ...
     */
    function initializeChainLinkAddresses(address _oracleAddress, string memory)
        public
        onlyOwner
    {
        ...
        // if (
        //     keccak256(abi.encode("rinkeby")) == keccak256(abi.encode(_network))
        // ) {
        //     jobId_ = "eb3b27aac93e4bf68406f164b86b049e";
        // } else if (
        //     keccak256(abi.encode("kovan")) == keccak256(abi.encode(_network))
        // ) {
        //     jobId_ = "81c63592d97a4485b1d1339b3578e07f";
        // } else {}
        // jobId_ = "eb3b27aac93e4bf68406f164b86b049e";
    ...
    /**
     * Initialize GFX Contract address
     */
    function initializeGFXAddress(address _gfxAddress) public onlyOwner {
        ...
        recordChainlinkFulfillment(_requestId)
    {
        ...
        pools[poolId].initialPrice = getLastPrice(pools[poolId].poolType);
    ...
    function tempLockPool(uint256 _poolId) public onlyOwner {
        ...
        pools[_poolId].initialPrice = getLastPrice(pools[_poolId].poolType);
        ...
        recordChainlinkFulfillment(_requestId)
    {
        ...
        pools[poolId].finalPrice = getLastPrice(pools[poolId].poolType);
    ...
    function tempEndPool(uint256 _poolId) public onlyOwner {
        ...
        pools[_poolId].finalPrice = getLastPrice(pools[_poolId].poolType);
     ...
     */
    function getLastPrice(PoolType _poolType) public view returns (int256) {
        (, int256 price, , , ) = priceFeeds[_poolType].latestRoundData();
        return price;
    }
