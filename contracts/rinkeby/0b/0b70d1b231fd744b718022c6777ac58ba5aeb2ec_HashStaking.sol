...
contract HashStaking is Ownable {
    ...
    function updateProceedsLimit(uint _proceedsLimit) external onlyOwner {
        ...
        require(_proceedsLimit > getTotalProceeds(), "The value is too small");
    ...
    function addPlan(uint _period, uint _percent, uint _specialPercent) external onlyOwner {
        ...
        require(proceedsLimit >= getTotalProceeds(), "Unable to add the plan: too high percents");
    ...
    function updatePlan(uint _planId, uint _percent, uint _specialPercent) external onlyOwner {
        ...
        require(proceedsLimit >= getTotalProceeds(), "Unable to update the plan: too high percents");
    }
    ...
    function makeDeposit(uint _amount, address _referral) external {
        ...
        uint maxProceeds = _amount * _getMaxPercent(special) / 100;
        if (referral != address(0)) {
            maxProceeds = maxProceeds * (100 + referralPercent) / 100;
        }
        require(getTotalProceeds() + maxProceeds <= proceedsLimit, "Unable to make a deposit, the amount is too large");
    ...
    function closeDeposit(uint _depositId) external {
        ...
        if (deposits[_depositId].referral != address(0)) {
            referralProceeds = proceeds * (100 + referralPercent) / 100;
    ...
    function makeDepositCheck(address _address, uint _amount, address _referral) external view returns(bool success, uint maxAmount) {
        if (stakingFinished == true) {
            ...
            success = getTotalProceeds() + maxProceeds <= proceedsLimit;
            maxAmount = proceedsLimit - getTotalProceeds() - maxProceeds;
        }
    }
    ...
    function getTotalProceeds() public view returns (uint proceeds) {
        proceeds = 0;
        ...
        for (uint i = 0; i < deposits.length; i++) {
            if (deposits[i].closed > 0) {
                ...
                currentProceeds = deposits[i].amount * _getMaxPercent(deposits[i].special) / 100;
    ...
    function _getMaxPercent(bool _special) internal view returns (uint percent) {
        ...
        percent = percent * period / (365 * 24 * 60 * 60);
    }
