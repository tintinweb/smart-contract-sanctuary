...
contract BURNER is Context, AccessControlEnumerable, ERC721Enumerable, ERC721Burnable, ERC721Pausable {
	...
	uint[8] _arrayGasTiers = [0, 40, 75, 125, 150, 200, 300, 9999];
	...
	function tokenByIndex(uint256 _index) public view override returns (uint256) {
		...
		require(_index < _maxCap, "ID out of bounds!");
	...
	function getRenderData(uint tokenIndex, uint currentGas, string memory currentDay) public view returns (string memory) {
		require(tokenIndex >= 0, "ID out of bounds!");
		require(tokenIndex < _maxCap, "ID out of bounds!");
		require(_exists(tokenIndex), "Not minted yet!");

		bytes memory currentSetup = new bytes(9);
		...
		for(uint i = 8; i <= 9; i++) {
			if(tokenTier == 1) {
				...
				if(i == 8 || i == 9) {
					if(tokenTier == 2) {
						currentSetup[7] = seed[7];
						currentSetup[8] = 0;
					}
					else if(tokenTier == 3) {
						currentSetup[7] = seed[7];
						currentSetup[8] = seed[8];
					}
					else {
						currentSetup[7] = 0;
						currentSetup[8] = 0;
	...
	function setHTML(string memory _newhtml) public {
		require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Not authorized!");
		_html = _newhtml;
	}
