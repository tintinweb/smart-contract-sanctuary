...
contract NameTag is ERC721Enumerable, Ownable, StringUpper, ReentrancyGuard {
    ...
    event AddedToAllowContractList(address indexed _address, uint256 indexed _price, uint8 _limit);
    ...
    function addToAllowContractList(
        ...
        require(length == prices.length && length == limits.length, "NT: All lists should be the same length");
        for(uint index = 0; index < length; index += 1) {
//            todo check address to supportsInterface [ERC721, ERC1155] and skip it. ([0x70a08231] is hash of balanceOf)
            _allowContractList.add(addresses[index ]);
            allowContractListParams[addresses[index]].price = prices[index];
            allowContractListParams[addresses[index]].limit = limits[index];
            emit AddedToAllowContractList(addresses[index], prices[index], limits[index]);
        }
    }
    ...
    function removeFromAllowContractList(address[] memory addresses) external onlyOwner {
        ...
        for(uint index = 0; index < addresses.length; index += 1) {
            if (_allowContractList.remove(addresses[index])) {
                emit RemovedFromAllowContractList(addresses[index], allowContractListParams[addresses[index]].purchasedAmount);
                delete allowContractListParams[addresses[index]];
            }
        }
    }
    ...
    function contractPurchasedAmountByAddress(address _contract, address owner) external view returns(uint8) {
        ...
        return allowContractListParams[_contract].purchasedAmountByAddress[owner];
    }
    ...
    function buyTokensByContract(string[] memory _names, address _contract) external payable whenPresaleActive nonReentrant returns (uint256[] memory) {
        ...
        require(inAllowContractList(_contract), "NT: Contract address is not in the allowed list");
//        todo  check msg.sender balance with different types [ERC721, ERC1155]
        require(IERC721(_contract).balanceOf(msg.sender) > 0, "NT: Sender balance on the contract is 0");

        uint8 purchasedAmount = allowContractListParams[_contract].purchasedAmountByAddress[msg.sender];
        require(_names.length + purchasedAmount <= allowContractListParams[_contract].limit, "NT: Presale contract limit exceeded");
        require(allowContractListParams[_contract].price * _names.length <= msg.value, "NT: Presale, insufficient funds");

        allowContractListParams[_contract].purchasedAmountByAddress[msg.sender] += uint8(_names.length);
        allowContractListParams[_contract].purchasedAmount += _names.length;
