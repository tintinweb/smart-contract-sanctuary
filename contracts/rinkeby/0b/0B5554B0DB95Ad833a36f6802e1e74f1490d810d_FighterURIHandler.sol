    ...
    ERC721HolderUpgradeable
{
    ...
    {
        uint256 seed = _seeder.getSeed(address(_fighter), tokenId);
    ...
    function _baseURI() internal pure returns (string memory) {
        return "https://raid.party/api/metadata/fighter/";
    }
}
...
abstract contract Enhanceable is Initializable, Seedable {
    ...
    CountersUpgradeable.Counter private _enhancementCounter;
    ISeeder private _seeder;
    IERC721Upgradeable private _token;
    ...
    {
        _seeder = ISeeder(seeder);
        _token = IERC721Upgradeable(token);
    }
    ...
    function enhance(uint256 tokenId, uint256) public virtual {
        ...
        _enhancements[tokenId] = EnhancementRequest(
            _enhancementCounter.current(),
            msg.sender
        );
        _seeder.requestSeed(_enhancementCounter.current());
        emit EnhancementRequested(tokenId, block.timestamp);
        _enhancementCounter.increment();
    }
    ...
    function _getSeed(uint256 tokenId) internal view returns (uint256) {
        return _seeder.getSeed(address(this), _enhancements[tokenId].id);
    }
}