...
contract Sequences is ERC721, ERC721URIStorage, Ownable{
    ...
    function getEuclideanRow(uint256 row, uint256 id, uint256 rand) internal view returns (bytes memory) {
        ...
        //uint256 offset = rand % STEPS_PER_BAR;
        uint256 offset = 0;
        pulses = (rand % WIDTH) + 1;

        if(pulses < 2)
            pulses = 2;
        onsets = rand % (pulses / 2) + 1;
        onsets = 2 * (onsets / 4) + 1;
        int256 previous = -1;
    ...
    function getRow(uint256 row, uint256 id) internal view returns (bytes memory) {
        ...
        if(rand % 21 > 4){
            output = getEuclideanRow(row, id, rand);
            return output;
        }
