...
contract Gunnerz is ERC721, Ownable {
    ...
    uint256 public MAX_FREE_MINT = 2500;
    ...
    function mintPaid(uint256 _quantity)
        public
        payable
        publicSaleActive
        hasCorrectAmount(msg.value, _quantity)
    ...
    function mintPrice(uint256 _quantity) public view returns (uint256) {
        if (totalSupply() + _quantity <= MAX_FREE_MINT) {
            return 0;
        }
    ...
    function decreaseTotalSupply(uint256 _total) public onlyOwner {
        require(_total <= MAX_SUPPLY, "Increase supply not allowed");
        require(_total >= totalSupply(), "Must be above total minted");
        MAX_SUPPLY = _total;
        MAX_FREE_MINT = _total * 25 / 100;
    }
