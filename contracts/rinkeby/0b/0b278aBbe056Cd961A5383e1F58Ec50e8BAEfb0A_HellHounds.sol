...
contract HellHounds is ERC721Enumerable, Ownable, PrometheusPassVoucherSigner {  
    ...
    constructor (string memory newBaseURI, address voucherSigner) ERC721 ("Hell Hounds", "HH") PrometheusPassVoucherSigner(voucherSigner) {
    ...
    // Presale Minting V2
    function mintPresaleV2(uint256 _amount, PrometheusPassVoucher.Voucher calldata v) public payable {
        uint256 supply = totalSupply();
        uint256 reservedAmt = presaleReserved[msg.sender];
        require(presaleActive, "PrometheusPass: Private sale not open");
        require(claimedVouchers[v.voucherId] == false, "PrometheusPass: Voucher already claimed");
        require(PrometheusPassVoucher.validateVoucher(v, getVoucherSigner()), "PrometheusPass: Invalid voucher");
        require( supply + _amount <= MAX_SUPPLY, "Not Enough Supply" );
        require( msg.value == price * _amount,   "Incorrect Amount Of ETH Sent" );
        presaleReserved[msg.sender] = reservedAmt - _amount;
        for(uint256 i; i < _amount; i++){
