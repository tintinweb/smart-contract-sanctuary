...
contract WildEXOplanetGame is ERC721, EIP712, Ownable{
    ...
    uint256 public publicSaleEnableTime;
    uint256 public revealTime;

    uint128 public maxSupply = 6907;
    uint128 public runningSupply;
    
    //claimed bitmask
    mapping(uint256 => uint256) public whitelistSaleClaimBitMask;

    //number of nft in wallet
    mapping(address => uint8) public nftNumber;
    ...
    {
      baseUri = _baseUri;
      publicSaleEnableTime = block.timestamp + 1 days;
      revealTime = block.timestamp + 2 days;
    } 

    function mint(address to, uint8 amount) public payable {
      require(block.timestamp >= publicSaleEnableTime, "NFT: Public sale not enabled!");
      require(runningSupply < maxSupply, "NFT: Max Supply Of NFT reached!");
      require(nftNumber[to]+amount <= 2, "NFT: NFT max amount exceed!");
      require(msg.value >= mintFee*amount, "NFT: Not enough Minting Fee!");
    ...
    function claimWhitelistSale(bytes[] calldata _signature, address to, uint256[] calldata _nftIndex) external payable{
      require(block.timestamp <= publicSaleEnableTime, "NFT: Whitelist sale ended!");
      require(runningSupply < maxSupply, "NFT: Max Supply Of NFT reached!");
      require(nftNumber[to] + _nftIndex.length <=2, "NFT: Max NFT already minted!");
      require(msg.value >= mintFee * _nftIndex.length, "NFT: Not enough Mint Fee!");
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        if(block.timestamp>=revealTime){
          ...
          return "ipfs://<cid unrevealed>/1.json";
        }
    }
