...
contract FuckLootV2 is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    uint256 public initialReserve = 20;
    ...
    function getFucked(uint256 _batchCount) payable public {
        ...
        //require(estimateGas > 299999, "Gas limit provided is too low"); //After EIP 1559 estimated gas limit should be gettable via estimateGas so this should check gaslimit of the transaction.
        require(started, "Sale has not started");
        require(_batchCount > 0 && _batchCount <= maxBatch, "Batch purchase limit exceeded");
        require(totalMinted + _batchCount <= totalCount + initialReserve, "Not enough inventory");
   ...
   */
    ...
    function distributeFunds() public payable onlyOwner{
        ...
        uint256 contract_balance = address(this).balance;
        require(payable(mainWallet).send( (contract_balance * 500) / 1000));
        require(payable(faucetWallet).send( (contract_balance * 500) / 1000));
     
    }

    function reserveNFTs(address to, uint256 _reserve) public onlyOwner { //Check this to make sure its how we want to reserve some of the initial supply
        
        require(nftsReserved + _reserve <= initialReserve, "Exceeds reserve supply");
        for (uint256 i = 0; i < _reserve; i++) {
            _mint(to, 1 + totalMinted++);
            nftsReserved++;
        }
