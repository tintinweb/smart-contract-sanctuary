...
contract LoanMarket{
    ...
    mapping(uint256 => LoanData.LoanDetails) public loanOffers;
    ...
    public
    {
        require(_stakedCollateralTokens.length == _stakedCollateralAmounts.length,"GLM: tokens and amounts length must be same");
        //create uniquie loan hash
        borrowerloanOfferIds[msg.sender].push(loanId);
        loanOfferIds.push(loanId);
        //loop through all staked collateral tokens.
        for(uint256 i  = 0 ; i  <  _stakedCollateralTokens.length ; i++){
            require(govWorldProtocolRegistry.isTokenApproved(_stakedCollateralTokens[i]),"GLM: One or more tokens not approved.");
            uint256 allowance =  IERC20(_stakedCollateralTokens[i]).allowance(msg.sender, address(this));
            require( allowance >= _stakedCollateralAmounts[i],"GLM: Transfer amount exceeds allowance.");
            //contract will now hold the staked collateral tokens
            IERC20(_stakedCollateralTokens[i]).transferFrom(msg.sender, address(this), _stakedCollateralAmounts[i]);
        }

        LoanData.LoanDetails memory loanDetails = LoanData.LoanDetails(
            _stakedCollateralTokens,
            _stakedCollateralAmounts,
            new bytes32[](0),
            _loanAmount,
            _apyOffer,
            _loanType,
    ...
    /**
    * @dev Returns current all loan offer ids for a borrower.
    */
    ...
    * @param _status LoanStatus INACTIVE=1, ACTIVE=2, CLOSED=3, CANCELLED=4, LIQUIDATED=5, TERMINATED=6
    */
    function getLoanOfferByStatus(uint _status)
        view
        public
        returns(uint256[] memory)
    {
         uint256[] memory activeOfferIds = new uint[](loanOfferIds.length);
         for(uint256 i = 0 ; i  < loanOfferIds.length ;  i++){
             LoanData.LoanDetails memory details = loanOffers[i];
             if(uint(details.loanStatus)  == _status){
                 activeOfferIds[i] = i;
             }
         }
    ...
    /**
    * @dev Amount of  _tokenA in tertms needed to buy 1 _tokenB
    * @param _tokenA address of alt coin
    * @param _tokenB address of stable coin
    */
        ...
        returns(uint)
    {
        ...
        IDexPair pair = IDexPair(IDexFactory(dexFactory).getPair(_tokenA, _tokenB));
        IERC20Decimals token1 = IERC20Decimals(pair.token1());
        (uint resToken0, uint resToken1,) = pair.getReserves();
        uint res0 = resToken0*(10**token1.decimals());
        return(res0/resToken1); // return amount of token0 needed to buy token1
    }

    function getLtv(address _borrower, uint256 _loanId)
    external
    returns(uint256)
    {
        LoanData.LoanDetails memory loanDetails = loanOffers[_loanId];
        uint256[] memory stakedCollateralAmounts = loanDetails.stakedCollateralAmounts;
        address[] memory stkaedCollateralTokens = loanDetails.stkaedCollateralTokens;

        address borrowedToken = loanDetails.borrowStableCoin;
        uint256 totalCollateralInBorrowedToken = 0;
        for(uint256 i = 0 ; i < stkaedCollateralTokens.length ; i++){
            uint256 priceInBorrowed = this.getTokenPrice(borrowedToken, stkaedCollateralTokens[i]); 
            console.log("priceInBorrowed: %d", priceInBorrowed);
            uint256 priceCollateralInBorrowed = priceInBorrowed.mul(stakedCollateralAmounts[i]);
            totalCollateralInBorrowedToken =  totalCollateralInBorrowedToken.add(priceCollateralInBorrowed);
        }
        return totalCollateralInBorrowedToken;
    }
...
library LoanData {
	...
	struct LoanDetails{
		...
		address[] stkaedCollateralTokens;
		uint256[] stakedCollateralAmounts;
		//borrowers staked NFT assets
		bytes32[] collateralErc721Tokens;
		//total Loan Amoount in USD
...
contract GovWorldAdminRegistry is Ownable {
    ...
    //list of pending removed admins to be approved by approved admins.
    ...
    modifier onlyAddGovAdminRole(address _admin) {
        require(
            ...
            "GAR: onlyAddGovAdminRole can add admin."
        );
        _;
    }
    ...
    modifier onlyEditGovAdminRole(address _admin) {
        require(
            ...
            "GAR: OnlyEditGovAdminRole can edit or remove admin."
        );
        _;
    }

    event NewAdminApprovedByAll(address indexed _newAdmin);
    event NewAdminApproved(address indexed _newADmin, address indexed _admin);
    event AdminRemoved(address indexed _admin);
    event RemoveAdminApproved(
    ...
    function isApprovedByAll(address _newAdmin) external view returns (bool) {
        ...
        //get All admins with add and edit govAdmin rights
     ...
     * @param _admin Address of the new admin
     */
    function isRemovedByAll(address _admin) internal view returns (bool) {
        //following two loops check if all currenctly
    ...
    function _makeApproved(address _newAdmin, AdminAccess memory _adminAccess)
        internal
    {
        //this admin by default approves one existing admins.
        ...
        onlyAddGovAdminRole(msg.sender)
    {
        ...
        //if length of allApprovedAdmins is 0, need to call addDefaultAdmin Owner
        require(
        ...
        require(
            ...
            "GAR: Already an admin, cannot add himself again"
        );
        ...
        onlyAddGovAdminRole(msg.sender)
    {
        ...
        require(
            pendingAddedAdminRoles[_newAdmin].addGovAdmin,
            "GAR: Non Pending admin can not be approved. Invalid rights."
        );
        ...
        if (this.isApprovedByAll(_newAdmin)) {
            //loop for reverse approval of approved admin.
            for (uint256 i = 0; i < allApprovedAdmins.length; i++) {
                approvedByAdmins[allApprovedAdmins[i]].push(_newAdmin);
            }
    ...
    /**
     * @dev any admin can reject the pending admin during the approval process and onc rejection means
     * not pending anymore.
     * @param _admin Address of the new admin
     */
    function removePendingAdmin(address _admin)
        external
        onlyEditGovAdminRole(msg.sender)
    {
        //the admin that is adding _newAdmin must not already have approved.
        require(
            _notApproved(_admin, msg.sender),
            "GAR: Can not remove admin, you already approved."
        );
    ...
    function _makePendingForRemove(address _admin) internal {
        ...
        emit RemoveAdminApproved(_admin, msg.sender);
    }
        ...
        onlyEditGovAdminRole(msg.sender)
    {
        ...
        require(
            _notRemoved(_admin, msg.sender),
            "GAR: Admin already removed this admin. "
        );

        //if length is 1 there is only one admin and he/she is removing another admin
        if (allApprovedAdmins.length == 1) {
            //this admin by default approves existing admin.
            removedByAdmins[msg.sender].push(_admin);
            _removeAdmin(_admin);
        } else {
            //this admin is now in the pending list.
        ...
        onlyEditGovAdminRole(msg.sender)
    {
        ...
        require(
            pendingRemovedAdminRoles[_admin].editGovAdmin,
            "GAR: Non Pending admin can not be approved for remove."
        );

        removedByAdmins[_admin].push(msg.sender);

        //if the _admin is approved by all other admins for removal
        if (isRemovedByAll(_admin)) {
    ...
    function _removeKey(address _valueToFindAndRemove) internal {
        address[] memory auxArray;
        for (uint256 i = 0; i < allApprovedAdmins.length; i++) {
            if (allApprovedAdmins[i] != _valueToFindAndRemove) {
                auxArray[i] = allApprovedAdmins[i];
            }
        }
        allApprovedAdmins = auxArray;
    }

    function _getIndex(address _valueToFindAndRemove)
        internal
        view
    ...
    //using this function externally in Gov Protocol Registry Smart Contract
    function isAddEditTokenAccess(address admin) external view returns (bool) {
        if (
            approvedAdminRoles[admin].addToken ||
            approvedAdminRoles[admin].editToken == true
        ) return true;
    ...
    /**
    //get list of admins approved by the approving Admin Role
    * @param _approvingAdmin admin who approved the admins
     */

    function getapprovedAdmins(address _approvingAdmin)
        public
        view
        returns (address[] memory)
    {
        return approvedByAdmins[_approvingAdmin];
    }
}
...
contract GovWorldProtocolRegistry {
    ...
    // Token Data: BlueChip, Gem, LaunchPad, NFTPlatfrom, Network
    struct Market {
        string name;
        string symbol;
        bool isSP;
    ...
    //mapping tokenContractAddress to the Market
    mapping(address => Market) public approvedTokens;

    //mapping hash of the nftcontract + tokenId to the NFT Data
    ...
    event NFTAdded(bytes32 nftPlatform, address nftContract, uint256 tokenId);
    ...
    modifier isContractAddress(address _contractAddress) {
        require(
            _contractAddress.isContract(),
            "Please enter the valid contract address"
        );
        _;
    }

    modifier isContractBulkAddress(address[] memory _contractAddress) {
        for (uint256 i = 0; i < _contractAddress.length; i++) {
            require(
                _contractAddress[i].isContract(),
                "One or more Contract Addresses are invalid NFT Contracts"
            );
            _;
        }
    }
    ...
    //modifier: only admin can add or remove Toke/NFT
    modifier onlyAddEditTokenorNFTRole(address admin) {
        require(
            govAdminRegistry.isAddEditTokenAccess(admin),
            "GovProtocolRegistry: No admin right to add, update or remove Token/NFT."
        );
        _;
    }

    /** check function if Token Contract address is already added */
    ...
    */
    function addToken(address _tokenAddress, Market memory marketData)
        external
        onlyAddEditTokenorNFTRole(msg.sender)
        isContractAddress(_tokenAddress)
    {
    ...
    function addNFT(
        ...
        external
        onlyAddEditTokenorNFTRole(msg.sender)
        isContractAddress(_nftContract)
    {
        require(
            IERC721(_nftContract).ownerOf(_nftTokenId) != address(0),
            "invalid token"
        );
        bytes32 hash = _makeHashNFTContractandTokenId(
            _nftContract,
            _nftTokenId
        );

        //checking NFT Contract
        require(!_isAlreadyAddedNFT(hash), "GPL: already added NFT Contract");
    ...
    function addBulkNFT(
        ...
        external
        onlyAddEditTokenorNFTRole(msg.sender)
        isContractBulkAddress(_nftContracts)
    {
        require(
    ...
    function removetoken(address _removeTokenAddress)
        external
        onlyAddEditTokenorNFTRole(msg.sender)
        isContractAddress(_removeTokenAddress)
    {
        require(
            _isAlreadyAddedToken(_removeTokenAddress),
        ...
        isContractAddress(_nftContract)
    {
        ...
        //checking NFT Contract
        require(
            _isAlreadyAddedNFT(hash),
            "GPL: cannot remove NFT, add nft token id first"
        );

        _removeNFTKey(_getIndex(hash));

        delete approvedNFTs[hash];
        emit NFTRemoved(approvedNFTs[hash].platform, _nftContract, _nftTokenId);
    }
    ...
    *@dev function which remove bulk NFTs key from array and data from mapping
    ...
    function removeBulkNFTs(
        ...
        external
        onlyAddEditTokenorNFTRole(msg.sender)
        isContractBulkAddress(_nftContract)
    {
        require(
        ...
        for (uint256 i = 0; i < _nftContract.length; i++) {
            ...
            bytes32 hash = keccak256(
                abi.encodePacked(_nftContract[i], _nftTokenId[i])
            );
    ...
    function _addNFT(
        ...
        bytes32 hash = keccak256(abi.encodePacked(_nftContract, _nftTokenId));

        approvedNFTs[hash] = NFTData({
            platform: _nftPlatform,
            nftContractAddress: _nftContract,
            nftTokenId: _nftTokenId
        });

        allapprovedNFTContracts.push(hash);
    }
    ...
    function _addBulkNFT(
        ...
        for (uint256 i = 0; i < _nftContracts.length; i++) {
            require(
                IERC721(_nftContracts[i]).ownerOf(_nftTokenIds[i]) !=
                    address(0),
                "invalid token"
            );
            bytes32 hash = keccak256(
                abi.encodePacked(_nftContracts[i], _nftTokenIds[i])
            );

            //checking NFT Contract and Token ID
            require(
                !_isAlreadyAddedNFT(hash),
                "NFT Contract and Token Id already added, TokenId must be unique"
            );

            approvedNFTs[hash] = NFTData({
                platform: _nftPlatfrom,
                nftContractAddress: _nftContracts[i],
                nftTokenId: _nftTokenIds[i]
            });

            allapprovedNFTContracts.push(hash);
        }

        emit BulkNFTAdded(_nftPlatfrom, _nftContracts, _nftTokenIds);
    }
    ...
    function _removeNFTKey(uint256 index) internal {
        for (uint256 i = index; i < allapprovedNFTContracts.length - 1; i++) {
    ...
    function _makeHashNFTContractandTokenId(
        address _nftContract,
        uint256 _tokenId
    ...
    /** check function if Token Contract address is already added */
    function _isAlreadyAddedToken(address _tokenAddress)
        internal
        view
        returns (bool)
    {
    ...
    /** check function if NFT Contract address is already added */
    function _isAlreadyAddedNFT(bytes32 _hashofContractandTokenId)
        internal
        view
        returns (bool)
    {
...
contract GovWorldTierLevel {
    ...
    constructor(address _govAdminRegistry, bytes32 _bronze, bytes32 _silver, bytes32 _gold, bytes32 _platinum, bytes32 _allStar)  {

        //TODO add tier levels according to default tier levels by client
        govAdminRegistry = GovWorldAdminRegistry(_govAdminRegistry);

        _addTierLevel(_bronze, TierData(15000, 30, false, false, true, false, true, false, false, false));
        _addTierLevel(_silver, TierData(30000, 40, false, false, true, true, true, false, false, false));
        _addTierLevel(_gold, TierData(75000, 50, true, true, true, true, true, true, true, true));
        _addTierLevel(_platinum, TierData(150000, 70, true, true, true, true, true, true, true, true));
        _addTierLevel(_allStar, TierData(300000, 70, true, true, true, true, true, true, true, true));
    }

   
    modifier onlyEditTierLevelRole(address admin) {
        require(govAdminRegistry.isEditAdminAccessGranted(admin), "GTL: No admin right to add or remove tier level.");
        _;
    }
        ...
        onlyEditTierLevelRole(msg.sender)
    {
        ...
        require(!_isAlreadyTierLevel(_newTierLevel),"GTL: already added tier level");
        //adding tier level called by the admin
        _addTierLevel(_newTierLevel, _tierData);
    }


    function updateTierLevel(bytes32 _updatedTierLevelKey, TierData memory _newTierData)
        external
        onlyEditTierLevelRole(msg.sender)
    {

        require(_isAlreadyTierLevel(_updatedTierLevelKey), "GovWorldTier: cannot update Tier, create new tier first");
    ...
    function removeTierLevel(bytes32 _existingTierLevel)
        ...
        onlyEditTierLevelRole(msg.sender)
    {
        require(_isAlreadyTierLevel(_existingTierLevel), "GovWorldTier: cannot remove, Tier Level not exist");
        delete tierLevels[_existingTierLevel];
        
        _removeTierLevelKey(_existingTierLevel);
    }
    ...
    function _updateTierLevel(
        ...
        //update Tier Level to the updatedTier mapping
    ...
    * @param _existingTierLevel already existing tierlevel
     */

    function _removeTierLevelKey(bytes32 _existingTierLevel)  
        internal
    {
        bytes32[] memory auxArray = new bytes32[](allTierLevelKeys.length-1);
        for (uint i = 0; i < allTierLevelKeys.length; i++){
            if(allTierLevelKeys[i] != _existingTierLevel){
                auxArray[i] = allTierLevelKeys[i];
                // console.log('Loop => %d',i);
            }
        }
        allTierLevelKeys = auxArray;
    }
