...
contract DotcMain is owned {
    ...
    struct User{
	uint40 frozen_timespan; 			//保证金冻结时间，默认填FROZEN_SPAN, admin可以修改
	...
	for(uint i=0; i<collateral_tokens.length; i++)
	{
		...
		if(collateral_wallet_values[i]>0)
		{
			...
			user.balance[token_addr] = user.balance[token_addr].add(collateral_wallet_values[i]);
		}
	}
	return true;
    }
	...
	require(dotc_amount.add(wallet_amount) >= minAdQuantityMantissa, "minimum ad quantity required.");
	...
	if(wallet_amount > 0)
		IERC20(token_addr).transferFrom(msg.sender, address(this), wallet_amount);
	User storage user = accounts[msg.sender];
	total_ad_num = total_ad_num.add(1);
	//user.balance[token_addr] = user.balance[token_addr].add(wallet_amount).sub(dotc_amount);
	user.balance[token_addr] = user.balance[token_addr].add(wallet_amount);
	...
	for(uint i=0; i<collateral_tokens.length; i++)
	{
		...
		if(tk.addr == token_addr)
			ind = i;
		sellerCollateral[msg.sender][total_ad_num][i].token = tk;
		sellerCollateral[msg.sender][total_ad_num][i].token_amount = collateral_dotc_values[i].add(collateral_wallet_values[i]);
		user.frozen[tk.addr] = user.frozen[tk.addr].add(sellerCollateral[msg.sender][total_ad_num][i].token_amount);
		if(collateral_wallet_values[i]>0)
			IERC20(tk.addr).transferFrom(msg.sender, address(this), collateral_wallet_values[i]);
	}
	if(ind != 999)
	...
	*/
	...
	if(ad.token_amount == 0)
	{
		...
		//_removeAtIndex(userAdsMap[seller_addr], ind);
	}else
		ad.status = 2;
     ...
     *
     */
    function new_buy_order(address token_addr, uint256 buy_price, uint256 buy_amount, address seller_addr, uint256 sell_ad_id, address[] memory collateral_tokens, uint256[] memory collateral_dotc_values, uint256[] memory collateral_wallet_values) public returns (uint256){
	require(is_stop == false, "System maintance");
	//_updateUserStored(msg.sender);
	require(isListed(token_addr), "token is not supported yet.");
	require(token_addr != address(0), "Token contract address is 0x0.");
	...
	uint256 order_id = _new_order(seller_addr, sell_ad_id, msg.sender, token_addr, buy_amount, buy_price);
    	...
    	emit NewBuyOrder(token_addr, msg.sender, order_id, buy_price, buy_amount, collateral_tokens, collateral_dotc_values, collateral_wallet_values);
	return order_id;
    }

    //删除指定索引的数组元素
    /*
    function _removeAtIndex(uint256[] storage array,  uint index) internal returns (uint[] memory) {
  ...
  */
	...
	for(uint i=0; i<userOrdersMap[account].length; i++)
	{
		...
		if(account == order.buyer)
		{
			...
			for(uint j=0; j<buyerCollateral[account][i].length; j++)
			{
				...
				user.balance[cl.token.addr] = user.balance[cl.token.addr].add(cl.token_amount);
				user.frozen[cl.token.addr] = user.frozen[cl.token.addr].sub(cl.token_amount);
			}				
			//delete order id
			delete userOrdersMap[account][i];
			//_removeAtIndex(userOrdersMap[account],i);
		}
	...
	buyer.balance[order.token] = buyer.balance[order.token].add(order.token_amount).sub(penalty);
	...
	Order memory order = orderMaps[order_id];
	User storage user = accounts[account];
	uint40 frozen_span = user.frozen_timespan;
