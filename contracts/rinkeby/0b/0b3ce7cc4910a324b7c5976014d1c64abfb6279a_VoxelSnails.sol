...
contract VoxelSnails is IERC721, Ownable, Functional {
    ...
    constructor() {
        _name = "Voxel Snails";
        _symbol = "VXSNL";
        _baseURI = "https://website/metadata/";
        
        totalTokens = 5555;
        //SlimySnails = ERC721(0x1466A19c4D83A3802e970BbEA456A0D1e09e3aAB);
        SlimySnails = ERC721(0xA54fEB14fF05db5C5e50fB46881BF4c39Acb19C6);
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return  interfaceId == type(IERC721).interfaceId ||
                ...
                interfaceId == VoxelSnails.onERC721Received.selector;
    }
    ...
    function airDrop(uint256 tokenStart, uint256 tokenStop) external onlyOwner {
        require(tokenStop < totalTokens);
        address holder;

        for (uint256 i = tokenStart; i <= tokenStop; i++){
        	//check that the token has not already been minted
            if(!_exists(i)){
            	//get the address of the token owner
            	holder = SlimySnails.ownerOf(i);

        	    //mint new Voxel to holder address
		        _safeMint(holder, i);
