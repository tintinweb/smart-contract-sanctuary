...
contract Primate69Club is ERC721, Ownable {
  ...
  uint256 public presaleStartTime = 1644467451; 
  ...
  function presaleMint(uint256 _mintAmount) public payable {
    uint256 s = tokenSupply.current();
    require(!paused);
    require(whitelistedAddresses[msg.sender], "Primate69Club: You are not whitelisted");
    require(isPresaleOpen(), "Primate69Club: Presale is not open");
    require(_mintAmount > 0, "Primate69Club: Please mint atleast one NFT");
    require(s + _mintAmount <= maxSupply, "Primate69Club: Total mint amount exceeded");
    require(msg.value == cost * _mintAmount,"Primate69Club: not enough ether sent for mint amount");
    (bool successT, ) = treasury.call{ value: (msg.value*92)/100 }(""); // forward amount to treasury wallet
