...
*/
    ...
    uint256 public INITIAL_EMISSIONS_RATE = 853000000000000;

    // translates to 2 gold / day
    uint256 public FINAL_EMISSIONS_RATE = 341200000000000;

    uint256 PERIOD_ONE = 13491808;

    address NULL_ADDRESS = 0x0000000000000000000000000000000000000000;

    address public fortressAddress;
    ...
    // Mapping of staker to fortress
    mapping(address => uint256[]) internal stakerToTokenIds;

    constructor() ERC20("Gold", "GOLD") {}

    function setFortressAddress(address _fortressAddress) public onlyOwner {
        fortressAddress = _fortressAddress;
        return;
    }
    ...
    function unstakeAll() public {
        ...
        for (uint256 i = stakerToTokenIds[msg.sender].length; i > 0; i--) {
            ...
            uint256 tokenStakedTimestamp = tokenIdToTimeStamp[tokenId];
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp < PERIOD_ONE) {
            ...
            if (timeBetweenStakeAndPeriodOne > 0) {
                // started before
                // reward for initial period
                rewards = timeBetweenStakeAndPeriodOne * INITIAL_EMISSIONS_RATE;
                // reward for final period
                rewards =
                    rewards +
                    (blockTimestamp -
                        (tokenStakedTimestamp + timeBetweenStakeAndPeriodOne)) *
                    FINAL_EMISSIONS_RATE;
            } else {
                // started after
                // reward for final period
                rewards =
        ...
        returns (uint256)
    {
        ...
        return secondsStaked * getEmissionRate();
    }
    ...
    function getEmissionRate() public view returns (uint256) {
        if (block.number < PERIOD_ONE) {
            return INITIAL_EMISSIONS_RATE;
        } else {
            return FINAL_EMISSIONS_RATE;
        }
    }
}
