...
contract CxpDEX is Pausable {
    ...
    function slippage(address tokenIn, uint amountIn) public view returns (uint) {
        require(amountIn > 0, 'UniswapV2Library: INSUFFICIENT_INPUT_AMOUNT');
        if (tokenIn == address(tokenA)) {
            uint256 tokensBought = swapPrice(tokenIn, address(tokenB), amountIn);
            return currentRate.sub(tokensBought.div(amountIn)).mul(10000).div(currentRate);
        } else {
            uint256 tokensBought = swapPrice(tokenIn, address(tokenA), amountIn);
            return currentRate.sub(amountIn.div(tokensBought)).mul(10000).div(currentRate);
        }
    }
