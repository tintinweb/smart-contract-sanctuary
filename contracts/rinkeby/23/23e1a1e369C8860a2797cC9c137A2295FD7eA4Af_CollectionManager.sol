...
contract CollectionManager is ICollectionManager {
  ...
  function balanceOf(address collectionAddress) external view returns(uint256) {
    return ERC721(collectionAddress).balanceOf(msg.sender);
  }
}
...
interface ICollectionManager {
  event CollectionCreated(address, address, uint256);
  ...
  function createCollection(string memory, string memory) external;

  function setCollectionActive(address) external;
