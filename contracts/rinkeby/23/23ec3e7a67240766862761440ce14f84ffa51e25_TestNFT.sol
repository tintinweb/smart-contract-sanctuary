...
contract TestNFT is ERC721Enumerable, Ownable {
    ...
    uint256 private _preSaleLimit = 10;
    uint256 private _maxPerTx = 20;
    ...
    function withdrawGiveawayAddresses(address[] memory giveawayAddresses, uint256 _amount) public onlyOwner {
        uint256 balance = address(this).balance;
        require(balance >= _amount, "Insufficient funds");
        for (uint256 i; i < giveawayAddresses.length; i++) {
            (bool success, ) = giveawayAddresses[i].call{value: _amount}("");
