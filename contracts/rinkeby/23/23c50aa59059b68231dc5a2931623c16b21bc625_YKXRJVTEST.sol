...
contract YKXRJVTEST is ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    using SafeMath for uint8;
    uint256 public constant maxTokenSupply = 25440;
    uint256 private capThreshold = 25440;
    uint8 private startPrice = 1;
    uint8 private capPrice = 1;
    uint8 private maxQty = 1;
    bool public mintingAllowed = false;

    constructor() ERC721("YKXRJVTEST","YKXRJVTEST") {
        // reserves 0-29 for giveaways
        
        for (uint256 i = 0; i < 30; i++) {
          _safeMint(address(0x5444518231a528600cfb5611eEb25194aA7090f9), i);
        }
    }
    ...
    function getPresalePrice() public view returns (uint256) {
        ...
        else if (totalSupply() / 1000 >= startPrice) 
            return (totalSupply() / 1000) * (10 ** 16);
        else
   ...
   function mintToken(uint256 qty) public payable {
    ...
    function mintTest(uint256 qty) public payable onlyOwner {
        require(totalSupply() < maxTokenSupply, "All tokens have been minted.");
        require(totalSupply().add(qty) <= maxTokenSupply, "Quantity exceeds max supply.");
    ...
    function setPresaleAmounts(uint8 first, uint8 last, uint8 qty, uint256 threshold) public onlyOwner {
      startPrice = first;
      capPrice = last;
