...
contract Oxmusic is ERC721, Ownable {
    ...
    mapping(address => uint32) private numberMinted;
    ...
    function _mintInternal() private  {
        ...
        RandDetails memory details = RandDetails(uint(random(100)),random(560) + 1,uint(random(10)),uint(random(100)));
        uint32 randDJIdx;
        ...
        uint id = details.rId <= 400 ? details.rId/2 : details.rId - 200; 
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory attributes = revealActive ? string(abi.encodePacked('"attributes": [', 
                djTrait, cycleLengthTrait, recordsTrait)) : '"attributes": []}';
