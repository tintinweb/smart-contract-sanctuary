...
contract TokenPool is ITokenPool, ERC20 {
    ...
    function setPoolId(uint256 _poolId) external override {
        ...
        poolInfo.poolId = BoringMath.to64(_poolId);
    }
     ...
     */
        ...
        if (balanceOf(_user) == 0 && staker.lastRewardPoints == 0 ) {
          staker.lastRewardPoints = poolInfo.rewardsPerTokenPoints;
        }
    ...
    function updateReward(
        ...
        override
    {
        PoolInfo storage info = poolInfo;

        rewardsContract.updateRewards();
        uint256 currentRewards = rewardsContract.poolRewards(uint256(info.poolId), uint256(info.lastUpdateTime), block.timestamp);

        if (totalSupply() > 0) {
            info.rewardsPerTokenPoints = info.rewardsPerTokenPoints + BoringMath.to128(currentRewards * 1e18 * pointMultiplier / totalSupply());
        }
        
        info.lastUpdateTime = BoringMath.to64(block.timestamp);
        uint256 rewards = rewardsOwing(_user);
        Staker storage staker = stakers[_user];
        if (_user != address(0)) {
            staker.lastRewardPoints = info.rewardsPerTokenPoints; 

            address[] memory rewardTokens = rewardsContract.rewardTokens();

            for(uint i = 0; i < rewardTokens.length; i++) {
                /// GP: TODO I'm pretty sure there needs to be different rewards owing for each reward token
                /// GP: TODO For example, 1000 JELLY and 20 DD
    ...
    /// @notice Returns the poolId
    function poolId() external override view returns (uint256) {
        return uint256(poolInfo.poolId);
    }
    ...
    function rewardsOwing(
        ...
        returns(uint256)
    {
        uint256 newRewardPerToken = uint256(poolInfo.rewardsPerTokenPoints - stakers[_user].lastRewardPoints);
    ...
    {   
        ...
        uint256 newRewardPerToken = uint256(info.rewardsPerTokenPoints) + (currentRewards
                                                                * 1e18
                                                                * pointMultiplier
                                                                * totalSupply())
                                                         - uint256(stakers[_user].lastRewardPoints);
    ...
    function _beforeTokenTransfer(
        ...
        // GP: TODO Understand what this does and comment below
    ...
    ) public 
    {
        ...
        poolInfo.lastUpdateTime = BoringMath.to64(block.timestamp);
        initialised = true;
        vault = _vault;
    }
