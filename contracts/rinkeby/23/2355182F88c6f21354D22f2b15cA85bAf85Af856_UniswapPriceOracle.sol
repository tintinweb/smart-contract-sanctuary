...
contract UniswapPriceOracle is UniswapPriceOracleStorage, PriceOracle, OracleErrorReporter {
    ...
    event AssetPairUpdated(address asset, address pair, uint startTimeStamp);
    ...
    function update(address asset) public returns (uint) {
        ...
        if (isNewAsset(asset)) {
            ...
            if (pair != address(0)) {
                ...
                assetPairStartedTime[asset] = getBlockTimeStamp();
    ...
    function isNewAsset(address asset) public view returns (bool) {
        return bool(cumulativePrices[assetPair[asset]][asset].blockTimeStampPrevious == 0
                    || cumulativePrices[assetPair[asset]][asset].blockTimeStampPrevious <= assetPairStartedTime[asset]);
    }
    ...
    function _updateAssetPair(address asset, address pair) public returns (uint) {
        ...
        assetPairStartedTime[asset] = timeStamp;

        emit AssetPairUpdated(asset, pair, timeStamp);
...
contract UniswapPriceOracleStorage {
    ...
    mapping(address => uint) public assetPairStartedTime;
