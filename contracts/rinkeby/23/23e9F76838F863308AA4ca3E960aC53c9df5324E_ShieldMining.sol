...
contract ShieldMining is IShieldMining, OwnableUpgradeable, ReentrancyGuard, AbstractDependant {
    ...
    function updateTotalSupply(
    ...
    ) external override updateReward(_policyBook, _liquidityProvider, _userLeveragePool) {
        require(
        ...
        IPolicyBook coveragePool = IPolicyBook(_policyBook);
        IPolicyBookFacade _policyFacade = IPolicyBookFacade(coveragePool.policyBookFacade());

        uint256 _userLeveragePoolsCount = _policyFacade.countUserLeveragePools();

        uint256 _poolUtilizationRation =
            coveragePool.totalCoverTokens().mul(PERCENTAGE_100).div(coveragePool.totalLiquidity());

        if (_userLeveragePool != address(0)) {
            _participatedLeverageAmounts = clacParticipatedLeverageAmount(
                _userLeveragePool,
                _policyFacade,
                _poolUtilizationRation
            );
    ...
    function clacParticipatedLeverageAmount(
        address _userLeveragePool,
        IPolicyBookFacade _policyFacade,
        uint256 _poolUtilizationRation
    ...
    {
        address _userPool = _userLeveragePool != address(0) ? _userLeveragePool : _policyBook;
        uint256 reward = _rewards[_msgSender()][_userPool];

        if (reward > 0) {
            delete _rewards[_msgSender()][_userPool];
    ...
    function getReward(address _userLeveragePoolAddress) public override nonReentrant {
        ILeveragePortfolio _userLeveragePool = ILeveragePortfolio(_userLeveragePoolAddress);
        address[] memory _coveragePools =
            _userLeveragePool.listleveragedCoveragePools(
                0,
                _userLeveragePool.countleveragedCoveragePools()
            );
        for (uint256 i = 0; i < _coveragePools.length; i++) {
            if (getShieldTokenAddress(_coveragePools[i]) != address(0)) {
                getReward(_coveragePools[i], _userLeveragePoolAddress);
            }
        }
    }
