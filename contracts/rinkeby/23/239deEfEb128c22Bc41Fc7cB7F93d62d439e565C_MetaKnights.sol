...
contract MetaKnights is ERC721Enumerable, Ownable {
    ...
    uint256 public RESERVED_NFT = 1000;

    uint256 public RESERVE_PRICE = 0.03 ether;

    uint256 public NFT_PRICE = 0.05 ether;
    uint256 public MAX_MINT = 10;
    
    uint256 public MAX_NFT_WALLET = 20;
    ...
    constructor(string memory baseURI, uint256 _publicSaleStartTimestamp, address testERCAddress) ERC721("NFT", "MK") {
        baseTokenURI = baseURI;
        publicSaleStartTimestamp = _publicSaleStartTimestamp;
        _testERCAddress = testERCAddress;
    }
    ...
    function reserveMINT(uint256 amountOfNFTs) external payable {
        ...
        require(amountOfNFTs > 0, "Must mint at least one kat");
    ...
    function mintNFT(uint256 amountOfNFTs) external payable {
        ...
        require(totalSupply() < MAX_NFT_SUPPLY, "All tokens have been minted");
        require(amountOfNFTs <= MAX_MINT, "Casnnot purchase this many tokens in a transaction");
        require(totalSupply() + amountOfNFTs <= MAX_NFT_SUPPLY, "Minting would exceed max supply");
        require(_totalClaimed[msg.sender] + amountOfNFTs <= MAX_NFT_WALLET, "Purchase exceeds max allowed per wallet");
        require(amountOfNFTs > 0, "Must mint at least one kat");
