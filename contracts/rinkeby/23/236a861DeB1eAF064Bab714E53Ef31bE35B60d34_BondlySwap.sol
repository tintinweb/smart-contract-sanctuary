...
contract BondlySwap is Ownable {
    ...
    event BatchAddedNewTokens(address[] tokenAddresses);
    event RemovedToken(address indexed tokenAddress);
    event BatchRemovedTokens(address[] tokenAddresses);
    event NFTListed(uint256 listId, address indexed lister);
    event ListVisibilityChanged(
        uint256 listId,
        bool isPrivate,
    ...
    function _removeToken(address contractAddr) external onlyOwner {
        require(supportTokens[contractAddr] == true, "BSwap: not supported");
        supportTokens[contractAddr] = false;

        emit RemovedToken(contractAddr);
    }
    ...
    function _batchRemoveToken(address[] calldata contractAddrs)
        external
        onlyOwner
    {
        ...
        emit BatchRemovedTokens(contractAddrs);
    }
        ...
        onlyOwner
    {
        ...
        emit BatchAddedNewTokens(contractAddrs);
    }
    ...
    function _sendToken(
        ...
        if (tokenType == TokenType.T1155) {
            IERC1155 _t1155Contract = IERC1155(contractAddr);
            require(
                _t1155Contract.balanceOf(from, tokenId) >= count,
                "BSwap: Do not have nft"
            );
            require(
                _t1155Contract.isApprovedForAll(from, address(this)) == true,
                "BSwap: Must be approved"
            );
            _t1155Contract.safeTransferFrom(from, to, tokenId, count, "");
        } else if (tokenType == TokenType.T721) {
            IERC721 _t721Contract = IERC721(contractAddr);
            require(
                _t721Contract.ownerOf(tokenId) == from,
                "BSwap: Do not have nft"
            );
            require(
                _t721Contract.isApprovedForAll(from, address(this)) == true,
                "BSwap: Must be approved"
            );
            IERC721(contractAddr).safeTransferFrom(from, to, tokenId, "");
        } else {
            IERC20 _t20Contract = IERC20(contractAddr);
            require(
                _t20Contract.balanceOf(from) >= tokenId,
                "BSwap: Do not enough funds"
            );
            require(
                _t20Contract.allowance(from, address(this)) >= tokenId,
                "BSwap: Must be approved"
            );
    ...
    function createSwap(
        uint256[] calldata arrTokenTypes,
        address[] calldata arrContractAddr,
        uint256[] calldata arrTokenIds,
        uint256 swapType,
        uint256 endTime,
        address bidder,
        bool _isPrivate,
        ...
        require(
            ...
            "BSwap: expected at least 1 desire"
        );

        require(batchCount >= 1, "BSwap: expected at least 1 count");
        for (i = 0; i < arrTokenTypes.length; i += 1) {
        ...
        if (isWhitelisted == false) {
            uint256 feeValue = platformFee.mul(10**16);
            require(msg.value >= feeValue, "BSwap: out of fee");

            feeCollector.transfer(feeValue);
            msg.sender.transfer(msg.value.sub(feeValue));
        }
        ...
        for (i = 1; i < arrTokenTypes.length; i++) {
            if (arrContractAddr[i] == address(0)) break;
        ...
        onlyNotEmergency
    {
        ...
        for (uint256 i = 0; i < targets[listId].tokenTypes.length; i++) {
            ...
            if (tempCardTokenType == TokenType.T1155) {
                ...
                require(
                    _t1155Contract.balanceOf(msg.sender, tempCardTokenId) >=
                        batchCount,
                    "BSwap: Do not have nft"
                );
                require(
    ...
    function setVisibility(
        uint256 listId,
        bool _isPrivate,
        address _allowedBidder
    ) external onlyValidList(listId) onlyListOwner(listId) {
        listings[listId].isPrivate = _isPrivate;
        listings[listId].allowedBidder = _allowedBidder;

        emit ListVisibilityChanged(listId, _isPrivate, _allowedBidder);
    }
    ...
    function isAvailableList(uint256 listId)
        public
        view
        onlyValidList(listId)
        returns (bool)
    ...
    function isActiveList(uint256 listId)
        public
        view
        onlyValidList(listId)
        returns (bool)
