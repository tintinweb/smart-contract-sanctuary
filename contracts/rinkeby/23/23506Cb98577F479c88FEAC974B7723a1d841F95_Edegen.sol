...
contract Edegen is VRFConsumerBase, ERC721Enumerable, Ownable {  
  ...
  struct Token {
    ...
    event eRoll(uint256 tokenId, uint256 num_rolls, bytes32 seed, uint8 level, uint8 s1, uint8 s2, uint8 s3, bool jackpot_flag,
		bool won_mint_pass);

    event eRedeem(uint256 tokenId,uint8 level, uint256 spawned_by);
    ...
    function initial() public requireAdmin {
      //mint starting round to the admin.
      //  TODO change this to lottery
      for (uint i=0;i<num_machines;i++) {
	_mint(admin_address,i);
	emit eInitial(i,0);
      }
    }
    ...
    function redeem(uint256 tid) public payable requireOwner(tid) returns(uint256)  {
      ...
      emit eRedeem(i,tokens[i].level,tokens[i].spawned_by);
	...
	tokens[tid].s3 = uint8((a >> 16*2) % uint256(tokens[tid].level+2)); //no possibility of jackpot
      ...
      emit eRoll(tid,tokens[tid].num_rolls,bytes32(tokens[tid].seed),tokens[tid].level,tokens[tid].s1, tokens[tid].s2, tokens[tid].s3, tokens[tid].jackpot_flag,emit_mint_pass);
    }
