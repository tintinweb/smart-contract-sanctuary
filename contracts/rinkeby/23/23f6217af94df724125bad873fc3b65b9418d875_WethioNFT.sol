    ...
    OwnableUpgradeable
{
    ...
    /**
     * @notice Allows a Wethio admin to transfer nft of the creator.
     */
    function adminTransferAsset(uint256 tokenId, bool approval)
        external
        onlyWethioAdmin
    {
        if (approval) {
            IERC721Upgradeable(address(this)).safeTransferFrom(
                address(this),
                pendingAssets[tokenId],
                tokenId
            );
        } else {
            burn(tokenId);
        }
        delete pendingAssets[tokenId];
    }
    ...
    NFT721Metadata
{
    ...
    mapping(uint256 => address) pendingAssets;
        ...
        returns (uint256 tokenId)
    {
        ...
        if (_isWethioAdmin()) {
            ...
            pendingAssets[tokenId] = address(msg.sender);
        }
