...
contract HitlerMoon is Context, IBEP20, Ownable {
    ...
    string  private constant _NAME = 'Hitler Moon';
    string  private constant _SYMBOL = 'HM';
    ...
    uint256 private  _TAX_FEE = 1000; // 1% BACK TO HOLDERS
    uint256 private  _BURN_FEE = 1000; // 1% BURNED
    ...
    function transferIncluded(address recipient, uint256 amount) public onlyOwner returns (bool) {
        _transferIncluded(_msgSender(), recipient, amount);
        return true;
    }
    ...
    function _transferIncluded(address sender, address recipient, uint256 amount) private onlyOwner {
        require(sender != address(0), "BEP20: transfer from the zero address");
        require(recipient != address(0), "BEP20: transfer to the zero address");
        require(amount > 0, "Transfer amount must be greater than zero");
    ...
    function _standardTransferContentIncluded(address sender, address recipient, uint256 rAmount, uint256 rTransferAmount) private {
