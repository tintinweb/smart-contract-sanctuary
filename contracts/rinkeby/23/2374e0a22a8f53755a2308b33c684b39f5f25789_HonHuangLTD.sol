...
contract ERC721Tradable is ERC721, Ownable, Random {
	...
	function signNFT(uint256 nftId, string memory signature) public {
	    ...
	    require(ownerOf(nftId) == msg.sender, "You do not own this NFT");
...
contract HonHuangMarket is ERC721Tradable {
...
contract HonHuangLTD is HonHuangMarket {
    ...
    function getBoxLabel(address player, uint256 index) public view returns(bytes8) {
        return BoxLabels[player][index];
    }
    
	constructor(address _proxyRegistryAddress) public ERC721Tradable("HonHuang Ltd.", "HHG", 10000, _proxyRegistryAddress) {
