...
contract UniswapLiquidityManager is PeripheryPayments, IUniswapLiquidityManager {
    ...
    function createPair(
        ...
        (uint24 _fee, uint160 _sqrtPriceX96) = abi.decode(abi.encodePacked(data),(uint24,uint160));
    ...
    function collectPositionFees(address _pool) public {
        // only unipilot
    ...
    function withdraw(bool pilotToken, bool wethToken, uint256 liquidity, bytes calldata data)
        external
        override
    {
        // only unipilot
    ...
    function removeLiquidityWithFees(address pool, uint256 liquidity)
        public
...
interface IUniswapLiquidityManager is IULMEvents {
    ...
    function withdraw(bool pilotToken, bool wethToken, uint256 liquidity, bytes calldata data) external;
...
interface IHandler {
    ...
    function isAuthorizedForToken(uint256 tokenId) external view returns(bool approved);
...
contract Unipilot is IUnipilot, ERC721Permit, PeripheryPayments {
    ...
    constructor(address uniswapLiquidityManager)
        ERC721Permit("Unipilot Positions NFT-V1", "PILOT-POS-V1", "1")
    {
        governance = msg.sender;
        exchangeWhitelist[uniswapLiquidityManager] = true;
    }
    ...
    // function isAuthorizedForToken(uint256 tokenId) public view returns(bool approved) {
    //     approved = _isApprovedOrOwner(msg.sender, tokenId);
    // }
    ...
    function createPoolAndDeposit(IHandler.DepositParams memory params, bytes[] calldata data) external payable {
        IHandler(params.exchangeAddress).createPair(params.token0, params.token1, data[0]);
        ...
        deposit(
            IHandler.DepositParams({
                ...
                token0: params.token0,
                token1: params.token1,
                amount0Desired: params.amount0Desired - 1,
    ...
    function deposit(IHandler.DepositParams memory params, bytes calldata data)
        public
        payable
    {
        require(
        ...
        pay(params.token0, _msgSender(), params.exchangeAddress, params.amount0Desired);
        pay(params.token1, _msgSender(), params.exchangeAddress, params.amount1Desired);

        if (msg.value > 0)
            TransferHelper.safeTransferETH(params.exchangeAddress, msg.value);
    ...
    function withdraw(IHandler.WithdrawParams calldata params, bytes calldata data)
        external
    {
        require(
            exchangeWhitelist[params.exchangeAddress] == true,
        ...
        IHandler(params.exchangeAddress).withdraw(params.pilotToken, params.wethToken, params.liquidity, data);
    }

    function collect(IHandler.CollectParams calldata params, bytes calldata data)
        external
        payable
    {
        require(
