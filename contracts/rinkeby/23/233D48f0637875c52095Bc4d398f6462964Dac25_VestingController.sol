...
contract StakedBalanceFetcher is Ownable {
    ...
    event StakingContractAdded(address stakingContract);
    event StakingContractRemoved(address stakingContract);

    StakingContractI[] stakingContracts;
...
contract VestingStorage is Ownable {
    ...
    mapping(string => VestingCategory) vestingCategoriesByName;
...
contract VestingController is StakedBalanceFetcher, VestingStorage, RecoverTokens {
    ...
    event AmountClaimed(address indexed vester, address recipient, uint amount);
