...
contract Misfit is ERC721Enumerable, EIP712, Ownable {
    ...
    // Wallet to receive the funds from contract
    ...
    // voucher for client to redeem
    struct NFTVoucher {
        // specify client to redeem this voucher
        address client;
        // ID to check if voucher has been redeemed
        uint256 redeemId;
        // token price
        uint256 price;
        // max amount to mint
    ...
    // setup ERC721, EIP712 and wallet
    constructor(
        uint256 maxSupply_,
    ...
    // Withdraw all funds from contract to wallet
        ...
        payable
    {
        ...
        require(
            _clientRedeemId[_msgSender()] < voucher.redeemId,
        ...
        view
    {
        bytes32 digest = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256(
                        "NFTVoucher(address client,uint256 redeemId,uint256 price,uint256 amount)"
                    ),
                    _msgSender(),
                    voucher.redeemId,
                    voucher.price,
