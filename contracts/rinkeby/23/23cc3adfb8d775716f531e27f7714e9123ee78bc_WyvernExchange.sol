...
contract ExchangeCore is ReentrancyGuarded, Ownable {
        ...
        reentrancyGuard
    {
        ...
        if (buy.maker == msg.sender) {
            require(validateOrderParameters(buy), "validate order parameters buy failed.");
        } else {
        ...
        if (sell.maker == msg.sender) {
            require(validateOrderParameters(sell), "validate order parameters sell failed.");
        } else {
            sellHash = requireValidOrder(sell, sellSig);
        }
        
        /* Must be matchable. */
        require(ordersCanMatch(buy, sell), "orders can match failed.");
        ...
        require(size > 0, "extcode size positive failed.");
        ...
        require(ArrayUtils.arrayEq(buy.calldata, sell.calldata), "array utils array eq failed.");

        /* Retrieve delegateProxy contract. */
        OwnableDelegateProxy delegateProxy = registry.proxies(sell.maker);

        /* Proxy must exist. */
        require(delegateProxy != address(0), "delegate proxy address failed.");

        /* Assert implementation. */
        require(delegateProxy.implementation() == registry.delegateProxyImplementation(), "delegate proxy implementation failed.");
        ...
        require(proxy.proxy(sell.target, sell.howToCall, sell.calldata), "proxy call sell failed");
...
contract AuthenticatedProxy is TokenRecipient, OwnedUpgradeabilityStorage {
    ...
    function proxy(address dest, HowToCall howToCall, bytes calldata)
        ...
        returns (bool result)
    {
        require(msg.sender == user || (!revoked && registry.contracts(msg.sender)), "proxy check failed.");
