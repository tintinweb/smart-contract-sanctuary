...
contract LarvaGeekPunks is ERC721("Larva Geek Punks", "GEPU") {
  ...
  uint256 public constant totalSupply = 5_222;
  ...
  function purchaseTokens(uint256 _amount) external payable tokensAvailable(_amount) {
        require(circulatingSupply < 1111, "Free claiming ended.");
        require(_amount > 0 && _amount <= 20, "Mint min 1, max 20");
        for (uint256 i = 0; i < _amount; i++)
        _mint(msg.sender, ++circulatingSupply);
  }
