...
contract PushToENS is SupportsENS {
  ...
  event Pushed(bytes32 hash, uint256 amount);
  event Pulled(bytes32 hash, uint256 amount);
  ...
  // PUSH FUNCTIONS
    ...
    */
  function _push (bytes32 hash, uint256 amount) internal {
    //prevent overflow
  ...
  function pushEther2Ethadd (address ethAddress) public payable {
    _push(keccak256(abi.encode(ethAddress)), msg.value);
  }
  ...
  function pushTokens2ENS (bytes32 nameHash, address tokenAddress, uint256 amount) public {
     ...
     // save
    _push(keccak256(abi.encode(nameHash,tokenAddress)), amount);
  }
   ...
   function pushNFT2ENS (bytes32 nameHash, address nftAddress, uint256 tokenId) public {
     ...
     // save
    _push(keccak256(abi.encode(nameHash, nftAddress, tokenId)), 1);
  }
  ...
  // PULL FUNCTIONS
    ...
    */
  function _pull (bytes32 hash, uint256 amount) internal {
    //Checks
  ...
  function pullEther2Ethadd (address payable ethAddress, uint256 amount) public {
      //generic pull
      _pull(keccak256(abi.encode(ethAddress)), amount);
  ...
  function pullToken2ENS (bytes32 nameHash, address tokenAddress, uint256 amount) public {
     ...
     //generic pull
      _pull(keccak256(abi.encode(nameHash,tokenAddress)), amount);

     // transfer tokens
     // getSafeENSAddress prevents returning empty address
     require(token.transferFrom( address(this), getSafeENSAddress(nameHash), amount), "Token transfer failed");
  }
  ...
  function pullNFT2ENS (bytes32 nameHash, address nftAddress, uint256 tokenId) public {
     ...
     //generic pull
      _pull(keccak256(abi.encode(nameHash, nftAddress, tokenId)), 1);

     // transfer tokens
     // getSafeENSAddress prevents returning empty address
     NFT.safeTransferFrom(address(this), getSafeENSAddress(nameHash), 1);
  }
