...
contract TomiQuery2 {
    ...
    function generateProposal(address ballot_address, bool revenueProposal) public view returns (Proposal memory proposal){
    ...
    function queryProposalList() public view returns (Proposal[] memory proposal_list){
        ...
        for(uint i = 0;i < count;i++) {
            ...
            uint ballot_type = ITomiGovernance(governance).ballotTypes(ballot_address);
            bool isRevenueProposal = ballot_type == T_REVENUE;
            proposal_list[count - i - 1] = generateProposal(ballot_address, isRevenueProposal);
        }
    }
    ...
    function iterateProposalList(uint _start, uint _end) public view returns (Proposal[] memory proposal_list){
        ...
        for(uint i = 0;i < count;i++) {
            ...
            uint ballot_type = ITomiGovernance(governance).ballotTypes(ballot_address);
            bool isRevenueProposal = ballot_type == T_REVENUE;
            proposal_list[index] = generateProposal(ballot_address, isRevenueProposal);
            index++;
        }
    }
