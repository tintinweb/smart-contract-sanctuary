...
contract Receiver is Ownable {
    ...
    function getTotalByAddr() public view returns (uint) {
        uint percent = _percentByAddr[_msgSender()];
        require(percent > 0, "Receiver: This address has no share");
        return _totalReceived * percent / 100;
    }

    function getPaidByAddr() public view returns (uint) {
        return _paidByAddr[_msgSender()];
    }

    function getBalanceByAddr() public view returns (uint) {
        return getTotalByAddr() - getPaidByAddr();
    }

    function withdrawByAddr() public returns (bool) {
        address recipient = payable(_msgSender());        
        uint amount = getBalanceByAddr();
