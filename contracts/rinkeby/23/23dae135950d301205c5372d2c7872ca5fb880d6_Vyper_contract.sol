...
def __init__():
    ...
    self.tokenName = "TestToken2"
    self.tokenSymbol = "TT2"
...
def supportsInterface(_interfaceID: bytes32) -> bool:
    ...
    """
    tempa: Bytes[4] = slice(_interfaceID, 0, 4)
    tempb: bytes32 = convert(convert(tempa, uint256), bytes32)
    return self.supportedInterfaces[tempb]

    
@view
@external
def testSupportsInterface(_interfaceID: Bytes[4]) -> bytes32:
    #return slice(_interfaceID, 0, 4)
    #return extract32(_interfaceID, 0, output_type=bytes32)
    return convert(convert(_interfaceID, uint256), bytes32)
...
def tokenURI(_tokenId: uint256) -> String[80]:
    return concat(self.token_uri, "{id}.json")
