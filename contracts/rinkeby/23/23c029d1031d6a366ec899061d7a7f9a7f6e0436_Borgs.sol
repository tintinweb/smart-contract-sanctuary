...
abstract contract Editable is Ownable {
    ...
    */
    ...
    function isEditable() view external returns(bool){
        return _isEditable;
    }
}
...
contract BorgFactory is CommonObjects, Editable{
    ...
    function _getBorgPeice(string memory borgPeiceName, uint256 expectedSize) internal view onlyOwner returns (string[] memory imagePixals){
        BorgPeice storage borgPeice = _borgPeices[borgPeiceName];
    ...
    function _combineBorgPeices(string[] memory borgPeiceNames, uint256 expectedSize) internal view returns (string[] memory hexPixals){
        // Init the 
        hexPixals = new string[](expectedSize);
...
contract Borgs is ERC721Enumerable, BorgFactory, NumberUtils{
    ...
    function getBorgImage(uint256 borgId) public view returns(string[] memory){
        ...
        return _combineBorgPeices(borg.borgPeiceNames, IMAGE_SIZE);
    }
    
    function getLayersBorgPeice(string memory layerName, uint256 position) public view onlyOwner returns (string[] memory){
        // Get existing layer
        ...
        string[] memory borgPeice = _getBorgPeice(layerItem.borgPeiceName, IMAGE_SIZE);
    ...
    function _filterRarestBorgPeices(uint256[] memory layerItemPosition1, uint256[] memory layerItemPosition2) internal view returns(uint256[] memory rarestLayerPositions, string[] memory borgPeiceNames){
        // Check the peices size is the same
