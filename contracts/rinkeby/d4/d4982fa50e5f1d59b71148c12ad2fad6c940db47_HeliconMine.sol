...
contract HeliconMine is Ownable {
    ...
    function getPendingRewardInternalOld(address _lpToken, address _user) internal view returns(uint256){
        uint256 pid = getPid(_lpToken);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 daysNow = block.number.sub(startBlock).div(avgBlockPerDay);
        uint256 HeliconReward = 0;
        for(uint256 i = 1; i <= daysNow;i++){
                //计算当天最大区块
                uint256 todayMaxBlock = startBlock.add(avgBlockPerDay.mul(i));
                //平均每个区块奖励值
                uint256 unRewardedBlock = todayMaxBlock.sub(pool.lastRewardBlock);
                uint256 singleBlockRewarded = getReleasePerBlock(i,IERC20(pool.lpToken).decimals());
                HeliconReward = HeliconReward.add(
                    unRewardedBlock
                    .mul(singleBlockRewarded)
                    .mul(pool.allocPoint)
                    .div(totalAllocPoint)
