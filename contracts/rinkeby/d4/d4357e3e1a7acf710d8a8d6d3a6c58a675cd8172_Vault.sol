...
contract Vault is IVault, PausableUpgradeable, AccessControlEnumerableUpgradeable {
    ...
    AggregatorV3Interface public override ethPriceFeed;
    // 18 decimal
    ...
    function initialize(address _rhoAddr, address _underlyingAddr, address _udlyEthPriceFeedAddr, uint256 _mFee, uint256 _rFee, uint256 _mngtFee, uint256 _rLowerBound, uint256 _rUpperBound ) public initializer {
        ...
        _setEthPriceFeed(_udlyEthPriceFeedAddr);
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    function setEthPriceFeed(address addr) external override onlyRole(FEE_MNG_ROLE) whenNotPaused {
        _setEthPriceFeed(addr);
    }
    function _setEthPriceFeed(address addr) internal {
        ethPriceFeed = AggregatorV3Interface(addr);
    }

    function setMintingFee(uint256 _feeInWei) external override onlyRole(FEE_MNG_ROLE) whenNotPaused {
    ...
    /* redeem Fee */
    function setRedeemFee(uint256 _feeInWei) external override onlyRole(FEE_MNG_ROLE) whenNotPaused {
        _setRedeemFee(_feeInWei);
    }
    function _setRedeemFee(uint256 _feeInWei) internal {
        redeemFee = _feeInWei;
    }
    function setManagementFee(uint256 _feeInWei) external override onlyRole(FEE_MNG_ROLE) whenNotPaused {
    ...
    function supportsAsset(address _asset) external override view returns (bool) {
        if (_asset == address(underlying)) { return true; }
        return false;
    }

    function reserve() external override view returns (uint256) {
    ...
    function redeem(uint256 amountInRho) external override whenNotPaused {
        ...
        uint256 diff = amountInUnderlying - reserveBalance + _calcReserveLowerBound(totalUnderlyingToBe);  // in underlying

        _withdraw(diff, allocation);
        rhoToken.burn(_msgSender(), amountInRho);
        underlying.safeTransfer(_msgSender(), amountInUnderlying);
    }
    ...
    function rebase() external override onlyRole(REBASE_ROLE) whenNotPaused {
        // rebalance fund
        (uint256 underlyingInvested, ) = _rebalance();
        ...
        uint256 rhoRebasing = rhoToken.rebasingSupply();
    ...
    function _rebalance() internal returns(uint256 underlyingInvested, uint index) {
        return _allInAllOut();
    }

    function _allInAllOut() internal returns(uint256 underlyingInvested, uint index)  {
        uint originalIndex = allocation;
        uint256 _underlyingInvested = _updateBalanceOfUnderlying(originalIndex);
        IRhoStrategy originalStrategy = strategies[originalIndex].target;
        if (
            _underlyingInvested < originalStrategy.switchingLockTarget() &&
            block.number < originalStrategy.switchLockedUntil()
        ) {
            // fund is locked
            return (_underlyingInvested, originalIndex);
        }
        uint256 _underlyingUninvested = underlying.balanceOf(address(this));
        uint256 tvl = _underlyingUninvested + _underlyingInvested;
        uint256 upperBound = _calcReserveUpperBound(tvl);
        uint256 excessReserve = 0;
        if (_underlyingUninvested > upperBound) {
            // allocate extra fund
            uint256 lowerBound = _calcReserveLowerBound(tvl);
            excessReserve = _underlyingUninvested - lowerBound;
        }
        uint256 deployAmt = _underlyingInvested + excessReserve;
        uint optimalIndex = _findOptimalStrategy(deployAmt);
        if (originalIndex == optimalIndex) {
            // optimal strategy unchanged
            // only deploy fund that exceed, no withdrawal
            uint256 _gas0 = gasleft();
            _deploy(excessReserve, optimalIndex);
            uint256 _gas1 = gasleft();
            uint256 _ethPrice = 10**ethPriceFeed.decimals() * 1e18 / Flurry.getPriceFromChainlink(ethPriceFeed);
            uint256 _switchingCost = Flurry.calculateGasFee(_ethPrice, _gas0, _gas1); // in 18
            uint256 _switchingCostInUnderlying = _switchingCost * _underlyingOne / 1e18;
            strategies[optimalIndex].target.switchingLock(excessReserve + _switchingCostInUnderlying);
            return (_updateBalanceOfUnderlying(optimalIndex), optimalIndex);
        }
        // optimal strategy changed
        // withdraw all, then deploy withdrawn amount + excess reserve
        uint256 gas0 = gasleft();
        if (_underlyingInvested > 0) {
            _withdrawAll(originalIndex);
        }
        _deploy(deployAmt, optimalIndex);
        allocation = optimalIndex;
        uint256 gas1 = gasleft();
        uint256 ethPrice = 10**ethPriceFeed.decimals() * 1e18 / Flurry.getPriceFromChainlink(ethPriceFeed);
        uint256 switchingCost = Flurry.calculateGasFee(ethPrice, gas0, gas1); // in 18
        uint256 switchingCostInUnderlying = switchingCost * _underlyingOne / 1e18;
        strategies[optimalIndex].target.switchingLock(deployAmt + switchingCostInUnderlying);
        return (_updateBalanceOfUnderlying(optimalIndex), optimalIndex);
    }
    function _updateBalanceOfUnderlying(uint i) internal returns(uint256 tvl){
        return strategies[i].target.updateBalanceOfUnderlying();
    }

    function _findOptimalStrategy(uint256 tvl) internal view returns(uint index) {
        uint256 optimalSupplyRate;
        uint optimalIndex;
        bool hasEnableStrategy = false;
        for (uint i = 0; i < strategies.length; i++) {
            if (!strategies[i].enabled) continue;
            hasEnableStrategy = true;
            uint256 s = strategies[i].target.effectiveSupplyRate(tvl, true);
            if (s > optimalSupplyRate) {
                // optimal by far
                optimalIndex = i;
                optimalSupplyRate = s;
            }
        }
        require(hasEnableStrategy, "no enabled strategy");
        return optimalIndex;
    }
    ...
    function _withdraw(uint256 amount, uint from) internal {
        strategies[from].target.withdrawUnderlying(amount);
    }
    function _withdrawAll(uint from) internal {
        strategies[from].target.withdrawAll();
    }
    function _deploy(uint256 amount, uint to) internal {
    ...
    function supplyRate() external view returns (uint256) {
        uint256 invested = strategies[allocation].target.balanceOfUnderlying();
        uint256 APY = strategies[allocation].target.effectiveSupplyRate();
        uint256 rebasingSupply = rhoToken.rebasingSupply(); // in 18
        if (rebasingSupply == 0) {
            return 0;
        }
        uint256 result = APY * invested * _rhoOne / rebasingSupply / _underlyingOne;
        return result;
    }
...
library Flurry {
    ...
    function getPriceFromChainlink(AggregatorV3Interface pricefeed) internal view returns(uint256) {
        (, int price, , , ) = pricefeed.latestRoundData();
        return uint256(price);
    }
    function isBlockPassed(uint256 targetBlock) internal view returns(bool){
        return block.number > targetBlock;
    }
    function calculateGasFee(uint256 ethpriceIn18, uint256 gas0, uint256 gas1) internal view returns (uint256) {
        uint256 gasUsedInWei = (gas0 - gas1) * tx.gasprice;

        return gasUsedInWei * ethpriceIn18 / 1e18;
    }
