...
contract Traitsniffer is ERC721Enumerable, Ownable {
    uint256 public subscriptionPrice = 10000000000000000;
    uint256 public maxUsers = 101;
    uint256 constant DAY_IN_UNIX = 86400;
    ...
    constructor() ERC721("Traitsniffer", "SNIFF") {
        subscriptionPlans.push(Subscription(1,1));
        subscriptionPlans.push(Subscription(2,5));
        subscriptionPlans.push(Subscription(3,10));
        subscriptionPlans.push(Subscription(4,15));
        subscriptionPlans.push(Subscription(5,30));
//        subscriptionPlans.push(Subscription(1,1));
//        subscriptionPlans.push(Subscription(2,7));
//        subscriptionPlans.push(Subscription(3,14));
//        subscriptionPlans.push(Subscription(4,30));
//        subscriptionPlans.push(Subscription(5,60));
        //@ TODO BASE URI
    }

    function register(uint _id) public payable {
        require(msg.value == getPrice(_id), "Sent Ether value is not correct");
        require(getActiveSubCount() < maxUsers, "Max user count reached");
        require(subscriptionPlans[_id]._days > 0, "Requested Subscription ID does not exist");
        require(userToTimeRegistered[msg.sender] == 0, "Address is already registered");
        require(whitelisted[msg.sender] == false, "Address is already whitelisted");
...
//        uint256 blockTimeStamp = block.timestamp + (DAY_IN_UNIX * subscriptionPlans[_id]._days);
    ...
    function hasAccess(address _address) external view returns (bool) {
        return userToTimeRegistered[_address] > block.timestamp || whitelisted[_address] == true;
    }
    ...
    function updateSubscription(uint _id) public payable{
        ...
        require(expireTime != 0, "User is not registered yet");
        uint256 timestamp = block.timestamp;
    ...
    function getPrice(uint _id) public view returns(uint256) {
        return subscriptionPrice * (subscriptionPlans[_id]._multiplier);
    }
    ...
    function addAddressWithTimestamp(address _address, uint256 _timestamp) external onlyOwner {
    ...
    function banAddress(address _address) external onlyOwner {
        require(userToTimeRegistered[_address] > 0, "Address is not registered");
        userToTimeRegistered[_address] = 0;
    }

    function whitelistAddress(address _address) external onlyOwner {
        require(whitelisted[_address] == false, "Address is already whitelisted");
        if (!isRegistered(_address)) {
            registeredAddresses.push(_address);
        }
    ...
    function getAddressIndex(address _address) internal view returns (uint) {
