...
// 5) Added migrateV1toV2() function for initializing counters during migration from V1 to V2
// 6) Added migrate() function that transfers collateral, backstop, BXTB, CHIP, yBXTB, etc. to
...
interface Migrate_YieldTokenService {
    function migrate(
        uint _BXTBAmount, uint _USDTAmount, uint _CHIPBackstop,
        uint _CHIPCommissionsAvailable, uint _CHIPCommissionsTotal, uint _CHIPOutstanding,
        uint _CHIPTotal, uint _yieldTokenOutstanding, uint _yieldTokenTotal) external;
}
...
contract YieldTokenService is ApproveAndCallFallBack, SettlementInterface, Owned {
    ...
    // This should be removed after V2 and performed in migrate()
    ...
    function migrateV1toV2(uint _initBackstopAmount, uint _initCommissionsAmount, uint _initCommissionsAvailable) external onlyOwner {
        if((totalPoolCHIPBackStop == 0) && (totalPoolCHIPCommissions == 0) &&
           (totalPoolCHIPCommissionsAvailable == 0) && (projectDeactivated == false)) {
            ...
            externalSupplyCHIP = outstandingCHIP.subSafe(totalPoolCHIPCommissionsAvailable).subSafe(totalPoolCHIPBackStop);
            ...
            // Enable staking and commissions
            allowStaking = true;
    ...
    // Use after V2
    // *** Target contract must adjust own collateralizationRatio and externalSupplyCHIP
//    function migrate(uint _BXTBAmount, uint _USDTAmount, uint _CHIPBackstop, uint _CHIPCommissionsAvailable,
//        uint _CHIPCommissionsTotal, uint _CHIPOutstanding, uint _CHIPTotal, uint _yieldTokenOutstanding,
//        uint _yieldTokenTotal) external override {
//        require(msg.sender == address(***current service contract***), "Caller not allowed");  // Only accept call from the current Service Contract
//
//        totalPoolBXTB = _BXTBAmount;
//        totalPoolUSDTCollateral = _USDTAmount;
//        totalPoolCHIPBackStop = _CHIPBackstop;
//        totalPoolCHIPCommissionsAvailable = _CHIPCommissionsAvailable;
//        totalPoolCHIPCommissions = _CHIPCommissionsTotal;
//        outstandingCHIP = _CHIPOutstanding;
//        totalSupplyCHIP = _CHIPTotal;
//        outstandingYieldToken = _yieldTokenOutstanding;
//        totalSupplyYieldToken = _yieldTokenTotal;
//
//        // Calculate external CHIP supply
//        externalSupplyCHIP = outstandingCHIP.subSafe(totalPoolCHIPCommissionsAvailable).subSafe(totalPoolCHIPBackStop);
//
