...
contract CryptoBirdos is AccessControl, DetailedERC721 {
    ...
    uint256 oneBirdoPrice = 0.004 ether;
    uint256 threeBirdoPrice = 0.0105 ether;
    uint256 fiveBirdoPrice = 0.015 ether;
   ...
   function _generateRandomDna(uint256 _block) public view returns (uint256) {
        uint256 rand = uint(keccak256(abi.encodePacked(now * _block)));
    ...
    function initialSale(address payable _owner) public payable {
        require(_owner != address(0));
        require(_owner == msg.sender);
        require(ownershipTokenCount[_owner] == 0);

        require(birdos.length < 101);

        uint256 blockNumber = block.number - 1;
        _createToken(_owner, _generateRandomDna(blockNumber), initialSalePrice, 0, 0);
    }
