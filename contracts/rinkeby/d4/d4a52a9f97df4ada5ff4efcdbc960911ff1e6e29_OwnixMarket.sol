...
contract OwnixMarket is ERC1155Holder, Ownable, Pausable, IOwnixMarket {
   ...
   */
  function placeBid(uint256 _tokenId, uint256 _index, uint256 _price) public whenNotPaused() {
   ...
   */
  function placeBidWithPermit(uint256 _tokenId, 
    uint256 _index, 
    uint256 _price,
    uint256 _deadline,
    uint8 _v,
    bytes32 _r,
    bytes32 _s) external whenNotPaused() {
     
      if (ownixToken.allowance(msg.sender, address(this)) < _price) {
        IERC20Permit(address(ownixToken)).permit(msg.sender, address(this), _price, _deadline, _v, _r, _s);
      }

      placeBid(_tokenId, _index, _price);
  }
