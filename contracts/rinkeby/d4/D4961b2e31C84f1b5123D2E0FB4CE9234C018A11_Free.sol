...
//    uint8 constant decimals = 18;
    ...
    function depositToken(
        ...
        uint[] memory swapAmounts = getEstimatedETH(token,amount);
        uint256 mintAmount = uint256(swapAmounts[0] *1000);
...
//    }
    ...
    function getEstimatedETH(address sourceTokenAddress,uint sourceTokenAmount) public view returns (uint[] memory) {
        address[] memory path = new address[](2);
        path[0] = uniswapRouter.WETH();
        path[1] = sourceTokenAddress;
        return uniswapRouter.getAmountsIn(sourceTokenAmount, path);
    }
