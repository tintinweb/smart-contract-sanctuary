...
contract InkersHKCollection is ERC1155, Ownable, Pausable  {
    ...
    uint256 constant FIRST_ORIGINAL_PRICE = 0.05 ether;
    uint256 constant FIRST_COPY_PRICE = 0.01 ether;
    uint constant MAXIMUM_ORIGINALS = 120;
    
    constructor() public ERC1155("https://preprod.nft.inkers.app/api/metadatas/{id}-metadatas.json") {
    }
    ...
    function generateNewOriginal() payable public whenNotPaused returns (uint256) {
        ...
        require(
            _tokenIds.current() / 3 < MAXIMUM_ORIGINALS,
    ...
    function orderPrint(uint256 id) whenNotPaused public {
        ...
        if (isCopy(id)) {
            require(
                ...
                "Must own an unprinted token for this id"
            );
    ...
    function getCopyPrice(uint256 id) external view returns (uint256) {
        ...
        uint index;
        uint256 nextCopyPrice = FIRST_COPY_PRICE;
        for (index = 0; index < nextCopyCount; index++) {
            nextCopyPrice = nextCopyPrice * 15 / 10;
        }
        return nextCopyPrice;
    }
    function getNextOriginalPrice() external view returns (uint256) {
        uint256 current = _tokenIds.current()/3/5;
        uint index;
        uint256 nextOriginalPrice = FIRST_ORIGINAL_PRICE;
        for (index = 0; index < current; index++) {
            nextOriginalPrice = nextOriginalPrice * 105 / 100;
        }
