...
// File contracts/PythonNFT.sol

contract PythonNFT is ERC721Burnable, Ownable {
    ...
    constructor(address payable _treasury) public ERC721("PythonNFT", "PYTHON") {
        treasury = _treasury;
    ...
    function mintPythonNFT(uint numberOfTokens) public payable {
        require(saleIsActive, "PythonNFT: Public sale must be active to mint");
        require(totalSupply().add(numberOfTokens) <= TOTAL_SUPPLY, "PythonNFT: Exceeds max supply");
        require(PRICE.mul(numberOfTokens) <= msg.value, "PythonNFT: ETH sent is incorrect");
        require(numberOfTokens <= MAX_MINT_PER_TX, "PythonNFT: Above max tx count");
    ...
    function mintPythonNFTPrivateSale(uint numberOfTokens, uint256 maxCount, bytes memory signature) public payable {
        require(privateSaleIsActive, "PythonNFT: Private Sale must be active to mint");
        require(totalSupply().add(numberOfTokens) <= TOTAL_SUPPLY, "PythonNFT: Exceeds max supply");
        require(PRICE.mul(numberOfTokens) <= msg.value, "PythonNFT: ETH sent is incorrect");
        require(numberOfTokens <= MAX_MINT_PER_TX, "PythonNFT: Above max tx count");
        ...
        require(recoveredAddress != address(0) && recoveredAddress == address(whitelistSigner), "PythonNFT: invalid signature");

        require(presaleBoughtCounts[msg.sender].hasMinted.add(numberOfTokens) <= maxCount, "PythonNFT: Presale max count exceeded");
    ...
    */
    function setStartingIndex() public onlyOwner {
        require(startingIndex == 0, "PythonNFT: Starting index is already set");
        require(startingIndexBlock != 0, "PythonNFT: Starting index block must be set");
