...
abstract contract TradableErc20 is ERC20 {
    ...
    function sendRewardToRandomUser() external onlyOwner {
        require(lastRewardTime + rewardInterval >= block.timestamp);
        _trySendRewardToRandomUser();
    }

    function _trySendRewardToRandomUser() private {
        if (lastRewardTime + rewardInterval < block.timestamp) return;
