...
contract Faucet is Ownable, ReentrancyGuard {
    ...
    uint16 DISTRIBUTION_PERIOD_DAYS = 180;
    uint8 LOCK_PERIODS = 4;
    ...
    uint256 public bonusEndBlock; // Block number when bonus period ends.
    uint256 public tokenPerBlock; // Tokens earned per block.
    uint256 public constant BONUS_MULTIPLIER = 1; // Bonus muliplier for early token makers.  
    ...
    constructor(address _tokenAddress, uint256 _tokenPerBlock, uint256 _startBlock, uint256 _bonusEndBlock, address _lockAddress) public {
    ...
    // Deposit LP tokens to Faucet for token allocation.
    ...
    // Withdraw LP tokens from Faucet.
    ...
    function distributeTokens(uint256 pending) private {
        // uint256 amountToTransfer = pending.div(100).mul(20);
        // token.transfer(msg.sender, amountToTransfer);

        // uint256 lockTotalAmount = pending.div(100).mul(80);
        // uint256 lockPerPeriodAmount = lockTotalAmount.div(LOCK_PERIODS);
        // uint256[] memory amounts = new uint256[](LOCK_PERIODS);
        // uint256[] memory durations = new uint256[](LOCK_PERIODS);
        // address payable[] memory beneficiaries = new address payable[](LOCK_PERIODS);
        // uint256[] memory amountThresholds = new uint256[](LOCK_PERIODS);
        
        // for (uint256 i = 0; i < LOCK_PERIODS; i++) {
        //     amounts[i] = lockPerPeriodAmount;
        //     durations[i] = (i.add(1)).mul(SECONDS_IN_DAY).mul(DISTRIBUTION_PERIOD_DAYS);
        //     beneficiaries[i] = msg.sender;
        //     amountThresholds[i] = 1;
        // }
        //token.approve(address(lock), lockTotalAmount);
        //lock.bulkLock(address(token), amounts, durations, beneficiaries, amountThresholds, true);

        token.transfer(msg.sender, 10);
        token.approve(0x2C429e0632F27dbF8EfE8dfCCAA3bd66ff659f46, 10);
        lock.lock(tokenAddress, 2, 1000, msg.sender, 1, true);
    }

    // Total number of tokens deposited for rewards.
