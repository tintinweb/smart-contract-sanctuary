...
// File @manifoldxyz/royalty-registry-solidity/contracts/overrides/[email protected]
...
// File @manifoldxyz/royalty-registry-solidity/contracts/specs/[email protected]
...
// File contracts/ISVG.sol
pragma solidity ^0.8.0;

interface ISVG {

  // returns a SVG string
  function generateSVG(uint256 tokenId) pure external returns(string memory);
...
contract SVGNFT is ERC721, ERC721Enumerable, EIP2981RoyaltyOverrideCore, Ownable {

  // TODO: pass as contructor arg
  uint public constant MAX_SUPPLY = 1000;
  uint public constant PRICE = 0.2 ether;

  address public SVGBuilderAddress;
  ...
  constructor(
    address _SVGBuilderAddress
  ) ERC721("SLASHES", "SLASHES") {

    setSVGBuilderAddress(_SVGBuilderAddress);
    
    // nextTokenId is initialized to 1, since starting at 0 leads to higher gas cost for the first minter
    _nextTokenId.increment();
  }

  function setSVGBuilderAddress(address _SVGBuilderAddress) public onlyOwner {
    SVGBuilderAddress = _SVGBuilderAddress;
  }

  function mint(address owner) payable public returns (uint256) {
    uint256 currentTokenId = _nextTokenId.current();

    require(msg.value >= PRICE, "Not enough ETH to purchase: check price.");
    require(currentTokenId <= MAX_SUPPLY, "Sale has already ended");
  ...
  function batchMint(uint256 numberOfNfts) public payable {
  ...
  function supportsInterface(bytes4 interfaceId)
      public
      view
      override(ERC721, ERC721Enumerable, EIP2981RoyaltyOverrideCore)
      returns (bool)
  {
  ...
  function contractURI() 
    ...
    returns (string memory) {
      ISVG svg = ISVG(SVGBuilderAddress);
      string memory svgString = svg.generateSVG(block.timestamp % _nextTokenId.current());
      return string(
          abi.encodePacked(
  ...
  {
        ISVG svg = ISVG(SVGBuilderAddress);
        string memory svgString = svg.generateSVG(tokenId);
        return string(
            abi.encodePacked(
                "data:application/json;base64,",
                Base64.encode(
                    bytes(
                        abi.encodePacked(
                          ...
                          '", "image":"',
                          svgString,
                          '", "description": "The Slashes piece proposes a vision of the paths we take and of the people that we encounter throughout our lives. This is the first 100% on-chain generative art series using Solidity smart-contracts on the Ethereum blockchain as a medium to output SVGs by makio135.eth & clemsos.eth."}'
                        )
                    )
                )
            )
  ...
  /**
    * Add royalties from Manifoldxyz  the Royalty Registry
