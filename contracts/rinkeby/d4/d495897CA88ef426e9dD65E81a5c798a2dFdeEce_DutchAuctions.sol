...
contract DutchAuctions is OwnableClone, RevenueSplit {
  ...
  function calculateCurrentPrice(uint256 listingId) public view returns (uint256) {
    ...
    uint256 delta = listing.priceReductionFinnies * FINNY_TO_WEI;
    uint256 startPrice = listing.startPriceFinnies  * FINNY_TO_WEI;
    uint64 startTime = listing.openTime + (listing.startOffsetMin * 60);
    uint64 endTime = listing.openTime + (listing.endOffsetMin * 60);
  ...
  function bid(
    ...
    uint64 closeTime = listing.openTime + (listing.closeOffsetMin * 60);
    // solhint-disable-next-line not-rely-on-time
  ...
  function getListings(uint16 start, uint16 length) public view returns (OutputListing[] memory) {
    ...
    for (uint16 idx = 0; idx < actualLength; idx++) {
      ...
      uint256 reduction = listing.priceReductionFinnies * FINNY_TO_WEI;
      uint256 startPrice = listing.startPriceFinnies  * FINNY_TO_WEI;
      uint64 startTime = listing.openTime + (listing.startOffsetMin * 60);
      uint64 endTime = listing.openTime + (listing.endOffsetMin * 60);
      uint64 closeTime = listing.openTime + (listing.closeOffsetMin * 60);
