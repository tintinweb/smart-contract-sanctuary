...
contract MarcoNFT1155V2 is ERC1155 {
    ...
    function setSignerAddress(address newSigner) public onlyOwner returns (bool) {
        require (newSigner != address(0), "Signer: new signer is the zero address");
        signer = newSigner;
        emit SignerAddressUpdated(signer);
        return true;
    }
    ...
    */
    ...
    function mint(string memory uri, uint256 supply, uint256 fee, Sign memory sign) public payable {
        require(mintingFee == msg.value, "Mintingfee: mintingfee is not equal to value");
        verifySign(uri, sign);
