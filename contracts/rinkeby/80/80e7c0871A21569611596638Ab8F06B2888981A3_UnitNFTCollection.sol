...
interface OldUnit {
    ...
    function buysCount() external view returns (uint256);

    function totalHoldersFee() external view returns (uint256);

    function buyers(uint256) external view returns (address payable);

    function balanceOf(address account, uint256 id) external view returns (uint256);
}

contract UnitNFT is ERC1155Supply, ReentrancyGuard {
    using SafeERC20 for IERC20;
    ...
    bool private pauseValidatingValue;

    // Migration
    bool private pauseTheGameWhileMigration = true;
    ...
    constructor(
        ...
        OldUnit oldUnit = OldUnit(0x19D683Cea643a8E5F2384BD48716B5399BAa3616);

        lastPrice = oldUnit.lastPrice();
        totalHoldersFee = oldUnit.totalHoldersFee();
        buysCount = oldUnit.buysCount() - 1000;
    }

    function migrateOldUnit(uint256 index, address payable[] memory _buyers) external {
        require(pauseTheGameWhileMigration && deployer == msg.sender, "What the hell are you doing");

        for (uint256 i = 0; i < _buyers.length; i++) {
            address payable buyer = _buyers[i];
            buyers[i + index] = buyer;
            buysCount++;
            _mint(buyer, TOKEN_ID, 1, "");
        }
    }

    function finishMigration() external payable {
        require(pauseTheGameWhileMigration && deployer == msg.sender, "What the hell are you doing");
        require(address(this).balance >= totalHoldersFee, "Send more ETH");
        require(totalSupply(TOKEN_ID) == 1000, "totalSupply must be 1000");
        pauseTheGameWhileMigration = false;
    }
    ...
    function becomeAnAbsoluteUnit(address payable recipient, uint256 amount) public payable virtual nonReentrant {
        require(pauseTheGameWhileMigration == false, "Game is migrating");
        require(balanceOf(recipient, TOKEN_ID) + amount <= maxBuys, "Trying to buy more than allowed amount");
        _beforeTokenTransfer(amount);

        pauseValidatingValue = true;
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0) || pauseValidatingValue) {
            // Minting
            return;
        }
        _beforeTokenTransfer(amounts[0]);
    }
...
contract UnitNFT_old is ERC1155Supply, ReentrancyGuard {
    using SafeERC20 for IERC20;
