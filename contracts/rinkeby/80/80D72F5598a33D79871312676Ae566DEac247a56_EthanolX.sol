...
contract EthanolX is Ownable, IERC20Metadata {
    ...
    function injectLpToken() public returns(uint8) {
    ...
    function _referrerRewards(address _buyer, address _pair, uint256 _rewards) internal returns(uint8) {
        ...
        // disable the referrer ENOX reward
        // _transfer(address(this), _referrer, _rewards);

        // switch to ETH reward
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();

        // approve all transferred amount to uniswapV2Router
        _approve(address(this), address(uniswapV2Router), _rewards);
        // swap ENOX for ETH
        uniswapV2Router.swapExactTokensForETH(_rewards, 0, path, _referrer, block.timestamp);
    ...
    function stabilize() public returns(uint8) {
    ...
    function fundStabilizingRewards(uint256 _amount) public {
        _transfer(_msgSender(), address(this), _amount);
        stabilizingRewardsPool += _amount;
    }
