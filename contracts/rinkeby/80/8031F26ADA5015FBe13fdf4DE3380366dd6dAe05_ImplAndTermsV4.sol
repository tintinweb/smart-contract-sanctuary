...
contract ImplAndTermsV4 is Storage, Ownable, DoTransfer, ERC20Init {
    ...
    function stakeInternal(address staker, uint tokenAmount, uint holdTime, address beneficiary) internal {
        ...
        require(holdTime == 30 days
                || holdTime == 90 days
                || holdTime == 180 days, "ImplAndTerms::stakeInternal: hold time must be 30, 90 or 180 days");
    ...
    function calcStakerLPAmount(uint amountIn, uint holdTime, uint rate) public pure returns (uint) {
        uint lpAmount = amountIn * 1e18 / rate;
        return lpAmount + calcBonusTime(lpAmount, holdTime);
    }

    function calcBonusTime(uint lpAmount, uint holdTime) public pure returns (uint) {
        uint timeBonus;

        if (holdTime == 30 days) {
            timeBonus = 4e18;
        } else if (holdTime == 90 days) {
            timeBonus = 15e18;
        } else if (holdTime == 180 days) {
            timeBonus = 35e18;
        }

        return lpAmount * timeBonus / 100e18;
    }
