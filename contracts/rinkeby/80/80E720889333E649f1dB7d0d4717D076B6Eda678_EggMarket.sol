...
contract EggMarket is BaseAccessControl {
    ...
    uint private _maxPresaleCount;
    uint private _totalPresaleCount;
    uint private _price;
    ...
    event TokenPresold(address indexed buyer, address indexed to, uint count);
    event TokenBought(address indexed buyer, address indexed to, uint tokenId);
    ...
    function initialize(
        ...
        address eggToken, 
        uint maxPresaleCnt,
        uint presaleEggPrice,
        uint presaleMaxBuyCnt, 
        uint eggPrice, 
        uint maxBuyCnt) public initializer {
        __BaseAccessControl_init(accessControl);
        
        _tokenAddress = eggToken;
        _presalePrice = presaleEggPrice;
        _presaleMaxBuyCount = presaleMaxBuyCnt;
        _maxPresaleCount = maxPresaleCnt;
        _price = eggPrice;
        _maxBuyCount = maxBuyCnt;
    ...
    function setPresaleMaxBuyCount(uint newValue) external onlyRole(COO_ROLE) {
        _presaleMaxBuyCount = newValue;
    }

    function maxPresaleCount() public view returns(uint) {
        return _maxPresaleCount;
    }

    function setMaxPresaleCount(uint newValue) external onlyRole(COO_ROLE) {
        _maxPresaleCount = newValue;
    }

    function totalPresaleCount() public view returns(uint) {
        return _totalPresaleCount;
    }
    ...
    function setPrice(uint newValue) external onlyRole(CFO_ROLE) {
        _price = newValue;
    }
    ...
    function setMaxBuyCount(uint newValue) external onlyRole(COO_ROLE) {
    ...
    function togglePublicSaleStarted() external onlyRole(COO_ROLE) {
       _publicSaleStarted = true;
    }

    function claim(address to, uint count) external {
        require(publicSaleStarted(), "EggMarket: unable to claim");
        require(!AddressUpgradeable.isContract(to), "EggMarket: address cannot be contract");
        require(_presales[_msgSender()] >= count, "EggMarket: bad count");
        EggToken eggt = EggToken(tokenAddress());
        for (uint i = 0; i < count; i++) {
            uint tokenId = eggt.mint(to);
            emit TokenClaimed(_msgSender(), to, tokenId);
        }
        _presales[_msgSender()] -= count;
    }

    function buy(address to, uint count) external payable {
        require(!AddressUpgradeable.isContract(to), "EggMarket: address cannot be contract");
    ...
    function _buy(address to, uint count) internal {
        require(count <= maxBuyCount(), "EggMarket: bad count");
        require(msg.value >= count * price(), "EggMarket: incorrect amount sent to the contract");
        EggToken eggt = EggToken(tokenAddress());
        for (uint i = 0; i < count; i++) {
            uint tokenId = eggt.mint(to);
            emit TokenBought(_msgSender(), to, tokenId);
        }
    }

    function _presaleBuy(address to, uint count) internal {
        require(totalPresaleCount() + count <= maxPresaleCount(), "EggMarket: exceed max presale count");
        require(_presales[to] + count <= presaleMaxBuyCount(), "EggMarket: exceed max presale count per user");
        require(msg.value >= count * presalePrice(), "EggMarket: incorrect amount sent to the contract");
        _presales[to] += count;
        _totalPresaleCount += count;
        emit TokenPresold(_msgSender(), to, count);
    }
...
contract EggToken is ERC721Upgradeable, BaseAccessControl {
    ...
    mapping(DragonInfo.Types => uint) private _totalDragonSupply;

    uint internal _totalSupply;
    uint internal _hatchTime;
    address internal _dragonCreator;
    address internal _eggMarket;
    ...
    function initialize(
        uint totalEggSupply,
        uint totalLegendaryDragonSupply, 
        uint totalEpic20DragonSupply, 
        uint totalCommonDragonSupply, 
        uint htchTime, 
        string calldata defaultCid,
        address accessControl,
        address dragonCreator) public initializer {
        
        __ERC721_init("CryptoDragons Eggs", "CDE");
        __BaseAccessControl_init(accessControl);
        
        _totalSupply = totalEggSupply;

        _totalDragonSupply[DragonInfo.Types.Legendary] = totalLegendaryDragonSupply;
        _totalDragonSupply[DragonInfo.Types.Epic20] = totalEpic20DragonSupply;
        _totalDragonSupply[DragonInfo.Types.Common] = totalCommonDragonSupply;
        
        _hatchTime = htchTime; 
        _defaultMetadataCid = defaultCid;

        _dragonCreator = dragonCreator;
    ...
    function setEggMarketAddress(address newAddress) external onlyRole(CEO_ROLE) {
        _eggMarket = newAddress;
    }

    function canHatch(uint tokenId) public view returns(bool) {
    ...
    function hatch(uint tokenId) external returns (uint) {
        ...
        (DragonInfo.Types dragonType, uint genes) = randomGenes(info.mintedAt);
        _totalDragonSupply[dragonType]--;
    ...
    function randomGenes(uint salt) internal view returns (DragonInfo.Types, uint) {
        DragonInfo.Types t = randomDragonType(salt ^ block.difficulty ^ block.timestamp);
        
        uint genes = GenesLib.randomSetGenesToPositions(
            0, GenesLib.createOrderedRangeArray(COMMON_RANGE.from, COMMON_RANGE.to), 
            Random.rand(salt ^ block.number ^ block.difficulty), true);
        ...
        if (t == DragonInfo.Types.Epic20) {
            genes = GenesLib.randomSetGenesToPositions(
                ...
                Random.rand(block.difficulty ^ salt ^ block.timestamp), false);
        }
        else if (t == DragonInfo.Types.Legendary) {
            genes = GenesLib.randomSetGenesToPositions(
                genes, GenesLib.createOrderedRangeArray(RARE_RANGE.from, EPIC_RANGE.to), 
                Random.rand(salt ^ block.number ^ block.timestamp ^ block.difficulty), false);
        }

        return (t, genes);
    } 

    function randomDragonType(uint salt) internal view returns (DragonInfo.Types) {
        uint remainingLegendarySupply = _totalDragonSupply[DragonInfo.Types.Legendary];
        uint remainingEpic20Supply = _totalDragonSupply[DragonInfo.Types.Epic20];
        uint remainingCommonSupply = _totalDragonSupply[DragonInfo.Types.Common];
...
library EggInfo {
        ...
        return Details (
            {
                ...
                hatchedAt: uint256(uint64(value >> 64)),
                dragonId: uint256(uint32(value >> 128))
            }
        );
    }
    ...
    function getValue(Details memory details) internal pure returns (uint) {
        ...
        result |= uint(details.hatchedAt) << 64;
        result |= uint(details.dragonId) << 128;
        return result;
    }
}
...
contract DragonCreator is BaseAccessControl {
    ...
    function initialize(address accessControl, address tknContract) public initializer {
        __BaseAccessControl_init(accessControl);
