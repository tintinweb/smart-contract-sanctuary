...
contract CompoundLens {
    ...
    function getCompBalanceMetadata(AMPT ampt, address account) external view returns (CompBalanceMetadata memory) {
    ...
    function getCompBalanceMetadataExt(AMPT ampt, ComptrollerLensInterface comptroller, address account) external returns (CompBalanceMetadataExt memory) {
        uint balance = ampt.balanceOf(account);
        comptroller.claimAmpt(account);
    ...
    function getCompVotes(AMPT ampt, address account, uint32[] calldata blockNumbers) external view returns (CompVotes[] memory) {
