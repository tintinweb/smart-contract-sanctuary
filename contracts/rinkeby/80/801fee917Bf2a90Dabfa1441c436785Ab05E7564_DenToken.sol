...
contract DenToken is Ownable, ERC20Capped {
    ...
    function availableDenForWolf(uint16 _wolfId) public view returns(uint amountToClaim, uint denAvalibleToRelease) {
        ...
        if (amountMinted < 36_500e18) { // 36500 den  // if there are tokens avalible to mint for that specific wolf so:
            uint numberOfDays = uint((block.timestamp - _lastClaimTimestamp[_wolfId]) / 1 minutes); // *change to minutes for testing
            if (numberOfDays == block.timestamp / (1 minutes)) { // checks if nothing was claimed yet
                amountToClaim = 10e18; // 10 den
            } else {
                amountToClaim = numberOfDays * 10e18; // 10 den * days
                // assert(amountToClaim == numberOfDays * 10e18);
                ...
                // uint amount = numberOfDays * 10e18;
                // if (amount + amountMinted >= 36_500e18) {
                //     amountToClaim = 36_500e18 - amountMinted;
                // } else {
