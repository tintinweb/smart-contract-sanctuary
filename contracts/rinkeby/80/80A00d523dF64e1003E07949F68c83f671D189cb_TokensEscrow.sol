...
contract TokensEscrow is Ownable, ReentrancyGuard {
    ...
    event Finised(bytes32 indexed scriptHash, address[2] destinations, uint256[2] amounts);
    ...
    modifier checkAddresses(address address1, address address2) {
        require(address1 != address2, 'addresses are same');
        _;
    }
    ...
    constructor(address _FLyTokenAddress) {
    ...
    function addTokensDeal(
        ...
        checkAddresses(sellTokenAddress, buyTokenAddress)
        returns (bytes32 scriptHash)
    {
        ...
        sellToken.safeTransferFrom(msg.sender, address(this), valueSell);
    }
        ...
        allowedBuyValue(scriptHash, value)
    {
        ...
        emit Finised(scriptHash, [deal.seller, deal.buyer], [deal.valueSell, deal.valueBuy]);
    }

    function getCommision(bytes32 scriptHash) public view transactionExists(scriptHash) returns (uint256 commission) {
        Deal storage deal = deals[scriptHash];
        IERC20 sellToken = IERC20(deal.sellTokenAddress);
        commission = FullMath.mulDiv(deal.valueBuy, commisionInfo.beforeLimitPct, 100 * 10**commisionInfo.pctDigits);

        if (address(FLyToken) == address(sellToken) && deal.valueSell > commisionInfo.limitValue) {
