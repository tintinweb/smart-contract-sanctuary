    ...
    ReentrancyGuard
{
    ...
    function deposit(
        ...
        uint256 shares,
        bytes memory data
    ) external payable override onlyUnipilot returns (uint128 baseLiquidityAdded, uint128 rangeLiquidityAdded, uint256 mintedTokenId) {
        ...
        (baseLiquidityAdded, rangeLiquidityAdded) = _addLiquidityInManager(sender, pool, amount0Desired, amount1Desired, shares);
    ...
    function _increaseLiquidity(
        ...
        if (
            ...
            (baseLiquidityAdded, rangeLiquidityAdded) = _addRangeLiquidity(sender, pool, amount0Desired, amount1Desired, shares);
        } else {
            ...
            position.totalLiquidity += shares;
            (baseLiquidityAdded, rangeLiquidityAdded) = (a.baseLiquidity, a.rangeLiquidity);
        }
    }
    ...
    function _addLiquidityInManager(
        ...
        if (position.totalLiquidity > 0) {
            (baseLiquidityAdded, rangeLiquidityAdded) = _increaseLiquidity(sender, pool, amount0Desired, amount1Desired, shares);
        } else {
                    ...
                    if (tokenDetails.bidLiquidity > tokenDetails.rangeLiquidity) {
                        _addLiquidityUniswap(
                            AddLiquidityParams({
                                ...
                                tickLower: ticks.bidTickLower,
                                tickUpper: ticks.bidTickUpper,
                                amount0Desired: amount0Desired.sub(tokenDetails.amount0Added),
                                amount1Desired: amount1Desired.sub(tokenDetails.amount1Added)
                            })
                        );
                        ...
                        (baseLiquidityAdded, rangeLiquidityAdded) = (tokenDetails.baseLiquidity, tokenDetails.bidLiquidity);
                    } else {
                        _addLiquidityUniswap(
                            AddLiquidityParams({
                                sender: sender,
                        ...
                        (baseLiquidityAdded, rangeLiquidityAdded) = (tokenDetails.baseLiquidity, tokenDetails.rangeLiquidity);
                    }
                }
...
interface IUnipilot {
    ...
    function deposit(IHandler.DepositParams memory params, bytes memory data)
        external
        payable
        returns(uint128 baseLiquidity, uint128 rangeLiquidity, uint256 mintedTokenId);
}
...
interface IUniswapLiquidityManager is IULMEvents {
    ...
    function deposit(
        ...
        uint256 shares,
        bytes memory data
    ) external payable returns (uint128 baseLiquidity, uint128 rangeLiquidity, uint256 mintedTokenId);
...
interface IHandler {
    ...
    function deposit(
        ...
        uint256 shares,
        bytes calldata data
    ) external returns (uint128 baseLiquidity, uint128 rangeLiquidity, uint256 mintedTokenId);
