 ...
 */
contract Presale is Ownable, ITemplePool, Pausable {
    ...
    mapping(address => Allocation) public userAllocation;

    event MintComplete(address minter, uint256 daiAccepted, uint256 templeMinted);
    ...
    function increaseAddressAllocation(address staker, uint256 amount) external onlyOwner {
      userAllocation[staker].amount += amount;
    }

    function decreaseAddressAllocation(address staker, uint256 amount) external onlyOwner {
      userAllocation[staker].amount -= amount;
    }

    function setAddressAllocationEpoch(address staker, uint256 epoch) external onlyOwner {
      userAllocation[staker].epoch += epoch;
    }
    ...
    function mintAndStake(uint256 _amountPaid) external whenNotPaused {
      Allocation storage allocation = userAllocation[msg.sender];

      require(_amountPaid <= allocation.amount, "Amount requested exceed address allocation");
      require(allocation.epoch <= STAKING.currentEpoch(), "User's allocated epoch is in the future");

      (uint256 _dai, uint256 _temple) = TREASURY.intrinsicValueRatio();

      allocation.amount -= _amountPaid;
      uint256 _templeMinted = _amountPaid * _temple / _dai / mintMultiple;
      
      // pull dai from staker and immediately transfer back to treasury
      SafeERC20.safeTransferFrom(DAI, msg.sender, address(TREASURY), _amountPaid);

      // mint temple
      // TODO: Is it worth combining this by provviding staker and spender directly to the staking contract
      SafeERC20.safeTransferFrom(TEMPLE, address(TREASURY.MINT_ALLOWANCE()), address(this), _templeMinted);

      // Send minted temple to standard staking contract, with lock-in
      SafeERC20.safeIncreaseAllowance(TEMPLE, address(STAKING), _templeMinted);
      STAKING.stakeWithLockIn(msg.sender, _templeMinted, lockInPeriod);

      emit MintComplete(msg.sender, _amountPaid, _templeMinted);
    }
