    ...
    AccessControlEnumerable
{
    ...
    constructor(address seeder, address admin) ERC721("Fighter", "FIGHTER") {
    ...
    function safeMint(address to, uint256 count) external onlyRole(MINTER_ROLE) {
        uint256 tokenIdCounter = _tokenIdCounter;
        _tokenIdCounter += count;
        for (uint256 i = 0; i < count; i++) {
            _seeder.requestSeed(tokenIdCounter + i);
    ...
    function setSeeder(ISeeder seeder)
        external
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _seeder = seeder;
        emit SeederUpdated(msg.sender, address(seeder));
    }
    ...
    function burn(uint256 tokenId) public override(ERC721Burnable, IFighter) {
        _burnCounter += 1;
        super.burn(tokenId);
    }
    ...
    function burnBatch(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(_isApprovedOrOwner(_msgSender(), tokenIds[i]), "Fighter::burnBatch: caller is not owner nor approved");
            _burn(tokenIds[i]);
        }
    }
...
interface ISeeder {
    ...
    event Seeding(address indexed caller, uint256 indexed identifier);
