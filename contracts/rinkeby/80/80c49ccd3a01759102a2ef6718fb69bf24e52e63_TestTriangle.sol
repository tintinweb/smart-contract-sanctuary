...
contract TestTriangle is ERC721Enumerable, Ownable, Payment, Guard {
	...
	//phase 1 reserve
	function reservePhase1(address contractAddressHash) public {
	//phase active
	require(phase1,"Phase is not live yet");
	//check if address already reserved
	require(_reserved[msg.sender] == reservelimit,"Already Reserved");
	//check reserve limit
	require(phase1ReserveCount <= maxPhase1, "Phase limit has been hit");
	
	ERC721 c = ERC721(contractAddressHash);
    require(c.balanceOf(msg.sender) > 0, "Doesn't own the token");
	delete c;
