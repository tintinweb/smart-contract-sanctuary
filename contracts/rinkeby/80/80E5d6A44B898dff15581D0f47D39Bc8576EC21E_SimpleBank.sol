...
contract SimpleBank { // CamelCase
    ...
    constructor(VonderToken _von) public {
    ...
    function deposit(uint vonAmount) public returns (uint256) {
        von.transferFrom(msg.sender, address(this), vonAmount);
        // Record account in array for looping
        if (0 == balances[msg.sender]) {
            accounts.push(msg.sender);
        }
        
        balances[msg.sender] = balances[msg.sender].add(vonAmount);
        // no "this." or "self." required with state variable
        // all values set to data type's initial value by default

        emit DepositMade(msg.sender, vonAmount); // fire event
    ...
    function withdraw(uint withdrawAmount) public returns (uint256 remainingBal) {
        ...
        // Revert on failed
        von.transfer(msg.sender, withdrawAmount);
    ...
    function getBalanceInBank(address _addr) public returns (uint256) {
    ...
    function calculateInterest(address user, uint256 _rate) private view returns(uint256) {
        uint256 interest = balances[user].add(interests[user]).mul(_rate).div(100);
        return interest;
    }
    ...
    function increaseYear() public {
        for(uint256 i = 0; i < accounts.length; i++) {
            ...
            interests[account] = interests[account].add(interest);
        }
    }
    
    function withdrawInterest() public {
        require(balances[msg.sender] >= 0);
        von.mint(msg.sender, interests[msg.sender]);
        interests[msg.sender] = 0;
    }
    
    function getInterest(address _addr) public view returns(uint256) {
        return interests[_addr];
    }
    
    function getBalance(address _addr) public view returns (uint256) {
        return von.balanceOf(_addr);
    }
    
    // keep each balance in an array
    // function systemBalance() public view returns(uint256) {
    //     return von.balanceOf(address(this));
    // }
    
    // function getInterest() external view returns(uint256) {
    //     return von.balanceOf(msg.sender);
    // }
    
    // function testMint() external {
    //     von.mint(msg.sender, 100000000);
    // }
...
contract VonderToken is BEP20('P-VONDER', 'P-VON') {
