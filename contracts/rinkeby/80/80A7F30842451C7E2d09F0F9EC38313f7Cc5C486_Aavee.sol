...
contract Aavee is IExchange, AdapterStorage {
    ...
    function getKey() public override pure returns(bytes32) {
      return keccak256(abi.encodePacked("AaveV1", "1.0.0"));
    }
      ...
      private
    {
        ...
        Utils.approve(lData.spender, address(fromToken), fromAmount);
        ...
        if (address(fromToken) == address(data.aToken)) {
            ...
            if (address(fromToken) == Utils.ethAddress()) {
                IAaveLendingPool(exchange).deposit{value: fromAmount}(fromToken, fromAmount, lData.refCode);
            }
            else {
                IAaveLendingPool(exchange).deposit(fromToken, fromAmount, lData.refCode);
            }
        }
        else {
