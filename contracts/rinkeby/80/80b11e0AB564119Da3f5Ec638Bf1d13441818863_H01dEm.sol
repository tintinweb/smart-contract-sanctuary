...
contract H01dEm is ERC721Enumerable, Ownable {
    ...
    function devClaim() public onlyOwner returns (uint) {
        require(handList.length < 100, "Can only claim first 100");
        require(tx.origin == msg.sender, "Can not be called using a contract.");
    ...
    // remove on prod
    function showIndex(uint32 tokenId) public view returns (uint32) {
        return handList[tokenId];
    }

    // prod change to private
    function getHandSeed() public view returns (uint32) {
