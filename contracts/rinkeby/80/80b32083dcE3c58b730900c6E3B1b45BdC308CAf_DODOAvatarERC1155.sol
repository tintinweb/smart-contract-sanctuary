...
// File: contracts/DODOAvatarERC1155.sol


contract DODOAvatarERC1155 is ERC1155, InitializableOwnable {
    using Strings for uint256;

    string internal _baseUri = "";

    // ============ Event =============
    event ComponentMint(address creator, uint256 tokenId, uint256 amount);
    event ComponentBurn(address account, uint256 tokenId, uint256 amount);
    ...
    function mint(uint256 tokenId, uint256 amount) external onlyOwner {
        ...
        emit ComponentMint(msg.sender, tokenId, amount);
    }

    function burn(address account, uint256 tokenId, uint256 amount) external onlyOwner {
        _burn(account, tokenId, amount);
        emit ComponentBurn(account, tokenId, amount);
    }
