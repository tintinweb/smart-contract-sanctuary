...
interface IOREERC20 is IERC20Mintable, IERC20 {
...
contract Vault79Treasury is Ownable {
    ...
    IOREERC20 public immutable Ore;
    ...
    IERC20 public Ingot;
    uint public INGOTQueue; // Delays change to INGOT address
    
    uint public totalReserves; // Risk-free value of all assets
    uint public totalDebt;

    uint public defaultRate; // default rate of paxg vs others
    ...
    constructor (
        ...
        isReserveToken[ _PAXG ] = true;
        reserveTokens.push( _PAXG );
    ...
    /**
        @notice allow approved address to deposit an asset for Ore
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        if ( isReserveToken[ _token ] ) {
            ...
            if ( reserveTokens[ 0 ] != _token ) {
                require( allocatedRate() >= defaultRate, "Allocated rate exceeds" );
            }
        } else {
        ...
        // mint Ore needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        limitRequirements(msg.sender, send_);
        Ore.mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn Ore for reserves
        @param _amount uint
        @param _token address
     */
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = Ingot.balanceOf( msg.sender ); // Can only borrow against INGOT held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with Ore
        @param _amount uint
     */
    function repayDebtWithOre( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved as debtor" );
        require( isReserveSpender[ msg.sender ], "Not approved as spender" );

        Ore.burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, address(Ore), _amount, _amount );
    }
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( Ore.totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns Ore valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match Ore decimals
            value_ = _amount.mul( 10 ** Ore.decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyOwner returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.INGOT ) { // 9
            INGOTQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
    ...
    /**
        @notice takes allocated rate of paxg vs others
        @return uint
     */
    function allocatedRate() public view returns ( uint ) {
        uint reserves;
        for( uint i = 0; i < reserveTokens.length; i++ ) {
            reserves = reserves.add (
                valueOf( reserveTokens[ i ], IERC20( reserveTokens[ i ] ).balanceOf( address(this) ) )
        ...
        uint paxgReserve = valueOf( reserveTokens[ 0 ], IERC20( reserveTokens[ 0 ] ).balanceOf( address(this) ) );
        return reserves > 0 ? paxgReserve.mul( 10000 )/ reserves : 0;
    }

    /**
        @notice set default allocation rate of paxg vs others
     */
    function setDefaultRate( uint _defaultRate ) external onlyOwner {
        require( _defaultRate >= 100, "Rate too small" );
