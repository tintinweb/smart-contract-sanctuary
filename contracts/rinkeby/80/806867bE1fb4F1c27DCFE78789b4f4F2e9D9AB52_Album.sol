...
contract Album is Ownable, ERC721Holder, AlbumBuyoutManager {
    ...
    constructor(
        ...
        token = IERC20(_token);
        require(
            _nftAddrs.length == _nftIds.length,
        ...
        for (uint256 i = 0; i < nfts.length; i++) {
            require(
                IERC721(_nftAddrs[i]).ownerOf(_nftIds[i]) == address(this),
    ...
    function checkOwedAmount(uint256 _amount)
        internal
        override
...
abstract contract AlbumBuyoutManager {
    ...
    uint256 public buyoutCost;
    uint256 public buyoutEnd;
    bool public bought;
    ...
    function claim(uint256 _amount) public {
        ...
        // token.transferFrom(msg.sbuyoutEnder, address(this), _amount);
        // uint256 owed = (_amount / token.totalSupply()) * cost;
        uint256 owed = checkOwedAmount(_amount);
