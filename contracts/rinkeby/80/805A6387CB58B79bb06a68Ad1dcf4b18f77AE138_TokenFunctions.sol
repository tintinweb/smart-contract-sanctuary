...
contract Token is ERC721, Ownable, Pausable {
  ...
  mapping(uint256 => TokenStructs.Artwork) internal artworkOnXYZ;
  mapping(address => bool) internal whitelistedAddresses;

  function _onlyWhitelistedAddresses() internal {
    require(whitelistedAddresses[msg.sender] == true, "Not whitelisted");
  }

  function _artworkStatusNotOpenForBid(uint256 artworkID) internal {
    require(
  ...
  function _artworkStatusOpenForBid(uint256 artworkID) internal {
    require(
      artworkOnXYZ[artworkID].openForBid == true,
      "Dtatus is not for bid"
    );
  }

  function _artworkStatusOpenForSale(uint256 artworkID) internal {
    require(
      artworkOnXYZ[artworkID].artworkStatus ==
        TokenStructs.ArtworkStatus.OPENFORSALE,
      "Status is not for sale"
    );
  }

  function _artworkStatusNotForSale(uint256 artworkID) internal {
    require(
      artworkOnXYZ[artworkID].artworkStatus ==
        TokenStructs.ArtworkStatus.NOTFORSALE,
      "status is open for sale"
    );
  }

  function _notArtworkOwner(uint256 artworkID) internal {
    require(artworkOnXYZ[artworkID].owner != msg.sender, "You are the owner");
  }

  function _onlyArtworkOwner(uint256 artworkID) internal {
    require(
  ...
  function _artworkNotPausedForTrade(uint256 artworkID) internal {
    require(artworkOnXYZ[artworkID].paused == false, "Paused for trade");
  }

  function _artworkPausedForTrade(uint256 artworkID) internal {
  ...
  function _artworkPausedForTradeByContractOwner(uint256 artworkID) internal {
    require(
  ...
  event UpdatedArtworkStatusNotForSale(
    uint256 artworkID,
    TokenStructs.ArtworkStatus artworkStatus,
    address updatedBy
  );
  ...
  event UpdatedArtworkStatusToUnpause(
    uint256 artworkID,
    bool artworkStatus,
    address updatedBy
  );
  ...
  constructor() ERC721("Token", "TAST") {
    ...
    WhitelistAddressUpdated(msg.sender, true, msg.sender);
  }
  ...
  function addNewArtwork(
    ...
    artworkOnXYZ[newArtworkId].artworkStatus = TokenStructs
      .ArtworkStatus
      .NOTFORSALE;
    ...
    returns (bool)
  {
    ...
    artworkOnXYZ[artworkID].artworkStatus = TokenStructs
      .ArtworkStatus
      .NOTFORSALE;
  ...
  function enableAuctionOnItem(
    ...
    require(minimumValidBid < reservePrice, "Minimum Bid < reserve price");

    if (bidType == TokenStructs.BidTypes.TIMEDAUCTION) {
      require(
        expiryTimestamp > block.timestamp,
        "Expiry timestamp > current time"
      );
    } else if (bidType != TokenStructs.BidTypes.UNLIMITEDAUCTION) {
  ...
  function cancelAuctionOnItem(uint256 artworkID) external returns (bool) {
    ...
    if (
      artworkOnXYZ[artworkID]
        .openArtworkAuction[artworkOnXYZ[artworkID].openForBid]
        .bidType == TokenStructs.BidTypes.TIMEDAUCTION
    ) {
      require(
        artworkOnXYZ[artworkID]
          .openArtworkAuction[artworkOnXYZ[artworkID].openForBid]
      ...
      artworkOnXYZ[artworkID]
        .openArtworkAuction[artworkOnXYZ[artworkID].openForBid]
        .bidType == TokenStructs.BidTypes.UNLIMITEDAUCTION
    ) {
  ...
  function finalizeTimedAuction(uint256 artworkID) external returns (bool) {
    _onlyArtworkOwner(artworkID);
    _artworkStatusOpenForBid(artworkID);
    _artworkStatusNotForSale(artworkID);
    _artworkNotPausedForTrade(artworkID);
    ...
    if (
      ...
      artworkOnXYZ[artworkID]
        .openArtworkAuction[artworkOnXYZ[artworkID].openForBid]
        .bidType = TokenStructs.BidTypes.NONE;
      artworkOnXYZ[artworkID]
        .openArtworkAuction[artworkOnXYZ[artworkID].openForBid]
        .minimumValidBid = 0;
      artworkOnXYZ[artworkID]
    ...
    returns (bool)
  {
    ...
    require(
      artworkOnXYZ[artworkID]
        .openArtworkAuction[artworkOnXYZ[artworkID].openForBid]
        .bidType == TokenStructs.BidTypes.UNLIMITEDAUCTION,
      "not available on UnlimitedAuction"
    );
...
contract TokenFunctions is Token {
  ...
  function updateArtworkStatusToPausedByContractOwner(
    uint256 artworkID,
    bool status
