...
// File contracts/ArpeggiGenesis.sol
...
contract ArpeggiStudioGenesis is ERC721, Ownable {
    ...
    // uint256 public constant PASS_MINT_PRICE = 0.25 ether;
    uint256 public constant PASS_MINT_PRICE = 0.05 ether;

    address payable public labAddress;
    address public creatorsReserve;
    address public developersReserve;


    constructor(address _creatorsReserve, address _developersReserve) ERC721("Arpeggi Studio Pass Genesis", "ARPEGGILABS") {
        labAddress = payable(_msgSender());
        creatorsReserve = _creatorsReserve;
        developersReserve = _developersReserve;
    }
     ...
     */
    mapping (uint256 => string) public samples;
    uint256 public samplesCount = 0;
    ...
    /**
     * @dev Triggers paused state. The contract must not be paused.
     */
     ...
     */
    uint256 public constant CREATORS_RESERVE = 64;

    /**
     * @dev Allocate passes to Arpeggi Creators Reserve wallet
     */
    function mintCreatorsReserve() external onlyOwner whenPaused {
        for (uint i=0; i < CREATORS_RESERVE; i++){
            _mintToken(creatorsReserve);
        }
    }
     ...
     * @dev Max number of Passes that the contract owner can mint.
     */
    uint256 public constant DEVELOPERS_RESERVE = 24;

    /**
     * @dev Allocate passes to Arpeggi Creators Reserve wallet
     */
    function mintDevelopersReserve() external onlyOwner whenPaused {
        for (uint i=0; i < DEVELOPERS_RESERVE; i++){
            _mintToken(developersReserve);
        }
    }
    ...
    function mint() external payable whenNotPaused {
        ...
        require(numMinted <= MAX_PASSES, "No More Passes Left");
        require(PASS_MINT_PRICE <= msg.value, "Incorrect Ether value.");
        ...
        // only one mint allowed per early access address
        if (onlyEarlyAccess == true) {
    ...
    function pressSong(uint256 _tokenId, bytes calldata _composition, string calldata _title, string calldata _artist) external whenNotPaused {
        ...
        // do we need to check the format or contents of _composition here?

        _pressSongToken(msg.sender, _tokenId, _title, _artist, _composition);
    }

    function _pressSongToken(address _artistAddress, uint256 _tokenId, string calldata _title, string calldata _artist, bytes calldata _composition) internal returns(uint256 _songToken){
        Passes[_tokenId].title = _title;
        Passes[_tokenId].artist = _artist;
        Passes[_tokenId].composition = _composition;
        Passes[_tokenId].mintTime = block.timestamp;
        Passes[_tokenId].artistAddress = _artistAddress;
        return _tokenId;
    }

    function _splitFunds() internal {
        labAddress.transfer(msg.value);
    }
     ...
     * @dev Set the base URL for external_url metadata field.
     */
    function setBaseSongExternalUrl(string memory url) public onlyOwner {
