...
contract HeroesCommon is ERC721Enumerable, Ownable {
    ...
    uint256 public stopMintAfter = 1000;
    ...
    constructor(string memory name_,
        string memory symbol_, uint256 _amount, bool _mintMagicBox) ERC721(name_, symbol_)  {
        //Let's mint MagicBox
        if (_mintMagicBox) {
    ...
    function _multiMint(address to, uint256 amount, uint8 channel) internal returns (uint256) {
        require(stopMintAfter > totalSupply(), "Minting is paused");
        require((totalSupply() + reservedForPartners + 1) < MAX_TOTAL_SUPPLY, "No more common heroes");
        uint256 counter;
