...
contract SummaSwapV3Pool is ISummaSwapV3Pool, NoDelegateCall {
        ...
        returns (int256 amount0, int256 amount1)
    {
        ...
        if (zeroForOne) {
            ...
            if (state.fa > 0) sendToken0(state.fa, data);
        } else {
            feeGrowthGlobal1X128 = state.f;
            if (state.p > 0) protocolFees.token1 += state.p;
            if (state.fa > 0) sendToken1(state.fa, data);
        }
    ...
    function sendToken0(uint128 amount,bytes calldata data) private {
        TransferHelper.safeTransfer(
            token0,
            ITradeMint(ISummaSwapV3Factory(factory).tradeMintAddress())
                .getRelation(data),
    ...
    function sendToken1(uint128 amount, bytes calldata data) private {
        TransferHelper.safeTransfer(
            token1,
            ITradeMint(ISummaSwapV3Factory(factory).tradeMintAddress())
                .getRelation(data),
...
interface ITradeMint{
    ...
    function getFee(bytes calldata data,uint24 fee) external view returns (uint24);
    
    function getRelation(bytes calldata data) external view returns (address);
