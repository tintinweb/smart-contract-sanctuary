...
contract randType {
    ...
    function _randType() external returns (uint40[] memory) {
        ...
        uint256 sumWeight = 0;
        for (uint32 i = 0; i < availableWeight.length; i++) {
            if (availableType[i]) {
                availableWeight[availableWeightIndex] = typeWeight[i];
                availableWeightIndex.add(1);
                sumWeight.add(typeWeight[i]);
            }
        }
        ...
        for (uint32 i = 0; i < availableWeightIndex.sub(1); i++) {
            probStep = uint40(maxUInt32.mulDiv(uint256(availableWeight[i]), uint256(sumWeight)));
            if (i == 0) {
                probRange[i] = probStep;
            }
            else {
                probRange[i] = probStep + probRange[i.sub(1)];
            }
        }
        probRange[availableWeightIndex.sub(1)] = uint40(maxUInt32);
        ...
        for (uint32 i = 0; i < availableType.length; i++) {
            if (!availableType[i]) {
                ...
                // Not neccesary
                // if (probRangeIndex == len(probRange).sub(1)) {
                //     break;
                // }
            }
        }

        resindex = probRangeIndex;
        return probRange;
    }
}