...
contract Clownz is ERC721, Ownable {
    ...
    uint public constant MAX_PURCHASE_PRESALE = 8;
    uint public constant WHITELIST_SIZE = 2;
    uint public constant MAX_SUPPLY_PRESALE = 100;
    ...
    uint public currentWhitelistSize = 0;
    ...
    function addToWhitelist(address[] memory addresses) public onlyOwner {
        for (uint i = 0; i < addresses.length; i++) {
          ...
          require(WHITELIST_SIZE >= currentWhitelistSize, "The whitelist is full");
        
          _whitelist[addresses[i]] = true;
          _whitelistMinted[addresses[i]] > 0 ? _whitelistMinted[addresses[i]] : 0;
          currentWhitelistSize = currentWhitelistSize.add(1);
        }
    }
    ...
    function removeFromWhitelist(address[] memory addresses) public onlyOwner {
        for (uint i = 0; i < addresses.length; i++) {
            ...
            currentWhitelistSize = currentWhitelistSize.sub(1);
        }
    }
    ...
    */
    function mintClownPresale(uint numberOfTokens) public payable returns (uint[] memory) {
        ...
        require(totalSupply().add(numberOfTokens) <= MAX_SUPPLY_PRESALE, "Purchase would exceed the presale supply of Clownz");
        ...
        require(_whitelistMinted[msg.sender].add(numberOfTokens) <= MAX_PURCHASE_PRESALE, "Can only mint 10 tokens per address in presale");
        uint[] memory result = new uint[](numberOfTokens);
