...
contract Polkadog is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    mapping (address => mapping(uint256 => uint256)) private _transactionTime;
    ...
    function _transfer(
            ...
            if(amount >= transactionLimit){
                ...
                if(_totalTransactions[from] > 0){
                    ...
                    for(uint256 i =_totalTransactions[from]; i>=0 ; i--){
                        ...
                        uint256 pastTime = _transactionTime[from][i];
        ...
        if(to == uniswapV2Pair) {
            ...
            _transactionTime[from][totalTransactions] = block.timestamp;
            uint256 transactionTime = _transactionTime[from][totalTransactions];
    ...
    function checkTimeBound(address from, uint256 currentTime) internal {
        if(_totalTransactions[from] > 0){          
            if((currentTime - _transactionTime[from][_totalTransactions[from]]) > _perDayTimeBound ){
                _totalTransactions[from] = 0;
            }
        }
    }
    ...
    function getTime(address from, uint256 index) public view returns(uint256) {
        return _transactionTime[from][index];
    }

    function getAmount(address from, uint256 index) public view returns(uint256) {
        return _transactionAmount[from][index];
    }
