...
contract PaymentSplitter is Context {
     ...
     * @param payees The address of the payee to remove.
     * @param index The index of payees array to remove.
     */
    function _deletePayee(address payees, uint256 index) virtual public {
        require(index > 0, "PaymentSplitter: index > 0");
        require(_payees[index] == payees, "PaymentSplitter: account and address must match");
        require(_shares[payees] > 0, "PaymentSplitter: account needs shares");
        
        delete _payees[index];
        _totalShares = _totalShares - _shares[payees];
        delete _shares[payees];
    }
...
contract STONERNFT is Context, AccessControlEnumerable, ERC721Enumerable, PaymentSplitter, Ownable, Pausable {
    ...
    constructor(address admin, address admin2)
        ...
        ERC721("StonerPunks.com", "STONER")
    {
	    _api_entry = "https://api.stonerpunks.com/meta/";

        _setupRole(DEFAULT_ADMIN_ROLE, admin);
        _setupRole(DEFAULT_ADMIN_ROLE, admin2);
        
        setItemPrice(500000000000000000);
    }
    
    function contractURI() public pure returns (string memory) {
		return "https://api.stonerpunks.com/contract/";
	}
    ...
    function _deletePayee(address payees, uint256 index) public override onlyOwner() {
        require(index > 0, "PaymentSplitter: index > 0");
        
        super._deletePayee(payees, index);
    }
    ...
    function transferBNB(address payable recipient, uint256 amount) external onlyOwner() {
        require(amount <= 1000000000000000000, "STONERNFT:: 1 BNB Max");
