...
contract Token is ERC721, Ownable, Pausable {
  ...
  event UpdatedArtworkStatusOfSale(
    uint256 artworkID,
  ...
  function updateArtworkStatusOfSale(
    uint256 artworkID,
    TokenStructs.ArtworkStatus artworkStatus
    ...
    require(
      artworkOnXYZ[artworkID].artworkStatus != artworkStatus,
      "Artwork with same status"
    );

    artworkOnXYZ[artworkID].artworkStatus = artworkStatus;

    emit UpdatedArtworkStatusOfSale(
      artworkID,
      artworkOnXYZ[artworkID].artworkStatus,
      msg.sender
    );
  ...
  function claimItemAfterAuction(uint256 artworkID) external returns (bool) {
    _artworkStatusOpenForBid(artworkID);
    _artworkStatusNotForSale(artworkID);
    _artworkNotPausedForTrade(artworkID);
    _artworkNotPausedForTradeByContractOwner(artworkID);
