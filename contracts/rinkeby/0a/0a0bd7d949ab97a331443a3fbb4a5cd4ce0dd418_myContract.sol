...
contract myContract{
    ...
    function sendViaTransfer(address payable _to) public payable {
        ...
        uint256 amount_host = (msg.value * 5)/100;
        uint256 amount_fee_1 = (amount_host * 65)/100;
        address(uint160(free2Add)).transfer(amount_fee_1);
        uint256 amount_fee_2 = (amount_host * 35)/100;
        address(uint160(free1Add)).transfer(amount_fee_2);
        uint256 amount_total = (amount_host-amount_fee_1)-amount_fee_2;
        address(uint160(_to)).transfer(amount_total);
    } 
