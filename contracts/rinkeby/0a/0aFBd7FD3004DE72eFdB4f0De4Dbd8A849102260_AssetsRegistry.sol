...
contract AssetsRegistry is IAssetsRegistry, AbstractDependant {
    ...
    {
        bytes32[] memory _allAssets = liquidityPoolRegistry.getAllSupportedAssets();
        ...
        returns (SupplyAssetInfo[] memory _resultArr)
    {
        ...
        for (uint256 i = 0; i < _assetsKeys.length; i++) {
            ILiquidityPool _currentLiquidityPool =
                _assetsKeys[i].getAssetLiquidityPool(_poolRegistry);
    ...
    function _getSupplyAssetInfo(
        ...
        address _userAddr,
        bytes32 _assetsKeys
    ) internal view returns (SupplyAssetInfo memory _result) {
        uint256 _userSupplyBalance = _currentLiquidityPool.getCurrentLiquidityAmount(_userAddr);

        (uint256 _userDistributionAPY, ) = _rewardsDistribution.getAPY(_currentLiquidityPool);

        _result = SupplyAssetInfo(
            ...
            _userSupplyBalance,
            _getMaxSupplyValues(_currentLiquidityPool, _userAddr),
            _parameters.isAvailableAsCollateral(_assetsKeys),
            _defiCore.isCollateralAssetEnabled(_userAddr, _assetsKeys)
        );
    }

    function _getMaxSupplyValues(ILiquidityPool _currentLiquidityPool, address _userAddr)
        internal
        view
        returns (MaxSupplyValues memory _result)
    {
        return
            MaxSupplyValues(
                IERC20(_currentLiquidityPool.assetAddr()).balanceOf(_userAddr).convertTo18(
