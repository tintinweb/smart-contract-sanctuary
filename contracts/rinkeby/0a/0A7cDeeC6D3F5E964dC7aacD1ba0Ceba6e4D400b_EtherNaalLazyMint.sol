...
contract EtherNaalLazyMint is ERC721URIStorage, EIP712{
    ...
    {
        owner = msg.sender;
        //Royalty Fee is fixed to be 2.5% of sales, company fee and creator fee are 30% and 70% of the royalty fee
    ...
    function getSellerPercentage() public view returns (uint256) {
        //Returning % multiplied by 100 to keep it uniform across contract
        uint256 sellerPercentage = 10000 - royalty;
        return sellerPercentage;
    }

    function setRoyaltyPercentage(uint256 _royalty, uint256 _cfee, uint _creatorFee) public onlyOwner returns (bool) {
    ...
    function getCreatorRoyalty() public view returns(uint256) {
        return creatorFee;
    }
...
/// @notice Returns a hash of the given NFTVoucher, prepared using EIP712 typed data hashing rules.
  ...
  function _hash(NFTVoucher calldata voucher) internal view returns (bytes32) {
    return _hashTypedDataV4(keccak256(abi.encode(
      keccak256("NFTVoucher(uint256 tokenId,uint256 minPrice,string uri, bool valid)"),
      voucher.tokenId,
      voucher.minPrice,
  ...
  function _verify(NFTVoucher calldata voucher) internal view returns (address) {
    require(voucher.valid == true, "Voucher is no more valid and has already been claimed");
    bytes32 digest = _hash(voucher);
    return ECDSA.recover(digest, voucher.signature);
  }

    function buyTokenOnSale(uint256 tokenId, address _nftAddress, NFTVoucher calldata voucher)
        public
        payable
    {   
        address seller = _verify(voucher);

        require(creatorWhitelist[seller] == true, "Signer/Seller should be whitelisted");
        ...
        _mint(seller, voucher.tokenId);
        _setTokenURI(voucher.tokenId, voucher.uri);
        
        salePrice[tokenId] = 0;

        nftAddress.transferFrom(seller, msg.sender, tokenId);

        uint256 sellerPercentage = getSellerPercentage();
        uint256 creatorRoyalty = creatorFee;
        uint256 platformFees = companyFee;
        
        //Dividing by 100*100 as all values are multiplied by 100
        uint256 toSeller = (msg.value * sellerPercentage * 1) / 10000;
        
        //Dividing by 100*100*100 as all values are multiplied by 100
        uint256 toCreator = (msg.value*creatorRoyalty*royalty*1) / 1000000;
        uint256 toPlatform = (msg.value*platformFees*royalty*1) / 1000000;
