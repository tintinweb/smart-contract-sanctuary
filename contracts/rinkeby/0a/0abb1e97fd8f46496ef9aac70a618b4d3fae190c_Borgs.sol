...
contract CommonObjects{
    ...
    struct BorgAttribute{
        ...
        bytes8[] colors;
        int8 colorSize;
        mapping (bytes8 => uint32[]) colorPositions;
    }
    ...
    struct Layer{
        ...
        uint32 layerItemSize;
        uint32 layerItemsCumulativeTotal;
    }
    
    struct LayerItem{
        bool exists;
        uint32 chance;
        bytes32 borgAttributeName;
    }
    ...
    struct Borg{
        ...
        bytes32[] borgAttributeNames;
        uint32[] borgAttributeLayerPositions;
...
contract Borgs is ERC721Enumerable, ERC721Holder, CommonObjects, NumberUtils{
    ...
    mapping (bytes32 => BorgAttribute) public _borgAttributes;
    
    // Mapped layers (name/position:layer data)
    mapping (bytes32 => Layer) public _layers;
    
    // Mapped borg (id:borg)
    mapping (uint256 => Borg) private _borgs;
    
    // Mapped attribute usage (attribute name:usage)
    mapping (bytes32 => uint256) private _borgAttributesUsed;
    ...
    function getBorg(uint256 borgId) public view returns(string memory name, bytes8[] memory image, bytes32[] memory attributes, uint256 parentId1, uint256 parentId2, uint256 childId){
    ...
    function getAttributesUsedCount(bytes32[] memory attributes) public view returns(uint256[] memory){
    ...
    function getBorgAttribute(bytes32 borgAttributeName) public view onlyOwner returns (bytes8[] memory imagePixals){
        ...
        for(uint256 i = 0;i<borgAttribute.colors.length;i++){
            ...
            uint32[] memory positions = borgAttribute.colorPositions[color];
    ...
    function addLayers(bytes32[] calldata layerNames) external onlyOwner editable{
    ...
    function addLayerItems(CreateLayerItems[] calldata createLayerItems) external onlyOwner editable{
        ...
        for(uint256 i =0;i<createLayerItems.length;i++){
            // Get the layer
            Layer storage layer = _layers[createLayerItems[i].layerName];
            ...
            uint32 position = layer.layerItemSize;
            ...
            layer.layerItemsCumulativeTotal = (layer.layerItemsCumulativeTotal + createLayerItems[i].chance);
        }
    }
    
    function createBorgAttribute(bytes32 borgAttributeName, bytes8[] calldata colors, uint32[][] memory positions) external onlyOwner editable{
        // Reauire positions have been provided for the colours
        require(colors.length == positions.length, 'Color/position length must be the same');
        ...
        for(uint256 i =0;i<colors.length;i++){
            ...
            // Add the color
            borgAttribute.colors.push(colors[i]);
            borgAttribute.colorSize = borgAttribute.colorSize + 1;
            borgAttribute.colorPositions[colors[i]] = positions[i];
            borgAttribute.exists = true;   
        }
    }
    
    function nameBorg(uint256 borgId, string calldata newName) public {
    ...
    function generateBorg() external payable usable returns(uint256){
        ...
        bytes32[] memory borgAttributeNames = new bytes32[](_layerMetaData.length);
        uint32[] memory layerItemPositions = new uint32[](_layerMetaData.length);
    ...
    function breedBorgs(uint256 borgId1, uint256 borgId2) external usable returns(uint256){
        ...
        uint32[] memory layerItemPosition1 = borg1.borgAttributeLayerPositions;
        uint32[] memory layerItemPosition2 = borg2.borgAttributeLayerPositions;
        
        // Filter out the rareset of the 2 sets of items (layer by layer)
        (uint32[] memory borgAttributeLayerPositions, bytes32[] memory borgAttributeNames) = _filterRarestBorgPeices(layerItemPosition1, layerItemPosition2);
    ...
    function previewBreedBorgs(uint256 borgId1, uint256 borgId2) external view usable returns(bytes8[] memory image, bytes32[] memory attributes){
        // Get the first borg to breed
        Borg storage borg1 = _borgs[borgId1];
        require(borg1.exists, 'Borg 1 doesnt exist');
        require(borg1.descendantId < 1, 'Borg1 already has a descendant');
        ...
        // Filter out the rareset of the 2 sets of items (layer by layer)
        (uint32[] memory borgAttributeLayerPositions, bytes32[] memory borgAttributeNames) = _filterRarestBorgPeicesForPreview(layerItemPosition1, layerItemPosition2);
    ...
    function _filterRarestBorgPeices(uint32[] memory layerItemPosition1, uint32[] memory layerItemPosition2) internal returns(uint32[] memory rarestLayerPositions, bytes32[] memory borgPeiceNames){
        ...
        rarestLayerPositions = new uint32[](layerItemPosition1.length);
        bytes32[] memory rarestBorgAttributeNames = new bytes32[](layerItemPosition1.length);
    ...
    function _filterRarestBorgPeicesForPreview(uint32[] memory layerItemPosition1, uint32[] memory layerItemPosition2) internal view returns(uint32[] memory rarestLayerPositions, bytes32[] memory borgPeiceNames){
        // Create the new item arrays
        rarestLayerPositions = new uint32[](layerItemPosition1.length);
        bytes32[] memory rarestBorgAttributeNames = new bytes32[](layerItemPosition1.length);
    ...
    function _updateBorgAttributesUsed(bytes32 borgAttributeName) internal{
    ...
    function _combineBorgAttributes(bytes32[] memory borgAttributeNames) internal view returns (bytes8[] memory hexPixals){
        // Init the 
    ...
    function _getRandomLayerItemName(bytes32 layerItemName) internal returns(bytes32 borgAttributeName, uint32 position){
        ...
        for(uint32 i=0;i<layer.layerItemSize;i++){
            // Get the layer item
    ...
    function _createBorg(bytes32[] memory borgAttributeNames, uint32[] memory layerItemPositions, uint256 parentId1, uint256 parentId2) internal returns(uint256){
        // Get a new id
