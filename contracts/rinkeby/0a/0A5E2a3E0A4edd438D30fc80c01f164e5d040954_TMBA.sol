...
contract TMBA is ERC721, Ownable {
    ...
    uint256 public constant NUM_TEAM_NFTS = 20;
    ...
    constructor(string memory uri) ERC721("The Token MBA", "T-MBA") {
        baseURI = uri;

        for (uint i = 0; i < NUM_TEAM_NFTS; i++) {
    ...
    function setYearbook(uint256 tokenId, string memory text) external {
        require(ERC721.ownerOf(tokenId) == msg.sender, "T-MBA: sender is not owner of token");
        _yearbook[tokenId] = text;
        emit YearbookUpdated(msg.sender, tokenId);
    }

    function mint(uint256 count) public payable returns (bool) {
        require(count > 0 && count <= 5, "T-MBA: Cannot mint more than 5 tokens");
        require(_tokenIds.current() + count <= MAX_NFTS, "T-MBA: Cannot mint more than max supply");
        require(msg.value == PRICE_PER_NFT * count , "T-MBA: Ether value sent is not correct");
