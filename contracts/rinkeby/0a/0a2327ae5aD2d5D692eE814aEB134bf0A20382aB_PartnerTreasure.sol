...
contract PartnerTreasure is BaseTreasure {
    ...
    function _isUnlockTimeValid(uint256 _times) internal view returns (bool) {
        // check timestamp
        return _getBlockTimestamp() >= initialTimestamp + _times * 365 * 1 seconds;
    }
}
...
abstract contract BaseTreasure is AdminAccessControl {
    ...
    function batchTransferToken(
    ...
    ) external onlyRole(ADMIN_ROLE) returns (bool[] memory) {
        require(
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            results[i] = myteContract.transfer(recipients[i], amounts[i]);
        }
