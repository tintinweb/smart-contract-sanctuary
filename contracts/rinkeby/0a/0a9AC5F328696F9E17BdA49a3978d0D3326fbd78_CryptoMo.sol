...
// Copyright transfers to the NFT Holder
...
contract CryptoMo is Ownable, ERC721Enumerable, PaymentSplitter {
    ...
    // Crypto Mo Release Time- THIS NEEDS TO BE CHANGED BEFORE DEPLOYMENT
    uint public publicSale = 1641784500;  // Secret Whitelist ??? Use airdrop function for those who already minted a Mo??
        ...
        PaymentSplitter(distro, distro_shares)
    {
        ...
        // teamclaimNFT
        accounts[teamclaim[0]] = Account( 100, 0, true); //
        accounts[teamclaim[1]] = Account( 100, 0, true); //
        accounts[teamclaim[2]] = Account( 100, 0, true); //
    ...
    function increaseReserved(uint _increaseReservedBy, address _addr) external onlyOwner {
        require(_reserved + totalSupply() + _increaseReservedBy <= MAX_MO, "This would exceed the max supply");
    ...
    function claimReserved(uint _amount) external onlyAdmin {
        ...
        require(totalSupply() + _amount <= MAX_MO, "You would exceed the max supply limit");
    ...
    function airDropNFT(address[] memory _addr) external onlyOwner {
        require(totalSupply() + _addr.length <= (MAX_MO - _reserved), "You would exceed the airdrop limit");
    ...
    function mint(uint _amount) external payable noReentrant {
        ...
        require(_isSaleLive || _isPreSaleLive, "Sale must be active to mint");
        require(totalSupply() + _amount <= (MAX_MO - _reserved), "Purchase would exceed max supply of MO");
        require((_amount + accounts[msg.sender].mintedNFTs) <= walletLimit, "Sorry you can only mint 50 per wallet");
        require(!isContract(msg.sender), "Contracts can't mint");



        if(_isPreSaleLive) {
            require(msg.value >= (PRESALE_MO_PRICE * _amount), "Sorry that is not enough ether, the price is 0.05 ETH per NFT");
        } else if (_isSaleLive) {
            require(msg.value >= (PUBLIC_MO_PRICE * _amount), "Sorry that is not enough ether, the price is 0.07 ETH per NFT");
            require(block.timestamp >= publicSale, "Public sale has not started yet");
        }
    ...
    function burn(uint _id) external returns (bool, uint) {
        require(msg.sender == ownerOf(_id) || msg.sender == getApproved(_id) || isApprovedForAll(ownerOf(_id), msg.sender), "You must own this token before you can burn");
        _burn(_id);
