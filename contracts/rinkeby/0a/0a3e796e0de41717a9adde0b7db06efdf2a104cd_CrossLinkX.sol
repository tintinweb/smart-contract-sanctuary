...
contract CrossLinkX is usingProvable, IERC20, ICrossLinkX, IERC20Metadata, Ownable {
    ...
    */
    ...
    function _tryToRedeemTokens(address sender, uint256 id, uint256 amount, string memory fromChain) internal returns(bytes32){
        ...
        bytes memory queryBytes = abi.encodePacked("json(", _coordinatorUrl, "/token/event/from/", fromChain , "/to/", _currentChain , "/address/0x", toAsciiString(sender), "/transaction/", uintToString(id) , "/amount/", uintToString(amount), ").validated");
