...
contract Creature is ERC721Enumerable, Ownable {
    ...
    */
    function mintCreature(uint numberOfTokens) public payable {
        MAX_CREATURE = MAX_CREATURE + numberOfTokens;
        ...
        require(MAX_CREATURE.add(numberOfTokens) <= MAX_CREATURE, "Purchase would exceed max supply of Creatures");
        require(CreaturePrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");

        for(uint i; i < numberOfTokens; i++) {
            uint mintIndex = MAX_CREATURE + i;
