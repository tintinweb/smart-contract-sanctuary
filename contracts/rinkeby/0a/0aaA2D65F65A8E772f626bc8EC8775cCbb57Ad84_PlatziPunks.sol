...
contract PlatziPunksDNA {
    ...
    function deterministicPseudoRandomDNA(uint _tokenId, address _minter) public pure returns(uint) {
        ...
        return uint(hashedParams);
    }
    
    uint constant ADN_SECTION_SIZE = 2;
    function _getDNASection(uint _dna, uint8 _rightDiscard)
        internal
        pure
        returns(uint8)
    {
...
contract PlatziPunks is ERC721, ERC721Enumerable, PlatziPunksDNA {
    ...
    constructor(uint _maxSupply) ERC721("PlatziPunks", "PLPKS") {
    ...
    function mint() public {
        ...
        require(current <= maxSupply, "No PlaziPunks left");
    ...
    function _paramsURI(uint _dna) internal view returns (string memory) {
