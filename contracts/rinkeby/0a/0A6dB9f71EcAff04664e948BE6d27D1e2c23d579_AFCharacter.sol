...
contract AFCharacter is AFRoles, IAFCharacter {
    ...
    function takeRandomCharacter(bytes32 randomHex) external onlyContract returns (uint256) {
        ...
        uint256 randomCharacterArrayIndex = uint256(randomHex) % arrayCount;
...
interface IAFCharacter {
    function takeRandomCharacter(bytes32 randomHex) external returns (uint256);
