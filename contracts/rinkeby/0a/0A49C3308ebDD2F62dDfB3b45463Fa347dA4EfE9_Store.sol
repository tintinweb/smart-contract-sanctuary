...
contract Store {
  ...
  function unboost(uint32 oid, uint256 amount) public payable requireOwner(oid) {
    //limit withdrawal to the amount boosted
    if (amount > orders[oid].boostAmount) {
      amount = orders[oid].boostAmount;
    }
    if (amount > 0) {
  ...
  //return (TODO: sorted by boost amount) queue of pending orders,
  ...
  //deposits ether in case of screw up to allow refunding of
  // those in queue
