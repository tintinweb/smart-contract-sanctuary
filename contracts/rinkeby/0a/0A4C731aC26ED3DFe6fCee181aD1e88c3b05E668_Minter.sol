...
contract Minter is AccessControlEnumerableUpgradeable, Provenance, PaymentSplitterUpgradeable {
    ...
    address public server;             // game server wallet that sends pets
    ...
    function setServerSigner(address serverAdd) public onlyAdmin {
        server = serverAdd;
    }
    ...
    //below likely will be used in building sales for land owners on the website
    ...
    //to show people how many they can mint. likely land owners minting buildings
    ...
    function mintServer(address player) public onlyAdmin {
      require(msg.sender == server, "only the game server can call this");
      IBaseToken(tokenContract).mint(player);
    }
