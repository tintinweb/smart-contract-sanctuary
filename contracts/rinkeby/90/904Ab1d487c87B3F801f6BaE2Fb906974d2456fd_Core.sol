...
contract Core is OwnableUpgradeable, ICore {
    ...
    uint256 public conditionalReinforcementFix; // should be 20k
    uint256 public conditionalMarginality;
    ...
    uint256 public lastBetNumber; //start from 1

    // totally locked prizes value - sum of maximum payouts of all execution Condition.
    ...
    uint256 public totallyPrizesLocked;
    ...
    function initialize(
        ...
        conditionalMarginality = marginality_; // in decimals ^9
        conditionalReinforcementFix = reinforcement_; // in token decimals
        decimals = 10**9;
        mathAddress = math_;
    }

    function getLockedPrizes() external view override returns (uint256) {
        return totallyPrizesLocked;
    }

    /**
      * @dev create condition from oracle
      * @param oracleConditionID_ the current match or game id
      * @param rate1_ start rate for team 1
      * @param rate2_ start rate for team 2
    ...
    function createCondition(
        ...
        newCondition.fundBank[0] = (conditionalReinforcementFix * rate2_) /
        (rate1_ + rate2_);
        newCondition.fundBank[1] = (conditionalReinforcementFix * rate1_) /
        (rate1_ + rate2_);

        newCondition.reinforcement = conditionalReinforcementFix;
        newCondition.timestamp = timestamp_;
        newCondition.ipfsHash = ipfsHash_;
        ILP(lpAddress).lockReserve(conditionalReinforcementFix);

        // save new condition link
        newCondition.marginality = conditionalMarginality; //not used yet
     ...
     * @param minRate_ rate slippage
     * @return betID with rate of this bet and updated funds
     * @return rate
        ...
        )
    {
        ...
        uint256 rate =
            IMath(mathAddress).getRateFromBanks(
                condition.fundBank[0],
                condition.fundBank[1],
                    amount_,
                    outcomeWin_,
                condition.marginality,
                decimals
            );
        require(rate >= minRate_, "Core: rate too small");
        require(amount_ > decimals, "Core: small bet");

        Bet storage newBet = bets[lastBetNumber];
        ...
        condition.fundBank[outcomeWin_ - 1] =
            condition.fundBank[outcomeWin_ - 1] +
            amount_;
        condition.prizing[outcomeWin_ - 1] += (rate * amount_) / decimals;

        // calc maximum prizes value
        uint256 maxPrizes =
            (
                condition.prizing[0] > condition.prizing[1]
                    ? condition.prizing[0]
                    : condition.prizing[1]
            );
        if (maxPrizes > condition.maxPrizes) {
            // if new maxPrizes greater than previouse saved -> save new value
            // and add greater delta to global totallyPrizesLocked
            totallyPrizesLocked += (maxPrizes - condition.maxPrizes);
            condition.maxPrizes = maxPrizes;
        }
    ...
    /**
      * @dev resolve the prize
      * @param tokenId it is betID
      * @return success
      * @return amount of better win
      */
    function resolvePrize(uint256 tokenId)
        external
        override
        OnlyLP
        returns (bool success, uint256 amount)
    {
        ...
        // and exists amount of locked prizes -> release locked prizes from global state
        if (condition.maxPrizes != 0) {
            // decrease global totallyPrizesLocked on prize payment value
            totallyPrizesLocked -= condition.maxPrizes;
            condition.maxPrizes = 0;
        }

        (success, amount) = _viewPrize(tokenId);
    ...
    */
    function resolveCondition(uint256 conditionID_, uint256 outcomeWin_) external override onlyOracle {
        ...
        uint256 bettersPrizing;
        condition.outcomeWin = outcomeWin_;
        if (outcomeWin_ == 1) {
            bettersPrizing = condition.prizing[0];
        } else {
            bettersPrizing = condition.prizing[1];
        }

        uint256 profitReserve =
            (condition.fundBank[0] + condition.fundBank[1]) - bettersPrizing;
    ...
    function viewPrize(uint256 tokenId_)
        external
        view
        override
        returns (bool success, uint256 amount)
    {
        return (_viewPrize(tokenId_));
    }

    function getconditions(uint256 id)
        external
        view
    ...
    function _viewPrize(uint256 tokenId)
        internal
        view
        returns (bool success, uint256 amount)
    {
        ...
        if (
            ...
            uint256 winAmount =
                (currentBet.rate * currentBet.amount) / decimals;
    ...
    */
    ...
    ) public view returns(uint256)
    {
        ...
        IMath(mathAddress).getRateFromBanks(
            ...
            conditions[conditionID_].fundBank[1],
                amount_,
                outcomeWin_,
            conditions[conditionID_].marginality,
            decimals
        );
        return rate;
    }
    ...
    function getCurrentReinforcement() external view override returns(uint256) {
        return conditionalReinforcementFix;
    }
}
...
interface ILP {
    ...
    function viewPrize(uint256 tokenId) external view returns (bool, uint256);

    function withdrawPrize(uint256 conditionID, uint256 betID) external;
...
interface ICore {
    ...
    function getLockedPrizes() external view returns (uint256);

    function createCondition(uint256 oracleConditionID, uint256 rate1_, uint256 rate2_, uint256 timestamp, bytes32 ipfsHash) external;

    function resolveCondition(uint256 conditionID_, uint256 outcomeWin_) external;

    function viewPrize(uint256 tokenId) external view returns (bool, uint256);

    function resolvePrize(uint256 tokenId) external returns (bool, uint256);
