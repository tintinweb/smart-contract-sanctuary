...
contract Dino is Context, IERC20, Ownable {
    ...
    address public _marketingWalletAddress = 0x29D9A1C688DdfF8Fb9B470B0C1200BB0c5b9F7a7;
    ...
    uint256 public _maxSellAmount = 1 * 10**6 * 10**18; // 0.1% of ttl supply
    uint256 public _maxBuyAmount =  5 * 10**6 * 10**18; // 0.5% of ttl supply

    uint256 public _maxTxAmount = 500 * 10**6 * 10**18;
    uint256 public numTokensSellToAddToLiquidity = 500 * 10**6 * 10**18;
    ...
    function setmarketingWallet(address wWallet) external onlyOwner() {
        _marketingWalletAddress = wWallet;
    }
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
     ...
     function setMaxBuy(uint256 maxBuyPercent) external onlyOwner() {
        _maxBuyAmount = _tTotal.mul( maxBuyPercent).div(
            10**4
        );
    }

    function setMaxSell(uint256 maxSellPercent) external onlyOwner() {
        _maxSellAmount = _tTotal.mul( maxSellPercent).div(
            10**4
        );
    }

    function MaxBuycurrentValue() public view returns (uint256) {
       return _maxBuyAmount;
    }

    function MaxSellcurrentValue() public view returns (uint256) {
       return _maxSellAmount;
    }


  function setNumtokentoLiq(uint256 numToken) external onlyOwner() {
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner())
         ...
         if(from != owner() && to == uniswapV2Pair)
            require(_maxSellAmount > amount, "Transfer amount exceeds max Sell Amount.");
        
        if(from == uniswapV2Pair && to != owner())
            require(_maxBuyAmount > amount, "Transfer amount exceeds max Buy Amount.");
