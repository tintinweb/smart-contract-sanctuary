...
contract Goldmine721 is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;
    //reward contract
    Reward20 rewardContract;
    ...
    uint256 public whiteListedCost = 0.0001 ether;
    ...
    constructor(string memory _baseUri, address _rewardAddress)
        ERC721("GOLDMINE", "GLDMINERS")
    {
        baseURI = _baseUri;
        rewardContract = Reward20(_rewardAddress);
    }

    function claimRewards() public {
        uint256 blance = getTotalClaimable(msg.sender);
        rewardContract.mint(msg.sender, blance);
    ...
    function getTotalClaimable(address _user) public view returns (uint256) {
        ...
        uint256 diff = lastUpdate[_user] > 0 ? time - lastUpdate[_user] : 0;
        uint256 pending = (balanceOfUser * BASE_RATE * diff) / 60;
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        for (uint256 i = 1; i <= _mintAmount; i++) {
            ...
            //do rewards
            uint256 clamable = getTotalClaimable(msg.sender);
            rewards[msg.sender] += clamable;
            lastUpdate[msg.sender] = block.timestamp;
        }
    }
