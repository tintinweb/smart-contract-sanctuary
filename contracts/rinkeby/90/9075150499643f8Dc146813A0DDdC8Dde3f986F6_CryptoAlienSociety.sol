...
contract CryptoAlienSociety is ERC721Enumerable, Ownable {
    ...
    // Token detail
    struct CryptoAlienSocietyDetail {
    ...
    // Token Detail
    mapping(uint256 => CryptoAlienSocietyDetail) private _crytoAlienSociety;
    ...
    function multipleApprove(address to, uint256[] memory idlist) public {
        for (uint256 i = 0; i < idlist.length; i++) {
            super.approve(to, idlist[i]);
        }
    }
    ...
    function mintCryptoAlienSociety(uint256 numberOfTokens) public payable {
        ...
        for (uint256 i = 1; i <= numberOfTokens; i++) {
            ...
            if (tokenId <= MAX_TOKENS) {
                _safeMint(msg.sender, tokenId);
                _crytoAlienSociety[tokenId] = CryptoAlienSocietyDetail(
                    first_encounter
                );
                emit TokenMinted(tokenId, msg.sender, first_encounter);
            }
     ...
     */
    function getCryptoAlienSocietyDetail(uint256 tokenId)
        public
        view
        returns (CryptoAlienSocietyDetail memory detail)
    {
        require(_exists(tokenId), "Token was not minted");

        return _crytoAlienSociety[tokenId];
    }
}
