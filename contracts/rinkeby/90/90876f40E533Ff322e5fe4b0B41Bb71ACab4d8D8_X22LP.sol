...
contract X22LP is ReentrancyGuard {
        ...
        validAmount(amount)
    {
        ...
        tokens[_index].safeTransferFrom(
            msg.sender,
            address(this),
            amount.mul(poolPart).div(DENOMINATOR)
        );
        uint256[3] memory amountToStrategy;
        for (uint8 i = 0; i < N_COINS; i++) {
            if (i == _index) {
                amountToStrategy[i] = amount
                    .mul((DENOMINATOR).sub(poolPart))
                    .div(DENOMINATOR);
                tokens[i].safeTransferFrom(msg.sender, address(strategy), amountToStrategy[i]);
            } else {
                amountToStrategy[i] = 0;
            }
        }
        if (checkValidArray(amountToStrategy)) {
            _deposit(amountToStrategy);
        }
    ...
    function requestWithdrawWithXPT(
        uint256 amount,
        uint256 _index,
        bool payingCharges
        ...
        if (tokenAmount > currentPoolAmount) {
            require(
                YieldPoolBalance < tokenAmount.mul(10**18),
                "yield pool balance low"
            );
        }
        if (payingCharges == true) {
            ...
            for (uint8 i = 0; i < N_COINS; i++) {
                if (i == _index) {
                    amountWithdraw[i] = tokenAmount.sub(currentPoolAmount);
                } else {
                    amountWithdraw[i] = 0;
                }
            }
            ...
            uint256 temp = (tokenAmount.mul(fees)).div(DENOMINATOR);
            selfBalance = selfBalance.sub(
                (tokenAmount.sub(temp)).mul(1e18).div(10**decimal)
            );
    ...
    function claimTokens(bool payingCharges) external nonReentrant {
        ...
        uint256 tokenAmount;
        tokenAmount = requestedAmount[msg.sender].mul(total).div(
            XPTtoken.totalSupply()
        );
        uint256 currentPoolAmount = getBalances(_index);
        if (tokenAmount > currentPoolAmount) {
            require(
        ...
        tokenAmount = tokenAmount.mul(10**decimal).div(10**18);
        if (payingCharges) {
            temp = (tokenAmount.mul(fees)).div(DENOMINATOR);
        } else {
            require(
                requestedTime[msg.sender] + coolingPeriod <= block.timestamp,
                "You have to wait for 8 days after requesting for withdraw"
            );
        }
        uint256[N_COINS] memory amountWithdraw;
        for (uint8 i = 0; i < N_COINS; i++) {
