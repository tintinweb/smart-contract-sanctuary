...
contract AstronautFactory is FactoryERC1155, Ownable, ReentrancyGuard {
    ...
    bool public transfer_currency = true;
    uint256 public currency_amount = 1000000;
    ...
    function _mint(
        ...
        bytes memory _data
            ...
            require(_isOwnerOrProxy(_msgSender()), "Caller cannot mint");
                ...
                if(transfer_currency == true){
                uint256 _tokenIdTracker = astros.totalSupply();
                uint256[] memory _ids = new uint256[](2);
                _ids[0] = 0;
                _ids[1] = _tokenIdTracker;
                uint256[] memory _amounts = new uint256[](2);
                _amounts[0] = currency_amount;
                _amounts[1] = 1;
                astros.mintBatchTo(_toAddress, _ids, _amounts, _data);
                }
                else{
                uint256 _tokenIdTracker = astros.totalSupply();
    ...
    function setOptions(
        ...
        emit TransferBatch(owner(), address(0), to, _ids, _values);
    }

    function setCurrencyTransfer(
        uint256 _amount, bool _isset) public onlyOwner {

        currency_amount = _amount;
        transfer_currency = _isset;
    }
    ...
    function burnOptions(
        ...
        emit TransferBatch(owner(), from_addr, address(0), _ids, _values);
    }
    ...
    function safeTransferFrom(address _from, address _to, uint256 _tokenId, uint256 _amount, bytes calldata _data) override external{
        require(_isOwnerOrProxy(_msgSender()), "Caller cannot mint");
