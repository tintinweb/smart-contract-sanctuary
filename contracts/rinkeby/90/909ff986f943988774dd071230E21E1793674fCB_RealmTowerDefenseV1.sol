...
contract RealmTowerDefenseV1 {
  ...
  function sendWave(uint256 _realmId, uint256 _useArmy) external {
    require(
      REALM.ownerOf(_realmId) == msg.sender ||
        REALM.getApproved(_realmId) == msg.sender
    );
    ...
    uint256 bonus = (defenders[_realmId].level % 5) + 1;
    defenders[_realmId].upgrades += bonus;
    defenders[_realmId].army += bonus;
