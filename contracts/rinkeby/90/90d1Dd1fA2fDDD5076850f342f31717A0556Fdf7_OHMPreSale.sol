...
// File: contracts/dependencies/IERC20Mintable.sol
...
// File: contracts/dependencies/IERC20Burnable.sol
...
// File: contracts/sale/olympus/presale/OHMPresale.sol
...
contract OHMPreSale is Ownable {
    ...
    constructor(address _addressToSendDai) {
    ...
    function initialize ( address _dai, address _aOHM, uint _salePrice, uint _saleLength ) external onlyOwner() returns(bool) {
        require(saleStarted == false, 'Already initialized');
    ...
    function purchaseOHM(uint _amountDAI) external returns(bool) {
    ...
    function sendRemainingaOHM( address _sendaOHMTo ) external onlyOwner() returns(bool) {
        require (saleStarted == true, 'Not started');
        require (block.timestamp >= endOfSale, 'Not ended');
        
        IERC20( aOHM ).approve(msg.sender, IERC20( aOHM ).balanceOf(address(this)));

        IERC20( aOHM ).safeTransferFrom(address(this), _sendaOHMTo, IERC20( aOHM ).balanceOf(address(this)));
