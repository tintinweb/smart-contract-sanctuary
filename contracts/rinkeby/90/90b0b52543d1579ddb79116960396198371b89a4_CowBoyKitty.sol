...
// File: contracts/CowBoyKitty.sol

pragma solidity ^0.8.0;

contract CowBoyKitty is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    constructor(string memory _initBaseURI, string memory _initNotRevealedUri)
        ERC721("CowBoyKitty", "COWBOYKITTY")
    {
    ...
    function presaleMint(uint256 _amount) external payable {
        ...
        require(presaleActive, "CowBoyKitty - Presale is not active.");
        ...
        if (msg.sender != owner()) {
            ...
            require(
                ...
                "CowBoyKitty - Presale Only for Whitelist users."
            );
            require(_amount <= maxMintAmount);
            require(
                msg.value == cost * _amount,
                "CowBoyKitty::preSaleMint: insufficient ethers"
            );
        }
    ...
    function mint(uint256 _amount) external payable {
        require(!paused, "the contract is paused");
        require(mainsaleActive, "CowBoyKitty::mint: Mint is not active.");
        uint256 supply = totalSupply();
        require(supply + _amount <= maxSupply, "max NFT limit exceeded");
        if (msg.sender != owner()) {
            ...
            require(
                msg.value >= cost * _amount,
                "CowBoyKitty::mint: insufficient ethers"
            );
        }
        for (uint256 i = 1; i <= _amount; i++) {
            addressMintedBalance[msg.sender]++;
    ...
    function withdraw() public onlyOwner {
        ...
        (bool os1, ) = payable(0xc9D2C6eF131487251CeF64D5a6757bA953faE07c).call{
