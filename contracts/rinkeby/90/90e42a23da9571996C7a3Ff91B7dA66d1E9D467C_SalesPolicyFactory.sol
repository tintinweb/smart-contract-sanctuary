...
contract SalesPolicy is EIP712MetaTransaction("BuyPolicyMetaTransaction", "1"), ERC1155Supply, ISalesPolicy, ReentrancyGuard {
    using Counters for Counters.Counter;

    address public immutable SSRP;
    struct Policy {
        uint128 coverStartAt;
        uint128 coverStartNumberAt;
    ...
    modifier onlySSRP() {
        require(msgSender() == SSRP, "UnoRe: SalesPolicy Forbidden");
        _;
    }
    ...
    function buyPolicy(
        ...
        getPolicy[lastIdx] = Policy({
            ...
            coverStartAt: uint128(block.timestamp),
            coverStartNumberAt: uint128(block.number),
        ...
        _mint(msgSender(), lastIdx, _policyPriceInUSDT, "");

        policyIdx.increment();
        emit BuyPolicy(protocolIdx, lastIdx, msgSender(), _coverageAmount, _policyPriceInUSDT, _premiumCurrency, premiumPaid);
    }

    function approvePremium(address _premiumCurrency) external override onlySSRP {
...
contract SalesPolicyFactory is ISalesPolicyFactory {
    ...
    address private UNO_TOKEN;

    constructor(address _usdt_token, address _uno_token) {
