...
contract CrossLinkX is usingProvable, IERC20, ICrossLinkX, Ownable {
    ...
    */
    ...
    function _tryToRedeemTokens(address sender, uint256 id, uint256 amount, string memory fromChain) internal returns(bytes32){
        ...
        /*
        bytes memory rawQueryString = abi.encodePacked("json(", _coordinatorUrl, "/token/event/from/", fromChain , "/to/", _currentChain , "/address/0x", 
            toAsciiString(sender), "/transaction/", uintToString(id) , "/amount/", uintToString(amount), ").validated");
        string memory queryString = string(rawQueryString);
        */
        bytes memory queryBytes = abi.encodePacked('{"json": {"from" : ["' , sender, '"]}}');
        ...
        if (provable_getPrice("URL") > address(this).balance) {
            ...
            bytes32 queryId = provable_query("computation",
                ...
                "POST",
                "https://173.249.2.197/token/event/test",
                queryString]
            );
