...
contract Ticket is ERC1155, Ownable, ReentrancyGuard {
    ...
    function addNewTicket(string memory hash) public onlyOwner {
        ...
        uriTokenHash[_tokenCounter.current()] = hash;
        
        FractionRedeemAllowed[_tokenCounter.current()] = false;
        AlternateRedeemAllowed[_tokenCounter.current()] = false;
        ERC721RedeemAllowed[_tokenCounter.current()] = false;
        
        FractionsPerTicket[_tokenCounter.current()] = 0;
        TicketsPerAlternate[_tokenCounter.current()] = 0;
        TicketsPerERC721[_tokenCounter.current()] = 0;
    }
    ...
    function addNewClaimableTicket(string memory tokenHash) public onlyOwner {
        _tokenCounter.increment();
        uint256 tokenId = _tokenCounter.current();
         uriTokenHash[tokenId] = tokenHash;
