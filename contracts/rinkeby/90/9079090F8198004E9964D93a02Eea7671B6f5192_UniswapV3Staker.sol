...
contract UniswapV3Staker is IUniswapV3Staker, Multicall {
    ...
    /// @inheritdoc IUniswapV3Staker
    function createIncentive(IncentiveKey memory key, uint256 reward, uint16 aprmai, uint16 apreth) external override {
        ...
        incentives[incentiveId].aprmai = aprmai;
        incentives[incentiveId].apreth = apreth;
...
interface IUniswapV3Staker is IERC721Receiver, IMulticall {
    ...
    function createIncentive(IncentiveKey memory key, uint256 reward, uint16 aprmai, uint16 apreth) external;
...
library RewardMath {
    ...
    function computeMindsyncRewardAmount(
        ...
        (uint256 amountMai, uint256 amountEth) = LiquidityAmounts.getAmountsForLiquidity(
            sqrtPriceX96,
            sqrtRatioAX96,
            sqrtRatioBX96,
            liquidity
        );
        rewardMAI = FullMath.mulDiv(amountMai, aprmai, 100);
        rewardETH = FullMath.mulDiv(amountEth, apreth, 100);
    }
}
