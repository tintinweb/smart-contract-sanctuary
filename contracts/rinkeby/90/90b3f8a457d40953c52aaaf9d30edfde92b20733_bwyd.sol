...
contract bwyd is ERC721PresetMinterPauserAutoId, Ownable {
    ...
    /*Admin Allocation for team and competition drops */
    uint256 public constant ADMIN_MINT = 611;
    uint256 public adminMinted;
    string private _baseURIextended;
    string public notRevealedURI;
    string public openSeaDataURI;
    ...
    function contractURI() public view returns (string memory) {
        //Retrun Metadata for collection and royalties.
        return openSeaDataURI;
    }
    
    function setHangryDatoURL(string memory _openSeaDataURI) public onlyOwner {
        openSeaDataURI = _openSeaDataURI;
     }
    
    function createTokens(uint256 _howMany) external payable {
        require(!paused(), "HANGRY Contract paused, no NFTs can be minted right now.");
        require(_howMany > 0,"HANGRY: minimum 1 tokens need to be minted!");
        ...
        require(msg.value >= price.mul(_howMany),"HANGRY: insufficient ETH to mint! Try minting less NFTs");
    ...
    function _mintToken(address to) private {
        ...
        require(
            ...
            "HANGRY: one of these tokens already exists!"
        );
        ...
        adminOnly
    {
        require(
            _howMany > 0,
            "HANGRY: minimum 1 tokens need to be minted!"
        );
        /*require(_howMany <= 20, "YourNFToken: max 20 tokens at once!");*/
        require(
            getAdminRemainingMint() > 0,
            "HANGRY: All admin token minted"
        );
        require(_howMany <= getAdminRemainingMint(), "HANGRY: Token should be less than remaining amount to mint");
        if (mintedTokens == 0) {
            lastIPFSID = getRandom(
                1,
                totalTokenToMint,
    ...
    /* Function to mint comp winners and investor NFTS */
    function mintSpecificTokenAdmin(uint256 _tokenId, address to) external adminOnly{
        require(getAdminRemainingMint() > 0, "HANGRY: All admin token minted");
        require(adminMinted <= getAdminRemainingMint(), "HANGRY: Token should be less than remaining amount to mint");
       
       _mintSpecificToken(_tokenId,to);
       adminMinted++;
    }
    
    /* Function to mint specific ID */
    function _mintSpecificToken(uint256 _tokenId, address to) private {
        ...
        _safeMint(to, mintedTokens);
        _setTokenURI(mintedTokens, _tokenId.toString());
    }
    ...
    function getIpfsIdToMint() private view returns (uint256 _nextIpfsId) {
        require(
            !isAllTokenMinted(),
            "HANGRY: all tokens have been minted!"
        );
