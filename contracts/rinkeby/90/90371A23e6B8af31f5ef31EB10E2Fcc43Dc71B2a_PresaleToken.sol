...
contract PresaleToken is ERC1155, Ownable {
  ...
  uint16 public constant MAX_SUPPLY = 4975;
  uint16 public constant MAX_SUPPLY_TEAM = 5000;
  uint16 public constant MAX_PER_ACCOUNT = 50;

  // variables
  mapping(uint256 => address) private _ownerOfToken;
  mapping(address => uint256) private _totalTokensOf;
  bool public salesActive = false;
  bool public teamClaimed = false;
  uint16 public claimed = 0;
  ...
  function claim(uint16 amount) external payable {
    ...
    require(_totalTokensOf[msg.sender] + amount <= MAX_PER_ACCOUNT, "Claim: Can not claim that many.");
    ...
    for (uint256 i = 0; i < amount; i++) {
      _mint(msg.sender, claimed, 1, '');
      _ownerOfToken[claimed] = msg.sender;
      _totalTokensOf[msg.sender] += 1;
  ...
  function teamClaim() external onlyOwner {
    ...
    for (uint256 i = MAX_SUPPLY; i < MAX_SUPPLY_TEAM; i++) {
      _mint(msg.sender, i, 1, '');
      _ownerOfToken[i] = msg.sender;
      _totalTokensOf[msg.sender] += 1;
    }
    teamClaimed = true;
  }
  ...
  function safeTransferFrom(
    ...
    uint256 totalFromBalance = _totalTokensOf[from];
    require(totalFromBalance >= amount, "Transfer: Insufficient balance for transfer");
    unchecked {
      _totalTokensOf[from] = totalFromBalance - amount;
    }
    _totalTokensOf[to] += amount;
    _ownerOfToken[id] = to;
  }
  ...
  function ownerOf(uint16 tokenId) external view returns(address) {
    address owner = _ownerOfToken[tokenId];
