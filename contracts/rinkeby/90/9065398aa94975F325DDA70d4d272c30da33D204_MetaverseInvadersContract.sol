...
abstract contract ERC721EnumMTIV is ERC721MTIV, IERC721Enumerable {
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view virtual override returns (uint256 tokenId) {
        require(index < ERC721MTIV.balanceOf(owner), "ERC721Enum: owner ioob");
        uint ownedNftInd;
    ...
    function tokensOfOwner(address owner) public view returns (uint256[] memory) {
        require(0 < ERC721MTIV.balanceOf(owner), "ERC721Enum: owner ioob");
...
contract MetaverseInvadersContract is ERC721EnumMTIV, Ownable, ReentrancyGuard {
    ...
    address public _trustedContract = address(0);
    ...
    function mint(uint256 _invaders) external payable {
        require(isSaleActive, "MTIV: Sale is not active");
        require(_invaders > 0, "MTIV: Minted amount should be positive" );
        require(_invaders <= MAX_SALE_MINT, "MTIV: Minted amount exceeds sale limit" );

        uint256 totalSupply = totalSupply();

        require(totalSupply + _invaders <= _publicSupply(), "MTIV: The requested amount exceeds the remaining supply" );
        require(msg.value >= COST * _invaders);

        for (uint256 i = 0; i < _invaders; i++) {
            _safeMint(msg.sender, totalSupply + i);
        }
    }

    function presaleMint(uint256 _invaders) external payable {
        ...
        require(_invaders <= availableMints, "MTIV: Too many invaders requested");
        require(totalSupply + _invaders <= _publicSupply(), "MTIV: The requested amount exceeds the remaining supply");
        require(msg.value >= COST * _invaders , "MTIV: Insufficient funds in your wallet");

        presaleWhitelistMints[msg.sender] += _invaders;
    ...
    function gatherInvadersToBuildTheRocket(address[] calldata _earlyInvaders) external onlyOwner {
        for (uint256 i = 0; i < _earlyInvaders.length; i++) {
            require(_earlyInvaders[i] != address(0), "MTIV: Null address is not allowed");
            presaleWhitelist[_earlyInvaders[i]] = true;
            presaleWhitelistMints[_earlyInvaders[i]] > 0 ? presaleWhitelistMints[_earlyInvaders[i]] : 0;
        }
    }
    ...
    // founders minting
     function reserveInvadersTroups(address _to, uint256 _reservedInvaders) public onlyOwner {
        uint256 supply = totalSupply();
        require(
            _reservedInvaders > 0 && _reservedInvaders <= FOUNDERS_RESERVE_MINTS,
            "MTIV: Not enough reserve left for team"
        );
        for (uint256 i = 0; i < _reservedInvaders; i++) {
            _safeMint(_to, supply + i);
        }
        FOUNDERS_RESERVE_MINTS -= _reservedInvaders;
    }

    function transferFrom(address _from, address _to, uint256 _tokenId) public override {
        ERC721MTIV.transferFrom(_from, _to, _tokenId);
        _transferNotice(_from, _to, _tokenId);
    }

    function safeTransferFrom(address _from, address _to, uint256 _tokenId, bytes memory _data) public override {
        ERC721MTIV.safeTransferFrom(_from, _to, _tokenId, _data);
        _transferNotice(_from, _to, _tokenId);
    }
}