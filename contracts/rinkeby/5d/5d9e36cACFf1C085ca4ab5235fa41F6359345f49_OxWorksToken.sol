...
contract OxWorksToken is IOxWorksToken, Ownable, ERC721Checkpointable {
     ...
     * Noun. Nounders reward OxWorks are minted every 10 OxWorks, starting at 0,
     * until 183 nounder OxWorks have been minted (5 years w/ 24 hour auctions).
    ...
    function _mintTo(address to, uint256 nounId) internal returns (uint256) {
        IOxWorksSeeder.Seed memory seed = seeds[nounId] = seeder.generateSeed(nounId, descriptor);
...
interface IOxWorksToken is IERC721 {
    event NounCreated(uint256 indexed tokenId, IOxWorksSeeder.Seed seed);
