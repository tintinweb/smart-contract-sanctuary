    ...
    // WhitelistAdminRole
{
     ...
     * @dev increments the value of _currentTokenID
     */
    function _incrementOrderTypeId() private {
        _currentOrderID++;
    }
...
contract AlexNFT is ERC1155Tradable {
    ...
    function buy(address _ownerAddress, uint256 _tokenId, uint _amount) external payable {
        ...
        _trade(_tokenId, _amount, _ownerAddress, price, isagent, agentcommission, paymentReceiver);

        emit Purchase(previousOwner, newOwner, price * _amount, _tokenId, contractURI());
    }
    
    function _trade(uint _tokenId, uint _amount, address _owner, uint256 _price, bool _isagent, uint256 agentcommission, address payable paymentReceiver) internal {
