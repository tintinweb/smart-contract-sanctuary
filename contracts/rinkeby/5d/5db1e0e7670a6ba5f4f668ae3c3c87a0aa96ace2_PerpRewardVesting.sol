...
contract PerpRewardVesting is MerkleRedeemUpgradeSafe, BlockContext {
    ...
    function claimWeeks(address _account, Claim[] memory _claims) public virtual override {
        super.claimWeeks(_account, _claims);
    }
    ...
    function verifyClaim(
        ...
        //  ---------+------+---+-----+------+---+
        //           |          |     |     now  |
        //           |        week2   |          merkleRootTimestampMap[week2]
        //           |                |
        ...
        // merkleRootTimestampMap[_week] > 0, to prevent from invalid _week
        require(
            _blockTimestamp() >= merkleRootTimestampMap[_week] && merkleRootTimestampMap[_week] > 0,
            "Invalid claim"
        );
        return super.verifyClaim(_liquidityProvider, _week, _claimedBalance, _merkleProof);
    }
...
contract MerkleRedeemUpgradeSafe is PerpFiOwnableUpgrade, DecimalERC20 {
    ...
    function disburse(address _liquidityProvider, uint256 _balance) private {
        if (_balance > 0) {
            ...
            _transfer(token, _liquidityProvider, Decimal.decimal(_balance));
        }
    }
    ...
    function seedAllocations(
        ...
        _transferFrom(token, _msgSender(), address(this), Decimal.decimal(_totalAllocation));
    }
}
