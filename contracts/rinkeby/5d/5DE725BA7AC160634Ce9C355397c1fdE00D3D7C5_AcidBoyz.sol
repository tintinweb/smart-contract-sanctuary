...
import "./Storage/RandomNumber.sol";

contract AcidBoyz is ERC721URIStorage {
    RandomNumber rndNum;
    
    uint104 private tokenId;
    event AcidBoyMinted(uint256 indexed tokenId, string tokenURI);
    
    constructor (address _rndNum) ERC721("Acid-boyz", "ACDBZ") {
        rndNum = RandomNumber(_rndNum);
    }
    ...
    function getTokenURI() public returns(string memory) {
        rndNum.getRandomNumber();
        uint256 number = rndNum.getRandomResult();
