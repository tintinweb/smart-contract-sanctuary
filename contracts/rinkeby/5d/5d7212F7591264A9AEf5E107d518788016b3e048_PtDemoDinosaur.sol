...
contract PtDemoDinosaur is ERC721Enumerable, PaymentSplitter, Ownable, ReentrancyGuard {
    ...
    function getPrice() public view returns (uint256) {
        ...
        if (block.timestamp <= _saleConfig.startTime + 3 minutes) {
            _price = 0.01 ether;
        } else if (
            (block.timestamp >= _saleConfig.startTime + 3 minutes) &&
            (block.timestamp <= _saleConfig.startTime + 6 minutes)
        ) {
            _price = 0.008 ether;
        } else if (
            (block.timestamp > _saleConfig.startTime + 6 minutes) &&
            (block.timestamp <= _saleConfig.startTime + 12 minutes)
        ) {
            _price = 0.006 ether;
        } else {
            _price = 0.006 ether;
    ...
    function publicSaleMint(uint256 ammount) public payable nonReentrant {
        ...
        require(msg.value >= price * ammount, "PtDemoDinosaur: Insuficient funds");
        require(ammount <= 5, "PtDemoDinosaur: You can only mint up to five token at once!");
        require(tokensPerWallet[msg.sender] + ammount <= 5, "PtDemoDinosaur: You can't mint more than 5 tokens!");
        require(supply + ammount <= MAXSUPPLY, "PtDemoDinosaur: Mint too large!");
        uint256 initial = 1;
