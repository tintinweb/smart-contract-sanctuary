...
contract PaymentSplitter is Ownable { 
    ...
    function multisendEther(address payable[] memory account, uint256[] memory amount) public payable {
        require(address(this).balance != 0);
        uint length = account.length;
        require( length >= 1  && amount.length == length);
        for(uint i = 0; i>length; i++){

        Address.sendValue(payable(account[i]), amount[i]);
