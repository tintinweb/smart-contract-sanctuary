...
error AllTokensAreInUse(uint16 code);
...
contract Booster is ERC1155, Ownable, Pausable {
    ...
    function mint(uint16[] calldata tokenIds) external whenNotPaused {
        uint gen0ids = 0;
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            // Additinal check for Gen0, tokenId < 10000
            require(tokenIds[i] < 10000, "Not a Gen0 token, cannot mint");

            if (usedTokens[tokenIds[i]] == false) {
                gen0ids += 1;
    ...
    function burn(address account, uint amount) external payable whenNotPaused {
        require(minted > 0 && amount > 0 && minted - amount >= 0, "Invalida parameters");
