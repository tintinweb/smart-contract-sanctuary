...
contract NFTwitch4 is ERC721, ChainlinkClient, Ownable {
    ...
    public recordChainlinkFulfillment(_requestId)
  {
    ...
    bytes32 addr = bytes32(uint256(msg.sender) << 96);
  ...
  function requestFollows(address _oracle, string memory _jobId) public
  {
    ...
    public recordChainlinkFulfillment(_requestId)
  {
    ...
    while (data[senderID].followCount > data[senderID].requiredFollowCount)
    {
      ...
      require(data[senderID].requiredFollowCount > data[senderID].level, "overflow");
    	...
    	require(data[senderID].level > data[senderID].level - 1, "overflow");
    ...
    require(data[senderID].allowedMints > data[senderID].allowedMints - numberOfNewMints, "overflow");

  }

  // Returns the current level associated with this address
  function getStreamerData() public view returns (uint256, uint256, uint256, uint256)
  {
  ...
  function mint_nft() public
  {
    ...
    // TODO: This is not right, we need to get the nextID
    // based on the total NFTs made across all streamers,
    // not just the streamer minting the token.

    // Options:
    // 1. Have a massive array of NFTs spanning all streamers,
    // and a mapping of streamers to an array of indices (which NFTs they own)
    // 2. Have a mapping of streamers to an array of NFTs for that streamer,
    // and just keep the nextId as a variable that is incremented each time.
    // 3. Have two arrays and push the structs to both,
    // using whichever is more convenient for lookup.
  ...
  // Get the user ID associated with the address
  function getSenderID(address addr) public view returns (uint256)
  {
    return ids[addr];
  }
