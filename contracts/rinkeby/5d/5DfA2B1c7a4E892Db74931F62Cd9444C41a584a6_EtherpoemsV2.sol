 ...
 *  ______ _   _                                               __      _____
 * |  ____| | | |                                              \ \    / /__ \
 * | |__  | |_| |__   ___ _ __ _ __   ___   ___ _ __ ___  ___   \ \  / /   ) |
 * |  __| | __| '_ \ / _ \ '__| '_ \ / _ \ / _ \ '_ ` _ \/ __|   \ \/ /   / /
 * | |____| |_| | | |  __/ |  | |_) | (_) |  __/ | | | | \__ \    \  /   / /_
 * |______|\__|_| |_|\___|_|  | .__/ \___/ \___|_| |_| |_|___/     \/   |____|
 *
 */
...
contract EtherpoemsV2 is ERC721, Ownable {
  ...
  function mintMultiple(
    string[] memory _poemIDs,
    string[] memory _tokenURIs,
    string[] memory _poemNames,
    string[] memory _poemTexts,
    string[] memory _poemAuthors
  ) external payable reentrancyGuard {
    uint256 _tokenURIsLength = _tokenURIs.length;
    uint256 salePrice = getNFTPrice();
    require(
      msg.value >= _tokenURIsLength.mul(salePrice),
      "Insufficient funds to purchase."
    );
    require(
      _tokenURIsLength <= 20,
      "Mint Multiple: You can't mint more than 20 Etherpoems at a time."
    );
    require(
      _tokenURIsLength == _poemIDs.length,
      "Mint Multiple: Poems ID's length mismatch with Token URIs length"
    );
    require(
      _tokenURIsLength == _poemNames.length,
      "Mint Multiple: Poems Name's length mismatch with Token URIs length"
    );
    require(
      _tokenURIsLength == _poemTexts.length,
      "Mint Multiple: Poems Text's length mismatch with Token URIs length"
    );
    require(
      _tokenURIsLength == _poemAuthors.length,
      "Mint Multiple: Poems Authors' length mismatch with Token URIs length"
    );

    // 2. validate again and mint poems
    for (uint256 i = 0; i < _tokenURIsLength; i++) {
      string memory newPoemID = _poemIDs[i];
      string memory newURI = _tokenURIs[i];
      string memory newPoemName = _poemNames[i];
      string memory newPoemText = _poemTexts[i];
      string memory newPoemAuthor = _poemAuthors[i];
      _validateTokenToBeMinted(newURI, newPoemID);
      _mint(newPoemID, newURI, newPoemName, newPoemText, newPoemAuthor);
    }

    // 2. Send payment to payment splitter
  ...
  /**
   * Community grant minting for each EtherpoemsV1 NFT Owner.
   */
  function mintWithEtherpoemsV1(
    uint256 _EtherpoemsV1TokenID,
    string memory _poemID,
    string memory _myTokenURI,
    string memory _poemName,
    string memory _poemText,
    ...
    // Etherpoms NFT is with in valid range
    require(
      _EtherpoemsV1TokenID >= 0 && _EtherpoemsV1TokenID < 206,
      "Invalid Etherpoms NFT"
    );
    // Etherpoms NFT is not already used to claim etherPoem NFT
    require(
      EtherpomsNFTClaimed[_EtherpoemsV1TokenID] == 0,
      "Already minted with this Etherpoms NFT"
    );
    // etherPoem claimer owns the Etherpoms NFT
    require(
      IERC721(EtherpomsV1).ownerOf(_EtherpoemsV1TokenID) == msg.sender,
      "Not the owner of this Etherpoms NFT"
    );
    _validateTokenToBeMinted(_myTokenURI, _poemID);
    // Mark the Etherpoms NFT to be claimed
    EtherpomsNFTClaimed[_EtherpoemsV1TokenID] = 1;
    _mint(_poemID, _myTokenURI, _poemName, _poemText, _poemAuthor);
  }
