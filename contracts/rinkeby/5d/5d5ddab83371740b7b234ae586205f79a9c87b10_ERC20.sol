...
interface IERC20 {
    ...
    function burbalance() external returns (uint256);
    ...
    /**
     * @dev Emitted when `amount` tokens are burned from 'owner'
     *
     */
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    uint8 private _charity_percentage;
    uint8 private _marketing_percentage;
    uint8 private _reward_percentage;
    uint8 private _transaction_burn;
     ...
     * construction.
     */
    constructor (string memory name_, string memory symbol_, uint8 decimal_, uint256 totalSupply_, uint8 charity_percentage_, uint8 marketing_percentage_, uint8 reward_percentage_, uint8 transaction_burn_, address charity_address_, address marketing_address_) {
        _name = name_;
        _symbol = symbol_;
        _decimal = decimal_;
        _charity_percentage = charity_percentage_;
        _marketing_percentage = marketing_percentage_;
        _reward_percentage = reward_percentage_;
        _transaction_burn = transaction_burn_;
     ...
     * - the caller must have a balance of at least `amount`.
     */
    function burbalance() public virtual override returns (uint256) {
    ...
    function _transferStandard(address sender_address, address recipient_address, uint256 tAmount) private {
        ...
        uint256 currentRate =  _getRate();
        (,uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, currentRate);
        
        address sender = sender_address;
        address recipient = recipient_address;
        
        // subtract from sender
        uint256 sub_amount = tAmount;
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 charity_amount = totalAmount.mul(_charity_percentage).div(10**2);
        uint256 marketing_amount = totalAmount.mul(_marketing_percentage).div(10**2);
        uint256 burn_amount = totalAmount.mul(_transaction_burn).div(10**2);
