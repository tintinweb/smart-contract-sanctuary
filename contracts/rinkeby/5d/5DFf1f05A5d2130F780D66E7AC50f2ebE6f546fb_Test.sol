    ...
    Ownable
{
    ...
    string public baseTokenURI = "Empty";
   
    uint256 public Mint_price = 0.08 ether;
    uint256 public buy3_Discount = 0.18 ether;
    uint256 public Discount = 0.06 ether;
    ...
    function mint1orMultiples(uint amountToAdopt) external payable {
        require(amountToAdopt.mul(Discount) <= msg.value);
        require(amountToAdopt <= 12, "Only 12 per transaction");
        require(_currentTokenId.add(amountToAdopt) < MAX_SUPPLY, "Max Supply Reached");
        _mint(_msgSender(), _getNextTokenId());
            _incrementTokenId();
    }   
    ...
    function mintMany(uint256 num, address _to) public onlyOwner {
        ...
        require(num <= 500, "Max 20 Allowed.");
        for(uint256 i=0; i<num; i++){
