...
contract Burger is ERC721Enumerable, Pausable, Ownable, ReentrancyGuard {
   ...
   function mintNormalBURG(uint8 boxType)
        ...
        returns (uint256)
    {
        ...
        require(IERC20(BURG).allowance(msg.sender, address(this)) >= boxPriceBURG[boxType], "Not enought allowance" );

        IERC20(BURG).transferFrom(msg.sender, owner(), boxPriceBURG[boxType].mul(8).div(100) );      
        IERC20(BURG).transferFrom(msg.sender, address(0), boxPriceBURG[boxType].mul(92).div(100) );      
    ...
    function setBURGAddress(address _newAddress) external onlyOwner returns (bool) {
        BURG = _newAddress;
        return true;
    }
