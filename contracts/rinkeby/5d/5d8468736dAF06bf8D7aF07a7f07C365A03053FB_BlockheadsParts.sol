 ...
 */
contract BlockheadsParts is ERC721Tradable, ERC2981ContractWideRoyalties, JSON {
        ...
        returns (string memory)
    {
        ...
        JSON.JSONData memory json;
        addString(json, "name", "Blockhead Part");
        addString(json, "description", "Blockheads Parts");
        addString(
            json,
            "image",
            string(
        ...
        addAttribute(json, "Piece", label);
        addAttribute(json, "Layer", LAYER_NAMES[layer.layerIndex]);

        return
            string(
                abi.encodePacked(
                    "data:application/json;base64,",
                    Utils.base64Encode(bytes(serialize(json)))
                )
            );
    }
}
...
contract JSON {
    ...
    function addAttribute(
        ...
        data.attributes[data.numAttributes] = Attribute(traitType, value);
        data.numAttributes++;
        return data;
    }
        ...
        returns (string memory)
    {
        ...
        bool hasAttributes = data.numAttributes > 0;
        ...
        for (uint256 i = 0; i < data.numNumbers; i++) {
            ...
            if (
                i < data.numNumbers - 1 || hasBools || hasRaws || hasAttributes
            ) {
                json = abi.encodePacked(json, ", ");
            }
        }
        ...
        for (uint256 i = 0; i < data.numBools; i++) {
            ...
            if (i < data.numBools - 1 || hasRaws || hasAttributes) {
                json = abi.encodePacked(json, ", ");
            }
        }
        ...
        for (uint256 i = 0; i < data.numRaws; i++) {
            ...
            if (i < data.numRaws - 1 || hasAttributes) {
                json = abi.encodePacked(json, ", ");
            }
        }
        if (hasAttributes) {
            json = abi.encodePacked(json, '"attributes": [');
            for (uint256 i = 0; i < data.numAttributes; i++) {
                Attribute memory attr = data.attributes[i];
                json = abi.encodePacked(
                    json,
                    '{"trait_type": "',
                    attr.traitType,
