  ...
  ReentrancyGuard
{
  ...
  uint256 public constant LEGENDARY_COUNT = 14;
  ...
  // Presale
  mapping(address => uint256) private presaleNonce;
  ...
  uint256 private companyMintCount = 0;
  ...
  external onlyOwner {
    require((companyMintCount + mintCount) <= LEGENDARY_COUNT + COMPANY_RESERVE);
    require((companyMintCount + mintCount + _lastTokenId) <= TOTAL_SUPPLY);
    companyMintCount += mintCount;
    _mintTokens(msg.sender, companyMintCount);
  }
    ...
    function mintPresale(
        ...
        require(presaleMaxMint <= maxMintPerTransaction, "The max mint is over required amount");
        
        // Verify signature
        bytes32 message = getPresaleSigningHash(msg.sender, tokenCount, presaleMaxMint).toEthSignedMessageHash();
        require(ECDSA.recover(message, sig) == signVerifier, "Permission to call this function failed");

        presaleNonce[msg.sender]++;
  ...
  function getPresaleSigningHash(
    ...
    return
      keccak256(
        abi.encodePacked(sender, tokenCount, presaleMaxMint, presaleNonce[sender])
      );
  }
  ...
  function _mintTokens(
    ...
    require(_lastTokenId < TOTAL_SUPPLY, "This has been fully minted");
    require((_lastTokenId + tokenCount) <= TOTAL_SUPPLY, "Cannot purchase more than the available supply");
