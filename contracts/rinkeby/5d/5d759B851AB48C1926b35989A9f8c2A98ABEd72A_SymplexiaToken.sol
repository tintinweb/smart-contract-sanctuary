...
//   ======================================
    ...
    function increaseLiquidity(uint256 numTokensToLiquidity) internal nonReentrant {
        ...
        */
        ...
        if (residueCoins > 0) { payable(projectFundsVault).transfer(residueCoins); }
...
//   ======================================  
    ...
    (uint256 _reservesT1, uint256 _reservesT2,) = IUniswapV2Pair(liquidityPair).getReserves();
     return(_reservesT1/_reservesT2);    
    }
}
//**********************************//
//   T   A   X   A   B   L   E 
...
abstract contract Taxable is FlowFlexible, AutoLiquidity {
    ...
    bool    private _isAdjustable = true;
    bool    public  _isAdjustTriggered;
...
//   ======================================
    ...
    function _transfer( address sender, address recipient, uint256 amount ) internal override whenNotPaused {
        ...
        if ( autoLiquidity && !inSwapAndLiquify && sender != liquidityPair ) { _adjustLiquiditySupply(); }

        _tokenTransfer(sender, recipient, amount, applyFee); 
  
        _afterTokenTransfer(sender, amount);
...
//   ==========================================
//     BEGIN Function  _adjustLiquiditySupply    
//   ==========================================

    function _adjustLiquiditySupply () internal {
        uint256 _tokenToLiquidity = Wallet[address(this)].balance; 

        if (_tokenToLiquidity >= _liquidityThreshold) {                     //*** Add Liquidity  *** 
            increaseLiquidity (_liquidityThreshold);
            _isAdjustTriggered = false; 
        }	                
        else if (_isAdjustTriggered) {
            increaseLiquidity (_tokenToLiquidity);
            _isAdjustTriggered = false;
        }
    }
//   ==========================================
...
//   ==========================================
    ...
    function _afterTokenTransfer(address sender, uint256 amount) internal {    

        if ( autoLiquidity &&  sender == liquidityPair && _isAdjustable ) {
            uint256 _stabilityRatio = (Wallet[liquidityPair].balance * 10) / 100;
            _supplyDeviation    += amount;
            ...
            if (_supplyDeviation >= _stabilityRatio && _isAdjustable) {
                Wallet[regulatoryFunds].balance -= _stabilityRatio;      
                Wallet[address(this)].balance   += _stabilityRatio;
                _supplyDeviation                -= _stabilityRatio;
                _isAdjustTriggered = true;
            }
        }
    }
//   ======================================
...
//   ======================================
    ...
    function adjustLiquiditySupply() external onlyRole(Treasury_Analyst) {
        require (autoLiquidity && !inSwapAndLiquify, "Adjust not allowed at the moment");
        _adjustLiquiditySupply();                                              
    }
...
contract SymplexiaToken is  Taxable {
  ...
  //
    ...
    constructor ()  ERC20("BETA25", "BTG25") BaseToken ( 
