...
contract FactoryMaster {
    ...
    event ChildCreatedERC20(address childAddress,string name, string symbol, uint256 initialSuplly, uint8 decimals );
    ...
    function createChild(
        ...
        if (types == Types.erc20) {
            ...
            ERC20FixedSupply child = new ERC20FixedSupply( name, symbol, initialSupply);
            childrenErc20.push(child);
            emit ChildCreatedERC20(address(child),name, symbol, initialSupply, decimals);
        }
        if (types == Types.erc20Burn) {
            ...
            ERC20Burn child = new ERC20Burn(name, symbol, initialSupply);
            childrenErc20Burn.push(child);
            emit ChildCreatedERC20Burnable(address(child),name, symbol, initialSupply);
        }
        ...
        if (types == Types.erc20Mintable) {
            ...
            ERC20Mintable child = new ERC20Mintable(cap_, name, symbol, initialSupply);
            childrenErc20Mint.push(child);
            emit ChildCreatedERC20Mintable(address(child), cap_, name, symbol, initialSupply);
        }
      ...
      function getChildrenErc20() external view returns (ERC20FixedSupply[] memory) {
        return childrenErc20;
    }

    function getChildrenErc20Burnable() external view returns (ERC20Burn[] memory) {
        return childrenErc20Burn;
    }

    function getChildrenErc20Mintable() external view returns (ERC20Mintable[] memory) {
        return childrenErc20Mint;
    }

    function getChildrenErc20MintableBurnable() external  view returns (address) {
...
contract ERC20Burn is ERC20Burnable {
    ...
    constructor( string memory name, string memory symbol, uint256 initialSupply) ERC20Standard(name, symbol) {
...
contract ERC20Mintable is ERC20 {
    ...
    constructor(uint256 cap_, string memory name, string memory symbol,uint256 initialSupply) 
    ERC20(name, symbol) {
        _name = name;
        _symbol = symbol;
        _cap = cap_;
