...
contract MetaDrinks is ERC721Enumerable, ReentrancyGuard, Ownable, MetaDrinksDataGenerator {
    // errors
    // TCWE - Tokens count for whitelisted address exceeded
    // SNS - Sale is not started yet or paused
    // TMTA - Too many tokens per address
    // TMTT - Too many tokens per transaction
    // WV - Wrong ethers value
    // AOT - Must mint at least one token
    // EMS - Minting would exceed max supply
    ...
    // ERC721 props
    string public constant ERC721_NAME = "MD Test #2";
    string public constant ERC721_SYMBOL = "MDT1";
    ...
    constructor() ERC721(ERC721_NAME, ERC721_SYMBOL) {
    ...
    function mintTokensInternal(address _toAddress, uint256 _amount) internal {
        ...
        require(_amount > 0, "AOT");

        // checks tokens limit plus amount not reached (tested)
        require(tokenCounter + _amount + reservedTokensCount <= MAX_TOKENS_COUNT, "EMS");
    ...
    // todo: test none existing tokens
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        MetaDrinksTypes.Drink memory drink = genDrink(uint256(keccak256(abi.encodePacked(tokenId, "MD"))));
...
library MetaDrinksSvgGenerator {
    function genSvg(MetaDrinksTypes.Drink memory _drink) internal pure returns (string memory) {
        return string(abi.encodePacked(
                '<svg viewBox="0 0 800 800" preserveAspectRatio="xMinYMin meet" xmlns="http://www.w3.org/2000/svg" font-family="Courier New" font-size="71" fill="#FFF"><style>.t{font-size:42px;alignment-baseline:text-before-edge;}.s{ fill:#000;}</style><path fill="#000" d="M0 0h800v800H0z"/><g text-anchor="end" transform="translate(-30)">',
                genDrinkName(_drink),
    ...
    function genSvgHighlightedText(uint256 _index, string memory _text, string memory _highlightedText) internal pure returns (string memory) {
        string memory yOffset = MetaDrinksUtils.uint2str(_index * 58);
        return string(abi.encodePacked(
                '<rect y="', yOffset, '" x="', MetaDrinksUtils.uint2str(uint256((MetaDrinksUtils.len(_text) + 1) * 2522 / 100) - 10), '" width="', MetaDrinksUtils.uint2str(uint256(MetaDrinksUtils.len(_highlightedText) * 2522 / 100) + 20), '" height="48px"/>',
                '<text y="', yOffset, '" class="t">', _text, ' <tspan class="t s">', _highlightedText, "</tspan></text>"
            ));
    }
}
...
contract MetaDrinksDataGenerator {
    // names
    string[75] internal namesListA = ["07QmNI", "0NeYSoT", "0rIMU", "1Mndnv", "4a9d", "4t0G", "5VcfC", "5ztcqphM", "62ENaPXl", "6kigpW2", "6xJ2l", "7PB2An", "8eLuWeM", "8rZAH5E0", "A5Gv", "AIH8d03a", "ATArPSRQ", "B0uXh4r", "BYo3", "Cakqn", "DAaZ6EgZ", "DGZonbT", "DWGWiyG", "EQEIRNM", "GDQD2", "GxVOh", "JLEHtBP", "JfQBYp", "K0wp0", "Lrk0y", "M2NfMN6p", "OdS", "OozvC", "Qis7lOZM", "RX7rXl", "RojN0z", "Ry2egm8", "SCZRzwu", "SEaqWh", "TKbKRu", "TVp3z", "UAt4BV8w", "
    // alco base
    string[20] internal alcoBasesList = ["6bI", "GBNqmV", "IadubR4mu9wfRY7fIs7Z0", "IgfVr", "Iw5L", "OVN9eNpnVmqInAG4SX", "Qimi5VxTF", "RQfroS", "RQtvic8yU", "RYFx9T7Ipj", "Vbjly8", "Zj4Katb", "eHovOxGS", "hZXvR0m", "hqOGwZH5", "mbjBTLC", "qUTChN", "sGN", "sPbswH", "vTkiF4g"];

    // bitter sweets
    string[17] internal bitterSweetsList = ["5Qe3X", "5ebY51R2", "6CBG3TxsPSknbGJDt", "8MPRnkqG3eMaSzNG9j", "Bgwd22EryKxqY", "CPbI6sI3", "DZJiYnQnsrWuzyC", "MFyvKSg8UHA3F2W", "P47824", "WqB0pTI8pvcPdZ", "gIBjhLfOKy9Gim", "nJiq4XWoIdyHJL", "o3C1weLlkSrN47db5xYGc", "ppNWYDcRO6Gsas", "vUFVr1cCGfL", "x95G", "ymxNvwGnbgyI5d"];

    // sour parts
    string[17] internal sourPartsList = ["2ayG8y", "4z73VGvPZ15xZzfK", "D3MorvG11PaH2TEw", "DDQJCeQEzLEpz2d", "I0CxH52pjta", "JBxo5rOIY8", "JW82OBmh5r5JEu", "SGA5sgMpdPktY", "XwjGkADGypFjhO5", "YD1qlKvNldIBww3xJ7", "YDF7PnbGZeJtPMI", "Yf1Te1e0ubPlLqq", "hk2jNvyr8", "my2ePozj5I", "qBfi8PA0jbOpSbGB4", "rLQ7VJSYxDS", "vLBswM1VAe"];

    // fruits and herbs
    string[15] internal fruitsAndHerbs = ["0Sf90BVTdReokE", "1l8Ge4MKEYvECzlOh", "3QfSzbVLtmZCC", "4W4zep3WNe", "FLpsT8ZotvHC", "FgVeLS8UM8", "GTvCZY5H5W", "R31KlwR91", "Wy9JxBYpD", "ZAfn9Efe21E81P", "e0fVafXUV80A0y5", "jh9JXatFa1B", "puikk42uI", "qwYQLIRcXon", "s6PqriFYeR9fri"];

    // dressings
    string[18] internal dressingsList = ["1lBCWdDu", "7SAkXRA", "8Ngj06", "CTFM9hG8P5hu", "Cs5MReJcGQ1JviX95dbn", "FnIy8jDfCtiH9R0GkNK8", "I0mP1b5ltDbI2T", "ORD51fDa1WLYuLqG", "RUAFsu6cg", "WjOh6I5", "cHFTotbvV", "eVqrIcK7", "h72aesBiC30ReZ", "keB3HA0", "laZHZ6VPoEEd", "mTHgZ6pZAccJ", "v5ZF2oJvILUz", "z5e1WKSYjJl5"];
    uint256[] internal dressingsPinchPostfix;

    // methods
    string[4] internal methodsList = ["B0Jxmtod1upYGflbaH0g", "KrM89", "sJniC", "yKNG"];

    // glasses
    string[16] internal glassesList = ["2Hivbmx", "4lULv6EJ", "8MQ5RfH12Caf", "Eue5hD91Ge", "J70K", "PlePXAjz", "SMMnabF", "XvyNgw", "eu4", "hhKgInHlhodTQ", "hvah", "m4CS", "n5fwk5", "x7XraMZb1", "y5QvBeGB", "z63tuLaZi"];
    ...
    // top ups
    string[17] internal topUpsList = ["3cTi3PfoqF", "NrnqcBsjbQ9H", "PIvmp", "QwTJNmjnJ", "SrJ4Z", "YDgjraoSd2xJ", "b7ko", "cSEZ2V8lbUG", "eqmuMJaeRBdNX", "f2uV0YQH", "lf5a86HMRmGct", "mSuce", "n5PEZRiHx", "pMbhPlrx", "rrapDbZ", "y6zQ", "yYrhakchsR9uRZB"];
    ...
    // todo: test at least 20
    function genDrink(uint256 _randomness) public view returns (MetaDrinksTypes.Drink memory) {
        uint256 alcoBasePartsCount = genAlcoBasePart(_randomness);
        uint256 fruitOrHerbRandomness = MetaDrinksUtils.reRollRandomness(_randomness, "FH");
        uint256 dressingIndex = MetaDrinksUtils.reRollRandomness(_randomness, "DR") % dressingsList.length;
        uint256 glassIndex = MetaDrinksUtils.reRollRandomness(_randomness, "GL") % glassesList.length;
        string memory glassPostfix = genGlassPostfix(glassIndex);
        return MetaDrinksTypes.Drink(
            genSymbol(alcoBasePartsCount),
            namesListA[MetaDrinksUtils.reRollRandomness(_randomness, "NA") % namesListA.length],
            namesListB[MetaDrinksUtils.reRollRandomness(_randomness, "NB") % namesListB.length],
            namesListC[MetaDrinksUtils.reRollRandomness(_randomness, "NC") % namesListC.length],
            alcoBasesList[MetaDrinksUtils.reRollRandomness(_randomness, "AB") % alcoBasesList.length],
            string(abi.encodePacked(MetaDrinksUtils.uint2str(alcoBasePartsCount), alcoBasePartsCount == 1 ? " ual" : " uals")),
            bitterSweetsList[MetaDrinksUtils.reRollRandomness(_randomness, "BS") % bitterSweetsList.length],
            sourPartsList[MetaDrinksUtils.reRollRandomness(_randomness, "SP") % sourPartsList.length],
            ...
            methodsList[MetaDrinksUtils.reRollRandomness(_randomness, "ME") % methodsList.length],
            genGlass(glassIndex),
            bytes(glassPostfix).length != 0,
            glassPostfix,
            !MetaDrinksUtils.isUintArrayContains(glassesBlocksTopUps, glassIndex),
            topUpsList[MetaDrinksUtils.reRollRandomness(_randomness, "TU") % topUpsList.length]
        );
    }

    // todo: probability test
    function genAlcoBasePart(uint256 _randomness) internal pure returns (uint256) {
        uint256 prob = MetaDrinksUtils.reRollRandomness(_randomness, "ABP") % 10;
        // 10%
        if (prob == 0) return 0;
        // 20%
        if (prob < 3) return 1;
    ...
    function genSymbol(uint256 _partsCount) internal pure returns (string memory) {
        if (_partsCount == 0) return "X";
        if (_partsCount == 1) return "Y";
        if (_partsCount == 2) return "Z";
        return "F";
    }
}
