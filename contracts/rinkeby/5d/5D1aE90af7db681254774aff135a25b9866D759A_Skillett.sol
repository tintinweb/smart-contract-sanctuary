...
contract Skillett is ERC721Enumerable, Ownable, AccessControl {
    ...
    function mintTokens(uint256 num) public payable isSaleOpen(){
        ...
        require( num <= MAXMINT_PUBLIC, "Skillettt: You can mint a maximum of 10 Skilletts per transaction");
        require( supply + num <= TOTAL_NUMBER_OF_SKILLETT, "Skillettt: Exceeds maximum Skilletts supply");
        require( msg.value >= PRICE * num, "Skillettt: Ether sent is less than PRICE * num");
    ...
    function presale_mintTokens(uint256 num) public payable isPresaleOpen() preSaleAllowedAccount(msg.sender){
        uint256 supply = totalSupply();
        require(presale_left > num, "Skillettt: Exceeds presale reserved Skilletts supply");
        require(supply + num <= TOTAL_NUMBER_OF_SKILLETT, "Skillettt: Exceeds maximum Skilletts supply");
        require(num <= MAXMINT_PRIVATE, "Skillettt: You can mint a maximum of 10 Skilletts per transaction");
	    require(msg.value >= PRICE * num, "Skillettt: Ether sent is less than PRICE * num");
    ...
    function airdrop(address[] calldata _addresses) external onlyRole(WHITE_LIST_ROLE) {
        uint256 supply = totalSupply();
        require(airdrop_left > _addresses.length, "Skillettt: Exceeds airdrop reserved Skilletts supply");
        require(supply + _addresses.length <= TOTAL_NUMBER_OF_SKILLETT, "Skillettt: Exceeds maximum Skilletts supply");
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function safeTransferFrom(
        ...
        uint256 tokenId
                ...
                console.log("Transfer From: %s, To: %s, TID: %s", from, to, tokenId);
