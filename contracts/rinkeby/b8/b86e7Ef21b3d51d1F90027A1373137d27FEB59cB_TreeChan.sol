...
contract TreeChan is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIds;
    string public websiteURL = "https://trevoraron.github.io/usdc-frontend/";
    uint256 public immutable maxNodes = 10000;
    uint256 public immutable mintFeeWei = 2e16;
    ...
    modifier paysFee() {
        require(msg.value >= mintFeeWei, "fee too low");
        _;
    }

    modifier stillCanMint() {
        require(_tokenIds.current() <= maxNodes, "fully minted");
        _;
    }

    constructor() ERC721("Tree Chan Node", "TCN") Ownable() {}

    function setWebsiteURL(string calldata _url) public onlyOwner {
        websiteURL = _url;
    }

    function newThread(string memory _message)
        public
        payable
        stillCanMint()
        paysFee()
                ...
                bytes(
                    string(
                        abi.encodePacked(
                            '{"name": "Post #',
                            toString(tokenId),
                            '", "description": "tree-chan is an experimantal on chain messaging produ t", "image": "data:image/svg+xml;base64,',
