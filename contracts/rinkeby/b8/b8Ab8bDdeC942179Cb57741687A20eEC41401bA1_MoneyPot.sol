...
contract MoneyPot {
    ...
    uint256 totalRewardClaimable; // total number of rewards sended to the money pot
    ...
    constructor (address _sDFIANCE, address _rewardToken, address _feeManager, address _papr) public {
        ...
        globalAverageSwapTime = block.timestamp;
        virtualLiquidityPool.nb_RewardToken = 1;
        virtualLiquidityPool.nb_sDFIANCE = 1;
    }
    ...
    function giftVirtualPool(uint256 _giftAmount) internal {
        totalRewardClaimable += _giftAmount;
        virtualLiquidityPool.nb_RewardToken += _giftAmount;
    }
    ...
    */
    ...
    function totalRewardAllowedToClaim() public returns (uint256) {
        return totalRewardClaimable;
    }

    // get the total reward amount that the user can claim (used in the front-end and for harvest reward)
    function getPendingReward(address _account) public returns (uint256) {
        ...
        uint256 finalPercentageOfReward = globalRewardWithTotalBonus > 0 ? rewardWithBonus.div(globalRewardWithTotalBonus) : 0;
    ...
    function getTotalHoldingHoursShares(address _account) private view returns (uint256) {
        return I_sDFIANCE(sDFIANCE).getAvgSwapTimeInHours(_account).mul(ERC20(sDFIANCE).balanceOf(_account));
    }
