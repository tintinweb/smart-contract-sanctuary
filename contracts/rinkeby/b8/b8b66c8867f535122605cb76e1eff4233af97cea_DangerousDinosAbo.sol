...
contract DangerousDinosAbo is ERC721Enumerable, Ownable {
  ...
  uint256 private maxGSDReserveCount = 333;
  uint256 private reservedGSDCount = 0;
  address private GSDAddress = 0x68759F9B5f923893F48dD1cBcb6fF90Ba55D9bB5;
  ...
  constructor(string memory baseURI) ERC721("Dangerous Dinos Abo", "DD") {
    setBaseURI(baseURI);
  }
  ...
  function reserveGSDNft() public onlyAuthorized {
    require(reservedGSDCount <= maxGSDReserveCount, "Max GSD Reserves taken already!");
    uint256 supply = totalSupply();
    uint256 i;

    for (i = 0; i < 333; i++) {
      emit AssetMinted(supply + i, GSDAddress);
      _safeMint(GSDAddress, supply + i);
      reservedGSDCount++;
    }
  }
