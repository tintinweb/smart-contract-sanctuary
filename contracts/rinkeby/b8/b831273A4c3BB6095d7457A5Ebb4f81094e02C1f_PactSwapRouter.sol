...
contract PactSwapRouter is IPactSwapRouter {
    ...
    function getPairFromFactory(address tokenA, address tokenB) internal view returns (address) {
        address pair = IPactSwapFactory(_factory).getPair(tokenA, tokenB);
        require(pair != address(0), 'UniswapV2Router: only existing pair allowed');
        return pair;
    }
    ...
    function addLiquidity(
        ...
        address pair = getPairFromFactory(tokenA, tokenB);
    ...
    function addLiquidityETH(
        ...
        address pair = getPairFromFactory(token, _WETH);
    ...
    modifier onlyGovernance() {
        require(IPactSwapFactory(_factory).governance() == msg.sender, "Governance: caller is not the governance");
        _;
    }
    ...
    function removeIncentivesPoolLiquidity(
        ...
        uint amountPactMin,
        uint deadline
    ) public virtual override ensure(deadline) onlyGovernance() returns (uint amountToken, uint amountPact) {
        IPactSwapFactory factory_ = IPactSwapFactory(factory());
        IUniswapV2Pair pair = IUniswapV2Pair(getPairFromFactory(token, factory_.requiredToken()));

        (amountToken, amountPact) = removeLiquidity(
            token,
            factory_.requiredToken(),
            pair.balanceOf(factory_.incentivesPool()),
            amountTokenMin,
            amountPactMin,
            address(this),
            deadline
        );
        TransferHelper.safeTransfer(factory_.requiredToken(), factory_.incentivesPool(), amountPact);

        if (token == _WETH) {
            IWETH(_WETH).withdraw(amountToken);
            TransferHelper.safeTransferETH(factory_.governance(), amountToken);
        } else {
            TransferHelper.safeTransfer(token, factory_.governance(), amountToken);
        }
    }
}
...
interface IPactSwapRouter is IUniswapV2Router01 {
    function removeIncentivesPoolLiquidity(
        address token,
        uint amountTokenMin,
        uint amountPactMin,
...
library UniswapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'81a9995806f51951a54c2e15e923a37ba383f87034fa9f028f9274983576e582' // init code hash
            ))));
    }
