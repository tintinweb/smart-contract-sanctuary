    ...
    * Modifiers */
    ...
    constructor(uint256 _supply,address _NFTAddr, uint256 _tokenId) ERC20("Fusible V0", "FT V0") {
        ...
        _mint(_msgSender(), _supply);
         bytes32 receiptId = sha256( abi.encodePacked(tx.origin,_NFTAddr,_tokenId,block.timestamp));// Replace this with the FT address.
    ...
    onlyReceiptOwner(_receiptId) receiptExist(_receiptId) 
       external   payable returns (uint tokenAmount, uint amountETH, uint liquidity){
        require(IERC20(address(this)).approve(AMM_ROUTEV2, type(uint256).max-1));//only uint256
     ...
     */
    function buyMintedTokens(bytes32 _receiptId) receiptExist(_receiptId)  external payable returns(uint256){
