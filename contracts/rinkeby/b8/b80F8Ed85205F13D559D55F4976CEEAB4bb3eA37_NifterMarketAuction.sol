...
contract NifterMarketAuction is
  Initializable,
  OwnableUpgradeable,
  AccessControlUpgradeable,
  Payments,
  INifterMarketAuction
{
  ...
  //erc1155 contract
  IERC1155Upgradeable public erc1155;
  ...
  mapping(address => uint256) public nifterBidBalance;
   ...
   */
  function __NifterMarketAuction_init(
    address _iMarketSettings,
    address _iERC1155CreatorRoyalty,
    address _nifter,
    address _nifterToken
  ) public initializer {
    __Ownable_init();
    __PullPayment_init();
    ...
    erc1155 = IERC1155Upgradeable(_nifter);
    nifterToken = IERC20Upgradeable(_nifterToken);
   ...
   * - only owner
   * - _address != address(0)
   * @param _nifter address of the IMarketplaceSettings.
   */
  function setData(
    address _nifter,
    address _royalty,
    ...
    nifter = INifter(_nifter);
    erc1155 = IERC1155Upgradeable(_nifter);
    iERC1155CreatorRoyalty = INifterRoyaltyRegistry(_royalty);
    nifterToken = IERC20Upgradeable(_token);
  ...
  function restore(
    address _oldAddress,
    address _oldNifterAddress,
    uint256 _startIndex,
    uint256 _endIndex
  ) external onlyOwner {
    NifterMarketAuction oldContract = NifterMarketAuction(payable(_oldAddress));
  ...
  function buy(
    ...
    require(nifter.getIsForSale(_tokenId, _owner) == true, "not sale");
    ...
    if (feeConfig == 2 || feeConfig == 3)
      return
        salePrice[_tokenId][_owner].amount.add(
          salePrice[_tokenId][_owner].amount.mul(nifter.getServiceFee(_tokenId)).div(100)
        );
  ...
  function setInitialBidPriceWithRange(
    ...
    activeBid[_tokenId][_owner] = ActiveBid(payable(_owner), nifter.getServiceFee(_tokenId), _bidAmount, _paymentMode);
  ...
  function bid(
    ...
    if (_paymentMode == 0) {
      uint256 amount = feeConfig == 2 || feeConfig == 3
        ? _newBidAmount.add(_newBidAmount.mul(nifter.getServiceFee(_tokenId)).div(100))
        : _newBidAmount;
      require(msg.value >= amount, "no correct price");
    }
    require(nifter.getIsForSale(_tokenId, _owner) == true, "not for sale");
    //Check bid range
    uint8 priceType = nifter.getPriceType(_tokenId, _owner);
  ...
  function _transferNFT(
    ...
    nifter.setApprovalForAllByNMA(_fr, address(this), true);
  ...
  function acceptBid(uint256 _tokenId, address _owner) public {
    ...
    if (currentBid.paymentMode == 0) {
      ...
      nifterBidBalance[currentBid.bidder] = nifterBidBalance[currentBid.bidder].sub(currentBid.amount);
    }
    ...
    if (erc1155.balanceOf(_owner, _tokenId) == 0) {
      ...
      //remove from sale ifter accepting the bid
      nifter.removeFromSale(_tokenId, _owner);
    } else {
  ...
  function lazyMintingBid(
    ...
    nifter.addNewTokenAndSetThePriceWithIdAndMinter(
      _tokenAmount,
      true,
      _price,
      _priceType,
    ...
    // uint8 serviceFee = nifter.getServiceFee(_tokenId);
    if (_paymentMode == 0) {
      uint256 amount = feeConfig == 2 || feeConfig == 3
        ? _newBidAmount.add(_newBidAmount.mul(nifter.getServiceFee(_tokenId)).div(100))
        : _newBidAmount;
    ...
    nifter.setBid(_price, msg.sender, _tokenId, _creator);
  ...
  function lazyMintingBuy(
    ...
    nifter.setPrice(_price, _tokenId, _creator);
    ...
    if (erc1155.balanceOf(_creator, _tokenId) == 0) {
      ...
      nifter.removeFromSale(_tokenId, _creator);
    }
  }
  ...
  function _refundBid(uint256 _tokenId, address _owner) internal {
    ...
    if (currentBid.paymentMode == 0) {
      ...
      if (nifterBidBalance[currentBid.bidder] > 0) {
  ...
  function _setBid(
    ...
    // Set bid.
    activeBid[_tokenId][_owner] = ActiveBid(_bidder, nifter.getServiceFee(_tokenId), _amount, _paymentMode);
    //add bid balance
    if (_paymentMode == 0) {
      bidBalance[_bidder] = feeConfig == 2 || feeConfig == 3
        ? bidBalance[_bidder].add(_amount.add(_amount.mul(nifter.getServiceFee(_tokenId)).div(100)))
        : bidBalance[_bidder].add(_amount);
    } else {
      nifterBidBalance[_bidder] = nifterBidBalance[_bidder].add(_amount);
    }
  }
}
...
contract Payments is SendValueOrEscrow {
  ...
  IERC20Upgradeable public nifterToken;
  ...
  function safeTransfer(address payable _payee, uint256 _amount) internal {
    ...
    if (_amount > 0) {
      nifterToken.safeTransfer(_payee, _amount);
    }
  }
  ...
  function safeTransferFrom(
    ...
    nifterToken.safeTransferFrom(_from, _to, _amount);
  }
  ...
  function payout(
    ...
    if (_paymentMode == 0) {
      ...
      // if (payments[0] > 0) {
      //   // SendValueOrEscrow.sendValueOrEscrow(_marketplacePayee, payments[0]);
      //   nifterToken.safeTransfer(_marketplacePayee, payments[0]);
      // }

      // royaltyPayment
      if (payments[1] > 0) {
        // SendValueOrEscrow.sendValueOrEscrow(_royaltyPayee, payments[1]);
        nifterToken.safeTransfer(_royaltyPayee, payments[1]);
      }
      // primarySalePayment
      if (payments[2] > 0) {
        // SendValueOrEscrow.sendValueOrEscrow(_primarySalePayee, payments[2]);
        nifterToken.safeTransfer(_marketplacePayee, payments[2]);
      }
      // payeePayment
      if (payments[3] > 0) {
        // SendValueOrEscrow.sendValueOrEscrow(_payee, payments[3]);
        nifterToken.safeTransfer(_payee, payments[3]);
      }
    }
  }
