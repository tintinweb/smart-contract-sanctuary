...
contract MyGovernor is Governor, GovernorCountingSimple {
    ...
    function setDelay(uint _delay) public onlyGovernance {
        delay = _delay;
    }
    
    function setPeriod(uint _period) public onlyGovernance {
        period = _period;
    }
    
    function setMinVotes(uint _minVotes) public onlyGovernance {
        minVotes = _minVotes;
    }

    function proposeBlacklist(address _target) public returns(uint) {
        address[] memory targets = new address[](1);
        uint256[] memory values = new uint256[](1);
        bytes[] memory calldatas = new bytes[](1);
        string memory description = 'Blacklist this address';
        targets[0] = _target;
        values[0] = 0;
        calldatas[0] = abi.encodePacked(uint(0));
    ...
    function totalVotes(uint256 proposalId)
        public
        view
        virtual
        returns (uint)
    {
        (uint againstVotes, uint forVotes, uint abstainVotes) = proposalVotes(proposalId);
    ...
    function _quorumReached(uint256 proposalId) internal view virtual override(Governor, GovernorCountingSimple) returns (bool) {
        // (, uint forVotes, ) = proposalVotes(proposalId);
    ...
    function _voteSucceeded(uint256 proposalId) internal view virtual override(Governor, GovernorCountingSimple) returns (bool) {
        require(totalVotes(proposalId) >= minVotes, 'Not enough votes');
        (, uint forVotes, ) = proposalVotes(proposalId);
        return forVotes > totalVotes(proposalId) / 2;
    }
    
    function executeBlacklist(address _target) public returns(uint) {
        address[] memory targets = new address[](1);
        uint256[] memory values = new uint256[](1);
        bytes[] memory calldatas = new bytes[](1);
        bytes32 descriptionHash = keccak256(bytes('Blacklist this address'));
        targets[0] = _target;
        values[0] = 0;
        calldatas[0] = abi.encodePacked(uint(0));
