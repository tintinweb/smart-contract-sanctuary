...
contract MovieVotingMasterChef is BasicMetaTransaction, AccessControl {
        ...
        isInitialized
    {
        ...
        uint256 pending =
            user.amount.mul(pool.accStarsPerShare).div(ACC_STARS_PRECISION).sub(
                user.rewardDebt
            );
        ...
        user.penalizedRewards = user.penalizedRewards.sub(_penalizedRewards);
    ...
    function withdrawPartial(
        ...
        user.penalizedRewards = user.penalizedRewards.add(
            pending.mul(oldUserAmount.sub(_amount)).div(oldUserAmount)
        );
