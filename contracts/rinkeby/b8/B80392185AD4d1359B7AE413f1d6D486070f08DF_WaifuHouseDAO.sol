...
contract WaifuHouseDAO is Delegated, ERC721EnumerableM, PaymentSplitter {
  ...
  constructor()
    ERC721EnumerableM("Wiafu House DAO", "WAIFU", 1, TOTAL_SUPPLY)
    PaymentSplitter(addressList,shareList){
    addGroup(350, 6999, 1 ether, 1 ether, 1640390400, 1640390400);
    addGroup(150, 2000, 2 ether, 2 ether, 1640390400, 1640390400);
    addGroup(100, 1000, 3 ether, 3 ether, 1640390400, 1640390400);
  }
  ...
  function mint(uint id, uint quantity) external payable {
    ...
    require(group.balance + quantity <= group.supply, "Quantity too big" );


    uint price = getPrice( id, block.timestamp );
    require( msg.value >= price * quantity, "Not enough ETH sent" );
    for( uint i; i < quantity; ++i ){
      _safeMint( msg.sender, group.balance++, "" );
    }
  }


  //delegated
  function mintTo(uint[] calldata groupIds, uint[] calldata quantities, address[] calldata recipients ) external payable onlyDelegates{
    require(groupIds.length == quantities.length,   "Must provide equal groupIds and quantities" );
    require(quantities.length == recipients.length, "Must provide equal quantities and recipients" );
    for(uint i; i < groupIds.length; ++i ){
      Group storage group = groups[ groupIds[i] ];
      require(group.balance + quantities[i] <= group.supply, "Quantity too big" );

      for( uint j; j < quantities[i]; ++j ){
        _safeMint( recipients[i], group.balance++, "" );
      }
    }
  }
...
import "../Blimpie/IBatch.sol";

abstract contract ERC721EnumerableM is Context, ERC165, IBatch, IERC721, IERC721Enumerable, IERC721Metadata {
    ...
    constructor(string memory name_, string memory symbol_, uint offset, uint quantity_) {
        _name = name_;
        _symbol = symbol_;
        _offset = offset;
        _quantity = quantity_;
