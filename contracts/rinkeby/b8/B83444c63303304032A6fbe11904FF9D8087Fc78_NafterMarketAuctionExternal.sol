  ...
  IERC721ReceiverUpgradeable
{
  ...
  struct TokenInfo {
    ...
    uint8 nftType; // 0 for erc1155 and 1 for erc721
  }
  ...
  function onERC1155Received(
    ...
    _addTokenToNafterMarket(_owner, _tokenId, _amount, 0);
  ...
  function onERC721Received(
    ...
    _addTokenToNafterMarket(_owner, _tokenId, 1, 1);
  ...
  function buy(
    ...
    require(tokenOwnerInfo[_tokenId][_owner].priceType == 0, "only fixed sale");
    require(tokenOwnerInfo[_tokenId][_owner].isForSale == true, "not sale");

    // SalePrice memory sp = salePrice[_tokenId][_owner];
    require(salePrice[_tokenId][_owner].paymentMode == _paymentMode, "wrong payment mode");
    ...
    _transferNFT(address(this), msg.sender, _tokenId, 1, _contractAddress, tokenInfo[_tokenId].nftType);
    tokenInContract[_tokenId][_owner] = tokenInContract[_tokenId][_owner] - 1;

    if (_paymentMode == 1) {
      Payments.safeTransferFrom(msg.sender, address(this), salePrice[_tokenId][_owner].amount);
    }
    Payments.payout(
      salePrice[_tokenId][_owner].amount,
      !iMarketplaceSettings.hasTokenSold(_tokenId),
      tokenInfo[_tokenId].serviceFee,
    ...
    if (tokenInContract[_tokenId][_owner] == 0) {
      // Wipe the token price.
      _resetTokenPrice(_tokenId, _owner);
      // nafter.removeFromSale(_tokenId, _owner);
    }

    emit Sold(msg.sender, _owner, salePrice[_tokenId][_owner].amount, _tokenId);
  }
    ...
    if (feeConfig == 2 || feeConfig == 3)
      return
        salePrice[_tokenId][_owner].amount.add(
          salePrice[_tokenId][_owner].amount.mul(tokenInfo[_tokenId].serviceFee).div(100)
        );
  ...
  function bid(
    ...
    if (_paymentMode == 0) {
      uint256 amount = feeConfig == 2 || feeConfig == 3
        ? _newBidAmount.add(_newBidAmount.mul(tokenInfo[_tokenId].serviceFee).div(100))
        : _newBidAmount;
      require(msg.value >= amount, "no correct price");
    }
    // (bool isForSale, uint8 priceType, , , ) = nafter.getTokenOwnerInfo(_tokenId, _owner);
    require(tokenInContract[_tokenId][_owner] > 0, "no balance");
    require(tokenOwnerInfo[_tokenId][_owner].isForSale == true, "not for sale");

    uint8 priceType = tokenOwnerInfo[_tokenId][_owner].priceType;
    ...
    _setOwnerBid(_newBidAmount, msg.sender, _tokenId, _owner);
   ...
   * @param _contractAddress address of erc1155 or erc721
   */
  ...
  function acceptBid(
    ...
    // Transfer token.
    _transferNFT(address(this), currentBid.bidder, _tokenId, 1, _contractAddress, tokenInfo[_tokenId].nftType);
    tokenInContract[_tokenId][_owner] = tokenInContract[_tokenId][_owner] - 1;
  ...
  // /**
  //  * @dev Deletes the token in case
  //  * @param _tokenId uint256 ID of the token.
  //  * @param _amount amount of the token to delete
  //  */
  // function deleteToken(uint256 _tokenId, uint256 _amount) public {
  //   senderMustBeTokenOwner(_tokenId);
  //   uint256 balance = tokenInContract[_tokenId][msg.sender];
  //   if (
  //     (activeBid[_tokenId][msg.sender].bidder == msg.sender || activeBid[_tokenId][msg.sender].bidder == address(0)) ==
  //     false
  //   ) require(balance - _amount > 0, "active bid");
  //   // nafter.burn(address(this), _tokenId, _amount);
  // }
  ...
  //  * @param _amount amount of the token to delete
  //  */
  // function deleteNonContractToken(uint256 _tokenId, uint256 _amount) public {
  //   senderMustBeTokenOwner(_tokenId);
  //   // nafter.burn(msg.sender, _tokenId, _amount);
  // }
  ...
  function cancelBid(uint256 _tokenId, address _owner) external {
    ...
    require(activeBid[_tokenId][_owner].bidder == msg.sender, "cant cancel");
    _refundBid(_tokenId, _owner);
   ...
   * @param _nftType type of the NFT contract 0 for ERC1155 and 1 for ERC721
   */
  function putOnSale(
    uint8 _priceType,
    uint256 _price,
    ...
    senderMustBeTokenOwner(_tokenId);
    require(activeBid[_tokenId][msg.sender].bidder == address(0), "bid");

    _setPrices(msg.sender, _priceType, _price, _startTime, _endTime, _tokenId, _paymentMode);
  }
   ...
   * @param _contractAddress address of NFT Contract
   * @param _nftType type of NFT Contract 0 for ERC1155 and 1 for ERC721
   */
  function removeFromSale(uint256 _tokenId, uint256 _amount, address _contractAddress, uint8 _nftType) external {
  ...
  function _setPrices(
    ...
    if (_priceType == 0) {
      salePrice[_tokenId][_owner] = SalePrice(payable(_owner), _price, _paymentMode);
    } else if (_priceType == 1 || _priceType == 2) {
      activeBid[_tokenId][_owner] = ActiveBid(payable(_owner), tokenInfo[_tokenId].serviceFee, _price, _paymentMode);
      activeBidRange[_tokenId][_owner] = ActiveBidRange(_startTime, _endTime);
    }
    _setPrice(_price, _priceType, _tokenId, _owner);
  }
  ...
  function _setBid(
    ...
    // Set bid.
    activeBid[_tokenId][_owner] = ActiveBid(_bidder, tokenInfo[_tokenId].serviceFee, _amount, _paymentMode);
    // add bid balance
    if (_paymentMode == 0) {
      bidBalance[_bidder] = feeConfig == 2 || feeConfig == 3
        ? bidBalance[_bidder].add(_amount.add(_amount.mul(tokenInfo[_tokenId].serviceFee).div(100)))
  ...
  function _addTokenToNafterMarket(
    ...
    tokenOwnerInfo[_tokenId][_owner] = TokenOwnerInfo(false, 0, new uint256[](0), new uint256[](0), new address[](0));
  }
}
