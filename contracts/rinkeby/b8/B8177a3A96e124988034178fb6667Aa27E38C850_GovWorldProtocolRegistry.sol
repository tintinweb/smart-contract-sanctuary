...
contract GovWorldProtocolRegistry is GovWorldProtocolBase {
    ...
    //         "GovProtocolRegistry: No admin right to update unearned apy fee"
    //     );
    //     _;
    ...
    modifier onlyEditFeeRole(address admin) {
        require(
            govAdminRegistry.isSuperAdminAccess(admin) == true,
    ...
    */
     ...
     */
    function setUnearnedAPYPercentageForLender(uint _percentage) public onlyEditFeeRole(msg.sender) {
        require(_percentage <= 10000 && _percentage > 0, "GPL: Gov Percentage Error");
        lenderUnearndedAPYPercentage = _percentage;
    }

    function setpercentageforAdminWallet(uint _percentage) public onlyEditFeeRole(msg.sender) {
        require(_percentage <= 10000 && _percentage > 0, "GPL: Gov Percentage Error");
     ...
     */
    function setGovPlatfromFee(uint256 _percentage) public onlyEditFeeRole(msg.sender) {
        require(_percentage <= 10000 && _percentage > 0, "GPL: Gov Percentage Error");
        govPlatformFee = _percentage;
    }

    function setThresholdFee(uint256 _percentage) public onlyEditFeeRole(msg.sender) {
        require(_percentage <= 10000 && _percentage > 0, "GPL: Gov Percentage Error");
        govThresholdFee = _percentage;
    }

    function setAutosellFee(uint256 _percentage) public onlyEditFeeRole(msg.sender) {
        require(_percentage <= 10000 && _percentage > 0, "GPL: Gov Percentage Error");
        govAutosellFee = _percentage;
    }

    function setfeeReceiverAdminWallet(address _newWallet) public onlyEditFeeRole(msg.sender) {
    ...
    function getAdminWalletPercentage() external view override returns(uint256) {
...
abstract contract GovWorldProtocolBase is IGovWorldProtocolRegistry, Ownable{
        ...
        internal
    {
        ...
        if(marketData.isSP){
            ...
            string memory gTokenName = string(abi.encodePacked("gov", spToken.name()));
            string memory gTokenSymbol = string(abi.encodePacked("gov", spToken.symbol()));
            ...
            approvedTokens[_tokenAddress] = Market(marketData.dexRouter ,marketData.isSP, marketData.isReversedLoan, marketData.tokenLimitPerReverseLoan, address(0x0), false);
        }
        // Market memory _marketData = Market(
        // approvedTokens[_tokenAddress].isSP,    
        ...
        emit TokensAdded(_tokenAddress, approvedTokens[_tokenAddress].isSP,    
        approvedTokens[_tokenAddress].isReversedLoan,
        approvedTokens[_tokenAddress].tokenLimitPerReverseLoan,
        approvedTokens[_tokenAddress].gToken); 
        ...
        internal
    {
        ...
        if(_marketData.isSP){
            approvedTokens[_tokenAddress] = Market(_marketData.dexRouter, _marketData.isSP, _marketData.isReversedLoan, _marketData.tokenLimitPerReverseLoan, oldMarketData.gToken, _marketData.isMint);   
        } else {
            approvedTokens[_tokenAddress] = Market(_marketData.dexRouter, _marketData.isSP, _marketData.isReversedLoan, _marketData.tokenLimitPerReverseLoan, address(0x0), false);
        }
    }
