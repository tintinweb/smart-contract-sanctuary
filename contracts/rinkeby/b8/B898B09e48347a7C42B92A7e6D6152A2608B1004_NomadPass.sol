...
contract NomadPass is ERC1155, AccessControl, Pausable, ERC1155Burnable, ERC1155Supply, EIP712 {
    ...
    event MaxItemChanged(uint256 tokenId, uint256 newMaxItem);
    ...
    onlyRole(OPERATOR_ROLE) {
        ...
        require(open.length == numPasses, "INVALID_REDEEMS_OPEN_ARRAY_SIZE");
        require(close.length == numPasses, "INVALID_REDEEMS_CLOSE_ARRAY_SIZE");

        for (uint256 i = 0; i < numPasses; i++) {

            require(open[i] <= close[i], "INVALID_REDEEM_DATES");
    ...
    /// @dev After doing the setup, this method can be used to modify the sale dates. This dates are used in the redeemPass method
    ...
    onlyRole(OPERATOR_ROLE)
    {
        ...
        require(open.length == ids.length, "INVALID_REDEEMS_OPEN_ARRAY_SIZE");
        require(close.length == ids.length, "INVALID_REDEEMS_CLOSE_ARRAY_SIZE");

        for (uint256 i = 0; i < ids.length; i++) {
            require(open[i] <= close[i], "INVALID_REDEEM_DATES");
            Pass storage pass = passes[i];
    ...
    {
        require(ids.length > 0, "INVALID_TOKENS_ARRAY_SIZE");
        require(prices.length == ids.length, "INVALID_REDEEM_PRICES_ARRAY_SIZE");
    ...
    onlyRole(OPERATOR_ROLE)
    {
        ...
        require(pass.maxItems > 0, "TOKEN_ID_NOT_EXIST");
        pass.maxItems = maxItems;
        emit MaxItemChanged(id, maxItems);
    }
    ...
    payable
    {
        ...
        if(pass.maxPerWallet > 0){
            require(balanceOf(msg.sender, passId) <= pass.maxPerWallet, "INVALID_MAX_PER_WALLET");
        }
