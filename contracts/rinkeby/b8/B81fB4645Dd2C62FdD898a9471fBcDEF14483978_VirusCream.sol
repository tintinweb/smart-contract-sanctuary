    ...
    ERC721Pausable
{
    ...
    event recastedEvent(address indexed to, uint oldTokenId, uint newTokenId);

    uint256 public mintFee = 0.02 ether;
    uint256 public recastFee = 0.018 ether;
    uint8 public recastFeeRate = 9;
    uint8 public recastFeeBase = 10;
    ...
    function recast(
        ...
        payable
    {
        require(msg.value >= recastFee, "Recast Fee is not enough");
        burn(tokenId);
        uint currentId = _tokenIdTracker.current();
        _mint(msg.sender, currentId);
        _tokenIdTracker.increment();
        _setTokenURI(currentId, uri);
        _pay();
        emit recastedEvent(msg.sender, tokenId, currentId);
    }

    function setContractURILocation(
        string memory uri
    )
        external
        onlyOwner
    ...
    function setRecastFeeRateAndBase(
        uint8 rate,
        uint8 base
    )
        external
        onlyOwner
    {
        require(rate > 0, "Recast fee rate must be greater than 0");
        require(base > 0, "Recast fee base must be greater than 0");
        require(base >= rate, "Recast fee base must be greater than or equal to rate");
        recastFeeRate = rate;
        recastFeeBase = base;
        recastFee = mintFee.mul(recastFeeRate).div(recastFeeBase);
    }
