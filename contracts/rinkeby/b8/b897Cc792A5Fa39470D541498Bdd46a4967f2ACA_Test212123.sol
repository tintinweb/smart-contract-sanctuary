...
contract Test212123 is ERC721Enumerable, Ownable, ReentrancyGuard {
  using Strings for uint256;
  using Counters for Counters.Counter;
  Counters.Counter private _mintIds;
  ...
  string private _tokenId = "TEST";
  string private _baseTokenURI = "";

  // Addresses
  address private mint_address = 0x89DA91eaD9e61C85d95B088BdE49A1Ea6D307536;
  address private withdraw_address = 0x89DA91eaD9e61C85d95B088BdE49A1Ea6D307536;

  // Collection params
  uint256 public constant TOT = 10;
  ...
  modifier onlyIfAvailable(uint256 _qty) {
    ...
    require(_claimIds.current() < TOT, "Collection is sold out");
    require(_claimIds.current() + _qty <= TOT, "Not enough NFTs available");
  ...
  function claim(uint256 _qty) external payable nonReentrant onlyIfAvailable(_qty){
      if(status == MintStatus.PRESALE){
          ...
          require(_presaleClaimed[msg.sender] + _qty <= MAX_PER_STATUS[uint256(status)], "You already claimed your presale");
      }
      for (uint i = 0; i < _qty; i++) {
          _claim();
      }
  ...
  function _claim() private {
      ...
      require(ownerOf(_claimIds.current() + 1) == mint_address, "Already claimed");
      _claimIds.increment();
      _safeTransfer(mint_address, msg.sender, _claimIds.current(), "");
  }
  ...
  function getStatus() external view returns(string memory status_, uint qty_, uint price_, string memory msg_){
      if(status == MintStatus.CLOSED){
          return ("CLOSED", MAX_PER_STATUS[uint256(status)], PRICE, "Minting is closed");
          ...
          if(_presaleList[msg.sender] == true) {
            if(_presaleClaimed[msg.sender] <= MAX_PER_STATUS[uint256(status)] ) {
                  return ("PRESALE", MAX_PER_STATUS[uint256(status)] - _presaleClaimed[msg.sender], PRICE, "You are in presale");
              } else {
                  return( "PRESALE", 0, PRICE, "You claimed your presale");
              }
          } else {
              return ("PRESALE", 0, PRICE, "You are not in presale");
          }
      } else {
          return ("PUBLIC", MAX_PER_STATUS[uint256(status)], PRICE, "Public sale");
      }
  }
