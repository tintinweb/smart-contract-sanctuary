...
contract DogMoon is ERC721, ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint256 public fomoDogWhiteListPrice = 0.08 ether;
  ...
  function preserveMint(uint numDogMoons, address to) public onlyOwner {
    require(totalSupply() + numDogMoons <= tierSupply, 'Preserve mint would exceed tier supply');
    require(totalSupply() + numDogMoons <= MAX_SUPPLY, 'Preserve mint would exceed max supply');
    _mintDogMoon(numDogMoons, to);
  ...
  function getDogMoonByOwner(address _owner) public view returns (uint256[] memory) {
  ...
  function mintDogMoons(uint numDogMoons) public payable {
    require(_isSaleActive, 'Sale must be active to mint DogMoons');
    require(totalSupply() + numDogMoons <= tierSupply, 'Sale would exceed tier supply');
    require(totalSupply() + numDogMoons <= MAX_SUPPLY, 'Sale would exceed max supply');
    require(balanceOf(msg.sender) + numDogMoons <= maxBalance, 'Sale would exceed max balance');
    require(numDogMoons <= maxMint, 'Sale would exceed max mint');
    require(numDogMoons * mintPrice <= msg.value, 'Not enough ether sent');
    _mintDogMoon(numDogMoons, msg.sender);
    emit TokenMinted(totalSupply());
  }

  function whiteListMintDogMoons(uint numDogMoons) public payable {
    require(_isWhiteListSaleActive, 'Sale must be active to mint DogMoons');
    require(totalSupply() + numDogMoons <= tierSupply, 'Sale would exceed tier supply');
    require(totalSupply() + numDogMoons <= MAX_SUPPLY, 'Sale would exceed max supply');
    require(balanceOf(msg.sender) + numDogMoons <= maxBalance, 'Sale would exceed max balance');
    require(numDogMoons <= maxMint, 'Sale would exceed max mint');
    ...
    require(numDogMoons * price <= msg.value, 'Not enough ether sent');
    _mintDogMoon(numDogMoons, msg.sender);
    emit TokenMinted(totalSupply());
  }

  function auctionMintDogMoon(uint numDogMoons) public payable {
    require(_isAuctionActive, 'Auction must be active to mint DogMoons');
    require(block.timestamp >= auctionStartTime, 'Auction not start');
    require(totalSupply() + numDogMoons <= tierSupply, 'Auction would exceed tier supply');
    require(totalSupply() + numDogMoons <= MAX_SUPPLY, 'Auction would exceed max supply');
    require(balanceOf(msg.sender) + numDogMoons <= maxBalance, 'Auction would exceed max balance');
    require(numDogMoons <= maxMint, 'Auction would exceed max mint');
    require(numDogMoons * getAuctionPrice() <= msg.value, 'Not enough ether sent');
    _mintDogMoon(numDogMoons, msg.sender);
    emit TokenMinted(totalSupply());
  }

  function _mintDogMoon(uint256 numDogMoons, address recipient) internal {
    uint256 supply = totalSupply();
    for (uint256 i = 0; i < numDogMoons; i++) {
