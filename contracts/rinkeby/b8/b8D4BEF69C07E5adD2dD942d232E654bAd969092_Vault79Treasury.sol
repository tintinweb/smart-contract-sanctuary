...
interface IPaxgBondDepository {
    function assetPrice() external pure returns (uint);
}
...
contract Vault79Treasury is Ownable {
    ...
    function allocatedRate() public view returns ( uint ) {
        ...
        uint paxgReserve = IPaxgBondDepository( reserveDepositors[reserveDepositors.length - 1] ).assetPrice().mul(
            valueOf( reserveTokens[ 0 ], IERC20( reserveTokens[ 0 ] ).balanceOf( address(this) ) ) )/ 1e8;
