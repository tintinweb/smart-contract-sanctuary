...
contract RedeemPool is Operator {
    ...
    function startRedeem() public onlyOperator {
        uint256 rewardAll = IERC20(cash).balanceOf(address(this)).sub(cashToClaim);
        ...
        cashToClaim = cashToClaim.add(cashToClaim);
        history.push(RedeemSnapshot({
    ...
    function _claimRewad(address _director) internal updateReward(_director) {
        ...
        if (seat.rewardEarned == 0) {
            return;
        }
        cashToClaim = cashToClaim.sub(seat.rewardEarned);
        IERC20(cash).safeTransfer(_director, seat.rewardEarned);
        seat.rewardEarned = 0;
        seats[_director] = seat;
    }

    function rechargeCash(uint256 _amount) public {
        IERC20(cash).safeTransferFrom(_msgSender(), address(this), _amount);
    }

    function recharge(address token, uint256 _amount) public {
    ...
    modifier updateReward(address _director) {
        ...
        if (seat.lastSnapshotIndex == lastIndex && history[lastIndex].end > block.timestamp){
                _;
                return;
        }
    ...
    function rewardEarned() public view returns (uint256) {
        Redeemseat memory seat = seats[_msgSender()];
        if (seat.deposit == 0) {
        ...
        return seat.rewardEarned.add(seat.deposit.mul(history[seat.lastSnapshotIndex].rewardPerBond));
    }
}
