...
contract ZooGame {
    ...
    uint256 roundPhase = 0; //0 is deactive, 1 is deposit, 2 is get price
    ...
    function addAnimal(address _token, address _lPTokenWithUSD)
        public
        onlyAdmin
    {
        require(!isAnimalExist(_token), "animal aready exist");
        require(roundPhase == 0, "round is running");
        animals.push(Animal({token: _token, lPTokenWithUSD: _lPTokenWithUSD}));
    }

    function isAnimalExist(address _token) public view returns (bool) {
        if (animals.length > 0) {
    ...
    function finishDeposit() public onlyAdmin {
        ...
        roundPhase = 2;
        for (uint256 i = 0; i < animals.length; i++) {
            currentRound.animals[animals[i].token].firstPrice = getTokenPrice(
                animals[i].lPTokenWithUSD,
                animals[i].token
            );
        }
    ...
    function endRound() public onlyAdmin {
        ...
        roundPhase = 0;
        if (
            block.timestamp >= currentRound.startTime.add(600) &&
            block.timestamp <= currentRound.startTime.add(900)
        ) {
            determineTheWinner();
            calFee();
        } else {
    ...
    function determineTheWinner() private {
        for (uint256 i = 0; i < animals.length; i++) {
            AnimalInRound storage animalInRound =
                currentRound.animals[animals[i].token];
            if (animalInRound.totalAmount > 0) {
                animalInRound.lastPrice = getTokenPrice(
                    animalInRound.animal.lPTokenWithUSD,
                    animalInRound.animal.token
                );
                if (animalInRound.lastPrice >= animalInRound.firstPrice) {
                    animalInRound.range = animalInRound
                        .lastPrice
                        .sub(animalInRound.firstPrice)
                        .div(animalInRound.firstPrice);
                } else {
                    animalInRound.range = animalInRound
                        .firstPrice
                        .sub(animalInRound.lastPrice)
                        .div(animalInRound.firstPrice);
                }
    ...
    function updateWiningAnimal(AnimalInRound memory _animal) private {
        if ((currentRound.winningPriceIncrease < _animal.range)) {
            currentRound.winningPriceIncrease = _animal.range;
            currentRound.winner = _animal.animal.token;
            currentRound.isDraw = false;
        } else if (currentRound.winningPriceIncrease == _animal.range) {
            currentRound.isDraw = true;
        }
    }

    function getTokenPrice(address _pairAddress, address _token)
        private
        view
        returns (uint256)
    {
        ...
        require(
            _token == token0Address || _token == token1Address,
            "pair address not match token"
        );
        ...
        if (!currentRound.isDraw) {
            for (uint256 i = 0; i < animals.length; i++) {
                AnimalInRound storage animalInRound =
                    currentRound.animals[animals[i].token];
                if (animalInRound.totalAmount > 0) {
                    remainingFee[animals[i].token] = remainingFee[
                        animals[i].token
                    ]
                        .add(animalInRound.totalAmount.div(20));
                }
            }
        }
    }
    ...
    function claimReward() public lock {
        require(animals.length > 0, "nothing to racing");
        uint256 totalRewardPening = 0;
        for (uint256 i = 0; i < animals.length; i++) {
            totalRewardPening = totalRewardPening.add(
                calRewardPending(msg.sender, animals[i])
            );
        }
        require(totalRewardPening > 0, "nothing to claim");
        IBEP20(zooToken).safeTransfer(msg.sender, totalRewardPening);
        delete usersSummary[msg.sender].unclaimRounds;
    }

    function calRewardPending(address _player, Animal memory _animal)
        private
        view
        returns (uint256)
    {
    ...
    function getRewardPendingInRound(
        uint256 _round,
        address _player,
        Animal memory _animal
    ) private view returns (uint256) {
        uint256 _totalPendingRound = 0;
        for (uint256 i = 0; i < userInfo[_player][_round].length; i++) {
            BidTicket storage _ticket = userInfo[_player][_round][i];
            if (
                historyRounds[_round].isDraw == false &&
                historyRounds[_round].winner == _ticket.bid
            ) {
                uint256 _allocPoint =
                    historyRounds[_round].animals[_ticket.bid].totalAmount.div(
                        _ticket.value
                    );
                uint256 _getRewardPendingInRound =
                    historyRounds[_round].animals[_animal.token]
                        .totalAmount
                        .div(_allocPoint);
                _getRewardPendingInRound = _getRewardPendingInRound.mul(95).div(
                    100
                );
                _totalPendingRound = _totalPendingRound.add(
                    _getRewardPendingInRound
                );
            } else if (
                historyRounds[_round].isDraw && _ticket.bid == _animal.token
            ) {
    ...
    function deposit(address _animalToken, uint256 _value) public lock {
        ...
        require(
            currentRound.animals[_animalToken].animal.token == _animalToken,
            "animal not exist"
        );
        IBEP20(zooToken).safeTransferFrom(msg.sender, address(this), _value);
        userInfo[msg.sender][round].push(
            // BidTicket(_animalToken, block.timestamp, _value)
            BidTicket({bid: _animalToken, time: block.timestamp, value: _value})
        );
    ...
    function withdrawalFee() public onlyAdmin {
        ...
        for (uint256 i = 0; i < animals.length; i++) {
            if (remainingFee[animals[i].token] > 0) {
                IBEP20(animals[i].token).safeTransfer(
                    adminAddress,
                    remainingFee[animals[i].token]
                );
