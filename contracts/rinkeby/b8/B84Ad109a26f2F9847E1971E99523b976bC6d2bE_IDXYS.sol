...
contract IDXYS {
  ...
  */
    ...
    function _withdrawFromComp(address _asset,address _cToken,uint256 _amount) internal {
        ...
        if(_asset == ETHER){                                                                    
            CEther cToken = CEther(_cToken);
            uint256[] memory TX = _Computation(_cToken,msg.value, pTokenAmount);
            cToken.approve(address(cToken), TX[1]);                        
            require(cToken.redeem(TX[1]) == 0, "CEther Withdraw?");
            ...
            payable(msg.sender).transfer(TX[2]);
            ...
            cToken.approve(address(cToken),TX[1]);                                    
            require(cToken.redeem(TX[1]) == 0, "CToken Withdraw?");
            balances[msg.sender][_asset] = balances[msg.sender][_asset].sub(_amount);
            pBalances[msg.sender][_cToken] = pBalances[msg.sender][_cToken].sub(pTokenAmount);  // we adjust the balance of both asset and left the 
            asset.transfer(msg.sender, TX[2]);
    ...
    function _Computation(address _asset, uint256 _amount, uint256 _pAmount) internal view returns (uint256[] memory ){
       ...
       //so for what he want we check the price expressed cDai 
        ...
        txData[1] =  withdrawn;                                      // the amount to redeem
        txData[2] = daiValue;                                         // the amount to send to user
                                                                  // the fees 
