...
contract TheDude is ERC721Enumerable, Ownable {
  using Strings for uint256;

  uint public constant avaliableAmount = 500;
  ...
  function claim(string memory name, uint salt) external {
      ...
      dudes[tokenId].id = dudeId;
      dudes[tokenId].name = name;
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
    ...
    return string(abi.encodePacked(_baseURI(), "/", dudes[_tokenId].id));
  }
