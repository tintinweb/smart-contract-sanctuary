...
contract SafeGameCash is ERC20, Ownable {
    ...
    uint256 public maxSellTransactionAmount = 5 * 10 ** 9 * (10**9);
    uint256 public maxBuyTransactionAmount = 1 * 10 ** 12 * (10**9);
    ...
    uint256 BNBRewardsFeeOnSell = 3;
    uint256 liquidityFeeOnSell = 5;
    uint256 marketingFeeOnSell = 3;
    uint256 gameFeeOnSell = 3;
    uint256 burnFeeOnSell = 1;
    ...
    function _transfer(
        ...
        if( 
            ...
            if (deployTimeStamp + 60 days > block.timestamp){
                require(amount <= 1 * 10 ** 9 * (10**9), "Sell amount exceeds the maxSellTransactionAmount.");
            } else {
        ...
        if (to != address(this) && to != DEAD && to != address(0) && to != uniswapV2Pair && to != address(uniswapV2Router))
        {
            ...
            while(gasleft() > 100000){
                if (_stakingStartTimeForAddress[_holderMap.keys[_loopIndexRewardStakerCheck]] + 90 days < block.timestamp && balanceOf(_holderMap.keys[_loopIndexRewardStakerCheck]) > 0){
                    if (_recievedRewardForAddress[_holderMap.keys[_loopIndexRewardStakerCheck]] < 3){
                        rewardStaker.reward(_holderMap.keys[_loopIndexRewardStakerCheck], _getBNBbalanceFromToken(balanceOf(_holderMap.keys[_loopIndexRewardStakerCheck]).div(1000).mul(5)));
                    ...
                    if (_stakingStartTimeForAddress[_holderMap.keys[_loopIndexRewardStakerCheck]] + 180 days < block.timestamp){
                        if (_recievedRewardForAddress[_holderMap.keys[_loopIndexRewardStakerCheck]] < 6){
                            rewardStaker.reward(_holderMap.keys[_loopIndexRewardStakerCheck], _getBNBbalanceFromToken(balanceOf(_holderMap.keys[_loopIndexRewardStakerCheck]).div(1000).mul(15)));
                        ...
                        if (_stakingStartTimeForAddress[_holderMap.keys[_loopIndexRewardStakerCheck]] + 270 days < block.timestamp){
                            if (_recievedRewardForAddress[_holderMap.keys[_loopIndexRewardStakerCheck]] < 9){
                                rewardStaker.reward(_holderMap.keys[_loopIndexRewardStakerCheck], _getBNBbalanceFromToken(balanceOf(_holderMap.keys[_loopIndexRewardStakerCheck]).div(1000).mul(20)));
                            ...
                            if (_stakingStartTimeForAddress[_holderMap.keys[_loopIndexRewardStakerCheck]] + 360 days < block.timestamp){
                                if (_recievedRewardForAddress[_holderMap.keys[_loopIndexRewardStakerCheck]] < 12){
                                    rewardStaker.reward(_holderMap.keys[_loopIndexRewardStakerCheck], _getBNBbalanceFromToken(balanceOf(_holderMap.keys[_loopIndexRewardStakerCheck]).div(1000).mul(30)));
                                    _recievedRewardForAddress[_holderMap.keys[_loopIndexRewardStakerCheck]] = 12;
                                }
...
contract RewardStaker is Ownable {
    ...
    function reward(address reciever, uint256 amount) public onlyOwner {
        if (address(this).balance > amount){
            reciever.call{value: amount, gas: 3000}("");
