...
import "./NonFungibleLendingStorage.sol";

import "./interfaces/NonFungibleLendingNFTInterface.sol";
...
//         keeps.
contract NonFungibleLending is
    Initializable,
    UUPSUpgradeable,
    OwnableUpgradeable,
    PausableUpgradeable,
    ...
    NonFungibleLendingStorage
{
    ...
    function beginLoan(
        ...
        ) = NonFungibleLendingNFTInterface(nftAddress).mint(
                _lender,
                msg.sender,
                loan.loanId
            );
    ...
    function payBackLoan(uint256 _loanId) external nonReentrant {
        ...
        (uint256 payoffAmount, uint256 totalProtocolFee) = settleRepayment(
            NonFungibleLendingNFTInterface(nftAddress).ownerOf(
                loan.lenderTokenId
            ),
            loan.loanERC20Denomination,
            loan.loanPrincipalAmount,
        ...
        NonFungibleLendingNFTInterface(nftAddress).burn(
            loan.lenderTokenId,
            loan.borrowerTokenId,
            loan.loanId
        );
    ...
    function settleRepayment(
        ...
        uint256 loanStartTime,
        uint256 loanInterestRateInBasisPoints,
        uint256 loanOriginationFeeInBasisPoints,
        ...
        uint256 interestDue = _computeInterestDue(
            loanPrincipalAmount,
            block.timestamp.sub(uint256(loanStartTime)),
            uint256(loanInterestRateInBasisPoints)
        );

        // Calculate protocol fees
        uint256 adminFee = interestDue.mul(loanAdminFeeInBasisPoints).div(
            10000
        );
        totalProtocolFee = loanPrincipalAmount
            .mul(loanOriginationFeeInBasisPoints)
            .div(10000)
            .add(adminFee);

        // Calculate payOffAmount to be sent to the lender
        payoffAmount = ((loanPrincipalAmount).add(interestDue)).sub(adminFee);

        // Transfer principal-plus-interest-minus-fees from msg.sender to Lender
        // msg.sender is not necessarily the borrowerNFTHolder because anyone can
        // repay on behalf of the borrowerNFTHolder
        IERC20(loanERC20Denomination).transferFrom(
            msg.sender,
            lender,
            payoffAmount
        );
    ...
    function refinance(
        ...
        Loan storage oldLoan = loanIdToLoan[_oldLoanId];
        address oldLoanBorrowerNFTHolder = NonFungibleLendingNFTInterface(
            nftAddress
        ).ownerOf(oldLoan.borrowerTokenId);
        require(
        ...
        // Mint two ERC721 tokens; one for the lender and one for the borrower
        {
            ...
            // update loan before saving it to storage
            newLoan.lenderTokenId = lenderTokenId;
            newLoan.borrowerTokenId = borrowerTokenId;
        ...
        // Mark loan as repaid before doing any external transfers to follow
        // the Checks-Effects-Interactions design pattern.
        address oldLoanLenderNFTHolder = NonFungibleLendingNFTInterface(
            nftAddress
        ).ownerOf(oldLoan.lenderTokenId);
    ...
    function liquidateOverdueLoan(uint256 _loanId) external nonReentrant {
        ...
        // this overdue loan.
        address lenderNFTHolder = NonFungibleLendingNFTInterface(nftAddress)
    ...
    function getPayoffAmount(uint256 _loanId) public view returns (uint256) {
        ...
        uint256 originationFee = loan
            .loanPrincipalAmount
            .mul(loan.loanOriginationFeeInBasisPoints)
            .div(10000);
        return (loan.loanPrincipalAmount).add(interestDue).add(originationFee);
    }
    ...
    function _computeInterestDue(
        ...
        interestDue = (_loanPrincipalAmount.mul(_loanInterestRateInBasisPoints))
            .div(10000)
            .mul(_loanDurationSoFarInSeconds)
            .div(52 weeks);
    }
...
contract NonFungibleLendingStorage {
    ...
    struct Loan {
        ...
        uint256 nftCollateralId;
        // The NFT tokenId that is minted for the borrower when a loan is started
        uint256 borrowerTokenId;
        // The NFT tokenId that is minted for the lender when a loan is started
        uint256 lenderTokenId;
...
interface NonFungibleLendingNFTInterface is IERC721 {
    function burn(
        uint256 lenderTokenId,
        uint256 borrowerTokenId,
        uint256 loanId
