...
contract DiggCollection is PoolTokenWrapper, Ownable, Pausable {
	...
	function quarticRoot(uint256 b) internal pure returns (uint256 result) {
		(uint256 mantissa, uint8 exponent) = PowerImplLib._power(b, 1e18, uint8(1), uint8(4));
		return mantissa.mul(uint256(1e18)) >> exponent;
	}
	...
	function earned(address account, uint256 pool) public view returns (uint256) {
		...
		uint256 rewardRate = quarticRoot((multiplyer.mul(poolBalance).mul(uint256(1e6)))).div(273220);
