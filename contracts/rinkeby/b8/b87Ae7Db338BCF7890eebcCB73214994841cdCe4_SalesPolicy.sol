...
contract SalesPolicy is ERC1155, ISalesPolicy, ReentrancyGuard {
    address public immutable SSIP;
    struct Policy {
        uint64 policyIdx;
        uint64 coverageDuration;
        uint64 coverStartAt;
        uint64 coverStartNumberAt;
        uint256 coverageAmount;
        uint256 policyPriceInUNO;
        uint256 policyPriceInUSDT;
    }

    uint256 public override totalPremiumSoldInUNO;
    uint256 public override totalUtilizedAmount;
    ...
    uint256 private constant MAX_INTEGER = type(uint256).max;

    mapping(address => Policy) public getPolicyPerUser;
    ...
    modifier onlySSIP() {
        require(msg.sender == SSIP, "UnoRe: SalesPolicy Forbidden");
        _;
    }
    ...
    function buyPolicy(
        ...
        (string memory protocolName, , ) = ISingleSidedInsurancePool(SSIP).getProtocolData(protocolIdx);
        address _signer = getSender(protocolName, _policyPrice, _coverageDuration, _coverageAmount, r, s, v);
        require(_signer != address(0) && _signer == msg.sender, "UnoRe: invalid signer");

        uint256 lastIdx = allPolicies.length > 0 ? allPolicies[allPolicies.length - 1] + 1 : 0;
        allPolicies.push(lastIdx);

        uint256 policyPriceInUNO = IExchangeAgent(exchangeAgent).getTokenAmountForUSDT(UNORE_TOKEN, _policyPrice);

        getPolicyPerUser[msg.sender] = Policy({
            coverageAmount: _coverageAmount,
            coverageDuration: uint64(_coverageDuration),
            coverStartAt: uint64(block.timestamp),
            coverStartNumberAt: uint64(block.number),
            policyPriceInUNO: policyPriceInUNO,
            policyPriceInUSDT: _policyPrice,
            policyIdx: uint64(lastIdx)
        });

        TransferHelper.safeTransferFrom(UNORE_TOKEN, msg.sender, premiumPool, policyPriceInUNO);
        _mint(msg.sender, lastIdx, _policyPrice, "");
        totalUtilizedAmount += _policyPrice;

        totalPremiumSoldInUNO += policyPriceInUNO;

        emit BuyPolicy(protocolIdx, lastIdx, msg.sender, _coverageAmount, policyPriceInUNO);
    }

    function setProtocolURI(string memory newURI) external override onlySSIP {
        _setURI(newURI);
        emit LogSetProtocolURIInPolicy(protocolIdx, address(this), newURI);
    }

    function setPremiumPool(address _premiumPool) external override onlySSIP {
        require(_premiumPool != address(0), "UnoRe: zero address");
        premiumPool = _premiumPool;
        emit LogSetPremiumPoolInPolicy(_premiumPool, address(this), protocolIdx);
    }

    function setExchangeAgent(address _exchangeAgent) external override onlySSIP {
    ...
    {
        Policy memory _policy = getPolicyPerUser[_user];
        return (
            _policy.coverageAmount,
            _policy.coverageDuration,
            _policy.coverStartAt,
            _policy.coverStartNumberAt,
            _policy.policyIdx
        );
    }
    ...
    function getSender(
        ...
        bytes32 msgHash = keccak256(abi.encodePacked(_productName, _policyPrice, _coverageDuration, _coverageAmount));
