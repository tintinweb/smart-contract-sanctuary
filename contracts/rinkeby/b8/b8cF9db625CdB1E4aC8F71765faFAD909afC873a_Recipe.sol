...
contract Recipe is ERC721, ReentrancyGuard, Ownable, ItemMetadata {
    ...
    function random(string memory input) private pure returns(uint) {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[1] = tokenName(weaponId(tokenId));

        parts[2] = '</text><text x="10" y="40" class="base">';

        parts[3] = tokenName(chestId(tokenId));

        parts[4] = '</text><text x="10" y="60" class="base">';

        parts[5] = tokenName(headId(tokenId));

        parts[6] = '</text><text x="10" y="80" class="base">';

        parts[7] = tokenName(waistId(tokenId));

        parts[8] = '</text><text x="10" y="100" class="base">';

        parts[9] = tokenName(footId(tokenId));

        parts[10] = '</text><text x="10" y="120" class="base">';

        parts[11] = tokenName(handId(tokenId));

        parts[12] = '</text><text x="10" y="140" class="base">';

        parts[13] = tokenName(neckId(tokenId));

        parts[14] = '</text><text x="10" y="160" class="base">';

        parts[15] = tokenName(ringId(tokenId));
