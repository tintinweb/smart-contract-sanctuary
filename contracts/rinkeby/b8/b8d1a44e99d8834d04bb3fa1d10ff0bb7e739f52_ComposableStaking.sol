...
contract ComposableStaking is Ownable {
    ...
    function toAgnostic( uint amount_ ) public view returns ( uint ) {
        return amount_.mul( IERC20( sOHM ).decimals() ).div( sOHMInterface( sOHM ).index() );
    }

    function fromAgnostic( uint amount_ ) public view returns ( uint ) {
        return amount_.mul( sOHMInterface( sOHM ).index() ).div( IERC20( sOHM ).decimals() );
    }
    ...
    function addExposureTo( address _asset, uint _amountSOHM, uint _minimumToReceive, address _reserveToUse, address _router ) external returns ( bool ) {
        ...
        ITreasury( treasury ).incurDebt( _amountSOHM, _reserveToUse ); // receives reserves of staked OHM

        IERC20( _reserveToUse ).approve( _router, _amountSOHM.mul( IERC20( _reserveToUse ).decimals() ).div( IERC20( sOHM ).decimals() ) ); 
    ...
    function removeExposureFrom( address _asset, uint _amountOfAsset, uint _minimumToReceive, address _reserveToUse, address _router ) external returns ( bool ) {
        ...
        if( _amountOut <= _userDebt ) {
            _updateStakeInfo( _asset, _amountOut, _amountOfAsset, false );
            IERC20( _reserveToUse ).approve( treasury, _amountOut ); 
            ITreasury( treasury ).repayDebtWithReserve( _amountOut, _reserveToUse );
            IERC20( sOHM ).transfer( msg.sender, _amountOut ); // transfers out sOHM
        } else if ( _amountOut > _userDebt && _userDebt != 0 ) {
            _updateStakeInfo( _asset, _userDebt, _amountOfAsset, false );
            IERC20( _reserveToUse ).approve( treasury, _amountOut ); 
            ITreasury( treasury ).repayDebtWithReserve( _userDebt, _reserveToUse );

            uint _amountRemaining = _amountOut.sub( _userDebt );
            ITreasury( treasury ).deposit( _amountRemaining, _reserveToUse, 0 );

            IERC20( sOHM ).transfer( msg.sender, _userDebt );
            IERC20( OHM ).transfer( msg.sender, _amountRemaining );
        } else {
            IERC20( _reserveToUse ).approve( treasury, _amountOut ); 
            ITreasury( treasury ).deposit( _amountOut, _reserveToUse, 0 );
            IERC20( OHM ).transfer( msg.sender, _amountOut );
        }
