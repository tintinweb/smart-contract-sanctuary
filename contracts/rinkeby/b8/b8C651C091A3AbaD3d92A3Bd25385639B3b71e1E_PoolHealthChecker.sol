...
contract PoolHealthChecker {
    ...
    */
    function poolAccountingInvariant(address pool_) external view returns (bool isMaintained_) {
        ...
        uint256 fdtTotalSupply = pool.totalSupply() * (10 ** liquidityAssetDecimals) / WAD;
        uint256 interestSum    = pool.interestSum();
        uint256 poolLosses     = pool.poolLosses();

        uint256 liquidityLockerBal = liquidityAsset.balanceOf(pool.liquidityLocker());
        uint256 principalOut       = pool.principalOut();

        return fdtTotalSupply + interestSum - poolLosses <= liquidityLockerBal + principalOut;
    }
