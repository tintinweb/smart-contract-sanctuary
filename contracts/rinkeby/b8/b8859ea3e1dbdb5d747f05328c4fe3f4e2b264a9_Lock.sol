...
contract Lock is BaseGovernanceWithUserUpgradable, ERC721Upgradeable {
    ...
    modifier onlyBeneficiaryManager() {
        require(hasRole(BENEFICIARY_MANAGER_ROLE, _msgSender()), "!beneficiaryManager");
        _;
    }
    ...
    function initialize(
        ...
        initializer 
        {
            ...
            __Lock_init(_schedule, _depositManager, _splitManager, _tokenERC20, _canAddBeneficiaries, _canRemoveBeneficiaries);
    }
    ...
    function __Lock_init(
        ...
        initializer 
        {
            ...
            grantRole(BENEFICIARY_MANAGER_ROLE, _msgSender());
    }
     ...
     * @param data ABI-encoded data of beneficiaries (merkle tree, arrays of addresses and amounts, etc - specific to IDepositManager)
    ...
    function addBeneficiaries(bytes calldata data, uint256 totalAmount) external onlyBeneficiaryManager {
        ...
        (address[] memory addresses, uint[] memory amounts) = 
            _decodeDataBeneficiearies(data);
        uint[] memory IDs = _setNFTs(addresses, addresses.length);
        depositManager.addDeposits(addresses, amounts, IDs, totalAmount);

        require(tokenERC20.transferFrom(_msgSender(), address(this), totalAmount), "ERROR: Transfer tokens to lock failed");
    }

    /**
     * @notice Remove beneficiaries to the Lock
     * @param data ABI-encoded data of beneficiaries (merkle tree, arrays of addresses and amounts, etc - specific to IDepositManager)
     * @param refundAmount Amount of tokens allocated for removed beneficiaries to be refunded
     */
    function removeBeneficiaries(bytes calldata data, uint256 refundAmount) external onlyBeneficiaryManager {
        require(canRemoveBeneficiaries, "ERROR: Cannot remove beneficiaries");
        (address[] memory addresses, uint[] memory amounts, uint[] memory IDs) =  
            abi.decode(data, (address[], uint[], uint[]));
        require(
            addresses.length == amounts.length && addresses.length == IDs.length, 
            "ERROR: Addresses and amounts  does not have same length"
        );
        depositManager.removeDeposits(addresses, amounts, IDs, refundAmount);
        for(uint i; i < addresses.length; i++) {
            bool success = tokenERC20.transfer(addresses[i], amounts[i]);
            require(success);
        }
    }
    ...
    function claimUnlocked(uint256 nftId, uint256 amountToClaim) external {
        require(_isApprovedOrOwner(_msgSender(), nftId), "You are not the owner or approved for this NFT");
        (, , uint initialAmount, ) =  depositManager.getProperties(nftId);

        uint unlockedAmount = schedule.unlockedAmount(lockStartTimestamp, initialAmount);

        require(unlockedAmount >= amountToClaim);
        depositManager.updateClaimedAmount(nftId, amountToClaim);
        bool success = tokenERC20.transfer(_msgSender(), amountToClaim);
        require(success);
    }

    function split(uint originId, uint[] memory splitParts, address[] memory addresses) external {
        require(_isApprovedOrOwner(_msgSender(), originId), "You are not the owner or approved for this NFT");
        require(
            splitParts.length == addresses.length, 
            "ERROR: splitParts and addresses does not have same length"
        );
        uint[] memory IDs = _setNFTs(addresses, addresses.length);
        depositManager.split(originId, IDs, splitParts, addresses);
    }
    ...
    function getNFTsInfo(uint nftId) external view returns(address beneficiarie, uint lockStartTime, uint amountLocked, uint amountUnlocked) {
        return depositManager.getProperties(nftId);
    }
}
...
interface IDepositManager {
     ...
     * @dev beneficiaries and amounts must be have the same length
     * @param addresses Addresses of beneficiaries
     * @param amounts Amounts of beneficiaries
     * @param IDs New NFT-IDs
     * @param totalAmount Total amount of tokens to be locked for additional beneficiaries
     */
    function addDeposits(address[] memory addresses, uint[] memory amounts, uint[] memory IDs, uint256 totalAmount) external;

    /**
     * @notice Remove a deposit with beneficiaries
     * @dev Beneficiaries must have the enough claimable amount (initial - claimed)
     * @param addresses Addresses of beneficiaries
     * @param amounts Amounts of beneficiaries
     * @param IDs New NFT-IDs
     * @param refundAmount Amount of tokens allocated for removed beneficiaries to be refunded
     */
    function removeDeposits(address[] memory addresses, uint[] memory amounts, uint[] memory IDs, uint256 refundAmount) external;
     ...
     * @param addresses Addresses of beneficiaries
     */
    function split(uint originId, uint[] memory splitedIds, uint[] memory splitParts, address[] memory addresses) external;
}
