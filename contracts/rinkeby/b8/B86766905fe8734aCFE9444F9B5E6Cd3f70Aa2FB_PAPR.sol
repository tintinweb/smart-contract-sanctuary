...
import './interfaces/IDFIMasterChef.sol';
import './interfaces/IPancakePair.sol';
import '@openzeppelin/contracts/math/SafeMath.sol';
/**
 * @title PAPR base contract
 * @notice PAPR contract rewritten to be compatible with DFIANCE Dex (some changes (modification of variables in another contract) added in 
            transfer, transferfrom and mint function)
...
contract PAPR is ERC20Burnable, DoubleOperator {
    ...
    bool public updateMoneyPot; // if true, papr will update money pot at each token movement
    bool public updateDFIMasterChef; // if true, papr will update DFIANCE master chef at each token movement
    ...
    modifier whitelist(address who) {
        require(blacklistUsers[who] == false, "You have been papr-ed, no printeri-no.");
        _;
    }

    function addBlacklist(address _blacklist) external onlyDoubleOperator {
        require(_blacklist != address(0), "You cannot blacklist 0x address");
        require(blacklistUsers[_blacklist] == false, "This user has already been blacklisted");
        blacklistUsers[_blacklist] = true;
    }

    function removeBlacklist(address _blacklist) external onlyDoubleOperator {
        require(_blacklist != address(0), "You cannot de-blacklist 0x address");
        require(blacklistUsers[_blacklist] == true, "This user has not been blacklisted");
    ...
    function mint(address recipient_, uint256 amount_) public onlySecondaryOperator returns (bool) {
        ...
        if (updateMoneyPot) moneyPotContract.updatePaprBonusShare(recipient_); // inform the money pot of papr token possible movement
        if (updateDFIMasterChef) dfiMasterChefContract.updatePaprAmount(_recipient); // verify that the user still holds enough papr to stay in DFIANCE master chef pools
    ...
    function getPoolToken(address pool, string memory signature, function() external view returns(address) getter) private returns (address token) {
        (bool success, ) = pool.call(abi.encodeWithSignature(signature)); // if the call succeed (pool address has the "signature" method or "pool" is an EOA)
        if (success) {
            uint32 size;
            assembly {
    ...
    // check if the recipient is a liquidity pool with papr into
    function isPaprLiquidityPool(address recipient) public returns (bool) {
    ...
    function _updateExternalContracts(address sender, address recipient) public {
        ...
        if (updateMoneyPot) { 
            // inform the money pot of papr token possible movement
            moneyPotContract.updatePaprBonusShare(sender);
            moneyPotContract.updatePaprBonusShare(recipient);
        }
        if (updateDFIMasterChef) {
            ...
            // verify that the user still holds enough papr to stay in DFIANCE master chef pools
            dfiMasterChefContract.updatePaprAmount(_addressToUpdate);
        }
    }
    ...
    function _transferOverride(address sender, address recipient, uint256 amount) internal returns (bool) {
        bool _senderExcludedAddresses = isSenderExcludedAddresses(sender);

        // if the recipient is a liquidity pool with papr, its a papr sell => take fees
        if (_senderExcludedAddresses == true) {  
            _transfer(sender, recipient, amount);

        } else if (isPaprLiquidityPool(recipient)) {
    ...
    function burn(uint256 amount) public virtual override {
        ...
        if (updateMoneyPot) moneyPotContract.updatePaprBonusShare(_msgSender()); // inform the money pot of papr token possible movement
        if (updateDFIMasterChef) dfiMasterChefContract.updatePaprAmount(_sender); // verify that the user still holds enough papr to stay in DFIANCE master chef pools
    }
...
interface IDFIMasterChef {
    function updatePaprAmount(address[] calldata _of) external;
}
