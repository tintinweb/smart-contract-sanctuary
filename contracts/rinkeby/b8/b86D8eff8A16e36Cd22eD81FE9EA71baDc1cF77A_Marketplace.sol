...
contract Marketplace {
    ...
    external tokenOwnerForbidden(tokenId) payable {
        ...
        uint256 balanceLimit=uint256(token.balanceLimit());
        require( balanceOfUser< balanceLimit,
...
contract DragonFactory is Ownable, ERC721Enumerable, ERC721URIStorage {
    ...
    uint256 public price=100000000000000; // price per dragon
    uint256 public balanceLimit=9; // max dragons per address
    // Maximum amounts of mintable tokens
    uint256 public maxTokenSupply=10000; // total available dragons
    ...
    function setRoyaltyPercentage(uint96 _royaltiesPercentage) public onlyOwner {
    ...
    function reserveNFT(
        string memory _hash
    ) onlyOwner external returns (uint256 tokenId) {
    ...
    /// @param _hash - the ipfs hash of token
    /// @return tokenId - the id of the token
    function mintNFT( string memory _hash) external payable returns (uint256 tokenId) {
        require(
            msg.value >= price,
            "Not enough ether to purchase NFTs."
        );
        require(
            balanceOf(msg.sender) < balanceLimit,
    ...
    {
        require(totalSupply() <= maxTokenSupply, "Sold out");
