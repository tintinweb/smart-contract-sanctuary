...
contract SotaMarketV2 is Manager, ERC1155Holder, ERC721Holder, ReentrancyGuard {
	...
	function _match(
        ...
        // If the buyer does not have the referral
        ...
        // If the order does not have the retailer
	...
	function acceptBid(uint256 _bidId, uint256 _quantity) external whenNotPaused returns (bool) {
        ...
        Order memory order;
        if (_orderId == 0) {
            uint256 orderId = createOrder(bid.tokenAddress, address(0), bid.paymentToken, bid.tokenId, _quantity, bid.bidPrice, 0);
