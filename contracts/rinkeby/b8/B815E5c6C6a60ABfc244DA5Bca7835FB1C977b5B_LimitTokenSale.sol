...
contract LimitTokenSale is Ownable, ReentrancyGuard {
  ...
  uint256 private constant PRICE_DECIMALS = uint256(10) ** 3;
  uint256 private constant TOKEN_DECIMALS = uint256(10) ** 18;
  uint32 private constant  TIME_LOCK = 5184000; //60 days
  ...
  event Submitted(uint256 indexed round, address indexed beneficiary, uint256 amount);
  event TokenClaimed(uint256 indexed round, address indexed beneficiary, uint256 amount);
  event LeftoverClaimed(uint256 indexed round, address indexed beneficiary, uint256 amount);
  event Resubscribed(uint256 indexed round, address indexed beneficiary, uint256 amount);
  event Recovered(address token, uint256 amount);
  event Withdrawn(uint256 amount);
  event WhitelistRequiredSet(bool isWhitelistRequired);
  ...
  constructor(address _saleToken, address _collateralToken, address _tokenVesting) public {
    ...
    saleToken.approve(_tokenVesting, uint256(-1));
    saleDeployed = block.timestamp;
  }

  /**
   * @dev Throws if caller exceeds investment limit
   */
  modifier onlyIfInvestmentAllowed(address _owner, uint256 _index, uint256 _amount) {
    require(balances[_index][_owner].add(_amount) <= rounds[_index].maxInvestment, "LimitTokenSale: exceeded round investment limit");
    _;
  }
  ...
  /**
   * @param _isWhitelistRequired  new whitelist requirement value
   *
   * @dev method updates whitelist requirement
   */
  function setWhitelistRequired(bool _isWhitelistRequired) external onlyOwner {
    isWhitelistRequired = _isWhitelistRequired;

    emit WhitelistRequiredSet(isWhitelistRequired);
  }
  ...
  function recoverERC20(address _token, uint256 _amount) external onlyOwner {
    if(_token == address(saleToken) || _token == address(collateralToken)) {
      require(saleDeployed + TIME_LOCK < block.timestamp, "LimitTokenSale: recover sale/collateral tokens locked");
    }
  ...
  function openRound(uint256 _index) external onlyOwner {
    ...
    require(rounds[_index].state == SaleLib.State.closed, "LimitTokenSale: round is not closed");
  ...
  function finalizeRound(uint256 _index) external onlyOwner {
    require(_index < rounds.length, "LimitTokenSale: round not defined");
    require(rounds[_index].state == SaleLib.State.open, "LimitTokenSale: round is not opened");
   ...
   */
  function initRound(uint256 _price, uint256 _vesting, uint256 _totalSupply, uint256 _maxInvestment, uint8 _pid) external onlyOwner {
   ...
   */
  function updateRound(uint256 _index, uint256 _price, uint256 _vesting, uint256 _totalSupply, uint256 _maxInvestment, uint8 _pid) external onlyOwner {
    require(rounds[_index].state == SaleLib.State.closed, "LimitTokenSale: Round is not closed");
    require(_vesting <= 100, "LimitTokenSale: Invalid vesting percent");
    ...
    rounds[_index].maxInvestment = _maxInvestment;
   ...
   */
  function getRound(uint256 _index) external view returns(SaleLib.State state, uint256 price, uint256 vesting, uint256 totalSupply, uint256 totalBalance, uint256 maxInvestment, uint8 pid) {
    state = rounds[_index].state;
    price = rounds[_index].price;
    vesting = rounds[_index].vesting;
    totalSupply = rounds[_index].totalSupply;
    totalBalance = rounds[_index].totalBalance;
    maxInvestment = rounds[_index].maxInvestment;
    pid = rounds[_index].pid;
  }
   ...
   * @dev method returns round details
   */
  function getResubscription(uint256 _roundIndex, uint256 _index) external view returns(uint256 price, uint256 totalSupply, uint256 totalBalance) {
    price = resubscriptions[_roundIndex][_index].price;
    totalSupply = resubscriptions[_roundIndex][_index].totalSupply;
    totalBalance = resubscriptions[_roundIndex][_index].totalBalance;
  }

  /**
   * @param _index index of round
   *
   * @dev method returns beneficiaries round leftover
   */
  function roundLeftoverOf(uint256 _index, address _beneficiary) public view returns(uint256) {
    SaleLib.Round storage round = rounds[_index];
    if(round.state == SaleLib.State.finalized) {
      uint256 balance = balances[_index][_beneficiary];
      uint256 refund = refunds[_index][_beneficiary];

      uint256 saleTokenTotal = toSaleTokenAmount(round.totalBalance, round.price);
      if(saleTokenTotal > round.totalSupply) {
        uint256 collateralTokenAmount = balance.mul(round.totalSupply).div(saleTokenTotal);
        uint256 leftover = balance.sub(collateralTokenAmount);
        return leftover.sub(refund);
      }
      return 0;
    }
    return 0;
   ...
   *
   * @dev method submits beneficiaries to the round 
   */
  function submit(uint256 _index, uint256 _amount) external nonReentrant onlyIfInvestmentAllowed(_msgSender(), _index, _amount) {
    SaleLib.Round storage round = rounds[_index];
    require(!isWhitelistRequired || whitelisted[_msgSender()], "LimitTokenSale: Beneficiary is not whitelisted");
    require(round.state == SaleLib.State.open, "LimitTokenSale: Round is not open");
    
    collateralToken.safeTransferFrom(_msgSender(), address(this), _amount);
    balances[_index][_msgSender()] = balances[_index][_msgSender()].add(_amount);
    round.totalBalance = round.totalBalance.add(_amount);
  ...
  /**
   * @param _index    index of round
   * @param _resubmit resubmit to next round amount
   * @param _refund   refund amount
   * @dev method claims beneficiary reward
   */
  function claimToken(uint256 _index, uint256 _resubmit, uint256 _refund) external nonReentrant {
    SaleLib.Round storage round = rounds[_index];
    uint256 balance = balances[_index][_msgSender()];

    require(round.state == SaleLib.State.finalized, "LimitTokenSale: Round is not finalized");
    require(balance > 0, "LimitTokenSale: Beneficiary is not a participant");
    ...
    if(saleTokenTotal > round.totalSupply) {
      uint256 collateralTokenAmount = balance.mul(round.totalSupply).div(saleTokenTotal);
      saleTokenAmount = toSaleTokenAmount(collateralTokenAmount, round.price);
      saleBalance = saleBalance.add(collateralTokenAmount);
    } else {
      saleTokenAmount = toSaleTokenAmount(balance, round.price);
      saleBalance = saleBalance.add(balance);
    }
    saleTokenAmount = saleTokenAmount.add(resubscriptionsBalances[_index][_msgSender()]);

    if(round.vesting > 0) {
      uint256 saleTokenVestingAmount = saleTokenAmount.mul(round.vesting).div(100);
      saleTokenAmount = saleTokenAmount.sub(saleTokenVestingAmount);
      tokenVesting.addBeneficiary(round.pid, _msgSender(), saleTokenVestingAmount);  
    }
    saleToken.safeTransfer(_msgSender(), saleTokenAmount);

    if(_resubmit > 0) {
      resubmitInternal(_index, _index + 1, _resubmit);
    }
    ...
    if(_refund > 0) {
      ...
      uint256[] memory refunds_ = new uint256[](1);
      refunds_[0] = _refund;

      claimLeftoverInternal(indexes, refunds_);
    }

    emit TokenClaimed(_index, _msgSender(), saleTokenAmount);
  }

  /**
   * @param _indexes indexes of rounds to refund from
   * @param _refunds amounts of refund
   *
   * @dev method claims beneficiary leftover
   */
  function claimLeftover(uint256[] memory _indexes, uint256[] memory _refunds) public nonReentrant {
    claimLeftoverInternal(_indexes, _refunds);
  }

  function claimLeftoverInternal(uint256[] memory _indexes, uint256[] memory _refunds) internal {
    require(_indexes.length == _refunds.length, "LimitTokenSale: Invalid params");

    uint256 collateralTotalRefund;
    for(uint256 i = 0; i < _indexes.length; i++) {
      require(rounds[_indexes[i]].state == SaleLib.State.finalized, "LimitTokenSale: Round is not finalized");

      uint256 leftover = roundLeftoverOf(_indexes[i], _msgSender());
      require(leftover >= _refunds[i], "LimitTokenSale: Too big refund");
      refunds[_indexes[i]][_msgSender()] = refunds[_indexes[i]][_msgSender()].add(_refunds[i]);
      collateralTotalRefund = collateralTotalRefund.add(_refunds[i]);

      emit LeftoverClaimed(_indexes[i], _msgSender(), _refunds[i]);
    }

    collateralToken.safeTransfer(_msgSender(), collateralTotalRefund);
  }

  /**
   * @param _index      index of round to resubmit from
   * @param _nextIndex  index of round to resubmit to
   * @param _amount     amount of resubmition
   *
   * @dev method resubmits beneficiary leftover amount
   */
  function resubmit(uint256 _index, uint256 _nextIndex, uint256 _amount) public nonReentrant {
    resubmitInternal(_index, _nextIndex, _amount);
  }

  function resubmitInternal(uint256 _index, uint256 _nextIndex, uint256 _amount) internal {
    require(rounds[_index].state == SaleLib.State.finalized, "LimitTokenSale: Round is not finalized");
    require(_nextIndex < rounds.length, "LimitTokenSale: No next round to resubmit");
    require(rounds[_nextIndex].state != SaleLib.State.finalized, "LimitTokenSale: Next round is finalized");
    
    uint256 leftover = roundLeftoverOf(_index, _msgSender());
    require(leftover >= _amount, "LimitTokenSale: Too big resubmition");

    refunds[_index][_msgSender()] = refunds[_index][_msgSender()].add(_amount);
    balances[_nextIndex][_msgSender()] = balances[_nextIndex][_msgSender()].add(_amount);
    rounds[_nextIndex].totalBalance = rounds[_nextIndex].totalBalance.add(_amount);

    emit Submitted(_nextIndex, _msgSender(), _amount);
  }
   ...
   *
   * @dev method resubscribe beneficiary leftover amount
   */
  function resubscribe(uint256 _index, uint256[] calldata _roundIndexes, uint256[] calldata _amounts) external nonReentrant {
    ...
    for(uint256 i = 0; i < _roundIndexes.length; i++) {
      ...
      SaleLib.Resubscription storage resubscription = resubscriptions[round][_index];
      uint256 saleTokenTokenAmount = toSaleTokenAmount(_amounts[i], resubscription.price);
      
      require(resubscription.totalBalance + saleTokenTokenAmount <=  resubscription.totalSupply, "LimitTokenSale: No enough tokens in resubscriptions");
      resubscription.totalBalance = resubscription.totalBalance.add(saleTokenTokenAmount);
      resubscriptionsBalances[round][_msgSender()] = resubscriptionsBalances[_index][_msgSender()].add(saleTokenTokenAmount);
  ...
  function withdraw(uint256 _amount) external onlyOwner {
    saleBalance = saleBalance.sub(_amount);
  ...
  /**
   * @param _collateralTokenAmount amount of collateral tokens to convert
   *
   * @dev method returns sale token amount
   */
  function toSaleTokenAmount(uint256 _collateralTokenAmount, uint256 price) private pure returns(uint256) {
    return _collateralTokenAmount.mul(TOKEN_DECIMALS).div(price).div(PRICE_DECIMALS);
  }
}
