...
// File contracts/QAOLottery.sol

pragma solidity ^0.8.1;


contract QAOLottery is Ownable {

    /**** MEMBERS ********************************************* */

    uint256 private _ticketPrice = 1 ether;

    uint256 private _intervalTicketStart = 0;
    uint256 private _intervalTicketEnd = 0;
    mapping(uint256 => address) private _ticketToAddress;

    uint256 public constant LOTTERY_INTERVAL = 1 weeks;
    uint256 private _currentIntervalEnd;

    QAOToken private _qaoToken;

    event TicketPurchased(uint256 ticketId, address ticketOwner);
    event LotteryWinner(uint256 ticketId, uint256 pool);


    constructor(address qaoTokenAddr) {
        _qaoToken = QAOToken(qaoTokenAddr);
        _currentIntervalEnd = block.timestamp + LOTTERY_INTERVAL;
    }
    ...
    /* set QAO ticket price (in Wei!) */
    function setTicketPrice(uint256 newPriceInWei) external onlyOwner {
        _ticketPrice = newPriceInWei;
    }

    /* purchase a ticket for the current QAO token price */
    function buyTicket() external {
        require(block.timestamp < _currentIntervalEnd, "QAO Lottery: Current lottery interval is over.");
        require(_qaoToken.transferFrom(_msgSender(), address(this), _ticketPrice), "QAO Lottery: Required allowance not maintained.");
        _ticketToAddress[_intervalTicketEnd] = _msgSender();

        emit TicketPurchased(_intervalTicketEnd, _msgSender());
        _intervalTicketEnd = _intervalTicketEnd + 1;
    } 

    /* generate random number between 0 and the provided number -1. */
    function _generateRandomNumber(uint256 max) private view returns (uint256){
        uint256 randomHash = uint256(keccak256(abi.encodePacked(_msgSender(),
                                                                _qaoToken.balanceOf(_msgSender()),
                                                                _qaoToken.totalSupply(),
                                                                intervalTickets(),
                                                                intervalPool(),
                                                                block.timestamp,
                                                                block.gaslimit, 
    ...
    /* if the current interval is over, this function can be called to pick the winner of the current
       interval, send the lottery pool to the winning address and start the the interval.
    */
    function pickWinner() external {
        // check if current interval is done
        require(_currentIntervalEnd > block.timestamp, "QAO Lottery: Current lottery interval is not over.");

        // pick winner and send pool
        uint256 randomTicket = _intervalTicketStart + _generateRandomNumber(intervalTickets());
        uint256 pool = intervalPool();
        _qaoToken.transfer(_ticketToAddress[randomTicket], pool); 
        emit LotteryWinner(randomTicket, intervalPool());

        // start new interval
        _currentIntervalEnd = block.timestamp + LOTTERY_INTERVAL;
        _intervalTicketStart = _intervalTicketEnd;
    }

    /* get the total amount of tickets purchased in the current interval */
    function intervalTickets() public view returns (uint256) {
        return (_intervalTicketEnd - _intervalTicketStart);
    } 

    /* get the QAO lottery pool of the current interval (in Wei)*/
    function intervalPool() public view returns (uint256) {
        return _qaoToken.balanceOf(address(this));
    }

    /* get the end timestamp of the current lottery interval (epoch in seconds) */
    function intervalTimestampEnd() external view returns (uint256) {
        return _currentIntervalEnd;
    } 
