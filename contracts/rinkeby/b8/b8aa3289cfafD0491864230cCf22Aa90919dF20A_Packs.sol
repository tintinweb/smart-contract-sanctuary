...
contract Packs is IPacks, ERC721, ReentrancyGuard {
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    uint256 edition = LibPackStorage.safeParseInt(LibPackStorage.substring(LibPackStorage.toString(tokenId), bytes(LibPackStorage.toString(tokenId)).length - 5, bytes(LibPackStorage.toString(tokenId)).length)) - 1;
          ...
          Base64.encode(
            bytes(
              abi.encodePacked(
                ...
                ds.editioned ? ' #' : '',
                ds.editioned ? LibPackStorage.toString(edition + 1) : '',
                '", "description":"',
