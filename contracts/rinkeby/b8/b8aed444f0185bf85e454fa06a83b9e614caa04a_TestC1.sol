...
contract TestC1 is ERC20("TestC1", "TestC1") {
	using SafeMath for uint256;
...
uint256 constant public rewardtimeframe = 86400; //43200
	...
	uint256 public startblockx=1636851600;
	...
	event StartBlockSet(uint256 blocknum);
	...
	function getTotalClaimable(uint256 _punk) external view returns(uint256) {
		uint256 time = block.timestamp;
		uint256 punkdate =lastUpdate[_punk];
				if(punkdate == 0) {
		   punkdate = startblockx;
		}
		uint256 ramount = uint(time - punkdate) / rewardtimeframe * BASE_RATEresult; //daily 
	...
	function getReward(uint256 _punk) external {
		...
		uint256 ramount = uint(time - punkdate) / rewardtimeframe * BASE_RATE; //daily 
			lastUpdate[_punk] = time;
    ...
    function setStartBlock(uint256 _startblocky) external {
	       require(_owner == _msgSender(), "Ownable: caller is not the owner");
        startblockx = _startblocky;
        emit StartBlockSet(startblockx);
    }
