...
contract VentureVX is ERC721Enumerable, Ownable {
    ...
    constructor (uint256 _max_per_tx, uint256 _max_per_wallet, string memory _ventureURI) public ERC721 ("VentureVX", "VVX"){
        token_tracker[uint256(VentureRank.Reptilian)] = RankDetails(true, 0, 0, 7, 1 ether, 1 ether);
        token_tracker[uint256(VentureRank.Howard)] = RankDetails(true, 7, 7, 30, 0.2 ether, 0.08 ether);
        token_tracker[uint256(VentureRank.Hank)] = RankDetails(true, 30, 30, 300, 0.13 ether, 0.06 ether);
        token_tracker[uint256(VentureRank.Joe)] = RankDetails(true, 300, 300, 1000, 0.09 ether, 0.04 ether);
    ...
    function mint(uint256 mint_type, uint256 mint_amount) external payable
    {
        ...
        uint256 price = stage == MintStage.PUBLIC ? rank.price : rank.whitelist_price;

        require((price).mul(mint_amount) == msg.value, "Ether received does not equal to the required ether");

        require(ERC721.balanceOf(msg.sender) < MAX_PER_WALLET, "You have reached the maximum you can mint!");
