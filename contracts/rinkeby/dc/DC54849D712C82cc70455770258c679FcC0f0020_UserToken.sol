...
contract IERC1155 is IERC165 {
    ...
    */
    function safeTransferFrom(address _from, address _to, uint256 _id, uint256 _value, bytes memory _data) internal;
    ...
    */
    function safeBatchTransferFrom(address _from, address _to, uint256[] memory _ids, uint256[] memory _values, bytes memory _data) internal;
...
contract ERC1155Base is HasSecondarySaleFees, Ownable, ERC1155Metadata_URI, HasContractURI, ERC1155 {
    ...
    function _mint(address user, uint256 _id, Fee[] memory _fees, uint256 _supply, string memory _uri) internal {
        ...
        balances[_id][user] = _supply;
        _setTokenURI(_id, _uri);

        // Transfer event with mint semantic
        emit TransferSingle(user, address(0x0), msg.sender, _id, _supply);
        emit URI(_uri, _id);
    }

    function burn(address _owner, uint256 _id, uint256 _value) internal {
...
contract MembershipToken is Ownable, ERC1155Base {
    string public name;
    ...
    function mint (address user, uint256 id, Fee[] memory fees, uint256 supply, string memory uri) internal {
        _mint(user, id, fees, supply, uri);
    }
    
    function _burn (address _owner, uint256 _id, uint256 _value) internal {
        burn(_owner, _id, _value);
    }
}



contract UserToken is MembershipToken {
    event CreateERC1155_v1(address indexed creator, string name, string symbol);
    // event mTokenListen(address indexed order, uint _id, bool KYC, bool ceritifier,bool champion);
    ...
    constructor(string memory name, string memory symbol, string memory contractURI, string memory tokenURIPrefix) MembershipToken(name, symbol, contractURI, tokenURIPrefix) public {
    ...
    function certifierUpdate(address user) internal {
    ...
    function values(address user) public view returns (string[] memory) {
        return info[user].Values;
    }

    function valuesAdd(address user, string memory _value) public returns(bool) {
        
        bool element = true;
        for (uint i=0; i< info[user].Values.length; i++) {
            if (keccak256(bytes(info[user].Values[i])) == keccak256(bytes(_value))) {
                element = false;
                break;
            }
        }
        if (element){ 
            info[user].Values.push(_value);
        }
        return element;
    }

    function valuesDelete(address user, string memory _value) public returns(bool) {
        bool element = false;
        for (uint i=0; i< info[user].Values.length; i++) {
            if (keccak256(bytes(info[user].Values[i])) == keccak256(bytes(_value))) {
                delete info[user].Values[i];
                element = true;
                continue;
            }
            if (element) {
                info[user].Values[i-1] = info[user].Values[i];
            }
        }

        if (element) {
            delete info[user].Values[info[user].Values.length-1];
            info[user].Values.length--;
        }
