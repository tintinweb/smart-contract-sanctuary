...
abstract contract OfferModel {
    string public constant ITEM__TYPE = "Item(address tokenAddress,uint256 tokenId)";
    string public constant LOAN_PARAMS__TYPE = "LoanParams(uint256 itemValue,uint256 redemptionPrice,uint32 timelockPeriod)";
    string public constant OFFER__TYPE = "Offer(uint32 nonce,uint40 expirationTime,address loanTokenAddress,Item collateralItem,LoanParams loanParams)"
                                           "Item(address tokenAddress,uint256 tokenId)"
...
abstract contract OfferSigVerifier is OfferModel, EIP712Domain {
    ...
    bytes32 public constant ITEM__TYPEHASH = keccak256(abi.encodePacked(ITEM__TYPE));
    bytes32 public constant LOAN_PARAMS__TYPEHASH = keccak256(abi.encodePacked(LOAN_PARAMS__TYPE));
    bytes32 public constant OFFER__TYPEHASH = keccak256(abi.encodePacked(OFFER__TYPE));

    function _hashItem(Item calldata item) public pure returns (bytes32) {
    ...
    function _hashLoanParams(LoanParams calldata params) public pure returns (bytes32) {
        return keccak256(abi.encode(
    ...
    function _hashOffer(Offer calldata offer) public view returns (bytes32) {
    ...
    function _verifyOffer(address signerAddress, bytes calldata signature, Offer calldata offer) public view returns (bool) {
