...
contract Affirmations is ERC721Enumerable, Ownable, VRFConsumerBase {
    ...
    */
    ...
    function tokenURI(uint256 tokenId) override view public returns (string memory) {
        ...
        return bytes(_baseTokenURI).length > 0 ? string(abi.encodePacked(_baseTokenURI, chosenURI.toString())) : "";
    }
    ...
    // 24 for DAO
    // 300 for marketing
    function mintGifts(uint256 numberOfMints, address _mintTo) public onlyOwner {
        ...
        for(uint i = 0; i < numberOfMints; i++) {
            ...
            if (mintNum < totalAffirmations) {
                _safeMint(_mintTo, mintNum);
            }
        }
    }

    function manifest(uint256 numberOfMints) public payable {
        require(saleStarted && mintStartIndexDrawn, "Sale hasn't started");
