...
contract YungApes is Ownable, ERC721 {
  ...
  function addToWhitelist(address[] memory wallets) public onlyOwner {
    require(wallets.length + privateWhitelistNextId <= maxWhitelistAddresses, "This would exceed max whitelist spots.");
  ...
  function mint(uint256 numberOfTokens) external payable {
    ...
    require(currentMintedByAddress + numberOfTokens <= mintLimitPerAddress, "You don't have enough mints left for this many tokens for this address");
  ...
  function mintInternalPresale(address wallet, uint tokenId) internal {
    ...
    if(addressesOfTokenIds[tokenId] == wallet){
