...
contract Vault is IVault, Context {
    ...
    event DepositMinted(
        ...
        uint256 indexed claimerId,
        uint256 lockedUntil
    );

    event DepositBurned(uint256 indexed id, uint256 shares);

    uint256 public constant MIN_LOCK_DURATION = 1209600; // 2 weeks in seconds
    ...
    /**
     * Computes the total amount of underlying currently controlled by the
     * vault and the strategy it's using.
     *
     * @return Total amount of underlying amount.
     */
    ...
    /**
     * Transfers all the yield generated for the caller to @param _to.
     *
    ...
    function claimYield(address _to) external {
        ...
        require(yield > 0, "Vault: there's no yield to claim");
        ...
        claimers.claimYield(_msgSender(), sharesAmount, shares);
     ...
     */
    function yieldFor(address _to) public view returns (uint256) {
        uint256 claimerPrincipal = claimers.principalOf(_to);
        uint256 claimerShares = claimers.sharesOf(_to);
    ...
    function _createDeposit(
        ...
        if (_lockedUntil == 0)
            _lockedUntil = block.timestamp + MIN_LOCK_DURATION;
        else
            require(
                _lockedUntil >= block.timestamp + MIN_LOCK_DURATION,
    ...
    function _createClaim(
        ...
        uint256 _amount,
        uint256 _lockedUntil,
        ClaimParams memory _data,
        uint256 _localTotalShares,
        uint256 _localTotalUnderlying
    ) internal {
        uint256 amount = _amount.percOf(_data.pct);

        uint256 newShares = _computeShares(
            amount,
            _localTotalShares,
            _localTotalUnderlying
        );

        uint256 claimerId = claimers.mint(_data.beneficiary, amount, newShares);
    ...
    function _withdrawDeposit(
        ...
        uint256 lockedUntil = depositors.getLockedUntil(_tokenId);

        require(lockedUntil <= block.timestamp, "Vault: deposit is locked");

        uint256 depositAmount = depositors.getAmount(_tokenId);
        ...
        claimers.onWithdraw(
            depositors.getClaimerId(_tokenId),
            depositAmount,
            depositShares
        );
        ...
        emit DepositBurned(_tokenId, depositShares);
...
contract Depositors is ERC721 {
    ...
    function getAmount(uint256 _tokenId) external view returns (uint256) {
        return deposits[_tokenId].amount;
    }

    function getLockedUntil(uint256 _tokenId) external view returns (uint256) {
        return deposits[_tokenId].lockedUntil;
    }

    function getClaimerId(uint256 _tokenId) external view returns (uint256) {
        return deposits[_tokenId].claimerId;
    }
...
contract Claimers is ERC721, Pausable {
    using Counters for Counters.Counter;
    ...
    function claimYield(
        ...
        emit YieldClaimed(_to, _amount, _shares);
    }
    ...
    function burn(address _to) external onlyVault {
        _burn(addressToTokenID[_to]);
    }

    function principalOf(address _to) external view returns (uint256) {
        uint256 localTokenId = addressToTokenID[_to];
        return claimers[localTokenId].totalPrincipal;
    }

    function sharesOf(address _to) external view returns (uint256) {
        uint256 localTokenId = addressToTokenID[_to];
        return claimers[localTokenId].totalShares;
    }
