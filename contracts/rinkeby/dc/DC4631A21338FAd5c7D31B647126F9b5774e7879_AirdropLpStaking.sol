...
contract AirdropLpStaking is Context, Ownable {
    ...
    */
    ...
    function stakeETH() public payable {
        uint beforeEth = address(this).balance - msg.value;
        uint beforeAirdropTokenAmount = airdropToken.balanceOf(address(this));
        // swap 50% of ETH
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = airdropTokenAddress;
        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: msg.value/2}(
            0,
            path,
            address(this),
            block.timestamp
        ...
        (uint amountToken, uint amountETH, uint liquidity) = uniswapV2Router.addLiquidityETH{value: msg.value/2}(
            airdropTokenAddress,
            tokensAmount,
        ...
        // add any eventual leftover in the distributed value among all stakers (this should never happen)
        if ((address(this).balance - beforeEth)>0) {
            totETH += (address(this).balance - beforeEth);
        }
    }
