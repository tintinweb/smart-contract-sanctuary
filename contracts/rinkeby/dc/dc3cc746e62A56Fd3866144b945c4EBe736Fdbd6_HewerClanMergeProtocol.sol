...
contract HewerClanMergeProtocol is Ownable, ERC721Enumerable {
    ...
    event MergeRequestApproved(uint256 tagTokenId);

    constructor(string memory uri, address hewerClanContractAddress) ERC721("HewerBox", "HB") {
        setBaseURI(uri);
    ...
    function getOwnerTagTokenIdsAvailableForMerge() public view returns (uint [] memory) {
        uint balance = hewerClan.balanceOf(msg.sender);
        uint [] memory tokenIds = new uint [](balance);

        for (uint i = 0; i < balance; i++) {
            uint tagTokenId = hewerClan.tokenOfOwnerByIndex(msg.sender, i);

            if (!tagIsMerged(tagTokenId)) {
                tokenIds[i] = tagTokenId;
            }
        }
    ...
    function getOwnerNftTokenIdsAvailableForMerge(string memory name) public view returns (uint [] memory) {
        if (collectionIsRegistered(name)) {
            Collection collection = collectionNameToCollectionEntry[name].collection;

            uint balance = collection.balanceOf(msg.sender);
            uint [] memory tokenIds = new uint [](balance);

            for (uint i = 0; i < balance; i++) {
                uint tokenId = collection.tokenOfOwnerByIndex(msg.sender, i);

                if (!collectionNftIsMerged(name, tokenId)) {
                    tokenIds[i] = tokenId;
                }
            }
