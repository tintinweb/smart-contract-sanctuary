...
contract BattleRoyaleArena is Ownable, VRFConsumerBase {
  ...
  function executeEliminationByQueue() external onlyOwner returns(bool) {
    for (uint i = 0; i < battleQueue.size(); i++) {
      address payable nftAddress = battleQueue.atIndex(i);
  ...
  function executeElimination(address payable _nftAddress) public onlyOwner returns(bool) {
