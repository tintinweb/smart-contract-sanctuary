...
contract Operators is Ownable
{
    ...
    /**
    * @dev Owner can claim ether that transferred
    * to this contract address
    */
...
// File: CreatureERC721.sol

//v0.0.1
contract Creatures is ERC721URIStorage, Operators {
    ...
    function mintWithURI(
        ...
        animals[tokenId] = Animal(AnimalType(_animalType), Rarity(_rarity), 0, "");
    }

    function editAnimalNextFarm(uint256 tokenId, uint256  _nextFarmTime) external onlyOwner {
        animals[tokenId].nextFarm = _nextFarmTime;
    }

    function setURI(uint256 tokenId, string memory _tokenURI) external {
        require(ownerOf(tokenId) == msg.sender || isOperator(msg.sender),
            'Only token owner or operator can change URI.');
    	_setTokenURI(tokenId, _tokenURI);
    }
    ...
    function getAnimalNextFarm(uint256 _tokenId) external view returns(uint256) {
        return uint256(animals[_tokenId].nextFarm);
    }
    ...
    function baseURI() public view  override returns (string memory) {
        //Change this before mainnet!!!
        return 'https://degens.farm/creatures/';
    }
