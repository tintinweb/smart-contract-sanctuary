...
contract CheetahClubTestPass is ERC1155, Ownable {

  uint public constant ClubPass = 0;

  uint public constant MAX_TOKENS = 3000;
  uint public numMinted = 0;

  uint public ClubPassPriceWei = 0.069 ether;
  ...
  string private _contractUri = "https://gateway.pinata.cloud/ipfs/QmT9FmgfPHb36dxMy5jbbA1jY5rSaTv2EdX56q4HscAjEj/1.json";

  string public name = "CheetahClubTestPass";
  string public symbol = "CCP";

  constructor() public ERC1155("https://gateway.pinata.cloud/ipfs/QmT9FmgfPHb36dxMy5jbbA1jY5rSaTv2EdX56q4HscAjEj/1.json"){}
  ...
  function _internalMint(uint passType, uint amount) internal {
    ...
    if (passType == ClubPass) {
      checkPayment(ClubPassPriceWei * amount);
    } else {
  ...
  function ownerMint(uint passType, uint amount) public onlyOwner {
    incrementNumMinted(amount);
    require(passType == ClubPass, "Invalid passType");
    _mint(msg.sender, passType, amount, "");
  }
  ...
  function isOnWhitelist(address addr, bytes32[] calldata proof, uint whitelistType) public view returns (bool) {
    ...
    if (whitelistType == FIRST_BUYERS) {
      ...
      revert("Invalid whitelistType, must be 1");
    }
  ...
  function setClubPassPriceWei(uint newPrice) public onlyOwner {
    ClubPassPriceWei = newPrice;
  }
