...
import "../imports/utils/WithNonce.sol";
import "../imports/utils/WithDelegate.sol";
import "../imports/Voting.sol";
...
abstract contract ERC1155Votes is ERC1155, WithNonce, WithDelegate {
    ...
    uint256 constant SILVER_TICKET = 1;
    uint256 constant GOLD_TICKET = 2;
    uint256 constant PLATINUM_TICKET = 3;
    ...
    function getIdVotingPower(uint256 id) public view returns (uint256) {
        if (id == SILVER_TICKET) {
            return 25;
    ...
    function _mint(
        ...
        _votes.mint(to, getIdVotingPower(id), _hookDelegateVotesChanged);
    }
    ...
    function _mintBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            super._mint(to, ids[i], amounts[i], data);
            _votes.mint(to, getIdVotingPower(ids[i]), _hookDelegateVotesChanged);
        }
    }
    ...
    function _burn(
        ...
        _votes.burn(from, getIdVotingPower(id), _hookDelegateVotesChanged);
    }
    ...
    function _burnBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            super._burn(from, ids[i], amounts[i]);
            _votes.burn(from, getIdVotingPower(ids[i]), _hookDelegateVotesChanged);
        }
    }
    ...
    function _safeTransferFrom(
        ...
        _votes.transfer(from, to, getIdVotingPower(id), _hookDelegateVotesChanged);
    }
    ...
    function _safeBatchTransferFrom(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            _votes.transfer(from, to, getIdVotingPower(ids[i]), _hookDelegateVotesChanged);
        }
    }
