...
contract BattleContract is Ownable {
    ...
    function _claimGFX(uint256 _battleId, address owner) private {
        Battle memory battle = battles[_battleId];
        uint256 amount = battle.award * (10**18);

        gfx_.transfer(owner, amount);
    }

    function getMyAddress() public returns (address) {
        return nft_.sendAddress();
    }
}
...
interface INFTContract {
    ...
    function sendAddress() external returns (address);
}
