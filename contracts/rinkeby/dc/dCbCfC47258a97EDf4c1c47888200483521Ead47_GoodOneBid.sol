...
contract GoodOneBidStorage {
  ...
  uint256 public constant FIFTEEN_MINUTE = 900;
  ...
  // List by token address => token id => address
  mapping(address => mapping(uint256 => List)) public listsByToken;
...
contract GoodOneBid is Ownable, Pausable, GoodOneBidStorage {
  ...
  function listNFT(
    ...
    require(
      ERC1155Interface(_tokenAddress).balanceOf(msg.sender, _tokenId) ==
        _amount,
      "Creator does not have NFT amount"
    );

    require(
      listsByToken[_tokenAddress][_tokenId].id == bytes32(0),
      "List is alreay exists"
    );

    require(_duration > FIFTEEN_MINUTE, "Duration must be bigger the 15 min");
    ...
    listsByToken[_tokenAddress][_tokenId] = List({
      id: listId,
      owner: msg.sender,
  ...
  function onERC1155Received(
    ...
    List memory listToken = listsByToken[msg.sender][_tokenId];
  ...
  function unlistNFT(
    ...
    require(
      ERC1155Interface(_tokenAddress).balanceOf(address(this), _tokenId) ==
        _amount,
      "Escow does not have NFT amount"
    );

    require(
      listsByToken[_tokenAddress][_tokenId].owner == msg.sender ||
        msg.sender == owner(),
    ...
    ERC1155Interface(_tokenAddress).safeTransferFrom(
      address(this),
      listsByToken[_tokenAddress][_tokenId].owner,
      _tokenId,
      _amount,
      ""
    );
    ...
    delete listsByToken[_tokenAddress][_tokenId];
    delete bidsByToken[_tokenAddress][_tokenId];

    emit NFTUnlisted(msg.sender, _tokenAddress, _tokenId, _amount);
  }
  ...
  function changeReservePrice(
    ...
    listsByToken[_tokenAddress][_tokenId].reservePrice = _reservePrice;
  ...
  function placeBid(
    ...
    List memory listToken = listsByToken[_tokenAddress][_tokenId];
    Bid memory highestBid = bidsByToken[_tokenAddress][_tokenId];
    ...
    // Check minimum raise
    require(
      (_price > highestBid.price.add(bidMinimumRaiseAmount) ||
        _price >
        highestBid.price.add(
          highestBid.price.mul(bidMinimumRaisePerMillion).div(ONE_MILLION)
    ...
    if (highestBid.bidder == address(0)) {
      listToken.expiresAt = block.timestamp.add(listToken.duration);
      listsByToken[_tokenAddress][_tokenId] = listToken;
    ...
    // check if place bid in the last FIFTEEN_MINUTE
    if (listToken.expiresAt.sub(block.timestamp) < FIFTEEN_MINUTE) {
      listToken.expiresAt = block.timestamp.add(FIFTEEN_MINUTE);
      listsByToken[_tokenAddress][_tokenId] = listToken;
    ...
    if (bidFeePerMillion > 0) {
      ...
      feeAmount = _price.mul(bidFeePerMillion).div(ONE_MILLION);
  ...
  function settleBid(
    ...
    List memory listToken = listsByToken[_tokenAddress][_tokenId];
    address owner = listsByToken[_tokenAddress][_tokenId].owner;

    require(
      listToken.expiresAt < block.timestamp,
      "Can't accpect list before been ended"
    );
    ...
    require(
      goodOneToken.transfer(listToken.owner, price.sub(saleShareAmount)),
  ...
  {
    require(
      _ownerCutPerMillion < ONE_MILLION,
    ...
    onlyOwner
  {
    require(
      ...
      "The owner cut should be bigger then 0 "
    );
  ...
  /**
   * @dev withdraw the erc20 tokens from the contrcat
  ...
  /**
   * @dev withdraw the erc1155 tokens from the contrcat
   * @param _tokenAddress - address of the ERC1155 token
   * @param _tokenId - uint256 of the token id
   * @param _withdrawAddress - The withdraw address
   * @param _amount - The withdraw amout
  ...
  function _requireBidderBalance(address _bidder, uint256 _amount)
    ...
    view
  {
    require(goodOneToken.balanceOf(_bidder) >= _amount, "Insufficient funds");
    require(
