...
contract TinyPaws is ERC1155, Ownable, ReentrancyGuard {
    ...
    uint constant private TOKEN_ID = 0;
    ...
    modifier maxSupplyCheck(uint amount)  {
        require(tokensMinted + nftMinted + amount <= MAX_SUPPLY, "");
        require(amount < 10, "");
        _;
    }

    function preOrder(uint plan) external payable nonReentrant {
        require(saleState == SaleState.PreOrder, "salestate");
    ...
    function mintTokens(uint amount) internal maxSupplyCheck(amount) {
        require(tokensMinted + amount <= MAX_SUPPLY, "Tokens supply reached limit");
        tokensMinted += amount;
        _mint(msg.sender, TOKEN_ID, amount, "");
    }

    function publicMintNFTs(uint amount) external payable {
        mintNFTs(amount);
    }

    function publicMintNFTs1(uint amount) external payable {
        mintNFTs1(amount);
    }
    ...
    function mintNFTs1(uint amount) internal maxSupplyCheck(amount) {
