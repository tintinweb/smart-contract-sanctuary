...
contract Syndicate is ERC1155, Ownable {
    ...
    Stages stage = Stages.PRE_INIT;

    constructor(
        string memory _uri,
        uint256 _lifetimeSupply,
        uint256 _mintPrice
    ) ERC1155(_uri) {
        // Premint subscription supply
        lifetimeSupply = _lifetimeSupply;
        subscriptionMintPrice = _mintPrice;
    }
    ...
    function setStageWhitelist() external atStage(Stages.PRE_INIT) onlyOwner {
        // increment counter
    ...
    function _doSubscriptionMint(uint8 _amount) internal {
        require(
            _amount <= balanceOf(address(this), SUBSCRIPTION_ID),
            "No more tokens or contract not initialized"
        );
        _safeTransferFrom(
            address(this),
            msg.sender,
    ...
    function uri(uint256 _id) public view override returns (string memory) {
        require(_id < ltId.current(), "Invalid id");
        return
            string(
