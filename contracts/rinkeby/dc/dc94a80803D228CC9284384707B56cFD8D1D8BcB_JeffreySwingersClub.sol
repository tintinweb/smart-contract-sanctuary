...
contract JeffreySwingersClub is ERC721, Ownable {
    ...
    string private baseURI = "ipfs://QmSZwpT2MSGRtGKPrcy16HrSUHuC1rTBVQ51fF9WdMVcWp/";
    ...
    */
    ...
    constructor() ERC721("JeffreySwingersClub", "JEFFREY") {
        ...
        _safeMint(msg.sender, 171);
        _safeMint(msg.sender, 281);
        _safeMint(msg.sender, 300);
        _safeMint(msg.sender, 631);
        _safeMint(msg.sender, 632);
        _safeMint(msg.sender, 633);
        _safeMint(msg.sender, 649);
        ...
        payable
    {
        ...
        require(_isApprovedOrOwner(msg.sender, tokenId), "Caller is not approved");
        require(victims[tokenId].length < 3, "Can only name 3 victims.");
        require(bytes(_name).length > 0 && bytes(_name).length < 30, "Name must be between 0 and 30 bytes");
        ...
        payable
    {
        ...
        require(victims[tokenId].length == 3, "Must name 3 victims first");
        require(bytes(_name).length > 0 && bytes(_name).length < 30, "Name must be between 0 and 30 bytes");
        ...
        payable
    {
        ...
        require(bytes(_secret).length > 0 && bytes(_secret).length < 30, "Must be between 0 and 30 bytes.");
        ...
        payable
    {
        ...
        uint256 tokenId = _getRandomUniqueTokenID();
    ...
    // Yeah
    function _getRandomUniqueTokenID()
        private
        view
        returns(uint256)
    {
        uint256 tokenId = (uint256(keccak256(abi.encodePacked(block.timestamp + block.difficulty))) + totalSupply) % maxSupply;
