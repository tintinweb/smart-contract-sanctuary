...
contract Controller is Initializable, OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe {
        ...
        onlyAuthorized(msg.sender, _args.owner)
    {
        ...
        require(milkExchange != address(0),"milkEx = 0");
        ...
        otoken.mintOtoken(milkExchange, _args.amount);

        uint32 uintPrice = BytesUtil.toUint32(_args.price);

        Exchange(milkExchange).place(msg.sender, address(otoken), _args.amount, uintPrice);        

        emit ShortOtokenMinted(_args.otoken, _args.owner, _args.to, _args.vaultId, _args.amount,uintPrice);
    }
    
    function setMilkExchage(address _ex) external onlyOwner{
        milkExchange = _ex;        
    }
    ...
    function _settleVault(Actions.SettleVaultArgs memory _args) internal onlyAuthorized(msg.sender, _args.owner) {
        ...
        if(Exchange(milkExchange).userToken(msg.sender,address(otoken)) > 0){
            Exchange(milkExchange).settleCollateral(msg.sender,address(otoken));
        }
