...
// File: powersurge/ContentMixin.sol
...
// File: powersurge/powersurgenft.sol
    ...
    Ownable
{
    ...
    uint256 private mintPrice = 5e16;

    bool public isPublicMintEnabled = false;
    bool public isPresaleMintEnabled = false;

    mapping(address => bool) public whitelist;

    constructor() ERC721("PowerSurge", "PWR") EIP712("PowerSurge", "1.0.0") {
        baseTokenURI = "https://api.powersurgenft.io/api/v1/";
    }


    /**
     * @dev Airdrop nft to several addresses.
     ...
     * @param _num number of tokens to be minted
     */
    function mint(uint256 _num) external payable {
        require(
            isPublicMintEnabled || (isPresaleMintEnabled && whitelist[msg.sender]),
            "Minting is not currently enabled."
        );
        require(
        ...
        require(
            _num <= 30,
            "Up to 30 PowerSurge can be minted in a tx."
        );
        require(
            msg.value == _num * mintPrice,
    ...
    function _mintList(uint256 _num) private {
    ...
    function disablePublicMinting() external onlyOwner {
        ...
        /**
     * @dev enable presale minting
     */
    function enablePresaleMinting() external onlyOwner {
        isPresaleMintEnabled = true;
    }
    ...
    function premint(uint8 _num) external onlyOwner {
        _mintList(_num);
    }
