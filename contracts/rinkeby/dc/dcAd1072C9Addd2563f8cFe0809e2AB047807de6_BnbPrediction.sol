...
//        _setUniswapV2Router(_routerAddress);
     ...
     */
    function genesisStartRound(string memory _symbol) external onlyOperator whenNotPaused(_symbol) {
     ...
     */
    function genesisLockRound(string memory _symbol) external onlyOperator whenNotPaused(_symbol) {
     ...
     */
    function executeRound(string memory _symbol) external onlyOperator whenNotPaused(_symbol) {
        require(
     ...
     * 投跌
     */
    function betBear(string memory _symbol) external payable whenNotPaused(_symbol) notContract {
     ...
     * 投涨
     */
    function betBull(string memory _symbol) external payable whenNotPaused(_symbol) notContract {
        require(symbolSwitch[_symbol] == 1,"the_currency_is_not_yet_open");
        uint256  current = currentEpochMap[_symbol];
        require(_bettable(_symbol,current), "Round not bettable");
        require(msg.value >= minBetAmount, "Bet amount must be greater than minBetAmount");
     ...
     * 由管理员调用暂停，触发停止状态
     */
    function pause(string memory _symbol) public onlyAdminOrOperator whenNotPaused(_symbol) {
        _pause(_symbol);
     ...
     */
    function unpause(string memory _symbol) public onlyAdmin whenPaused(_symbol) {
