...
contract View {
    ...
    KyokoMiningManager public immutable kyokoMiningManager;
    TimeLockPool public immutable escrowPool;

    constructor(address _kyokoMiningManager, address _escrowPool) {
        kyokoMiningManager = KyokoMiningManager(_kyokoMiningManager);
        escrowPool = TimeLockPool(_escrowPool);
    }

    function fetchData(address _account) external view returns (Data memory result) {
        uint256 rewardPerSecond = kyokoMiningManager.rewardPerSecond();
        uint256 lastDistribution = kyokoMiningManager.lastDistribution();
        uint256 pendingRewards = rewardPerSecond * (block.timestamp - lastDistribution);

        result.totalWeight = kyokoMiningManager.totalWeight();

        KyokoMiningManager.Pool[] memory pools = kyokoMiningManager.getPools();
