...
//    mapping(uint256 => mapping(uint256 => uint256)) campaignArtistFreeTokenMapping;
    ...
    function register(address accessPassAddress,
        ...
        campaignList[campaignId].maxSupply = maxSupply;
    ...
    function mapTokenToArtist ( uint256  campaignId , uint256  artistTokenId,  uint256 [] calldata  tokenIdList ) public  onlyRole (CAMPAIGN_ADMIN_ROLE) {
        require(campaignList[campaignId].state == CampaignState.PENDING, "Can not add hash to Started campaign");
        require(tokenIdList.length  + availableHashList[campaignId].length <= campaignList[campaignId].maxSupply, "can not exceed maxSupply.");

        for (uint i = 0; i < tokenIdList.length; i++) {
            tokenToArtistMapping[tokenIdList[i]] = artistTokenId;
    ...
    //function to enable "test" mint that closely resembles public mint but is limited in access
    ...
    function _checkCampaignStateForAdminMinting(uint256 campaignId) internal returns (uint256) {
        if (campaignList[campaignId].state == CampaignState.PENDING || campaignList[campaignId].state == CampaignState.PAUSED) {
            revert("Admin mint not available");
        } else if (campaignList[campaignId].state == CampaignState.READY || campaignList[campaignId].state == CampaignState.PRESALE) {
            IAccessPass accessPass = IAccessPass(campaignList[campaignId].accessPassAddress);
            if (accessPass.balanceOf(msg.sender, campaignList[campaignId].vipSalePassId) > 0) {
                accessPass.burn(msg.sender, campaignList[campaignId].vipSalePassId, 1);
                return campaignPriceList[campaignId].unitPriceVipSale;
    ...
    function preMintWithTokenId(uint256 campaignId, address receiver, uint256 tokenId) public onlyRole(CAMPAIGN_ADMIN_ROLE) {
        require(campaignList[campaignId].state == CampaignState.READY, "Campaign is not ready.");
        _mintToken(campaignId, tokenId, receiver);
    ...
    function _mintToken(uint256 campaignId, uint256 tokenId, address receiver) private {
...
//
//        _mintTokenAtIndex(campaignId, foundAt, receiver);
//
//    }
    ...
    function totalSupply() public view returns (uint256) {
        return artistTokenReserve + tokenIdCounter.current();
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        return string(abi.encodePacked(_baseURIextended, tokenId));
    }
