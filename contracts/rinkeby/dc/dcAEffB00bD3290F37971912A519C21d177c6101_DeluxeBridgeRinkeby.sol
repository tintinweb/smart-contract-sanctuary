 ...
 */
contract DeluxeBridgeRinkeby is ERC1155Holder, Ownable, ReentrancyGuard {
    /**
    ...
    function getTransferredByCollector(address _collector) external view returns (uint256[] memory) {
        require(_collector != address(0), "_collector is address 0");
        uint32 j = 0;
        for (uint32 i = 0; i < idsReceived.length; i++) {
            if (idsAndSenders[idsReceived[i]] == _collector) {
                j++;
            }
        }
        uint256[] memory ids = new uint256[](j);
        j = 0;
    ...
    function getBridgedByCollector(address _collector) external view returns (uint256[] memory) {
        require(_collector != address(0), "_collector is address 0");
        uint32 j = 0;
        for (uint32 i = 0; i < oldsIdsBridged.length; i++) {
            if (idsAndSenders[oldsIdsBridged[i]] == _collector) {
                j++;
            }
        }
        uint256[] memory ids = new uint256[](j);
    ...
    /**
     * @dev gets received ids (not claimed yet)
     */
    function getReceived() external view returns (uint256[] memory) {
        return idsReceived;
    }
