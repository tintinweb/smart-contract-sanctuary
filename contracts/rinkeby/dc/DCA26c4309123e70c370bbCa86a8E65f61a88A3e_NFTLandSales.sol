...
contract NFTLandSales is EIP712, AccessControl, Initializable  {
  ...
  mapping(uint => uint) public priceByTier;
  mapping(string => PresaleConfig) public presaleConfigByTicketId;
  mapping(string => bool) public presaleActiveByTicketId;
  mapping(string => uint) public presaleNumSoldByTicketId;
  ...
  function configureTier(uint tier, uint numTokens, uint firstTokenId, uint initialPrice ) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(statsByTier[tier].numTokens == 0, 'tier already configured');
    statsByTier[tier].numTokens = numTokens;
    statsByTier[tier].numSold = 0;
    firstTokenIdByTier[tier] = firstTokenId;
    priceByTier[tier] = initialPrice;
    shoeByTier[tier].size = numTokens;
  }

  function configurePresale(string calldata ticket, address ticketSigner, uint maxSell) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    presaleActiveByTicketId[ticket] = false;
  }

  function setTierPrice(uint tier, uint newPrice ) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(statsByTier[tier].numTokens != 0, 'tier not configured');
    priceByTier[tier] = newPrice;
  }
  ...
  function buyPresale(uint tier, string calldata ticket, bytes calldata signature) external payable {
    require(presaleActiveByTicketId[ticket], 'sale not started');
    require(statsByTier[tier].numTokens != 0, 'tier not available');
    require(priceByTier[tier] == msg.value, 'wrong price');
    require(statsByTier[tier].numSold < statsByTier[tier].numTokens, 'Sold out');
    
    PresaleConfig storage presale = presaleConfigByTicketId[ticket];
    require(presaleNumSoldByTicketId[ticket] < presale.maxSell, 'Presale Sold out');
    ...
    require(signer == presale.ticketSigner, 'invalid ticket');

    presaleNumSoldByTicketId[ticket]++;
    uint tokenId = pickTokenId(tier);
    tokenContract.mint(msg.sender, tokenId);
    emit TokenPurchased(tokenId, msg.sender);
  }

  function buy(uint tier) external payable {
  ...
  function setPresaleActive(string calldata ticket, bool active) external onlyRole(DEFAULT_ADMIN_ROLE) {
    presaleActiveByTicketId[ticket] = active;
  }
