...
contract IDXYS {
  ...
  */
    ...
    function _depositOnComp(address _asset,address _cToken, uint256 _amount) internal {
        ...
        pBalances[msg.sender][_cToken] = balances[msg.sender][_cToken].add(userAmount);                      // ajust balance in collateral token
        balances[msg.sender][_cToken] = balances[msg.sender][_cToken].add(_amount);                          // ajust balance in underlying token
    }
