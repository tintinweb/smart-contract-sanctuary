...
contract ApeTraders is ERC721, Ownable {
    ...
    uint public constant MAX_NFT_PURCHASE_PRESALE = 7777;
    uint public constant MAX_NFT_PURCHASE = 7777;
    mapping(address => bool) public presaleList;
    mapping(address => uint256) public presaleClaimedList;
    ...
    function mint(uint256 count) external payable {
        ...
        require(count > 0 && count <= MAX_NFT_PURCHASE, "You can mint between 1 and 999 at once");
    ...
    function mintPresale(uint256 count) public payable {
        ...
        presaleClaimedList[msg.sender] += count;
        bool success = false;
        (success,) = owner().call{value : msg.value}("");
        require(success, "Failed to send to owner");
    }
    ...
    function addToPresaleList(address[] memory addresses) public onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
          ...
          presaleClaimedList[addresses[i]] > 0 ? presaleClaimedList[addresses[i]] : 0;
        }
    }
