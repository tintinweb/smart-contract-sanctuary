...
contract ArenasData is Ownable, ERC721, ERC721Holder {
    ...
    mapping(uint256 => Arena.Struct) public terrains;
    
    event NewTerrain(uint256 indexed terrainId, Arena.Struct terrain);
    event EditTerrain(uint256 indexed terrainId, Arena.Struct terrain);
    ...
    function createTerrain(Arena.Struct memory terrain) external onlyOwner {
        (bool success, ) = methodsContractAddress.delegatecall(msg.data);
        require(success,error);
    }
    
    function editTerrain(uint256 _id, Arena.Struct memory terrain) external onlyOwner {
        (bool success, ) = methodsContractAddress.delegatecall(msg.data);
        require(success,error);
    }
