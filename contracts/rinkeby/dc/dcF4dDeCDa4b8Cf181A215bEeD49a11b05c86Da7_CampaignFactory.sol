    ...
    AllocatedWhitelist
{
        ...
        returns (uint256)
    {
        return
            etherConversionRate
                .mul(1 ether)
                .mul(
                Math.max(maxBuyAmount, getAllocatedUserAmountInUSD(msg.sender))
            )
                .mul(1e8)
    ...
    function _updatePurchasingState(
        ...
        require(
            userTokensMapping[_beneficiary].tokensBought.add(_tokenAmount) <=
                etherConversionRate
                    .mul(1 ether)
                    .mul(
                    Math.max(
                        maxBuyAmount,
                        getAllocatedUserAmountInUSD(msg.sender)
                    )
                )
                    .mul(1e8)
                    .div(getLatestEthPrice())
