...
contract Booster is IBooster, AccessControl {
    ...
    if (currentLock.end == 0) {
      ...
      for (uint256 i = 0; i < rewardDefinitions.length; ++i) {
        if (lockPeriod >= rewardDefinitions[i].length) {
          uint256 reward = (currentLock.totalAmount.mul(rewardDefinitions[i].length).mul(rewardDefinitions[i].apr)).div(SECONDS_PER_YEAR.mul(1E18));
          ...
          emit Lock(recipient, currentLock.totalAmount.sub(reward), currentLock.totalAmount);
  ...
  function _addMore(
    ...
    uint256 reward = (amount.mul(lock_.end.sub(ts)).mul(lock_.apr)).div(SECONDS_PER_YEAR.mul(1E18));    
