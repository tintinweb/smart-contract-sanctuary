...
contract PKKT is ERC721Enumerable, Ownable {
    uint256 public constant MAX_KITTIES = 30;

    uint256 private _maxPerTx = 11; // Set to one higher than actual, to save gas on lte/gte checks.
    uint256 private _price = 0.01 * 10**16; // .01 ETH
    uint256 private _saleTime = 0; // August 23rd @ 6:30 EST
    uint256 private _free_supply = 1;

    string private _baseTokenURI;

    constructor(string memory baseURI)
        ERC721("NTF test", "PKKT")
    {
        setBaseURI(baseURI);
    }
    ...
    function mint(uint256 _count) public payable saleIsOpen {
        ...
        require(totalSupply < MAX_KITTIES, "All kitties are already minted.");
        require(
            totalSupply + _count <= MAX_KITTIES,
            "This amount of kitties will exceed max supply."
        );

        if (totalSupply < _free_supply) {
            require(_count == 1, "Only one Kitty can be mint for free");
