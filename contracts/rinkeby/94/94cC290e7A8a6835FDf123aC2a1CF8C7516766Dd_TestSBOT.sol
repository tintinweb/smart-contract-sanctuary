...
contract TestSBOT is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint256[3] public MAX_PER_STATUS = [0, 2, 10];
  ...
  // Constructor
  constructor() ERC721A(_name, _symbol, MAX_PER_STATUS[2] , TOTAL_SUPPLY) {}
  ...
  function ownerMint(address[] calldata _address_list, uint256[] calldata _qty_list) external nonReentrant onlyOwner {
    ...
    for(uint i; i < _qty_list.length; i ++){
      ...
      require(_qty_list[i] < MAX_PER_STATUS[2], "qty must be positive");
      qty_tot += _qty_list[i];
    }
  ...
  function getStatus() external view returns(string memory status_, uint qty_, uint price_, string memory msg_, uint256 available_){
    ...
    if(status == MintStatus.CLOSED){
        ...
        if(_presaleClaimed[msg.sender] < MAX_PER_STATUS[uint256(status)] ) {
            return ("PRESALE", MAX_PER_STATUS[uint256(status)] - _presaleClaimed[msg.sender], PRICE, "You are in presale", public_supply);
        } else {
            return ("PRESALE", 0, PRICE, "You already claimed your presale", public_supply);
        }
    } else {
