...
contract Staking {
    ...
    /// @notice This struct stores information regarding StakesA tokens.
    struct StakesA {
    ...
    /// @notice This struct stores information regarding StakesB tokens.
    struct StakesB {
        uint256 stakedAmount;
        uint256 blockNumber;
        uint256 totalStakes;
    ...
    /// @notice Mapping for storing stakeRewardsA Information.
    mapping(address => uint256) public stakeRewardsA;
    /// @notice Mapping for storing stakeRewardsB Information.
    mapping(address => uint256) public stakeRewardsB;
    /// @notice Mapping for storing StakesA record Information.
    mapping(address => StakesA) public stakeRecordsA;
    /// @notice Mapping for storing StakesB record Information.
    ...
    /// @notice This struct stores information regarding EmissionRate changes.
    struct Emission {
        uint256 tokenAShare;
        uint256 tokenBShare;
    ...
    /// @notice Mapping for storing Emission Information.
    ...
    /// @notice This struct stores information regarding Reward token A information.
    struct RewardsA {
        uint256 totalReward;
        uint256 count;
    }
    /// @notice Mapping for storing RewardsA Information.
    mapping(address => RewardsA) public rewardRecordsA;

    /// @notice method to initialize the contract and variables
    function init() public {
        ...
        tokenBShare = 50;
        tokenAShare = 50;
        tokenA = IBEP20(0xE7aea5a5c9D93E0bd2588Ed397dDAAE450d956ce);
        tokenB = IBEP20(0x81731E742D8b7B2D9D86c5FBC2589d3822af159F);
    ...
    /// @notice Admin method to pause the contract
    /// @param _pause boolean for pause/unpause the contract.
    function pauseContract(bool _pause) public {
        require(msg.sender == _owner, "Not authorized to use this function");
        isPause = _pause;
    }

    /// @notice Admin method to change share for tokenA and tokenB in the contract
    /// @param tokenBAmount share percentage for tokenB.
    /// @param tokenAAmount share percentage for tokenA.
    ...
    /// @notice Admin method to change tokenPerBlocks Reward
    /// @param _reward value in wei.
    ...
    /// @notice Admin method to change ownership of the contract
    /// @param newOwner address to whom you want to transfer ownership of the contract.
    function changeOwnership(address newOwner) public {
        require(msg.sender == _owner, "Not authorized to use this function");
        _owner = newOwner;
    }

    /// @notice Admin method to add the funds for the reward
    ...
    /// @notice public method to stakeTokenA in the contract
    /// @param amount value in wei for the stake tokenA.
    function StakeA(uint256 amount) public isPaused {
    ...
    /// @notice public method to unstakeTokenA from the contract
    /// @param _amount value in wei for the unstake tokenA.
    ...
    /// @notice public method to withdraw reward from the contract against stake tokenA
    function withdrawRewardA() public isPaused {
        stakeRewardsA[msg.sender] = SafeMath.add(stakeRewardsA[msg.sender], calculateRewardA(msg.sender));
        ...
        rewardRecordsA[msg.sender].totalReward = SafeMath.add(rewardRecordsA[msg.sender].totalReward, stakeRewardsA[msg.sender]);
        rewardRecordsA[msg.sender].count = SafeMath.add(rewardRecordsA[msg.sender].count, 1);
        stakeRewardsA[msg.sender] = 0;
        stakeRecordsA[msg.sender].blockNumber = block.number;
        stakeRecordsA[msg.sender].emissionRate = emissionId;
    }

    /// @notice public method to check reward value against stake tokenA
    /// @param _stakeHolder address for checking the reward.
    ...
    /// @notice public method to stakeTokenB in the contract
    /// @param amount value in wei for the stake tokenB.
    function StakeB(uint256 amount) public isPaused {
        require(amount > 0, "Create Stake");
    ...
    /// @notice public method to unstakeTokenB from the contract
    /// @param _amount value in wei for the unstake tokenB.
    ...
    /// @notice public method to withdraw reward from the contract against stake tokenB
    function withdrawRewardB() public isPaused {
        stakeRewardsB[msg.sender] = SafeMath.add(stakeRewardsB[msg.sender], calculateRewardB(msg.sender));
        ...
        rewardRecordsA[msg.sender].totalReward = SafeMath.add(rewardRecordsA[msg.sender].totalReward, stakeRewardsB[msg.sender]);
        rewardRecordsA[msg.sender].count = SafeMath.add(rewardRecordsA[msg.sender].count, 1);        
        stakeRewardsB[msg.sender] = 0;
        stakeRecordsB[msg.sender].blockNumber = block.number;
        stakeRecordsB[msg.sender].emissionRate = emissionId;
    ...
    /// @notice public method to check reward value against stake tokenB
    /// @param _stakeHolder address for checking the reward.
    ...
    /// @notice Admin method to withdraw the funds for the contract
    /// @param userAddress address where you want tokenA and tokenB from the contract.
    function emergencyWithdraw(address userAddress) public {
        require(msg.sender == _owner, "Not authorized to use this function");
