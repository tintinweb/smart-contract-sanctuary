...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, uint256 _harvestInterval, bool _withUpdate, uint256 _withdrawFee) public onlyOwner {
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, uint256 _harvestInterval, bool _withUpdate, uint256 _withdrawFee) public onlyOwner {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public payable nonReentrant {
        ...
        require(msg.sender.balance >= pool.withdrawFee, "withdraw: insufficient ETH");
        updatePool(_pid);
        payOrLockupPendingFera(_pid);
        if (_amount > 0) {
        ...
        if (pool.withdrawFee > 0) {
            feeAddress.call{value: pool.withdrawFee}("");
        }
