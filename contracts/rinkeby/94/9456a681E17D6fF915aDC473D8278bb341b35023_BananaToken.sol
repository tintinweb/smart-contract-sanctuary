...
contract BoredBananas is Ownable, ERC721Enumerable {
  ...
  uint256 public constant mintPrice = 5000000000000000; // 0.05 ETH
  ...
  function _selectBatchWinner(uint batch) private {
    ...
    uint256 winningToken = batch.sub(1).mul(1000).add(uint256(keccak256(abi.encodePacked(block.gaslimit, block.difficulty, block.timestamp))) % 1000).add(1);
  ...
  function _selectGoldenBanana() private {
    uint256 winningToken = (uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, block.gaslimit))) % 10000).add(1);
    winningTokens.push(winningToken);
