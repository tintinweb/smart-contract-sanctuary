...
contract CyOp is Context, IERC20, Ownable {
    ...
    uint256 public protocolFee = 9; //gowth hacking(4%) + protocol(6%)
    uint256 private prevProtocolFee = 9; //gowth hacking(4%) + protocol(6%)
    address payable public treasury1; //4%
    address payable public treasury2; //6%
    
    string private constant _name = "CyOp";
    ...
    constructor (address _treasury1, address _treasury2) {
        ...
        _isExcludedFromFee[treasury1] = true;
        _isExcludedFromFee[treasury2] = true;
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        ...
        require (owner == Ownable.owner() || (sellingEnabled && block.timestamp > contractStartTime + 15 minutes), "Selling is disabled.");
    ...
    function sendETHToTreasury(uint256 amount) private {
        treasury1.transfer((amount.mul(treasury1Fee).div(protocolFee)).div(100));
        treasury2.transfer((amount.mul(treasury2Fee).div(protocolFee)).div(100));
    }
