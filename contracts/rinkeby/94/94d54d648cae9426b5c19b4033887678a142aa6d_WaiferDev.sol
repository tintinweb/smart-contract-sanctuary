...
contract WaiferDev is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    function _transfer(
        ...
        if(overMaxTokenBalance && marketingFeeEnabled) {
            ...
            uint256 marketingAmount = marketingWalletAmount * 10 ** 18;
                 // swap tokens in contract address to eth
                swapTokensForEth(marketingAmount, address(this));
                address(this).balance - marketingAmount;
                // Send eth to Marketing address
                transferBNBToAddress(payable(marketingWalletAddress), marketingAmount);
         ...
         if(overMaxTokenBalance && liquidityFeeEnabled && from != uniswapV2Pair){
                ...
                uint256 liquidityAmount = liquidityPercentageAmount * 10 ** 18;
                swapAndLiquify(liquidityAmount, owner());
                if(taxDisableInLiquidity){
                    enableFee = true;
                    taxDisableInLiquidity = false;
                }
