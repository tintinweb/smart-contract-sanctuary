...
contract LNFT1 is ERC721Enumerable, Ownable, ERC721Burnable {
    using Strings for uint256;

    // Variables
    string baseURI;
    string public baseExtension = ".json"; // Base extension of metadata file
    uint256 public cost = 0.0001 ether; // Cost of each NFT
    uint256 public maxSupply = 9999; // Max amount of NFTs that can be minted in total
    uint256 public maxMintAmount = 20; // Max amount to be minted at once
    bool public paused = false; // Contract to be paused/unpaused when deployed
    uint256 public nftPerAddressLimit = 20; // Maximum NFTs mintable on one wallet
    ...
    function mintFor(address to, uint256 _mintAmount) public payable {
        uint256 supply = totalSupply();
        require(!paused, "the contract is paused");
        if (onlyWhitelisted == true) {
            require(isMember(to), "Wallet address is not yet whitelisted.");
        }
        require(_mintAmount > 0, "need to mint at least 1 NFT");
        require(_mintAmount <= maxMintAmount, "max mint amount per session exceeded");
        require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");
    ...
    // Checks if whitelisted minting only
    function isWhitelistedOnly() public view returns (bool) {
    ...
    function addMemberList(address[] memory _memberList) public {
