...
contract XCuteNativeCurrencyPriceOracle is IXCuteNativeCurrencyPriceOracle {
    ...
    constructor(
        ...
        pair = _pair;
        observation = Observation({timestamp: 0, price: 0, lastCumulative: 0});
    }
    ...
    function work() external override {
        ...
        uint224 _averagePriceCumulative;
        unchecked {
            _averagePriceCumulative = uint224(
                (_priceCumulative - observation.lastCumulative) / _timeElapsed
            );
        }
...
interface IXCuteNativeCurrencyPriceOracle is IJob {
    function quote() external returns (uint256);
}
