...
contract Bridge{
    ...
    mapping (address /*receiver*/ => mapping (uint256 /*amount*/ => mapping(uint256 /*fee*/ => mapping (uint256 /*index*/ => mapping (address /*token Address From*/ => uint256 /*Index*/))))) private _receiveOrderIndex;    
    //================================================================================================================================================================================
    ...
    mapping (uint256 => uint256) private _requestFee;
    //================================================================================================================================================================================
    ...
    function contractsOf(address contractAddress) external view returns(address parallelContract, address mainContract){
        parallelContract = _parallelContract[contractAddress];
        mainContract = _mainContract[contractAddress];
    }
    
    function contractsOfThat(address contractAddress) external view returns(address parallelContract, address mainContract){
        parallelContract = _parallelContractOfThat[contractAddress];
        mainContract = _mainContractOfThat[contractAddress];
    }
    //================================================================================================================================================================================    
    
    function receiveOrderIndex(address receiver, uint256 amount, uint256 fee, uint256 index, address tokenAddressFrom) external view returns(uint256) {
        return _receiveOrderIndex[receiver][amount][fee][index][tokenAddressFrom];
    }
    function addTokenOrderIndex(address owner, address main, address parallel, uint256 fee) external view returns(uint256) {
        return _addTokenIndex[owner][main][parallel][fee];
    }
    ...
    function doubleSendStats(uint256 ID, uint256 ID1) external view returns(address sender, address receiver, uint256 amount, uint256 fee, address tokenAddressFrom, address sender1, address receiver1, uint256 amount1, uint256 fee1, address tokenAddressFrom1){
    ...
    function orderProgress(uint256 ID, uint256 ID1) external view returns(uint256 votedFT, bool processed, uint256 votedFT1, bool processed1){
        votedFT = _votedFT[ID];
        processed = _processed[ID];
        votedFT1 = _addTokenVotedFT[ID1];
        processed1 = _addTokenProcessed[ID1];
        
    }
    function depositOrderProgress(address receiver, uint256 amount, uint256 fee, uint256 index, address tokenAddressFrom) external view returns(bool){
        return _processed[_receiveOrderIndex[receiver][amount][fee][index][tokenAddressFrom]];
    }
    ...
    function signTransaction(uint256 ID, address signer, address receiver, uint256 amount, uint256 fee, address tokenAddressFrom) internal returns(bool) {
        uint256 totalVoted;
    ...
    function transfer(uint256 amount, uint256 fee, address to, address tokenAddressFrom) external {
        require(amount > 0);
        ...
        if(tokenAddressFrom == address(0)){
            ...
            FT.burn(amount+fee);
            _sendTokenAddressFrom[index] = address(0);
        }
        else{
            IERC20 token = IERC20(tokenAddressFrom);
            token.transferFrom(msg.sender, address(this), amount);
            FT.transferFrom(msg.sender, address(this), fee);
            if(_mainContract[tokenAddressFrom] != address(0)){token.burn(amount); FT.burn(fee); _sendTokenAddressFrom[index] = _mainContract[tokenAddressFrom];}
            else{_sendTokenAddressFrom[index] = _parallelContract[tokenAddressFrom];}
        }
    ...
    function deposit(address receiver, uint256 amount, uint256 fee, uint256 index, address tokenAddressFrom) external {
        uint256 orderIndex = _receiveOrderIndex[receiver][amount][fee][index][tokenAddressFrom];
        ...
        if(orderIndex == 0){
            ...
            _receiveOrderIndex[receiver][amount][fee][index][tokenAddressFrom] = ordersCount;
        ...
        if(_votedFT[orderIndex] * 2 >= _totalStakedFT){
            ...
            signTransaction(orderIndex, msg.sender, receiver, amount, fee, tokenAddressFrom);
        }

    }
    
    function claim(address receiver, uint256 amount, uint256 fee, uint256 index, address tokenAddressFrom) external {
        uint256 orderIndex = _receiveOrderIndex[receiver][amount][fee][index][tokenAddressFrom];
        ...
        if(_processed[orderIndex]){
            ...
            if(signTransaction(orderIndex, msg.sender, receiver, amount, fee, tokenAddressFrom)){
                if(_mainContractOfThat[tokenAddressFrom] != address(0)){ //sending from the parallel contract back to the main
                    IERC20 token = IERC20(_mainContractOfThat[tokenAddressFrom]);
                    token.transfer(receiver, amount);
                }
    ...
    function voteOnProposal(uint256 ID) external returns(bool){
        ...
        if(votedFT * 2 >= _totalStakedFT){
            ...
            uint256 requests = _addTokenRequests;
            _owner[requests] = address(0);
            _addTokenMainContract[requests] = _main[ID];
            _addTokenParallelContract[requests] = _parallel[ID];
            _requestFee[requests] = _fee[ID];
