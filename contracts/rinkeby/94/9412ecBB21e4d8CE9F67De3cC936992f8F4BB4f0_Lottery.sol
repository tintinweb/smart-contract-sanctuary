...
contract LotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("Pyro Lottery Ticket", "JLT") {}
...
// 4 numbers
contract Lottery is ERC20('Pyro Lottery Pool Token', 'JLPT') {
    using SafeMath for uint256;
    ...
    // Price Fee address // Plan is to use for donations to charity.
    address public priceFeeAddress;
    // Price fee in basis points initially set to 5%
    uint16 priceFeeBP;
    // Max Price fee: 10%.
    uint16 public constant MAXIMUM_PRICE_FEE_BP = 1000;
    // =================================
    ...
    event SetFeeAddress(address indexed _oldAddress, address indexed _newAddress);
    event PriceFeeBpChanged(uint256 _oldBp, uint256 _newBp);
    ...
    Ownable()
    {
        require(_priceFeeBP <= MAXIMUM_PRICE_FEE_BP, "add: invalid price fee basis points");
        priceFeeAddress = _priceFeeAddress;
        priceFeeBP = _priceFeeBP;
    ...
    function reset(uint256 _price) external onlyOwner {
    ...
    function internalBuy(uint256 _price, uint8[4] memory _numbers) internal {
        ...
        grandTotal = grandTotal.add(_price);
        lastTimestamp = block.timestamp;
        emit Buy(address(this), tokenId);

    }

    function injectIntoInitialPot(uint256 _price) internal {
        require (_price >= minPrice, 'price must above minPrice');
        pyro.safeTransferFrom(address(msg.sender), address(this), _price);
        internalBuy(_price, nullTicket);
    }

    function injectIntoPot(uint256 _price) external inDrawingPhase {
        require (_price >= minPrice, 'price must above minPrice');
        pyro.safeTransferFrom(address(msg.sender), address(this), _price);
        internalBuy(_price, nullTicket);
    }
    ...
    function multiBuy(uint256 _price, uint8[4][] memory _numbers) external inDrawingPhase {
        ...
        pyro.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if (reward>0) {
            uint256 priceFee = reward.mul(priceFeeBP).div(10000);
            reward = reward.sub(priceFee);
            //Send to price fee address
            pyro.safeTransfer(address(priceFeeAddress), priceFee);
            //Send to user
            pyro.safeTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            uint256 priceFee = totalReward.mul(priceFeeBP).div(10000);
            totalReward = totalReward.sub(priceFee);
            //Send to price fee address
            pyro.safeTransfer(address(priceFeeAddress), priceFee);
            //Send to user
            pyro.safeTransfer(address(msg.sender), totalReward);
        }
    ...
    function getRewardView(uint256 _tokenId) public view returns(uint256) {
        ...
        if (matchingNumber > 1) {
            ...
            reward = amount.mul(1e12).div(getMatchingRewardAmount(_issueIndex, matchingNumber)).mul(poolAmount); // scaling is done before
        }
    ...
    function adminWithdraw(uint256 _amount) external onlyOwner {
        pyro.safeTransfer(address(msg.sender), _amount);
    ...
    function setPriceFeeAddress(address _priceFeeAddress) public onlyOwner {
        require(_priceFeeAddress != address(0), "setPriceFeeAddress: invalid address");
        address oldPriceFeeAddress = priceFeeAddress;
        priceFeeAddress = _priceFeeAddress;
        emit SetFeeAddress(oldPriceFeeAddress, _priceFeeAddress);
    }

    function updatePriceFeeBP(uint16 _priceFeeBP) public onlyOwner {
        require(_priceFeeBP <= MAXIMUM_PRICE_FEE_BP, "add: invalid price fee basis points");
        require(_priceFeeBP != priceFeeBP, "updatePriceFeeBP: same fee bp set");
        uint256 previousPriceFeeBP = priceFeeBP;
        priceFeeBP = _priceFeeBP;
        emit PriceFeeBpChanged(previousPriceFeeBP, _priceFeeBP);
    }
}