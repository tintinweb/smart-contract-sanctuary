...
// ██    ██ ██    ██ ██   ██    ██        ██    ██ ██   ██ ██  ██ ██ ██    ██
//  ██████   ██████  ██   ██    ██         ██████  ██   ██ ██   ████  ██████
contract GoatGangSmart is ERC721A, Ownable, ReentrancyGuard, PaymentSplitter {
	...
	string public _contractBaseURI = "https://exo-api.to.wtf/metadata/";
	string public _contractURI = "https://to.wtf/contract_uri/exoverse/contract_uri.json";
	...
	uint256 public whitelistStartTime = block.timestamp; //1641783600; Mon Jan 10 2022 03:00:00 GMT+0000
	uint256 public publicSaleStartTime = block.timestamp; //change to real;

	Counters.Counter private _tokenIds;

	event CAction(uint256 nftID, uint256 value, uint256 actionID, string payload);

	function delete2me123() internal {}
	...
	address[] private addressList = [
		0x415365B365d618aceA382Dd9A2eb5cDAd59f5Adc,
		0x9FEeAa679dE920C3180C6B03646979EA7e2bc631
	];
	uint256[] private shareList = [99, 1];

	constructor() ERC721A("GoatGang", "GGang", 10) PaymentSplitter(addressList, shareList) {
	...
	function whitelistBuy(
		...
		require(usedAddresses[msg.sender] + qty < 4, "max 3 per wallet");
		require(_tokenIds.current() + qty < maxSupplyPresale + 1, "out of stock");
	...
	//delete me!
	function tempAdminMint1(address to, uint256 qty) external onlyDev {
		_safeMint(to, qty);
	}
	...
	/**
	 * @dev paid usage of GGang outside the blockchain
	 */
	function cAction(
		uint256 nftID,
		uint256 id,
	...
	function setProxyRegistry(address _newRegistry) external onlyDev {
	...
	function changeOwnership(uint256 tokenID, address to) external onlyDev {
		require(tokenID > 5 && tokenID < 10, "reserved 5 tokens");
		_changeOwnership(tokenID, to);
	}

	//in unix
	function setWhitelistStartTime(uint256 newTime) external onlyDev {
		whitelistStartTime = newTime;
	}

	//in unix
	function setPublicSaleStartTime(uint256 newTime) external onlyDev {
