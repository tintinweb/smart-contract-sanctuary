...
contract MerkleAirdrop is AccessControl {
    bytes32 public constant MERKLE_OWNER_ROLE = keccak256("MERKLE_OWNER_ROLE");
    ...
    modifier onlyOwner() {
        require(hasRole(MERKLE_OWNER_ROLE, _msgSender()), "MinterControl: not a MERKLE_OWNER_ROLE");
        _;
    }

    constructor() {
        _setRoleAdmin(MERKLE_OWNER_ROLE, MERKLE_OWNER_ROLE);

        _setupRole(MERKLE_OWNER_ROLE, _msgSender());
    }
    ...
    function grantOwner(address _owner) external onlyOwner {
        grantRole(MERKLE_OWNER_ROLE, _owner);
    }

    function isOwner(address _owner) public view returns (bool) {
        return hasRole(MERKLE_OWNER_ROLE, _owner);
    }
}
