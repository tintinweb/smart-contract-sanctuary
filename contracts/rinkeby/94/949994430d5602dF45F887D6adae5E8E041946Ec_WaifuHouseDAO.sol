...
contract WaifuHouseDAO is WaifuHouseDAOCore, IERC721Batch, IERC721Enumerable {
  ...
  function _mint(address to, uint tokenId) internal override{
    ...
    owners[ tokenId ].baseHearts = 10;
    owners[ tokenId ].epoch = block.timestamp;
...
abstract contract WaifuHouseDAOCore is ERC165, IERC721Metadata, Signed {
  ...
  uint public HEARTS_MULTI = 1;
  uint public HEARTS_PERIOD = 86400;
  ...
  function getHearts( uint tokenId ) public view returns( uint hearts ){
    uint extra = (block.timestamp - owners[tokenId].epoch) / HEARTS_PERIOD;
    return owners[tokenId].baseHearts + extra * HEARTS_MULTI;
  }
  ...
  function setHearts(uint[] calldata tokenIds, uint[] calldata hearts) external onlyDelegates{
  ...
  function setHeartsOptions( uint multiplier, uint period ) external onlyDelegates{
    HEARTS_MULTI = multiplier;
...
contract Signed is Delegated{
  ...
  function createHash( string memory data ) public view returns ( bytes32 ){
  ...
  function isAuthorizedSigner( address extracted ) public view virtual returns( bool ){
    return extracted == _signer;
  }

  function verifySignature( string memory data, bytes calldata signature ) public view {
