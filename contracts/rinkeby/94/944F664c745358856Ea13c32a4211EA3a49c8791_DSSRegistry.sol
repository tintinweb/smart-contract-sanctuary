...
contract DSSRegistry is Ownable,ERC1155Base {
    ...
    constructor(string memory _name,
                ...
                address _dao) ERC1155Base(contractURI, tokenURIPrefix) public {
        name = _name;
        symbol = _symbol;
    ...
    function addDev(address _dev) external onlyDAO {
      for(uint256 i = 0 ; i < scDevs.length;i++){
        require(scDevs[i] != _dev,"Dev already added");
      }
      scDevs.push(_dev);
    }

    function removeDev(address _dev) external onlyDAO {
      require(scDevs.length > 2,"Require to keep at least 2 devs");
      for(uint256 i = 0 ; i < scDevs.length;i++){
        if(scDevs[i] == _dev){
          delete scDevs[i];
          scDevs.pop();
