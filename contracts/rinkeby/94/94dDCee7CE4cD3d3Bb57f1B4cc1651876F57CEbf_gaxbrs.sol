...
contract gaxbrs is ERC721PresetMinterPauserAutoId, VRFConsumerBase, Data  {
    ...
    mapping(uint256 => Device) public a_unit;
    ...
    // address public tokenAddress = 0xdbfb96a79efFEA8Ce8D4abc8B177ab03fef2C450;
    ...
    uint256 internal ONE_HOUR = 1; //10min
    uint256 internal HALF_HOUR = 1; //5min
    uint256 internal MAX_TIMESTAMP = 2**256 - 1;
    uint256 internal MAX_CALC = 16;
    ...
    constructor(
        ...
        ERC721PresetMinterPauserAutoId("paignbu", "lnd", "ipfs://") 
    {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        Device memory device = a_unit[tokenId];
    ...
    function addPasses(uint256 tokenId, uint256 amount) public onlyOwner {
        Device storage device = a_unit[tokenId];
    ...
    function x_play(uint256 tokenId) public {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "!o");
        require(alive(tokenId), "!x");
        
        Device storage device = a_unit[tokenId];
        
        uint256 timeBetweenPlays = ONE_HOUR.add(device.prestige);
    ...
    function generationUp(uint256 tokenId) internal {
        ...
        uint256 maxPlaysPerGen; // = device.generation.mul(2).add(4); // 4,6,8,10 ect..
        // uint256 MAX_CALC = 16;
        uint256 amount;
        
        // if !prestige 16, calculate reward. Else on prestige 16+, rewards set to 1 indefinitely
        if (device.prestige < MAX_CALC) {
            maxPlaysPerGen = device.generation.mul(2).add(4);
            amount = (device.generation.mul(reward)).mul(device.prestige.add(1)).add(reward);
        } else {
            maxPlaysPerGen = 2;
            amount = reward;
        }
    ...
    function a_getTimeData(uint256 tokenId) public view returns (
        ...
        if (device.createdAt != MAX_TIMESTAMP && alive(tokenId)) {
            ...
            // in a play
            if (block.timestamp.sub(device.lastPlayTime) > ONE_HOUR.add(device.prestige)) {
                elapsed = block.timestamp.sub(device.lastPlayTime);
                // not in a feed
                if (device.feedLevel > fullCycles) {
                    return (true, 0, ONE_HOUR.add(device.prestige), maxPlaysPerGen,  false, device.feedLevel.mul(ONE_DAY).sub(feedElapsedTime), feedWindow );
                }
                // in feeding window
                if (device.feedLevel == fullCycles && modulo < feedWindow)  {
                    return (true, 0, ONE_HOUR.add(device.prestige), maxPlaysPerGen, true, 0,  feedWindow.sub(modulo));
                }
            //not in a play
            } else {
                elapsed = block.timestamp.sub(device.lastPlayTime);
                // not in a feed
                if (device.feedLevel > fullCycles) {
                    return (false, ONE_HOUR.add(device.prestige).sub(elapsed), ONE_HOUR.add(device.prestige), maxPlaysPerGen, false, device.feedLevel.mul(ONE_DAY).sub(feedElapsedTime), feedWindow);
                }
                // in feeding window
                if (device.feedLevel == fullCycles && modulo < feedWindow)  {
                    return (false, ONE_HOUR.add(device.prestige).sub(elapsed), ONE_HOUR.add(device.prestige), maxPlaysPerGen, false, 0, feedWindow.sub(modulo));
                }
            }
        }
