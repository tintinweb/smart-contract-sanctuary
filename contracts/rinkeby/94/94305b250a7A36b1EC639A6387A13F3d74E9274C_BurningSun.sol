...
contract BurningSun is ERC721Enumerable, Ownable {
    ...
    constructor(uint256 _maxSupply, uint16 _reserveCount)
        ERC721("BurningSun token", "BURNING")
    {
        require(
    ...
    function publicMint(uint8 quantity) external payable saleIsOpen {
        ...
        require(
            _nextTokenId + 1 <= maxSupply,
            "BurningSun: max supply exceeded"
        );
        require(
            tokenPrice * 1 * quantity == msg.value,
    ...
    /// @dev not needed for production
    function getTokensReserved() public view returns (uint256) {
    ...
    function getProvenanceHash() public view returns (string memory) {
        return BURNING_SUN_PROVENANCE;
    }

    /// @dev not needed for production
    function getWhitelister() public view returns (address) {
        return whitelister;
    }
