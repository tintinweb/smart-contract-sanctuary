...
contract Uniwine is ERC721, ERC721Enumerable, ERC721Burnable {
    ...
    event Redeemed(uint indexed tokenId);

    ERC20Burnable fungibleContract;
    string tokenBaseUri;
    mapping (uint => bool) redeemedStatus;
    ...
    function tokenURI(uint tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked(
            ...
            isRedeemed(tokenId) ? '-redeemed.json' : '.json'
        ));
    }
    ...
    function isRedeemed(uint tokenId) public view virtual returns (bool) {
        return redeemedStatus[tokenId];
    }

    function convertToRedeemed(uint[] memory tokenIds) public {
        for (uint i=0; i<tokenIds.length; i++) {
            uint tokenId = tokenIds[i];
            require(!isRedeemed(tokenId), "Already redeemed");
            redeemedStatus[tokenId] = true;
            console.log("Converted to redeemed: %s", tokenId);
