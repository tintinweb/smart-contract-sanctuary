...
contract NFTScanner is Context {
    ...
    function getOGNFTInfo() external view returns (OGNFT[] memory) {
        uint256 _balance = ogNFT.balanceOf(_msgSender());

        OGNFT[] memory ogNFTInfo = new OGNFT[](_balance);

        for (uint256 i = 0; i < _balance; i++) {
            uint256 _nftTokenId = ogNFT.tokenOfOwnerByIndex(_msgSender(), i);
    ...
    function getOGNFTStakingInfo() public view returns (OGNFT[] memory) {
    ...
    function getBalance() external view returns (uint256) {
        uint256 balance = latteNFT.balanceOf(_msgSender());
        return balance;
    }

    function getBoosterInfo() external view returns (BoosterNFT[] memory) {
        uint256 balance = latteNFT.balanceOf(_msgSender());
        BoosterNFT[] memory boosterNFTInfo = new BoosterNFT[](balance);

        for (uint256 i = 0; i < balance; i++) {
            uint256 _nftTokenId = latteNFT.tokenOfOwnerByIndex(_msgSender(), i);
    ...
    function getBoosterStakingInfo(address[] memory _stakeTokens)
        external
        view
