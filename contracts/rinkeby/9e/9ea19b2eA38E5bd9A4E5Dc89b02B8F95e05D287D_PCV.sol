...
contract PCV {
    ...
    function convertLPToToken(address _lpPair, address _tokenDesired) public onlyGov {
        ...
        (address _tokenA, address _tokenB, uint256 amountA, uint256 amountB) = removeLiquidity(_lpPair, liquidity);
        if (!isReserveAsset[_tokenA]) {
            swap(_tokenA, _tokenDesired, amountA);
        }

        if (!isReserveAsset[_tokenB]) {
            swap(_tokenB, _tokenDesired, amountB);
        }
    }
