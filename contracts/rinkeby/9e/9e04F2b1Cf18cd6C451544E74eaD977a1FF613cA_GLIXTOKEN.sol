...
// by @berkozdemir aka princess camel aka guerilla pimp minion god bastard
                                                             
// A UTILITY TOKEN FOR GLICPIXXX RELATED ARTSY FART THINGIES

// THE TOKEN IS DEPLOYED ALONGSIDE "ARTGLIXXX VOLUME ONE" - ON-CHAIN GENERATIVE ART COLLECTION

// $GLIX WILL BE YIELDED FROM GENESIS ARTGLIXXX TOKEN AND WILL BE BURNED TO GIVE BIRTH TO NEWER GENERATIONS
...
contract GLIXTOKEN is ERC20, Ownable {
    ...
    // THESE WHITELISTS ARE GIVEN TO ARTGLIXXX AND GLIX UTILITY CONTRACTS
    mapping (address => bool) public minterPermission;
    mapping (address => bool) public burnerPermission;

    event minterPermissionGiven(address _address);
    event minterPermissionRemoved(address _address);
    event burnerPermissionGiven(address _address);
    event burnerPermissionRemoved(address _address);
