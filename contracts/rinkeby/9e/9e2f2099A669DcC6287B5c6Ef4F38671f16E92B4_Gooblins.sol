...
import "./interfaces/IGooblins.sol";
import "./interfaces/IMine.sol";
import "./interfaces/ITraits.sol";

contract Gooblins is IGooblins, ERC721Enumerable, Ownable, Pausable {
    struct LastWrite {
        uint64 time;
        uint64 blockNum;
    }
    ...
    uint256 public PAID_TOKENS; // number of tokens that can be claimed for a fee - 10,000
    uint16 public override minted; // number of tokens have been minted so far

    event VillagerMinted(uint256 indexed tokenId);
    event GoblinMinted(uint256 indexed tokenId);
    event VillagerStolen(uint256 indexed tokenId);
    event GoblinStolen(uint256 indexed tokenId);
    event VillagerBurned(uint256 indexed tokenId);
    event GoblinBurned(uint256 indexed tokenId);

    string public baseURI;

    // mappings
    mapping(uint256 => VillagerGoblin) private tokenTraits; // mapping from tokenId to a struct containing the token's traits
    ...
    mapping(address => LastWrite) private lastWriteAddress; // Disallow some access to functions if they occur while a change is being written.
    mapping(uint256 => LastWrite) private lastWriteToken;

    // Probabilities & Aliases
    // 0 - 8 are associated with villagers, 9 - 13 are associated with Goblins
    uint8[][18] public rarities;
    uint8[][18] public aliases;

    IMine public mine; // STAKING - reference to the Mine for choosing random Bear thieves
    ...
    constructor(uint256 _maxTokens)
        ERC721("Ghost Town Goblins", "GoblinsGame")
    {
    ...
    modifier requireContractsSet() {
        require(
            address(traits) != address(0) && address(mine) != address(0),
            "Contracts not set"
        );
        _;
    }
    ...
    function setContracts(address _traits, address _mine) external onlyOwner {
    ...
    // Main Mint Functions no payment or game logic
        ...
        whenNotPaused
    {
        ...
        if (tx.origin != recipient && recipient != address(mine)) {
            // Stolen!
            if (tokenTraits[minted].isVillager) {
                emit VillagerStolen(minted);
            } else {
                emit GoblinStolen(minted);
            }
        }
    ...
    function burn(uint256 tokenId) external override whenNotPaused {
        ...
        if (tokenTraits[minted].isVillager) {
            emit VillagerBurned(tokenId);
        } else {
            emit GoblinBurned(tokenId);
        }
        _burn(tokenId);
    }
    ...
    function generate(
        ...
        uint256 seed,
        LastWrite memory lw
    ) internal returns (VillagerGoblin memory t) {
        t = selectTraits(seed);
        if (existingCombinations[structToHash(t)] == 0) {
            tokenTraits[tokenId] = t;
            existingCombinations[structToHash(t)] = tokenId;
            if (t.isVillager) {
                emit VillagerMinted(tokenId);
            } else {
                emit GoblinMinted(tokenId);
            }
            return t;
        }
    ...
    function selectTraits(uint256 seed)
        internal
        view
        returns (VillagerGoblin memory t)
    {
        t.isVillager = (seed & 0xFFFF) % 10 != 0;
        uint8 shift = t.isVillager ? 0 : 9; // 0 if its a Villager, 9 if its Goblin
        seed >>= 16;
     ...
     */
    function structToHash(VillagerGoblin memory s)
        internal
        pure
        returns (uint256)
    {
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        tx.origin,
                        blockhash(
                            blockNumber > 1 ? blockNumber - 2 : blockNumber
                        ), // Different block than Game Logic to ensure if needing to re-randomize that it goes down a different path
                        timestamp,
                        seed
                    )
                )
     ...
     * @return villager - whether or not a token is a villager
     */
    function isVillager(uint256 tokenId)
        external
        view
        override
        ...
        returns (bool)
    {
        // Sneaky goblins will be oofed if they try to peep this after mint. Nice try.
        IGooblins.VillagerGoblin memory s = tokenTraits[tokenId];
        return s.isVillager;
    }
...
interface IMine {
	function addManyToMineAndTheiving(
		address account,
		uint16[] calldata tokenIds
	) external;

	function randomGoblinOwner(uint256 seed) external view returns (address);
}
...
// game data storage
interface IGooblins is IERC721Enumerable {
	struct VillagerGoblin {
		bool isVillager;
		uint8 fur;
		uint8 head;
		uint8 ears;
	...
	function isVillager(uint256 tokenId) external view returns (bool);
}
