...
contract Tiger is ERC721A, Ownable, ReentrancyGuard {
    /// ============ Immutable storage ============
    ...
    /// @notice Creates a new Tiger contract
    ...
    constructor(
        ...
        uint256 _mintPrice
    ) ERC721A("The Tiger Archives", "TIGER", _maxPublicBatchPerAddress, _collectionSize) {
        require(_amountForPromotion <= _collectionSize, "Tiger/invalid-promotion-amount");
        require(_mintPrice > 0, "Tiger/invalid-mint-price");
    ...
    ) external payable isMinterValid isMintLive(presaleTime) {
        require(invocations > 0, "Tiger/invalid-invocation-lower-boundary"); // todo: check?
        require(totalSupply() + invocations <= collectionSize, "Tiger/invalid-total-supply");
        require(_numberMinted(msg.sender) + invocations <= maxInvocation, "Tiger/invalid-invocation-upper-boundary");
        require(msg.value == mintPrice * invocations, "Tiger/invalid-mint-value");
        require(
            MerkleProof.verify(proof, presaleMerkleRoot, keccak256(abi.encodePacked(msg.sender, maxInvocation))),
            "Tiger/invalid-address-proof"
        );
    ...
    function publicMint(uint256 invocations) external payable isMinterValid isMintLive(publicTime) {
        require(_numberMinted(msg.sender) + invocations <= maxPublicBatchPerAddress, "Tiger/invalid-mint-invocation");
        require(totalSupply() + invocations <= collectionSize, "Tiger/invalid-total-supply");
        require(msg.value == mintPrice * invocations, "Tiger/invalid-mint-value");
        _safeMint(msg.sender, invocations);
        emit Created(msg.sender, invocations);
    ...
    function setMintTime(uint256 _presaleTime, uint256 _publicTime) public onlyOwner {
        require(_presaleTime > _currentTime(), "Tiger/invalid-presale-time");
        require(_publicTime > _presaleTime, "Tiger/invalid-public-time");
    ...
    /// @notice ensures that contracts cant mint
    /// @dev this is validation against botters that create many contracts to mint
    modifier isMinterValid() {
        require(tx.origin == msg.sender, "Tiger/invalid-mint-caller");
        _;
    }
    ...
    modifier isMintLive(uint256 time) {
        require(time > 0 && block.timestamp > time, "Tiger/invalid-mint-time");
        _;
    }
    ...
    function promotionMint(uint256 invocations) external onlyOwner {
        require(totalSupply() + invocations <= amountForPromotion, "Tiger/invalid-promotion-supply");
        require(invocations % maxBatchSize == 0, "Tiger/invalid-batch-multiple");
    ...
    function withdrawMoney() external onlyOwner nonReentrant {
        ...
        require(success, "Tiger/invalid-withdraw-money");
    }
