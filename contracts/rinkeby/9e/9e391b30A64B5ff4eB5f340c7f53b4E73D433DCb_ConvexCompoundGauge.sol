...
interface IBooster {
    // function proxyUsers(address _user) external view returns (address);
...
contract ConvexCompoundGauge {
    ...
    constructor(address _convexBooster, address _compoundBooster) public {
        ...
        // calculateCurveAndCompoundPriceModel = _calculateCurveAndCompoundPriceModel;
    }
    ...
    function borrow(
        ...
        // (uint256 token0Amount, uint256 token1Amount) = ICalculateCurveAndCompoundPriceModel(calculateCurveAndCompoundPriceModel).getAmount(
        //     pool.convexPid,
        //     _token0,
        //     _supportPid
        // );
        // uint256 token0Price = getToken0Price(_token0);
        (address swapAddress, , , , ) = IConvexBooster(convexBooster).poolInfo(
            pool.convexPid
        );
        uint256 token0Price = ICurveSwap(swapAddress).calc_withdraw_one_coin(
            _token0,
            pool.curveCoinIds[_supportPid]
        );

        uint256 lendingAmount = token0Price
            .sub(token0Price.mul(pool.lendingThreshold).div(1000))
            .sub(token0Price.mul(80).div(100));

        uint256 utilizationRate = getUtilizationRate(
            IBooster(compoundBooster).totalSupplyOf(_pid),
            lendingAmount,
            0
        );
        ...
        uint256 interestAmount = lendingAmount
            .mul(interestRate)
            .mul(_borrowNumbers)
            .div(1e18);
        bytes16 lendingId = generateId(msg.sender, _pid, pool.borrowIndex);

        ICompoundBooster(compoundBooster).lockToken(
            _pid,
            lendingId,
            msg.sender,
            lendingAmount
    ...
    function _repayBorrow(
        ...
        if (isErc20) {
            // ICompoundBooster(compoundBooster).unLockToken{value: msg.value}(
            ...
            ICompoundBooster(compoundBooster).unLockTokenErc20(
                userLending.lendingId,
                lendingInfo.user,
                userLending.lendingAmount,
                userLending.interestValue
    ...
    function DemoAddPool() public {
        ...
        address _interest = 0x358AA13c52544ECCEF6B0ADD0f801012ADAD5eE3;

        uint256 _lendingThreshold = 1000;

        uint256[] memory supportPids = new uint256[](2);
        supportPids[0] = 0;
        supportPids[1] = 1;
