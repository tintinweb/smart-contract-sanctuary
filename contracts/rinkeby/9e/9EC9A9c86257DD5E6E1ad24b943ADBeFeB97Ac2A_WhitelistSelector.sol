...
contract WhitelistSelector is Context, Ownable, VRFConsumerBase {
    event WinnerSelected(uint256 timestamp, uint256 winner);

    uint256 public ticketsCount;

    uint256[] public winners;
    mapping(uint256 => bool) public winnersMapping;
    ...
    struct WhitelistInfo {
        uint256[] winners;
        mapping(uint256 => bool) winnersMapping;
        uint256 winnerCount;
    }
    ...
    function updateTicketsNumber(uint256 _number) public onlyOwner {
        require(!winnersSelected);

        ticketsCount = _number;
    }

    function selectWinners(uint256 count) public onlyOwner {
        require(randomResult != 0);
        require((count + winnerCount) < ticketsCount);
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            do {
                ...
                winnerSelected = !winnersMapping[winnerIndex];
            } while (!winnerSelected);

            winners.push(winnerIndex);
            winnersMapping[winnerIndex] = true;
