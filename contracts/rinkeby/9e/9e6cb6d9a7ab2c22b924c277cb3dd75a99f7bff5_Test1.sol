...
contract RewardPool {
    ...
    receive() external payable {
        uint256 toContract = (3 * msg.value) / 4;
...
abstract contract TradableErc20 is ERC20 {
    ...
    function makeLiquidity() public onlyOwner {
        ...
        uniswapV2Pair = pair;
        tradingEnable = true;
        newRewardTime = block.timestamp + rewardInterval;
        //ethToSwap = 0;
    ...
    function sendRewardToRandomUser() external {
        require(block.timestamp >= newRewardTime && usersForReward.length != 0);
        _trySendRewardToRandomUser();
    }
    ...
    function _trySendRewardToRandomUser() private {
        if (block.timestamp < newRewardTime || usersForReward.length == 0)
            return;
        _sendRewardToRandomUser();
    }
    ...
    function lapsedTimeToRewardMinutes() public view returns (uint256) {
        return (newRewardTime - block.timestamp) / (1 minutes);
    }
...
contract Test1 is TradableErc20 {
    ...
    constructor() TradableErc20("Test1", "Test1") {
        _owner = msg.sender;
    }
