...
contract SunriseToken is ISunriseToken, Ownable, ERC721Enumerable {
    ...
    // The Sunrise Art Club address (creators org)
    ...
    /**
     * @notice Mint a Sunrise to the minter.
     * Sunrise core team and community reward Sunrises are minted
     * every 10 Sunrises, starting at 1, until 36 Sunrises
     * have been minted (1 year w/ 24 hour auctions).
     * @dev Call _mintTo with the to address(es).
     */
    function mint() public override onlyMinter returns (uint256) {
        require(_currentSunriseId <= maxSupply, 'Max supply reached');
        if (_currentSunriseId <= maxSupply && _currentSunriseId % 10 == 0) {
