...
contract Broker is ERC721Holder{
    ...
    function getFixedPriceTokensForSale() public view returns (tokenDet[] memory) {
    ...
    function collect(uint tokenID, address _mintableToken) public{
        ...
        for(uint i=0; i<tokensForSalePerUser[lastOwner2].length; i++){
            if(
                tokensForSalePerUser[lastOwner2][i].NFTAddress==_mintableToken
                &&
        ...
        uint index3;
        for(uint i=0; i<auctionTokens.length; i++){
            if(
    ...
    function buy(uint tokenID, address _mintableToken) payable public{
        ...
        uint index3;
        for(uint i=0; i<fixedPriceTokens.length; i++){
            if(
        ...
        fixedPriceTokens[index3] = fixedPriceTokens[fixedPriceTokens.length - 1];
