...
contract TomiPool is Ownable {
    ...
    function initialize(address _TOMI, address _WETH, address _FACTORY, address _PLATFORM, address _CONFIG, address _GOVERNANCE) external onlyOwner {
        TOMI = _TOMI;
        WETH = _WETH;
        FACTORY = _FACTORY;
        PLATFORM = _PLATFORM;
    ...
    function addRewardFromPlatform(address _pair, uint _amount) external {
        ...
        if(governanceAmount > 0) {
            TransferHelper.safeTransfer(TOMI, GOVERNANCE, governanceAmount);
