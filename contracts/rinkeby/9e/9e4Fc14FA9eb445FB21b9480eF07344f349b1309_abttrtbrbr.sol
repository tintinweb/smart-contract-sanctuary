    ...
    function getTimeData(uint256 tokenId) public view returns (
        ...
        if (device.createdAt != MAX_TIMESTAMP && alive(tokenId)) {
            ...
            if (block.timestamp.sub(device.lastPlayTime) > timeInbetweenPlays) {
                ...
                // not in a feed
                if (device.feedLevel > fullCycles) {
                    return (true, 0, elapsed, false, device.feedLevel.mul(ONE_DAY).sub(feedElapsedTime), feedWindow );
                }
            ...
            //not in a play
                ...
                // not in a feed
                if (device.feedLevel > fullCycles) {
                    return (false, timeInbetweenPlays.sub(elapsed), elapsed, false, device.feedLevel.mul(ONE_DAY).sub(feedElapsedTime), feedWindow);
                }
