...
contract GenesisMana is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    ILoot private _lootContract = ILoot(0x79E2d470f950f2Cf78eeF41720E8ff2cf4B3CD78); // Rinkeby Loot
    ...
    function tokenURI(uint256 tokenId) override public pure returns (string memory) {
        ...
        string memory name = 'test'; //_getName(_bagRefs[tokenId].bagId, _bagRefs[tokenId].itemType);
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="black" /><text x="10" y="20" class="base">';
        parts[1] = name;
        parts[2] = '</text></svg>';
        string memory output = string(abi.encodePacked(parts[0], parts[1], parts[2]));
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "', name, '", "description": "This item is from a bag of Loot", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
        output = string(abi.encodePacked('data:application/json;base64,', json));
        return output;
    }

    function claimById(uint256 tokenId)
      external
    {
        ...
        require(
            _msgSender() == _lootContract.ownerOf(tokenId),
            "MUST_OWN_TOKEN_ID"
        );
    ...
    function claimAllForOwner()
      external
    {
        uint256 tokenBalanceOwner = _lootContract.balanceOf(_msgSender());
        ...
        for (uint256 i = 0; i < tokenBalanceOwner; i++) {
            ...
            // the _claim() function
            _claim(
                _lootContract.tokenOfOwnerByIndex(_msgSender(), i),
                _msgSender()
            );
        }
    }
    ...
    function _claim(uint256 tokenId, address tokenOwner) internal {
        ...
        require(
            ...
            "GMANA_CLAIMED_FOR_TOKEN_ID"
        );

        // Check there are genesis objects
          require(
            _genesisItem(_lootContract.getWeapon(tokenId)),
            "NO_GENESIS_ITEM"
          );
    ...
    //     require(_lootContract.ownerOf(tokenId) == msg.sender, "Not Loot owner");
