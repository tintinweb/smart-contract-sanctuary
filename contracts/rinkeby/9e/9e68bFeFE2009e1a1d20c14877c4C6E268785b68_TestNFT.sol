...
contract TestNFT is ERC721, Ownable {
    ...
    */
    function mintTest(uint numberOfTokens) public payable {
        ...
        require(totalSupply().add(numberOfTokens) <= MAX_TESTS, "Purchase would exceed max supply of Tests");
        require(testPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
        
        for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply();
            if (totalSupply() < MAX_TESTS) {
        ...
        if (startingIndexBlock == 0 && (totalSupply() == MAX_TESTS || block.timestamp >= REVEAL_TIMESTAMP)) {
    ...
    function setStartingIndex() public {
        ...
        startingIndex = uint(blockhash(startingIndexBlock)) % MAX_TESTS;
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number.sub(startingIndexBlock) > 255) {
            startingIndex = uint(blockhash(block.number - 1)) % MAX_TESTS;
        }
