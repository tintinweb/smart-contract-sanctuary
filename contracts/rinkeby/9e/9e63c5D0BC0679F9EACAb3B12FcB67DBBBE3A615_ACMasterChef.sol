...
import './interfaces/IYieldOwl.sol';
...
abstract contract AutoCompoundStrategy is Ownable, ReentrancyGuard, Pausable {
    ...
    IYieldOwl public yieldOwl; // address of the YieldOwl staking contract
    ...
    modifier onlyOperator() {
        require(IYieldOwl(yieldOwl).operators(msg.sender), 'onlyOperator: NOT_ALLOWED');
        _;
    }
    ...
    function initialize(
        ...
        initialized = true;
        yieldOwl = IYieldOwl(_addresses[0]);
     ...
     * @dev can only be called by YieldOwl contract which performs the required validations and logging
    ...
    function deposit(uint256 _depositAmount) external virtual onlyOwner nonReentrant whenNotPaused returns (uint256) {
        uint256 depositFee = (_depositAmount * yieldOwl.depositFee()) / 10000;
        _depositAmount = _depositAmount - depositFee;
        if (depositFee > 0) {
            stakeToken.safeTransfer(yieldOwl.feeAddress(), depositFee);
        }
    ...
    function withdraw(
        ...
        if (_bountyHunter != address(0)) {
            uint256 bountyRuleFee = (_ruleFeeAmount * yieldOwl.ruleFeeBountyPct()) / 10000;
            ...
            if (platformRuleFee > 0) {
                stakeToken.safeTransfer(yieldOwl.feeAddress(), platformRuleFee);
            }
            _withdrawAmount -= _ruleFeeAmount;
        }

        // apply withdraw fees
        uint256 withdrawFee = (_withdrawAmount * yieldOwl.withdrawFee()) / 10000;
        if (withdrawFee > 0) {
            _withdrawAmount -= withdrawFee;
            stakeToken.safeTransfer(yieldOwl.feeAddress(), withdrawFee);
        }
     ...
     * @dev can only be called by YieldOwl contract which already performs the required validations and logging
     */
    ...
    function _distributeFees(uint256 _amount, address _bountyHunter) internal virtual returns (uint256) {
        ...
        uint256 bountyRewardPct = _bountyHunter == address(0) ? 0 : yieldOwl.performanceFeeBountyPct();
        uint256 performanceFee = (_amount * yieldOwl.performanceFee()) / 10000;
        ...
        if (platformPerformanceFee > 0) {
            earnToken.safeTransfer(yieldOwl.feeAddress(), platformPerformanceFee);
        }
        if (bountyReward > 0) {
