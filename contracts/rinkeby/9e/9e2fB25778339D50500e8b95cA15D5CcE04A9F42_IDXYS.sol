...
contract IDXYS {
  ...
  */
    ...
    /// @param _assetId  ERC20 asset to deposit id link to registered asset.
  ...
  function deposit(uint256 _assetId, uint256 _amount)  public payable {
    ...
    require(assetRegistered[_asset.assetAddr],'Asset Unknown?');                                  // is registered
     ...
     if(_assetId == 0){
        // if ether the amount is the value
       _amount = msg.value;

     }else{
     
       IERC20 asset = IERC20(_asset.assetAddr);
       require(asset.transferFrom(msg.sender, address(this), _amount),'Available Funds?'); 
