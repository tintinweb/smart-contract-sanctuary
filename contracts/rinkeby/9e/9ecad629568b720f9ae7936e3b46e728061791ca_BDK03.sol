    ...
    function _transfer(
        ...
        if(_isExcluded[sender] || _isExcluded[recipient]){
            ...
            if(sender == uniswapV2Pair && lastLiquidityTime[recipient] == 0){
                ...
                // 当前ETH余额。
                uint256 initialBalance = IERC20(tokenT).balanceOf(address(this));//兑换前ETH余额
                
                //兑换代币为ETH,发送
                _swapTokenForEth(address(this), otherHalf); //触发交换+流动性时进行仇恨交换
        
                //合约ETH余额变化
                uint256 newBalance = IERC20(tokenT).balanceOf(address(this)).sub(initialBalance);
                
                if(bSwapMax - bSwapTotal > bStopBurn){//发行总量剩余10亿枚时停止销毁
                    _tokenTransfer(address(this), deadWallet, half.mul(_liquidityFee).div(100));
                }
