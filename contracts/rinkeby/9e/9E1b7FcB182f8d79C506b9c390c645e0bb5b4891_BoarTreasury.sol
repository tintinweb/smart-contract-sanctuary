...
contract BoarTreasury is Ownable {
  using SafeMath for uint256;
  ...
  address public sBOAR;
  uint256 public sBOARQueue; // Delays change to sBOAR address
  ...
  constructor(
    ...
    isReserveToken[_LINK] = true;
    reserveTokens.push(_LINK);

    blocksNeededForQueue = _blocksNeededForQueue;
  }

  /**
        @notice allow approved address to deposit an asset for BOAR
        @param _amount uint
  ...
  function deposit(
    ...
    // mint BOAR needed and store amount of rewards for distribution
    send_ = value.sub(_profit);
    IERC20Mintable(BOAR).mint(msg.sender, send_);
  ...
  /**
        @notice allow approved address to burn BOAR for reserves
        @param _amount uint
        @param _token address
     */
  function withdraw(uint256 _amount, address _token) external {
    ...
    uint256 value = valueOfToken(_token, _amount);
    IBOARERC20(BOAR).burnFrom(msg.sender, value);
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sBOAR).balanceOf(msg.sender); // Can only borrow against sBOAR held
  ...
  /**
        @notice allow approved address to repay borrowed reserves with BOAR
        @param _amount uint
     */
  function repayDebtWithBOAR(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");

    IBOARERC20(BOAR).burnFrom(msg.sender, _amount);

    debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
    totalDebt = totalDebt.sub(_amount);

    emit RepayDebt(msg.sender, BOAR, _amount, _amount);
  }
  ...
  function mintRewards(address _recipient, uint256 _amount) external {
    ...
    IERC20Mintable(BOAR).mint(_recipient, _amount);
  ...
  function excessReserves() public view returns (uint256) {
    return totalReserves.sub(IERC20(BOAR).totalSupply().sub(totalDebt));
  }
  ...
  function updateReserve(address _token, uint256 _amount) external {
    ...
    if (isReserveToken[_token]) {
      // uint256 assetPrice = IBond(msg.sender).assetPrice();
      // totalReserves = totalReserves.add(assetPrice.mul(_amount).div(1e8));

      totalReserves = totalReserves.add(_amount);
    } else if (isLiquidityToken[_token]) {
      totalReserves = IBondCalculator(bondCalculator[_token]).valuation(
        _token,
        _amount
      );
    }
  ...
  /**
        @notice returns BOAR valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
  ...
  {
    if (isReserveToken[_token]) {
      value_ = _amount.mul(10**IERC20(BOAR).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SBOAR) {
      // 9
      sBOARQueue = block.number.add(blocksNeededForQueue);
    } else return false;
