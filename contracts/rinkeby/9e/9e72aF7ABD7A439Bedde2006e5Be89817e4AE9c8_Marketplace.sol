...
contract Marketplace{
    ...
    event OfferingPlaced(bytes32 indexed offeringId, address indexed hostContract, address indexed offerer,  uint tokenId, uint price, uint currencyIndex);
    ...
    function placeOffering(address _offerer, address _hostContract, uint _tokenId, uint _price, uint _currencyIndex) external {
       ...
       /// require (msg.sender == operator, "Only operator dApp can create offerings");
        ...
        offeringNonce += 1;
        emit  OfferingPlaced(offeringId, _hostContract, _offerer, _tokenId, _price, _currencyIndex);
    }
