...
contract BooV2 is OwnableUpgradeable, ERC721EnumerableUpgradeable {
    ...
    function initialize( 
        ...
        _setSalesStartTime(salesStartTime);
    ...
    function buyAmount(uint256 _amount) external payable {
        uint256 totalBuyAmount = currentTokenId.add(_amount).sub(1);
        uint256 totalPrice = mintPrice.mul(totalBuyAmount);
        uint256 totalBuylimit = buyCount[msg.sender].add(totalBuyAmount);

        require(totalBuyAmount <= itemLimit, "exceeds item limit");
        require(
            block.timestamp >= salesStartTime,
            "sales period hasn't started"
        );
        ...
        require(msg.value >= totalPrice, "ETH amount sent is not enough");
        require(totalBuylimit < buyLimit, "buy limit reached");

        buyCount[msg.sender] = buyCount[msg.sender].add(totalBuyAmount);
        for (uint256 index = 0; index < totalBuyAmount; index++) {
            currentTokenId++;
            _safeMint(msg.sender, currentTokenId - 1);
        }
    }
