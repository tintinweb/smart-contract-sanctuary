...
import "module-dexswap-core/contracts/interfaces/IDexSwapPair.sol";
import "module-dexswap-core/contracts/interfaces/IDexSwapFactory.sol";
import "./interfaces/IStakableTokenValidator.sol";
import "./interfaces/IDexTokenRegistry.sol";

contract DexSwapStakableTokenValidator is IStakableTokenValidator, Ownable {
    ...
    constructor(
        ...
        require(
            ...
            "DexSwapStakableTokenValidator: 0-address token registry address"
        );
        require(
            _dexTokenRegistryListId > 0,
            "DexSwapStakableTokenValidator: invalid token list id"
        );
        require(
            _dexSwapFactoryAddress != address(0),
            "DexSwapStakableTokenValidator: 0-address factory address"
        );
        ...
        override
    {
        require(
            ...
            "DexSwapStakableTokenValidator: 0-address stakable token"
        );
        ...
        try _potentialDexSwapPair.token0() returns (address _fetchedToken0) {
            ...
            revert(
                "DexSwapStakableTokenValidator: could not get token0 for pair"
            );
        }
        require(
            dexTokenRegistry.isTokenActive(dexTokenRegistryListId, _token0),
            "DexSwapStakableTokenValidator: invalid token 0 in DexSwap pair"
        );
        address _token1;
        try _potentialDexSwapPair.token1() returns (address _fetchedToken1) {
            ...
            revert(
                "DexSwapStakableTokenValidator: could not get token1 for pair"
            );
        }
        require(
            dexTokenRegistry.isTokenActive(dexTokenRegistryListId, _token1),
            "DexSwapStakableTokenValidator: invalid token 1 in DexSwap pair"
        );
        require(
            dexSwapFactory.getPair(_token0, _token1) == _stakableTokenAddress,
            "DexSwapStakableTokenValidator: pair not registered in factory"
        );
    }
}
