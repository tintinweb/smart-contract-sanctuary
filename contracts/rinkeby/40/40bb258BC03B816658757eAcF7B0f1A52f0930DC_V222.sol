...
contract V222 is ERC721, Ownable, IMintedBeforeReveal {
  ...
  // Maximum amount of  BABY APES  in whitelist free mint.
    uint256 public constant MAX_FM_SUPPLY = 5;
    ...
    function isMintedBeforeReveal(uint256 index) public view override returns (bool) {
        ...
        function getFMMaxAmount() public view returns (uint256) {
        require(block.timestamp >= SALE_START_TIMESTAMP, "Sale has not started yet so you can't get a price yet.");
        require(totalSupply() < MAX_FM_SUPPLY, "Sale has already ended and all sold out, no more left to sell.");
    ...
    function getFMPrice() public view returns (uint256) {
        require(block.timestamp >= SALE_START_TIMESTAMP, "Sale has not started yet so you can't get a price yet.");
        require(totalSupply() < MAX_FM_SUPPLY, "Sale has already ended, no more items left to sell.");
    ...
    function freemint(uint256 numberOfTokens) public payable {
        ...
        require(totalSupply() < MAX_FM_SUPPLY, "Sale has already ended.");
        require(numberOfTokens > 0, "You cannot mint 0 items, please increase to more than 1");
        require(numberOfTokens <= getFMMaxAmount(), "You are not allowed to buy this many items at once.");
        require(SafeMath.add(totalSupply(), numberOfTokens) <= MAX_FM_SUPPLY, "Exceeds maximum supply. Please try to mint less.");
        require(SafeMath.mul(getFMPrice(), numberOfTokens) == msg.value, "Amount of Ether sent is not correct.");
