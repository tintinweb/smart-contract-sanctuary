...
contract Obtainable is ContextMixin, ERC721, ERC1155, IERC2981, NativeMetaTransaction, Ownable, IOpenSeaContractURI, IRaribleRoyalties {
  ...
  ) ERC721(_assetName, _assetSymbol) ERC1155(_uri)  {
  ...
  function uri(
    ...
    public 
    view 
    override(ERC1155, ERC721, IERC165) 
    returns (bool) {
      ...
      //   super.supportsInterface(interfaceId) ||
      //   interfaceId == this.targetNFT.selector ^ this.sendRoyalty.selector ^ this.withdrawRoyalty.selector;
    }
    /**
    ...
    */
    function create1155(
      ...
      _mint1155(_initialOwner, _id, _initialSupply, _data);
  ...
  /**
    ...
    */
    function mint1155(
        ...
        _mint1155(_to, _id, _quantity, _data);
        tokenSupply[_id] = tokenSupply[_id].add(_quantity);

        // emit rarible royalty info
        address[] memory recipients = new address[](1);
        recipients[0] = _royaltyRecipient;
        emit SecondarySaleFees(_id, recipients, getFeeBps(_id));
    }
    ...
    */
    function mintTo(address _to) public onlyOwner {
        ...
        _mint721(_to, newTokenId);
        _incrementTokenId();
        // emit rarible royalty info
        address[] memory recipients = new address[](1);
        recipients[0] = _royaltyRecipient;
        emit SecondarySaleFees(newTokenId, recipients, getFeeBps(newTokenId));
    }

    /**
    * @dev Batch Mints a token to an address with a tokenURI.
    * @param _to[] addresses of the future owner of the token
    */
    function batchMintTo(address[] memory _to) public onlyOwner {
      for (uint256 i = 0; i < _to.length; i++) {
        uint256 newTokenId = _getNextTokenId();
        _mint721(_to[i], newTokenId);
        _incrementTokenId();
        // emit rarible royalty info
        address[] memory recipients = new address[](1);
        recipients[0] = _royaltyRecipient;
    ...
    function tokenURI(uint256 _tokenId) override public view returns (string memory) {
        return string(abi.encodePacked(assetUri, Strings.toString(_tokenId)));
    }
  ...
  function batchMint(
    ...
    for (uint256 i = 0; i < _ids.length; i++) {
      ...
      address[] memory recipients = new address[](1);
      recipients[0] = _royaltyRecipient;
      emit SecondarySaleFees(_id, recipients, getFeeBps1155(_id, quantity));
  ...
  function _setCreator(address _to, uint256 _id) internal creatorOnly(_id)
  {
    ...
    // set address that royalties are sent to
    function setRoyaltyCollectionNetadataCID(string memory cid)  public onlyOwner {
        _collectionMetadataCID = cid;
    }
    ...
    function getFeeRecipients(uint256 tokenId) override public view returns (address payable[] memory) {
        require(_exists721(tokenId), "invalid token");
    ...
    // rarible royalties
    function getFeeBps1155(uint256 tokenId, uint256 quantity) public view returns (uint[] memory) {
        require(exists(tokenId), "invalid token");
        uint256[] memory ret = new uint[](1);
        ret[0] = uint(_feeBps * quantity);
        return ret;
    }
...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    function safeTransferFrom(
        ...
        _safeTransferFrom1155(from, to, id, amount, data);
    }
...
contract ERC721 is Context, IERC721, ERC165, IERC721Metadata {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists721(tokenId), "ERC721Metadata: URI query for nonexistent token");
    ...
    function getApproved(uint256 tokenId) public view virtual override returns (address) {
        require(_exists721(tokenId), "ERC721: approved query for nonexistent token");
     ...
     */
    function _exists721(uint256 tokenId) internal view virtual returns (bool) {
    ...
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal view virtual returns (bool) {
        require(_exists721(tokenId), "ERC721: operator query for nonexistent token");
        address owner = ERC721.ownerOf(tokenId);
    ...
    function _mint721(address to, uint256 tokenId) internal virtual {
        ...
        require(!_exists721(tokenId), "ERC721: token already minted");
