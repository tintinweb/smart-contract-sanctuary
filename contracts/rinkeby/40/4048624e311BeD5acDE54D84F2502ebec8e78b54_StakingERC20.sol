...
contract StakingERC20 {
     ...
     * @param _hotWallet for token transfer if any
     */
    constructor(address _stakingToken, uint256 _stakingPool, uint256 _stakingPoolRewards, uint256 _poolOpenTime, uint256 _stakingStartTime, uint256 _stakingDuration, ManagerRole _manager, address _hotWallet) {
    ...
    function calculateBlocks(uint256 _days) internal {
        ...
        noOfStakingBlocks = 30;
    }
    
    function stakingEndsOn() internal {
        stakingEndBlock = currentBlockNumber().add(noOfStakingBlocks);
    }
    ...
    function stake(uint256 _noOfTokens) external {
        ...
        require(stakingPool >= currentPool.add(StakeHolders[msg.sender].amount.add(_noOfTokens)), "Stake:: Pool is open, your staking exceeded Staking Pool!");
    ...
    function rewardsToMaintain() external onlyManager view returns (uint256) {
        ...
        uint256 totalStakingIncludingRewards = stakingPool.add(stakingPoolRewards);
        return totalStakingIncludingRewards.sub(currentBalance);
    }
    
    function getHotWallet() external onlyManager view returns (address) {
        return hotWallet;
    }
    ...
    function safeWithdrawalBefore(uint256 _noOfTokens) external onlyManager {
        ...
        require(IERC20(stakeToken).balanceOf(address(this)) >= _noOfTokens, "Invalid Balance");
        IERC20(stakeToken).transfer(msg.sender, _noOfTokens);
    }
    
    function safeWithdrawalAfter() external onlyManager {
        require(currentBlockNumber() > stakingEndBlock, "SafeWithdrawalAfter:: Can't withdraw early");
        uint256 totalStakingIncludingRewards = stakingPool.add(stakingPoolRewards);
        require(IERC20(stakeToken).balanceOf(address(this)) >= totalStakingIncludingRewards, "Invalid Balance");
        uint256 difference = IERC20(stakeToken).balanceOf(address(this)).sub(totalStakingIncludingRewards);
        if(difference > 0) {
