...
contract GovPriceConsumer is IGovPriceConsumer, OwnableUpgradeable {
    ...
    function initialize(
        ...
        require(
            _stableFeedTokens.length > 0,
            "GPCPriceConsumer: No tokens provided for price feed"
        );
        require(
            _chainLinkFeedContracts.length > 0,
            "GPC: No price feed chainlink contract address provided"
        );
        require(
            _stableFeedTokens.length == _chainLinkFeedContracts.length,
            "GPC: Price feed tokens and contracts should be of same length"
        );
        ...
        for (uint256 i = 0; i < _stableFeedTokens.length; i++) {
            usdPriceAggrigators[_stableFeedTokens[i]] = ChainlinkDataFeed(
                AggregatorV3Interface(_chainLinkFeedContracts[i]),
                _enabled[i],
                _decimals[i]
            );
            allFeedTokensContractChainlink.push(_chainLinkFeedContracts[i]);
            allFeedTokenAddress.push(_stableFeedTokens[i]);
            emit PriceFeedAdded(
                _stableFeedTokens[i],
                _chainLinkFeedContracts[i],
                _enabled[i],
                _decimals[i]
            );
        }
