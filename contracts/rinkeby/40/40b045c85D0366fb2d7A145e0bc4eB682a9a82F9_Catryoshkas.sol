...
contract Catryoshkas is ERC721, ERC721Enumerable, Ownables {

    using SafeMath for uint256;

    string private _metadataAPI;
    uint256 private _saleStartUTS;
    uint256 private _extra_supply = 1;
    uint256 private _minted_supply = 0;
    uint256 private constant _MAX_MINT_SUPPLY = 8888;
    uint256 private constant _MINT_PRICE = 0.044 ether;
    
    mapping(uint256 => bool) public _specialCatsBreed;
    ...
    modifier checkMintingSupplyAvailable(uint256 numberOfTokens_) {
        
        require(_minted_supply.add(numberOfTokens_) <= _MAX_MINT_SUPPLY, "Exceeds max minted supply");
        _;

    }

    function mintTokens(uint256 numberOfTokens_) public payable checkMintingSupplyAvailable(numberOfTokens_) {
    ...
    function _mintTokens(uint256 numberOfTokens_) internal {
        ...
        uint256 supplyTracker = _minted_supply;
        
        for (uint256 index = 0; index < numberOfTokens_; index++) {
            
            _safeMint(msg.sender, supplyTracker);
            supplyTracker = supplyTracker.add(1);

        }

        _minted_supply = _minted_supply.add(numberOfTokens_);

    }

    function reserveTokens(uint256 numberOfTokens_, address to_) public onlyOwners {
        
        uint256 supplyTracker = _MAX_MINT_SUPPLY.add(_extra_supply);

        for (uint256 index = 0; index < numberOfTokens_; index++) {
            
            _safeMint(to_, supplyTracker);
            supplyTracker = supplyTracker.add(1);

        }

        _extra_supply = _extra_supply.add(numberOfTokens_);
    ...
    function getMintedSupply() public view returns (uint256) {
        return _minted_supply;
    }

    function getExtraSupply() public view returns (uint256) {
        return _extra_supply;
    }
    ...
    function _bindSpecials(uint256[] memory specials_)  public onlyOwners {

        for (uint index = 0; index < specials_.length; index++) {
             
            _specialCatsBreed[specials_[index]] = false;
...
abstract contract Ownables is Ownable {
    ...
    function _resetAgreement(address owner_) internal {
        ...
        if ( owner_ == owner() ){
            ...
            _ownerController.sendToAddress = owner();
        ...
        if( owner_ == secondOwner() ){
            ...
            _secondOwnerController.sendToAddress = secondOwner();
