...
contract ChainlinkWrapper is Initializable, IPriceOracles {
    ...
    function getLatestRoundData(address token) public view returns (int256, uint256) {
        AggregatorInterface aggregator = AggregatorInterface(tokenChainlinkMap[token]);

        if (isEthOrBnB(token)) {
            aggregator = AggregatorInterface(ethToUsdPriceOracle);
        }
