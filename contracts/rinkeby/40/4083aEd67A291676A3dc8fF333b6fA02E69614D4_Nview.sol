...
contract Nview is NPassCore {
  ...
  function getCirclesSVG(uint[8] memory numbers, string memory color) public pure returns(string memory){
    ...
    for(uint i = 0; i < numbers.length; i++){
      for(uint j = 1; j <= numbers[i]; j++){
        ...
        // TODO fix correct distance and circle size, ask about rotation
        (x_pos, y_pos) = getCirclePos(i, j);
        // TODO refine smoothness circle size changing based on value
        radius_pr = pseudoRandomRadius(3, numbers[i], i, j);
        // TODO circle opacity changing based on value
        // put color function into getcirclesSVG and add parameter for calc
        col_opacity = pseudoRandomColorOpacity(numbers[i], i, j);

        string memory currentCircle = getCircle(x_pos, y_pos , color, col_opacity, radius_pr);
        string memory currentNumberSVG;
        if(j > 1){
          string memory currentCircleOutline = getCircleOutline(x_pos, y_pos , color, col_opacity, radius_pr);
  ...
  function pseudoRandomColorOpacity(uint numberi, uint i , uint j) public pure returns(string memory){
    if (j <= 2) return "1";
    string memory value = (100 - ((uint(keccak256(abi.encodePacked(numberi, i, j))) % 10) + j * 2)).toString();
  ...
  function pseudoRandomRadius(uint radius_base, uint numberi, uint i , uint j) public pure returns (uint){
    return (uint(keccak256(abi.encodePacked(numberi, i, j)))) % 7 + radius_base;
  }
  ...
  function getCircleOutline(uint x, uint y, string memory color, string memory color_opacity, uint radius) public pure returns(string memory) {
    return 
      string(
        abi.encodePacked(
          "<circle cx='",
