...
contract Emeth is AssignerRole, VerifierRole {
    ...
    uint256 constant MIN_DEPOSIT = 1000000000000000000000; // 10,000 * 0.1 EMT = 1,000 EMT
    uint256 constant DEPOSIT_PER_CAPACITY = 100000000000000000; // 0.1 EMT
    uint256 ASSIGNER_FEE = 0; // 0 EMT
    uint256 VERIFIER_FEE = 0; // 0 EMT

    // EMT
    address constant tokenAddress = 0x8Eb44f2e527DBa1cC8e8Ec2D59C9Aaf5aB1a8984;
    uint256 constant BASE_SLOT_REWARD = 288000000000000000000000; // 12,000 EMT x 24
    uint256 constant SLOT_INTERVAL = 24 hours;
    uint256 constant HALVING_PERIOD = 365 days;
    uint256 constant HALVING_AMOUNT = 600000000000000000000; // 600 EMT
    ...
    // Constructor
    constructor() {
        startTime = block.timestamp.div(SLOT_INTERVAL).mul(SLOT_INTERVAL);
    }
    ...
    function slotReward(uint256 _slot) public view returns (uint256) {
        uint256 reward = 0;
        uint256 slotTime = _slot.mul(SLOT_INTERVAL);
        uint256 halvingAmount = slotTime.sub(startTime).mul(HALVING_AMOUNT).div(HALVING_PERIOD);
    ...
    function _calculateDeposit(uint256 _gas, uint256 _timeLimit) internal pure returns (uint256) {
    ...
    function _adjustedCapacity(uint256 _requiredCapacity, address _node) internal view returns (uint256) {
