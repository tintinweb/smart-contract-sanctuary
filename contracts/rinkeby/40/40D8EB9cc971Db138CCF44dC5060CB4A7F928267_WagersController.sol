...
contract WagersController is Ownable {
    ...
    modifier eventNotOver(bytes32 eventId){
        require (!events.getVoteReady(eventId), "EventId should not be ready for now");
        _;
    }
    ...
    function makeWager(
        ...
        payable
    {
        require(makerChoice <= events.getNumOutcomes(eventId), "OutcomeNumber should be bigger than makerChoice");
        ...
        //     takerChoice = 1;
        // }
        uint256 winningValue = value + (value*100) / odds;       
        wagers.makeWager( 
            wagerId,
            eventId,
            value,
        ...
        payable
    {
        ...
        uint256 odds = wagers.getOdds(id);
        ...
        if (odds < 100){         
            ...
            uint256 diff = odds - 100;
            uint256 divider = (100/diff) + 1;
        ...
        rewards.addEth(msg.sender, msg.value);
        wagers.setTaker(id, payable(msg.sender));    
        wagers.setLocked(id); 
        uint256 totalValue = expectedValue + wagers.getOrigValue(id);    
        ...
        external 
    {
        ...
        uint256 eventWinner = events.getCurrentWinner(eventId); // Shouldn't matter if bettors agree
    ...
    /** @dev Settles the wager if both the maker and taker have voted, pays out if they agree, otherwise they need to wait for oracle settlement.               
      * @param wagerId bytes32 id for the wager.         
      */
    function settle(bytes32 wagerId) internal { // , bytes32 eventId
        address payable maker = wagers.getMaker(wagerId);
        address payable taker = wagers.getTaker(wagerId);
        uint256 origValue = wagers.getOrigValue(wagerId);
        uint256 payoutValue = wagers.getWinningValue(wagerId); 
        ...
        if (wagers.getMakerWinVote(wagerId) == wagers.getTakerWinVote(wagerId)) {
            if (wagers.getMakerWinVote(wagerId) == wagers.getMakerChoice(wagerId)) {
                ...
                if (wagers.getMakerWinVote(wagerId) == 0) { /// No winner could be decided
                    wagers.setWinner(wagerId, payable(address(0)));                    
                } else {
                    rewards.subEth(taker, wagers.getWinningValue(wagerId) - origValue);
    ...
    function lateSettle (bytes32 wagerId, uint256 eventWinner) internal {
        address payable maker = wagers.getMaker(wagerId);
        address payable taker = wagers.getTaker(wagerId);
       ...
       * @param wagerId bytes32 id for the wager.         
       */
    function payout(bytes32 wagerId, address maker, address taker, uint256 payoutValue) internal {  
        require(!wagers.getSettled(wagerId));
        wagers.setSettled(wagerId);           
      //  uint256 origVal =  wagers.getOrigValue(wagerId);
        uint256 winVal = wagers.getWinningValue(wagerId);
    ...
    */
    function lateSettledPayout(bytes32 wagerId) internal {        
        ...
        uint256 origValue = wagers.getOrigValue(wagerId);
        uint256 winningValue = wagers.getWinningValue(wagerId);
        uint256 payoutValue = winningValue;         
        uint256 fee = (payoutValue/100) * 3; //Fee is now 3 percent since oracles were used
