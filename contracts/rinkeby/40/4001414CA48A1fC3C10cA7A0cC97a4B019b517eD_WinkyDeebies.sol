...
contract WinkyDeebies is AccessControl, ERC721Enumerable {
    ...
    constructor(address owner_, address dev_, address deebiesContract_, string memory uri_) ERC721("Winky Deebies", "WINKY_DEEBIES")  {
    ...
    function claimWinkyDeebie(uint256 tokenId) public {
        if(_msgSender() != owner()){
    ...
    function baseClaim(uint256 tokenId) private {
        require(IERC721(deebiesContract).ownerOf(tokenId) == _msgSender(), string(abi.encodePacked('You are not the owner of ', tokenId.toString())));
    ...
    function claimMultipleWinkyDeebies(uint256[] memory tokenIds) public {
        if(_msgSender() != owner()){
            require(!paused, "Pause");
        }
