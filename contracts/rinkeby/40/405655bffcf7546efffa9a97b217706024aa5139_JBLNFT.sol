...
contract JBLNFT is ERC721{
    ...
    constructor ()  public ERC721("B-NFT", "B-NFT"){
        owner = msg.sender; 
        _set_JBL_TOKEN(0x199d6407f88462fd6fafc90f0900f02c97312fdd);//CToken
    }
    ...
    function mintCard(address account,string _URI) public returns (uint256) {
        ...
        string memory name = "B-NFT";
        string memory tokenURI = "https://emoceanx.i-recu.com/nft/test.jpg";

        token.transferFrom(msg.sender, address(this), PRICE*1*10**18);
        
        mint_total = mint_total-1;

        cards.push(Card(name, 4, tokenURI, NFT_APY, PRICE));//4:活動
        _mint(account, cardId); 
        _setTokenURI(cardId, _URI);
        
        return cardId;
        
    }
    
    function mintBatch(address account, uint256 num, string _URI) public {
        emit _mintBatch(msg.sender, account, num);
        for(uint t = 0; t < num; t++) {
            mintCard(account, _URI);
        }
    } 
    ...
    function _setTokenURI_byOwner(uint256 tokenId, string memory _tokenURI) public onlyOwner {
        _tokenURIs[tokenId] = _tokenURI;
    }
