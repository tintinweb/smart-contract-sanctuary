...
library NFTSVG {
  ...
  function _generateSVGDefs(SVGParams memory params) private view returns (string memory svgDef) {
    ...
    for(uint256 i = 0; i < 5; ++i){
      ...
      subDefs = string(abi.encodePacked(
        ...
        (subIndex + 1).toString(),
        '" >',
        '<animate attributeName="d" values="m',
        space.toString(),
        ',230l0,0 0,0;m190,190l30,-',
        drift1.toString(),
        ' 30,',
        drift1.toString(),
    ...
    svgDef = string(
      abi.encodePacked(
        ...
        '<defs>',
        '<path id="r1" >',
        '<animate id="p1" attributeName="d" values="m',
        space.toString(),
        ',230l0,0 0,0;m190,',
        r1.toString(),
        'l30,-',
  ...
  function idToColor(uint256 _id) public view returns (uint256, string memory) {
    ...
    return (256 - firstChunk, rgbString);
     // //require(tokenId < TOTAL_MAX_SUPPLY, "24-bit color exceeded"); //16777216
     // uint256 limit = 16777216;
     // uint256 key = (_tokenId < limit) ? _tokenId : _tokenId % limit;
     // bytes32 val = bytes32(key);
