...
contract NFTMarketplace is Pausable, FeeManager, ERC721Holder {
    ...
    function acceptDirectSellOrder(address _tokenAddress, uint256 _tokenId, uint256 _Price) payable public whenNotPaused
    {
        DirectOrder memory directorder = DirectorderByTokenId[_tokenAddress][_tokenId];
        // require(directorder.seller == msg.sender, "Marketplace: Only owner can sell the NFT");
        require(directorder.askingPrice == _Price, "Marketplace: Less amount sent to buy");

        


        uint256 saleShareAmount = directorder.askingPrice.mul(FeeManager.cutPerMillion).div(1e6);
        directorder.seller.transfer(msg.value - saleShareAmount);
    ...
    function createDirectSellOrder(address _tokenAddress, uint256 _tokenId, uint256 _askingPrice) public whenNotPaused{
