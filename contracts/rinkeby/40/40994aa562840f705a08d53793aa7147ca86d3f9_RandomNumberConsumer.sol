...
contract RandomNumberConsumer is VRFConsumerBase {
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        emit RandomNumber(users[requestId],requestId,randomNumber);
    }
    ...
    function _mintSelectedNFT(uint256 index,uint256 randomness) internal virtual returns(uint256 id,uint256 qty, string memory rarity){
        ...
        nftsAvailable[_id] = nftsAvailable[_id] - 1;

        if(nftsAvailable[_id] <= 0 && rarityArray.length >= 1){
            if(rarityArray.length == 1){
                rarityArray.pop();
            }else{
                emit Running("Running in Removal Part");
                for(uint256 i=randomNumber; i < rarityArray.length-1; i++){
        ...
        string memory name_ = raritiesName[index];
        return(_id,1,name_);
    }
