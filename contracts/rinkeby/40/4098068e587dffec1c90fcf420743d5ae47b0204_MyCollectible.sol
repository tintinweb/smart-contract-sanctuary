...
contract MyCollectible is ERC721, Ownable {
    ...
    uint256 public CAPSULE_PRICE = 0.008 ether;
    uint256 public totalSaleSupply;
    bool public saleIsActive = true;
    address private key = 0x8f20d89bEe77ea2AbBaF46b5DEF3Ef109ab9d358;
    string public _tokenBaseURI;

    constructor(uint256 _MAX_SUPPLY) ERC721("MyCollectible", "MCO") {
        MAX_CAPSULES = _MAX_SUPPLY;
    }
    ...
    function _verify(bytes32 hash, bytes memory signature) internal view returns (bool) {
        ...
        function _hash(address _address, uint256 max)
        internal view returns (bytes32)
    {
        return keccak256(abi.encode(max, _address));
    }
        function mintPublic(uint256 numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint a capsule.");
        require(totalSaleSupply + numberOfTokens <= MAX_CAPSULES, "Purchase would exceed max supply of capsules for sale.");
    ...
    function mintWhitelist(bytes memory signature, uint256 numberOfTokens, uint256 max) public payable {
        require(_verify(_hash(msg.sender, max), signature), "This hash's signature is invalid.");
        require(totalSaleSupply + numberOfTokens <= MAX_CAPSULES, "Purchase would exceed max supply of capsules for sale.");
        require(CAPSULE_PRICE * numberOfTokens <= msg.value, "Ether value sent is not correct");
        for (uint i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = totalSaleSupply + 1;
