...
contract NewManager is Ownable, IERC721Receiver, Pausable {
    ...
    uint public distance = 1000; // TODO: 424800;
    ...
    event TokenStaked(address owner, uint16 shipId, uint16[] tokenIds, uint boosters, uint timestamp);
    ...
    function stakeTokens(address _account, uint16 _shipId, uint16[] calldata _tokenIds, uint16 _rumAmount) public {
        ...
        emit TokenStaked(msg.sender, _shipId, _tokenIds, _rumAmount, block.timestamp);
    }
    ...
    function unstakeTokensIntoLand(address _owner, uint16 _index) external whenNotPaused {
        ...
        if (!shipToken.isApprovedForAll(address(this), address(land))) shipToken.setApprovalForAll(address(land), true);
        if (!goldHunter.isApprovedForAll(address(this), address(land))) goldHunter.setApprovalForAll(address(land), true);
        ...
        // Call method from ILand
        land.stakeTokens(_owner, userStake.tokenIds);
