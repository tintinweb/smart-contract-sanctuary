...
contract TokenDistribution is Ownable {
    ...
    receive() external payable isPresale isWhitelisted {
        buyTokens();
    }
    ...
    function buyTokens() public payable isPresale isWhitelisted requirements {
        
        (uint256 tokens, uint256 pricePerTokenEth) = calculateNumberOfTokens(msg.value);
        require(tokens > 0, "Insufficient funds");

        uint256 tradeAmounInUSD = (tokens * tokenPriceUSD) / 10 ** 18;
        
        require(tradeAmounInUSD >= minLimitUSD, "Send amount is below min limit");
        require(tradeAmounInUSD + contributionInUSD[msg.sender] <= maxLimitUSD, "Send amount is above max limit");

        preboughtTokens[msg.sender] += tokens;
        contributionInUSD[msg.sender] += tradeAmounInUSD;
        weiRaised += msg.value;
    ...
    function getPriceInEthPerToken() public view returns(uint256) {
        ...
        uint256 oraclePriceUSD = uint256(oraclePriceTemp);

        // oracle units
        // tokenPrice is 5000000 = $0.05
        // if we set token price in cents 5, we can get decimals from oracle(8)
        // 8 decimals is for $1, for cents would be 6 >> decimals - 2
        // and calc it on fly: 5 * (10 ** 6) = 5000000
        return divide(tokenPriceUSD, oraclePriceUSD, 18);
    }
