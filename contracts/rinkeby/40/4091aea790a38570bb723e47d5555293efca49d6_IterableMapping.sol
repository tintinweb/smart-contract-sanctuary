...
contract JPOPDOGEDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() public DividendPayingToken("JPOPDOGE_Dividend_Tracker", "JPOPDOGE_Dividend_Tracker") {
    ...
    function withdrawDividend() public override {
        require(false, "JPOPDOGE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main JPOPDOGE contract.");
    }
...
contract JPOPDOGE is ERC20, Ownable {
    ...
    address payable public devAddress = 0x7Ec926D395D886a1bfcaabD610A5438fdB6785D2;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "JPOPDOGE: The dividend tracker already has that address");

        JPOPDOGEDividendTracker newDividendTracker = JPOPDOGEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "JPOPDOGE: The new dividend tracker must be owned by the JPOPDOGE token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "JPOPDOGE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "JPOPDOGE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "JPOPDOGE: The UniSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "JPOPDOGE: Automated market maker pair is already set to that value");
    ...
    function allowPreTrading(address account, bool allowed) public onlyOwner {
        ...
        require(canTransferBeforeTradingIsEnabled[account] != allowed, "JPOPDOGE: Pre trading is already the value of 'excluded'");
        canTransferBeforeTradingIsEnabled[account] = allowed;
    }

    function setMaxPurchaseEnabled(bool enabled) public onlyOwner {
        require(maxPurchaseEnabled != enabled, "JPOPDOGE: Max purchase enabled is already the value of 'enabled'");
    ...
    function reinvestInactive(address payable account) public onlyOwner {
        ...
        require(tokenBalance <= minimumTokenBalanceForDividends, "JPOPDOGE: Account balance must be less then minimum token balance for dividends");

        uint256 _lastTransfer = lastTransfer[account];
        require(block.timestamp.sub(_lastTransfer) > 12 weeks, "JPOPDOGE: Account must have been inactive for at least 12 weeks");
    ...
    function _claim(address payable account, bool reinvest, uint256 minTokens) private {
        ...
        require(withdrawableAmount > 0, "JPOPDOGE: Claimer has no withdrawable dividend");
    ...
    function _transfer(
        ...
        if(tradingStartTime > block.timestamp) {
            require(canTransferBeforeTradingIsEnabled[from], "JPOPDOGE: This account cannot send tokens until trading is enabled");
        }
        ...
        if (maxPurchaseEnabled && automatedMarketMakerPairs[from]) {
            require(amount <= maxPurchaseAmount, "JPOPDOGE: Exceeds max purchase amount");
        }
