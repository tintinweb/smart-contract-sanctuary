...
/**
 * @title Wilde Numbers
 * Midnight Walker - a contract for non-fungible wild numbers
 */
contract WildNumber is ERC721Tradable {
    string public WILD_NUMBERS_COLLECTION_NAME = "Wild Numbers";
    string public WILD_NUMBERS_SYMBOL = "Wild-Numbers";
...
// File: contracts/WildNumberFactory.sol
...
contract WildNumberFactory is FactoryERC721, Ownable {
    ...
    uint256 public MAX_RESERVED = 10;
    uint256 public CURRENT_RESERVED = 0;

    // define this based on array of options
    uint256 NUM_OPTIONS = 4;

    uint256 SINGLE_WILD_NUMBER_OPTION = 1;
    uint256 FIVE_WILD_NUMBERS_OPTION = 2;
    uint256 TEN_WILD_NUMBERS_OPTION = 3;
    uint256 TWENTY_WILD_NUMBERS_OPTION = 4;

    uint256 SINGLE_WILD_NUMBER_OPTION_AMOUNT = 1;
    uint256 FIVE_WILD_NUMBERS_OPTION_AMOUNT = 5;
    uint256 TEN_WILD_NUMBERS_OPTION_AMOUNT = 10;
    uint256 TWENTY_WILD_NUMBERS_OPTION_AMOUNT = 20;
    ...
    constructor(
        ...
        bundleAmounts[SINGLE_WILD_NUMBER_OPTION] = SINGLE_WILD_NUMBER_OPTION_AMOUNT;
        bundleAmounts[FIVE_WILD_NUMBERS_OPTION] = FIVE_WILD_NUMBERS_OPTION_AMOUNT;
        bundleAmounts[TEN_WILD_NUMBERS_OPTION] = TEN_WILD_NUMBERS_OPTION_AMOUNT;
        bundleAmounts[TWENTY_WILD_NUMBERS_OPTION] = TWENTY_WILD_NUMBERS_OPTION_AMOUNT;

        fireTransferEvents(address(0), owner());
    }

    function name() external pure override returns (string memory) {
        return "Wild Numbers Sale";
    }
    ...
    function setFullMetadataRevealTimestamp(uint256 _daysFromNowUntilReveal)
        ...
        onlyOwner
    {
        WildNumber wildNumber = WildNumber(nftAddress);
        wildNumber.setFullMetadataRevealTimestamp(_daysFromNowUntilReveal);
    }

    /*
     * Reserve Wild Numbers for collaborators
     */
        ...
        onlyOwner
    {
        require(
            !isSaleActive,
            "Cannot mint reserved wild numbers while sale is active."
        );
        ...
        require(
            canMint(_optionId),
            "Cannot Mint: Invalid Wild Number Package Option or No Wild Number Tokens Left"
        );

        WildNumber wildNumber = WildNumber(nftAddress);
        wildNumber.mintTo(_toAddress, bundleAmounts[_optionId]);
    ...
    function canMint(uint256 _optionId) public view override returns (bool) {
        ...
        uint256 currentWildNumbersSupply = wildNumber.tokenCount();
        uint256 numItemsAllocated = bundleAmounts[_optionId];

        return
            currentWildNumbersSupply <= (wildNumber.totalSupply() - numItemsAllocated);
    }
     ...
     */
    function emergencyOverrideWildNumberMasterCID(string memory _metadataCID)
        public
        onlyOwner
    {
        WildNumber wildNumber = WildNumber(nftAddress);
        ...
        require(
            !wildNumber.metadataIsPermanentlyFrozen(),
            "Cannot Override Wild Number Master CID: Metadata is permanently frozen."
        );

        wildNumber.emergencyOverrideMasterCID(_metadataCID);
    }

    /*
     * After we are sure metadata is sound, call this to prevent changing it forever.
     */
    function freezeWildNumberMetadataPermanently() public onlyOwner {
        WildNumber wildNumber = WildNumber(nftAddress);

        require(
            !wildNumber.metadataIsPermanentlyFrozen(),
            "Cannot Freeze Wild Numbers Metadata: Metadata is already permanently frozen."
        );

        wildNumber.freezeMetadataPermanently();
    }
