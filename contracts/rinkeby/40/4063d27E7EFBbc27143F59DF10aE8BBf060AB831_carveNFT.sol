...
contract carveNFT is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    uint256 constant maxSupply = 10000; // total amount of nfts we have
    mapping(address => bool) whitelist;
    uint256 whitelistMintTime;
    Counters.Counter currentCounter;
    ...
    modifier saleIsLive{
        require(config.timeOfDrop != 0);
        require(config.price != 0);
        require(config.maxMintAmount != 0);
        require(config.maxWallet != 0);
        _;
    }
    ...
    function rmvFromWhitelist(address[] memory addressesToRmv) public onlyOwner {
        for (uint256 i=0; i < addressesToRmv.length ; i++) {
            whitelist[addressesToRmv[i]] = false;
        }
    }   
    
    function setWhitelistTime(uint256 timeOfDrop) public onlyOwner {
        whitelistMintTime = timeOfDrop;
    }
    
    function revealNFTs(string memory revealedURL) public onlyOwner {
        setBaseURI(revealedURL);
    }

    function getCurrentTokenID() internal view returns(uint256) {
        return currentCounter.current();
    }
    ...
    function _fullMint(address minter) internal {
        currentCounter.increment();
        _safeMint(minter, getCurrentTokenID());
    }

    function mint(uint256 _amnt) public payable saleIsLive {
        // see how many have already been minted
        require(getCurrentTokenID() + _amnt <= maxSupply, "We cannot supply this amount"); // check we have enough to mint 
        ...
        for (uint256 i = 1; i <= _amnt;i++) {
            _fullMint(msg.sender); // mint the user there nft
        }
    }
