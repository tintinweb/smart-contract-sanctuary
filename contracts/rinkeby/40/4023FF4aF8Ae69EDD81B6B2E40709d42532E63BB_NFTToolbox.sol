...
contract NFTToolbox is TestNetERC721, Ownable { 
    ...
    function controllerAdd(address[] calldata _addr) public onlyOwner {
        for(uint8 i = 0; i < _addr.length; i++) {
            controllers.add(_addr[i]);
        }
    }
 ...
 */
contract TestNetERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    using SafeMath for uint256;
    ...
    */
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = TestNetERC721.ownerOf(tokenId);
        require(to != owner, "ERC721: approval to current owner");

        require(_msgSender() == owner || TestNetERC721.isApprovedForAll(owner, _msgSender()),
    ...
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal view virtual returns (bool) {
        ...
        address owner = TestNetERC721.ownerOf(tokenId);
        return (spender == owner || getApproved(tokenId) == spender || TestNetERC721.isApprovedForAll(owner, spender));
    }
    ...
    function _burn(uint256 tokenId) internal virtual {
        address owner = TestNetERC721.ownerOf(tokenId); // internal owner
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
        require(TestNetERC721.ownerOf(tokenId) == from, "ERC721: transfer of token that is not own"); // internal owner
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(TestNetERC721.ownerOf(tokenId), to, tokenId); // internal owner
    }
    ...
    function _takeToken(uint256 tokenId, address to) internal {
