...
//test if changing cost works with the ether there.
 ...
 //add original metadata in native initial contract   
    function setHatchlingzMetadata(string[] memory metadata, uint256 species) external onlyOwner {
        require(species == 0 || species == 1 || species ==2, "not a valid animal type");
     ...
     function initiateMetadata1() public onlyOwner {
        for (uint256 i = 1; i<= 3; i++){
            string memory temp = string(abi.encodePacked("chicken",i.toString()));
            _commonMetadata.push(temp);
        }
    }

    function clearHatchlingzMetadata(uint8 species) external onlyOwner {
        require(species == 0 || species == 1 || species ==2, "not a valid animal type");
            if(species == 0){
                        delete _commonMetadata;
