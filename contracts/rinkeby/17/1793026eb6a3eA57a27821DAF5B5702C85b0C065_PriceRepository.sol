...
contract PriceRepository is Ownable, IPriceRepository, PoolACL {
    ...
    function getPrice1(address token1, address token2) public view returns (uint256 price) {
        if (token1 == token2) return 1e8;
        ...
        uint[] memory ratio = _router.getAmountsOut(1e8, addresses);

        if (ratio[1] == 0) {
            ratio = _router.getAmountsIn(1e8, addresses);
            price = uint256(ratio[0]);
        } else {
            price = uint256(ratio[1]);
        }
    }
}