...
contract USDSat is USDSatMetadata, ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    /// @notice Define in which block the halving must occur
    uint256 public blockHalving;

    /// @notice Used to inflate blockHalving each epoch incrementation
    ...
    private {
        uint256 i = 0;
        uint256 _blockOmega = 0;
        uint256 _epochLen = epochLen;

        while (i < epochMax) {
            if (i > 0) _epochLen *= inflateRatio;
            if (i == 9) {
                blockOmega = blockGenesis + _blockOmega;
    ...
    private {
        ...
        if (block.number > blockHalving && epoch < epochMax) {
            epochRegulator();
        }
