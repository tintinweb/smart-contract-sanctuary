...
contract PlayingCard is ERC721, Authorizable {
    ...
    uint public constant TOTALCARDS = 4; 
    uint256[TOTALCARDS+1] public counter;
    ...
    modifier validCardType(uint256 _cardType){
        require(_cardType != 0 && _cardType <= TOTALCARDS);
        _;
    }
    
    constructor(string memory _baseTokenURI,uint256[TOTALCARDS+1] memory _counter) public ERC721("PlayingCard", "4PLAY") {
    ...
    function mint(address _to,uint256 _cardType) public onlyAuthorized() validCardType(_cardType){
        require(_to != address(0),"NFT4Play: _to address not a valid");
        totalSupply += 1;
        _holderTokens[_to].add(counter[_cardType]);
        idCardType[counter[_cardType]] = _cardType;
        _safeMint(_to,counter[_cardType]);
        counter[_cardType]++;
    }

    function batchMint(address _to,uint256 _cardType,uint256 _numberOfToken) public onlyAuthorized() {
    ...
    function batchMint(address[] memory _addresses,uint256 _cardType,uint256[] memory _numberOfTokens) external onlyAuthorized() {
        ...
        for (uint256 i = 0; i < _addresses.length; i++) {
            batchMint(_addresses[i],_cardType, _numberOfTokens[i]);
        }
    }
