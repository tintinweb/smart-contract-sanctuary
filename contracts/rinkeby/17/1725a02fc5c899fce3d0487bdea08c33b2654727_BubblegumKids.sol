...
contract BubblegumKids is ERC721Enumerable, Ownable {
    ...
    // Dates
    uint256 public constant PRESALE_START_TIME = 1697239600; // 12pm EST September 29th 2021
    uint256 public constant SALE_START_TIME = 1907239600; // 3pm EST September 30th 2021
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (offset == 0) {
            return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, "unknown")) : "";
        } else {
    ...
    function presaleMint(uint256 amountToMint) public payable {
        require(balanceOf(msg.sender) <= 3, "You can only mint 3 before the sale begins");
   ...
   function mint(uint256 amountToMint) public payable {
        ...
        require(totalSupply() + amountToMint <= NFT_SUPPLY, "The amount of Bubblegum Kids you are trying to mint exceeds the MAX_NFT_SUPPLY.");
        ...
        for (uint256 i = 0; i < amountToMint; i++) {
            ...
            // Choose Cool Cat winner at 25% of sale
            if (totalSupply() == 5) {
