...
contract ArtGrailAuction is ReentrancyGuard, Ownable {
    ...
    function createAuction(
        ...
        require(
            IERC1155(_nftAddress).balanceOf(_msgSender(), _tokenId) == _quantity && IERC1155(_nftAddress).isApprovedForAll(_msgSender(), address(this)),
            "ArtGrailAuction.createAuction: Not owner and or contract not approved"
        );
    ...
    function resultAuction(address _nftAddress, uint256 _tokenId) external nonReentrant {
        ...
        require(IERC1155(_nftAddress).balanceOf(_msgSender(), _tokenId) >= auction.quantity && _msgSender() == auction.owner, "ArtGrailAuction.resultAuction: Sender must be item owner");
        ...
        // Ensure this contract is approved to move the token
        require(IERC1155(_nftAddress).isApprovedForAll(_msgSender(), address(this)), "ArtGrailAuction.resultAuction: auction not approved");
        ...
        if (winningBid > auction.reservePrice) {
            ...
            address creatorAddress = IArtGrailNFT(_nftAddress).creators(_tokenId);

            // Send royality fee to creator
            (bool royalityTransferSuccess, ) = creatorAddress.call{ value: platformRoyaltyAboveReserve }("");
        ...
        IERC1155(_nftAddress).safeTransferFrom(auction.owner, winner, _tokenId, auction.quantity, "");
    ...
    function cancelAuction(address _nftAddress, uint256 _tokenId) external nonReentrant {
        ...
        require(IERC1155(_nftAddress).balanceOf(_msgSender(), _tokenId) > 0 && _msgSender() == auction.owner, "ArtGrailAuction.cancelAuction: Sender must be item owner");
...
interface IArtGrailNFT is IERC721 {
    using SafeMath for uint256;
