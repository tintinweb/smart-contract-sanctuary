...
interface INFTologyNFT {
	function initialize(string memory _name, address creator, bool bPublic) external;
...
contract NFTologyFactory is Ownable {
	...
	function initialize(string memory _name, string memory _uri) public {
		createCollection(_name, _uri, true);
    }
	...
	function createCollection(string memory _name, string memory _uri, bool bPublic) public returns(address collection) {
        ...
        INFTologyNFT(collection).initialize(_name, msg.sender, bPublic);

		collectionCounter = collectionCounter.add(1);
		collections[collectionCounter] = Collection(collectionCounter, _name, collection, msg.sender, _uri);
...
contract NFTologyNFT is ERC1155, AccessControl {
	...
	function addItem(uint256 _maxSupply, uint256 _supply, uint256 _fee, string memory _uri) external returns (uint256) {
		...
		emit ItemAdded(itemCounter, _maxSupply, _supply);
