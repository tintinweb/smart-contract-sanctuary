...
contract ERC721Mintable is ERC721, MinterRole {
    ...
    modifier onlyMinter() {
        ...
        require(
            anyoneCanMint || isMinter(msg.sender),
            "MinterRole: caller does not have the Minter role"
        );
        _;
    }
...
contract ERC721Matcha is ERC721Full, ReentrancyGuard {
    ...
    // Mapping from token ID to sell price in Ether or to bid price, depending if it is an auctionor not
    ...
    */
    ...
    function createAuction(uint256 tokenId, uint _closingTime, address payable _beneficiary, uint256 _reservePrice) public payable {
    ...
    function withdraw(uint256 tokenId) public nonReentrant returns (bool) {
        ...
        require(auctions[tokenId].highestBidder == msg.sender, "You are not the highest bidder");
        require(now >= auctions[tokenId].auctionEnd, "Auction not yet ended.");
        require(auctions[tokenId].open, "Auction is ended.");
        require(auctions[tokenId].highestBid<auctions[tokenId].reserve, "The highest bid is equal or more than the reserve price, cannot withdraw. Call auctionFinalize.");
    ...
    // update admin beneficiary address
    function updateAdmin(address payable _admin) public {
