...
contract SamuraiCats is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    /*
     *  Mint max supply :
     * -> Private sale : From 1 to 300
     * -> Pre-sale : From 301 to 1000
     * -> Public sale : From 1001 to 4747
     */
    uint256 public maxSupplyPrivateSale = 300;
    ...
    function mintPreSale(uint256 numOfToken) external payable nonReentrant {
        require(
            preSaleList[msg.sender] || privateSaleList[msg.sender],
            "not in presale or private list"
        );
        require(
    ...
    function batchAddToPreSaleList(address[] memory addrs) external onlyOwner {
        uint256 length = addrs.length;
        for (uint256 i; i < length; ) {
