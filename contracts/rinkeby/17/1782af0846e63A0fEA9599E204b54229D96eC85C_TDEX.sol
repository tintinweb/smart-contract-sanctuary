...
contract TDEX {
    ...
    mapping(address => mapping(address => bool)) userApproveMap;            //  授权
    ...
    function insertToken(address _tokenContract) external returns (bool replaced)
    {
        // 假装拿到了token
    ...
    function removeToken(address _tokenContract) external returns (bool replaced)
    {
    ...
    function getToken(address _tokenContract) external view returns (string memory symbol, string memory name, address tokenContract, uint decimals)
    {
    ...
    function approveToken(address _tokenContract) external
    {
        if (userApproveMap[msg.sender][_tokenContract] == false)
        {
            SafeERC20.safeApprove(IERC20(_tokenContract), address(this), type(uint256).max);
            userApproveMap[msg.sender][_tokenContract] = true;
        }
    }

    function sendBuyOrder(address _tokenContract, uint256 _price, uint256 _usdt_amount) external
    {
        Token memory token = tokenMap.data[_tokenContract];
        ...
        if (userApproveMap[msg.sender][USDT.tokenContract] == false)
        {
            SafeERC20.safeApprove(IERC20(USDT.tokenContract), address(this), type(uint256).max);
            userApproveMap[msg.sender][USDT.tokenContract] = true;
        }

        // SafeERC20.safeTransferFrom(IERC20(USDT.tokenContract), msg.sender, orderManager, _usdt_amount);
        
        uint256 orderId = OrderManager(orderManager).createOrder(_tokenContract, msg.sender, _price, 0, _usdt_amount, 0);
        if (orderId > 0)
        {
            OrderManager(orderManager).handleMatchOrder(_tokenContract, orderId);
            OrderManager(orderManager).insertOrder(_tokenContract, orderId, msg.sender);
        }
    }

    function sendSellOrder(address _tokenContract, uint256 _price, uint256 _token_amount) external
    {
        Token memory token = tokenMap.data[_tokenContract];
        require(token.decimals > 0, "This contract address is not supported");
        ...
        SafeERC20.safeTransferFrom(IERC20(_tokenContract), msg.sender, orderManager, _token_amount);

        uint256 orderId = OrderManager(orderManager).createOrder(_tokenContract, msg.sender, _price, _token_amount, 0, 1);
        if (orderId > 0)
        {
            OrderManager(orderManager).handleMatchOrder(_tokenContract, orderId);
            OrderManager(orderManager).insertOrder(_tokenContract, orderId, msg.sender);
