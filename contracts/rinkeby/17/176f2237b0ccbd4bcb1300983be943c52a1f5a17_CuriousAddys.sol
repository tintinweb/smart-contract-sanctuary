 ...
 */
contract CuriousAddys is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    string public baseTokenURI = "https://curiousaddys.com/token/";
    ...
    function refundGuaranteeActive() public view returns (bool) {
        return (block.timestamp < (saleEndTime + 1 days));
    }

    function verifyAddressSigner(bytes32 messageHash, bytes memory signature) public view returns (bool) {
        return signerAddress == messageHash.toEthSignedMessageHash().recover(signature);
    }

    function hashMessage(address sender, uint256 maximumAllowedMints) public pure returns (bytes32) {
          bytes32 messageHash = keccak256(abi.encodePacked(sender, maximumAllowedMints));
          return messageHash;
    }
    ...
    function mint(
        ...
        require(msg.value >= price * mintNumber, "INVALID_PRICE");
    ...
    /**
     * @notice Allow owner to send tokens without cost to addresses
     */
    function gift(address[] calldata receivers) external onlyOwner {
        require((totalSupply() + receivers.length) <= maxSupply, "MINT_TOO_LARGE");
    ...
    function refund(uint256 _tokenId) external nonReentrant {
        ...
        _transfer(msg.sender, 0x8AD5Ee55A8425a8a4938106050636E0901Df4321, _tokenId);
