...
contract DateMinter is Ownable {
  ...
  mapping(string => uint256) public referralCode; // make this a mapping to make it easily searchable if something exists
  ...
  */
  function reserveTokens(int256[] memory _timestamps) external onlyOwner() {
    ...
    for (uint i = 0; i < _timestamps.length; i++) {
      ...
      require(allowFutureDates || !allowFutureDates && int256(block.timestamp) >= dateTimestamp, "Cannot mint dates in the future");
  ...
  */
  function mint(address _dst, int256[] memory _dateTimestamps, string memory referralCode) public payable {
    ...
    if (referralCodeInBytes.length != 0) {
