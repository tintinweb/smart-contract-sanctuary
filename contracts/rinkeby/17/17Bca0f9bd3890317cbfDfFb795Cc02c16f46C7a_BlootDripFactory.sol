...
contract BlootDripFactory is VRFConsumerBase, Ownable, AbstractERC1155Factory {
    ...
    ) VRFConsumerBase(_vrfCoordinator, _linkToken) ERC1155("ipfs://ipfs/"){
        keyHash = _keyHash;
        fee = _fee;
    ...
    function increaseSupply(uint256 index, uint256 additionalSupply) external onlyOwner {
        require(totalSupply(index) > 0, "URI: nonexistent token");

        if(additionalSupply > 0) {
            _mint(msg.sender, index, additionalSupply, "");
        }
    }        

    function editMetadata(uint256 index, string memory _ipfsMetadataHash) external onlyOwner {
        require(totalSupply(index) > 0, "URI: nonexistent token");

        ipfsHashes[index] = _ipfsMetadataHash;
    }  

    function pickWinners(uint256 numWinners, uint256 variantId) external onlyOwner {
        require(drawCounter.current() > lastDraw, "new VRF result not received yet");
