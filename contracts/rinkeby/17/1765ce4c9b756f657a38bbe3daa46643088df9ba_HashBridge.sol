...
contract HashBridge is Ownable {
	...
	event OrderAdd(uint indexed orderId, uint indexed offerId, uint rate, address withdrawAddress, uint amount, uint payAmount, address payAddress, uint reservedUntil);
	...
	function addOrder(uint _offerId, address _withdrawAddress, uint _amount, uint _payAmount) public {
		...
		orders.push(Order(_offerId, rate, _withdrawAddress, _amount, _payAmount, _payAddress, reservedUntil, false));
		emit OrderAdd(orderId, _offerId, rate, _withdrawAddress, _amount, _payAmount, _payAddress, reservedUntil);
	}
