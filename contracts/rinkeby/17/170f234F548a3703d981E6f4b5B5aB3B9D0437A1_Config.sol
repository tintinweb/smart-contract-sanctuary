...
contract Config is IConfig, Initializable {
    ...
    function initialize(uint8 _beta) public initializer {
        beta = _beta;
    }
    ...
    function setLiquidateThreshold(uint256 _liquidateThreshold)
        external
        override
    {
        require(
            _liquidateThreshold > 9000 && _liquidateThreshold <= 10000,
            "9000 < liquidateThreshold <= 10000"
        );
        liquidateThreshold = _liquidateThreshold;
    }

    function setLiquidateFeeRatio(uint256 _liquidateFeeRatio)
        external
        override
    {
        require(
            _liquidateFeeRatio > 0 && _liquidateFeeRatio <= 2000,
            "0 < liquidateFeeRatio <= 2000"
        );
