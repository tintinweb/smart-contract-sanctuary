...
library Decimal {
    ...
    /// Taken from https://github.com/albertocuestacanada/DecimalMath/blob/master/contracts/DecimalMath.sol
    /// @param x A fixed point Decimal.
    /// @param n An unsigned integer.
    /// @return An unsigned Decimal.
    function expD(decimal memory x, uint256 n) internal pure returns (decimal memory) {
        if (x.d == 0) return zero();
        if (n == 0) return one();

        decimal memory z = n % 2 != 0 ? x : one();
        uint256 d = x.d;
        for (n /= 2; n != 0; n /= 2) {
            d = d.muld(d);
...
abstract contract DecimalERC20 {
    ...
    function _transfer(
        ...
        require(__unsafeTransfer(_token, _to, roundedDownValue), "DecimalERC20: transfer failed");
        _validateBalance(_token, _to, roundedDownValue, balanceBefore);
    }

    // CAUTION: this function does not require the transfer being successful but only return the result
    function _unsafeTransfer(
        ...
        if (__unsafeTransfer(_token, _to, roundedDownValue)) {
            return _isNotInflationaryToken(_token, _to, roundedDownValue, balanceBefore);
        }
        return false;
    }
    ...
    function __unsafeTransfer(
        ...
        // solhint-disable avoid-low-level-calls
        (bool success, bytes memory data) =
            address(_token).call(abi.encodeWithSelector(_token.transfer.selector, _to, _roundedDownValue));
    ...
    function _validateBalance(
        ...
        require(
            _isNotInflationaryToken(_token, _to, _roundedDownValue, _balanceBefore),
            "DecimalERC20: balance inconsistent"
        );
    }

    function _isNotInflationaryToken(
        IERC20 _token,
        address _to,
        uint256 _roundedDownValue,
        Decimal.decimal memory _balanceBefore
    ) private view returns (bool) {
        return _balanceOf(_token, _to).cmp(_balanceBefore.addD(_toDecimal(_token, _roundedDownValue))) == 0;
    }
}
