...
contract Stake {
    ...
    function stake(uint8 _plan, uint256 _tokens) external {
        ...
        userStakedTokensByContract[msg.sender].add(_tokens);
        ...
        if (stakePlan.isFixedMaturityDate) {
            ...
            FixedStake memory fixedStake;
            fixedStake.createdDate = block.timestamp;
            fixedStake.maturityDate = block.timestamp + stakePlan.fixedDays * 1 days;
            fixedStake.tokens = _tokens;
            fixedStake.interestRate = stakePlan.interestRate;
            fixedStakes[_plan][msg.sender].push(fixedStake);
            fixedStakeLengthByUser[_plan][msg.sender] += 1;
    ...
    function withdrawFixedMaturityDate(uint8 _plan) external {
        require(totalPlans >= _plan, "Plan doesn't exist");
        StakePlan storage stakePlan = stakePlans[_plan];
        require(stakePlan.isFixedMaturityDate, "Wrong Plan");
        require(
            fixedStakes[_plan][msg.sender].length > 0,
            "No token to withdraw"
        );
        require(stakePlan.maturityDate >= block.timestamp, "Not Right Time");
        stakePlan.totalStaked = stakePlan.totalStaked.sub(
            fixedStakes[_plan][msg.sender][0].tokens
        );
        userStakedTokensByPlan[_plan][msg.sender] = userStakedTokensByPlan[
            _plan
        ][msg.sender].sub(fixedStakes[_plan][msg.sender][0].tokens);
        require(
        ...
        token.safeTransfer(
            msg.sender,
            fixedStakes[_plan][msg.sender][0].tokens.mul(stakePlan.interestRate)
        );
    ...
    function withdraw(uint8 _plan, uint8 _index) external {
        ...
        userStakedTokensByContract[msg.sender].sub(fixedStake.tokens);
        uint256 maturityToken = fixedStake.tokens.mul(stakePlan.interestRate);
