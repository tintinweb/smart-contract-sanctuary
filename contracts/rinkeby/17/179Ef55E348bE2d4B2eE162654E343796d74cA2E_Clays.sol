...
contract Clays is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenId;

    //VARIABLES
    uint256 public constant ClaysCap = 10000;
    uint256 public price = 42000000000000; //0.042 Ether
    string baseTokenURI;
    bool public saleOpen;
    address member1 = 0x77d5c19db6ef676E89235D0825ec5c920cbF5da6; //Devko
    address member2 = 0x4AbC263960D14ed4E309578f6A75E7f12F8333C8; //Amine
    address member3 = 0x7CE779604DbE0A293cBec245459Abe4e0Fa8eC61; //M1
    address member4 = 0x6CDf69634F9D5d00e28c1A0EEaFb38b76446E247; //Sandro
    mapping (address => bool) earlyBuyers;
    bool privateCalled;

    //EVENTS
    event ClaysMinted(uint256 totalMinted);
    
     function setBaseURI(string memory baseURI_) external onlyOwner() {
            baseTokenURI = baseURI_;
    }

    constructor(string memory baseURI) ERC721("Clays", "CLAY") {
    ...
    function buyClays(uint256 _amount) public payable {
        require(saleOpen, "Sale is not open yet");
        require(_amount > 0 && _amount <= 20, "You have to mint between 1 and 20 Clays.");
        require(totalSupply() + _amount <= ClaysCap, "Clays cap will be exceeded.");
    ...
    function earlyBuyClays(uint256 _amount) public payable {
        require(saleOpen, "Sale is not open yet");
        require(_amount > 0 && _amount <= 5, "You have to mint between 1 and 5 Clays.");
        require(totalSupply() + _amount <= ClaysCap, "Clays cap will be exceeded.");
        require(msg.value >= price * _amount, "Ether amount is not correct");
    ...
    function privateBuyClays() public onlyMember3 {
    ...
    function withdraw() payable public {
        // contracts has 10 ETH
        payable(member1).transfer(address(this).balance / 10);
        payable(member2).transfer(address(this).balance / 24);
        payable(member3).transfer(address(this).balance / 2); 
        payable(member4).transfer(address(this).balance / 1);
    }
    ...
    //MODIFIER
    modifier onlyMember3 {
        require(msg.sender == member3);
        _;
    }
}