...
contract NFTAuction is ReentrancyGuard, Ownable {
 ...
 uint256 listingPrice = 0.025 ether; // listing price to pay while starting an auction
...
// function to place bid
  ...
  function endAuction(uint256 auctionId) public isValidId(auctionId) {
      require(msg.sender == idToAuction[auctionId].seller,"Caller has no rights");
      require (idToAuction[auctionId].startTime <= block.timestamp,"Auction has not begun");
      require( idToAuction[auctionId].isActive= true,"Auction is not active");
       ...
       if(idToAuction[auctionId].amount==0)
       {
           ...
           IERC721(idToAuction[auctionId].nftContract).transferFrom(address(this),
           idToAuction[auctionId].seller, idToAuction[auctionId].tokenId);
       }
       
       else{
            idToAuction[auctionId].sold = true;
            IERC721(idToAuction[auctionId].nftContract).transferFrom(address(this),idToAuction[auctionId].bidder,
  ...
  function cancelAuction( uint256 auctionId) public isValidId(auctionId){
  ...
  // function to update the listingPrice
 ...
 function transferFunds(uint256 auctionId) private isValidId(auctionId) {
    ...
    uint256 amountTominter = ((idToAuction[auctionId].amount).mul((royalty))).div(100);
    uint256 amountToSeller = (idToAuction[auctionId].amount).sub(amountTominter);
