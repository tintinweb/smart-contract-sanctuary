...
// File: viralswap/interfaces/IWETH.sol
...
// File: viralswap/interfaces/IUniswapRouter02.sol
...
// File: viralswap/libraries/TransferHelper.sol
...
library ViralswapLibrary {
    ...
    function vaultFor(address factory, address tokenA, address tokenB) internal pure returns (address vault) {
        ...
        vault = address(uint(keccak256(abi.encodePacked(
                ...
                factory,
                keccak256(abi.encodePacked(token0, token1)),
                hex'05a6c30e46206aea7da8c57dd62767d14a848f5d4a6fe945425452aeb8818572' // init code hash
            ))));
    }
...
// File: viralswap/ViralswapRouter02.sol
...
contract ViralswapRouter02 is IViralswapRouter02 {
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            IViralswapPair(ViralswapLibrary.pairFor(factory, input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        amounts = ViralswapLibrary.getAmountsOut(factory, amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'ViralswapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, ViralswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    function swapTokensForExactTokens(
        ...
        amounts = ViralswapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'ViralswapRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, ViralswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
        ...
        returns (uint[] memory amounts)
    {
        ...
        amounts = ViralswapLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'ViralswapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(ViralswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'ViralswapRouter: INVALID_PATH');
        amounts = ViralswapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, 'ViralswapRouter: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(ViralswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
    ...
    **/
    function buyExactTokensForTokens(
        ...
        amounts = new uint[](path.length);
        amounts[0] = amountIn;
        amounts[1] = ViralswapLibrary.quoteVault(factory, path[0], path[1], amountIn);
    ...
    function vaultFor(address tokenA, address tokenB) public view returns (address) {
        return ViralswapLibrary.vaultFor(factory, tokenA, tokenB);
    }
    function quoteVault(address tokenIn, address tokenOut, uint amountIn) public view virtual override returns (uint amountOut) {
        return ViralswapLibrary.quoteVault(factory, tokenIn, tokenOut, amountIn);
    }
