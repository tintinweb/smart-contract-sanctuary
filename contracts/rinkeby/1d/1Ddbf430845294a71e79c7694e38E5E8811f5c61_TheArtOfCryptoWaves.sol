...
contract TheArtOfCryptoWaves is ERC721Enumerable, Ownable, AccessControl {
    ...
    // uint256 public constant TOTAL_NUMBER_OF_TOKENS = 2121;
    uint256 public constant TOTAL_NUMBER_OF_TOKENS = 21;

    uint256 public limit_per_wallet = 5;
    uint256 public giveaway_reserved = 42;
    address public marketing_wallet;

    // bool public paused_mint = true;
    bool public paused_mint = false;
    string private _baseTokenURI = "";

    modifier whenMintNotPaused() {
        require(!paused_mint, "TheArtOfCryptoWaves: mint is paused");
        _;
    }

    event MintPaused(address account);
    event MintUnpaused(address account);
    event NewMainSaleMint(address sender, uint256 tokenId);
    ...
    function mint(uint256 num) public payable whenMintNotPaused {
        ...
        require(
            tokenCount + num <= limit_per_wallet,
            "TheArtOfCryptoWaves: You reached the minting limit per wallet"
        );
        require(
            supply + num <= TOTAL_NUMBER_OF_TOKENS - giveaway_reserved,
        ...
        for (uint256 i; i < num; i++) {
            ...
            emit NewMainSaleMint(msg.sender, supply + i);
        }
    }
    ...
    function withdrawAmountToWallet(uint256 amount)
        public
        onlyRole(WHITE_LIST_ROLE)
    {
        ...
        require(
            payable(marketing_wallet).send(amount),
    ...
    function withdrawAllToWallet() public onlyRole(WHITE_LIST_ROLE) {
        ...
        require(
            payable(marketing_wallet).send(_balance),
