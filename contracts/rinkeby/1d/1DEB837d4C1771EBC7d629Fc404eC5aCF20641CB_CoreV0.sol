...
interface IFToken{
    ...
    function setValuePerShare(uint _vps) external payable  returns(bool);
}
...
contract CoreV0 is ICoreV0 {
    ...
    *Methods */
    /**
    * @dev Step 1: Deposit NFT and Mint erc20 tokens
    * @param _NFTContract, NFT/ERC721 Contract address.
    * @param _tokenId, NFT Contract's token Id.
    * @param _supply, Total supply for the new Erc20 to be minted.
    */
    function depositNFTAndMintTokenFT(
        address _NFTContract,
        uint256 _tokenId,
         uint256 _supply
    ) 
        external override
        tokensTransferable(_NFTContract, _tokenId)
        ...
        returns (bytes32 receiptId)
    {
        ...
        (address erc20Addr, bool success)= mintERC20Token( _supply, _NFTContract, _tokenId);
        require(success,"Unable to Mint Erc20 Tokens");
        //Constant UNIRV2 Addr
        require(IERC20(erc20Addr).approve(AMM_ROUTEV2, type(uint256).max));//only uint256
        
        nftIdtoRec[_NFTContract][_tokenId]= receiptId;
    ...
    /**  Step 2: Approve the ERC20 minted tokens for LP seed init
    * param _erc20Token, ReceiptId.
    * return bool
    *
    function approveTokensForInitLPSeed(
        address _erc20Token) 
        internal  returns(bool) {
       // address _erc20Token = receipts[_receiptId].mintedERC20Addr;
        return IERC20(_erc20Token).approve(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D,  IERC20(_erc20Token).balanceOf(address(this)));
        
    }
    /**Step 3: initUniLPwithETH -> 2 is merged in to 3.
     * Call this only once
     */
    ...
    onlyReceiptOwner(_receiptId) receiptExist(_receiptId) 
       external override  payable returns (uint tokenAmount, uint amountETH, uint liquidity){
     ...
     */
    function buyMintedTokens(bytes32 _receiptId) receiptExist(_receiptId) override external payable returns(uint256){
    ...
    function exit(bytes32 _receiptId) receiptExist(_receiptId) public payable {
        ...
        require(msg.value == _eth,"Insufficent amount");
        uint valuePerFT = _eth.mul(10**6).div(curveParameters[_erc20Token].totalSupply);
        //1/1000 = 0.001
        ...
        uint256 sellerGains = amountToBurn.mul(valuePerFT);//1000*0.001=1
        ...
        require(burnERC20Token( _erc20Token, amountToBurn),"Burning failed check burnol amount!");
        safeTransferETH(receipts[_receiptId].seller,sellerAmountETH);
        if(!(_xyFlag) && remainingFT>0){

            IFToken(_erc20Token).setValuePerShare{value:amountETHForFT}(amountETHForFT.div(remainingFT));
    ...
    function testvaluePerFT(uint256 amtIn,address _erc20Token)public view returns(uint256 valuePerFT,uint256 amountToBurn,uint256 sellerGains){
         ...
         valuePerFT = amtIn.mul(10**6).div(curveParameters[_erc20Token].totalSupply);
         valuePerFT = valuePerFT.div(10**6);
         amountToBurn= getBalance(_erc20Token,address(this));
         sellerGains = amountToBurn.mul(valuePerFT);
    ...
    function getPrices(address _pair) public view returns(uint price0Cumulative, uint price1Cumulative, uint32 blockTimestamp){
       (price0Cumulative,  price1Cumulative,  blockTimestamp)= UniswapV2OracleLibrary.currentCumulativePrices(_pair);
       price0Cumulative = price0Cumulative.div((2**112));
       price1Cumulative = price1Cumulative.div((2**112));
       //console.log("Prices %s %s %s",(price0Cumulative,  price1Cumulative,  blockTimestamp));
    }

    
    function mintERC20Token( uint256 _supply,address _NFTAddr, uint256 _tokenId) internal returns(address erc20Addr,bool success){
        //address nftOwn = address(this);

         FT newNFTERC20addr = new FT( _supply, _NFTAddr,  _tokenId, address(this));
        //address newNFTERC20addr = IFTfactory(0x1102Bd416CB31Bca681b9f2BB896155895f97428).createFuseERC20( _supply, _NFTAddr,  _tokenId,  msg.sender);
        //nfterc20[newNFTERC20addr] = Erc20DB(msg.sender,1*10**17,0);
        return (address(newNFTERC20addr),true);
    }
    
    function burnERC20Token(address _erc20Address,uint256 _tokenAmt) public returns(bool){
        IFToken(_erc20Address).burn(_tokenAmt);
        return true;
    }
    ...
    function getReceiptId(address _nftAddr, uint _tknId) override public view returns (bytes32) {
    ...
    receive() external payable {
        ...
        require(msg.sender == address(AMM_ROUTEV2),"Not from UniswapV2 Route");
...
contract FT is  ERC20,AccessControl {
    ...
    constructor(uint256 _supply,address _NFTAddr, uint256 _tokenId, address _mintAt) ERC20("Fusible V0", "FT V0") {
        nftAddr = _NFTAddr;
        tokenId = _tokenId;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(OPERATOR_ROLE,_msgSender());
        _mint(_mintAt, _supply);
    }




    function burn(uint256 _tknAmt) external onlyOperator returns(bool) {
    ...
    function setValuePerShare(uint _vps) external payable onlyOperator returns(bool){
        valuePerFT = _vps;
        return(true);
    }
    ...
    function burnMyFT() public {
        ...
        require(address(this).balance>=balFToken,"Not enough balance");
        _burn(_msgSender(),balFToken);
        safeTransferETH(_msgSender(), valuePerFT.mul(balFToken));
    ...
    receive() onlyOperator external payable {
        // custom function code
        // require(msg.sender == address(),"Not from UniswapV2 Route");
        //what to do with dust?
       // emit Received(msg.value,msg.sender);
...
interface ICoreV0 {
    ...
    function depositNFTAndMintTokenFT(
        ...
        address _NFTContract,
        uint256 _tokenId, uint256 _supply
    )  external  returns (bytes32 receiptId);
    //Step 2
    //function approveTokensForInitLPSeed(bytes32 _receiptId,uint32 _rRatio,bool _cap,uint256 _maxCap, bool _rRtype) external returns(bool);
    //Step 2
    function initUniLPwithETH(bytes32 _receiptId,uint _amountTokenDesired,
    uint32 _rRatio,uint256 _maxCap,bool _rRtype) external  payable returns (uint amountToken, uint amountETH, uint liquidity);
    //Step 4
    function buyMintedTokens(bytes32 _receiptId)  external payable returns(uint256);
    //Getter for receiptId.
    
    //function getReceiptId(address _nftAddr, uint _tknId) external view returns (bytes32);
    
    function getReceiptData(bytes32 _receiptId)
        external
        view
        returns (
            address seller,
