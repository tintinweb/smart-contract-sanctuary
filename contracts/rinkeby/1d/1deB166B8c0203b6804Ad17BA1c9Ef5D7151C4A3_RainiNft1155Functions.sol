...
contract RainiNft1155Functions is AccessControl, ReentrancyGuard {
  ...
  function merge(uint256 _cardId, uint256 _level, uint256 _mintAmount, uint256[] memory _tokenIds, uint256[] memory _burnAmounts, uint256[] memory _data) 
    external payable nonReentrant {
      ...
      require(_level > 0 && _cardLevel.conversionRate > 0);
      ...
      for (uint256 i = 0; i < _tokenIds.length; i++) {
        require(_burnAmounts[i] <= nftContract.balanceOf(_msgSender(), _tokenIds[i]));
        IRainiNft1155.TokenVars memory _tempTokenVars =  nftContract.tokenVars(_tokenIds[i]);
        require(_tempTokenVars.cardId == _cardId);
        require(_tempTokenVars.level < _level);
        ...
        nftContract.burn(_msgSender(), _tokenIds[i], _burnAmounts[i]);

        if (_locals.willCallPool || _locals.willCallSubContract) {
          mergedTokensIds[_locals.currentTokenToMint] = _tokenIds[_locals.currentTokenToMint];
          if (_locals.totalPointsBurned > _locals.currentTokenToMint * _cardLevel.conversionRate || i == _tokenIds.length - 1) {
            _locals.currentTokenToMint++;
            if (_locals.willCallPool) {
              nftStakingPool.mergeTokens(_locals.currentTokenToMint + nftContract.maxTokenId(), mergedTokensIds, address(this));
            }
            if (_locals.willCallSubContract) {
              subContract.onMerged(_locals.currentTokenToMint + nftContract.maxTokenId(), mergedTokensIds, address(this), _data);
            }
      ...
      require(_locals.totalPointsBurned == _locals.cost);

      require(mergeFees[_level] * _mintAmount <= msg.value);
  ...
  function mintWithPoints(uint256[] memory _cardId, uint256[] memory _amount, bool[] memory _useUnicorns, uint256[][] memory _data, address[] memory _rainbowPools, address[] memory _unicornPools)
    ...
    for (uint256 i = 0; i < _cardId.length; i++) {
      ...
      require(block.timestamp >= card.mintTimeStart);
      require(cardLevel.numberMinted + _amount[i] <= card.allocation);
      require(numberMintedByAddress[_msgSender()][_cardId[i]] + _amount[i] <= card.maxMintsPerAddress);

      if (_useUnicorns[i]) {
        require(card.costInUnicorns > 0);
        _locals.totalPriceUnicorns += card.costInUnicorns * _amount[i] * POINT_COST_DECIMALS;
      } else {
        require(card.costInRainbows > 0);
    ...
    for (uint256 n = 0; n < 2; n++) {
      ...
      if (totalPrice > 0) {
        ...
        for (uint256 i = 0; i < loopLength; i++) {
          IStakingPool pool;
          if (loopTypeUnicorns) {
            require((unicornPools[_unicornPools[i]]));
            pool = IStakingPool(_unicornPools[i]);
          } else {
            require((rainbowPools[_rainbowPools[i]]));
        ...
        if (remainingPrice > 0) {
          ...
          require(totalPrice - remainingPrice >= minPoints);
          uint256 pointsToEth = loopTypeUnicorns ? unicornToEth : rainbowToEth;
          require(msg.value * pointsToEth > remainingPrice);
    ...
    // Add minting fees
    _amountEthToWithdraw += _locals.fee;

    require(_amountEthToWithdraw <= msg.value);
