...
contract Catnip is Context, IBEP20, Initializable {
    ...
    function airDropClaim() public {
        ...
        uint percentToGetOut = numberOfWeeksSinceRelease.mul(10);  

        uint amountToGiveClaimer = airDropTokensTotal[claimer].mul(percentToGetOut).div(100);

        require(amountToGiveClaimer > amountOfNIPClaimedSoFar,"You have claimed all the airdrop available so far, check back later weeks for more, if you got this error please contact Yoshiko in the discord.");
        // this is to catch a strange test case, I doubt it can ever happen in productions, but if for some reason teh amoung to give is less than the amount claimed so far, then cancel the operation.

        amountToGiveClaimer = amountToGiveClaimer.sub(amountOfNIPClaimedSoFar);
        require(amountToGiveClaimer > 0, "You have claimed all the airdrop available so far, check back later weeks for more");

        approveInternal(ownerOfToken, claimer, amountToGiveClaimer.mul(10**9));        // approves amount to claim from owner address
        transferFrom(ownerOfToken, claimer, amountToGiveClaimer.mul(10**9));       // transfers the airdrop amount from the owner address to the claimer
    ...
    function airDropClaimWeekSimulation(uint256 NumberOfWeeksToTest) public {
        ...
        // console.log("percentToGetOut", percentToGetOut);
    ...
    function initializeAirdropMapping() internal {
        ...
        airDropTokensTotal[0x8C9ad9Ff0655Db057317FDd37B4aabcC5411E87D] = 3727457;
