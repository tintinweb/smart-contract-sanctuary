...
contract Vendor is Ownable {
  ...
  function sellTokens(uint256 _amountOfTokens) public {
    require( _amountOfTokens > 0, "You need to sell some amount of tokens");
    
 
    uint256 amountToSend = _amountOfTokens / tokensPerEth;
    uint256 allowance = yourToken.allowance(msg.sender, address(this));

    require (allowance >= _amountOfTokens, "Additional Allowance required");
    require (yourToken.transferFrom(msg.sender, address(this), _amountOfTokens), "Failed to send tokens");

    (bool sent,) = (payable(msg.sender)).call{ value: amountToSend }("");
    require( sent, "Faild to send Ether");

    emit SellTokens(msg.sender, amountToSend, _amountOfTokens);
