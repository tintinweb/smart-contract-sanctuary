...
contract NFTMigration {
    function migrate(uint256 nftId, address to) public virtual {}
}

contract SecondMiddleware is IERC721Receiver, Ownable{
    ...
    MintABI private previouswrapper;
    Withdrawal private nftContract;
    NFTMigration private migrateContract;
    ...
    function setPrevWrapper(address _contractAddress) public onlyOwner{
        previouswrapper = MintABI(_contractAddress);
    }

    function getPrevWrapper() public view returns(address) {
        return address(previouswrapper);
    }
    ...
    function migrateNFT(uint256 tokenId) public {
            migrateContract.migrate(tokenId,address(this));
            // address to = previouswrapper.ownerOf(tokenId);
            // previouswrapper.migratorBurn(tokenId);
            // tokenMinter.mint(to, tokenId);
	}
}
