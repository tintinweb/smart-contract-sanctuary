...
contract SpaceInvaders is AccessControlUpgradeable {
    ...
    int128 public mineSpaceshipFee;
    ...
    uint256 public spaceshipTotalLimit;
    ...
    function initialize() public initializer {
        ...
        mineSpaceshipFee = ABDKMath64x64.divu(10, 1); // $10
    }
    ...
    function issueSpaceship() public onlyNonContract oncePerBlock(msg.sender) returns (uint256) {
        uint256 shipFee = usdToSip(mineSpaceshipFee);

        require(token.balanceOf(msg.sender) >= shipFee, "Not enough token amount");
    ...
    function _createSpaceship(address player) private returns (uint256) {
        ...
        require(length <= spaceshipTotalLimit, "Full");
    ...
    function _oncePerBlock(address user) internal {
        require(lastBlockNumberCalled[user] < block.number);
...
contract Spaceship is ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, AccessControlUpgradeable {
    ...
    Ship[] public tokens;
    ShipCosmetics[] public cosmetics;

    function initialize() public initializer {
        __ERC721_init("SI Ship", "SIPS");
    ...
    function mint(
        ...
        tokens.push(Ship(rarity, class, element));
        cosmetics.push(ShipCosmetics(0, RandomUtil.combineSeeds(seed, 1)));
    ...
    function mySpaceships() public view returns (uint256[] memory ownerTokens) {
    ...
    function spaceshipsOfOwner(address _owner) public view returns (uint256[] memory ownerTokens) {
        ...
        if (tokenCount == 0) {
            ...
            for (shipId = 0; shipId <= tokenCount - 1; shipId++) {
                result[resultIndex] = tokenOfOwnerByIndex(_owner, shipId);
                resultIndex++;
            }
            return result;
        }
    ...
    {
        Ship memory s = tokens[id];
        return (s.rarity, s.class, s.element);
    }
    ...
    function _noFreshLookup(uint256 id) internal view {
        require(id < firstMintedOfLastBlock || lastMintedBlock < block.number);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0) && to != address(0)) {
            ...
            require(lastTransferTimestamp[tokenId] < block.timestamp.sub(TRANSFER_COOLDOWN));
