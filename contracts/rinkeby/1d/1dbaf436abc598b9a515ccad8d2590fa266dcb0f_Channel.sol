...
contract Channel {
    event Message(string indexed message, address addr);
    
    mapping(address => bool) public posted;
    // address[] messagePosters;
    
    constructor() {}
    
    function postMessage(string calldata message, bytes calldata signature, bytes32 hash) public {
        // require(ICorruptions(0x5BDf397bB2912859Dbd8011F320a222f79A28d2E).balanceOf(msg.sender) > 0, "not owner");
        
        (address addr, ECDSA.RecoverError err) = ECDSA.tryRecover(hash, signature);
        
        require (err == ECDSA.RecoverError.NoError, "couldnt recover address from hash and signature");
        require (msg.sender == addr, "address recovered does not match sender");
        posted[addr] = true;
        emit Message(message, addr);
    }

    function hasSentMessage(address addr) public view returns(bool) {
        return posted[addr];
    }
}