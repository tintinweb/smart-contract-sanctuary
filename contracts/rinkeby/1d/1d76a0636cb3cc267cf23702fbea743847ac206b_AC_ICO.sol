...
//   }
   ...
   **/
  function buyTokens(uint256 _value) public payable whenSaleIsActive {
    uint256 weiAmount = _value; // Calculate tokens to sell
  ...
  function payTokens(uint256 _value,address _address) public payable whenSaleIsActive {
    uint256 weiAmount = _value; // Calculate tokens to sell
    uint256 tokens = weiAmount.mul(RATE);
    

    token.transferFrom(msg.sender,_address, tokens); // Send tokens to buyer
    buyTokens(_value);
  }
}