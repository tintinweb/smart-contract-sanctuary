...
contract FaithReservationEvent is AccessControl {
    ...
    bool public claimsOpen = false;                    // is the claims window open
    bool public holdsOpen = false;                     // is the holds window open
    ...
    constructor(address _claimTokenContract, uint256 _price, address _adminRole) {
        ...
        claimTokenContract = IReservationToken(_claimTokenContract);        // reference to the ERC20 token contract for tokens stored in the vault
        price = _price;                                                     // price per token
    }

    function getTokensRemainingToClaim(address _sender) public view returns (uint256 totalRemaining) {
      return _holds[_sender] - _claims[_sender];
    }

    // hold tokens with wei
    function hold() public payable {
        _hold(_msgSender(), msg.value);

        // SEND PAYBLE from this contract ONTO THE ADMINISTRATOR
    }

    function holdForOffchainBuyer(address buyer, uint256 weiAmount) public onlyRole(ADMIN_ROLE) {
        offChainBalancesTotal += weiAmount;
        _offchainbalances[buyer] += weiAmount;
        _hold(buyer, weiAmount);    
    }

    function _hold(address buyer, uint256 buyAmount) internal {

        // check to make sure holds are open
        require(holdsOpen, "HOLDS_NOT_OPEN");

        // check the passed wei matches what is passed and isn't empty
        require(buyAmount != 0, "PAYABLE_IS_ZERO");

        // calculate the number of tokens from the ethereum passed 
        uint256 numberOfTokensImplicit = (buyAmount * 10 ** 18) / price;
        
        // check that the totalnumber of tokens requested wont go over the number in the vault to start
        require(holdsTotal + numberOfTokensImplicit <= depositsTotal, "NOT_ENOUGH_TOKENS_IN_VAULT");

        // keep track of the callers total tokens held and eth deposted
        _holds[buyer] += numberOfTokensImplicit;
        _balances[buyer] += buyAmount;

        // keep track of contracts total number of tokens held and total eth deposited
        holdsTotal += numberOfTokensImplicit;
        balancesTotal += buyAmount;

        emit Held(buyer, numberOfTokensImplicit);
    }
    ...
    function claimForOffchainBuyer(address claimer) public onlyRole(ADMIN_ROLE) {
        _claim(claimer);
    }

    // TODO - check the function declaration - is private correct or shiould it be internal - any thing else needed? should this be mutex? prob eh
    function _claim(address claimer) internal {
        // check to make sure claims are open
        require(claimsOpen, "CLAIMS_NOT_OPEN");

        // check the claimer has tokens to be issued
        require(_holds[claimer] != 0, "NO_TOKENS_ON_HOLD_FOR_CLAIMER");

        // check the claimer has tokens remaining to be issued
        require(_holds[claimer] >= _claims[claimer], "NO_TOKENS_REMAINING_TO_CLAIM");
       
        // track total tokens claimed
        _claims[claimer] += _holds[claimer];
        claimedTotal += _holds[claimer];    

        // TODO check result of the transfer call - boolean?
        // send tokens to the caller
        claimTokenContract.transfer(claimer, _holds[claimer]);

        emit Claimed(claimer, _holds[claimer]);    
    }
    ...
    function depositVault(uint256 amountToDeposit) public onlyRole(ADMIN_ROLE) {
        ...
        require(claimTokenContract.transferFrom(_msgSender(), address(this), amountToDeposit), 'FAILED_TO_DEPOSIT');

        depositsTotal += amountToDeposit;

        emit DepositedVault(_msgSender(), amountToDeposit);
    }
    ...
    function withdraw() public onlyRole(ADMIN_ROLE) {        
        ...
        require(!holdsOpen && !claimsOpen, 'CANNOT_WITHDRAW_WHEN_OPEN');
        ...
        emit Withdrawn(payable(_msgSender()), balancesTotal);
    }
    ...
    function withdrawVault() public onlyRole(ADMIN_ROLE) returns (bool result) {
        ...
        require(!holdsOpen && !claimsOpen, 'CANNOT_WITHDRAW_TOKENS_IN_VAULT_WHEN_OPEN');

        // TODO check result of the transfer call - boolean?
        // Send all tokens in vault to the caller.
        claimTokenContract.transfer(_msgSender(), scaledTokensRemaining);

        emit WithdrawnVault(_msgSender(), scaledTokensRemaining);
        return true;
    }
    ...
    function openHolds() public onlyRole(ADMIN_ROLE) {
        require(depositsTotal - holdsTotal > 0, 'NOT_ENOUGH_TOKENS_TO_OPEN_HOLDS');
        holdsOpen = true;
    }
    ...
    function changePrice(uint _price) public onlyRole(ADMIN_ROLE) {
        price = _price;
    }
}
