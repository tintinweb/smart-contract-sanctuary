...
contract Swap is OwnableUpgradeable {
    ...
    event SwapToken(address token,uint256 amount,uint256 receiveAmount);
        ...
        returns (uint256 receiveAmount, uint256 blockNumber)
    {
        ...
        require(token.allowance(msg.sender, address(this)) > _amount,"Allowance not enought.");
        uint256 rate = getSwapRate(_token); 
        require(rate != 0,"Not found rate for swap.");
        receiveAmount = _amount.mul(rate).div(1000000000000000000);
        ...
        emit SwapToken(_token,_amount,receiveAmount);    
    }

    function deposit() public payable{
        uint256 rate = getETHRate().mul(10000000000);
        uint256 receiveAmount = msg.value.mul(rate).div(1000000000000000000);

        emit Deposit(msg.sender,rate,receiveAmount);
    }

    function withdraw(address payable _receiver,uint256 _amount) public payable onlyOwner{
    ...
    function getSwapRate(address _token) internal view returns(uint256){
        if(_token == usdtToken){
            return getUSDTRate().mul(10000000000);
