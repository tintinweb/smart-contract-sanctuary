...
contract PixelToken is ERC20, Ownable {
    ...
    address public marketingWallet = 0x045121dd99E4Ab1db5359C2C5321C958E42e2596;
    ...
    address private constant BURN_WALLET =
    ...
    event FeesUpdated(
        uint8 oldMarketingFee,
        uint8 newMarketingFee,
        uint8 oldLiquidityFee,
        uint8 newLiquidityFee,
        uint8 oldBurnFee,
        uint8 newBurnFee
    );

    constructor() ERC20("PIXEL", "PIX", 9) {
    ...
    function updateUniswapV2Router(IUniswapV2Router02 newAddress)
        external
        onlyOwner
    {
        ...
        address _uniswapV2Pair = IUniswapV2Factory(newAddress.factory())
            .createPair(address(this), newAddress.WETH());
        automatedMarketMakerPairs[_uniswapV2Pair] = true;
        uniswapV2Pair = _uniswapV2Pair;
    }
    ...
    function _getValues(address sender, uint256 amount)
        private
    ...
    function _transfer(
        ...
        if (amount == 0) super._transfer(sender, recipient, 0);
        ...
        if (
            ...
            uint256 marketingTokensToSwap = (numTokensSellToAddToLiquidity *
                _marketingFee) / (_marketingFee + liquidityFee);
            uint256 liquidityTokens = (numTokensSellToAddToLiquidity -
                marketingTokensToSwap) / 2;
        ...
        if (!(isExcludedFromFee[sender] || isExcludedFromFee[recipient])) {
            uint256 feeDenominator = block.timestamp > antibotEndTime
                ? 100
                : totalFees + 1;
            uint256 burnFeeAmount = (amount * burnFee) / feeDenominator;
            super._transfer(sender, BURN_WALLET, burnFeeAmount);
            uint256 otherFeeAmount = (amount * (liquidityFee + marketingFee)) /
                feeDenominator;
