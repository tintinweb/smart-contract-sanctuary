...
contract TomiStaking is Ownable, ReentrancyGuard, Pausable {
    ...
    function totalRevenueShare() external view returns (uint256) {
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < userInfo.length; i++) {
            sumOfTotal = sumOfTotal.add(
                _balances[userInfo[i]].mul(_lastStakedTime[userInfo[i]])
            );
        }

        uint256 totalTimeStaked = block.timestamp.sub(_lastStakedTime[account]);
        return
    ...
    function getRewardForDuration() external view returns (uint256) {
        return (rewardRate.mul(rewardsDuration)).add(_revenueShared);
    }
