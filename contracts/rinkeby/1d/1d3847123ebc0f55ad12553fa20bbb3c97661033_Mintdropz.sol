...
contract Mintdropz is ERC721, Ownable {
    ...
    function _mintNFT(uint numberOfTokens) public payable onlyOwner {
        uint256 _bananaPrice = 3000000000000000000; // each token price is 0.03 ETH
        require(saleIsActive, "Sale must be active to mint nft");
        require(numberOfTokens > 0 && numberOfTokens <= maxBananaPurchase, "Can only mint 20 tokens at one time");
        require(totalSupply().add(numberOfTokens) <= MAX_BANANAS, "Purchase would exceed max supply of Bananas");
        require(msg.value >= _bananaPrice.mul(numberOfTokens), "Ether value sent is not correct");
        
        uint256 restAmount = msg.value - _bananaPrice.mul(numberOfTokens);
        payable(msg.sender).transfer(restAmount);
        payable(owner()).transfer(address(this).balance);
