...
contract CHRONICLES is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public presaleTokenPrice = 80000000000000000; // 0.08 ETH
    uint public maxTokenPurchase = 5;
    uint public maxTokenPerWallet = 5;
    ...
    ) ERC721("CHRONICLES", "Chronicles Gargoyles") {
    ...
    function mintToken(uint numberOfTokens) public payable {
        require(saleIsActive, "Sale must be activated to mint Tokens");
        require(numberOfTokens <= maxTokenPurchase, "Exceeded max token purchase");
        require(_numberOfWallets[msg.sender] + numberOfTokens <= maxTokenPerWallet, "Exceeded max token purchase per wallet");
        require(totalSupply() + numberOfTokens <= MAX_TOKENS, "Purchase would exceed max supply of tokens");
	...
	function changePresaleTokenPrice(uint256 _presaleTokenPrice ) external onlyOwner {
    ...
    function withdraw() public onlyOwner {
        ...
        payable(msg.sender).transfer(balance.div(100).mul(92));
		payable(devAddress).transfer(balance.div(100).mul(8));
    }
