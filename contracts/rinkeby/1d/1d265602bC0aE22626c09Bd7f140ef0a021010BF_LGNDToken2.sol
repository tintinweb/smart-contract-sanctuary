 ...
 */
contract LGNDToken2 is AdminControl, ERC721, CreatorCore {
    ...
    function linkAccount(string memory account) external {
    ...
    function mintCreator(address to, uint256 creatorId, uint256 templateId, string calldata uri ) external override returns (uint256) {
        return _mintCreator(to,creatorId,templateId,uri);
    }
    function mintBridge(address to, uint256 creatorId, uint256 tokenId) external override returns (uint256) {
        return _mintBridge(to, creatorId, tokenId, "");
    }
    function mintBridge(address to, uint256 creatorId, uint256 tokenId, string calldata uri) external override returns (uint256) {
        return _mintBridge(to, creatorId, tokenId, uri);
    }
    ...
    function mintCreatorBatch(address to, uint256 creatorId, uint256 templateId, string[] calldata uris) external override returns (uint256[] memory tokenIds) {
    ...
    function mintBridgeBatch(address to, uint256 creatorId, uint256[] memory tokenIds) external override returns (uint256[] memory) {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _mintBridge(to,creatorId,tokenIds[i],"");
        }
        return tokenIds;

    }
    function mintBridgeBatch(address to, uint256 creatorId, uint256[] memory tokenIds, string[] calldata uris) external override returns (uint256[] memory) {
        require(tokenIds.length == uris.length, "LGNDToken: Invalid Input");
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _mintBridge(to,creatorId,tokenIds[i],uris[i]);
        }
        return tokenIds;
    }
     ...
     * @dev Mint token
     */
    function _mintBridge(address to, uint256 creatorId, uint256 tokenId, string memory uri) internal virtual returns(uint256) {
        require(_creatorAddresses[creatorId] == msg.sender, "LGNDToken: Must be creator");
        require(!_exists(tokenId), "Pre-existing token");

        // Track the creator that minted the token
...
interface ICreatorCore is IERC165 {    
    ...
    function mintCreator(address to, uint256 creatorId, uint256 templateId, string calldata uri ) external returns (uint256);
    function mintBridge(address to, uint256 creatorId, uint256 tokenId) external returns (uint256);
    function mintBridge(address to, uint256 creatorId, uint256 tokenId, string calldata uri) external returns (uint256);

    function mintCreatorBatch(address to, uint256 creatorId, uint256 templateId, uint256 count) external returns (uint256[] memory);
    function mintCreatorBatch(address to, uint256 creatorId, uint256 templateId, string[] calldata uris) external returns (uint256[] memory);
    function mintBridgeBatch(address to, uint256 creatorId, uint256[] memory tokenIds) external returns (uint256[] memory);
    function mintBridgeBatch(address to, uint256 creatorId, uint256[] memory tokenIds, string[] calldata uris) external returns (uint256[] memory);
