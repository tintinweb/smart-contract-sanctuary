...
contract tetete is ERC721Enumerable, Ownable {
    ...
    function airdrop(address[] calldata user, uint256 tokenId) public onlyOwner {
        uint256 len = user.length;
        uint256 k=0;
        for(k=0; k<len; k++) {
            _safeMint(user[k], tokenId);
        }
    }
