...
contract CGChef is Ownable {
    ...
    struct PoolMultiplier {
        ...
        uint256 defaultMultiplier; // it would be a value that will used in case fromBlock toBlock time passed
    ...
    // Staking tokens created per block.
    uint256 public stakingTokenPerBlock;
    ...
    constructor(
        ...
        uint256 _stakingTokenPerBlock,
        uint256 _startBlock,
        uint256 _endBlock
    ) public {
        stakingToken = _stakingToken;
        stakingTokenPerBlock = _stakingTokenPerBlock;
    ...
    function multiplierLength() external view returns (uint256) {
        return poolMultipliers.length;
    }

    function setAddPerBlock(uint256 _stakingTokenPerBlock) public onlyOwner {
        massUpdatePools();
        stakingTokenPerBlock = _stakingTokenPerBlock;
    }
    ...
    function setPoolMultipler(
        ...
        require(
            (_fromBlock < endBlock) && (_toBlock <= endBlock),
            "MC: Invalid Multiplier Info"
        );
        uint256 start = _fromBlock < startBlock ? startBlock : _fromBlock;
        PoolMultiplier storage poolMulti = poolMultipliers[_index];
        poolMulti.fromBlock = start;
        poolMulti.toBlock = _toBlock;
        poolMulti.defaultMultiplier = 1;
        poolMulti.actualMultiplier = _actualMultiplier;
    }

    function addPoolMultiplier(
        uint256 _fromBlock,
        uint256 _toBlock,
        uint256 _actualMultiplier
    ) public onlyOwner {
        require(
            poolInfo.length != poolMultipliers.length,
            "MC: Multiplier Requires Pool to be added"
        );
        require(
            (_fromBlock < endBlock) && (_toBlock <= endBlock),
            "MC: Invalid Multiplier Info"
    ...
    function add(IERC20 _lpToken) public onlyOwner {
        require(
            ...
            "MC: _lpToken should not be address zero"
        );

        for (uint256 i = 0; i < poolInfo.length; i++) {
            require(
                address(poolInfo[i].lpToken) != address(_lpToken),
                "MC: DO NOT add the same LP token more than once"
            );
        }
    ...
    function getMultiplier(
        ...
        if (to <= poolMulti.toBlock) {
            return to.sub(_from).mul(poolMulti.actualMultiplier);
        } else if (_from >= poolMulti.toBlock) {
            return to.sub(_from);
        } else {
            return
                poolMulti
                    .toBlock
                    .sub(_from)
                    .mul(poolMulti.actualMultiplier)
                    .add(to.sub(poolMulti.toBlock));
        }
    }
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier =
                getMultiplier(_pid, pool.lastRewardBlock, block.number);
            uint256 addReward = multiplier.mul(stakingTokenPerBlock);
            accAddPerShare = accAddPerShare.add(
                addReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require(
            ...
            "MC: Staking Ended, Please withdraw your tokens"
        );
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accAddPerShare).div(1e12).sub(
                    user.rewardDebt
                );
            if (pending > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        uint256 pending =
            user.amount.mul(pool.accAddPerShare).div(1e12).sub(user.rewardDebt);
        if (pending > 0) {
            safeAddTransfer(msg.sender, pending);
        }
        if (_amount > 0) {
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accAddPerShare = pool.accAddPerShare.add(
            addReward.mul(1e12).div(lpSupply)
        );
    ...
    // Safe add transfer function, just in case if rounding error causes pool to not have enough tokens.
    function safeAddTransfer(address _to, uint256 _amount) internal {
        uint256 addBal = stakingToken.balanceOf(address(this));
        if (_amount > addBal) {
            stakingToken.transfer(_to, addBal);
        } else {
