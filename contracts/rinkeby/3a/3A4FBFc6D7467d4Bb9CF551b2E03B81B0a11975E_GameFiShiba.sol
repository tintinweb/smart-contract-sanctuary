    ...
    Pausable
{
    ...
    bytes24 public constant SIRE_LOCK = "SIRE_LOCK";
    bytes24 public constant BIRTH_LOCK = "BIRTH_LOCK";
    //Birth lock
    uint64 public constant SIRE_LOCK_TIME = 100;
    ...
    function kinshipFilter(uint father, uint mother) public view returns(bool){
        ...
        for (uint i = 0; i < _posterity[father].length; i++) {
            ...
            for ( uint j = 0; j < _posterity[_posterity[father][i]].length; j++ ) {
                require(
                    _posterity[_posterity[father][i]][j] != mother,
                    "Close relatives"
                );
            }
        }
        ...
        returns (uint)
    {
        ...
        uint64 locktime = SIRE_LOCK_TIME + uint64(block.timestamp);
