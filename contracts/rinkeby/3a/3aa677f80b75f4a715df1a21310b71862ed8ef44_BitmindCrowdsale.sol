...
contract BitmindCrowdsale {
    ...
    constructor(address _token, address _tokenUSDT) public {
        owner = msg.sender;
        Token = ERC20(_token);
    ...
    //Buy Token using USDT FUNCTION
    // converts USDT to Token and sends new Token to the contributors
    function buytokens(uint _amount) public {
        uint totalAmount;
        totalAmount = _amount;
        require(startDate>0 && now.sub(startDate) <= 7 days, 'BitmindMsg: Crowdsale is not started');
        require(Token.balanceOf(address(this))>0,"BitmindMsg: INSUFFICIENT BMD");
        require(totalAmount <= Token.balanceOf(address(this)),'BitmindMsg: Amount higher than supply');
        //Transfer Process
        TokenUSDT.transferFrom(msg.sender,address(this),totalAmount);
        collectedUSDT = collectedUSDT.add(totalAmount);
        totalSold =totalSold.add(totalAmount);
    ...
    function withdrawUSDT()public{
         require(msg.sender==owner && TokenUSDT.balanceOf(address(this))>0);
    ...
    function startCrowdsale()public{
        require(msg.sender==owner && startDate==0, "BitmindMsg: Owner Only");
        startDate=now;
    }
    
    //function to return the available BMD in the contract
    function availableBMD()public view returns(uint256){
