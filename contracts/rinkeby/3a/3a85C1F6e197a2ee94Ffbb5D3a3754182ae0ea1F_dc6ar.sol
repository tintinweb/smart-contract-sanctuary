...
contract dc6ar is ERC721 , Ownable, Pausable{
    using Strings for uint256;
    using SafeMath for uint256;

    IRNG rng;

    enum CardType { Unresolved, Resolved } 
    ...
    mapping(address => Bid)  public entries;
    mapping(uint => address) public entrants;
    ...
    // bid limits
    uint256                     immutable reserve;
    uint256                     immutable increment;
    
    uint256                      public nextTokenId;
    uint256                      public bidEntry = 0;
    uint256                      public numberProcessed;
    
    // Random Stuff
    mapping (uint256 => bytes32) public randomRequests;
    uint256                      public lastRandomRequested;
    uint256                      public lastRandomProcessed;
    uint256                      public randomOneOfSixteen;

    // pricing stuff
    address payable             wallet;

    mapping (uint256 => uint256)  randomAssignmentOrder;
    // Validation
    uint256                     startPos;
    bytes32                     tokenIdHash;
    ...
    uint256      public lastAuctionWinner = tMax;
    uint256      public lastLotteryWinner = tMax;
    ...
    event FirstBid(uint256 entry, address bidder, uint256 bid, uint256 TokenId);
    event IncreasedBid(uint256 entry, address bidder, uint256 increment, uint256 newTotal, uint256 TokenId);
    ...
    event Reserve(uint256 reserve);
    
    event WheresWallet(address wallet);
    event Increment(uint increment);
    ...
    event TakenOnTimeout(uint tokenId,address addr,uint  amount);
    event AuctionWinningAmountPaid(uint tokenId,address addr,uint  amount);
    event RaffleRefundPaid(uint tokenId,address  addr, uint balance);
    event CompleteRefund(uint tokenId,address addr,uint amount);
...
// need events
        ...
        emit Reserve(_reserve);
        emit Increment(_increment);
    }

    function bid() external payable {
        uint256 entry = bidEntry++;
        require(block.timestamp >= start && block.timestamp <= end,"Raffle and Auction not currently running");
        Bid storage oldBid = entries[msg.sender];
        oldBid.entry = uint32(entry);
        if (oldBid.amount > 0) {
            oldBid.amount += uint80(msg.value);
            require(msg.value >= increment,"increment too small");            
            emit IncreasedBid(entry,msg.sender,msg.value,oldBid.amount, oldBid.tokenId);
            return;
        }
        oldBid.amount = uint80(msg.value);
        require(msg.value >= reserve,"bid does not meet reserve price");
        uint next = nextTokenId++;
        oldBid.tokenId = next;
        entrants[next] = msg.sender;
        _mint(msg.sender,next);
        emit FirstBid(entry,msg.sender, msg.value,next);      
    ...
    function numberPending() public view returns (uint256) {
        return nextTokenId - numberProcessed;
    }

    function needProcessing() public view returns (bool) {
        uint count = 15;
        
        return ((numberPending() > count) || (numberPending() > 0 && block.timestamp > end)) && randomAvailable();
    }
    ...
    function processRand() internal {
        ...
        uint pending = numberPending();
        for (uint i = 0; (i < count) && (pending > 0); i++) {
            resolve(random & mask);
            random = random >> shift;
        }
    }
   ...
   function resolve(uint256 random) internal {       
        if (nextTokenId <=  numberProcessed) return;
        uint256 card_pos = numberProcessed++;
        uint256 r = random;
        uint256 chance = (r & tr_ass_order_mask)+1;       
        Bid storage b = entries[entrants[card_pos]];
        b.chance = chance;
        emit Resolution(card_pos,chance);
    }

    function setAuctionWinner(uint tokenId) external onlyOwner {
        lastAuctionWinner = tokenId;
    }

    function setRaffleWinner(uint tokenId) external onlyOwner {
        lastLotteryWinner = tokenId;
    }

    function confirmResults() external onlyAllowed {
        resultsConfirmed = block.timestamp;
        emit ResultsConfirmed();
    }


    function claimFunds(uint tokenId) public {
        uint confirmationDate = resultsConfirmed;
        require(confirmationDate != 0,"Results not yet confirmed");
        address addr = entrants[tokenId];
        require(!entries[addr].claimed,"Already claimed");
        entries[addr].claimed = true;
        if ((block.timestamp - confirmationDate > 180 days) && (msg.sender == wallet)) {
            uint256 amount = entries[addr].amount;
            wallet.transfer(amount);
            emit TakenOnTimeout(tokenId,addr, amount);
            return;
        }
        if (isAuctionWinner(tokenId)) {
            uint256 amount = entries[addr].amount;
            entries[addr].auction_winner = true;
            wallet.transfer(amount);
            emit AuctionWinningAmountPaid(tokenId,addr, amount);
            return;
        }
        if (isRaffleWinner(tokenId)) {
            uint256 amount = reserve; // 
            uint256 paid = entries[addr].amount;
            uint256 balance = paid.sub(reserve);
            entries[addr].lottery_winner = true;
            wallet.transfer(amount);
            emit AuctionWinningAmountPaid(tokenId, addr, amount);
            if (balance > 0) {
                payable(addr).transfer(balance);
                emit RaffleRefundPaid(tokenId, addr, balance);
            }
            return;
        }
        uint256 amount = entries[addr].amount;
        payable(addr).transfer(amount);
        emit CompleteRefund(tokenId,addr,amount);
    }

    function isAuctionWinner(uint256 tokenId) public view returns (bool) {
        uint256 law = lastAuctionWinner;
        require(_exists(law) , "Auction Winner Not Declared Yet");
        Bid memory winningBid = entries[entrants[law]];
        Bid memory thisBid = entries[entrants[tokenId]];
        if (thisBid.amount > winningBid.amount) return true;
        if (thisBid.amount < winningBid.amount) return false;
        return (thisBid.entry <= winningBid.entry); // == means it is lastAuctionWinner!!!
    }

    function isRaffleWinner(uint256 tokenId) public view returns (bool) {
        uint256 llw = lastLotteryWinner;
        require(_exists(llw), "Lottery Winner Not Declared Yet");
        if (isAuctionWinner(tokenId)) return false;
        Bid memory winningBid = entries[entrants[llw]];
        Bid memory thisBid = entries[entrants[tokenId]];
        if (thisBid.chance < winningBid.chance) return true;
        if (thisBid.chance > winningBid.chance) return false;
        return (thisBid.entry <= winningBid.entry); // == means it is lastAuctionWinner!!!
    }
    ...
    function cardType(uint256 tokenId) public view returns (CardType) {
        ...
        if (traitAssignmentOrder[tokenId] > 0) return CardType.Resolved;
        return CardType.Unresolved;
    }
    ...
    function how_long_more() public view returns (uint Days, uint Hours, uint Minutes, uint Seconds) {
        require(block.timestamp < start,"Missed It");
        uint gap = start - block.timestamp;
        Days = gap / (24 * 60 * 60);
        gap = gap %  (24 * 60 * 60);
        Hours = gap / (60 * 60);
        gap = gap % (60 * 60);
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override {
        ...
        require(released || from == address(0), "Tokens cannot be transferred until sale is over");
    }

    function release() external onlyAllowed {
        require(sale_is_over(),"transfers only allowed after sale is over");
        released = true;
    }
