...
contract C98NSiGrc12 is MSiG {
    ...
    function hasVoted(address owner) external view override returns (bool, uint16) {
        VoteProgress memory progress = _voteProgress;
        uint16 vPower = _votePowers[owner];
        if (progress.requestId == 0) {
            return (false, vPower);
        }
        return (progress.requestId == _votes[owner], vPower);
    }
...
contract C98NSiGrc12Factory {
    ...
    function createMultiSig(address[] memory owners_, uint16[] memory vPowers_, uint16 requiredVote_)
        external returns (C98NSiGrc12 wallet) {
        wallet = new C98NSiGrc12(owners_, vPowers_, requiredVote_);
    }
}