...
function exchange(
    ...
    uint256 amountOfQuoteAsset = amountOfBaseAsset  / exchange_rate;

    // 2. This contract must have enough assets to exchange
    uint256 reserve = IERC20(quoteAsset).balanceOf(address(this));
    require(reserve >= amountOfQuoteAsset, "Not enough quote asset in the reserve.");
    ...
    require(allowance >= amountOfBaseAsset, "Check the asset allowance");

    // Exchange!
    IERC20(baseAsset).transferFrom(msg.sender, address(this), amountOfBaseAsset);
    IERC20(quoteAsset).transfer(msg.sender, amountOfQuoteAsset);

    emit Exchanged(baseAsset, amountOfBaseAsset, quoteAsset, amountOfQuoteAsset);
    return true;
  }
