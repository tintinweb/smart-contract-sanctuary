 ...
 * {ERC721Enumerable}.
 */
contract EvaNFT3 is ProxyTarget, Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable, IERC2309, Pausable, ReentrancyGuard {
    ...
    function approve(address to, uint256 tokenId) public virtual override whenNotPaused {
        address owner = EvaNFT3.ownerOf(tokenId);
    ...
    function _batchMint(address to, uint count) internal {
        ...
        // As long as we only mint to the owner, or a bank in the royalties, we should never have to do this block of code:
        if (to != Ownable.owner()) {
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
        require(EvaNFT3.ownerOf(tokenId) == from, "EvaNFT: transfer of token that is not own");
    ...
    function _approve(address to, uint256 tokenId) internal virtual whenNotPaused {
        ...
        emit Approval(EvaNFT3.ownerOf(tokenId), to, tokenId);
    }
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view virtual override returns (uint256) {
        require(index < EvaNFT3.balanceOf(owner), "EvaNFT: owner index out of bounds");
    ...
    function tokenByIndex(uint256 index) public view virtual override returns (uint256) {
        require(index < EvaNFT3.totalSupply(), "EvaNFT: global index out of bounds");
        return index + 1;
    }
    ...
    function _addTokenToOwnerEnumeration(address to, uint256 tokenId) private {
        ...
        uint256 length = EvaNFT3.balanceOf(to);
    ...
    function _removeTokenFromOwnerEnumeration(address from, uint256 tokenId) private {
        ...
        uint256 lastTokenIndex = EvaNFT3.balanceOf(from) - 1;
