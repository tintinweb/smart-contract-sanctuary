...
contract LockDown is Ownable {
    ...
    event NewRelease(uint256 amount);
    ...
    function getLpBalance(uint256 balance) public view returns(uint256 amount) {
    ...
    function addLiquidity(uint256 s) external onlyOwner payable returns(uint256 paperAmount){
        ...
        paperAmount = getLpBalance(ethAmount);
        require(
            paperAmount > 0,
            "Address: insufficient balance"
        );
        ...
        IUniswapV2Router02(router).addLiquidityETH(
            ...
            ethAmount.mul(100 - s).div(100),
            developers,
            now + 1200
        );
        emit NewRelease(paperAmount);
    }
