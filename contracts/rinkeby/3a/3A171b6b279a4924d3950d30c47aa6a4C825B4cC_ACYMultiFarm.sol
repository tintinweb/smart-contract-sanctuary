...
// TODO: do we need emergencyWithdraw?
...
contract ACYMultiFarm is AccessControlEnumerable, Pausable, ReentrancyGuard {
    ...
    event NewPool(uint256 indexed poolId, address lpToken, uint256 lockDuration);
    ...
    function createPool(IERC20 _lpToken, uint256 _lockDuration, bytes32[] calldata _rewardTokens, uint256[] calldata _allocPoints, bool _withUpdate) public onlyRole(POOL_MANAGER_ROLE) {
        // check input format
        ...
        emit NewPool(poolId, address(_lpToken), _lockDuration);
    }
    ...
    function deposit(uint256 _poolId, uint256 _amount) external whenNotPaused() {
        ...
        for (uint ii = 0; ii < pool.rewardTokens.length; ii++) {
            ...
            user.rewardDebt[token] = (user.lpAmount * pool.accTokenPerShare[token]) / 1e12;
        }
    ...
    function withdraw(uint256 _poolId, uint256 _positionId, uint256 _amount) external nonReentrant {
        ...
        require(block.timestamp - user.stakeTimestamp >= pool.lockDuration, "ACY: No early unlock");
        require(_amount <= user.lpAmount, "ACY: Insufficient LP balance");

        // store pending rewards
        for (uint ii = 0; ii < pool.rewardTokens.length; ii++) {
            bytes32 token = pool.rewardTokens[ii];
            uint256 pending = (user.lpAmount * pool.accTokenPerShare[token]) / 1e12 - user.rewardDebt[token];
            user.pendingRewards[token] += pending;
        }
    ...
    function _harvest(uint256 _poolId, uint256 _positionId, bool _closeIfEmpty, bool _revertOnFailure) private nonReentrant {
        ...
        for (uint ii = 0; ii < pool.rewardTokens.length; ii++) {
            bytes32 token = pool.rewardTokens[ii];
            uint256 pending = user.pendingRewards[token] + (user.lpAmount * pool.accTokenPerShare[token]) / 1e12 - user.rewardDebt[token];

            // we have enough funding to pay out rewards
            if (availableBalance[token] >= pending) {
                user.pendingRewards[token] = 0;
                user.rewardDebt[token] = (user.lpAmount * pool.accTokenPerShare[token]) / 1e12;

                tokenContract[token].safeTransfer(msg.sender, pending);
                availableBalance[token] -= pending;

                emit Harvest(msg.sender, _poolId, _positionId, token, pending);
                continue;
            }
        ...
        if (user.lpAmount == 0 && _closeIfEmpty) {
    ...
    function updatePool(uint256 _poolId) public {
        ...
        uint256 numBlocks = block.number - pool.lastUpdateBlock;

        for (uint ii = 0; ii < pool.rewardTokens.length; ii++) {
            bytes32 token = pool.rewardTokens[ii];
            uint256 tokenReward = numBlocks * rewardPerBlock[token] * pool.allocPoint[token] / totalAllocPoint[token];
            pool.accTokenPerShare[token] += tokenReward * 1e12 / lpSupply;
        }
    ...
    function pending(uint256 _poolId, uint256 _id, bytes32 _token) public view returns (uint256) {
        ...
        return user.pendingRewards[_token] + user.lpAmount * accTokenPerShare / 1e12 - user.rewardDebt[_token];
    }
    ...
    function getPoolTokenRewardInfo(uint256 _poolId, bytes32 _token) public view returns (uint256 ap, uint256 totalAp, uint256 tokenPerBlock, uint256 tokenInPoolPerBlock) {
        ...
        tokenInPoolPerBlock = tokenPerBlock * ap / totalAp;
    }
