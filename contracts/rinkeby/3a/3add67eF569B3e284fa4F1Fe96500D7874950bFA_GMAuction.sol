...
interface IGMAuction {
    ...
    function setCommission(uint8 percent) external;
...
/**
  ...
  */
contract GMAuction is IGMAuction, Ownable, IERC721Receiver {
    ...
    */
    ...
    function completeAuction(uint256 artId) external whenEnded(artId) override {
        ...
        uint256 paidToBeneficiary = highestBid - commission;
        stablecoin.transfer(treasury, commission);
        stablecoin.transfer(auctions[artId].beneficiary, paidToBeneficiary);
    ...
    function setTreasury(address treasury_) public onlyOwner override {
        treasury = treasury_;
    }

    function setCommission(uint8 percent) external onlyOwner override {
        require(percent < 100, "GMAuction: don't be that greedy");
