...
contract BoringBananaCo is ERC721, Ownable {
    ...
    uint256 public constant bananaPrice = 25000000000000000; //0.035 ETH
    ...
    // 50 For team + giveaways    uint256 public bananaReserve = 150;
    uint256 public bananaReserve = 150;
    ...
    function initiateSale() public onlyOwner {
        ...
        REVEAL_TIMESTAMP = saleStart + (86400 * 7); // ONE WEEK
        saleIsActive = true;
    }
    ...
    function changeBananaName(uint _index, string memory _name) public {
        require(ownerOf(_index) == msg.sender, "Hey, your wallet doesn't own this banana!");
        require(sha256(bytes(_name)) != sha256(bytes(bananaNames[_index])), "New name is same as the current one");
        bananaNames[_index] = _name;
        
    }
    
    function viewBananaName(uint _index) public view returns( string memory ){
        require( _index < totalSupply(), "Choose a banana within range" );
        return bananaNames[_index];
    }
