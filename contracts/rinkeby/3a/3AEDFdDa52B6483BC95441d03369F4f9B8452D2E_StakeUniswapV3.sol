    ...
    DSMath
{
    ...
    function miningCoinage() public lock {
        ...
        if (block.timestamp > (coinageLastMintBlockTimetamp + miningIntervalSeconds) ) {
            uint256 miningAmount = ( block.timestamp - coinageLastMintBlockTimetamp ) * IIStake2Vault(vault).miningPerSecond();

            if (miningAmount > 0) {
                uint256 prevTotalSupply = AutoRefactorCoinageI(coinage).totalSupply();
                miningAmount = miningAmount*(10**9);
                uint256 afterTotalSupply = prevTotalSupply + miningAmount;
        ...
        )
    {
        if(startTime > 0) {
            ...
            (balanceCoinageOfUser, minableAmount,  totalDepositAmount) = rewardAllocatedAmountCoinage(
                _depositTokens.owner,
                _depositTokens.liquidity
            );

            if(secondsAbsolute > 0 && secondsInsideDiff < secondsAbsolute ){
                miningAmount = minableAmount * (secondsInsideDiff / secondsAbsolute);
                nonMiningAmount = minableAmount - miningAmount;

            } else if(secondsAbsolute > 0){
    ...
    function rewardAllocatedAmountCoinage(address user, uint256 amount)
        internal
        view
        returns (uint256 balanceCoinageOfUser, uint256 minableAmount, uint256 totalDepositAmount)
    {
        ...
        totalDepositAmount = _userStaked.totalDepositAmount;

        uint256 balanceOfUserRay = AutoRefactorCoinageI(coinage).balanceOf(user);
        if (balanceOfUserRay > 0) {
            balanceCoinageOfUser = balanceOfUserRay / (10**9);
            if (balanceCoinageOfUser > totalDepositAmount) {
                minableAmount = (balanceCoinageOfUser - totalDepositAmount) * (amount / totalDepositAmount);
            }
        }
    }
