...
contract Paragons is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function _hash(string calldata salt, address _address, uint amount) internal view returns (bytes32){
        return keccak256(abi.encode(salt, address(this), _address, amount));
    }
    ...
    function presaleMint(string calldata salt, bytes calldata token, uint256 amount) external payable nonReentrant {
        ...
        require(_verify(_hash(salt, msg.sender, amount), token), 'Invalid token.');
