...
/**
 * @notice Access Controls contracts
 */
...
contract PointList is AccessControls {
    ...
    function hasPoints(address account, uint256 amount) public view  returns (bool) {
...
contract HD is  ERC721 , PointList {
    ...
    constructor() ERC721("List NFT", "LIST") public {
    }

    /**
     * @dev Mint a token
     */
    function mintToken(uint256 numTokens) public {
        require(numTokens > 0 && numTokens <= 20, "You can only mint between 1 and 20 tokens at a time");
        require(block.timestamp >= SALE_START_TIMESTAMP, "Claim has not started");

        uint256 newBalance = claimed[msg.sender].add(numTokens);
        require(hasPoints(msg.sender, newBalance));
        for (uint i = 0; i < numTokens; i++) {
            uint tokenId = totalSupply() ;
            require(tokenId < MAX_SUPPLY, "Cannot mint more tokens than the maximum limit");
            _safeMint(msg.sender, tokenId);
        }
        claimed[msg.sender] = newBalance;
    }
    ...
    function setBaseURI(string memory baseURI_) public  {
        require(
            ...
            "setBaseURI: Sender must be admin"
        );
    ...
    /**
     * @dev See {IERC721Metadata-tokenURI}.
     * overides concatenated baseURI + tokenID in standard erc721 contract so that all tokens have the same token URI
     */
