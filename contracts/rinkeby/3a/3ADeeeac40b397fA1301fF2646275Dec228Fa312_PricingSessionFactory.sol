...
contract PublicPricingSession is ReentrancyGuard, PostSessionLibrary {
    ...
    constructor(address _multisig, address voteMask_, address _hasher, address _treasury, address _parentContract, address _nftAddress, uint _tokenid, uint _initialAppraisal, uint _bounty) {
        TREASURY = _treasury;
    ...
    */
    function setVote(
        ...
        bytes32 concealed = keccak256(abi.encodePacked(_appraisal, msg.sender));
        Voter memory newVote = Voter(0, concealed, 0, msg.value, true);
    ...
    function updateMyVote(
        ...
        bytes32 concealed = keccak256(abi.encodePacked(_appraisal, msg.sender));
        nftVotes[msg.sender].cAppraisal = concealed;
    ...
    */
    function weightVote(uint _appraisal) nonReentrant checkParticipation external {
        require(endTime < block.timestamp
                && !oneWeight[msg.sender] 
                && nftVotes[msg.sender].cAppraisal == keccak256(abi.encodePacked(_appraisal, msg.sender)),
                "time not over"
        );
        ...
        emit newAppraisalAdded(msg.sender, nftVotes[msg.sender].stake, _appraisal, weight);
    ...
    function setFinalAppraisal() nonReentrant checkParticipation external {
        require(
            !finalAppraisalSet
            && (block.timestamp > endTime + 1 hours || votesWeighted)
        );
    ...
    */
    function harvestLoss() nonReentrant checkParticipation external {
        ...
        payout(TREASURY, totalCommission/2);
        payout(MULTISIG, totalCommission/2);
    ...
    function claimProfit() checkHarvestLoss nonReentrant checkParticipation external {
        ...
        if(amountOfClaims == uniqueVoters) {
            claimsComplete = true;
            endSession();
        }
    }
    ...
    */
    function exchangeForPP() checkHarvestLoss nonReentrant checkParticipation external {
        ...
        oneClaim[msg.sender] = true;
        amountOfClaims++;
        uint ppPayout = (totalSessionStake * winnerPoints[msg.sender] / totalWinnerPoints) * 1e18 / (0.0001 ether + 0.0001 ether * IPPTreasury(TREASURY).tokensClaimed() / 1000000);
    ...
    function endSession() checkParticipation public {
        ...
        uint cPayout = 25*address(this).balance/100;
    ...
    function getPpPayout() view external returns(uint) {
        return (totalSessionStake * winnerPoints[msg.sender] / totalWinnerPoints) * 1e18 / (0.0001 ether + 0.0001 ether * IPPTreasury(TREASURY).tokensClaimed() / 1000000);
    }
    ...
    modifier oneVoteEach {
        require(!nftVotes[msg.sender].exists);
        _;
    }
    ...
    modifier checkHarvestLoss() {
        require(harvestLossComplete || block.timestamp > timeFinalAppraisalSet + 1 hours);
        _;
    }
}
...
contract PricingSessionFactory is ReentrancyGuard {
    ...
    event publicSessionCreated(address creator_, address nftAddress_, uint tokenid_, uint initialAppraisal_, uint bounty_);

    /* ======== STATE VARIABLES ======== */

    address immutable public Treasury;
    address immutable public HASHER;
    address immutable public VOTEMASK;
    ...
    mapping(address => mapping(uint => uint)) public checkStartTime;
    ...
    function createNewPublicSession(
        address nftAddress, 
        uint tokenid, 
        uint initialAppraisal
    ) stopOverwrite(nftAddress, tokenid) public payable {
        PublicPricingSession session = new PublicPricingSession(MULTISIG, VOTEMASK, HASHER, Treasury, address(this), nftAddress, tokenid, initialAppraisal, msg.value);
        ...
        emit publicSessionCreated(msg.sender, nftAddress, tokenid, initialAppraisal, msg.value);
    }
