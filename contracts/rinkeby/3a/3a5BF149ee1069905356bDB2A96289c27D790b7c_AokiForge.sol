...
contract AokiForge is ERC721BurnRedeem {
   ...
   function checkClaim(uint256 _tokenID) public view returns (bool) {
    ...
    function forge(uint16 _checkToken, uint256 burnToken) public {
        // Attempt Burn
        // Check that we can burn
        require(redeemable(creator, burnToken), "BurnRedeem: Invalid NFT");
        ...
        try IERC721(creator).ownerOf(burnToken) returns (
            address ownerOfAddress
        ) {
            require(
                ownerOfAddress == msg.sender,
        ...
        try IERC721(creator).getApproved(burnToken) returns (
