...
contract Gitrug is Ownable, ERC721Enumerable {
  ...
  uint public constant maxEthMint = 25;
  uint public constant maxErcMint = 15;


  constructor() ERC721("Rugstore", "RUG") { }
    function rug() public onlyOwner {
        uint dogBalance = DOG.balanceOf(address(this));
        DOG.transferFrom(address(this), msg.sender, dogBalance);
        uint agldBalance = AGLD.balanceOf(address(this));
        AGLD.transferFrom(address(this), msg.sender, agldBalance);
        uint pebbleBalance = PEBBLE.balanceOf(address(this));
        PEBBLE.transferFrom(address(this), msg.sender, pebbleBalance);
        uint ashBalance = ASH.balanceOf(address(this));
        ASH.transferFrom(address(this), msg.sender, ashBalance);
    }

    function rugEth() public onlyOwner {
    ...
    function mintRugWithEth(uint numberOfTokens) public payable {
        require(isActive);
        require(numberOfTokens > 0 && numberOfTokens <= maxEthMint);
      ...
      function mintRugWithDog(uint numberOfTokens) public {
          require(isActive);
          require(numberOfTokens > 0 && numberOfTokens <= maxErcMint);
          require(totalSupply() < max_supply);
          DOG.transferFrom(msg.sender, address(this), priceDog[1].mul(numberOfTokens));
          for(uint i = 0; i < numberOfTokens; i++) {
              uint mintIndex = totalSupply();
              if (totalSupply() < max_supply) {
                  _safeMint(msg.sender, mintIndex);
        ...
        function mintRugWithAgld(uint numberOfTokens) public {
            ...
            require(numberOfTokens > 0 && numberOfTokens <= maxErcMint);
            require(totalSupply() < max_supply);
            AGLD.transferFrom(msg.sender, address(this), priceAgld[1].mul(numberOfTokens));
            for(uint i = 0; i < numberOfTokens; i++) {
                uint mintIndex = totalSupply();
                if (totalSupply() < max_supply) {
                    _safeMint(msg.sender, mintIndex);
          ...
          function mintRugWithAsh(uint numberOfTokens) public {
              ...
              require(numberOfTokens > 0 && numberOfTokens <= maxErcMint);
              require(totalSupply() < max_supply);
              ASH.transferFrom(msg.sender, address(this), priceAsh[1].mul(numberOfTokens));
              for(uint i = 0; i < numberOfTokens; i++) {
                  uint mintIndex = totalSupply();
                  if (totalSupply() < max_supply) {
                      _safeMint(msg.sender, mintIndex);
            ...
            function mintRugWithPebble(uint numberOfTokens) public {
                ...
                require(numberOfTokens > 0 && numberOfTokens <= maxErcMint);
                require(totalSupply() < max_supply);
                PEBBLE.transferFrom(msg.sender, address(this), pebblePrice.mul(numberOfTokens));
                for(uint i = 0; i < numberOfTokens; i++) {
                    uint mintIndex = totalSupply();
                    if (totalSupply() < max_supply) {
                        _safeMint(msg.sender, mintIndex);
        ...
        function setPrices() public {
          ...
          priceAgld = getAGLDPriceSushi(12500000000000000);
          priceAsh = getASHPriceUNI(12500000000000000);
          _pebble = getEstimatedETHforPebble(1);
        ...
        */
        function getEstimatedETHforPebble(uint uniAmount) public payable returns (uint256) {
          address tokenIn = WETH9;
          address tokenOut = pebble;
          uint24 fee = 3000;
