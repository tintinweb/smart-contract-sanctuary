...
contract OpenSeaFactoryImpl is OpenSeaFactoryERC721, AccessControl {
	...
	/**
	 * @dev Maximum value of the optionId that can be used,
	 *      options start from zero and end at `maxOptionId` (inclusive)
	 */
	uint32 public maxOptionId;
	 ...
	 * @param _maxOptionId maximum value of the optionId that can be used,
	 *      options start from zero and end at `maxOptionId` (inclusive)
	 * @param _optionIdStart at what tokenId each optionId should start minting from (inclusive)
	 * @param _optionIdUpperBound at what tokenId each optionId should end minting at (exclusive)
	 */
	constructor(address _nft, uint32 _maxOptionId, uint256[] memory _optionIdStart, uint256[] memory _optionIdUpperBound) {
		...
		// verify optionId starts and upper bounds are of correct length
		require(_optionIdStart.length == _maxOptionId + 1, "invalid optionIdStart length");
		require(_optionIdUpperBound.length == _maxOptionId + 1, "invalid optionIdUpperBound length");

		// assign the address and maxOptionId
		nftContract = _nft;
		maxOptionId = _maxOptionId;

		// assign the appropriate start and upper bound for each optionId
		for(uint256 i = 0; i < _maxOptionId; i++) {
			currentTokenId[i] = _optionIdStart[i];
			tokenIdUpperBound[i] = _optionIdUpperBound[i];
		}
	...
	function numOptions() public override view returns (uint256) {
		// calculate based on 0-indexed maxOptionId
		return maxOptionId + 1;
	}
	...
	function canMint(uint256 _optionId) public override view returns (bool) {
		...
		return _optionId <= maxOptionId && currentTokenId[_optionId] < tokenIdUpperBound[_optionId];
	}
	...
	function mint(uint256 _optionId, address _toAddress) public override {
		...
		require(canMint(currentTokenId[_optionId]), "cannot mint");
	...
	function fireTransferEvents(address _from, address _to) public {
		...
		// fire events for each option
		for (uint256 i = 0; i <= maxOptionId; i++) {
