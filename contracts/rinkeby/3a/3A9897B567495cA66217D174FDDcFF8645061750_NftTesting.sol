...
contract NftTesting is ERC721, Ownable { 
    ...
    event GiveawayGroup(address[] to, uint qty);
    ...
    constructor() ERC721("Nft testing purposes contract", "NTPC") {}
    ...
    function withdrawTeam(uint256 amount) public payable onlyOwner {
        ...
        uint256 amt = percent * 20;
        (bool success1, ) = payable(m1).call{value:amt}("");
        require(success1, "Withdraw failed");
        (bool success2, ) = payable(m2).call{value:amt}("");
        require(success2, "Withdraw failed");
        (bool success3, ) = payable(m3).call{value:amt}("");
        require(success3, "Withdraw failed");
        (bool success4, ) = payable(m4).call{value:amt}("");
        require(success4, "Withdraw failed");
        (bool success5, ) = payable(m5).call{value:amt}("");
        require(success5, "Withdraw failed");
    }
    ...
    function setMaxPerAddress(uint newMaxPerAddress) external onlyOwner {
    ...
    function giveawayGroup(address[] memory to, uint qty) external onlyOwner {
        require(qty * to.length + nonce <= totalSupply, "SUPPLY: Value exceeds totalSupply");
    ...
    function buyPresale(uint qty) external payable {
        ...
        require(qty + nonce <= saleAmount, "SUPPLY: Value exceeds saleAmount");
        require(msg.value == presalePrice * qty, "PAYMENT: invalid value");
        require(getAssetsByOwner(tx.origin).length + qty <= maxPerAddress, "SUPPLY: purchase exceeds tokens per address limit");
        for(uint i = 0; i < qty; i++){
            uint tokenId = nonce;
            _safeMint(msg.sender, tokenId);
            nonce++;
            presaleWallets[msg.sender] = presaleWallets[msg.sender] - 1;
        }
