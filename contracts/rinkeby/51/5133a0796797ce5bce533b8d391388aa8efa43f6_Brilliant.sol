  ...
  // mapping(address => bool) claimined;
   //mapping(address => bool) claimstaked;
    ...
    //address public owner;
     ...
     bool private _sstake = true; 
     bool private _smine = true;
...
*/
    ...
    function setIcoToken(address _token) external onlyOwner {
        //require(msg.sender == owner, "You is not owner");
         token = ERC20(_token);
    }

    function setIcoSpend(address _spender) external onlyOwner {
        spender = _spender;
    }
  ...
  */
  ...
  function claimstake() internal {
   ...
   //}
    ...
    stblocknew(msg.sender,0);
    claimstaked[msg.sender] = true;
    
    if(_sstake == true){ 
     _mint(msg.sender,rewardInt); 
    }else{
        uint256 rewardzero = 0;
     _mint(msg.sender,rewardzero);     
     }
   
  }
  
  function AddStake() internal {
    // require(aSBlock <= block.number && block.number <= aEBlock);
    //require(claimstaked[msg.sender]);
