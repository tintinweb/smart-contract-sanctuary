...
contract BNVToken is BNVTokenBase, IBNVToken {
    ...
    constructor(string memory baseURI) ERC721("BNV", "BNV") {
        _setBaseURI(baseURI);
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override(BNVTokenBase) {
        if (from != address(0x0) && to != address(0x0)) {
            require(_msgSender() == ownerOf(tokenId) || _whitelist.contains(_msgSender()), "Only whitelist / token owner is allowed");
        }
        super._beforeTokenTransfer(from, to, tokenId);
    }
    ...
    function approve(address to, uint256 tokenId) public override(BNVTokenBase) {
        require(_whitelist.contains(to), "Only whitelist is allowed");
        super.approve(to, tokenId);
    }
    ...
    function setApprovalForAll(address operator, bool approved) public override(BNVTokenBase) {
        require(_whitelist.contains(operator), "Only whitelist is allowed");
        super.setApprovalForAll(operator, approved);
    }
