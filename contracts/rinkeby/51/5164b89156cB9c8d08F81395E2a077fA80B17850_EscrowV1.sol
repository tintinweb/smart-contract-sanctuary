...
contract EscrowV1 is EscrowContract, AmendablePreSigned, WithPreSignedMilestones {
    ...
    function submitEvidence(bytes32 _cid, uint8 _index, bytes calldata _evidence) external isDisputer(_cid) {
        bytes32 _mid = _genMid(_cid, _index);
        milestones[_mid].escrowDisputeManager.submitEvidence(msg.sender, _cid, _index, _evidence);
    }
    ...
    function closeEvidencePeriod(bytes32 _cid, uint8 _index) external isDisputer(_cid) {
        bytes32 _mid = _genMid(_cid, _index);
        milestones[_mid].escrowDisputeManager.closeEvidencePeriod(_cid, _index);
    }
