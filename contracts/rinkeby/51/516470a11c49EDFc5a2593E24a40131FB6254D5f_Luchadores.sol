...
contract Luchadores is ERC721Pausable, Ownable, ReentrancyGuard, VRFConsumerBase {
  ...
  function generateLuchador(uint256 _quantity, uint256 _deadline) external payable nonReentrant whenNotPaused {
    require(_tokenIds.current() < 1000, "Maximum reached");
    require(_tokenIds.current() + _quantity <= 10000, "exceeds max supply");
    require(_quantity > 0 && _quantity <= 20, "must be in range 1-20");
    ...
    // path[0] = WETH9;
    // path[1] = LinkToken;
    // swapRouter.swapETHForExactTokens{value: msg.value}(amountOut, path, address(this), _deadline);

    // TODO get ~price with oracle (?)
    fundLink(2 * 10 ** 18, 0.02 * 10 ** 18);
...
// -> CONTINUE HERE
  ...
  /// @notice getLink swaps a minimum possible amount of WETH9 for a fixed amount of LINK.
  ...
  /// @param amountInMaximum The amount of WETH9 we are willing to spend to receive the specified amount of LINK.
  /// @return amountIn The amount of WETH9 actually spent in the swap.
  function fundLink(uint256 amountOut, uint256 amountInMaximum) public returns (uint256 amountIn) {
    ...
    // If the actual amount spent (amountIn) is less than the specified maximum amount, we must refund the msd.sender and approve the swapRouter to spend 0
