...
contract CcatFaucet is mortal {
    ...
    event AcceptErc(address indexed from, uint amount);
    event AcceptEth(address indexed from, uint amount);
    ...
    fallback() external payable {
        emit AcceptErc(msg.sender, msg.value);
    }

    // Accept any incoming amount
    receive() external payable {
        emit AcceptEth(msg.sender, msg.value);
    }
}
