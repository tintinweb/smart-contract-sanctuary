...
// Inspired/Copied from BGANPUNKS V2 (bastardganpunks.club) & Chubbies
contract EthersparksTest is ERC721, Ownable {
    using SafeMath for uint256;
    uint256 public constant MAX_ETHERSPARKS = 100;
    ...
    constructor() ERC721("Ethersparks", "ETHERSPARKS") {
        setBaseURI("https://gateway.pinata.cloud/ipfs/QmTtwYjAHyCS9QbWEkUBBSwe3kbFRDwHdD2mwNcKnzxrca/");
    }
    ...
    function calculatePrice() public view returns (uint256) {
        ...
        if (currentSupply == 100) {
            return 1000000000000000000; // 100: 1.00 ETH
        } else if (currentSupply >= 80) {
            return 640000000000000000; // 80-99:  0.64 ETH
        } else if (currentSupply >= 60) {
            return 320000000000000000; // 60-79:  0.32 ETH
        } else if (currentSupply >= 40) {
            return 160000000000000000; // 40-59:  0.16 ETH
        } else if (currentSupply >= 20) {
            return 80000000000000000; // 20-39:  0.08 ETH
        } else if (currentSupply >= 0) {
            return 40000000000000000; // 0-19:   0.04 ETH
        }
    }
    ...
    function adoptEthersparks(uint256 numEthersparks) public payable {
        require(totalSupply() < MAX_ETHERSPARKS, "Sale has already ended");
        require(
            numEthersparks > 0 && numEthersparks <= 10,
            "You can adopt minimum 1, maximum 10 Ethersparks"
        );
        require(
    ...
    function reserveGiveaway(uint256 numEthersparks) public onlyOwner {
        uint256 currentSupply = totalSupply();
        require(
            totalSupply().add(numEthersparks) <= 30,
