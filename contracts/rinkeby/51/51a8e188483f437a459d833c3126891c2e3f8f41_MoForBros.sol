...
contract MoForBros is ERC721, ERC721Enumerable, Ownable {

    using Counters for Counters.Counter;
    using Strings for uint256;

    Counters.Counter private _tokenIdCounter;
    uint256 private constant tokenPrice = 30000000000000000; // 0.03 ETH
    uint private constant maxTokenPurchase = 20;
    uint256 public maxSupply = 3030;
    ...
    address  devAddress = 0x346964b6E53E985D60c5666BCB1b5dD66CE25901;
    address  donorAddress = 0xd217A955600ad7117BaDC1A244c57b78f36D3797;
    ...
    event welcomeToMoForBros(uint256 indexed id);
    

    receive() external payable {}
    
    constructor(string memory baseURI, string memory _notrevealURI) ERC721("MoForBros", "MFB") { 
    ...
    function internalMint(address to) internal {
        ...
        emit welcomeToMoForBros(_tokenIdCounter.current());
    }

    function withdraw() external onlyOwner {
        uint256 balance = address(this).balance;
        require(balance > 0);
        payable(devAddress).transfer((balance * 15) / 100);
        payable(donorAddress).transfer((balance * 85) / 100);
    }
