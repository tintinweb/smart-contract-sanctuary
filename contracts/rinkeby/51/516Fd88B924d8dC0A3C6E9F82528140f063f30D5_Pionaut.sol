...
contract Pionaut is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {    
    ...
    uint public royaltyFeePercent1 = 50;
    uint public royaltyFeePercent2 = 50;   

    address public royaltyWallet1 = 0x0f22F0f1C70b0277dEE7F0FF1ac480CB594Ca450;
    address public royaltyWallet2 = 0x6a9307652d93CaB45634cAD19cf9E60b0D7504E7;
    ...
    function withdraw() public onlyOwner {
        ...
        payable(royaltyWallet1).transfer((balance * royaltyFeePercent1) / totalFeeUnitPercent);
        payable(royaltyWallet2).transfer((balance * royaltyFeePercent2) / totalFeeUnitPercent);
    }

    function setFee(uint256 _royaltyFeePercent1, uint256 _royaltyFeePercent2, uint256 _totalFeeUnitPercent) public onlyOwner{
        royaltyFeePercent1 = _royaltyFeePercent1;
        royaltyFeePercent2 = _royaltyFeePercent2;
        totalFeeUnitPercent = _totalFeeUnitPercent;
    }

    function setWallets(address _royaltyWallet1, address _royaltyWallet2) public onlyOwner{
        royaltyWallet1 = _royaltyWallet1;
        royaltyWallet2 = _royaltyWallet2;
    }
    ...
    function setPublicSaleMaxSupply(uint256 newAmount) public onlyOwner {
    ...
    function randomPickId() internal view returns (uint256) {
        require(totalSupply() < publicMaxSupply, "Max Supply Limited!");
        uint256 randomId = 0;
        uint loopCount = 0;
        do {
            uint256 temp = (uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, loopCount))) % publicMaxSupply) + 1;
            loopCount ++;
    ...
    function mint(uint256 amount) public payable {
        ...
        require(totalSupply() + amount <= publicMaxSupply, 'max public sale amount is limited.');        
        for (uint256 i = 0; i < amount; i++) {
            uint256 tokenId = randomPickId();
            internalMint(msg.sender, tokenId);
        }
