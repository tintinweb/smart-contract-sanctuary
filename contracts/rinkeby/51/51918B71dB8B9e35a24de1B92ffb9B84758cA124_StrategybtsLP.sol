...
// File: contracts/SUPRA/interfaces/bdollar/IShareRewardPool.sol
...
interface IShareRewardPool {
    ...
    function getEstimatedPendingDivs(address _holder) external view returns (uint256);
    function emergencyWithdraw(uint256 amountToWithdraw) external;
    function claim()external;
}

// File: contracts/SUPRA/strategies/BDollar/StrategybtsLP.sol
 ...
 * bts rewards from providing liquidity are farmed every few minutes, sold and split 50/50. 
 ...
 */
contract StrategybtsLP is Ownable, Pausable {
     ...
     * {bts} - Token generated by staking our funds. In this case it's the btss token.
     * {SUPRA} - supraFinance token, used to send funds to the treasury.
     ...
     */
    address constant public busd = address(0xc625f1f9b1d9C422621937e277AFf0C988FfFFc8);
    address constant public wbnb = address(0xc625f1f9b1d9C422621937e277AFf0C988FfFFc8);
    address constant public bts = address(0xeBdC557EBF858Ae8169ecA64726ac2D041aE4989);    
    ...
    address constant public shareRewardPool = address(0x27b4c186237BaD3856D42D290300214e14193a24);

    /**
     * @dev supra Contracts:
     * {rewards} - Reward pool where the strategy fee earnings will go.
     * {treasury} - Address of the supraFinance treasury
     ...
     *
     * {REWARDS_FEE} - 3% goes to SUPRA holders through the {rewards} pool.
     ...
     * {btsToWbnbRoute} - Route we take to get from {bts} into {wbnb}.
     * {wbnbToSUPRARoute} - Route we take to get from {wbnb} into {SUPRA}.
     * {btsToLp0Route} - Route we take to get from {bts} into {lpToken0}.
     * {btsToLp1Route} - Route we take to get from {bts} into {lpToken1}.
     */
    address[] public btsToWbnbRoute = [bts, busd, wbnb];    
    address[] public btsToLp0Route;
    address[] public btsToLp1Route;
    ...
    constructor(address _lpPair, address _vault, address _strategist) public {
        ...
        if (lpToken0 == busd) {
            btsToLp0Route = [bts, busd];
        } else if (lpToken0 != bts) {
            btsToLp0Route = [bts, busd, lpToken0];
        }

        if (lpToken1 == busd) {
            btsToLp1Route = [bts, busd];
        } else if (lpToken1 != bts) {
            btsToLp1Route = [bts, busd, lpToken1];
        }

        IERC20(lpPair).safeApprove(shareRewardPool, uint(-1));
        IERC20(bts).safeApprove(unirouter, uint(-1));
     ...
     * It deposits {lpPair} in the shareRewardPool to farm {bts}
     */
    function deposit() public whenNotPaused {
        uint256 pairBal = IERC20(lpPair).balanceOf(address(this));

        if (pairBal > 0) {
            IShareRewardPool(shareRewardPool).deposit(pairBal);
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (pairBal < _amount) {   
            IShareRewardPool(shareRewardPool).withdraw(_amount.sub(pairBal));
     ...
     * 3. It swaps the {bts} token for {lpToken0} & {lpToken1}
     * 4. Adds more liquidity to the pool.
     * 5. It deposits the new LP tokens.
     */
    function harvest() external whenNotPaused {
        require(!Address.isContract(msg.sender), "!contract");
        IShareRewardPool(shareRewardPool).deposit(0);
        chargeFees();
        addLiquidity();
        deposit();
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(bts).balanceOf(address(this)).mul(45).div(1000);
        IPancakeRouter(unirouter).swapExactTokensForTokens(toWbnb, 0, btsToWbnbRoute, address(this), now.add(600));
    ...
    /**
     * @dev Swaps {bts} for {lpToken0}, {lpToken1} & {wbnb} using PancakeSwap.
     */
    function addLiquidity() internal {   
        uint256 btsHalf = IERC20(bts).balanceOf(address(this)).div(2);
        
        if (lpToken0 != bts) {
            IPancakeRouter(unirouter).swapExactTokensForTokens(btsHalf, 0, btsToLp0Route, address(this), now.add(600));
        }

        if (lpToken1 != bts) {
            IPancakeRouter(unirouter).swapExactTokensForTokens(btsHalf, 0, btsToLp1Route, address(this), now.add(600));
        }
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount) = IShareRewardPool(shareRewardPool).getEstimatedPendingDivs(address(this));
        return _amount;
    }
    ...
    function retireStrat(uint256 _amount) external {
        ...
        IShareRewardPool(shareRewardPool).emergencyWithdraw(_amount);
    ...
    function pause() public onlyOwner {
        ...
        IERC20(bts).safeApprove(unirouter, 0);
