...
contract LuckyBuddha is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    constructor(string memory _initBaseURI)
        ERC721("LuckyBuddha", "LUCKYBUDDHA")
    {
    ...
    function mint(uint256 _amount) external payable {
        require(!paused, "LuckyBuddha:the contract is paused");
        uint256 supply = totalSupply();
        require(
            supply + _amount <= maxSupply,
            "LuckyBuddha:max NFT limit exceeded"
        );
        if (msg.sender != owner()) {
            ...
            require(
                ...
                "LuckyBuddha:max NFT per address exceeded"
            );
            require(
                msg.value >= cost * _amount,
                "LuckyBuddha::mint: insufficient ethers"
            );
        }
