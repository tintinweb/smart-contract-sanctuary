...
contract Trade {
    ...
    function verifyOwnerSign(address buyingAssetAddress, address caller, string memory tokenURI, Sign memory sign) internal view {
        ...
        bytes32 hash = keccak256(abi.encodePacked(buyingAssetAddress, caller, tokenURI, sign.nonce));
    ...
    function mint(string memory tokenURI, uint256 fee, uint256 supply, address nftAddress, BuyingAssetType nftType,  Sign memory ownerSign) public returns(bool) {
        require(!usedNonce[ownerSign.nonce],"Nonce: Invalid Nonce");
        usedNonce[ownerSign.nonce] = true;
        if(nftType == BuyingAssetType.ERC721) {
            verifyOwnerSign(nftAddress, msg.sender, tokenURI, ownerSign);
            transferProxy.erc20safeTransferFrom(IERC20(MFGToken), msg.sender, IERC721(nftAddress).contractOwner(), mintingFee);
