...
interface IERC165 {
    ...
    event rarityClaim( uint256 price);
    ...
    mapping(address => uint256) public ingredientClaimableList;
    ...
    function claimIngredientReward() public payable {
        uint256 claimableAmount = ingredientClaimableList[msg.sender];
        require( claimableAmount >= 0, "You can not claim the specified amount");
        payable(msg.sender).transfer(claimableAmount);
        ingredientClaimableList[msg.sender] = 0;
        totalClaimable -= claimableAmount;
    }
    function claimRarityRewards( uint256 nftId) public payable {
        RarityReward memory rarityReward = rarityRewardsList[nftId];
        require(rarityReward.rewardPrice > 0 && !rarityReward.claimed, "You're not eligible to claim");
        payable(msg.sender).transfer(rarityReward.rewardPrice);
        rarityReward.claimed = true;
        rarityRewardsList[nftId] = rarityReward;
        totalClaimable -= rarityReward.rewardPrice;
        emit rarityClaim(rarityReward.rewardPrice);
    }
    ...
    function purchaseAndMintIngretient( uint256 _ingredientId) public payable {
        ...
        if(artist != address(0)) {
            uint256 currentClaimable = ingredientClaimableList[ingredientDetail.artist];
            if(!(currentClaimable > 0)) {
                currentClaimable += (msg.value * ARTIST_SHARE / 100);
                ingredientClaimableList[ingredientDetail.artist] = currentClaimable;
