...
contract DAOFactory is Ownable {
    ...
    mapping(address => address[]) public createdDao;// daoId -> DAO

    address public daoImpl;
    address public tokenImpl;
    address public votingProxy;

    event CreateDAO(uint id, address daoProxy, address tokenProxy, address creator);

    constructor (address _daoImpl, address _tokenImpl, address _votingProxy) {
        daoImpl = _daoImpl;
        tokenImpl = _tokenImpl;
        votingProxy = _votingProxy;
    }
    ...
    function createDAO(DAOInterface.DAOBasic memory basic, DAOInterface.Distribution memory dis, DAOInterface.DAORule memory rule) public returns (address) {
        ...
        DAOInterface(daoProxy).initialize(basic, dis, rule, tokenProxy, votingProxy);
        uint curId = curDaoId;
        uint id = curId + 1;
        daoMap[id] = daoProxy;
        createdDao[msg.sender].push(daoProxy);
        curDaoId = id;
        emit CreateDAO(id, daoProxy, tokenProxy, msg.sender);
        return daoProxy;
    }
}
