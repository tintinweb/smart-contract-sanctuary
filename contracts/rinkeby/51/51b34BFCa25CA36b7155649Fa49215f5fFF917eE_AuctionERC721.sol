...
contract AuctionERC721 {
    ...
    function finalizeAuction(uint256 _auctionId, uint256 _endDate) public {
        ...
        if (bidsLength == 0) {
            ...
            uint256 singlePrice = lastBid.amount.mul(100).div(105);

						singlePrice = singlePrice.sub(lastBid.amount.mul(serviceFee.add(commission)).div(10**8));

            if (!owner.send(singlePrice.sub(
                lastBid.amount.mul(100).div(105).div(100).mul(royaltyFee)
            ))) {
                revert();
            }
            if (!creator.send(singlePrice.div(100).mul(royaltyFee))) {
                revert();
            }

            if (!_webOwner.send(lastBid.amount.sub(singlePrice))) {
                revert();
            }
    ...
    function fixedPriceAuction(uint256 _auctionId) public payable {
        ...
        (
            ...
            bool onSale
				...
				singlePrice = singlePrice.sub(msg.value.mul(serviceFee.add(commission)).div(10**8));

            if (!owner.send(singlePrice.sub(
                msg.value.mul(100).div(105).div(100).mul(royaltyFee)
            ))) {
                revert();
            }
            if (!creator.send(singlePrice.div(100).mul(royaltyFee))) {
            ...
            if (!_webOwner.send(msg.value.sub(singlePrice))) {
                revert();
            }
