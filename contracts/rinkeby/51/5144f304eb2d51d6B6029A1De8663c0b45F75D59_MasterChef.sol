...
contract MasterChef is Ownable {
    ...
    function poolPendingReward(uint256 _pid)
        external
        view
        returns (uint256 currentRewardBlock, uint256 pendingRewardMint)
    {
    ...
    function deposit(
        ...
        if (_amount > 0) {
            require(!poolPause[_pid], "Pool PAUSED");
        }
    ...
    function enterStaking(uint256 _amount, address _referrer) public {
        ...
        if (_amount > 0) {
            require(!poolPause[0], "Pool PAUSED");
        }
        if (whitelistedUser[msg.sender] != true) {
            if (pool.poolLimit > 0) {
                require(
