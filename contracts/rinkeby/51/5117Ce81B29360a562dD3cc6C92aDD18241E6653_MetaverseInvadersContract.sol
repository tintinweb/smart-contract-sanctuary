...
abstract contract ERC721EnumMTIV is ERC721MTIV, IERC721Enumerable {
    ...
    function tokensOfOwner(address owner) public view returns (uint256[] memory) {
        ...
        require(0 < noTokens, "ERC721Enum: No tokens associated with this account");
...
contract MetaverseInvadersContract is ERC721EnumMTIV, Ownable, ReentrancyGuard {
    ...
    uint256 constant private MAX_SUPPLY = 7777;

    //invaders that will help us building community
    uint256 private FOUNDERS_RESERVE_MINTS = 77;

    //start/stop PRESALE phase
    bool public isPresaleActive = false;

    //max amount of NFTs to mint per account - PRESALE
    uint256 constant private MAX_PRESALE_MINT = 3;

    //allowed early adopters to start invading the Metaverse
    mapping(address => bool) public presaleWhitelist;
    //transcending in METAVERSE INVADERS
    mapping(address => uint256) public presaleWhitelistMints;

    //start/stop SALE phase
    bool public isSaleActive = false;

    //max amount of NFTs to mint per account - SALE
    ...
    function _publicSupply() internal view virtual returns (uint256) {
        return MAX_SUPPLY - FOUNDERS_RESERVE_MINTS;
    }
        ...
        payable 
    {
        ...
        presaleWhitelistMints[msg.sender] += _noInvaders;
        ...
        onlyOwner 
    {
        for (uint256 i = 0; i < _whitelistedAddresses.length; i++) {
            ...
            presaleWhitelistMints[_whitelistedAddresses[i]] > 0 ? presaleWhitelistMints[_whitelistedAddresses[i]] : 0;
        }
    }
    ...
    // founders minting
    function reservedFounders(address _to, uint256 _amount) 
        public 
        onlyOwner 
    {
        uint256 supply = ERC721EnumMTIV.totalSupply();

        require(_amount > 0, "MTIV: Minted amount must be greater than 0");
        require(_amount <= FOUNDERS_RESERVE_MINTS, "MTIV: Not enough reserve left for team");

        for (uint256 i = 0; i < _amount; i++) {
            _safeMint(_to, supply + i);
        }

        FOUNDERS_RESERVE_MINTS -= _amount;
    }
