...
contract UnitiDex_Governance is Ownable  {
    ...
    uint256 public tokenFreezeDuration=300;// default 1 day or 24 hours
  ...
  constructor(uint256 _MinTokenForVote,address _RYIPToken) public{
    ...
    function ClaimToken() public returns(bool) {
        ...
        for (uint256 i=0;i<details[msg.sender].length ;i++){
    ...
    function showFreezeToken() public view returns(uint256){
        ...
        for (uint256 i=0;i<details[msg.sender].length ;i++){
            ...
            if (now<details[msg.sender][i].time && details[msg.sender][i].amount>0){ // if deadline time is over.
                    vreturn= vreturn+details[msg.sender][i].amount;
            }
        }
        return vreturn;
    ...
    function resetClaimToken(uint256 _index) internal {

        delete details[msg.sender][_index];
    }

    function showUnFreezeToken() public view  returns(uint256){
      uint256 vreturn=0;
        //for (uint256 i=deadlineStarIndex[msg.sender];i<=deadlineLastIndex[msg.sender] ;i++){
        for (uint256 i=0;i<details[msg.sender].length ;i++){
