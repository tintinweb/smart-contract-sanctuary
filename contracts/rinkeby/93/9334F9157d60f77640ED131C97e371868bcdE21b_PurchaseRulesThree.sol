...
//    mapping(uint => bool) private auction;
    ...
    function setColors(uint xTopLeft, uint yTopLeft, uint xBottomRight, uint yBottomRight, uint[] memory colors) public {
        ...
        xBottomRight - xTopLeft >= 10 && yBottomRight - yTopLeft >= 10, "Invalid rectangle coordinates");
    ...
    function getPixelInfo(uint x, uint y) public view returns (uint color, string memory owner){
//        uint256 pixelPrice, bool auction){
        uint pixelId = getPixelId(x, y);
        return (pixelColors[x][y], ownerUrls[ownerOf(pixelId)]);
//        return (pixelColors[x][y], ownerUrls[ownerOf(pixelId)],pixelPrice[pixelId],auction[pixelId]);
    ...
    function getPixelId(uint x, uint y) public view returns (uint256){
        return x * 1000 + y;
    }

    function getPixelCoordinates(uint256 id) public view returns (uint x, uint y) {
        return (uint(id / 1000), uint(id % 1000));
    }

    function setPurchaseRule(address purchaseRule) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ERC721PresetMinterPauserAutoId: must have minter role to mint");
        grantRole(MINTER_ROLE, purchaseRule);
    }
...
contract PurchaseRulesThree {
    ...
    constructor(MIL1 _mil1, Pixel _pixel){
        mil1 = _mil1;
        pixel = _pixel;
    }
