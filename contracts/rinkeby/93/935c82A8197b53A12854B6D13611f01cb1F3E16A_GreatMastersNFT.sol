...
// Part: OpenZeppelin/openzeppel[email protected]/Address
...
contract GMMetadata is IGMMetadata, Ownable {
    ...
    function add(string calldata ipfsHash) external onlyOwner override(IGMMetadata) returns (uint256 tokenId){
        tokenId = nextTokenId;
     ...
     * @dev Returns the address of the current owner.
     */
    function owner() public view override(Ownable, IGMMetadata) returns (address) {
        return super.owner();
    }
}
...
// File: GreatMastersNFT.sol

contract GreatMastersNFT is ERC721, Ownable {
    using Strings for uint256;
    ...
    ERC721(name_, symbol_) {
        metadata = new GMMetadata(name_, symbol_);
        minter = minter_;
    }
    ...
    function mint(address to, string calldata ipfsHash) public onlyOwner returns(uint256 tokenId) {
    ...
    function setMetadata(IGMMetadata metadata_) public onlyOwner {
        metadata = metadata_;
    }
}