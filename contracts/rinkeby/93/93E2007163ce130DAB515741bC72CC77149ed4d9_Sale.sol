...
contract Sale is Ownable {
    ...
    struct saleRecord {
        ...
        uint256 tokenPrice; // sale token price in payment tokens
    ...
    // saleID => wallet => allocation_value
    mapping(uint256 => mapping(address => uint256)) saleAllocations;
    ...
    function createSale(
        ...
        sales[totalSales] = saleRecord(msg.sender, token, name, paymentToken, dates, tokenPrice, 0, 0);
        emit SaleCreated(msg.sender, totalSales);
    }

    function getSaleRecord(uint256 id) public view returns (saleRecord memory) {
        return (sales[id]);
    }
    ...
    function registerStake(uint256 saleID) public {
        ...
        uint256 curAllocation = saleAllocations[saleID][msg.sender];
        // Get current stake
        uint256 curStake = stakingContract.getCurrentStake(msg.sender);

        require(curStake > curAllocation, "Sale:only add stake!");

        saleAllocations[saleID][msg.sender] = saleAllocations[saleID][msg.sender].add(curStake.sub(curAllocation));
    ...
    function getSalestage(uint256 saleId) public view returns (stages) {
        if (
            saleId == 0 ||
