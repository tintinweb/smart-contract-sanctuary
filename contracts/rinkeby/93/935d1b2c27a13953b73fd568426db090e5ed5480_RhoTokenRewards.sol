...
contract RhoTokenRewards is IRhoTokenRewards, Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /**
     * @notice Rewards earned per block for the entire pool of RhoToken holders
     */
    ...
    function rewardsPerToken() public view override returns (uint256) {
        ...
        return _rewardsPerToken.add(
            (lastBlockApplicable().sub(_lastUpdateBlock)).mul(_rewardsRate).mul(_rhoTokenOne).div(totalSupply)
        );
    }
     ...
     */
    function earned(address addr) internal view returns (uint256) {
        return _rhoToken.balanceOf(addr).mul(rewardsPerToken().sub(_rewardsPerTokenPaid[addr])).div(_rhoTokenOne).add(_rewards[addr]);
    }
    ...
    function setRewardAmount(uint256 reward) external override onlyOwner {
        ...
        if (block.number >= _rewardsEndBlock) {
            _rewardsRate = reward.div(_rewardsDurationInBlocks);
        } else {
            uint256 blocksRemaining = _rewardsEndBlock.sub(block.number);
            uint256 leftover = blocksRemaining.mul(_rewardsRate);
            _rewardsRate = reward.add(leftover).div(_rewardsDurationInBlocks);
        }
        ...
        require(_rewardsRate <= balance.div(_rewardsDurationInBlocks), "Insufficient balance for rewards");
    ...
    function setRewardsDuration(uint256 rewardsDurationInSeconds) external override onlyOwner {
        ...
        _rewardsDurationInBlocks = rewardsDurationInSeconds.mul(1e3).div(BLOCK_TIME_MS);
    ...
    function claimReward() external override {
        ...
        updateReward(user);
        if (_rewards[user] > 0) {
            _rewards[user] = _flurryRewards.claimRhoTokenReward(user, _rewards[user]);
        }
    }
