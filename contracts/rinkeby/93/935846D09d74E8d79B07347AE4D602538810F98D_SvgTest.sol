...
contract SvgTest is ERC721Enumerable, Ownable {
	...
	function generatePaths() private pure returns (string memory result) {
		string
			memory d = 'M458,510c-2.87772,0 -2,-5.14413 -2,-8c0,-17.68123 20.73542,-4.26458 25,0c12.25498,12.25498 20.53301,-22.46699 33,-10c2.69412,2.69412 32,32.26511 32,7';
		uint16 scale = 10;
		uint16 rotation = 30;
		...
		for (uint8 i = 0; i < 34; i++) {
			...
			result = string(abi.encodePacked(result, generatePath(d, path)));
		}
	}

	function generatePath(string memory d, Path memory path)
		private
		pure
		returns (string memory)
	{
		...
		return
			string(
				abi.encodePacked(
					...
					SvgTestLibrary.decimalString(path.scale1, 1, false),
					'))" style="transform: rotate(calc(1 + ',
					SvgTestLibrary.decimalString(path.rotation1, 0, false),
					'deg)) scale(calc(1 + ',
					SvgTestLibrary.decimalString(path.scale1, 1, false),
					'));"></path>',
					'<path d="',
					d,
					'" stroke-width="calc(2 / (1 + ',
					SvgTestLibrary.decimalString(path.scale2, 1, false),
					'))" style="transform: rotate(calc(1 + ',
					SvgTestLibrary.decimalString(path.rotation2, 0, false),
					'deg)) scale(calc(1 + ',
					SvgTestLibrary.decimalString(path.scale2, 1, false),
					'));"></path>',
					'<path d="',
					d,
					'" stroke-width="calc(2 / (1 + ',
					SvgTestLibrary.decimalString(path.scale3, 1, false),
					'))" style="transform: rotate(calc(1 + ',
					SvgTestLibrary.decimalString(path.rotation3, 0, false),
					'deg)) scale(calc(1 + ',
					SvgTestLibrary.decimalString(path.scale3, 1, false),
					'));"></path>'
				)
