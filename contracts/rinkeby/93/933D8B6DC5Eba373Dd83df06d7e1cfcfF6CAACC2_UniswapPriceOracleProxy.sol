...
contract UniswapPriceOracle is UniswapPriceOracleStorage, PriceOracle, OracleErrorReporter {
    ...
    function getReservesFromPair(address asset) public view returns (uint112, uint112, uint32) {
        ...
        if (token0 == WETHToken || token1 == WETHToken) {
            if (token0 == asset) {
                (assetReserve, equivalentInETH, blockTimeStamp) = pair.getReserves();
            } else {
                (equivalentInETH, assetReserve, blockTimeStamp) = pair.getReserves();
             }
        } else {
            uint power;
            uint stableCoinReserve;

            if (token0 == asset) {
                (assetReserve, stableCoinReserve, blockTimeStamp) = pair.getReserves();
                power = EIP20Interface(token1).decimals();
                equivalentInETH = uint112(getCourseInETH(token1) * stableCoinReserve / (10**power));
            } else {
                (stableCoinReserve, assetReserve, blockTimeStamp) = pair.getReserves();
                power = EIP20Interface(token0).decimals();
                equivalentInETH = uint112(getCourseInETH(token0) * stableCoinReserve / (10**power));
            }
        }

        return (assetReserve, equivalentInETH, blockTimeStamp);
    }
    ...
    function getETHAmount(address asset, uint amountIn) public view returns (uint) {
        return cumulativePrices[assetPair[asset]][asset].priceAverage.mul(amountIn).decode144();
    }

    function _setNewAddresses(address registry_, address WETHToken_, address ETHUSDPriceFeed_) external returns (uint) {
        // Check caller = admin
