...
contract Snowflakes is ERC1155Base {
    string public name;
    ...
    function mint(uint256 id, Fee[] memory fees, uint256 supply, string memory uri) public payable {
        ...
        require(supply == 1,"Can't mint more than 1 unit of token");
...
contract SnowflakesLikes {

  Snowflakes public collectible;
  ...
  constructor(Snowflakes _collectible) public {
      collectible = _collectible;
  }
  
  event LikeOrUnlike(uint256 id);

  function like(uint256 id) public {
    require(id > 0);
    require(id <= collectible.totalSupply());
    require(liked[msg.sender][id] == false);
    liked[msg.sender][id] = true;
    likes[id] = likes[id].add(1);
    emit LikeOrUnlike(id);
  }

  function unlike(uint256 id) public {
    require(id > 0);
    require(id <= collectible.totalSupply());
    require(liked[msg.sender][id] == true);
    liked[msg.sender][id] = false;
    likes[id] = likes[id].sub(1);
    emit LikeOrUnlike(id);
