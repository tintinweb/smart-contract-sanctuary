...
contract Betting is Ownable {
  ...
  using SafeMath for uint;
	...
	uint public referrerFee = 1e16; //1%
	uint public globalFee; //in % (10^18 meanth 100%)
	...
	event GameClosed(address indexed winner, uint indexed betType, uint indexed gameIndex, uint prize1, uint prize2, uint fee1, uint fee2);
	...
	modifier onlyRegistrated() {
		require(userReferrer[msg.sender] != address(0), 'register first');
		require(!inGame[msg.sender], 'already in game'); 
		_; 
	}

	constructor(address _managerAddress, address _eth_usd_consumer_address, address[] memory supportedTokens, address[] memory chainlink_price_consumer_addresses, bool[] memory _isStablecoin) public Ownable() {
	...
	function registration(address _referrer) public {
		require(userReferrer[msg.sender] == address(0), "user already exists");
		if(_referrer == address(0) || (_referrer != owner() && userReferrer[_referrer] == address(0))) {
			_referrer = owner();
		}
	...
	function bet(address tokenAddress, uint t) public payable onlyRegistrated {
		...
		if(msg.value > 0) {
			ethBet(msg.sender, t, msg.value);
			return;
		}

		tokenBet(msg.sender, t, tokenAddress);
	}
	...
	function closeGame(uint t, uint gameIndex, address payable winnerAddress) public onlyManager {
		...
		uint gameFee1 = _game.amount1.mul(globalFee).div(1e18);
		uint gameFee2 = _game.amount2.mul(globalFee).div(1e18);

		uint transactionFee1 = tokenPrice[_game.token1].mul(transactionFee);
		uint transactionFee2 = tokenPrice[_game.token2].mul(transactionFee);
		...
		if(_game.token1 == _game.token2) {
			...
			if(_game.token1 == ETH_ADDRESS) {
				...
				emit GameClosed(winnerAddress, t, gameIndex, _game.amount1.add(_game.amount2).sub(completedFee), 0, completedFee, 0);
				return;
			}
		...
		uint totalFee2 = gameFee2.add(transactionFee2);

		internalTransfer(winnerAddress, _game.token1, _game.amount1, totalFee1);
		internalTransfer(winnerAddress, _game.token2, _game.amount2, totalFee2);
		
		emit GameClosed(winnerAddress, t, gameIndex, _game.amount1.sub(totalFee1), _game.amount2.sub(totalFee2), totalFee1, totalFee2);
	}

	function internalTransfer(address payable winnerAddress, address tokenAddress, uint value, uint fee) internal {
		if(tokenAddress == ETH_ADDRESS) {
			safeEthTransfer(owner(), fee);
			safeEthTransfer(winnerAddress, value.sub(fee));
			// address(uint160(winnerAddress)).transfer(value.sub(fee));
			return;
		}
	...
	function ethBet(address user, uint t, uint ethValue) internal {
		...
		uint referrerAmount = ethValue.mul(referrerFee).div(1e18);

		safeEthTransfer(userReferrer[user], referrerAmount);
		ethValue = ethValue.sub(referrerAmount);
	...
	function tokenBet(address user, uint t, address tokenAddress) internal {
		...
		uint referrerAmount = tokenValue.mul(referrerFee).div(1e18);
		IERC20(tokenAddress).transfer(userReferrer[user], referrerAmount);

		tokenValue = tokenValue.sub(tokenValue.mul(referrerFee).div(1e18));
		...
		if (typeGames[t][typeGames[t].length-1].gamer2 == address(0)) {
			require(typeGames[t][typeGames[t].length-1].gamer1 != user, 'double registration');
			typeGames[t][typeGames[t].length-1].gamer2 = user;
			typeGames[t][typeGames[t].length-1].token2 = tokenAddress;
			typeGames[t][typeGames[t].length-1].amount2 = tokenValue;

			emit NewGamer(user, t, typeGames[t].length-1, tokenAddress, tokenValue);
			emit GameStarted(typeGames[t][typeGames[t].length-1].gamer1, typeGames[t][typeGames[t].length-1].gamer2, t, typeGames[t].length-1);
			return;
		}
	...
	function newGame(address user, uint t, address tokenAddress, uint value) internal {
		typeGames[t].push(Game(user, address(0), tokenAddress, address(0), value, 0, false));
