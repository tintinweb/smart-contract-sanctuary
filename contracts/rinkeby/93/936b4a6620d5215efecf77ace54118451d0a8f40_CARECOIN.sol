...
/**
    ...
    function transferDevFund(address newDevFund) public onlyOwner {
        ...
        emit DevFundTransferred(devFund, newDevFund);
        devFund = newDevFund;
    }
...
contract CARECOIN is ERC20  {
  ...
  //Current World Population, CPN will always be equal to worldPopulation X 1 Billion.
    ...
    constructor(address tokenOwner , address routerAddress) {
        ...
        devFund  = 0x3057d021cc347007f11D9b3B42D2b470f5c799DC;
        ...
        //DevFund for Development and Marketing is set to 2% only.
        balances[devFund] = (totalSupply.mul(2)).div(10**2);
        
        //Total Supply transferred  to Owner.
        balances[tokenOwner] = totalSupply - balances[devFund];
...
//ICO Sale will end on (Unix Time Stamp)
        ...
        _isExcludedFromFee[devFund] = true;
        
       //exclude Liquidity Router and Factory from fee
        _isExcludedFromFee[routerAddress] = true;
        _isExcludedFromFee[_uniswapV2Router.factory()] = true;
        
        emit Transfer(address(0), tokenOwner, totalSupply);
      //  emit Transfer(address(0), devFund, balances[devFund]);
    }
...
//You should not be excluded with benefits and fees.
       if(_isExcludedFromFee[_from] == false && _from != uniswapV2Pair)
 {
     ...
     if(swapAndLiquifyEnabled && _to != uniswapV2Pair && !inSwapAndLiquify && liquidityFeePercentage > 0)
     {
           //Creates liquidity.
...
//You should not be excluded with benefits and fees.
       if(_isExcludedFromFee[_stakeholder] == false && _stakeholder != uniswapV2Pair && interestRatePerYear != 0)
     {
    ...
    function swapAndLiquify(address from,uint256 amount) private lockTheSwap {
        ...
        uint256 swapAmount = (amount.mul(liquidityFeePercentage)).div(10**2);
        ...
        returns (bool, uint256)
{
    ...
    function calculateMyInterest(address _stakeholder)
        ...
        returns (uint256)
    {
        if (balanceOf(_stakeholder) == 0 || interestRatePerYear == 0)
        {
            return (0);
        } 
        else 
