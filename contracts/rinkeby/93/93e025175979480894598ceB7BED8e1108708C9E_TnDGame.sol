...
import './interfaces/IWhitelistVerify.sol';

contract TnDGame is ITnDGame, Ownable, ReentrancyGuard, Pausable {
  ...
  uint256 private priceDecrementAmt = 0.00 ether; //0.01
  uint256 private timeToDecrementPrice = 10 minutes;
  uint256 private startedTime;
  // max $GT cost
  uint256 private maxGtCost = 72000 ether;

  // reference to the Temple for choosing random Demon thieves
  ITemple public temple;
  // reference to $GT for burning on mint
  IGameToken public gtToken;
  // reference to Traits
  ITraits public traits;
  // reference to NFT collection
  IGameNft public tndNFT;
  // reference to altar collection
  IAltar public altar;
  // ref to whitelist
  IWhitelistVerify public whitelist;
  ...
  function setContracts(
    ...
    tndNFT = IGameNft(_tnd);
    temple = ITemple(_temple);
    altar = IAltar(_altar);
    randomizer = IRandom(_rand);
    whitelist = IWhitelistVerify(_whitelist);
  }
  ...
  /**
   * mint a token - 90% Templar, 10% Demons
   * The first 25% are claimed with eth, the remaining cost $GT
   */
  function mint(
    uint256 amount,
    bool stake,
    ...
    uint16 minted = tndNFT.minted();
    uint256 maxTokens = tndNFT.getMaxTokens();
    uint256 paidTokens = tndNFT.getPaidTokens();
    ...
    if (minted < paidTokens) {
      ...
      if (hasPublicSaleStarted) {
        ...
        bool canClaim = whitelist.canClaim(_msgSender(), whitelistProof, amount);
        if (!canClaim) {
          return;
        }

        whitelist.addClaimed(_msgSender(), amount);
      }
    } else {
      require(msg.value == 0);
    }
    ...
    for (uint256 i = 0; i < amount; i++) {
      ...
      if (recipient != _msgSender() && altar.balanceOf(_msgSender(), treasureChestTypeId) > 0) {
        // If the mint is going to be stolen, there's a 50% chance
        //  a demon will prefer a treasure chest over it
        if (seed & 1 == 1) {
          altar.safeTransferFrom(_msgSender(), recipient, treasureChestTypeId, 1, '');
          recipient = _msgSender();
        }
      }
      tokenIds[i] = minted;
      if (!stake || recipient != _msgSender()) {
        tndNFT.mint(recipient, seed);
      } else {
        tndNFT.mint(address(temple), seed);
      }
      totalGtCost += mintCost(minted, maxTokens, paidTokens);
    }
    tndNFT.updateOriginAccess(tokenIds);
    if (totalGtCost > 0) {
      gtToken.burn(_msgSender(), totalGtCost);
      gtToken.updateOriginAccess();
    }
    if (stake) {
      temple.addManyToTempleAndFlight(_msgSender(), tokenIds);
    }
  ...
  function mintCost(
    ...
    if (tokenId > (maxTokens * 17) / 20) return maxGtCost;
  }

  function payTribute(uint256 gtAmt) external whenNotPaused nonReentrant {
    require(tx.origin == _msgSender(), 'Only EOA');
    uint16 minted = tndNFT.minted();
    uint256 maxTokens = tndNFT.getMaxTokens();
    uint256 paidTokens = tndNFT.getPaidTokens();
    uint256 gtMintCost = mintCost(minted, maxTokens, paidTokens);
    require(gtMintCost > 0, 'Sacrificial altar currently closed');
    require(gtAmt >= gtMintCost, 'Not enough gt given');
    gtToken.burn(_msgSender(), gtAmt);
    if (gtAmt < gtMintCost * 2) {
      altar.mint(1, 1, _msgSender());
    } else {
      altar.mint(2, 1, _msgSender());
    }
  }
  ...
  function makeTreasureChests(uint16 qty) external whenNotPaused {
    ...
    // $GT exchange amount handled within altar contract
    // Will fail if sender doesn't have enough $GT
    // Transfer does not need approved,
    //  as there is established trust between this contract and the altar contract
    altar.mint(treasureChestTypeId, qty, _msgSender());
  }
  ...
  function sellTreasureChests(uint16 qty) external whenNotPaused {
    ...
    require(treasureChestTypeId > 0, 'DEVS DO SOMETHING');
    // $GT exchange amount handled within altar contract
    altar.burn(treasureChestTypeId, qty, _msgSender());
  }

  function sacrifice(uint256 tokenId, uint256 gtAmt) external whenNotPaused nonReentrant {
    require(tx.origin == _msgSender(), 'Only EOA');
    ...
    IGameNft.TemplarDemon memory nft = tndNFT.getTokenTraits(tokenId);
    uint16 minted = tndNFT.minted();
    uint256 maxTokens = tndNFT.getMaxTokens();
    uint256 paidTokens = tndNFT.getPaidTokens();
    uint256 gtMintCost = mintCost(minted, maxTokens, paidTokens);
    require(gtMintCost > 0, 'Sacrificial altar currently closed');
    if (nft.isTemplar) {
      // Templar sacrifice requires 3x $GT curve
      require(gtAmt >= gtMintCost * 3, 'not enough gt provided');
      gtToken.burn(_msgSender(), gtAmt);
      // This will check if origin is the owner of the token
      tndNFT.burn(tokenId);
      altar.mint(3, 1, _msgSender());
    } else {
      // Demon sacrifice requires 4x $GT curve
      require(gtAmt >= gtMintCost * 4, 'not enough gt provided');
      gtToken.burn(_msgSender(), gtAmt);
      // This will check if origin is the owner of the token
      tndNFT.burn(tokenId);
      altar.mint(4, 1, _msgSender());
    }
  }
  ...
  function selectRecipient(
    ...
    address thief = temple.randomDemonOwner(seed >> 144); // 144 bits reserved for trait selection
  ...
  function setMaxGtCost(uint256 _amount) external requireContractsSet onlyOwner {
    maxGtCost = _amount;
  }

  function setPresalePrice(uint256 _price) external requireContractsSet onlyOwner {
    presalePrice = _price;
  }
...
interface IWhitelistVerify {
  function isWhitelisted(address addr, bytes32[] memory proof) external view returns (bool);

  function getClaimableAmount(address addr, bytes32[] memory proof) external view returns (uint256);
  ...
  function addClaimed(address addr, uint256 amount) external;
}
