...
contract RangePool is ERC20, Ownable {
    ...
    function swap( address firstToken, uint amount, address secondToken ) external {
        ...
        IERC20( secondToken ).safeTransfer( msg.sender, amount.sub( amount.mul(fee).div(1e9) ) ); // take fee on amount
    }

    /* ========== ADD LIQUIDITY ========== */

    // add token to pool as liquidity. returns number of pool tokens.
    function add( address token, uint amount ) external returns ( uint tokensToGive_ ) {
        tokensToGive_ = amount.mul(1e18).div(redemptionValue());
        ...
        _mint( msg.sender, tokensToGive_ );
    }

    // add liquidity evenly across all tokens 
    function addAll( uint amount ) external returns ( uint tokensToGive_ ) {
        ...
        totalTokens = totalTokens.add( amount ); // add amount second (to not skew pool)
    ...
    function remove( address token, uint amount ) external {
        _burn( msg.sender, amount ); // burn pool token

        amount = amount.sub( amount.mul(fee).div(1e9) ); // remove fee
        amount = amount.mul( redemptionValue() ).div(1e18); // convert
    ...
    function removeAll( uint amount ) external {
        ...
        amount = amount.sub( amount.mul(fee).div(1e9) );
        amount = amount.mul( redemptionValue() ).div(1e18);

        for ( uint i = 0; i < tokens.length; i++ ) {
            IERC20 token = IERC20( tokens[i] );
            uint send = amount.mul( token.balanceOf( address(this) ) ).div( totalTokens );

            emit Remove( tokens[i], send );
    ...
    // number of tokens 1 share can be redeemed for
