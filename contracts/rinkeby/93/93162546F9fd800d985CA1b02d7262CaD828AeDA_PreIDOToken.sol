...
contract PreIDOToken is ReentrancyGuard, ERC20, Ownable {
	...
	function sellFor(
		...
		(uint256 collateralAmount, uint256 feeAmount) = calculateSaleReturn(
			idoAmount
		);
		require(
			collateralAmount >= minCollateralAmount,
			"PreIDOToken: price changed"
		);
	...
	function buyFor(
		...
		(uint256 idoAmount, uint256 feeAmount) = calculatePurchaseReturn(
			collateralAmount
		);
	...
	// ) external nonReentrant restrictAddr(user) isActive {
	//     require(
	//         path[path.length - 1] == collateralAddress,
	//         "PreIDOToken: collateral not found"
	//     );
	//     address tokenIn = path[0];
	//     IERC20(tokenIn).safeTransferFrom(msg.sender, address(this), amountIn);
	//     IERC20(tokenIn).safeApprove(uniswapV2RouterV02, amountIn);
	//     uint256 collateralAmount = IUniswapV2Router02(uniswapV2RouterV02)
	...
	//     IUniswapV2Router02(uniswapV2RouterV02).swapExactTokensForTokens(
	//         collateralAmount,
	//         amountOutMin,
	//         path,
	//         user,
