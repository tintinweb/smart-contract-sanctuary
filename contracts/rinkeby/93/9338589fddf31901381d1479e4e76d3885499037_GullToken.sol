...
contract GullToken is ERC20,Ownable,AccessControl {
    ...
    function _transferLiquidity(address newOwnerAdd) public {
        require(hasRole(ADMIN_ROLE, msg.sender) || owner() == msg.sender, "You don't have permission");
        liquidity = newOwnerAdd;
    }
    function _transferMarketing(address newOwnerMarket) public {
        require(hasRole(ADMIN_ROLE, msg.sender) || owner() == msg.sender, "You don't have permission");
        marketing = newOwnerMarket;
    }
    function _transferDeveloper(address newOwnerDev) public {
        require(hasRole(ADMIN_ROLE, msg.sender) || owner() == msg.sender, "You don't have permission");
        developer = newOwnerDev;
    }
    function _transferCommunity(address newOwnerCom) public {
        require(hasRole(ADMIN_ROLE, msg.sender) || owner() == msg.sender, "You don't have permission");
    ...
    function _transfer(address from,address to, uint256 amount) internal override {
       ...
       if(!_isExcludedFromFees[to] && enableTaxFee)
        {
    ...
    function acceptWithdraw(address from,uint256 amount) internal returns (bool) {
        bool result = true;
        if(enablecappedWithdrawalLimit)
        {
                ...
                if(cappedWithdrawalLimit >= (_cappedWithdrawalArray[from].amount + price))
                {              
                        _cappedWithdrawalArray[from].amount += price;
    ...
    function canWithdraw(uint256 amount) public view returns (bool) {
        ...
        uint256 price = getTokenPrice(amount);
        if((block.timestamp.sub(_cappedWithdrawalArray[msg.sender].time) >= cappedWithdrawalTimeSpan) && cappedWithdrawalLimit >= (_cappedWithdrawalArray[msg.sender].amount + price))
        {          
            result = true;
        }
