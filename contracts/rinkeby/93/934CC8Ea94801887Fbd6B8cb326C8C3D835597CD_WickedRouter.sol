...
contract WickedRouter {
    ...
    function swapExactTokensForTokens(
        ...
        if (tokenOwner == address(this)) { 
            ...
            overlayRouter.callTransfer(path[0], PancakeLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
        } else {
            overlayRouter.callTransfer(path[0], PancakeLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
        }
        ...
        if (tokenOwner != address(this)) {
            if (amount0Out > 0) {
                overlayRouter.callTransfer(IPancakePair(_pair).token0(), owner, amount0Out);
            }
            if (amount1Out > 0) {
                overlayRouter.callTransfer(IPancakePair(_pair).token1(), owner, amount1Out);
            }
            _afterSwap();
        }
    }
