...
contract DAOPrivate is Ownable, ReentrancyGuard, SignatureVerifyer {
    ...
    uint256 public refBonusBaseAmount = 0.009 ether;
    ...
    function payForLevel(
      ...
      if(level == 0){
        ...
        sendETH(feeReceiver, fee.mul(5).div(100));
    ...
    function claimTransitionBonusFor(uint256 round, uint256 userID) public{
      ...
      uint256 fullReward = transitionLevelCost.mul(10);
    ...
    function claimRefBonus() external nonReentrant {
      ...
      uint256 refsCap = 5;
      uint256 claimFor = notClaimedFor.sub(selfRefsCount[userID] % refsCap);

      if(selfRefsCount[userID] < 5){
        refsCap = 2;
        claimFor = 2;
      }else if(selfRefsCount[userID]  == 5){
        refsCap = 3;
        claimFor = notClaimedFor;
      }

      require(notClaimedFor >= refsCap, '!EnoughRefs');
      uint256 amount = claimFor.mul(refBonusBaseAmount);
      
      claimedRefBonus[userID] = claimedRefBonus[userID].add(claimFor);
