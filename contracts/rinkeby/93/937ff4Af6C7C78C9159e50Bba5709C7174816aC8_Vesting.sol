...
contract Vesting {
    ...
    /**
     * @notice Claim the specified number of tokens. For a successful transaction, the requirement
     * `_amount > 0 && _amount <= unreleased` must be met.
     * @param _amount The number of tokens to claim.
     */
    function claim(uint256 _amount) external {
        VestingSchedule storage schedule = schedules[msg.sender];

        require(schedule.start != 0, "Vesting: No vesting schedules for the caller");

        uint256 unreleased = releasableAmount(msg.sender);
        require(_amount > 0 && _amount <= unreleased, "Vesting: Insufficient number of unreleased tokens");

        uint256 mntRemaining = mnt.balanceOf(address(this));
        require(_amount <= mntRemaining, "Vesting: Insufficient token balance in the Vesting contract");

        schedule.released = schedule.released.add(_amount);
        mnt.transfer(msg.sender, _amount);
