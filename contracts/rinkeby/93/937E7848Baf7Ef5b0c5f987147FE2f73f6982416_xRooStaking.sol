...
import "../nftx/interface/INFTXStakingZap.sol";
...
import "../nftx/interface/ITimelockRewardDistributionToken.sol";
...
contract xRooStaking {
    ...
    INFTXStakingZap public NFTXStakingZap;
    INFTXLPStaking public NFTXLPStaking;
    IVaultTokenUpgradeable public NFTXRewardToken;
    ITimelockRewardDistributionToken public NFTXRewardDistributionToken;
    ...
    constructor(
        address payable _NFTXZap,
        address _NFTXLPStaking,
        uint256 _vaultId,
        address _NFTXRewardToken,
        ...
        NFTXStakingZap = INFTXStakingZap(_NFTXZap);
        NFTXLPStaking = INFTXLPStaking(_NFTXLPStaking);
        NFTXRewardToken = IVaultTokenUpgradeable(_NFTXRewardToken);
        NFTXRewardDistributionToken = ITimelockRewardDistributionToken(
        ...
        IERC20Upgradeable(address(WETH)).approve(_NFTXZap, type(uint256).max);
        SLPToken.approve(_sushiRouter, type(uint256).max);
        NFTXRewardDistributionToken.approve(
            address(NFTXLPStaking),
            type(uint256).max
        );
        NFTXRewardToken.approve(address(sushiRouter), type(uint256).max);
    ...
    function addLiquidityERC721(
        ...
        ForwardProxy proxy = _getProxy(msg.sender, true);
        ...
        _addLiquidityERC721(msg.sender, _minWethIn, _wethIn, _ids, proxy);

        _drainProxy(proxy, false, payable(msg.sender)); // NFTX refunds to proxy, sushi does not.
    }
        ...
        payable
    {
        ...
        _addLiquidityERC721(msg.sender, _minWethIn, msg.value, _ids, proxy);
        _drainProxy(proxy, true, payable(msg.sender));
    }
    ...
    function addLiquidityERC20(
        ...
        (uint256 rewardTokenUsed, uint256 wethUsed) = _addLiquidityERC20(
            msg.sender,
            _minWethIn,
            _wethIn,
            _amount,
        ...
        if (rewardTokenUsed < _amount)
            NFTXRewardToken.transfer(msg.sender, _amount - rewardTokenUsed);
        if (wethUsed < _wethIn)
            IERC20(address(WETH)).transfer(msg.sender, _wethIn - wethUsed);
    }
        ...
        payable
    {
        ...
        if (wethUsed < msg.value)
            IERC20(address(WETH)).transfer(msg.sender, msg.value - wethUsed);
    }
    ...
    function liquidityOf(address _user) external view returns (uint256) {
        address proxy = _getProxyAddress(_user);
        return NFTXRewardDistributionToken.balanceOf(proxy);
    }
    ...
    function timelockUntil(address _user) external view returns (uint256) {
        return NFTXRewardDistributionToken.timelockUntil(_getProxyAddress(_user));
    }

    // ---------- INTERNAL CONTRACT METHODS ----------

    /**
     * Deploys a proxy for a given user
     * @dev will fail if already exists
     */
    function _createProxy(address _user) internal returns (address addr) {
        bytes memory bytecode = type(ForwardProxy).creationCode;
        ...
        ForwardProxy proxy = ForwardProxy(payable(addr));

        // approve adding liquidity to NFTX
        IERC20(proxy.cast(address(SLPToken))).approve(
            address(NFTXLPStaking),
            type(uint256).max
        );

        // approve removing WETH
        IERC20(proxy.cast(address(WETH))).approve(
            address(this),
            type(uint256).max
        );

        // approve removing vault token
        IERC20(proxy.cast(address(NFTXRewardDistributionToken))).approve(
            address(NFTXLPStaking),
            type(uint256).max
        );

        // approve removing rewards
        IERC20(proxy.cast(address(NFTXRewardToken))).approve(
            address(this),
            type(uint256).max
        );

        // approve sushi unstaking
        IERC20(proxy.cast(address(SLPToken))).approve(
            address(sushiRouter),
            type(uint256).max
        );
    ...
    /**
     * Gets the address of the proxy contract that manages users' reward distribution tokens.
     * @dev does not check to see if that contract exists.
     */
    function _getProxyAddress(address _user) internal view returns (address) {
        bytes32 salt = keccak256(abi.encodePacked(_user));
        bytes32 hash = keccak256(
            abi.encodePacked(
                ...
                address(this),
                salt,
                keccak256(type(ForwardProxy).creationCode)
            )
        );
     ...
     * @dev Not to be used for security, only to determine if proxies are deployed.
     */
    ...
    /**
     * Gets the proxy for a given user.
     * @dev will fail if the proxy doesn't exist
     */
    function _getProxy(address _user, bool _create)
        internal
        returns (ForwardProxy)
    {
        address proxy = _getProxyAddress(_user);

        bool exists = _isContract(proxy);
        if (!_create) require(exists, "Not deployed");
        else if (!exists) _createProxy(_user);

        return ForwardProxy(payable(proxy));
    }

    /**
     * Adds liquidity on behalf of _to, does not do any refunds to the _user
     */
    function _addLiquidityERC721(
        ...
        RTStakedToken.setApprovalForAll(address(NFTXStakingZap), true);

        uint256 liquidity = NFTXStakingZap.addLiquidity721To(
            vaultId,
            _ids,
            _minWethIn,
            _wethIn,
        ...
        uint256 numCurrentPeriods = userData.lastTimestamp == 0
            ? 0
            : (block.timestamp - userData.lastTimestamp) / rewardPeriod;

        uint256 currentRTReward = numCurrentPeriods *
            periodicReward.mul(userData.stake);

        userData.RTRewardModifier += int256(currentRTReward);
        userData.lastTimestamp = block.timestamp;
        userData.stake += _ids.length * base;
        users[_user] = userData;

        emit Staked(_user, userData.stake, liquidity);
    }
    ...
    {
        address proxyAddress = _getProxyAddress(_user);
        uint256 nftxReward = NFTXRewardDistributionToken.dividendOf(
            proxyAddress
        );
        ...
        require(rtReward >= 0, "Negative Reward");
    ...
    function _claimRewards(address _user) internal {
        ...
        if (rewards.NFTXRewardToken > 0) {
            ForwardProxy proxy = _getProxy(_user, false);

            INFTXLPStaking(proxy.cast(address(NFTXLPStaking))).claimRewards(
                vaultId
            ); // puts rewards in proxy

            NFTXRewardToken.transferFrom(
                address(proxy),
                _user,
                NFTXRewardToken.balanceOf(address(proxy))
            );
        }
    ...
    function _removeLiquidity(
        ...
        uint256 amount = NFTXRewardDistributionToken.balanceOf(address(proxy));
        uint256 stake = users[_user].stake;
        _claimRewards(_user);
        delete users[_user];

        // remove from NFTXLPStaking
        INFTXLPStaking(proxy.cast(address(NFTXLPStaking))).withdraw(
            vaultId,
            amount
        ); // gives proxy <amount> SLP

        IUniswapV2Router01(proxy.cast(address(sushiRouter))).removeLiquidity(
            address(NFTXRewardToken),
            address(WETH),
            amount,
            _amountTokenMin,
            _amountWETHMin,
    ...
    function _addLiquidityERC20(
        ...
        uint256 _amount,
        ForwardProxy _proxy
    ) internal returns (uint256 rewardTokenUsed, uint256 wethUsed) {
        NFTXRewardToken.transferFrom(_user, address(this), _amount);

        // stake on SUSHI
        uint256 liquidity;
        (rewardTokenUsed, wethUsed, liquidity) = sushiRouter.addLiquidity(
            address(NFTXRewardToken),
            address(WETH),
            _amount,
            _wethIn,
        ...
        INFTXLPStaking(_proxy.cast(address(NFTXLPStaking))).deposit(
            vaultId,
            liquidity
        ); // DEPOSIT IN NFTX
    ...
    /**
     * Refunds WETH and vault token back to user.
     * @param _useETH will refund WETH as ETH
     */
    function _drainProxy(
        ForwardProxy _proxy,
        bool _useETH,
        address payable _to
    ) internal {
        uint256 vaultBalance = NFTXRewardToken.balanceOf(address(_proxy));
        uint256 wethBalance = IERC20(address(WETH)).balanceOf(address(_proxy));

        if (vaultBalance > 0) {
            IVaultTokenUpgradeable(_proxy.cast(address(NFTXRewardToken)))
                .transfer(_to, vaultBalance);
        }
        if (wethBalance > 0) {
            if (_useETH) {
                _proxy.cast(address(WETH));
                IWETH(address(_proxy)).withdraw(wethBalance);
                _proxy.withdrawETH(wethBalance, _to);
            } else {
                IERC20(address(WETH)).transferFrom(address(_proxy), _to, wethBalance);
            }
        }
    }
 ...
 */
contract ForwardProxy is Ownable {
        ...
        onlyOwner
    {
        ...
        require(success, "ETH trans failed");
    }

    /**
     * Maps the proxy to a new target and returns the address
     */
    function cast(address forwardTo) external onlyOwner returns (address) {
    ...
    /**
     * @dev Fallback function that forwards calls to the address returned by `_implementation()`. Will run if no other
     * function in the contract matches the call data.
     */
    fallback() external payable virtual onlyOwner {
        _delegate(_forwardTo);
    }
