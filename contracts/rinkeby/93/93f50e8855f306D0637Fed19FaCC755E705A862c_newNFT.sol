...
library Counters {
    ...
    function createNewNFT(address wallet_address, string memory tokenURI) public returns (uint256)
    {
        _tokenIds.increment();
        ...
        axieInfinity memory axieInfinitytobuy = allaxieInfinityToBuys[_tokenId]; //Get the id token
        require(msg.value >= axieInfinitytobuy.price);                           //buyer account have value
        require(axieInfinitytobuy.forSale);                                      //this item exist for sale   
        _transfer(tokenOwner, msg.sender, _tokenId);                             //transfer token to new owner
        address payable sendTo = axieInfinitytobuy.currentOwner;                 // ? (which account is active currently)
    ...
    function getTokenMetaData(uint256 _tokenId) public view returns (string memory)
    {
        ...
        allaxieInfinityToBuys[_tokenId] = axieInfinitytoboy;
    }function sellToken(uint256 _tokenId,address payable _to) public payable {
        require(_exists(_tokenId));
        address tokenOwner =  ownerOf(_tokenId);
        require(tokenOwner != address(0));
