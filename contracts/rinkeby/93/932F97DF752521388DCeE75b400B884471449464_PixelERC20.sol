...
contract PixelERC20 is ERC20, Ownable {
    ...
    function pendingReward(address user) internal view returns(uint256) {
        if (numOGTigers(user) > 0) {
            uint256 value1 = PixelTigers.numberOfLegendaries(user) * LEGENDARY_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START)) /86400 + PixelTigers.numberOfUniques(user) * UNIQUE_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START)) /86400 + numOGTigers(user) * OG_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START)) /86400;
            uint256 normalTigers = PixelTigers.ownerGenesisCount(user) - (PixelTigers.numberOfLegendaries(user) + PixelTigers.numberOfUniques(user) + numOGTigers(user));
            uint256 value2;
            if (normalTigers > 0) {
                //for those minted already (if), for those to be minted (else)
                if (minttime[user] == 0){
                    value2 = normalTigers * BASE_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START)) /86400;
                } else{ 
                    value2 = normalTigers * BASE_RATE * (block.timestamp - (lastUpdate[user] >= minttime[user] ? lastUpdate[user] : minttime[user])) /86400;
                }
            }
            return value1 + value2;
        } else{
            return PixelTigers.numberOfLegendaries(user) * LEGENDARY_RATE * (block.timestamp - (lastUpdate[user] >= minttime[user] ? lastUpdate[user] : minttime[user])) /86400 + PixelTigers.numberOfUniques(user) * UNIQUE_RATE * (block.timestamp - (lastUpdate[user] >= minttime[user] ? lastUpdate[user] : minttime[user])) /86400 + (PixelTigers.ownerGenesisCount(user) - (PixelTigers.numberOfLegendaries(user) + PixelTigers.numberOfUniques(user))) * BASE_RATE * (block.timestamp - (lastUpdate[user] >= minttime[user] ? lastUpdate[user] : minttime[user])) /86400;
        }
    }
