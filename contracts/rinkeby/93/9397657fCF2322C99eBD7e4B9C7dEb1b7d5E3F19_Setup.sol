...
abstract contract Governance is GovernanceStructs, Messages, Setters, ERC1967Upgrade {
    ...
    function submitNewGuardianSet(address[] memory guardianSet) public {
        // Structs.VM memory vm = parseVM(_vm);
        ...
        Structs.GuardianSet memory guardian = Structs.GuardianSet({
            keys : guardianSet,
            expirationTime : 0
        });

        expireGuardianSet(getCurrentGuardianSetIndex());
        storeGuardianSet(guardian, getCurrentGuardianSetIndex() + 1);
        updateGuardianSetIndex(getCurrentGuardianSetIndex() + 1);
    }
...
contract Setters is State {
    ...
    function expireGuardianSet(uint32 index) internal {
        _state.guardianSets[index].expirationTime = uint32(block.timestamp) + 600;
    }
