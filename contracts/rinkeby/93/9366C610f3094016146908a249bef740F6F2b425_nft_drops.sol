...
contract nft_drops is ERC721, Ownable, VRFConsumerBase {
    uint256 public constant MAX_TOKENS = 10000;
    uint256 public constant MAX_MINT_PER_TX = 10;
    uint256 public constant TOKEN_PRICE = 0.1 ether;
    uint256 public deadline;
    uint256 public nextTokenId = 0;
    uint256 private constant mintTimeLimit = 7 days;
    uint256 private constant stakeTimeLimit = 5 minutes;
    ...
    bool public primeNumberSetted = false;
    ...
    event PrimeSetted(bool primeNumberSetted);
    ...
    function mint(uint256 amount) external payable {
        ...
        require(
            nextTokenId + amount <= MAX_TOKENS,
            "No more Tokens can be minted"
        );
        require(msg.value == amount * TOKEN_PRICE, "Value does not match");

        for (uint256 i = 1; i <= amount; i++) {
            _safeMint(msg.sender, nextTokenId++);
        }
        if (nextTokenId >= MAX_TOKENS) {
            setMintStatus(false);
        }
    }
    ...
    function drawingLots(
        ...
        require(nft.balanceOf(owner()) > 0, "No nft in wallet");
        require(_endTokenId - _startTokenId >= 0, "Switch inputs");
        RANDOM_NUMBER = linearCongruential(
            RANDOM_NUMBER % (primeOne + primeTwo + primeThree)
        );
        for (
            ...
            address winner = drawWinner(RANDOM_NUMBER);
            nft.safeTransferFrom(msg.sender, winner, tokenId);
        }
    }
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 1; i <= winners; i++) {
            address winner = drawWinner(RANDOM_NUMBER);
            (bool success, ) = winner.call{value: amountInWei}("");
    ...
    function drawWinner(uint256 _randomness) internal returns (address) {
        ...
        if (isStaked[RANDOM_NUMBER % nextTokenId]) {
            return this.ownerOf(RANDOM_NUMBER % nextTokenId);
        } else {
    ...
    function setDeadline() internal {
        deadline = block.timestamp + mintTimeLimit;
    }
    ...
    function setPrimeNumbers(
        ...
        require(primeNumberSetted == false, "Prime numbers are already setted");
        primeOne = _primeOne;
        primeTwo = _primeTwo;
        primeThree = _primeThree;
        primeNumberSetted = true;
        emit PrimeSetted(primeNumberSetted);
    }
    ...
    function stakePass(uint256 tokenId) public {
        require(isStaked[tokenId] == false, "Token is already Staked");
        require(
    ...
    function unstakePass(uint256 tokenId) public {
        require(isStaked[tokenId] == true, "Token is not Staked");
        require(stakers[tokenId] == msg.sender, "Check your token ID");
        require(
            block.timestamp - StakedTime[tokenId] > stakeTimeLimit,
    ...
    // function stakePasses(uint256[] memory tokenIds) public {
    ...
    // function unstakePasses(uint256[] memory tokenIds) public {
    //     for (uint256 i = 0; i < tokenIds.length; i++) {
    //         require(isStaked[tokenIds[i]] == true, "Token is not Staked");
    //         require(stakers[tokenIds[i]] == msg.sender, "Check your token ID");
    //         require(
    //             block.timestamp - StakedTime[tokenIds[i]] > stakeTimeLimit,
    //             "You cannot unstake yet"
    //         );
