...
contract EBMarketSales is ERC721Holder, ERC1155Holder, Ownable, ReentrancyGuard, TokensHandler {
    ...
    event NewSale(uint indexed SaleID, address[] nftAddresses, uint[] nftIds, uint32[] nftTypes, uint price, address indexed currency);
    event Purchase(uint indexed SaleID, address[] nftAddresses, uint[] nftIds, uint32[] nftTypes, uint price, address indexed currency);
    ...
    function sell(address[] calldata _nftAddresses, uint[] calldata _nftIds, uint32[] calldata _nftTypes, uint _price, address _currency) external {
        ...
        emit NewSale(id,_nftAddresses,_nftIds,_nftTypes,_price,_currency);
        ++id;
    }
    ...
    function buy(uint saleId) external payable isAccesible(saleId) nonReentrant {
        ...
        emit Purchase(saleId,sales[saleId].nftAddresses,sales[saleId].nftIds,sales[saleId].nftTypes,sales[saleId].price,sales[saleId].currency);
