...
*/
contract SporesNFTMarket is Status, ReentrancyGuard {
    ...
    //  Market version
    bytes32 public constant VERSION = keccak256("MARKET_721");
    ...
    */
    constructor(address _registry) Status() {
    ...
    */
    function updateRegistry(address _newRegistry) external isActive onlyOwner {
        require(
    ...
    /**
       ...
       @param _sellId               A number of selling information (BE requirement)
    ...
    */
    function buyNFT721NativeCoin(
    ...
    ) external payable isActive nonReentrant {
        ...
        (uint256 _fee, uint256 _payToSeller) =
            _calcPayment(_price, SINGLE_UNIT, _feeRate);

        // transfer a payment to '_paymentReceiver' and Fee Collector
        _paymentTransfer(payable(_paymentReceiver), _payToSeller);
    ...
    */
    ...
    function _checkTradingAuthorization(
        address _seller,
        address _paymentReceiver,
        address _contractNFT,
        address _paymentToken,
...
contract Status is Ownable {
    ...
    modifier isActive() {
        require(status == STATUS.ACTIVE, "Status: Contract is deprecated");
        _;
    }
    ...
    /**
       @notice Change Status of a contract - Being Deprecated
       @dev  Caller must be Owner
    */
    ...
    /**
       @notice Change Status of a contract - Active
       @dev  Caller must be Owner
    */
