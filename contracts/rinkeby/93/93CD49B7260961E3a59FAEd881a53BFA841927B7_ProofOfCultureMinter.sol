...
contract ProofOfCultureMinter is Ownable {
  ...
  * @dev Gets the price up to 9999 tokens.
         If the token is a reservation token, then we take the price from one price tier higher.
  */
  function getPrice(bool isReservation) public view returns (uint256) {
    ...
    if (isReservation) {
      if (currentSupply == 9998) {
          return 9000000000000000000; // 9998 9 ETH
      } else if (currentSupply >= 9992) {
          return 9000000000000000000; // 9992 - 9997 9 ETH
      } else if (currentSupply >= 8493) {
          return 5000000000000000000; // 8493 - 9992 5 ETH
      } else if (currentSupply >= 6494) {
          return 2400000000000000000; // 6494 - 8493 2.4 ETH
      } else if (currentSupply >= 4995) {
          return 1900000000000000000; // 4995 - 6494 1.9 ETH
      } else if (currentSupply >= 3496) {
          return 1450000000000000000; // 3496  - 4995 1.45 ETH
      } else if (currentSupply >= 2497) {
          return 105000000000000000; // 2497 - 3496 1.05 ETH
      } else if (currentSupply >= 1498) {
          return 700000000000000000; // 1498 - 2497 0.7 ETH
      } else if (currentSupply >= 499) {
          return 400000000000000000; // 499 - 1498 0.4 ETH
      } else {
          return 200000000000000000; // 0 - 498 0.2 ETH 
      }
    } else {
      if (currentSupply == 9998) {
          return 9000000000000000000; // 9998 9 ETH
      } else if (currentSupply >= 9992) {
          return 5000000000000000000; // 9992 - 9997 5 ETH
      } else if (currentSupply >= 8493) {
          return 2400000000000000000; // 8493 - 9992 2.40 ETH
      } else if (currentSupply >= 6494) {
          return 1900000000000000000; // 6494 - 8493 1.90 ETH
      } else if (currentSupply >= 4995) {
          return 1450000000000000000; // 4995 - 6494 1.45 ETH
      } else if (currentSupply >= 3496) {
          return 1050000000000000000; // 3496  - 4995 1.05 ETH
      } else if (currentSupply >= 2497) {
          return 700000000000000000; // 2497 - 3496 0.7 ETH
      } else if (currentSupply >= 1498) {
          return 400000000000000000; // 1498 - 2497 0.4 ETH
      } else if (currentSupply >= 499) {
          return 200000000000000000; // 499 - 1498 0.2 ETH
      } else {
          return 100000000000000000; // 0 - 498 0.1 ETH 
      }
    }
  }

  /**
  * @dev Get discounted price of reservations if you buy in bulk.
  */
  function getDiscountedBulkPriceForReservations(uint256 count) internal view returns (uint256) {
    uint256 nonDiscountedPrice = getPrice(true).safeMul(count);
    if (count >= 25) {
      return nonDiscountedPrice.safeMul(65).safeDiv(100); // 35% off
    } else if (count >= 20) {
      return nonDiscountedPrice.safeMul(75).safeDiv(100); // 25% off
    } else if (count >= 15) {
      return nonDiscountedPrice.safeMul(80).safeDiv(100); // 20% off
    } else if (count >= 10) {
      return nonDiscountedPrice.safeMul(85).safeDiv(100); // 15% off
    } else if (count >= 5) {
      return nonDiscountedPrice.safeMul(90).safeDiv(100); // 10% off
    } else {
  ...
  *      Note that we will only use this once after individually crafting the image.
  ...
  */
  ...
  /**
  * @dev Mint up to 25 hashtags at once. We will try our best to mint all but
  ...
  */
  function reserve(address _dst, uint256 numberOfTokens) public payable {
    ...
    uint256 discountedPrice = getDiscountedBulkPriceForReservations(numberOfTokens);
    require(discountedPrice <= msg.value, "Ether value sent is not correct");
    ...
    treasury.transfer(discountedPrice);
    msg.sender.transfer(msg.value - discountedPrice);
  }
