...
contract Allowlist is Ownable {
  ...
  modifier whenNotOnAllowlist() {
    require(!allowlist[msg.sender], "RetroPhonesAllowlist: Already in the list");
    _;
  }

  modifier whenOnAllowlist() {
    require(allowlist[msg.sender], "RetroPhonesAllowlist: Not in the list");
    _;
  }
  ...
  function addAddressToAllowlist(address _addr) public whenAllowlistNotFull whenNotOnAllowlist returns(bool success) {
    if (!allowlist[_addr]) {
  ...
  function removeAddressFromWhitelist(address _addr) public whenAllowlistNotEmpty whenOnAllowlist returns(bool success) {
    if (allowlist[_addr]) {
      memberCount = memberCount - 1;
