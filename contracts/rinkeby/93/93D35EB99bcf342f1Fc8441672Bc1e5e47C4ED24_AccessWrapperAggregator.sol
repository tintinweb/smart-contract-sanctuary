...
// File: contracts-test/vendor/SafeMath.sol
...
// File: contracts-test/vendor/SignedSafeMath.sol
...
// File: contracts-test/vendor/Median.sol
...
// File: contracts-test/vendor/SafeMath128.sol
...
// File: contracts-test/vendor/SafeMath32.sol
...
// File: contracts-test/vendor/SafeMath64.sol
...
// File: contracts-test/interfaces/AggregatorInterface.sol
...
// File: contracts-test/interfaces/AggregatorV3Interface.sol
...
// File: contracts-test/interfaces/AggregatorV2V3Interface.sol
...
// File: contracts-test/interfaces/AggregatorValidatorInterface.sol
...
// File: contracts-test/interfaces/PhbTokenInterface.sol

// SPDX-License-Identifier: MIT
pragma solidity ^0.6.0;

interface PhbTokenInterface {
...
// File: contracts-test/OnchainAggregator.sol
 ...
 */
contract OnchainAggregator is AggregatorV2V3Interface, Owned {
  ...
  PhbTokenInterface public phbToken;
   ...
   * reserve of RESERVE_ROUNDS * oracleCount() PHB earmarked for payment to
   ...
   * @param _phb The address of the token
   * @param _paymentAmount The amount paid of PHB paid to each oracle per submission, in wei (units of 10⁻¹⁸ PHB)
  ...
  constructor(
    ...
    phbToken = PhbTokenInterface(_phb);
  ...
  /**
   * @notice recalculate the amount of PHB available for payouts
   */
    ...
    public
  {
    ...
    uint256 nowAvailable = phbToken.balanceOf(address(this)).sub(funds.allocated);
  ...
  /**
   * @notice query the available amount of PHB for an oracle to withdraw
   */
  ...
  /**
   * @notice transfers the oracle's PHB to another address. Can only be called
   * by the oracle's admin.
   * @param _oracle is the oracle whose PHB is transferred
   * @param _recipient is the address to send the PHB to
   * @param _amount is the amount of PHB to send
   */
    ...
    external
  {
    ...
    // Safe to downcast _amount because the total amount of PHB is less than 2^128.
    ...
    assert(phbToken.transfer(_recipient, uint256(amount)));
  }

  /**
   * @notice transfers the owner's PHB to another address
   * @param _recipient is the address to send the PHB to
   * @param _amount is the amount of PHB to send
   */
    ...
    onlyOwner()
  {
    ...
    require(phbToken.transfer(_recipient, _amount), "token transfer failed");
  ...
  /**
   * @notice called through PHB's transferAndCall to update available funds
...
// File: contracts-test/interfaces/AccessControllerInterface.sol
...
// File: contracts-test/WriteAccessController.sol
...
// File: contracts-test/ReadAccessController.sol
...
// File: contracts-test/AccessWrapperAggregator.sol
...
/**
 * @title AccessControlled OnchainAggregator contract
 * @notice This contract requires addresses to be added to a controller
 * in order to read the answers stored in the OnchainAggregator contract
 */
contract AccessWrapperAggregator is OnchainAggregator, ReadAccessController {

  /**
   * @notice set up the aggregator with initial configuration
   * @param _phb The address of the token
   * @param _paymentAmount The amount paid of TOKEN paid to each oracle per submission, in wei (units of 10⁻¹⁸ TOKEN)
   * @param _timeout is the number of seconds after the previous round that are
   * allowed to lapse before allowing an oracle to skip an unfinished round
   * @param _validator is an optional contract address for validating
   * external validation of answers
