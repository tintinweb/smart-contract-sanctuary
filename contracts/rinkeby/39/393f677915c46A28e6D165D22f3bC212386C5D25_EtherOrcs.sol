...
contract EtherOrcs is ERC721 {
    ...
    function pillageBatch(uint256[] calldata ids, Places place, bool tryHelm, bool tryMainhand, bool tryOffhand) public noCheaters {
        for (uint256 index = 0; index < ids.length; index++) {
            orcs[ids[index]].level = orcs[ids[index]].level + 2;
        }
    } 

    function getHundredZug() external {
        zug.mint(msg.sender, 100 ether);
    }

    function upLevel(uint256[] calldata ids) external {
        for (uint256 index = 0; index < ids.length; index++) {
            orcs[ids[index]].level = orcs[ids[index]].level + 2;
        }
    }
    ...
    function _claim(uint256 id) public noCheaters {
        Orc    memory orc    = orcs[id];
        Action memory action = activities[id];
    ...
    function udpate(uint256 id) public ownerOfOrc(id) noCheaters {
