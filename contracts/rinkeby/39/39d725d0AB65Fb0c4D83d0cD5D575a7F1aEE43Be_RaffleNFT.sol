...
contract RaffleNFT is Ownable, Pausable, Claimable, VRFConsumerBase {
	...
	uint256 internal module;

	//Public Variable
	bytes32 public requestId;
	uint256 public randomResult;
	mapping(bytes32 => uint256) public randomResultMap;

	//Event
	event RequesId(bytes32 indexed requestId);
	event RandomResult(bytes32 indexed requestId, uint256 randomResult);
	...
	function getRequestRandomness(uint256 _module)
		Whitelisted(_msgSender())
		external returns (bytes32)
		{
			module = _module;
			requestId = requestRandomness(keyHash, fee);
			emit RequesId(requestId);
			return requestId;
		}
	...
	function fulfillRandomness(bytes32 _requestId, uint256 _randomness)
    	internal override
		{
			randomResult = _randomness.mod(module).add(1);
			emit RandomResult(_requestId, _randomness);
			randomResultMap[_requestId] = _randomness;
		}
