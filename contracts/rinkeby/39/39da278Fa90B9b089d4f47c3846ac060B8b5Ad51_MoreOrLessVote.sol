...
contract MoreOrLessVote is ERC721Enumerable, Ownable {
    ...
    uint16 public constant maxSupply = 10; // add 1 (so 1000) to account for mint 0
    ...
    Vote[maxSupply + 1] public voteInfos; // add 2, so 1001 (token 0 and token 1000)
    MoreOrLessArt.Art[maxSupply + 1] public artInfos; // add 2, so 1001 (token 0 and token 1000)
