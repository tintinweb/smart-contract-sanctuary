...
contract BarnyardFashionistasStore is EIP712, Ownable {
    ...
    function addToBloom(address _address) external onlyOwner {
        filter.add( bytes32(uint256(uint160(_address)) << 96) );
    }
    ...
    //Presale function - remove pass, add whitelist functions from pass
    function mintWithBloom(
        uint256 _amount
    ) external payable presaleOpened{

        uint256 _whitelistAmount = 4;
        ...
        // *** Instead of the whitelist - check the bloom here

        bool mayParticipate = filter.check( bytes32(uint256(uint160(msg.sender)) << 96) );
        require( mayParticipate, "Not a member of presale");
