...
contract MidoNFTSale is IMidoTokenSale, Whitelist {
    ...
    MidoNFT public immutable midoNFT;

    SaleRecord internal _saleRecord;
    SaleConfig[] internal _saleConfigList;
    ...
    function newSale(
        ...
        _saleConfigList.push(SaleConfig({
            whitelistTime: _whitelistTime,
            publicTime: _publicTime,
            endTime: _endTime,
            maxSupply: _maxSupply
        }));
        currentId=uint64(_saleConfigList.length-1);
        midoNFT.increaseTotalSupply(_maxSupply);
    }
    /**
     * @dev Return the config, with times (whitelistTime, publicTime, endTime) and max supply
     * @param _index is index of a sale config list
     */
    function getSaleConfig(uint64 _index) external view returns(SaleConfig memory config) {
        config = _saleConfigList[_index];
    } 
    ...
    function _validateAndUpdateWithBuyAmount(address buyer, uint64 amount) internal {
        SaleConfig memory config = _saleConfigList[currentId];
        ...
        if (config.whitelistTime <= timestamp && timestamp < config.publicTime) {
            ...
            require(totalPaid >= amount * SALE_PRICE, 'invalid paid value');
            require(
