...
import "./interfaces/INounsPartyVault.sol";
	...
	UUPSUpgradeable
{
	...
	function initialize(INounsAuctionHouse _nounsAuctionHouse, uint256 _nounsAuctionHouseBidIncrease)
		external
		initializer
	{
		__Ownable_init();
	...
	function setENSName(IENS _ens, string memory _name) external onlyOwner {
		_ens.setName(_name);
	}

	function createBid() public payable nonReentrant whenNotPaused {
		(uint256 nounId, uint256 amount) = calcBidAmount();
		currentBidAmount = amount;
		emit BidCreated(nounId, currentBidAmount, msg.sender);
	...
	function calcBidAmount() public view returns (uint256 _nounId, uint256 _amount) {
		...
		require(auction.nounId > 0, "zero noun");
		require(!auction.settled, "Inactive auction");
		require(auction.bidder != address(this), "Already winning");

		uint256 newBid = _round_eth1_1000(
			auction.amount + ((auction.amount * 1000 * (bidIncrease + nounsAuctionHouseBidIncrease)) / uint256(1000000))
		);
		...
		uint256 minBid = auction.amount + ((auction.amount * 1000 * nounsAuctionHouseBidIncrease) / uint256(1000000));
		...
		// We can't bid more than the wallet's balance.
		require(address(this).balance >= newBid, "Insufficient balance");

		return (auction.nounId, newBid);
	}

	/// @dev round to next 0.001 ETH increment
	// solhint-disable-next-line func-name-mixedcase
	function _round_eth1_1000(uint256 _amount) private pure returns (uint256) {
		return _amount - (_amount % ETH1_1000);
	}

	function _authorizeUpgrade(address _newImplementation) internal override onlyOwner {} // solhint-disable-line no-empty-blocks
...
interface INounsPartyVault {
	event BidCreated(uint256 indexed nounId, uint256 amount, address sender);
}
