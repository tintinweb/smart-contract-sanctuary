...
contract FishingGameNFTFixedPriceSale is Context, Ownable {
    ...
    function purchaseNFT(uint256 tokenId, string memory symbol, uint256 amount) public payable {
        address contractAddress = _tokenContracts[symbol];
        require(contractAddress != address(0x0), "Err: Symbol-Does-Not-Supported-Yet");
        ...
        if (compareStrings("ETH", symbol) || compareStrings("BNB", symbol) || compareStrings("MATIC", symbol)) {
            require(amount == 0 && msg.value == model.unitPrice, "Err: Amount-Not-Match-UnitPrice");
        } else {
            require(msg.value == 0 && amount == model.unitPrice, "Err: Amount-Not-Match-UnitPrice");
        }

        _nftContract.safeTransferFrom(_nftContract.ownerOf(tokenId), _msgSender(), tokenId);

        emit PurchaseNFTEvent(_msgSender(), tokenId, symbol, amount);
    }
