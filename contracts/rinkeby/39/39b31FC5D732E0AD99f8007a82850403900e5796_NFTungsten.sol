...
contract NFTungsten is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
  ...
  uint8[] public availableTokens;
  mapping(uint8 => bool) tokenExists;

  uint public constant tokenPrice = 740000000000000000; //0.74 ETH
  uint public constant maxTokens = 74;
  uint public constant maxMint = 4;
  ...
  function safeMint(uint mintQuantity) public payable {
    require(totalSupply() < maxTokens, "All tokens have been minted");
    require((totalSupply() + mintQuantity) <= maxTokens, "Cannot mint more than the maxTokens available, adjust mint quantity");
    require(mintQuantity <= maxMint, "Cannot exceed maxMint quantity");
