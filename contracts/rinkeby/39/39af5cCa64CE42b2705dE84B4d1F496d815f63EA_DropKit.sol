...
contract DropKit is IDropKit, Initializable, OwnableUpgradeable {
    ...
    function createDropKitCollection(
        ...
        dropKit.initialize(name, symbol, maxAmount);
    ...
    function addFeesClaimed(uint256 amount) external override {
        ...
        if (_partners[_referrals[_msgSender()]].isValue) {
            ...
            uint256 partnerFee = partition(_partners[partner].value, amount);
    ...
    function commission(uint256 amount) public view override returns (uint256) {
        ...
        return partition(rate, amount);
    }

    function partition(uint256 rate, uint256 amount)
        public
        pure
        returns (uint256)
    {
    ...
    OwnableUpgradeable
{
    ...
    modifier onlyMintable(uint256 numberOfTokens) {
        require(numberOfTokens <= _maxPerMint, "Exceeded max");
        require(numberOfTokens > 0, "Greater than 0");
        require(_mintCount[_msgSender()] <= _maxPerWallet, "Exceeded max");
        require(
    ...
    function batchAirdrop(
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            airdrop(numberOfTokens[i], recipients[i]);
        }
    }
