...
contract BitmindCrowdsale {
  ...
  /**
    ...
    /**
     * Event for Pause Increasing Price
     * @param owner : Address who can pause increasing price of Crowdsale contract
     * @param time : time when changeOwner function executed
     */
    event PauseIncreasingPrice(address indexed owner, uint256 time);
    
    /**
     * Event for Unpause Increasing Price 
     * @param owner : Address who can unpause increasing price of Crowdsale contract
     * @param time : time when changeOwner function executed
     */
    event UnpauseIncreasingPrice(address indexed owner, uint256 time);
    ...
    function changeToken(uint256 amount) public onlyOwner {
        if(amount == 0){
            token_available = cap;
        }else{
            token_available = amount;
        }
    }
    ...
    function Purchase(uint256 amount) external onlyWhileOpen returns(bool){
        ...
        total_received = total_received.add(amount.div(1e18));
        crowdsaleInfo[crowdsaleid.length].total_received = total_received.add(amount.div(1e18));
    ...
    function getPrice() public view returns(uint256){
        ...
        if(token_available >= cap.sub(cap.mul(40).div(100))){
            return price;
        }else if(token_available >= cap.sub(cap.mul(70).div(100))){
            return price.add(rate.mul(1));
        }else if(token_available >= cap.sub(cap.mul(90).div(100))){
            return price.add(rate.mul(2));
        }else if(token_available > cap.sub(cap.mul(100).div(100))){
            return price.add(rate.mul(3));
        }else{
            return price.add(rate.mul(3));
        }
    }
