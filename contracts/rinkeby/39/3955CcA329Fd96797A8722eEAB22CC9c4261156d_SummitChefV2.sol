...
contract SummitChefV2 is BoringOwnable, BoringBatchable,ReentrancyGuard {
...
interface IRewarder {
    using BoringERC20 for IERC20;
    function onSummitReward(uint256 pid, address user, address recipient, uint256 summitAmount, uint256 newLpAmount) external;
    function pendingTokens(uint256 pid, address user, uint256 summitAmount) external view returns (IERC20[] memory, uint256[] memory);
}
...
interface IMasterChef {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. SUMMIT to distribute per block.
