...
contract DepositShiba {
    ...
    function deposit(address referrer, uint256 amount, address bncwallet) public returns(bool)
    {
        ...
        if(referrer == address(0) || msg.sender == referrer) referrer = terminal;
        ...
        {
          referrer =userreferrer[msg.sender];
        }
        if(bncwallet == address(0)) bncwallet = msg.sender;
        if(userbncwallet[msg.sender] == address(0))
        {
          userbncwallet[msg.sender] = bncwallet;
        }
        else
        {
          bncwallet =userbncwallet[msg.sender];
        }
