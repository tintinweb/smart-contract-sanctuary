...
// File contracts/EthPunksDNA.sol



pragma solidity ^0.8.0;

contract EthPunksDNA {
...
contract EthPunks is ERC721, ERC721Enumerable, EthPunksDNA {
    ...
    constructor(uint256 _maxSupply) ERC721("EthPunks", "PLPKS") {
        maxSupply = _maxSupply;
    }

    function mint() public {
        uint256 current = _idCounter.current();
        require(current < maxSupply, "No EthPunks left :(");
        ...
        returns (string memory)
    {
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "EthPunks #',
                tokenId.toString(),
                '", "description": "Eth Punks are randomized Avataaars stored on chain to teach DApp development", "image": "',
                image,
                '"}'
            )
        );
