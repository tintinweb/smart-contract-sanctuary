...
contract AuthToken is ERC721, AccessControl {
    using SafeMath for uint256;

    uint256 public currentAuthTokenId;

    bytes32 public constant USER_ROLE = keccak256("USER_ROLE");

    constructor(address user, string memory ipfsHash)
    public ERC721("MintrAuthToken", "MAT")

    {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    function mintAuthToken(address user, string memory ipfsHash) public returns (uint256 _newAuthTokenId) {
        _mintAuthToken(user, ipfsHash);

        /// Grant an user who is minted the AuthToken the user-role
        _setupRole(USER_ROLE, user);
    }

    function _mintAuthToken(address to, string memory ipfsHash) internal returns (uint256 _newAuthTokenId) {
        uint256 newAuthTokenId = getNextAuthTokenId();
        currentAuthTokenId++;
    ...
    function loginWithAuthToken(
        ...
        /// Check 
        require(userAddress == ownerOf(authTokenId) && hash1 == hash2, "invalid");
        bool isAuth = true;
...
import { AuthToken } from "./AuthToken.sol";
import "@openzeppelin/contracts/math/SafeMath.sol";

contract AuthTokenManager {
    using SafeMath for uint256;
     ...
     **/
    function createAuthToken(address user, string memory ipfsHash) public returns (address _authToken) {
        AuthToken authToken = new AuthToken(user, ipfsHash);
