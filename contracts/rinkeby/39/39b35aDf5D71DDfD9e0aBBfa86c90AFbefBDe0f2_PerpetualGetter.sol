...
interface IPerpetualGetter {
    ...
    function getAMMState(bytes32 _iPerpetualId) external view 
        returns (int128[10] memory);

    function getTraderState(bytes32 _iPerpetualId, address _traderAddr) external view
        returns (int128[6] memory);
...
contract PerpetualGetter is PerpetualRebalanceFunctions, IFunctionList, IPerpetualGetter {
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;
    using ABDKMath64x64 for int128;
    ...
    function getPerpetualCountInPool(uint256 _poolId) external view override returns (uint256) {
        return perpetualIds[_poolId].length;
    }

    function getPerpetualId(uint256 _poolId, uint256 _perpetualIndex) external view override returns (bytes32) {
        return perpetualIds[_poolId][_perpetualIndex];
    }

    function getPoolIdByPerpetualId(bytes32 _perpetualId) external view override returns (uint256) {
        return perpetualPoolIds[_perpetualId];
    }

    function getLiquidityPool(uint256 _id) external view override returns (LiquidityPoolData memory) {
    ...
    function getCheckpoints(uint256 _poolId, address _account) external view override returns (Checkpoint[] memory) {
    ...
    function getAMMState(bytes32 _iPerpetualId) external view override 
        returns (int128[10] memory)
    {
        int128[10] memory ammState;
        ...
        ammState[6] = perpetual.indexS2PriceData.fPrice;
        ammState[7] = perpetual.indexS3PriceData.fPrice;
        ammState[8] = perpetual.currentPremiumEMA.fPrice;
        ammState[9] = perpetual.currentPremium.fPrice;
        return ammState;
    }

    function getTraderState(bytes32 _iPerpetualId, address _traderAddr) external view override
        returns (int128[6] memory)
    { 
        /*
        ...
        */
        ...
        int128[6] memory traderState;
        PerpetualData memory perpetual = _getPerpetual(_iPerpetualId);
        
        traderState[0] = _getMarginBalance(perpetual, _traderAddr);
        traderState[1] = _getAvailableMargin(perpetual, _traderAddr, true); 
    ...
    function getFunctionList() external view override returns (bytes4[] memory) {
        bytes4[] memory functionList = new bytes4[](20);
        ...
        functionList[15] = this.isTraderMarginSafe.selector;
