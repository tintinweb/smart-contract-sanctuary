...
contract PixelERC20 is ERC20, Ownable {
    ...
    function numTigersRewardsUnclaimed(address user) internal view returns(uint256) {
    ...
    function claimReward() external {
        ...
        _mint(msg.sender, 20 ether * numTigersRewardsUnclaimed(msg.sender) + pendingReward(msg.sender) + storeRewards[msg.sender]);
    ...
    function pendingReward(address user) internal view returns(uint256) {
        if (PixelTigers.numberOfLegendaries(user)>0){
            ...
            return (PixelTigers.ownerGenesisCount(user)- PixelTigers.numberOfUniques(user))* BASE_RATE * (block.timestamp - (lastUpdate[user] >= minttime[user] ? lastUpdate[user] : minttime[user])) /86400 + PixelTigers.numberOfUniques(user)* UNIQUE_RATE * (block.timestamp - (lastUpdate[user] >= minttime[user] ? lastUpdate[user] : minttime[user])) /86400;
        } else{
