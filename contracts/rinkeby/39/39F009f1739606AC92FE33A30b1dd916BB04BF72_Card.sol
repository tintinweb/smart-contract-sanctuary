...
contract Card is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string
            memory prefix = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: black; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="white" />';

        uint8 lines = 8;
        for (uint8 i = 0; i < lines; i = i + 1) {
            uint256 offset = (i + 1) * 20;
            parts[i] = string(
                abi.encodePacked(
                    '<text x="10" y="',
                    toString(offset),
                    '" class="base">',
                    getCommonMonster(tokenId, i),
                    "</text>"
                )
            );
        }
        ...
        output = string(abi.encodePacked(prefix, output, "</svg>"));
