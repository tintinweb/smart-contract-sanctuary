...
contract SimpleCollectible is 
    ...
    function generateMetadata(uint256 tokenId) public view returns (string memory) {
        string memory totalGasUsed = string(abi.encodePacked(gas[tokenId].totalGasUsed));
        string memory gasLimit = string(abi.encodePacked(gas[tokenId].gasLimit));
        string memory baseFee = string(abi.encodePacked(gas[tokenId].baseFee));
