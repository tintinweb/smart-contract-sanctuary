...
contract BigFanMinter is ERC721URIStorage, Ownable {
    ...
    modifier isApprovedTokenURI(string memory tokenURI) {
        require(approvedUris[tokenURI].approved == true, "The token is not approved");
        require(approvedUris[tokenURI].minted == false, "The token has been minted");
        _;
    }

    function approveTokenURI(string memory tokenURI) public onlyOwner  returns(bool success) {
        approvedUris[tokenURI].approved = true;
        approvedUris[tokenURI].minted = false;
        return true;
    }

    function startDrop(string memory name, uint256 price) public onlyOwner returns(bool success) {
        drops[name].active = true;
        drops[name].token_price = price;
        return true;
    }

    function pauseDrop(string memory name) public onlyOwner returns(bool success) {
