...
contract AwooFinance is Context, IERC20, IERC20Metadata, Ownable {
    ...
    uint256 private _totalCharity;
    uint256 private _totalOp;
    ...
    function feeOf(address account) private view returns (uint256) {
        uint256 userBalance = _balances[account];

        uint256 rSupply =
            _totalSupply.sub(_totalFee).sub(_totalCharity).sub(_totalOp);

        uint256 feeOfUser =
            userBalance.mul(10**6).mul(_totalFee).div(rSupply).div(10**6);
    ...
    function balanceOf(address account)
        ...
        returns (uint256)
    {
        if (account == address(this)) return _balances[account];
        uint256 feeOfUser = feeOf(account);
    ...
    function _tokenTransfer(
        ...
        _totalCharity = _totalCharity.add(tCharity);
        _balances[address(this)] = _balances[address(this)].add(tOp);
        _totalOp = _totalOp.add(tOp);
