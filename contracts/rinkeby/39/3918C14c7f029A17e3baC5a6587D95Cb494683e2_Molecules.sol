...
//    }
    //    bool ready;
//    event RegisterMolecule(bytes32 indexed hash, uint8[] composition);
//    event CreateMolecule(bytes32 indexed hash, uint tokenId);
...
//    function initMolecules(uint8[][] calldata composition, string[] calldata formula) public onlyOwner {
//        require(composition.length == formula.length,"mismatch");
//        for(uint i = 0; i < composition.length; i++){
//            configs.push(Molecule(composition[i],formula[i]));
//        }
//    }
    function initMolecules(uint8[][] calldata composition) public onlyOwner {
        for(uint i = 0; i < composition.length; i++){
//            bytes32 _configId = keccak256(abi.encodePacked(composition[i]));
            configs[configId(composition[i])] = toFixedLength(composition[i]);
//            emit RegisterMolecule(_configId,composition[i]);
        }
    }
...
//        require(configs[ keccak256(abi.encodePacked(_config))].length != 0,"Molecule does not exist");
        require(configs[ configId(_composition)][0] != 0,"Molecule does not exist");
        return _getFormula(_composition);
    }

    function readConfig(uint8[] calldata _composition) public view returns(uint8[32] memory){

        return configs[ configId(_composition)];
//        require(configs[ keccak256(abi.encodePacked(_config))].length != 0,"Molecule does not exist");
//        require(configs[ configId(_composition)][0] != 0,"Molecule does not exist");
//        return _getFormula(_composition);
    }
...
//    function getConfig(uint _index) public view returns( uint8[] memory composition, string memory formula){
//        require(_index < configs.length,"exists");
//
//        return (configs[_index].composition,configs[_index].formula);
//    }
    ...
    function myAtoms(uint start_index, uint limit) public view returns(uint[] memory tokenIds, uint8[] memory atomicNumbers){
        ...
        if(balance == 0){
            ...
            return (__tokenIds,__atomicNumbers); //todo
        }
        require(start_index < balance,"Invalid start index");
        uint sampleSize = balance - start_index;
        if(limit != 0 && sampleSize > limit){
...
//        require(_atomIds.length == configs[_config].composition.length,"mismatch");
//        uint8[] memory _config;// = uint8[](_atomIds.length); //TODO:
...
//            require(atomReader.getAtomicNumber(_atomIds[i]) == configs[_config].composition[i],"ingredient");
        }
