...
contract AstronautFactory is FactoryERC1155, Ownable, ReentrancyGuard {
    ...
    uint256 internal numberofOptions = 1;

    modifier onlyOwnerOrMaster() {
        require(owner() == _msgSender() || master == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
    ...
    function factorySchemaName() override external view returns (string memory) {
        return "ERC1155";
    }
    ...
    function uri(uint256 _optionId) override external view returns (string memory) {
       return bytes(baseMetadataURI).length > 0 ? string(abi.encodePacked(baseMetadataURI, _optionId.toString())): "";
    }
    ...
    function _mint(
        ...
        bytes memory _data
            ...
            require(
                _isOwnerOrProxy(_msgSender()), "Caller cannot mint");

            require((_amount > 1 && maxSupply[_option] > 1 && _amount <= maxSupply[_option]) || (_amount == 1 && maxSupply[_option] == 0));

            ERC1155Astronauts astros = ERC1155Astronauts(nftAddress);
            if(_amount > 1){
                astros.mint(_toAddress, _option, _amount, _data);
            }
            else
            {
                uint256 _tokenIdTracker = astros.totalSupply();
                astros.mint(_toAddress, _tokenIdTracker, 1, _data);
            }
    }

    function setMaxSupply(
        uint256 _tokenId, uint256 _supply) public onlyOwnerOrMaster {
        require(_supply > 1);
        maxSupply[_tokenId] = _supply;
    }
    function getMaxSupply(
        uint256 _tokenId) external view returns(uint256) {
    ...
    {
        ERC1155Astronauts astros = ERC1155Astronauts(nftAddress);
        return astros.balanceOf(_owner, _tokenId);
    ...
    function setOptionURI(string memory _newURI) public onlyOwnerOrMaster {
        baseMetadataURI = _newURI;
    }

    function totalSupply() public view returns (uint256) {
        ERC1155Astronauts astros = ERC1155Astronauts(nftAddress);
        return astros.totalSupply();
    }

    function withdraw() public onlyOwnerOrMaster {
        uint256 balance = address(this).balance;
        require(balance > 0);
        _widthdraw(master, address(this).balance);
    }
...
contract ERC1155Astronauts is ContextMixin, ERC1155Burnable, ERC1155Pausable, Ownable, NativeMetaTransaction {
    using Strings for uint256;
    using SafeMath for uint256;
    ...
    function mintBatch(
        ...
        bytes memory data
    ) public virtual onlyOwnerOrMaster {
     ...
     */
    function pause() public virtual onlyOwnerOrMaster {
        _pause();
    }
     ...
     * - the caller must have the `PAUSER_ROLE`.
     */
    function unpause() public virtual onlyOwnerOrMaster {
        _unpause();
    }
    ...
    function setURI(string memory _newURI) public onlyOwnerOrMaster {
        _setURI(_newURI);
    }

    function setCustomURI(
        uint256 _tokenId,
        string memory _newURI) public onlyOwnerOrMaster {
        customUri[_tokenId] = _newURI;
    }

    function resetCounter(
        uint256 _count) public onlyOwnerOrMaster {
