...
contract UserDepWith {
	...
	function invest(uint amount) public payable returns (string memory)  {
	    require(amount > 0, 'Zero amount');
	    User storage user = users[msg.sender];
	    if(user.addr != msg.sender) {
	        users[msg.sender] = User(msg.sender , amount, 0, block.timestamp);
	    } else {
	        user.investment_amt = user.investment_amt + amount;
	    }
