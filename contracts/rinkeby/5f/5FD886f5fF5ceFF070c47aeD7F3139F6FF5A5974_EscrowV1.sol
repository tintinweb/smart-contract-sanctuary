...
contract EscrowV1 is EscrowContract, AmendablePreSigned, WithPreSignedMilestones {
    ...
    function executeSettlement(bytes32 _cid, uint16 _index) external nonReentrant {
        ...
        if (_index > MILESTONE_INDEX_BASE) {
            ...
            _disputer = _m.escrowDisputeManager;
        ...
        if (_ruling == RULE_PAYER_WON || _ruling == RULE_PAYEE_WON) {
            ...
            uint256 _refundDelta = 0;
            if (_refundedAmount > _m.refundedAmount) _refundDelta = _refundedAmount - _m.refundedAmount;
            uint256 _releaseDelta = 0;
            if (_releasedAmount > _m.releasedAmount) _releaseDelta = _releasedAmount - _m.releasedAmount;

            address _arbiter = _disputer.ARBITER();
            if (_refundedAmount > 0) _cancelMilestone(_mid, _refundedAmount, _refundDelta, _arbiter);
            if (_releasedAmount > 0) _releaseMilestone(_mid, _releasedAmount, _releaseDelta, _arbiter);
        }
    }
