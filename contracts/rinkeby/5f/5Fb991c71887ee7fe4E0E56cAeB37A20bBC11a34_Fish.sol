...
contract Fish is ERC721, Ownable {
  ...
  mapping(uint256 => uint256) public UpgradesToId;
  ...
  using SafeMath for uint256;
    uint public constant MAX_Fish = 10000;
    ...
    function calculatePriceForToken(uint _id) public view returns (uint256) {
        require(_id < MAX_Fish, "Sale has already ended");
   ...
   function adoptFish(uint256 numFish) public payable {
        require(totalSupply() < MAX_Fish, "Sale has already ended");
        require(numFish > 0 && numFish <= 20, "You can adopt minimum 1, maximum 20 fish");
        require(totalSupply().add(numFish) <= MAX_Fish, "Exceeds max number of Fishes");
        require(msg.value >= calculatePrice().mul(numFish), "Ether value sent is below the price");

        for (uint i = 0; i < numFish; i++) {
	...
	function calculatePriceForUpgrade() public view returns (uint256) {
        require(hasSaleStarted == true, "Sale hasn't started");
	...
	function addExtra(uint256 id, uint256 numUpgrades) public payable {
	 
	 require(msg.value >= calculatePriceForUpgrade().mul(numUpgrades), "Not enough ether sent for upgrade");
     UpgradesToId[id] = UpgradesToId[id] + 10;
	 
	}
	
	
    function getExtra(uint256 id) public view returns (uint256)  {
	 
	 
     return UpgradesToId[id];
	 
	}
	

    function reserveGiveaway(uint256 numFish) public onlyOwner {
        uint currentSupply = totalSupply();
        require(totalSupply().add(numFish) <= 30, "Exceeded giveaway supply");
