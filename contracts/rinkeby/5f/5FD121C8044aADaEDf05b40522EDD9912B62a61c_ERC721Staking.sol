...
contract ERC721Staking is StakingBase, IERC721Receiver {
    ...
    function stake(uint256 pid, uint256 id) external existPool(pid) {
        _stake(pid, id);
    }

    function stakeAll(uint256 pid, uint256[] calldata ids) external existPool(pid) {
    ...
    function stakeAll(uint256 pid) external existPool(pid) {
        ...
        for (uint256 indexAt = 0; indexAt < userBalance; indexAt++) {
            _stake(pid, IERC721Enumerable(pool.token).tokenOfOwnerByIndex(msg.sender, indexAt));
        }
    }

    function unstake(uint256 pid, uint256 id) external existPool(pid) {
        _unstake(pid, id);
    }

    function unstakeAll(uint256 pid) external existPool(pid) {
        ...
        for (uint256 indexAt = 0; indexAt < totalTokens; indexAt++) {
            _unstake(pid, user.getTokenIdAt(indexAt));
        }
    }

    function unstakeAll(uint256 pid, uint256[] memory ids) external existPool(pid) {
        require(ids.length > 0, "TOKEN_IDS_REQUIRED");
        for (uint256 indexAt = 0; indexAt < ids.length; indexAt++) {
...
abstract contract StakingBase is Base, IStaking {
    ...
    function setAllocPoint(
        ...
        bool withUpdate
    ) external onlyOwner(msg.sender) existPool(pid) {
        if (withUpdate) {
            _massUpdatePools();
        }
    ...
    function emergencyUnstakeAll(uint256 pid) external existPool(pid) {
        PoolInfoLib.PoolInfo storage pool = poolInfo[pid];
        UserInfoLib.UserInfo storage user = userInfo[pid][msg.sender];
    ...
    function _stake(uint256 pid, uint256 amountOrId) internal {
        ...
        pool.totalDeposit = pool.totalDeposit.add(valuedAmountOrId);
    ...
    function _unstake(uint256 pid, uint256 amountOrId) internal {
        ...
        pool.totalDeposit = pool.totalDeposit.sub(valuedAmountOrId);
