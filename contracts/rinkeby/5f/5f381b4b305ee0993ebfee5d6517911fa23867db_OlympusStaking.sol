...
interface IWarmup {
    function send( address staker_, uint amount_ ) external;
}
...
contract OlympusStaking is Ownable {
    ...
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        IERC20( sOHM ).safeTransfer( warmup, _amount );
        return true;
    }
    ...
    function claim ( address _recipient ) public {
        ...
        if ( epoch.number >= info.expiry ) {
            ...
            IWarmup( warmup ).send( _recipient, IsOHM( sOHM ).balanceForGons( info.gons ) );
        }
    }
    ...
    function forfeit() external {
        ...
        IWarmup( warmup ).send( address(this), IsOHM( sOHM ).balanceForGons( info.gons ) );
    ...
    function setContract( CONTRACTS _contract, address _address ) external onlyManager() {
        if( _contract == CONTRACTS.DISTRIBUTOR ) { // 0
            ...
            require( warmup == address( 0 ), "Warmup cannot be set more than once" );
            warmup = _address;
