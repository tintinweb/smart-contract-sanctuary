...
contract BrainDanceNft is ERC721, Ownable, HeroFactory {
    ...
    // 0: not started
    // 1: in ticket sale
    // 2: in presale (4 hours)
    // 3: in public sale (24 hours)
    // 4: in public sale
    // 5: sale ended
    // 6: stopped
    ...
    uint256 public presaleTokenLimit = 2500;
    ...
    function mintWhitelist(uint256 sign, uint256 count) public payable {
        require(statusFlag == 2 && verifySignature(sign), "Paused or permission error");
        require(count >= 1 && count <= 3, "count error");
        require(msg.value >= mintPrice * count, "Value below price");

        uint256 balance = balanceOf(msg.sender);
        uint256 realcount = count;
        if (realcount > 3 - balance) {
            realcount = 3 - balance;
        }

        uint256 availablePublicToken = INITIAL_TOKEN_COUNT - mintedInitialTokenCount;
        if (realcount > availablePublicToken) {
            realcount = availablePublicToken;
        }
        ...
        mintedInitialTokenCount += realcount;

        if (count > realcount) {
            _widthdraw(msg.sender, (count - realcount) * mintPrice);
        }
        emit MintedNewNFT(mintedInitialTokenCount, realcount);
    }
    
    function mint(uint256 count) public payable {
        require(statusFlag == 3 || statusFlag == 4, "Paused");
        require(count >= 1 && count <= 3, "count error");
        require(msg.value >= mintPrice * count, "Value below price");
        ...
        if (statusFlag == 3) {
            uint256 i = 0;
            uint256 temp = realcount < (tickets[msg.sender] - balance) ? realcount : (tickets[msg.sender] - balance);
            if (temp > 0) {
                for (; i < temp; i++) {
                    _mintHero(mintedInitialTokenCount + i);
                    _safeMint(msg.sender, mintedInitialTokenCount + i);
                }
                presaleTokenCount += temp;
                mintedInitialTokenCount += temp;
            } else {
                temp = 0;
            }

            uint256 unsoldTicket = presaleReservedTokenCount - presaleTokenCount;
            uint256 availablePublicToken = INITIAL_TOKEN_COUNT - mintedInitialTokenCount - unsoldTicket;
            if (realcount > availablePublicToken + temp) {
                realcount = availablePublicToken + temp;
            }
            for (; i < realcount; i++) {
                _mintHero(mintedInitialTokenCount + i);
                _safeMint(msg.sender, mintedInitialTokenCount + i);
            }
            mintedInitialTokenCount += (realcount - temp);
        } else {
            uint256 availablePublicToken = INITIAL_TOKEN_COUNT - mintedInitialTokenCount;
            if (realcount > availablePublicToken) {
                realcount = availablePublicToken;
    ...
    function mintBreedToken(
        ...
        require(statusFlag != 6, "Breed Paused");
    ...
    function buyTicket(uint256 count) public payable {
        ...
        require(msg.value >= ticketPrice * count, "Value below price");
        require(count >= 1 && count <= 3, "count error");

        uint256 balance = tickets[msg.sender];
        uint256 realcount = count;
        if (balance + realcount > 3) {
            realcount -= (balance + count - 3);
        }

        uint256 end = presaleReservedTokenCount + realcount;
        if (end > presaleTokenLimit) {
            realcount -= (end - presaleTokenLimit);
            end = presaleTokenLimit;
        }

        presaleReservedTokenCount = end;
        tickets[msg.sender] = tickets[msg.sender] + uint8(realcount);

        if (count > realcount) {
            _widthdraw(msg.sender, (count - realcount) * ticketPrice);
        }
    }
    ...
    function setPresaleTokenLimit(uint256 limit) external onlyStarOwner {
        presaleTokenLimit = limit;
    }
