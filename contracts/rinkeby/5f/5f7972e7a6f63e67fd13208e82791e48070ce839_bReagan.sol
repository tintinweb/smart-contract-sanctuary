...
contract bReagan is Context, IERC20, Ownable {
    ...
    uint256 private _numTokensSellToAddToLiquidity = 100_000 * 10**9;
...
// Functions
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && sender != uniswapV2Pair && overMinTokenBalance ) {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tMarketingLiquidityFee) = _getTValues(tAmount, _taxFee, _marketingFee);
        uint256 currentRate = _getRate();
    ...
    function _setMarketingFee(uint256 marketingFee) external onlyOwner() {
        require(marketingFee >= 0 && marketingFee <= 499, 'marketingFee should be in 0 - 49.9');
