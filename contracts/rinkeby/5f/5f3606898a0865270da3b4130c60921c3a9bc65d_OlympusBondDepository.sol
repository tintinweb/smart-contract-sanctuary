...
contract OlympusBondDepository is Ownable {
    ...
    event BondCreated( uint indexed deposit, uint payout, uint indexed expires, uint price );
    event BondRedeemed( uint indexed payout, uint indexed remaining );
    event BondPriceChanged( uint indexed priceInUSD, uint indexed debtRatio );
    ...
    function deposit( 
        ...
        bondInfo[ _depositor ] = Bond({
            valueRemaining: info.valueRemaining.add( value ), // add on to previous amount if exists
            payoutRemaining: info.payoutRemaining.add( payout ), // add on to previous amount if exists
        ...
        emit BondPriceChanged( bondPriceInUSD(), _calcDebtRatio() );
        adjust();
        return payout;
    }
    ...
    function redeem( bool _stake ) external returns ( uint ) {        
        ...
        if ( percentVested >= 10000 ) { // if fully vested, pay full amount & clear info
            ...
            return stakeOrSend( _stake, info.payoutRemaining, 0 );
        } else {
            ...
            return stakeOrSend( _stake, payout, bondInfo[ msg.sender ].payoutRemaining );
        }
    }
    ...
    /**
        ...
        @return uint
     */
    function stakeOrSend( bool _stake, uint _amount, uint _remaining ) internal returns ( uint ) {
        emit BondRedeemed( _amount, _remaining );
        emit BondPriceChanged( bondPriceInUSD(), _calcDebtRatio() );
