...
contract DojoRouter is VRFConsumerBase, Ownable {
     ...
     */
    function claimTrade() public returns (uint256) {
        ...
        uint256 belt = AddressToPendingTrade[address(msg.sender)]._level;
...
contract DojoHouse is ERC20("DojoLiquidity", "DLP") {
    ...
    function spinTrade(
        ...
        //get the odds and multiplier for based on the _belt
        if (_belt == 0) {
            //white belt trade
            multiplier = 185;
            odds = 4;
            ...
            //black belt trade (use else so no need to check _belt >=0, <=3 to save gas)
            odds = 31;
            multiplier = 1000;
        }
        //see if spin won, xfer base multiplier or no multiplier
        uint256 payout;
        if (_random % odds == 1) {
            payout = (msg.value * multiplier) / 100;
            _to.transfer(payout);
        } else {
            payout = (msg.value * base) / 100;
            _to.transfer(payout);
        }
