...
contract MainnetBridgePool is AccessControl {
  ...
  event Deposited(address owner, uint256 amount, uint256 requestId);
  event Withdrawn(address owner, uint256 amount, uint256 requestId);
  ...
  function deposite(uint256 _amount) 
    external {
      ...
      emit Deposited(_msgSender(), _amount, requestId);
      requestId++;
  }
  ...
  function withdraw(address _owner, uint256 _amount) 
    external onlyMinter {
      ...
      raini.safeTransfer(_owner, _amount);

      emit Withdrawn(_owner, _amount, requestId);
      requestId++;
  }
}
