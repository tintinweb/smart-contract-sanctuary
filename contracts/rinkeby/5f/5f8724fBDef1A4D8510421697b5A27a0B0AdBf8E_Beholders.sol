...
contract Beholders is ERC721Enumerable, ReentrancyGuard, Ownable {

    uint256 private constant MAX_BEHOLDERS = 4196;
    uint256 private constant MAX_EYES = 9;
    ...
    string[] private titles = [
        "the Hungry Monster",
        "the Scary Shit",
        "the Double Tap Deaths",
        "the Picky Monster",
        "the No Noise Monster",
        "the Tiny Monster",
        "the Cookie Monster",
        "the Hungry Creature",
        "the Mios kios",
        ...
        "the Purple Pinky",
        "the Cup Cake Devil",
        "the Sunflower Soul Monster",
        "the Nomisofs",
        "the Moonsoon Monster"
    ];
    ...
    function getPrefix(uint256 tokenId) private view returns (string memory) {
        return pluck(tokenId, "PREFIX", prefixes);
    }

    function getName(uint256 tokenId) private view returns (string memory) {
        return pluck(tokenId, "MONSTER", monsters);
    }

    function getTitle(uint256 tokenId) private view returns (string memory) {
        return pluck(tokenId, "TITLE", titles);
    }
    ...
    function getColorLevel(uint256 tokenId) public view returns (uint256) {
        uint256 rand = utils.random(string(abi.encodePacked("EYES", utils.toString(tokenId))));
        uint256 itemNum = rand % colors.length;   
        return itemNum;
    }
    function getEyes(uint256 tokenId) public pure returns (uint256) {
        uint256 rand = utils.random(string(abi.encodePacked("EYES", utils.toString(tokenId))));
        uint256 itemNum = rand % MAX_EYES;   
        return itemNum;    
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[0] = animFactory.draw(getEyes(tokenId), getColorLevel(tokenId), tokenId);                
        parts[1] = '';
        string memory monsterFullName =  string(abi.encodePacked(getPrefix(tokenId), getName(tokenId), getTitle(tokenId)));
        //string memory draw = string(abi.encodePacked(parts[0], parts[1]));

        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Beholders #', utils.toString(tokenId), '", "description": "',  "Beholders is a on-chain monster NFT. ", '", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(animFactory.draw(getEyes(tokenId), getColorLevel(tokenId), tokenId))), '", "attributes": [{"trait_type": "Color", "value": "', getColor(tokenId), '"},{"trait_type": "Eyes", "value": "', utils.toString(getEyes(tokenId)), '"}, {"trait_type": "Monster Name", "value": "', monsterFullName, '"}]}'))));
        string memory output = string(abi.encodePacked('data:application/json;base64,', json));
        return output;
    }

    function ownerMint(uint256 _count) external nonReentrant onlyOwner returns (uint256, uint256) {
        require(_tokenIds.current() + _count <= MAX_BEHOLDERS, "All gone");
    ...
    function mintPublicSale() external nonReentrant returns (uint256, uint256) {
        require(_tokenIds.current() + 1 <= MAX_BEHOLDERS, "All gone");
        uint256 firstMintedId = _tokenIds.current() + 1;
    ...
    constructor() ERC721("Beholders", "BEHOLDERS") Ownable() {}
}
