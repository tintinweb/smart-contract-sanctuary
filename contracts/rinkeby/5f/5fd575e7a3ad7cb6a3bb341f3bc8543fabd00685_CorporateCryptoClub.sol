...
contract CorporateCryptoClub is ERC721Enumerable, Ownable {
  ...
  uint256 private gsdReserveAtATime = 111;
  uint256 private gsdReserveCount = 0;
  uint256 private maxGSDReserveCount = 222;
  ...
  function reserveGSDNft(address _ToGSDWalletAddress) public onlyAuthorized {
    require(gsdReserveCount <= maxGSDReserveCount, "Max Reserves taken already!");
    uint256 supply = totalSupply();
    uint256 i;

    for (i = 0; i < gsdReserveAtATime; i++) {
      emit AssetMinted(supply + i, _ToGSDWalletAddress);
      _safeMint(msg.sender, supply + i);
      gsdReserveCount++;
    }
  }
