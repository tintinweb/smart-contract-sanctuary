...
contract UZV1Factory is IUZV1Factory, UZV1ProAccess {
    ...
    function createNewPoolProxy(address _implement) external { 
        ...
        TransparentUpgradeableProxy proxyContract = new TransparentUpgradeableProxy(_implement, owner(), _initializationCalldata);
        emit Test(address(proxyContract), _implement);
        // nProxy initializing = nProxy(proxyContract);
        // // address newPoolImplement = ClonesUpgradeable.clone(implement);
        // initializing.upgradeTo(_implement);
        // IUZV1RewardPool newPool = IUZV1RewardPool(proxyContract);
        // newPool.initialize(router, address(_membershipToken));
        // emit Test(proxyContract, _implement);
