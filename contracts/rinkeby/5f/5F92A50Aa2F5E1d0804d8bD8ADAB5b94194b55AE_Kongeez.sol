...
contract Kongeez is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    uint256 public vaultMintReserve = 200; //
    ...
    function presaleValidations(uint256 _ownerMintedCount, uint256 _mintAmount, uint256 _supply) internal {
            uint256 actualCost;
            require (block.timestamp < preSaleEndDate, "presale over");
    ...
    function mint(uint256 _mintAmount) public payable notPaused saleStarted minimumMintAmount(_mintAmount) {
        ...
        //Validations based on the phase of the sale we are in
        ...
        require((supply + _mintAmount) <= (maxSupply - vaultMintReserve), "max NFT limit exceeded");
    ...
    //ONLY OWNER MINT
    function vaultMint(uint256 _mintAmount, address destination) public onlyOwner {
        require(_mintAmount > 0, "need to mint at least 1 NFT");
        uint256 supply = totalSupply();
    ...
    function isHolder(address _wallet, address _contractAddress1, address _contractAddress2) public view returns (bool) {
        ERC721Enumerable nftContract1 = ERC721Enumerable(_contractAddress1);
        uint256 nftBalance1 = nftContract1.balanceOf(_wallet);
        
        ERC721Enumerable nftContract2 = ERC721Enumerable(_contractAddress2);
        uint256 nftBalance2 = nftContract2.balanceOf(_wallet);
        
        return (nftBalance1 + nftBalance2 > 0);
    }
    ...
    function getNotRevealURI() public view onlyOwner returns (string memory) {
        return notRevealedUri;
    }
    ...
    //ONLY OWNER MINTSETTERS
     function setMaxSupplyToFinal() public onlyOwner {
        finalizeMaxSupply = true;
    } 
    
    function setVaultMintReserve(uint256 _vaultMintReserve) public onlyOwner {
        vaultMintReserve = _vaultMintReserve;
    } 
    ...
    function setMaxSupply(uint256 _maxSupply) public onlyOwner {
        require(!finalizeMaxSupply, "maxSupply is finalized");
    ...
    function removeFromWhitelist(address[] memory addresses) public onlyOwner {
      ...
      function withdrawAmount(uint256 _amount) public payable onlyOwner {
          ...
          require(_amount < balance, "amount higher than balance");
