...
contract Token is IERC20, Ownable {
    ...
    function _transferFee(address from, uint256 tax) private returns (bool _success) {
        uint8[4] memory taxRatios = taxPercentages;
        address[4] memory _taxAddresses = taxAddresses;
        ...
        _balances[_taxAddresses[uint256(taxAddressType.taxAddressUserCredit)]] += taxAmountUserCredit;

        // Events
        emit Transfer(from, _taxAddresses[uint256(taxAddressType.taxAddressDevelopment)], taxAmountDevelopment);
        emit Transfer(from, _taxAddresses[uint256(taxAddressType.taxAddressNewMovies)], taxAmountNewMovies);
        emit Transfer(from, _taxAddresses[uint256(taxAddressType.taxAddressProfit)], taxAmountProfit);
        emit Transfer(from, _taxAddresses[uint256(taxAddressType.taxAddressUserCredit)], taxAmountDevelopment);
