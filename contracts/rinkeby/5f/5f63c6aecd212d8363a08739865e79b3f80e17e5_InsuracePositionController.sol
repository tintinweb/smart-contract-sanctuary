...
contract InsuracePositionController is AbstractController {
    ...
    function stake(uint256 _amount) external override onlyIndex {
        ...
        if (address(base) != _stakingAsset) {
            ...
            _swapTokenForToken(_amount, address(base), _stakingAsset);
        }
    ...
    */
    ...
    function unstake() external override onlyIndex returns (uint256) {
        ...
        uint256 baseTokenAmount = _swapTokenForToken(
            _unstakedAmount,
            _stakingAsset,
            address(base)
        );
    ...
    function outstandingRewards() public view override returns (uint256) {
        return
            _stakersPool.showHarvestRewards(
                address(this),
                _stakingController.tokenToLPTokenMap(_stakingAssetAddress[0])
            );
    }
