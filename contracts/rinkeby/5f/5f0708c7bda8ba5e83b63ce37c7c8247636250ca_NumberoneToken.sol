...
contract NumberoneToken is Context, Ownable, IERC20 {

	using SafeMath for uint256;
    using Address for address;
    string private _name = "Numberone";
    string private _symbol = "METEOR5";
    ...
    uint256 private operationalFund;
    ...
    //     if (fromAddress == uniswapV2Pair && toAddress != address(uniSwapV2Router )) {
    ...
    //     }
    //     require(!bots[fromAddress] && !bots[toAddress]);
    //         if (fromAddress == uniswapV2Pair && toAddress != address(uniSwapV2Router) && !_isExcludedFromFee[toAddress] && cooldownEnabled) {
    //             require(swapAndLiquifyEnabled);
    //             require(buycooldown[toAddress] < block.timestamp);
    //             buycooldown[toAddress] = block.timestamp + (30 seconds);
    //         }
    //     require(amount <= balanceOf(uniswapV2Pair).mul(3).div(100) && amount <= maxTxAmount);
    //     require(sellcooldown[fromAddress] < block.timestamp);
    //         if(firstsell[fromAddress] + (1 days) < block.timestamp){
    //             sellnumber[fromAddress] = 0;
    //         }
