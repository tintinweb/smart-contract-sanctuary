...
contract PixelBeasts is ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => bool) private unboxedNFT;

    // withdraw addresses
    address addr_1 = 0xBEBFf53bB6796d21033f38a46808B47E90777345;
    address generator = 0x1E2fe8DC9e0605AE167Bdf69EEADbA459B076241;
    address addr_3 = 0x1Da6Cf54Ef6F057D86ad1898f7C531c654052C22;

    // 9999 in total
    constructor(string memory baseURI) ERC721("PixelBeasts", "PXB")  {
        setBaseURI(baseURI);

        // team gets the first 3 items
        _safeMint( addr_1, 0);
    ...
    function unboxNFT(uint256 tokenId, bool val) public {
        address owner = ownerOf(tokenId);
        require(msg.sender == owner, "NFT does not belong to you.");
        unboxedNFT[tokenId] = val;
    }

    function isUnboxed(uint id) public view returns(bool) {
        return unboxedNFT[id];
    }
    ...
    function sendGenerator(uint256 nft1, uint256 nft2) public {
        ...
        require( unboxedNFT[nft1],              "NFT 1 is not unboxed" );
        require( unboxedNFT[nft2],              "NFT 2 is not unboxed" );
