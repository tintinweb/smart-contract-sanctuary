...
contract TMAPStacking is ReentrancyGuard, Ownable {
   ...
   */
  function setRewardsPerRound(uint256 rewards) external onlyOwner {
    require(rewards > 0, "Rewards can not be 0");
    tmapPerRound = rewards;
    emit RewardsPerDay(rewards);
  }
  ...
  function setDsp(address _nft) external onlyOwner {
    require(dsp == address(0), "NFT contract already set");
    require(_nft != address(0), "NFT can not be address(0)");
    dsp = _nft;
  }
