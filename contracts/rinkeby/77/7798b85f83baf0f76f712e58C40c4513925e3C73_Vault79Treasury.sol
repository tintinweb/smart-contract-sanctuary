...
contract Vault79Treasury is Ownable {
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        if ( isReserveToken[ _token ] ) {
            ...
            if ( reserveTokens[ 0 ] != _token ) {
                require( allocationRate() >= defaultRate, "Allocation too small" );
            }
        } else {
    ...
    /**
        @notice takes allocation rate of paxg vs others
        @return uint
     */
    function allocationRate() public view returns ( uint ) {
        uint reserves;
        for( uint i = 1; i < reserveTokens.length; i++ ) {
            reserves = reserves.add ( 
                valueOf( reserveTokens[ i ], IERC20( reserveTokens[ i ] ).balanceOf( address(this) ) )
            );
        }
        uint paxgReserve = valueOf( reserveTokens[ 0 ], IERC20( reserveTokens[ 0 ] ).balanceOf( address(this) ) );
        return paxgReserve.mul( 10000 )/ reserves;
    }
