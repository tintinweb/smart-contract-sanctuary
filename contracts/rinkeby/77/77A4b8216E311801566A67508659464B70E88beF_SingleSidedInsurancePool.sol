...
contract SingleSidedInsurancePool is ISingleSidedInsurancePool, ReentrancyGuard {
    ...
    address private exchangeTokens;

    uint256 public immutable LOCK_TIME = 120 seconds;
    uint256 ACC_UNO_PRECISION = 1e18;
    ...
    event ProtocolCreated(address indexed _masterChef, uint16 _protocolIdx);
    event RiskPoolCreated(address indexed _masterChef, address indexed _pool);
    ...
    function setExchangeTokens(address _exchangeTokens) external onlyOwner {
        exchangeTokens = _exchangeTokens;
        for (uint256 ii = 0; ii < protocolIds.current(); ii++) {
            address salesPolicy = getProtocol[uint16(ii)].salesPolicy;
            ISalesPolicy(salesPolicy).setExchangeTokens(_exchangeTokens);
        }
    }
    ...
    // This action can be done only by masterChef owner
    function addProtocol(
        ...
        address _salesPolicy = ISalesPolicyFactory(salesPolicyFactory).newSalesPolicy(
            lastIdx,
            exchangeTokens,
            premiumPool,
            currency,
    ...
    /**
     * @dev create Risk pool with UNO from masterChef owner
     */
    ...
    function enterInPool(address _from, uint256 _amount) external override nonReentrant {
        ...
        if (rewarder != address(0)) {
            IRewarder(rewarder).onUnoReward(_from, 0, IERC20(riskPool).balanceOf(_from));
        }
    ...
    function leaveFromPoolInPending(address _to, uint256 _amount) external override nonReentrant {
        ...
        if (rewarder != address(0)) {
            IRewarder(rewarder).onUnoReward(_to, _pendingUno, amount);
        }
     ...
     */
    function leaveFromPending(address _to) external override nonReentrant {
        require(_to != address(0), "UnoRe: zero address");
        require(_to == msg.sender, "UnoRe: Forbidden");
        require(block.timestamp - lastWithdrawTime[_to] > LOCK_TIME, "UnoRe: Locked time");
        IRiskPool(riskPool).leaveFromPending(_to);
        updatePool();
    }
    ...
    function cancelWithdrawRequest(address _to) external nonReentrant {
        require(_to != address(0), "UnoRe: zero address");
        IRiskPool(riskPool).cancelWithrawRequest(_to);
    }
        ...
        )
    {
        ...
        passTime = block.timestamp - requestTime;
    }
...
interface ISalesPolicy {
    ...
    function setExchangeTokens(address _exchangeTokens) external;
