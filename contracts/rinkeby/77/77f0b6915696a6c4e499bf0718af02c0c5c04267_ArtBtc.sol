...
contract ArtBtc is ERC721Full, Ownable {
    ...
    function pushMetaData(string[] memory _ipfsList)
        public
        onlyOwner
        reentrancyGuard
    {
        require(ipfsList.length <= 200, "Invalid List size");
        for (uint256 i = 0; i < _ipfsList.length; i++) {
            ipfsList.push(_ipfsList[i]);
        }
    }
    ...
    function tokenURI(uint256 _tokenId) external view returns (string memory) {
        ...
        return Strings.strConcat(baseTokenURI(), ipfsList[--_tokenId / 50]);
    }
