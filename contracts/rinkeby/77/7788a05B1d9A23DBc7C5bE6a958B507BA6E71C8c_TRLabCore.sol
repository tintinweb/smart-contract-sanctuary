    ...
    UUPSUpgradeable
{
    ...
    mapping(uint256 => LibArtwork.ArtworkRelease)
        public tokenIdToArtworkRelease;
    ...
    {
        collection = _getCollection(_collectionId);
    }
    ...
    function createCollection(string calldata _metadataPath)
        ...
        onlyOwnerOrCreator
    {
        _createCollection(_msgSender(), _metadataPath);
    }
    ...
    function createArtworkAndReleases(
        ...
        _batchArtworkRelease(owner, artworkId, _numReleases);
    }
    ...
    function _batchArtworkRelease(
        ...
        require(
            _count > 0,
            "Failed print edition.  Creation count must be > 0."
        );
        ...
        Royalty memory royalty = Royalty({
            receiver: _artwork.royaltyReceiver,
            bps: _artwork.royaltyBps
        });

        for (uint32 i = 0; i < _count; i++) {
            uint32 newPrintEdition = currentPrintIndex + 1 + i;
            LibArtwork.ArtworkRelease memory _artworkRelease = LibArtwork
                .ArtworkRelease({
    ...
    function _getCollection(uint256 collectionId)
        internal
        view
        returns (LibArtwork.Collection memory)
    {
        return artworkStore.getCollection(collectionId);
    }

    function _getArtwork(uint256 artworkId)
        internal
        view
        returns (LibArtwork.Artwork memory)
    {
    ...
    ERC165Upgradeable
{
    ...
    function _setRoyalty(uint256 _id, Royalty memory _royalty) internal {
        ...
        require(_royalty.bps <= 10000, "Royalty value should be positive");
