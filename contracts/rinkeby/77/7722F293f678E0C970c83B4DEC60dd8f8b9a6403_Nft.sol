...
contract Nft is ERC721, ERC721URIStorage, Pausable, PaymentSplitter, AccessControl {

    uint256 private _numPerBatch = 5; // TODO: change this to the non-test amount
    uint16 public constant _maxMint = 5;
    uint private _batchDurationDays = 8;    
    uint private _lastBatchDate = block.timestamp;    

    uint256 private _priceMultiplier = 10000000000000000; // 0.01 Ether per batch
    string private _baseHash; // this changes with every batch
        ...
        public  
    {
        ...
        // cant set the next batch if past batchDuration
        require (block.timestamp <= _lastBatchDate + _batchDurationDays * 1 days, "Did not sell out in time");
    ...
    function driverMint(address[] memory to) public {
    ...
    function addAdmin(address addy) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "not admin");
        grantRole(DEFAULT_ADMIN_ROLE, addy);
    }
