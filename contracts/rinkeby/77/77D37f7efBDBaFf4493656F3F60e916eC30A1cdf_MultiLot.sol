...
contract MultiLot is AbstractLot {
  ...
  function withdrawRefund(uint256 _lotId) external {
    ...
    if (
      lot.startSize < lot.totalDepositPoolA &&
      lot.userDepositPoolA[msg.sender] > 0
    ) {
      uint256 refundRatio = (ratioPrecision *
        (lot.totalDepositPoolA - lot.startSize)) / lot.totalDepositPoolA;
      refundAmount =
        (lot.userDepositPoolA[msg.sender] * refundRatio) /
        ratioPrecision;
    } else if (
      lot.startSize < lot.totalDepositPoolB &&
      lot.userDepositPoolB[msg.sender] > 0
    ) {
      uint256 refundRatio = (ratioPrecision *
        (lot.totalDepositPoolB - lot.startSize)) / lot.totalDepositPoolB;
      refundAmount =
