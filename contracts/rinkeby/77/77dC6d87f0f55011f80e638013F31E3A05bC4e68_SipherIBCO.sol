...
contract SipherIBCO is Ownable {
    ...
    /**
     * @dev Get total withdrawable ether of a user
     */
    function getWithdrawableAmount() external view returns (uint) {
        return _withdrawCap(accumulated[msg.sender]);
    }

    /**
     * @dev Get total locked ether of a user
     */
    function getLockedAmount() external view returns (uint) {
        uint userAccumulated = accumulated[msg.sender];
        return userAccumulated - _withdrawCap(userAccumulated);
    }
