...
// File: contracts\gachemachine.sol
...
contract Gachemachine is ERC721Enumerable, Ownable {
  ...
  constructor (uint _maxSupply, uint _maxPerTx, string memory _name, string memory _symbol ) ERC721(_name,_symbol) {
  ...
  function claimmint(uint256 _count ) public{
    ...
    require(totalSupply().add(_count) <= maxSupply);
  ...
  function mint(uint256 _count ) public payable {
    ...
    require(_count > 0 && _count <= maxPerTx);
    require(totalSupply().add(_count) <= maxSupply);
    require(msg.value >= price.mul(_count), "Ether value sent is not correct!");
  ...
  function withdraw() public onlyOwner {
      ...
      uint256 _f1Amt = balance.mul(10).div(100);
      uint256 _f2Amt = balance.mul(90).div(100);      
      require(payable(c1).send(_f1Amt));
      require(payable(owner()).send(_f2Amt));
