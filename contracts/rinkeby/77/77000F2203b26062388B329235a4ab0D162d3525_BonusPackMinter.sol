...
contract BonusPackMinter is Ownable, ContextMixin {
    ...
    function redeemBonusPack(uint256 tokenId, bytes32[] memory proof) public {
        ...
        require(proof.verify(merkleRoot, keccak256(abi.encodePacked(msg.sender))), "BonusPackMinter: Not Allocated");
    ...
    function batchRedeemBonusPacks(uint256[] calldata tokenIds, bytes32[] memory proof) public 
    {
        require(merkleRoot != 0, "BonusPackMinter: no MerkleRoot yet");
        uint256 tokenId;        
        address minter = msg.sender;
        require(proof.verify(merkleRoot, keccak256(abi.encodePacked(minter))), "BonusPackMinter: Not Allocated");
