...
contract RevolverRoll is Swapper, RandomNumberGenerator, PokeMeReady {
    ...
    event MinBetChange(uint minBet);

    uint256 public minBet;

    mapping(uint256 => Game) public games;
    mapping(bytes32 => uint256) private vrfRequestIdToGameId;
    ...
    constructor(
        ...
        Swapper(_uniswapRouter, _LinkToken, _Weth9Token, _UniswapPoolFee)
        RandomNumberGenerator(_VRFCoordinator, _rngKeyHash, _LinkToken, _rngFee)
        PokeMeReady(_pokeMe) 
    {
        minBet = _minBet;
    }
    ...
    function exitGame(uint256 _gameId) external {
        require(lastGameId <= _gameId, "ID_NOT_VALID");
        require(games[_gameId].status == Status.WaitingToPlayer2, "GAME_IS_NOT_IN_THE_CORRECT_STATE");
        require(games[_gameId].player1 == msg.sender, "YOU_ARE_NOT_PLAYER");
        
        games[_gameId].status = Status.Closed;

        uint256 bet = games[_gameId].amountCollected;
        games[_gameId].amountCollected = 0;
        (bool success,) = games[_gameId].player1.call{value: bet}("");
    ...
    /**
     * @notice Request a random number to vrf chainlink for the game
     * @param _gameId: Id of an existing game
     * @dev Only called by gelato when the players are ready
     ...
     */
    function startGame(uint256 _gameId) external onlyPokeMe {
        require(lastGameId <= _gameId, "ID_NOT_VALID");
        require(games[_gameId].status == Status.PlayersReady, "GAME_IS_NOT_IN_THE_CORRECT_STATE");
        
        games[_gameId].status = Status.Started;

        (uint256 etherSwapped) = swapExactOutputSingle(rngFee, games[_gameId].amountCollected);
        games[_gameId].amountCollected -= etherSwapped;

        bytes32 vrfRequestId = getRandomNumber();
        games[_gameId].vrfRequestId = vrfRequestId;
        vrfRequestIdToGameId[vrfRequestId] = _gameId;

        require(IERC20(linkAddress).balanceOf(address(this)) == 0, "NOT_ALL_LINK_TOKENS_ARE_USED");

        emit StartGame(_gameId);

        // (uint256 fee) = payFee();
        // games[_gameId].amountCollected -= fee;    
    }
     ...
     */
    function finishGame(uint256 _gameId) external onlyPokeMe {
        require(lastGameId <= _gameId, "ID_NOT_VALID");
        require(games[_gameId].status == Status.Started, "GAME_IS_NOT_IN_THE_CORRECT_STATE");
        uint256 randomNumber = games[_gameId].randomNumber;
        require(randomNumber > 0, "RANDOM_NUMBER_IS_NOT_READY");

        games[_gameId].status = Status.Claimable;

        uint8 rounds = 0; 
        for(uint8 i = 1; i<78; i+=2){
            uint256 numberPlayer1 = (randomNumber % 10 ** i) / 10 ** (i-1);
            if (numberPlayer1 == 0){
                rounds = (i/2)+1;
                games[_gameId].winner = games[_gameId].player2;
                break;
            }
            uint256 numberPlayer2 = randomNumber % 10 ** (i+1) / 10 ** i;
            if (numberPlayer2 == 0){
                rounds = (i/2)+1;
                games[_gameId].winner = games[_gameId].player1;
                break;
            }
        }
        ...
        emit FinishGame(games[_gameId].winner, _gameId, rounds);
    ...
    function claimPrize(uint256 _gameId) external {
        ...
        require(msg.sender == games[_gameId].winner, "YOU_ARE_NOT_THE_WINNER");
    ...
    function withdrawTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != linkAddress, "CANNOT_WITHDRAW_LINK_TOKENS");
        IERC20(_tokenAddress).safeTransfer(address(msg.sender), _tokenAmount);
    }

    /**
     * @notice Get Games Ids on status WaitingToPlayer2
     * @return _gamesId Ids of games
     */
    function getIdsGameOnWaitingPlayer() external view returns(uint256[] memory _gamesId) {
        uint256 count = 0;
        for (uint256 i=0; i <= lastGameId; i++){
            if (games[i].status == Status.WaitingToPlayer2) {   
                _gamesId[count] = i;
                count++;
            }
        }
    ...
    function fulfillRandomness(bytes32 _requestId, uint256 _randomness) internal override {
        uint256 gameId = vrfRequestIdToGameId[_requestId];
        require(gameId > 0, "NO_VALID_REQUEST_ID");
        games[gameId].randomNumber = _randomness;
    }
...
abstract contract Swapper {
    ...
    constructor(address _uniswapRouter, address _LinkToken, address _Weth9Token, uint24 _UniswapPoolFee) {
        uniswapRouter = IUniswapRouter(_uniswapRouter);
        linkAddress = _LinkToken;
        weth9Address = _Weth9Token; 
     ...
     * @param _amountInMaximum: Maximum amount of Ether
     * @return amountIn Exact amount of ether swapped
     */
    function swapExactOutputSingle(uint256 _amountOut, uint256 _amountInMaximum) internal returns (uint256 amountIn) {
        ...
        amountIn = uniswapRouter.exactOutputSingle{value: _amountInMaximum}(params);
...
abstract contract RandomNumberGenerator is VRFConsumerBase, Ownable {
    ...
    constructor(address _vrfCoordinator, bytes32 _rngKeyHash, address _linkToken, uint256 _rngFee) VRFConsumerBase(_vrfCoordinator, _linkToken) {
        rngFee = _rngFee;
...
abstract contract PokeMeReady {
    ...
    /**
     * @notice Pay Gelato Fee and transaction fee
     */
