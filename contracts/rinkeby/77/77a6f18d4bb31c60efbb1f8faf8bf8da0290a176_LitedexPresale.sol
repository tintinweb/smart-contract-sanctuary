...
contract TimeCrowdsale is Ownable {
    ...
    modifier onlyWhileOpen {
        require(_start > 0 && _end > 0, 'Litedex: time is not initialized');
    ...
    function setTimeCrowdsale(uint256 start, uint256 end) external onlyOwner onlyWhileNotOpen returns(uint256 startTime, uint256 endTime){
        require(end > start && start > 0 && end > 0, 'Litedex: time is invalid');
        _start = start;
        _end = end;
        return (start,end);
    }
    
    function updateStartTime(uint256 time) external onlyOwner onlyWhileNotOpen returns(uint256 startTime){
