...
// File: contracts/StudentTestNFT.sol
...
contract StudentTestNFT is ERC721, Ownable, ERC721Enumerable {
    using SafeMath for uint256;
    using Strings for uint256;

    uint256 public constant tokenPrice = 13000000000000000; // 0.013 MOVR
    ...
    event StudentTestNFTMinted(uint256 tokenId, address owner);

    constructor() ERC721("Student Test NFT", "STN") {}
    ...
    function mintStudentTestNFT(uint256 numberOfTokens) external payable {
        require(saleIsActive, "Sale must be active to mint StudentTestNFT!");
        require(
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            ...
            if (totalSupply() < MAX_TOKENS) {
                ...
                emit StudentTestNFTMinted(id, msg.sender);
            }
        }
    }

    function presaleStudentTestNFT(uint256 numberOfTokens) external payable {
        ...
        returns (string memory)
    {
        ...
        if (isRevealed == false) {
            return
                "ipfs://QmbteEg4by5w4av245SgGd4uJK8HWSXfTzJR7nA8KyLqAg/hidden.json";
        }
