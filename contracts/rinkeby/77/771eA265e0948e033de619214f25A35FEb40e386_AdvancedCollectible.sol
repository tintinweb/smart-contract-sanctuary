...
contract AdvancedCollectible is ERC721, VRFConsumerBase {
    ...
    mapping(uint256 => DBZ) public tokenIdToDBZ;
    mapping(bytes32 => address) public requestIdToSender;

    event requestedCollectible(bytes32 indexed requestId, address requester);
    event DBZAssigned(uint256 indexed tokenId, DBZ dbz);
    ...
    constructor(
        ...
        bytes32 _keyhash,
        uint256 _fee
    ) public VRFConsumerBase(_vrfcoordinator, _linktoken) ERC721("Goku", "GK") {
        tokenCounter = 0;
        keyhash = _keyhash;
        fee = _fee;
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber)
        ...
        override
    {
        DBZ dbz = DBZ(randomNumber % 3);
        uint256 newtokenId = tokenCounter;
        tokenIdToDBZ[newtokenId] = dbz;

        emit DBZAssigned(newtokenId, dbz);
