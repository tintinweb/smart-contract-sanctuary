...
// File: contracts/SatoshiPunks.sol

contract SatoshiPunks is ERC721, Ownable {
    string public constant name = "The Christies";
    ...
    function tokenURI(uint256 _tokenId) public view returns (string memory) {
        Punk memory _punk = punks[_tokenId - 1];
    ...
    function createItem( string calldata _name, string calldata _json, string calldata _img ) external onlyOwner returns (uint) {
        require(MAX_ITEMS > totalSupply());

        Punk memory _punk = Punk({
            name: _name,
            json: _json,
            img: _img
        });
        uint mintId = punks.push(_punk);
