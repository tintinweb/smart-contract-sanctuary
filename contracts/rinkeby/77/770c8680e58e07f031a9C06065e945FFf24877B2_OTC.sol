...
contract OTC is Ownable {
    ...
    uint topAskOrder = 0; // id of the next ask order to be filled (through this is the order with the highest max buy price)
    uint newAskId = 0;
    ...
    uint topBidOrder = 0; // id of the next bid order to be filled (through this is the order with the lowest min sell price)
    uint newBidId = 0;
    ...
    function updateAverageOrderPrice(uint8 _side, bool _add, uint _orderExtremumPrice) private {
        if (_side == 0) {
            if (_add) {
    ...
    */
    function createBuyOrder(uint _anchorAmount, uint _maxBuyPrice, uint _lastId, uint _orderIteration) external lastIdValid(0, _maxBuyPrice, _lastId) returns (bool) {
        ...
        if (_lastId == 0) { // if the user set the last order id to 0 (undefined order), its only valid if there is no possible predecessor (no topAskOrder)
            require(topAskOrder == 0, "cannot set last id order to 0");
            topAskOrder = newAskId + 1;
        }
        IERC20(papr).safeTransferFrom(msg.sender, address(this), _anchorAmount);
    ...
    */
    function createSellOrder(uint _paprAmount, uint _minSellPrice, uint _lastId, uint _orderIteration) external lastIdValid(1, _minSellPrice, _lastId) returns (bool) {
        ...
        if (_lastId == 0) { // if the user set the last order id to 0 (undefined order), its only valid if there is no possible predecessor (no topBidOrder)
            require(topBidOrder == 0, "cannot set last id order to 0");
        ...
        if (_minSellPrice < BidOrderBook[topBidOrder].minSellPrice) { // if the max buy price is the hightest possible, the order can possibly be directly filled
    ...
    function executeTransaction(uint _buyOrderId, uint _sellOrderId) private {
        ...
        if (buyOrder.anchorAmount >= sellOrder.paprAmount.mul(transactionPrice)) { // if the buy order anchor token amount is more than the sell order amount (converted in anchor token)
            ...
            uint transactionAmount = buyOrder.anchorAmount;
