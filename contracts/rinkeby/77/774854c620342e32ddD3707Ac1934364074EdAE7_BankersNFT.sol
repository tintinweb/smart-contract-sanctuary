...
contract BankersNFT is ERC721Enumerable, Ownable {
    /* Created by: 0xInuarashi
    Discord: 0xInuarashi#1234
    Twitter: https://twitter.com/0xinuarashi */

    constructor() payable ERC721("BankersNFT", "UNBK") {}
    ...
    uint256 public tokensReservedForWhitelist = 500;
    uint256 public tokensMintedForWhitelist = 0;

    uint256 public tokensReservedForDutchAuction = 9400;
    uint256 public tokensMintedForDutchAuction = 0;

    uint256 public tokensReservedForReserved = 100;
    ...
    address internal Unbanked = 0x333e7e956FeA76dA56c6a2EA7DE97d0B043c54bb; // INSERT ADDRESS;
    address internal Assune = 0x333e7e956FeA76dA56c6a2EA7DE97d0B043c54bb; // INSERT ADDRESS;
    address internal TheBrain = 0x333e7e956FeA76dA56c6a2EA7DE97d0B043c54bb; // INSERT ADDRESS;
    address internal AntoineMingo = 0x333e7e956FeA76dA56c6a2EA7DE97d0B043c54bb; // INSERT ADDRESS;

    uint256 internal Unbanked_Share = 30;
    uint256 internal Assune_Share = 10;
    uint256 internal TheBrain_Share = 10;
    uint256 internal AntoineMingo_Share = 50;
    ...
    function withdrawEther() public onlyOwner {
        ...
        uint256 _Unbanked_ETH = ((_totalETH * Unbanked_Share) / 100);
        uint256 _Assune_ETH = ((_totalETH * Assune_Share) / 100);
        uint256 _TheBrain_ETH = ((_totalETH * TheBrain_Share) / 100);
        uint256 _AntoineMingo_ETH = ((_totalETH * AntoineMingo_Share) / 100);

        payable(Unbanked).transfer(_Unbanked_ETH);
        payable(Assune).transfer(_Assune_ETH);
        payable(TheBrain).transfer(_TheBrain_ETH);
        payable(AntoineMingo).transfer(_AntoineMingo_ETH);
    }
        ...
        returns (uint256[] memory)
    {
        ...
        _ethToSendArray[0] = _Unbanked_ETH;
        _ethToSendArray[1] = _Assune_ETH;
        _ethToSendArray[2] = _TheBrain_ETH;
        _ethToSendArray[3] = _AntoineMingo_ETH;

        _ethToSendArray[4] = _totalETH; // Total ETH
        _ethToSendArray[5] =
            _Unbanked_ETH +
            _Assune_ETH +
            _TheBrain_ETH +
            _AntoineMingo_ETH; // Total ETH added from shareholders
    ...
    modifier emergencyOnly() {
        require(
            shareholderToUnlockGovernance[Unbanked] &&
                shareholderToUnlockGovernance[Assune] &&
                shareholderToUnlockGovernance[TheBrain] &&
                shareholderToUnlockGovernance[AntoineMingo],
    ...
    uint256 public whiteListPrice = 0.01 ether; // fixed price for whitelisted addresses
        ...
        dutchAuction
    {
        ...
        require(50 >= amount_, "You can only mint up to 5 per transaction!");
        require(
