    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256[] private breedPercentages = [14, 14, 14, 14, 14, 14, 14, 2];

    mapping(uint256 => uint8) private boxTypeById;

    uint256 public boxPriceCWOLF = 1e16;

    string public strBaseTokenURI = "";

    address CWOLF;

    event MintNFT(address indexed _to, uint256 indexed _id);
    ...
    function randomMinMax(uint256 _min, uint256 _max) public view returns (uint256) {
    ...
    function generateWolf() public returns (uint256) {
        // external
    ...
    function mintWithCWOLF()
        ...
        returns (uint256)
    {
        // TODO: Mintear varias a la vez.
        require(
            IERC20Upgradeable(CWOLF).allowance(msg.sender, address(this)) >=
                boxPriceCWOLF,
            "Not enought allowance"
        );

        IERC20Upgradeable(CWOLF).transferFrom(
            msg.sender,
            owner(), // TODO: CAMBIAR A LA BILLETERA DE RECOMPENSAS
            boxPriceCWOLF
        );

        uint256 idMinted = mint(msg.sender);
        return idMinted;
    }
    ...
    */
    ...
    function setCWOLFAddress(address _newAddress)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function updateBreedPercentages() external returns (bool) {
        return true;
    }
}
