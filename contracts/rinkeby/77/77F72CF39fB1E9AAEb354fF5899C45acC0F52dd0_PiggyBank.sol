...
contract PiggyBank is Ownable {
    ...
    */
    function depositByRange(uint256 start, uint256 end) public payable onlyOwnerOrManager {
        uint256 tokenCount = end - start + 1;
        uint256 claimableAmountPerToken = msg.value / tokenCount;

        for(uint256 i = start; i <= end; i++) {
            // Iterate over the tokens
            _claimableEth[i] += claimableAmountPerToken;
        }
    ...
    * Deposit eth for distribution to token owners.
    */
    function depositByTokenIds(uint256[] calldata tokenIds) public payable onlyOwnerOrManager {
        uint256 claimableAmountPerToken = msg.value / tokenIds.length;

        for(uint256 i = 0; i < tokenIds.length; i++) {
            // Iterate over the tokens
            _claimableEth[tokenIds[i]] += claimableAmountPerToken;
        }
