...
contract animalMain is  ERC721,Ownable {
    ...
    string public name_ = "PStoken";

    struct animal {
        bytes32 dna;        //DNA
        uint8 star;         //星數
        uint16 roletype;    //動物(1老虎 2狗 3獅子 4鳥)
        uint8 color;        //顏色(1黑 2白 3棕)
    }
    ...
    function seeAnimalColor(uint256 _tokenId) public view returns (uint8 color) {
    ...
    function createAnimal() public payable {
        ...
        color = uint(random())%3 + 1;
        
        //動手玩創意，可以限制每次建立動物需要花費多少ETH
        require(msg.value == 100 wei);
       
        uint id = animals.push(animal(dna, uint8(star), uint8(roletype), uint8(color))) - 1;
