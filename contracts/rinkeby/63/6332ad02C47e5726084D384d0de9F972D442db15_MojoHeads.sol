...
contract MojoHeads is ERC721URIStorage, AccessControl, IERC2981, Ownable {
    ...
    function register(address accessPassAddress,
        ...
        uint256 maxOngoingTokens,
        uint256 maxSupply,
        uint256 globalStartingTokenId,
        bool mintFromArtistEnabled) public onlyRole(CAMPAIGN_ADMIN_ROLE) returns (uint256) {

        require(!_exists(globalStartingTokenId), "starting token Id already minted");
        require(globalStartingTokenId > artistTokenReserve, "starting token Id can't be in reserve");
        ...
        campaignList[campaignId].availableSupply = maxSupply;
        campaignList[campaignId].currentGobalTokenId = globalStartingTokenId;
    ...
    function mapTokenToArtist(uint256 campaignId, uint256[] calldata tokenIdList, uint256[] calldata artistTokenList) public onlyRole(CAMPAIGN_ADMIN_ROLE) {
...
//        require(tokenIdList.length == campaignList[campaignId].maxSupply, "tokenIdList is less than the max supply of campaign");
    ...
    function publicMint(uint256 campaignId, uint256 amount, address receiver) public payable {
        if (campaignList[campaignId].state == CampaignState.PRESALE) {
        ...
        require(amount <= campaignList[campaignId].availableSupply, "Campaign does not have enough left.");

        campaignList[campaignId].availableSupply = campaignList[campaignId].availableSupply - amount;
        uint256 totalCost = 0;

        for (uint i; i < amount; i++) {
            uint256 price = _checkCampaignStateForMinting(campaignId);
            totalCost = totalCost + price;
            campaignMint(campaignId, campaignList[campaignId].currentGobalTokenId, receiver);
        }
    ...
    function preMint(uint256 campaignId, address receiver) public onlyRole(CAMPAIGN_ADMIN_ROLE) {
        ...
        require(1 <= campaignList[campaignId].availableSupply, "Campaign is sold out.");
    ...
    function preMintBatch(uint256 campaignId, address[] memory receiverList) public onlyRole(CAMPAIGN_ADMIN_ROLE) {
        require(campaignList[campaignId].state != CampaignState.PENDING, "Campaign is not ready yet.");
        require(receiverList.length <= campaignList[campaignId].availableSupply, "Campaign does not have enough left.");

        for (uint i = 0; i < receiverList.length; i++) {
            campaignMint(campaignId, campaignList[campaignId].currentGobalTokenId, receiverList[i]);
        }
    }

    function campaignMint(uint256 campaignId, uint256 tokenId, address receiver) private {
        _mintWithoutValidation(receiver, tokenId);
        _setTokenURI(tokenId, campaignList[campaignId].defaultUri);
        campaignList[campaignId].availableSupply -= 1;
        campaignList[campaignId].currentGobalTokenId += 1;
    }
...
//        require(artistTokenId <= artistTokenReserve, "Artist token id reserves finished.");
//        require(!_exists(artistTokenId), "Artist token is already minted.");
//
//        uint256 price = _checkCampaignStateForMinting(campaignId);
//        require(msg.value >= price, "Insufficient funds.");
//
//        _mintWithoutValidation(receiver, artistTokenId);
//        _setTokenURI(artistTokenId, string(abi.encodePacked(campaignList[campaignId].defaultUri, artistTokenId)));
//
//        if (msg.value > price) {
...
//    }
    ...
    function mintArtistTokenByAdmin(address to, uint256 artistTokenId, string calldata uri) public onlyRole(CAMPAIGN_ADMIN_ROLE) {
        require(artistTokenId <= artistTokenReserve, "Artist token id reserves finished.");
        require(!_exists(artistTokenId), "Artist token is already minted.");
        _mintWithoutValidation(to, artistTokenId);
        tokenRoyaltyMapping[artistTokenId] = defaultRoyaltyAddress;
        _setTokenURI(artistTokenId, uri);
    }

    function _mintWithoutValidation(address to, uint256 tokenId) internal {
        _mint(to, tokenId);
        totalSupply += 1;
    }
