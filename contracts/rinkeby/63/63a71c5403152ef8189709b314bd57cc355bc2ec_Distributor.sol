...
contract Distributor is Policy {
  ...
  address public immutable pegasusNFTAddress;
  ...
  function distribute() external returns (bool) {
    // security vulnerability here potentially
  ...
  function nextRewardAt(uint256 _rate) public view returns (uint256) {
    // total levels of nfts + outstanding tokens
    return
      //unstaked value needs to get added here
      INFT(pegasusNFTAddress)
        .tokenValue()
        .add(IERC20(PEGA).totalSupply())
        .mul(_rate)
        .div(1_000_000); //used to be regular ohm
    // mul(_rate).div(1000000) gives us the actual rate of increase this time how many to mint
  }
