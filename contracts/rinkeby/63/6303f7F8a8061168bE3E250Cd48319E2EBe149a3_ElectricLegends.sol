...
contract ElectricLegends is Context, Ownable, ERC721Enumerable {
    ...
    constructor(string memory baseTokenURI) ERC721("Electric Legends", "ELL") {
    ...
    function mintMultiple(address to, uint8 count) public virtual onlyOwner {
        require(totalSupply() < MAX_SUPPLY,
            "ElectricLegends: reached max token supply");
        for (uint8 numMinted = 0; numMinted < count; numMinted++) {
            _mint(to, totalSupply());
