...
contract Vesting is Ownable {
    ...
    // In a case when there are some YEL tokens left on a contract this 
    // function allows the contract owner to retrieve excess tokens
    function retrieveAccessTokens(uint256 _amount) external onlyOwner {
        require(
            block.timestamp >= endTime,
            "Vesting: can be executed by the owner only after 26 weeks.");
        require(
