...
contract RainiEvolvingNFTv1 is AccessControl {
  ...
  mapping(uint256 => CardVars) public cardsX;
  ...
  constructor (address _nftContractAddress, string memory _uri) {
    ...
    approvedTokenContracts[_nftContractAddress] = true;
    baseUri = _uri;
  }
  ...
  function initCards(
    ...
    bool[] memory _allowHempisphereSet) 
    external onlyOwner {
      for (uint256 i; i < _cardId.length; i++) {
        require (!cardsX[_cardId[i]].exists, 'Card exits');
        cardsX[_cardId[i]] = CardVars({
          ...
          allowHempisphereSet: _allowHempisphereSet[i],
          exists: true
        });
  ...
  function onMinted(address _to, uint256 _tokenId, uint256 _cardId, uint256 _cardLevel, uint256 _amount, bytes1 _mintedContractChar, uint256 _number, uint256[] memory _data) 
    external onlyEditor {
      ...
      if (tokenVars[_tokenId].exists || !cardsX[_cardId].exists) {
        return;
      }

      CardVars memory _card = cardsX[_cardId];
  ...
  function onTransfered(address from, address to, uint256 id, uint256 amount, bytes memory data) external onlyEditor {
      ...
      if (cardsX[tokenVars[id].cardId].resetsOnTransfer) {
        tokenVars[id].lastState = uint32(getState(id));
        tokenVars[id].initTime = uint32(block.timestamp);
      }
  }
  
  function onMerged(uint256 _newTokenId, uint256[] memory _tokenId, address _nftContractAddress, uint256[] memory data) external onlyEditor {
  }

  function getState(uint256 tokenId) public view returns (uint256) {
    EvolvingTokenVars memory _tokenVars =  tokenVars[tokenId];

    if (!_tokenVars.evolves) {
      return 0;
    }

    CardVars memory _cardVars = cardsX[_tokenVars.cardId];

    uint256 _timeSinceStart = block.timestamp - _tokenVars.initTime;
  ...
  function uri(uint256 id) external view returns (string memory) {
    ...
    return string(abi.encodePacked(baseUri, nftContract.cardPathUri(_tokenVars.cardId), "/", _tokenVars.mintedContractChar, "l", uint2str(_tokenVars.level), "n", uint2str(_tokenVars.number), "s", uint2str(getState(id)), ".json"));
  }
