...
contract FoT is ERC20 {
    //events
    event RateSet(uint256 burnRate, uint256 rewardRate);
    ...
    constructor(address _presale) ERC20('FoT', 'FOT') {
    ...
    function setRate(uint256 burnRate, uint256 rewardRate) public onlyGovernance {
        require(MAX_GOVERN_VALUE_RATE >= burnRate && burnRate >= MIN_GOVERN_VALUE_RATE, 'invalid burn rate');
        require(MAX_GOVERN_VALUE_RATE >= rewardRate && rewardRate >= MIN_GOVERN_VALUE_RATE, 'invalid reward rate');

        _burnRate = burnRate;
        _rewardRate = rewardRate;

        emit RateSet(burnRate, rewardRate);
    }

    /**
     * @dev for set reward
     */
    function setStakingPool(address stakingPool) public onlyGovernance {
    ...
    function _transfer(
        ...
        uint256 burnFee = (value * (_burnRate)) / (RATE_BASE);
        if (burnFee > 0) {
        ...
        uint256 rewardFee = (value * _rewardRate) / RATE_BASE;
        if (rewardFee > 0) {
            _balances[_stakingPool] = _balances[_stakingPool] + (rewardFee);
        ...
        _balances[to] = _balances[to] + sendAmount;
        emit Transfer(from, to, sendAmount);
    }

    function mint(address to, uint256 value) external onlyPresale {
        _mint(to, value);
    }
}
...
/**
 * Vanilla OpenZeppelin implementation, however with certain `private` variables modified to be internal so as to implement a custom transfer event
 */
