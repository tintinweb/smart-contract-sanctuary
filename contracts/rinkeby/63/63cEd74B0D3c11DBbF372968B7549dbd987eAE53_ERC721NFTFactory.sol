 ...
 *
 */
contract ERC721NFTFactory is Ownable {
    address public beacon;
    address transferProxy;
    address lazyTransferProxy;

    event Create721NFTProxy(address proxy);
    event Create721NFTUserProxy(address proxy);
    ...
    function createToken(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, uint salt) external {
        ...
        ERC721NFTMinimal token = ERC721NFTMinimal(address(beaconProxy));
    ...
    function createToken(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators, uint salt) external {
        ...
        ERC721NFTMinimal token = ERC721NFTMinimal(address(beaconProxy));
        token.transferOwnership(_msgSender());
        emit Create721NFTUserProxy(beaconProxy);
    }
    ...
    function getData(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI) view internal returns(bytes memory){
        return abi.encodeWithSelector(ERC721NFTMinimal(0).__ERC721NFT_init.selector, _name, _symbol, baseURI, contractURI, transferProxy, lazyTransferProxy);
    }
    ...
    function getData(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators) view internal returns(bytes memory){
        return abi.encodeWithSelector(ERC721NFTMinimal(0).__ERC721NFTUser_init.selector, _name, _symbol, baseURI, contractURI, operators, transferProxy, lazyTransferProxy);
    }
}
...
library LibRoyaltiesV2 {
    /*
     * bytes4(keccak256('getNFTV2Royalties(uint256)')) == 0xcad96cca
     */
...
contract ERC721NFTMinimal is ERC721BaseMinimal {
    /// @dev true if collection is private, false if public
    bool isPrivate;

    event CreateERC721NFT(address owner, string name, string symbol);
    event CreateERC721NFTUser(address owner, string name, string symbol);

    function __ERC721NFTUser_init(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators, address transferProxy, address lazyTransferProxy) external initializer {
        __ERC721NFT_init_unchained(_name, _symbol, baseURI, contractURI, transferProxy, lazyTransferProxy);
        ...
        isPrivate = true;
        emit CreateERC721NFTUser(_msgSender(), _name, _symbol);
    }

    function __ERC721NFT_init(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address transferProxy, address lazyTransferProxy) external initializer {
        __ERC721NFT_init_unchained(_name, _symbol, baseURI, contractURI, transferProxy, lazyTransferProxy);
    ...
    function __ERC721NFT_init_unchained(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address transferProxy, address lazyTransferProxy) internal {
        _setBaseURI(baseURI);
...
import "../ERC721NFT/IERC721LazyMint.sol";
import "../ERC721NFT/Mint721Validator.sol";
