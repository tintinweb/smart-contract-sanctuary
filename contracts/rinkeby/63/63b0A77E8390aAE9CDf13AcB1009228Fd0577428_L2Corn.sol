...
import "./interfaces/Manageable.sol";
...
// MODIFIERS
    ...
    modifier requireFarmer(address farmAddress) {
        require(
            getFarmCollectibleTotalOfType(farmAddress,CollectibleType.Farmer)>0,
            "To do this, a farmer must live on this farm"
        );
        _; // Call the actual code
    }

    /**
     * @dev There must be a tractor on this farm
     */
    modifier requireTractor(address farmAddress) {
        require(
            getFarmCollectibleTotalOfType(farmAddress,CollectibleType.Tractor)>0,
            "To do this, there must be a tractor on the farm to do this"
        );
        _; // Call the actual code
    }
...
// STATE VARIABLES
     ...
     */
    uint256 private constant FAILSAFE_MAX_AMOUNT = 1000 * (10**18);
...
// EVENT
    ...
    constructor(
        address[] memory farmlandAddresses_,
        uint256 failsafeBlockDuration_,
        address[] memory farmAddresses_,
        uint256[] memory composted_
    ) 
        public
        nonReentrant 
        ERC777 ("Corn", "CORN", new address[](0))
        {
            require(farmlandAddresses_.length == 5,             "Invalid number of contract addresses");
            require(farmlandAddresses_[0] != address(0),        "Invalid Land Contract address");
            require(farmlandAddresses_[1] != address(0),        "Invalid Farmers Contract address");
            require(farmlandAddresses_[2] != address(0),        "Invalid Tractors Contract address");
            require(farmlandAddresses_[3] != address(0),        "Invalid Gateway Contract address");
            require(farmlandAddresses_[4] != address(0),        "Invalid L1Corn Contract address");
            require(farmerNFTAddress == address(0),             "Farmers Contract already initiated");
            require(tractorNFTAddress == address(0),            "Tractors Contract already initiated");
            // Set the Farmland contract addresses
            landContract = IERC777(farmlandAddresses_[0]);
            farmerNFTAddress = payable(farmlandAddresses_[1]);
            tractorNFTAddress = payable(farmlandAddresses_[2]);
            // Set L1 Corn Bridging addresses
            L2_GATEWAY = farmlandAddresses_[3];
            l1Address = farmlandAddresses_[4]; // L1 Corn Address
            failsafeTargetBlock = block.number.add(failsafeBlockDuration_);
            _registerInterface(IERC721Receiver.onERC721Received.selector);
            ERC1820.setInterfaceImplementer(address(this),TOKENS_RECIPIENT_INTERFACE_HASH,address(this));
            // Used to migrate the Corn Composted from L1 contract
            uint256 _length = farmAddresses_.length;
            for(uint i = 0; i < _length; i++){
                ownerOfFarm[farmAddresses_[i]].lastHarvestedBlockNumber = block.number; // Set the last harvest height to the current block
                ownerOfFarm[farmAddresses_[i]].compostedAmount = composted_[i]; // Set amount harvested
            }
            isPaused(true); // start the contract in paused mode
            // Add premine to provide initial liquidity
            _mint(_msgSender(), 150000 * (10**18), "", "");
        }
...
// SETTERS
        ...
        preventSameBlock(_msgSender())
    {
        ...
        if (block.number < failsafeTargetBlock) {
            require(amount <= FAILSAFE_MAX_AMOUNT,"You can only allocate a maximum of 1000 LAND during failsafe.");
        }

        Farm storage farm = ownerOfFarm[_msgSender()]; // Shortcut accessor

        // Limit the max farm size for farms without NFTs
        if ( farm.amount.add(amount) > maxFarmSizeWithoutFarmer && getFarmCollectibleTotalOfType(farmAddress,CollectibleType.Farmer)<1 ) {
            revert("You need a farmer to build a farm this size");
        }
        if ( farm.amount.add(amount) > maxFarmSizeWithoutTractor && getFarmCollectibleTotalOfType(farmAddress,CollectibleType.Tractor)<1 ) {
        ...
        if (farm.amount == 0) { // Check to see if there is LAND in the farm
            ...
            if ( getFarmCollectibleTotalOfType(farmAddress,CollectibleType.Farmer)>0 ) { // Ensures that there is a farmer
        ...
        requireFarm(_msgSender(), true) // Ensure the address you are releasing has a farm on the LAND
    {
        ...
        globalCompostedAmount = globalCompostedAmount.sub(farm.compostedAmount);
        ...
        requireFarm(farmAddress, true) // Ensure the address you are composting to has a farm on the LAND
    {
        ...
        // Get a composting bonus if you own a farmer or tractor
        uint256 bonusAmount = getCompostBonus(farmAddress, amount);

        // Update farm & global composted amounts variables
        farm.compostedAmount += amount.add(bonusAmount);
        globalCompostedAmount += amount.add(bonusAmount);

        emit Composted(_msgSender(), farmAddress, amount, bonusAmount);
    ...
    function directCompost(
        ...
        requireTractor(farmAddress) // Ensure theres tractor on the farm
    {
        require(
            targetBlock <= block.number,
        ...
        require(
            farm.harvesterAddress == _msgSender(),
            "You must be the owner of the farm to harvest and compost"
        );
    ...
    function equipCollectible(uint256 tokenID, CollectibleType collectibleType)
        ...
        FarmlandCollectible farmlandCollectible = FarmlandCollectible(getNFTAddress(collectibleType));
        ...
        (_expiry,_boostTrait,,,,) = farmlandCollectible.collectibleTraits(tokenID);
        (_uri) = farmlandCollectible.tokenURI(tokenID);
        if (collectibleType == CollectibleType.Farmer) {
            _maxBoostLevel = _boostTrait.mul(100).add(10000);       // _boostTrait input up to 100 gives a boost of 100% to 200%
        } else {
            _maxBoostLevel = _boostTrait.mul(100).div(4).add(5000); // _boostTrait input between 100 and 400 gives a boost of between 75% to 150%
        }
        ...
        // Receive the Collectibles from the address that is calling this function to crop smart contract.
        IERC721(farmlandCollectible).safeTransferFrom(_msgSender(),address(this),tokenID); // [RE-ENTRANCY WARNING] external call, must be at the end    
    }
    ...
    function releaseCollectible(uint256 index)
        ...
        // TODO: Add release all collectibles

        // Ensure this size farm has a farmer
        if ( farm.amount > maxFarmSizeWithoutFarmer && getFarmCollectibleTotalOfType(_msgSender(),CollectibleType.Farmer) < 2 && collectibleType == CollectibleType.Farmer) {
            revert("You need at least one farmer to run a farm this size");
        }
        // Ensure this size farm has a farmer and a tractor
        if ( farm.amount > maxFarmSizeWithoutTractor && getFarmCollectibleTotalOfType(_msgSender(),CollectibleType.Tractor) < 2 && collectibleType == CollectibleType.Tractor) {
        ...
        IERC721(farmlandCollectible).safeTransferFrom(address(this),_msgSender(),collectibleID); // [RE-ENTRANCY WARNING] external call, must be at the end
    }
...
// GETTERS
        ...
        returns (uint256 availableToHarvest)
    {
        ...
        uint256 _lastBlockInGrowthCycle;
        uint256 _blocksMinted;
        if ( getFarmCollectibleTotalOfType(farmAddress, CollectibleType.Farmer) < 1 ) {
            // set the end of growth cycle ... calculate last block
            _lastBlockInGrowthCycle = farm.lastHarvestedBlockNumber.add(maxGrowthCycle);
            _blocksMinted = maxGrowthCycle;
        } else {
            _lastBlockInGrowthCycle = farm.lastHarvestedBlockNumber.add(maxGrowthCycleWithFarmer);
            _blocksMinted = maxGrowthCycleWithFarmer;
        }

        // check the growing cycle
        if (targetBlock < _lastBlockInGrowthCycle) {
            _blocksMinted = targetBlock.sub(farm.lastHarvestedBlockNumber);
        }

        // Calculate amount to harvest before boosts
        uint256 _availableToHarvestBeforeBoosts = amount.mul(_blocksMinted);

        // Adjust for boosts
        availableToHarvest = getTotalBoost(farmAddress).div(PERCENT_MULTIPLIER).mul(_availableToHarvestBeforeBoosts).div(HARVEST_PER_BLOCK_DIVISOR);
    }
    ...
    {
        uint256 myRatio = getAddressRatio(farmAddress);
        uint256 globalRatio = getGlobalRatio();

        // Avoid division by 0 & ensure 1x boost if nothing is locked
        if (globalRatio == 0 || myRatio == 0) {return PERCENT_MULTIPLIER;}

        // The final multiplier is returned with PERCENT (10000x) multiplication and needs to be divided by 10000 for final number
        // Min 1x, Max depends on the global maxCompostBoost attribute
        compostBoost = Math.min(maxCompostBoost,myRatio.mul(PERCENT_MULTIPLIER).div(globalRatio).add(PERCENT_MULTIPLIER)); 
    }
        ...
        returns (uint256 maturityBoost)
    {
        ...
        uint256 _totalMaxBoost;
        uint256 _targetBlockNumber;
        
        // Ensure this address has a farm on the LAND
        if ( farm.amount == 0 ) {return PERCENT_MULTIPLIER;}
        
        // Ensure there are collectibles and then pull the totals into local variables
        if (farm.numberOfCollectibles > 0) {
            (_totalMaxBoost, _targetBlockNumber) = getFarmCollectibleTotals(farmAddress);
        } else {
            // If there are no collectibles it sets the starting block to when the farm is built, otherwise when the last collectible was added. So adding a collectible restarts the counter.
            _targetBlockNumber = farm.blockNumber;
        }

        // Ensures Maturity & Collectible boost doesn't exceed the maximum
        _totalMaxBoost = _totalMaxBoost.add(maxMaturityBoost);
        if ( _totalMaxBoost > maxMaturityCollectibleBoost ) {
            _totalMaxBoost = maxMaturityCollectibleBoost;
        }

        // Calculates the boost extension by removing 10000 from the totalmaxboost; i.e., the extension over and above 1x e.g., the 2x to get to a 3x boost
        uint256 _boostExtension = _totalMaxBoost.sub(PERCENT_MULTIPLIER);
        ...
        uint256 _blockDiff =
            block.number
            .sub(_targetBlockNumber)
            .mul(_boostExtension)
            .div(endMaturityBoost)
            .add(PERCENT_MULTIPLIER);

        maturityBoost = Math.min(_totalMaxBoost, _blockDiff); // Min 1x, Max depends on the boostExtension attribute 
    }
    ...
    {
        uint256 _maturityBoost = getFarmMaturityBoost(farmAddress);
        uint256 _compostBoost = getFarmCompostBoost(farmAddress);
        ...
        totalBoost = _compostBoost.mul(_maturityBoost).div(PERCENT_MULTIPLIER);
    }
        ...
        )
    {
        if ( getFarmCollectibleTotalOfType(farmAddress,CollectibleType.Farmer) >0 ){
            compostBonus = bonusCompostBoostWithFarmer.mul(amount).div(PERCENT_MULTIPLIER); // Add an additional 10% for farms with farmer
        }
        if ( getFarmCollectibleTotalOfType(farmAddress,CollectibleType.Tractor) >0 ){
            compostBonus = bonusCompostBoostWithTractor.mul(amount).div(PERCENT_MULTIPLIER); // Add an additional 20% for farms with a tractor 
        }
    }
    ...
    {
        if (collectibleType == CollectibleType.Farmer) {
            collectibleAddress = farmerNFTAddress;
        } else {
                collectibleAddress = tractorNFTAddress;
        }
    }
    ...
    {
        bool _expired = false;
        uint256 _total = ownerOfCollectibles[farmAddress].length;
        uint256 _expiry;
        uint256 _addedBlockNumber;

        for (uint i = 0; i < _total; i++) {
            _expiry = ownerOfCollectibles[farmAddress][i].expiry;
            _addedBlockNumber = ownerOfCollectibles[farmAddress][i].addedBlockNumber;
            ...
            if ( !_expired ) {
                ...
                if ( lastAddedBlockNumber < _addedBlockNumber) {
                    lastAddedBlockNumber = _addedBlockNumber;
                }
            }
        }
    }
    ...
    {
        uint256 _total = ownerOfCollectibles[farmAddress].length;
        Collectible[] memory _collectibles = new Collectible[](_total);
        for (uint i = 0; i < _total; i++) {
            _collectibles[i].id = ownerOfCollectibles[farmAddress][i].id;
            _collectibles[i].collectibleType = ownerOfCollectibles[farmAddress][i].collectibleType;
            _collectibles[i].maxBoostLevel = ownerOfCollectibles[farmAddress][i].maxBoostLevel;
            _collectibles[i].addedBlockNumber = ownerOfCollectibles[farmAddress][i].addedBlockNumber;
            _collectibles[i].expiry = ownerOfCollectibles[farmAddress][i].expiry;
            _collectibles[i].uri = ownerOfCollectibles[farmAddress][i].uri;
        }
        ...
        returns (uint256 myRatio)
    {
        ...
        uint256 _addressLockedAmount = farm.amount;
        uint256 _addressBurnedAmount = farm.compostedAmount;

        // If you haven't harvested or composted anything then you get the default 1x boost
        if (_addressLockedAmount == 0) {
            return 0;
        }

        // Compost/Maturity ratios for both address & network. Note that we multiply both ratios by the ratio multiplier before dividing. For tiny CROP/LAND burn ratios.
        myRatio = _addressBurnedAmount.mul(RATIO_MULTIPLIER).div(_addressLockedAmount);
    }
        ...
        returns (uint256 globalRatio) 
    {
        ...
        if (globalAllocatedAmount == 0) {return 0;}

        // Compost/Maturity for both address & network, we multiply both ratios by the ratio multiplier before dividing. For tiny CROP/LAND burn ratios.
        globalRatio = globalCompostedAmount.mul(RATIO_MULTIPLIER).div(globalAllocatedAmount);
    }
        ...
        returns (uint256 globalAverageRatio) 
    {
        ...
        // Compost/Maturity for both address & network, we multiply both ratios by the ratio multiplier before dividing. For tiny CROP/LAND burn ratios.
        globalAverageRatio = globalCompostedAmount.mul(RATIO_MULTIPLIER).div(globalAllocatedAmount).div(globalTotalFarms);
    }
    ...
    function getAddressTokenDetails(address farmAddress)
        ...
        returns (
            ...
            bool isOperatorTractor
        )
    {
        blockNumber = block.number;
        isOperatorLand = IERC777(landContract).isOperatorFor(address(this), farmAddress);
        isOperatorFarmer = IERC721(FarmlandCollectible(getNFTAddress(CollectibleType.Farmer))).isApprovedForAll(farmAddress,address(this));
        isOperatorTractor = IERC721(FarmlandCollectible(getNFTAddress(CollectibleType.Tractor))).isApprovedForAll(farmAddress,address(this));
...
abstract contract FarmlandCollectible is ERC721, Ownable, ReentrancyGuard, Pausable, Initializable {
    using SafeMath for uint256;
    ...
    require( msg.value >= price * numTokens,                   "Ether sent is not correct" );
        ...
        // Air drop for NFT purchases
        if ( tokenDropAmount > 0 && IERC20(token).balanceOf(address(this)) > tokenDropAmount) {
            require(IERC20(token).transfer(msg.sender, tokenDropAmount), 'Drop Transfer failed.');
        }
    }

    function storeTraits(uint256 id) public virtual {}

    function initialize(string memory uri_, uint256 reserved_, uint256 maxSupply_, uint256 price_, address payable farmersAddress_, address cornAddress_) external virtual initializer onlyOwner {}

    function randMod(uint256 modulus) internal returns(uint) {
        randNonce++;
    ...
    function giveAway(address to, uint256 amount) external nonReentrant onlyOwner {
       ...
       for(uint256 i = 0; i < amount; i++){
           uint256 _id = _supply + i;
            storeTraits(_id); // Store Collectible traits on chain
           _safeMint (to, _id);
           _setTokenURI(_id, string(abi.encodePacked(_id.toString(),".json")));
       }
       reserved -= amount;
    }
    ...
    // Withdraw any remaining air drop tokens
...
interface IFarmlandCollectible {

    function mintCollectible(uint256 _numTokens) external payable;
    function storeTraits(uint256 _id) external;
    function initialize(string memory _baseURI, uint256 _reserved, uint256 _maxSupply, uint256 _price, address payable _farmersAddress, address _cornAddress) external;
    function randMod(uint256 _modulus) external returns(uint);
    function walletOfOwner(address _owner) external view returns(uint256[] memory);
    function giveAway(address _to, uint256 _amount) external;
    function setPrice(uint256 _newPrice) external;
    function setBaseURI(string memory _baseURI) external;
    function isPaused(bool _value) external;
 ...
 */
abstract contract Manageable is ERC777, Ownable, Pausable, IArbToken //, Initializable
 {
    /**
     ...
     */
    uint256 internal maxGrowthCycle = 134400;
     ...
     */
    uint256 internal maxGrowthCycleWithFarmer = 268800;
