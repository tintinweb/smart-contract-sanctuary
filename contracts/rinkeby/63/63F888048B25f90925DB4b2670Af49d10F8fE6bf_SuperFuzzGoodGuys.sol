...
contract SuperFuzzGoodGuys is Ownable, ERC721Enumerable, PaymentSplitter, ReentrancyGuard {
    ...
    // address private GoldTicketAddress = 0x77f9a627A41b39c23469a7111f91C8487582c019; // TODO: Mainnet address
//    address private GoldTicketAddress = 0x77f9a627A41b39c23469a7111f91C8487582c019;  // TODO: Localhost Address
     address private GoldTicketAddress = 0x191F2f591E5d4E4a004eb4BBf3CC90C4DF4A4b95;  // TODO: rinkeby Address
    SuperFuzzGoldTicket goldTicket;

    // address private GoldTicketAddress = 0x77f9a627A41b39c23469a7111f91C8487582c019; // TODO: Mainnet address
//    address payable BadBatchAddress = payable(0x457E089448b41903A3778F592E7F4A0B87d2F2a8);  // TODO: Localhost Address
     address payable BadBatchAddress = payable(0x155140D39fdeC4eC485a6f054206d8e3b8DeF48B);  // TODO: rinkeby Address
    SuperFuzzBadBatch badBatch;

    string public PROVENANCE = "";
    string private _baseURIextended;
    uint public maxTokens = 7777;
    uint public reservedTokens = 25;
    uint public tokensForSale = maxTokens - reservedTokens;
    ...
    mapping(uint => bool) private hasGoldIdPurchased; // use this to limit people from transferring and minting
    mapping(uint => bool) private hasBadBatchIdPurchased; // use this to limit people from transferring and minting

    // TODO: Remove for production
    uint private debugTime = 3 minutes;

    // Sale stages Timestamps ---- TODO: update for production
    uint public goldTicketPreSale = block.timestamp + debugTime; // TODO: update to 9am PST sunday 26th -> create setter
    uint public badBatchPreSale = goldTicketPreSale + debugTime; // TODO: update to 12pm PST sunday 26th -> create setter
    uint public whitelistOnePreSale = badBatchPreSale + debugTime; // TODO: update to 3pm PST sunday 26th -> create setter
    uint public whitelistTwoPreSale = whitelistOnePreSale + debugTime; // TODO: update to 6pm PST sunday 26th -> create setter
    uint public publicSale = whitelistTwoPreSale + debugTime; // TODO: update to 8pm PST sunday 26th -> create setter

    // Distribution addresses
    address private multiSig = 0x0dAAc5f8A8E7F3231562c9574276b38520fe2a67; // TODO: Update for production
    address private coreTeam = 0x087aE7bb96397ce11e3D57493Ee46aF2e703a019; // TODO: update for production
    address private dev = 0x46c5038e30B3FFca7F088c021575Efa787F171aC; // TODO: update for production
    address private genArt = 0x8673259e07A5f2FC3bE1c761aB5097160760c979; // TODO: update for production
    ...
    constructor()
        ...
        PaymentSplitter(_team, _team_shares)
    {
        _baseURIextended = "ipfs://bafybeibrgdlav4stwuhvcozjudk6uvlndxlclx6gacj5bhmkfwo46p5wyq/"; // TODO: update to reveal address api
    ...
    function resetTimer() public onlyTeam {
        goldTicketPreSale = block.timestamp + debugTime; // TODO: update to 9am PST sunday 26th -> create setter
        badBatchPreSale = goldTicketPreSale + debugTime; // TODO: update to 12pm PST sunday 26th -> create setter
        whitelistOnePreSale = badBatchPreSale + debugTime; // TODO: update to 3pm PST sunday 26th -> create setter
        whitelistTwoPreSale = whitelistOnePreSale + debugTime; // TODO: update to 6pm PST sunday 26th -> create setter
        publicSale = whitelistTwoPreSale + debugTime; // TODO: update to 8pm PST sunday 26th -> create setter
    }
    ...
    modifier verifyAirDrop(address _to, uint _amount) {
        require(totalSupply() <= tokensForSale, "We Sold All Out, See you on the next drop!");
        require(totalSupply().add(_amount) <= tokensForSale, "Purchase would exceed max supply of Bad Batch");
        _;
    }
    ...
    modifier verifyBuy(address _to, uint _amount) {
        ...
        require(totalSupply() <= tokensForSale, "Error 7,777: Sold Out!");
        require(totalSupply().add(_amount) <= tokensForSale, "Hold up! Purchase would exceed max supply. Try a lower amount.");
        require(accounts[_to].tokenBalance.add(_amount) >= maxPerWallet == false, "Nice Try! You shall not have more than 5 in your wallet.");
        console.log("Total tokens plus request", accounts[_to].tokenBalance.add(_amount));
        if (currentState == Status.WhitelistTwo) {
            ...
            console.log("bad batch balance", badBatch.balanceOf(_to));
            ...
            require(false, "Nice Try! Sale is closed! ");
        }
        _;
    }
    ...
    function setTotalSupply(uint _amount) external onlyTeam {
        tokensForSale = _amount - reservedTokens;
    }
    ...
    function airDropMany(address[] memory _addr) external onlyTeam verifyAirDrop(msg.sender, _addr.length) returns (bool, uint) {
        address _to = msg.sender;
        uint id;
        for (uint i = 0; i < _addr.length; i++) {
            id = totalSupply() + 1;
    ...
    function mintGoodGuy(uint _amount) external payable nonReentrant verifyBuy(msg.sender, _amount) returns (bool, uint) {
        address _to = msg.sender;
        uint id;
        for (uint i = 0; i < _amount; i++) {
            id = totalSupply() + 1;
...
//    function airDropToGoldTicketOwners() external onlyTeam returns (address[] memory) {
//        // require(goldTicket.balanceOf(_to) > 0, "Must own a gold ticket");
//        uint goldTicketSupply = goldTicket.totalSupply();
//        address[] storage goldTicketHolders;
//        for (uint i = 0; i < goldTicketSupply; i++) {
//            address _address = goldTicket.ownerOf(i);
//           goldTicketHolders.push(_address);
//        }
//        return (goldTicketHolders);
//    }


    function burn(uint _id) external returns (bool, uint) {
        require(ownerOf(_id) == msg.sender, "Sneaky sneaky! You don't own this Token.");
