...
// https://rinkeby.etherscan.io/address/0xfaee4b062449a24a3d0f211d08caac43ee3a1d9b#code
...
contract AssetContractShared is AssetContract, ReentrancyGuard {
    ...
    modifier creatorOnly(uint256 _id) virtual override {
        require(
    ...
    constructor(
        ...
        address _proxyRegistryAddress, //   代理
     ...
     */
    function addSharedProxyAddress(address _address) public onlyOwnerOrMinter {
    ...
    //2 :  0x555555c2DF9667fD8A479453C9F70F3C9f6Df977
    //3 :  BigNumber {
    //  _hex: '0x69053524dacecd62bd71d2b6dfd797afa3518bbb000000000000040000000001',
    //  _isBigNumber: true
    //}
    ...
    function safeTransferFrom(
        ...
        if (mintedBalance < _amount) {
            ...
            mint(_to, _id, _amount.sub(mintedBalance), _data); //   先给 target mint 对应的数量，然后再进行 transfer
            if (mintedBalance > 0) {
...
contract AssetContract is ERC1155Tradable {
     ...
     * Indicates that this contract can return _balances for
    ...
    function setTemplateURI(string memory uri) public onlyOwnerOrMinter {
        templateURI = uri;
    }
    ...
    function balanceOf(address _owner, uint256 _id)
        ...
        returns (uint256)
    {
        if (!_exists(_id) && _isOwner(_owner)) {
    ...
    function mint(
        ...
        uint256 _quantity,
        bytes memory _data
    ) public virtual override onlyOwnerOrMinter {
        require(!_exists(_id), 'AssetContract#mint: TOKEN_EXISTS');
        _mint(_to, _id, _quantity, _data);
    }
    ...
    function batchMint(
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            require(!_exists(_ids[i]), 'AssetContract#batchMint: TOKEN_EXISTS');
        }
        _batchMint(_to, _ids, _quantities, _data);
    }
    ...
    function _mint(
        ...
        super._mint(_to, _id, _quantity, _data); // 先完成 mint
        if (_data.length > 1) {
