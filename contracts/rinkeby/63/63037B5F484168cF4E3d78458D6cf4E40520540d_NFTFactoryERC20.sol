...
import "../interfaces/factorys/INFTFactoryERC20.sol";
...
contract NFTFactoryERC20 is INFTFactoryERC20, Managed {
    ...
    EnumerableSet.AddressSet private _nftMembershipAddress;
        ...
        )
    {
        ...
        for (uint256 index = 0; index < length; index++) {
            address nf = _nftMembershipAddress.at(index);
            nft[index] = nf;
            count[index] = IERC20(nf).balanceOf(_addr);
        }
    ...
    function getTotalSupply(address _nftAddress)
        external
        view
        override
        returns (uint256)
    {
        return nftInfo[_nftAddress].totalSupply;
    }

    function getRemainingSupply(address _nftAddress)
        external
        view
        override
        returns (uint256)
    {
        return nftInfo[_nftAddress].remainingSupply;
    }
        ...
        returns (uint256)
    {
        ...
        return
            info.basePrice.add(
                info.totalSupply.sub(info.remainingSupply).mul(
                    info.priceModifier
                )
            );
    }

    function getBasePrice(address _nftAddress)
        external
        view
        override
        returns (uint256)
    {
        return nftInfo[_nftAddress].basePrice;
    }

    function getPreconditionalAddress(address _nftAddress)
        external
        view
        override
        returns (address)
    {
        return nftInfo[_nftAddress].prerequisiteNFT;
    }
    ...
    function addNewTier(
        ...
        require(_nftAddress != address(0), "NFTF: Can't equal zero address");

        require(
            !_nftMembershipAddress.contains(_nftAddress),
            "NFTF: already exists"
        );

        require(_basePrice > 0, "NFTF: Can't be 0");

        require(_isInfinity || _totalSupply > 0, "NFTF: Can't be 0");
    ...
    function setBasePrice(address _nft, uint256 _amount)
        ...
        requirePermission(ROLE_ADMIN)
    {
        require(_nftMembershipAddress.contains(_nft), "NFTF: not exists");
    ...
    function buy(address _nft, uint256 _amount) external override {
        ...
        if (info.prerequisiteNFT != address(0)) {
            require(
                IERC20(info.prerequisiteNFT).balanceOf(_msgSender()) >= _amount,
                "NFTFactory: There is not enough previous tokens"
            );
        }
        IKAISHIToken kaishiToken =
            IKAISHIToken(management.contractRegistry(CONTRACT_KAISHI_TOKEN));

        uint256 price = calculatePrice(_nft, _amount);

        require(
            kaishiToken.balanceOf(_msgSender()) >= price,
            "NFTFactory: Not enough Kaishi tokens on balance to buy"
        );

        uint256 percentage = price.div(2);
        kaishiToken.burnFrom(_msgSender(), percentage);
        IERC20(address(kaishiToken)).safeTransferFrom(
            _msgSender(),
        ...
        if (info.prerequisiteNFT != address(0)) {
            IERC20 erc20In = IERC20(info.prerequisiteNFT);
            erc20In.safeTransferFrom(msg.sender, address(this), _amount);
            NFTInfo storage infoInNFT = nftInfo[info.prerequisiteNFT];
            infoInNFT.remainingSupply = infoInNFT.remainingSupply.add(_amount);
        }

        info.remainingSupply = info.remainingSupply.sub(_amount);

        IERC20 erc20Out = IERC20(_nft);
        erc20Out.safeTransfer(msg.sender, _amount);

        emit Buy(_msgSender(), _nft, _amount);
    }
        ...
        returns (uint256)
    {
        ...
        uint256 sellToken = info.totalSupply.sub(info.remainingSupply);
        uint256 currentPrice =
            info.basePrice.add(sellToken.mul(info.priceModifier));

        uint256 newPrice =
            info.basePrice.add(
                sellToken.add(_amount.sub(1)).mul(info.priceModifier)
            );
        return newPrice.add(currentPrice).mul(_amount).div(2);
    }
...
interface INFTFactoryERC20 {
    ...
    function getTotalSupply(address nftAddress_)
        external
        view
        returns (uint256);

    function getRemainingSupply(address nftAddress_)
        external
        view
        returns (uint256);
    ...
    function getBasePrice(address nftAddress_) external view returns (uint256);

    function getPreconditionalAddress(address nftAddress_)
        external
        view
        returns (address);
