...
import "./PartyManagerInterface.sol";
...
import "./PartiesStorageLibrary.sol";
 ...
 * @author Keyko
 *
 * @dev Contract in charge of store and manage the different parties
 *      created by the community
 *
 */
contract PartyManager is
    IPartyManager,
    Initializable,
    ...
    PausableUpgradeable
{
    ...
    using PartiesStorageLibrary for PartiesStorageLibrary.PartiesList;
    PartiesStorageLibrary.PartiesList internal partiesList;

    using PartiesStorageLibrary for PartiesStorageLibrary.RewardsList;
    PartiesStorageLibrary.RewardsList internal rewardsList;

    // Address used to lock the funds
    ...
    // It stores the "participations" of users in a specific party
    ...
    /**
     * @notice It creates a new party with the configuration given by the creator
     */
    ...
    /**
     * @notice It returns the attributes of a specific party
     */
    function getParty(uint256 _partyId)
        external
        view
        returns (
            PartiesStorageLibrary.PartyState state,
            address creator,
    ...
    {
        return partiesList.get(_partyId);
    }
     ...
     * after depositing
     */
    function updateLockDuration(uint256 _partyId, uint256 _lockDuration)
        external
        onlyRole(BACKEND_ADMIN_ROLE)
    {
        partiesList.updateLockDuration(_partyId, _lockDuration);
    }

    /**
     * @notice It returns an array containing the address of the participants to the party
     */
    function getParticipants(uint256 _partyId)
        external
        view
        returns (address[] memory participants)
    {
        return partiesList.getParticipants(_partyId);
    }

    /**
     * @notice It checks if an address is a selected participant for a specific party
     */
    function isParticipant(uint256 _partyId, address _participant)
        public
        view
        returns (bool)
    {
        address[] memory participants = partiesList.getParticipants(_partyId);
        (_partyId);
    ...
    /**
     * @notice It allows a valid participant to deposit some amount for joining a party
     * Requirements:
     * - The party needs to be in `WaitingParticipants` state
     * - The `amount` should be the one defined by the party creator
    ...
    function participantDeposit(
        ...
        ) = partiesList.get(_partyId);

        require(_isOpen(state), "Party is not open");
        require(
        ...
        //increasing the balance of the party
        partiesList.increaseBalance(_partyId, _amount);

        //checking if the participant has deposited before
        if (multiToken.balanceOf(_participant, _partyId) == 0) {
            // if the participant hasn't deposited before, he is a new participant
            partiesList.addParticipant(_partyId, _participant);
            emit ParticipantJoinedParty(_partyId, _participant);

            // checking if the minimum number of participants joined the party so the
            // yielding can start
            if (participantsNumber.add(1) >= minimumParticipants) {
                if (_isWaiting(state)) {
                    partiesList.updateState(
                        _partyId,
                        PartiesStorageLibrary.PartyState.Created
                    );
                    emit PartyStateChanged(
                        _partyId,
                        msg.sender,
                        PartiesStorageLibrary.PartyState.WaitingParticipants,
                        PartiesStorageLibrary.PartyState.Created
                    );
                }
            }
        ...
        if (state == PartiesStorageLibrary.PartyState.Yielding) {
            ...
            multiToken.mint(_participant, _partyId, _amount, twaAmount, "");
        } else {
            multiToken.mint(_participant, _partyId, _amount, _amount, "");
        }

        multiToken.updateParticipantLastDepositDate(_partyId, _participant);
    }
    ...
    function participantWithdraw(
        ...
        PartiesStorageLibrary.PartyState state;
        uint256 startedYieldingBlock;
        uint256 yieldingDuration;
        ...
        if (state == PartiesStorageLibrary.PartyState.Yielding) {
            ...
            feeAmount = startedYieldingBlock.add(yieldingDuration).sub(
                block.number
            );
            // calculating the percent of the remaining period relative to the yielding duration
            feeAmount = feeAmount.mul(100).div(yieldingDuration);
            ...
            // if the party is not yielding, using just the coefficent
            feeAmount = _feeCoefficent.mul(_amount).div(10000);
        }

        require(
            state != PartiesStorageLibrary.PartyState.Rewarding &&
                state != PartiesStorageLibrary.PartyState.Finished,
            "Party Manager: You can not withdraw now"
        );
        require(
            multiToken
                .getParticipantLastDepositDate(_partyId, _participant)
                .add(lockDuration) <= block.number,
            "Party Manager: You can not withdraw until the lock period has passed!"
        );

        _handleParticipantWithdraw(_partyId, _participant, _amount, feeAmount);
    }

    function startYielding(uint256 _partyId)
        public
        onlyRole(BACKEND_ADMIN_ROLE)
    {
        (
        ...
        require(
            state == PartiesStorageLibrary.PartyState.Created,
            "Party is not ready"
        );
        partiesList.updateState(
            _partyId,
            PartiesStorageLibrary.PartyState.Yielding
        );
        emit PartyStateChanged(
            _partyId,
            msg.sender,
    ...
    /**
     * @notice If the party was yielding and the duration is over, it allows to stop the yielding phase
     * and start the rewarding phase
     * Everybody can call this method!
     * @param _partyId id of the party for which the winners are calculated
     ...
     * are calculated
     * @param _rewardCoefficient the percentage of the balance the balance that is not being withdrawed
    ...
    function stopYielding(
        ...
        require(
            state == PartiesStorageLibrary.PartyState.Yielding,
            "Party is not yielding"
        );

        require(
            block.number >= blockNumberUpdated + duration,
            "Party can't stop yielding before duration"
        );

        partiesList.updateState(
            _partyId,
            PartiesStorageLibrary.PartyState.Rewarding
        );
        ...
        rngService.requestRandomNumber(_partyId);
    ...
    function distributeRewards(uint256 _partyId, uint256 _randomNumber)
        external
        override
        nonReentrant
    {
        ...
        require(
            state == PartiesStorageLibrary.PartyState.Rewarding,
            "Party is not rewarding"
        );

        uint256 partyTwaBalance = 0;
        for (uint256 i = 0; i < participants.length; i++) {
            partyTwaBalance = partyTwaBalance.add(
                multiToken.twaBalanceOf(participants[i], _partyId)
            );
        }
        ...
        if (minimumParticipants == 0) {
            prizeStrategyService.computeRewards(
                _partyId,
                participants
            );
        }
    ...
    function transferAndBurn(uint256 _partyId, address _participant) private {
        (uint256 amount, uint256 twaAmount) = multiToken.balancesOf(
            _participant,
            _partyId
        );
        stableToken.safeTransferFrom(vaultAddress, _participant, amount);
        multiToken.burn(_participant, _partyId, amount, twaAmount);
        partiesList.decreaseBalance(_partyId, amount);
    }

    /**
     * @notice It generates a hash that can be used as unique partyId
     */
    function hashPartyId(
        address _owner,
        uint256 _minimumParticipants,
        uint256 _duration
    ...
    function _handleRewards(
        ...
        uint256 _partyTwaBalance,
        uint256 _randomNumber
    ) internal {
        uint256 minimumParticipants;
        (, , , , , minimumParticipants, , , , , ) = partiesList.get(_partyId);
        
        _distributeRewards(_partyId, _participants, _partyTwaBalance, _randomNumber);        

        if (minimumParticipants > 0) {
            for (uint256 i = 0; i < _participants.length; i++) {
                transferAndBurn(_partyId, _participants[i]);
            }

            partiesList.updateState(
                _partyId,
                PartiesStorageLibrary.PartyState.Finished
            );
            ...
            partiesList.updateCreatedBlock(_partyId);
            partiesList.updateState(
                _partyId,
                PartiesStorageLibrary.PartyState.Yielding
            );
    ...
    function _distributeRewards(uint256 _partyId, address[] memory _participants, uint256 _partyTwaBalance, uint256 _randomNumber) internal {
        ...
        for (uint256 i = 0; i < prizes.length; i++) {
            multiToken.mint(
                ...
                _partyId,
                prizes[i],
                multiToken.calculateTwaAmount(_partyId, winners[i], prizes[i]),
                ""
            );
            uint256 winnerPrize = multiToken.prizeOf(winners[i], _partyId).add(prizes[i]);
            multiToken.setPrize(winners[i], _partyId, winnerPrize);
            partiesList.increasePrizes(_partyId, prizes[i]);
            partiesList.increaseBalance(_partyId, prizes[i]);
        }

        uint256 prizesAmount;
        (, , , , , , , , , prizesAmount, ) = partiesList.get(_partyId);
        emit RewardsDistributed(_partyId, prizesAmount, winners, prizes);
    }
    ...
    function _handleParticipantWithdraw(
        ...
        require(
            ...
            "PartyManager: insufficient founds!"
        );

        // burning _amount + fee multi tokens from the participant balance
        multiToken.burn(_participant, _partyId, _amount, twaAmountToBeBurned);

        // transfering the participant _amount - fee rand stable tokens
        stableToken.safeTransferFrom(
            vaultAddress,
            _participant,
            _amount.sub(_feeAmount)
        );

        // decreasing the party balance
        partiesList.decreaseBalance(_partyId, _amount.sub(_feeAmount));

        multiToken.updateParticipantLastWithdrawDate(_partyId, _participant);

        // if the user doesn't have any token in the balance, it means
        // he left the party
        if (amount.sub(_amount) == 0) {
            partiesList.removeParticipant(_partyId, _participant);
            emit ParticipantLeftParty(_partyId, _participant);
        }
        multiToken.setReward(_participant, _partyId, 0);
        emit ParticipantWithdraw(_partyId, _participant, _amount);
    }

    /**
     * @dev Uses RandStableTokenInterface to burn the Rand internal tokens obtained when users
     * are joining to a party
    ...
    /**
     * @dev Checks if a party is open for the users to join
     * @param _state state of the party
     */
    function _isOpen(PartiesStorageLibrary.PartyState _state)
        internal
        pure
        returns (bool)
    {
        if (_state == PartiesStorageLibrary.PartyState.Rewarding) return false;
        if (_state == PartiesStorageLibrary.PartyState.Finished) return false;
        if (_state == PartiesStorageLibrary.PartyState.Cancelled) return false;
        return true;
    }
     ...
     * @param _state state of the party
     */
    function _isWaiting(PartiesStorageLibrary.PartyState _state)
        internal
        pure
        returns (bool)
    {
        if (_state == PartiesStorageLibrary.PartyState.Created) return false;
        if (_state == PartiesStorageLibrary.PartyState.Yielding) return false;
        if (_state == PartiesStorageLibrary.PartyState.Rewarding) return false;
        if (_state == PartiesStorageLibrary.PartyState.Finished) return false;
        if (_state == PartiesStorageLibrary.PartyState.Cancelled) return false;
        return true;
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            ...
            "Party Manager: Backend contract address cannot be address 0!"
        );
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            ...
            "Party Manager: Random Number Generator contract address cannot be address 0!"
        );
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            ...
            "Party Manager: Prize Strategy contract address cannot be address 0!"
        );
...
/**
 * @title Party Manager Interface
 * @author Keyko
 *
 * @dev Interfacefor the contract in charge of store and manage the different parties
 *      created by the community
 *
 */
interface IPartyManager {
    ...
    // Emitted when a new public party is created
    event PublicPartyCreated(
        uint256 indexed _partyId,
        uint256 _when,
        uint256 _duration
    );

    // Emitted when a party participant joins the party depositing funds
    event ParticipantJoinedParty(
        uint256 indexed _partyId,
        address indexed _participant
    );

    // Emitted when a party participant leaves the party by withdrawing all the funds
    event ParticipantLeftParty(
        uint256 indexed _partyId,
        address indexed _participant
    );

    // Emitted when a party participants withdraw an amount from the party
    ...
    // Emitted when the state of the party changed
    event PartyStateChanged(
        uint256 indexed _partyId,
        address indexed _who,
        PartiesStorageLibrary.PartyState _previousState,
        PartiesStorageLibrary.PartyState _newState
    );

    // Emitted when the rewards of a specific party are distributed correctly
    ...
    /**
     * @notice used to distribute the rewards for a specific party after the yielding
     *         has finished
     * @param _partyId the id of the party for which the rewards are being distributed
 ...
 * @author Keyko
 *
 * @dev ERC1155 contract where each token represents a party and the balances of the token represent
 *  the balances of the user for the party
 * @dev Each token has a normal balance and a time weighted one, the time weighted balance represents the
 * contribution of the user to the party
    ...
    PausableUpgradeable
{
    ...
    function initialize() external initializer {
        __ERC1155_init("https://rand.network/api/party/{id}.json");
        AccessControlUpgradeable.__AccessControl_init();
    ...
    /**
     * @notice Used to get the party prize won by an account, 0 in case
     * it didn't won anything
     * @param account the address of the users for which the prize is checked
     * @param id the id of the party for which the prize is checked
     */
    ...
    /**
     * @notice Used to update the date of the last withdraw for an user
     * @dev Called inside the participantWithdraw function of the Party Manager
     * @param id - id of the party
     ...
     * @return the block number when the user made laste withdraw from the party
     */
     ...
     * @dev Called inside the participantDeposit function of the Party Manager
     * @param id - id of the party
     ...
     * @return the block number when the user made laste deposit from the party
     */
 ...
 * @author Keyko
 *
 * @dev Provides an interface for requesting random numbers from 3rd-party
...
interface IRandomNumberGenerator {
     ...
     * @param partyId The indexed ID of the party for which random number request has been made
     ...
     * @param partyId The ID of the party for which random number request has been made
     ...
     */
    function requestRandomNumber(uint256 partyId)
        external
     ...
     */
    function isRequestComplete(uint256 partyId)
        external
        view
     ...
     * @param _partyId The ID of the party for which the 'lockBlock' is returned
     * @return lockBlock The block number at which the RNG service will start generating time-delayed randomness.
     * The calling contract should "lock" all activity until the result is available via the `requestId`
     */
    function getLockBlock(uint256 _partyId)
        external
        view
     ...
     */
    function getRandomNumber(uint256 partyId)
        external
        view
...
interface IPrizeStrategy {
     ...
     * @param partyId id of the party for which the winners are calculated
     * @param winners array containing the winners addresses 
     * @param winnersTiers array containing the tiers that the winners won
     */
    event Winners(uint256 partyId, address[] winners, uint256[] winnersTiers);

    /**
     * @notice Emitted after winners prizes are calculated inside 'getWinnersPrizes'
     * @param partyId id of the party for which the winners prizes are calculated
     * @param prizes array containing the prizes for the winners
     */
    event WinnersPrizes(uint256 partyId, uint256[] prizes);

    /**
     * @notice Used to set up the parameters necessary for calculating the prizes for a party
     ...
     * their balance in the party since the last prize distribution
     * @param partyId the id of the party for which the rewards are being computed
     * @param users array of the participants of the party
     */
    ...
    /**
     * @notice Used to calculate the winners address and the tier that they won after the party
     * has finished
     * @param _partyId id of the party for which the winners are calculated
     * @param _users array of the addresses of the users which were in the party
    ...
    function getWinners(
        ...
        uint256 _randomNumber
    ) external returns (address[] memory winners, uint256[] memory winnersTiers);

    /**
     * @notice Used to calculate the winners prizes based on the tire that they have won
     * @param _partyId id of the party for which the winners prizes are calculated
     ...
     */
    function getWinnersPrizes(
        uint256 _partyId,
        uint256[] memory _tiers
    ) external returns (uint256[] memory winnerPrizes);
}
...
library PartiesStorageLibrary {
    ...
    function removeParticipant(
        ...
        for(uint256 i = 0; i < _self.participants[_partyId].length; i++) {
            if(_self.participants[_partyId][i] == _participant) {
                _self.participants[_partyId][i] = _self.participants[_partyId][_self.participants[_partyId].length - 1];
                _self.participants[_partyId].pop();
                break;
            }
        }
    }
        ...
        )
    {
        ...
        prizesAmount = party.prizesAmount;
        lockDuration = party.lockDuration;
    }
    ...
    function updateLockDuration(PartiesList storage _self, uint256 _partyId, uint256 _lockDuration)
        internal
    {
        _self.parties[_partyId].lockDuration = _lockDuration;
    }
    ...
    function increasePrizes(
        ...
        _self.parties[_partyId].prizesAmount += _amount;
    }
