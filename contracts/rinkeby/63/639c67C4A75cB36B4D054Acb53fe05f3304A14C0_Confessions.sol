...
contract Confessions is ERC721Pausable, ReentrancyGuard, Ownable {
  ...
  uint256 private constant MAX_TEXT_LENGTH = 200;

  constructor() ERC721("Confession", "CON") Ownable() {
    mintCost = 0.1 ether;
  }
  ...
  // We'll do our best to limit the length through primitive byte check
  function mintConfession(string[] calldata _text) public payable nonReentrant {
    require(msg.value == mintCost, "Invalid payment sent");
    for (uint256 index = 0; index < _text.length; index++) {
      string memory line = _text[index];
      require(
        bytes(line).length < MAX_TEXT_LENGTH,
        "Text line is too long. Are you minting from contract?"
      );
    }

    _safeMint(msg.sender, confessionsSize);
    confessions[confessionsSize] = Confession(_text, msg.sender);
    confessionsSize++;
  }
  ...
  function getSVG(uint256 tokenId) public view returns (string memory) {
    ...
    string
      memory svg = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="##3264a8" />';

    svg = string(
      abi.encodePacked(
        svg,
        '<text x="10" y="40" class="base">0x',
