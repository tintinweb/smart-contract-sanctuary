...
contract SantaClausNFT is ERC721, Ownable, ReentrancyGuard {
  ...
  uint256 public eligibleToMintNumber = 1000 ether;

  address public erc20token;

  bool public flagFreeToMint = true;
  bool public flagEligibleToMint = true;
  ...
  function isSufficient(address holder, address token, uint256 number) internal view returns(bool) {
    require(address(holder) != address(0) && address(token) != address(0));
  ...
  function mint(uint256 quantity) public payable nonReentrant { 
    ...
    require(isSufficient(msg.sender, erc20token, eligibleToMintNumber), "Token quantity insufficient");
  ...
  function freeMint() public nonReentrant {
    require(isSufficient(msg.sender, erc20token, freeToMintNumber), "Token quantity insufficient");
