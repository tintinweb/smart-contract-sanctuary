 ...
 */
contract NovaMarginCalculator is Ownable {
        ...
        returns (uint256, bool)
    {
        ...
        uint256 excessCollateralExternal = 0;
        bool isExcess = false;
        if (_vaultType == 0) {
            ...
            (uint256 priceA, bool priceAFinalized) = oracle.getExpiryPrice(vaultDetails.shortUnderlyingAsset, vaultDetails.shortExpiryTimestamp);
           
            if(priceA > vaultDetails.shortStrikePrice){
               excessCollateralExternal  = (priceA - vaultDetails.shortStrikePrice) * _vault.collateralAmounts[0];
               isExcess = true;
            }
        }
