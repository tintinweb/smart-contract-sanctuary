...
contract BullAlphaEth is Pausable, AlphaTokenEth {
    ...
    uint256 public ALPHA_TOKEN_VALUE = 0;
    uint256 public HWM = 0;
    ...
    event LogChangeHwm(uint256 _hwm);
    ...
    event LogClaimableFeesMinted(uint256 CLAIMABLE_FEE);
    event LogHwmFeesMinted(uint256 _amount);
    ...
    */
    ...
    function getTVL() public view returns(uint256) {
        return ALPHA_TOKEN_VALUE * totalSupply() * COEFF_SCALE_WEI;
    }
    ...
    */
    ...
    function setHwm(uint256 _hwm) external {
        require(msg.sender == manager, "Alpha: unauthorized");
        HWM = COEFF_SCALE_WEI * _hwm;
        emit LogChangeHwm(_hwm);
    }
    ...
    function mintClaimableFees() external {
        require(msg.sender == manager, "Alpha: incorrect receiver");
        _mint(msg.sender, CLAIMABLE_FEE);
        CLAIMABLE_FEE = 0;
        emit LogClaimableFeesMinted(CLAIMABLE_FEE);
    }

    function mintHwmFees(uint256 _amount) external {
        require(msg.sender == manager, "Alpha: incorrect receiver");
        require(HWM > ALPHA_TOKEN_VALUE, "Alpha: fee not applicable");
        _mint(msg.sender, _amount);
        HWM = ALPHA_TOKEN_VALUE;
        emit LogHwmFeesMinted(_amount);
    }
    ...
    function _deposit(uint256 _amount) internal {
        ...
        deposits[msg.sender].watermark = ALPHA_TOKEN_VALUE; // new deposit = new watermark

        uint256 _amountAlpha = ALPHA_TOKEN_VALUE * _amount;
    ...
    function _requestWithdrawal(uint256 _amount) internal {
        ...
        uint256 stableValueAfterFees = _netAmountAlpha / ALPHA_TOKEN_VALUE;
