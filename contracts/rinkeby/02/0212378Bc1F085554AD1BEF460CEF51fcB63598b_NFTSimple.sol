...
contract NFTSimple is VRFConsumerBase, ERC721 {
    ...
    */
     ...
     */
    function createCollectibles(uint256 amount, string memory _tokenURI, uint256 userProvidedSeed) public returns (bytes32 requestId) {
    ...
    function getTransactionFromIndex(uint256 _index) public view returns(uint256, Breed, string memory ){
        uint256 tokenId = tokenByIndex(_index);
        Breed breed = Breed(tokenIdToBreed[tokenId]);
        string memory URI = indexToTokenURI[_index];
        return (tokenId, breed, URI);
    }
    /**
