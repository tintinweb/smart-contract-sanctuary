...
contract GamingChickNest is OwnableContract{
    ...
    function updateRarityRareNumberMap(uint32 a, uint32[] memory b) public onlyAdmin{
        rareNumberMap[a] = b;
    }
    ...
    function getAttributes(uint256 _tokenId) public view returns(HeadPortrait721Interface.Attribute memory attribute){
        ...
        attribute.skinColor = skinColorArray[uint32(random % skinColorArray.length)] * 10000 + 3 * 100;
        ...
        if(beakRarity != 0 && (random % 1000) < backgroundRarity){
            ...
            if(skinColorAndPartColorBeakMap[beakArray[random % beakArray.length]]){
                if(attribute.hatFront != 0 && isDependentHatFront[key]){
                    attribute.beak = beakArray[uint32(random % beakArray.length)] * 10000 + hatFrontLastThird * 1000 + three * 100;
                }else{
                    random = uint256(keccak256(abi.encode(random, 16)));
                    attribute.beak = beakArray[uint32(random % beakArray.length)] * 10000 + uint32(random % partColorCount + 1) * 1000 + three * 100;
                }
            }else{
                if(attribute.hatFront != 0 && isDependentHatFront[key]){
                    attribute.beak = beakArray[uint32(random % beakArray.length)] * 10000 + hatFrontLastThird * 1000;
                }else{
                    random = uint256(keccak256(abi.encode(random, 16)));
                    attribute.beak = beakArray[uint32(random % beakArray.length)] * 10000 + uint32(random % partColorCount + 1) * 1000;
                }
            }
        }
        // EYES
    ...
    function refreshRandom(uint256 _random, uint256 i) internal pure returns(uint256){
        uint256 random = uint256(keccak256(abi.encode(_random, i)));
        return random;
    }
