...
contract OpenExhibitionToken is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    function excludeFromFeeAndRewards(address account) public onlyOwner {
   ...
   function setCashbackFeePercent(uint256 cashbackFee) external onlyOwner {
    ...
    function setLiquifyEnabled(bool enabled) public onlyOwner {
        _liquifyEnabled = enabled;
        emit LiquifyEnabledUpdated(enabled);
    }
    
    function setMinTokenForLiquify(uint256 amount) public onlyOwner {
    ...
    function calculateCashback(uint256 amount) private view returns (uint256) {
    ...
    function _cashback(uint256 tAmount, uint256 rAmount, address sender) private {
        if (_msgSender() == _nftExchangeAddress) {
            ...
            uint256 cbRAmount = calculateCashback(rAmount);
            ...
            if (_rOwned[address(this)].sub(_rTotalForLiquidity) >= cbRAmount) {
                ...
                uint256 cbTAmount = calculateCashback(tAmount);
