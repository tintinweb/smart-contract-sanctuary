...
contract Auction is Owned {
    ...
    event Logs(address indexed _from, uint256 indexed _amount, uint256 _price_current,  uint256  _block_time, uint256  _time_start, uint256 indexed _index);
    ...
    receive() external payable  {
        if(msg.value > price_current &&  block.timestamp > time_start){
            records[record_id] = Record(record_id, msg.sender, msg.value, block.timestamp);
            record_id++;
            record_count = record_id;
            price_current = msg.value; 
            emit Logs(msg.sender, msg.value, price_current, block.timestamp, time_start, record_id);
            ...
            revert("Can't create bid auction");
        }
    }
