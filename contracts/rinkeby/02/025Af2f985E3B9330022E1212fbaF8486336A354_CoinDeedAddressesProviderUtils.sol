...
library CoinDeedAddressesProviderUtils {
    ...
    /** Helps the buy function after management fees are deducted.
      ...
      * @dev Relies on the lending pool to bubble an error if there's no liquidiity. */
    ...
    ) external
    {
        ...
        IDao dao = IDao(coinDeedAddressesProvider.dao());
        ...
        if (state == ICoinDeed.DeedState.CLOSED) {
            ...
            uint256 amount;
            if (pair.tokenA == address(0x00)) {
                amount = dao.claimCoinDeedManagementFee{value: totalManagementFee}(pair.tokenA, totalManagementFee);
            } else {
                if (pair.tokenA == USDT_ADDRESS) {
                    IERC20(pair.tokenA).safeApprove(address(dao), 0);
                }
                IERC20(pair.tokenA).safeApprove(address(dao), totalManagementFee);
                amount = dao.claimCoinDeedManagementFee(pair.tokenA, totalManagementFee);
            }
        ...
        if (payOff) {
            ...
            if (amountReceived > userBorrow) {
                ...
                if (pair.tokenA == address(0x00)) {
                    ...
                    token.transfer(msg.sender, amountReceived - userBorrow);
                }
            }
 ...
 */
interface IDao {
    /// @notice Info of each TokenOracle.
    struct TokenOracelInfo {
        IOracle oracle;
    ...
    /**
     * @notice This function will assign oracle link to get the price
     * @dev It will take the oracle address according to the token to get the price
     * This function take 3 parameters {_tokenAddress}, {_tokenOracle} and {_oracleDecimals}
     */
    function addOracleForToken(
        address _token,
