...
/**
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        // If both are set, conmightykaijuenate the baseURI and tokenURI (via abi.encodePacked).
        if (bytes(_tokenURI).length > 0) {
            return string(abi.encodePacked(base, _tokenURI));
        }
        // If there is a baseURI but no tokenURI, conmightykaijuenate the tokenID to the baseURI.
...
contract SkullIsland is Ownable, IERC721Receiver {
    ...
    // kaiju earn 10000 $GGamma per day
    ...
    uint public constant MAXIMUM_GLOBAL_Gamma = 2400000000 ether;
    ...
    // emergency rescue to allow unstaking without any checks but without $GGamma
    ...
    constructor() {
        ...
        _randomSource[7] = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
        _randomSource[8] = 0xf66852bC122fD40bFECc63CD48217E88bda12109;
        _randomSource[9] = 0xEA674fdDe714fd979de3EdF0F56AA9716B898ec8; 
    }
    ...
    function getAccountkaijuIDs(address user) external view returns (uint16[] memory) {
        uint16[] memory kaijuStakeIDs;
        for (uint i = 0; i < kaijuStake[user].length; i++) {
            kaijuStakeIDs[i] = kaijuStake[user][i].tokenId;
        }
        return kaijuStakeIDs;
    }
    ...
    function getAccountKongIDs(address user) external view returns (uint16[] memory) {
        uint16[] memory KongStakeIDs;
        for (uint i = 0; i < KongStake[user].length; i++) {
            KongStakeIDs[i] = KongStake[user][i].tokenId;
        }
        return KongStakeIDs;
    }
    ...
    function addTokensToStake(address account, uint16[] calldata tokenIds) external {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            if (msg.sender != address(KongKaiju)) {
                ...
                require(KongKaiju.ownerOf(tokenIds[i]) == msg.sender, "This NTF does not belong to address");
    ...
    function _claimFromKaiju(uint16 tokenId, bool unstake) internal returns (uint owed) {
        ...
        if (totalGammaEarned < MAXIMUM_GLOBAL_Gamma) {
            ...
            owed = 0; // $GGamma production stopped already
        } else {
            owed = ((lastClaimTimestamp - stake.value) * DAILY_Gamma_RATE) / 1 days; // stop earning additional $GGamma if it's all been earned
        }
        if (unstake) {
            ...
            _payTax((owed * NewTAX_PERCENTAGE) / 100); // Pay some $GGamma to Kongs!
    ...
    function _claimFromKong(uint16 tokenId, bool unstake) internal returns (uint owed) {
        require(KongKaiju.ownerOf(tokenId) == address(this), "This NTF does not belong to address");
        ...
        if (unstake) {
            totalKongStaked -= 1; // Remove Alpha from total staked
