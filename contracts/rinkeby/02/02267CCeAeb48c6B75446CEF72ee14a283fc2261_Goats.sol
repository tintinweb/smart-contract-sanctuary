 ...
 *   ██████   ██████  ██   ██    ██    ███████
 */
contract Goats is ERC721Enumerable, Ownable, ReentrancyGuard {
	...
	IERC721 public featuredProject;
	uint256 public featuredProjectPrice = 30000000000000000; //0.03 ETH each

	uint256 public priceForLessThan3 = 45000000000000000; //0.045 ETH each
	uint256 public priceForLessThan10 = 40000000000000000; //0.04 ETH each
	uint256 public priceForGreaterThan10 = 35000000000000000; //0.035 ETH each

	uint256 public changeNamePrice = 10000000000000000; //0.01 ETH each
	...
	event NameChange(uint256 nftID, string payload);

	constructor() ERC721("Goat Gang", "GOAT") {}
	...
	/*
	 * buy via a featured project
	 */
	function featuredBuy() public payable nonReentrant {
		require(
			featuredProject.balanceOf(msg.sender) > 0,
			"you have to own an NFT in the featured project to do this action"
		);
		require(maxToMint - tokenIndex > 0, "greater than max supply");
		require(isMintingActive, "minting is not active");

		require(featuredProjectPrice == msg.value, "exact value in ETH needed");
	...
	/*
	 * In case tokens are not sold...giveaways, airdrops etc
	 */
	...
	function _mintToken(address destinationAddress) private {
		tokenIndex++;
		require(!_exists(tokenIndex), "already exist");
	 ...
	 * change name
	 */
	function changeName(uint256 nftID, string memory newName) external payable {
		require(ownerOf(nftID) == msg.sender, "ownership required");
		require(changeNamePrice == msg.value, "exact value in ETH needed");
		emit NameChange(nftID, newName);
	}
	...
	function setFeaturedProject(address contractAddress) public onlyOwner {
		featuredProject = IERC721(contractAddress);
	}

	//used by admin to modify the price for the featured project
	function modifyFeaturedProjectPrice(uint256 _newPrice) public onlyOwner {
		featuredProjectPrice = _newPrice;
	}

	//used by admin to modify the price for name change
	function modifyPriceChangeName(uint256 _newPrice) public onlyOwner {
