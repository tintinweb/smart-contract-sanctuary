...
/**
 * @title Milky Way NFT
 * @notice Milky Way NFT collection
 */
contract MwNFT is ERC721Enumerable, Ownable {
    ...
    /**
     * @notice Returns a list of token URIs of NFTs owned by `user`
     * @param user: address
     */
    function tokensOfOwnerURI(address user)
        external
        view
        returns (string[] memory)
    {
        uint256 maxIndex = balanceOf(user);
        string[] memory tokensURIs = new string[](maxIndex);
        for (uint256 i = 0; i < maxIndex; i++) {
            uint256 nftIndex = tokenOfOwnerByIndex(user, i);
            tokensURIs[i] = tokenURI(nftIndex);
        }
        return tokensURIs;
    }
