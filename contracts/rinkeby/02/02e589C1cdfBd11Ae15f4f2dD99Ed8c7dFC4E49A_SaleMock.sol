...
contract Sale is Ownable, AccessControl {
     ...
     * @return total users payments done at the sale
     */

    function getSaleBalanceRest(uint256 saleID, address userID) public view returns (uint256 balanceRest, uint256) {
        balanceRest = (
            sales[saleID].staking.getCurrentStake(userID).mul(1e9).div(sales[saleID].staking.getCurrentTotalStake())
        );
        return (
