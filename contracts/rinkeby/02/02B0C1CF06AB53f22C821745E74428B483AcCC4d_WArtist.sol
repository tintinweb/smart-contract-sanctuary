	...
	VRFConsumerBaseUpgradeable
{
	...
	mapping(uint8 => uint16) private bodyPartCounts;
	...
	mapping(bytes32 => Artist) public requestToArtist;
	...
	function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override {
		Body memory body = Body(
			uint16(((randomNumber % 10000) * bodyPartCounts[uint8(BodyCategory.ARM)]) / 10000 + 1),
			uint16(((randomNumber % 100000) * bodyPartCounts[uint8(BodyCategory.EYE)]) / 100000 + 1),
			uint16(((randomNumber % 1000000) * bodyPartCounts[uint8(BodyCategory.HAIR)]) / 1000000 + 1),
			uint16(((randomNumber % 10000000) * bodyPartCounts[uint8(BodyCategory.HEAD)]) / 10000000 + 1),
			uint16(((randomNumber % 100000000) * bodyPartCounts[uint8(BodyCategory.MOUTH)]) / 100000000 + 1),
			uint16(((randomNumber % 1000000000) * bodyPartCounts[uint8(BodyCategory.OUTFIT)]) / 1000000000 + 1),
			uint16(((randomNumber % 10000000000) * bodyPartCounts[uint8(BodyCategory.SOUL)]) / 10000000000 + 1)
		);

		uint8 totalRarity = uint8(
			(body.arm + body.eye + body.mouth + body.hair + body.head + body.outfit + body.soul) / 7
		);
	...
	function getBodyPartCounts(BodyCategory category) external view returns (uint16) {
		return bodyPartCounts[uint8(category)];
	}
