...
contract Auction is ERC721Upgradeable, OwnableUpgradeable {
    ...
    function _startAuction(address pool) internal {
        require(IPoolMaster(pool).state() == IPoolMaster.State.Default, "NID");
        require(isWhitelistedBidder[msg.sender], "NWB");

        IPoolMaster(pool).transferReserves();
        auctionInfo[pool].end = block.timestamp + factory.auctionDuration();

        emit AuctionStarted(pool, msg.sender);
    }
}
