...
contract DateMinter is Ownable {
  ...
  // price curve variables
  uint256 public curStartingPrice;
  uint256 public curPricePerMint;
  uint256 public curFlatPriceUpTo;
  ...
  mapping(int256 => uint256) public timestampToTokenId;
  ...
  constructor(
    ...
    uint256 _tokenType,
    uint256 _startingPrice,     // 0.05ETH
    uint256 _pricePerMint,      // 0.002ETH
    uint256 _flatPriceUpTo,     // 1000
    uint256 _curMaxSupplyLimit, // 2021
    ...
    curStartingPrice = _startingPrice;
    curPricePerMint = _pricePerMint;
    curFlatPriceUpTo = _flatPriceUpTo;
  ...
  */
  function getPrice() public view returns (uint256) {
    ...
    if (curTokenCount < curFlatPriceUpTo) {
      return curStartingPrice;
    }
    return curTokenCount.safeAdd(1).safeSub(curFlatPriceUpTo).safeMul(curPricePerMint).safeAdd(curStartingPrice); 
  }
  ...
  */
  ...
  /**
  * @dev Set starting price of the flat portion of the pricing curve.
  */
  function setCurStartingPrice(uint256 _curStartingPrice) external onlyOwner() {
    curStartingPrice = _curStartingPrice;
  }

  /**
  * @dev Set the number of tokens that can be minted at the flat rate before it goes up.
  */
  function setCurFlatPriceUpTo(uint256 _curFlatPriceUpTo) external onlyOwner() {
    curFlatPriceUpTo = _curFlatPriceUpTo;
  }

  /**
  * @dev Set the price increase per mint after the flat price phase.
  */
  function setCurPricePerMint(uint256 _curPricePerMint) external onlyOwner() {
    curPricePerMint = _curPricePerMint;
  }

  /**
  * @dev Mint a given date (input is a timestamp)
  */
  function mint(address _dst, int256 _dateTimestamp) public payable {
    uint256 curTokenCount = generationToTokenCount[curGeneration];
    require(curTokenCount < curMaxSupplyLimit, "Sale has already ended");

    // check if not minted
    require(timestampToTokenId[_dateTimestamp] == 0, 'timestamp already exists');
    ...
    require(allowFutureDates || !allowFutureDates && int256(block.timestamp) >= _dateTimestamp, "Cannot mint dates in the future");
    ...
    dt.timestamp = _dateTimestamp;
    dt.generation = curGeneration;

    tokenIdToTimestamp[tokenId] = dt;
    timestampToTokenId[_dateTimestamp] = tokenId;
    generationToTokenCount[curGeneration] += 1;

    emit UpdatedRegistry(tokenId, _dateTimestamp);
  }
}
...
library LibDateMath {
    ...
    */
    ...
    // ------------------------------------------------------------------------
    function _daysToDate(int _days) internal pure returns (uint year, uint month, uint day) {
    ...
    function timestampToDate(int256 timestamp) internal pure returns (uint year, uint month, uint day) {
    ...
    function getDateAsString(int256 timestamp) internal pure returns (string memory) {
        ...
        if (month < 10) {
            monthStr = LibString.strConcat("0", uint2str(month));
        } else {
            monthStr = uint2str(month);
        }
        if (day < 10) {
            dayStr = LibString.strConcat("0", uint2str(day));
        } else {
            dayStr = uint2str(day);
        }
        return LibString.strConcat(LibString.strConcat(monthStr, "/", dayStr, "/"), uint2str(year));
    }
