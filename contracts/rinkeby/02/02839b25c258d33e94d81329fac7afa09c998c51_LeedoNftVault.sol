...
contract LeedoNftVault is ERC721Enumerable, Ownable { 
    ...
    //address public nftAddr = 0xed8a775Fde8454c99Eb4e511D7D4D6b969024C5d; //mainnet
    address public nftAddr = 0xf1E04195aeFA3F8E979f8b8467D30B81a9bA46E2;
    address public erc20Addr = 0xf1E04195aeFA3F8E979f8b8467D30B81a9bA46E2; 
    address public bridgeAddr = 0x8A93e4B4B5aB9ECA75D5ED976C58341d8FdEA6e8;
    address public daoAddr;
    
    uint public expiration = block.number + 365*2 days;
    ...
    function stake(uint256[] calldata tokenIds) external returns (bool) {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            require(!_exists(tokenId), 'SquidVault: The tokenId already exists');
            require(nft.ownerOf(tokenId) == _msgSender(), 'Owner is differenent' );
            ...
            require(safeMint(_msgSender(), tokenId), 'SquidValut: Staking failed');
        }
        return true;
    }
    ...
    function stakeByCount(uint256 count) external returns (uint256[] memory){
        ...
        require(count <= nft.balanceOf(_msgSender()), 'Count is more than the owner has');
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            require(safeMint(_msgSender(), tokenIds[i]), 'SquidVault: Staking failed');
        }
        return tokenIds;
    }       
    ...
    function withdraw(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            uint tokenId = tokenIds[i];
            require(ownerOf(tokenId) == _msgSender(), 'SquidVault: LP owner is different');            
            _burn(tokenId);
            ILeedoNft(nftAddr).safeTransferFrom(
                address(this),
                _msgSender(),
