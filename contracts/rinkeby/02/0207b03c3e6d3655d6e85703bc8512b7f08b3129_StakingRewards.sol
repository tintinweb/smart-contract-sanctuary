...
contract StakingRewards is OlympusAccessControlled, ReentrancyGuard {
    ...
    function earned(address account) public returns (uint256) {
        ...
        uint256 newEarned = _userInfos[account].tazorNum.mul(1).mul(_apr).div(10 ** 9); // reward TAZ amount
    ...
    function stakeTaz(uint256 amount) external nonReentrant updateReward(msg.sender) {
        ...
        UserInfo memory user = _userInfos[msg.sender];
        user.tazNum = user.tazNum.add(amount);
