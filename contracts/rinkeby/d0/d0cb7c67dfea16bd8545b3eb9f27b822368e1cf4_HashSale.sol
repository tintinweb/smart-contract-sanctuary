...
contract HashSale is Ownable {
	...
	event NewSale(uint indexed saleId, address indexed customerAddress, uint payAmount, uint tokenAmount, string referral, uint8 stage, uint stageSales, bool isSaleFinished);
	...
	function buyTokens(bytes calldata _agreementSignature, string calldata _referral) payable public {
		...
		emit NewSale(saleId, msg.sender, msg.value, tokenAmount, _referral, currentStage, stages[currentStage].tokensSold, isFinished);
	}
