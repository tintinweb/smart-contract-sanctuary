...
// File: contracts/mining/LPMining.sol
...
contract LPMining is Ownable {
    ...
    function pendingToken(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accTokenPerShare = accTokenPerShare.add(tokenReward.mul(1e30).div(lpSupply));
        }
        return user.amount.mul(accTokenPerShare).div(1e30).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accTokenPerShare = pool.accTokenPerShare.add(tokenReward.mul(1e30).div(lpSupply));
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accTokenPerShare).div(1e30).sub(user.rewardDebt);
            uint256 fee = pending.mul(harvestFeeRate).div(100);
            safeTokenTransfer(feeAddr, fee);
            // 扣除手续费后获得 pending 值
        ...
        user.rewardDebt = user.amount.mul(pool.accTokenPerShare).div(1e30);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        safeTokenTransfer(feeAddr, harvestFee);
        // 扣除手续费后获得 pending 值
        ...
        if (pool.lpToken == token) {
            safeTokenTransfer(address(msg.sender), fund);
        } else {
            pool.lpToken.safeTransfer(address(msg.sender), fund);
        }
