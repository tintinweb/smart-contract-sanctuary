...
// File contracts/solidity/token/RewardDistributionTokenUpgradeable.sol
...
contract RewardDistributionTokenUpgradeable is OwnableUpgradeable, ERC20Upgradeable, IRewardDistributionToken {
  ...
  function distributeRewards(uint amount) external override virtual onlyOwner {
...
contract NFTXLPStaking is PausableUpgradeable {
    ...
    function depositFor(uint256 vaultId, uint256 amount, address to) external {
        ...
        // Check the pool in case its been updated.
        updatePoolForVault(vaultId);
        withdrawTime[to] = block.timestamp + 24 hours;
        StakingPool memory pool = vaultStakingInfo[vaultId];
        _deposit(pool, amount, to);
    }
    ...
    function _deposit(StakingPool memory pool, uint256 amount, address account) internal {
        ...
        _rewardDistributionTokenAddr(pool).mint(account, amount);
    }
    ...
    function _withdraw(StakingPool memory pool, uint256 amount, address account) internal {
        require(pool.stakingToken != address(0), "LPStaking: Nonexistent pool");
        require(withdrawTime[account] < block.timestamp, "Still timelock");
