...
contract Treasury is ContractGuard {
    ...
    address[] public excludedFromTotalSupply = [
        address(0xB20D46757D1DCFbAa18D2d4eA26D7EfD810464dd), // GalaxyGenesisPool
        address(0x8Fc7c6C7A26155CBE7e34bbFae56A05eCFB23627) // new GalaxyRewardPool
        //address(0xA7B16703470055881e7EE093e9b0bF537f29CD4d) // old GalaxyRewardPool
    ];
    ...
    // 28 first epochs (1 week) with 4.5% expansion regardless of GXY price
    ...
    uint256 public mintingFactorForPayingDebt; // print extra GXY during debt phase
    ...
    function getGalaxyPrice() public view returns (uint256 galaxyPrice) {
        try IOracle(galaxyOracle).consult(galaxy, 1e18) returns (uint144 price) {
            ...
            revert("Treasury: failed to consult GXY price from the oracle");
        }
    }
    ...
    function getRedeemableBonds() public view returns (uint256 _redeemableBonds) {
        ...
        if (_galaxyPrice > galaxyPriceCeiling) {
            uint256 _totalGalaxy = IERC20(galaxy).balanceOf(address(this));
            uint256 _rate = getBondPremiumRate();
            if (_rate > 0) {
                _redeemableBonds = _totalGalaxy.mul(1e18).div(_rate);
            }
        }
    }
    ...
    function getBondDiscountRate() public view returns (uint256 _rate) {
        ...
        if (_galaxyPrice <= galaxyPriceOne) {
            if (discountPercent == 0) {
                ...
                uint256 _bondAmount = galaxyPriceOne.mul(1e18).div(_galaxyPrice); // to burn 1 GXY
    ...
    function initialize(
        ...
        galaxyPriceOne = 10**18;
        galaxyPriceCeiling = galaxyPriceOne.mul(101).div(100);

        // Dynamic max expansion percent
        supplyTiers = [0 ether, 50000 ether, 100000 ether, 150000 ether, 200000 ether, 500000 ether, 1000000 ether, 2000000 ether, 5000000 ether];
        ...
        maxSupplyContractionPercent = 300; // Upto 3.0% supply for contraction (to burn GXY and mint gBOND)
    ...
    function buyBonds(uint256 _galaxyAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        require(galaxyPrice == targetPrice, "Treasury: GXY price moved");
        require(
    ...
    function _sendToMasonry(uint256 _amount) internal {
        ...
        IERC20(galaxy).safeApprove(masonry, 0);
        IERC20(galaxy).safeApprove(masonry, _amount);
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs) {
            ...
            _sendToMasonry(galaxySupply.mul(bootstrapSupplyExpansionPercent).div(10000));
        } else {
            if (previousEpochGalaxyPrice > galaxyPriceCeiling) {
                // Expansion ($GXY Price > 1 $FTM): there is some seigniorage to be allocated
                ...
                if (seigniorageSaved >= bondSupply.mul(bondDepletionFloorPercent).div(10000)) {
                    ...
                    _savedForMasonry = galaxySupply.mul(_percentage).div(1e18);
                } else {
