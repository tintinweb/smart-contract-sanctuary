...
contract BounceDAO is ERC721Enumerable, Ownable, Functions, Metadata {
    using Strings for uint256;

    uint256 public constant RESERVE = 1;
    uint256 public constant PUBLIC = 1999;
    ...
    constructor() ERC721("BounceDAO", "BDAO") {}
...
function mintWhitelist(uint256 numberOfTokens) external override payable {
    ...
    require(numberOfTokens <= PURCHASE_LIMIT, 'Cannot purchase this many tokens');
    require(totalPublicSupply + numberOfTokens <= PUBLIC, 'Purchase would exceed max supply');
    require(_WhitelistClaimed[msg.sender] + numberOfTokens <= PURCHASE_LIMIT, 'Purchase exceeds max allowed');
    require(PRICE * numberOfTokens <= msg.value, 'ETH amount is not sufficient');
