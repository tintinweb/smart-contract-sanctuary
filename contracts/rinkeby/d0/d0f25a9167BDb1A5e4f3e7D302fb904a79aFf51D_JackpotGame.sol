...
contract JackpotGame is Ownable, ReentrancyGuard {
  ...
  uint256 public winnerAwardFactor = 9500;
  uint256 public marketFeeFactor = 100;
  uint256 public buyBackFeeFactor = 300;
  uint256 public gameFeeFactor = 100;
  ...
  function updateFeeFactors(
    ...
    winnerAwardFactor = winnerAwardFactor_;
    marketFeeFactor = marketFeeFactor_;
    buyBackFeeFactor = buyBackFeeFactor_;
    gameFeeFactor = gameFeeFactor_;
  }
  ...
  function ticketNumber() external view returns (uint256) {
    uint256 result = (_betMapping[msg.sender].endTicket -
      _betMapping[msg.sender].startTicket) / 1;
    return result;
  }
  ...
  function endGame() external payable onlyOwner gameEnded nonReentrant {
  ...
  function _awardShares() private {
    ...
    uint256 winnerAwardAmount = (address(this).balance * winnerAwardFactor) /
      SHARES_DENOMINATOR;
    uint256 marketFeeAmount = (address(this).balance * marketFeeFactor) /
      SHARES_DENOMINATOR;
    uint256 buyBackFeeAmount = (address(this).balance * buyBackFeeFactor) /
      SHARES_DENOMINATOR;
