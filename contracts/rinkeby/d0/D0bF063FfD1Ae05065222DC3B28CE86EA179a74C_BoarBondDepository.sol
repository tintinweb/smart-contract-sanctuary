...
contract BoarBondDepository is Ownable {
  ...
  address public immutable BOAR; // token given as payment for bond
  address public immutable principle; // token used to create bond
  address public immutable treasury; // mints BOAR when receives principle
  ...
  struct Bond {
    uint256 payout; // BOAR remaining to be paid
  ...
  function deposit(
    ...
    require(payout >= 10000000, "Bond too small"); // must be > 0.01 BOAR ( underflow protection )
    require(payout <= maxPayout(), "Bond too large"); // size protection because there is no slippage

    // profits are calculated
    uint256 fee = payout.mul(terms.fee).div(10000);
    uint256 feePartner = payout.mul(terms.feePartner).div(10000);
    uint256 profit = value.sub(payout).sub(fee).sub(feePartner);

    /**
            principle is transferred in
            approved and
            deposited into the treasury, returning (_amount - profit) BOAR
         */
    ...
    if (fee != 0) {
      ...
      IERC20(BOAR).safeTransfer(DAO, fee);
    }

    if (feePartner != 0) {
      // fee is transferred to partner dao
      IERC20(BOAR).safeTransfer(partnerDAO, feePartner);
    }
  ...
  function stakeOrSend(
    ...
    if (!_stake) {
      ...
      IERC20(BOAR).transfer(_recipient, _amount); // send payout
    } else {
      // if user wants to stake
      if (useHelper) {
        // use if staking warmup is 0
        IERC20(BOAR).approve(stakingHelper, _amount);
        IStakingHelper(stakingHelper).stake(_amount, _recipient);
      } else {
        IERC20(BOAR).approve(staking, _amount);
  ...
  function maxPayout() public view returns (uint256) {
    return IERC20(BOAR).totalSupply().mul(terms.maxPayout).div(100000);
  }
  ...
  /**
   *  @notice calculate current ratio of debt to BOAR supply
   *  @return debtRatio_ uint
   */
  function debtRatio() public view returns (uint256 debtRatio_) {
    uint256 supply = IERC20(BOAR).totalSupply();
  ...
  /**
   *  @notice calculate amount of BOAR available for claim by depositor
   *  @param _depositor address
  ...
  /**
   *  @notice allow anyone to send lost tokens (excluding principle or BOAR) to the DAO
   *  @return bool
   */
