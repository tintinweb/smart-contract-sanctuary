 ...
 */
contract Jelly is JellyTokenAccess, ERC20, Pausable, ERC20Permit {
    ...
    */
    ...
    constructor(string memory name, string memory symbol, address owner, uint256 initialSupply) ERC20(name,symbol) ERC20Permit(name) {
    ...
    /**
     * @dev Creates $JELLY when the time is just right.
     */
    ...
    //     require(hasPauserRole(_msgSender()), "JELLY: must have pauser role to pause");
    //     _pause();
    // }
    ...
    //     require(hasPauserRole(_msgSender()), "JELLY: must have pauser role to unpause");
    //     _unpause();
    // }


    // hasOperatorRole when paused
    function _beforeTokenTransfer(
        address from,
        address to,
        uint256 amount
        ...
        require(!paused() || hasOperatorRole(_msgSender()), "JELLY: token transfer while paused");
    }
