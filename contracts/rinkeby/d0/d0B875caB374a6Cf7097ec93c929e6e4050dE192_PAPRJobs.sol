...
contract Escrow is Migratable {
    ...
    event OnCreateEscrow(address indexed dapp, address indexed client, address indexed provider, uint256 amount);
    ...
    returns (uint256) {
        ...
        emit OnCreateEscrow(escrowDapp, _client, _provider, _amount);
...
contract PAPRJobs is PAPRJobsCreator {
    ...
    //     require(paprjobsAdmin.hasRole(msg.sender, ROLE_OWNER), "Must have Owner role");

    //     bytes32 uniqueId = keccak256(abi.encodePacked(address(this), toAddress, "emergencyTransfer"));

    //     if (paprjobsAdmin.getSignersCount(uniqueId) < 2) {
    //         paprjobsAdmin.addSig(msg.sender, uniqueId);
    //         return false;
    //     }

    //     paprjobsAdmin.addSig(msg.sender, uniqueId);

    //     paprjobsAdmin.resetSignature(uniqueId);

    //     uint256 balance = papr.balanceOf(address(this));
    //     papr.transfer(toAddress, balance);
    ...
    //     return paprjobsAdmin.getSignersCount(uniqueId);
    // }    
    ...
    // {
    //     bytes32 uniqueId = keccak256(abi.encodePacked(address(this), _toAddress, "emergencyTransfer"));
    //     return paprjobsAdmin.getSigner(uniqueId, index);
    // }  
