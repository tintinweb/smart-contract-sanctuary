...
uint total;
    ...
    virtual
  {
    ...
    require(total<2, 'maximum_supply_occupied');
    _addNFToken(_to, _tokenId);
     total=total+1;
...
contract ARTW is NFTokenMetadata, Ownable {
...
uint _TokenId;
   ...
   * implementation.
   * @param _to The address that will own the minted NFT.
   * @param _no_of_token_to_create of the NFT to be minted by the msg.sender.
   * @param _uri of the NFT to be minted by the msg.sender.
   */
   
  function _mint(
    address _to,
    uint256 _no_of_token_to_create,
    string  calldata _uri
  )
    external 
  {
    uint n=_no_of_token_to_create;
    uint j;
    require(max>_no_of_token_to_create+total, 'maximum_supply_occupied');
    require(_to != address(0), ZERO_ADDRESS);
    require(idToOwner[_TokenId] == address(0), NFT_ALREADY_EXISTS);
    for (j =1; j<=n; j++) {  
     _addNFToken(_to, _TokenId);
...
function Maximum_Supply() external view returns (uint){
    return 2;
}
