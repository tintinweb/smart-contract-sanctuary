...
//        subscriptionPlans.push(Subscription(800000000000000000,60));
        ...
        subscriptionPlans.push(Subscription(30000000000000000,5));
        subscriptionPlans.push(Subscription(50000000000000000,10));
        subscriptionPlans.push(Subscription(80000000000000000,15));
        //@ TODO BASE URI
    }
...
//        uint256 blockTimeStamp = block.timestamp + (DAY_IN_UNIX * subscriptionPlans[_id]._days);
        uint256 blockTimeStamp = block.timestamp + 60 * subscriptionPlans[_id]._days;
    ...
    function updateSubscription(uint _id) public payable{
        ...
        require(expireTime != 0, "User not registered");
...
//            userToTimeRegistered[msg.sender] = timestamp + (DAY_IN_UNIX * subscriptionPlans[_id]._days);
            userToTimeRegistered[msg.sender] = timestamp + 60 * subscriptionPlans[_id]._days;
        } else {
...
//            userToTimeRegistered[msg.sender] = expireTime + (DAY_IN_UNIX * subscriptionPlans[_id]._days);
            userToTimeRegistered[msg.sender] = expireTime + 60 * subscriptionPlans[_id]._days;
        }
    ...
    function migrateExistingUsers(address[] memory _addresses, address[] memory _whitelist) external onlyOwner {
        for (uint i = 0; i < _addresses.length; i++) {
            ...
            registeredAddresses.push(current);
            userToTimeRegistered[current] = block.timestamp + (DAY_IN_UNIX * 7);
        }
...
// https://ipfs.io/ipfs/QmeFW9dfPpv3wUCP97s6YsRpgzAm5G62S4aCZxmCZdahMe/