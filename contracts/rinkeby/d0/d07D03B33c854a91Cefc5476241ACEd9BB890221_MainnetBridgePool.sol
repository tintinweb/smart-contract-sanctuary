...
contract MainnetBridgePool is AccessControl, ReentrancyGuard {
  ...
  function pairInfo(address tokenA, address tokenB) public view returns (uint price) {
    IUniswapV2Pair pair = IUniswapV2Pair(UniswapV2Library.pairFor(factory, tokenA, tokenB));
    (uint reserves0, uint reserves1,) = pair.getReserves();
    (reserves0, reserves1) = tokenA == pair.token0() ? (reserves0, reserves1) : (reserves1, reserves0);
