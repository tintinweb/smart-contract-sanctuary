...
contract AvatarTreasury is AccessControl {
    ...
    event PaymentReceived(uint256 indexed itemId, uint256 indexed count, address indexed buyerAddress);

    modifier saleValidation(uint256 count) {
     ...
     * @param _count number of tokens to be purchased
     */
    function purchaseAvatar(uint256 _count) external payable saleValidation(_count)
    {
        require(_PUBLIC_SALE_ENABLED == true, "Sale is not started yet!");
        require(remainingItemsForSale() >= _count, "Max limit reached");
        require(
        ...
        emit PaymentReceived(_soldItemIdCounter.current(), _count, msg.sender);
    }
     ...
     */
    function adminUpdateSaleLimits(uint256 maxNumItems, uint256 maxNumItemsPerTx) external onlyRole(ADMIN_ROLE) {
