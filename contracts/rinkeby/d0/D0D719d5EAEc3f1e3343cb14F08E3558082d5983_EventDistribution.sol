...
contract EventDistribution is VRFConsumerBase, AccessControl {
  ...
  IERC20 public NDR;
  IERC20 public POINT;
  PriceAggregator public PRICE_AGGREGATOR;
  PlatformStakingErc20 public PLATFORM_STAKINGE_ERC20;
  EventStore public EVENT_STORE;

  uint256 public slippage;

  uint256 private constant REDEEM_IN_PROGRESS = 1;
  mapping(bytes32 => Redeem) internal redeemers;
  mapping(address => mapping(address => uint256)) internal redeemies;

  bytes32 public linkKeyHash;
  uint256 public linkFee;

  event Bought(address nft, address beneficiary);
  event RedeemRequested(address nft, address beneficiary);
  event RedeemFilled(address nft, address beneficiary, uint256 tokenId);
  ...
  modifier onlyIfEventOpen(address _nft) {
    require(EVENT_STORE.isEventOpen(_nft), "Event is not open");
    _;
  }

  modifier onlyIfEventClosed(address _nft) {
    require(EVENT_STORE.isEventClosed(_nft), "Event is not closed");
    _;
  }
  ...
  constructor(
    ...
    address _DROPS,
    address _POINT,
    address _PRICE_AGGREGATOR,
    address _EVENT_STORE,
    address _PLATFORM_STAKING_ERC20,
    ...
    VRFConsumerBase(_VRF_COORDINATOR, _LINK) {
      ...
      DROPS = IERC20(_DROPS);
      POINT = IERC20(_POINT);
      PRICE_AGGREGATOR = PriceAggregator(_PRICE_AGGREGATOR);
      PLATFORM_STAKINGE_ERC20 = PlatformStakingErc20(_PLATFORM_STAKING_ERC20);
      EVENT_STORE = EventStore(_EVENT_STORE);
  ...
  function recoverERC20(address token, uint256 amount) external onlyAdmin {
    require(token != address(NDR) && token != address(DROPS) && token != address(POINT), "Cannot tokens");
    IERC20(token).safeTransfer(msg.sender, amount);

    emit Recovered(token, amount);
  }

  function setLinkKeyHash(bytes32 _linkKeyHash) external onlyAdmin {
    linkKeyHash = _linkKeyHash;
  }
  ...
  function getTicketPrice(address _nft) external view returns(uint256) {
    EventLib.Event memory _event = EVENT_STORE.getEvent(_nft);
  ...
  function buyWithPoints(address _nft, uint256 _amount) external payable onlyIfEventOpen(_nft) {
    require(block.timestamp - PRICE_AGGREGATOR.getLatestRound(ETH_PAIR) < 15 minutes, "EventDistribution: Eth price is outdated");
    require(EVENT_STORE.canBuyWithPoints(_nft, msg.sender), "EventDistribution: points not allowed");

    (uint256 priceEth, uint256 decimalsEth, ) = PRICE_AGGREGATOR.getLatestPrice(ETH_PAIR);
    (uint256 pricePoint, uint256 decimalsPoint, ) = PRICE_AGGREGATOR.getLatestPrice(POINT_PAIR);
    ...
    POINT.safeTransferFrom(msg.sender, address(this), _amount);
    EVENT_STORE.assignTicket(_nft, msg.sender);

    cashback(msg.sender, usdAmount);

    emit Bought(_nft, msg.sender);
  }

  function buyWithDrops(address _nft, uint256 _amount) external onlyIfEventOpen(_nft) {
    require(block.timestamp - PRICE_AGGREGATOR.getLatestRound(DROPS_PAIR) < 15 minutes, "EventDistribution: Drops price is outdated");
    require(EVENT_STORE.canBuyWithDrops(_nft, msg.sender), "EventDistribution: drops not allowed");

    //fix price
    (uint256 priceDrops, uint256 decimals, ) = PRICE_AGGREGATOR.getLatestPrice(DROPS_PAIR);
    ...
    DROPS.safeTransferFrom(msg.sender, address(this), _amount);
    EVENT_STORE.assignTicket(_nft, msg.sender);
  ...
  function buyWithNdr(address _nft, uint256 _amount) external onlyIfEventOpen(_nft) {
    require(block.timestamp - PRICE_AGGREGATOR.getLatestRound(NDR_PAIR) < 15 minutes, "EventDistribution: NDR price is outdated");
    require(EVENT_STORE.canBuyWithNdr(_nft, msg.sender), "EventDistribution: drops not allowed");

    //fix price
    (uint256 priceNdr, uint256 decimals, ) = PRICE_AGGREGATOR.getLatestPrice(NDR_PAIR);
    ...
    NDR.safeTransferFrom(msg.sender, address(this), _amount);
    EVENT_STORE.assignTicket(_nft, msg.sender);
  ...
  function cashback(address _account, uint256 _amount) private {
    (uint256 priceDrops, uint256 decimals, ) = PRICE_AGGREGATOR.getLatestPrice(DROPS_PAIR);
    (uint256 cashbackRate, uint256 cashbackDecimals) = PLATFORM_STAKINGE_ERC20.cashbackRateOf(_account);

    uint256 cashbackUsdAmount = _amount.mul(cashbackRate).div(cashbackDecimals).div(100);
    uint256 cashbackPointsAmount = cashbackUsdAmount.mul(decimals).div(priceDrops);
    DROPS.safeTransfer(_account, cashbackPointsAmount);
  }

  function redeem(uint256 _seed, address _nft) external onlyIfEventClosed(_nft) {
    require(LINK.balanceOf(address(this)) >= linkFee, "EventDistribution: Not enough LINK to pay fee");
    require(redeemies[_nft][msg.sender] == 0, "EventDistribution: Redeem already in progress");

    bytes32 requestId = requestRandomness(linkKeyHash, linkFee, _seed);

    redeemies[_nft][msg.sender] = REDEEM_IN_PROGRESS;
    redeemers[requestId] = Redeem(msg.sender, _nft);

    emit RedeemRequested(_nft, msg.sender);
  }

  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    Redeem memory redeemer = redeemers[requestId];

    require(redeemies[redeemer.nft][redeemer.requestor] == REDEEM_IN_PROGRESS, "EventDistribution: Redeem already ended");
    redeemies[redeemer.nft][redeemer.requestor] = 0;
    
    uint256 count = EVENT_STORE.getTicketCount(redeemer.nft, redeemer.requestor);
    for(uint8 i = 0; i < count; i++) {
      uint256 tokenId = EVENT_STORE.swapTicketToNft(redeemer.nft, redeemer.requestor, randomness);
      EventErc1155(redeemer.nft).mint(redeemer.requestor, tokenId, 1);

      emit RedeemFilled(redeemer.nft, msg.sender, tokenId);
    }
  }
...
contract EventStore is AccessControl {
  ...
  modifier onlyDistribution() {
    require(hasRole(DISTRIBUTION_ROLE, _msgSender()), "Caller is not a distribution");
    _;
  }
  ...
  function initEvent(address _nft, uint256 _startTime, uint256 _endTime, uint256 _cover) external onlyAdmin {
    require(_startTime > block.timestamp && _endTime > _startTime, "Invalid start or end event dates");
  ...
  function setEventNfts(
    ...
    require(_ids.length == _amounts.length, "Invalid nfts params");
    require(_prices.length == _counts.length, "Invalid prices params");

    for(uint8 i = 0; i < _ids.length; i++) {
      for(uint8 j = 0; j < _amounts[i]; j++) {
        events[_nft].nfts.push(_ids[i]);
      }
    }
    
    for(uint8 i = 0; i < _prices.length; i++) {
      for(uint8 j = 0; j < _counts[i]; j++) {
        events[_nft].prices.push(_prices[i]);
      }
    }

    require(events[_nft].prices.length == events[_nft].nfts.length, "Invalid amount of prices for nfts");
  }
  ...
  function assignTicket(address _nft, address _beneficiaries) external onlyDistribution {
    events[_nft].filled = events[_nft].filled.add(1);
    tickets[_nft][_beneficiaries] = tickets[_nft][_beneficiaries].add(1);
  }

  function swapTicketToNft(address _nft, address _beneficiaries, uint256 _baseRandom) onlyDistribution external returns(uint256) {
  ...
  function random(uint256 _baseRandom, uint256 _range) private returns (uint256 result) {
    ...
    uint256 randNumb = uint256(keccak256(
      abi.encodePacked(_baseRandom, block.timestamp, block.number, blockhash(block.number - 1), block.difficulty, msg.sender, _nonce))) % _range;
    return randNumb;
  }
...
contract PriceAggregator is Ownable {
  ...
  modifier onlyController() {
    require(controller == msg.sender, "PriceAggregator: caller is not the controller");
    _;
  }
...
contract PlatformStakingErc20 is RefillableStakingErc20 {
  ...
  function setRate(uint256 _rate, uint256 _maxRate, uint256 _decimals) external onlyOwner() {
    rate = _rate;
    maxRate = _maxRate.mul(10 ** 18);
    decimals = _decimals;
  }

  function cashbackRateOf(address account) public view returns (uint256, uint256) {
    uint256 rateOf = rate.mul(balances[account]).div(decimals);
    return (Math.min(maxRate, rateOf), decimals);
  }
}
...
contract EventErc1155 is ERC1155, AccessControl {
  ...
  function initCards(uint256[] calldata tokenIds, uint256[] calldata maxSupplys) public {
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Caller is not an admin");
    require(tokenIds.length == maxSupplys.length, "TokenIds should have the same length as maxSupplys");
    for(uint8 i = 0; i < tokenIds.length; i++) {
      totalSupply[tokenIds[i]] = maxSupplys[i];
    }
  }
