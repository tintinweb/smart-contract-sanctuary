 ...
 */
contract TestGenies5 is ERC721Enumerable, Ownable {
    ...
    modifier whenSaleActive {
        require(isSaleActive || (block.number >= 0 && block.number >= saleStartBlock), "TheSampleNft: Sale is not active");
        _;
    }

    modifier whenPresaleActive {
        require(isPresaleActive || (block.number >= 0 && block.number >= presaleStartBlock), "TheSampleNft: Presale is not active");
        require(presaleMinted < MAX_PRESALE_ITEMS, "TheSampleNft: Presale sold out");
        _;
    }

    constructor() ERC721("Test Genies 5", "TGEN5") {
        presaleStartBlock = 0;
        saleStartBlock = 0;
