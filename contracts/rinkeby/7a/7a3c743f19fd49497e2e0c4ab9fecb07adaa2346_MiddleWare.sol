...
interface EstateInterface{
    ...
    function getEstateLandIds(uint256 tokenId, uint256 index) external view returns (uint256);
}
...
contract MiddleWare{
    ...
    function getLandData(uint256 estId, uint256 _pageNumber, uint256 pageSize) public view returns(int[] memory x, int[] memory y, uint256 pageNumber){
        ...
        for(uint256 i=start; i<end; i++){
            uint256 landId = estateInterface.getEstateLandIds(estId, i);
            (int _x, int _y) = landInterface.decodeTokenId(landId);
            x[count] = _x;
            y[count] = _y;
