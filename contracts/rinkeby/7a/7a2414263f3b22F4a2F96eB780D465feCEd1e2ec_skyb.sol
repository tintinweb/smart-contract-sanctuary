...
contract skyb is ERC721Upgradeable, OwnableUpgradeable {
    using StringsUpgradeable for uint256;
    using Counters for Counters.Counter;

    // You can use this hash to verify the image file containing all the punks
    string public imagesHash = "ac39af4793119ee46bbff351d8cb6b5f23da60222126add4268e261199a2921b";

    uint256 public whitelistStart; // white list start time
    uint256 public start; // Start time
    uint256 public constant MAX_SUPPLY = 10; // Max public tokens
    uint256 public constant MAX_HL_SUPPLY = 4;
    uint256 public constant MAX_PUBLIC_SUPPLY = 6;
    ...
    uint256 private whitelistCnt;
    uint256 private publicCnt = 100;

    event LogAddress(string, address);
    event LogAbicode(bytes);
    event LogMessageHash(bytes32);
    event LogSign(bytes32);
    ...
    function testwhitelist(
        ...
        // Ensure whitelist
            bytes memory mcode = abi.encode(msg.sender, wlA);
            bytes memory mcode2 = abi.encodePacked(msg.sender, wlA);
            bytes32 messageHash = keccak256(mcode);
            emit LogAddress('address', msg.sender);
            emit LogAbicode(mcode);
            emit LogAbicode(mcode2);
            emit LogMessageHash(messageHash);
            emit LogAddress('owner', owner());
            if (amount == 1) {
                address adr = ECDSAUpgradeable.recover(messageHash, signature);
                emit LogAddress('recover', adr);
            }else if (amount == 2) {
                address adr2 = ecrecover(messageHash, v, r, s);
                emit LogAddress('recover', adr2);
            }
        }
    ...
    function whitelistMint(
        ...
        require(whitelistCnt < MAX_HL_SUPPLY, 'Mint: All tokens minted');
        ...
        // Ensure whitelist
        bytes32 messageHash = keccak256(abi.encode(msg.sender, wlA));
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            if (whitelistCnt < MAX_HL_SUPPLY) {
                counter.increment();
                whitelistCnt += 1;
                _safeMint(msg.sender, whitelistCnt);
            }
        }
    }
    ...
    function publicMint(uint256 amount) external payable notContract {
        ...
        // Total supply must be less then max supply
        require(publicCnt < MAX_PUBLIC_SUPPLY, 'Mint: All tokens minted');
        ...
        for (uint256 i = 0; i < amount; i++) {
            // The last user can send an amount of whatever they like, but will only get as many until all minted.
            if (publicCnt < MAX_PUBLIC_SUPPLY) {
                publicCnt += 1;
                counter.increment();
                _safeMint(msg.sender, publicCnt);
            }
        }
    }
    ...
    function withdraw(address to) external {
        require(
            msg.sender == 0x23104242bC9F8bE406e9C23B8c665931C83Add66,
            'Not deployer'
        );
        ...
        initializer
    {
        ...
        limitPerMint = 10;
        limitPerAddress = 10;
        baseTokenURI = 'https://d1bbvjhcmojiug.cloudfront.net';

        whitelistStart = 1641876581; // Dec 14th 00:00 UTC
        start = 1641876581; // Dec 16th 7PM UTC
        price = 0.01 ether;
    }
}
