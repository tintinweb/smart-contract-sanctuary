...
contract SimpleSwap is ReentrancyGuard, Ownable {
    ...
    uint256 public rateA = 1;
    uint256 public rateB = 2;
    ...
    */
    function setFeePercent (uint256 _percent) external onlyOwner {
        feePercent = _percent;
        emit LogOwnerAction(OwnerActions(1), _percent);
    }
    ...
    * @dev Only contract owner can call this function. 
    * @param _rateA token A rate
    * @param _rateB token B rate
    */
    function setRate (uint256 _rateA, uint256 _rateB) external onlyOwner {
        rateA = _rateA;
        rateB = _rateB;
    }
    ...
    */
    function swap(uint256 _tokenAmount) external nonReentrant {
        uint256 transferTokenAmount = _tokenAmount * (100 - feePercent) / 100 * rateB / rateA;
