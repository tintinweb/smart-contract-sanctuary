...
contract MonsterbudsV2 is MonsterBudHolders, ERC721URIStorageUpgradeable, IMonsterBuds {
    ...
    */
    ...
    function createPPPcollectible(uint256 _tokenId) external returns (uint256){
        require(_tokenId <= 100 && ownerOf(_tokenId) == msg.sender ,"$MONSTERBUDS: Not a PPP Token or owner of PPP token");
        require(pppMintStatus[_tokenId] == false, "$MONSTERBUDS: Token is already minted by selected PPP");
        ...
        emit FreeTokenDetails(msg.sender, token_uri, newItemId);
        return newItemId;
