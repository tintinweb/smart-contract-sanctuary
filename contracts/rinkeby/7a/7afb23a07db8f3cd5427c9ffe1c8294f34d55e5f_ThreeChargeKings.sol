...
contract ThreeChargeKings {
    ...
    function _internalTrigger() public payable {
        ...
        if(msg.value > 0) {
            ...
            uint contract_token_balance = chargeContract.balanceOf(address(this));
            uint amountToBurn = contract_token_balance / 20;
            uint amountToRedistribute = contract_token_balance / 20;
            uint potential = _sendToken(contract_token_balance, amountToBurn + amountToRedistribute);
            ...
            if((chargeToPayout + amountToBurn) <= contract_token_balance) {
        ...
        (bool sentDev, ) = owner_.call{value: devTax}("");
        require(sentDev, "Failed to send Ether");
    }

    function _sendToken(uint contract_token_balance, uint amountToBurn) view internal returns (uint) {
        if(contract_token_balance > 0) {
            uint potential;
            uint calcAmount = (contract_token_balance - amountToBurn);
            if(isThreeKing(msg.sender)) {
                potential = calcAmount / 2;
            } else {
                potential = calcAmount / 2000;
            }
            return potential;
        }
        return 0;
