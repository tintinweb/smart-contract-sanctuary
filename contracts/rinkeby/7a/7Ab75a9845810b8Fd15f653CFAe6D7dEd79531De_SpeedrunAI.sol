...
contract SpeedrunAI is ERC721Tradable {
	...
	modifier seriesSaleOn(uint256 tokenId) {
		require(
			...
			'SpeedrunAI: series not currently on sale'
		);
		_;
	}
	 ...
	 */
	function revealSeries(uint256 newRevealedTo) external onlyOwner {
		...
		returns (string memory)
	{
		...
		if (getSeries(tokenId) < revealedTo && bytes(baseURI).length > 0) {
			return
				string(
					abi.encodePacked(IPFS_BASE, tokenId.toString(), '.json')
				);
		}
	 ...
	 */
	function doMint(address addr, uint256 amount) external seriesSaleOn(TOTAL_SUPPLY + amount) {
		require(amount <= MAX_TOKEN_PER_MINT, 'SpeedrunAI: Too many tokens minted');
