    ...
    PaymentSplitter
{
    ...
    event RequestedRandomness(uint256 _season, bytes32 _requestId);
        ...
        returns (string memory)
    {
        ...
        // TODO: check that the uri actually works
    ...
    // function seasonExists(uint256 _season) internal returns(bool) {
    ...
    function requestSeasonRandom(uint256 _season) public onlyOwner {
        require(seasons[_season].season_number != 0, "Season does not exist");
        require(seasons[_season].tokenOffset == 0, "Offset is already set");

        bytes32 requestId = requestRandomness(LINK_KEY_HASH, LINK_FEE);
        setSeasonRequestID(requestId, _season);
        emit RequestedRandomness(_season, requestId);
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness)
        ...
        override
    {
        uint256 seasonID = season_randIDs[requestId];
        // tokenOffset = randomness % MAX_SUPPLY;
        uint256 offset = randomness % seasons[seasonID].unit_count;
        season_randoms[seasonID] = offset;
