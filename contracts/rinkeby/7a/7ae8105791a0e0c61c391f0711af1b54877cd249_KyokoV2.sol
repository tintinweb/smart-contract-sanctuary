    ...
    PausableUpgradeable
{
    ...
    mapping(uint256 => NFT) public nftMap; //depositId => NFT
    ...
    mapping(uint256 => OFFER) public offerMap; //offerId => OFFER
    // mapping(address => EnumerableSetUpgradeable.UintSet) internal userOfferMap; //user address => offerId set

    mapping(uint256 => uint256) lendMap; //lTokenId => depositId
    ...
    struct OFFER {
        ...
        bool accept;
        bool cancel;
        uint256 offerId; //最终成交的offerId
        uint256 lTokenId; //最终成交时给他mint NFT的lenderId
        address user; //出此offer的用户
    }
    ...
    modifier checkWhiteList(address _address) {
        require(whiteList[_address], "The address is not whitelisted");
        _;
    }

    modifier checkCollateralStatus(uint256 _depositId) {
        NFT memory _nft = nftMap[_depositId];
    ...
    function modify(
        ...
        NFT storage _nft = nftMap[_depositId];
        require(
        ...
        whenNotPaused
    {
        require(
            ...
            "this offer not in the deposit NFT tx."
        );
        ...
        IERC20Upgradeable(_offer.erc20Token).safeTransfer(msg.sender, _totalAmount);
    ...
    function acceptOffer(uint256 _depositId, uint256 _offerId)
        ...
        whenNotPaused
    {
        //确保_offerId是对应这个_depositId
        require(
            depositIdOfferMap[_depositId].contains(_offerId),
            "this offer not in the deposit NFT tx."
        );
        //确保这个_depositId对应这个用户msg.sender
        require(
            nftHolderMap[msg.sender].contains(_depositId),
    ...
    function lend(uint256 _depositId) external whenNotPaused {
    ...
    function _lend(
        ...
        if (lendMode) {
            //用户直接lend的模式，先把钱(本金+手续费)从用户转到当前合约
            IERC20Upgradeable(_erc20Token).safeTransferFrom(
                address(msg.sender),
                address(this),
                totalAmount
    ...
    function claimCollateral(uint256 _depositId) external {
        ...
        require(!_nft.marks.hasWithdraw, "you have withdrawn this NFT.");
    ...
    /**
     * @dev 出借ERC20的人在对方还钱之后，可以取回ERC20(包括利息)
     */
    ...
    /**
     * @dev 超期之后NFT出借方仍然没有还钱，由出钱一方执行此操作，触发进入清算倒计时状态
     */
    ...
    /**
     * @dev 倒计时结束，NFT出借方仍然没有还钱，则把NFT发给出钱一方
     */
