...
contract MetaLegends is ERC721Enumerable, AccessControl, Ownable {
    ...
    address private creatorAddr = 0xC33424A82f65aa2746504eBf37cdffDC2daf9Ab9;
    ...
    mapping(address => uint256) public totalWhitelistClaimed;
    mapping(address => uint256) public totalPublicSaleClaimed;
    mapping(address => uint256) public totalGiveways;
    ...
    // Events
    event publicSaleClaimed(
        address indexed from,
        uint256 price,
        uint256 timestamp,
        uint256[] tokenIds
    );
    event whitelistSaleClaimed(
        address indexed from,
        uint256 timestamp,
        uint256[] tokenIds
    );
    event giveway(address indexed from, uint256 timestamp, uint256[] tokenIds);
    ...
    */
     ...
     */
    function givewayMint(uint256 _nb, address _to) external onlyAdmin {
        ...
        totalGiveways[_to] = totalGiveways[_to].add(_nb);
        emit giveway(_to, block.timestamp, _tokenIdsMinted);
    }
        ...
        onlyRole(whitelistedRole)
    {
        ...
        require(
            totalWhitelistClaimed[msg.sender].add(_nb) <=
                totalWhitelistClaimable,
            "Limit exceeded."
        );
        ...
        totalWhitelistClaimed[msg.sender] = totalWhitelistClaimed[msg.sender]
            .add(_nb);
        emit whitelistSaleClaimed(msg.sender, block.timestamp, _tokenIdsMinted);
    }
    ...
    function publicSaleMint(uint256 _nb) external payable {
        ...
        require(
            totalPublicSaleClaimed[msg.sender].add(_nb) <=
                totalPublicSaleClaimable,
            "Limit exceeded."
        );
        ...
        totalPublicSaleClaimed[msg.sender] = totalPublicSaleClaimed[msg.sender]
            .add(_nb);
        emit publicSaleClaimed(
            msg.sender,
            currentPrice,
            currentTimestamp,
            _tokenIdsMinted
