...
contract MyNFT is MyBase, VRFConsumerBase {
  ...
  constructor(
    ...
    MyBase(_tokenName, _tokenSymbol, _mintFee, _maxSupply, _limitMint)
   ...
   */
  function _mintMy(address _to,uint256 numberToken) internal returns (bool) {
  ...
  function mintMy(uint256 numberToken)
    public
    payable
    online
    mintable(numberToken)
    returns (bool) {
    return _mintMy(_msgSender(), numberToken);
  }
  ...
  function airdrop(address _to, uint256 numberToken) public offline onlyOwner {
    _mintMy(_to, numberToken); // mint for marketing & influencer
  }
}
