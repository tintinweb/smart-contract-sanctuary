...
contract Minter is ERC721PresetMinterPauserAutoIdUpgradeable, EIP712Upgradeable
{
    ...
    event TokenMinted(address receiver);
    event TreasuryChanged(address newTreasury);
    event SaleStartUpdated(uint256 startTime);
    ...
    /**
    * @dev configure stat date/time of sale
    *
    * Requirements:
    * - the caller must have the `MINTER_ROLE`.
    */
    function setSaleStart(uint256 startTime) external {
        require(hasRole(MINTER_ROLE, _msgSender()), "must have minter role to reveal");

        saleStartTime = startTime;
        emit SaleStartUpdated(startTime);
    }
     ...
     * - the caller must have the `MINTER_ROLE`.
     */
    function revealNFT(string calldata baseTokenURI) external {
        require(hasRole(MINTER_ROLE, _msgSender()), "must have minter role to reveal");
    ...
    function preSaleMint(address to, uint256 count) public virtual {
        require(hasRole(MINTER_ROLE, _msgSender()), "must have minter role to mint");

        // Gas optimization
        uint256 _nextTokenId = _tokenIdTracker.current();
