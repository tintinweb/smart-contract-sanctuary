  ...
  ReentrancyGuard
{
  ...
  //_id is the token id, amount is how much the winner has own, _winner is the address of the winner, and _Type is the the type of claim. _type 1 is raffle, _type 2 is pool
  event claimedRaffle(uint256 _id, uint256 _amount, address _winner);
  event claimedPool(uint256 _id, uint256 _amount, address _winner);
  ...
  function claimPool(uint256[] calldata nftIDs) external nonReentrant {
    ...
    if (ethPayout) {
      ...
      balance = IERC20(babyDogeToken).balanceOf(address(this)).sub(prizePool);
    }

    for (uint256 i = 0; i < nftIDs.length; i++) {
      require(ownerOf(nftIDs[i]) == msg.sender);
      require(iterationClaimed[currentIteration.current()][nftIDs[i]] == false);
      ...
      if (iterationToClaim[currentIteration.current()] > 0) {
        iterationToClaim[currentIteration
          .current()] = iterationToClaim[currentIteration.current()].sub(1);
      }
      emit claimedPool(nftIDs[i], claimable, msg.sender);
    }
    require(claimable > 0);
    if (ethPayout) {
  ...
  function claimRaffle(uint256 _id) external nonReentrant {
    ...
    for (uint256 i = 0; i < toClaimPrize.length; i++) {
      if (toClaimPrize[i] == _id) {
        ...
        emit claimedRaffle(_id, prize, ownerOf(_id));
  ...
  function fulfillRandomness(bytes32 requestId, uint256 randomness)
    ...
    override
  {
    uint256 randInt = randomness % totalSupply().sub(5);
    winners = [
      randInt,
      randInt.add(5),
      randInt.add(4),
