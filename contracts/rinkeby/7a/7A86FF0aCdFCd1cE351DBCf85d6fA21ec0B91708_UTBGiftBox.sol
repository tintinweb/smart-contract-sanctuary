...
contract UTB is ERC20, Ownable {
    mapping(address => bool) private centers;

    event CenterAdded(address indexed newCenter);
    event CenterRemoved(address indexed oldCenter);

    constructor() ERC20("Utilitybia Token", "UTB") {
    ...
    function burn(uint256 amount) external onlyCenter(msg.sender) {
        require(amount > 0, "Amount invalid");
        super._burn(msg.sender, amount);
    }
}

// contracts/utilities/NFTStatefulURI.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;
...
abstract contract NFTStatefulURI is ERC721URIStorage, Ownable {
    ...
    constructor(
        ...
        string[] memory states_
    ) ERC721(name, symbol) {
        baseURI = baseURI_;
        states = states_;
    }

    function tokenState(uint256 tokenId) internal view returns (uint8) {
        return tokenStates[tokenId];
    }

    function _mintItem(address to, uint256 assetId) internal returns (uint256) {
        _tokenIds.increment();

        uint256 newItemId = _tokenIds.current();
        tokenAssets[newItemId] = assetId;
        _mint(to, newItemId);
        _setTokenURI(newItemId, assets[assetId].asset);
...
// contracts/utilities/UTBBase.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;
...
import "./NFTStatefulURI.sol";
import "../UTB.sol";

abstract contract UTBBase is NFTStatefulURI {
    ...
    event PointSet(address point);
    event AssetAdded(uint256 assetId, Asset asset);
    event AssetDiscounted(uint256 assetId);
    ...
    mapping(Rarity => uint256) public fees;
    mapping(Rarity => uint256) public feePoints;
    mapping(Rarity => uint256) public limits;
    uint256 discountFee;
    ...
    modifier hasPoint() {
        require(point.isCenter(address(this)), "Point token not set");
        _;
    }
    ...
    function setPoint(address _point) external onlyFactory {
        require(_point != address(0), "Point invalid");
        point = UTB(_point);
        emit PointSet(_point);
    }
    ...
    function registerAsset(
        ...
        require(rarity < 3, "Rarity invalid");
        totalAssets += 1;
        uint256 assetId = totalAssets;
        assets[assetId] = Asset(asset, author, Rarity(rarity));
        authorAssets[author].push(assetId);
        emit AssetAdded(assetId, assets[assetId]);
    }

    function getAsset(uint256 assetId) public view returns (string memory) {
        return assets[assetId].asset;
    }

    function getAuthor(uint256 assetId) public view returns (address) {
        return assets[assetId].author;
    }

    function getRarity(uint256 assetId) public view returns (Rarity) {
        return assets[assetId].rarity;
    }
    ...
    ) public payable onlyAuthor(assetId) {
        require(discount > block.timestamp, "Discount invalid");
        require(discounts[assetId] <= block.timestamp, "Discounted already");
        require(discountPercent < 30, "Discount too much");

        uint256 discountFee_;
        if (discount <= block.timestamp + 8 days) {
            discountFee_ = discountFee;
        } else if (discount <= block.timestamp + 29 days) {
            discountFee_ = discountFee * 2;
        } else {
            discountFee_ = discountFee * 5;
        }
        require(discountFee_ <= msg.value, "Fee insufficient");

        discounts[assetId] = discount;
        discountPercents[assetId] = discountPercent;
        emit AssetDiscounted(assetId);
    }

    function fee(uint256 assetId) public view returns (uint256) {
        if (discounts[assetId] < block.timestamp) {
            return fees[assets[assetId].rarity];
        } else {
            return
                fees[assets[assetId].rarity]
                    .mul(100 - discountPercents[assetId])
                    .div(100);
        }
    }
    ...
    function feePoint(uint256 assetId) public view returns (uint256) {
        if (discounts[assetId] < block.timestamp) {
            return feePoints[assets[assetId].rarity];
        } else {
            return
                feePoints[assets[assetId].rarity]
                    .mul(100 - discountPercents[assetId])
                    .div(100);
        }
    }
    ...
    modifier onlyOnSale(uint256 assetId) {
        require(assetId > 0 && assetId <= totalAssets, "Asset invalid");
        require(
            mints[assetId] + mintWithPoints[assetId] <=
                limits[assets[assetId].rarity],
            "Fully minted already"
        );
        _;
    }

    function buyItem(uint256 assetId)
        public
        payable
        onlyOnSale(assetId)
        returns (uint256)
    {
        ...
        mints[assetId] += 1;
        return _mintItem(msg.sender, assetId);
    }

    function buyItemTo(uint256 assetId, address to)
        public
        payable
        onlyOnSale(assetId)
        returns (uint256)
    ...
    modifier onlyPoints(uint256 assetId) {
        require(assetId > 0 && assetId <= totalAssets, "Asset invalid");
        require(
            mintWithPoints[assetId] <
                limits[assets[assetId].rarity].mul(20).div(100),
            "Fully minted with points"
        );
        _;
    }

    function _mintItemWithPoints(address to, uint256 assetId)
        internal
        hasPoint
        returns (uint256)
    {
        require(point.isCenter(address(this)), "Point token not set");
        require(
            point.balanceOf(msg.sender) >= feePoint(assetId),
            "Point insufficient"
        );

        point.transferFrom(msg.sender, address(this), feePoint(assetId));
        mintWithPoints[assetId] += 1;

        return _mintItem(to, assetId);
    }

    function buyItemWithPoints(uint256 assetId)
        public
        onlyPoints(assetId)
        returns (uint256)
    {
    ...
    function buyItemWithPointsTo(uint256 assetId, address to)
        public
        onlyPoints(assetId)
        returns (uint256)
    {
    ...
    function reward(uint256 assetId) public view returns (uint256) {
        return
            mints[assetId]
                .mul(fees[assets[assetId].rarity].mul(15).div(100))
                .sub(rewardClaims[assetId]);
    }
    ...
    function withdraw(uint256 amount) public onlyOwner {
        require(amount <= address(this).balance, "Amount insufficient");
        payable(owner()).transfer(amount);
    }

    function withdrawPoints() public hasPoint onlyOwner {
        require(point.balanceOf(address(this)) > 0, "Amount insufficient");
        point.burn(point.balanceOf(address(this)).div(2));
        point.transfer(owner(), point.balanceOf(address(this)));
    }
    ...
    modifier onlyNFTFor(uint256 tokenId, address to) {
        require(ownerOf(tokenId) == to, "Permission denied");
        _;
    }

    function fundRaise() public payable {
        funds += msg.value;
    }

    function deposit(uint256 tokenId) public payable virtual;

    function registerETH(uint256 tokenId, uint256 amount) public virtual;
    ...
    function escapeERC20(
        ...
        require(
            amount > 0 && amount <= IERC20(token).balanceOf(address(this)),
            "Amount insufficient"
        );
...
contract UTBGiftBox is UTBBase {
    using SafeMath for uint256;
    ...
    ) NFTStatefulURI("Utilitybia - Gift Box", "UTBGiftBox", baseURI, states) {
        fees[Rarity.COMMON] = _fees[0];
        fees[Rarity.RARE] = _fees[1];
        fees[Rarity.LEGENDARY] = _fees[2];
        feePoints[Rarity.COMMON] = _feePoints[0];
        feePoints[Rarity.RARE] = _feePoints[1];
        feePoints[Rarity.LEGENDARY] = _feePoints[2];
        discountFee = _discountFee;

        limits[Rarity.COMMON] = 2000;
        limits[Rarity.RARE] = 250;
        limits[Rarity.LEGENDARY] = 25;
    }

    modifier onlyStatus(uint256 tokenId, Status status) {
        require(tokenState(tokenId) == uint8(status), "Status invalid");
        _;
    }
    ...
    function wrap(uint256 tokenId)
        ...
        onlyStatus(tokenId, Status.NORMAL)
    {
        setTokenState(tokenId, uint8(Status.READY));
        uint256 points = feePoints[assets[tokenAssets[tokenId]].rarity]
        .mul(20)
        .div(100);
    ...
    function registerETH(uint256 tokenId, uint256 amount)
        public
        override
        onlyOwner
    {
    ...
    function depositERC20(
        ...
        address token,
        uint256 amount
    ) public override onlyStatus(tokenId, Status.NORMAL) {
    ...
    function depositERC721(
        ...
        IERC721(erc721Token).transferFrom(
            msg.sender,
            address(this),
            erc721TokenId
        );
    ...
    function depositERC1155(
        ...
        depositERC1155s[tokenId][erc1155Token].push(
            TkERC1155(erc1155TokenId, erc1155TokenAmount)
        );
    }
    ...
    function open(uint256 tokenId)
        public
        onlyNFTOwner(tokenId)
        onlyStatus(tokenId, Status.READY)
    {
        setTokenState(tokenId, uint8(Status.SURPRISE));
        uint256 tokenAsset = tokenAssets[tokenId];
        uint256 points = feePoints[assets[tokenAsset].rarity].mul(10).div(100);
        point.mint(msg.sender, points);
    }

    function giftETH(uint256 tokenId) public view returns (uint256 amount) {
        if (tokenState(tokenId) == uint8(Status.SURPRISE)) {
    ...
    function giftERC20s(uint256 tokenId)
        public
        view
    ...
    function giftERC20Amount(uint256 tokenId, address token)
        public
        view
    ...
    function giftERC721s(uint256 tokenId)
        public
        view
        returns (address[] memory tokens)
    {
    ...
    function giftERC721Ids(uint256 tokenId, address token)
        public
        view
    ...
    function giftERC1155s(uint256 tokenId)
        public
        view
        returns (address[] memory tokens)
    {
    ...
    function giftERC1155Ids(uint256 tokenId, address token)
        public
        view
    ...
    function claimDeposits(uint256 tokenId)
        ...
        override
        onlyNFTOwner(tokenId)
        onlyStatus(tokenId, Status.SURPRISE)
    {
        ...
        if (tokenERC20s[tokenId].length > 0) {
            mapping(address => uint256) storage tokenDeposits = depositERC20s[
                tokenId
            ];
            for (uint256 i = 0; i < tokenERC20s[tokenId].length; i++) {
                IERC20(tokenERC20s[tokenId][i]).transfer(
                    msg.sender,
                    tokenDeposits[tokenERC20s[tokenId][i]]
                );
        ...
        if (tokenERC721s[tokenId].length > 0) {
                ...
                mapping(address => uint256[]) storage tokenDeposits
             = depositERC721s[tokenId];
        ...
        if (tokenERC1155s[tokenId].length > 0) {
            for (uint256 i = 0; i < tokenERC1155s[tokenId].length; i++) {
                for (
                    uint256 j = 0;
                    j <
                    depositERC1155s[tokenId][tokenERC1155s[tokenId][i]].length;
                    j++
                ) {
                    IERC1155(tokenERC1155s[tokenId][i]).safeTransferFrom(
                        address(this),
                        msg.sender,
                        depositERC1155s[tokenId][tokenERC1155s[tokenId][i]][j]
                            .tokenId,
                        depositERC1155s[tokenId][tokenERC1155s[tokenId][i]][j]
                            .amount,
                        "0x01"
