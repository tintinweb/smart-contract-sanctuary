...
import "../exchange/OneKindExchange.sol";
    ...
    Ownable
{
    ...
    address exchangeContractAddress; // address of the exchange contract
    ...
    function isApprovedForAll(address owner, address operator)
        ...
        view
        virtual
        override(ERC721DefaultApproval, ERC721)
        returns (bool)
    {
    ...
    function mint(address to) public virtual {
        require(
            _msgSender() == owner(),
            "OneKindERC721: Only contract owner an mint"
        );
    ...
    function adminMint(address to) public virtual returns (uint256) {
        require(
            OneKindExchange(exchangeContractAddress).isAdmin(_msgSender()) ==
                true,
            "OneKindERC721: must have admin minter role to mint"
        );
    ...
    function setBaseURI(string memory permanentBaseURI) public virtual {
        require(
            OneKindExchange(exchangeContractAddress).isAdmin(_msgSender()) ==
                true,
            "OneKindERC721: must have admin role to change metadata url"
        );
        ...
        virtual
    {
        require(
            OneKindExchange(exchangeContractAddress).isAdmin(_msgSender()) ==
                true,
            "OneKindERC721: must have admin role to change token URIs"
        );
        require(
            _exists(tokenId),
...
abstract contract ERC721DefaultApproval {
    ...
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal virtual view returns (bool) {
        return defaultApprovals[spender];
    }
