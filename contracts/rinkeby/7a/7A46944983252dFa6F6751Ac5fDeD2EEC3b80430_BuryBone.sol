...
contract BuryBone is ERC20("BuryBone", "yBONE"){
    using SafeMath for uint256;
    IERC20 public bone;
    ...
    // Enter the bar. Pay some bones. Earn some shares.
        ...
        mapping(address => mapping(uint256 => TimeLock)) lockData;
        
         // Enter the bar. Pay some bones. Earn some shares.

    function enterLOCK(uint256 _amount,uint _releaseTime) public {
        uint releaseTime = (_releaseTime * 1 days) + block.timestamp;
        uint256 totalBONE = bone.balanceOf(address(this));
        uint256 totalShares = totalSupply();
        lockedbalances[msg.sender].push(_amount);
        lockData[msg.sender][_amount] = TimeLock(_amount, releaseTime);
      ...
      function leavelock(uint256 _share) public {
        lockData[msg.sender][_share]._amount -= _share;
        uint256 totalShares = totalSupply();
        uint256 what = _share.mul(bone.balanceOf(address(this))).div(totalShares);
            for(uint i =0; i<lockedbalances[msg.sender].length ; i++){
                // lockData[msg.sender][i]._amount;
                if(lockData[msg.sender][lockedbalances[msg.sender][i]]._releaseTime<block.timestamp){
                        if(lockData[msg.sender][lockedbalances[msg.sender][i]]._amount<_share){
