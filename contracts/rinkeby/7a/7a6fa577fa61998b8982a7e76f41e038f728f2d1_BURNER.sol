...
contract BURNER is Context, AccessControlEnumerable, ERC721Enumerable, ERC721Burnable, ERC721Pausable {
	...
	string private _banner_collection_URI = "https://crashblossom.co/burner/assets/img/banner.png";
	...
	string private _tokenImageURI = "https://crashblossom.co/burner/assets/img/icon.gif";
	...
	function tokenShowcaseURI() public view returns (string memory) {
		return _showcaseURI;
	}

	function tokenByIndex(uint256 _index) public view override returns (uint256) {
		require(_index >= 0, "ID out of bounds!");
		require(_index < _tokenCounter, "ID out of bounds!");
	...
	function _getTier(uint256 _tokenId) private view returns (uint256) {
		if(_tokenId % 6 == 0) {
			return 3;
		}
	...
	function retrieveData(uint256 _tokenId) external view returns (string memory) {
	...
	function getTokenData(uint16 _tokenId) public view returns (string memory) {
		// Example: [[3],[8,48,121,183,106,92,29,196,191],[[0,40],[41,75],[76,125],[126,150],[150,200],[200,300],[300,999999]]]
		bytes memory csv;
		uint tier = _getTier(_tokenId);
		bytes32 seed = keccak256(abi.encodePacked(blockhash(block.number - 1), block.timestamp));
		uint16 i;
		uint16 value;

		csv = abi.encodePacked(csv, '[');
			csv = abi.encodePacked(csv, '[');
				csv = abi.encodePacked(csv, _uint2str(tier)); //Tier (1,2,3)
			csv = abi.encodePacked(csv, '],');
			csv = abi.encodePacked(csv, '['); // Index for each layer (layers order)
				for(i = 0; i < 8; i++) {
					value = uint8(seed[i]);
					csv = abi.encodePacked(csv, _uint2str(value));
					csv = abi.encodePacked(csv, ',');
				}
				value = uint8(seed[i]);
				csv = abi.encodePacked(csv, _uint2str(value));
			csv = abi.encodePacked(csv, '],');
			csv = abi.encodePacked(csv, '['); // Tiers distribution according to gas cost (tiers data)
				csv = abi.encodePacked(csv, _tiersJSON);
			csv = abi.encodePacked(csv, ']');
		csv = abi.encodePacked(csv, ']');
	...
	
	*/
}