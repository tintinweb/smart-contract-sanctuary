...
contract SongsOfTheMetaverseV1 is ERC721Enumerable, ReentrancyGuard, Ownable {
  ...
  string[] private songList = [
    ...
    "ipfs://QmQgaK2yGg3Th8wm99KrusFAvdGBjKnQPTouG37nFu5hTP"
  ];
  ...
  function getSongFromList(string memory songName) internal view returns (string memory) {
    if (keccak256(bytes(songName)) == keccak256(bytes("Through Hell's Gate"))) {
      ...
      return songList[4];
    } else if (keccak256(bytes(songName)) == keccak256(bytes("A Vision Of Greatness"))) {
      return songList[5];
    } else {
      return songList[1];
    }
        ...
        bytes(
          string(
            abi.encodePacked(
              ...
              toString(tokenId),
              '", "description": "The Songs of the Metaverse. First collection of 5 unique/original songs to accompany you on the journey ahead. Compatible with Loot (for Adventurers)", "animation_url": "',
              song,
