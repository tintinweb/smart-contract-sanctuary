...
contract CryptoPhunks is Ownable, ERC721Enumerable, ReentrancyGuard {
  ...
  constructor() ERC721("EpicPhunks", "EPHIC") {}
  ...
  function maxNumFreePhunks() public view virtual returns (uint256) {
    ...
    // the number we give away will be (1900 - however many free dev phunks are minted)
    return 2;
  }
    ...
    returns (uint256)
  {
    if (isFreeSaleOn) {
      return 0.00 ether;
    } else if (_numMinted < 3) {
      return 0.001 ether;
    } else if (_numMinted < 5) {
      return 0.002 ether;
    } else if (_numMinted < 10) {
      return 0.01 ether;
    }
