...
contract ERC721UpgradeableBlocommerceV2 is
    ContextUpgradeable,
    OwnableUpgradeable,
    ERC721URIStorageUpgradeable,
    ERC721Enumerable,
    ERC721UpgradeableBlocommerceLazy
{
    ...
    modifier only721Creator(uint256 _tokenId) {
        address creator = getNFTCreator(_tokenId);
    ...
    function mintAndTransfer(
        address _to,
        LibERC721LazyMint.NFT memory nft,
        address _signer,
        bytes calldata _signature
    ) external {
        super.mintAndTransferLazy(_to, nft, _signer, _signature);
    }

    function mintNFT(string memory _uri) public {
        require(_msgSender() != address(0), "Owner Address is zero");
        _createNFT(_uri, _msgSender());
    }

    function deleteNFT(uint256 _tokenId) public only721Owner(_tokenId) {
        address ownerofToken = ownerOf(_tokenId);
        require(ownerofToken == _msgSender(), "must be the owner of the token");
        _burn(_tokenId);
    }

    function updateNFTMetadata(uint256 _tokenId, string memory _uri)
        public
        only721Owner(_tokenId)
    {
        _setTokenURI(_tokenId, _uri);
        emit TokenURIUpdated(_tokenId, _uri);
    }

    function _set721Creator(uint256 _tokenId, address _creator) internal {
        Creators[_tokenId] = _creator;
    }

    function transferOfNFT(
        address from_,
        address to_,
        uint256 tokenID_
    ) public {
    ...
    function _createNFT(string memory _uri, address _creator)
        internal
        returns (uint256)
    {
        uint256 newId = idCounter + 1;
        idCounter++;
        _safeMint(_creator, newId);
        _setTokenURI(newId, _uri);
        _set721Creator(newId, _creator);
    ...
    uint256 private testID;
    uint256 private testIDD;
    uint256 private testIDDD;
    // uint256 private testID;
    // uint256 private testID;
    // uint256 private testID;
}
...
abstract contract ERC721UpgradeableBlocommerceLazy is Initializable, EIP712Upgradeable, ERC721URIStorageUpgradeable {
    using StringsUpgradeable for uint256;
    using AddressUpgradeable for address;
    event TransferDone(address from, address to, uint256 tokenId);
    ...
    function mintAndTransferLazy(address to, LibERC721LazyMint.NFT memory nft, address signer, bytes calldata signature)
     internal
    {
        ...
        emit TransferDone(signer, to, nft.tokenId); 
    }

    function _hash(LibERC721LazyMint.NFT memory nft)
    internal view returns (bytes32)
    {
        return _hashTypedDataV4(keccak256(abi.encode(
            keccak256("NFT(uint256 tokenId,string tokenURI,address creator)"),
            nft.tokenId,
            keccak256(bytes(nft.tokenURI)),
            nft.creator
        )));
    }
