...
contract OpenI is Ownable, ERC721Enumerable {
     using Strings for uint256;

    uint public constant MAX_NFT_SUPPLY = 420;
    uint public constant NFT_PRICE = 0.021 ether;
    string public baseExtension = ".json";
    string private baseURI;
    bool public isSaleActive = true;
    bool public reveal;
    uint public maxFreeNFTPerWallet = 0;
    uint public maxNFTPerWallet = 42;
    ...
    function mint(uint amount) external payable {
        ...
        require(totolSupply + amount <= MAX_NFT_SUPPLY, "MAX_NFT_SUPPLY constraint violation");
        ...
        for (uint i = 0; i < amount; i++) {
            _safeMint(msg.sender, totolSupply + 1);
            totolSupply += 1;
        }
    }
     function withdraw() external onlyOwner {
        uint balance = address(this).balance;
        uint marketing = balance/2;
        uint team1 = balance/5;
        uint team2 = balance /10 ;
        payable(goldTicketAddress).transfer(marketing);
        payable(winnerAddress).transfer(team2);
        payable(sharityAddress).transfer(team1 + team1);
    }
