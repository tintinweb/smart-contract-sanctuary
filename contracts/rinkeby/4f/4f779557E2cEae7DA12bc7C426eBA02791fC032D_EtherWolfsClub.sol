...
contract EtherWolfsClub is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    uint256 private constant PRICE_PER_EW = 5 * 10**16; // 0.05ETH Per Ether Wolf

    uint256 private constant MAX_ELEMENTS_CROWDSALE = 2450 ; // 2450 Ether Wolfs in CrowdSale
    uint256 private constant MAX_ELEMENTS_PRESALE = 5 * 10**1; // 50 Ether Wolfs in PreSale
    ...
    address private constant ownerAddress = 0xeCEb7A944BD064fd7cf5e77AA0c7Ac9644ef9084; // Wallet Address of Owner
    ...
    event CreateEtherWolf(address to, uint256 indexed id);
    ...
    constructor (string memory baseURI) ERC721("Ether Wolfs Club", "EWC") {
        setBaseURI(baseURI);
        pause(true);
    ...
    function mint(address payable _to, uint256[] memory _ids) public payable saleIsOpen {
        ...
        require(total <= maxSalesAmount, "MINT: Please go to the Opensea to buy Ether Wolfs.");
        require(_ids.length <= maxMintAmount, "MINT: Current count exceeds maximum mint count.");
        require(msg.value >= price(_ids.length), "MINT: Current value is below the sales price of Ether Wolfs");
    ...
    function price(uint256 _count) public pure returns (uint256) {
        return PRICE_PER_EW.mul(_count);
    }
