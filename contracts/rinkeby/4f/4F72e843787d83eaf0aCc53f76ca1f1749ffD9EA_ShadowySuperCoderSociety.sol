...
import "./IShadowySuperCoderSociety.sol";

error SSCS__InsufficientFundsSent();
error SSCS__InvalidReserveRecipient();
error SSCS__MaxBurnAmountExceeded();
error SSCS__MaxMintsPerCallExceeded();
error SSCS__MaxReservedElementsExceeded();
error SSCS__MintCapExceeded();
error SSCS__NonexistentToken();
error SSCS__NotWhitelistedForPrivatePhase();
error SSCS__OffsetAlreadySet();
error SSCS__PrivatePhaseIsOver();
error SSCS__PublicPhaseNotYetStarted();
error SSCS__RandomnessAlreadyRequested();
error SSCS__SaleIsActive();
error SSCS__SaleIsNotActive();
error SSCS__TooEarlyToReveal();
error SSCS__VrfRequestIdMismatch();

/// @title ShadowySuperCoderSociety
/// @author scorpion9979
/// @notice An NFT collection of 10,000 unique Shadowy Super Coder avatars.
contract ShadowySuperCoderSociety is
    IShadowySuperCoderSociety,
    ERC721Enumerable,
    Ownable,
    PaymentSplitter,
    ReentrancyGuard,
    VRFConsumerBase
{
    ...
    /// @dev The planned collection size of 10,000 unique elements.
    ...
    uint256 public constant MAX_RESERVED_ELEMENTS = 200;

    /// @dev The duration of the private phase of sale.
    uint256 public constant PRIVATE_PHASE_DURATION = 24 hours;

    /// @inheritdoc IShadowySuperCoderSociety
    uint256 public override fee = 0.05 ether;

    /// @inheritdoc IShadowySuperCoderSociety
    uint256 public override maxMintsPerCall = 5;

    /// @inheritdoc IShadowySuperCoderSociety
    uint256 public override mintCap = COLLECTION_SIZE;
    ...
    constructor(
        ...
        ERC721("Shadowy Super Coder Society", unicode"☠️")
        PaymentSplitter(shareholders_, shares_)
        VRFConsumerBase(vrfCoordinator_, chainlinkToken_)
    {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        if (!_exists(tokenId)) {
            revert SSCS__NonexistentToken();
        }
        string memory bURI = _baseURI();
        if (offset == 0) {
            return bytes(bURI).length > 0 ? string(abi.encodePacked(bURI, "anon", ".json")) : "";
        } else {
    ...
    function burnUnsold(uint256 burnAmount) public override onlyOwner {
        if (saleIsActive) {
            revert SSCS__SaleIsActive();
        }
        if (burnAmount > mintCap - totalSupply()) {
            revert SSCS__MaxBurnAmountExceeded();
        }
    ...
    function mintPrivate(uint256 mintAmount, bytes32[] calldata merkleProof) public payable override nonReentrant {
        if (!saleIsActive) {
            revert SSCS__SaleIsNotActive();
        }
        unchecked {
            if (block.timestamp > startTime + PRIVATE_PHASE_DURATION) {
                revert SSCS__PrivatePhaseIsOver();
            }
        }
        if (!isWhitelisted(msg.sender, merkleProof)) {
            revert SSCS__NotWhitelistedForPrivatePhase();
        }

        mintInternal(mintAmount);
        emit Mint(msg.sender, mintAmount, fee, MintPhase.PRIVATE);
    }
    ...
    function mintPublic(uint256 mintAmount) public payable override nonReentrant {
        ...
        unchecked {
            if (block.timestamp <= startTime + PRIVATE_PHASE_DURATION) {
                revert SSCS__PublicPhaseNotYetStarted();
            }
        }

        mintInternal(mintAmount);
        emit Mint(msg.sender, mintAmount, fee, MintPhase.PUBLIC);
    }
    ...
    /// @inheritdoc IShadowySuperCoderSociety
    function reserve(address recipient, uint256 reserveAmount) public override onlyOwner {
        if (recipient == address(0)) {
            revert SSCS__InvalidReserveRecipient();
        }
        if (reserveAmount + reservedElements > MAX_RESERVED_ELEMENTS) {
            revert SSCS__MaxReservedElementsExceeded();
        }
        ...
        emit Reserve(recipient, reserveAmount);
    }

    /// @inheritdoc IShadowySuperCoderSociety
    function reveal() public override onlyOwner {
        if (startTime == 0 || block.timestamp < revealTime) {
            revert SSCS__TooEarlyToReveal();
        }
        if (offset != 0) {
            revert SSCS__OffsetAlreadySet();
        }
        if (vrfRequestId != 0) {
            revert SSCS__RandomnessAlreadyRequested();
        }
    ...
    /// @inheritdoc IShadowySuperCoderSociety
    function setMaxMintsPerCall(uint256 newMaxMintsPerCall) public override onlyOwner {
        maxMintsPerCall = newMaxMintsPerCall;
        emit SetMaxMintsPerCall(newMaxMintsPerCall);
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        if (vrfRequestId != requestId) {
            revert SSCS__VrfRequestIdMismatch();
        }
    ...
    function mintInternal(uint256 mintAmount) internal {
        if (mintAmount > maxMintsPerCall) {
            revert SSCS__MaxMintsPerCallExceeded();
        }
        uint256 totalSupply = totalSupply();
        uint256 mintCost;
        unchecked {
            if (mintAmount + totalSupply > mintCap) {
                revert SSCS__MintCapExceeded();
            }
            mintCost = fee * mintAmount;
            if (msg.value < mintCost) {
                revert SSCS__InsufficientFundsSent();
            }
        }
...
/// @title IShadowySuperCoderSociety
/// @author scorpion9979
interface IShadowySuperCoderSociety {
    /// ENUMS ///
    ...
    /// @notice Emitted when the contract owner burns unsold NFTs after sale.
    /// @param burnAmount The amount of NFTs burned.
    ...
    /// @param fee The mint fee per each minted NFT.
    /// @param phase The mint phase.
    event Mint(address indexed minter, uint256 mintAmount, uint256 fee, MintPhase phase);

    /// @notice Emitted when sale is paused.
    event PauseSale();

    /// @notice Emitted when NFTs are reserved by the team.
    /// @param recipient The recipient account address of reserved NFTs.
    /// @param reserveAmount The amount of NFTs that were reserved.
    event Reserve(address recipient, uint256 reserveAmount);
    ...
    /// @param fee The new mint fee that was set.
    event SetFee(uint256 fee);

    /// @notice Emitted when the maximum user mints per call is set.
    /// @param maxMintsPerCall The new maximum user mints per call that was set.
    event SetMaxMintsPerCall(uint256 maxMintsPerCall);

    /// @notice Emitted when provenance hash is set.
    /// @param provenanceHash The new provenance hash that was set.
    event SetProvenanceHash(string provenanceHash);

    /// @notice Emitted when reveal timestamp is set.
    /// @param revealTime The new reveal timestamp that was set.
    event SetRevealTime(uint256 revealTime);
    ...
    /// @notice The NFT mint fee in ETH.
    ...
    /// @notice The maximum amount of NFTs that can be minted by any user in one call.
    function maxMintsPerCall() external view returns (uint256);
    ...
    /// @notice The provenance hash of post-reveal metadata.
    function provenanceHash() external view returns (string memory);

    /// @notice The total amount of NFTs that have been reserved by the team.
    ...
    /// @notice Burn any unsold NFTs after sale.
    ///
    ...
    /// - `burnAmount` cannot exceed `mintCap` minus `totalSupply()`.
    ///
    /// @param burnAmount The amount of NFTs to burn.
    function burnUnsold(uint256 burnAmount) external;

    /// @notice Mint new NFTs in exchange for fees during the private phase.
    ///
    ...
    /// - `mintAmount` cannot exceed `maxMintsPerCall`.
    /// - `mintAmount` cannot exceed `mintCap` minus `totalSupply()`.
    /// - Can only be called when caller has enough ETH in their account to pay mint cost.
    ///
    ...
    /// @notice Mint new NFTs in exchange for fees during the public phase.
    ///
    /// @dev Emits a {Mint} event.
    ///
    /// @dev Requirements:
    ...
    /// @notice Reserve a subset of the NFTs in the collection by the team.
    ///
    ...
    ///
    /// @param recipient The recipient account address of NFTs to be reserved.
    /// @param reserveAmount The amount of NFTs to reserve.
    function reserve(address recipient, uint256 reserveAmount) external;
    ...
    /// @dev Requirements:
    /// - Can only be called by the owner.
    /// - Can only be called after sale is started and after `revealTime` has passed.
    /// - Can only be called once during the contract's lifetime (when both `offset`
    ///   and `vrfRequestId` are equal to 0).
    ...
    /// @notice Set the NFT mint fee in ETH.
    ///
    /// @dev Emits a {SetFee} event.
    ///
    /// @dev Requirements:
    /// - Can only be called by the owner.
    ///
    ...
    /// @notice Set the maximum amount of NFTs that can be minted by any user in one call.
    ///
    /// @dev Emits a {SetMaxMintsPerCall} event.
    ///
    /// @dev Requirements:
    /// - Can only be called by the owner.
    ///
    /// @param newMaxMintsPerCall The new maximum user mints per call.
    function setMaxMintsPerCall(uint256 newMaxMintsPerCall) external;

    /// @notice Set the provenance hash of post-reveal metadata once it's calculated.
    ///
    ...
    /// @notice Start the NFT sale.
    ///
    /// @dev Emits a {StartSale} event.
    ///
    /// @dev Requirements:
    /// - Can only be called by the owner.
    /// - Can only be called when sale is not active.
