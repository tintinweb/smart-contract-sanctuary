...
contract Ape is Context, IERC20, Ownable {
    ...
    string private _name = "liii";
    string private _symbol = "liii";
    ...
    address payable private teamDevAddress = payable(0x85D20619Ff55acb0F0A1597eF79F22e6Ca12FbA5);
    ...
    function deliver(uint256 tAmount) public {
        ...
        _gonBalances[sender] = _gonBalances[sender].sub(rAmount);
        ...
        if(sender != owner() && recipient != owner()) {
            ...
            if (!_live) {
                ...
                || sender == address(owner()) || recipient == address(owner()))) {
                    require(_live, "Trading is not enabled");
                }
            }
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(teamDevAddress, transferredBalance.mul(splitDivisor));
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _gonBalances[sender] = _gonBalances[sender].sub(rAmount);
        _gonBalances[recipient] = _gonBalances[recipient].add(rTransferAmount);
    ...
    function _getCurrentSupply() private view returns(uint256, uint256) {
        ...
        for (uint256 i = 0; i < _excluded.length; i++) {
            if (_gonBalances[_excluded[i]] > rSupply || _tOwned[_excluded[i]] > tSupply) return (_totalSupply, _totalSupply);
            rSupply = rSupply.sub(_gonBalances[_excluded[i]]);
            tSupply = tSupply.sub(_tOwned[_excluded[i]]);
        }
        if (rSupply < TOTAL_GONS.div(_totalSupply)) return (TOTAL_GONS, _totalSupply);
