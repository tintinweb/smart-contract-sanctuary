...
contract Fern is Context, IERC20, IERC20Metadata, Ownable {
  ...
  function lastClaim(uint256 tokenId) public view returns (uint256) {
    require(IERC721Enumerable(_nftAddress).ownerOf(tokenId) != address(0), "zero address");
    // あとで これコメントアウトして問題ないか動作確認
    //require(tokenId <= IERC721Enumerable(_nftAddress).totalSupply(), "invalid");
