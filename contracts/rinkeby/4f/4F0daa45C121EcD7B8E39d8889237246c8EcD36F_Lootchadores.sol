...
contract Lootchadores is ERC721Enumerable, Ownable, ERC165Storage, PaymentSplitter {
    ...
    function persistStats(uint256 tokenId, uint256 abilityId) internal {
        luchaAbilites[tokenId] = AbilityStats({
             charisma: extractValue(IAbilities(ABILITY_SCORES_ADDRESS).getCharisma(abilityId)),
            constitution: extractValue(IAbilities(ABILITY_SCORES_ADDRESS).getConstitution(abilityId)),
            dexterity: extractValue(IAbilities(ABILITY_SCORES_ADDRESS).getDexterity(abilityId)),
            intelligence: extractValue(IAbilities(ABILITY_SCORES_ADDRESS).getIntelligence(abilityId)),
            strength: extractValue(IAbilities(ABILITY_SCORES_ADDRESS).getStrength(abilityId)),
            wisdom: extractValue(IAbilities(ABILITY_SCORES_ADDRESS).getWisdom(abilityId)),
            tokenId: abilityId
        });
    }
    ...
    function extractValue(string memory ability) internal pure returns (uint256) {
        strings.slice memory value = ability.toSlice();
        value.split(" ".toSlice());
        return safeParseInt(value.toString(), 0);
    }
