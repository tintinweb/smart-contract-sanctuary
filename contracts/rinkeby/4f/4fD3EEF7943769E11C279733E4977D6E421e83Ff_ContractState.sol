...
import {TokenIdLib} from "./TokenId.sol";
import {Redeem, CollectionFlat, Series} from "./Redeem.sol";
import {Lock, NFTStatus} from "./Lock.sol";
...
contract ContractState {
    ...
    {
        (uint256[] memory tokenIds, string[] memory tokenURIs) =
            nft.tokensOfOwner(owner);

        NFTStatus[] memory statuses = new NFTStatus[](tokenIds.length);
        bool[] memory redeemed = new bool[](tokenIds.length);
        uint256[] memory unlockTimestamp = new uint256[](tokenIds.length);

        for (uint256 i = 0; i < tokenIds.length; i++) {
            statuses[i] = lock.nftStatus(nft, tokenIds[i]);
            redeemed[i] = lock.nftRedeemed(nft, tokenIds[i]);
            unlockTimestamp[i] =
                lock.lockPeriod(
                    nft,
                ...
                lock.nftLockTimestamp(nft, tokenIds[i]);
        }

        return (statuses, redeemed, unlockTimestamp, tokenIds, tokenURIs);
    }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            unlockTimestamp[i] =
                ...
                lock.nftLockTimestamp(nft, tokenIds[i]);
            tokenURIs[i] = nft.tokenURI(tokenIds[i]);
        }
    ...
    {
        uint256[] memory tokenIds = lock.getOwnerNFTs(nft, owner);
    ...
    function getCollections(Redeem redeem, Lock lock)
        public
        view
        returns (CollectionFlatWithRelic[] memory)
    {
        CollectionFlat[] memory collections = redeem.getCollections();
    ...
    function getMoonwalkRelicInfo(MoonwalkRedeem redeem, Lock lock)
        public
        view
        returns (CollectionRelicInfo memory)
    {
        Moonwalk nft = redeem.nft();

        // uint256[] memory minted = redeem.seriesMinted();
        uint256[3] memory minted =
            [nft.seriesMinted(0), nft.seriesMinted(1), nft.seriesMinted(2)];

        SeriesRelicInfo[] memory series = new SeriesRelicInfo[](minted.length);
        ...
        for (uint256 seriesId = 0; seriesId < series.length; seriesId++) {
            series[seriesId] = SeriesRelicInfo({
                id: seriesId,
                minted: minted[seriesId],
                power: lock.power(nft, collectionId, seriesId),
                lockPeriod: lock.lockPeriod(nft, collectionId, seriesId)
            });
        }
...
contract Lock is Ownable {
    ...
    mapping(IERC721 => mapping(uint256 => mapping(uint256 => uint256)))
        public power;
    mapping(IERC721 => mapping(uint256 => mapping(uint256 => uint256)))
        public lockPeriod;
    ...
    event NFTRedeemed(IERC721 indexed nft, uint256 indexed tokenId);
    ...
    function setPower(
        ...
        for (uint256 i = 0; i < seriesIds.length; i++) {
            power[nft][collectionId][seriesIds[i]] = powers[i];

            require(
                lockPeriod[nft][collectionId][seriesIds[i]] == 0 ||
                    lockPeriods[i] <=
                    lockPeriod[nft][collectionId][seriesIds[i]],
                "CANNOT_INCREASE_LOCK_PERIOD"
            );
            lockPeriod[nft][collectionId][seriesIds[i]] = lockPeriods[i];
        }
    }

    // USER METHODS ////////////////////////////////////////////////////////////

    function lockNFTs(IERC721 nft, uint256[] memory tokenIds) public {
        uint256 powerOwed;
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            powerOwed += power[nft][collectionId][seriesId];
        ...
        if (powerOwed > 0) {
            token.mint(msg.sender, powerOwed);
        }
    }

    // Only callable by the owner of the NFTs.
    function redeemNFTs(IERC721 nft, uint256[] memory tokenIds) public {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            require(
                nftStatus(nft, tokenId) != NFTStatus.Lockable,
                "NEVER_LOCKED"
            );
            ...
            emit NFTRedeemed(nft, tokenId);
        }
    }

    function lockAndRedeemNFTs(IERC721 nft, uint256[] memory tokenIds) public {
        lockNFTs(nft, tokenIds);
        redeemNFTs(nft, tokenIds);
    }

    // Only callable by the owner of the NFTs.
    function unlockNFTs(IERC721 nft, uint256[] memory tokenIds) public {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            require(nftOwner[nft][tokenId] == msg.sender, "NOT_NFT_OWNER");
            require(
    ...
    function lockMultipleNFTs(
        ...
        for (uint256 i = 0; i < nftArray.length; i++) {
            lockNFTs(nftArray[i], tokenIdsArray[i]);
        }
    }
    ...
    function redeemMultipleNFTs(
        ...
        require(nftArray.length == tokenIdsArray.length, "MISMATCHED_LENGTHS");
        for (uint256 i = 0; i < nftArray.length; i++) {
            redeemNFTs(nftArray[i], tokenIdsArray[i]);
        }
    }

    function lockAndRedeemMultipleNFTs(
        IERC721[] memory nftArray,
        uint256[][] memory tokenIdsArray
    ) public {
        require(nftArray.length == tokenIdsArray.length, "MISMATCHED_LENGTHS");
        for (uint256 i = 0; i < nftArray.length; i++) {
            lockAndRedeemNFTs(nftArray[i], tokenIdsArray[i]);
        }
    }

    function unlockMultipleNFTs(
        IERC721[] memory nftArray,
        uint256[][] memory tokenIdsArray
    ) public {
        require(nftArray.length == tokenIdsArray.length, "MISMATCHED_LENGTHS");
        for (uint256 i = 0; i < nftArray.length; i++) {
            unlockNFTs(nftArray[i], tokenIdsArray[i]);
        }
    }
        ...
        returns (NFTStatus)
    {
        ...
        uint256 tokenLockPeriod = lockPeriod[nft][collectionId][seriesId];
...
contract Token is Ownable, ERC20 {
    mapping(address => bool) minterRegistry;
    ...
    function registerMinter(address minter) public onlyOwner {
        minterRegistry[minter] = true;
    }

    function mint(address recipient, uint256 amount) public {
        require(minterRegistry[msg.sender], "NOT_MINTER");
