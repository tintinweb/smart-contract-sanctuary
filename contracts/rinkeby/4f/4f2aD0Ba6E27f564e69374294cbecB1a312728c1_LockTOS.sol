...
contract LockTOS is LockTOSStorage, AccessibleCommon, ILockTOS {
    ...
    function withdrawAll() external ifFree returns (uint256[] memory, uint256[] memory) {
        uint256[] storage locks = userLocks[msg.sender];
        uint256[] memory successIds;
        uint256[] memory failIds;
        if (locks.length == 0) {
        ...
        for (uint256 i = locks.length - 1; i >= 0; i--) {
            ...
            if (lock.amount > 0 && lock.start > 0 && lock.end < block.timestamp) {
                (bool success, ) = address(this).delegatecall(abi.encodeWithSignature("withdraw(uint256)", locks[i]));

                if(success){
                    successIds[successIds.length] = locks[i];
                } else {
                    failIds[successIds.length] = locks[i];
                }
                // _withdraw(locks[i]);
    ...
    function balanceOfLockTest(uint256 _lockId) public view
    returns (uint256 _balance, uint256 _blocktime, int256 _currentBias, int256 _slope, int256 _bias, uint256 _timestamp ) {
        uint256 len = lockPointHistory[_lockId].length;
        if (len == 0) {
            return (0,0,0,0,0,0);
        }
        ...
        LibLockTOS.Point memory point = lockPointHistory[_lockId][len - 1];
        int256 currentBias = point.slope.mul(block.timestamp.sub(point.timestamp).toInt256());
        _balance = uint256(point.bias > currentBias ? point.bias.sub(currentBias) : 0).div(MULTIPLIER);
    ...
    function locksInfo(uint256 _lockId)
        ...
        public
        view
        returns (uint256 start, uint256 end, uint256 amount, int256 boostValue)
    {
        return (
    ...
    function _deposit(address _addr, uint256 _lockId, uint256 _value, uint256 _unlockTime) internal ifFree {
       ...
       LibLockTOS.LockedBalance memory lockedNew = LibLockTOS.LockedBalance({
        ...
        LibLockTOS.Point memory userPoint = LibLockTOS.Point({
            ...
            boostValue: lockedNew.boostValue
        });
    ...
    function _recordHistoryPoints() internal returns (LibLockTOS.Point memory lastWeek) {
        ...
        if (pointHistory.length > 0) {
            ...
            lastWeek = LibLockTOS.Point({bias: 0, slope: 0, timestamp: timestamp, boostValue: 0});
        }
