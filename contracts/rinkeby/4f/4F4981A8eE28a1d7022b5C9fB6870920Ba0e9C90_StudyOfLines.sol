...
// keeps track of the number of tokens issu

// Accessing the Ownable method ensures that only the creator of the smart contract can interact with it
contract StudyOfLines is ERC721PresetMinterPauserAutoId {
    // the name and symbol for the NFT

    bool private _active = false;
    string private _tokenURIPrefix;
    address private constant _owner =
        0xBD2B82232e0954ade8a437709C95eDae33b9252a;
    uint256 private constant _max_mintable = 1000;
    uint256 private constant _max_givable = 100;
    ...
    constructor()
        ERC721PresetMinterPauserAutoId(
            "Study of Lines",
            "SOL",
            "https://www.studyoflines.art/api/token/"
        )
    {}
    ...
    function activate() external {
        require(
            ...
            "ERC721PresetMinterPauserAutoId: must have admin role"
        );
    ...
    function getMaxMintable() external pure returns (uint256) {
        return _max_mintable;
    }

    function getMaxGivable() external pure returns (uint256) {
        return _max_givable;
    }
    ...
    function buy(uint256 amount) external payable {
        ...
        // require(amount <= 5, "Too many requested"); //no mint limit
        require(
            amount <= _max_mintable - super.amountMinted(),
            "Not enough left"
        );
        require(
            msg.value >= amount * 100000000000000000,
    ...
    function giveaway(address receiver, uint256 amount) external {
        ...
        require(_active, "Inactive");
        require(
            amount <= _max_givable - super.amountGived(),
            "Not enough left to give"
        );

        for (uint256 i = 0; i < amount; i++) {
            uint256 tokenId = super.give(receiver);
    ...
    ERC721Pausable
{
    ...
    function amountMinted() internal view returns (uint256) {
        return _tokenIdTracker.current();
    }

    function amountGived() internal view returns (uint256) {
        return _givedAmountTracker.current();
    }
