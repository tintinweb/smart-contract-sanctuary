...
contract EmethCore is AssignerRole, VerifierRole {
    ...
    uint256 public VERIFIER_FEE = 10000000000000000000; // 0 EMT
    ...
    function request(bytes16 _jobId, uint256 _programId, string calldata _dataset, string calldata _param, uint256 _gas, uint256 _gasPrice, uint256 _timeLimit) external returns (bool) {
        ...
        uint256 fee = _gas.mul(_gasPrice) + VERIFIER_FEE;

        IERC20 token = IERC20(tokenAddress);
        require(token.balanceOf(msg.sender) >= fee, 'insufficient fee balance');
    ...
    function decline(bytes16 _jobId) external onlyAssignedNode(_jobId) returns (bool) {
        ...
        // Unlock Capacity
        _unlockCapacity(jobAssign.node, jobAssign.lockedCapacity);
    ...
    function estimateGas(uint256 _datasetSizeMB, uint256 _algoComplexity) public pure returns (uint256) {
        return _datasetSizeMB.mul(_algoComplexity).div(1000);
    }

    function estimateFee(uint256 _datasetSizeMB, uint256 _algoComplexity, uint256 _gasPrice) public view returns (uint256) {
        return estimateGas(_datasetSizeMB, _algoComplexity).mul(_gasPrice).add(VERIFIER_FEE);
    }
    ...
    function _unlockCapacity(address _node, uint256 _lockedCapacity) private returns (bool) {
        if(nodes[_node].lockedCapacity > _lockedCapacity) {
            nodes[_node].lockedCapacity = nodes[_node].lockedCapacity.sub(_lockedCapacity);
        } else {
            nodes[_node].lockedCapacity = 0;
        }
        return true;
    }
    ...
    // Test Functions
    function setAssignerFee(uint256 _fee) external onlyOwner returns (bool) {
        ASSIGNER_FEE = _fee;
        return true;
    }

    function setVerifierFee(uint256 _fee) external onlyOwner returns (bool) {
        VERIFIER_FEE = _fee;
        return true;
    }

    function setMaxSlotGasPerNode(uint256 _maxSlotGas) external onlyOwner returns (bool) {
        MAX_SLOT_GAS_PER_NODE = _maxSlotGas;
        return true;
    }

    function updateJob (bytes16 _jobId, address _owner, uint256 _status, uint256 _requestedAt, uint256 _assignedAt) external onlyOwner returns (bool) {
        Job storage job = jobs[_jobId];
        job.owner = _owner;
        job.status = _status;
    ...
    function updateJobDetail(bytes16 _jobId, uint256 _programId, string calldata _param, string calldata _dataset, string calldata _result) external onlyOwner returns (bool) {
        JobDetail storage jobDetail = jobDetails[_jobId];
    ...
    function updateJobAssign(bytes16 _jobId, address _node, uint256 _timeLimit, uint256 _gas, uint256 _gasPrice, uint256 _lockedCapacity, uint256 _retryCount) external onlyOwner returns (bool) {
        JobAssign storage jobAssign = jobAssigns[_jobId];
        jobAssign.node = _node;
        jobAssign.timeLimit = _timeLimit;
        jobAssign.gas = _gas;
    ...
    function updateNode(address _node, bool _active, uint256 _totalCapacity, uint256 _lockedCapacity, uint256 _workers, uint256 _deposit) external onlyOwner returns (bool) {
        Node storage node = nodes[_node];
        node.active = _active;
        node.totalCapacity = _totalCapacity;
