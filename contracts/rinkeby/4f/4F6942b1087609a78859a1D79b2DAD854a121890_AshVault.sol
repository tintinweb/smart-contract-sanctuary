...
contract AshVault is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    function createTokenURI(address recipient, uint256 tokenId) internal view returns(string memory) {
        ...
        string memory json = Base64.encode(
            bytes(string(
                abi.encodePacked(
                    '{"name": "AshVault #', uint2str(tokenId+= 1), '/', uint2str(maxSupply), '",',
                    '"description": "This token changes depending on its owner\'s $ASH token balance. Each $ASH symbol represents 10 $ASH. Click to invert.",',
