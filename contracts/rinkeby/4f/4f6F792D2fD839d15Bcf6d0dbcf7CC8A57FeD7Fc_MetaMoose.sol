...
interface IForest {
    function randomHunterOwner() external returns (address);
...
contract MetaMoose is ERC721, Ownable {
    ...
    address private constant fundWallet = 0xA629E14908F5cE17F3AFA3BAF5F7318d06091362;

    event TokenStolen(address owner, uint16 tokenId, address thief);
    event Lucky(address owner, uint16 tokenId);

    constructor(uint maxSupply) ERC721("Meta Moose", "Meta Moose") {
        MAX_MINT_SUPPLY = maxSupply;

        // Set default price for each phase
        phasePrice[1] = 20000 ether;
        phasePrice[2] = 40000 ether;
        phasePrice[3] = 80000 ether;
    }
    ...
    function availableTokens(uint id) public view returns (uint16) {
        return _availableTokens[id];
    }
    ...
    function addAvailableTokens(uint16 _from, uint16 _to) external onlyOwner {
        _addAvailableTokens(_from, _to);
    }

    function removeFromAvailableTokens(uint16 _from, uint16 _to) external onlyOwner {
        for (uint16 i = _from; i <= _to; i++) {
            for (uint16 j = 0; j < _availableTokens.length; j++) {
                if (_availableTokens[j] == i) {
                    _availableTokens[j] = _availableTokens[_availableTokens.length - 1];
    ...
    function switchToSalePhase(uint16 _phase, bool _setTokens) external onlyOwner {
        phase = _phase;
        ...
        if (phase == 0) {
            _addAvailableTokens(0, 8887);
        } else if (phase == 1) {
            _addAvailableTokens(8888, 13331);
        } else if (phase == 2) {
            _addAvailableTokens(13332, 15553);
        } else if (phase == 3) {
            _addAvailableTokens(15554, 17775);
        }
    }
    ...
    */
    function publicMint(uint amount, bool stake) external payable {
        ...
        if (phase == 0) {
            // Paid mint
            require(mintPrice.mul(amount) == msg.value, "Invalid payment amount");
        } else {
            // Mint via Penny token burn
            require(msg.value == 0, "Now minting is done via Penny");
            totalPennyCost = phasePrice[phase].mul(amount);
            if (totalPennyCost > 0) {
        ...
        for (uint i = 0; i < amount; i++) {
            address recipient = _selectRecipient();
            ...
            // 5% chance to be a lucky man
            if (phase != 0 && _availableTokens.length > (amount - i - 1) && randomGen.getRandomNumber(_tokensMinted, 100) < 5) {
                if (!isLucky) {
                    isLucky = true;
                }
    ...
    */
    // function privateMint(uint _count) external payable {
    ...
    function _selectRecipient() private returns (address) {
        if (phase == 0) {
            return msg.sender; // During ETH sale there is no chance to steal NTF
        }

        // 10% chance to steal NTF
        if (randomGen.getRandomNumber(hunterMinted, 100) >= 10) {
            return msg.sender; // 90%
        }

        address thief = forest.randomHunterOwner();
