...
contract StakingImp is Ownable(address(0)), ReentrancyGuard {
    ...
    uint256 public rewardPerTokenStored; // per 1 staking token, i.e. per 10**18 units
    ...
    uint public constant stakingTokenUnits = 1e18;
    ...
    function initialize(address _stakingToken, uint _unstakingFeeRatio, address _owner, uint emissionStart, uint firstCheckPoint, uint _rewardPerSecond) public {
    ...
    function getRewardThenStake() public nonReentrant updateReward(msg.sender) {
        ...
        if (reward > 0) {
            require(reward < stakingToken.balanceOf(address(this)).sub(totalStake), "Staking: not enough tokens to pay out reward.");
    ...
    function removeStake(uint stake, uint maximumFee) public nonReentrant updateReward(msg.sender) {
        ...
        require(unstakingFee <= maximumFee, "Staking: fee too high.");
        uint stakeWithoutFee = stake.sub(unstakingFee);
        require(stakes[msg.sender] >= stake, "Staking: INSUFFICIENT_STAKE");
        stakes[msg.sender] = stakes[msg.sender].sub(stake);
        totalStake = totalStake.sub(stake);
        stakingToken.safeTransfer(msg.sender, stakeWithoutFee);
    ...
    function showPendingReward(address account) public view returns (uint256) {
        ...
        if (totalStake != 0) {
            ...
            rewardPerTokenStoredActual = rewardPerTokenStored.add(totalEmittedTokensSinceLastUpdate.mul(stakingTokenUnits).div(totalStake));
        } else {
            rewardPerTokenStoredActual = rewardPerTokenStored;
        }
        return rewards[account].add((rewardPerTokenStoredActual.sub(userRewardPerTokenPaid[account])).mul(stakes[account]).div(stakingTokenUnits));
    }
    ...
    function _updateReward(address account) internal {
        if (totalStake != 0) {
            ...
            rewardPerTokenStored = rewardPerTokenStored.add(totalEmittedTokensSinceLastUpdate.mul(stakingTokenUnits).div(totalStake));
        }
        lastUpdateTime = lastTimeRewardApplicable();
        if (account != address(0)) {
            uint256 _rewardPerTokenStored = rewardPerTokenStored;
            rewards[account] = rewards[account].add((_rewardPerTokenStored.sub(userRewardPerTokenPaid[account])).mul(stakes[account]).div(stakingTokenUnits));
