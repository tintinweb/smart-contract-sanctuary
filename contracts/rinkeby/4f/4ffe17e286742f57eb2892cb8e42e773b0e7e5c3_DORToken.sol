...
contract DORToken is ERC721Enumerable, ReentrancyGuard, Pausable, WhitelistAdminRole {
    ...
    mapping(uint256 => bool) claimRandomFlags; // claim flags for random
    ...
    function setClaimRandomFlag(uint256 tokenId, bool flag) internal {
        claimRandomFlags[tokenId] = flag;
    }
    ...
    function getClaimRandomFlag(uint256 tokenId) public view returns (bool) {
        return claimRandomFlags[tokenId];
    }
    ...
    function canClaimRandom(address account) public view returns (bool) {
        ...
        uint256 cnt;
        for (uint256 i = 0; i < tokenTypes.length; i++) {
            if (tokenTypes[i] == uint256(TokenType.RARE) && !getClaimRandomFlag(tokenIds[i])) {
                cnt = cnt.add(1);
            }
        }
    ...
    function claimRandom() public {
        ...
        for (uint256 i = 0; i < tokenTypes.length && c > 0; i++) {
            if (tokenTypes[i] == uint256(TokenType.RARE) && !getClaimRandomFlag(tokenIds[i])) {
                setClaimRandomFlag(tokenIds[i], true);
                c = c.sub(1);
            }
        }
