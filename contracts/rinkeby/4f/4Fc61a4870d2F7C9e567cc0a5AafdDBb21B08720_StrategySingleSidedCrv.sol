...
import {BaseStrategy, VaultAPI} from "BaseStrategy.sol";

interface IUni {
    function getAmountsOut(uint256 amountIn, address[] calldata path) external view returns (uint256[] memory amounts);
}

contract StrategySingleSidedCrv is BaseStrategy {
    ...
    function _initializeStrat(
        ...
        require(want_decimals == 0);
        require(_poolSize > 1 && _poolSize < 5);
    ...
    /// @notice Check if want is weth token
    /// @return If want is weth token - true, if not, then false
    function isWantWETH() internal view returns (bool) {
        return address(want) == address(weth);
    }

    /// @notice Returned stratagy name
    /// @return Strategy name
    function name() external override view returns (string memory) {
        return strategyName;
    }

    /// @notice Update min time per invest
    /// @param _minTimePerInvest Min time per invest
    ...
    function withdrawSome(uint256 _amount) internal returns (uint256 _liquidatedAmount, uint256 _loss) {
        ...
        if (amountFromVault > 0) {
            ...
            if (withdrawProtection) {
                ...
                require(yBalance.sub(yvToken.balanceOf(address(this))) >= amountFromVault.sub(1));
            }
        }
    ...
    function cloneSingleSidedCurve(
        ...
        StrategySingleSidedCrv(newStrategy).initialize(
                    _vault,
                    _strategist,
                    _maxSingleInvest,
                    _minTimePerInvest,
