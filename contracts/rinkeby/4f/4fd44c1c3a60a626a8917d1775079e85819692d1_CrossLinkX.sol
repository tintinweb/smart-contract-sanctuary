...
contract CrossLinkX is usingProvable, IERC20, ICrossLinkX, IERC20Metadata, Ownable {
    ...
    string public CoordinatorUrl; //TODO: Change to private
    ...
    */
    ...
    function test() public payable returns(bool){
        bytes32 bt = provable_query("URL", "json(https://api.pro.coinbase.com/products/ETH-USD/ticker).price");
        return true;
    }
    ...
    function _tryToRedeemTokens(address sender, uint256 id, uint256 amount, string memory fromChain) internal returns(bytes32){
        ...
        string memory queryString = string(abi.encodePacked("json(", CoordinatorUrl, "/token/event/from/", fromChain , "/to/", _currentChain , "/address/0x", 
