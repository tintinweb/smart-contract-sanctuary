...
// File: contracts/MoonBoys.sol

/**
 * @title MOONBOYS contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract MoonBoys is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public MAX_MOONBOY_SUPPLY;
    ...
    constructor() ERC721("MoonBoys", "MOONER") {
        MAX_MOONBOY_SUPPLY = 10000;
        REVEAL_TIMESTAMP = block.timestamp + (86400 * 7);
        mintPrice = 50000000000000000; // 0.050 ETH
        maxToMint = 30;
        saleIsActive = false;
        wallet1 = 0xed4B0bD877D3148ACC79C9eE9a1856b040391833; //1
        wallet2 = 0x54b52c5c4ceB42CF23cbE89aF8878FBdD9fCBfFA; //2
    }
    ...
    */
    function mint(uint256 numberOfTokens) external payable {
        ...
        require(totalSupply().add(numberOfTokens) <= MAX_MOONBOY_SUPPLY, "Purchase would exceed max supply");
        require(mintPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
        
        for(uint256 i = 0; i < numberOfTokens; i++) {
            uint256 mintIndex = totalSupply();
            if (totalSupply() < MAX_MOONBOY_SUPPLY) {
        ...
        if (startingIndexBlock == 0 && (totalSupply() == MAX_MOONBOY_SUPPLY || block.timestamp >= REVEAL_TIMESTAMP)) {
    ...
    function setStartingIndex() external {
        ...
        startingIndex = uint256(blockhash(startingIndexBlock)) % MAX_MOONBOY_SUPPLY;
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number.sub(startingIndexBlock) > 255) {
            startingIndex = uint256(blockhash(block.number - 1)) % MAX_MOONBOY_SUPPLY;
        }
