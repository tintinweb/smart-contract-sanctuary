...
contract Paragons is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 public constant MAX_PRESALE_PARAGONS = 4000;
    uint256 public constant MAX_PURCHASE_PER_TX = 3;
    uint256 public constant PRESALE_MAX_PURCHASE = 1;
    uint256 public constant AMOUNT_RESERVED = 100;
    uint256 public constant PARAGON_PRICE = 0.12 ether;
    uint256 public constant PRESALE_PARAGON_PRICE = 0.09 ether;

    mapping(bytes => bool) public usedToken;
    mapping(address => bool) public presaleMinted;
    address payable[] private _artistWallet = [payable(address(0xfA528fBAA85E8be1e72968aC283EAaaf5809C5a1)), payable(address(0x1Dac9dB572528fc9ae3aFE4c0A449Be53Eb98e86))];
    address payable[] private _founderWallets = [payable(address(0xfA528fBAA85E8be1e72968aC283EAaaf5809C5a1)), payable(address(0x1Dac9dB572528fc9ae3aFE4c0A449Be53Eb98e86))];
    address payable[] private _marketingWallets = [payable(address(0xfA528fBAA85E8be1e72968aC283EAaaf5809C5a1)), payable(address(0x1Dac9dB572528fc9ae3aFE4c0A449Be53Eb98e86))];
    ...
    function mintReserve(address reserveAddress) public onlyOwner {
        require(_tokenIds.current() == 0, "Reserved Minting has already occurred.");
    ...
    function presaleMint(string calldata salt, bytes calldata token, uint256 amount) external payable nonReentrant {
        require(state == State.PreSale, 'Presale is not active.');
        require(!Address.isContract(msg.sender), 'Contracts are not allowed to mint.');
        require(!presaleMinted[msg.sender], 'The wallet address has already minted.');
        require(_tokenIds.current() + amount <= MAX_PRESALE_PARAGONS, 'Max supply of tokens exceeded presale limit.');
        require(_verify(_hash(salt, msg.sender), token), 'Invalid token.');
        require(msg.value >= PRESALE_PARAGON_PRICE, 'Ether value sent is incorrect.');
    ...
    function mint(string calldata salt, bytes calldata token, uint256 amount) external payable nonReentrant {
        ...
        require(!Address.isContract(msg.sender), 'Contracts are not allowed to mint.');
        require(amount <= MAX_PURCHASE_PER_TX, 'Amount should not exceed 3 per transaction.');
    ...
    function setBaseURI(string calldata newBaseURI) external onlyOwner {
     ...
     function withdrawAllEth(address payable payee) public virtual onlyOwner {
         ...
         for(uint i = 0; i < _artistWallet.length; i++) {
             _artistWallet[i].transfer(currentBalance * 2 / 100);
         }
