...
contract MockCloudVesting is OwnableUpgradeable {
    ...
    function initialize(address _presale, address _cloud) public virtual initializer {
        __Ownable_init();
     ...
     */
    function adminSetVestingStart(uint256 _vestingStart) external {
        require(
            _vestingStart > presale.PRE_SALE_END() && block.timestamp < _vestingStart,
            "Incorrect time provided"
        );
        vestingStart = _vestingStart;
    }

    function setDurationMOCK(uint256 _duration) external{
    ...
    function claim() external {
        ...
        require(block.timestamp > presale.PRE_SALE_END(), "Not allowed to claim now");
        uint256 unclaimed = claimable(_msgSender());
        require(unclaimed > 0, "TokenVesting: no tokens are due");

        _claimed[_msgSender()] += unclaimed;
        IERC20Upgradeable(CLOUDToken).safeTransfer(_msgSender(), unclaimed);
        emit CLOUDClaimed(_msgSender(), unclaimed);
    }
    ...
    function claimable(address _user) public view returns (uint256) {
        return _vestedAmount(_user) - _claimed[_user];
    }

    /**
     * @dev Calculates the amount that has already vested.
     * @param _user Address of the user who purchased tokens
     * @return Amount of CLOUD already vested
     */
    function _vestedAmount(address _user) private view returns (uint256) {
        if (block.timestamp >= vestingStart + vestingDuration) {
            return presale.purchased(_user);
        } else {
            return (presale.purchased(_user) * (block.timestamp * vestingStart)) / vestingDuration;
        }
    }
}
