...
contract NFT is ERC721Enumerable, Ownable {
  ...
  constructor( string memory _projectName, string memory _projectSymbol, string memory _initBaseURI) ERC721(_projectName, _projectSymbol) {
  ...
  function airdropToWhitelisted() public payable onlyOwner {

    require(!paused, "The sale is paused");
    uint256 supply = totalSupply();
    uint256 listLength = whitelistedAddresses.length;
    require(supply + listLength <= maxSupply, "Max NFT limit exceeded");

    for (uint256 i = 0; i < listLength; i++) {
      _safeMint(whitelistedAddresses[i], supply + i + 1);
    }
  }
