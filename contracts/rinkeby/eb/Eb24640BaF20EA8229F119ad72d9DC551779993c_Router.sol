...
contract Router is OwnableUpgradeable, IRouter {
    ...
    function initialize(address _beaconLP, address _nftAzuroBetToken)
        public
        virtual
        initializer
    {
        ...
        feedRegistry.push(feedData(address(0), address(0), false));
    }
    ...
    function registerFeed(
        ...
        feedRegistry.push(feedData(feedAddress, feedOwner, isActive));
        feedtoNumberMap[feedAddress] = feedRegistry.length - 1;
        // fill up coreTypes for FEED
        for (uint256 index = 0; index < _coreTypes.length; index++) {
            FeedtoCoreType[feedAddress][_coreTypes[index]] = isActive;
        }
    ...
    function createCondition(
        ...
        for (uint256 index = 0; index < lpRegs.length; index++) {
            ...
            require(
                FeedtoCoreType[msg.sender][coreTypeList[index]],
                "not active coreType"
            );
            //get lp
            ILP(lpRegistry[index].pool).createConditionTest( // FIX ME index
                oracleConditionID,
                coreTypeList[index],
                rates,
                timestamp,
    ...
    function bet(
        ...
        IAzuroBet(nftAzuroBetToken).inclastBetNumber();
    ...
    /**
     * @dev withdraw prize interface function
