...
contract UnipilotFarm is IUnipilotFarm, IERC721Receiver {
    ...
    mapping(uint256 => mapping(address => bool)) public farmingActive;
    ...
    function insertPool(address poolAddress) internal onlyGovernance {
        ...
        returns (uint256)
    {
        ...
        poolInfo[poolAddress].rewardPerBlock = value.mul(1e18);
        emit UpdatePilotPerBlock(old, poolInfo[poolAddress].rewardPerBlock);
        ...
        returns (uint256)
    {
        ...
        poolInfo[poolAddress].rewardMultiplier = value.mul(1e18);
        emit UpdateMultiplier(old, poolInfo[poolAddress].rewardMultiplier);
    ...
    function nftStatus(uint256 tokenId) external view override returns (bool) {
        return farmingActive[tokenId][userInfo[tokenId].poolAddress];
    }
        ...
        returns (bool)
    {
        ...
        farmingActive[tokenId][poolAddress] = true; // user's farming active
        ...
        return farmingActive[tokenId][poolAddress];
    }
    ...
    function withdrawNFT(uint256 tokenId) external override returns (bool isLeft) {
        ...
        farmingActive[tokenId][poolAddress] = false;
        ...
        returns (uint256)
    {
        ...
        poolInfo[poolAddress].altRewardPerBlock = value.mul(1e18);
        emit UpdateAltPerBlock(old, poolInfo[poolAddress].altRewardPerBlock);
        ...
        returns (uint256)
    {
        ...
        poolInfo[poolAddress].altRewardMultiplier = value.mul(1e18);
        emit UpdateAltMultiplier(old, poolInfo[poolAddress].altRewardMultiplier);
    ...
    function toggleActiveAlt(address poolAddress) external onlyGovernance returns (bool) {
        ...
        emit UpdateActiveAlt(old, poolInfo[poolAddress].isAltActive);
        ...
        returns (address)
    {
        ...
        emit UpdateAltToken(old, poolInfo[poolAddress].altToken);
...
interface IUnipilotFarm {
    ...
    event UpdatePilotPerBlock(uint256 old, uint256 updated);

    event UpdateMultiplier(uint256 old, uint256 updated);

    event UpdateAltPerBlock(uint256 old, uint256 updated);

    event UpdateAltMultiplier(uint256 old, uint256 updated);

    event UpdateActiveAlt(bool old, bool updated);

    event UpdateFarmingLimit(uint256 old, uint256 updated);

    event UpdateAltToken(address old, address updated);
