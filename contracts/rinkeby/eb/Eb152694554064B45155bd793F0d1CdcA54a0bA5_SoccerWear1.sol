...
contract SoccerWear1 is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getSubstitute(uint256 tokenId, string[14] memory previousOutputArray) public view returns (string memory) {
        ...
        return _pluck(tokenId, "SUBSTITUTE", allPlayers, previousOutputArray);
    }

    function getAllPlayers() public view returns (string[] memory) {
        uint256 i;
        uint256 length = _goalkeepers.length;
        string[] memory allPlayersConc;

        for (i; i < length; i++) {
            uint256 iLocal = 0;
            allPlayersConc[i] = _goalkeepers[iLocal];
            iLocal++;
        }
        length += _rightFullbacks.length;
        for (i; i < length; i++) {
            uint256 iLocal = 0;
            allPlayersConc[i] = _rightFullbacks[iLocal];
            iLocal++;
        }
        length += _leftFullbacks.length;
        for (i; i < length; i++) {
            uint256 iLocal = 0;
            allPlayersConc[i] = _leftFullbacks[iLocal];
            iLocal++;
        }
        length += _centerBacks.length;
        for (i; i < length; i++) {
            uint256 iLocal = 0;
            allPlayersConc[i] = _centerBacks[iLocal];
            iLocal++;
        }
        length += _sweepers.length;
        for (i; i < length; i++) {
            uint256 iLocal = 0;
            allPlayersConc[i] = _sweepers[iLocal];
            iLocal++;
        }
        length += _defendingMidfielders.length;
        for (i; i < length; i++) {
            uint256 iLocal = 0;
            allPlayersConc[i] = _defendingMidfielders[iLocal];
            iLocal++;
        }
        length += _rightMidfielders.length;
        for (i; i < length; i++) {
            uint256 iLocal = 0;
            allPlayersConc[i] = _rightMidfielders[iLocal];
            iLocal++;
        }
        length += _centralMidfielders.length;
        for (i; i < length; i++) {
            uint256 iLocal = 0;
            allPlayersConc[i] = _centralMidfielders[iLocal];
            iLocal++;
        }
        length += _strikers.length;
        for (i; i < length; i++) {
            uint256 iLocal = 0;
            allPlayersConc[i] = _strikers[iLocal];
            iLocal++;
        }
        length += _attackingMidfielders.length;
        for (i; i < length; i++) {
            uint256 iLocal = 0;
            allPlayersConc[i] = _attackingMidfielders[iLocal];
            iLocal++;
        }
        length += _leftMidfielders.length;
        for (i; i < length; i++) {
            uint256 iLocal = 0;
            allPlayersConc[i] = _leftMidfielders[iLocal];
            iLocal++;
        }
    ...
    function _pluck(
        ...
        uint256 rand = _random(string(abi.encodePacked(keyPrefix, _toString(tokenId))));
    ...
    constructor() ERC721("SoccerWear1", "SOCWEAR1") Ownable() {}
}
