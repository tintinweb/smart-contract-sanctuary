...
// first version testing address = > 0x38765074c5f8d568d8B3f26956B40B5bF3f3Bb43
// second version testing address = > 
/**
...
contract APIConsumer is ChainlinkClient {
    ...
    uint32[] public valuationArr;
    
    string public valuationString;
    ...
    event RequestValuationPriceFulfilled(
        bytes32 indexed requestId,
        string indexed valuation
    );

    event RequesFulfilledWithUintArr(
        bytes32 indexed requesId,
        uint32[] indexed valuation
    );
    ...
    function fulfillPriceValuation(bytes32 _requestId, string memory _valuation)
    public
    recordChainlinkFulfillment(_requestId)
    {
        emit RequestValuationPriceFulfilled(_requestId, _valuation);
        valuationString = _valuation;
    }
    
    
    
    function testLargeResponse(address _oracle, string memory _jobId, string memory payload)
    public
    {
        
        Chainlink.Request memory req = buildChainlinkRequest(stringToBytes32(_jobId), address(this), this.fulfillPriceValuation.selector);
        req.add("payload", payload);
        // stores the uint256 array as bytes and adds it to request
        req.add("apiKey", "123456879");
        string[] memory arr = new string[](5);
        for(uint i = 0; i <5; i++){
            arr[i] = uint2str(i*100);
        }
        req.addStringArray("idArray", arr);
        sendChainlinkRequestTo(_oracle, req, ORACLE_PAYMENT);
    }

    function testLargeResponseWithArrayReturned(address _oracle, string memory _jobId, string memory payload) public {
        Chainlink.Request memory req = buildChainlinkRequest(stringToBytes32(_jobId), address(this), this.fulfillPriceWithUintArray.selector);
        req.add("payload", payload);
        // stores the uint256 array as bytes and adds it to request
        req.add("apiKey", "123456879");
        string[] memory arr = new string[](5);
    ...
    function fulfillPriceWithUintArray(bytes32 _requestId, uint32[] memory _valuation) public recordChainlinkFulfillment(_requestId) {
        valuationArr = _valuation;
        emit RequesFulfilledWithUintArr(_requestId, _valuation);
    }


    function requestCustomAdapterWithArray(address _oracle, string memory _jobId, string memory payload, uint256[] calldata IDs)
    public
    {
        ...
        // stores the uint256 array as bytes and adds it to request
        req.addBytes("idArray", uintArrayToBytes(IDs));
        sendChainlinkRequestTo(_oracle, req, ORACLE_PAYMENT);
    }

    function requestCustomAdapter(address _oracle, string memory _jobId, string memory payload)
    public
    {
    ...
    function uintArrayToBytes(uint256[] calldata x) private pure returns (bytes memory b) {
        for (uint256 i = 0; i < x.length; i++) {
            b = abi.encodePacked(b, x[i]);
        }
    }
...
library Chainlink {
  ...
  // custom implementation @Phil
  function addUintArray(Request memory self, string memory key, uint256[] calldata values)
    internal pure
  {
    self.buf.encodeString(key);
    self.buf.startArray();
    for (uint256 i = 0; i < values.length; i++) {
      self.buf.encodeUInt(values[i]);
    }
    self.buf.endSequence();
  }
}