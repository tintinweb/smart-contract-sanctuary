...
contract GeckTest is ERC721Enumerable {
    ...
    function mintInternal() internal {
        ...
        if (legendariesMinted < 9) {
            ...
            if (legendaryDecider == 0) {
                ...
                _tokenIdToLegendaryName[thisTokenId] = traitTypes[
                    legendariesMinted
                ][0].traitName;
    ...
    function legToMetadata(uint256 _tokenId)
        public
        view
        returns (string memory)
    {
    ...
    function hashToMetadata(uint256 _tokenId, string memory _hash1)
        public
        view
        returns (string memory)
    {
        ...
        returns (string memory)
    {
        ...
        if (tokenIsLegendary) {
            ...
            tokenString = hashToMetadata(_tokenId, tokenHash);
        }
    ...
    function addLegNames(Legendary[] memory names) public onlyOwner {
        for (uint256 i = 0; i < names.length; i++) {
            legNames[0].push(Legendary(names[i].legName));
        }
