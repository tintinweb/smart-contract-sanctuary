...
contract TokenGateDutch is Ownable {
    ...
    function setTokenGateAddress(address _tokenGateAddress) external onlyAdmin {
        ...
        auctionLengthInBlocks = 4 * 60; // roughly an hour
    }
    ...
    function getCurrentThreshold() external view returns (uint256) {
        ...
        if (currentBlock - auctionLengthInBlocks > startBlock) {
    ...
    function getThresholdAtBlock(uint _startBlock, uint256 _currentBlock, uint256 _startThresh, uint256 _endThresh, uint256 _auctionLen) external pure returns (uint256) {
