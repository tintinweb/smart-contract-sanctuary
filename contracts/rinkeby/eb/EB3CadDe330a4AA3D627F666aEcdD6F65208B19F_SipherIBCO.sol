...
contract SipherIBCO is Ownable {
    ...
    uint256 public constant START = 1638378000;
    uint256 public constant END = START + 3 days;
    uint256 public constant TOTAL_DISTRIBUTE_AMOUNT = 40000000 * 10**18;
    uint256 public constant MINIMAL_PROVIDE_AMOUNT = 6000 ether;
    ...
    function claim() external {
        ...
        if (totalProvided >= MINIMAL_PROVIDE_AMOUNT) {
            uint256 sipherAmount = (TOTAL_DISTRIBUTE_AMOUNT * userShare) / totalProvided;
            SIPHER.safeTransfer(msg.sender, sipherAmount);
            emit Claimed(msg.sender, userShare, sipherAmount);
        } else {
            uint256 sipherAmount = (TOTAL_DISTRIBUTE_AMOUNT * userShare) / MINIMAL_PROVIDE_AMOUNT;
            SIPHER.safeTransfer(msg.sender, sipherAmount);
            emit Claimed(msg.sender, userShare, sipherAmount);
        }
    }
    ...
    function _withdrawCap(uint256 userAccumulated) internal pure returns (uint256 withdrawableAmount) {
        uint256 takeBackPercentage;
        if (userAccumulated <= 1 ether){
            takeBackPercentage = 100;
            return userAccumulated;
        } else if (userAccumulated > 1 ether && userAccumulated <= 150 ether) {
            uint256 accumulatedTotalInETH = userAccumulated/(10**18);
            takeBackPercentage = (3*accumulatedTotalInETH**2 + 70897 - 903*accumulatedTotalInETH)/1000;
            return  userAccumulated * takeBackPercentage/100;
        } else if ( userAccumulated > 150 ether) {
            takeBackPercentage = 3;
    ...
    function withdraw(uint256 amount) external {
        ...
        uint256 userInvested = provided[msg.sender];
        provided[msg.sender] = userInvested - amount;
