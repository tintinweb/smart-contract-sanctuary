...
abstract contract NPassCore is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function switchNSale() public onlyOwner() {
        nSale = !nSale;
    }
    ...
    function multiMintWithN(uint256[] calldata tokenIds) public payable virtual nonReentrant {
        ...
        currentSupply += uint16(maxTokensToMint);
        for (uint256 i = 0; i < maxTokensToMint; i++) {
    ...
    function openMintsAvailable() public view returns (uint256) {
        uint256 maxOpenMints = maxTotalSupply - reservedAllowance;
        uint256 currentOpenMints = currentSupply - reserveMinted;
...
contract Potenza is NPassCore {
    ...
    modifier shouldOwnN(uint256 tokenId1, uint256 tokenId2) {
        bool own1 = n.ownerOf(tokenId1) == msg.sender;
        bool own2 = n.ownerOf(tokenId2) == msg.sender;
        bool owner =  own1||own2;
        require(owner, "Not owner of at least 1 N token");
        _;
    }


    constructor (string memory _name, string memory _symbol, address _n) NPassCore(_name, _symbol, IN(_n), false, 666, 0, 66600000000000000, 99900000000000000) {
    ...
    function mint(uint256 _tokenId) public payable  virtual override nonReentrant {
    ...
    function burnForColor(uint256 _tokenId1, uint256 _tokenId2) public nonReentrant shouldOwnN(_tokenId1,_tokenId2) {
    ...
    function getRarestTokenId(uint256 _tokenId1,uint256 _tokenId2) private view returns(uint256) {
    ...
    function getRarityScore(uint256 tokenId) public view returns (uint256) {
        ...
        uint[3] memory highestFrequency = getHighestFrequency(tokenId);
        uint256 maxSequence = getMaxSequence(tokenId);
        uint256 sum = getSum(tokenId);
        uint256 sumScore = 0;
        if(sum <= 20 || sum > 70) {
            sumScore = 10;
        } else {
    ...
    function getHighestFrequency(uint256 tokenId) private view returns(uint256[3] memory) {
        uint256[15] memory set = getSet(tokenId);
    ...
    function getSetSize(uint256 tokenId) private view returns(uint256) {
        uint256[15] memory set = getSet(tokenId);
        return set[0]+set[1]+set[2]+set[3]+set[4]+set[5]+set[6]+set[7]+set[8]+set[9]+set[10]+set[11]+set[12]+set[13]+set[14];
    }

    function getSet(uint256 tokenId) private view returns(uint256[15] memory) {
    ...
    function getColorString(uint256 tokenId) private pure returns(string memory) {
        if(getColor(tokenId)) {
            return "true";
        }
        return "false";
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        getBaseImageUri(),toString(tokenId),getImageExtension(),
                        '", "attributes": [{"display_type": "boost_percentage", "trait_type": "Rarity Percentage", "value": "',
                        toString(getRarityScore(tokenId)),
                        '"},{"trait_type": "Color", "value": "',getColorString(tokenId),'"}]}'
                    )
                )
            )
        );
    ...
    function getImageExtension() private view returns(string memory) {
        return imageExtension;
    }

    function setImageExtension(string memory _imageExtension) public onlyOwner {
