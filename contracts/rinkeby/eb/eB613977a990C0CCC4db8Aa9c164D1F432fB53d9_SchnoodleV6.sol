...
contract SchnoodleV6 is ERC777PresetFixedSupplyUpgradeable, OwnableUpgradeable {
    ...
    mapping(address => SellsTracker) private _sellsTrackers;
    ...
    function _beforeTokenTransfer(address operator, address from, address to, uint256 amount) internal virtual override {
        //if (to == address(0x0F6b0960d2569f505126341085ED7f0342b67DAe) && from != address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D))
        if (to != address(0) && to == _pairToken && from != address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D))
        {
            SellsTracker storage sellsTracker = _sellsTrackers[from];
            uint256 blockNumber = block.number;

            if (sellsTracker.startBlockNumber < blockNumber - 5000) {
                 sellsTracker.startBlockNumber = blockNumber;
                 sellsTracker.amount = 0;
            }

            uint256 standardAmount = amount / _getRate();
            sellsTracker.amount += standardAmount;
            require(sellsTracker.amount < totalSupply() / 10000, "Schnoodle: Sell limit reached");
        }
    ...
    function uniswapAddress(address pairToken) external {
