...
contract HPDS is ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    uint256 public HPDSPrice = 100000000000000000; // 1 FTM
    uint256 public maxHPDSPurchase = 10;
    uint256 public maxHPDS = 10000;
    ...
    event HPDSPriceChanged(uint256 price);
    ...
    constructor(address[] memory _admin) ERC721("HalloweenPandas", "HPDS") {
    ...
    function reserveHPDS(address _to, uint256 _amount)
        external
        onlyAdmin
        onReserve
    {
        require(_amount <= _reserved, "Exceeds reserved HPDS supply");
    ...
    function mintHPDS(uint256 num) public payable {
        uint256 supply = totalSupply();
        require(saleIsActive, "Sale is not active");
        require(num > 0, "Cannot buy 0");
        require(
            num < maxHPDSPurchase + 1,
            "Exceeds max number of HPDS in one transaction"
        );
        require(
            supply + num < maxHPDS - _reserved,
            "Purchase would exceed max supply of HPDS"
        );
        require(
            HPDSPrice.mul(num) <= msg.value,
