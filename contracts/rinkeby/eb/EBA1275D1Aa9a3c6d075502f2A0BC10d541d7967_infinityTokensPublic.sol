...
contract infinityTokensPublic is ERC721, OwnableClone, HasSecondarySaleFees {
	...
	function NFTMachineFor(uint256 NFTId, address _recipient) public payable {
		require(mintingActive[NFTId] == true, "Mint not active");
		require(totalMinted[NFTId] < editionSizeMemory[NFTId], "Cannot mint more");
		require(msg.value == priceMemory[NFTId]);
		
		_mintInternal(NFTId, _recipient);
	}
    ...
    function getMetadata(uint256 tokenId) public view returns (uint256 NFTRef, string memory staticMetadata, string memory dynamicMetadata, uint256 editionSize, uint256 editionNumber, uint256 evolutionLevel, uint256 remainingRedemptions, bool isActive, uint256 price) {		require(_exists(tokenId), "Token does not exist.");
