...
library NFTSVG {
  ...
  function _generateSVGDefs(SVGParams memory params) private view returns (string memory svgDef) {
    ...
    for(uint256 i = 0; i < 5; ++i){
      ...
      subDefs = string(abi.encodePacked(
        _buildWireBack(subIndex, rgbPrimary),
        subDefs,
        '<path id="r',
        (subIndex + 1).toString(),
        '" >',
    ...
    svgDef = string(
      abi.encodePacked(
        ...
        '</tspan></text>',
        '<path id="w1" >',
          '<animate attributeName="d" values="m5,160l310,0 0,0;m5,315l310,0 0,0;" dur="12s" repeatCount="indefinite" begin="p1.begin"/>',
          '<animate attributeName="stroke" values="rgb(0,0,0);',
          rgbPrimary,
          '" dur="12s" repeatCount="indefinite" begin="p1.begin"/>',
          '<animate attributeName="stroke-width" values=".1;.5;" dur="12s" repeatCount="indefinite" begin="p1.begin"/>',
        '</path>',
        '</defs>'
      )
    );
  ...
  function _buildWireBack(uint256 index, string memory color) private pure returns (string memory wirePaths){
