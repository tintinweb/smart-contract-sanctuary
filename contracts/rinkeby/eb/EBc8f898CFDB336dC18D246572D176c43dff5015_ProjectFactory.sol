...
contract ProjectFactory {
     ...
     */
    constructor(address _implementation, address _homeFi) {
    ...
    function createProject(
        ...
        require(msg.sender == homeFi, "Project::!HomeFiContract");
        _clone = ClonesUpgradeable.clone(implementation);
        Project(_clone).initialize(_hash, _currency, _sender, homeFi);
    }
}
...
contract Project is IProject {
    ...
    function inviteContractor(
        ...
        for (uint256 i = 0; i < _phaseCosts.length; i++) {
            ...
            phases[phaseCount].phaseCost = _phaseCosts[i]; //starts from 1
        ...
        contractorAccepted
    {
        ...
        for (uint256 i = 0; i < _phaseCostsLength; i++) {
            ...
            phaseCount += 1;
            checkPrecision(_phaseCosts[i]);
            phases[phaseCount].phaseCost = _phaseCosts[i];
        ...
        nonReentrant
    {
        ...
        if (msgSender() == builder) {
            if (currency == homeFi.etherCurrency()) {
                require(_cost == msg.value, "Project::!msg.value");
            } else {
                IERC20Upgradeable _token = IERC20Upgradeable(currency);
        ...
        contractorAccepted
    {
        ...
        (
            ...
            address _projectAddress
                ...
                _data,
                (uint256, bytes32[], bytes32[], uint256[], uint256, address)
            );
        require(_phaseID != 0 && _phaseID <= phaseCount, "Project::!Phase");
        require(!phases[_phaseID].paid, "Project::Paid");
        ...
        require(
            _hash1.length == _hash2.length &&
                _hash1.length == _taskCosts.length,
            "Project::Lengths !match"
        );
        ...
        for (uint256 i = 0; i < _hash1.length; i++) {
            ...
            tasks[taskCount].initialize(_hashArray, _taskCosts[i]);
            phases[_phaseID].phaseToTaskList.push(taskCount);
            nonFundedPhaseToTask[_phaseID].push(taskCount);
        }
        eventsInstance.tasksAdded(_phaseID, _taskCosts);
    }
    ...
    function updateTaskHash(bytes calldata _data, bytes calldata _signature)
        ...
        override
    {
        (bytes32[2] memory _taskHash, uint256 _nonce, uint256 _taskID) = abi
        ...
        require(_nonce == hashChangeNonce, "Project::!Nonce");
        tasks[_taskID].taskHash = _taskHash;
        hashChangeNonce += 1;
    ...
    function acceptInviteSC(uint256 _taskID) external override {
        tasks[_taskID].acceptInvitation(msgSender());
        eventsInstance.scConfirmed(_taskID);
    }
    ...
    function autoWithdraw(uint256 _amount) internal override {
        ...
        if (currency == homeFi.etherCurrency()) {
            payable(builder).transfer(_amount);
        } else {
            IERC20Upgradeable _token = IERC20Upgradeable(currency);
            _token.transfer(builder, _amount);
        }
 ...
 */
abstract contract IDisputes is Initializable, BasicMetaTransaction {
    /// INTERFACES ///
