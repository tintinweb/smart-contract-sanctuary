...
contract ERC20 is Context, IERC20 {
    ...
    uint256 public _totalSupply = 25000000000000000000000;
     ...
     * - the caller must hfave a balance of at least `amount`.
     */
...
// File: contracts/CRYOTOKEN.sol

pragma solidity ^0.6.2;



contract testlol is ERC20, Ownable {
  constructor() public ERC20("k4", "k4") { // Symbol and Name
  ...
  function setTransferFee(uint256 fee) public onlyOwner {
    require(fee <= 1400, "Fee cannot be greater than 14%");
    if (fee == 0) {
      activeFee = false;
    } else {
      activeFee = true;
  ...
  function setPairAddress(address account) public onlyOwner {
    pairAddress = account;
    emit PairAddressSet(account);
  }
  
  function _cooldownTimer(address recipient, uint256 amount) internal {    
    if (recipient != pairAddress && recipient != feeRecipient) {
      require(now > launchedAt + 3 minutes  || amount <= 5e19);
      require(now > launchedAt + 3 minutes  || block.timestamp - lastBuy[recipient] > 60);
  ...
  function transfer(address recipient, uint256 amount) public override returns (bool) {
    if(_msgSender() != owner() && recipient != owner()) {
        ...
        _cooldownTimer(recipient, amount);
    }
    if (activeFee && feeException[_msgSender()] == false) {
      if (((now - launchedAt)/3600 % 4) == 0) {
        transferFee = 600;
      }
      else if (((now - launchedAt)/3600 % 4) == 1) {
        transferFee = 800;
      }
      else if (((now - launchedAt)/3600 % 4) == 2) {
        transferFee = 1000;
      }
      else if (((now - launchedAt)/3600 % 4) == 3) {
        transferFee = 1200;
      }
  ...
  function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
    if (activeFee && feeException[recipient] == false) {
      ...
      uint256 fee = transferFee.mul(amount).div(10000);
      uint amountLessFee = amount.sub(fee);
      _transfer(sender, recipient, amountLessFee);
