...
contract Trash is Context, IERC20 {
    using SafeMath for uint256;

    // Constants
    uint256 public SECONDS_IN_A_DAY = 86400;

    uint256 public constant INITIAL_ALLOTMENT = 10 * (10 ** 18);

     uint256 public constant BURN_MULTIPLIER = 20;
     
     uint256 public constant SPECIAL_MULTIPLIER = 50;
    ...
    function lastClaim(uint256 tokenIndex) public view returns (uint256) {
        require(Library(_trashcans).ownerOf(tokenIndex) != address(0), "Owner cannot be 0 address");
        require(tokenIndex < Library(_trashcans).totalSupply(), "NFT at index has not been minted yet");
   ...
   function accumulated(uint256 tokenIndex) public view returns (uint256) {
     ...
     */
    function setTrashAddress(address trashcans) public {
        require(_trashcans == address(0), "Already set");
    ...
    function claim(uint256[] memory tokenIndices) public returns (uint256) {
        ...
        for (uint i = 0; i < tokenIndices.length; i++) {
            ...
            require(tokenIndices[i] < Library(_trashcans).totalSupply(), "NFT at index has not been minted yet");
            ...
            require(Library(_trashcans).ownerOf(tokenIndex) == msg.sender, "Sender is not the owner");
