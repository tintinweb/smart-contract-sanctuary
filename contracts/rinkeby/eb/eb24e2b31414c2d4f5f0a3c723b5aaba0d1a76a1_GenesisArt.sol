...
contract GenesisArt is ERC1155, Ownable, EIP712 {
    ...
    uint256 public trueOwnerFirstComissionPercentage;
    uint256 public trueOwnerSecondComissionPercentage;
    uint256 public platformComissionPercentage;
    
    address realOwner;
    
    constructor (string memory _uri, address _realOwner, address tradingToken1, address tradingToken2,
    uint256 _trueOwnerFirstComissionPercentage, uint256 _trueOwnerSecondComissionPercentage, uint256 _platformComissionPercentage,
    string memory eip712Name, string memory eip712Version) ERC1155(_uri) EIP712(eip712Name, eip712Version) {
        require(_realOwner != address(0), "GenesisArt: Real Owner zero address");
        require(tradingToken1 != address(0) && tradingToken2 != address(0), "GenesisArt: Trading Token zero address");
        require(_trueOwnerFirstComissionPercentage >= 0 && _trueOwnerFirstComissionPercentage <= 100, "GenesisArt: First true owner comission not allowed");
        
        uint256 sumSecondComissionPercentage = _trueOwnerSecondComissionPercentage + _platformComissionPercentage;
        
        require(sumSecondComissionPercentage >= 0 && sumSecondComissionPercentage <= 100, "GenesisArt: Second true owner comission not allowed");
        ...
        trueOwnerFirstComissionPercentage = _trueOwnerFirstComissionPercentage;
        trueOwnerSecondComissionPercentage = _trueOwnerSecondComissionPercentage;
        platformComissionPercentage = _platformComissionPercentage;
    }
    
    string private constant FORWARD_DATA_TYPE = "ForwardData(address from,address to,address tradingTokenAddr,uint256 id,uint256 amount,uint256 value,uint256 nonce)";
        ...
        external 
    {
        ...
        if (data.to == owner()) {
            uint256 toRealOwnerAmount = data.value.div(100).mul(trueOwnerFirstComissionPercentage);
            uint256 toPlatformAmount = data.value - toRealOwnerAmount;
            
            tradingToken.safeTransfer(realOwner, toRealOwnerAmount);
            tradingToken.safeTransfer(owner(), toPlatformAmount);
            
            bytes memory transferData;
            safeTransferFrom(data.to, data.from, data.id, data.amount, transferData);
        } else {
            uint256 toRealOwnerAmount = data.value.div(100).mul(trueOwnerSecondComissionPercentage);
            uint256 toPlatformAmount = data.value.div(100).mul(platformComissionPercentage);
            uint256 toAssetOwnerAmount = data.value - toRealOwnerAmount - toPlatformAmount;
