...
contract CryptoNeko is ERC721, Ownable  {
    ...
    uint256 public MAX_NEKO_CAT = 257;
   ...
   constructor() ERC721("CryptoNeko", "CryptoNeko") {
        //_baseTokenURI ="";
        _baseTokenURI ="https://crpytoneko.net/api/";
        
    }

    
    function tokensOfOwner(address _catowner) external view returns(uint256[] memory ) {
        uint256 tokenCount = balanceOf(_catowner);
        if (tokenCount == 0) {
    ...
    function calculatePrice() internal view returns (uint256) {
        ...
        price = 100000000000000000; //0.04
        } else if (numTokens >= 32 && numTokens < 64) {
        price = 200000000000000000; //0.08
        } else if (numTokens >= 64 && numTokens < 128) {
        price = 300000000000000000; //0.16
        } else {
        price = 400000000000000000; //0.32
    }
    return price;
    }
    // mint cat
