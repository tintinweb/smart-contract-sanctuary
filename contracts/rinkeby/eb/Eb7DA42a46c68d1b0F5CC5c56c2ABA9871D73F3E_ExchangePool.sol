...
contract ExchangePool is Ownable, IERC721Receiver {
    ...
    bytes32 public constant CLAIM_TYPEHASH = keccak256("Claim(address user,address tokenContract,uint256 tokenId)");

    event TokenReceived(address operator, address from, uint256 tokenId, address tokenContract, uint256 timestamp);
    event TokenClaimed(address user, address tokenContract, uint256 tokenId, uint256 timestamp);

    constructor() {}

    function claim(address user, address tokenContract, uint tokenId, uint8 v, bytes32 r, bytes32 s) external {
        bytes32 domainSeparator = keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes(CONTRACT_NAME)), getChainId(), address(this)));
        bytes32 structHash = keccak256(abi.encode(CLAIM_TYPEHASH, user, tokenContract, tokenId));
        ...
        emit TokenClaimed(user, tokenContract, tokenId, block.timestamp);
    }
