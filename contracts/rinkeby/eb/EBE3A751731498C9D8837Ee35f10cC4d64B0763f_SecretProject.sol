...
interface IERC721OwnerOfContract {
    function ownerOf(uint256 tokenId) external view returns (address);
}

contract SecretProject is ERC721Enumerable, ERC721Burnable, Ownable, ReentrancyGuard {
    ...
    // Faction Ids
    uint256 public constant FACTION_A_ID = 0;
    uint256 public constant FACTION_B_ID = 1;

    // Minting constants
    uint256 public maxMintPerTransaction;
    uint256 public constant FACTION_A_MINT_SUPPLY = 4400;
    uint256 public constant FACTION_B_MINT_SUPPLY = 4400;

    uint256 public constant FACTION_A_STAFF_SUPPLY = 44;
    uint256 public constant FACTION_B_STAFF_SUPPLY = 44;

    // 0.088 ETH
    uint256 public constant MINT_PRICE = 88000000000000000;

    // Keep track of supply
    uint256 public factionAMintCount = 0;
    uint256 public factionBMintCount = 0;
    ...
    // Presale
    mapping (address => bool) presaleClaimed; // Everyone who got in on the presale can only claim once.
    address signVerifier = 0xd974C841FF9ad100a992555F4587CA61c838E6Aa;


    constructor() 
    ERC721("--- SECRET PROJECT ----", "AAA") {
    ...
    // Sale is only finished
    function isSaleFinished()
    internal view returns (bool) {
        return factionAMintCount == FACTION_A_MINT_SUPPLY && factionBMintCount == FACTION_B_MINT_SUPPLY;
    }
    ...
    function mintPresale(
        ...
        // Verify signature
        bytes32 message = getPresaleSigningHash(tokenCount, presaleMaxMint, factionId).toEthSignedMessageHash();
        ...
        _handleFactionMint(
            ...
            factionId, 
            msg.sender, 
            FACTION_A_MINT_SUPPLY,
            FACTION_B_MINT_SUPPLY
        );
    }

    // @dev Allows minting a max of 4 tokens
    // @param tokensCount No. of tokens to mint
    // @param factionId A: 0 and B: 1
    ...
    public onlyOwner {
        ...
        _handleFactionMint(
            ...
            factionId, 
            recipient, 
            FACTION_A_MINT_SUPPLY + FACTION_A_STAFF_SUPPLY,
            FACTION_B_MINT_SUPPLY + FACTION_B_STAFF_SUPPLY
        );
    }
    ...
    // --Module--
    // Give Module contract total control over burning and minting new tokens.
    address private moduleContract;

    function setModuleContract(address module) 
    external onlyOwner {
    ...
    external
    {
        require(msg.sender == moduleContract);
    ...
    function _handleFactionMint(
        ...
        uint256 factionId,
        address recipient,
        uint256 factionATotalSupply,
        uint256 factionBTotalSupply
    ) private {
        if (factionId == 0) {
            require(factionAMintCount < factionATotalSupply, "Faction A has been fully minted");
            require((factionAMintCount + tokenCount) <= factionATotalSupply, "Cannot purchase more than supply available");
            factionAMintCount += tokenCount;

            _mint(recipient, tokenCount, FACTION_A_ID);
        } else if (factionId == 1) {
            require(factionBMintCount < factionBTotalSupply, "Faction A has been fully minted");
            require((factionBMintCount + tokenCount) <= factionBTotalSupply, "Cannot purchase more than supply available");
            factionBMintCount += tokenCount;

            _mint(recipient, tokenCount, FACTION_B_ID);
        }
    }
