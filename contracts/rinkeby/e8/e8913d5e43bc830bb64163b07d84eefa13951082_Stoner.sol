...
contract Stoner is ERC721Enumerable, Ownable {
    ...
    uint256 public StonerPrice_whitelist = 80000000000000000;
	uint256 public StonerPrice_public = 80000000000000000;
    uint public maxStonerTx = 5;
	uint public maxStonerPurchase = 10;
	
    uint256 public constant MAX_Stoner = 6666;
    uint public StonerReserve = 150;
	uint public StonerWhitelistReserve = 4000;
	...
	event WhitelistedVIPMint(address minter);
    event MerkleRootUpdated(bytes32 new_merkle_root);
	event MerkleRootVIPUpdated(bytes32 new_merkle_root_vip);
	...
	event wallet5AddressChanged(address _wallet5);
	event wallet6AddressChanged(address _wallet6);
  
    
    string public baseTokenURI;
    
    constructor() ERC721("Stoner", "SAC") { }

    

    modifier onlyAuthorized {
        require(_msgSender() == owner() || _msgSender() == Authorized , "Address not authorized to execute the method");
        _;
    }
    ...
    function reserveStoner(address _to, uint256 _reserveAmount) public onlyAuthorized {        
        uint supply = totalSupply();
        require(_reserveAmount > 0 && _reserveAmount <= StonerReserve, "Not enough reserve");
        for (uint i = 0; i < _reserveAmount; i++) {
            _safeMint(_to, supply + i);
        }
        StonerReserve = StonerReserve.sub(_reserveAmount);
    }
    ...
    function flipPublicSaleState() public onlyAuthorized {
     ...
     function mintStoner(uint numberOfTokens) public payable {
        require(publicSaleIsActive, "Public Sale must be active to mint Stoner");
        require(numberOfTokens > 0 && numberOfTokens <= maxStonerTx, "Can only mint 2 tokens at a time");
        require(msg.value == StonerPrice_public.mul(numberOfTokens), "Ether value sent is not correct");
        require(max_mints_per_address[msg.sender].add(numberOfTokens) <= maxStonerPurchase,"Max 10 mints per wallet allowed");
        
        for(uint i = 0; i < numberOfTokens; i++) {
            if ((totalSupply() < MAX_Stoner) && (totalSupply() < StonerWhitelistReserve)) {
                _safeMint(msg.sender, totalSupply()+1);
                max_mints_per_address[msg.sender] = max_mints_per_address[msg.sender].add(1);
            } else {
               publicSaleIsActive = !publicSaleIsActive;
                payable(msg.sender).transfer(numberOfTokens.sub(i).mul(StonerPrice_public));
                break;
            }
        }
    }
   ...
   // to set the merkle root
    ...
    function updateMerkleRootVIP(bytes32 newmerkleRoot) external onlyAuthorized {
        ...
        emit MerkleRootVIPUpdated(merkleRootVIP);
    }


    function whitelistedMints(uint numberOfTokens, bytes32[] calldata merkleProof ) payable external  {
        address user_ = msg.sender;
		require(whitelistSaleIsActive, "Whitelist Sale must be active to mint Stoner");
        require(numberOfTokens > 0 && numberOfTokens <= 1, "Can only mint 1 tokens at a time");
        require(msg.value == StonerPrice_whitelist.mul(numberOfTokens), "Ether value sent is not correct");
        require(max_mints_per_address[msg.sender].add(numberOfTokens) <= 1,"Max 1 mints allowed per whitelisted wallet");

        // Verify the merkle proof
        require(MerkleProof.verify(merkleProof, merkleRoot,  keccak256(abi.encodePacked(user_))  ), "Invalid proof");
		

            if (totalSupply() < MAX_Stoner) {
                _safeMint(msg.sender, totalSupply()+1);
                max_mints_per_address[msg.sender] = max_mints_per_address[msg.sender].add(1);
            } 
	...
	function whitelistedVIPMints(uint numberOfTokens, bytes32[] calldata merkleProof ) payable external  {
        ...
        require(msg.value == StonerPrice_whitelist.mul(numberOfTokens), "Ether value sent is not correct");
        require(max_mints_per_address[msg.sender].add(numberOfTokens) <= 1,"Max 2 mints allowed per whitelisted wallet");

        // Verify the merkle proof
        require(MerkleProof.verify(merkleProof, merkleRootVIP,  keccak256(abi.encodePacked(user_))  ), "Invalid proof");
    ...
    function setPriceWL(uint256 newPriceWL) public onlyAuthorized {
        StonerPrice_whitelist = newPriceWL;
    }

     function setPrice(uint256 newPrice) public onlyAuthorized {
        StonerPrice_public = newPrice;
    }

   function setMaxStonerTx(uint256 newMaxStonerTx) public onlyAuthorized {
        maxStonerTx = newMaxStonerTx;
    }

function setMaxStonerPurchase(uint256 newMaxStonerPurchase) public onlyAuthorized {
        maxStonerPurchase = newMaxStonerPurchase;
    }
