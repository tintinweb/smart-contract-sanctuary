...
contract Sale is Ownable{
    ...
    function calc(uint256 amount, uint256 royal, uint256 _serviceValue, uint256 _newAdser) internal pure returns(uint256, uint256, uint256, uint256){
        ...
        uint256 tot = _newAdser.div(10);
        (uint256 adminfee, uint256 _newOwnfee) = feeDiv(fee, tot);
        return (adminfee, _newOwnfee, roy, netamount);
    }
    function feeDiv(uint256 amount, uint256 newAdser) internal pure returns(uint256, uint256){
        uint256 newAdminfee = amount.mul(newAdser).div(100);
        uint256 adminfee = amount.sub(newAdminfee);
    ...
    function setnewAdminser(uint256 _serviceValue) internal{
        _newAdminser = _serviceValue;
    }
...
contract ExchangeContract is Sale{
    ...
    function _transferOwnership(address newOwner, address token721, address token1155) public onlyOwner {
        require(newOwner != address(0), "Ownable: new owner is the zero address");
