...
contract AutumnLeaf is ERC721Enumerable, Ownable, ReentrancyGuard{
    ...
    string[4] private backgroundtype = ["solid","aspen pattern","pin cherry pattern","sugar maple pattern"];
    ...
    function traits(Leaf memory leaf) internal view returns(string memory){
        ...
        parts[9] = string(abi.encodePacked(bunches[leaf.bunches],' ',leafnames[leaf.leafstyle]));
    ...
    function getBackgroundPattern(uint256 tokenId) internal view returns(uint256){
        uint256 rand = random(string(abi.encodePacked("PATTERN", toString(tokenId),_msgSender())));
        rand = rand % 201;
        uint256 ps;
    ...
    function getShadowBody(uint256 effect) internal pure returns(string memory){
        ...
        if(effect == 0){
            return string(
                ...
                '<feDropShadow dx="0" dy="4" flood-color="#000000" flood-opacity="0.9" stdDeviation="0"/></filter>'));
        }
        if(effect == 1)
            frequency = '0.02';
        return string(
    ...
    function getRotateAngles() internal view returns(uint256){
        uint256 rand = random(string(abi.encodePacked("EFFECT TYPE",_msgSender())));
        rand = rand % 201;
        uint256 ps;
    ...
    function getSelectedleaves(Leaf memory leaf,uint256 tokenId) internal view returns(string memory){
        ...
        if(leaf.bunches==1){
            selectedLeaf = string(abi.encodePacked(pathstart[4],colors[getColor(tokenId,"ASPENS",0,24)],pathend[4],'</g>'));
        }
        if(leaf.bunches==2){
            selectedLeaf = string(abi.encodePacked(pathstart[5],colors[getColor(tokenId,"PIN_CHERRY",0,24)],pathend[5],'</g>'));
        }
    ...
    function getSVGImage(Leaf memory leaf,uint256 tokenId) internal view returns(string memory){
        ...
        parts[6] = pathend[leaf.leafstyle];
        parts[7] = '</g>';
        parts[8] = getInfoBunchestop(leaf);
        (leaf.bunches > 0 && leaf.bunches < 3)? parts[9] = getAnimationTop(leaf): parts[9] = '';
        parts[10] =getSelectedleaves(leaf,tokenId);
        parts[11]='<defs>';
        parts[12]= getShadowBody(leaf.effect);
        parts[13] = getLeafFilterFill(leaf,tokenId);
        parts[14] = getBackgroundFilter(leaf,tokenId);
        parts[15] = '</defs></svg>';
        ...
        output = string(abi.encodePacked(output,parts[11],parts[12],parts[13],parts[14],parts[15]));
        return output;
    }
