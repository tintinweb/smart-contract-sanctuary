...
contract ColdPotatoes is NFTokenMetadata, Ownable 
{
    // Properties
    address payable feeAddress;
    // STAGING
    uint constant public hotOnePrice = 0.05 ether;
    uint constant public maxhotOnes = 56520;    // Total hot potatoes
    ...
    mapping(address => uint256) public hotOnesClaimed; // Map of how many hot Ones are minted per address.
    ...
    // URI Data
    string private metaAddress = "https://api.easiestwebservers.com/metadata/";
    ...
    // Tells the world how many hot Ones a given address has minted.
    ...
    function mint(uint8 _mintNum) public payable
    {
        ...
        require(_mintNum > 0, "At least one hotpotato is needed");
        require(_mintNum + hotOnesClaimed[msg.sender] <= mintLimit, "Can't mint more than 10 Potatoes per Wallet.");
        require(msg.value >= hotOnePrice * _mintNum, "Potatoes are expensive, need more ETH to afford that.");
        require(current_minted + _mintNum <= maxhotOnes, "Thousand apologies, not enough of those left in stock.");
        if(isWhiteListActive)
        {
            require(whiteList[msg.sender], "You need to be on the whitelist...");
        }
    ...
    {
        require(_mintNum + current_minted <= maxhotOnes, "Cannot mint more the total supply.");
    ...
    // Get how many hot Ones are minted right now.
    function totalSupply() public view returns(uint) 
    {
        return current_minted;
    }

    // Get how many hot Ones MAX can be minted.    
