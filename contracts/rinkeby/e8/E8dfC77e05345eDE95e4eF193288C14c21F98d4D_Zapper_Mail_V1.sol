...
contract Zapper_Mail_V1 is SignatureVerifier, Ownable {
     ...
     * @param _registryAddress The address of new ENS registry to use.
     */
    function updateRegistry(address _registryAddress) public onlyOwner {
        ...
        registry = EnsRegistry(_registryAddress);
    }

    /**
     * @dev Allows to update to new ENS resolver.
     * @param _resolverAddress The address of new ENS resolver to use.
     */
    function updateResolver(address _resolverAddress) public onlyOwner {
        require(
            address(resolver) != _resolverAddress,
            "new resolver should be different from old"
        );
        resolver = EnsResolver(_resolverAddress);
    }
