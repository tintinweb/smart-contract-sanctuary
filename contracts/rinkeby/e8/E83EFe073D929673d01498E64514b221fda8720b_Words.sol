...
contract Words {
    ...
    function getRandomWord(
        ...
        uint256 rand = random(string(abi.encodePacked(block.difficulty, block.timestamp, tokenId.toString(), keyPrefix, seedNum.toString())));
    ...
    function getFiveSyllableLine(uint256 tokenId, uint256 lineNum) external view returns (string memory, uint256, uint256) {
        ...
        uint256 rand = random(string(abi.encodePacked(block.difficulty, block.timestamp, msg.sender, tokenId.toString(), 'FIVE', lineNum.toString()))) % TOTAL_ADJS;
        ...
        uint256 adjSyllables = 5 - numSyllablesLeft;
        uint256 nounSyllables = numSyllablesLeft;
        ...
        return (string(abi.encodePacked(line[0], ' ', line[1])), adjSyllables, nounSyllables);
    }

    function getSevenSyllableLine(uint256 tokenId, uint256 lineNum) external view returns (string memory) {
        string[7] memory line;
        string memory baseSeed = string(abi.encodePacked(block.difficulty, block.timestamp, msg.sender, tokenId.toString(), 'SEVEN', lineNum.toString()));
        uint256 numSyllablesLeft = 7;
        uint256 randVerbIdx = random(string(abi.encodePacked(baseSeed, 'VERB'))) % TOTAL_VERBS;
        uint256 randPrepIdx = random(string(abi.encodePacked(baseSeed, 'PREP'))) % TOTAL_PREPS;
        uint256 randAdjIdx = random(string(abi.encodePacked(baseSeed, 'ADJ'))) % TOTAL_ADJS;

        // verb preposition (adj) noun
        if (randVerbIdx < verb1.length) {
            line[0] = getRandomWord(tokenId, lineNum, "VERB1", verb1);
            numSyllablesLeft--;
        } else if (randVerbIdx < verb1.length + verb2.length) {
            line[0] = getRandomWord(tokenId, lineNum, "VERB2", verb2);
            numSyllablesLeft -= 2;
        } else if (randVerbIdx < verb1.length + verb2.length + verb3.length) {
            line[0] = getRandomWord(tokenId, lineNum, "VERB3", verb3);
            numSyllablesLeft -= 3;
        } else {
            line[0] = getRandomWord(tokenId, lineNum, "VERB4", verb4);
            numSyllablesLeft -= 4;
        }
        line[1] = ' ';

        if (randPrepIdx < preposition1.length) {
            line[2] = getRandomWord(tokenId, lineNum, "PREP1", preposition1);
            numSyllablesLeft--;
        } else {
            line[2] = getRandomWord(tokenId, lineNum, "PREP2", preposition2);
            numSyllablesLeft -= 2;
        }
        line[3] = ' ';
        line[5] = ' ';
        ...
        if (numSyllablesLeft >= 4) {
            if (randAdjIdx < adj1.length) {
                line[4] = getRandomWord(tokenId, lineNum, "ADJ1", adj1);
                numSyllablesLeft--;
            } else if (randAdjIdx < adj1.length + adj2.length) {
                line[4] = getRandomWord(tokenId, lineNum, "ADJ2", adj2);
                numSyllablesLeft -= 2;
            } else if (randAdjIdx < adj1.length + adj2.length + adj3.length) {
                line[4] = getRandomWord(tokenId, lineNum, "ADJ3", adj3);
                numSyllablesLeft -= 3;
            } else {
                if (numSyllablesLeft == 5) {
                    line[4] = getRandomWord(tokenId, lineNum, "ADJ4", adj4);
                    numSyllablesLeft -= 4;
                } else {
                    line[4] = '';
                    line[5] = '';
        ...
        if (numSyllablesLeft == 1) {
            line[6] = oneSyllableNouns.getRandomNoun(tokenId, 1);
        } else if (numSyllablesLeft == 2) {
            line[6] = twoSyllableNouns.getRandomNoun(tokenId, 1);
        } else if (numSyllablesLeft == 3) {
            line[6] = getRandomWord(tokenId, lineNum, "NOUN3", noun3);
        } else {
            line[6] = getRandomWord(tokenId, lineNum, "NOUN4", noun4);
        }

        return string(abi.encodePacked(line[0], line[1], line[2], line[3], line[4], line[5], line[6]));
    }
