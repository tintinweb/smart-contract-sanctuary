...
// hevm: flattened sources of src/Hustler.sol
...
contract HustlerMetadata {
    ...
    function parts(uint256 hustlerId) public view returns (bytes[] memory) {
        bytes[] memory parts_ = new bytes[](12);
...
contract Hustler is ERC1155, ERC1155Receiver, HustlerMetadata, Ownable {
    ...
    function onERC1155Received(
        ...
        require(_msgSender() == address(swapmeet), Errors_1.IsNotSwapMeet);
        ...
        require(sig == equip, Errors_1.EquipSignatureInvalid);
        require(balanceOf(from, hustlerId) == 1, Errors_1.IsHolder);
        require(value == 1, Errors_1.ValueNotOne);
    ...
    function onERC1155BatchReceived(
        ...
        (bytes4 sig, uint256 hustlerId) = abi.decode(data, (bytes4, uint256));
        require(sig == equip, Errors_1.EquipSignatureInvalid);
        require(operator == address(this) || balanceOf(from, hustlerId) == 1, Errors_1.IsHolder);
    ...
    function batchEquip(
        ...
        uint256 j = 0;
        for (uint256 i = 0; i < ids.length; i++) {
            require(values[i] == 1, Errors_1.ValueNotOne);
    ...
    modifier onlyHustler(uint256 id) {
        require(balanceOf(_msgSender(), id) == 1, Errors_1.IsHolder);
        _;
    }
}