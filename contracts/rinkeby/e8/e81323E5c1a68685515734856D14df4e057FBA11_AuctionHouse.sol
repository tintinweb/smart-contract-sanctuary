...
contract AuctionHouse is Ownable {
	...
	event BidPlaced(uint256 auction_id, address bidder, uint256 price);
	event AuctionWon(uint256 auction_id, uint256 highestBid, address winner);
	event OnSale(address nftContract, uint256 itemID, uint256 price, uint256 endTime);
	...
	function putForSale(
		...
		require(price >= 0.01 ether, "price should be >= 0.01 ether");
		require(nftContract != address(0), "tokenContract != address(0)");
		require(durationInSeconds >= 86400, "minimum sale time is 1 day");
		...
		emit OnSale(nftContract, nftID, price, block.timestamp + durationInSeconds);
	}
