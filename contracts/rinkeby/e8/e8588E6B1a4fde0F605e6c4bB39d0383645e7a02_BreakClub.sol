...
contract BreakClub is ERC721, ReentrancyGuard, Ownable {
  ...
  function mint(uint256 count) public payable nonReentrant {
    ...
    require(totalSupply() + count - 1 < MAX_PUBLIC_SUPPLY, "Exceeds max supply");
    ...
    for (uint256 i = 0; i < count; i++) {
      ...
      _safeMint(msg.sender, totalSupply() + totalReservedSupply());
    }
  ...
  function ownerMint(uint256 count, address recipient) external onlyOwner() {
    ...
    for (uint256 i = 0; i < count; i++) {
      ...
      _safeMint(recipient, totalSupply() + totalReservedSupply());
    }
  }
