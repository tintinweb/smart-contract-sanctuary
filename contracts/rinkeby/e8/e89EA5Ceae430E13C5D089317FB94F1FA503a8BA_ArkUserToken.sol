...
contract ERC1155Base is HasSecondarySaleFees, Ownable, ERC1155Metadata_URI, HasContractURI, ERC1155 {
    ...
    function _mint(address user, uint256 _id, Fee[] memory _fees, uint256 _supply, string memory _uri) internal {
        require(membership[user]==0, "User already a member");
        ...
        emit TransferSingle(msg.sender, address(0x0), user, _id, _supply);
        emit URI(_uri, _id);
    }
...
contract ArkToken is Ownable, ERC1155Base {
    ...
    function mint(address user, Fee[] memory fees, uint256 supply, string memory uri) public returns(uint256) {
        id = id + 1;
...
contract ArkUserToken is ArkToken {
    ...
    function mint(address user, Fee[] memory fees, uint256 supply, string memory uri) public returns(uint256) {
        uint256 iid = super.mint(user, fees, supply, uri);
        return iid;
    }
}