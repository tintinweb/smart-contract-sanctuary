...
contract DragonVIP is
    ...
    ERC1155Supply
{
    uint256 public balanceLimit; // max passes per address
    uint256 public tokenLimit; // total available passes
    uint256 public price; // price per dragon
    string public name;
    string public symbol;
    uint256 private constant TOKEN_TYPE = 0; // ERC-1155 supports multiple token types, we only need one
    ...
    constructor(
        ...
        balanceLimit = 15;
        tokenLimit = 1000;
        price = 75000000000000000; // 0.075 ether
        name = _name;
        symbol = _symbol;
    }
    ...
    // Set the price per pass
    function setPrice(uint256 _price) external onlyOwner {
        price = _price;
    }

    // Set the total available passes
    ...
    function mint(address to, uint256 qty) external payable {
        require((qty + totalSupply(TOKEN_TYPE)) <= tokenLimit, "sold out");
        require(
            (qty + balanceOf(to, TOKEN_TYPE)) <= balanceLimit,
            "exceeded balance limit"
        );
        require(msg.value >= (price * qty), "insufficient funds");
        _mint(to, TOKEN_TYPE, qty, "");
    }
