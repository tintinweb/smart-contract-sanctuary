...
library ChickenNoodleSoupGeneratorLibrary {
    ...
    function selectTrait(
        uint16 seed,
        uint8 traitType,
        uint8[][10] storage rarities,
        uint8[][10] storage aliases
    ...
    function selectTraits(
        ...
        seed >>= 16;
        t.pupils = selectTrait(uint16(seed & 0xFFFF), 2, rarities, aliases);

        seed >>= 16;
        t.hats = selectTrait(uint16(seed & 0xFFFF), 3, rarities, aliases);

        seed >>= 16;
        t.bodyAccessories = t.isChicken
            ? 0
            : selectTrait(uint16(seed & 0xFFFF), 4, rarities, aliases);

        seed >>= 16;
        uint8 tier = selectTrait(uint16(seed & 0xFFFF), 5, rarities, aliases);
