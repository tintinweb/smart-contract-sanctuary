...
import "./types/NiftyAccessControlled.sol";

contract Distributor is IDistributor, NiftyAccessControlled {
    /* ========== DEPENDENCIES ========== */
    ...
    IERC20 private immutable nifty;
    ...
    constructor(
        ...
        address _staking,
        address _authority
    ) NiftyAccessControlled(INiftyAuthority(_authority)) {
        require(_treasury != address(0), "Zero address: Treasury");
        treasury = ITreasury(_treasury);
        require(_nifty != address(0), "Zero address: NIFTY");
        nifty = IERC20(_nifty);
    ...
    function nextRewardAt(uint256 _rate) public view override returns (uint256) {
        return nifty.totalSupply().mul(_rate).div(rateDenominator);
    }
...
import "../interfaces/INiftyAuthority.sol";

abstract contract NiftyAccessControlled {
    /* ========== EVENTS ========== */

    event AuthorityUpdated(INiftyAuthority indexed authority);

    string UNAUTHORIZED = "UNAUTHORIZED"; // save gas

    /* ========== STATE VARIABLES ========== */

    INiftyAuthority public authority;

    /* ========== Constructor ========== */

    constructor(INiftyAuthority _authority) {
    ...
    function setAuthority(INiftyAuthority _newAuthority) external onlyGovernor {
