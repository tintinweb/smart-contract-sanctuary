...
contract FoxGames_v1_2 is IFoxGame, OwnableUpgradeable, IERC721ReceiverUpgradeable,
  ...
  // Date when corruption begins to spread...
  uint48 private corruptionStartDate;

  // The last time $CROWN was claimed
  uint48 public lastCrownClaimTimestamp;

  // End date after which carrot will no long accrue
  uint48 public carrotFaucetEndTimestamp;
  ...
  uint48 private phase2LaunchTimestamp;
   ...
   */
  function setPhase2LaunchTimestamp(uint48 timestamp) external onlyOwner {
    phase2LaunchTimestamp = timestamp;
  }
   ...
   */
  function getSigningHash(address recipient, uint16[] calldata tokenIds, bool unstake, uint32[] calldata blocknums, uint256[] calldata seeds) public pure returns (bytes32) {
    return keccak256(abi.encodePacked(recipient, tokenIds, unstake, blocknums, seeds));
  }
  function getSigningHash(address recipient, uint8 token, uint32 blocknum, uint256 seed) public pure returns (bytes32) {
    return keccak256(abi.encodePacked(recipient, token, blocknum, seed));
  }
  function isValidSignature(address recipient, uint8 token, uint32 blocknum, uint256 seed, bytes memory sig) public view returns (bool) {
    bytes32 message = getSigningHash(recipient, token, blocknum, seed).toEthSignedMessageHash();
    return ECDSAUpgradeable.recover(message, sig) == signVerifier;
  }
  function isValidSignature(address recipient, uint16[] calldata tokenIds, bool unstake, uint32[] calldata blocknums, uint256[] calldata seeds, bytes memory sig) public view returns (bool) {
    bytes32 message = getSigningHash(recipient, tokenIds, unstake, blocknums, seeds).toEthSignedMessageHash();
    return ECDSAUpgradeable.recover(message, sig) == signVerifier;
  }
   ...
   */
  function claimRewardsAndUnstake(bool unstake, uint16[] calldata tokenIds, uint32[] calldata blocknums, uint256[] calldata seeds,  bytes calldata sig) external  {
    require(tx.origin == msg.sender, "eos");
    require(isValidSignature(msg.sender, tokenIds, unstake, blocknums, seeds, sig), "invalid signature");
  ...
  function _claimRabbitsFromKeep(IFoxGameNFT nftContract, uint16 tokenId, bool unstake, uint256 seed, bool elevatedRisk) internal returns (uint128 reward) {
    ...
    uint48 stakeStart = stake.time < phase2LaunchTimestamp ? phase2LaunchTimestamp : stake.time; // phase 2 reset
    require(!(unstake && block.timestamp - stakeStart < RABBIT_MINIMUM_TO_EXIT), "needs 2 days of crown");
  ...
  function _claimHunterFromCabin(IFoxGameNFT nftContract, uint16 tokenId, bool unstake) internal returns (uint128 reward) {
    ...
    uint48 stakeStart = timeStake.time < phase2LaunchTimestamp ? phase2LaunchTimestamp : timeStake.time; // phase 2 reset
    if (totalCrownEarned < MAXIMUM_GLOBAL_CROWN) {
   ...
   */
  function _getCarrotForRabbit(uint16 tokenId, uint48 time) internal view returns (TokenReward memory claim) {
    // Carrot time-based rewards
    uint128 reward;
    TimeStake storage stake = rabbitStakeByToken[tokenId];
    if (carrotFaucetEndTimestamp == 0 || time < carrotFaucetEndTimestamp) {
      reward = (time - stake.time) * RABBIT_EARNING_RATE;
    } else if (stake.time < carrotFaucetEndTimestamp) {
      reward = (carrotFaucetEndTimestamp - stake.time) * RABBIT_EARNING_RATE;
    }
   ...
   * @return claim carrot claim object
   */
  function _getCarrotForFox(IFoxGameNFT nftContract, uint16 tokenId, uint48 time) internal view returns (TokenReward memory claim) {
    uint8 cunning = _getAdvantagePoints(nftContract, tokenId);
    EarningStake storage stake = foxStakeByCunning[cunning][foxHierarchy[tokenId]];
   ...
   * @return claim carrot claim object
   */
  function _getCarrotForHunter(IFoxGameNFT nftContract, uint16 tokenId, uint48 time) internal view returns (TokenReward memory claim) {
    require(foxNFTGen1.ownerOf(tokenId) == address(this), "not staked");
    uint8 marksman = _getAdvantagePoints(nftContract, tokenId);
    EarningStake storage earningStake = hunterStakeByMarksman[marksman][hunterHierarchy[tokenId]];
    require(earningStake.owner == msg.sender, "not owner");
    ...
    } else if (timeStake.time < carrotFaucetEndTimestamp) {
      reward += (carrotFaucetEndTimestamp - timeStake.time) * HUNTER_EARNING_RATE;
    }
  ...
  /**
   * Set end date for carrot accrual
   * @param timestamp Date to end
   */
  function setCarrotFaucetEndTimestamp(uint48 timestamp) external onlyOwner {
    carrotFaucetEndTimestamp = timestamp;
  }
...
interface IFoxGame {
  ...
  function isValidSignature(address, uint8, uint32, uint256, bytes memory) external view returns (bool);
  function isValidSignature(address, uint16[] calldata, bool, uint32[] calldata, uint256[] calldata, bytes memory) external view returns (bool);
