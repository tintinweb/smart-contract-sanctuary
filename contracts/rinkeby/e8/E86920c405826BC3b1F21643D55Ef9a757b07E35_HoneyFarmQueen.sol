...
contract HoneyFarmQueen is Ownable, ReentrancyGuard {
    //this is 1
    uint8 public constant HONEY_BEARS_REWARDS_PER_ROUND = 100;
    //this is 0.13
    uint8 public constant HONEY_UNSTAKED_BEE_REWARDS_PER_ROUND = 13;
    //this is 0.09
    uint8 public constant HONEY_STAKED_BEE_REWARDS_PER_ROUND = 9;

    uint8 public constant MIN_USAGE_PER_HIVE = 3;

    uint8 public constant REWARD_FOR_BURNING_BEE = 12;
    ...
    function claimBearsHoney(uint16[] calldata _bearsIds) external nonReentrant {
        ...
        if (honey.totalSupply() + amount > honey.maxSupply()) amount = (honey.maxSupply() - honey.totalSupply()) / 1e18;

        honey.mint(msg.sender, (amount * 1e18) / 100);
    ...
    function claimBeesHoney(uint16[] calldata _beesIds) external nonReentrant {
        ...
        if (honey.totalSupply() + amount > honey.maxSupply()) {
            amount = (honey.maxSupply() - honey.totalSupply()) / 1e18;
        }
    ...
    function mintBee(uint16 _hiveId) external nonReentrant {
        ...
        require(honey.balanceOf(msg.sender) >= MIN_BURN_AMOUNT_FOR_BEE * 1e18, "Not enough Honey");
        ...
        idsAndBees[beeId] = Bee(beeId, 1, 10, 0, STARTING_POINT, 0, 0);

        honey.burn(msg.sender, MIN_BURN_AMOUNT_FOR_BEE * 1e18);
    ...
    function activateHive(uint16 _hiveId) external nonReentrant {
        ...
        require(honey.balanceOf(msg.sender) >= MIN_AMOUNT_FOR_ACTIVATE_HIVE * 1e18, "Not enough Honey");

        honey.burn(msg.sender, MIN_AMOUNT_FOR_ACTIVATE_HIVE * 1e18);
    ...
    function activateBees(uint16[] calldata _beesIds) external nonReentrant {
        ...
        amountOfHoney = amountOfHoney * 1e18;
    ...
    function keepBeesActive(uint16[] calldata _beesIds) external nonReentrant {
        ...
        amountOfHoney = (amountOfHoney * 1e18) / 100;
        require(honey.balanceOf(msg.sender) >= amountOfHoney, "Not enough honey");
        honey.burn(msg.sender, amountOfHoney);
    }
    ...
    function stakeBees(uint16[] calldata _beesIds) external {
        uint256 amountOfHoney = 0;
        for (uint16 i = 0; i < _beesIds.length; i++) {
            uint16 _beeId = _beesIds[i];
            Bee storage bee = idsAndBees[_beeId];
    ...
    function unstakeBees(uint16[] calldata _beesIds) external {
        for (uint16 i = 0; i < _beesIds.length; i++) {
            uint16 _beeId = _beesIds[i];
            Bee storage bee = idsAndBees[_beeId];
    ...
    function randBeeId() internal returns (uint16 _id) {
        uint16 entropy;
        uint16 MAX_SUPPLY = bees.getMaxSupply();
        uint16 TOTAL_SUPPLY = uint16(bees.totalSupply());
