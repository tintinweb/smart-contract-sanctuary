...
import './utils/DoubleOperator.sol';
...
contract TEST is ERC20Burnable, DoubleOperator {
    using SafeMath for uint256;

    /* ========== STATE VARIABLES ========== */

    //fees
    uint256 public feesTaxRate = 5;
    ...
    // Check the mapping for excluded addresses with fees
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        bool _recipientExcludedAddresses = isRecipientExcludedAddresses(recipient);

        if (_senderExcludedAddresses == true) {  
            _transfer(sender, recipient, amount);
        } else if (_recipientExcludedAddresses == true) {
            _transfer(sender, recipient, amount);
        }
        else {
        ...
        _approve(sender, _msgSender(), allowance(sender, _msgSender()).sub(amount, "Brrr, ERC20: transfer amount exceeds allowance"));
        return true;
    }    
...
import './DoubleOwnable.sol';

abstract contract DoubleOperator is Context, DoubleOwnable {
