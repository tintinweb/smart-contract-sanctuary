...
contract MeowNFT is ERC721Enumerable, Ownable {

    using SafeMath for uint256;
    using Strings for uint256;

    string public baseTokenURI;
    uint256 public presalePrice = 25 * 10**6;
    uint256 public publicPrice = 35 * 10**6;
    uint256 public saleState = 0; // 0 = paused, 1 = presale, 2 = live
    uint256 public constant MAX_SUPPLY = 10;
    uint256 public offsetIndex;
    uint public offsetIndexBlock;
    uint256 public revealTimeStamp = block.timestamp + 14 days;
    IERC20 public usdc;
    ...
    constructor(IERC20 _usdc, string memory baseURI, string memory preRevealURI) ERC721("Meow", "MEOW")  {
        usdc = _usdc;
        setBaseURI(baseURI);
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (totalSupply() >= MAX_SUPPLY || block.timestamp >= revealTimeStamp || offsetIndexBlock > 0) {
    ...
    function adopt(uint256 num) public {
        uint256 supply = totalSupply();
        uint256 price = saleState == 1 ? presalePrice : publicPrice;
        require( saleState >= 1,             "Sale not live" );
        require( num <= 10,                  "You can mint a maximum of 10 fantastic meow" );
        require( supply + num <= MAX_SUPPLY,  "Exceeds maximum Meow supply" );
        require( usdc.transferFrom(msg.sender, address(this), price * num),  "USDC sent is not correct" );
