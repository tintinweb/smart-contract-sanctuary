...
// File: localhost/eth/node_modules/@openzeppelin/contracts/introspection/IERC165.sol
...
contract InscriptionMetadata {
     ...
     * @param _baseURI      The tokenID of the NFT
     */
    function addInscriptionURI(uint256 _uriID, string memory _baseURI) public {
        ...
        require(bytes(uri.baseURI).length == 0, "Base URI already exists");
    ...
    function removeInscriptionURI(uint256 _uriID) public {
        ...
        require(bytes(uri.baseURI).length != 0, "Inscription URL does not exist");
...
contract Inscribe is SignatureHelper, InscriptionMetadata {

    constructor () {}
    
    // Events emitted when modifying inscription 
    event InscriptionAdded(address _nftAddress, uint256 _tokenID, address _inscriber);
    event InscriptionRemoved(address _nftAddress, uint256 _tokenID, address _inscriber);
    ...
    // Keeps track of a NFT's inscription
    mapping (address => mapping (uint256 => mapping(address => Inscription))) inscriptionMapping;
    ...
    /** 
     * Verifies the address of the inscription signed the NFT
     * @param _nftAddress  The address of the NFT contract
     * @param _tokenID      The tokenID of the NFT
     * @param _address      The address to check
     */
    function verifyInscription(address _nftAddress, uint256 _tokenID, address _address) external view returns (bool) {
        return inscriptionMapping[_nftAddress][_tokenID][_address].isSigned;
    }
    ...
    function addInscription(address _nftAddress, uint256 _tokenID, address _inscriber, uint256 _nonce, uint256 _uriID, bytes memory _sig) external {
        ...
        require(inscriptionMapping[_nftAddress][_tokenID][_inscriber].isSigned == false, "NFT already has an inscription by this address");
        ...
        inscriptionMapping[_nftAddress][_tokenID][_inscriber] = Inscription(true, _uriID);
        emit InscriptionAdded(_nftAddress, _tokenID, _inscriber);
    }
     ...
     * @param _tokenID      The tokenID of the NFT that is being signed
     */
    function removeInscription(address _nftAddress, uint256 _tokenID, address _inscriber) external {
        IERC721 nftContractInterface = IERC721(_nftAddress);
        require(nftContractInterface.ownerOf(_tokenID) == msg.sender, "Check that the address calling this function owns the specified NFT");
        
        // Remove Inscription
        delete inscriptionMapping[_nftAddress][_tokenID][_inscriber];
        emit InscriptionRemoved(_nftAddress, _tokenID, _inscriber);
    }
    
    // Inscription JSON stored in a custom URI
    // JSON standard follow the specifications in order to be considered valid
    function inscriptionURI(uint256 _tokenID, address _nftAddress, address _inscriber) external view returns (string memory _inscriptionURI) {
        Inscription memory inscription = inscriptionMapping[_nftAddress][_tokenID][_inscriber];
        require(inscription.isSigned == true, "Inscriber did not sign the following token");
        
        BaseInscriptionURI memory uri = inscriptionURIMapping[inscription.uriID];
        require(bytes(uri.baseURI).length != 0, "Inscription Base URL does not exist");
        
        string memory parameters = string(abi.encodePacked(_nftAddress, string(abi.encodePacked("/", _tokenID))));
