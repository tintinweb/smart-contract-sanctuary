...
//   ======================================
    ...
    function increaseLiquidity(uint256 numTokensToLiquidity) internal nonReentrant {
        ...
        uint256 _deficitTokens = ( _tokensDesired > liquidityAmount) ? (_tokensDesired - liquidityAmount) : 0;

        if (_deficitTokens != 0 && Wallet[regulatoryFunds].balance >= _deficitTokens) {
           Wallet[regulatoryFunds].balance -= _deficitTokens;
           Wallet[address(this)].balance   += _deficitTokens;
           liquidityAmount                 += _deficitTokens;
        }
...
//   ======================================
    ...
    function _transfer( address sender, address recipient, uint256 amount ) internal override whenNotPaused {
        ...
        if (autoLiquidity && !inLiquidityProcess  && sender != liquidityPair && 
            Wallet[address(this)].balance >= _liquidityThreshold) { 
        ...
        if (isAdjustable && !inLiquidityProcess) {_afterTokenTransfer(sender, recipient, amount);}
...
//   ==========================================
    ...
    function _getDeviation (uint256 _tokenPrice) internal view returns (bool isNeedAttenuation, uint256 buyingStack, uint256 sellingStack) {  
        uint256 _buyingStack  = _tradingTrack.buyingStack;
        uint256 _sellingStack = _tradingTrack.sellingStack;

        if (_tradingTrack.lastTokenPrice != _tokenPrice)  {
            if      (_tradingTrack.lastTxnType == 1) {_buyingStack  += _tradingTrack.lastTxnValue; }
            else if (_tradingTrack.lastTxnType == 2) {_sellingStack += _tradingTrack.lastTxnValue; }
        }

        if (_buyingStack  >= _sellingStack) {
            _buyingStack  -= _sellingStack;
            _sellingStack  = 0;  }
        else {
            _sellingStack -= _buyingStack;
            _buyingStack   = 0;  }

       return ( _buyingStack > (Wallet[liquidityPair].balance  / 10), _buyingStack, _sellingStack); 
    }
//   ======================================
...
//   ======================================
    ...
    function attenuateImpulse() external onlyRole(Treasury_Analyst) {
        uint256 _newTokenPrice = getTokenPrice();
        (bool needAttenuation, uint256 numTokensToLiquidity, ) = _getDeviation(_newTokenPrice);
        require (needAttenuation, "Adjust not allowed at the moment");
    ...
    function deviationLevel () external view returns (bool needAttenuation, uint256 buyingStack, uint256 sellingStack) {
        uint256 _newTokenPrice = getTokenPrice();
        (needAttenuation, buyingStack, sellingStack) = _getDeviation(_newTokenPrice);
    }
//   ======================================
...
contract SymplexiaToken is  Taxable {
   ...
   // swapRouterAddress
    ...
    constructor ()  ERC20("BETA60", "BTG60") BaseToken ( 
