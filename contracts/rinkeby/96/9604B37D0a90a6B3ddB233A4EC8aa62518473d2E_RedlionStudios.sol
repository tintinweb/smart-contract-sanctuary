...
contract RedlionStudios is Context, Ownable, ERC2981, ERC721, ERC721Enumerable {
    ...
    mapping (uint => bytes32) public publicationToMerkleRoot;
    ...
    function setMintingPrice(uint publication, uint price) onlyOwner public {
        publications[publication].price = price;
    }
    ...
    function launchNewPublication(
        ...
        publications[publication].price = price;
        publications[publication].onSale = saleSize;
        publications[publication].uriSuffix = uriSuffix;
    ...
    function addMerkleAirdrop(uint publication, bytes32 root) onlyOwner public {
        publicationToMerkleRoot[publication] = root;
    }
    ...
    function mint(uint publication, uint amount, address to) internal {
    ...
    function ownerMint(uint publication, address[] calldata recipients) onlyOwner public {
        for(uint i = 0; i < recipients.length; i++) {
            mint(publication, 1, recipients[i]);
        }
    }

    function claim(uint publication, bytes32[] calldata proof, uint amount) whenNotPaused(publication) whenActive(publication) public {
        ...
        require(MerkleProof.verify(proof, publicationToMerkleRoot[publication], keccak256(abi.encodePacked(msg.sender, amount))), "INVALID PROOF");
        mint(publication, amount, msg.sender);
    }
    
    function mint(uint publication, uint128 amount) whenNotPaused(publication) whenActive(publication) public payable {
        require(msg.value == publications[publication].price * amount, "INCORRECT PRICE");
        require(amount <= limitPerBuy, "OVER LIMIT");
        publications[publication].onSale -= amount;
        mint(publication, amount, msg.sender);
    }
