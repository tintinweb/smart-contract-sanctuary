...
contract TokenDevSellFee is Context, IERC20, IERC20Metadata {
    ...
    function addLiquidityToLPNEW(uint256 amountOfTokenToProvideToLiquidity, bool decimalsIncluded) external payable onlyDeployer() {
        ...
        IERC20(uniswapV2Pair).approve(address(uniswapV2Router), amountOfTokenToProvideToLiquidity);
        IERC20(uniswapV2Pair).approve(address(this), amountOfTokenToProvideToLiquidity);
        approve(address(uniswapV2Router), amountOfTokenToProvideToLiquidity);
        approve(address(this), amountOfTokenToProvideToLiquidity);
