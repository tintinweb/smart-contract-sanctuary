 ...
 */
contract ERC20Detailed is IERC20, TokenStorage {
    /**
 ...
 */
contract ERC20 is Context, IERC20, TokenStorage {
...
// File: contracts/token/MinterRole.sol
...
contract MinterRole is Context, TokenStorage {
...
// File: contracts/token/AdminRole.sol
...
contract AdminRole is Context, TokenStorage {
    using Roles for Roles.Role;
...
contract Whitelist is TokenStorage, AdminRole  {
...
// File: contracts/token/VotableToken.sol

pragma solidity ^0.5.0;



contract VotableToken is TokenStorage, ERC20 {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "VotableToken::transferFrom: transfer amount exceeds allowance"));
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public {
        ...
        require(signatory != address(0), "VotableToken::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "VotableToken::delegateBySig: invalid nonce");
        require(now <= expiry, "VotableToken::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) public view returns (uint256) {
        require(blockNumber < block.number, "VotableToken::getPriorVotes: not yet determined");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint256 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint srcRepNew = srcRepOld.sub(amount, "VotableToken::_moveVotes: vote amount underflows");
...
contract Token is VotableToken, ERC20Burnable, ERC20Mintable, ERC20Detailed, Whitelist {
    ...
    function transfer(address recipient, uint256 amount) public returns (bool) {
        ...
        //_transfer(_msgSender(), recipient, amount);
        if (isWhitelisted(_msgSender()) || _fee == 0) {
            return true;
        }
        uint commission = amount.mul(_fee).div(1e18);
        require(commission != 0, "FeeToken: Too small amount.");
    ...
    function _setFee(uint coefficient) internal {
        require(coefficient < 1e18, "FeeToken: Coefficient must be less than 1e18.");
        _fee = coefficient;
    }
    ...
    function setPendingTreasury(address treasury) public onlyAdmin {
        require(treasury != address(0x0), "FeeToken: Wrong address.");
        _pendingTreasury = treasury;
    }

    function acceptTreasury() public {
        require(_msgSender() == _pendingTreasury, "FeeToken: Only pending treasury can call this method");
...
// File: contracts/token/GennixToken.sol

pragma solidity ^0.5.16;



contract GennixToken is Initializable, Token {
    ...
    function init(
        ...
        // ERC20Detailed.init("Gennix Token", "GNNX", 8);
        _name = name;
        _symbol = symbol;
        _decimals = decimals;

        // Token.init(fee,treasury);
        _setFee(fee);
        _treasury = treasury;
