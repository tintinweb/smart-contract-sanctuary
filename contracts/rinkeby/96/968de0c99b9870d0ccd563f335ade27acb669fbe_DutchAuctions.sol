...
contract DutchAuctions is OwnableClone, RevenueSplit {
  ...
  ListingTemplate[] internal templates;
  ...
  function calculateCurrentPrice(uint256 listingId) public view returns (uint256) {
    ...
    ListingTemplate storage template = templates[listing.template];
  ...
  function addListingTemplates( ListingTemplate[] memory newTemplates ) public onlyOwner {
    for (uint idx = 0; idx < newTemplates.length; idx++) {
      templates.push(newTemplates[idx]);
    }
  }
