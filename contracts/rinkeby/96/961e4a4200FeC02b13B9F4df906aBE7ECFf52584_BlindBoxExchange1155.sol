...
contract BlindBoxExchange1155 is AccessControl, Pausable {
    ...
    address internal createdByAddress;
    ...
    function setTokenIdRange(uint256 _minId,uint256 _maxId)public  whenNotPaused{
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
        nowId = _minId;
        maxId = _maxId;
    }
    ...
    function setCreatedByAddress(address _createdByAddress)whenNotPaused public{
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
        createdByAddress = _createdByAddress;
    }

    function _mintNft(uint256[] memory mintNftTokenIds)internal whenNotPaused returns(bool) {

        uint256[] memory amountArray;
        address _createdByAddress;
        if (createdByAddress == address(0)){
            _createdByAddress = msg.sender;
        }else{
            _createdByAddress = createdByAddress;
        }
        Common1155NFT Common1155NFTContract = Common1155NFT(nftContractAddress);
        if (mintNftTokenIds.length == 1){
            Common1155NFTContract.mint(msg.sender,mintNftTokenIds[0],1,abi.encode(address(_createdByAddress)));
        }else{
            amountArray = _generateAmountArray(mintNftTokenIds.length);
            Common1155NFTContract.mintBatch(msg.sender,mintNftTokenIds,amountArray,abi.encode(address(_createdByAddress)));
        }
        return true;
    }
