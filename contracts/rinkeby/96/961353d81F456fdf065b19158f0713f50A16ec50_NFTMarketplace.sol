...
contract NFTFactory is ERC721, AccessControl, Ownable {
    ...
    string public uri; // = "https://gateway.pinata.cloud/ipfs/QmVWqWrWSW7EWHkpYnWsUf56qVEdJfyv3QH29A8BRnB67n/";
    uint256 public nextTokenId = 1;

    event NFTminted(uint256 indexed tokenId, address indexed newOwner);

    constructor(
        string memory _uri, uint256 _maxSupply, string memory _name, string memory _symbol
    ...
    function safeMint(address _to, uint256 _quantity) public {
        ...
        require(nextTokenId + _quantity <= maxSupply, "Maximum supply exceeded");
        for (uint256 i = nextTokenId; i <= nextTokenId + _quantity; i++) {
            _safeMint(_to, i);
            emit NFTminted(i, _to);
            minted[i] = true;
        }
...
contract NFTMarketplace is IERC721Receiver, Ownable {
    ...
    /// @notice Constructor
    /// @param _tokenAddress Token currency address
    ...
    /// @notice Mints NFT to sender
    /// @param _to address of sender
    // function mintTo(address _to) internal {
    //     require(remaining() > 0, "No supply left");
    //     uint256 _newTokenId = _getNextTokenId();
    //     nft.safeMint(_to, _newTokenId);
    //     _incrementTokenId();
    //     minted += 1;
    //     emit NFTSold(_newTokenId, _to);
    // }

    /// @notice Computes next token ID from _currentTokenId
    // function _getNextTokenId() private view returns (uint256) {
    //     return _currentTokenId.add(1);
    // }

    /// @notice Increments value of _currentTokenId
    // function _incrementTokenId() private {
    //     _currentTokenId++;
    // }

    /// @notice Returns remaining NFTs
    // function remaining() public view returns (uint256 _remaining) {
    //     return nft.maxSupply() - minted;
    // }

    /// @notice Buy an NFT
    /// @param _quantity Quantity of NFTs
    function buy(uint256 _quantity) public {
        ...
        require(_quantity <= nft.maxSupply() - nft.nextTokenId(), "Quantity exceeds remaining");
        require(_quantity <= maxQtyPerPurchase, "Quantity exceeds limit");

        uint256 _total = price * _quantity;

        // Transfer tokens
        token.transferFrom(msg.sender, mktWallet, _total);

        // Mint NFTs
        nft.safeMint(msg.sender, _quantity);
