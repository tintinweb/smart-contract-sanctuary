...
contract RollerInu is Context, IERC20, Ownable {
    ...
    string private _name = "Zenoverse";
    string private _symbol = "ZEV";
    ...
    uint private _cooldownDuration = 15 seconds;

    uint256 public maxTxAmount = 1000000000 * 10**_decimals; // 1% of total supply
    uint256 public numTokensToSwap = 100000000 * 10**_decimals; // 0.1% of total supply
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair &&
            ...
            cooldown[to] = block.timestamp + (_cooldownDuration);
        }
		require(
    ...
    function setCooldownEnabled(bool onoff) external onlyOperator {
        cooldownEnabled = onoff;
    }

    function setCooldownDuration(uint sec) external onlyOperator {
        _cooldownDuration = sec;
    }

    function setOpenTrading(bool onoff) external onlyOperator {
        _tradingOpen= onoff;
    }
}