...
contract TempleStaking is Ownable, ITemplePool {
    ...
    struct User {
        // temple + rewards accounted for in staking contract as an as an ABDKMath64x64
        // value is captured as an integral and fractional component (that is, core
        // token in attos / 1e18)
        int128 Balance;
    ...
    /** User's balance as an ABDKMath64x64 (includes pending rewards) */
    function _balance(address _staker, int128 _accumulationFactor) private view returns(int128) {
        User storage user = userDetails[_staker];
        return user.Balance.mul(_accumulationFactor).div(user.AccumulationFactorAtStake);
    }

    /** returns User's balance (includes pending rewards) */
    function balance(address _staker) public view returns(uint256) {
        int128 _pendingBalance = _balance(_staker, _currentAccumulationFactor(currentEpoch()));
        uint256 _integralDigits = _pendingBalance.toUInt();
        uint256 _fractionalDigits = _pendingBalance.sub(ABDKMath64x64.fromUInt(_integralDigits)).mul(ABDKMath64x64.fromUInt(1e18)).toUInt();

        return (_integralDigits * 1e18) + _fractionalDigits;
    }

    /** updates rewards in pool */
    function updateAccumulationFactor() public {
    ...
    function _stake(address _staker, uint256 _amount) internal {
        ...
        if (user.AccumulationFactorAtStake != 0) {
            user.Balance = user.Balance.mul(accumulationFactor).div(user.AccumulationFactorAtStake);
        }

        user.Balance = user.Balance.add(ABDKMath64x64.divu(_amount, 1e18));
    ...
    function unstake(uint256 _amount) external {        
        ...
        require(user.Balance > 0, "User has no stake");

        // Ensure any accumulated rewards are updated as part of the user's balance
        _stake(msg.sender, 0);
        uint256 _maxUnstakeable = balance(msg.sender);
        ...
        if (_amount > 0) {
            user.Balance = user.Balance.sub(ABDKMath64x64.divu(_amount, 1e18));
    ...
    /**
     * Unstake a users complete balance
     */
