...
contract Rebels is ERC721, Ownable {
    ...
    // Number of tokens reserved for the owner but not yet minted
    uint public reservedTotal;
    ...
    // True of the token sale is enabled for all addresses
     ...
     * @param maxReserved Number of tokens reserved for the owner
     *
     * Requirements:
     * - Reserved token amount must be equal or smaller than the total supply
     */
    constructor(uint price_, uint maxSupply, uint maxReserved) ERC721("Rebels", "RBLS") {
        require(maxReserved <= maxSupply, "Can't reserve more than the total supply");
        MAX_SUPPLY = maxSupply;
        reservedTotal = maxReserved;
        price = price_;
    }

    /**
     * @dev Throws if the given number of tokens can't be sold
     * @param number Requested number of tokens
     *
     * Requirements:
     * - Minting the given number of tokens shouldn't exceed the remaining supply
    ...
    modifier canBuy(uint number) {
        require(number <= remainingSupply(), "Not enough tokens left");
     ...
     *
     * Requirements:
     * - There should be enough reserved tokens to mint the requested number
     */
    function reservedMint(uint number) external onlyOwner {
        require(number <= reservedTotal, "Not enough reserved tokens left");
     ...
     *
     * Requirements:
     * - Hash can only be set (or changed) before the first token sale
     */
    function setProvenanceHash(string memory hash) external onlyOwner {
        require(soldTotal == 0, "Hash can only be set before first sale");
        provenanceHash = hash;
    }
    ...
    /**
     * @dev Returns the number of tokens than can still be minted in a public or a whitelisted sale
     * @return Number of available tokens
     */
    function remainingSupply() public view returns (uint256) {
        return MAX_SUPPLY - soldTotal - reservedTotal;
    }
}
