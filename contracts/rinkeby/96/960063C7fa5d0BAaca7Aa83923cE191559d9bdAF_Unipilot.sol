...
contract Unipilot is IUnipilot, ERC721Permit, PeripheryPayments {
    ...
    mapping(address => bool) public exchangeManagerWhitelist;

    /// @param initialExchangeManager uniswap liquidity manager contract address
    constructor(address initialExchangeManager)
        ERC721Permit("Unipilot Positions NFT-V1", "PILOT-POS-V1", "1")
    {
        governance = msg.sender;
        exchangeManagerWhitelist[initialExchangeManager] = true;

        emit ExchangeWhitelisted(initialExchangeManager);
    }
    ...
    function setExchangeAddress(address exchangeAddress) external onlyGovernance {
        ...
        exchangeManagerWhitelist[exchangeAddress] = true;
        emit ExchangeWhitelisted(exchangeAddress);
    }

    /// @dev Blacklist/Whitelist exchange managers
    function setExchangeStatus(address exchangeAddress) external onlyGovernance {
        exchangeManagerWhitelist[exchangeAddress] = !exchangeManagerWhitelist[
            exchangeAddress
        ];
        emit ExchangeStatus(exchangeAddress, exchangeManagerWhitelist[exchangeAddress]);
    }
    ...
    function createPoolAndDeposit(
        ...
        payable
        override
        isExchangeManagerWhitelist(params.exchangeAddress)
        returns (
    ...
    {
        IExchangeManager(params.exchangeAddress).createPair(
            params.token0,
            params.token1,
            data[0]
        );
        ...
        (amount0Added, amount1Added, mintedTokenId) = deposit(
    ...
    function withdraw(IExchangeManager.WithdrawParams calldata params, bytes memory data)
        ...
        isExchangeManagerWhitelist(params.exchangeAddress)
    {
        IExchangeManager(params.exchangeAddress).withdraw(
            params.pilotToken,
            params.wethToken,
            params.liquidity,
            params.tokenId,
    ...
    function collect(IExchangeManager.CollectParams calldata params, bytes memory data)
        ...
        isExchangeManagerWhitelist(params.exchangeAddress)
    {
        IExchangeManager(params.exchangeAddress).collect(
            params.pilotToken,
            params.wethToken,
            params.tokenId,
            data
        ...
        )
    {
        ...
        ) = IExchangeManager(params.exchangeAddress).getReserves(
            params.token0,
            params.token1,
            data
        );
        ...
        (amount0Added, amount1Added, mintedTokenId) = IExchangeManager(
