...
contract WizardStorage {
    ...
    function encode( uint16 t0, uint16 t1, uint16 t2, uint16 t3, uint16 t4, uint16 t5) internal view returns(bytes memory) {
    ...
    function decode(bytes memory data) internal view returns(uint16 t0, uint16 t1, uint16 t2, uint16 t3, uint16 t4, uint16 t5) {
        assembly {
            let len := mload(
                add(
                    data,
