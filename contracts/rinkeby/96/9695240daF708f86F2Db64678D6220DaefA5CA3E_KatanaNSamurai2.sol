...
contract KatanaNSamurai2 is ERC721, Ownable {
	...
	// }
	constructor() ERC721("K2", "K2") {
		setBaseURI("http://api.katanansamurai.art/Metadata/");
	}

	// Claim functions
    // ------------------------------------------------------------------------
	function claimSamurai(uint256 amount) public {
		uint256[] memory tokenId;
		tokenId = KNS1Contract.tokensOfOwner(msg.sender);

		require(numClaim.add(tokenId.length) <= MAX_CLAIM, "Exceeds the MAX_CLAIM.");
		require(amount > 0 && amount <= tokenId.length, "You can't claim the amount of tokens.");
		require(hasClaimed[msg.sender].add(amount) <= tokenId.length, "Exceed the amount that can be claimed");
		...
		emit claimEvent(msg.sender, amount, numClaim);
	}
