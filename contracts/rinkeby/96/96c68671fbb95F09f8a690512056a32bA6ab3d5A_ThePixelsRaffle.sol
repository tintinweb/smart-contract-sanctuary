...
contract ThePixelsRaffle is Ownable, VRFConsumerBase {
  ...
  function getWinner(uint256 raffleIndex, uint256 winnerIndex) external  returns (uint256) {
    uint256 timestamp = raffles[raffleIndex].timestamp;
    uint256 length = raffles[raffleIndex].length;
    return uint256(keccak256(abi.encodePacked(timestamp, length, winnerIndex))) % length;
  }
}
