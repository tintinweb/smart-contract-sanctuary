...
contract eightbitMekascontract is ERC721, Ownable {
    ...
    function reservemekas(address _to, uint256 _reserveAmount) public onlyOwner {        
        uint supply = totalSupply();
        require(_reserveAmount > 0 && _reserveAmount <= mekaReserve, "Not enough reserve meka left");

       for (uint i = 0; i < _reserveAmount; i++) {
            for (uint k = 0; k < MAX_mekaS; k++) {
            uint mintIndex = k;
            if (!_exists(mintIndex) && (mintIndex <MAX_mekaS)) {
