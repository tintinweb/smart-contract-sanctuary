 ...
 *
 * - ability for users to buy randomly chosen tokens from the collection
 */
contract NomoPlayersDropMechanic is ReentrancyGuard {
     ...
     * @param  tokensArray array of all tokenIds that minted in ERC721 contract instance
     ...
     * @param _daoWalletAddr address of the associated DAO wallet instance
     * @param _strategyContractAddr address of the associated Strategy contract contract instance
    ...
    /**
     * @dev Distributes the requested quantity by the user and transfers the respective ETH value
     * to DAO wallet address and strategy contract address.
     *
     * @notice Buyer sends particular value in ETH and request quantity.
     ...
     * NomoPlayersDropMechanic transfers the ETH value received to the pool.
     *
     * @param quantity the quantity of tokens which user requests to buy
     *
     * Requirements:
     *
     * - the caller must send the exact value required in ETH.
     */
    function buyTokens(uint256 quantity) external payable nonReentrant {
        ...
        Address.sendValue(daoWalletAddr, msg.value.div(5).mul(1));
        Address.sendValue(strategyContractAddr, msg.value.div(5).mul(4));

        emit LogTokensBought(transferredTokens);
    }

    /**
     * @dev Returns the current length of the minted collection.
     */
...
library RandomGenerator {
    ...
    function randomize(Random storage randomIndex, uint256 tokensLength) internal returns (uint256) {
        randomIndex.random =
            uint256(
                keccak256(
                    abi.encode(
