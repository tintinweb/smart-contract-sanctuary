...
contract LilFranksMintPassportFactory is AbstractMintPassportFactory  {
    ...
    function makePreLeaf(address _addr) public view returns (bytes memory) {
        bytes memory node = abi.encodePacked(_addr);
        return node;
    }

    function makePostLeaf(bytes memory _preleaf) public view returns (bytes32) {
        bytes32 node = keccak256(_preleaf);
        return node;
    }
