...
contract CastleContract is ERC1155Holder, Ownable {
    ...
    uint16[4] public puzzlesPerTier = [5, 10, 20, 765];
    ...
    event LockedPieces(address indexed _address, uint8 indexed tier);
    event RetrievedPieces(address indexed _address, uint8 indexed tier);
     ...
     */
    function canLockPiecesForTier(address wallet, uint8 tier)
        public
        view
        returns (bool success)
    {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            if (pieceContract.balanceOf(wallet, ids[i]) == 0) flag = true;
        }
        return !flag;
    }
    ...
    function lockPieces(uint8 tier) external returns (bool success) {
        require(canLockPiecesForTier(msg.sender, tier), "INCOMPLETE_PUZZLE");
        require(
        ...
        emit LockedPieces(msg.sender, tier);
        return true;
    }
    ...
    function retrievePieces(uint8 tier) external returns (bool success) {
        ...
        emit RetrievedPieces(msg.sender, tier);
        return true;
    }
}
...
contract PieceContract is ERC1155, Ownable {
    ...
    string private baseURI =
        "ipfs://bafybeiflhv4zhchlcezojeoj35xh5pljwjq3dsdlglc6lk4jlb6kkheffi/";
