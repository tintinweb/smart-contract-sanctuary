...
contract ManateezComic is ERC721, Ownable {
    ...
    uint256 constant OG_TEEZ_PAYMENT = 500;
    
    uint256 constant MERCH_PASS_PAYMENT = 100;
    uint256 constant OG_TEEZ_SUPPLY = 10000;
    ...
    IAlgae  algaetoken;
    IERC721Enumerable  mightymanateez;
    IERC721Enumerable  merchpass;
    ...
    function sendAlgaeToManateezOwners() external{
        require(ogTeezClaiming, "OG Teez claiming is disabled");
        address _user = msg.sender;
        for(uint i = 0; i < OG_TEEZ_SUPPLY; i++){
            if(mightymanateez.ownerOf(i) == _user && !claimedManateez[i]){
                rewards[_user] += OG_TEEZ_PAYMENT;
                claimedManateez[i] = true;
            }
        }
    }
    ...
    function sendAlgaeToMerchPassOwners() external {
        require(merchPassClaiming, "MerchPass claiming is disabled");
        address _user = msg.sender;
        uint256 _amountOwned = merchpass.balanceOf(_user);
        for(uint256 i = 0; i < _amountOwned; i++){
            if(!claimedMerchPass[i]){
                rewards[_user] += _amountOwned.mul(MERCH_PASS_PAYMENT);
                claimedMerchPass[i] = true;
            }
        }
    }
    ...
    constructor(address _algae, IERC721Enumerable _mightyManateez, IERC721Enumerable _merchPass) ERC721("MightyManateez Comic Issue 1", "MMCOMIC") {
