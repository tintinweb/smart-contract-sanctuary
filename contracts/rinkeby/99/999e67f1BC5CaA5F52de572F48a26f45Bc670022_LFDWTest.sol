...
contract LFDWTest is ERC721, Ownable, ReentrancyGuard {
    ...
    address private constant _frederickLevins = 0x15557c8b7246C38EE71eA6dc69e4347F5DAc2104; // Web dev
    ...
    string private constant _baseUrl = "ipfs://QmRF2XNUAGdrFod83PypjuavTJkK8mzgk9rFV4a6hsidPV";
    bool private _adminsSet = false;
    
    address[8] private _shareholders = [_ferb,_wickedDig,_picoPyro,_frederickLevins,_kingOfTheJungle,_reasearchAndDevelopment,_marketing,_comm];
    ...
    constructor() ERC721("LFDW - Test", "LFDWT") {
        isActive = false;
        
        _shares[_ferb] = 49285;
        _shares[_wickedDig] = 24515;
        _shares[_picoPyro] = 3000;
        _shares[_frederickLevins] = 100;
    ...
    function mint(uint256 qty) public payable nonReentrant {
        ...
        require(
            qty < PER_ORDER_LIMIT + 1,
            "Cannot purchase more than the per-order limit"
        );
        require(
            (totalSold() + qty) < MAX_SUPPLY + 1,
    ...
    // The first 85 go to Ferb for distributing to original holders
    function mintDistributions() public onlyOwner {
        for (uint256 i = 0; i < 85; i++) {
            _tokenIds.increment();
            uint256 mintIndex = totalSold();
            _safeMint(_ferb, mintIndex);
        }
    }
