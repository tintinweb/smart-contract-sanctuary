...
contract SipherNFTSale is ISipherNFTSale, Whitelist {
    ...
    uint64[17] public _auctionTimeArray; // store the time array for public auction sale
    ...
    constructor(
        ...
        createAuctionTimeArray(_publicTime,REDUCE_PRICE_INTERVAL);
    }

    function createAuctionTimeArray(uint64 _publicTime, uint64 _interval) internal {
        for (uint64 i = 0; i < _auctionTimeArray.length; i++) 
        {
            _auctionTimeArray[i] = (_publicTime + (i+1)*_interval) ;
        }
    }
    ...
    function getPublicSaleCurrentPrice() public view returns (uint256 currentPrice) {
        ...
        if (timestamp < _saleConfig.publicTime){
            ...
            for (uint64 i = 0; i < _auctionTimeArray.length; i++) 
            {
                currentPrice = SALE_PUBLIC_STARTING_PRICE - i*REDUCE_PRICE_LEVEL;
                if(timestamp < _auctionTimeArray[i]){
                break;  
                } 
            }
            return currentPrice;
