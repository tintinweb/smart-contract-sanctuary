...
contract UniveETHBridge is Ownable{
    ...
    function claim(address toWallet, uint256 amount, bytes32 txHash) public onlyOwner{
        require(claimed[txHash] <= 0, "already claimed");
        require(amount < maxTxAmount, "Claim amount exceeds Max Tx amount");

        IERC20 token = IERC20(tokenAddress);
        require(amount < token.balanceOf(address(this)), "Claim amount exceeds Bridge balance");

        token.transfer(toWallet, amount);
        claimed[txHash] = amount;
    }
