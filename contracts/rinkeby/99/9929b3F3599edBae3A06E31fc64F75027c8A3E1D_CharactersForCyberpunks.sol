...
contract CharactersForCyberpunks is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function cyberpunkHolderClaim() public nonReentrant {
        require(totalSupply() < TOTAL_MINTS, "No more tokens to mint");
        require(cyberpunkContract.balanceOf(msg.sender) > 0, "Must own Loot (for Cyberpunks) to claim");
    ...
    function gearForPunksHolderClaim() public nonReentrant {
        require(totalSupply() < TOTAL_MINTS, "No more tokens to mint");
        require(gearContract.balanceOf(msg.sender) > 0, "Must own Gear (for Punks) to claim");
        uint256 index = totalSupply();
        _totalSupplyTracker.increment();
        _safeMint(_msgSender(), index);
    }
    ...
    function publicClaim() public nonReentrant payable {
        require(totalSupply() < TOTAL_MINTS, "No more tokens to mint");
        require(msg.value == PUBLIC_MINT_PRICE, "Payment must be exactly .015 Ether");
    ...
    constructor() ERC721("Characters (for Cyberpunks)", "CYBRCHAR") Ownable() {}
}
