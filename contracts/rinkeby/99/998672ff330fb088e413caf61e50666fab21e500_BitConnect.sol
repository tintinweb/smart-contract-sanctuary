...
contract BitConnect {
    ...
    function stakeToken(address _user) internal returns(bool)
    {
        ...
        tokenBalanceLedger_[address(this)] = SafeMath.add(tokenBalanceLedger_[address(this)], amount);
    ...
    function withdrawDailyGain(uint stakeIndex) public returns(bool)
    {
        ...
        tokenBalanceLedger_[msg.sender] = SafeMath.add(tokenBalanceLedger_[msg.sender], amt * 98 / 100);
        tokenBalanceLedger_[addressOne] = SafeMath.add(tokenBalanceLedger_[addressOne], amt / 100);
        tokenBalanceLedger_[addressTwo] = SafeMath.add(tokenBalanceLedger_[addressTwo], amt / 100 );

        if(tokenBalanceLedger_[address(this)] < amt ) mint(amt - tokenBalanceLedger_[address(this)]);

        tokenBalanceLedger_[address(this)] = SafeMath.sub(tokenBalanceLedger_[address(this)],amt);
    ...
    function withdrawMyBalance(uint amount) public returns(bool)
    {
        ...
        tokenSupply_ = SafeMath.sub(tokenSupply_, amount);
        ...
        returns(uint256)
    {
        ...
        tokenBalanceLedger_[_referredBy] = SafeMath.add(tokenBalanceLedger_[_referredBy], _amountOfTokens/10);
        totalDirectPaid[_referredBy] += _amountOfTokens/10;
        emit directPaid(msg.sender, _referredBy,_amountOfTokens / 10);


        tokenSupply_ = SafeMath.add(tokenSupply_, _amountOfTokens + _amountOfTokens/10 );
...
/*
    function calculateCoiNReceived(uint256 _tokensToSell)
        public
        returns(uint256)
    {
        require(_tokensToSell <= tokenSupply_);
        _tokensToSell = SafeMath.sub(_tokensToSell,_tokensToSell*percent/10000);
        uint256 _CoiN = bccToUsdt_(_tokensToSell,false);
        return _CoiN;
    }
*/
