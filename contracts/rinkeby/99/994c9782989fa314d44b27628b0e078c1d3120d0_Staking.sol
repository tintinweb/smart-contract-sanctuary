...
contract Staking is Ownable {
    ...
    event Unstaked(address indexed from, uint indexed tokenId, uint timestamp, uint reward);
    ...
    constructor(IERC721Enumerable nftContract_, IERC20 bccContract_, uint baseRewardPerDay_) {
    ...
    function unstakeMultiple(uint[] memory tokenIds) whenNotPaused eoaOnly external {
        for(uint i = 0; i < tokenIds.length; i++) {
            uint reward = claim(tokenIds[i]);
            unstake(tokenIds[i],reward);
        }
    }
    
    function claimAll() whenNotPaused eoaOnly external {
        uint[] memory ownedTokens = addressToOwnedTokens[msg.sender];
        require(ownedTokens.length > 0, "You don't own any tokens to claim!");
    ...
    function getClaimAmountOfAccount(address account) external view returns(uint total) {
        uint[] memory ownedTokens = addressToOwnedTokens[account];
        
        for(uint i = 0;i < ownedTokens.length; i++) {
            total += getClaimAmount(ownedTokens[i],block.timestamp);
        }
    }
    
    function balanceOf(address addr) external view returns(uint) {
        return addressToOwnedTokens[addr].length;
    }
    
    function rewardPerDayPerToken() external view returns(uint) {
        return (baseRewardPerSecond * 86400) / 1e18;
    }
    
    function getClaimAmount(uint tokenId, uint endTimestamp) public view returns(uint) {
        require(ownerOf[tokenId] != address(0));
        uint rewardChangedAt_ = rewardChangedAt;
        if(endTimestamp < rewardChangedAt_ || rewardChangedAt_ == 0) {
            return ( (endTimestamp - lastClaim[tokenId]) * ( rewardChangedAt_ == 0 ? baseRewardPerSecond : oldBaseReward) );
        } else {
            return ((endTimestamp - rewardChangedAt_) * baseRewardPerSecond) + ((rewardChangedAt_ - lastClaim[tokenId]) * oldBaseReward);
        }
    }
    ...
    function claim(uint tokenId) internal returns(uint reward) {
        require(ownerOf[tokenId] == msg.sender);
        reward = getClaimAmount(tokenId,block.timestamp);
    ...
    function unstake(uint tokenId, uint reward) internal {
        require(ownerOf[tokenId] == msg.sender);
    ...
    function setBaseRewardPerDay(uint perDay, uint timestamp) external onlyOwner {
        ...
        emit RewardChanged(oldBaseReward, baseRewardPerSecond, block.timestamp);
    }
    ...
    function emergencyReturnToken(uint tokenId) whenNotPaused eoaOnly external emergencyOnly {
        unstake(tokenId,0);
