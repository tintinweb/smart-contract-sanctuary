...
interface IERC165 {
    ...
    function traitRarity() internal {
        if(totalPizzas > 0) {
            ...
            for(uint256 i = 1; i <= totalIngredients; i++) {
                ...
                if(ingredientDetail.ingType == 2) {
                    sauceUsed++;
                }
                if(ingredientDetail.ingType == 3) {
                    cheeseUsed++;
                }
                if(count > 0) {
    ...
    function callTraitRarity() public {
        traitRarity();
    }
    function calCulateRarity() public {
        calculateRarity();
    }
    ...
    function purchaseAndMintIngretient( uint256 _ingredientId) public payable {
        ...
        require(currentMintCount < ingredientDetail.totalCount, "This ingredient is already sold out");
    ...
    function purchaseAndMintIngretients( uint256[] memory _ingredientIds) public payable { // 3
    ...
    //called on rebake
    function rebakePizza( uint256 _pizzaId, string memory metadata, uint256 base, uint256 sauce, uint256 cheese, uint256[] memory meats, uint256[] memory toppings, uint256[] memory oldIngs ) public onlyNFTOwner(_pizzaId) {
        // verifyUserIngredientTypes(base, sauce, cheese, meats, toppings, true);
    ...
    function randomBakePizzaAndMint(string memory metadata, uint256 base, 
        uint256 sauce, uint256 cheese, uint256[] memory meats, uint256[] memory toppings ) public payable 
    ...
    //called on random bake
    function buyAndBakePizzaAndMint(string memory metadata, uint256 base, 
        uint256 sauce, uint256 cheese, uint256[] memory meats, uint256[] memory toppings ) public payable {
        // verifyUserIngredientTypes(base, sauce, cheese, meats, toppings, false);
        uint256[] memory tops;
        uint256[] memory mets;
    ...
    function unbakePizza( uint256 _pizzaId, uint256[] memory ingredientIds) public onlyNFTOwner(_pizzaId) {
