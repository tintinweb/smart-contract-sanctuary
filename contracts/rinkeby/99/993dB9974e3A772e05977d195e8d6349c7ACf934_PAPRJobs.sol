...
contract Escrow is Migratable {
    ...
    returns (bool) {
        ...
        uint256 initialPAPRAmountConverted = escrows[_escrowId].amount.mul(10 **18).div(IPapr(paprgov).getPaprPrice());

        uint256 isInProfit = getTotalPayoutPAPR(_escrowId).sub(initialPAPRAmountConverted);


        uint256 payToDappAmount = isInProfit.mul(15).div(100);
        uint256 payToDevAmount = isInProfit.mul(5).div(100);
        ...
        uint256 providerPayoutCan1 = escrows[_escrowId].payoutAmount.sub(payToDappAmount);
        uint256 providerPayoutCan = providerPayoutCan1.sub(payToDevAmount);
    ...
    returns (uint256) {
        ...
        //to do : calculate profits made
        escrows[_escrowId].profits = ERC20(paprgov).balanceOf(address(this));
        uint256 totalPayoutPAPR = escrows[_escrowId].profits;
