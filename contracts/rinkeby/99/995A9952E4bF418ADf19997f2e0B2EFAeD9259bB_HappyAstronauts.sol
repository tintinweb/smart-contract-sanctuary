    ...
    AccessControl
{
    ...
    address public constant devAddress =
        0x2EDd6cf1b5B1C69a3F4380b8F586ceC10aa902b2;
    ...
    function _mintAnElement(address _to) private {
        ...
        _tokenURIs[id] = id.toString();
        emit CreateAstronaut(id);
    }

    function mintTokens(uint256 _count, string[] memory tokenURIs_) 
        public 
        payable
    {
        uint256 total = _totalSupply();
        ...
        require(_count == tokenURIs_.length, "Mint count does not match the token count");

        for (uint256 ind = 0; ind < _count; ind++) {
            uint256 id = _totalSupply();
            _tokenIdTracker.increment();
            _safeMint(msg.sender, id);
            _tokenURIs[id] = tokenURIs_[ind];
        }
