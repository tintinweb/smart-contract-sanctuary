...
contract RandomSVG is ERC721URIStorage, VRFConsumerBase {
    ...
    //varibles connected to the svg art
...
//pass the VRF token address etc to the main constuctor to give to the VRF constructor 
//will do similar for the oracle eventually?
...
//this function rquests the random number, but the number is returned in a second transaction to the fulfill randomness function
    ...
    function finishMint (uint256 tokenId) public{
            //actually uses the random number (otherwise, because node calls fulfill randomness, we'd be asking the node to do the coputation)
...
//this is where the actual art gets made
    ...
    function formatTokenURI(string memory imageURI) public pure returns (string memory){
        ...
        return string(abi.encodePacked(baseURL,
            ...
            bytes(abi.encodePacked(
                '{"name": "Hannahs First NFT (Collection)",  ',
                '"description": "NFTs with SVG artwork that lives on-chain", ',
                '"attributes": "", ',
                '"image": "', imageURI, '"}'
                                )
                )
