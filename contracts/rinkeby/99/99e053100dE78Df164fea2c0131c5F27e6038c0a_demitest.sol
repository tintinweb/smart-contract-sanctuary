...
contract demitest is ERC721, ERC721Enumerable, Ownable {
  ...
  string private _OtherDemiHumanURI;
  address private s1 = 0x819A899c0325342CD471A485c1196d182F85860D ;
  address private s2 = 0xDFdC274D86351EcAe8964A66625e7D930fB80A98 ;
  address private s3 = 0x02A882747e601BD9fB0eB43FE1B3e185cc1f4883 ;
  ...
  function mint_presaled1(uint8 NUM_TOKENS_MINT) public payable onlyRealUser {
    require(_isPreSaleD1Active, 'Sales must be active to mint DemiHuman');
    require(totalSupply().add(NUM_TOKENS_MINT) <= 16, 'Exceeding max supply');
    require(_allowList[msg.sender], 'You are not in the Allow List');
    require(NUM_TOKENS_MINT <= 2, 'You can not mint over 2 DemiHuman at a time');
    require(NUM_TOKENS_MINT > 0, 'At least one DemiHuman should be minted');
  ...
  function mint_presaled2(uint8 NUM_TOKENS_MINT) public payable onlyRealUser {
    require(_isPreSaleD2Active, 'Sales must be active to mint DemiHuman');
    require(totalSupply().add(NUM_TOKENS_MINT) <= 20, 'Exceeding max supply');
    ...
    require(bayc_balance > 0 || mayc_balance > 0 || cool_balance > 0, "Must hold at least one BAYC or MAYC or CoolCats to mint a DemiHuman");
    require(NUM_TOKENS_MINT <= 2, 'You can not mint over 2 DemiHuman at a time');
    require(NUM_TOKENS_MINT > 0, 'At least one DemiHuman should be minted');
    require(PRICE*NUM_TOKENS_MINT <= msg.value, 'Not enough ether sent');
  ...
  function mint_public(uint8 NUM_TOKENS_MINT) public payable onlyRealUser {
    require(_isPublicSaleActive, 'Sales must be active to mint DemiHuman');
    require(totalSupply().add(NUM_TOKENS_MINT) <= MAX_SUPPLY, 'Exceeding max supply');
    require(NUM_TOKENS_MINT <= 20, 'You can not mint over 20 DemiHuman at a time');
    require(NUM_TOKENS_MINT > 0, 'At least one DemiHuman should be minted');
    require(PRICE*NUM_TOKENS_MINT <= msg.value, 'Not enough ether sent');
    _mint(NUM_TOKENS_MINT, msg.sender);
    emit TokenMinted(totalSupply());
  ...
  function reserve(uint256 num) public onlyOwner {
    require(totalSupply().add(num) <= MAX_SUPPLY, 'Sale would exceed max supply');
