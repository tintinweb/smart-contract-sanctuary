...
contract SVGNFT is ERC721URIStorage{
    ...
    event CreateedSVGNFT(uint256 indexed tokenId, string tokenURI);

    constructor() ERC721 ("SVG NFT","svgNFT"){
        tokenCounter = 0;
    }

    function create(string memory imageURI) public {
        _safeMint(msg.sender, tokenCounter);
        // string memory imageURI = svgToImageUri(svg);
        string memory tokenURI = formatTokenURI(imageURI);
        _setTokenURI(tokenCounter,tokenURI);
        emit CreateedSVGNFT(tokenCounter,tokenURI);
    ...
    function formatTokenURI(string memory imageURI) public pure returns (string memory){
        ...
        return string(abi.encodePacked(
            baseURL,
            Base64.encode(
                bytes(abi.encodePacked('{"name": "PNG NFT", "description" : "An cool NFT" , "attributes" : "" , "image" : "' , imageURI , '"}'))
            )
        ));
    }
}
