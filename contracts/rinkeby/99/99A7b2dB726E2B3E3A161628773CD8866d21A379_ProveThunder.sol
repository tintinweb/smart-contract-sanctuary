...
contract ProveThunder is ERC721, ERC721Enumerable, ReentrancyGuard, Ownable, Pausable {
  ...
  constructor() ERC721("Prove Thunder", "PROVE") Ownable() {}
  ...
  function reserveTokens(address _to, uint256 _reserveAmount) public onlyOwner {
    ...
    // Specific contract code is completed
    // Add items
    ...
    //   for (string i = 0; i < worlds.length; i++) {
    //     string w = i + uint256(keccak256(abi.encodePacked(block.timestamp))) % (worlds.length - i);
    //     string temp = worlds[n];
    //     worlds[n] = worlds[i];
    //     worlds[i] = temp;
    //   }
    // }

    function shuffleStringArray(string[] memory sourceArray) internal view returns (string[] memory) {

      // Set results array
      string[] memory result = new string[](12);

    //   Initialize through sourceArray
    //   for (uint i = 0; i < sourceArray.length; i++) {
    //     result[i] = i + 1;
    //   }

      // Create a random value
      uint256 randomValue = random(string(abi.encodePacked(block.timestamp, block.difficulty)));

      // Set the last item
      uint last_item = sourceArray.length - 1;

      // Shuffle array using `size - 1` iterations
      for (uint i = 1; i < sourceArray.length - 1; i++) {
        // Select a random item
        uint selected_item = uint(randomValue) % last_item;

        // Swap selected item and last item
        string memory aux = sourceArray[last_item];
        result[last_item] = result[selected_item];
        result[selected_item] = aux;

        // Decrease size of possible shuffle
        last_item--;

        // Generate new randomness
        // uint256 randomValue = keccak256(abi.encodePacked(randomValue));
      }

      return result;
    }

    function getWorldsProgress() public view returns (string[] memory) {
      return shuffleStringArray(worlds);
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"attributes": "something", "name": "Generated #', toString(tokenId), '", "description": "Altered Acoustics description.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
