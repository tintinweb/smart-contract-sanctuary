...
contract ColorLoot is ReentrancyGuard, ERC721Enumerable, Ownable {
    ...
    address public constant lootAddress = 0x392cFE80f35De074d99e934031d3f04EfbFa24e0;
    ...
    address public constant lootAvatarsAddress = 0xe16495FA2e20f357Bbe94b48313335f2F5Eb7Fb5;
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[0] = string(abi.encodePacked(
            '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="black" /><text x="10" y="20" fill="', getItemRarity(parts[1]).color, '">'));
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Bag #', toString(tokenId), '", "description": "Color loot privides rarity stats on chain to loot. Other contracts can access gear rarity of loot on chain now. Feel free to use Color Loot in any way you want.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
