...
contract GoldenFrogs is ERC721, ERC721Enumerable, ERC721URIStorage {
    ...
    uint offsetBlock;
    uint offset;
    address mrF = 0x38857Ed3a8fC5951289E58e20fB56A00e88f0BBD;
    string baseURI;
    ...
    function mint(address og) public {
        require(msg.sender == mrF);
        require(frogId < maxFrogs);
        require(offset != 0);
        frogId += 1;
        _mint(og, frogId + offset % maxFrogs);
    }
    ...
    function receiveTokens(address from, address token, uint amount) public {
        require(ERC20(token).allowance(from, address(this)) >= amount);
        ERC20(token).transferFrom(from, address(this), amount);
        rewardTokens[token].received += amount;
    }
    ...
    function withdrawTokens(address token, uint32[] memory ids) public {
        uint totalRewards;
        uint receivedTokenPerFrog = rewardTokens[token].received / totalSupply();
        for (uint i = 0; i < ids.length; i++) {
            require(ownerOf(ids[i]) == msg.sender, "Must own the token");
            totalRewards += receivedTokenPerFrog - rewardTokens[token].claimedTokens[ids[i]];
            claimedEth[ids[i]] = receivedTokenPerFrog;
        }
