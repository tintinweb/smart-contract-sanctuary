...
contract Regulars is
  ...
  PausableUpgradeable
{
  uint256 public constant UNIT_PRICE = 60000000000000000; // 0.06 ether

  string private _ipfsHash;
  bytes32 public currentPresaleID;
  uint256 public currentPresaleLimit;
  uint256 public currentPresalePrice;
  mapping(bytes32 => mapping(address => uint256)) public presaleMint;

  event Presale(bytes32 whitelistRoot, uint256 amount, uint256 price);
  ...
  modifier onlyWhitelisted(bytes32 leave, bytes32[] memory proof) {
    require(MerkleProofUpgradeable.verify(proof, currentPresaleID, leave), 'Whitelist proof is not valid');
    _;
  }
    ...
    initializer()
  {
    ...
    // contract starts paused - should be unpaused after presales
    _pause();
  ...
  function mintPresale(uint256 numberOfTokens, uint256 quota, bytes32[] calldata proof)
  external payable
    whenPaused()
    withPayment(numberOfTokens * currentPresalePrice)
    onlyRemaining(numberOfTokens + currentPresaleLimit)
    onlyWhitelisted(keccak256(abi.encodePacked(_msgSender(), quota)), proof)
  {
    if (quota > 0) {
      presaleMint[currentPresaleID][_msgSender()] += numberOfTokens;
      require(presaleMint[currentPresaleID][_msgSender()] <= quota, 'Whitelist quota reached');
    }
  ...
  external payable
    whenNotPaused()
    withPayment(numberOfTokens * UNIT_PRICE)
    onlyRemaining(numberOfTokens)
  {
    _mintBatch(_msgSender(), numberOfTokens);
  }
   ...
   */
  function startPresale(bytes32 whitelistRoot, uint256 amount, uint256 price)
  external
    onlyOwner()
  {
    currentPresaleID    = whitelistRoot;
    currentPresaleLimit = remaining() > amount ? remaining() - amount : 0;
    currentPresalePrice = price;
    emit Presale(whitelistRoot, amount, price);
  }
...
import "@amxx/hre/contracts/CardDeck.sol";
