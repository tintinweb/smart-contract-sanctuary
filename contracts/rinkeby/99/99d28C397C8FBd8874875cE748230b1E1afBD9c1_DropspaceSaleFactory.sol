...
import "./DropspaceSale.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/utils/math/SafeMath.sol";

contract DropspaceSaleFactory is IDropspaceSaleFactory, Ownable {
    using SafeMath for uint256;

    uint256 public override totalSales = 0;
    uint256 public override flatFee = 1 ether;
    uint256 public override percentageFee = 10;

    address payable public override revenueWallet;
    
    mapping(uint256 => address) public override sales;

    constructor(address payable _revenueWallet) {
        revenueWallet = _revenueWallet;
        emit RevenueWalletChanged(revenueWallet);
    }

    function setSaleFlatFee(uint256 _flatFee) external override onlyOwner {
        flatFee = _flatFee;
        emit FlatFeeChanged(flatFee);
    }

    function setSalePercentageFee(uint256 _percentageFee) external override onlyOwner {
        percentageFee = _percentageFee;
        emit PercentageFeeChanged(percentageFee);
    }

    function setRevenueWallet(address payable _revenueWallet) external override onlyOwner {
        revenueWallet = _revenueWallet;
        emit RevenueWalletChanged(revenueWallet);
    }

    function withdraw() external override onlyOwner {
        revenueWallet.transfer(address(this).balance);
    }
    ...
    function createSale(
        ...
        DropspaceSale sale = new DropspaceSale(_supplyLimit, _mintLimit, _mintPrice, 
            _devSaleShare, revenueWallet, _withdrawalWallet, _name, _ticker, _baseURI);
        uint256 saleID = totalSales;
        address saleAddress = address(sale);

        sales[saleID] = saleAddress;
        totalSales = totalSales + 1;

        emit SaleCreated(saleID, saleAddress);
        return (saleID, address(sale));
    }
