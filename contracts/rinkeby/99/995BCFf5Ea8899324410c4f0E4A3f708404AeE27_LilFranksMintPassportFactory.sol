...
contract LilFranksMintPassportFactory is AbstractMintPassportFactory  {
    ...
    Counters.Counter private mpCounter; //setup a counter
    ...
    // Sets the basic information on a particular mint pass category
    ...
    // Edits the basic information on a particular mint pass category
    ...
    // Burns the mint pass token, only allowed from redeemable contract
    ...
    // function makeLeaf(address _addr) public view returns (string memory) {
    //     return string(abi.encodePacked(toAsciiString(_addr)));
    // }

    function verifyMerkleProof(bytes32[] calldata merkleProof, uint256 mpIndex) public view returns (bool) {
        // if(mintPasses[mpIndex].merkleRoot == 0xb92ca5e8bf9d9857f07e265516244178cf4460f6dc80b3b1f7e060416dc9cb9a){
        //     return true;
        // }
        // string memory leaf = makeLeaf(msg.sender);
        bytes32 node = keccak256(abi.encode(msg.sender));
        return MerkleProof.verify(merkleProof, mintPasses[mpIndex].merkleRoot, node);
    }

    function verifyMerkleProofTest(bytes32[] calldata merkleProof, bytes32 node, uint256 mpIndex) public view returns (bool) {
        // if(mintPasses[mpIndex].merkleRoot == 0xb92ca5e8bf9d9857f07e265516244178cf4460f6dc80b3b1f7e060416dc9cb9a){
        //     return true;
        // }
        // // string memory leaf = makeLeaf(msg.sender);
