...
contract Migrator is AccessControl {
    ...
    constructor(address _admin, uint256 _minimumRequiredSignature, address _trusty_address, address _token_setter_address) {
		...
		grantRole(TOKEN_ADDRESS_SETTER_ROLE, _token_setter_address);
    ...
    function migrate(address userAddress, uint[] calldata amount, uint expireBlock, uint256 migrationStatus, bytes calldata sigs)
        public 
    {
        ...
        bytes32 sighash = keccak256(abi.encodePacked(userAddress, amount, expireBlock, migrationStatus, getChainID()));
		require(verify(sighash, sigs), "Migrator::migrate: Invalid signatures");

        bytes32 _hash = keccak256(abi.encodePacked(userAddress, amount, migrationStatus));
        if(!isSigned[_hash][msg.sender]){
            isSigned[_hash][msg.sender] = true;
            signatures[_hash].oracles.push(msg.sender);
            signatures[_hash].expireBlocks.push(expireBlock);
        }
        else{
            for (uint256 index = 0; index <  signatures[_hash].oracles.length; ++index){
                if(signatures[_hash].oracles[index] == msg.sender){
                    signatures[_hash].expireBlocks[index] = expireBlock;
                }
            }
        }
        
        emit UpdateSignatures(userAddress, amount, expireBlock, migrationStatus, msg.sender);
        if(signatures[_hash].oracles.length >= minimumRequiredSignature){
            uint256 expBlock = type(uint256).max;
            for(uint256 index = 0; index < signatures[_hash].oracles.length; ++index){
                if(expBlock > signatures[_hash].expireBlocks[index]){
                    expBlock = signatures[_hash].expireBlocks[index];
                }
            }
    ...
    event Migrate(address userAddress, uint[] amount, uint256 migrationStatus);
    event SetTokensAddress(address[] addresses);
    event SetStartBlock(uint256 value);
    event UpdateSignatures(address userAddress, uint[] amount, uint expireBlock, uint256 migrationStatus, address oracle);
}
