...
contract Registry is IRegistry, AccessControl {
    ...
    event TicketsSold(address eventAddress, uint256[] ticketTypes, uint256[] amounts);
    ...
    function createEvent(
        ...
        address eventAddress = IFactory(factory).createEvent(name, symbol, uri, _events.length);
        ...
        getEvent[eventAddress] = EventStruct(eventAddress, msg.sender, name, uri, symbol);
        emit EventCreated(eventAddress, name, uri, msg.sender);
    }

    function buyBatchTickets(
        address eventAddress,
        uint256[] memory ticketTypes,
        uint256[] memory amounts
    ) external payable {
        require(ticketTypes.length == amounts.length, 'ids and amounts must be same size');
        uint256 price;
        for (uint256 i = 0; i < ticketTypes.length; i++) {
            price = price.add(_prices[eventAddress][ticketTypes[i]].mul(amounts[i]));
            require(_amounts[eventAddress][ticketTypes[i]] >= amounts[i], 'Not enough tickets.');
            _amounts[eventAddress][ticketTypes[i]] = _amounts[eventAddress][ticketTypes[i]].sub(amounts[i]);
        }
        require(msg.value >= price, 'Incorrect payment.');
        IEvent(eventAddress).consecutiveMint(msg.sender, amounts, ticketTypes);
        ...
        uint256 fee = price.mul(10).div(100);
        uint256 rest = price.sub(fee);
        //send profit to creator and keep fee on registry
        payable(getEvent[eventAddress].creator).transfer(rest);

        emit TicketsSold(eventAddress, ticketTypes, amounts);
    }
}
...
contract Event is Context, AccessControlEnumerable, ERC721Burnable, ERC721Pausable, IERC2309, IEvent {
    ...
    function consecutiveMint(
        ...
        _consecutiveMint(to, amounts, ticketTypes);
    }

    function _consecutiveMint(
        address to,
        uint256[] memory amounts,
        uint256[] memory ticketTypes
    ) internal virtual {
        require(to != address(0), 'ERC721: mint to the zero address');
        require(amounts.length > 0, 'ERC721: mint empty list');
        require(ticketTypes.length == amounts.length, 'ticketTypes and amounts must be same size');

        for (uint256 i = 0; i < ticketTypes.length; i++) {
            uint256 typeOffset = ticketTypes[i].add(1).mul(10**9);
            uint256 startId = typeOffset.add(_tokenIdTracker[ticketTypes[i]].current());
            for (uint256 j = 0; j < amounts[i]; j++) {
                uint256 tokenId = typeOffset.add(_tokenIdTracker[ticketTypes[i]].current());
                _beforeTokenTransfer(address(0), to, tokenId);
                _balances[to] += 1;
                _owners[tokenId] = to;
                _tokenIdTracker[ticketTypes[i]].increment();
            }
            emit ConsecutiveTransfer(startId, typeOffset.add(_tokenIdTracker[ticketTypes[i]].current()), address(0), to);
        }
    }

    function mint(address to) public virtual {
        require(hasRole(MINTER_ROLE, _msgSender()), 'ERC721PresetMinterPauserAutoId: must have minter role to mint');
        _mint(to, _tokenIdTracker[0].current());
        _tokenIdTracker[0].increment();
    }
