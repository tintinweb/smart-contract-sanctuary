...
contract PX is ERC721, ERC721Metadata, ERC721Enumerable {
    ...
    function alphaMass() public view virtual returns (uint256) {
        return decodeMass(_values[_alphaId]);
    }
    ...
    function mint(uint256[] memory values_) external {
        ...
        uint256 index = _tokenCount;
        uint256 massAdded = 0;

        uint256 alphaValue = decodeMass(_values[_alphaId]);
        ...
        emit ConsecutiveTransfer(_tokenCount, index, address(0), _omnibus);

        _tokenCount += values_.length;

        _balances[_omnibus] = _tokenCount;

        _totalMass += massAdded;

        if(_alphaId != alphaId) {
            _alphaId = alphaId;
            emit NewLargestMass(alphaId, alphaValue);
        }       
    }
     ...
     */
     function merge(uint256 tokenIdRcvr, uint256 tokenIdSndr) public returns (uint256 candidate) {
        ...
        if (massRcvr >= massSndr) {
            ...
            if(_values[tokenIdRcvr] > _values[_alphaId]){
        ...
        if(_values[tokenIdSndr] > _values[_alphaId]){
            _alphaId = tokenIdSndr;
