...
contract ClownPunkSociety is ERC721Enumerable, Ownable {
    ...
    uint256 private MINT_PRICE_IN_WEI;
    ...
    function configureEnvironment(
        ...
        bool shouldVerifyCryptoPunk) public onlyOwner unsealed {
        ...
        MINT_PRICE_IN_WEI = tierPriceInWei;
        shouldVerifyCryptoPunkOwnership = shouldVerifyCryptoPunk;
    }
    
    constructor() ERC721("ClownPunkSociety", "CLWN") {
        configureEnvironment(
            0xc271100C2e6379C9A526Fdb4f63a0E4212f5CddF,
            0x1a3b191A27D8F516dbd8fAE1164920028aB66DD0,
            0x20828C513920A8858FD2C985Ba700e1a484E7a8f, // odd (even) punks owned by test account 1 (2)
            (5 ether) / 100,
            true);
    }
    ...
    function punkToClownAssets(uint16 punkIndex) internal view returns (bytes memory childAssets) {
        ...
        for (uint8 j = 0; j < 8; ++j) {
            ...
            // Need to special case the Mona Lisa here (remove Mole to make room for Nose).
            if ((j == 7) && (asset != 0)) {
                punkTraits[uint8(Type.Face)] = 0;
            }
        }
        ...
        for (uint8 j = 0; j < 10 && assetIndex < 8; ++j) {
            ...
            bool isNose = (j == uint8(Type.Nose));
            if (isNose || (asset != 0)) {
                childAssets[assetIndex++] = isNose ? bytes1(isMale ? 18 : 109) : bytes1(asset);
            }
        }
    }
    
    function ownsCryptoPunk(uint16 index) internal view returns (bool) {
        return cryptoPunksMarket.punkIndexToAddress(index) == msg.sender;
    }
    ...
    function mintClownFromPunk(uint16 punkIndex) external payable {
        ...
        require(punksToClowns[punkIndex] == 0, "Already minted a clown from this punk");

        if (!ownsCryptoPunk(punkIndex)) {
            require(!shouldVerifyCryptoPunkOwnership, "Minting open to CryptoPunk owners only.");
            require(MINT_PRICE_IN_WEI  <= msg.value, "Insufficient Ether sent");
        }

        clowns[clownIndex] = punkToClownAssets(punkIndex);
    ...
    function tokenURI(uint256 index) public view override returns (string memory)
    {
        ...
        string memory json = base64Encode(bytes(string(abi.encodePacked(
            ...
            '", "description": "Clown Punks are the funny siblings of the original CryptoPunks. Minting is free for CryptoPunk owners. 10% of sales donated to GiveDirectly.org. All metadata and images are fully generated and stored on-chain. Inspired by LarvaLabs (not affiliated).", "image": "data:image/svg+xml;base64,', 
    ...
    function metadataAttributes(bytes memory punkAssets, uint16 index) internal view returns (string memory text) {
        ...
        text = appendAttribute(text, "Father", Strings.toString(clownsToPunks[index]), false, true, true);
    }
