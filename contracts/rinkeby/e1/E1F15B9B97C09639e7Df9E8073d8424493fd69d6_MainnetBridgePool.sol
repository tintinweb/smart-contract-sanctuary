...
contract MainnetBridgePool is AccessControl, ReentrancyGuard {
  ...
  event FeeSet(uint256 fee, uint256 decimals);
  ...
  address private WETH = Uniswap(UNIROUTER).WETH();
...
function getFee(uint _amount) public view returns (uint[] memory) {
    address[] memory path = new address[](2);
    path[0] = address(raini);
    path[1] = WETH;
    //.mul(fee).div(10 ** feeDecimals)
    return Uniswap(UNIROUTER).getAmountsOut(_amount, path);
  }

  function setFee(uint256 _fee, uint256 _decimals)
    external onlyOwner {
      fee = _fee;
      feeDecimals = _decimals;
      emit FeeSet(_fee, _decimals);
  }
