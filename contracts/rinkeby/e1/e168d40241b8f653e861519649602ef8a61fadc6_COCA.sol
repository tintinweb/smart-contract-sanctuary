...
contract COCA is ERC721, Ownable, ERCWithdrawable {
    ...
    function presaleMint(uint16 amount) external payable onlySender whitelistSale {
        ...
        require(amount <= 1, "Max 1 mint per transaction");
        require(addressToPublicMinted[msg.sender] < 2, "Max 2 mints per account!");
    ...
    function publicMint(uint16 amount) external payable onlySender publicSale {
        ...
        require(addressToPublicMinted[msg.sender] < 5, "Max 5 mints per account!");
        require(maxTokens > totalSupply, "No more remaining tokens!");
