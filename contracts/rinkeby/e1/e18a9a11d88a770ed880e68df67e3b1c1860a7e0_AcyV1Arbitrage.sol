...
contract AcyV1Arbitrage is IAcyV1Arbitrage{
    ...
    function swap0(uint amountIn,address inputToken,address outputToken) payable external {
        address pair_address = UniswapV2Library.pairFor(factory, inputToken, outputToken);
        TransferHelper.safeTransferFrom(
            inputToken, msg.sender, pair_address, amountIn
        );
        uint balanceBefore = IERC20(outputToken).balanceOf(address(this));
        
        (address input, address output) = (inputToken, outputToken);
        (address token0,) = UniswapV2Library.sortTokens(input, output);
        IUniswapV2Pair pair = IUniswapV2Pair(UniswapV2Library.pairFor(factory, input, output));
    }
    
    function swap1(uint amountIn,address inputToken,address outputToken) payable external {
        address pair_address = UniswapV2Library.pairFor(factory, inputToken, outputToken);
        TransferHelper.safeTransferFrom(
            inputToken, msg.sender, pair_address, amountIn
        );
        ...
        // require(
        //     IERC20(outputToken).balanceOf(address(this)).sub(balanceBefore) >= 0,
        //     'UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT'
        // );
    }
    
    function swap2(uint amountIn,address inputToken,address outputToken) payable external {
        address pair_address = UniswapV2Library.pairFor(factory, inputToken, outputToken);
        TransferHelper.safeTransferFrom(
            inputToken, msg.sender, pair_address, amountIn
        );
