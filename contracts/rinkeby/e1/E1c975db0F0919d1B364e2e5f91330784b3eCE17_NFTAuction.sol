...
contract NFTAuction is Context, ReentrancyGuard {
    ...
    /// @notice The list of tokenIds of Auction.
    EnumerableSet.UintSet internal auctionIds;
    ...
    function createAuction(
        ...
        require(
            ...
            "NFTAuction.createAuction: Sender must have the minter role"
        );

        // Check owner of the token is the creator and approved
        require(
            marketTradingNft.ownerOf(_tokenId) == _msgSender() && marketTradingNft.isApproved(_tokenId, address(this)),
    ...
    function resultAuction(uint256 _tokenId) external nonReentrant {
        ...
        require(marketTradingNft.isApproved(_tokenId, address(this)), "NFTAuction.resultAuction: auction not approved");
        ...
        marketTradingNft.setPrimarySalePrice(_tokenId, winningBid);
        ...
        (bool creatorTransferSuccess,) = marketTradingNft.ownerOf(_tokenId).call{value : winningBid.sub(platformFeeInETH)}("");
        require(creatorTransferSuccess, "NFTAuction.resultAuction: Failed to send the designer their royalties");

        // Transfer the token to the winner
        marketTradingNft.safeTransferFrom(marketTradingNft.ownerOf(_tokenId), winner, _tokenId);
    ...
    function cancelAuction(uint256 _tokenId) external nonReentrant {
        ...
        require(
            accessControls.hasAdminRole(_msgSender()) || accessControls.hasSmartContractRole(_msgSender()),
            "NFTAuction.cancelAuction: Sender must be admin or smart contract"
        );
    ...
    /**
     ...
     @param _tokenId Token ID of the garment being auctioned
    ...
    function updateAuctionReservePrice(uint256 _tokenId, uint256 _reservePrice) external {
        require(
            ...
            "NFTAuction.updateAuctionReservePrice: Sender must be admin"
        );
    ...
    function updateAuctionStartTime(uint256 _tokenId, uint256 _startTime) external {
        require(
            accessControls.hasAdminRole(_msgSender()),
            "NFTAuction.updateAuctionStartTime: Sender must be admin"
        );
    ...
    function updateAuctionEndTime(uint256 _tokenId, uint256 _endTimestamp) external {
        require(
            accessControls.hasAdminRole(_msgSender()),
            "NFTAuction.updateAuctionEndTime: Sender must be admin"
        );
        require(
            auctions[_tokenId].endTime > 0,
    ...
    /**
     @notice Method for getting all Auction's tokenId
     */
    function getAllAuctionTokenId()
    external
    view
    returns (bytes32[] memory _tokenIds) {
        return auctionIds._inner._values;
    }
    ...
    function _createAuction(
        ...
        // add tokenId to Auction list
        auctionIds.add(_tokenId);
