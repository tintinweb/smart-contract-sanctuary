...
contract StandardCreativeAuction is IStandardCreativeAuction, Ownable, ReentrancyGuard {
  ...
  function finishAuction(uint256 auctionId) public payable auctionEnded(auctionId) {
    // Lookup auction
    Auction storage auction = _auctions[auctionId];
  ...
  function finishAuctionTest(uint256 auctionId) public payable auctionEnded(auctionId) {
    // Lookup auction
    Auction storage auction = _auctions[auctionId];

    if (auction.topBidAddress == _msgSender()) {
      payable(owner()).transfer(auction.topBidAmount);
      auction.finished = true;
      emit AuctionFinished(auctionId);
    } else {
      // Refund caller if eligible
