...
-------------------------------------------------
*/
contract CryptoArt is ERC721Enumerable, ReentrancyGuard, Ownable {
    using Strings for uint256;
    uint256 public constant maxSupply = 25;
    uint256 public constant mintPrice = 0.01 ether;
    string private currentBaseURI;
    string[] private z = ['<svg xmlns:xlink="http://www.w3.org/1999/xlink" style="fill-opacity:1;color-rendering:auto;color-interpolation:auto;text-rendering:auto;stroke:black;stroke-linecap:square;stroke-miterlimit:10;shape-rendering:auto;stroke-opacity:1;fill:black;stroke-dasharray:none;font-weight:normal;stroke-width:1;font-family:Dialog;font-style:normal;stroke-linejoin:miter;font-size:12px;stroke-dashoffset:0;image-rendering:auto" width="400" height="400" xmlns="http://www.w3.org/2000/svg"><defs id="genericDefs"/><g><g><rect x="0" width="400" height="400" y="0" style="stroke:none"/></g><g style="fill:white;text-rendering:optimizeLegibility;font-size:30px;font-family:Processing Sans Pro;stroke:white"><text x="68" xml:space="preserve" y="200" style="stroke:none">[',',',']</text></g></g></svg>'];
    string private rl4="data:application/json;base64,";
    string private rl1='{"name": "a collection of random points #';
    string private rl3='"}';
    ...
    function random() private view returns (uint) {
        ...
        return randomHash % 600;
    } 

    function randomOne(uint256 tokenId) internal view returns (Point memory) {
        tokenId = tokenId; // avoid dupes
        Point memory point;
        point.pointX = uint(random());
        point.pointY = uint(random());
        return point;
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory){
        Point memory point = randomOne(tokenId);
        string memory px = string(abi.encodePacked(point.pointX));
        string memory py = string(abi.encodePacked(point.pointY));
        string memory output = string(abi.encodePacked(z[0], px, z[1], py, z[2]));
        return string(abi.encodePacked(rl4,Base64.encode(bytes(string(abi.encodePacked(rl1, tokenId.toString(), tr8, Base64.encode(bytes(output)), rl3))))));
        }
}