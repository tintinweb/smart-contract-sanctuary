...
contract GMBot is ERC1155, Ownable, ERC1155Burnable, AccessControl {
    struct Subscription {
        uint256 price;
        uint256 time;
    }
    ...
    constructor() ERC1155("https://gateway.pinata.cloud/ipfs/QmQNZf2iT94vFRA5PtwReMJxaANapZBD5zjJ45sMhgxtyK/GMBot.json") {
     ...
     /**
     * @dev Start/Stop the public sale
     */
    ...
    */
    function subscribe(uint256 _subscriptionIndex) external payable {
        ...
        require(ERC1155(address(this)).balanceOf(msg.sender, 1) > 0, "You must own a GM Bot NFT to use this function.");
        require(_subscriptionIndex < 3);

        _subscribe(_subscriptionIndex);
    }

    /**
    * @dev Internal subscribe function
    */
    ...
    /**
    * @dev Change the Subscription Plans.
    */
    function setSubscriptionPlans(uint256 _index, uint256 _priceInWei, uint256 _timeInDays) external onlyOwner {
        subscriptionOptions[_index].price = _priceInWei;
    ...
    */
    ...
    /**
    * @dev Change the max amount of tokens able to be minted.
    */
    ...
    /**
    * Returns subscription plan's price/time
    */
    function getSubscriptionPlan(uint256 _index) external view returns(uint256, uint256) {
        return (subscriptionOptions[_index].price, subscriptionOptions[_index].time);
    }

    /**
    * Returns address' time left on subscription in seconds.
    */
    ...
    */
    ...
    /**
    * @dev Burn all address' tokens and sets their subscription time to 0
    */
    function burnToken(address _address) external onlyOwner {
        uint256 _tokenAmount = ERC1155(address(this)).balanceOf(msg.sender, 1);
        _burn(_address, 1, _tokenAmount);

        users[_address].end_of_subscription = 0;
        CLAIMED_TOKENS = CLAIMED_TOKENS - _tokenAmount;
    }
    ...
    */
    ...
    function _beforeTokenTransfer(
        ...
        // if somehow the person buying has time on their subscription, we addon time from the seller to the buyer
        // else we just equally distribute time to the seller
        if(users[to].end_of_subscription <= 0) {
           users[to].end_of_subscription = users[from].end_of_subscription;
        } else {
            users[to].end_of_subscription += users[from].end_of_subscription;

        }
        // Reset seller's subscription to 0
