    ...
    UUPSUpgradeable
{
    ...
    mapping(string => uint256) internal _tokenUsedURIs;      // starting index = 1 (0 = default not used)
    mapping(uint256 => address) internal _creatorCoinMap;    // tokenId => address
    mapping(uint256 => Bid) internal _profileDetails;        // tokenId => bid

    uint256 internal protocolFee;             // global fee
    ...
    function setTokenURI(uint256 tokenId, string memory _tokenURI) private {
        ...
        _tokenUsedURIs[_tokenURI] = tokenId.add(1); // adds 1 to preserve 0 being the default not found case
    }

    // custom function to initialize creator coin
    function initializeCreatorCoin(uint256 tokenId) external {
        require(_exists(tokenId) && _creatorCoinMap[tokenId] == address(0x0));

        CreatorCoin creatorCoinContract = new CreatorCoin(
            address(this),
            nftErc20Contract,
            _tokenURIs[tokenId],
            string(abi.encodePacked("@", _tokenURIs[tokenId]))
        );

        _profileOwnerFee[tokenId] = 1000; // default fee is 10%

        // set tokenId => creator coin contract
        _creatorCoinMap[tokenId] = address(creatorCoinContract);
    }
    ...
    */
    ...
    function mintBurnHelper(uint256 _type, uint256 _amount, uint256 tokenId) private nonReentrant {
        ...
        if (_type != 0) { // mint
            // fee denominated in NFT.com ERC20
            ...
            // transfer NFT.com tokens to creator coin contract (require)
    ...
    */
    ...
    /**
     @notice function to return URI for a profile token
     @param tokenId the ID of the NFT.com profile
...
interface ICreatorCoin {
    function performAction(uint256, address, address, uint256, uint256, uint256, uint256) external;
}
