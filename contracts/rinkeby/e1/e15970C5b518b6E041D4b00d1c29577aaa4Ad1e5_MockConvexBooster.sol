...
// File: contracts/mocks/tokens/MockCRV.sol
...
// File: contracts/mocks/tokens/MockCVX.sol
...
interface IConvexStashFactory {
    function CreateStashTokens(address _operator,uint256 _pid, address _convexStash) external returns (address);
}
...
// File: contracts/mocks/lending/MockConvexBooster.sol
...
contract ConvexRewardPool {
    ...
    IERC20 public rewardConvexToken;
    ...
    constructor(address _op) public {
        rewardToken = IERC20(new MockCRV());
        rewardConvexToken = IERC20(new MockCVX());
        operator = _op;
    }
    ...
    function getReward(address _for) public updateReward(_for) {
        ...
        if (reward > 0) {
            ...
            rewardConvexToken.safeTransfer(_for, reward);
...
contract MockCurveSwap {
    ...
    constructor(int256 _flag) public {
        ...
        if (_flag == 0) {
            ...
            coins.push(0x5592EC0cfb4dbc12D3aB100b257153436a1f0FEa); // compound testnet dai
            // usdc
            // coins.push(0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48);
            coins.push(0x4DBCdF9B62e891a7cec5A2568C3F4FAF9E8Abe2b); // compound testnet usdc
            // usdt
            coins.push(0xdAC17F958D2ee523a2206206994597C13D831ec7);
        }

        // steth pool
        if (_flag == 1) {
            // eth
            coins.push(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE);
            // stETH
            coins.push(0xae7ab96520DE3A18E5e111B5EaAb095312D7fE84);
        }
        ...
        returns (uint256)
    {
        if (flag == 0) {
            if (tokenId == 0) {
                return (tokenAmount * 1e18) / 981781786775024400;
            }

            if (tokenId == 1) {
                return (tokenAmount * 1e18) / 981524505583;
            }

            if (tokenId == 2) {
                return (tokenAmount * 1e18) / 981481275472;
            }
        }
...
contract MockConvexBooster is Ownable {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;

    // struct PoolInfo {
    //     address swapAddress; // like 3pool https://github.com/curvefi/curve-js/blob/master/src/constants/abis/abis-ethereum.ts
    //     uint256 targetPid;
    //     address lpToken;
    //     address staker;
    //     address rewardPool;
    ...
    MockCurveLP public curve3CRV;
    MockCurveLP public curveSteCRV;

    constructor() public {
        curve3CRV = new MockCurveLP("Mock Curve LP-3CRV", "MCLP-1", msg.sender);
        curveSteCRV = new MockCurveLP(
            "Mock Curve LP-steCRV",
            "MCLP-2",
            msg.sender
        );
    }
    ...
    function demoAddPool() public {
        poolInfo.push(
            PoolInfo({
                convexPid: 9, // convex pool 9 = 3crv (Curve.fi DAI/USDC/USDT) https://etherscan.io/address/0x6c3F90f043a72FA612cbac8115EE7e52BDe6E490
                curveSwapAddress: address(new MockCurveSwap(0)),
                lpToken: address(curve3CRV),
                originCrvRewards: address(0),
                originStash: address(0),
                virtualBalance: address(0),
                rewardCrvPool: address(new ConvexRewardPool(address(this)))
            })
        );

        poolInfo.push(
            PoolInfo({
                convexPid: 25, // convex pool 25 = steCRV (Curve.fi ETH/stETH) https://etherscan.io/address/0x06325440D014e39736583c165C2963BA99fAf14E
                curveSwapAddress: address(new MockCurveSwap(1)),
                lpToken: address(curveSteCRV),
                originCrvRewards: address(0),
                originStash: address(0),
                virtualBalance: address(0),
                rewardCrvPool: address(new ConvexRewardPool(address(this)))
    ...
    function demoMintCurve3CRV() public {
        curve3CRV.mint(1000000 * 10**uint256(curve3CRV.decimals()));
    }

    function demoMintCurveSteCRV() public {
        curveSteCRV.mint(1000000 * 10**uint256(curveSteCRV.decimals()));
    }

    function recycleAssets(address _asset) public {
    ...
    function depositFor(
        ...
        IConvexRewardPool(pool.rewardCrvPool).stakeFor(_user, _amount);
    ...
    function withdrawFor(
        ...
        IConvexRewardPool(pool.rewardCrvPool).withdrawFor(_user, _amount);
