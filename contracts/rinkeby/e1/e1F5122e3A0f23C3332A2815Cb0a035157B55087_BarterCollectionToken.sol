...
// File: BarterCollectionToken.sol

contract BarterCollectionToken is Ownable, ERC1155 {
    ...
    IERC20 feeToken;
    uint256 public mintFee = 10 * 10**18;  // static
    ...
    constructor (IERC20 feeToken_, address newOwner) ERC1155("") {
        feeToken = feeToken_;
    ...
    function mint(uint256 id, uint256 amount, uint256 value, string memory uri_, bytes memory signature) public {
        require(creators[id] == address(0), "Token is already minted");
        require(amount != 0, "Amount should be positive");
        require(bytes(uri_).length > 0, "uri should be set");

        address signer = keccak256(abi.encodePacked(msg.sender, id, amount, uri_)).toEthSignedMessageHash().recover(signature);
