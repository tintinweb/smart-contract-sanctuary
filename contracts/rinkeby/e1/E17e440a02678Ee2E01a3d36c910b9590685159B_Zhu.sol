...
contract Zhu is IZhu, ERC20('Zhu Token', 'ZHU'), Governed {
    using TCPSafeMath for uint256;

    
    
    function reserves() external override view returns (uint) {
        return balanceOf(address(this));
    }

    
    
    function distributeReserves(address dest, uint count) external override lockContract {
        require(msg.sender == address(governor.auctions()), 'Not Authorized');

        _transfer(address(this), dest, count);
    }

    
    function burnReserves(uint count) external override lockContract {
        governor.requireZhuReservesBurnAccess(msg.sender);

        _burn(address(this), count);
    }

    
    function mintTo(address dest, uint count) external override lockContract {
        governor.requireZhuMintingAccess(msg.sender);
