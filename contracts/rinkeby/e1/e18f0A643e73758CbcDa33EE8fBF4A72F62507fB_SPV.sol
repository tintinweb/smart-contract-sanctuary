...
contract SPV is Ownable {
    ...
    constructor (address _treasury, address _USDC, address _USDT, address _DAI, address _SPVWallet, uint _endTime) {
        ...
        require( _endTime > block.timestamp, "Must be in future" );
        endTime = _endTime;
        updateTotalValue();
    }
    ...
    function setEndTime(uint _endTime) external onlyOwner() {
        require(endTime > block.timestamp, "Must be in future");
        endTime = _endTime;
    }
    ...
    function uniswapTokenPrice(address _token) public view returns (uint) {
        ...
        return reserve.mul(10 ** keeperDecimals).mul( 10 ** IERC20Extended(reserve).decimals() ).div( tokenAmount ).div( 10 ** IERC20Extended(_token).decimals() );
    }
    ...
    function getTokenBalance( uint _index ) internal view returns (uint) {
        ...
        uint tokenBalance = IERC20Extended(_token).balanceOf( SPVWallet ).mul( 10 ** keeperDecimals ).div( 10 ** IERC20Extended( _token ).decimals() ) ;
        return tokenBalance.mul(tokens[ _index ].price);
    }


    function updateTotalValue() public {
        uint newValue;
    ...
    function auditTotalValue() public {
        uint newValue;
        for ( uint i = 0; i < tokens.length; i++ ) {
    ...
    function returnToTreasury( uint _index, address _token, uint _amount ) external {
        ...
        ITreasury( treasury ).SPVDeposit( _amount, _token );
        updateTotalValue();
...
interface ITreasury {
    ...
    function SPVDeposit( uint _amount, address _token ) external;
}
