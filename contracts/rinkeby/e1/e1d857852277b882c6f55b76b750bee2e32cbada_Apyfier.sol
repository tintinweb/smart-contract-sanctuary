...
contract Apyfier is Ownable {
    ...
    */
    ...
    function _updateStrategy(uint256 _sid, uint256 _amount, bool _isDeposit) private {
        ...
        (uint256 _strategyBalance,, uint256 _shareValue) = _updateStats(_sid, _strategy);//0,0

        uint256 _oldShareAmount = _user.strategyShares[_sid];//0
        uint256 _oldAmount = _oldShareAmount * _shareValue / 1e18;  //0
        uint256 _newAmount = _oldAmount;//0

        if (_isDeposit) {
            _newAmount += _amount;//10000000
            _strategy.pendingBalance += _amount;//10000000
            _strategy.lastBalance += _amount;//10000000
            if (_strategy.status == StrategyStatus.Active) {
    ...
    function _rebalance(Strategy storage _strategy, uint256 _strategyBalance, uint256 _withdrawalAmount, bool _force) private {
        uint256 _pending = _strategy.pendingBalance;   //10000000 
        uint256 _total = _strategyBalance + _pending;//10000000
         // fix for last withdrawal imprecision
        if (_withdrawalAmount > _total) {
            _withdrawalAmount = _total;
        }
        uint256 _perfectPendingBalance = _withdrawalAmount + ((_total - _withdrawalAmount) * pendingBalanceFactor) / 1e18;//0
        if (_pending > _perfectPendingBalance) {
            uint256 _moveAmount = _pending - _perfectPendingBalance;//1000000
            uint256 _tokenAvailable = token.balanceOf(address(this));//10000000
    ...
    function _calculateShareValue(Strategy storage _strategy) private view returns (uint256 _strategyBalance, uint256 _balance, uint256 _shareValue, uint256 _gain) {
        _strategyBalance = _strategy.status == StrategyStatus.Active ? _strategy.instance.balance() : 0;//0
        _balance = _strategyBalance + _strategy.pendingBalance;//0+0 == 0
        ...
        _shareValue = _strategy.shareTotal > 0 ? _balance * 1e18 / _strategy.shareTotal : _strategy.shareValue; //0 
    }
