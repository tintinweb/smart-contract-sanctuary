...
//    mapping (address => uint256) public newUsersId;
    ...
    event Withdraw(uint256 _pid, address indexed user, uint256 amount);
    event Deposit(uint256 _pid, address indexed user, uint256 amount);


    constructor(IToken _rToken) public {
        rToken = _rToken;
    }
    ...
    */
    function harvest(uint256 _amount, uint256 _lastBlockNumber, uint256 _currentBlockNumber, bytes32 _msgForSign, bytes memory _signature) public {
        ...
        userRewardInfo[msg.sender].rewardDebt = userRewardInfo[msg.sender].rewardDebt.add(_amount);
        userRewardInfo[msg.sender].lastBlock = _currentBlockNumber;
        if (_amount > 0) {
            rToken.transfer(msg.sender, _amount);
        }
        emit Harvest(msg.sender, _amount, _currentBlockNumber);
    }


    function emergencyRefund(uint256 _pid) public onlyOwner {
        emit EmergencyRefund(msg.sender, rToken.balanceOf(address(this)));
    ...
    function getMultiplier(uint256 _id) public view returns(uint256) {
