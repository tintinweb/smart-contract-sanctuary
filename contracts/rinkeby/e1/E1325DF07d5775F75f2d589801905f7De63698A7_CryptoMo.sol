...
// 0xe14178837A5bDBE37c157753Cb443F1f1Ef46430 - ONLY for testnet
...
contract CryptoMo is Ownable, ERC721Enumerable, PaymentSplitter {
    ...
    uint public _reserved; // How many are kept in reserve by admins
    ...
    ERC721("Crypto Moes", "MOES")
    PaymentSplitter(team, team_shares)
    {
        _baseURIExtended = "ipfs://QmbHjsvFJT8uP64xRRSKoXuoq4VYXeRaao1VKzK3JFyEvE/";
        accounts[msg.sender] = Account( 25, 0, 0, true, true ); // this is only for managing the contract
        accounts[admins[0]] = Account( 150, 0, 0, true, true );
        accounts[admins[1]] = Account( 50, 0, 0, true, true );
        accounts[admins[2]] = Account( 50, 0, 0, true, true );
        accounts[admins[3]] = Account( 25, 0, 0, true, true );

        _reserved = 300; // TODO CHECK!! this must be the sum of the reserved above ****

        _team = team;
        _team_shares = team_shares;

        presale = 1634068800;
        publicSale = presale + 24 hours; // 24 hours after presale 1PM West coast 10/13
    ...
    function resetSaleTimes() external onlyAdmin {
        presale = block.timestamp + 2 minutes;
        publicSale = presale + 2 minutes;
    }
    ...
    function adminMint(uint32 _amount) external onlyAdmin {
        ...
        // console.log(_amount, accounts[msg.sender].nftsReserved, "Amount requested more then you have reserved")
    ...
    function mintMoe(uint _amount) external payable noReentrant {
        ...
        require(block.timestamp >= presale, "Sale has not started yet");
