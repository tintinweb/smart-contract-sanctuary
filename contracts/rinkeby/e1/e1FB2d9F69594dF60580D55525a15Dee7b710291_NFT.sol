...
contract NFT is ERC721, ERC721Enumerable, Ownable {
  ...
  // 共用
  string private _baseURIExtended = ''; // URI
  uint256 public constant MAX_SUPPLY = 1000; // 最大發行量
  uint256 public tierSupply = 0; // 可發行量
  uint256 public maxBalance = 1; // 最大擁有數量

  // 公售
  bool public isActive = false; // 是否開始
  uint256 public price = 1 ether; // 購買價格
  uint256 public maxMint = 1; // 最大單次購買數量
  uint256 public startAt = 0; // 開始時間

  // 白名單
  bool public whiteListIsActive = false; // 是否開始
  uint256 public whiteListPrice = 1 ether; // 購買價格
  uint256 public whiteListMaxMint = 1; // 最大單次購買數量
  uint256 public whiteListStartAt = 0; // 開始時間
  ...
  function withdraw(address to) virtual public onlyOwner {
  ...
  function mint(uint256 qty) virtual external payable {
  ...
  function mintWhiteList(uint256 qty) external payable {
    require(
