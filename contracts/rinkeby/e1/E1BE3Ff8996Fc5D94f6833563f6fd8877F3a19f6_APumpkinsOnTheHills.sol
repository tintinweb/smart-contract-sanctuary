...
contract APumpkinsOnTheHills is ERC721Enumerable, Ownable{
    using SafeMath for uint256;
    using Address for address;
    using Strings for uint256;
    
       event getTrickorTreat(
        uint _value
    );

    uint256 public constant NFT_PRICE = 10000000000000000; // 0.08 ETH
    ...
    constructor() ERC721("PumpkinsOnTheHills","POTH"){
    }
    ...
    function mintPumpkin(uint numberOfTokens) public payable {
	...
	function random(
      ...
      function trickOrTreat() public payable returns (uint) {
        require(saleIsActive, "Sale is not active at the moment");
        require(totalSupply().add(MAX_NFT_PURCHASE) <= MAX_SUPPLY, "Purchase would exceed max supply of Pumpkins");
        require(NFT_PRICE.mul(5) == msg.value, "Sent ether value is incorrect");
        
   uint256 r = random(1, 3, uint256(uint160(address(msg.sender))));
