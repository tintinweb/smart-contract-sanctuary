...
contract KikaBar is ERC20("KiKaBar", "xKIKA"), OwnableContract{
    using SafeMath for uint256;
    IERC20 public kika;
    ...
    // Define the KIKA token contract
    constructor(IERC20 _kika) public {
        kika = _kika;
    }

    // Enter the bar. Pay some KIKAs. Earn some shares.
    // Locks KIKA and mints xKIKA
    function enter(uint256 _amount) public {
        require(_isEnableDayOfMonth() || enterEnable, "Can not enter now");

        // Gets the amount of KIKA locked in the contract
        uint256 totalKika = kika.balanceOf(address(this));
        // Gets the amount of xKIKA in existence
        uint256 totalShares = totalSupply();
        // If no xKIKA exists, mint it 1:1 to the amount put in
        if (totalShares == 0 || totalKika == 0) {
            _mint(msg.sender, _amount);
        }
        // Calculate and mint the amount of xKIKA the KIKA is worth. The ratio will change overtime, as xKIKA is burned/minted and KIKA deposited + gained from fees / withdrawn.
        else {
            uint256 what = _amount.mul(totalShares).div(totalKika);
            _mint(msg.sender, what);
        }
        // Lock the KIKA in the contract
        kika.transferFrom(msg.sender, address(this), _amount);
    }

    // Leave the bar. Claim back your KIKAs.
    // Unlocks the staked + gained KIKA and burns xKIKA
    function leave(uint256 _share) public {
        require(_isEnableDayOfMonth() || leaveEnable, "Can not leave now");

        // Gets the amount of xKIKA in existence
        uint256 totalShares = totalSupply();
        // Calculates the amount of KIKA the xKIKA is worth
        uint256 what = _share.mul(kika.balanceOf(address(this))).div(totalShares);
        _burn(msg.sender, _share);
        kika.transfer(msg.sender, what);
    }

    // get ratio of xKIKA to KIKA (decimal is 6)
    function getRatio() public view returns (uint256 ratio) {
        ...
        if (totalShares == 0) {
            ...
            // Calculates the amount of KIKA one xKIKA is worth
            ratio = kika.balanceOf(address(this)).mul(1e6).div(totalShares);
        }
    }

    function _isEnableDayOfMonth() public view returns (bool) {
    ...
    function setEnterEnable(bool _enterEnable) external onlyAdmin {
        enterEnable = _enterEnable;
    }

    // Set leave the bar status
    function setLeaveEnable(bool _leaveEnable) external onlyAdmin {
        leaveEnable = _leaveEnable;
    }

    // Set the enable Days of month
    function setEnableDayOfMonth(uint8 _startDayOfMonth, uint8 _endDayOfMonth) external onlyAdmin {
