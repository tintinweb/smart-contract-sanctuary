...
contract PixelMarketplace is IERC1155Receiver, ReentrancyGuard, AccessControl, Ownable {
  ...
  constructor(IERC20 pixlTokenAddress) {
  ...
  */
  function createMarketListing(
    ...
    require(price > 0, "Price must be at least 1 $PIXL");

    tokenIdToPixlPrice[tokenId] = price;
  }

  /* 
  * Creates the sale of a PixelverseItem from the "seller" to the msg.sender.
  * Transfers ownership of one instance of an ERC1155 with a given id, 
  ...
  */
  function createMarketSale(uint256 tokenId) public payable nonReentrant {
    ...
    uint price = tokenIdToPixlPrice[tokenId];
    ...
    pixelverseItem.safeTransferFrom(address(this), msg.sender, tokenId, 1, "0x0");
  }
  ...
  */
  ...
  function withdrawItems(uint256 _tokenId, uint256 _amount) public onlyOwner {
    uint256 numItems = pixelverseItem.balanceOf(address(this), _tokenId);
    require(numItems >= _amount, "Insufficient funds: not enough unsold Items");
  ...
  function withdrawAllItems(uint256 _tokenId) public onlyOwner {
    uint256 numItems = pixelverseItem.balanceOf(address(this), _tokenId);
    require(numItems > 0, "No unsold Items left on the contract!");
    pixelverseItem.safeTransferFrom(address(this), msg.sender, _tokenId, numItems, "0x0");
  }
