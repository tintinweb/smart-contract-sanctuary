...
contract Bank is OwnableUpgradeable, IBank {
    ...
    function getPendingReward(address stakingToken, address user)
        public
        view
        returns (uint256)
    {
        return IStakingRewards(stakingRewards[stakingToken]).earned(user);
    }

    function getPendingRewards(address[] calldata stakingTokens, address user)
        external
        view
        returns (uint256 reward)
    {
        for (uint256 i = 0; i < stakingTokens.length; i += 1) {
            reward += getPendingReward(stakingTokens[i], user);
        }
    }
