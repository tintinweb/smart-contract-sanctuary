...
contract MSBreed is BreedProxyAdmin {
    ...
    bytes32 public constant fertileString = keccak256("fertile");
    ...
    function checkBreed(
        ...
        require(
            ...
            "MSBreed: serverAddress is not set"
        );
        require(matronId != 0, "MSBreed: matronId is equal to 0");
        require(sireId != 0, "MSBreed: sireId is equal to 0");
        bytes32 serverHash = keccak256(abi.encode(_nftContract, matronId, sireId, fertileString))
    ...
    function breed(
        ...
        checkBreed(_nftContract, matronId, sireId, serverSig);
        ...
        internal
    {
        ...
        for (uint256 i = 0; i < charges[_nftContract].length; i++) {
            if (charges[_nftContract][i].token == address(0)) {
                ...
                IERC20Upgradeable(charges[_nftContract][i].token)
                    .safeTransferFrom(
                        _msgSender(),
                        address(this),
                        charges[_nftContract][i].price * tokenQuantity
