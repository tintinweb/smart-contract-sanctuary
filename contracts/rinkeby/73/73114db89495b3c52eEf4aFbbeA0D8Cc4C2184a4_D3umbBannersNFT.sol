...
contract D3umbBannersNFT is ERC721, Ownable {
    
    using SafeMath for uint256;

    uint256 public constant MAX_NFTS = 2999;  //3000 when including tokenID = 0
    uint256 public constant NFTCost = 50000000000000000; // 0.05 ETH
    uint256 public constant NFTCost3_Discount = 45000000000000000; // 0.045 ETH (if ordering 3 or 4 banners)
    uint256 public constant NFTCost5_Discount = 42000000000000000; // 0.043 ETH (if ordering 5, 6, 7, 8 or 9 banners)
    uint256 public constant NFTCost10_Discount = 40000000000000000; // 0.040 ETH (if ordering 10 banners)
    uint public constant maxNFTPurchase = 10;
    
    uint256 public constant MAX_PRESALE = 99; //Tokens of other contract
    ...
    address MaestroPupsAddress = 0x6108Ab04fC2F587B4146eaa4A2C4A281731F55D6; //0x874Dd3F2317bEaBb4f069b42B539010c54B195eC;
    ...
    constructor() ERC721("DumbBanners", "DUM") {  
...
*/
    ...
    function AllPreSaleTokensClaimed() external view returns(uint256[] memory) {
        uint256[] memory result = new uint256[](PRESALE_SUPPLY);
        for (uint256 i = 0; i < nftPreSaleSupply.length; i++) {
            result[i] = nftPreSaleSupply[i];
        }
        return result;
    }
