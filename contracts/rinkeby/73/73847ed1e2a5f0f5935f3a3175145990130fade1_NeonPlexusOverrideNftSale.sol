...
// File contracts/NeonPlexusOverrideNftSale.sol
...
interface INeonPlexusOverrideNftSale { 
  function MAX_SUPPLY() external returns (uint256);
  function safeMint(address to, uint256 tokenId) external;
  function owner() external returns (address);
}

contract NeonPlexusOverrideNftSale is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public amountForTreasury = 50;
    ...
    constructor(address pNftContract, 
        ...
        uint256 pAmountForSale) Ownable() ReentrancyGuard() {
        ...
        // Adding to balance of dev team so they can mint for treasury portion whenever.
        balance[pOwner] += amountForTreasury;
        // Transfer to owner of NFT instead of deployer (multisig set up)
        transferOwnership(pOwner);
    }
    ...
    function setAmountForSale(uint256 _amountForSale) public onlyOwner {
        require(_amountForSale >= 0, "should be >= 0");
    ...
    function gift(address account, uint256 count) public onlyOwner {
        require(count > 0, "Cannot gift 0");
    ...
    function resetStartBlock() public onlyOwner {
        startBlock = 0;
    }
    ...
    function buy(uint256 count) external payable nonReentrant {
        ...
        // Lock this address for the phase is wave 0 or 1
        if (wave < 2) {
    ...
    function getWave() public view returns (uint256) {
        uint256 blocksSinceStart = startBlock == 0 ?  0 : block.number - startBlock;
        return blocksSinceStart/waveBlockLength;
    }

    function tryStartSale() internal {
    ...
    function _mint(address account, uint256 count) internal {
        ...
        for (uint256 i = 0; i < count; i++) {
            INeonPlexusOverrideNftSale(nftContract).safeMint(account, currentSupply + i);
        }
