...
contract BunnyMembership is ERC721, ERC721Enumerable, ERC721URIStorage, AccessControl {
    ...
    uint256 public constant PRICE_MULTIPLIER = 0.00001 ether;                                   // this is used for limiting prices 
    ...
    mapping(address => uint8) private _discountType;                                            // discount type
    ...
    constructor() ERC721("BunnyMembership", "BMT") {
        ...
        configureMembership( 0, 555, 30, 20 );
        configureMembership( 1, 2555, 100, 5 );
    ...
    function _configureDiscountTable( uint _mType, uint _mp0,  uint _mp1,  uint _mp2,  uint _mp3,  uint _mp4 ) private onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        require( _mType < MAX_MEMBERSHIP_TYPES,     "Invalid membership type" );
        require( _discountTable[_mType][0] == 0,    "Discount table is configured Already" );
    ...
    function addToWhitelist( address _user, uint8 _dType ) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function addToWhitelistArray(address[] memory _users) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function _mintToken( uint _mType, address _to, string memory _uri ) private {
    ...
    //function mintMembership( uint _mType, uint _quanity, address _to, string memory _uri ) public onlyRole(MINTER_ROLE) {
    // removed the fact the only minter role can call this TODO describe new way

    function mintMembership( uint _mType, uint _quanity, address _to, string memory _uri ) public payable {

        // is the membership type available for sale
        require( isAvailableForSale(_mType), "M1: not available for sale" );
        ...
        if ( _premintIsOn )
            require( isOnWhitelist(_to), "M2: not on whitelist" );

        // trying to mint zero tokens
        require( _quanity != 0, "M3: mint zero tokens" );

        // make sure not trying to mint too many
        require( _quanity <= howManyCanMint(_to), "M4: mint too many" );
        ...
        require( _forSaleSupply[_mType] >= _quanity, "M4: mint fewer" );

        // did they give us enough money
        //

        // Mint it!
        _mintToken( _mType, _to, _uri );
    }
