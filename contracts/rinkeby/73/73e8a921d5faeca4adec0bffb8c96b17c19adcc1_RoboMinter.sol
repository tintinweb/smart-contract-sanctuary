...
// File: contracts/NFT/RoboMinter.sol
...
contract RoboMinter is Ownable, ERC721 {
    ...
    event SharesChanged(uint8 _value1);
    ...
    event RoboNFTMinted(address indexed _user, uint256 indexed _tokenId, string _tokenURI);
    event ReserveRoboNFTs(uint256 _numberOfTokens);
    ...
    constructor(
        ...
        emit SharesChanged(wallet1Share);
    ...
    function changeSupplyLimit(uint256 _supplyLimit) external onlyOwner {
        require(_supplyLimit >= totalSupply, "Value should be greater currently minted RoboNFTs.");
    ...
    function buyRoboNFTs(uint _numberOfTokens) external payable {
        ...
        _mintRoboNFTs(_numberOfTokens);
    }

    function _mintRoboNFTs(uint _numberOfTokens) internal {
        require(totalSupply.add(_numberOfTokens) <= supplyLimit, "Not enough tokens left.");

        uint256 newId = totalSupply;
        for(uint i = 0; i < _numberOfTokens; i++) {
            _safeMint(msg.sender, newId);
            emit RoboNFTMinted(msg.sender, newId, tokenURI(newId));
    ...
    function reserveRoboNFT(uint256 _numberOfTokens) external onlyOwner {
        _mintRoboNFTs(_numberOfTokens);
        emit ReserveRoboNFTs(_numberOfTokens);
    }
    ...
    */
     ...
     * param _namingPrice: The price for naming your RoboNFT
    */
    ...
    */
    function withdrawAll() external {
        require(msg.sender == wallet1Address, "Only share holders can call this method.");
        _withdraw();
    }
}