...
contract Pizza is ERC721Enumerable, Ownable {
    ...
    mapping(address => uint256) public _mintLimit;

    //uint256s
    uint256 MAX_SUPPLY = 5555;
    uint256 SEED_NONCE = 0;
    uint256 RESERVED = 100;
     ...
     */
    function mintInternal(uint256 _quantity) internal {
    ...
    function bakePizza() public payable {
        ...
        require(totalSupply().add(1) <= MAX_SUPPLY - RESERVED, "Public Sale Exceeded");
        require(_mintLimit[_msgSender()] <= 2,"Max mints per wallet = 3");
        // if (totalSupply() < MINTS_PER_TIER) return mintInternal();
        // return mintInternal();
        
      _mintLimit[_msgSender()] = _mintLimit[_msgSender()].add(1);
            return mintInternal(1);
    }
    
     /**
     * @dev Mints new tokens ownerOnly.
     */
    function bakeReservedPizza(uint256 _quantity) public onlyOwner {
            return mintInternal(_quantity);
    }
   ...
   /**
        ...
        returns (string memory)
    {
        ...
        if(Oven.compareStrings(traitTypes[2][Oven.parseInt(Oven.substring(_hash, 2, 3))].traitName, "Thick")){  
             crustColor = Oven.substring(traitTypes[3][Oven.parseInt(Oven.substring(_hash, 3, 4))].pixels, 14, 16);
