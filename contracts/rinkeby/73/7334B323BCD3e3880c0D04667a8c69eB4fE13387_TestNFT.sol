...
contract TestNFT is ERC721, ERC165Storage, IERC2981, Ownable {
  ...
  event MinterUpdated(address indexed minter, bool indexed canMint);

  constructor(address[] memory _minters) ERC721("Test NFT", "TEST") {
  ...
  function setMinter(address minter, bool canMint) external {
    ...
    emit MinterUpdated(minter, canMint);
  }

  function mint(address creator, address to, string memory tokenUri) external {
    require(minters[msg.sender] == true || msg.sender == owner(), "unauthorized minter");
    _tokenId++;
