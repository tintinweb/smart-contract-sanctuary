...
contract Exchange is Ownable {
    ...
    event Settlement(
        bytes buyOrderId,
        uint256 buyMakerAmount,
        uint256 buyTakerAmount,
        bytes sellOrderId
    );
    ...
    function placeBuyOrder(
        ...
        require(
            ERC20(makerToken).balanceOf(msg.sender) >= makerAmount,
            "placeBuyOrder: EX11"
        );
        require(
            ERC20(makerToken).allowance(msg.sender, address(this)) >=
                makerAmount,
            "placeBuyOrder: EX12"
        );
    ...
    function batchSettlements(
        bytes[] memory buyOrderIds,
        uint256[] memory buyMakerAmounts,
        uint256[] memory buyTakerAmounts,
        bytes[] memory sellOrderIds
    ) public {
        require(
            buyOrderIds.length == sellOrderIds.length &&
                sellOrderIds.length == buyMakerAmounts.length &&
                buyMakerAmounts.length == buyTakerAmounts.length &&
                buyOrderIds.length != 0,
            "batchSettlements:EX5"
        );
        for (uint16 i = 0; i < buyOrderIds.length; i++) {
            settlement(
                buyOrderIds[i],
                buyMakerAmounts[i],
                buyTakerAmounts[i],
    ...
    function settlement(
        bytes memory buyOrderId,
        uint256 buyMakerAmount,
        uint256 buyTakerAmount,
        bytes memory sellOrderId
    ) public {
        Order memory buyOrder = orderMap[buyOrderId];
        Order memory sellOrder = orderMap[sellOrderId];

        require(
            buyOrder.maker != address(0) && sellOrder.maker != address(0),
            "settlement:EX1"
        );
        require(
            buyOrder.status == OrderStatus.Init ||
                buyOrder.status == OrderStatus.PartFilled,
            "settlement buy: EX9"
        );
        require(
            sellOrder.status == OrderStatus.Init ||
                sellOrder.status == OrderStatus.PartFilled,
            "settlement sell: EX9"
        );

        require(
            now < sellOrder.expiry && now < sellOrder.expiry,
            "settlement:EX8"
        );
        require(
        ...
        uint256 buyFillableMakerAmount = SafeMath.sub(
            buyOrder.makerAmount,
            buyOrder.FilledMakerAmount
        );
        uint256 buyFillableTakerAmount = SafeMath.sub(
            buyOrder.takerAmount,
            buyOrder.FilledTakerAmount
        );

        uint256 sellFillableMakerAmount = SafeMath.sub(
            sellOrder.makerAmount,
            sellOrder.FilledMakerAmount
        );
        uint256 sellFillableTakerAmount = SafeMath.sub(
            sellOrder.takerAmount,
            sellOrder.FilledTakerAmount
        );

        require(
            buyMakerAmount <= buyFillableMakerAmount &&
                buyTakerAmount <= buyFillableTakerAmount,
            "settlement:EX14"
        );

        require(
            buyMakerAmount <= sellFillableTakerAmount &&
                buyTakerAmount <= sellFillableMakerAmount,
            "settlement:EX15"
        );

        if (sellOrder.orderType == 0) {
            // place sell
            userToken[sellOrder.maker][sellOrder.makerToken] = SafeMath.sub(
                userToken[sellOrder.maker][sellOrder.makerToken],
                buyTakerAmount
            );
        }
        ...
        if (buyOrder.takerAmount > buyOrder.FilledTakerAmount) {
            buyOrder.status = OrderStatus.PartFilled;
        } else {
            buyOrder.status = OrderStatus.AllFilled;
        }

        if (sellOrder.makerAmount > sellOrder.FilledMakerAmount) {
            sellOrder.status = OrderStatus.PartFilled;
        } else {
            sellOrder.status = OrderStatus.AllFilled;
        }
        ...
        // reback buyer remaing assert
        if(buyOrder.FilledTakerAmount == buyOrder.takerAmount && buyOrder.FilledMakerAmount < buyOrder.makerAmount){
            uint256 rebackMakerAmount = buyOrder.makerAmount - buyOrder.FilledMakerAmount;
            ERC20(buyOrder.makerToken).transfer(buyOrder.maker, rebackMakerAmount);
        }

        orderMap[buyOrderId] = buyOrder;
        orderMap[sellOrderId] = sellOrder;
        emit Settlement(
            buyOrderId,
            buyMakerAmount,
            buyTakerAmount,
    ...
    function _place(
        ...
        userToken[seller][makerToken] = SafeMath.add(
            userToken[seller][makerToken],
            makerAmount
        );
        ...
        returns (uint256 takerTokenFilledAmount, uint256 makerTokenFilledAmount)
    {
        ...
        uint256 fillableTakerAmount = SafeMath.sub(
            order.takerAmount,
            order.FilledTakerAmount
        );
        require(fillableTakerAmount >= fillTakerAmount, "takerOrder: EX4");
        uint256 fillMakerAmount = SafeMath
            .mul(order.makerAmount, fillTakerAmount)
            .div(order.takerAmount);

        require(
            ERC20(takerTokenAdr).balanceOf(msg.sender) >= fillTakerAmount,
            "takerOrder: EX2"
        );
        require(
        ...
        require(
            ERC20(takerTokenAdr).allowance(msg.sender, address(this)) >=
                fillTakerAmount,
            "takerOrder: EX3"
        );
        // should safe math
        ...
        ERC20(order.takerToken).transferFrom(
            msg.sender,
            order.maker,
            fillTakerAmount
        );
    ...
    function batchTakeOrders(
        bytes[] memory orderIds,
        uint256[] memory takerAmounts
    )
        public
        returns (
            uint256[] memory takerTokenFilledAmounts,
            uint256[] memory makerTokenFilledAmounts
        )
    {
        require(
            orderIds.length == takerAmounts.length && orderIds.length != 0,
            "EX5"
        );
        ...
        for (uint16 i = 0; i < orderIds.length; i++) {
            ...
            ) = takeOrder(orderIds[i], takerAmounts[i]);
     ...
     */
    function cancelOrder(bytes memory orderId, uint256 vaultId)
        external
        returns (bool)
    {
        Order memory order = orderMap[orderId];
        ...
        uint256 remainingMakerAmount = SafeMath.sub(
            order.makerAmount,
            ...
            if (Token(order.makerToken).isPut()) {
                collateralAmount = SafeMath
                    ...
                    .mul(
                        10 **
                            uint256(
                                ERC20(Token(order.makerToken).collateralAsset())
                                    .decimals()
                            )
                    )
     ...
     */
    function settleCollateral(address maker, address token)
        external
        view
        returns (uint256)
    {
     ...
     */
    function getRemaingTakerAmount(bytes memory orderId)
        public
        view
        returns (uint256)
    {
    ...
    function redeem(
        ...
        require(
            sellerToken[msg.sender][oToken] >= amount,
            "sellerToken balacnce not enough"
        );
        require(
            ERC20(oToken).balanceOf(msg.sender) >= amount,
            "oToken balance of not enough"
        );
        ...
        if (Token(oToken).isPut()) {
            collateralAmount = SafeMath
                ...
                .mul(
                    10 **
                        uint256(
                            ERC20(Token(oToken).collateralAsset()).decimals()
                        )
                )
        ...
        sellerToken[msg.sender][oToken] = SafeMath.sub(
            sellerToken[msg.sender][oToken],
            amount
        );
    ...
    function manualClaim(address oToken) public {
        uint256 sellAmount = userToken[msg.sender][oToken];
    ...
    function calcOrderId(address addr, uint64 x)
        public
        pure
