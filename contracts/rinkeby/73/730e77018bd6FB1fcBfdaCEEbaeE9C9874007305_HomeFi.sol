 ...
 */
contract HomeFi is IHomeFi, ERC721, Initializable {
        ...
        returns (uint256)
    {
        ...
        eventsInstance.nftCreated(_tokenIds, _to);
        return _tokenIds;
    }
    ...
    function setAddr(
        ...
        wrappedToken[ETHER_CURRENCY] = _rETHAddress;
        wrappedToken[DAI_CURRENCY] = _rDaiAddress;
        wrappedToken[USDC_CURRENCY] = _rUSDCAddress;
        addrSet = true;
    }

    function validCurrency(address _currency) public pure override {
        require(
            _currency == ETHER_CURRENCY ||
                _currency == DAI_CURRENCY ||
                _currency == USDC_CURRENCY,
            "invalid currency"
        );
    }
        ...
        nonReentrant()
    {
        ...
        address _project =
            projectFactoryInstance.createProject(_hash, _currency, msgSender());
        projectExist[_project] = true;
        projectSerial = projectSerial += 1;
        projects[projectSerial] = _project;
        uint256 _id = mintNFT(msgSender(), string(_hash));
        projectTokenId[_project] = _id;
        eventsInstance.projectAdded(projectSerial, _project, msgSender());
