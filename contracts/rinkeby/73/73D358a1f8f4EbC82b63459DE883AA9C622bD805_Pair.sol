...
contract Pair is IPair, PairERC20, ReentrancyGuard {
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address to, bytes calldata data) external nonReentrant onlyRouter override {
        ...
        if (amount0Out > 0) {
            balance0 = _transferFeeToAdmin(token0, balance0);
        } else {
            balance1 = _transferFeeToAdmin(token1, balance1);
        }
    ...
    function _transferFeeToAdmin(address tokenFee, uint256 balance) private returns (uint256) {
        (address feeTo, uint256 fee, uint256 precision) = IPairFactory(factory).getInfoAdminFee();
        if (feeTo == address(0)) {
            return balance;
        }
        uint256 feeAmount = balance * precision / fee;
        TransferHelper.safeTransfer(tokenFee, feeTo, balance - feeAmount);
