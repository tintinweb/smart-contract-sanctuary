...
contract CombatALIX is Ownable {
    ...
    uint256[5] public rewardBonus = [50, 50, 50, 50, 50];
    uint256[4] public baseRewardMonster = [200, 300, 400, 500];
    bool public paused;
    ...
    uint256 public timeLimitClaim = 5 days;
    uint256 public feeRateEarly = 20;
    ...
    function setFeeRate(uint256 _feeRate) public onlyOwner {
        feeRateEarly = _feeRate;
    }

    function setTimeLimitClaim(uint256 _time) public onlyOwner {
        timeLimitClaim = _time;
    }

    function setRewardBonus(uint256[5] memory _rewardBonus) public onlyOwner {
    ...
    function setBaseReward(uint256[4] memory _baseRewardMonster)
        public
        onlyOwner
    {
    ...
    function combatReward(
        ...
        uint256 bonusPercentage = rewardBonus[_alinxLevel - 1];
        ...
        returns (uint256)
    {
        ...
        uint256 limit = limitCombat[_alinxLevel - 1];
    ...
    function combat(uint256 _tokenId, Monster _monster) external {
        ...
        uint8 alinxLevel = uint8(alinxNFT.getAlix(_tokenId).level);
        ...
        if (rnd < winRate) {
            ...
            reward = combatReward(winRate, alinxLevel, monsterLevel);
            // safeReward(_msgSender(), reward);
            rewards[_msgSender()] = rewards[_msgSender()].add(reward);
        }
        ...
        generateRandom();
        emit CombatEvent(_tokenId, _monster, _msgSender(), result, reward);
    }
    ...
    function claimEarly() public {
        uint256 reward = rewards[_msgSender()];
        require(reward > 0, "nothing to claim");
        uint256 earlyFee;
        if (
            userClaimedTime[_msgSender()].add(timeLimitClaim) > block.timestamp
        ) {
            earlyFee = reward.mul(feeRateEarly).div(100);
        }
        if (earlyFee > 0) {
            safeReward(dev, earlyFee);
        }

        safeReward(_msgSender(), reward.sub(earlyFee));
        userClaimedTime[_msgSender()] = block.timestamp;
