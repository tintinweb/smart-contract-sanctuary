...
contract KeepWatchCrew is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    using SafeMath for uint256;
    using Strings for uint256;

    enum SaleModes { NONE, VIP_WHITELIST, PUBLIC_SALE }
    ...
    uint256 public constant MAX_ELEMENTS = 69;
    uint256 public PRICE = 69 * (10**15);
    uint256 public DISCOUNT_PRICE = 59 * (10**15);
    uint256 public maxVIPMint = 1;
    uint256 public maxWhitelistMint = 4;
    uint256 public maxPublicMint = 8;

    uint256 public publicSaleDate = 1639260000;

    address public constant payoutAddress =
        0x434dE59168ad5AE1D2155521eF69c8bAE1F01078;
    ...
    function mint(uint256 _count) public payable {
        ...
        bool isInWhitelist = isWhitelisted(sender);
        bool isInVIP = isVIP(sender);

        if (saleMode == SaleModes.VIP_WHITELIST) {
            if (sender != owner()) {
                require(isInVIP || isInWhitelist);

                if (isInVIP) {
                    if (mintedAmount == 0) {
                        require(_count == maxVIPMint, "You can't mint more than allowed number of tokens");
                    } else {
                        require(isInWhitelist, "You are not allowed to mint");
                        uint256 maxMint = maxVIPMint + maxWhitelistMint;
                    ...
                    require(_count + mintedAmount <= maxWhitelistMint, "You can't mint more than allowed number of tokens");
                    require(msg.value >= DISCOUNT_PRICE.mul(_count), "Value below price");
                }
            }
        }
    ...
    // Change Mode
    function activateNextSaleMode() public onlyOwner {
        if (saleMode == SaleModes.NONE) {
            saleMode = SaleModes.VIP_WHITELIST;
        } else if (saleMode == SaleModes.VIP_WHITELIST) {
            saleMode = SaleModes.PUBLIC_SALE;
        }
    }

    // Whitelist general users
    ...
    // Whitelist VIPs
    function whitelistVIPs(address[] calldata _users) public onlyOwner {
        delete vipAddresses;
        vipAddresses = _users;
    }
    ...
    function updateMaxWhitelistAmount(uint256 _amount) public onlyOwner {
        maxWhitelistMint = _amount;
    }

    function updateMaxPublicAmount(uint256 _amount) public onlyOwner {
