...
contract Dop {
    ...
    /* Restriction variables - START */

    address private _owner;
    bool private _paused = true;
    mapping(address => bool) private _whitelist;
    mapping(address => uint256) private _lastTx;
    uint256 private _maxTxAmount = 20000 * 10**18; // 20k DOP
    uint256 private _maxTrade = 200000 * 10**18; // 200k DOP
    uint256 private _maxTxTime = 30 seconds; // 30 seconds

    /* Restriction variables - END */
    ...
    function _transferTokens(
        ...
        address dst,
        uint96 amount
    ) internal notRestricted(src, dst, amount) {
        require(
            src != address(0),
     ...
     */
    function whitelist(address account) internal {
        require(account != address(0), "Whitelist: Invalid account");
        _whitelist[account] = true;
    }
    function whitelists(address[] memory accounts) external onlyOwner() {
        require(accounts.length <= 256, "Whitelist: Exceed length");
        for (uint256 i = 0; i < accounts.length; i++) {
            whitelist(accounts[i]);
        }
    }
    function unWhitelist(address account) external onlyOwner() {
        require(account != address(0), "Whitelist: Invalid account");
        require(_whitelist[account], "Whitelist: Not in whitelist");
        _whitelist[account] = false;
    }

    /**
     * @dev Enable restriction.
     */
    ...
    function setLimit(uint256 limit) external onlyOwner() {
        require(limit < _maxTrade, "Limit: exceed max trade amount");
        _maxTxAmount = limit;
    }

    /**
     * @dev Throws if any restrictions.
     */
    modifier notRestricted(
        ...
        if (_paused) {
            if (_whitelist[sender]) {
                if (!_whitelist[receipient]) {
                    require(amount < _maxTxAmount, "Restrict: exceed maximum amount");
                    _lastTx[receipient] = now + _maxTxTime;
                }
            } else {
                require(amount < _maxTxAmount, "Restrict: exceed maximum amount");
                require(_lastTx[sender] <= now, "Restriction: too much");
                _lastTx[sender] = now + _maxTxTime;
                if (!_whitelist[receipient]) {
                    require(_lastTx[receipient] <= now, "Restriction: too much");
                    _lastTx[receipient] = now + _maxTxTime;
                }
            }
        }
