...
contract FarmV2 is AccessControl{
    ...
    function rewardedStones(address staker) public view returns (uint256) {
        ...
        // solium-disable-next-line security/no-block-members
        uint256 _seconds = block.timestamp.sub(stakers[staker].timestamp).div(86400);

        return stakers[staker].stones.add(stakers[staker].amount.div(1e18).mul(_seconds).mul(1000));
    }
    ...
    function deposit(uint256 amount ) public {
        ...
        uint calc = oldAmount.div(11574074074074000);
        oldAmount = calc.div(86400);
