...
contract Spottie is Ownable, ERC165, ICreatorExtensionTokenURI {
    ...
    constructor(address creator, uint256 max_supply, uint256 saleStart, string memory unrevealedURI) {
        _creator = creator;
    ...
    function tokenURI(address creator, uint256 tokenId) external view override returns (string memory) {
        ...
        return bytes(_baseURIextended).length == 0 ?
            _unrevealedURI :
            string(abi.encodePacked(_baseURIextended, _tokenEdition[tokenId]));
    }
    ...
    */
    ...
    function mintTokens(uint numberOfTokens) public payable {
        ...
        if (startingIndexBlock == 0 && (_numMinted == MAX_SUPPLY || block.timestamp >= REVEAL_TIMESTAMP)) {
