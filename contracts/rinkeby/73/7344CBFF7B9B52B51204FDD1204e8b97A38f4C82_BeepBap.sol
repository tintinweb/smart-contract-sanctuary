...
contract BeepBap is ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    Counters.Counter private _tokenCounter; // 5492 set in libraby

    uint256 public maxSupply = 10000;
    uint256 public ccId;

    // address public wallet = ; // mainnet
    // address public wallet = ; // rinkeby
    address public wallet = 0x70997970C51812dc3A010C7d01b50e0d17dc79C8; // local
    ...
    constructor() ERC721("BeepBap_NAME", "BeepBap") {
        _tokenCounter.setValue();
    }
    ...
    function buy(uint256 tokenQuantity) external payable {
        ...
        require(_tokenCounter.current() + tokenQuantity <= maxSupply, "project sold out");
        require(collection[ccId].privateAmountMinted + collection[ccId].publicAmountMinted + tokenQuantity <= collection[ccId].collectionSupply, "collection sold out");
        require(collection[ccId].publicAmountMinted + tokenQuantity <= collection[ccId].publicSupply, "exceeded public supply");
        require(tokenQuantity <= collection[ccId].maxMint, "exceeded max limit per mint");
        require(collection[ccId].price * tokenQuantity <= msg.value, "insufficient eth");
    ...
    function presaleBuy(uint256 tokenQuantity) external payable {
        ...
        require(collection[ccId].presalerList[msg.sender], "address not in presale");
        require(collection[ccId].privateAmountMinted + tokenQuantity <= collection[ccId].preSaleSupply, "exceeded presale supply");
        require(collection[ccId].presalerListPurchases[msg.sender] + tokenQuantity <= collection[ccId].preSaleMaxMint, "exceeded presale mint allocation");
        require(collection[ccId].price * tokenQuantity <= msg.value, "insufficient eth");
    ...
    //  this function will allow you to overwrite the current collection in case a error exists in the collection arguments
    //  use with caution
    ...
    function getTokenCounter() external view returns (uint256 tokenCount) {
        tokenCount = _tokenCounter.current();
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721, ERC721URIStorage) returns (string memory) {
        ...
        for (uint256 i = 1; i <= ccId; i++) {
            if (tokenId >= collection[i].startRange && tokenId <= collection[i].endRange) {
                return string(abi.encodePacked(collection[i].URIPath, tokenId.toString()));
            }
        }
    }
...
library Counters {
    ...
    function setValue(Counter storage counter) internal {
        counter._value = 5492;
    }
}
