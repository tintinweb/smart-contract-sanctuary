...
contract ParcelMinter is Ownable, ContextMixin {
    mapping(uint256 => address) public bookedParcels;
    ...
    function whitelistLands(uint256[] calldata tokenIds, address[] calldata addr) external onlyOwner {
        require(tokenIds.length == addr.length, "Wrong data");
        for (uint256 i = 0; i < addr.length; i++) {
            bookedParcels[tokenIds[i]] = addr[i];
        }
    }
    ...
    function redeemParcels(uint256 tokenId) public {
        ...
        require(bookedParcels[tokenId] == minter, "Not whitelisted");
    ...
    function batchRedeemParcels(uint256[] calldata tokenIds) public {
        uint256 tokenId;
        address minter = msg.sender;
