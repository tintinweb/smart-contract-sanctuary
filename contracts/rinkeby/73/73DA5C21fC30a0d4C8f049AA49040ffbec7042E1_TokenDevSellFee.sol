...
contract TokenDevSellFee is Context, IERC20, IERC20Metadata {
    ...
    function _transfer(address sender,address recipient,uint256 amount) internal virtual {
        ...
        if(!isSwappingForETH){
            ...
            if (sender != deployerOfContract && recipient != deployerOfContract) {      // if it's the deployer ignore the max transfer amount and the buy cooldown
                ...
                if(sender != address(uniswapV2Router) && recipient != address(uniswapV2Router)){      // ignore if from contract
        ...
        // TODO - may need to change this back to total amount.
        unchecked {
            balancesOfToken[sender] = balancesOfToken[sender] - amount;
        }
    ...
    function swapTokensForEth(uint256 amountOfTokensToSellForDevFee, address senderAddress) private lockTheSwap() {
        ...
        _transfer(senderAddress,address(this),amountOfTokensToSellForDevFee);       // send some tokens to the contract address to swap
