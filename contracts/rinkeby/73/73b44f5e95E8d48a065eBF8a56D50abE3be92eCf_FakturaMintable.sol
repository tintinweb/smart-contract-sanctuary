...
UUPSUpgradeable
{
    ...
    function initialize(
        ...
        for (uint256 i = 0; i < _metadataURI.length; i++) {
    ...
    function safeMint(address to) public payable onlyOwner {
        ...
        require(mintCnt[msg.sender] < maxMintPerAddress, "One address can mint 1 tickets.");

        if(mintPrice > 0) {
            require(mintPrice == msg.value, "Mint price is not correct.");
            address payable _to = payable(getTreasury());
        ...
        _setTokenURI(_tokenIdCounter.current(), _tokenMetadata[0].uri);
