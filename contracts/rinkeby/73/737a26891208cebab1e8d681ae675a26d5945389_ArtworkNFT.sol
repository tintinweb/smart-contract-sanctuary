...
contract ArtworkNFT is ERC721Pausable, AccessControl, Ownable {
    ...
    constructor(
      ...
      function mint(address to, string memory _tokenURI) public payable returns (uint256 tokenId) {
        ...
        //erc.safeTransfer(mintFeeAddr, mintFeeAmount);
            erc.transferFrom(msg.sender,mintFeeAddr,mintFeeAmount);

        tokenId = nextTokenId;
        _mint(to, tokenId);
        nextTokenId++;
        _setTokenURI(tokenId, _tokenURI);
        if (msg.value > mintFeeAmount) erc.transferFrom(msg.sender ,msg.sender, msg.value - mintFeeAmount);
    }
