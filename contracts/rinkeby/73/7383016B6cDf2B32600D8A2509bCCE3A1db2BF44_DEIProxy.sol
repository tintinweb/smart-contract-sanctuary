...
contract DEIProxy is Ownable {
	...
	uint public collateral_missing_decimals_d18 = 1; // missing decimal of collateral token (avax=1 (DAI), heco=1e10 (HUSD))
	...
	uint public while_times = 3; // set error rate in getAmountsInCollateralToDeus
	...
	function setWhileTimes(uint _while_times) external onlyOwner {
		while_times = _while_times;

		emit WhileTimesSet(_while_times);
	}
	...
	function nativeCoinToDei(
		...
		if(collateral_ratio == 1000000) {
			IDEIPool(pool_collateral).mint1t1DEI(collateral_amount, proxy_input.collateral_price, proxy_input.expire_block, proxy_input.sigs);
			...
			IDEIPool(pool_collateral).mintAlgorithmicDEI(deus_amount, proxy_input.deus_price, proxy_input.expire_block, proxy_input.sigs);
		} else {
			uint collateral_to_deus = getAmountsInCollateralToDeus(collateral_ratio, collateral_amount, proxy_input.deus_price);
			...
			IDEIPool(pool_collateral).mintFractionalDEI(
		...
		{
			uint residual_collateral_amount = IERC20(collateral_address).balanceOf(address(this));
			if (proxy_input.transferResidual && residual_collateral_amount > 0) IERC20(collateral_address).transfer(msg.sender, residual_collateral_amount);
		}
		
		uint dei_amount = IERC20(dei_address).balanceOf(address(this));
		require(dei_amount > proxy_input.min_amount_out, "DEIProxy::collateralToDei: INSUFFICIENT_OUTPUT_AMOUNT");
		IERC20(dei_address).transfer(msg.sender, dei_amount);
	
		emit Swap(address(0), msg.value, dei_amount, proxy_input.deus_price, collateral_ratio);
	...
	function ERC20ToDei(
		...
		require(proxy_input.path.length >= 2, "DEIProxy::Wrong path");
		...
		emit Swap(proxy_input.path[0], erc20amount, dei_amount, proxy_input.deus_price, collateral_ratio);

		return dei_amount;
	}

	function collateralToDei(
		ProxyInput memory proxy_input,
		...
		emit Swap(collateral_address, collateral_amount, dei_amount, proxy_input.deus_price, collateral_ratio);
	...
	function DeusToDei(
		...
		if(collateral_ratio == 1000000) {
			...
			uint deus_to_collateral = getAmountsInDeusToCollateral(collateral_ratio, deus_amount, proxy_input.deus_price);
		...
		{
			uint residual_collateral_amount = IERC20(deus_address).balanceOf(address(this));
			if (proxy_input.transferResidual && residual_collateral_amount > 0) IERC20(collateral_address).transfer(msg.sender, residual_collateral_amount);
		}
		
		uint dei_amount = IERC20(dei_address).balanceOf(address(this));
		require(dei_amount > proxy_input.min_amount_out, "DEIProxy::DeusToDei: INSUFFICIENT_OUTPUT_AMOUNT");
		IERC20(dei_address).transfer(msg.sender, dei_amount);
	
		emit Swap(deus_address, deus_amount, dei_amount, proxy_input.deus_price, collateral_ratio);
	}


	/* ========== VIEWS ========== */

	function getAmountsInDeusToCollateral(uint collateral_ratio, uint deus_amount, uint deus_price) public view returns(uint) {
		uint deus_to_callateral;
		uint times = while_times;
		while(times > 0) {
			uint deus_for_swap = deus_amount * collateral_ratio / scale;
			...
			uint collateral_amount = amounts_out[amounts_out.length - 1];
			uint deus_needed = (collateral_amount * (scale - collateral_ratio) / deus_price);
	...
	function getAmountsOutDeusToDei(uint deus_amount, uint deus_price) public view returns(uint) {
		uint collateral_ratio = IDEIStablecoin(dei_address).global_collateral_ratio();
		uint dei_amount;
		if(collateral_ratio == 1000000) {
			...
			dei_amount = deus_amount * deus_price / scale;
		} else {
			uint deus_to_collateral = getAmountsInDeusToCollateral(collateral_ratio, deus_amount, deus_price);
			...
			uint collateral_amount = amounts_out[amounts_out.length - 1];
			dei_amount = ((deus_amount - deus_to_collateral) * deus_price / scale)  + collateral_amount * collateral_missing_decimals_d18;
		}
		uint minting_fee = DEIPool(pool_collateral).minting_fee();
		dei_amount = (dei_amount * (uint(1e6) - minting_fee)) / 1e6;

		return dei_amount;
	}

	function getAmountsInCollateralToDeus(uint collateral_ratio, uint collateral_amount, uint deus_price) public view returns(uint) {
		uint collateral_to_deus;
		uint times = while_times;
		while(times > 0) {
			uint collateral_for_swap = collateral_amount * (scale - collateral_ratio) / scale;
			...
			uint deus_amount = amounts_out[amounts_out.length - 1];
			uint deus_to_collateral = (deus_amount * deus_price) / (scale * collateral_missing_decimals_d18);
			uint collateral_needed = collateral_ratio * deus_to_collateral / (scale - collateral_ratio);
	...
	function getAmountsOutNativeCoinToDei(uint native_amount, uint deus_price, address[] calldata path) public view returns(uint) {
		uint[] memory amounts_out = IUniswapV2Router02(uniswap_router).getAmountsOut(native_amount, path);
		uint collateral_amount = amounts_out[amounts_out.length - 1];
		return getAmountsOutCollateralToDei(collateral_amount, deus_price);
	}

	function getAmountsOutERC20ToDei(uint erc20amount, uint deus_price, address[] calldata path) public view returns(uint) {
		uint[] memory amounts_out = IUniswapV2Router02(uniswap_router).getAmountsOut(erc20amount, path);
		uint collateral_amount = amounts_out[amounts_out.length - 1];
		return getAmountsOutCollateralToDei(collateral_amount, deus_price);
	}

	function getAmountsOutCollateralToDei(uint collateral_amount, uint deus_price) public view returns(uint) {
		uint collateral_ratio = IDEIStablecoin(dei_address).global_collateral_ratio();
		uint dei_amount;
		if(collateral_ratio == 1000000) {
			dei_amount = collateral_amount * collateral_missing_decimals_d18;
			...
			uint collateral_to_deus = getAmountsInCollateralToDeus(collateral_ratio, collateral_amount, deus_price);
			...
			uint deus_amount = amounts_out[amounts_out.length - 1];
			dei_amount = ((collateral_amount - collateral_to_deus) * collateral_missing_decimals_d18) + (deus_amount * deus_price / scale);
		}
		uint minting_fee = DEIPool(pool_collateral).minting_fee();
		dei_amount = (dei_amount * (uint(1e6) - minting_fee)) / 1e6;
	...
	event Swap(address input_token, uint input_amount, uint dei_amount, uint deus_price, uint collateral_ratio);
