...
contract MMOracle is Ownable {
    ...
    uint16[] public roundIndexes; // Array of indexes from where each round starts.
    ...
    uint16 public tournamentStage; // Divided by `roundIndex` for getting the winners for each round.
    ...
    constructor(
        ...
        uint16 _tournamentStage,
        uint8 _bracketLength,
        uint16[] memory _roundIndexes
    ) {
        round = 0;
        roundIndex = 1;
    ...
    {
        require(
            _newBracketResults.length == bracketLength,
            "MMOracle: lengths don't match"
        );
    ...
    function calcBracketPoints(uint16[] memory bracket)
        ...
        returns (uint16)
    {
        uint16 bracketPoints = 0;
        uint16 startIndex = roundIndexes[round];
        uint16 roundWinners = tournamentStage / roundIndex;

        for (uint16 i = 0; i < roundWinners; i++) {
