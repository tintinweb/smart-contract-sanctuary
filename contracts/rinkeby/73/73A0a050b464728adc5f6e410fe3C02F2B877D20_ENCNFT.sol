...
library EnumerableMap {
    ...
    */
    function at(UintToAddressMap storage map, uint256 index) internal view returns (uint256, address) {
        ...
        return (uint256(key), address(uint160(uint(value))));
    }
...
contract ENCNFT is ERC721, Ownable {
    using Strings for uint256;
    ...
    function mint(
        ...
        string memory decryptKeyForOwner
    ) external onlyOwner {
        _mint(_to, _tokenId);
        _setTokenURI(_tokenId, tokenURI_);
        tokenHistoryEncryption[_tokenId].push(EncryptData(_to, decryptKeyForOwner));
    }
    ...
    function transferTokenWithEncryption(
        ...
        require(
            ownerOf(_tokenId) == ownerToken,
            "Transfer can only the owner of the token"
        );
        safeTransferFrom(msg.sender, _to, _tokenId, _data);
