...
contract TOKordinator is Ownable, ReentrancyGuard {
    ...
    IUniswapFactory internal uniswap;
    ...
    ISwapFactory internal tokenStand;
    ...
    constructor(address _weth, address _uniswap, address _uniswapV2, address _sushiSwap, address _tokenStand) public {
        weth = IWETH(_weth);
        uniswap = IUniswapFactory(_uniswap);
        uniswapV2 = IUniswapV2Factory(_uniswapV2);
        sushiswap = IUniswapV2Factory(_sushiSwap);
        tokenStand = ISwapFactory(_tokenStand);
    }
    ...
    function multiSwap(
        ...
        require(
            ...
            "TOKordinator: return amount was not enough"
        );
    ...
    function getUniswapV2AmountsOut(uint256 amountIn, IERC20[] memory path)
        public
        view
        returns (uint256[] memory)
    {
        return UniswapV2Library.getAmountsOut(uniswapV2, amountIn, path);
    }

    function getSushiswapAmountsOut(uint256 amountIn, IERC20[] memory path)
        public
        view
        returns (uint256[] memory)
    {
        return UniswapV2Library.getAmountsOut(sushiswap, amountIn, path);
    }

    function getTokenStandAmountsOut(uint256 amountIn, IERC20[] memory path)
        public
        view
        returns (uint256[] memory)
    {
        return TokenStandLibrary.getReturns(tokenStand, amountIn, path);
    }
    ...
    function _swapOnTokenStandInternal(
        ...
        if (address(fromToken) != address(0)) {
            require(
                ...
                "TOKordinator: approve TokenStand has failed"
            );
