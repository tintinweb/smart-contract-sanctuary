...
contract DragonMarket is BaseAuctionReceiver {
    ...
    function initialize(address accessControl, address dragonToken, uint fees100) public initializer {
        __BaseAuctionReceiver_init(accessControl, dragonToken, fees100);
    }
...
abstract contract BaseAuctionReceiver is LockableReceiver {
    ...
    function __BaseAuctionReceiver_init(address accessControl, address dragonToken, uint fees100) internal initializer {
        __LockableReceiver_init(accessControl, dragonToken);
        _fees100 = fees100;
    }
    ...
    */
    ...
    function processERC721(address /*operator*/, address /*from*/, uint tokenId, bytes calldata data) 
    ...
    function priceOf(uint tokenId) public view returns (uint) {
        ...
        uint max = MathUpgradeable.max(settings.startingPrice, settings.finalPrice);
        uint min = MathUpgradeable.min(settings.startingPrice, settings.finalPrice);
    ...
    function calcPriceChangeStep(
        ...
        uint max = MathUpgradeable.max(weiStartingPrice, weiFinalPrice);
        uint min = MathUpgradeable.min(weiStartingPrice, weiFinalPrice);
        uint diff = max.sub(min);
...
contract DragonToken is ERC721Upgradeable, BaseAccessControl {

    using AddressUpgradeable for address;
    using CountersUpgradeable for CountersUpgradeable.Counter;
    
    CountersUpgradeable.Counter private _dragonIds;
    ...
    function initialize(string calldata defaultCid, address accessControl) public initializer {
        __ERC721_init("CryptoDragons", "CD");
        __BaseAccessControl_init(accessControl);
...
abstract contract LockableReceiver is BaseAccessControl, IERC721ReceiverUpgradeable {
    ...
    function __LockableReceiver_init(address accessControl, address tokenContractAddress) internal initializer {
        __BaseAccessControl_init(accessControl);
...
abstract contract BaseAccessControl is Initializable, ContextUpgradeable {
    ...
    function __BaseAccessControl_init(address accessControl) internal initializer {
    ...
    function setAccessControlAddress(address accessControl) external onlyRole(CEO_ROLE) {
        _accessControl = accessControl;
    }

    function hasRole(bytes32 role, address account) public view returns (bool) {
        return AccessControlUpgradeable(accessControlAddress()).hasRole(role, account);
    }
