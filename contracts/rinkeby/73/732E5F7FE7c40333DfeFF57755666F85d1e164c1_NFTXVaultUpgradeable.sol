...
interface INFTXVaultFactory is IBeacon{
  ...
  event NewFeeDistributor(address oldReceiver, address newReceiver);
...
interface INFTXVault {
    ...
    function description() external returns (string calldata);
    ...
    INFTXVault
{
    ...
    string public override description; // Not touched at all currently.
    ...
    function __NFTXVault_init(
        ...
        setFees(0.01 ether /*mintFee*/, 0 /*randomRedeemFee*/, 0.1 ether /*targetRedeemFee*/);
    }

    // Technically the DAO can un-finalize. Should we block this?
    function finalizeFund() external override virtual {
    ...
    function setVaultFeatures(
        ...
        if (_enableSwap) 
            require(_enableRandomRedeem || _enableTargetRedeem, "NFTXVault: Redeem required for swap");
        enableSwap = _enableSwap;

        emit EnableMintUpdated(_enableMint);
        emit EnableRandomRedeemUpdated(_enableRandomRedeem);
        emit EnableTargetRedeemUpdated(_enableTargetRedeem);
        emit EnableSwapUpdated(_enableSwap);
    }
    ...
    function swapTo(
        ...
        onlyOwnerIfPaused(3);
        require(enableSwap, "NFTXVault: Swapping not enabled");
    ...
    function withdrawNFTsTo(
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            uint256 tokenId = i < specificIds.length ? 
                specificIds[i] : getRandomTokenIdFromFund();
    ...
    function transferERC721(address assetAddr, address to, uint256 tokenId) internal virtual {
        address kitties = 0x16baF0dE678E52367adC69fD067E5eDd1D33e3bF;
        address punks = 0x59f85A6290B75B006Df3dEe50110CA4E345C8618;
        bytes memory data;
