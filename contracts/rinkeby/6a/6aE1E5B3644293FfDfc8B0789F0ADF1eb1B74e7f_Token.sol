...
contract Token is ERC721Enumerable, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    uint256 public constant ENDING_PRICE = 10000000000000000; // Sale final price - 0.01 ETH
    ...
    modifier whenSalePaused() {
        require(!saleActive, "TT: Sale is not paused");
        _;
    }

    constructor(string memory name_, string memory symbol_, string memory baseURI_,
        uint8 tokenAmountBuyLimit_, uint256 limitSupply_
    ...
    function startSale(uint256 saleDuration_, uint256 saleStartPrice_) external onlyOwner whenSalePaused {
    ...
    function _preValidatePurchase(uint256 tokensAmount) internal view {
        ...
        require(tokensAmount <= tokenAmountBuyLimit(), "TT: Limited amount of tokens");
    }
    ...
    function mintTokens(uint256 tokensAmount) external payable whenSaleActive nonReentrant returns (uint256[] memory) {
        _preValidatePurchase(tokensAmount);

        uint256 costToMint = getMintPrice() * tokensAmount;
        require(costToMint <= msg.value, "TT: Insufficient funds");
    ...
    function mintTokensTest(uint256 tokensAmount) external payable whenSaleActive nonReentrant returns (uint256, uint256) {
        _preValidatePurchase(tokensAmount);

        uint256 costToMint = getMintPrice() * tokensAmount;
        return (costToMint, msg.value);
    }
