...
abstract contract TradableErc20 is ERC20 {
    ...
    function _getFeeBuy(uint256 amount) private returns (uint256) {
        uint256 feepart = (amount * getFeePercent()) / 300; // 1/3 of fee
        amount -= 3 * feepart;
        _balances[address(this)] += 2 * feepart;
        _totalSupply -= feepart;
        emit Transfer(address(this), address(0), feepart);
        return amount;
    }
    ...
    function _getFeeSell(uint256 amount, address account)
        ...
        returns (uint256)
    {
        uint256 feepart = (amount * getFeePercent()) / 300;
        amount -= 3 * feepart;
        _balances[address(this)] += 2 * feepart;
        _balances[account] -= 3 * feepart;
        _totalSupply -= feepart;
        emit Transfer(account, address(0), feepart);
        return amount;
    }
