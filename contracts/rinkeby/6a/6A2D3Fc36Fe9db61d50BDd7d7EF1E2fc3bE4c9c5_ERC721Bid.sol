...
/**
 * @title Roles
 * @dev Library for eggtging addresses assigned to a Role.
 */
library Roles {
  struct Role {
...
contract ERC721Interface {
    ...
    function mint(address _to, string _tokenURI) external returns (uint256);
}
...
contract ERC721Bid is Ownable, Pausable, ERC721BidStorage {
    ...
    */
    constructor(address _eggtToken, address _owner) Ownable() Pausable() public {
    ...
    */
        ...
        returns (bytes4)
    {
        ...
        _requireBidderBalance(_from, price);
        ...
        Order memory order = Order({
          ...
          tokenId: bid.tokenId,
          price: price.sub(saleShareAmount),
        ...
        if (ownerCutPerMillion > 0) {
            ...
            require(
                eggtToken.transferFrom(_from, owner(), saleShareAmount),
        ...
        require(
            eggtToken.transferFrom(_from, this, price.sub(saleShareAmount)),
        ...
        if(msg.sender == auctionContract){
          ...
          IEGGVERSEAuction(auctionContract).acceptBid(_tokenId, bidder, block.timestamp, price);
        }
    ...
    function finalizeOrder(address _tokenAddress, uint256 _tokenId) public {
      Order memory order = orders[_tokenAddress][_tokenId];
      ...
      require(
          eggtToken.transferFrom(address(this), bidder, price),
          "Transfering EGGT to bidder failed"
      );

      string memory URI = ERC721Interface(_tokenAddress).tokenURI(_tokenId);
      uint256 certificateTokenId = ERC721Interface(_certificateAddress).mint(bidder, URI);

      // auction에 이벤트 보냄
      // auction 컨트랙트 데이터 등록용 함수 호출
      // 이건 역경매 제시자가 직접 호출하는 함수이므로 막지 않는다. 
      // if(msg.sender == auctionContract){
...
interface IEGGVERSEAuction {
  function acceptBid(uint256 _tokenId, address acceptBidder, uint256 acceptedAt, uint256 acceptedPrice) external;
