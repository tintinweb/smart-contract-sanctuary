...
contract PickRandomWinner is VRFConsumerBase, Ownable {
    ...
    mapping(bytes32 => uint256) private requestStatus; 

    event RequestWinner(bytes32 urlHash, bytes32 requestId);
    event ReturnWinner(bytes32 urlHash, uint256 result);
    ...
    function requestWinnerSigningHash(uint256 blockExpiry, uint256 documentId, bytes32 urlHash, uint256 totalSigners) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(blockExpiry, documentId, urlHash, totalSigners));
    }

    // Set this to internal once done testing
    function verify(bytes memory signature, uint256 blockExpiry, uint256 documentId, bytes32 urlHash, uint256 totalSigners) public returns (bool) {
    ...
    function requestWinner(bytes memory signature, uint256 blockExpiry, uint256 documentId, bytes32 urlHash, uint256 totalSigners) public {
    ...
    function getDocumentWinner(bytes32 urlHash) public view returns (Document memory) {
