...
contract PPunk is ERC721Enumerable, Ownable {
    ...
    // function getMintedCurrentHolders () public onlyOwner
    // returns (currentHolderToken storage) {
    ...
    function mintCHWhiteList(uint256 numberOfTokens) public {
        //can only mint the amount of tokens they own
        require(isPreSaleActive, "PPunk: Sale is not active");

        require(isPreSaleActive, "PPunk: Sale is not active");

        require(
            this.isWhitelisted(msg.sender, 1),
            "PPunk: You don't hold any NFTs from previous collection"
        );
