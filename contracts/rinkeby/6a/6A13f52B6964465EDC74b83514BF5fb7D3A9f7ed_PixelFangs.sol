...
contract PixelFangs is ERC721Enumerable, Ownable {  
    ...
    constructor (string memory newBaseURI, address fngAddress) ERC721 ("PixelFangs", "PXF") {
        ...
        // Deploy with fanggang contract address
        fg = fng(fngAddress);

        // Reserve 3 for the team + 1 diamond fangster
        for (uint256 i; i < 4; i++) {
            claimed[i] = true;
    ...
    function claimToken(uint256[] memory requestTokenIds) public {
        ...
        // require( requestTokenIds is array, "param ist array with ids" );

        uint256 supply = totalSupply();

        // Returns uint256 bigNumber array of owned fangsters that looks like this: [1,2,3,4];
        uint256[] memory ownedTokens = fg.tokensOfOwner(msg.sender);
        // Create empty dynamic array with ownerTokens length -- Memory arrays have fixed length =/
        uint256[] memory unclaimedTokens = new uint256[](ownedTokens.length);
        // Need to track count extra because you can't remove elements completely out of array
        ...
        for(uint i = 0; i < requestTokenIds.length; i++) {
            ...
            if (!claimed[requestTokenIds[i]]) {
                // loop through all owners tokens
                for (uint256 k = 0; k < ownedTokens.length; k++) { 
                    // check if owendTokens contains reqId
                    if (ownedTokens[k] == requestTokenIds[i]) { 
                        // Add unclaimed Token to the dynamic array - can't use push here (memory array)
                        unclaimedTokens[unclaimedTokensLastIndex] = ownedTokens[k];
                        // Track actual dynamic array size
                        unclaimedTokensLastIndex++;
                        // break out of k-loop and continue i-loop cuz we found our match -- save gas
                        break;
                    } 
                }
            }
        ...
        // Set loop limit to max 50 otherwise we hit the max gas limit per tx
