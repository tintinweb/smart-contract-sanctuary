...
contract Staker {
  ...
  event Stake(address _from, uint256 balance);
  ...
  function stake() external payable {
    this.stakeFrom(msg.sender, msg.value);
  }

  function stakeFrom(address _from, uint256 value) public beforeDeadline {
    balances[_from] += value;
    emit Stake(_from, balances[_from]);
  ...
  function withdraw(address payable _to) external allowWithdraw notCompleted {
    if (balances[_to] == 0) {
      return;
    }
