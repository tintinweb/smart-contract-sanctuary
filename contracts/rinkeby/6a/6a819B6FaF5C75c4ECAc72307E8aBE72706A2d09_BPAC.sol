...
contract BPAC is ERC721EnumerableUpgradeable, OwnableUpgradeable {
        ...
        initializer
    {
        ...
        reveal_start_time = _reveal_start_time;
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (block.timestamp < reveal_start_time) {
            return string(abi.encodePacked(baseURI, "default.json"));
        }
        // reveal_start_time
        uint256 batch_id = 0;
        uint256 reveal_time = reveal_start_time;
        // ugly magic numbers, 300 * 20 = 60,000
        for (; batch_id < 20; batch_id++) {
            if (block.timestamp < reveal_time) {
                break;
            }
            reveal_time = reveal_time + 1 days;
        }
        uint256 reveal_nft_id = batch_id * 300;
        if (tokenId > reveal_nft_id) {
            // still in a blind box
            return string(abi.encodePacked(baseURI, "default.json"));
        }
        else {
            // https://raw.githubusercontent.com/PrincessApeClub/NFTAsset/master/json/0.json
