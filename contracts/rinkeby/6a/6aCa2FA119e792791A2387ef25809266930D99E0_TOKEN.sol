...
contract TOKEN is ERC20, Ownable {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) override internal view {        
        if(from != owner() && to != owner() && from != address(this)) {
            ...
            if(from == uniswapPair && buyLimitEnd > block.timestamp)
                require(amount <= maxBuyAmount);
        }
    }
    
    function setMaxTxPercent(uint256 multiple) external onlyOwner {
        maxBuyAmount = totalSupply().mul(multiple).div(1000);
    }
    ...
    function addLiquidity() external onlyOwner {
        ...
        maxBuyAmount = totalSupply().div(100);
    }
