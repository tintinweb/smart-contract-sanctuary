    ...
    SplitForwarderFactory
{
    ...
    ) ERC721(_name, _symbol) ERC2771Context(_trustedForwarder) SplitForwarderFactory(_splitForwarder, _splitPool) {
     ...
     * @param _tokenRoyalty royalty basis points for a token
     */
    function _mint(string memory _tokenURI, address payable _creator, address payable _paymentAddress, uint32 _tokenRoyalty) internal returns (uint256) {
        ...
        FirstDibsPayments._setTokenPaymentAddress(_paymentAddress, newTokenId);
        FirstDibsPayments._setPerTokenRoyalties(newTokenId, _tokenRoyalty);
        return newTokenId;
    }
    ...
    function mintWithParams(string memory _tokenURI, address payable _creator, bytes32 _merkleRoot, address payable _paymentAddress, uint32 _tokenRoyalty) public {
        require(hasRole(MINTER_ROLE, _msgSender()), 'mint: must have MINTER_ROLE');
        address paymentAddress = _msgSender();
        address creator = _msgSender();
        if (_merkleRoot != 0) {
            require(_paymentAddress == address(0), "mintWithParams: cannot set payment address and split at the same time");
            paymentAddress = SplitForwarderFactory.createSplitForwarder(_merkleRoot);
        } else if (_paymentAddress != address(0)) {
            paymentAddress = _paymentAddress;
        } else if (_creator != address(0)) {
            paymentAddress = _creator;
        }
        if (_creator != address(0)) {
            creator = _creator;
        }
        _mint(_tokenURI, payable(creator), payable(paymentAddress), _tokenRoyalty);
    }
    ...
    function _beforeTokenTransfer(
        ...
        address _to,
        uint256 _tokenId
    ) internal whenNotPaused override(ERC721, ERC721Pausable) {
...
contract SplitForwarderFactory is ISplitForwarderFactory {
    ...
    constructor(address _splitForwarder, address _splitPool) {
        require(_splitForwarder != address(0), "constructor: splitForwarder must be set");
        require(_splitPool != address(0), "constructor: splitPool must be set");
     ...
     */
    function getSplitForwarderAddress(bytes32 _merkleRoot) external view override returns(address) {
