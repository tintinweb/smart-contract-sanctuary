...
contract BeesDeluxe is ERC721Enumerable, ERC721Burnable, AccessControl {
    uint16 public constant MAX_SUPPLY = 20700;
    ...
    string private baseUri = "https://ipfs.io/ipfs/QmbDeceD3nRHc4Ew5zgFanJmGxWyhSnDmEwoHHywzj7sbs/";
     ...
     */
    function mint(address _owner, uint256 _tokenId) external onlyRole(GOVERNANCE_ROLE) {
     ...
     */
    function burnByQueen(uint256 _tokenId) external onlyRole(GOVERNANCE_ROLE) {
     ...
     * only emergency
     */
    function setTokenUri(uint256 _tokenId, string calldata _tokenURI) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function getMaxSupply() external pure returns (uint16) {
        return MAX_SUPPLY;
    }
    ...
    function tokensOfOwner(address _owner) external view returns (uint16[] memory) {
        uint16 tokenCount = uint16(balanceOf(_owner));
        if (tokenCount == 0) {
