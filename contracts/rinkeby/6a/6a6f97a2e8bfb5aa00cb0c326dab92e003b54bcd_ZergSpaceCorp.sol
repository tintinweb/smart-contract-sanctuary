...
// File: Projects/ZergSpaceCorp.sol



contract ZergSpaceCorp is ERC721, ERC721Enumerable, Ownable, ERC721Burnable {
    using Address  for address;
    ...
    bool public saleOpen = true; // !!!!!!!!!!!!!!!!!!!!!!!!!  FALSE !!
  
    uint public maxSupply;
    uint public price;
    uint public maxMintRequest;

    string public baseURI = "https://kruegger.pythonanywhere.com/assets/planets/v1.0/";
    string public provenance = "the place for provenance hash for whole nft bunch";
    ...
    PlanetZerg[] planetZerg;
    
    // **********************************************************************

    address aWallet = 0x000000000000000000000000000000000000dEaD;
    address bWallet = 0x000000000000000000000000000000000000dEaD;
    address cWallet = 0x000000000000000000000000000000000000dEaD;
    address dWallet = 0x000000000000000000000000000000000000dEaD;
    ...
    ) ERC721("ZergSpaceCorp", "ZSCorp") {
        maxSupply      = 5000;
        price          = 0.001 ether;
        maxMintRequest = 3;
        baseURI        = "https://kruegger.pythonanywhere.com/assets/planets/v1.0/";
    ...
    // **********************************************************************
    function mintPlanetZerg(uint _amount) public payable {
        address owner = owner();
        ...
        for(uint i = 0; i < _amount; i++) {
            planetZerg.push(
                // PlanetZerg(_tokenIds.current(), block.timestamp, block.number, blockhash(block.number), _msgSender() )
                PlanetZerg(_tokenIds.current(), block.timestamp, _msgSender() )
            );
    ...
    function getPlanetZerg(uint _tokenId) public view returns (PlanetZerg memory) {
        require(_exists(_tokenId), "There is no Planet with this number");
        return planetZerg[_tokenId];
    }
    ...
    function withdraw() public onlyOwner {
        ...
        payable(aWallet).transfer((balance * 710) / 1000);
        payable(bWallet).transfer((balance * 50)  / 1000);
        payable(cWallet).transfer((balance * 40)  / 1000);
        payable(dWallet).transfer((balance * 200) / 1000);
    }
