...
contract MonsterbudsV2 is MonsterBudHolders, ERC721URIStorageUpgradeable, IMonsterBuds {
    ...
    */
    ...
    function selfBreedCollectiable(uint256 req_token_id, uint256 accept_token_id) external override payable returns (uint256) {
        ...
        require(selfBreedStatus == true && block.timestamp >= breedingInfo[req_token_id].timstamp && block.timestamp >= breedingInfo[accept_token_id].timstamp,"$MONSTERBUDS: Breeding is closed");
        require(owner_req == owner_accept && owner_req == msg.sender, "$MONSTERBUDS: Cannot Self Breed");
        require(breedValue == msg.value && breedingInfo[req_token_id].breedCount < 2 && breedingInfo[accept_token_id].breedCount < 2, "$MONSTERBUDS: Amount is incorrect OR exceeds max breed count");
