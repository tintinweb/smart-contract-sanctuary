...
contract StakBankFactory is Ownable, Pausable {
    ...
    // JStak token address
    IJSTAK public jStak = IJSTAK(0xf8b7B49F0E39938965Bbc7Fa6562361506ca5D70);
     ...
     */
    function getCreatedLPBanksByToken(address _creator, IERC20 _stakingToken)
        public
        view
    ...
    function registerStakBank(
        ...
        bytes32 salt = keccak256(
            abi.encodePacked(msg.sender, _stakingToken, tokenIndex)
        );
        assembly {
            pool := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IStakBank(pool).initialize(_name, _stakingToken, _rewardToken, jStak);
        getStakBanks[msg.sender][_stakingToken].push(pool);
...
contract StakBank is ReentrancyGuard, Pausable, Ownable {
    using SafeMath for uint256;
    ...
    /**
     * @notice Owner can collect the remaining rewards in the contract after the pool ends
     */
    function collectRemainingRewards() external onlyOwner {
        require(block.timestamp > periodFinish, "STAK_BANK::POOL_NOT_END");
     ...
     */
    function setStakingTokenTokenContract(IERC20 _token) external onlyOwner {
        stakingToken = _token;
    }

    /**
     * @notice Owner can close the pool
     */
