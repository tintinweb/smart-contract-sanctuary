...
contract YungApes is Ownable, ERC721 {
  ...
  //Squid ape contract instance
  IYungApe squidApeContract;
  ...
  address payable private gnosis;

  mapping (address => uint256[]) private tokenIdsOfAddresses;

  event GnosisWalletSet (address indexed newGnosisAddress);

  constructor() ERC721("YungApes", "YA") { 
    deployer = msg.sender;
  }
  
  modifier whitelisted {
    require(squidApeContract.balanceOf(msg.sender) > 0, "Doesn't hold any Squid Apes!");
    _;
  }
  ...
  function addToWhitelist(address[] memory wallets) public onlyOwner {
    ...
    for(uint i = 0; i < wallets.length; i++) {
      tokenIdsOfAddresses[wallets[i]].push(currentId);
      currentId++;
    }
  }
  ...
  function setGnosisSafeWallet(address payable _newAddress) external onlyOwner {
    gnosis = _newAddress;
    emit GnosisWalletSet(gnosis);
  }
  ...
  function claimAllYungApes() external whitelisted {
    require(preSaleState, "Presale is not active");   
    

    for(uint index = 0; index < squidApeContract.balanceOf(msg.sender); index++){
      if(!_exists(squidApeContract.tokenOfOwnerByIndex(msg.sender, index))){
        mintInternalPresale(msg.sender, squidApeContract.tokenOfOwnerByIndex(msg.sender, index));
      }
    }
...
abstract contract IYungApe is Ownable, IERC721Enumerable {
    function ownerOf(uint256 tokenId) public view virtual override returns (address);
    function balanceOf(address owner) public view virtual override returns (uint256);
    function tokenOfOwnerByIndex(address owner, uint256 index) external view virtual override returns (uint256 tokenId);
}
