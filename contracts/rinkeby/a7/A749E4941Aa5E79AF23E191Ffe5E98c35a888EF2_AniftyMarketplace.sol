...
contract AniftyMarketplace is ERC1155Holder {
    ...
    string public constant name = "MP";
        ...
        )
    {
        SaleData memory saleData = SaleData(
            new address[](_saleIds.length), // sellers
        ...
        for (uint256 i = 0; i < _saleIds.length; i++) {
            saleData.sellers[i] = sales[_saleIds[i]].seller;
        ...
        )
    {
        AuctionData memory auctionData = AuctionData(
            new address[](_auctionIds.length), // sellers
        ...
        for (uint256 i = 0; i < _auctionIds.length; i++) {
            auctionData.sellers[i] = auctions[_auctionIds[i]].seller;
    ...
    function listNewSale(
        ...
        require(saleSupportedTokens[_token], "MP: Invalid token");
    ...
    function buyFromSale(uint256 _saleId) external payable {
        ...
        require(
            sale.seller != address(0) && sale.buyer == address(0),
            "MP: !sale"
        );
        require(sale.price > 0, "MP: Sale cancelled");
        // Set buyer for sale
        sales[_saleId].buyer = msg.sender;
        if (sale.token == address(0)) {
            require(msg.value >= sale.price, "MP: Payable value too low");
            ...
            token.transferFrom(
                msg.sender,
                sale.seller,
                sale.price.sub(commissionAmount).sub(royaltyAmount)
            );
            // Transfer commission wallet the commission fee
    ...
    function listNewAuction(
        ...
        require(_initialPrice > 0, "MP: Price > 0");
        require(supportedTokens[_token], "MP: Invalid token");
        require(_endTimestamp > block.timestamp, "MP: Invalid end timestamp");
        // Transfer tokens into marketplace contract
        Anifty.safeTransferFrom(
            msg.sender,
            address(this),
    ...
    function cancelAuction(uint256 _auctionId) external {
        require(auctions[_auctionId].seller == msg.sender, "MP: Not owner");
    ...
    function sellerClaimBySig(
        ...
        require(signatory == auctionSignature.bidder, "MP: Invalid signature");
    ...
    function _sellClaim(AuctionSignature memory auctionSignature) internal {
        ...
        require(auction.seller == msg.sender, "MP: Not owner");
        require(auction.buyer == address(0), "MP: Auction completed");
        require(auction.initialPrice > 0, "MP: Auction cancelled");
        require(
            auctionSignature.price >= auction.initialPrice,
            "MP: Invalid Bid"
        );
        // Make sure bidder is bidding the correct token
        require(auction.token == auctionSignature.token, "MP: Invalid token");
        ...
        .mul(commissionFee)
        .div(PRECISION);
        uint256 royaltyAmount = auctionSignature
        .price
        .mul(artistRoyalties[auction.tokenId].royalty)
        .div(PRECISION);
        // Transfer token to seller
        token.transferFrom(
            auctionSignature.bidder,
            address(msg.sender),
            auctionSignature.price.sub(commissionAmount).sub(royaltyAmount)
        );
        ...
        if (royaltyAmount > 0) {
            ...
            token.transferFrom(
                auctionSignature.bidder,
                artistRoyalties[auction.tokenId].wallet,
                royaltyAmount
            );
        }
        Anifty.safeTransferFrom(
