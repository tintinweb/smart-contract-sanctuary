...
/**
 * @title PixlBros NFT
 * a contract for non-fungible PixlBros RoyaltyRewards.
 */
contract PixlBrosRoyaltyRewards is Ownable {
    using SafeMath for uint256;

    // Mapping from token ID to time of the withdraw list
    mapping(uint256 => uint256) private _lastwithdrawTimeETH;
    // Mapping from token ID to time of the withdraw list
    mapping(uint256 => uint256) private _lastwithdrawTimeERC20s;




    // Array with all token ids, used for enumeration
    uint256[] private _timeETH;
    // Array with all token ids, used for enumeration
    uint256[] private _timeERC20;
    ...
    */
    ...
    // Mapping from tokenId to lastIndex of the owner tokens list
    mapping(uint256 => uint256) private tokenIdsLastIndex;
    // Mapping from time to ETH of the owner tokens list
    mapping(uint256 => uint256) private dataValue;
    // Mapping from time to ETH of the owner tokens list
    mapping(uint256 => uint256) private dataPart;
    ...
    uint256 royaltiesId = 0;

    //WETH rinkeby: 0xc778417e063141139fce010982780140aa0cd5ab
    //WETH 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2
    constructor(address _nftAddress, address _wEthAddress){
    ...
    {
        dataValue[royaltiesId] = msg.value;
        dataPart[royaltiesId] = msg.value.div(MAX_TOKENS);
        royaltiesId = royaltiesId++;
    }

    /// @dev Sells tokens in exchange for Ether, exchanging them 1:1.
    function convertWETHtoRoyalties()
    public
    {
        IWETH tokenContract = IWETH(wEthAddress);
        uint256 balance = tokenContract.balanceOf(address(this));

        //wEthAddress.call(bytes4(keccak256("withdraw(uint)")), balance);

        tokenContract.withdraw(balance);
    }

    function calcRewardByTokenId(uint256 tokenId) public returns (uint256) {
        uint256 index = tokenIdsLastIndex[tokenId];
        uint maxiteration = royaltiesId - index;
        uint256 balance = calcRewardByTokenIdIterations(tokenId, maxiteration);
        //uint256 balance = address(this).balance;
        //payable(msg.sender).transfer(balance);
        return balance;
    }

    function calcRewardByTokenIdIterations(uint256 tokenId, uint iterations) public returns (uint256) {
        uint256 index = tokenIdsLastIndex[tokenId];
        require(index < royaltiesId, "index is out of range");
        uint maxiteration = royaltiesId + iterations;
        require(maxiteration < royaltiesId, "index is out of range");
        uint256 balance = 0;
        for (uint i = index; i < maxiteration; i++) {
            balance.add(dataPart[royaltiesId]);
        }
        tokenIdsLastIndex[tokenId] = maxiteration;
        return balance;
        //uint256 balance = address(this).balance;
        //payable(msg.sender).transfer(balance);
    }

    function getAllRewards() public onlyCreator {
        //uint256 balance = address(this).balance;
        //payable(msg.sender).transfer(balance);

        PixlBros nft = PixlBros(nftAddress);
        uint256[] memory tokenIdsByOwner = nft.tokensByOwner(msg.sender);
        uint256 index;
        uint256 balance = 0;
        uint256 length = tokenIdsByOwner.length;
        for (index = 0; index < length; index++) {
            uint256 tokenId = tokenIdsByOwner[index];
            balance.add(calcRewardByTokenId(tokenId));
            //save index
            tokenIdsLastIndex[tokenId] = royaltiesId;
        }
        payable(msg.sender).transfer(balance);
