...
interface IUnipilot is IUnipilotEvents {
    ...
    function collectForETH(bool pilotToken, uint256 tokenId) external payable;
...
contract Unipilot is ERC721Permit, IUnipilot {
    ...
    function collect(
        ...
        address _recipient = recipient == address(0)
    ...
    function collectForETH(bool _pilotToken, uint256 tokenId)
        external
        payable
        override
        isAuthorizedForToken(tokenId)
    {
        ...
        collect(_pilotToken, address(0), tokenId);
        address token = _token0 == WETH ? _token1 : _token0;
        if (!_pilotToken)
            IPeripheryPayments(address(liquidityManager)).unwrapWETH9(0, msg.sender);
        if (!_pilotToken)
