...
contract Elemental is ERC721Enumerable, Ownable {
  ...
  function mint(uint numberOfTokens) public payable {
    ...
    for (uint i = 0; i < numberOfTokens; i++) {
      ...
      if(tokenId < 1501)
        waterPool += (msg.value/numberOfTokens) * 20 / 100;
      else if (tokenId < 3001)
        firePool += (msg.value/numberOfTokens) * 20 / 100;
      else if (tokenId < 4501)
        earthPool += (msg.value/numberOfTokens) * 20 / 100;
      else
        airPool += (msg.value/numberOfTokens) * 20 / 100;
  ...
  function mintGod(uint stagger) public {
    ...
    if(stagger == 0){
        ...
        godOfWater = true;
        payable(msg.sender).transfer(waterPool);
        waterWon = true;
        waterWinner = msg.sender;
    } else if(stagger == 1500){
        _safeMint(msg.sender, 1501);
        godOfFire = true;
        payable(msg.sender).transfer(firePool);
        fireWon = true;
        fireWinner = msg.sender;
    } else if(stagger == 3000){
        _safeMint(msg.sender, 3001);
        godOfEarth = true;
        payable(msg.sender).transfer(earthPool);
        earthWon = true;
        earthWinner = msg.sender;
    } else if (stagger == 4500){
        _safeMint(msg.sender, 4501);
        godOfAir = true;
        payable(msg.sender).transfer(airPool);
        airWon = true;
