...
contract StalkerNFTToken is MinterAccess, ERC721 {

    // Base Token URI
    string private _baseTokenURI;

    constructor (address newOwner, string memory tokenURIPrefix) ERC721("Stalker Meta Human", "SMH NFT") {
    ...
    function burn(uint256 tokenID) public virtual {
        require(_isApprovedOrOwner(_msgSender(), tokenID), "Stalker Meta Human Token: caller is not owner nor approved");
        _burn(tokenID);
    }
    ...
    function mintTokenBatch(address[] memory receivers, uint64[] memory tokenIDs) public virtual onlyMinter {
        require(receivers.length > 0,"Stalker Meta Human Token: must be some receivers");
        require(receivers.length == tokenIDs.length, "Stalker Meta Human Token: must be the same number of receivers/tokenIDs");
