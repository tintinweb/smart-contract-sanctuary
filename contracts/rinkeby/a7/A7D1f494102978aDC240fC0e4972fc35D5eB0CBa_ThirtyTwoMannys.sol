...
import {SwordsChessEvent} from "../mods/SwordsChessEvent.sol";
...
contract ThirtyTwoMannys is SwordsChessEvent, ERC721Holder {
    address private constant burnAddress =
    ...
    mapping(uint256 => uint256) public mannyIds;
    mapping(uint256 => uint256) public lootIds;

    // * CONSTRUCTOR * //
    constructor(
        uint256 eventId_,
        uint256[5] memory mintPrices_,
    ...
    function mint(uint256) public payable override {
        // Disables mint function from parent contract
        revert("disabled");
    }
    ...
    function mint(
        ...
        _importMannyAndLootSword(tokenId, mannyTokenId, lootTokenId);
        SwordsChessEvent.mint(tokenId);
    }
    ...
    function mintKing(
        ...
        _importMannyAndLootSword(tokenId, mannyTokenId, lootTokenId);
        SwordsChessEvent.mintKing(tokenId, toAddress);
    }
    ...
    {
        _destroyMannyAndSwords(tokenId);
        SwordsChessEvent.submitMoveAndDestroy(move, tokenId, destroyedBy);
    }

    function finishEvent(EndState endState_)
        public
        override
        onlyWhileEventActive
        onlyRole(PRODUCER)
    {
        if (endState_ == EndState.WIN_W) {
            for (uint8 i = 1; i <= 16; i++) {
                if (i == 5) {
                    continue; // DONT BURN THE KING
                }
                if (_exists(i)) {
                    _destroyMannyAndSwords(i);
                }
            }
        } else if (endState_ == EndState.WIN_B) {
        ...
        SwordsChessEvent.finishEvent(endState_);
    }

    function claimPrize() public override {
        SwordsChessEvent.claimPrize();
        IERC721(mannyContract).safeTransferFrom(
            address(this),
            msg.sender,
            mannyIds[tokenOwned[msg.sender]]
        );
        uint256 lootTokenId = lootIds[tokenOwned[msg.sender]];
        IERC721(lootTokenId > 8000 ? mLootContract : lootContract)
            .safeTransferFrom(address(this), msg.sender, lootTokenId);
    }
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked(
                    "https://32swords.com/api/token?event=",
                    eventId,
                    "&token=",
                    tokenId
                )
    ...
    function _importMannyAndLootSword(
        uint256 tokenId,
        uint256 mannyTokenId,
        uint256 lootTokenId
    ) private {
        ...
        IERC721 _lootContract = IERC721(
            lootTokenId > 8000 ? mLootContract : lootContract
        );
        IERC721 _mannyContract = IERC721(mannyContract);
        require(
            _lootContract.ownerOf(lootTokenId) == _msgSender(),
            "not your loot"
        );
        require(
            _mannyContract.ownerOf(mannyTokenId) == _msgSender(),
            "not your manny"
        );
        _lootContract.safeTransferFrom(
            _msgSender(),
            address(this),
            lootTokenId
        );
        lootIds[tokenId] = lootTokenId;
        _mannyContract.safeTransferFrom(
            _msgSender(),
            address(this),
            mannyTokenId
        );
        mannyIds[tokenId] = mannyTokenId;
    }

    function _destroyMannyAndSwords(uint256 tokenId) private {
        IERC721(mannyContract).safeTransferFrom(
            address(this),
            burnAddress,
            mannyIds[tokenId]
        );
        IERC721(lootIds[tokenId] > 8000 ? mLootContract : lootContract)
            .safeTransferFrom(address(this), burnAddress, lootIds[tokenId]);
    }
}
...
import {ISwordsChessEvent} from "../interfaces/ISwordsChessEvent.sol";

contract SwordsChessEvent is ISwordsChessEvent, SwordsEvent {
    // * STORAGE * //
    uint256 public pawnMintPrice;
    uint256 public rookMintPrice;
    uint256 public knightMintPrice;
    uint256 public bishopMintPrice;
    uint256 public queenMintPrice;
    ...
    {
        pawnMintPrice = mintPrices[0];
        rookMintPrice = mintPrices[1];
        knightMintPrice = mintPrices[2];
        bishopMintPrice = mintPrices[3];
        queenMintPrice = mintPrices[4];
    }
    ...
    function mint(uint256 tokenId)
        ...
        onlyNonKings(tokenId)
    {
        if (tokenId == 4 || tokenId == 28) _mintQueen(tokenId);
        else if (tokenId == 3 || tokenId == 6 || tokenId == 27 || tokenId == 30)
            _mintBishop(tokenId);
        else if (tokenId == 2 || tokenId == 7 || tokenId == 26 || tokenId == 31)
            _mintKnight(tokenId);
        else if (tokenId == 1 || tokenId == 8 || tokenId == 25 || tokenId == 32)
            _mintRook(tokenId);
        else _mintPawn(tokenId);
    }

    function _mintPawn(uint256 tokenId) private {
        require(msg.value == pawnMintPrice, "not right amount of ether");
        _mintPiece(tokenId);
    }

    function _mintRook(uint256 tokenId) private {
        require(msg.value == rookMintPrice, "not right amount of ether");
        _mintPiece(tokenId);
    }

    function _mintBishop(uint256 tokenId) private {
        require(msg.value == bishopMintPrice, "not right amount of ether");
        _mintPiece(tokenId);
    }

    function _mintKnight(uint256 tokenId) private {
        require(msg.value == knightMintPrice, "not right amount of ether");
        _mintPiece(tokenId);
    }

    function _mintQueen(uint256 tokenId) private {
        require(msg.value == queenMintPrice, "not right amount of ether");
        _mintPiece(tokenId);
    }

    // * Internal Mint Function * //
    function _mintPiece(uint256 tokenId) internal {
        _mint(tokenId);
    }
    ...
    function startEvent() public virtual override onlyRole(PRODUCER) {
        require(tokensActive == 32, "not all pieces have been minted");
        SwordsEvent.startEvent();
    }
    ...
    function submitMoveAndDestroy(
        ...
        onlyNonKings(tokenId)
    {
        SwordsEvent.submitMoveAndDestroy(move, tokenId, destroyedBy);
    }
    ...
    function finishEvent(EndState endState_)
        ...
        onlyRole(PRODUCER)
    {
        require(endState_ > EndState.NONE, "invalid state");
        endState = uint8(endState_);
        if (endState_ == EndState.WIN_W) {
            for (uint8 i = 1; i <= 16; i++) {
                if (i == 5) {
                    tokensActive--;
        ...
        emit EventFinished(endState);
        SwordsEvent._finishEvent();
    }

    // * WINNER FUNCTIONS * //
    function claimPrize() public virtual override {
        uint256 tokenId = tokenOwned[msg.sender];
        require(
            tokenId != 0 &&
                (endState == uint8(EndState.DRAW) ||
                    ((endState == uint8(EndState.WIN_W) ||
                        endState == uint8(EndState.STALEMATE_W)) &&
                        tokenId >= 17) ||
                    ((endState == uint8(EndState.WIN_B) ||
                        endState == uint8(EndState.STALEMATE_B)) &&
                        tokenId <= 16)),
            "not a winner"
        );
        SwordsEvent.claimPrize();
    }

    // * INTERNAL CHECKS * //
    function _requireValidToken(uint256 tokenId) internal view {
...
abstract contract SwordsEvent is
    ERC721,
    ISwordsEvent,
    AccessControl,
    ReentrancyGuard,
    ERC2981Base
{
    ...
    uint8 public endState;
    address public marketContract;
    uint256 public moveNumber;
    ...
    constructor(
        ...
        emit EventStateChanged(EventState.INITIALIZED, 0);
    }
    ...
    function startEvent() public virtual onlyRole(PRODUCER) {
        require(state == EventState.INITIALIZED, "event already started");
        state = EventState.ACTIVE;
        emit EventStateChanged(state, 0);
    }

    function submitMove(string calldata move)
        public
        virtual
        onlyWhileEventActive
        onlyRole(PRODUCER)
        ...
        onlyExistingToken(tokenId)
    {
        ...
        _burn(tokenId);
        tokensActive--;
        emit PieceDestroyed(tokenId, destroyedBy, moveNumber);
    }
        ...
        onlyRole(PRODUCER)
    {
        ...
        emit EventStateChanged(state, moveNumber);
    }

    // * WINNER FUNCTIONS * //
    function claimPrize() public virtual {
        require(state == EventState.FINISHED, "event not finished");
        require(balanceOf(msg.sender) != 0, "no surviving piece held");
        uint256 tokenId = tokenOwned[msg.sender];
        require(
            prizeClaimed[tokenId] == false,
            "already claimed for this piece"
        );
        payable(msg.sender).call{value: (totalPrize / tokensActive)}("");
...
interface ISwordsEvent {
    ...
    event MoveMade(string move, uint256 moveNumber);
    ...
    function submitMove(string calldata move) external;
