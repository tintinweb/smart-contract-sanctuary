    ...
    ERC1155Supply
{
    ...
    uint256 public tokenType; // ERC-1155 supports multiple token types; we need just one
    ...
    function mint(address to, uint256 qty) external payable {
        require((qty + totalSupply(tokenType)) <= tokenLimit, "sold out");
        require(
            (qty + balanceOf(to, tokenType)) <= balanceLimit,
            "exceeded balance limit"
        );
        require(msg.value >= (price * qty), "insufficient funds");
        _mint(to, tokenType, qty, "");
    }
