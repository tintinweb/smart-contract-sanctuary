...
contract ForTubeOracle is Initializable, IPriceOracles {
    ...
    //所有喂价地址列表，每个节点一个喂价地址，下架节点时，默认下架最后一个节点。下架时，需要删除下架节点的数据。
    ...
    mapping (uint256 => mapping (address => Price)) public prices; // nodeId => token => price
    mapping (address => Price) public finalPrices;//最终结果
    mapping (address => Price[]) public finalPricesBuffer;//

    //使用新的address作为Key
    EnumerableSet.AddressSet private _feeders;// 支持的喂价者列表
    mapping (address => mapping (address => Price)) public _prices;

    function initialize(address _ACL, address[] memory _initFeeders)
        public
        initializer
    {
        ACL = _ACL;
    ...
    function enable(address feeder) public onlyAdmin {
        // feeders.push(feeder);
    ...
    function disable(address feeder) public onlyAdmin {
        // uint n = feeders.length;
        // require(feeder == feeders[n - 1], "feeder must be the last one");
        // feeders.pop();
        // require(feeders.length >= 1, "at least one feeder work");

        // uint256 nodeId = id(feeder);
        require(_feeders.contains(feeder), "not exist");
        _feeders.remove(feeder);

        for (uint i = 0; i < _tokens.length(); i++) {
            // delete prices[nodeId][_tokens.at(i)];
            delete _prices[feeder][_tokens.at(i)];
        }
        emit Disable(feeder);
    }

    function enables(address[] calldata feeders) external onlyAdmin {
    ...
    function disables(address[] calldata feeders) external onlyAdmin {
        for (uint256 i = 0; i < feeders.length; i++) {
            disable(feeders[i]);
        }
    ...
    // function id(address nodeAddress) public view returns (uint256) {
    //     uint256 i = uint256(-1);
    //     for (i = 0; i < feeders.length; i++) {
    //         if (nodeAddress == feeders[i]) {
    //             return i;
    //         }
    //     }
    ...
    function set(address token, uint val, uint exp) public auth {
        // uint256 nodeId = id(msg.sender);
        // require(nodeId != uint256(-1), "invalid nodeid");
        require(_feeders.contains(msg.sender), "unauth feeder");

        _prices[msg.sender][token].price = val;
        _prices[msg.sender][token].expiration = now + exp;
        ...
        finalPrices[token].price = uint256(Median.calculateInplace(priceFilter));
        finalPrices[token].expiration = now + exp;
    ...
    function getLastPriceByFeeder(address feeder, address token) public view returns (uint price, uint expiration) {
        // uint256 nodeId = id(feeder);
        // return (prices[nodeId][token].price, prices[nodeId][token].expiration);
