    ...
    ReentrancyGuard
{
    ...
    uint256 public constant presaleLimit = 20;
    ...
    modifier onlyPresaleAllowlist() {
        require(_presaleAllowlist[msg.sender], "Not on presale whitelist");
        _;
    }
    ...
    function buyPresale(uint256 numberOfTokens)
        ...
        nonReentrant
    {
        require(saleIsActive && isPresale, "Presale is not active");
        require(
    ...
    function buy(uint256 numberOfTokens) external payable nonReentrant {
        require(saleIsActive && !isPresale, "Sale is not active");
        require(
            numberOfTokens <= maxMints,
            "Too many tokens for one transaction"
        );
