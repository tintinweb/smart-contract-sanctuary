...
contract animalMain is  ERC721,Ownable {
  ...
  event Create(uint _tokenId, bytes32 dna,uint8 star,uint8 look, uint16 roletype);
  ...
  function seeAnimalLook(uint256 _tokenId) public view returns (uint8 look) {
    return animals[_tokenId].look;
  }
  ...
  function createAnimal() public payable {
      ...
      uint id = animals.push(animal(dna, uint8(star),uint8(look), uint8(roletype))) - 1;
