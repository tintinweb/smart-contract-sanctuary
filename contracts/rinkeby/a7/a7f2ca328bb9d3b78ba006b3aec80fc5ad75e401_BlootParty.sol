...
contract BlootParty is ERC721Enumerable, ReentrancyGuard, Ownable {
    //Loot Contract
    ...
    function getWeapon(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "WEAPON", weaponOfChoice);
    }

    function getCrew(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "CREW", crew);
    }
    ...
    function getDoubleUp(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "DOUBLEUP", doubleUp);
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[9] = getVibe(tokenId);

        parts[10] = '</text><text x="10" y="120" class="base">';

        parts[11] = getDoubleUp(tokenId);
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Party #', toString(tokenId), '", "description": "Have the best Bloot Party ETH can buy! Bloot Party is basically worthless and thats OK. Free to use, inspired by Bloot.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    constructor() ERC721("Bloot Party", "BLTY") Ownable() {}
}
