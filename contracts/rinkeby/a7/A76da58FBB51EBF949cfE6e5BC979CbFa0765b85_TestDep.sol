...
contract TestDep is ERC721Enumerable, Ownable {
    ...
    /**
     * @notice Main contract configuration.
     * @member mintEnabled Switch to control minting.
     * @member perMint Current max amount allowed in mint().
     * @member perAccoutn Current max mints per account.
     * @member supply Total minted, offset for reserves.
     * @member max Final token id to be minted.
     * @member imagesHash Hash of all image hashes.
     */
    struct Conf {
        bool mintEnabled;
        uint8 perMint;
     ...
     */
    function setMetaHash(uint256 id, uint256 _hash) public onlyOwner {
    ...
    /**
     * @notice Sets URI for image hashes unless it is locked.
     */
    ...
    /**
     * @notice Sets minting state.
     */
    function setMintEnabled(bool mintEnabled) public onlyOwner {
        conf.mintEnabled = mintEnabled;
    }
