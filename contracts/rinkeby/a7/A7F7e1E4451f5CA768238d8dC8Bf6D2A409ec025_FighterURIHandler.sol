    ...
    ERC721HolderUpgradeable
{
    ...
    function initialize(address fighter, address confetti) public initializer {
        __Enhanceable_init(fighter);
    ...
    {
        uint256 seed = ISeeder(_fighter.getSeeder()).getSeedSafe(
            address(_fighter),
            tokenId
        );
        ...
        override(Enhanceable, IEnhanceable)
    {
        ...
        if (enhancement > 2) {
            _fighter.safeTransferFrom(msg.sender, address(this), burnTokenId);
            _fighter.burn(burnTokenId);
        }
...
abstract contract Enhanceable is IEnhanceable, Initializable, Seedable {
    ...
    IRaidERC721 private _token;

    function __Enhanceable_init(address token) public initializer {
    ...
    function enhance(uint256 tokenId, uint256) public virtual {
        ...
        ISeeder(_token.getSeeder()).requestSeed(_enhancementCounter);
    ...
    function _getSeed(uint256 tokenId) internal view returns (uint256) {
        return
            ISeeder(_token.getSeeder()).getSeedSafe(
                address(this),
                _enhancements[tokenId].id
            );
    }
}