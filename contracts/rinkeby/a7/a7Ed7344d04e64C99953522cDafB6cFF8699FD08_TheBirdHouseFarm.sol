...
*/
    ...
    mapping(uint256 => uint256) internal tokenIdToBlock;
     ...
     * @param _blockNumber The block number to calculate from.
     */

    function calculateRewardByBlock(uint256 _blockNumber)
        internal
        view
        returns (uint256)
    {
        uint256 blocksPassed = block.number - _blockNumber;
        return blocksPassed * EMISSIONS_RATE;
    }
        ...
        returns (uint256)
    {
        ...
        if (tokenIdToBlock[_tokenId] == 0) {
            return calculateRewardByBlock(startingBlock);
        } else {
            return calculateRewardByBlock(tokenIdToBlock[_tokenId]);
        }
    }
    ...
    function claimRewards() public returns (uint256) {
        ...
        for (uint256 i; i < tokenCount; i++) {
            ...
            rollingRewards = rollingRewards + viewRewardForToken(_tokenId);
            tokenIdToBlock[_tokenId] = block.number;
        }
