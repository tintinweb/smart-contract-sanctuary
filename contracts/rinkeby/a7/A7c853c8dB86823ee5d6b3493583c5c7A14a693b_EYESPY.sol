...
contract EYESPY is ERC721("EYESPY V27", "EYE") {
  using Strings for uint256;

  string public baseURI;
  string public baseExtension = ".json";
  uint256 public cost = 0.04 ether;
  uint256 public constant maxSupply = 2021;
  ...
  function mintPresale(uint256 _mintAmount) external payable {
    ...
    require(addressWhitelistBalance[msg.sender] + _mintAmount <= whitelistMaxMint);

    addressWhitelistBalance[msg.sender] += _mintAmount;
  ...
  function ownerMint(uint _mintAmount) external payable onlyOwner {
    require(!isSaleActive, "sale is not active");
    require(totalSupply + _mintAmount <= maxSupply);
    for (uint256 i = 1; i <= _mintAmount; i++) {
      _safeMint(msg.sender, ++totalSupply);
