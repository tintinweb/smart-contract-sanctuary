...
contract FreeBohoEvent is ERC721Holder, Ownable {
     ...
     */
    event Claim(address claimee, uint256 amount);
    ...
    function withdrawAll() public onlyOwner {
        ...
        for (uint256 i = 0; i < balance; i++) {
            bohoBonesContract.safeTransferFrom(
                address(this),
                msg.sender,
                bohoBonesContract.tokenOfOwnerByIndex(address(this), 0)
            );
        }
    }
     ...
     */
    function megaClaimFreeBohos() public eventIsActive isNotClaimed(msg.sender) {
        ...
        // Emit claim event
        emit Claim(msg.sender, freeBohoBonesClaims[msg.sender]);
