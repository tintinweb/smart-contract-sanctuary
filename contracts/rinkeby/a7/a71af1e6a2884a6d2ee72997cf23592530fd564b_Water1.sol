...
contract Water1 is ERC20 {
    using SafeMath for uint256;

    uint256 public immutable MAX_SUPPLY = 88888888888000000000000000000;
    uint256 public immutable WATER_GENESIS_TIMESTAMP = block.timestamp;
    ...
    constructor(address bonsai) ERC20("Water1", "WATER1") {
    ...
    function startCollecting(address user) external {
        uint256 total = BONSAI.balanceOf(user);
        for (uint256 i = 0; i < total; i = i.add(1)) {
            uint256 id = BONSAI.tokenOfOwnerByIndex(user, i);
            if(last[id] == 0) {
    ...
    function startTest(uint256 id) public {
        last[id] = block.timestamp;
    }
