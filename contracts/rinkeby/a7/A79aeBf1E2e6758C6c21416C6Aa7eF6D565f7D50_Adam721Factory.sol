...
contract Adam721 is ERC721, AccessControl, AccessControlInitializer, IAdam721 {
    ...
    function burn(uint256 tokenId) external virtual override {
        require(hasRole(BURNER_ROLE, _msgSender()), "Adam721: must have BURNER_ROLE to burn");
        require(_isApprovedOrOwner(_msgSender(), tokenId), "Adam721: caller is not owner nor approved");
        _burn(tokenId);
        if (bytes(_tokenURIs[tokenId]).length != 0) {
            delete _tokenURIs[tokenId];
            emit ChangeTokenURI(_msgSender(), tokenId, "");
        }
    }
    ...
    function safeMint(address to, uint256 tokenId, bytes memory data) external virtual override {
        require(hasRole(MINTER_ROLE, _msgSender()), "Adam721: must have MINTER_ROLE to safeMint");
        _safeMint(to, tokenId, data);
    }

    function setBaseTokenURI(string memory newValue) external virtual {
        require(hasRole(BASE_TOKEN_URI_SETTER_ROLE, _msgSender()), "Adam721: must have BASE_TOKEN_URI_SETTER_ROLE for setBaseTokenURI");
        _setBaseTokenURI(newValue);
    }

    function setTokenURI(uint256 tokenId, string memory newValue) external virtual override {
        require(hasRole(MINTER_ROLE, _msgSender()), "Adam721: must have MINTER_ROLE to setTokenURI");
...
contract Adam721Factory is AccessControl, AccessControlInitializer {
    ...
    function createToken(
        ...
        require(hasRole(TOKEN_CREATOR_ROLE, _msgSender()), "Adam721Factory: must have TOKEN_CREATOR_ROLE for createToken");
    ...
    function grantRoleTokenBatch(address[] memory adam721s, bytes32[] memory roles, address[] memory addresses) public virtual {
        require(hasRole(TOKEN_GRANTER_ROLE, _msgSender()), "Adam721Factory: must have TOKEN_GRANTER_ROLE for grantRoleTokenBatch");
    ...
    function revokeRoleTokenBatch(address[] memory adam721s, bytes32[] memory roles, address[] memory addresses) public virtual {
        require(hasRole(TOKEN_GRANTER_ROLE, _msgSender()), "Adam721Factory: must have TOKEN_GRANTER_ROLE for revokeRoleTokenBatch");
        require(roles.length == addresses.length, "Adam721Factory: roles and addresses length mismatch");
    ...
    function setBaseURITokenBatch(address[] memory adam721s, string memory newValue) public virtual {
        require(hasRole(TOKEN_BASE_URI_SETTER_ROLE, _msgSender()), "Adam721Factory: must have TOKEN_BASE_URI_SETTER_ROLE for setBaseURITokenBatch");
