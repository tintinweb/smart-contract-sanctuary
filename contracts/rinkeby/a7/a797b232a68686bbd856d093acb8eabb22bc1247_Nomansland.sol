...
contract SnoToken is ERC20, Ownable {
    ...
    address public auctionAddress = 0x806A9b71a117d1DAf21aDffd9C22D0C4D683fDF5;
...
contract Auction is Ownable {
    ...
    uint256 public constant START = 1628265600; // Aug 7, 2021 @ 00:00 GMT +8
    ...
    function claim(uint256 _roundID) public saleIsOpen {
        require(_roundID < getRoundID(), 'Round must end');
        uint256 totalRewards = rewards(msg.sender, _roundID);
        require(totalRewards > 0, 'Rewards must be more than 0.');
        claimed[msg.sender][_roundID] = bidders[msg.sender][_roundID];
        if(token.balanceOf(address(this)) < DAILY_SNOW){
           token.mint(address(this), DAILY_SNOW); 
        }
