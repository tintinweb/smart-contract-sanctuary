...
contract MessageMinter is Pausable, Ownable {
    ...
    function claim(uint256 tokenId, bytes memory signature) public whenNotPaused {
        require(hasClaimed[msg.sender][tokenId] == false, "AlreadyClaimed");

        bytes32 messageHash = keccak256(abi.encodePacked(address(this), msg.sender, tokenId));
        ...
        collection.mint(msg.sender, tokenId, 1, "");
    }
}
