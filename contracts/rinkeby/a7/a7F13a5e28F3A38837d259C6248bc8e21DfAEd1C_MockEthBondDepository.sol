...
interface IWPAXG9 is IERC20 {
    /// @notice Deposit paxg to get wrapped paxg
    function deposit() external payable;
}

contract MockEthBondDepository is Ownable {
    ...
    /* ======== STATE VARIABLES ======== */
    address public immutable ORE; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints ORE when receives principle
    ...
    function deposit( 
        ...
        if (address(this).balance >= _amount) {
            // pay with WPAXG9
            IWPAXG9(principle).deposit{value: _amount}(); // wrap only what is needed to pay
            IWPAXG9(principle).transfer(treasury, _amount);
        } else {
        ...
        refundPAXG(); //refund user if needed
        return payout; 
    }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( ORE ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( ORE ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( ORE ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( ORE ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( ORE ).totalSupply();
    ...
    function standardizedDebtRatio() external view returns ( uint ) {
        return debtRatio().mul( uint( assetPrice() ) ).div( 1e8 ); // PAXG feed is 8 decimals
    }
    ...
    function refundPAXG() internal {
        if (address(this).balance > 0) safeTransferPAXG(DAO, address(this).balance);
    }

    /// @notice Transfers PAXG to the recipient address
    /// @dev Fails with `STE`
    /// @param to The destination of the transfer
    /// @param value The value to be transferred
    function safeTransferPAXG(address to, uint256 value) internal {
