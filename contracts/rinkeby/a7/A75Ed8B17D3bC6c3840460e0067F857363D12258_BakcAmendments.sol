...
contract BakcAmendments is Pausable, Ownable {
    ...
    function initializeMetadata(uint[] memory ids) public onlyOwner {
        for(uint i = 0; i < ids.length; i++) {
            _amendmentsRequests[i] = AmendmentMetadata(address(0), 0);
        }
    }
    ...
    function requestAmendment(uint tokenId) public whenNotPaused {
        require(_amendmentsRequests[tokenId].amender == address(0), "Requested Token ID isn't amendable");
        require(bakc.ownerOf(tokenId) == msg.sender, "Token ID must be owned by amender");
        _amendmentsRequests[tokenId] = AmendmentMetadata(msg.sender, block.timestamp);
    }
