    ...
    AccessControlUpgradeable
{
    ...
    mapping(address => uint256) public avgIdx;
    bytes32 STRATEGIST_ROLE;
    ...
    function initialize(
        ...
        COMP = 0x2EAa9D77AE4D8f9cdD9FAAcd44016E746485bddb;
    ...
    function mint(
        ...
        avgIdx[msg.sender] += getCurrentRate() * _amount;
        
        _mint(msg.sender, getCurrentRate() * _amount / mentissa);
    ...
    // TotalValue in asset / totalShares supply  == assetPerShare

    // buy value / asset per share = amount you get
    ...
    /// @return value in native asset of the vault

    function curentVaultValue() public view returns(uint256 value){
       if(totalSupply() == 0 ){
           value = mentissa;   // 1 : 1 @ 0 supply
       }
       else if(totalSupply() > 0){
        CErc20 cToken = CErc20(collateral);
        value = cToken.exchangeRateStored() * cToken.balanceOf(address(this)) / mentissa;
       }
        ...
        returns (uint256 price)
    {
        if(totalSupply() == 0){
          return  mentissa;
           ...
           return totalSupply() * mentissa / curentVaultValue();
        }
