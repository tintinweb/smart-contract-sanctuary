    ...
    ReentrancyGuardUpgradeable
{
    ...
    function buyCoverInsurace(IDistributor.BuyInsuraceQuote memory quote) external override payable nonReentrant {
        ...
        if (quote.currency != ETH){
             ...
             if(IERC20Upgradeable(quote.currency).allowance(msg.sender,address(this)) >= quote.premium){
                  ...
                  // approve funds withdrawal to insurace contract
                IERC20Upgradeable(quote.currency).approve(
                        address(masterCover),
                        quote.premium
                    );
