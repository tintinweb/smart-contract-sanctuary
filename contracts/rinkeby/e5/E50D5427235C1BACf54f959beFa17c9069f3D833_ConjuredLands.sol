...
contract ConjuredLands is ReentrancyGuard, EscrowManagement, ERC721, ERC721Enumerable, Ownable, SignedMessages  {
    ...
    uint8 public premiumMintingSlots = 20;
    uint256 public salesStartTime = 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff; 
    ...
    ERC721(_name, _symbol)
    {
        ...
        //alignmentMaxSupply[0] = 3444; // good
        //alignmentMaxSupply[1] = 3444; // evil
        ...
        __baseURI = "ipfs://XXXXXXXXXXXXXXXXXXXXXX/";
    ...
    function tokenURI(uint256 tokenId) public view override returns(string memory) {
        if (getAlignmentByIndex(tokenId) != -1) {
    ...
    */
    ...
    function approveAirdropper(address contractAddress, bool approval) public onlyOwner(){
    ...
    function allowedToken(uint256 tokenId) public view returns(bool){
        // only tokens in the giveaway section are allowed for airdrops
        return (getAlignmentByIndex(tokenId) == -1);
    }

    function airdropperMint(address to, uint256 tokenId) public{
    ...
    function mintByAlignment_SignedMessage(uint8 _alignment, uint256 _numberOfTokens, uint256 _setPrice, uint256 expirationTimestamp, uint256 _nonce, bytes memory _sig) public payable{
        // check validity and execute
        require(expirationTimestamp <= block.timestamp, "Expired");
        bytes32 message = SignedMessages.prefixed(keccak256(abi.encodePacked(msg.sender, _alignment, _numberOfTokens, _setPrice, expirationTimestamp, _nonce)));
        require(msg.value == _setPrice, "Wrong payment");
        require(SignedMessages.consumePass(message, _sig, _nonce), "Error in signed msg");
