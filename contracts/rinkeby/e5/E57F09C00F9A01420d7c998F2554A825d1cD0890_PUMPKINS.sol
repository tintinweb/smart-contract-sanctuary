...
contract PUMPKINS is ERC721Creator {
    constructor() ERC721Creator("PUMPKINS V3", "PUMPKINS V3") {}
    
    function mint(string calldata uri) external returns (uint256) {
        return mintBase(msg.sender, uri);
    }
    
    function mintMetadataOnChain(string calldata uri) external returns (uint256) {
        string memory tokenURI = formatTokenURI(uri);
        return mintBase(msg.sender, tokenURI);
    }
    
    function mintImageOnChain(string calldata image) external returns (uint256) {
        string memory packedImage = string(abi.encodePacked(image));
        string memory baseURL = "data:image/svg+xml;base64,";
        string memory imageBase64Encoded = Base64.encode(bytes(string(abi.encodePacked(packedImage))));
        string memory imageURI = string(abi.encodePacked(baseURL, imageBase64Encoded));
            ...
            abi.encodePacked(
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked(
                            '{"name":"',
                            "PUMPKINS", // You can add whatever name here
                            '", "description":"on chain pumpkins", "attributes":"", "image":"',imageURI,'"}'
                        )
                    )
                )
            )
...
contract ERC721Creator is AdminControl, ERC721, ERC721CreatorCore {
     ...
     * @dev See {IERC721CreatorCore-mintBase}.
     */
    function mintBase(address to, string memory uri) public virtual override nonReentrant adminRequired returns(uint256) {
