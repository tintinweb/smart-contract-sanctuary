...
contract Forest is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    // Harvester earn 10000 $BATT per day
    uint public constant DAILY_BATT_RATE = 10000 ether;
    uint public constant MINIMUM_TIME_TO_EXIT = 5 minutes;
    uint public constant TAX_PERCENTAGE = 20;
    uint public constant TAX_PERCENTAGE_UNSTAKE = 40;
    uint public constant MAXIMUM_GLOBAL_BATT = 2400000000 ether;

    uint public totalBattEarned;

    uint public lastClaimTimestamp;
    uint public hunterReward;

    bool private _paused = false;
    // emergency rescue to allow unstaking without any checks but without $BATT
    ...
    modifier updateEarnings() {
        if (totalBattEarned < MAXIMUM_GLOBAL_BATT) {
            totalBattEarned += ((block.timestamp - lastClaimTimestamp) * totalHarvesterStaked * DAILY_BATT_RATE) / 1 days;
    ...
    function addTokensToStake(address account, uint16[] calldata tokenIds) external whenNotPaused updateEarnings {
    ...
    function _stakeHunters(address account, uint16 tokenId) internal {
        ...
        hunterStake[account].push(Stake({
            tokenId: uint16(tokenId),
            value: uint80(hunterReward)
        }));

        emit TokenStaked(account, tokenId, hunterReward);
    }

    function claimFromStake(uint16[] calldata tokenIds, bool unstake) external whenNotPaused updateEarnings {
        uint owed = 0;
        for (uint i = 0; i < tokenIds.length; i++) {
            if (!metaMoose.isHunter(tokenIds[i])) {
    ...
    function _claimFromHarvester(uint16 tokenId, bool unstake) internal returns (uint owed) {
        ...
        if (totalBattEarned < MAXIMUM_GLOBAL_BATT) {
            owed = ((block.timestamp - stake.value) * DAILY_BATT_RATE) / 1 days;
        } else if (stake.value > lastClaimTimestamp) {
            owed = 0; // $BATT production stopped already
        } else {
            owed = ((lastClaimTimestamp - stake.value) * DAILY_BATT_RATE) / 1 days; // stop earning additional $BATT if it's all been earned
        }
        if (unstake) {
            if (owed > 0) {
                // pay 40% tax
                uint tax = owed * TAX_PERCENTAGE_UNSTAKE / 100;
                _payTax(tax);
                owed = owed - tax;
            }
            totalHarvesterStaked -= 1;
    ...
    function randomHunterOwner() external view returns (address) {
        if (totalHunterStaked == 0) return address(0x0);

        uint holderIndex = randomGen.getRandomNumber(totalHunterStaked, hunterHolders.length);
