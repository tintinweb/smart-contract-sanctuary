...
contract NiftyTreasury is NiftyAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    /**
     * @notice allow approved address to deposit an asset for NIFTY
    ...
    function deposit(
        ...
        // mint NIFTY needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        NIFTY.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn NIFTY for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        NIFTY.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new NIFTY using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        NIFTY.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or NIFTY from the treasury, using sNIFTY as collateral. This might allow an
     * sNIFTY holder to provide NIFTY liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(NIFTY)) {
            require(permissions[STATUS.NIFTYDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sNIFTY.changeDebt(value, msg.sender, true);
        require(sNIFTY.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(NIFTY)) {
            NIFTY.mint(msg.sender, value);
            niftyDebt = niftyDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sNIFTY.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with NIFTY
     * @param _amount uint256
     */
    function repayDebtWithNIFTY(uint256 _amount) external {
        require(
            permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.NIFTYDEBTOR][msg.sender],
            notApproved
        );
        NIFTY.burnFrom(msg.sender, _amount);
        sNIFTY.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        niftyDebt = niftyDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(NIFTY), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SNIFTY) {
            sNIFTY = IsNIFTY(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SNIFTY) {
            // 9
            sNIFTY = IsNIFTY(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(NIFTY.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns NIFTY valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(NIFTY)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return NIFTY.totalSupply() - niftyDebt;
    }
}
