...
contract NFTI is IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    function setRevilTime(uint256 time_) public {
    ...
    function buyItem(uint256 numberOfNfts) public payable {
        ...
        for (uint256 i = 0; i < numberOfNfts; ++i) {
            _safeMint(msg.sender, _getNewRandomTokenId());
        }
        ...
        IERC20(_nctAddress).mintTokens(msg.sender, numberOfNfts * _BUY_COST);
        _owner.transfer(msg.value);
    }

    function burn(uint256 tokenId) public {
        require(msg.sender == _tokenOwners.get(tokenId, "ERC721: owner query for nonexistent token"), "You are not owner of token");
        ...
        IERC20(_nctAddress).mintTokens(msg.sender, _BURN_COST);
    }

    function upgradeToken(uint256 tokenId) public returns (uint256) {
        require(isRevilEnd(), "Revil is not ended");
        require(msg.sender == ownerOf(tokenId), string(abi.encodePacked("You are not owner ", tokenId.toString())));
        ...
        require(IERC20(_nctAddress).balanceOf(msg.sender) >= _UPGRADE_COST, "Not enought ERC20 tokens");
        ...
        for (uint256 offset = 0; offset < _UPGRADE_SUPPLY_PER_PERSON; ++offset) {
            ...
            if (!_exists(upgradeTokenId)) {
                IERC20(_nctAddress).transferFrom(msg.sender, address(this), _UPGRADE_COST);
                IERC20(_nctAddress).burn(_UPGRADE_COST);
                _safeMint(msg.sender, upgradeTokenId);
    ...
    function claim() public returns (uint256) {
        require(_privateTokens[msg.sender] != 0, "You don't have private tokens");

        _safeMint(msg.sender, _privateTokens[msg.sender]);
        _privateTokens[msg.sender] = 0;

        return _privateTokens[msg.sender];
    }
}