...
struct Draw { uint256 winnerPlayerIndex; bool prizeClaimed; address[] players; uint256 prizePool; State drawEnded; uint256 deadline; uint256 winnerPrize; uint256 numberOfTickets; }
...
// MODIFIERS
    ...
    modifier drawComplete(uint256 drawIndex) {
        ...
        require(draw.drawEnded == State.Closed,                       "This draw is still ongoing");
        _;
    }
...
// PUBLIC FUNCTIONS
    ...
    {
        Draw storage draw = draws[getCurrentDraw()];                                  // Shortcut accessor for the Draw
        require(draw.drawEnded == State.Open,                                         "The lottery has ended. Please check if you won the price!");
        ...
        if (numberOfPlayers > 0 ) {
            if (!isVRFActive) {
                ...
                draw.drawEnded = State.Closed;                                        // Flag draw as ended
            } else {
                randomContract.getRandomNumber(getCurrentDraw());                     // Request random number
                draw.drawEnded = State.Pending;
            }
...
// VRF callback
    ...
    {
        Draw storage draw = draws[drawIndex];                                         // Shortcut accessor for the Draw
        require(draw.drawEnded != State.Pending,                                      "This draw is needs to be pending");
        uint256 numberOfPlayers = draw.players.length;                                // Store the number of players in a local variable saves gas
