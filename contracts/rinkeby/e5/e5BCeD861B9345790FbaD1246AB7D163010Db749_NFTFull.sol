...
contract NFTFull is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 public whitelistingPrice;
     ...
     *  @dev REVEAL_TIMESTAMP must be 0
     */
    function setRevealTimestamp(uint256 _revealTimeStamp) public onlyOwner {
        require(REVEAL_TIMESTAMP == 0, "Already Revealed");
        REVEAL_TIMESTAMP = _revealTimeStamp;
        emit RevealTimestamp(_revealTimeStamp);
    }

    /*
     *  @title Set max gas price
     *  @param maximum gas price
     *  @dev Caller must be contract owner
     */
    ...
    function adjustSettings(
        uint256 _price,
        uint256 _whitelistingPrice,
        uint256 _maxNFTPerWallet
    ) external onlyOwner {
        price = _price;
        whitelistingPrice = _whitelistingPrice;
        maxNFTPerWallet = _maxNFTPerWallet;
    }
     ...
     *  @param address to add to whitelist
     *  @param number of tokens to send to this address
     *  @dev Caller must be contract owner
     */
    function addToWhitelist(address whitelistAddress, uint256 numberOfTokens)
        public
        onlyOwner
    {
        whitelist[whitelistAddress] = numberOfTokens;
    }
     ...
     *  @dev Caller must be contract owner
     */
    function flipWhitelistSaleIsActive() external onlyOwner {
     ...
     *  @dev Called must be a contract
     *  @dev Whitelist sale must be active
     *  @dev must not be exceeding maximum number of tokens per wallet
     *  @dev must send enough ETH for the transaction
     */
    function whitelistMint(uint256 _numberOfTokens)
        public
        payable
        nonReentrant
    {
        ...
        require(
            whitelist[msg.sender] >= _numberOfTokens,
            "Not whitelisted to mint this many tokens"
        );
        require(whitelistSaleIsActive, "Whitelist sale not Active");
        require(
            _numberOfTokens + balanceOf(msg.sender) <= maxNFTPerWallet,
            "Over Max per wallet!"
        );
        require(
        ...
        require(
            whitelistingPrice * _numberOfTokens <= msg.value,
            "Not Enough ETH!"
        );
        whitelist[msg.sender] -= _numberOfTokens;
     ...
     */
    function mintNFTs(uint256 _numberOfTokens) public payable nonReentrant {
        ...
        require(price * _numberOfTokens <= msg.value, "Not Enough ETH!");
        if (maxGasPrice != 0) {
