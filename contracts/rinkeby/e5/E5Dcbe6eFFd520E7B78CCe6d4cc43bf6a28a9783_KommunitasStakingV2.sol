...
contract KommunitasStakingV2 is Ownable {
    ...
    event EmergencyUnlocked (address indexed _of, uint256 _amount);
     ...
     /* ========== CONSTRUCTOR ========== */
    constructor(address _komToken, address _komVToken, uint256 _apy, uint256[3] memory _lockPeriod, uint256[3] memory _multiplier, uint256 _minPrivateSale) {
        ...
        minPrivateSale = _minPrivateSale;
    }
    ...
    function stake(uint256 _amount, uint256 _lockPeriod) isStakePaused public {
        ...
        locks[msg.sender].push(TokenLock(staked[_lockPeriod].lockPeriodIndex, userId, _amount, block.timestamp, matureUntil, lockReward, 0, false, false));
    ...
    function unlock(uint256 _sid) public returns (uint256) {
        ...
        locks[msg.sender][_sid].claimedTime = block.timestamp;
    ...
    function preMatureWithdraw(uint256 _sid, uint256 _amount) external returns (uint256) {
        ...
        if(locks[msg.sender][_sid].amount == _amount){
            ...
            locks[msg.sender][newSid].claimedTime = block.timestamp;
            locks[msg.sender][newSid].premature = true;
    ...
    /**
     * @dev Gets the total Staked of all Users start from private sale
     */
    function getTotalStakedMinPrivateSale() public view returns(uint256 total){
        total = 0;
        
        for(uint256 i=0; i<userStaked.length; i++){
            if(getUserStakedTokens(userStaked[i]) >= minPrivateSale){
                total += getUserStakedTokens(userStaked[i]);
            }
        }
    }
    ...
    /**
     * @dev Gets the total staking info
     */
    function getStakedInfo() public view returns(KomStaked memory first, KomStaked memory second, KomStaked memory third){
        first = staked[lockPeriod[0]];
        second = staked[lockPeriod[1]];
        third = staked[lockPeriod[2]];
    }
    ...
    function setMinPrivateSale(uint256 _minPrivateSale) public onlyOwner{
        require(_minPrivateSale > 0, "Can't be 0");
        minPrivateSale = _minPrivateSale;
    }
