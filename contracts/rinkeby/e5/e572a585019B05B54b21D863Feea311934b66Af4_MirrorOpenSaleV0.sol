...
import {IMirrorOpenSaleV0, IMirrorOpenSaleV0Events} from "./interface/IMirrorOpenSale.sol";
    ...
    Reentrancy
{
        ...
        Reentrancy.nonReentrant
    {
        ...
        _withdraw(saleConfig_.operator, saleConfig_.token, tokenId, h, saleConfig_.recipient, msg.value);
    }
    ...
    function _withdraw(
        ...
        uint256 feePercentage = IMirrorFeeRegistry(feeRegistry).getProducerFee(token);
    ...
    function _royaltyInfo(
        ...
        if (
            ...
            (royaltyRecipient, royaltyAmount) = IERC2981(token)
...
contract MirrorFeeRegistry is IMirrorFeeRegistry, Ownable {
    ...
    function updateFee(uint256 newFee) external override onlyOwner {
        require(newFee < maxFee, "must be less than max fee");

        globalFee = newFee;
    }

    function waiveFee(address account) external override onlyOwner {
