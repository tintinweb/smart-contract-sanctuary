...
/// @title The Shapes ERC-721 token
...
import { IShapesToken } from './interfaces/IShapesToken.sol';
...
contract ShapesToken is ERC721, IShapesToken, Ownable {
    // An address who has permissions to mint Shapes
    address public minter;

    // The Shapes token URI descriptor
    IShapesDescriptor public descriptor;

    // The Shapes token seeder
    IShapesSeeder public seeder;
    ...
    // The shape seeds
    mapping(uint256 => IShapesSeeder.Seed) internal seeds;

    // The internal shape ID tracker
    uint256 private _currentShapeId;
     ...
     * @notice Constructor inherits from ShapesDescriptor and ShapesSeed
     */
    constructor(
        address entropySourceAddress,
        IShapesDescriptor _descriptor,
        IShapesSeeder _seeder,
        IProxyRegistry _proxyRegistry
    ) ERC721('Shapes', 'SHAPE_BLURP', entropySourceAddress) {
    ...
    // TODO FIGURE OUT HOW WE WANT TO INSTIGATE A MINT. NOUNS DID NOT DO IT THIS WAY
    // TINYBOXES HAS A create fn
    /**
     * @notice Mint a Shape.
     * @dev Call _mintTo with the to address(es).
     */
    function mint(address recipient) public override returns (uint256) {
        notSoldOut();
        return _mintTo(recipient, _currentShapeId++);
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), 'ShapesToken: URI query for nonexistent token');
     ...
     */
    function setDescriptor(IShapesDescriptor _descriptor) external override onlyOwner whenDescriptorNotLocked {
        descriptor = _descriptor;
     ...
     * @dev Only callable by the owner when not locked.
     */
    function setSeeder(IShapesSeeder _seeder) external override onlyOwner whenSeederNotLocked {
        seeder = _seeder;
    ...
    /**
     * @notice Mint a Shape with `shapeId` to the provided `to` address.
     */
    function _mintTo(address to, uint256 shapeId) internal returns (uint256) {
        IShapesSeeder.Seed memory seed = seeds[shapeId] = seeder.generateSeed(shapeId, descriptor);

        _mint(owner(), to, shapeId);
        emit ShapeCreated(shapeId, seed);
...
/// @title Interface for ShapesToken
...
interface IShapesToken is IERC721 {
    event ShapeCreated(uint256 indexed tokenId, IShapesSeeder.Seed seed);

    event MinterUpdated(address minter);

    event MinterLocked();

    event DescriptorUpdated(IShapesDescriptor descriptor);

    event DescriptorLocked();

    event SeederUpdated(IShapesSeeder seeder);
    ...
    function setDescriptor(IShapesDescriptor descriptor) external;

    function lockDescriptor() external;

    function setSeeder(IShapesSeeder seeder) external;
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, AccessControl {
    ...
    address payable trosky = payable(0x7A832c86002323a5de3a317b3281Eb88EC3b2C00);
     ...
     */
    constructor(string memory name_, string memory symbol_, address entropySourceAddress) {
        _name = name_;
        _symbol = symbol_;
