...
contract NightLifeStaking is Context, Ownable {
    ...
    function stake(uint256 _amount) public {
        uint256 lpBalance = lpToken.balanceOf(_msgSender());
        require(_amount > 0, "Invalid stake amount");
        require(lpBalance >= _amount, "Staker does not have enough balance");
        ...
        if (_stake.amount > 0) {
            ...
            uint256 _fee = reward.div(100);
            uint256 _reward = reward.sub(_fee);
            rewardToken.transfer(_msgSender(), _reward);
            rewardToken.transfer(NLIFEWallet, _fee);
            _stake.amount = _stake.amount.add(_amount);
            emit Withdraw(_msgSender(), _reward);
            emit Withdraw(NLIFEWallet, _fee);
        } else {
    ...
    function unstake() public {
        ...
        lpToken.transfer(_msgSender(), _amount);
