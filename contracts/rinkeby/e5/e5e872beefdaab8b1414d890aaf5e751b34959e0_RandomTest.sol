...
contract RandomTest {
    ...
    function decompose(uint256 r_, uint256 size_) public pure returns(uint256[] memory) {
        uint256 count = randomnessCount(size_);
        uint256[] memory result = new uint256[](count);
        for (uint256 i = 0; i < count; ++i) {
            result[i] = r_.mod(size_);
            r_ = r_.div(size_);
        }
        return result;
    }
    ...
    function useRandomness(
        ...
        uint256 randomness = psuedoRandomness();

        bool[] memory filled = new bool[](size_);
        ...
        uint256 t;
        uint256 remaining = size_.sub(count.mul(nftMapping[collectionId_].length));
        ...
        for (i = 0; i < count; ++i) {
            ...
            t = randomness.mod(remaining);
            randomness = randomness.div(remaining);
