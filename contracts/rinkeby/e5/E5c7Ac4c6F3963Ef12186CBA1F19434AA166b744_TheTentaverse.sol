...
contract TheTentaverse is ERC721EnumerableLite, Signed {
  ...
  function resurrect( address[] calldata recipients, uint[] calldata tokenIds ) external onlyDelegates{
    require(tokenIds.length == recipients.length,   "Must provide equal tokenIds and recipients" );
    for(uint i; i < tokenIds.length; ++i ){
      require( !_exists( tokenIds[i] ), "Can't resurrect existing token" );
      _transfer( address(0), recipients[i], tokenIds[i] );
      --_burned;
    }
  }
