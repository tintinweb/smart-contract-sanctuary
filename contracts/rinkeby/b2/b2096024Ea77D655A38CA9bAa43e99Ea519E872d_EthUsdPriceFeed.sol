...
library Witnet {
  ...
  function asErrorMessage(Result memory _result) public pure returns (ErrorCodes, string memory) {
    ...
    if (errorCode == ErrorCodes.SourceScriptNotCBOR && error.length >= 2) {
      ...
      errorMessage = "The structure of the request is invalid and it can not be parsed";
      } else if (errorCode == ErrorCodes.BridgePoorIncentives) {
      errorMessage = "The wit cost of the request exceeds the limit";
      } else if (errorCode == ErrorCodes.BridgeOversizedResult) {
      errorMessage = "The request result length exceeds the limit";
      }
     else {
...
contract BitcoinPriceRequest is Request {
  constructor () public Request(hex"") { }
}
