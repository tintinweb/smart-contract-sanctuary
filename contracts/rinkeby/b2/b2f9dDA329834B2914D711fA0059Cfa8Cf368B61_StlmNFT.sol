...
contract StlmNFT is ERC721("Living Module v3", "STLMv3"), Ownable {
     ...
     */
    constructor(uint256 _baseTokenId, string memory _baseUri) public {
    ...
    function mint(address _beneficiary, address _creator, string memory _tokenUri) external onlyOwner returns (uint256) {
        ...
        emit StlmCreatorUpdated(tokenId, _creator);
        return tokenId;
    }
    ...
    function batchMint(address[] calldata _owners, address[] calldata _creators, string[] calldata _uris) external onlyOwner {
        ...
        for(uint256 i=0; i<_owners.length; i++) {
            ...
            emit StlmCreatorUpdated(tokenId, creators[tokenId]);
        }
    }
