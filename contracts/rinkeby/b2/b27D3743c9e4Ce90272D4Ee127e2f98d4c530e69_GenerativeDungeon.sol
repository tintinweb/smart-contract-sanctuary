...
contract GenerativeDungeon is ERC721Enumerable, Ownable {
    ...
    uint256 public WALLET_LIMIT_PLUS_ONE = 4; // per mint wallet limit of 3
    uint256 public WHITELIST_WALLET_LIMIT_PLUS_ONE = 3; // per mint wallet limit of 2
    ...
    struct AccountInfo {
        ...
        bool isWhitelist; // whether or not this address is on the whitelist
    }

    mapping(address => AccountInfo) public accountInfoList;

    // we feed in our our array of whitelisted addresses here (use external vs public for performance benefits/to save gas when passing in large arrays)
    function setWhitelist(address[] memory _addresses) external onlyOwner {
        for(uint i = 0; i < _addresses.length; i++) {
            accountInfoList[_addresses[i]].isWhitelist = true;
        }
    }
    ...
    function reserveNfts() public onlyOwner {        
        ...
        for (uint256 i=1; i < 31; i++) { // start at Token #1
    ...
    */
    function mint(uint256 _numberOfTokens) public payable nonReentrant {
        ...
        if (whitelistSaleIsActive) {
            require(accountInfoList[msg.sender].isWhitelist, "Not Whitelisted");
