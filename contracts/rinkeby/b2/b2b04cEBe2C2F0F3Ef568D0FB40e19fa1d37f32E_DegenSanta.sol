...
contract DegenSanta is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    uint256 private constant PRICE = 2.5 * 10**16; // 0.025ETH Per DS
    uint256 private constant PRICE_PRESALE = 2.5 * 10**16; // 0.025ETH Per DS
    uint256 private constant PRICE_PREMINT = 0; // Free Per DS

    uint256 private constant MAX_ELEMENTS = 3333 ; // 3333 DSes in CrowdSale
    uint256 private constant MAX_ELEMENTS_PRESALE = 3 * 10**2; // 300 DSes in PreSale
    uint256 private constant MAX_ELEMENTS_PREMINT = 333; // 333 DSes for GiveAway.
    ...
    uint256 private constant MAX_MINT_PREMINT = 333; // Upper Limit per Mint is 28

    address private constant developerAddress = 0x09964eAB43f1bd45Ab98416Dd94d16C8b76AB5Aa; // Wallet Address of Developer
    ...
    event CreateDegenSanta(address to, uint256 indexed id);

    modifier saleIsOpen {
        if (_msgSender() != owner()) {
            require(SALE_OPEN == true, "SALES: Please wait a big longer before buying DSes ;)");
        }
    ...
    constructor (string memory baseURI) ERC721("DEGEN SANTA", "DS") {
        setBaseURI(baseURI);
    ...
    function mint(address payable _to, uint256[] memory _ids) public payable saleIsOpen {
        ...
        require(total <= _maxElements, "MINT: Please go to the Opensea to buy DS.");
        require(_ids.length <= _maxMint, "MINT: Current count exceeds maximum mint count.");
        
         if (_to != owner()) {
            require(msg.value >= price(_ids.length), "MINT: Current value is below the sales price of DS");
        }
