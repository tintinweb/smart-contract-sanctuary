...
abstract contract Ownable is Context {
   ...
   */
  modifier groupOwner() {
    require(getOwner(msg.sender), "GroupOwner: caller is not the owner");
    _;
  }
  ...
  function getOwner(address newOwner) public virtual returns (bool) {
    return ownerByAddress[newOwner];
  }

  function getOwners() public virtual returns (address[] memory) {
    return owners;
  }
}
