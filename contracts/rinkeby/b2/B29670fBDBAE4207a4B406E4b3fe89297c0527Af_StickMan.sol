...
contract StickMan is ERC721, Pausable, Ownable, ERC721Burnable, RandomlyAssigned {
   
    //uint256 public constant MAX_SUPPLY = 3333; //MAX general supply
    uint256 public constant MAX_SUPPLY = 10; //MAX general supply
    uint256 public constant MAX_ODDERS_PER_MINT = 5;
    uint256 public constant ODDER_MINT_PRICE = 0.0002 ether; // 0.02 ETH
    bool public ODDER_GENERAL_SALE_STARTED = false;
    bool public ODDER_PRESALE_STARTED = false;
    string public ODDER_BASE_URI = "ipfs://QmTuuwKaW1Jpr33KMSp1cwjb27C7GxbJafDxXXL82m2oWU/";
    mapping(address => uint256) whiteListMintCount;
    
    //event debugLog(string msg, uint256 value);

    constructor() ERC721("StickMan", "STIK")  RandomlyAssigned(MAX_SUPPLY, 1) {}
    ...
    function setWhitelistBulk(address[] memory addr,uint[] memory numberOfOdders) public onlyOwner {
        for(uint256 i = 0; i < addr.length; i++){
            whiteListMintCount[addr[i]] = numberOfOdders[i];
        }
    }
