...
//MINT
    ...
    function presale(uint256 _reserveAmount, address _whitelister) external payable isPlayer {
    ...
    function mint() external payable isPlayer returns (uint256 id) {
    ...
    function _mintElf(address _to) internal returns (uint16 id) {
        ...
        uint256 rand = _rand();
         ...
         {        
           ...
           sentinels[id] = DataStructures._setElf(elf.owner, elf.timestamp, elf.action, elf.campaign, elf.attackPoints, elf.primaryWeapon, elf.level, _traits, _class);
        }
...
//GAMEPLAY//
    ...
    function doAction(uint256[] calldata ids, uint action_) external payable isPlayer {
          require(isGameActive);

          for (uint256 index = 0; index < ids.length; index++) {  
            _doAction(ids[index], action_, msg.sender);
          }
    }

    function _doAction(uint256 id_, uint action, address elfOwner) internal {
            ...
            sentinels[id_] = DataStructures._setElf(elf.owner, elf.timestamp, elf.action, elf.campaign, elf.attackPoints, elf.primaryWeapon, elf.level, actions.traits, actions.class);
            emit Action(msg.sender, action, id_); 
    }

    function sendCampaign(uint256[] calldata ids, uint256 campaign_, uint256 sector_, bool rollWeapons_, bool rollItems_, bool useitem_) external payable isPlayer {
          require(isGameActive);

          for (uint256 index = 0; index < ids.length; index++) {  
            _sendCampaign(ids[index], campaign_, sector_, rollWeapons_, rollItems_, useitem_, msg.sender);
          }
    }

      function _sendCampaign(uint256 id_, uint256 campaign_, uint256 sector_, bool rollWeapons, bool rollItems, bool useItem, address elfOwner) internal {
            ...
            DataStructures.ActionVariables memory actions;
                    ...
                    elf.inventory = useItem ? 0 : elf.inventory;

                    elf.inventory = rollItems ? campaigns.rollItems(elf.level) : elf.inventory;

                    (elf.weaponTier, elf.primaryWeapon) = rollWeapons ? campaigns.rollWeapons(elf.level, sector_) : (elf.weaponTier, elf.primaryWeapon);
            ...
            sentinels[id_] = DataStructures._setElf(elf.owner, elf.timestamp, elf.action, elf.campaign, elf.attackPoints, elf.primaryWeapon, elf.level, actions.traits, actions.class);
            emit Action(msg.sender, 2, id_); 
    }

    function heal(uint256 magicId, uint256[] calldata _elves) public isPlayer {
        
        require(isGameActive);
        DataStructures.Elf memory dElf = DataStructures.getElf(sentinels[magicId]);
        require(ownerOf[magicId] == msg.sender || dElf.owner == msg.sender);
     
        require(dElf.sentinelClass == 0); 

        DataStructures.ActionVariables memory dActions;
       
        dElf.timestamp = block.timestamp.add(12 hours);
        dElf.action = 7;

        //uint levelTier = dElf.level.div(20).add(1);
        
        dElf.level = dElf.level.add(3);
        dElf.level = dElf.level > 100 ? 100 : dElf.level; 
        
        for (uint256 index = 0; index < _elves.length; index++) {

                   DataStructures.Elf memory elf = DataStructures.getElf(sentinels[(_elves[index])]);
                   DataStructures.ActionVariables memory actions;
            ...
            sentinels[_elves[index]] = DataStructures._setElf(elf.owner, elf.timestamp, elf.action, elf.campaign, elf.attackPoints, elf.primaryWeapon, elf.level, actions.traits, actions.class);

        }

        dActions.traits = DataStructures.packAttributes(dElf.hair, dElf.race, dElf.accessories);
        dActions.class =  DataStructures.packAttributes(dElf.sentinelClass, dElf.weaponTier, dElf.inventory);
                       
        sentinels[magicId] = DataStructures._setElf(dElf.owner, dElf.timestamp, dElf.action, dElf.campaign, dElf.attackPoints, dElf.primaryWeapon, dElf.level, dActions.traits, dActions.class);
...
//ADMIN
    ...
    function setElfManually(uint id, uint8 _primaryWeapon, uint8 _weaponTier, uint8 _attackPoints, uint8 _level, uint8 _inventory) external {
        onlyOwner();
        ...
        sentinels[id] = DataStructures._setElf(elf.owner, elf.timestamp, elf.action, elf.campaign, elf.attackPoints, elf.primaryWeapon, elf.level, actions.traits, actions.class);
