...
contract Marketplace is Ownable, Pausable, MarketplaceStorage, NativeMetaTransaction {
  ...
  /**
    ...
    * @param _kmonToken - Address of the main ERC20 accepted for this marketplace
    * @param _ownerCutPerMillion - owner cut per million
    * @param _kmonOwnerCutPerMillion - owner cut per million for KMON payments
    */
  constructor (
    address _kmonToken,
    uint256 _ownerCutPerMillion,
    uint256 _kmonOwnerCutPerMillion,
    address _owner
  )
    public
  {
    ...
    setKmonOwnerCutPerMillion(_kmonOwnerCutPerMillion);

    require(_owner != address(0), "Invalid owner");
    transferOwnership(_owner);

    require(_kmonToken.isContract(), "The accepted token address must be a deployed contract");
    kmonToken = ERC20Interface(_kmonToken);
    acceptedToken[_kmonToken] = true;
    acceptedToken[0x0000000000000000000000000000000000000000] = true;
  }
  ...
  /**
    * @dev Sets the share cut for the owner of the contract that's
    *  charged to the seller on a successful sale
    * @param _kmonOwnerCutPerMillion - Share amount, from 0 to 999,999
    */
  function setKmonOwnerCutPerMillion(uint256 _kmonOwnerCutPerMillion) public onlyOwner {
    require(_kmonOwnerCutPerMillion < 1000000, "The owner cut should be between 0 and 999,999");

    kmonOwnerCutPerMillion = _kmonOwnerCutPerMillion;
    emit ChangedKmonOwnerCutPerMillion(kmonOwnerCutPerMillion);
  }
  ...
  /**
    ...
    */
    function acceptToken(address token) external onlyOwner {
        require(acceptedToken[token] == false, "Token already accepted");
        acceptedToken[token] = true;
  }

  /**
    * @dev Revoke a token as an exchange unit on the Market
    * - Can only be called by market admin
    * @param token Token address
    */
    function revokeToken(address token) external onlyOwner {
        require(acceptedToken[token] == true, "Token already revoked");
  ...
  /**
    ...
    * @param paymentToken - Payment token - 0x0 for ETH/BNB
    ...
    internal
  {
    ...
    require(acceptedToken[paymentToken], "_createOrder::The payment token is not valid");
    ...
    if (publicationFeeInWei > 0) {
      require(
        kmonToken.transferFrom(sender, owner(), publicationFeeInWei),
   ...
   internal returns (Order memory)
  {
    ...
    require(order.paymentToken == _paymentToken, "The payment token is not correct");
    ...
    _transferMoney(_paymentToken, _price, seller, sender);
  ...
  {
    require(acceptedToken[paymentToken], "_transferMoney::The payment token is not valid");
    
    uint256 fee = _calculateFee(paymentToken, price);

    if (paymentToken == address(0)) {
      require(msg.value == price, "_transferMoney::The price is not correct");
      ...
      require(
        ERC20Interface(paymentToken).transferFrom(sender, owner(), fee),
        "_transferMoney::Transfering the cut to the Marketplace owner failed"
      );

      // Transfer sale amount to seller
      require(
        ERC20Interface(paymentToken).transferFrom(sender, seller, price.sub(fee)),
  ...
  /**
    ...
    */
  function _calculateFee(address paymentToken, uint256 price) internal view returns (uint256 fee) {
    if (paymentToken == address(kmonToken)) {
      if (kmonOwnerCutPerMillion > 0) {
        // Calculate sale share
        fee = price.mul(kmonOwnerCutPerMillion).div(1000000);
      }
    } else {
      if (ownerCutPerMillion > 0) {
        // Calculate sale share
        fee = price.mul(ownerCutPerMillion).div(1000000);
      }
    }
  }
...
contract MarketplaceStorage {
  ERC20Interface public kmonToken;
  ...
  uint256 public kmonOwnerCutPerMillion;
  ...
  event ChangedKmonOwnerCutPerMillion(uint256 kmonOwnerCutPerMillion);
}
