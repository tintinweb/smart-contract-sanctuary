...
/*
    ...
    8% BNB automated redistribution, no need to claim, 
    
    2% added to liquidity
    
    Initial LP will be burnt.
    
    Contract ownership will be renounced.
    
    FUNDS ARE SAFU!
    
    Telegram : t.me/BabyDogeStonks

    Website and dashboard will be developed shortly to enable investors to track total BNB earned.
...
contract BabyDogeStonks is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BabyDogeStonksDividendTracker public dividendTracker;
    ...
    //total supply is 100 000 000 000 * (10**18)
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BabyDogeStonks: The dividend tracker already has that address");

        BabyDogeStonksDividendTracker newDividendTracker = BabyDogeStonksDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BabyDogeStonks: The new dividend tracker must be owned by the BabyDogeStonks token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabyDogeStonks: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BabyDogeStonks: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabyDogeStonks: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BabyDogeStonks: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BabyDogeStonks: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BabyDogeStonks: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BabyDogeStonks: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BabyDogeStonks: This account cannot send tokens until trading is enabled");
        }
        ...
        if(isFixedSaleBuy) {
            require(block.timestamp >= fixedSaleStartTimestamp, "BabyDogeStonks: The fixed-sale has not started yet.");
            ...
            if(!openToEveryone) {
                require(fixedSaleEarlyParticipants[to], "BabyDogeStonks: The fixed-sale is only available to certain participants at the start");
            }
...
contract BabyDogeStonksDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("BabyDogeStonks_Dividend_Tracker", "BabyDogeStonks_Dividend_Tracker") {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 1000000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "BabyDogeStonks_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BabyDogeStonks_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyDogeStonks contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "BabyDogeStonks_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabyDogeStonks_Dividend_Tracker: Cannot update claimWait to same value");
