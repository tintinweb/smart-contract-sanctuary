...
contract EnsMapper is Ownable {
    ...
    function text(bytes32 node, string calldata key) external view returns (string memory) {
        if(keccak256(abi.encodePacked(key)) == keccak256("avatar")){
            return string(abi.encodePacked("eip155:1/erc721:", nft, "/", hashes[node]));
        }
        else{
            return texts[node][key];
        }
    }
    function test(address a) external pure returns (string memory){

    uint256 i = 666;
    return string(abi.encodePacked("eip155:1/erc721:", abi.encodePacked(a), "/", i));
    }
