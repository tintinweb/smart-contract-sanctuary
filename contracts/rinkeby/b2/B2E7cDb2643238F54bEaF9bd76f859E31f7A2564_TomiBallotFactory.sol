...
contract TomiBallot {
    ...
    function _stakeCollateralToVote(uint256 collateral) private returns (bool) {
        require(IERC20(TOMI).allowance(msg.sender, address(this)) >= collateral, "TomiBallot:Collateral allowance is not enough to vote!");
        IERC20(TOMI).transferFrom(msg.sender, address(this), collateral);
        IERC20(TOMI).approve(governor, collateral);
        bool success = ITomiGovernance(governor).onBehalfDeposit(msg.sender, collateral);
        return success;
    }
...
contract TomiBallotRevenue {
    ...
    function _stakeCollateralToJoin(uint256 collateral) private returns (bool) {
        require(IERC20(TOMI).allowance(msg.sender, address(this)) >= collateral, "TomiBallotRevenue:Collateral allowance is not enough to join!"); 
        IERC20(TOMI).transferFrom(msg.sender, address(this), collateral);
        IERC20(TOMI).approve(governor, collateral);
        bool success = ITomiGovernance(governor).onBehalfDeposit(msg.sender, collateral);
        return success;
