...
// we might be able to delete this -- if we can find a way to convert a single uint into a one-element array in one line.
    ...
    // function getCompaniesByRange2(uint _memberID) public view returns (company[]){
    //     uint[] memory _companies = new uint[](2);
    //     return offers[_companies];
    // }

    // function getCompaniesByRange(uint startIndex, uint endIndex) public returns (company[]) {
    //     require(endIndex < companies.length,"endIndex out of range");
    //     uint[] memory _companies = new uint[](endIndex - startIndex);
    //     return _companies;
    // }
