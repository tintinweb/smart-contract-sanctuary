...
contract RBTStake is Owned{
    ...
    function StakeTokens(address _ofUser, uint256 _tokens) external onlySaleContract returns(bool){
        if(pendingReward(_ofUser) > 0) {
            purchasedRBT[_ofUser].pending = purchasedRBT[_ofUser].pending.add(pendingReward(_ofUser));
        }
    ...
    function ClaimReward() external isPurchaser isClaimable {
        ...
        purchasedRBT[msg.sender].pending = 0;
    ...
    function _calculateReward(uint256 toDate, address _user) internal view returns(uint256) {
        uint256 totalStakeTime = (toDate.sub(purchasedRBT[_user].stakeDate)).div(2 minutes); // in days
    ...
    function pendingReward(address _user) public view returns(uint256){
        ...
        return reward.sub(rewardRBT[_user]).add(purchasedRBT[_user].pending);
    }
