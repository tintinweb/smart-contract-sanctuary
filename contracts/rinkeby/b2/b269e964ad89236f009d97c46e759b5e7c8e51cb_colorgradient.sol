...
contract colorgradient is ERC721Enumerable, Ownable,ReentrancyGuard {
    ...
    string[] private colors = ["#3FB330", "#0CA5AD", "#F475AD", "#FA7737", "#A05B30", "#A13045", "#C276F5", "#0076F5", "#E876F5", "#C4ADC7", "#FAEE82", "#1F3F2A", "#9D3691", "#997395", "#F5539C",
    "#FFF119", "#FFA25C", "#234E3A", "#549E6F", "#DA9EDE", "#4F242C", "#1F71BB", "#D63A42", "#85BFC9", "#DAA4D6", "#783B00", "#000000", "#AAAAB3", "#D93998", "#5A3671",
    "#946A63", "#1C6A63", "#9184B3", "#F0A17A", "#709436", "#42FCA2", "#5CFFDC", "#FCF942", "#34FFFF", "#8047D9", "#D0ABD9", "#369CA1", "#A05394", "#ED7842", "#FF001A",
    "#2DE0FF", "#13606E", "#61A773", "#E0ABD3", "#FF90E4", "#E0D8D3", "#C2BDFF", "#A6001E", "#28B2E0", "#4A002B", "#6B00AA", "#FFBABD", "#FFBA13"];
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721) returns (string memory) {
        ...
        uint32 hexColorB = _hexColors2[tokenId];
         hexColorA = hexColorA / 17;
         hexColorB = hexColorB / 17;
        string memory hexStringA = colors[hexColorA];
        string memory hexStringB = colors[hexColorB];
        //string memory image = Base64.encode(bytes(generateSVGImage(hexStringA,hexStringB)));
        
       string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "colorGrads #', toString(tokenId), '", "description": "ColorGrads are fully on-chain, randomly generated colors."', '"image": "data:image/svg+xml;base64,', 
       Base64.encode(bytes(generateSVGImage(hexStringA,hexStringB))), '"}'))));
...
/*        return string(
    ...
    function getHexColors(uint256 tokenId) public view returns (string memory) {
        ...
        string memory hexStringB = colors[hexColorB];
        return string(
            abi.encodePacked(hexStringA,'"and"',hexStringB
            )
        );
    }
    
    function generateRandomHexColor(uint256 tokenId) public returns (uint32,uint32) {
        uint32 hexColorA = uint32(_rng(tokenId) % 1000);
        uint32 hexColorB = uint32(_rng(tokenId) % 1000);

        while (existingHexColorsA[hexColorA] || existingHexColorsA[hexColorB]) {
          if(existingHexColorsA[hexColorA]){
            hexColorA = uint32(uint256(hexColorA + block.timestamp * tokenId) % 1000);}
          else{
            hexColorB = uint32(uint256(hexColorB + block.timestamp) % 1000);} 
        }
    ...
    function _rng(uint256 tokenId) internal returns(uint256) {
        ...
        uint256 rand;
        if (rand_counter % 2 == 0){
            rand = uint256(keccak256(abi.encodePacked("Color One",toString(tokenId),counter)));
            return rand;
        }
        else{
          rand = uint256(keccak256(abi.encodePacked("Color two", toString(tokenId), counter)));
          return rand;
        }
    }
