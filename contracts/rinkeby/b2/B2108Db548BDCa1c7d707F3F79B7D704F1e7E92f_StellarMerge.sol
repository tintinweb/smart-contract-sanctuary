...
contract StellarMerge is ERC721Tradable {
    ...
    constructor(address _proxyRegistryAddress)
        ERC721Tradable("Staller", "ES", _proxyRegistryAddress) {
    ...
    function tokenURI(uint256 uTokenId) public view virtual override returns (string memory) {
        require(_exists2(uTokenId), "ERC721: operator query for nonexistent token");
    ...
    function _transfer2(address from, address to, uint256 uTokenId) internal {
        require(_exists2(uTokenId), "ERC721: transfer attempt for nonexistent token");
    ...
    function ownerOf(uint256 uTokenId) public view override returns (address) {
        require(_exists2(uTokenId), "ERC721: owner query for nonexistent token by zm");
        // bytes memory byteString;
        // byteString = abi.encodePacked(byteString, 'uTokenId:', uTokenId.toString());
        // require(uTokenId > 0, string(byteString)); 
    ...
    function _exists2(uint256 uTokenId) internal view returns (bool) {
        return m_mapValues[uTokenId] != 0;
    }

    function _isApprovedOrOwner(address operator, uint256 uTokenId) override internal view virtual returns (bool) {
        require(_exists2(uTokenId), "ERC721: operator query for nonexistent token");
        address owner = m_mapOwners[uTokenId];
    ...
    function _burn2(uint256 uTokenId) internal {
        address owner = ownerOf(uTokenId);
        _approve(address(0), uTokenId);
