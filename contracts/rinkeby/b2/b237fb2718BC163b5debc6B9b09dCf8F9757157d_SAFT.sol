...
contract SAFT is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 100000000000000000000 * 10**3;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    address public marketingWallet;

    string private _name = "Simple-As-Fuck-Token-1";
    string private _symbol = "SAFT1";
    ...
    uint256 public _maxTxAmount = 1000000000000000000 * 10**3;
    uint256 public numTokensSellToAddToLiquidity = 1000000000000000000 * 10**3;
    ...
    function transferMany(address[] memory to, uint256 amount) external {
    ...
    function airdrop(address recipient, uint256 amount) external onlyOwner() {
        removeAllFee();
        _transfer(_msgSender(), recipient, amount * 10**3);
        restoreAllFee();
    }
    ...
    function airdropArray(address[] calldata newholders, uint256[] calldata amounts) external onlyOwner(){
        ...
        while(iterator < newholders.length){
            airdropInternal(newholders[iterator], amounts[iterator] * 10**3);
            iterator += 1;
        }
    }
    ...
    function setSwapThresholdAmount(uint256 SwapThresholdAmount) external onlyOwner() {
        ...
        numTokensSellToAddToLiquidity = SwapThresholdAmount * 10**3;
    }
    ...
    function _transfer(
        ...
        if (antiSnipeGlobal) {
            ...
            //when buy
            if (antiSnipeBuffer && uniswapV2Pair == from && !_isExcludedMaxTransactionAmount[to]) {
                blacklistBot(to, true);
            }

            //when not a buy
            else if (botBlacklist[from] && uniswapV2Pair!=from && !_isExcludedMaxTransactionAmount[from]) {
        ...
        if (limitsInEffect) {
            if (
                ...
                //when buy
                if (uniswapV2Pair == from && !_isExcludedMaxTransactionAmount[to]) {
                    require(amount <= maxTransactionAmount, "Buy transfer amount exceeds the maxTransactionAmount.");
                    require(amount + balanceOf(to) <= maxWallet, "Max wallet exceeded");
                }

                //when sell
                else if (uniswapV2Pair == to && !_isExcludedMaxTransactionAmount[from]) {
