    ...
    ERC721Burnable
{
    ...
    uint256 public maxTokenSupply = 8888; // total available dragons
    ...
    // Indicate to OpenSea that an NFT's metadata is no longer changeable by anyone
    ...
    function setRoyaltyPercentage(uint96 _royaltiesPercentage)
        public
        onlyOwner
    {
    ...
    {
        return append(super.tokenURI(tokenId), "/metadata.json");
    }
    ...
    function reserveNFT(string memory _hash)
        external
        onlyOwner
        ...
        returns (uint256 tokenId)
    {
        ...
        emit Mint(newItemId);
        emit PermanentURI(_hash, newItemId);
        return newItemId;
    }
