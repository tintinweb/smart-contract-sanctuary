...
contract SvgTest is ERC721Enumerable, Ownable {
	...
	function generatePaths() private pure returns (string memory result) {
		for (uint8 i = 0; i < 25; i++) {
			uint256 scale1 = 1 + (i * 3);
			uint256 rotation1 = 1 + (i * 3);
			uint256 scale2 = 1 + (i * 3);
			uint256 rotation2 = 1 + (i * 3);
			uint256 scale3 = 1 + (i * 3);
			uint256 rotation3 = 1 + (i * 3);
			uint256 scale4 = 1 + (i * 3);
			uint256 rotation4 = 1 + (i * 3);
		...
		returns (string memory)
	{
		return
			string(
				abi.encodePacked(
					'<path d="M29,60.125c57.6456,0 116.23006,3 169,3" stroke-width="calc(2 / ',
					path.scale1,
