...
contract LockTOS is LockTOSStorage, AccessibleCommon, ILockTOS {
        ...
        returns (LibLockTOS.Point memory lastWeek)
    {
        ...
        while (pointTimestampIterator != timestamp) {
            ...
            //     timestamp
            // );
            pointTimestampIterator = timestamp;
            if(timestamp > pointTimestampIterator.add(epochUnit) ) pointTimestampIterator = pointTimestampIterator.add(epochUnit);
    ...
    function pointHistoryLength() external view returns (uint256) {
