...
abstract contract RelayerManager is BaseModule {
   ...
   */
  function isExecutedTx(address _wallet, bytes32 _signHash)
    external
    view
  ...
  function refund(
    ...
    // Only refund when the owner is one of the signers was used
    if (_gasPrice > 0 && (_option == OwnerSignature.Required)) {
      address refundAddress = _refundAddress == address(0)
        ? msg.sender
        : _refundAddress;
      uint256 refundAmount;
...
abstract contract SecurityManager is BaseModule {
  ...
  modifier onlyWhenRecovery(address _wallet) {
    require(
      recoveryConfigs[_wallet].executeAfter > 0,
      'SM: no ongoing recovery'
    );
    _;
  }
  ...
  modifier onlyGuardianOrSelf(address _wallet) {
    require(
      _isSelf(msg.sender) || isGuardian(_wallet, msg.sender),
   ...
   */
  function executeRecovery(address _wallet, address _recovery)
    external
    onlySelf
    notWhenRecovery(_wallet)
  {
    validateNewOwner(_wallet, _recovery);
    uint64 executeAfter = uint64(block.timestamp + recoveryPeriod);
    recoveryConfigs[_wallet] = RecoveryConfig(
      _recovery,
      executeAfter,
      uint32(guardianStorage.guardianCount(_wallet))
    );
    _setLock(
      _wallet,
      block.timestamp + lockPeriod,
      SecurityManager.executeRecovery.selector
    );
   ...
   * @param _wallet The target wallet.
   */
  function finalizeRecovery(address _wallet)
    external
    onlyWhenRecovery(_wallet)
  {
    RecoveryConfig storage config = recoveryConfigs[_wallet];
    require(
      uint64(block.timestamp) > config.executeAfter,
      'SM: ongoing recovery period'
    );
   ...
   * @param _wallet The target wallet.
   */
  function cancelRecovery(address _wallet)
    external
    onlySelf
    onlyWhenRecovery(_wallet)
  {
   ...
   */
  function transferOwnership(address _wallet, address _newOwner)
    external
    onlySelf
   ...
   * @param _wallet The target wallet.
   */
  function getRecovery(address _wallet)
    external
    view
    returns (
      address _address,
   ...
   * @param _wallet The target wallet.
   */
  function lock(address _wallet)
    external
    onlyGuardianOrSelf(_wallet)
    onlyWhenUnlocked(_wallet)
  {
    _setLock(
      _wallet,
      block.timestamp + lockPeriod,
      SecurityManager.lock.selector
    );
   ...
   * @param _wallet The target wallet.
   */
  function unlock(address _wallet)
    external
    onlyGuardianOrSelf(_wallet)
    onlyWhenLocked(_wallet)
  {
    require(
      locks[_wallet].locker == SecurityManager.lock.selector,
      'SM: cannot unlock'
    );
    _setLock(_wallet, 0, bytes4(0));
   ...
   */
  function getLock(address _wallet)
    external
    view
    returns (uint64 _releaseAfter)
  {
    ...
    onlyWhenUnlocked(_wallet)
  {
    ...
    require(
      config.pending[id] == 0 ||
        block.timestamp > config.pending[id] + securityWindow,
   ...
   */
  function confirmGuardianAddition(address _wallet, address _guardian)
    external
    onlyWhenUnlocked(_wallet)
  {
    bytes32 id = keccak256(abi.encodePacked(_wallet, _guardian, 'addition'));
   ...
   * @param _guardian The guardian.
   */
  function cancelGuardianAddition(address _wallet, address _guardian)
    external
    onlyWalletOwnerOrSelf(_wallet)
    onlyWhenUnlocked(_wallet)
  {
    ...
    onlyWalletOwnerOrSelf(_wallet)
  {
    ...
    config.pending[id] = block.timestamp + securityPeriod;
    emit GuardianRevokationRequested(
      _wallet,
      _guardian,
      block.timestamp + securityPeriod
    );
   ...
   * @param _guardian The guardian.
   */
  function confirmGuardianRevokation(address _wallet, address _guardian)
    external
  {
    bytes32 id = keccak256(abi.encodePacked(_wallet, _guardian, 'revokation'));
    GuardianManagerConfig storage config = guardianConfigs[_wallet];
   ...
   * @param _guardian The guardian.
   */
  function cancelGuardianRevokation(address _wallet, address _guardian)
    external
    onlyWalletOwnerOrSelf(_wallet)
    onlyWhenUnlocked(_wallet)
  {
   ...
   * @return _isGuardian `true` if the address is a guardian for the wallet otherwise `false`.
   */
  function isGuardianOrGuardianSigner(address _wallet, address _guardian)
    external
    view
    returns (bool _isGuardian)
  {
    (_isGuardian, ) = Utils.isGuardianOrGuardianSigner(
      guardianStorage.getGuardians(_wallet),
      _guardian
    );
  }
   ...
   */
  function guardianCount(address _wallet)
    external
    view
  ...
  function getGuardians(address _wallet)
    external
    view
    returns (address[] memory _guardians)
  {
...
/**
 * @title SquirrelModule
 * @notice Single module for the Argent wallet.
 * @author Julien Niset - <[email protected]>
 */
contract SquirrelModule is
  BaseModule,
  SecurityManager,
  RelayerManager,
  TransactionManager
{
  bytes32 public constant NAME = 'SquirrelModule';
  ...
  constructor(
    ...
    BaseModule(_guardianStorage, _authoriser, NAME)
    SecurityManager(
      _recoveryPeriod,
      _securityPeriod,
      _securityWindow,
   ...
   * @inheritdoc IModule
   */
  function addModule(address _wallet, address _module)
    external
    override
    onlyWalletOwnerOrSelf(_wallet)
    onlyWhenUnlocked(_wallet)
    ...
    returns (uint256, OwnerSignature)
  {
    ...
    if (
      ...
      methodId == SquirrelModule.addModule.selector ||
    ...
    if (methodId == SecurityManager.executeRecovery.selector) {
      ...
      uint256 numberOfSignaturesRequired = _majorityOfGuardians(_wallet);
    ...
    if (methodId == SecurityManager.cancelRecovery.selector) {
      ...
      uint256 numberOfSignaturesRequired = Utils.ceil(
        recoveryConfigs[_wallet].guardianCount + 1,
        2
      );
      return (numberOfSignaturesRequired, OwnerSignature.Optional);
    }
    if (
      methodId == TransactionManager.multiCallWithGuardians.selector ||
      methodId == SecurityManager.transferOwnership.selector
    ) {
      // owner + majority of guardians
      uint256 majorityGuardians = _majorityOfGuardians(_wallet);
      uint256 numberOfSignaturesRequired = majorityGuardians + 1;
      return (numberOfSignaturesRequired, OwnerSignature.Required);
    }
    if (
      methodId == SecurityManager.finalizeRecovery.selector ||
    ...
    if (
      methodId == SecurityManager.lock.selector ||
  ...
  function _majorityOfGuardians(address _wallet)
    internal
    view
    returns (uint256)
  {
...
abstract contract TransactionManager is BaseModule {
   ...
   * @param _wallet The target wallet.
   */
  function enableERC1155TokenReceiver(address _wallet)
    external
    onlyWalletOwnerOrSelf(_wallet)
    onlyWhenUnlocked(_wallet)
  {
...
abstract contract BaseModule is IModule {
   ...
   */
  function _isOwner(address _wallet, address _addr)
    internal
    view
    returns (bool)
  {
...
library Utils {
  ...
  function isGuardianOrGuardianSigner(
    ...
    for (uint256 i = 0; i < _guardians.length; i++) {
      if (!isFound) {
        ...
        if (
          isContract(_guardians[i]) && isGuardianOwner(_guardians[i], _guardian)
        ) {
          isFound = true;
          continue;
        }
   ...
   */
  function isGuardianOwner(address _guardian, address _owner)
    internal
    view
    returns (bool)
  {
