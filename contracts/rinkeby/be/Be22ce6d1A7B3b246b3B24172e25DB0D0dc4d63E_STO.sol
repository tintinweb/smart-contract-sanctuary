...
contract STO is Ownable {
    ...
    mapping(address => bool) public isWhitelistedMapping;
    mapping(address => uint256) public userRoleMapping;
    mapping(uint256 => address) public propertyListerMapping; // mapping to save who minted tokens for that property while propertyListerMapping
    // a mapping to save the tokenvalue per token for each token
    mapping(uint256 => uint256) public propertyValueMapping;
    ...
    constructor(address _creatorCoinAddress) {
    ...
    function whitelistAddress(
        ...
        isWhitelistedMapping[_userAddress] = _status;
        userRoleMapping[_userAddress] = _userRole;
    }
    ...
    function create(
        ...
        require(
            isWhitelistedMapping[_to],
            "you can't perform this operation for this user."
        );
        require(
            userRoleMapping[_to] == 2,
            "Only investor can list his property."
        );
        creatorCounter++;
        // saving who minted the property tokens
        propertyListerMapping[creatorCounter] = _to;

        // saving how much each token is worth for this property
        propertyValueMapping[creatorCounter] = _propertyValue;

        totalSupply[creatorCounter] = totalSupply[creatorCounter].add(
            _noOfTokens
        );
    ...
    function buy(uint256 _id, uint256 _usdtTokenAmount) external {
        ...
        require(
            creatorCoinInstance.balanceOf(propertyListerMapping[_id], _id) >=
                _propertyTokenAmount,
            "Property lister has insufficient tokens"
        );

        // send usdt from user to contract
