...
contract Multicall is Ownable, ReentrancyGuard {
    function allCubeTokens(CubePool pool) external view returns (CubeToken[] memory _cubeTokens) {
        _cubeTokens = new CubeToken[](pool.numCubeTokens());
        for (uint256 i = 0; i < pool.numCubeTokens(); i++) {
            _cubeTokens[i] = pool.cubeTokens(i);
        }
    }

    function getCubeTokenInfo(CubePool pool, CubeToken cubeToken)
        external
        view
        returns (
            string memory name,
