...
contract ERC20FundedCrowdsale is Crowdsale, AllowanceCrowdsale, WhitelistCrowdsale {
    ...
    uint256 private _weiRaised; // How many FundingTokens have been raised
     ...
     */
    function _weiAmount(uint256 fundingTokenAmount) internal view returns (uint256) {
    ...
    function _forwardFunds(uint256 fundingTokenAmount) internal {
        fundingToken().safeTransferFrom(msg.sender, wallet(), _weiAmount(fundingTokenAmount));
    }
    ...
    function _processPurchase(address beneficiary, uint256 tokenAmount) internal {
        ...
        // Not delivering tokens, will be vested later in batches
    ...
    function _preValidatePurchase(address beneficiary, uint256 weiAmount) internal view {
        ...
        require(weiAmount > 1000000000000000000, "Crowdsale: minimum investment is 5000 USDC");
...
contract Crowdsale is Context, ReentrancyGuard {
     ...
     */
    function buyTokens(address beneficiary, uint256 fundingTokenAmount) public nonReentrant payable {
//        uint256 weiAmount = msg.value;
        uint256 weiAmount = _weiAmount(fundingTokenAmount);
        _preValidatePurchase(beneficiary, weiAmount);

        // calculate token amount to be created
        uint256 numberTokensPurchased = _getTokenAmount(weiAmount);
        _weiRaised = _weiRaised.add(weiAmount);

        _processPurchase(beneficiary, numberTokensPurchased);
        emit TokensPurchased(_msgSender(), beneficiary, weiAmount, numberTokensPurchased);
