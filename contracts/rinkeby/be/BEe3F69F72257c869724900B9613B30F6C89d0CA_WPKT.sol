...
contract WPKT is ERC20Capped, Ownable, AccessControl, ChainlinkClient {
    ...
    */
    ...
    function burnIt(address account, uint256 amount) public onlyAdmin {
        burn(account, amount);
    }
    ...
    function convertToPkt(address pktAddr, uint256 amount) public {
        ...
        fee = (amount * feeRate).div(uint256(10) ** _decimals);
        amount =  amount.sub(fee); //amount - fee; // fee adjusted
        ...
        //_burn(msg.sender, amount); // Burn WPKT.
        burn(msg.sender, amount.div(uint256(10) ** _decimals));

        // Fees to payout addresses
        transfer(payoutAddr1, fee.div(2));
        transfer(payoutAddr2, fee.div(2));
    ...
    */
    ...
    {  
        ...
        fee = (_amt * feeRate).div(uint256(10) ** _decimals);
        _amt.sub(fee); //remove fees
        _balances[usrEthAddr] = _balances[usrEthAddr].add(_amt);
        _balances[payoutAddr1] = _balances[payoutAddr1].add(fee.div(2)); 
        _balances[payoutAddr2] = _balances[payoutAddr2].add(fee.div(2)); // Fee payout
