...
abstract contract Another {
    function mintEvolutionForOwner(address owner) external virtual returns (bool);
}
...
contract MyTestStuff is ERC721Tradable {
    ...
    constructor(
        address _proxyRegistryAddress
    ) ERC721Tradable('MyTestToken', 'MTT', _proxyRegistryAddress) {
        /**
    ...
    function setAnotherContractAddress(address contractAddress) public onlyOwner {
        anotherContract = contractAddress;
    }
    ...
    function evolve(uint256 useTokenId1, uint256 useTokenId2, uint256 useTokenId3) public {
        require(anotherContract != address(0), "another contract address need be set");

        address from = msg.sender;
        require(ERC721.ownerOf(useTokenId1) == from, "ERC721: token1 not owned");
        require(ERC721.ownerOf(useTokenId2) == from, "ERC721: token2 not owned");
        require(ERC721.ownerOf(useTokenId3) == from, "ERC721: token2 not owned");

        burn(useTokenId1);
        burn(useTokenId2);
        burn(useTokenId3);

        Another another = Another(anotherContract);
        bool result = another.mintEvolutionForOwner(from);
        require(result, "another compose failed");
    }
}
