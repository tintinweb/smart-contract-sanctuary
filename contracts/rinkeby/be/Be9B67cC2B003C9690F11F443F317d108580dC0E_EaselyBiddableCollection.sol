 ...
 */
contract EaselyBiddableCollection is ERC721Enumerable, Ownable, IEaselyContractInitializer {    
    using Strings for uint256;
    ...
    /** 
     * @dev Verifies if an order is both active and that it is signed properly.
     */
    function _validHash(
        address owner,
        uint256 version,
        ...
        bytes32 hash = _hashToCheck(owner, version, tokenId, pricesAndTimestamps, ipfsHash, claimedIpfsHash, claimable);
        if (_cancelledOrFinalizedSales[hash] || ecrecover(hash, v, r, s) != owner) {
            return false;
        }
    ...
    /** 
     * @dev Helper external method to determine if {buyExistingToken} will error or not. Logic is duplicated
     * because the actual method also returns a different message for each part that can error.
     */
    function isExistingTokenBuyable(
        address seller,
        uint256 version,
        uint256 tokenId,
        uint256[4] memory pricesAndTimestamps,
        ...
        uint256 startPrice = pricesAndTimestamps[0];
        uint256 endPrice = pricesAndTimestamps[1];
        uint256 startTime = pricesAndTimestamps[2];
        uint256 endTime = pricesAndTimestamps[3];

        if (startPrice < endPrice || block.timestamp < startTime || startTime >= endTime) {
            return false;
        }

        if(_addressToActiveVersion[seller] != version || ownerOf(tokenId) != seller) {
            return false;
        }

        if (!_validHash(seller, version, tokenId, pricesAndTimestamps, "", "", false, v, r, s)) {
            return false;
        }
    ...
    /** 
     * @dev Helper external method to determine if {buyNewToken} will error or not. Logic is duplicated
     * because the actual method also returns a different error message for each part that can error.
     */
    function isNewTokenBuyable(
        bool claimable,
        uint256 version,
        uint256[4] memory pricesAndTimestamps,
        string memory ipfsHash,
        ...
        // Cannot use these variables due to Solidity: Stack too deep
        // uint256 startPrice = pricesAndTimestamps[0];
        // uint256 endPrice = pricesAndTimestamps[1];
        // uint256 startTime = pricesAndTimestamps[2];
        // uint256 endTime = pricesAndTimestamps[3];

        if (pricesAndTimestamps[0] < pricesAndTimestamps[1] || 
            block.timestamp < pricesAndTimestamps[2] || 
            pricesAndTimestamps[2] >= pricesAndTimestamps[3]) {
            return false;
        }

        if(_addressToActiveVersion[owner()] != version) {
            return false;
        }

        if (!_validHash(owner(), version, 0, pricesAndTimestamps, ipfsHash, claimedIpfsHash, claimable, v, r, s)) {
            return false;
        }
    ...
    function claimToken(uint256 tokenId) external {
        ...
        require(ownerOf(tokenId) == msgSender, "Only current tokenOwner can claim");
        ...
        if (bytes(_tokenIdToPostClaimURI[tokenId]).length > 0) {
            _safeMint(msgSender, nextTokenId);
            ...
            // once we set the URI of the new NFT we can delete it from the map to save gas
    ...
    function settleAuction(uint256 tokenId) external {
        ...
        if (tokenOwner == owner()) {
            ...
            address[] memory ownerRoaylties = new address[](1);
            uint256[] memory ownerBPS = new uint256[](1);
            ownerRoaylties[0] = owner();
            ownerBPS[0] = secondaryOwnerBPS;

            IEaselyPayout(payoutContractAddress).splitPayable{ value: auction.price }(tokenOwner, ownerRoaylties, ownerBPS);
        }
