...
contract ePING is Context, IERC20, Ownable {
    ...
    //     require(msg.sender == caller, "ePing: Not Owner");
    //     _;
    // }
    ...
    constructor  () public{
        ...
        IUniswapV2Router02 _PancakeSwapV2Router = IUniswapV2Router02(0xA61a3cB59a01f13E3C2373932d093870671e8964); //BSC Testnet
     ...
     function toggleTaxFee() external onlyOwner {
        isTakefee = !isTakefee;
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        _transfer(_msgSender(), recipient, amount , takeFee);
        return true;
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
         ...
         {
             require(isTakefee == true , "ePing: Take Fee Mode Is not on");
             takeFee = true;
        //    uint256 taxFee = 10000000000000000000;
        //    uint256 tax = taxFee *(amount) / 100000000000000000000 ; 
        //    amount = amount + tax;
         }
        _transfer(sender, recipient, amount , takeFee);
...
//!(_isExcludedFromFee[from] || _isExcludedFromFee[to])
    ...
    function _burn(address from, uint value) internal  {
       ...
       //require(allowance(from , _msgSender())  <= value , "ePING: Owner is not approved to burn the  mentioned amount of tokens");
        _approve(from, _msgSender(), _allowances[from][_msgSender()].sub(value, "ERC20: transfer amount exceeds allowance"));
         _tOwned[from] = _tOwned[from].sub(value); 
        //balances[from] = balances[from].sub(value);
        // balances[from] = balances[from].sub(value); // - value;
       // sub from _towned
    ...
    function mint(address from , uint value) external  onlyOwner override{
       _mint(from , value);
      }
    
     function burn(address from , uint value) external onlyOwner override{
        _burn(from , value);
      }
