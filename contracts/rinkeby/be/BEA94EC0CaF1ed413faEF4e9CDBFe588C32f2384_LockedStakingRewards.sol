...
contract LockedStakingRewards is Ownable, ReentrancyGuard {
    ...
    uint256 public constant depositDuration = 1 days;
    ...
    function withdraw(uint256 _tokenAmount, uint256 _pool) external nonReentrant {
        require(isTransferPhase(_pool), "pool is locked currently");
        uint256 _sharesAmount = tokenToShares(_tokenAmount, _pool);
    ...
    function getPoolInfo(uint256 _pool) public view returns(bool, Pool memory) {
        return (isTransferPhase(_pool), pool[_pool]);
    }
