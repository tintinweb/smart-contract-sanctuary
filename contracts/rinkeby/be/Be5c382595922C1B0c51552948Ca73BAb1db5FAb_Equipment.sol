...
/// @title RaidParty Equipment ERC-1155 Contract
...
contract Equipment is IEquipment, ERC1155, AccessControlEnumerable {
    // Contract state and constants
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    IEquipmentURIHandler private _handler;
    ...
    function uri(uint256 id) public view override returns (string memory) {
        return _handler.uri(id);
    }

    function setHandler(IEquipmentURIHandler handler)
        external
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _handler = handler;
        emit HandlerUpdated(_msgSender(), address(handler));
    }
