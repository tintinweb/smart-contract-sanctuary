    ...
    PausableUpgradeable
{
    ...
    function setVaultConfig(address _vaultConfig) external onlyOwner {
        l1VaultConfig = IL1VaultConfig(_vaultConfig);
    }
    ...
    /// @param _destinationNetwork chain id of the destination network
    /// @param _destination Address of the receiver on the L2 network
    /// @param _token Address of the ERC20 token
    /// @param _amount Amount need to be send
    ...
    modifier notOverMaxCap(address _token, uint256 _amount) {
        ...
        require(
            _tokenBalance + _amount <= l1VaultConfig.getMaxAssetCap(_token),
...
interface IL1VaultConfig is IVaultConfigBase {
    function allowToWithdraw(address _token) external view returns (bool);

    function getMaxAssetCap(address _token) external view returns (uint256);
...
interface IVaultConfigBase {
    ...
    function setReceiptTokenFactoryAddress(address _receiptTokenFactoryAddress)
        external;
