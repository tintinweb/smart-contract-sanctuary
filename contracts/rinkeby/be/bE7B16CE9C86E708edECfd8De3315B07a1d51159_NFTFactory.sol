...
contract NFTFactory is ERC721, ERC2981PerTokenRoyalties, Ownable {
    ...
    constructor(string memory baseURI) ERC721('Just Frens', 'FRENS') {
        setBaseURI(baseURI);
    }
    ...
    /// @param royaltyValue the royalties as2ked for (EIP2981)
    function mint(
        address to,
        address royaltyRecipient,
        uint256 royaltyValue
    ) external {
        _mint(to, royaltyRecipient, royaltyValue);
    }
    ...
    function mintBatch(
        ...
        for (uint256 i; i < recipients.length; i++) {
            _mint(recipients[i], royaltyRecipients[i], royaltyValues[i]);
        }
    }
}
