...
contract PolkaBridgeINO is ERC1155, Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(uint256 => mapping(address => User))) public whitelist; // poolId, packageId, userId - listuser
    mapping(uint256 => mapping(uint256 => Package)) public poollist; // (poolId, packageId) -> package
    ...
    function addWhitelist(address user, uint256 pid) public onlyOwner {
        ...
        for (uint256 i = 1; i <= pools[poolIndex].TotalPackageCount; i++)
        {
            uint256 packageId = poollist[pid][i].Id;
            whitelist[pid][packageId][user].Id = pid;
            whitelist[pid][packageId][user].UserAddress = user;
            whitelist[pid][packageId][user].IsWhitelist = true;
            whitelist[pid][packageId][user].WhitelistDate = block.timestamp;
            whitelist[pid][packageId][user].IsActived = true;
        }
    }
        ...
        public onlyOwner        
    {
        ...
        for (uint256 i = 0; i < user.length; i++) {
            for (uint256 j = 1; j <= pools[poolIndex].TotalPackageCount; j++) {
                uint256 packageId = poollist[pid][j].Id;
                whitelist[pid][packageId][user[i]].Id = pid;
                whitelist[pid][packageId][user[i]].UserAddress = user[i];
                whitelist[pid][packageId][user[i]].IsWhitelist = true;
                whitelist[pid][packageId][user[i]].WhitelistDate = block.timestamp;
                whitelist[pid][packageId][user[i]].IsActived = true;
            }
        }
    }
    ...
    function updateWhitelist(
        ...
        {
            uint256 packageId = poollist[pid][i].Id;
            whitelist[pid][packageId][user].IsWhitelist = isWhitelist;
            whitelist[pid][packageId][user].IsActived = isActived;
        }
    }

   // Check if user IsWhitelist
    function IsWhitelist(address user, uint256 pid) public view returns (bool) {
        return whitelist[pid][1][user].IsWhitelist;
    }
    ...
    function addPackageToPool(
        ...
        pools[poolIndex].TotalPackageCount = pools[poolIndex].TotalPackageCount.add(1);
        uint256 lastIndex = packages.length.sub(1);
        poollist[_PoolId][id] = packages[lastIndex];
    }
    ...
    //     return whitelist[pid][0][msg.sender].PurchasedItemCount;
    // }
        ...
        nonReentrant
    {
        ...
        require(ethAmount >= quantity.div(packages[packageIndex].RatePerETH).mul(1e18), "insufficient funds");
        
        whitelist[poolId][packageId][msg.sender].TotalETHPurchase = whitelist[poolId][packageId][msg.sender]
            .TotalETHPurchase
            .add(ethAmount);

        uint256 remainToken = getRemainINOToken(packageId);
        require(
            remainToken > packages[packageId].MinimumTokenSoldout,
            "INO sold out"
        );
        require(remainToken >= quantity, "INO sold out");
        // require(
        //     remainToken.sub(quantity) >= packages[packageId].MinimumTokenSoldout,
        //     "INO sold out"
        // );

        whitelist[poolId][packageId][msg.sender].PurchasedItemCount = whitelist[poolId][packageId][
            msg.sender
        ].PurchasedItemCount.add(quantity);

        //Add code to check duplication
        bool duplicated;
        {
            for(uint i=0; i<packages[packageIndex].UsersPurchased.length; i++)
                if(packages[packageIndex].UsersPurchased[i] == msg.sender)
                    duplicated = true;
        }
        if (!duplicated)
            packages[packageIndex].UsersPurchased.push(msg.sender);
        packages[packageId].TotalSoldCount = packages[packageId].TotalSoldCount.add(
            quantity
        );
    ...
    // To Do: Claim Pool
    function claimPool(uint256 pid)
        public nonReentrant        
    {
        // require(!whitelist[pid][1][msg.sender].IsClaimed, "user already claimed");
        uint256 poolIndex = pid.sub(1);
        for (uint256 i = 1; i <= pools[poolIndex].TotalPackageCount; i++)
        {
            uint256 packageId = poollist[pid][i].Id;
            uint256 packageIndex = packageId.sub(1);
            address[] memory users = packages[packageIndex].UsersPurchased;
            for(uint j = 0; j < users.length; j++)
            {
                uint256 itemCount = whitelist[pid][packageId][users[j]].PurchasedItemCount;
                polkaBridgeNFT.mintNFT(users[j], packageId, itemCount);
                whitelist[pid][packageId][users[j]].IsClaimed = true;
            }
        }
    }
    ...
    function getWhitelistfo(uint256 poolId)
        public
        view
        returns (
            address,
    ...
    {
        return (
            whitelist[poolId][1][msg.sender].UserAddress,
            whitelist[poolId][1][msg.sender].IsWhitelist,
            whitelist[poolId][1][msg.sender].TotalETHPurchase,
            whitelist[poolId][1][msg.sender].IsClaimed
        );
    }
...
abstract contract PolkaBridgeNFT is ERC1155, Ownable {
