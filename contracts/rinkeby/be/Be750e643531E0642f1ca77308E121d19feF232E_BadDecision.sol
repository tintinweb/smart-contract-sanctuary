...
contract BadDecision is ERC721Enumerable, EIP712, AccessControl {
    ...
    constructor(address owner) ERC721("Bad Decisions Test v1", "BD") EIP712("Bad Decisions Test v1", "1") {
        _setupRole(
            OWNER_ROLE,
            address(owner)
        );
    ...
    function _baseURI() internal view virtual override returns (string memory) {
        return "https://development.baddecisions.studio/api/token/info/";
    }
    ...
    function withdraw(uint amount) public onlyRole(OWNER_ROLE) {
        uint balance = address(this).balance;
        require(balance > amount, "Not enough moneys");
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "Voucher(uint256 product,uint256 count,uint256 price,uint256 starttime,uint256 endtime,bool auction,address winner)"
                        ),
                        voucher.product,
                        voucher.count,
                        voucher.price,
