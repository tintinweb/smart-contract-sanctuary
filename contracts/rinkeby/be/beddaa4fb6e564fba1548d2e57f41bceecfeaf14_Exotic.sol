...
import "./EXOTICDividendTracker.sol";
    ...
    ERC20Upgradeable 
{
    ...
    // Only owner of this contract can burn his own token
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        require(newDividendTracker.owner() == address(this), "The new one must be owned by this contract");
    ...
    // Use 2 * values only to update fees
    function changeFees(uint256 newEthRewardsFee, uint256 newLiquidityFee, uint256 newTeamFee) external onlyOwner {
        // uint256 temporaryNewRewardsFee = newEthRewardsFee / 2;
        // if ((2 * temporaryNewRewardsFee) != newEthRewardsFee)
        //     revert ValueIsNotEvenNumber(newEthRewardsFee);

        // uint256 temporaryNewLiqudityFee = newLiquidityFee / 2;
        // if ((2 * temporaryNewLiqudityFee) != newLiquidityFee)
        //     revert ValueIsNotEvenNumber(newLiquidityFee);

        // uint256 temporaryNewTeamFee = newLiquidityFee / 2;
        // if ((2 * temporaryNewTeamFee) != newLiquidityFee)
        //     revert ValueIsNotEvenNumber(newTeamFee);
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "The Uniswap pair cannot be removed");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        ...
        require(newValue >= 200000 && newValue <= 500000, "newValue must be between 200,000 and 500,000");
    ...
    function swapForNotHolders(uint256 tokens) private {
        // split the contract balance into halves to submit to Uniswap liquditiy pool
        ...
        // More ETH balance for the contract
        // swap tokens for ETH
        ...
        // 1% Exotic Toekn + 1% ETH from swapToeksnForETH => 2% for liquidity to Uniswap
    ...
    function swapToSendDividendsForHolders(uint256 tokens) private {
        swapTokensForEth(tokens);
        uint256 dividends = address(this).balance;
        (bool success,) = address(dividendTracker).call{value: dividends}("");

        if(success) {
   	 		emit SendDividendsForHolders(tokens, dividends);
        }
    }
    ...
    function _transfer(
        ...
        if(
            ...
            swapToSendDividendsForHolders(sellTokensForHolders);
        ...
        if(_isExcludedFromFees[from] || _isExcludedFromFees[to] || useExcludeFromFeesUntilAtAmount && amount <= excludeFromFeesUntilAtAmount) {
            takeFee = false;
        }
        ...
        if(takeFee) {
        	uint256 fees = amount.mul(totalFees).div(100); // 10%
...
// SPDX-License-Identifier: MIT
// This doens't work with upgradable
...
import "@openzeppelin/contracts/utils/math/SafeMath.sol"; // It is uncessary after 0.8 but include it for a while.
...
contract EXOTICDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
...
// const exotic = await upgrades.deployProxy(Exotic, { initializer: 'initialize', unsafeAllowLinkedLibraries: true });
