...
/**
 * @title BBTEST1 contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract BBTEST1 is ERC721Enumerable, Ownable {
    ...
    function _mintWithoutValidation(address to, uint amount) internal {
        require(mintedItems + amount <= maxItems, "All items sold");
    ...
    function presaleMint(address _to, uint256 _mintAmount) public payable {
        ...
        require(block.timestamp <= publicSaleStartTimestamp, "You are not in presale period");
        require(_mintAmount > 0);
        require(_mintAmount <= maxMintAmount, "Over the maxItemsPerTx");
        require(supply + _mintAmount <= maxItems);

        if (msg.sender != owner()) {
            if (whitelisted[msg.sender] == true) {
                if (presaleWallets[msg.sender] < 10) {
                    require(presaleWallets[msg.sender] + _mintAmount <= 10, "Max 10 items can be purchased by per wallet in presale period");
    ...
    function publicSaleMint(address _to, uint256 _mintAmount) public payable {
        ...
        require(_mintAmount > 0);
        require(_mintAmount <= maxMintAmount, "Over the maxItemsPerTx");
        require(supply + _mintAmount <= mintedItems);
