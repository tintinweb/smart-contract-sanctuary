...
contract BigRichCard is ERC1155Tradable {
    ...
    constructor (address _proxyRegisteryAddress, string memory baseUrl)
        ERC1155Tradable(
            "Big Rich Card",
            "BRC",
            string(abi.encodePacked(baseUrl, "/api/nft/metadata/{id}")),
    ...
    function contractURI() public view returns (string memory) {
        return string(
            abi.encodePacked(_baseUrl, "/api/nft/contract/big-rich-erc1155")
        );
    }

    function setBaseUrl(string memory _newUrl) public onlyOwner {
        _baseUrl = _newUrl;
        setURI(string(abi.encodePacked(_baseUrl, "/api/nft/metadata/cat/{id}")));
    }
}
