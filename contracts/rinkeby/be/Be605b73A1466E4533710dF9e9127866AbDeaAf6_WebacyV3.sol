...
contract WebacyV3 is AccessControl {
    ...
    mapping(address => mapping(uint256 => mapping(address => uint256)))
        ...
        mapping(address => mapping(uint256 => mapping(address => uint256)))
        private _memberToVersionToContractToAllowableTokensIds;
    //Token Beneficiary section *
    ...
    function getApprovedAssets(address owner) public view returns (Assets memory) {
        ...
        // INIT FULLFILL ERC20 BENEFICIARIES
    ...
    function updateStoredAssetsVersion() external {
        _memberToERC721SCVersion[msg.sender] = _memberToERC721SCVersion[msg.sender] + 1;
        _memberToBackupWalletsVersion[msg.sender] = _memberToBackupWalletsVersion[msg.sender] + 1;

        _memberToERC20SCVersion[msg.sender] = _memberToERC20SCVersion[msg.sender] + 1;
    ...
    function _saveBackupWallet(address[] memory backupAddresses, string[] memory backupAliases) private {
        ...
        // require(_memberToVersionToBackupWallets[msg.sender][backupWalletsVersion].length == 0, "Backupwallets already been set");
        ...
        if (_memberToVersionToBackupWallets[msg.sender][backupWalletsVersion].length == 0) {
            for (uint256 i = 0; i < backupAddresses.length; i++) {
                ...
                backupWalletToVersionToMember[backupAddresses[i]][backupWalletsVersion] = msg.sender;
    ...
    function transferAssets() external {
        require(
            ...
            "Backup wallet as no related member"
        );
        ...
        for (uint256 i = 0; i < ERC20Contracts.length; i++) {
            ...
            WebacyProxy(webacyProxyForMember).transferERC20TokensAllowed(ERC20Contracts[i], member, msg.sender, amount);
        }

        for (uint256 iContracts = 0; iContracts < ERC721Contracts.length; iContracts++) {
            AssetBeneficiary[] memory assetBeneficiaries = _memberToVersionToContractToAssetBeneficiary[member][ERC721SCVersion][ERC721Contracts[iContracts]];
            for (uint256 iAssets = 0; iAssets < assetBeneficiaries.length; iAssets++) {
                WebacyProxy(webacyProxyForMember).transferERC721TokensAllowed(ERC721Contracts[iContracts], member, msg.sender, assetBeneficiaries[iAssets].tokenId);
            }
        }
    }
...
contract WebacyProxyFactory is IWebacyProxyFactory, AccessControl {
    uint8 public version;
    address private webacyAddress;
    bytes32 public constant EXECUTOR_ROLE = keccak256("EXECUTOR_ROLE");

    mapping(address => WebacyProxy) memberToContract;

    constructor(uint8 _version, address _webacyBusiness) {
        version = _version;
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(EXECUTOR_ROLE, _webacyBusiness);
    }
