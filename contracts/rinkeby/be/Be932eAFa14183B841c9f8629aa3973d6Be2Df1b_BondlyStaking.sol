...
contract BondlyStaking is IBondlyStaking, OwnableUpgradeable {
	...
	function stake(uint256 _amountBONDLY) external override updateRewardPool {
		...
		// bondlyToken.transferFrom(address(msg.sender), address(this), _amountBONDLY);

		uint256 amountxBONDLY = _convertToxBONDLY(_amountBONDLY);
		// xBondlyToken.mint(address(msg.sender), amountxBONDLY);

		totalPool = totalPool.add(_amountBONDLY);

		emit StakedBONDLY(_amountBONDLY, amountxBONDLY, address(msg.sender));
	}

	function withdraw(uint256 _amountxBONDLY) external override updateRewardPool {
		require(
			xBondlyToken.balanceOf(address(msg.sender)) >= _amountxBONDLY,
			"Withdraw: not enough xBONDLY tokens to withdraw"
		);

		uint256 amountBONDLY = _convertToBONDLY(_amountxBONDLY);
		xBondlyToken.burn(address(msg.sender), _amountxBONDLY);
		...
		bondlyToken.transfer(address(msg.sender), amountBONDLY);

		emit WithdrawnBONDLY(amountBONDLY, _amountxBONDLY, address(msg.sender));
	}
