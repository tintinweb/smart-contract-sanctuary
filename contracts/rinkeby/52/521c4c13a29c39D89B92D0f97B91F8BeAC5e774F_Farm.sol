...
contract Authorizable is AccessControl {
...
contract Farm is Authorizable, ReentrancyGuard {
...
*/
    ...
    function initialize(
      ...
      uint256 _duration
        ...
        //require(_initReward == _rewardToken.balanceOf(address(this)),   "StakingRewards: wrong reward amount supplied");
    ...
    function updateReward(address account) internal {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            rewardTokenStruct.rewardPerTokenStored = rewardPerToken(tokenAddresses[i]);
            rewardTokenStruct.lastUpdateTime = lastTimeRewardApplicable(tokenAddresses[i]);
            if (account != address(0)) {
                rewardTokenStruct.rewards[account] = earned(tokenAddresses[i], account);
                rewardTokenStruct.userRewardPerTokenPaid[account] = rewardTokenStruct.rewardPerTokenStored;
        }
        }
    ...
    function lastTimeRewardApplicable(address rewardTokenAddress) public view returns (uint256) {
        return Math.min(block.timestamp, rewardTokens[rewardTokenAddress].periodFinish);
    }

    function rewardPerToken(address rewardTokenAddress) public view returns (uint256) {
        RewardToken storage rewardTokenStruct = rewardTokens[rewardTokenAddress];
        if (_totalSupply == 0) {
    ...
    function earned(address rewardTokenAddress, address account) public view returns (uint256) {
        RewardToken storage rewardTokenStruct = rewardTokens[rewardTokenAddress];
        uint256 amount = _balances[account]
                    .mul(rewardPerToken(rewardTokenAddress).sub(rewardTokenStruct.userRewardPerTokenPaid[account]))
                    .div(1e36)
    ...
    function getReward(address rewardTokenAddress) public nonReentrant{
        ...
        
    {
        ...
        require(_balances[msg.sender]  >= amount, "Insufficient amount for emergency Withdraw");
    ...
    function _notifyRewardAmount(address rewardTokenAddress, uint256 reward) internal {
        updateReward(address(0));
        RewardToken storage rewardTokenStruct = rewardTokens[rewardTokenAddress]; 
        rewardTokenStruct.rewardRate = reward.mul(1e18).div(rewardTokenStruct.duration);
        rewardTokenStruct.lastUpdateTime = block.timestamp;
        rewardTokenStruct.periodFinish = block.timestamp.add(rewardTokenStruct.duration);
    ...
    /* -----------     FARMING EVENTS   ----------------------- */
    event SenderDeposited(address indexed _sender, uint256 _amount, uint256 _timestamp);
    event SenderWithdrawed(address indexed _sender, uint256 _amount, uint256 _timestamp);
    ...
    constructor(IERC20 token, uint256 tokenMultiplier) {
        _token = token;
        fixedConstantPerToken = tokenMultiplier * 1e18;
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    function getTokenAddress() external view onlyRole(DEFAULT_ADMIN_ROLE) returns(address) {
        return address (_token);
    }

    function giveAway(address _address, uint256 points) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function payment(uint256 amount) external returns (bool) {
        address buyer = msg.sender;
        consolidate(buyer);

        require(rewardedPoints(buyer) > 0, "amount equal to 0");
        require(rewardedPoints(buyer) >= amount, "Insufficient points!");

        stakers[buyer].points = (stakers[buyer].points - amount);
    ...
    function getConsolidatedRewards(address buyer) public view returns(uint256){
        return stakers[buyer].points;
    }

    function rewardedPoints(address staker) public view returns (uint256) {
        uint256 _seconds = block.timestamp.sub(stakers[staker].timestamp);
        uint256 earnPerSec = fixedConstantPerToken / (60*60*24);
        uint256 rewarded = ((stakers[staker].amount * earnPerSec) * _seconds) / 1e18;
        return rewarded;
    }
    ...
    function deposit(uint256 amount) public nonReentrant() {
        ...
        require(_token.balanceOf(msg.sender)>=amount,"Insufficient amount for deposit");
