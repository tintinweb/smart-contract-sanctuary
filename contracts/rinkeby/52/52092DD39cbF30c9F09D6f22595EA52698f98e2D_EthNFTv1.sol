...
contract EthNFTv1 is NFTv1 {
  ProxyRegistry immutable proxyRegistry;

  constructor(string memory name_, string memory symbol_, address proxy) NFTv1(name_, symbol_) {
    proxyRegistry = ProxyRegistry(proxy);
  }
...
contract NFTv1 is Ownable, ERC721 {
  string private _URI;
  ...
  function init(uint maxSupply_, uint reserved_, uint price_, address[] calldata beneficiaries_, uint[] calldata ratio_) external onlyOwner {
    require(maxSupply == 0, "inited");
  ...
  function ratios() external view returns (uint[] memory) {
    return _ratio;
  }
