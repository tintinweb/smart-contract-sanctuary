...
contract ObjectsFactory is Ownable
{
   ...
   { 
    ...
    function putSaleRequest(uint ObjectsId, uint salePrice) public payable
    {
        ...
        ObjectsAgainstId[ObjectsId].index = upForSaleList.push(ObjectsId);
	...
	function removeObjectFromSale(uint ObjectsId)  public
	{
		...
		uint index = ObjectsAgainstId[ObjectsId].index;


		if (upForSaleList.length > 1) {
			upForSaleList[index] = upForSaleList[upForSaleList.length-1];
			upForSaleListPrice[index] = upForSaleListPrice[upForSaleListPrice.length-1];
		  }
		upForSaleList.length--; // Implicitly recovers gas from last element storage
