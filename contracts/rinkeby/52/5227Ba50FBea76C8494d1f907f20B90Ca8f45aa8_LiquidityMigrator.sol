...
/// @title Uniswap V2,V3 & Sushiswap Migrator
contract LiquidityMigrator is ILiquidityMigrator, IERC721Receiver, Context {
    ...
    function getTickAtSqrtPrice(int24 tick) private pure returns (uint160 sqrtPriceX96) {
    ...
    // function getUnipilotTokenAmounts(
    //     address pair,
    //     address liquidityManager,
    //     uint256 baseLiquidity,
    //     uint256 rangeLiquidity
    // )
    //     private
    //     view
    //     returns (
    ...
    //     ) = IUniswapLiquidityManager(liquidityManager).liquidityPositions(pair);

    //     (amount0Base, amount1Base) = getTokenAmounts(
    //         pair,
    //         baseLiquidity,
    //         baseTickLower,
    //         baseTickUpper
    ...
    //     (amount0Range, amount1Range) = getTokenAmounts(
    //         pair,
    //         rangeLiquidity,
    //         rangeTickLower,
    //         rangeTickUpper
    ...
    function migrateV2Liquidity(MigrateV2Params calldata params) external {
        ...
        IUnipilot(params.unipilotAddress).createPoolAndDeposit(
    ...
    function migrateV3Liquidity(MigrateParams calldata params, bytes calldata data)
        external
    {
        MigrateVars memory localVars;
        address pair = IUniswapV3Factory(UNISWAP_FACTORY).getPool(
            params.token0,
            params.token1,
            params.fee
        );
        ...
        (, , , , , , , localVars.liquidityV3, , , , ) = INonfungiblePositionManager(
            POSITION_MANAGER
        ...
        positionManager.decreaseLiquidity(
            INonfungiblePositionManager.DecreaseLiquidityParams({
                ...
                liquidity: localVars.liquidityV3,
                amount0Min: 0,
                amount1Min: 0,
        ...
        (localVars.amount0V3, localVars.amount1V3) = positionManager.collect(
        ...
        (
            localVars.baseTickLower,
            localVars.baseTickUpper,
            ,
            localVars.rangeTickLower,
            localVars.rangeTickUpper,
            ,
            ,
            ,
            ,
        ...
        ) = IUniswapLiquidityManager(params.liquidityManager).liquidityPositions(pair);

        (localVars.amount0Base, localVars.amount1Base) = getTokenAmounts(
            pair,
            localVars.baseLiquidity,
            localVars.baseTickLower,
            localVars.baseTickUpper
        );
        (localVars.amount0Range, localVars.amount1Range) = getTokenAmounts(
            pair,
            localVars.rangeLiquidity,
            localVars.rangeTickLower,
            localVars.rangeTickUpper
        );

        uint256 amount0Total = localVars.amount0Base + localVars.amount0Range;
        uint256 amount1Total = localVars.amount1Base + localVars.amount1Range;
        ...
        (localVars.baseLiquidity, localVars.rangeLiquidity, ) = IUnipilot(
            params.unipilotAddress
        ).deposit(
                IHandler.DepositParams({
                    exchangeAddress: params.liquidityManager,
                    token0: params.token0,
                    token1: params.token1,
                    amount0Desired: localVars.amount0V3,
                    amount1Desired: localVars.amount1V3
                }),
                data
            );
    }
