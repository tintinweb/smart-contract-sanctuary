...
contract SplytLineNFT is ERC721Enumerable, Ownable {
  ...
  */ 
  ...
  function tokensOfOwner(address owner) external view returns (string[] memory tokens) {
    string[] memory _tokens = new string[](balanceOf(owner));
    for (uint256 i = 0; i < _tokens.length; i++) {
      _tokens[i] = tokenOfOwnerByIndex(owner, i).toString();
    }
    return _tokens;
  }
