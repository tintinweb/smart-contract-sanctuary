...
contract BBAC is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    modifier onlyPresale() {
        require(presaleMintingEnabled, "PRESALE_NOT_ACTIVE");
        _;
    }
    ...
    function reservedApes() public pure returns (uint256) {
    ...
    function claimReservedApes(uint256 quantity, address addr)
        external
        onlyOwner
    {
    ...
    function claimPresaleApes(uint256 quantity) external payable onlyPresale {
        uint256 maxQuantity = 2;
        require(quantity > 0, "QUANTITY_CANNOT_BE_ZERO");
        require(maxQuantity >= quantity, "QUANTITY_CANNOT_BE_GREATER_THAN_MAX");
    ...
    function mint(uint256 _mintAmount) public payable onlyPublicSale {
        require(totalSupply() < MAX_SUPPLY, "SOLD_OUT");
        require(_mintAmount > 0, "QUANTITY_CANNOT_BE_ZERO");
        require(_mintAmount <= maxMintAmount, "EXCEEDS_MAX_MINT");
        require(
        ...
        require(
            _totalClaimed[msg.sender] + _mintAmount <= nftPerAddressLimit,
            "EXCEEDS_MAX_ALLOWANCE"
        );

        require(price * _mintAmount == msg.value, "INVALID_ETH_AMOUNT");
