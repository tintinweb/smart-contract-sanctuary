...
contract BasicIssuanceModule is ModuleBase, ReentrancyGuard {
    ...
    event UniswapBuy(address indexed token,uint _quantity);
    ...
    function buyAndissue(ISetToken _setToken, uint256 _quantity,uint256 deadline) payable public  returns(bool){
        ...
        for (uint256 i = 0; i < components.length; i++) {
            ...
            path[0] = weth;
            path[1] = components[i];
            (bool success, bytes memory result) = router2Address.delegatecall(abi.encodeWithSignature("swapETHForExactTokens(uint256,address[],address,uint256)",componentQuantities[i],path,address(this),deadline));  
            uint256 amount = abi.decode(result, (uint256));
            // 将收到的底层资产发送到setToken合约
            IERC20(components[i]).transfer(address(_setToken),componentQuantities[i]);
            emit UniswapBuy(components[i],componentQuantities[i]);
        }
