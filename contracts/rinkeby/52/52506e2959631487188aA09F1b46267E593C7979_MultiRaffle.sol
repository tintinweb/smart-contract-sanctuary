...
contract MultiRaffle is Ownable, ERC721, VRFConsumerBase {
    ...
    /// @param numEntries Number of entries from participant
    /// @param firstTicket The ID of your first ticket.
    event RaffleEntered(address indexed user, uint256 numEntries, uint256 firstTicket);
    ...
    function enterRaffle(uint256 numTickets) external payable {
        ...
        uint256 firstTicket = raffleEntries.length;
        for (uint256 i = 0; i < numTickets; i++) {
            raffleEntries.push(msg.sender);
        }

        // Emit successful entry
        emit RaffleEntered(msg.sender, numTickets, firstTicket);
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350">'
                ...
                '<rect width="100%" height="100%" fill="black" /><text x="10" y="20" class="base">';
