...
contract ERC1155Tradable is ERC1155, ERC1155MintBurn, ERC1155Metadata, Ownable, Random {
    ...
    function buyNFT() payable public returns(uint256) {
        if (ifAirdropped[_msgSender()] == true) {
            ...
            require(msg.value >= NFTprice, "Insuffcient ETH");
        }
    ...
    function buyMultipleNFTs(uint256 _amount) payable public {
        require(msg.value >= NFTprice.mul(_amount));
        for (uint256 i=0; i<_amount; i++) {
            bytes4 headseed = bytes4(keccak256(abi.encodePacked(random(totalNFTs, "HEAD"), "HEADSEED", i)));
            bytes4 bodyseed = bytes4(keccak256(abi.encodePacked(random(totalNFTs, "BODY"), "BODYSEED", i)));
            bytes4 limbseed = bytes4(keccak256(abi.encodePacked(random(totalNFTs, "LIMP"), "LIMPSEED", i)));
            bytes4 weaponseed = bytes4(keccak256(abi.encodePacked(random(totalNFTs, "WEAPON"), "WEAPONSEED", i)));
            bytes4[4] memory seeds = [headseed, bodyseed, limbseed, weaponseed];
...
contract HonHuangLTD is HonHuangMarket {
    ...
    uint256 public usdtPrice = 100*10**6;
    ...
    function buyNFTwithUSDT() public returns(uint256) {
    ...
    function buyMultipleNFTswithUSDT(uint256 _amount) public {
        require(usdtERC20.allowance(msg.sender, address(this)) >= usdtPrice.mul(_amount), "Insuffcient approved USDT");
        usdtERC20.transferFrom(msg.sender, address(this), usdtPrice.mul(_amount));
