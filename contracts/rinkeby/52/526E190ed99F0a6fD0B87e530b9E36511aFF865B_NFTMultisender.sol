...
// File contracts/NFTMultisender.sol
...
contract NFTMultisender is Ownable {
    ...
    function setFee(uint256 _fee) external onlyOwner {
        require(_fee > 0, "NFTMultisender::setFee: Invalid fee.");
        fee = _fee;
    ...
    function multisend(address _tokenAddress, address[] memory _addresses, uint256[] memory _tokenIds) external payable {
        ...
        require(msg.value == fee, "NFTMultisender::multisend: Insufficient funds.");
        for(uint256 i=0;i<_addresses.length;i=i+1){
            IERC721(_tokenAddress).safeTransferFrom(address(this), _addresses[i], _tokenIds[i]);
        }
