...
contract ASMAIFAAllStarsCharacter is ERC721Enumerable, Ownable {
  ...
  constructor(address signer, address _boxContract)
  ...
  function mint(
    ...
    require(!minted[boxTokenId], 'Characters already minted for the box.');
    require(
    ...
    // TODO: special logic for 2384

    for (uint256 i = 0; i < hashes.length; i++) {
      require(!mintedHash[hashes[i]], 'Character hash already used.');
      ...
      mintedHash[hashes[i]] = true;
      tokenHashes[newItemId] = hashes[i];

      emit Minted(msg.sender, recipient, hashes[i], newItemId, boxTokenId);
    }
  ...
  function configure(
    ...
    // TODO: double confirm the configure validation
    require(!isConfigured[tokenId], 'character can only be configured once.');
    require(_verify(_hash(hash), signature), 'Invalid signature.');
  ...
  function reserve(
    ...
    for (uint256 i = 0; i < reserveIds.length; i++) {
      ...
      emit Reserved(msg.sender, recipient, hash, newItemId);
    }
  }
}