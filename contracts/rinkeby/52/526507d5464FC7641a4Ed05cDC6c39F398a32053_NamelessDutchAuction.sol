...
contract NamelessDutchAuction is AccessControl, Initializable  {
  ...
  mapping(uint => uint) private listingNextTokenId;
  ...
  function calculateCurrentPrice(ListingStorage storage template) internal view returns (uint256) {
    ...
    } else if (window.decayEndTime == window.decayStartTime) {
      return pricing.finalPrice;
    }
  ...
  function bid(uint256 listingId) external payable {
    ...
    require(listing.numTokens > 0, 'Sold Out');

    uint256 currentPrice = calculateCurrentPrice(listing);
    uint256 tokenId = listingNextTokenId[listingId];
    listing.numTokens = listing.numTokens - 1;
    if (listing.numTokens > 0) {
      listingNextTokenId[listingId] = listingNextTokenId[listingId] + 1;
    } else {
      listingNextTokenId[listingId] = 0;
    }
  ...
  function addListings( ListingInput[] calldata newListings) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    while(idx < newListings.length) {
      ...
      listingNextTokenId[listingId] = newListings[idx].firstTokenId;
      idx++;
    }
  }
