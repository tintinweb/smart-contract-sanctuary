...
contract MainnetMultiBridgePool is AccessControl, ReentrancyGuard {
  ...
  function getUniV3TokenPrice(address _token) 
  ...
  function getUniV2TokenPrice(address _token) 
    public view returns (uint256) {
  ...
  function getFee(address _token, uint256 _amount) 
    public view returns (uint256) {
      ...
      uint256 _tokenToEth = getUniV3TokenPrice(_token);
      if (_tokenToEth == 0) {
        _tokenToEth = getUniV2TokenPrice(_token);
        if (_tokenToEth == 0) {
