...
contract ChickTestFinal is ERC721, ERC721Enumerable, Ownable {
  
  // Provenance
  string public CHICK_HASH = "";
  ...
  constructor(string memory baseURI) ERC721("Chickadeez", "CDZ") {
    setBaseURI(baseURI);
  }
  ...
  function mint(uint256 numNFTs) public payable {
    ...
    for (uint i = 0; i < numNFTs; i++) {
      uint mintIndex = totalSupply() + 0; // +1 so it doesn't start on index 0.
