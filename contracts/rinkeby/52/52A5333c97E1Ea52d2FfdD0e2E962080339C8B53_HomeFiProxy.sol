...
contract HomeFiProxy {
    ...
    modifier onlyProxyOwner() {
        ...
        require(msg.sender == _proxy.proxyOwner(), "Proxy::!Owner.");
        _;
    }
    ...
    function addNewContract(bytes2 _contractName, address _contractAddress)
        ...
        onlyProxyOwner
    {
        require(_contractAddress != address(0), "Proxy::0 address");
        require(
    ...
    function upgradeMultipleImplementations(
        ...
        for (uint256 i = 0; i < _contractNames.length; i++) {
            require(_contractAddresses[i] != address(0), "Proxy::0 address");
