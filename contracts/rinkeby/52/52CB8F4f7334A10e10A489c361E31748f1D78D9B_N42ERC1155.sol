...
contract N42ERC1155 is ERC1155, Ownable, ERC1155Burnable, ERC1155Supply {
    string public name;
    ...
    function singleMint(uint256 id) public onlyOwner {
        require(!exists(id), "The id already exists.");
        _mint(msg.sender, id, 1, "");
    }

    function mintBatch(uint256 startId, uint256 endId) public onlyOwner {
        require(startId > 0, "startId must be bigger than 0.");
        require(endId > startId, "endId must be bigger than startId.");
        uint256 mintAmount = endId - startId + 1;
        uint256 index = 0;
        uint256[] memory ids = new uint256[](mintAmount) ;
        uint256[] memory amounts = new uint256[](mintAmount) ;

        for (uint256 tempId = startId; tempId <= endId; tempId++) {
            require(!exists(tempId), "The id ${tempId} already exists.");
            ids[index] = tempId;
            amounts[index] = 1;
            index++;
        }
    ...
    function singleBurn(address account, uint256 id) public {
        require(msg.sender == account);
        require(exists(id), "The id does not exist.");
        _burn(account, id, 1);
    }
