...
contract Ecoz is ERC721, Ownable {
    ...
    uint256 jagSupply = 0;
    uint256 buckSupply = 0;
    uint256 treeSupply = 0;

    bool ogSale = false;
    bool whitelistSale = false;
    bool publicSale = false;

    mapping (uint256 => uint256) weightJaguar;
    mapping (uint256 => uint256) weightBushBuck;
    mapping (uint256 => uint256) weightBananaTree;
    ...
    */
    ...
    function MintJaguar(uint256 mintAmount) public payable {  
        require(jagSupply + mintAmount <= 450,             "Over max supply");
        if (msg.sender != owner()) {
            ...
            if (publicSale != true) {
                ...
                require(mintAmount <= 2,                    "Only two mints per transaction allowed");
            }          
        }
    ...
    function MintBushBuck(uint256 mintAmount) public payable { 
        require(buckSupply + mintAmount <= 900,            "Over max supply");
        if (msg.sender != owner()) {
            require(msg.value == cost * mintAmount,        "Ether sent is not correct");
    ...
    function MintBananaTree(uint256 mintAmount) public payable {       
        require(treeSupply + mintAmount <= 1800,           "Over max supply"); 
        if (msg.sender != owner()) {
            require(msg.value == cost * mintAmount,        "Ether sent is not correct");
    ...
    */
    ...
    function addWhitelistBundle(address[] memory wlBundle) public onlyOwner {
        for (uint256 i = 0; i < wlBundle.length; i++) {
            wlAddress[wlBundle[wlCounter + 1]] = true;
            wlCounter++;
        }
    }
    ...
    function addOGBundle(address[] memory ogBundle) public onlyOwner {
        for (uint256 i = 0; i < ogBundle.length; i++) {
            ogAddress[ogBundle[ogCounter + 1]] = true;
            ogCounter++;
        }
    }
...
contract Ecozsystem is Ecoz {
    ...
    function BreedJaguar() public {
        ...
        Thrive.burn(msg.sender, breedCostJag);
        babyJagSupply++;
    ...
    function BreedBushBuck() public {
        ...
        Thrive.burn(msg.sender, breedCostBuck);
        babyBuckSupply++;
    ...
    function BreedBananaTree() public {
        ...
        Thrive.burn(msg.sender, breedCostTree);
        babyTreeSupply++;
