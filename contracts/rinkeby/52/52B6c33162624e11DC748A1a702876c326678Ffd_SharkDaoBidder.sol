...
contract SharkDaoBidder {
    ...
    constructor(address nounsAuctionHouseAddress) {
        owner = payable(msg.sender);
        auctionHouse = INounsAuctionHouse(nounsAuctionHouseAddress);        
    }
    ...
    function changeOwner(address newOwner) external onlyOwner {
        ...
        require(newOwner != address(0), "Cannot transfer to null address");
    ...
    function setMaxBid(uint256 nounId, uint256 maxBid) external payable onlyOwner {
        // The max bid can ONLY be set by the owner
        maxBids[nounId] = maxBid;
        require(address(this).balance >= maxBid);
    }
     ...
     */
    function addDaoBidder(address bidder) public onlyDaoBidder {
        daoBidders[bidder] = true;
    }
    
    function removeDaoBidder(address bidder) public onlyDaoBidder {
        delete daoBidders[bidder];
    }

    function submitBidUnderMax(uint256 nounId, uint256 proposedBid) public onlyDaoBidder {
        // Bids can be submitted by ANYONE in the DAO
        require(proposedBid <= maxBids[nounId], "Proposed bid is above max");
        auctionHouse.createBid{value: proposedBid}(nounId);
    }
}
