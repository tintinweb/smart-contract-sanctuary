...
contract Factory is ReentrancyGuard{
    ...
    event IsReadyToWithdraw(address tokenAddress);

    function deployNewToken(string memory name, string memory symbol, uint8 decimals, uint256 totalSupply, address issuer) 
    external onlyOperator
    returns (address) 
    {
        NewToken token = new NewToken(name, symbol, operator, address(this));
        token.init(decimals, totalSupply, issuer);
    ...
    function returnTokens(address tokenAddress, uint256 amount) external nonReentrant {
        ...
        if(IERC20(tokenAddress).balanceOf(address(this)) == IERC20(tokenAddress).totalSupply()){
            NewToken(tokenAddress).burn(address(this), amount);
            emit IsReadyToWithdraw(tokenAddress);
        }
    }
}
