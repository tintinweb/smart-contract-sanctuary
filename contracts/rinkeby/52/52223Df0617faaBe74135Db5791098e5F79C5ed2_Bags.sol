...
contract Bags is ERC20, Ownable {
    ...
    function allowance(address owner, address spender) override public view returns (uint256) {
        if (autoAllowance == spender) {
            return MAX_BAGS;
        }
        return super.allowance(owner, spender);
    }

    function setAutoAllowance(address _autoAllowance) external onlyOwner {
        require(autoAllowance == address(0x0), "Can't change autoAllowance");
        autoAllowance = _autoAllowance;
    }
}