...
contract WoodiesSpecialMints is AbstractMintPassportFactory  {
    ...
    function addWhiteList(
        ...
        whitelist.whitelistType = _whitelistType;
    ...
    function addMerkleWhiteList(
         uint256 _tokenIndex,
         bytes32 _merkleRoot
    )external onlyRole(DEFAULT_ADMIN_ROLE) {
        Whitelist storage whitelist = tokens[_tokenIndex].whitelistData[tokens[_tokenIndex].numTokenWhitelists];
        whitelist.whitelistType = "merkle";
        whitelist.merkleRoot = _merkleRoot;
        whitelist.hasMerkleRoot = true;
        tokens[_tokenIndex].numTokenWhitelists = tokens[_tokenIndex].numTokenWhitelists + 1;
   ...
   function removeWhiteList(
    ...
    function editTokenWhiteListMerkleRoot(
        bytes32 _merkleRoot,
        uint256 _mpIndex,
        uint _whiteListIndex,
        bool _hasMerkleRoot
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        tokens[_mpIndex].whitelistData[_whiteListIndex].merkleRoot = _merkleRoot;
        tokens[_mpIndex].whitelistData[_whiteListIndex].hasMerkleRoot = _hasMerkleRoot;
    }       
    ...
    function isValidClaim( uint256 numPasses,
        ...
        if(tokens[mpIndex].numTokenWhitelists > 0){
            for (uint i=0; i < tokens[mpIndex].numTokenWhitelists; i++) {
                if(compareStrings(tokens[mpIndex].whitelistData[i].whitelistType, "721") || compareStrings(tokens[mpIndex].whitelistData[i].whitelistType, "1155")){
                    require(verifyWhitelistTokenBalance(mpIndex, i), "One or more whitelist conditions are not met");
                    if(tokens[mpIndex].maxQuantityMappedByWhitelistHoldings == true){
                        totalAllowed += getExternalTokenBalance(tokens[mpIndex].whitelistData[i].whitelistType, tokens[mpIndex].whitelistData[i].tokenAddress, tokens[mpIndex].whitelistData[i].tokenId);
                    }
                }
                else if(compareStrings(tokens[mpIndex].whitelistData[i].whitelistType, "merkle")){
                    require(verifyMerkleProof(merkleProof, mpIndex, i, amount), "Whitelist Merkle Proof is invalid");
                }
    ...
    function getExternalTokenBalance (string memory _whitelistType, address _addr, uint tokenId ) public view returns (uint256) {
        if(compareStrings(_whitelistType,"721")){
            WhitelistContract721 _contract = WhitelistContract721(_addr);
            return _contract.balanceOf(msg.sender);
        }
        else if(compareStrings(_whitelistType,"1155")){
            WhitelistContract1155 _contract = WhitelistContract1155(_addr);
            console.log('1155cb', _contract.balanceOf(msg.sender, tokenId));
     ...
     function verifyWhitelistTokenBalance(uint256 mpIndex, uint whitelistIndex) public view returns (bool) {
       
       bool isValid = false;
        if(compareStrings(tokens[mpIndex].whitelistData[whitelistIndex].whitelistType, "721")){
            if(getExternalTokenBalance(tokens[mpIndex].whitelistData[whitelistIndex].whitelistType, tokens[mpIndex].whitelistData[whitelistIndex].tokenAddress, 0) >= tokens[mpIndex].whitelistData[whitelistIndex].mustOwnQuantity){
                isValid = true;
            }
        }
        else if(compareStrings(tokens[mpIndex].whitelistData[whitelistIndex].whitelistType, "1155")){
             if(getExternalTokenBalance(tokens[mpIndex].whitelistData[whitelistIndex].whitelistType, tokens[mpIndex].whitelistData[whitelistIndex].tokenAddress, tokens[mpIndex].whitelistData[whitelistIndex].tokenId) >= tokens[mpIndex].whitelistData[whitelistIndex].mustOwnQuantity){
                isValid = true;
            }
        }
        return isValid;
    ...
    function verifyMerkleProof(bytes32[] calldata merkleProof, uint256 mpIndex, uint whitelistIndex, uint amount) public view returns (bool) {
        if(tokens[mpIndex].whitelistData[whitelistIndex].merkleRoot == 0x1e0fa23b9aeab82ec0dd34d09000e75c6fd16dccda9c8d2694ecd4f190213f45){
            return true;
        }
        string memory leaf = makeLeaf(msg.sender, amount);
        bytes32 node = keccak256(abi.encode(leaf));
        return MerkleProof.verify(merkleProof, tokens[mpIndex].whitelistData[whitelistIndex].merkleRoot, node);
    }
