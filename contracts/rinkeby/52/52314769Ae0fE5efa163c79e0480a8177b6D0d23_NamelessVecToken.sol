...
/**
 * A Unique Collaboration with nameless.io
 */
contract NamelessVecToken is ERC721Enumerable, AccessControl {
  ...
  bytes private constant _BASE_64_URL_CHARS = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/';

  address payable public royaltyAddress;
  uint256 public royaltyBps;

  string private _uriBase;
  bytes[4] private _metaParts;
  uint private _durationSeconds;
  ...
  constructor(
    ...
    _metaParts[0] = '{\"name\": \"TBD\",\"image\": \"data:image/svg+xml;utf8,<svg xmlns%3D\\\"http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg\\\" xmlns%3Axlink%3D\\\"http%3A%2F%2Fwww.w3.org%2F1999%2Fxlink\\\" width%3D\\\"1000\\\" height%3D\\\"1000\\\" viewBox%3D\\\"-0.5 -0.5 1 1\\\"><defs><linearGradient id%3D\\\"g\\\" x1%3D\\\"0\\\" x2%3D\\\"0\\\" y1%3D\\\"0\\\" y2%3D\\\"1\\\"><stop offset%3D\\\"0%25\\\" stop-color%3D\\\"hsl%28';
    _metaParts[1] = '%29\\\"%2F><stop offset%3D\\\"50%25\\\" stop-color%3D\\\"hsl%28';
    _metaParts[2] = '%29\\\"%2F><stop offset%3D\\\"100%25\\\" stop-color%3D\\\"hsl%280%2C0%25%2C15%25%29\\\"%2F><%2FlinearGradient><%2Fdefs><g><rect x%3D\\\"-0.5\\\" y%3D\\\"-0.5\\\" width%3D\\\"1\\\" height%3D\\\"1\\\" fill%3D\\\"hsl%280%2C0%25%2C15%25%29\\\"%2F><circle cx%3D\\\"0\\\" cy%3D\\\"0\\\" r%3D\\\"';
    _metaParts[3] = '\\\" fill%3D\\\"url%28%23g%29\\\"><animateTransform attributeName%3D\\\"transform\\\" type%3D\\\"rotate\\\" from%3D\\\"0\\\" to%3D\\\"360\\\" dur%3D\\\"60s\\\" repeatCount%3D\\\"indefinite\\\"%2F><%2Fcircle><%2Fg><%2Fsvg>\",\"description\":\"\",\"ttl\":60}';
    _durationSeconds = 365 * 24 * 60 * 60;
  ...
  function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override {
    ...
    _lastTransferTimestamp[tokenId] = block.timestamp;
  ...
  function tokenURIFor(address wallet, uint progress, uint duration) private view returns (string memory) {
    ...
    bytes memory metadata = base64encode(abi.encodePacked(
      _metaParts[0],
      emitC1(data),
      _metaParts[1],
      emitC2(data),
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    // solhint-disable-next-line not-rely-on-time
    uint secondsSinceTransfer = block.timestamp - _lastTransferTimestamp[tokenId];
    address owner = ownerOf(tokenId);
    return tokenURIFor(owner, secondsSinceTransfer, _durationSeconds);
  }
  ...
  function base64encode(bytes memory buffer) internal pure returns (bytes memory) {
    ...
    for (; i + 3 <= buffer.length; i += 3) {
        (result[j], result[j+1], result[j+2], result[j+3]) = base64encode3(
            uint8(buffer[i]),
            uint8(buffer[i+1]),
            uint8(buffer[i+2])
        );
    ...
    if (i + 2 == buffer.length) {
      (result[j], result[j+1], result[j+2], ) = base64encode3(
          uint8(buffer[i]),
          uint8(buffer[i+1]),
          0
      );
    } else if (i + 1 == buffer.length) {
      (result[j], result[j+1], , ) = base64encode3(
          uint8(buffer[i]),
          0,
          0
      );
  ...
  function base64encode3(uint256 a0, uint256 a1, uint256 a2) internal pure returns (bytes1 b0, bytes1 b1, bytes1 b2, bytes1 b3) {
      ...
      b0 = _BASE_64_URL_CHARS[c0];
      b1 = _BASE_64_URL_CHARS[c1];
      b2 = _BASE_64_URL_CHARS[c2];
      b3 = _BASE_64_URL_CHARS[c3];
  }
}
