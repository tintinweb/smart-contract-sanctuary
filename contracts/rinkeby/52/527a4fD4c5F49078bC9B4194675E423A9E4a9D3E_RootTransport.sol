    ...
    ERC165Storage
{
    ...
    function setTransportMinter(address _minterAddress) external onlyOwner {
    ...
    function _beforeTokenTransfer(
        ...
        if (
            transportMinter.allocatedTransports(tokenId) != address(0) &&
            transportMinter.allocatedTransports(tokenId) != auctionAddress &&
            from != address(0)
        ) {
            require(
                block.timestamp > (startDate + 13 days),
                "RootTransport: Locked until December 25th"
            );
        }
    }
