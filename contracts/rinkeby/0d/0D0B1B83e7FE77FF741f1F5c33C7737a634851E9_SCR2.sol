...
// https://github.com/OpenZeppelin/openzeppelin-contracts/issues/2789
    ...
    ERC2981ContractWideRoyalties
{
    ...
    uint256 public feeWei; // = 0.05 ether; // interpreted as wei // https://docs.soliditylang.org/en/v0.8.7/units-and-global-variables.html#ether-units

    string internal baseURI;
    string internal temporaryURI;
    bool public hiddenURI;
    uint256 internal newTokenId;
    uint256 public totalTokensRemaining; // NEW: TEST
    uint256[] public listMintedTokensIds; // NEW: TEST
    ...
    mapping(uint256 => uint256) public mapIndexToTokenId; // NEW: TEST maps vacant index positions to end position token ids
    ...
    constructor(
    ...
    ) ERC721("Sample", "SAM") VRFConsumerBase(_vrfCoordinator, _linkToken) {
        ...
        require(
            _percentageRoyalty <= 100,
            "ERC2981Royalties: Too high | 0 < _percentageRoyalty <= 100"
        );
    ...
    // TODO: TEST nonReentrant !!! -- guard against multiple token request before balanceOf() is updated!
    function requestRandomIndex() public payable nonReentrant whenNotPaused {
        // returns (bytes32 requestIndex) | add after "whenNotPaused"?
        require(
        ...
        require(
            ...
            "Reached maximum amount of token allowed to be requested."
        );
    ...
    function fulfillRandomness(bytes32 requestIndex, uint256 randomNumber)
        ...
        override
    {
        uint256 newRandomIndex = (randomNumber % totalTokensRemaining); // eg if XXXX is 10, newRandomIndex will be 0-9
        require(
            newRandomIndex < totalTokensRemaining,
            "Index out of token pool range."
        );

        // newTokenId = tokenPool[newRandomIndex];
        newTokenId = getTokenIdAtIndex(newRandomIndex); // NEW: TEST
        require(
            !_exists(newTokenId),
            "This token has already been minted. Code Error!"
        ); // make sure token NOT minted yet | this line should never be called, if called, there is some error in code!
        ...
        ); // prevents random account from setting URI

        _setTokenURI(
            newTokenId,
            string(
                abi.encodePacked(
                    _frontTokenName(newTokenId.toString()),
                    newTokenId.toString(),
                    ".json"
                )
            )
        ...
        // _removeToken(newRandomIndex);
        adjMapIndexToTokenId(newRandomIndex);
        listMintedTokensIds.push(newTokenId);

        require(
            balanceOf(minter) == mapAddressToTokenIds[minter].length,
            "Number of tokens minted does not match number of tokens owned."
        );
    ...
    // function _removeToken(uint256 _index) internal {
    //     require(_index < tokenPool.length, "Index out of token pool range.");
    //     tokenPool[_index] = tokenPool[tokenPool.length - 1];
    //     tokenPool.pop();
    // }
    ...
    function getTokenIdAtIndex(uint256 _index) internal view returns (uint256) {
    ...
    function adjMapIndexToTokenId(uint256 _index) internal {
        mapIndexToTokenId[_index] = getTokenIdAtIndex(totalTokensRemaining - 1);
    ...
    function _frontTokenName(string memory _token)
        internal
        pure
        returns (string memory frontName)
    {
        require(bytes(_token).length < 5, "9999 tokens max.");
    ...
    // TODO: only allowed to call this whenPaused ????? concern is what if called halfway execution of minting?
    ...
    function mintedTokens() external view onlyOwner returns (uint256[] memory) {
        return listMintedTokensIds;
    }

    function mintedTokensSize() external view onlyOwner returns (uint256) {
    ...
    // function remainingTokensSize() external view onlyOwner returns (uint256) {
    ...
    function withdraw() external onlyOwner {
        ...
        // uses onlyOwner, thus reentrancy attacks NOT possible
