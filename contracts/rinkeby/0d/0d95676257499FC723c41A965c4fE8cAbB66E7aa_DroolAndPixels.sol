...
contract DroolAndPixels is IDroolsAndPixels, ERC721, Ownable {
  ...
  mapping(uint256 => uint8[]) public override animatedPixels;
...
interface IDroolsAndPixels is IERC721 {
  ...
  function animatedPixels(uint256 tokenId, uint256 index) external view returns (uint8);
...
library MultiPartRLEToSVG {
  ...
  function _generateSVGRects(SVGParams memory params, string[] storage palette)
        ...
        returns (string memory svg)
    {
        ...
        string memory part;
        uint8 blueCount;
        uint8[] memory animatedPixels = params.animatedPixels;
