...
contract Juice is ERC20Burnable, Ownable {
    ...
    uint256 public MAX_WALLET_STAKED = 50;
    ...
    */
    ...
    function emissionRateByTokenId(uint256 tokenId) public view returns (uint256) {
        ...
        if (tokenId <= 8) return 1 * EMISSIONS_RATE * 24 * 6; // every 10 min
        if (tokenId > 8 && tokenId <= 12)
            return 2 * EMISSIONS_RATE * 24 * 6; // every 10 min
        if (tokenId > 12 && tokenId <= 14)
            return 3 * EMISSIONS_RATE * 24 * 6; // every 10 min
        if (tokenId > 14 && tokenId <= 5)
            return 4 * EMISSIONS_RATE * 24 * 6; // every 10 min
