...
library Set {
    ...
    function register(User[] storage user, address payable _addr, string memory id) internal {
    ...
    function findID(User[] storage user, address payable _addr) public view returns(string memory) {
        uint256 i;
        for(i = 0; i < user.length; i++) {
            if(user[i].wallet == _addr)
                break;
    ...
    function findAddr(User[] storage user, string memory _id) public view returns(address payable) {
        uint256 i;
        for(i = 0; i < user.length; i++) {
    ...
    function pay(Board storage self, User[] storage user, address payable _addr, uint256 _count, uint256 n, address payable _owner) internal {
        uint256 _price;
