...
contract QubitsOnTheIce is ERC721Enumerable, Ownable {
    ...
    function getBoxStatus(uint id) public view returns(bool) {
        return unboxedQubit[id];
    }


    function _qubinateProcess(uint256 num) private  {
        uint256 supply = totalSupply();
        require( !_qubinatorState,   "Qubinator is offline" );
        require( supply + num < 15000,      "Exceeds maximum Qubits supply" );
        require( msg.value >= _qubinatorPrice * num,             "Ether sent is not correct" );
    ...
    function recycle(uint256 qubit1, uint256 qubit2) public {
        require( !_qubinatorState,   "Qubinator is offline" );
        require(_exists(qubit1), "Recycle: Qubit 1 does not exist.");
        require(_exists(qubit2), "Recycle: Qubit 2 does not exist.");
        require(ownerOf(qubit1) == _msgSender(), "Recycle: Qubit 1 caller is not token owner.");
        require(ownerOf(qubit2) == _msgSender(), "Recycle: Qubit 2 caller is not token owner.");
        _burn(qubit1);
        _burn(qubit2);
        _qubinateProcess(1);
    }
    ...
    function enableQubinator(bool val) public onlyOwner {
        _qubinatorState = val;
    }
