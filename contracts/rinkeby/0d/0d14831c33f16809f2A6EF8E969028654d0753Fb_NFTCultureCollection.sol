...
contract NFTCultureCollection is NFTCultureCollectionBase {
    constructor()
        NFTCultureCollectionBase(
            ...
            0xA5e9F27820D4CBA20a59D46F81CfBfE6Ce6F92d2  // The NFTCultForgeComponents contract on rinkeby.
        )
    {
...
contract NFTCultureCollectionBase is ERC1155Supply, Ownable {
    ...
    mapping(uint256 => uint256) private _cultTokenClaimYield;
    mapping(uint256 => uint256) private _cultTokenEmberYield;
    mapping(uint256 => uint256) private _cultTokenClaimsConsumed;
    mapping(uint256 => uint256) private _cultTokenEmbersReceived;

    function _addForgeComponentClaimables(
        uint256 forgeComponent,
        uint256 claimYield,
        uint256 emberYield
    ) internal {
        _cultTokenClaimYield[forgeComponent] = claimYield;
        _cultTokenEmberYield[forgeComponent] = emberYield;
    }
    ...
    constructor(
        ...
        _addForgeComponentClaimables(201, 1, 0);
        _addForgeComponentClaimables(202, 1, 0);
        _addForgeComponentClaimables(203, 2, 1);
        _addForgeComponentClaimables(301, 1, 0);
        _addForgeComponentClaimables(302, 1, 0);
        _addForgeComponentClaimables(303, 2, 1);
        _addForgeComponentClaimables(401, 1, 0);
        _addForgeComponentClaimables(402, 1, 0);
        _addForgeComponentClaimables(403, 2, 1);
        _addForgeComponentClaimables(997, 1, 1);
        _addForgeComponentClaimables(901, 3, 2);
        _addForgeComponentClaimables(902, 3, 3);
        _addForgeComponentClaimables(903, 6, 5);
    }
    ...
    {
        return
            _cultTokenClaimYield[_getForgeComponent(nftcultTokenId)] -
            _cultTokenClaimsConsumed[nftcultTokenId];
    }
    ...
    {
        return
            _cultTokenEmberYield[_getForgeComponent(nftcultTokenId)] -
            _cultTokenEmbersReceived[nftcultTokenId];
    }
    ...
    function addForgeComponentClaimables(
        ...
        _addForgeComponentClaimables(forgeComponent, claimYield, emberYield);
    }
    ...
    function claimPiecesInCollection(
        ...
        require(balanceOf(owner(), pieceId) >= count, 'No pieces left');
        require(
            nftCult.ownerOf(nftcultTokenId) == msg.sender,
            'Cult token not owned'
        );

        //uint256 forgeComponent = _getForgeComponent(nftcultTokenId);
        uint256 forgeComponent = 201;

        uint256 claimYield = _cultTokenClaimYield[forgeComponent];
        require(claimYield > 0, 'Cult token not claimable');

        uint256 claimsConsumed = _cultTokenClaimsConsumed[nftcultTokenId];
        require(claimsConsumed + count <= claimYield, 'No claims remaining');

        // Transfer the pieces.
        _safeTransferFrom(owner(), msg.sender, pieceId, count, '');
        _cultTokenClaimsConsumed[nftcultTokenId] = claimsConsumed + count;

        // A fun bonus.
        // uint256 embersReceivedFromCultToken = _cultTokenEmbersReceived[
        //     nftcultTokenId
        // ];
        // uint256 emberYield = _cultTokenEmberYield[forgeComponent];
