...
contract OwlToken is ERC20Pausable, CollaborativeOwnable {
  ...
  address public alphaPassAddress = address(0);

  uint256 public startBlock;

  uint256 public interval = 86400;
  uint256 public rate = 5 ether;

  mapping(address => uint256) public rewards;
  mapping(address => uint256) public lastUpdate;

  constructor() ERC20("Night Owls Token", "OWL") {
  ...
  // Called by AlphaPass to redeem the tokens for an alpha pass
  function redeemTokens(address addr, uint256 tokenAmount) external {
    require(_msgSender() == alphaPassAddress, "forbidden");
    require(balanceOf(addr) >= tokenAmount, "insufficent");

    claimRewards();
    _transfer(addr, alphaPassAddress, tokenAmount);
  }
  ...
  function claimRewards() public whenNotPaused {
    _mint(_msgSender(), rewards[_msgSender()] + getPendingReward(_msgSender()));
    rewards[_msgSender()] = 0;
    lastUpdate[_msgSender()] = block.timestamp;
  }

  function claimRewardsAndTransfer(address addr, uint256 amount) external whenNotPaused {
    claimRewards();
    _transfer(_msgSender(), addr, amount);
  }
  ...
  function getPendingReward(address addr) internal view returns(uint256) {
    require(nightOwlsAddress != address(0), "night owls contract address not set");

    return IERC721(nightOwlsAddress).balanceOf(addr) *
      rate *
      (block.timestamp - (lastUpdate[addr] >= startBlock ? lastUpdate[addr] : startBlock)) / 
      interval;
  }
  ...
  function setStartBlock(uint256 newStartBlock) external onlyCollaborator {
    if (newStartBlock == 0) {
      startBlock = block.timestamp;
    } else {
  ...
  function setRewardParameters(uint256 newInterval, uint256 newRate) external onlyCollaborator {
  ...
  function setAlphaPassAddress(address newContractAddress) external onlyCollaborator {
    alphaPassAddress = newContractAddress;
  }
}
