...
contract KPITokensManager is Ownable, IKPITokensManager {
    ...
    mapping(address => string) public templateDescription;
    ...
    function instantiate(address _template) external returns (address) {
        if (msg.sender != factory) revert Forbidden();
        if (_template == address(0)) revert InvalidTemplate();
        if (bytes(templateDescription[_template]).length == 0)
            revert NonExistentTemplate();
        return Clones.clone(_template);
    }
        ...
        external
    {
        ...
        if (bytes(templateDescription[_template]).length != 0)
            revert AlreadyAdded();
        templateDescription[_template] = _description;
    }
    ...
    function removeTemplate(address _template) external {
        ...
        if (bytes(templateDescription[_template]).length == 0)
            revert NonExistentTemplate();
        delete templateDescription[_template];
    }
    ...
    function updgradeTemplate(
        ...
        if (bytes(templateDescription[_template]).length == 0)
            revert NonExistentTemplate();
        delete templateDescription[_template];
        templateDescription[_newTemplate] = _newDescription;
    }
}
...
/**
 * @title ITemplateManager
 * @dev ITemplateManager contract
 * @author Federico Luzzi - <[email protected]>
 * SPDX-License-Identifier: GPL-3.0-or-later
 */
