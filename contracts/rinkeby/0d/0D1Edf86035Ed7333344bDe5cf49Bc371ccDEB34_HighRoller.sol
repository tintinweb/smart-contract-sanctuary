...
contract HighRoller is ERC721Enumerable, Ownable{
    ...
    uint256 public PURCHASE_LIMIT1 = 20;
    uint256 public PURCHASE_LIMIT2 = 3;
    uint256[] public STAGE_PRICE = [0.0777 ether, 0.08547 ether, 0.09829 ether];
    ...
    function setPurchaseLimit1(uint256 _limit) public onlyOwner{
        PURCHASE_LIMIT1 = _limit;
    }

    function setPurchaseLimit2(uint256 _limit) public onlyOwner{
        PURCHASE_LIMIT2 = _limit;
    }
    ...
    function privateSale(uint256 numberOfTokens) external payable {
        ...
        require(_allowListClaimed1[msg.sender] + numberOfTokens <= PURCHASE_LIMIT1, 'Purchase exceeds max allowed');
    ...
    function preSale(uint256 numberOfTokens) external payable {
        ...
        require(totalSupply() < MAX_ELEMENTS, 'All tokens have been minted');
        require(totalSupply() + numberOfTokens < MAX_ELEMENTS, 'Purchase would exceed max supply');
        require(_allowListClaimed2[msg.sender] + numberOfTokens <= PURCHASE_LIMIT2, 'Purchase exceeds max allowed');
    ...
    function publicMint(uint256 numberOfTokens) external payable {
        ...
        require(totalSupply() < MAX_ELEMENTS, 'All tokens have been minted');
        require(totalSupply() + numberOfTokens < MAX_ELEMENTS, 'Purchase would exceed max supply');
        require(_allowListClaimed3[msg.sender] + numberOfTokens <= PURCHASE_LIMIT2, 'Purchase exceeds max allowed');
