    ...
    PaymentSplitter
{
    ...
    uint256 public GIFT_SUPPLY = 45;
    uint256 public PUBLIC_SUPPLY = 9955;
    uint256 public MAX_SUPPLY = GIFT_SUPPLY + PUBLIC_SUPPLY;
    ...
    uint256 public totalGiftMinted;
    ...
    constructor(
        ...
        payable
        ERC721("Strange Hands", "STRNGHAND")
        Pausable()
    ...
    function presalePurchase(uint256 _quantity)
        external
        payable
        override
        nonReentrant
    ...
    function _mint(uint256 _quantity) internal {
        ...
        require(
            _quantity + totalPublicMinted <= PUBLIC_SUPPLY,
        ...
        for (uint256 i = 0; i < _quantity; i++) {
            ...
            uint256 tokenID = GIFT_SUPPLY + totalPublicMinted;
    ...
    function getTokenOffset() public view override returns (uint256) {
        require(tokenOffset != 0, "Offset has not been generated");

        return tokenOffset;
    }

    function offsetId(uint256 _tokenId) public view override returns (uint256) {
        require(_exists(_tokenId), "Query for nonexistent token");

        return (_tokenId + getTokenOffset()) % MAX_SUPPLY;
    }

    // @dev override ERC721 function for returning the baseTokenURI
    function _baseURI()
        internal
        view
        virtual
    ...
    function addPresaleApprovedAddresses(address[] calldata _addresses)
        external
        override
        onlyOwner
    {
    ...
    function gift(address[] calldata _recipients) external override onlyOwner {
        require(
            _recipients.length + totalGiftMinted <= GIFT_SUPPLY,
            "_quantity exceeds supply"
        );

        for (uint256 i = 0; i < _recipients.length; i++) {
            uint256 tokenID = totalGiftMinted;
...
interface IStrange {
    ...
    function getTokenOffset() external view returns (uint256);

    function offsetId(uint256 _tokenId) external view returns (uint256);
