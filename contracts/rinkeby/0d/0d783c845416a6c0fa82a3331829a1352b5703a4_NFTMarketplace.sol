...
contract NFTMarketplace is Pausable, FeeManager, ERC721Holder {
    ...
    function safePlaceBid(address _tokenAddress, uint256 _tokenId) payable public whenNotPaused {
       ...
       // address owner = IERC721(_tokenAddress).ownerOf(_tokenId);
        // require( tokenOwner != msg.sender, "The owner of NFT cannot place bid itself");
    ...
    function _createBid(address _tokenAddress, uint256 _tokenId,  uint256 value)  internal   {
        ...
        uint256 acceptableBid = bid.bidPrice + ( bid.bidPrice* 100000000000000000); // The bid must be 10% higher than the last bid
        
        if(bid.bidId != 0) { 
            
            
                require(value >= acceptableBid, "Marketplace: bid price should be 10 higher than last bid");
        ...
        {
            require(value == order.askingPrice, "Marketplace: Bid should be equal to reserve price");
