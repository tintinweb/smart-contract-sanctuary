...
interface   IPEZToken
{
    function    pezNftStake(string memory collectionName, uint256 tokenId, address userWallet) external returns(TPNS memory);
...
struct TAffiliate 
{
    ...
    string      username;
    bool        enabled;    
    uint256     saleCount;      // increment +1 a chaque MINT/stake amener
}
//--------------------------------------------------------------------------------
struct TTimePhases
{
...
contract DLZToken     is  ERC721Enumerable, ReentrancyGuard
{
    ...
    uint256 public     stakePresalesPrice = 0.0008 ether;

    uint256 public     salesTimestamp     = 2000000000;
    uint256 public     salesPrice         = 0.0005 ether;
    uint256 public     stakeSalesPrice    = 0.0008 ether;
    
    uint256 public     stakingOnlyPrice   = 0.0005 ether;
    ...
    string  public     baseURI = '';       // FOR SECURITY REASON THIS WILL BE SET LATER
    ...
    mapping(uint256 => bool)    public tokenStakingStates;
    ...
    uint256 public     mintingMode = 1;        // 1: Presale(whitelist)   2: PublicSale   0: disabled 
    
    mapping(address => uint256) public whitelistingUsers;
    ...
    event b5(uint256 isWhiteStaking);
    ...
    constructor()   ERC721("DL01 Token", "DL01")   // temporary Symbol and title
    {
        ...
        setPezContract(0x25E5619545e6c1c5d4e8e316197917FaB4cF4DCE);
    }
    //=============================================================================
    ...
    function    setPresalesTimestamp( uint256 newPresalesTimestamp)  external onlyOwner         { presalesTimestamp = newPresalesTimestamp; }
    function    setSalesTimestamp(    uint256 newSalesTimestamp)     external onlyOwner         { salesTimestamp    = newSalesTimestamp;    }
    ...
    function    addWhitelistUsers(address[] memory wallets)    external onlyOwner
    {
        ...
        for (uint i=0; i<n; i++)
        {
            ...
            whitelistingUsers[wallet]      = 1;
            whitelistingTimestamps[wallet] = block.timestamp;
    ...
    function    addWhitelistUser()    external 
    {
        ...
        whitelistingUsers[msg.sender]      = 1;
        whitelistingTimestamps[msg.sender] = block.timestamp;
        
        emit onWhitelistUserAdded(msg.sender, block.timestamp);
    }
    //=============================================================================
    function    getWhitelistedUsersSubscribeTimestamps(address[] memory wallets)    external view returns(uint256[] memory)
    {
    ...
    //=============================================================================
    function    getMintingMode() public view returns(string memory)
    {
             if (mintingMode==1)    return "presales";       // 1: Presale(whitelist)   2: PublicSale   0: disabled 
        else if (mintingMode==2)    return "sales";
        else                        return "off";
    }
    //=============================================================================
    ...
    //=============================================================================
    function    getPriceForStaking()   view public returns(uint256)
    {
        return stakingOnlyPrice;
    }
    //=============================================================================
    ...
    function    reserve(uint256 quantity)                   external onlyOwner
    {
        ...
        for (uint256 i=0; i<quantity; i++)
        {
            ...
            PEZ.pezNftStake(collectionName, mintedTokenCount, owner());
        }
    }
    //=============================================================================
    function    stakeMyNft(uint256 tokenId, uint256 affHash)                 external payable callerIsUser nonReentrant
    {
        ...
        tokenStakingStates[tokenId]  = true;            // valider ce token comme etant un token avec staking
    ...
    function    mint(uint256 quantity, uint256 affHash)                      external payable callerIsUser nonReentrant
    {
        ...
        mintEx(quantity, unitPrice, isStakingAllowed, affHash);
    }
    //=============================================================================
    function    mintWithStaking(uint256 quantity, uint256 affHash)           external payable callerIsUser nonReentrant 
    {
        bool    isStakingAllowed  = false;
        ...
        if (isStakingAllowed==true)         unitPrice = getPriceForMintingWithStake();
        else                                unitPrice = getPriceForMinting();           // Un whitelisté ne paiera pas le plein pot
        
        mintEx(quantity, unitPrice, true, affHash);
    }
    //=============================================================================
    function    mintEx(uint256 quantity, uint256 unitPrice, bool isWithStaking, uint256 affHash) internal callerIsUser
    {
...
/*
        ...
        emit b4(n<maxMintPerWallet ? 1:0);
        
        emit b5(isWithStaking ? 1:0);
        emit b6(affHash);
...
*/
        ...
        for (uint256 i=0; i < quantity; i++)
        {
            ...
            tokenStakingStates[mintedTokenCount]  = isWithStaking;

            //----- Now start the staking of this NFT if it's asked to do so.
        
            if (isWithStaking)
            {
                TPNS memory p = PEZ.pezNftStake(collectionName, mintedTokenCount, msg.sender);
                
                emit pns(p.collection, p.tokenId, p.userWallet, p.caller);
                //emit pns(collectionName, mintedTokenCount, msg.sender, address(this));
            }
    ...
    function    getAffiliation(uint256[] memory affHashes) view external returns(uint256[] memory)
    {
        ...
        for (uint256 i=0; i<nHash; i++)
        {
            ...
            affSales[i] = affiliate.saleCount;
        }
    ...
    {
        string memory   phase = getMintingMode();
    ...
    function    getNftBonusStates() external view returns(TBonusState[] memory)
    {
        ...
        for (uint256 i=0; i<count; i++)
        {
            ...
            result[i]       = TBonusState(tokenId, tokenStakingStates[tokenId]);
        }
