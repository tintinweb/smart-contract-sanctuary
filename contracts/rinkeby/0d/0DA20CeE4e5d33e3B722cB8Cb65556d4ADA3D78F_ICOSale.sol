...
contract ICOSale is Ownable, AllowanceCrowdsale{
    ...
    **/
    enum ICOStage { Stage1, Stage2 }
    ICOStage public stage = ICOStage.Stage1;
    ...
    **/
    ...
    onlyOwner
    {
        require(_stage == 0 || _stage == 1, "Stage can be either 0 or 1.");
        if(uint(ICOStage.Stage1) == _stage) {
            stage = ICOStage.Stage1;
        } else if (uint(ICOStage.Stage2) == _stage) {
            stage = ICOStage.Stage2;
        }
    }
...
contract Crowdsale is Context, ReentrancyGuard, Ownable {
    ...
    // number of tokens bought by the investors.
    ...
    /**
     * lets the investors @param _beneficiary to withdraw their tokens.
     */
