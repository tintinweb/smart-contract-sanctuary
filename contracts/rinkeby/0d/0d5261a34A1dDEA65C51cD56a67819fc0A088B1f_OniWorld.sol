    ...
    Ownable
{
    ...
    string[5] public baseTokenURI = [
        "https://gateway.pinata.cloud/ipfs/QmYrfA72SNSGFjva5UxmNMhF2qHx9kFbcCBgcJMsi1354s/0/",
        "https://gateway.pinata.cloud/ipfs/QmYrfA72SNSGFjva5UxmNMhF2qHx9kFbcCBgcJMsi1354s/1/",
        "https://gateway.pinata.cloud/ipfs/QmYrfA72SNSGFjva5UxmNMhF2qHx9kFbcCBgcJMsi1354s/2/",
        "https://gateway.pinata.cloud/ipfs/QmYrfA72SNSGFjva5UxmNMhF2qHx9kFbcCBgcJMsi1354s/3/",
        "https://gateway.pinata.cloud/ipfs/QmYrfA72SNSGFjva5UxmNMhF2qHx9kFbcCBgcJMsi1354s/4/"
        ];
    ...
    mapping(uint256 => oniInfo) public oni;
    uint8 maxLevel = 4;

    //OniBaseURIs
    // mapping(uint8 => string) oniBaseURIs;
    ...
    event ClaimedFree(address owner, uint256 claimTokenId);
    ...
    modifier onlyOniOwner() {
        uint256[] memory tokens = _walletOfOwner(_msgSender());
        uint256 balance = balanceOf(_msgSender());
        require(tokens.length == balance, 'only oni owner of all tokens can use this function');
        _;
    }
    ...
    //level up functions
    /** 
     * @dev 1. nft that win on the web to get leveling up
     * 2. check if tokenId is ownerOf(msg.sender) ?
     * 3. check if tokenId is on which level ? also winning verification data.
     * 4. increase level
     * 5. setNewBaseURI
    */
    ...
    */
    function _increseOniLevel(uint256 tokenId, uint8 level) internal {
        uint8 previousLevel = oni[tokenId].level;
        require(previousLevel <= maxLevel, 'max level!');
        oni[tokenId].level = previousLevel + level;
        oni[tokenId].fail = false;

        emit OniLevelIncreased(tokenId, previousLevel, oni[tokenId].level);
    }
    
    /**
     * @dev calculate chance
    */
    function _calculateChange() internal view returns(uint256) {
    ...
    */
    ...
    function _updateFailToken(uint256 tokenId) internal {
        oni[tokenId].level = 1;
        oni[tokenId].fail = true;
    }
    
    /**
     * @dev next round
    */
    function _setNextRound() public onlyOwner {
        currentRound++;
    }
    ...
    */
    function stopTesting(bool val) public onlyOwner {
        testEnded = val;
    }
    
    //Testing function
    /** 
    * @dev 3 round to be at level 3 (need to pass 3 stages (0 - 2))
    */
    function oniTest(uint256[] calldata tokens) public onlyOniOwner() {
      ...
      require(!testEnded, 'Testing has been ended');
      
      if(tokens.length < 2) {
          //calculate chance;
          uint chance = _calculateChange();
         ...
         if(currentRound == 1 && checkOniLevel(tokens, 0)) {
             //round 1
             require(tokens.length == 2, '[round1] need 2 level 0 for this testing.');
         ...
         if(currentRound == 2) {
             ...
             if(checkOniLevel(tokens, 2)) {
                 require(tokens.length == 2, '[round2] need 2 level 2 for this testing');
                 _batchUpdate(tokens, 1);
             } 
             if (checkOniLevel(tokens, 0)) {
                 require(tokens.length == 4, '[round2] need 4 level 1 for this testing');
         ...
         if(currentRound == 3) {
             ...
             if(checkOniLevel(tokens, 3)) {
                 require(tokens.length == 2, '[round3] need 2 Oni level 3 for this testing');
                 _batchUpdate(tokens, 1);
             }
             if(checkOniLevel(tokens, 2)) {
                 require(tokens.length == 4, '[round3] need 4 level 2 for this testing');
                 _batchUpdate(tokens, 2);
             }
             if(checkOniLevel(tokens, 0)) {
                 require(tokens.length == 8, '[round3] need 8 level 0  for this testing');
             }
         }
      }
    }
   ...
   //Owner Functions 
   function setFreeMint(bool result) external onlyOwner {
       is_free_mint = result;
   }

    function setBuy1Get1(bool result) external onlyOwner{
        is_one_get_one = result;
    }
    
    function setRevealTime(uint256 temp_time) external onlyOwner{
    ...
    function claimFree() external {
        ...
        emit ClaimedFree(_msgSender(), _currentTokenId);
    }
  ...
  function withdraw() external onlyOwner {
    ...
    /**
    * @dev getTokenURI for token level > 0
    */
    function _getTokenUri(uint256 tokenId, uint8 level) internal view returns(string memory) {
        string memory baseURI = baseTokenURI[level];
    ...
    function tokenURI(uint256 tokenId)
        ...
        if(oni[tokenId].level > 0) {
            return _getTokenUri(tokenId, oni[tokenId].level);
        }

        return _getTokenUri(tokenId, 0);
    }
