...
contract Hancom1155 is ERC1155, Ownable {
    ...
    address payable private _addressA;    // 한컴
    address payable private _addressB;    // 투게더아트
    uint16 private _shareRateA = 55;    // B의 수수료율은 100 - shareRateA
    uint16 private _fee = 250; // 플랫폼 수수료 2.5%
    ...
    ERC1155(string(abi.encodePacked(baseURI_, "{id}"))) payable {
        // Arguments of ERC1155 constructor should be contract metadata ipfs URL
    ...
    // NOTE: Ownable.transferOwnership 과 중첩
    /*function switchOwnership(address newOwner) public onlyOwner {
        transferOwnership(newOwner);
    }*/

    /*function getTitleExists(string memory title) public view returns (bool) {
        return titleExists[title];
    }*/
    ...
    /*function getCurrencyAddress(uint16 currencyId) public view onlyOwner returns (address) {
        return _currencyAddresses[currencyId];
    }*/
    ...
    /*function setShareB(address address) public onlyOwner {
        addressB = address;
    }*/
    ...
    /*function setFee(uint16 fee) public onlyOwner {
        fee = fee;
    }*/

    /*function getFee() public view onlyOwner returns (address, address, uint16) {
        return (addressA, addressB, shareRateA);
    }*/
    ...
    function mintToken(string memory title, string memory uri, uint16 royalty, uint256 amount,
        ...
        //string memory uri = string(abi.encodePacked(baseURI, uri));
        ...
        // ERC1155 function uri use the value added baseURI
        // -> input value : baseURI + uri
        // NOTE: tokens 에 저장된 값을 활용
        //setTokenURI(tokenId, uri);

        //titleExists[title] = true;

        //tokens[tokenId] = Token(true, title, uri, royalty, payable(_msgSender()), amount);
    ...
    // tradeToBuy.saleId.toString(), tradeToBuy.buyAmount.toString(), apiDataId
    function buyToken(uint256 saleId, uint256 amount, uint256 dataId) external payable {
...

        {
            ...
            // Transfer NFT(tokenId) by the amount
            //safeTransferFrom2(sale.seller, _msgSender(), sale.tokenId, amount, "0x0");
            _safeTransferFrom(sale.seller, _msgSender(), sale.tokenId, amount, "0x0");
        }

        {
            // 판매 조정
            if (sale.amount > amount) {// 판매 후 잔여 수량이 있으면
                // 판매정보 수량 = 기존 판매수량 - 판매된 수량
                sales[saleId].amount = sale.amount - amount;
            } else {// 판매 후 잔여 수량이 없으면
                // 판매정보 삭제
                delete sales[saleId];
            }
        }
        ...
        {
            // 구매자 보유수량 조정
            if (!holds[sale.tokenId][_msgSender()].isPresent) {// 보유정보에 없으면
                // 새로운 보유정보 추가
                holds[sale.tokenId][_msgSender()] = Hold(true, amount);
            } else {// 보유정보에 있으면
                // 기존 보유정보 수량에 구매수량만큼 더함
    ...
    function changeSale(uint256 tokenId, uint256 saleId, bool putOnSale, uint16 currencyId, uint256 price, uint256 amount, uint256 dataId) external {
        ...
        if (putOnSale) {// 판매시작
            ...
            if (restHoldAmount > 0) {// 보유잔량이 남아 있으면
                // 판매자 보유잔량 = 변경 후 보유잔량
            ...
            // 현재 보유수량이 없으면
            if (holdAmount == 0) {
                // 신규 보유정보 추가, 수량은 기존 판매정보의 수량
                holds[tokenId][_msgSender()] = Hold(true, sales[changeSaleId].amount);
            } else {// 현재 보유수량이 있으면
                // 기존 보유정보 수정, 수량은 현재 보유수량 + 기존 판매정보의 수량
    ...
    function burn(address account, uint256 tokenId, uint256 amount) public {
        ...
        // 해당 토큰 총 수량과 일치해야만 소각 가능

        //require(tokens[tokenId].amount == amount);
        // 해당 토큰 보유자의 보유수량 이하까지만 소각 가능
        //require(holds[tokenId][account].amount >= amount);
        // 메세지센더가 컨트랙트 오너일 때
        if (_msgSender() == owner()) {
            _burn(account, tokenId, amount);
        } else {
            // 그 외에는 _msgSender 가 본인이어야 함
            require(_msgSender() == account);
            _burn(account, tokenId, amount);
        }

        //delete titleExists[tokens[tokenId].title];
        delete tokens[tokenId];
    }

    // NOTE: https://github.com/rarible/protocol-contracts/blob/master/tokens/contracts/erc-1155/ERC1155BurnableUpgradeable.sol
    ...
    /*function burnToken(uint256 tokenId, address[] memory accounts, uint256[] memory amounts) public onlyOwner {
        // 입력받은 토큰 소유자, 수량으로 반복 처리
        // TODO: 컨트랙트 내에서 account 당 총 수량을 가져올 수 없어 밖에서 넣어줘야 함
        require(tokens[tokenId].isPresent
            && accounts.length == amounts.length);

        for (uint256 i = 0; i < accounts.length; i++) {
            burn(accounts[i], tokenId, amounts[i]);
        }
    }*/
