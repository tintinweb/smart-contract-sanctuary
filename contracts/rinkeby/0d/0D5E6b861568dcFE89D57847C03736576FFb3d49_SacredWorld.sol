...
contract SacredWorld is ERC721Enumerable, Ownable {
    ...
    //mint cost
    uint256 public presaleCost = 0.00008 ether;
    uint256 public publicCost = 0.00008 ether;

    //max supply
    uint256 public presaleMaxSupply = 10;
    uint256 public publicMaxSupply = 20;

    //max mint
    uint256 public presaleMintLimit = 2;
    uint256 public publicMintLimit = 4;
    ...
    function mint(uint256 _mintAmount) public payable {
        require(presaleStart == true || publicStart == true, "Neither of the sale is start yet!");
        uint256 supply = totalSupply();
        require(_mintAmount > 0, "need to mint at least 1 NFT");
        require(pause == true, "minting not started yet!");
        ...
        if(presaleStart == true){
            ...
            require(totalSupply() + _mintAmount <= presaleMaxSupply, "max NFT presale limit exceeded");
            require(addressMintedBalance[msg.sender] + _mintAmount <= presaleMintLimit, "You can mint max 3 NFTs!");
            for (uint256 i = 1; i <= _mintAmount; i++) {  
                _safeMint(msg.sender, supply + i);
            }
            transactions[msg.sender].mintTime = block.timestamp;
            addressMintedBalance[msg.sender]+=_mintAmount;
        }

        else if(publicStart == true){
            require(_mintAmount <= publicMintLimit, "You can mint in range (1-5) NFT!");
            require(msg.value >= presaleCost * _mintAmount, "insufficient funds");
            require(totalSupply() + _mintAmount <= publicMaxSupply, "max NFT presale limit exceeded");
            require(addressMintedBalance[msg.sender] + _mintAmount <= publicMintLimit, "You can mint max 5 NFTs!");
            for (uint256 i = 1; i <= _mintAmount; i++) {  
                _safeMint(msg.sender, supply + i);
            }
            transactions[msg.sender].mintTime = block.timestamp;
    ...
    mapping(address=>uint256) prevTokens;

    function calculateTokens(address _user) public view returns(uint256){
        uint256 tokensOfUser;
        uint256 tokens=(block.timestamp - transactions[_user].mintTime)/(1 minutes);
        uint256 totalTokens= (tokens*5)*(addressMintedBalance[msg.sender]);
        tokensOfUser+=totalTokens;
        return tokensOfUser;
    }

    function claimTokens() public {
        require(calculateTokens(msg.sender) > 0,"You don't have tokens for reward!");
        prevTokens[msg.sender] = 0;
        transactions[msg.sender].mintTime = block.timestamp;
    ...
    function claimById(uint8 _tokenId) public{
        ...
        require(calculateTokens(msg.sender) > 0,"You don't have tokens for reward!");
        uint256 tokensById = calculateTokens(msg.sender)/addressMintedBalance[msg.sender];
