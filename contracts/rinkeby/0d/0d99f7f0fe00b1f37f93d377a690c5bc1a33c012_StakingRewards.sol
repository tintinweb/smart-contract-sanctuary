...
contract StakingRewards is IStakingRewards, Initializable, AccessControlEnumerableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    */
     ...
     */
    mapping(address => uint256) public _rewardsPerTokenPaid;

    /**
     * @notice Staking rewards earned per block for the entire staking pool
     */
    uint256 public _stakingRate;
    ...
    function initialize(address flurryToken) public initializer {
        ...
        _rewardsTokenOne = 10**IERC20MetadataUpgradeable(_rewardsTokenAddress).decimals();
    ...
    function rewardsPerToken() public view override returns (uint256) {
        ...
        return _rewardsPerToken.add(
            (lastBlockApplicable().sub(_lastUpdateBlock)).mul(_stakingRate).mul(_rewardsTokenOne).div(_totalStakes)
        );
    }

    /**
     * @notice Calculate and allocate rewards token for stake holder
     * Staking rewards should be calculated from _lastUpdateBlock to lastBlockApplicable
     */
    function updateReward(address addr) public {
        _rewardsPerToken = rewardsPerToken();
        _lastUpdateBlock = lastBlockApplicable();
        if (addr != address(0)) {
            _rewards[addr] = earned(addr);
            _rewardsPerTokenPaid[addr] = _rewardsPerToken;
            emit RewardEarned(addr, _rewards[addr]);
        }
    }

    function earned(address addr) public view returns (uint256) {
        return _stakes[addr].mul(rewardsPerToken().sub(_rewardsPerTokenPaid[addr])).div(_rewardsTokenOne).add(_rewards[addr]);
    }
    ...
    function getRewardForDuration() external view override returns (uint256) {
        return _stakingRate.mul(_rewardsDurationInBlocks);
    }
    ...
    function setRewardAmount(uint256 reward) external override onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        require(_rewardsDurationInBlocks > 0, "Rewards duration is 0");

        if (block.number >= _rewardsEndBlock) {
            _stakingRate = reward.div(_rewardsDurationInBlocks);
        } else {
            uint256 blocksRemaining = _rewardsEndBlock.sub(block.number);
            uint256 leftover = blocksRemaining.mul(_stakingRate);
            _stakingRate = reward.add(leftover).div(_rewardsDurationInBlocks);
        }
        ...
        require(_stakingRate <= balance.div(_rewardsDurationInBlocks), "Insufficient balance for rewards");

        _lastUpdateBlock = block.number;
        _rewardsEndBlock = block.number.add(_rewardsDurationInBlocks);
        emit RewardAdded(reward);
        emit RewardsEndUpdated(_rewardsEndBlock);
    }
     ...
     * Can only called by Owner
     */
    function setRewardsDuration(uint256 _rewardsDurationInSeconds) external override onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            block.number > _rewardsEndBlock,
            "Previous rewards period must be completed before changing the duration for the new period"
        );
        _rewardsDurationInBlocks = _rewardsDurationInSeconds.mul(1e3).div(BLOCK_TIME_MS);
        emit RewardsDurationUpdated(_rewardsDurationInBlocks);
    }
    ...
    function shortenRewardsDuration() external override onlyRole(DEFAULT_ADMIN_ROLE) {
        if (_rewardsEndBlock > block.number) {
     ...
     */
    function stake(uint256 amount) external nonReentrant override {
        address user = _msgSender();
        require(amount > 0, "Cannot stake 0 tokens");
        require(_rewardsToken.balanceOf(user) >= amount, "Not Enough balance to stake");
        updateReward(user);
    ...
    function claimReward() external override nonReentrant {
        ...
        updateReward(user);
        if (_rewards[user] > 0) {
            _rewards[user] = grantFlurryInternal(user, _rewards[user]);
        }
    }
    ...
    */
    function grantFlurryInternal(address user, uint256 amount) internal returns (uint256) {
...
interface IStakingRewards {
     ...
     */
    function stakeOf(address addr) external view returns (uint256);
