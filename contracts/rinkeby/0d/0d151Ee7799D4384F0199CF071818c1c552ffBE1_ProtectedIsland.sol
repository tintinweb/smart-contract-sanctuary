...
contract ProtectedIsland is Ownable, IERC721Receiver, Pausable {
  ...
  mapping(address => StakedTokens) stakedUser;
  ...
  //   uint256 public constant MINIMUM_TO_EXIT = 20;
  uint256 public constant MINIMUM_TO_EXIT = 2 days; // TODO: COMMENT THIS IN
  ...
  function _addTortoiseToProtectedIsland(address account, uint256 tokenId) internal whenNotPaused _updateEarnings {
    ...
    stakedUser[account].tokens.push(tokenId);
  ...
  */
  function getStakedTokens(address _wallet) external view returns (uint256 [] memory) {
      return stakedUser[_wallet].tokens;
  }
  ...
  */
  function deleteStakedTokenFromUserByIndex(uint _index) internal returns(bool) {
    for (uint i = _index; i < stakedUser[_msgSender()].tokens.length - 1; i++) {
      stakedUser[_msgSender()].tokens[i] = stakedUser[_msgSender()].tokens[i + 1];
    }

    stakedUser[_msgSender()].tokens.pop();
    return true;
  } 
  ...
  */
  function deleteStakedToken(address _user, uint256 _tokenId) internal {
    for (uint256 index = 0; index < stakedUser[_user].tokens.length; index++) {
      if(stakedUser[_user].tokens[index] == _tokenId) {
