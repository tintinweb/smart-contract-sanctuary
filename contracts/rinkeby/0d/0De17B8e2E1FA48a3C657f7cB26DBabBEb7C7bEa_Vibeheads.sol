...
contract Vibeheads is ERC721Enumerable, EIP712 {
    ...
    string private constant SIGNING_DOMAIN = "VibePass";
    string private constant SIGNATURE_VERSION = "1";
    string public provenance;
    string public baseURI;
    string public concealedURI;
    ...
    function mint(uint256 mintAmount) external payable returns (uint256) {
        ...
        if (msg.sender == owner) {
            ...
            require(mintAmount <= maxMintAmount, "Mint amount greater than max mint amount");
        }
        require(mintAmount > 0, "Mint amount should be greater than 0");
        uint256 supply = totalSupply();
        require(supply + mintAmount <= maxSupply, "Sold out!");
        require(msg.value >= mintCost * mintAmount, "Insufficient funds");
        _mintVibehead(msg.sender, mintAmount, supply);
        return supply + mintAmount;
    }

    function redeem(VibePass calldata pass, uint256 yieldAmount) external payable returns (uint256) {
        require(!preSalePaused, "Presale is paused");
        address redeemer = msg.sender;
        address signer = _verify(pass);
        require(signer != address(0), "Unable to recover signer from signature");
        require(signer == owner, "Invalid Vibe Pass");
        require(msg.sender == pass.recipient, "This Vibe Pass does not belong to you");
        require(yieldAmount > 0, "Yield amount should be greater than 0");
        require(yieldAmount <= pass.maxAmount, "Yield amount greater than allowed limit");
        require(yieldAmount <= pass.maxAmount - presaleRedeemed[redeemer], "Vibe Pass limit reached");
        uint256 supply = totalSupply();
        require(supply + yieldAmount <= maxSupply, "Max Vibeheads limit crossed");
        require(msg.value >= cost * yieldAmount, "Insufficient funds");
        presaleRedeemed[redeemer] += yieldAmount;
        _mintVibehead(redeemer, yieldAmount, supply);
        return supply + yieldAmount;
    }

    function setUri(string memory _base, string memory _concealed) external {
        require(msg.sender == owner, "You can't change Uris");
        baseURI = _base;
        concealedURI = _concealed;
    }
    ...
    function reveal() external {
        require(msg.sender == owner, "You can't change reveal");
        revealed = true;
    }
