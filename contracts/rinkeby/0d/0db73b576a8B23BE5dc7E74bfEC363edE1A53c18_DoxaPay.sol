...
// File: gist-71572af562f01852a1e328dba89471fe/doxa/doxa-pay.sol
...
contract DoxaPay is Ownable {
    ...
    event DoxaPayment(address indexed sender, string indexed id, uint indexed amount);
    event ETHPayment(address indexed sender, string indexed id, uint indexed amount);
    event AppWalletPayment(address indexed sender, string indexed id, uint indexed amount);
    
    constructor(address admin, address _token, address _wallet) {
        token = IERC20(_token);
        adminAddress = admin;
    ...
    function payDoxa(uint amount, string memory id) public {
        ...
        emit DoxaPayment(msg.sender, id, amount);
    }
    ...
    function payETH(string memory id) public payable {
        uint amount = msg.value;
        payable(adminAddress).transfer(amount);
        emit ETHPayment(msg.sender, id, msg.value);
    }
    ...
    function payUsingAppWallet(uint amount, string memory id) public {
        ...
        emit AppWalletPayment(msg.sender, id, amount);
    }
