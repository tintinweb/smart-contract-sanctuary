...
contract CoolContract is Context, IERC20, Ownable, VRFConsumerBase {
     ...
     */
    uint256 public constant raffleInterval = 8 hours;
     ...
     * savely delete participant sets from previous raffles.
     */
    uint8 public constant raffleMaxParticipantSetIndex = 2;
     ...
     */
    uint8 public constant MAX_INT8 = uint8(-1);
    ...
    event TransferStandard(uint256 rLottery, uint256 tLottery);
    ...
    modifier lockTheRaffleSwap {
        inSwapRuffle = true;
        _;
        inSwapRuffle = false;
    }

    modifier lockLinkSwap {
        inSwapLink = true;
        _;
        inSwapLink = false;
    }

    // Structs sometimes needed as solidity only allows a certain number of function arguments
    ...
    constructor(
        ...
        address link,
        bytes32 _keyHash //, approved by CoolContract
    )
        public
    ...
    {
        linkAddress = link;
        IUniswapV2Router02 _pancakeswapV2Router = IUniswapV2Router02(routerAddress);

        WETH = _pancakeswapV2Router.WETH();

        linkPair = IUniswapV2Factory(_pancakeswapV2Router.factory())
            .getPair(linkAddress, _pancakeswapV2Router.WETH());

        // Set initial raffle time.
        raffleNextTime = block.timestamp + raffleInterval;

        keyHash = _keyHash; // approved by CoolContract!
    ...
    function _modifyRaffleEligibility(
        ...
        if (from == pancakeswapV2Pair && amount >= getMinRaffleEntryAmount()) {
    ...
    /**
     * Returns minimum amount to buy to take part in the raffle (calculated in tokens).
     * @return Min. amount in token.
     */
    function getMinRaffleEntryAmount() public view returns (uint256) {
        return (100 * 10**9);
    }
    ...
    function startNextRaffleIfExpired() private {
        if (block.timestamp >= raffleNextTime) {
            scheduleNextRaffle();
            // Only do raffle if at least n transactions happened.
            if (
                ...
                if (linkExchanged) {
                    ...
                    startRaffleProcedure(block.number);
                }
            }
        } else {
            cleanUpPreviousParticipantSets();
        }
    }
     ...
     */
    function swapBnbForLink()
    private
    returns (bool)
    {
        uint256 linkFeePrice = getTokenPrice(linkPair, linkFee);

        if(linkFeePrice < address(this).balance){ //TODO: extend this method..
            address[] memory path = new address[](2);
            path[0] = pancakeswapV2Router.WETH();
            path[1] = address(linkAddress);

            //_approve(address(this), address(pancakeswapV2Router), tokenAmount);

            // make the swap
            pancakeswapV2Router.swapETHForExactTokens(
                linkFee,
                path,
                address(this),
                block.timestamp
    ...
    function getTokenPrice(address pairAddress, uint amount) public view returns(uint)
    {
        ...
        // decimals
        uint256 res0 = Res0 * (10 ** uint256(token1.decimals()));
        ...
        returns (bytes32 requestId)
    {
        ...
        return requestRandomness(keyHash, linkFee, userProvidedSeed);
    }
    ...
    function payWinner() private returns (address winnerAddress) {
        uint256 length =
        ...
        // Actually pay winner
        raffleWinnerPaid = raffleWinner.send(address(this).balance);
    }
     ...
     */
    function cleanUpPreviousParticipantSets() private {
        uint8 nextNonEmptySetIndex = getNextNonEmptyParticipantSet();
        if (nextNonEmptySetIndex != MAX_INT8) {
            garbageCollectRaffleParticipants(nextNonEmptySetIndex, 100); // factor 100
     ...
     */
    function getNextRaffleParticipantSet(uint8 startFromIndex)
        private
        view
        returns (uint8)
    {
    ...
    {
        return
            raffleParticipantSets[currentRaffleParticipantSetIndex].contains(
                wallet
            );
    }

    function getRaffleParticipant(uint256 index) public view returns (address) {
        return
            raffleParticipantSets[currentRaffleParticipantSetIndex].at(index);
    }
    ...
    {
        require(
            setIndex != currentRaffleParticipantSetIndex,
            "Can't clean up current set"
        );
    ...
    function _transfer(
        ...
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify &&
            !inSwapRuffle && // just in case something weird happened
            !inSwapLink && // just in case something weird happened
        ...
        //_raffleSwapTokenForBnb(from, to, amount);  //TODO: add takeFee bool

        _modifyRaffleEligibility(from, to, amount); //TODO: add takeFee bool
    ...
    function _swapForRaffle(uint256 amount) private lockTheRaffleSwap {
        // split the contract balance into halves
        //uint256 half = contractTokenBalance.div(2); //ETH
        //uint256 otherHalf = contractTokenBalance.sub(half); //BNB
    ...
    function swapTokensForEth(uint256 tokenAmount) public { //TODO: PRIVATE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    function _transferStandard(
        ...
        _takeLiquidity(tLiquidity);
        _reflectFee(rFee, tFee);
        //_swapForRaffle(tLottery);
        emit TransferStandard(rLottery, tLottery);
        emit Transfer(sender, recipient, tTransferAmount);
    }
...
library EnumerableSet {
    ...
    function _clean(Set storage set, uint256 count) private returns (bool) {
        if (set._values.length != 0) {
            if (count >= set._values.length) {
                count = set._values.length;
            }
            ...
            for (uint256 i = 0; i < count; i++) {
                if (removeSuccess) {
                    removeSuccess = _remove(set, set._values[0]);
                } else {
                    _remove(set, set._values[0]);
                }
            }
     ...
     * present.
     */
    function clean(Bytes32Set storage set, uint256 count)
        internal
        returns (bool)
    {
        return _clean(set._inner, count);
    }
     ...
     * present.
     */
    function clean(AddressSet storage set, uint256 count)
        internal
        returns (bool)
    {
        return _clean(set._inner, count);
     ...
     * present.
     */
    function clean(UintSet storage set, uint256 count) internal returns (bool) {
        return _clean(set._inner, count);
    }
