...
/** @title Launchpad Sale contract */
contract Sale is Ownable, AccessControlEnumerable {   
/** @dev This is core contract for making token sale process:
  ...
  */   
    ...
    struct saleRecord {
        ...
        // sale token price in payment tokens (Rate)
        uint256 tokenPrice; //1 paymentToken = 1000, to get real need to divide by 10000 (20211209)
    ...
    // saler withdraw payment tokens
    // saleID => bool. Defaulte false
    mapping(uint => bool) internal withdrawCompleted;
    ...
    mapping(uint256 => mapping(address => stakedallocs)) public saleAllocations;

    // payment from the user by sale, need to check only one payment by user
    // saleID => wallet => payment
    mapping(uint256 => mapping(address => uint256)) public salePayments;
    ...
    modifier onlySaleOperator() {
        require(hasRole(CREATE_SALE_ROLE, msg.sender), "Sale::onlySaleOperator: incorrect sale role");
        _;
    }
    ...
    constructor(address _stakingContract) {
        require(_stakingContract != address(0), "Sale::constructor: need staking contract!");
        totalSales = 0;
     ...
     /// @notice dates array:
     /// @notice 0-approveBlockStart
     /// @notice 1-approveBlockStop
     /// @notice 2-saleBlockStart
     /// @notice 3-saleBlockStop
     /// @notice 4-claimBlockStart
     /// @notice 5-claimBlockStop
    ...
    ) external onlySaleOperator {
        require(tokenAmount > 0, "Sale::createSale: 0 amount");
        require(
            dates[0] < dates[1] &&
            dates[1] < dates[2] &&
            dates[2] < dates[3] &&
    ...
    function withdraw(uint256 saleID) external {
        require(sales[saleID].saleOwner == msg.sender, "Sale::withdraw: only seller");
        require(getSalestage(saleID) == stages.CLAIM, "Sale::withdraw: only CLAIM stage");
        require(!withdrawCompleted[saleID], "Sale::wihdraw: already withdraw this sale");

        withdrawCompleted[saleID] = true;

        // withdraw all not sold tokens
        sales[saleID].token.safeTransfer(msg.sender, sales[saleID].tokenAmount - sales[saleID].soldTokens);

        if (sales[saleID].tokenAmount > 0) {
            sales[saleID].paymentToken.safeTransfer(msg.sender, sales[saleID].receivedPayments);
        }
    }
    ...
    function changeStaking(address _newStaking) external onlyOwner {
        require(_newStaking != address(0), "Sale::changeStaking: address is empty");
        baseStaking = ISwapStakingContract(_newStaking);
    }

    /// @param _minStakeRegAmount min tokens amount for register stake 
    function setMinStakeRegAmount(uint256 _minStakeRegAmount) external onlyOwner {
        minStakeForRegister = _minStakeRegAmount;
    }

    /// @param _wl new address of whitelist contract
    ...
    /**
     * @dev Checks staked tokens, if ok, save staked amount for allocation calcs
     * @notice allowed only at APPROVE stage.
     * @param saleID id of sales list
     * @param _msgForSign hashd data with prefix for sign
     * @param _signature signed data
     */

    function registerStake(uint256 saleID, bytes32 _msgForSign, bytes memory _signature) external {
        require(getSalestage(saleID) == stages.APPROVE, "Sale::registerStake: only APPROVE stage");
        // Get current stake
        (uint256 curStake, ) = sales[saleID].staking.userStakes(msg.sender);
        
        require(curStake > 0, "Sale::registerStake: have no stake");
        require(curStake >= minStakeForRegister, "Sale::registerStake: too little stake for register");
        
        // Get current allocation
        uint256 curAllocation = saleAllocations[saleID][msg.sender].staked;
        require(curStake > curAllocation, "Sale::registerStake: add stake");

        if (whiteList != address(0)) {
            require(IWhiteList(whiteList).enabledForCampaign(msg.sender, saleID, _msgForSign, _signature), 
                "Sale::registerStake:only whitelisted users");
        }
        ...
        require(sales[saleID].participants[0] >= sales[saleID].participants[1], 
            "Sale::registerStake: participants exceeded");
        ...
        if (sales[saleID].maxUserStakeAmount > 0
            && curAllocation + (curStake - curAllocation) >= sales[saleID].maxUserStakeAmount) {
            saleAllocations[saleID][msg.sender].staked = sales[saleID].maxUserStakeAmount;
            staked = sales[saleID].maxUserStakeAmount - curAllocation;
        }
        else {
            saleAllocations[saleID][msg.sender].staked += curStake - curAllocation;
            staked = curStake - curAllocation;    
        }
        // increase total allocations
        sales[saleID].stakeAllocationAmount += staked;
        emit SaleAllocAdded(msg.sender, saleID, staked);
    }

    /**
     * @dev register token sale, accumulated at salePayments[saleID][msg.sender] respects stake limits
     * @notice can call multiple times
     * @param saleID id of sales
     * @param paymentAmount amount of payment token to get from user
     */

    function registerSale(uint256 saleID, uint256 paymentAmount) external {
        require(getSalestage(saleID) == stages.SALE, "Sale::registerSale: only SALE stage");
        // slither-disable-next-line divide-before-multiply
        require(
            salePayments[saleID][msg.sender] + paymentAmount <=
                saleAllocations[saleID][msg.sender].staked * 1e9 / sales[saleID].stakeAllocationAmount
                    * sales[saleID].tokenAmount
                    * sales[saleID].tokenPrice
                    / 1e13, // 10**4 * 10**9  - 2021-12-09 
            "Sale::registerSale: payment exceeds"
        );

        salePayments[saleID][msg.sender] = salePayments[saleID][msg.sender] + paymentAmount;
        sales[saleID].receivedPayments = sales[saleID].receivedPayments + paymentAmount;
        sales[saleID].soldTokens = sales[saleID].soldTokens + paymentAmount * 1e4 / sales[saleID].tokenPrice; //1e4
        sales[saleID].paymentToken.safeTransferFrom(msg.sender, address(this), paymentAmount);
    }
    ...
    {
        // slither-disable-next-line divide-before-multiply
        balanceRest = saleAllocations[saleID][userID].staked * 1e9 / sales[saleID].stakeAllocationAmount;
        payments = salePayments[saleID][userID];
        // slither-disable-next-line divide-before-multiply
        return (
            balanceRest * sales[saleID].tokenAmount * sales[saleID].tokenPrice / 1e13   //1e13
    ...
    function claim(uint256 saleID) external {
        require(getSalestage(saleID) == stages.CLAIM && !saleAllocations[saleID][msg.sender].claimed
            , "Sale::claim: only CLAIM stage or claimed");
        (uint256 purchasedTokens, , ) = viewSale(saleID, msg.sender);
        if (purchasedTokens > 0) {
            saleAllocations[saleID][msg.sender].claimed = true;
            sales[saleID].token.safeTransfer(msg.sender, purchasedTokens);
        }
    }

    /**
     * @dev grantSaleRole runs accesscontrol grantRole for create sales only by owner
     * @param salesman address of saller
     */

    function grantSaleRole(address salesman) external onlyOwner {
    ...
    function revokeSaleRole(address salesman) external onlyOwner {
        revokeRole(CREATE_SALE_ROLE, salesman);
    }

    /**
     * @dev transfer ownership with all roles to new owner
     * @param newOwner address of new owner
     */

    function transferOwnership(address newOwner) public override onlyOwner {
        grantRole(CREATE_SALE_ROLE, newOwner);
        grantRole(DEFAULT_ADMIN_ROLE, newOwner);
        revokeRole(CREATE_SALE_ROLE, msg.sender);
    ...
    /**
     * @dev revokeRole runs accesscontrol revokeRole for create sales only by owner
     * @return saleAdmins list of users with sale role
     */

    function getSaleRoleList() external view onlyOwner returns (address[] memory saleAdmins) {
        ...
        )
    {
        if (getSalestage(saleID) == stages.SALE || getSalestage(saleID) == stages.CLAIM) {
            return (
                salePayments[saleID][wallet] * 1e4 / sales[saleID].tokenPrice,  //1e4
    ...
    /**
      ...
      * @return sale owner
      * @return address of sale token
      * @return name of token to be sold
      * @return address of payment token(dai,usdt,etc)
      * @return address of staking contract
      * @return dates params of sale
    ...
    /**
      * @param saleID id of sale
      * @return limit of stake amount per user
      * @return amout af allocated tokens
      * @return amount of received payment tokens
      * @return payment tokens of msg.sender in this sale
      */
    ...
    function getSaleAllocations(uint saleID, address user) external view returns(uint) {
