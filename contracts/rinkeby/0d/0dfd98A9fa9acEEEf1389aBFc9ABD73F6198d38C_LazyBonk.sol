...
contract LazyBonk is ERC721Enumerable, Ownable, AccessControl {
    ...
    address def = 0x0000000000000000000000000000000000000000;
    ...
    modifier whenMintNotPaused() {
        require(!paused_mint, "LazyBonesSpaceTrip: mint is paused");
        _;
    }

    modifier whenPreMintNotPaused() {
        require(!paused_pre_mint, "LazyBonesSpaceTrip: pre mint is paused");
        _;
    }

    modifier preMintAllowedAccount(address account) {
        require(is_pre_mint_allowed(account), "LazyBonesSpaceTrip: account is not allowed to pre mint");
        _;
    }
  ...
  constructor() ERC721("LazyBonk", "LB") {
    setBaseURI(_currentBaseUrl);
     ...
     _setupRole(WHITE_LIST_ROLE, def);
  }
