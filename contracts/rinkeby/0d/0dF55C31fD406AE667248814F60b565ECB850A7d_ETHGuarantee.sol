...
abstract contract Guarantee is Pausable {
    ...
    function register(uint256 _guarantorIndex, address _demandA, address _demandB, uint256 _fund, uint32 _feePercentLimit, string calldata _content) external payable whenNotPaused {
        require(_guarantorIndex < guarantor.guarantorIndex(), "");
        require(guarantor.guarantorStates(_guarantorIndex) == Guarantor.GuarantorState.notLocked, "");
        require(_feePercentLimit > 0 && _feePercentLimit < 100, "");
    ...
    function deposit(uint256 _guaranteeIndex, uint256 _amount) external payable whenNotPaused {
        require(_guaranteeIndex < guaranteeIndex, "");
    ...
    function withdraw(
        ...
        require(_guaranteeIndex < guaranteeIndex, "");
        require(guarantees[_guaranteeIndex].guarantorIndex == _guarantorIndex, "");

        require(_guarantorIndex < guarantor.guarantorIndex(), "");
        require(guarantor.guarantors(_guarantorIndex) == msg.sender, "");
        require(guarantor.guarantorStates(_guarantorIndex) == Guarantor.GuarantorState.notLocked, "");

        require(_amountToA + _amountToB <= guarantees[_guaranteeIndex].fund);
        require(_feePercent <= guarantees[_guaranteeIndex].feePercentLimit, "");

        uint256 feeA = _amountToA  * uint256(_feePercent) / 100;
        uint256 feeB = _amountToB  * uint256(_feePercent) / 100;

        _processWithdraw(guarantees[_guaranteeIndex].demandA, _amountToA);
        _processWithdraw(guarantees[_guaranteeIndex].demandB, _amountToB);
