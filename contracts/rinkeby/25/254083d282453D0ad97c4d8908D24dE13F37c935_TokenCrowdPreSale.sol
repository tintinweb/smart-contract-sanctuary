...
contract TokenCrowdPreSale is Ownable {
    ...
    bool public enable90 = false; //false by default
    ...
    function Enable90() onlyOwner public {
        enable90 = true;
    } 

    function Disable90() onlyOwner public {
        enable90 = false;
    } 
    ...
    function sellingState() public view returns (string) {
        if(saleState == 0) {
            return "Sale not started yet";
        } else if(saleState == 1) {
            return "Sale is now open";
            //startTime = now;
        } else {
    ...
    function claim10Pct() claimable public {       
        withdrawal10(msg.sender);
    }

    function claim90Pct() claimable public {
        require(enable90 == true);
        withdrawal90(msg.sender);
    }

    function withdrawal10(address funder) internal {
        require(fundersProperty[funder].reservedToken > 0);    

        tokenReward.transfer(funder, fundersProperty[funder].reservedToken*10/100);
    ...
    function withdrawal90(address funder) internal {
        require(fundersProperty[funder].reservedToken > 0);  

        tokenReward.transfer(funder, fundersProperty[funder].reservedToken*90/100);
