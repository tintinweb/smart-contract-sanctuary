...
contract Stakable   is Ownable
{
    ...
    event       NftStakeInitiated(string collectionName, address stakerWallet, uint since, uint periodGain, uint periodDurationInSec);
    ...
    constructor() 
    {
        ...
        setStakableNftCollection("DOOLNIES", 0xf9BCDC243a2A3aD1577251D96721e908198F3f4B, 86400, 10000000000000000000,13000000000000000000,1731801600,1701801600);
...
/*
    ...
    mapping(uint256 => bool)                        internal nftTokenHashes;
    mapping(uint256 => address)                     internal nftTokenAddresses;
*/
    TStaker[]                                       public stakers;

    mapping(address => uint256)                     public stakerIndexes;
    mapping(string  => TStakableNftCollectionInfo)  public stakableNftCollections;
    ...
    function    setStakableNftCollection(string memory  collectionName, 
                                         ...
                                         uint256        halvingTimestamp)       public onlyOwner
    {
    ...
    {
        return nftStakeEx(collectionName, tokenId, true, userWallet, periodDurationInSec, periodGain);
    }
    //----------------------------------------------------------------------
    function    doPezNftStake(string memory collectionName, uint256 tokenId, address userWallet) internal returns(bool)  
    {
        ...
        return nftStakeEx(collectionName, tokenId, false, userWallet, 0,0);
    }/*
    //----------------------------------------------------------------------
...
*/
    ...
    function    pezNftStake(string memory collectionName, uint256 tokenId, address userWallet) external returns(TPNS memory)  
    {
        if (nftStakeEx(collectionName, tokenId, false, userWallet, 0,0))
        {
            emit D2(1);
        }
        else emit D2(0);
                           ...
                           uint256 imposedPeriodDurationInSec, uint256 imposedPeriodGain)   internal returns(bool)
    {
        ...
        /////require(staker.isStaking==false, "NFT is already staked");
        emit C4(staker.isStaking ? 1:0);
        ...
        emit NftStakeInitiated(collectionName, userWallet, staker.since, staker.periodGain, staker.periodDurationInSec);
   ...
   //----------------------------------------------------------------------
                           ...
                           uint256 imposedPeriodDurationInSec, uint256 imposedPeriodGain)   internal returns(bool)
    {
        ...
        emit NftStakeInitiated(collectionName, userWallet, staker);
