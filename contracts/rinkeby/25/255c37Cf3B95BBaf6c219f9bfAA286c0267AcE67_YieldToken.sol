...
contract YieldToken is Initializable, ERC20CappedUpgradeable {
    ...
    IERC721Upgradeable public DJS_OF_DEFI;

    // Tue Mar 18 2031 17:46:47 GMT+0000
    uint256 public constant START = 1931622407;
    ...
    modifier onlyDoD() {
        require(
            msg.sender == address(DJS_OF_DEFI),
            "YieldToken: Only DJsofDeFi can call this"
        );
        _;
    }
    ...
    function initialize(address _uninterestedUnicorns, uint256 maxSupply) external initializer {
        ...
        DJS_OF_DEFI = IERC721Upgradeable(_uninterestedUnicorns);
    }
    ...
    function updateRewardOnMint(address _user, uint256 _amount) external onlyDoD {
    ...
    // called on transfers
    function updateReward(address _from, address _to) external onlyDoD {
        uint256 time = max(block.timestamp, START);
        uint256 timerFrom = lastUpdate[_from];
        if (timerFrom > 0) {
            rewards[_from] += DJS_OF_DEFI
                .balanceOf(_from)
    ...
    function getReward(address _to) external onlyDoD {
    ...
    function burn(address _from, uint256 _amount) external onlyDoD {
        _burn(_from, _amount);
    }

    function getTotalClaimable(address _user) external view returns (uint256) {
        uint256 time = max(block.timestamp, START);
        uint256 pending = DJS_OF_DEFI
            .balanceOf(_user)
