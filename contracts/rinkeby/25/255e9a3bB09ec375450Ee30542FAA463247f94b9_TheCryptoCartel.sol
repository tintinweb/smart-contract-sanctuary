...
contract TheCryptoCartel is ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Pausable, AccessControl {
    ...
    bytes32 public constant WHITELIST_ROLE = 0xdc72ed553f2544c34465af23b847953efeb813428162d767f9ba5f4013be6760; // keccak256("WHITELIST_ROLE");
    string public BASE_URI = "https://gateway.pinata.cloud/ipfs/Qmc3fmS89i6t5r3WyzvSzDMBKcGmFjZwKg8CL6nMTc4yi6";
    string public constant DEFAULT_METADATA = "/def.json";
    ...
    uint8 MINT_LIMIT = 6;
    uint8 WHITELIST_TRANSACTION_LIMIT = 5;

    uint16 SUPPLY_LIMIT = 8000;
    uint16 WHITELIST_LIMIT = 1000;

    uint256 public constant PRICE_PER_TOKEN_PRE_SALE = 0.069 ether;
    uint256 public constant PRICE_PER_TOKEN = 0.08 ether;

    uint256 public VAL = 0.0;

    mapping(address => uint8) private wlTransactionLimitMapping;
    ...
    function addToWhitelist(address[] memory addresses) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_whitelistCounter.current() + addresses.length < WHITELIST_LIMIT, "Whitelist is full. Wait till Cartel resumes minting!");
        require(whitelistEnabled == true, "Whitelisting is disabled by the Cartels!");
    ...
    function setSaleStatus(bool _isSaleActive) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function mintForWhitelistedMember(uint256 numberOfTokens) external payable onlyRole(WHITELIST_ROLE) {
        uint256 ts = totalSupply();
        
        require(ts + numberOfTokens <= SUPPLY_LIMIT, "Reached max limit. No more minting possible!");
        require(whitelistEnabled, "Whitelist sale is not active!");
