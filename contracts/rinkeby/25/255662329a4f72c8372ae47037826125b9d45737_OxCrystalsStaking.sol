...
contract OxCrystalsStaking is ERC20Burnable, Ownable {
    ...
    mapping(address => bool) private IS_CLAIMABLE; // set nft contract address which can be staked
    mapping(address => uint256) private INITIAL_REWARD; //set one time claimable reward for nft
    mapping(address => uint256) private MAX_EMISSION_TIME; //set for how much time nft will yeild reward
    mapping(address => uint256) private CLAIMING_START_TIME; //set from what point to start claim proccess
