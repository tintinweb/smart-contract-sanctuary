...
// Created By: Lorenzo
contract DuckyET is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function mintDuckyET(uint256 numberOfDucky) external payable nonReentrant {
        require(
            saleActive ||
            (whitelist[msg.sender] && numberOfDucky <= whitelistMaxMint && allowWhitelistRedemption),
            "You may not mint a spooky boy yet. Stay tuned!");
        require(numberOfDucky > 0, "You cannot mint 0  Spooky Boys.");
        require(SafeMath.add(_numMinted.current(), numberOfDucky) <= MAX_PUBLIC_MINT, "Exceeds maximum supply.");
        require(numberOfDucky <= MAX_DUCKY_ET_PURCHASE, "Exceeds maximum DuckyET in one transaction.");
    ...
    function airdropDuckyET(address[] calldata to, uint256[] calldata numberOfDucky)
    external
    onlyOwner
    {
