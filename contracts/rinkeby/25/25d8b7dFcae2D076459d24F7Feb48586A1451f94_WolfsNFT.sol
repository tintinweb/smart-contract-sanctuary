    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(uint256 => Wolf) public WolfProperties;
    ...
    address CWOLF;
    address wolfsNFTHelperAddress;
    event MintNFT(address indexed _to, uint256 indexed _id);

    constructor() initializer {}

    function initialize(address wolfsNFTHelperAddress_, address owner_)
        public
        initializer
    {
        __ERC721_init("WolfNFT", "WolfNFT");
        __Ownable_init();
        wolfsNFTHelperAddress = wolfsNFTHelperAddress_;
    ...
    function mint(address _to) internal returns (uint256) {
        ...
        WolfsNFTHelper wolfNFTHelper = WolfsNFTHelper(wolfsNFTHelperAddress);
        ...
        (
            ...
            uint256 attack,
            uint256 defense
        ) = wolfNFTHelper.generateWolf(tokenId);
        ...
        Wolf memory wolfGenerated;

        wolfGenerated.breed;
        wolfGenerated.gender;
        wolfGenerated.level;
        wolfGenerated.attack;
        wolfGenerated.defense;

        WolfProperties[tokenId] = wolfGenerated;
    ...
    function updateAttackDefensePercentages() external returns (bool) {
        return true;
    }

    function updateAttackDefenseLevels() external returns (bool) {
        return true;
    }
}
...
contract WolfsNFTHelper is Initializable, OwnableUpgradeable {
    ...
    function generateBreed(uint256 index) internal view returns (Breed) {
        uint256 randomNumber = Random.randomMinMax(index, 1, 100);
        ...
        if (randomNumber <= breedPercentages[0]) {
            ...
            return Breed.ELECTRIC;
        } else if (
            randomNumber > sumPercentages(7, breedPercentages) &&
            randomNumber <= sumPercentages(8, breedPercentages)
        ) {
            return Breed.LEGENDARY;
        }
        return Breed.LAND;
    }

    function generateGender(uint256 index) internal view returns (Gender) {
        uint256 randomNumber = Random.randomMinMax(index, 0, 1);
    ...
    function generateLevels(uint256 index)
        internal
        view
        returns (
            uint256,
    ...
    function generateWolf(uint256 index)
        external
        returns (
            uint256 breed,
            uint256 gender,
    ...
    {
        // TODO: Requerir address del contrato ???

        Breed breed = generateBreed(index);
        Gender gender = generateGender(index);
        (uint256 attack, uint256 defense, Level level) = generateLevels(index);
...
library Random {
    function randomMinMax(uint id, uint256 _min, uint256 _max)
        internal
        view
        returns (uint256)
    {
