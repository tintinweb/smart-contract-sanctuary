...
contract ToodleLootBag is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Toodle Loot Bag #', toString(tokenId), '", "description": "Toodle Loot Bag is an NFT collection that can directly interact with the Cryptoodle universe. There are only 10K Toodle Loot Bags available, one for each Toodle Leader to use for leading a civilization.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
