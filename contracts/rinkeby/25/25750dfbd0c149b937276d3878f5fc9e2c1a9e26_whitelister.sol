...
contract whitelister {
    ...
    function setWhitelists() external returns(bool){
        require(msg.sender == owner, 'you are not owner');
        for(uint _i=0; _i<member.length;_i++){
            (bool _isApproved,) = up.userInfo(1,member[_i]);
            if(_isApproved == true){
                IUpgradedPS(sc).setWhitelist(4, member[_i], true);
            }
        }
        return true;
    }
