...
contract Kinochromes is ERC721, ReentrancyGuard, Ownable {
    ...
    function buildShape(uint256 tokenId) internal view returns (string memory) {
      string[2] memory shapes = [
        ...
        // '<rect id="r" height="32" width="32"><animateTransform attributeName="transform" type="rotate" from="0 16 16" to="360 16 16" dur="10s" repeatCount="indefinite"/></rect>',
        ...
        return string(abi.encodePacked(
          ...
          Base64.encode(
            bytes(
              string(
                abi.encodePacked(
                  '{"name": "', (tokenId == 0x1FF || tokenId == 0x32) ? 'Specialfast #' : 'Colourfast #',
                  toString(tokenId),
