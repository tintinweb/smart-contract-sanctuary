...
contract  MINDNFT  is ERC721Enumerable, Ownable
{
    ...
    uint256 public priceFirst = 0.30 ether;
    uint256 public secondPrice = 0.40 ether;
    ...
    constructor(string memory baseURI,address nft) ERC721("MIND NFT", "MNFT")  {
        setBaseURI(baseURI);
        erc=IERC721(nft);
    }
    ...
    function preSalemint(uint chosenAmount) public payable isSaleOpen{
        ...
        require(erc.balanceOf(msg.sender)>0,"user dont have music token");
        ...
        if(totalsupply()+chosenAmount<=500)
        {
        require(priceFirst.mul(chosenAmount) == msg.value, "Sent ether value is incorrect");
        }
        else
        {
