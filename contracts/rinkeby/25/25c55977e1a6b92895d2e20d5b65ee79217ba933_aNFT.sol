 ...
 */
contract aNFT is Context, AccessControlEnumerable, ERC721Enumerable, ERC721Burnable, ERC721Pausable {
     ...
     */
    function mint(address to, bytes32 nfcId) public virtual {
        require(hasRole(MINTER_ROLE, _msgSender()), "ERC721PresetMinterPauserAutoId: must have minter role to mint");

        // We cannot just use balanceOf to create the new tokenId because tokens
        // can be burned (destroyed), so we need a separate counter.
        _mint(to, nfcId, _tokenIdTracker.current());
        _tokenIdTracker.increment();
    }
    
    function _mint(address to, bytes32 nfcId, uint256 tokenId) internal {
        require(nfcIdToTokenId[nfcId] == 0, "Unable to mint Token with duplicate NFC ID");
