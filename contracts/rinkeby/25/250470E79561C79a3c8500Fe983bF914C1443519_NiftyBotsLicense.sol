...
contract NiftyBotsLicense is ERC721, Ownable, ERC721Burnable, ERC721Enumerable {
    ...
    uint256 public seasonDuration = 7890000 seconds; // roughly 3 months
    ...
    constructor() ERC721("NiftyBots License", "NBL") {
        ...
        tierConfigs[4] = TierConfig(Tier.FOREVER, 3 ether, FOREVER_LICENSE_DURATION);
    }
     ...
     */
    function toggleFreeFroAll() external onlyOwner {
    ...
    function setDiscounts(address[] calldata addresses, uint8[] calldata discounts) public onlyOwner {
        ...
        for (uint256 i; i < addresses.length; i++) {
            require(addresses[i] != address(0), "Can't add the null address");
            require(discounts[i] > 0 && discounts[i] < 101, "Discount must be in range 1..100");
            discountList[addresses[i]] = discounts[i];
        }
    }

    function removeDiscounts(address[] calldata addresses) external onlyOwner {
        require(addresses.length < 11, "Can only update 10 at a time.");
        for (uint256 i; i < addresses.length; i++) {
            require(addresses[i] != address(0), "Can't remove the null address");
    ...
    function mintLicense(uint256 tierId) external payable {
