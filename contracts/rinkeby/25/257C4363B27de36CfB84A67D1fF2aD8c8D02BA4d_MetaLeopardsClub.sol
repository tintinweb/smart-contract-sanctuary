...
contract MetaLeopardsClub is ERC721, ERC721Enumerable, Pausable, AccessControl {
    ...
    function premiumWhiteListMint(uint8 _mintAmount) public payable onlyRole(PREMIUM_WHITE_LISTED) {
        uint256 supply = totalSupply();
        require(!paused(), "Pausable: paused");
        require(premiumWhiteListCanMint == true);
        require(_mintAmount > 0);
        require(supply + _mintAmount <= maxSupply);
    ...
    // only project owner
    function setPremiumWhiteListCost(uint256 _newPremiumWhiteListCost) public onlyRole(PROJECT_OWNER) {
