...
contract Yammies is ERC721, Ownable {
    ...
    uint256 public presaleAmount = 200;

    // Will be set when sale is activated
    ...
    function reserveYammies(address _to, uint256 _reserveCount) public onlyOwner {        
        uint256 supply = totalSupply();
        require(_reserveCount > 0 && _reserveCount <= reservedAmount, "Not enough reserves left");
        for (uint256 i = 0; i < _reserveCount; i++) {
            _safeMint(_to, supply + i);
        }
        reservedAmount = reservedAmount.sub(_reserveCount);
    }
    ...
    function mintYammies(uint256 _count) public payable {
        ...
        require(_count > 0 && _count < maxPerMint + 1, "Exceeds maximum Yammies you can purchase in a single transaction");
        require(totalSupply + _count < maxSupply + 1, "Exceeds maximum Yammies available for purchase");
        require(msg.value >= price.mul(_count), "Ether value sent is not correct");

        if(totalSupply < presaleAmount){
