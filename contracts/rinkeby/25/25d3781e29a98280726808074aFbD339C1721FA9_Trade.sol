...
contract Trade {
    ...
    function buyAsset(Order memory order) public payable returns(bool) {
        Fee memory fee;
        if(order.directPay) {
            fee = getFees(msg.value, order.nftType, order.nftAddress, order.tokenId);
        } else {
            fee = getFees(order.amount, order.nftType, order.nftAddress, order.tokenId);
        }
