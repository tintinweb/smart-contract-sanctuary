...
contract OUSD is ERC20, Ownable {
    ...
    function _takeInterest (address depositor) internal {
        userInfo[depositor].interest += currentInterest(depositor);
    ...
    function currentInterest(address depositor) public view returns (uint256) {
        uint256 blocksPassed = block.number - userInfo[depositor].lastBlock;
        return userInfo[depositor].borrowed * terms.IR * blocksPassed / INTEREST_DECIMALS;
    }
    ...
    // 18 decimal value of collateral
    function collateralValue (address depositor) public view returns (uint256) {
        return gOHM.balanceFrom(userInfo[depositor].collateral) * oracle.assetPrice() * 10;
    }

    // 5 decimal collateral utilization
    function currentUtilization (address depositor) external view returns (uint256) {
        return (userInfo[depositor].borrowed + userInfo[depositor].interest) * 1e5 / collateralValue(depositor);
    }
