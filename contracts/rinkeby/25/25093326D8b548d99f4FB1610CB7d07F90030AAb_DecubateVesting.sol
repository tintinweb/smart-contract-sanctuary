...
//** Author Alex Hong & Aaron Fitzpatrick : Decubate Vesting Contract 2021.6 */
...
contract DecubateVesting is IDecubateVesting, Ownable, ReentrancyGuard {
    ...
    function claimInitialUnlock(address _wallet) external override onlyOwner returns (bool) {
        ...
        bool success = _decubateToken.transfer(
            vipPools[_wallet].wallet,
            unlockAmount
        );
        
        require(success, "Unsuccesful transaction");
    ...
    function revoke(uint256 _option, address _wallet) public onlyOwner {
        ...
        uint256 balance = vestingPools[_option].whitelistPool[_wallet].dcbAmount;

        uint256 unreleased = calculateReleasableAmount(_option, _wallet);
        uint256 refund = balance.sub(unreleased);

        vestingPools[_option].whitelistPool[_wallet].revoke = true;

        _decubateToken.safeTransfer(address(this), refund);
