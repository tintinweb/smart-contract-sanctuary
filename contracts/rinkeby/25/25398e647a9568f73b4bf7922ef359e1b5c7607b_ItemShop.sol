...
contract ItemShop is OwnableUpgradeable {
  ...
  event ValidTokenAdd(address indexed token);
  event ValidTokenRemove(address indexed token);
  ...
  function payInvoice(
    ...
    require(validTokens[_token], "payInvoice: invalid token");
    require(_amount > 0, "payInvoice: invalid amount");

    IERC20(_token).safeTransferFrom(msg.sender, treasury, _amount);
    payments[_invoiceId][_token] += _amount;
    emit Payment(_invoiceId, _userId, _token, _amount);
  }

  function addValidToken(address _token) public onlyOwner {
    validTokens[_token] = true;
    emit ValidTokenAdd(_token);
  }

  function removeValidToken(address _token) public onlyOwner {
    validTokens[_token] = false;
    emit ValidTokenRemove(_token);
  }
