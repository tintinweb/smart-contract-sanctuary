...
// Staking is timelocked 60 days; removal prior to end of timelock = 60 - days staked reduction as a percent
// abandoned LIFT is migrated to IdeaFund
// abandoned CTRL is migrated to IdeaFund
...
contract Boardroom is ShareWrapper, ContractGuard, Operator {
    ...
    struct StakingSeatControl {        
        uint256 lastSnapshotIndex;
        uint256 rewardEarned; 
        //uint256[2][] stakingWhenQuatity; - dont currently care when people take out control shares
        bool isEntity;
    }
    ...
    //TODO the website should be able to iterate on this function and display all of the 
    ...
    function getStakedAmountsShare() public view returns (uint256[2][] memory stakedS) {
         ...
         for (uint256 i = 0; i < seatS.stakingWhenQuatity.length; i++) {
             ...
             if(arrStaked[1] == stakedTimeStamp) {
                ...
                if (daysStaked >= daysRequiredStaked){
                    ...
                    // console.log('arrStaked0:', arrStaked[0]);
                    // console.log('totalSupply:', gettotalSupplyShare());
                    settotalSupplyShare(gettotalSupplyShare().sub(arrStaked[0])); 
                    setbalanceOfShare(msg.sender, getbalanceOfShare(msg.sender).sub(arrStaked[0]));
                    IERC20(share).safeTransfer(msg.sender, reducedAmount);
                    IERC20(share).safeTransfer(address(ideaFund), arrStaked[0].sub(reducedAmount));
        ...
        onlyOperator
    {
        ...
        require(
            gettotalSupplyShare() > 0 || gettotalSupplyControl() > 0,
...
//console.log("Share Supply: ", gettotalSupplyShare());
//console.log("Control Supply: ", gettotalSupplyControl());

//console.log("Share Price: ", IOracle(theOracle).priceOf(share));
//console.log("Control Price: ", IOracle(theOracle).priceOf(control));
...
//console.log("sValue:", shareValue);
//console.log("cValue:", controlValue);
//console.log("totalStakeValue:", totalStakedValue);
...
//console.log("rewardPer Share Value: ", rewardPerShareValue);
//console.log("rewardPer ControL Value: ", rewardPerControlValue);
...
//console.log("rewardshareperstake", rewardPerShareValue.mul(1e18).div(gettotalSupplyShare()));

            uint256 nextRPS = prevRPS.add(rewardPerShareValue.mul(1e18).div(gettotalSupplyShare()));
//console.log("PrevPRS:", prevRPS);
//console.log("nextRPS:", nextRPS);
...
//console.log("rewardcontrolperstake", rewardPerControlValue.mul(1e18).div(gettotalSupplyControl()));

            uint256 nextRPC = prevRPC.add(rewardPerControlValue.mul(1e18).div(gettotalSupplyControl()));

//console.log("PrevPRS:", prevRPC);
//console.log("nextRPS:", nextRPC);
