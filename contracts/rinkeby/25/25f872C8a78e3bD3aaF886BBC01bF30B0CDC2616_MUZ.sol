...
contract MUZ is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint256 public price = 0.03 ether; // Price per MUZ
  uint256 public discountprice = 0.02 ether; // Discount Pre-sale Price
  uint256 public maxSupply = 3000; // Total MUZ Suplly
  uint256 public maxMintAmount = 15; // Max mint per transaction
  uint256 public nftPerAddressLimit = 50; // MUZ limit for address 
  ...
  function whitelistMint(uint256 _mintAmount) public payable {  // public check etherscan
    require(!paused, "Sale not active!");
    uint256 supply = totalSupply();
    require(_mintAmount > 0, " Min 1 MUZ");
    require(_mintAmount <= maxMintAmount, "max mint amount per session exceeded");
    require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");

    if (msg.sender != owner()) {
        uint256 ownerMintedCount = addressMintedBalance[msg.sender];
        require(ownerMintedCount + _mintAmount <= nftPerAddressLimit, "max MUZ per address exceeded");
  ...
  function setdiscountPrice(uint256 _newdiscountPrice) public onlyOwner {
    discountprice = _newdiscountPrice;
  }
  ...
  address t1 = 0xD7C358434D82046616E9A15045F6e36583eA6069; //MasterMUZ
  address t2 = 0x878975Cf4a97774af6875B3B37b0920b83121F35; //DaddyMUZ
