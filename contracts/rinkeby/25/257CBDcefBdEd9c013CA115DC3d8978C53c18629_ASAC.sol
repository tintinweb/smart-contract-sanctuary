...
contract ASAC is Ownable, ERC721 {
    ...
    function minterMaxQtyToMint(address _minter) external view virtual returns (uint256) {
        // free
        if (mintedQty < FREE_MINT_MAX_QTY) {
            return maxFreeQtyPerWallet - minterToTokenQty[_minter];
        }
        // paid
        else {
            return MAX_QTY_PER_WALLET - minterToTokenQty[_minter];
        }
    }
