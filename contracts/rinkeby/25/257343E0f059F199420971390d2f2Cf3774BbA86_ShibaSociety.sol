    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (tokenId < 5000) {
            return bytes(baseURI1_).length > 0 && totalSupply >= MAX_NFT ? string(abi.encodePacked(baseURI1_, 
                tokenId.toString())) : string(abi.encodePacked(
                    "ipfs://QmX3biYCx4WJZk1jyRj6m6NmbT2HH9xYJBawLTn1Uos5hP/", tokenId.toString()));
        } else {
            return bytes(baseURI2_).length > 0 && totalSupply >= MAX_NFT ? string(abi.encodePacked(baseURI2_, 
                tokenId.toString())) : string(abi.encodePacked(
                    "ipfs://QmZGx59dePiUnHHUj637GXxcGtrGFHAUdR4tnzE8YBYZET/", tokenId.toString()));
        }
    }
