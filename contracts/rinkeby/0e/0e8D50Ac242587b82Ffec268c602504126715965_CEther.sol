...
contract CEther is CToken {
    ...
    function repayBorrowMax() external payable {       
        uint err = repayBorrowMaxBehalfInternal(msg.sender);
        requireNoError(err, "REPAY_BORROW_MAX_FAILED");
    }

    function repayBorrowBehalfMax(address borrower) external payable {       
        uint err = repayBorrowMaxBehalfInternal(borrower);
        requireNoError(err, "REPAY_BORROW_BEHALF_MAX_FAILED");
    }

    function repayBorrowMaxBehalfInternal(address borrower) internal returns (uint) {
        require(accrueInterest() == uint(Error.NO_ERROR), "ACCRUE_INTEREST_FAILED");
        uint borrows = borrowBalanceStored(borrower);   

        uint err;                
        uint received = msg.value;
        if (received > borrows) {
            (err,) = repayBorrowBehalfInternal(borrower, borrows);
            msg.sender.transfer(received - borrows);
        } else {
            (err,) = repayBorrowBehalfInternal(borrower, received);
        }
        return err;
    }
