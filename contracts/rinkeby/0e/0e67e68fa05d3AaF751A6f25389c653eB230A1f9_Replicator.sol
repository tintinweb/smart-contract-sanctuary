...
contract Replicator is ReentrancyGuard, ERC721, Ownable, ReplicatorInterface {
     ...
     */
    bytes4 private constant _InterfaceId_ReplicatorInterface = 0xf33c31c8;
    ...
    // Base URI
    string private constant _baseURI = "https://api.maddogjones.com/replicator/";
    ...
    constructor (string memory _name, string memory _symbol, uint256 firstPrintTime, uint256 printInterval, uint8 maxGenerations, uint8[] memory generationPrintLimits, uint8[] memory generationJamVariants, uint8[] memory generationJamProbabilities) public ERC721(_name, _symbol)
    {
        ...
        _registerInterface(_InterfaceId_ReplicatorInterface);
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked(_baseURI, uint256(_tokenGeneration[tokenId]).toString(), "/", uint256(_tokenVariant[tokenId]).toString(), "/", tokenId.toString()));
    }
    ...
    function canPrint(uint256 tokenId) public view override returns (bool) {
        ...
        return (_tokenPrintsRemaining[tokenId] > 0 && _tokenNextPrintTime[tokenId] > 0 &&
               now >= _tokenNextPrintTime[tokenId] && _tokenGeneration[tokenId] + 1 <= _maxGenerations);
    }
...
interface ReplicatorInterface is IERC721 {
