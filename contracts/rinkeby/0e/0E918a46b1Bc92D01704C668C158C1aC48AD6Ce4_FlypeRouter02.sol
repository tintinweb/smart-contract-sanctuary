...
contract FlypeRouter02 is FlypeLibrary {
    ...
    constructor(address _factory, address _WETH) FlypeLibrary(_factory, _WETH){
        factory = _factory;
        WETH = _WETH;
    }
...
contract FlypeLibrary is Ownable {
    ...
    function getPrices(address tokenA, address tokenB) public view returns (uint priceA, uint priceB) {
        if(tokenA == _weth){
            priceA = IFlypeUPM(flypeUPM).getPriceETH();
            priceB = IFlypeUPM(flypeUPM).getPrice(pairFor(tokenA, tokenB));
        }
        else {
            priceA = IFlypeUPM(flypeUPM).getPrice(pairFor(tokenA, tokenB));
            priceB = IFlypeUPM(flypeUPM).getPriceETH();       
        }
    }
