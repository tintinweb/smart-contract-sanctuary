...
interface ILock {
    ...
    function calculate_points(uint256 _amount, uint256 _time)
        external
        view
        returns (uint256);
...
contract BasisLockPoint is ERC20Burnable, ILock, Operator {
    ...
    function calculate_points(uint256 _amount, uint256 _time)
        ...
        returns (uint256)
    {
        return _amount.mul(_time).mul(k);
    }
    ...
    function _getUserLock(address _pool, address _owner)
        internal
        view
    ...
    function _createLock(
        ...
        require(_amount <= storeBalance, 'BLP: lock amount too much');
        require(
            _end.sub(block.timestamp) <= MAX_LOCK_TIME,
            'BLP: cannt long than MAX_LOCK_TIME'
        );
        UserLock memory user = _getUserLock(_pool, _owner);
        require(_amount >= user.amount, 'BLP: cannt reduce amount');
        require(_end >= user.end, 'BLP: cannt reduce time');

        uint256 deltaAmount = _amount.sub(user.amount);
        uint256 deltaTime = _end.sub(user.end);
        uint256 pointAmount = calculate_points(deltaAmount, deltaTime);
        if (pointAmount > 0) {
