...
// File: contracts/libs/IPyroReferral.sol
...
// PyroToken with Governance.
contract PyroToken is BEP20 {
    ...
    // Min amount to liquify. (default 500 PYROs)
    uint256 public minAmountToLiquify = 500 ether;
    // The swap router, modifiable. Will be changed to PyroSwap's router when our own AMM release
    IUniswapV2Router02 public pyroSwapRouter;
    // The trading pair
    address public pyroSwapPair;
    // In swap and liquify
    ...
    event PyroSwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "PYRO::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the PyroToken contract.
     */
    constructor() public BEP20("PyroSwap Token", "PYRO") {
    ...
    /// @dev overrides transfer function to meet tokenomics of PYRO
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        // swap and liquify
        if (
            swapAndLiquifyEnabled == true
            && _inSwapAndLiquify == false
            && address(pyroSwapRouter) != address(0)
            && pyroSwapPair != address(0)
            && sender != pyroSwapPair
            && sender != owner()
        ) {
            swapAndLiquify();
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "PYRO::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "PYRO::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the pyroSwap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = pyroSwapRouter.WETH();

        _approve(address(this), address(pyroSwapRouter), tokenAmount);

        // make the swap
        pyroSwapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        pyroSwapRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from pyroSwapRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "PYRO::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "PYRO::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "PYRO::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updatePyroSwapRouter(address _router) public onlyOperator {
        pyroSwapRouter = IUniswapV2Router02(_router);
        pyroSwapPair = IUniswapV2Factory(pyroSwapRouter.factory()).getPair(address(this), pyroSwapRouter.WETH());
        require(pyroSwapPair != address(0), "PYRO::updatePyroSwapRouter: Invalid pair address.");
        emit PyroSwapRouterUpdated(msg.sender, address(pyroSwapRouter), pyroSwapPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "PYRO::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "PYRO::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "PYRO::delegateBySig: invalid nonce");
        require(now <= expiry, "PYRO::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "PYRO::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying PYROs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "PYRO::_writeCheckpoint: block number exceeds 32 bits");
