...
interface INFTTest {
    function genesis(address owner) external view returns(uint256);
}

contract NFTUT is ERC20, Ownable {
    ...
    constructor(address nftAddress) ERC20("NFT Utility Token", "NFTUT") {
        NFTTest = INFTTest(nftAddress);
    ...
    function getPendingReward(address user) internal view returns(uint256) {
        return NFTTest.genesis(user) * BASE_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START)) / 86400;
    }
