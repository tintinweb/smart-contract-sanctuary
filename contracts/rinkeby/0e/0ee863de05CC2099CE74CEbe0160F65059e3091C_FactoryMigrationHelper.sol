...
contract MigrationHelper {
	...
	function sendNewTokensToContract(uint256 _amount) public onlyOwner(){
		// Allow the contract
		newTokenAddress.approve(address(this), _amount.div(10**decimalsNewToken));

		// Transfer the tokens from msg.sender to the contract
		newTokenAddress.transferFrom(msg.sender, address(this), _amount.div(10**decimalsNewToken));
	}
    ...
    newTokenAddress.approve(address(this), newAmountToReceive);
    newTokenAddress.transferFrom(address(this), msg.sender, newAmountToReceive);

    received[msg.sender] = newAmountToReceive;
