...
contract OverHypedPandasSeries1 is ERC721Tradable, ReentrancyGuard {
  ...
  function getAvailableByTier(uint256 tier) public view returns(uint256 amount) {
    return _tierAmounts[tier] - _tierSales[tier];
  }

  function getTierOf(uint256 tokenId) public view returns(uint256 tier) {
  ...
  function getTotalTierSales(uint256 tier) public view onlyOwner returns(uint256 total) {
