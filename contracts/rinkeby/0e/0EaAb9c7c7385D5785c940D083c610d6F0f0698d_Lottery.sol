...
contract Lottery is VRFConsumerBase, Ownable{
    ...
    // there are on decimals in solidity so that is why conversion to gwei is needed
    // one gwei is 10**18 eth
    ...
    function endLottery() public onlyOwner{
        //not really random//uint(keccack256(abi.encodePacked(nonce, msg.sender, block.difficulty, block.timestamp))) % players.length;
        lotteryState = LOTTERY_STATE.CLACULATING_WINNER;
        bytes32 requestId = requestRandomness(keyhash, fee);
        emit RequestedRandomness(requestId);
    }

    function fulfillRandomness(bytes32 _requestId, uint256 _randomness) internal override {
        require(lotteryState == LOTTERY_STATE.CLACULATING_WINNER, "Lottery is not calculating winner");
