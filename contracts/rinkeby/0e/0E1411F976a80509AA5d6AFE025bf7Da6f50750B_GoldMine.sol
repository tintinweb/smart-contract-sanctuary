...
import {Miners} from "../Characters/Miners.sol";
import {Gold} from "../Resources/Gold.sol";
...
contract GoldMine {
    ...
    uint256 private _maxAmountPerDay = 10;
    uint256 private _difficultyFactor = 11;
    string private mineType="Gold";



    constructor(Miners minersAddress, Gold goldAddress){
        _miners = minersAddress;
        _gold = goldAddress;
    }

    function addMinerToMine(uint256 minerId) public{
        require(!_miners.isMinerMining(minerId), "Miner Already Mining!");
        require(keccak256(abi.encodePacked(mineType)) == keccak256(abi.encodePacked(_miners.getResourceType(minerId))),
            "Incompatible Miner Type");

        if (_miners.getMinerBalancesByMines(msg.sender, mineType).length>0) {
            claimGold();
        } else {
            _miners.setStartTimes(msg.sender);
        }
        _miners.addMinerToMine(msg.sender, mineType, minerId);

    }

    function removeMinerFromMine(uint256 minerId) public{
        require(_miners.isMinerMining(minerId), "Miner Not Mining!");

        _miners.removeMinerFromMine(msg.sender, mineType, minerId);

    }

    function claimGold() public{
        require(_miners.getMinerBalancesByMines(msg.sender, mineType).length>0, "No miners mining here!");

        _mintGold();
        _miners.setStartTimes(msg.sender);
    }

    function _mintGold() private {
        uint64 miningFactor = calculateMiningFactor();
        uint256 blocksPassed = block.timestamp-_miners.getStartTimes(msg.sender);
        uint256 randomAmount = _random()*blocksPassed* miningFactor;
        _gold._mintGold(msg.sender, randomAmount*(10**_difficultyFactor));

    }

    function _random() private view returns (uint) {
        uint randomHash = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp)));
        return 10 + (randomHash % 90);
    }

    function calculateMiningFactor() private view returns(uint64){
        uint64 miningFactor = 0;
        uint256[] memory minersMining = _miners.getMinerBalancesByMines(msg.sender, mineType);
        for(uint i=0; i<minersMining.length; i++) {
            if (_miners.isMinerSkilled(minersMining[i])) {
                miningFactor += 2;
            } else {
                miningFactor += 1;
            }
...
/*
This contract contains information about miners.
Each miner is stored in a Miner Struct and will have all the required attributes which other contracts can use.
All the attributes can be accessed via the get functions and some attributes can be set via set functions.
Only authorized contracts can set using the set functions.
*/
contract Miners {
    struct Miner  {
        string minerType;
    ...
    function getResourceType(uint256 minerId) public view returns(string memory){
        return miners[minerId].resourceType;
    }
...
import  "../Characters/MinerNFT.sol";
...
contract Gold is ERC20 {
    ...
    function _mintGold(address owner, uint256 amount) public {
        _mint(owner, amount);
...
contract MinerNFT is  ERC721URIStorage {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    Miners _miners;

    constructor(Miners minerStructAddress) ERC721("Miner", "MINER") {
        _miners = minerStructAddress;
    }

    // function to create the MinerNFT
    function createToken(string memory minerType, string memory resourceType, bool isSkilled,
        bool isMining, string memory tokenURI, address origin) public returns (uint) {
        // increment the token everytime we receive the request.
        _tokenIds.increment();

        // get current value of the token id.
        uint256 _currTokenId = _tokenIds.current();

        // mint the token id for the sender and set him as owner for this tokenId
        _mint(origin, _currTokenId);

        // set the uri of the token
        _setTokenURI(_currTokenId, tokenURI);

        _miners.addMiner(origin,  _currTokenId, minerType, resourceType, isSkilled, isMining, tokenURI);
        return _currTokenId;
    }
}
