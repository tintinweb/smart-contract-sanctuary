...
contract BluntHeads is ERC1155, Ownable, Pausable, ERC1155Burnable
{
    ...
    string public _BASE_URI = "https://ipfs.io/ipfs/Qmea7uYRwiDnefF93nVJWeEPZn8cGEstsunsE766vw9A4g/";
    ...
    constructor() ERC1155("https://ipfs.io/ipfs/Qmea7uYRwiDnefF93nVJWeEPZn8cGEstsunsE766vw9A4g/{id}.json") { _reserveBluntHeads(100); }
    ...
    //Mints Blunt Heads
    function BluntHeadsMint(uint numberOfTokens) public payable
    {
        require(_SALE_IS_ACTIVE, "Sale must be active to mint Blunts");
        require(numberOfTokens <= _MAX_BLUNTS_PURCHASE, "Can only mint 20 Blunts at a time");
        require(_BLUNTS_MINTED.add(numberOfTokens) <= _MAX_BLUNTS, "Purchase would exceed max supply of Blunts");
        require(_BLUNT_PRICE.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct. 0.08 ETH Per Blunt | 80000000000000000 WEI");
    ...
    //Reserves Blunt Heads
    function _reserveBluntHeads(uint numberOfTokens) public onlyOwner
    {
        //Mints Blunts
        for(uint i = 0; i < numberOfTokens; i++) 
        {
    ...
    //Sets Max Blunts for future Blunt Expansion Packs
    function __setMaxBlunts(uint256 MAX_BLUNTS) public onlyOwner { _MAX_BLUNTS = MAX_BLUNTS; }

    //Sets Max Blunts Purchaseable by Wallet
    function __setMaxBluntsPurchase(uint256 MAX_BLUNTS_PURCHASE) public onlyOwner { _MAX_BLUNTS_PURCHASE = MAX_BLUNTS_PURCHASE; }

    //Sets Future Blunt Price
    function __setBluntPrice(uint256 CARD_PRICE) public onlyOwner { _BLUNT_PRICE = CARD_PRICE; }

    //Flips Allowing Multiple Purchases for future Blunt Expansion Packs
