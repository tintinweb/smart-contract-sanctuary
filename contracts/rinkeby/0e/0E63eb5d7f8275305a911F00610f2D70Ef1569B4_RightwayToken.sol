...
library RightwayMetadata {
  ...
  function getAdditionalContent(RightwayDecoder.Drop storage drop, string storage contentApi, AdditionalContent memory content ) internal view returns (MetadataAdditionalContent memory) {
    MetadataAdditionalContent memory result;
    (result.uri, result.contentLibraryArweaveHash, result.contentIndex, result.contentType) = getContentDetails(drop, contentApi, content.contentIndex);
    result.slug = content.slug;
    return result;
  }

  function getAdditionalContents(RightwayDecoder.Drop storage drop, string storage contentApi, TokenState memory state) internal view returns (MetadataAdditionalContent[] memory) {
    ...
    for (uint idx = 0; idx < state.additionalContent.length; idx++) {
      result[outputIdx++] = getAdditionalContent(drop, contentApi, state.additionalContent[idx]);
    }
  ...
  function getTokenMetadata(TokenMetadata memory result, string storage creator, RightwayDecoder.Drop storage drop, string storage contentApi, uint256 tokenId, TokenState memory state) public view {
    ...
    result.additionalContent = getAdditionalContents(drop, contentApi, state);
  }
...
contract RightwayToken is ERC721Enumerable, Saleable, Ownable {
  ...
  function addTokenContent( uint256[] calldata tokens, string calldata slug, uint16 contentIndex) public onlyOwner unsealed {
...

{
  ...
  "libraries": {
    "/home/bart/git/InfinityTokens-contract/contracts/custom/RightwayMetadata.sol": {
      "RightwayMetadata": "0x693EFb76c0CcF8dCF19dF2f0F427f5d2e628BE1b"
    },
    "/home/bart/git/InfinityTokens-contract/contracts/custom/RightwayDecoder.sol": {
      "RightwayDecoder": "0x01426e8849208f247726248E7b33912033CF7820"
    }
  },
  "outputSelection": {
    "*": {
