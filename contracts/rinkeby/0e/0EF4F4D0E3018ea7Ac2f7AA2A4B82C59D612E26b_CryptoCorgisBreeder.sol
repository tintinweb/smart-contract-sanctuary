...
contract CryptoCorgisBreeder is Ownable, FactoryERC1155, ERC1155 {
  ...
  event CorgiSold(uint256 corgiNumber, uint256 blockNumber, uint8 mutantCorgiId);
  ...
  function getClaimedMutantCorgis() external view returns (uint256[] memory) {
    // 1-indexed.
    uint256[] memory blockNumbers = new uint256[](NUMBER_MUTANT_CORGIS);
    for (uint8 i = 1; i < NUMBER_MUTANT_CORGIS + 1; i++) {
      blockNumbers[i - 1] = mutantCorgiIdToBlockNumber[i];
    }
    return blockNumbers;
  }
  ...
  function mint(
    ...
    emit CorgiSold(nextCorgiId, _blockNumber, mutantId);
  }
}
