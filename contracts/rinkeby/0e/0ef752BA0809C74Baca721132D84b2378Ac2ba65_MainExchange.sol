...
contract MainExchange is Exchange {
    string public constant name = "Main Exchange";

    string public constant version = "3.1";

    constructor(
        address _authenticatedProxy,
        uint256 chainId,
        address[] memory registryAddrs,
        bytes memory customPersonalSignPrefix
    ) Exchange(_authenticatedProxy)public {
...
contract Exchange is ExchangeCore {
    ...
    constructor(address _authenticatedProxy)ExchangeCore(_authenticatedProxy){
...
contract ExchangeCore is ReentrancyGuarded, StaticCaller, EIP712 {
    ...
    function atomicMatch(
        ...
        if (keccak256(firstCall.data) != keccak256(bytes(""))) {
        ...
        if (keccak256(secondCall.data) != keccak256(bytes(""))) {
...
contract AuthenticatedProxy is TokenRecipient, OwnedUpgradeabilityStorage, AccessControl {
    /* Whether initialized. */
    ...
    function proxy(
        ...
        HowToCall howToCall,
        bytes memory data
    ) public onlyRole(DEFAULT_ADMIN_ROLE) returns (bool result) {
        bytes memory ret;
...
contract ProxyRegistry is Ownable, ProxyRegistryInterface {
    ...
    {
        require(
            address(proxies[user]) == address(0),
