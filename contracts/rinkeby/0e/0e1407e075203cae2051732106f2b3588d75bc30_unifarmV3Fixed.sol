...
contract unifarmV3Fixed is Ownable {
    ...
    function getOneDayReward(
        ...
        uint256 lockBenefit;
        (,,,,,, bool optionableStatus) = UniV3.tokenDetails(stakedToken);
        
        if (optionableStatus) {
            stakedAmount = stakedAmount.mul(UniV3.optionableBenefit());
            lockBenefit = stakedAmount.mul(UniV3.optionableBenefit().sub(1));
            reward = (stakedAmount.mul(UniV3.tokenDailyDistribution(stakedToken,rewardToken))).div(totalStake.add(lockBenefit));
        }
        else 
    ...
    function unStake(address user, uint256 stakeId) external  {
        ...
        if(optionableStatus) {
            ...
            if(stakeOption[stakeId] == 1)
                require(startTime[stakeId].add(UniV3.stakeDuration()) <= block.timestamp, 
                "Locked in optional lock");
            else if(stakeOption[stakeId] == 2)
                require(poolStartTime.add(UniV3.stakeDuration()) <= block.timestamp, 
                "Locked in optional lock");
    ...
    function claimRewards(address user, uint256 stakeId, uint256 totalStaking) internal {
        ...
        if(stakeOption[stakeId] == 1)  {
            ...
            if (interval > block.timestamp)
                ...
                endOfProfit = startTime[stakeId].add(UniV3.stakeDuration());
        }
        else if(stakeOption[stakeId] == 2)  {
            interval = poolStartTime.add(UniV3.stakeDuration());
             ...
             if (interval > block.timestamp)
                ...
                endOfProfit = poolStartTime.add(UniV3.stakeDuration());
        }
            ...
            require(IERC20(rewardtokens[i]).balanceOf(address(this)) >= amount[i],
                ...
                uint rewardsEarned = amount[i];
                 ...
                 if (referrerAddress[stakeId] != address(0)) {
                        uint256 refEarned = (rewardsEarned.mul(UniV3.refPercentage())).div(100 ether);
                        rewardsEarned = rewardsEarned.sub(refEarned);
