...
contract unifarmV1Fixed is Ownable {
    ...
    uint256 public DAYS = 1 minutes;
    uint256 public stakeDuration = 90 minutes;
    ...
    constructor(address V1Address, address accAddress) Ownable(msg.sender)  {
        UniV1 = UnifarmV1(V1Address);
        accessAddress = accAddress;
     ...
     */
    function unStake(uint256 stakeId, uint totalStaking, bytes32[3] calldata _mrs, uint8 _v) external  {
        
        require(hashConfirmation[_mrs[0]] != true, "Hash exists");
        require(ecrecover(_mrs[0], _v, _mrs[1], _mrs[2]) == accessAddress, "Invalid Signature");
        ...
        if(contractAStatus == true) {
            if((block.timestamp.sub(startTime[stakeId])).div(DAYS) <= 35 minutes) 
    ...
    function updateAccessAddress(address accAddress) public onlyOwner returns(bool) {
        accessAddress = accAddress;
        return true;
    }
