...
contract FirstPrice is BaseAuction, Testable {
    ...
    // Bid ID's to their information
    mapping(uint256 => Bid) internal lotBids_;
    // Bidder => Lot ID => Bid ID
    mapping(address => mapping(uint256 => uint256)) internal allBids_;
    ...
    {
        uint256 highestBidID = lotPrices_[_lotID].currentHighestBidID;

        return (
            lotBids_[highestBidID].amount,
            highestBidID,
            lotBids_[highestBidID].bidder
        );
    }

    function getExistingBidID(address _bidder, uint256 _lotID)
        external
        view
        returns (uint256)
    {
        return allBids_[_bidder][_lotID];
    }
    ...
    function createLot(
        ...
        require(_minSalePrice != 0, "Lot price cannot be 0");
        ...
        lotPrices_[_lotID].minPrice = _minSalePrice;
     ...
     */
    function bid(uint256 _lotID) external payable nonReentrant() {
        ...
        uint256 bidID;
        uint256 bidAmount;
        // Checking if user has made a bid, if so adds to existing bid
        if (allBids_[msg.sender][_lotID] == 0) {
            bidAmount = msg.value;
            require(
                msg.value >= lotPrices_[_lotID].minPrice &&
                    msg.value >
                    lotBids_[lotPrices_[_lotID].currentHighestBidID].amount,
                "Bid below price or leading bid"
            );
            // Getting the ID for the bid
            bidCounter_ += 1;
            bidID = bidCounter_;
            // Storing the bid information
            lotBids_[bidID].lotID = _lotID;
            lotBids_[bidID].amount = msg.value;
            lotBids_[bidID].bidder = msg.sender;
            // Storing that the user has bid
            allBids_[msg.sender][_lotID] = bidID;
            // Storing this bid as current highest
            lotPrices_[_lotID].currentHighestBidID = bidID;
        } else {
            // There is a bid that exists, adding to bid
            bidID = allBids_[msg.sender][_lotID];
            // Working out the bid new total
            bidAmount = lotBids_[bidID].amount.add(msg.value);
            // Checking that bid is valid
            require(
                bidAmount >= lotPrices_[_lotID].minPrice &&
                    bidAmount >
                    lotBids_[lotPrices_[_lotID].currentHighestBidID].amount,
                "Bid below price or leading bid"
            );
            lotBids_[bidID].amount = bidAmount;
            // Storing this bid as current highest
            lotPrices_[_lotID].currentHighestBidID = bidID;
        }

        emit BidOnLot(_lotID, bidID, msg.sender, bidAmount);
    }

    /**
     * @param   _bidID The ID of the bid
     * @notice  The claim function can be called by any address, but will only
     *          send winnings or refund bids to the address that originally
     *          created the bid. This function also handles refunds in the case
     *          of a canceled lot.
     *          This function will revert if the bid has been claimed, or if the
     *          lot has not passed it's end time.
     */
    function claim(uint256 _bidID) external nonReentrant() {
        require(lotBids_[_bidID].bidder != address(0), "Invalid bid ID");
        // Ensures the bid has not been claimed
        require(lotBids_[_bidID].claimed == false, "Bid has been claimed");
        // Stores the lot ID for this bid for ease of access
        uint256 lotID = lotBids_[_bidID].lotID;
        // Getting the current status of the lot
        IHub.LotStatus status;
        (, , , status) = auctionHubInstance_.getLotInformation(lotID);
        // Ensures the bid has passed it's end time
        require(!_isLotBiddable(lotID), "Cannot claim till lot closed");
        // Setting bid as claimed
        lotBids_[_bidID].claimed = true;
        // If the lot has been canceled
        if (status == IHub.LotStatus.AUCTION_CANCELED) {
            // Everyone then withdraws their bid
            _insecureLoser(
                lotID,
                lotBids_[_bidID].bidder,
                lotBids_[_bidID].amount
            );
        } else if (lotPrices_[lotID].currentHighestBidID == _bidID) {
            // If the user won the bid
            // sending the user their token and setting lot to claimed
            _winner(lotID, lotBids_[_bidID].bidder);
            // Handling the payment of the winning bid (royalties)
            _insecureHandlePayment(lotID, lotBids_[_bidID].amount);
        } else {
            // If the status has not been changed yet
            if (
                status != IHub.LotStatus.AUCTION_RESOLVED_AND_CLAIMED ||
                status != IHub.LotStatus.AUCTION_RESOLVED
            ) {
                // Sets the status to complete
                auctionHubInstance_.lotAuctionCompleted(auctionID_, lotID);
            }
            // Sending their funds back
            _insecureLoser(
                lotID,
                lotBids_[_bidID].bidder,
                lotBids_[_bidID].amount
