...
contract BullRunConfig is AccessControlUpgradeable {
    ...
    function initialize(address token_, address distributor_, address vault_) public initializer {
        require(deployer == msg.sender, "BullRunConfig: only deployer can execute initializer");
        __AccessControl_init();
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        updateAll(token_, distributor_, vault_);
    }
    ...
    function updateAll(address token_, address distributor_, address vault_) public {
        updateToken(token_);
    ...
    function grantPauserRole(address account) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _setupRole(keccak256("PAUSER_ROLE"), account);
    }
