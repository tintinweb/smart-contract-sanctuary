...
contract BattleCats is ERC721Enumerable, Ownable {
  ...
  uint256 public maxMintAmountPresalePerWallet = 20;
  ...
  // internal
    ...
    function preSaleMint(uint256 _mintAmount) internal {
        ...
        require(_mintAmount <= maxMintAmountPresalePerWallet, "max mint amount per session exceeded");
        ...
        require(
            _totalMintedPresale[msg.sender] + _mintAmount <= maxMintAmountPresalePerWallet,
    ...
    function addOneToPresaleListing(address _address)
        external
        onlyOwner
        {
            require(
  ...
  function saleMint(uint256 _mintAmount) internal {
    ...
    function reserveNFTs() public onlyOwner{
        ...
        uint256 supply = totalSupply();
        require(supply + reserve <= maxSupply, "max NFT limit exceeded");
  ...
  // No benefit because from bulk we can also transfer one
  function GiftOne(address to) external  onlyOwner {
      if (!paused){
        uint256 supply = totalSupply();
  ...
  function giftNFTs(address[] calldata _addresses)
        external
        onlyOwner
    ...
    function freeMint() external{
        ...
        uint256 supply = totalSupply();
        require(supply + 1 <= maxSupply, "max NFT limit exceeded");
        require(_giftFreeMint[msg.sender]==true, 'address not in free mint');
  ...
  function setTotalPresaleTickets( uint256 _totalPresaleTickets) public onlyOwner(){
