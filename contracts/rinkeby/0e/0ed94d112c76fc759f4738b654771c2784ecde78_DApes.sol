...
contract DApes is ERC1155PausableUpgradeable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function fakeMintOut(uint256 collectionID, uint256 end) public onlyOwner {
        Collection storage collection = collections[collectionID];
        for (; collection.nextID.current() < end; collection.nextID.increment()) {
            _usedNonces.set(collection.nextID.current());
            _supply.increment();
        }
    }
