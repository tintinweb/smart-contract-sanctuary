...
contract NounsToken is INounsToken, Ownable, ERC721Enumerable {
    ...
    // an array that contains our list of publicly available clothes (in the form)
    // of SVG snippets
    string[] public clothingList;

    // nested mapping which tracks state of clothing per noun. for example, if
    // noun with token id of 1 is wearing item 3 items of clothing at `clothingList`
    // index 4, 9, and 14, this should represented by `clothingState[1][4,9,14]`.
    // then when `tokenURI` is called, we'll iterate through this token ids
    // clothes, and 
    ...
    // use this to override an existing clothing snipppet
    ...
    constructor() ERC721('Fast Food Nouns', 'FFN') {
        // populate clothingList with hat at zero index
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        // encode the substring bytes (SVG minus closing tag)
        ...
        for (uint256 i = 0; i < clothingState[tokenId].length; i++) {
            ...
            finalSVGBytes = abi.encode(finalSVGBytes, clothingList[clothingIndexToAdd]);
        }
        // finally, encode the closing svg tag in
        finalSVGBytes = abi.encode(finalSVGBytes, '</svg>');
    ...
    function _mintTo(address to, uint256 nounId) internal returns (uint256) {
        ...
        _mint(to, nounId);
        // when we mint, automatically set them as wearing the fast food hat
