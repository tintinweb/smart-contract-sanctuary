...
contract KfcInu is Context, IERC20, Ownable {
    ...
    uint256 private _whaleTaxEth = 2_000_000_000_000_000_000;
    uint256 private _whaleTaxSupply = 500_000_000_000 * 10**9;
    uint256 private _whaleTaxFee = 20;
    ...
    event WhaleTaxActivated(address sender, uint256 tTransferAmount, uint256 tFee, uint256 tMarketingFee);
    event AmountOut(uint256 tTransferAmount, uint256 amountOut);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _marketingFee == 0 && _whaleTaxFee == 0) return;

        _previousTaxFee = _taxFee;
        _previousMarketingFee = _marketingFee;
        _previousWhaleTaxFee = _whaleTaxFee;
    ...
    function restoreAllFee() private {
        ...
        _whaleTaxFee = _previousWhaleTaxFee;
    }
    ...
    function _getValues(uint256 tAmount, address recipient) private returns (uint256, uint256, uint256, uint256, uint256, uint256) {
    ...
    function _getTValues(uint256 tAmount, address recipient, uint256 taxFee, uint256 marketingFee) private returns (uint256, uint256, uint256) {
        uint256 tFee = tAmount.mul(taxFee).div(100);
        uint256 tMarketing = tAmount.mul(marketingFee).div(100);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tMarketing);

        // like robin hood took from rich to return the plebs...
        if (whaleTaxEnabled && recipient == uniswapV2Pair) {
            ...
            if (!activateWhaleTax) {
                ...
                emit AmountOut(tTransferAmount, amountOut);
            ...
            if (activateWhaleTax) {
                ...
                emit WhaleTaxActivated(tx.origin, tAmount, tFee, tMarketing);
            }
        }
    ...
    function _setWhaleTaxFee(uint256 whaleTaxFee) external onlyOwner() {
        require(whaleTaxFee >= 0 && whaleTaxFee <= 49, 'whaleTaxFee should be in 0 - 49');
    ...
    function _setWhaleTaxEnabled(bool enabled) external onlyOwner(){
        whaleTaxEnabled = enabled;
    }

    function _setWhaleTaxEth(uint256 whaleTaxEth) external onlyOwner() {
        _whaleTaxEth = whaleTaxEth;
    }

    function _setWhaleTaxSupply(uint256 whaleTaxSupply) external onlyOwner() {
        _whaleTaxSupply = whaleTaxSupply;
    }
}