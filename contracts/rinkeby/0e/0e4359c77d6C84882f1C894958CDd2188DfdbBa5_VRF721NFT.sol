...
contract VRF721NFT is ERC721, Ownable {
  ...
  function mint(address recipient, uint256 quantity) external payable {
    ...
    require((_tokenIds.current() + quantity) <= supplyLimit, "VRF721NFT::mint: would cause total supply to exceed max supply");
  ...
  function initiateRandomDistribution() external {
    require(block.timestamp > mintingEndTimeUnix, "VRF721NFT::mint: minting period has not ended");
    uint256 supply = _tokenIds.current();
