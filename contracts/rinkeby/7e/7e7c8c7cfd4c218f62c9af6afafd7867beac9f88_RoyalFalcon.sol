...
contract RoyalFalcon is ERC721Enumerable, Ownable {
  ...
  //      require(balanceOf(msg.sender) == 0, "Only 1 mint per account");
        ...
        require(!isPaused,"pause");
        require(_mintAmount > 0,"not 0");
        require(_mintAmount <= maxMintAmount,"max full");
        require(supply + _mintAmount <= maxSupply,"supply not");

        if (msg.sender != owner()) {
            require(msg.value >= cost * _mintAmount,"sin fondos");
        }
