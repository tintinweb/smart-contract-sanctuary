...
import "./interfaces/IPiraUtil.sol";
import "./interfaces/IPiraNFT.sol";
import "./PiraAdminModifier.sol";

contract PiraNFTMarket is ReentrancyGuard, IPiraUtil, PiraAdminModifier {
    ...
    address private _royaltyContractAddress;
    address private _adminContractAddress;

    constructor(address royaltyContractAddress, address adminContractAddress)
        PiraAdminModifier(adminContractAddress)
    {
        _royaltyContractAddress = royaltyContractAddress;
    ...
    function changeOwner(address payable owner) public onlyAdmin {
        _owner = owner;
    }

    function changeRoyaltyAddress(address royaltyContractAddress)
        public
        onlyAdmin
    {
        _royaltyContractAddress = royaltyContractAddress;
        ...
        function changeAdminAddress(address adminContractAddress)
        public
        onlyAdmin
    {
        _adminContractAddress = adminContractAddress;
        ...
        nonReentrant
    {
        ...
        _payRoyalty(nftContract, itemId, idToMarketItem[itemId].tokenId, price);
    ...
    function _payRoyalty(
        ...
        IPiraNFT tokenContract = IPiraNFT(nftContract);
        IPiraRoyalty royaltyContract = IPiraRoyalty(_royaltyContractAddress);

        address payable author = payable(tokenContract.authorOf(tokenId));
        RoyaltyRule memory rule = royaltyContract.getItemRoyaltyRuleDetails(
            nftContract,
            tokenId
        );

        uint256 valueToSeller = sellingPrice;
        if (rule.exists) {
            if (_isFirstSale(nftContract, tokenId)) {
                uint256 firstSaleFee = sellingPrice.div(100).mul(
                    rule.firstSalePiraFee
                );
                _owner.transfer(firstSaleFee);
                valueToSeller = valueToSeller.sub(firstSaleFee);
            } else {
                uint256 fee = sellingPrice.div(100).mul(rule.piraFee);
                _owner.transfer(fee);
                valueToSeller = valueToSeller.sub(fee);
            }
            uint256 royalty = sellingPrice.div(100).mul(rule.royaltyFee);
            author.transfer(royalty);
            valueToSeller = valueToSeller.sub(royalty);
        }

        idToMarketItem[itemId].seller.transfer(valueToSeller);
    }

    function _isFirstSale(address nftContract, uint256 tokenId)
        private
        view
        returns (bool)
    {
...
interface IPiraNFT {
    ...
    function mintByOther(string memory tokenURI, address author) external returns (uint256);

    function tokenURI(uint256 tokenId, string memory piraHash) external view returns (string memory);

    function enablePiraHash(string memory piraHash) external;

    function disablePiraHash() external;
