...
contract PickRandomWinner is VRFConsumerBase, Ownable {
    ...
    function requestWinnerSigningHash(uint256 blockExpiry, uint256 documentId, string memory urlHash, uint256 totalSigners) public view returns (bytes32) {
        return keccak256(abi.encodePacked(blockExpiry, documentId, urlHash, totalSigners));
    }
    
    // Check if the contract has enough Link
    // Fetch the current user
    //  - Check signing hash to make sure that total signers is authorized 
    ...
    function requestWinner(bytes memory signature, uint256 blockExpiry, uint256 documentId, string memory urlHash, uint256 totalSigners) public {
        ...
        require(ECDSA.recover(signingHash, signature) == privSignedVerifier, "Permission to call this function failed");
