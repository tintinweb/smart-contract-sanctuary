...
import "./interfaces/IIggyPunks.sol";

/**
 * @title IggyPunks NFTs
 * @dev Implementation of the {IIggyPunks} interface.
 * @author Ahmed Ali Bhatti <github.com/ahmedali8>
 */
contract IggyPunks is IIggyPunks, Ownable, ERC721Enumerable {
    ...
    string private constant BASE_TOKEN_URI =
        "ipfs://QmbSjVCAF7VfDk3ekX3e7TLcs2N9rwAzB4tnvYoCchCp9d/";

    /**
     * @dev See {IIggys-MAX_SUPPLY}.
     */
    uint256 public constant override MAX_SUPPLY = 5000;

    /**
     * @dev See {IIggys-PRICE_PER_TOKEN}.
     */
    uint256 public constant override PRICE_PER_TOKEN = 0.04 ether;
    ...
    /**
     * @dev See {IIggys-punkcrushers}.
     */
    address public override punkcrushers;

    /**
     * @dev See {IIggys-iggyClaims}.
     * if an address has claimed his iggy or not.
     */
    mapping(address => bool) public iggyClaims;

    /**
     * @dev Sets {dev}, {punkcrushers} and {ERC721-constructor}.
     */
    constructor(address _dev, address _punkcrushers)
        ERC721("IggyPunks", "IGGYPNK")
    {
        dev = _dev;
        punkcrushers = _punkcrushers;
    }

    /**
     * @dev See {IIggys-baseURI}.
     */
    ...
    /**
     * @dev See {IIggys-iggysOfOwner}.
     */
    function iggysOfOwner(address _owner)
        public
        view
        override
    ...
    /**
     * @dev See {IIggys-claimFreeIggy}.
     *
     * Emits a {NFTMinted} event indicating the mint of a new NFT
     * and {IggyClaimed} event indicating claimed Iggy owner.
     *
     * Requirements:
     *
     * - caller must have 2 punkcrushers.
     * - caller must not have already claimed Iggy.
     */
    function claimFreeIggy() external override {
        uint256 _balance = IERC721(punkcrushers).balanceOf(_msgSender());
        require(
            _balance >= 2,
            "ClaimFreeIggy: user must have 2 punkcrushers to claim free iggy"
        );
        require(
            !iggyClaims[_msgSender()],
            "ClaimFreeIggy: Iggy already claimed"
        );
        ...
        // update claim state
        iggyClaims[_msgSender()] = true;

        emit IggyClaimed(_msgSender());
    }
     ...
     * Requirements:
     *
     * - `_noOfTokens` must be less than 5.
     * - `_value` must be equal to correct tokenPrice.
     */
    function mint(uint256 _noOfTokens) external payable override {
        uint256 _value = msg.value;
        require(_noOfTokens <= 5, "Mint: max 5 per transaction");
     ...
     */
    function reserveTokens(uint256 _noOfTokens) external override onlyOwner {
        require(_noOfTokens <= 20, "ReserveTokens: max 20 per transaction");
        _mint(_noOfTokens);
    }

    /**
     * @dev Internal function called in {claimFreeIggy}, {mint} and {reserveTokens}.
     * mints tokens and transfers to `caller`.
     *
     * Emits a {FundsTransferred} event indicating transfer of funds and
     * {NFTMinted} event indicating NFT mint details.
     *
     ...
     */
    function _mint(uint256 _noOfTokens) internal {
    ...
    /**
     * @dev Internal function called in {mint}.
...
/**
 * @dev Interface of the IggyPunks.
 * @author Ahmed Ali Bhatti <github.com/ahmedali8>
 */
interface IIggyPunks {
    /**
     * @dev Emitted when an iggy is claimed.
     */
    event IggyClaimed(address indexed beneficiary);
    ...
    /**
     * @dev Returns the max allowed supply i.e. 5,000.
     */
    function MAX_SUPPLY() external view returns (uint256);

    /**
     * @dev Returns the price per token.
     */
    ...
    /**
     * @dev Returns the address of {punkcrushers}.
     */
    function punkcrushers() external view returns (address);

    /**
     * @dev Returns a boolean value for address if iggy claimed.
     */
    function iggyClaims(address) external view returns (bool);
    ...
    /**
     * @dev Returns list of all iggys of a wallet.
     */
    function iggysOfOwner(address _owner)
        external
        view
    ...
    /**
     * @dev Mints a Iggy if `caller` has 2 punkcrushers.
     *
     * Emits a {NFTMinted} and {IggyClaimed} events.
     */
    function claimFreeIggy() external;
