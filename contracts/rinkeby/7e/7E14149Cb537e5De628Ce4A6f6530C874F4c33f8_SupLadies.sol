...
contract SupLadies is Delegated, ERC721EnumerableB, PaymentSplitter {
  ...
  bool public isVoucherActive = false;
  ...
  function mintVouchersFromAccount( uint quantity ) external {
    require( isVoucherActive,       "Voucher sale is not active" );
    require( quantity <= MAX_ORDER, "Order too big" );
  ...
  function useVouchers( uint quantity ) external {
    require( isVoucherActive,       "Sale is not active" );
    require( quantity <= MAX_ORDER, "Order too big" );
  ...
  function setActive(bool isActive_, bool isPineapplesActive_, bool isVoucherActive_) external onlyDelegates{
    require( isMintActive != isActive_ ||
      isPineapplesActive != isPineapplesActive_ ||
      isVoucherActive != isVoucherActive_, "New value matches old" );
    isMintActive = isActive_;
    isPineapplesActive = isPineapplesActive_;
    isVoucherActive = isVoucherActive_;
  }
