...
contract EJuneToken is EGovernanceBase, ERC20Burnable {
    /**
     * @dev Grants `DEFAULT_ADMIN_ROLE`, `JUNE_MINTER_ROLE` to the
     ...
     * Requirements:
     *
     * - the caller must have the `MINTER PERMISSION`.
     */
    function mint(address to, uint256 amount) public virtual onlyJuneMinterPermission{
        _mint(to, amount);
    }

    function transferToGatewayOracle(address from, uint256 amount) public onlyFromJuneGatewayOracleAddress{
        _transfer(from, governance.juneGatewayOracleAddress(), amount);
    }

    function transferToJuneWallet(address from, uint256 amount) public onlyFromJuneNFTMarketAddress{
        _transfer(from, governance.juneWallet(), amount);
    }
