...
contract HashBridge is Ownable {
	...
	event OrderAdd(uint indexed orderId, uint indexed offerId, uint rate, address withdrawAddress, uint amount, uint payAmount, address payAddress, uint reservedUntil, address ownerAddress);
	...
	function addOrder(uint _offerId, address _withdrawAddress, uint _amount, uint _payAmount) public {
		...
		emit OrderAdd(orderId, _offerId, rate, _withdrawAddress, _amount, _payAmount, _payAddress, reservedUntil, msg.sender);
	}
