...
contract HighStreetNftPool is ReentrancyGuard, ERC721Holder {
     ...
     * @param nfts an array which stored the NFT id withdrew
     */
    event EmergencyWithdraw(address indexed _by, uint256 amount, uint256[] nfts);
    ...
    function unstakeReward(
        ...
        _unstakeReward(msg.sender, _depositId, _amount);
    }

    /**
     * @notice Emergency withdraw specified amount of NFTs
     *
     * @dev Reentrancy safety enforced via `ReentrancyGuard.nonReentrant`
     *
     * @param _listIds index ID to unstake from, zero-indexed
     */
    function emergencyWithdraw(uint256[] calldata _listIds) external nonReentrant {
        // delegate call to an internal function
        _emergencyWithdraw(msg.sender, _listIds);
    }
    ...
    function _unstakeReward(
      ...
      // verify an amount is set
        ...
        require(stakeDeposit.rewardAmount >= _amount, "amount exceeds stake");

        // update the deposit, or delete it if its depleted
        if (stakeDeposit.rewardAmount - _amount == 0) {
            delete user.deposits[_depositId];
        } else {
            stakeDeposit.rewardAmount -= _amount;
        }
        ...
        // emit an event
        emit UnstakedReward(msg.sender, _staker, _amount);
    }

    /**
     * @dev Used internally, mostly by children implementations, see emergencyWithdraw()
     *
     * @param _staker an address which unstakes the NFT (which has staked some tokens earlier)
     * @param _listIds index ID to unstake from, zero-indexed
     */
    function _emergencyWithdraw(
        ...
        emit EmergencyWithdraw(msg.sender, amount, nfts);
    }

    /**
     * @dev Mints HIGH tokens
     *
     * @param _to an address to mint tokens to
