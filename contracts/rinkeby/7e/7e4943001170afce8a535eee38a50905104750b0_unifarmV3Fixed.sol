...
contract unifarmV3Fixed is Ownable {
    ...
    function unStake(address user, uint256 stakeId) external whenNotPaused {
        ...
        if(unstakeStatus[user][stakeId] == false && activeStatus[stakeId] == true) 
         ...
         (address stakedToken, uint256 totalStaking) = swapPaidToken(tokenAddress[stakeId]);
     ...
     function _rewardCalculation(address user, address referrerAddress, address stakedToken, uint256 stakedAmount, uint256 interval, uint256 totalStake
        ...
        if (referrerAddress != address(0)) {
            uint256 refEarned =
                ...
                require(IERC20(stakedToken1).transfer(
                    referrerAddress,
                    refEarned), "Transfer Failed");
    ...
    function swapPaidToken(address tokenAddress) internal  returns(address, uint256) {
        ...
        if(tokenAddress == oldPaidAddress) {
            ...
            uint256 totalStaking1 = totalStaking.sub(totalUnStakingB[tokenAddress]);
