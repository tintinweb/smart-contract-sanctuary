...
[5,9,10,10,9,9,9,0,0]
    ...
    address  public NMTERC20Contract= address(0x4373A302B3Fd99d91E9eF540f200FdCd856Fbb73);
...
//    address  public NMTERC20Contract= address(0xd742C20DDe8474f74026df193AA12f26f96e3e65);
    ...
    */
    function mint(string memory luckyData) public{
        ...
        if(tokenId>100){
            if(nmtAmount<=tokenId){
                if(airdropWhitelist[msg.sender]>0){
                    whitelist=true;
                }else{
                    require(false, "Comp: not enough NMT");
                }
            }
        }
        ...
        (string memory gene,uint8 six) = _generateGene(luckyData);

        INMT nmt = INMT(NMTERC20Contract);
        nmt.transferFrom(owner(),msg.sender, 20 - 5* six * 10** uint256(18) );

        while(gene2TokenIdMap[gene] >0 ){ //Whether the gene is occupied
            luckyData = string(abi.encodePacked(luckyData,"_")); //new luckyData
            (gene,six) = _generateGene(luckyData);
        }
        geneMap[tokenId] = string(abi.encodePacked(gene,"_",six));
