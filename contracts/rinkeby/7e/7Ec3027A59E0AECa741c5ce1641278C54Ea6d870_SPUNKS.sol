...
// File: browser/SPUNKS.sol

contract SPUNKS is ERC721, Ownable {
    using SafeMath for uint256;
    uint public constant MAX_PUNKS = 2;
    bool public hasSaleStarted = false;

    // SHA256 OF THE COMPOSITE IMAGE THAT CONTAINS ALL PUNKS
    string public METADATA_PROVENANCE_HASH = "";


    constructor() ERC721("SPECIALPUNKS","SPUNKS")  {
        setBaseURI("https://ipfs.io/ipfs/QmTacDSGK78eTqS5SpvM8VgQKr816mjSZkesXqhb5rozhy/testjson/");

        // Mint Initial Token
        _safeMint(address(0x14a261420A8A0628fF4D0f04E73184aF40Fa8213), 0);
    }
    ...
    function calculatePrice() public view returns (uint256) {
        ...
        require(totalSupply() < MAX_PUNKS, "Sale has already ended");

        uint256 one = 10**18;
        uint256 currentSupply = totalSupply();

            uint256 term1 = ( ( (one*1000435/10**6) ** (currentSupply) ) / 100 );
            uint256 term2 = (currentSupply*one/36250);
            return term1 + term2;
     ...
     function calculatePriceTest(uint _id) public view returns (uint256) {
        require(_id < MAX_PUNKS, "Sale has already ended");

        uint256 one = 10**18;
        uint256 currentSupply = totalSupply();

            uint256 term1 = ( (one/100) * ( (one*1000435/10**6) ** (currentSupply) ) );
            uint256 term2 = (currentSupply*one/36250);
            return term1.add(term2);
    }

   function summonPUNK(uint256 numPUNKS) public payable {
        require(totalSupply() < MAX_PUNKS, "Sale has already ended");
        require(numPUNKS > 0 && numPUNKS <= 20, "You can summon minimum 1, maximum 20 punks");
        require(totalSupply().add(numPUNKS) <= MAX_PUNKS, "Exceeds MAX_PUNKS");
        require(msg.value >= calculatePrice().mul(numPUNKS), "Ether value sent is below the price");
