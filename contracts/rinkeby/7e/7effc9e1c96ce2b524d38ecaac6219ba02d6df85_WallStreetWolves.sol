...
contract WallStreetWolves is ERC721Enumerable, Ownable {
    ...
    bool private _enableSuperWhiteList = false;
    bool private _enableOnChainSuperWhiteList = true;
    mapping(address => bool) private _mappingSuperWhiteList;
    ...
    function setSuperWhiteList(address[] memory whiteListAddress, bool bEnable) external onlyOwner {
        for (uint256 i = 0; i < whiteListAddress.length; i++) {
            _mappingSuperWhiteList[whiteListAddress[i]] = bEnable;
        }
    }
    ...
    function superPurchase(uint256 numberOfTokens) external {
        require(_publicWOLFIE.current() < WOLFIE_MAX,"Purchase would exceed WOLFIE_MAX");
        
        if (msg.sender != owner()) {
            require (_enableSuperWhiteList == true, "Super Purchase is disabled.");

            if (_enableOnChainSuperWhiteList == true) {
                require(_mappingSuperWhiteList[msg.sender] == true, "Not registered to Super WhiteList");   
            }
    ...
    function presale(uint256 numberOfTokens) external payable {
        ...
        if (msg.sender != owner()) {
            ...
            require(block.timestamp > _activeDateTime - PRESALE_HOURS && block.timestamp < _activeDateTime , "Mint is not activated for presale");
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        if (msg.sender != owner()) {
            ...
            require(block.timestamp > _activeDateTime,"Contract is not active for public sale");
