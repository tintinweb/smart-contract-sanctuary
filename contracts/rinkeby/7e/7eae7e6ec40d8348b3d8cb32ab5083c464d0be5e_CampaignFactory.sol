...
contract Campaign is ReentrancyGuard, Pausable, Whitelist, Blacklist {
        ...
        nonReentrant
    {
        ...
        require(
            _validPurchase(_msgSender()),
            "ICO_CAMPAIGN::PURCHASE_NOT_ALLOWED"
        );
    ...
    function _validPurchase(address _address)
        internal
        view
        returns (bool withinPeriod)
    {
        if (doubleCheck) {
            withinPeriod =
                block.timestamp >= getTierTimestampFromTier(_address) &&
    ...
    function setOpenTimeAndRealeaseDateAndVerifyStartTime(
        ...
        openTime = _openTime;
        closeTime = _openTime.add(_duration);
        _updateOpenTimeAndDuration(_openTime, _duration, _verifyStartTime);
        emit CampaignStatsChanged();
    }
...
contract Whitelist is Ownable {
    ...
    mapping(address => UserTier) whitelist;

    event TiersChanged(uint256 tier1, uint256 tier2, uint256 tier3);
    event WhitelistSettedUp(
        IStakBank stakBank,
    ...
    function verifyStake() external {
        require(
            !whitelist[_msgSender()].isWhitelited,
        ...
        if (staked >= tier3) {
            whitelist[_msgSender()].userTier = tier3;
        } else if (staked >= tier2) {
            whitelist[_msgSender()].userTier = tier2;
        } else {
            whitelist[_msgSender()].userTier = tier1;
        }

        whitelist[_msgSender()].isWhitelited = true;
    }

    function getTierTimestampFromTier(address _address)
        public
        view
        returns (uint256 tierTimestamp)
    {
        uint256 tier = whitelist[_address].userTier;
        uint256 durationPart = campaignDuration.div(3);
        if (tier == tier3) {
            tierTimestamp = campaignOpenTime;
        } else if (tier == tier2) {
            tierTimestamp = campaignOpenTime.add(durationPart);
        } else if (tier == tier1) {
            tierTimestamp = campaignOpenTime.add(durationPart.mul(2));
        }
    }
    ...
    function setTiers(
        ...
        tier2 = _tier2;
        tier3 = _tier3;
        emit TiersChanged(tier1, tier2, tier3);
    }
    ...
    function isWhitelisted(address _address) public view returns (bool) {
        if (doubleCheck) {
            return
                whitelist[_address].isWhitelited &&
                stakBank.balanceOf(_address) >= whitelist[_address].userTier;
        } else {
            return stakBank.balanceOf(_address) >= tier1;
        }
    }
    ...
    function _updateOpenTimeAndDuration(
        uint256 _openTime,
        uint256 _duration,
        uint256 _verifyStartTime
    ) internal {
