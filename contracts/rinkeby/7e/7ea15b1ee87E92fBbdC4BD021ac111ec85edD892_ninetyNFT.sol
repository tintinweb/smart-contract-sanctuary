...
contract ninetyNFT is ERC721Enumerable, Ownable {
    ...
    /// @dev Emitted when a XXX token is claimed based on a Rat held
    ...
    IERC721 public BAYC = IERC721(0xE2E10b44A9d290b13510ce1f9D41Cb02b32f6038);
    string public XXXarchives_PROVENANCE = "";
    bool private onTransfer = true;
    uint256 public constant MAX_XXX_SUPPLY = 10000;
    uint256 public XXX_PACK_LIMIT = 10;
    uint256 public XXX_PRICE = 6 ether / 100;
    uint256 public XXX_PRICE_PRESALE = 4 ether / 100;
    ...
    I90Archive private MDR =
        I90Archive(0x99B9791D1580BF504a1709d310923A46237c8f2C);

    uint256 public startingIndex = 1000;
    ...
    mapping(uint256 => XXXArchives) public deadbod;
    mapping(uint256 => XXXArchiveParts) public deadpart;
    ...
    // constructor() ERC721("The 90s Archives", "90s") {
    // }

    constructor() ERC721("The XXX Archives", "80") {
    }
    ...
    */
    function setProvenanceHash(string memory provenanceHash) public onlyOwner {
        XXXarchives_PROVENANCE = provenanceHash;
    }
    ...
    function setArchivePartsOwner(uint256[] memory partIDs, address[] memory _owner, uint256[] memory bodySection) public onlyOwner {
        ...
        for (uint256 i = 0; i < partIDs.length; i++) {
            deadpart[partIDs[i]] = XXXArchiveParts({
                owner : _owner[i],
    ...
    function preSetTraitNFTs(uint256[] memory tokenIDs, uint256[] memory archiveTraits) public onlyOwner {
        ...
        for (uint256 i = 0; i < tokenIDs.length; i++) {
            deadbod[tokenIDs[i]] = XXXArchives({
                    bg : archiveTraits[j],
    ...
    /**
     * @dev Mint `numberOfXXXs` 90Archives.
     * - Will only mint up to (`MAX_XXX_SUPPLY` - `ratClaimsReserve`) 90Archives.
     * - Will mint up to `XXX_PACK_LIMIT` items at once.
     * - Will only mint after the sale is started.
     * - ETH sent must equal (`numberOfXXXs` * `XXX_PRICE`)
     *
     * @param numberOfXXXs The number of 90Archives NFTs to mint.
     */
    function mintXXXsPreSale(uint256 numberOfXXXs) public payable returns (uint256[] memory){
        //Whitelisted
        if(preSaleStarted){
        ...
        require(totalSupply() < MAX_XXX_SUPPLY, "Archieves sold out");
        require(balanceOf(msg.sender) + numberOfXXXs <= XXX_PACK_LIMIT + OGNFTWhitelistLimit[msg.sender], "Buy limit exceeded");
        require(numberOfXXXs > 0, "Need to mint at least one");
        require(XXX_PRICE_PRESALE * numberOfXXXs == msg.value, "Invalid ETH Amount");

        uint256[] memory resultIDs = new uint256[](numberOfXXXs);
        //Minting loop
        for (uint256 i = 0; i < numberOfXXXs; i++) {
     ...
     * @param numberOfXXXs The number of 90Archives NFTs to mint.
     */
    function mintXXXsPublic(uint256 numberOfXXXs) public payable returns (uint256[] memory){
        _enforceSaleStarted();
        require(totalSupply() < MAX_XXX_SUPPLY, "Archieves sold out");
        require(numberOfXXXs <= XXX_PACK_LIMIT, "Buy limit exceeded");
        require(numberOfXXXs > 0, "Need to mint at least one");
        if(BAYC.balanceOf(msg.sender) > 0 || specialWhitelist[msg.sender]){
            require(XXX_PRICE_PRESALE * numberOfXXXs == msg.value, "Invalid ETH Amount");
        }
        else{
            require(XXX_PRICE * numberOfXXXs == msg.value, "Invalid ETH Amount Public");
        }
    ...
    function mintXXXsOG(uint256 numberOfXXXs) public returns (uint256[] memory){
        //Whitelisted
        require(totalSupply() < MAX_XXX_SUPPLY, "Archieves sold out");
        require(numberOfXXXs <= XXX_PACK_LIMIT, "Buy limit exceeded");
        require(numberOfXXXs > 0, "Need to mint at least one");
        require(numberOfXXXs + balanceOf(msg.sender) <= OGNFTWhitelistLimit[msg.sender], "Mint limit exceeded");
    ...
    function mintXXXsAdmin(uint256[] memory archiveIDs, address[] memory receivers) public onlyOwner {
        require(totalSupply() < MAX_XXX_SUPPLY, "Archieves sold out");
     ...
     *
     * @param tokenId Identity of an existing (minted) XXX NFT.
     */
