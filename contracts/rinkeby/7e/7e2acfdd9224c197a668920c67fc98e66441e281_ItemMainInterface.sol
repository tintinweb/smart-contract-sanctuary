...
contract ItemMainInterface is IItemMainInterface, DynamicMetadataCapableElement {
    ...
    function _dynamicMetadataElementSupportsInterface(bytes4 selector) internal override view returns (bool) {
    ...
    function approve(uint256 itemId, address operator, address spender, uint256 amount) override external {
        ItemData storage itemData = _checkItemPermissionAndRetrieveData(itemId);
        require(spender != address(0), "approve to the zero address");
        itemData.allowance[operator][spender] = amount;
    }
