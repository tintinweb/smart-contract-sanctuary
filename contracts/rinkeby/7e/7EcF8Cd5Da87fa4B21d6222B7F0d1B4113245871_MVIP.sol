...
contract MVIP is ERC721Creator {
     uint256 public price = 10000000000000000; //0.04 ETH
     bool public saleIsActive = true;

    constructor(string memory tokenName, string memory symbol) ERC721Creator(tokenName, symbol) {}

    /* Claim 1 token  */
    function claim(address to) public nonReentrant payable {
      ...
      /* uint r = ((keccak256(abi.encodePacked(block.difficulty, block.timestamp, _tokenCount))) % 100) + 1; */
      string memory uri;
      uint r = (random()%100)+1;
      if (r < 41 ){
        uri = "ipfs://QmU4K5G1cJK84GWpMxLD8RtT9w6BjBNW1E1J6mvRHLoSav";
      } else if (r >= 41 && r <  69){
        uri = "ipfs://QmZ5z62h3DRQjMRmE1jURU8YYvGqVvwT6hdDVpKAJiBKCT";
      } else if (r >= 69 && r < 86){
        uri = "ipfs://QmZVFji6FUapjupvrkkKtNMXSsNgmqa4BDsvBF7oPzdS7z";
      } else if (r >= 86 && r < 96){
        uri = "ipfs://QmamqS5JRCw8cXbQN23c2gfb5V2anZCwa2pAPfgndBQ8RT";
      } else if (r >= 96 && r < 100){
        uri = "ipfs://QmXYissrc1YSgRyVuJXfgaANGTBA1UcqcTeiNNJ6RqfJgQ";
      } else {
        uri = "ipfs://QmaudNedZghGF8WHRd3yXi5mBPL7fP9UfTpNLCDYK5dkma";
      }
      _mintBase(to, uri);
    }

    /* claim multiple tokens */
    function claimBatch(address to, string[] calldata uris) public nonReentrant payable {
      require(saleIsActive, "Sale must be active to mint");
      require(msg.value >= price*uris.length, "Price is not correct.");
