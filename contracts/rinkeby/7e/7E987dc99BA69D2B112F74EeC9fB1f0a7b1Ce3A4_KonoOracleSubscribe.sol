// https://github.com/pancakeswap/pancake-farm/blob/master/contracts/BnbStaking.sol
 ...
 */
// contract KonoOracleSubscribe is OwnableUpgradeable {
contract KonoOracleSubscribe is Ownable {
    ...
    // roughly around 12 kono per month
    uint256 public konoPerBlock = 70000000000000;

    // roughly 172800 blocks per month
    uint256 public immutable month = 172800;
    ...
    // limit total amount of supply of Kono
    uint256 public limitAmount = 1000000000000000000000000;
    ...
    // as code that is inside a constructor or part of a global variable declaration
    // is not part of a deployed contract’s runtime bytecode
    constructor(address _adminAddress, address _kono) {
        adminAddress = _adminAddress;
        kono = ERC20(_kono);
    }

    // // OpenZeppelin upgradeable ensures this can only be called once by proxy
    // function initialize(address _adminAddress, address _bkono)
    //     public
    //     payable
    //     initializer
    // {
    ...
    function derivePayable(uint8[] calldata _sources, uint256 _leaseEnd)
        internal
        view
        returns (uint256)
    {
        return (_leaseEnd - block.number) * konoPerBlock * _sources.length;
    }
    ...
    function subscribe(
        ...
        uint256 _leaseEnd
    ) external payable validLease(_leaseEnd) {
        console.log("Msg sender", msg.sender);
        console.log("Sender balance is %s tokens", kono.balanceOf(msg.sender));
        console.log("Trying to send %s tokens", msg.value);
        console.log("Block number %s", block.number);
        console.log("Block period %s", (_leaseEnd - block.number));
        ...
        uint256 amountToPay = derivePayable(_sources, _leaseEnd);
        console.log("Amount to pay %s", amountToPay);
        require(amountToPay <= msg.value, "Not Enough Kono");
        
        assert(kono.transferFrom(msg.sender, address(this), amountToPay));

        // TODO: avoid this block for stack too deep compilation error
        // {
        //     uint256 amountToPay = derivePayable(_sources, _leaseEnd);
        //     require(amountToPay <= msg.value, "Not Enough Kono");
        //     assert(kono.transferFrom(msg.sender, address(this), amountToPay));
    ...
    function markUnsubscribed(
        ...
        // TODO: remove the user key if the subscriptions of the user is 0
        ...
        if (_amount > 0) {
            ...
            // Remove the account info if the amount is 0
            if (user.amount == 0) {
