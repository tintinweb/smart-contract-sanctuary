...
contract MotherDuckersTest1 is ERC721, Ownable, ReentrancyGuard {
  ...
  uint256 public Mother_Duckers_PRICE = 0.033 ether;
  ...
  function presaleMint(uint256 _quantity) external payable nonReentrant {
    ...
    require(_quantity <= MAX_Mother_Duckers_WHITELIST_CAP, "You can only mint a maximum of 2 for presale");
  ...
  function addToWhitelist(address[] calldata _addresses) external onlyOwner {
    for (uint256 i = 0; i < _addresses.length; i++) {
        ...
        whitelistAddressMintCount[_addresses[i]] > 0 ? whitelistAddressMintCount[_addresses[i]] : 0;
    }
  }  
