...
contract MonsterApeClub is ERC721Enumerable, Ownable {
    ...
    function buy(uint256 _start, uint256 _end, uint256 _amount, bytes memory _signature) public payable {
        ...
        address signer = signatureWallet(_start, _end, _amount, _signature);
    ...
    /**
     * @notice Signatures wallet and return the signer address
     *
     * @param _start mint start timestamp
     * @param _end mint end timestamp
     * @param _amount tokens to mint
     * @param _signature signed message confirming access to the mint
     */
    function signatureWallet(uint256 _start, uint256 _end, uint256 _amount, bytes memory _signature) public pure returns (address) {
        return ECDSA.recover(keccak256(abi.encode(_start, _end, _amount)), _signature);
    }
