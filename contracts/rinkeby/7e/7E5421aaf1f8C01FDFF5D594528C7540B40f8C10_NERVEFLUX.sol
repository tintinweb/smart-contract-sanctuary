...
contract NERVEFLUX is ERC20, Ownable {
    ...
    uint8 public charityFee = 5;
    uint8 public burnFee = 30;
    uint8 public stakingFee = 5;
    uint16 private totalFee = liquidityFee + charityFee + burnFee + stakingFee;

    address public _charityWalletAddress = address(0x123);
    address public _stakingWalletAddress = address(0x1234);
    ...
    constructor() ERC20("NERVEFLUX", "NERVE") {
    ...
    function blacklistAddress(address account, bool value) external onlyOwner{
        if(value && !_isBlacklisted[account]){
    ...
    function setLiquidityFee(uint8 value) external onlyOwner{
        liquidityFee = value;
        totalFee = liquidityFee + charityFee + burnFee + stakingFee;
    }
    ...
    function setBurnFee(uint8 _burn) external onlyOwner{
        burnFee = _burn;
        totalFee = liquidityFee + charityFee + burnFee + stakingFee;
    }

    function setStakingFee(uint8 _staking) external onlyOwner{
        stakingFee = _staking;
        totalFee = liquidityFee + charityFee + burnFee + stakingFee;
    }
    
    function setMinForSwap(uint256 amount) external onlyOwner{
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 stakingTokens = contractTokenBalance.mul(stakingFee).div(totalFee);
            super._transfer(from, _stakingWalletAddress , stakingTokens);

            uint256 charityTokens = contractTokenBalance.mul(charityFee).div(totalFee);
