...
contract DeployerFactory {
  ...
  function deploy(uint256 salt, bytes calldata bytecode) public {
    ...
    bytes memory metamorphicCode =
      (hex"000000000000000000000000828954676f2634d404251f05e4f619ff83f7eceb");

    // determine the address of the metamorphic contract.
    address metamorphicContractAddress =
      _getMetamorphicContractAddress(salt, metamorphicCode);
          ...
          uint256( // convert to uint to truncate upper digits.
            keccak256( // compute the CREATE2 hash using 4 inputs.
              abi.encodePacked( // pack all inputs to the hash together.
                ...
                keccak256(abi.encodePacked(metamorphicCode)) // the init code hash.
              )
            )
          )
        )
