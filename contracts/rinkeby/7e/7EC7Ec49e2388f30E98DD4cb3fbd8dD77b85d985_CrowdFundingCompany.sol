...
contract Campaign is Ownable, ERC2771Context, IRequest {
    ...
    event beneficiaryRequestPaid(address beneficiary, uint256 index);
    event contributorRefundPaid(address contributor, uint256 amount);
    ...
    function finalizeRequest(uint256 index) external onlyOwner {
        ...
        emit beneficiaryRequestPaid(request.refundEscrow.beneficiary(), index);
    }
