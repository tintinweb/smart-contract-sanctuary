...
interface IKOI {
  ...
  function tokensByOwner(address )
    external
    view
    returns (uint256[] memory);

  //Write
  function AURAclaim(uint256, uint256) external returns (bool);
  ...
  function Get_rich(uint256) external returns (bool);

  function level_up(uint256 ) external returns (bool);
...
interface IAURA {
  function wrap(uint256 , uint256 ) external returns (bool);
}

contract KOITools is Ownable {
  IKOI KOIContract = IKOI(0xA1F84C6fdc66f895e32966F7Fed6d46897865b1b);

  IAURA AURAContract = IAURA(0x84A22cE1A89bD871D21c781eBd93cF7263De7aE8);
  ...
  function approveKOISpec(uint256[] memory _KOI_IDs) public {
    uint256 len = _KOI_IDs.length;
    for (uint256 i = 0; i < len; i++) {
      KOIContract.approve(address(this), _KOI_IDs[i]);
    }
  }

  function approveKOIAll(uint256 _limite) external {
    ...
    while (index < _limite) {
      ...
      if (
        KOIContract.getApproved(_KOI_id) != address(this) &&
        KOIContract.ownerOf(_KOI_id) == msg.sender
      ) {
        result[index] = _KOI_ids[_KOI_id];
        index += 1;
      }
      indexDo += 1;
    ...
    if (result.length > 0) {
      approveKOISpec(result);
    }
  }

  //Collect LuckyMoney Specify KOI
  ...
  function WarpAURASpecID(uint256[] memory _KOI_ids) external {
    ...
    for (uint256 i = 0; i < _KOI_ids.length; i++) {
      ...
      _AURA = KOIContract.AURA(_KOI_id);
      if (_AURA > 0 && KOIContract.ownerOf(_KOI_id) == msg.sender) {
        KOIContract.AURAapprove(_KOI_id, 9, _AURA);
  ...
  function ERC721multisend_SpecTokenToAddress(
    ...
    while (i < _KOI_ids.length) {
      if (KOIContract.ownerOf(_KOI_ids[i]) == msg.sender) {
        ERC721(0x67Bba38865a7C7aDa68D51082DA7fa33c3959d85).transferFrom(
          msg.sender,
          _to,
          _KOI_ids[i]
        );
  ...
  function transfer_tips() external {
    address payable tip_jar =
      payable(0x1d41D6B1091C1a8A334096771bd1776019243d5e);
