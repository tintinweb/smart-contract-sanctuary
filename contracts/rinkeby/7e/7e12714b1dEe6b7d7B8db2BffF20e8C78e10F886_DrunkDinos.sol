...
contract DrunkDinos is ERC721Enumerable, ERC721Burnable, Pausable, Ownable, PaymentSplitter {
    ...
    uint16 _maxPerTx = 20;
    uint16 _reservedTokenCount = 250;
    uint16 _totalMinted = 0;
    uint256 _saleStart;
    uint256 _presaleStart;
    string _baseURIValue;
    mapping(address => uint16) _freeClaimsRemaining;
    ...
    ) ERC721("Drunk Dinos", "DD") PaymentSplitter(payees, paymentShares) {
    ...
    function canAccessPresale() public pure returns (bool) {
        return true;
    }
    ...
    function remainingFreeClaims(address addr)
        ...
        returns (uint16)
    {
        return _freeClaimsRemaining[addr];
    }
    ...
    function setMaxPerTransaction(uint8 count) public onlyOwner {
        _maxPerTx = count;
    }
    ...
    modifier mintCountMeetsSupply(uint256 numberOfTokens) {
        require(
            _totalMinted.add(_reservedTokenCount).add(numberOfTokens) <=
                MAX_SUPPLY,
    ...
    function addFreeClaim(
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            _freeClaimsRemaining[addresses[i]] = claimCount[i];
        }
    }
        ...
        presaleMustBeActive
    {
        require(
            canAccessPresale(),
            "You do not have access to the presale"
        );

        _mintTokens(numberOfTokens, _msgSender());
    }

    function claimFreeMints(uint16 numberOfTokens)
        public
        mintCountMeetsSupply(numberOfTokens)
        whenNotPaused
        doesNotExceedMaxPerTransaction(numberOfTokens)
        cannotMintFromContract
    {
        ...
        _freeClaimsRemaining[_msgSender()] -= numberOfTokens;
    ...
    function mintReserved(uint16 numberOfTokens, address to) public onlyOwner {
        ...
        require(
            numberOfTokens.add(_totalMinted) <= MAX_SUPPLY,
            "Would exceed reserved supply"
        );
