  ...
  IERC721Enumerable
{
  ...
  function _batchTransfer(address from, address to, uint256 fromId, uint256 toId) internal {
    uint128 quantity = uint128(toId - fromId);
    for (uint256 tokenId = fromId; tokenId <= toId; tokenId++) {
      emit Transfer(from, to, tokenId);
    }
    require(_checkOnERC721Received(from, to, fromId, ""),"ERC721A: transfer to non ERC721Receiver implementer");
    _ownerships[fromId] = TokenOwnership(to, uint64(block.timestamp));
    if (_ownerships[toId + 1].addr == address(0x0)) {
        _ownerships[toId + 1] = TokenOwnership(from, uint64(block.timestamp));
    }
