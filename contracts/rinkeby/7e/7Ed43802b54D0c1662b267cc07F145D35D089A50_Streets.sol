...
contract Streets is ERC721, Ownable, ReentrancyGuard {
    using Strings for uint256;
    mapping (uint256 => uint256) private _salesStages;
    mapping (uint256 => bool) private _idAlreadyMint;
    ...
    uint256 public constant MINT_MAX_STREETS_PRIVATESALES = 3;
    ...
    constructor() ERC721("WipStreets", "WIPST") {
        // Sales stages -> max allowed by drop
    ...
    function setSalesStage (uint256 _stage) external onlyOwner{
        require(_stage <= MAX_SALES_STAGES, "Selected stage is outside index");
        if (_stage == 0) {
    ...
    function preSalesMint(uint256 _id) external payable nonReentrant {
        ...
        require(_totalSupply + 1 <= _maxStreets, "Exceeded max PreSales streets allowed");
        require(_id <= (_maxStreets), "ID not allowed for PreSales"); 
        require(_idAlreadyMint[_id] == false, "ID already minted");
        require(balanceOf(msg.sender) < MINT_MAX_STREETS_PRIVATESALES, "Minting max reached by the owner");
        if (owner() != msg.sender) {
    ...
    function publicSalesMint(uint256 _batch_count) external payable nonReentrant {
        ...
        uint256 _totalStreets = _totalSupply;
        uint256 _maxStreets = _salesStages[salesStage];
        require(_batch_count > 0, "Batching must be > 0");
        require(_totalStreets + _batch_count <= _maxStreets, "Exceeded max streets supply for this stage");
        require(_batch_count <= BATCH_MAX_STREETS_PUBLICSALES, "Batching max reached for the mint");
        if (owner() != msg.sender) {
...
// Verify Tokens order
// Verify last tokens URI for each drop
// Test Revert
