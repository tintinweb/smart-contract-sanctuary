...
contract PoolRegister is IPoolRegister, Managed {
    using EnumerableSet for EnumerableSet.AddressSet;
    EnumerableSet.AddressSet private pools;

    mapping(address => bool) public privates;
        ...
        returns (address[] memory result)
    {
        ...
        for (uint256 i = _offset; i < to; i++) {
            result[i - _offset] = pools.at(i);
        }
    }

    function add(address _pool, bool _isPrivate)
        external
        override
        requirePermission(CAN_ADD_POOL)
    {
        require(pools.add(_pool), "PoolRegister: pool exists");
        privates[_pool] = _isPrivate;
        emit AddPool(msg.sender, _pool, _isPrivate);
    }

    function remove(address _pool)
        external
        override
        requirePermission(ROLE_ADMIN)
    {
        require(pools.remove(_pool), "PoolRegister: pool exists");
        delete privates[_pool];
        emit RemovePool(msg.sender, _pool);
    }
}
