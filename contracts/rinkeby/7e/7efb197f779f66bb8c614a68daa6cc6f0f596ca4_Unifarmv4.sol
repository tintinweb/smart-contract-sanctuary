...
contract Unifarmv4 is Admin {
    ...
    function claimRewards(address userAddress, uint256 stakeId, uint256 stakedAmount, uint256 totalStake) internal {
        // Local variables
        uint256[2] memory interval;
        uint256[2] memory staked_Total;
        uint256 endOfProfit; 

        interval[0] = poolStartTime.add(stakeDuration);
        
        // Interval calculation
        if (interval[0] > block.timestamp) 
            endOfProfit = block.timestamp;
           
        else 
            endOfProfit = poolStartTime.add(stakeDuration);
        
        interval[0] = endOfProfit.sub(stakingDetails[userAddress].startTime[stakeId]); 
        interval[1] = endOfProfit.sub(poolStartTime);
        
        staked_Total[0] = stakedAmount;
        staked_Total[1] = totalStake;

        // Reward calculation
        if (interval[0] >= HOURS) 
            _rewardCalculation(userAddress, stakeId, staked_Total, interval);
    }
    ...
    function _rewardCalculation(
        address userAddress,
        uint256 stakeId,
        uint256[2] memory stakedAndTotal,
        uint256[2] memory interval
    ) internal {
        ...
        count[0] = interval[0].div(DAYS); 
        conCount[0] = interval[1].div(DAYS); 
        
        count[1] = interval[0].div(HOURS);
        conCount[1] = interval[1].div(HOURS);
        ...
        while (i < intervalDays.length) {
            uint256 userStakingDuration = poolStartTime.add(intervalDays[i]*DAYS);
            address rewardToken;
            
            if (conCount[0] >= intervalDays[i] && userStakingDuration >= stakingDetails[userAddress].startTime[stakeId]) {
                uint256 balHours = conCount[1].sub((intervalDays[i].sub(1)).mul(24));
                rewardToken = tokensSequenceList[stakingDetails[userAddress].tokenAddress[stakeId]][i];
                ...
                if ( rewardToken != stakingDetails[userAddress].tokenAddress[stakeId] 
                    ...
                    rewardsEarned = balHours.mul(getOneDayReward(stakedAndTotal[0], stakingDetails[userAddress].tokenAddress[stakeId], rewardToken, stakedAndTotal[1]));

                    // Referrer Earning

                    if (
                        stakingDetails[userAddress].referrer[stakeId] != address(0)) {
                        uint256 refEarned = (rewardsEarned.mul(refPercentage)).div(100 ether);
                        rewardsEarned = rewardsEarned.sub(refEarned);

                        require(IERC20(rewardToken).transfer(stakingDetails[userAddress].referrer[stakeId],refEarned), "Transfer Failed");
            ...
            else if(userStakingDuration <= stakingDetails[userAddress].startTime[stakeId]) {
                ...
                if ( rewardToken != stakingDetails[userAddress].tokenAddress[stakeId] 
                    ...
                    rewardsEarned = count[1].mul(getOneDayReward(stakedAndTotal[0], stakingDetails[userAddress].tokenAddress[stakeId], rewardToken, stakedAndTotal[1]));
                    // Referrer Earning
