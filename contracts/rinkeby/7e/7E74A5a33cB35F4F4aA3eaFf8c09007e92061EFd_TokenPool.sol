...
contract TokenPool is ITokenPool, IERC20 {
        ...
        override
    {
        ...
        if (stakedTokenTotal > 0) {
            rewardsPerTokenPoints = rewardsPerTokenPoints + (currentRewards * 1e18 * pointMultiplier / stakedTokenTotal);
        }
    ...
    function claimRewards(address _user) public override {
        ...
        for(uint i = 0; i < rewardTokens.length; i++) {
            ...
            // GP: TODO Think about safe transfer
            IERC20(rewardTokens[i]).transfer(_user, payableAmount);
