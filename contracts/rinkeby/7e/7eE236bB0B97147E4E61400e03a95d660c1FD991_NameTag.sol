...
//contract NameTag is ERC721Enumerable, Ownable, StringUpper, DenyList, AllowList, ReentrancyGuard {
...
//        todo check invalid address
    ...
    function _preValidatePurchase(uint256 tokensAmount) internal view {
        ...
        if (presaleActive) {
            require(_remainingPresaleTime() > 0, "NT: Presale is over");
            require(inAllowList(msg.sender), "NT: Address isn't in the allow list");
            require(tokensAmount + presalePurchasedAmount[msg.sender] <= presaleLimit, "NT: Presale, limited amount of tokens");
            require(presalePrice * tokensAmount <= msg.value, "NT: Presale, insufficient funds");
        } else {
    ...
    function buyTokens(string[] memory _names) external payable whenAnySaleActive nonReentrant returns (uint256[] memory) {
        ...
        if (presaleActive) {
            presalePurchasedAmount[msg.sender] += _names.length;
        }
...
//        todo
//        (bool success, ) = wallet.call.value{value:amount}("");
//        require(success, "NT: Withdraw failed.");
    }
