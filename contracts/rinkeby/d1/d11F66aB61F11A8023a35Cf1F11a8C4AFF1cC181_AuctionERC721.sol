...
contract AuctionERC721 {
     ...
     */
    function finalizeAuction(uint256 _auctionId, uint256 _endDate, uint256 listingType) public {
        ...
        if (bidsLength == 0) {
            ...
            uint256 singlePrice = getSinglePrice(listingType,lastBid.totalAmount);
     ...
     */
    function bidOnUnlimitedAuction(uint256 _auctionId, uint256 listingType) public payable {
      uint256 ethAmountSent = getSinglePrice(listingType,msg.value);
     ...
     */
    function bidOnTimedAuction(uint256 _auctionId, uint256 listingType) public payable {
        uint256 ethAmountSent = getSinglePrice(listingType,msg.value);
     ...
     */
    function fixedPriceAuction(uint256 _auctionId, uint256 listingType) public payable {

        Auction memory myAuction = auctions[_auctionId];
				uint256 singlePrice = getSinglePrice(listingType,myAuction.startPrice);
		...
		function getSinglePrice(uint256 listingType,uint256 amount) public view returns (uint256) {
				uint256 serviceFeePercent;
        if(listingType == 1){
					serviceFeePercent = 250;
				} else{
					serviceFeePercent = 2000;
				}
				uint256 singlePrice = amount.mul(10000).div(10000 + serviceFeePercent);
				return singlePrice;
    }
}
