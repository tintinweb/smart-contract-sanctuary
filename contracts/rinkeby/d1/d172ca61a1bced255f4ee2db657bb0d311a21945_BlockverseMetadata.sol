...
contract BlockverseMetadata is IBlockverseMetadata, Ownable {
    ...
    function tokenURI(uint256 tokenId, IBlockverse.BlockverseFaction faction) external view override returns (string memory) {
        faction = getTokenFaction(tokenId);
    ...
    function getTokenFaction(uint256 tid) public view returns (IBlockverse.BlockverseFaction faction) {
        uint256 seed = getTokenSeed(tid);

        if (seed == 0) {
            faction = IBlockverse.BlockverseFaction.UNASSIGNED;
        } else if (blockverse.getTokenFaction(tid) == IBlockverse.BlockverseFaction.UNASSIGNED) {
            faction = IBlockverse.BlockverseFaction(((seed >> (5 * 16)) % 6) + 1);
        } else {
            faction = blockverse.getTokenFaction(tid);
        }
    }
    ...
    function setContracts(address _blockverse) external onlyOwner {
