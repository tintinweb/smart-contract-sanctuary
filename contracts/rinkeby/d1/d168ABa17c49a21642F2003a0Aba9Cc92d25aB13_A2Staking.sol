...
contract A2Staking is Context, Ownable, ReentrancyGuard {
    ...
    event Staked(uint256 indexed pool, address indexed beneficiary, uint256 stake, uint256 stakeAmount, uint256 period);
    ...
    function addPool(IERC20 token, uint256 minAmount, uint256 cliff, uint256 duration, uint256 penalty) external onlyOwner returns(uint256) {
    ...
    function stakeInternal(address beneficiary, uint256 poolId, uint256 amount) internal {
        ...
        require(amount >= pool.minAmount, "low amount");
    ...
    function calculateUnstakePenalty(Pool storage pool, Stake storage stakee, uint256 unstakeAmount) internal view returns(uint256) {
        ...
        if (timePassed >= pool.cliff) return 0;
        if(timePassed >= pool.duration) return unstakeAmount; //unstake is prohibited
