...
contract Vesting is Initializable {
    ...
    uint256 constant TOTAL_AMOUNT = 500e27; // Total FAST : 500b
    uint256 constant PRESALE_ALLOC = 100e27; // FAST allocation for presale : 100b
    uint256 constant INFLUENCER_ALLOC = 100e27; // FAST allocation for influencers: 100b
    uint256 constant TEAM_ALLOC = 150e27; // FAST allocation for team : 150b
    uint256 constant UNLOCK_UNIT = 10; // 10% of the total allocation will be unlocked
    uint256 constant INITIAL_LOCK_PERIOD = 6 weeks; // No tokens will be unlocked for the first month
    ...
    function initialize(address _fastToken, address _presaleContract, address payable _multiSigAdmin) public initializer {
        owner = msg.sender;

        fastToken = IFastToken(_fastToken);
        presaleContract = _presaleContract;
        multiSigAdmin = _multiSigAdmin;
        
        /// Allow presale contract to withdraw Unsold fastToken to multiSig admin
        fastToken.approve(presaleContract, MAX_UINT256);
    ...
    function withdrawToken(address recipient) external returns (uint256) {
        ...
        require(fastToken.transfer(recipient, _withdrawable));
    ...
    function getVested(address beneficiary) public view virtual returns (uint256 _amountVested) {
        ...
        uint256 firstVestingStart = startTime.add(INITIAL_LOCK_PERIOD);

        if (_vestingSchedule.vestingPlan == 0) { // Presale
            uint256 secondVestingStart = firstVestingStart.add(20 weeks);
            if (block.timestamp > firstVestingStart && block.timestamp <= secondVestingStart) {
                vestedPercent = (block.timestamp - firstVestingStart).div(20 weeks) * 100;
            } else if (block.timestamp > secondVestingStart) {
                vestedPercent = 100;
            }
        } else if (_vestingSchedule.vestingPlan == 1) { // Influencer
            uint256 secondVestingStart = firstVestingStart.add(24 weeks);
            
            if (block.timestamp > firstVestingStart && block.timestamp <= secondVestingStart) {
                vestedPercent = (block.timestamp - firstVestingStart).div(24 weeks).mul(UNLOCK_UNIT);
            } else if (block.timestamp > secondVestingStart) {
                vestedPercent = 10 + (block.timestamp - secondVestingStart).div(4 weeks).mul(UNLOCK_UNIT);
            }
        } else if (_vestingSchedule.vestingPlan == 2) { // Team
            uint256 secondVestingStart = startTime.add(24 weeks);
            if (block.timestamp > startTime && block.timestamp <= secondVestingStart) {
                vestedPercent = (block.timestamp - startTime).div(24 weeks) * 100;
            } else if (block.timestamp > secondVestingStart) {
                vestedPercent = 100;
            }
        }
