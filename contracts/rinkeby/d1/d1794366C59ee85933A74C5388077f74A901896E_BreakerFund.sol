...
contract BreakerFund {
     ...
     */
    function _calcReward(address _for) private view returns(uint) {
     ...
     */
    function _calcCoinReward(address _for) private view returns(uint) {
        return IERC20(token).balanceOf(_for).mul(
    ...
    function withdrawReward() public returns(uint) {
        uint value = _calcReward(msg.sender).add(owedETH[msg.sender]);
    ...
    function withdrawCoinReward() public returns(uint) {
        uint value = _calcCoinReward(msg.sender).add(owedCoins[msg.sender]);
    ...
    function freezeReward(address _for) external nonZeroAddress(_for) returns(uint) {
        uint value = _calcReward(_for);
        if (value > 0) {
            rewardETH[_for] = totalRewardETH;
            owedETH[_for] = owedETH[_for].add(value);
        }
        return value;
    }
    ...
    function freezeCoinReward(address _for) external nonZeroAddress(_for) returns(uint) {
        uint value = _calcCoinReward(_for);
        if (value > 0) {
            rewardCoins[_for] = totalRewardCoins;
            owedCoins[_for] = owedCoins[_for].add(value);
        }
        return value;
    }
