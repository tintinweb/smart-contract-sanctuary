...
contract EnsMapper is Ownable {
    ...
    string public domainLabel = "testing123";
    ...
    function setDomain(string calldata label, uint256 token_id) public {
        require(nft.ownerOf(token_id) == msg.sender || owner() == msg.sender, "not owner of token");
        require(tokenHashmap[token_id] == 0x0, "Token has already been set");
        require(address_whitelist[msg.sender] || publicClaimOpen || owner() == msg.sender, "address not currently on the whitelist");
        require(labelToId(label) == 0, "Label is currently being used");
        require(owner() == msg.sender || getAllCatsWithDomains(msg.sender).length == 0, "sub-domain already exists in this wallet");
        ...
        if(address_whitelist[msg.sender] && !publicClaimOpen){
            address_whitelist[msg.sender] = false;
        }
      
    }

    function getDomainHash() public view returns (bytes32 namehash) {
            namehash = 0x0;
    ...
    function name(bytes32 node) view public returns (string memory){
        return string(abi.encodePacked(hashToDomainMap[node], ".", domainLabel, ".eth"));
    }
    ...
    function getClaimableIdsForAddress(address addy) public view returns(uint256[] memory){
        if(address_whitelist[addy] || publicClaimOpen || owner() == msg.sender){
