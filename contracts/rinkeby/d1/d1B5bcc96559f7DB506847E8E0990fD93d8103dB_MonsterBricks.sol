...
contract MonsterBricks is ERC1155, ERC1155Burnable, ERC1155Supply, Ownable, Pausable, ReentrancyGuard {
  ...
  mapping(uint256 => string) internal brickNames;
  ...
  function createNewRelease(string memory _name) public onlyOwner {
    _tokenIds.increment();
    brickNames[_tokenIds.current()] = _name;
  }
  ...
  constructor(string memory _baseURI) ERC1155(_baseURI) {
    ...
    currentMerkleRoot = 0x2588688e60c2492a6ba516126e3ac2eebfa532d1c78b47458cbe721d7782a1cf;
  }

  function uri(uint256 _tokenId) public view override returns (string memory) {
    require(_tokenId <= _tokenIds.current(), "URI requested for invalid token");
    return
  ...
  function claimBrick(address _account, uint256 _quantity, bytes32[] calldata _proof) public whenNotPaused nonReentrant {
    ...
    require(lastTokensClaimed[_account] < _tokenIds.current());

    lastTokensClaimed[_account] = _tokenIds.current();

    _mint(_account, _tokenIds.current(), _quantity, "");
  }
