...
contract TokenVesting is Context,Ownable{
    ...
    // uint256 public releaseCycle = 1 days;
    
    // // @notice rate of release 0.5%/1Days
    // uint256 public releaseRate = 0.0005e18;

    // uint256 public awardLowRate = 0.2e18;
    // uint256 public awardMiddleRate = 0.3e18;
    // uint256 public awardHighRate = 0.5e18;

    // uint256 public awardLowAmount = 5000e18;
    // uint256 public awardMiddleAmount = 10000e18;
    // uint256 public awardHighAmount = 20000e18;

    // /// @notice cycle of award 200 Days
    // uint256 public awardCycle = 200 days;


        /// @notice cycle of release 1 Days
    uint256 public releaseCycle = 5;
    ...
    uint256 public awardLowAmount = 50e18;
    uint256 public awardMiddleAmount = 100e18;
    uint256 public awardHighAmount = 200e18;

    /// @notice cycle of award 200 Days
    uint256 public awardCycle = 1000;
    ...
    /// @notice event of LockUp
    event LockUp(
      uint256 _amountIn
    ); 
    
    /// @notice event of UnLock
    event UnLock(
      uint256 _amountOut
    ); 
    ...
    function lockUp(uint256 _amountIn) public returns (bool){
      upDataRelease(true);
      ...
      //add lockUpAmount
      tokenVestUsers[msg.sender]._lockUpAmount = tokenVestUsers[msg.sender]._lockUpAmount.add(_amountIn);
      
      emit LockUp(_amountIn);
      return true;
    ...
    function unlock() public returns(uint256){
      require(tokenVestUsers[msg.sender]._lockUpAmount > 0,"TokenVesting-unlock/lockUpAmount-error");
      upDataRelease(false);
      uint amountOut = tokenVestUsers[msg.sender]._unlockWaitingAmount.add(tokenVestUsers[msg.sender]._awardAmount);
      require(amountOut > 0,"TokenVesting-unlock/unlockAmount-error");
      require(IERC20(token).balanceOf(address(this)) > 0,"TokenVesting-unlock/balance-error");

      //transfer token to msg sender
      require(IERC20(token).transfer(msg.sender,amountOut),"TokenVesting-unlock/transfer-failed");   
      emit UnLock(amountOut);
      if(!clearLockParams()){
        tokenVestUsers[msg.sender]._unlockWaitingAmount = 0;
    ...
    function clearLockParams() private returns(bool){
       if(tokenVestUsers[msg.sender]._unlockAmount >= tokenVestUsers[msg.sender]._lockUpAmount){
    ...
    function upDataRelease(bool isUpDataCreateTimestamp) private returns(uint256,uint256) {
      ...
      //If all of them are released,do not update
      if(tokenVestUsers[msg.sender]._lockUpAmount == 0){
        return (0,0);
      }
      ...
      if(releaseDay >= 1){
        indexReleaseDeltaunlockReleaseAmount = FixedPoint.multiplyUintByMantissa(tokenVestUsers[msg.sender]._lockUpAmount,releaseDay.mul(releaseRate));
        if(tokenVestUsers[msg.sender]._unlockAmount < tokenVestUsers[msg.sender]._lockUpAmount){

          tokenVestUsers[msg.sender]._unlockAmount = tokenVestUsers[msg.sender]._unlockAmount.add(indexReleaseDeltaunlockAwardAmount);
          tokenVestUsers[msg.sender]._unlockWaitingAmount = tokenVestUsers[msg.sender]._unlockWaitingAmount.add(indexReleaseDeltaunlockReleaseAmount);

          if(tokenVestUsers[msg.sender]._unlockAmount > tokenVestUsers[msg.sender]._lockUpAmount){
            tokenVestUsers[msg.sender]._unlockAmount = tokenVestUsers[msg.sender]._lockUpAmount;
          }
        }
      ...
      if(newAwardSeconds >= awardCycle){
        if(tokenVestUsers[msg.sender]._lockUpAmount >= awardHighAmount){
          indexReleaseDeltaunlockAwardAmount =  FixedPoint.multiplyUintByMantissa(tokenVestUsers[msg.sender]._lockUpAmount,awardHighRate);
        }else if(tokenVestUsers[msg.sender]._lockUpAmount >= awardMiddleAmount){
          indexReleaseDeltaunlockAwardAmount =  FixedPoint.multiplyUintByMantissa(tokenVestUsers[msg.sender]._lockUpAmount,awardMiddleRate);
        }else if(tokenVestUsers[msg.sender]._lockUpAmount >= awardLowAmount){
          indexReleaseDeltaunlockAwardAmount =  FixedPoint.multiplyUintByMantissa(tokenVestUsers[msg.sender]._lockUpAmount,awardLowRate);
        }
 
        tokenVestUsers[msg.sender]._awardAmount = tokenVestUsers[msg.sender]._awardAmount.add(indexReleaseDeltaunlockAwardAmount);
        tokenVestUsers[msg.sender]._awardCreateTimestamp = tokenVestUsers[msg.sender]._awardCreateTimestamp.add(awardCycle);
      }
