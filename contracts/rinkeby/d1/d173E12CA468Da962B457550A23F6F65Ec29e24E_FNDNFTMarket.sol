...
import "./mixins/FNDFeeManagerNode.sol";
  ...
  NFTMarketReserveAuction
{
  ...
  function adminUpdateConfig(
    ...
    _updateFNDFeeManagerNode(fndFeeManager);
  }
...
import "@openzeppelin/contracts-upgradeable/utils/AddressUpgradeable.sol";
import "../interfaces/IFNDFeeManager.sol";

import "./FoundationTreasuryNode.sol";
import "./SendValueWithFallbackWithdraw.sol";

abstract contract FNDFeeManagerNode is FoundationTreasuryNode, SendValueWithFallbackWithdraw {
  using AddressUpgradeable for address;

  IFNDFeeManager private feeManager;

  event FNDFeeManagerNodeUpdated(address indexed feeManager);

  function _updateFNDFeeManagerNode(address _feeManager) internal {
    require(
      _feeManager == address(0) || _feeManager.isContract(),
      "FNDFeeManagerNode: Address provided is not a contract"
    );
    feeManager = IFNDFeeManager(_feeManager);
    emit FNDFeeManagerNodeUpdated(_feeManager);
  }

  function _distributeFNDFees(
    address nftContract,
    uint256 tokenId,
    ...
    if (address(feeManager) == address(0)) {
      ...
      feeManager.distributeFees{ value: foundationFee }(nftContract, tokenId, seller, isPrimarySale);
    }
  }
  ...
  FNDFeeManagerNode
{
    ...
    )
  {
    ...
    isPrimarySale = _getIsPrimary(nftContract, tokenId, creator, seller);
    if (isPrimarySale) {
    ...
    )
  {
    ...
    address payable ownerRevTo;
    bool isPrimarySale;
    (foundationFee, creatorFeeTo, creatorFee, ownerRevTo, ownerRev, isPrimarySale) = _getFees(
      nftContract,
      tokenId,
      seller,
      price
    ...
    _distributeFNDFees(nftContract, tokenId, seller, isPrimarySale, foundationFee);
