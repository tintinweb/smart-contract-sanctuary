...
contract StablePool {
    ...
    function check( bool _add, address _token, uint _amount ) public view returns ( bool ) {
        ...
        if( _add ) { // use upper bound
            ...
            uint maximum = totalTokens.mul( tokenInfo[ _token ].highAP ).div( 1e5 ); // maximum balance of token
            ...
            uint minimum = totalTokens.mul( tokenInfo[ _token ].lowAP ).div( 1e5 ); // minimum balance of token
            uint balance = IERC20( _token ).balanceOf( address(this) ); // current balance of token
