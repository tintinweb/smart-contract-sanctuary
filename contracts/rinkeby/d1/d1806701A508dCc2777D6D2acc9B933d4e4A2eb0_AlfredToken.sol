...
// Идеи:
//      - Сбор денег, любой человек может оргранизовать сбор денег

struct CollectingMoney{
    uint TargetAmount;
    uint CurrentAmount;
}
...
contract AlfredToken{
    ...
    function AddMoneyCollect(uint _TargetAmount) public payable returns(uint){
        collects.push();
        collects[collects.length - 1].TargetAmount = _TargetAmount;
        collects[collects.length - 1].CurrentAmount = 0;
        return collects.length - 1; // возвращает номер сбора
    }
    
    function AddMoneyToCollect(uint collectNumber, uint count) public payable{
        require(balances[msg.sender] >= count);
        collects[collectNumber].CurrentAmount += count;
        balances[msg.sender] -= count;
        
        if(collects[collectNumber].CurrentAmount > collects[collectNumber].TargetAmount){
            balances[msg.sender] += collects[collectNumber].CurrentAmount - collects[collectNumber].TargetAmount;
        }
    }
    
    function GetDataFromCollect(uint collectNumber) public view returns(uint, uint){
        return(collects[collectNumber].TargetAmount, collects[collectNumber].CurrentAmount);
    }
