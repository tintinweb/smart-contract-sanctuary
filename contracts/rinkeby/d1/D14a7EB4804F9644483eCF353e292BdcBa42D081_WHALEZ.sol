...
contract WHALEZ is ERC721URIStorageUpgradeable, AccessControlEnumerableUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    // mapping for logging Podz to struct with corresponding WHALEZ metadata URI and WHALEZ ID 
        ...
        returns (uint256[] memory _ids) 
    {
        ...
        string[] memory _chestURIs = new string[](numberOfChests);

        for (uint i = 0; i < numberOfChests; i++){
            _idsArray[i] = chestIds.current(); 
            setChesttoWhale(_idsArray[i]);   
            chestIds.increment();     
        }

        for (uint i = 0; i < numberOfChests; i++){
            _chestURIs[i] = chestURI;        
        }
    ...
    function setChestId() internal virtual {
        chestIds._value = 100000;
    }

    function setChestURI(string memory _chestURI) internal virtual {
        chestURI = _chestURI;
    }

    function setChesttoWhale(uint256 _chestId) internal virtual {
