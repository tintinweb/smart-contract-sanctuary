...
contract AniftySale is ERC1155Holder {
    ...
    event CancelSale(uint256 saleId, uint256 endTimestamp, address seller);
    ...
    modifier editSaleModifier(uint256 _saleId) {
        require(sales[_saleId].seller == msg.sender, "MP: Not owner");
        require(sales[_saleId].endTimestamp == 0, "MP: Sale ended");
        _;
    }
    ...
    modifier onlyAdminOrOwner() {
        require(
            ...
            "MP: Not admin or owner"
        );
        _;
    }

    modifier onlyAdmin() {
        require(admin == msg.sender, "MP: Not admin");
        _;
    }
    ...
    function listNewSale(
        ...
        require(_price > 0, "MP: Price > 0");
        require(_amount > 0, "MP: Amount > 0");
        require(supportedTokens[_token], "MP: Invalid token");
    ...
    // Cancel sale of NFT
    function cancelSale(uint256 _saleId) external editSaleModifier(_saleId) {
    ...
    function buyFromSale(uint256 _saleId, uint256 _amount) external payable {
        ...
        require(sale.seller != address(0), "MP: !sale");
        require(_amount > 0, "MP: Amount > 0");
        require(sale.endTimestamp == 0, "MP: Sale cancelled/ended");
        require(_amount <= sale.amount, "MP: !amount");
        ...
        if (sale.token == address(0)) {
            require(msg.value >= paymentAmount, "MP: Payable value too low");
            if (msg.value > paymentAmount)
                msg.sender.transfer(msg.value.sub(paymentAmount));
            // Anifty takes commission for every sale
            uint256 commissionAmount = paymentAmount.mul(commissionFee).div(
                PRECISION
            );
            // Artists get royalty for every sale
            uint256 royaltyAmount = paymentAmount
            .mul(artistRoyalties[sale.tokenId].royalty)
            .div(PRECISION);
            // Transfer seller the price - commission fee
            sale.seller.transfer(
                paymentAmount.sub(commissionAmount).sub(royaltyAmount)
            );
            // Transfer commission wallet the commission fee
            commissionWallet.transfer(commissionAmount);
            if (royaltyAmount > 0) {
                artistRoyalties[sale.tokenId].wallet.transfer(royaltyAmount);
            }
        } else {
            ...
            if (royaltyAmount > 0) {
                token.transferFrom(
                    msg.sender,
                    artistRoyalties[sale.tokenId].wallet,
                    royaltyAmount
                );
            }
        }
    ...
    function setOwners(address[] memory _owners, bool _set) external onlyAdmin {
