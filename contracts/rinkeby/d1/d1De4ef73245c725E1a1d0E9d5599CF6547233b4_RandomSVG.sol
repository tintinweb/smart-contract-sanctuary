...
contract RandomSVG is ERC721URIStorage, VRFConsumerBase {
    ...
    function generateSVG(uint256 _randomNumber) public view returns (string memory finalSVG) {
        //this is where I need to modify the code to generate multiple random shapes as per my imagination
        //use the expand function from https://docs.chain.link/docs/chainlink-vrf-best-practices/ to be able to generate many random numbers for the different shapes and colours
        //have if statements with functions calls in them 
        //function calls to go to functions containing for loops for each shape type
    ...
    function generatePath(uint256 _randomNumber) public view returns(string memory pathSvg) {
        //work out max number of path commands
        uint256 numberOfPathCommands = (_randomNumber % maxNumberOfPathCommands) +1;
        //strat the path for this turn of the loop from the above loop in generateSVG
        pathSvg = "<path d='";
        //run a new loop with the actual path generation
        for(uint i=0; i < numberOfPathCommands; i++){
            //generate an entirely new random number - so not like the one above
            uint256 newRNG = uint256(keccak256(abi.encode(_randomNumber, size + i)));
            //request a new path command to be generated - either M or L as those are the two in the array
            string memory pathCommand = generatePathCommand(newRNG);
            //add the path the the current path command
            pathSvg = string(abi.encodePacked(pathSvg, pathCommand));
        }
        //pick a colour for this path
        string memory color = colors[_randomNumber % colors.length];
        //add the path and the colour to the svg
        pathSvg = string(abi.encodePacked(pathSvg, "' fill='transparent' stroke='", color, "'/>"));
    }
    
    */
    // need to pass this funtion the original random number and then the number of random numbers I need - may be useful
    ...
    // 2 replaced:
    function generateShape(uint256 _randomNumber) public view returns(string memory pathSvg) {
        //need to work out a shape
        pathSvg = "";
        if (_randomNumber % 2 == 0) {
            //draw a circle
            //need to work out how the circle and square sides will not exceed canvas - maybe if coordinate plus radius is bigger than size, subtract radius from coordinate etc...
            uint256[] memory circleVars = expand(_randomNumber, 6);
            pathSvg = string(abi.encodePacked(pathSvg, "<circle cx='", uint2str((circleVars[0] % size)), "' cy='", uint2str((circleVars[1] % size)), "' r='", uint2str((circleVars[2] % (size / 2))), "' stroke='#", hexColor(circleVars[3]), "' stroke-width='", uint2str((circleVars[4] % (size / 5))), "' fill='#", hexColor(circleVars[5]), "' />"));
            //6 random numbers needed - x, y, radius, stroke width, stroke colour, fill colour
        } else {
            //draw a square
    ...
    */
    ...
    //function to create the image uri needed by the minter function above - this just creates the image that will form part of the token uri
    ...
    //function to create the token uri using the image uri
