...
contract UnclePortals is ERC1155Supply, Ownable, ReentrancyGuard {
  ...
  mapping(uint256 => TokenBalance) public tokenBalances;
  ...
  function toggleTrading(uint256[] calldata _tokenIds, bool _status) external onlyOwner {
    for (uint256 i = 0; i < _tokenIds.length; i++) {
  ...
  function withdraw(uint256 _tokenId) public onlyOwner nonReentrant {
    require(tokenBalances[_tokenId].balance > 0, "Not enough balance");
    TokenBalance storage tokenBalance = tokenBalances[_tokenId];
    for (uint256 i = 0; i < tokenBalance.paymentRatios.length; i++) {
      uint256 balance = (tokenBalance.balance * tokenBalance.paymentRatios[i]) / 1000;
  ...
  function _beforeTokenTransfer(
    ...
    for (uint256 i = 0; i < ids.length; i++) {
      ...
      tokenBalances[tokenId].balance += msg.value;
