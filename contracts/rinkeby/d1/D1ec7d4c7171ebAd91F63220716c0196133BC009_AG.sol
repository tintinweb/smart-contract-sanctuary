...
contract AG is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public maxMintable = 2; //Max mintable per wallet in public sale is 5
    ...
    uint256 public constant SALE_PRICE = 55000000000000000; // 0.05 eth
    ...
    constructor(string memory _tokenName, string memory _tokenSymbol, string memory __baseURI, address __signer) ERC721(_tokenName, _tokenSymbol) {
    ...
    // Only 100 can be minted in presale, Need to discuss
    // Only 3 per account is allowed to be minted in presale
    function preSaleMint(uint256 amount, 
        ...
        external payable{
            ...
            //uint256 rp = remainingPresale;
            require(mp + amount <= maxMintable, "exceeds max tokens per wallet"); //!remove the comment
            require(msg.value >= SALE_PRICE * amount, "incorrect ether value send");
            bytes32 data = keccak256(abi.encodePacked(msg.sender, max));
            address signer = ECDSA.recover(data, signature);
            require(signer == _signer, "invalid signature");

            //require(rp >= amount, "exceeds max presale supply");
            //remainingPresale = rp - amount;
