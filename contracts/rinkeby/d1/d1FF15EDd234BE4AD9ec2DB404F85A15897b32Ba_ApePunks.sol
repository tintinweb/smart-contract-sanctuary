...
contract ApePunks is MyERC721Enumerable, Ownable, ERC721Burnable {
    ...
    HodlGang private _hodlGang;
    mapping (address => bool) private _walletClaim;

    event CreatePunk(uint256 indexed id);
    constructor(string memory baseURI) ERC721("ApePunks", "APK") {
        setBaseURI(baseURI);
    }
    ...
    function mintHodlGang() public  saleIsOpen {
        address _to = _msgSender();
        uint256 total = _totalSupply();
        require(total <= MAX_ELEMENTS, "Sale end");
        if(_walletClaim[_to] == false && _hodlGang.walletOfOwner(_to).length > 0){
    ...
    function setHodlGang(address hodlGang) external onlyOwner {
        _hodlGang = HodlGang(hodlGang);
    }
}
