...
contract FundMe {
    ...
    function checkBalance() public view returns(uint256){
        return getConversionRate(addressToAmountFunded[msg.sender]);
    }

    function fund() public payable {
        uint256 mimimumUSD = 50;
        require(
    ...
    function getEntranceFee() public view returns (uint256) {
        // price = 2000USD * 10^8 unit = 10^18 wei = 1ETH
        // 50USD = 50 * 10^18 / 2000
        uint256 minimumUSD = 50;
        uint256 priceDecimal = 10**8;
        uint256 price = getPrice();
        uint256 precision = 1 * 10**18; //to wei
        return minimumUSD * precision/ (price/priceDecimal) ; //need * precision first, no decimals in Solidity
