...
contract Protokol is Ownable, IProtokol {
    ...
    function isProjectOwner(uint256 _id) public view override returns (bool) {
        return projects[_id].projectOwner == _msgSender();
    }

    function isKOL(uint256 _id) public view override returns (bool) {
        return kols[_id].walletAddr == _msgSender();
    }

    function isCampaignActive(uint256 _id) public view override returns (bool) {
        return campaigns[_id].active;
    }

    function isCampaignKOL(uint256 _id) public view override returns (bool) {
        return campaignKols[_id].walletAddr == _msgSender();
    }
    ...
    function registerCampaign(
        ...
        require(_dateRange != 0, "invalid dateRange");
...
interface IProtokol {
    ...
    function isProjectOwner(uint256 _id) external view returns (bool);

    function isKOL(uint256 _id) external view returns (bool);

    function isCampaignActive(uint256 _id) external view returns (bool);

    function isCampaignKOL(uint256 _id) external view returns (bool);
