...
contract KittyPartyFactory {
     ...
     event KittyLive(address indexed kreator,address[] kittens, address kitty);
    ...
    function createKitty(address[] memory memberAddresses, uint256 _amountPerRound, uint8 _roundDuration)
    external
    returns (address kittyAddress) {
        address kitty = Clones.clone(tomCatContract);
        KittyParty(kitty).initialize(memberAddresses, _amountPerRound, _roundDuration);
        //Add to the kitten list
        myKitties[msg.sender].push(kitty);
 
        emit KittyLive(msg.sender, memberAddresses, kitty);
        return kitty;
     }
     
     function getMyKitties() external view returns(address[] memory){
         return myKitties[msg.sender];
     }
