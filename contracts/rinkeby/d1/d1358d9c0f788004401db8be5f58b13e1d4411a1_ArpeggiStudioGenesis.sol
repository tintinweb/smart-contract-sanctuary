...
contract ArpeggiStudioGenesis is ERC721Pausable, Ownable {
    using ECDSA for bytes32;
    using SafeMath for uint256;

    /**
     * @dev Maximum number of pressings per pass.
     */
    uint256 public constant MAX_PASSES = 512;

    uint256 public numMinted = 0;

    // uint256 public constant PASS_MINT_PRICE = 0.15 ether;
    uint256 public constant PASS_MINT_PRICE = 0.05 ether;

    address payable public labAddress;

    constructor() ERC721("ArpeggiStudioPassGenesis", "ARPEGGILABS") {
     ...
     */
    uint256 public constant CREATIVE_RESERVE = 100;

    /**
     * @dev Allow minting of the genesis passes.
     */
    function mintGenesis() external onlyOwner whenPaused {
        require(numMinted < CREATIVE_RESERVE, "Don't be greedy");
        _splitFunds();
    ...
    function pressSong(uint256 _tokenId, bytes calldata _composition, string calldata _title, string calldata _artist) external payable {
        ...
        _pressSongToken(_tokenId, _title, _artist, _composition);
    }

    function _pressSongToken(uint256 _tokenId, string calldata _title, string calldata _artist, bytes calldata _composition) internal returns(uint256 _singleToken){
     ...
     */
    string private _basePassExternalUrl = "http://arpeggi.io/metadata/pass/";
    /**
     ...
     * @dev Base URL for external_url metadata field.
     */
    string private _baseSingleExternalUrl = "http://arpeggi.io/metadata/song/";
    /**
     * @dev Set the base URL for external_url metadata field.
     */
    ...
    function tokenURI(uint256 _tokenId) public override view returns (string memory) {
        if (Passes[_tokenId].mintTime == 0) {
            ...
            return string(abi.encodePacked(_baseSingleExternalUrl, Strings.toString(_tokenId)));
        }
    }
