...
contract CoinDeedFactory is ICoinDeedFactory, AccessControl {
    ...
    uint256 public override maxPriceDrop = 8_000;
    uint256 public override liquidationBonus = 300;
    ...
    function createDeed(
    ...
    ) external override onlyNotProcessing {
        ...
        uint256 borrowIndex = ILendingPool(coinDeedAddressesProvider.lendingPool()).borrowIndex(pair.tokenA);

        require(borrowIndex > 0, "LendingPool does not exist.");
    ...
    function emitBrokersEnabled() external onlyDeedOrProcessing override {
        emit BrokersEnabled(
            msg.sender
        );
    }
