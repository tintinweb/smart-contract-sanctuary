...
contract OrcBazaarZug{
    ...
    address zugAddress = 0xEB468b1c73E8D0159C163E9F0D95ABBD0B20b616;
    ERC20Like zug = ERC20Like(zugAddress);
    //ERC20 public BONESHARDS = ERC20(0x9f8f72aa9304c8b593d555f12ef6589cc3a579a2);
    ...
    function closeOfferingWithZug(bytes32 _offeringId) external {
        require(zug.balanceOf(msg.sender) >= offeringRegistry[_offeringId].price, "Not enough ZUG balance");
        ...
        zug.transfer(offeringRegistry[_offeringId].offerer,sellerPayable);
        //balances[offeringRegistry[_offeringId].offerer] += msg.value;
        emit OfferingClosed(_offeringId, msg.sender);
    } 

      function testTransfer(address _address, uint256 _amount) external {
        zug.transfer(_address, _amount);
    } 

     function approve(address _address, uint256 _amount) external {
          zug.approve(_address, _amount);
    } 


    function commission(uint _amount) internal returns(uint) {
        uint comm = _amount.div(1000).mul(25);
        zug.transfer(0xCcB6D1e4ACec2373077Cb4A6151b1506F873a1a5,comm);
        uint _sellerPayable = _amount.sub(comm);
    ...
    function zugBalance() external view returns (uint) {
        return (zug.balanceOf(0xCcB6D1e4ACec2373077Cb4A6151b1506F873a1a5));
    }
