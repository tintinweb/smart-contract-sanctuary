...
contract paymentContract is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable  {
  ...
  function commitMint_headDAO(uint16[] calldata daotokenIds, bool stake) external payable whenNotPaused {
    address msgSender = _msgSender();
    uint256 minted = ERC721Contract.minted() + pendingMintAmt;
    uint256[] memory deposits = HeadDAOStaking.depositsOf(msgSender);
    uint256 amount = daotokenIds.length;

    require(deposits.length > 0 || whitelists[msgSender], "You are not whitelsited");
    ...
    for (uint i = 0; i < amount; i++) {
      ...
      if (!whitelists[msgSender]) {
        for (uint j; j < deposits.length; j++) {
          if (deposits[j] == daoTokenID) owned = true;
        }
        require(owned,"You dont own this token id");
      }
    ...
    if (whitelists[msgSender]) whitelists[msgSender] = false;
