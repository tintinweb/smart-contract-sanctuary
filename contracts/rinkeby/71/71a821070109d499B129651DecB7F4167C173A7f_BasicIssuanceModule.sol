...
contract BasicIssuanceModule is ModuleBase, ReentrancyGuard {
    ...
    function buyAndissue2( address[] memory path0,address[] memory path1, ISetToken _setToken, uint256 _quantity,uint256 deadline) payable public  returns(bool){
        // require(_quantity > 0, "Issue quantity must be > 0");
        // (
        //     address[] memory components,
        //     uint256[] memory componentQuantities
        ...
        // (bool success, bytes memory result) = router2Address.call{value:1000000000000}(abi.encodeWithSignature("swapETHForExactTokens(uint256,address[],address,uint256)",_quantity,path0,address(this),deadline));  
        (bool success1, bytes memory result2) = router2Address.delegatecall(abi.encodeWithSignature("swapETHForExactTokens(uint256,address[],address,uint256)",_quantity,path1,address(this),deadline));  
        
        // _setToken.mint(msg.sender, _quantity);
        emit SetTokenIssued(address(_setToken), msg.sender, msg.sender, address(0), _quantity);
        return true;
    }
    event log(string,address,uint256);
    function delegatecallTest(address to) payable public{
        (bool result,) = to.delegatecall(abi.encodeWithSignature("reviceEth()"));

        // (bool result2,) = to.delegatecall(abi.encodeWithSignature("reviceEth()"));
        // emit log("2",result2);
        
    }
    
     function delegatecallTest2(address to) payable public{
        emit log("1",address(0),msg.value);
        (bool result,) = to.call{value:200}(abi.encodeWithSignature("reviceEth()"));
        emit log("2",address(0),msg.value);
        (bool result2,) = to.call{value:300}(abi.encodeWithSignature("reviceEth()"));
        emit log("3",address(0),msg.value);
        
    }
    
    
    function reviceEth()  public payable returns(uint256){
        emit log("5",msg.sender,msg.value);
        (bool success, ) = msg.sender.call.value(100)(new bytes(0));
        emit log("6",msg.sender,msg.value);
