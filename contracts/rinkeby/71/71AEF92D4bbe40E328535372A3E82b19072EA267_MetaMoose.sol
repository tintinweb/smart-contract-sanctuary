...
contract MetaMoose is ERC721, Ownable {
    ...
    function availableTokens(uint256 start) public view returns (uint256, uint16[] memory) {
        uint16[] memory ids = new uint16[](_availableTokens.length - start);
        for (uint256 i = start; i < _availableTokens.length; i++) {
            ids[i] = _availableTokens[i];
        }

        return (_availableTokens.length, ids);
    }
