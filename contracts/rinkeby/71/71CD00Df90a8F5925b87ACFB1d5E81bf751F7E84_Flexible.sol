...
// Can we make it as claim rewards section where users can claim rewards over time instead of removing their stakings
// if that is the case then make a new function claimRewards and just return interest from calculate and pass it to new
// new function. Withdraw can just return initial staking
...
contract Flexible is Ownable {
    ...
    /**
     *  Requirements:
     *  `rate_` New APY multiplied by 100
    ...
    /**
     *  Requirements:
     *  `amount` Min staking amount by users
     *  @dev to set min staking amount
     */
    ...
    function _stake(address from, uint256 amount) private returns (bool) {
        ...
        deposits[from] = Deposits(amount, block.timestamp, index, false, true);
    ...
    function _calculate(address from) private view returns (uint256) {
        ...
        interest = interest.add(amount.mul(rate).mul(time).div(315360000000));
        amount += interest;
    ...
    modifier _minStakeAmount(uint256 amount) {
        require(
            amount >= minStakeAmount,
            "Users can't stake less than min stake amount"
        );
        _;
    }
}