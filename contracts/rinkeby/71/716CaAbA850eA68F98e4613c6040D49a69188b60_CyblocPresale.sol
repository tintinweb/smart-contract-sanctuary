...
contract CyblocPresale {
  ...
  event CyblocPacksOpened(address indexed receiver, uint8 indexed packType);
    ...
    payable
  {
    ...
    packsPurchased[msg.sender][packType] = packsPurchased[msg.sender][packType].add(quantity);
  ...
  // function unpack(address receiver, uint8 packType, uint256 _initialSeed) public {
  ...
  //   uint16[5] memory classProbabilities = packInfo.classProbabilities;

  //   require(totalPacksOpened[packType] < packInfo.quantity);

  //   uint256 _genes;

  //   for (uint256 i = 0; i < NUM_CYBLOC_PER_PACK; i++) {
  //       // Mint the ERC721 Cybloc.
  //     // Spawn an Cybloc with the generated `genes`.
  //     // _genes = _pickGenes(packType, totalCyblocInPack, classProbabilities, _initialSeed);
  //     coreContract.spawnCybloc(_genes, receiver);
  //   }
  //   emit CyblocPacksOpened(receiver, packType);
  // }
