...
contract RedEnvelope is ERC1155, Ownable {
    ...
    uint256 constant public MAX_CALIM_PER_WALLET = 2;
    ...
    address constant public TREASURY = 0xc1E355F74dEBf5e415C136538f0DbB6c9192e400;
    ...
    function claim(uint256 _amount) external payable {
        require(totalNFTClaimed[msg.sender] + _amount <= MAX_CALIM_PER_WALLET, "Too many claims per wallet");
        require(_amount > 0 && _amount <= MAX_CALIM_PER_WALLET, "Wrong claim amount");
        require(_amount*PRICE == msg.value, "Insufficient ETH");

        for (uint256 i = 0; i < _amount; ++i) {
            // mint all requested amount one by one, randomly
            RedEnvelopeType mintedType = mintNFT(msg.sender);
            totalNFTClaimed[msg.sender] += 1;
            totalSupply[mintedType] += 1;
        }
    }

    function mintNFT(address _to) internal returns (RedEnvelopeType) {
        // mint one NFT randomly for '_to'
        uint256 id = uint256(keccak256(
            abi.encodePacked(block.difficulty, block.timestamp))) % MAX_SUPPLY; // miners can abuse this?
        ...
        for (uint i = 0; i < types.length; ++i) {
            ...
            if (id < maxSupply[(uint256)(types[i])] &&
            totalSupply[types[i]] < maxSupply[(uint256)(types[i])]) {
                _mint(_to, (uint256)(types[i]), 1, "");
                return types[i];
            }
        }
        revert("All NFT have been claimed");
    }
    ...
    function uri(uint256 _id) override public view returns (string memory) {
        require(_id < 5 && _id >= 0, "Token ID does not exist");
