...
/**
 * @title ERC-721 Non-GUNSLINGERSble Token Standard, optional metadata extension
...
/**
 * @dev Implementation of https://eips.ethereum.org/EIPS/eip-721[ERC721] Non-GUNSLINGERSble Token Standard, including
...
/**
 * @title ERC-721 Non-GUNSLINGERSble Token Standard, optional enumeration extension
 * @dev See https://eips.ethereum.org/EIPS/eip-721
 */
...
contract ALIENSV2 is ERC721Enumerable, Ownable {

    using Counters for Counters.Counter;
    using Strings for uint256;

    uint256 public constant GUNSLINGERS_PUBLIC = 7777;
    uint256 public constant GUNSLINGERS_MAX = GUNSLINGERS_PUBLIC;
    uint256 public constant PURCHASE_LIMIT = 7;
    uint256 public constant PRICE = 10_000_000_000_000_000; // 0.01 ETH
    uint256 public allowListMaxMint = 7;
    string private _contractURI = "";
    string private _tokenBaseURI = "";
    address public proxyAddress;
    bool _isActive = true;

    Counters.Counter private _publicGUNSLINGERS;

    constructor() ERC721("Alien V2", "Alien V2") {}
    ...
    {
        require(
            _publicGUNSLINGERS.current() < GUNSLINGERS_PUBLIC,
            "Purchase would exceed GUNSLINGERS_PUBLIC"
        );

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicGUNSLINGERS.current();

            if (_publicGUNSLINGERS.current() < GUNSLINGERS_PUBLIC) {
                _publicGUNSLINGERS.increment();
                _safeMint(to, tokenId);
            }
        }
    }
    ...
    function purchase(uint256 numberOfTokens) external payable {    
        ...
        require(
            ...
            "Only Incubator contract may call this funtion"
        );
