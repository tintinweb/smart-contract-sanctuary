...
contract Unifarmv4 is Admin {
    ...
    function _rewardCalculation2(
        ...
        uint8 i = 1;
        while (i < intervalDays.length) {
            uint256 userStakingDuration = poolStartTime.add(intervalDays[i]*86400);
            
            if (conCount[0] >= intervalDays[i] && userStakingDuration >= stakingDetails[msg.sender].startTime[stakeId]) {
            ...
            else if(userStakingDuration <= stakingDetails[msg.sender].startTime[stakeId]) {
