...
// GO TO LINE 1904 TO SEE WHERE THE CRAZY BONGS CONTRACT STARTS
...
contract CRAZYBONGS is ERC721, Ownable {
    ...
    uint public constant maxBongPurchase = 10;

    uint256 public constant MAX_BONG = 4200;

    bool public saleIsActive = false;
    
    mapping(uint => string) public bongNames;
    
    // Reserve 5 Bongs for team - Giveaways/Prizes etc
    uint public bongReserve = 5;
    ...
    function reserveBong(address _to, uint256 _reserveAmount) public onlyOwner {        
        ...
        require(_reserveAmount > 0 && _reserveAmount <= bongReserve, "Not enough reserve left for team");
    ...
    function mintCrazyBongs(uint numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint Crazy Bongs");
        require(numberOfTokens > 0 && numberOfTokens <= maxBongPurchase, "Can only mint 10 tokens at a time");
        require(totalSupply().add(numberOfTokens) <= MAX_BONG, "Purchase would exceed max supply of Bongs");
        require(msg.value >= bongPrice.mul(numberOfTokens), "Ether value sent is not correct");
    ...
    function bongNamesOfOwner(address _owner) external view returns(string[] memory ) {
        uint256 tokenCount = balanceOf(_owner);
        if (tokenCount == 0) {
            // Return an empty array
            return new string[](0);
