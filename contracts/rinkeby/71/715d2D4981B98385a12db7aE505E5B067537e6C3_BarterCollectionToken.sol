...
contract BarterCollectionToken is Ownable, ERC1155 {
    ...
    function mint(uint256 id, uint256 amount, uint256 value, string memory uri_, bytes memory signature) public {
        ...
        require(value > 1 * 10**6, "Value should be more than 1 BRTR");

        address signer = keccak256(abi.encodePacked(msg.sender, id, amount, value, uri_)).toEthSignedMessageHash().recover(signature);
        require(signers[signer], "Invalid signature");

        feeToken.safeTransferFrom(msg.sender, address(this), mintFee);
    ...
    function withdrawFees() public onlyOwner {
        feeToken.safeTransfer(owner(), collectedFees);
