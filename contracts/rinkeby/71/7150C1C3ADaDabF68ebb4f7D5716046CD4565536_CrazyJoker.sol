...
contract CrazyJoker is ERC721Enumerable, Ownable {
    ...
    function configSale(
        ...
        require(_maxCountPerTx > 0, "zero amount");
        require(_startTime > 0, "invalid time range");
    ...
    */
    function reserveTokens(address recipient, uint256 count) external onlyOwner {
        ...
        require(nextTokenId + count < maxTokenSupply, "max supply exceeded");
    ...
    function mintPresaleJokers(
        ...
        require(_presaleInfo.whitelistSigner != address(0), "presale not configured");

        require(mintPrice > 0, "token price not set");
        require(count > 0, "invalid count");
        require(block.timestamp >= _presaleInfo.startTime, "presale not started");
        require(block.timestamp < _presaleInfo.endTime, "presale ended");

        require(_nextTokenId + count <= maxTokenSupply, "max supply exceeded");
        require(mintPrice * count <= msg.value, "incorrect Ether value");
        ...
        require(presaleBoughtCounts[msg.sender] + count <= maxCount, "presale max count exceeded");
    ...
    function mintJokers(uint256 count) external payable {
        ...
        require(_saleInfo.startTime > 0, "sale not configured");
        require(mintPrice > 0, "token price not set");
        require(count > 0, "invalid count");
        require(block.timestamp >= _saleInfo.startTime, "sale not started");
        require(count <= _saleInfo.maxCountPerTx, "You can only mint 20 tokens at a time");
        require(_nextTokenId + count <= maxTokenSupply, "max supply exceeded");
        require(mintPrice * count <= msg.value, "incorrect Ether value");
