...
contract TestPunks is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    // number of reserve tokens in total; 
    uint256 public remainingReserveTokens = 200;

    // wallet address for NFT Reserves
    address public constant ReservesWallet = 0xdA365F72A6E6911f30939Cf236f4B3CFf7D09860;
    ...
    string public constant FPOriginalProvHash = "this is the original hash sequence";

    // SHA256 of concatenated hash string containing all the hashes of all tokens
    string public FPFinalProvHash;
    ...
    // mapping to keep count of how many tokens an address has bought during each phase
    mapping(address => uint256) public _firstPresaleCounter;
    mapping(address => uint256) public _secondPresaleCounter;
    ...
    bool internal tokenURIDotJson = true;

    uint256 public startingIndexBlock;

    uint256 public startingIndex;
    
    constructor() ERC721("TestPunks", "TP") {}

    function mintFuturePunks(uint256 numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint");
        require(salePhase != 0, "no sale phase is active");
        require(numberOfTokens > 0 && numberOfTokens <= maxPurchase, "Not within the range of maxPurchase");
        require((totalSupply() + numberOfTokens) <= MAX_NFT_SUPPLY - remainingReserveTokens, "Purchase would exceed max supply of FuturePunks");
        require(msg.value >= tokenPrice * numberOfTokens, "Ether value sent is not correct");

        if (salePhase == 1) {
            require(isInPresale(msg.sender), "user is not whitelisted");
            require(_firstPresaleCounter[msg.sender] + numberOfTokens <= presaleLimit, "max NFT per address exceeded");
        } else if (salePhase == 2) {
            require(isInPresale(msg.sender), "user is not whitelisted");
            require(_secondPresaleCounter[msg.sender] + numberOfTokens <= presaleLimit, "max NFT per address exceeded");
        }
        ...
        if (salePhase == 1) {
            _firstPresaleCounter[msg.sender] += numberOfTokens;
        } else if (salePhase == 2) {
            _secondPresaleCounter[msg.sender] += numberOfTokens;
        }
    ...
    function nextSalePhase() public onlyOwner {
        require(salePhase < 3, "The last phase is already active!");
        salePhase++;
    }
    ...
    function lockMetadata(string memory finalHash) public onlyOwner {
        require(metadataLocked != true, "FuturePunks: You've already locked the metadata!");

        FPFinalProvHash = finalHash;
        metadataLocked = true;
    }
    ...
    function switchDotJson() public onlyOwner {
        require(metadataLocked != true, "Metadata has already been locked and cannot be changed anymore!");
        tokenURIDotJson = !tokenURIDotJson;
    }
    ...
    function getReserves(uint256 amount) public onlyOwner {
        require(remainingReserveTokens > 0, "All the reserve tokens have been minted already!");
        require(remainingReserveTokens - amount >= 0, "Amount would exceed the number of reserve tokens");
        require(totalSupply() + amount <= MAX_NFT_SUPPLY, "Amount would exceed max supply of FuturePunks");
        for (uint256 i = 0; i < amount; i++) {
            _safeMint(ReservesWallet, totalSupply());
        }
        remainingReserveTokens -= amount;
