...
contract UniqueFansToken is Ownable, SignerRole, ERC1155Base {
    string public name;
    ...
    function mint(uint256 id, uint8 v, bytes32 r, bytes32 s, Fee[] memory fees, uint256 supply, string memory uri, address userWallet) public {
        require(isSigner(StringLibrary.getAddress(abi.encodePacked(this, id, userWallet), v, r, s)), "signer should sign tokenId");
...
contract UniqueFansUserToken is UniqueFansToken {
    event CreateERC1155_v1(address indexed creator, string name, string symbol);

    constructor(string memory name, string memory symbol, string memory contractURI, string memory tokenURIPrefix, address signer) UniqueFansToken(name, symbol, signer, contractURI, tokenURIPrefix) public {
        emit CreateERC1155_v1(msg.sender, name, symbol);
    }

    function mint(uint256 id, uint8 v, bytes32 r, bytes32 s, Fee[] memory fees, uint256 supply, string memory uri, address userWallet) onlyOwner public {
        super.mint(id, v, r, s, fees, supply, uri, userWallet);
    }
}