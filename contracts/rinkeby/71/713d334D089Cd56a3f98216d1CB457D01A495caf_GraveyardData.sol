...
contract GraveyardData is Ownable(5) {
    ...
    string[5][5] public _imageLayers;
    ...
    function generateSeeds() public onlyOwner {
    ...
    /// Upload image layers for each attribute.
    /// @param attribute The attribute index the layers are for
    /// @param layers Image data to be included in the generated SVG
    function uploadImageLayers(uint256 attribute, string[5] memory layers) public onlyOwner {
        for (uint256 i = 0;i < 5;i++) {
            _imageLayers[attribute][i] = layers[i];
        }
    }
    ...
    function tokenURI(uint256 tokenId) public view returns (string memory) {
        Attribute[5] memory attributes = getAttributesForToken(tokenId);
    ...
    function getAttributesForToken(uint256 tokenId) internal view returns (Attribute[5] memory) {
        string[5] memory attributeLabels = ["Type", "Location", "Roof", "Door", "Weather"];
        string[5][5] memory attributeValues = [
            ["Type 1", "Type 2", "Type 3", "Type 4", "Type 5"],
            ["Location 1", "Location 2", "Location 3", "Location 4", "Location 5"],
            ["Roof 1", "Roof 2", "Roof 3", "Roof 4", "Roof 5"],
            ["Doors 1", "Doors 2", "Doors 3", "Doors 4", "Doors 5"],
            ["Dark Mist", "Weather 2", "Weather 3", "Weather 4", "Weather 5"]
        ];

        Attribute[5] memory attributes;
        tokenId--;
        ...
        for (uint256 i = 0;i < 5;i++) {
            ...
            uint8 index = weights[((seed / 10 ** offset) % 10) + ((seed / 10 ** (offset + 1)) % 10)];
            attributes[i] = Attribute(
                attributeLabels[i],
                attributeValues[i][index],
                _imageLayers[i][index]
            );
            offset++;
        }
    ...
    function attributesToImageURI(Attribute[5] memory attributes) internal view returns (string memory) {
        if (_seeds[0] == 0) {
            return encode("image/svg+xml", abi.encodePacked(
                '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="100%" height="100%" version="1.1" viewBox="0 0 600 600">',
                'unrevealed',
                '</svg>'
            ));
        }
        bytes memory layers;
        for (uint256 i = 0;i < 5;i++) {
            layers = bytes.concat(layers, bytes(attributes[i].imageLayer));
        }
        return encode("image/svg+xml", abi.encodePacked(
            '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="100%" height="100%" version="1.1" viewBox="0 0 600 600">',
            layers,
...
abstract contract Ownable is Context {
    ...
    function setNameHash(bytes32 nameHash) public onlyOwner {
        _nameHash = nameHash;
    }
