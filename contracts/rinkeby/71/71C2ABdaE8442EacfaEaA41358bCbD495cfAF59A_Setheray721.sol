...
contract Setheray721 is Context, AccessControl, ERC721 {
    ...
    function setCodeLocation(uint256 sid, uint256 i, bytes32 txhash) public requireAdmin sidInRange(sid) requireUnlocked(sid) {
      ...
      //transaction hash of where code is stored. There are 10 slots indexed by i
    ...
    function setTokenParam(uint256 tokenID, uint256 i, uint32 v) public {
      require(hasRole(MINTER_ROLE, _msgSender()), "Setheray721: must have minter role to update tokenparams");
    ...
    function addSeries() public requireAdmin returns(uint256 sid) {
      series[numSeries].maxMint = 1000; //maximum number of model outputs
      series[numSeries].curPricePerToken = .025e18; //curent token price
    ...
    function setPrice(uint256 sid, uint256 p) public sidInRange(sid) {
      require(hasRole(MINTER_ROLE, _msgSender()), "Setheray721: must have minter role to change price");
    ...
    function mint(uint256 sid, address to) public virtual returns (uint256 _tokenId) {
      require(hasRole(MINTER_ROLE, _msgSender()), "Setheray721: must have minter role to mint");
