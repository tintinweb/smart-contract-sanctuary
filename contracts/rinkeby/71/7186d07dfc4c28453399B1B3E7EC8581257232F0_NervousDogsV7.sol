 ...
 */
contract NervousDogsV7 is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function revealMetadata(string memory uri) public onlyOwner {
        ...
        //TODO: Should emit `event PermanentURI(string _value, uint256 indexed _id);`
    ...
    function startPresale() external onlyOwner whenSalePaused {
    ...
    function startCollectorsSale() external onlyOwner whenSalePaused {
        state = SaleState.Collectors;
    }

    /// Start Sale, if paused.
    function startSale() external onlyOwner whenSalePaused {
    ...
    function mintDog(uint8 numberOfTokens) public payable nonReentrant {
        require(
            state == SaleState.Presale || state == SaleState.Sale,
            "NervousDogs: Cannot mint when sale is paused"
        );
    ...
    function claimDog() public payable nonReentrant {
        require(
            state == SaleState.Collectors,
            "NervousDogs: Cannot claim when sale is inactive"
        );
