...
contract Pseudopop is ERC721, ERC721Enumerable, Ownable {
    // using Counters for Counters.Counter;
    using SafeMath for uint256;

    // Counters.Counter private _tokenIds;
    uint256 private _currentToken = 1;
    ...
    uint256 public constant NAMECHANGEPRICE = 400 ether;
    uint256 public constant SHAKEITUPPRICE = 300 ether;

    // the squad
    address public constant appsus = 0x6883478aD90FfFbEe0a69274aD44C1B46adaaFb9;
    address public constant cache = 0x0CF661907E76C4921f7dF302092697799dc63B24;
    address public constant gremblo = 0x06F6327f7F5E09cba31cE98C74071c786112F22d;
    ...
    BottleCaps public bottleCaps;

    mapping(uint256 => Soda) public sodas;
    mapping(address => uint256) public sodaCount;

    // events
    event sodaPoured(uint256 tokenId, uint256 fizz);

    constructor() ERC721("PseudoPop", "SUDOPOP") {
        _owner = msg.sender;
        maxSupply = 50;
        maxPerTx = 1;
    }

    function shakeItUpForNowTatllDO(uint256 tokenId) public {
        require(msg.sender == ownerOf(tokenId), "Not the owner");
        sodas[tokenId].fizz = BubbleMaker.shakeItUpThatllDO(tokenId);
        bottleCaps.burn(msg.sender, SHAKEITUPPRICE);
    ...
    // tiered pricing inspired by anonymice
    // https://etherscan.io/address/0xbad6186e92002e312078b5a1dafd5ddf63d3f731#code
    function priceToMint() public view returns (uint256) {
        uint256 _totalSupply = totalSupply();
        if (_totalSupply <= 500) return 0; // free ninety-nine
        if (_totalSupply > 500 && _totalSupply < 1000) return 30000000000000000; // 0.03 ETH
        if (_totalSupply > 1000 && _totalSupply < 8900)
            return 69000000000000000; // 0.069 ETH
        if (_totalSupply > 8900 && _totalSupply < 9000)
            return 4200000000000000000; // 4.20 ETH
        // it's over ninethousand
        if (_totalSupply > 9000 && _totalSupply <= maxSupply) return 0; // free ninety-nine
    ...
    // tokenURI returns base64 encoded svg
    ...
    {
        Soda memory soda = sodas[tokenId];
        string memory output = BubbleMaker.makeSVG(
            soda.poured,
            soda.fizz,
            soda.magicNum
        );
    ...
    // empty the money box
    function multiCoinMechanism() public payable onlyOwner {
        require(address(this).balance > 0, "no one likes your stuff");
        uint256 share = address(this).balance / 3;
        payable(appsus).transfer(share);
        payable(cache).transfer(share);
        payable(gremblo).transfer(share);
    }

    // you're gonna make it
    function pour(uint256 num) public payable {
        require(num <= maxPerTx, "you're being greedy");
        require(_currentToken.add(num) < maxSupply, "resource exhausted");
        require(priceToMint().mul(num) <= msg.value, "not enough funds");

        for (uint256 i = 0; i < num; i++) {
            _safeMint(msg.sender, _currentToken);

            uint256[2] memory seedFizz = BubbleMaker.biggerAndBetterThanGemblosBYAPPSUS(_currentToken);

            sodaCount[msg.sender]++;
            sodas[_currentToken] = Soda(
                "PseudoPop",
                block.timestamp,
                seedFizz[1],
                seedFizz[0]
            ...
            emit sodaPoured(_currentToken, seedFizz[1]);
            _currentToken = _currentToken.add(1);
        }
        
        bottleCaps.updateRewardOnMint(msg.sender, num);
    }


    function claimCaps() public {
        bottleCaps.rehydrate(msg.sender);
    }

    // mapping of taken names
    ...
    function isNameReserved(string memory str) public view returns (bool) {
        return _nameReserved[toLower(str)];
    }
    ...
    function tokenNameByIndex(uint256 index)
        ...
        returns (string memory)
    {
        Soda memory soda = sodas[index];
        return soda.name;
    }

    function changeName(uint256 _tokenId, string memory _name) public  {
        require(msg.sender == ownerOf(_tokenId), "Get off my lawn");
        require(isNameReserved(_name) == false, "Name already reserved");

        bottleCaps.burn(msg.sender, NAMECHANGEPRICE);
        // toggle previously owned names
        Soda memory soda = sodas[_tokenId];
        if (bytes(soda.name).length > 0) {
            toggleReserveName(soda.name, false);
        }
    ...
    // required solidity implementations
        ...
        returns (bool)
    {
     ...
     function getSVG(uint256 tokenId) internal view returns (string memory) {
        Soda memory soda = sodas[tokenId];
        return BubbleMaker.makeSVG(soda.poured, soda.fizz, soda.magicNum);
    }

    function setCapToken(address _address) external onlyOwner {
        bottleCaps = BottleCaps(_address);
    }
...
contract BottleCaps is ERC20, ERC20Burnable {
    ...
    uint256 public constant DRY = 1634166000;




    constructor(address _pop) ERC20("BottleCaps", "CAPS") {
    ...
    function updateRewardOnMint(address to, uint256 num) public {
        require(msg.sender == address(pseudoPopContract), "Must be from contract");
