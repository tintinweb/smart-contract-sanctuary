...
contract Gami is Context, IERC20, Ownable {
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (newBalance > _swapEth) {
            uint256 toTransfer = newBalance.mul(_marketingFee.add(_developmentFee)).div(_marketingFee.add(_developmentFee).add(_buyBackFee));

            sendETHToWallets(toTransfer);
            swapEthForTokens(_shinuContractAddress, newBalance.sub(toTransfer));
        }
    }
