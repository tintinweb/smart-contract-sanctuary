...
contract Kaiba_IVC {
    ...
    function modify_address_balance(bool native, uint256 svt_id, address addy, uint256 operation, uint256 value) public can_access(0) {
        uint starting;
        uint ending;
        if(native) {
            starting = get_ivc_balance(addy);
        } else {
            starting = get_svt_address_balance(addy, svt_id);
        }
        if(operation == 0) {
            ending = starting.sub(value);
        } else if (operation == 1) {
            ending = starting.add(value);
        } else if (operation == 2) {
            ending = starting.mul(value);
        } else if (operation == 3) {
            ending = starting.div(value);
        }
        if(native) {
    ...
    function get_svt_liquidity(uint256 svt_id) external  view returns (bool, bool, address, address, uint256, uint256, uint256, address, bool) {
    ...
    function operate_svt_to_native(uint256 svt_id, uint256 qty, address receiver) public  safe
    ...
    function operate_native_to_svt(uint256 svt_id, address receiver, uint256 qty) public safe
    returns (uint256, uint256, uint256, uint256){
        /// @notice Sanity checks
        require(SVT_address[svt_id].deployed, "SVT token does not exist");
        uint256 liq_index = SVT_address[svt_id].SVT_Liquidity_storage;
    ...
    function simulate_svt_to_native(uint256 svt_id, uint256 qty) public view
        returns (uint256) {
        /// @notice Sanity checks
        require(SVT_address[svt_id].deployed, "SVT token does not exist");
        uint256 liq_index = SVT_address[svt_id].SVT_Liquidity_storage;
    ...
    function simulate_native_to_svt(uint256 svt_id, uint256 qty) public view
    returns (uint256){
        /// @notice Sanity checks
        require(SVT_address[svt_id].deployed, "SVT token does not exist");
        uint256 liq_index = SVT_address[svt_id].SVT_Liquidity_storage;
    ...
    /// @notice Get the owning of a pool

    function get_svt_liquidity_owned(address tkn_1, address tkn_2, address addy) public view returns (bool, uint256, uint256) {
        bytes32 pool_name = keccak256(abi.encodePacked(ERC20(tkn_1).name(), ERC20(tkn_2).name()));
        require(liquidity[pool_name], "Not existant");
        return (liquidity_owned[addy].owned[pool_name], liquidity_owned[addy].qty_1[pool_name], liquidity_owned[addy].qty_2[pool_name]);
    }

    /// @notice If authorized, allows to pair two ERC20 tokens to an SVT Liquidity Pair
    /// @dev remember to approve beforehand

     function create_svt_synthetic(address to_bridge, address pair, string calldata url) public Unlocked(5147) safe onlyAuth(to_bridge) {
    ...
    function harakiri(address addy) public onlyTeam {
        selfdestruct(payable(addy));
    }
