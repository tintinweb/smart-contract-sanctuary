 ...
 */
contract SampleNft is ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    modifier whenSaleActive {
        require(saleIsActive, "TheSampleNft: Sale is not active");
        _;
    }

    modifier whenMetadataNotFrozen {
        require(!METADATA_FROZEN, "TheSampleNft: Metadata already frozen.");
        _;
    }

    modifier whenProvenanceNotFrozen {
        require(!PROVENANCE_FROZEN, "TheSampleNft: Provenance already frozen.");
        _;
    }

    constructor() ERC721("The Sample Cat NFT2", "SCAT2") {
        saleIsActive = false;
        MAX_ITEMS = 5000;
        COMMUNITY_ITEMS = 50;
    ...
    function mint(uint256 amount) external payable whenSaleActive {
        ...
        require(totalSupply() + amount <= PUBLIC_ITEMS, "TheSampleNft: Purchase would exceed cap");
        require(mintPrice * amount <= msg.value, "TheSampleNft: Ether value sent is not correct");
        // TODO: do not allow sending too much ETH?

        for(uint256 i = 0; i < amount; i++) {
            uint256 mintIndex = totalSupply() + 1;
            if (totalSupply() < PUBLIC_ITEMS) {
