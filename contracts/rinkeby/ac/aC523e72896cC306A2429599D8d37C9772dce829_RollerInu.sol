...
contract RollerInu is ERC20('RollerInu', 'ROI'), Ownable {
    ...
    function _transfer(address _sender, address _recepient, uint256 _amount) internal override onlyTradable(_sender) {
        ...
        if (_isExcludedFromFee[_sender]) {
            ...
            super._transfer(_sender, deadAddress, marketingFee);
        }
    }
    ...
    function enableTrading(bool _onoff) external onlyOperator {
        _tradingOpen = _onoff;
    }

    function updateFees(uint16 _burnTaxRate, uint16 _devTaxRate, uint16 _marketingTaxRate) external onlyOperator {
