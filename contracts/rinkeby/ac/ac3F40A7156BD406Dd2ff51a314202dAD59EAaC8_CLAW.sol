...
contract CLAW is Ownable, IERC20, IERC20Metadata, SafeToken, LockToken {
    ...
    event AwardPrize(address indexed to, uint256 tokenId);
    
    uint256 private gameCost;

    function setGameCost(uint256 value) external onlyOwner {
    ...
    function transferPrizeNFT(address nftContract, address to, uint256 tokenId) external onlyOwner returns (bool) {
        IERC721(nftContract).safeTransferFrom(owner(), to, tokenId);
        emit AwardPrize(to, tokenId);
        return true;
    }
}