...
contract Otomotors is ERC721Enumerable, Ownable {
    ...
    function startNewBatch() external onlyOwner {
        uint256 expectedSupply = 0;
         ...
         for(uint i = 1; i <= currentBatch; i++) {
           expectedSupply += getBatchSize(i);
         }
        
        require(expectedSupply == totalSupply(), "The previous batch must be complete to start the next batch");
    ...
    function devMint(address _to, uint256 _count) external onlyOwner {
        ...
        require(totalSupply() + _count <= batchMax, "Purchase would exceed max supply of Otomotors");
    ...
    function mint(address _to, uint256 _count) external payable {
        ...
        require(_count <= maxPurchase, "Can only mint 20 tokens at a time");
        require(totalSupply() + _count <= batchMax, "Purchase would exceed max supply of Otomotors");
    ...
    function getBatchMaxSupply(uint256 batchNumber) public view returns (uint256) {
        uint256 maxSupply = 0;
        
        for(uint i = 1; i <= batchNumber; i++) {
