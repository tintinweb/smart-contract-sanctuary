...
contract Bones1 is ERC20("Bone", "BONE"), Ownable {
    ...
    function TokensOwned(address _contractAddress, address _userAddress) external view returns(uint256[] memory) {
        require(trustedContracts [_contractAddress], "Not a trusted contract");
    
        uint256 tokenCount = iSkellies(_contractAddress).balanceOf(msg.sender);
        ...
        while(i <= tokenCount){
            i++;
            id++;
            if(iSkellies(_contractAddress).ownerOf(i) == _userAddress) {
                tokensId[i] = id;
