...
contract TomiStaking is Ownable, ReentrancyGuard, Pausable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    /* ========== STATE VARIABLES ========== */

    address public tomiPlatform;
    IERC20 public tomi;
    ...
    mapping(address => uint256) private _getRewardRevenueSharedBeforeExit;

    address[] private userInfo;

    /* ========== CONSTRUCTOR ========== */

    constructor(address _rewardsDistribution, address _tomiToken, address _tomiPlatform) {
        rewardsDistribution = _rewardsDistribution;
        tomi = IERC20(_tomiToken);
        tomiPlatform = _tomiPlatform;
    }
    ...
    function getRewardRevenueShare(address account)
        ...
        returns (uint256)
    {
        if ((userInfo.length < 1) ||  (_revenueShared == 0)) {
            return 0;
        }
        uint256 sumOfTotal = 0;
        uint256 totalTimeStaked;

        for (uint256 i = 0; i < userInfo.length; i++) {
            totalTimeStaked = block.timestamp.sub(_lastStakedTime[userInfo[i]]);
            sumOfTotal = sumOfTotal.add(
                _balances[userInfo[i]].mul(totalTimeStaked)
            );
        }

        totalTimeStaked = block.timestamp.sub(_lastStakedTime[account]);
        return
            _balances[account].mul(totalTimeStaked).mul(_revenueShared).div(
                sumOfTotal
            );
    }
    ...
    function removeUser(address account) internal {
        uint256 userLength = userInfo.length;
        for (uint256 i = 0; i < userLength; i++) {
            if (userInfo[i] == account) {
                userInfo[i] = userInfo[userLength - 1];
                userInfo.pop();
                return;
            }
        }
    ...
    function updateRevenueShare(uint256 revenueShared) external onlyPlatform {
    ...
    modifier onlyPlatform() {
        require(msg.sender == tomiPlatform, "TomiStaking: caller is not TomiPlatform");
        _;
    }
