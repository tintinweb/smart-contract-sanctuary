...
contract GOO is Context, IERC721Receiver, IERC20 {
    ...
    function getPassiveClaimable(address _user) public view returns(uint256) {
		uint256 time = min(block.number, endBlock);// / PASSIVE_REWARD_PERIOD;
		...
		return pending * (BASE_RATE - totalStaked);
	}
	...
	function claimPassiveRewards() public {
	    uint256 passiveClaim = getPassiveClaimable(_msgSender());
        if (passiveClaim > 0){
            _updatePassiveClaims(_msgSender());
            _mint(_msgSender(), passiveClaim);
        }
	}
    ...
    function unstakeAll() external {
        ...
        EnumerableSet.UintSet storage depositSet = stakingDeposits[_msgSender()];
        for (uint256 idx = depositSet.length() - 1; idx >= 0; idx++){
            stakingDeposits[_msgSender()].remove(depositSet.at(idx));
            goobersContract.safeTransferFrom(address(this), _msgSender(), depositSet.at(idx), '');
        }
        unchecked{
            totalStaked -= depositSet.length();
        }
    }
	...
	function _updatePassiveClaims(address _user) internal {
	    uint256 time = min(block.number, endBlock);// / PASSIVE_REWARD_PERIOD;
		uint256 goobersCount = goobersContract.balanceOf(_user);
		for (uint256 idx = 0; idx < goobersCount; idx++){
		    if (lastUpdate[goobersContract.tokenOfOwnerByIndex(_user, idx)] < time ){
		        lastUpdate[goobersContract.tokenOfOwnerByIndex(_user, idx)] = time;
		    }
		}
	}
	...
	function _transfer(
        ...
        if (passiveClaim > 0) {
            _updatePassiveClaims(sender);
        }
        
        uint256 stakingClaim = getStakingClaimable(sender);
        if (stakingClaim > 0) {
            _updateStakingClaims();
        }
        
        _mint(sender, passiveClaim + stakingClaim);
