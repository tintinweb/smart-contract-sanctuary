...
contract GutterSpecies is ERC721Enumerable, Ownable, ReentrancyGuard {
	...
	uint256 public maxFreePass = 1500;
	uint256 public maxPrivatePass = 1500;
	uint256 public maxPublicPass = 3000;

	//keeps track of how many passes were minted
	uint256 public counterFreePass = 0;
	uint256 public counterPrivatePass = 0;
	uint256 public counterPublicPass = 0;

	//maximum total passes
	uint256 public maxTotalPass3 = 3000;
	uint256 public maxTotalPass4 = 6000; //it's 3000, because it starts from 3000
	...
	function getPassForPair(
		...
		counterFreePass++;
		require(counterFreePass < maxFreePass, "no more passes available");
	...
	function getPassPrivateSale(uint256 catOrRatID, uint256 passType) public payable nonReentrant {
		...
		counterPrivatePass++;
		require(counterPrivatePass < maxPrivatePass, "no more passes available");
	...
	function getPassPublicSale(uint256 passType) public payable nonReentrant {
		...
		counterPublicPass++;
		require(counterPublicPass < maxPublicPass, "no more passes available");
	...
	function _mintToken(address destinationAddress, uint256 tokenType) private {
		if (tokenType == 3) {
			tokenIndexPass3++;
			require(tokenIndexPass3 <= maxTotalPass3, "only 3000 passes allowed");
			require(!_exists(tokenIndexPass3), "Token already exist.");
			_safeMint(destinationAddress, tokenIndexPass3);
		}
		if (tokenType == 4) {
			tokenIndexPass4++;
			require(tokenIndexPass4 <= maxTotalPass4, "only 3000 passes allowed");
	...
	//let's do it all in one function
	function changeLimitsPasses(
		uint256 _maxFreePass,
		uint256 _maxPrivatePass,
		uint256 _maxPublicPass
	) external onlyOwner {
		maxFreePass = _maxFreePass;
		maxPrivatePass = _maxPrivatePass;
		maxPublicPass = _maxPublicPass;
	}

	//let's do it all in one function
	function changeTotalSupplyPasses(uint256 _passes3, uint256 _passes4) external onlyOwner {
		maxTotalPass3 = _passes3;
		maxTotalPass4 = _passes4;
	}
