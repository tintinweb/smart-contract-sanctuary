...
contract RecyclingPlatform is Context, AccessControlEnumerable {
    ...
    function buyRawToken(bool isResource) public {

        (uint256 tokensToSaleF,
        uint256 tokensToSaleR,
        uint256 tokensAlreadySoldF,
        uint256 tokensAlreadySoldR) = rawToken.getTotalTokens();
        if (isResource) {
            require(tokensToSaleR > 0, 'No NFTokens for sale');
        } else {
            require(tokensToSaleF > 0, 'No NFTokens for sale');
        }
        (uint256 base_price,
    ...
    function buyRawTokens(bool isResource, uint256 total) public {
        for (uint256 i = 0; i < total; i++) {
            buyRawToken(isResource);
        }
    }
}
