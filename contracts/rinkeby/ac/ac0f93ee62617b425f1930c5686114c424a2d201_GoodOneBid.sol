...
contract GoodOneBidStorage {
    ...
    mapping(address => mapping(uint256 => List)) public listByToken;
...
contract GoodOneBid is Ownable, Pausable, GoodOneBidStorage {
    ...
    */
        ...
        returns (bytes4)
    {
        ...
        List memory listToken = listByToken[msg.sender][_tokenId];

         // Check if the list is valid.
        require(
            // solium-disable-next-line operator-whitespace
            listToken.id == listId &&
            listToken.amount == _value &&
            listToken.owner == _from,
            "Invalid list"
        );
    ...
    function list(address _tokenAddress, uint256 _tokenId, uint256 _amount, uint256 _reservePrice, uint256 _duration) public {
        require(
        ...
        listByToken[_tokenAddress][_tokenId] = List({
            id: listId,
            owner: msg.sender,
        ...
        ERC1155Interface(_tokenAddress).safeTransferFrom(msg.sender, address(this), _tokenId, _amount, _bytes32ToBytes(listId));
    }

    function unlist(address _tokenAddress, uint256 _tokenId, uint256 _amount) public {
        require(
        ...
        require(
            listByToken[_tokenAddress][_tokenId].owner == msg.sender || msg.sender == owner(),
        ...
        delete listByToken[_tokenAddress][_tokenId];
    ...
    function changeReservePrice(address _tokenAddress, uint256 _tokenId, uint256 _amount, uint256 _reservePrice) public {
        ...
        listByToken[_tokenAddress][_tokenId].reservePrice = _reservePrice;
    ...
    */
    function placeBid(
        ...
        whenNotPaused()
    {
        List memory listToken = listByToken[_tokenAddress][_tokenId];
        ...
        // TODO: minumum reaise 
        require(_price > highestBid.price && _price > listToken.reservePrice, "Price should be bigger than highest Bid and reserve price");

        require(block.timestamp > listToken.expiresAt, "List has ended");

        if(listToken.expiresAt.sub(block.timestamp) < FIFTEEN_MINUTE) {
            
            uint256 newListTokenExpires = block.timestamp.add(FIFTEEN_MINUTE);
            listToken.expiresAt = newListTokenExpires;
    ...
    */
        ...
        whenNotPaused()
    {
        ...
        address owner = listByToken[_tokenAddress][_tokenId].owner;
     
        //TODO: or the higest bidder

        require(msg.sender == owner, "Sender must be the NFT owner");
    ...
    * @param _amount - The withdraw amout
    */
    function withdraw(address _tokenAddress, uint256 _tokenId ,address _withdrawAddress, uint256 _amount) external onlyOwner {
        require(_withdrawAddress != address(0), "address can't be the zero address");

        ERC1155Interface(_tokenAddress).safeTransferFrom(address(this), _withdrawAddress, _tokenId, _amount, "");
    }
