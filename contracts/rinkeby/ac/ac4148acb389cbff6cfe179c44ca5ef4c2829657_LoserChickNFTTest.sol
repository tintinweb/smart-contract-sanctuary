...
contract LoserChickNFTTest is OwnableContract, ERC721("TrumpChick", "TrumpChick"){
    ...
    mapping(uint256 => ChickAttribute) public _tokenIdChickAttribute;

    event AccessoriesInfo(uint256 tokenId, ChickAttribute chickAttribute);
    ...
    function getTokenIdChickAttribute(uint256 tokenId) public view returns(ChickAttribute memory){
        return _tokenIdChickAttribute[tokenId];
    }

    function tokenOfOwnerPage(address owner, uint256 page) external view returns (uint256 total, uint256[12] memory chicks){
        ...
        if (total > start) {
            ...
            for (uint256 i=0; i<countOnPage; i++) {
                chicks[i] = tokenOfOwnerByIndex(owner, start + i);
            }
        }
    }
     ...
     */
    function creatAccessoriesInfo(address owner, uint256 index, bool flag) public onlyAdmin{
        ...
        bytes32 random = keccak256(abi.encodePacked(block.number, now, msg.sender));
        if(index == 0 && flag){
        ...
        _tokenIdChickAttribute[tokenId] = chickAttribute;
        emit AccessoriesInfo(tokenId, chickAttribute);
    }

    // Private Methods
    function _mintChick(address owner) public returns(uint256){
        _tokenIds.increment();
