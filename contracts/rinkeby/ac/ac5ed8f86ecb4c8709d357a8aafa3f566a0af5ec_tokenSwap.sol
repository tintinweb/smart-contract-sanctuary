...
contract tokenSwap is Ownable {
   ...
   function buy(address _tokenIn, address _tokenOut, uint256 _amountIn, uint256 _amountOutMin, uint256 deadline) external {
    ...
    pathOut[1] = _tokenIn;
    if (_tokenIn == WETH) {
        IUniswapV2Router(SwapAddress).swapExactETHForTokens{value: _amountIn}(_amountOutMin, path, payable(address(this)), deadline);
        uint256 sellamount = IERC20(_tokenIn).balanceOf(address(this)).mul(SellRate).div(100);
        IUniswapV2Router(SwapAddress).swapExactTokensForETH(sellamount, 0, pathOut, payable(address(this)), deadline);
    } else {
