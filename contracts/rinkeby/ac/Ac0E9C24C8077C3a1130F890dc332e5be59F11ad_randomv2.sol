...
contract randomv2 is VRFConsumerBase, Ownable {
    ...
    {
        if (fulfiled){
        return  uint256(keccak256(abi.encode(randomResult, tx.origin, block.timestamp)));
        } else {
        return uint256(keccak256(abi.encode(randomResult, tx.origin, ( block.timestamp + 1 ))));
        }
    }
