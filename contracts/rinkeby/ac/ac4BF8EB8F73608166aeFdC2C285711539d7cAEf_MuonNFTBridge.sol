...
contract MuonNFTBridge is Ownable {
    ...
    function depositFor(
        ...
        if (mintable[tokenId]) {
            ...
            token.safeTransferFrom(address(msg.sender), address(this), nftId);
        }
    ...
    function claim(
        ...
        require(
            block.timestamp - muonTimestamp < acceptableMuonDelay,
            "expired"
        );
        ...
        if (mintable[tokenId]) {
            token.mint(user, nftId);
        } else {
            token.safeTransferFrom(address(this), user, nftId);
        }

        claimedTxs[fromChain][txId] = true;
        emit Claim(user, nftId, fromChain, tokenId, txId);
    }
    ...
    function emergencyWithdrawERC721Tokens(
        ...
        IMuonNFTBridgeERC721(_tokenAddr).safeTransferFrom(
            address(this),
            _to,
            _id
        );
