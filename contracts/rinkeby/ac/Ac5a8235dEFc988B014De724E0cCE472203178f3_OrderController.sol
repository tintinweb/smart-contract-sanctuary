...
contract OrderController is Ownable {
    ...
    event OrderUpdated(uint256 id, uint256 amountLeftToFill, uint256 fee);
    ...
    function matchOrders(
        ...
        for (uint256 i = 0; i < matchedOrderIds.length; i++) {
            ...
            if (newOrder.amountLeftToFill.mul(matchedOrderAmountA) >= matchedOrderAmountLeftToFill.mul(matchedOrderAmountB)) {
                ...
                emit OrderUpdated(matchedOrder.id, 0, fee);
            } else {
                ...
                emit OrderUpdated(matchedOrder.id, matchedOrderAmountLeftToFill, fee);
            }
        }
        emit OrderUpdated(id, newOrder.amountLeftToFill, totalFee);
