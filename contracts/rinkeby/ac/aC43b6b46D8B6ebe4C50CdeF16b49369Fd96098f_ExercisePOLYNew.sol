...
contract ExercisePOLYNew {
    ...
    function initialize( address _pOLY, address _ohm, address _dai, address _treasury, address _previousClaimContract ) external returns ( bool ) {
    ...
    function exercisePOLYOldVault( uint _amountToExercise ) external returns ( bool ) {
    ...
    function exercisePOLYNewVault( uint _amountToExercise ) external returns ( bool ) {
        require( getPOLYAbleToClaim( msg.sender ).sub( _amountToExercise ) >= 0, 'Not enough OHM vested' );
        require( maxAllowedToClaim[ msg.sender ].sub( amountClaimed[ msg.sender ] ).sub( _amountToExercise ) >= 0, 'Claimed over max' );
        ...
        IVaultNew( treasury ).depositReserves( _amountToExercise, DAI );
        IPOLY( pOLY ).burnFrom( msg.sender, _amountToExercise );
    ...
    function getPOLYAbleToClaim( address _vester ) public view returns (uint) {
        return ( OHMCirculatingSupply().mul( percentCanVest[ _vester ] ).mul( 1e9 ).div( 10000 ) ).sub( amountClaimed[ _vester ] );
    }

    function OHMCirculatingSupply() public view returns ( uint ) {
