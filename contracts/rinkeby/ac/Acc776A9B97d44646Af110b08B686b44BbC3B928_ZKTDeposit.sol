...
contract ZKTDeposit is Ownable, Initializable, Testable {
    ...
    event UpdateLockTime(address indexed from, uint lockTime);
    ...
    function deposit(uint amount) external lock(){
        require(amount > 0, "ZKTDeposit: amout is zero");
//        token.safeApprove(address(this), amount);

        token.safeTransferFrom(msg.sender, address(this), amount);
//
        uint currentDay = getCurrentTime() / ONE_DAY;
        Order[] storage orderArr = orders[msg.sender];
        bool isExist = false;
        uint len = orderArr.length;
        if (len == 0){
            orderArr.push(Order({value: amount, startDay: currentDay}));
        } else {
            uint i = len - 1;
            while(i >= 0){
                if (orderArr[i].startDay == currentDay){
                    orderArr[i].value = orderArr[i].value + amount;
                    isExist = true;
                    break;
                }
                if (i == 0){
    ...
    function withdraw(uint amount) external lock() {
        ...
        uint currentDay = getCurrentTime() / ONE_DAY;
        // query available
        uint value = _available(msg.sender, currentDay);
        require(value >= amount, "ZKTDeposit: Insufficient amount available");
        // update
        _updateBalance(msg.sender, amount, currentDay);
        // transfer
    ...
    function balanceOf(address account) external view returns(uint){
        uint amount = 0;
        Order[] storage orderArr = orders[account];
        uint len = orderArr.length;
        for(uint i = 0; i < len; i++){
            amount = amount + orderArr[i].value;
        }
        return amount;
    }
    ...
    function _available(address account, uint currentDay) internal view returns(uint){
        ...
        uint len = orderArr.length;
        for(uint i = 0; i < len; i++){
            if (orderArr[i].startDay + lockTime <= currentDay){
                amount = amount + orderArr[i].value;
            }
        }
        return amount;
    ...
    function _updateBalance(address account, uint withdrawals, uint currentDay) internal {
        uint i = 0;
        uint tmp = 0;
        Order[] storage orderArr = orders[account];
        uint len = orderArr.length;
        while (i < len){
            if (orderArr[i].startDay + lockTime <= currentDay){
                tmp = tmp + orderArr[i].value;
                if (tmp > withdrawals) {
                    //add excess
                    orderArr[i].value = tmp - withdrawals;
                    break;
                } else {
                    orderArr[i].value = orderArr[len - 1].value;
                    orderArr[i].startDay = orderArr[len - 1].startDay;
                    orderArr.pop();
                    len = len - 1;
                    if (tmp == withdrawals){
                        break;
