...
contract Shoes is ERC721URIStorage {
  ...
  mapping(uint256 => uint256) public zoraTokenId;
  uint256[] marketplaceStatusArray;

  constructor(string memory name, string memory symbol) ERC721(name, symbol) {}

  // The function to mint the NFT
  function mintNFT(string memory _tokenURI, uint256 _marketplaceStatus, uint256 _price) public {
    // get token id
  ...
  // The function to get price for a specific NFT
  function getZoraTokenId(uint256 _tokenId) public view returns (uint256) {
    return zoraTokenId[_tokenId];
  }

  // The function to get price for a specific NFT
  function setZoraTokenId(uint256 _tokenId, uint256 _zoraTokenId) public {
    zoraTokenId[_tokenId] = _zoraTokenId;
  }
}
