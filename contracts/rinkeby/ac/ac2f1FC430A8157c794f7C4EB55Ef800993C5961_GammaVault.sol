    ...
    ReentrancyGuard
{
    ...
    event Snapshot(int24 tickLower, int24 tickUpper, int24 newTickLower, int24 newTickUpper, int24 tick, uint256 totalAmount0, uint256 totalAmount1);
     ...
     * @param amount0 Max amount of token0 to deposit
     * @param amount1 Max amount of token1 to deposit
     */
    function deposit(
        uint256 amount0,
        uint256 amount1
    ...
    function withdraw(
        ...
        uint256 amount1Min,
        address to
    ) external nonReentrant onlyGovernance returns (uint256 amount0, uint256 amount1) {
        require(to != address(0) && to != address(this), "to");

        // Withdraw proportion of liquidity from Uniswap pool
        _burnLiquidityShare(baseLower, baseUpper);
    ...
    function _burnLiquidityShare(
        ...
        if (liquidity > 0) {
            (uint256 burned0, uint256 burned1, uint256 collect0, uint256 collect1) =
                _burnAndCollect(tickLower, tickUpper, _toUint128(liquidity));

            amount0 = burned0.add(collect0);
            amount1 = burned1.add(collect1);
        }
    }
    ...
    function rebalance(
        ...
        emit Snapshot(baseLower, baseUpper, _baseLower, _baseUpper, tick, balance0, balance1);
    ...
    function rebalanceTokens() external {
        require(msg.sender == strategy || msg.sender == governance, "strategy || governance");
        _checkRange(baseLower, baseUpper);

        uint256 balance0 = getBalance0();
        uint256 balance1 = getBalance1();
        (uint128 liquidity, uint256 amount0, uint256 amount1) = liquidityAmounts();
        require(amount0 > balance0 || amount1 > balance1, "does not need to swap");

        // if needed amount0, but balance is less than that. so we'd need to swap
        // swap with input, we should swap with output
        if (balance0 > amount0) {
            pool.swap(address(this), true, int256(balance0 - amount0), TickMath.MIN_SQRT_RATIO + 1, "");
        } else if (balance1 > amount1) {
            pool.swap(address(this), false, int256(balance1 - amount1), TickMath.MAX_SQRT_RATIO - 1, "");
        }

        balance0 = getBalance0();
        balance1 = getBalance1();
        uint128 liquidityAmount = _liquidityForAmounts(baseLower, baseUpper, balance0, balance1);
        _mintLiquidity(baseLower, baseUpper, liquidityAmount);
    }

    function liquidityAmounts() public view returns (uint128, uint256, uint256) {
        if (baseLower == baseUpper) {
            return (0, 0, 0);
        }
        uint256 balance0 = getBalance0();
        uint256 balance1 = getBalance1();
        uint160 sqrtRatioLowerX96 = TickMath.getSqrtRatioAtTick(baseLower);
        uint160 sqrtRatioUpperX96 = TickMath.getSqrtRatioAtTick(baseUpper);
        uint128 liqAmount0 = LiquidityAmounts.getLiquidityForAmount0(sqrtRatioLowerX96, sqrtRatioUpperX96, balance0);
        uint128 liqAmount1 = LiquidityAmounts.getLiquidityForAmount1(sqrtRatioLowerX96, sqrtRatioUpperX96, balance1);
        uint128 liquidity = liqAmount0 > liqAmount1 ? liqAmount0 : liqAmount1;
