...
/**
 * @notice Access Controls contract for the Instagram Platform
 */
contract InstagramAccessControls is AccessControl {
...
import "../AccessControl/InstagramAccessControls.sol";
import "@openzeppelin/contracts/token/ERC721/ERC721.sol";
import "@openzeppelin/contracts/utils/EnumerableSet.sol";

/**
 * @title Instagram Post NFT
 * @dev Issues ERC-721 tokens 
 */
contract InstagramNFT is ERC721("Instagram Post NFT", "IPN") {

    // @notice event emitted upon construction of this contract, used to bootstrap external indexers
    event InstagramNFTContractDeployed();

    // @notice event emitted when token URI is updated
    event InstagramTokenUriUpdate(
    ...
    InstagramAccessControls public accessControls;
    ...
    /**
     @notice Constructor
     @param _accessControls Address of the InstagramNFT access control contract
     */
    constructor(InstagramAccessControls _accessControls) public {
        accessControls = _accessControls;
        emit InstagramNFTContractDeployed();
    }

    /**
     @notice Mints a InstagramNFT AND when minting to a contract checks if the beneficiary is a 721 compatible
    ...
    function mint(address _beneficiary, string calldata _tokenUri, address _postCreator) external returns (uint256) {
        require(
            ...
            "InstagramNFT.mint: Sender must have the admin or minter role"
        );
        // Valid args
    ...
    function burn(uint256 _tokenId) external {
        ...
        require(
            ...
            "InstagramNFT.burn: Only garment owner or approved"
        );
    ...
    function setTokenURI(uint256 _tokenId, string calldata _tokenUri) external {
        require(
            ...
            "InstagramNFT.mint: Sender must have the admin or contract role"
        );
        _setTokenURI(_tokenId, _tokenUri);
        emit InstagramTokenUriUpdate(_tokenId, _tokenUri);
    }
    ...
    function setPrimarySalePrice(uint256 _tokenId, uint256 _salePrice) external {
        ...
        require(_exists(_tokenId), "InstagramNFT.setPrimarySalePrice: Token does not exist");
        require(_salePrice > 0, "InstagramNFT.setPrimarySalePrice: Invalid sale price");
     ...
     */
    function updateAccessControls(InstagramAccessControls _accessControls) external {
        require(accessControls.hasAdminRole(_msgSender()), "InstagramNFT.updateAccessControls: Sender must be admin");
        accessControls = _accessControls;
    }
    /////////////////
    // View Methods /
    ...
    function _assertMintingParamsValid(string calldata _tokenUri, address _postCreator) pure internal {
        require(bytes(_tokenUri).length > 0, "InstagramNFT._assertMintingParamsValid: Token URI is empty");
        require(_postCreator != address(0), "InstagramNFT._assertMintingParamsValid: Post createErrors is zero address");
    }
}
