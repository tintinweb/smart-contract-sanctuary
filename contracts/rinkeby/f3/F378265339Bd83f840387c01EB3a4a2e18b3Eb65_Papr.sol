...
contract Papr is ERC20Burnable, DoubleOperator {
    ...
    mapping(address => bool) public boardroomAddress;
    ...
    function isBoardroomAddress(address adress) public view returns(bool isIndeed) {
        return boardroomAddress[adress];
    }
    ...
    function addBoardroomAddress(address newAddress) external onlyDoubleOperator returns(bool success) {
        require(!isAddressesWithFees(newAddress));
        boardroomAddress[newAddress] = true;
        return true;
    }

    function removeBoardroomAddress(address oldAddress) external onlyDoubleOperator returns(bool success) {
        require(isAddressesWithFees(oldAddress));
        boardroomAddress[oldAddress] = false;
        return true;
    }
