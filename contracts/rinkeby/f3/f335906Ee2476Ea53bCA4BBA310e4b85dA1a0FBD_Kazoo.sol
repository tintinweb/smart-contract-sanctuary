...
contract Kazoo is ERC721URIStorage, Ownable {
    ...
    function _mint(address _addr, bool _isSponsored) private
    {
        ...
        uint256 _mNumber = SVGKazoo.randomNumber(_msgSender(), RANDOM_NUMBER, kazooId, randomSeed++ );
        bool _isSpecial = (_mNumber % SPECIAL_NUMBER == 0); //if it is a multiple of 16 (NO MANS SKY)

        KazooObject memory object = KazooObject(_addr, kazooId, _mNumber, "<svg></svg>","Unnamed Kazoo", new string[](MAX_NAMES), new uint256[](MAX_SPONSORS), 0, _isSponsored, _isSpecial, new string[](13), "");

        //set it inside the class wide array for the other methods to see
        ...
        //build the SVG
        buildKazooSVG(kazooId);

        //update our local object
        object = kazoos[kazooId];

        //finally, mint
        _safeMint(_addr, kazooId);

        //set the token URI to something which
        _setTokenURI(kazooId, SVGKazoo.placeholderMetadata() );

        //emit we are done and increment the kaz id
        emit mintedKazoo(kazooId++, object); //emit the updated method
    }

    //sets tokenURI on second param true
    function rebuildKazoo(uint256 _kazooId) private {
        ...
        for(uint256 i = 0; i < 13; i++){
            _styles[i] = SVGKazoo.getStyle( _kaz.backgroundColours[i], _kaz.borderColour, 2);
        }

        string memory _textColor;

        if( _kaz.masterNumber < RANDOM_NUMBER / 2)
            _textColor = SVGKazoo.getHexCode( RANDOM_NUMBER );
        else
            _textColor = SVGKazoo.getHexCode( _kaz.masterNumber >> 3 );

        _kaz.kazoo = SVGKazoo.buildKazoo(_styles, _kaz.names, SVGKazoo.getHexCode( _kaz.masterNumber >> 2 ), _textColor, _kaz.isSpecial, _kaz.masterNumber);

        kazoos[_kazooId] = _kaz;
    }

    function buildKazooSVG(uint256 _kazooId) private {
        require(_kazooId >= 0 && _kazooId <= kazooId, "invalid kazooId");
        KazooObject memory _kaz = kazoos[_kazooId];

        string[] memory _styles = new string[](13);
        //will effectively make the border darker
        _kaz.borderColour = SVGKazoo.getHexCode( _kaz.masterNumber >> 1 );
        string memory _textColor;

        //if the overall master number is basically either dark or light
        if( _kaz.masterNumber < RANDOM_NUMBER / 2)
            _textColor = SVGKazoo.getHexCode( RANDOM_NUMBER );
        else
            _textColor = SVGKazoo.getHexCode( _kaz.masterNumber >> 3 );
        ...
        for(uint256 i = 0; i < 13; i++){
            ...
            _kaz.backgroundColours[i] = SVGKazoo.getHexCode(( _kaz.masterNumber << 1 ) * ((i+1) * 10));
        }
    ...
    function countNames(string memory _name) private view returns (uint256 r){

        for(uint256 i = 0; i < kazooId; i++){
            KazooObject memory _kaz = kazoos[i];

            if( SVGKazoo.stringCompare(_name, _kaz.name ) ){
                r++; //increment result
            }
        }
    }

    function nameKazoo(uint256 _kazooId) private {
        require(_kazooId >= 0 && _kazooId <= kazooId, "invalid kazooId");
        KazooObject memory _kaz = kazoos[_kazooId];

        uint256 _randomNameNumber = SVGKazoo.randomNumber(_msgSender(), _kaz.masterNumber >> 2, _kazooId, randomSeed++ );
        uint256 _times = _randomNameNumber % ( MAX_NAMES - 1);
        _kaz.names = spinName(_times, _kaz.masterNumber, _randomNameNumber);
        ...
        //will return a number of names which are the same as this, if it isn't zero then we'll add an umber
        uint256 _dupes = countNames(_name);
        if(_dupes != 0)
            _name = string(abi.encodePacked(_name, " #", SVGKazoo.toString(_dupes + 1)));
...
library SVGKazoo {
    function getHeader(string memory _backgroundColor)
        public
        pure
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    _backgroundColor,
                    "}",
                    ".italic { font: italic 20px sans-serif; } .bold{ font: bold 20px sans-serif; } .comic-sans{font-family:Comic Sans MS, Comic Sans, cursive;font-size:20px;}.impact{font-family:Impact, fantasy;font-size:20px; }",
                    "</style>"
                )
            );
    }

    function getStyle(string memory _backgroundColor)
        public
        pure
        returns (string memory)
    {
        ...
        returns (string memory)
    {
        ...
        for (uint256 i = 0; i < _names.length; i++) {
            ...
            if (i < 2 && i != 0 )
                _sp = (stringLength(_names[i - 1]) * 12); //comic sans is a bit larger and more cursive
            else if(i >= 2 )
                _sp = (stringLength(_names[i - 1]) * 13) + 1; //the other font

            _cl = _cl + ( _sp + 1 ); //padding

            //if the current location + the length of the current name sets us over
            if (i != 0 && _cl + (stringLength(_names[i]) * 10) > 245 && _cl < 345)
                //if its greater than 185 then we want to skip to 345 and continue from there
                _cl = 345;
    ...
    function base64Encode(string memory _d) public pure returns(string memory){
    ...
    function randomNumber(address _address, uint _modulus, uint _nonce, uint _randomSeed) public view returns(uint256)
    {
    ...
    function placeholderMetadata() public pure returns(string memory){
        return string(abi.encodePacked("data:application/json;base64,", Base64.encode(string(abi.encodePacked("{ name: 'Metadata Missing Kazoo', description: 'This kazoo needs its metadata set! Tell its owner to login to kazookid.eth and sync the metadata.', image: 'http://i.stack.imgur.com/ajwm5.png', attributes: []}")))));
    }
