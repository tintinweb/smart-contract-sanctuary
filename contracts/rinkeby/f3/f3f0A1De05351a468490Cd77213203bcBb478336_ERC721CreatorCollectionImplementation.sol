...
contract ERC721CreatorCollectionImplementation is ERC721CreatorCollectionBase, AdminControlUpgradeable {
     ...
     * Initializer
     */
    function initialize(address creator, uint16 tokenMax_, uint256 tokenPrice_, uint16 purchaseLimit_, uint256 presaleTokenPrice_, uint16 presalePurchaseLimit_, address signingAddress) public initializer {
        __Ownable_init();
        _initialize(creator, tokenMax_, tokenPrice_, purchaseLimit_, presaleTokenPrice_, presalePurchaseLimit_, signingAddress);
    }
...
abstract contract ERC721CreatorCollectionBase is CollectionBase, CreatorExtension, IERC721Collection {
    ...
    function _initialize(address creator, uint16 tokenMax_, uint256 tokenPrice_, uint16 purchaseLimit_, uint256 presaleTokenPrice_, uint16 presalePurchaseLimit_, address signingAddress) internal {
    ...
    function state() external override view returns(CollectionState memory) {
        return CollectionState(tokenMax, tokenMax-tokenCount, tokenPrice, purchaseLimit, presaleTokenPrice, presalePurchaseLimit, _addressMintCount[msg.sender], active, startTime, endTime, presaleInterval);
    }
