...
contract DefiYieldIndividualCrowdsale is IndividuallyCappedCrowdsale {
  ...
  constructor(uint256 price_, address wallet_, IERC20 token_, IERC20 collateral_, DefiYieldTokenVesting vesting_, uint8 pool_)
    Crowdsale(price_, wallet_, token_, collateral_)
  {
    _vesting = vesting_;
    _pool = pool_;
  ...
  function setDirectWithdrawPercentage(uint256 percentage_)
    ...
    onlyAdmin
  {
    require(percentage_ >= 0 && percentage_ <= 100, "DefiYieldIndividualCrowdsale::setDirectWithdrawPercentage: withdraw percentage should be >= 0 and <= 100");
  ...
  function _deliverTokens(address beneficiary_, uint256 tokenAmount_)
    ...
    override
  {
    uint256 directWithdrawAmount = (tokenAmount_ * _directWithdrawPercentage) / 100;
    uint256 vestingAmount = tokenAmount_ - directWithdrawAmount;
    ...
    if (vestingAmount > 0) {
      _vesting.addBeneficiary(_pool, beneficiary_, vestingAmount);
    }
  }
...
abstract contract IndividuallyCappedCrowdsale is Crowdsale, AdminRole {
  ...
  /**
   * @dev Sets beneficiary's maximum contribution.
   * @param beneficiary_  address to be capped.
   * @param cap_  collateral tokens limit for individual contribution.
   */
  function setCap(address beneficiary_, uint256 cap_)
    external
    onlyAdmin
  {
  ...
  /**
   * @dev Returns the cap of a beneficiary.
   * @param beneficiary_  address whose cap is to be checked.
   * @return current cap for individual beneficiary.
   */
  function getCap(address beneficiary_)
    external
    view
    returns (uint256)
  {
    return _caps[beneficiary_];
  }

  /**
   * @dev Returns the contributed amount so far by a beneficiary.
   * @param beneficiary_  address of a beneficiary.
   * @return beneficiary contribution so far.
   */
  function getContribution(address beneficiary_)
    external
    view
    returns (uint256)
  {
    return _contributions[beneficiary_];
  }
    ...
    override
  {
    ...
    require(
      _contributions[beneficiary_].add(collateralAmount_) <= _caps[beneficiary_],
      "IndividuallyCappedCrowdsale::_preValidatePurchase: beneficiary's cap exceeded"
    );
  }
    ...
    override 
  {
    ...
    _contributions[beneficiary_] = _contributions[beneficiary_] + collateralAmount_;
  }
}
