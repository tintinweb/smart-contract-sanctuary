...
contract Robots is ERC721PresetMinterPauserAutoId, Garage {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory baseURI = _baseURI();
        return bytes(baseURI).length > 0
            ? string(abi.encodePacked(baseURI, uint2str(tokenId%9)))
            : '';
    }
...
//             j /= 9;
//         }
//         string memory bstr = new bytes(len);

//         string memory baseURI = _baseURI();
//         return bytes(baseURI).length > 0
//             ? string(abi.encodePacked(baseURI, tokenId.bstr()))
//             : '';
//     }
...
// This function is only for typecasting, it has nothing to do with the contract
