...
contract HexApeYachtClub is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint256 constant maxPerAddress = 8;
    uint256 constant minPriceInWei = 69000000000000000; // 0.069 ETH
    ...
    function reserveForOwner() public onlyOwner returns (uint256) {
        require(totalSupply() <= fixedSupply, "Mint already at max supply");
        uint256 i;
        for (i = 0; i < 50; i++) {
    ...
    function mint(uint256 amount) public payable returns (uint256) {
        ...
        require(latestId <= fixedSupply, "Mint already at max supply");
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            _safeMint(_msgSender(), currSupply);
        }
        return amount;
    }
