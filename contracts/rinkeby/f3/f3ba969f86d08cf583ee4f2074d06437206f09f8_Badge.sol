...
interface WhitelistInterface {
    function addWhitelist(string memory newWhitelist) external;

    function checkWhitelist(string memory addr) external view returns(uint8);

    function setWhitelist(string memory addr) external;
}
...
contract Badge is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    //Whitelist Contract
    address public whiteListAddress = 0xc7cD535FCeDC96828701FE71A51eb2C1D4330501;
    ...
    function getRealMintPrice(uint256 tokenId) public view returns (uint256) {
        if(tokenId<7799)
        {
            return baseMintPrice*(1+totalSupply()/ladderUnit);
        }
        return mintPrice;
    }
    ...
    function getRealLootUserMintPrice(uint256 tokenId) public view returns (uint256) {
        if(tokenId<7799)
        {
            return baseLootUserMintPrice*(1+totalSupply()/ladderUnit);
        }
    ...
    function claim(uint256 tokenId, uint256 genderId, uint256 raceId, uint256 elementId, uint256 strNum, uint256 staNum, uint256 agiNum, uint256 chaNum, uint256 intNum, uint256 shiNum) public payable nonReentrant {
        require((tokenId >= 1 && tokenId <= 7799) || (tokenId >= 8000 && tokenId <= maxTotalSupply), "Token ID invalid");
    ...
    function claimWithWhitelist(uint256 tokenId, uint256 genderId, uint256 raceId, uint256 elementId, uint256 strNum, uint256 staNum, uint256 agiNum, uint256 chaNum, uint256 intNum, uint256 shiNum) public nonReentrant {
        require((tokenId >= 1 && tokenId <= 7799) || (tokenId >= 8000 && tokenId <= maxTotalSupply), "Token ID invalid");
//        require(whitelist[string(abi.encodePacked(msg.sender))] == 1, "Not in whitelist");
//        whitelist[string(abi.encodePacked(msg.sender))]=2;

//        require(checkWhitelist(string(abi.encodePacked(msg.sender))) == 1, "Not in whitelist");
//        setWhitelist(string(abi.encodePacked(msg.sender)));
//        require(whitelistContract.checkWhitelist(string(abi.encodePacked(msg.sender))) == 1, "Not in whitelist");
        require(whitelistContract.checkWhitelist(string(abi.encodePacked(msg.sender))) == 1, string(abi.encodePacked(msg.sender)));
        whitelistContract.setWhitelist(string(abi.encodePacked(msg.sender)));
