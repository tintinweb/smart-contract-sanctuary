...
contract KommunitasVoting is ERC20("KommunitasVoting", "KOMV"), Ownable {
    ...
    modifier isSwapPaused{
        require(!swapPaused, "Swap to New KomV is Paused");
        _;
    }
    ...
    function swapToKomV2() isSwapPaused public {
    ...
    /// @notice Creates `_amount` token to `_to`. Must only be called by the Permissioned Contract.
    function mint(address _to, uint256 _amount) public onlyPermission {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    }
    
    /// @notice Destroys `_amount` token to `_to`. Must only be called by the Permissioned Contract.
    function burn(address _to, uint256 _amount) public onlyPermission {
        _burn(_to, _amount);
...
contract KommunitasStakingV2 is Ownable {
    using SafeMath for uint256;
    ...
    constructor(address _komToken, address _komVToken, uint256 _apy, uint256[3] memory _lockPeriod, uint256[3] memory _multiplier) {
        ...
        for(uint256 i=0; i<3; i++){
            ...
            staked[lockPeriod[i]].multiplier = _multiplier[i];
    ...
    view returns (uint256 _lockReward) {   
        uint256 multiplier = staked[_lockPeriod].multiplier;
        if (multiplier == 0){
            multiplier = 10;
        }
     ...
     */
    function stake(uint256 _amount, uint256 _lockPeriod) isStakePaused public {
        require(msg.sender != address(0),"Zero Address");
        require(_amount >= MIN_STAKING_AMOUNT, "Minimum staking amount is 100 KOM");
        require(lockPeriod[staked[_lockPeriod].lockPeriodIndex] == _lockPeriod, "Lock Period Not Exist");

        uint256 matureUntil = block.timestamp.add(_lockPeriod);
        uint256 lockReward = calculateReward(_amount, _lockPeriod);

        uint userId;
        if(everStaked(msg.sender) == false){
            userStaked.push(msg.sender);
            userId = userStaked.length-1;
        }else{
            userId = locks[msg.sender][0].userStakedIndex;
        }
        
        komToken.transferFrom(msg.sender, address(this), _amount);
        locks[msg.sender].push(TokenLock(staked[_lockPeriod].lockPeriodIndex, userId, _amount, block.timestamp, matureUntil, lockReward, false));
        staked[_lockPeriod].kommunitasStaked += _amount;
    ...
    function unlock(uint256 _sid) public returns (uint256) {
        ...
        staked[lockPeriod[locks[msg.sender][_sid].lockPeriodIndex]].kommunitasStaked -= unlockablePrincipalStakedAmount;
     ...
     */
    function preMatureWithdraw(uint256 _sid, uint256 _amount) external returns (uint256) {
        require(locks[msg.sender].length > _sid, "Exceed the user history stake length");
        require(locks[msg.sender][_sid].maturity > block.timestamp, "Not pre-mature");
        require(!locks[msg.sender][_sid].claimed, "Already claimed");
        require(locks[msg.sender][_sid].amount >= _amount, "pre-mature amount exceeds");
        
        uint256 unlockableTokens;
        
        if(locks[msg.sender][_sid].amount == _amount){
            unlockableTokens = locks[msg.sender][_sid].amount;
            
            locks[msg.sender][_sid].claimed = true;
        } else{
            unlockableTokens = _amount;
            
            TokenLock memory temp = locks[msg.sender][_sid];
            locks[msg.sender].push(temp);
            uint newSid = locks[msg.sender].length-1;
            locks[msg.sender][newSid].amount = _amount;
            locks[msg.sender][newSid].reward = calculateReward(_amount, lockPeriod[locks[msg.sender][newSid].lockPeriodIndex]);
            locks[msg.sender][newSid].claimed = true;
            
            locks[msg.sender][_sid].amount = locks[msg.sender][_sid].amount.sub(_amount);
            locks[msg.sender][_sid].reward = calculateReward(locks[msg.sender][_sid].amount, lockPeriod[locks[msg.sender][_sid].lockPeriodIndex]);
        }
        ...
        staked[lockPeriod[locks[msg.sender][_sid].lockPeriodIndex]].kommunitasStaked -= unlockableTokens;
    ...
    function everStaked(address _of) public view returns(bool){
        ...
        return (getUserStakedLength(_of) > 0);
    }
    ...
    function setPeriod(uint256 _multiplier, uint256 _oldLockPeriod, uint256 _newLockPeriod) public onlyOwner {
        ...
        require(lockPeriod[staked[oldPeriodInSecs].lockPeriodIndex] == oldPeriodInSecs, "Old Lock Period Not Exist");
        
        // old index lock period to move
        uint256 indexToMove = staked[oldPeriodInSecs].lockPeriodIndex;
        
        // assign new lock period
        lockPeriod[indexToMove] = newPeriodInSecs;
        
        staked[newPeriodInSecs].lockPeriodIndex = indexToMove;
        staked[newPeriodInSecs].multiplier = _multiplier;
