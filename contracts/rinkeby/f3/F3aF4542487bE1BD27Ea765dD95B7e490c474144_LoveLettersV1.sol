...
contract LoveLettersV1 is Context, Ownable, ERC721Enumerable {
  ...
  mapping(uint256 => string) public tokenIdToLetter;

  address public constant staffVaultAddress = 0x37b2ee54A9C20eca0010DafCF6Eb2d5cA45B5ACe;
...
//Constructor
  ...
  function writeLetter(string memory letter, address to) public payable {
    require(saleStarted, "You can not write letters until the project starts!");
  ...
  function readLetters(uint256 tokenId) public view returns (string memory) {
  ...
  function tokenURI(uint256 tokenId) override public view returns (string memory){
    ...
    parts[0] =  "<svg width='500' height='500' viewBox='0 0 285 350' fill='none' xmlns='http:\\/\\/www.w3.org/2000/svg'><path fill='black' d='M150,0,L75,200,L225,200,Z'></path></svg>";
    parts[1] = '';
    string memory output = string(abi.encodePacked(parts[0], parts[1]));
    string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Bag #', toString(tokenId), '", "description": "Love Letters descrption", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
