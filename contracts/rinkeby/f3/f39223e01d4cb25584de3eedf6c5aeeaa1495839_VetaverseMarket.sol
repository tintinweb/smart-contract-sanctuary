...
// Vetaverse Marketplace Contract
...
contract VetaverseNFT is ERC721 {
...
contract VetaverseMarket is Ownable, ERC721Holder {
    using SafeMath for uint256;
    using Address for address;
    ...
    function createCollection(string memory _name, string memory _uri, bool bPublic) public returns(address collection) {
        ...
        IVetaverseNFT(collection).initialize(_name, _uri, msg.sender, bPublic);
    ...
    function list(address _collection, uint256 _token_id, uint256 _price) OnlyItemOwner(_collection,_token_id) public {
        ...
        IVetaverseNFT nft = IVetaverseNFT(_collection);
    ...
    function delist(uint256 _id) external {
        ...
        IVetaverseNFT(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, pairs[_id].token_id);
    ...
    function buy(uint256 _id) external ItemExists(_id) {
		...
		IVetaverseNFT(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, pair.token_id);
		pairs[_id].bValid = false;
    ...
    modifier OnlyItemOwner(address tokenAddress, uint256 tokenId) {
        IVetaverseNFT tokenContract = IVetaverseNFT(tokenAddress);
