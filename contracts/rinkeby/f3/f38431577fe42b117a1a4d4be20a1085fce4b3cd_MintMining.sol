...
contract MintMining is OwnableContract, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor(address _mts, uint256 _mining_start_block) public {
        mts = IERC20(_mts);

        uint256 blockCountPerDay = 28800; //  3 sec per block in bsc , 20 * 60 * 24 =  28800

        phase1StartBlockNumber = _mining_start_block;
        phaseEndBlockNumberArray.push(phase1StartBlockNumber.add(blockCountPerDay.mul(30))); // 1~30 
        phaseEndBlockNumberArray.push(phase1StartBlockNumber.add(blockCountPerDay.mul(210))); // 31~210
        phaseEndBlockNumberArray.push(phase1StartBlockNumber.add(blockCountPerDay.mul(390))); // 211~390
        phaseEndBlockNumberArray.push(phase1StartBlockNumber.add(blockCountPerDay.mul(570))); // 391~570
        phaseEndBlockNumberArray.push(phase1StartBlockNumber.add(blockCountPerDay.mul(750))); // 571~750
        phaseEndBlockNumberArray.push(phase1StartBlockNumber.add(blockCountPerDay.mul(930))); // 751~930
        phaseEndBlockNumberArray.push(phase1StartBlockNumber.add(blockCountPerDay.mul(1110))); // 931~1110
        phaseEndBlockNumberArray.push(phase1StartBlockNumber.add(blockCountPerDay.mul(1290))); // 1111~1290
        phaseEndBlockNumberArray.push(phase1StartBlockNumber.add(blockCountPerDay.mul(1470))); // 1291~1470

        uint256 phase1PerBlockReword = 150 * 1e18; // 150 mts per block
        phasePerBlockRewardArray.push(phase1PerBlockReword);
        uint256 phase2PerBlockReword = 75 * 1e18; // 75 mts per block
        phasePerBlockRewardArray.push(phase2PerBlockReword);
        uint256 phase3PerBlockReword = 375 * 1e17; // 37.5 mts per block
        phasePerBlockRewardArray.push(phase3PerBlockReword);
        uint256 phase4PerBlockReword = 1875 * 1e16; // 18.75 mts per block
        phasePerBlockRewardArray.push(phase4PerBlockReword);
        uint256 phase5PerBlockReword = 9 * 1e18; // 9 mts per block
        phasePerBlockRewardArray.push(phase5PerBlockReword);
        uint256 phase6PerBlockReword = 45 * 1e17; // 4.5 mts per block
        phasePerBlockRewardArray.push(phase6PerBlockReword);
        uint256 phase7PerBlockReword = 224 * 1e16; // 2.24 mts per block
        phasePerBlockRewardArray.push(phase7PerBlockReword);
        uint256 phase8PerBlockReword = 112 * 1e16; // 1.12 mts per block
        phasePerBlockRewardArray.push(phase8PerBlockReword);
        uint256 phase9PerBlockReword = 5 * 1e17; // 0.5 mts per block
        phasePerBlockRewardArray.push(phase9PerBlockReword);
    }
    ...
    function add(string memory _poolName, uint256 _allocPoint, address _lpToken, bool _withUpdate) public onlyAdmin {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function updateBlockReward(uint256 _phaseIndex, uint256 _phasePerBlockReward) public onlyOwner {
        require(_phaseIndex < phasePerBlockRewardArray.length, "invalid _phaseIndex");
        phasePerBlockRewardArray[_phaseIndex] = _phasePerBlockReward;
    }

    function updatePhaseEndBlockNumber(uint256 _phaseIndex, uint256 _phaseEndBlockNumber) public onlyOwner {
        require(_phaseIndex < phaseEndBlockNumberArray.length, "invalid _phaseIndex");
        phaseEndBlockNumberArray[_phaseIndex] = _phaseEndBlockNumber;
    }
    ...
    function addMiningPhase(uint256 _phaseEndBlockNumber, uint256 _phasePerBlockReword) public onlyOwner {
        ...
        phasePerBlockRewardArray.push(_phasePerBlockReword);
    }
    ...
    function safeMiningTokenTransfer(address _to, uint256 _amount) internal {
        uint256 bal = mts.balanceOf(address(this));
        require(bal >= _amount, "ClaimMiningContract' balance is not enough.");
        mts.safeTransfer(_to, _amount);
    }
}