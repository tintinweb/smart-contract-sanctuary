...
contract FIRE is Ownable, ERC20 {
    ...
    event Swapped(address collection, uint256 assetId, address account, uint256 amount, bool isERC721);
     ...
     */
    modifier started() {
        require(isStarted, "FIRE: NFT swapping does not start yet");
        _;
    }
    ...
    modifier notInBlacklist(address collection, uint256 assetId) {
        require(!_collectionBlacklist[collection] && !_assetBlacklist[collection][assetId], "FIRE: collection or asset is in blacklist");
        _;
    }
        ...
        onlyOwner
    {
        ...
        require(length == assetIds.length, "FIRE: length of arrays is not equal");
    ...
    function _issueToken(address collection, uint256 assetId, address account, bool isERC721)
        private
    {
        (uint256 amount) = calculateToken(collection, assetId);

  
            if (isERC721) {
                require(!isERC721Paused, "FIRE: ERC721 swapping paused");

            } else {
                require(!isERC1155Paused, "FIRE: ERC1155 swapping paused");
            }
            
        require(amount > 0, "FIRE: amount is invalid");

        // Issues tokens
        _mint(account, amount);

        emit Swapped(collection, assetId, account, amount, isERC721);
    }
        ...
        notInBlacklist(collection, assetId)
    {
        ...
        require(!msgSender.isContract(), "FIRE: caller is invalid");
