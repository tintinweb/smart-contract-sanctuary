...
contract KommunitasFactory is IKommunitasFactory{
    ...
    address public override stakingV1;
    address public override stakingV2;
    ...
    mapping(address => bool) public override payment;
    ...
    function createProject(
        ...
        require(_payment != address(0) && _tokenOrAdminProject != address(0), "Can't assign to address(0)");
        require(getProject[_tokenOrAdminProject] == address(0), "Project exist");
        require(payment[_payment], "Payment not acceptable");
        require(_fee >= 0 && _fee <=10, "Fee range between 0% - 10%");
        require(_boosterRunning > 0 && _boosterDelay > 0, "Can't be 0");
        
        project = address(new KommunitasProject{salt: keccak256(abi.encodePacked(_tokenOrAdminProject))}());

        getProject[_tokenOrAdminProject] = project;
        allProjects.push(project);
        
        KommunitasProject(getProject[_tokenOrAdminProject]).initialize(
            ERC20(_payment),
            _tokenOrAdminProject,
        ...
        emit ProjectCreated(_tokenOrAdminProject, project, allProjects.length-1);
    }
    
    // WARNING: Be careful of gas spending!
    function updateAllProjects() public override {
        require(allProjectsLength() > 0, "No Project exist");
        for(uint i=0; i<allProjectsLength(); i++){
            if(!KommunitasProject(allProjects[i]).buyEnded()){
    ...
    function setPayment(address _token, bool _accepted) public override onlyOwner{
        require(_token != address(0), "Can't set to address(0)");
        payment[_token] = _accepted;
    }
    ...
    function setStakingV1(address _stakingV1) public override onlyOwner{
        require(_stakingV1 != address(0), "Can't set to address(0)");
        stakingV1 = _stakingV1;
    }
    
    function setStakingV2(address _stakingV2) public override onlyOwner{
        require(_stakingV2 != address(0), "Can't set to address(0)");
        stakingV2 = _stakingV2;
    }
}