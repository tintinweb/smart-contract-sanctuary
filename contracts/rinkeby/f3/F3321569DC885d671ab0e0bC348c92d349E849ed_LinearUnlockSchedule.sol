...
contract LinearUnlockSchedule is BaseGovernanceWithUserUpgradable {
    ...
    function initialize(ILock lock, bytes calldata data) public initializer {
        ...
        __LinearUnlockSchedule_init(lock, _lockEndTimestamp, _cliffEndTimestamp, governanceAddress);
    }

    function __LinearUnlockSchedule_init(ILock _lock, uint256 _lockEndTimestamp, uint256 _cliffEndTimestamp, address governanceAddress) internal initializer {
        __BaseGovernanceWithUser_init(governanceAddress);
        __LinearUnlockSchedule_init_unchained(_lock, _lockEndTimestamp, _cliffEndTimestamp);
    }

    function __LinearUnlockSchedule_init_unchained(ILock _lock, uint256 _lockEndTimestamp, uint256 _cliffEndTimestamp) internal initializer {
        require(_cliffEndTimestamp <= _lockEndTimestamp, "cliff after lock end");
        require(block.timestamp <= _lockEndTimestamp, "lock end passed");
        lockContract = _lock;
        endLockTime = _lockEndTimestamp;
        cliffTimestamp = _cliffEndTimestamp;
    }
    ...
    function unlockedAmount(uint256 lockStartTimestamp, uint256 initialAmount) external view returns(uint256) {
        ...
        if(block.timestamp < cliffTimestamp) {
            _unlockedAmount = 0;
        } else if (block.timestamp >= endLockTime) {
            _unlockedAmount = initialAmount;
        } else {
            _unlockedAmount = initialAmount * (block.timestamp - lockStartTimestamp) / (uint256(endLockTime - lockStartTimestamp));
        }
