...
contract MultiSigWallet {
    ...
    function getPendingTransactions()
      public
      view
      returns (address [] memory, address [] memory, uint [] memory, uint [] memory,uint, uint, uint) {
        ...
        return (fromAddress, toAddress, userAmount, userSignatureCount,totalTxCnt,pendingTxCnt,i);
    }
    
    function getPendingTransactions_test(uint index)
      public
      view
      returns (address [] memory, address [] memory, uint [] memory, uint [] memory) {
          ...
          uint[] memory userSignatureCount = new uint[](pendingTxCnt);
           ...
           if(_transactions[index].isActive){
                fromAddress[index] = _transactions[index].from;
                toAddress[index] = _transactions[index].to;
                userAmount[index] = _transactions[index].amount;
                userSignatureCount[index] = _transactions[index].signatureCount;
            }
          
        return (fromAddress, toAddress, userAmount, userSignatureCount);
    }
    
    function getPendingTransactions_newtest()
      public
      view
      returns (address [] memory, address [] memory, uint [] memory, uint [] memory) {
          address[] memory fromAddress = new address[](pendingTxCnt);
