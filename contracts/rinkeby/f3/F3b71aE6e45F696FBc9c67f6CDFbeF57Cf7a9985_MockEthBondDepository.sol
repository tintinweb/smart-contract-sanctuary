...
contract MockEthBondDepository is Ownable {
    ...
    IPAXG public immutable principle; // token used to create bond
    ...
    enum PARAMETER { VESTING, PAYOUT, DEBT, MINPRICE, CONTROL }
    /**
    ...
    function setBondTerms ( PARAMETER _parameter, uint _input ) external onlyPolicy() {
        ...
        } else if ( _parameter == PARAMETER.CONTROL ) { // 4
    ...
    function deposit( 
         ...
         */
        principle.transferFrom( msg.sender, address(treasury), _amount );
        
        treasury.updateReserve( address(principle), value );
    ...
    function standardizedDebtRatio() external view returns ( uint ) {
        return debtRatio().mul( uint( assetPrice() ) )/ 1e8; // PAXG feed is 8 decimals
    }
    ...
    function recoverLostToken( address _token ) external returns ( bool ) {
        require( IERC20(_token) != Ore, "NAT" );
        require( IPAXG(_token) != principle, "NAP" );
