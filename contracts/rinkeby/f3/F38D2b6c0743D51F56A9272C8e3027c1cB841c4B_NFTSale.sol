...
import './interfaces/IDragonNFT.sol';

contract NFTSale is Ownable, Whitelistable {
    using Counters for Counters.Counter;
    using Address for address;
    using SafeMath for uint256;

    Counters.Counter private _soldCount;
    IDragonNFT public NFT;
    ...
    event BatchPurchase(address _buyer, uint256 _price, uint256 _amount);
    ...
    constructor(
        ...
        NFT = IDragonNFT(_nftAddress);
    }

    /**
     * @notice - Activate Sales
     * @dev - callable only by owner
     *
     * @param _isActive - enable sales
     */
     ...
     * @dev - callable only by owner
     *
     * @param _available - Price of NFT
     */
    function setAvailable(uint256 _available) public onlyOwner {
        available = _available;
    }
    ...
    function purchase() external payable onlyDuringSale onlyWhitelisted {
        require(_soldCount.current() <= available, 'Sold out');
     ...
     */
    function batchPurchase(uint256 amount) external payable onlyDuringSale onlyWhitelisted {
        ...
        emit BatchPurchase(msg.sender, price, amount);
    }

    modifier onlyDuringSale() {
        require(isActive, 'Sale is not activated');
        _;
    }
}
...
abstract contract Whitelistable is Ownable {
    ...
    event BatchWhitelisted(address[] _users);
    event Blacklisted(address _user);
    event BatchBlackisted(address[] _users);
    event SetWhitelistEnabled(bool _enabled);
     ...
     */
    function whitelistBatch(address[] memory _users) external onlyOwner {
     ...
     * @param _users - Addresses of Users
     */
    function blacklistBatch(address[] memory _users) external onlyOwner {
        for (uint256 i = 0; i < _users.length; i++) {
            _whitelist[_users[i]] = false;
        }
        emit BatchBlackisted(_users);
    }

    /**
     * @notice Enable/Dsiable Whitelist Feature
     *
     * @param _enabled - Enable/Disable Sale for all
     */
    function setWhitelistEnabled(bool _enabled) external onlyOwner {
        enabled = _enabled;
        emit SetWhitelistEnabled(_enabled);
    }
    ...
    modifier onlyWhitelisted() {
        require(_whitelist[_msgSender()] || _msgSender() == owner() || enabled, 'Caller is not Whitelisted');
        _;
    }
}
