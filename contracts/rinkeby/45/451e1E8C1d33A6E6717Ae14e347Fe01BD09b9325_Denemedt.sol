...
contract Denemedt is ERC721, ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    using Strings for uint256;

    uint256 public constant tokenPrice = 6000000000000000; //0.006 eth.fiyat
    ...
    event ddtMinted(uint256 tokenId, address owner);

    constructor() ERC721("DenemeDT", "DT") {}
...

    {
        ...
        function mintDdt(uint256 numberOfTokens) external payable{
                require(saleIsActive, "sale aktif min tokenle reyiz");
                require(
                ...
                require(
                    ...
                    "veya daha fazla alman lazm reyizz"
                );
                require(
                    msg.value >= tokenPrice.mul(numberOfTokens),
                    "Gonderilen ether dogru degil"
        ...
        function presaleDdt(uint256 numberOfTokens) external payable {
            require(presaleIsActive, "presale acik degil kardesim");
            require(_presaleList[msg.sender], "presale listte yoksun kardesim");
            require(
                totalSupply().add(numberOfTokens) <= MAX_TOKENS,
                "presale gectin kardesim"
            );
            require(
                    numberOfTokens > 0 && numberOfTokens <= presaleMaxMint,
                    "bunu alamazsin kardesim"
                    );
            require(
                _presaleListClaimed[msg.sender].add(numberOfTokens) <= presaleMaxMint,
                "toplami gectin kardesim"
                );
            require(
                    msg.value >= tokenPrice.mul(numberOfTokens),
                    "yanlis para kardesim"
                 );
                 ...
                 for(uint256 i = 0; i < numberOfTokens; i++)  {
                    ...
                    function addToPresaleList(address[] calldata addresses) external onlyOwner {
                        for (uint256 i = 0; i < addresses.length; i++) {
                            require(addresses[i] != address(0), "bos adres ekleyemezsin kardesim");
                ...
                {
                    for (uint256 i = 0; i < addresses.length; i++){
                        require(addresses[i] != address(0), "bos adres eklenemez kardesim");
                    ...
                    returns (string memory)
                    {
                        ...
                        if(isRevealed == false){
                        return
                        "ipfs://QmZDNmFVJ8xBAtWeZkVJqBZJiDLoDtEvtn2hey11PwV81x/hidden.json";
                        }
                        return
