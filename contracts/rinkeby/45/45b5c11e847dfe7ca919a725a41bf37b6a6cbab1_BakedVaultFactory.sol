...
contract BakedVaultFactory  {
    ...
    address public playshareFeesCollector;
    ...
    constructor(address _baked_address,uint256 _playshare_percent) public {
        factory_owner = msg.sender;
        baked_address = _baked_address;  
    ...
    function createCampaign(uint[] memory _data,address _token,uint256 _token_Amount,uint256 _vesting_percentage,uint256 _lock_Duration) external returns (address campaign_address){
        ...
        uint256 maximumWallet = _data[6].mul(1e18);
        uint256 TokenAmount = _token_Amount.add(maximumWallet);
        ...
        BakedVault(campaign_address).initilaize(_data,_token,msg.sender, baked_address, factory_owner,_token_Amount,_vesting_percentage,_lock_Duration,playshareFeesCollector,totalTokenAmount,playshare_percent );
        campaigns.push(campaign_address);
        tokenCampaign[_token] = campaign_address;


        require(IERC20(address(_token)).balanceOf(msg.sender) >= totalTokenAmount ,"You don't have sufficent token to create campaign");
        IERC20(address(_token)).transferFrom(msg.sender,address(campaign_address),totalTokenAmount);
