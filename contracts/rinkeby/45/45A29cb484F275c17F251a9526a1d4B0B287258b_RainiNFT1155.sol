...
contract RainiNFT1155 is ERC1155, AccessControl, ReentrancyGuard {
  ...
  struct Card {
    ...
    uint256 maxSupply; // number of base tokens mintable
    uint256 allocation; // number of base tokens mintable with points on this contract
    string pathUri;
  }
  ...
  uint256 public ethFeeBasisPoints = 100;
  ...
  function merge(uint256 _id, uint256 _level, uint256 _mintAmount, uint256[] memory _tokenIds, uint256[] memory _burnAmounts) 
    external {
      ...
      _mintToken(_msgSender(), _id, _level, _mintAmount, contractChar, 0);
  }

  function initCards(uint256[] memory _costInUnicorns, uint256[] memory _costInRainbows, uint256[] memory _maxMintsPerAddress,  uint16[] memory _maxSupply, uint256[] memory _allocation, string[] memory _pathUri, uint16[][] memory _conversionRates, bool[][] memory _isGrouped)
    external onlyOwner() {
  ...
  function mintWithPoints(uint256[] memory _cardId, uint256[] memory _amount, bool[] memory _useUnicorns, address[] memory _rainbowPools, address[] memory _unicornPools)
    ...
    // uint256 fee = ((unicornToEth * _totalPriceUnicorns + rainbowToEth * _totalPriceRainbows) * ethFeeBasisPoints) / 10000;
    // _amountEthToWithdraw += fee;
