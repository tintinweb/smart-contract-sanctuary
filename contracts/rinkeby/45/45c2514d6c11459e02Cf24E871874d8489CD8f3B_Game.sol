...
contract Game is Ownable {
	...
	event NewPayment(Player player, uint value);
	...
	receive() external payable {
		if(players[_msgSender()].score == 0) {
			prize.mint(_msgSender(), PARTICIPANT, 1, "");
			playerAddresses.push(_msgSender());
			players[_msgSender()] = Player(stringToBytes32("Anonymous"), 0, 0, _msgSender(), false, "");
		}
	...
	function _createPayment() private {
		...
		player.lastPayment = uint32(block.timestamp);

		lifetimeBalance += msg.value;

		uint newScore = player.score;

		emit NewPayment(player, msg.value);
