...
contract DRPToken is ERC721Enumerable, Ownable {
  ...
  uint16 public constant MAX_SUPPLY = 90;
  uint16 public constant MAX_SUPPLY_DEV = 100;
  uint8 public constant MAX_PER_ACCOUNT = 5;
  bool public salesActive = true;
  bool public revealed = false;
  bool public whitelistClaimActive = true;
  ...
  function whitelistClaimAmount(address wallet) external view returns (uint8) {
    return _whitelist[wallet];
  }

  function setWhitelist(address[] calldata addresses) external onlyOwner {
    uint8 claimAmount = MAX_PER_ACCOUNT;
    for (uint256 i = 0; i < addresses.length; i++){
      _whitelist[addresses[i]] = claimAmount;
    }
  }

  function whitelistClaim(uint8 amount) external payable {
    require(whitelistClaimActive, "Whitelist claim is not active");
    require(amount <= _whitelist[msg.sender], "Claim amount exceeded");
    require(totalSupply() + amount <= MAX_SUPPLY, "Claim: Will exceed total supply");
  ...
  function devClaim() external onlyOwner {
    for(uint16 i = MAX_SUPPLY; i < MAX_SUPPLY_DEV; i++){
