...
contract LottoWorldPolygon is ERC721Tradable, ReentrancyGuard, Factory {
    ...
    event WinnerAddress(address winner, uint amount, uint tokenID);
    ...
    constructor(
        ...
        address _payments
    ) ERC721Tradable("LottoWorld test NFT", "LottoTEST", _proxyRegistryAddress) {
    ...
    function ruffleToWinner(uint256 _boxnum, uint prize) public payable onlyOwner{
        ...
        uint256 randIndex = LootBoxRandomness._random(state) % allOwner.length;
        require(allOwner[randIndex] != address(0));
        emit WinnerAddress(allOwner[randIndex], prize);
        address payable winner = payable(allOwner[randIndex]);
        winner.transfer(msg.value);
    }

    function ruffleToNumberWinner(uint256 _boxnum, uint prize, uint ruffleNum) public payable onlyOwner{
        require(msg.value == prize);
        address[] memory allOwner = getAllOwner(_boxnum);
        uint256 randIndex = LootBoxRandomness._random(state) % ruffleNum;
        require(allOwner[randIndex] != address(0));
        emit WinnerAddress(allOwner[randIndex], prize, randIndex);
        address payable winner = payable(allOwner[randIndex]);
        winner.transfer(msg.value);
    }
}
