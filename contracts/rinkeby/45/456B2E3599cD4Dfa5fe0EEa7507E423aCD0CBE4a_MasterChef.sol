...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accSimpPerShare;   // Accumulated VONs per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The SIMP TOKEN!
    SimpliToken public simp;
    // Dev address.
    ...
    // Update the given pool's SIMP allocation point and deposit fee. Can only be called by the owner.
    ...
    function updatePool(uint256 _pid) public {
        ...
        simp.mint(address(this), simpReward);
    ...
    // Safe simp transfer function, just in case if rounding error causes pool to not have enough VONs.
    function safeSimpTransfer(address _to, uint256 _amount) internal {
        uint256 simpBal = simp.balanceOf(address(this));
        if (_amount > simpBal) {
            simp.transferWithLock(_to, simpBal);
        } else {
            simp.transferWithLock(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _simpPerBlock) public onlyOwner {
        massUpdatePools();
        simpPerBlock = _simpPerBlock;
    }

    // Setup Simpli Token Reward Release
    function setSimpRewardLock(uint256 lock) public onlyOwner {
        simp.setRewardLock(lock);
    }

    function setSimpTotalBlockRelease(uint256 totalBlockRelease) public onlyOwner {
        simp.setTotalBlockRelease(totalBlockRelease);
    }
}
