...
/// @title PriceOracle
/// @notice Uses a collection of sub price oracles to fetch settlement price and current price data for the MarketController
/// @notice The price data is for a given underlying-price token pair and Market expiration date
...
/// Markets whose expirations are Friday 8am UTC as well
...
contract PriceOracle is IPriceOracle, OwnableUpgradeable, Proxiable {
    ...
    /// @dev the time length in seconds between successive settlement dates. Should
    /// be either 1 day or 1 week
    // TODO remove this once we improve the MinterAmm tests to expect weekly offsets
    ...
    /// be either 1 day or 1 week. The only reason we allow 2 different values is for testing; the
    /// old Amm tests rely on this offset being 1 day, but when we moved to v2 protocol we changed to
    /// weekly offsets. Once we've moved the tests over we can get rid of this ambiguity and use
    /// weekly offsets everywhere.
    ...
    /// out of gas
    /// @dev This function should be called each dateOffset as soon as the block.timestamp passes Friday 8am UTC, so that
    /// the current market price we set is as close to the price at Friday 8am UTC. If we this is called at 8:30am,
    /// for instance, and the Friday 8am UTC price hasn't been set yet, then we're going to set a price that is 30
        ...
        override
    {
        ...
        uint256 priorFriday8am = get8amWeeklyOrDailyAligned(block.timestamp);
        ...
        while (currentSettlementPrice == 0) {
            ...
            // go back exactly 1 dateOffset's worth of time to the previous Friday 8am UTC date
            priorFriday8am -= dateOffset;
    ...
    /// @param underlyingToken Should be equal to the Markets' underlyingToken field
    /// @param priceToken Should be equal to the Markets' priceToken field
    /// @param settlementDate Should be equal to the expirationDate of the market calling this function
    ...
    function getCurrentPrice(address underlyingToken, address priceToken)
        ...
        returns (uint256)
    {
        // TODO abstract the oracle interface so we can use more oracles than just Chainlink
        (, int256 latestAnswer, , , ) =
    ...
    /// @dev earliestSettlementDate should be the next Market settlementDate the PriceOracle
