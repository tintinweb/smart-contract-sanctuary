...
contract testNftFactory is Ownable, ERC721Enumerable{
    ...
    // function buyPresale(bool stake,uint8 tokenType, uint tokenAmount,address receiver) external payable override {
    //     require(tokenType < 2,"Invalid type");
    //     require (HungerBrainz_MAINSALE_PRICE*tokenAmount <= msg.value, "INSUFFICIENT_ETH");
    //     require(isPresale);
    //     require(msg.sender == address(whitelist));
    //     require(userPurchase[msg.sender] + tokenAmount <= 3);
    //     // require(userPurchase[msg.sender] + tokenAmount <= 13);
    //         if(stake)
    ...
    //             game.alertStake(totalSupply());
    //             tokenOwner[totalSupply()] = receiver;
    //         }
    //         else{
    //         for (uint i =0;i<tokenAmount;i++) {
    ...
    // function buyStakeInternal(bool stake,uint8 tokenType, uint tokenAmount,address receiver) internal {
    //     address sendTo = stake ? address(game):receiver;
    //     for(uint i=0;i<tokenAmount;i++){ 
    ...
    // function buy(uint8 tokenType, uint tokenAmount) external payable {
    //     require(tokenType < 2,"Invalid type");
    ...
    // function getToken(uint256 _tokenId) external override view returns(uint8, uint8, bool, uint,uint) {
    //     _tokenId--;
    //     return (
    ...
    function _burn(uint tokenId) internal override{
        ...
        (uint8 nftType,,,,)=metadataHandler.getToken(tokenId-1);
        amount[nftType]--;
        super._burn(tokenId);
    }
}