...
import "../FlypeRouter02.sol";
import "../interfaces/IFlypeRouter02.sol";
import "./interfaces/IOracleRouter.sol";
import "./interfaces/IFlypePairRouter.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

contract FlypeUPM is Ownable{

    address public oracleRuoter;
    address public pairRouter;
    address public flypeRouter;

    constructor(address _oracleRuoter, address _pairRouter, address _flypeRouter) {
        oracleRuoter = _oracleRuoter;
        pairRouter = _pairRouter;
        flypeRouter = _flypeRouter;
    }

    /// @notice Set oracle router
    /// @param _oracleRuoter Oracle router address
    function setOracleRouter(address _oracleRuoter) external onlyOwner{
                oracleRuoter = _oracleRuoter;
    }

    /// @notice Set pair router
    /// @param _pairRouter Pair router address
    function setPairRouter(address _pairRouter) external onlyOwner{
                pairRouter = _pairRouter;
    }

    /// @notice Set Flype router
    /// @param _flypeRouter Flype router address
    function setFlypeRouter(address _flypeRouter) external onlyOwner{
                flypeRouter = _flypeRouter;
    }

    /// @notice Returns price of given lpToken
    /// @param lpPair Address of lpToken
    /// @param nominalValue decimals of lpToken 
    function getPrice(address lpPair, uint nominalValue) external view returns (uint price) {                       
        address lptoken = IFlypePair(lpPair).token0() == IFlypeRouter02(flypeRouter).WETH() ? IFlypePair(lpPair).token1() : IFlypePair(lpPair).token0();
        (uint reserveA, uint reserveB) = IFlypePairRouter(pairRouter).getReserves(lptoken);//dec 18
        // @todo implement decimals control for oracle 
        (uint priceA, uint priceB) = IOracleRouter(oracleRuoter).getPrices(lptoken);//dec 8
        
        uint totalSupply = IFlypePairRouter(pairRouter).totalSupply(lptoken);//dec 18
        price = 2 * (sqrt(reserveA * priceA * reserveB * priceB) * nominalValue / totalSupply);        
    }

    /// @notice Returns price of ether
    function getPriceETH() external view returns(uint priceETH){
        priceETH = IOracleRouter(oracleRuoter).getPriceETH();
    }
    
    /// @notice Returns square root of a given number
    /// @param y The number from which the square root is taken
...
interface IOracleRouter {
    function getPrices(address lpPair) external view returns (uint priceA, uint priceB);
    function getPriceETH() external view returns(uint priceETH);
    function setOracles(address lpPair, address token1, address token2) external;
...
interface IFlypePairRouter {
    function getReserves(address pair) external view returns (uint priceA, uint priceB);
    function totalSupply(address pair) external view returns (uint totalSupply);
...
contract FlypeLibrary is Ownable {
    ...
    function getPrices(address tokenA, address tokenB) public view returns (uint priceA, uint priceB) {
        ...
        uint price0 = IFlypeUPM(flypeUPM).getPrice(pairFor(tokenA, tokenB), nominalValue);
...
contract FlypeRouter02 is FlypeLibrary {
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = sortTokens(input, output);
            IFlypePair pair = IFlypePair(pairFor(input, output));
            uint256 amountInput;
            uint256 amountOutput;
            {
                ...
                (uint256 priceInput, uint256 priceOutput) = getPrices(input, output);
