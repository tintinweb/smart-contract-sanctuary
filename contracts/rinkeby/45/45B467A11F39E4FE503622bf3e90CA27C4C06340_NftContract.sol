...
contract NftContract is ERC721Enumerable, Ownable, Pausable, PaymentSplitter {
  ...
  constructor(
      ...
      uint256[] memory numericValues, // price - 0, maxSupply - 1, maxPerAddress - 2, publicSaleTime - 3
    ...
    updatePresaleList(presaleAddresses);
  }

  function setSaleInformation(
    uint256 publicSaleTime, bool presaleActive,
    uint256 maxPerAddress, uint256 price
  ) external onlyOwner {
  ...
  function addToPreSaleAllowList(address[] calldata addresses) external onlyOwner {
    updatePresaleList(addresses);
  }

  function updatePresaleList(address[] memory addresses) private {
  ...
  function purchase(uint256 count) public payable whenNotPaused {
    ...
    require(
      isPublicSaleActive() || (_presaleActive && _preSaleAllowList[msg.sender]),
      "BASE_COLLECTION/CANNOT_MINT"
    );

    // Sent value matches required ETH amount
    require(_price * count <= msg.value, 'BASE_COLLECTION/INSUFFICIENT_ETH_AMOUNT');
  ...
  function requireMintingConditions(address to, uint256 count) internal view {
    ...
    require (totalMintFromAddress <= _maxPerAddress, "BASE_COLLECTION/EXCEEDS_INDIVIDUAL_SUPPLY");
  }
  ...
  function MAX_TOTAL_MINT_PER_ADDRESS() external view returns (uint256) {
