...
contract LandBaseToken is ERC721BaseToken {
    ...
    // Our grid is 300 x 300 lands
    uint256 internal constant GRID_SIZE = 300;
    ...
    constructor(
        ...
        string memory symbol,
        string memory baseTokenURI
    ) ERC721BaseToken(name, symbol, baseTokenURI) {
    ...
    function toCoordinate(uint256 id) external returns(uint256 x, uint256 y) {
    ...
    function mintQuad(address to, uint256 size, uint256 x, uint256 y, bytes calldata data) external {
        ...
        for (uint256 i = 0; i < size*size; i++) {
            id = _idInPath(i, size, x, y);
            require(_owners[id] == address(0), "Already minted");
            emit Transfer(address(0), to, id);
        }

        _owners[quadId] = address(to);
    ...
    function _regroup24x24(address from, address to, uint256 x, uint256 y, bool set) internal returns (bool) {
        ...
        return ownerOfAll || _owners[quadId] == address(from);
    }
...
contract DIGILand is LandBaseToken {
    constructor(
    ) LandBaseToken("DigiNation's Lands","DIGILand", "https://api.digination.com/lands/") {
