...
contract TradeERCFund is ISimpleERCFund, Operator {
    ...
    function deposit(
        ...
        IERC20(token).safeTransferFrom(_msgSender(), address(this), amount);
        _sellCashToPrice(1e18);
        emit Deposit(_msgSender(), block.timestamp, reason);
    }

    function _sellCashToPrice(uint256 _price) internal {
        (uint256 cashReserve, uint256 usdtReserve) = UniswapV2Library
            .getReserves(factory, cash, usdt);

        if (usdtReserve.mul(1e18).div(cashReserve) <= minSellPrice) {
            return;
        }

        uint256 sellAmount = Math.sqrt(
            cashReserve.mul(usdtReserve).div(_price).div(1e18)
        );
    ...
    function sellCashToPrice(uint256 _price) public onlyOperator {
        _sellCashToPrice(_price);
    }
