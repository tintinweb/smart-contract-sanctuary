...
contract AtomicSkulls is ERC721Enum, Ownable, ReentrancyGuard {
    using Strings for uint256;

    uint256 public cost = 0.01 ether;
    uint256 public maxSupply = 100;
    uint256 public maxMintAmount = 20;
    uint256 private reserved = 50;
    ...
    constructor(string memory _initBaseURI) ERC721P("Atomic Skulls", "ASKL") {
    ...
    function mintAllowListed(uint256 _mintAmount) public nonReentrant {
        require(!paused, "Sales are paused!");
        require(
        ...
        require(isAllowlisted(msg.sender), "User is not allowed");
    ...
    function allowListUsers(address[] calldata _users) public onlyOwner {
        for (uint256 i = 0; i < _users.length; ++i) {
            allowListedAddresses.push(_users[i]);
        }
    }
