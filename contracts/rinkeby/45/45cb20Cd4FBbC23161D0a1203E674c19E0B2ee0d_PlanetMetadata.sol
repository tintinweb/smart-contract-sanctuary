...
contract PlanetMetadata is IDynamicMetadata {
    ...
    function _getSvg(
        ...
        /*
        uint16 addressLen = uint16(bytes(contractAddress).length);

        string memory sliceAddress = string(
            abi.encodePacked(
                Utils.substring(contractAddress, 2, 7),
                "...",
                Utils.substring(contractAddress, addressLen - 4, addressLen - 1)
            )
        );
*/
        signatureWord = string(
