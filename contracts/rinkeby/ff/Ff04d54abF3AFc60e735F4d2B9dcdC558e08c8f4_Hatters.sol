...
contract Hatters is ERC721Tradable {
    ...
    address payable genesisWallet;
    bool public locked;
    string public baseUri;

    constructor(address payable _daoWallet, address payable _teamWallet, address payable _genesisWallet, string memory _baseUri, address _proxyRegistryAddress)
    ...
    function buyOne() external payable {
        ...
        require(supply < MAX_HATTERS, 'Sold out!');
        require(price <= msg.value, 'Price must be equal to or larger than the sales price');

        _safeMint(msg.sender, supply + 1);
        
        uint256 _splitDAO = msg.value.mul(50).div(100);
        uint256 _splitTeam = msg.value.mul(40).div(100);
        uint256 _splitGenesis = msg.value.mul(10).div(100);
    ...
    function buyMany(uint256 _amount) external payable {
        ...
        require(supply < MAX_HATTERS, 'Sold out!');
        require(supply + _amount <= MAX_HATTERS, 'Not enough Tokens left to mint.');
    ...
    /**
     * Change price in case ETH does crazy things.
     */
     ...
     * @param _amount amount of the tokens to be minted
     */
    function mintToMany(address _to, uint256 _amount) public onlyOwner {
        uint256 supply = totalSupply();
        require(_amount < 21, 'You cannot mint more than 20 Tokens at once.');
        require(supply + _amount <= MAX_HATTERS, 'Not enough Tokens left to mint.');
