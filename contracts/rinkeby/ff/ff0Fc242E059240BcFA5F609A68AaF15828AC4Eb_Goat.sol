...
import "./interface/IProtectedIsland.sol";
...
contract Goat is IGoat, ERC721Enumerable, Ownable, Pausable {
    ...
    event TortoiseStolen(uint256 tokenId, uint256 value);
    event GoatStolen(uint256 tokenId, uint256 value);
    event TortoiseBurned(uint256 indexed tokenId);
    event GoatBurned(uint256 indexed tokenId);
    event TortoiseMinted(uint256 indexed tokenId);
    event GoatMinted(uint256 indexed tokenId);
    ...
    */
    constructor(uint256 _maxTokens) ERC721("Enchanted Game", 'EGAME') { 
    ...
    modifier requireContractsSet() {
        require(address(traits) != address(0) && address(protectedIsland) != address(0) && address(randomizer) != address(0), "Contracts not set");
        _;
    }
    ...
    function setContracts(address _traits, address _protectedIsland, address _rand) external onlyOwner {
    ...
    */
    function mint(address recipient, uint256 seed) external override whenNotPaused {
        ...
        if(tx.origin != recipient && recipient != address(protectedIsland)) {
            // Stolen!
            if(tokenTraits[minted].isTortoise) {
                totalTortoiseStolen += 1;
                emit TortoiseStolen(minted, block.timestamp);
            }
            else {
                totalGoatStolen += 1;
                emit GoatStolen(minted, block.timestamp);
            }
        }
    ...
    */
    function burn(uint256 tokenId) external override whenNotPaused {
        ...
        if(tokenTraits[tokenId].isTortoise) {
            emit TortoiseBurned(tokenId);
        }
        else {
            emit GoatBurned(tokenId);
        }
        _burn(tokenId);
    }
    ...
    function generate(uint256 tokenId, uint256 seed, LastWrite memory lw) internal returns (GoatTortoise memory t) {
        ...
        if (existingCombinations[structToHash(t)] == 0) {
            ...
            if (t.isTortoise) {
                ...
                emit TortoiseMinted(tokenId);
            } else {
                totalGoatMinted += 1;
                emit GoatMinted(tokenId);
            }
            return t;
        }
    ...
    */
    function isTortoise(uint256 tokenId) external view override returns (bool) {
        // Sneaky dragons will be slain if they try to peep this after mint. Nice try.
        IGoat.GoatTortoise memory s = tokenTraits[tokenId];
        return s.isTortoise;
    }
