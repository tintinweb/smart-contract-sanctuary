...
contract Artieverse is ERC721, ERC721Enumerable, Pausable, AccessControl, ERC721Burnable, Mintable, Ownable, IERC2981 {
    ...
    uint24 private _royaltyPercent;
    ...
    constructor(string memory tokenName, string memory symbol) ERC721(tokenName, symbol) Ownable() {
    ...
    function mintFor(address to, uint256 amount, bytes calldata mintingBlob) external override {
        require(hasRole(MINTER_ROLE, msg.sender));
        require(!paused(), "PAUSABLE_PAUSED");

        string memory mintingBlobStr = string(mintingBlob);
        // console.logString(mintingBlobStr);

        uint pos = 0;
        while(mintingBlob[pos] != ':') {
            pos++;
        }

        string memory tokenIdStr = String.substring(mintingBlobStr, 0, pos);

        uint tokenId = String.toUint(tokenIdStr);

        _safeMint(to, tokenId);
        emit MintFor(to, amount, tokenId);
    }
    ...
    returns (address receiver, uint256 royaltyAmount) {
            return (_royaltyAddress, (salePrice * _royaltyPercent) / 10000);
    }

    function setRoyaltyAddress(address royaltyAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _royaltyAddress = royaltyAddress;
    }

    /// @dev Sets token royalties
    /// @param value percentage (using 2 decimals: 10000 = 100%, 0 = 0%)
    function setRoyalties(uint256 value) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(value <= 10000, 'ERC2981Royalties: Too high');
        _royaltyPercent = uint24(value);
    }
    ...
    function contractBalance() external view onlyRole(DEFAULT_ADMIN_ROLE) returns (uint) {
    ...
    function transferERC721(address contractAddress, address to, uint256 tokenId) public onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        erc721Contract.safeTransferFrom(contractAddress, to, tokenId);
    }

    function transferERC20(address contractAddress, address to, uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
        IERC20 erc20Contract = IERC20(contractAddress);
        uint256 erc20balance = erc20Contract.balanceOf(address(this));
        require(amount <= erc20balance, "ERC20_BALANCE_LOW");
...
interface Mintable {
    function mintFor(address to, uint256 id, bytes calldata mintingBlob) external;
}
