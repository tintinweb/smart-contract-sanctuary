...
contract CoinDeed is ICoinDeed, AccessControl {
    ...
    function reserveWholesale(uint256 saleId) external override {
        ...
        require(wholesale.tokenOffered == pair.tokenB, "tokens does not match");
        require(wholesale.tokenRequested == pair.tokenA, "Requested token not match");
        require(wholesale.minSaleAmount <= deedParameters.deedSize, "deedsize is too low");
    ...
    function exitDeed(bool _payoff) onlyInState(DeedState.OPEN) payable public override {
        ...
        if (depositAmount > 0) {
            lendingPool.withdraw(pair.tokenB, depositAmount);
        }
        ...
        if(_payoff) {
            if(pair.tokenA == address(0x00)) {
                require(msg.value == buyerLoan, "Buyer not pay enough ether");
            }
            else {
            ...
            coinDeedFactory.emitPayOff(msg.sender, exitAmount);
        }
    ...
    function withdrawStake() external override {
        require(CoinDeedUtils.withdrawStakeCheck(state, executionTime, stakes[msg.sender], msg.sender == manager), "Cannot withdraw");
    ...
    function sell() onlyInState(DeedState.OPEN) external override {
        ...
        uint256 amountToSwap = lendingPool.totalDepositBalance(pair.tokenB, address(this));
    ...
    function executeRiskMitigation() external override onlyInState(DeedState.OPEN) {
        ...
        // Provide address provider to pass oracle
         uint256 sellAmount = coinDeedAddressesProvider.checkRiskMitigationAndGetSellAmount(
            pair, riskMitigation, totalDeposit, totalBorrow, riskMitigationTriggered);
        // Commence the sell and repayment
        if (sellAmount > 0) {
            uint256 receivedAmount = _sell(sellAmount);
            if (totalBorrow > receivedAmount) {
                _repay(receivedAmount);
            } else {
                _repay(totalBorrow);
            }
        }
    ...
    function _editExecutionTime(ExecutionTime memory executionTime_) internal {
        require(executionTime_.recruitingEndTimestamp > block.timestamp, "RecruitingEnd must in future.");
        require(executionTime_.recruitingEndTimestamp < executionTime_.buyTimestamp, "RecruitingEnd must be lt buy");
        require(executionTime_.buyTimestamp < executionTime_.sellTimestamp, "Buy must be lt sell.");
    ...
    function _editRiskMitigation(RiskMitigation memory riskMitigation_) internal {
        ...
        require(
            (riskMitigation_.secondTrigger <= coinDeedFactory.maxPriceDrop() / deedParameters.leverage) &&
...
interface IWholesaleFactory {
    ...
    */
    ...
    function edit(uint256 saleId, bool isPrivate, uint256 deadline, uint256 minSaleAmount) external;
}
