...
contract OriginalsPrimaDonna is ERC721, ERC721Enumerable, ERC721URIStorage, AccessControl, Pausable, ReentrancyGuard {
  ...
  Counters.Counter private _goldTokenIdCounter;
  ...
  /// @notice The address use to withdraw any balance to
  address payable public immutable DAO_ADDRESS;

  // ========== Mutable Variables ==========

  string public baseURI;
  uint16 maximumSupply;
  uint16 maximumGoldSupply;
  ...
  constructor(
    ...
    ) ERC721("ApeDAO Collective", "APEDCOLLECTIVE") {
    ...
    walletLimitByWindow[TokenRequirementType.GOLD] = 20;
  ...
  function mint(uint256 _quantity) internal {
    if(mintTokenRequirement == TokenRequirementType.GOLD) {
      require(_goldTokenIdCounter.current() + _quantity <= maximumGoldSupply, "Quantity must be less than remaining supply for Gold tokens");
    }

    require(_quantity > 0, "Quantity must be greater than 0");
    require(_tokenIdCounter.current() + _quantity <= maximumSupply, "Quantity must be less than remaining supply");

    uint16 walletLimit = walletLimitByWindow[mintTokenRequirement];

    if(mintTokenRequirement == TokenRequirementType.GOLD && doubleGoldWalletHolder[msg.sender]) {
    ...
    for(uint8 i = 0; i < _quantity; i++) {
      ...
      if(mintTokenRequirement == TokenRequirementType.GOLD) {
        _goldTokenIdCounter.increment();
      }

      numMintsPerAddress[msg.sender][mintTokenRequirement] += 1;
  ...
  function withdraw() public onlyAdmin {
    Address.sendValue(payable(DAO_ADDRESS), address(this).balance);
  }
