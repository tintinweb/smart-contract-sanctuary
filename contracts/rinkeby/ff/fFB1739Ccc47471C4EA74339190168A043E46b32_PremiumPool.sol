...
contract PremiumPool is IPremiumPool, ReentrancyGuard {
    ...
    function depositToSyntheticSSRPRewarder(address _rewarder) external payable onlyOwner nonReentrant {
        ...
        uint256 ethAmountToDeposit = 0;
        if (SSRP_PREMIUM_ETH > 0) {
            ethAmountToDeposit += SSRP_PREMIUM_ETH;
            SSRP_PREMIUM_ETH = 0;
        }
        for (uint256 ii = 0; ii < availableCurrencyList.length; ii++) {
            if (SSRP_PREMIUM[availableCurrencyList[ii]] > 0) {
                if (
                    IERC20(availableCurrencyList[ii]).allowance(address(this), exchangeAgent) <
                    SSRP_PREMIUM[availableCurrencyList[ii]]
                ) {
                    TransferHelper.safeApprove(availableCurrencyList[ii], exchangeAgent, MAX_INTEGER);
                }
                uint256 convertedETHAmount = IExchangeAgent(exchangeAgent).convertForETH(
                    availableCurrencyList[ii],
                    SSRP_PREMIUM[availableCurrencyList[ii]]
                );
                ethAmountToDeposit += convertedETHAmount;
                SSRP_PREMIUM[availableCurrencyList[ii]] = 0;
            }
        }
        if (ethAmountToDeposit > 0) {
            TransferHelper.safeTransferETH(_rewarder, ethAmountToDeposit);
            emit LogDepositToSyntheticSSRPRewarder(_rewarder, ethAmountToDeposit);
        }
    }
