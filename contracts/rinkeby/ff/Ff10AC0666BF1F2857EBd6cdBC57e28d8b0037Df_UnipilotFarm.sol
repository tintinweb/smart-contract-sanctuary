...
contract UnipilotFarm is IUnipilotFarm, ReentrancyGuard, IERC721Receiver {
        ...
        returns (bool)
    {
        ...
        (, , , , , , , , , , uint256 totalLiquidity) = IUniswapLiquidityManager(ulm)
    ...
    function checkLimit(uint256 _tokenId, DirectTo _status)
        internal
        view
        returns (uint256 globalReward)
    {
        ...
        if (_status == DirectTo.GRforPilot) {
            poolState = TempInfo({
        ...
        } else if (_status == DirectTo.GRforAlt) {
            poolState = TempInfo({
    ...
    function toggleActiveAlt(address _pool, address _altTokenAddress)
        external
        onlyGovernance
        returns (bool)
    {
        ...
        if (poolState.isAltActive && _altTokenAddress == address(0)) {
            poolState.isAltActive = !poolState.isAltActive;
            poolAltState.altToken = _altTokenAddress;
        } else {
            poolState.isAltActive = true;

            poolAltState = PoolAltInfo({
                globalReward: 0,
                lastRewardBlock: block.number,
                altToken: _altTokenAddress,
                startBlock: block.number
            });
