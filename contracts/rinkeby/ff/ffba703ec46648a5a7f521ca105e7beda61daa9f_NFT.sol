...
contract NFT is ERC721, Ownable {
     ...
     */
     function checkPriceForETH(uint256 numberOfNfts) public view returns (uint256) {
         uint256 currentSupply = totalSupply().add(numberOfNfts);
         uint256 newSupply = numberOfNfts;
        if(currentSupply >=8000 && currentSupply < (numberOfNfts.add(8000)))
        {
            if(currentSupply == 8000 && currentSupply < (numberOfNfts.add(8000)))
                newSupply = currentSupply.sub(7999);
            else
                newSupply = currentSupply.sub(8000);
        }
        else if(currentSupply >=3000  && currentSupply < (numberOfNfts.add(3000)))
        {   
            if(currentSupply == 3000 && currentSupply < (numberOfNfts.add(3000)))
                newSupply = currentSupply.sub(2999);
            else
                newSupply = currentSupply.sub(3000);
        }
        
        return getNFTPriceForETH(newSupply, currentSupply).add(getNFTPriceForETH((numberOfNfts.sub(newSupply)), totalSupply()));
     }
     
    /**
    * @dev check supply with numberOfNfts
    */
    function checkPriceForLEASH(uint256 numberOfNfts) public view returns (uint256) {
        
        uint256 currentSupply = totalSupply().add(numberOfNfts);
        uint256 newSupply = numberOfNfts;
        if(currentSupply >=9000 && currentSupply < (numberOfNfts.add(9000)))
        {
            if(currentSupply == 9000 && currentSupply < (numberOfNfts.add(9000)))
                newSupply = currentSupply.sub(8999);
            else
                newSupply = currentSupply.sub(9000);
        }
        else if(currentSupply >=3000  && currentSupply < (numberOfNfts.add(3000)))
        {   
            if(currentSupply == 3000 && currentSupply < (numberOfNfts.add(3000)))
        ...
        return getNFTPriceForLEASH(newSupply, currentSupply).add(getNFTPriceForLEASH((numberOfNfts.sub(newSupply)), totalSupply()));
    }
    
    /**
     * @dev Gets current NFT Price
     */
    function getNFTPriceForLEASH(uint256 numberOfNfts,uint256 currentSupply) internal view returns (uint256) {
        require(block.timestamp >= SALE_START_TIMESTAMP, "Sale has not started");
        require(currentSupply < MAX_NFT_SUPPLY, "Sale has already ended");

        if (currentSupply >= 9000) {
            uint256 num =7 ; 
            return (getLeashEthPrice().mul(num).div(10)).mul(numberOfNfts); // 9000 - 1000 0.7 LEASH
        } else if (currentSupply >= 3000) {
            uint256 num =5 ;
            return (getLeashEthPrice().mul(num).div(10)).mul(numberOfNfts); // 3000 - 8999 0.5 LEASH
        } else {
            uint256 num =3 ;
            return (getLeashEthPrice().mul(num).div(10)).mul(numberOfNfts); // 0 - 2999 0.3 LEASH
        }
    }
    
    function getNFTPriceForETH(uint256 numberOfNfts,uint256 currentSupply) internal view returns (uint256) {
        require(block.timestamp >= SALE_START_TIMESTAMP, "Sale has not started");
        ...
        if (currentSupply >= 8000) {
            return numberOfNfts.mul(500000000000000000); // 9000 - 1000 0.5 ETH
        } else if (currentSupply >= 3000) {
            return numberOfNfts.mul(300000000000000000); // 3000 - 7999 0.3 ETH
        } else {
            return numberOfNfts.mul(100000000000000000); // 0 - 2999 0.1 ETH
        }
    }

    function checkNameChangePrice() public view returns (uint256) {
    ...
    function changeName(uint256 tokenId, string memory newName) public {
        ...
        SHIB.transferFrom(msg.sender, address(this), checkNameChangePrice());
        ...
        ERC20Burnable(address(SHIB)).burn(checkNameChangePrice());
    ...
    */
    function mintNFT(uint numberOfNfts, uint256 _value, bool _isETH) public payable {
        ...
        {
            require(checkPriceForLEASH(numberOfNfts) == _value, "LEASH value sent is not correct");
        ...
        {
            require(checkPriceForETH(numberOfNfts) == msg.value, "ETH value sent is not correct");
        }
