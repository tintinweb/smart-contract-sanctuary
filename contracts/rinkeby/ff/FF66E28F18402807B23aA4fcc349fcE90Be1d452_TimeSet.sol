...
interface IsYSL{
    function YSLSupply() external returns(uint);
...
interface IReferral{
    function hasReferral(address) external view returns(bool);
    function proccessReferral(address, address, bytes32) external;
...
/// @author Anna Korunska & Artem Martiukhin
...
contract PriceDiscovery is Ownable {
    ...
    enum Stage{
        PrivateSale, Pub1, Pub2, Pub3, Pub4, Pub5, Pub6, Pub7, Referral, Whitelist, Closed
    }
    enum Token{
        BUSD, WBNB
    }
    ...
    uint constant public COEFFICIENT_DECIMALS = 10**6;

    mapping(address=>bool) whitelist;
    /// @notice When depositing funds during discovery, 
    /// users receive additional bonuses depending on a discovery day.
    uint[] public bonusCoefficient 
        = [3000000, 2000000, 1800000, 1600000, 1500000, 1300000, 1200000, 1000000, 100000, 4000000];
    ...
    /// @param _sYSLInitialTotalSupply sYSL token initial supply.
        ...
        uint _privateStartTime)
    {
        ...
        initialTotalSupply = _sYSLInitialTotalSupply;
    ...
    /// @notice Call to withdraw deposited tokens after discovery is finished.
    /// @param amount Amount of tokens to withdraw.
    /// @param to Address to send assets to.
    function withdrawBUSDTo(uint amount, address to) external onlyOwner {
       IERC20(tokens[uint(Token.BUSD)]).safeTransfer(to, amount);
    }
    ...
    function deposit(uint amount, Token token, bytes32 signature, address creator) external discoveryCurrent supportedToken(token) {
        bool hasRef = IReferral(referralContract).hasReferral(_msgSender());
        uint256 usdDeposited = _deposit(amount, token);

        if ( creator!=address(0) && !hasRef ) {
            IReferral(referralContract).proccessReferral(_msgSender(), creator, signature);
        }
        if ( hasRef ) {
            amountDepositedByUserAndStage[creator][uint8(Stage.Referral)] +=
                            usdDeposited * bonusCoefficient[uint8(Stage.Referral)] / COEFFICIENT_DECIMALS;
        }
    }

    function _deposit(uint amount, Token token) internal returns(uint256) {
        Stage stage = getCurrentStage(); // stage is already private or public
        ...
        require(usdDeposited>=minimumDeposit[uint(stage)], "Not enough to deposit on current sale day");
        
        amountOfReceivedBonuses = usdDeposited*bonusCoefficient[uint8(stage)]/COEFFICIENT_DECIMALS;
        if(stage==Stage.PrivateSale && whitelist[_msgSender()]){
          amountOfReceivedBonuses = usdDeposited*bonusCoefficient[uint8(Stage.Whitelist)]/COEFFICIENT_DECIMALS;
        }
        amountWithBonusesByUserAndStage[_msgSender()][uint8(stage)] =
                    amountWithBonusesByUserAndStage[_msgSender()][uint8(stage)] + amountOfReceivedBonuses;
        amountDepositedByUserAndStage[_msgSender()][uint8(stage)] =
    ...
    function sYSLwithdraw() external discoveryFinished {
        ...
        if (amountDeposited > 0) {
            ...
            uint256 sYSLamount = (amountDeposited / getDiscoveredPrice())*10**18;
    ...
    /// @return recalculated sYSL initial price
     function getDiscoveredPrice() public view returns(uint256){
        return totalBUSDcollected / initialTotalSupply;
    }

    /// @notice Call to calculate sYSL supply.
    /// @return Recalculated initial sYSL supply
     function getDiscoveredSupply() public view returns(uint256){
        return totalBUSDwithBonusesCollected / getDiscoveredPrice();
    }
    ...
    function getLockTimeByUser(address _user) public view returns(uint256) {
        ...
        for (uint8 i = 0; i < uint8(Stage.Closed); i++) {
            uint256 curStageLock = lockTime[i] * amountWithBonusesByUserAndStage[_user][i] / total;
    ...
    function getCurrentStage() public view returns(Stage _stage) {
        if (block.timestamp < privateSaleStartTime ||
            ...
            _stage = Stage((block.timestamp-priceDiscoveryStartTime)/(1 days));
        }
    }
...
contract TimeSet is PriceDiscovery{
    ...
    function setLockTime(uint stage, uint time) public{
        lockTime[stage]=time;
    }

    function clearAll() public{
        for(uint8 i=0; i<=7; i++){
        totalBUSDcollected -= amountDepositedByUserAndStage[_msgSender()][i];
        amountDepositedByUserAndStage[_msgSender()][i]=0;
        amountWithBonusesByUserAndStage[_msgSender()][i]=0;
        }
        uint burned=IERC20(sYSL).balanceOf(_msgSender());
        IERC20(sYSL).transferFrom(_msgSender(), address(0), burned);


        withdrawn[_msgSender()] = false;
    }
}