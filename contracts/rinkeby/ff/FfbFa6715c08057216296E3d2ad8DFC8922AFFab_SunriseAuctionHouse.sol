    ...
    OwnableUpgradeable
{
    ...
    function _settleAuction() internal {
        ...
        if (_auction.amount > 0) {
            withdraw(_auction.amount);
        }

        address sunriseArtClub = 0x2EfDC5AEC299BF959cb0f0D8fF42268686731614; // Sunrise Art Club

        // Send to Sunrise Art Club wallet if bids have been placed
        address winner = _auction.bidder == address(0) ? sunriseArtClub : _auction.bidder;
    ...
    function withdraw(uint256 amount) public payable {
        ...
        require(payable(_sunriseArtClub).send((amount * 7500) / 10000)); // 75%
        require(payable(_swopes).send((amount * 1050) / 10000)); // 10.5%
        require(payable(_lauren).send((amount * 625) / 10000)); // 6.25%
        require(payable(_eddie).send((amount * 250) / 1000)); // 2.5%
        require(payable(_dan).send((amount * 250) / 10000)); // 2.5%
        require(payable(_ann).send((amount * 125) / 10000)); // 1.25%
        require(payable(_jetzi).send((amount * 100) / 10000)); // 1%
        require(payable(_community).send((amount * 100) / 10000)); // 1%
    }
}
