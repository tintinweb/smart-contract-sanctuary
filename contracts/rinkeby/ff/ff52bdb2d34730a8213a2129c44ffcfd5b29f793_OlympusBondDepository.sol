...
contract OlympusBondDepository is Ownable {
    ...
    event BondCreated( uint indexed deposit, uint indexed payout, uint indexed expires, uint price, uint debtRatio );
    event BondRedeemed( uint indexed payout, uint indexed debtRatio );
    ...
    /**
        ...
        @param _DAOShare uint
        @return bool
     */
    function setBondTerm( 
        uint _controlVariable, // Scaling variable for bond price
        uint _vestingTerm, // Length in blocks for bond to vest
        uint _minimumPrice, // Lowest price a bond can be purchased
        uint _maxPayout, // Maximum amount (as % of circ supply) a bond can pay out
        uint _DAOShare // Amount of profits DAO takes
    ...
    function deposit( 
        ...
        uint newValue = info.valueRemaining.add( value );
        uint newPayout = info.payoutRemaining.add( payout );
        uint newVesting = info.vestingPeriod.add( terms.vestingTerm.mul( value ).div( newValue ) );

        // Store depositor info
        bondInfo[ _depositor ] = Bond({
            valueRemaining: newValue, // add on to previous amount if exists
            payoutRemaining: newPayout, // add on to previous amount if exists
            vestingPeriod: newVesting,
            lastBlock: block.number,
            pricePaid: price
        });

        emit BondCreated( _amount, payout, block.number.add( newVesting ), price, _calcDebtRatio() );
        adjust();
        return payout;
    }
    ...
    function redeem( bool _stake ) external returns ( uint ) {        
        ...
        if ( percentVested >= 10000 ) { // if fully vested, pay full amount & clear info
            ...
            return stakeOrSend( _stake, info.payoutRemaining );
        } else {
    ...
    function stakeOrSend( bool _stake, uint _amount ) internal returns ( uint ) {
        emit BondRedeemed( _amount, _calcDebtRatio() );
    ...
    function bondPrice() public view returns ( uint price_ ) {        
        price_ = terms.controlVariable.mul( _calcDebtRatio() ).add( 1000000000 ).div( 1e7 );
    ...
    /**
        ...
        @return price_ uint
     */
    function pureBondPrice() external view returns ( uint price_ ) {
        price_ = terms.controlVariable.mul( _calcDebtRatio() ).add( 1000000000 ).div( 1e7 );
    }
    ...
    function debtRatio() external view returns ( uint ) {
        return _calcDebtRatio();
    }

    function _calcDebtRatio() internal view returns ( uint debtRatio_ ) {   
