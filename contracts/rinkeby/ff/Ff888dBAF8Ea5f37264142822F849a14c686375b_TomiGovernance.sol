...
contract TomiGovernance is TgasStaking, Ownable {
    ...
    function createRevenueBallot(uint _amount, bool _wallet, string calldata _subject, string calldata _content) external returns (address) {
        ...
        uint endTime = block.timestamp + getConfigValue(ConfigNames.VOTE_DURATION);
        uint executionTime = endTime + getConfigValue(ConfigNames.EXECUTION_DURATION);

        if(_amount > 0) {
            _amount = _transferForBallot(_amount, _wallet, executionTime);
	...
	function listToken(address _token, uint _amount, bool _wallet, string calldata _subject, string calldata _content) external returns (address) {
        ...
        uint executionTime = endTime + getConfigValue(ConfigNames.EXECUTION_DURATION);
        if(_amount > 0) {
            applyTokenOf[msg.sender][_token] = _transferForBallot(_amount, _wallet, executionTime);
        }
...
contract TgasStaking is BaseToken {
    ...
    function estimateLocktime(address user, uint _endTime) internal view returns(uint) {
        ...
        return _endTime > allowance[user] ? _endTime: allowance[user]; 
    }
