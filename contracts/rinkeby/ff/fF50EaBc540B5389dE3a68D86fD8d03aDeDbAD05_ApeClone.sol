...
contract ApeClone is ERC721Enumerable, Ownable {
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(block.timestamp >= launchDate, "Error: Launch Date is not satisfied");
    require(_mintAmount > 0, "Error: Mint amount has to be bigger than 0");
    require(_mintAmount <= maxMintAmount, "Error: Maximum NFT mint amount exceeded");
    require(supply + _mintAmount <= maxSupply, "Error: Sorry we are sold out");
    ...
    returns (string memory)
  {
    ...
    if(block.timestamp <= revealDate || totalSupply() >= maxSupply) {
  ...
  function updateMaxSupply(uint256 _addAmount) public onlyOwner {
    maxSupply += _addAmount;
  }
