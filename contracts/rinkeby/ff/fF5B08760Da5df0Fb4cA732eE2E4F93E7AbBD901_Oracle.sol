...
contract Oracle is Epoch {
    ...
    function initialize() external onlyOperator
    {
        ...
        staking = pairStakingtoPeg.token0();
        peg = pairStakingtoPeg.token1();
        share = pairPegtoShare.token1();

        priceStakingCumulativeLast = pairStakingtoPeg.price0CumulativeLast(); // fetch the current accumulated price value (1 / 0)
        pricePegCumulativeLast = pairStakingtoPeg.price1CumulativeLast(); // fetch the current accumulated price value (0 / 1)
        priceShareCumulativeLast = pairPegtoShare.price1CumulativeLast();
    }
