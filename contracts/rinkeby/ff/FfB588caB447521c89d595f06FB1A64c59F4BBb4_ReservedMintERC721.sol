...
contract ReservedMintERC721 is ERC721Enumerable, Ownable {
    ...
    uint256 public maxReservePerTransaction = 10;
    uint256 public maxReservePublic = 20;
    ...
    // does not check against total supply as presale spots are curated manually and this saves some gas
    function reservePresale(uint256 reserveAmount) external payable {
        require(presaleAllowed, "Presale has not started yet");
        require(reserveAmount > 0, "Must reserve at least one");
        require(presaleWhitelist[_msgSender()], "Wallet is not on whitelist");
    ...
    function reservePublic(uint256 reserveAmount) external payable {
        require(publicSaleAllowed, "Public sale has not started yet");
        require(reserveAmount > 0, "Must reserve at least one");
    ...
    function claimPresale() external {
        ...
        require(walletReservedPresaleClaims > 0, "Nothing to claim");

        for (uint256 i = 0; i < walletReservedPresaleClaims; i++) {
            uint256 mintIndex = totalSupply().add(1);
            reservedPresaleClaims[_msgSender()] = reservedPresaleClaims[_msgSender()].sub(1);
    ...
    function claimAll() external {
        ...
        require(walletReservedPublicClaims.add(walletReservedPresaleClaims) > 0, "Nothing to claim");
        ...
        for (uint256 i = 0; i < walletReservedPublicClaims; i++) {
            uint256 mintIndex = totalSupply().add(1);
            reservedPublicClaims[_msgSender()] = reservedPublicClaims[_msgSender()].sub(1);
            _safeMint(_msgSender(), mintIndex);
        }
    }

    function addToPresaleWhitelist(address[] calldata walletAddresses) external onlyOwner {
        for (uint256 i = 0; i < walletAddresses.length; i++) {
            presaleWhitelist[walletAddresses[i]] = true;
        }
    }

    function removeFromPresaleWhitelist(address[] calldata walletAddresses) external onlyOwner {
        for (uint256 i = 0; i < walletAddresses.length; i++) {
            presaleWhitelist[walletAddresses[i]] = false;
        }
    }

    function isInPresaleWhitelist(address walletAddress) external view returns (bool) {
        return presaleWhitelist[walletAddress];
    }
