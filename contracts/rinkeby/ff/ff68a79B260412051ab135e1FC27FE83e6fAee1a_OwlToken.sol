...
contract OwlToken is ERC20Pausable, CollaborativeOwnable {
  ...
  address public nightOwlsContract = address(0);
  address public alphaPassContract = address(0);
  ...
  function onAlphaPassMint(address addr, uint256 tokenAmount) external {
    require(_msgSender() == alphaPassContract, "forbidden");
    require(balanceOf(addr) >= tokenAmount, "insufficent");

    _transfer(addr, alphaPassContract, tokenAmount);
  }

  // Called by the NightOwls contract when a token is transferred
  function onNightOwlTransfer(address from, address to) external {
    require(_msgSender() == nightOwlsContract, "forbidden");
  ...
  function getPendingReward(address addr) internal view returns(uint256) {
    require(nightOwlsContract != address(0), "erc721 contract not set");

    return IERC721(nightOwlsContract).balanceOf(addr) *
      rate *
