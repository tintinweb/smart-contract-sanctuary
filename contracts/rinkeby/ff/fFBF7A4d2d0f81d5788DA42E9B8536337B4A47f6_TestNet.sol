...
contract TestNet is Context, IERC20, Ownable {
    ...
    function excludeFromReward(address account) external onlyOwner {
        require(account != 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); // UniswapV2 router
    ...
    function addBots(address[] memory botAddresses) external onlyOwner {
        for (uint256 i = 0; i < botAddresses.length; i++) {
            require(
                botAddresses[i] != 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
            ); // UniswapV2 router
    ...
    function swapAndLiquify(uint256 takeAmount) public noReentrant {
        // Marketing vs Liquidity allocation.
        uint256 marketingPecentage = _marketingFee.mul(10000).mul(10**9).div(
            _marketingFee.add(_liquidityFee)
        );
        uint256 toMarketing = marketingPecentage.mul(takeAmount).div(10000).div(
            10**9
        );
        ...
        // Recent ETH Balance *  50% of the LP Alloc Tokens  / Marketing + Tokens That were made for ETH
        uint256 ethLpPart = ethRecivedFromSwap.mul(tokenHalfForETH).div(
            toSwapForEth
        );
    ...
    function setSwapLiquify() external onlyOwner {
        ...
        )
    {
        ...
        uint256 tMarketingLiquidityFee = tAmount.mul(marketingLiquidityFee).div(
            100
        );
