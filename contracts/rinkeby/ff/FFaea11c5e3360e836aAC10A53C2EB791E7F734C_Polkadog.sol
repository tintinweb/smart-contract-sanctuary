...
contract Polkadog is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    function getTxCount(address from) public view returns(uint256) {
        return _totalTransactions[from];
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if(_takeBothTax) {
            (tTransferAmount, tFee, tLiquidity, tBurn, tMarketingFee) = _getTValues(tAmount, recipient);
            (rAmount, rTransferAmount, rFee) = _getRValues(tAmount, tFee, tLiquidity, tBurn, tMarketingFee, recipient);
        } else {
            (tTransferAmount, tFee, tLiquidity, tBurn, tMarketingFee) = _getTValues(tAmount, recipient);
            (rAmount, rTransferAmount, rFee) = _getRValues(tAmount, tFee, tLiquidity, tBurn, tMarketingFee, recipient);
        }
