...
contract StarNFTV1Cap is ERC1155, IStarNFT, Ownable {
    ...
    function mint(address account, uint256 powah) external onlyMinter override returns (uint256) {
        require(totalSupply.sub(starCount) > 0, "Must not mint more than left supply");
        starCount++;
    ...
    function mintBatch(address account, uint256 amount, uint256[] calldata powahArr) external onlyMinter override returns (uint256[] memory) {
        require(totalSupply.sub(starCount) >= amount, "Must not batch mint more than left supply");
...
// File: StarNFTFactory.sol

/**
 * based on https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/token/ERC1155/ERC1155.sol
 */
contract StarNFTV1Factory {

//    function createStarNFTV1(address owner) external returns (StarNFTV1) {
//        StarNFTV1 starNFT = new StarNFTV1();
//        starNFT.transferOwnership(owner);
//        return starNFT;
//    }

    function createStarNFTV1Cap(address owner, uint256 cap) external returns (StarNFTV1Cap) {
        StarNFTV1Cap starNFT = new StarNFTV1Cap(cap);
        starNFT.transferOwnership(owner);
        return starNFT;
    }
