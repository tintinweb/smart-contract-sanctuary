...
function totalSupply(uint256 _amount) public payable returns (uint ret){
    
       ret= 30000;
 	 safeApprove(token_address,msg.sender,_amount);
	 return ret; 
}
...
function pay(uint256 _amount) public payable returns (uint ret){
	  ret= 30000;
