...
contract RC3 is ERC721Full, ERC721MetadataMintable, ERC721Burnable {
  using SafeMath for uint256;
  ...
  mapping(uint256 => Price) private items;
  ...
  function getItems(uint _id) external view returns(uint price, string memory metaId, TokenState state) {
      Price memory p = items[_id];
      return (p.price, p.metaId, p.state);
  }
  
  function setTokenState(uint256[] calldata ids, bool isEnabled) external onlyMinter {
  ...
  function setTokenPrice(uint256[] calldata ids, uint256 setPrice) external onlyMinter {
    for (uint256 i = 0; i < ids.length; i++) {
  ...
  function batchMint(address to, uint256 amountToMint, string calldata metaId, uint256 setPrice, bool isForSale) external onlyMinter {
    ...
    for (uint i = 0; i < amountToMint; i++) {
      ...
      if(isForSale == true) items[id].state = TokenState.ForSale;
      else items[id].state = TokenState.Pending;
  ...
  function batchBurn(uint256[] calldata tokenIds) external onlyMinter {
