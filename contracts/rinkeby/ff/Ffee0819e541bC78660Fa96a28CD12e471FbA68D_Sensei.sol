...
contract Sensei is Context, IERC20, Ownable {
    ...
    uint256 public _maxTxAmount = 500 * 10**9; // 500 (+ 9 decimals)
    uint256 private numTokensSellToAddToLiquidity = 5 * 10**9;  // 500 (+ 9 decimals)
    ...
    constructor () public {
       ...
       // _tresorWalletAddress = 0x81A86F040CDA219d9c52dC690aD58A70903cAB68;
        ...
        emit Transfer(address(0), msg.sender, _rTotal); // Team - Anubis - 1%
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256[3] memory rValues, uint256[5] memory tValues) =  _getValues(tAmount);
        uint256 rAmount = rValues[0];
        uint256 rTransferAmount = rValues[1];
    ...
    function _getValues(uint256 tAmount) private view returns (uint256[3] memory, uint256[5] memory) {
        uint256[5] memory tValues = _getTValues(tAmount);
        uint256[3] memory rValues = _getRValues(tAmount, tValues[1], tValues[2], tValues[3], tValues[4], _getRate());

        return (rValues, tValues);
    }

    function _getTValues(uint256 tAmount) private view returns (uint256[5] memory) {
        uint256[5] memory tValues = [0, calculateTaxFee(tAmount), calculateLiquidityFee(tAmount), calculateMarketingFee(tAmount), calculateBurnFee(tAmount)];
        tValues[0] = _getTTransferAmount(tAmount, tValues);

        return tValues;
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 tBurn, uint256 currentRate) private pure returns (uint256[3] memory) {
        ...
        uint256[5] memory tempRValues = [rAmount, rLiquidity, rFee, rMarketing, rBurn];
    ...
    function _getTTransferAmount(uint256 tAmount, uint256[5] memory tValues) private pure returns(uint256) {
        // return tAmount.sub(tFee).sub(tLiquidity).sub(tMarketing).sub(tBurn).sub(tTresor);
        return tAmount.sub(tValues[1]).sub(tValues[2]).sub(tValues[3]).sub(tValues[4]);
    }

    function _getRTransferAmount(uint256[5] memory rValues) private pure returns(uint256) {
        return rValues[0].sub(rValues[2]).sub(rValues[1]).sub(rValues[3]).sub(rValues[4]);
    }
