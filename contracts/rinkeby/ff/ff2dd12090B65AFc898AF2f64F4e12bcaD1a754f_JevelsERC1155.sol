...
import "./extensions/ERC1155TokenRecipient.sol";
import "./proxy/OwnedUpgradabilityProxy.sol";

contract AuthenticatedProxy is OwnedUpgradabilityProxy {

    constructor(address owner, address initialImplementation, ProxyRegistry addrRegistry)
    {
        setUpgradeabilityOwner(owner);
        _upgradeTo(initialImplementation);
        initialize(owner, addrRegistry);
    }
     ...
     * @param callData Call data to send
     ...
     * @param callData Call data to send
     */
    function proxyAssert(address dest, HowToCall howToCall, bytes calldata callData)
    public
    {
...
contract ProxyRegistry is Ownable, ProxyRegistryInterface {
    ...
    address public _delegateProxyImplementation;

    /* Authenticated proxies by user. */
    mapping(address => AuthenticatedProxy) public _proxies;
    ...
    onlyOwner
    {
        require(!contracts[addr], "Contract is already allowed in registry");
        contracts[addr] = true;
    }
    ...
    returns (AuthenticatedProxy proxy)
    {
        proxy = new AuthenticatedProxy(msg.sender, _delegateProxyImplementation, this);
    ...
    returns (AuthenticatedProxy proxy)
    {
        require(address(_proxies[user]) == address(0), "User already has a proxy");
        proxy = new AuthenticatedProxy(user, _delegateProxyImplementation, this);
        _proxies[user] = proxy;
        return proxy;
    }
    ...
    public
    {
        AuthenticatedProxy proxy = _proxies[from];

        /* CHECKS */
        require(AuthenticatedProxy(payable(msg.sender)) == proxy, "Proxy transfer can only be called by the proxy");
        require(address(_proxies[to]) == address(0), "Proxy transfer has existing proxy as destination");

        /* EFFECTS */
        delete _proxies[from];
        _proxies[to] = proxy;
    }

    function delegateProxyImplementation() public view override returns (address) {
        return _delegateProxyImplementation;
    }

    function proxies(address owner) public view override returns (AuthenticatedProxy) {
...
contract ERC1155TokenRecipient is TokenRecipient {
    event ReceivedERC1155Tokens(address indexed from, uint256 id, uint256 amount, address indexed ERC1155Address, bytes extraData);
     ...
     * @param ERC1155Address Address of ERC1155 that holds the tokens
     * @param extraData Additional data to log
     */
    function receiveERC1155Approval(address from, uint256 id, uint256 amount, address ERC1155Address, bytes calldata extraData) public {
        ERC1155Tradable ERC1155 = ERC1155Tradable(ERC1155Address);
        require(ERC1155.safeTransferFrom(from, address(this), id, amount, extraData));
        emit ReceivedERC1155Tokens(from, id, amount, ERC1155Address, extraData);
    }
}
...
import "./Proxy.sol";
import "./OwnedUpgradabilityStorage.sol";

/**
 * @title OwnedUpgradeabilityProxy
 * @dev This contract combines an upgradeability proxy with basic authorization control functionalities
 */
contract OwnedUpgradabilityProxy is Proxy, OwnedUpgradabilityStorage {
    /**
     ...
     * @param implementation_ representing the address of the new implementation to be set.
     * @param data_ represents the msg.data to bet sent in the low level call. This parameter may include the function
     * signature of the implementation to be called with the needed payload
     */
    function upgradeToAndCall(address implementation_, bytes memory data_) payable public onlyProxyOwner {
        upgradeTo(implementation_);
        (bool success,) = address(this).delegatecall(data_);
 ...
 */
contract OwnedUpgradabilityStorage {
