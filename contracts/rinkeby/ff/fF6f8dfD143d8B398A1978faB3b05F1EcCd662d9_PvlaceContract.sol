...
/* when publishing set:
- provenance
O owner payment contract
O sale start timestamp
O time LENGTH (seconds in day * 10)
- make sure tokens burn after reveal! (aka they can never be minted)
- pausable
- owner functions
X make withdraw a public function (or set up access roles)
- create move money event (for owner contract)
X metadata link
...
contract PvlaceContract is ERC721, Ownable, Pausable {

    event Withdraw(address indexed _from, address indexed to, uint _value);
    ...
    // Public variables
    uint256 public constant SALE_START_TIMESTAMP = 1618253803;

    // Tokens revealed after 10 days!
    uint256 public constant REVEAL_TIMESTAMP =
        SALE_START_TIMESTAMP + (300); //(86400 * 10)
     ...
     * @dev Mint NFT
     */
    function mintNFT(uint256 numberOfNfts) public payable whenNotPaused {
    ...
    function getNFTMaxAmount() public view returns (uint256) {
        ...
        require(
            block.timestamp <= REVEAL_TIMESTAMP,
            "Sale has already ended. (passed 10 days)"
        );
        require(totalSupply() < MAX_NFT_SUPPLY, "Sale has already ended");
    ...
    */
    function withdraw() public {
        ...
        emit Withdraw(msg.sender, OWNER_PAYMENT_CONTRACT, balance);
    }
}
