...
contract HelioswapFactory is IHelioswapFactory, BaseHelioswapFactory {
    ...
    mapping(IERC20 => mapping(IERC20 => Helioswap)) private _pools;
    ...
    function pools(IERC20 tokenA, IERC20 tokenB)
        ...
        view
        override
        returns (Helioswap pool, uint256 decayPeriod, uint256 fee, uint256 slippageFee, uint256[2] memory virtualBalancesForAddition, uint256[2] memory virtualBalancesForRemoval)
    {
        ...
        returns (Helioswap pool)
    {
        ...
        (IERC20 token1, IERC20 token2) = sortTokens(tokenA, tokenB);
        require(
            _pools[token1][token2] == Helioswap(0),
            "Factory: pool already exits"
        );
...
interface IHelioswapFactory {
    function pools(
        IERC20 token1,
        IERC20 token2
    ) external view returns (Helioswap, uint256, uint256, uint256, uint256[2] memory, uint256[2] memory);
...
contract Helioswap is BaseHelioswap {
    ...
    constructor(
        ...
        require(_token0 != _token1, "Helioswap: dupplicate token");
        token0 = _token0;
        token1 = _token1;
    }
