...
contract CryptopiaCollectables is ICryptopiaCollectables, ERC721Enumerable, ContextMixin, NativeMetaTransaction, Ownable {
    ...
    uint constant LEGENDARY_REWARD = 1;
    uint constant MIN_LEGENDARY_SCORE = 87_500;

    uint constant RARE_REWARD = 2;
    uint constant MIN_RARE_SCORE = 72_500;

    uint constant HOW_DO_YOU_LIKE_YOUR_EGGS = 3;
    uint constant WHO_LIKES_EGGS = 9_500;

    bytes32 public rareMystery;
    bytes32 public legendaryMystery;
    string private _contractURI;
    string private _baseTokenURI;
    bytes32 private _currentRandomSeed;
    uint private _currentTokenId = 3; 
    mapping (uint => bytes32) tokenRandomness;
    ...
    constructor() 
        ERC721("Cryptopia Collectables", "CUNIQUE") {}
    ...
    /// @param _legendaryMystery ???
    /// @param _rareMystery ???
    function initialize(
        address _proxyRegistry, 
        string calldata _initialContractURI, 
        string calldata _initialBaseTokenURI,    
        bytes32 _legendaryMystery, 
        bytes32 _rareMystery) public override {
        _initializeEIP712(name()); // <-- Initializer
        
        _currentRandomSeed = _random();
        proxyRegistry = ProxyRegistry(_proxyRegistry);
        _contractURI = _initialContractURI;
        _baseTokenURI = _initialBaseTokenURI;
        legendaryMystery = _legendaryMystery;
        rareMystery = _rareMystery;

        _mint(address(this), LEGENDARY_REWARD);
        _mint(address(this), RARE_REWARD);
        _mint(address(this), HOW_DO_YOU_LIKE_YOUR_EGGS);
    }
    ...
    function getTokenURI(uint _tokenId) override public view returns (string memory) {
        return string(abi.encodePacked(getBaseTokenURI(), Strings.toString(_tokenId)));
    }
    ...
    ///     < 10000 = Special (5%)
    ///     < 9500  = 3 attributes (15%)
    ///     < 8000  = 2 attributes (15%)
    ///     < 6500  = 1 attributes (15%)
    ///     < 5000  = None (50%)
    ...
    function getTokenTraits(uint _tokenId)
        ...
        returns (
            ...
            uint32 easterEgg)
    {
        bytes32 seed = tokenRandomness[_tokenId];
        faction = _randomAt(seed, 0);
        cardType = _randomAt(seed, 1);
        backgroundType = _randomAt(seed, 2);
        backgroundAttributes = _randomAt(seed, 3);
        characterPose = _randomAt(seed, 4);
        characterAttributes = _randomAt(seed, 5);
        characterSpecials = _randomAt(seed, 6);
        easterEgg = _randomAt(seed, 7);
    }


    /// @dev Calculate the token score that determins the rarity
    /// @param _tokenId Token to obtain score for
    function getTokenScore(uint _tokenId) override public view returns (uint) {
        if (_tokenId <= 3)
        {
            return 100_000; // Specials
        }
    ...
    function isLegendary(uint _tokenId) override public view returns (bool) {
        return getTokenScore(_tokenId) >= MIN_LEGENDARY_SCORE;
    }
    ...
    function becomeLegend(string calldata _myLegendaryWords, uint _legendaryTokenId) override public onlyWhenInitialized
    {
        require(ownerOf(_legendaryTokenId) == msg.sender, "Only the token owner shall call..");
        require(isLegendary(_legendaryTokenId), "Only the owner of a legendary token shall become legend..");
        require(legendaryMystery == keccak256(abi.encodePacked(_myLegendaryWords)), "Good try.. just not legendary..");
        require(ownerOf(LEGENDARY_REWARD) == address(this), "Another legend has gone before you..");

        _transfer(address(this), msg.sender, LEGENDARY_REWARD);
    }
    ...
    function isRare(uint _tokenId) override public view returns (bool) {
        uint score = getTokenScore(_tokenId);
        return score >= MIN_RARE_SCORE && score < MIN_LEGENDARY_SCORE;
    }
    ...
    function takeARareChance(string calldata _myUnusualWords, uint _rareTokenId) override public onlyWhenInitialized
    {
        require(ownerOf(_rareTokenId) == msg.sender, "Only the token owner shall call..");
        require(isRare(_rareTokenId), "Only the owner of a rare token can take a rare chance..");
        require(rareMystery == keccak256(abi.encodePacked(_myUnusualWords)), "Good try.. just not uncommon enough..");
        require(ownerOf(RARE_REWARD) == address(this), "You're too late..");

        _transfer(address(this), msg.sender, RARE_REWARD);
    }
    ...
    function likesEggs(uint _tokenId) override public view returns (bool) {
        return _randomAt(tokenRandomness[_tokenId], 7) >= WHO_LIKES_EGGS;
    }


    /// @dev Take a chance and get rewarded with a unique reward
    /// @param _someSauce What's an egg without sause
    function bakeAnEgg(uint _someSauce) override public onlyWhenInitialized
    {
        require(ownerOf(_someSauce) == msg.sender, "Only the egg owner shall call..");
        require(likesEggs(_someSauce), "You gotta like eggs..");
        require(ownerOf(3) == address(this), "This party was over before it started..");

        _transfer(address(this), msg.sender, 3);
    }
    ...
    /// @dev Pick random properties for `_tokenId`
    /// @param _tokenId  id of the token to randomize properties for
    function _randomize(uint _tokenId) private {
        tokenRandomness[_tokenId] = _random();
    }
    ...
    function _randomAt(bytes32 _hash, uint _index) private pure returns(uint32) {
        return uint32(
            uint(keccak256(abi.encodePacked(_hash, _index)))
            .mod(INVERSE_BASIS_POINT));
    }   
