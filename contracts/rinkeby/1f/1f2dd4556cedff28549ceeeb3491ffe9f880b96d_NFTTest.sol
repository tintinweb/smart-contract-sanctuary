...
contract NFTTest is Random {
    ...
    string internal basicURI = "https://yanyi-test.oss-cn-hangzhou.aliyuncs.com/";
    ...
    function _genesisNFT() internal returns(bool) {
        uint[4] memory ids = [uint(0), uint(0), uint(0), uint(0)];
        _deleteElement(ids);
        uint headrate = _calculateHashrate(0, "HEADRATE");
        require(headToHashrate[0] == 0, "Head Element Repeated");
        headToHashrate[0] = headrate;
        uint bodyrate = _calculateHashrate(0, "BODYRATE");
        require(bodyToHashrate[0] == 0, "Body Element Repeated");
        bodyToHashrate[0] = bodyrate;
        uint limbrate = _calculateHashrate(0, "LIMBRATE");
        require(limbToHashrate[0] == 0, "Limb Element Repeated");
        limbToHashrate[0] = limbrate;
        uint weaponrate = _calculateHashrate(0, "WEAPONRATE");
        require(weaponToHashrate[0] == 0, "Weapon Element Repeated");
        weaponToHashrate[0] = weaponrate;
    ...
    function uri(uint256 _id) public view returns (string memory) {
        return string(abi.encodePacked(basicURI, _uint2str(_id), ".json"));
    }
