...
contract AdmireFloor is ERC721, ERC721Enumerable, ReentrancyGuard, Ownable, Pausable {
  ...
  constructor() ERC721("Admire Floor", "ADMIRE") Ownable() {}
  ...
  function reserveTokens(address _to, uint256 _reserveAmount) public onlyOwner {
    ...
    function shuffleStringArray(uint256 tokenId, string[] memory sourceArray) internal view returns (string[] memory) {
    ...
    //   }
      ...
      uint256 randomValue = random(string(abi.encodePacked(block.timestamp, block.difficulty, toString(tokenId))));
      ...
      for (uint i = 1; i < sourceArray.length - 1; i++) {
        ...
        // randomValue = keccak256(abi.encodePacked(randomValue));
      }
    ...
    function getWorldsProgress(uint256 tokenId) public view returns (string[] memory) {
      return shuffleStringArray(tokenId, worlds);
    }
