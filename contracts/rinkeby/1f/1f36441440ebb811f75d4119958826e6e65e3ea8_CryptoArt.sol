...
contract CryptoArt is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function randomOne(uint256 tokenId) internal view returns (Point memory point) {
        Point memory point;
     ...
     * 
     */
     function tokenURI(Point memory point) internal view returns (string memory){
         string memory px = string(abi.encodePacked(point.pointX));
         string memory py = string(abi.encodePacked(point.pointY));
         return string(abi.encodePacked(z[0], px, z[1], py, z[2]));
         }
}