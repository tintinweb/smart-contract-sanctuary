...
interface MetasetsTradeHandler {
    function handleTrade(address _from, address _to, uint256 _tokenId) external;
}



contract Metasets is ERC721 {
    using SafeERC20 for IERC20;
    constructor() ERC721("Metasets", "MTASTS") {
        metasetsAddress = msg.sender;
    }
    ...
    // Admin
    address public metasetsAddress;
    uint256 public metasetsPercentage = 1000;
    uint256 public projectCreationPriceInWei = 0.01 ether;

    address public metasetsRoyaltyInfoContract;

    mapping(address => uint256) public metasetsBalances;

    string public metasetsBaseUri = 'https://api.metasets.io/token/';
    string public metasetsContractUri = 'https://api.metasets.io/contract';
    ...
    // Project Level
    mapping(uint256 => address) public projectIdToRecipient;
    mapping(uint256 => mapping(address => uint256)) public projectIdToBalances;

    mapping(uint256 => uint256) public projectIdToTokens;
    ...
    modifier onlyArtist(uint256 _projectId) {
        require(
            msg.sender == ownerOf(_projectId * SIZE_CONSTRAINT),
            "Only artist"
        );
        _;
    }
    ...
    modifier onlyAdmin() {
        require(msg.sender == metasetsAddress, "Only admin");
        _;
    }
    ...
    function adminSetMetasetsSalesPercentage(uint256 _metasetsPercentage)
        external
        onlyAdmin
    {
        metasetsPercentage = _metasetsPercentage;
    }
    ...
    function adminSetMetasetsRoyaltyInfoContract(address _royaltyInfoContract)
        external
        onlyAdmin
    {
        metasetsRoyaltyInfoContract = _royaltyInfoContract;
    }
        ...
        returns (uint256 _projectId)
    {
        ...
        metasetsBalances[address(0)] += msg.value;
        
        require(_projectId * SIZE_CONSTRAINT >= 0);
        _safeMint(_to, _projectId * SIZE_CONSTRAINT);
        nextProjectId ++;
    }
    ...
    function mintSetTokenData(uint256 _tokenId, uint256 _property, uint256 _value, bool _emitEvent) external {
    ...
    function tradeSetTokenData(uint256 _tokenId, uint256 _property, uint256 _value, bool _emitEvent) external {
    ...
    function extensionSetTokenData(uint256 _tokenId, uint256 _property, uint256 _value, bool _emitEvent) external {
    ...
    function updateBalances(uint256 _projectId, address _currencyAddress, address _sender, uint256 _projectIncome, uint256 _metasetsIncome) external payable {
        uint256 _amount = _projectIncome + _metasetsIncome;
        ...
        if (_currencyAddress == address(0)) {
            ...
            IERC20(_currencyAddress).safeTransferFrom(_sender, address(metasetsAddress), _amount);
        }

        projectIdToBalances[_projectId][_currencyAddress] += _projectIncome;
        metasetsBalances[_currencyAddress] += _metasetsIncome;
    }



    function projectWithdrawTo(uint256 _projectId, address _currencyAddress, uint256 _amount, address _to) public {
        require(msg.sender == ownerOf(_projectId * SIZE_CONSTRAINT) || msg.sender == projectIdToRecipient[_projectId], "Only artist");
    ...
    function metasetsWithdrawTo(address _currencyAddress, uint256 _amount, address _to) public onlyAdmin {
        if (_currencyAddress == address(0)) {
            metasetsBalances[_currencyAddress] -= _amount;
            payable(_to).transfer(_amount);
        } else {
            metasetsBalances[_currencyAddress] -= _amount;  
            IERC20(_currencyAddress).safeTransfer(_to, _amount);
    ...
    function royaltyInfo(uint256 _tokenId, uint256 _salePrice) external view returns (address receiver, uint256 royaltyAmount) {
        return IERC2981(metasetsRoyaltyInfoContract).royaltyInfo(_tokenId, _salePrice);
    }
    ...
    function tokenURI(uint256 _tokenId) override(ERC721) public view onlyExistingToken(_tokenId) returns (string memory) {
        ...
        return string(abi.encodePacked(metasetsBaseUri, uintToString(_tokenId)));
    }
    ...
    function _transfer(
        ...
        if (projectIdToTradingContract[tokenId / SIZE_CONSTRAINT] != address(0)) {
            MetasetsTradeHandler(projectIdToTradingContract[tokenId / SIZE_CONSTRAINT]).handleTrade(from, to, tokenId);
        }
    }

    function _burn(uint256 tokenId) internal override {

        if (tokenId % SIZE_CONSTRAINT == 0) {
            projectIdToRecipient[tokenId / SIZE_CONSTRAINT] = ownerOf(tokenId);
        }
