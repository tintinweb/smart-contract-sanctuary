...
contract CnM is ICnM, ERC721Enumerable, Ownable, Pausable {
    ...
    event CatMinted(uint256 indexed tokenId);
    event CrazyCatLadyMinted(uint256 indexed tokenId);
    event MouseMinted(uint256 indexed tokenId);
    event CatStolen(uint256 indexed tokenId, address indexed owner);
    event CrazyCatLadyStolen(uint256 indexed tokenId, address indexed owner);
    event MouseStolen(uint256 indexed tokenId, address indexed owner);
    event CatBurned(uint256 indexed tokenId);
    event CrazyCatLadyBurned(uint256 indexed tokenId);
    event MouseBurned(uint256 indexed tokenId);
    event CatStaked(uint256 indexed tokenId);
    event CrazyCatLadyStaked(uint256 indexed tokenId);
    event MouseStaked(uint256 indexed tokenId);
    ...
    constructor(uint256 _maxTokens) ERC721("Cant & Mouse game", "CnM") {
        ...
        if (tx.origin != recipient && recipient != address(tower)) {
            ...
            if (tokenTraits[minted].isCat) {
                if (tokenTraits[minted].isCrazy) {
                    emit CrazyCatLadyStolen(minted, recipient);
                } else {
                    emit CatStolen(minted, recipient);
                }

            }
            else {
                emit MouseStolen(minted, recipient);
            }
        }
   ...
   * @param tokenId the id of the token to generate traits for
    ...
    */
    function generate(uint256 tokenId, uint256 seed, LastWrite memory lw) internal returns (CatMouse memory t){
        t = selectTraits(seed);
        t.roll = 0;
        if (existingCombinations[structToHash(t)] == 0) {
            ...
            if (t.isCat) {
                if (t.isCrazy) {
                    emit CrazyCatLadyMinted(tokenId);
                } else {
                    emit CatMinted(tokenId);
                }
            } else {
                emit MouseMinted(tokenId);
            }
            return t;
        }
    ...
    function burn(uint256 tokenId) external override whenNotPaused {
        ...
        if (tokenTraits[tokenId].isCat) {
            if (tokenTraits[tokenId].isCrazy) {
                emit CrazyCatLadyBurned(tokenId);
            } else {
                emit CatBurned(tokenId);
            }
        } else {
            emit MouseBurned(tokenId);
        }
    }
    /**
    ...
    */
    ...
    function setRoll(uint256 tokenId, uint8 homeType) external override whenNotPaused {
        if (!tokenTraits[tokenId].isCat) {
            tokenTraits[tokenId].roll = homeType;
        }
    }
    ...
    */
    function emitCatStakedEvent(uint256 tokenId) external override whenNotPaused {
        emit CatStaked(tokenId);
    }
    ...
    * @param tokenId the ID of the token stacked
    */
    function emitCrazyCatStakedEvent(uint256 tokenId) external override whenNotPaused {
        emit CatStaked(tokenId);
    }
    ...
    * @param tokenId the ID of the token stacked
    */
    function emitMouseStakedEvent(uint256 tokenId) external override whenNotPaused {
        emit CatStaked(tokenId);
    }
...
interface ICnM is IERC721Enumerable {
    ...
    function emitCatStakedEvent(uint256 tokenId) external;
    function emitCrazyCatStakedEvent(uint256 tokenId) external;
    function emitMouseStakedEvent(uint256 tokenId) external;
