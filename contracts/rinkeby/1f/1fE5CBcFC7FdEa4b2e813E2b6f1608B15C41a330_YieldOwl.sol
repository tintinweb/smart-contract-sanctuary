...
import './interfaces/IYieldOwlStrategy.sol';
import './interfaces/IYieldOwlCondition.sol';
import './interfaces/IYieldOwlAction.sol';

/**
 * @title YieldOwl Staking Contract
 * @notice handles deposits, withdraws, strategy execution and bounty rewards
 * @author YieldOwl
 */
contract YieldOwl is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        IYieldOwlStrategy strategy; // address of the strategy for the pool
    }
    ...
    event Add(IERC20 stakeToken, IYieldOwlStrategy strategy);
    ...
    event SetStrategyRouter(IYieldOwlStrategy strategy, address router);
    event SetStrategySwapRouterEnabled(IYieldOwlStrategy strategy, bool enabled);
    event SetStrategySwapPath(IYieldOwlStrategy _strategy, address _token0, address _token1, address[] _path);
    event SetStrategyExtraEarnTokens(IYieldOwlStrategy _strategy, address[] _extraEarnTokens);
    ...
    function stakedTokens(uint256 _pid, address _user) public view returns (uint256) {
        ...
        IYieldOwlStrategy strategy = pool.strategy;
     ...
     */
    function add(IYieldOwlStrategy _strategy) public onlyOperator {
     ...
     */
    function addMany(IYieldOwlStrategy[] calldata _strategies) external onlyOperator {
    ...
    function addRule(
        ...
        require(IYieldOwlCondition(_condition).isCondition(), 'addRule: BAD_CONDITION');
        require(IYieldOwlAction(_action).isAction(), 'addRule: BAD_ACTION');
    ...
    function checkRule(
        ...
        return
            IYieldOwlCondition(rule.condition).check(
    ...
    function executeRule(
        ...
        IYieldOwlStrategy strategy = poolInfo[_pid].strategy;
        ...
        _tryEarn(strategy);
        IYieldOwlAction action = IYieldOwlAction(rule.action);
     ...
     */
    function setStrategySwapRouterEnabled(IYieldOwlStrategy _strategy, bool _enabled) external onlyOwner {
     ...
     */
    function setStrategyExtraEarnTokens(IYieldOwlStrategy _strategy, address[] calldata _extraEarnTokens)
        external
        onlyOwner
    {
    ...
    function _tryEarn(IYieldOwlStrategy _strategy) internal {
