...
contract NFTDistributor is Ownable {
    ...
    uint256 public constant airdrop_period = 1 hours;
    uint256 public constant discount_period = 30 minutes;
    ...
    function claim(uint256 index, address account, bytes32[] calldata merkleProof) external {
        require(isAirdropping(), "NFTDistributor: Not airdrop period.");
    ...
    function discountedPurchase(uint256 index, uint256 nonce, bytes32[] calldata merkleProof, uint256 amount) payable external {
        require(isDiscountedPeriod(), "NFTDistributor: Not discounted sale.");

        bytes32 node = keccak256(abi.encodePacked(index, msg.sender, nonce));
        require(MerkleProof.verify(merkleProof, merkleRoot, node), "NFTDistributor: Invalid proof.");
    ...
    function purchase(uint256 amount) payable external {
        ...
        require(!isDiscountedPeriod(), "NFTDistributor: Discounted sale not finished.");
    ...
    function _randomMint(address _to) internal {
        ...
        if (pendingCount <= 100)
            _idMin = 0;
        else if (pendingCount <= 1900)
            _idMin = 100;
        else if (pendingCount <= 4000)
            _idMin = 1900;
        else if (pendingCount <= 6200)
            _idMin = 4000;
    ...
    function isAirdropping() public view returns(bool) {
        return block.timestamp < startTime + airdrop_period && startTime > 0;
    }

    function isDiscountedPeriod() public view returns(bool) { 
        return block.timestamp < startTime + discount_period;
    }

    function getCurrentPrice() public view returns(uint256) {
        if (pendingCount <= 100)
            return 3 ether;
        if (pendingCount <= 1900)
            return 1 ether;
        if (pendingCount <= 4000)
            return 0.7 ether;
        if (pendingCount <= 6200)
            return 0.3 ether;
