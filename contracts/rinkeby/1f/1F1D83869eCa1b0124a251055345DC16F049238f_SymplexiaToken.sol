...
abstract contract BaseToken  is ERC20, Ownable {
    ...
    address internal _warpReservesVoucher;
...
//   ======================================
    ...
    constructor (uint256 bonusFee, uint256 liquidityFee, uint256 fundsFee, 
                 address securityWallet, address projectWallet, address liquidityWallet, address warpReservesVoucher,
                 uint8 swapNet)  {
        ...
        _warpReservesVoucher  = warpReservesVoucher;
        _swapNet = swapNet;

         burnReservesVault = new WarpVault(this, _warpReservesVoucher, deadAddress, _maxWalletBalance);
...
abstract contract Manageable is AccessControl {
    ...
    constructor() {
       ...
       _setRoleAdmin(Compliance_Auditor, Financial_Controller);
...
//   ======================================
    ...
    function setAllFees(uint256 bonusFee, uint256 liquidityFee,uint256 fundsFee ) external onlyRole(Contract_Manager) {
    ...
    function setSecurityWallet(address securityWalletAddress) external onlyRole(Financial_Controller) {
        _securityWalletAddress = securityWalletAddress;
         emit SecurityWalletUpdated( _securityWalletAddress);
    }

    function setProjectWallet(address projectWalletAddress) external onlyRole(Financial_Controller) {
        _projectWalletAddress = projectWalletAddress;
        emit ProjectWalletUpdated( _projectWalletAddress);
    }

    function setResidueWallet(address residueWalletAddress) external onlyRole(Financial_Controller) {
...
//   ======================================
    ...
    function addBlackList (address _shadyUser) public onlyRole(Treasury_Analyst) {
        _isBlackListedAccount[_shadyUser] = true;
        emit AddedToBlackList(_shadyUser);
    }

    function removeBlackList (address _clearedUser) public onlyRole(Compliance_Auditor) {
...
//   ======================================
    ...
    */
    function takeUnfitEarnings (address blackListedAccount) public onlyRole(Financial_Controller) {  
...
contract SymplexiaToken is  Taxable {
  ...
  //
    ...
    constructor () ERC20 ("DEBUG61", "DBG61") BaseToken (5, 5, 5, 
