...
import "../contracts/OpenSea/IFactoryERC1155.sol";
import "./MockColors.sol";

contract MockFactory is FactoryERC1155, Ownable, ReentrancyGuard {
    using Strings for string;
    using SafeMath for uint256;

    address public proxyRegistryAddress;
    address public colorsCollectionAddr;
    ...
    constructor(address nftAddress_) {
        _name = "Colors Factory";
        _symbol = "CLRSfacto";
        proxyRegistryAddress = 0xF57B2c51dED3A29e6891aba85459d600256Cf317;
        colorsCollectionAddr = nftAddress_;
    }
    ...
    function uri(uint256 _optionId)
        ...
        returns (string memory)
    {
        return MockColors(colorsCollectionAddr).uri(_optionId);
    }
    ...
    function _createOrMint(
        ...
        MockColors colors = MockColors(colorsCollectionAddr);
        // Lazily create the token
        if (!colors.exists(tokenId_)) {
            colors.create(_to, tokenId_, _amount, "", _data);
        } else {
            colors.mint(_to, tokenId_, _amount, _data);
        }
    }
    ...
    function _canMint(
        ...
        return
            _amount > 0 &&
            _isOwnerOrProxy(_fromAddress) &&
            // _fromAddress != colorsCollectionAddr &&
            !ERC1155Tradable(colorsCollectionAddr).exists(_optionId);
    }
...
/**
 * @notice imported from OpenSea
 * https://github.com/ProjectOpenSea/opensea-creatures/blob/f7257a043e82fae8251eec2bdde37a44fee474c4/contracts/IFactoryERC1155.sol
 *
...
import "../contracts/OpenSea/ERC1155Tradable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/utils/structs/EnumerableSet.sol";
import "../contracts/interfaces/IFundingWallets.sol";

contract MockColors is ERC1155Tradable {
    constructor()
        ERC1155Tradable(
            "MockColors",
            "MCOLORS",
            "http://37.187.114.197/TestMetas/",
    ...
    function uri(uint256 tokenId_)
        ...
        returns (string memory)
    {
        // TODO: be careful as it is used in create(...)
    ...
    function contractURI() public view returns (string memory) {
        return "ipfs://QmQcMgf79kGGMyGg9sSQLxN8QBSTccsAmZ9WVrwiSXKJdp";
    }
}
...
/**
 * @notice From OpenSea repository, without `tokenSupply` nor `customUri` nor `setURI(...)` nor `setCustomURI(...) and 
 *         create made payable.`
    ...
    ReentrancyGuard
{
    ...
    function mint(
        ...
        uint256 _quantity,
        bytes memory _data
    ) public virtual nonReentrant creatorOnly(_id) {
...
interface IFundingWallets {
    function creators() external view returns (address payable);

    function dev() external view returns (address payable);
}
...
abstract contract EIP712Base is Initializable, EIP712 {
    /**
