...
contract Sale is Ownable{
    ...
    constructor(uint256 _serviceValue, uint256 _transvalue, uint256 id) public{
    ...
    function _saleToken(address payable from, uint256 tokenId, uint256 amount) internal{
        require(amount >= order_place[from][tokenId].price , "Insufficient fund");
    ...
    function swapfee() internal{
        address payable admin = address(uint160(owner()));
        admin.transfer(transvalue);
        
    }
}
contract NoftenExchangeContract is Sale{
     event Approve(address indexed owner, uint256 indexed token_id,bool approved);
    uint256 public tokenCount;
    constructor(uint256 _serviceValue, uint256 _transvalue, uint256 id) Sale(_serviceValue, _transvalue, id) public{
    ...
    function transferOwnershipForColle(address newOwner, address token721, address token1155) public onlyOwner {
        require(newOwner != address(0), "Ownable: new owner is the zero address");
        Noften721 tok= Noften721(token721);
        Noften1155 tok1155= Noften1155(token1155);
    ...
    function mint(address token ,string memory tokenuri, uint256 value, uint256 tokenId, uint256 royal, uint256 _type, uint256 supply) public{
       ...
       if(_type == 721){
           ...
           if(value != 0){
            ...
            Noften1155 tok = Noften1155(token);
    ...
    function burn(address from, uint256 tokenId, address token, uint256 _type, uint256 NOFToken ) public{
        require( balances[tokenId][msg.sender] >= NOFToken || msg.sender == owner(), "Your Not a Token Owner or insufficient Token Balance");
