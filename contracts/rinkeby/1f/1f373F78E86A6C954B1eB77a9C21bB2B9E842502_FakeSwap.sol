...
contract FakeSwap is Ownable, Pausable, IERC721Receiver, IERC1155Receiver {
    ...
    function fakeCreateSwapIntent(uint256 id) public {
        emit swapEvent(msg.sender, (block.timestamp-(block.timestamp%secs)), swapStatus.Opened, id, msg.sender);
    }

    function fakeCancelSwapIntent(uint256 id) public  {
        emit swapEvent(msg.sender, (block.timestamp-(block.timestamp%secs)), swapStatus.Cancelled, id, address(0));
    }

    function fakeCloseSwapIntent(uint256 id) public {
        emit swapEvent(msg.sender, (block.timestamp-(block.timestamp%secs)), swapStatus.Closed, id, msg.sender);
    }
