...
import { ERC721UpgradeableURI } from "./base/ERC721UpgradeableURI.sol";
import { SafeERC20 } from "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import { SafeCast } from "@openzeppelin/contracts/utils/math/SafeCast.sol";

// import for spec
import { IHermesTreasury } from "./interfaces/IHermesTreasury.sol";
...
contract HermesNFT is ERC721Enumerable, ERC721UpgradeableURI, IHermesNFT {
    using SafeERC20 for IERC20;

    IDepository public immutable bondDepository;

    ITeller public immutable bondTeller;

    IERC20 public immutable gOHM;

    address public hermesFERRecipient;

    struct Receipt {
        uint64 claimedAt;
    }

    mapping(uint256 => Receipt) public bondReceipt;

    constructor(
        IDepository _depository,
        ITeller _teller,
        IERC20 _gOHM,
        IERC721URIDescriptor _descriptor
    ) ERC721UpgradeableURI("Hermes OHM Bond NFT", "H-OHM", _descriptor) {
        bondDepository = _depository;
        bondTeller = _teller;
        gOHM = _gOHM;
        hermesFERRecipient = msg.sender;
    }

    /// @inheritdoc IHermesNFT
    function deposit(
        address _depositor,
        uint256 _maxPrice,
        uint256 _BID
    ) external override returns(uint256 payout, uint256 nftId) {
        // get principal address of given BID
        IERC20 principal = bondDepository.bonds(_BID).principal;
        uint256 amount = principal.balanceOf(address(this));
        // approve principal token to depository
        principal.safeApprove(address(bondDepository), amount);
        (payout, nftId) = bondDepository.deposit(
            amount,
            _maxPrice,
            address(this),
            _BID,
        ...
        // flush the allowance to make sure things work correctly
        principal.safeApprove(address(bondDepository), 0);
        _mint(_depositor, nftId); 
    }
    ...
    function claim(
        ...
        for(uint256 i = 0 ; i < _nftIds.length; i++) {
            require(_isApprovedOrOwner(msg.sender, _nftIds[i]), "!approved");
            Receipt memory receipt = bondReceipt[_nftIds[i]];
            if(receipt.claimedAt != 0) {
                revert AlreadyClaimed(_nftIds[i], receipt.claimedAt);
            }
            claimed += bondTeller.bonderInfo(address(this), _nftIds[i]).payout;
            receipt.claimedAt = SafeCast.toUint64(block.timestamp);
            bondReceipt[_nftIds[i]] = receipt;
        }
        gOHM.transfer(msg.sender, claimed);
    }

    function tokenURI(uint256 _index) public view override(ERC721, ERC721UpgradeableURI) returns(string memory) {
        return super.tokenURI(_index);
    }

    function changeHermesFERRecipient(address _recipient) external onlyOwner {
        hermesFERRecipient = _recipient;
    }
}
...
abstract contract ERC721UpgradeableURI is ERC721Enumerable, Ownable {
    IERC721URIDescriptor public descriptor;

    event DescriptorChanged(IERC721URIDescriptor descriptor);
    ...
    function changeDescriptor(IERC721URIDescriptor _descriptor) public virtual onlyOwner {
        descriptor = _descriptor;
        emit DescriptorChanged(_descriptor);
    }

    function tokenURI(uint256 _index) public virtual override view returns(string memory) {
        return descriptor.describe(_index);
    }
}
...
interface IHermesTreasury {
    /// @notice pull fund from treasury
    /// @dev can only be called by approved addresses
    /// @param _token token to pull
    /// @param _amount token amount to pull
    function quoteFund(IERC20 _token, uint256 _amount) external;

    /// @notice add allowance to `_spender`
    /// @dev can only be called by governance
    /// @param _token token address to allow
    /// @param _spender address of receive the allowance
    /// @param _amount amount of allowance to increase
    /// @return allowed total amount of allowance after increase
    function increaseAllowance(IERC20 _token, address _spender, uint256 _amount) external returns(uint256 allowed);

    /// @notice decrease allowance of `_spender`
    /// @dev can only be called by governance
    /// @param _token token address to allow
    /// @param _spender address to reduce the allowance
    /// @param _amount amount of allowance to decrease
    /// @return allowed total amount of allowance after decrease
    function decreaseAllowance(IERC20 _token, address _spender, uint256 _amount) external returns(uint256 allowed);

    /// @notice amount of token approved to `_spender`
    /// @param _token token address to query
    /// @param _spender address of spender to query
    /// @return allowed total amount of allowance
    function allowance(IERC20 _token, address _spender) external view returns(uint256 allowed);
}