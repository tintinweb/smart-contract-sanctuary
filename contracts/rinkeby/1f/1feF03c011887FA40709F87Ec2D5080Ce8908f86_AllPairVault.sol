...
contract AllPairVault is OndoRegistryClient, IPairVault {
  ...
  modifier transition(uint256 _vaultId, OLib.State _nextState) {
    Vault storage vault_ = Vaults[_vaultId];
  ...
  modifier maybeOpenDeposit(uint256 _vaultId) {
    Vault storage vault_ = Vaults[_vaultId];
  ...
  event Invested(uint256 indexed vaultId, uint256 indexed redeemAt);
    ...
    returns (uint256 vaultId)
  {
    ...
    require(_params.hurdleRate < 10000000, "Maximum hurdle is 10000%");
    require(
  ...
  function setRollover(
    ...
    emit SetRollover(msg.sender, _rolloverId, _vaultId);
  }
  ...
  function _deposit(
    ...
    uint256 _amount,
    address _payer
  ) internal whenNotPaused maybeOpenDeposit(_vaultId) {
    Vault storage vault_ = Vaults[_vaultId];
    ...
    transition(_vaultId, OLib.State.Live)
  {
    ...
    emit Invested(_vaultId, vault_.redeemAt);
  }
   ...
   *      the performance fee. In v2 we can make this more flexible.
    ...
    returns (uint256 fee)
  {
    ...
    if (address(performanceFeeCollector) != address(0)) {
      ...
      if (junior.received > juniorHurdle) {
        fee = (vault.performanceFee * junior.received) / denominator;
  ...
  function isVault(address _target) external view override returns (bool) {
    return registry.authorized(OLib.VAULT_ROLE, _target);
  }
...
contract TrancheToken is ERC20Upgradeable, ITrancheToken, OwnableUpgradeable {
  ...
  modifier onlyVault {
    require(vault.isVault(msg.sender), "Invalid access: Only Vault can call");
    _;
  }
