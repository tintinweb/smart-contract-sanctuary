...
contract TimeLockedWalletFactory {
    ...
    function getWallets(address _originalOwner) public view returns (address[] memory addr) {
        
        if(wallets[_originalOwner].length == 0) {
            addr[0] = address(0);
        ...
        returns (address)
    {
        ...
        address wallet = address(timeLockedWallet);
        originalOwner = _originalOwner;
        beneficiary = _beneficiary;
        unlockTime = _unlockTime;
