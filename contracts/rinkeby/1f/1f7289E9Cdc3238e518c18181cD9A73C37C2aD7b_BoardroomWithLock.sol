    ...
    Ownable
{
    ...
    uint256 public baseRateRatio = 40;
    ...
    function updateWorkingSupply(address _director) public {
        uint256 votingBalance =
            lockToken.voteBalanceOf(address(this), _director);
        ...
        uint256 limit = liquidity.mul(baseRateRatio).div(100);
        if (votingTotal > 0) {
            limit += totalLiquidity
                .mul(votingBalance)
                .div(votingTotal)
                .mul(100 - baseRateRatio)
                .div(100);
        }
        limit = Math.min(liquidity, limit);
        ...
        if (workingSupply > 0) {
            for (uint256 i = 0; i < rewardPools.length(); i++) {
                try IRewardPool(rewardPools.at(i)).collect() returns (
                    ...
                    uint256 nextRPS =
                        prevRPS.add(amount.mul(1e18).div(workingSupply));
