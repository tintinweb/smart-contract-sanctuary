    ...
    ReentrancyGuardUpgradeable
{
    ...
    function depositTokensAndPlatformFee(address _homeERC20, uint256 _amount)
        internal
    {
        uint256 platformFee = _amount.mul(depositFee).mul(multiplier).div(
            maxPlatformFee
        );
        if (depositFee > 0) {
    ...
    function withdrawTokensAndPlatformFee(
        ...
        uint256 platformFee = _amount.mul(withdrawFee).mul(multiplier).div(
            maxPlatformFee
        );
        if (withdrawFee > 0) {
    ...
    function updateGovernance(address _newGovernance) external onlyGovernance {
        require(_newGovernance != address(0), "UpdateGovernance:: New Governance can not be Zero Address");
