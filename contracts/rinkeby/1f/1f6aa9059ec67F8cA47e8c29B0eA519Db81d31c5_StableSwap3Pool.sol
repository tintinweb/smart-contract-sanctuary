...
contract StableSwap3Pool is ReentrancyGuard {
    ...
    */
    ...
    function add_liquidity(
        ...
        for (uint256 i = 0; i < N_COINS; i++) {
            ...
            if (token_supply == 0) assert(in_amount > 0); // dev: initial deposit requires all coins
    ...
    */
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 dy_0 = (xp[i] - new_y) / PRECISION_MUL[i]; // w/o fees
    ...
    */
    ...
    function ramp_A(uint256 _future_A, uint256 _future_time) external {
        ...
        assert(block.timestamp >= initial_A_time + MIN_RAMP_TIME);
        assert(_future_time >= block.timestamp + MIN_RAMP_TIME); // dev: insufficient time

        uint256 _initial_A = _A();
        assert(_future_A > 0 && _future_A < MAX_A);
        assert(
    ...
    function commit_new_fee(uint256 new_fee, uint256 new_admin_fee) external {
        assert(msg.sender == owner); // dev: only owner
        assert(admin_actions_deadline == 0); // dev: active action
    ...
    function apply_new_fee() external {
        assert(msg.sender == owner); // dev: only owner
        assert(block.timestamp >= admin_actions_deadline); // dev: insufficient time
        assert(admin_actions_deadline != 0); // dev: no active action
    ...
    function commit_transfer_ownership(address _owner) external {
        assert(msg.sender == owner); // dev: only owner
        assert(transfer_ownership_deadline == 0); // dev: active transfer
    ...
    function apply_transfer_ownership() external {
        assert(msg.sender == owner); // dev: only owner
        assert(block.timestamp >= transfer_ownership_deadline); // dev: insufficient time
        assert(transfer_ownership_deadline != 0); // dev: no active transfer
    ...
    function kill_me() external {
        assert(msg.sender == owner); // dev: only owner
        assert(kill_deadline > block.timestamp); // dev: deadline has passed
        is_killed = true;
    }
