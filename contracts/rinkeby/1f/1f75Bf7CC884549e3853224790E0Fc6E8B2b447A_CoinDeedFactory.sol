...
contract CoinDeedFactory is ICoinDeedFactory, AccessControl {
    ...
    function setPlatformFee(uint256 _platformFee) public override  onlyRole(ADMIN_ROLE) {
        require(_platformFee <= BASE_DENOMINATOR, "Invalid platform fee");
        platformFee = _platformFee;
    }

    function setFeedRegistry(address _feedRegistryAddress) public override onlyRole(ADMIN_ROLE) {
        feedRegistryAddress = _feedRegistryAddress;
    }

    function setMaxPriceDrop(uint256 _maxPriceDrop) public override onlyRole(ADMIN_ROLE) {
