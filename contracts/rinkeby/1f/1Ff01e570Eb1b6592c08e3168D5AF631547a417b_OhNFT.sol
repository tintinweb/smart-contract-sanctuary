...
contract OhNFT is OhBase, VRFConsumerBase {
  using SafeMath for uint256;
  event OhNFTRandomnessRequest(uint timestamp);
  event OhNFTRandomnessFulfil(bytes32 requestId, uint256 tokenId);
  ...
  constructor(
    ...
    OhBase(_tokenName, _tokenSymbol, _privateSalePrice, _publicSalePrice, _maxAirdrop, _maxSupply, _limitMint)
  ...
  function requestChainlinkVRF() public onlyOwner {
    ...
    requestedVRF = true;
    emit OhNFTRandomnessRequest(now);
  }
  ...
  function fulfillRandomness(bytes32 _requestId, uint256 randomNumber)
    ...
    emit OhNFTRandomnessFulfil(_requestId, seed);
  }
    ...
    returns (string memory)
  {
    ...
    if (!requestedVRF || !isTokenRevealed) return defaultURI;    
...
contract OhBase is Ownable, ERC721 {
    using SafeMath for uint256;
