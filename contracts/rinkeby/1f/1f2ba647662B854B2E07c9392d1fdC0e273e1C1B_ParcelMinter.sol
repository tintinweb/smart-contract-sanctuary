...
contract ParcelMinter is Ownable, ContextMixin {
    ...
    function redeemParcels(uint256 tokenId, bytes32[] memory proof) public {
        ...
        require(proof.verify(merkleRoot, keccak256(abi.encodePacked(msg.sender))), "ParcelMinter: Not Allocated");
    ...
    function batchRedeemParcels(uint256[] calldata tokenIds, bytes32[] memory proof) public {
        require(merkleRoot != 0, "ParcelMinter: no MerkleRoot yet");
        uint256 tokenId;        
        address minter = msg.sender;

        require(proof.verify(merkleRoot, keccak256(abi.encodePacked(minter))), "ParcelMinter: Not Allocated");
