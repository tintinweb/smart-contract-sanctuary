...
contract SPK is ERC20Upgradeable, Owned {
    uint constant public INITIAL_FUND = 2e6 ether;
    ...
    modifier onlyMinter() {
        require(msg.sender == minter, "SPK: called by non-minter");
        _;
    }

    function initialize(address ownerAddr, address minterAddr) initializer public {
        __ERC20_init("Sparkmutual Token", "SPK");
        __Owned_init(ownerAddr);
        _mint(ownerAddr, INITIAL_FUND);
        minter = minterAddr;
    }

    /// @dev Set minter of SPK
    /// @param minterAddress Address of the minter
    function setMinter(address minterAddress) external virtual onlyOwner {
        require(minterAddress != address(0), 'SPK: zero address');
        minter = minterAddress;
        emit SetMinter(minterAddress);
    }

    /// @dev Mint SPK tokens to the receipt
    /// @param to Address of the receipt
    /// @param amount Amount of SPK to be minted
    function mint(address to, uint amount) external virtual onlyMinter {
        require(to != address(0), 'SPK: zero address');
        _mint(to, amount);
        emit Mint(to, amount);
    }
