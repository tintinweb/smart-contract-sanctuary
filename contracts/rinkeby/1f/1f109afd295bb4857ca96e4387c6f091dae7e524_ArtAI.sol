...
contract ArtAI is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    // purchase price
    uint256 private _purchasePrice = 1000000 wei;
    ...
    function namePurchaseStatus(string memory _paintingName)
        external
        view
        returns (bool)
    {
        return _namePurchases[_paintingName];
    }
    ...
    function ownerMint(
        ...
        string memory _paintingStyle,
        string memory _trait1,
        string memory _trait2,
        string memory _trait3
    ) external onlyOwner returns (uint256) {
        uint256 _newTokenId = _tokenSupply + 1;
        _safeMint(_owner, _newTokenId);

        _tokenImageHashes[_newTokenId] = _imageHash;
        _tokenMetadata[_newTokenId] = Metadata(
            _imageName,
            _paintingStyle,
            _trait1,
            _trait2,
    ...
    {
        Metadata memory _tokenData = _tokenMetadata[_tokenId];
        string memory _hash = _tokenImageHashes[_tokenId];

        return (
            _hash,
            _tokenData.imageName,
            _tokenData.paintingStyle,
            _tokenData.trait1,
            _tokenData.trait2,
            _tokenData.trait3
        );
