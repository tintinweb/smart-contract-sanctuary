...
contract BabyWhale is ERC20, Ownable {
    ...
    uint private _exchangeWalletCount;
    uint private _projectWalletCount;
    ...
    function updateExchangeWallets(address wallet, bool isCreate) external onlyOwner {
        if(isCreate){
            require(!_exchangeWallets[wallet], "This wallet already exists in exchange wallets");
            _exchangeWalletCount++;
            _exchangeWallets[wallet] = true;
        } else {
            require(_exchangeWallets[wallet], "This wallet does not exists in exchange wallets");
            _exchangeWalletCount--;
            delete _exchangeWallets[wallet];
        }
    }

    function getTotalExchangeWallet() external view returns (uint) {
    ...
    function isExchangeWallets(address sender, address recipient) internal view returns (bool){
        return _exchangeWallets[sender] || _exchangeWallets[recipient];
    }

    function updateProjectWallet(address wallet, bool isCreate) external onlyOwner {
        if(isCreate){
            require(!_projectWallets[wallet], "This wallet already exists in project wallets");
            _projectWalletCount++;
            _projectWallets[wallet] = true;
         } else {
            require(_projectWallets[wallet], "This wallet does not exists in project wallets");
            _projectWalletCount--;
            delete _projectWallets[wallet];
        }
    }

    function getTotalProjectWallet() external view returns (uint) {
    ...
    function isProjectWallets(address sender, address recipient) internal view returns (bool) {    
        ...
        returns (bool)
    {
        require(
            amount < _maxTxAmount ||
                isProjectWallets(msg.sender, recipient) || isExchangeWallets(msg.sender, recipient),
    ...
    function transferFrom(
        ...
        require(
            amount < _maxTxAmount ||
                isProjectWallets(sender, recipient) || isExchangeWallets(sender, recipient),
            "Transfer amount exceeds the maxTxAmount."
        );
        require(
            balanceOf(recipient).add(amount) < _maxBalance ||
