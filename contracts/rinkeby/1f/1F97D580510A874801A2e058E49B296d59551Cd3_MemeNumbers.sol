...
contract MemeNumbersRenderer is ITokenRenderer {
    ...
    function renderNFTImage(IMemeNumbers instance, string memory word)
        public
        view
    ...
    function tokenURI(
        ...
        uint256 tokenId,
        string memory word
    ) public override(ITokenRenderer) view returns (string memory) {
        return
            string(
                abi.encodePacked(
...
//TODO figure out Token ID to strings for _exists and ownerof
//TODO figure out what to do about burn
...
contract MemeNumbers is ERC721, Ownable {
    ...
    // We create a mapping from the nft's tokenId => the NFT's text
    mapping(uint256 => string) public nftStringValue;

    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    uint256 public auctionStarted;
    string[BATCH_SIZE] private forSale;
    ...
    function isForSale(string memory str) view public returns (bool) {
        for (uint256 i=0; i<forSale.length; i++) {
            //TODO: compare strings anf figure out if something exists if (forSale[i] == str) return  true; //!_exists(str);
            return true;
        }
        return false;
    }
    ...
    function getForSale() view external returns (string[] memory nums) {
        string[] memory batch = new string[](BATCH_SIZE);
        uint256 count = 0;
        for (uint256 i=0; i<forSale.length; i++) {
    ...
    function isMintedByBurn(uint256 tokenId) view external returns (bool) {
        return viaBurn[tokenId];
    }
     ...
     * @param str1 string to burn, must own
     * @param op Operation to burn num1 and num2 with, one of: add, sub, mul, div
     * @param str2 Number to burn, must own
     */
    function operate(string calldata str1, string calldata op, string calldata str2) public pure returns (string memory) {
     ...
     * @param str Number to mint, must be in the current for-sale sequence.
     *
     * Emits a {Refresh} event.
     */
    function mint(address to, string memory str) external payable {
        uint256 price = currentPrice();
        require(price <= msg.value, Errors.UnderPriced);
        require(isForSale(str), Errors.NotForSale);

        _tokenIds.increment();

        uint256 newNftTokenId = _tokenIds.current();
        nftStringValue[newNftTokenId] = str;
    ...
    function mintAll(address to) external payable {
        ...
        for (uint256 i=0; i<forSale.length; i++) {
            ...
            uint256 newNftTokenId = _tokenIds.current();
            nftStringValue[newNftTokenId] = forSale[i];
            _mint(to, newNftTokenId);
        }
     ...
     * @param str1 Number to burn, must own
     * @param op Operation to burn num1 and num2 with, one of: add, sub, mul, div
     * @param str2 Number to burn, must own
     */
    function burn(address to, string calldata str1, string calldata op, string calldata str2) external {
        //require(ownerOf(str1) == _msgSender(), Errors.MustOwnNum);
        //require(ownerOf(str2) == _msgSender(), Errors.MustOwnNum);

        string memory str = operate(str1, op, str2);
        //require(!_exists(str), Errors.AlreadyMinted);
        _tokenIds.increment();

        uint256 newNftTokenId = _tokenIds.current();
        nftStringValue[newNftTokenId] = str;
        _mint(to, newNftTokenId);
        viaBurn[newNftTokenId] = true;
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory) {
        ...
        return renderer.tokenURI(IMemeNumbers(address(this)), tokenId, nftStringValue[tokenId]);
    }
