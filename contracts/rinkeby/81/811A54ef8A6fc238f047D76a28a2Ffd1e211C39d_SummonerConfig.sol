    ...
    PausableUpgradeable
{
    ...
    function moveFunds(
        ...
        require(
            l1VaultConfig.whitelistedWallets(_destination) == true,
            "Wallet not recognized"
        );
        require(
            l1VaultConfig.getTokenBalance(_token) >= _amount,
    ...
    modifier onlySupportedToken(address _tokenAddress) {
        require(
            l1VaultConfig.isTokenSupported(_tokenAddress),
...
contract MosaicNFT is IMosaicNFT, Ownable, ERC721Pausable {

    event MosaicNFTMinted(
        address indexed nftOwner,
        string nftUri,
        uint256 nftId
    );
    ...
    // the original nft address
    address public originalNftAddress;
    // the original network id
    uint256 public originalNetworkId;
    // original Token name
    ...
    constructor(
        address _originalNftAddress, uint256 _originalNetworkId,
        string memory name, string memory symbol
    )
    public
    ERC721(string(abi.encodePacked("mNFT-", name)), string(abi.encodePacked("mNFT-", symbol))) {
        originalNftAddress = _originalNftAddress;
        originalNetworkId = _originalNetworkId;
        originalName = name;
        originalSymbol = symbol;
        // to supported interface IMosaicNFT
        _registerInterface(_INTERFACE_ID_MOSAIC_NFT);
    }

    function mintNewNFT(address _to, string memory _tokenURI, uint256 nftId) public onlyOwner {
        _safeMint(_to, nftId);
        _setTokenURI(nftId, _tokenURI);
        idTracker = nftId;
        emit MosaicNFTMinted(
            _to,
            _tokenURI,
            nftId
        );
    ...
    function getOriginalNftInfo() external view override returns (address, uint256, string memory, string memory){
        return (originalNftAddress, originalNetworkId, originalName, originalSymbol);
    }
...
contract VaultHop is BridgeBase {
    ...
    function addBridgeAddress(address tokenAddress, address bridgeAddress)
        external
        onlyAdmin
    {
        require(
...
contract VaultBiconomy is BridgeBase {
    ...
    constructor(address _liquidityPoolManagerAddress) public {
        liquidityPoolManager = ILiquidityPoolManager(
            _liquidityPoolManagerAddress
        );
    }

    function setLiquidityPoolManagerAddress(address _router)
        external
        onlyAdmin
    {
        require(_router != address(0), "Invalid address");
