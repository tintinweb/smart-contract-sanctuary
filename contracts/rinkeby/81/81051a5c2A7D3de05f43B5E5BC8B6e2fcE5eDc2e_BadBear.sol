...
contract BadBear is ERC721, Ownable, ContextMixin {
    ...
    using Counters for Counters.Counter;
    Counters.Counter private m_TokenId;

    uint256 private constant MAX_SUPPLY = 5555;

    uint256 private m_MaxMintPerAddress = 10;
    ...
    function mintBadBear(uint256 numberOfTokens) public payable {
        ...
        require(
            balanceOf(msg.sender).add(numberOfTokens) <= m_MaxMintPerAddress,
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            m_TokenId.increment();
            uint256 newItemId = m_TokenId.current();
