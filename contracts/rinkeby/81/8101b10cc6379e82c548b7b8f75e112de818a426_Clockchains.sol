...
*/
	...
	function ownerOf(uint256 tokenId) public view virtual override returns (address) {
		require(tokenId == 0, "Trying to peek before Big Bang, huh?");
		address owner = _owners[tokenId];
		return owner;
	}
	...
	function _bigbang() internal virtual {
		// Esta es la parte que todavía falta mejorar. En vez de iterar directamente sobre storage,
		// deberíamos hacerlo sobre memoria y después equiparar storage a memoria.

		for(uint256 i = 0; i < 365; i++) {
			_owners[i] = _msgSender();
		}
...
contract Clockchains is Context, ERC721Enumerable, ERC721Burnable, IERC2309 {
	...
	function MassMint() public virtual {
		require(_msgSender() == smart_contract_owner, "Not authorized!");
		require(ERC721Enumerable.totalSupply() == 0, "Big Bang already happened!");
