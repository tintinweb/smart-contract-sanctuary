...
error BotFarmFrens__TokenIdOffsetAlreadySet();
...
contract BotFarmFrens is IBotFarmFrens, ERC721Enumerable, Ownable, ReentrancyGuard, VRFConsumerBase {
    ...
    /// @inheritdoc IBotFarmFrens
    uint256 public override tokenIdOffset;
    ...
    function reveal() public override onlyOwner {
        if (tokenIdOffset != 0) {
            revert BotFarmFrens__TokenIdOffsetAlreadySet();
        }
        if (vrfRequestId != 0) {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        uint256 moddedId = (tokenId + tokenIdOffset) % MAX_ELEMENTS;
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        tokenIdOffset = (randomness % (MAX_ELEMENTS - 1)) + 1;
    }
...
interface IBotFarmFrens {
    ...
    /// @notice Emitted when BFF metadata is revealed.
    event Reveal();
    ...
    function tokenIdOffset() external view returns (uint256);
    ...
    /// @notice Reveal the BFF metadata.
    ///
