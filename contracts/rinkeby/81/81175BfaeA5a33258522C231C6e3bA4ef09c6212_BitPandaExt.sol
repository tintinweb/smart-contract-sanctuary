...
contract BitPandaExt is BitPanda {
    ...
    mapping (uint256 => bool) public hasVoted; // indicate if a tokenId has voted for the current round
    ...
    function reAssign() private {
        ...
        for(uint256 i=1;i<=MAX_TOKENS;i++){ // reset mapping to false
            if(_exists(i)){
                hasVoted[i] = false;
            }
        }
