...
contract CurseWords is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory metadata = string(abi.encodePacked(
            ...
            getAuxSize(tokenId),
            '"}], '));
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Empty Curse #', toString(tokenId), '", "description": "Empty Curse #', toString(tokenId), '", "description": "Empty Curses (for Assholes) are randomly generated, minimally artistic curse words generated on-chain. They do nothing...?", ', metadata, '"image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
