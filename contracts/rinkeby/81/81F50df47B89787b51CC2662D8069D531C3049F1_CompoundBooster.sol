...
interface ICompoundProxyUserTemplate {
    function init( address _op, bytes32 _lendingId, address _user, address _rewardComp ) external;
...
contract CompoundProxyUserTemplate {
    ...
    function borrow(
        ...
        uint256 returnBorrow = ICompoundCEther(_asset).borrow(_amount);

        emit Borrow(_asset, _for, _amount, returnBorrow, block.timestamp);
    ...
    function borrowErc20(
        ...
        emit Borrow(_asset, _for, _amount, borrowState, block.timestamp);
    ...
    function repayBorrowBySelf(
        ...
        IERC20(_asset).safeTransfer(op, cTokenBal);
    ...
    function recycle(
        ...
        if (borrows == 0) {
            ...
            uint256 cTokenBal = IERC20(_asset).balanceOf(address(this));
            IERC20(_asset).safeTransfer(_treasuryFund, cTokenBal);
...
contract CompoundBooster is Ownable {
    ...
    function Demo() public {
        ...
        setComptroller(0x2EAa9D77AE4D8f9cdD9FAAcd44016E746485bddb);
    }
        ...
        returns (bool)
    {
        ...
        poolInfo.push(
            PoolInfo({
                ...
                // rewardLiquidatePool: rewardLiquidatePool,
    ...
    function withdraw(uint256 _pid, uint256 _amount) public returns (bool) {
        ...
        // ICompRewardPool(pool.rewardLiquidatePool).withdrawFor(
        //     _msgSender(),
        //     _amount
        // );
    ...
    /* lending interfaces */
    function cloneUserTemplate(uint256 _pid,bytes16 _lendingId, address _sender) internal {
        LendingInfo memory lendingInfo = lendingInfos[_lendingId];

        if (lendingInfo.createdAt == 0) {
    ...
    // // lending id  = 0x18c3fec0353f3d66cbfc362c7f49fed2
    // function DemolockToken1() public {
    //     lockToken(0,0x18c3fec0353f3d66cbfc362c7f49fed2,0x0779Cfc15116283792698Da362F9ACBd1C4b8abf,300000000000000000);
    // }
    
    // function DemolockToken2() public {
    //     lockToken(1,0x287b29d8e20138a3f92f036cbe8c8998,0x0779Cfc15116283792698Da362F9ACBd1C4b8abf,80000000);
    // }
    ...
    function lockToken(
        ...
        uint256 exchangeRateStored = ICompound(pool.lpToken)
            .exchangeRateStored();
        uint256 cTokens = _amount.mul(1e18).div(exchangeRateStored);

        cTokens = cTokens.mul(100).div(80);

        require(IERC20(pool.lpToken).balanceOf(pool.treasuryFund) > frozenCTokens[_pid],"Insufficient balance");

        uint256 bal = IERC20(pool.lpToken).balanceOf(pool.treasuryFund).sub(
            frozenCTokens[_pid]
        );

        require(bal >= cTokens, "Insufficient balance");
        ...
        cloneUserTemplate(_pid,_lendingId, _user);

        // address myProxyUser = proxyUsers[_lendingId];
        LendingInfo storage lendingInfo = lendingInfos[_lendingId];

        lendingInfo.cTokens = cTokens;
        lendingInfo.amount = _amount;
    ...
    function unLockToken(
        ...
        frozenCTokens[lendingInfo.pid] = frozenCTokens[lendingInfo.pid].sub(lendingInfo.cTokens);

        ICompoundProxyUserTemplate(lendingInfo.proxyUser).repayBorrow{
            value: msg.value.sub(_interestValue)
        }(pool.lpToken, payable(_user));
    ...
    function unLockTokenErc20(
        ...
        ICompoundProxyUserTemplate(lendingInfo.proxyUser).repayBorrowErc20(
            pool.lpToken,
        ...
        if (_interestValue > 0) {
            ...
            IERC20(lendingInfo.underlyToken).safeTransfer(pool.rewardInterestPool,exchangeReward);
            IERC20(lendingInfo.underlyToken).safeTransfer(lendflareDepositer,lendflareDeposterReward);
        }
    }
    ...
    function totalSupplyOf(uint256 _pid) public view returns (uint256) {
        ...
        return IERC20(pool.lpToken).balanceOf(pool.treasuryFund).add(frozenCTokens[_pid]);
    }
