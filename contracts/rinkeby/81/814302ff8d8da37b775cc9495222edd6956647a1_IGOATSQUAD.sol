...
contract IGOATSQUAD is ERC721Enumerable, Ownable {
    // I-GOAT-Squad Contract Constants
    uint256 public MINT_ETH_COST = 1 * 10**16; // 0.01 ETH
    ...
    // The Genesis GOAT provenance index.
    uint256 public GENESIS_GOAT = 0;
    // Pre set reveal time.
    uint256 public REVEAL_TIMESTAMP = 1767225599;
    ...
    // And it begins...
    // Name: I-GOAT-Squad, Symbol: GOATS
    ...
    //////////////////////////////////////////////////////////////////////////////////////////
    /**
     * Withdraws the Sender's pending amount.
     */
    ...
    /**
     * Withdraws the specified amount from the owner's pending amount
     * (including extra funds from non mint sources).
     */
    function withdrawOwner(uint amount) external onlyOwner {
        require(address(this).balance > 0, "Address balance can't be 0");
        require(amount > 0, "Withdraw amount can't be 0");
        require(amount <= address(this).balance,
            "Withdraw amount can't be more than what the contract has.");
        ...
        require(amount <= amountAvailableOwner, 
            "The request amount can't be higher than what the owner has available.");
    ...
    /**
     * Company addresses can check their corresponding ETH balance with this funtion.
     */
    ...
    /**
     * Owner addresses can check their corresponding ETH balance.
     */
     ...
     */
    function reserveGOATS() external onlyOwner {
        require(totalSupply() < RESERVED_SUPPLY, "We already claimed the reserved supply");
    ...
    /**
     * External owner access to set the Genesis GOAT index.
     */
     function setGenesisGOAT() external onlyOwner {
        require(GENESIS_GOAT == 0, "Genesis index already set");
    ...
    /**
     * External owner access to set the Genesis Block and then Genesis GOAT index.
     */
    ...
    /////////////////////////////////////////////////////////////////////////////////////////
    /**
     * Sets the Genesis GOAT index.
     * Ideally, we won't get 0 as the Genesis GOAT, but it could be the case.
     * We need to keep things as random as possible.
     */
    function setGenesisGOATInner() private {
        require(GENESIS_GOAT == 0, "Genesis index already set");
        require(GENESIS_BLOCK != 0, "Genesis block must be set");
        ...
        GENESIS_GOAT = uint(keccak256(
            abi.encodePacked(block.timestamp, block.difficulty, msg.sender))) % MAX_SUPPLY;
    }

    /**
     * Sets the Genesis Block and then Genesis GOAT index.
     */
    function setGenesisBlockInner() private {
        ...
        // Now set the Genesis GOAT.
        setGenesisGOATInner();
    }

    /**
     * Updates the company member balances.
     */
    function updateAccounting() private {
        // 90% / Adresses... Rounds towards 0.
        // Owner the gets the remaning (10% + dust + unknown origin).
     ...
     */  
    function getHODLerTokenIDs(address HODLer) external view returns(uint256[] memory) {
        uint tokenCount = balanceOf(HODLer);

        uint256[] memory tokensIDs = new uint256[](tokenCount);
        for (uint i = 0; i < tokenCount; i++) {
            // Store the token indices.
            tokensIDs[i] = tokenOfOwnerByIndex(HODLer, i);
        }
     ...
     * Mints GOATs
     */  
    function mintGOAT(uint tokenAmount) external payable {
