...
contract Resurrectables is ERC1155, Ownable, ERC1155Burnable, RandomlyAssigned {
    constructor() ERC1155("") RandomlyAssigned(777, 0) {}
    ...
    function _getTokenId(uint randomId) internal pure returns (uint) {
        ...
        if (randomId < 113) return 2;
        return 3;
    }
    
    function mint(uint quantity) external ensureAvailabilityFor(quantity) {
        for (uint i = 0; i < quantity; i++) {
            uint randomId = nextToken();
            _mint(msg.sender, _getTokenId(randomId), 1, "");
        }
    }
