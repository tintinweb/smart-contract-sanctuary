...
abstract contract PreSaleRound is Ownable, IPreSaleRound {
  ...
  function capacity() public view virtual override returns (uint256);

  function saleParams() public view virtual override returns (
    uint256 price,
    uint256 minPurchase,
    uint256 maxPurchase);

  function releaseParams() public view virtual override returns (
    uint256 initialRelease,
    uint256  cliffPeriod,
    uint256 distributionPeriod);
  ...
  function closed() public view override returns (bool) {
    return _closed;
  }
  ...
  function buy() public payable whenActive {
    ...
    balances[msg.sender] = Math.max(Math.min(balance.add(tknbits), maxPurchase), minPurchase);
    uint256 soldTknbits = balances[msg.sender] - balance;
    soldExt += soldTknbits;
  ...
  function sold() public view override returns (uint256) {
   ...
   * @param beneficiary_ Address of wallet that bought some tokens
   * @return Number of TKNbits bought by beneficiary in current stage
   */
  function boughtBy(address beneficiary_) public view override returns (uint256) {
    return balances[beneficiary_];
  }
...
contract MuziblePreSaleRoundTest2 is PreSaleRound {
...
contract MuziblePreSaleRoundTest3 is PreSaleRound {
...
contract MuziblePreSaleRoundTest4 is PreSaleRound {
