...
/**
 * @title MetaMoose contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */

contract MetaMoose is ERC721Burnable {
    ...
    constructor() ERC721("Meta Moose", "Meta Moose") {
        MAX_MINT_SUPPLY = 8888;
        mintPrice = 0.088 ether;
        maxMint = 10;
        publicSaleIsEnabled = false;
    }
     ...
     */
    function removeFromWhiteList(address[] memory _list) external onlyOwner {
        for (uint256 i; i < _list.length; i++) {
    ...
    /**
     * Set maximum count to mint per transaction.
     */
     ...
     * Mint Moose by owner
     */
    function reserveMoose(address _to, uint256 _count) external onlyOwner {
    ...
    /*
    * Pause public sale if enabled, make enabled if paused
    */
    ...
    /*
    * Pause private sale if enabled, make enabled if paused
    */
    ...
    */
    function publicMint(uint256 _count) external payable {
        ...
        require(_count <= maxMint, "Invalid amount to mint");
        require(totalSupply().add(_count) <= MAX_MINT_SUPPLY, "Purchase would exceed max supply");
    ...
    */
    function privateMint(uint256 _count) external payable {
        require(privateSaleIsEnabled, "Private sale must be active to mint");
        require(_count <= maxMint, "Invalid amount to mint");
        require(totalSupply().add(_count) <= MAX_MINT_SUPPLY, "Purchase would exceed max supply");
        require(mintPrice.mul(_count) <= msg.value, "Ether value sent is not correct");
