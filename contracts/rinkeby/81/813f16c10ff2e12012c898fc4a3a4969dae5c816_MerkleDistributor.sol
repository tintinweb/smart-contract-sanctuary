...
interface IMerkleDistributor{
    ...
    // This event is triggered whenever a call to #claimoflandholder succeeds.
    event Claimoflandholder(address indexed receiver, uint indexed amount);
}
...
contract MerkleDistributor is IMerkleDistributor{
    ...
    function claim(uint256 index, address account, uint256 _amount, bytes32[] calldata merkleProof) external override {
        ...
        (uint _tokenamount,) = IERC20(holdertoken).vaults(account);
    ...
    function claimoflandholder(address _holder) external {
        ...
        (uint _tokenamount, uint _acquiredTime) = IERC20(holdertoken).vaults(_holder);
