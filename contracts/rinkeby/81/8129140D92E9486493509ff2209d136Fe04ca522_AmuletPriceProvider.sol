...
contract AmuletPriceProvider is Ownable {
    ...
    function setFallen(bool _isFallen)  external onlyOwner {
        isFallen = _isFallen;
    }
    ...
    function getLastPrice(address _amulet) external view returns (uint256) {
        ...
        if (feed[_amulet].providerType == OracleType.Uniswap) {
            ...
            if (token0 == WETH) {
                rate = (uint256)(_reserve0)*1e18/_reserve1;
            } else {
                rate = (uint256)(_reserve1)*1e18/_reserve0;
            }
        } else if (feed[_amulet].providerType == OracleType.ChainLink) {
            ( 
                uint80 roundID, 
                int price,
                uint startedAt,
