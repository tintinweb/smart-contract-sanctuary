...
contract AuctionHouse is IAuctionHouse, ReentrancyGuard {
    ...
    constructor(address _zora, address _weth) public {
        ...
        zora = _zora;
        wethAddress = _weth;
        timeBuffer = 0; // extend 15 minutes after every bid made in last 15 minutes
    ...
    function createAuction(
        ...
        emit AuctionCreated(auctionId, tokenId, tokenContract, duration, reservePrice, tokenOwner, zora, 0, auctionCurrency);
...
interface IMarket {
    ...
    struct BidShares {
        ...
        uint8 creator;
        // % of sale value that goes to the galery
        uint8 galery;
    ...
    function splitShare(uint8 sharePercentage, uint256 amount)
        external
        pure
        returns (uint256);
    ...
    function setGaleryAddress(address galeryAddress) external;
...
interface IMedia {
    ...
    function configure(address auctionHouseContract) external;
    function mintForCreator(address creator, MediaData calldata data) external;

    function getPreviousTokenOwner(uint256 tokenId)
        external
        view
        returns (address);
 ...
 */
contract Media is Whitelist, IMedia, ERC721Burnable, ReentrancyGuard {
    using Counters for Counters.Counter;
    using SafeMath for uint256;
     ...
     */
    constructor(address marketContractAddr) public ERC721X("Wonzimer", "WZM") {
        _owner = msg.sender;
     ...
     */
    function configure(address _auctionHouseContract) external override {
        require(msg.sender == _owner, "Media: Only owner");
        require(auctionHouseContract == address(0), "Media: Already configured");
        require(
            _auctionHouseContract != address(0),
            "Media: cannot set house contract as zero address"
        );
    ...
    ) public override onlyWhitelisted nonReentrant {
        require(
    ...
    function mintForCreator(address creator, MediaData memory data)
        public
        override
        onlyWhitelisted
        nonReentrant
    {
        _mintForCreator(creator, data);
    }
 ...
 */
abstract contract ERC721Burnable is Context, ERC721X {
    /**
...
contract Whitelist is Ownable {
    ...
    modifier onlyWhitelisted() {
        require(isWhitelisted(msg.sender), "Only whitelisted user");
        _;
    }

    function addToWhitelist(address _addedAddress) public onlyOwner {
        whitelist[_addedAddress] = true;
        emit AddedToWhitelist(_addedAddress);
    }

    function removeFromWhitelist(address _removedAddress) public onlyOwner {
        whitelist[_removedAddress] = false;
        emit RemovedFromWhitelist(_removedAddress);
    }
...
contract Market is IMarket {
    ...
    BidShares public shareFirstTime =
        BidShares(uint8(10), uint8(30), uint8(60), uint8(0));

    BidShares public shareOnSale =
        BidShares(uint8(10), uint8(5), uint8(85), uint8(0));
        ...
        returns (bool)
    {
        ...
        if (
            ...
            Media(mediaContract).getPreviousTokenOwner(tokenId)
        ) {
            bidShares = shareFirstTime;
        } else {
            bidShares = shareOnSale;
        }
        return
            bidAmount != 0 &&
            (bidAmount ==
                splitShare(bidShares.creator, bidAmount)
                    .add(splitShare(bidShares.galery, bidAmount))
    ...
    function splitShare(uint8 sharePercentage, uint256 amount)
        ...
        returns (uint256)
    {
        return amount.mul(sharePercentage).div(100);
    }
    ...
    function setGaleryAddress(address galeryAddress) external override {
        require(msg.sender == _owner, "Market: Only owner");
        require(
            galeryAddress != address(0),
            "Market: cannot set galery contract as zero address"
        );

        galery = galeryAddress;
    }
        ...
        onlyMediaCaller
    {
        ...
        require(
            isValidBid(tokenId, expectedBid.amount),
    ...
    function _finalizeNFTTransfer(uint256 tokenId, address bidder) private {
        ...
        if (
            Media(mediaContract).getPreviousTokenOwner(tokenId) == Media(mediaContract).tokenCreators(tokenId)
        ) {
            bidShares = shareFirstTime;
        } else {
            bidShares = shareOnSale;
        ...
        // Transfer bid share to galery
        token.safeTransfer(galery, splitShare(bidShares.galery, bid.amount));
