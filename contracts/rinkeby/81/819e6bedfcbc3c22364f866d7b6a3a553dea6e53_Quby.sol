...
// Inspired/Copied fromm CHUBBIES/bastardganpunks)
contract Quby is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    // // Truth.　
    string public constant R = "CryptoQuby! Collection";

    constructor(string memory baseURI) ERC721("Quby","QUBY")  {
        setBaseURI(baseURI);
    }
   ...
   function adoptQuby(uint256 numQuby) public payable {
        require(totalSupply() < MAX_QUBY, "Sale has already ended");
        require(numQuby > 0 && numQuby <= 20, "You can adopt minimum 1, maximum 20 quby");
        require(totalSupply().add(numQuby) <= MAX_QUBY, "Exceeds MAX_QUBY");
    ...
    function reserveGiveaway(uint256 numQuby) public onlyOwner {
        uint currentSupply = totalSupply();
