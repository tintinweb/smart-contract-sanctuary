...
// interface ICalculateCurveAndCompoundPriceModel {
//     function getToken0Price(uint256 _pid,uint256 _token) external view returns (uint256);
//     function getToken1Amount(uint256 _pid,uint256 _token) external view returns (uint256);
//     function getToken1Price(uint256 _pid,uint256 _token) external view returns (uint256);
//     function getToken0Amount(uint256 _token,uint256 _pid) external view returns (uint256);
//     function getAmount( uint256 _pid, uint256 _token0,  uint256 _supportPid ) external view returns (uint256, uint256);
// }
...
contract ConvexCompoundGauge {
    ...
    // address immutable calculateCurveAndCompoundPriceModel;
    ...
    mapping(uint256 => bool) public borrowNumberLimit;
    ...
    constructor(address _convexBooster, address _compoundBooster) public {
    ...
    function borrow(
        ...
        // require(borrowNumberLimit[_borrowNumber], "!borrowNumberLimit");
        // 转账给liquidateSponsor
        PoolInfo storage pool = poolInfo[_pid];

        uint256 supportPid = pool.supportPids[_supportPid];
        int128 curveCoinId = pool.curveCoinIds[_supportPid];

        _borrow(_pid, pool, supportPid, curveCoinId, _borrowNumber, _token0);
    ...
    function _borrow(
        ...
        // uint256 borrowNumber = ICompoundBooster(compoundBooster).getBlocksPerYears(_pid, true).mul(_borrowNumber);
        ...
        userLendings[msg.sender].push(
            UserLending({
                ...
                curveCoinId: curveCoinId,
                interestValue: borrowRate.mul(lendingAmount).div(1e18),
        ...
        _updateStatistics(_pid, token0Price, lendingAmount, _token0);
    }
    ...
    function _updateStatistics(
        ...
        borrowInfo.borrowAmount = borrowInfo.borrowAmount.add(token0Price);
        borrowInfo.supplyAmount = borrowInfo.supplyAmount.add(lendingAmount);
    ...
    function _repayBorrow(
        ...
        require(
            userLending.state == UserLendingState.LENDING,
            "!UserLendingState"
        );
        require(
            block.number <=
        ...
        require(
            _amount >= userLending.lendingAmount.add(userLending.interestValue),
            "amount error"
        );

        userLending.state = UserLendingState.EXPIRED;
        ...
        if (isErc20) {
            ...
            ICompoundBooster(compoundBooster).repayBorrow{value: msg.value}(
                userLending.lendingId,
                lendingInfo.user,
                userLending.interestValue
            );
    ...
    function liquidate(bytes16 _lendingId) public {
        ...
        userLending.state = UserLendingState.LIQUIDATED;
        ...
        returns (uint256)
    {
        ...
        if (userLending.state == UserLendingState.LIQUIDATED) return 0;
        ...
        returns (UserLendingState)
    {
        ...
        return userLending.state;
    }

    function getLendingAmount(
        ...
        )
    {
        ...
        uint256 lendingAmount = (_token0Price *
            1e18 *
        ...
        uint256 blocksRate = compoundRatePerBlock.mul(_borrowBlocks).add(1e18);
