...
contract Polkadog is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    uint256 public MAX = ~uint256(100000000 * 10**18);
    ...
    uint256 public _transaction_burn = 2;
    uint256 private _previousTransactionBurn = _transaction_burn;
    ...
    function setBurnPercent(uint256 burn_percentage) external onlyOwner() {
        _transaction_burn = burn_percentage;
    }
    ...
    function calculateTransactionBurn(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_transaction_burn).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _transaction_burn == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousLiquidityFee = _liquidityFee;
        _previousTransactionBurn = _transaction_burn;
