...
// File: ERC721AdvancedToken.sol

/**
 * @title MintableUserToken
 * @dev Only owner can mint tokens.
 */
contract ERC721AdvancedToken is MintableOwnableToken {
    //using Address for address;
    ...
    constructor(
        ...
        address signer
    ) MintableOwnableToken(name, symbol, contractURI, tokenURIPrefix, signer) 
    ...
    ///        The URI must link to JSON object with various fields: `name, description, image, external_url, attributes`.
    ///        Can also contain another various fields.
    function mintAndApprove(uint256 tokenId, uint8 v, bytes32 r, bytes32 s, Fee[] memory _fees, string memory tokenURI)  public {
        super.mint(tokenId, v, r, s, _fees, tokenURI);
        super.approve(proxyOperator, tokenId);
    }
    ...
    function lazyMint(
        ...
        bytes32 creatorHash = keccak256(abi.encode(
            tokenId,
            keccak256(bytes(tokenURI))
        ));
        address creator = msg.sender;
        validate(creator, creatorHash, creatorSignature);
        super.mint(tokenId, v, r, s, _fees, tokenURI);
    }
    ...
    // function mintAndTransfer(LibERC721LazyMint.Mint721Data memory data, address to) public   {
    ...
    function setProxy(address _proxyOperator) external onlyOwner {
        require(_proxyOperator != address(0));
    ...
    function validate(address signer, bytes32 structHash, bytes memory signature) internal view {
        ...
        require(
                structHash.recover(signature) == signer,
                "SIGNATURE_ERROR"
        );
    }
