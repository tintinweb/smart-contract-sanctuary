...
contract AutumnLeaf is ERC721Enumerable, Ownable, ReentrancyGuard{
    ...
    string[55] private colornames=["berry","wine","blush","apple","scarlet","coffee","squash","peanut","mustard","brick","cider","honey","amber","orange","butterscotch","moss","pickle","seaweed","pine","olive","fawn","tangerine","apricot","crail","stiletto",
    ...
    function traits(Leaf memory leaf) internal view returns(string memory){
        ...
        parts[1] = (leaf.bunches > 0)?string(abi.encodePacked(leafnames[leaf.leafstyle],' ',leafnames[leaf.bunches])):leafnames[leaf.leafstyle];
    ...
    function getBunches(uint256 tokenId) internal view returns(uint256){
        ...
        if(rand >= 185 && rand < 190){return ps=5;}
        if(rand >= 190 && rand < 198){return ps=6;}
        if(rand >= 198 && rand < 201){return ps=7;}
        return ps;
    }
    ...
    function getShadowBody() internal pure returns(string memory){
            return string(
                abi.encodePacked('<filter xmlns="http://www.w3.org/2000/svg" id="C" x="-50%" y="-50%" width="200%" height="200%">',
                '<feDropShadow dx="-6" dy="11" flood-color="#000000" flood-opacity="0.14" stdDeviation="0"/></filter>'));
    }
    ...
    function getSelectedleaves(Leaf memory leaf,uint256 tokenId) internal view returns(string memory){
        ...
        if(leaf.bunches==3){
            selectedLeaf = string(abi.encodePacked(pathstart[3],' stroke="',stemcolors[colorindex],'" ','fill="',colors[colorindex],'" ','/>',pathstemone[3],' stroke ="',
            stemcolors[colorindex],'"','/>','</g>'));
        }
        if(leaf.bunches==4){
