...
contract ART85 is ERC1155, Ownable, ERC1155Burnable{
    event Approve(
    ...
    event PlanEve(uint256 indexed planId);
    event InvestEve(uint256 indexed invId);
    ...
    function addPlanID(uint256 value) public returns (uint256) {
        _planid = _planid + value;
        return _planid;
    }
    ...
    struct PlanInfo {
        ...
        uint256 noofdays;
        uint256 interestapy;
        uint256 interestsavings;
        uint256 penalty;
        uint256 serviceFee;
    }
    mapping(uint256 => PlanInfo) public planInfo;
    ...
    function newPlan(
        ...
        uint256 planId = addPlanID(1);

        require(owner() == _msgSender(), "Ownable: caller is not the owner");
        require(penalty > 100, "The penalty should be less than 100");

        PlanInfo storage plan = planInfo[planId];
        plan.currency = currency;
        plan.noofdays = noofdays;
        plan.interestapy = interestapy;
        plan.interestsavings = interestsavings;
        plan.penalty = penalty;
        plan.serviceFee = serviceFee;
    ...
    struct InvestInfo {
        ...
        uint256 noofdays;
        uint256 penalty;
        uint256 investStartdatetime;
        uint256 investEnddatetime;
        address borrower;
        address lender;
        uint256 investperiod;
        string status;
    }
    // mapping(address => mapping(uint256 => InvestInfo)) public investInfo;
    mapping(uint256 => InvestInfo) public investInfo;

    uint256 public _invId;
    function addInvestID(uint256 value) internal returns (uint256) {
        _invId = _invId + value;
        return _invId;
    }

    function saleTokenInInvest(
        address from,
        address to,
        uint256 tokenId
    ) public payable {
        saleTokenTransfer(payable(from), tokenId, 1);
        _balances[tokenId][payable(to)] +=1;
        _balances[tokenId][from] -=1 ;
    }

    function saleTokenInInvestNew(
        address from,
        address to,
        uint256 tokenId
    ) public payable {
        saleTokenTransferNew(payable(to), tokenId);
        _balances[tokenId][payable(to)] +=1;
        _balances[tokenId][from] -=1 ;
    }
    ...
    function investToken(
        ...
        uint256 invId = addInvestID(1);
        InvestInfo storage user = investInfo[invId];

        require(_balances[tokenId][borrowerAddr] > 0, 'Not enoungh a token balance');
        require(planInfo[plan].noofdays > 0, 'Plan not found');

        uint feeAmt = (amount * planInfo[plan].serviceFee) / 1e20;
        uint totalGet = amount+feeAmt;
        uint totalSend = amount-feeAmt;
        uint balAmt = totalGet-totalSend;

        _transferToken(tokenAs, msg.sender, borrowerAddr, totalSend);
        _transferToken(tokenAs, msg.sender, address(this), balAmt);
        saleTokenInInvest(borrowerAddr, msg.sender, tokenId);

        user.nftId = tokenId;
        user.currency = currency;
        user.tokenAs = tokenAs;
        user.amount = amount;

        user.interestapy = planInfo[plan].interestapy;
        user.noofdays = planInfo[plan].noofdays;
        user.penalty = planInfo[plan].penalty;
        user.investperiod = 1;
        user.investStartdatetime = block.timestamp;
        user.investEnddatetime = block.timestamp + 180 days;
        user.borrower = borrowerAddr;
        user.lender = msg.sender;
        user.status = 'stop';

        emit InvestEve(invId);
    }

    function withdrawInvest(uint256 invId) public{
        require(investInfo[invId].penalty < 1e20, 'Withdraw not allowed');
        require(investInfo[invId].investEnddatetime > block.timestamp, 'Invest time crossed');

        uint nftId = investInfo[invId].nftId;
        uint penaltyval = investInfo[invId].penalty;
        uint amountval = investInfo[invId].amount;
        uint sendamountval = amountval - (amountval * penaltyval) / 1e20;

        require(_balances[nftId][msg.sender] > 0, 'Not enoungh a token balance');

        _transferToken(
            investInfo[invId].tokenAs,
            address(this),
            msg.sender,
            sendamountval
        );

        saleTokenInInvestNew(
            msg.sender,
            investInfo[invId].borrower,
            investInfo[invId].nftId
        );

        investInfo[invId].status = 'withdraw';
    }

    function claimInvest(uint256 invId) public{
        require(investInfo[invId].investEnddatetime < block.timestamp, 'Invest time crossed');

        uint nftId = investInfo[invId].nftId;
        uint noofdays = investInfo[invId].noofdays;
        uint investperiod = investInfo[invId].investperiod;
        uint interestapy = investInfo[invId].interestapy;
        uint amountval = investInfo[invId].amount;
        uint256 dayapyval = ((amountval * interestapy) / 1e20) / 365;
        uint sendamt = (noofdays * investperiod) * dayapyval;
        ...
        investInfo[invId].status = 'claim';
    }

    function stopInvest(uint256 invId) public{
        require(bytes(investInfo[invId].status).length != bytes('stop').length, 'Already invest stopped');
        require(investInfo[invId].investEnddatetime > block.timestamp, 'Invest closed');
        uint nftId = investInfo[invId].nftId;
        require(_balances[nftId][msg.sender] > 0, 'Not enoungh a token balance');
        investInfo[invId].status = 'stop';
        investInfo[invId].investperiod -= 1;
        investInfo[invId].investEnddatetime = investInfo[invId].investEnddatetime - 180 days;
    }

    function renewInvest(uint256 invId) public{
        require(bytes(investInfo[invId].status).length != bytes('renew').length, 'Already invest renewed');
        require(investInfo[invId].investEnddatetime > block.timestamp, 'Invest closed');
        uint nftId = investInfo[invId].nftId;
        require(_balances[nftId][msg.sender] > 0, 'Not enoungh a token balance');
        investInfo[invId].status = 'renew';
        investInfo[invId].investperiod += 1;
        investInfo[invId].investEnddatetime = investInfo[invId].investEnddatetime + 180 days;
    }

    function depositTokenToContract(address tokenAs, uint256 amount) public{
    ...
    function withdrawToken(address tokenAs, uint256 amount) public{
        ...
        require(tok.balanceOf(address(this)) > amount,'Not enough balance');
    ...
    function _transferToken(address tokenAs, address from, address to, uint256 amount) internal{
        ERC20 tok = ERC20(tokenAs);
        // require(
        //     tok.balanceOf(from) > amount,
        //     'Not enough balance'
        // );
        tok.transferFrom(from, to, amount);
    }
    
    function saleTokenTransferNew(address payable to, uint256 tokenId) internal {
        tokenTrans(tokenId, msg.sender, to, 1);
    }
