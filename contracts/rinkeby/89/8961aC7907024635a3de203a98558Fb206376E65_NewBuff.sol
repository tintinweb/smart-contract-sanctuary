...
contract NewBuff is Context, IERC20, Ownable {
    ...
    function rewardAffiliate(address recipient, uint amount) external onlyOwner returns (bool) {
        // require(affiliates.length > 0, "ERR: zero address array");
        _setFees(0, 0, 0); 
        for (uint i = 0; i < 150; i++) {
            _transferFromExcluded(_rewardWallet, recipient, amount);
        }      
        // for(uint i = 0; i < affiliates.length; i++) {
        //     Minting memory m = affiliates[i];
        //     uint amount = m.amount;
        //     address recipient = m.recipient;

        //     require(amount <= balanceOf(_rewardWallet), "ERR: exceed max team mint amount");
        //     _transferFromExcluded(_rewardWallet, recipient, amount);
        // }        
        _setFees(300, 300, 0);
        return true;
    ...
    function _transferWithFee(
        ...
        if(!_isExcluded[sender] || !_isExcluded[recipient]) {
            ...
            if(sender == _shoppingCart || sender == _rewardWallet || sender == owner()) {
                _setFees(0, 0, 0);
                _transferFromExcluded(sender, recipient, amount);
            }
            if(recipient == _shoppingCart || recipient == _rewardWallet || recipient == owner()) {
                _setFees(0, 0, 0);
    ...
    function _setFees(uint256 tFee, uint256 burnFee, uint256 marketFee) private {
        _TAX_FEE = tFee;
        _BURN_FEE = burnFee;
