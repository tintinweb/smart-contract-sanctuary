...
contract HashBridge is Ownable {
	...
	event VoteAdd(uint indexed voteId, uint indexed orderId, uint voteWeight, uint guarantee, bool success, bool ultimate);
	...
	function addOrder(uint _offerId, address _withdrawAddress, uint _amount, uint _payAmount) external {
		...
		require(_getExchangerHashBalance(offers[_offerId].ownerAddress) >= commissionForDeal, "Exchanger does not have enough tokens for the deal arbitration");
	...
	function vote(uint _orderId, bool _success) external {
        ...
        require(voteWeight >= minVoteWeight, "Not enough tokens on balance");
        ...
        if (ultimate) {
            ...
            for (uint i = 0; i < votes.length; i++) {
    			if (votes[i].orderId == _orderId && votes[i].success == _success) {
    			    ...
    			    if (commission > 0) {
    			        TransferHelper.safeTransferFrom(hashAddress, orders[_orderId].ownerAddress, votes[i].voterAddress, commission);
    		...
    		if (commissionForDeal > commissionSent) {
    		    TransferHelper.safeTransferFrom(hashAddress, orders[_orderId].ownerAddress, msg.sender, commissionForDeal - commissionSent);
    		}
        ...
        emit VoteAdd(voteId, _orderId, voteWeight, guarantee, _success, ultimate);
	}
	...
	function _getExchangerHashBalance(address _address) private returns(uint balance) {
	    balance = _getHashBalance(_address);
