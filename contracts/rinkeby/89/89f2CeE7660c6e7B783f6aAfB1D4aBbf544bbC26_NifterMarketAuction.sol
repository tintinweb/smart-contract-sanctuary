...
interface INifter {
     ...
     */
    function getPriceType(uint256 _tokenId)
    external
    view 
    returns (uint8);
...
contract NifterMarketAuction is INifterMarketAuction, Ownable, Payments {
    ...
    mapping(uint256 => SalePrice) private tokenPrices;

    // Mapping of ERC1155 contract to mapping of token ID to the current bid amount.
    mapping(uint256 => ActiveBid) private tokenCurrentBids;
    mapping(uint256 => ActiveBidRange) private tokenCurrentBidRange;
    ...
    function ownerMustHaveMarketplaceApproved(
        ...
        address owner = nifter.ownerOfToken(_tokenId);

        require(
            erc1155.isApprovedForAll(owner, address(this)),
    ...
    view
    {
        require(msg.sender == nifter.ownerOfToken(_tokenId) || msg.sender == address(nifter), 'sender must be the token owner');
    }
    ...
    function setSalePrice(
        ...
        tokenPrices[_tokenId] = SalePrice(msg.sender, _amount);
        nifter.setPrice(_amount, _tokenId);
        emit SetSalePrice(_amount, _tokenId);
    }
    ...
    function safeBuy(
        ...
        require(
            tokenPrices[_tokenId].amount == _amount,
    ...
    function buy(uint256 _tokenId) public payable {
        ...
        SalePrice memory sp = tokenPrices[_tokenId];
        ...
        require(
            tokenPriceFeeIncluded(_tokenId) == msg.value,
        ...
        // Transfer token.
        erc1155.safeTransferFrom(nifter.ownerOfToken(_tokenId), msg.sender, _tokenId, 1, '');
        ...
        Payments.payout(
            ...
            _makePayable(nifter.ownerOfToken(_tokenId)),
            owner,
        ...
        emit Sold(msg.sender, nifter.ownerOfToken(_tokenId), sp.amount, _tokenId);
    }
    ...
    returns (uint256)
    {
        ...
        if (_priceSetterStillOwnsTheToken(_tokenId)) {
            return tokenPrices[_tokenId].amount;
        }
        return 0;
    }
     ...
     */
    function tokenPriceFeeIncluded(uint256 _tokenId)
    public
    view
    returns (uint256)
    {
        ...
        if (_priceSetterStillOwnsTheToken(_tokenId)) {
            return
            tokenPrices[_tokenId].amount.add(
                tokenPrices[_tokenId].amount.mul(
                    nifter.getServiceFee(_tokenId)
                ).div(100)
            );
        }
    ...
    function setInitialBidPriceWithRange(uint256 _bidAmount, uint256 _startTime, uint256 _endTime, address _owner, uint256 _tokenId) external override {
        ...
        senderMustBeTokenOwner(_tokenId);
        _setBid(_bidAmount, payable(_owner), _tokenId);
        _setBidRange(_startTime,  _endTime, _tokenId);
    ...
    function bid(
        ...
        tokenCurrentBids[_tokenId].amount;
        require(
        ...
        //Check bid range
        ActiveBidRange memory range = tokenCurrentBidRange[_tokenId];
        uint8 priceType = nifter.getPriceType(_tokenId);
        ...
        address tokenOwner = nifter.ownerOfToken(_tokenId);
        ...
        // Set the new bid.
        _setBid(_newBidAmount, msg.sender, _tokenId);
        nifter.setBid(_newBidAmount, msg.sender, _tokenId);
    ...
    function safeAcceptBid(
        ...
        require(
            tokenCurrentBids[_tokenId].amount == _amount,
    ...
    function cancelBid(uint256 _tokenId) external {
        ...
        require(
            _addressHasBidOnToken(msg.sender, _tokenId),
    ...
    returns (uint256, address)
    {
        return (
        tokenCurrentBids[_tokenId].amount,
        tokenCurrentBids[_tokenId].bidder
        );
    }
    ...
    internal
    {
        tokenPrices[_tokenId] = SalePrice(address(0), 0);
    }
    ...
    function _addressHasBidOnToken(
        ...
        return tokenCurrentBids[_tokenId].bidder == _bidder;
    }
    ...
    returns (bool)
    {
        return tokenCurrentBids[_tokenId].bidder != address(0);
    }
    ...
    function _resetBid(uint256 _tokenId) internal {
        tokenCurrentBids[_tokenId] = ActiveBid(
            address(0),
            0,
            0
        );
    ...
    function _setBidRange(
        ...
        tokenCurrentBidRange[_tokenId] = ActiveBidRange(_startTime,  _endTime);
    }
