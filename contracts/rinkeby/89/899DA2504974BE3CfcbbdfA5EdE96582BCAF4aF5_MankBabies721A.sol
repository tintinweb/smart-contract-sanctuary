...
contract MankBabies721A is ERC721A, ReentrancyGuard, Ownable {
  using Strings for uint256;
  ...
  function randomOne(uint256 tokenId) internal view returns (Ape memory) {
    ...
    tokenId=uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp,tokenId))); // avoid dupes
...
/*
    uint256 public salePrice = 0.001 ether;
  uint256 public saleMintMax = 5;
  uint256 public totalSaleSupply = 5000;  //9999
  mapping(address => uint8) public saleMintCount;
