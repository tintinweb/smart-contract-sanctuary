...
contract WolfyStreetBets is Ownable, ReentrancyGuard, BaseRelayRecipient {
    using SafeMath for uint256;
    ...
    /**
     * biconomy trusted forwarders (Matic/Polygon)
     *
    ...
    */ 
    ...
    function stakeAsset(uint256 _amount, bool _isLowRisk) public  {        
        ...
        require(block.timestamp <= startTime.add(12 hours),"12 hour staking window has now passed!" ); // Can stake upto 12 hours from start pool.
    ...
    function withdrawSuppliedLiquidity(uint256 _amount, bool _isLowRisk) external nonReentrant {
        require(!poolStarted, "Cannot withdraw Liquidity while pool is running");
        if (_isLowRisk) {
    ...
    function  distributeLiquidityRewards(bool _isLowRisk) internal {
        for (uint i=0;i<LiquidityDetailsRecord.length;i++) {
            if(_isLowRisk && LiquidityDetailsRecord[i].isLowRisk == true) {
                //((LiquidityDetailsRecord[i]._etherLowRisk.mul(10**decimals())).mul(100*10**decimals()).div(totalLiquidityLowRisk.mul(10**decimals()));
