...
struct Ape {
  ...
  uint8 hat;
	...
	Ape memory ape = randomOne(tokenId);
        return string(abi.encodePacked(rl4,Base64.encode(bytes(string(abi.encodePacked(rl1,tokenId.toString(),'","image": "data:image/svg+xml;base64,',Base64.encode(bytes(genSVG(ape))),rl3))))));
    }
