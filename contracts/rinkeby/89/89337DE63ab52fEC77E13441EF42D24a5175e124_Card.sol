...
contract Card is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Metaverse Cards #',
                        stringTokenId,
                        '", "animation_url": "',
                        animationUrl,
                        '", "description": "Cards are collectable game based on crypto primatives. The cards can be unbundled and swapped. Everything is on chain.", "image": "data:image/svg+xml;base64,',
    ...
    function mintPublic() public {
        ...
        require(totalPublicSupply < 7778, "There are no more NFTs for public minting.");
