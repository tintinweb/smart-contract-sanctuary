    ...
    ERC721("Myobu lottery ticket", "MLT")
{
     ...
     * _lastTokenID: Used to mint NFTs, increases for each NFT minted
     * _feesClaimed: Used to store how many fees are claimed for the current lottery, resets per lottery
     * _rewardClaimed: Used to store if the reward has been claimed for the current lottery, resets per lottery
     * _currentLotteryID: A counter of how much lotteries there have been, increases by 1 each new lottery, used to get
    ...
    modifier onlyEnded {
        require(
            _lottery[_currentLotteryID.current()].endTimestamp <=
                block.timestamp,
    ...
    /**
     * @dev Modifier that requires that there is a lottery ongoing (on)
     */
    modifier onlyOn {
        require(
            _lottery[_currentLotteryID.current()].endTimestamp >
    ...
    /**
     * @dev Sets the chainlink contracts, the myobu address, the fee receiver, the ticket price, the ticket fee, the lottery length
     * the WETH contract, the link token and the chainlink fee. The owner can change if needed
     */
    constructor() {
        ...
        /// @dev so he can be able to start the lottery
        _rewardClaimed = true;
    }

    /**
     * @dev Buys 'X' amount of tickets with ETH, requires that he has at least (_minimumMyobuBalance) myobu,
     * and then loops over how much tickets he needs and mints the ERC721 tokens
     * @param amount: The amount of tickets to buy
     */
    function buyTickets(uint256 amount) external payable override onlyOn {
        Lottery memory lottery_ = _lottery[_currentLotteryID.current()];
        uint256 neededETH = amount * lottery_.ticketPrice;
        require(
            _myobu.balanceOf(_msgSender()) >= lottery_.minimumMyobuBalance,
            "MLT: You don't have enough myobu"
        );
        require(msg.value == neededETH, "MLT: Not enough ETH or too much ETH");
        uint256 tokenID = _lastTokenID;
        _lastTokenID += amount;
        /// @dev When it loops, only do math once
        for (uint256 i = tokenID; i < amount + tokenID; i++) {
            _mint(_msgSender(), i);
        }
    }
    ...
    function unclaimedFees() public view override returns (uint256) {
        return
            ((address(this).balance *
                _lottery[_currentLotteryID.current()].ticketFee) / 10000) -
            _feesClaimed;
    }

    /**
     * @dev Function that claims fees, saves gas so that its doesn't happen per ticket buy.
     * It works by updating _feesClaimed for each time this function is called.
     * So that it doesn't need an unclaimed fees variable instead it calculates fees by
     * doing (_ticketFee% of its balance) - feesclaimed.
     */
    ...
    {
        require(_rewardClaimed == false, "MLT: Reward already claimed");
        require(
    ...
    /**
     * @dev Gets a winner and sends him his amount won in $WETH, if he doesn't have myobu at the time of winning
     * send the _feeReceiver the jackpot
     */
    function fulfillRandomness(bytes32, uint256 randomness) internal override {
        _rewardClaimed = true;
        Lottery memory lottery_;
        /// @dev Get a random number in range of the token IDs
        uint256 x = lottery_.startingTokenID;
        uint256 y = _lastTokenID;
        uint256 resultInRange = x + (randomness % (y - x));
        address winner = ownerOf(resultInRange);
        uint256 amountWon = jackpot();
        if (_myobu.balanceOf(winner) >= lottery_.minimumMyobuBalance) {
            /// @dev Wraps ETH and sends it because the winner might be a contract with no fallback function
            /// which might make this function fail, make it unavailable to get their reward
            _WETH.deposit{value: amountWon}();
            _WETH.transfer(winner, amountWon);
        } else {
            /// @dev He sold his myobu, give the jackpot to the fee receiver. doesn't wrap
            payable(_feeReceiver).transfer(amountWon);
        }
    }
    ...
    function createLottery(
        ...
        uint256 newlotteryID = _currentLotteryID.current();
     ...
     * @dev Balance - The percentage for the next lottery - Fees
     */
    function jackpot() public view override returns (uint256) {
        ...
        return
            balance -
            ((balance *
                _lottery[_currentLotteryID.current()]
    ...
    function currentLotteryID() external view override returns (uint256) {
        return _currentLotteryID.current();
    }
    ...
    /**
     * @return If the reward has been claimed, can only be viewed at the end or will
     * only return false.
     */
    ...
    /**
     * @return How much fees have been claimed */
    function feesClaimed() external view override returns (uint256) {
        return _feesClaimed;
    }
    ...
    /**
     * @dev If a large amount of LINK that isn't needed is sent to this contract, the owner can
     * recover them using this function
     */
    ...
    /**
     * @dev In case the myobu token gets changed later on, the owner can call this
     ...
     * @param newFeeReceiver: The address that recieves all fees
     */
     ...
     * @param newChainlinkFee: The amount of LINK to pay for a VRF Oracle call
     */
    ...
    function extendCurrentLottery(uint256 extraTime) external onlyOwner onlyOn {
        uint256 currentLotteryEnd = _lottery[_currentLotteryID.current()]
        .endTimestamp;
        uint256 currentLotteryStart = _lottery[_currentLotteryID.current()]
        .startTimestamp;
        require(
            currentLotteryEnd + extraTime <= currentLotteryStart + 2629744,
            "MLT: Must be under or equal to 1 month"
        );
        _lottery[_currentLotteryID.current()].endTimestamp += extraTime;
    }
}
...
interface IMyobuLottery {
     ...
     * @param startingTokenID: The ERC721 token ID that the lottery starts at
     ...
     * @param ticketFee:  The percentage of a ticket sales that go to the _feeReceiver
    ...
    /**
     * @dev Function that distributes the reward, Gets a winner and sends him his amount won in $WETH, 
     * if he doesn't have myobu at the time of winning send the _feeReciever the jackpot
     */
    function distributeReward() external returns (bytes32 requestId);

    /**
     * @return The current jackpot
     * @dev Balance - The amount for the next lottery - Fees
     */
    ...
    /**
     * @return The info of a lottery (A struct)
     * Includes: The end of the lottery, and the starting token ID
     * See the Lottery struct for more info
     */
    ...
    /**
     * @return How much fees have been claimed */
    function feesClaimed() external view returns (uint256);
}
