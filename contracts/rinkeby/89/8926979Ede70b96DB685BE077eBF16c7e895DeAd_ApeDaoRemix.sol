...
contract ApeDaoRemix is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable, ReentrancyGuard {
  ...
  uint256 public constant maximumSupply = 111 * 50; // 50 editions of 111
  uint256 public constant maximumMintLimit = 5; // Maximum 5 per transaction
  uint256 public constant defaultClaimableApedTokens = 25; // 25 APED per token

  mapping(uint256 => uint256) internal apedTokensClaimedByToken;
  mapping(uint256 => uint256) internal apedTokenClaimOverrides;
  ...
  function getClaimableApedTokens(uint256 _tokenId) public view returns (uint256) {
    ...
    return max(max(apedTokenClaimOverrides[_tokenId], defaultClaimableApedTokens) - apedTokensClaimedByToken[_tokenId], 0);
  }

  /* Configuration of APED Tokens */

  function setClaimableApedTokens(uint256[] memory _tokenIds, uint256 _amount) public onlyOwner {
    for (uint8 i = 0; i < _tokenIds.length; i++) {
      apedTokenClaimOverrides[_tokenIds[i]] = _amount;
    }
  }
