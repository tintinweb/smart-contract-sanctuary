...
contract BuilderShop {
   ...
   //BenefitNft registry is hard coded
   address fitnftRegistryContract = 0x91C4FDc29D09585400522dcB9D11b4eaA64841ed;

   modifier onlyValidSender() {
       BenefitNftRegistry nftg_registry = BenefitNftRegistry(fitnftRegistryContract);
       bool is_valid = nftg_registry.isValidBenefitNftSender(msg.sender);
   ...
   function isValidBuilderShop(address builder_shop) public view returns (bool isValid) {
       //public function, allowing anyone to check if a contract address is a valid BenefitNft contract
   ...
   function createNewBuilderInstance(
       ...
       uint num_nifties,
       uint[] memory fitnft_quantities,
       string memory token_base_uri,
       string memory creator_name)
   public onlyValidSender returns (BenefitNftBuilderInstance tokenAddress) {

       contractId = contractId + 1;

       BenefitNftBuilderInstance new_contract = new BenefitNftBuilderInstance(
           _name,
           _symbol,
           contractId,
           num_nifties,
...
contract BenefitNftBuilderInstance is ERC721Full {
...
//   string public creatorName;
   ...
   //BenefitNft registry contract
   address public fitnftRegistryContract = 0x91C4FDc29D09585400522dcB9D11b4eaA64841ed;

   //master builder - ONLY DOES STATIC CALLS
   address public masterBuilderContract = 0x63b6885D64aF85A04BE2F10481bfe915d664dfC2;
   ...
   mapping (uint => Counters.Counter) public _numBenefitNftMinted;
   mapping (uint => uint) public _numBenefitNftPermitted;
   mapping (uint => uint) public _fitnftPrice;
   mapping (uint => string) public _fitnftIPFSHashes;
   mapping (uint => bool) public _IPFSHashHasBeenSet;

   //EVENTS

   //purchase + creation events
   event BenefitNftPurchased(address _buyer, uint256 _amount, uint _tokenId);
   event BenefitNftCreated(address new_owner, uint _fitnftType, uint _tokenId);
   ...
   constructor(
       ...
       //offset starts at 1 - there is no fitnftType of 0
       for (uint i=0; i<(num_nifties); i++) {
           _numBenefitNftPermitted[i+1] = fitnft_quantities[i];
       }
   }
   
   function setBenefitNftIPFSHash(uint fitnftType, 
                            string memory ipfs_hash) onlyValidSender public {
        //can only be set once
        if (_IPFSHashHasBeenSet[fitnftType] == true) {
            revert("Can only be set once");
        } else {
            _fitnftIPFSHashes[fitnftType] = ipfs_hash;
            _IPFSHashHasBeenSet[fitnftType]  = true;
        }
    }

   function isBenefitNftSoldOut(uint fitnftType) public view returns (bool) {
       if (fitnftType > numNiftiesCurrentlyInContract) {
           return true;
       }
       if (_numBenefitNftMinted[fitnftType].current() > _numBenefitNftPermitted[fitnftType]) {
           return (true);
       } else {
           return (false);
       }
   ...
   function giftBenefitNft(address collector_address, 
                      uint fitnftType) onlyValidSender public {
       //master for static calls
       BuilderMaster bm = BuilderMaster(masterBuilderContract);
       _numBenefitNftMinted[fitnftType].increment();
       //check if this BenefitNft is sold out
       if (isBenefitNftSoldOut(fitnftType)==true) {
           revert("300fit NFT sold out!");
       }
       //mint a BenefitNft
       uint specificTokenId = _numBenefitNftMinted[fitnftType].current();
       uint tokenId = bm.encodeTokenId(contractId, fitnftType, specificTokenId);
       string memory tokenIdStr = bm.uint2str(tokenId);
       string memory tokenURI = bm.strConcat(baseURI, tokenIdStr);
       string memory ipfsHash = _fitnftIPFSHashes[fitnftType];
       //mint token
       _mint(collector_address, tokenId);
       _setTokenURI(tokenId, tokenURI);
       _setTokenIPFSHash(tokenId, ipfsHash);
       //do events
       emit BenefitNftCreated(collector_address, fitnftType, tokenId);
   }

}

contract BenefitNftRegistry {
   function isValidBenefitNftSender(address sending_key) public view returns (bool);
...
contract BuilderMaster {
   ...
   function getBenefitNftTypeId(uint tokenId) public view returns (uint);
   function getSpecificBenefitNftNum(uint tokenId) public view returns (uint);
   function encodeTokenId(uint contractId, uint fitnftType, uint specificBenefitNftNum) public view returns (uint);
...
/**
* Contracts and libraries below are from OpenZeppelin, except BenefitNft builder instance
**/
