...
contract FloralFractal is Ownable, ERC721Enumerable  {
  ...
  // Sale
  //uint public FF_PRESALE_PRICE = 0.05 ether;
  uint256 public price = 0.05 ether; //0.05 Ether
  uint public MAX_FF = 1000;
  uint private reservedFF = 20;
  ...
  uint private nbForSale = MAX_FF - reservedFF;
  ...
  function airdrop(address[] calldata _recipients) external onlyOwner {
      require(
          totalSupply() + _recipients.length <= (MAX_FF - reservedFF),
