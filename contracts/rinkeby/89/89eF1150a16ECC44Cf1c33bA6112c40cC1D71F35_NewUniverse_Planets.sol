...
contract NewUniverse_Planets is ERC721, VRFConsumerBase {
    ...
    ERC721("NewUniverse_Planets", "NU_PLNTS")  {
        vrfCoordinator = _VRFCoordinator;
        keyHash = _keyHash;
        fee = 0.1 * 10 ** 18;
        // 0.1 LINK
    ...
    mapping(bytes32 => string) requestToPlanetName;
    mapping(bytes32 => address) requestToSender;
    mapping(bytes32 => uint256) requestToTokenId;


    function requestNewRandomPlanet(string memory name) public returns (bytes32) {
        require(
            LINK.balanceOf(address(this)) >= fee,
            "Not enough LINK - fill contract with faucet"
        );
        bytes32 requestId = requestRandomness(keyHash, fee);
        requestToPlanetName[requestId] = name;
    ...
    override
    {
        ...
        uint256 gas = (randomNumber % 100);
        uint256 liquids = ((randomNumber % 10000) / 100);
        uint256 metals = ((randomNumber % 1000000) / 10000);
        uint256 organisms = ((randomNumber % 100000000) / 1000000);
        uint256 crystals = ((randomNumber % 10000000000) / 100000000);
        uint256 minerals = ((randomNumber % 1000000000000) / 10000000000);
...
//        uint32 minerals = 50;
        ...
        string memory planetType = "lava";
        string memory name = "lava";
