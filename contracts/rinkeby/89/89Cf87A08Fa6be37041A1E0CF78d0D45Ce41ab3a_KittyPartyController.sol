...
contract KittyPartyController is KittyPartyStateTransition, IKittenPartyInit {
    ...
    function applyWinnerStrategy() public timedTransitions() atStage(KittyPartyStages.Payout){
        ...
        mintValue(kreator, amountToSendKreator);
    ...
    returns(bool){
        ...
        // console.log("Trying to amountInDAIPerRound %s with allowance to %s", amountInDAIPerRound, allowance);
...
enum KittyPartyStages {
    ...
    uint8 public durationInDays;
    uint256 public currentRound = 1;
    uint256 public activityDuration = 20;
    ...
    modifier timedTransitions() {
        if (stage == KittyPartyStages.InitialCollection && block.timestamp >= (creationTime + activityDuration * 1 minutes)) {
           nextStage();
           nextStage();// we do a double jump here
        }
        if (stage == KittyPartyStages.Collection && block.timestamp >= creationTime + activityDuration * 1 minutes) {
            nextStage();
        }
        if (stage == KittyPartyStages.Staking && block.timestamp >= creationTime + durationInDays * 1 minutes) {
            nextStage();
        }
        if (stage == KittyPartyStages.Payout && block.timestamp >= creationTime + activityDuration * 1 minutes && numberOfRounds >= currentRound) {
