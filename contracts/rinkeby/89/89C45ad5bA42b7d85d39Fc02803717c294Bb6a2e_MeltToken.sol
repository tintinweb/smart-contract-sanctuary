...
contract MeltToken is DynamicArweaveHash {
    ...
    // Artist's address 
    constructor(address creator) DynamicArweaveHash(0xE487Ed5ec80ccA6e8A44F6D6f3f712088df8Ad4F) {
        _creator = creator;
        _creationTimestamp = block.timestamp;
    }

    function _getName() internal pure override returns(string memory) {
        return "Jake 365 Melting Token"; // To confirm with artist
    }

    function _getDescription() internal pure override returns(string memory) {
        return "A token that changes images every day for 365 days..."; // To confirm with artist
    }

    function _getImageHash() internal view override returns(string memory) {
        uint256 daysPassed = ((block.timestamp - _creationTimestamp)/86400) % 365;
        return arweaveHashes[daysPassed];
    }
