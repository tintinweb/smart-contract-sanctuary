...
contract Stakable   is Ownable
{
    ...
    event       ClaimedNftBonus( address indexed wallet, uint256 amount, string collectionName);
    ...
    //----------------------------------------------------------------------
    function    listUnclaimedNftBonuses(string memory collectionName, address userWallet, uint256[] memory tokenIds) external view returns(TNftReward[] memory)
    {
        return listUnclaimedNftBonusesEx(collectionName, userWallet, tokenIds);
    }
    //----------------------------------------------------------------------
    function    listUnclaimedNftBonusesEx(string memory collectionName, address userWallet, uint256[] memory tokenIds) internal view returns(TNftReward[] memory)
    {
        TNftBonusCollectionInfo memory nft = nftBonusCollections[collectionName];
    ...
    //----------------------------------------------------------------------
    function    calculateFoRewardForNft(TNftBonusUser memory user) internal view returns(uint256)
    {
        if (user.since > block.timestamp)     return 0;     // overflow error
        if (user.foPeriodDurationInSec==0)      return 0;     // div zero

        uint256 age = block.timestamp - user.since;

        uint256 reward = (age * user.foPeriodGain) / user.foPeriodDurationInSec;

        if (block.timestamp < user.endTimestamp)              // Ya du reward a recuperer
        {
            if (block.timestamp >= user.halvingTimestamp)     // on est dans la periode normale
            {
                reward /= 2.0;
            }
        }
...
contract    PEZToken    is  Context, IERC20, IERC20Metadata , Ownable, ReentrancyGuard, Stakable//, IPEZToken
{
    ...
    mapping(address => bool) public  mintingAllowedContracts;
    ...
    //----------------------------------------------------------------------
    function    externalMint(address recipient, uint256 amountInWei)  external
    {
        bool isAllowedContract = mintingAllowedContracts[ msg.sender ];

        require(isAllowedContract==true, "Forbidden caller. Must be an allowed contract");

        _mint(recipient, amountInWei);
    }
    //---------------------------------------------------------------------- for FANS collection contracts mostly
    function    setExternalMintContract(address contractToAdd) external
    {
        require(contractToAdd!=address(0x0), "BlackHole not allowed");

        mintingAllowedContracts[contractToAdd] = true;
    }
    //----------------------------------------------------------------------
    ...
    function    claimNftBonusEx(string memory collectionName, address userWallet, uint256[] memory tokenIds) internal returns(bool)
    {
        ...
        TNftReward[]            memory rewards = listUnclaimedNftBonusesEx(collectionName, userWallet, tokenIds);
        TNftBonusCollectionInfo memory nft     = nftBonusCollections[collectionName];
        ...
        for (uint256 i=0; i<n; i++)
        {
            ...
            if (reward>0)
            {
                ...
                emit ClaimedNftBonus(userWallet, reward, nftBonusUser.collection);
