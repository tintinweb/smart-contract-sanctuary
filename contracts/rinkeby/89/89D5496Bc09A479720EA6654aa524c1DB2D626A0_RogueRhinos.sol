...
contract RogueRhinos is ERC721Enumerable, ERC721Burnable, Ownable {
	uint256 private constant MAX_RHINOS = 9999;
	uint256 private constant MINT_MAX = 20;
	uint256 public mintPrice = 0.002 ether; //SET LOW FOR TESTING
	uint256 public currentSupply = 0;
	address public msgSigner = address(0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266); //default hardhat signer
	address private deityAddress; //trusted contract with burning rights for future utility
	 ...
	 */
	constructor(string memory baseURI_) ERC721("RogueRhinos", "RYNO") {
		setBaseURI(baseURI_);

		teamMap[msg.sender] = true; //team1
		teamMap[0x2B031166645D849D5b06fEA65977cb487B4dD993] = true; //team2
		teamMap[0x7AFF24491e6b145D56Af5EFD3aFC05b8eFBf64c4] = true; //team3
		teamMap[0xdF076b2F3Ba014C93f0953cB7A996355064c62a2] = true; //team4

		teamList[0] = msg.sender; //team1
		teamList[1] = 0x2B031166645D849D5b06fEA65977cb487B4dD993; //team2
		teamList[2] = 0x7AFF24491e6b145D56Af5EFD3aFC05b8eFBf64c4; //team3
		teamList[3] = 0xdF076b2F3Ba014C93f0953cB7A996355064c62a2; //team4
	}
	...
	modifier onlyTeam {
		require(teamMap[msg.sender] == true, "Caller not on rhinos team");
		_;
	}
	...
	//external
	/**
     * @notice list the ids of all the rhinos in _owner's wallet
     * @param _owner is the wallet address of the rhino owner
     * */
    function listMyRhinos(address _owner) 
        external 
        view 
        returns (uint256[] memory) 
    {
        uint256 nRhinos = balanceOf(_owner);
        uint256[] memory rhinoList = new uint256[](nRhinos);
        for (uint256 ii = 0; ii < nRhinos; ii++) {
        	rhinoList[ii] = tokenOfOwnerByIndex(_owner, ii);
        }
        return rhinoList;
	...
	function mintRhino(uint256 _numRhinos) public payable saleIsLive {
		require(
			...
			"you can mint between 1 and 100 rhinos at once"
		);
		require(
	...
	/**
	 * @notice mint rhinos from the whitelist.  
	 ...
	 * @param _claimQuota initial whitelist quota to claim
	 */
		...
		presaleIsLive
	{
		...
		require(
			(_numToMint + minted[msg.sender]) <= _claimQuota,
     ...
     * @param _to is the address to transfer admin rights to
     */
    function transferMembership(address _to) public onlyTeam {
        ...
        emit MembershipTransferred(msg.sender, _to);
    }
	...
	function withdraw() public onlyTeam {
		...
		//SET SPLITS BEFORE DEPLOY
		payable(teamList[0]).send(bal * 40 / 100); //team0
		payable(teamList[1]).send(bal * 30 / 100); //team1
		payable(teamList[2]).send(bal * 20 / 100); //team2
		payable(teamList[3]).send(bal * 10 / 100); //team3
	}
	 ...
	 */
	function _mintRhinos(address _to, uint256 _num) private {
		require(
			currentSupply + _num <= MAX_RHINOS,
			"Too few Rhinos left!"
		);
