...
contract CubeViews {
    ...
    // function allUnderlyingPrices(CubePool pool) external view returns (uint256[] memory prices) {
    //     ChainlinkFeedsRegistry feedRegistry = pool.feedRegistry();
    //     uint256 n = pool.numCubeTokens();
    //     prices = new uint256[](n);
    //     for (uint256 i = 0; i < n; i++) {
    //         (, string memory symbol, , , , , , , , ) = pool.params(pool.cubeTokens(i));
    //         prices[i] = feedRegistry.getPrice(symbol);
    //     }
    // }
