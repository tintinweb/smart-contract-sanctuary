...
contract BearXNFTStaking is Ownable{
    ...
    uint256 constant DURATION_FOR_REWARDS = 1 minutes;
    ...
    function unStake(uint256[] memory _ids) public isOnlyStaker{
        ...
        if(sumofgenesisbear >= 5){
            ...
            if(flag && stakes[msg.sender].length != 0){
                ...
                require(IERC20(ROOTxTokenAddress).balanceOf(address(this)) >= claimAmount * (10 ** 18), "ROOT: staking pool has insufficient funds");
                IERC20(ROOTxTokenAddress).transfer(msg.sender, claimAmount * (10 ** 18));
            }
            for(uint256 i = 0; i < stakes[msg.sender].length; i ++){
                ...
                if(calDay(stakes[msg.sender][i].claimedDate) <= 3)
                    continue;
                IERC20(SROOTxTokenAddress).approve(UNISWAP_V2_ROUTER, _amountIn);
                ...
                require(IERC20(SROOTxTokenAddress).balanceOf(address(this)) >= claimAmountOfSROOTxToken, "staking pool has insufficient funds");
                IERC20(SROOTxTokenAddress).transfer(msg.sender, claimAmountOfSROOTxToken);
             ...
             for(uint256 i = 0; i < stakes[msg.sender].length; i ++){
                if(isGenesisBear(stakes[msg.sender][i].nft_id) ) {
                    ...
                    require(IERC20(ROOTxTokenAddress).balanceOf(address(this)) >= claimAmountOfROOTxToken * (10 ** 18), "staking pool has insufficient funds");
                    IERC20(ROOTxTokenAddress).transfer(msg.sender, claimAmountOfROOTxToken * (10 ** 18));
                    stakes[msg.sender][i].claimedDate = block.timestamp;
                }
            }
        }
        for(uint256 i =0; i<_ids.length; i++){
            IERC721(BearXNFTAddress).transferFrom(address(this),msg.sender,_ids[i]);
            totalStakes--;
    ...
    function claimOfROOTxToken() external isOnlyStaker returns(uint256, bool){
        ...
        if(sumofgenesisbear >= 5){
            ...
            require(IERC20(ROOTxTokenAddress).balanceOf(address(this)) >= finalOfROOTxToken * (10 ** 18), "staking pool has insufficient funds");
            IERC20(ROOTxTokenAddress).transfer(msg.sender, finalOfROOTxToken * (10 ** 18));
            status = true;
        }
        else {
            for(uint256 i = 0; i < stakes[msg.sender].length; i ++){
    ...
    function claimALL() external isOnlyStaker returns(uint256, uint256, uint256){
        ...
        if(sumofgenesisbear >= 5){
            ...
            if(flag && stakes[msg.sender].length != 0){
                uint256 claimAmount =  10;
                finalOfROOTxToken += claimAmount;
                require(IERC20(ROOTxTokenAddress).balanceOf(address(this)) >= finalOfROOTxToken * (10 ** 18), "ROOTxTokenAddrres: staking pool has insufficient funds");
                IERC20(ROOTxTokenAddress).transfer(msg.sender, finalOfROOTxToken * (10 ** 18));
            ...
            for(uint256 i = 0; i < stakes[msg.sender].length; i ++){
                ...
                finalOfWETH += _amountOutMin;
                require(IERC20(SROOTxTokenAddress).balanceOf(address(this)) >= claimAmountOfSROOTxToken, "SROOTxToken: staking pool has insufficient funds");
                IERC20(SROOTxTokenAddress).transfer(msg.sender, claimAmountOfSROOTxToken);
                stakes[msg.sender][i].claimedDate = block.timestamp;
            }
