...
// File: contracts/TetrisRewardGame.sol
...
contract TetrisGame is Ownable, IERC721Receiver, ERC165, ERC721Holder {
    ...
    function sendReward(uint gameId) public payable onlyOwner {
        require(games[gameId] != 0, "Error (storeScore): game not found");
        require(rewardedNftIds[gameId] != 0, "Error (storeScore): nft not found");
    ...
    function storeScore(uint gameId, uint256 amount) public {
        require(games[gameId] != 0, "Error (storeScore): game not found");
        require(amount > highScores[gameId], "Error (storeScore): low score");
    ...
    function setRewardedNFTAddress(uint gameId, address nftAddress) public onlyOwner {
        require(games[gameId] != 0, "Error (setRewardedNFTAddress): game not found");
        nftAddresses[gameId] = nftAddress;
    }

    function resetHighScore(uint gameId) public onlyOwner {
        require(games[gameId] != 0, "Error (resetHighScore): game not found");
        highScores[gameId] = 0;
        highScoreAddresses[gameId] = address(0);
    }

    function addGame() public onlyOwner {
        games.push(games.length + 1);
        highScores[games.length + 1] = 0;
        highScoreAddresses[games.length + 1] = address(0);
    }
}