...
contract PielandOctopieUpgradeable is ERC721BonfireWithPreSaleBasedOnWhitelistUpgradeable {
    function initialize() initializer public {
      ERC721BonfireWithPreSaleBasedOnWhitelistUpgradeable.init("Octopie", "OCTOPIE", "https://metadata.livetoken.co/api/metadata/OCTOPIE/", 30, 5, 5, .08 ether, 10);
    }
}
...
contract ERC721BonfireWithPreSaleBasedOnWhitelistUpgradeable is ERC721BonfireBaseUpgradeable {
    ...
    function mintPreSale(uint256 numberOfTokens) public payable whenPreSaleOngoing {
        ...
        require(MAX_PER_TX == 0 || (numberOfTokens > 0 && numberOfTokens <= MAX_PER_TX), "Cannot mint that many at once");
        require(numMintedInPreSale + numberOfTokens <= MAX_PRE_SALE, "Purchase would exceed max pre-sale supply");
        require(MAX_TOTAL_PER_WALLET == 0 || preSaleMintedAmounts[msg.sender] + numberOfTokens <= MAX_TOTAL_PER_WALLET, "Exceeds pre-sale max per wallet");
...
//   * derived classes include ERC721BonfireTicket, ERC721BonfireWithPreSaleBasedOnOwnershipOfAnotherNFT, ERC721BonfireWithPreSaleBasedOnTickets, and ERC721BonfireWithPreSaleBasedOnWhitelist
...
//     * contract owner calls giveAway() and pays gas
//     * contract owner calls reserve() and pays gas
...
contract ERC721BonfireBaseUpgradeable is ERC721EnumerableUpgradeable, OwnableUpgradeable {
    ...
    function mint(uint256 numberOfTokens) public payable virtual whenMainSaleOngoing {
        require(MAX_PER_TX == 0 || numberOfTokens > 0 && numberOfTokens <= MAX_PER_TX, "Cannot mint that many at once");
        require(MAX_TOTAL_SUPPLY == 0 || totalSupply() + numberOfTokens <= MAX_TOTAL_SUPPLY, "Purchase would exceed max supply");
        require(MAX_TOTAL_PER_WALLET == 0 || mainSaleMintedAmounts[msg.sender] + numberOfTokens <= MAX_TOTAL_PER_WALLET, "Exceeds max per wallet");
        require(MINT_PRICE * numberOfTokens <= msg.value, "Insufficient ether sent");
    ...
    function giveAway(address _to, uint256 numberOfTokens) external onlyOwner {
        require(MAX_TOTAL_SUPPLY == 0 || totalSupply() + numberOfTokens <= MAX_TOTAL_SUPPLY, "Give away exceeds max supply");
