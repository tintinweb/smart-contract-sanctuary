...
contract Pledge is Context , Ownable {
    ...
    mapping(address => bool) private _userValue;
    ...
    function pledgeNft(uint256 _tokenId, uint256 _lock) public returns(bool){
        ...
        uint256 length = usderPledge.add(1);
        _ownedTokens[_msgSender()][length] = _tokenId;
    ...
    function distribute(uint256 _amount) public returns(bool){
        ...
        for (uint256 i = 0; i < length; i++){
            if( personalWeight(usergroup[i]) > 0){
                uint256 earnings = _amount.mul(personalWeight(usergroup[i])).div(allWeights);
                _personalEarnings[_msgSender()] =  _personalEarnings[_msgSender()].add(earnings);
            }
        }
    ...
    function takeOutNft(uint256 _tokenId) public returns(bool){
        ...
        if (tokenIndex != lastTokenIndex) {
            uint256 lastTokenId = _ownedTokens[_msgSender()][lastTokenIndex];

            _ownedTokens[_msgSender()][tokenIndex] = lastTokenId; // Move the last token to the slot of the to-delete token
            _ownedTokensIndex[lastTokenId] = tokenIndex; // Update the moved token's index
        }

        // This also deletes the contents at the last position of the array
        delete _ownedTokensIndex[_tokenId];
        delete _ownedTokens[_msgSender()][lastTokenIndex];
        ...
        allWeights =  allWeights.sub(_nftWeights[_tokenId]);
    ...
    constructor()  Ownable() {
        ...
        setBusd(0x84058D5a15a511f93F4c058647424dBf1B36425E);
        setNftAddress(0xE805CfFA5963a314867ecE86a5989343f154F620);
        _name = "Pledge";
        _symbol = "Pledge";
        setNftWeights(1,25);
        setNftWeights(2,500);
