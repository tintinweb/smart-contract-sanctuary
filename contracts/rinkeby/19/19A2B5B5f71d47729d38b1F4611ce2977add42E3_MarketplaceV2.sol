...
contract MarketplaceV2 is Ownable {
    ...
    function putOnSale(
        ...
        if (_tokenType == TokenType.ERC1155) {
            require(Collection(_collection).creators(_id) != address(0), "Wrong token id");
            require(IERC1155(_collection).balanceOf(msg.sender, _id) >= _amount, "Wrong amount");
        } else {
            require((IERC721(_collection).ownerOf(_id) == msg.sender) && (_amount == 1), "Wrong amount");
        }
        positionsCount++;
    ...
    function transferWithFees(uint256 _position, uint256 _amount) internal {
        ...
        uint256 fees = transferFees(price, position).add(buyerFee.mul(2));
    ...
    function transferFees(uint256 _price, Position memory position) internal returns(uint256) {
        HasSecondarySaleFees collection = HasSecondarySaleFees(position.nft.collection);
        uint256 result = 0;
        if (
            position.nft.tokenType == TokenType.ERC1155 && IERC1155(position.nft.collection).supportsInterface(_INTERFACE_ID_FEES) ||
            (position.nft.tokenType == TokenType.ERC721 || position.nft.tokenType == TokenType.ERC721Deprecated) && IERC721(position.nft.collection).supportsInterface(_INTERFACE_ID_FEES)
        ) {
            uint[] memory fees = collection.getFeeBps(position.nft.id);
            address payable[] memory recipients = collection.getFeeRecipients(position.nft.id);
