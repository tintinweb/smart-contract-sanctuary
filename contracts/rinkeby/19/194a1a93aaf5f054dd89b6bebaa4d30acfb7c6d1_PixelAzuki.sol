...
contract PixelAzuki is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("PIXEL Azuki", "PIXELAZUKI", maxBatchSize_, collectionSize_) {
  ...
  function freeMint(uint256 quantity) external payable callerIsUser {
    ...
    require(quantity > 0 && quantity <= maxPerAddressDuringMint, "Can only mint max token at a time");  
    require(totalSupply() + quantity <= MAX_FREETOKEN, "reached max supply");
