...
/// @title NftOsMigrationCollection
...
contract NftOsMigrationCollection is IERC721, Ownable, ERC721Enumerable {
    uint256 private constant MAX_TOKEN_COUNT = 6;
    uint256 private constant OLD_TOKEN_NUMBER_MIN = 7;
    uint256 private constant OLD_TOKEN_CREATOR_ADDRESS_MASK = 0x00000000000000000000000061283d3154d78a4a9942aef0fd1e70ba7a93a4c5;
    ...
    constructor(IProxyRegistry _proxyRegistry, address _openseaStoreFront)
        ERC721("NftOsMigrationCollection", "COLLECTION")
    {
        proxyRegistry = _proxyRegistry;
        openseaSf = IOpenseaStorefront(_openseaStoreFront); // Rinkeby
    }
    ...
    function isValidId(uint256 _tokenId) internal pure returns (bool) {
        /** 
         * @dev Bitmasking for validating IDs
         * Check that token ID following Opensea format matches with our collection:
         * First 20 bytes are the creator wallet address.
         * Next 7 bytes are the NFT token number.
         * Last 5 bytes are fixed to 0x1.
         * There will only be MAX_TOKEN_COUNT tokens.
         */
        if (
            _tokenId >> 96 !=
            OLD_TOKEN_CREATOR_ADDRESS_MASK
        ) return false;
        if (
            _tokenId &
        ...
        uint256 id = (_tokenId &
            ...
            40;
        if (id > (MAX_TOKEN_COUNT + OLD_TOKEN_NUMBER_MIN)) return false;
        if (id <= OLD_TOKEN_NUMBER_MIN) return false;
        return true;
    }

    /// @notice Converts Opensea token id to a new normalized contract id
    function convertTokenId(uint256 _id) internal pure returns (uint256) {
        // Extract token Id from Opensea contract and shift it
        uint256 tokenId = (_id &
            0x0000000000000000000000000000000000000000ffffffffffffff0000000000) >>
            40;
        // This way, tokenIds start at 1 instead of 0
        tokenId = tokenId - OLD_TOKEN_NUMBER_MIN;
        return tokenId;
    }

    /// @notice Performs NFT migration from Opensea to this contract performing a mint and burn
    function migrateFromOpensea(uint256 osId) external {
        require(mintingEnabled, "Minting is not enabled yet");

        require(isValidId(osId), "Not valid Token Id");
        ...
        uint256 newId = convertTokenId(osId);
