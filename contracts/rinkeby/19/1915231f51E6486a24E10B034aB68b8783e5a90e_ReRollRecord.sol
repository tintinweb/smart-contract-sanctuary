...
contract ReRollRecord {
   ...
   address payable public multisig;
   uint256 ROYALTY_PRICE = 3330000000000000; // 3% of each word changed in reroll
   address payable DRIBNET = payable(0x370433a205B84839B507420B8E22900BAb902a8b);

   ThreeWords ThreewordsContract;

   mapping(uint256 => bool) private _isTokenLocked;
   mapping(address => uint256) private whoLocked;
   mapping(uint256 => bytes32) private _tokenIdToPhraseId;
   mapping(uint256 => string[]) private _tokenIdToWords;
   mapping(uint256 => string) public lastWord1;
   mapping(uint256 => string) public lastWord2;
   mapping(uint256 => string) public lastWord3;


   //constructor
   constructor(address payable _multisig, address ThreewordsContractAddy) {
       multisig = _multisig;
       ThreewordsContract  = ThreeWords(ThreewordsContractAddy);
   }
   ...
   function ReRoll(uint256 tokenId, string memory _word1, string memory _word2, string memory _word3) public payable {
       address caller = msg.sender;
       require(!_isTokenLocked[tokenId], "Token Cannot be Locked");
       require(ThreewordsContract.ownerOf(tokenId) == caller, "Caller is not the owner");
       if (_tokenIdToPhraseId[tokenId] == 0){
         string[] memory words = ThreewordsContract.tokenIdToWords(tokenId);
         lastWord1[tokenId] = words[0];
         lastWord2[tokenId] = words[1];
         lastWord3[tokenId] = words[2];
         _tokenIdToPhraseId[tokenId] = ThreewordsContract.tokenIdToPhraseId(tokenId);
       }

       uint256 w1 = areStringsDifferent(_word1, lastWord1[tokenId]);
       uint256 w2 = areStringsDifferent(_word2, lastWord2[tokenId]);
       uint256 w3 = areStringsDifferent(_word3, lastWord3[tokenId]);

       uint256 totalPrice = 111000000000000000*(w1+w2+w3);
       ...
       if (phraseId == _tokenIdToPhraseId[tokenId]) {
           ...
           if (w1 == 1){
              lastWord1[tokenId] = _word1;
           }
           if (w2 == 1){
              lastWord2[tokenId] = _word2;
           }
           if (w3 == 1){
              lastWord3[tokenId] = _word3;
           }
           _tokenIdToPhraseId[tokenId] = phraseId;
...
function areStringsDifferent(string memory a, string memory b) view returns(uint256) {
