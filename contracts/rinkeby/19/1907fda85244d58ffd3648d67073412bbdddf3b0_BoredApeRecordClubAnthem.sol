...
// most of the code is standard library code needed to create the NFT
// the main code to create the NFT is at the end 
...
// main code to create NFT
contract BoredApeRecordClubAnthem is ERC721Enum, Ownable, ReentrancyGuard {
	...
	constructor(
	) ERC721P("Bored Ape Record Club Anthem", "BARC")
	{
    	setBaseURI("https://ipfs.io/ipfs/QmaJw7hVqTGsqcP26gGazcdtDhjGKWmABan3dTqmayML3J/");
	}
	
    // show baseurl of metadata
	...
	function mint(uint256 _mintAmount) public payable nonReentrant{
        ...
        require(status, "Minting not started yet" );
        require(_mintAmount > 0, "Cant mint 0" );
        require(_mintAmount <= maxMint, "Must mint less than the max" );
        require(s + _mintAmount <= maxSupply, "Cant mint more than max supply" );
        require(s + _mintAmount <= wave, "Cant mint more than current wave" );
        require(msg.value >= cost * _mintAmount, "Must send eth of cost per nft");
    ...
    // presale minting to whitelist addresses
	function mintPresale(uint256 _mintAmount) public payable {
        require(!presale, "Presale not open yet");
        uint256 s = totalSupply();
        uint256 reserve = presaleWhitelist[msg.sender];
        require(reserve > 0, "None left for you");
        require(_mintAmount <= reserve, "Cant mint more than your allocation");
        require(s + _mintAmount <= maxSupply, "Cant mint more than max supply");
        require(s + _mintAmount <= wave, "Cant mint more than current wave" );
        require(msg.value >= cost * _mintAmount, "Must send eth of cost per nft");
	...
	function gift(uint[] calldata quantity, address[] calldata recipient) external onlyOwner{
        ...
        require( s + totalQuantity <= maxSupply, "Cant mint more than total supply" );
	...
	// record addresses of whitelist people
    ...
    function withdraw() public payable onlyOwner {
        ...
        require(success,"withdraw failed for unknown reason");
    }
}