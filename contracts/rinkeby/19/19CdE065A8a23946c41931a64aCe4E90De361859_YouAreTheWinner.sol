...
contract YouAreTheWinner is Ownable {
    ...
    returns (bool)
  {
    ...
    bool[] memory checkUniq = new bool[](maxLottoNumber);
  ...
  function bet(uint256[] memory _Numbers) external {
    ...
    ERC20(stakeToken).transferFrom(msg.sender, address(this), ticketPrice);
    ...
    if ((currentTicket >= maxLottoNumber / ticketPrice)) {
      prizeOpen();
    }
    updateSalt();
  }
  ...
  function prizeOpen() private {
    require((currentTicket >= maxLottoNumber / ticketPrice), "Not yet");
    ...
    if (ERC20(stakeToken).balanceOf(address(this)) >= totalBonus) {
      PayoutReward = (totalBonus * (100 - feePercent)) / 100;
    } else {
      PayoutReward =
        ((ERC20(stakeToken).balanceOf(address(this))) * (100 - feePercent)) /
        100;
    }
    ...
    PayoutReward = (ERC20(stakeToken).balanceOf(address(this)));
  ...
  function setFeePercent(address _address)
    public
    view
    returns (uint256[][] memory)
  {
