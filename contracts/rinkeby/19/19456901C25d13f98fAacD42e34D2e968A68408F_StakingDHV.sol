...
contract StakingPools is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event ClaimRewards(address indexed user, address indexed asset, address[] tokens, uint256[] amounts);
    ...
    function addPool(
        ...
        for (uint256 i = 0; i < _rewardsTokens.length; i++) {
            poolInfo[_pid].accuracy[i] = 10**ERC20(_rewardsTokens[i]).decimals();
        }
    }
    ...
    function setOnPause(uint256 _pid, bool _paused) external hasPool(_pid) onlyOwner {
    ...
    function deposit(uint256 _pid, uint256 _amount) public virtual nonReentrant hasPool(_pid) poolRunning(_pid) {
        ...
        IERC20(pool.assetToken).safeTransferFrom(address(_msgSender()), address(this), _amount);

        emit Deposit(_msgSender(), pool.assetToken, _amount);
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public virtual nonReentrant poolRunning(_pid) hasPool(_pid) {
        ...
        poolInfo[_pid].poolSupply -= _amount;
        IERC20(pool.assetToken).safeTransfer(address(_msgSender()), _amount);
        emit Withdraw(_msgSender(), pool.assetToken, _amount);
    }

    /// @notice Update pool and claim pending rewards for the user
    /// @param _pid Pool's id
    function claimRewards(uint256 _pid) public nonReentrant poolRunning(_pid) {
        updatePool(_pid);
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_msgSender()];
        ...
        emit ClaimRewards(_msgSender(), pool.assetToken, pool.rewardsTokens, amounts);
    }
...
contract StakingDHV is StakingPools, AccessControlUpgradeable {
    using SafeERC20 for IERC20;
    ...
    uint256 public constant dhvTokenDecimals = 1e12;
    ...
    /// @notice Checks the admin role.
    modifier onlyAdmin() {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Unauthorized");
        _;
    }

    /// @notice Checks the cluster lock role.
    modifier onlyClusterLock() {
        require(hasRole(CLUSTER_LOCK_ROLE, _msgSender()), "Unauthorized");
        _;
    }
    ...
    function initialize() public override initializer {
        ...
        _setupRole(CLUSTER_LOCK_ROLE, _msgSender());
    ...
    function setDHVToken(address _dhvTokenAddress) external onlyAdmin {
    ...
    function setClusterRate(address _cluster, uint256 _rate) public onlyAdmin {
    ...
    function coverCluster(
        ...
        uint256 _amount,
        uint256 _pid
    ) external hasDHV(_pid) hasCluster(_cluster) onlyClusterLock {
        uint256 amountInDhv = (_amount * dhvTokenDecimals) / clusterRate[_cluster];
        ...
        lockedDhvForClusterByUser[_cluster][_user] = lockedDhvForClusterByUser[_cluster][_user] + amountInDhv;
        totalLockedDhvByUser[_user] = totalLockedDhvByUser[_user] + amountInDhv;
    }
    ...
    function releaseCluster(
        ...
        uint256 amountInDhv = (_amount * dhvTokenDecimals) / clusterRate[_cluster];
        uint256 releaseInTotal = amountInDhv;
        if (amountInDhv > lockedDhvForClusterByUser[_cluster][_user]) {
            ...
            releaseInTotal = amountInDhv;
        }
        lockedDhvForClusterByUser[_cluster][_user] = lockedDhvForClusterByUser[_cluster][_user] - amountInDhv;
        totalLockedDhvByUser[_user] = totalLockedDhvByUser[_user] - releaseInTotal;
    }
    ...
    function releaseClusterTotal(
        ...
        totalLockedDhvByUser[_user] = totalLockedDhvByUser[_user] - lockedDhvForClusterByUser[_cluster][_user];
