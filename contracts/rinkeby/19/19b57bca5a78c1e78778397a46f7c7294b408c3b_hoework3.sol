...
contract hoework3 is usingOraclize {
    ...
    mapping(bytes32 => address) public queryIdToPlayer;
    mapping(bytes32 => uint) public queryIdToBetOption;
    ...
    function getRandomNumber() private returns(bytes32 queryId) {
        queryId = oraclize_newRandomDSQuery(0, 5, 200000);
        return queryId;
    }
    ...
    function getLastGame(address player_) view public returns(uint id, uint8 result, uint8 bet, uint8 banker) {
        ...
        for(uint i=0; i<_gameResults.length; i++) {
            ...
            gameResult.result = _gameResults[_gameResults.length-i-1].result;
            gameResult.betOption = _gameResults[_gameResults.length-i-1].betOption;
            gameResult.bankerOption = _gameResults[_gameResults.length-i-1].bankerOption;
        }

        if(findRecord == true)
            return (id,gameResult.result,gameResult.betOption,gameResult.bankerOption);
        else
            return (0,0,0,0);
    }
}