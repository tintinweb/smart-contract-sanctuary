...
contract DNA {
        ...
        returns (uint256)
    {
        ...
        bytes memory encodedParams = abi.encodePacked(combinedParams); // Parse integer to bytes
        bytes32 hashedParams = keccak256(encodedParams); // Create a hash
...
// File contracts/PoemsContract.sol

pragma solidity ^0.8.0;

contract PoemContract is ERC721, ERC721Enumerable, DNA {
    ...
    constructor(uint256 _maxSupply) ERC721("Poem", "PLPMS") {
        maxSupply = _maxSupply;
    }

    function mint() public {
        uint256 current = _idCounter.current();
        require(current < maxSupply, "There are not more available");
        ...
        returns (string memory)
    {
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "Poem #',
                tokenId.toString(),
                '", "description": "this is the description", "image": "',
                image,
                '"}'
            )
        );
