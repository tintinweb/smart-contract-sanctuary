...
contract AniCatsBeta is ERC721, Ownable {
    ...
    uint constant public MAX_SUPPLY = 15; // 9000 unique AniCats will be in total.
    uint constant public PRICE = 0.005 ether; // 0.07 ETH + gas per transaction

    string private _apiURI;
    uint private _presaleTokensLimit = 5;
    uint private _reservedTokensLimit = 2; // 200 AniCats will be reserved for marketing needs.


    uint private _tokensPerMintLimit = 1; // Up to 20 AniCats per transaction.
    uint private _presaleTokensPerMintLimit = 1; // Up to 10 AniCats per transaction during the presale.
    uint private _presaleTokensPerWalletLimit = 3; // Up to 10 AniCats per unique wallet during the presale.
