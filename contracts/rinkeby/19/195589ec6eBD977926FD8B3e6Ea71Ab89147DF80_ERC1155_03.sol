...
contract ERC1155_03 is ERC1155Supply, Ownable, RoyaltiesV2Impl {
    string public name;
    ...
    mapping(uint256 => string) public jsonHash; 
    mapping(address => bool) public proxyRegistryAddress;
    mapping(uint256 => address) public creators; 

    constructor(string memory _name, string memory _symbol)
        ERC1155("https://nft.mytk.testcom/item/{id}.json")
    {
        name = _name;
        symbol = _symbol;
        baseUri = "https://nft.mytk.testcom/item/";
    }
 
    function nextTokenId() public view returns (uint256 _id) {
        return tokenIdTracker.current();
    }
     function proxyRegistry(address _proxy, bool _status) public onlyOwner {
        proxyRegistryAddress[_proxy] = _status;
    }
    ...
    function mint(
        ...
        uint256 _amount,
        bytes memory _data,
        string calldata _jsonHash
    ) public onlyOwner returns (uint256 _id) {
        uint256 id = tokenIdTracker.current();
        super._mint(_to, id, _amount, _data);
        jsonHash[id] = _jsonHash;
        ...
        onlyOwner
    {
        ...
        revert("can't transferOwnership");
    }
    ...
    { 
        if (proxyRegistryAddress[_operator] == true) {
            return true;
        }
