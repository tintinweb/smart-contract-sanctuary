...
contract Escrow is Migratable {
    ...
    returns (uint256) {
        ...
        uint256 computeAmount = _amount.div(1e18).mul(10);
        uint256 computeAmount2 = _amount.div(1e18).sub(computeAmount);
        uint256 daiValueInEscrow = getPaprPrice().mul(computeAmount2);
    ...
    returns (uint256) {
        ...
        uint256 totalPayoutCAN = escrows[_escrowId].amount;
        //uint256 totalPayoutCAN = busdValue.mul(escrowAmount);
