...
contract Moonpet is Initializable, ERC721HolderUpgradeable, OwnableUpgradeable {
    ...
    //     mooncats = 0x5a3b5aD2397A92a6Ff417489A24B16edEc7bC418; //change this to real mooncat TODO
    ...
    function claimMiningRewards(uint256 nftId) external isAllowed(nftId) {
        // TODO think about this more, but this might ok answer as long as people understand this can be gamed by mev.
        lastGas = tx.gasprice;
        ...
        isAllowed(_attacker)
    {
        ...
        if (chanceOfAttack <= 80) {
            ...
            Pet memory thisPet = petDetails[_victim];

            address victimOwner = IERC721Upgradeable(thisPet.nft).ownerOf(
                thisPet.id
            );
            // the defender win 19%
    ...
    function fatality(uint256 _deadId, uint256 _tokenId) external {
        ...
        require(
            !isPetSafe(_deadId) &&
                tx.gasprice <= gas &&
                petDead[_deadId] == false, //inspired by NFT GAS by 0Xmons
    ...
    function giveLife(address nft, uint256 _id) external {
        require(IERC721Upgradeable(nft).ownerOf(_id) == msg.sender, "!OWNER"); //TODO check this
        require(
        ...
        isNftInTheGame[nft][_id] = true; //TODO Check
    ...
    function isPetSafe(uint256 _nftId) public view returns (bool) {
        ...
        if (
            (_timeUntilStarving != 0 &&
                _timeUntilStarving >= block.timestamp) || lastGas >= gas //todo check this, here if the last gas is set as the high gas ratio we set, everyone is alive.
        ) {
            return true;
        } else {
            return false;
