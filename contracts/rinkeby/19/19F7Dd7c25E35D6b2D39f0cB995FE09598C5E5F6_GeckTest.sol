...
contract GeckTest is ERC721Enumerable { 
    ...
    mapping(uint256 => Legendary[]) public legNames;
    mapping(string => bool) hashToMinted;
    mapping(uint256 => mapping(uint256 => string)) internal tokenIdToHash;
    mapping(uint256 => bool) public _tokenIdToLegendary;
    mapping(uint256 => string) public _tokenIdToLegendaryName; //want to make this store the actual name of the legendary
    ...
    //uint256s
    uint256 public constant MAX_PRIVATE_SUPPLY = 40;
    uint256 public constant MAX_PUBLIC_SUPPLY = 60;
    ...
    constructor(
        ...
        LEGENDARY_TIERS[0] = [10, 10, 10, 40, 40, 40, 80, 80, 80];
    ...
    function mintInternal() internal {
        ...
        if (legendariesMinted < 9) {
            ...
            if (legendaryDecider == 0) {
                ...
                _tokenIdToLegendaryName[thisTokenId] = traitTypes[legendariesMinted][0].traitName;

                tokenIdToHash[thisTokenId][tokenIndex[thisTokenId]] = Library.toString(legendariesMinted);
        ...
        tokenIdToHash[thisTokenId][tokenIndex[thisTokenId]] = hash(thisTokenId, msg.sender, 0);

        hashToMinted[tokenIdToHash[thisTokenId][tokenIndex[thisTokenId]]] = true;
    ...
    function mintWhiteList(uint256 amount, bytes memory signature) public payable {
    	require(mintWhiteListActive);
    ...
    function burnAndRecycle(uint256 tokenId) external {
        ...
        tokenIdToHash[tokenId][tokenIndex[tokenId]] = hash(tokenId, msg.sender, 0);
    }
    ...
    function legToMetadata(uint256 _tokenId) public view returns (string memory){
        string memory metadataString;
        metadataString = string(
            abi.encodePacked(
                '{"trait_type":"LEGENDARY"',
                '"value":"',
                legNames[0][_tokenIdToLegendaryNumber[_tokenId]].legName,
                '"}'
            )
        );
    ...
    function legToImage(uint256 _tokenId) public view returns (string memory){
        string memory metadataString;
        metadataString = string(
            abi.encodePacked(
                artURI(),
    ...
    function addLegNames(Legendary[] memory names)
        public
        onlyOwner
    {
    ...
    function withdraw() public payable onlyOwner {
        uint sale1 = address(this).balance * 25/1000;
        uint sale2 = address(this).balance * 25/1000;
        uint sale3 = address(this).balance * 25/1000;
        uint sale4 = address(this).balance * 925/1000;
