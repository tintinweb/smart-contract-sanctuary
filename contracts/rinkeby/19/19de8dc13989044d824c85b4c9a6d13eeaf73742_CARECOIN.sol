...
contract CARECOIN is ERC20  {
  ...
  mapping(address => bool) private _isExcludedFromMaxTx;
   ...
   modifier lockTheSwap {
    ...
    uint256 private constant billion = 1 ;//10**9;
  ...
  //Current World Population, CPN will always be equal to worldPopulation X 1 Billion.
    ...
    event EnsureMaxTxAmount(uint256 actualAmount,uint256 txAmount);
...
uint256 _currentSupply = currentSupply();
        ...
        _value = ensureMaxTxAmount(_from,_to,_value,_currentSupply);
       ...
       uint256 amtForAntiWhale =  ExecuteAntiWhaleFunctions(_from,_to,_value,_currentSupply);
...
/**
     ...
     */
function calculateAntiWhaleFee(uint256 amount,uint256 _currentSupply) public pure returns (uint256)
{
        //uint256 currentSupply = currentSupply();
        uint256 fee = 0;
        uint256 txSize = amount.mul(10 ** 4).div(_currentSupply);
        if (txSize <= 1) {
            fee = 0;
        } else if (txSize <= 25) {
            fee = 2;
...
function ExecuteAntiWhaleFunctions(address _from,address _to, uint256 _value, uint256 _currentSupply)
        internal
        returns (uint256)
{
    if(!antiWhaleEnabled)
...
//You should not be UniswapPair.
       if( _from != uniswapV2Pair && _to != uniswapV2Pair && _from != address(this))
 {
      ...
      uint256 antiWhaleFee = calculateAntiWhaleFee(_value,_currentSupply);
      if(antiWhaleFee > 0)
     {
    ...
    /**
     * @notice A simple method that Calculates the Max tx Allowed based on the maxTxPercent.
     * Collects the amount to allowed to be Transfer.
     
     */
function calculateMaxTxAmount(uint256 _currentSupply) internal view returns(uint256)
{
    ...
    /**
     * @notice A simple method that implement the maxTxPercent, and make sure Transaction can only be allowed as per maxTxPercent.
     
     * @param from The stakeholder to calculate Functions Amount from.
     * * @param to The stakeholder to which Transaction is going.
     * * @param amount Tokens to be used to calculate all functions.
     */
 function ensureMaxTxAmount(
        address from,
        address to,
        uint256 amount,uint256 _currentSupply) internal  returns (uint256) 
        {
        if (
        ...
        ) 
        {
               ...
               uint256 maxAmountAllowed = calculateMaxTxAmount(_currentSupply);
               
               //If Amount desired to send is greater than Max Amount Allowed
                if(amount > maxAmountAllowed)
                {
                    //Make it equal to only Amount Tx Allowed.
                emit EnsureMaxTxAmount(amount,maxAmountAllowed);
