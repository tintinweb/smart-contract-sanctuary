...
contract GameFiShibaCoinPool is Ownable, Pausable {
    ...
    event LBChange(uint256);

    event NPBChange(uint256);

    mapping(uint256 => uint256) blockNumber;

    uint256 private _longestBlockNumber = 28800;
    uint256 private _numPerBlockNumber = 0x13bcbf936b38e;
    ...
    function withdraw(uint256 tokenId) public whenNotPaused returns (uint256) {
        ...
        _gameFiToken.transferFrom(address(this), msg.sender, _nunmOfBlock);
        return _nunmOfBlock;
    }

    function estimatedIncome(uint256 tokenId) public whenNotPaused view returns (uint256) {
        require(
    ...
    function withdraw() public whenPaused onlyOwner{
        _gameFiToken.transferFrom(address(this), msg.sender, _gameFiToken.balanceOf(address(this)));
    }
}
