...
error CCAOnlyAdminAllowedToCall();
...
contract CyberCatsAlliance is Context, AccessControlEnumerable, ERC721Enumerable, ERC721Pausable, ReentrancyGuard {
    enum Workflow {
        Before,
        Presale,
        Sale,
    ...
    modifier onlyAdmin() {
        if (!hasRole(DEFAULT_ADMIN_ROLE, _msgSender())) {
            revert CCAOnlyAdminAllowedToCall();
        }
        _;
    }
    ...
    constructor(string memory _placeHolderURI) ERC721("Cyber Cats Alliance", "CCA") {
        ...
        _setupRole(WL_ROLE, _msgSender());
        _setupRole(OG_ROLE, _msgSender());
    ...
    function teamAllocation() external onlyAdmin {
    ...
    function setPlaceHolderUri(string memory _uri) external onlyAdmin {
        placeHolderUri = _uri;
    }

    function setMaxTotalSupply(uint256 _maxTotalSupply) external onlyAdmin {
        maxTotalSupply = _maxTotalSupply;
    }

    function setMaxMintAmount(uint256 _maxMintAmount) external onlyAdmin {
        maxMintAmount = _maxMintAmount;
    }

    function setAllocationNFTforTeam(uint256 _allocationNFTforTeam) external onlyAdmin {
        allocationNFTforTeam = _allocationNFTforTeam;
    }

    function setPresalePriceWL(uint256 _presalePriceWL) external onlyAdmin {
        presalePriceWL = _presalePriceWL;
    }

    function setPresalePriceOG(uint256 _presalePriceOG) external onlyAdmin {
        presalePriceOG = _presalePriceOG;
    }

    function setMaxGasPriceForRefund(uint256 _maxGasPriceForRefund) external onlyAdmin {
        maxGasPriceForRefund = _maxGasPriceForRefund;
    }

    function nextWorkflowStep() external onlyAdmin {
    ...
    function mint(uint256 amount) external payable nonReentrant whenNotPaused {
        ...
        if (workflowStatus < Workflow.Presale) {
            ...
            if (hasRole(OG_ROLE, _msgSender())) {
                _priceToken = presalePriceOG;
            } else if (hasRole(WL_ROLE, _msgSender())) {
        ...
        if (
            workflowStatus == Workflow.Presale &&
            hasRole(OG_ROLE, _msgSender()) &&
            (tx.gasprice <= maxGasPriceForRefund)
        ) {
    ...
    function reveal(string memory _uri) public onlyAdmin {
        if (workflowStatus != Workflow.Reveal) {
