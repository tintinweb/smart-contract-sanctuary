...
contract ERC721Matcha is ERC721Full, ReentrancyGuard {
    ...
    */
    ...
    function canBuy(uint256 tokenId) public view returns (bool) {
        if (!auctions[tokenId].open && sellBidPrice[tokenId]>0 && sellBidPrice[tokenId]>0) {
            return true;
        } else {
            return false;
        }
    ...
    function canWithdraw(uint256 tokenId) public view returns (bool) {
        if (auctions[tokenId].highestBidder == msg.sender &&
            now >= auctions[tokenId].auctionEnd &&
            auctions[tokenId].open &&
            (auctions[tokenId].highestBid<auctions[tokenId].reserve || getApproved(tokenId) != address(this))
        ) {
            return true;
        } else {
            return false;
    ...
    function withdraw(uint256 tokenId) public nonReentrant returns (bool) {
        ...
        require(auctions[tokenId].highestBid<auctions[tokenId].reserve ||  // The highest bid is equal or more than the reserve price
               getApproved(tokenId) != address(this),                      // this contract can still do the transfer
               "The highest bid is equal or more than the reserve price, cannot withdraw. Call auctionFinalize.");
