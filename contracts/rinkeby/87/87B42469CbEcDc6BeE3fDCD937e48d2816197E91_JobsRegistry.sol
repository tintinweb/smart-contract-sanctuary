 ...
 */
contract JobsRegistry is Ownable {
    ...
    function addJob(address _job) external {
        if (_msgSender() != owner() || !jobsCreator[_msgSender()])
            revert Forbidden();
    ...
    function addFreeCredit(address _job, uint256 _amount) external {
        ...
        _jobFromStorage.credit += _amount;
        INativeToken(nativeToken).mint(address(this), _amount);
    }

    function addNativeTokenCredit(address _job, uint256 _amount) external {
        _addNativeTokenCredit(_job, _amount);
    }

    function addNativeTokenCreditWithPermit(
        address _job,
        uint256 _amount,
        ...
        INativeToken(nativeToken).permit(
            _msgSender(),
            address(this),
            _permittedAmount,
            block.timestamp,
    ...
    function _addNativeTokenCredit(address _job, uint256 _amount) internal {
        Job storage _jobFromStorage = job[_job];
        if (!_jobFromStorage.exists) revert NonExistentJob();
        uint256 _fee = (_amount * fee) / BASE;
        _jobFromStorage.credit += _amount - _fee;
        IERC20(nativeToken).safeTransferFrom(
            _msgSender(),
            address(this),
            _amount
        );
        IERC20(nativeToken).safeTransfer(owner(), _fee);
    }

    // FIXME: reserves might be manipulated just before the call
    // to add a fraudolent amount of credit
    function addLiquidityCredit(
        ...
        INativeToken(nativeToken).mint(address(this), _creditFromLiquidity);
        _jobFromStorage.credit += _creditFromLiquidity;
        IERC20(_liquidity).safeTransferFrom(
            _msgSender(),
            address(this),
            _amount
        );
        IERC20(_liquidity).safeTransfer(owner(), _fee);
    }
    ...
    function registerWork(
        ...
        if (_msgSender() != master) revert Forbidden();
        Job storage _jobFromStorage = job[_job];
        if (!_jobFromStorage.exists) revert NonExistentJob();
        if (_jobFromStorage.credit < _usedCredit) revert NotEnoughCredit();
        _jobFromStorage.credit -= _usedCredit;
        // inifinite approval to optimize gas costs
        if (IERC20(nativeToken).allowance(address(this), bonder) < _usedCredit)
            IERC20(nativeToken).approve(
                bonder,
                0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff
            );
        IBonder(bonder).accrueReward(_worker, _usedCredit);
    }
