...
abstract contract Adjustable  is Pausable, Manageable {
    bool public  allowContingencyPause = true;

  // Called by the Manager on emergency, triggers stopped state
    function contingencyPause() external onlyManager {
      require(  allowContingencyPause, "This contract dont permit contingency pauses." );
      _pause();
  }
...
//   ======================================
      ...
      function _tokenLiquify( address sender, uint256 contractTokenBalance ) internal {    
       ...
       // The condition "sender != swapPair" stops "swapAndLiquify" for all "buy" transactions.

        if ( overMinTokenBalance && autoLiquidity &&  sender != swapPair )   { swapAndLiquify ( _numTokensToLiquidity );   }	//*** Add Liquidity  ***
...
//   ======================================
    ...
    function swapAndLiquify(uint256 numTokensToLiquidity) private nonReentrant() {
       ...
       // has been manually sent to the contract. 
         ...
         * So we provide a method to withdraw these funds, otherwise those coins would be locked in 
...
contract SymplexiaToken is  Taxable {
  ...
  //
    ...
    constructor () ERC20 ("DEBUG30", "DBG30") BaseToken (5, 5, 5, 
                                     0x9dc83Ab4f953B78ddcEEd5f7c1c879cd890B43Ac, 
                                     0xB2019936b9ea80e3fAC12007597495794C9b749d, 99) {}
