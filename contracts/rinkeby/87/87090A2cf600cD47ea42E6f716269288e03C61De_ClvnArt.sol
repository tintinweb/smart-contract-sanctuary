...
contract ClvnArt is ClvnArtERC721 {
	...
	function addressRgba(address account)
		public
		pure
		returns (
			string memory,
	...
	{
		address account = originalOwner[tokenId];
			...
			string(
				abi.encodePacked(
					...
					Base64.encode(
						bytes(
							abi.encodePacked(
								'{"name":"The clvn Art Project - Token',
								' #',
								...
								Base64.encode(bytes(generateSVG(account))),
								'"}'
							)
						)
					)
	...
	function generateSVG(address account) private pure returns (string memory) {
		return
			string(
				abi.encodePacked(
					...
					'</style>',
					generateRects(account),
					'</svg>'
				)
			);
	}

	function generateRects(address account)
		private
		pure
		returns (string memory result)
	{
		result = string(abi.encodePacked(result, generateRect(account, 0)));
	}

	function generateRect(address account, uint256 index)
		private
		pure
		returns (string memory)
	{
