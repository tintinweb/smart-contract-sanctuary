...
contract DogeVille is Context, IERC20, Ownable {
    ...
    constructor (string memory _NAME, uint256 _baseRate, string memory _SYMBOL, uint256 _DECIMALS, uint256 _supply, uint256 _txFee,uint256 _lpFee,uint256 _DexFee,address routerAddress,address tokenOwner, address _rektRacoonsAddress) public {
        _name = _NAME;
        _symbol = _SYMBOL;
        _decimals = _DECIMALS;
    ...
    function updateTime(uint256 _tokenId) external {
    ...
    function setNumTokensSellToAddToLiquidity(uint256 amountToUpdate) external onlyOwner {
        ...
        uint256 amountTransfer = numberOftoken*10**9; //for testing
        address payable _owner = payable(address(uint160(owner())));
