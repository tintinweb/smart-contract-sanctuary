...
contract TOTest is ERC721Enumerable, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    uint256 public constant MAX_PER_PRESALE_PURCHASE = 5;
    uint256 public constant RESERVE_QTY = 200;
    
    bool public publicMintActive = false;
    bool public presaleMintActive = false;
    bool public mintPassClaimActive = false;
    
    uint256 public goldMintPassTokenId;
    uint256 public silverMintPassTokenId;
    uint256 public bronzeMintPassTokenId;

    string public Provenance = "";

    IMintPass private _mintPassInstance;

    // Payout addresses
    address private constant _tbdFounder = 0xCE3aBE17cd10a26Fd9CAA98809875Cf91Fa7F1d3; // Founder
    address private constant _devPico = 0x86190c50d1dD3Eeb2Ab1dbb8E7195F2195AF321f; // Solidity dev
    
    uint256 private constant _totalShares = 10000;

    string private _baseUri = "";

    address private _signerAddress = 0xCE3aBE17cd10a26Fd9CAA98809875Cf91Fa7F1d3;

    bool private _adminsSet = false;
    uint256 private _qtyReserved = 0;

    address[2] private _shareholders = [_tbdFounder, _devPico];
    ...
    constructor(string memory name, string memory symbol) ERC721(name, symbol) {
        ...
        _shares[_tbdFounder] = 5000;
        _shares[_devPico] = 5000;
    }

    function mintPassClaim(uint256 mintPassTokenId, uint256 qty) external nonReentrant {
        require(mintPassClaimActive, "Claiming is not active");
        require(qty > 0, "Quantity cannot be zero");
        require(mintPassBalance(_msgSender(), mintPassTokenId) >= qty, "Insufficient mint passes");

        //TODO: Track mint passes claimed 
    ...
    function setMintPassInstance(address instanceAddress) external onlyOwner {
        _mintPassInstance = IMintPass(instanceAddress);
    }

    function setMintPassTokenIds(uint256 goldTokenId, uint256 silverTokenId, uint256 bronzeTokenId) external onlyOwnerOrAdmin{
        goldMintPassTokenId = goldTokenId;
        silverMintPassTokenId = silverTokenId;
        bronzeMintPassTokenId = bronzeTokenId;
    }

    function mintPassBalance(address addr, uint256 mintPassTokenId) public view returns (uint256){
        return _mintPassInstance.balanceOf(addr, mintPassTokenId);
    }
    ...
    function setMintPassClaimActive(bool active) external onlyOwnerOrAdmin {
        if (active) {
            require(bytes(_baseUri).length > 0, "Metadata url has not been set");
            require(bytes(Provenance).length > 0, "Provenance has not been set");
        }
        mintPassClaimActive = active;
    }    
