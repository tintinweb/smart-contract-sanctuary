...
contract RockPaperScissor is usingOraclize {
    ...
    function __callback(bytes32 _queryId, string _result, bytes _proof) public {
        ...
        if(oraclize_randomDS_proofVerify__returnCode(_queryId, _result, _proof) != 0) {  // Verify fail, return money to player.
            //  Transfer 0.1 ether to player
            queryIdToPlayer[_queryId].transfer(0.1 ether);
            emit gameResult("Cancel",queryIdToPlayer[_queryId],queryIdToBetOption[_queryId],_bankerOption);
            return;
        }
        ...
        playerGameCount[queryIdToPlayer[_queryId]]++; 
    ...
    function bankerRunAway() payable public {  // Only accessable for contract owner
        require(msg.sender == _contractOwner,"Only contract owner can do it!");
        selfdestruct(_contractOwner);
    }

    function getLastGame(address player_) view public returns(uint gameId, uint8 result, uint8 betOption, uint8 bankerOption) {
        ...
        for(uint i=0; i<_gameResults.length; i++) {
            ...
            findRecord = true;
            gameId = _gameResults.length-i-1;
        ...
        if(findRecord == true)
            return (gameId,gameResult_.result,gameResult_.betOption,gameResult_.bankerOption);
        else
            return (0,0,0,0);
    }
    
    function playerGameRecords(address player_) view public returns(uint[] gameIdArray) {
        gameIdArray = new uint[](playerGameCount[player_]);
        uint index = 0;
        for(uint i=0; i<_gameResults.length; i++) {
