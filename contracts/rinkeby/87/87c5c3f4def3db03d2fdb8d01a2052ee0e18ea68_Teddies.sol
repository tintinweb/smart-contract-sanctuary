...
// Lead Dev / Artist - Esteban Tinoco - Twitter:@Darth_TG
...
contract Teddies is ERC721, Ownable {
    ...
    uint public constant MAX_TEDDIES = 10000;
    ...
    uint public constant teddiesPrice = 10000000000000000; // 0.01 ETH
    uint public constant teddiesPriceSOS = 10000000000000000000000000; // 10 Million SOS

    IERC20 SOSTOKEN = IERC20(0x3b484b82567a09e2588A13D54D032153f0c0aEe0);

    constructor() ERC721("TEDDIESNFT","TEDDIES")  {
    ...
    function mintTeddies(uint numberOfTokens) public payable {
        require(hasSaleStarted == true, "Minting has not begun.");
        require(numberOfTokens > 0 && numberOfTokens <= MAX_PURCHASE, "Can only mint 10 Teddies at a time!");
        require(totalSupply().add(numberOfTokens) <= MAX_TEDDIES, "Exceeds total amount of Teddies available! Try again!");
        require(msg.value >= teddiesPrice.mul(numberOfTokens), "Ether value is not correct.");

        for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply();
            if (totalSupply() < MAX_TEDDIES) {
                _safeMint(msg.sender, mintIndex);
            }
        }
    }

    function mintTeddiesWithSOS(uint numberOfTokens) public payable {
        require(hasSaleStarted == true, "Minting has not begun.");
        require(numberOfTokens > 0 && numberOfTokens <= MAX_PURCHASE, "Can only mint 10 Teddies at a time!");
        require(totalSupply().add(numberOfTokens) <= MAX_TEDDIES, "Exceeds total amount of Teddies available! Try again!");
        require(msg.value >= teddiesPriceSOS.mul(numberOfTokens), "SOS value is not correct.");
        require(SOSTOKEN.allowance(msg.sender, address(this)) >= teddiesPriceSOS,"Insufficient SOS Balance!");
        require(SOSTOKEN.transferFrom(msg.sender,address(this),teddiesPriceSOS),"Transfer Failed!");
