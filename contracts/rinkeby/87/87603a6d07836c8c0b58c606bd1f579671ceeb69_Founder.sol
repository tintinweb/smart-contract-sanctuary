...
contract Founder is ERC721Enumerable, ERC2981, ReentrancyGuard, Ownable {
    ...
    mapping(address => uint256) private allowMintPerAddress;
    ...
    uint256 public grantIssued;
    
    uint256 public allIssued;
    ...
    modifier onlyAllowlisted() {
        require(
            mintPerAddress[_msgSender()] < allowMintPerAddress[_msgSender()], 
    ...
    function mintPublic(
    ) public payable onlyAllowlisted {
        require(
            publicIssued < MAX_MINT_PAID - allowMintPerAddress[_msgSender()], 
    ...
    function mintGrant(
        ...
        require(
            grantIssued < MAX_MINT_GRANT, 
            "There are no more NFTs for grant minting."
        );
        grantIssued += 1;
        _mint(_addr);
    }
    
    function mintGrantBatch(
        address[] memory _addrs
    ) external onlyOwner {
        require(
            grantIssued + _addrs.length < MAX_MINT_GRANT, 
            "There are not enough NFTs for grant minting."
        );
    ...
    function setPublicMax(
        ...
        require(
            _publicMax <= MAX_MINT_PAID, 
            "You can not set it that high."
        );
    ...
    function removeAddressFromAllowList(
        ...
        if (allowMintPerAddress[_addr] > mintPerAddress[_msgSender()]){
            allowMintPerAddress[_addr] -= 1;
            emit AllowListAddressAdded(_addr);
        }
    }
    ...
    constructor() ERC721("Founders of Zo World", "\\z/") Ownable() {}
}