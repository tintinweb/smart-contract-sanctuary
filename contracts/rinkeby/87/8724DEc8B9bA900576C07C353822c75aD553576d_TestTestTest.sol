...
contract TestTestTest is ERC721Enumerable, Ownable {
    ...
    uint16[8] LEGENDARY_TIERS = [10, 10, 10, 10, 10, 10, 10, 10];
    ...
    function flipBreedingSwitch() public {
        ...
        require(msg.sender == PUZZLE_ADDRESS);
    ...
    function initiateBreeding(
        ...
        require(BREEDING_LIVE == true);

        require(totalSupply() < MAX_BABY_SUPPLY);
    ...
    function findBreedingEventIndex(uint256 _breedingEventId)
        ...
        returns (uint256)
    {
        for (uint256 i = 0; i < getBreedingEventsLengthByAddress(msg.sender); i++ ) {
            if (_addressToBreedingEvents[msg.sender][i].breedingEventId ==_breedingEventId) return i;
        }
    ...
    function pullParentsByBreedingEventId(uint256 _breedingEventId) public {
        uint256 thisBreedingEventIndex = findBreedingEventIndex(_breedingEventId);

        pullParentsByBreedingEventIndex(thisBreedingEventIndex);
    }

    function pullParentsByBreedingEventIndex(uint256 _breedingEventIndex) public {
        //Pull this breeding event
        BreedingEvent memory thisBreedingEvent = _addressToBreedingEvents[
            msg.sender
        ][_breedingEventIndex];
    ...
    function generateChildHash(
        ...
        for (uint8 i = 0; i < 9; i++) {
            ...
            if (i == 8) {
                ...
                //If parent 1 character is glitched or irradited, give child a 5% chance
                ...
                //If parent 2 character is glitched or irradiadted, give child a 5% chance
