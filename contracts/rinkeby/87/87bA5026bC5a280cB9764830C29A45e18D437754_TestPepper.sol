...
contract TestPepper is ERC721Enumerable, Ownable {
    using Strings for uint256;

    uint256 public constant MAX_PEPPERS = 10000;
    uint256 public constant MAX_PURCHASES_PER_TRANSACTION = 20;
    uint256 public constant TOTAL_RESERVED_PEPPERS = 10; // Reserved for supporters and fundraising

    bool public isSaleActive = false;
    uint256 public price = 50000000000000000; // 0.05 ETH

    string internal _baseTokenURI;
    uint256 internal _goldenPepperId = 0;
    uint256 internal _revealTimestamp = 1627754400; // Sat Jul 31 2021 18:00:00 GMT+0000

    constructor(string memory baseURI) ERC721("Bobba", "PEPPER")  {
    ...
    /**
     *  Finding out the Golden Pepper, only callable after revealing date
     */
    function getGoldenPepper() public view returns (uint256){
        if(msg.sender != owner()){
            require (block.timestamp > _revealTimestamp, "Not revealed yet.");
        }
        return _goldenPepperId;
    }
    ...
    */ 
    ...
    /**
    *   Update timestamp for revealing date
    *   This function will be called when setting the initial tokenURI and when updating the revealing date
    */
    function summonGoldenPepper() public onlyOwner {

        require(_goldenPepperId == 0, "Golden Pepper is already minted.");

        uint256 randomHash = uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty)));
        uint256 luckyId = SafeMath.add(randomHash % MAX_PEPPERS, 1);

        // Make sure that the Golden Pepper is not a reserved pepper
        if (luckyId > TOTAL_RESERVED_PEPPERS && luckyId <= totalSupply()){
            _goldenPepperId = luckyId;
        }
    }
    ...
    function setRevealTimestamp(uint256 timestamp) public onlyOwner {
        _revealTimestamp = timestamp;
    }

    function getRevealTimestamp() public onlyOwner view returns (uint256){
