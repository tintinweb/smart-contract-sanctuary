...
contract KeykoNFT is ERC721, Ownable {
    ...
    function changeName(uint256 tokenId, string memory name) external {
        require(
            ...
            "only token owner can change it's property"
        );
        tokenData[tokenId].name = name;
    }

    function changeAge(uint256 tokenId, uint256 age) external {
        require(
            ownerOf(tokenId) == msg.sender,
            "only token owner can change it's property"
        );
    ...
    function changeHeight(uint256 tokenId, uint256 height) external {
        require(
            ownerOf(tokenId) == msg.sender,
            "only token owner can change it's property"
        );
    ...
    function changeHairColor(uint256 tokenId, string memory hairColor)
        external
    {
        require(
            ownerOf(tokenId) == msg.sender,
    ...
    function changeEyesColor(uint256 tokenId, string memory eyesColor)
        external
    {
        require(
            ownerOf(tokenId) == msg.sender,
