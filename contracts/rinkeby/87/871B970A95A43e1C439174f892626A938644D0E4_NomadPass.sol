 ...
 *
 */
contract NomadPass is ERC1155, AccessControl, Pausable, ERC1155Burnable, ERC1155Supply, EIP712 {
    ...
    mapping(bytes32 => address) private redeemedPasses;
    ...
    event MaxPerWalletChanged(
        uint256[] ids,
        uint256[] maxPerWallet
    );

    event MaxPerTransactionChanged(
        uint256[] ids,
        uint256[] maxPerTransaction
    );
    ...
    /// @param maxItems An array containing the maximum of passes, for example if we defined numPasses = 2, and we want 2500 and 5000 maxItems, this array should be [2500, 5000]
    ...
    onlyRole(OPERATOR_ROLE) {
        ...
        require(maxItems.length == numPasses, "INVALID_MAX_ITEMS_ARRAY_SIZE");

        require(maxPerWallet.length == numPasses, "INVALID_MAX_WALLET_ARRAY_SIZE");
        require(maxPerTransaction.length == numPasses, "INVALID_MAX_TRANSACTION_ARRAY_SIZE");

        require(prices.length == numPasses, "INVALID_PRICES_ARRAY_SIZE");

        require(open.length == numPasses, "INVALID_OPEN_ARRAY_SIZE");
        require(close.length == numPasses, "INVALID_CLOSE_ARRAY_SIZE");
        ...
        for (uint256 i = 0; i < numPasses; i++) {
            ...
            Pass storage pass = passes[passCounter.current()];
    ...
    returns (uint256)
    {
        return  passCounter.current();
    }

    ///@dev Resets the internal counter if something bad happens
    ...
    /// @dev After doing the setup, this method can be used to modify the sale dates. This dates are used in the mintPass method
    ...
    onlyRole(OPERATOR_ROLE)
    {
        require(ids.length > 0, "INVALID_TOKENS_ARRAY_SIZE");
        require(open.length == ids.length, "INVALID_OPEN_ARRAY_SIZE");
        require(close.length == ids.length, "INVALID_CLOSE_ARRAY_SIZE");

        for (uint256 i = 0; i < ids.length; i++) {
            require(open[i] <= close[i], "INVALID_DATES");
            Pass storage pass = passes[ids[i]];
    ...
    {
        require(ids.length > 0, "INVALID_TOKENS_ARRAY_SIZE");
        require(maxPerWallet.length == ids.length, "INVALID_MAX_WALLET_ARRAY_SIZE");
    ...
    {
        require(ids.length > 0, "INVALID_TOKENS_ARRAY_SIZE");
        require(maxPerTransaction.length == ids.length, "INVALID_MAX_TRANSACTION_ARRAY_SIZE");
    ...
    {
        require(ids.length > 0, "INVALID_TOKENS_ARRAY_SIZE");
        require(redeemContracts.length == ids.length, "INVALID_REDEEM_CONTRACTS_ARRAY_SIZE");
    ...
    /// @dev After doing the setup, this method can be used to modify the redeem price of each pass. The prices are used in the redeemPass method
    /// @param ids The ids of the passes
    ...
    {
        require(ids.length > 0, "INVALID_TOKENS_ARRAY_SIZE");
        require(prices.length == ids.length, "INVALID_PRICES_ARRAY_SIZE");
    ...
    onlyRole(OPERATOR_ROLE)
    {
        ...
        require(ids.length > 0, "INVALID_TOKENS_ARRAY_SIZE");
        require(maxItems.length == ids.length, "INVALID_MAX_ITEMS_ARRAY_SIZE");
    ...
    /// @dev This method can be used to modify the name of the smart contract. The name property is used by OpenSean
    ...
    /// @dev This method can be used to mint a pass using a signature. The main advantage of a signature over a Merkle Tree is that
    ...
    /// @param signature The signature for the operation, it is generated off-chain. See: https://github.com/OpenZeppelin/workshops/tree/master/06-nft-merkle-drop
    function mintPass(
        uint256 passId,
        uint256 amount,
        uint256 nonce,
    ...
    payable
    {
        ...
        require(redeemedPasses[hash] == address(0), "TOKEN_ALREADY_REDEEMED");
        ...
        if(pass.maxPerTransaction > 0){
            require(amount <= pass.maxPerTransaction, "INVALID_MAX_PER_TRANSACTION");
        }

        if(pass.maxPerWallet > 0){
            uint256 balance = balanceOf(msg.sender, passId);
            require((balance + amount) <= pass.maxPerWallet, "INVALID_MAX_PER_WALLET");
        }
        ...
        redeemedPasses[hash] = msg.sender;
    ...
    onlyRole(OPERATOR_ROLE)
    {
        ...
        require(pass.maxItems > 0, "TOKEN_ID_NOT_EXIST");
        require((totalSupply(tokenId) + amount) <= pass.maxItems, "PURCHASE_EXCEED_MAX_ITEMS");
    ...
    returns (bytes32)
    {
        return _hashTypedDataV4(keccak256(abi.encode(
                keccak256("NFT(address account,uint256 tokenId,uint256 amount,uint256 nonce)"),
                account,
                tokenId,
                amount,
                nonce
