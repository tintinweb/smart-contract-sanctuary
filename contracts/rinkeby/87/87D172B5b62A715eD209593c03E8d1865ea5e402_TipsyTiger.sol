...
contract TipsyTiger is ERC721Enumerable, Ownable {
    ...
    uint256 public presalePerAddress = 5;
    ...
    function mintTiger(uint256 num) public payable {
        ...
        require( supply + num <= 7970, "Sold out!" );
    ...
    function mintPresale(uint256 _mintNum) public payable {
        ...
        require(_mintNum <= presalePerAddress - presaleMintedAmount[msg.sender], 'Exceeded presale limit');
        uint256 supply = totalSupply();
    ...
    // only owner
    // set mint prices
    function setPrice(uint256 _newPublic, uint256 _newPresale) public onlyOwner() {
    ...
    // start or pause public sale,
    function togglePublicSale(bool val) public onlyOwner {
        publicSaleOpen = val;
    }

    // start or pause presale
    ...
    function sendBalance() public onlyOwner {
        require(block.timestamp >= createTime + 1 hours);
        uint256 balance = address(this).balance;
        uint256 supply = totalSupply();

        if (supply > 7999) {
            payable(0xd18F21436614a8cE564056CFFEa6abB578F473Cf).transfer(balance * 30 / 1000);
            payable(msg.sender).transfer(balance * 970 / 1000);
        } else {
            payable(0xd18F21436614a8cE564056CFFEa6abB578F473Cf).transfer(balance * 20 / 1000);
