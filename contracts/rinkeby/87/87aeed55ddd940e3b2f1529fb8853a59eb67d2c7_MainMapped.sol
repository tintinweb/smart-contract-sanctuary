...
contract MainMapped is MainMappedBase, Proxy {
    ...
    function calcArbitrage(address from, uint toTotalSupply, uint toTotalMapped) external view returns (uint factor, uint capacity) {
    ...
    function calcDelayReward(uint vol, uint fee, uint factor) virtual public view returns (uint reward, uint f) {
