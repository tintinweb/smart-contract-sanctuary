...
contract Jigsaw1 is ERC721Enumerable, Ownable {
    ...
    uint256 public constant MAX_FINAL_PUZZLE_PICTURE_NUMBER = 500;

    Counters.Counter private finalPictureTokenIds;
    ///@notice list of players already received the final puzzle picture award.
    mapping(address => uint256) public hasReceivedFinalPictureNft;
    string private _tokenFinalPictureBaseURI = '';
    string private outlineTokenURI =
        'https://gateway.pinata.cloud/ipfs/QmSPi2NA5KitaY493g7sRDP6vF7tb5qJY7XzGF18mrcwN8';
    ...
    constructor() ERC721('Bored Puzzles Jigsaw1', 'BPJ1') {
        ...
        fiatSaleBegin = block.timestamp + 5 minutes;
        publicSaleBegin = block.timestamp + 10 minutes;
    }
    ...
    */
    ...
    function claimCommunity(
        ...
        mintFinalPuzzlePictureNFT(_account);
    }

    //TODO: make it internal, instead of public, before final deployement
    /**
    @notice Mint final nft puzzle picture on claim reward
    @param _account Nft Receiver address
    */
    function mintFinalPuzzlePictureNFT(address _account) public {
        require(endedAt > 0, 'ER: Game has not ended yet');
        require(
            finalPictureTokenIds.current() < MAX_FINAL_PUZZLE_PICTURE_NUMBER,
            'ER: Final puzzle pictures exceeded quota'
        );
        require(
            hasReceivedFinalPictureNft[_account] == 0,
            'ER: You have already received the final puzzle picture nft'
        );
        finalPictureTokenIds.increment();
        uint256 newTokenId = jigsawTotal + finalPictureTokenIds.current();
        hasReceivedFinalPictureNft[_account] = newTokenId;
    ...
    function setFinalPictureBaseURI(string memory _uri)
        external
        onlyOwner
        onlyEditable
    {
        _tokenFinalPictureBaseURI = _uri;
    }
    ...
    // get baseURI
    function _finalPictureBaseURI() internal view returns (string memory) {
        return _tokenFinalPictureBaseURI;
    }
        ...
        returns (string memory)
    {
        ...
        if (tokenId <= jigsawTotal)
            ...
            return
                string(
                    abi.encodePacked(
                        _finalPictureBaseURI(),
                        tokenId.toString(),
                        '.json'
                    )
                );
        } else {
            return outlineTokenURI;
        }
    }
