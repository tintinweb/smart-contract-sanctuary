...
contract CoolContract is Context, IERC20, Ownable, VRFConsumerBase {
    ...
    uint256 public _lotterySwapMinAmount = 100000;
    ...
    event SwapForRuffleEnabledUpdated(bool enabled);
    ...
    event LiquidifyRequirements(bool LiquidityOverMinTokenBalance, bool inSwapAndLiquify, bool inSwapRuffle, bool inSwapLink, bool fromPancakeswapV2Pair);

    event LotteryRequirements(bool LiquidityOverMinTokenBalance, bool inSwapAndLiquify, bool inSwapRuffle, bool inSwapLink, bool fromPancakeswapV2Pair);
    ...
    function setLotterySwapMinAmount(uint256 lotterySwapMinAmount) external onlyOwner() {
        _lotterySwapMinAmount = lotterySwapMinAmount;
    }
    ...
    function setSwapForRaffleEnabled(bool _enabled) public onlyOwner {
        ...
        emit SwapForRuffleEnabledUpdated(_enabled);
    }
    ...
    function _takeForRaffle(uint tLottery) private {
        _lotteryBalanceForPeriod = _lotteryBalanceForPeriod + tLottery; // Save liquidity to overall balance until next raffle.
        uint256 currentRate = _getRate();
    ...
    function _transfer(
        ...
        emit LiquidifyRequirements(LiquidityOverMinTokenBalance, !inSwapAndLiquify, !inSwapRuffle, !inSwapLink, (from != pancakeswapV2Pair));

        emit LotteryRequirements(LotteryOverMinTokenBalance, !inSwapAndLiquify, !inSwapRuffle, !inSwapLink, (from != pancakeswapV2Pair));
    ...
    if (
            ...
            swapTokensForEth(numTokensSellForLottery);
            _lotteryBalanceForPeriod = 0;
        }
    ...
    function _swapForRaffle(uint256 amount) private lockTheRaffleSwap {
        ...
        if (_lotteryFee != 0 && amount >= _lotterySwapMinAmount) {
