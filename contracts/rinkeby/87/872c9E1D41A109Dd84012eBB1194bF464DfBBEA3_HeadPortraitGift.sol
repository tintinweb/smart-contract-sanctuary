...
contract HeadPortraitGift is OwnableContract{
    ...
    function batchClaim(uint256[] memory orderId, uint256[] memory amount, bytes[] memory signature) public returns(uint256[] memory){
        ...
        uint256[] memory tokenIdArray = new uint256[](orderId.length);
        for(uint256 i=0; i<orderId.length; i++){
            tokenIdArray[i] = tokenId;
            claim(orderId[i], amount[i], signature[i]);
        }
        return tokenIdArray;
    }

    function claim(uint256 orderId, uint256 amount, bytes memory signature) internal{
