...
contract NAME is Context, IERC20, Ownable {
    ...
    address public marketingAddr = address(0);

    uint256 public _liquidityFee = 7;
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 public _maxAllowedBalanceAmount = 10000000000000 * 10**9; //1% of totalSupply
    uint256 public _maxTxAmount = 5000000000000 * 10**9; //0.5% of totalSupply
    uint256 public _blocksToWait = 1440; // Around 5s/Block = 2hrs
    ...
    constructor () {
        ...
        //For RINKEBY: 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
    ...
    function setMarketingFeePercent(uint256 marketingFee) external onlyOwner() {
        require(marketingFee <= 10, "Percentage too high. Please use a lower fee.");
    ...
    function setSellLimitForBlockCooldown(uint256 newAmount) external onlyOwner() {
        limitToSellBeforeBlockCooldown = newAmount;
    }
    ...
    function _transfer(
        ...
        require(block.number >= amountBlockToSellNext[from],"You have sold too much, try to sell again in 1000 Blocks.");
        if(to == uniswapV2Pair){        // if this is the Uniswap Pair then we want to count how much he sold
            amountSoldTrack[from] += amount;
        }

        uint256 currentBalance = balanceOf(to);
        if(!_isAdminAccount[from] || !_isAdminAccount[to]) {
            require((currentBalance + amount) <= _maxAllowedBalanceAmount, "Account balance exceeds the maxAllowedBalanceAmount.");
        }

        // Limit Sell to 0.5% of Total Supply
        ...
        if(amountSoldTrack[from] >= limitToSellBeforeBlockCooldown){
            amountBlockToSellNext[from] = block.number.add(1000);
