...
contract Pack is ERC1155, Ownable, IPackEvent {
  ...
  uint private _seed = 0;
  uint private prevSeed = 0; // TESTING PURPOSES ONLY
  ...
  constructor(bytes32 _keyHash) ERC1155("") {
    keyHash = _keyHash;
  }
  ...
  * @param packId The ERC1155 tokenId of the pack token being opened.
  ...
  function openPack(uint packId) external {
    ...
    prevSeed = _seed;
    uint seed = uint(keccak256(abi.encodePacked(msg.sender, _seed)));
    _seed = seed;
  ...
  function requestRandomNumber(bytes32 _keyHash, uint _fee, uint _randomnessSeed) internal view returns (bytes32 requestId) {
    requestId = keccak256(abi.encodePacked(_keyHash, _fee, msg.sender, _randomnessSeed));
  }

  /// @dev FOR LOCAL TESTING PURPOSES.
  function testChainlinkCall(uint randomNumber) external {
    uint currentSeed = uint(keccak256(abi.encodePacked(msg.sender, prevSeed)));
    bytes32 requestId = requestRandomNumber(keyHash, chainlinkFee, currentSeed);

    testFulfillRandomness(requestId, randomNumber);
  } 

  /// @dev Called by Chainlink VRF random number provider.
  function testFulfillRandomness(bytes32 requestId, uint randomness) internal {
