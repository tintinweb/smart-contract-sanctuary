    ...
    controllerPanel
{
    ...
    function _assignCard(address _address, uint256 _typeID) internal {
        generic.mintPurchase(_address, _typeID, 0);
    }

    function presetPrice(uint256 _type, uint256 _price) external onlyAllowed {
...
abstract contract typePricingFixed {
    ...
    function setPricing(uint256 _type, uint256 _price) internal {
        typePrice[_type] = _price;
    }

    function getPricing(uint256 _type) public view returns (uint256) {
...
interface IEC721 {
    ...
    function mintWithVRF(address _receiver, uint256 _cardType)
        external
        returns (uint256);
