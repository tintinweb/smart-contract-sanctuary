...
contract LiquidityMining is Ownable, ReentrancyGuard {
        ...
        checkStart
    {
        ...
        require(stakingToken.balanceOf(msg.sender)>=amount,"Insufficient amount for deposit");
        ...
        checkStart
    {
        ...
        require(_balances[msg.sender] >= amount,"Insufficient amount for withdraw");
    ...
    function emergencyWithdraw(uint256 amount) external nonReentrant protected {
        require(amount > 0, "StakingRewards: Cannot withdraw 0");
...
contract StakingFactory is Ownable {
    ...
    function createLiq(
        ...
        require(_startTime > _duration, "start time must be greater than duration");

        LiquidityMining _liqmining = new LiquidityMining();
        IERC20 token = IERC20(_token);
        
        token.approve(address(_liqmining), _initreward);
        token.transferFrom(msg.sender,address(_liqmining), _initreward);

        _liqmining.initialize(_token, _initreward, _startTime, _duration);

        _liqmining.transferOwnership(msg.sender);
        liqs.push(_liqmining);
