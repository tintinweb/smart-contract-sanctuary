...
contract TreesMakeTheWorldGoRound is ERC721, Ownable {
    ...
    // TODO: replace before deploy
    uint256 public constant NFT_SUPPLY = 21;
    // uint256 public constant NFT_SUPPLY = 10001;
        ...
        returns (string memory)
    {
        ...
        // 1) we haven't added the CID for this token
        if (!_hasCID(tokenId)) {
            return string(abi.encodePacked(_unrevealedURI));
        } else {
            return string(abi.encodePacked(_tokenUriList[tokenId]));
        }
    }

    // verify that the CID has been set for the requested token
    function _hasCID(uint256 tokenId) public view returns (bool) {
    ...
    function mint(uint256 amountToMint) public payable {
        require(
            saleStarted == true && block.timestamp > PUBLIC_SALE_START_TIME,
            "Sale not active"
        );
    ...
    */
    ...
    function batchTokenUriList(
        ...
        require(_exists(lastToken), "Out of range");
        for (uint256 i = startingTokenId; i < lastToken + 1; i++) {
            _tokenUriList[i] = tokenUriList[i - 1];
        }
    }
