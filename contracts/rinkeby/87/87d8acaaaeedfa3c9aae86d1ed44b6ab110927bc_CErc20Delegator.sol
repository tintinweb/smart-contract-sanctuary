...
contract CErc20Delegator is CTokenInterface, CErc20Interface, CDelegatorInterface {
   ...
   /**
    ...
    function _setTrustedSupplier(address account, bool exists, uint supplyAllowance) external returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("_setTrustedSupplier(address, bool, uint)", account, exists, supplyAllowance));
        return abi.decode(data, (uint));
    }

    function _setTrustedBorrower(address account, bool exists, uint borrowAllowance) external returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("_setTrustedBorrower(address, bool, uint)", account, exists, borrowAllowance));
        return abi.decode(data, (uint));
    }
}