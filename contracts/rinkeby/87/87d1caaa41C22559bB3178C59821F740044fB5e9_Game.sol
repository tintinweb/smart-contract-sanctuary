...
contract Game is Ownable, IERC1155Receiver { 
    ...
    ERC1155Tradable public tokenERC1155;
    ...
    constructor(address groupies_, address tokenERC1155_, uint256 tokenId_, string memory solution_) {
        groupies = IERC721(groupies_);
        tokenERC1155 = ERC1155Tradable(tokenERC1155_);
        _tokenId = tokenId_;
        _solution = sha256(abi.encode(solution_));
    }

    //Sets tokenId that wants to be gifted
    function setTokenId(uint256 tokenId_) external onlyOwner {
        _tokenId = tokenId_;
    }

    //for this function call
    function setSolution(string memory newSolution_) external onlyOwner {
        _solution = sha256(abi.encode(newSolution_));
    }
