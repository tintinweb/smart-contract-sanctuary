...
//		}
		//back amout to Bider
//		if (bid.quantity > _quantity) {
//			_paid(bid.paymentToken, bid.bidder, bid.bidPrice.mul( bid.quantity.sub(_quantity) ));
//		}
	...
	function adminMigrateOrders(address oldMarket) external onlyOwner() {
		...
		for (uint256 i = 0; i < totalOrders; i++) {
			...
			if (quantity > 0) {
				if (isERC721) {
					if (IERC721(tokenAddress).ownerOf(tokenId) == oldMarket) {
