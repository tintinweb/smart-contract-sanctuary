...
contract RobinHood is Context, IERC20, Ownable {
    ...
    uint256 private _whaleTaxPriceImpact = 3;
    uint256 private _whaleTaxSupply = 500_000_000_000 * 10**9;
    uint256 private _whaleTaxFee = 30;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(sender != owner() && recipient != owner()) {
            ...
            if (whaleTaxEnabled && recipient == uniswapV2Pair) {
                if (amount >= _whaleTaxSupply || amount <= balanceOf(uniswapV2Pair).mul(_whaleTaxPriceImpact).div(100)) {
    ...
    function setLiquidityAdded(bool added) public onlyOwner {
    ...
    function _setWhaleTaxPriceImpact(uint256 whaleTaxPriceImpact) external onlyOwner() {
        _whaleTaxPriceImpact = whaleTaxPriceImpact;
    }
