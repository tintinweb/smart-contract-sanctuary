...
// File: ..\contracts\AvnFTTreasuryV2.sol


pragma solidity 0.7.5;

contract AvnFTTreasuryV2 is IAvnFTTreasuryV2, Owned {
  ...
  constructor(IAvnFTTreasury _avnFTTreasury)
  {
  ...
  receive() external payable onlyPermitted() {}
  ...
  function unlockETH(address _recipient, uint256 _amount)
    onlyPermitted
    external
    override
  {
  ...
  function unlockERC777Tokens(address _token, address _recipient, uint256 _amount)
    onlyPermitted
    external
    override
  {
    IERC777(_token).send(_recipient, _amount, "");
  }

  function unlockERC20Tokens(address _token, address _recipient, uint256 _amount)
    onlyPermitted
    external
    override
  {
    assert(IERC20(_token).transfer(_recipient, _amount));
  }

  function recoverERC777Tokens(address _token)
    onlyOwner
    external
    override
  {
    uint256 lockedBalance = IERC777(_token).balanceOf(address(avnFTTreasury));
    avnFTTreasury.unlockERC777Tokens(_token, lockedBalance, "");
  }

  function recoverERC20Tokens(address _token)
    onlyOwner
    external
    override
  {
    uint256 lockedBalance = IERC20(_token).balanceOf(address(avnFTTreasury));
    avnFTTreasury.unlockERC20Tokens(_token, lockedBalance);
  }
}