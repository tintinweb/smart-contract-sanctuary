...
contract QollateralManager is IQollateralManager {

  using SafeMath for uint;

  /// @notice Address of the `Qontroller`
  address public qontrollerAddress;
  ...
  mapping(address => address[]) private iterableAccountAssets;
  ...
  function depositCollateral(address tokenAddress, uint amount) external {
    ...
    IQontrollerV1 qontroller = IQontrollerV1(qontrollerAddress);
  ...
  function getTotalNetBorrowValue(address account) external view returns(uint){
    return _getTotalNetBorrowValue(account);
  }
  ...
  /// @return uint Borrow value of account in USD
  function getNetBorrowValue(
                          address marketAddress,
                          address account
                          ) external view returns(uint){
    return _getNetBorrowValue(marketAddress, account);
  }
  ...
  function getAccountBalances(
    ...
    return accountBalances[account][tokenAddress];
  }
  ...
  function getIterableAccountAssets(
    ...
    return iterableAccountAssets[account];
  }
  ...
  function getIterableAccountMarkets(
    ...
    return iterableAccountMarkets[account];
  }
  ...
  function getAccountAssets(
    ...
    return accountAssets[tokenAddress][account];
  }
  ...
  function getAccountMarkets(
    ...
    return accountMarkets[fixedRateLoanMarketAddress][account];
  }
  ...
  function _addAccountMarket(address account) external {
    ...
    if(!accountMarkets[msg.sender][account]){
      accountMarkets[msg.sender][account] = true;
      iterableAccountMarkets[account].push(msg.sender);
    }
  }
  ...
  function _getTotalCollateralValue(
    ...
    for(uint i=0; i<iterableAccountAssets[account].length; i++){
      ...
      // Get the collateral value for the i'th `tokenAddress`
      uint valueUSD = _getCollateralValue(tokenAddress, account, applyRiskFactor);
  ...
  /// @param account Account to query
  /// @return uint Borrow value of account in USD
  function _getTotalNetBorrowValue(address account) internal view returns(uint){

    uint totalValueUSD = 0;
    for(uint i=0; i<iterableAccountMarkets[account].length; i++){
      
      // Get the market address in i'th slot of `iterableAccountMarkets[account]`
      address marketAddress = iterableAccountMarkets[account][i];

      // Get the net borrow value for the i'th `tokenAddress`
      uint valueUSD = _getNetBorrowValue(marketAddress, account);
  ...
  function _getNetBorrowValue(
    ...
    uint borrowsInLocal = market.getAccountBorrows(account);
    ...
    if(lendsInLocal >= borrowsInLocal){
      ...
      address tokenAddress = market.getPrincipalTokenAddress();
      QTypes.Asset memory asset = qontroller.getAsset(tokenAddress);
      address oracleFeed = asset.oracleFeed;
      uint borrowValueUSD = _getValueUSD(tokenAddress, oracleFeed, borrowValueLocal);
...
contract QontrollerV1 is IQontrollerV1 {
  ...
  /// tokenAddress => Asset
  mapping(address => QTypes.Asset) private assets;
  ...
  mapping(address => mapping(uint => address)) private fixedRateLoanMarkets;

  /// @notice Mapping to determine whether a `fixedRateLoanMarket` address
  /// is enabled or not
  /// fixedRateLoanMarket => bool
  mapping(address => bool) private enabledMarkets;
  ...
  constructor() public {
    ...
    // deploy single instance of `QollateralManager` contract
  ...
  function _addAsset(
    ...
    QTypes.Asset memory asset = QTypes.Asset(true, _oracleFeed, _riskFactor, maturities);
  ...
  function _createFixedRateLoanMarket(
    ...
    // Only allow Assets that have already been added
    ...
    enabledMarkets[address(frlm)] = true;
  ...
  function getAsset(address tokenAddress) external view returns(QTypes.Asset memory) {
  ...
  function isMarketEnabled(address fixedRateLoanMarketAddress) external view returns(bool){
    return enabledMarkets[fixedRateLoanMarketAddress];
  }
}
