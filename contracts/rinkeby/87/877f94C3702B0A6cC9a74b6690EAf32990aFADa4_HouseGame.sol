...
contract HouseGame is Ownable, ReentrancyGuard, Pausable {
  ...
  modifier requireContractsSet() {
      require(address(cheddarToken) != address(0) && address(houseTraits) != address(0)
        && address(tower) != address(0) && address(houseNFT) != address(0)
        , "Contracts not set");
      _;
  }

  function setContracts(address _cheddar,address _houseTraits, address _tower, address _house) external onlyOwner {
  ...
  function reveal(address addr) internal {
    ...
    for (uint k = 0; k < commit.amount; k++) {
      ...
      if (!commit.stake || recipient != addr) {
        ...
        houseNFT.mint(address(tower), seed);
        tokenIdsToStake[k] = minted;
      }
    }
    houseNFT.updateOriginAccess(tokenIds);
    if(commit.stake) {
      tower.addManyHouseToStackingPool(addr, tokenIdsToStake);
    }
    delete _mintCommits[addr][commitIdCur];
    delete _pendingCommitId[addr];
  ...
  */
  ...
  function selectRecipient(uint256 seed) internal view returns (address) {
    ...
    address thief = tower.randomCrazyCatOwner(seed >> 144); // 144 bits reserved for trait selection
