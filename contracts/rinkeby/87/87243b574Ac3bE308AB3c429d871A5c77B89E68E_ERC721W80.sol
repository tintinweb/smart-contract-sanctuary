...
import "./LibW80.sol";
import "../roles/W80OperatorRole.sol";

contract ERC721W80 is ERC721Base, W80OperatorRole {
    ...
    function __ERC721W80_init(
        ...
        _registerInterface(0x676aae1f); // bytes4(keccak256("setRequestStatus(uint256,LibW80.RequestStatus)"))
        _setBaseURI(baseURI);
    ...
    {
        require(_exists(tokenId), "ERC721W80: the token does not exist");
        _owner = ownerOf(tokenId);
        _requestStatus = requestStatus[tokenId];
    }

    function setRequestStatus(
        uint256 tokenId,
        LibW80.RequestStatus _requestStatus
    ) public onlyW80Operator {
        requestStatus[tokenId] = _requestStatus;
        emit RequestStatusSet(tokenId, _requestStatus);
    }
    ...
    function mintAndTransfer(
        LibERC721LazyMint.Mint721Data memory data,
        address to
    ) public virtual override(ERC721Lazy) {
        requestStatus[data.tokenId] = LibW80.RequestStatus.ORDERED;
...
contract W80OperatorRole is OwnableUpgradeable {
    mapping(address => bool) operators;

    function __W80OperatorRole_init() external initializer {
        __Context_init_unchained();
        __Ownable_init_unchained();
    }
