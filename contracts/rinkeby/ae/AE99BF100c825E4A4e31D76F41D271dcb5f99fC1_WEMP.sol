 ...
 */
contract WEMP is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    mapping (address => bool) private setForNoFee; // Tax Flag
    address[] private _excluded;
   
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 1000000 * 10**9 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % 600000 * 10**9 * 10**18));
    ...
    uint256 public liquidityFeeToSell = 100 * 10**18;
    ...
    event ExcludeFromFee(address account, bool excludeFromFee);
    ...
    constructor () {
        ...
        charity_address = 0xa3547a336973573744083114291de06072C722bf;
        ...
        uint256 deflationAmount = _tTotal * 40 / 100;
        _amount_burnt += deflationAmount;
    ...
    function excludeFromFee(address account) external onlyOwner {
        setForNoFee[account]= true;
        emit ExcludeFromFee(account, false);
    }
    
    function includeInFee(address account) external onlyOwner {
        setForNoFee[account] = false;
        emit IncludeInFee(account, true);
    }

    function isIncludedInFee(address account) external view returns(bool) {
        return !setForNoFee[account];
    }
    
    function setTaxFeePercent(uint256 taxFee) external onlyOwner {
        require((taxFee + _liquidityFee + _transactionBurn + _charityFee) < 100, "TaxFee should be less than 100%");
        _taxFee = taxFee;
        emit SetTaxFeePercent(taxFee);
    }

    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner {
        require((_taxFee + liquidityFee + _transactionBurn + _charityFee) < 100, "LiquidityFee should be less than 100%");
        _liquidityFee = liquidityFee;
        emit SetLiquidityFeePercent(liquidityFee);
    }

    function setBurnPercent(uint256 burn_percentage) external onlyOwner {
        require((_taxFee + _liquidityFee + burn_percentage + _charityFee) < 100, "burn_percentage should be less than 100%");
        _transactionBurn = burn_percentage;
        emit SetBurnPercent(burn_percentage);
    }

    function setCharityFeePercent(uint256 charityFee) external onlyOwner {
        require((_taxFee + _liquidityFee + _transactionBurn + charityFee) < 100, "charityFee should be less than 100%");
        _charityFee = charityFee;
        emit SetCharityFeePercent(charityFee);
    }

    function setCharityAddress(address account) external onlyOwner {
        require(account != address(0), "ERC20: Charity address cannot be a zero address");
        charity_address = account;
        emit SetCharityAddress(account);
    }
    ...
    function _getTValues(uint256 amount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        uint256 tAmount = amount;
        uint256 tFee = calculateTaxFee(tAmount);
        uint256 tLiquidity = calculateLiquidityFee(tAmount);
        uint256 tCharityFee = calculateCharityFee(tAmount);
        uint256 tBurn = calculateTransactionBurn(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tBurn).sub(tCharityFee);
        return (tTransferAmount, tFee, tLiquidity, tBurn, tCharityFee);
    }

    function _getRValues(uint256 amount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tCharityFee) private view returns (uint256, uint256, uint256) {
        uint256 currentRate = _getRate();
        uint256 tAmount = amount;
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rliquidity).sub(rBurn).sub(rCharityFee);
    ...
    function _takeCharityFee(address sender, uint256 tCharityFee) private {
        uint256 currentRate =  _getRate();
        uint256 rCharityFee = tCharityFee.mul(currentRate);
        _rOwned[charity_address] = _rOwned[charity_address].add(rCharityFee);
        if(_isExcluded[charity_address])
            _tOwned[charity_address] = _tOwned[charity_address].add(tCharityFee);
        
        emit Transfer(sender, charity_address, tCharityFee);
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _transactionBurn == 0 && _charityFee == 0) return;
    ...
    function _transfer(
        ...
        //else remove fee
        if(!_enableFee || setForNoFee[from] || setForNoFee[to]){
            takeFee = false;
        }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tCharityFee)  = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tBurn, tCharityFee);

        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _reflectFee(rFee, tFee);
        _takeLiquidity(sender, tLiquidity);
        _takeCharityFee(sender, tCharityFee);
        if(tBurn > 0) {
            _amount_burnt += tBurn;
