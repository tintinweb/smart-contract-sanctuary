...
contract Vault79BondDepository is Ownable {
    ...
    IERC20 public immutable Ore; // token given as payment for bond
    IERC20 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints Ore when receives principle
    ...
    struct Bond {
        uint payout; // Ore remaining to be paid
    ...
    enum PARAMETER { VESTING, PAYOUT, FEE, DEBT, MINPRICE, CONTROL }
    /**
    ...
    function setBondTerms ( PARAMETER _parameter, uint _input ) external onlyOwner() {
        ...
        } else if ( _parameter == PARAMETER.CONTROL ) { // 5
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 Ore ( underflow protection )
        ...
        uint balanceBefore = Ore.balanceOf(address(this));
        /**
            principle is transferred in
            approved and
            deposited into the treasury, returning (_amount - profit) Ore
         */
        ...
        require(balanceBefore.add(profit) == Ore.balanceOf(address(this)), "Not enough Ore to cover profit");
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            Ore.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                Ore.approve( address(stakingHelper), _amount );
                stakingHelper.stake( _amount, _recipient );
            } else {
                Ore.approve( address(staking), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return Ore.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
    ...
    /**
     *  @notice calculate current ratio of debt to Ore supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = Ore.totalSupply();
    ...
    /**
     *  @notice calculate amount of Ore available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or Ore) to the DAO
     *  @return bool
     */
    function recoverLostToken( IERC20 _token ) external returns ( bool ) {
        require( _token != Ore, "NAT" );
