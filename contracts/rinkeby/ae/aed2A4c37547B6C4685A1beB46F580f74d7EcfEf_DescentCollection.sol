...
contract DescentCollection is ERC721Enumerable, ERC721Burnable, ReentrancyGuard, Ownable {
    ...
    function _priceCheck() public view returns (uint256) {
        require(isMintingActive, "Mint has not started.");
        require(totalSupply() < MAX_DESCENTS,"They are all gone, check OpenSea for secondary.");

        uint256 currentSupply = totalSupply();
        uint256 result;

        if (currentSupply >= 111) {
            result = 0.15 ether;
            return result;
        } else {
            result = 0.15 ether;
    ...
    function mint(uint256 _numberToMint) public payable nonReentrant() {
        require(isMintingActive, "Mint hasn't started yet.");
        require(totalSupply() < MAX_DESCENTS, "They are all gone, check OpenSea for secondary.");
        require(_numberToMint > 0 && _numberToMint <= 1, "You can only mint 1 Descent.");
        require(totalSupply().add(_numberToMint) <= MAX_DESCENTS, "That's way too many.");
        require(_priceCheck().mul(_numberToMint) <= msg.value, "Ether value sent too small.");
    ...
    function _mint(uint256 _numberToMint) internal {
        require(_numberToMint == 1, "You can only mint 1 Descent.");
        ...
        for (uint256 i = 0; i < _numberToMint; i++) {
            ...
            emit DecentMinted(msg.sender, RandomTokenId);
        }
        _numOfAvailableTokens = updatedNumAvailableTokens;

    }

    function reserve(uint256[] calldata descents) public onlyOwner {
        require(descents.length <= reservedTokens, "3 Reserved");
        require(totalSupply().add(descents.length) <= MAX_DESCENTS, "Exceeds MAX_DESCENTS");

        for ( uint256 i = 0; i < descents.length; i++) {
            uint256 index = descents[i];
            ...
            emit DecentMinted(msg.sender, index);
        }
    }

    function toggleMintingAndCheckState() public onlyOwner returns(bool) {
