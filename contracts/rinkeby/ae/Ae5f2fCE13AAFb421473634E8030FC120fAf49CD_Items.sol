...
// File: contracts/new_ıtem_collection.sol
...
contract Items is ERC1155, Ownable, ERC1155Burnable  {
    ...
    uint256[] public conversionList;
    uint256 public balanceUnit  = 0.00000000000000001 ether;
    ...
    constructor() ERC1155("ITEMS_NEW") {
        conversionList.push(0);
    }
...
function openChest(uint256 _chestId,uint256 amount) public payable  returns(bool success)  {

     require(chestContract.ownerOf(_chestId) == _msgSender());
     require(tokenContract.balanceOf(msg.sender) >= amount);    
     require(conversionList[_chestId] != 0 );

     tokenContract.transferFrom(_msgSender(),tokenStorage, amount);
     chestContract.safeTransferFrom(_msgSender(),blackHoleAddress,_chestId);
    _mint(_msgSender(), conversionList[_chestId], 1,"");

    emit OperationResult(true,conversionList[_chestId]);

    conversionList[_chestId] = 0;
    return true;
   }

    function setPermissions(uint256[] memory ids) public onlyOwner {
        for (uint256 i; i < ids.length; i++) {
         conversionList.push(ids[i]);
        }
    }
    ...
    function setBalances(address[] calldata _addresList, uint256[] calldata _balanceList) public onlyOwner returns (bool success)
    {
        ...
        {
         tokenContract.transferFrom(tokenStorage,_addresList[i],_balanceList[i] * balanceUnit);
        }
       
    return true;
    }

    function changeConversion(uint256 index,uint256 value) public onlyOwner {
        conversionList[index] = value;        
    }
    ...
    function burnItem(address account,uint256 id,uint256 amount) public
        onlyOwner
    {
        _burn(account, id, amount);
    }

     function burnItems(address account,uint256[] memory ids,uint256[] memory amounts) public
        onlyOwner
    {
