...
contract TheLinksTestv2 is ERC721, Ownable {
    using SafeMath for uint256;
	...
	uint256 public purchasedLinks;
    uint256 public startingIpfsId;
    address public fundWallet;
    uint256 private _lastIpfsId;

    constructor (string memory _tokenBaseUri, address _fundWallet) ERC721("TheLinksTestv2", "TheLinksTestv2") {
	...
	//purchase multiple links at once
	function purchaseLinks(uint256 _howMany) public payable {
	    require(_howMany > 0, "Minimum 1 tokens need to be minted");
	    require(_howMany <= LinksRemainingToBeMinted(), "Purchase amount is greater than the token available");
        require(isSaleActive, "Sale is not active");
		require(_howMany <= 20, "max 20 links at once");
		require(itemPrice.mul(_howMany) == msg.value, "Insufficient ETH to mint");
		for (uint256 i = 0; i < _howMany; i++) {
			_mintLink(_msgSender());
		}
	}
    
    function _mintLink(address _to) private {
        if(purchasedLinks == 0) {
        ...
        purchasedLinks++;
        require(!_exists(purchasedLinks), "Mint: Token already exist.");
        _mint(_to, purchasedLinks);
        _setTokenURI(purchasedLinks, uint2str(_lastIpfsId));
    }
    ...
    function LinksRemainingToBeMinted() public view returns (uint256) {
        return totalTokenToMint.sub(purchasedLinks);
    }

    function isAllTokenMinted() public view returns (bool) {
        return purchasedLinks == totalTokenToMint;
    }

    function getIpfsIdToMint() public view returns(uint256 _nextIpfsId) {
        require(!isAllTokenMinted(), "All tokens have been minted");
        if(_lastIpfsId == totalTokenToMint && purchasedLinks < totalTokenToMint) {
            _nextIpfsId = 1;
        } else if(purchasedLinks < totalTokenToMint) {
