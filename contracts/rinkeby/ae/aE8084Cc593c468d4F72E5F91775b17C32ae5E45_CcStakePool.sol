...
contract CcStakePool is ReentrancyGuard, AddressPrivileges, SafeTransfer{
    ...
    function setPoolInfo(address DebtToken,address stakeToken,uint64 interestrate) public onlyOwner{
        debtCoin = IDebtToken(DebtToken);
    ...
    function DebtTotalSuply()public view returns (uint256){
        return debtCoin.totalSupply();
    }

    function tokenNetworth() public view returns (uint256){
        uint256 tokenNum = DebtTotalSuply();
        return (tokenNum > 0 ) ? _totalSupply.mul(calDecimal)/tokenNum : calDecimal;
    }

    function stake(uint256 amount) external payable nonReentrant notPause {
        ...
        debtCoin.mint(msg.sender,mintAmount);
        emit Stake(msg.sender,_poolToken,amount,mintAmount);
    }

    function unstake(uint256 amount) external nonReentrant notPause {
        ...
        debtCoin.burn(msg.sender,amount);
    ...
    function borrow(uint256 amount) public onlyMinter returns(uint256) {
    ...
    function borrowAndInterest(uint256 amount) public onlyMinter{
        //l1*r + (l0-l1) = -amount
    ...
    function repay(uint256 amount,bool bAll) public payable onlyMinter{
        amount = getPayableAmount(_poolToken,amount);
        if(!bAll){
    ...
    function repayAndInterest(uint256 amount) public payable onlyMinter {
        amount = getPayableAmount(_poolToken,amount);
        //l1*r + (l0-l1) = amount
