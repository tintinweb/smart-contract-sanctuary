...
contract Tapestry is ERC721, Ownable {
  ...
  constructor() public ERC721("Tapestry", "TAP") {}
  ...
  function mint(
    ...
    require(newItemId == lineNumber, "Position is incorrect");
    if (linesOwned[msg.sender] != 0) {
        ...
        require((1 + lastLineForOwner[msg.sender]) < newItemId, "1 lines between mints required");
    }
    // Validate token to be minted
    _validateTokenToBeMinted(tokenUri, line);
    // mark token URI as minted
    myTokenURI[tokenUri] = 1;
    // mark token name as taken, avoid same line minted twice
    myTokenName[line] = 1;
    // save line on chain
    lines[newItemId].text = line;
    ...
    emit TokenBought(newItemId, line, bytes(line).length);
