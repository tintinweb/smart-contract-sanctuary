...
contract WebacyProxy is AccessControl {
    ...
    function transferApprovedERC20Tokens(address _contractAddress, address _ownerAddress, address _recipentAddress, uint256 _amount) external onlyRole(EXECUTOR_ROLE) onlyWhenUnlocked {
        IERC20(_contractAddress).transferFrom(_ownerAddress, _recipentAddress, _amount);
    }

    function transferApprovedERC721Tokens(address _contractAddress, address _ownerAddress, address _recipentAddress, uint256 _tokenId) external onlyRole(EXECUTOR_ROLE) onlyWhenUnlocked {
