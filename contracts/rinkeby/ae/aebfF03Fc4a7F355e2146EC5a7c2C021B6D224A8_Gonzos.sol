...
library MoreCounters {
  ...
  function nextRange(Counter storage counter, uint16 amount) internal returns (uint256, uint256) {
    unchecked {
    uint256 start = counter._value;
      counter._value += amount;
      return (start, counter._value - 1);
    }
  }
...
contract Gonzos is ERC721, ERC721Enumerable, Ownable {
  using MoreCounters for MoreCounters.Counter;

  bool public active = true;
  uint256 public created = block.timestamp;

  MoreCounters.Counter private _sequenceIdTracker;

  uint256 public constant tokenPrice = 0.015 ether; // 0.015 ETH
  uint16 public constant tokensToWithhold = 50; // 20 per transaction.
  uint256 public constant limitPerTx = 10; // 20 per transaction.
  uint256 public constant totalLimit = 1000; // 10240 tokens.

  string private _baseUri = "https://metadata.staxxinvaders.com/tokens/";
  ...
  function mintTokens(uint16 count) public payable onlyActive {
    ...
    (uint256 start, uint256 end) = _sequenceIdTracker.nextRange(count);
  ...
  function reserveTokens() public onlyOwner {
    (uint256 start, uint256 end) = _sequenceIdTracker.nextRange(tokensToWithhold);
