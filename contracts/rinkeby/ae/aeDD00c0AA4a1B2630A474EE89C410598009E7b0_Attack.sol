...
contract Attack is IAttack, Ownable, Pausable {
    ...
    event BearsAttackFailed(address indexed owner, uint256 indexed nonce, uint256 indexed tokenId, uint256 hiveId);
    event BearsAttackResolved(address indexed owner, uint256 indexed nonce, uint256 tokenId, uint256 successes, uint256 value);
    event BeekeeperAttackPrepared(address indexed owner, uint256 indexed nonce, uint256 indexed tokenId, uint256 hiveId);
    event BeekeeperCollectionFailed(address indexed owner, uint256 indexed nonce, uint256 indexed tokenId, uint256 hiveId);
    event BeekeeperAttackResolved(address indexed owner, uint256 indexed nonce, uint256 tokenId, uint256 value, uint256 owedPerBee);
    ...
    function _resolveAttack(
        ...
        uint256 seed = random(hiveId, tokenId);

        (owed, successes) = _attack(strength, hiveId, seed);
        ...
        if (successes >= 1) {
            ...
            address _owner;
            if (beesContract.getOwnerOf(tokenId) != address(hiveContract)) _owner = beesContract.getOwnerOf(tokenId);
            else _owner = hiveContract.getWaitingRoomOwner(tokenId);
            beesContract.increaseTokensPot(_owner, owed);
        }
        hiveContract.incTotalAttacks(hiveId);
        emit BearsAttackResolved(_msgSender(), nonce, tokenId, successes, owed);
    }
    ...
    function _attack(
        ...
        uint256 start = hiveContract.getHiveOccupancy(hiveId) / 2;
        if (beesAffected < 5) beesAffected++;
        for (uint256 y = start; y < start + beesAffected; y++) {
    ...
    function manyBearsAttack(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            // _resolveAttack(hiveIds[i]);
            ICryptoBees.Token memory t = beesContract.getTokenData(tokenIds[i]);

            // check if bear can attack
            if (t.cooldownTillTimestamp < block.timestamp && (hiveContract.getHiveProtectionBears(hiveIds[i]) < block.timestamp)) {
                // unresolvedAttacks[hiveIds[i]] = UnresolvedAttack({tokenId: tokenIds[i], block: uint64(block.number), nonce: uint48(nonce), howMuch: 0});
                _resolveAttack(nonce, hiveIds[i], tokenIds[i], t.strength);
                ...
                // delete unresolvedAttacks[hiveIds[i]];
                emit BearsAttackFailed(_msgSender(), nonce, tokenIds[i], hiveIds[i]);
            }
        }
    }
    ...
    function _resolveCollection(
        ...
        (owed, owedPerBee) = _collect(strength, hiveId, seed, howMuch);
        if (owed > 0) {
        ...
        emit BeekeeperAttackResolved(_msgSender(), nonce, tokenId, owed, owedPerBee);
    }
    ...
    function manyBeekeepersCollect(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (t.cooldownTillTimestamp < block.timestamp && hiveContract.isHiveProtectedFromKeepers(hiveIds[i]) == false) {
                ...
                _resolveCollection(nonce, hiveIds[i], tokenIds[i], t.strength, howMuch[i]);
                ...
                // delete unresolvedCollections[hiveIds[i]];
                emit BeekeeperCollectionFailed(_msgSender(), nonce, tokenIds[i], hiveIds[i]);
            }
        }
    }

    function random(uint256 hiveId, uint256 tokenId) internal view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(tx.origin, hiveId, tokenId, block.timestamp)));
    }
}
