...
contract Factory is ERC1155, ERC1155Holder, AccessControl, Ownable {
    ...
    uint public lastCollectionId;


    constructor(string memory _uri) ERC1155(_uri) {
        // First element (index 0) is empty, so collections start from 1
        Collections.push("");
        maxSupply.push(0);
        minted.push(0);
    }

    function addCollection(uint256 _maxSupply, string memory _name) external onlyOwner {
    ...
    /// @notice Mints 1 NFT
    function safeMint(uint _collectionId, address _beneficiary) public {
        require(hasRole(MINTER_ROLE, msg.sender), "Caller does not have minter role");
        require(minted[_collectionId] < maxSupply[_collectionId], "Sold out");
        _mint(_beneficiary, _collectionId, 1, "");
        minted[_collectionId] += 1;
    }
