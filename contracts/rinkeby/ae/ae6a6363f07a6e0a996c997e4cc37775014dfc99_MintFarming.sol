...
// File: contracts/farming/FarmingStorage.sol

pragma solidity ^0.5.16;



contract FarmingStorage is FarmingInterface {
    // The reward token.
    ...
    // The address from which the award is paid
...
// File: contracts/farming/MintFarming.sol
...
contract MintFarming is FarmingStorage, Ownable, Initializable {
    using SafeMath for uint256;
    ...
    function init(
        ...
        _registerPool(ERC20Detailed(address(_rewardToken)), _allocPoint, startBlock);
    }
    ...
    // Sets reward address. Can be called only by the owner.
    function setRewardAddress(address _rewardAddress) public onlyOwner {
        require(_rewardAddress != address(0), "You can't set zero address");
        rewardAddress = _rewardAddress;
    }
    ...
    function getPendingReward(ERC20Detailed _token, address _user) public view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && tokenSupply != 0) {
            ...
            accRewardPerShare = accRewardPerShare.add(totalReward.mul(10 ** decimals).div(tokenSupply));
        }
        return user.amount.mul(accRewardPerShare).div(10 ** decimals).sub(user.rewardDebt);
    }
    ...
    function updatePool(ERC20Detailed _token) public {
        ...
        pool.accRewardPerShare = pool.accRewardPerShare.add(totalReward.mul(10 ** decimals).div(tokenSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(ERC20Detailed _token, uint256 _amount) public {
        ...
        uint256 decimals = _token.decimals();
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accRewardPerShare).div(10 ** decimals).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accRewardPerShare).div(10 ** decimals);
    ...
    function _reward(address _to, uint256 _amount) internal {
        rewardToken.safeTransferFrom(rewardAddress, _to, _amount);
    }
}