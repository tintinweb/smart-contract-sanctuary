...
contract AvatarTreasury is AccessControl {
    ...
    event PaymentReceived(
        uint256 indexed purchaseItemId,
    ...
    function purchaseMintingRight() external payable {
        ...
        require(
            msg.value >= _ITEM_PRICE,
            "Value is not sufficient for purchase"
        );
        _purchaseIdCounter.increment();
        emit PaymentReceived(_purchaseIdCounter.current(), msg.sender);
    }
    ...
    /**
     * @notice Allows an admin to update sale limits.
     */
    ...
    function remainingItemsForSale() public view returns (uint256) {
        return _MAX_NUM_ITEMS.sub(_purchaseIdCounter.current());
    }
