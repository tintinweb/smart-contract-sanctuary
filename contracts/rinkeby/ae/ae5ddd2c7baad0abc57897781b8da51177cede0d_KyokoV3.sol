    ...
    PausableUpgradeable
{
    ...
    function getNftHolderMap(address holder) public view returns(uint256[] memory) {
        return nftHolderMap[holder].values();
    }

    function getDepositIdOfferMap(uint256 _depositId) public view returns(uint256[] memory) {
        return depositIdOfferMap[_depositId].values();
    }
