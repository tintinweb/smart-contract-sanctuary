...
contract Oracle is Ownable {
    ...
    event UpdatedTimeLock(uint indexed newTimeLock);

    bool              public initialized;
    ConditionalTokens public conditionalTokens;
    uint              public timeLockFinish;
    ...
    modifier checkTimeLock() {
        require(block.timestamp >= timeLockFinish, "Oracle: still locked");
        _;
    }

    function initialize(ConditionalTokens _conditionalTokens, uint _timeLockFinish) external initializer {
        require(_conditionalTokens != ConditionalTokens(0), "Oracle: conditionalTokens cannot be null");
        require(_timeLockFinish != 0, "Oracle: timeLockFinish cannot be null");
        require(_timeLockFinish > block.timestamp, "Oracle: timeLockFinish must be greater than current block.timestamp");

        timeLockFinish = _timeLockFinish;
        conditionalTokens = _conditionalTokens;
    }

    function reportPayouts(bytes32 questionId, uint[] calldata payouts) external protected checkTimeLock onlyOwner {
        conditionalTokens.reportPayouts(questionId, payouts);
    }

    function updateTimeLock(uint256 newTimeLock) external protected onlyOwner {
        require(newTimeLock >= block.timestamp, "Oracle: must be greater than current timestamp");
        require(newTimeLock >= timeLockFinish, "Oracle: must be greater than current timeLock");
        timeLockFinish = newTimeLock;
        emit UpdatedTimeLock(newTimeLock);
    }
