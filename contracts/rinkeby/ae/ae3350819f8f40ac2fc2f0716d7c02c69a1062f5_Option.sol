...
contract Option is IOption, Initializable, ReentrancyGuard, Ownable {
  ...
  function initialize (
      ...
      option.optionToken = _createZoomToken("OPTION");
      option.proofToken = _createZoomToken("PROOF");
  ...
  function _createZoomToken(
    ...
    bytes memory initData = abi.encodeWithSelector(ZOOMERC20_INIT_SIGNITURE, string(abi.encodePacked(option.name, "_", _suffix)));
    ...
    external override nonReentrant
  {
    ...
    require(targetToken.balanceOf(msg.sender) >= _amount, "ZOOM: amount > targetToken balance");
    ...
    require(maxOptionAmount <= option.totalSupply,"ZOOM: option token is over flow");
    require(maxProofAmount <= option.totalSupply,"ZOOM: proof token is over flow");
    ...
    external override nonReentrant returns ( uint256 _mintAmount )
  {
    ...
    // Validate sender targetToken balance is > amount
    IERC20 targetToken = IERC20(option.targetToken);
    require(targetToken.balanceOf(tx.origin) >= _amount, "ZOOM: amount > targetToken balance");
  ...
  */
  function unsubscribeOption( uint256 _amount ) external override  {
    ...
    require(_amount <= optionToken.balanceOf(msg.sender), "ZOOM: low OPTION balance");
    require(_amount <= proofToken.balanceOf(msg.sender), "ZOOM: low PROOF balance");
  ...
  */
    ...
    external override nonReentrant returns ( uint256 _burnAmount )
  {
    ...
    require(_amount <= optionToken.balanceOf(msg.sender), "ZOOM: low OPTION balance");
    require(_amount <= proofToken.balanceOf(tx.origin), "ZOOM: low PROOF balance");
  ...
  function excuteOption( uint256 _amount ) external override  {
    ...
    IZoomERC20 optionToken = IZoomERC20(option.optionToken);
    require(optionToken.balanceOf(msg.sender) >= _amount, "ZOOM: optionToken amount not enaugh");

    uint256 costBaseAmount  = _amount.mul(option.baseAmount).div(option.targetAmout);
    IERC20 baseToken = IERC20(option.baseToken);
    require(baseToken.balanceOf(msg.sender) >= costBaseAmount, "ZOOM: baseToken amount not enaugh!");
  ...
  function collectOption( uint256 _amount ) external override {
    ...
    IZoomERC20 proofToken = IZoomERC20(option.proofToken);
    require(proofToken.balanceOf(msg.sender) >= _amount, "ZOOM: proofToken amount not enaugh");
