...
contract Mittens is ERC20, Ownable {
    ...
    event SwapTokentForEth(uint256 tokenAmount, address from, address to);
    ...
    function _beforeTokenTransfer(
        ...
        console.log("_beforeTokenTransfer ", from, to, amount);
        if(!inSwap)ArrangeProfit(from, to);
        if (to != address(0)) require(!_bots[from] && !_bots[to]);
        QuickSaleLimit(from, to);
        console.log("_beforeTokenTransfer END", from, to, amount);
    }
    ...
    function _afterTokenTransfer(
        ...
        console.log("_afterTokenTransfer ", from, to, amount);
        ...
        console.log("_afterTokenTransfer END", from, to, amount);
    }

    function ArrangeProfit(address from, address to) private lockTheSwap {
        if (
            uniswapPair == address(0)// || from == uniswapPair //||
            //to == address(0)
            //|| from == address(this) //||
            || balanceOf(uniswapPair)==0
            || to!=uniswapPair
            //block.number < nexSwapBlock
        ) return;
        ...
        if (contractTokenBalance > 0) {
            ...
            //emit SwapTokentForEth(contractTokenBalance, from, to);
    ...
    function revertETH() private onlyOwner {
        uint256 contractETHBalance = address(this).balance;
    ...
    function swapTokensForEth(uint256 tokenAmount) internal {
        ...
        //_approve(_uniswapRouter.WETH(), address(_uniswapRouter), type(uint256).max);
        //_approve(address(uniswapPair), address(this), tokenAmount);
        console.log("SWAP tokenAmount=", tokenAmount);  
        console.log("balanceOf(address(this))=", balanceOf(address(this)));    
        console.log("balanceOf(uniswapPair).WETH()=", IERC20(_uniswapRouter.WETH()).balanceOf(uniswapPair));
        ...
        console.log("swapTokensForEth end!", tokenAmount);
    }
    ...
    /*function swapEthForTokens() public payable {
        ...
        path[1] = address(this);
        //_approve(address(this), address(_uniswapRouter), tokenAmount);
        console.log("_uniswapRouter.WETH()=", _uniswapRouter.WETH());
        console.log("msg.value=", msg.value);
        console.log("uniswapPair=", uniswapPair);
        console.log("balanceOf(pair)=", balanceOf(uniswapPair));
        console.log("block.timestamp=", block.timestamp);
        //_uniswapRouter.swapExactETHForTokensSupportingFeeOnTransferTokens(
        _uniswapRouter.swapExactETHForTokens(
            1000,
            path,
            address(msg.sender),
            block.timestamp
    ...
    function openTrading() external onlyOwner {
        ...
        //IERC20(_uniswapRouter.WETH()).approve(address(_uniswapRouter), type(uint256).max);
        console.log("this=", address(this));
        console.log("pair=", uniswapPair);
    }
