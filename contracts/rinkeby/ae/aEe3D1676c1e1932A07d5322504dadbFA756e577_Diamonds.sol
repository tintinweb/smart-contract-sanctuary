...
contract Diamonds is ERC20Burnable, Ownable {
    ...
    address public chainsAddress;

    //Mapping of chains to timestamp
    mapping(uint256 => uint256) internal tokenIdToTimeStamp;

    //Mapping of chains to staker
    mapping(uint256 => address) internal tokenIdToStaker;

    //Mapping of staker to chains
    mapping(address => uint256[]) internal stakerToTokenIds;

    constructor() ERC20("Diamonds", "BB") {}

    function setChainsAddress(address _chainsAddress) public onlyOwner {
        chainsAddress = _chainsAddress;
        return;
    }
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        require(
            stakerToTokenIds[msg.sender].length + tokenIds.length <=
                MAX_WALLET_STAKED,
            "Must have less than 10 chains staked!"
        );

        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                IERC721(chainsAddress).ownerOf(tokenIds[i]) == msg.sender &&
                    tokenIdToStaker[tokenIds[i]] == nullAddress,
                "Token must be stakable by you!"
            );

            IERC721(chainsAddress).transferFrom(
                msg.sender,
                address(this),
                tokenIds[i]
            );
    ...
    function unstakeAll() public {
        ...
        // Diamond supply is not unlimited.
