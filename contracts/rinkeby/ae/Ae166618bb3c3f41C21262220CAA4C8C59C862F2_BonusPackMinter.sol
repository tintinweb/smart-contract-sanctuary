...
contract BonusPackMinter is Ownable, ContextMixin {
    mapping(uint256 => address) public allocatedBonusPacks;
    ...
    function allocateBonusPacks(uint256[] calldata tokenIds, address[] calldata addr) external onlyOwner {
        require(tokenIds.length == addr.length, "BonusPackMinter: Wrong data format");
        for (uint256 i = 0; i < addr.length; i++) {
            allocatedBonusPacks[tokenIds[i]] = addr[i];
        }
    }

    function redeemBonusPack(uint256 tokenId) public {
        require(isMinted[tokenId] == false, "BonusPackMinter: Already Minted");

        address minter = msg.sender;
        require(allocatedBonusPacks[tokenId] == minter, "BonusPackMinter: Not Allocated");
    ...
    function batchRedeemBonusPacks(uint256[] calldata tokenIds) public {
        uint256 tokenId;
        address minter = msg.sender;
