...
contract BearStaking is Ownable, IERC721Receiver{
  ...
  function calculateReward(uint256 _lastClaimed) view internal returns(uint256){
    return block.timestamp.sub(_lastClaimed).mul(rate).div(rewardPeroid); 
  }
  ...
  function claimReward(uint256 _tokenId) public {
    ...
    uint256 reward = calculateReward(bear.lastClaimed);
    bear.lastClaimed = block.timestamp;
    //transfer honey
  ...
  function getClaimableToken(address wallet) external view returns(uint256){
      ...
      for(uint256 i=0; i<stakedBears.length; i++){
          reward += calculateReward(stakedBear[stakedBears[i]].lastClaimed);
      }
      return reward;
  }
