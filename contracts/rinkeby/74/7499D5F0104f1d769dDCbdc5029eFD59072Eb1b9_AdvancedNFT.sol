...
contract AdvancedNFT is ERC721, VRFConsumerBase {
    ...
    mapping(bytes32 => uint256) public reqIdToTokenId;
    event RequestIdToTokenId(bytes32 reqId, uint256 tokenId);
        ...
        returns (uint256)
    {
        ...
        /* in this code, I mint an NFT before the randomness is returned.
			this may have a risk when the call back of vrf doesn't return => the new created nft will not have a class attribute.
			Should change to: Wait until the call back of vrf is trigged, 
								then mint the nft. This mechanic is demo at: 
									https://github.com/PatrickAlphaC/nft-demo/blob/main/contracts/AdvancedCollectible.sol
		*/
        bytes32 reqId = getRandomNumber();
        reqIdToTokenId[reqId] = newItemId;
        emit RequestIdToTokenId(reqId, newItemId);
        ...
        override
    {
        ...
        uint256 tokenId = reqIdToTokenId[requestId];
        tokenIdToClass[tokenId] = CLASS(classIdx);
        emit TokenIdToClass(tokenId, CLASS(classIdx));
    }
