...
contract InsuracePositionController is AbstractController {
    ...
    function stake(uint256 _amount) external override onlyIndex {
        require(canStake(), "InsuracePositionController: IPC1");
        ...
        uint256 investmentAmount = _stakingAssetAmount(stakingAsset);

        require(
            investmentAmount >= _stakingController.minStakeAmtPT(stakingAsset),
    ...
    function _stakingAssetAmount(address _stakingAsset) internal view returns(uint256 _amount){
        if(_stakingAsset != ETHER){
            _amount = IERC20(_stakingAsset).balanceOf(address(this));
    ...
    function outstandingRewards() public view override returns (uint256) {
        (, uint256 _totalInVesting) = rewardsInVesting();
        (_totalInVesting, ) = _applyFees(_totalInVesting);
        return rewardsInEarnings().add(_totalInVesting);
    }

    function rewardsInEarnings() public view returns (uint256 _rewards) {
        _rewards = _stakersPool.stkRewardsPerAPerLPT(lpToken, address(this));
    }

    function rewardsInVesting()
        public
        view
        returns (uint256 _withdrawableNow, uint256 _totalInVesting)
    {
        (_withdrawableNow, _totalInVesting) = _rewardController.getRewardInfo();
    }
        ...
        returns (uint256 _amount, uint256 _unstakeAvailable)
    {
        ...
        // amount to unstake
        _amount = ILPToken(lpToken).burnableAmtOf(address(this));
    }
