...
contract Expandables is ERC721Enumerable, ERC721Burnable, VRFConsumerBase, PaymentSplitter, Pausable, Ownable  {    
    ...
    bytes32 merkleRoot;
    uint256 whitelistOpen;
    uint256 whitelistClose;

    Reveal[] reveals;

    mapping(uint256 => uint8) oneOnes;
    mapping(address => uint256) claimedTokens;
    mapping(address => uint256) freeWhitelistedTokens;

    bytes32 internal immutable keyHash;
    uint256 internal immutable fee;

    string constant public TRAIT_LINK = "ipfs://423t4rt3f3ff34";
    ...
    function mint(uint256 amount) external payable whenNotPaused {
        require (block.timestamp > whitelistClose, "public sale not started");
    ...
    function freeMint(uint256 amount) external whenNotPaused {
        require(freeWhitelistedTokens[msg.sender] >= amount, "amount too high");

        freeWhitelistedTokens[msg.sender] -= amount;
    ...
    function enableFreeMint(address account, uint256 amount) public onlyOwner {
        freeWhitelistedTokens[account] += amount;
    }    
    ...
    function getClaimedPandas(address userAdress) external view returns (uint256) {
        return claimedTokens[userAdress];
    } 
