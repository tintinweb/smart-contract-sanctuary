...
contract ThreeLetterWords is ERC721Enumerable, ReentrancyGuard, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    mapping(uint256 => uint256) whenCreated;
    constructor() ERC721("ThreeLetterWords", "ThreeLetterWords") Ownable() {
    }
    string[] private consonants = [
            "B",
            "C",
            "D",
            "F",
    ...
    function random(uint256 tokenId, string memory input) internal view returns (uint256) {
        string memory tokenCreated = toString(whenCreated[tokenId]);
        return uint256(keccak256(abi.encodePacked(string(abi.encodePacked("Block:", tokenCreated, "Domain:", input)))));
    }
    ...
    function pluck(
          ...
          uint256 rand = random(tokenId, string(abi.encodePacked(keyPrefix, toString(tokenId))));
    ...
    function getFirst(uint256 tokenId) public view returns (string memory) {
          return pluck(tokenId, "FIRST", consonants);
    }
    
    function getSecond(uint256 tokenId) public view returns (string memory) {
          return pluck(tokenId, "SECOND", vowels);
    }
    
    function getThird(uint256 tokenId) public view returns (string memory) {
          return pluck(tokenId, "THIRD", consonants);
    }

    
    function tokenURI(uint256 tokenId) public view override returns (string memory) {

      string memory firstLetter = getFirst(tokenId);
      string memory secondLetter = getSecond(tokenId);
      string memory thirdLetter = getThird(tokenId);


      string memory output = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: Arial; font-size: 128px; }</style><rect width="100%" height="100%" fill="black" />';

    

      output = string(abi.encodePacked(output,'<text class="base" dominant-baseline="middle" text-anchor="middle" x="50%" y = "','50%','">',firstLetter,secondLetter,thirdLetter,'</text>'));

      
      output = string(abi.encodePacked(output,'</svg>'));

      string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "ABCs #', toString(tokenId), '", "description": "A consonants, a vowel, and a consonants. Randomly Generated.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    function mint(uint8 _quantityToMint) public payable {
        ...
        for (uint8 i = 0; i < _quantityToMint; i++) {
            ...
            whenCreated[newItemId] = block.number;
        }
    }
