...
contract SalesPolicy is ERC721, ISalesPolicy, ReentrancyGuard {
    ...
    modifier onlyFactory() {
        require(msg.sender == factory, "UnoRe: SalesPolicy Forbidden");
        _;
    }
    ...
    function buyPolicy(
        ...
        if (_premiumCurrency == address(0)) {
            ...
            if (msg.value > premiumPaid) {
                TransferHelper.safeTransferETH(msg.sender, msg.value - premiumPaid);
            }
            ...
            TransferHelper.safeTransferFrom(_premiumCurrency, msg.sender, address(this), premiumPaid);
            IPremiumPool(premiumPool).collectPremium(_premiumCurrency, premiumPaid);
        } else {
            premiumPaid = _policyPriceInUSDT;
            TransferHelper.safeTransferFrom(_premiumCurrency, msg.sender, address(this), _policyPriceInUSDT);
        ...
        policyIdx.increment();
        emit BuyPolicy(protocolIdx, lastIdx, msg.sender, _coverageAmount, _policyPriceInUSDT, _premiumCurrency, premiumPaid);
    }
