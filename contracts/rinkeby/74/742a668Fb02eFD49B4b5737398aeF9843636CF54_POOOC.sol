...
contract POOOC is ERC721EnumerableB, Ownable, PaymentSplitter, ReentrancyGuard {
  ...
  constructor(address[] memory _payees, uint256[] memory _shares)
    ERC721B("Proof of concept", "POOOC")
    PaymentSplitter(_payees, _shares) {
    _owners.push(address(0));
  }

  function mintPreSale(uint count, bytes memory signature) external payable nonReentrant() {
    require(isActive, "Must be active" );
    require(verifySignature(msg.sender, count, signature), "Signature verification failed");
    require(claims[msg.sender] + count <= maxClaimable, "You already minted the max amount");
  ...
  function verifySignature(address sender, uint256 count, bytes memory signature) private view returns(bool) {
    return keccak256(abi.encodePacked(sender, count))
  ...
  function setMaxClaimable(uint _maxClaimable) external onlyOwner {
