...
contract FB_v2 is ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function initialize(
        ...
        require(mintingAddress != address(0), "Zero address not allowed");
        _mintingAddress = mintingAddress;

        require(minter != address(0), "Zero address not allowed");
        _minter = minter;

        require(presaleLocked != address(0), "Zero address not allowed");
    ...
    function changeMintingAddress(address newAddress) external {
        ...
        emit MintingAddressChanged(newAddress);
    }
    ...
    /// @dev check transfers of presaleLocked address amount for available balance
    ...
    */
    ...
    /// @dev Emits when minting address changed
    event MintingAddressChanged(address indexed newAddress);
    /// @dev Emits when minter address changed
