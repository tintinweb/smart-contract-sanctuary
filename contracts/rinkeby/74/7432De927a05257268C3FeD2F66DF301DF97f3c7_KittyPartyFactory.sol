...
contract KittyPartyFactory {
    ...
    constructor(address _tomCatContract, address _accountantContract){
    ...
    returns (address kittyAddress) {
        address kitty = ClonesUpgradeable.clone(tomCatContract);
        ...
        KittyPartyController(kitty).transferOwnership(msg.sender);
        emit KittyLive(msg.sender, kitty);
        // console.log("%s is the owner",KittyPartyController(kitty).owner() );
        (bool success,) = address(accountantContract).call(abi.encodeWithSignature("setupMinter(address)", kitty));
        require(success, "Not able to set minter role!");
    ...
    function getMyKitties(address candidateAddress) external view returns(address[] memory){
     ...
     //mint new erc20 tokens when child requests to an array of addresses
     //These erc20 tokens represent a receipt in a liquid form, when there is a winner again this is minted and given winner
     //mint new erc721 tokens when child requests -> upon completion
...
enum KittyPartyStages {
    ...
    modifier timedTransitions() {
        if (stage == KittyPartyStages.InitialCollection && block.timestamp >= creationTime + 10 minutes) {
           nextStage();
           nextStage();// we do a double jump here
        }
        if (stage == KittyPartyStages.Collection && block.timestamp >= creationTime + 100 minutes) {
            nextStage();
        }
        if (stage == KittyPartyStages.Staking && block.timestamp >= creationTime + durationInDays * 100 minutes) {
            nextStage();
        }
        _;
    }
