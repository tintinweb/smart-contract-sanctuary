...
contract STO is Ownable {
    ...
    struct PropertyInfo {
        ...
        uint256 totalSupply;
        uint256 tokenToBeSold;
        uint256 tokenActuallySold;
    ...
    // a struct to keep track of users and incentives
    ...
    function create(
        uint256 _propertyValue,
        uint256 _noOfTokens,
        uint32 _percentageTobeSold, //10^3
        address _to,
        ...
        creatorCounter++;
        uint256 _tokenToBeSold = (
            (uint256(_percentageTobeSold).mul(_noOfTokens))
        ).div(10**5);
    ...
    function buy(uint256 _id, uint256 _usdtTokenAmount) external {
        ...
        require(
            propertyMapping[_id].tokenToBeSold >= _propertyTokenAmount,
        ...
        propertyMapping[_id].tokenToBeSold = propertyMapping[_id]
            .tokenToBeSold
            .sub(_propertyTokenAmount);
        propertyMapping[_id].tokenActuallySold = propertyMapping[_id]
            .tokenActuallySold
        ...
        onlyOwner
    {
        ...
        PropertyIncentive memory newIncentive = PropertyIncentive({
            ...
            tokenSoldAtTime: propertyMapping[_id].tokenActuallySold,
            timestamp: block.timestamp
        });
    ...
    function claimIncentive(uint256 _id) external {
        ...
        uint256 _incentiveAmount = _calculateIncentiveAmount(_id, claimCount);
    ...
    // a function to calculate the incentive token amount to be claimed
    function _calculateIncentiveAmount(uint256 _id, uint256 _claimCounter)
        public
        view
        returns (uint256)
    {
        return (
            (
                balanceOf(msg.sender, _id).mul(
                    propertyIncentiveMapping[_id][_claimCounter]
                        .tokenIncentiveAmount
                )
            ).div(propertyIncentiveMapping[_id][_claimCounter].tokenSoldAtTime)
        );
    }
