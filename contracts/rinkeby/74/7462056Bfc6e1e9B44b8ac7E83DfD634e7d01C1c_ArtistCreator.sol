...
contract ArtistCreator is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    mapping(bytes32 => address) private artists;
    ...
    event CreatedArtist(string name, address indexed artistAddress);
    ...
    function createArtist(string memory _name, bytes calldata signature) public {
        require((getSigner(signature) == admin), 'invalid authorization signature');
        require((getArtistByName(_name) == address(0)), 'artist already exists');

        BeaconProxy proxy = new BeaconProxy(
            beaconAddress,
            abi.encodeWithSelector(Artist(address(0)).initialize.selector, tx.origin, _name)
        );

        bytes32 nameHash = keccak256(abi.encodePacked(_name));
        artists[nameHash] = address(proxy);

        emit CreatedArtist(_name, address(proxy));
    }

    /// Get artist given the artist name
    /// @param _artistName name of artist
    /// @return Artist Edition NFT contract address
    function getArtistByName(string memory _artistName) public view returns (address) {
        bytes32 nameHash = keccak256(abi.encodePacked(_artistName));
        return artists[nameHash];
    }
...
contract Artist is ERC721Upgradeable, IERC2981Upgradeable, OwnableUpgradeable {
    ...
    // `nextTokenId` starts at 1 (set in constructor) and increments with each token purchased, globally across all editions.
    uint256 public nextTokenId;
    // Editions start at 1 (set in constructor), in order that unsold tokens don't map to the first edition.
    ...
    // Optional mapping for permanent token URIs
    mapping(uint256 => string) private _permanentURIs;
    ...
    */
    function initialize(address _owner, string memory _name) public initializer {
    ...
    function buyEdition(uint256 _editionId) external payable {
        ...
        tokenToEdition[nextTokenId] = _editionId;

        emit EditionPurchased(_editionId, nextTokenId, editions[_editionId].numSold, msg.sender);
    ...
    function setStartTime(uint256 _editionId, uint256 _startTime) public onlyOwner {
        editions[_editionId].startTime = _startTime;
    }

    function setEndTime(uint256 _editionId, uint256 _endTime) public onlyOwner {
        editions[_editionId].endTime = _endTime;
    }

    function setPermanentURI(uint256 _tokenId, string memory _tokenURI) public onlyOwner {
        string memory _permanentURI = _permanentURIs[_tokenId];
        require(bytes(_permanentURI).length == 0, 'Already set permanent URI');

        _permanentURIs[_tokenId] = _tokenURI;
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        // If set, return permanentURI
        string memory permanentURI = _permanentURIs[_tokenId];
        if (bytes(permanentURI).length > 0) {
            return permanentURI;
        }

        // Otherwise, return concatenated baseURI and tokenId
        ...
        string memory baseURI = editions[editionId].baseURI;
