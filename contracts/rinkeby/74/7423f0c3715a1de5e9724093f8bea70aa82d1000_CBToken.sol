...
contract CBToken is ERC20, AccessControl, Pausable {
    // CONFIG START

    uint256 denominator = 100;
    
    // TOKEN
    string tokenName = "CoinBay Token";
    string tokenSymbol = "CBT";
    ...
    address liquidityWallet = 0x75e3CF0E8454F0D5fdFF10F2847A7d825c124aE9;
    address router02 = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;

    // BUY TAX
    uint256 public devTaxBuy = 1;
    uint256 public marketingTaxBuy = 1;
    uint256 public liquidityTaxBuy = 1;

    // SELL TAX
    uint256 public devTaxSell = 2;
    uint256 public marketingTaxSell = 3;
    uint256 public liquidityTaxSell = 3;

    uint256 public maxTxAmount = 30_000 * 10**18 + 1;
    uint256 public maxWalletAmount = 100_000 * 10**18 + 1;
    ...
    function handleFees(address sender, address recipient, uint256 amount) internal returns (uint256 fee) {
        ...
        if(isBuy) {
            fees = amount * 10**18 / denominator * (devTaxBuy + marketingTaxBuy + liquidityTaxBuy) / 10**18;

            taxSum = devTaxBuy + marketingTaxBuy + liquidityTaxBuy;

            devAmount = fees * 10**18 / taxSum * devTaxBuy / 10**18;
            marketingAmount = fees * 10**18 / taxSum * marketingTaxBuy / 10**18;
            liquidityAmount = fees * 10**18 / taxSum * liquidityTaxBuy / 10**18;
            ...
            fees = amount * 10**18 / denominator * (devTaxSell + marketingTaxSell + liquidityTaxSell) / 10**18;

            taxSum = devTaxSell + marketingTaxSell + liquidityTaxSell;
            
            devAmount = fees * 10**18 / taxSum * devTaxSell / 10**18;
            marketingAmount = fees * 10**18 / taxSum * marketingTaxSell / 10**18;
            liquidityAmount = fees * 10**18 / taxSum * liquidityTaxSell / 10**18;
            ...
            if(feeTokens > 0) {
                ...
                uint256 liquidityETH = liquidityTokens / 2 * 10**18 / feeTokens * ethGained / 10**18;
                uint256 liquidityToken = liquidityTokens / 2;

                _UniswapV2Router02.addLiquidityETH{value: liquidityETH}(
                    address(this),
                    liquidityToken,
                    0,
                    0,
    ...
    function setTaxesBuy(uint256 dev, uint256 marketing, uint256 liquidity) external onlyRole(OWNER_ROLE) {
        devTaxBuy = dev;
        marketingTaxBuy = marketing;
        liquidityTaxBuy = liquidity;
    }

    function setTaxesSell(uint256 dev, uint256 marketing, uint256 liquidity) external onlyRole(OWNER_ROLE) {
        devTaxSell = dev;
        marketingTaxSell = marketing;
        liquidityTaxSell = liquidity;
    }
