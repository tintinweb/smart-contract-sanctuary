...
contract Creatures is ERC721URIStorage {
    ...
    function mint(
        ...
        _mint(to, tokenId);
        animals[tokenId] = Animal(AnimalType(_animalType), Rarity(_rarity), index, uint64(block.timestamp), uint64(block.number), "");
    }
    ...
    function getBirthblockAndRarity(uint256 _tokenId) external view returns(uint64, uint8) {
        return (animals[_tokenId].birthblock, uint8(animals[_tokenId].rarity));
    }
