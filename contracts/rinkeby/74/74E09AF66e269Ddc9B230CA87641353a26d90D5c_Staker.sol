...
contract Staker {
  ...
  modifier afterDeadline(){
    require( block.timestamp >= deadline, "Deadline not reached, cannot execute withdraw");
    _;
  }
  modifier thresholdReached(){
    require( address(this).balance >= threshold , "Minimum Threshold not reached");
    _;
  }
  ...
  function execute() public afterDeadline thresholdReached notCompleted
  {
  ...
  function withdraw(address who) public afterDeadline  thresholdNotReached notCompleted {

    uint256 toReturn = stakedAmountOf(who);
