...
contract LiquidMarketVoting is Ownable {
  ...
  address[] public votersReceived;

  uint256 private _votesYes;

  uint256 private _votingStart;

  uint private constant VOTING_PRICE = 100;
  ...
  function vote(uint _vote) public {
    ...
    require(_vote < 2, 'Voting value is incorrect');
    require(_votingStart + _votingPeriod > block.timestamp, 'Voting is ended');

    for (uint i = 0; i < votersReceived.length; i++) {
      require(votersReceived[i] != msg.sender, 'Voter already voted');
    }

    require(IERC20(_tokenAddress).transferFrom(_msgSender(), address(this), VOTING_PRICE), 'Transfer amount exceeds balance');
    IERC20Burnable(_tokenAddress).burn(VOTING_PRICE);
  ...
  function getVotesNo() public view returns (uint256) {
    return votersReceived.length - _votesYes;
  }
}