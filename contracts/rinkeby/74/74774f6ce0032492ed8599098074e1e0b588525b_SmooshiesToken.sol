 ...
 */
contract SmooshiesToken is Context, IERC20 {
    ...
    address private _smooshiesAddress;
    ...
    function lastClaim(uint256 tokenIndex) public view returns (uint256) {
        require(IMintedBeforeReveal(_smooshiesAddress).ownerOf(tokenIndex) != address(0), "Owner cannot be 0 address");
        require(tokenIndex < IMintedBeforeReveal(_smooshiesAddress).totalSupply(), "NFT at index has not been minted yet");
    ...
    function accumulated(uint256 tokenIndex) public view returns (uint256) {
        ...
        if (lastClaimed == emissionStart) {
            uint256 initialAllotment = IMintedBeforeReveal(_smooshiesAddress).isMintedBeforeReveal(tokenIndex) == true ? INITIAL_ALLOTMENT.mul(PRE_REVEAL_MULTIPLIER) : INITIAL_ALLOTMENT;
     ...
     */
    function setSmooshiesAddress(address smooshiesAddress) public {
        require(_smooshiesAddress == address(0), "Already set");
        
        _smooshiesAddress = smooshiesAddress;
    }
    ...
    function claim(uint256[] memory tokenIndices) public returns (uint256) {
        ...
        for (uint i = 0; i < tokenIndices.length; i++) {
            ...
            require(tokenIndices[i] < IMintedBeforeReveal(_smooshiesAddress).totalSupply(), "NFT at index has not been minted yet");
            ...
            require(IMintedBeforeReveal(_smooshiesAddress).ownerOf(tokenIndex) == msg.sender, "Sender is not the owner");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if (msg.sender != _smooshiesAddress) {
