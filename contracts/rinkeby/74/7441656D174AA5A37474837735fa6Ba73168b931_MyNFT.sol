...
contract MyNFT is MyBase, VRFConsumerBase {
  using SafeMath for uint256;
  event MyNFTRandomnessRequest(uint timestamp);
  event MyNFTRandomnessFulfil(bytes32 requestId, uint256 tokenId);
  event MyNFTReveal(uint timestamp);
  ...
  constructor(
    ...
    MyBase(_tokenName, _tokenSymbol, _privateSalePrice, _publicSalePrice, _maxAirdrop, _maxSupply, _limitMint)
  ...
  function requestChainlinkVRF() public onlyOwner {
    ...
    requestedVRF = true;
    emit MyNFTRandomnessRequest(now);
  }
  ...
  function fulfillRandomness(bytes32 _requestId, uint256 randomNumber)
    ...
    emit MyNFTRandomnessFulfil(_requestId, seed);
  }
...
contract MyBase is Ownable, ERC721 {
    using SafeMath for uint256;
