...
contract TokenBondDepository is Ownable {
  ...
  function deposit(
    ...
    uint256 profit = payout.sub(fee).sub(feePartner);
    ...
    // updated payout removing fees
    payout = payout.sub(fee).sub(feePartner);
  ...
  // 10 CRV 1-1
  // _value  = 10
  // 10 / ? => payout = what user get
  // miniunmumPrice = 1.53 => 10/ 1.53 => 6.53 LOB for 10 CRV

  // 10 CRV = 40$
  // bondPrice = 1.53 => bondPriceInUSD = 61$
