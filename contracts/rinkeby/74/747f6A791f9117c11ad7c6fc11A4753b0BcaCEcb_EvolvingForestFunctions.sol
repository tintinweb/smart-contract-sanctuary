...
contract EvolvingForestFunctions is AccessControl, ReentrancyGuard {
  ...
  constructor(address _nftContract, address _verifier, uint256 _maxMintsPerTx, uint256 _price) {
  ...
  function buy(uint256 _amount, bytes memory sig, uint256 maxMints) 
    external payable nonReentrant {

      require(_amount <= maxMintsPerTx, '_amount over max');
      uint256 _supplyLeft = (nftContract.totalSupply() + 1) - nftContract.nextTokenId();
      require(_supplyLeft > 0, 'supply exhausted');

      if (_amount > _supplyLeft) {
        _amount = _supplyLeft;
      }
