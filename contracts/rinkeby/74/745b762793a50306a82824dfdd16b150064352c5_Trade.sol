...
contract Trade {
    ...
    uint public perTokenPrice = 0.0001 ether;
    ...
    constructor (uint8 _buyerFee, uint8 _sellerFee, TransferProxy _transferProxy, IERC20 _token) {
    ...
    function buyAsset(Order memory order, Sign memory sign) payable public returns(bool) {
        if(order.directPay) {
            order.amount = ETHtoTokenConversion(order.amount, msg.value);
        }
    ...
    function ETHtoTokenConversion(uint amount, uint value) internal returns(uint){
        uint amt = amount.div(perTokenPrice);
        payable(owner).transfer(value);
        token.transferFrom(owner,msg.sender,amt);
        token.approve(address(transferProxy), amt);
        return amt;
    }
    
    function setPrice(uint256 value) public returns(bool) {
        perTokenPrice = value;
        emit tokenPriceUpdated(perTokenPrice);
        return true;
    }
}