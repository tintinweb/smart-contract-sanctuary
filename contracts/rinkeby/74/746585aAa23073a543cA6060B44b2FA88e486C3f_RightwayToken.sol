...
contract RightwayToken is ERC721Enumerable, Saleable, AccessControl {
  ...
  function addTokenContent( uint256[] calldata tokens, string calldata slug, string calldata contentLibraryArweaveHash, uint16 contentIndex, string calldata contentType) public onlyRole(CONTENT_ROLE) {
  ...
  function mint(address to, uint256 tokenId, uint16 attributesStart, uint16 attributesLength) public onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  function mintBatch(address to, uint256[] calldata tokenIds, uint16 attributesStart, uint16 attributesLength) public onlyRole(DEFAULT_ADMIN_ROLE) {
    for (uint idx = 0; idx < tokenIds.length; idx++) {
      mint(to, tokenIds[idx], attributesStart, attributesLength);
    }
  }

  function redeem(uint256 tokenId, uint64 timestamp, string memory memo) public onlyRole(REDEEM_ROLE) {
   ...
   */
  function _processSaleOffering(uint256 offeringId, address buyer, uint256 price) internal override {
...

{
  ...
  "libraries": {
    "/home/bart/git/InfinityTokens-contract/contracts/custom/RightwayMetadata.sol": {
      "RightwayMetadata": "0x503FE186d5E49B7289A85B62Ec3A8e24Ce585BDd"
    },
    "/home/bart/git/InfinityTokens-contract/contracts/custom/RightwayDecoder.sol": {
      "RightwayDecoder": "0x3356709adB81e106C60C60367ee1B8070A18ebE2"
    }
  },
  "outputSelection": {
    "*": {
