...
contract BitmindCrowdsale {
  ...
  /**
    ...
    function initialize(
        ...
        require(initial_multiplier > 0, "BitmindMsg: initial_multiplier must higher than 0");
        multiplier = initial_multiplier;
        
        require(initial_rate > 0, "BitmindMsg: initial_rate must higher than 0");
        rate = initial_rate;
        
        emit Initializing(Token, Pair, msg.sender, MinContribution, MaxContribution, StartTime, EndTime, initial_rate, initial_price, initial_multiplier);
        return true;
    }
    ...
    function getPrice() public view returns(uint256){
        ...
        for(uint256 i = 1; i <= multiplier; i++){
            if(block.timestamp >= start_time.add(phase.mul(i.sub(1))) && block.timestamp <= start_time.add(phase.mul(i))){
                return price.add(rate.mul(i.sub(1)));
            }
        }
    }
    ...
    function Purchase(uint256 amount) external onlyWhileOpen returns(bool){
        ...
        uint256 tokenReached = amount.div(getPrice()).mul(1e18);
        require(remainingToken() > 0 && remainingToken() >= tokenReached, "BitmindMsg: INSUFFICIENT BMD");
        ...
        totalSold = totalSold.add(tokenReached);
