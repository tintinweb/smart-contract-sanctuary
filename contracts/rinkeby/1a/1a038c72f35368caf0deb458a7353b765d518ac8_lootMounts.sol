...
contract lootMounts is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function pluck(
        ...
        if (keccak256(abi.encodePacked(keyPrefix)) == keccak256("MOUNT") && greatness >= 14 && greatness < 19) {
            output = string(abi.encodePacked(output, " ", suffixes[rand % suffixes.length]));
        }
        if (keccak256(abi.encodePacked(keyPrefix)) == keccak256("MOUNT") && greatness >= 19) {
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Mount #',
                        toString(tokenId),
                        '", "description": "Mounts are randomized generated and stored on chain. Images and other functionality are intentionally omitted for others to interpret. Feel free to use mounts in any way you want. Inspired and compatible with Loot (for Adventurers)", "image": "data:image/svg+xml;base64,',
    ...
    function multiMint(uint256[] memory tokenIds) public payable nonReentrant {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(tokenIds[i] > 8000 && tokenIds[i] < 9000, "Token ID invalid");
    ...
    constructor() ERC721("lootMounts", "LMOUNT") Ownable() {}
}
