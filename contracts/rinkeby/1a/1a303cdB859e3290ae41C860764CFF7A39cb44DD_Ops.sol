...
contract Ops is Ownable {
    ...
    function verify_application(uint256 _bank_id, uint256 _application_id, string memory _curr_date) public {
        ...
        id_to_application[_application_id].status = "Accepted";
    ...
    function reject_application(uint256 _bank_id, uint256 _application_id, string memory _reason , string memory _curr_date) public {
        require(bank_to_authorized_accounts[_bank_id][msg.sender] == true, "User is not authorized!");
        require(id_to_application[_application_id].application_completed == false, "Application already verified");
        id_to_application[_application_id].application_completed = true;
        id_to_application[_application_id].status = "Rejected";
        id_to_application[_application_id].verified_by = msg.sender;
        id_to_application[_application_id].verified_on = _curr_date;
        id_to_application[_application_id].rejection_reason = _reason;
    ...
    function edit_user(uint256 _user_id, address _account_address, string memory _fname, string memory _lname, uint256 _phone_number, string memory _birthdate, string memory _residential_address, string memory _ip_address, string memory _location_coordinates, string memory _curr_date, string[] memory _document_names, string[] memory _document_links) public{
        require(msg.sender == id_to_user[_user_id].account_address, "User is not authorized!");
        ...
        if(_document_names.length != 0){
            add_update_user_documents(_user_id, _document_names, _document_links);
        }
    }
    ...
    function get_universal_documents() view public returns(string[] memory){
        return universal_documents;
    }

    function get_additional_docs(uint256 _bank_id) view public returns(AdditionalDoc[] memory){
    ...
    function total_banks() view public returns(uint256){
        return all_banks.length;
    }

    function vote_status(uint256 _addDoc_id) view public returns(uint256[] memory){
