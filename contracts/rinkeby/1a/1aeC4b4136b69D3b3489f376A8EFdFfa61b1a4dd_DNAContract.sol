...
contract DNAContract is ERC721Enumerable, Ownable {
    ...
    */
    ...
    function StringReplace(string memory _string, uint256 _pos) internal pure returns (string memory) {
        ...
        for(uint i = 0; i < _stringBytes.length; i++) {
            ...
            if(_pos == 0) {
                result[0] = _stringBytes[1];
                result[1] = _stringBytes[0];
            }
            else if(_pos == 1) {
                result[2] = _stringBytes[3];
                result[3] = _stringBytes[2];
            }
            else if(_pos == 2) {
                result[4] = _stringBytes[5];
                result[5] = _stringBytes[4];
            }
        }
        return  string(result);
    }
    ...
    function GenerateLadder(uint256 tokenId) public view returns (string memory) {
        ...
        string[4] memory LadderColor = [
            StringReplace(DNAColors[neucleotideIdxArray[0]], 0),
            StringReplace(DNAColors[neucleotideIdxArray[0]], 1),
            StringReplace(DNAColors[neucleotideIdxArray[1]], 0),
            StringReplace(DNAColors[neucleotideIdxArray[1]], 1) 
    ...
    function EyeType(uint256 Idx) public view returns (string memory) {
        ...
        if (Idx == 0) {
            eyeString = string(abi.encodePacked(
                ...
                '</g>',
                '<g id="Right_eye">',   
                    '<ellipse cx="475" cy="400" rx="35" ry="50" style="fill:white;stroke:black;stroke-width:2" />',
