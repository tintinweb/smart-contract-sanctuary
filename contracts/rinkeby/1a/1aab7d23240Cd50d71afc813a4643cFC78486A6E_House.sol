...
contract House is IHouse, ERC721Enumerable, Ownable, Pausable {
    ...
    event ShakeMinted(uint256 indexed tokenId);
    event RanchMinted(uint256 indexed tokenId);
    event MansionMinted(uint256 indexed tokenId);

    event ShakeStolen(uint256 indexed tokenId);
    event RanchStolen(uint256 indexed tokenId);
    event MansionStolen(uint256 indexed tokenId);
    ...
    // 0 - 7 are associated with Shake, 8 - 15 are associated with Ranch, 16 - 23 are associated with Mansion
    ...
    function mint(address recipient, uint256 seed) external override whenNotPaused {
        ...
        if(tx.origin != recipient && recipient != address(tower)) {
            ...
            if(tokenTraits[minted].roll == 0) {
                emit ShakeStolen(minted);
            } else if(tokenTraits[minted].roll == 1) {
                emit RanchStolen(minted);
            } else {
                emit MansionStolen(minted);
            }
        }
    ...
    */
    function generate(uint256 tokenId, uint256 seed, LastWrite memory lw) internal returns (HouseStruct memory t) {
        t = selectTraits(seed);
        t.roll = 0;
        if (existingCombinations[structToHash(t)] == 0) {
            ...
            if(t.roll == 0) {
                emit ShakeMinted(tokenId);
            } else if(t.roll == 1) {
                emit RanchMinted(tokenId);
            } else {
                emit MansionMinted(tokenId);
            }
            return t;
        }
