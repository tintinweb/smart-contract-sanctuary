...
// -----------------------------------------------------------------------------------------------------
// ETHSwapAgentLogic Contract
//
//
// This contract is modified from the ETHSwapAgentImpl contract in Open Source Ethereum and Binance 
// Smart Chain Bi-directional bridge Solution by BINANCE CHAIN
// (https://github.com/binance-chain/eth-bsc-swap-contracts/blob/master/contracts/ETHSwapAgentImpl.sol)
//                        
// July 15 2021                                  
// -----------------------------------------------------------------------------------------------------

contract ETHSwapAgentLogic is OwnableUpgradeable {
    ...
    function __ETHSwapAgentLogic_init(uint256 fee) external initializer {
    ...
    function fillBSC2ETHSwap(bytes32 bscTxHash, address erc20Addr, address toAddress, uint256 amount) onlyOwner external returns (bool) {
        ...
        IERC20Upgradeable(erc20Addr).safeTransfer(toAddress, amount);
    ...
    function swapETH2BSC(address erc20Addr, uint256 amount) payable external notContract returns (bool) {
        ...
        IERC20Upgradeable(erc20Addr).safeTransferFrom(msg.sender, address(this), amount);
        if (msg.value != 0) {
