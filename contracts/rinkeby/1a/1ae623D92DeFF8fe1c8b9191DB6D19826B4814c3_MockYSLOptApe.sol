...
contract YSLOptApe is YSLOpt {
    ...
    function transferOut(
        ...
        IStrategySwap(stratSwap).swapLPToBusd(fee, apeSwap, BUSD, path0, path1, lpToken);
        ...
        IStrategySwap(stratSwap).swapLPToBusd(_amount, apeSwap, BUSD, path0, path1, lpToken);
    ...
    function transferIn(address _user, uint256 _amount) public stratOnly returns (uint256) {
        ...
        IStrategySwap(stratSwap).swapBusdToLP(_amount, apeSwap, BUSD, path0, path1, lpToken);
    ...
    function collectOptimisationTax(uint256 _amount) internal override returns (uint256 summary) {
        ...
        IStrategySwap(stratSwap).swapLPToBusd(tax, apeSwap, BUSD, path0, path1, lpToken);
        uint256 amountUSDAfter = IERC20(BUSD).balanceOf(address(this));
    ...
    function compound() external {
        ...
        IERC20(BANANA).approve(apeSwap, IERC20(BANANA).balanceOf(address(this)));
    ...
    function earn(
        ...
        uint256 usersCompUSD = IStrategySwap(stratSwap).swapLPToBusd(usersComp, apeSwap, BUSD, path0, path1, lpToken);
    ...
    function getUserDepositedUSD(address _user) public view returns (uint256 amount) {
...
import "../YSLfarm/YSLOptApe.sol";

contract MockYSLOptApe is YSLOptApe {
    constructor(
        address _adapter,
        uint256 _apeId,
        address[] memory _path0,
        ...
        YSLOptApe(_adapter, _apeId, _path0, _path1, _team, _lpToken, _feeAddress, _controllerFee, _referral, _stratSwap)
    {}
