...
contract AXL_test is ERC721, ERC721Enumerable, Ownable, ERC721Burnable {
    ...
    //constructor(string memory baseURI, string memory _notrevealURI) ERC721("AXLNFTs", "AXL") { 
    constructor(string memory baseURI, string memory _notrevealURI) ERC721("AXL_test", "AXL_TEST") { 
    ...
    function internalMint(address to) internal {
        ...
        uint rand = uint(keccak256(abi.encodePacked(block.timestamp, msg.value)));
        rand = rand % randomArray.length;

        _tokenIdCounter.increment();
        _safeMint(to, randomArray[rand]);

        for(uint256 i = rand; i < randomArray.length; i ++)
        {
