...
contract SecretSocietyNFT is ERC721, Ownable {
	...
	string public baseURL = "http://api.secretsocietynft.com/collection/";
	...
	function mintNFT(address recipient) public payable returns(uint256) {
		require(msg.value == REGULAR_MINT_COST, "SecretSocietyNFT: Invalid mint fee provided.");
		...
		_setTokenURI(newItemId, strConcat(baseURL, uint2str(newItemId)));
		return newItemId;
	}
	...
	function reserveGiveaway(uint256 numTokens, address recipient) public onlyOwner {
		...
		for(index = 1; index <= numTokens; index++) {
			...
			_setTokenURI(currSupply + index, strConcat(baseURL, uint2str(currSupply + index)));
		}
	}
