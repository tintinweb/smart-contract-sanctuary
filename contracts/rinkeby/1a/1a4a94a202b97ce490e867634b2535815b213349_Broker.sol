...
contract Broker is ERC721Holder, Storage {
    ...
    function buy(
        ...
        address payable creator = Token.creators(tokenID);
        uint256 royalitiy = (royalities *
            auctions[_mintableToken][tokenID].buyPrice) / 10000;
        ...
        if (auctions[_mintableToken][tokenID].erc20Token == address(0)) {
            ...
            require(
                erc20Token.allowance(msg.sender, address(this)) >=  auctions[_mintableToken][tokenID].buyPrice,
    ...
    function withdrawERC20(uint256 tokenID, address _mintableToken)
        public
        onlyOwner
    {
        IERC20 erc20Token = IERC20(
