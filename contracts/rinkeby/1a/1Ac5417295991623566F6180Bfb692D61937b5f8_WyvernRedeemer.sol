...
import "./external/interfaces/IWyvernExchange.sol";
import {IBounty, IBountyRedeemer} from "./AbstractBounty.sol";

contract WyvernRedeemer is IBountyRedeemer, ReentrancyGuard {
    address immutable wyvernExchange;

    constructor(address _wyvernExchange) {
        wyvernExchange = _wyvernExchange;
    }

    function onRedeemBounty(
        address, /* _initiator */
        bytes calldata _data
        ...
        IWyvernExchange(wyvernExchange).atomicMatch_{value: msg.value}(
            addrs,
            uints,
