...
contract GeishaTeahouse is ERC721Enumerable, ERC721Pausable, Ownable, ERC721Burnable {
    ...
    uint256 private redeemSupply = 0;
    bool public redeemOpen = false;
    mapping(address => uint256) public addressMintedBalance;
    mapping(address => uint256) public addressRedeemBalance;
    bytes32 public root;
    ...
    function setRedeem(bool _redeemOpen) public onlyOwner {
    ...
    function buy(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable whenNotPaused {
        ...
        if (sale == Sale.WHITELIST) {
            ...
            uint256 ownerRedeemCount = addressRedeemBalance[msg.sender];
            require(
                ownerMintedCount + ownerRedeemCount + _mintAmount <= maxMintWhitelist,
                "max mint amount exceeded"
            );
        }
        require(msg.value >= actualCost * _mintAmount, "insufficient funds");
        for (uint256 i = 1; i <= _mintAmount; i++) {
            addressRedeemBalance[msg.sender]++;
            redeemSupply++;
        }
    }
    ...
    function redeem() public whenNotPaused {
        ...
        uint256 ownerMintedCount = addressRedeemBalance[msg.sender];
        require(ownerMintedCount > 0, "no balance found");
        require(redeemOpen == true, "redeem closed");
