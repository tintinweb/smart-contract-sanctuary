...
contract Project is IProject {
        ...
        nonReentrant
    {
        ...
        if (msgSender() == builder) {
            if (currency == homeFi.etherCurrency()) {
                ...
                IToken20 _token = IToken20(currency);
                _token.safeTransferFrom(msgSender(), address(this), _cost);
            }
        }
        ...
        contractorAccepted
    {
        ...
        for (uint256 i = 0; i < _hash1.length; i++) {
            ...
            tasks[_taskCount].initialize(_hashArray, _taskCosts[i]);
    ...
    function recoverTokens(address _tokenAddress) external override {
        ...
        if (_tokenAddress == homeFi.etherCurrency()) {
            uint256 _leftOutEther = address(this).balance;
            if (_leftOutEther > 0) {
                payable(builder).transfer(_leftOutEther);
            }
        } else {
    ...
    function autoWithdraw(uint256 _amount) internal override {
        ...
        if (currency == homeFi.etherCurrency()) {
            ...
            IToken20 _token = IToken20(currency);
            _token.safeTransfer(builder, _amount);
        }
    ...
    function payFee(address _recipient, uint256 _amount) internal override {
        ...
        if (currency == homeFi.etherCurrency()) {
            ...
            IToken20 _token = IToken20(currency);
            _token.safeTransfer(_treasury, _builderFee);
...

{
  ...
  "libraries": {
    "contracts/libraries/Tasks.sol": {
      "Tasks": "0x8f2c8b06649a7f1786e364d04b830c69913e88dc"
    }
  }
}