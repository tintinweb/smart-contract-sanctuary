...
contract EventStore is AccessControl {
  ...
  function isEventOpen(address _nft) external view returns(bool) {
    return !events[_nft].paused && events[_nft].startTime <= block.timestamp && events[_nft].endTime >= block.timestamp;
  }

  function isEventClosed(address _nft) external view returns(bool) {
    return !events[_nft].paused && events[_nft].endTime <= block.timestamp;
  }
