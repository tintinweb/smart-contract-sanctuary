...
contract MyFactory is FactoryERC721, Ownable {
    ...
    /*
     * Three different options for minting Tickets (silver, gold and platinum).
     */
    uint256 NUM_OPTIONS = 1;
    uint256 DEFAULT_OPTION = 0;

    event TryMint(address from);
    ...
    function mint(uint256 optionId_, address to_) override public {
        emit TryMint(_msgSender());
        // Must be sent from the owner proxy or owner.
        assert(_msgSender() == proxyRegistryAddress || _msgSender() == owner());
        require(canMint(optionId_));

        MyToken token = MyToken(nftAddress);
        token.safeMint(to_);
    }
    ...
    function canMint(uint256 optionId_) override public view returns (bool) {
        ...
        return currentSupply + 1 <= TOTAL_SUPPLY;
    }
    ...
    function isApprovedForAll(address _owner, address _operator) public view returns (bool) {
        ...
        if (owner() == _owner && _operator == proxyRegistryAddress) {
            return true;
        }
