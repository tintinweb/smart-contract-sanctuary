...
contract Rates is IRates, Governed, Time {
    ...
    uint64 public minTimeBetweenUpdates = (23 hours) + (30 minutes);
    ...
    IUniswapV3Pool[] public referencePools;
    ...
    function completeSetup() external override onlyGovernor {
        referencePools = governor.getReferencePools();
    }

    
    
    function update() external lockProtocol runnable {
        RateData memory _currentRateData = currentRateData;
        require(_currentRateData.nextUpdateTime <= _currentTime(), 'Update not ready');

        IPrices prices = governor.prices();
        uint countPools = referencePools.length;
        uint[] memory _prices = new uint[](countPools);

        for(uint i = 0; i < countPools; i++) {
            
            _prices[i] = prices.systemObtainReferencePrice(referencePools[i]);
        }

        
        uint calculatedPrice = _calculatePrice(_prices, validRangeForRawPrices);

        
        RateData memory newRateData = RateData({
            rate: _calculateRate(_currentRateData.rate, interestRateParameters, calculatedPrice),
            rewardCount: _calculateRewardCount(_currentRateData, _currentTime()).toUint128(),
            nextUpdateTime: _futureTime(minTimeBetweenUpdates)
        });
        ...
        governor.mintTCP(msg.sender, newRateData.rewardCount);

        emit RateUpdated(newRateData.rate, calculatedPrice, newRateData.rewardCount, newRateData.nextUpdateTime);
    }

    
    
    function predictInterestRate() external view returns (uint price, int rate) {
        IPrices prices = governor.prices();
        uint countPools = referencePools.length;
        uint[] memory _prices = new uint[](countPools);

        for(uint i = 0; i < countPools; i++) {
            _prices[i] = prices.calculateTwappedPrice(referencePools[i], true);
        }

        price = _calculatePrice(_prices, validRangeForRawPrices);
        rate = _calculateRate(currentRateData.rate, interestRateParameters, price);

        return (price, rate);
    }

    
    function getRewardCount() external view returns (uint) {
        return _calculateRewardCount(currentRateData, _currentTime());
    }
    ...
    function _calculateRewardCount(
        RateData memory rd,
        uint64 currentTime
    ) internal pure returns (uint) {
        if (currentTime <= rd.nextUpdateTime) return 0;
        uint halfReward = rd.rewardCount / 2;
        uint rewardCount = halfReward + halfReward.mulDiv(currentTime - rd.nextUpdateTime, 30 minutes);

        return rewardCount.min(rd.rewardCount.mul(5));
    }
    ...
    function _calculatePrice(
        ...
        return TCPSafeMath.ONE._div(_calculatePriceImpl(_prices, _validRangeForRawPrices));
    }
    ...
    function _calculatePriceImpl(
        ...
        uint minPrice = TCPSafeMath.ONE.sub(_validRangeForRawPrices);
        uint maxPrice = TCPSafeMath.ONE.add(_validRangeForRawPrices);

        uint _countPrices = _prices.length;
        require(_countPrices > 0);

        uint[] memory validPrices = new uint[](_countPrices);
        uint countValidPrices;
        uint j;
        uint _price;
        
        for(uint i = 0; i < _countPrices; i++) {
            _price = _prices[i];
            if (minPrice < _price && _price < maxPrice) {
                countValidPrices++;
                validPrices[j++] = _price;
            }
        }

        
        if (countValidPrices == 0) {
            countValidPrices = _prices.length;
            validPrices = _prices;
        }
        ...
        while(countValidPrices > 2) {
            
            min = validPrices[0];
            max = validPrices[1];
            for(uint i = 1; i < countValidPrices; i++) {
                _price = validPrices[i];
                if (_price < min) min = _price;
                if (_price > max) max = _price;
            }
            
            if (min == max) return min;

            
            uint[] memory newValidPrices = new uint[](countValidPrices - 2);
            j = 0;
            bool maxRemoved;
            bool minRemoved;
            for(uint i = 0; i < countValidPrices; i++) {
                _price = validPrices[i];
                if (_price == max && !maxRemoved) maxRemoved = true;
                else if (_price == min && !minRemoved) minRemoved = true;
                else newValidPrices[j++] = _price;
            }
            countValidPrices -= 2;
            validPrices = newValidPrices;
        }

        
        if (countValidPrices == 1) return validPrices[0];
        else return validPrices[0].average(validPrices[1]);
    }
    ...
    function _calculateRate(
        ...
        bool tooLow = price < TCPSafeMath.ONE;
        uint error = tooLow ? TCPSafeMath.ONE - price : price - TCPSafeMath.ONE;

        
        if (error >= rp.acceptableError) {
            
            uint stepsOff = (rp.maxSteps.min((error / rp.errorInterval) + 1)).toUint64();

            
            int adjustmentAmount = rp.interestRateStep.mul(stepsOff).toInt256();

            
            rate = tooLow
                ? rate.add(adjustmentAmount)
                : rate.sub(adjustmentAmount);
        }

        
        if (rate < rp.minRate) rate = rp.minRate;
        if (rate > rp.maxRate) rate = rp.maxRate;
    ...
    function setErrorInterval(uint128 interval) external onlyGovernor {
        require(0 < interval && interval < TCPSafeMath.ONE);
    ...
    function setMinRate(int128 min) external onlyGovernor {
        require(min < interestRateParameters.maxRate);
    ...
    function setMaxRate(int128 max) external onlyGovernor {
        require(interestRateParameters.minRate < max);
    ...
    function setValidRangeForRawPrices(uint128 range) external onlyGovernor {
        ...
        require(validRangeForRawPrices < TCPSafeMath.ONE);
    ...
    function setMinTimeBetweenUpdates(uint64 time) external onlyGovernor {
        ...
        require(minTimeBetweenUpdates > 0);
