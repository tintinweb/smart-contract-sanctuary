...
contract Origins is ERC721Enumerable, ReentrancyGuard, Ownable {
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    parts[9] = getColor(tokenId);
    parts[10] = '">';
    parts[11] = getRealm(tokenId);
    parts[12] = "</tspan></text>";
    parts[13] = "</g></svg>";
    ...
    string memory atrrOutput = makeAttributeParts(getCity(tokenId), getProvince(tokenId), getRealm(tokenId));
    string memory json =
      _encode(
        bytes(
          string(
