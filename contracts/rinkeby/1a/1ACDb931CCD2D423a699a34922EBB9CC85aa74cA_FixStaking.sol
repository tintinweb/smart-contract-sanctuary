...
contract FixStaking is AccessControl, Pausable {
    ...
    {
        require(_minStake > 0, "setMinMaxStake: minumum amount cannot be ZERO");
        require(
            _maxStake > _minStake,
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            ...
            "withdrawERC20: You can't withdraw reward token"
        );
        require(
        ...
        isPoolEmpty(_poolIndex)
    {
        require(
            ...
            "withdrawReserve: transfer failed."
        );

        pools[_poolIndex].reserve = 0;
        emit ReserveWithdrawed(_poolIndex);
    }
        ...
        isPoolExist(_poolIndex)
    {
        ...
        require(
            pool.reserve >=
                calculateRew(totStakedAmount, pool.apy, pool.duration),
        ...
        isFinished(msg.sender, _stakerIndex)
    {
        ...
        uint256 reward = calculateRew(staker.amount, duration, pool.apy);
        ...
        require(
            pool.reserve >= totalReward,
            "withdraw: This pool has no enough reward reserve."
        );
     ...
     * @param _amount address of the person whose reward will be calculated.
     * @param _apy user index to be calculated.
     * @param _duration user index to be calculated.
    ...
    function calculateRew(
        ...
        // uint256 closedTime = getClosedTime(_staker, _stakerIndex);
        // uint256 endTime = block.timestamp > closedTime
        //     ? closedTime
        //     : block.timestamp;
    ...
    {
        StakerInfo memory staker = stakers[_staker][_stakerIndex];
        PoolInfo memory pool = pools[_stakerIndex];

        closedTime = getClosedTime(_staker, _stakerIndex);
        uint256 duration = _getStakerDuration(closedTime, staker.startTime);
        reward = calculateRew(staker.amount, duration, pool.apy);
