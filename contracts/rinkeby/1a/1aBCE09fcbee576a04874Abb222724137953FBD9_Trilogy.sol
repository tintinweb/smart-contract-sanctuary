...
contract Trilogy is ERC721TradableTrilogy {
    ...
    function check_trilogy(uint256 trilogy_id) public {
        ...
        while (verified_julias < 2) {
            ...
            if      (owned<250*5*1 ) owned_trilogy = uint256(ITrilogyFinder(FINDER_1_ADDRESS).get_trilogy_id(owned%(250*5)));
            else if (owned<250*5*2 ) owned_trilogy = uint256(ITrilogyFinder(FINDER_2_ADDRESS).get_trilogy_id(owned%(250*5)));
            else if (owned<250*5*3 ) owned_trilogy = uint256(ITrilogyFinder(FINDER_3_ADDRESS).get_trilogy_id(owned%(250*5)));
            else if (owned<250*5*4 ) owned_trilogy = uint256(ITrilogyFinder(FINDER_4_ADDRESS).get_trilogy_id(owned%(250*5)));
