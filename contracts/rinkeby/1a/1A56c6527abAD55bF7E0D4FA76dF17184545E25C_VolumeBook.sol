...
contract VolumeBook is Ownable, IVolumeBook {
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;
    EnumerableSet.AddressSet private _whitelist; // Only lp token in white list will record volume
    EnumerableSet.AddressSet private _intermediator; // token in intermedator will be used for quote 
    


    address public router; // Use for add volume only
    address public updater; // Use for update cycleNum only
    address public factory; // Use for volume calculation (price quote) only, inital together with oracle
    address public oracle; // Use for volume calculation quote (price quote) only, inital together with oracle
    address public anchorToken; // Use for volume calculation base value

    ILottery public lottery; //Use for lottery to check if anybody win
    
    uint256 constant public PERIOD = 1 days;

    uint256 public override lastUpdateTime; // Latest update cycle time;

    uint256 public override currentCycleNum; //Latest Cycle Number


    mapping (uint256 => mapping (address => uint256)) public cycleUserTradeVolume; // User trade volume in one cycle
    mapping (uint256 => uint256) public cycleTotalTradeVolume; // Total Trade Volume in a certain cycle


    event AddVolume(address indexed user, uint256 cyleNum, uint256 volume);
    ...
    constructor(
        ...
        require(_router != address(0), "VolumeBook: zero address");
        require(_oracle != address(0), "VolumeBook: zero address");
        require(_anchorToken != address(0), "VolumeBook: zero address");
        ...
        require(factory != address(0), "VolumeBook: zero address");
    ...
    function setRouter(address _newAddress) public onlyOwner {
        require(_newAddress != address(0), "VolumeBook: zero address");
        router = _newAddress;
    }

    function setUpdater(address _newAddress) public onlyOwner {
        require(_newAddress != address(0), "VolumeBook: zero address");
        updater = _newAddress;
    }
    ...
    function getWhitelistLength() public override view returns (uint256) {
    ...
    function getWhitelist(uint256 _index) public override view returns (address){
    ...
    function getIntermediator(uint256 _index) public view returns (address){
        require(_index <= getIntermediatorLength() - 1, "index out of bounds");
    ...
    function getQuantity(address outputToken, uint256 outputAmount) public view returns (uint256) {
        ...
        if (outputToken == anchorToken) {
            ...
            // console.log("directly quote '%s'", quantity);
        } else {
    ...
    function getUserVolume(address user, uint256 cycleNum) public override view returns (uint256) {
        return cycleUserTradeVolume[cycleNum][user];
    }

    function getTotalTradeVolume(uint256 cycleNum) public override view returns (uint256) {
        return cycleTotalTradeVolume[cycleNum];
    }

    function canUpdate() public override view returns(bool) {
       if (block.timestamp >= lastUpdateTime.add(PERIOD)) {
           return true;
       } 
       return false;
    }
    // only updater
    function addCycleNum() override external {
        require(msg.sender == updater, "VolumeBook: Only updater can update cycleNum");
        require(canUpdate(), "VolumeBook: update too early");
        currentCycleNum = currentCycleNum + 1;
        lastUpdateTime = block.timestamp;
    }

    function _addVolume(address user, uint256 volume) internal {
        cycleUserTradeVolume[currentCycleNum][user]
            = cycleUserTradeVolume[currentCycleNum][user].add(volume);
        cycleTotalTradeVolume[currentCycleNum] = cycleTotalTradeVolume[currentCycleNum].add(volume);
        if (address(lottery) != address(0)) {
            lottery.setSlot(user);
        }
        // console.log("Add volume '%s',  '%s'", user, volume);
        emit AddVolume(user, currentCycleNum, volume);
    }

    // only router
    function addVolume(address user, address input, address output, uint256 amount) external override returns (bool) {
            require(msg.sender == router, "VolumeBook: Only Router can change user volume");
            
            require(user != address(0), "VolumeBook: Taker swap account is the zero address");
            require(input != address(0), "VolumeBook: Taker swap input is the zero address");
            require(output != address(0), "VolumeBook: Taker swap output is the zero address");
            // check if the pair lp token is in whitelist
            if (!isWhitelist(ICCFactory(factory).getPair(input, output))) {
                return false;
            }
