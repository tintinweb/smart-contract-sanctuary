...
contract AlphaWolves is AlphaWolvesERC721 {

    modifier alphaWolfOwner(uint256 wolfId) {
        require(ownerOf(wolfId) == msg.sender, "Cannot interact with a AlphaWolves you do not own");
        _;
    }
    ...
    /**
     * @dev Keeps track of the state of betaWolf
     * 0 - Unminted
     * 1 - Fusion
     * 2 - Revealed
     */
    mapping(uint256 => uint256) public betaWolf;

    event BetaWolfClone(uint256 wolfId, uint256 parent1, uint256 parent2);
    event BetaWolfRevealed(uint256 wolfId);

    constructor(string memory name, string memory symbol, uint256 supply, uint256 genCount) AlphaWolvesERC721(name, symbol, supply, genCount) {}

    function clone(uint256 parent1, uint256 parent2) external alphaWolfOwner(parent1) alphaWolfOwner(parent2) {
        uint256 supply = totalSupply();
        require(supply < maxSupply,                               "Cannot clone any more beta wolves");
        require(parent1 < maxGenCount && parent2 < maxGenCount,   "Cannot clone with beta wolves");
        require(parent1 != parent2,                               "Must select two unique wolves");

        RWaste.burn(msg.sender, FUSION_PRICE);
        uint256 wolfId = maxGenCount + babyCount;
        betaWolf[wolfId] = 1;
        babyCount++;
        _safeMint(msg.sender, wolfId);
        emit BetaWolfClone(wolfId, parent1, parent2);
    }

    /**
     * @dev Keeps track of the state of betaWolves
     * 0 - Unminted
     * 1 - Fusion
     * 2 - Revealed
     */
    function reveal(uint256 wolfId) external alphaWolfOwner(wolfId) { 
        betaWolf[wolfId] = 2;
        emit BetaWolfRevealed(wolfId);
    }
...
contract AlphaWolvesERC721 is ERC721Enumerable, Ownable {
    ...
    function mintPresale(uint256 numberOfMints) public payable {
        ...
        require(supply.add(numberOfMints) <= maxGenCount,   "Purchase would exceed max supply of Genesis AlphaWolves");
