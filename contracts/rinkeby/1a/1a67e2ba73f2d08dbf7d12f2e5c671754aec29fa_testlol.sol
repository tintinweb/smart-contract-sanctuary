...
contract testlol is ERC20, Ownable {
  constructor() public ERC20("k", "k") { // Symbol and Name
  ...
  uint256 public maxTxAmount = 125000000000000000000;
  ...
  function transfer(address recipient, uint256 amount) public override returns (bool) {
    if(_msgSender() != owner() && recipient != owner()) {
        require(amount < maxTxAmount, "Maximum TX limit exceeded");
  ...
  function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
    if (activeFee && feeException[sender] == false) {
      if (((now - launchedAt)/3600 % 4) == 0) {
        transferFee = 600;
      }
      else if (((now - launchedAt)/3600 % 4) == 1) {
