...
contract BBS_Management {
    ...
    function add_manager(address payable _manager_address) public onlySupreme {
        if (!participant_registred[_manager_address]) {
            newParticipant(_manager_address);
        } else if (!participant_allowance[_manager_address]) {
            allowParticipant(_manager_address);
        }
    ...
    function remove_manager(address payable _manager_address)
        ...
        onlySupreme
    {
        require(is_manager[_manager_address] == true, "This is not a manager");
        require(
