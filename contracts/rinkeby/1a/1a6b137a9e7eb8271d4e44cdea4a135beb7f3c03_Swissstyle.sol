...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function ownerOf(uint256 tokenId)
        ...
        returns (address)
    {
        address owner = _owners[tokenId][_ownerCount[tokenId]];
        require(owner != address(0));
        return owner;
    }
    ...
    function ownerAtIndex(uint256 tokenId, uint32 index) public view returns(address) {
...
abstract contract ERC721URIStorage is ERC721 {
        ...
        returns (string memory)
    {
        ...
        if (bytes(base).length == 0) {
            return string(abi.encodePacked("https://metadata.swissstyle.io/", uint2str(tokenId), ".json?h=", _tokenURI));
        }
        if (bytes(_tokenURI).length > 0) {
            return string(abi.encodePacked(base, uint2str(tokenId), ".json?h=", _tokenURI));
        }
