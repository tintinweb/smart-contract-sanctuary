...
contract DefinaCard721 is ERC721Enumerable, ERC721URIStorage, Initializable {
    ...
    //minters[newMinter_][cardId_] = amount_;   限制某个人的cardID 还能mint多少数量？
    ...
    // cardId => cardInfo
    //cardId 对应的明细数据，包含了卡牌的名字，卡牌的id，卡牌的阵营，卡牌的稀有度，卡牌的最大数量，卡牌的图片地址
    mapping(uint => CardInfo) public cardInfoes;
    //保存卡牌的cardId集合
    EnumerableSet.UintSet private cardIds;

    // token => cardId
    //保存token ID对应的cardId 集合
    ...
    //创建卡牌，包含卡牌的名字，卡牌的id，卡牌的阵营，卡牌的稀有度，卡牌的最大数量，卡牌的图片地址  
    ...
    function mint(uint cardId_) public returns (uint256) {
        ...
        emit Mint(cardId_);
    //tokenID对应的cardId,tokenID是唯一的，自增的
        cardIdMap[tokenId] = cardId_;
        //mint给盲盒地址，然后盲盒在转给mint的地址（即持有者）
