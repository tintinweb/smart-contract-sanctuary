...
contract Uniswap  {
    ...
    event UniswapPoolCreated(address pool);

    constructor(address _positionManager) {
        positionManager = INonfungiblePositionManager(_positionManager);
    }

    function createPool(address _tokenA, address _tokenB, uint160 _sqrtPriceX96) external {
        address uniswapPool = positionManager.createAndInitializePoolIfNecessary(_tokenA, _tokenB, 3000, _sqrtPriceX96);
        pool = IUniswapV3Pool(uniswapPool);

        emit UniswapPoolCreated(uniswapPool);
    }

    function addLiquidity(uint256 _amount0, uint256 _amount1) external payable returns(uint256 tokenId, uint128 liquidity, uint256 amount0, uint256 amount1) {
