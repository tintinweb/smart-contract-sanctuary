...
contract AC_ICO is Ownable {
  using SafeMath for uint256;
  Token token;

  uint256 public constant RATE = 1; // Number of tokens per Ether
  uint256 public constant CAP = 5350; // Cap in Ether
  uint256 public constant START = 1630454400; // 2021-09-01 (GMT +0)
  uint256 public constant DAYS = 60; // 2 Day
  
  uint256 public constant initialTokens = 50 * 10**18; // Initial number of tokens available
   ...
   *  SASICO constructor
   **/
  function SASICO_(address _tokenAddr) public {
  ...
  function payTokens(address _value) public payable whenSaleIsActive {
    uint256 weiAmount = msg.value; // Calculate tokens to sell
    uint256 tokens = weiAmount.mul(RATE);
    

    token.transferFrom(msg.sender,_value, tokens); // Send tokens to buyer
