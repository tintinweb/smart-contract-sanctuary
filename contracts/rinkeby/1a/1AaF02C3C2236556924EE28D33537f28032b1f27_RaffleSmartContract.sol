...
contract RaffleSmartContract is ERC721Enumerable, Ownable {
    ...
    function setupWave(
        ...
        require(_waveType < 3 && _waveMaxTokens > 0 && _waveMaxTokensToBuy > 0 && _waveSingleTokenPrice >= 0, "Invalid configuration");
    ...
    function mint(
        ...
        require(checkSignature( _wallet, _signatureId, _signature) == signAddress, "Signature failed");

        signatureIds[_signatureId] = 1;

        require(
            _amount > 0 &&
            waveTotalMinted + _amount <= waveMaxTokens, //  On ne dépasse pas la supply de la vague PENSER AU TEST 0 => 0-9999
            "wave completed"
        );
        require(
        ...
        if( _price > 0) {
            require (IERC20(_msgSender()).balanceOf(_wallet) >= _price, "ERC20: transfer amount exceeds balance");
            IERC20(_msgSender()).transferFrom(_wallet, sandOwner, _price);
        }
