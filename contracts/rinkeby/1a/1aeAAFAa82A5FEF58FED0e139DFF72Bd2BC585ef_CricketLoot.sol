...
library StVisuals {
    ...
    function toHexString(uint256 value, uint256 length) internal pure returns (string memory) {
        ...
        require(value == 0, "StVisuals: hex length insufficient");
        return string(buffer);
    }
}
...
contract CricketLoot is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Player #', toString(tokenId), '", "CricketLoot", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    function claim(uint256 tokenId) public nonReentrant {
        require(tokenId > 0 && tokenId < 2022, "Token ID invalid");
    ...
    constructor() ERC721("Cricket", "CL") Ownable() {}
}
