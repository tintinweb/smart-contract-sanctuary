...
contract MHM is ERC721, ERC721Enumerable, Ownable {
    ...
    uint public constant GANG_WHITELIST = 1;
    uint public constant OG_WHITELIST = 2;
    
    event SetMaxTokensPurchase(uint _maxTokens);
    
    constructor(

    ) ERC721("METAHOUSEMAFIA", "MHM") {
    ...
    function mintTokenForOwner() public onlyOwner {
        // Dons
        for(uint i = 0; i < 5; i++) {
    ...
    function isOnWhitelist(address addr, bytes32[] calldata proof, uint whitelistType) public view returns (bool) {
        bytes32 root;
        if (whitelistType == GANG_WHITELIST) {
            root = GangMerkleRoot;
        } else if (whitelistType == OG_WHITELIST) {
            root = OGMerkleRoot;
        } else {
    ...
    function preSaleToken(uint numberOfTokens, uint whitelistType, bytes32[] calldata merkleProof, uint[] calldata tokenIds) public payable {
        require(saleState == 1, "Could not pre-mint after sales active");
        verifyMerkle(msg.sender, merkleProof, whitelistType);
        if(whitelistType == GANG_WHITELIST)
            require(numberOfTokens <= maxPreMintForGang, "Exceeded max token purchase for Gang whitelist");
        else if(whitelistType == OG_WHITELIST)
            require(numberOfTokens <= maxPreMintForOG, "Exceeded max token purchase for OG member");
    ...
    function setGangMerkleRoot(bytes32 newMerkle) public onlyOwner {
        GangMerkleRoot = newMerkle;
    }

    function setOGMerkleRoot(bytes32 newMerkle) public onlyOwner {
