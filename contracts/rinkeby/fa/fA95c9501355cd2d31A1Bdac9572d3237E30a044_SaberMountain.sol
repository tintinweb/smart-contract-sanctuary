...
contract SaberMountain is ERC721Enumerable, Ownable {
  ...
  /* Define */
    ...
    /* MINT FUNCTIONS */
    function publicMint(uint qty) external payable publicMintingOpen {
      // Require nonzero amount
      require(qty > 0, "Sale Amount must be greater than zero");

      // Check proper amount sent
      require(msg.value == qty*mintPrice, "You need more ETH");

      /* send to mint */
      _mintLoop(msg.sender, qty);
     }

    function whitelistMint(uint qty) external payable whitelistMintingOpen {
      // Require nonzero amount
      require(qty > 0, "Sale Amount must be greater than zero");

      // Check proper amount sent
      /* require(msg.value == qty * whitelistMintPrice, "You need more ETH"); */
    ...
    /* Intenal */
    function _mintLoop(address to, uint amount) internal {
      require(totalSupply() + amount <= teamMintTotal, "All of Monkeys are out in the Cosmo and so we are sold out");
      require(amount <= teamMintTotal, "Max mint amount is 500 Monkeys for team");
