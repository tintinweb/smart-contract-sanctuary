...
contract BlootWheels is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getAlign(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "Align", directions);
    }
    ...
    function getWave(uint256 tokenId) public view returns (string memory) {
        ...
        string memory upDown = getAlign(tokenId);

        for(uint j = 1; j < 5; j++)
        {
            string memory h1 = getHeight(tokenId * j * 10);
            string memory h2 = getHeight(tokenId * j);
            wave = string(abi.encodePacked(wave, '<path id="gentle-wave', toString(j), '" d="M-300 310c30 0 58-', h1, ' 88-', h1, 's 58 ', h1, ' 88 ', h1 ));
            ...
            wave = string(abi.encodePacked(wave, ' v', keccak256(abi.encodePacked(upDown)) == keccak256("true")? string(abi.encodePacked('-', toString(71- j * 2))): toString(51- j * 2) ,'h-1000z"/>'));
        }
