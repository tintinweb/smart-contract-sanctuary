 ...
 */
contract NFTBracket is ERC721, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    address public marchMadness;

    mapping(uint256 => string) public tokenIdToTokenURI; // IPFS hash where we could retrieve the raw data for a given nftBracket.
    ...
    modifier onlyFromMarchMadness() {
        require(msg.sender == marchMadness, "NFTBracket: Invalid call");
        _;
    }

    constructor() ERC721("NFTBracket", "NFTB") {}
    ...
    function setMarchMadness(address _marchMadness) external onlyOwner {
        require(
            ...
            "NFTBracket: March Madness can't be zero address"
        );
     ...
     * @param bracketHash hash representation of the raw data of the bracket.
     * @param tokenUri ipfs hash where the raw data will be stored.

     * Requirements:
     *
     * - can be called from the MarchMadness contract only
     */
    function mint(
        address to,
        bytes32 bracketHash,
        string memory tokenUri
    ) external onlyFromMarchMadness returns (uint256 newTokenId) {
        _tokenIds.increment();
     ...
     * @param tokenId id which is going to be updated
     * @param bracketHash new hash of the raw data
     * @param tokenUri ipfs hash where the raw data will be stored.
    ...
    function update(
        ...
        bytes32 bracketHash,
        string memory tokenUri
    ) external onlyFromMarchMadness {
        require(
            _isApprovedOrOwner(from, tokenId),
            "NFTBracket: only approved or owner can update"
        );

        tokenIdToTokenURI[tokenId] = tokenUri;
        tokenIdTokenHash[tokenId] = bracketHash;
    }
    ...
    {
        require(_exists(tokenId), "NFTBracket: nonexistent token");

        string memory baseURI = _baseURI();
        return
            bytes(baseURI).length > 0
                ? string(abi.encodePacked(baseURI, tokenIdToTokenURI[tokenId]))
                : "";
    }
}
