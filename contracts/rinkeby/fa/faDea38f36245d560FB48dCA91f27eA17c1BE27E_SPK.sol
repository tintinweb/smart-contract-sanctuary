...
contract SPK is ISPK, ERC20Upgradeable, Owned {
    ...
    modifier onlyMinter() {
        require(msg.sender == minter, "SPK1");
        _;
    }

    function initialize(address ownerAddr, address initialFundAddr) initializer public {
        __ERC20_init("Sparkmutual Token", "SPK");
        __Owned_init(ownerAddr);
        _mint(initialFundAddr, INITIAL_FUND);
    }

    /// @notice Set minter of SPK
    /// @param minterAddr Address of the minter
    function setMinter(address minterAddr) external virtual onlyOwner {
        require(minterAddr != address(0), 'SPK2');
        minter = minterAddr;
        emit SetMinter(minterAddr);
    }

    /// @notice Mint SPK tokens to the receipt
    ...
    function mint(address to, uint amount) external override onlyMinter {
        require(to != address(0), 'SPK3');
        _mint(to, amount);
        emit Mint(to, amount);
    }

    /// @notice Burn SPK tokens from the receipt
    /// @param to Address of the receipt
    /// @param amount Amount of SPK to be burned
    function burn(address to, uint amount) external override onlyMinter {
        require(to != address(0) && amount > 0, 'SPK4');
        _burn(to, amount);
        emit Burn(to, amount);
    }
