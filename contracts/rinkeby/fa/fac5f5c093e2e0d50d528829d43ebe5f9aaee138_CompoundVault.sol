    ...
    AccessControlUpgradeable
{
    ...
    function redeem(uint256 _amount) external whenNotPaused returns (uint256 transferedAmount) {
        ...
        if(receipt[1] > 0){
           collaterals[address(this)] += receipt[1];
        }
        ...
        returns (uint256[] memory)
    {
        ...
        uint256[] memory txData = new uint256[](8);
        uint256 rate;
        if(farming == ETHER){
        ...
        if (underlyingMax >= _amount) {
            ...
            txData[4] = underlyingMax;
            txData[5] = amountWithdraw;

            // the amount + fees are below availabal malance
            if (amountWithdraw <= collaterals[msg.sender]) {
                txData[0] = 0; // 0 error
            ...
            } else if (amountWithdraw > collaterals[msg.sender]) {
                // we take the maxAvailable
                gainQuotient = quotient(underlyingMax, shares[msg.sender], 18); // must be update on withdraw
                amountWithdraw = _getCollateralAmount(
                    (gainQuotient * shares[msg.sender]) / 1e18
                );
                deductedFees = _getCollateralAmount(
                   ((((gainQuotient * shares[msg.sender]) / 1e18) - shares[msg.sender]) / feeBase) * fees
