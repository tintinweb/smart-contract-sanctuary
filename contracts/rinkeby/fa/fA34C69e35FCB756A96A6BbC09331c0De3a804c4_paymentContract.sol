...
// 70000000000000000
  ...
  function last_seed() external view returns (uint256) {
    ...
    require(whitelistedContracts[_msgSender()], "Only admins can call this");
    require(seeds[seeds.length-1] != 0, "seed is Not ready");
  ...
  function mint(uint256 amount, bool stake) external payable whenNotPaused {
    ...
    for (uint i = 0; i < amount; i++) {
      ...
      _tokenIndex[minted] = _nextID;

    }


    ERC721Contract.updateOriginAccess(tokenIds);
    
    if (headCost > 0) {
      ERC20Token.burn(_msgSender(), headCost);
      ERC20Token.updateOriginAccess();
    }

    if (stake) stakingContract.stakeMany(msgSender, tokenIds);
  ...
  function HeadDaoFreeMint(uint256 amount, bool stake) external payable whenNotPaused {
  
    address msgSender = _msgSender();
    uint16 minted = ERC721Contract.minted();
    uint256[] memory deposits = HeadDAOStaking.depositsOf(msgSender);
    uint256 daoMinted = daoMints[msgSender];
    ...
    require(deposits.length > 0, "You dont own any HeadDAO NFT");
    require(deposits.length >= daoMinted + amount && amount > 0 && amount <= 10, "Invalid Amount to mint");
    require(msg.value == 0);
