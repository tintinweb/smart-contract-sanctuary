...
contract Gems is ERC1155, Ownable {
    mapping(bytes => Recipe) public recipes;
    mapping(bytes => bool) public crafted;

    constructor() payable ERC1155("https://testapinftmarco.herokuapp.com/api/token/{id}") {
        bytes memory b = new bytes(1);
        b[0] = 0x61;
        recipes[b] = Recipe(1, 300000000000000, 100, true);
    }
    ...
    function createRecipe(
        ...
        recipes[recipe] = Recipe(nft_id, price, amount, true);
        return true;
    }
    ...
    function create(
        ...
        require(recipes[_data].exists, "Recipe doesn't exists");
        require(msg.value >= recipes[_data].price, "Need to send more ETH");
        require(!crafted[_s], "Gem already crafted");
        require(recipes[_data].remaining > 0, "This gems are all crafted");

        uint256 a = SolRsaVerify.pkcs1Sha256VerifyRaw(_data, _s, _e, _m);
        if (a == 0) {
            _mint(msg.sender, recipes[_data].nft_id, 1, _data);
            recipes[_data].remaining--;
            crafted[_s] = true;
            return true;
        }
        return false;
    ...
    function uri (uint256 _tokenId) pure override public returns (string memory) {
        return string(
            abi.encodePacked(
                "https://testapinftmarco.herokuapp.com/api/token/",
