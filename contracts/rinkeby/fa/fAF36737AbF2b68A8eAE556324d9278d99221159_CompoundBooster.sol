...
interface ICompoundComptroller {
    ...
    function markets(address _cToken) external view returns(bool isListed, uint256 collateralFactorMantissa, bool isComped);
}
...
contract CompoundBooster is Ownable {
    ...
    function demoInit() public {
        init(0xFf4013AE40aC8F81f1C234f59cBb6754EC073130,0x0000000000000000000000000000000000000000,0x0000000000000000000000000000000000000000,0x0000000000000000000000000000000000000000,0x0000000000000000000000000000000000000000);
    }
        ...
        returns (bool)
    {
        ...
        // );
        address rewardCompPool = ICompoundPoolFactory(compoundPoolFactory).CreateCompoundRewardPool(rewardComp, address(this));
        address rewardLendflareTokenPool = ICompoundPoolFactory(compoundPoolFactory).CreateCompoundRewardPool(lendflareToken, address(this));
        ...
        // );
        address interestRewardPool = ICompoundPoolFactory(compoundPoolFactory).CreateCompoundInterestRewardPool(lendflareToken, address(this));
        // aToken
        ...
        // );
        address treasuryFundPool = ICompoundPoolFactory(compoundPoolFactory).CreateTreasuryFundPool(address(this));
    ...
    function borrow(
        ...
        // cTokens = cTokens.mul(100).div(getCollateralFactorMantissa(pool.lpToken));

        require(IERC20(pool.lpToken).balanceOf(pool.treasuryFund) >= cTokens,"!Insufficient balance");
    ...
    function getBorrowRatePerBlock(uint256 _pid) external view returns (uint256) {
        PoolInfo memory pool = poolInfo[_pid];
    ...
    function getCollateralFactorMantissa(uint256 _pid) public view returns(uint256) {
        PoolInfo memory pool = poolInfo[_pid];

        ICompoundComptroller comptroller = ICompound(pool.lpToken).comptroller();
         (bool isListed, uint256 collateralFactorMantissa,) = comptroller.markets(pool.lpToken);

         return isListed ? collateralFactorMantissa : 800000000000000000;
    }

    function getBlocksPerYears(uint256 _pid,bool isSplit) public view returns(uint256) {
        // WhitePaperInterestRateModel
        //uint public constant blocksPerYear = 2102400;
        PoolInfo memory pool = poolInfo[_pid];

        uint256 blocks = ICompoundInterestRateModel(ICompound(pool.lpToken).interestRateModel()).blocksPerYear();
