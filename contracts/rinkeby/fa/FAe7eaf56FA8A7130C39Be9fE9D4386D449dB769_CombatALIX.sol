...
interface IAlinx is IERC721 {
    ...
    function getAlix(uint256 id) external view returns (Alix memory);
}

contract CombatALIX is Ownable {
    ...
    IERC20 public alixToken;
    IAlinx public alinxNFT;

    uint256 public timeLimitCombat = 24 hours;
    uint256[5] public limitCombat = [1, 1, 1, 1, 1];
    bool public paused;

    mapping(uint256 => uint256[]) public combatSessionsTime;
    mapping(address => CombatSession[]) public combatSessions;

    constructor(IERC20 _alixToken, address _alinxNFT) {
        alixToken = _alixToken;
        alinxNFT = IAlinx(_alinxNFT);
    }
    ...
    function setLimitCombat(uint256[5] memory _limitCombat) public onlyOwner {
    ...
    function combatReward(
        ...
        // rare 4: 75%
        // rare 5: 90%
        uint256 bonusPercentage = 30 + 15 * _alinxLevel;
        uint256 baseReward = 200 + 100 * _monsterLevel;
    ...
    function getWinRate(uint256 _monsterLevel, uint256 winRateRnd)
        private
        pure
        returns (uint256)
    {
        ...
        } else if (_monsterLevel == 1) {
            return winRateRnd.add(50);
        } else if (_monsterLevel == 2) {
            return winRateRnd.add(30);
        } else if (_monsterLevel == 3) {
            return winRateRnd.add(10);
        }
        return 0;
    }

    function nextTimeToCombat(uint256 _tokenId, uint256 _alinxLevel)
        public
        view
        returns (uint256)
    {
        uint256[] storage sessions = combatSessionsTime[_tokenId];
        uint256 limit = limitCombat[_alinxLevel];
    ...
    function combat(uint256 _tokenId, Monster _monster) external {
        ...
        require(alixToken.balanceOf(address(this)) > 0, "No reward");

        require(alinxNFT.ownerOf(_tokenId) == _msgSender(), "not own");
        if (Address.isContract(_msgSender()) || _msgSender() != tx.origin) {
            revert("not accept here");
        }

        uint256 alinxLevel = uint8(alinxNFT.getAlix(_tokenId).level);

        require(alinxLevel > 0, "require evolved");
        if (nextTimeToCombat(_tokenId, alinxLevel) > 0) {
        ...
        uint256 winRate = getWinRate(monsterLevel, rnd.div(10));

        if (rnd < winRate) {
            result = CombatResult.WIN;
            uint256 reward = combatReward(winRate, alinxLevel, monsterLevel);
    ...
    function safeReward(address _receiver, uint256 _amount) internal {
        uint256 balance = alixToken.balanceOf(address(this));
        // console.log("receiver %s reward %s", _receiver, _amount);
        if (balance > 0) {
            if (balance >= _amount) {
                alixToken.transfer(_receiver, _amount);
            } else {
                alixToken.transfer(_receiver, balance);
            }
        }
    }
