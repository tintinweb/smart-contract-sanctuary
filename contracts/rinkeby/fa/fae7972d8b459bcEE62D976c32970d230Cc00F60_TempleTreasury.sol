...
contract TempleTreasury is Ownable {
    ...
    // Mint price as a factor on intrinsicValue
    uint256 public mintFactor = 1e18;

    // Ratio of snapshot mark to market DAI to minted TEMPLE.
    // This represents the intrinsic value of TEMPLE and is core 
    // to all TEMPLE tokenomics
    ...
    // this will bootstrap our mintFactor
    ...
    event MintComplete(address _minter, uint256 _dai, uint256 _temple);
    ...
    /**
     * What factor of intrinsinc value is treasury willing to sell temple
     *
     * This is changed by community vote
     */
    function setMintFactor(uint256 amount) external onlyOwner {
      mintFactor = amount;
    }

    /** 
     * Accept DAI into treasury and mint TEMPLE  
     *
     * pre-condition of mint, `msg.sender` has approved `amountPaid` as an allowance
     */
    function mint(uint256 amountPaid) external {
      uint256 templeMinted = mintQty(amountPaid);
      SafeERC20.safeTransferFrom(DAI, msg.sender, address(this), amountPaid);
      TEMPLE.mint(msg.sender, templeMinted);
      emit MintComplete(msg.sender, amountPaid, templeMinted);
    }

    /** 
     * Calculate an expected dai for temple exchange (for a given amount paid in + mint factor)
     */
    function mintQty(uint256 amountPaid) public view returns (uint256) {
      return amountPaid * intrinsicValueRatio.temple / intrinsicValueRatio.dai / mintFactor;
    }
    ...
    function harvest(uint256 scalingFactor) external onlyOwner {
      ...
      uint256 newHarvest = ((reserveAsDai * intrinsicValueRatio.dai / intrinsicValueRatio.temple) - totalTempleSupply) * scalingFactor / 100;
