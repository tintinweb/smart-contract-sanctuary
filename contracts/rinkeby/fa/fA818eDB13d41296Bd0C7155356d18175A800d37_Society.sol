...
contract Society {
    ...
    uint256 public total_participants = 1;
    ...
    function getPriceToPay(address seller, uint256 amount)
        public
        view
        returns (uint256, uint256)
    {
        ...
        uint256 eth_to_pay;
        require(
            amount <= participant_info[seller].on_sale,
            "Not enought available shares"
        );
        eur_to_pay = participant_info[seller].sale_price * amount;
        eth_to_pay = eur_to_eth(eur_to_pay);
        return (eur_to_pay, eth_to_pay);
    }
    ...
    function getMarketInfo() public view returns (Participant_info[] memory) {
        ...
        for (uint256 i = 0; i < participants.length; i++) {
            if (participant_info[participants[i]].titles_amount > 0) {
                _market[counter] = participant_info[participants[i]];
                counter += 1;
            }
        }
        return _market;
    ...
    function buy_shares(uint256 shares_amount, address payable seller_address)
        ...
        onlyAllowed
    {
        require(marketBlocked == false, "Marked is blocked at the moment");
        ...
        participant_info[msg.sender].titles_amount += (shares_amount +
            owner_to_buy_amount);
        if (
            participant_info[seller_address].titles_amount == 0 &&
            buyer_had_shares
        ) {
