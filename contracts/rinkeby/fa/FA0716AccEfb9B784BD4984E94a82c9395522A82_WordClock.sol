...
contract WordClock is ERC721Enumerable, Ownable, ERC165Storage {
    ...
    function paddedNumberRow(uint256 rowIndex) internal view returns (string[7] memory row) {
        ...
        for (int256 index = 0; index < 7; index++) {
            ...
            row[uint256(index)] = toPaddedNumberString(uint256(unit));
        }
    }
    ...
    function animationUrl(uint256 tokenId) internal view returns (bytes memory) {
        ...
        return abi.encodePacked(animationBaseUrl(), animationQuery(tokenId));
    }

    function animationBaseUrl() internal view returns (string memory) {
        return _animationBaseUrl;
    }
    ...
    function toHtmlHexString(uint256 value, uint256 length) internal pure returns (string memory) {
