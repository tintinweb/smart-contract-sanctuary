...
contract NFT is Ownable, ERC721, IMerkleDistributor {
    ...
    event MerketRootChanged(bytes32 _merkleRoot);
    event SaleStartTimeChanged(uint256 _saleStartTime);
    event Minted(address indexed _user, uint256 indexed _tokenId, string _tokenURI);

    constructor(string memory _baseURI, bytes32 _merkleRoot) ERC721("NFT Test", "NFT_T") {
        baseURI = _baseURI;
    ...
    function setSaleStartTime(uint256 _saleStartTime) external onlyOwner {
        ...
        emit SaleStartTimeChanged(_saleStartTime);
    }

    function setMerkleRoot(bytes32 _merkleRoot) external onlyOwner {
        merkleRoot = _merkleRoot;
        emit MerketRootChanged(_merkleRoot);
    }

    function mint(uint _num) external payable {
        require(block.number >= saleStartTime, "Sale is not active.");
