...
interface IERC165 {
    ...
    function traitRarity() internal {
        if(totalPizzas > 0) {
            ...
            for(uint256 i = 1; i < totalIngredients; i++) {
                ...
                if(count > 0) {
                    ingredientRarityPercent[ingredientDetail._ingredientId] = ingredientUsedCount[ingredientId].div(totalPizzas).mul(100);
                }
            }
            ingredientRarityPercent[5000] = hundered.sub(sauceUsed.div(totalPizzas).mul(100)); // percentage for sauce not used
            ingredientRarityPercent[5001] = hundered.sub(cheeseUsed.div(totalPizzas).mul(100)); // percentage for cheese not used
        }
    }
    ...
    function createIngredient( string memory ingredientTokenURI, uint256 price, address artist, uint256 ingType) public {
        ...
        totalIngredients+=1;
        if(ingType == 4) {
            meatIngredientsCount+=1;
            meatIngredients.push(_ingredientId);
        }
        if(ingType == 5) {
            toppingIngredientsCount+=1;
    ...
    function bakePizzaAndMint(string memory metadata, uint256 base, uint256 sauce, uint256 cheese, uint256[] memory meats, uint256[] memory toppings ) public {
        ...
        for(uint256 x = 0; x < toppings.length; x++) {
            verifyUserIngredientType(toppings[x], 4);
        }
    ...
    function randomBakePizzaAndMint(string memory metadata, uint256 base, 
        ...
        verifyIngredientType(base, 1);
        verifyIngredientType(sauce, 2);
        verifyIngredientType(cheese, 3);
        for(uint256 x = 0; x < meats.length; x++) {
            verifyIngredientType(meats[x], 4);
        }
        for(uint256 x = 0; x < toppings.length; x++) {
            verifyIngredientType(toppings[x], 5);
        }
        uint256[] memory tops;
        uint256[] memory mets;
