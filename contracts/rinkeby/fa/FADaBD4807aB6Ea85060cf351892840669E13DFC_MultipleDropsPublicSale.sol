...
contract MultipleDropsPublicSale is Ownable, IERC165 {
    ...
    function deposit() public payable {
        ...
        // ugly but ensures length is resetted
        uint256 randomIndexToken1 = randomAvailableTokenIndex();
        sendRandomNft(randomIndexToken1);
        if (amount > 1) {
            uint256 randomIndexToken2 = randomAvailableTokenIndex();
            sendRandomNft(randomIndexToken2);
        }
        if (amount > 2) {
            uint256 randomIndexToken3 = randomAvailableTokenIndex();
            sendRandomNft(randomIndexToken3);
        }
    }

    function sendRandomNft(uint256 randomIndexToken) private {    
        randomIndexToken = randomAvailableTokenIndex();
        emit NftStartTransfer(_availableTokens[randomIndexToken], randomIndexToken, msg.sender, block.timestamp);
        transferNft(_availableTokens[randomIndexToken], msg.sender);
        _buyers[msg.sender] = _buyers[msg.sender] + 1;
        delete _availableTokens[randomIndexToken];
    }
    /**
    ...
    function transferNft(uint256 nftId, address to) private {
        require(
            ...
            "NFT is not owned by this contract"
        );
    ...
    function isNftIdAvailable(uint256 nftId) public view returns (uint256) {
       return ERC1155(nftContractAddress).balanceOf(owner(), nftId);
    }

    function randomIndex() public view returns (uint8) {
       return randomAvailableTokenIndex();
    }
