...
contract LP is ERC20Upgradeable, OwnableUpgradeable {
    ...
    uint public rewardFeeRate; // in decimals 10^9


    mapping(address => Front) public refs;
    ...
    function changeRewardRate(uint newRate_) external onlyOwner {
        rewardFeeRate = newRate_;
    }
     ...
     * @param referee - address of frontend
     * @return reward - amount of frontend reward fot its traffic
     */
    function pendingReward(address referee) public view returns(uint256 reward) {
        Front memory front = refs[referee];
        if (front.amount == 0 ) return 0;
        uint toClaim = totalRewards * (front.amount*rateDecimals/totalBetsAmount) / rateDecimals;
        reward = toClaim - front.claimed;
    }
    ...
    function claimReward() external {
        Front storage front = refs[msg.sender];
        uint toClaim = totalRewards * (front.amount*rateDecimals/totalBetsAmount) / rateDecimals;
        uint reward = toClaim - front.claimed;
        front.claimed = toClaim;
    ...
    function bet(
        ...
        if (referee == address(0x0)) referee = address(this);
        refs[referee].amount += amount;
        ...
        onlyCore
    {
        if (profitReserve >= initReserve) { // pool win
            ...
            uint refsRewards = profit * rewardFeeRate / rateDecimals;
            totalReserve = totalReserve.add(profit - refsRewards + pendingReward(address(this))); // and add to pool rewards from non-referee bets
            totalRewards += refsRewards;
        } else { // pool lose
