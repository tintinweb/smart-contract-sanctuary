...
contract NFT is ERC721, Ownable {
    ...
    uint256 public constant SALE_START_TIMESTAMP = 1633114182;
    ...
    constructor(string memory name, string memory symbol,  address _leashToken, address _shibToken, address  _leashpriceToken, address _shibPriceToken, address _moderator) ERC721(name, symbol) {
    ...
    function getNFTPriceForLEASH(uint256 numberOfNfts) public view returns (uint256) {
        ...
        if (currentSupply >= 9000) {
            // uint256 num = 7 ; 
            return (getLeashEthPrice().mul(_lc).div(100)).mul(numberOfNfts); // 9000 - 1000 LEASH per 0.3 ETH
        } else if (currentSupply >= 3000) {
            // uint256 num = 5 ;
            return (getLeashEthPrice().mul(_lb).div(100)).mul(numberOfNfts); // 3000 - 8999 LEASH per 0.2 ETH
        } else {
            // uint256 num = 3 ;
            return (getLeashEthPrice().mul(_la).div(100)).mul(numberOfNfts); // 0 - 2999 LEASH per 0.1 ETH
        }
    }
    ...
    function changeName(uint256 tokenId, string memory newName) public {
        ...
        ERC20Burnable(address(SHIB)).burn(getNameChangePrice());
