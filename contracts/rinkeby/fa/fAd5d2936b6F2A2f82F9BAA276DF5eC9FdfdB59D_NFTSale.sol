...
contract NFTSale is ERC1155, Ownable {
    ...
    uint mintedNumber = 0;
    constructor() public ERC1155("NFT_BASE_URL") {
    }
    ...
    function setMaxNFTs(uint256 maxNfts) public onlyOwner {
        MAX_NFT = maxNfts;
    }
    ...
    function mintNFT(uint numberOfTokens) public payable {
        ...
        require(mintedNumber.add(numberOfTokens) <= MAX_NFT, "Purchase would exceed max supply of NFT");
        require(price.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
        for(uint i = 0; i < numberOfTokens; i++) {
            if (mintedNumber < MAX_NFT) {
                _mint(msg.sender, mintedNumber, price, "");
                mintedNumber++;
            }
        }
    }
    function reserveNFT(uint numberOfTokens) public onlyOwner {
        require(saleIsActive, "Sale must be active to mint NFT");
        require(mintedNumber.add(numberOfTokens) <= MAX_NFT, "Purchase would exceed max supply of NFT");
        for(uint i = 0; i < numberOfTokens; i++) {
            if (mintedNumber < MAX_NFT) {
