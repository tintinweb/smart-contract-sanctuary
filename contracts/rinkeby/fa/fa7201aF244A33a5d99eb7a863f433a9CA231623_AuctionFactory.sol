...
contract AuctionFactory is IAuctionFactory, ERC721Holder, Ownable {
    ...
    event LogAuctionBalanceWithdrawal(
        address recipient,
        uint256 auctionId,
        uint256 amount,
        uint256 time
        ...
        returns (bool)
    {
        ...
        if (isValid) {
            auctionBalance[auctionId] = 0;
            for (uint256 i = 0; i < winners.length; i++) {
                auction.winners[i + 1] = winners[i];
                auctionBalance[auctionId] = auctionBalance[auctionId].add(
                    auction.balanceOf[winners[i]]
                );
            }
    ...
    {
        Auction storage auction = auctions[auctionId];
        uint256 amountToWithdraw = auctionBalance[auctionId];
