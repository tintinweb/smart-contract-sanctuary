...
contract Staking is Ownable {
    ...
    function stake(uint256 _amount, uint256 _index) external {
        require(_amount > 0, "User can't stake 0");
        require(
    ...
    function unstake(uint256 _index) external {
        ...
        require(
            ...
            "User must wait until the lock period is done"
        );
    ...
    function getStakeholderInfo(uint256 _index, address _staker)
        public
        view
        returns (
            uint256,
    ...
    {
        uint256 _depositTime = stakingPools[_index]
            .stakeholders[_staker]
            .depositTime;
        uint256 _amount = stakingPools[_index].stakeholders[_staker].amount;
        uint256 _reward = calculateReward(_index, _staker);

        return (_depositTime, _amount, _reward);
    }

    function setLockPeriod(uint256 _index, uint256 _lockPeriod)
        external
        onlyOwner
    {
        stakingPools[_index].lockPeriod = _lockPeriod;
