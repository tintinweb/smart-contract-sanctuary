...
// File: contracts/VirtualBalanceInterfaces.sol
...
contract ConvexStashTokens {
    ...
    address public immutable convexStash;
    uint256 public immutable pid;
    address public immutable operator;
    address public immutable virtualBalance;
...
contract ConvexBooster is Ownable {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;
    ...
    constructor(address _convexBooster, address _convexRewardFactory) public {
        convexRewardFactory = _convexRewardFactory;
        // convexStashFactory = _convexStashFactory;
        convexBooster = _convexBooster;
    }

    function addConvexPool(uint256 _convexPid, address _curveSwapAddress)
        public
    {
        (
            address lpToken,
        ...
        address rewardCrvPool = IConvexRewardFactory(convexRewardFactory)
            .CreateRewards(address(convexBooster));

        address virtualBalance/*  = address(
            new VirtualBalanceWrapper(address(this))
        ) */;
    ...
    function depositFor(
        ...
        if (balance > 0) {
            ...
            IConvexBooster(convexBooster).deposit(_pid, balance, true);
            // IConvexRewardPool(pool.rewardPool).stakeFor(_user, _amount);
            // VirtualBalanceWrapper(pool.virtualBalance).stakeFor(
            //     _user,
            //     _amount
            // );
        }
    ...
    function withdrawFor(
        ...
        //     pool.rewardPool
        // );
        IConvexBooster(convexBooster).withdraw(_pid, _amount);
    ...
    function liquidate(
        ...
        // IConvexStaker(convexStaker).liquidate(
        //     liquidater,
        //     _user,
        //     _pid,
        //     pool.lpToken,
