...
contract DTETH  is ERC20Burnable, ERC20Pausable, TokenRecover, AccessControl {
  ...
  // function freezeAccount(address account) public virtual {
  //   require(hasRole(FREEZER_ROLE, _msgSender()), "Freeze: must have freezer role to freeze");
  //   require(!hasRole(DEFAULT_ADMIN_ROLE, account), "Cannot freeze admin");
  //   grantRole(FROZEN_ROLE, account);
  // }
  //
  // function unFreezeAccount(address account) public virtual {
  //   require(hasRole(FREEZER_ROLE, _msgSender()), "unfreeze: must have freezer role to freeze");
...
event DTC(address indexed account, uint256 value);

function swap2dtc(uint256 amount) public virtual  {
  emit DTC(msg.sender, amount);
  _burn(_msgSender(), amount);
}
