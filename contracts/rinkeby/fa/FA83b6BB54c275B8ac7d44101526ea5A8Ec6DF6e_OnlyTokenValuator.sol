...
contract OnlyTokenValuator is ITokenValuator, Base {
    ...
    function setAsFungibleToken(address[] calldata tokens, bool asFungible) external {
    ...
    // TODO Add restrictions
    function setValuations(address[] calldata tokens, uint256 valuation) external {
        require(tokens.length > 0, "TOKENS_REQUIRED");
        for (uint256 indexAt = 0; indexAt < tokens.length; indexAt++) {
    ...
    function valuate(
        ...
        console.log("Valuating token>   ", token);
        console.log("valuations[token]>   ", valuations[token]);
        return valuations[token];
    }
