...
contract OfficialMarket is FixedPriceSale {
    ...
    bool public isEcchiMarketplace = true;
    ...
    constructor( address _platformAddress, uint16 _platformCut, address _artistNFTAddress) {
    ...
    function getIsEcchiMarketplace() public view returns (bool) {
        return isEcchiMarketplace;
    }
    ...
    onlyRole(LISTER_ROLE)
    {
        bool _isERC721;
        address _seller;
        bool _isArtistNFT = _nftAddress == artistNFTAddress;
        
        if(_isArtistNFT) {
            require(IArtistNFT(_nftAddress).getIsArtistNFT(), "Artist contract incorrectly set");
        }

        require(
            IERC165(_nftAddress).supportsInterface(InterfaceSignature_ERC721) ||
            IERC165(_nftAddress).supportsInterface(InterfaceSignature_ERC1155),
            "tokenContract does not support ERC721 or ERC1155 interface"
        );

        _isERC721 = IERC165(_nftAddress).supportsInterface(InterfaceSignature_ERC721);
        
        require(platformCutOnPrimarySales < 1000, "Cuts greater than 100%");
        ...
        if(_isERC721) {
            ...
            IERC721(_nftAddress).safeTransferFrom(_owner, address(this), _tokenId);
        } else {
            require(IERC1155(_nftAddress).balanceOf(msg.sender, _tokenId) >= _amount, "Sender is not owner");
    ...
    onlyRole(LISTER_ROLE)
    {
        ...
        require(_fixedPrice.length == _numberOfTokens, "fixed prices not equal to number of tokens");
        require(_startingTime.length == _numberOfTokens, "starting times not equal to number of tokens");
        require(_amount.length == _numberOfTokens, "amount not equal to number of tokens");

        for(uint i = 0; i < _numberOfTokens; i++) {
            createSaleFixedPrice(_nftAddress, _tokenId[i], _amount[i], _fixedPrice[i], _startingTime[i]);
        }
    }
}
...
contract FixedPriceBase is SaleBase {
    ...
    mapping (address => mapping(uint256 => FixedPrice)) tokenIdToFixedSale;

    event FixedSaleCreated(bool isERC721, uint256 tokenId, address nftContract, uint128 fixedPrice, uint64 startingTime,  address seller,  uint16 platformCut, uint256 amount);
    event FixedSaleSuccessful(bool isERC721, uint256 tokenId, address nftContract, uint256 totalPrice, address winner,  address seller,  uint16 platformCut);
    event FixedSaleFinished(uint256 tokenId, address nftContract, address seller);
     ...
     */
    function _addFixedSale(uint256 _tokenId, FixedPrice memory _fixedSale) internal {
        // update mapping
        tokenIdToFixedSale[_fixedSale.nftContract][_tokenId] = _fixedSale;
     ...
     */
    function _removeFixedSale(address _nftAddress, uint256 _tokenId) internal {
        delete tokenIdToFixedSale[_nftAddress][_tokenId];
    }
     ...
     */
    function _buy(address _nftAddress, uint256 _tokenId, uint256 _amount) internal {
        // get reference to the fixed price sale struct
        FixedPrice storage fixedSale = tokenIdToFixedSale[_nftAddress][_tokenId];
        ...
        // delete the sale
        _removeFixedSale(_nftAddress, _tokenId);
        ...
        _transfer(referenceFixedSale.nftContract, msg.sender, _tokenId, referenceFixedSale.isERC721, referenceFixedSale.amount);
     ...
     */
    function _finishFixedSale(address _nftAddress, uint256 _tokenId) internal {
        FixedPrice storage fixedSale = tokenIdToFixedSale[_nftAddress][_tokenId];
        ...
        _transfer(referenceFixedSale.nftContract, referenceFixedSale.seller, _tokenId, referenceFixedSale.isERC721, referenceFixedSale.amount);
...
contract FixedPriceSale is FixedPriceBase {
    ...
    bool public isEcchiFixedSale = true;
    ...
    internal
    {
        ...
        _addFixedSale(_tokenId, fixedSale);
    }
     ...
     * @param _tokenId Id of the token to buy
     */
    function buy(address _nftAddress, uint256 _tokenId) external payable {
        // do not allow sellers to buy their own artwork
        require(tokenIdToFixedSale[_nftAddress][_tokenId].seller != msg.sender ,
            "Sellers not allowed");
     ...
     */
    function finishFixedSale(address _nftAddress, uint256 _tokenId) external {
        _finishFixedSale(_nftAddress, _tokenId);
    }
     ...
     */
    function getFixedSale(address _nftAddress, uint256 _tokenId)
    external
    view
    returns
    (
