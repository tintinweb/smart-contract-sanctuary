...
contract PartyDegenerates is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 public constant DEGENERATES_END_PRICE = 5000000000000000; //0.5 ETH
    uint16 public constant MAX_DEGENERATES_PER_TRANSACTION = 50;
    uint16 public constant MAX_DEGENERATES_TO_MINT = 10000;
    uint16 public constant NUM_RESERVED_NFTS = 100; //TODO: number of reserved tokens for the team
    address RESERVE_VAULT_WALLET = 0x18632eE94d6395e0cd1ea6c7Ee702712BAf7C6D9; //TODO send the reserve NFTs to this wallet before activating the public sale

    string private baseURI;

    // Starting index block for the entire collection
    uint256 public collectionStartingIndexBlock;
    uint256 public numDegeneratesMinted;
    // Public sale params
    ...
    // Starting index for Minted Degenerates
    uint256 public mintedDegeneratesStartingIndex;
    ...
    event StartingIndicesSet(uint256 indexed _mintedDegeneratesStartingIndex);
    ...
    modifier startingIndicesNotSet() {
        require(
            mintedDegeneratesStartingIndex == 0,
            "Minted Degenerates starting index is already set"
        );
        _;
    }

    /* for example
    0	saleDuration	uint256	32400
    1	saleStartPrice	uint256	3000000000000000000 - 3ETH*/
    ...
    function getMintPrice() public view whenPublicSaleActive returns (uint256) {
        ...
        if (elapsed >= publicSaleDuration) {
            ...
            uint256 currentPrice = ((publicSaleDuration - elapsed) *
                publicSaleDegenerateStartingPrice) / publicSaleDuration;
            return
    ...
    */
    function reserve() public onlyOwner notLocked {
        for (uint256 i = _tokenIds.current(); i < NUM_RESERVED_NFTS; i++) {
            if (i < MAX_DEGENERATES_TO_MINT) {
                _tokenIds.increment();
                _safeMint(RESERVE_VAULT_WALLET, i + 1);
            }
        }
    }

    /*  for example
    0	numDegeneratesToMint	uint256	1 */
    ...
    {
        require(
            numDegeneratesMinted + numDegeneratesToMint <=
        ...
        for (uint256 i = _tokenIds.current(); i < numDegeneratesToMint; i++) {
            if (i < MAX_DEGENERATES_TO_MINT) {
                _tokenIds.increment();
    ...
    function totalDegeneratesMutated() external view returns (uint256) {
        return totalSupply() - numDegeneratesMinted;
    }
    ...
    /* for example
    0	uri	string	https://partydegenerates.com/api/ 
    each token will be https://partydegenerates.com/api/{id} 
    https://api.fatales.io/tokens/1
    https://boredapeyachtclub.com/api/mutants/1

    set up here: https://api.partydegenerates.com/degenerates/1
