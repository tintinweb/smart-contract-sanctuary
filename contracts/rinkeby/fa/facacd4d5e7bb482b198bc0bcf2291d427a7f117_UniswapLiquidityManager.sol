...
interface IUnipilot is IUnipilotEvents {
    function isAuthorizedForToken(address sender, uint256 tokenId) external view returns(bool approved);
...
interface IUniswapLiquidityManager is IULMEvents {
    ...
    function withdraw(bool pilotToken, bool wethToken, address sender, uint256 liquidity, bytes calldata data) external;
...
contract Unipilot is IUnipilot, ERC721Permit, PeripheryPayments {
    ...
    function isAuthorizedForToken(address sender, uint256 tokenId) public view override returns(bool approved) {
    ...
    function createPoolAndDeposit(IHandler.DepositParams memory params, bytes[] calldata data) external payable {
        ...
        (address token0, address token1, uint256 amount0, uint256 amount1) = params.token0 < params.token1 
            ...
            : (params.token1, params.token0, params.amount1Desired, params.amount0Desired);
        ...
        payable
    {
        ...
        if (amount0 > 0) pay(params.token0, _msgSender(), params.exchangeAddress, amount0);
        if (amount1 > 0) pay(params.token1, _msgSender(), params.exchangeAddress, amount1);
        ...
        external
    {
        ...
        IHandler(params.exchangeAddress).withdraw(params.pilotToken, params.wethToken, _msgSender(), params.liquidity, data);
    }
