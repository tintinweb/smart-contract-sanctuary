...
contract TernSale is ERC721Enumerable, Ownable {
    ...
    uint256 public gasPriceLimit = 200e9;
    ...
    modifier onlySigned(bytes memory signature) {
        bytes32 hash = keccak256(abi.encode(msg.sender, _signer.salt)).toEthSignedMessageHash();

        require(hash.recover(signature) == _signer.addr, "Invalid signature");
        _;
    }
