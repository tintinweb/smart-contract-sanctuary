...
contract ValuartMarket is Initializable, ERC721Upgradeable, OwnableUpgradeable {
    ...
    function bid(address tokenAddress, uint tokenId) external payable {
        ...
        if (auction.highestBid > 0) {
            ...
            if (msg.value <= auction.highestBid) revert('The bid amount is too low.');

            // Increasing the auction end date if the end date is less than 15 minutes from now
    ...
    function withdrawRefundWithAddress(address refundAddress) external onlyOwner() {
        uint refund = refunds[refundAddress];

        if (refund > 0) {
            refunds[refundAddress] = 0;
            payable(refundAddress).transfer(refund);
        }
    }
    ...
    function buy(address tokenAddress, uint tokenId) external payable {
        ...
        if (tokenCreator != address(0)) tokenCreator.transfer(royalty);
        if (tokenSeller != address(0)) tokenSeller.transfer(tokenPrice - royalty);
