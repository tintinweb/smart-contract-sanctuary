   ...
   */
    function _claimMouseFromTower(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
        ...
        if (totalCHEDDAREarned < MAXIMUM_GLOBAL_CHEDDAR) {
            ...
            owed = (lastClaimTimestamp - stake.value) * DAILY_CHEDDAR_RATE / 1 days;
...
//        require(!(unstake && block.timestamp - stake.value < MINIMUM_TO_EXIT), "Still claiming...");
        ...
        if (totalCHEDDAREarned < MAXIMUM_GLOBAL_CHEDDAR) {
            ...
            owed = (lastClaimTimestamp - stake.value) * dailyHouseCheddarRate / 1 days;
            // stop earning additional $CHEDDAR if it's all been earned
        }
        if (unstake) {
   ...
   */
    ...
    function isOwner(uint256 tokenId, address owner) external view override returns (bool) {
        if (cnmNFT.isCat(tokenId)) {
            if (cnmNFT.isCrazyCatLady(tokenId)) {
                return crazyYeild[tokenId].owner == owner;
            } else {
                return yeild[tokenId].owner == owner;
            }
        } else {
