...
contract FixStaking is AccessControl, Pausable {
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        require(
            _maxStake > _minStake,
            "setMinMaxStake: The maximum amount cannot be less than the minimum amount."
        );
        ...
        isPoolExist(_poolIndex)
    {
        ...
        // require(
        //     stakerAmount[msg.sender] + _amount <= maxStake,
        ...
        require(pool.reserve > 0, "deposit: This pool has no reward reserve");
        ...
        // staker.closedTime = timestamp + duration;
     ...
     */
    function withdraw(uint256 _stakerIndex) external whenNotPaused lock {
        ...
        // stakerAmount[msg.sender] -= staker.amount;
        pool.reserve -= totalReward;
        ...
        if (closedTime <= block.timestamp) {
            ...
            staker.pendingReward = totalReward;
     ...
     */
    function emergencyWithdraw(uint256 _stakerIndex) external whenPaused {
        StakerInfo storage staker = stakers[msg.sender][_stakerIndex];

        require(
            staker.amount > 0 || staker.pendingAmount > 0,
            "withdraw: Insufficient amount."
        );

        uint256 withdrawAmount = staker.amount + staker.pendingAmount;
     ...
     */
    function claimPending(uint256 _stakerIndex) external whenNotPaused {
        StakerInfo storage staker = stakers[msg.sender][_stakerIndex];
        ...
        uint256 totalPending = staker.pendingAmount + staker.pendingReward;

        _transferAndRemove(msg.sender, totalPending, _stakerIndex);

        emit Claimed(
            msg.sender,
            staker.pendingAmount,
        ...
        )
    {
        ...
        StakerInfo storage stakerInf = stakers[_staker][_stakerIndex];
        closedTime = getClosedTime(_staker, _stakerIndex);
        return (reward, mainPenalty, subPenalty, closedTime, stakerInf);
    }
        ...
        returns (uint256)
    {
        ...
        //uint256 closedTime = staker.startTime + (pool.endTime - pool.startTime);
        // THIS IS ONLY FOR TESTING DON'T FORGET REMOVE !!!!!!!!
        uint256 closedTime = staker.startTime + 600;
    ...
    receive() external payable {
        revert("This contract not recieve ETH");
    }
}
