    ...
    function addRow(uint256 _orderId, 
        ...
        if (_prevOrderId != 0 ) {
            ...
            for (uint256 i=0; i < table[_prevOrderId].authorisedIds.length; i += 1) {
                if(i < tmpAddr.length){
                    tmpAddr[i] = table[_prevOrderId].authorisedIds[i];
                } else {
                    tmpAddr.push(table[_prevOrderId].authorisedIds[i]);
                }
            }

            for (uint256 i=0; i < table[_prevOrderId].prevOrderIds.length; i += 1){
                if(i < tmpUint.length){
                    tmpUint[i]=table[_prevOrderId].prevOrderIds[i] ;
                } else{
                    tmpUint.push(table[_prevOrderId].prevOrderIds[i]) ;
                }
            }
        }
        
        tmpAddr.push(_senderId);
        tmpAddr.push(_receiverId);
        tmpUint.push(_prevOrderId);
       
        _authorisedIds = tmpAddr;
        _prevOrderIds = tmpUint;
