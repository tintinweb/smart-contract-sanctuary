...
contract WYVERNSBREATH is Context, IERC20, Ownable {
    ...
    mapping(uint => uint) private wyvernTypeToEarningsPerDay;
    mapping(uint => uint) private wyvernTokenToWyvernType;
    ...
    constructor()  {
        ...
        nft = IERC721(0x787aeF9c902202b77Db6d787acb143f10b67A261);
        //3501

        // 0 means Genesis
        wyvernTypeToEarningsPerDay[0] = 5;
        // 1 means Legendary
        wyvernTypeToEarningsPerDay[1] = 25;
        // 2 means Ascended/ascension
        wyvernTypeToEarningsPerDay[2] = 0;

        for (uint i = 1; i <= 3500; i++) {
            wyvernTokenToWyvernType[i] = 0;
            // set tokens 1 to 3500 as Genesis, with code 0, which means Genesis
        }
   ...
   */
    ...
    function getPendingReward(uint tokenId, uint todayCount, uint wyvernTypeCode) internal view returns (uint){
        ...
        if (lastUpdate[tokenId] == 0) {
            ...
            return wyvernEarningsPerDay * (10 ** _decimals);
        } else {
