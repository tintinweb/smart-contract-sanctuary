...
contract DeeperNFT is ERC721, AccessControl{
    ...
    function transferOwnership(address newAdmin) external {
        ...
        emit TransferOwnership(newAdmin);
    ...
    function changeNFTInfo(address oldAddress, address newAddress, uint256 id) onlyRole(ADMIN_ROLE) external{
	...
	function getProperty(uint256 id) external view returns(Properties memory){
		return token_properties[id];
	}
    ...
    function chageBaseURI(string calldata newBaseURI) onlyRole(ADMIN_ROLE) external{
        baseURI = newBaseURI;
