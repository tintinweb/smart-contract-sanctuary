...
contract CryptoCriminals is ERC721, Ownable, ReentrancyGuard, PaymentSplitter {
  ...
  uint public constant salePrice = 0.065 ether;
  uint public constant presalePrice = 0.055 ether;
  ...
  function recruitGangMember(uint[] memory tokenIds) external onlyEOA {
    ...
    require(tokenPointsCanRecruit(tokenIds), "token points dont add to more than 10, get more tokens");
    require(tokenTimeCanRecruit(tokenIds),"you must wait till you can recruit again");

    setTokensRecruitingTimer(tokenIds); 
    _mintCC(msg.sender, 1); // either insidethis function or implement new minting function for recruiting and add the current time to tokenid to recruit time mapping
  }

  function pseudoRandomPointAssignment(uint[] memory tokenIds) private view returns (uint){
    uint result;
    for(uint i = 0; i < tokenIds.length; i++){
      result = uint(keccak256(abi.encodePacked(result, msg.sender.balance ,tokenIds[i])));
    }
    result = uint(keccak256(abi.encodePacked(result, block.timestamp)));
    return (result % 3) + 1;
  }
