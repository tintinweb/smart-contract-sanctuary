...
contract LarvaLads is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getTraits(Larva memory larvaLad) internal view returns (string memory) {
        ...
        if (larvaLad.layerThree == 4) {
            parts[4] = 'Alien';
            parts[5] = '"}, {"trait_type": "Mouth","value": "';
            parts[6] = 'Alien';
        }
        ...
        if (larvaLad.layerThree < 3 || larvaLad.layerThree > 5) {
            parts[10] = 'Normal';
            parts[11] = '"}, {"trait_type": "Mouth","value": "';
    ...
    function getBaseColorCode(Larva memory larvaLad) internal view returns (string memory) {
        string memory output;
        output = string(abi.encodePacked(baseColors[larvaLad.baseColor]));
        return output;
    }

    function getLayerThreeSVG(Larva memory larvaLad) internal view returns (string memory) {
        string memory output;
        output = string(abi.encodePacked(thirdLayers[larvaLad.layerThree]));
        return output;
    }

    function getLayerFourSVG(Larva memory larvaLad) internal view returns (string memory) {
        string memory output;
        output = string(abi.encodePacked(fourthLayers[larvaLad.layerFour]));
        return output;
    }

    function getLayerFiveSVG(Larva memory larvaLad) internal view returns (string memory) {
        string memory output;
        output = string(abi.encodePacked(fifthLayers[larvaLad.layerFive]));
        return output;
    }

    function getLayerSixSVG(Larva memory larvaLad) internal view returns (string memory) {
        string memory output;
        output = string(abi.encodePacked(sixthLayers[larvaLad.layerSix]));
        return output;
    }

    function getTokenSVG(Larva memory larvaLad) internal view returns (string memory) {
        ...
        parts[1] = '<path fill="';
        parts[2] = getBaseColorCode(larvaLad);
        ...
        parts[4] = getLayerThreeSVG(larvaLad);
        parts[5] = getLayerFourSVG(larvaLad);
        parts[6] = getLayerFiveSVG(larvaLad);
        parts[7] = getLayerSixSVG(larvaLad);
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Larva Lad #', toString(tokenId), '", "description": "Larva Lads are fully on-chain and randomly generated at mint."', getTraits(larvaLad), '"image": "data:image/svg+xml;base64,', Base64.encode(bytes(getTokenSVG(larvaLad))), '"}'))));
