...
contract WnDGame is IWnDGame, Ownable, ReentrancyGuard, Pausable {
  ...
  // uint256 private minPrice = 0.1 ether;
  ...
  modifier requireContractsSet() {
      require(address(gpToken) != address(0) && address(traits) != address(0) && address(wndNFT) != address(0) && address(tower) != address(0), "Contracts not set");
      _;
  }

  function setContracts(address _gp, address _traits, address _wnd, address _tower) external onlyOwner {
    gpToken = IGP(_gp);
  ...
  function mint(uint256 amount, bool stake) external payable whenNotPaused nonReentrant {
    ...
    uint16 minted = uint16(wndNFT.totalSupply());
    ...
    if (minted < paidTokens) {
      ...
      if(hasPublicSaleStarted) {
        ...
        require(_whitelistAddresses[_msgSender()].numMinted + amount <= MAX_PRESALE_MINTS, "too many mints");
    ...
    require(lw.blockNum < block.number, "hmm what doing?");
   ...
   * the next 20% are 20000 $GP
   * the next 40% are 40000 $GP
   * the final 20% are 80000 $GP
   * @param tokenId the ID to check the cost of to mint
   * @return the cost of the given token ID
   */
  function mintCost(uint256 tokenId, uint256 maxTokens, uint256 paidTokens) public pure returns (uint256) {
    if (tokenId <= paidTokens) return 0;
    if (tokenId <= maxTokens * 2 / 5) return 24000 ether;
    if (tokenId <= maxTokens * 4 / 5) return 48000 ether;
    return 96000 ether;
  }
