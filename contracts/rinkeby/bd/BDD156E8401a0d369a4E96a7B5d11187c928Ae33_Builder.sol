...
contract Builder is IBuilderData{
    ...
    function setPrizePoolAddress(address prize_pool_address) public isOwner{
        _prize_pool_address = prize_pool_address;
        _prize_Pool_Instance = PrizePool(prize_pool_address);
    }

    function setPrizeStrategyAddress(address prize_strategy_address) public isOwner{
        _prize_strategy_address = prize_strategy_address;
        _prize_strategy_instance = MultipleWinners(prize_strategy_address);
    }
    ...
    function calculateEarlyExitFee(address from, address controlledToken, uint256 amount)public returns (uint256 exitFee, uint256 burnedCredit){
        IERC20Upgradeable(controlledToken).transferFrom(msg.sender,address(this), amount);
        _prize_Pool_Instance.calculateEarlyExitFee(address(this), controlledToken, amount);
        IERC20Upgradeable(controlledToken).transfer(msg.sender, amount);
    }

    function withdrawInstantlyFrom(address from, uint256 amount, address controlledToken, uint256 maximumExitFee, address tokenAddress) public returns (uint256 exitFee){
        IERC20Upgradeable(controlledToken).transferFrom(msg.sender,address(this), amount);
        IERC20Upgradeable(controlledToken).approve(_prize_pool_address, amount);
        _prize_Pool_Instance.withdrawInstantlyFrom(address(this), amount, controlledToken, maximumExitFee);
        IERC20Upgradeable(tokenAddress).transfer(msg.sender, amount);
    }

    function captureAwardBalance() external  returns (uint256){
        _prize_Pool_Instance.awardBalance();
    }

    function award(address to, uint256 amount, address controlledToken ) public{
        _prize_Pool_Instance.award(to, amount, controlledToken);
    }

    function awardExternalERC20(address to, address externalToken, uint256 amount) public{
        _prize_Pool_Instance.awardExternalERC20(to, externalToken, amount);
    }

    function awardExternalERC721(address to, address externalToken, uint256[] calldata tokenIds) public{
        _prize_Pool_Instance.awardExternalERC721(to, externalToken, tokenIds);
    }

    function balanceOfCredit(address user, address controlledToken) public returns (uint256){
        _prize_Pool_Instance.balanceOfCredit(user, controlledToken);
    }

    function balance() external returns (uint256){
        _prize_Pool_Instance.balance();
    }

    function addExternalErc20Award(address _externalErc20, uint256 amount) external isOwner{
        IERC20Upgradeable(_externalErc20).transferFrom(msg.sender,_prize_pool_address, amount);

        _prize_strategy_instance.addExternalErc20Award(IERC20Upgradeable(_externalErc20));
    }

    function addExternalErc721Award(address _externalErc721,uint256[] calldata _tokenIds, uint256 _tokenId) public isOwner{
        IERC721Upgradeable(_externalErc721).transferFrom(msg.sender, _prize_pool_address, _tokenId);

        _prize_strategy_instance.addExternalErc721Award(IERC721Upgradeable(_externalErc721), _tokenIds);
    }
    function canAwardExternal(address _externalToken) public view returns (bool){
        _prize_Pool_Instance.canAwardExternal(_externalToken);
    }

    function prizePeriodStartedAt() public view returns (uint256){
        _prize_strategy_instance.prizePeriodStartedAt();
    }

    function prizePeriodEndAt() public view returns (uint256){
        _prize_strategy_instance.prizePeriodEndAt();
    }

    function creditPlanOf(address controlledToken)public view isOwner returns (uint128 creditLimitMantissa, uint128 creditRateMantissa){
        _prize_Pool_Instance.creditPlanOf(controlledToken);
    }

    function setCreditPlanOf( address _controlledToken, uint128 _creditRateMantissa, uint128 _creditLimitMantissa) public isOwner{
        _prize_Pool_Instance.setCreditPlanOf(_controlledToken, _creditRateMantissa, _creditLimitMantissa);
    }

    function setNumberOfWinners(uint256 count) public isOwner {
        _prize_strategy_instance.setNumberOfWinners(count);
    }

    function startAward() public isOwner{
        _prize_strategy_instance.startAward();
    }

    function completeAward() public isOwner{
        _prize_strategy_instance.completeAward();
    }
}
