...
contract Heroes is ERC721Enumerable, Pausable, PaymentSplitter {
    ...
    uint256 public maxTotalSupply = 3100;
    ...
    address[] private _team = [
        0x8353A934B3A9d29e1F16cc0Bece4549069F34E1a,
        0xEdD0d1eDE4F94E68c0f42d3C699fFDCC53b60a9F
    ];
    ...
    constructor()
        ERC721("Heroes of Age NFTs", "HOAN")
        ...
        onlyOwner
    {
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                ...
                "Heroes of Age: Can't add a zero address"
            );
        ...
        onlyOwner
    {
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                _addresses[ind] != address(0),
                "Heroes of Age: Can't remove a zero address"
            );
    ...
    function setUpPresale(uint256 _duration) external onlyOwner {
        require(
            ...
            "Heroes of Age: Unauthorized Transaction"
        );
    ...
    function setUpSale() external onlyOwner {
        ...
        require(
            ...
            "Heroes of Age: Sale not started"
        );
        uint256 _startTime = block.timestamp;
    ...
    function getPrice() public view returns (uint256) {
        ...
        if (
            ...
            _price = 20000000000000000; //0.02 ETH
        } else {
            _price = 10000000000000000; //0.01 ETH
        }
        return _price;
        ...
        whenNotPaused
    {
        require(
            ...
            "Heroes of Age: max total supply exceeded"
        );

        require(
            giftCount + _addresses.length <= maxGiftSupply,
            "Heroes of Age: max gift supply exceeded"
        );
        ...
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                _addresses[ind] != address(0),
                "Heroes of Age: recepient is the null address"
            );
            _tokenIds.increment();
    ...
    function presaleMint(uint256 _amount) internal {
        ...
        require(
            ...
            "Heroes of Age: Presale must be active to mint hero"
        );
        require(
            block.timestamp >= _presaleConfig.startTime,
            "Heroes of Age: Presale not started"
        );
        require(
            block.timestamp <=
                _presaleConfig.startTime + _presaleConfig.duration,
            "Heroes of Age: Presale is ended"
        );
        require(
        ...
        require(
            ...
            "Heroes of Age: Can only mint 2 tokens"
        );
        require(
            totalNFT + _amount <= maxTotalSupply,
            "Heroes of Age: max supply exceeded"
        );
        uint256 _price = getPrice();
        require(
            _price * _amount <= msg.value,
            "Heroes of Age: Ether value sent is not correct"
        );
        uint256 _newItemId;
    ...
    function saleMint(uint256 _amount) internal {
        SaleConfig memory _saleConfig = saleConfig;
        require(_amount > 0, "Heroes of Age: zero amount");
        require(_saleConfig.startTime > 0, "Heroes of Age: sale is not active");
        require(
        ...
        require(
            ...
            "Heroes of Age:  Can only mint 10 tokens at a time"
        );
        require(
            totalNFT + _amount <= maxTotalSupply,
            "Heroes of Age: max supply exceeded"
    ...
    function burn(uint256 tokenId) external {
        require(isBurnEnabled, "Heroes of Age: burning disabled");
        require(
            _isApprovedOrOwner(msg.sender, tokenId),
            "Heroes of Age: burn caller is not owner nor approved"
        );
        _burn(tokenId);
        totalNFT = totalNFT - 1;
    }
