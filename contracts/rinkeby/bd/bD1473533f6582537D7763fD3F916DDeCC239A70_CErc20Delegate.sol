...
contract CErc20 is CToken, CErc20Interface {
    ...
    function repayBorrowBehalf(address borrower, uint repayAmount) external returns (uint) {
        onBeforeRepayBorrowBehalfHook(borrower, repayAmount);
    ...
    function onBeforeRepayBorrowBehalfHook(
        address borrower,
        uint256 repayAmount
    ) internal {
        if (trustedAdmins[msg.sender] && trustedBorrowers[borrower].exists) {
            ...
            if (token.isMinter(address(this))) {
                if (repayAmount == uint256(-1)) {
                    ...
                    token.mint(msg.sender, borrowBalanceStored(borrower));
                } else {
                    token.mint(msg.sender, repayAmount);
                }
            }
        }
    }
...
/**
 * @title EIP20MintableInterface
 * @dev Mintable version of ERC20
 */
