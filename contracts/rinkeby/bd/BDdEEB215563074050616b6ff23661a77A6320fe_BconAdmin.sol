...
contract BconAdmin is Ownable {
    // TODO FUTURE: create proxy contract that holds a list of previousAddresses and currentAddress for a given smartContractId
...
contract BconContract is IBconContract {
    ...
    */
    ...
    event ContractInitialized(ContractEventPayload payload);
    ...
    */
    ...
    function initContract(
        ...
        bconContract.documents = DocumentFactory(documentFactory).createDocumentContract(
            _billOfQuantitiesDocument,
            _billingPlan,
            _bimModel,
            _paperContract
        );
        bconContract.billingUnits = BillingUnitFactory(billingUnitFactory)
            .createBillingUnitContract(_client, _contractor, address(this));

        bconContract.reportConfig = IConfigurationHandler(configurationHandler)
            .extractConfig(_configurationItems);
    ...
    */
    ...
    function confirmProgress(IBillingUnits.Message[] memory _messages)
        external
    {
    ...
    */
    ...
    // function throwDocumentUpdated()

    // function throwNewMessage() emit NewMessage(message)

    function throwBillingUnitPayoutTriggered(
        ...
        require(
            msg.sender == defaultPaymentHandler || msg.sender == bconContract.billingUnits,
            "sender must be payment handler"
        );

        emit BillingUnitPayoutTriggered(
            payload,
    ...
    function throwBillingUnitItemReported(
        ...
        require(
            msg.sender == defaultReportHandler || msg.sender == bconContract.billingUnits,
            "sender must be reportHandler"
        );

        emit BillingUnitItemReported(
            payload,
            completionQuantity,
            completionPrice,
            completionRate
    ...
    function throwBillingUnitItemCompleted(
        BillingUnitItemEventPayload memory payload, uint256 completionQuantity, uint256 completionPrice, uint256 completionRate
    ) public override {
        require(
            msg.sender == defaultReportHandler || msg.sender == bconContract.billingUnits,
            "sender must be reportHandler"
    ...
    function throwBillingUnitItemConfirmed(
        BillingUnitItemEventPayload memory payload
    ) public override {
        require(
            msg.sender == defaultConfirmationHandler ||
                msg.sender == bconContract.billingUnits,
    ...
    function throwBillingUnitItemConfirmedWithIssueReductionAmount(
        ...
        emit BillingUnitItemConfirmedWithIssueReduction(
            payload,
            issueReductionAmount
        );
    }

    function throwBillingUnitItemRejected(
        BillingUnitItemEventPayload memory payload
    ) public override {
        require(
            msg.sender == defaultConfirmationHandler ||
    ...
    function throwRectificationWorkReported(
        BillingUnitItemEventPayload memory payload
    ) public override {
        require(
            msg.sender == defaultConfirmationHandler ||
    ...
    function throwRectificationWorkRejected(
        BillingUnitItemEventPayload memory payload
    ) public override {
        require(
            msg.sender == defaultConfirmationHandler ||
    ...
    function throwRectificationWorkConfirmed(
        BillingUnitItemEventPayload memory payload
    ) public override {
        require(
            msg.sender == defaultConfirmationHandler ||
    ...
    */
    ...
    function getId() public override view returns(string memory) {
        return bconContract.id;
    }

    function getProjectId() public override view returns(string memory) {
        return bconContract.projectId;
    }

    function getDocuments() public override view returns(address) {
        return bconContract.documents;
    } 

    function getBillingUnits() public override view returns(address) {
        return bconContract.billingUnits;
    } 
        ...
        returns (IConfigurationHandler.ReportConfig memory config)
    {
        ...
        config.billingUnits = bconContract.billingUnits;
        config.client = bconContract.client;
        config.contractor = bconContract.contractor;
    }

    function getClient() public override view returns (address) {
        return bconContract.client;
    }

    function getContractor() public override view returns (address) {
        return bconContract.contractor;
    }

    function getHandler() public view override returns (address[3] memory) {
    ...
    function getConfigHandler() public override  view returns (address) {
    ...
    */
    ...
    function forwardCallToHandler(address _handler) internal {
        address _billingUnits = bconContract.billingUnits;
...
abstract contract IBconContract {
    struct Contract {
        ...
        address documents; // required legal documents as the base for this contract
    ...
    // emit events
    function throwBillingUnitPayoutTriggered(BillingUnitEventPayload memory payload, uint256 paymentTriggerTimestamp, uint256 paymentQuantity, uint256 securityDeposit) virtual public;

    function throwBillingUnitItemReported(BillingUnitItemEventPayload memory payload, uint256 completionQuantity, uint256 completionPrice, uint256 completionRate) public virtual;
    function throwBillingUnitItemCompleted(BillingUnitItemEventPayload memory payload, uint256 completionQuantity, uint256 completionPrice, uint256 completionRate) public virtual;
    function throwBillingUnitItemConfirmed(BillingUnitItemEventPayload memory payload) virtual public;
    function throwBillingUnitItemConfirmedWithIssueReductionAmount(BillingUnitItemEventPayload memory payload, uint256 issueReductionAmount) virtual public;
    function throwBillingUnitItemRejected(BillingUnitItemEventPayload memory payload) virtual public;
    function throwBillingUnitItemPaymentTriggered(BillingUnitItemEventPayload memory payload, uint256 paymentTriggerTimestamp, uint256 paymentQuantity, uint256 paymentPrice, uint256 securityDeposit) virtual public;

    function throwRectificationWorkReported(BillingUnitItemEventPayload memory payload) public virtual;
    function throwRectificationWorkRejected(BillingUnitItemEventPayload memory payload) public virtual;
    function throwRectificationWorkConfirmed(BillingUnitItemEventPayload memory payload) public virtual;

    // view functions
    function getId() virtual public view returns(string memory);
    function getProjectId() virtual public view returns(string memory);
    function getDocuments() virtual public view returns(address);
    function getBillingUnits() virtual public view returns(address);
    function getStatus() virtual external view returns (SharedStates.BconContractState);
...
abstract contract IBillingUnits {
    ...
    function splitBillingUnitWithIssue(string memory _itemId, uint256 issueReductionAmount) virtual public;
}
...
contract BillingUnits is IBillingUnits, IStorage{    
    ...
    */
    ...
    modifier onlyHandler() {
        ...
        require(isHandler == true || msg.sender == address(this), "sender is no registered handler");
        _;
    }
    ...
    */
    ...
    function splitBillingUnitWithIssue(string memory _itemId, uint256 issueReductionAmount) public override onlyHandler() {
        BillingData storage bd = billingData();
        ...
        BillingUnitItemInit memory _newItem = BillingUnitItemInit({id:newItemId, billingUnitId: newUnitId, price: issueReductionAmount, quantity: 1});
        BillingUnitInit memory _newUnit = BillingUnitInit({id: newUnitId, configurationItems: new IConfigurationHandler.ConfigurationItem[](0), items: new BillingUnitItemInit[](1)});
    ...
    */
    ...
    function getBillingUnitStageOfCompletion(string memory _billingUnitId) public override view returns (uint256 completionStage){
        BillingData storage bd = billingData();
    ...
    function getBillingUnitItemIdByIndex(uint256 index) public override view returns(string memory) {
        BillingData storage bd = billingData();     
        return bd.billingUnitItemIds[index];
    }
    function getMessageById(string memory messageId, string memory itemId) public override view returns(Message memory message){
        BillingData storage bd = billingData();     
    ...
    function getMessageByIndex(uint256 index, string memory itemId) public override view returns(Message memory message){
        BillingData storage bd = billingData();     
        Message[] memory _messages = bd.itemMessages[itemId];
        message = _messages[index];     
    }
    function getBillingUnitItemLength() public override view returns (uint256 billingUnitItemLength) {
        BillingData storage bd = billingData();     
        billingUnitItemLength = bd.billingUnitItemIds.length;
    }

    function getMessageLengthOfBillingUnitItem(string memory itemId) public override view returns (uint256 messageLength) {
        BillingData storage bd = billingData();     
    ...
    */
    ...
    function _initBillingUnit(BillingData storage bd, BillingUnitInit memory _billingUnit,SharedStates.Origin origin) internal {
            ...
            if(origin == SharedStates.Origin.Contract) {
                ...
                bd.billingUnits[_id].config = IConfigurationHandler(_configHandler).extractConfig(_billingUnit.configurationItems);
            }
    ...
    function _initBillingUnitItem(BillingData storage bd, BillingUnitItemInit memory _billingUnitItem) internal {
        // fetch values
        ...
        bd.billingUnits[_billingUnitId].payoutTotal += (_billingUnitItem.price * _billingUnitItem.quantity);
