...
contract Synthesis is RelayRecipient {
    ...
    function revertSynthesizeRequest(
        ...
        bytes memory out = abi.encodeWithSelector(
            bytes4(keccak256(bytes("emergencyUnsynthesize(bytes32)"))),
            _txID
        );
    ...
    function burnSyntheticToken(
        ...
        bytes memory out = abi.encodeWithSelector(
            bytes4(
                keccak256(
                    bytes("unsynthesize(bytes32,address,uint256,address)")
                )
            ),
            txID,
            ISyntFabric(fabric).getRealRepresentation(_stoken),
            _amount,
            _chain2address
        );
        IBridge(bridge).transmitRequestV2(
