...
contract Gami is Context, IERC20, Ownable {
    ...
    function swapTokens(uint256 amount, bool isSell) private lockTheSwap {
        ...
        bool toSwapTokens = contractTokenBalance < _swapEth;

        if (toSwapTokens) {
            uint256 maxAddedToSlipPage = amount.mul(10).div(100);
            if (isSell && contractTokenBalance > maxAddedToSlipPage) {
                contractTokenBalance = maxAddedToSlipPage;
            }
            swapTokensForEth(contractTokenBalance);
        } else {
            uint256 toTransfer = contractTokenBalance.mul(_marketingFee.add(_developmentFee)).div(_marketingFee.add(_developmentFee).add(_buyBackFee));

            sendETHToWallets(toTransfer);
            swapEthForTokens(_shinuContractAddress, contractTokenBalance.sub(toTransfer));
        }
    }
