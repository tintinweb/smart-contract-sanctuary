...
contract SharkSociety is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public sharkPrice = 0.07 ether;
    uint256 public discountPrice1 = 0.07 ether;
    uint256 public discountPrice2 = 0.07 ether;

    uint256 public qtyDiscount1 = 5;
    uint256 public qtyDiscount2 = 10;

    uint public constant maxSharksPurchase = 20;
    address constant devAddress = 0x3412A4540Bf2B4B597a7D96497C3052632e4f95B;
    // FIXME: CHANGE THIS ADDRESS!!!
    address constant teamTreasuryAddress = 0xB3CfBf8dC8F5d2f6607d8d8E251F7aae62a5d10a;
    // FIXME: CHANGE THIS ADDRESS!!!
    ...
    @param mysteryShark - URI for pre-reveal Mystery Shark metadata
     */
    constructor(string memory name,
                ...
                string memory mysteryShark) ERC721(name, symbol) {
    ...
    */
    ...
    function withdraw() public onlyOwner {
        ...
        uint devPayout = balance.mul(10).div(100);
        uint teamPayout = balance.mul(90).div(100);
        payable(devAddress).transfer(devPayout);
        payable(teamTreasuryAddress).transfer(teamPayout);
    }
    ...
    /**
     * Reserve Sharks for future activities and for supporters
     */
    function reserveSharks(uint256 amount) public onlyOwner {
    ...
    */
    ...
    function lock() public onlyOwner notLocked {
        ...
        require(totalSupply() == MAX_TOKENS, "Not all Sharks are minted yet!");
        locked = true;
    }
    ...
    /**
     * @dev Sets the Mystery Shark's URI
     */
    function setMysterySharkURI(string memory mystery) public onlyOwner notLocked {
        mysterySharkURI = mystery;
    }
    ...
    */
    function mintSharks(uint numberOfTokens) public payable {
        ...
        require(numberOfTokens <= maxSharksPurchase, "Can only mint 20 Sharks at a time.");
    ...
    */
    function presaleMintSharks(uint numberOfTokens) public payable {
        ...
        require(numberOfTokens <= maxSharksPurchase, "Can only mint 20 Sharks at a time.");
        require(balanceOf(msg.sender).add(numberOfTokens) <= maxSharksPurchase, "Purchase would exceed presale limit of 20 Sharks per address.");
        require(totalSupply().add(numberOfTokens) <= MAX_TOKENS, "Purchase would exceed max supply of Sharks.");
        require(getPricePerUnit(numberOfTokens).mul(numberOfTokens) == msg.value, "Ether value sent is not correct.");
        _mintSharks(numberOfTokens, msg.sender);
    }
      ...
      */
      function getPricingOptions() public view returns (uint256[6] memory) {
          uint256[6] memory arr = [1, sharkPrice, qtyDiscount1, discountPrice1, qtyDiscount2, discountPrice2];
          return arr;
      }

    /**
     * @dev Get the price per unit based on number of sharks to mint
     */
