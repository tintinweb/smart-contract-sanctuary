...
contract PudgyHalloween is ERC1155, Ownable {
    ...
    function getRandomPudgyHalloween(uint256 _tokenIds) private view returns(uint256){
        uint256 bigNumber = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, _tokenIds, RANDOM_SEED))) % 100;
        if(bigNumber <= 10) return 1;
        if(bigNumber <= 30) return 2;
        return 3;
    }

    function getPudgyHalloweenIds(uint256 _token) public view returns(uint256){
