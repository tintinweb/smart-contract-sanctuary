...
contract Token is Context, IERC20, Ownable {
    ...
    //address testnet router = pcs 0xD99D1c33F9fC3444f8101754aBC46c52416550D1; Public = pcs 0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    function setAllFeePercent(uint8 taxFee, uint8 liquidityFee, uint8 burnFee, uint8 walletFee, uint8 devFee, uint8 buybackFee) external onlyOwner() {
        ...
        require(devFee >= 0 && devFee <=maxDevFee,"DF err");
    ...
    function setFeeDev(address payable newFeeDev) external onlyOwner {
        require(newFeeDev != address(0), "ZERO ADDRESS");
        feeDev = newFeeDev;
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _burnFee + _walletFee + _devFee + _buybackFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _walletFee == 0 && _devFee == 0 && _buybackFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        //This needs to be distributed among burn, wallet, dev, and liquidity
        //burn
        uint8 totFee  = _burnFee + _walletFee + _devFee + _liquidityFee + _buybackFee; 
        ...
        if(_devFee != 0){
            ...
            _tokenTransferNoFee(address(this), feeDev, spentAmount);
            totSpentAmount = totSpentAmount + spentAmount;
        }        
