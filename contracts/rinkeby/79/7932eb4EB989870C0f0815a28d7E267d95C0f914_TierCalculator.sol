...
contract TierCalculator is AdminableUpgradeable {
    ...
    modifier onlyTokenOrTokenLpPool(address _pool) {
        address stakingToken = address(IStakingPool(_pool).stakingToken());

        // stakingToken is BLP or BLP UNI-V2 LP
        require(
            stakingToken == token ||
    ...
    function getTierIndex(address _user, address _deal) external view returns (bool success, uint256 tierIndex) {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            ...
            address stakingToken = address(stakingPool.stakingToken());

            // stakingToken is BLP
            if (stakingToken == token) {
                blpBalance = blpBalance.add(amountPool);
            }
            // stakingToken is BLP UNI-V2 LP
            else if (isTokenUniLp(IUniswapV2Pair(stakingToken))) {
                // calculate BLP amount on UNI-V2 LP
                uint256 tokenAmount = IERC20Upgradeable(token).balanceOf(stakingToken)
                    .mul(amountPool)
                    .div(IERC20Upgradeable(stakingToken).totalSupply());
                blpBalance = blpBalance.add(tokenAmount);
            }
        }

        // add user's locked BLP on locker to staked BLP (max MAX_POOLS length)
        for (uint256 i = 0; i < lockers.length; i++) {
            blpBalance = blpBalance.add(ILocker(lockers[i]).getLockedBLP(_user));
        }
