 ...
 */
contract ERC721CollectionImplementation is ERC721CollectionBase, ERC721Upgradeable, AdminControlUpgradeable {

    /**
     * Initializer
     */
    function initialize(string memory _name, string memory _symbol, uint16 tokenMax_, uint256 tokenPrice_, uint16 purchaseLimit_, uint256 presaleTokenPrice_, uint16 presalePurchaseLimit_, address signingAddress) public initializer {
        __ERC721_init(_name, _symbol);
        __Ownable_init();
        _initialize(tokenMax_, tokenPrice_, purchaseLimit_, presaleTokenPrice_, presalePurchaseLimit_, signingAddress);
    }

    /**
     * @dev See {IERC165-supportsInterface}.
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721CollectionBase, ERC721Upgradeable, AdminControlUpgradeable) returns (bool) {
        return ERC721CollectionBase.supportsInterface(interfaceId) || ERC721Upgradeable.supportsInterface(interfaceId) || AdminControlUpgradeable.supportsInterface(interfaceId);
    }
     ...
     */
    function balanceOf(address owner) public view virtual override (ERC721Upgradeable, ERC721CollectionBase) returns (uint256) {
     ...
     */
    function _mint(address to, uint256 tokenId) internal override (ERC721Upgradeable, ERC721CollectionBase) {
...
abstract contract ERC721CollectionBase is CollectionBase, IERC721Collection {
    ...
    function _initialize(uint16 tokenMax_, uint256 tokenPrice_, uint16 purchaseLimit_, uint256 presaleTokenPrice_, uint16 presalePurchaseLimit_, address signingAddress) internal {
    ...
    function _mint(address to) internal virtual {
        ...
        emit Unveil(tokenCount, address(this), tokenCount);
    }
    ...
    function purchase(uint16 amount, bytes32 message, bytes calldata signature, string calldata nonce) external payable virtual override {
        ...
        if (block.timestamp - startTime < presaleInterval) {
            require(balanceOf(msg.sender) == 0 && amount <= presalePurchaseLimit, "Too many requested");
            _validatePresalePrice(amount);
        } else {
            require(amount <= (purchaseLimit-balanceOf(msg.sender)), "Too many requested");
    ...
    function state() external override view returns(CollectionState memory) {
        return CollectionState(tokenMax, tokenMax-tokenCount, tokenPrice, purchaseLimit, presaleTokenPrice, presalePurchaseLimit, uint16(balanceOf(msg.sender)), active, startTime, endTime, presaleInterval);
    }
