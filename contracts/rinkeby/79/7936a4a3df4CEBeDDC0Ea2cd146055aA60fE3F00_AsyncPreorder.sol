...
/// @dev ERC1155 standard used for fungible (pre-order) and non-fungible (final minted NFT) tokens types
    ...
    OwnableUpgradeable
{
    ...
    // uint256 public defaultPlatformPrimarySalePercentage; // not needed.  = basisPointsDenominator - defaultArtistPrimarySalePercentage
    ...
    // event ArtistCutUpdated(uint256 tokenId, uint2556 preorderArtistPercentage);
    ...
    // function changeArtistCut(uint256 tokenId, uint256 _preorderArtistPercentage)
    //     external
    //     onlyOwner
    // {
    //     require(_preorderArtistPercentage <= 10000);
    //     preorderArtistPercentage[tokenId] = _preorderArtistPercentage;
    //     emit ArtistCutUpdated(tokenId, _preorderArtistPercentage);
    // }

    // Add more functions to change deafults etc etc.
    ...
    /// @param _masterTokenId Unique ID of the artwork preorder product
    ...
    function createPreorder(
        ...
        // only admin modifier etc...
        require(_numberOfEditions > 0 && _minimumPrice > 0);
        require(!isPreorder[_masterTokenId]); // is not already a master

        isPreorder[_masterTokenId] = true;

        numberOfTokens[_masterTokenId] = _numberOfEditions;
        pricePerToken[_masterTokenId] = _minimumPrice;
        artistAddress[_masterTokenId] = _artistAddress;

        // emit information in event here
    ...
    // Will customie contracts to allow flexible bps arragements for priotitized artists.
