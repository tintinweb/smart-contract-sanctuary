        ...
        if (epoch < BootstrapEpochs) {
            ...
            if (paprPrice > paprPriceCeiling) {
                ...
                if (seigniorageSaved >= inkSupply.mul(inkDepletionFloorPercent).div(10000)) {
                    ...
                    uint256 ratio = printingRatioMultiplier.mul(1e14).mul(_countTimesOverCents());
                    uint256 percentCombined = _percentage.mul(stepRatio.add(ratio));
                    uint256 _seigniorage = paprSupply.mul(percentCombined).div(1e20);

                    _savedForFund = percentCombined;
