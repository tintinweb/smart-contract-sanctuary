...
contract RealmTowerDefense {
  ...
  function buildTower(uint256 _realmId, uint256 _kind) external {
    ...
    emit BuildTower(_realmId, _kind, 1);
  }
  ...
  function sendWave(uint256 _realmId) external {
    ...
    uint256 _difficulty = 100 * defenders[_realmId].level;
    uint256 _wave = _random(_realmId, _difficulty) + _difficulty;
    uint256 threshold = defenders[_realmId].level;
    uint256 diff;

    if (defenders[_realmId].level < 10) {
      threshold = defenders[_realmId].level;
    } else {
      threshold =
        defenders[_realmId].level -
        ((defenders[_realmId].level / 10) * (defenders[_realmId].level % 11));
    }

    diff = _wave % defenders[_realmId].level;
