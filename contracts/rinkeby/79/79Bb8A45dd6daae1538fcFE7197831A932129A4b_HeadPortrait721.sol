...
// File: contracts\HeadPortrait721.sol
...
contract HeadPortrait721 is ERC721Enumerable, OwnableContract{

    using Strings for uint256;

    string public baseURI;

    mapping(uint256 => uint256[]) public tokenIdToAttributesMap;

    constructor() ERC721("ChickHeadPortrait","ChickHeadPortrait"){

    }

    function mint(address user, uint256 tokenId, uint256[] memory attributes) public onlyDev{
        require(tokenId > 1000, "tokenId is error.");
        _safeMint(user, tokenId);
        tokenIdToAttributesMap[tokenId] = attributes;
    }

    function mintOfAdmin(address user, uint256 tokenId, uint256[] memory attributes) public onlyAdmin{
        _safeMint(user, tokenId);
        tokenIdToAttributesMap[tokenId] = attributes;
    }
