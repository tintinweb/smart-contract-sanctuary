...
contract ShogunNFT is ERC721Enumerable, Ownable {
  ...
  address payable public treasury =
    payable(0xfD8A2B7747F8C1F00891e1765Ca60194e22f68e5); // treasury wallet address
  address public stakingContractAddress;

  string public baseURI; // to update after pinning to ipfs
  string public notRevealedUri; // should be set with constructor
  ...
  uint256 public presaleWindow = 0.25 hours; // 15 minutes
  uint256 public presaleStartTime = 1633190400; // 3rd October 0000
  uint256 public publicSaleStartTime = 1633192200; // 3rd October 0030

  bool public paused = false; // can remove
  ...
  modifier onlyStakingContract() {
    require(
      ...
      "Only callable from staking contract"
    );
    _;
  }
  ...
  // dev team mint
  function devMint(uint256 _mintAmount) public payable onlyEOA onlyOwner {
  ...
  // public sale
  function publicMint(uint256 _mintAmount) public payable onlyEOA {
    require(!paused); // checks if the contract is paused
    require(
    ...
    require(
      ...
      "exceeded max mint amount per transaction"
    );
    require(
      supply + _mintAmount <= maxSupply,
      "total mint amount exceeded supply, try lowering amount"
    ...
    require(
      ...
      "not enough ether sent for mint amount"
    );
  ...
  // presale mint
  function presaleMint(uint256 _mintAmount) public payable onlyEOA {
    require(!paused, "contract is paused");
    require(
      (isPresaleOpen() || presaleOpen),
      "presale has not started or it has ended"
    );
    ...
    require(
      msg.value >= cost * _mintAmount,
      "not enought ethere sent for mint amount"
    );
  ...
  //*************** PUBLIC FUNCTIONS ******************//
  // checks the TokenIds of what the address is holding
  ...
  // comes from an underlying library that we are inheriting from (OpenSea calls this)
    ...
    returns (string memory)
  {
    ...
    // return respective string depending on conditions
    if (!revealed) {
  ...
  function _beforeTokenTransfer(
    ...
    require(_isLocked[tokenId] == false, "ShogunSamurai: Token is Locked");
