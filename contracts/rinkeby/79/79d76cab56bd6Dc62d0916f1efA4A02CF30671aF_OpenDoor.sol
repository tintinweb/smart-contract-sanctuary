        ...
        bytes(
            string(
                abi.encodePacked(
                    ...
                    '","name": "Meta User Dungeon//',doorMetaData[uint256(keccak256(abi.encodePacked("RING", currentToken))) % 33],'", "attributes": [{"ring": "', doorMetaData[uint256(keccak256(abi.encodePacked("RING", currentToken))) % 33] ,'","element": "', doorMetaData[33 +uint256(keccak256(abi.encodePacked("ELEMENT", currentToken))) % 16] ,'", "suit": "', doorMetaData[49 +uint256(keccak256(abi.encodePacked("SUIT", currentToken))) % 4] ,'"}]}'
                )
            )
        )
    );
