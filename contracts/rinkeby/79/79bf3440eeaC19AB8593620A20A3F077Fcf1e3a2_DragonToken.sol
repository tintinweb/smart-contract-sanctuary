...
interface IChangeableVariables {
    event AddressChanged(bytes indexed fieldName, address previousAddress, address newAddress);
    event ValueChanged(bytes indexed fieldName, uint previousValue, uint newValue);
    event BoolValueChanged(bytes indexed fieldName, bool previousValue, bool newValue);
}