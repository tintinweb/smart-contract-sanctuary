...
contract NftWorld is ERC721Enumerable, Ownable {
  ...
  uint _reserved = 200;
  string _baseTokenURI = "https://api.kokeshi.world/";
  bool public isActive = false;

  constructor() ERC721("Nft World", "NFTWORLD") {}

  function mintAmulet(address _to, uint _count) public payable {
  ...
  function reserveAmulet(address _to, uint _count) public onlyOwner{
    require(totalSupply() + _count <= MAX_SUPPLY, "Max limit");
  ...
  function price(uint _count) public pure returns (uint256) {
    return _count * 50000000000000000;
  }
