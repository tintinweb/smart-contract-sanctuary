...
contract YELCrowdsale is Ownable {
    ...
    function buyTokens() public payable isPresale {
        ...
        (uint256 buyLimitTokens, uint256 buyLimitWei) = remainingTokens();
        require(buyLimitTokens > tokens, "The maximum transaction limit at the moment is: ");
    ...
    function remainingTokens() public view returns(uint256, uint256) {
        uint256 maxTokenPriceUSD = initialPrice * 2**5;
        uint256 _priceInWeiPerToken = getPriceInWeiPerToken(maxTokenPriceUSD);
        uint256 lowestPurchesedTokens = 1e18 * weiRaised / _priceInWeiPerToken;

        return (threshold - lowestPurchesedTokens, threshold * _priceInWeiPerToken / 1e18 - weiRaised);
    }
