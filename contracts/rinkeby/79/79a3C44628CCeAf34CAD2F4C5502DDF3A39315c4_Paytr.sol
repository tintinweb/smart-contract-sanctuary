...
contract Paytr{
    ...
    event PaymentInfo(address sender, uint256 amount, bytes32 paymentReference, address supplier, uint256 tokenAddress ); //To be added for Request: address to, invoice amount, payment reference, supplier, interest amount
    event PaymentInfoErc20(address sender, uint256 amount, bytes32 paymentReference, address supplier, address tokenAddress);
   ...
   constructor() public {
    ...
    function supplyEthToCompound(address payable _cEtherContract, uint amount, address supplier, uint dueDate, bytes32 paymentReference)
        public
        payable
        returns (bool)
    {
    ...
    function createDueListAndPay() public payable {
        ...
        for (uint i = 0; i < invoices.length; i++) {
             ...
             if (block.timestamp >= invoices[i].dueDate) {
                ...
                bytes32 paymentReference = invoices[i].paymentReference;
    ...
    function createDueListAndPayErc20() public payable {
        ...
        for (uint i = 0; i < invoicesErc20.length; i++) {
             ...
             if (block.timestamp >= invoicesErc20[i].dueDate) {
                ...
                bytes32 paymentReference = invoicesErc20[i].paymentReference;
        ...
        for (uint i = 0; i < dueInvoicesErc20.length; i++) {
            ...
            if (dueInvoicesErc20[i].supplier != 0x0000000000000000000000000000000000000000) {
                ...
                //address sender, invoice amount, payment reference, supplier, interest amount
                emit PaymentInfoErc20(dueInvoicesErc20[i].sender, dueInvoicesErc20[i].amount, dueInvoicesErc20[i].paymentReference, dueInvoicesErc20[i].supplier, erc20TokenAddress);
