...
contract TimeWarpPool is Ownable, StakingLocks {
    ...
    function reward(uint256 amount) external onlyOwner {
        ...
        uint256 stakedWithMultipliers = _stakedWithMultipliers();
        ...
        for (uint8 i = 0; i < lockTypes.length; i++) {
            ...
            uint256 percent = staked.mul(PRECISION).div(stakedWithMultipliers);
    ...
    function _stakedWithMultipliers() internal view returns (uint256) {
        uint256 reserves;
        for (uint8 i = 0; i < lockTypes.length; i++) {
