...
contract OmnuumNFT is ERC1155 {
    ...
    constructor(string memory uri, address managerContract) ERC1155(uri) {
        // @dev only mint manager contract can mint
        minterAddress = managerContract;
        // @dev set operator
        operatorAddress = msg.sender;

        OmnuumMintManager(minterAddress).registerNftContract(2, 0.1 ether, false);
    }

    function updateMinterAddress(address _managerContract) public {
        require(operatorAddress == msg.sender, "only OMNUUM can update minter address for emergency");
        minterAddress = _managerContract;
    }

    function mint(address to, uint16 amount, uint256 price) public payable returns (uint256[] memory) {
        require(amount >= 0, "amount should be above 0");
        require(msg.value >= price * amount, "not enough money");

        OmnuumMintManager(minterAddress).prepareMint(msg.sender, amount, msg.value, price);

        uint256[] memory tokenIds = new uint[](amount);

        if (amount == 1) {
            tokenIds[0] = baseMint(to);
        } else {
            for (uint32 i; i < amount; i++) {
                tokenIds[i] = baseMint(to);
            }
        }
    ...
    function mintBatch(address[] calldata tos, uint32[] calldata amounts) public payable returns (uint256[] memory) {
        uint256 length = tos.length;

        require(length == amounts.length, "address length and amount length not equal");

        // 총 발급해야 하는 토큰 갯수 구하기
        uint32 mintTokenCount = 0;
        for (uint16 i = 0; i < length; i++) {
            mintTokenCount += amounts[i];
        }

        require(length < 1000, "mint length cannot over 1000");
        require(_tokenIdCounter.current() + length <= maxSupply, "token count cannot over max amount");

        uint256[] memory tokenIds = new uint256[](mintTokenCount);
        uint256 startId = _tokenIdCounter.current() + 1;
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (amount > 1) {
                for (uint256 j = 0; j < amount; j++) {
                    ...
                    tokenIds[tokenId - startId] = tokenId;
    ...
    // mint core
    function baseMint(address to) public payable returns (uint256) {
