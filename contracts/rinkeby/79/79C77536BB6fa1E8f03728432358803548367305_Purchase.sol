...
contract Purchase is Whitelist,ReentrancyGuard {
    ...
    constructor(address _nft,address _x22,address _eth_usd,bool _isWhitelist) public Whitelist(_isWhitelist){
    ...
    function buyWithETH(uint256 _numberOfToken) public payable nonReentrant() whenNotPaused() onlyWhitlisted(){
        ...
        if(_safeTransferETH(treasureAddress,priceInETH.mul(_numberOfToken))){
    ...
    function buyWithX22(uint256 amount,uint256 _numberOfToken) public nonReentrant() whenNotPaused() onlyWhitlisted(){
        require (_numberOfToken > 0 && _numberOfToken <= nft.balanceOf(nftHolder),"Purchase: _numberOfToken must be greater than zero");
        require(amount >= amountToPay().mul(_numberOfToken), "Purchase: amount of x22 must be equal to token price");
        x22.transferFrom(msg.sender, treasureAddress, amountToPay().mul(_numberOfToken));
        nft.batchMint(msg.sender,_numberOfToken);
