...
/**
 * @title Interface for MettaCreative Protocol's Media
 */
interface IMedia {
 ...
 */
contract MettaMedia is IMedia, ERC721Burnable, ReentrancyGuard {
     ...
     */
    constructor(address marketContractAddr) public ERC721("METTA", "METTA") {
    ...
    function bulkmint( MediaData[] memory dataArray,IMarket.BidShares[] memory bidSharesArray) public nonReentrant {
        require(dataArray.length == bidSharesArray.length);
        for(uint i = 0; i < dataArray.length; i++){
            _mintForCreator(msg.sender, dataArray[i], bidSharesArray[i]);
        }
    }
