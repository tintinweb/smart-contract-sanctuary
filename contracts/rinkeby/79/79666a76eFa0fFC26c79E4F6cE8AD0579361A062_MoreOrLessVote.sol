...
contract MoreOrLessVote is ERC721Enumerable, Ownable {
    ...
    uint256 public highVotes;
    uint256 public lowVotes;

    uint public maxSupply = 19;

    string[2] votesEnumerated = ["LESS", "MORE"];
    ...
    struct Vote {       
        ...
        uint256[2] votesAtTime;
        uint256 nonce;
    }

    Vote[20] public voteInfos;

    mapping(address => uint256) public seeds;
    mapping(uint256 => uint256) public nonces;
    mapping(address => uint) public votes;
    address[20] public votedBy;
    string[20] public imageMetadata;
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        return string(abi.encodePacked(
            ...
            (tokenId).toString(),
            '",',
            '"description":"A social experiment by @yungwknd.",',
            '"image":"',
                voteInfos[tokenId].render,
            '", "attributes":[',
                getMetadata(tokenId),
            ']',
        '}'));
    ...
    function getMetadata(uint256 tokenId) public view returns (string memory) {
        string memory metadata = string(abi.encodePacked(
            imageMetadata[tokenId],
            ',',MoreOrLessArt._wrapTrait("Token Vote", votesEnumerated[votes[votedBy[tokenId]]]),
            ',',MoreOrLessArt._wrapTrait("Voted By", MoreOrLessArt.addressToString(votedBy[tokenId])),
    ...
    function mintHigh() public payable {
        // require(seeds[msg.sender] == 0, 'Cannot vote twice.');
        require(msg.value > 100000000, 'Not enough ETH');
        require(totalSupply() < maxSupply, 'No more voting');
        uint256 mintNum = totalSupply() + 1;
        _safeMint(msg.sender, mintNum);
        uint256 num = MoreOrLessArt.random(mintNum);
        seeds[msg.sender] = num;
        voteInfos[mintNum].nonce = num;
        votedBy[mintNum] = msg.sender;
        voteInfos[mintNum].votesAtTime = [highVotes, lowVotes];
        votes[msg.sender] = 1;
        voteInfos[mintNum].render = _generateImage(mintNum);
        tokens[msg.sender] = mintNum;
        voteInfos[mintNum].influencer = highVotes == lowVotes;
        highVotes++;
        voteInfos[mintNum].isTheDecider = highVotes == 10 && lowVotes < 10;
    }

    function mintLow() public payable {
        // require(seeds[msg.sender] == 0, 'Cannot vote twice.');
        ...
        voteInfos[mintNum].influencer = highVotes == lowVotes;
        lowVotes++;
        voteInfos[mintNum].isTheDecider = lowVotes == 10 && highVotes < 10;
    }

    function _generateImage(uint256 mintNum) public returns (string memory) {
        uint256 whichShape = MoreOrLessArt.seededRandom(0, 4, seeds[msg.sender], voteInfos[mintNum].nonce);
        voteInfos[mintNum].nonce++;
        uint numRects = MoreOrLessArt.seededRandom(0,10, seeds[msg.sender], voteInfos[mintNum].nonce);
        voteInfos[mintNum].nonce++;
        uint numCircles = MoreOrLessArt.seededRandom(0,10, seeds[msg.sender], voteInfos[mintNum].nonce);
        voteInfos[mintNum].nonce++;
        uint numTriangles = MoreOrLessArt.seededRandom(0,10, seeds[msg.sender], voteInfos[mintNum].nonce);
        voteInfos[mintNum].nonce++;
        uint numLines = MoreOrLessArt.seededRandom(0,10,seeds[msg.sender], voteInfos[mintNum].nonce);
        voteInfos[mintNum].nonce++;
        string memory triangles = MoreOrLessArt._generateTriangles(seeds[msg.sender], numTriangles);
        string memory circles = MoreOrLessArt._generateCircles(seeds[msg.sender], numCircles);
        string memory rectangles = MoreOrLessArt._generateRectangles(seeds[msg.sender], numRects);
        string memory lines = MoreOrLessArt._generateLines(seeds[msg.sender], numLines);

        if (whichShape == 0) {
            imageMetadata[mintNum] = string(abi.encodePacked(
                imageMetadata[mintNum],
                MoreOrLessArt._wrapTrait("Circles", numCircles.toString()),
                ',',MoreOrLessArt._wrapTrait("Lines", numLines.toString())
            ));
            return string(abi.encodePacked(MoreOrLessArt._generateHeader(mintNum), circles, lines, MoreOrLessArt._imageFooter));
        } else if (whichShape == 1) {
            imageMetadata[mintNum] = string(abi.encodePacked(
                imageMetadata[mintNum],
                MoreOrLessArt._wrapTrait("Rectangles", numRects.toString()),
                ',',MoreOrLessArt._wrapTrait("Lines", numLines.toString())
            ));
            return string(abi.encodePacked(MoreOrLessArt._generateHeader(mintNum), rectangles, lines, MoreOrLessArt._imageFooter));
        } else if (whichShape == 2) {
            imageMetadata[mintNum] = string(abi.encodePacked(
                imageMetadata[mintNum],
                MoreOrLessArt._wrapTrait("Triangles", numTriangles.toString()),
                ',',MoreOrLessArt._wrapTrait("Lines", numLines.toString())
            ));
            return string(abi.encodePacked(MoreOrLessArt._generateHeader(mintNum), triangles, lines, MoreOrLessArt._imageFooter));
        }
        ...
        return string(abi.encodePacked(MoreOrLessArt._generateHeader(mintNum), circles, triangles, rectangles, lines, MoreOrLessArt._imageFooter));
    }
...

{
  ...
  "libraries": {
    "contracts/MoreOrLessArt.sol": {
      "MoreOrLessArt": "0xb46493c3785d188ffcfed2b0039304cf68754246"
    }
  }
}