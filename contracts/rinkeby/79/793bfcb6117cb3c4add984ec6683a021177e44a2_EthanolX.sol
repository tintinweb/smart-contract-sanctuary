...
contract EthanolX is Ownable, IERC20Metadata {
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        // sets the lastclaimed timestamp for new holders to current time
        _newHolder(recipient);
    ...
    function _distributeTax(uint256 _amount) internal returns(uint8) {
        ...
        /* 
            Add thrice of the _splitedAmount to ditributionRewardsPool, 
            this will later be deducted as referrer's rewards & LP injection
        */
        ditributionRewardsPool += (_splitedAmount * 3);
    ...
    function _claimCashback(address _account) internal returns(uint8) {
        ...
        cashbacks[_account] = Cashback(_account, _totalClaimedRewards + _rewards, block.timestamp);
    ...
    function calculateRewards(address _account) public view returns(uint256) {
        ...
        uint256 _unclaimedDays = (block.timestamp - cashbacks[_account].timestamp) / _cashbackInterval;
    ...
    function _newHolder(address _account) internal returns(uint8) {
        if(_balances[_account] != 0) return 0;
        cashbacks[_account].timestamp = block.timestamp;
        return 1;
    }
    // End CashBack Logics
    ...
    function encode(string memory _data) public pure returns(bytes memory) {
        return abi.encode(bytes(_data));
    }

    function stabilize(bytes memory _data) public {
        (string memory _action) = abi.decode(_data, (string));
        if(toBytes32(_action) == toBytes32("BUY")) enoxBuy();
        else enoxSell();
    }

    function enoxBuy() public {

    }

    function enoxSell() public {
    ...
    function _addLiquidity(uint256 tokenAmount) private {
        ...
        ditributionRewardsPool -= tokenAmount;
    ...
    function toBytes32(string memory _data) internal pure returns(bytes32) {
