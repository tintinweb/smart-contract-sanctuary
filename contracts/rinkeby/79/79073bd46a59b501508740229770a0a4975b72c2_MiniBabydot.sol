...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public dividendToken = 0xc778417E063141139Fce010982780140Aa0cD5Ab;
...
contract MiniBabydot is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public immutable uniswapV2Pair;

    address public _dividendToken = 0xc778417E063141139Fce010982780140Aa0cD5Ab;
    ...
    MiniBabydotDividendTracker public dividendTracker;

    address public buyBackWallet;
    
    uint256 public maxBuyTranscationAmount = 100000000 * (10**18); // 0.1% of total supply
    uint256 public maxSellTransactionAmount = 100000000 * (10**18);// 0.1% of total supply
    uint256 public swapTokensAtAmount = 20000000 * (10**18);
    uint256 public maxWalletToken = 1000000000 * (10**18); // 1% of total supply
    ...
    constructor() ERC20("MiniBabydot", "MiniBabydot") {
        uint256 _dividendRewardsFee = 11;
        ...
        dividendTracker = new MiniBabydotDividendTracker();

        buyBackWallet = 0xEbF0C5f50C8aaea52057345D866692dD4625F4Bc;
    ...
    function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
        require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "MiniBabydot: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
        sellFeeIncreaseFactor = multiplier;
    }
    
    function setMarketingDivisor(uint256 divisor) external onlyOwner {
        require(marketingDivisor >= 0 && marketingDivisor <= 100, "MiniBabydot: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function afterPreSale() external onlyOwner {
        dividendRewardsFee = 11;
        marketingFee = 7;
        maxBuyTranscationAmount = 500000000 * (10**18);
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "MiniBabydot: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "MiniBabydot: The dividend tracker already has that address");

        MiniBabydotDividendTracker newDividendTracker = MiniBabydotDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MiniBabydot: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "MiniBabydot: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MiniBabydot: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MiniBabydot: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MiniBabydot: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MiniBabydot: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "MiniBabydot: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MiniBabydot: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MiniBabydot: Cannot update gasForProcessing to same value");
...
contract MiniBabydotDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("MiniBabydot_Dividend_Tracker", "MiniBabydot_Dividend_Tracker") {
        claimWait = 1800;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "MiniBabydot_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "MiniBabydot_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MiniBabydot contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "MiniBabydot_Dividend_Tracker: claimWait must be updated to between 0.5 and 24 hours");
        require(newClaimWait != claimWait, "MiniBabydot_Dividend_Tracker: Cannot update claimWait to same value");
