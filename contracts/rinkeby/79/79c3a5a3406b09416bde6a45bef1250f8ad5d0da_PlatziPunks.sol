...
contract PlatziPunksDNA  {
    ...
    function deterministicPseudoRandomDNA(uint256 _tokenId, address _minter)  public pure
    returns(uint256)
    {
        uint256 combinedParams = _tokenId + uint160(_minter);
        bytes memory encodedParams = abi.encodePacked(combinedParams);
        //Keccack256 toma entrada y da una salida
...
contract PlatziPunks is ERC721, ERC721Enumerable, PlatziPunksDNA   {
    ...
    function _paramsURI(uint256 _dna) internal view returns (string memory){
        ...
        {
        params =string(abi.encodePacked("accessoriesType=",
