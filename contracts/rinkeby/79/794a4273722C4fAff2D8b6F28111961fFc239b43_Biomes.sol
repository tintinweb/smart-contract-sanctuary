...
contract Biomes is ERC721, ERC721Enumerable, Ownable {
    ...
    event BiomeSwapped(uint256 tokenId);

    constructor() ERC721("Biomes", "BIOMES") {
        _swapStarted = false;
    }

    modifier whenSwapStarted() {
        require(_swapStarted);
        _;
    }

    function toggleSwapStarted() external onlyOwner {
        _swapStarted = !_swapStarted;
    }
    ...
    // TODO: remove in prod
    function returnRawId(uint256 _id) public pure returns(uint256) {
    ...
    // TODO: REMOVE IN PROD
    function swapBiomeRinkeby(uint256 _tokenId) external {
        uint256 id = 32 + totalSupply();

		_safeMint(msg.sender, id);
		OPENSEA_STORE.safeTransferFrom(msg.sender, burn, _tokenId, 1, "");

		emit BiomeSwapped(id);
    }
