...
import "./DeckNFTBase.sol";

contract DeckNFT is DeckNFTBase {
    ...
    constructor(
        ...
        public
        DeckNFTBase(_mintFeePerToken, _maxDeck)
    {
    ...
    function tokenURI(uint256 _optionId) public view virtual override returns (string memory) {
        return string(abi.encodePacked(defaultURI, Strings.toString(_optionId)));
    }
     ...
     */
    function _mintDeck(uint256 numberToken) internal returns (bool) {
        for (uint256 i = 0; i < numberToken; i++) {
            uint256 tokenIndex = totalSupply();
            if (tokenIndex < MAX_DECK) _safeMint(_msgSender(), tokenIndex);
        }
        return true;
    }

    function mintDeck(uint256 numberToken)
        public
        payable
        online
...
/**
 * @dev DeckNFTBase contract.
 * @notice Setup admin control functional, include MINT_FEE_PER_TOKEN, and Total Supply
 */
contract DeckNFTBase is Ownable, ERC721 {
    uint256 public constant LIMIT_TOKEN_PER_TX = 10;

    uint256 public MAX_DECK;
    ...
    /**
     * @param _mintFeePerToken The Fee (in ETH) collector/user pays for Deck when one token is minted.
     * @param _MAX_DECK Total supply of contract
     */
    constructor(uint256 _mintFeePerToken, uint256 _MAX_DECK)
        public
        ERC721("Deck", "FDT")
    {
        MAX_DECK = _MAX_DECK;
    ...
    modifier mintable(uint256 numberToken, uint256 totalSupply) {
        ...
        require(totalSupply.add(numberToken) < MAX_DECK, "Sold out");
        _;
    }
