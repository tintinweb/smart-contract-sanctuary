...
contract NFTicketContest is Ownable, VRFConsumerBase {
    ...
    /** ADDRESSES */
    address private serverWalletAddress;
    ...
    uint256 public MINTS_PER_USER = 50;
    uint256 public PRE_MINTS_PER_USER = 10;
    uint256 public MINT_PRICE = 0.05 ether;
    uint256 public PRE_MINT_PRICE = 0.05 ether;
    uint256 public WINNER_ALLOCATION = 7;
    uint256 public OWNER_ALLOCATION = 3;
    uint256 public ALLOCATION_MULTIPLIER = 10;
    uint256 public activeContestId;

    /** MAPPINGS */
    mapping(uint256 => mapping(address => uint256)) public addressToMints; 
    mapping(uint256 => mapping(uint256 => uint256)) private nftToSlot;
    mapping(uint256 => mapping(uint256 => uint256)) private slotToNft;
    ...
    mapping(uint256 => mapping(uint256 => uint256)) private mintCache;
    ...
    modifier isMintingPeriod() {
        ...
        require(block.timestamp >= _contest.preMintStart, "Contest not started");
        require(block.timestamp <= _contest.mintStart + _contest.mintDuration, "Mint phase over");
        _;
    }
    ...
    modifier isNextEliminationPeriod() {
        ...
        require(block.timestamp >= _contest.mintStart + _contest.mintDuration + (_contest.eliminationDuration * _eliminationIndex), "Elimination round not active.");        
        _;
    }
    ...
    modifier canFinalize() {
        ...
        require(!_contest.finalized, "Contest already finalized");
        _;
    }

    modifier checkMintProof(uint256 contest, bytes32[] calldata proof) {
        Contest memory _contest = contests[activeContestId];

         if (block.timestamp < _contest.mintStart && block.timestamp >= _contest.preMintStart) {
            bytes32 leaf = keccak256(abi.encodePacked(msg.sender, contest));
            require(contest == activeContestId, "NOT ACTIVE CONTEST");
    ...
    modifier onlyServerWallet() {
        require(msg.sender == serverWalletAddress, "NOT ALLOWED");
        _;
    }
    ...
    function drawSlot(uint256 remaining, uint256 contestId) internal returns (uint256 index) {
        //RNG
        uint256 i = uint256(keccak256(abi.encodePacked(msg.sender, remaining, contestId, block.timestamp))) % remaining;
    ...
    function newContest(uint256 preMintStart, uint256 mintStart, uint256 mintPeriod, uint256 eliminationPeriod) external onlyOwner {
        uint256 startIndex = nfticket.nextTokenId();
        if (contests.length > 0) {
            Contest memory _lastContest = contests[activeContestId];
            require(_lastContest.eliminationsLeft == 0, "CONTEST STILL ACTIVE");        
            startIndex = _lastContest.ticketStartIndex + _lastContest.ticketCount;
            nfticket.setNextTokenId(startIndex);
        }

        Contest memory _contest = Contest({
            ticketStartIndex: startIndex,
            ...
            eliminationDuration: eliminationPeriod,
    ...
    function mint(uint256 amount, uint256 contestId, bytes32[] calldata proof) external isMintingPeriod checkMintProof(contestId, proof) {
        ...
        bool isPreSale = block.timestamp < _contest.mintStart && block.timestamp >= _contest.preMintStart;

        require(_nextMint + amount <= _lastMintId, "Max NFTs minted for this contest");         
        
        if (isPreSale) {
            require(WETH.allowance(msg.sender, address(this)) >= PRE_MINT_PRICE * amount, "WETH SENT NOT CORRECT");
            require(addressToMints[activeContestId][msg.sender] + amount <= PRE_MINTS_PER_USER, "Sender already minted max");
        } else {
        ...
        addressToMints[activeContestId][msg.sender] += amount;
    ...
    function triggerElimination() external isNextEliminationPeriod {
        ...
        uint256 decision = contestToVrf[activeContestId][_eliminationIndex] % 2;
        if (decision == 0) {
        ...
        emit Elimination(_eliminationIndex, _contest.eliminationsLeft);
    }

    function finalize() external canFinalize {
        Contest storage _contest = contests[activeContestId];
        _contest.finalized = true;
        ...
        if (nfticket.exists(_winnerNFT)) {
            address _winner = nfticket.ownerOf(_winnerNFT);
            _contest.winnerAddress = _winner;

            uint256 _toTransferWinner = WETH.balanceOf(address(this)) * WINNER_ALLOCATION / ALLOCATION_MULTIPLIER;
            uint256 _toTransferOwner = WETH.balanceOf(address(this)) * OWNER_ALLOCATION / ALLOCATION_MULTIPLIER;

            WETH.transfer(_winner, _toTransferWinner);
            WETH.transfer(owner(), _toTransferOwner);

            emit Finalize(_winner, _winnerNFT, _toTransferWinner);
        } else {
            emit Finalize(address(0), _winnerNFT, 0);
        }
    }
    ...
    function viewSlotOfTicket(uint256 contestId, uint256 tokenId) external view onlyOwner returns (uint256) {
        return nftToSlot[contestId][tokenId];
    }

    function viewTicketOfSlot(uint256 contestId, uint256 slotId) external view onlyOwner returns (uint256) {
        return slotToNft[contestId][slotId];
    }
    ...
    function isTokenBurned(uint256 tokenId) public view returns (bool) {
        ...
        if (slot == 0 && slotToNft[contestId][slot] != tokenId) { // if token has not been minted return true
            return true;
        } else {
    ...
    function getAliveTickets() external view returns(uint256[] memory) {
        require(contests.length > 0, "NO CONTEST EXISTS");
        Contest memory _contest = contests[activeContestId];
        uint256 _eliminationIndex = ELIMINATION_ROUNDS - _contest.eliminationsLeft;
        uint256 amountAlive = (NFT_PER_CONTEST / (2 ** (_eliminationIndex)));
        uint256[] memory aliveTickets = new uint256[] (amountAlive);
        uint256 _index;

        require(amountAlive == _contest.eliminatedUpperBound - _contest.eliminatedLowerBound, "CALCULATION INCORRECT.");

        for (uint i = _contest.eliminatedLowerBound; i < _contest.eliminatedUpperBound; i++) {
            uint256 ticketIndex = slotToNft[activeContestId][i];
            aliveTickets[_index] = ticketIndex;
            _index++;
        }
    ...
    function setWinnerAllocation(uint256 _newWinnerAllocation) external onlyOwner {
        WINNER_ALLOCATION = _newWinnerAllocation;
    }

    function setOwnerAllocation(uint256 _newOwnerAllocation) external onlyOwner {
        OWNER_ALLOCATION = _newOwnerAllocation;
    }

    function setAllocationMultiplier(uint256 _newAllocationMultiplier) external onlyOwner {
        ALLOCATION_MULTIPLIER = _newAllocationMultiplier;
    }
    ...
    function setServerWalletAddress(address _newAddress) external onlyOwner {
        serverWalletAddress = _newAddress;
    }

    function setLastMintStart(uint256 _newStart) external onlyOwner {
         Contest storage _contest = contests[activeContestId];
         _contest.mintStart = _newStart;
    }

    function setLastContestMintDuration(uint256 _newMintDuration) external onlyOwner {
        Contest storage _contest = contests[activeContestId];
         _contest.mintDuration = _newMintDuration;
    }

    function setLastContestEliminationPeriod(uint256 _newElimininationPeriod) external onlyOwner {
        Contest storage _contest = contests[activeContestId];
         _contest.eliminationDuration = _newElimininationPeriod;
    }
