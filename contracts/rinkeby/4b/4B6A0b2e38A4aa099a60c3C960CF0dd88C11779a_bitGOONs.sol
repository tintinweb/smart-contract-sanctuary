...
abstract contract BasedFishMafia  {
    function balanceOf(address ownew) view public virtual returns (uint);
}
abstract contract ZombieFishMafia {
    function balanceOf(address ownew) view public virtual returns (uint);
}
...
contract bitGOONs is ERC721, Ownable {
  ...
  //constants
    uint public holdersaleCost = 0.05 ether;
    uint public presaleCost = 0.05 ether;
    uint public publicsaleCost = 0.055 ether;
    uint public maxSupply = 5000;
    uint private holderMaxMintAmount = 10;
    uint private publicMaxMintAmount = 100;
  ...
  //variable s
    ...
    address public BFMaddress;
    address public ZFMaddress;
  ...
  //Actions
    ...
    event UnrevealedURIChanged(string unrevealedURL);

    constructor () ERC721("bTGNtest", "btgn") {   
    ...
    function mint_function(uint _mintAmount) internal {
        ...
        require(current + _mintAmount <= maxSupply,"You are exceeding the max supply ");
  ...
  // public
    ...
    function holderSaleMint(uint _mintAmount) external payable{
        ...
        BasedFishMafia BFM = BasedFishMafia(BFMaddress);
        ZombieFishMafia ZFM = ZombieFishMafia(ZFMaddress);
        uint BFMBalance = BFM.balanceOf(msg.sender);
        uint ZFMBalance = ZFM.balanceOf(msg.sender);
        require(!(BFMBalance==0 && ZFMBalance==0),"You dont own any BFM or ZFM");

        if (!holderSaleUsed[msg.sender]){
            holderSaleUsed[msg.sender]=true;
            holderSaleRemaining[msg.sender]=holderMaxMintAmount;
        }
        require(holderSaleRemaining[msg.sender]-_mintAmount>=0,"Max Mint amount Reached - 10");
    ...
    function preSaleMint(uint _mintAmount, uint _MaxMintAmount,bool _earlyInvest, bytes32[] memory proof) external payable {
        require(presale, "Presale not activated");
        require(_earlyInvest || msg.value >= presaleCost * _mintAmount,"Insufficient Funds!");
        require( MerkleProof.verify(proof,merkleRoot, keccak256(abi.encodePacked(msg.sender,_MaxMintAmount,_earlyInvest))),"You're not in the whitelist!");
        
        if (!whitelistUsed[msg.sender]){
            whitelistUsed[msg.sender]=true;
            whitelistRemaining[msg.sender]=_MaxMintAmount;
        }
        require(whitelistRemaining[msg.sender]-_mintAmount>=0,"You minted the max amount");
    ...
    function publicSaleMint(uint _mintAmount) external payable {
        ...
        require(publicMaxMintAmount >= publicSaleMinted[msg.sender]+_mintAmount,"Max Mint amount Reached");
    ...
    function setUnrevealedURI(string memory _newUnrevealedURI) public onlyOwner() {
        UnrevealedURL = _newUnrevealedURI;
        emit UnrevealedURIChanged(_newUnrevealedURI);
    }

    function setPhase(uint phase) public onlyOwner() {
        require (phase<4,"Select Sale Phase");
        if (phase==0) paused=!paused;
        if (phase==1) presale=!presale;
        if (phase==2) publicsale=!publicsale;
        if (phase==3) holdersale=!holdersale;
    }

    function setPrice(uint pricephase, uint price) public onlyOwner() {
        require (pricephase<4,"Set Price");
        if (pricephase==0){
            publicsaleCost=price;
            presaleCost=price;
            holdersaleCost=price;
        }
        if (pricephase==1) publicsaleCost=price;
        if (pricephase==2) presaleCost=price;
        if (pricephase==3) holdersaleCost=price;
