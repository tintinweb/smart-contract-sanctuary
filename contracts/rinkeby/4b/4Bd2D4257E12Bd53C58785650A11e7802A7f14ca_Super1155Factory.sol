...
/**
  @title A basic smart contract for spawning and tracking the ownership of SuperFarm Super1155.
  @author 0xthrpw;

  This is the governing registry of all SuperFarm Child Super1155s.
*/
contract Super1155Factory {

  /// A mapping for an array of all Super1155s deployed by a particular address.
  mapping (address => address[]) public itemRecords;

  /// A mapping of a Super1155 address to its corresponding farm address
  mapping (address => address) public shopMap;

  /// An event for tracking the creation of a new Super1155.
  event ItemCreated(address indexed itemAddress, address indexed shopAddress, address indexed creator);

  /**
    Construct a new Super1155 for use with a mintshop pool.

    @param _proxyRegistryAddress The proxy address used by the item.
    @param _shop The address of the staker contract associated with this item
    @param _name The item collection name
    @param _uri The metadata uri used by the collection
    */
  ...
  function createItem(
    ...
    Super1155 newItem = new Super1155(_name, _uri, _proxyRegistryAddress);

    Super1155.ItemGroupInput memory newItemConfig = Super1155.ItemGroupInput({
      name: _name,
      supplyType: Super1155.SupplyType.Uncapped,
  		supplyData: 0,
  		itemType: Super1155.ItemType.Semifungible,
  		itemData: _totalCap,
  		burnType: Super1155.BurnType.Replenishable,
  		burnData: 0
    });

    newItem.configureGroup(1, newItemConfig);

    newItem.setPermit(
      _shop,
      0xffffffffffffffffffffffffffffffff00000000000000000000000000000000, //UNIVERSAL()
      0xfdf81848136595c31bb5f76217767372bc4bf906663038eb38381131ea27ecba, //MINT()
      type(uint256).max
    );

    newItem.transferOwnership(msg.sender);
    address newItemAddress = address(newItem);
    itemRecords[_shop].push(newItemAddress);
    shopMap[newItemAddress] = _shop;
    emit ItemCreated(newItemAddress, _shop, msg.sender);
    return newItemAddress;
  }

  /**
    Get the number of entries in the Super1155 records mapping for the given user.

    @param _shop The address of the pointshop deployer
    @return The number of Super1155s added for a given address.
  */
  function getItemCount(address _shop) external view returns (uint256) {
    return itemRecords[_shop].length;
  }
