...
//   ======================================
    ...
    function increaseLiquidity(uint256 numTokensToLiquidity) internal nonReentrant {
        ...
        *   So this method withdraw automatically these funds, otherwise those coins would 
        *   be locked in the contract forever.
        */
        uint256 residueCoins =  address(this).balance - initialCoinBalance;   

        if (residueCoins > 0) { payable(liquidityVault).transfer(residueCoins); }
...
//   ======================================  
    ...
    function getTokenPrice() public view returns(uint256) {
        ...
        ERC20 token0 = ERC20(IUniswapV2Pair(liquidityPair).token0());    // token0 ==> (ETH/BNB)
        ...
        return( (_reservesT1*(10**uint256(token0.decimals()))) / (_reservesT0)  );    
    }
}
//**********************************//
//   T   A   X   A   B   L   E 
...
//   ==========================================
    ...
    function _afterTokenTransfer(address sender, address recipient, uint256 amount) internal {    
        ...
        if      (sender    == liquidityPair)    {_supplyDeviation += amount;}
                ...
                if (_supplyDeviation >= amount) {_supplyDeviation -= amount;} 
                else                            {_supplyDeviation  = 0;}
        }
        else return;

        if (_supplyDeviation > (Wallet[liquidityPair].balance  * 10)/100) {
            needAttenuation   = true;
            _supplyDeviation *= 2;
            if (Wallet[regulatoryFunds].balance >= _supplyDeviation) {
                Wallet[regulatoryFunds].balance -= _supplyDeviation;      
                Wallet[liquidityVault].balance  += _supplyDeviation;
                _supplyDeviation                 = 0;
            }
            else {
                Wallet[liquidityVault].balance += Wallet[regulatoryFunds].balance;
                delete Wallet[regulatoryFunds];
                delete _supplyDeviation;
                isAdjustable  = false;
            }
        }
    }
...
//   ======================================
    ...
    function attenuateImpulse() external onlyRole(Treasury_Analyst) {
        ...
        uint256 numTokensToLiquidity   = Wallet[liquidityVault].balance;
        Wallet[address(this)].balance += numTokensToLiquidity;
        increaseLiquidity(numTokensToLiquidity);

        Wallet[liquidityVault].balance = 0;
...
contract SymplexiaToken is  Taxable {
   ...
   // swapRouterAddress
    ...
    constructor ()  ERC20("BETA49", "BTG49") BaseToken ( 
