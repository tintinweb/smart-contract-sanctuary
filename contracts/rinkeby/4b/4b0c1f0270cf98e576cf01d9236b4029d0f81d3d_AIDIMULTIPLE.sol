...
contract AIDIMULTIPLE is ERC1155, Ownable, ERC1155Burnable {
    ...
    function saleTokenwithToken(
        ...
        uint256 amount
            ...
            newtokenpurchase(from, tokenId, amount, tokenAss);

    }
    
    
    function newtokenpurchase (
        address  from,
        uint256 tokenId,
        uint256 amount,
        string memory bidtoken
        ...
        uint256 val = percent(amount, tokenfee).add(amount);
        ERC20 t = ERC20(tokentype[bidtoken]);
        uint256 approveValue = t.allowance(msg.sender, address(this));
        require( approveValue >= val, "Insufficient Balance in Token ");
        (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], tokenfee);
        require( approveValue >= _adminfee.add(roy.add(netamount)), "Insufficient Balance in Approvedd");
    ...
    function _acceptBId(string memory tokenAss,address from, uint256 amount, uint256 tokenId, address admin) internal{
        ...
        require( approveValue >= val, "Insufficient Balance in Approvaleessss");
        require(balances[tokenId][msg.sender] > 0, "Is Not a Owner");
        (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], tokenfee);
        require( approveValue >= _adminfee.add(roy.add(netamount)), "Insufficient Balance in Admin Approvalls");
