...
contract BlockClock is ERC721EnumerableEssential, Ownable, LinearDutchAuction {
    ...
    constructor()
        ...
        LinearDutchAuction(
            LinearDutchAuction.DutchAuctionConfig({
                startPoint: block.timestamp, // 0 for disabled upon deployment, or a timestamp for release? - update via config?
                startPrice: 2 ether,
                unit: AuctionIntervalUnit.Time,
                decreaseInterval: 60, // 1 minutes
                decreaseSize: 0.025 ether,
                numDecreases: 77 // rests at 0.075
            }),
            0.075 ether,
            Seller.SellerConfig({
                totalInventory: 1024,
    ...
    function publicMintWithConfig(int8 offset, uint24 onHex) public payable {
        _purchase(msg.sender, 1);
        clockConfig[nextTokenId] = ClockConfig({hourOffset: offset, onHex: onHex});
    }

    function claimGift(int8 offset, uint24 onHex) public {
    ...
    function setAnimationBaseUrl(string memory newBaseUrl) public onlyOwner {
