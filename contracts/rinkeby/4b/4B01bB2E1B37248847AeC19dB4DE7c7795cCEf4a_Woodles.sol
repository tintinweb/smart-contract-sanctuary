...
contract Woodles is ERC721, Ownable {
    using Counters for Counters.Counter;

    uint256 public constant MAX_TOKEN_COUNT = 5555;
    uint256 public constant MAX_TOKEN_COUNT_PER_TX = 20;
    ...
    mapping(address => bool) public freeMintingAddresses;
    bool public isSaleActive = true;
    uint256 public saleStartTimestamp = 1; // TODO date
    uint256 public saleEndTimestamp = 2; // TODO date maybe auto init or in constructor
    uint256 public startIndexBlock;
    uint256 public startIndex;

    constructor() ERC721("Woodles", "Woodles") {}
    ...
    function seedFreeMintingAddresses(address[] memory addresses)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < addresses.length; i++) {
            freeMintingAddresses[addresses[i]] = true;
        }
    }
    ...
    function setSaleStartTimestamp(uint256 newSaleStartTimestamp)
        external
        onlyOwner
    {
    ...
    function setSaleEndTimestamp(uint256 newSaleEndTimestamp)
        external
        onlyOwner
    {
        saleEndTimestamp = newSaleEndTimestamp;
    }
        ...
        precheckSaleIsActive
    {
        ...
        if (freeMintingAddresses[msg.sender]) {
            price -= MINT_PRICE;
        }
        require(msg.value >= price, "Too less ether sent");
        if (freeMintingAddresses[msg.sender]) {
            freeMintingAddresses[msg.sender] = false;
        }
    ...
    function setStartIndex() public {
        require(startIndex == 0, "Start index already set");
        require(startIndexBlock != 0, "Start index block not set");
        startIndex = uint256(blockhash(startIndexBlock)) % MAX_TOKEN_COUNT;
        if (block.number - startIndexBlock > 255) {
            // Function got called late (EVM only stores last 256 block hashes)
            startIndex = uint256(blockhash(block.number - 1)) % MAX_TOKEN_COUNT;
        }
        if (startIndex == 0) {
            startIndex = 1;
        }
    ...
    function setStartIndexBlockInEmergency() public onlyOwner {
        require(startIndex == 0, "Start index already set");
        startIndexBlock = block.number;
    }
