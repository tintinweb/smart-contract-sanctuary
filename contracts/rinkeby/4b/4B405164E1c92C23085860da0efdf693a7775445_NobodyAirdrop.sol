...
contract NobodyAirdrop is ERC721, Ownable {
  ...
  address public nobodyaddress = 0xF18972281A4F2fef7d0a4A4FF5d0Aee48e181B76; 

  nobodyinterface nobodycontract = nobodyinterface(nobodyaddress);  
    
  address public creator = 0x264fFE58b1a497FEbE7A45D9D70B5Ca9b7Cf27C8; 

  mapping(address => uint256) presaleAllowance;

  constructor (uint _maxNobody, uint _maxNobodyPerPurchase ) ERC721("nobodyairdrop", "nobodyairdrop") {
  ...
  function presalemint(uint256 _numNobody ) public payable {
    require(isPreSaleActive, "Sale is not active!" );
    require(isWhitelisted(msg.sender,_numNobody), "Insufficient reserved tokens for your address");
    ...
    _mintnobody(msg.sender, _numNobody);
    presaleAllowance[msg.sender] -= _numNobody;
  }

  function mint(uint256 _numNobody ) public payable {
  ...
  function _mintnobody(address _to, uint256 _numNobody ) internal {
  ...
  function airdropNobody(address _to, uint256 _numNobody ) public onlyOwner {        
      _mintnobody(_to, _numNobody);
  }

  function airdroplist(address[] memory _to) public onlyOwner {
     for (uint256 i = 0; i < _to.length; i++) {
        _safeMint(_to[i], numTokens);
        numTokens += 1;
     }
  }
