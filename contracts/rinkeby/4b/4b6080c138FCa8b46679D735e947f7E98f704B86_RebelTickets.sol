...
contract RebelTickets is ERC20, ReentrancyGuard {
    ...
    mapping(address => mapping(uint => bool)) public isTokenRewarded;
    ...
    function claim(IERC721Enumerable minterContract, uint8 scale) internal {
        ...
        for (uint i = 0; i < balance; i++) {
            ...
            if (!isTokenRewarded[address(minterContract)][tokenId]) {
                notRewardedTokens += 1;
            }
        }
        require(notRewardedTokens > 0, "");
        uint rewardPerToken = rewards[notRewardedTokens > 15 ? 15 : notRewardedTokens];
        _mint(_msgSender(), rewardPerToken * notRewardedTokens / scale);
    }
