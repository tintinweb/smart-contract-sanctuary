...
contract Finan  is Ownable{
    ...
    modifier withdrawMax (uint _cycleId,uint _amount) {
      ...
      if(keccak256(abi.encodePacked(signCycle[_cycleId].name)) == keccak256(abi.encodePacked("KBKC"))){
        if(uniPlay.balanceOf(address(this)).sub(stakingKbkcCount) > _amount*decimals){
          allow = true;
        }
      }
      if(keccak256(abi.encodePacked(signCycle[_cycleId].name)) == keccak256(abi.encodePacked("ETH/KBKC"))){
        if(kbkcPlay.balanceOf(address(this)).sub(stakingUniCount) > _amount*decimals){
          allow = true;
        }
      }
    ...
    function setExtractionShare(uint _share,uint _decimals) public onlyOwner{
        extractionShare = _share * 10 ** _decimals;
    }
    ...
    function profitComput(uint _amount,uint _cycleId) public view returns (uint) {
        if(keccak256(abi.encodePacked(signCycle[_cycleId].name)) == keccak256(abi.encodePacked("KBKC"))){
            return _amount*decimals*signCycle[_cycleId].share/decimals;
        }
        if(keccak256(abi.encodePacked(signCycle[_cycleId].name)) == keccak256(abi.encodePacked("ETH/KBKC"))){
            uint haveKbkcCount = kbkcPlay.balanceOf(uniAddress)*_amount*decimals/uniPlay.totalSupply();
    ...
    /* 修改周期 */
    function editCycle (uint _cycleId,uint _timeLen,uint _share,uint _decimals) external onlyOwner {
        CycleList[_cycleId].timeLen = _timeLen;
        signCycle[_cycleId].timeLen = _timeLen;
        CycleList[_cycleId].share = _share * 10 ** _decimals;
        signCycle[_cycleId].share = _share * 10 ** _decimals;
    }
    ...
    function deposit (address _depositer,uint _amount,uint _cycleId) external existCycle(_cycleId) returns (uint){
        if(keccak256(abi.encodePacked(signCycle[_cycleId].name)) == keccak256(abi.encodePacked("KBKC"))){
            stakingKbkcCount = stakingKbkcCount.add(_amount*decimals);
            ownerKbkcCount[_depositer] = ownerKbkcCount[_depositer].add(_amount*decimals);

            kbkcPlay.transferFrom(_depositer,address(this), _amount*decimals);
        }
        if(keccak256(abi.encodePacked(signCycle[_cycleId].name)) == keccak256(abi.encodePacked("ETH/KBKC"))){
            stakingUniCount = stakingUniCount.add(_amount*decimals);
            ownerUniCount[_depositer] = ownerUniCount[_depositer].add(_amount*decimals);
    ...
    function withdraw (address _withdrawer,uint _amount,uint _cycleId) external onlyOwner existCycle(_cycleId) withdrawMax(_cycleId,_amount) returns (uint){
        if(keccak256(abi.encodePacked(signCycle[_cycleId].name)) == keccak256(abi.encodePacked("KBKC"))){
            kbkcPlay.transfer(_withdrawer, _amount*decimals);
        }
        if(keccak256(abi.encodePacked(signCycle[_cycleId].name)) == keccak256(abi.encodePacked("ETH/KBKC"))){
            uniPlay.transfer(_withdrawer,_amount*decimals);
        }
