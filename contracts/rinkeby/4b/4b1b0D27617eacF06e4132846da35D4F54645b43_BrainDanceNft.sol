...
contract BrainDanceNft is ERC721, Ownable, HeroFactory {
    ...
    // initial token count
    uint256 public constant INITIAL_TOKEN_COUNT = 10;
    ...
    uint256[] public countLimit = [0, 1, 1, 2, 3];
    mapping(uint256 => mapping(address => uint256)) stepBalance;
    ...
    function mintVip(bytes32[] memory proof, uint256 count) public payable {
        ...
        require(count >= 1 && count + stepBalance[statusFlag][msg.sender] <= countLimit[statusFlag], "count error");
    ...
    function walletOfOwner(address owner) external view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < breedTokenCount; i++) {
            if (_exists(i) && ownerOf(i + INITIAL_TOKEN_COUNT) == owner) {
    ...
    function setOwner1Address(address owner1) public onlyStarOwner1 {
