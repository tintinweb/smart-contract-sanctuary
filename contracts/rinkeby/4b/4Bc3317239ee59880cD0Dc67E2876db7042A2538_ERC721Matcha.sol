...
contract AuctionMochi is ReentrancyGuard {
    ...
    function withdraw() public nonReentrant returns (bool) {
        ...
        require(highestBid<reserve, "The highest bid is equal or more than the reserve price, cannot withdraw. Call auctionFinalize.");
    ...
    /// to the beneficiary.
    /// anyone can finalize the bid
    /// changing this function to internal because if not the NFT owner can call directly 
    /// this without transfering the ownership of the NFT
    function _auctionFinalize() internal nonReentrant {
        ...
        require(now >= auctionEnd, "Auction not yet ended.");
        require(!ended, "Auction is ended.");
        require(highestBid>=reserve, "Auction has not reached its minimum reserve price.");
...
contract ERC721Matcha is ERC721Full, ReentrancyGuard {
    ...
    // Mapping from token ID to auction address
    // if auction is the zero address means that there is no auction
    // otherwise this is the address of the auction contract
    mapping(uint256 => address) public auction;
    ...
    */
    ...
    function sell(uint256 tokenId, uint256 price, address payable wallet) public {
        ...
        require(auction[tokenId]==address(0), "ERC721Matcha: Cannot sell an item which has an active auction");
    ...
    function buy(uint256 tokenId) public payable nonReentrant {
        ...
        // is on sale
        require(auction[tokenId]==address(0) && sellBidPrice[tokenId]>0, "ERC721Matcha: The collectible is not for sale");
