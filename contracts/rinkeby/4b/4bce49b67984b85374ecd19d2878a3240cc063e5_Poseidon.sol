...
// TO DO: Account for emission changes in contract.
...
contract Poseidon is Ownable {
    ...
    // Last time emissions changed;
    uint256[] public emissionsChanges;
    ...
    constructor(
        ...
        emissionsChanges.push(block.number);
    }
    ...
    function pendingDRIP(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 dripReward;
            if (emissions != minter.emissionsFor(address(this))) {
                dripReward = emitted(pool.lastRewardBlock, true).mul(pool.allocPoint).div(totalAllocPoint);
            } else {
                dripReward = emitted(pool.lastRewardBlock, false).mul(pool.allocPoint).div(totalAllocPoint);  
            }
    ...
    function massUpdatePools() public {
        ...
        bool changed = (emissions != minter.emissionsFor(address(this)));
        minter.mint();
        for (uint256 pid = 0; pid < length; ++pid) {
            _updatePool(pid, changed);
        }
        emissions = minter.emissionsFor(address(this));
        emissionsChanges.push(block.number);
    }

    function updatePool(uint256 _pid) public {
        require(emissions == minter.emissionsFor(address(this)), "Mass update pools");
        _updatePool(_pid, false);
    }
    ...
    function _updatePool(uint256 _pid, bool changed) internal {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        uint256 dripReward = emitted(pool.lastRewardBlock, changed).mul(pool.allocPoint).div(totalAllocPoint);
        if (!changed) {
            minter.mint();
        }
        pool.accDripPerShare = pool.accDripPerShare.add(dripReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for DRIP allocation.
    function _deposit(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
    ...
    function _withdraw(uint256 _pid, uint256 _amount) public {
        ...
        _updatePool(_pid, (emissions == minter.emissionsFor(address(this))));
    ...
    // total emissions accounting for change
    function emitted(uint256 lastRewardBlock, bool changed) internal view returns (uint256) {
        if (changed) {
            uint256 preChange = lastRewardBlock.sub(emissionsChanges[emissionsChanges.length - 1]).mul(emissions);
            uint256 postChange = block.number.sub(lastRewardBlock).mul(minter.emissionsFor(address(this)));
            return preChange.add(postChange);
        } else {
            return block.number.sub(lastRewardBlock).mul(emissions);
        }
    }
