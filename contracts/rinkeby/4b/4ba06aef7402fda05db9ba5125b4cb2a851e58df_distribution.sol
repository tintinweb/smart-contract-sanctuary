...
contract distribution{
    ...
    constructor(Token _tokenA, Token _tokenB)
    {
        ...
        tokenB=_tokenB;
        admin=msg.sender;
        priceFeed1=AggregatorV3Interface(0x78F9e60608bF48a1155b4B2A5e31F32318a1d85F); //ETH/USD    
    ...
    function setETH() public returns(uint256)
    {
       (, int price,, ,) = priceFeed1.latestRoundData();
            oneETHPrice=uint256(price).div(100000000);
            //oneETHPrice=23;
            return oneETHPrice;
    }
    ...
    function setBTC() public returns(uint256)
    {
        (, int price,, ,) = priceFeed2.latestRoundData();
            oneBTCPrice=uint256(price).div(100000000);
            //oneBTCPrice=11;
            return oneBTCPrice;
    }
    ...
    function depositETH(uint256 _amount) payable public returns(uint256)
    {
        require(msg.value>0,"Insufficient amount");
        require(msg.value==_amount,"Enter equal amount");
        amountETH = msg.value;
        totalETHAmount=totalETHAmount.add(amountETH);
        tokenAmount1=amountETH.mul(oneETHPrice).div(tokenValue);
        require(Token(tokenA).balanceOf(address(this))>=tokenAmount1,"Not Enough Tokens");
        Token(tokenA).transfer(msg.sender,tokenAmount1);           //token transfser from contract address to user
        emit TokenTransfer(address(this),msg.sender,tokenAmount1);
        return totalETHAmount;
    }
    function depositBTC(uint256 _amount) payable public returns(uint256)
    {
        require(msg.value>0,"Insufficient amount");
        require(msg.value==_amount,"Enter equal amount");
        amountBTC=msg.value;
        totalBTCAmount=totalBTCAmount.add(amountBTC);
        tokenAmount2 = amountBTC.mul(oneBTCPrice).div(tokenValue);
        require(Token(tokenB).balanceOf(address(this))>=tokenAmount2,"Not Enough Tokens");
        Token(tokenB).transfer(msg.sender,tokenAmount2);           //token transfser from contract address to user
        emit TokenTransfer(address(this),msg.sender,tokenAmount2);
