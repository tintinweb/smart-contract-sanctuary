  ...
  INafterMarketAuction
{
  ...
  event AcceptBid(address indexed _bidder, address indexed _seller, uint256 _amount, uint256 _tokenId);
  ...
  function senderMustBeTokenOwner(uint256 _tokenId) internal view {
    require(
      isOwnerOfTheToken(_tokenId, msg.sender) ||
  ...
  function buy(
    ...
    if (_paymentMode == 1) {
      Payments.safeTransferFrom(msg.sender, address(this), amount);
    }
    Payments.payout(
      sp.amount,
  ...
  function bid(
    ...
    uint256 amount = _newBidAmount.add(_newBidAmount.mul(nafter.getServiceFee(_tokenId)).div(100));
    ...
    if (_paymentMode == 0) {
      ...
      // Refund previous bidder.
      _refundNafterBid(_tokenId, _owner);
  ...
  function acceptBid(uint256 _tokenId, address _owner) public {
    ...
    emit AcceptBid(currentBid.bidder, msg.sender, currentBid.amount, _tokenId);
  }
  ...
  function lazyMintingBid(
    ...
    require(
      _newBidAmount > _price && _newBidAmount >= _price.add(_price.mul(minimumBidIncreasePercentage).div(100)),
      "high minimum percentage"
    );
    nafter.verify(
      _creator,
    ...
    uint256 amount = _newBidAmount.add(_newBidAmount.mul(serviceFee).div(100));
    if (_paymentMode == 0) {
  ...
  function lazyMintingBuy(
    ...
    uint256 amount = tokenPriceFeeIncluded(_tokenId, _creator);
    if (_paymentMode == 0) {
      require(msg.value >= amount, "no correct price");
    }
  ...
  function _setBid(
    ...
    bidBalance[_bidder] = bidBalance[_bidder].add(_amount.add(_amount.mul(nafter.getServiceFee(_tokenId)).div(100)));
  }
}
...
contract Payments is SendValueOrEscrow {
  ...
  function refundNafter(
    ...
    if (_amount > 0) {
      nafterToken.safeTransfer(_payee, _amount.add(calcPercentagePayment(_amount, _marketplacePercentage)));
    }
  }
