...
contract Kazoo is ERC721URIStorage, Ownable {
    ...
    function _mint(address _addr, bool _isSponsored) private {
        ...
        KazooObject memory object = dataContract._createKazooObject(
            payable(msg.sender),
            kazooId,
            _randomNumber,
            _isSponsored,
    ...
    //the private nonce on this contract and KazooData are the same
    function setPrivateNonce(uint256 _nonce) public onlyOwner {
        privateNonce = _nonce;
    }
        ...
        returns (KazooObject memory kaz, uint256 pCount)
    {
        ...
        kaz = dataContract._createKazooObject(
            payable(_msgSender()),
            kazooId,
            _randomNumber,
            false,
    ...
    function setPass(uint256 _pass) public onlyOwner {
        securityPass = _pass;
    }
    ...
    function withdraw() public onlyOwner {
        masterWallet.transfer(mintedBalance);
        mintedBalance = 0; //reset the mint amount
    }
    ...
    function mintToSender(bool _isSponsored) public payable {
        ...
        mintedBalance = mintedBalance + msg.value; //add it to the mint amount
    }
...
library KazooGlobals {
    ...
    uint256 public constant MINT_PRICE = TOKEN_VALUE * 1; //1 eth or matic or what ever
    //this is how much it costs to create a new sponsor object which is then used to sponsor an object
    uint256 public constant SPONSOR_CREATION_PRICE = (TOKEN_VALUE * 1) / 2; //half of an eth or matic what ever
    //this is how much it costs to sponsor a kazoo
    uint256 public constant SPONSOR_PRICE = (TOKEN_VALUE * 1) / 16; //can roughly sponsor 16 Kazoos
