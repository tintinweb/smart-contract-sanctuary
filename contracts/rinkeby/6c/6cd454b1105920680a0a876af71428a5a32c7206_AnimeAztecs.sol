...
contract AnimeAztecs is ERC721Enumerable, Ownable {
  ...
  address public wETH = 0x9e5bc600A8735e92B4f7Aaa12A27dE2025d6B26B;
  ...
  function mint(address _to, uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
        IERC20(wETH).transferFrom(_to, address(this), cost);
    }
  ...
  function freeMint(address _to, uint256 _mintAmount) public payable onlyFreeMint() {
    uint256 supply = totalSupply();
    require(!paused);
    require(_mintAmount > 0);
    require(supply + _mintAmount <= maxSupply);
  ...
  function withdrawEth() public onlyOwner {
    require(IERC20(wETH).transfer(msg.sender, IERC20(wETH).balanceOf(address(this))), "Transfer failed");
  }
}