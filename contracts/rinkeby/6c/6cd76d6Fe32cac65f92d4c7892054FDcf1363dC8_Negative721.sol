...
contract Negative721 is Context, ERC721, Ownable {
    ...
    event Negate(address minter, uint256 price, address targetContract, uint256 targetId);
    ...
    function negate(address contractToNegate, uint256 tokenId) public payable returns(uint256){
        require(msg.value >= pricePerPiece, "not enough eth sent");
        require(mintActive || msg.sender == owner(), "sale not open");
        require(!negatedTokens[keccak256(abi.encodePacked(contractToNegate, tokenId))], "token is already minted");
        
        negatedTokens[keccak256(abi.encodePacked(contractToNegate, tokenId))] = true;
        emit Negate(msg.sender, pricePerPiece, contractToNegate, tokenId);
