...
contract Art is ERC721Enumerable, Ownable {
  ...
  uint256 public constant PRICE_IN_WEI = 0.06 ether;
  ...
  function publicMintPause(bool _state) external onlyOwner {
    ...
    payable
  {
    ...
    require(msg.value >= PRICE_IN_WEI * _count, "Ether sent is not correct");
    require(
      addressToMinted[_msgSender()] + _count <= 3,
  ...
  function publicMint(uint256 _count) public payable {
    ...
    require(totalSupply + _count < MAX_SUPPLY, "Excedes max supply");
    require(_count < MAX_PER_TX, "Exceeds max per transaction");
    require(_count * PRICE_IN_WEI <= msg.value, "Invalid funds provided");
