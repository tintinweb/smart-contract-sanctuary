...
contract TalkingWithEllon is ERC1155Base {
    string public name;
    string public symbol;
    ERC1155Base[] public collectibles;
    uint256 private _totalSupply = 0;

    mapping(uint256 => string) public message;
    mapping(bytes32 => bool) private uriHashUsed;
    mapping(address => bool) private isCollectibleAccepted;
    mapping(address => mapping(uint256 => bool)) private wasCollectibleUsed;

    event MessageChanged(string message);

    constructor(string memory _name, string memory _symbol, ERC1155Base[] memory _tokens, string memory contractURI, string memory tokenURIPrefix) ERC1155Base(contractURI, tokenURIPrefix) public {
        name = _name;
        symbol = _symbol;
        collectibles = _tokens;
        for(uint i=0;i<_tokens.length;i++){
          isCollectibleAccepted[address(_tokens[i])] = true;
        }
        _registerInterface(bytes4(keccak256('MINT_WITH_ADDRESS')));
    }


    function mint(uint256 _collectibleId,uint256 _id,Fee[] memory fees, string memory uri,string memory _message) public payable {
        require(collectibles[_collectibleId].balanceOf(msg.sender,_id) > 0,"Only actual owner of a snowflake or hashavatar can mint this token");
        require(!wasCollectibleUsed[address(collectibles[_collectibleId])][_id] ,"Collectible already used to mint this token");
        bytes32 uriHash = keccak256(abi.encodePacked(uri));
        require(!uriHashUsed[uriHash],"Token with same uri already was minted");
        require(totalSupply().add(1) <= 50000);
        uriHashUsed[uriHash] = true;
        _totalSupply = _totalSupply.add(1);
        wasCollectibleUsed[address(collectibles[_collectibleId])][_id] = true;
        message[_totalSupply] = _message;
        _mint(_totalSupply, fees, 1, uri);
    }

    function changeMessage(uint256 id,string memory _message) public {
      require(this.balanceOf(msg.sender,id) > 0);
      message[id] = _message;
