...
contract UsdtDaoToken is ERC20, ERC20Permit, ERC20Votes, Ownable {
     ...
     * @param _timelockAddress The claim period ends unix time.
     */
    constructor(address _timelockAddress, uint256 _claimPeriodEnds)
        ERC20("USDT DAO", "UDAO")
        ERC20Permit("USDT DAO")
    {
        ...
        _mint(_timelockAddress, timelockSupply);
    }
    ...
    function claimTokens(uint256 amount, bytes32[] calldata merkleProof) public {
        ...
        require(valid, "UsdtDao: Valid proof required.");
        require(!claimed[msg.sender], "UsdtDao: Tokens already claimed.");
        
        uint amountToClaim;
        if(totalUsersClaimed < 10000) {
            amountToClaim = amount + ((amount * 6) / 100); // add 6%
        } else if(totalUsersClaimed < 25000) {
            amountToClaim = amount + ((amount * 4) / 100); // add 4%
        } else {
            amountToClaim = amount + ((amount * 2) / 100); // add 2%
        }
    
        claimed[msg.sender] = true;
        totalUsersClaimed = totalUsersClaimed++;
    ...
    /**
     * @dev Allows the owner to withdraw unclaimed tokens after the claim period ends.
     * @param dest The address to withdraw the tokens to.
     */
    function withdraw(address dest) public onlyOwner {
    ...
    function setMerkleRoot(bytes32 _merkleRoot) public onlyOwner {
        require(merkleRoot == bytes32(0), "UsdtDao: Merkle root already set");
