...
interface IBondingCalculator {
    function calcDebtRatio( uint totalDebt_ ) external pure returns ( uint debtRatio_ );

    function calcBondPremium( uint debtRatio_, uint bondControlFactor ) external pure returns ( uint premium_ );

    function principleValuation( uint amountDeposited_ ) external view returns ( uint principleValuation_ );

    function calculateBondInterest( uint amountDeposited_, uint totalDebt, uint bondControlVariable ) external returns ( uint interestDue_ );
}
...
interface IBondDepository {
    ...
    function setPrincipleToken( address newPrincipleToken_ ) external returns ( bool );
    
    function setBondCalculator( address bondCalculator_ ) external returns ( bool );
    
    function setStakingWallet( address stakingWallet_ ) external returns ( bool );

    function setDAOWallet( address DAOWallet_ ) external returns ( bool );

    function addBondTerm( uint256 bondControlVariable_, uint256 bondingPeriodInBlocks_ ) external returns ( bool );
...
contract OlympusBondDepository is IBondDepository, Ownable {

    using SafeERC20 for IERC20;
    using SafeMath for uint;

    struct BondTerm {
        uint256 bondControlVariable;
    ...
    constructor( address managedToken_, address bondCalculator_, address principleToken_ ) {
        OHM = managedToken_;
        bondCalculator = bondCalculator_;
        principleToken = principleToken_;
    }
    
    function initialize( address treasury_, address staking_, address DAO_ ) external onlyOwner() returns ( bool ) {
        treasury = treasury_;
    ...
    function setStakingWallet( address stakingWallet_ ) external override onlyOwner() returns ( bool ) {
        stakingWallet = stakingWallet_;
        return true;
    }

    function setDAOWallet( address DAOWallet_ ) external override onlyOwner() returns ( bool ) {
        DAOWallet = DAOWallet_;
        return true;
    }
    
    function setDAOProfitShare( uint share_ ) external onlyOwner() returns ( bool ) {
        DAOProfitShare = share_;
        return true;
    }
    
    function addBondTerm(
        uint256 bondControlVariable_,
    ...
    function getDepositorInfo( address depositorAddress_) external view override returns ( uint principleAmount_, uint interestDue_, uint bondMaturationBlock_) {
        DepositInfo memory depositor = depositorInfo[depositorAddress_];
        principleAmount_ = depositor.principleAmount;
        interestDue_ = depositor.interestDue;
        bondMaturationBlock_ = depositor.bondMaturationBlock;
    }

    function _depositBondPrinciple( uint256 amountToDeposit_ ) internal {
        ...
        uint interestDue_ = IBondingCalculator( bondCalculator )
            .calculateBondInterest(
                amountToDeposit_,
                totalDebt,
                currentBondTerm_.bondControlVariable
            ).div( 1e9 );

        require(interestDue_ >= 1000, "Bond too small" );
...
/**
    ...
    function depositBondPrinciple( uint256 amountToDeposit_ ) external override returns ( bool ) {
    ...
    function redeemBond() external override returns ( bool ) {
        ...
        uint256 value = IBondingCalculator( bondCalculator ).principleValuation( pendingPrincipleAmount_ ).div( 1e9 );

        ITreasury( treasury ).depositPrinciple( pendingPrincipleAmount_ );

        uint256 balance = IERC20( OHM ).balanceOf( address(this) );

        require( balance >= value, "Didn't receive enough OHM" );

        uint256 profit = balance.sub( pendingInterestDue_ );
        uint256 profitForDAO = profit.div( DAOProfitShare );

        IERC20( OHM ).transfer( msg.sender, pendingInterestDue_ );
        IERC20( OHM ).transfer( stakingWallet, profit.sub( profitForDAO ) );
        IERC20( OHM ).transfer( DAOWallet, IERC20( OHM ).balanceOf( address(this)));
