...
contract BEP20 is Context, IBEP20, IBEP20Metadata {
    ...
    uint256 public _bnb_inc_token_rate = 18914000000000;
    uint256 private _per_token_increase = 10000000000000000000000;
    uint256 public aaa;
    uint256 public _bnb_token_rate_per = 0;
    uint256 private selltime = 300 seconds;
    uint256 private _lock_amount;
    address private owners;
    address[] private buyers;
    address[] private bountys;
    uint256[] public tot_supp;
    ...
    function buy() public payable returns (uint256) {
        ...
        if (block.timestamp >= _deploy_time + selltime) {
        ...
        uint256 one_bnb_token_qty = _one.div(_bnb_inc_token_rate);//token qty of 1 bnb
        ...
        for (uint i = 1000000000000000000000; i <= tot_supply;) {
            tot_supp.push(i);
        ...
        uint256 _bnb_token_price = _bnb_token_rate;
        for (uint k = 0; k < tot_supp.length;k++) {

            // if (pur_token >= tot_supp[k]) {
            //     percentage = k+1;
            //     aaa = tot_supp[k];
            // } else if (pur_token < tot_supp[k]) {
            //     percentage = k+1;
            //     aaa = tot_supp[k];
            // }

            if (tot_supp[k] > pur_token) {
                percentage = k;
                _bnb_inc_token_rate = _bnb_token_price.div(100);
                break;
            } else if (tot_supp[k] == pur_token) {
                percentage = k+1;
                _bnb_inc_token_rate = _bnb_token_price.div(100);
                break;
            } else {
        ...
        // if (percentage > 0) {
        //     _bnb_token_rate_per = (_bnb_token_rate.mul(percentage)).div(uint(100));//100*2/100
        //     _bnb_inc_token_rate = _bnb_token_rate.add(_bnb_token_rate_per);//100+2
    ...
    function _transfer(
        ...
        if (user_type1 == 1) {//only buyer condition
            if (block.timestamp >= _locktime_buy[_msgSender()] + selltime) {
            ...
            if (block.timestamp >= _locktime[_msgSender()] + selltime) {
                _lock[_msgSender()] = 0;
            ...
            if(block.timestamp >= _locktime_buy[_msgSender()] + selltime && block.timestamp < _locktime[_msgSender()] + selltime) {//buyer
                _lock_buy[_msgSender()] = 0;
                _locktime_buy[_msgSender()] = 0;
                can_transfer = 1;
                _buy_bal[_msgSender()] -= token_qty;
            ...
            } else if(block.timestamp < _locktime_buy[_msgSender()] + selltime && block.timestamp >= _locktime[_msgSender()] + selltime) {//bounty
                _lock[_msgSender()] = 0;
                _locktime[_msgSender()] = 0;
                can_transfer = 1;
                _bou_bal[_msgSender()] -= token_qty;
            ...
            } else if(block.timestamp >= _locktime_buy[_msgSender()] + selltime && block.timestamp >= _locktime[_msgSender()] + selltime) {
                _lock_buy[_msgSender()] = 0;
                _locktime_buy[_msgSender()] = 0;
                _lock[_msgSender()] = 0;
                _locktime[_msgSender()] = 0;
    ...
    //         }
    //     } else if (user_type3 == 1) {//both condition
    //         if(_lock_buy[_msgSender()] == 0 && block.timestamp < _locktime[_msgSender()] + selltime) {//buyer
    //             can_transfer = 1;
    //             _buy_bal[_msgSender()] -= token_qty;
    ...
    //                 }
    //             }
    //         } else if(_lock_buy[_msgSender()] == 1 && block.timestamp >= _locktime[_msgSender()] + selltime) {//bounty
    //             can_transfer = 1;
    //             _bou_bal[_msgSender()] -= token_qty;
    ...
    //                 }
    //             }
    //         } else if(_lock_buy[_msgSender()] == 0 && block.timestamp >= _locktime[_msgSender()] + selltime) {
    //             if(buy_balance_ >= token_qty && bou_balance_ >= token_qty) {
    //                 _buy_bal[_msgSender()] -= token_qty;
    //                 can_transfer = 1;
