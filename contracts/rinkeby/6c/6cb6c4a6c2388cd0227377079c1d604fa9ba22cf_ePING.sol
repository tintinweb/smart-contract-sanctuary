...
contract ePING is Context, IERC20, Ownable {
    ...
    bool public isWalletTransferFeeEnabled = true; //TEMPORARY FOR TESTING
    ...
    uint256 private constant MAX = 16 * 10**9 * 10**_decimals;
   ...
   function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
    ...
    function _mint(address recipient, uint256 amount) private {
        require(amount > 0, "Transfer amount must be greater than zero");
        require(_tTotal <= 4 * 10**9 * 10**_decimals, "Total supply cannot exceed 4B");
