...
// import { ERC20, ERC20Interface } from "./LifeToken.sol";
...
contract StakingV2 is Ownable, ReentrancyGuard {
    ...
    uint256 private constant DAY = 1 seconds;
    uint256 private constant MONTH = 30 seconds;
    uint256 private constant YEAR = 365 seconds;
    uint256 public minStake = 10_000 ether ;
    uint256 public maxStake = 2_500_000_000 ether;
    uint256 public totalStaked = 0;

    // The period after which the new value of the parameter is set
    uint256 public constant PARAM_UPDATE_DELAY = 7 seconds;
    ...
    function setminStake(uint256 _limit) public onlyOwner {
        minStake = _limit;
    }
    function setmaxStake(uint256 _limit) public onlyOwner {
        minStake = _limit;
    }
    /**
    ...
    {
        require(_amount > minStake, "Amount should be more than 10K");
    ...
    function compound(
        ...
        uint256 secondsCount = _n.div(DAY);

        return
            ABDKMath64x64.mulu(
                pow(ABDKMath64x64.add(ABDKMath64x64.fromUInt(1), ABDKMath64x64.divu(_ratio, 10**18)), secondsCount),
                _principal
            );
    }
    ...
    function calcRewards(address _sender, uint256 _depositId) public view validDepositId(_depositId) returns (uint256) {
        ...
        if (_depositId == 1) {
            ratio = 821917808000000; // APY 30% InterestRate = 0.08
            lockPeriod = MONTH;
        } else if (_depositId == 2) {
            ratio = 958904110000000; // APY 35% InterestRate = 0.09
            lockPeriod = MONTH * 6; // 6 months
        } else if (_depositId == 3) {
            ratio = 1068493150000000; // APY 39% InterestRate = 0.1
            lockPeriod = MONTH * 12; // 12 months
        }
...
// CONTRACTS
// import { ERC20, ERC20Interface } from "../contracts/LifeToken.sol";
...
library SafeERC20 {
    ...
    function safeApprove(
        ...
        require(
            ...
            "ERC20: error non zero allowance"
        );
