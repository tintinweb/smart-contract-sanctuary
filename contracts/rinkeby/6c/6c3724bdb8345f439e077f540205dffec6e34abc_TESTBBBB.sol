...
contract TESTBBBB is ERC721Enumerable, Ownable {
    ...
    function getPublicLeft() public view returns (uint256) {
        return MAX_NFT_SUPPLY - reservedAmount - totalSupply();
    }
    ...
    function addListAddressToWhitelist(address[] memory addrs, uint[] memory claimAmounts) public onlyOwner {
        for(uint i = 0; i < addrs.length; i++) {
            addAddressToWhitelist(addrs[i], claimAmounts[i]);
        }
    }
    ...
    function giveaway(address _to, uint256 _amount) external onlyOwner() {
        require( _amount <= reservedAmount, "Exceeds reserved bots supply" );
