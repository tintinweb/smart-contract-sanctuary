...
contract CHQueensArtifacts is Delegated, ERC1155{
  ...
  function create( address initialOwner, uint supply, string calldata uri_, bytes calldata data_ )
    ...
    _mint(initialOwner, id, supply, data_ );
    return id;
  }
  ...
  function burnBatch( address account, uint256[] calldata ids, uint256[] calldata quantities) external payable{
    uint totalRequired;
    for(uint i; i < ids.length; ++i){
      ...
      require( token.isBurnActive,                       "Burn is not active"        );
      require( msg.value >= token.burnPrice * quantity,  "Ether sent is not correct" );
      totalRequired += ( token.burnPrice * quantity );
    }
  ...
  function batchMint( address account, uint256[] calldata ids, uint256[] calldata quantities, bytes calldata data) external payable{
    uint totalRequired;
    for(uint i; i < ids.length; ++i){
      uint256 id = ids[i];
      require( id < tokens.length, "Specified token (id) does not exist" );
