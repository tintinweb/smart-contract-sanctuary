...
contract ChainRunners is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 private constant MAX_PUBLIC_RUNNERS = 9800;
    ...
    function mintPublicSale(uint256 _count) public payable nonReentrant whenPublicSaleActive returns (uint256, uint256) {
        require(_count > 0 && _count <= MAX_PER_ADDRESS, "Invalid runner count");
        require(totalSupply() + _count <= MAX_PUBLIC_RUNNERS, "Attempted to mint more runners than total supply");
    ...
    function mintOwner() public onlyOwner nonReentrant returns (uint256, uint256) {
        uint256 firstMintId = MAX_PUBLIC_RUNNERS + 1;
        for (uint256 i = firstMintId; i <= MAX_RUNNERS; i++) {
            mint(i);
        }
        return (firstMintId, MAX_RUNNERS - MAX_PUBLIC_RUNNERS);
    }
    ...
    // TODO Mainly for testing so far, could remove
