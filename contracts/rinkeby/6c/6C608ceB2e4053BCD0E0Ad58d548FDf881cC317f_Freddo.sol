...
contract Freddo is ERC20, Ownable {
	...
	iCheetahContract public CheetahContract;
	...
	constructor(address _cheetahContractAddress) ERC20("Tsonta", "TSONTA"){
		CheetahContract = iCheetahContract(_cheetahContractAddress); 
	}
   ...
   function setFirstRedeemRate(uint256 _FirstRedeemRate) external {
	...
	function getRewards(uint256[] calldata _tokenIds) external {
		...
		uint256 tamount;
		 for(uint256 i = 0; i < _tokenIds.length; i++) {
			 uint256 _cheetah = _tokenIds[i];
		require(msg.sender == address(CheetahContract.ownerOf(_cheetah)), "You are not the owner of this punk");
		require(!isPauseEnabled, "Staking is on pause");
		uint256 time = block.timestamp;
		if(lastUpdate[_cheetah] == 0) {
		   ramount= FirstRedeemRate;
		}else{
		ramount = uint(time - lastUpdate[_cheetah]) / rewardtimeframe * BASE_RATE; //daily 
		}
		if(ramount>1){
			lastUpdate[_cheetah] = time;
	...
	function getTotalClaimableU(address address_) external view returns(uint256) {
		...
		uint _balance = CheetahContract.balanceOf(address_); // get balance of address
        uint contractMaxToken = CheetahContract.totalSupply(); //total supply of contract
        ...
        for (uint id = 1; id <= contractMaxToken; id++) {
            if (address_ == CheetahContract.ownerOf(id)) { _tokens[_index] = id; _index++;}
        }
        uint256 _cheetah;
		uint256 tamount;
		for (uint256 ind = 0; ind < _tokens.length; ind++) {
			_cheetah=_tokens[ind];
		uint256 time = block.timestamp;
		uint256 ramount;
		if(lastUpdate[_cheetah] == 0) {
			ramount= FirstRedeemRate;
		...
		ramount = uint(time - lastUpdate[_cheetah]) / rewardtimeframe * BASE_RATE;} //daily 
	//uint256 ramount = rewards[_cheetah] + pending;
		    //require(lastUpdate[_cheetah] > 0, "Ask your punk to start grinding for you");
		tamount= tamount+ramount;
	...
	function getClaimable(uint256 _cheetah) external view returns(uint256) {
		uint256 time = block.timestamp;
		uint256 ramount;
		...
		ramount = uint(time - lastUpdate[_cheetah]) / rewardtimeframe * BASE_RATE;
        } //daily 
	//uint256 ramount = rewards[_cheetah] + pending;
		    //require(lastUpdate[_cheetah] > 0, "Ask your punk to start grinding for you");
		return ramount;
	...
	function getReward(uint256 _cheetah) external {
		require(msg.sender == address(CheetahContract.ownerOf(_cheetah)), "You are not the owner of this punk");
		require(!isPauseEnabled, "Staking is on pause");
		uint256 ramount;
		uint256 time = block.timestamp;
	...
	function airdropToTokens(uint256[] calldata _tokenIds,uint256 coins) external {
		...
		coins = coins * 1 ether;
		for (uint256 ind = 0; ind < _tokenIds.length; ind++) {
        _mint(address(CheetahContract.ownerOf(_tokenIds[ind])), coins);
		emit RewardPaid(CheetahContract.ownerOf(_tokenIds[ind]), coins);
	    }
	}	
}
