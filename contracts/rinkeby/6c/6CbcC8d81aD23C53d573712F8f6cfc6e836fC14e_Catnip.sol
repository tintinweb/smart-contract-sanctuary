...
contract Catnip is Context, IBEP20, Initializable {
    ...
    event PreSalePurchase(address buyer, uint256 amountNIPpurchased, uint256 amountBNBsold, int priceOfBNB);
    ...
    function initialize() public initializer {
        ...
        // payableTeamWalletAddr()      // TODO try this out in rinkeby, remove if it works
        ...
        tokenSymbol = "NIP6";        //CHANGEIT - Change the NAme to the PRoduction one NIP
        tokenName = "Catnip6";       //CHANGEIT - Change the NAme to the PRoduction one Catnip
        MAXintNum = ~uint256(0);
        ...
        reflectTokensOwned[payableTeamWalletAddr()] = _rTotal;    // CHANGEIT - change this back
        ...
        preSaleRate = 1;    // CHANGEIT - Might want to change the presale rate.  // presale - 0.1c  or 0.0001 USD

        // RINKEBY ETH/USD - 0x8A753747A1Fa494EC906cE90E9f37563A8AF630e
        // BSC - BNB/USD - 0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE
        priceFeed = AggregatorV3Interface(0x8A753747A1Fa494EC906cE90E9f37563A8AF630e);  // CHANGEIT - Price feed address for chainlink, need to change it when going to BSC
    ...
    function presaleBuy(uint256 keyCode) external payable {
        ...
        // 100 BNB = 10,000,000,000 JAGER


        require(isPreSaleEnabled, "Presale must be enabled. It is currently disabled. Contact the admin.");
        require(keyCode == 1564432445631, "Don't use this contract presale function except through our website at......"); // TODO get the website

        // send BNB to contract
        // amountOfBNBtoInput = getMsgValue();     // this will get the value of the BNB sent.

        uint256 amountOfBNBtoInput = msg.value;

        require(amountOfBNBtoInput > 0, "BNB must be greater than 0");  
        require(amountOfBNBtoInput <= 10000000000, "Capped at 100 BNB, please input less BNB.");       // CHANGEIT - presale limits
        
        address buyer = _msgSender();

        int currentPrice = getThePrice();

        uint256 howMuchMoneyInput = amountOfBNBtoInput.mul(uint256(currentPrice)).div(100000000);       // convert from Jager to BNB with Divison, then multiply with the price

        uint256 amountOfNIPtoGive = howMuchMoneyInput.div(preSaleRate).div(1000);

        uint256 totalBalanceOfNIPinContract = balanceOf(address(this));

        require(amountOfNIPtoGive < totalBalanceOfNIPinContract, ("Not enough NIP left in the Presale. Please check the NIP left in the contract itself if you would like to buy."));

        // send BNB to contract
        // payableTeamWalletAddr().transfer(amountOfBNBtoInput);

        // send NIP to buyer
        approveInternal(address(this), buyer, amountOfNIPtoGive);   
        transferFromIgnoreMaxLimit(address(this), buyer, amountOfNIPtoGive);    
        approveInternal(address(this), buyer, 0);          

        emit PreSalePurchase(buyer, amountOfNIPtoGive, amountOfBNBtoInput, currentPrice);
    }
    ...
    // function transferBNBToTeamWallet(IBEP20 token, uint256 value) internal {
    //     token.safeTransfer(payableTeamWalletAddr(), value); 
    // }
...
// SPDX-License-Identifier: MIT
// https://github.com/smartcontractkit/chainlink/blob/master/evm-contracts/src/v0.7/interfaces/AggregatorV3Interface.sol
