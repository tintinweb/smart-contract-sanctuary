...
contract BallerBars is Context, ERC20Burnable, Ownable {
    ...
    **/
     ...
     * @param chainsGeneration Chains contract generation
     */
    function lastClaim(uint256 _tokenId, uint256 chainsGeneration) public view returns (uint256) {
        return __lastClaim(_tokenId,getChainsContract(chainsGeneration));
    }
     ...
     * @param _tokenId The token to reference
     * @param chainsGeneration The generation of token
     */
    function accumulated(uint256 _tokenId, uint256 chainsGeneration) public view returns (uint256) {
        return _accumulated(_tokenId,getChainsContract(chainsGeneration));
    }
    ...
    function _accumulated(uint256 _tokenId, IChains chainsContract) internal view returns (uint256) {
        ...
        uint256 thisTokenEmissionPerDay = (0.1 ether + ((33 ether / 1000) * _getBoost(_tokenId,chainsContract))) * _baseMultiplier;
     ...
     * @param chainsGeneration The generation of token
     */
    function getBoost(uint256 _tokenId, uint256 chainsGeneration) public view returns (uint256) {
        return _getBoost(_tokenId,getChainsContract(chainsGeneration));
    }
     ...
     * @param chainsGeneration The generation of chains contract to reference
     */
    function claim(uint256[] memory _tokenIds, uint256 chainsGeneration) public returns (uint256) {

        require(_paused==false,"PAUSED");

        IChains chainsContract = getChainsContract(chainsGeneration);
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            require(
                _tokenIds[i] < chainsContract.totalSupply(),
                "TOKEN_NOT_MINTED"
            );
    ...
    function getChainsGeneration(uint256 _tokenId) public view returns (uint256) {
        try IChains(_genOneChainsAddress).ownerOf(_tokenId) returns (address genOneOwner){
            if (genOneOwner != address(0) && genOneOwner != 0x000000000000000000000000000000000000dEaD) {
                return 1;
            }
        }catch{
            try IChains(_genTwoChainsAddress).ownerOf(_tokenId) returns (address genTwoOwner){
                if (genTwoOwner != address(0) && genTwoOwner != 0x000000000000000000000000000000000000dEaD) {
                    return 2;
                }
            }catch{
                // Do Nothing
    ...
    function getChainsContract(uint256 generation) internal view returns (IChains) {
        if(generation == 1){
            return IChains(_genOneChainsAddress);
        }else if(generation == 2){
            return IChains(_genTwoChainsAddress);
        }else{
            revert("INVALID_GEN");
        }
    }
