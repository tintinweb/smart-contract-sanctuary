...
library Counters {
	...
	function cursor(Counter storage counter) internal view returns (uint256) {
...
pragma solidity ^0.8.0;
contract ProjectX is Context, AccessControlEnumerable, ERC721Enumerable, ERC721Burnable, ERC721Pausable {

	/*
	My project is a series of character artworks, some of which I want them to change over time. Ex: the artwork in the NFT changes from Night to day depending on when you look at it, and potentially Week to Week.

	So I’m looking for help to develop a custom smart contract that allows for the nft to “change states” like that.
	*/
	...
	uint256 private _maxcap = 100;
	address private smart_contract_owner = 0xC12Df5F402A8B8BEaea49ed9baD9c95fCcbfE907;
	string private _baseTokenURI = "https://arielbecker.com/efdot/token.php";
	string private _contractURI = "https://arielbecker.com/efdot/contract.php";
	string private _rulesJSON = '';
	string private _contractJSON = '{"name": "Project X","description": "Changing characters over time and other parameters.","image": "https://arielbecker.com/efdot/icon.png","external_link": "https://arielbecker.com/efdot","seller_fee_basis_points": 1000, "fee_recipient": "0xC12Df5F402A8B8BEaea49ed9baD9c95fCcbfE907"}';

	struct TokenStruc {
		string metadata; // Metadata will be stored here.
		string data; // And any onchain data, here.
	}

	Counters.Counter private _tokenIdTracker;
	TokenStruc[] private efdots;
	...
	constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
		...
		_tokenIdTracker.increment(); // We don't want a #0 token here.
	}
	...
	function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
		// La URL base debe tener ya la ruta para el token, por ejemplo "https://arielbecker.com/efdot/token.php"
	...
	function retrieveData(uint256 _tokenId) public view returns (string memory, string memory) {
		require(_tokenId >= 0, "Token ID cannot be less than zero!");
		require(_tokenId < totalSupply(), "Token ID out of bounds!");
		return(efdots[_tokenId].data, efdots[_tokenId].metadata);
	}
	...
	function setRulesJSON(string memory _json) public {
		require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Not authorized!");
		_rulesJSON = _json;
	}
	...
	function setMintFee(uint256 newpercentage) public {
		require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Not authorized!");
	...
	function mint(string memory _data, string memory _metadata) public payable virtual {
		require(msg.value >= _mintfee, "Not enough ETH!");
		require(_tokenIdTracker.current() < _maxcap, "Max cap reached!");
		efdots.push(TokenStruc(_data, _metadata));
		_mint(_msgSender(), _tokenIdTracker.cursor());
