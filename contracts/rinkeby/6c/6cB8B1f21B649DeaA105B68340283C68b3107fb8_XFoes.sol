...
contract XFoes is ERC721Enumerable, ReentrancyGuard, Ownable {
    uint256 public price = 10000000000000000; //0.01 ETH

    //Loot Contract
    address public xlootAddress = 0x8bf2f876E2dCD2CAe9C3d272f325776c82DA366d;
    LootInterface public xlootContract = LootInterface(xlootAddress);
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "xFoe #',
                        toString(tokenId),
                        '", "description": "Every xAdventurer needs a xfoe. Randomly generated and stored on chain. Building the @xlootproject metaverse.", "image": "data:image/svg+xml;base64,',
    ...
    function mint(uint256 tokenId) public payable nonReentrant {
        require(tokenId > 15777 && tokenId <= 19778, "Token ID invalid");
    ...
    function multiMint(uint256[] memory tokenIds) public payable nonReentrant {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(tokenIds[i] > 15777 && tokenIds[i] < 19778, "Token ID invalid");
            _safeMint(msg.sender, tokenIds[i]);
        }
    }

    function mintWithLoot(uint256 lootId) public payable nonReentrant {
        require(lootId > 8000 && lootId < 15778, "Token ID invalid");
        require(xlootContract.ownerOf(lootId) == msg.sender, "Not the owner of this loot");
    ...
    function multiMintWithLoot(uint256[] memory lootIds) public payable nonReentrant {
        for (uint256 i = 0; i < lootIds.length; i++) {
            require(xlootContract.ownerOf(lootIds[i]) == msg.sender, "Not the owner of this loot");
            _safeMint(_msgSender(), lootIds[i]);
        }
    }

    function withdraw() public onlyOwner {
        payable(0x75471391C9a9834e8247b1544A6C4aEA59839ca2).transfer(address(this).balance);
    }
