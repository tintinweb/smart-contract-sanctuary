...
contract Bloodline is ERC721Tradable, ReentrancyGuard {
    ...
    function getName(uint256 tokenId) public view returns (string memory) {
        ...
        if (keccak256(bytes(gender)) == keccak256(bytes("Female"))) {
            name = string(abi.encodePacked(pluck(tokenId, "FIRSTNAME", FirstNameForFemale), " ", lastName));
        } else {
            name = string(abi.encodePacked(pluck(tokenId, "FIRSTNAME", FirstNameForMale), " ", lastName));
        }
    ...
    function getChildName(string memory gender, string memory lastName, uint256 rand) internal view returns (string memory) { 
        ...
        if (keccak256(bytes(gender)) == keccak256(bytes("Female"))) {
            firstName = pluck(descendentID, string(abi.encodePacked("FIRSTNAME", toString(rand))), FirstNameForFemale);
        } else {
            firstName = pluck(descendentID, string(abi.encodePacked("FIRSTNAME", toString(rand))), FirstNameForMale);
        }
    ...
    function getAppearance(uint256 tokenId) public view returns (string memory) {
        ...
        if (greatness == 4) {
            string memory gender = getGender(tokenId);
            string memory specialAppearance;
            if (keccak256(bytes(gender)) == keccak256(bytes("Female"))) {
                specialAppearance = pluck(tokenId, "SPECIALFORFEMALE", specialForFemale);
            } else {
                specialAppearance = pluck(tokenId, "SPECIALFORMALE", specialForMale);
            }
            if (keccak256(bytes(appearance)) == keccak256(bytes("skinny")) 
                    && (keccak256(bytes(specialAppearance)) == keccak256(bytes("gorgeous")) || keccak256(bytes(specialAppearance)) == keccak256(bytes("handsome")))) 
                    return appearance;
    ...
    function getChildAppearance(string memory gender, uint256 childID, uint256 maleTokenId, uint256 femaleTokenId) internal view returns (uint256, string memory) {
        string memory appearance = pluck(childID, string(abi.encodePacked("BUILD", toString(maleTokenId), toString(femaleTokenId))), Build);
        ...
        if ((maleGreatness == 4 && femaleGreatness == 4) || greatness == 4) {
            string memory specialAppearance;
            if (keccak256(bytes(gender)) == keccak256(bytes("Female"))) {
                specialAppearance = pluck(descendentID, "SPECIALFORFEMALE", specialForFemale);
            } else {
                specialAppearance = pluck(descendentID, "SPECIALFORMALE", specialForMale);
            }
            if (keccak256(bytes(appearance)) == keccak256(bytes("skinny")) 
                    && (keccak256(bytes(specialAppearance)) == keccak256(bytes("gorgeous")) || keccak256(bytes(specialAppearance)) == keccak256(bytes("handsome")))) 
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Resident #', toString(tokenId), '", "description": "Bloodline is a virtual world with randomized NFT residents. The first generation has 5000 residents. A simple rule in this world: a heterosexual couple of the same generation can give birth to a new resident.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), ', "attributes": [{"trait_type": "Base", "value": "Starfish"}]', '"}'))));
    ...
    function giveBirth(uint256 maleTokenId, uint256 femaleTokenId) public nonReentrant returns (uint256) {
        require(keccak256(bytes(getGender(maleTokenId))) == keccak256(bytes("Male")) && keccak256(bytes(getGender(femaleTokenId))) == keccak256(bytes("Female")), "Gender invalid");
        ...
        string memory gender = pluck(descendentID, string(abi.encodePacked("GENDER", toString(rand))), Gender);
        ...
        characterMap[descendentID] = pluck(descendentID, string(abi.encodePacked("CHARACTER", toString(rand))), Character);
        hobbyMap[descendentID] = pluck(descendentID, string(abi.encodePacked("HOBBY", toString(rand))), Hobby);
    ...
    constructor(address _proxyRegistryAddress) ERC721Tradable("Bloodline", "BLOODLINE", _proxyRegistryAddress) Ownable() {}
}
