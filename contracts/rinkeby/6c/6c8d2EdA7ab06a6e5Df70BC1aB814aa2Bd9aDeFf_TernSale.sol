...
contract TernSale is ERC721Enumerable, Ownable {
    ...
    uint256 public gasPriceLimit = 300 gwei;
    ...
    function getOwnedTokens(address from) external view returns (uint256[] memory) {
        uint tokensMinted = IERC721(address(this)).balanceOf(from);
        uint256[] memory tokens = new uint256[](tokensMinted);

        for (uint256 i = 0; i < tokensMinted; i++) {
    ...
    function mint(bytes memory signature) public payable onlySigned(signature) {
        ...
        require(saleState == SaleState.ON,      "Sale not live");
    ...
    function withdrawBalance() public payable onlyOwner {
        ...
        _widthdraw(_withdrawAddress, _balance);
    }
