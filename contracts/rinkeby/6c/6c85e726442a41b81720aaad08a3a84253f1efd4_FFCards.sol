...
contract FFCards is ERC1155 {
    ...
    string metadataURI="https://api.fastfood.cards/card/{id}";
    ...
    uint256 constant MAX_PER_TX=20;  // buy limit per tx
    ...
    function buy(uint256 id,uint256 amount) public  payable{
        ...
        uint256 cardSupply= normalMinted[id-1];
        ...
        if(normal || specialMinted[id-1]){
          normalMinted[id-1]=cardSupply+1;
          mint(id,msg.sender,amount);
        }
        else{
           specialMinted[id-1]=true;
