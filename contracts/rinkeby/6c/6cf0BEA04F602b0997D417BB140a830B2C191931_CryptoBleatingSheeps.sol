...
contract CryptoBleatingSheeps is ERC721Enumerable, Ownable {
  ...
  uint256 public discountprice = 0.05 ether; // Discount Price for CDB Holders
  ...
  bool public onlyCdblisted = true;
  address[] public whitelistedAddresses;
  address[] public cdbListedAddresses;
...
function cdbListedMint(uint256 _mintAmount) public payable {  // public check etherscan
    require(!paused, "Sale not active!");
    uint256 supply = totalSupply();
    require(_mintAmount > 0, " Min 1 CBS");
    require(_mintAmount <= maxMintAmount, "max mint amount per session exceeded");
    ...
    if (msg.sender != owner()) {
        if(onlyCdblisted == true) {
            require(isCdblisted(msg.sender), "address is not cdbListed");
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(ownerMintedCount + _mintAmount <= nftPerAddressLimit, "max CBS per address exceeded");
        }
  ...
  function isWhitelisted(address _user) public view returns (bool) {
    ...
    return false;
     ...
     function isCdblisted(address _user) public view returns (bool) {
    for (uint i = 0; i < cdbListedAddresses.length; i++) {
      if (cdbListedAddresses[i] == _user) {
          return true;
      }
    }
    return false;
  ...
  function setOnlyCdblisted(bool _state) public onlyOwner {
    onlyCdblisted = _state;
  }
  
  function whitelistUsers(address[] calldata _users) public onlyOwner { //ARRAY users
    delete whitelistedAddresses;
    whitelistedAddresses = _users;
  }

  function cdbListUsers(address[] calldata _users) public onlyOwner { //ARRAY users
    delete cdbListedAddresses;
    cdbListedAddresses = _users;
  }
