...
contract NFTTPreSale is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        if (msg.sender != owner()) {
            if(onlyWhitelisted) {
                require(whitelistedAdresses[msg.sender] == true,"user not add in whitelisted");
                require(supply + _mintAmount <= maxSupplyPresale,"amount is greater than the maximum on PreSale");
            }
    ...
    function mintOnAddress(address _user,uint256 amount) public payable onlyOwner{
        uint256 supply = totalSupply();
        require(amount <= maxMintAmount,"amount is greater than the maximum one mint amounts");
    ...
    function setMintAmountPresale(uint256 _newmaxMintAmount) public onlyOwner() {
        maxSupplyPresale = _newmaxMintAmount;
    }
