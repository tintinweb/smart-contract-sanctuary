...
contract Goat is IGoat, ERC721Enumerable, Ownable, Pausable {
    ...
    */
    ...
    function swapTokens(uint[] memory _tokenIds, bool _stake) external whenNotPaused {
        ...
        uint16[] memory tokenIds = _stake ? new uint16[](amount) : new uint16[](0);
        uint256 seed;
        for (uint i = 0; i < amount; i++) {
            minted++;
            seed = random(minted, lastWrite[tx.origin].time, lastWrite[tx.origin].blockNum);
        ...
        if (_stake) protectedIsland.addManyToProtectedIslandAndPack(_msgSender(), tokenIds);
        // update lastWrite for sender
        updateOriginAccess();
    }
