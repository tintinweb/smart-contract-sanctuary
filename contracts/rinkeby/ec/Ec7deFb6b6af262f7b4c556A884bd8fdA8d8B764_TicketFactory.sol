...
contract TicketFactory is ERC1155, ERC1155Holder, AccessControl, Ownable {
    ...
    function addEvent(uint256 _maxSupply, string memory _name) external onlyOwner {
        Events.push(_name);
        maxSupply.push(_maxSupply);
        // test - delete!!
        // uint256 _eventId = Events.length - 1;
        // _mint(msg.sender, _eventId, _maxSupply, "");      
    }
    ...
    /// @notice Mints 1 ticket
    function safeMint(uint _eventId, address _beneficiary) public {
        require(hasRole(MINTER_ROLE, msg.sender), "Caller does not have minter role");
        _mint(_beneficiary, _eventId, 1, "");        
    }
