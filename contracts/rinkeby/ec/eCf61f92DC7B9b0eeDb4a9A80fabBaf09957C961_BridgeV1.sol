...
contract BridgeV1 is Initializable, OwnableUpgradeable {
    ...
    event AddOutsidePeggedToken(address indexed sourceNetworkTokenAddress, address indexed peggedTokenAddress);
    event DepositTokens(address indexed sourceNetworkTokenAddress, uint256 amount, address indexed receiverAddress);
    event ReleaseTokens(address indexed sourceNetworkTokenAddress, uint256 amount, address indexed receiverAddress);
    ...
    /**
     * @param _isSource Whether the bridge is a source network bridge or target
     */
    function initialize(bool _isSource) public {
    ...
    function addTokenToWhitelist(address tokenAddress) public onlyOwner {
        ...
        collectedBridgeFees[tokenAddress] = 0;
    }
    ...
    function addOutsidePeggedToken(
        ...
        bridgeFees[peggedTokenAddress] = 0;
        emit AddOutsidePeggedToken(sourceNetworkTokenAddress, peggedTokenAddress);
    }

    /**
     * The core function that accepts token transfers to other networks
     */
    function deposit(address tokenAddress, uint256 amount, address receiverAddress) public payable {
        ...
        if (isSource) {
            ...
            require(token.transferFrom(msg.sender, address(this), amount) == true, "Could not deposit token");
        } else {
            // on the target bridge side tokens are burned again after they've been minted before
            ERC20Burnable(token).burnFrom(msg.sender, amountAfterFees);
            // keeping the bridge fees in this contract
            require(token.transferFrom(msg.sender, address(this), bridgeFees[tokenAddress]) == true, "Could not take bridge fees");
        }

        // We always dispatch the deposit event with the "true" token address in the source network
        emit DepositTokens(isSource ? tokenAddress : reverseOutsidePeggedTokens[tokenAddress], amountAfterFees, receiverAddress);
    }

    /**
     * Returns the current bridging fee for a certain token
     */
    function getBridgeFee(address tokenAddress) public view returns (uint256) {
        return bridgeFees[tokenAddress];
    }

    /**
     * Returns whether a token is whitelisted or not
     */
     ...
     */
    function release(address receiverAddress, address sourceNetworkTokenAddress, uint256 amount) public {
        ...
        if (isSource) {
            ...
            require(token.transferFrom(address(this), receiverAddress, amount) == true, "Could not release tokens");
        } else {
        ...
        emit ReleaseTokens(sourceNetworkTokenAddress, amount, receiverAddress);
    }
     ...
     */
    function setBridgeFee(address tokenAddress, uint256 fee) public {
        require(permittedOracleAddresses[tokenAddress][msg.sender] == true, "Fee change not permitted");
     ...
     */
    function withdrawBridgeFees(address tokenAddress, uint256 amount) public returns (bool) {
        require(permittedOracleAddresses[tokenAddress][msg.sender] == true || msg.sender == owner(), "Claim not permitted");
        require(collectedBridgeFees[tokenAddress].sub(amount) >= 0, "Not enough bridge fees collected");
        collectedBridgeFees[tokenAddress] = collectedBridgeFees[tokenAddress].sub(amount);
        return IERC20(tokenAddress).transferFrom(address(this), msg.sender, amount);
    }
}
