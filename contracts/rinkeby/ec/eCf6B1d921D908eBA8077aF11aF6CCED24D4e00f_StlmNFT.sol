...
contract StlmNFT is ERC721("Living Module v3", "STLMv3"), Ownable {
     ...
     */
    function batchMint(address[] calldata _owners, address[] calldata _creators, string[] calldata _uris) external onlyOwner returns (uint256[] memory ) {
        require(_owners.length == 1 || _owners.length == _uris.length, "Length should be one or same");
        require(_creators.length == 1 || _creators.length == _uris.length, "Length should be one or same");
