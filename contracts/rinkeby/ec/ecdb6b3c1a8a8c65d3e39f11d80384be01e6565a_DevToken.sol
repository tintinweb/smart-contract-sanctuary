...
contract DevToken is Ownable, ERC721Pausable {
    ...
    mapping(address => uint256[]) public ImgPerAddress;
    ...
    */
    function mintPack(string[] memory tokenURI, uint256[] memory mintedImg) public payable saleIsOpen returns (uint256) {
        require((ImgPerAddress[msg.sender].length + mintedImg.length) <= MAX_BY_MINT, "Exceeds number");
    ...
    function _mintAnElement(address _to, string memory tokenURI) private {
        ...
        _setTokenURI(newItemId, tokenURI);
        ImgPerAddress[msg.sender].push(newItemId);
        emit MintPack(msg.sender, newItemId);
    }
    ...
    function burnIndividual(uint256 requestId) public {
        ...
        for(uint256 i=0; i<= ImgPerAddress[msg.sender].length; i++) {
            if(ImgPerAddress[msg.sender][i] == requestId) {
                delete ImgPerAddress[msg.sender][i];
            }
        }
