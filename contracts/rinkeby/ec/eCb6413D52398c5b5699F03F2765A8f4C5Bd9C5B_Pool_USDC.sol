...
interface IARTHPool {
    ...
    function redeem(uint256 arthAmount, uint256 collateralOutMin, uint256 id)
        external;
...
contract TestArthPool is AccessControl, IARTHPool {
    ...
    mapping(uint256 => amountsRecord) public mintersDetails;
    uint256 mintId = 0;
...
// END AMOS //
    ...
    function mint(
        ...
        uint256 id = ++mintId;
        amountsRecord storage mintersdetails = mintersDetails[id];
        mintersdetails.id = id;
        mintersdetails.amount = collateralAmountD18;
    ...
    function redeem(
        ...
        uint256 collateralNeeded = calcCr(arthAmount, _arthController.getGlobalCollateralRatio());

        if (collateralNeeded <= mintersDetails[_id].amount && msg.sender == mintersDetails[_id].sender) {
            ...
            collateralNeeded = (collateralNeeded.mul(uint256(1e6).sub(_arthController.getMintingFee()))).div(1e6);
       ...
       lastRedeemed[msg.sender] = block.number;
        ...
        _ARTH.poolBurnFrom(msg.sender, arthAmount);
         ...
         redeemBis(arthAmount, collateralOutMin); 
            
        }
    }
    

    function redeemBis(uint256 arthAmount, uint256 collateralOutMin) internal returns (uint256) {
...

{
  ...
  "libraries": {
    "/var/www/html/boltfork/arthcoin-v2/contracts/Arth/Pools/ArthPoolLibrary.sol": {
      "ArthPoolLibrary": "0xe27E128107005FC5B32d93C61ADE749F2adFd64A"
    }
  },
  "outputSelection": {
    "*": {
