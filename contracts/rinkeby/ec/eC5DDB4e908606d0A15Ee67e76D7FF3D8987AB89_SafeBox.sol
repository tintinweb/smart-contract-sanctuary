...
contract SafeBox is Governable, ERC20, ReentrancyGuard {
  ...
  function deposit(uint amount) external nonReentrant {
    uint uBalanceBefore = uToken.balanceOf(address(this)); //0  
    uToken.safeTransferFrom(msg.sender, address(this), amount); // 10 
    uint uBalanceAfter = uToken.balanceOf(address(this)); // 10 
    uint cBalanceBefore = cToken.balanceOf(address(this)); // 0 
    require(cToken.mint(uBalanceAfter.sub(uBalanceBefore)) == 0, '!mint'); // 10 - 0
