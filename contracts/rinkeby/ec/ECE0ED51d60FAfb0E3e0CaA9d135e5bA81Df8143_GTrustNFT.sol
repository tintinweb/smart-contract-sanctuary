...
// File contracts/GTrustNFTDNA.sol



pragma solidity ^0.8.0;

contract GTrustNFTDNA {
...
contract GTrustNFT is ERC721, ERC721Enumerable, GTrustNFTDNA {
    ...
    constructor(uint256 _maxSupply) ERC721("GTrustNFT", "GTN") {
        maxSupply = _maxSupply;
    }
    
    function mint() public {
        uint256 current = _idCounter.current();
        require(current < maxSupply, "Cannot mint more than maxSupply");
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), "ERC721Enumerable::tokenURI is not implemented");
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "GTrustNFT #',
                tokenId.toString(),
                '", "description": "GTrust NFT are randomized Avataaars", "image": "',
                image,
                '"}'
                )
            );
