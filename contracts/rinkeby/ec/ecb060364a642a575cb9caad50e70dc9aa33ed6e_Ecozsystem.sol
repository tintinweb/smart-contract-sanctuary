...
interface IJag {
    ...
    function jaguarTokenURI(uint256 jagID) external view returns(string memory);
}
/*
interface IBuck {
    function buckRandomizer(address user, uint256 buckID) external returns(uint256); 
    function bushBuckTokenURI(uint256 buckID) external view returns(string memory);
}

interface ITree {
    function treeRandomizer(address user, uint256 treeID) external returns(uint256); 
    function bananaTreeTokenURI(uint256 treeID) external view returns(string memory);
}
*/
//--------------------------------------------------------------------------------------------------------------------------------------------------------
...
contract Ecoz is ERC721, Ownable {
    ...
    uint256 public jagCost = 0.1 ether;
    uint256 public buckCost = 0.075 ether;
    uint256 public treeCost = 0.05 ether;
    uint256 public maxMintAmount = 2;
    uint256 public jagSupply = 0;
    uint256 public buckSupply = 0;
    uint256 public treeSupply = 0;
    uint256 public wlCounter = 0;
    uint256 public ogCounter = 0;
    ...
    mapping (address => uint256) public weightOfOwner;
    mapping (address => uint256) public balanceJaguar;
    mapping (address => uint256) public balanceBushBuck;
    mapping (address => uint256) public balanceBananaTree;
    mapping (address => uint256) public jagMinted;
    mapping (address => uint256) public buckMinted;
    mapping (address => uint256) public treeMinted;
    ...
    constructor(address jagAddress/*, address buckAddress, address treeAddress*/) ERC721("Ecoz","ECOZ") {
		jaguarsURI = IJag(jagAddress); //JaguarsURI.sol
		//bushBucksURI = IBuck(buckAddress); //BushBucksURI.sol
		//bananaTreesURI = ITree(treeAddress); //BananaTreesURI.sol
	}
    ...
    */
    ...
    function MintJaguar(uint256 _mintAmount) public payable {  
        if (msg.sender != owner()) {
            require(msg.value == jagCost * _mintAmount,     "Ether sent is not correct");
            require(jagSupply + _mintAmount <= 450,         "Over max supply");

            if (ogSale == true) {   
                require(ogAddress[msg.sender] == true,                                                                          "The OG presale has not started");
                require(jagMinted[msg.sender] + buckMinted[msg.sender] + treeMinted[msg.sender] + _mintAmount <= maxMintAmount, "max amount of Ecoz minted");
                require(jagMinted[msg.sender] + _mintAmount <= 2,                                                               "Only 2 mints per species allowed");
            }
    ...
    function MintBushBuck(uint256 _mintAmount) public payable { 
        if (msg.sender != owner()) {
            require(msg.value == jagCost * _mintAmount,     "Ether sent is not correct");
            require(buckSupply + _mintAmount <= 900,        "Over max supply");

            if (ogSale == true) {   
                require(ogAddress[msg.sender] == true,                                                                          "The OG presale has not started");
                require(jagMinted[msg.sender] + buckMinted[msg.sender] + treeMinted[msg.sender] + _mintAmount <= maxMintAmount, "max amount of Ecoz minted");
                require(buckMinted[msg.sender] + _mintAmount <= 2,                                                               "Only 2 mints per species allowed");
            }
            else if (whitelistSale == true) {
                require((ogAddress[msg.sender] == true) || (wlAddress[msg.sender] == true),                                     "This address is not whitelisted");
                require(jagMinted[msg.sender] + buckMinted[msg.sender] + treeMinted[msg.sender] + _mintAmount <= maxMintAmount, "max amount of Ecoz minted");
    ...
    function MintBananaTree(uint256 _mintAmount) public payable {        
        if (msg.sender != owner()) {
            require(msg.value == jagCost * _mintAmount,     "Ether sent is not correct");
            require(treeSupply + _mintAmount <= 1800,       "Over max supply");

            if (ogSale == true) {   
                require(ogAddress[msg.sender] == true,                                                                          "The OG presale has not started");
                require(jagMinted[msg.sender] + buckMinted[msg.sender] + treeMinted[msg.sender] + _mintAmount <= maxMintAmount, "max amount of Ecoz minted");
                require(treeMinted[msg.sender] + _mintAmount <= 2,                                                               "Only 2 mints per species allowed");
            }
            else if (whitelistSale == true) {
                require((ogAddress[msg.sender] == true) || (wlAddress[msg.sender] == true),                                     "This address is not whitelisted");
                require(jagMinted[msg.sender] + buckMinted[msg.sender] + treeMinted[msg.sender] + _mintAmount <= maxMintAmount, "max amount of Ecoz minted");
    ...
    */
    ...
    function setJagCost(uint256 _newCost) public onlyOwner {
        jagCost = _newCost;
    }

    function setBuckCost(uint256 _newCost) public onlyOwner {
        buckCost = _newCost;
    }   

    function setTreeCost(uint256 _newCost) public onlyOwner {
        treeCost = _newCost;
    }
    ...
    function addWhitelistBundle(address[] memory wlBundle) public onlyOwner {
        for (uint256 i = 0; i < wlBundle.length; i++) {
            require(!wlAddress[wlBundle[i]],        "Address already Exist");
    ...
    function addOGBundle(address[] memory ogBundle) public onlyOwner {
        for (uint256 i = 0; i < ogBundle.length; i++) {
            require(!ogAddress[ogBundle[i]],        "Address already Exist");
    ...
    /*       
     ____   ____     ___    ___   ___        _____  __ __  ____     __ ______  ____  ___   ____   _____
    |    \ |    \   /  _]  /  _] |   \      |     ||  |  ||    \   /  ]      ||    |/   \ |    \ / ___/
    |  D  ||  D  ) /  [_  /  [_ |     \     |   __||  |  ||  _  | /  /|      | |  ||     ||  _  (   \_ 
    |    / |    / |    _]|    _]|  D  |     |  |_  |  |  ||  |  |/  / |_|  |_| |  ||  O  ||  |  |\__  |
    |    \ |    \ |   [_ |   [_ |     |     |   _] |  :  ||  |  /   \_  |  |   |  ||     ||  |  |/  \ |
    |  D  ||  .  \|     ||     ||     |     |  |   |     ||  |  \     | |  |   |  ||     ||  |  |\    |
    |____| |__|\_||_____||_____||_____|     |__|    \__,_||__|__|\____| |__|  |____|\___/ |__|__| \___|
...
contract Ecozsystem is Ecoz {
    ...
    uint256 babyJagSupply = 0;
    uint256 babyBuckSupply = 0;
    uint256 babyTreeSupply = 0;

    uint256 JAG_BREED_COST = 600;
    uint256 BUCK_BREED_COST = 480;
    uint256 TREE_BREED_COST = 360;

    uint256 [] populationRange = [3, 6, 9, 12];
    uint256 [] jagRangeOver = [615, 630, 645, 660, 690];
    uint256 [] jagRangeUnder = [585, 570, 555, 540, 510];
    uint256 [] buckRangeOver = [495, 510, 525, 540, 570];
    uint256 [] buckRangeUnder = [465, 450, 435, 420, 390];
    uint256 [] treeRangeOver = [375, 390, 405, 420, 450];
    uint256 [] treeRangeUnder = [345, 330, 315, 300, 270];
    
    bool public breedingPaused = false;
            
    mapping (address => uint256) public balanceBabyJaguar;
    mapping (address => uint256) public balanceBabyBushBuck;
    mapping (address => uint256) public balanceBabyBananaTree;
    ...
    //Claim OG Ecoz Thrive bonus
    function claimBonus(address user) external {
        require(ogAddress[msg.sender] == true,              "This address is not eleigible for a bonus");
        require(bonusClaimed[msg.sender] == false,          "Bonus claimed already");
        Thrive.mintBonus(user,150 ether);
    ...
    function transferFrom(address from, address to, uint256 tokenID) public override {
        if (tokenID <= 1350) {
            if (tokenID <= 450) {
                ...
                balanceBabyJaguar[from]--;
                balanceBabyJaguar[to]++;
            }
            ...
            if (tokenID <= 2250) {
                ...
                balanceBabyBushBuck[from]--;
                balanceBabyBushBuck[to]++;
            }
            ERC721.transferFrom(from, to, tokenID);
        }
            ...
            if (tokenID <= 5850) {
                ...
                balanceBabyBananaTree[from]--;
                balanceBabyBananaTree[to]++;
            }
            ERC721.transferFrom(from, to, tokenID);
        }
    }
    ...
    function BreedJaguar() public {
        ...
        require(babyJagSupply + 1 <= 900,                   "Max amount of jaguars bred");
                
        Thrive.burn(msg.sender, JAG_BREED_COST);
        babyJagSupply++;
        weightJaguar[babyJagSupply + 450] = jaguarsURI.jagRandomizer(msg.sender, babyJagSupply + 450);
        balanceBabyJaguar[msg.sender]++;
        updateBreedCosts();
    ...
    function BreedBushBuck() public {
        ...
        require(babyBuckSupply + 1 <= 1800,                 "Max amount of bush bucks bred");
        
        Thrive.burn(msg.sender, BUCK_BREED_COST);
        babyBuckSupply++;
        //weightBushBuck[babyBuckSupply + 2250] = bushBucksURI.buckRandomizer(msg.sender, babyBuckSupply + 2250);
        balanceBabyBushBuck[msg.sender]++;
        updateBreedCosts();
    ...
    function BreedBananaTree() public {
        ...
        require(babyTreeSupply + 1 <= 3600,                 "Max amount of banana trees bred");     
        
        Thrive.burn(msg.sender, TREE_BREED_COST);
        babyTreeSupply++;
        //weightBananaTree[babyTreeSupply + 5850] = bananaTreesURI.treeRandomizer(msg.sender, babyTreeSupply + 5850);
        balanceBabyBananaTree[msg.sender]++;
        updateBreedCosts();
    ...
    function tokenURI(uint256 tokenID) public view virtual override returns(string memory) {
        ...
        if (tokenID <= 1350) {
            return jaguarsURI.jaguarTokenURI(tokenID);
        }
        else if ((tokenID > 1350) && (tokenID <= 4050)) {
            //return bushBucksURI.bushBuckTokenURI(tokenID);
            return "wrongID";
        }
        else {
            //return bananaTreesURI.bananaTreeTokenURI(tokenID);
            return "wrongID";
        }
    }

    function updateBreedCosts() public {
        uint256 jagPopulation = jagSupply + babyJagSupply;
        uint256 buckPopulation = buckSupply + babyBuckSupply;
        uint256 treePopulation = treeSupply + babyTreeSupply;
        ...
        if (((jagPopulation * 2) >= buckPopulation) && ((buckPopulation * 2) >= treePopulation)) {
            ...
            for (uint8 i; i < populationRange.length; i++) {
                ...
                if (a <= populationRange[i]) {
                    JAG_BREED_COST = jagRangeOver[i];
                }
                if (a > populationRange[3]) {
                    JAG_BREED_COST = jagRangeOver[4];
                }
            }
            for (uint8 j; j < populationRange.length; j++) {
                ...
                if (b <= populationRange[j]) {
                    BUCK_BREED_COST = buckRangeOver[j];
                    TREE_BREED_COST = treeRangeUnder[j];
                }
                if (b > populationRange[3]) {
                    BUCK_BREED_COST = buckRangeOver[4];
                    TREE_BREED_COST = treeRangeUnder[4];
                }
            }
            ...
            for (uint8 i; i < populationRange.length; i++) {
                if (a <= populationRange[i]) {
                    JAG_BREED_COST = jagRangeUnder[i];
                }
                if (a > populationRange[3]) {
                    JAG_BREED_COST = jagRangeUnder[4];
                }
            }
            for (uint8 j; j < populationRange.length; j++) {
                if (b <= populationRange[j]) {
                    BUCK_BREED_COST = buckRangeUnder[j];
                    TREE_BREED_COST = treeRangeOver[j];
                }
                if (b > populationRange[3]) {
                    BUCK_BREED_COST = buckRangeUnder[4];
                    TREE_BREED_COST = treeRangeOver[4];
                }
            }
        }
        else if (((jagPopulation * 2) >= buckPopulation) && ((buckPopulation * 2) < treePopulation)) {
