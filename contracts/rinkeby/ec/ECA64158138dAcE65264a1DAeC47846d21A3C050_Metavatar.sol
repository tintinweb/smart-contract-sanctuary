...
contract Metavatar is ERC721, ReentrancyGuard, Ownable {
    ...
    function _bulkMint(uint256 numTokens, address destination) private {
        ...
        require(numTokens <= MAX_PER_TX, "Mint fewer metavatars");
        require(numTokens > 0, "Must mint at least 1");
        require(
            _mintedPerAddress[destination] + numTokens <= MAX_PER_ADDRESS,
    ...
    function claimForSelf() public payable virtual {
        require(msg.value >= price, "Need more funds to mint metavatar nft");
        _mint(msg.sender);
    }

    function claimForFriend(address walletAddress) public payable virtual {
        require(msg.value >= price, "Need more funds to mint metavatar nft");
        _mint(walletAddress);
    }

    function generateMetavatar() public view {
        string memory chosen_color = colors[
            random(
                string(
                    abi.encodePacked(
                        block.difficulty,
        ...
        console.log("Random color chosen ", chosen_color);
    }
    ...
    function getRandomInRange(
        ...
        console.log("rand value: ", randValue);
        return randValue;
    }
    ...
    function getFloatingPoint(
        ...
        string memory floatingNum = string(
            abi.encodePacked(
                "0.",
                Strings.toString(getRandomInRange(tokenId, keyword, min, max))
            )
        );
        console.log("Rand float value: ", floatingNum);
    }
        ...
        returns (string memory)
    {
        ...
        (metav, svg) = MetavatarGenerator.genMetavatarWithSeed(seed);
        ...
        // string memory svg_output = string(
        //     '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 400"><path d="M250 0 L75 200 L225 200 Z" /></svg>'
        // );
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        '","description": "Unique pfps (on Chain) for the entire metaverse.", "image": "data:image/svg+xml;base64,',
...
library MetavatarGenerator {
    ...
    struct Constants {
        uint256 MAX_SHAPES;
        uint256 MIN_SHAPE_SIZE;
        uint256 MAX_SHAPE_SIZE;
        uint256 MIN_BACKGROUND_OPACITY;
        uint256 MAX_BACKGROUND_OPACITY;
        uint256 MIN_SHAPE_OPACITY;
        uint256 MAX_SHAPE_OPACITY;
    }
        ...
        returns (MetavatarStruct memory, string memory)
    {
        ...
        Constants memory C;
        C.MAX_SHAPES = 3;
        C.MIN_SHAPE_SIZE = 800;
        C.MAX_SHAPE_SIZE = 1280;
        C.MIN_BACKGROUND_OPACITY = 10;
        C.MAX_BACKGROUND_OPACITY = 50;
        C.MIN_SHAPE_OPACITY = 50;
        C.MAX_SHAPE_OPACITY = 100;
        // string[2] memory modes = ["#FFFFFF", "#000000"];
        // uint256 MAX_SHAPES = 3;
        // uint256[3] memory TYPE_SHAPE = [uint256(0), 1, 2]; // 0: rectange, 1: ellipse, 2: triange
        // string[3] memory TYPE_SHAPE = ["rectangle", "ellipse", "triangle"];
        // uint256 MIN_SHAPE_SIZE = 800; // 0.5 * 1600
        // uint256 MAX_SHAPE_SIZE = 1280; // 0.8 * 1600
        // Having opacity factor of 100 since we can't have float values in solidity
        // uint256 MIN_BACKGROUND_OPACITY = 10; // 0.1
        // uint256 MAX_BACKGROUND_OPACITY = 50; // 0.5
        // uint256 MIN_SHAPE_FILL_OPACITY = 50;
        // uint256 MAX_SHAPE_FILL_OPACITY = 100;
        // string[2] memory COLOR_FILL = ["solid", "linear_gradient"];
        ...
        metav.numShapes = getRandomInRange(seed, "SHAPES", 1, C.MAX_SHAPES + 1);
        ...
        uint256 opacityValue = getRandomInRange(
            seed,
            "BG_OPACITY",
            C.MIN_BACKGROUND_OPACITY,
            C.MAX_BACKGROUND_OPACITY
        );
        metav.bgOpacity = string(
            abi.encodePacked(".", Strings.toString(opacityValue))
        );
        metav.noiseOpacity = !metav.lightMode
            ? string(
                abi.encodePacked(".", Strings.toString(100 - opacityValue))
            )
            : "1.0";

        svgParts[
            0
        ] = '<svg width="400" height="400" viewBox="0 0 1600 1600" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" style="border-radius: 0px">';
        svgParts[1] = string(
            abi.encodePacked(
        ...
        for (uint256 i = 0; i < metav.numShapes; i++) {
            ...
            if (sh.shapeType == 0) {
                shapeSvg = createRectangle(sh);
            } else if (sh.shapeType == 1) {
                shapeSvg = createEllipses(sh);
            } else {
                shapeSvg = createTriangle(sh);
            }
        ...
        svgParts[4] = string(
            abi.encodePacked(
                '</g></g><g style="mix-blend-mode:overlay"><rect width="1600" height="1600" fill="url(#pattern0)" fill-opacity="',
                metav.noiseOpacity,
                '" />'
            )
        );
        svgParts[
            5
        ] = '</g><defs><filter id="filter0_f_50_327" x="0" y="0" width="1600" height="1600" filterUnits="userSpaceOnUse" color-interpolation-filters="sRGB"><feFlood flood-opacity="0" result="BackgroundImageFix"/><feBlend mode="normal" in="SourceGraphic" in2="BackgroundImageFix" result="shape"/><feGaussianBlur stdDeviation="125" result="effect1_foregroundBlur_50_327"/></filter><pattern id="pattern0" patternContentUnits="objectBoundingBox" width="0.08375" height="0.08375"><use xlink:href="#image0_50_327" transform="scale(0.000380682)"/></pattern>';
        svgParts[
            6
        ] = '<image id="image0_50_327" width="220" height="220" xlink:href="https://gateway.pinata.cloud/ipfs/QmdyrckeBdKTj5QFdwzFzZJAfsnTYF8vNE7gYDXQTEtQm6"/>';
        svgParts[7] = "</defs></svg>";
    ...
    function createRectangle(Shape memory sh)
        private
        pure
        returns (string memory)
    {
    ...
    function createEllipses(Shape memory sh)
        private
        pure
        returns (string memory)
    {
    ...
    function createTriangle(Shape memory sh)
        private
        pure
        returns (string memory)
    {
    ...
    function getRandomInRangeWithSeed(
        string memory seed,
        uint256 min,
        uint256 max
    ) public pure returns (uint256) {
