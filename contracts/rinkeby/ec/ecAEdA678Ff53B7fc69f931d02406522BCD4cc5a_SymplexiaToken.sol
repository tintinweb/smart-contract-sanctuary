...
//   ====================================== 
    constructor () {
        ...
        _rawBalances[_msgSender()] = _tokensSupply;
        ...
        // _excludeFromBonus(liquidityPair);                   // To study a little bit more
...
//   ======================================
    ...
    function balanceOf(address account) public view override returns (uint256) {
           return _rawBalances[account].add(_calcBonus(account));
    }
...
//   This Function is responsible for taking all fee, if 'applyFee' is _Enabled
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, uint256 applyFee) private {
        ...
        uint256 transferAmount = (tAmount).sub(tAmountFees);
        ...
        bonusBalance = _calcBonus(sender);
        if (bonusBalance > 0)
            bonusQuota = bonusBalance.mul(tAmount).div( _rawBalances[sender].add(bonusBalance) );
        else
            bonusQuota = 0;

        txAmount = tAmount.sub(bonusQuota);
        _rawBalances[sender] = _rawBalances[sender].sub(txAmount);
        _totalBonusDue =  _totalBonusDue.sub(bonusQuota); 
    
        bonusBalance = _calcBonus(recipient);
        if (bonusBalance > 0)
            bonusQuota = bonusBalance.mul(transferAmount).div( _rawBalances[recipient].add(bonusBalance) );
        else
            bonusQuota = 0;

        txAmount = transferAmount.sub(bonusQuota);
        _rawBalances[recipient] =  _rawBalances[recipient].add(txAmount);
        _totalBonusDue =  _totalBonusDue.add(bonusQuota); 

        emit Transfer(sender, recipient, tAmount);
        
        // Collect all fees and share bonus
        ...
        _rawBalances[address(this)]          = _rawBalances[address(this)].add(tLiquidityFee); 
        _rawBalances[_securityWalletAddress] = _rawBalances[_securityWalletAddress].add(tSecurityFee);
        _rawBalances[_projectWalletAddress]  = _rawBalances[_projectWalletAddress].add(tProjectFee);
        _totalBonusDue =  _totalBonusDue.add(tBonus); 
...
//   ======================================
    ...
    function _calcBonus (address account) private view returns (uint256) {
        if (_isExcludedFromBonus[account] || _totalBonusDue == 0 || _rawBalances[account] == 0){
            return 0;
        } else {
            return _totalBonusDue.mul(_getFactorRFI(_rawBalances[account]));

        }
    }

    function _getFactorRFI(uint256 amount) private view returns (uint256) {

        uint256 expurgedBalance = 0;
        for (uint256 i=0; i < _noBonus.length; i++){
            expurgedBalance = expurgedBalance.add(_rawBalances[_noBonus[i]]);
        }

        uint256 _bonusBalances = _tokensSupply.sub(expurgedBalance);
        if (_bonusBalances > 0) {return amount.div(_bonusBalances);} else {return 0;}
    ...
    function BonusBalance() public view returns (uint256) {
        return _totalBonusDue;
    }

    function _excludeFromBonus(address account) internal {
         uint256 _bonus = _calcBonus(account); 
        _rawBalances[account] = _rawBalances[account].add(_bonus);
...
//   ======================================
    ...
    */
    ...
    function includeInBonus(address account) public onlyOwner() {
        ...
        uint256 _adjustedBalance = _rawBalances[account].div(_getFactorRFI(_totalBonusDue).add(1) );
        uint256 _bonusBalance = _rawBalances[account].sub(_adjustedBalance);      // It has a bit distorted cumulative effect 
        for (uint256 i = 0; i < _noBonus.length; i++) {
            if (_noBonus[i] == account) {
                _noBonus[i] = _noBonus[_noBonus.length - 1];
                _isExcludedFromBonus[account] = false;
                _rawBalances[account] =_adjustedBalance;
                _totalBonusDue = _totalBonusDue.add(_bonusBalance);
               _noBonus.pop();
                break;
            }
        }
...
contract SymplexiaToken is  Taxable {
  ...
  //
    ...
    constructor () ERC20 ("DEBUG42", "DBG42") BaseToken (5, 5, 5, 
