...
contract ChainlinkOracle is PriceOracle {
    ...
    mapping(address => uint) internal dexPrices;
    ...
    event PriceRecordUpdated(address token, address baseToken, address lpToken);
    event NewAdmin(address oldAdmin, address newAdmin);
    ...
    function getPrice(CToken cToken) internal view returns (uint price) {
        ...
        } else if (getChainlinkPrice(getFeed(symbol)) != 0) {
            price = getChainlinkPrice(getFeed(symbol));
        } else {
            price = dexPrices[address(token)];
            return price;
        }
    ...
    function setDexPriceRecord(address token, address baseToken, address lpToken) external onlyAdmin() {
        EIP20Interface baseAsset = EIP20Interface(baseToken);
        string memory baseSymbol = baseAsset.symbol();
        if (prices[baseToken] == 0) {
            if (compareStrings(baseSymbol, "WETH")) {
                require(getChainlinkPrice(getFeed("ETH")) > 0, "invalid feed address for WETH");
            } else {
                require(getChainlinkPrice(getFeed(baseSymbol)) > 0, "invalid base address");
            }
        }

        PriceInfo storage priceInfo = priceRecords[token];
        priceInfo.token = token;
        priceInfo.baseToken = baseToken;
        priceInfo.lpToken = lpToken;
        emit PriceRecordUpdated(token, baseToken, lpToken);
    }

    function updateDexPrice(address token) external onlyOperator() {
        PriceInfo storage priceInfo = priceRecords[token];
        uint rawTokenAmount = EIP20Interface(priceInfo.token).balanceOf(priceInfo.lpToken);
        uint tokenDecimalDelta = uint(18).sub(EIP20Interface(priceInfo.token).decimals());
        uint tokenAmount = rawTokenAmount.mul(10**tokenDecimalDelta);
        uint rawBaseTokenAmount = EIP20Interface(priceInfo.baseToken).balanceOf(priceInfo.lpToken);
        uint baseTokenDecimalDelta = uint(18).sub(EIP20Interface(priceInfo.baseToken).decimals());
        uint baseTokenAmount = rawBaseTokenAmount.mul(10**baseTokenDecimalDelta);
        uint baseTokenPrice;

        if (prices[priceInfo.baseToken] != 0) {
            baseTokenPrice = prices[priceInfo.baseToken];
        } else {
            if (compareStrings(EIP20Interface(priceInfo.baseToken).symbol(), "WETH")) {
                baseTokenPrice = getChainlinkPrice(getFeed("ETH"));
            } else {
                baseTokenPrice = getChainlinkPrice(getFeed(EIP20Interface(priceInfo.baseToken).symbol()));
            }
        }

        uint tokenPrice = baseTokenPrice.mul(baseTokenAmount).div(tokenAmount);

        emit PricePosted(token, dexPrices[token], tokenPrice, tokenPrice);
        dexPrices[token] = tokenPrice;
    }
    ...
    function assetDexPrices(address asset) external view returns (uint) {
    ...
    modifier onlyOperator() {
        require(msg.sender == admin || msg.sender == operator, "only operators may call");
        _;
    }
}