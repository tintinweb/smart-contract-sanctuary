...
contract Cryptendont is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;

    Counters.Counter private _tokenId;

    uint256 public constant MAX_CRYPTENDONT = 800;
    uint256 public constant RESERVES = 20;
    uint256 public price = 60000000000000000;
    uint256 public constant MAX_BY_MINT = 10;

    address public constant creatorAddress = 0x45E489Ed5AD1c1Feb25a1e6915Fc6FDb92B9FB03; // 100%
    
    uint private SaleTime = 1630627200; // october 8, 2021 07:00 PM EST
    uint private saleTime = 1630627200; // october 8, 2021 07:00 PM EST
    uint private preSaleTime = 1631966400; // octover 7, 2021 07:00 PM EST
    uint256 private _preSaleSupply = 75;
    string public baseTokenURI;
    
    mapping(address => bool) addressToPreSaleEntry;
    mapping(address => uint256) addressToPreSaleMints;
    constructor(string memory baseURI) ERC721("Cryptendont","CTD") {
        setBaseURI(baseURI);
        pause(true);
    }

    modifier CryptendontSaleIsOpen() {
        require(block.timestamp >= SaleTime, "Sale has not started yet.");
        require(totalSupply() <= MAX_CRYPTENDONT, "CTD sale has ended.");
        if (_msgSender() != owner()) {
            require(!paused(), "Pausable: paused");
        }
        _;
    ...
    modifier saleIsOpen {
        require(_totalSupply() <= MAX_CRYPTENDONT, "Sale end");
        if (_msgSender() != owner()) {
    ...
    function preCryptendontMinted(address _address) public view returns (uint256){
        return addressToPreSaleMints[_address];
    }
    function cryptendontSaleMint(uint256 _count) public payable CryptendontSaleIsOpen {
        uint256 totalSupply = totalSupply();
        
        require(totalSupply < MAX_CRYPTENDONT, "All Cryptendonts are already minted.");
        require(totalSupply + _count <= MAX_CRYPTENDONT, "This amount will exceed max supply.");
    ...
    function mint(uint256 _count) public payable saleIsOpen {
        uint256 totalSupply = totalSupply();
        require(totalSupply + _count <= MAX_CRYPTENDONT, "Max limit");
        require(totalSupply <= MAX_CRYPTENDONT, "Sale has ended");
    ...
    function reserve(uint256 _count) public onlyOwner {
        ...
        for (uint256 i = 0; i < _count; i++) {
            _safeMint(creatorAddress, total + i);
        }
    }
