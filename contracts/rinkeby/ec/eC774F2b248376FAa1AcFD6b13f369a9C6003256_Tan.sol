...
contract Tan is
    ...
    function getGenerationHash(uint generation) external returns (uint) {
        return uint(keccak256(toBytes(generationSeed[generation])));
    }

    function getGenerationSeed(uint generation) external returns (uint) {
        return generationSeed[generation];
    }

    function getHash(uint seed, uint tokenId) external pure returns (uint) {
        return uint(keccak256(toBytes(seed)));
        // return uint(keccak256(bytes32(seed)));
        // return uint(keccak256(abi.encode(seed)));
