...
*/
// A Friggin Eggs Project - $YOLK IS A UTILITY TOKEN FOR THE FRIGGIN EGGS ECOSYSTEM.
// $YOLK is NOT an investment and has NO economic value. 
// It will be earned by active holding within the Friggin Eggs ecosystem. Each Genesis Eggs will be eligible to claim tokens at a rate of 10 $YOLK per day.
...
contract Yolk is ERC20, Ownable {
    ...
    uint256 constant public BASE_RATE = 1 ether;   // 1 $YOLK per day per Friggin Egg
    uint256 public START;   // time of launch (used to calculate reward)
    bool rewardPaused = true;   // By default, stop the reward when deploying
    ...
    constructor(address FrigginEggsAddress) ERC20("Yolk", "YOLK") {
        ...
        START = block.timestamp;    // initialize the time of START
    }
    
    // Function for the FrigginEggs and allowed addresses to use to update rewards
    function updateReward(address from, address to) external {
        require(allowedAddresses[msg.sender] || msg.sender == address(FrigginEggs), "You do not have permission to call this function");
        if(from != address(0)){
    ...
    function claimReward() external {
        ...
        require(FrigginEggs.tokensOfOwner(msg.sender).length > 0, "You need to own a Friggin Egg to claim $YOLK");
        ...
        if (lastUpdate[msg.sender] != 0) {
            ...
            _mint(msg.sender, FrigginEggs.tokensOfOwner(msg.sender).length * BASE_RATE);
        }
    ...
    function burn(address user, uint256 amount) external {
        require(allowedAddresses[msg.sender] || msg.sender == address(FrigginEggs), "Address does not have permission to burn");
        _burn(user, amount);
    }
    ...
    function getPendingReward(address user) internal view returns(uint256) {
        //return FrigginEggs.tokensOfOwner(user).length * BASE_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START)) / 86400;
