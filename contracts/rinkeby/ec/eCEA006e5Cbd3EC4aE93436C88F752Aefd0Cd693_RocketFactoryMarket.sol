...
contract RocketFactoryMarket is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    */
    function makeAnOffer(
        ...
        require(
            IERC721(itemTypeToTokenAddress[_itemType]).ownerOf(_itemId) ==
                _offeree,
            "A requested item does not belong to the specified wallet"
        );
    ...
    function cancelOffer(address _offeree)
        public
        callerIsUser()
        nonReentrant()
    {
        ...
        if (offer.erc20 != address(0x0)) {
            IERC20(offer.erc20).transferFrom(
                address(this),
                msg.sender,
                offer.price
            );
    ...
    function rejectOffer(address _offerer)
        public
        callerIsUser()
        nonReentrant()
    {
