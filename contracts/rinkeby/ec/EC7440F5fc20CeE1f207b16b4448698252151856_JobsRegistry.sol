...
contract JobsRegistry is IJobsRegistry, Ownable {
    ...
    function addJob(address _job) external override {
        if (msg.sender != owner() && !jobsCreator[msg.sender])
            revert Forbidden();
        if (job[_job].exists) revert JobAlreadyAdded();
        job[_job].exists = true;
        emit AddJob(_job);
    }

    // FIXME: any credit previously applied to the job gets lost. Should we do something with it?
    ...
    function addLiquidityCredit(
        ...
        if (!_jobFromStorage.exists) revert NonExistentJob();
        uint256 _fee = (_amount * fee) / BASE;
        uint256 _creditFromLiquidity = ILiquidityWeightCalculator(
            _liquidityWeightCalculator
        ).calculate(_liquidity, _amount - _fee);
        _jobFromStorage.credit[nativeToken] += _creditFromLiquidity;
        IERC20(nativeToken).safeTransferFrom(
            owner(),
            address(this),
            _creditFromLiquidity
        );
        IERC20(_liquidity).safeTransferFrom(msg.sender, address(this), _amount);
        IERC20(_liquidity).safeTransfer(feeReceiver, _fee);
