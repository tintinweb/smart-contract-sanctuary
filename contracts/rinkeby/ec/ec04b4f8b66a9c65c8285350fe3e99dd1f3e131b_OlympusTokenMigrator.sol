...
interface IwsOHM is IERC20 {
  ...
  function sOHMValue(uint256 _amount) external view returns (uint256);
...
// File: migration/OlympusTokenMigrator.sol
...
contract OlympusTokenMigrator is OlympusAccessControlled {
    ...
    function migrate(
        ...
        uint256 wAmount = oldwsOHM.wOHMValue(_amount);
    ...
    function migrateAll(TYPE _to) external {
        ...
        uint256 wAmount = wsOHMBal.add(oldwsOHM.wOHMValue(ohmBal.add(sOHMBal)));
        if (ohmMigrated) {
            require(oldSupply >= oldOHM.totalSupply(), "OHMv1 minted");
            _send(wAmount, _to);
        } else {
    ...
    function bridgeBack(uint256 _amount, TYPE _to) external {
        ...
        uint256 amount = oldwsOHM.sOHMValue(_amount);
