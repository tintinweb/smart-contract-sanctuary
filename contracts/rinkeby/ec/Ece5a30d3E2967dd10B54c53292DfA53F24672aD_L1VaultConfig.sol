...
contract L1VaultConfig is VaultConfigBase, IL1VaultConfig {
    ...
    function _deployReceipt(address _underlyingToken)
        private
        returns (address)
    {
        require(
        ...
        whitelistedTokens[_underlyingToken]
            .underlyingReceiptAddress = newReceipt;
        emit TokenReceiptCreated(_underlyingToken);
        return newReceipt;
    }
...
abstract contract VaultConfigBase is IVaultConfigBase, OwnableUpgradeable {
    ...
    event TokenReceiptCreated(address _underlyingToken);
    ...
    /// @param _receiptTokenFactoryAddress Address of the already deployed Receipt Token Factory
    function setReceiptTokenFactoryAddress(address _receiptTokenFactoryAddress)
        external
        override
        onlyOwner
        validAddress(_receiptTokenFactoryAddress)
    {
        receiptTokenFactory = ITokenFactory(_receiptTokenFactoryAddress);
    }
