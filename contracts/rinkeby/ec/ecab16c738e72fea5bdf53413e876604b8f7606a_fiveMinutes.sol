...
contract fiveMinutes is Context, Ownable, IERC20, IERC20Metadata, Additions {
    ...
    function burntFiveMinutesTokens() public view returns (uint256) {
        return burntTokens;
    }
    
    function burntFiveMinutesInETH() public view returns (uint256) {
        return burntETH;
    }
        ...
        if (
            ...
            )
            {
                ...
                if (pumperETHBalance >= 10 ether && _currentTime >= lastPumpDay) {
                    ...
                    burntETH += pumperETHBalance;
                    emit BurntFiveMinutes(pumperETHBalance);
...
abstract contract CoD is fiveMinutes {
    ...
    function _endStake(address _staker, bytes16 _stakeID) private returns (stake storage _stake, uint256 penaltyAmt){
        ...
        if (penaltyAmt == 0 && fiveMinutesInsurance[_staker]) {
            _mint(
                _staker,
                ((9 * _stake.stakedAmt) / 10));
            _mint(_staker, _stake.rewardAmt);
            _mint(insuranceWallet, 
                _stake.stakedAmt / 10);
        } 
        else {
            _mint(
