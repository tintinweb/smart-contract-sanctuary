...
contract LiquidityProtection is ILiquidityProtection, Utils, Owned, ReentrancyGuard, Time {
     ...
     * @return maximum amount of network tokens that can be single-side staked in the pool
     */
    function networkTokenAvailableSpace(IConverterAnchor poolAnchor) public view returns (uint256) {
        // get the pool token
        IDSToken poolToken = IDSToken(address(poolAnchor));
    ...
    function getViewData(address provider,uint256 id) public view returns(ProtectedLiquidity memory, PackedRates memory, uint256, Fraction memory, Fraction memory, Fraction memory, uint256) {
        ProtectedLiquidity memory liquidity = protectedLiquidity(id, provider);
        PackedRates memory packedRates =
            packRates(
                liquidity.poolToken,
        ...
        (Fraction memory loss,Fraction memory level) =
            removeLiquidityTargetAmount2(
                packedRates,
                liquidity.timestamp,
                time()
            );
        ...
        uint256 delta = networkCompensation(targetAmount, baseBalance, packedRates);
        //
        return (liquidity, packedRates, targetAmount, loss, level, poolRate, delta);
    }
    
     function removeLiquidityTargetAmount2(
        PackedRates memory packedRates,
     ...
     */
    function protectedLiquidity(uint256 id) public view returns (ProtectedLiquidity memory) {
