...
contract GrumpyMutant is ERC721Enumerable, Ownable {

    using SafeMath for uint256;

    // Token detail
    struct GrumpyMutantDetail {
    ...
    // Token Detail
    mapping(uint256 => GrumpyMutantDetail) private _grumpyMutant;
  ...
  modifier onlyWhitelisted() {
    ...
    function mintTokenId(uint256 tokenId) public onlyOwner {
        ...
        uint256 first_encounter = block.timestamp;
        _safeMint(msg.sender, tokenId);
        _grumpyMutant[tokenId] = GrumpyMutantDetail(first_encounter);
        emit TokenMinted(tokenId, msg.sender, first_encounter);
    }
     ...
     */
     ...
     function mintFromWhiteList(uint256 numberOfTokens) public payable onlyWhitelisted {
        ...
        require(
            ...
            "Purchase would exceed max supply of Grumpy Mutant"
        );
        require(
     ...
     * Mint Grumpy Mutant
     */
    function mintGrumpyMutant(uint256 numberOfTokens) public payable {
     ...
     */
    function getGrumpyMutantDetail(uint256 tokenId)
        public
        view
        returns (GrumpyMutantDetail memory detail)
    {
        require(_exists(tokenId), "Token was not minted");

        return _grumpyMutant[tokenId];
    }
