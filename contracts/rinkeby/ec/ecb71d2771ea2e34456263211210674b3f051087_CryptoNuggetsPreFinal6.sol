...
contract CryptoNuggetsPreFinal6 is ERC721, Ownable, IMintedBeforeReveal {
    ...
    function mint(uint256 numberOfMonstro) public payable {
        ...
        require(numberOfMonstro > 0, "You cannot mint 0 items, please increase to more than 1");
        require(numberOfMonstro <= getMaxAmount(), "You are not allowed to buy this many items at once.");
        require(SafeMath.add(totalSupply(), numberOfMonstro) <= MAX_NUGGET_SUPPLY, "Exceeds maximum supply. Please try to mint less.");
        require(SafeMath.mul(getPrice(), numberOfMonstro) == msg.value, "Amount of Ether sent is not correct.");
