...
contract BlueChipKedu is ERC721Enumerable, Ownable {
    ...
    // Owner can mint up to 30 Custom Kedu for giveaways
    function giveAway(address _to, uint256 _numMints) public onlyOwner {
    ...
    function tokenURI(uint256 _tokenID) public view virtual override returns (string memory) {
        string memory baseURIRevealed = baseURIForID[_tokenID];
        require(_exists(_tokenID), "ERC721Metadata: URI query for nonexistent token");
        if(isRevealed[_tokenID]) {
            return bytes(baseURIRevealed).length > 0 ? string(abi.encodePacked(baseURIRevealed, _tokenID.toString(), ".json")) : "";
        } else {
