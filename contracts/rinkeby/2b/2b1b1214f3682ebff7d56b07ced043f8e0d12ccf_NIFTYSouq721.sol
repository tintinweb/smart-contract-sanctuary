...
contract NIFTYSouq721 is
    ...
    
 {
    ...
    mapping(string => address) private offTokentype;
    mapping(string => uint256) private offServiceValue;
    ...
    constructor(
        ...
        addTokenType(_type,tokenAddress);
    }
    ...
    function getOffTokenAddress(string memory _type) public view returns(address, uint256){
        return (offTokentype[_type], offServiceValue[_type]);
    }
    function addTokenType(string[] memory _type,address[] memory tokenAddress) public onlyOwner{
        require(_type.length == tokenAddress.length,"Not equal for TokenType and TokenAddress");
        for(uint i = 0; i < _type.length; i++) {
            tokentype[_type[i]] = tokenAddress[i];
        }
    }
    function addOffTokenType(string[] memory _type,address[] memory tokenAddress, uint256[] memory _offService) public onlyOwner{
        require(_type.length == tokenAddress.length && _offService.length == tokenAddress.length ,"Not equal for TokenType and TokenAddress");
        for(uint i = 0; i < _type.length; i++) {
            offTokentype[_type[i]] = tokenAddress[i];
            offServiceValue[_type[i]] = _offService[i];
        }
    }
    ...
    function newtokenasbid(string memory tokenAss,address from, address admin, uint256 amount, uint256 tokenId) internal {
        ...
        require(balances[tokenId][from] > 0, "Is Not a Owner");
        uint256 _adminfee; uint256 roy; uint256 netamount;
        if(offServiceValue[tokenAss] > 0){
            (_adminfee, roy, netamount) = calc(amount, _royal[tokenId], offServiceValue[tokenAss]);
        }
        else{
            (_adminfee, roy, netamount) = calc(amount, _royal[tokenId], serviceValue);
        }
