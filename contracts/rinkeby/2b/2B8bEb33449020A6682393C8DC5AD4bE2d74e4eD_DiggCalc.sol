...
contract DiggCalc is FractionalExponents {
	using SafeMath for uint256;

	uint32 constant rootN = 135;
	uint32 constant rootD = 100;
	...
	function earned(
		...
		uint256 blockTime = startEpoch; // block.timestamp;
		// uint256 poolBalance = balanceOf(account, pool);
		uint256 lastUpdate = endEpoch; // p.lastUpdateTime[account]

		// psuedomath: rewardRate = (1.35 * (balance * 1000)^(1/4)) / 86400
		(uint256 mantissa, uint8 exponent) = power(multiplyer.mul(poolBalance), uint256(1e18), rootN, rootD);
		...
		return rewardRate.mul(blockTime.sub(lastUpdate).add(poolBalance));
	}
}