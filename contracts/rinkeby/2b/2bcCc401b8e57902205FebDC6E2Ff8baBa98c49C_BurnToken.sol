...
contract BurnToken is ERC721 {
     ...
     */
    function devMintTokensToAddress(address _address, uint16 amount) external onlyOwner contractIsNotFrozen {
    ...
    function devMintTokensToAddresses(address[] memory _addresses) external onlyOwner contractIsNotFrozen {
        ...
        for (uint256 i; i < _addresses.length; i++) {
            _mint(_addresses[i], currentSupply.current());
            currentSupply.increment();
        }
    }
     ...
     * @dev Mint a token
     */
    function mint(uint8 amount, uint8 _maxMint, bytes calldata _signature) external payable callerIsUser contractIsNotFrozen {
        bytes32 messageHash = generateMessageHash(msg.sender, _maxMint);
     ...
     */
    function generateMessageHash(address _address, uint8 _maxMint) internal pure returns (bytes32) {
        return keccak256(
            abi.encodePacked(
