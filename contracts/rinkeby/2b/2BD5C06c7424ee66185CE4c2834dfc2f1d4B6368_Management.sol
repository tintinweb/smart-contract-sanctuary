...
contract Management is IManagement, EIP712, Ownable {
    address internal _kycSigner;

    // Contract Registry
    mapping(uint256 => address payable) internal _contractRegistry;

    // Permissions
    mapping(address => mapping(uint256 => bool)) internal _permissions;

    mapping(address => mapping(address => bool)) internal _poolOwners;

    constructor() EIP712("Management", "v1") {}

    function kycSigner() external view override returns (address) {
        return _kycSigner;
    }
    ...
    function isKYCPassed(
        ...
        if (_permissions[_recipient][WHITELISTED_KYC]) {
            return true;
        } else {
            require(_deadline > block.timestamp, "Time out");
            bytes32 structHash = keccak256(
                abi.encode(KYC_CONTAINER_TYPEHASE, _recipient, _deadline)
            );
            bytes32 hash = _hashTypedDataV4(structHash);
            address messageSigner = ECDSA.recover(hash, _v, _r, _s);

            return messageSigner == _kycSigner;
        }
    }
    ...
    function requireAccess(address _sender, address _pool)
        ...
        returns (bool)
    {
        if (_permissions[_sender][ROLE_ADMIN]) {
            return true;
        } else {
            return _poolOwners[_sender][_pool];
        }
    }
    ...
    function permissions(address _subject, uint256 _permission)
        ...
        returns (bool)
    {
        return _permissions[_subject][_permission];
    }
    ...
    function setPoolOwner(
        ...
        require(
            _permissions[msg.sender][CAN_SET_POOL_OWNER],
            ERROR_ACCESS_DENIED
        );
        _poolOwners[_pool][_owner] = _value;
        emit PoolOwnerSet(_pool, _owner, _value);
    }
    ...
    function setPermission(
        ...
        _permissions[_address][_permission] = _value;
    ...
    function setPermissions(
        ...
        for (uint256 i = 0; i < permissions_.length; i++) {
            _permissions[_address][permissions_[i]] = _value;
        }
        emit PermissionsSet(_address, permissions_, _value);
    }
    ...
    function registerContract(uint256 _key, address payable _target)
        ...
        onlyOwner
    {
        _contractRegistry[_key] = _target;
    ...
    {
        require(
            _permissions[msg.sender][CAN_SET_KYC_WHITELISTED],
            ERROR_ACCESS_DENIED
        );

        for (uint256 i = 0; i < _address.length; i++) {
            _permissions[_address[i]][WHITELISTED_KYC] = _value;
        }
    ...
    {
        require(
            _permissions[msg.sender][CAN_SET_PRIVATE_WHITELISTED],
            ERROR_ACCESS_DENIED
        );

        for (uint256 i = 0; i < _address.length; i++) {
            _permissions[_address[i]][WHITELISTED_PRIVATE] = _value;
        }
