...
contract SipherNFTSale is ISipherNFTSale, Whitelist {
  ...
  uint32 public constant CAP_PER_WHITELISTED_ADDRESS = 1;
  uint32 public constant CAP_PER_ADDRESS = 5;
  uint256 public constant SALE_PUBLIC_STARTING_PRICE = 10**18; //1 ether
  uint256 public constant SALE_BASE_PRICE = 10**17; // 0.1 ether
  uint256 public constant SALE_AUCTION_PRICE_LEVEL = 10**17; //0.01 minutes
  uint32 public constant SALE_AUCTION_TIME_INTERVAL = 120; //15 minutes
  ...
  function getPublicSaleCurrentPrice() public view returns (uint256) {
    SaleConfig memory config = _saleConfig;
    uint256 timestamp = _blockTimestamp();
    //require (((config.publicTime <= timestamp) && (timestamp < config.endTime)), 'Public Sale only');
    uint256 elapsed = getElapsedPublicSaleTime();
    uint256 currentPrice = SALE_PUBLIC_STARTING_PRICE - ((elapsed / SALE_AUCTION_TIME_INTERVAL )*SALE_AUCTION_PRICE_LEVEL);
    return
   ...
   */
  function buy(uint32 amount, bytes32[] memory proofs) external payable override {
    ...
    require(whitelistedMerkelRoot != bytes32(0), 'SipherNFTSale: Whitelisted merkle root is not set yet');

    _validateAndUpdateWithBuyAmount(buyer, amount, proofs);
  ...
  *       - can buy up to MAX_OWNER_BOUGHT_INITIAL before endTime with price = 0
   ...
   */
  function _validateAndUpdateWithBuyAmount(address buyer, uint32 amount, bytes32[] memory proofs) internal {
    SaleConfig memory config = _saleConfig;
    ...
    if (config.whitelistTime <= timestamp && timestamp < config.publicTime) {
      ...
      require(isWhitelistedAddress(buyer, proofs), 'SipherNFTSale: only whitelisted buyer');
...
contract Whitelist is IWhitelist, Ownable {
    ...
    function isWhitelistedAddress(address buyer, bytes32[] memory proofs)
        public
        view
        override
        returns (bool)
    {
        ...
        bytes32 computedHash = keccak256(abi.encode(buyer));
...
interface IWhitelist {
  ...
  function isWhitelistedAddress(address buyer, bytes32[] memory proofs) external view returns (bool);
...
interface ISipherNFTSale {
   ...
   *   The total sold tokens should be capped to maxSupply
   */
  function buy(uint32 amount, bytes32[] memory proofs) external payable;
