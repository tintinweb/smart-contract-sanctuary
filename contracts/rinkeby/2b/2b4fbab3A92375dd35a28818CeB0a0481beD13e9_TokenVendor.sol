...
contract TokenVendor is UUPSUpgradeable, OwnableUpgradeable, VRFConsumerBaseUpgradeable, ChainlinkClient {
    ...
    event OrderCompleted(address buyer, uint256 paidAmount, uint256 sellAmount);
    ...
    function fulfillETHPrice(bytes32 requestId, bytes32 data) public recordChainlinkFulfillment(requestId) {
        priceRequestMap[requestId].priceData = uint256(data);
        completeOrder(priceRequestMap[requestId]);
    }
}
