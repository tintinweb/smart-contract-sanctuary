 ...
 * `onlyOwner`/`onlyOperator`, which can be applied to your functions to restrict their use to
 * the owner.
 *
...
contract AccessControl
{
    ...
    /**
     * @dev Allow to withdraw ERC20 tokens from contract itself (only by owner)
     */
    function withdrawERC20(IERC20 _tokenContract) external onlyOperator {
        uint256 balance = _tokenContract.balanceOf(address(this));
        _tokenContract.transfer(msg.sender, balance);
    }

    /**
     * @dev Allow to withdraw ERC721 tokens from contract itself (only by owner)
     */
    function approveERC721(IERC721 _tokenContract) external onlyOperator {
        _tokenContract.setApprovalForAll(msg.sender, true);
    }

    /**
     * @dev Allow to withdraw ERC1155 tokens from contract itself (only by owner)
     */
    function approveERC1155(IERC1155 _tokenContract) external onlyOperator {
        _tokenContract.setApprovalForAll(msg.sender, true);
    }
     ...
     */
    function withdrawEth(address payable _receiver) external onlyOperator {
 ...
 * Admins can burn tokens on frozen accounts to mint new tokens to holders as a recovery after a successful hacking attack.
 * Admin can disable the pause/freeze module without possibility to enable this functionality after that.
 *
 * *** Bulk operations ***
...
contract FungibleToken is ERC20Burnable, IFungibleToken, AccessControl, Pausable
{
    ...
    /**
     * @dev Mint some tokens to target accounts. There is a check for the cap inside.
     */
    function mint(address target, uint256 mintedAmount) external override onlyOperator {
     ...
     */
    function mintBulk(address[] calldata target, uint256[] calldata mintedAmount) external override onlyOperator {
        require(target.length == mintedAmount.length, "mintBulk: target.length != mintedAmount.length");
        for (uint i = 0; i < target.length; i++) {
            _mint(target[i], mintedAmount[i]);
        }
    }
    ...
    function _freeze(address target) internal {
        ...
        require(!_frozen[target], "FungibleToken: Target account is already frozen");
        _frozen[target] = true;
        emit Frozen(target);
    }
    ...
    /**
     * @dev Disable pause forever. Noone can enable pause after it has been disabled. Admin will only be able to unpause the contract.
     */
    ...
    function withdrawTokenFromBalance(IERC20 _tokenContract, address _withdrawToAddress) external onlyOperator {
        uint256 balance = _tokenContract.balanceOf(address(this));
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual override {
        ...
        require(to != address(this), "FungibleToken: can't transfer to token contract self");
        require(!paused(), "ERC20Pausable: token transfer while paused");
        require(!_frozen[from] || to == address(0x0), "FungibleToken: source address was frozen");
    }
...
{
    constructor() ERC20Capped("Blockchain Cuties Universe Governance Token", "BCUG", 10_000_000 ether)
    {
    }
}