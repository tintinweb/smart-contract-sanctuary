...
contract SeraphStorage {
    ...
    */
    ...
    /// @notice Struct representing a pending permit indexed by the keccak256 of the contract/function
    /// selector and calldata.
    struct CallDataCache {
        /// @notice The permit that this cache is pointing to, if valid. If not valid this is ignored
        uint256 _permitId;
    }
    ...
    /// used is `nextCallPermitId`
    mapping(uint256 => CallPermit) public callPermits;
    uint256 nextCallPermitId;
    ...
    /// The key value is a hash, calculated using contractAddress, functionSelector and the call data
    /// It keeps track of pending permits based on the hash (address+function+calldata)
    mapping(bytes32 => CallDataCache) internal _hashToIdCache;
...
contract Seraph is Ownable, SeraphStorage {
    ...
    modifier onlyKMSPermit(uint256 permitId){
        ...
        // TODO: This should not happen, the _kmsWalletsToClientID value should always match _kmsWallet
     ...
     *
     * @dev The permitId must be less than the nextCallPermitId and bigger than 0.
    ...
    modifier permitExists(uint256 permitId){
        require(permitId > 0 && permitId < nextCallPermitId, "Seraph: Permit not found");
        _;
    }
    ...
    function _addPermit(address contractAddress, bytes4 functionSelector, bytes calldata callData, uint256 requiredPeers, address[] calldata peers) internal {
        ...
        // TODO: Think about hash collision
        ...
        require(_hashToIdCache[_permitHash]._permitId == 0, "Seraph: Only one pending permit with the same calldata per contract/function is allowed");

        CallPermit memory _permit;

        uint256 _newPermitId = nextCallPermitId;
    ...
    function _addCache(bytes32 permitHash, uint256 permitId) internal {
        _hashToIdCache[permitHash]._permitId = permitId;
    }
    ...
    function _invalidateCache(bytes32 permitHash) internal {
        _hashToIdCache[permitHash]._permitId = 0;
    }
    ...
    function initSeraph(address newGnosisHalbornWallet, bytes16 newkmsAdminKeyId, address newkmsAdminWallet) external initializer {
        ...
        // We start at 1, the default permitId for none existing permits will be 0
        nextCallPermitId += 1;
    ...
    function checkUnblocked(bytes4 functionSelector, bytes calldata callData) external contractExists(msg.sender) functionExists(msg.sender, functionSelector) {
        ...
        require(_hashToIdCache[_permitHash]._permitId != 0, "Seraph: No permit for this function with this call data has been signed");
        ...
        CallPermit storage _permit = callPermits[_hashToIdCache[_permitHash]._permitId];
        ...
        emit PermitExecuted(
            _hashToIdCache[_permitHash]._permitId,
            _permit._clientId,
            msg.sender,
            functionSelector,
            callData
    ...
    function getAllPermits() external view returns(CallPermit[] memory) {
        CallPermit[] memory _permits = new CallPermit[](nextCallPermitId - 1);
        for (uint256 i = 1; i < nextCallPermitId; i++) {
