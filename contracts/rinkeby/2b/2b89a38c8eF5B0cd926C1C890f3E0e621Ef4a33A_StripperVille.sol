...
contract StripperVille is Assets, ERC721 {
    uint256 public stripperPrice = 0.095 ether;
    ...
    modifier isMine(uint256 tokenId) {
        require(
            _msgSender() == ownerOf(tokenId),
            "OWNERSHIP: sender is not the owner"
        );
        _;
    }
    ...
    function setStripperPrice(uint256 newPrice) external onlyAdmin {
        stripperPrice = newPrice;
        emit NewStripperPrice(_msgSender(), newPrice);
    }

    function setMaxMint(uint256 newMaxMint) external onlyAdmin {
        _maxMint = newMaxMint;
        emit NewMaxMint(_msgSender(), newMaxMint);
    }

    function buyStripper(uint256 qty) external payable canMint(qty) {
    ...
    function giveaway(address to, uint256 qty) external onlyOwner canMint(qty) {
        for (uint256 i = 0; i < qty; i++) {
            _mintTo(to);
        }
    ...
    function _mintTo(address to) internal {
        ...
        uint256 earn = ((rand % 31) + 70) * (10**18);
        uint256 tokenId = strippersCount + 1;
        assets.push(
            Asset(tokenId, STRIPPER, earn, 0, block.timestamp, "", true)
        );
    ...
    function createClub(string calldata clubName) external onlyAdmin {
        uint256 tokenId = clubsCount + 1000000;
        assets.push(
            Asset(tokenId, CLUB, 0, 0, block.timestamp, clubName, true)
        );
    ...
    function closeClub(uint256 tokenId) external onlyAdmin {
        require(
            ownerOf(tokenId) == owner(),
            "Ownership: Cannot close this club"
        );
        (Asset memory asset, uint256 i) = getAssetByTokenId(tokenId);
        require(asset.tokenType == CLUB, "CLUB: asset is not a club");
        assets[i].active = false;
        emit CloseClub(_msgSender(), tokenId);
    }

    function reopenClub(uint256 tokenId) external onlyAdmin {
        (Asset memory asset, uint256 i) = getAssetByTokenId(tokenId);
        require(asset.tokenType == CLUB, "CLUB: asset is not a club");
    ...
    function setStripperName(uint256 tokenId, string calldata name)
        external
        isMine(tokenId)
    {
        (Asset memory asset, uint256 i) = getAssetByTokenId(tokenId);
    ...
    function withdrawAsset(uint256 tokenId, uint256 amount) external onlyAdmin {
        require(
            tx.origin == ownerOf(tokenId),
            "OWNERSHIP: sender is not the owner"
        );
        (, uint256 i) = getAssetByTokenId(tokenId);
