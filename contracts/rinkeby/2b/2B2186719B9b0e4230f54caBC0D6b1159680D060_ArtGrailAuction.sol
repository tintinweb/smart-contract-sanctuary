...
contract ArtGrailAuction is ReentrancyGuard, Ownable {
    ...
    constructor(address payable _feeRecipient) public {
        require(
            ...
            "ArtGrailAuction: Invalid Platform Fee Recipient"
        );
        ...
        whenNotPaused
    {
        require(
            ...
            "ArtGrailAuction.placeBid: No contracts permitted"
        );
        ...
        require(
            ...
            "ArtGrailAuction.placeBid: Bidding outside of the auction window"
        );
        ...
        require(
            ...
            "ArtGrailAuction.placeBid: Failed to outbid highest bidder"
        );
        ...
        whenNotPaused
    {
        ...
        require(
            highestBid.bidder == _msgSender(),
            "ArtGrailAuction.withdrawBid: You are not the highest bidder"
        );

        // Check withdrawal after delay time
        require(
            _getNow() >= highestBid.lastBidTime.add(bidWithdrawalLockTime),
            "ArtGrailAuction.withdrawBid: Cannot withdraw until lock time has passed"
        );

        require(
            _getNow() < auctions[_nftAddress][_tokenId].endTime,
            "ArtGrailAuction.withdrawBid: Past auction end"
        );
        ...
        nonReentrant
    {
        ...
        require(
            ...
            "ArtGrailAuction.resultAuction: Sender must be item owner"
        );

        // Check the auction real
        require(
            auction.endTime > 0,
            "ArtGrailAuction.resultAuction: Auction does not exist"
        );

        // Check the auction has ended
        require(
            _getNow() > auction.endTime,
            "ArtGrailAuction.resultAuction: The auction has not ended"
        );

        // Ensure auction not already resulted
        require(
            !auction.resulted,
            "ArtGrailAuction.resultAuction: auction already resulted"
        );

        // Ensure this contract is approved to move the token
        require(
            IERC721(_nftAddress).isApprovedForAll(_msgSender(), address(this)),
            "ArtGrailAuction.resultAuction: auction not approved"
        );
        ...
        require(
            ...
            "ArtGrailAuction.resultAuction: reserve not reached"
        );

        // Ensure there is a winner
        require(
            winner != address(0),
            "ArtGrailAuction.resultAuction: no open bids"
        );
        ...
        if (winningBid > auction.reservePrice) {
            ...
            require(
                ...
                "ArtGrailAuction.resultAuction: Failed to send platform fee"
            );

            uint256 platformRoyaltyAboveReserve = aboveReservePrice
                .mul(platformRoyalty)
                .div(1000);

            address creatorAddress = IArtGrailNFT(_nftAddress).creators(
                _tokenId
            );

            // Send royality fee to creator
            (bool royalityTransferSuccess, ) = creatorAddress.call{
                value: platformRoyaltyAboveReserve
            }("");

            require(
                royalityTransferSuccess,
                "ArtGrailAuction.resultAuction: Failed to send platform royalty"
            );
            ...
            require(
                ...
                "ArtGrailAuction.resultAuction: Failed to send the owner their royalties"
            );
        } else {
            (bool ownerTransferSuccess, ) = auction.owner.call{
                value: winningBid
        ...
        nonReentrant
    {
        ...
        require(
            IERC721(_nftAddress).ownerOf(_tokenId) == _msgSender() &&
                _msgSender() == auction.owner,
            "ArtGrailAuction.cancelAuction: Sender must be item owner"
        );

        // Check auction is real
        require(
            auction.endTime > 0,
            "ArtGrailAuction.cancelAuction: Auction does not exist"
        );

        // Check auction not already resulted
        require(
            !auction.resulted,
            "ArtGrailAuction.cancelAuction: auction already resulted"
        );
    ...
    function updateAuctionReservePrice(
        ...
        require(
            _msgSender() == auction.owner,
            "ArtGrailAuction.updateAuctionReservePrice: Sender must be item owner"
        );
        require(
            auction.endTime > 0,
            "ArtGrailAuction.updateAuctionReservePrice: No Auction exists"
        );
    ...
    function _createAuction(
        ...
        require(
            ...
            "ArtGrailAuction.createAuction: Cannot relist"
        );

        // Check end time not before start time and that end is in the future
        require(
            _endTimestamp > _startTimestamp,
            "ArtGrailAuction.createAuction: End time must be greater than start"
        );
        require(
            _endTimestamp > _getNow(),
            "ArtGrailAuction.createAuction: End time passed. Nobody can bid."
        );
    ...
    function _refundHighestBidder(
        ...
        require(
            successRefund,
            "ArtGrailAuction._refundHighestBidder: failed to refund previous bidder"
        );
        ...
        onlyOwner
    {
        require(
            ...
            "ArtGrailAuction.updatePlatformFeeRecipient: Zero address"
        );
