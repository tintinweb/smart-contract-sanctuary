...
contract TheOtherAvatars is ERC721Enumerable, TheOtherAvatarsRoyalties, CustomCounter, ReentrancyGuard, Signable {
    ...
    constructor() ERC721("The Other Avatars", "TOA") CustomCounter(artistTokensCount) TheOtherAvatarsRoyalties(withdrawAddress) {
        ...
        string memory baseContractURI = "https://www.saatchiart.com/nft/metadata.json"; // TODO: Fix link
    ...
    function artistMint(address[] memory artists, uint256[] memory tokens) public lock onlyOwner {
        for (uint i; i < artists.length; i++) {
            ...
            require(tokenId <= artistTokensCount, "tokenId does not belong to artists");
...
abstract contract TheOtherAvatarsRoyalties is Ownable, IRaribleV2 {
    uint256 internal constant numberOfArtists = 190; // TODO: FIXME
    uint256 internal constant artistBits = 8; // 2^^8 = 256 which is > 190 TODO: FIXME
    ...
    constructor(address defaultAccount) {
        _artistAtIndex[0] = defaultAccount;
        _defaultAccount = defaultAccount;
    }

    // TODO: FIX values; 1000 is 10%
    function getRaribleV2Royalties(uint256 id) override external view returns (Part[] memory) {
        ...
        if (index == 0) {
            ...
            result[0] = Part(defaultAccount, 300);
            result[1] = Part(account, 700);
            return result;
        }
    }
