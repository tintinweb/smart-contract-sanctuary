...
contract NFTCollectible is NFTUpgradeable, INFTCollectible {
    ...
    function initialize(string memory name_, string memory symbol_, address owner_, uint256 totalSupply_) public virtual override initializer {
    ...
    function supportsInterface(bytes4 interfaceId) public view override(NFTUpgradeable, IERC165Upgradeable) returns (bool) {
        return interfaceId == type(INFTCollectible).interfaceId || super.supportsInterface(interfaceId);
    }
        ...
        return string(
            abi.encodePacked(
                ...
                abi.encodePacked(
                    ...
                    imageURIEmpty ? tokenIdString.textShadow("").encode() : config.imageURI,
                    '"}'
                ).encode()
            )
        );
    ...
    function _mint(address to, uint256 tokenId, string memory uri) internal virtual override  {
        require(hasRole(MINTER_ROLE, msg.sender));
        require(tokenCount() < _totalSupply);
    ...
    function mintURI(address to, uint256 tokenId, string memory uri) public virtual override {
        _mint(to, tokenId, uri);
    }

    function mintTo(address to) public virtual override {
        _mint(to, tokenCount()+1, "");
    }

    function mintToURI(address to, string memory uri) public virtual override {
        _mint(to, tokenCount()+1, uri);
    }
}
...
interface INFTCollectible is INFTUpgradeable {
    function initialize(string memory name_, string memory symbol_, address owner_, uint256 totalSupply_) external;
    function totalSupply() external view returns (uint256);
    function mint(address to, uint256 tokenId) external;
    function mintURI(address to, uint256 tokenId, string memory uri) external;
    function mintTo(address to) external;
    function mintToURI(address to, string memory uri) external;
}
