 ...
 * owner.
 */
contract TokenVestingV1 is Ownable {
   ...
   */
  constructor(address _beneficiary, uint256 _start, uint256 _cliff, uint256 _duration, uint256 _margin,  bool _revocable) {
    require(_beneficiary != address(0));
    require(_cliff <= _duration);
    require(_start > block.timestamp, "TokenVesting: start time is before current time");
    margin = _margin;
  ...
  function releaseCalulation(uint256 _totalPrice)
        public
        view
        returns (uint256)
    {
  ...
  function releasableAmount(IERC20 token) public view returns (uint256) {
    uint256 tokenReleased = releaseCalulation((released[address(token)]));
    return vestedAmount(token).sub(tokenReleased);
  }
