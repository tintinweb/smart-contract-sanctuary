...
contract MetaLeopards is ERC721, ERC721Enumerable, Pausable, AccessControl {
    ...
    address public projectOwner = 0xAAbfd3351E0B7EB2FeB01C930A76Dec28B9938d7;
    ...
    function addWhiteListed(address _newWhiteListed) public {
        if (hasRole(WHITE_LIST_MANAGER, msg.sender) || hasRole(ROLE_ADMIN, msg.sender) || hasRole(PROJECT_OWNER, msg.sender) == true) {
    ...
    function setBaseURI(string memory _newBaseURI) public {
        if (hasRole(CONTRACT_MANAGER, msg.sender) || hasRole(ROLE_ADMIN, msg.sender) || hasRole(PROJECT_OWNER, msg.sender) == true) {  
            baseURI = _newBaseURI;
        }
    }

    function setNotRevealedURI(string memory _newNotRevealedUri) public {
        if (hasRole(CONTRACT_MANAGER, msg.sender) || hasRole(ROLE_ADMIN, msg.sender) || hasRole(PROJECT_OWNER, msg.sender) == true) {  
    ...
    function startWhiteListMint() public {
        if (hasRole(CONTRACT_MANAGER, msg.sender) || hasRole(ROLE_ADMIN, msg.sender) || hasRole(PROJECT_OWNER, msg.sender) == true) {
    ...
    // only role admin
    function addContractManager(address _newContractManager) public {
        if (hasRole(ROLE_ADMIN, msg.sender) || hasRole(PROJECT_OWNER, msg.sender) == true) {
            _grantRole(CONTRACT_MANAGER, _newContractManager);
        }
    }
  
    function removeContractManager(address _contractManager) public {
        if (hasRole(ROLE_ADMIN, msg.sender) || hasRole(PROJECT_OWNER, msg.sender) == true) {
    ...
    function addWhiteListManager(address _newWhiteListManager) public {
        if (hasRole(ROLE_ADMIN, msg.sender) || hasRole(PROJECT_OWNER, msg.sender) == true) {
            _grantRole(WHITE_LIST_MANAGER, _newWhiteListManager);
        }
    }
  
    function removeWhiteListManager(address _whiteListManager) public {
        if (hasRole(ROLE_ADMIN, msg.sender) || hasRole(PROJECT_OWNER, msg.sender) == true) {
