...
contract FirstDibsToken is ERC721, AccessControl, ERC721Burnable, ERC721Pausable, IERC721Creator {
    ...
    event UpdatedTokenURI(address indexed sender, uint256 indexed tokenId, string tokenURI);
    ...
    function _setTokenCreator(address payable _creator, uint256 _tokenId) private {
        require(_creator != address(0), 'FirstDibsToken: creator is zero address');
    ...
    /**
     * @dev Public function that mints a token. Sets msg.sender to creator and owner
     * @param _tokenURI uint256 metadata URI of the token.
     */
    function mint(string memory _tokenURI) public {
        mintWithCreator(_tokenURI, _msgSender());
    }
    ...
    function mintWithCreator(string memory _tokenURI, address payable _creator)
        ...
        returns (uint256)
    {
        require(hasRole(MINTER_ROLE, _msgSender()), 'FirstDibsToken: must have MINTER_ROLE');
    ...
    /**
     * @dev Update token URI for the given token
     * @param _tokenId uint256 token id where the metadata URI should be updated
     * @param _tokenURI string new metadata URI of the token
     */
    function updateTokenURI(uint256 _tokenId, string memory _tokenURI) external {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()) || _isApprovedOrOwner(_msgSender(), _tokenId),
            'FirstDibsToken: must be admin, approved or owner'
        );
        _setTokenURI(_tokenId, _tokenURI);
        emit UpdatedTokenURI(_msgSender(), _tokenId, _tokenURI);
    }
    ...
    function pauseToken() public {
        require(hasRole(PAUSER_ROLE, _msgSender()), 'FirstDibsToken: must have PAUSER_ROLE');
        _pause();
    }
     ...
     * Checks that the contrace isn't paused before doing a transfer
     */
