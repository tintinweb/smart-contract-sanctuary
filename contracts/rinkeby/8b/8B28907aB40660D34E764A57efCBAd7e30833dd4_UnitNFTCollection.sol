...
contract UnitNFTCollection is ERC721Enumerable, Ownable {
    string private _baseTokenURI;
    mapping(address => uint256) private airdropEligibleAddresses;
    ...
    ) ERC721("Unit NFT Collection", "Unit") {
        setBaseURI(baseURI);
        for (uint256 i = 0; i < _eligibleAddresses.length; i++) {
            address eligibleAddress = _eligibleAddresses[i];
            uint256 count = _counts[i];
            airdropEligibleAddresses[eligibleAddress] = count;
        }
    }
