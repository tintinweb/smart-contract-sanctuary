...
contract HoleyPeople is ERC721Enumerable, ReentrancyGuard, AccessControl {
    ...
    // Nov ** 18:00 UTC
    uint32 public nHolderSaleLaunchTime;
    // Nov ** 19:00 UTC (1 hour later)
    // uint32 public whitelistSaleLaunchTime = nHolderSaleLaunchTime + 3600 * 24;
    uint32 public whitelistSaleLaunchTime = nHolderSaleLaunchTime;
    // Nov ** 20:00 UTC (1 hour later)
    // uint32 public publicSaleLaunchTime = whitelistSaleLaunchTime + 3600 * 24;
    uint32 public publicSaleLaunchTime = whitelistSaleLaunchTime;

    /** MINTING **/

    uint256 public constant MAX_SUPPLY = 8888;
    uint256 public constant WHITELIST_RESERVE = 500;
    uint256 public constant MINT_LIMIT_PER_WALLET = 5;

    uint256 public constant nHolderPrice = 0.0349 ether;
    uint256 public constant whitelistPrice = 0.0449 ether;
    uint256 public constant publicPrice = 0.0699 ether;
    ...
    constructor(
        ...
        _setupRole(ADMIN_ROLE, 0x0000000000000000000000000000000000000000); // TODO: Add Daniel Address
        _owner = msg.sender;
        nHolderSaleLaunchTime = launchStartTime;
        customBaseURI = "arweave";
    ...
    /**
    Change the start time for the drop
     */
    function setDropStart(uint32 _dropStart) public onlyAdmin {
        nHolderSaleLaunchTime = _dropStart;
    }
    ...
    /**
    Updates the whitelist sale state for n holders
     */
    function setwhitelistSaleState(bool whiteListSaleState) public onlyAdmin {
        whitelistSaleActive = whiteListSaleState;
    }
    ...
    /**
    Return true if the high whitelist sale is active
     */
    function _isWhitelistSaleActive() public view returns (bool) {
        // return ((block.timestamp >= whitelistSaleLaunchTime || whitelistSaleActive) && !isSalePaused);
        return whitelistSaleActive && !isSalePaused;
    }

    /**
    Return true if the regular whitelist sale is active
     */
    function _isnHolderSaleActive() public view returns (bool) {
        // return ((block.timestamp >= nHolderSaleLaunchTime || nHolderSaleActive) && !isSalePaused);
    ...
    function _isPublicSaleActive() public view returns (bool) {
        // return ((block.timestamp >= publicSaleLaunchTime || publicSaleActive) && !isSalePaused);
    ...
    function totalMintsAvailable() public view returns (uint256) {
        ...
        } else if (_isWhitelistSaleActive()) {
            return WHITELIST_RESERVE - totalSupply();
    ...
    function mint(uint8 amount) public payable virtual nonReentrant {
        ...
        require(balanceOf(msg.sender) + amount <= MINT_LIMIT_PER_WALLET, "HoleyPeople:MINT_ABOVE_MAX_MINT_ALLOWANCE");
    ...
    {
        require(balanceOf(msg.sender) + amount <= MINT_LIMIT_PER_WALLET, "HoleyPeople:MINT_ABOVE_MAX_MINT_ALLOWANCE");
        require(_isWhitelistSaleActive(), "HoleyPeople:USER_NOT_ALLOWED_TO_MINT");
        require(amount <= totalMintsAvailable(), "HoleyPeople:MAX_ALLOCATION_REACHED");
        require(_verify(_leaf(msg.sender), proof), "HoleyPeople:Invalid merkle proof");
        require(msg.value == whitelistPrice * amount, "HoleyPeople:INVALID_PRICE");
    ...
    function setBaseURI(string memory customBaseURI_) external onlyAdmin {
    ...
    address private constant devAddress = 0xB6fE450D565F99066D44692Ede257cD179a0C846; // TODO: Add dev address
    address private constant artistAddress = 0xB25E3db936175d4890529E0eB5161E603370140c; // TODO: Add Daniel address
    ...
    function withdraw() public onlyAdmin {
        ...
        if (devfee >= 2 ether) {
            payable(artistAddress).transfer(balance);
        } else if(devfee + balance/2 > 2 ether) {
            payable(devAddress).transfer(2 ether - balance);
            payable(artistAddress).transfer(balance);
        } else {
            payable(devAddress).transfer(balance/2);
            payable(artistAddress).transfer(balance/2);
        }
    }
