...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Token address
    address public NATIVE = 0xD4E50Ea663592Dc310887F7AA46905D81d7C0822;
    ...
    // Approx 8/3/2021
    uint256 public startBlock = 9683430; // https://bscscan.com/block/countdown/5496000
    ...
    function isLpExists(address _strat) public view returns (bool) {
        for (uint256 pid = 0; pid < poolInfo.length; ++pid) {
            if (poolInfo[pid].strat == _strat) return true;
        }
    }
    ...
    function add(
        ...
        require(!isLpExists(_strat), "LP already exists.");
        if (_withUpdate) {
            massUpdatePools();
        }
        ...
        returns (uint256)
    {
        ...
        return
            user.shares.mul(accNATIVEPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function withdraw(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        uint256 pending = user.shares.mul(pool.accNATIVEPerShare).div(1e12).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeNATIVETransfer(msg.sender, pending);
