...
contract DartsDAO is ReentrancyGuard, AccessControl {
    ...
    event NewCharityProposal(address indexed proposer);
...
//        require(hasRole(STAKEHOLDER_ROLE, msg.sender), message);
    ...
    modifier onlyTokenHolder(uint tokenId) {
        require(IERC1155(_voteToken).balanceOf(msg.sender, tokenId) > 0, "Need to hold 1155 token");
        _;
    }
    ...
    onlyTokenHolder(_tokenId)
    {
        ...
        emit NewCharityProposal(msg.sender);
    }
    ...
    function votable(CharityProposal storage charityProposal) private {
        require(IERC1155(_voteToken).balanceOf(msg.sender, charityProposal.tokenId) > 0);
    ...
    function getHolder(uint _tokenId)
    public
    view
    returns (bool)
    {
        return IERC1155(_voteToken).balanceOf(msg.sender, _tokenId) > 0;
    }
}
