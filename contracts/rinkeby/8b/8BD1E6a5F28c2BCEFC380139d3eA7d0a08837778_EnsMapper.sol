...
/* This code should work for any ERC-721 contract. Please feel free to reuse :-)

- Change IERC721Enumerable contract address in constructor
- Update domainLabel value
- Set controller address of the parent domain to this deployed contract
...
contract EnsMapper is Ownable {
    ...
    event RegisterSubdomain(bytes32 indexed node, address indexed registrar, uint256 indexed token_id);
    ...
    function getTokenDomain(uint256 token_id) public view returns(string memory uri){
    ...
    function domainMap(string calldata label) public view returns(bytes32){
        ...
        return hashes[big_hash] > 0 ? big_hash : bytes32(0x0);
    }
    ...
    function renounceOwnership() public override onlyOwner {
        ...
        super.renounceOwnership(); //<< just put this in to stop VS complaining about state mutability
    }

    function setDomain(string calldata label, uint256 token_id) public {
        require(nft.ownerOf(token_id) == msg.sender || owner() == msg.sender, "Not authorised");
        ...
        require(owner() == msg.sender || block.timestamp > nextRegisterTimestamp[msg.sender], "Wallet must wait more time to register");
           
        bytes32 encoded_label = keccak256(abi.encodePacked(label));
        bytes32 big_hash = keccak256(abi.encodePacked(domainHash, encoded_label));

        //contract owner can update / overwrite records. 
        ...
        emit RegisterSubdomain(big_hash, msg.sender, token_id);     
    }
    ...
    function resetHash(uint256 token_id) public {
        require(owner() == msg.sender || nft.ownerOf(token_id) == msg.sender, "Not authorised");
        ...
        nextRegisterTimestamp[msg.sender] = 0;
    }
    ...
    function text(bytes32 node, string calldata key) external view returns (string memory) {
        require(hashes[node] > 0 && tokenHashmap[hashes[node]] != 0x0, "Invalid address");
