...
contract TraumaLlamas is ERC721, Ownable {
    ...
    function addToWhitelist(address[] memory _addr, uint256[] memory _amount)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < _addr.length; i++) {
            presaleReserved[_addr[i]] = _amount[i];
        }
    }
        ...
        mintCountMeetsPresaleSupply(numberOfTokens)
    {
        require(presaleIsActive, "Presale is not active");
        require(
            numberOfTokens > 0 && numberOfTokens <= MAX_PER_PRESALE_TRANSACTION,
