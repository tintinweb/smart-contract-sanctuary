...
contract HauntedHouses is ERC721, ERC721Enumerable, Ownable {
    ...
    // Public constants
    uint256 public constant BUYABLE_SUPPLY = 2000;
    uint256 public constant MAX_NFTS = 7923;

    address public constant CRYPTO_PUMPKINS = 0x9697B950F92C69AE262Aa1C57f9EE68336B33dAD;
    ...
    uint256 public houseReserve = 100;
    ...
    mapping(uint256 => mapping(uint256 => Window)) public windows;
    ...
    function mint(uint256 quantity) public payable {
        ...
        if (startingIndexBlock == 0 && (bought == BUYABLE_SUPPLY)) {
    ...
    //     ERC721Enumerable nfts = ERC721Enumerable(CRYPTO_PUMPKINS);
    ...
    //     ERC721Enumerable nfts = ERC721Enumerable(CRYPTO_PUMPKINS);
    //     uint256 numNFTs = nfts.balanceOf(msg.sender);
    //     require(_indexEnd < numNFTs, "Not Enough Pumpkins");
    ...
    function setWindow(IERC721 _external_nft, uint256 _content, uint256 _house, uint256 _window_index)
        external
    {
        require(msg.sender == ownerOf(_house), "Must Own house to Set Windows");
        require(_window_index < 3, "Ran out of windows");
        //IERC721 external_nft = IERC721(_nft);
        require(msg.sender == _external_nft.ownerOf(_content), "Must own content");
        windows[_house][_window_index] = Window(_external_nft, _content);

    }

    function getWindows(uint256 _tokenId) external view returns (Window[3] memory)
    {
        Window[3] memory results = [windows[_tokenId][0], windows[_tokenId][1], windows[_tokenId][2]];
        return results;
    }

    function reserveHouses(address _to, uint256 _reserveAmount) public onlyOwner {
        require(_reserveAmount > 0 && _reserveAmount <= houseReserve, "Not Enough reserves left for team");
        for (uint256 i = 0; i < _reserveAmount; i++) {
            _mintInternal(_to);
        }
        houseReserve = houseReserve - _reserveAmount;
    }
