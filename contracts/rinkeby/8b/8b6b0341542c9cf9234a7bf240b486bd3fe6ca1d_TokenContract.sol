...
contract TokenContract is ERC20, Ownable, Pausable {
    ...
    uint private immutable _holdDaysThreshold = 2; // just 2 seconds
    ...
    /*
        Distribute Ethereum
    */
    function distributeEthereum() public onlyOwner{
        ...
        uint rewardPerHolder = _balance.div(rewardersCount);
        require(rewardPerHolder > 0, "MyToken: Nothing to reward");
        for (uint i = 0; i < rewardersCount; i++) {
            payable(rewarders[i]).send(rewardPerHolder);
        }
    }
}