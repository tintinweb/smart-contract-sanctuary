...
contract SomeTestNFT is ERC721, Ownable {
    ...
    uint constant public PRESALE_PER_TX_LIMIT = 10; // Up to 5 AniCats per transaction during the presale.
    uint constant public PRESALE_PER_WALLET_LIMIT = 10; // Up to 5 AniCats per unique wallet during the presale.
    uint constant public MINT_PER_TX_LIMIT = 20; // Up to 20 AniCats per transaction.

    string private _apiURI = "https://test-api-for-cats.herokuapp.com/token/";
    uint public reservedTokensLimit = 200; // 200 AniCats will be reserved for marketing needs.

    uint public claimLimit = 8; // Take from all Mintpasses sold count
    uint public presaleTokensLimit = 1000;
    ...
    function presaleAllowedForWallet() public view returns(bool) {
        return presaleList[msg.sender] ||
               friendContract.balanceOf(msg.sender) > 0 ||
               mintPassContract.balanceOf(msg.sender, TICKET_ID) > 0;
    }
