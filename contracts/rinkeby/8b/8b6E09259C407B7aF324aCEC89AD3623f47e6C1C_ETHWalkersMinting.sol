...
contract ETHWalkersMinting is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    address payable public payoutsAddress = payable(address(0x951807EbBe1842375C892F1d89D2C6a6038A1abb)); // TODO: Update with address from ETHWalkersPayments.sol
    ...
    function preSaleApesZenMintETHWalkers(uint numberOfTokens, address _to) external payable {
        ...
        if(block.timestamp >= publicSale) {
            ...
            require((boredApes.balanceOf(_to) > 0 || mutantApes.balanceOf(_to) > 0 || zenAcademy.balanceOf(_to, 0) > 0), "Must own BAYC, MAYC, or ZA token to mint during Ape presale");
        } else {
