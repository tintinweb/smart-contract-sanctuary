...
contract TestToken is StandardToken, Ownable {
    ...
    function batchAssignTokens(address[] _addr, uint256[] _amounts, DefrostClass[] _defrostClass) public onlyOwner canAssign {
        ...
        for (uint256 index = 0; index < _addr.length; index++) {
            ...
            if (defrostClass == DefrostClass.PublicSale || defrostClass == DefrostClass.Airdrop) {
                ...
                require (totalPublicSaleSupply <= 6000000 && totalAirdropSupply <= 3000000);
      ...
      if( InitUnTrigger && InitTknUnlockStatus()) {
        ...
        for (uint256 index = 0; index < icedBalancesStrategicSale.length; index++) {
          ...
          if (amountToDefrost > 0) {
            ...
            Defrosted(currentAddress, amountToDefrost, uint256(DefrostClass.StrategicSale));
          }
        }
    ...
    function InitTknUnlockStatus() public view returns (bool) {
        if(count <= 1)
        {
            if(now >= time_of_investment + (30 * 60)) {
                monthIndex = 4;
                count = 2;
                return true;
            }
    ...
    function CycleTknUnlockStatus() public view returns (bool) {
        if((monthIndex > 3 && monthIndex <= 13)){
            if(now >= time_of_investment + (monthIndex * 60)) { 
                monthIndex++; 
                return true;
            }
            else{
  ...
  */
  function balanceOf(address _owner) public view returns (uint256 balance) {
    return balances[_owner]+mapicedBalancesStrategicSale[_owner]+mapicedBalancesPvtSale2[_owner]+mapicedBalancesPvtSale1[_owner];
  }
