...
contract UniStrategy is IUniStrategy {
    ...
    /// @dev swapPercentage is the percentage of swap amount during readjust liquidity.
    uint8 public override swapPercentage;
     ...
     **/
    function setPricethreshold(uint24 _priceThreshold) external onlyGovernance {
        require(
            _priceThreshold < 1e6 && _priceThreshold > 0,
            "UniStrategy:: Invalid Price Impact"
        );
        emit PriceThresholdUpdated(pricethreshold, _priceThreshold);
    ...
    function setRangeMultiplier(int24 _rangeMultiplier) external onlyGovernance {
        ...
        emit RangeMultiplierUpdated(rangeMultiplier, _rangeMultiplier);
        rangeMultiplier = _rangeMultiplier;
    }

    function setSwapPercentage(uint8 _swapPercentage) external onlyGovernance {
        require(_swapPercentage > 0, "UniStrategy:: PERCENTAGE_TOO_SMALL");
        require(_swapPercentage <= 100, "UniStrategy:: PERCENTAGE_TOO_LARGE");
        emit SwapPercentageUpdated(swapPercentage, _swapPercentage);
    ...
    function setBaseMutiplier(int24 _baseMultiplier) external onlyGovernance {
        ...
        emit BaseMultiplierUpdated(baseMultiplier, _baseMultiplier);
    ...
    function setMaxTwapDeviation(int24 _twapDeviation) external onlyGovernance {
        emit MaxTwapDeviationUpdated(maxTwapDeviation, _twapDeviation);
    ...
    function setTwapDuration(uint32 _twapDuration) external onlyGovernance {
        emit TwapDurationUpdated(twapDuration, _twapDuration);
        ...
        onlyGovernance
    {
        ...
        poolStrategy[_pool].twapDuration = params.twapDuration;

        emit StrategyUpdated(oldStrategy, poolStrategy[_pool]);
    }
