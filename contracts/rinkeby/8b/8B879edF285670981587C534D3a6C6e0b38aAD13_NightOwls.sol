...
contract NightOwls is ERC721Enumerable, CollaborativeOwnable {
  using SafeMath for uint256;
  using Address for address;
  using Strings for uint256;

  event MintNightOwl(address indexed sender, uint256 startWith, uint256 quantity);

  uint160 public constant maxPrepaidQuantity = 4;
  uint256 public maxSupply = 3369;
  uint256 public reservedCount = 69;
  ...
  mapping(address => uint256) private _prepaidList;
  uint256 public prepaidCount = 0;
  
  address public collaboratorWithdrawAddress = address(0);
  uint256 public developerPercentage = 20;

  constructor() ERC721("Night Owls", "OWL") {}
  ...
  function remainingForMint() public view returns (uint256) {
    ...
    uint256 available = maxSupply.sub(ts.add(reservedCount).add(prepaidCount));
    return available;
  }
  ...
  function mint(uint256 quantity) public payable {
    ...
    require(mintPrice.mul(quantity) <= msg.value);

    uint256 ts = totalSupply();
    emit MintNightOwl(_msgSender(), ts, quantity);
  ...
  function prepaidMint() public {
    require(prepaidMintIsActive);

    uint256 quantity = _prepaidList[_msgSender()];
    require(quantity > 0);

    uint256 ts = totalSupply();
    require(ts.add(quantity) <= maxSupply.sub(reservedCount));
    ...
    _prepaidList[_msgSender()] = 0;
    prepaidCount = prepaidCount.sub(quantity);
  }

  function prepaidBalanceOf(address addr) public view returns (uint256) {
    return _prepaidList[addr];
  }
  ...
  function airDrop(address to, uint quantity) external onlyCollaborator {
    require(to != address(0));
    require(quantity > 0);
    require(quantity <= remainingForMint());

    uint256 ts = totalSupply();
    emit MintNightOwl(to, ts, quantity);
  ...
  function reservedMint(address to, uint256 quantity) external onlyCollaborator {
    require(to != address(0));
    require(quantity > 0 && quantity <= reservedCount);
    ...
    reservedCount = reservedCount.sub(quantity);
  }
  ...
  function addToPrepaidList(address[] calldata data) external onlyCollaborator {
    for (uint256 i = 0; i < data.length; i++) {
      uint256 existing = _prepaidList[data[i]];
      if (existing < maxPrepaidQuantity) {
        _prepaidList[data[i]] = existing + 1;
        prepaidCount = prepaidCount.add(1);
      }
    }
  }

  function removeFromPrepaidList(address[] calldata data) external onlyCollaborator {
    for (uint256 i = 0; i < data.length; i++) {
      uint256 existing = _prepaidList[data[i]];
      prepaidCount = prepaidCount.sub(existing);
      _prepaidList[data[i]] = 0;
    }
  }

  function setCollaboratorWithdrawAddress(address collaboratorAddress) external onlyCollaborator {
    collaboratorWithdrawAddress = collaboratorAddress;
  }

  function withdraw() external onlyCollaborator {
    require(collaboratorWithdrawAddress != address(0));

    uint256 balance = address(this).balance;

    uint256 developerBalance = balance.mul(developerPercentage).div(100);
    uint256 collaboratorBalance = balance.sub(developerBalance);

    payable(collaboratorWithdrawAddress).transfer(collaboratorBalance);
    payable(owner()).transfer(developerBalance);
  }
}
