...
contract MonstersNFT is ERC721Enumerable, Ownable {
    
    using SafeMath for uint256;
    
    bool public mintingActive;
    uint256 public mintingPrice = .02 ether;
    ...
    constructor(uint256 mintingStart, string memory _uri, address poeToken, uint256 buffer) ERC721("Monster Squad", "MNSTR") {
        mintingActive = false;
    ...
    function mint(uint256 _amount) public payable {
        ...
        require(msg.value >= mintingPrice.mul(_amount) ||
            (IERC20(poe).balanceOf(msg.sender) > 0 &&
            claimedFree[msg.sender] == false), "Not enough ether sent"); //10% discount for POE holders
        ...
        if (claimedFree[msg.sender] == false && block.timestamp < mintingStartTimestamp && totalSupply() < 5/*00*/) { //free claim for presale particiapnts
            claimedFree[msg.sender] = true;
            _safeMint(msg.sender, tsupply + 1);
            return;
        }
        else if(block.timestamp < mintingStartTimestamp && numPurchased[msg.sender] == 0 && totalSupply() > 5/*00*/){
            numPurchased[msg.sender] += _amount;
            _safeMint(msg.sender, tsupply + 1);
            return;
        }
        else if(block.timestamp > mintingStartTimestamp){
