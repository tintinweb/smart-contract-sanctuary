 ...
 * @title HDVFactory
 */
contract HDVFactory is FactoryERC1155, Ownable, ReentrancyGuard {
    ...
    string private nameX;
    // Contract symbol
    string private symbolX;
    ...
    mapping(uint256 => uint256) public optionsToIDsMap;
    ...
    constructor(
        ...
        NUM_OPTIONS = _idsMap.length;

        for (uint256 i = 0; i < _idsMap.length; i++) {
            optionsToIDsMap[i] = _idsMap[i];
        }
    }

    function _optionToID(uint256 _optionId) public view returns (uint256) {
        return optionsToIDsMap[_optionId];
    }
    ...
    {
        return uriX; // TODO Upravit uri premapovat etc?
    }
    ...
    function _mint(
        ...
        if (_option < NUM_OPTIONS) {
            ...
            ERC1155Tradable tradeable = ERC1155Tradable(nftAddress);
            tradeable.mint(_toAddress, _optionToID(_option), _data);
        } else {
    ...
    //     tradable.mint(_to, _id, 1, _data);
    //     // V mojom kode nie je ziadne create .. takze ostava len mint
        ...
        returns (uint256)
    {
        ...
        uint256 id = _optionToID(_optionId);

        ERC1155Tradable tradeable = ERC1155Tradable(nftAddress);

        if (tradeable.tokenSupply(id) > 0) {
            return 0;
        } else {
            return 1;
        }
