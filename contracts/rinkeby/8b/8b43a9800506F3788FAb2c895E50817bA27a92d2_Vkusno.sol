...
//  _   _  _                                _____         _                      
// | | | || |                              |_   _|       | |                     
// | | | || | __ _   _  ___  _ __    ___     | |    ___  | | __  ___  _ __   ___ 
// | | | || |/ /| | | |/ __|| '_ \  / _ \    | |   / _ \ | |/ / / _ \| '_ \ / __|
// \ \_/ /|   < | |_| |\__ \| | | || (_) |   | |  | (_) ||   < |  __/| | | |\__ \
//  \___/ |_|\_\ \__,_||___/|_| |_| \___/    \_/   \___/ |_|\_\ \___||_| |_||___/
...
contract Vkusno is ERC721, Ownable, ReentrancyGuard {
    using ECDSA for bytes32;
	using Strings for uint256;

    uint256 public totalSupply = 0;

	string private _baseTokenURI = "ipfs://xqyx/";
	string private _contractURI = "ipfs:/yyeyh";
	...
	constructor(address _feeAddress, uint256 _feePercentage, address _signerAddress) ERC721("Vkusno Tokens", "VK") {
    ...
    function presaleMintTokenfy(bytes32 hash, bytes memory sig, uint256 qty, uint256 value) external nonReentrant presaleValid(hash, sig, qty) {
        require(tokenfyEnabled, "tokenfy not enabled");
    ...
    function withdrawTokenfy() public onlyOwner {
        IERC20 tokenfyToken = IERC20(tokenfyAddress);
        uint256 balance = tokenfyToken.balanceOf(address(this));
        tokenfyToken.approve(address(this), balance);
		
        uint256 fee = balance * feePercentage / 100;
        tokenfyToken.transferFrom(address(this), feeAddress, fee);
        tokenfyToken.transferFrom(address(this), msg.sender, balance - fee);
    }

    // reclaiming accidentally sent tokens
	function reclaimERC20(IERC20 erc20Token, address to) public onlyOwner {
        require(address(erc20Token) != tokenfyAddress, "can't reclaim tokenfy");
    ...
    function changePresaleTokenfyPrice(uint256 newPrice) external onlyOwner {
