...
interface ILock {
    ...
    struct LockPool {
        ...
        address store;
        uint256 pid;
        uint256 totalAmount; //todo:
        uint256 totalMintPoint; //todo:
    }
    struct UserLock {
        uint256 amount;
        uint256 end;
    ...
    function calculatePoints(uint256 _amount, uint256 _time)
        external
        view
        returns (uint256);
...
contract BasisLockPoint is ERC20Burnable, ILock, Operator {
    ...
    modifier checkFromLockPool {
        LockPool memory pool = lockPools[_msgSender()];
        require(pool.enable, 'BLP: not from lock pool');
        _;
    }
    ...
    function calculatePoints(uint256 _amount, uint256 _time)
        ...
        returns (uint256)
    {
        return _amount.mul(_time).div(recipK);
    }
    ...
    function voteTo(address _rewardPool, uint256 _amount) external override {
    ...
    function setLockPool(
        ...
        emit SetLockPool(_pool, _enable, _siglePool, _store, _pid);
    }
    ...
    function setK(uint256 _recipK) external onlyOwner {
        recipK = _recipK;
    }
