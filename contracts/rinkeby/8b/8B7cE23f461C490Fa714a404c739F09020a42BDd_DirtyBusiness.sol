 ...
 * @author Written by michaelshimeles; Developed by trilar
 */
contract DirtyBusiness is NPassCore {
    ...
    function getSex(uint256 tokenId) public view returns (string memory) {
        return (n.getSecond(tokenId) <= 6) ? "Male" : "Female";
    }

    // N3
    function getHeight(uint256 tokenId) public view returns (string memory) {
        return height[n.getThird(tokenId)];
    }

    // N4
    function getBodyBuild(uint256 tokenId) public view returns (string memory) {
        if (n.getFourth(tokenId) == 0) {
            return "Athletic Body Build";
        } else if (n.getFourth(tokenId) <= 5) {
            return "Average Body Build";
        } else if (n.getFourth(tokenId) <= 10) {
            return "Bodybuilder";
        } else {
            return "Runner Body Build";
        }
    }

    // N5
    function getMentalStrength(uint256 tokenId) public view returns (string memory) {
        if (n.getFifth(tokenId) == 0 || n.getFifth(tokenId) >= 11) {
            return "High Mental Strength";
        } else if (n.getFifth(tokenId) >= 5) {
            return "Medium Mental Strength";
        } else {
            return "Low Mental Strength";
        }
    }

    // N6
    function getAimAccuracy(uint256 tokenId) public view returns (string memory) {
        if (n.getSixth(tokenId) == 0 || n.getSixth(tokenId) == 14) {
            return "Aim Accuracy: Sharpshooter";
        } else if (n.getSixth(tokenId) <= 5) {
            return "Aim Accuracy: 60%";
        } else if (n.getSixth(tokenId) <= 10) {
            return "Aim Accuracy: 70%";
        } else {
            return "Aim Accuracy: 80%";
        }
    }

    // N7
    function getSpecialty(uint256 tokenId) public view returns (string memory) {
        if (n.getSeventh(tokenId) == 0) {
            return "War Strategy";
        } else if (n.getSeventh(tokenId) <= 3) {
            return "Leader";
        } else if (n.getSeventh(tokenId) <= 6) {
            return "Shooter";
        } else if (n.getSeventh(tokenId) <= 9) {
            return "Martial Arts";
        } else if (n.getSeventh(tokenId) <= 12) {
            return "Hacker";
        } else {
    ...
    function getItem(uint256 tokenId) public view returns (string memory) {
        string memory prefix = (getNSum(tokenId) <= 20 || getNSum(tokenId) >= 70) ? "Gold " : "";

        if (n.getEight(tokenId) <= 1) {
            return string(abi.encodePacked(prefix, "Belt"));
        } else if (n.getEight(tokenId) <= 3) {
            return string(abi.encodePacked(prefix, "Chain"));
        } else if (n.getEight(tokenId) <= 5) {
            return string(abi.encodePacked(prefix, "Grill"));
        } else if (n.getEight(tokenId) <= 7) {
            return string(abi.encodePacked(prefix, "Gun"));
        } else if (n.getEight(tokenId) <= 9) {
            return string(abi.encodePacked(prefix, "Cigar"));
        } else if (n.getEight(tokenId) <= 11) {
            return string(abi.encodePacked(prefix, "Brass Knuckles"));
        } else if (n.getEight(tokenId) <= 13) {
            return string(abi.encodePacked(prefix, "Fedora"));
        } else {
            return string(abi.encodePacked(prefix, "Watch"));
        }
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        parts[
            0
        ] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="black" /><text x="10" y="20" class="base"><u>';

        parts[1] = getName(tokenId); // N1

        parts[2] = '</u></text><text x="10" y="40" class="base">';

        parts[3] = getSex(tokenId); // N2

        parts[4] = '</text><text x="10" y="60" class="base">';

        parts[5] = getHeight(tokenId); // N3

        parts[6] = '</text><text x="10" y="80" class="base">';

        parts[7] = getBodyBuild(tokenId); // N4

        parts[8] = '</text><text x="10" y="100" class="base">';

        parts[9] = getMentalStrength(tokenId); // N5
