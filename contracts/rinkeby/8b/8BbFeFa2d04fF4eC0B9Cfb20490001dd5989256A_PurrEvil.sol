...
contract PurrEvil is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function mint(address _to, uint256 _mintAmount) public payable {
    ...
    require(presaleIsActive,                                "Sale paused");
    require(_mintAmount <= maxMintAmount,                   "You can summon a maximum of 20 Cats");
    require(supply + _mintAmount <= maxSupply,              "Exceeds maximum Cats supply");

    if (msg.sender != owner()) {
        if(whitelisted[msg.sender] != true) {
          require(saleIsActive,                             "Sale paused");    
          require(msg.value >= cost * _mintAmount,          "Ether sent is not correct");
        }
    }
    ...
    // reserve tokens for giveaways and rewards
    function reserveCats() public onlyOwner {
