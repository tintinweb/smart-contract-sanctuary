...
contract YELCrowdsale is Ownable {
    address public token;
    address public oracle;
    uint256 public threshold;

    uint256 public initialPrice;
    uint256 public tokenPriceUSD; // price format is 8 decimals precision: $1 = 100000000, $0.01 = 1000000
    ...
    function claimTokens() public claimStart hasTokensToClaim {
        uint256 usersWeis = contributionInWei[msg.sender];
        contributionInWei[msg.sender] = 0;
        uint256 _priceInWeiPerToken = getPriceInWeiPerToken(tokenPriceUSD);
        uint256 usersTokens = usersWeis / _priceInWeiPerToken;
    ...
    function buyTokens() public payable isPresale {
        uint256 _priceInWeiPerToken = getPriceInWeiPerToken(tokenPriceUSD);
        uint256 tokens = msg.value / _priceInWeiPerToken;

        require(tokens > 0, "Insufficient funds");

        uint256 tradeAmountInUSD = (tokens * tokenPriceUSD) / 1e18;
    ...
    function updateTokenPrice(uint256 _priceInWeiPerToken) internal {
        uint256 purchesedTokens = weiRaised / _priceInWeiPerToken;
        if (purchesedTokens > threshold && tokenPriceUSD / initialPrice < 16) {
    ...
    function getPriceInWeiPerToken(uint256 _tokenPriceUSD) public view returns(uint256) {
        ...
        return divide(_tokenPriceUSD, oraclePriceETHUSD, 18);
    }
