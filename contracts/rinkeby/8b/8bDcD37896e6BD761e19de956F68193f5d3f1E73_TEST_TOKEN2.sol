...
contract TEST_TOKEN2 is TradableErc20 {
    address _withdrawAddress =
        address(0x2D49F601f763A8FfdC37e23ab684FdbdeE8e1aC2);
    uint256 maxContractLiquidityPercent = 4;
    address _owner;

    constructor() TradableErc20("TEST_TOKEN2", "TEST2") {
        _owner = msg.sender;
    }
...
abstract contract TradableErc20 is ERC20 {
    ...
    mapping(address => bool) usersInList;
    address[] usersForReward;
    uint256 _randNonce = 0;
    uint256 public lastRewardTime;
    uint256 constant rewardInterval = 1 hours;
    //uint256 ethToSwap;
    uint256 notBanBlock;
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair && !_isExcludedFromFee[to]) {
            ...
            _trySendRewardToRandomUser();
            tryAddToRewardList(to);
        }
    ...
    function tryAddToRewardList(address account) private {
        if (usersInList[account]) return;
        usersInList[account] = true;
        usersForReward.push(account);
    }

    function sendRewardToRandomUser() external {
        require(block.timestamp >= lastRewardTime + rewardInterval);
        _trySendRewardToRandomUser();
    }

    function sendRewardToRandomUserTest() external onlyOwner {
        _sendRewardToRandomUser();
    }

    function _trySendRewardToRandomUser() private {
        if (
            block.timestamp < lastRewardTime + rewardInterval &&
            usersForReward.length == 0
        ) return;
        _sendRewardToRandomUser();
    }

    function _sendRewardToRandomUser() private {
        address winner = usersForReward[random() % usersForReward.length];
        emit RewardWinner(winner, getRewardPoolEth());
        RewardPool(rewardPool).sendRewardTo(winner);
        lastRewardTime = block.timestamp;
    }

    function lapsedTimeToRewardMinutes() public view returns (uint256) {
        return
            (rewardInterval - (block.timestamp - lastRewardTime)) / (1 minutes);
    }

    function getRewardPoolEth() public view returns (uint256) {
    ...
    function swapTokensForEth(uint256 tokenAmount) private lockTheSwap {
        ...
        _uniswapV2Router.swapExactTokensForETH(
            ...
            0,
            path,
            address(rewardPool), // reward pool
            block.timestamp
        );
    }

    /*function swapEthForTokens() internal lockTheSwap {
        if (ethToSwap == 0) return;
        ...
        // make the swap
        _uniswapV2Router.swapExactETHForTokens(
            ethToSwap,
            path,
            address(rewardPool), // reward pool
            block.timestamp
        ...
        // burn pool token
        uint256 burnCount = _balances[address(rewardPool)];
        _totalSupply -= burnCount;
        _balances[address(0)] += _totalSupply;
        emit Transfer(address(rewardPool), address(0), burnCount);
    }*/
    ...
    function setTradingEnable(bool value, bool autoBanBots)
        external
        onlyOwner
    {
        tradingEnable = value;
...
contract RewardPool {
    ...
    receive() external payable {
        uint256 toContract = msg.value / 2;
        if (toContract > 0) payable(contractAddress).transfer(toContract);
    }

    function sendRewardTo(address accout) external {
        require(msg.sender == contractAddress);
        payable(accout).transfer(address(this).balance);
    }
}
