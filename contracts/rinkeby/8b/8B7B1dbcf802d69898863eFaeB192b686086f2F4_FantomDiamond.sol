...
contract FantomDiamond is ERC721PresetMinterPauserAutoId, IERC2981, Ownable {
    using Counters for Counters.Counter;

    uint256 public constant MAX_DIAMOND = 51;
    ...
    constructor(string memory _baseTokenURI, address _paymentManager)
        ERC721PresetMinterPauserAutoId('Fantom Diamond', 'DIA', _baseTokenURI)
    {
        // set the royalty manager
        paymentManager = _paymentManager;
    }

    /// @notice Mint some diamonds
    function mintDiamond(uint8 _quantity) public payable {
        require(mintable, 'Diamonds are not mintable yet.');
        require(_quantity <= MINT_PER_TX, 'Too many diamonds for this transaction.');
        require(msg.value == MINT_PRICE * _quantity, 'Not the right payment for all these diamonds.');
    ...
    function veryImportantDiamonds(address _to) external onlyOwner {
        require(!mintable, 'Very important diamonds are not mintable anymore.');
        _tokenIds.increment();
    ...
    function _mint(address to, uint256 _id) internal override {
        require(_id < MAX_DIAMOND, 'All the moon diamonds are minted.');
        super._mint(to, _id);
    }
