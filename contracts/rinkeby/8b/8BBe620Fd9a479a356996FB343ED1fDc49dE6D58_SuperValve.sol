...
contract SuperValve is SuperAppBase, AccessControl {
    ...
    function withdraw(address[] memory _pipeAddresses) public {
        ...
        // TODO: this should never be less than 0, but it may be?
        updateValveBalance(newValveBalance);
        ...
        returns (uint256 pipeWithdrawalAmount)
    {
        ...
        if (pipe.totalWithdrawableBalance(_user, previousFlowRate) > 0) {
            ...
            pipe.setUserFlowWithdrawData(_user, previousFlowRate);
    ...
    function updateValveBalance(int256 _newValveBalance) internal {
        totalValveBalance = _newValveBalance;
        valveFlowRateLastUpdated = block.timestamp;

        emit ValveBalanceUpdated(_newValveBalance, block.timestamp);
    }
    ...
    function _modifyMultiFlow(
        ...
        updateValveBalance(updatedValveBalance);
        emit ValveBalanceModify(previousValveInflowRate, totalValveBalance, updatedValveBalance, block.timestamp);
