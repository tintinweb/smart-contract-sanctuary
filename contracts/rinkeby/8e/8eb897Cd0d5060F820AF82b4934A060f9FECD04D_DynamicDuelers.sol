...
contract DynamicDuelers is ERC721, ERC721Enumerable, Ownable {

    // Provenance
    string public duelerHash = "";

    uint16 constant retailSupply = 6000;
    uint256 public devReserve = 2500;
    uint256 public stakerSupply = 0;
    ...
    uint64 public priceIncreaseAt = 7000;
    ...
    function mint(uint256 numNFTs) public payable {
        ...
        require(retailSupply > totalSupply() - devMinted, "Sale has ended");
        ...
        require(retailSupply >= totalSupply() + numNFTs - devMinted, "Exceeds retail supply");
        ...
        for (uint i = 0; i < numNFTs; i++) {
            ...
            _safeMint(msg.sender, 1000 + mintIndex); //I set 1000+ in order not to go on top of Ethereum Chain and predict which nft art will be minted. (@encryptedSandwich)
        }
    ...
    function devMint(uint256 numNFTs) public onlyOwner {
        require(numNFTs <= devReserve, "Not enough in the dev reserve, mint a smaller amount");
    ...
    function airdropMint(uint256 numNFTs, address receiver) public onlyOwner {
        require(numNFTs <= devReserve, "Not enough in the dev reserve, mint a smaller amount");
        
        for (uint i = 0; i < numNFTs; i++){
            uint mintIndex = totalSupply() + 1;
            _safeMint(receiver, 1000 + mintIndex); //I set 1000+ in order not to go on top of Ethereum Chain and predict which nft art will be minted. (@encryptedSandwich)
        }
