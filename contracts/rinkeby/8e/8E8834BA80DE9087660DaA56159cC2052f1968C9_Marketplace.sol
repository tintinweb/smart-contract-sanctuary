...
contract Marketplace is Pausable, Ownable, ReentrancyGuard {
    ...
    event OfferCancelled(uint256 indexed _offerID, uint256 indexed _tokenId, address indexed _offerer);
    ...
    event CounterOfferCancelled(uint256 indexed _tradeID, uint256 indexed _tokenId, address indexed _offerer);
        ...
        isTokenOwner(tokenId)
    {
        ...
        if(buyer == address(0) && isAuction){
            ...
            asset[tokenId].auctionId = totalTrades;
        ...
        isTokenOwner(tokenId)
    {
        ...
        if(trade.isCounterOffer){
            emit CounterOfferUpdated(trade.id, tokenId, trade.buyer, price, startTime, endTime);
        }
        return;
    }
        ...
        isTokenOwner(tokenId)
    {
        ...
        if(trade.isCounterOffer){
            ...
            emit CounterOfferCancelled(trade.id, tokenId, _msgSender());
        }
        return;
    }
    ...
    function buy(uint256 tradeId, uint256 tokenId) external payable whenNotPaused nonReentrant {
        ...
        if(trade.isCounterOffer){
            emit CounterOfferAccepted(trade.id, trade.seller, _msgSender(), tokenId, price);
        }
    ...
    // Offer
    function makeOffer(uint256 tokenId, uint256 price, uint256 endTime) external payable whenNotPaused nonReentrant {
        if(asset[tokenId].isAuctioned){
            require(price >= asset[tokenId].trades[asset[tokenId].auctionId].price, "Floor price not met");
        }
        ...
        totalOffers++;
        offers[totalOffers] = Offer(totalOffers, _msgSender(), price, endTime, true);
        //asset[tokenId].isOffered[_msgSender()][trade.isAuction] = true;
        //IERC20(WETH).transferFrom(_msgSender(), address(this), price);

        emit Offered(totalOffers, _msgSender(), tokenId, price, endTime);
        return;
    }
    ...
    function updateOffer(uint256 offerId, uint256 tokenId, uint256 price, uint256 endTime) external whenNotPaused nonReentrant {
        ...
        emit OfferUpdated(offer.id, tokenId, _msgSender(), price, endTime);
    }
    ...
    function acceptOffer(uint256 offerId, uint256 tokenId) external whenNotPaused nonReentrant isTokenOwner(tokenId) {
        ...
        require(IERC20(WETH).balanceOf(offerer) >= price, "Insufficient balance from offerer");

        uint256 feeAmount = price * fee / 10000;
        IERC20(WETH).transferFrom(offerer, feeCollector, feeAmount);
        IERC20(WETH).transferFrom(offerer, _msgSender(), price - feeAmount);

        masterContract.transferFrom(_msgSender(), offerer, tokenId);
        emit OfferAccepted(offer.id, _msgSender(), offerer, tokenId, price);
    ...
    function cancelOffer(uint256 offerId, uint256 tokenId) external whenNotPaused nonReentrant {
        ...
        emit OfferCancelled(offer.id, tokenId, _msgSender());
    }
    ...
    function createCounterOffer(
       ...
       Offer memory offer = offers[offerId];
        ...
        emit CounterOfferCreated(totalTrades, _msgSender(), offer.offerer, tokenId, price, startTime, endTime);
    }
    ...
    function checkBalance(uint256[] memory offerIDs) external view returns(bool[] memory canPay){
        uint256 userBalance;
        uint256 offeredPrice;
        for(uint i; i<offerIDs.length; i++){
            userBalance = IERC20(WETH).balanceOf(offers[offerIDs[i]].offerer);
            offeredPrice = offers[offerIDs[i]].price;
            canPay[i] = false;
            if(userBalance >= offeredPrice){
                canPay[i] = true;
            }
        }
    }
