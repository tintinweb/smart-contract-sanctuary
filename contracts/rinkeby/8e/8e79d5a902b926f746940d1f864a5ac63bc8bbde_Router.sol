...
contract Router is Ownable {
     ...
     */
    event Contribute(address indexed account, address indexed wallet, uint256 indexed class, uint256 tokenId, uint256 weight);

    /**
     * @dev Emitted when `oldRed` wallet is updated to `newRed` wallet.
     */
    event UpdateRed(address oldRed, address newRed);

    /**
     * @dev Emitted when `oldYellow` wallet is updated to `newYellow` wallet.
     */
    event UpdateYellow(address oldYellow, address newYellow);

    /**
     * @dev Emitted when `oldBlue` wallet is updated to `newBlue` wallet.
     */
    event UpdateBlue(address oldBlue, address newBlue);

    /**
     * @dev Emitted when `oldContender` wallet is updated to `newContender` wallet.
     */
    event UpdateContender(address oldContender, address newContender);

    /**
     * @dev Emitted when `wallet` has a new status `isSpecial`.
     */
    event ToggleSpecialStatus(address wallet, bool isSpecial);

    /**
     * @dev Emitted when `oldCap` has been updated to `newCap`.
     */
    event UpdateCap(uint256 oldCap, uint256 newCap);

    /**
     * @dev Emitted when `oldMultiplier` has been updated to `newMultiplier`.
     */
    event UpdateBonusMultiplier(uint256 oldMultiplier, uint256 newMultiplier);

    /**
     * @dev Emitted when the game has ended.
     */
    event EndGame(bool isEnded);
    ...
    function toggleSpecialWalletStatus(address wallet) external onlyOwner {
        ...
        emit ToggleSpecialStatus(wallet, specialWallets[wallet]);
    }
    ...
    function setCap(uint256 cap_) external onlyOwner {
        require(cap_ != cap, "Same soft cap");
        uint256 oldCap = cap;
        cap = cap_;
        emit UpdateCap(oldCap, cap);
    }
    ...
    function setContenderWallet(address contender_) external onlyOwner {
        ...
        specialWallets[contender_] = true;
        address oldContender = contender;
        contender = contender_;
        emit UpdateContender(oldContender, contender);
    }
    ...
    function setRed(address red_) external onlyOwner {
        ...
        specialWallets[red_] = true;
        address oldRed = red;
        red = red_;
        emit UpdateRed(oldRed, red);
    }
    ...
    function setYellow(address yellow_) external onlyOwner {
        specialWallets[yellow] = false;
        specialWallets[yellow_] = true;
        address oldYellow = yellow;
        yellow = yellow_;
        emit UpdateYellow(oldYellow, yellow);
    }
    ...
    function setBlue(address blue_) external onlyOwner {
        ...
        specialWallets[blue_] = true;
        address oldBlue = blue;
        blue = blue_;
        emit UpdateBlue(oldBlue, blue);
    }
    ...
    function setBonusMultiplier(uint256 multiplier_) external onlyOwner {
        ...
        uint256 oldMultiplier = BONUS_MULTIPLIER;
        BONUS_MULTIPLIER = multiplier_;
        emit UpdateBonusMultiplier(oldMultiplier, BONUS_MULTIPLIER);
    }
    ...
    function _contribute(address account) private {
        ...
        emit Contribute(account, targetWallet, class, tokenId, weight);
    }
