...
contract PoolFactory is Initializable, OwnableUpgradeable, IPoolFactory {
    ...
    function initialize(address _feeRecipient, uint256 _feePercent)
        external
        initializer
    {
    ...
    function approvePool(uint256 poolId) external onlyAdmin {
        ...
        require(isApproved[poolId] == false, "Pool is already verified");
    ...
    function createPool(uint256 poolId) external returns (address) {
        require(poolId < poolInfos.length, "Invalid PoolId");
        require(isApproved[poolId] == true, "Pool is not approved yet");
        require(
            isCreated[poolId] == false,
            "Already created for this registration"
        );
...
contract Pool is Ownable {
    ...
    function claimableAmount(address user) external view returns (uint256) {
        return claimable[user].mul(ratio);
    }
    ...
    function claim() external {
        ...
        uint256 amount = claimable[msg.sender].mul(ratio);
    ...
    function buy(uint256 weiAmount) public {
        ...
        uint256 amount = weiAmount.mul(ratio);
        require(
            totalOwed.add(amount) <= token.balanceOf(address(this)),
            "sold out"
        );
...
interface IPoolFactory {
    event PoolCreated(uint256 indexed poolId, address indexed addr);
