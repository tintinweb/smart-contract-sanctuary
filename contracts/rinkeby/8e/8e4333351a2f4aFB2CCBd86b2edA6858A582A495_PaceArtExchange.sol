...
contract Exchange is ExchangeCore {
    ...
    {
        return hashOrder(
          Order(addrs[0], addrs[1], addrs[2], uints[0], uints[1], addrs[3], feeMethod, side, saleKind, addrs[4], howToCall, callData, addrs[5], staticExtradata, addrs[6], uints[2], uints[3], uints[4], uints[5], uints[6])
        );
    }
    ...
    function validateOrderParameters_ (
        ...
        returns (bool)
    {
        Order memory order = Order(addrs[0], addrs[1], addrs[2], uints[0], uints[1], addrs[3], feeMethod, side, saleKind, addrs[4], howToCall, callData, addrs[5], staticExtradata, addrs[6], uints[2], uints[3], uints[4], uints[5], uints[6]);
    ...
    function ordersCanMatch_(
        ...
        returns (bool)
    {
        Order memory buy = Order(addrs[0], addrs[1], addrs[2], uints[0], uints[1], addrs[3], FeeMethod(feeMethodsSidesKindsHowToCalls[0]), SaleKindInterface.Side(feeMethodsSidesKindsHowToCalls[1]), SaleKindInterface.SaleKind(feeMethodsSidesKindsHowToCalls[2]), addrs[4], AuthenticatedProxy.HowToCall(feeMethodsSidesKindsHowToCalls[3]), calldataBuy, addrs[5], staticExtradataBuy, addrs[6], uints[2], uints[3], uints[4], uints[5], uints[6]);
        Order memory sell = Order(addrs[7], addrs[8], addrs[9], uints[7], uints[8], addrs[10], FeeMethod(feeMethodsSidesKindsHowToCalls[4]), SaleKindInterface.Side(feeMethodsSidesKindsHowToCalls[5]), SaleKindInterface.SaleKind(feeMethodsSidesKindsHowToCalls[6]), addrs[11], AuthenticatedProxy.HowToCall(feeMethodsSidesKindsHowToCalls[7]), calldataSell, addrs[12], staticExtradataSell, addrs[13], uints[9], uints[10], uints[11], uints[12], uints[13]);
...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    function sizeOf(Order memory order)
        ...
        returns (uint)
    {
        return ((0x14 * 7) + (0x20 * 9) + 4 + order.callData.length + order.staticExtradata.length);
        ...
        reentrancyGuard
    {
        ...
        // Transfer Royalty Fee. Prevent stack too deep errors
        {
            ...
            LibPart.Part memory royalty = RoyaltiesV2(target).getPaceArtV2Royalties(tokenId);
            ...
            if (royalty.account != address(0) && royalty.value > 0) {
                transferTokens(
                    ...
                    payout, 
                    royalty.account, 
                    SafeMath.div(SafeMath.mul(royalty.value, price), INVERSE_BASIS_POINT)
                );
            }
        }
