...
contract KSTStaking is Ownable {
    ...
    function stake(uint256 value) external notHalted {
        require(value > 0, "KSTStaking: stake value should be greater than 0");
    ...
    function unstake(uint256 value) external lockable {
        require(_balances[_msgSender()] >= value, 'KSTStaking: insufficient staked balance');

        _balances[_msgSender()] = _balances[_msgSender()].sub(value,"KSTStaking: insufficient staked balance");
        _token.transfer(_msgSender(), value);
        emit Unstake(_msgSender(),block.timestamp,value);
    }

    function lock(address user, uint256 userUnlockTime) external onlyIDO {
        require(userUnlockTime > block.timestamp, "KSTStaking: unlock is in the past");
        if (_unlockTime[user] < userUnlockTime) {
            _unlockTime[user] = userUnlockTime;
            emit Lock(user,block.timestamp,userUnlockTime,_msgSender());
        }
    }
    ...
    modifier lockable() {
        require(_unlockTime[_msgSender()] <= block.timestamp, "KSTStaking: account is locked");
        _;
    }

    modifier notHalted() {
        require(!halted, "KSTStaking: Deposits are paused");
        _;
    }
}
