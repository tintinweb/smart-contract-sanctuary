 ...
 */
  ...
  function testAlternative() public view returns(uint){
    ...
    for(uint i = 0; i< nftprojects.length; i++){
      if(nftprojects[i].balanceOf(msg.sender) > 0){
        allowance += 1;
      }
    }
    return allowance;
  ...
  function mintMetablobs(address to) external onlyEOA nonReentrant {
    ...
    if(to == msg.sender){
      ...
      require(totalSupply() + 1 <= MAX_NFT_SUPPLY, "thats enough Metablobs mintin' for today, buddy");
      tokensPerAddress[msg.sender] += 1;
      mintM(to);
    }
    else {
      // or to someone you like very much :
      require(totalSupply() + 2 <= MAX_NFT_SUPPLY, "thats enough Metablobs mintin' for today, buddy");
      mintM(to);
