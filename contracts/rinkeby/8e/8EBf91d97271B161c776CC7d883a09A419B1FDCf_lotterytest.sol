...
contract lotterytest is ERC721, Ownable, ReentrancyGuard, VRFConsumerBase {
  ...
  function getWinnerAddress(uint randomVal) private returns (address){
    uint winnerTokenId = thisSeasonsTickets[randomVal % getCurrentSeasonTicketCount()];
  ...
  function changeSeasonToBaseURI(uint _season, string memory newSeasonToBaseURI) external onlyOwner {
    seasonToBaseURI[_season] = newSeasonToBaseURI;
  }
