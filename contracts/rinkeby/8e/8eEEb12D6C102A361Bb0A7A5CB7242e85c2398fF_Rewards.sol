...
contract Rewards is Ownable {
    ...
    event RewardClaimed(address indexed owner, uint256 value);
    ...
    constructor(ERC20 _token, address _rewardDistributor, Operator _operator) {
        token = _token;
    ...
    modifier onlyOperator {
        require(operator.isOperator(msg.sender), "Rewards:: Unauthorized Access");
        _;
    }

    function claimRewards(Balance memory _balances, uint8 v, bytes32 r, bytes32 s) external {
        string memory messageSigned = prepareMessage(_balances);
        address signedBy = StringLibrary.recover(messageSigned, v, r, s);
        require(operator.isOperator(signedBy), "ClaimReward:: Unauthorized Signer");
        address recipient = _balances.recipient;
        require(msg.sender == recipient, "ClaimReward:: Unauthorized Access");
        uint256 toClaim = _balances.value.sub(claimed[recipient]);
        require(toClaim > 0, "ClaimReward:: Can not claim Zero Tokens");
        claimed[recipient] = _balances.value;
        require(token.transferFrom(rewardDistributor, msg.sender, toClaim), "ClaimReward:: Tranfer Failed");
        emit RewardClaimed(recipient, toClaim);
