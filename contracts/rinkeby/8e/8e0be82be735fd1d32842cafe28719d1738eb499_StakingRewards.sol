...
contract StakingRewards is OlympusAccessControlled, ReentrancyGuard {
    ...
    mapping(address => UserInfo) private _userInfos;    // number of staked TAZOR token per user
    ...
    function balanceOfTazor(address account) external view returns (uint256) {
        return _userInfos[account].tazorNum;
    }

    function balanceOfTaz(address account) external view returns (uint256) {
        return _userInfos[account].tazNum;
    }
   
    // add new reward to previously earned reward
    function earned(address account) public returns (uint256) {
        uint256 timeinterval = block.timestamp.sub(_userInfos[account].lastUpdateTime);

        uint256 rate = getTazorAndTazRate(); // get TAZOR/TAZ rate
        uint256 _apr = getAPRValue().mul(timeinterval).div(365 days);
        uint256 newEarned = _userInfos[account].tazorNum.mul(rate).mul(_apr).div(10 ** 9); // reward TAZ amount

        _userInfos[account].reward = newEarned.add(_userInfos[account].reward);
        return _userInfos[account].reward;
    }
    ...
    function stakeTazor(uint256 amount) external nonReentrant updateReward(msg.sender) {
        ...
        _userInfos[msg.sender].tazorNum = _userInfos[msg.sender].tazorNum.add(amount);
    ...
    function stakeTaz(uint256 amount) external nonReentrant updateReward(msg.sender) {
        ...
        _userInfos[msg.sender].tazNum = _userInfos[msg.sender].tazNum.add(amount);
        tazToken.safeTransferFrom(msg.sender, address(this), amount);
        emit TazStaked(msg.sender, amount);
    }


    function withdrawTazor(uint256 amount) public nonReentrant updateReward(msg.sender) {
        require(amount > 0, "Cannot withdraw 0");
        // require(block.timestamp > _userInfos[msg.sender].lastUpdateTime + 12 hours, "can't draw tokens");
        _totalTazorSupply = _totalTazorSupply.sub(amount);
        _userInfos[msg.sender].tazorNum = _userInfos[msg.sender].tazorNum.sub(amount);
        tazorToken.safeTransfer(msg.sender, amount);
        emit TazorWithdrawn(msg.sender, amount);
    }

    function withdrawTaz(uint256 amount) public nonReentrant updateReward(msg.sender) {
        require(amount > 0, "Cannot withdraw 0");
        // require(block.timestamp > _userInfos[msg.sender].lastUpdateTime + 12 hours, "can't draw tokens");
        _totalTazSupply = _totalTazSupply.sub(amount);
        _userInfos[msg.sender].tazNum = _userInfos[msg.sender].tazNum.sub(amount);
    ...
    function getReward() public nonReentrant {
        ...
        require(block.timestamp > _userInfos[msg.sender].lastUpdateTime + 12 hours, "can't get rewards");

        uint256 reward = _userInfos[msg.sender].reward;
        if (reward > 0) {
            _userInfos[msg.sender].reward = 0;
     
            tazToken.safeTransfer(msg.sender, reward);
            _userInfos[msg.sender].lastUpdateTime = block.timestamp;            
            emit RewardPaid(msg.sender, reward);
        }
    }

    function getAPRValue() public view returns (uint256) {
        // this is equation (5 * taz / 100000) + 0.1  : 10**9 standard
        return _userInfos[msg.sender].tazNum.mul(10000).mul(5).add(10**8);
    }


    function exitTazor() external {
        withdrawTazor(_userInfos[msg.sender].tazorNum);
        getReward();
    }
    ...
    modifier updateReward(address account) {
        ...
        if (account != address(0)) {
            if (_userInfos[account].lastUpdateTime == 0) {
                _userInfos[account].lastUpdateTime = block.timestamp;
            }
            _userInfos[account].reward = earned(account);
            _userInfos[account].lastUpdateTime = block.timestamp;
        }
        _;
    }
