...
contract Sk8ers is ERC721Enumerable, Ownable {
    ...
    uint256 private _saleTime = 1638939600; // 8 December 2021 00:00:00
    ...
    constructor(string memory baseURI, string memory theContractURI, address[] memory preSaleAddresses) ERC721("Sk8ers", "SK8") {
        setBaseURI(baseURI);
    ...
    modifier preSaleIsOpen() {
        require(block.timestamp >= _saleTime - 172800, "The Sk8ers presale has not yet started.");
    ...
    function reserve(uint256 _count) public onlyOwner {
        require(block.timestamp < _saleTime, "Sale has already started");
        require(totalSupply() + _count <= RESERVES, "Reserves limit reached");
