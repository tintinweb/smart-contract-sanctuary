...
contract ERC1155Base is HasSecondarySaleFees, Ownable, ERC1155Metadata_URI, HasContractURI, ERC1155 {
    ...
    function _mint(address _from, address _to, uint256 _id, Fee[] memory _fees, uint256 _value, string memory _uri) internal {
        if (keccak256(abi.encodePacked(_tokenURI(_id))) == keccak256(abi.encodePacked(""))) {
            ...
            // call onERC1155Received if the destination is a contract.
            if (_to.isContract()) {
                _doSafeTransferAcceptanceCheck(msg.sender, _from, _to, _id, _value, "");
            }
        } else {
            _issue(_to, _id, _value);
        }
    }
...
contract iNFTspaceBlind is Ownable, SignerRole, ERC1155Base {
    ...
    uint256 public rewardThresholdWorks;
    ...
    function payMintFee() public payable {
        ...
        if (minters[msg.sender].totalPayMintWorks % rewardThresholdWorks == 0) {
            minters[msg.sender].remainMintWorks = minters[msg.sender].remainMintWorks.add(times.add(1));
        } else {
