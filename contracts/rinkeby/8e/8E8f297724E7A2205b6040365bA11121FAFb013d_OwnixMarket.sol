...
interface IOwnixMarket {
   ...
   * @param _reservePrice The reserve price
   */
  function listSaleNFT(uint256 _tokenId, uint256 _amount, uint256 _reservePrice) external;
   ...
   * @param _tokenId - The token id
   */
  function unlistSaleNFT(uint256 _tokenId) external;
  ...
  function changeReservePrice(uint256 _tokenId, uint32 _index, uint256 _reservePrice) external;
   ...
   /**
   * @dev Change sale reserve price
   * @param _tokenId - The token id
   * @param _reservePrice - The new reserve price
   */
  function changeSaleReservePrice(uint256 _tokenId, uint256 _reservePrice) external;
  ...
  /**
   * @dev Place a sales order for an ERC1155 token.
   * @param _tokenId - The  token id
   */
  function placeSaleOrder(uint256 _tokenId) external;
   ...
   * @param _s - Must produce valid secp256k1 signature from the holder along with `r` and `v`
   */
  function placeSaleOrderWithPermit(address _bidder, uint256 _tokenId, uint256 _deadline, uint8 _v, bytes32 _r, bytes32 _s) external;
  ...
  */
  ...
  function getAuction(uint256 _tokenId, uint32 _index) external returns (Auction memory);
   ...
   /**
   * @dev Get sale by token id 
   * @param _tokenId - The token id
   * @return sale
   */
  function getSale(uint256 _tokenId) external returns (Sale memory);
...
contract OwnixMarket is ERC1155Holder, Ownable, Pausable, IOwnixMarket, ReentrancyGuard {
  ...
  function listSaleNFT(uint256 _tokenId, uint256 _amount, uint256 _reservePrice) override external nonReentrant() whenNotPaused() {
    require(_tokenId > 0, "token id can't be zero");
    require(_amount > 0, "amount can't be zero");
    require(_reservePrice > 0, "reservePrice id can't be zero");
    ...
    if(sale.tokenId == 0) {
      sale = Sale({
       ...
       sale.reservePrice = _reservePrice;
    }
  ...
  function unlistSaleNFT(uint256 _tokenId) override external nonReentrant() whenNotPaused() {
    Sale memory sale = sales[_tokenId];
    require(sale.nftOwner == msg.sender,  "Must be nft owner unlist NFT");
   
    emit NFTSaleUnlisted(
     sale.nftOwner,
     ownixNFTToken, 
     _tokenId,
     sale.amount);
  ...
  function changeSaleReservePrice(uint256 _tokenId, uint256 _reservePrice) override external whenNotPaused() {
    Sale memory sale = sales[_tokenId];
    require(sale.nftOwner == msg.sender, "Must be nft owner to change reserve");

    sale.reservePrice = _reservePrice;

    sales[_tokenId] = sale;

    emit SaleReservePriceChanged(
      msg.sender,
      ownixNFTToken,
      _tokenId,
      _reservePrice
  ...
  function placeSaleOrder(uint256 _tokenId) override external whenNotPaused() nonReentrant() { 
      _placeSaleOrder(msg.sender, _tokenId);
  }
  ...
  function placeSaleOrderWithPermit(
    address _bidder,
    uint256 _tokenId, 
    uint256 _deadline,
    uint8 _v,
    ...
    bytes32 _s) override external whenNotPaused() nonReentrant() {
      ...
      IERC20Permit(address(ownixToken)).permit(_bidder, address(this), sales[_tokenId].reservePrice, _deadline, _v, _r, _s);
      _placeSaleOrder(_bidder, _tokenId);
  }
  ...
  function getSale(uint256 _tokenId) override external view returns (Sale memory)  {
    return sales[_tokenId];
  }
  ...
  function _placeBid(address _bidder, uint256 _tokenId, uint32 _index, uint256 _price) private whenNotPaused() {
   ...
   /**
   * @dev Place a sale order for an ERC1155 token.
   * @param _bidder - address the bidder address
   * @param _tokenId - uint256 of the token id
   */
  function _placeSaleOrder(address _bidder, uint256 _tokenId) private whenNotPaused() {
    Sale memory sale = sales[_tokenId];
    require(sales[_tokenId].tokenId != 0, "Sale must be listed first");
    require(sales[_tokenId].amount > 0, "Sale amount is zero");

    // Transfer tokens to the market
    require(
      ownixToken.transferFrom(_bidder, address(this), sale.reservePrice),
      "Transferring the sale price amount to the marketplace failed"
    );

    uint256 saleShareAmount;
    if (ownerSharePerMillion > 0) {
      // Calculate sale share
      saleShareAmount = sale.reservePrice * ownerSharePerMillion / ONE_MILLION;
      // Transfer share amount to the bid contract
      require(
        ownixToken.transfer(feeCollector, saleShareAmount),
        "Transferring the share failed"
    ...
    uint256 royaltiesAmount;
    if (royaltiesPerMillion > 0) {
       address firstTimeOwner = ownixNFTToken.getFirstTimeOwner(sale.tokenId);
       if(firstTimeOwner != sale.nftOwner) {
        // Calculate royalties
        royaltiesAmount = sale.reservePrice * royaltiesPerMillion / ONE_MILLION;
        // Transfer royalties Owner
        require(
          ownixToken.transfer(firstTimeOwner, royaltiesAmount),
          "Transferring the royalties failed"
    ...
    // Transfer ownixToken from bidder to seller
    require(
      ownixToken.transfer(sale.nftOwner, sale.reservePrice - (saleShareAmount + royaltiesAmount)),
      "Transferring ownixToken to nft owner failed"
    );

    if (inflationPerMillion > 0) {
      // Calculate mint tokens
      uint256 mintShareAmount = sale.reservePrice * inflationPerMillion / ONE_MILLION;
      // Mint the new ownix tokens to the inflationCollector
      ownixToken.mint(inflationCollector, mintShareAmount);
    }

    sale.salesAmount = sale.salesAmount + 1;
