...
contract KommunitasStakingV2 is Ownable {
    ...
    function stake(uint256 _amount, uint256 _lpid) isStakePaused public {
        ...
        locks[msg.sender].push(TokenLock(_lpid, userId, _amount, block.timestamp, matureUntil, lockReward, false));
    ...
    function preMatureWithdraw(uint256 _sid, uint256 _amount) external returns (uint256) {
        ...
        if(locks[msg.sender][_sid].amount == _amount){
            ...
            locks[msg.sender][newSid].reward = calculateReward(_amount, locks[msg.sender][newSid].lockPeriodIndex);
            locks[msg.sender][newSid].claimed = true;
            
            locks[msg.sender][_sid].amount = locks[msg.sender][_sid].amount.sub(_amount);
            locks[msg.sender][_sid].reward = calculateReward(locks[msg.sender][_sid].amount, locks[msg.sender][_sid].lockPeriodIndex);
        }
