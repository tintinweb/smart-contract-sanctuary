...
struct Farm {
    ...
    uint256 collectiblemaxBoostLevel;
    uint256 collectibleblocksToMaxBoost;
    uint256 collectibleAddedBlockNumber;
    uint256 collectibleExpiry;
}
...
contract L2Corn is ERC777, IERC777Recipient, ReentrancyGuard, IERC721Receiver, ERC165, ERC721Holder, IArbToken {
    ...
    modifier requireCollectible(address targetAddress, bool requiredState) {
        if (requiredState) {
            require(
                farms[targetAddress].collectibleID != 0,
                "You must have a collectible on your farm"
            );
        } else {
            require(
                farms[targetAddress].collectibleID == 0,
                "You must not have a collectible on your farm"
            );
        }
        _; // Call the actual code
    }
    ...
    /**
     * @dev This will be Farmland Collectible NFT token smart contract address
    */
    ...
    */
    ...
    function equipCollectible(uint256 tokenID)
        ...
        nonReentrant()
        preventSameBlock(_msgSender())
        requireFarm(_msgSender(), true) // You can't add collectible if you don't have a farm
        requireCollectible(_msgSender(), false) // You can't add collectible if you already have one equipped
        {
        ...
        // Store Collectible traits on the farm
        Farm storage farm = farms[_msgSender()];
        farm.collectibleID = tokenID;
        farm.collectibleAddedBlockNumber = block.number;
        farm.collectibleName = _name;
        farm.collectiblemaxBoostLevel = _maxBoostLevel;
        farm.collectibleblocksToMaxBoost = _blocksToMaxBoost;
        farm.collectibleExpiry = _expiry;
    ...
    function releaseCollectible()
        ...
        Farm storage farm = farms[_msgSender()];
        uint256 tokenID = farm.collectibleID;
        string memory collectibleName = farm.collectibleName;
        
        // Wipe collectible traits
        farm.collectibleID = 0;
        farm.collectibleAddedBlockNumber = 0;
        farm.collectibleName = "";
        farm.collectiblemaxBoostLevel = 0;
        farm.collectibleblocksToMaxBoost = 0;
        farm.collectibleExpiry = 0;
    ...
    {
        Farm memory targetFarm = farms[targetAddress]; // Shortcut accessor
    ...
    /**
     * @dev PUBLIC: Find out a farms Collectible boost for the current LAND address (Using 1 block = 13.5 sec formula)
     */
        ...
        returns (uint256 collectibleMultiplier)
    {
        ...
        if (targetFarm.collectibleID == 0) {
            return _percentMultiplier;
        }

        // You don't get a boost until minimum blocks passed 
        uint256 targetBlockNumber = targetFarm.collectibleAddedBlockNumber;
        ...
        if (targetFarm.collectiblemaxBoostLevel <= _percentMultiplier) {
            return _percentMultiplier;
        }
        // You don't get a boost if the your collectible has expired; if Expiry equals zero then no expiry
        if (targetFarm.collectibleExpiry > 0 && block.timestamp >= targetFarm.collectibleExpiry) {
            return _percentMultiplier;
        }

        // Calculates the boost extension; i.e., the extension over and above 1x e.g., the 2x to get to a 3x boost
        uint256 collectibleBoostExtension = targetFarm.collectiblemaxBoostLevel.sub(_percentMultiplier);
        ...
        uint256 blockDiff =
            block.number
            .sub(targetBlockNumber)
            .mul(collectibleBoostExtension) 
            .div(targetFarm.collectibleblocksToMaxBoost)
            .add(_percentMultiplier);

        collectibleMultiplier = Math.min(targetFarm.collectiblemaxBoostLevel, blockDiff); // Min 1x, Max depends on the collectible attributes 
    }
