  ...
  Ownable
{
  ...
  mapping (uint256 => uint256) public signatureChangePriceHistory;
  ...
  function setSaleStartBlock(uint _saleStartBlock) public onlyOwner {
  ...
  function mintSignature(
    ...
    external
  {
    require(_tokenId.add(1) <= maxSupply, "Purchase would exceed max supply of NFT");
    require(initialMintPrice <= msg.value, "Ether value sent is not correct");
    require(saleTime(_tokenId) <= block.number, "Sale not yet started");

    _mint(msg.sender, _tokenId);
    _setTokenUri(_tokenId, _uri);
    _setSignatureChangePriceHistory(_tokenId, initialMintPrice);
    ...
    canOperate(_tokenId) 
  {
    ...
    uint256 oldPrice = signatureChangePriceHistory[_tokenId];
    ...
    _setTokenUri(_tokenId, _uri);
    _setSignatureChangePriceHistory(_tokenId, newPrice);
    emit TokenUriSet(_tokenId, _uri);
  }
   ...
   */
  function _setSignatureChangePriceHistory(
    uint256 _tokenId,
    uint256 _price
  )
    internal
  {
    signatureChangePriceHistory[_tokenId] = _price;
  }
