...
contract BarbershopTest is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint16 public barberMinted = 0;
  uint16 public barberStolen = 0;
  uint16 public babyStolen = 0;
  mapping(uint16 => address) private _randomSource;
  mapping(uint16 => uint) public phasePrice;
  mapping(uint16 => bool) private _isBarber;

  ITheChair public theChair;
  ...
  function _baseURI() internal view virtual override returns (string memory) {
    return baseURI;
      ...
      function giveAway(uint _amount, address _address) public onlyOwner {
        require(tokensMinted + _amount <= maxSupply, "All tokens minted");
  ...
  // public
        function mint(uint _amount, bool _stake) public payable {
        ...
        uint totalHairCost = 0;
        if (phase == 1) {
            // Paid mint
            require(mintPrice(_amount) == msg.value, "Invalid payment amount");
        } else {
            // Mint via Penny token burn
            require(msg.value == 0, "grow your hair to mint dum dum");
            totalHairCost = mintPrice(_amount);
            require(hair.balanceOf(msg.sender) >= totalHairCost, "Your locks arent long enough");
        }

        if (totalHairCost > 0) {
            hair.burn(msg.sender, totalHairCost);
        }
        ...
        for (uint i = 0; i < _amount; i++) {
            ...
            if (recipient != msg.sender) {
                isBarber(tokenId) ? barberStolen += 1 : babyStolen += 1;
            ...
            if (!_stake || recipient != msg.sender) {
                ...
                _safeMint(address(theChair), tokenId);
                tokenIds[i] = tokenId;
            }
        }
        if (_stake) {
            theChair.addTokensToStake(msg.sender, tokenIds);
        }
    }
        ...
        if (getSomeRandomNumber(barberMinted, 100) >= 10) {
            return msg.sender; // 90%
        }

        address thief = theChair.randomPirateOwner();
        if (thief == address(0x0)) {
            return msg.sender;
        }
        return thief;
    }
        function isBarber(uint16 id) public view returns (bool) {
        return _isBarber[id];
    }
        function updateRandomCounter() internal {
        _randomIndex += 1;
        _randomCalls += 1;
      ...
      function switchToSalePhase(uint16 _phase, bool _setTokens) public onlyOwner {
        ...
        if (phase == 1) {
            internalAddTokens(1, 2);
        } else if (phase == 2) {
            internalAddTokens(3, 5);
        } else if (phase == 3) {
  ...
  function setTheChair(address _chair) external onlyOwner {
      theChair = ITheChair(_chair);
  }
