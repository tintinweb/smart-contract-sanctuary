...
contract OOC is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    event CreateOOC(uint256 indexed id);
	
    constructor(string memory baseURI, uint256 _maxNFT, uint256 _price, uint256 _maxByMint, uint256 _availableToMint, uint256 _maxHoldingNFT) ERC721("Obscure Octopus Club", "OOC") {
        MAX_NFT = _maxNFT;
		PRICE = _price;
		MAX_BY_MINT = _maxByMint;
		AVAILABLE_TO_MINT = _availableToMint;
		MAX_HOLDING_NFT = _maxHoldingNFT;
		setBaseURI(baseURI);
        pause(true);
    }
