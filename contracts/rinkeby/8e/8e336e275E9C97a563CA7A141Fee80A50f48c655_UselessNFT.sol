...
contract UselessNFT is ERC721, Ownable, ERC721URIStorage, ERC721Enumerable, Pausable {
    ...
    constructor(string memory _metaBaseURL) ERC721("UselessNFT", "LOLZ") {
        bytes memory __testMetaBaseURL = bytes(_metaBaseURL);
        require(__testMetaBaseURL.length > 0, "MetaBaseURL missing");
        
        metaBaseURL = _metaBaseURL;

        _performMint(reservedSupply);
    }
    ...
    function setDataMap(string memory url) public onlyOwner {
        bytes memory __testURL = bytes(url);
        require(__testURL.length > 0, "MetaBaseURL missing");
        
        finalDataMapURL = url;
    }

    function setPermanentURI(string memory uri, uint256 tokenId) public onlyOwner {
        bytes memory __testUri = bytes(uri);
        require(__testUri.length > 0, "URI missing");
    ...
    function updateLowTierPrice(uint256 newPrice)  public onlyOwner {
        LOW_TIER_PRICE = newPrice;
    }

    function updateMidTierPrice(uint256 newPrice)  public onlyOwner {
        MID_TIER_PRICE = newPrice;
    }

    function updateHighTierPrice(uint256 newPrice)  public onlyOwner {
        HIGH_TIER_PRICE = newPrice;
    }

    function updateMinMintable(uint value)  public onlyOwner {
        MIN_MINTABLE = value;
    }

    function updateMaxMintable(uint value)  public onlyOwner {
        MAX_MINTABLE = value;
    }

    function updateBonusMintable(uint value)  public onlyOwner {
        BONUS_MINTABLE = value;
    }

    function updatePreSaleMaxSupply(uint256 newMax)  public onlyOwner {
        preSaleMaxSupply = newMax;
    }
    ...
    function withdrawWithLimit(uint256 amount) public onlyOwner {
