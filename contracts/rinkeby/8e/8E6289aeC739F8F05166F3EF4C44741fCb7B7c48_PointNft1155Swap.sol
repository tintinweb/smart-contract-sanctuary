...
contract PointNft1155Swap is Ownable, ReentrancyGuard, ERC1155Receiver {
  ...
  function swapByUnicorn(uint256 _id, uint256 _amount)
    external payable nonReentrant {
      ...
      if (_balance < _totalPrice) {
        require(msg.value.mul(unicornToEth).div(1 ether).add(_balance) >= _totalPrice, "PointNft1155Swap: not enough balance");
        unicornPool.burn(_msgSender(), _balance);

        uint256 _amountToWithdraw = msg.value.sub(_totalPrice.sub(_balance).mul(1 ether).div(unicornToEth));
  ...
  function swapByRainbow(uint256 _id, uint256 _amount)
    external payable nonReentrant {
      ...
      if (_balance < _totalPrice) {
        require(msg.value.mul(rainbowToEth).div(1 ether).add(_balance) >= _totalPrice, "PointNft1155Swap: not enough balance");
        rainbowPool.burn(_msgSender(), _balance);

        uint256 _amountToWithdraw = msg.value.sub(_totalPrice.sub(_balance).mul(1 ether).div(rainbowToEth));
