...
contract SilverNFT is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    // presale stuff
    IERC721Enumerable private constant PRESALE_TOKEN = IERC721Enumerable(0x1AD4C63b2eC01891e27e3AE4c8A5E9C7D20a147a);
    mapping(uint256 => uint256) private _presaleTokenUsage;
    bool public _isPresaleMintingDone = false;

    // sale stuff
    uint256 public _cap = 20;
    uint256 public _maxBuyAmount = 6;
    ...
    string private _tokenBaseURI = 'ipfs://Qmaztir4UavpHy9A9wZiFxc7LLNcG9eZCrUJm6B4A7Ju3D';
    ...
    function mint(uint256 mintAmount) public payable nonReentrant {
        ...
        if(!_isPresaleMintingDone) {
            require(mintAmount <= presaleTokenMaxBuyAmount(), "you dont have enough presale tokens");
            uint256 reduce_amount = mintAmount;
            for(uint256 i = 0; i < presaleTokenAmount(); i++) {
    ...
    function presaleTokenAmount() public view returns(uint256) {
        return PRESALE_TOKEN.balanceOf(_msgSender());
    }

    function presaleTokenMaxBuyAmount() public view returns(uint256) {
