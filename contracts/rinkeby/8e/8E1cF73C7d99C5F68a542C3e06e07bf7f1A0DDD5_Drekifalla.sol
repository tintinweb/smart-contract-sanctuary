// SPDX-License-Identifier: MIT
    ...
    contract Drekifalla is ERC721, Ownable, ERC721Enumerable {
      using SafeMath for uint256;
      using Strings for uint256;
    
      uint256 public constant tokenPrice = 0.08 ether;
      uint256 public constant presaletokenPrice = 0.06 ether; // 0.0019 ETH 
      ...
      event DrekifallaMinted(uint256 tokenId, address owner);
    
      constructor() ERC721("Drekifalla", "DF") {}
      ...
      function mintDrekifalla(uint256 numberOfTokens) external payable {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
          ...
          if (totalSupply() < MAX_TOKENS) {
            ...
            emit DrekifallaMinted(id, msg.sender);
          }
        }
      }
    
      function presaleDrekifalla(uint256 numberOfTokens) external payable {
        ...
        returns (string memory)
      {
        ...
        if (isRevealed == false) {
          return
            "ipfs://QmZgPxtSsStoxLju4gf36gxyrMqVuiMwJvYkUBBDihmsiC/hidden.json";
        }
