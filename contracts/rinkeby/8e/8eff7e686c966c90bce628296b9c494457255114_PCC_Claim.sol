...
contract PCC_Claim is ERC721, Ownable, ReentrancyGuard, Pausable, ERC721Enumerable {
   ...
   string[50] public ImageArtist;
    ...
    function addWhitelist(uint256 index, address[] memory addresses) public onlyOwner tokenExists(index) {
        require(_tokenCounter[index].current() == 0, "Cannot add whitelist after minting has started");
        for(uint256 i; i < addresses.length; i++){
            Whitelist[index][addresses[i]] = true;
        }
    }
    
    function addAddressToWhitelist(uint256 index, address addy) public onlyOwner tokenExists(index) {
        require(!AddressRedeemed[index][addy], "Address has already redeemed");
        Whitelist[index][addy] = true;
    }

    function addNewCollection(uint256 _size, string memory _description, string memory _hash, string memory _name, string memory _series, string memory _artist) public onlyOwner {
        uint256 start_number = ImageOffset[_collectionCount.current()];

        ImageDescription[_collectionCount.current()] = _description;
        ImageHashes[_collectionCount.current()] = _hash;
        ImageName[_collectionCount.current()] = _name;
        ImageSeries[_collectionCount.current()] = _series;
        ImageArtist[_collectionCount.current()] = _artist;
                _collectionCount.increment();
        ImageOffset.push(start_number.add(_size));
    }
    
    function updateCollectionDetails(uint256 _index, string memory _description, string memory _hash, string memory _name, string memory _series, string memory _artist) public onlyOwner tokenExists(_index){
