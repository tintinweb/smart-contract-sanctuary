...
contract MultiSenderV4 is ServiceCharges { 
    ...
    modifier getServiceFee(string memory _paymentMethod) {
        if( keccak256(abi.encodePacked(_paymentMethod)) == keccak256(abi.encodePacked("eth")) ) require(msg.value >= ethFee, "Not Enought ETH sent");
        else if( keccak256(abi.encodePacked(_paymentMethod)) == keccak256(abi.encodePacked("zuz")) ) IERC20(zuzAddress).transferFrom(msg.sender, feeRecipient, zuzFee);
        else if( keccak256(abi.encodePacked(_paymentMethod)) == keccak256(abi.encodePacked("usdt")) ) IERC20(usdtAddress).transferFrom(msg.sender, feeRecipient, usdtFee);
        else if( keccak256(abi.encodePacked(_paymentMethod)) == keccak256(abi.encodePacked("usdc")) ) IERC20(usdtAddress).transferFrom(msg.sender, feeRecipient, usdcFee);
        else require(false, "Invalid payment-method");
        _;
    }

    function transferERC20(address _tokenAddress, address[] memory _recipients, uint[] memory _amounts, string memory _paymentMethod) public payable getServiceFee(_paymentMethod) {
    ...
    function transferERC721(address _tokenAddress, address[] memory _recipients, uint[] memory _tokenIds, string memory _paymentMethod) public payable getServiceFee(_paymentMethod) {
    ...
    bytes defaultData0;
    function transferERC1155(address _tokenAddress, address[] memory _recipients, uint[] memory _tokenIds, uint[] memory _amounts, string memory _paymentMethod) public payable getServiceFee(_paymentMethod) {
        require(_recipients.length == _tokenIds.length, "number of recipients and tokenIds should be same");
        require(_recipients.length == _amounts.length, "number of recipients and amounts should be same");
        for(uint i=0; i<_recipients.length; i++) {
