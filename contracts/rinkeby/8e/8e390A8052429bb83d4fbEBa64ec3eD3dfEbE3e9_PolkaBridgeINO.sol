...
contract PolkaBridgeINO is ERC1155, Ownable, ReentrancyGuard {
   ...
   // Check if user IsWhitelist
    ...
    function addPackageToPool(
        ...
        // uint[] storage packageIds = pools[poolIndex].PackageIds;
        // packageIds.push(id);
    ...
    function claimPool(uint256 pid) public nonReentrant {
        ...
        require(block.timestamp > pools[poolIndex].End, "not on time for claiming NFTs");
        uint[] memory packageIds = pools[poolIndex].PackageIds;
        uint256 packagesLength = packageIds.length;
        require(!whitelist[pid][packageIds[0]][msg.sender].IsClaimed, "user already claimed");
        for (uint256 i = 0; i < packagesLength; i++)
        {
            uint256 packageId = packageIds[i];
            uint256 itemCount = whitelist[pid][packageId][msg.sender].PurchasedItemCount;
            polkaBridgeNFT.mintNFT(msg.sender, packageId, itemCount);
            whitelist[pid][packageId][msg.sender].IsClaimed = true;
        }
    }
