    ...
    function receiveApproval(address _tokenOwner, uint256 _amount, address _tokenContract, bytes memory _data) public override {
        ...
        if(mode == 0x10) {
            ...
            if(msg.sender == BXTBContract) {
                ...
                // Normally we would check outstandingCHIP too, but because we assume it
                // can never be greater than outstandingYieldToken, the test is redundant
            ...
            require((_amount <= outstandingCHIP) && (outstandingCHIP > 0), "Cashout size exceeded");
