...
// File: TheGringosNFT.sol

pragma solidity ^0.8.0;

contract TheGringosNFT is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenId;

    uint256 public MAX_SUPPLY = 10000;
    uint256 public PRESALE_RESERVED = 7000;
    uint256 public MAX_ALLOWED = 20;
    uint256 public MAX_ALLOWED_PRESALE = 3;
    uint256 public OWNER_RESERVED = 100;
    uint256 public price = 350000000000000000; //0.35 Ether
    string baseTokenURI;
    ...
    constructor(string memory baseURI) ERC721("The Gringos", "GRINGO") {
        setBaseURI(baseURI);
    }
    ...
    function setPresaleReserved(uint256 _presale_reservef) public onlyOwner {
        PRESALE_RESERVED = _presale_reservef;
    }
    ...
    function presaleMintNFT(uint256 _count) public payable {
        if (msg.sender != owner()) {
            ...
            if(presaleOpen == true && presaleWhiteListRequired == true){
                require(checkPresaleEligiblity(msg.sender) == true, "You are not eligibale for Presale");
            }
        }
        
        require(
            _count > 0 && _count <= MAX_ALLOWED_PRESALE,
