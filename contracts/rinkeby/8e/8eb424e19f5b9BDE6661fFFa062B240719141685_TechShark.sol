...
contract TechShark is ERC721Enumerable, Ownable {
    ...
    mapping (uint256 => SharkTraits[]) public tokenTraits;
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        uint256 randomId = _tokenURIs[tokenId];

        string memory metadata = string(abi.encodePacked(
            '{"name": "TechShark #',
            Strings.toString(randomId),
            '", "description": "TechBay NFT - TechShark.", "image": "ipfs://QmTAPhUdwiyGZSBUyV7JKYMw1uFKDeA8R2yh4wwh6pCuVa/',
            Strings.toString(randomId),
            '.jpg", "attributes":',
    ...
    function compileAttributes(uint256 tokenId) private view returns (string memory) {
        SharkTraits[] memory s = tokenTraits[tokenId];
        string memory traits;
        for (uint256 i = 0; i < s.length; i++) {
            string memory comma = i > 0 ? '' : ',';
