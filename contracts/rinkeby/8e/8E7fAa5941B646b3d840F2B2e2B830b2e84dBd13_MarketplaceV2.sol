...
contract MarketplaceV2 is Ownable {
    ...
    function putOnSale(
        ...
        emit NewPosition(msg.sender, positionsCount, _collection, _id, _amount, _price, _currency);
    ...
    function transferWithFees(uint256 _position, uint256 _amount) internal {
        ...
        if (buyerFee > 0) {
            transfer(marketplaceBeneficiaryAddress, position.currency, buyerFee.mul(2));
        }
        uint256 fees = transferFees(position.nft, position.currency, price).add(buyerFee.mul(2));
        transfer(position.owner, position.currency, price.sub(fees));

        position.amount = position.amount.sub(_amount);
    }
    
    function transfer(address _to, address _currency, uint256 _amount) internal {
    ...
    function transferFees(nftToken memory _nft, address _currency, uint256 _price) internal returns(uint256) {
        ...
        for (uint256 i = 0; i < fees.length; i++) {
            ...
            if (fee > 0) {
                transfer(recipients[i], _currency, fee);
