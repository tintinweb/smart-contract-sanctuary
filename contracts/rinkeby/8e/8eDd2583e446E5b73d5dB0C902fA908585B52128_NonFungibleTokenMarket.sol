...
import { IMarketMaker, INftMarketMaker } from "./IMarketMaker.sol";
 ...
 */
contract NonFungibleTokenMarket is TokenMarket, INftMarketMaker {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    bytes4 constant private ERC721_INTERFACE_ID = 0x80ac58cd; // 721 interface id

    constructor(address currency, uint8 fee) {
    ...
    function createMarket(address tokenContract, uint256 tokenId, Stakeholder[] calldata stakeholders)
      ...
      returns (uint256) {
        ...
        return _createMarket(tokenContract, tokenId, 1, tokenOwner, stakeholders);
    }

    function createBid(uint256 marketId, address currency, uint256 amount)
      external
      override
      payable
      ...
      nonReentrant {
          ...
          if (markets[marketId].ask.amount > 0  && markets[marketId].ask.currency != address(0) && currency == markets[marketId].ask.currency && amount >= markets[marketId].ask.amount) {
            _finalizeTransfer(marketId, currency, msg.sender, amount, 1);
            return;
          }
    ...
    function transferToken(address tokenContract, address owner, address buyer, uint256 tokenId, uint256 tokenCount)
      ...
      override
      returns(bool) {
        require(tokenCount == 1, "Cannot transfer more than 1 ERC721 token");
        try IERC721(tokenContract).safeTransferFrom(owner, buyer, tokenId) {
          return true;
        } catch {
          return false;
        }
...
interface IMarketMaker {
    ...
    function setFee(address currency, uint8 feePercentage)
      external;
...
abstract contract TokenMarket is IMarketMaker, ReentrancyGuard, Ownable {
    ...
    uint8 public ownerFeePercentage;
    ...
    function _createMarket(address tokenContract, uint256 tokenId, uint256 tokenCount, address tokenOwner, Stakeholder[] calldata stakeholders)
      ...
      returns (uint256) {
        ...
        uint8 totalPct = ownerFeePercentage;
        for (uint i = 0; i < stakeholders.length; i++) {
          markets[marketId].stakeholders.push(stakeholders[i]);
          totalPct = SafeCast.toUint8(SafeMath.add(uint256(totalPct), uint256(stakeholders[i].stakePercentage)));
    ...
    function _finalizeTransfer(uint256 marketId, address currency, address buyer, uint256 amount, uint256 tokenCount)
      internal
      {
        ...
        if (currency != feeExemptCurrencyAddress) {
          uint256 cut = SafeMath.div(SafeMath.mul(amount, ownerFeePercentage), 100);
        ...
        for (uint i = 0; i < market.stakeholders.length; i++) {
          uint256 cut = SafeMath.div(SafeMath.mul(amount, market.stakeholders[i].stakePercentage), 100);
    ...
    function _validateIncomingBid(uint256 amount, address currency) internal view {
        IERC20 currencyToken = IERC20(currency);
        require(currencyToken.balanceOf(msg.sender) >= amount, "Caller's token balance is insufficient for bid");
