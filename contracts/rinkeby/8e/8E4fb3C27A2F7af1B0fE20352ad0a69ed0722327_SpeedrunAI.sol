...
contract SpeedrunAI is ERC721Tradable {
	...
	// Supply
	uint256 public saleToSeries = 2; // exclusive
	uint256 public TOTAL_SUPPLY = 0;
	uint256 public MAX_TOKEN_PER_MINT = 5;
		...
		ERC721Tradable('SpeedrunAI', 'SRAI', proxyRegistryAddress)
	{
		...
		_doMint(msg.sender, 10); // Deployer get the first 10
	}
	...
	function setTokenPrice(uint256 newCostPerToken) external onlyOwner {
		PRICE_PER_TOKEN = newCostPerToken;
	}
	...
	function mint(address addr, uint256 amount)
		external
		payable
		seriesSaleOn(TOTAL_SUPPLY + amount - 1)
		notContract
	{
		require(
			msg.value >= amount * PRICE_PER_TOKEN,
			'SpeedrunAI: incorrect amount'
		);
		require(amount <= MAX_TOKEN_PER_MINT, 'SpeedrunAI: too many tokens');
		_doMint(addr, amount);
	}
