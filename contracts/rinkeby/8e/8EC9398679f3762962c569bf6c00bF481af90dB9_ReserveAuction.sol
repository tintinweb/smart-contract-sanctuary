...
contract ReserveAuction is ZoraProxyStorage, ReserveAuctionStorageV1, IReserveAuction, ReentrancyGuard {
    ...
    function configure(address _zora, address _weth) public onlyAdmin {
        ...
        minBid = 1 * 10**17; // 0.1 eth
    }

    function updateZora(address _zora) public override onlyAdmin {
        require(
            IERC165(_zora).supportsInterface(interfaceId),
            "Doesn't support NFT interface"
        );
    ...
    function updateMinBid(uint256 _minBid) public override onlyAdmin {
        minBid = _minBid;
    }

    function updateTimeBuffer(uint256 _timeBuffer) public override onlyAdmin {
    ...
    function createAuction(
        ...
        Decimal.D256 memory curatorFeePercentage,
    ...
    nonReentrant
    {
        ...
        require(
            amount > lastBidAmount && amount.sub(lastBidAmount) > minBid,
            "Must send more than last bid by minBid amount"
        );
        require(
            IMarket(IMediaExtended(zora).marketContract()).isValidBid(
                tokenId,
                amount.sub(_splitBid(auctions[tokenId].curatorFeePercentage, amount))
            ),
    ...
    function endAuction(uint256 tokenId) external override notPaused auctionExists(tokenId) nonReentrant {
        ...
        if(auctions[tokenId].curator != address(0)) {
            curatorFee = _splitBid(auctions[tokenId].curatorFeePercentage, creatorProfit);
    ...
    function _splitBid(Decimal.D256 memory percentage, uint256 amount) private pure returns (uint256) {
        return Decimal.mul(amount, percentage).div(100);
    }
...
import { Decimal } from "@zoralabs/core/dist/contracts/interfaces/IMarket.sol";
...
interface IReserveAuction {
    struct Auction {
        ...
        uint256 reservePrice;
        Decimal.D256 curatorFeePercentage;
        address payable creator;
    ...
    function updateZora(address _zora) external;

    function updateMinBid(uint256 _minBid) external;

    function updateTimeBuffer(uint256 _timeBuffer) external;
    ...
    function createAuction(
        ...
        address payable creator,
        address payable curator,
        Decimal.D256 calldata curatorFeePercentages,
        address auctionCurrency
    ) external;
