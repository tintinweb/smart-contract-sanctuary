...
contract MetaMarauders is ERC721, Ownable, ReentrancyGuard {
    ...
    function mint(uint256 amountToMint) public payable returns (bool) {
        ...
        for (uint256 i; i < amountToMint; i++) {
            _safeMint(msg.sender, supply.add(i));
        }
        return true;
    }

    function airdrop(address recipient, uint256 amountToMint)
        public
        onlyOwner
        returns (bool)
    {
        uint256 supply = totalSupply();
        require(
            amountToMint <= reservedSupplyAllocation,
            "Amount exceeds reserved allocation"
        );
        require(
            mintableSupply >= supply.add(amountToMint),
            "Mintable hardcap reached"
        );
        for (uint256 i; i < amountToMint; i++) {
            _safeMint(recipient, supply.add(i));
        }
        reservedSupplyAllocation.sub(amountToMint);
        return true;
    }
    ...
    function setSaleActiveStatus(bool saleActiveStatus_) public onlyOwner {
        saleIsActive = saleActiveStatus_;
    }

    function setSaleEndingStatus(bool saleEndingStatus_) public onlyOwner {
    ...
    function setPrice(uint256 newPriceSale) public onlyOwner {
