...
contract OneCoinGlobal is Ownable {
    ...
    uint256 constant public shareholder_full_bonus_rate = 10;
    ...
    uint256[] public invest_period_months =    [1,  2,  3,  6,  12, 18];   //month
    uint256[] public invest_period_day_rates = [30, 35, 40, 45, 50, 50];   //Ten thousand of
    ...
    uint256 constant public platform_bonus_rate = 8;
    ...
    */
        ...
        external 
    {
        ...
        require(_eth_amount > address(this).balance, "Insufficient eth balance");
        ...
        if(_eth_amount >= eth_min_withdraw_num){
    ...
    */
        ...
        private
    {
        ...
        total_platform_bonus += (token_amount * platform_bonus_rate / 100);
        ...
        private
    {
        ...
        while(true){
            ...
            if(players[ref].level_id > last_level_id){
                ...
                uint256 last_level_bonus = team_level_bonuses[last_level_id-1];
    ...
    */
        ...
        returns(uint256 value)
    {
        ...
        if(value > 0){
            ...
            if(total_withdrawn >= yield_reduce_section1[0] * invest_token_decimal){
                value = value * (100 - yield_reduce_section1[1]) / 100;
            }
            /* total yield reached 10,000,000,start second reduce */
            if(total_withdrawn >= yield_reduce_section2[0] * invest_token_decimal){
                value = value * (100 - yield_reduce_section2[1]) / 100;
            }
            /* total yield reached 15,000,000,start third reduce */
            if(total_withdrawn >= yield_reduce_section3[0] * invest_token_decimal){
        ...
        returns(uint256 value)
    {
        ...
        for(uint256 i = 0; i < player.deposits.length; i++) {
            ...
            if(from < to && dep.status == 0) {
                ...
                uint256 _day_eth_payout = (token_amount * invest_reward_eth_month_rate / 100 / 30) * (invest_coin_usd_price * (10**8)) / eth_usd_price ;
