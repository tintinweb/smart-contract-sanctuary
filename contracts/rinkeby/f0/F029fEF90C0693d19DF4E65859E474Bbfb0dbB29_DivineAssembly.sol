...
contract DivineAssembly is ERC721Enumerable, AccessControl, Ownable, PaymentSplitter {
    ...
    uint256 public totalGivewayClaimed = 0;
    uint256 public whitelistSalePrice = 0.08 ether;
    ...
    mapping(address => uint256) public givewaysClaimable;
    ...
    */
    constructor(string memory name, string memory symbol) ERC721(name, symbol) PaymentSplitter(_team, _teamShares) {
    ...
    /**
    @dev Grant giveway role for given addresses
     */
    function addAddressesForGiveway(address[] calldata addresses, uint256 _nb)
        external
        onlyAdmin
    {
        for (uint32 i = 0; i < addresses.length; i++) {
            givewaysClaimable[addresses[i]] = _nb;
        }
    }
    ...
    @dev Remove given addresses from the whitelist role
     */
    function removeAddressesFromGiveway(address[] calldata addresses)
        external
        onlyAdmin
    {
        for (uint32 i = 0; i < addresses.length; i++) {
            givewaysClaimable[addresses[i]] = 0;
        }
    }
    ...
    /**
    @dev Mint for admins to aidrop NFTs on specific address
     */
    function adminMint(address _to, uint256 _nb) external onlyAdmin {
        require(totalSupply().add(_nb) <= MAX_SUPPLY, "Not enough tokens left.");
        ...
        emit AirDrop(_to, block.timestamp, _tokenIdsMinted);
    }
    ...
    /**
    @dev Claiming a giveway
     */
    function claimGiveway(uint256 _nb)
        external
    {
        require(totalSupply().add(_nb) <= MAX_SUPPLY, "Not enough tokens left.");
        require(
            givewaysClaimed[msg.sender].add(_nb) <= givewaysClaimable[msg.sender],
            "Limit exceeded."
        );
        ...
        totalGivewayClaimed = totalGivewayClaimed.add(_nb);
        givewaysClaimed[msg.sender] = givewaysClaimed[msg.sender].add(_nb);
        emit GivewayClaim(msg.sender, block.timestamp, _tokenIdsMinted);
    }
