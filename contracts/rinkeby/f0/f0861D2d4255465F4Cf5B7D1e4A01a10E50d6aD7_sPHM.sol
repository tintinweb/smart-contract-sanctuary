...
contract sPHM is PhantomStorageMixin, IsPHM, ERC20 {
    ...
    /**
     * @dev Perform a rebase. In reality, this calculates the new APY and updates the _scalingFactor.
     * This needs to be called by the contract responsible for triggering regases.
     */
    function doRebase(uint256 epochNumber) external onlyRegisteredContracts {
        uint256 rewardYield = PHM().totalSupply().mul(_rewardRate).div(
            totalSupply() + gPHM().totalSupply().mul(scalingFactor())
        );
        _apy = (ONE_18D + rewardYield).powu(periodsPerYear());
        if (_apy == 0) {
            _apr = 0; // prevent division by 0
        } else {
            uint256 N = periodsPerYear().fromUint();
            _apr = N.mul((_apy + ONE_18D).pow((ONE_18D).div(N)) - ONE_18D);
        }
        _scalingFactorBase = scalingFactor();
        _timestampOfLastRebase = block.timestamp;

        emit Phantom_Rebase(epochNumber, rewardYield, _scalingFactorBase);
    ...
    function scalingFactorBase() external view returns (uint256) {
