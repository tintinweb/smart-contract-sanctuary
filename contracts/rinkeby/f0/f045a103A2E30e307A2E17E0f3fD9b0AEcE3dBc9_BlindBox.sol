...
contract BlindBox is NonGenerativeBB {
    ...
    */
    ...
    @param series this is struct containing all details of new series to be started
    ...
    */
    function StartSeries(Series memory series, bool isGenerative ) onlyOwner public {
        if(isGenerative){
            // start generative series
            generativeSeriesId.increment();
            generativeSeries(series.collection, series.name, series.seriesURI, series.boxName, series.boxURI, series.startTime, series.endTime);
            emit SeriesInputValue(series,generativeSeriesId.current(), isGenerative );

        } else {
            nonGenerativeSeriesId.increment();
            // start non-generative series
            nonGenerativeSeries(series.collection, series.name, series.seriesURI, series.boxName, series.boxURI, series.startTime, series.endTime);
            emit SeriesInputValue(series,nonGenerativeSeriesId.current(), isGenerative );
        }
        extraPsrams(series, isGenerative);
        
    }
    function extraPsrams(Series memory series, bool isGenerative) internal {
        if(isGenerative){
      setExtraParamsGen(series.baseCurrency, series.allowedCurrencies, series.bankAddress, series.perBoxPrice, series.maxBoxes, series.perBoxNftMint);  

        } else {
      setExtraParams(series.baseCurrency, series.allowedCurrencies, series.bankAddress, series.perBoxPrice, series.maxBoxes, series.perBoxNftMint);  
    ...
    */
    ...
    event SeriesInputValue(Series _series, uint256 seriesId, bool isGenerative);
}
...
contract Utils is Ownable, BlindboxStorage{
    ...
    function init() public {
        ALIA = IERC20(0x8D8108A9cFA5a669300074A602f36AF3252B7533);
         MATIC = IERC20(0x094616F0BdFB0b526bD735Bf66Eca0Ad254ca81F); //for eth chain wrapped ethereum
        USD = IERC20(0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee);
        platform = 0xF0d2D73d09A04036F7587C16518f67cE622129Fd;
        nft = INFT(0x326Cae76A11d85b5c76E9Eb81346eFa5e4ea7593);
        dex = IDEX(0xC992d6755fe9b68271C8814Ea043AAf8Feee2A24);
...
/** 
    ...
    */
    function nonGenerativeSeries(address bCollection,string memory name, string memory seriesURI, string memory boxName, string memory boxURI, uint256 startTime, uint256 endTime) onlyOwner internal {
    ...
    function setExtraParams(uint256 _baseCurrency, uint256[] memory allowedCurrecny, address _bankAddress, uint256 boxPrice, uint256 maxBoxes, uint256 perBoxNftMint) internal {
        ...
        _CopiesData[nonGenerativeSeriesId.current()].total = maxBoxes;
...
/** 
    ...
    */
    function buyNonGenBox(uint256 seriesId, uint256 currencyType) validateCurrencyType(seriesId,currencyType, false) internal {
        ...
        emit BuyBoxNonGen(_boxId.current(), seriesId, price, nonGenSeries[seriesId].price);
    ...
    */
    function _openNonGenBox(uint256 boxId) private {
        ...
        for (uint256 j = 0; j < boxesNonGen[boxId].countNFTs; j++) {
          ...
          dex.mintWithCollection(collection, msg.sender, nonGenSeries[sId].uris[rand1].uri );
...
/** 
    ...
    */
    ...
    event BuyBoxNonGen(uint256 boxId, uint256 seriesId, uint256 paidPrice, uint256 orignalPrice);
...
interface IDEX {
   ...
   function mintWithCollection(address collection, address to, string memory tokenURI ) external;
...
/** 
    ...
    */
    function generativeSeries(address bCollection, string memory name, string memory seriesURI, string memory boxName, string memory boxURI, uint256 startTime, uint256 endTime) onlyOwner internal {
        require(startTime < endTime, "invalid series endTime");
    ...
    function mintGenerativeNFTs(address collection, uint256 boxId, string[] memory uris) public onlyOwner {
        ...
        require(boxesGen[boxId].countNFTs == uris.length, "insufficient URIs to mint");
         for (uint256 i = 0; i < uris.length; i++) {
            dex.mintWithCollection(collection, genBoxOwner[boxId], uris[i]);
         }
