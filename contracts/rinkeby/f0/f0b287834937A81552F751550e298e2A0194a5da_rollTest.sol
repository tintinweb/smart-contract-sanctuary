...
contract rollTest is ERC721, ERC721Enumerable, ReentrancyGuard, Ownable{
    
    uint256 public lootOwnerPrice = 15000000000000000; //0.015 ETH
    uint256 public publicPrice = 30000000000000000; //0.03 ETH
    address public lootAddress = 0xB52bac7B6BEdEaB22AC91d1a6621e11C8E23B2BE; //REMEMBER TO CHANGE
    ...
    constructor(string memory baseUri) ERC721("tubaaa", "tubeee") { ///REMEMBER TO CHANGE
        baseURI = baseUri;
    }
    ...
    function getResourceNode(uint256 tokenId, uint256 resourceIndex) public view returns (string memory){
        ...
        if (roll <= 5) {
            output = string(abi.encodePacked("Rich ", output)); // 5% chance 
        }

        if (roll > 5 && roll <= 15) {
            output = string(abi.encodePacked("Abundant ", output)); // 10% chance
        }

        if (roll > 60) {
            output = string(abi.encodePacked("Common ", output)); // 40% chance
        }
        
        if (roll > 30 && roll <= 60) {
            output = string(abi.encodePacked("Poor ", output)); // 30% chance
        }

        if (roll > 15 && roll <= 30) {
            output = string(abi.encodePacked("Scarce ", output)); //15% chance
        }
