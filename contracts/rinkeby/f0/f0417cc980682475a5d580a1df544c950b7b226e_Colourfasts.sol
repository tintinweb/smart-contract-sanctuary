...
// File: contracts/Colourfasts.sol
...
contract Colourfasts is ERC721Enumerable, ReentrancyGuard, Ownable {
    constructor() ERC721("Colourfasts", "[c]") {}
    mapping(bytes10 => bool) private hashToMinted;
    mapping(uint8 => bytes10) private tokenIdToDNA;
    ...
    function changeColor(bytes10 _rgb, uint position, uint8 x) internal pure returns (bytes1) {
        // if (x == 0) {
        //   return ~getColor(_rgb, position);
        // }
        // if (x % 2 == 0) {
        //   return getColor(_rgb, position) | getColor(_rgb, 1);
        // }
        // return getColor(_rgb, position) << x;
        if (position == 0) {
          bytes1 a = subtractBitwise(getColor(_rgb, 1), getColor(_rgb, 0));
          return subtractBitwise(a, bytes1(x));
        }
        if (position == 1) {
          bytes1 b = subtractBitwise(getColor(_rgb, 2), getColor(_rgb, 1));
          return subtractBitwise(b, bytes1(x));
          // if (x % 2 == 0) {
          //   return subtractBitwise(b, bytes1(x)) << x;
          // } else {
          //   return subtractBitwise(b, bytes1(x)); // do some logic to make bit shifting happen only occasionally
          // }
          
          
        }
        bytes1 c = subtractBitwise(getColor(_rgb, 0), getColor(_rgb, position));
        return ~subtractBitwise(c, bytes1(x));
    }
    ...
    function whitelistClaim() public nonReentrant {
        ...
        uint8 index = uint8(totalSupply());
        require(index >= 51 && index < 512, "All have been minted");
        _internalMint(index);
    ...
    function _internalMint(uint8 tokenId) private {
        tokenIdToDNA[tokenId] = generateHash(tokenId);
        _safeMint(_msgSender(), tokenId);
    }

    // Takes in a tokenId and a "max" number as the ceiling to randomly pull
    function genRandomNum(uint8 tokenId, uint8 max) internal returns (bytes1) {
      return bytes1(randMod(tokenId, max));
    }

    function getAttributeAtPos(uint8 tokenId, uint8 pos) internal view returns (uint8) {
      return uint8(tokenIdToDNA[tokenId][pos]);
    }

    function generateHash(uint8 tokenId) internal returns (bytes10) {
      // bytes7 scheme is r/g/b/background/filter/pattern/transform/shape
      // like this: 0xd0714c04020301
      // where d0 = red, 71 = green, 4c = b, 04 = background, 02 = filter, etc
        bytes10 dna = bytes10(
          abi.encodePacked(
            ...
            genRandomNum(tokenId, 3),   // filter index 4
            genRandomNum(tokenId, 8),   // pattern index 5
            genRandomNum(tokenId, 4),   // transform index 6
        ...
        bytes5 dnaMinusColor = bytes5(
          abi.encodePacked(
            dna[3], dna[4], dna[5], dna[6], dna[7]            
          )
        );
        // No dupes
    ...
    function ownerClaim() public nonReentrant onlyOwner {
        uint8 index = uint8(totalSupply());
        require(index >= 0 && index < 51, "Choose an unclaimed index between 0 and 51, inclusive");
        for (uint8 i = index; i < index + 5; i++) {
          _internalMint(i);
        }
    }

    function getAttributeTitleValues(uint8 tokenId, uint8 pos) view internal returns (string memory) {
      if (pos == 3) {
        return [
          'Solid',
          'Radial Gradient',
    ...
    function getAttributes(uint8 tokenId) view internal returns (string memory) {
      return string(
        abi.encodePacked(
    ...
    function getDNASeed(bytes10 DNA, uint8 tokenId) public pure returns (uint64) {
        return uint64(uint256(keccak256(abi.encodePacked(
            DNA,
            tokenId
        ))));
    }    
    function makeArt(uint8 tokenId) internal view returns (string memory) {
        string memory o;
        // uint colorIndex = 0;
        ...
        if (uint8(DNA[5]) > 6) {
          ...
          for (uint8 y = 0; y < 8; y++) {
            for (uint8 x = 0; x < 8; x++) {
              ...
              o = string(abi.encodePacked(
                  o,
                  createSquare(DNA, x, y, toString(v))
              ));
            }
          }
        }
    ...
    function createSquare(bytes10 DNA, uint8 x, uint8 y, string memory delay) pure internal returns (string memory) {
    return string(
        abi.encodePacked(
          '<use class="u',toString(uint8(x)),'" href="#r" x="', toString(uint8(x) * 32),
          '" y="', toString(uint8(y) * 32), '" fill="rgb(',
          toString(uint8(changeColor(DNA, 0, x))), ',',
          toString(uint8(changeColor(DNA, 1, y))), ',',
    ...
    function buildShape(uint8 tokenId) internal view returns (string memory) {
      string[2] memory shapes = [
        '<rect id="r" height="32" width="32" />',
        '<circle id="r" cx="16" cy="16" height="32" width="32" r="8" />'
      ];
      return shapes[getAttributeAtPos(tokenId, 7)];
    }

    function buildAnimationDuration(uint8 tokenId, uint8 pos) internal view returns (uint16) {
    ...
    function buildBackground(uint8 tokenId) internal view returns (string memory) {
    ...
    function invertColors(uint8 tokenId) internal view returns (string[2] memory) {
    ...
    function buildAnimation(uint8 tokenId) internal view returns (string memory) {
      string[2][3] memory filters = [
        ['hue-rotate(0deg)', 'hue-rotate(180deg)'],
        ['saturate(50%) invert(60%)', 'saturate(180%) invert(90%)'],
        ['sepia(0%)', 'sepia(80%)']
      ];
      string[2][4] memory transforms = [
        ['scale(1)', 'scale(1)'],
        ['scale(1)', 'scale(0.8)'],
        ['scale(1)', 'scale(1.6)'],
      ...
      string memory o = string(
        abi.encodePacked(
          ...
          ';}75%{filter: initial; transform: scale(1);}}'
        )
      );
    ...
    function generateStyle(uint8 tokenId) internal view returns (string memory) {
      string[2] memory bgColors = invertColors(tokenId);
      return string(abi.encodePacked(
        '<style>.s1{--a:rgb(', bgColors[0],
        ');--b:rgb(', bgColors[1], ');}.u0,.u1,.u2,.u3,.u4,.u5,.u6,.u7{animation:',toString(buildAnimationDuration(tokenId, 8)) ,'ms infinite alternate a,',toString(buildAnimationDuration(tokenId, 9)),'ms infinite alternate b;transform-origin:50% 50%;}',
        buildAnimation(tokenId),
        '@keyframes b{from{opacity: 1;}to {opacity: 0.5;}}',
        tokenId % 2 == 0 ? '.u0{animation-delay:1s;}.u1{animation-delay:2s;}.u2{animation-delay:3s;}.u3{animation-delay:4s;}.u4{animation-delay:5s;}.u5{animation-delay:6s;}.u6{animation-delay:7s;}.u7{animation-delay:8s;}' : '',
        '</style>'
      ));
    }

    function generateSvg(uint8 tokenId) internal view returns (string memory) {
        // string memory art = makeArt(tokenId);
        // string[2] memory bgColors = invertColors(tokenId);
        return string(abi.encodePacked(
    ...
    function randMod(uint8 tokenId, uint8 _modulo) internal returns(uint8) {
          // increase nonce
          randNonce++; 
