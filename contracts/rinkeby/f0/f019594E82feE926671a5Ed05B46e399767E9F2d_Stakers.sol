...
contract Stakers is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function random(address ms1) private view returns (uint8) {
  return uint8(uint256(keccak256(abi.encodePacked(block.timestamp, ms1)))%3);
    }
    ...
    function mintChip(uint256 chipQuantity) public payable {
        ...
        for(uint256 i; i < chipQuantity; i++){
            ...
            if(random(msg.sender) == 2){
                skinTone[supply + i] = "c";
            }
             else if(random(msg.sender) == 1){
                skinTone[supply + i] = "b";
            }
             else{
                 skinTone[supply + i] = "a";
