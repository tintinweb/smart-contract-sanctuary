...
contract stakingContract is OwnableUpgradeable, IERC721ReceiverUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function stakeMany(address account, uint16[] calldata tokenIds) external blockExternalContracts whenNotPaused nonReentrant() {   
    require(account == _msgSender() || _msgSender() == address(erc721Contract) || _msgSender() == address(PaymentContract), "DONT GIVE YOUR TOKENS AWAY");  
     


    for (uint i = 0; i < tokenIds.length; i++) {
      if (_msgSender() != address(PaymentContract)) {

        require(erc721Contract.ownerOf(tokenIds[i]) == _msgSender(), "AINT YO TOKEN");
        erc721Contract.transferFrom(_msgSender(), address(this), tokenIds[i]);  //safeTransferFrom
  ...
  function claimFromStaking(uint16[] calldata tokenIds, bool unstake, bool payKings) external blockExternalContracts whenNotPaused _updateEarnings nonReentrant() {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      if (isHead(tokenIds[i]))
        owed += _claimHeads(tokenIds[i], unstake, payKings);
      else
        owed += _claimHunters(tokenIds[i], unstake);
    }

    erc20Contract.updateOriginAccess();

    if (owed == 0) return;
    erc20Contract.mint(_msgSender(), owed);
  ...
  function _claimHeads(uint256 tokenId, bool unstake, bool payKings) private returns (uint256 owed) {
    ...
    if (unstake) {
      ...
      uint256 lastSeed = PaymentContract.last_seed();
      uint256 seed = uint256(keccak256(abi.encodePacked(tx.origin, blockhash(block.number - 1), block.timestamp, lastSeed))) + unstakeCounter[tokenId];
      ...
      if (payKings) {
        require(owed > headKingTax,"Not Enough to pay the Kings Yet");
        owed = owed - headKingTax;
        _payHuntersTax(75*headKingTax/100);    

      } else {
        _payHuntersTax(owed * headTax / 100);    
      }
  ...
  // function _alphaForHunter(uint256 tokenId) private view returns (uint8) {

  //   IMint.Traits memory yo = erc721Contract.getTokenTraits(tokenId);
  //   return maxAlpha - yo.alphaIndex; 
  // }
