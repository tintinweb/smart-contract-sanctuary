...
/**
 * @dev Place – Represents place information for a geographic location.
 *
...
/**
 * @dev Location – Represents a geographic coordinate with altitude.
 *
    ...
    Ownable
{
    ...
    event ProviderUpdated(IPlacesProvider provider);

    event PlaceCreated(uint256 tokenId);
    event PlaceBurned(uint256 tokenId);

    event GroundersGiftingUpdated(bool isEnabled);
    event MintFeeUpdated(uint256 mintFee);

    /**
     * @notice Location integer resolution allowing for 14 places of decimal storage and enable on-chain computation.
     * @dev IPlaceDrop.Place.Location returns both integer and string representations for use.
     *
     *      MAX_LATITUDE_INT = 9000000000000000;
     *      MIN_LATITUDE_INT = -9000000000000000;
     *      MAX_LONGITUDE_INT = 18000000000000000;
     *      MIN_LONGITUDE_INT = -18000000000000000;
     *
     */
    int256 private constant GEO_RESOLUTION_INT = 100000000000000;

    /**
     * @notice True when random gifting to the grounders is enabled.
     */
    bool public isGroundersGiftingEnabled;

    /**
     * @notice Places DAO treasury.
     */
    address payable public placesDAO;

    /**
     * @notice Grounders address for gifts.
     */
    address public grounders;

    IPlacesProvider private placesProvider;
    IPlacesDescriptor private placesDescriptor;
    ...
    modifier onlyGrounders() {
        require(
            (_msgSender() == grounders || _msgSender() == owner()),
            "Not a grounder"
        );
        _;
    }
     ...
     */
    function setPlacesDAO(address payable _placesDAO) external onlyGrounders {
        placesDAO = _placesDAO;
        emit PlacesDAOUpdated(_placesDAO);
    }
     ...
     * @dev Only callable by the grounders.
     */
    function setProvider(IPlacesProvider _placesProvider)
        external
        onlyGrounders
    {
        ...
        _randomGrounderOffset =
            ...
            10;
        emit ProviderUpdated(_placesProvider);
    }
     ...
     * @dev Only callable by the grounders.
     */
    function setDescriptor(IPlacesDescriptor _placesDescriptor)
        external
        onlyGrounders
    {
        placesDescriptor = _placesDescriptor;
        emit DescriptorUpdated(_placesDescriptor);
    }
     ...
     * @dev Only callable by the grounders.
     */
    function setPaused(bool _paused) external onlyGrounders {
        if (_paused) {
            _pause();
        } else {
            _unpause();
     ...
     * @dev Only callable by the grounders.
     */
    function setCounter(uint256 countValue) external onlyGrounders {
        _tokenIdCounter.reset();
        while (countValue < _tokenIdCounter.current()) {
     ...
     * @dev Only callable by the grounders.
     */
    function setGroundersGifting(bool _isGroundersGiftingEnabled)
        external
        onlyGrounders
    {
        isGroundersGiftingEnabled = _isGroundersGiftingEnabled;
        emit GroundersGiftingUpdated(_isGroundersGiftingEnabled);
    }
     ...
     * @dev Only callable by the grounders.
     */
    function setMintFee(uint256 _mintFee) external onlyGrounders {
        mintFee = _mintFee;
    ...
    constructor(
        address payable _placesDAO,
        address _grounders,
        IPlacesDescriptor _placesDescriptor,
        IPlacesProvider _placesProvider,
        address _proxyAddress
    ) ERC721("Places", "PLACES") {
        placesDAO = _placesDAO;
        grounders = _grounders;
        placesDescriptor = _placesDescriptor;
        placesProvider = _placesProvider;
        proxyRegistry = IProxyRegistry(_proxyAddress);

        isGroundersGiftingEnabled = true;
        mintFee = 0;
    ...
    function getLocation(uint256 tokenId)
        public
        view
        returns (IPlacesDrop.Location memory)
    {
        require(_exists(tokenId), "Invalid token");
        return placesProvider.getPlace(tokenId).location;
    }
    ...
    {
        require(_exists(tokenId), "Invalid token");
        return placesProvider.getPlace(tokenId);
    }
    ...
    function mint()
        ...
        returns (uint256)
    {
        require(msg.value >= mintFee, "Fee required");
        require(
            !_exists(_tokenIdCounter.current()) &&
                _tokenIdCounter.current() < placesProvider.getPlaceSupply(),
            "Invalid token"
        );
        ...
        if (mintFee > 0) {
            address payable neighborhoodTreasury = placesProvider.getTreasury(
            ...
            bool sentPlacesDAO = placesDAO.send(daoAmount);
        ...
        returns (string memory)
    {
        ...
        return
            placesDescriptor.constructTokenURI(
                tokenId,
                placesProvider.getPlace(tokenId)
            );
    }
