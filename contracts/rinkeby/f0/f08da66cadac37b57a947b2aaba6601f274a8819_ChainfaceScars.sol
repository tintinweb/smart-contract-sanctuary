...
contract ChainfaceScars {
    ...
    function addScars(uint256 tokenId) public view returns (string memory scarSVG) {
        ...
        for(uint i = 0; i < numberOfScars; i++) {
            ...
            (scale, xShift, yShift, rotate) = scarSalt(uint256(keccak256(abi.encodePacked(tokenId,i))));

            scarSVG = string(abi.encodePacked(scarSVG,scarPlacement1,toString(scale)));
    ...
    function scarSalt(uint256 seed) public pure returns (uint256 scale, uint256 xShift, uint256 yShift, uint256 rotate) {
        scale = 1;
        xShift =  uint256(keccak256(abi.encodePacked(seed)))%350;
        yShift =  uint256(keccak256(abi.encodePacked(seed)))%375;
        rotate = uint256(keccak256(abi.encodePacked(seed)))%30;
    }
