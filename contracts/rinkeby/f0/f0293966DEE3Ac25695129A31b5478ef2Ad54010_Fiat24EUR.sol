...
contract Fiat24Account is ERC721PausableUpgradeable, AccessControlUpgradeable {
  ...
  function mint(address _to, uint256 _tokenId, bool _isMerchant, uint256 _merchantRate) public {
     ...
     require(_mintAllowed(_to, _tokenId), "mint not allowed");
      ...
      _tokenOwnersHistoric[_to] = _tokenId;
      isMerchant[_tokenId] = _isMerchant;
      if(_isMerchant) {
        nickNames[_tokenId] = "Merchant";
        if(_merchantRate == 0) {
  ...
  function mintByF24(uint256 _tokenId, bool _isMerchant) public {
  ...
  function _mintAllowed(address to, uint256 tokenId) internal view returns(bool){
      //return !(this.balanceOf(to) >= 1) && !_tokenOwnersHistoric.contains(to);
  ...
  function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override {
      ...
      if(AddressUpgradeable.isContract(to) && (from != address(0))) {
          ...
          //require(!_tokenOwnersHistoric.contains(to) || (_tokenOwnersHistoric.get(to) == tokenId), "Receiver has already account");
...
contract Fiat24Token is ERC20PausableUpgradeable, AccessControlUpgradeable {
  ...
  function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
      ...
      if(recipient == fiat24account.ownerOf(9102)) {
        ...
        if(fiat24account.balanceOf(recipient) > 0 && fiat24account.isMerchant(fiat24account.tokenOfOwnerByIndex(recipient, 0))) {
            uint256 commissionFee = amount.mul(fiat24account.merchantRate(fiat24account.tokenOfOwnerByIndex(recipient, 0))).div(10000);
  ...
  function tokenTransferAllowed(address from, address to, uint256 amount) public view returns(bool){
      ...
      if(from != address(0) && to != address(0)){
        ...
        if(fromHasMembership) {
            fromClientStatus = fiat24account.status(fiat24account.tokenOfOwnerByIndex(from, 0));
        }
        if(toHasMembership) {
            toClientStatus = fiat24account.status(fiat24account.tokenOfOwnerByIndex(to, 0));
        }
        ...
        if(toAmount > WalkinLimit) {
            if((fromHasMembership && toHasMembership) &&
                ...
                if(fromHasMembership &&
                    (fromClientStatus == Fiat24Account.Status.Live || toClientStatus == Fiat24Account.Status.SoftBlocked) &&
                    (amount <= WalkinLimit) ) {
                    return true;
                }
            }
