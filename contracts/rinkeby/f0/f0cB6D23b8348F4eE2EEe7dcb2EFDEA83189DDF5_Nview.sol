...
contract Nview is NPassCore {
  ...
  function getCirclesSVG(uint[8] memory numbers, string memory color) public pure returns(string memory){
    ...
    for(uint i = 0; i < numbers.length; i++){
      for(uint j = 0; j < numbers[i]; j++){
        ...
        // TODO fix correct distance and circle size
        (x_pos, y_pos) = getCirclePos(i, j);
        // TODO refine smoothness circle size changing based on value
        radius_pr = pseudoRandomRadius(5, numbers[i], i, j);
        // TODO circle opacity changing based on value
        // put color function into getcirclesSVG and add parameter for calc

        string memory currentNumberSVG = getCircle(x_pos, y_pos , color, radius_pr);
  ...
  // calculate the positions using the precalculated trig values rounded up for int use
  function getCirclePos(uint index, uint factor) public pure returns(uint,uint){
    /* var xCircle = middle_c + cos(angle * i) * j;
      var yCircle = middle_c - sin(angle * i) * j; */
    ...
    int base = 350; // center
    int res_x = base + (cos_values[index] * int(factor)) / int(3) ;
    int res_y = base - (sin_values[index] * int(factor)) / int(3) ;
    return (uint(res_x), uint(res_y));
  }

  function pseudoRandomRadius(uint radius_base, uint numberi, uint i , uint j) public pure returns (uint){
    return (uint(keccak256(abi.encodePacked(numberi, i, j)))) % 15 + radius_base;
  }

  function getCircle(uint x, uint y, string memory color, uint radius) public pure returns(string memory) {
    return 
      string(
        abi.encodePacked(
          "<circle cx='",
