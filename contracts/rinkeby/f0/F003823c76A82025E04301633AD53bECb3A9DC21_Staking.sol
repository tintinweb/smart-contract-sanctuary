...
contract Staking is Extract,Withdraw{
    ...
    function computStakingProfit(uint _id,uint _time)  public view returns (uint profit){
        ...
        for(uint i=0;i<=passMonths;i++){
            ...
            num = num/getDaysInMonth(computMonth,computYear)/decimals/decimals;
            if(i==passMonths){
    ...
    /* function extractionProfit(uint _cycleId,uint _depositId,uint _timestamp) external isExpire(_depositId) existCycle(_cycleId){
        if(getYmd(_timestamp) >= signDeposit[_depositId].endTime){
            ...
            uint curMonth;
            uint curYear;
            if(curDay < 2 && curMonth < 2){
                curYear = curTime/10000+1;
                curYear = curYear*10000;
                curMonth = 12;
                curDay = getDaysInMonth(12,curYear);
            }else if(curDay < 2){
                curYear = curTime/10000;
                curMonth = curTime/100%100 - 1;
                curDay = getDaysInMonth(curMonth,curYear);
            }
            signDeposit[_depositId].lastExtractTime = curYear*10000+curMonth*100+curDay;
            DepositList[_depositId].lastExtractTime =  curYear*10000+curMonth*100+curDay;
        }
