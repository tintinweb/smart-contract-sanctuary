...
contract Polkadog is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    uint256 private constant numTokensSellToAddToLiquidity = 100 * 10**18;
    uint256 public maxTokensToSell = 10 * 10**18;
    uint256 public transactionLimit = 50000 * 10**18;
    uint256 public rewardLimit = 1000 * 10**18;
    ...
    constructor (address[] memory marketing_address_) {
     ...
     * See {ERC20-_burn}.
     */
    function burn(uint256 amount) external onlyOwner virtual returns (bool) {
    ...
    function migrateMint(address to, uint amount) external virtual returns (bool) {
        require(msg.sender == migrator_admin, 'only migrator admin');
        require(to != address(0), "Address cant be zero address");
        require(amount > 0, "Transfer amount must be greater than zero");
        _mint(to, amount);
        return true;
    }
    ...
    function setMarketingAddress(address[] memory account) external onlyOwner() {
    ...
    function transferToFeeWallet() public {
        _marketingFeeBalance = 0;
        swapTokensForEth(_marketingFeeBalance, 0x48AC0A7Ac7A488eEAC4D3E1e46c882033971D02B);
    ...
    function getAddressIndex() public returns(uint8) {
        if(marketing_address_index == marketing_address.length - 1) {
            marketing_address_index = 0;
        } else {
            marketing_address_index += 1; 
        }
        return marketing_address_index;
    }

    function getAddress(uint8 index) view public returns(address) {
        return marketing_address[marketing_address_index];
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) internal {
        ...
        if(_takeBothTax) {
            ...
            // take high fee - need to implement
            afterLimitFee();
