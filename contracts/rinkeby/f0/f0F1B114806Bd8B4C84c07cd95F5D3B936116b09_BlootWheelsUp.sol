...
contract BlootWheelsUp is ERC721Enumerable, ReentrancyGuard, Ownable {
    bool private toggle = false;
    uint256 public limit = 8888;
    uint256 public constant claimLimitPerUser = 2;
    mapping(address => uint256) public claimedCount;
    address public constant nft1 = address(0xb2c02e550ad5845831D03aA38155FeE26931d33E);
    address public constant nft2 = address(0xFF9C1b15B16263C61d017ee9F65C50e4AE0113D7);
    address public constant nft3 = address(0x4F8730E0b32B04beaa5757e5aea3aeF970E5B613);
    uint256 public nft2Mintable = 1000;
    uint256 public nft3Mintable = 1000;
    uint256 public ownerMintable = 137;
    uint256 public freeMintUnlock;
    ...
    string[] private heights = ["14", "16", "18", "20", "22", "24", "26", "28", "30", "32"];

    string[] private directions = ["true", "false"];

    // string[] private blends = ["normal", "multiply", "difference", "darken", "hue", "luminosity", "color", "soft-light"];

    string[] private patterns = ['<polygon points="0,0 2,5 0,10 5,8 10,10 8,5 10,0 5,2" fill="#FFA500" opacity="0.5" />', '<polygon points="0,5 5,0 10,5 10,10 5,5 0,10" fill="#FFA500" opacity="0.5"/>', '<circle id="pattern-circle" cx="5" cy="5" r="5" fill="#FFA500" opacity="0.5"/>', '<polygon points="0,0 10,10 0,10" fill="#FFA500" opacity="0.5"/>', '<rect x="0" width="5" height="5" y="0" fill="#FFA500" opacity="0.5"></rect><rect x="5" width="5" height="5" y="5" fill="#FFA500" opacity="0.5"></rect>', '<g id="cube"><polygon points="0,0 5,5 5,10 0,5" fill="#FFA500" opacity="0.5" /><polygon points="5,5 10,0 10,5 5,10" fill="#FFDB99" opacity="0.5" /></g><use x="0" y="0" xlink:href="#cube" />', '<polygon points="0,5 5,0 10,5 5,10" fill="#FFA500" opacity="0.5" />', '<polygon points="-1,9 9,-1 11,1 1,11" fill="#FFA500" opacity="0.5" />'];
    ...
    function getHeight(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "HEIGHT", heights);
    }

    function getDirection(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "Direction", directions);
    }

    // function getBlend(uint256 tokenId) public view returns (string memory) {
    //     return pluck(tokenId, "Blend", blends);
    // }

    function getPattern(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "Pattern", patterns);
    }

    function getWave(uint256 tokenId) public view returns (string memory) {
        string memory wave = '<defs>';
        uint256 rand = random(string(abi.encodePacked("p", toString(tokenId)))) % 4;
        string memory fill1 = rand == 0 ? 'url(#wave-pattern)' : string(abi.encodePacked('rgba(', getColor(tokenId), ',0.7)'));
        string memory fill2 = rand == 1 ? 'url(#wave-pattern)' : string(abi.encodePacked('rgba(', getColor(tokenId * 10), ',0.5)'));
        string memory fill3 = rand == 2 ? 'url(#wave-pattern)' : string(abi.encodePacked('rgba(', getColor(tokenId * 20), ',0.3)'));
        string memory fill4 = rand == 3 ? 'url(#wave-pattern)' : string(abi.encodePacked('rgba(', getColor(tokenId * 30), ',0.4)'));
        string memory pattern = getPattern(tokenId);

        for(uint j = 1; j < 5; j++)
        {
            string memory h1 = getHeight(tokenId * j * 10);
            string memory h2 = getHeight(tokenId * j);
            wave = string(abi.encodePacked(wave, '<path id="gentle-wave', toString(j), '" d="M-300 250c30 0 58-', h1, ' 88-', h1, 's 58 ', h1, ' 88 ', h1 ));
            wave = string(abi.encodePacked( wave, 's 58-', h2, ' 88-', h2, 's 58 ', h2, ' 88 ', h2 ));

            for(uint i = 0; i< 2; i++)
            {
                wave = string(abi.encodePacked( wave, 's 58-', h1, ' 88-', h1, 's 58 ', h1, ' 88 ', h1 ));
                wave = string(abi.encodePacked( wave, 's 58-', h2, ' 88-', h2, 's 58 ', h2, ' 88 ', h2 )); 
            }
            
            wave = string(abi.encodePacked(wave, ' v', toString(101 - j * 2) ,'h-1050z"/>'));
        }

        wave = string(abi.encodePacked(wave, '</defs><g class="parallax"><defs><pattern id="wave-pattern" x="0" y="0" patternUnits="userSpaceOnUse" width="5%" height="5%" viewBox="0 0 10 10"><rect width="100%" height="100%" fill="rgb(150, 90, 25)"></rect>', pattern, '</pattern></defs>'));
        wave = string(abi.encodePacked(wave, '<use xlink:href="#gentle-wave1" x="48" y="1" fill="', fill1, '"/><use xlink:href="#gentle-wave2" x="48" y="3" fill="', fill2, '" /><use xlink:href="#gentle-wave3" x="48" y="5" fill="', fill3, '" /><use xlink:href="#gentle-wave4" x="48" y="7" fill="', fill4, '" /></g></svg>'));
        return wave;
    }
    ...
    function getDurationFromTime() public view returns (string[4] memory) {
        ...
        for(uint i = 0; i < 4; i++)
        {
            curTime = (curTime * curTime) % 997;
            durations[i] = string(abi.encodePacked(toString(2 + (curTime / 10) % 10), '.', toString((curTime / 100) % 10)));
        }
        return durations;
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory leftRight = getDirection(tokenId);
        string memory shimmer = '<defs><linearGradient id="shimmer" x1="0%" y1="0%" x2="100%" y2="0%"><stop stop-color="rgba(212,175,55,0.1)"></stop><stop stop-color="#d4af37"><animate attributeName="offset" values=".05;.95" dur="2s" begin="0" repeatCount="indefinite"/></stop><stop stop-color="rgba(212,175,55,0.1)"><animate attributeName="offset" values="0;1" dur="2s" begin="0" repeatCount="indefinite"/></stop></linearGradient></defs>';
        ...
        ] = string(abi.encodePacked('<svg class="waves" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 350 350" shape-rendering="auto"> <style>.base { font-family: serif; font-size:14px; } .waves { position: relative; width: 100%; height: 15vh; margin-bottom: -7px; min-height: 100px; max-height: 150px;} .parallax > use { animation: move-forever 25s cubic-bezier(0.55, 0.5, 0.45, 0.5) infinite; } .parallax > use:nth-child(0) {animation-delay: -2s; animation-duration: ',durations[0], 's;} .parallax > use:nth-child(1) {animation-delay: -2s; animation-duration: ',durations[1], 's;} .parallax > use:nth-child(2) {animation-delay: -2s; animation-duration: ',durations[2], 's;} .parallax > use:nth-child(3) {animation-delay: -2s; animation-duration: ',durations[3]));
        parts[0] = string(abi.encodePacked(parts[0], 's;} @keyframes move-forever { 0% { transform: translate3d(', keccak256(abi.encodePacked(leftRight)) == keccak256("true")? '-': '', '175px, 0, 0);} 100% { transform: translate3d(', keccak256(abi.encodePacked(leftRight)) == keccak256("true")? '': '-', '175px, 0, 0);}} </style> <rect width="100%" height="100%" fill="#003C5F" />', shimmer, '<text x="10" y="20" fill="url(#shimmer)" class="base">'));
        parts[1] = toggle == true ? getField4(tokenId) : getField1(tokenId);

        parts[2] = '</text><text x="10" y="40" fill="url(#shimmer)" class="base">';

        parts[3] = toggle ? getField1(tokenId) : getField4(tokenId);

        parts[4] = '</text><text x="10" y="60" fill="url(#shimmer)" class="base">';

        parts[5] = getField6(tokenId);

        parts[6] = '</text><text x="10" y="80" fill="url(#shimmer)" class="base">';

        parts[7] = getField7(tokenId);

        parts[8] = '</text><text x="10" y="100" fill="url(#shimmer)" class="base">';

        parts[9] = getField8(tokenId);

        parts[10] = '</text><text x="10" y="120" fill="url(#shimmer)" class="base">';

        parts[11] = getField9(tokenId);

        parts[12] = '</text><text x="10" y="140" fill="url(#shimmer)" class="base">';

        parts[13] = getField10(tokenId);

        parts[14] = '</text><text x="10" y="160" fill="url(#shimmer)" class="base">';
    ...
    function mint(uint256 tokenId) external nonReentrant {
        require (freeMintUnlock <= block.timestamp, "Free mint locked");
        require(limit == 0 || totalSupply() < limit - ownerMintable, "Limit reached");
        _safeMint(_msgSender(), tokenId);
    }

    function multiMint(uint256[] memory tokenIds) external nonReentrant {
        require (freeMintUnlock <= block.timestamp, "Free mint locked");
        require(limit == 0 || totalSupply() + tokenIds.length <= limit - ownerMintable, "Limit reached");
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _safeMint(_msgSender(), tokenIds[i]);
        }
    }

    function mintWithNft1(uint256 tokenId) external {
        require(tokenId <= 6751, "Invalid tokenId");
        require(IERC721(nft1).ownerOf(tokenId) == msg.sender, "Invalid owner");
        _safeMint(_msgSender(), tokenId);
    }

    function multiMintWithNft1(uint256[] memory tokenIds) payable external nonReentrant {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(tokenIds[i] <= 6751, "Invalid tokenId");
            require(IERC721(nft1).ownerOf(tokenIds[i]) == msg.sender, "Invalid owner");
            _safeMint(_msgSender(), tokenIds[i]);
        }
    }

    function mintWithNft2(uint256 nft2TokenId, uint256 tokenIdToMint) external {
        require(nft2Mintable > 0, "No remaining");
        require(tokenIdToMint > 6751 && tokenIdToMint <= 8888, "Invalid tokenId");
        require(IERC721(nft2).ownerOf(nft2TokenId) == msg.sender, "Invalid owner");
        require(claimedCount[msg.sender] < claimLimitPerUser, "Limit reached");
        claimedCount[msg.sender] += 1;
        nft2Mintable -= 1;
        _safeMint(_msgSender(), tokenIdToMint);
    }

    function mintWithNft3(uint256 nft3TokenId, uint256 tokenIdToMint) external {
        require(nft3Mintable > 0, "No remaining");
        require(tokenIdToMint > 6751 && tokenIdToMint <= 8888, "Invalid tokenId");
        require(IERC721(nft3).ownerOf(nft3TokenId) == msg.sender, "Invalid owner");
        require(claimedCount[msg.sender] < claimLimitPerUser, "Limit reached");
        claimedCount[msg.sender] += 1;
        nft3Mintable -= 1;
        _safeMint(_msgSender(), tokenIdToMint);
    ...
    function ownerMint(uint256 tokenId) external onlyOwner {
        require(ownerMintable > 0, "No remaining");
        require(tokenId > 6751 && tokenId <= 8888, "Invalid tokenId");
        ownerMintable -= 1;
        _safeMint(_msgSender(), tokenId);
    }

    function testMint(uint256 tokenId) external onlyOwner {
        _safeMint(_msgSender(), tokenId);
    }
    ...
    constructor() ERC721("Bloot Wheels", "WHEELS") Ownable() {
        freeMintUnlock = block.timestamp + 2 days;
    }
}
