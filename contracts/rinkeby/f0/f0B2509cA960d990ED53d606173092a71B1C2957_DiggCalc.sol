...
contract DiggCalc is FractionalExponents {
	...
	function earned(
		...
		(uint256 mantissa, uint8 exponent) = power(multiplyer.mul(amount), uint256(1e18), 1, 4);
		uint256 noCoefficient = mantissa.mul(uint256(1 ether)).div(uint256(1) << uint256(exponent));

		uint256 rewardRate = noCoefficient.mul(135).div(8640000); // multiply by (1.35/86400)
		return rewardRate.mul(currentEpoch.sub(depositEpoch).add(poolBalance));
	}
}