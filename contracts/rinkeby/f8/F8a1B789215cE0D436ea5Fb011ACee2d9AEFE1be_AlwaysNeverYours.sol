...
/**
 * @title ERC721 token for Always Never Yours
 * @author swaHili
...
contract AlwaysNeverYours is ERC721URIStorage, ReentrancyGuard {
    ...
    address public constant AUCTION_HOUSE = 0xE7dd1252f50B3d845590Da0c5eADd985049a03ce;
     ...
     */
    constructor(address _admin) ERC721("AlwaysNeverYours", "ANY") {
        admin = _admin;
    }
     ...
     */
    modifier onlyAdmin() {
        require(admin == _msgSender(), "Only Admin: Invalid authorization");
        _;
    }
    ...
    modifier onlyCreator(uint256 _tokenId) {
        require(assets[_tokenId].creator == _msgSender(), "Only Creator: Invalid authorization");
        _;
    }
    ...
    modifier onlyAdminOrCreator(uint256 _tokenId) {
        require(admin == _msgSender() || assets[_tokenId].creator == _msgSender(), "Only Admin Or Creator: Invalid authorization");
        _;
    }
    ...
    modifier onlyOwner(uint256 _tokenId) {
        require(ownerOf(_tokenId) == _msgSender(), "Only Owner: Invalid authorization");
        _;
    }
    ...
    function listAssetInWei(uint256 _tokenId, uint256 _priceAmount) external validToken(_tokenId) onlyOwner(_tokenId) {
        ...
        require(_priceAmount > 0, "List Asset: Must set price greater than 0");
        require(asset.priceAmount != _priceAmount, "List Asset: Price must be different than current value");
        require(
            !foreclosure(_tokenId) || assets[_tokenId].creator == sender || admin == sender,
            "List Asset: Foreclosure has already begun"
        );

        asset.priceAmount = _priceAmount;
        asset.taxAmount = _priceAmount / TAX_DENOMINATOR;
    ...
    function depositTax(uint256 _tokenId) external payable validToken(_tokenId) onlyOwner(_tokenId) nonReentrant {
        Asset storage asset = assets[_tokenId];
        require(asset.priceAmount > 0, "Deposit Tax: Must first set sales price");
        require(msg.value >= asset.taxAmount, "Deposit Tax: Initial deposit must not be less than tax price");
        require(!foreclosure(_tokenId), "Deposit Tax: Foreclosure has already begun");
    ...
    function buyAsset(uint256 _tokenId) external payable validToken(_tokenId) nonReentrant {
        address sender = _msgSender();
        Asset storage asset = assets[_tokenId];
        require(ownerOf(_tokenId) != sender, "Buy Asset: Already owner");
        require(asset.priceAmount > 0, "Buy Asset: Asset not up for sale");
        require(asset.priceAmount == msg.value, "Buy Asset: Invalid payment amount");
    ...
    function reclaimAsset(uint256 _tokenId) external validToken(_tokenId) onlyAdminOrCreator(_tokenId) {
        address sender = _msgSender();
        require(foreclosure(_tokenId), "Reclaim Asset: Time has not yet expired");
        require(ownerOf(_tokenId) != sender, "Reclaim Asset: Already owner");
     ...
     */
    function refundTax(uint256 _tokenId, address _currentOwner, uint256 _baseTaxValue) private returns(uint256) {
        Asset storage asset = assets[_tokenId];
     ...
     */
    function collectTax(uint256 _tokenId, address _currentOwner, uint256 _refundAmount) private {
        Asset storage asset = assets[_tokenId];
     ...
     * @param _creator Address of artist who created the asset
     */
    function transferPayments(uint256 _payment, address _currentOwner, address _creator) private {
     ...
     * @param _tokenId ID of the token
     */
    function initializeAsset(uint256 _tokenId) private {
        Asset storage asset = assets[_tokenId];
        asset.priceAmount = 0;
        asset.taxAmount = 0;
    ...
    function resetForeclosure(uint256 _tokenId) external onlyAdmin {
        require(foreclosure(_tokenId) == true, "Reset Foreclosure: Foreclosure has not yet begun");

        assets[_tokenId].foreclosureTimestamp = block.timestamp + BASE_INTERVAL;
    }

    /**
     * @notice Updates the `admin` account for controlling this contract.
    ...
    /**
     * @notice Updates the `baseTaxDenominator` used to calculate daily tax rate.
    ...
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal view virtual override returns (bool) {
        return (
            ...
            AUCTION_HOUSE == spender ||
            ((admin == spender || assets[tokenId].creator == spender) && foreclosure(tokenId))
        );
    }
}
