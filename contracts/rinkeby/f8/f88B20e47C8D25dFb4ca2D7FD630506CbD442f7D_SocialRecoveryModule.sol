...
contract SocialRecoveryModule is Module {
    ...
    string public constant NAME = "Social Recovery Modul";
    ...
    function recoverAccess(address prevOwner, address oldOwner, address newOwner)
        ...
        onlyFriend
    {
        (bytes32 dataHash,) = _getSwapOwnerHash(prevOwner, oldOwner, newOwner);
        require(!isExecuted[dataHash], "Recovery already executed");
        require(isConfirmedByRequiredFriends(dataHash), "Recovery has not enough confirmations");
        readyToRecover[dataHash] = now + timelapse;
    }
    ...
    function confirmRecoverAccess(address prevOwner, address oldOwner, address newOwner)
        ...
        onlyFriend
    {
        (bytes32 dataHash, bytes memory data) = _getSwapOwnerHash(prevOwner, oldOwner, newOwner);
        require(!isExecuted[dataHash], "Recovery already executed");
        require(now >= readyToRecover[dataHash], "Timelapse not completed");
    ...
    /// @dev Returns swapOwner function hash.
    /// @param prevOwner Owner that pointed to the owner to be replaced in the linked list
    /// @param oldOwner Owner address to be replaced.
    /// @param newOwner New owner address.
    /// @return Returns swapOwner function hash.
    function _getSwapOwnerHash(address prevOwner, address oldOwner, address newOwner)
        private
        pure
