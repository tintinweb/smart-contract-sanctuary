...
contract AnimalChess {
    ...
    function giveMoney(string memory roomID ,uint totalAmount) public onlyOwners {
        ...
        if (result == winner.A){
            adddValue(play_list[roomID].playA.player,totalAmount);
        } else if (result == winner.B){
            adddValue(play_list[roomID].playB.player,totalAmount);
        } else if (result == winner.none){
            adddValue(play_list[roomID].playA.player, SafeMath.mod(totalAmount,2));
            adddValue(play_list[roomID].playB.player, SafeMath.mod(totalAmount,2));
        }
    }
   ...
   function getBalance() public view returns (uint) {
    ...
    function adddValue(address _to, uint value1) private {
