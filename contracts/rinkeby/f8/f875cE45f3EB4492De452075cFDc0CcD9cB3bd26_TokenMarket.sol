...
contract TokenMarket is TokenMarketBase {
    ...
    */
    ...
    function loanPaybackEarly(uint256 _loanId, uint256 _paybackAmount) public {
        ...
        if((
            loanDetails.paybackAmount + _paybackAmount) == loanDetails.loanAmountInBorrowed)
            {
                _fullLoanPaybackEarly(_loanId);
            }
        else  if(_paybackAmount < loanDetails.loanAmountInBorrowed && _paybackAmount < loanDetails.paybackAmount)   
            {
