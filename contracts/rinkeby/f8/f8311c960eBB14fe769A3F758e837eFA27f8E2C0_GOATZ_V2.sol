...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _transfer(
        ...
        // payable(from).transfer(0.01 ether, owner());
...
contract GOATZ_V2 is ERC721, Ownable {
  ...
  uint256 public constant totalTokenToMint = 10;
    ...
    returns (string memory)
  {
    ...
    return
      bytes(baseURI).length > 0
        ? string(abi.encodePacked(baseURI, _tokenURIs[tokenId].toString()))
        : '';
  }
  ...
  function reserveNFT(uint256 _howMany, address _sendNftsTo) public onlyOwner {
    require(
      _howMany <= goatzRemainingToBeMinted(),
