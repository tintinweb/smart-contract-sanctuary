...
contract FangsToken is ERC20, IERC721Receiver {
    address public immutable lionsAddress;

    uint256 public DAY_15_RATE = 5 * 15;
    ...
    /**
     * Stake a lion.
     * @notice To use this method, transfer a lion to the contract with data.
     * @notice Do not call this method directly.
     * @dev You must supply the data param with your staking duration.
     */
    function onERC721Received(
        ...
        require(lionsAddress == msg.sender, "FangsToken: Only accepts HawaiianLions");
        uint8 lockMod = abi.decode(data, (uint8));
        require(lockMod > 0 && lockMod < 5, "FangsToken: Must lock for 15, 30, 45 or 60 days");

        StakedInfo memory info = StakedInfo(from, block.timestamp + (15 days * lockMod), DAY_15_RATE * lockMod);

        tokenStakedInfo[tokenId] = info;
     ...
     */
    function unstakeAndClaim(uint256 tokenId) external {
        ...
        // Send lion back
        IHawaiianLionsToken(lionsAddress).transferFrom(address(this), msg.sender, tokenId);
    }
     ...
     * @dev This is NOT gas efficient as so it highly recommend NOT integrating to this
    ...
    function listUnstakedLionsOfOwner(address owner) external view returns (uint256[] memory) {
        IHawaiianLionsToken lions = IHawaiianLionsToken(lionsAddress);
        uint256 lionsSupply = lions.totalSupply();
        uint256[] memory tokenIds = new uint256[](lionsSupply);
        uint256 count = 0;
        for (uint256 tokenId = 1; tokenId <= lionsSupply; tokenId++) {
            if (lions.ownerOf(tokenId) == owner){
