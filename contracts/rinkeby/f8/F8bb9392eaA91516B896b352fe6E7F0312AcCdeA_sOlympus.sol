...
contract sOlympus is ERC20Permit, Ownable {
    ...
    function rebase( uint256 profit_, uint epoch_ ) public onlyStakingContract() returns ( uint256 ) {
        ...
        uint256 initialCirculatingSupply = circulatingSupply();

        if ( profit_ == 0 ) {
            emit LogRebase( block.timestamp, _totalSupply );
            return _totalSupply;
        } else if ( initialCirculatingSupply > 0 ){
            rebaseAmount = profit_.mul( _totalSupply ).div( initialCirculatingSupply );
        } else {
        ...
        _storeRebase( initialCirculatingSupply, profit_, epoch_ );
        epoch++;
