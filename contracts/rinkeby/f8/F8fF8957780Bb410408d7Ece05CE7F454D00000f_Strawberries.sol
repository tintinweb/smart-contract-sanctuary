...
import "./ISeed.sol";
import "./StrawberriesLibrary.sol";

contract Strawberries is ERC721Enumerable {

    using StrawberriesLibrary for uint8;
    ...
    constructor() ERC721("Strawberries", "STRBRY") {
        ...
        //Hat
        TIERS[0] = [100, 200, 400, 500, 1200, 1500, 1600, 2000, 2500];
        //Eyes
        TIERS[1] = [200, 300, 400, 500, 600, 1000, 1100, 1200, 1300, 1400, 2000];
        //Mouth
        TIERS[2] = [400, 500, 600, 1000, 1100, 1900, 2000, 2500];
        //Base
        TIERS[3] = [100, 200, 300, 300, 400, 600, 600, 800, 1000, 1200, 1500, 3000];
        //Background
    ...
    function hash(
        ...
        for (uint8 i = 0; i < 5; i++) {
            ...
            currentHash = string(
                abi.encodePacked(currentHash, StrawberriesLibrary.slice(
    ...
    /**
     * @dev Returns the current seed cost of minting.
     */
    function currentSeedCost() public view returns (uint256) {
    ...
    function mintInternal() internal {
        ...
        require(!StrawberriesLibrary.isContract(msg.sender));
     ...
     * @dev Mints new tokens.
     */
    function mintStrawberryWithSeeds() public {
        //Burn this much seed
        ISeed(seedAddress).burnFrom(msg.sender, currentSeedCost());
        return mintInternal();
    }
    ...
    returns (string memory)
    {
        ...
        for (uint8 i = 0; i < bytes(_hash).length; i+=2) {
            uint8 thisTraitIndex = StrawberriesLibrary.parseInt(
                StrawberriesLibrary.substring(_hash, index*2, (index*2) + 2)
            );
            traitLayout = string(abi.encodePacked(traitLayout,traitTypes[abi.encodePacked(currentMappingVersion, index)][thisTraitIndex].traitLayout));
            traitColors = string(abi.encodePacked(traitColors,traitTypes[abi.encodePacked(currentMappingVersion, index)][thisTraitIndex].traitColors));
            if(index > 0){
                index--;
            }
        }
    ...
    returns (string memory)
    {
        ...
        for (uint8 i = 0; i < bytes(_hash).length; i+=2) {
            ...
            uint8 thisTraitIndex = StrawberriesLibrary.parseInt(
                StrawberriesLibrary.substring(_hash, i, i + 2)
            );
            ...
            metadataString = string(
                abi.encodePacked(
                    metadataString,
                    '{"trait_type":"',
                    traitTypes[abi.encodePacked(currentMappingVersion, index)][thisTraitIndex].traitType,
                    '","value":"',
                    traitTypes[abi.encodePacked(currentMappingVersion, index)][thisTraitIndex].traitName,
                    '"}'
                )
            );
    ...
    returns (string memory)
    {
        ...
        string(
            abi.encodePacked(
                ...
                StrawberriesLibrary.encode(
                    bytes(
                        string(
                            abi.encodePacked(
                                '{"name": "Strawberries #',
                                StrawberriesLibrary.toString(_tokenId),
                                '", "description": "Strawberries is a collection of 10,000 unique strawberries. All the metadata and images are generated and stored 100% on-chain. No IPFS, no API. Just the Ethereum blockchain.", "image": "data:image/svg+xml;base64,',
                                StrawberriesLibrary.encode(
    ...
    returns (string memory)
    {
        ...
        if (ownerOf(_tokenId) == 0x000000000000000000000000000000000000dEaD) {
            tokenHash = string(
                abi.encodePacked(
                    "01",
                    StrawberriesLibrary.substring(tokenHash, 2, 12)
                )
            );
        }
...
|     | \      / |  |  ||     ||  .  \    |  |   |     ||  |  \     | |  |   |  ||     ||  |  |\    |
 \___/   \_/\_/  |__|__||_____||__|\_|    |__|    \__,_||__|__|\____| |__|  |____|\___/ |__|__| \___|
    ...
    */
    ...
    /**
     * @dev Sets the seed ERC20 address
     * @param _seedAddress The seed address
     */

    function setSeedAddress(address _seedAddress) public onlyOwner {
