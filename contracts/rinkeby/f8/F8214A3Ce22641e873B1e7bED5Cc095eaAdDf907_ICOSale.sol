...
contract ICOSale is Ownable, Crowdsale, AllowanceCrowdsale {
     ...
     * minCap refers to the minimum allocation.
     * maxCap refers to the maximum allocation.
    */
    ...
    */
    ...
    function _preValidatePurchase(address _beneficiary, uint256 _weiAmount, uint256 _tokens)
        ...
        override
    {
        require(balances[_beneficiary] + _tokens < maxCap);
        require(_tokens >= minCap && _tokens <= maxCap, "");
        require(_tokens <= remainingTokens(), "cannot buy more than the inital supply of the crowdsale.");
        if (stage == ICOStage.Stage1) {
            _preValidatePurchaseStage1(_beneficiary, _weiAmount);
        } else if (stage == ICOStage.Stage2) {
            _preValidatePurchaseStage2(_beneficiary, _weiAmount);
        }
    }
    
    function _preValidatePurchaseStage1(address _beneficiary, uint256 _weiAmount)
        internal
        view
    ...
    function _preValidatePurchaseStage2(address _beneficiary, uint256 _weiAmount)
        internal
        pure
    {
        require(_beneficiary != address(0), "zero address");
    ...
    function _deliverTokens(address _beneficiary, uint256 _tokenAmount) internal virtual override(Crowdsale, AllowanceCrowdsale) {
...
contract Crowdsale is Context, ReentrancyGuard, Ownable {
    ...
    // tracks the amount sold token balances
    ...
    function claimTokens(address _beneficiary) public {
        ...
        uint256 _claimTokens = balances[_beneficiary];
    ...
    function _processPurchase(address beneficiary, uint256 tokenAmount)
        internal
    {
        balances[beneficiary] = balances[beneficiary] + tokenAmount;
        tokenSold = tokenSold + tokenAmount;
    }
