 ...
 * @author Eric Nordelo
 */
contract SPOracle is ChainlinkClient, AccessControl, Initializable {
    ...
    function initialize(
        ...
        for (uint256 i; i < _owners.length; i++) {
            _setupRole(ADMIN, _owners[i]);
        }

        setChainlinkToken(linkToken);
    }

    function setOracleInfo(PriceOracleInfo calldata _oracleInfo) external onlyRole(ADMIN) {
        linkToken = _oracleInfo.linkToken;
        chainlinkNode = _oracleInfo.chainlinkNode;
        jobId = stringToBytes32(_oracleInfo.jobId);
    ...
    function setAPIURL(string calldata _url) external onlyRole(ADMIN) {
        apiURL = _url;
    }
