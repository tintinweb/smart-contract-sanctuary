...
contract RedeemToken is SafeTransfer, Context {
  ...
  function withdrawTokens(
    ...
    require(id == _id && userItem.owner == _msgSender(), "RedeemToken: not found");
    require(userItem.unlockTime < block.timestamp, "RedeemToken: not unlocked yet");
    userItem.amount = userItem.amount.sub(_amount);

    if (userItem.amount == 0) {
      uint256[] storage userItems = users[_msgSender()].lockToItems[_tokenAddress];
  ...
  /**
   * @notice Retrieve token address at desired index for the specified user.
   * @param _user User address.
