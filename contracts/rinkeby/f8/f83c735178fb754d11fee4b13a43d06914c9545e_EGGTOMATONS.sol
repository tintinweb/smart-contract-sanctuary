...
contract EGGTOMATONS is ERC721Enumerable, Ownable, Payment, Guard {
	...
	mapping(address => uint256) private _mintedMP;
	...
	uint b = mintpass.balanceOf(msg.sender);
	require(publicStatus, "Contract is not active" );
	...
	if (_tokenAmount > 4 && _tokenAmount < 9){
	...
	require(mintpassStatus, "Contract is not active" );
	require(b > 0);
	require(_tokenAmount <= maxMP * b, "Mint less");
	require(s + _tokenAmount <= maxSupply, "Mint less");
	require(msg.value >= priceMP * _tokenAmount, "ETH input is wrong");
	if(_mintedMP[msg.sender] == 0){
	for (uint256 i = 0; i < _tokenAmount + 1; ++i) {
	_mintedMP[msg.sender] += 1;
	_safeMint(msg.sender, s + i, "");
	}
	}
	if(_mintedMP[msg.sender] > 0){
	...
	require(_tokenAmount > 0, "Mint more than 0" );
	require(_tokenAmount <= maxWL, "Mint less");
	require(s + _tokenAmount <= maxSupply, "Mint less");
	...
	function mintedMP(address owner) public view  returns (uint256){
    require(owner != address(0), 'Zero address');
    return _mintedMP[owner];
  }
