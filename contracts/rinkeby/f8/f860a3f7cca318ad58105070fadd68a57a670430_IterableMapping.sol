...
/*
What's better than earn $eth or $btc?  
Get rewards from both at the same time! 
#Bithereum is the first BTC & ETH reflection token.
Stealth launch soon: Join telegram!
https://t.me/bithereumbsc
*/
...
contract Bithereum is ERC20, Ownable {
    ...
    uint256 public ethDividendRewardsFee = 4;
    uint256 public previousEthDividendRewardsFee;
    uint256 private marketingFee = 5;
    uint256 private previousMarketingFee;
    uint256 public totalFees = uniDividendRewardsFee.add(marketingFee).add(ethDividendRewardsFee).add(liquidityFee);
    ...
    constructor() ERC20("Bithereum", "BTH") {
        ...
        ethDividendToken = 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82;
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "Bithereum: The marketing wallet is already this address");
    ...
    function setUniDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = uniDividendRewardsFee.add(marketingFee).add(ethDividendRewardsFee).add(liquidityFee);
            uniDividendEnabled = _enabled;
        }
    ...
    function setEthDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = ethDividendRewardsFee.add(marketingFee).add(uniDividendRewardsFee).add(liquidityFee);
            ethDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(ethDividendRewardsFee).add(uniDividendRewardsFee).add(liquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updatebtcDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(btcDividendTracker), "Bithereum: The dividend tracker already has that address");
 
        BTCDividendTracker newbtcDividendTracker = BTCDividendTracker(payable(newAddress));
 
        require(newbtcDividendTracker.owner() == address(this), "Bithereum: The new dividend tracker must be owned by the Bithereum token contract");
    ...
    function updateethDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(ethDividendTracker), "Bithereum: The dividend tracker already has that address");
 
        ETHDividendTracker newethDividendTracker = ETHDividendTracker(payable(newAddress));
 
        require(newethDividendTracker.owner() == address(this), "Bithereum: The new dividend tracker must be owned by the Bithereum token contract");
    ...
    function updateEthDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = ethDividendRewardsFee.add(uniDividendRewardsFee).add(marketingFee).add(liquidityFee);
    }
 
    function updateMarketingFee(uint8 newFee) external onlyOwner {
        marketingFee = newFee;
        totalFees = marketingFee.add(uniDividendRewardsFee).add(ethDividendRewardsFee).add(liquidityFee);
    }
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "Bithereum: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "Bithereum: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Bithereum: The PanadaSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "Bithereum: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "Bithereum: Cannot update gasForProcessing to same value");
	...
	function processDividendTracker(uint256 gas) external onlyOwner {
		(uint256 uniIterations, uint256 uniClaims, uint256 uniLastProcessedIndex) = btcDividendTracker.process(gas);
		emit ProcessedbtcDividendTracker(uniIterations, uniClaims, uniLastProcessedIndex, false, gas, tx.origin);
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "Bithereum: Trading has not started yet");
    ...
    function swapAndSendUniDividends(uint256 tokens) private {
        ...
        transferDividends(uniDividendToken, address(btcDividendTracker), btcDividendTracker, uniDividends);
    }
...
contract BTCDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BithereumDividend_Tracker", "BithereumDividend_Tracker", 0xB27ADAfFB9fEa1801459a1a81B17218288c097cc) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "BithereumDividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "BithereumDividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Bithereum contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "BithereumDividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BithereumDividend_Tracker: Cannot update claimWait to same value");
...
contract ETHDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("Bitherereum_ETH_Dividend_Tracker", "Bitherereum_ETH_Dividend_Tracker", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "Bitherereum_ETH_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "Bitherereum_ETH_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Bithereum contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "Bitherereum_ETH_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Bitherereum_ETH_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
