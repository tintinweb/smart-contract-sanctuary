...
contract DayNightNFT is
    ERC721URIStorageUpgradeable,
    OwnableUpgradeable,
    VRFConsumerBaseInitializable
{
    ...
    mapping(bytes32 => string) requestIdToNftName;
    ...
    mapping(uint256 => string) tokenIdToNftName;
    ...
    event WhichInitializerWorked(string message);
    ...
    function initialize(
        ...
        __ERC721_init("Dice", "DIC");
        __VRFConsumerBaseInitializable_init_(_VRFCoordinator, _LinkToken);
        VRFCoordinator = _VRFCoordinator;
        LinkToken = _LinkToken;
        keyHash = _keyhash;
        fee = 0.1 * 10**18;

        emit WhichInitializerWorked("the initializer was used");
    }


    function mintNft(address toAddress, bytes memory tokenUrisHashcode1, bytes memory tokenUrisHashcode2)
        public
        onlyOwner
        returns (uint256)
    {
    ...
    function requestNewRandomNumber(string memory nftName, uint256 tokenId)
        public
        returns (bytes32)
    {
        require(
        ...
        requestIdToNftName[requestId] = nftName;
        ...
        tokenIdToNftName[tokenId] = nftName;
...
abstract contract VRFConsumerBaseInitializable is VRFRequestIDBase, Initializable {
   ...
   */
  function __VRFConsumerBaseInitializable_init_(
    address _vrfCoordinator,
    address _link
