...
abstract contract ERC721TradableJulia is ContextMixin, ERC721Enumerable, NativeMetaTransaction, Ownable {
    ...
    uint256 public constant maximumPieces = 4106; // 1978 * 2 + 150
...
contract Julia is ERC721TradableJulia {
    ...
    uint32  private given_gifts = 0;
    uint32  private used_brots  = 0;
    uint256 private PRICE = 197800000000000000;    // 0.1978 ETH
    uint32  private PRESALE_TIME = 172800;        // 48 hours
    uint8   private MAX_QUANTITY = 5;            // Maximum allowed quantity to purchase per transaction
    uint8   public  COMMUNITY_QUOTA = 10;       // 10% of the sale proceeds stays in the community
    uint8   public  MAX_GIFTS = 150;
    
    mapping (uint256 => uint256) claimed;
    mapping (uint256 => uint8) lucky_tokens;
    mapping (uint256 => uint8) to_verify;
    ...
    address public  VIP_PASS_ADDRESS   = 0x6E96Fb1f6D8cb1463E018A2Cc6E09C64eD474deA;
    address public  FREE_MINT_ADDRESS  = 0x4035E762D8968174B9a777F243dC9bde0087dE46;
    ...
    function claimable_brots() external view returns(uint32) {
        require(_active, "Inactive");
    ...
    function pre_purchase(uint256 fractals) external payable {
        ...
        require(_active, "Inactive");
        require(fractals + 1978 - used_brots<= remaining() && fractals <= MAX_QUANTITY, "Too many fractals requested");
        require(msg.value == fractals*PRICE, "Invalid purchase fractals sent");
        for (uint i = 0; i < fractals; i++) {
            mintTo(msg.sender);
            
            // Potential gift
            if (is_lucky_token(_currentTokenId) && (given_gifts<MAX_GIFTS)) {
                mintTo(msg.sender);
                given_gifts++;
            }
        }
    ...
    function claim_from_brots(uint256[] memory brots) external {
        ...
        for (uint i = 0; i < brots.length; i++) {
            ...
            //TO-DO: need to check that the guy actually holds the Brot!!!
            to_verify[brots[i]] = 1;
        ...
        while (verified < brots.length) {
            owned = IERC721Enumerable(MANDELBROT_ADDRESS).tokenOfOwnerByIndex(msg.sender, i);
            owned = IERC721Enumerable(MANDELBROT_ADDRESS).tokenByIndex(owned);
            if (to_verify[owned]==1 && claimed[owned]==0) {
                verified++;
    ...
    function claim_from_free_mint(uint256 fractals) external {
        require(_active, "Inactive");
        require(block.timestamp < _activeTime + PRESALE_TIME, "Too late!");
        require(fractals + 1978 - used_brots <= remaining() && fractals <= MAX_QUANTITY, "You are claiming too many fractals");
        IERC20(FREE_MINT_ADDRESS).transfer(msg.sender, fractals);
        for (uint i = 0; i < fractals; i++) {
            mintTo(msg.sender);
        }   
    }

    function is_lucky_token(uint256 token_id) private view returns (bool) {
        return (lucky_tokens[token_id]==1);        
    }
    ...
    function set_gifts(uint256[] memory gift_ids) external onlyOwner {
        for (uint i=0; i < gift_ids.length; i++) {
            lucky_tokens[gift_ids[i]] = 1;
        }
    }
