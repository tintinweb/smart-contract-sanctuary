...
contract BattleContract is VRFConsumerBase, Ownable {
    ...
    uint256 private currentBattleId_;
    bytes32 private currentRequestId_;

    // Intefaces of Contracts
     ...
     */
    event NewBattleCreated(uint256 battleId);
     ...
     */
    event ClaimedGFX(uint256 battleId, uint256 amount);

    /**
     * @param battleId Battle Id
     * @param nftId Claimed NFT Id
     */
    event ClaimedNFT(uint256 battleId, uint256 nftId);
     ...
     * @param _battleId Battle ID
     */
    function _generateRandomNumber(uint256 _battleId)
        private
        returns (bytes32 requestId)
    {
        ...
        currentBattleId_ = _battleId;
        return requestRandomness(keyHash, fee, _battleId);
    }
    ...
    function fulfillRandomness(bytes32 _requestId, uint256 _randomness)
        ...
        override
    {
        uint256 winnerIndex = _randomness % participantsCount[currentBattleId_];

        Participant memory participant =
            participants[currentBattleId_][winnerIndex];
        battleHistories[currentBattleId_] = participant;

        _burnNFT(currentBattleId_, winnerIndex);

        currentRequestId_ = _requestId;

        emit NewWinner(currentBattleId_, participant.nftId);
    }
    ...
    function createNewBattle(
        ...
        emit NewBattleCreated(battle.id);
     ...
     * @param _battleId Battle ID
     */
    function completeBattle(uint256 _battleId)
        public
        onlyOwner
        _checkValidBattleId(_battleId)
     ...
     * @param _battleId Battle ID
     */
    function claimNFT(uint256 _battleId) public _isWinner(_battleId) {
        Participant storage winner = battleHistories[_battleId];
        require(!winner.isClaimedNFT, "BattleContract: Already Claimed NFT");

        nft_.transferNFT(msg.sender, winner.nftId);
        winner.isClaimedNFT = true;

        emit ClaimedNFT(_battleId, winner.nftId);
    }
     ...
     * @param _battleId Battle ID
     */
    function claimGFX(uint256 _battleId) public _isWinner(_battleId) {
        Participant storage winner = battleHistories[_battleId];
        ...
        emit ClaimedGFX(_battleId, battles[_battleId].award);
    }
}
