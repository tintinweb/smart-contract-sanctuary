...
import "./utils/IOfficialBuffDoge.sol";
import "./utils/Ownable.sol";
import "./utils/SafeMath.sol";

contract TokenExchange is Ownable {
    IOfficialBuffDoge private _newToken;
    ...
    constructor (address buffToken, address oldToken, address[] memory exchangeList) {
        _newToken = IOfficialBuffDoge(buffToken);
    ...
    function exchangeOldToken() external onlyOwner returns (bool) {
        ...
        for(uint k = 0; k < _exchangeList.length; k++) {
            ...
            if(balances > 0) {
                _newToken.transfer(_exchangeList[k], balances);
            }
        }
        return true;
    }
