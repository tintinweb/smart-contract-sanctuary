...
contract CryptoTraderTokenUriProviderV2 is CryptoTraderInterface {
     ...
     */
    function tokenUri(uint8 priceType) private view returns (string memory) {
        ...
        (uint256 prevPrice, uint256 currentPrice) = getPrice(priceType);
        ...
        for (uint256 i = 0; i < ranges.length; i++) {
            if (
                ...
                if (prevPrice < currentPrice) {
                    return ranges[i].tokenUriUp;
                } else {
                    return ranges[i].tokenUriDown;
                }
            }
        }
