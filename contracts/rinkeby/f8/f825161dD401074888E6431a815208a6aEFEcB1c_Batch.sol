...
contract Batch is IERC721Receiver, AccessControl {
    ...
    // Number of Nfts ID in an collection 
    ...
    */
    ...
    function setCollateralToken(IERC20 baseToken_) public onlyAdmin {
    ...
    */
    function _depositNFT(
        ...
        uint256 nftId;
        /* 
            here we are checking if this NFT with respect to token address and
            token id is previously stored or not

            if stored then apoint same nft id
                else
            new nft id
        */
        if (nftTokenIndex[tokenAddress_][tokenId_] > 0) {
            nftId = nftTokenIndex[tokenAddress_][tokenId_];
        } else {
            nftId = nextNftID.add(1);
            nftTokenIndex[tokenAddress_][tokenId_] = nftId;
        }
    ...
    function addNFTToCollection(
        ...
        uint256 nftId_ = _depositNFT(
            tokenAddress_, 
            tokenId_, 
            collectionId_, 
            collectionsNftCount[collectionId_]
        );
    ...
    // yet to test
    function _withdrawNFT(address who_, uint256 nftId_) private {
