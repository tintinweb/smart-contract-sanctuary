...
contract  DegenSantas  is ERC721Enumerable, Ownable
{
    ...
    constructor(string memory baseURI,address _nft) ERC721("Degen Santas", "SANTA")  {
        setBaseURI(baseURI);
        nft=IERC1155(_nft);
    }
    function setBaseURI(string memory baseURI) public onlyOwner {
     ...
     function mint(uint quantity) public {
        ...
        require(quantity+totalSupply()<=maxSupply,"quantity is greater then maxSupply");

       
        
        if(_addressExist[msg.sender]==false)
        {
             uint gold=nft.balanceOf(msg.sender,1);
             uint silver=nft.balanceOf(msg.sender,2);
             whiteListCounter=gold+silver;
             whiteListCounter=whiteListCounter*6;
