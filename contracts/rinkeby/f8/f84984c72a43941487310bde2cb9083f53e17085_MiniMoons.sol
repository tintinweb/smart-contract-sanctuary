...
contract MiniMoons is Context, ERC1155Pausable, Ownable {
    ...
    // Constants
    uint256 constant TOTAL_TYPES = 36;
    ...
    // Events
    // event Created(address indexed to, uint256 id, uint256 amount, string uri);
    ...
    function mint() public payable {
        ...
        tokenSupply[token[0]] = tokenSupply[token[0]].add(amount[0]);

        // Add count to totalSupply
        totalMints.add(amount[0]);
    }

    /**
     * @dev Public Batch Minting
     * @param _amount Amounts of tokens to mint
     */
    function mintBatch(uint256 _amount) public payable {
        ...
        // Pluck tokens based on _amount provided
        ...
        // Look through and update tokenSupply indexes
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            // Add count to totalSupply
            totalMints.add(_quantity);
        }
    }

    /**
     * @dev Plucks a random token and supply of 1.
     * @param _amount Amount of tokens to pluck
     */
        ...
        returns (uint256[] memory, uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < _amount; i++) {
            // Generate weighted token from manifest
            uint256 token = rand();

            // Make sure the plucked token is within range
            require(token > 0 && token < TOTAL_TYPES, "Token ID Out Of Range");

            // Add single token to array
            tokensIds[i] = token;
            tokenAmounts[i] = 1;
        }
    ...
    // PseudoRandom Number for testing range between 0-36
    function rand() internal virtual returns (uint256) {
        ...
        return (seed - ((seed / TOTAL_TYPES) * TOTAL_TYPES));
    }
}