...
contract NFTAuction is Context, Ownable, ReentrancyGuard {
    ...
    modifier isFeePercentagesLessThanMaximum(uint16[] memory _feePercentages) {
        uint16 totalPercent;
        for (uint256 i = 0; i < _feePercentages.length; i++) {
            totalPercent = totalPercent + _feePercentages[i];
        }
        require(totalPercent <= 10000, "Fee percentage exceed maximum");
        _;
    }
    ...
    constructor() {
        ...
        marketFee = [250];
        marketAddress = [address(0x00B91B2F8aFE87FCDc2b3fFA9ee2278cd1E4DDf8)];
    }
        ...
        isFeePercentagesLessThanMaximum(_feePercentages)
    {
        ...
        nftContractAuctions[_nftContractAddress][_tokenId].liveOn = true;
    }
    ...
    {
        require(nftContractAuctions[_nftContractAddress][_tokenId].liveOn, "Auction is not live on");
    ...
    //     // }
    //     _updateAuctionEnd(_nftContractAddress, _tokenId); //ctdarius
    // }
    ...
    function _resetAuction(address _nftContractAddress, uint256 _tokenId) internal {
        ...
        nftContractAuctions[_nftContractAddress][_tokenId].liveOn = false;
    }
        ...
        priceGreaterThanZero(_newMinPrice)
    {
        ...
        //     _updateAuctionEnd(_nftContractAddress, _tokenId);
        // }
        //_updateOngoingAuction(_nftContractAddress, _tokenId); //ctdarius
    }
    ...
    /**
     * @notice Set up new market's fee
     * @param _marketFee Market's fee 
     */
    function _setMarketFee(uint16[] memory _marketFee) private {
        marketFee = _marketFee;
    }

    function setMarketFee(uint16[] memory _marketFee) public onlyOwner {
        _setMarketFee(_marketFee);
    }

    /**
     * @notice Set up new market's address
     * @param _marketAddress Market's address
     */
    function _setMarketAddress(address[] memory _marketAddress) private {
        marketAddress = _marketAddress;
    }

    function setMarketAddress(address[] memory _marketAddress) public onlyOwner {
        _setMarketAddress(_marketAddress);
    }
}