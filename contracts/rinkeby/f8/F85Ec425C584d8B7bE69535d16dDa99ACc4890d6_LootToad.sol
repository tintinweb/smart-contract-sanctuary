...
contract LootToad is ERC721, PaymentSplitter, Ownable, Pausable {
  ...
  event Mint(address indexed tew, uint256 id);
  ...
  function mineReserves(address _to, uint256 _amount) public onlyOwner {
  ...
  function getOneNFT(address _to) public payable {
  ...
  function getMultipleNFT(uint256 _howMany, address _to) public payable {
    require(saleIsActive, 'Sale must be active to mint');
    require(_howMany <= _maxNftAtOnce, "too many NFT's at once");
  ...
  function master_mint(address to) private {
    _safeMint(msg.sender, _tokenIdCounter.current() + 1);
    _tokenIdCounter.increment();
    emit Mint(to, _tokenIdCounter.current() + 1);
  }
