...
contract animalMain is  ERC721,Ownable {
    ...
    uint16 roletype; //(哪種動物 {Cat, Dog, Tiger, Lion, Dolphin, Whale})
    }
    
    uint constant MAX_STAR = 5;
    enum role{Cat, Dog, Tiger, Lion, Dolphin, Whale}
    ...
    function createAnimal() public payable{
        ...
        dna = bytes32(keccak256(abi.encode(now, block.coinbase, msg.sender)));
        
        // determine star
        uint rand = uint(keccak256(abi.encode(now, block.coinbase, msg.sender)))%100;
        if(rand<40) star = 1;
        ...
        // determine animal
        rand = uint(keccak256(abi.encode(block.difficulty, block.coinbase, block.timestamp)))%100;
        if(rand<30) roletype = uint(role.Cat);
        else if (rand<60) roletype = uint(role.Dog);
        else if (rand<75) roletype = uint(role.Tiger);
        else if (rand<90) roletype = uint(role.Lion);
        else if (rand<95) roletype = uint(role.Dolphin);
        else if (rand<100) roletype = uint(role.Whale);
        else roletype = uint(role.Dog);
