...
interface IERC1155TokenCreator {
     ...
     * @param _contractAddress address of the ERC1155 contract
...
contract NifterMarketAuction is INifterMarketAuction, Ownable, Payments {
     ...
     */
    constructor(address _iMarketSettings, address _iERC1155CreatorRoyalty)
    public
    {
        require(
    ...
    function ownerMustHaveMarketplaceApproved(
        ...
        IERC1155 erc1155 = IERC1155(_originContract);
        INifter nifter = INifter(_originContract);
    ...
    {
        INifter nifter = INifter(_originContract);
        require(msg.sender == nifter.ownerOfToken(_tokenId) || msg.sender == _originContract, 'sender must be the token owner');
    }
    ...
    function buy(address _originContract, uint256 _tokenId) public payable {
        ...
        Payments.payout(
            sp.amount,
            !iMarketplaceSettings.hasERC1155TokenSold(_originContract, _tokenId),
            ...
            owner,
            iERC1155CreatorRoyalty.tokenCreator(_originContract, _tokenId),
            owner
        );

        // Set token as sold
        iMarketplaceSettings.markERC1155Token(_originContract, _tokenId, true);

        emit Sold(_originContract, msg.sender, nifter.ownerOfToken(_tokenId), sp.amount, _tokenId);
    }
    ...
    function bid(
        ...
        INifter nifter = INifter(_originContract);
        address tokenOwner = nifter.ownerOfToken(_tokenId);
        // address tokenOwner = erc1155.ownerOf(_tokenId);
