...
contract animalMain is  ERC721,Ownable {
  ...
  using SafeMath for uint256;
    string public name_ = "MapleStorytoken";
  ...
  function seeAnimalSize(uint256 _tokenId) public view returns (uint16 size) {
    return animals[_tokenId].size;
  }
  ...
  function createAnimal() public payable {
      ...
      size = uint(random()) % 100;
      
      //動手玩創意，可以限制每次建立動物需要花費多少ETH
      
      require(msg.value == 100 wei);
       
      uint id = animals.push(animal(dna, uint8(star), uint8(roletype), uint16(size))) - 1;
