...
contract Router is Ownable, ReentrancyGuard {
  uint256 bigNum = 1 ether * 100000000;
  ...
  function swapToken(uint256 tokenId0_, uint256 amount0_, uint256 tokenId1_, uint256 amount1Min_) external nonReentrant {
    ...
    ITicketToken(tokenAddress0).approve(poolAddress0, bigNum);
    IERC20(factory.daiToken()).approve(poolAddress0, bigNum);
    IERC20(factory.daiToken()).approve(poolAddress1, bigNum);
    ...
    IERC20(factory.daiToken()).transferFrom(_msgSender(), address(this), beforeBalance - IERC20(factory.daiToken()).balanceOf(address(this)));
