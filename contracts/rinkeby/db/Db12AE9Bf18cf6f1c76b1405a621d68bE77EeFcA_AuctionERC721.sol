...
contract AuctionERC721 is Ownable, IERC721Receiver, AbstractSale {
    ...
    function closeAuction(address erc721token, uint256 tokenId, address erc20Token,  Trade memory trade, BuyerSig memory buyerSig, SystemSig memory systemSig) public payable {
        // Check if the called is the owner of NFT
        address payable seller = address(uint160(IERC721(erc721token).ownerOf(tokenId)));
        ...
        // get Nonce of token
        uint256 nonce = nonceHolder.getNonce(IERC721(erc721token), tokenId);
        
        // Validate Trade Value of Buyer & Balance of Buyer
        require(validateTradeValue(trade.offeredPrice, IERC20(erc20Token), trade.buyer), "Buyer don't have enough balance");
        
        // Verify Bidder Order
        string memory messageSignedByBuyer = prepareMessage(address(erc721token), tokenId, erc20Token, trade.offeredPrice, trade.sellerFee, nonce);
        address isBuyer = StringLibrary.recover(messageSignedByBuyer, buyerSig.v, buyerSig.r, buyerSig.s);
        require(trade.buyer == isBuyer, "bidder should sign correct message");
        
        // Verify System Signer Order
        address isSystemSigner = StringLibrary.getAddress(abi.encodePacked(IERC721(erc721token), tokenId, msg.sender, trade.buyer, IERC20(erc20Token), trade.offeredPrice, trade.auctionStart, trade.auctionEnd, nonce), systemSig.v, systemSig.r, systemSig.s);
        require(owner() == isSystemSigner, "system signer should sign correct message");
        
        // Transfer NFT
        transferERC721NFT(IERC721(erc721token), trade.buyer, tokenId);
        
        // transferTrade Value
        transferTrade(erc721token, tokenId, trade.offeredPrice, trade.sellerFee, erc20Token, trade.buyer);

        // Increases the nonce
        changeNonce(erc721token, tokenId, nonce);
        emitAuction(erc721token, tokenId, erc20Token, trade.buyer, trade.offeredPrice, nonce + 1);
    }
    
    function transferTrade(address erc721token, uint256 tokenId, uint256 offeredPrice, uint256 sellerFee, address erc20Token, address buyer ) internal {
        uint256 offeredFee = offeredPrice;
        uint256 sellerFeeToSlash = transferERC20FeeToBeneficiary(offeredFee, sellerFee, erc20Token, buyer);
        
        // Transfer Createor Royalty to Creator
        uint256 creatorFee = getCreatorFee(erc721token, tokenId);
        address creator = getCreator(erc721token, tokenId);
        uint256 creatorSlash = offeredFee.mul(creatorFee).div(maxBuyerFeePercentage);
        erc20TransferProxy.erc20safeTransferFrom(IERC20(erc20Token), buyer, creator, creatorSlash);
        
        // Transfer Seller
        uint256 sellerSlash = offeredFee.sub(creatorSlash).sub(sellerFeeToSlash);
        erc20TransferProxy.erc20safeTransferFrom(IERC20(erc20Token), buyer, msg.sender, sellerSlash);
    }
    ...
    function emitAuction(address erc721token, uint256 tokenId, address erc20Token, address buyer, uint256 offeredPrice, uint256 nonce) internal {
        emit AuctionClosed(address(erc721token), tokenId, msg.sender, buyer, erc20Token, offeredPrice, nonce);
    }
