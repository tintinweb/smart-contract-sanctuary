...
contract BatchAirdrop is OwnableUpgradeable {
    ...
    function initialize(uint256 _maxCounts) initializer public {
        __Ownable_init();
        maxCounts = _maxCounts;
    }
    ...
    function batchTokenSend(
        ...
        for (uint256 i = 0; i < _users.length; i++) {
            require(address(_users[i]) != address(0), "contains an unverified address");
            
            TransferHelper.safeTransferFrom(_token, msg.sender, _users[i], _amounts[i]);
            total += _amounts[i];
        }
