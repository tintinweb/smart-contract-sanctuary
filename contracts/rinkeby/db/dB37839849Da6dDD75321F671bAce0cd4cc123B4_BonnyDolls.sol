...
contract BonnyDolls is Ownable, ERC721V2 {
    ...
    function mint(uint256 _count, bytes memory _hash, bytes memory _signature) external payable saleIsOpen {
        uint256 total = _owners.length;
        ...
        address signer = walletAuth(minter, _count, _hash, _signature);
    ...
    function walletAuth(address _minter, uint _count, bytes memory _hash, bytes memory _signature) public pure returns (address) {
        return ECDSA.recover(keccak256(abi.encode(_minter, _count, _hash)), _signature);
    }
