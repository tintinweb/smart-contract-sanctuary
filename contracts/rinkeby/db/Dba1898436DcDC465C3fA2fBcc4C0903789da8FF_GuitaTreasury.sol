...
contract GuitaTreasury is GuitaAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    IGUITA public immutable GUITA;
    IsGUITA public sGUITA;
    ...
    /**
     * @notice allow approved address to deposit an asset for GUITA
    ...
    function deposit(
        ...
        // mint GUITA needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        GUITA.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn GUITA for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        GUITA.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new GUITA using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        GUITA.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or GUITA from the treasury, using sGUITA as collateral. This might allow an
     * sGUITA holder to provide GUITA liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(GUITA)) {
            require(permissions[STATUS.GUITADEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sGUITA.changeDebt(value, msg.sender, true);
        require(sGUITA.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(GUITA)) {
            GUITA.mint(msg.sender, value);
            guitaDebt = guitaDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sGUITA.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with GUITA
     * @param _amount uint256
     */
    function repayDebtWithGUITA(uint256 _amount) external {
        require(
            permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.GUITADEBTOR][msg.sender],
            notApproved
        );
        GUITA.burnFrom(msg.sender, _amount);
        sGUITA.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        guitaDebt = guitaDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(GUITA), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SGUITA) {
            sGUITA = IsGUITA(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SGUITA) {
            // 9
            sGUITA = IsGUITA(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(GUITA.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns GUITA valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(GUITA)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return GUITA.totalSupply() - guitaDebt;
    }
}
