...
contract Crowdsale is Context, ReentrancyGuard, Listing {
    ...
    // 1628899200
    uint256 private _preIcoStartDate = 1628139603;

    // PRE-ICO End Date (27th August)
    // 1630047600
    uint256 private _preIcoEndDate = 1628140203;

    // PRE-ICO Tokens available (15%)
    // 75000000000 ether
    uint256 private _preIcoTokens = 200 ether;

    // ICO Start Date (28th August)
    // 1630108800
    uint256 private _icoStartDate = 1628140263;

    // ICO End Date (28th September)
    // 1632787200
    uint256 private _icoEndDate = 1628140383;

    // ICO Tokens available (70%)
    uint256 private _icoTokens = 200 ether;
    ...
    modifier checkPreIcoTime() {
        if((stage == CrowdsaleStage.PREICO) &&(block.timestamp < _preIcoStartDate))
            revert("Crowdsale: PRE_ICO_NOT_STARTED");
        if((stage == CrowdsaleStage.PREICO) && (block.timestamp > _preIcoEndDate))
    ...
    constructor(
        ...
        require(address(crowdsaleToken) != address(0),"Crowdsale: ZERO_ADDRESS");
    ...
    function buyTokens(address beneficiary) public payable nonReentrant {
        ...
        if ((stage == CrowdsaleStage.PREICO) &&( preIcoSold.add(tokens) > _preIcoTokens)
        ) {
        ...
        checkIcoTime
    {
        ...
        require(beneficiary != address(0),"Crowdsale: ZERO_ADDRESS");
    ...
    function setCurrentRate(uint256 value) private {
     ...
     function finish() public onlyOwner {
        ...
        uint256 ethAmount = address(this).balance.mul(100).div(40); // 40 %
    ...
    Ownable
{
    ...
    mapping (address => uint256) lastBurns;
