...
contract Test is ERC721, Ownable {
    ...
    uint constant public MAX_SUPPLY = 90; // 9000 unique  will be in total.
    uint constant public PRICE = 0.0 ether; // 0.07 ETH + gas per transaction
    uint constant public PRESALE_PRICE = 0.0 ether; // 0.07 ETH + gas per transaction
    uint constant public PRESALE_PER_TX_LIMIT = 5; // Up to 5  per transaction during the presale.
    uint constant public PRESALE_PER_WALLET_LIMIT = 5; // Up to 5  per unique wallet during the presale.
    uint constant public MINT_PER_TX_LIMIT = 20; // Up to 20  per transaction.

    string private _apiURI = "";
    uint public reservedTokensLimit = 10; // 200  will be reserved for marketing needs.

    uint public claimLimit = 2; // Take from all Mintpasses sold count
    uint public presaleTokensLimit = 5;

    bool public isClaimingAvailable = true;
    bool public isMintingAvailable = true;
    bool public isPresaleAvailable = true;
    ...
    IERC1155 public mintPassContract = IERC1155(0xC338680E9990A3bc674C450c2c77F0471e904e3c);
    IERC721 public friendContract = ERC721(0x64B7490089eF40244753d29fF1e431B6a9fa3359);

    constructor() ERC721("test", "TH") {
    }
    ...
    function configure(
        IERC721 _coolCatsContract,
        IERC1155 _mintPassContract,
        bool _isClaimingAvailable,
        bool _isMintingAvailable,
        bool _isPresaleAvailable
    ) external onlyOwner {
        friendContract = _coolCatsContract;
        mintPassContract = _mintPassContract;
        isMintingAvailable = _isMintingAvailable;
        isPresaleAvailable = _isPresaleAvailable;
    ...
    function presaleMint(uint amount) external payable {
        ...
        require(amount > 0 && amount <= PRESALE_PER_TX_LIMIT, "Can only mint 20 tokens at a time");
