...
contract Merge is ERC721, ERC721Metadata {
     ...
     */
     function mint(uint256[] calldata values_) external onlyValidSender {
        ...
        uint256 amountSkipped = 0;

        for (uint256 i = 1; i <= values_.length; i++) {            

            if(values_[i - 1] == 999999999) {
        ...
        uint256 amountNewlyMinted = values_.length - amountSkipped;
        _countToken += amountNewlyMinted;

        _balances[_omnibus] += amountNewlyMinted;
