...
contract ExchangeNFT is ERC721Holder, Ownable, Pausable {
    ...
    uint256 private _lastAuctionNumber;
    EnumerableMap.UintToUintMap private _asksMap;
    EnumerableMap.UintToUintMap private _askAuctionsMap;
    mapping(uint256 => address) private _tokenSellers;
    mapping(uint256 => mapping(address => uint256)) public _tokenBuyers;
        ...
        whenNotPaused
    {
        ...
        bool isAuctionStarted = _askAuctionsMap.contains(_tokenId);
        require(!isAuctionStarted, "Auction was already started");

        // nft.safeTransferFrom(address(msg.sender), address(this), _tokenId);

        _lastAuctionNumber = _lastAuctionNumber + 1;
        _asksMap.set(_tokenId, _price);
        _askAuctionsMap.set(_tokenId, _lastAuctionNumber);
        _tokenSellers[_tokenId] = address(msg.sender);
        _userSellingTokens[msg.sender].add(_tokenId);

        emit Ask(msg.sender, _lastAuctionNumber + 1, _tokenId, _price);
    }

    function cancelSellToken(uint256 _tokenId) public whenNotPaused {
        require(
            _userSellingTokens[msg.sender].contains(_tokenId),
            "Only Seller can cancel sell token"
        );

        require(_askAuctionsMap.contains(_tokenId), "Auction was not created");
        uint256 auctionId = _askAuctionsMap.get(_tokenId);
        // nft.safeTransferFrom(address(this), msg.sender, _tokenId);

        _asksMap.remove(_tokenId);
        _askAuctionsMap.remove(_tokenId);
        _userSellingTokens[_tokenSellers[_tokenId]].remove(_tokenId);
        delete _tokenSellers[_tokenId];

        emit CancelSellToken(msg.sender, auctionId, _tokenId);
    }
        ...
        whenNotPaused
    {
        ...
        emit Ask(msg.sender, auctionId, _tokenId, _price);
    }

    function bidToBuyToken(uint256 _tokenId, uint256 _price)
        public
        whenNotPaused
    {
        require(
            msg.sender != nft.ownerOf(_tokenId),
            "Tokwn owner can't buy token"
        );
        uint256 askPrice = _asksMap.get(_tokenId);
        ...
        _tokenBuyers[auctionId][msg.sender] = _price;
        emit Bid(msg.sender, auctionId, _tokenId, _price);
    }

    function sellToken(uint256 _tokenId, address _buyerId)
        public
        whenNotPaused
    {
        require(_askAuctionsMap.contains(_tokenId), "Auction was not created");
        ...
        require(msg.sender != address(0), "Wrong msg sender");
        require(
            msg.sender == nft.ownerOf(_tokenId),
            "Only owner can sell token"
        );
        require(_buyerId != nft.ownerOf(_tokenId), "Can't sell to token owner");

        require(_asksMap.contains(_tokenId), "Token not in sell book");
        require(
            _tokenBuyers[auctionId][_buyerId] != 0,
            "Buyer did not bid yet"
        );
        require(
            quoteErc20.balanceOf(_buyerId) >= _tokenBuyers[auctionId][_buyerId],
            "Buyer does not have enough money"
        );

        nft.safeTransferFrom(nft.ownerOf(_tokenId), _buyerId, _tokenId);

        uint256 sellPrice = _tokenBuyers[auctionId][_buyerId];
    ...
    function getAuctionId(uint256 _tokenId) public view returns (uint256) {
        return _askAuctionsMap.get(_tokenId);
    }
