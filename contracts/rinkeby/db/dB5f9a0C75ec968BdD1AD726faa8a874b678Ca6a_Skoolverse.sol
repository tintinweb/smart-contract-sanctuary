...
contract Skoolverse is ERC721Upgradeable, ERC721PausableUpgradeable, OwnableUpgradeable {
  ...
  function stay(uint _scholarzId, uint _landId) internal {
    ...
    require(placedOn[tenantOf[_landId]] == 0, "Land is already occupied"); // land must not be occupied
  ...
  function activate(uint _landId) internal {
    ...
    require(block.timestamp - placedOn[tenantOf[_landId]] >= activationTime, "Land is not ready");
  ...
  function stayDuration(uint _tokenId) external view returns(uint) {
    return placedOn[_tokenId] == 0 ? 0 : block.timestamp - placedOn[_tokenId];
  }
