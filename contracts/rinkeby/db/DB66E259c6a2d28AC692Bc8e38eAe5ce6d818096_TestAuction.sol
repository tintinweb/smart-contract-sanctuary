...
import "ERC1155.sol";
import "T_AuctionBase.sol";


// we need to handle incoming tokens erc1155/holder

contract TestAuction is MintAuction {

    constructor(address _nftAddress, uint256 _networkCut){
        require(_networkCut <= 10000);
        networkFee = _networkCut;

        ERC1155 candidate = ERC1155(_nftAddress);
        require(candidate == ERC1155(candidate));
        nft = candidate;
    }

    function withdrawlBalance(uint256 _tokenId) external payable {
        require(
            _ownerOf(_tokenId) ||
    ...
    )public payable canBeStoredWith128Bits(_startPrice) canBeStoredWith128Bits(_highestBid) canBeStoredWith64Bits(_duration)
    {
        require(_ownerOf(_tokenId));
        ...
        auction.highestBidder=payable(msg.sender); // default start until someone enters
        auction.highestBid=0;
        _addAuction(_tokenId, auction);
    }

    function bid(uint256 _tokenId, uint128 amount) public payable{
        _bid(amount, _tokenId);
    }

    function endAuction(uint256 _tokenId) public payable {
        _onAuctionEnd(_tokenId);
    }
...
contract MintAuction is ERC1155Holder {
    ...
    function _transfer(address receiver, uint256 _tokenId) internal {
    ...
    function _addAuction(uint256 _tokenId, Auction memory _auction) public payable {
        require(_auction.duration >= 60);
        tokenAddressAndIdToAuction[address(nft)][_tokenId] = _auction;
        emit AuctionCreated(
            address(nft),
            uint256(_tokenId),
    ...
    function _removeAuction(uint256 _tokenId) internal {
        delete tokenAddressAndIdToAuction[address(nft)][_tokenId];
    }
    ...
    function _onAuction(Auction storage _auction) internal view returns(bool){
        return(_auction.duration + block.timestamp > block.timestamp);
    }
    ...
    function _bid(uint128 _amount, uint256 _tokenId) public payable returns(uint256){
        Auction storage auction = tokenAddressAndIdToAuction[address(nft)][_tokenId];
        require(_onAuction(auction));
        require(_amount > auction.highestBid);
        ...
        emit Bid(_amount, address(nft), _tokenId);
        return auction.highestBid;
    }

    function _onAuctionEnd(uint256 _tokenId) internal {
        Auction storage auction = tokenAddressAndIdToAuction[address(nft)][_tokenId];
        require(!_onAuction(auction));
        ...
        emit AuctionSuccessful(_tokenId, finalPrice, winner);
    }
