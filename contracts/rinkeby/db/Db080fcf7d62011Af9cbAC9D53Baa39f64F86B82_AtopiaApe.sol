...
contract AtopiaApe is AtopiaBase {
	...
	function mintWithToken(uint256 index) external onlyState(1) {
		require(tokens[index].limit > 0);
		tokens[index].limit -= 1;
		require(totalSupply() < 10_000);
		IBucks(tokens[index].token).transferFrom(msg.sender, admin, tokens[index].price);
		batch(msg.sender, 1, 5 * apeYear);
	}
	...
	function getRewards(uint256[] memory tokenIds) external view returns (uint256 rewards) {
		for (uint256 i = 0; i < tokenIds.length; i++) {
			...
			if (space.lives(tokenId) == 0) {
				rewards += getRewardsInternal(tokenId) + pendings[tokenId];
			} else {
