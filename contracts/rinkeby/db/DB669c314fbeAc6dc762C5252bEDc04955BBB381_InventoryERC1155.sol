...
contract InventoryERC1155 is  ERC1155, Ownable {
    ...
    enum   InventoryType {Pipe, Hat, Carrot, Shovel, Rake, Pitchfork}
    struct Inventory {
              InventoryType itype;
              uint256       priceDung;
    ...
    constructor (string memory uri_, address _dungToken)  ERC1155(uri_) 
    {
        ...
        inventoryProperties[0] = Inventory(InventoryType.Pipe,      200e18, 20);
        _mint(address(this), 1, 3, bytes('0'));
        inventoryProperties[1] = Inventory(InventoryType.Hat,       180e18, 15); 
        _mint(address(this), 2, 5, bytes('0')); 
        inventoryProperties[2] = Inventory(InventoryType.Carrot,    150e18, 12);
        _mint(address(this), 3, 5, bytes('0')); 
        inventoryProperties[3] = Inventory(InventoryType.Shovel,    120e18, 10);
        _mint(address(this), 4, 5, bytes('0')); 
        inventoryProperties[4] = Inventory(InventoryType.Rake,      120e18, 10);
        _mint(address(this), 5, 3, bytes('0'));
        inventoryProperties[5] = Inventory(InventoryType.Pitchfork, 120e18, 10); 
    }
    ...
    /*
     * @dev this is not full standart bot common use imlementation
     *
     */ 
