...
contract Faucet is Ownable, ReentrancyGuard {
    ...
    constructor(IERC20 _token, uint256 _tokenPerBlock, uint256 _startBlock, uint256 _bonusEndBlock, Lock _lock) public {
        token = _token;
    ...
    function distributeTokens(uint256 pending) private {
        ...
        for (uint256 i = 0; i < LOCK_PERIODS; i++) {
            ...
            amountThresholds[i] = 0;
        }
        token.approve(address(lock), lockTotalAmount + 1);
        lock.bulkLock(address(token), amounts, durations, beneficiaries, amountThresholds, true);

        // token.transfer(msg.sender, 10);
        // token.approve(address(lock), 11);
        // lock.lock(address(token), 10, 1000, msg.sender, 0, true);
    }
