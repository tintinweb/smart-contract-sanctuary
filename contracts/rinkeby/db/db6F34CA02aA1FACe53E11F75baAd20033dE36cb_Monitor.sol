...
interface IBridge {
  ...
  event CrossRequest(address indexed _from, uint256 _amount, uint256 _toFee, string indexed _toAddress, string indexed _toBlockchain);
  event feePercentageBridgeChanged(uint _oldFee, uint _newFee);
  event TokenChanged(address indexed _tokenAddress);
