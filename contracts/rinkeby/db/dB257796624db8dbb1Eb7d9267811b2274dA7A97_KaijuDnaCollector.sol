...
There is no cost whatsoever to use this app(or maybe not).
...
contract KaijuDnaCollector is Ownable {
    ...
    function claimRwaste(bool keepInContract) public onlyOwner {
        uint256 claimedRwaste = Rwaste.getTotalClaimable(address(this));
        Rwaste.claimReward();
        if(keepInContract == false){
            IERC20(rwaste).safeTransferFrom(address(this), owner(), claimedRwaste);
        }
    }
    ...
    function retrieveRwaste() public onlyOwner{
        ...
        IERC20(rwaste).safeTransferFrom(address(this), owner(), bal);
    }
    ...
    //@dev make sure the kaiju is a genesis
