...
contract NFT_Token is ERC721Enumerable, Ownable {
    using Strings for uint256;  //manipulate uint to strings

    string public baseURI;  //the base uri --> link do IPFS
    uint256 public publicCost = 0.07 ether;   
    uint256 public preCost = 0.06 ether;  
    uint256 public maxPublicSupply;
    uint256 public maxPreSupply;
    uint256 public currentSupply = 0;
    uint256 public maxPublicMintAmount = 10;  //the user can only mint up to 20 nfts
    uint256 public maxPreMintAmount = 5;  //the user can only mint up to 20 nfts
    ...
    function mint(uint256 _mintAmount) public payable{ 
        ...
        if(preSale == true){
            require(isWhiteListed(msg.sender), "Adress Not White Listed");
            require(_mintAmount <= maxPreMintAmount); 
            require(currentSupply + _mintAmount <= maxPreSupply); 
            require(msg.value >= preCost * _mintAmount);
            ...
            if(publicSale == true){
                ...
                require(currentSupply + _mintAmount <= maxPublicSupply); 
