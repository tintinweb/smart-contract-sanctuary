...
contract GenTickets is ERC1155, ERC1155Receiver {
    ...
    //A: mapping (_KeyType => _ValueType) mapName // suppose var genTicket = genTickets[0] // then we can call as genTicket.numTickets etc
    ...
    // Expected TGE timestamp, start at max uint256 //A: a maximum number is set if TGE not provided by project team while creation of Tickets
        ...
        ERC1155(_uri)
    {
        ...
        //A: currently the we have only 4 tickets type so loop will run for max 4 but maybe in future we have more than 4 so safer side 50
    ...
    //A: 1st of this new created contract issues ticket
    function issue(address _to) external {
        ...
        //A: We saw this issue coming up when underlying tokens were lesser in wallet and we tried depositing more tokens inside NFT tiekctes of 4 types while creation
        ...
        bytes memory data;
        //A: our case loop will run for 4 types of tickets
        for (uint8 i = 0; i < 50; i++){
            if (numTicketTypes == i){
                break;
            }
    ...
    //A: then later user can redeem the ticket on or after TGE date
    function redeemTicket(address _to, uint256 _id, uint256 _amount) public {

        //A: address _to is my account address from which the redeem is clicked and the token is also redeemed to the same account holding token i.e TGS example
        //A: _id is the type of tciket you chose to redeem i.e black-0 type or other
        //A: _amount is how many tickets of black type i.e suppose you have number of tickets =3 then amount should be 3? so if size=2 and cliff=2 and tranche=0, then on TGE first date, token redeemable equals = 3 and then next day=3 =>> total =6 please verify from Pavan

        // nbhasdhabjsbdj, id=2, 1

        // id=7, amount=1
        uint tier = _id.mod(numTicketTypes); // 7%4=3
        GenTicket memory ticketType = genTickets[tier]; //b p "g" silver
        ...
        // Tranche past cliff
        uint tranche = _id.div(numTicketTypes); // 7/4 = 1
        ...
        uint eligibleTraches = (getBlockTimestamp() - ((ticketType.trancheLength).mul(ticketType.cliffTranches).add(TGE)).div(ticketType.trancheLength));
        if (eligibleTraches > ticketType.totalTranches) {
            eligibleTraches = ticketType.totalTranches;
        }
        ...
        uint tranchesWiseTokens = ticketType.ticketSize.div((ticketType.totalTranches) - (ticketType.cliffTranches));
        uint averageTokensAvailableToClaim = tranchesWiseTokens.mul(ticketType.numTickets);
        uint temp = (ticketType.totalTranches).mul(tranche);
        uint redeemedTokens=0;
        if (temp > 0){
            redeemedTokens  = ((ticketType.ticketSize).mul(ticketType.numTickets)).div(temp);
        }
        uint tokens = (eligibleTraches.mul(averageTokensAvailableToClaim)) - redeemedTokens;

        // NFT transfer// address(this) ticket ka address
        safeTransferFrom(address(msg.sender), address(this), _id, _amount, VALIDATOR);

        // Transfer underlying tokens with corresponding ticket size //A: we need to change our code here to fix vesting issue
