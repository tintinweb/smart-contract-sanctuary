...
contract Nview is NPassCore {
  ...
  /**
    ...
    */
  constructor(address _nContractAddress) NPassCore("Nview", "Nv", IN(_nContractAddress), true, 8888, 0, 0, 0) {}
  ...
  function getColorFromTokenId(uint[8] memory tokenIds) public pure returns(string memory){
    string memory c;
    uint sum_numbers = getSumFromTokenIdsNumbers(tokenIds);
    // check sum of numbers for corresponding color
    if(sum_numbers <= 27){
      c = colorRGBtoHex(245, 183, 177); //red
    }
    else if(sum_numbers > 27 && sum_numbers <= 32){
      c = colorRGBtoHex(245, 137, 252);  //purple
    }
    else if(sum_numbers > 32 && sum_numbers <= 38){
      c = colorRGBtoHex(114, 198, 255 );  //blue
    }
    else if(sum_numbers > 38 && sum_numbers <= 42){
      c = colorRGBtoHex(136, 237, 217);  //green-blueish
    }
    else if(sum_numbers > 42 && sum_numbers <= 46){
      c = colorRGBtoHex(131, 238, 177);  //green
    }
    else if(sum_numbers > 46 && sum_numbers <= 50){
      c = colorRGBtoHex(252, 227, 127);  //yellow
    }
    else if(sum_numbers > 50 && sum_numbers <= 55){
      c = colorRGBtoHex(254, 192, 139); //Peach- orange
    }
    else if(sum_numbers > 55 && sum_numbers < 60){
      c = colorRGBtoHex(240, 157, 235); //pink
    }
    else if(sum_numbers >= 60){
      c = colorRGBtoHex(247, 249, 249);  //white
    }
    return c;
  }
  ...
  function getColorStringFromTokenId(uint[8] memory tokenIds) public pure returns(string memory){
    string memory c;
    uint sum_numbers = getSumFromTokenIdsNumbers(tokenIds);
    // check sum of numbers for corresponding color
    if(sum_numbers <= 27){
  ...
  function color_string(uint tokenID) public view returns (string memory){
    return getColorStringFromTokenId(getTokenIDsNumbers(tokenID));
  }

  function getTokenSVG(uint tokenId) public view returns (string memory) {
    require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");

    uint[8] memory tokenIdNumbers = getTokenIDsNumbers(tokenId);
    string memory _color = getColorFromTokenId(tokenIdNumbers);
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
    string memory tokenSvg = getTokenSVG(tokenId);
...
abstract contract NPassCore is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        n = n_; // N OwnerRegistry 0x731B19acAA0aE77aEa8bf8515F6DCdB894c248C2
