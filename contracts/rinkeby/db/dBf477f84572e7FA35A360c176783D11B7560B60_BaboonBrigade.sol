...
contract BaboonBrigade is ERC721Enumerable, Ownable {
    ...
    /// @notice mint amount for each stage
    uint256 public constant BBG_GIFT_MAX = 100; // gift stage
    uint256 public constant BBG_PRESALE_MAX = 250; // presale first
    uint256 public constant BBG_MAX = 7777; // total amount

    /// @notice limit amount per mint for each stage
    uint256 public constant BBG_GIFT_LIMIT = 1; // gift stage
    uint256 public constant BBG_PRESALE_LIMIT = 5; // presale first stage
    uint256 public constant BBG_PUBLIC_LIMIT = 10; // public stage

    /// @notice price of each NFT for each stage
    uint256 public constant BBG_GIFT_PRICE = 0; // gift stage - free
    ...
    uint256 public BBG_PER_PRICE = BBG_GIFT_PRICE; // per price
    uint256 public BBG_PER_MINT = BBG_GIFT_LIMIT; // limit per mint
	...
	/// @notice authorized whitelist for presale stage
    ...
    event Sold(uint256 tokenId, address sender, uint256 payAmount);

    constructor() ERC721("BaboonBrigade", "BBG") {}

    /**
     * @notice add wallet addresses to whitelist on presale first stage
     * @dev Only admin
     * @param entries wallet address to be added on whitelist
     */
    function addToPresaleList(address[] calldata entries)
        external
        onlyOwner
    {
    ...
    /**
     * @notice remove addresses from presale list
     * @dev Only admin
     * @param entries wallet addresses to be removed from list
     */
    ...
    function buy(uint256 tokenQuantity) external payable {
		...
		if(_state != State.GIFT) {
			require(
				BBG_PER_PRICE * tokenQuantity <= msg.value,
				"Not Enough payments"
			);
		} else {
			require(msg.sender == owner(), "Owner can only do gift");
		}
		if (_state == State.PRESALE) {
            require(presalerList[msg.sender], "Not Authorize presale first");
        }

        for (uint256 i = 0; i < tokenQuantity; i++) {
			uint256 tokenId = totalSupply() + 1;
            _safeMint(creatorAddress, tokenId);
			_transfer(creatorAddress, msg.sender, totalSupply());
            companyAddress.transfer(msg.value);
            emit Sold(tokenId, msg.sender, BBG_PER_PRICE);
        }
    }
        ...
        returns (string memory)
    {
		...
		if(tokenId > BBG_GIFT_MAX && tokenId <= BBG_GIFT_MAX + BBG_PRESALE_MAX) {
			baseUri = tokenBaseURIPresale;
		} else if(tokenId > BBG_GIFT_MAX + BBG_PRESALE_MAX) {
     ...
     */
    function isInPresaleList(address addr) external view returns (bool) {
    ...
    /**
     * @notice set presale metadata uri
     * @param _tokenBaseURI token ID which need to be finished
     */
	function setTokenBaseURIPresale(string memory _tokenBaseURI) public onlyOwner {
		tokenBaseURIPresale = _tokenBaseURI;
	}
