...
contract S16Distributor is AccessControl, Ownable {
    ...
    uint256 private userPreSaleMaxEditionCap = 4; //TODO will change to 10 on mainnet // confirm again from client
    uint256 private userPublicSaleMaxEditionCap = 4; //TODO will change to 10 on mainnet // confirm again from client

    bool public _mintingPaused = false;
    uint256 private _trackerTokenId = 1;
    ...
    constructor(address _s16NFT, address _s16Token, address _s16PreSale,address _s16AdminWallet) {
        ...
        PRE_SALE_START_TIME = 1642145463; //PreSale Time //TODO change time on mainnet
        PUBLIC_SALE_TIME_START = 1642120263; //Public Sale Time //TODO change time on mainnet
    }
    ...
    function preSaleMint(address wallet, uint256 _editionQty, bool _claimNow) public payable {
       ...
       (uint256[] memory tokenIds, uint256[] memory editionQty) = _mintTokensEditions(_editionQty);
        ...
        if(_claimNow) {
            s16NFT.mintEditions(wallet, tokenIds, editionQty);
    ...
    function _mintTokensEditions(uint256 _editionQty) internal returns (uint256[] memory, uint256[] memory) {
     ...
     function claim() public {
        ...
        claimTokens[_wallet].isClaimed = true;
         s16NFT.mintEditions( _wallet,claimS16NFTs[_wallet].tokenIds,claimS16NFTs[_wallet].quantity);
