...
contract Saja is Context, IERC20, Ownable {
    ...
    uint256 private _cradleRate = 200;  //2%
    ...
    address private _tctBurnWallet = 0x1aE0EA34a72D944a8C7603FfB3eC30a6669E454C;
    address private _tctCommunityWallet = 0x17F6AD8Ef982297579C203069C1DbfFE4348c372;
    ...
    constructor () {
        ...
        _isExcludedFromRewards[0x9934AA2AF6604aD463afD2a9F5e0C24b84481268] = true; //wallet for uniswap liquidity pool
        _isExcludedFromRewards[0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D] = true; //wallet for uniswap liquidity pool
    ...
    function getTierSettings() public view returns (uint256, uint256, uint256, uint256,
        uint256, uint256, uint256, uint256, uint256)  {
        return (_tctCubTier, _tctLionessTier, _tctLionessTier, _tctAlphaTier,
        _cradleRate, _cubRewardRate, _lionessRewardRate, _lionessRewardRate, _alphaLionRewardRate);
    }
    ...
    function setRewardRates(uint256 defaultRate, uint256 cubRate,
        uint256 lionessRate, uint256 lionRate,
        uint256 alphaLionRate) external onlyOwner() {
        _cradleRate = defaultRate;
    ...
    function setTctRewardTier(uint256 alphaTier, uint256 lionTier, uint256 LionessTier, uint256 cubTier) external onlyOwner() {
    ...
    function setPromotion(uint256 promotionRate, bool doPromotion) public onlyOwner {
        _doPromotion = doPromotion;
        _promotionRate = promotionRate;
    }

    function initTctContract(address contractAddress) public onlyOwner {
    ...
    function _transferStandard(address sender, address recipient, uint256 amount) private {
        ...
        uint256 defaultRewardAmount = amount.div(_cradleRate);
    ...
    function distributeTokens(uint256 defaultAmount, uint256 cubAmount, uint256 lionessAmount, uint256 lionAmount, uint256 alphaLionAmount) private {
        ...
        for (uint256 i = 0; i < _holders.length; i++) {
            ...
            if (holderAddress != _deadAddress && holderAddress != owner()) {
    ...
    function includeAccounts(address[] memory addresses) external onlyOwner() {
        for (uint256 i = 0; i < addresses.length; i++) {
    ...
    function excludeAccounts(address[] memory accounts) external onlyOwner() {
