...
contract BullRunDividends is AccessControlProxyPausable {
  ...
  function payDividends(bytes32 merkleRoot_, string memory uri_) public onlyRole(DIVIDENDS_PAYER_ROLE){
  ...
  function claimable(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) public view returns(uint256){
    ...
    require(MerkleProofUpgradeable.verify(merkleProof, merkleRoot, node), "BullRunDividends: invalid proof");
  ...
  function _distribute(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof, address to, uint256 distribution) internal whenNotPaused {
    ...
    require(distributable > 0, "BullRunDividends: nothing to distribute");
    require(distribution <= distributable, "BullRunDividends: amount to distribute exceeds dividends balance claimable");

    IBullRunConfig configInterface = IBullRunConfig(config);
    IERC20 dividendsInterface = IERC20(configInterface.usd());
    address vault = configInterface.vault();
    distributed[account] = distributed[account] + distribution;
    if (account != to){
      require(dividendsInterface.transferFrom(vault, to, distribution), "BullRunDividends: dividends transfer failed");
      ...
      // require(dividendsInterface.transfer(to, retention), "BullRunDividends: retention transfer failed");
      require(dividendsInterface.transferFrom(vault, account, claimed), "BullRunDividends: dividends transfer failed");
  ...
  function _retention(address account, uint256 amount) internal view returns(uint256){
    ...
    return (amount * whitelistInterface.retention(account)) / 1e20;
  }
...
interface IBullRunWhitelist {
  ...
  function retention(address account) external view returns(uint256);
...
interface IBullRunConfig {
    ...
    function initialize(address token_, address whitelist_, address distributor_, address vault_, address usd_) external;
    ...
    function updateUsd(address token_) external;
