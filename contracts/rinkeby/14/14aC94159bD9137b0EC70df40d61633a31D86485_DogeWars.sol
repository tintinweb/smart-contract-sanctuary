...
contract DogeWars is ERC721, VRFConsumerBase, Ownable {
   ...
   function Attack (uint256 tokenId, uint256 _tokenId , uint256 _number)  public  returns (uint256 _winner)   {
     ...
     Game memory newGame = Game(index, tokenId, _tokenId, _number, ownerOf(_winner), block.timestamp);
       ...
       if ( sqrt(_number ** 2) + ( characters[tokenId].strength ** 10/3) + (characters[tokenId].intelligence ** 10/3) > (characters[_tokenId].strength ** 10/3) + (characters[_tokenId].intelligence ** 10/3) ** _number ){
