...
abstract contract AllowList {
    ...
    function addToAllowList(address[] memory addresses) external {
    ...
    function removeFromAllowList(address[] memory addresses) external {
        for(uint index = 0; index < addresses.length; index+=1) {
    ...
    function allowAddressByIndex(uint256 index) external view returns (address) {
        require(index < _allowList.length(), "AL: index out of bounds");
...
contract NameTag is ERC721Enumerable, Ownable, StringUpper, DenyList, ReentrancyGuard {
    using EnumerableSet for EnumerableSet.AddressSet;
    EnumerableSet.AddressSet private _allowList;
    ...
    ) external onlyOwner whenPresalePaused {
    ...
    function removeFromAllowList(address[] memory addresses) external onlyOwner whenPresalePaused {
    ...
    function startPublicSale(uint256 salePrice_, uint8 saleTransactionLimit_) external onlyOwner whenSalePaused {
    ...
    function _preValidatePurchase(uint256 tokensAmount) internal view returns(bool) {
        ...
        require(tokensAmount <= saleTransactionLimit, "NT: Limited amount of tokens");
    ...
    function _setName(uint256 tokenId, string memory _name) internal virtual returns(bool){
        bool status;
