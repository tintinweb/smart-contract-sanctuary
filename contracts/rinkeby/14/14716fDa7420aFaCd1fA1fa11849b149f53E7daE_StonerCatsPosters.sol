...
contract StonerCatsPosters is ERC721Enumerable, Ownable{
    ...
    function unclaimed() public view returns(uint[] memory){
        uint unclaimedTokens;
        uint balance = stonerCats.balanceOf(msg.sender);
        for(uint i = 0; i < balance; i++){
            if(claimable(stonerCats.tokenOfOwnerByIndex(msg.sender,i))){
                unclaimedTokens++;
            }
        }
        ...
        unclaimedTokens = 0;
        for(uint i = 0; i < balance; i++){
            uint tokenId = stonerCats.tokenOfOwnerByIndex(msg.sender,i);
    ...
    function claimed() public view returns(uint[] memory){
        uint balance = balanceOf(msg.sender);
        uint[] memory _tokenIds = new uint256[](balance);
        for(uint i = 0; i < balance; i++){
            _tokenIds[i] = tokenOfOwnerByIndex(msg.sender,i);
        }
        return _tokenIds;
    }
