...
contract SomaNetwork is ISomaNetwork, ProposedOwnableUpgradeable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    mapping (address => EnumerableSet.Bytes32Set) private _permissions;
    ...
    function initialize(address owner_) external initializer {
        ...
        grantRole(PAUSE_ROLE, owner_);
        grantRole(ASSIGN_KEY_ROLE, owner_);
        grantRole(NETWORK_MANAGER_ROLE, owner_);

        grantRole(BASIC_ROLE_MANAGER, owner_);
        grantRole(APPROVE_ACCESS_ROLE, owner_);
        grantRole(LOCAL_REVOKE_ACCESS_ROLE, owner_);
        grantRole(GLOBAL_REVOKE_ACCESS_ROLE, owner_);
    ...
    function rolesOf(address account) public view override returns (bytes32[] memory) {
        return _permissions[account]._inner._values;
    }
    ...
    function add(address target) external override {
        ...
        if (AddressUpgradeable.isContract(target)) {
            try IERC165(target).supportsInterface(type(INetworkable).interfaceId) returns (bool) {

            INetworkable networkable = INetworkable(target);

            bytes32 key = networkable.NETWORK_KEY();
            if (key != 0) _assignToKey(key, target);

            _grantPermissions(networkable.REQUIRED_PERMISSIONS(), target);
    ...
    function remove(address target) external override {
        ...
        _revokeAllPermissions(target);
        _revokeAccess(target);
    }

    function grantRole(bytes32 role, address account) public virtual override(AccessControlUpgradeable, IAccessControlUpgradeable) {
        super.grantRole(role, account);
        _permissions[account].add(role);
    }

    function revokeRole(bytes32 role, address account) public virtual override(AccessControlUpgradeable, IAccessControlUpgradeable) {
        super.revokeRole(role, account);
        _permissions[account].remove(role);
    }
    ...
    function _beforeOwnershipTransfer(address prevOwner, address newOwner) internal override {
        ...
        _grantPermissions(rolesOf(prevOwner), newOwner);

        if (prevOwner != address(0)) {
            _revokeAllPermissions(prevOwner);
            _revokeAccess(prevOwner);
        }
    }

    function _revokeAllPermissions(address target) private {
        bytes32[] memory permissions = rolesOf(target);
        for (uint i = 0; i < permissions.length; i++) {
            revokeRole(permissions[i], target);
        }
    }

    function _grantPermissions(bytes32[] memory permissions, address target) private {
        for (uint i = 0; i < permissions.length; i++) {
            grantRole(permissions[i], target);
        }
    }
 ...
 * the owner.
 */
abstract contract ProposedOwnableUpgradeable is Initializable, ContextUpgradeable {
     ...
     */
    function proposeOwnershipTransfer(address proposedOwner) external onlyOwner returns (bool) {
