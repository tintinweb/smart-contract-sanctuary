...
contract SipherIBCO is Ownable {
    ...
    uint256 public constant START = 1637855793; //1638378000;
    ...
    function claim() external {
        ...
        uint256 sipherAmount = _getEstReceivedToken();
    ...
    function _getEstReceivedToken() internal view returns (uint) {
        uint userShare = provided[msg.sender];
        return (TOTAL_DISTRIBUTE_AMOUNT * userShare) / Math.max(totalProvided, MINIMAL_PROVIDE_AMOUNT);
    }

    /**
     * @dev Calculate withdrawable ether from accumulated ether
     */
    function getWithdrawableInvestment(uint256 userAccumulated) external pure returns (uint256) {
        return _withdrawCap(userAccumulated);
    }
    
    /**
     * @dev Calculate locked etehr from accumulated Ether
     */
     ...
     */
    function getEstReceivedToken() external view returns (uint) {
        return _getEstReceivedToken();
    }
