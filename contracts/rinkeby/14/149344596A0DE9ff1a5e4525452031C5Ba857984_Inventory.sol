...
contract Inventory {
    ...
    function getTokenURI(uint16 id_, uint8 body_, uint8 helm_, uint8 mainhand_, uint8 offhand_, uint16 level_, uint16 zugModifier) public view returns (string memory) {
    ...
    function getBodyAttributes(uint8 body_) internal pure returns(string memory) {
        return string(abi.encodePacked('{"trait_type:":"Body","value":"',getBodyName(body_),'"},{"trait_type:":"Body Rarity","value":',toString(getBodyRarity(body_)),'}'));
    }

    function getHelmAttributes(uint8 helm_) internal pure returns(string memory) {
        return string(abi.encodePacked('{"trait_type:":"Helm","value":"',getHelmName(helm_),'"},{"trait_type:":"Helm Tier","value":',toString(getTier(helm_)),'}'));
    }

    function getMainhandAttributes(uint8 mainhand_) internal pure returns(string memory) {
        return string(abi.encodePacked('{"trait_type:":"Mainhand","value":"',getHelmName(mainhand_),'"},{"trait_type:":"Mainhand Tier","value":',toString(getTier(mainhand_)),'}'));
    }

    function getOffhandAttributes(uint8 offhand_) internal pure returns(string memory) {
        return string(abi.encodePacked('{"trait_type:":"Offhand","value":"',getHelmName(offhand_),'"},{"trait_type:":"Offhand Tier","value":',toString(getTier(offhand_)),'}'));
    }
    ...
    function getBodyRarity(uint8 id) internal pure returns(uint8) {
        if (id < 10) return 1;
        if (id < 16) return 2;
        if (id < 20) return 3;
        return 100;
