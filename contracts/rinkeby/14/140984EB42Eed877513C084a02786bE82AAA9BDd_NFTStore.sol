...
contract NFTStore is EIP712, Ownable {
	...
	*/
	...
	function mintPreSaleSigned(uint256 _amount,uint256 _passAmount, bytes32 r, bytes32 s, uint8 v) external payable preSaleOnTest {
		require(_amount <= mintedPerTxMax, "exceeds max tx");
		require(_amount <= mintedMax - mintedSoFar, "exceeds avail");
		require(price * _amount <= msg.value, "exceeds caller spend");
		vipSignedFinalize(_amount,_passAmount,r,s,v);
		mintedSoFar+=_amount;
		for (uint256 i = 0; i < _amount; i++) {
			nftcontract.mint(msg.sender);
		}
...
//		vipBloomFilter.add( bytes32(uint256(uint160(_address)) << 96) );
	...
	/*
	function vipSignedFinalizeMore(uint256 _amount, uint256 _max, bytes32 r, bytes32 s, uint8 v ) internal {
		require(_amount <= mintedMax - mintedSoFar, "exceeds max mint");
		require(_amount <= mintedVIPMax - mintedSoFar, "exceeds vip avail");
		require(_amount <= _max - mintedPerVIP[msg.sender], "exceeds mint avail for you");
		...
		bytes32 hashStruct = keccak256(abi.encode(TYPEHASH,msg.sender,_max));
