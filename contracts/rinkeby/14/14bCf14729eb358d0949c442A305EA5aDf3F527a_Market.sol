...
interface IMarket {
    ...
    event MakeDeal(address render,address lender,address nftAddress,uint256 nftId,uint64 startTime,uint64 endTime,uint256 secondPrice);
...
contract Market is OwnableContract,IMarket{
    ...
    function mintWNftAndOnLent (
        ...
        uint64 minDuration,
        uint256 secondPrice
    ) public onlyApprovedOrOwner(msg.sender,IBaseDoNFT(doNftAddress).getoNftAddress(),oNftId){
        uint256 nftId = IBaseDoNFT(doNftAddress).mintWNft(oNftId);
        onLent(doNftAddress, nftId, maxEndTime,minDuration, secondPrice);
    }
    ...
    function onLent(
        ...
        if(isOnLent(nftAddress,nftId)){
            setLent(nftAddress,nftId,maxEndTime,secondPrice);
        }else{
            creditMap[nftAddress].lendingMap[nftId] = Lending(msg.sender,nftAddress,nftId,secondPrice,maxEndTime,minDuration,IBaseDoNFT(nftAddress).getNonce(nftId),true);
        }
        emit OnLent(msg.sender,nftAddress, nftId, maxEndTime,minDuration,secondPrice);
    }

    function offLent(address nftAddress, uint256 nftId) public onlyApprovedOrOwner(msg.sender,nftAddress,nftId){
        creditMap[nftAddress].lendingMap[nftId].isOn = false;
    ...
    function setLent(
        ...
        creditMap[nftAddress].lendingMap[nftId].maxEndTime = maxEndTime;
        creditMap[nftAddress].lendingMap[nftId].secondPrice = secondPrice;
    }
    
    function makeDeal(address nftAddress,uint256 tokenId,uint256 durationId,uint64 startTime,uint64 endTime) public payable virtual returns(uint256 tid){
    ...
    function distributePayment(address nftAddress,uint256 nftId,uint64 startTime,uint64 endTime) public payable returns (uint256 totolPrice,uint256 leftTotolPrice,uint256 curFee,uint256 curRoyalty){
        Lending storage lending = creditMap[nftAddress].lendingMap[nftId];
        totolPrice = lending.secondPrice * (endTime - startTime);
    ...
    function isOnLent(address nftAddress,uint256 nftId) public view returns (bool){
        return creditMap[nftAddress].lendingMap[nftId].isOn;
    }
