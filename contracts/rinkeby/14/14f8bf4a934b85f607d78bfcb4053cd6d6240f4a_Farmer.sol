...
contract QfourToken is Once, Pausable, OwnableNoUpgrade, ERC20Votes {
...
interface IFarmer {
    ...
    */
    ...
    function getStakinInfo() external view returns( uint, uint64, uint, uint, uint);
...
abstract contract SubFarmer is Context, IFarmer {
    ...
    QfourToken public tokenAddr; ///QFour Token contract instance
    ...
    uint public treasury; // Farmer's treasury. Equivalent to maxSupply.
    uint public currentSupply; ///NOTE It may not sync with the totalSupply if fanTip is activated
    uint public cumFee; // Total fee generated at any time.
    
    constructor(uint supply, QfourToken _token) {
        treasury = supply * 10**18;
        _getter = Info(
            20.0e18 *(10**18),
            28800,
            0,
    ...
    */
    ...
    /**@dev Sets ERC20Token(QfourToken) address to a new one
    */
    function _upgradeERC20Address(QfourToken _tkAddr) internal virtual returns(uint8) {
        tokenAddr = _tkAddr;
        return 1;
    }
    ...
    ///@notice checkFlags if user has stake in the pool
    ...
    */
    ...
    function _sync(uint _with, uint8 cmd) private {
        ...
        (currentSupply, treasury) = cmd == 0 ? (
            supply.add(_with), treasury.sub(_with)
            ) : (supply.sub(_with), treasury);
    }
    ...
    function _stakingInfo() internal view virtual returns(
        ...
        uint) {
            ...
            _total = _getter._totalStaked;
        
            return (apr, avgbpd, minDur, _total, _getMinStake());
    }
    ...
    /**@dev User can stake directly from their locked balance.
        param - 'data' : Additional information from the user.
     */
    function _approveAndStake(
        address staker, 
        uint8 _flag, 
        address _proxy, 
        uint amount, 
        bytes memory data) internal virtual returns(uint8 ret) {
            _stakes[staker].count ++;
            if(_flag == 1) {
                require(Address.isContract(_proxy), "Denied");
            }
            ret = tokenAddr.approveAndStake(staker, amount, keccak256(data));
            uint(ret).isGThan(0);
            if(_completeStake(staker, _flag, _proxy, amount)) { ret = 1; }
            return ret;
    }
...
contract Farmer is OwnableNoUpgrade, SubFarmer, ReentrancyGuard {
    ...
    //solhint-disable-next-line
    constructor(QfourToken _token) SubFarmer(500_000_000, _token) {}
    ...
    function _minFee() internal view returns(uint ret) {
        ret = minFee;
        return ret;
    }
    ...
    */
    ...
    ///See SubFarmer
    function upgradeErc20Address(QfourToken _tkAddr) external onlyOwner returns(uint8) {
    ...
    */
    ...
    ///See SubFarmer
    function getStakinInfo() public view override returns(uint, uint64, uint, uint, uint) {
        return _stakingInfo();
    }
    ...
    function approveAndStake(
        ...
        address randAddress,
        uint nonce,
        uint amount) public whenNotPaused nonReentrant returns(uint8) {
            _increment(0);
            _approveAndStake(_msgSender(), 0, address(0), amount, abi.encode(pwd, randAddress, nonce));
            return 1;
    }

    ///@dev User can stake directly from their locked balance.
    function approveAndStakeByProxy(
        address src,
        address _proxy,
        string memory pwd,
        address randAddress,
        uint nonce,
        uint amount) external payable whenNotPaused nonReentrant returns(uint8) {
            _increment(1);
            return _approveAndStake(src, 1, _proxy, amount, abi.encode(pwd, randAddress, nonce));
    }
    ...
    /**@dev Compute user's signature
        ...
        @notice It is advuisable to use number combinations 
    */
    function _getLock(string memory pwd, address randAddress, uint nonce) private pure returns(bytes32) {
        return keccak256(abi.encode(pwd, randAddress, nonce));
    }
    ...
    function toggleFanTip(uint8 cmd) public onlyOwner returns(bool ret) {
        ret = tokenAddr.toggleFanTip(cmd);
        ret.isTrue();
        return ret;
    }
