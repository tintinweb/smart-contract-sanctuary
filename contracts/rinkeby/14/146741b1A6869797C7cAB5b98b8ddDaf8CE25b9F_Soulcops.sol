...
contract Soulcops is EIP712, ERC721EnumerableM, Ownable {
    ...
    uint256 public constant MAX_GIFT = 500;
    uint256 public constant MAX_PRESALE = 1600 + MAX_GIFT;
    uint256 public constant TOTAL = 8000;
    uint256 public constant PRICE = 0.1 ether;
    uint256 public constant MAX_NFT_SALE = 3;
    uint256 public giftQtyMinted;
    ...
    function setWhitelistSigner(address _whitelistSigner) external onlyOwner {
        require(_whitelistSigner != whitelistSigner, "Signer same!");
    ...
    function presaleQty() public view returns (uint256) {
        return MAX_PRESALE - giftQtyMinted;
    }

    function saleQty() external view returns (uint256) {
        return TOTAL - presaleQtyMinted - giftQtyMinted;
    }

    function gift(address receiver,uint256 tokenQuantity) external onlyOwner {
        uint256 supply = totalSupply();
        require(supply <= TOTAL, "All Soulcops minted");
        require(giftQtyMinted + tokenQuantity <= MAX_GIFT, "Gift Empty");
    ...
    function presaleBuy(uint256 tokenQuantity, uint256 maxCount, bytes memory signature) external payable {
        ...
        require(supply <= TOTAL, "All Soulcops minted");
        require(presaleQtyMinted + tokenQuantity <= presaleQty(), "Minting would exceed the presale allocation");
        require(listPresalePurchases[msg.sender] + tokenQuantity <= maxCount, "You can not mint exceeds maximum NFT");
        require(PRICE * tokenQuantity <= msg.value, "Insufficient ETH sent");
    ...
    function _baseURI() internal view override(ERC721M) returns (string memory) {
        return _tokenBaseURI;
    }

    function tokenURI(uint256 tokenId) public view override(ERC721M) returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");

        string memory baseURI = _baseURI();
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenId.toString())) : _defaultBaseURI;
