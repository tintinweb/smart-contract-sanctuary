...
contract StakingErc1155 is ReentrancyGuard, Pausable, Ownable {
  ...
  mapping(uint256 => mapping(address => uint256)) public nftTokenBalances;
  ...
  function getNftTokenMap(uint256 tokenId) external view returns (bool, bool, uint256, uint256) {
    return (
  ...
  function balanceOf(uint256 tokenId, address account) external view returns (uint256) {
    return nftTokenBalances[tokenId][account];
  }
  ...
  function initNftToken(uint256 _tokenId, uint256 _maxAmount, uint256 _strengthWeight) public onlyOwner {
    require(!nftTokenMap[_tokenId].hasValue, 'NFT token is already defined');
    nftTokens.push(_tokenId);
    nftTokenMap[_tokenId] = NftToken({ hasValue: true, active: true, maxAmount: _maxAmount, strengthWeight: _strengthWeight });
  }
    
  function initNftTokenBatch(uint256[] calldata _tokenIds, uint256[] calldata _maxAmounts, uint256[] calldata _strengthWeights) public onlyOwner {
    require(_tokenIds.length == _maxAmounts.length && _tokenIds.length == _strengthWeights.length, 'Invalid params length');
    
    for(uint8 i = 0; i < _tokenIds.length; i++) {
      initNftToken(_tokenIds[i], _maxAmounts[i], _strengthWeights[i]);
    }
  }

  function updateNftToken(uint256 _tokenId, uint256 _maxAmount, uint256 _strengthWeight) public onlyOwner {
  ...
  function stakeInternal(uint256 tokenId, uint256 amount) internal {
    require(nftTokenMap[tokenId].active, "NFT token not defined");
    require(nftTokenBalances[tokenId][msg.sender].add(amount) <= nftTokenMap[tokenId].maxAmount, "NFT max");
    
    
    uint256 strength = nftTokenMap[tokenId].strengthWeight.mul(amount);
    strengthWeight[msg.sender] = strengthWeight[msg.sender].add(strength);
    _totalStrength = _totalStrength.add(strength);

    _totalSupply = _totalSupply.add(amount);
    nftTokenBalances[tokenId][msg.sender] = nftTokenBalances[tokenId][msg.sender].add(amount);
  ...
  function withdrawNFTInternal(uint256 tokenId, uint256 amount) internal {
    ...
    nftTokenBalances[tokenId][msg.sender] = nftTokenBalances[tokenId][msg.sender].sub(amount);
  ...
  function withdraw() public updateReward(msg.sender) {
    ...
    for (uint8 i = 0; i < nftTokens.length; i++) {
      uint256 balance = nftTokenBalances[nftTokens[i]][msg.sender];
      if(balance > 0) {
        length++;
      }
    }
    ...
    for (uint8 i = 0; i < nftTokens.length; i++) {
      ...
      uint256 balance = nftTokenBalances[tokenId][msg.sender];
      if(balance > 0) {
        tokenIds[j] = tokenId;
        amounts[j] = balance;
        j++;
