...
contract BinaryOptions is Ownable, Authorizable, Pausable, Oracle {
    ...
    constructor(
        ...
        // oracleToken.transfer(address(this), 1 * (10 ^ 17));
        // oracleToken.transferFrom(msg.sender, address(this), 1 * (10**17));
        // emit OracleTokenTransfer(msg.sender, address(this), 1 * (10**17));
        emit Deployment(
            _strikePrice,
            _bidPeriod,
            _contractExpiryTime,
    ...
    //all events
    event Bid(address biddingAddress, uint256 bids, int256 amount);
    event ResultLong(uint256 strike, uint256 actual);
    event ResultShort(uint256 strike, uint256 actual);
    event PriceFeed(uint256 oraclePrice);
    event Paid(address payerAddress, uint256 amount);
    event Withdraw(address winner, uint256 amount);
    // event OracleTokenTransfer(address sender, address receiver, uint256 amount);
    ...
    function bidLong(uint256 _bids) public whenNotPaused {
        ...
        dvdToken.transferFrom(
            senderAddress,
            address(this),
            _bids * dvdDecimalAdjust
        );
        dvdBalance[senderAddress] = _bids;
        longBid++;
        totalBids++;
        longMap[longBid] = senderAddress;
        userLongMap[senderAddress] = userLongMap[senderAddress] + 1;
        emit Bid(senderAddress, _bids, int256(_bids * dvdDecimalAdjust));
    }
    ...
    function announceResult() public onlyOwner whenNotPaused {
        ...
        if (currentPrice > strikePrice) {
            ...
            for (uint256 i = 1; i <= longBid; i++) {
                ...
                emit ResultLong(strikePrice, currentPrice);
            ...
            for (uint256 i = 1; i <= shortBid; i++) {
                ...
                // approve dvdtoken withdrawals based on number of tickets
                emit ResultShort(strikePrice, currentPrice);
                approveWithdrawal(winner, dvdBalance[winner]);
            }
        }
    }
    ...
    function approveWithdrawal(address _winner, uint256 _amount) internal {
        dvdToken.approve(_winner, _amount);
    }
    ...
    function getAssetPairPrice() external returns (uint256) {
        currentPrice = uint256(getPrice(aggregator));
        emit PriceFeed(currentPrice);
        return currentPrice;
    }
    ...
    // function getUserBids(address _address) external view returns (uint256) {
    ...
    // function getUserShorts(address _address) external view returns (uint256) {
    //     return userShortMap[_address];
    // }
    function getUserShorts() external view returns (uint256) {
        return userShortMap[msg.sender];
    }

    // function getUserLongs(address _address) external view returns (uint256) {
    //     return userLongMap[_address];
    // }
