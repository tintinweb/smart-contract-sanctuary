...
contract Test0101 is ERC721Enumerable, Ownable, ReentrancyGuard {
	...
	string private _baseTokenURI = 'ipfs://QmYSp8bXno4SfDihmg5dJr8qAy4Tuc7N8t6WBRsCMvrD52/';

	// ECDSA
	address private constant SIGNER_ADDRESS = 0xFA025098d2298c5a3f6bE56B7C9B0Be63098E2B5; // TODO: change it
	...
	// Withdraw address
	address private WITHDRAW_ADDRESS = 0x7339e1763f81aD5Abd22B41BE71b47C09829932E; // TODO: type the right withraw address
	...
	modifier onlySignedTx(
		...
		require(
			keccak256(abi.encodePacked(msg.sender, _amount, _nonce)).toEthSignedMessageHash().recover(_signature) ==
				SIGNER_ADDRESS,
	...
	function premint(uint256 _amount) private {
		...
		require(_mintStatus >= _premintTier[msg.sender], 'Minting is still closed for your tier');
		require(
			_premintClaimed[msg.sender] < TIER_LIMIT[uint256(_mintStatus)],
			'You already claimed your premint NFTs'
		);
		require(_amount <= TIER_LIMIT[uint256(_mintStatus)], 'Exceeded the max amount of NFT mintable in your tier');
		require(
			_amount + _premintClaimed[msg.sender] <= TIER_LIMIT[uint256(_mintStatus)],
			'You do not have enough premint available'
		);

		if (_premintTier[msg.sender] == MintStatus.DIAMOND && _premintClaimed[msg.sender] == 0) {
			// Give one for free
	...
	function publicMint(uint256 _amount) private {
		...
		require(_amount <= PUBLIC_MINT_LIMIT, 'Exceeded the max amount of NFT mintable in one transaction');
		Enter(msg.sender, _amount);
	...
	function raffleMint(uint256 _amount) private {
		...
		require(_amount <= TIER_LIMIT[uint256(_mintStatus)], 'Exceeded the max amount of NFT mintable in raffle tier');

		if (_premintList[msg.sender]) {
			require(_premintClaimed[msg.sender] == 0, 'You already partecipated in the premint');
		}
	...
	function mint(
		...
		string memory _nonce,
		bytes memory _signature
	) external payable olnyIfAvailable(_amount) onlySignedTx(_amount, _nonce, _signature) nonReentrant {
	...
	// SILVER -> _tier = 3
	// BRONZE -> _tier = 4
	function addToPremintList(uint8 _tier, address[] calldata _addresses) external onlyOwner {
		...
		)
	{
		if (_premintList[_address]) {
			return (_premintTier[_address], _premintClaimed[_address], TIER_LIMIT[uint256(_premintTier[_address])]);
		} else {
