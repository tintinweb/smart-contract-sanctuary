...
contract WhiteRareAuctions is IAuctionHouse, ReentrancyGuard {
   ...
   function setRecordExternal (
      uint256[] calldata _tokens,
      uint256[] calldata _auctions,
      uint256 _bulkAuctionId,
      bool _state
      ) external {
        records.push(Records({
   ...
   function getBulkAuctions(uint256 _bulkIndex) public view returns (uint256[] memory _auctionIds) {
      Records storage record = records[_bulkIndex];
   ...
   function getAuctionPrice(uint256 _auctionId) public view returns(uint256) {
    ...
    function createBulkAuc(
        ...
        for (uint256 ct = 0; ct < tokenId.length; ct++){
            ...
            aucLocal[ct]=_auctionIdTracker.current();
        }
        setRecord(tokenId, aucLocal, bulkAucId, true);
    ...
    nonReentrant
    {
     ...
     function fixedBulkBuy (uint256 bulkAuctionId, uint256 amount) 
      external  
      payable 
      nonReentrant
    {
        uint256[] memory availableBulk = availableBulkAuctionTokens(bulkAuctionId);
        require(amount == getAuctionPrice(availableBulk[0]).mul(availableBulk.length));
        uint256 price = amount.div(availableBulk.length);
            for(uint256 i = 0; i< availableBulk.length;i++){
                fixedBuyInternal(availableBulk[0],price);
            }
        }
     ...
     function availableBulkAuctionTokens(uint256 bulkAuctionId) public view returns(uint256[] memory ){
        uint256[] memory auctionBulk = getBulkAuctions(bulkAuctionId);
        uint256[] memory tokenBulk = getBulkTokens(bulkAuctionId);
        uint256  ct = 0;
        uint256[] memory avialableTokens;
        for (ct; ct < tokenBulk.length; ct++){
            if(getAuctionStatus(auctionBulk[ct])==true){
                avialableTokens[ct]=auctionBulk[ct];
            }
        }
      
       return avialableTokens;
