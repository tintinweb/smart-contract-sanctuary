...
contract Friends is FriendsBase, Extendable {
    ...
    modifier viewExtend override(FriendsBase, Extendable) {
        _viewExtend();
        _;
    }
    ...
    function buyBuf() public view viewExtend returns(uint) {
        ...
        (uint buf, uint last) = (_buyBuf, _lastUpdateBuf);        // uses single storage slot
        return span.sub0(now.sub0(Math.max(last, config[_unlockBegin_]))).mul(buf).div(span);
    }
...
contract FriendsEx is FriendsBase, Extended {
    ...
    function __Friends_init_unchained() public initializer {
        ...
        config[_factorPrice_    ] = 0.0000025e18;   // $0.01
