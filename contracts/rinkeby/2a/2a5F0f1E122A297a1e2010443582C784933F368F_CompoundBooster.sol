...
contract CompoundBooster is Ownable {
    ...
    // mapping(bytes32 => address) public proxyUsers;
    mapping(bytes16 => LendingInfo) public users;
    ...
    function cloneUserTemplate(bytes16 _lendingId, address _sender) internal {
        LendingInfo memory user = users[_lendingId];
        ...
        if (user.createdAt == 0) {
            ...
            // proxyUsers[_lendingId] = template;
        }
    }
    ...
    function lockToken(
        ...
        cloneUserTemplate(_lendingId, _user);

        // address myProxyUser = proxyUsers[_lendingId];
        LendingInfo storage user = users[_lendingId];

        user.cTokens = cTokens;
        user.amount = _amount;
        user.state = LendingInfoState.LOCK;
        ...
        if (pool.isErc20) {
            ...
            ICompoundProxyUserTemplate(user.proxyUser).borrowErc20(
                pool.lpToken,
                underlyToken,
                _user,
                _amount
            ...
            ICompoundProxyUserTemplate(user.proxyUser).borrow(
                pool.lpToken,
                payable(_user),
                _amount
            );
    ...
    function unLockToken(
        ...
        frozenCTokens[_pid] = frozenCTokens[_pid].sub(user.cTokens);

        ICompoundProxyUserTemplate(user.proxyUser).repayBorrow{
            value: msg.value.sub(_interestValue)
        }(pool.lpToken, payable(_user));

        ICompoundProxyUserTemplate(user.proxyUser).recycle(
            pool.lpToken,
            pool.treasuryFund,
            address(0)
        );
    ...
    function unLockTokenErc20(
        ...
        IERC20(underlyToken).safeTransfer(
            user.proxyUser,
            _amount.sub(_interestValue)
        );

        ICompoundProxyUserTemplate(user.proxyUser).repayBorrowErc20(
            pool.lpToken,
            underlyToken,
            _user,
            _amount.sub(_interestValue)
        ...
        if (_interestValue > 0) {
            ...
            IERC20(underlyToken).safeTransfer(pool.rewardInterestPool,exchangeReward);
            IERC20(underlyToken).safeTransfer(lendflareDepositer,lendflareDeposterReward);
        }
    }
    ...
    function liquidate(
        ...
        address underlyToken = address(0);

        if (pool.isErc20) {
            underlyToken = ICompoundCErc20(pool.lpToken).underlying();
        }

        ICompoundProxyUserTemplate(user.proxyUser).repayBorrowBySelf(
            pool.lpToken,
            veLendFlareReward,
            underlyToken
        );
        ...
        // frozenCTokens[_pid] = frozenCTokens[_pid].sub(_amount);
        ...
        //     underlyToken
        // );
        // // ICompoundProxyUserTemplate(user.proxyUser).recycle(pool.lpToken);
