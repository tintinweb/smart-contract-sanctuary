...
contract Hifi is Context, AccessControlEnumerable, ERC721Enumerable, VRFConsumerBase {
    ...
    /// @notice Collection of HiFi details to describe each HiFi
    struct HiFiDetails {
        uint128 rarityTier;
    ...
    /// @notice Use the HiFi tokenId to read HiFi details
    mapping(uint256 => HiFiDetails) public getHiFiDetailsById;

    /// @notice Random digital per tokenId
    mapping(uint256 => uint256) public randomNumberPerToken;
    ...
    /// @notice Check if the HiFi of a specific ID is of a specific rarityTier
    /// @param tokenId The ID of the HiFi to check
    /// @param rarityTier The rarityTier to check against the tokenId
    /// @return (bool) 
    function isHiFiOfRarityTier(uint256 tokenId, uint256 rarityTier) external view returns (bool) {
        if(!_exists(tokenId)) {
            return false;
        }
        HiFiDetails memory currentHiFiDetails = getHiFiDetailsById[tokenId];
        return currentHiFiDetails.rarityTier == rarityTier;
    }
    ...
    /// @notice mint a new HiFi to an address
    /// @dev must be called by an account with MINTER_ROLE
    /// @param to Address to mint the HiFi to
    function mint(
        address to
    ...
    */
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        uint256 currentTokenId = _tokenIdTracker.current();
        randomNumberPerToken[currentTokenId] = randomness;
    }
