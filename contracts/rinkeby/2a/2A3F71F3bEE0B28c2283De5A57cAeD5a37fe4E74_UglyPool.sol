...
contract UglyPool is IERC721Receiver, Ownable() {
    ...
    function getPoolNFTIds(IERC721 _registry) public view returns (uint[] memory){ // address _owner, 
        uint _poolId = getPoolId(msg.sender, _registry);
    ...
    // // swaps random ugly
    function swapRandomUgly(IERC721 _registry, uint _id) public { 
        uint _poolId = getPoolId(msg.sender, _registry);
    ...
    function swapUgly(IERC721 _registry, uint _idFromPool, uint _idToPool) public { 
        uint _poolId = getPoolId(msg.sender, _registry);
    ...
    function sellUglies(IERC721 _registry, uint[] memory _ids) public { 
        uint _poolId = getPoolId(msg.sender, _registry);
