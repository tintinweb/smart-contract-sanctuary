...
contract RaffleWorld is Ownable, VRFConsumerBase {
    ...
    mapping(address => mapping(uint256 => uint256[])) public raffleEntriesPositionById;
    ...
    function _receiveFunds(address _tokenAddress, uint256 _amount) internal {
        if(_tokenAddress == address(0)) {
            require(msg.value == _amount, "RaffleWorld: you don't have enough funds!");         
        }   
        else {
            IERC20 token = IERC20(_tokenAddress);
            require(token.allowance(_msgSender(), address(this)) >= _amount, "RaffleWorld: you don't have enough funds!");
            token.transferFrom(_msgSender(), address(this), _amount);
        }
    }

    function _giveFunds(address _user, address _tokenAddress, uint256 _amount) internal {
        if(_tokenAddress == address(0)) {
    ...
    */
    function addRaffle(
        ...
        uint256 requiredLinkAmount = activeRaffles.add(1).mul(requiredRaffleLink);
        require(
    ...
    */
    ...
    function setRequiredRaffleLink(uint256 _requiredRaffleLink) external onlyOwner {
        requiredRaffleLink = _requiredRaffleLink;
        emit SetRequiredRaffleLink(_msgSender(), _requiredRaffleLink);
    }

   
    function setRaffleName(
        uint256 _raffleId,
        string memory _name
    ...
    function getWinnerStructure(uint256 _raffleIndex, uint256 _structureIndex) external view 
    returns(
        uint256 percentage,
        address winnerAddress
    ) {
        return (winnerStructure[_raffleIndex][_structureIndex].percentage, winnerStructure[_raffleIndex][_structureIndex].winnerAddress);
    }

    function boughtRaffleTicketsByUser(address _user, uint256 _raffleId) external view returns(uint256) {
        return raffleEntriesPositionById[_user][_raffleId].length;
    }

    function boughtRaffleTickets(uint256 _raffleId) external view returns(uint256) {
        return raffleEntries[_raffleId].length;
    }

    function getPrizesNumber(uint256 _raffleId) external view returns(uint256) {
        ...
        returns (bytes32 requestId)
    {
        ...
        emit GetRandomNumber(_raffleId);
    }
    ...
    function subscribeToRaffle(
        ...
        if (_refferedBy != address(0)) {
            ...
            if (
                ...
                refferals[_refferedBy].gold = refferals[_refferedBy]
                    .gold
                    .add(1);
            } else {
    ...
    {
        uint256 ticketsPrice = _getTicketsValue(_raffleId, raffleEntriesPositionById[_msgSender()][_raffleId].length);
        _giveFunds(_msgSender(), raffles[_raffleId].tokenContract, ticketsPrice);
    ...
    function withdrawOwnerFunds(uint256 _raffleId)
        ...
        onlyOwner
    {
        uint256 ownerFunds = raffles[_raffleId].ticketPrice.mul(raffles[_raffleId].ticketsNumber).sub(raffles[_raffleId].prizeAmount);
        require(keccak256(bytes(raffles[_raffleId].status)) == keccak256(bytes("ended")) );
