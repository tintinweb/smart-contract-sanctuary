...
contract Starseed is ERC721, ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint256 public constant ALL_CHARACTER_TYPES = 28;
    ...
    function claim(uint256[] calldata tokenIDs) external nonReentrant {
        ...
        uint256 allClaimCharacterTypes = 0;
        for (uint256 i = 0; i < tokenIDs.length; i++) {
            _claim(tokenIDs[i], msg.sender);
            allClaimCharacterTypes += uint8(polypixosCharacterTypes[tokenIDs[i]]);
        }
        require(allClaimCharacterTypes == ALL_CHARACTER_TYPES, "Must own all 8 sub-species");
