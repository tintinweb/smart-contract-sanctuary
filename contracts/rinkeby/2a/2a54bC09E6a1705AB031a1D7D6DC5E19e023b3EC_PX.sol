...
import {ERC721MetadataStructure, ERC721MetadataAttribute} from "./ERC721MetadataStructs.sol";
...
contract PX is ERC721, ERC721Metadata {
    ...
    // Pointer to a JSON generator contract
    IJsonGenerator public _jsonGenerator;
     ...
     *
     */
    constructor(address registry_, address omnibus_, address generator_, address jsonGenerator_) {
        _registry = registry_;
        _omnibus = omnibus_;
        _name = "z";
        _symbol = "Z";
        ...
        generator = ISvgGenerator(generator_);
        _jsonGenerator = IJsonGenerator(jsonGenerator_);
    }
    ...
    function royaltyInfo(uint256 _tokenId, uint256 _salePrice) external view returns (address receiver, uint256 royaltyAmount) {
        //TODO: make 'receiver' settable
        address receiver = 0x2Ce780D7c743A57791B835a9d6F998B15BBbA5a4;
        uint256 royaltyAmount = 10000;
        return (receiver, royaltyAmount);
    }

    function requireOnlyValidSender() private {
    ...
    function baseClassOf(uint256 tokenId) public view virtual returns (uint256) {
     ...
     */
    function setImageBaseURI(string calldata imageBaseURI, string calldata imageExtension) external {        
        requireOnlyValidSender();
     ...
     */
    function setSvgGenerator(address _generator) external {  
        requireOnlyValidSender();      
        generator = ISvgGenerator(_generator);
    }

    function setJsonGenerator(address jsonGenerator) external {      
        requireOnlyValidSender();  
        _jsonGenerator = IJsonGenerator(jsonGenerator);
    }

    /**
     * @dev This is how you change the canvas size of the output SVG
     */
    function setCanvasSize(uint256 canvasSize) external {        
        requireOnlyValidSender();
     ...
     */
    function getSvg(uint256 tokenId, uint256 canvasSize) private view returns (string memory) {
        require(address(generator) != address(0), "Generator not set");
        string memory class = getClassString(tokenId, false);
        bytes memory encodedParams = abi.encode(canvasSize, massOf(tokenId), alphaMass(), class);
     ...
     */     
    function getClassString(uint256 tokenId, bool offchainImage) public view returns (string memory) {

        bytes memory byteString;    
    
        byteString = abi.encodePacked(
          byteString,
          string(abi.encodePacked("m", decodeClass(_values[tokenId]).toString())));

        // Spaces at the beginning of classnames below are intentional - do not remove
        if(tokenId == _alphaId) {
            byteString = abi.encodePacked(
              byteString,
        ...
        byteString = abi.encodePacked(
          byteString,
          string(abi.encodePacked(offchainImage ? "_" : " ", "c", tensDigit.toString())));

        byteString = abi.encodePacked(
          byteString,
          string(abi.encodePacked(offchainImage ? "_" : " ", "c", onesDigit.toString())));
     ...
     */
    function getJson(uint256 tokenId, string memory svgData) private view returns (string memory) {
        
        string memory imageData = svgData;
        uint256 mass = decodeMass(_values[tokenId]);
        uint256 maxMass = alphaMass();

        if(bytes(_imageBaseURI).length > 0) {
            // Pointing off-chain, so we will return an image URI instead in 'image' attribute
            string memory class = getClassString(tokenId, true);
            int128 radiusMass = getRadius64x64(mass);
            int128 radiusMaxMass = getRadius64x64(maxMass);
            int128 scalePercentage = ABDKMath64x64.div(radiusMass, radiusMaxMass);    
            ...
            imageData = string(abi.encodePacked(imageBaseURI(), class, "_", truncatedRadius.toString(), imageExtension()));
        }

        ERC721MetadataAttribute memory zAttribute = ERC721MetadataAttribute({
            includeDisplayType: true,
            includeTraitType: true,
            isValueAString: false,
            displayType: "number",
            traitType: "z",
            value: massOf(tokenId).toString()
        });

        ERC721MetadataAttribute[] memory metadataAttributes = new ERC721MetadataAttribute[](1);
        metadataAttributes[0] = zAttribute;

        ERC721MetadataStructure memory metadata = ERC721MetadataStructure({
            isImageLinked: bytes(_imageBaseURI).length > 0, 
            name: string(abi.encodePacked(name(), " ", tokenId.toString(), " (", mass.toString(), ")")),
        ...
        return _jsonGenerator.generateERC721Metadata(metadata);
    }
    ...
    function getRadius64x64(uint256 mass) public view returns (int128) {
...
import {ERC721MetadataStructure, ERC721MetadataAttribute} from "../core/ERC721MetadataStructs.sol";

interface IJsonGenerator {    
    function generateERC721Metadata(ERC721MetadataStructure calldata metadata) external view returns (string memory);
}
