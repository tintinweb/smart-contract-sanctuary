...
contract Souls is ERC721 {
    ...
    address payable public un_frontier;
    address payable public writer; // Untitled Frontier collection address

    uint256 public buyableSoulSupply;

    // **** TEST VARS *** //
    // TODO. Take out before ending
    ...
    mapping(uint256 => bool) public soulsType;
     ...
     */
    constructor (string memory name_, string memory symbol_, address payable writer_, uint256 startDate_, uint256 endDate_, address certificates_) ERC721(name_, symbol_) {
        writer = writer_;
        un_frontier = payable(0xaF69610ea9ddc95883f97a6a3171d52165b69B03); // TEST: change to mirror
        startDate = startDate_;
        endDate = endDate_;
        descriptor = new SoulsDescriptor();

        // RINKEBY :: 0x600a4446094C341693C415E6743567b9bfc8a4A8
        anchorCertificates = ERC721(certificates_);
        // MAINNET todo

        // initial mint. one for un_frontier
        _createSoul(false,address(0x1c9468d649e548751011250107c2695994910200));

        //tests
        _createSoul(false,address(0xaF69610ea9ddc95883f97a6a3171d52165b69B03));
        _createSoul(true,address(0x651d94725B7537a008D23A2353dcd23cB6a53E47));
        _createSoul(true,address(0x9f3e324B12DB18797e62df5C6642Ff369b64Be1d));
    }
    ...
    */
    ...
    function mintSoul() public payable {
        ...
        require(msg.value >= 0.0086 ether, 'MORE ETH NEEDED'); //~$30

        if(msg.value >= 0.057 ether) { //~$200
            buyableSoulSupply += 1;
    ...
    function _createSoul(bool fullSoul, address _owner) internal {
        ...
        newlyMinted = tokenId; // TEST TEST TODO
        super._mint(_owner, tokenId);
    }

    /*
    todo: Change to a Mirror split
    */
    function withdrawETH() public {
        require(msg.sender == un_frontier, "NOT_COLLECTOR");
        un_frontier.transfer(address(this).balance);
    }
}
...
contract SoulsDescriptor {
    ...
    function generateName(string memory soulType, uint soulNr) public pure returns (string memory) {
        return string(abi.encodePacked(soulType, ' Lost Soul #', substring(toString(soulNr),0,8)));
    }
    ...
    function generateImage(uint256 tokenId, bool fullSoul) public pure returns (string memory) {
        ...
        // string memory label = generateLabel(fullSoul);
    ...
    // 
    /*function generateLabel(bool fullSoul) public pure returns(string memory) {
        string memory status = "a sketch. one of many. lost and forgotten.";
        if(fullSoul == true) { status = "fully painted. one of 256. lost, but not fully forgotten."; }
        return string(abi.encodePacked(
            '<text x="30" y="30" style="font-size: 18px" font-family="Times New Roman">status: ',status,'</text>'
        ));
    }*/
    ...
    function generateEyes(bytes memory hash) public pure returns (string memory) {
        ...
        string memory eyes = string(abi.encodePacked(
            svgCircle("250","270","20","black","1","white","filter: url(#eyeDisplacement)"),
            svgCircle("320","270","20","black","1","white","filter: url(#eyeDisplacement)")
        ));
