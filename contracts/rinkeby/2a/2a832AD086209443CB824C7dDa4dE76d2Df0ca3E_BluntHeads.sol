...
contract BluntHeads is ERC1155, Ownable, Pausable, ERC1155Burnable
{
    ...
    //Price
    uint256 public _BLUNT_PRICE = 0.09 ether;
    ...
    //Mints BluntHeads Blunts
    function BluntHeadsMint(uint numberOfTokens) public payable
    {
        require(_SALE_IS_ACTIVE, "Sale must be active to mint Blunts");
        require(numberOfTokens <= _MAX_BLUNTS_PURCHASE, "Can only mint 5 Blunts at a time");
        require(_BLUNTS_MINTED.add(numberOfTokens) <= _MAX_BLUNTS, "Purchase would exceed max supply of Blunts");
        require(_BLUNT_PRICE.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct. 0.09 ETH Per Blunt | 90000000000000000 WEI");
