...
contract SocialToken is ERC20, Ownable {
    ...
    function priceToMint(uint256 amount) internal returns (uint256) {
        ...
        uint256 result = curveIntegral(newSupply, effectiveSupply);
        return
            result.mul(MAX_PERCENTAGE.add(tradingSpread)).div(MAX_PERCENTAGE);
    }
    ...
    function rewardForBurn(uint256 amount) internal returns (uint256) {
        ...
        lowSupply = effectiveSupply.sub(amount);
    ...
    /**
     * @dev                Withdraw some amount of the InitialSupply minted of Social Token
     * @param amount       The amount of social tokens that you want to withdraw from initialSupply
     */
    function withdrawSocialToken(uint256 amount, address to) public onlyOwner {
        require(
        ...
        emit WithdrawSocialToken(to, amount);
    }
     ...
     */
    function withdrawFundingToken(uint256 amount, address to) public onlyOwner {
        require(
            fundingToken.balanceOf(address(this)) >= amount,
            "Withdrawl amount of funding token is not enough"
        );

        fundingToken.transfer(to, amount);

        emit WithdrawFundingToken(to, amount);
    }

    // /**
    //  * @dev                Airdrop a specific amount of social token to a given address
    //  * @param amount       The amount of social tokens
    //  */
    // function airDropSocialToken(uint256 amount, address[] users) public onlyOwner {
    //     require(
    //         fundingToken.balanceOf(this) >= amount,
    //         "Withdrawl amount of funding token is not enough"
    //     );

    //     fundingToken.transferFrom(address(this), msg.sender, amount);

    //     // emit AirdropSocialToken(users, amount);
    // }
     ...
     * @param amount       The amount of social tokens that you want to withdraw from initialSupply
     */
    function airDropSocialToken(uint256 amount, address to) public onlyOwner {
        require(
            initialSupply >= amount,
            "Remaining Initial supply is not enough"
        );
