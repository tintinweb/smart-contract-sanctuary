...
contract RussianRoulette is GameController, Pausable {
    ...
    function getGameInfo(uint256 requestId, uint256 index) external view returns (uint256, address, address, uint256, uint256, uint256) {
    ...
    function play(uint256 betId, address tokenAddress, uint256 betAmount, uint256 cylinderSize, uint256 bulletsNumber) external whenNotPaused payable {
        ...
        uint256 potentialWin = betAmount.mul(cylinderSize).div(cylinderSize.sub(bulletsNumber)); // bet * cylinderSize / (cylinderSize - bulletsNumber)
        ...
        request.games[request.tailIndex] = Game(betId, _msgSender(), tokenAddress, betAmount, cylinderSize, bulletsNumber);
        request.tailIndex = request.tailIndex.add(1);
        emit RussianRouletteRun(betId, _msgSender(), tokenAddress, betAmount, cylinderSize, bulletsNumber, _lastRequestId, request.tailIndex.sub(1));
    }
