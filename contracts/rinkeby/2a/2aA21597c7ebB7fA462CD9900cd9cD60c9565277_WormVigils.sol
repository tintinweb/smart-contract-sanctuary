...
contract WormVigils is Ownable, ERC721Enumerable {
	...
	// they don't needs to be public, which reduces gas
	address edworm;
	address edwone;
	...
	// track number of candles lit per vigil
	mapping(uint64 => uint) public vigilLumens;
	// track the candle specific data
	...
	constructor(address _edworm, address _edwone) ERC721('WormVigils', 'LUMEN') {
		// store the address of the old Edwone contract
		edworm = _edworm;
		edwone = _edwone;
	...
	function mintCandleWithPrestige(uint64 prestige) public payable {
		...
		for (uint i; i < lumenLevels.length; i++) {
			...
			if (msg.value >= _price) {
				// Could remove level and do length minus index to get it
		...
		// 5. Increment the lumens in this vigil
		vigilLumens[vigil] += lumen;
		...
		*/
		...
		for (uint i; i < sequence.length; i++) {
			if (sequence[i] == bytes16('_token_id')) {
				...
				// 2. replace '_disciple_id' with the actual token id
