...
contract MogulMarketplace is ERC1155Holder, AccessControl, ReentrancyGuard {
  ...
  function listTokens(
    ...
    token.safeTransferFrom(msg.sender, address(this), tokenId, numTokens, "");
    ...
    nonReentrant()
  {
    ...
    if (listing.isStarsListing) {
      stars.safeTransferFrom(
        msg.sender,
        address(this),
        listing.price.mul(amount)
      );
    } else {
      require(
        msg.value == listing.price.mul(amount),
   ...
   * auctionId: auction ID
   */
  function cancelAuction(uint256 auctionId) external onlyAdmin nonReentrant() {
    require(auctionIds.contains(auctionId), "Auction does not exist");
    Auction memory auction = auctions[auctionId];
