...
contract BSC_ETH_bridge{
    ...
    mapping (uint256 => address) private _BSCmain;
    mapping (uint256 => address) private _ETHparallel;
    
    mapping (uint256 => address) private _BSCparallel;
    mapping (uint256 => address) private _ETHmain;
    ...
    mapping (address /*BSC*/ => address /*ETH*/ ) private _parallelContract; //the Ethereum parallel contract of the BSC main contract
    mapping (address /*BSC*/ => address /*ETH*/ ) private _mainContract; //the Ethereum main contract of the BSC parallel contract */
    //================================================================================================================================================================================    
    ...
    function requestVerificationFrom(address node) external returns(bool){
        require(_stakedFT[msg.sender] > 0);
        require(_block[node] == 0);
        
        _block[node] = block.number;
        return true;
    }
    function lock(address node) external returns(bool){
        require(block.number - _block[node] >= 1800);
    ...
    function unlock() external returns(bool){
        require(block.number -_lockBlock[msg.sender] >= 302400);
    ...
    function transfer(uint256 amount, uint256 fee, uint256 createFee, uint256 signFee, address tokenAddressFrom, address tokenAddressTo) external returns(bool) {
        ...
        require(FT.transferFrom(msg.sender, address(this), fee));
    ...
    function signDeposit(address sender, uint256 amount, uint256 fee, uint256 signFee, uint256 createFee, uint256 index, address tokenAddressFrom, address tokenAddressTo) external returns(bool) {
        ...
        if(votedFT *2 > _totalStakedFT){
            ...
            if(token.balanceOf(tokenAddressTo) >= amount){}
            else{}
    ...
    function createProposal(address BSCmain, address ETHparallel, address ETHmain, address BSCparallel) external {
        _proposalCreated[_proposalsCount] = true;
        _proposalCreator[_proposalsCount] = msg.sender;
        _BSCmain[_proposalsCount] = BSCmain;
        _ETHparallel[_proposalsCount] = ETHparallel;
        _ETHmain[_proposalsCount] = ETHmain;
        _BSCparallel[_proposalsCount] = BSCparallel;
        _proposalsCount += 1;
    }
    ...
    function signProposal(uint256 proposalID) external returns(bool){
        ...
        if(votes * 2 > _totalStakedFT){
            _parallelContract[_BSCmain[proposalID]] = _ETHparallel[proposalID]; //the Ethereum parallel contract of the BSC main contract
            _mainContract[_BSCparallel[proposalID]] = _ETHmain[proposalID]; //the Ethereum main contract of the BSC parallel contract */
            return true;
        }
        else{return false;}
    }
