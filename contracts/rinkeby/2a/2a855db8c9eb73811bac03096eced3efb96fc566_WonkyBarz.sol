...
// File: contracts/WonkyBarz.sol

/**
 * @title WonkyBarz contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract WonkyBarz is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public MAX_WONKYZ_SUPPLY;
    ...
    constructor() ERC721("WonkyBarz", "WONKY") {
        MAX_WONKYZ_SUPPLY = 10000;
        REVEAL_TIMESTAMP = block.timestamp + (86400 * 7);
        mintPrice = 20000000000000000; // 0.02 ETH
        maxToMint = 30;
        saleIsActive = false;
        wallet1 = 0x14594254e2753f75e6623d1A64a0cd9CFA704573;
    }
    ...
    /**
     * Mint Wonkyz by owner
     */
    function reserveWonkyz(address _to, uint256 _numberOfTokens) external onlyOwner {
    ...
    * Mints tokens
    */
    function mintWonkyz(uint256 numberOfTokens) external payable {
        require(saleIsActive, "Sale must be active to mint");
        require(numberOfTokens <= maxToMint, "Invalid amount to mint per once");
        require(totalSupply().add(numberOfTokens) <= MAX_WONKYZ_SUPPLY, "Purchase would exceed max supply");
        require(mintPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
        
        for(uint256 i = 0; i < numberOfTokens; i++) {
            uint256 mintIndex = totalSupply();
            if (totalSupply() < MAX_WONKYZ_SUPPLY) {
        ...
        if (startingIndexBlock == 0 && (totalSupply() == MAX_WONKYZ_SUPPLY || block.timestamp >= REVEAL_TIMESTAMP)) {
    ...
    function setStartingIndex() external {
        ...
        startingIndex = uint256(blockhash(startingIndexBlock)) % MAX_WONKYZ_SUPPLY;
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number.sub(startingIndexBlock) > 255) {
            startingIndex = uint256(blockhash(block.number - 1)) % MAX_WONKYZ_SUPPLY;
        }
