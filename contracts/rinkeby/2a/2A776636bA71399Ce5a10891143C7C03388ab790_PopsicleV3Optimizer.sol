...
library PoolVariables {
    ...
    function getPositionTicks(
        ...
        //Calc base ticks
        (cache.tickLower, cache.tickUpper) = baseTicks(
            currentTick,
            baseThreshold,
            tickSpacing
        );
        ...
        if (zeroGreaterOne) {
            ...
            cache.tickUpper = PoolVariables.floor(
            ...
            cache.tickLower = PoolVariables.floor(
...
abstract contract EIP712 {
    ...
    function _buildDomainSeparator(
        ...
        return
            keccak256(abi.encode(typeHash, name, version, ChainId.get(), address(this)));
    }
...
contract ERC20 is Context, IERC20 {
    ...
    function transferFrom(
        ...
        _approve(
            sender,
            _msgSender(),
            _allowances[sender][_msgSender()].sub(amount, "TEA")
        );
        return true;
    }
        ...
        returns (bool)
    {
        _approve(
            _msgSender(),
            spender,
            _allowances[_msgSender()][spender].sub(subtractedValue, "DEB")
        );
        return true;
    }
...
contract PopsicleV3Optimizer is ERC20Permit, ReentrancyGuard, IPopsicleV3Optimizer {
    ...
    function init() external onlyGovernance {
        ...
        int24 baseThreshold = tickSpacing *
            IOptimizerStrategy(strategy).tickRangeMultiplier();
        (, int24 currentTick, , , , , ) = pool.slot0();
        ...
        )
    {
        ...
        shares = totalSupply() == 0
            ...
            : FullMath.mulDiv(liquidity, totalSupply(), liquidityLast);
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        _earnFees();
        _compoundFees();
        (amount0, amount1) = pool.burnLiquidityShare(
            tickLower,
            tickUpper,
            totalSupply(),
            shares,
    ...
    function rebalance() external override nonReentrant checkDeviation {
        ...
        int24 baseThreshold = tickSpacing *
            IOptimizerStrategy(strategy).tickRangeMultiplier();
        (cache.tickLower, cache.tickUpper) = PoolVariables.baseTicks(
            currentTick,
            baseThreshold,
            tickSpacing
        );
        ...
        cache.liquidity = pool.liquidityForAmounts(
            cache.amount0Desired,
            cache.amount1Desired,
            cache.tickLower,
            cache.tickUpper
        ...
        (cache.amount0, cache.amount1) = pool.amountsForLiquidity(
            cache.liquidity,
            cache.tickLower,
            cache.tickUpper
        );
        ...
        uint160 exactSqrtPriceImpact = sqrtPriceX96.mul160(
            IOptimizerStrategy(strategy).priceImpactPercentage() / 2
        ) / GLOBAL_DIVISIONER;
        uint160 sqrtPriceLimitX96 = zeroForOne
            ? sqrtPriceX96.sub160(exactSqrtPriceImpact)
            : sqrtPriceX96.add160(exactSqrtPriceImpact);
    ...
    function _earnFees() internal {
        ...
        uint256 earnedProtocolFees0 = collect0.mul(protocolFee).unsafeDiv(
            GLOBAL_DIVISIONER
        );
        uint256 earnedProtocolFees1 = collect1.mul(protocolFee).unsafeDiv(
            GLOBAL_DIVISIONER
        );
    ...
    modifier checkDeviation() {
        pool.checkDeviation(
            IOptimizerStrategy(strategy).maxTwapDeviation(),
            IOptimizerStrategy(strategy).twapDuration()
        );
        _;
    }
