    ...
    ContextMixin
{
    ...
    // Structs
    struct HyperBeetle {
        bytes32 momGene; // When genes are required but no parents yet minted (first gen)
        bytes32 dadGene; // Genes will be on all HB.
        uint256 momId; // All HB resulting from breeding
        uint256 dadId;
        bytes32 gene;
    }
    ...
    uint256 constant maxClaims = 20000 - (maxOriginSaleClaims * 2);
    ...
    function claim(uint256 _amount) external payable whenNotPaused {
        ...
        require(_amount <= 10, 'Claim: exceeds single transaction limit');

        // Must keep totalClaims separate from hbCounter
        // since hbCounter is incremented by breeding
        require(
            (_amount + totalClaims) <= maxClaims,
            'Claim: exceeds max claims'
        );

        // Must keep totalOriginSaleClaims separate from
        // totalClaims because totalClaims incremented
        // by mintTo
        uint8 multiplier = 1;
        ...
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < childCount; i++) {
            ...
            hb.momGene = mom.gene; // For consistency with generation one
            hb.dadGene = dad.gene;
