...
contract Commerce is Abstract1155Factory  {
   ...
   function editTokenWhiteListMerkleRoot(
    ...
    function purchase(
        ...
        //string memory quantitiesMinted = "";
     ...
     function filterInStockTokensFromPack(TokenPackConfig memory tokenPackConfig) internal view returns(uint256[] memory){
        tokenPackConfig.packTokens = shuffle(tokenPackConfig.packTokens, false);
    ...
    function shuffle(uint256[] memory numberArr, bool returnRandomIndex) internal view returns(uint256[] memory){
        if(!returnRandomIndex){
            ...
            uint randomHash = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender))) % numberArr.length;
