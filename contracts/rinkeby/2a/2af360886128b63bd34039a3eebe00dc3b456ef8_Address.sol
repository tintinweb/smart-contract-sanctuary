...
contract BoringBananasCo is ERC721, Ownable {
    ...
    uint256 public constant TEST_WICKED = 8888;
    ...
    constructor() ERC721("Test Wicked", "WCKD") { }
    ...
    function mintBoringBanana(uint numberOfTokens) public payable {
        ...
        require(totalSupply().add(numberOfTokens) <= TEST_WICKED, "Purchase would exceed max supply of Bananas");
        require(msg.value >= bananaPrice.mul(numberOfTokens), "Ether value sent is not correct");
        
        for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply();
            if (totalSupply() < TEST_WICKED) {
