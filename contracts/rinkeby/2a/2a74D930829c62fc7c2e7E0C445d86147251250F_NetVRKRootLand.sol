...
contract NetVRKRootLand is
    ...
    ContextMixin
{
    mapping(uint256 => uint256) tokenReleasedDates;
    ...
    /*************************** MODIFIER ********************************/
    modifier requireVestingPeriod(uint tokenId) {

        uint blockTimeStamp = block.timestamp;
        uint tokenReleasedDate = tokenReleasedDates[tokenId];
        // console.log("block date:", blockTimeStamp);
        // console.log("token release date: ", tokenReleasedDate);
        // console.log("address inside the vesting ", address(this));

        require(block.timestamp > (tokenReleasedDates[tokenId] + 30 days) || allowTransfer, "cannot transfer within the vesting  ");
        _;
    }
    ...
    {
        _mint(user, tokenId);
        tokenReleasedDates[tokenId] = block.timestamp;
       
        //console.log("token id", tokenId);
       // console.log("setting token id release date", tokenReleasedDates[tokenId]);
    ...
    requireVestingPeriod(tokenId) 
    {
        //console.log("inside the _beforeTokenTransfer");
        super._beforeTokenTransfer(from, to, tokenId);
    }

    function setTransferPermission(bool allow) external onlyOwner
    {
        allowTransfer = allow;
    }   

    function grantContractRole(address contractAddress) external onlyOwner
    {
         _setupRole(PREDICATE_ROLE, contractAddress);
    }
}
