...
contract FCXAccessControl is AccessControl {
    ...
    modifier onlyRole(bytes32 role) {
        require(hasRole(role, _msgSender()), "FCXAccessControl: sender requires permission");
        _;
    }

    /**
     * @dev toggle role (restricted, unrestricted)
     */
    function toggleRole(address account) public {
        ...
        if (hasRole(RESTRICTED_ROLE, account)) {
            ...
            grantRole(UNRESTRICTED_ROLE, account);
        } else if (hasRole(UNRESTRICTED_ROLE, account)) {
            revokeRole(UNRESTRICTED_ROLE, account);
            grantRole(RESTRICTED_ROLE, account);
        }
    }
    ...
    function getRoles(address account) public view returns (bytes32[] memory) {
        bytes32[3] memory roles = [ADMIN_ROLE, RESTRICTED_ROLE, UNRESTRICTED_ROLE];
