    ...
    ReentrancyGuard
{
    ...
    function doRebalanceTokens() public {
        if (baseLower == baseUpper) {
            return;
        }

        // TODO: need a min threshold or it will end up inf swap for dusts
    ...
    function rebalanceTokens() public view returns (uint128, uint128, uint128, uint128, uint256, uint256) {
        if (baseLower == baseUpper) {
            return (0,0,0,0,0,0);
        }
        uint256 balance0 = getBalance0();
        ...
        uint128 liquidity = liqAmount0 > liqAmount1 ? liqAmount0 : liqAmount1; // (liqAmount0 + liqAmount1) / 2;
        (uint256 amount0, uint256 amount1) = _amountsForLiquidity(baseLower, baseUpper, liquidity);

        uint128 liquidityB = _liquidityForAmounts(baseLower, baseUpper, balance0, balance1);

        return (liqAmount0, liqAmount1, liquidity, liquidityB, amount0, amount1);
