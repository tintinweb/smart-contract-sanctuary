    ...
    ReentrancyGuard
{
    ...
    function rebalanceTokens() external {
        ...
        if (balance0 > 0 && balance1 > 0) {
            // revert('balance0 > 0 && balance1 > 0');
        } else if (balance0 > 0) {
            pool.swap(address(this), true, int256(balance0.div(2)), TickMath.MIN_SQRT_RATIO + 1, "");
        } else if (balance1 > 0) {
            pool.swap(address(this), false, int256(balance1.div(2)), TickMath.MAX_SQRT_RATIO - 1, "");
        }
