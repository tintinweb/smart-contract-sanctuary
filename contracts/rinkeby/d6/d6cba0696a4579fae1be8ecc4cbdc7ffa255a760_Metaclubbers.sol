...
contract Metaclubbers is ERC721Enumerable, Ownable {
  ...
  address ethHolderWallet = address(owner());
  string baseURI;
  ...
  function privateMint(uint256 _mintAmount, address destination) public onlyOwner {
    ...
    require(supply + _mintAmount <= maxSupply + privateMintAmount);
    require(supply + _mintAmount <= 9999);
  ...
  function setCost(uint256 _newCost) public onlyOwner {
    require(_newCost <= 3 ether && _newCost >= 0.1 ether, "Cost must be between 3 and .1 ether");
    cost = _newCost;
  }
