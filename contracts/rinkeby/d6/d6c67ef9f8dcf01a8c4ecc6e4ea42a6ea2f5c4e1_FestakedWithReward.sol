...
library FestakedLib {
    ...
    returns (bool) {
        ...
        state.rewardBalance = state.rewardsTotal;
    ...
    returns (bool) {
        ...
        if (now < withdrawEnds) {
            return _withdrawEarly(tokenAddress, rewardTokenAddress, msg.sender, amount, withdrawEnds,
                stakingEnds, state, rewardState);
        } else {
            return _withdrawAfterClose(tokenAddress, rewardTokenAddress, msg.sender, amount, state, rewardState);
        }
    }
    ...
    returns (bool) {
        ...
        state.stakedBalance = state.stakedBalance.sub(amount);
        state._stakes[from] = state._stakes[from].sub(amount);
    ...
    _realAddress(from)
    returns (bool) {
        uint256 reward = (rewardState.rewardBalance.mul(amount)).div(state.stakedBalance);
        state._stakes[from] = state._stakes[from].sub(amount);
        bool principalPaid = _payDirect(from, amount, tokenAddress);
        bool rewardPaid = _payDirect(from, reward, rewardTokenAddress);
        require(principalPaid && rewardPaid, "Festaking: error paying");
...
contract FestakedWithReward is FestakedOptimized {
    ...
    external returns (bool) {
        return FestakedLib.addReward(rewardAmount, withdrawableAmount, withdrawStarts,
    ...
    external {
        ...
        FestakedLib.addMarginalReward(rewardTokenAddress, tokenAddress,
            withdrawStarts, address(this), stakeState.stakedBalance, rewardState);
    }
