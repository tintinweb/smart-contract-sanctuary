...
// token transfers
// ----------------------------------------------------------------------------
contract simurgShare is ERC20, Owned, SafeMath {
    ...
    */
    ...
    //function eachOnesGain(address _address) public view returns(uint){
    //    return balances[_address]/_totalSupply;
    //}
    
    function spreadTheGain() public payable{
        require(msg.value >= requiredAmount, "Can't send 0 value!");
        for (uint i = 0; i < indexedAddressesCounter; i++){
            payable(ownersAddresses[i]).transfer(msg.value*(balanceOf(ownersAddresses[i])/_totalSupply));
        }
    }
    // ------------------------------------------------------------------------
    // Total supply
