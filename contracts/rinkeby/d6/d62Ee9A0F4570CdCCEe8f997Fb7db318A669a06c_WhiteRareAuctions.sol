...
contract WhiteRareAuctions is IAuctionHouse, ReentrancyGuard {
   ...
   function getAuctionCurrentNumber() public view returns (uint256) {
     ...
     */
    function endAuction(uint256 auctionId) public override auctionExists(auctionId)  {
        require(
    ...
    function endAuctionMinimal(uint256 auctionId, address _to) public auctionExists(auctionId)  {
        require(
            uint256(auctions[auctionId].firstBidTime) != 0,
            "Auction hasn't begun"
        );
        ...
        if(auctions[auctionId].tokenContract == whiterarenft) {
            ...
            // Otherwise, transfer the token to the winner and pay out the participants below
            try IERC721(auctions[auctionId].tokenContract).safeTransferFrom(address(this), _to, auctions[auctionId].tokenId) {} catch {
                _handleOutgoingBid(auctions[auctionId].bidder, auctions[auctionId].amount, auctions[auctionId].auctionCurrency);
                _cancelAuction(auctionId);
                return;
            }
    ...
    auctionExists(auctionId)
    {
        ...
        endAuctionMinimal(auctionId, msg.sender);
    ...
    function fixedMinimalBuy(uint256 auctionId, uint256 amount)
    external
    payable
    auctionExists(auctionId)
    {
    ...
    function fixedBulkBuy(uint256[] memory _fixedAuc, uint256 _bulkAmount) public payable   {
        
        require(_fixedAuc.length < 51);
        for(uint256 i = 0; i < _fixedAuc.length; i++){
            uint256 auctionId = _fixedAuc[i];
            uint256 amount = _bulkAmount.div(_fixedAuc.length);
            
            address to = msg.sender;
            address currency = auctions[auctionId].auctionCurrency;
