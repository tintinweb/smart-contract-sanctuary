 ...
 * {ERC721Enumerable}.
 */
contract womaCertificates is ERC721 {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory name = string(abi.encodePacked(certificateType, ' woma Certificate #',toString(certificates[tokenId].nr)));
        string memory description = "woma City Certificate";
    ...
    function generateImage(uint256 tokenId) public view returns (string memory) {
        ...
        return string(
            abi.encodePacked(
                '<svg class="svgBody" width="300" height="300" viewBox="0 0 500 500" xmlns="http://www.w3.org/2000/svg">',
                paletteSection,
                skyscraper,
    ...
    function mintCertificate() public payable returns (uint256 tokenId) {
        ...
        require(msg.value >= 0.001 ether, 'MORE ETH NEEDED'); //~$20

        Certificate memory certificate;
        
        if(msg.value >= 0.005 ether) { //~$100
            deluxeCertificatesSupply += 1;
            require(deluxeCertificatesSupply <= 5, "MAX_DX_REACHED_100");
