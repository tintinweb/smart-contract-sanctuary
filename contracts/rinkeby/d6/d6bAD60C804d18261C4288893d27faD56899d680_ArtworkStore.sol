...
contract ArtworkStore is IArtworkStore, Initializable, TRLabOwnableUpgradeable, UUPSUpgradeable {
     ...
     * @param  _royaltyBps address the royalty Receiver
    ...
    function createArtwork(
        ...
        emit ArtworkCreated(id, _royaltyReceiver, _royaltyBps, _totalSupply, _metadataPath);
        return id;
    }
 ...
 * responsible for storing the common artwork data.
...
interface IArtworkStore {
     ...
     * @param  _metadataPath string the ipfs metadata path
     * @param  _royaltyReceiver address the royalty receiver
     * @param  _royaltyBps uint256 the royalty receiver
     * @return the id of the new artwork created
     */
    function createArtwork(
        address _creator,
...
abstract contract TRLabOwnableUpgradeable is OwnableUpgradeable {
    ...
    event TRLabAddressTransferred(
        address indexed previousTRLab,
        address indexed newTRLab
    );
    /**
    ...
    modifier onlyOwnerOrTRLab() {
        require(
            owner() == _msgSender() || trlabToken() == _msgSender(),
            "caller is not the owner or TRLab Token"
        );
        _;
    }
    ...
    function setTRLab(address newTRLab) external onlyOwner {
        require(
            newTRLab != address(0),
            "TRLabOwnable: new TRLab Token is the zero address"
        );
