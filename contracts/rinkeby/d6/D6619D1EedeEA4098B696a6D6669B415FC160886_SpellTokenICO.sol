...
contract SpellTokenICO {
    ...
    function buyForDAI(uint daiAmount) payable public {
        ...
        try ERC20(daiTokenAddress).transferFrom(msg.sender, address(this), daiAmount) {
            try ERC20(tokenAddress).transfer(msg.sender, tokenAmount) {
            } catch Error(string memory) {
                (bool success, ) = msg.sender.call{ value: msg.value }("Sorry, there is not enough tokens to buy");
                require(success, "External call failed");
            ...
            (bool success, ) = msg.sender.call{ value: msg.value }("Sorry, there is not enough DAI to transfer");
            require(success, "External call failed");
        } catch (bytes memory reason) {
            (bool success, ) = msg.sender.call{ value: msg.value }(reason);
            require(success, "External call failed");
    ...
    function returnDAI() payable public {
    ...
    function returnETH() payable public {
