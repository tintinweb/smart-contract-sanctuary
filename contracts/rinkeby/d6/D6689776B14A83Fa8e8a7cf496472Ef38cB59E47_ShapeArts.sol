...
library ShapeArtGenerator{
    ...
    function getBgColor(uint256 tokenId) internal pure returns(uint256){
       uint256 rand = random(string(abi.encodePacked("BG_COLOR", toString(tokenId))));
       rand = rand % 201;
       uint256 ps;
       if(rand>30 && rand<=60){ps=1;}
       if(rand>60 && rand<=90){ps=2;}
       if(rand>90 && rand<=130){ps=3;}
       if(rand>130 && rand<=160){ps=4;}
       if(rand>160 && rand<=190){ps=5;}
       return ps;
   }
    function getNumShapes(uint256 tokenId) internal pure returns(uint256){
       ...
       rand = rand % 201;
       uint256 ps=5;
       if(rand>120 && rand<=180){ps=6;}
       if(rand>180 && rand<=190){ps=7;}
       return ps;
   }
   ...
   function getShape(uint256 tokenId,uint256 i) internal pure returns(uint256){
    ...
    function makeShapeFactory(uint256 tokenId) internal pure returns(string memory){
        string[18] memory shapecolors=["#3EB650", "#ED2995", "#FA5456", "#41A39D", "#00E1DF", "#06B2BB", "#BEB947", "#2A3166", "#E12B38", "#FFED00", "#BAF2D8", "#DEE500", "#F71B2D", "#FFC900", "#F26419", "#B76CFD", "#A2592C", "#E42C6A"];
        ...
        string[6] memory colors=["#1C6FFC", "#FFFFFF", "#FAF182", "#000000", "#F1FFC4", "#7BFFC9"];
        uint8[9] memory heights=[100,90,164,152,94,160,144,192,146];
        ShapeArt memory shape;
        shape.bgcolor = getBgColor(tokenId);
        ...
        for(uint256 i=0;i<shape.numshapes;i++){
            ...
            if(shape.numshapes==5){attributes.yoffset=55;}
            if(shape.numshapes==6){attributes.yoffset=45;}
            if(shape.numshapes==7){attributes.yoffset=40;}
            ...
            parts[1] = string(abi.encodePacked(parts[1],paths[attributes.shapeindex],shapecolors[pluckNum(tokenId,toString(i*1800),0,19)],'" fill-opacity="0.',
                        toString(pluckNum(tokenId,toString((i+1)*1784),65,76)),
                        '"  transform="translate(',
                        toString(68+attributes.x)));
            parts[1] = string(abi.encodePacked(parts[1],' ',
                                toString(115 + attributes.y),')  rotate(',
                                toString(pluckNum(tokenId,toString((i+1)*1666),0,180))));
            parts[1] = string(abi.encodePacked(parts[1],' ',toString(attributes.width/2),' ',toString(attributes.height),')"/>'));
        }
        parts[0] = string(abi.encodePacked('<svg width="400" height="600" viewBox="0 0 400 600" fill="none" xmlns="http://www.w3.org/2000/svg"><rect width="400" height="600" fill="',colors[shape.bgcolor],'"/>'));
        parts[2] = '</svg>';
...
contract ShapeArts is ERC721Enumerable, Ownable, ReentrancyGuard{
    ...
    constructor() ERC721("ShapeArts", "shapearts")  Ownable(){}
    

    function tokenURI(uint256 tokenId) override public view returns (string memory){
       require(tokenId > 0 && tokenId <= minted, "Invalid token ID");
       string memory output = Base64.encode(bytes(string(abi.encodePacked('{"name": "Shape Art #', ShapeArtGenerator.toString(tokenId),'", "Description": "Abstract art on-chain for you to own and show off.","attributes":[],', 
        ' "image": "data:image/svg+xml;base64,', Base64.encode(bytes(ShapeArtGenerator.makeShapeFactory(tokenId))), '"}'))));
