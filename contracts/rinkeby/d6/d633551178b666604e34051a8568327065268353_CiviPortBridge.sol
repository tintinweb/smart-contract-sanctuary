...
contract CiviPortBridge is SecureContract
{
    ...
    uint64 public bridgeID;

    using BitShifter for bytes;

    constructor(address tokenContract, uint64 bridgeId) SecureContract()
    {
        _setupRole(_DEPOSIT, msg.sender);
        token = TokenInterface(tokenContract);
        bridgeID = bridgeId;
    }
    ...
    function deposit(bytes memory clientSignature, bytes memory serverSignature, bytes memory transactionData) public pause
    {
        address clientSigner = Signer.recoverPrefixedTxData(clientSignature, transactionData);
        address serverSigner = Signer.recoverPrefixedTxData(serverSignature, transactionData);

        require(hasRole(_DEPOSIT, serverSigner), "Permission denied");
        ...
        require(depositBridgeId == bridgeID, "Incorrect network");
        require(!depositNonces[nonce], "Nonce already used");

        token.mintTo(user, numTokens);
    ...
    function withdraw(bytes memory clientSignature, bytes memory transactionData) public pause
    {
        ...
        require(withdrawalBridgeId == bridgeID, "Incorrect network");
        require(!withdrawNonces[nonce], "Nonce already used");

        token.burnFrom(user, numTokens);
