...
contract Drillheadz is ERC721Enumerable, Ownable, ReentrancyGuard {

	uint256 public constant DRL_MAX_AMOUNT = 5555;
	uint256 public DRL_PER_TX = 2;
	uint256 public PRICE = 0.1 ether;
	uint256 public constant TIME_FOR_GROWN = 3 days;
	address public founder = 0x119ec6293D71c3eB4235091df135d0BC6D8B2291;
	...
	function mintPublic(uint256 amount) external payable nonReentrant() {
		...
		require(amount <= DRL_PER_TX, "You can't mint that many tokens");
		require(totalSupply() + amount <= DRL_MAX_AMOUNT, "Mint would exceed max supply of tokens");
	...
	function enableClaimFoundingMembers(address[] memory members, uint256[] memory amounts) external onlyOwner {
		...
		for (uint256 i = 0; i < members.length; i++) {
			...
			require(amounts[i] <= DRL_PER_TX, "amount is more than the allowable");
	...
	function setIpfsHash(string[] memory ipfsHashs, uint256[] memory idTokens) external onlyOwner {
		...
		for (uint256 i = 0; i < idTokens.length; i++) {
			...
			require(idTokens[i] <= DRL_MAX_AMOUNT, "id is more than the allowable");
	...
	function makePresaleMemberMint(uint256 tokenAmount, uint256 etherAmount) private {
		...
		require(totalSupply() + tokenAmount <= DRL_MAX_AMOUNT, "Mint would exceed max supply of tokens");
		require(mintedPresaleCount + tokenAmount <= DRL_MAX_AMOUNT, "Mint would exceed max pre-sale tokens");
	...
	function setEntityBirthdaysPerTx(uint256 newPerTx) public onlyOwner {
		...
		emit SetNewValue("DRL_PER_TX", DRL_PER_TX, newPerTx);
		DRL_PER_TX = newPerTx;
	}
