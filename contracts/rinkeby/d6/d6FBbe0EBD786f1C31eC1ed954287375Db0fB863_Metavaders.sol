    ...
    ERC721Enumerable
{
    ...
    uint256 public start_time = 1631473200; // start time:  Sunday, September 12, 2021 7:00:00 PM UTC
    // uint256 public start_time = 1630921712; // start time for testing only
    ...
    function mint(uint256 num) public payable virtual {
        ...
        require(block.timestamp > start_time,           "Mint not yet started");
        require( !_paused,                              "Mint is paused" );
        require( num < 20,                              "You can mint a maximum of 20 Metavaders per txn" );
        require( supply + num < max_mint - _reserved,   "Exceeds maximum Metavaders supply" );
        // require( tokenInWallet(_msgSender()) < 50,      "You can only mint a maximum of 50 Metavaders per wallet");
    ...
    function presale_mint(uint256 num) public payable virtual {
        uint256 supply = totalSupply();
        require( !_paused,                              "Mint is paused" );
        require( num < 5,                               "You can presale mint a maximum of 5 Metavaders per txn" );
        require( supply + num < 100,                    "Exceeds Presale Metavaders supply" );
        // require( tokenInWallet(_msgSender()) < 10,      "You can only presale mint a maximum of 10 Metavaders per wallet");
        require( msg.value >= _price * num,             "Ether sent is not correct");
    ...
    // Used start time in case of issues - Only done by owner
    ...
    function remainingSupply() public view returns(uint256) {
        return (max_mint - totalSupply() - _reserved);
    }
}
