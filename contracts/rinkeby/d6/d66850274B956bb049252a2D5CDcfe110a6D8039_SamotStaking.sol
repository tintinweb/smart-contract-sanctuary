...
contract SamotStaking is Ownable, IERC721Receiver, ReentrancyGuard, Pausable {
    ...
    address public og = 0x7cca1e4879a62A4B6173FAF0B865217722a47751;
    ...
    // Set functions
    function setOgContract(address _og) external onlyOwner {
        og = _og;
    }
    ...
    function setNFTContract(address _stakingDestinationAddress)
        external
        onlyOwner
    {
        stakingDestinationAddress = _stakingDestinationAddress;
    ...
    // Manual Staking function
    function manualstake(uint256[] calldata tokenIds, address _address)
        external
        onlyOwner
    {
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            _deposits[_address].add(tokenIds[i]);
        }
    }
