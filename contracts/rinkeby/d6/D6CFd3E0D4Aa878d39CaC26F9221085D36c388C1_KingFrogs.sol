    ...
    */
    ...
    function mintFrog(uint numberOfTokens) external payable {
        ...
        for(uint i = 0; i < numberOfTokens; i++) {
            ...
            uint mintIndex = numPublicMinted++;
    ...
    function addTraits(uint seed, uint tokenId) internal {
        for(uint8 i = 0; i < NUM_TRAITS; i++){            
            nonce++;
            frogTraits[tokenId][i] = determineTrait(i, seed);
        }
    }

    // function claimAllFrogs() external{
    //     uint numDucksOwned = SupDucks.balanceOf(msg.sender);
    //     uint tokenId;
    //     for(uint i = 0; i < numDucksOwned; i++){
    //         tokenId = SupDucks.tokenOfOwnerByIndex(msg.sender, i) + 10000;
    //         if(!_exists(tokenId)){
    ...
    function claimFrog(uint tokenId) external {
        require(msg.sender == SupDucks.ownerOf(tokenId), "You don't own this duck");
        require(!_exists(tokenId + 10000), "token already claimed");
        uint mintIndex = tokenId + 10000;
        _safeMint(msg.sender, mintIndex);
    }

    // function copyTraits(uint tokenId, uint mintIndex) internal {
    //     Duck memory duck;
    //     (duck.background, duck.skin, duck.clothes, duck.hat, duck.mouth, duck.eyes) = SupDucks.getTraits(tokenId);
    //     frogTraits[mintIndex][0] = duck.background;
    //     frogTraits[mintIndex][1] = duck.skin;
    //     frogTraits[mintIndex][2] = duck.hat;
    //     frogTraits[mintIndex][4] = duck.eyes;
    // }
    ...
    function getTraits(uint tokenId) external view returns (uint, uint, uint, uint, uint, uint){
        ...
        if(tokenId > MAX_FROGS_PUBLIC){
            ...
            return(
                ...
                duck.skin,
                duck.hat,
                uint8(uint(keccak256(abi.encodePacked(tokenId, deployedBlock))) % 4),
                duck.eyes,
                uint8(uint(keccak256(abi.encodePacked(tokenId, deployedBlock))) % 12)
            );
        }else{
            return (
    ...
    function getCurrentPrice() view public returns (uint){
        uint interval = (block.timestamp - START) / 30 minutes;
        if(interval > 10){
            return 0;
        }
        return mintPrices[interval];
    }
