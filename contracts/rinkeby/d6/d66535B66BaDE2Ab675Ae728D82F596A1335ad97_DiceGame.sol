...
/*
uint256 MAX_BET = 1 ether; see if it can be made a state variable
*/

contract DiceGame is VRFConsumerBase,ChainlinkClient {
    ...
    mapping (address => uint256) games_played;
    
    enum GAMESTAT{ RUNNING, PAUSED }
    GAMESTAT gameStatus;

    
    event DiceGameLost(bytes32 indexed requestId, uint256 first, uint256 second, uint256 third);
    event DiceGameWon(bytes32 indexed requestId, uint256 first, uint256 second, uint256 third, uint256 amountWon);
    event notEnoughBalance(uint256 first);
    event requestIDGenerated(bytes32 indexed requestId,bytes32 guess,address _address);
    event FlipGameWon(bytes32 indexed requestId, uint256 first, uint256 amountWon);
    event FlipGameLost(bytes32 indexed requestId, uint256 first);
    ...
    function rollDice(bytes32 guess, uint modulo) public payable returns (bytes32 requestId)  {
        ...
        if(games_played[msg.sender] > 0){
            ...
            // games_played[msg.sender] would be set to 0 when Chainlink returns the random number
            require(block.timestamp - games_played[msg.sender] > 10 minutes, 'Need to wait 10 minutes');
        ...
        games_played[msg.sender] = block.timestamp;
        
        emit requestIDGenerated(requestId,guess,msg.sender);
    ...
    function getHouseEdge(uint256 amount) private pure returns (uint256){
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
	    ...
	    if(bet.modulo == 6){
            ...
            if(gameWon){
                ...
                emit DiceGameWon(requestId, first, second, third,amountWon);
            }else{
                //game lost
                emit DiceGameLost(requestId, first, second, third);
            }
            ...
            if(gameWon){
                ...
                winnerList[bet.gambler] = winnerList[bet.gambler] + amountWon;
                totalAmountToBePaid += amountWon;
                emit FlipGameWon(requestId, flip_coin,amountWon);
            }else{
                //game lost
                emit FlipGameLost(requestId, flip_coin);
            }
    ...
    function withdraw() public {
		...
		if(balanceContract < amount){
		    emit notEnoughBalance(balanceContract);
		}else{
