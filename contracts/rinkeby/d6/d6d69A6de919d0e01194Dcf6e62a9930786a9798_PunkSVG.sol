...
contract PunkSVG is ERC721URIStorage {
    uint256 price;
    ...
    mapping(uint256 => string) public punkIndexToData;
    mapping(uint256 => bool) public punkDataCreated;

    event Assign(address indexed to, uint256 punkIndex);

    constructor() ERC721("PunkSVG", "punkSVG") {
    ...
    function create(uint256 punkIndex)
        public
        view
        onlyOwner
        returns (string memory tokenURI)
    {
        uint256 randomNumber = punkIndex; // make it more random
        string memory svg = generateSVG(randomNumber);
        string memory imageURI = svgToImageURI(svg);
        tokenURI = formatTokenURI(imageURI);
    }

    function setInitialOwner(address to, uint256 punkIndex) public onlyOwner {
        require(!allPunksAssigned, "Out of stock!");
        require(punkIndex <= totalSupply, "Void are you in ?");
        require(!punkDataCreated[punkIndex], "Punk data already exist!");
        require(punkIndexToAddress[punkIndex] != to, "Punk already assigned!");

        string memory punkSvgTokenUri = create(punkIndex);
        ...
        // index -> token uri
        punkIndexToData[punkIndex] = punkSvgTokenUri;

        // index -> bool
        punkDataCreated[punkIndex] = true;
    }

    function buyPunkSvg(uint256 punkIndex) public payable {
        require(msg.value >= price, "Need to pay more");
        require(
            punkIndexToAddress[punkIndex] != msg.sender,
            "Cannot reassign punk to the same addr"
        );
        require(punkIndex <= totalSupply, "Void are you in ?");
    ...
    function getImageData(uint256 punkIndex)
        public
        view
        returns (string memory punkSvgTokenUri)
    {
        require(
            punkIndex >= 0 && punkIndex <= totalSupply,
            "Only 5 Punks exist"
        );

        punkSvgTokenUri = punkIndexToData[punkIndex];

        return punkSvgTokenUri;
    }
