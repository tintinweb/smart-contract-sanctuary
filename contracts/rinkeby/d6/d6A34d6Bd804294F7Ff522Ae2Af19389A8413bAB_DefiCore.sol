...
contract DefiCore is IDefiCore, AbstractDependant {
    ...
    modifier onlyLiquidityPools() {
        require(
            assetParameters.existingLiquidityPools(msg.sender),
    ...
    function getUserLiquidationInfo(
        ...
        ILiquidityPool _receiveLiquidityPool =
            ILiquidityPool(_assetParameters.liquidityPools(_receiveAssetKey));

        uint256 _receiveAssetPrice = _receiveLiquidityPool.getAssetPrice();
        uint256 _bonusPrice =
            (_receiveAssetPrice *
                (DECIMAL - _assetParameters.getLiquidationDiscount(_receiveAssetKey))) / DECIMAL;

        uint256 _liquidationLimitByBorrow =
            (getTotalBorrowBalanceInUSD(_userAddr) *
                systemParameters.getLiquidationBoundaryParam()) / DECIMAL;

        uint256 _maxQuantityInUSD =
            _receiveAssetKey.getMaxQuantity(
                _userAddr,
        ...
        _liquidationInfo = UserLiquidationInfo(
            ...
            _receiveAssetPrice,
            _bonusPrice,
            _borrowLiquidityPool.getUserBorrowedAmount(_userAddr),
            _receiveLiquidityPool.getCurrentLiquidityAmount(_userAddr),
    ...
    function updateCompoundRate(bytes32 _assetKey) external {
        _assetKey.getAssetLiquidityPool(assetParameters).updateCompoundRate();
    }
    ...
    function addLiquidity(bytes32 _assetKey, uint256 _liquidityAmount) external {
        ...
        ILiquidityPool _assetLiquidityPool = _assetKey.getAssetLiquidityPool(assetParameters);
        ...
        assetsRegistry.updateSupplyAssets(msg.sender, _assetKey, address(_assetLiquidityPool));
    }
    ...
    function withdrawLiquidity(
        ...
        if (!_isMaxWithdraw) {
            ...
            _assetLiquidityPool.withdrawLiquidityMax(msg.sender);
        }
    ...
    function approveToDelegateBorrow(
        ...
        ILiquidityPool(_assetKey.getAssetLiquidityPool(_parameters)).approveToBorrow(
            msg.sender,
            _borrowAmount,
            _delegatee,
            _expectedAllowance
    ...
    function borrow(bytes32 _assetKey, uint256 _borrowAmount) external {
        ...
        assetsRegistry.updateBorrowAssets(msg.sender, _assetKey, address(_assetLiquidityPool));
    }
    ...
    function delegateBorrow(
        ...
        assetsRegistry.updateBorrowAssets(_borrowerAddr, _assetKey, address(_assetLiquidityPool));
    }
    ...
    function repayBorrow(
        ...
        _repayAmount = _assetLiquidityPool.repayBorrow(msg.sender, _repayAmount, _isMaxRepay);
    ...
    function delegateRepayBorrow(
        ...
        _assetLiquidityPool.closeBorrow(_recipientAddr, msg.sender, _repayAmount);

        emit BorrowRepaid(_recipientAddr, _assetKey, _repayAmount);

        assetsRegistry.updateBorrowAssets(_recipientAddr, _assetKey, address(_assetLiquidityPool));
    }
    ...
    function liquidation(
        ...
        uint256 _liquidationLimitByBorrow =
            (_totalBorrowBalanceInUSD * systemParameters.getLiquidationBoundaryParam()) / DECIMAL;
        uint256 _liquidationAmountInUsd = _borrowAssetsPool.getAmountInUSD(_liquidationAmount);

        require(
            _liquidationAmountInUsd <=
        ...
        uint256 _amountToLiquidateInUsd =
            _borrowAssetsPool.getAmountInUSD(
                _borrowAssetsPool.closeBorrow(_userAddr, msg.sender, _liquidationAmount)
            );
        ...
        uint256 _repayAmount =
            _supplyAssetKey.getRepayLiquidationAmount(
                _supplyAssetsPool.getAmountFromUSD(_amountToLiquidateInUsd),
                _assetParameters
            );
        ...
        _assetsRegistry.updateSupplyAssets(_userAddr, _supplyAssetKey, address(_supplyAssetsPool));
        _assetsRegistry.updateBorrowAssets(_userAddr, _borrowAssetKey, address(_borrowAssetsPool));
    }
...
library AssetsHelperLibrary {
    ...
    function getMaxQuantity(
        ...
        uint256 _userBorrowAmountInUSD =
            _borrowAssetsPool.getAmountInUSD(_borrowAssetsPool.getUserBorrowedAmount(_userAddr));
