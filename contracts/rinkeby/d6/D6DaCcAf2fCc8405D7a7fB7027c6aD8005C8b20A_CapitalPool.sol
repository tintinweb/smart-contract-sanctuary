...
contract CapitalPool is ICapitalPool, OwnableUpgradeable, AbstractDependant {
    ...
    function _setOrIncreaseTetherAllowance(address _contractAddress, uint256 _stbAmount) internal {
        ...
        if (exceedsAllowance) {
            if (!allowanceIsZero) {
                stblToken.approve(_contractAddress, 0);
            }
    ...
    function addPolicyHoldersHardSTBL(
        ...
        address _userLeveragePoolAddress;
        ...
        (_vStblDeployedByRP, , _lStblDeployedByLP, _userLeveragePoolAddress) = (
        ...
        // factors.vStblDeployedByRP = _protocolFee; // TODO Confirm on documentation
        factors.vStblDeployedByRP = _vStblDeployedByRP;
        factors.vStblOfCP = _vStblOfCP;
        factors.userLeveragePoolAddress = _userLeveragePoolAddress;
        ...
        reinsurancePoolBalance += reinsurancePoolPremium;
        leveragePoolBalance[_userLeveragePoolAddress] += userLeveragePoolPremium;
        ...
        if (reinsurancePoolPremium > 0) {
            reinsurancePool.addPolicyPremium(
                _epochsNumber,
                DecimalsConverter.convertTo18(reinsurancePoolPremium, stblDecimals)
            );
        }
        if (userLeveragePoolPremium > 0) {
            ILeveragePortfolio(_userLeveragePoolAddress).addPolicyPremium(
                _epochsNumber,
        ...
        )
    {
        ...
        uint256 _totalCoverTokens =
            DecimalsConverter.convertFrom18(
                (IPolicyBook(_policyBookAddress)).totalCoverTokens(),
                stblDecimals
            );

        if (_totalCoverTokens > 0 && factors.vStblOfCP > 0) {
            factors.poolUtilizationRation = _totalCoverTokens
                .mul(PERCENTAGE_100)
                .div(factors.vStblOfCP.mul(PERCENTAGE_100))
                .mul(PRECISION)
                .mul(100);
        } else {
        ...
        if (factors.lStblDeployedByLP > 0) {
            factors.participatedlStblDeployedByLP = factors
                .lStblDeployedByLP
                .mul(
                leveragePortfolioView.calcM(
                    factors.poolUtilizationRation,
        ...
        return _setupPremiumCalculation(factors);
    }
    ...
    function rebalanceLiquidityCushion() public override broadcastBalancing onlyMaintainer {
        ...
        emit LiquidityCushionLocked(_startTime, _endTime, _requiredLiquidity, _withdraw, _deposit);
    }
    ...
    function deployFundsToDefi(uint256 _stblAmount) internal {
        ...
        stblToken.safeApprove(address(yieldGenerator), _stblAmount);
        ...
        onlyOwner
    {
        ...
        uint256 _startTime = block.timestamp.sub(_duration);

        uint256 index = liquidityRegistry.findWithdrawlAfterTime(_startTime);
        ...
        )
    {
        ...
        factors.premiumPerDeployment = (factors.totalLiqforPremium.mul(PRECISION)).div(
            factors.premiumPerDay.mul(PRECISION)
        );
        ...
        return (
            factors.reinsurancePoolPremium,
            factors.userLeveragePoolPremium,
            factors.coveragePoolPremium
        );
    }
    ...
    ) public pure returns (uint256 deposit, uint256 withdraw) {
