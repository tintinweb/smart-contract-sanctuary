...
contract DelegateContract is Ownable, IERC721Receiver, ERC1155Receiver
{
   ...
   function instantBuy(address _nftTokenAddress, TokenType _tokenType, uint256 _tokenId, bytes memory _data) public 
   {
       ...
       else if(_tokenType == TokenType.ERC1155)
       {
            ...
            _erc1155Contract.safeTransferFrom(address(this), msg.sender, _tokenId, saleItem.quantity, _data);
            saleItem = SaleItem(address(0), 0, false, 0, address(0));
            instantBuyDetails[_nftTokenAddress][_tokenId] = saleItem;
   ...
   function OfferForSale(address _erc20TokenAddress, address _nftTokenAddress, uint256 _tokenId, uint256 _quantity, TokenType _tokenType, uint256 _startingPrice, string memory _userId, PricingType _pricingType, uint256 _startTime, uint256 _endTime) public
   {
       ...
       }else if(_tokenType == TokenType.ERC1155)
       {
           ...
           transferERC1155(_nftTokenAddress, msg.sender, address(this), _tokenId, _quantity, "0x0");
...
contract ERC1155NafitiBasic is ERC1155 {
    ...
    // Mapping of Token Uri to Token Type
    ...
    function mint(uint quantity, string memory _uri) public returns (uint256){
        uint256 token_type = totalTokens + 1;
        _mint(msg.sender, token_type, quantity, "");
        _tokenUri[token_type] = _uri;
        totalTokens++;
        TokenMinted(token_type, quantity, msg.sender);
        return token_type;
    }

    function tokenUri(uint256 tokenType) public view returns (string memory)
    {
        return _tokenUri[tokenType];
    }

    function uri(uint256 tokenType) external view virtual override returns (string memory) {
        return _tokenUri[tokenType];
    }
...
contract NaffToken is ERC20, Ownable {
    ...
    constructor() public ERC20("NAFF Token", "NAFF") {
        ...
        _transfer(address(this), msg.sender, initialSupply);
    }
}
