...
contract Lottery is VRFConsumerBase, Ownable {
    ...
    AggregatorV3Interface usdEthPriceFeed;
    enum LOTTERY_STATE {
        OPEN,
        CLOSED,
        CALCULATING_WINNER
    ...
    function getEntranceFee() public view returns (uint256) {
        ...
        uint256 adjustedPrice = uint256(price) * 10**10; //eth price feed comes with 8 decimals, so we add 10 more
    ...
    function endLottery() public onlyOwner {
        //uint(keccak256(abi.encodePacker(nonce,msg.sender,block.difficulty,block.timestamp))) % players.length;
