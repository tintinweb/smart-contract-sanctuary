...
// File: contracts/EGGVERSEAuction.sol
pragma solidity ^0.6.0;

contract EGGVERSEAuction is ERC721, Ownable {
    ...
    event CreateAuction(uint256 _tokenId, uint8 _auctionId, string _tokenURI, address _proposer);

    constructor(string memory _baseURI) public ERC721("Eggverse Auction", "EGGA") {
     ...
     */
    function getAuctionName(uint8 _auctionId)
        external
        view
        returns (string memory)
    {
        return auctionNames[_auctionId];
    }
    ...
    function mint(
        ...
        _tokenIds.increment();
        auctionIds[newId] = _auctionId;
        auctionCount[_auctionId] = auctionCount[_auctionId].add(1);
        _mint(_to, newId);
        _setTokenURI(newId, _tokenURI);

        emit CreateAuction(newId, _auctionId, _tokenURI, msg.sender);
        return newId;
    }
     ...
     */
    function setAuctionName(uint8 _auctionId, string calldata _name)
        external
        onlyOwner
    {
        auctionNames[_auctionId] = _name;
    }
