...
contract Museum is ERC721Enumerable, PaymentSplitter, Ownable {
  ...
  uint256 public constant MAX_SUPPLY = 4800;
  uint256 public constant NFT_PRICE = 0.04 ether;

  uint256 private _randomCounter = 0;
  ...
  function mint(uint256 amount) public payable {
    require(_isMintingActive, "Museum: sale is not active");
    require(amount > 0, "Museum: must mint more than 0");
    require(amount <= 20, "Museum: must mint fewer than 20");
    require(totalSupply() < MAX_SUPPLY, "Museum: sale has ended");
    require(totalSupply() + amount <= MAX_SUPPLY, "Museum: exceeds max supply");
    require(amount * NFT_PRICE == msg.value, "Museum: must send correct ETH amount");

    for (uint i = 0; i < amount; i++) {
      uint256 randomID = getRandomID(0);
  ...
  function admint(address owner, uint256 tokenID) public onlyOwner {
    require(!_exists(tokenID), "Museum: cannot mint an already existing token");
    require(tokenID <= MAX_SUPPLY, "Museum: cannot mint outside of the max supply");

    getRandomID(tokenID);
    _mint(owner, tokenID);
  }

  function getRandomID(uint256 chosenIndex) private returns (uint256){
    uint256 remaining = MAX_SUPPLY - totalSupply();

    uint256 index;
    if (chosenIndex == 0){
        index = uint256(keccak256(abi.encodePacked(_randomCounter, "museum", blockhash(block.number), block.timestamp, msg.sender, block.difficulty, gasleft()))) % remaining;
    } else {
        index = chosenIndex;
    }
