...
contract BuyBack is Ownable{
    ...
    * @param _GDAO token contract
    ...
    constructor(
        ...
        gasReimbursement = 5 * PERCENT_PRECISION;
        forGovVault = 25 * PERCENT_PRECISION;
        forLPool = 25 * PERCENT_PRECISION;
        forTreasury = 45 * PERCENT_PRECISION;
    ...
    * @dev notice, the rewards must sum to 100%. denominated as 10^14
     */
    function updateFundPlan(uint48 _gasReimbursement, uint48 _forGovVault, uint48 _forLPool, uint48 _forTreasury) external onlyOwner {
        require((_gasReimbursement + _forGovVault + _forLPool + _forTreasury ) == 
        ONE_HUNDRED_PERCENT, 
        "All inputs mustsum to 10^14 (100%)"
        );
    ...
    */
    ...
    function emergencyWithdrawl(address _coin, address _receiver) external onlyOwner{
        _emergencyWithdrawl(_coin, _receiver);
    }

    function emergencyWithdrawlAll(address _receiver) external onlyOwner{
    ...
    * @return false if there's a 0 balance of that token
    ...
    function sweepGDAO(uint256 _amm) external returns(bool){
        ...
        uint256 toETH = _applyPercent(_amm, forLPool)/2;
        ...
        {value:address(this).balance-toETH}
    ...
    * @notice stolen from Stack Overflow. A way to look up a token's price in eth */
    ...
    /**
    * @dev notice that the WETH percent is calculated for each ERC20 before sweep. Whereas the GDAO
    * Equivalent to distribute is based off the GDAO available. So there is always going to be less
    * GDAO deposited than WETH. Which is actually good for price so nice.
    */
    function _distributeGDAO() internal{
        uint256 GDAOBal = IERC20(GDAO).balanceOf(address(this));
        uint256 ETHBal = address(this).balance;
        //send the GDAO around based on percents
        _sendGDAO(tx.origin, _applyPercent(GDAOBal, gasReimbursement));
        _sendGDAO(GOV_VAULT, _applyPercent(GDAOBal, forGovVault));
        _sendGDAO(TREASURY, _applyPercent(GDAOBal, forTreasury));
        //Send the ETH balance of contract and GDAO percent to uniswap
        GDAOBal = IERC20(GDAO).balanceOf(address(this));
        _addLP(ETHBal, GDAOBal);       //To not deal with percent/rounding issues just send remainder of GDAO with ETHBal
    }
