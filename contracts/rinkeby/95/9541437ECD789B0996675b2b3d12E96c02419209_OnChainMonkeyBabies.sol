...
contract OnChainMonkeyBabies is ERC721Enumerable, ReentrancyGuard, Ownable {
  using Strings for uint256;
  ...
  //  uint randomHash = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp,tokenId)));


  function randomOne(uint256 tokenId) internal view returns (Ape memory) {
      uint randomHash = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp,tokenId)));
    tokenId=randomHash; // avoid dupes
  ...
  constructor() ERC721("OnChainMonkey Babies", "BABYMANK") Ownable() {}
}