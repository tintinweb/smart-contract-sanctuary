...
interface IVinylsData {
    function getYear(uint256 tokenId) external view returns (uint256); 
    function getColors(uint256 tokenId) external view returns (string[6] memory);
    function getGenre(uint256 tokenId) external view returns (string memory);   
    function getArtist(uint256 tokenId) external view returns (string memory);
    function getAlbum(uint256 tokenId) external view returns (string memory);
    function getTrack(uint256 tokenId, uint256 salt) external view returns (string memory);
    function getVinylType(uint256 tokenId) external view returns (string memory);
}
...
contract WBMCVinylsND is ERC721, ReentrancyGuard, Ownable {
   ...
   string public baseURI;
    ...
    //interfaces
    IVinylsData private vinylsDC;
    IVinylsMetadata private vinylsMeta;
    IVIBE20 private vibeToken;
    IWBMC private WBMC;
    IWBMCEnum private WBMCEnum;

    function setDataContr(address _vinylsDataContr) public onlyOwner {
        vinylsDC = IVinylsData(_vinylsDataContr);
    }
    ...
    function getYear(uint256 tokenId) public view returns (uint256) {
        require(_exists(tokenId), "!Token");
        return vinylsDC.getYear(tokenId);
    }

    function getColors(uint256 tokenId) public view returns (string[6] memory) {
        require(_exists(tokenId), "!Token");
        return vinylsDC.getColors(tokenId);
    }
    
    function getGenre(uint256 tokenId) public view returns (string memory) {
        require(_exists(tokenId), "!Token");
        return vinylsDC.getGenre(tokenId);
    }
    
    function getArtist(uint256 tokenId) public view returns (string memory) {
        require(_exists(tokenId), "!Token");
        return vinylsDC.getArtist(tokenId);
    }
    
    function getAlbum(uint256 tokenId) public view returns (string memory) {
        require(_exists(tokenId), "!Token");
        return vinylsDC.getAlbum(tokenId);
    }

    function getTrack(uint256 tokenId, uint256 salt) public view returns (string memory) {
        require(_exists(tokenId), "!Token");
        return vinylsDC.getTrack(tokenId, salt);
    }

    function getVinylType(uint256 tokenId) external view returns (string memory) {
        require(_exists(tokenId), "!Token");
        return vinylsDC.getVinylType(tokenId);
    }
    ...
    */
    ...
    function claimEthTo(address _to) public payable nonReentrant {  
        require(publicSale, "Sale not started");
        require(totalSupply() + 1 <= maxVinyls, "MaxSupply");
    ...
    function increaseWLQuota(address _wlAddress) public onlyOwner {
        whitelisted[_wlAddress]++;
    }
