...
contract Alienz is ERC721, Ownable {
    using SafeMath for uint256;
    uint public constant MAX_PANDA = 10000;
    bool public hasSaleStarted = false;
    
    
    constructor() ERC721("Alienz","ALIENZ")  {
        setBaseURI("https://alienzzz.herokuapp.com/api/red-panda/");
        
        // Founders
        _safeMint(address(0x2515F18cf299bd2F61F179E2F1471Ea43B2783CF), 0);
        _safeMint(address(0x2515F18cf299bd2F61F179E2F1471Ea43B2783CF), 1);
        _safeMint(address(0x2515F18cf299bd2F61F179E2F1471Ea43B2783CF), 2);
        _safeMint(address(0x2515F18cf299bd2F61F179E2F1471Ea43B2783CF), 3);
        _safeMint(address(0x2515F18cf299bd2F61F179E2F1471Ea43B2783CF), 4);
        _safeMint(address(0x2515F18cf299bd2F61F179E2F1471Ea43B2783CF), 5);
    ...
    function calculatePrice() public view returns (uint256) {
        ...
        require(totalSupply() < MAX_PANDA, "Sale has already ended");
        ...
        if (currentSupply >= 9900) {
            ...
            return 500000000000000000; // 0.05 ETH
    ...
    function adoptPanda(uint256 numPandas) public payable {
        require(totalSupply() < MAX_PANDA, "Sale has already ended");
        require(numPandas > 0 && numPandas <= 20, "You can adopt minimum 1, maximum 20 pandas");
        require(totalSupply().add(numPandas) <= MAX_PANDA, "Exceeds max pandas");
    ...
    function mintMulti(uint256[] memory _tokenId) public payable {
        require(totalSupply() < MAX_PANDA, "Sale has already ended");
        require(_tokenId.length > 0 && _tokenId.length <= 20, "You can adopt minimum 1, maximum 20 panda");
        require(totalSupply().add(_tokenId.length) <= MAX_PANDA, "Exceeds maximum pandas");
        require(msg.value >= calculatePrice().mul(_tokenId.length), "Ether value sent is below the price");

        for (uint i = 0; i < _tokenId.length; i++) {
            require(_tokenId[i]<MAX_PANDA, "Token ID exceeds maximum ID");
        }
