...
contract Alchemist is  ReentrancyGuardUpgradeable {
...
CDP.Context public _ctx;
  ...
  mapping(address => CDP.Data) public _cdps;
 ...
 /// @dev The address of the band oracle.
  ...
  /// @dev Set oracle.
  function setOracleAddress(address Oracle,address _bandOracle, uint256 peg) external onlyGov {
    _linkGasOracle = Oracle;
    bandOracle=_bandOracle;
    pegMinimum = peg;
    emit oracleAddressUpdated(Oracle,_bandOracle, peg);
  }
  ...
  function harvest(uint256 _vaultId) external expectInitialized returns (uint256, uint256) {
    ...
    if (_harvestedAmount > 0) {
      ...
      if (_feeAmount > 0) {
        token.transfer(rewards, _feeAmount);
      }
  ...
  function repay(uint256 _parentAmount, uint256 _childAmount) external nonReentrant noContractAllowed onLinkCheck expectInitialized {
    ...
    if (_parentAmount > 0) {
      token.transferFrom(msg.sender, address(this), _parentAmount);
  ...
  modifier onLinkCheck() {
    if(pegMinimum > 0 ){
      ...
      IStdReference.ReferenceData memory data =  (IStdReference(bandOracle).getReferenceData("BNB","BUSD"));
      uint256 CurrentPrice=(data.rate + oracleAnswer)/2;
      require(CurrentPrice > pegMinimum, "off peg limitation");
    }
    _;
  }
