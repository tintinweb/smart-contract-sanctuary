...
contract StakingTokenWrapper is ReentrancyGuard {
    ...
    function _stake(
        ...
        nonReentrant
    {
        _totalSupply = _totalSupply.add(_amount.sub(_stakeFee));
        _balances[_beneficiary] = _balances[_beneficiary].add(_amount.sub(_stakeFee));
...
contract Staking is Ownable, StakingTokenWrapper {
    ...
    ****************************************/
    ...
    uint256 public minStakingAmount = 1 * 1e18; // 1 Token
    uint256 public stakeFeePercent = 1 * 1e18; // 1%

    uint256 public ownerSharePercent = 40 * 1e18;  // 40% of stakeFeePercent
    uint256 public devSharePercent = 30 * 1e18;  // 30% of stakeFeePercent
    uint256 public teamSharePercent = 30 * 1e18;  // 30% of stakeFeePercent

    uint256 public threeMonthRewardPercent  = 1.5 * 1e18;  // 1.5%
    uint256 public sixMonthRewardPercent    = 2 * 1e18;  // 2%
    ...
    uint256 public constant THREE_MONTHS_DURATION  = 3 hours;
    uint256 public constant SIX_MONTHS_DURATION    = 6 hours;
    ...
    ****************************************/
    ...
    event ShareClaimed(address indexed user, uint256 amount);
    event TeamAddressAdded(address indexed teamAddress);
    ...
    ****************************************/
    ...
    modifier isAccount(address _account) {
        require(
            ...
            "Only external owned accounts allowed"
        );
        _;
    }
    ...
    ****************************************/
    ...
    function stake3Mon(uint256 _amount)
        external
    {
        stakeInternal(msg.sender, _amount, THREE_MONTHS_DURATION);
    }
    
    function stake6Mon(uint256 _amount)
        external
    {
        stakeInternal(msg.sender, _amount, SIX_MONTHS_DURATION);
    }
    ...
    {
        require(
            block.timestamp >= stakeEnded[msg.sender], 
            "Reward cannot be claimed before 30 days"
        );
    ...
    function stakeInternal(address _beneficiary, uint256 _amount, uint256 _period)
        internal
        isAccount(_beneficiary)
    {
        ...
        // calculate stake fee
        uint256 _stakeFee = (stakeFeePercent.mul(_amount)).div(1e20);

        // Save the fees to addresses
        feeShares[owner()] = feeShares[owner()].add((ownerSharePercent.mul(_stakeFee)).div(1e20));
        feeShares[dev] = feeShares[dev].add((devSharePercent.mul(_stakeFee)).div(1e20));

        uint256 _teamShareAmount = (teamSharePercent.mul(_stakeFee)).div(1e20);

        uint256 _teamAddressesLength = teamAddresses.length;
        for(uint256 i = 0; i < _teamAddressesLength; i++) {
            uint256 _teamPersonShare = _teamShareAmount.div(_teamAddressesLength);
            address _personAddress = teamAddresses[i];
            feeShares[_personAddress] = feeShares[_personAddress].add(_teamPersonShare);
        }

        super._stake(_beneficiary, _amount, _stakeFee);
        stakeStarted[_beneficiary] = block.timestamp;

        uint256 _amountAfterFee = _amount.sub(_stakeFee);
        userStakedTokens[_beneficiary] = userStakedTokens[_beneficiary].add(_amountAfterFee);
        ...
        if (_period == THREE_MONTHS_DURATION) {
            _rewardAmount = (threeMonthRewardPercent.mul(_amount)).div(1e20);
            ...
            _rewardAmount = (sixMonthRewardPercent.mul(_amount)).div(1e20);
            rewards[_beneficiary] = _rewardAmount;
        ...
        emit Staked(_beneficiary, _amount, _rewardAmount, _stakeFee);
    }
    ...
    function claimShares() public {
        uint256 _amount = feeShares[msg.sender];
        require(_amount > 0, "Not a valid caller");
        rewardsToken.transfer(msg.sender, _amount);
        feeShares[msg.sender] = 0;
        emit ShareClaimed(msg.sender, _amount);
    }
    ...
    ****************************************/
    ...
    function getShare(address _account)
        public
        view
        returns (uint256)
    {
        return feeShares[_account];
    }

    function getAllTeamAddresses()
        public
        view
    ...
    ****************************************/
    ...
    function setThreeMonthRewardPercent(uint256 _amount)
        public
        onlyOwner()
    {
        require(_amount > 0, "amount not valid");
        threeMonthRewardPercent = _amount;
    }

    function setSixMonthRewardPercent(uint256 _amount)
        public
        onlyOwner()
    {
        require(_amount > 0, "amount not valid");
        sixMonthRewardPercent = _amount;
    }

    function setOwnerSharePercent(uint256 _amount)
        public
        onlyOwner()
    {
        require(_amount > 0, "amount not valid");
        ownerSharePercent = _amount;
    }

    function setDevSharePercent(uint256 _amount)
        public
        onlyOwner()
    {
        require(_amount > 0, "amount not valid");
        devSharePercent = _amount;
    }

    function setTeamSharePercent(uint256 _amount)
        public
        onlyOwner()
    {
        require(_amount > 0, "amount not valid");
        teamSharePercent = _amount;
    }
        ...
        onlyOwner() 
    {
        ...
        emit TeamAddressAdded(_teamAddress);
    }
    ...
    {
        require(
            rewardsToken.transfer(receiver, _amount), 
