...
contract LeEscrow is Ownable, ReentrancyGuard {
    ...
    function _deposit(address _user, uint256 _amount, uint256 _tokenID) internal{
        ...
        require(_tokenID >= 0 , "tokenID cannot be less than zero");
        ...
        userBalances[_user][1] = userBalances[_user][1].add(_amount.mul((ti.rates.div(1e6))));
    }
    ...
    function _withdraw(address _user, uint256 _amount, uint256 _tokenID) internal{
        ...
        TokenInfo memory ti = listOfTokenInfo[_tokenID];
        IERC20 token = IERC20(ti.tokenAddress);
        require(userBalances[_user][_tokenID] >= _amount.mul((ti.rates.div(1e6))), "Balance Less than withdrawn amount");
        userBalances[_user][_tokenID] = userBalances[_user][_tokenID].sub(_amount.mul((ti.rates.div(1e6))));
        token.transferFrom(treasury, _user, _amount.mul(10**ti.decimal));
    }
    ...
    function addToken(address _tokenAddress, uint256 _rates, uint256 _decimal) public onlyOwner{
        ...
        require(_rates > 0, "rates must be more than 0");
        require(_decimal > 0, "decimal must be more than 0");
