...
contract TheHaunted5 is ERC1155, ERC1155Burnable, ERC1155Supply, Ownable, Pausable, ReentrancyGuard {
  ...
  uint256 internal MAX_TOKEN_COUNT = 10;
  uint256 internal SUPPLY_PER_TOKEN = 3;
  uint256[] internal mintableTokens = [6,7,8,9];
  uint256 public constant mintPrice = 10000000000000000; // 0.01 ETH
  mapping(address => uint256) internal numberOfTokensMinted;
  ...
  function allowlistMint(bytes32[] calldata _proof) public payable nonReentrant {
    ...
    if (totalSupply(tokenId) + 1 == SUPPLY_PER_TOKEN) {
  ...
  function mint() public payable whenNotPaused nonReentrant {
    // require(numberOfTokensMinted[msg.sender] == 0);
    require(mintPrice <= msg.value, "Not enough ETH");
  ...
  function mintInternal(address _account, uint256 _tokenId, uint256 _quantity) internal {
    ...
    require(totalSupply(_tokenId) + _quantity <= SUPPLY_PER_TOKEN);
