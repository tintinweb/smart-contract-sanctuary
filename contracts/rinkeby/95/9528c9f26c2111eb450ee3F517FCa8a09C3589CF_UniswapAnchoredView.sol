...
contract BridgedOracle is PriceOracle {
    ...
    function registerChainlinkAsset(address token) public {
        require(!chainlinkAssets[token], "Already registered");
        chainlinkAssets[token] = true;
    }

    function deregisterChainlinkAsset(address token) public {
        require(!chainlinkAssets[token], "Already deregistered");
        chainlinkAssets[token] = false;
    }

    function registerChainlinkAsset(address[] memory tokens) public {
        for (uint i = 0; i < tokens.length; i++) {
            require(!chainlinkAssets[tokens[i]], "Already registered");
            chainlinkAssets[tokens[i]] = true;
        }
    }
}
