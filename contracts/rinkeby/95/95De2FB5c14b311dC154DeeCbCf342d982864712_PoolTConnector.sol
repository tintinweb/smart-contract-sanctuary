...
contract PoolTConnector is IPoolTogetherData{
    ...
    address public _poolStrategyAddress;
    address private _owner;

    IPoolTogether poolInstance;
    ...
    function initializePool(CompoundPrizePoolConfig memory poolConfig, MultipleWinnersBuilder.MultipleWinnersConfig memory prizeStrategy, uint8 decimals) public{
        IPoolTogether poolInstance = IPoolTogether(_strategy_address);
        poolInstance.createCompoundMultipleWinners(poolConfig,prizeStrategy, decimals);
        _owner = msg.sender;
    }

    function setPoolAddress(address poolAddress) isOwner public{
        _poolAddress = poolAddress;
    }

    function setPoolStrategyAddress(address poolStrategyAddress) isOwner public {
        _poolStrategyAddress = poolStrategyAddress;
    }

    function deposit(address to, uint256 amount, address controlledToken, address referrer) public{
        //create instance of pool 
        IPoolTogether poolInstance = IPoolTogether(_poolAddress);
