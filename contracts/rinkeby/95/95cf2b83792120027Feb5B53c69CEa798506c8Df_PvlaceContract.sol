...
contract PvlaceContract is ERC721, Ownable {
    ...
    string OWNER_PAYMENT_CONTRACT =
        "0x6f6049dB1763Dc7bCbdFd16E77df3f4b54545F68";

    // Public variables

    // !!! CHANGE SALE_START_TIMESTAMP to proper time.
    uint256 public constant SALE_START_TIMESTAMP = 1617410515;
    ...
    constructor() public ERC721("nft test", "rtest") {
        // tokenCounter = 1;
        _setBaseURI(
            "https://raw.githubusercontent.com/joeyiny/pvlacenft/main/unrevealed/"
        );
    }

    /*
	function purchaseToken() public returns (uint256) {

    	//creates a token
    	//adds token to tokens[] array
    	tokens.push(Token("PVLACE TOKEN"));

    	//id = length of array -1
    	uint id = tokens.length-1;

    	//mints token and gives it to address 'to'
    	_safeMint(msg.sender, id);

    	emit Minted(msg.sender, id, "PVLACE TOKEN");

        tokenIndexToOwner[id] = msg.sender;
		_setTokenURI(id,Strings.fromUint256(id));
    ...
    function getNFTMaxAmount() public view returns (uint256) {
        ...
        if (currentSupply >= 176) {
            return 7; // All tokens that cost 2 ETH and above can be bought 7 at a time
            ...
            return 1; // First 50 tokens can only be bought 1 at a time
        }
    }
    ...
    function getNFTPrice() public view returns (uint256) {
        ...
        if (currentSupply >= 350) {
            return 100000000000000000000; // 350 100 ETH
        } else if (currentSupply >= 311) {
            return 10000000000000000000; // 311 - 349 10 ETH
        } else if (currentSupply >= 246) {
            return 5000000000000000000; // 246 - 310 5 ETH
        } else if (currentSupply >= 176) {
            return 2000000000000000000; // 176 - 245 2 ETH
        } else if (currentSupply >= 111) {
            return 1000000000000000000; // 111 - 175 1 ETH
        } else if (currentSupply >= 51) {
            return 500000000000000000; // 51 - 110 0.5 ETH
        } else {
