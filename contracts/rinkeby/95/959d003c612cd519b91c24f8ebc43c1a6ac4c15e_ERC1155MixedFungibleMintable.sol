...
// File: contracts/safemathAddresslib.sol
...
// File: contracts/minterc1155.sol
...
contract ERC1155MixedFungibleMintable is ERC1155MixedFungible {
    ...
    function _creators(uint256 nft_id) external view returns(address){
        return(creators[nft_id]);
    }
    
    function royality (uint256 nft_id) external view returns(uint256 _royalities){
        return (royalities[nft_id]);
    }
    ...
    function mintNonFungible(uint256 _type, address[] calldata _to , uint256 _royality ) external creatorOnly(_type) {

        // No need to check this is a nf type rather than an id since
        // creatorOnly() will only let a type pass through.
        require(isNonFungible(_type));
        royalities[_type] = _royality;
