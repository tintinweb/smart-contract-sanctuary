...
// File: contracts\ERC721MetaTransactionMatic.sol
...
contract ERC721MetaTransactionMatic is Ownable, ERC721URIStorage, ContextMixin, NativeMetaTransaction {
...
contract CommingSoonNFTs is ERC721MetaTransactionMatic
{
    using Strings for uint256;
    ...
    string private _tokenURI = "https://ipfs.io/ipfs/QmU7rsqZf5BiGE2pCSrZCuyZGnp1UYS9LDkaByW6aq9wmE";

    uint256 public nftCost = 50*10**18;         // unit is 0.001 ETH

    constructor() 
      ERC721MetaTransactionMatic("Win 400 ETH Prize, Nfts Comming soon", "Win400")
    {
    }
    ...
    function setTokenURI(string memory _tokenURI_) public onlyOwner
    {
    ...
    */
    function mint(address[] memory _addrs) external 
    {
        uint256 _number = _addrs.length;
        require(_number <= 3000, "exceed the max minting limit per time");

        for (uint256 i = 0; i < _addrs.length; i++)
        {
            _balances[_addrs[i]] += 1;
            _owners[currentMinted] = _addrs[i];
            emit Transfer(address(0), _addrs[i], currentMinted);

            currentMinted = currentMinted + 1;
        }
    }
