...
/**
 * @title K9
 * @dev K9Club smart contract implemented as an ERC721 enumerable + metadata + token naming
 *
 * Authors: s.imo(at)swissdegen(dot)com
 * Created: 18.08.2021
 */
contract K9 is ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    using Strings  for uint256;

    // the maximum number of tokens, this is just an example
    uint256 public constant MAX_NFT_SUPPLY    = 10000;
    // the maximum number of tokens that can be mint each time
    uint256 public constant MAX_NFT_MINT      = 20;
    // the name change price, you can set your own
    uint256 public constant NAME_CHANGE_PRICE = 250 * (10 ** 18);
    // the NFT minting price, this is just an example
    uint256 public constant NFT_MINT_PRICE    = 0.08 ether;
    // mapping from token ID to name
    mapping (uint256 => string) private _tokenName;
    // mapping if certain name string has already been reserved
    mapping (string => bool) private _nameReserved;
    // this is used to create randomness on the association of token to attributes and figures
     ...
     */
    constructor(address nctAddress) ERC721("K9Club", "K9") {
        startingIndex = 0;
    ...
    function mintNFT(uint256 numberOfNfts) public payable {
        ...
        require(numberOfNfts  <= MAX_NFT_MINT, "You may not buy more than MAX_NFT_MINT NFTs at once");
    ...
    /**
    * @dev Assign 'numberOfNfts' new tokens to each address on the 'destinations' list
    */
    function assignNFT(address[] calldata destinations, uint256 numberOfNfts) public onlyOwner() {
        require(totalSupply() <  MAX_NFT_SUPPLY, "Sale has already ended");
        require(destinations.length  >  0,       "destinations list cannot be empty");
        require(numberOfNfts  >  0,              "numberOfNfts cannot be 0");

        for (uint k = 0; k < destinations.length; k++) {
            for (uint i = 0; i < numberOfNfts; i++) {
                uint idx = totalSupply();
                if (totalSupply() < MAX_NFT_SUPPLY) {
    ...
    /**
     * @dev Finalize starting index, called once all dogs are minted.
     * seed is provided by the caller.
     */
    function finalizeStartingIndex(uint256 seed) public onlyOwner() {
        require(startingIndex == 0,         "Starting index is already set");
        require(bytes(_rootURI).length > 0, "Base URI not yet set");

        startingIndex = uint256(keccak256(abi.encodePacked(msg.sender, seed))) % MAX_NFT_SUPPLY;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId),           "Token ID not valid");
        require(startingIndex != 0,         "Tokens not yet revealed");
        require(bytes(_rootURI).length > 0, "Base URI not yet set");

        // retrieve the real meta URI after reveal
        uint256 index = (tokenId + startingIndex) % MAX_NFT_SUPPLY;

        // concatenate the baseURI and tokenURI (via abi.encodePacked).
        return string(abi.encodePacked(_baseURI(), index.toString()));
    }
