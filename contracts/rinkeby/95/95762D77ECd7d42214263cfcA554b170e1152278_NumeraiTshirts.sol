...
//  EnumerableMap.UintToAddressMap private _tokenOwners;
  ...
  {
    _myBaseURI = "ipfs://bafybeigucoeo2xgq3ivoco5nv2x6hx7vrhipxy6v2kkwmwzdrxqtcdw7r4/";
  }
  ...
  /// the user uses the signed voucher to redeem the NFT associated with a specific
  /// product.
  struct Voucher {
    uint256 tokenId;
    bytes signature;
  ...
  /// @notice Redeems a voucher for a NFT and returns the tokenId
  function redeem(Voucher calldata voucher) external returns (uint256) {
    // Make sure voucher is valid
    _verify(voucher);

    // Mint the NFT to msg.sender. Will revert if tokenId is already owned!
  ...
  /// @dev Will revert if the signature does not match the tokenId and productId
  ...
  /// @notice Redeems a voucher for a NFT and returns the tokenId
  function redeemToAddress(Voucher calldata voucher, address recipient) external onlyOwner returns (uint256) {
    // Make sure voucher is valid
    _verify(voucher);

    // Mint the NFT to recipient. Will revert if tokenId is already owned!
    _mint(recipient, voucher.tokenId);
  ...
  onlyOwner
  {
    ...
    for (uint256 i = 0; i < recipients.length; i++) {
      ...
      _mint(recipients[i],specialId[i]*1000000);
    }
  }
