...
contract GOVToken is ERC20Upgradeable, OwnableUpgradeable {
    ...
    // Non linear unlocks [i,j] i% per day for j days
    uint256[][] public publicNonLinearUnlock;
    ...
    function initialize() initializer public  {
        ...
        publicNonLinearUnlock = [
            [uint256(50000000000000000000), 1], //50% during the first day
            [uint256(0), 12], // 12 day lock after first day makes it 13 eaya
            [uint256(25000000000000000000), 1], // 25% on day 14
            [uint256(0), 13], //13 day lock after 14th day makes it 27 eaya
            [uint256(25000000000000000000), 1] // 25% at day 14
        ];
		// 0: Angel 28 days LOCK 7% i.e. 7,000,000 tokens :10 months @ rate of 10% per month 
        vestingTypes.push(VestingType(333333333333333300, 28 days, false));

        // 1: Seed 8.00%, 8,000,000, 14 Days LOCK, 1,200,000: 6.6 months @ rate of 15% per month 
        vestingTypes.push(VestingType(505050505050504960, 14 days, false));

        // 2: Strategic 10%, 10,000,000, 2 Days LOCK, 4 months @ rate of 25% per month 
        vestingTypes.push(VestingType(666666666666666800, 2 days, false));

        // 3: Pivate 10%, 10,000,000, 1 Days LOCK, 4 months @ rate of 25% per month 
        vestingTypes.push(VestingType(833333333333333400, 1 days, false));

        // 4: Public 3%, 3,000,000, 50% @ TGE, 25% @ Day 14, remaining 25% @ Day 28
        ...
        // 8: Farming and Staking 26%, 26,000,000, TBD Month LOCK, TBD Month vesting.
        vestingTypes.push(VestingType(uint256(100000000000000000000), 0, false));
        
        // 9: GOV Genius rewards 1.50%, 1,500,000 3 days LOCK, 24 months @ rate of 4.16% per month
        vestingTypes.push(VestingType(138888888888888900, 3 days, false));

        // 10: Marketting  10.00%, 10,000,000, 24 months @ rate of 4.16% per month 
        vestingTypes.push(VestingType(138888888888888900, 1 days, false));

        // 11: Ecosystem  10.00%, 10,000,000, 1 days LOCK, 36 months @ rate of 2.77% per month 
        vestingTypes.push(VestingType(92592592592592600, 14 days, false));

        //12: ETH liquidity 1.50% 
        _mint(address(msg.sender), 1500000e18);

        //13: BSC Liquidity 1.50%
        _mint(address(msg.sender), 1500000e18);
    }
    ...
    function calculatePublicNonLinear(uint256 _days, uint256 amount)
        public
        view
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < publicNonLinearUnlock.length; i++) {
            if (_days <= _days_remainder) break;

            if (_days.sub(_days_remainder) >= publicNonLinearUnlock[i][1]) {
                unlocked = unlocked.add(
                    mulDiv(amount, publicNonLinearUnlock[i][0], ONE_HUNDRED_PERCENT)
                        .mul(publicNonLinearUnlock[i][1])
                );
            }
            ...
            _days_remainder += publicNonLinearUnlock[i][1];
        }
        ...
        returns (uint256)
    {
        ...
        if (vestingWallets[sender].nonLinear == true) {
            dailyTransferableAmount = calculatePublicNonLinear(
                trueDays,
