...
contract CrowdsaleNativeToken is Ownable, ReentrancyGuard, Pausable, Events {
  ...
  // For payment in native token (wei) it's always 18
  ...
  function buy() public payable onlySalePhase whenNotPaused {
    require(msg.value != 0, 'CS: transaction value is zero');

    uint256 ethPrice = config.priceFeed.currentPrice(config.priceRestrictions);

    require(
      _toUsd(msg.value, ethPrice, PAYMENT_DECIMALS) <= config.maxUsdValue,
  ...
  function claim(uint256 value) public nonReentrant onlyVestingPhase whenNotPaused {
    ...
    uint256 maxTokensToUnlock = _calculateMaxTokensToUnlock(balance[msg.sender], percentage);

    require(value <= maxTokensToUnlock, 'CS: value exceeds unlocked percentage');
  ...
  function _calculateTokenAmount(
    ...
    uint256 result = (price * value * uint256(10)**tokenDecimals) /
      rate /
      uint256(10)**paymentDecimals;

    require(result > 0, 'CS: token amount is zero');
  ...
  function _calculateMaxTokensToUnlock(uint256 _balance, uint256 _percentage)
    internal
    pure
    returns (uint256)
  {
