...
// contract class
contract BabyPixelDoods is ERC721, Ownable {
    // utilities
    ...
    function mint(uint256 numberOfTokens) public payable {
        ...
        if (msg.sender != owner()) {
            if (totalSupply() + (numberOfTokens) > 15 && totalSupply() < 15) {
                require(
                    nftPrice * (totalSupply() + numberOfTokens - 15) <=
                        msg.value,
                    "The contract did not receive enough Ethereum"
                );
            } else if (
                totalSupply() + (numberOfTokens) > 15 && totalSupply() > 15
            ) {
                require(
