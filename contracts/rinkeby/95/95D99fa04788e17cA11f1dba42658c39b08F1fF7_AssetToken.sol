...
contract AssetToken is ERC20, CompoundRateKeeper {
    ...
    constructor(
        ...
        // require(bytes(_name).length > 3 && bytes(_symbol).length > 3,"Invalid name or symbol provided");
        require(_issuer != address(0), "Invalid Issuer address provided");
        require(_guardian != address(0), "Invalid Guardian address provided");
        require(_statePercent > 0, "Invalid MIN State Percent provided");
        require(_statePercent <= hundredPercent, "Invalid MAX State Percent provided ?");
    ...
    function _beforeTokenTransfer(
        ...
        if (state == ContractState.safeguard) {
            if (_to != address(this) && _from != guardian) {
    ...
    function setKya(string memory _kya) external onlyIssuer onlyUnfreezed onlyActiveState {
        require(bytes(_kya).length > 3, "SKY: invalid KYA provided");
    ...
    function approveMint(uint256 _mintRequestID, string memory _referenceTo) external onlyIssuer onlyActiveState {
    ...
    function cancelRedemptionRequest(uint256 _redemptionRequestID, string memory _motive) external onlyActiveState {
        ...
        require(!redemptionRequests[_redemptionRequestID].fromStake, "CRR: staked request, use unstake to redeem");

        if (_msgSender() != issuer) {
            require(mustBeAuthorizedHolder(_msgSender()), "CRR: _account not authorized");
            require(
                redemptionRequests[_redemptionRequestID].sender == _msgSender(),
        ...
        // delete stakedRedemptionRequests[_msgSender()];
        ERC20._transfer(address(this), redemptionRequests[_redemptionRequestID].sender, refundAmount);
    }
    ...
    function safeguardStake(uint256 _amount, string memory _recipient) external onlyActiveState {
        ...
        require(ERC20.balanceOf(_msgSender()) >= _amount, "SFS: insufficient funds from caller");
...
// ==================================================================================
// ==================================================================================
// do a new contract on swarm with the isAuthorized function
// user is tier 2 and not suspended (swarm)
// uint256 public constant TIER_2_ID = 2; ==>> tier2
//
// uint256 public constant SUSPENDED_ID = 0; ==> suspended
...
abstract contract AccessManager is AccessControl {
    ...
    modifier onlyIssuerOrAgentOrAuthorizedHolder() {
        ...
        if (state == ContractState.safeguard) {
            require(mustBeAuthorizedHolder(_msgSender()), "Safeguard State: account not authorized");
        }
        _;
    }
    ...
    function mustBeAuthorizedHolder(address _account) public view returns (bool) {
        require(_account != address(0), "MBA: address 0x0 provided");
