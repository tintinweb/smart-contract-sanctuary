...
contract MyContract {
    ...
    function addRow(uint256 _orderId, 
        ...
        if (_prevOrderId != 0 ) {
            ...
            address[] memory _authorisedIdsTmp = new address[](table[_prevOrderId].authorisedIds.length+2);
            for (uint256 i=0; i < table[_prevOrderId].authorisedIds.length; i++) {
                _authorisedIdsTmp[i] = table[_prevOrderId].authorisedIds[i];
            }
            _authorisedIdsTmp[_authorisedIdsTmp.length-1] = _senderId;
            _authorisedIdsTmp[_authorisedIdsTmp.length-2] = _receiverId;

            uint256[] memory _prevOrderIdsTmp = new uint256[](table[_prevOrderId].prevOrderIds.length+1);
            for (uint256 i=0; i < table[_prevOrderId].prevOrderIds.length; i++) {
                _prevOrderIdsTmp[i] = table[_prevOrderId].prevOrderIds[i];
            }
            _prevOrderIdsTmp[_prevOrderIdsTmp.length-1] = _prevOrderId;

            _authorisedIds = _authorisedIdsTmp;
            _prevOrderIds = _prevOrderIdsTmp;

        } else {
            address[] memory _authorisedIdsTmp;
            //_authorisedIdsTmp = [_senderId, _receiverId];
            _authorisedIdsTmp[0] = _senderId;
            _authorisedIdsTmp[1] = _receiverId;

            uint256[] memory _prevOrderIdsTmp;
            //_prevOrderIdsTmp = [_prevOrderId];
            _prevOrderIdsTmp[0] = _prevOrderId;
