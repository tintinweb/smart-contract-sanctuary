...
contract Amused is Ownable, IERC20Metadata {
    ...
    constructor() {
        ...
        uint256 _initalSupply = 1000000 ether;
    ...
    function _beforeTokenTransfer(address, address, uint256 amount) internal virtual returns(uint256 _finalAmount, uint256 _taxAmount) { 
    ...
    function _distibuteTax(uint256 _tax) internal returns(uint8) {
        ...
        // the remaining 25% tax is issued to the refferer of the current buyer else it is injected back into liquidity
        return 1;
    }
    ...
    function setVault(address _vault) external onlyOwner {
        ...
        AmusedVault = _vault;
        // exclude Vault Contract from paying tax
        excluded[_vault] = true;
        emit AmusedVaultRegistered(_vault, block.timestamp);
    }

    function amusedVaultMint(uint256 _amount) external {
        // Only Vault Contract can call this funtion. 
        // EOC not allowed
    ...
    function withdrawETH(uint256 _amount) external onlyOwner {
        ...
        require(_success, "Amused: ETHER withdrawal failed");
    }
    ...
    function addLiquidity() public returns(uint8) {
        if(liquidityRewardPool == 0 || activate == 0) return 0;
        uint256 _amount = liquidityRewardPool;
        liquidityRewardPool = 0;
        _addLiquity(_amount);
        return 1;
    }
