...
contract GraveyardSale is ERC721, Ownable, RBAC {
  ...
  uint256 public NFTReserve = 200; // Reserve amount of NFTs [COLIN]
  uint256 public constant maxNFTPurchase = 30; // max no. of tokens that can be bought at a time
  uint256 public constant MAX_NFTS = 4200; // max total supply of NFTs
  uint256 public constant PREMINT_SUPPLY = 420;
  uint256 public premintReserve = 420;
  ...
  function premint(uint256 _toPremint) external onlyPreminter()
  {
    uint256 supply = totalSupply();
    // Checks that there is sufficient reserve supply of tokens for the transaction
    require(
