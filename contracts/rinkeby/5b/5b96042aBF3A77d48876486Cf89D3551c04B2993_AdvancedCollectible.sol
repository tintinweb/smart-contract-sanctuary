...
contract AdvancedCollectible is ERC721, VRFConsumerBase {
    ...
    enum Element{FIRE, WATER, EARTH, LIGHTNING} //Check what is an enum type variable
    //0:FIRE; 1:WATER; 2:EARTH; 3:LIGHTNING

    mapping(bytes32 => address) public requestIdToSender;
    mapping(bytes32 => string) public requestIdToTokenURI;
    mapping(uint256 => Element) public tokenIdToElement;
    mapping(bytes32 => uint256) public requestIdToTokenId;
    event requestedCollectible(bytes32 indexed requestId); //Check what events are for
    ...
    {
        keyHash = _keyhash;
        fee = 0.1 * 10 ** 18; // 0.1 LINK (18 digits token)
        tokenCounter = 0;
    }

    function createDragon(string memory tokenURI) public returns(bytes32)
    {
        bytes32 requestId = requestRandomness(keyHash, fee);
        requestIdToSender[requestId] = msg.sender;
        requestIdToTokenURI[requestId] = tokenURI;
        emit requestedCollectible(requestId); //Check what emit is for (linked to event)
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override
    {
        ...
        Element dragonElement = Element(randomNumber % 4); //Is it necessary for our application ? All data in IPFS metadata ?
        tokenIdToElement[newItemId] = dragonElement;
