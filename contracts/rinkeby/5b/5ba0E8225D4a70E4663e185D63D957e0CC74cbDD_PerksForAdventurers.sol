...
contract PerksForAdventurers is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    address proxyRegistryAddress = 0xa5409ec958C83C3f309868babACA7c86DCB077c1; //testnet
    ...
    function pluck(uint256 tokenId, string memory keyPrefix, string[] memory sourceArray) internal view returns (string memory) {
        ...
        if (keccak256(abi.encodePacked(keyPrefix)) == keccak256(abi.encodePacked("TRAIT")) ){
            ...
            // output = string(abi.encodePacked(perkPrefixes[rand % perkPrefixes.length], " ", output));
            if (0 < rarity && rarity <= 60 ){
                perkRarity = perkPrefixes[0];
            } else if (60 < rarity && rarity <= 90 ){
                perkRarity = perkPrefixes[1];
            } else if (90 < rarity && rarity <= 98 ){
                perkRarity = perkPrefixes[2];
            } else {
                perkRarity = perkPrefixes[3];
            }
