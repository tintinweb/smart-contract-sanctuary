...
contract Reward721 is OwnableContract,ReentrancyGuard,ERC721Enumerable {
    string private _dclURI;
    
    constructor()ERC721("Reward721","Reward721"){
        initOwnableContract();
    }

    function mint(address to, uint256 tokenId) public nonReentrant onlyAdmin{
        _mint(to, tokenId);
    }

    function batchMint(address[] calldata to, uint256[] calldata tokenId) public onlyAdmin{
    ...
    function mintFromTo(address[] calldata to, uint256 startId,uint256 endId) public onlyAdmin{
        require(endId > startId,"invalid endId");
        require(to.length == endId - startId + 1,"invalid length");
        for (uint256 index = 0; index < to.length; index++) {
            mint(to[index], startId + index);
        }
    }
