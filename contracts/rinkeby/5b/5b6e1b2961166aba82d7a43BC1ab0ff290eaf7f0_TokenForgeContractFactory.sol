...
import "./TokenForgeERC20Token.sol";
import "@openzeppelin/contracts/utils/Context.sol";

contract TokenForgeContractFactory is Context {
    ...
    function create(
        ...
        TokenForgeERC20Token token = new TokenForgeERC20Token(name, symbol, isin, issuerName);
...
contract TokenForgeERC20Token is Context, AccessControlEnumerable, ERC20Burnable, WhiteList {
    ...
    function setISIN(string memory val_) external onlyRole(GOVERNANCE_ROLE) {
        if (keccak256(bytes(_ISIN)) == keccak256(bytes(val_))) {
            revert("TokenForgeERC20Token: ISIN would not change");
        }

        string memory oldValue = _ISIN;
        _ISIN = val_;
        emit PropertyChanged(PROP_ISIN, bytes(oldValue), bytes(_ISIN));
    }

    function issuerName() public view returns (string memory) {
        return _issuerName;
    }

    function setIssuerName(string memory val_) external onlyRole(GOVERNANCE_ROLE) {
        if (keccak256(bytes(_issuerName)) == keccak256(bytes(val_))) {
            revert("TokenForgeERC20Token: IssuerName would not change");
        }

        string memory oldValue = _issuerName;
        _issuerName = val_;
        emit PropertyChanged(PROP_ISSUER_NAME, bytes(oldValue), bytes(_issuerName));
    }
    ...
    function mint(address to, uint256 amount) public virtual {
        require(hasRole(MINTER_ROLE, _msgSender()), "TokenForgeERC20Token: must have minter role to mint");
        _mint(to, amount);
    }
