...
contract UpbondNFTMintable is Initializable, ERC1155Upgradeable, OwnableUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable {
   ...
   // constructor() initializer {}
    ...
    function setURI(string memory newuri) public onlyOwner {
        baseURI = newuri; //_setURI(newuri);
    }

    function mint(address account, uint256 id, uint256 amount, bytes memory data, string memory hash)
        public
        onlyOwner
    {
        ID.increment();
    ...
    function mintBatch(address to, uint256[] calldata ids, uint256[] calldata amounts, bytes[] calldata data, string[] memory hash)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < ids.length; i++) {
            mint(to, ids[i], amounts[i], data[i], hash[i]);
        }
    }

    function nextId() public view returns (uint256 id) {
        return ID.current();
    }

    function uri(uint256 tokenId) public view override returns (string memory) {
        string memory tokenUrlId;
        tokenUrlId = tokenUrl[tokenId];
        return string(abi.encodePacked(baseURI, tokenUrlId));
	}
...
import "./UpbondNFTMintable.sol";

contract UpbondNFTMintableCreator {
    using CountersUpgradeable for CountersUpgradeable.Counter;

    /// Counter for current contract id upgraded
    CountersUpgradeable.Counter private atContract;

    /// Address for implementation of UpbondNFTMintable to clone
    address public implementation;

    /// Initializes factory with address of implementation logic
    /// @param _implementation UpbondNFTMintable logic implementation contract to clone
    ...
    /// @param _url Metadata: The URI may point to a JSON file that conforms to the "ERC721
    function createAssets(
        ...
        UpbondNFTMintable(newContract)._initialize(
            msg.sender,
            _name,
            _symbol,
            _description,
        ...
        emit CreatedAssets(newId, msg.sender, newContract);
    ...
    /// @return UpbondNFTMintable Edition NFT contract
