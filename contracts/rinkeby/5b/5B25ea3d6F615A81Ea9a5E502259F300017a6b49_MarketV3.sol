...
contract MarketV3 is Manager, ERC1155Holder, ERC721Holder, ReentrancyGuard {
	...
	function _match(
		...
		uint256 loyaltyFee = ISOTANFT(order.tokenAddress).getLoyaltyFee(order.tokenId);
...
//		}
		...
		_match(bid.bidder, bid.paymentToken, _orderId, _quantity, bid.bidPrice,
	...
	function cancelBid(uint256 _bidId) external whenNotPaused() nonReentrant() {
		...
		if (bid.paymentToken == address(0)) {
			...
			IERC20(bid.paymentToken).safeTransferFrom(bid.bidder, address(this), bid.quantity.mul(bid.bidPrice));
		}
	...
	function adminMigrateData(address oldMarket) external onlyOwner() {
		totalOrders = ISotaMarket(oldMarket).totalOrders();
		totalBids = ISotaMarket(oldMarket).totalBids();
