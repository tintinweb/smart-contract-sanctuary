...
contract ERC20Staking is StakingBase {
    ...
    function _beforeUserStake(
        ...
        UserInfoLib.UserInfo storage
    ) internal view override {
        require(
            IERC20(pool.token).allowance(account, address(this)) >= valuedAmountOrId,
            "ALLOWANCE_LT_VALUED_AMOUNT"
        );
    }
    ...
    function _safePoolTokenTransferFrom(
        ...
        IERC20(poolToken).safeTransferFrom(from, to, amount);
    }
    ...
    function _getPoolTokenBalance(
        ...
        return IERC20(poolToken).balanceOf(account);
    }
    ...
    function _emergencyUnstakeAll(
        ...
        require(amount > 0, "USER_HASNT_STAKED_TOKENS");
        user.emergencyUnstakeAll();
        _safePoolTokenTransfer(pool.token, address(this), userAccount, pid, amount);
    ...
    function _sweep(
        ...
        uint256 amountToSweep = amount > 0 ? amount : IERC20(token).balanceOf(address(this));
        IERC20(token).safeTransfer(to, amountToSweep);
