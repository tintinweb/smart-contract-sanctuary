...
contract MSActivityCenter is ProxyAdmin {
        ...
        onlyWithdrawer
    {
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            IERC20Upgradeable(tokens[i]).safeTransfer(to, balance);
    ...
    function sale(uint256 activityId, uint256 tokenQuantity)
        public
        payable
        whenNotPaused
        checkSale(activityId, tokenQuantity)
    {
        chargeTokens(activityId, tokenQuantity);
        _mint(activityId, _msgSender(), tokenQuantity, true);
    }
    ...
    function privateSale(
        ...
        checkSale(activityId, tokenQuantity)
    {
        _mint(activityId, to, tokenQuantity, false);
    }
    ...
    function _mint(
        ...
        for (uint256 i = 0; i < tokenQuantity; i++) {
            uint256 tokenId = MSNFT(activities[activityId].nftContract).mint(
                to,
                0,
                0,
                0,
            ...
            emit Sale(
                activityId,
                to,
                activities[activityId].nftContract,
                tokenId,
                block.timestamp
            );
        }
    ...
    function chargeTokens(uint256 activityId, uint256 tokenQuantity) internal {
        for (uint256 i = 0; i < activityCharges[activityId].length; i++) {
            if (activityCharges[activityId][i].token == address(0)) {
                ...
                require(
                    IERC20Upgradeable(activityCharges[activityId][i].token).allowance(
                        _msgSender(),
                        address(this)
                    ) >= activityCharges[activityId][i].price * tokenQuantity,
                    "MSActivityCenter: Approved insufficient ERC20 tokens"
                );
                IERC20Upgradeable(activityCharges[activityId][i].token).safeTransferFrom(
                    _msgSender(),
                    address(this),
                    activityCharges[activityId][i].price * tokenQuantity
                );
    ...
    function version() pure public returns (string memory) { return "v1!"; }
}
...
contract ProxyAdmin is Initializable, ContextUpgradeable, PausableUpgradeable, AccessControlEnumerableUpgradeable, UUPSUpgradeable, OwnableUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    mapping(string => bytes32) public roleName;

    function initialize(address pauser, address creator, address withdrawer) public initializer {
    ...
    function adminHello() pure public returns (string memory) { return "Hi from Admin!"; }
