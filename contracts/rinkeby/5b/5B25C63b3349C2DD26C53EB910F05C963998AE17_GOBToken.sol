...
contract GOBToken is ERC1155, Ownable, ERC1155Burnable, Pausable {
    ...
    mapping(address => bool) claimedMPs;
    event Claimed(address indexed account);
    bool public paidMintIsOpen = false;
    uint256 _mintPrice;
    ...
    function claim(uint256 mpIndex, bytes32[] memory proof) public whenNotPaused {
        require(claimedMPs[msg.sender] == false, "Already claimed");
        require(proof.verify(merkleRoot, keccak256(abi.encodePacked(msg.sender))), "You are not in the list");
        claimedMPs[msg.sender] = true;
        _mint(msg.sender, mpIndex, 1, "");
    ...
    function mint(uint index) public payable
    {
        require(msg.value >= _mintPrice, "Claim: Ether value incorrect");
        require(paidMintIsOpen == true, "Public mint is closed");
        _mint(msg.sender, index, 1, "");
    }

    function updatePaidMint(bool isOpen) public {
        paidMintIsOpen = isOpen;
    }
