...
contract BowlBrains is ERC721A, Ownable {
    ...
    constructor() ERC721A("BowlBrains", "BBrain", MAX_MINTS, MAX_SUPPLY) {}

    function mint(uint256 numTokens) public payable {
        require(DROP_ACTIVE, "The sale has not started yet");
        require(numTokens > 0 && numTokens <= MAX_MINTS, "You must mint at least 1 and at most 10 BowlBrains");
        require(totalSupply() + numTokens <= MAX_SUPPLY, "No BowlBrains left :(");
        require(msg.value >= NFT_PRICE * numTokens, "Amount of ether sent is not enough");

        _safeMint(msg.sender, numTokens);
    }

    function mintWhitelist(uint256 numTokens) public payable {
        require(DROP_ACTIVE, "The sale has not started yet");
        require(numTokens > 0 && numTokens <= MAX_MINTS, "You must mint at least 1 and at most 10 BowlBrains");
        require(totalSupply() + numTokens <= MAX_SUPPLY, "No BowlBrains left :(");
        require(msg.value >= NFT_PRICE * numTokens, "Amount of ether sent is not enough");
