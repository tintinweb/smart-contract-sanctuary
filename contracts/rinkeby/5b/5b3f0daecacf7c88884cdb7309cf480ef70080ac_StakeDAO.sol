...
////// /nix/store/8xb41r4qd0cjb63wcrxf1qmfg88p0961-dss-6fd7de0/src/StakeDAO.sol
...
contract StakeDAO is LibNote {
  ...
  event Transfer(address indexed src, address indexed dst, uint wad);
    ...
    modifier auth {
        require(wards[msg.sender] == 1, "StakeDAO/not-authorized");
        _;
    }

    // --- ERC20 Data ---
    string  public constant name     = "Mock StakeDAO";
    ...
    function transferFrom(address src, address dst, uint wad) public returns (bool) {
      ...
      require(balanceOf[src] >= wad, "StakeDAO/insufficient-balance");
        if (src != msg.sender && _allowance( src, msg.sender ) != uint(-1)) {
            require(_allowance( src, msg.sender ) >= wad, "StakeDAO/insufficient-allowance");
    ...
    function burn(address usr, uint wad) external {
        require(balanceOf[usr] >= wad, "StakeDAO/insufficient-balance");
        if (usr != msg.sender && _allowance( usr, msg.sender ) != uint(-1)) {
            require(_allowance( usr, msg.sender ) >= wad, "StakeDAO/insufficient-allowance");
                    ...
                    bool allowed, uint8 v, bytes32 r, bytes32 s) external
    {
        ...
        require(holder != address(0), "StakeDAO/invalid-address-0");
        require(holder == ecrecover(digest, v, r, s), "StakeDAO/invalid-permit");
        require(expiry == 0 || block.timestamp <= expiry, "StakeDAO/permit-expired");
        require(nonce == nonces[holder]++, "StakeDAO/invalid-nonce");
