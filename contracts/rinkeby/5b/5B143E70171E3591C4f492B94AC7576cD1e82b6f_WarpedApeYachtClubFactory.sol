...
contract WarpedApeYachtClubFactory is FactoryERC1155, AuthorizedList, ReentrancyGuard {
    ...
    constructor(address _proxyRegistryAddress, address _nftAddress) {
        ...
        totalNFTs++; // TODO - Remove after testing
    }
    ...
    function name() override external pure returns (string memory) {
        return "Warped Ape Yacht Club Sale";
    }
    ...
    function uri(uint256 _optionId) override external pure returns (string memory) {
        if (_optionId == 0){
            return "https://bafkreieyrheyns4jqmtzb4gtkyb7dfxckwti6vdzgidvada4cqeenujiiq.ipfs.dweb.link/";
        }
...
//        );
    ...
    function _createOrMint(
        ...
        if (!NFT.exists(_id)) {
            NFT.create(_to, _id, _amount, SUPPLY_PER_NFT, idMetadataLocation[_id], _data);
            mintedNFTs++;
    ...
    function _isOwnerOrProxy(address _address) internal view returns (bool) {
        ...
        return authorizedCaller[_address] || address(proxyRegistry.proxies(owner())) == _address;
    }
...
contract WarpedApeYachtClub is ContextMixin, ERC1155Supply, NativeMetaTransaction, AuthorizedList, ProxyRegistry {
    using Strings for string;
    using SafeMath for uint256;
    ...
    constructor(string memory _name, string memory _symbol, string memory _baseNftUri, string memory _contractUri, address _proxyRegistryAddress) ERC1155(_contractUri){
        name = _name;
        symbol = _symbol;
    ...
    * @param _maxSupply cap on supply limit for the token
    ...
    */
    function create(
        ...
        string memory _uri,
        bytes memory _data
    ) public authorized returns (uint256) {
        require(!exists(_id), "token _id already exists");
        require(_initialSupply <= _maxSupply, "Mint quantity exceeds limit");

        if (bytes(_uri).length > 0) {
            string memory createdURI = string(abi.encodePacked(baseNftUri, _uri));
...
//        tokenSupply[_id] = tokenSupply[_id].add(_quantity);
    ...
    */
    ...
    function _safeTransferFrom(
        ...
        uint256 fromBalance = apeBalances[id][from];
        require(fromBalance >= amount, "ERC1155: insufficient balance for transfer");
        unchecked {
            apeBalances[id][from] = fromBalance - amount;
...
//            tokenSupply[_id] = tokenSupply[_id].add(quantity);
            apeBalances[_id][to] += quantity;
            apeHoldings[to] += quantity;

            if(tokenSupply(_id) > maxSupply[_id]){
