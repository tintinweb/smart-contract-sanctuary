...
contract TheDudesStaking is IERC721Receiver, Ownable {
  ...
  function unstake(uint256 tokenId) public {
    ...
    require(tokenIDs[tokenId] == msg.sender, "Owner of the token should unstake.");

    totalSupply--;
    tokenIDs[tokenId] = address(0);
    timelocks[tokenId] = 0;
    balances[msg.sender] -= 1;
  ...
  function withdrawDividends(uint256[] calldata tokenIds) public {
  ...
  function withdrawDividend(uint256 tokenId) public {
    ...
    address _user = tokenIDs[tokenId];
  ...
  function accumulativeDividendOf(uint256 tokenId) public view returns(uint256) {
    return magnifiedDividendPerShare.mul(balanceOf(tokenId)).toInt256Safe()
  ...
  function tokenStaked(uint256 tokenId) public view returns(bool) {
    return tokenIDs[tokenId] != address(0);
  }
