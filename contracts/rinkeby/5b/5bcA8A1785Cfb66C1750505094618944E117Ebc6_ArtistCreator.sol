...
contract ArtistCreator {
    ...
    /// Get artist given the created ID
    /// @param _artistId id of artist to get contract for
    /// @return Artist Edition NFT contract
    function getArtistAtId(string calldata _artistId) external view returns (address) {
...
contract Artist is ERC721Upgradeable, IERC2981Upgradeable, OwnableUpgradeable {
    ...
    // `tokenId` increments with each token purchased, globally across all editions.
    uint256 private tokenId;
    // Editions start at 1, in order that unsold tokens don't map to the first edition.
    uint256 private editionId = 1;
    ...
    function createEdition(
        ...
        emit EditionCreated(_quantity, _price, _fundingRecipient, editionId, _royaltyBPS);
    ...
    function buyEdition(uint256 _editionId) external payable {
        ...
        tokenToEdition[tokenId] = _editionId;
    ...
    function withdrawFunds(uint256 _editionId) external {
        ...
        uint256 remainingForEdition = (editions[_editionId].price * editions[_editionId].numSold) - // Compute total amount of revenue that has been generated for the edition so far.
        ...
        withdrawnForEdition[_editionId] += remainingForEdition;
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        uint256 _editionId = tokenToEdition[_tokenId];
        // If the token does not map to an edition, it'll be 0.
        require(_editionId > 0, 'Token has not been sold yet');

        // Concatenate the baseURI, editionId and tokenId, to create URI.
        return string(abi.encodePacked(baseURI, _editionId.toString(), '/', _tokenId.toString()));
    }
