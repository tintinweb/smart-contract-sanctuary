...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function transferFrom(address from, address to, uint256 tokenId) public virtual override {
        ...
        _transfer(from, to, tokenId, tokenId);
    }

    function transferManyFrom(address from, address to, uint256 fromTokenId, uint256 toTokenId) public virtual override {
        //solhint-disable-next-line max-line-length
        for(uint256 i = fromTokenId; i <= toTokenId; i ++)
        {
            require(_isApprovedOrOwner(_msgSender(), i), "ERC721: transfer caller is not owner nor approved");
        }

        _transfer(from, to, fromTokenId, toTokenId);
    }
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId) public virtual override {
        safeTransferFrom(from, to, tokenId, tokenId, "");
    }

    function safeTransferManyFrom(address from, address to, uint256 fromTokenId, uint256 toTokenId) public virtual override {
        safeTransferFrom(from, to, fromTokenId, toTokenId, "");
    }
    /**
     * @dev See {IERC721-safeTransferFrom}.
     */
    function safeTransferFrom(address from, address to, uint256 fromTokenId, uint256 toTokenId, bytes memory _data) public virtual override {
        for(uint256 i = fromTokenId; i <= toTokenId; i ++)
        {
            require(_isApprovedOrOwner(_msgSender(), i), "ERC721: transfer caller is not owner nor approved");
        }
     ...
     */
    function _safeTransfer(address from, address to, uint256 fromTokenId, uint256 toTokenId, bytes memory _data) internal virtual {
        _transfer(from, to, fromTokenId, toTokenId);
        for(uint256 i = fromTokenId; i <= toTokenId; i ++)
        {
            require(_checkOnERC721Received(from, to, i, _data), "ERC721: transfer to non ERC721Receiver implementer");
        }
    }
     ...
     * Emits a {Transfer} event.
     */
    function _safeMintMany(address to, uint256 fromTokenId, uint256 toTokenId) internal virtual {
        _safeMint(to, fromTokenId, toTokenId, "");
    }
     ...
     */
    function _safeMint(address to, uint256 fromTokenId, uint256 toTokenId, bytes memory _data) internal virtual {
        _mint(to, fromTokenId, toTokenId);
        for(uint256 i = fromTokenId; i <= toTokenId; i ++)
        {
     ...
     * Emits a {Transfer} event.
     */
    function _mint(address to, uint256 fromTokenId, uint256 toTokenId) internal virtual {
     ...
     * Emits a {Transfer} event.
     */
    function _burn(uint256 fromTokenId, uint256 toTokenId) internal virtual {
        address owner = ERC721.ownerOf(fromTokenId);

        for(uint256 i = fromTokenId; i <= toTokenId; i ++)
        {
            _beforeTokenTransfer(owner, address(0), i);

            // Clear approvals
            _approve(address(0), i);
        ...
        emit ConsecutiveTransfer(fromTokenId, toTokenId, owner, address(0));
    }
     ...
     * Emits a {Transfer} event.
     */
    function _transfer(address from, address to, uint256 fromTokenId, uint256 toTokenId) internal virtual {
        for(uint256 i = fromTokenId; i <= toTokenId; i ++)
        {
            require(ERC721.ownerOf(i) == from, "ERC721: transfer of token that is not own");
            require(to != address(0), "ERC721: transfer to the zero address");

            _beforeTokenTransfer(from, to, i);
        ...
        emit ConsecutiveTransfer(fromTokenId, toTokenId, from, to);
    }
...
interface IERC721 is IERC165 {
    ...
    function safeTransferManyFrom(address from, address to, uint256 fromTokenId, uint256 toTokenId) external;
    /**
    ...
    function transferManyFrom(address from, address to, uint256 fromTokenId, uint256 toTokenId) external;
    ...
    /**
      ...
      * Emits a {Transfer} event.
      */
    function safeTransferFrom(address from, address to, uint256 fromTokenId, uint256 toTokenId, bytes calldata data) external;
}
