...
contract OrcBazaarZug{
    ...
    ERC20 public zug = ERC20(0xEB468b1c73E8D0159C163E9F0D95ABBD0B20b616);
    ...
    function closeOfferingWithEth(bytes32 _offeringId) external payable {
    ...
    function commission(uint _amount) internal returns(uint) {
        uint comm = convertValue(_amount);
        zug.transfer(0xCcB6D1e4ACec2373077Cb4A6151b1506F873a1a5,comm);
        uint _sellerPayable = _amount.sub(comm);
        return _sellerPayable;
    }

    function convertValue(uint _amount) internal pure returns(uint) {
        return (_amount.div(1000).mul(25));
    }
