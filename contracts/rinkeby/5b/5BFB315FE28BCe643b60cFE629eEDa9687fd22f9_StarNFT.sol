...
interface IStarNFT is IERC1155 {
    /* ============ Events =============== */
    event QuasarAssetUpdated(uint256 indexed id, uint256 indexed oldAmount, uint256 indexed newAmount);
    ...
    function starInfo(uint256) external view returns (uint256 powah, uint256 mintBlock);
    function quasarInfo(uint256) external view returns (uint256 mintBlock, IERC20 erc20Addr, uint256 amount, uint256 campaignID);
    function superInfo(uint256) external view returns (uint256 _mintBlock, IERC20[] memory _stakeToken, uint256[] memory _amount, uint256 _cid);
    ...
    // asset-backing mint
    function mintQuasar(address account, uint256 powah, uint256 cid, IERC20 erc20Addr, uint256 amount) external returns (uint256);
    function burnQuasar(address account, uint256 id) external;

    // asset-backing forge
    function mintSuper(address account, uint256 powah, uint256 campaignID, IERC20[] calldata erc20s, uint256[] calldata amounts) external returns (uint256);
    function burnSuper(address account, uint256 id) external;
    // update
//    function updateQuasarBackedAsset(address owner, uint256 id, uint256 amount) external;
 ...
 * TODO: remove Context and Initializable to see if deployment gas reduces.
 */
contract StarNFT is ERC165, IERC1155, IERC1155MetadataURI, IStarNFT {
    ...
    modifier onlyGalaxyCommunity() {
        require(msg.sender == _galaxyCommunityAddress, "not the galaxy community");
        _;
    }
    /**
     * minter
     */
    modifier onlyMinter() {
        require(_minters[msg.sender], "not the minter");
        _;
    }
    /**
     * operator
    ...
    modifier onlyOperator() {
        require(_operators[msg.sender], "not the operator");
        _;
    }
    /* ============ Enums ================ */
    /* ============ Structs ============ */
    ...
    struct Super {
        // assetToken => amount
        //        mapping(IERC20 => uint256) backingAssets;
    ...
    // Used as the URI for all token types by relying on ID substitution, e.g. https://galaxy.eco/{address}/{id}.json
    ...
    function mintBatch(address account, uint256 amount, uint256[] calldata powahArr) external onlyMinter override returns (uint256[] memory) {
        ...
        require(powahArr.length == amount, "invalid powah array length");
    ...
    function mintQuasar(address account, uint256 powah, uint256 campaignID, IERC20 erc20Addr, uint256 erc20Amount) external onlyMinter override returns (uint256) {
        return _mintQuasar(account, powah, campaignID, erc20Addr, erc20Amount);
    }
    ...
    function mintSuper(address account, uint256 powah, uint256 campaignID, IERC20[] calldata erc20s, uint256[] calldata amounts) external onlyMinter override returns (uint256) {
        require(account != address(0), "Must not mint to null address");
        return _mintSuper(account, powah, campaignID, erc20s, amounts);
    }
    ...
    //    /**
    //      * update nft assert amount
    //      */
    //    function updateQuasarBackedAsset(address owner, uint256 id, uint256 amount) external onlyOperator override {
    //        require(isOwnerOf(owner, id), "not the owner");
    //        require(address(_quasars[id].erc20Addr) != address(0), "no quasar info");
    //
    //        emit QuasarAssetUpdated(id, _quasars[id].amount, amount);
    //        _doSafeQuasarUpdatedAcceptanceCheck(owner, id, _quasars[id].amount, amount);
    //
    //        _quasars[id].amount = amount;
    //    }
    ...
    function updatePowah(address owner, uint256 id, uint256 powah) external onlyOperator override {
        ...
        emit PowahUpdated(id, _infos[id].powah, powah);
        _doSafePowahUpdatedAcceptanceCheck(owner, id, _infos[id].powah, powah);

        _infos[id].powah = uint128(powah);
    }
    ...
    function transferGalaxyCommunity(address newGalaxyCommunity) external onlyGalaxyCommunity {
        require(newGalaxyCommunity != address(0), "invalid zero address");
    ...
    function addMinter(address minter) external onlyOwner {
        ...
        require(!_minters[minter], "already added");
    ...
    function removeMinter(address minter) external onlyOwner {
        require(_minters[minter], "minter not exist");
    ...
    function addOperator(address operator) external onlyOwner {
        require(operator != address(0), "invalid zero address");
        require(!_operators[operator], "already added");
    ...
    function removeOperator(address operator) external onlyOwner {
        require(_operators[operator], "operator not exist");
        delete _operators[operator];
    }
     ...
     */
    function starOwner() external view returns (address) {
        return _owner;
    }
    ...
    /**
     * See total star nft count, include burned nft.
     */
     ...
     * See total star nft count, include burned nft.
     */
    function starNFTCount() public view returns (uint256) {
        return _starCount;
    }
    ...
    function uri(uint256 id) external view override returns (string memory) {
        require(id <= _starCount, "star nft not exist");
    ...
    /**
      * get star info
      */
    function starInfo(uint256 id) external view override returns (
        uint256 powah,
        uint256 mintBlock
    ) {
        powah = _infos[id].powah;
        mintBlock = _infos[id].mintBlock;
    }

    /**
     * get quasar info
     */
    function quasarInfo(uint256 id) external view override returns (
        uint256 mintBlock,
        IERC20 erc20Addr,
        uint256 amount,
        uint256 campaignID
        ...
        mintBlock = _infos[id].mintBlock;
        erc20Addr = _quasars[id].erc20Addr;
     ...
     * get super info
     */
    function superInfo(uint256 id) external view override returns (uint256 mintBlock, IERC20[] memory erc20Addr, uint256[] memory amount, uint256 campaignID){
        mintBlock = _infos[id].mintBlock;
        campaignID = _supers[id].campaignID;
        erc20Addr = _supers[id].backingTokens;
     ...
     * acceptance magic value.
     */
    function _mintQuasar(address account, uint256 powah, uint256 campaignID, IERC20 erc20Addr, uint256 amount) private returns (uint256) {
    ...
    function _mintSuper(address account, uint256 powah, uint256 campaignID, IERC20[] calldata erc20s, uint256[] calldata amounts) private returns (uint256){
        require(account != address(0), "Must not mint to null address");
        require(erc20s.length > 0, "Array(erc20s) must not be empty");
        // Don't use validate arrays because empty arrays are valid
        require(erc20s.length == amounts.length, "Array(erc20s, amounts) length mismatch");

        uint256 sID = _mint(account, powah);
        _supers[sID].campaignID = campaignID;
        _supers[sID].backingTokens = erc20s;
        _supers[sID].backingAmounts = amounts;
        //        for (uint256 i = 0; i < erc20s.length; ++i) {
        //            require(erc20s[i] != IERC20(address(0)), "Super asset token must not be null address");
        //            require(amounts[i] > 0, "Super asset amount must be greater than 0");
        //            _supers[sID].backingAssets[erc20s[i]] = amounts[i];
        //        }
    ...
    function _burnSuper(uint256 id) private {
        //        require(erc20s.length == _supers[id].backingTokens.length, "Array(erc20s) must not be empty");
        //        for (uint256 i = 0; i < erc20s.length; ++i) {
        //            require(erc20s[i] != address(0), "Super asset token must not be null address");
        //            delete _supers[id].backingAssets[IERC20(erc20s[i])];
        //        }
