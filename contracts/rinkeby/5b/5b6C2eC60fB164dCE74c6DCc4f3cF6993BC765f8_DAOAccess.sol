...
contract DAOAccess is IDAOAccess {
    ...
    function _existCheckpoint(
        ...
        if(numDDFCheckpoints < 1)
        {
            return false;
        }
        return DDFCheckpoints[numDDFCheckpoints - 1].fromBlock == block.number;
    }

    function _isDDFByBlock(address sender, uint256 blockNumber)
        internal
        view
        returns (bool)
    {
        require(
            !_existCheckpoint(),
            "DDF checkpoint already exists, please try again later."
        );
