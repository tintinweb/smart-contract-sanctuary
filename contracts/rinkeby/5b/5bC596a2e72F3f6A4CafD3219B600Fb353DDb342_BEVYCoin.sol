...
contract BUCKStablecoin is ERC20Custom {
    ...
    function refreshCollateralRatio() public {
        ...
        if(global_collateral_ratio < effectiveCollateralRatio()){
            ...
            uint256 recollat_possible = (global_collateral_ratio.mul(totalSupply()).sub(totalSupply().mul((effectiveCollateralRatio().sub(1))))).div(1e6);
            ...
            if(treasuryCollateralBalance > 0){
                if(treasuryCollateralBalance >= recollat_possible){
                    ...
                    treasury.withdraw(amount_to_recollat.div(IBUCKPool(buck_pools_array[0]).getMissingDecimals()));
                } else{
