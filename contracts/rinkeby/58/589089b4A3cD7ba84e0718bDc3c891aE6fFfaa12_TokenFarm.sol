...
contract TokenFarm is ChainlinkClient, Ownable {
    ...
    mapping(address => mapping(address => uint256)) public TokenAddressBalance;
    ...
    function inToken(address token) public returns (bool) {
    ...
    function stakeTokens(uint256 _amount, address token) public {
        ...
        TokenAddressBalance[token][msg.sender] += _amount;
    }

    // Unstaking Tokens (Withdraw)
    function unstakeTokens(address token, uint256 _amount) public {
        uint256 balance = TokenAddressBalance[token][msg.sender];
        ...
        returns (uint256, uint8)
    {
        ...
        return (
            uint256(price),
            AggregatorV3Interface(PriceFeed[token]).decimals()
        );
    }
    /* 
