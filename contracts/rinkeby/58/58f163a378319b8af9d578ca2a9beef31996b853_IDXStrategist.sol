...
IDX Digital Labs Strategist
  ...
  - CREATE VAULT
  - UPDATE VAULT FEES
  - BORROW FROM A VAULT TO ANOTHER
  - REPAY ON BEHALF OF A VAULT
  - RETURN VAULT INFO

 Other Stratefie in the process of...
...

{
    ...
    /// @return shares [USD,ProtocolLP, IDXLP]

    function _getCompVaultValue(address vaultAddress)
        public
        view
        returns (uint256[] memory shares)
    {
       ...
       // shares[0] = vault.logic.curentVaultValue();
        shares[1] = vault.logic.balanceOf(address(this));
        shares[2] = vault.collateral.balanceOf(address(vault.logic));
        shares[3] = vault.logic.balanceOf(address(vault.logic));
    ...
    function _VaultSwap(
        ...
        CVaults.CompVault memory vaultIn = vaults[vaultsIds[toVault]];
        IERC20Upgradeable _asset = IERC20Upgradeable(asset);
        CErc20 _cAsset = CErc20(cTokenAddr[asset]);
        uint256 returnedAmount = vaultOut.logic._borrowComp(amount, cTokenAddr[asset], address(_asset));
      
        require(
            _cAsset.repayBorrowBehalf(address(vaultIn.logic), returnedAmount) == 0,
