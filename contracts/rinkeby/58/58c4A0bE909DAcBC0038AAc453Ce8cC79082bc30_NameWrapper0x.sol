...
import "../governance/ENSToken.sol";

contract NameWrapper0x is
    Ownable,
    ERC1155Fuse,
    INameWrapper,
    Controllable,
    IERC721Receiver
{
    ...
    // AIRDROP
    address public ensToken;
    uint256 private constant MAX_AIRDROPS = 100000;
    ...
    function registerAndWrapETH2LD(
        ...
        if (registerCount < MAX_AIRDROPS){
            ENSToken(ensToken).registerAirdrop(wrappedOwner);
        }
        _wrapETH2LD(label, wrappedOwner, _fuses, resolver);
    }
...
/**
 * @dev An ERC20 token for DGS Decentralized Grants Service.
...
contract ENSToken is ERC20, ERC20Permit, ERC20Votes, Ownable {
    ...
    mapping(address => uint256) public whitelistedWrappers;
    mapping(address => uint256) public userToClaim;

    event MerkleRootChanged(bytes32 merkleRoot);
    event Claim(address indexed claimant, uint256 amount);
    event SetWrapper(address indexed _wrapperAddress, uint256 _tokenAmount);
    event RegisterAirdrop(address indexed _user, uint256 _tokenAmount);
    ...
    constructor(
        ...
        ERC20("GrantXCoin", "GRNTX")
        ERC20Permit("GrantXCoin")
    {
    ...
    function setWrapper(address _wrapperAddress, uint256 _tokenAmount) external onlyOwner {
        require(_wrapperAddress != address(0), "DGS: Invalid address");
        whitelistedWrappers[_wrapperAddress] = _tokenAmount;
        emit SetWrapper(_wrapperAddress, _tokenAmount);
    }

    /**
     * @dev Register airdrop for _user. 
     * Can only be called by registry contract (mostly BaseRegistrarImplementation).
     * @param _user address - address at which airdrop is registered.
     */
    function registerAirdrop(address _user) external {
        uint256 _tokenAmount = whitelistedWrappers[msg.sender];
        require(_tokenAmount != 0, "DGS: Auth restricted");
        userToClaim[_user] = _tokenAmount;
        emit RegisterAirdrop(_user, _tokenAmount);
    }
    ...
    function claimTokens(address delegate) external {
        uint256 _amount = userToClaim[msg.sender];
        require(_amount > 0, "DGS: no tokens to claim");
    ...
    //     require(valid, "DGS: Valid proof required.");
    //     require(!isClaimed(index), "DGS: Tokens already claimed.");
    ...
    function sweep(address dest) external onlyOwner {
        require(block.timestamp > claimPeriodEnds, "DGS: Claim period not yet ended");
    ...
    //     require(merkleRoot == bytes32(0), "DGS: Merkle root already set");
    ...
    function mint(address dest, uint256 amount) external onlyOwner {
        require(amount <= (totalSupply() * mintCap) / 10000, "DGS: Mint exceeds maximum amount");
        require(block.timestamp >= nextMint, "DGS: Cannot mint yet");
    ...
    /**
     * @dev Burn owner tokens. Owner must have at least "amount" token.
     * @param amount uint256 - amount of tokens to burn from owner's account 
     */
