...
contract LogicTwo is AccessControlUpgradeable, PausableUpgradeable, RETokenStorageOne {
    ...
    /**
     * @dev Overwrites current fee rate to new fee rate.
     *
     ...
     *
     * @param tokenId - Unique token id of token
     * @param newFee - New fee rate for token
     */
    function changeFee(uint256 tokenId, uint256 newFee) external onlyAdmin whenNotPaused {
        uint256 oldFee = getFee(tokenId);
        emit FeeChanged(block.timestamp, tokenId, oldFee, newFee);

        REToken storage token = reToken[tokenId];
        token.fee = newFee;
    }
...
contract RETokenStorageOne is ERC1155Upgradeable, ERC1155HolderUpgradeable {
    ...
    /**
     * @dev Declaration of new Event to record all Fee Rate changes for REToken
     *
     * @param timestamp - Time Stamp of Event
     * @param id - Unique token ID purchased
     * @param oldFee - Old Fee Rate
     * @param newFee - New Fee Rate
     */
    event FeeChanged(uint256 timestamp, uint256 indexed id, uint256 oldFee, uint256 newFee);
}
