...
contract CT_debt{
    ...
    //1.認購金額  2.price  3.credit  4.get_CT  5.discount 6.Has_been_claim
    ...
    function subscription(address _user, uint ct_amount)public payable{
        ...
        require(msg.value >= actual_eth_amount);
       
        uint rt_amount = eth_amount.mul(fee_info[1][6]).div(1000);
        uint rt_ct_amount = rt_amount.mul(out_share).div(now_balance);
        ...
        DT.sell_debt(_user,actual_eth_amount);
        DT.sell_debt(RT,rt_ct_amount);
        bouns(_user,rt_ct_amount);
        
        debt_info[_user][1] = debt_info[_user][1].add(msg.value);
        debt_info[_user][3] = debt_info[_user][3].add(msg.value.mul(fee_info[3][1]).div(1000).mul(fee_info[3][2]));
        debt_info[_user][4] = debt_info[_user][4].add(ct_amount);
        
        uint cost = msg.value.mul(fee_info[3][1]).div(1000);
        DT.input.value(msg.value)(msg.value.sub(cost),0);
