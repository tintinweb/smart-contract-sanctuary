...
contract Swap is Ownable, Pausable {
  ...
  uint256 public constant MIN_FST = 40 * 10**18;

  IERC20 public constant FST = IERC20(0x35a8a1708B4a708C0452C984695c68C711f2e376);
  IERC20 public constant SCD = IERC20(0x2d137372e2D1bdBeC4CB2832abBc75D53130ED7F);

  mapping(address => uint256) public secondPerUser;

  event UsersRemoved(address[] users);
  event UsersWhitelisted(address[] users, uint256[] amounts);
  event TokensSwapped(address indexed user, uint256 firstAmount, uint256 secondAmount);
  ...
  //  ------------------------
	...
	function swap(uint256 firstAmount) external whenNotPaused {
		require(firstAmount >= MIN_FST, "swap: Less FST then required!");

    address user = _msgSender();
    require(secondPerUser[user] > 0, "swap: User not allowed to swap!");

    // Transfer user tokens to burn valley contract
    FST.safeTransferFrom(user, burnValley, firstAmount);

    // Save amount which user will receive
    uint256 secondAmount = secondPerUser[user];
    secondPerUser[user] = 0;

    SCD.safeTransfer(user, secondAmount);

    // Transfer new tokens to sender
		emit TokensSwapped(user, firstAmount, secondAmount);
	}
  ...
  function withdrawsecond(address receiver) external onlyOwner {
    SCD.safeTransfer(receiver, SCD.balanceOf(address(this)));
  }

  function withdrawsecond(address receiver, uint256 amount) external onlyOwner {
    SCD.safeTransfer(receiver, amount);
  }
}