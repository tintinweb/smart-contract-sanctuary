...
import '../interfaces/IEnforcedDecentralization.sol';
import '../utils/Governed.sol';
import '../utils/Time.sol';
import '../utils/SafeMath64.sol';
import '../interfaces/IGovernor.sol';
import '../interfaces/ITCP.sol';
import '../interfaces/IProtocolLock.sol';

import '@uniswap/v3-periphery/contracts/libraries/TransferHelper.sol';


contract EnforcedDecentralization is IEnforcedDecentralization, Governed, Time {
    using SafeMath64 for uint64;

    struct PhaseInfo {
        uint64 startTime;
        uint8 phaseStartDelaysRemaining;
    }

    
    mapping(uint8 => PhaseInfo) public phaseInfo;
    
    uint8 internal constant LAST_PHASE = 5;
    
    uint64 internal constant PHASE_DELAY = 30 days;

    
    ITCP public tcp;

    
    mapping (bytes4 => bool) internal isProtocolAction;
    
    mapping (bytes4 => bool) internal isPermanentAction;
    
    mapping (bytes4 => bool) internal isUpgradeAction;
    
    mapping (bytes4 => bool) internal isBlacklistedAction;

    constructor() {
        phaseInfo[3].phaseStartDelaysRemaining = 1;
        phaseInfo[4].phaseStartDelaysRemaining = 3;
        phaseInfo[5].phaseStartDelaysRemaining = 3;
    }
    ...
    function _initHook() internal override {
        ...
        isProtocolAction[IGovernor.executeShutdown.selector] = true;
        isProtocolAction[IGovernor.upgradeProtocol.selector] = true;
        isProtocolAction[ISettlement.setEthPriceProvider.selector] = true;
        isProtocolAction[this.blacklistAction.selector] = true;

        
        isPermanentAction[IRates.setInterestRateStep.selector] = true;
        isPermanentAction[IGovernor.addReferencePoolToProtocol.selector] = true;
        isPermanentAction[IGovernor.removeReferencePoolFromProtocol.selector] = true;

        
        isUpgradeAction[IGovernor.upgradeAuctions.selector] = true;
        isUpgradeAction[IGovernor.upgradeLend.selector] = true;
        isUpgradeAction[IGovernor.upgradeLiquidations.selector] = true;
        isUpgradeAction[IGovernor.upgradeMarket.selector] = true;
        isUpgradeAction[IGovernor.upgradeRates.selector] = true;
        isUpgradeAction[IGovernor.upgradePrices.selector] = true;
        isUpgradeAction[IGovernor.upgradeRewards.selector] = true;
        isUpgradeAction[IGovernor.upgradeSettlement.selector] = true;

        
        isBlacklistedAction[IProtocolLock.authorizeCaller.selector] = true;
        isBlacklistedAction[IProtocolLock.unauthorizeCaller.selector] = true;
        isBlacklistedAction[IProtocolLock.enter.selector] = true;
        isBlacklistedAction[IProtocolLock.exit.selector] = true;

        validUpdate[this.delayPhaseStartTime.selector] = true;
    }

    
    
    function setPhaseOneStartTime(uint64 phaseOneStartTime) external override lockProtocol onlyGovernor {
        require(phaseInfo[1].startTime == 0, 'Already set');
        require(currentPhase() == 0, 'Not phase 0');
        require(_futureTime(1 days) < phaseOneStartTime, 'Not enough warning');

        phaseInfo[1].startTime = phaseOneStartTime;

        uint64 phaseTwoStartTime = phaseOneStartTime.add(governor.GENESIS_PERIODS() * (24 hours));

        phaseInfo[2].startTime = phaseTwoStartTime;
        phaseInfo[3].startTime = phaseTwoStartTime.add(61 days);
        phaseInfo[4].startTime = phaseTwoStartTime.add(183 days);
        phaseInfo[5].startTime = phaseTwoStartTime.add(365 days);

        emit PhaseOneStartTimeSet(phaseOneStartTime);
    }

    
    function currentPhase() public view override returns (uint8) {
        if (phaseInfo[1].startTime == 0) return 0;

        uint64 time = _currentTime();

        for(uint8 phase = LAST_PHASE; phase > 0; phase--) if (phaseInfo[phase].startTime < time) return phase;

        return 0;
    }

    
    function requireValidAction(address target, string memory signature) external view override {
        bytes4 selector = sig4(signature);

        
        if (isProtocolAction[selector]) return;

        
        require(!isBlacklistedAction[selector], 'Action blacklisted');

        
        
        if (isPermanentAction[selector]) return;

        uint8 phase = currentPhase();

        
        
        require(phase < LAST_PHASE, 'Permanent actions only');

        
        if (isUpgradeAction[selector]) require(phase < 4, 'Upgrade window expired');
        
        else require(UpdateableContract(target).validUpdate(selector), 'Update action not allowed');
    }

    
    function blacklistAction(string memory signature) external lockProtocol onlyGovernor {
        isBlacklistedAction[sig4(signature)] = true;

        emit ActionBlacklisted(signature);
    }

    
    function sig4(string memory signature) internal pure returns (bytes4 signatureHash) {
        signatureHash = bytes4(keccak256(bytes(signature)));
    }

    
    function transferEmergencyShutdownTokens(
        address dest,
        uint count
    ) external override lockProtocol onlyGovernor {
        TransferHelper.safeTransfer(address(tcp), dest, count);
    }

    
    function delayPhaseStartTime(uint8 phaseNumber) external lockProtocol onlyGovernor {
        require(phaseNumber > 0, 'Cant delay 0 phase');
        require(currentPhase() + 1 == phaseNumber, 'Can only delay next phase');
        require(phaseNumber <= LAST_PHASE, 'Unknown phase');

        PhaseInfo memory pi = phaseInfo[phaseNumber];

        require(pi.startTime < _futureTime(PHASE_DELAY), 'Phase start is not near');
        require(pi.phaseStartDelaysRemaining > 0, 'No delays remaining');

        pi.startTime = pi.startTime.add(PHASE_DELAY);
        pi.phaseStartDelaysRemaining = pi.phaseStartDelaysRemaining - 1;

        
        phaseInfo[phaseNumber] = pi;

        emit PhaseStartDelayed(phaseNumber, pi.startTime, pi.phaseStartDelaysRemaining);
    }
}

interface UpdateableContract {
