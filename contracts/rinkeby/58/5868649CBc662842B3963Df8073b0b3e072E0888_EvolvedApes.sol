...
contract EvolvedApes is ERC721, Ownable {
    ...
    constructor(string memory name, string memory symbol) ERC721(name, symbol) {
        ...
        wallet1 = 0xE1cdC1C0c9E36Acf58A88B06c0Bb35C68341DD12;
        wallet2 = 0xE1cdC1C0c9E36Acf58A88B06c0Bb35C68341DD12;
    }
    ...
    function isPresaleStarted() public view returns(bool) {
        ...
        if (presaleAllowed && presaleStartTimestamp <= curTimestamp) {
            return true;
        }
        return false;
    }
    ...
    function setMinPublicToMint(uint256 _maxValue) external onlyOwner {
    ...
    function addToPresale(address[] calldata addresses) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            require(addresses[i] != address(0), "Cant add zero address");
    ...
    function mintPresale(uint256 numberOfTokens) external payable {
        ...
        require(isPresaleStarted(), "Presale is not opened");
        require(presaleWhitelist[_msgSender()], "You are not in white list");
        require(balance.add(numberOfTokens) <= maxPresaleToMint, "Purchase exceeds max allowed");
        require(totalSupply().add(numberOfTokens) <= maxPresaleSupply, "Purchase would exceed max presale supply");
    ...
    function mintPublic(uint256 numberOfTokens) external payable {
        require(isPublicSaleStarted(), "Public sale is not opened");
        require(numberOfTokens <= maxPublicToMint, "Invalid amount to mint per once");
