...
abstract contract KIAMintPass {
    ...
    function tokenOfOwnerByIndex(address owner, uint256 tokenIndex)
        external
        view
        virtual
...
contract KIA is ERC721Enumerable, Ownable {
    ...
    uint256 public _conversionRate = 7;

    KIAMintPass private KIAMP;

    uint256 private _saleTime = 1629759600; // Date and time (GMT): Wednesday, September 1, 2021 9:00:00 PM (https://www.epochconverter.com/)
    uint256 private _price = 5 * 10**16; // Currently .05 ETH
    uint256 private _maxPerTx = 16; // Set to one higher than actual, to save gas on lte/gte checks.

    string private _baseTokenURI;

    constructor(string memory baseURI, address KIAMintPassAddress)
        ERC721("KIA", "KIA")
    {
        setBaseURI(baseURI);
        KIAMP = KIAMintPass(KIAMintPassAddress);
    }
    ...
    // Note: It would be standard dev practice to bundle the premint function in this. However, because we have to be gas-aware this is not something that we can do.
    function preSaleMint(uint256 _count) public payable {
        uint256 totalSupply = totalSupply();
        require(
            _count <= _conversionRate,
            "A Mint Pass cannot mint that many Koalas."
        );
        require(
        ...
        require(
            KIAMP.balanceOf(msg.sender) > 0,
            "You must own a mint pass to claim Koalas during the pre-sale."
        );
        ...
        uint256 mintPassId = KIAMP.tokenOfOwnerByIndex(_msgSender(), 0);
        KIAMP.burn(mintPassId);
    }
    ...
    function reserveKoalas(uint256 _count) public onlyOwner {
        uint256 totalSupply = totalSupply();
        require(block.timestamp < _saleTime, "Minting has already started.");
