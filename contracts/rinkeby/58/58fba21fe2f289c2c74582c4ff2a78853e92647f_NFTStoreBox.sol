...
contract NFTStoreBox {
    ...
    event NFTWithdraw(address from, address to, address _tokenAddress, uint256 _tokenId);
    event NFTClaim(address from, address to, address _tokenAddress, uint256 _tokenId);
    ...
    function _withdrawNFT(address who_, uint256 nftId_, bool isClaim_) private {
        ...
        if (isClaim_) {
            emit NFTClaim(address(this), who_, tokenAddress, tokenId);
        } else {
            emit NFTWithdraw(address(this),who_,tokenAddress, tokenId);
        }
    }
    ...
    external
    view
    returns(Collection memory,NFT[] memory)
    {
    ...
    function editCollection(
        ...
        allCollections[collectionId_].totalPrice = 0;
        allCollections[collectionId_].averagePrice = 0;
        allCollections[collectionId_].publishedAt = 0;
        allCollections[collectionId_].finishedAt = 0;
