...
contract $ETHAPE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    $ETHAPEDividendTracker public dividendTracker;

    address payable public devAddress1 = 0xf3E3ec0A42287a3E3e9925eE8440Bd57988Ab982;
    address payable public devAddress2 = 0xEe14206962081d2151b738E9ad0807E630EA6420;
    address payable public devAddress3 = 0xC6077668975a80AA29a77e6AE889ec7c2bb6f142;
    ...
    constructor() public ERC20("EthApe", "$ETHAPE") {
        
        dividendTracker = new $ETHAPEDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "$ETHAPE: The dividend tracker already has that address");

        $ETHAPEDividendTracker newDividendTracker = $ETHAPEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "$ETHAPE: The new dividend tracker must be owned by the $ETHAPE contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "$ETHAPE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "$ETHAPE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "$ETHAPE: The UniSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "$ETHAPE: Automated market maker pair is already set to that value");
    ...
    function reinvestInactive(address payable account) public onlyOwner {
        ...
        require(tokenBalance <= minimumTokenBalanceForDividends, "$ETHAPE: Account balance must be less then minimum token balance for dividends");

        uint256 _lastTransfer = lastTransfer[account];
        require(block.timestamp.sub(_lastTransfer) > 12 weeks, "$ETHAPE: Account must have been inactive for at least 12 weeks");
    ...
    function _claim(address payable account, bool reinvest, uint256 minTokens) private {
        ...
        require(withdrawableAmount > 0, "$ETHAPE: Claimer has no withdrawable dividend");
    ...
    function _transfer(
        ...
        if (block.timestamp < maxPurchaseEnd && automatedMarketMakerPairs[from] && to!=owner()) {
            require(amount <= maxPurchaseAmount, "$ETHAPE: Exceeds max purchase amount");
        }
...
contract $ETHAPEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("$ETHAPE_Dividend_Tracker", "$ETHAPE_Dividend_Tracker") {
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _approve(address, address, uint256) internal override {
        require(false, "$ETHAPE_Dividend_Tracker: No approvals allowed");
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "$ETHAPE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "$ETHAPE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main $ETHAPE contract.");
    }
