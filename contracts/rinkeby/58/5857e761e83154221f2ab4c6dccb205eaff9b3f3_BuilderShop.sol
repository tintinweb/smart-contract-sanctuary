...
contract ERC721 is NiftyEntity, Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _getNiftyTypeId(uint tokenId) private view returns (uint) {
        return uint((tokenId - topLevelMultiplier) / midLevelMultiplier);
    }
...
contract NiftyBuilderInstance is ERC721, ERC721Burnable {
     ...
     * for the project.
     */
    function giftNifty(uint count, uint niftyType) public onlyValidSender {
        require(!_getFinalized(niftyType), "NiftyBuilderInstance: minting concluded for nifty type");
            
        uint tokenNumber = _mintCount[niftyType];
        uint tokenId = _encodeTokenId(niftyType, tokenNumber);
        
        for (uint i = 0; i < count; i++) {

            tokenId += 1;  
            _owners[tokenId] = _defaultOwner;
            emit Transfer(address(0), _defaultOwner, tokenId);
        }
...
contract BuilderShop is NiftyEntity {
     ...
     */ 
    function setDefaultOwner(address defaultOwner) onlyValidSender public {
    ...
    function createNewBuilderInstance(
        ...
        uint typeCount,
        string memory baseURI,
        string memory creator) public onlyValidSender { 
