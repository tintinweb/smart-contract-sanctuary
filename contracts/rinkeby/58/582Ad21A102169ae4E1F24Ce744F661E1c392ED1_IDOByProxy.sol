...
contract IDOByProxy is ReentrancyGuard, Initializable {
    ...
    modifier onlyWhitelisted(address _userAddress) {
        require(whitelist[_userAddress], "you are not whitelisted");
        _;
    }

    function addToWhitelist(address _userAddress) external onlyAdmin {
        whitelist[_userAddress] = true;
    }

    function addManyToWhitelist(address[] calldata _userAddresses)
        external
        onlyAdmin
    {
        for (uint256 i = 0; i < _userAddresses.length; i++) {
            whitelist[_userAddresses[i]] = true;
        }
    }

    function removeFromWhitelist(address _userAddress) external onlyAdmin {
    ...
    function deposit(uint256 _amount) public onlyWhitelisted(msg.sender) {
        require(
    ...
    function harvest() public onlyWhitelisted(msg.sender) nonReentrant {
    ...
    {
        require(
            _lpAmount < raisingToken.balanceOf(address(this)),
            "not enough token 0"
        );
        require(
