...
contract BadDecision is ERC721Enumerable, EIP712, AccessControl {
    ...
    function check(Voucher memory voucher, bytes calldata signature) private {
        ...
        if (voucher.auction) {
            require(
                parseAddress(voucher.winner) == msg.sender,
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "Voucher(uint256 product,uint256 count,uint256 price,uint256 starttime,uint256 endtime,bool auction,string winner)"
                        ),
                        voucher.product,
                        voucher.count,
                        voucher.price,
