...
contract CapitalAgent is ICapitalAgent, ReentrancyGuard {
    ...
    uint256 public totalCapitalStakedInUSDT;
    ...
    uint256 public POLICY_UPDATE_PERIOD = 1 days;
    ...
    constructor(address _exchangeAgent) {
        owner = msg.sender;
    ...
    function addPool(address _ssip) external override {
        ...
        poolInfo[_ssip] = PoolInfo({totalCapital: 0, totalCapitalInUSDT: 0, exist: true});
    ...
    function addPolicy(address _policy) external override nonReentrant {
        ...
        policyInfo[_policy] = PolicyInfo({utilizedAmount: 0, policyTokenIds: tokenIds, exist: true});
    ...
    function SSIPWithdraw(uint256 _withdrawAmount, uint256 _withdrawAmountInUSDT) external override nonReentrant {
        require(poolInfo[msg.sender].exist, "UnoRe: no exist ssip");
        require(_checkCapitalByMCR(_withdrawAmountInUSDT), "UnoRe: minimum capital underflow");
        _updatePoolCapital(msg.sender, _withdrawAmount, _withdrawAmountInUSDT, false);
    }

    function SSIPStaking(uint256 _stakingAmount, uint256 _stakingAmountInUSDT) external override nonReentrant {
        require(poolInfo[msg.sender].exist, "UnoRe: no exist ssip");
        _updatePoolCapital(msg.sender, _stakingAmount, _stakingAmountInUSDT, true);
    }

    function policySale(uint256 _coverageAmount, uint256 _policyTokenId) external override nonReentrant {
        ...
        policyInfo[msg.sender].policyTokenIds.push(_policyTokenId);
        _updatePolicyCoverage(msg.sender, _coverageAmount, true);
    }

    function updatePolicyStatus() external override nonReentrant {
        require(
            block.timestamp > lastUpdateTime && block.timestamp - lastUpdateTime >= POLICY_UPDATE_PERIOD,
            "UnoRe: not update period yet"
        );
        uint256 lastPolicyIdx = policyIds.current();
        for (uint256 ii = 0; ii < lastPolicyIdx; ii++) {
            address policyAddr = policyList[ii];
            uint256 len = policyInfo[msg.sender].policyTokenIds.length;
            for (uint256 kk = 0; kk < len; kk++) {
                (, uint256 _coverageAmount, uint256 _coverageDuration, uint256 _coverStartAt, ) = ISalesPolicy(policyAddr)
                    .getPolicyData(policyInfo[msg.sender].policyTokenIds[kk]);
                bool isExpired = block.timestamp >= _coverageDuration + _coverStartAt;
                if (isExpired) {
                    _updatePolicyCoverage(policyAddr, _coverageAmount, false);
                    ISalesPolicy(policyAddr).updatePolicyExpired(policyInfo[msg.sender].policyTokenIds[kk]);
                    emit LogUpdatePolicyExpired(policyAddr, policyInfo[msg.sender].policyTokenIds[kk]);
                }
            }
        }
    }
    ...
    function markToClaimPolicy(address _policy, uint256 _policyId) external onlyOwner nonReentrant {
        ISalesPolicy(_policy).updatePolicyExpired(_policyId);
    ...
    function _updatePoolCapital(
        ...
        poolInfo[_pool].totalCapitalInUSDT = isAdd
            ? poolInfo[_pool].totalCapitalInUSDT + _amountInUSDT
            : poolInfo[_pool].totalCapitalInUSDT - _amountInUSDT;
        totalCapitalStaked = isAdd ? totalCapitalStaked + _amount : totalCapitalStaked - _amount;
        totalCapitalStakedInUSDT = isAdd ? totalCapitalStakedInUSDT + _amountInUSDT : totalCapitalStakedInUSDT - _amountInUSDT;
    ...
    function _checkCapitalByMCR(uint256 _withdrawAmount) private view returns (bool) {
        return totalCapitalStakedInUSDT - _withdrawAmount >= (totalCapitalStakedInUSDT * MCR) / CALC_PRECISION;
    }
