...
contract IceWorld is ERC721Enumerable, Pausable, PaymentSplitter {
    ...
    function setUpSale(uint256 _duration) external onlyOwner {
        require(workflow == WorkflowStatus.Presale, "Unauthorized Transaction");
        PresaleConfig memory _presaleConfig = presaleConfig;
        uint256 _presaleEndTime = _presaleConfig.startTime +
            _presaleConfig.duration;
        require(block.timestamp > _presaleEndTime, "Sale not started");
        uint256 _startTime = block.timestamp;
   ...
   function giftReserve(address[] calldata _address, uint256[] calldata _id)
        external
        onlyOwner
        whenNotPaused
    {
     _giftAddress = _address ;
    _giftIds = _id;
 ...
 function freeMint(address[] calldata _address)
    ...
    function presaleMint(uint256 _amount) internal {
        ...
        require(
            ...
            "Presale must be active to mint Ape"
        );
        require(
        ...
        require(totalNFT + _amount <= maxTotalSupply, "max supply exceeded");
    ...
    function saleMint(uint256 _amount) internal {
        ...
        require(_saleConfig.startTime > 0, "sale is not active");
