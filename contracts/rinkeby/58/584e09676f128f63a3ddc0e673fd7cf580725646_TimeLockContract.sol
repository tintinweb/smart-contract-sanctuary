...
contract TimeLockContract {
    ...
    modifier refundable(address _addressId) {
        bytes32 _contractId = contractids[_addressId];
    ...
    function withdraw(address _addressId)
        external
        contractExists(_addressId)
        withdrawable(_addressId)
        returns (bool)
    {
    ...
    function refund(address _addressId)
        external
        contractExists(_addressId)
        refundable(_addressId)
        returns (bool)
    {
        bytes32 _contractId = contractids[msg.sender];
        LockContract storage c = contracts[_contractId];
