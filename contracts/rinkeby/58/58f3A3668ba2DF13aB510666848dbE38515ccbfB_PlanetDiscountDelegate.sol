    ...
    function changeUserBorrowDiscount(address borrower) external returns(uint,uint,uint){
        ...
        if( discount > 0 && currentBorrowBal > 0) {
            ...
            if(_dis.exist){
                ...
                address lastUser = usersWhoHaveBorrow[_market][usersWhoHaveBorrow[_market].length-1];
                BorrowDiscountSnapshot storage lastUserInArr = borrowDiscountSnap[_market][lastUser];
                usersWhoHaveBorrow[_market][_dis.index] = usersWhoHaveBorrow[_market][usersWhoHaveBorrow[_market].length-1];
                lastUserInArr.index = _dis.index;
                usersWhoHaveBorrow[_market].length--;
                delete borrowDiscountSnap[_market][borrower];
