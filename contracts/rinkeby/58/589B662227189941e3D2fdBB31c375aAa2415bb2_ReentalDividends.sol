...
import "./ReentalDeployer.sol";
import "./interfaces/IReentalWhitelist.sol";
import "./interfaces/IReentalToken.sol";
import "./utils/UUPSUpgradeableByRole.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts-upgradeable/security/PausableUpgradeable.sol";
import "@openzeppelin/contracts-upgradeable/utils/cryptography/MerkleProofUpgradeable.sol";

contract ReentalDividends is UUPSUpgradeableByRole {
  ...
  function _distribute(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof, address to, uint256 distribution) internal whenNotPaused {
    ...
    IERC20 dividendsInterface = IERC20(IReentalDeployer(config).get(keccak256("USDT")));
    address vault = IReentalDeployer(config).get(keccak256("REENTAL_VAULT"));
    distributed[account] = distributed[account] + distribution;
    if (account != to){
      require(dividendsInterface.transferFrom(vault, to, distribution), "ReentalDividends: dividends transfer failed");
      emit Distributed(account, to, distribution, true, 0);
    }else{
      uint256 retention = _retention(account, distribution);
      uint256 claimed = distribution - retention;
      // require(dividendsInterface.transfer(to, retention), "ReentalDividends: retention transfer failed");
      require(dividendsInterface.transferFrom(vault, account, claimed), "ReentalDividends: dividends transfer failed");
      emit Distributed(account, to, claimed, false, retention);
    }
  }

  // Gets retention for an amount
  function _retention(address account, uint256 amount) internal view returns(uint256){
    IReentalWhitelist whitelistInterface = IReentalWhitelist(IReentalDeployer(config).get(keccak256("REENTAL_WHITELIST")));
    return (amount * whitelistInterface.retentionPercent(account)) / 1e20;
  }
  ...
  function reinvest(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof, address to, uint256 distribution) public {
    require(msg.sender == IReentalDeployer(config).get(keccak256("REENTAL_CROWDSALE")), "ReentalDividends: only crowdsale can reinvest dividends");
  ...
  function extractTokens(address token) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    require(tokenInterface.transfer(_msgSender(), balance), "ReentalDividends: extraction failed");
  }

  function extractETH() public onlyRole(DEFAULT_ADMIN_ROLE) {
    require(payable(_msgSender()).send(address(this).balance), "ReentalDividends: funds transfer failed");
  }
}
...
interface IReentalDeployer {
    ...
    function extractTokens(address token) external;
    function extractETH(address token) external;
}

contract ReentalProxy is ERC1967Proxy {
...
contract ReentalDeployer is AccessControlUpgradeable {
    ...
    function setId(bytes32 id, address addr) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(get[id] == address(0), "ReentalDeployer: id already exists");
        get[id] = addr;
        name[addr] = id;
    ...
    function _deployProxy(bytes32 id, address implementation, bytes memory initializeCalldata) private {
        ReentalProxy proxy = new ReentalProxy(
            implementation,
            initializeCalldata
        );
