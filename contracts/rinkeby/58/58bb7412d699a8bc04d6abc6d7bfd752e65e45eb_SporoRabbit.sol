...
abstract contract DimmCityV1Base is ERC721Enumerable {
    ...
    function _beforeTokenTransfer(
        ...
        if (
            isAllowedToTransferTokens(from) == false &&
            to != address(0) &&
            from != address(0)
        ) {
    ...
    function isApprovedForAll(address _owner, address operator)
        ...
        returns (bool)
    {
        if (Utils.isOpenSeaProxy(ProxyRegistryAddress, _owner, operator))
            return true;

        return super.isApprovedForAll(_owner, operator);
    }

    function isAllowedToTransferTokens(address _owner)
        internal
        view
        returns (bool)
    {
        if (!EnableTransferGuard) return true;
    ...
    function withdraw() external nonZeroAddress(WithdrawalAddress) onlyAdmins {
