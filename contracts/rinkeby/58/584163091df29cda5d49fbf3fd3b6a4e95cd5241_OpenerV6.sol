...
contract OpenerV6 is Initializable, OwnableUpgradeable {
    ...
    ITransferFromAndBurnFrom private _sPmonToken;
    ITransferFromAndBurnFrom private _USDT;
    uint256 public _usdtStakerPercent;
    uint256 public _usdtFeePercent;
    uint256 public _usdtPerBooster;
    uint256 constant _usdtDecimalMultiplier = 10**6; 
    address public _usdtCollectorAddress;
    ...
    // function openBoosterWithSPMON(uint256 amount) public {
    //     require(!_closed, "Opener is locked");
    //     require(amount > 0, "Amount has to be larger than 0");
    //     require(
    //         _numOfBoosterIsInteger(amount),
    //         "Only integer numbers of booster allowed"
    //     );
    //     _sPmonToken.burnFrom(msg.sender, amount);
    //     _addHash(amount, _openedBoosters, msg.sender);
    //     emit Opening(msg.sender, amount, _openedBoosters);
    //     _openedBoosters = _openedBoosters + (amount / _decimalMultiplier);
    // }

    function openBoosterWithUSDT(uint256 boosterAmount) public {
        require(!_closed, "Opener is locked");
        require(boosterAmount > 0, "Amount has to be larger than 0");
        require(
            _numOfBoosterIsInteger(boosterAmount),
            "Only integer numbers of booster allowed"
        );

        //number of usdt berechnen und so
        uint256 usdtAmount = _usdtPerBooster * _usdtDecimalMultiplier * boosterAmount / _decimalMultiplier;
        _USDT.transferFrom(
            msg.sender,
            _usdtCollectorAddress,
            usdtAmount
        );

        _addHash(boosterAmount, _openedBoosters, msg.sender);
        emit Opening(msg.sender, boosterAmount, _openedBoosters);
        _openedBoosters = _openedBoosters + (boosterAmount / _decimalMultiplier);
    }
    ...
    function _addHash(
        ...
        uint256 lastId = firstId + (amount / _decimalMultiplier) * 3 - 1;
        registeredHashes[userAddress][
            keccak256(abi.encode(userAddress, firstId, lastId))
        ] = true;
    }
    ...
    function mint(
        ...
        require(
            idToMint >= firstId && idToMint <= lastId,
            "Not Owner of requested NFT"
        );

        _nftContract.mint(to, idToMint);
    }

    function _numOfBoosterIsInteger(uint256 amount)
        private
        view
        returns (bool)
    {
    ...
    function _distributeBoosterShares(address from, uint256 amount) private {
        ...
        if (_stakeShare > 0) {
            //if stakingPool isn't set, we transfer as fallback to the stakingaddress
            if (address(_stakingPool) == address(0)) {
                ...
                uint256 stakingAmount = (amount * _stakeShare) / 100;
                _pmonToken.transferFrom(from, address(this), stakingAmount);
                _stakingPool.addRewards(stakingAmount);
            }
        }
    ...
    function setStakeAddress(address stakeAddress) public {
        require(msg.sender == _ownerAddress, "Sender is not owner");
        _stakeAddress = stakeAddress;
    }
    ...
    function setSwapBackAddress(address swapBackAddress) public {
        require(msg.sender == _ownerAddress, "Sender is not owner");
        _swapBackAddress = swapBackAddress;
    }
    ...
    function setStakingPool(IStakingPool stakingPool) public {
        require(msg.sender == _ownerAddress, "Sender is not owner");
        require(
            address(stakingPool) != address(0),
            "Address of stakingPool is zero-address"
        );

        _stakingPool = stakingPool;

        IApprovable(address(_pmonToken)).approve(
            address(_stakingPool),
            10000000 * 10**uint256(18)
        );
    }
    ...
    function setOwner(address ownerAddress) public {
        ...
        if (_ownerAddress == address(0)) {
            ...
            require(
                ...
                "New OwnerAddress cannot be zero-address"
            );
            _ownerAddress = ownerAddress;
        }
    }

    // function setSPMON(address sPmonAddress) public {
    //     require(msg.sender == _ownerAddress, "Sender is not owner");
    //     _sPmonToken = ITransferFromAndBurnFrom(sPmonAddress);
    // }

    function setUSDT(address usdtAddress) public {
        require(msg.sender == _ownerAddress, "Sender is not owner");
        _USDT = ITransferFromAndBurnFrom(usdtAddress);
        // _usdtStakerPercent = stakerSharePercent;
        // _usdtFeePercent = feeSharePercent;
    }

    function setUsdtPerBooster(uint256 usdtPerBooster) public {
        require(msg.sender == _ownerAddress, "Sender is not owner");
        _usdtPerBooster = 10**uint256(6) * usdtPerBooster;
    }

    function setUsdtCollector(address usdCollector) public {
        require(msg.sender == _ownerAddress, "Sender is not owner");
        _usdtCollectorAddress = usdCollector;
    }
}
