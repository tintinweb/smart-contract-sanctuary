...
contract ErrorCode {
  ...
  enum Error {
              ...
              INVALID_PRINCIPAL,
              INVALID_PRINCIPAL_PLUS_INTEREST,
              INVALID_ENDBLOCK,
              INVALID_SIDE,
              INVALID_NONCE,
...
contract QodaV1FixedRateLoan is IQodaV1FixedRateLoan, ErrorCode {
  ...
  function borrow(
    ...
    // Explicitly revert signature mismatches
    require(isSignatureMatch, "invalid signature");
  ...
  /// @notice Returns the signatures (pointers) of all borrows for a given account
  /// @param account Address of the account to query
  /// @return bytes[] An array of all the signatures pointing to borrows for the account
  ...
  /// @notice Returns the signatures (pointers) of all borrows for a given account
  /// @param account Address of the account to query
  /// @return bytes[] An array of all the signatures pointing to lends for the account
  function getAccountLends(address account) external view returns(bytes[] memory) {
    return accountLends[account];
  }

  /// @notice Returns the FixedRateLoan object for a given signature pointer
  /// @param signature The Quote signature doubles as the key for `outstandingLoans`
  ...
  function _createFixedRateLoan(
    ...
    if(principalPlusInterest < principal){
      return fail(Error.INVALID_PRINCIPAL_PLUS_INTEREST);
    }

    if(lender == borrower) {
      return fail(Error.INVALID_COUNTERPARTY);
    }
