...
contract EZTWithdrawal {
    ...
    function getDigest(address user, uint256 amount) public view returns (bytes32 domainSeparator, bytes32 structHash, bytes32 digest) {
        domainSeparator = keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes(CONTRACT_NAME)), getChainId(), address(this)));
        structHash = keccak256(abi.encode(WITHDRAW_TYPEHASH, user, amount));
