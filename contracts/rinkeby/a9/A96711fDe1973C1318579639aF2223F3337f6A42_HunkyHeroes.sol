...
contract HunkyHeroes is ERC721Enumerable, ERC721Burnable {
    ...
    address public msgSigner = address(0x5F5668780ee882a968968845bA479A45CfC3c816); //update before live
    ...
    constructor(string memory baseURI_) ERC721("HunkyHeroes", "HERO") {
        ...
        //RESET SET THESE BEFORE MAINNET

        teamMap[0x5F5668780ee882a968968845bA479A45CfC3c816] = true; //Panda
        teamMap[0x7636D709c18DE85d3DD116D2065551f1D6d39b93] = true; //Chief
        teamMap[0x21D1332dDa69a368D26577471554bF9Ec05a6877] = true; //Mayor
    ...
    /**
     * @notice mint heroes from the whitelist heroListQuotas
     * @dev one whitelist mint per wallet
     * @param _msgHash whitelist hash 
        ...
        presaleIsLive
    {
        ...
        require(legitSigner(_sig, _msgHash),"invalid signature");
        require(legitHash(_msgHash),"invalid claim hash");
    ...
    */
    ...
    function withdraw() public onlyTeam {
        ...
        //Panda
        payable(0x5F5668780ee882a968968845bA479A45CfC3c816).send(
            bal * 2000 / 10000
        );
        //Chief
        payable(0x7636D709c18DE85d3DD116D2065551f1D6d39b93).send(
            bal * 4000 / 10000
        );
        //Mayor
        payable(0x21D1332dDa69a368D26577471554bF9Ec05a6877).send(
            bal * 4000 / 10000
        );
    }
    ...
    function _mintHeroes(address _to, uint256 _num) private {
        uint256 baseId = totalSupply() + 1;
        require(baseId + _num - 1 <= MAX_HEROES, "not enough Heroes left");

        for (uint256 h = 0; h < _num; h++) {
            uint256 mintId = baseId + h;
            _safeMint(_to, mintId);
        }
    }

    /**
     * @notice was the signature signed by the msgSigner?
     * @param _sig the signature to inspect
     * @param _hsh the hashed message which was signed
     * @dev the signer is recovered and compared to msgSigner
     */
    function legitSigner(bytes memory _sig, bytes32 _hsh)
    private
    view
    returns (bool)
    {
        bytes32 ethHash = ECDSA.toEthSignedMessageHash(_hsh);
        address recoveredSigner = ECDSA.recover(ethHash, _sig);
        return (recoveredSigner == msgSigner);
    }

    /**
     * @notice is the sent hash valid?
     * @param _hsh is the hash to test
     * @dev the _hsh is created offchain for whitelisting
     */
    function legitHash(bytes32 _hsh)
    private
    view
    returns (bool)
    {
