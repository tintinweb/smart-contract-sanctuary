...
contract TokenForgeContractFactory is Context, AccessControlEnumerable {
    ...
    function create(
        ...
        TokenForgeERC20Token.ContractParameters calldata params
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        TokenForgeERC20Token token = new TokenForgeERC20Token(name, symbol);
...
contract TokenForgeERC20Token is Context, AccessControlEnumerable, ERC20Burnable, WhiteList, BlackList {
    ...
    function setProperty(bytes32 key, string memory val_) external onlyRole(GOVERNANCE_ROLE) {
        ...
        if (keccak256(bytes(oldValue)) == keccak256(bytes(val_))) {
            revert("TokenForgeERC20Token: new value is equal to old value");
        }
