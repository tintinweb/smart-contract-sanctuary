...
contract ProjectSummit is ERC721, Ownable {
	...
	constructor(address _treasury, string memory _base) ERC721("ProjectSummit - JDM", "JDM") {
		treasury = _treasury;
		defaultBase = _base;
	}

	modifier onlySale(bool isSale) {
		require(onSale == isSale, "Not Allowed");
		_;
	}
	...
	function reserveTickets(uint256 amount) external payable onlySale(false) {
	...
	function mint(uint256 amount) external payable onlySale(true) {
		require(totalSupply() + amount <= maxSupply, "Max Supply Reached");
		bytes32 userHash = getUserHash(msg.sender, currentPhase);
	...
	function claim(uint256 amount) external onlySale(true) {
		bytes32 userHash = getUserHash(msg.sender, currentPhase);
		require(amount <= userTickets[userHash], "Amount Invalid");
		userTickets[userHash] = userTickets[userHash] - amount;
		tickets -= amount;
		_batchLimit(amount, userHash);
	}
	...
	function reserveTokens(uint256 amount) external onlyOwner onlySale(false) {
		if (tickets > 0) {
			tickets = 0;
		}
		require(totalSupply() + amount <= maxSupply, "Max Supply Reached");
	...
	function withdraw() external {
		...
		ITreasuryDAO(treasury).raise{ value: balance }();
	}
}