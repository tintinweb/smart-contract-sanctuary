...
contract Moonopoly is ERC1155, Ownable, Pausable, ERC1155Burnable
{
    ...
    string public _BASE_URI = "https://ipfs.io/ipfs/QmbSZkXCCoursLTFzcgHHqUHVwnwqDNHRUe68DpPttBM38/";
    ...
    function mintCard(uint numberOfTokens) public payable
    {
        ...
        require(numberOfTokens <= _MAX_CARDS_PURCHASE, "Can only mint 3 Cards at a time");
        ...
        for(uint i = 0; i < numberOfTokens; i++) 
        {
            ...
            {
                _mint(msg.sender, uint256(_MINT_MAPPING[_CARDS_MINTED]), 1, "");
                _CARDS_MINTED += 1;
            }
        }
    ...
    function saleState() public onlyOwner { _SALE_IS_ACTIVE = !_SALE_IS_ACTIVE; }

    function setBaseURI(string memory BASE_URI) public onlyOwner { _BASE_URI = BASE_URI; }

    function setMaxCards(uint256 MAX_CARDS) public onlyOwner { _MAX_CARDS = MAX_CARDS; }

    function setMaxCardsPurchase(uint256 MAX_CARDS_PURCHASE) public onlyOwner { _MAX_CARDS_PURCHASE = MAX_CARDS_PURCHASE; }

    function setCardPrice(uint256 CARD_PRICE) public onlyOwner { _CARD_PRICE = CARD_PRICE; }

    function pause() public onlyOwner { _pause(); }

    function unpause() public onlyOwner { _unpause(); }

    function setRandomMint(uint8[] memory MINT_MAPPING) public onlyOwner { _MINT_MAPPING = MINT_MAPPING; }
