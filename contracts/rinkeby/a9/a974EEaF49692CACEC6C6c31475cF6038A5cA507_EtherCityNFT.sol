...
import "./Arbitrum/ArbRetryableTx.sol";
import "./Arbitrum/libraries/AddressAliasHelper.sol";

contract EtherCityNFT is ERC721, Pausable, Ownable, RoyaltiesV2Impl {
    ...
    constructor(
        ...
        inbox = IInbox(_inbox);
        arbRetryableTx = ArbRetryableTx(0x000000000000000000000000000000000000006E);
    }
    ...
    function mint() public payable whenNotPaused {
        ...
        if (msg.sender != owner()) {
            ...
            // This line is for rinkeby
//            uint256 result = inbox.depositEth{value: (msg.value / 2)}(5239467605);
            uint256 ticketID = inbox.createRetryableTicket{value: (msg.value / 4)}(
                TEAM_WALLET,
                0,
                5062172930,
                TEAM_WALLET,
            ...
            uint256 ticketID2 = inbox.createRetryableTicket{value: (msg.value / 4)}(
                TEAM_WALLET,
                0,
                5062172930,
                TEAM_WALLET,
                AddressAliasHelper.undoL1ToL2Alias(TEAM_WALLET),
                0,
                0,
                ""
            );
            emit RetryableTicketCreated(ticketID2);
        ...
        setRoyalties(currentTokenId);
        currentTokenId++;
    }

    function test() public view returns (uint, uint) {
        return arbRetryableTx.getSubmissionPrice(0);
    }

    function anotherTest() public view returns (address) {
        return AddressAliasHelper.applyL1ToL2Alias(TEAM_WALLET);
    }
    ...
    function setRoyalties(uint _tokenId) internal {
        LibPart.Part[] memory _royalties = new LibPart.Part[](3);
        _royalties[0].value = 250;
        _royalties[0].account = payable(TEAM_WALLET);
        _royalties[1].value = 250;
        _royalties[1].account = payable(LP_WALLET);
        _royalties[2].value = 250;
        _royalties[2].account = payable(CHARITY_WALLET);
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function ownerOf(uint256 tokenId) public view virtual override returns (address) {
        address owner = _owners[step][tokenId];
    ...
    function getApproved(uint256 tokenId) public view virtual override returns (address) {
        ...
        return _tokenApprovals[step][tokenId];
    }
    ...
    function _exists(uint256 tokenId) internal view virtual returns (bool) {
        return _owners[step][tokenId] != address(0);
    }
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        ...
        require(step > 0 && step <= 9, "ERC721: Step should be greater than 1 and lower than 9");

        _beforeTokenTransfer(address(0), to, tokenId);

        _balances[step][to] += 1;
        _owners[step][tokenId] = to;
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        _balances[step][owner] -= 1;
        delete _owners[step][tokenId];
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        _tokenApprovals[step][tokenId] = to;
    ...
    /**
     * @dev Safely increase the current step
     *
     * Requirements:
     */
    function _increaseStep() internal virtual {
        step++;
    }
}
