...
contract V222 is ERC721, Ownable, IMintedBeforeReveal{
    ...
    bytes32 public rootFreeMintWhiteList;
  ...
  // Maximum amount of  BABY APES  in whitelist free mint.
    ...
    function setRootFreeMintWhiteList(bytes32 _root) public onlyOwner {
        rootFreeMintWhiteList = _root;
    }

    function verifyFreeMintWhiteList(address leaf, bytes32[] memory proof) public view returns (bool){
        bytes32 _leaf = bytes32(bytes20(leaf));
        return MerkleProof.verify(proof, rootFreeMintWhiteList, _leaf);
    }



    function FreeMint(uint256 numberOfTokens, address leaf, bytes32[] memory proof) public payable returns(bool){
        ...
        require(verifyFreeMintWhiteList(leaf, proof) == true, "You are not whitelisted");
