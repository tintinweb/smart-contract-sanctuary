...
interface IMasterChef {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. SHACK to distribute per block.
        uint256 lastRewardBlock;  // Last block number that SHACK distribution occurs.
        uint256 accShackPerShare; // Accumulated SHACK per share, times 1e12. See below.
    }
...
/// @notice The (older) MasterChef contract gives out a constant number of SHACK tokens per block.
/// It is the only address with minting rights for SHACK.
...
contract MiniChefV2 is BoringOwnable, BoringBatchable {
    ...
    /// `rewardDebt` The amount of SHACK entitled to the user.
    struct UserInfo {
        uint256 amount;
        int256 rewardDebt;
    }
    ...
    /// Also known as the amount of SHACK to distribute per block.
    struct PoolInfo {
        uint128 accShackPerShare;
        uint64 lastRewardTime;
        uint64 allocPoint;
    }

    /// @notice Address of SHACK contract.
    IERC20 public immutable SHACK;
    ...
    uint256 public shackPerSecond;
    uint256 private constant ACC_SHACK_PRECISION = 1e12;
    ...
    event LogUpdatePool(uint256 indexed pid, uint64 lastRewardTime, uint256 lpSupply, uint256 accShackPerShare);
    event LogShackPerSecond(uint256 shackPerSecond);

    /// @param _shack The SHACK token contract address.
    ...
    /// @notice Update the given pool's SHACK allocation point and `IRewarder` contract. Can only be called by the owner.
    ...
    /// @notice Sets the shack per second to be distributed. Can only be called by the owner.
    /// @param _shackPerSecond The amount of Shack to be distributed per second.
    function setShackPerSecond(uint256 _shackPerSecond) public onlyOwner {
        shackPerSecond = _shackPerSecond;
        emit LogShackPerSecond(_shackPerSecond);
    }
    ...
    /// @notice View function to see pending SHACK on frontend.
    /// @param _pid The index of the pool. See `poolInfo`.
    /// @param _user Address of user.
    /// @return pending SHACK reward for a given user.
    function pendingShack(uint256 _pid, address _user) external view returns (uint256 pending) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 shackReward = time.mul(shackPerSecond).mul(pool.allocPoint) / totalAllocPoint;
            accShackPerShare = accShackPerShare.add(shackReward.mul(ACC_SHACK_PRECISION) / lpSupply);
        }
        pending = int256(user.amount.mul(accShackPerShare) / ACC_SHACK_PRECISION).sub(user.rewardDebt).toUInt256();
    }
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.timestamp > pool.lastRewardTime) {
            ...
            if (lpSupply > 0) {
                uint256 time = block.timestamp.sub(pool.lastRewardTime);
                uint256 shackReward = time.mul(shackPerSecond).mul(pool.allocPoint) / totalAllocPoint;
                pool.accShackPerShare = pool.accShackPerShare.add((shackReward.mul(ACC_SHACK_PRECISION) / lpSupply).to128());
            }
            pool.lastRewardTime = block.timestamp.to64();
            poolInfo[pid] = pool;
            emit LogUpdatePool(pid, pool.lastRewardTime, lpSupply, pool.accShackPerShare);
        }
    }

    /// @notice Deposit LP tokens to MCV2 for SHACK allocation.
    /// @param pid The index of the pool. See `poolInfo`.
    ...
    function deposit(uint256 pid, uint256 amount, address to) public {
        ...
        user.rewardDebt = user.rewardDebt.add(int256(amount.mul(pool.accShackPerShare) / ACC_SHACK_PRECISION));

        // Interactions
        IRewarder _rewarder = rewarder[pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onShackReward(pid, to, to, 0, user.amount);
        }
    ...
    function withdraw(uint256 pid, uint256 amount, address to) public {
        ...
        // Effects
        user.rewardDebt = user.rewardDebt.sub(int256(amount.mul(pool.accShackPerShare) / ACC_SHACK_PRECISION));
        user.amount = user.amount.sub(amount);

        // Interactions
        IRewarder _rewarder = rewarder[pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onShackReward(pid, msg.sender, to, 0, user.amount);
        }
    ...
    /// @param pid The index of the pool. See `poolInfo`.
    /// @param to Receiver of SHACK rewards.
    function harvest(uint256 pid, address to) public {
        PoolInfo memory pool = updatePool(pid);
        UserInfo storage user = userInfo[pid][msg.sender];
        int256 accumulatedShack = int256(user.amount.mul(pool.accShackPerShare) / ACC_SHACK_PRECISION);
        uint256 _pendingShack = accumulatedShack.sub(user.rewardDebt).toUInt256();

        // Effects
        user.rewardDebt = accumulatedShack;

        // Interactions
        if (_pendingShack != 0) {
            SHACK.safeTransfer(to, _pendingShack);
        }

        IRewarder _rewarder = rewarder[pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onShackReward( pid, msg.sender, to, _pendingShack, user.amount);
        }

        emit Harvest(msg.sender, pid, _pendingShack);
    }

    /// @notice Withdraw LP tokens from MCV2 and harvest proceeds for transaction sender to `to`.
    /// @param pid The index of the pool. See `poolInfo`.
    /// @param amount LP token amount to withdraw.
    /// @param to Receiver of the LP tokens and SHACK rewards.
    function withdrawAndHarvest(uint256 pid, uint256 amount, address to) public {
        ...
        // Effects
        user.rewardDebt = accumulatedShack.sub(int256(amount.mul(pool.accShackPerShare) / ACC_SHACK_PRECISION));
        user.amount = user.amount.sub(amount);
    ...
    function emergencyWithdraw(uint256 pid, address to) public {
        ...
        IRewarder _rewarder = rewarder[pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onShackReward(pid, msg.sender, to, 0, 0);
        }
...
interface IRewarder {
    using BoringERC20 for IERC20;
    function onShackReward(uint256 pid, address user, address recipient, uint256 shackAmount, uint256 newLpAmount) external;
    function pendingTokens(uint256 pid, address user, uint256 shackAmount) external view returns (IERC20[] memory, uint256[] memory);
}
...
contract RewarderMock is IRewarder {
    ...
    function onShackReward (uint256, address user, address to, uint256 shackAmount, uint256) onlyMCV2 override external {
        uint256 pendingReward = shackAmount.mul(rewardMultiplier) / REWARD_TOKEN_DIVISOR;
    ...
    function pendingTokens(uint256 pid, address user, uint256 shackAmount) override external view returns (IERC20[] memory rewardTokens, uint256[] memory rewardAmounts) {
        IERC20[] memory _rewardTokens = new IERC20[](1);
        _rewardTokens[0] = (rewardToken);
        uint256[] memory _rewardAmounts = new uint256[](1);
        _rewardAmounts[0] = shackAmount.mul(rewardMultiplier) / REWARD_TOKEN_DIVISOR;
...
contract ComplexRewarderTime is IRewarder,  BoringOwnable{
    ...
    function onShackReward (uint256 pid, address _user, address to, uint256, uint256 lpToken) onlyMCV2 lock override external {
        PoolInfo memory pool = updatePool(pid);
        UserInfo storage user = userInfo[pid][_user];
        uint256 pending;
        if (user.amount > 0) {
            pending =
                (user.amount.mul(pool.accShackPerShare) / ACC_TOKEN_PRECISION).sub(
                    user.rewardDebt
        ...
        user.amount = lpToken;
        user.rewardDebt = lpToken.mul(pool.accShackPerShare) / ACC_TOKEN_PRECISION;
    ...
    /// @notice Sets the shack per second to be distributed. Can only be called by the owner.
    /// @param _rewardPerSecond The amount of Shack to be distributed per second.
    ...
    function pendingToken(uint256 _pid, address _user) public view returns (uint256 pending) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            uint256 time = block.timestamp.sub(pool.lastRewardTime);
            uint256 shackReward = time.mul(rewardPerSecond).mul(pool.allocPoint) / totalAllocPoint;
            accShackPerShare = accShackPerShare.add(shackReward.mul(ACC_TOKEN_PRECISION) / lpSupply);
        }
        pending = (user.amount.mul(accShackPerShare) / ACC_TOKEN_PRECISION).sub(user.rewardDebt).add(user.unpaidRewards);
    }
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.timestamp > pool.lastRewardTime) {
            ...
            if (lpSupply > 0) {
                uint256 time = block.timestamp.sub(pool.lastRewardTime);
                uint256 shackReward = time.mul(rewardPerSecond).mul(pool.allocPoint) / totalAllocPoint;
                pool.accShackPerShare = pool.accShackPerShare.add((shackReward.mul(ACC_TOKEN_PRECISION) / lpSupply).to128());
            }
            pool.lastRewardTime = block.timestamp.to64();
            poolInfo[pid] = pool;
            emit LogUpdatePool(pid, pool.lastRewardTime, lpSupply, pool.accShackPerShare);
...
contract MasterChefV2 is BoringOwnable, BoringBatchable {
    ...
    uint256 private constant MASTERCHEF_SHACK_PER_BLOCK = 1e20;
    uint256 private constant ACC_SHACK_PRECISION = 1e12;
    ...
    event LogPoolAddition(uint256 indexed pid, uint256 allocPoint, IERC20 indexed lpToken, IRewarder indexed rewarder);
    event LogSetPool(uint256 indexed pid, uint256 allocPoint, IRewarder indexed rewarder, bool overwrite);
    event LogUpdatePool(uint256 indexed pid, uint64 lastRewardBlock, uint256 lpSupply, uint256 accShackPerShare);
    event LogInit();

    /// @param _MASTER_CHEF The RadioShack Swap MCV1 contract address.
    /// @param _shack The SHACK token contract address.
    /// @param _MASTER_PID The pool ID of the dummy token on the base MCV1 contract.
    constructor(IMasterChef _MASTER_CHEF, IERC20 _shack, uint256 _MASTER_PID) public {
        MASTER_CHEF = _MASTER_CHEF;
        SHACK = _shack;
        MASTER_PID = _MASTER_PID;
    }

    /// @notice Deposits a dummy token to `MASTER_CHEF` MCV1. This is required because MCV1 holds the minting rights for SHACK.
    ...
    function pendingShack(uint256 _pid, address _user) external view returns (uint256 pending) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 shackReward = blocks.mul(shackPerBlock()).mul(pool.allocPoint) / totalAllocPoint;
            accShackPerShare = accShackPerShare.add(shackReward.mul(ACC_SHACK_PRECISION) / lpSupply);
        }
        pending = int256(user.amount.mul(accShackPerShare) / ACC_SHACK_PRECISION).sub(user.rewardDebt).toUInt256();
    }
    ...
    /// @notice Calculates and returns the `amount` of SHACK per block.
    function shackPerBlock() public view returns (uint256 amount) {
        amount = uint256(MASTERCHEF_SHACK_PER_BLOCK)
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            poolInfo[pid] = pool;
            emit LogUpdatePool(pid, pool.lastRewardBlock, lpSupply, pool.accShackPerShare);
        }
    }
    ...
    /// @notice Harvests SHACK from `MASTER_CHEF` MCV1 and pool `MASTER_PID` to this MCV2 contract.
...
contract ComplexRewarder is IRewarder, BoringOwnable{
    ...
    function pendingToken(uint256 _pid, address _user) public view returns (uint256 pending) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 blocks = block.number.sub(pool.lastRewardBlock);
            uint256 shackReward = blocks.mul(tokenPerBlock).mul(pool.allocPoint) / totalAllocPoint;
            accShackPerShare = accShackPerShare.add(shackReward.mul(ACC_TOKEN_PRECISION) / lpSupply);
        }
        pending = (user.amount.mul(accShackPerShare) / ACC_TOKEN_PRECISION).sub(user.rewardDebt).add(user.unpaidRewards);
    }
...
contract CloneRewarderTimeDual is IRewarder,  BoringOwnable{
    ...
    function onShackReward (uint256 pid, address _user, address to, uint256, uint256 lpTokenAmount) onlyMCV2 lock override external {
...
contract CloneRewarderTime is IRewarder,  BoringOwnable{
    ...
    function pendingToken(uint256 _pid, address _user) public view returns (uint256 pending) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            uint256 time = block.timestamp.sub(pool.lastRewardTime);
            uint256 shackReward = time.mul(rewardPerSecond);
            accToken1PerShare = accToken1PerShare.add(shackReward.mul(ACC_TOKEN_PRECISION) / lpSupply);
        }
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.timestamp > pool.lastRewardTime) {
            ...
            if (lpSupply > 0) {
                uint256 time = block.timestamp.sub(pool.lastRewardTime);
                uint256 shackReward = time.mul(rewardPerSecond);
                pool.accToken1PerShare = pool.accToken1PerShare.add((shackReward.mul(ACC_TOKEN_PRECISION) / lpSupply).to128());
            }
            pool.lastRewardTime = block.timestamp.to64();
            poolInfo[pid] = pool;
...
contract RewarderBrokenMock is IRewarder {
    ...
    function onShackReward (uint256, address, address, uint256, uint256) override external {
        revert();
    }
