...
contract Templar is Owner {
    ...
    function rand(uint256 i) public view returns (uint256) {
    ...
    function getReward(address userAddress) public view returns (uint256) {
        ...
        if (amount == 0) {
            ...
            return AllRewardPool * amount / SuperLuckyAmount;
        }
    }
    ...
    function claim() public {
        ...
        require(available.length > 0, "Insufficient permissions");

        
        (bool suc, ) = msg.sender.call{value: getReward(msg.sender)}(new bytes(0));
        require(suc, "TransferHelper: ETH_TRANSFER_FAILED");

        for (uint256 index = 0; index < available.length; index++) {
            WuFu721.transfer(DeadAddress, available[index]);
        }   
    }
}