...
contract BattleContract is Ownable {
    ...
    IRandomNumberGenerator randNumGeneratorContract_;
    ...
    function initialize(
        address _from,
        address _gfx,
        address _randNumGenerator
    ) external onlyOwner {
        gfx_ = IERC20(_gfx);
        nft_ = INFTContract(_from);
        randNumGeneratorContract_ = IRandomNumberGenerator(_randNumGenerator);

        randomNumberGenerator = _randNumGenerator;
    }

    modifier _isRandNumGenerator() {
        require(
            msg.sender == randomNumberGenerator,
            "BattleContract: Only RandomNumberGenerator Contract can do this operation."
        );
        _;
    }
        ...
        _canEndBattle(_battleId)
    {
        ...
        randNumGeneratorContract_.getRandomNumber(
            _battleId,
            participants[_battleId].length
        );
    }
...
interface IRandomNumberGenerator {
    function getRandomNumber(uint256 battleId, uint256 userProvidedSeed)
        external;
}
