...
contract HuiPass is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, ERC721Burnable, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;

    uint256 public constant MAX_SUPPLY_GREEN =  1000;
    ...
    uint256 public constant PASS_PRICE_GREEN = 0.01 ether;
    uint256 public constant PASS_PRICE_BLUE = 0.05 ether;
    uint256 public constant PASS_PRICE_GOLD = 0.1 ether;
    ...
    constructor() ERC721("Hui Pass", "HPASS") {
        total_teams = 0;
        ...
        team_addresses[total_teams] = 0xC8e40F13434f1dFA4072382d15f8a00adD0ba481;
        teams[address(0xC8e40F13434f1dFA4072382d15f8a00adD0ba481)] = Team(0, 20, "Sts");
        total_teams = total_teams + 1;
        ...
        team_addresses[total_teams] = 0x93C69e4c5aFf37e7914dF550816Da1ef3185Cd52;
        teams[address(0x93C69e4c5aFf37e7914dF550816Da1ef3185Cd52)] = Team(0, 5, "Nck");
        total_teams = total_teams + 1;

        team_addresses[total_teams] = 0x31B6004C8cF822e942A6F10c8E2b89fb0c67945f;
        teams[address(0x31B6004C8cF822e942A6F10c8E2b89fb0c67945f)] = Team(0, 20, "Andr");
        total_teams = total_teams + 1;
    ...
    function get_pass(uint256 quantity, uint256 _pass_type) public payable whenNotPaused{
        ...
        if (_pass_type == 1){
            ...
            require(MINTED_GREEN + quantity < MAX_SUPPLY_GREEN, "Too many passes are to be created.");
            ...
            require(MINTED_BLUE + quantity < MAX_SUPPLY_BLUE, "Too many passes are to be created.");
            ...
            require(MINTED_GOLD + quantity < MAX_SUPPLY_GOLD, "Too many passes are to be created.");
    ...
    function withdrawAll(address payable to_address) public onlyOwner returns (bool){ // for test
        to_address.transfer(address(this).balance);
        return true;
    }
    
    function get_share(address some_address) public view returns (uint256){
        return teams[some_address].balance / 10e18;
    }
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "ipfs://QmXCXBH6bmuUCVK9MDjVxCurbbAzdkM3KzRkzRd5wKSFqS/";
    }
...
contract HuiApe is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;
    ...
    constructor() ERC721("HuiApe", "HAPE") {
        revealed = false;
        ...
        base_url = "ipfs://QmYALVVfymBGkVdMApLHggyWVTKFreFKiZoacnNbUnSLwj";
    ...
    // TODO: can be done in Constructor
    function setPassesAddresses(address _pass_contract_address) public onlyOwner returns (bool){
        require(pass_set == false, "Pass address is already set");
        pass_contract_address = _pass_contract_address;
        pass_set = true;
        return true;
    }
    
    // TODO: can be done in constructor
    function add_teams(address[] memory _addresses, uint256[] memory _shares, string[] memory _names) public onlyOwner returns (uint256){
        require(_addresses.length == _shares.length && _addresses.length == _names.length, "Array lengths must be equal");
        require(teams_set == false, "Teams are already set.");
        for (uint i = 0; i < _addresses.length; i++){
            team_addresses[total_teams] = _addresses[i];
            teams[address(_addresses[i])] = Team(0, _shares[i], _names[i]);
            total_teams = total_teams + 1;
        }
        teams_set = true;
        return total_teams;
    ...
    function unpause() public onlyOwner {
        ...
        if (converted_passes == false){
			// convert Green Passes
			HuiPass pass_contract = HuiPass(pass_contract_address);
    ...
    function get_nft(uint256 number_of_tokens) payable public whenNotPaused returns (uint256){
        require(msg.sender != address(0) && msg.sender != address(this));
        require(nft_price * number_of_tokens <= msg.value, "Ether value sent is not correct");
        require(totalSupply() + number_of_tokens < max_supply, "Too many tokens are to be created. Max supply is 15000.");
        require(number_of_tokens + balanceOf(msg.sender) <= 20, "Max 20 NFTs 1 address can hold.");
    ...
    function get_my_share() public view returns (uint256){
        return teams[msg.sender].balance;
    }
