...
contract DojoRouter is Ownable {
    struct ETHTrade {
        address _token;
        uint256 _slippage;
    }
    ...
    mapping(address => ETHTrade) private AddressToETHTrade;
    ...
    function swapETHforTokens(
        ...
        AddressToETHTrade[address(msg.sender)] = ETHTrade(_tokenOut, _slippage);
    }

    /**
     * Claims the tokens after VRF Coordinator has given the random
     * User can also claim back ETH instead if with the Dojo contract
     */
    function claimETHTrade() external {
        require(dojo.checkClaimableTrade(msg.sender));
        //claim back the eth
        uint256 payout = dojo.claimTrade();
        //trade the Eth for token through uniswap router
        //Step 1. get reserves (eth = token A, desired = tokenB)
        (uint256 reserveA, uint256 reserveB) = UniswapV2Library.getReserves(
            address(factory),
            router.WETH(),
            AddressToETHTrade[msg.sender]._token
        );
        //Step 2. get amount out
        uint256 expectedOut = UniswapV2Library.getAmountOut(
            payout,
            reserveA,
            reserveB
        );
        //Step 3. set the min amount out
        uint256 minOut = (expectedOut *
            (1000 - AddressToETHTrade[msg.sender]._slippage)) / 1000;
        //Set the path
        address[] memory path = new address[](2);
        path[0] = router.WETH();
        path[1] = AddressToETHTrade[msg.sender]._token;
        uint256 deadline = block.timestamp + 30;
        router.swapExactETHForTokens{value: payout}(
            minOut,
            path,
            address(msg.sender),
            deadline
    ...
    /**
     * Swaps given tokens for ETH, then trades with the Hosue
     */
    function swapTokensforETH(
        ...
        //first check that they dont have a pending trade
        require(!dojo.checkPendingTrade(msg.sender));
        //Step 1. get reserves (eth = token A, desired = tokenB)
        (uint256 reserveA, uint256 reserveB) = UniswapV2Library.getReserves(
            router.WETH(),
            address(factory),
        ...
        //Step 2. get expected out
        uint256 expectedOut = UniswapV2Library.getAmountOut(
            _amount,
            reserveA,
            reserveB
        );
        //require that this is between the min and max trade size
        require(
            expectedOut > dojo.getMinimumTradeSize() &&
                expectedOut < dojo.getMaximumTradeSize()
        );
        //Step 3. set the amount min
        uint256 minOut = (expectedOut * (1000 - _slippage)) / 1000;
        address[] memory path = new address[](2);
        path[0] = AddressToETHTrade[msg.sender]._token;
        path[1] = router.WETH();
        uint256 deadline = block.timestamp + 30;
        ...
        //initate trade with dojo
        dojo.initiateTrade{value: amounts[0]}(_belt);
        //payout to be claimed directly from DegenDojo
    }
