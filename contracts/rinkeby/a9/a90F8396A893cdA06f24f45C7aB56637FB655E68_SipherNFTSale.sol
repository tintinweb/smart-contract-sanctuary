...
contract SipherNFTSale is ISipherNFTSale, Whitelist {
    ...
    uint32 public constant REDUCE_PRICE_INTERVAL = 300; //10 minutes
    uint256 public constant REDUCE_PRICE_LEVEL = 100000000000000000; //0.1 ether
    ...
    function getElapsedPublicSaleTime() public view returns (uint256) {
        ...
        if (_blockTimestamp() < config.publicTime) {
            ...
            return
                (config.publicTime <= timestamp) &&
                    (timestamp < config.privateTime)
    ...
    function getPublicSaleCurrentPrice() public view returns (uint256) {
        ...
        if (elapsed < REDUCE_PRICE_INTERVAL) {
            ...
            if (
                elapsed >= REDUCE_PRICE_INTERVAL &&
                elapsed < 6 * REDUCE_PRICE_INTERVAL
            ) {
                uint256 currentPrice = SALE_PUBLIC_STARTING_PRICE -
                    (elapsed / REDUCE_PRICE_INTERVAL) *
                    REDUCE_PRICE_LEVEL;
                ...
                uint256 currentPrice = SALE_PUBLIC_STARTING_PRICE -
                    5 *
                    REDUCE_PRICE_LEVEL -
                    (((elapsed / REDUCE_PRICE_INTERVAL) % 5) *
                        REDUCE_PRICE_LEVEL) /
                    2;
                return
                    (elapsed >= (6 * REDUCE_PRICE_INTERVAL) &&
                        elapsed < (10 * REDUCE_PRICE_INTERVAL))
                        ? currentPrice
                        : SALE_BASE_PRICE;
            }
        }
    ...
    function _validateAndUpdateWithBuyAmount(
        ...
        if (
            ...
            require(
                config.privateTime <= timestamp &&
                    timestamp < config.freeMintTime,
