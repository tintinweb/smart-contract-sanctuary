...
import "./LameloFactoryForge/contracts/IFactoryERC721.sol";

contract LameloForge is Ownable {
    address public lameloContractAddress;
    address public lameloForgeFactory;
    uint256 public forge_start = 1632280300;
    ...
    constructor(address _lameloContractAddress, address _lameloForgeFactory) {
        lameloContractAddress = _lameloContractAddress;
        lameloForgeFactory = _lameloForgeFactory;
    }
    ...
    function forge(uint16[] calldata tokenIds) public forgeActive() {
        ...
        for (uint16 i = 0; i < tokenIds.length; i++) {
            ...
            // 2 - check ownership
            if (IERC721(lameloContractAddress).ownerOf(thisId) != msg.sender) {
                revert("not owner");
            }
            // register as used
            setTokenUsed(thisId);
        ...
        Do the Give away.
        */
        IFactoryERC721(lameloForgeFactory).mint(seriesID, msg.sender);
        emit forgeWith(tokenIds[0], tokenIds[1], tokenIds[2], tokenIds[3]);
    }

    function ownerOf(uint256 _id) public view returns (address) {
        return IERC721(lameloContractAddress).ownerOf(_id);
    }
    /*
    // MetaData & URI
