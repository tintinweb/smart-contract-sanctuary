...
abstract contract ERC721LazyLite is ERC721Lazy, OwnableUpgradeable {
    ...
    LibPart.Part[] private _defaultFees;

    function __ERC721LazyLite_init_unchained() internal initializer {
        __ERC721Lazy_init_unchained();
    }

    function _setDefaultFees(address _creator, uint256 _feeBps) internal {
        LibPart.Part memory fee;
        fee.account = address(uint160(_creator));
        fee.value = _feeBps;
        _defaultFees.push(fee);
    }

    function _mintByOperator(address to, uint256 tokenId) internal {
        _mint(to, tokenId);
        _saveFees(tokenId, _defaultFees);
        creators[tokenId] = [to];
    }
...
contract ERC721RaribleLite is ERC721BaseLite {
    ...
    function setDefaultFees(address _creator, uint256 _feeBps) external {
        require(
            _msgSender() == _operator || _msgSender() == owner(),
            "ERC721RaribleLite: sender is not operator"
        );
        _setDefaultFees(_creator, _feeBps);
    }
    ...
    function mintByOperator(address to, uint256 tokenId) external {
        require(
            _msgSender() == _operator || _msgSender() == owner(),
            "ERC721RaribleLite: sender is not operator"
        );
        _mintByOperator(to, tokenId);
    }
}
