...
contract DiamondBoyzArmy is ERC721URIStorage {
    ...
    event SaleMint(address indexed tokenMinter, uint256 amount);
    ...
    function mintTokens(
        ...
        uint256 tokenLimitPerUser = 20;
        uint256 totalPrice = listingPrice * amount; 
        uint256 originalUserBalance = balanceOf(recipient);

        require(msg.value == totalPrice, "DBA: incorrect Ether value");

        require(
            _tokenIds.current() + amount <= collectionLimit,
            "DBA: Mint limit reached"
        );

        require(
            balanceOf(recipient) + amount <= tokenLimitPerUser,
            "DBA: Mint limit per user reached"
        );

        uint[] memory itemIds = new uint[](amount);
        ...
        assert(balanceOf(recipient) == originalUserBalance + amount);
    ...
    function mintTokensbyOwner(address recipient, string[] memory tokenURI, uint amount)
        public
        onlyOwner
        returns (uint256[] memory) 
    {
        uint256 tokenLimitPerOwner = 100; 
        uint256 originalUserBalance = balanceOf(recipient);
        ...
        require(
            balanceOf(recipient) + amount <= tokenLimitPerOwner,
    ...
    function setListingPrice(uint256 _newListingPrice) public onlyOwner {
        ...
        emit TokenPriceChanged(_newListingPrice);
    }
}
