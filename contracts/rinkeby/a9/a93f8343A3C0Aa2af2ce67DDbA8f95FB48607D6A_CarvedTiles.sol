...
contract CarvedTiles is ERC721Enumerable, Ownable {
    ...
    uint256 saleStartTime = 0;
    uint256 redemptionStartTime = 0;
    uint256 currentPrice = 1000000000000000000;
    uint256 maxSupply = 100;
    string currentContractURI =
        "ipfs://Qmda9yiJeKGYdx8UNi94r7dfj6KNaHNzPEoyiAhCa1VLuV";
    ...
    function mintItem(uint256 tokenId) public returns (uint256) {
    ...
    function redeemPanel(uint256 tokenId) public returns (string memory) {
        require(
            redemptions[tokenId] == false,
    ...
    /*
        To purchase a panel
            ** The panel must by owned by this contract
            ** You must send the minimum eth required
    */

    function purchasePanel(uint256 tokenId) public payable returns (uint256) {
        require(
            ownerOf(tokenId) == address(this),
            "Token must be owned by contract! Likely sold!"
        );
    ...
    */
        ...
        returns (string memory)
    {
        ...
        return string(abi.encodePacked(currentPermawebURI, tokenId.toString()));
    }
        ...
        returns (string memory)
    {
        ...
        if (redemptions[tokenId]) {
            ...
            return
                string(
                    abi.encodePacked(baseURI, "redeemed/", tokenId.toString())
                );
        } else {
            //This is not redeemed.
            return
                string(
                    abi.encodePacked(baseURI, "unredeemed/", tokenId.toString())
                );
        }
    }
}
