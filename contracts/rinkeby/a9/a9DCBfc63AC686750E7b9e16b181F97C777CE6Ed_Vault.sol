...
contract Directory is ContextUpgradeable, Common, Submissions, Uniquettes {
    ...
    function uniquetteListOnOpenSea(uint256 tokenId, uint256 price) public tokenExists(tokenId) nonReentrant {
        require(
            bytes(_fundedSubmissionHashByUniquetteTokenId[tokenId]).length > 0 ||
        ...
        uint256 effectivePrice = calculateEffectivePrice(_msgSender(), address(0), uniquette);
        require(uniquette.collateralValue <= price && price <= effectivePrice, "DIRECTORY/OUT_OF_RANGE_SALE_PRICE");
    ...
    Common
{
    ...
    virtual
    {
        ...
        return _marketer.putForSale(address(_marketer), tokenId, effectivePrice + protocolFeeAmount);
    }
