        ...
        if(lenderDetails.autoSell  == true) {
            ...
            for(uint256 i  = 0 ; i  <  loanDetails.stakedCollateralTokens.length ; i++){
                ...
                if(govClaimToken.isClaimToken(claimToken)) {
                    priceofCollateral = govPriceConsumer.getSUNTokenPrice(claimToken, loanDetails.borrowStableCoin, loanDetails.stakedCollateralTokens[i], loanDetails.stakedCollateralAmounts[i]);
                ...
                if (collateralAmountinStable <= loanDetails.loanAmountInBorrowed) {
                    ...
                    //if collateral token is then we will add the balance into our  liquidatedSUN token mapping
                    if(govClaimToken.isClaimToken(claimToken)){
                    ...
                    uint256 exceedAltcoinValue;
                     if(govClaimToken.isClaimToken(claimToken)){
                        exceedAltcoinValue = govPriceConsumer.getSUNTokenPrice(claimToken, loanDetails.stakedCollateralTokens[i], loanDetails.borrowStableCoin, loanDetails.stakedCollateralAmounts[i]);
                    } else {
                       exceedAltcoinValue = govPriceConsumer.getAltCoinPriceinStable(loanDetails.stakedCollateralTokens[i], loanDetails.borrowStableCoin, collateralAmountinStable.sub(loanDetails.loanAmountInBorrowed));
                    }
                    ...
                    if(govClaimToken.isClaimToken(claimToken)){
                        liquidatedSUNTokenbalances[lenderDetails.lender][loanDetails.stakedCollateralTokens[i]] += collateralToLender;
                    }
                    break;
                    }
                }
