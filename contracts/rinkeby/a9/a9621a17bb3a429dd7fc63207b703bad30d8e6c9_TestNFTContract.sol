    ...
    Ownable
{
    ...
    Counters.Counter private reserve_claimed;
    
    uint256 MAX_SUPPLY =  8888;
    uint256 MAX_PRESALE =  1000;
    uint256 MAX_RESERVE = 600;
    ...
    uint256 public NFT_price = 50000000000000000;

    string _name = "TEST NFT";
    string _symbol = "TNFT";
    ...
    function mintReserved(uint256 num, address _to) external onlyOwner {
        require((_currentTokenId.current() + num < MAX_SUPPLY), "Max Supply Reached");
        require(reserve_claimed.current() < MAX_RESERVE, "All Claimed");
       
        for(uint256 i=0; i<num; i++){
            uint256 newTokenId = _getNextTokenId();
            _mint(_to, newTokenId);
           _currentTokenId.increment();
            reserve_claimed.increment();
        }

    }
    function airdrop(address[] calldata _recipients) external onlyOwner {
        require(
            _currentTokenId.current() + _recipients.length < MAX_SUPPLY,
     ...
     function mintMany(uint256 num, address _to) external onlyOwner {
        require(_currentTokenId.current() + num < MAX_SUPPLY, "Max Supply Reached");
        require(num <= MAX_MINT, "Max limit exceeded.");
        for(uint256 i=0; i<num; i++){
            uint256 newTokenId = _getNextTokenId();
            _mint(_to, newTokenId);
             _currentTokenId.increment();
    ...
    function mintPresale() external payable{
        ...
        require(presaleOpen == true,"Presale not open");
        require(_currentTokenId.current()-reserve_claimed.current() < MAX_PRESALE, "Max Presale Supply Reached");
    ...
    function mintNFT(uint256 _count) external payable {
        require(saleOpen == true,"Sales not open");
         require(_currentTokenId.current()+_count < MAX_SUPPLY, "Max Supply Reached");
         require(_count <= MAX_MINT, "Above max mint limit -20");
         if (_currentTokenId.current()<MAX_PRESALE) {
             _count = 1;
         } else {
            require(msg.value >= NFT_price*_count, "Sent Amount Not Enough");
         }
