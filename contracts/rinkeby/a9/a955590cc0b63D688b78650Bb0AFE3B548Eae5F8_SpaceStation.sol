...
contract SpaceStation is ReentrancyGuard {
    ...
    event EventForgeNoStake(uint256 _cid, address _sender, uint256[] _nftIDs);
    event EventForgeWithStake(uint256 _cid, address _sender, uint256[] _nftIDs, uint256 _stakeAmount, address _erc20);

    /* ============ Modifiers ============ */
    /**
     * Throws if the address is not a validated starNFT contract
     */
    modifier onlyStarNFT(address _starNFTAddress)  {
    ...
    // Set that contains all validated starNFT contract addresses. TODO: PRIVATE ME
    mapping(address => bool) public _starNFTs;

    // Mapping that records fees totals owned by galaxy-treasury,
    // Separate from escrow backed-asset from users.
    // map(erc20=>balance) TODO: PRIVATE ME
    uint256 public _galaxy_treasury_network;
    mapping(address => uint256) public _galaxy_treasury_erc20;
     ...
     * NOTE: should only reset non-stake campaigns. TODO: TEST if the refunded gas is larger than cost.
     */
    function expireCampaign(uint256 _cid, Operation[] calldata _op) external {
...
//        require(_minStakeAmount >= 0, "Min stake amount should be greater than 0 for stake campaign");
    ...
    function claim(uint256 _cid) external payable {
        _payFees(_cid, Operation.Claim);
        emit EventClaim(_cid, msg.sender);
    }

    function stakeIn(uint256 _cid, uint256 stakeAmount) external payable nonReentrant {
    ...
    // CALL STAR_NFT*
    function stakeOutQuasar(address _starNFT, uint256 _nftID) external payable onlyStarNFT(_starNFT) nonReentrant {
        require(IStarNFT(_starNFT).isOwnerOf(msg.sender, _nftID), "Must be owner of this Quasar NFT");
        // 1.1 get info, make sure nft has backing-asset
        (uint256 _mintBlock, IERC20 _stakeToken, uint256 _amount, uint256 _cid) = IStarNFT(_starNFT).quasarInfo(_nftID);
        ...
        if (campaignStakeConfigs[_cid].burnRequired) {
            IStarNFT(_starNFT).burn(msg.sender, _nftID);
        } else {
            IStarNFT(_starNFT).burnQuasar(msg.sender, _nftID);
        }
        emit EventStakeOut(_starNFT, _nftID);
    }

    function stakeOutSuper(address _starNFT, uint256 _nftID) external payable onlyStarNFT(_starNFT) nonReentrant {
        require(IStarNFT(_starNFT).isOwnerOf(msg.sender, _nftID), "Must be owner of this Super NFT");
        // 1.1 get info, make sure nft has backing-asset
        ...
        if (campaignStakeConfigs[_cid].burnRequired) {
            ...
            IStarNFT(_starNFT).burnSuper(msg.sender, _nftID);
        }
        emit EventStakeOut(_starNFT, _nftID);
    }

    function forgeNoStake(uint256 _cid, address _starNFT, uint256[] calldata _nftIDs) external payable onlyStarNFT(_starNFT) nonReentrant {
        _payFees(_cid, Operation.Forge);
        // burn ingredients TODO: move to backend? user lost backing-asset here if backend somehow determines this is a bad recipe.
        IStarNFT(_starNFT).burnBatch(msg.sender, _nftIDs);
        emit EventForgeNoStake(_cid, msg.sender, _nftIDs);
    }

    function forgeStake(uint256 _cid, address _starNFT, uint256[] calldata _nftIDs, uint256 stakeAmount) external payable onlyStarNFT(_starNFT) nonReentrant {
        _payFees(_cid, Operation.Forge);
        _stakeIn(_cid, stakeAmount);
        // burn ingredients TODO: move to backend? user lost backing-asset here if backend somehow determines this is a bad recipe.
        IStarNFT(_starNFT).burnBatch(msg.sender, _nftIDs);
        emit EventForgeWithStake(_cid, msg.sender, _nftIDs, stakeAmount, campaignStakeConfigs[_cid].erc20);
    }
     ...
     */
    function addValidatedStarNFTAddress(address _starNFT) external onlyManager {
        require(_starNFT != address(0), "Validate StarNFT contract must not be null address");
        _starNFTs[_starNFT] = true;
    }
    /**
     * PRIVILEGED MODULE FUNCTION. Function that removes a validated starNFT address.
     */
    function removeValidatedStarNFTAddress(address _starNFT) external onlyManager {
        require(_starNFT != address(0), "Invalidate StarNFT contract must not be null address");
    ...
    function networkWithdraw() external onlyTreasuryManager {
        ...
        uint256 amount = _galaxy_treasury_network;
        require(amount > 0, "Treasury of network should be greater than 0");

        // modify state first
        _galaxy_treasury_network = 0;
    ...
    function erc20Withdraw(address erc20) external onlyTreasuryManager nonReentrant {
        ...
        uint256 amount = _galaxy_treasury_erc20[erc20];

        // modify state first
        _galaxy_treasury_erc20[erc20] = 0;
        // send erc20 fees to manager
        require(IERC20(erc20).approve(address(this), amount), "Failed to approve token allowance");
        require(IERC20(erc20).transferFrom(address(this), manager, amount), "Failed to send Erc20 fees to treasury manager");
    }
     ...
     *
     */
    function stakeOutInfo(address _starNFTAddress, uint256 _nft_id) external onlyStarNFT(_starNFTAddress) view returns (
        bool _allowStakeOut,
        uint256 _allowBlock,
        bool _requireBurn,
        uint256 _earlyStakeOutFine,
        ...
        (uint256 _createBlock, IERC20 _stakeToken, uint256 _amount, uint256 _cid) = IStarNFT(_starNFTAddress).quasarInfo(_nft_id);
    ...
    function superStakeOutInfo(address _starNFTAddress, uint256 _nft_id) external onlyStarNFT(_starNFTAddress) view returns (
        bool _allowStakeOut,
        uint256 _allowBlock,
        bool _requireBurn,
        uint256 _earlyStakeOutFine,
        ...
        (uint256 _createBlock, IERC20[] memory _stakeToken, , uint256 _cid) = IStarNFT(_starNFTAddress).superInfo(_nft_id);
    ...
    function _setStake(
        ...
        require(_erc20 != address(0), "Invalid ERC20 token address");
    ...
    function _payFees(uint256 _cid, Operation _op) private {
        ...
        if (feeConf.networkFee > 0) {
            ...
            _galaxy_treasury_network = _galaxy_treasury_network.add(msg.value);
        }
        ...
        if (feeConf.erc20Fee > 0) {
            ...
            _galaxy_treasury_erc20[feeConf.erc20] = _galaxy_treasury_erc20[feeConf.erc20].add(feeConf.erc20Fee);
        }
    }
    ...
    function _payFine(uint256 _cid, uint256 _mintBlock) private {
        ...
        if (block.number < _mintBlock + lockBlockNum) {
            if (campaignStakeConfigs[_cid].isEarlyStakeOutAllowed) {
                ...
                _galaxy_treasury_network = _galaxy_treasury_network.add(_fine);
            } else {
