...
contract WalletFactory is Managed {
  ...
  function getAddressForCounterfactualWallet(
    ...
    bytes32 hash = keccak256(
      abi.encodePacked(bytes1(0xff), address(this), newsalt, keccak256(code))
    );
  ...
  function newSalt(
    ...
    return
      keccak256(
        abi.encodePacked(
          keccak256(abi.encodePacked(_owner, _modules, _guardian)),
          _salt
        )
      );
  }
...
contract Managed is Owned {
  ...
  function revokeManager(address _manager) external virtual onlyOwner {
    ...
    require(
      managers[_manager] == true,
      'M: Target must be an existing manager'
    );
