...
// preoccupied running around accumulating stuff, but you can bait them 
// with ETH and rescue them. They need to be caught and rescued soon!
// Bait is denominated in ETH (/wei) and rises in Fibonnacci increments.
...
contract Derpys is ERC721, ERC721Enumerable, ERC721Pausable, Ownable {
	...
	// Check the ipfs provenance hash here when all derpys are rescued
	...
	// get the rescue cost in wei for the next loose derpy
	function getRescueCost() public view returns (uint256) {
		//require(saleIsLive == true, "The sale is not live!");
    ...
    function rescueDerpy(uint256 numDerpys) public payable whenNotPaused {
    	...
    	require(numDerpys > 0 && numDerpys <= 50, "You can rescue between 1 and 50 derpys");
    	require(currentSupply() + numDerpys <= MAX_DERPYS, "There aren't that many Derpys left!");
    	require(msg.value >= getRescueCost() * numDerpys, "The Ether value sent is less than the needed rescue price!");
    ...
    function listMyDerpys(address owner_) external view returns (uint256[] memory) {
    	...
    	if (nDerpys == 0) {
    		//uint256[] memory derpList = new uint256[](0);
    		//return derpList;
    ...
    function startRescue() public onlyOwner whenPaused {
    	require(bytes(baseURI).length > 0, "Metadata must be uploaded and its location must be set via setBaseURI()");
    	_unpause();
    }

    function stopRescue() public onlyOwner whenNotPaused {
    	_pause();
    }

    function withdrawRescueBounty() public payable onlyOwner {
