...
contract Mikan is  ERC721URIStorage {
    ...
    mapping(uint256 => string) additionalURIMap;

    mapping(uint256 => uint256) storageChannel;
    

    enum selectedStorageByOwner { IPFS , ARWEAVE ,  ADD }
    ...
    function toggleStorageByOwner( uint256 _id ) public {
        require(_msgSender() == ownerOf(_id));
        storageChannel[_id] = (storageChannel[_id] + 1 ) % 3;
        selectedStorageByOwnerMap[_id] = selectedStorageByOwner(storageChannel[_id]);
    }
    ...
    function setAdditionalURIMap(uint256 _id , string memory _add) public {
        require( _msgSender() == creator || _msgSender() == staff1 || _msgSender() == staff2 );
        additionalURIMap[_id] = _add;
    }
    ...
    function tokenURI(uint256 _id) override(ERC721URIStorage) public view returns (string memory) {
        if ( selectedStorageByOwnerMap[_id] == selectedStorageByOwner.IPFS ) {
            ...
            return arweaveURIMap[_id];  
        } else if (selectedStorageByOwnerMap[_id] == selectedStorageByOwner.ADD){
            return additionalURIMap[_id];
        } 
