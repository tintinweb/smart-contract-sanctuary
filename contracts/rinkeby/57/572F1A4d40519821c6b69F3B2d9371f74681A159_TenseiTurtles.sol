...
interface IERC1155Proxy{
  function burnFrom( address account, uint[1] calldata ids, uint[1] calldata quantities ) external payable;
}
...
contract TenseiTurtles is ERC721B, Delegated, IERC721Batch, IERC721Enumerable, PaymentSplitterMod {
  ...
  uint32 public COOLDOWN_META   = 259200; // 3 days
  uint32 public COOLDOWN_TENSEI = 259200;
  ...
  uint public leafEvolveQuantity = 210 ether;
  ...
  function ownerOf( uint tokenId ) public override( IERC721, ERC721B ) view returns( address owner_ ){
    ...
    require(owner != address(0), "TENSEI: owner query for nonexistent token");
    return owner;
  }
  ...
  function claimLeaves( uint[] calldata tokenIds ) external {
    ...
    for( uint i; i < tokenIds.length; ++i ){
      ...
      if( tokenLeaves > 0 ){
        owners[ i ] = turtles[ tokenIds[i] ].owner;
  ...
  function stake( uint[] calldata tokenIds ) external {
    ...
    for( uint i; i < tokenIds.length; ++i ){
      require(turtles[tokenIds[i]].owner == msg.sender, "TENSEI: staking token that is not owned");
      turtles[ tokenIds[i] ].lastStake = time;
    }
  }
  ...
  function mint( uint quantity ) external payable {
    if( isMintActive ){
      ...
      require( accessList[ msg.sender ], "TENSEI: Account is not on the access list" );
    }
    else{
  ...
  function evolve( uint[] calldata tokenIds ) external payable {
    require( isEvolveActive, "TENSEI: Evolution is not active" );
    require( flaskAddress != address(0), "TENSEI: Flask contract unset" );

    if( flaskQuantity > 0 )
      IERC1155Proxy( flaskAddress ).burnFrom( msg.sender, [ flaskToken ], [ tokenIds.length * flaskQuantity ] );
  ...
  function breed( uint turtleA, uint turtleB ) external payable{
    ...
    //verify cooldown
    uint32 time = uint32(block.timestamp);
    require( time - tensei.lastBreeding > COOLDOWN_TENSEI, "TENSEI: Breeding cooldown active" );
    require( time - meta.lastBreeding > COOLDOWN_META, "TENSEI: Breeding cooldown active" );
    ...
    tensei.lastBreeding = uint32(block.timestamp);
    meta.lastBreeding = uint32(block.timestamp);
    _mint( msg.sender, supply++, TurtleType.Hybrid );
    emit Breed( msg.sender, supply );
  }


  //onlyDelegates
  function mintTo(uint[] calldata quantity, address[] calldata recipient, TurtleType[] calldata types_ ) external payable onlyDelegates{
    ...
    for(uint i; i < recipient.length; ++i){
      for(uint j; j < quantity[i]; ++j){
        _mint( recipient[i], supply++, types_[i] );
      }
    }
  }

  function evolve_(uint[] calldata tokenIds) external payable onlyDelegates{
    IERC1155Proxy( flaskAddress ).burnFrom( msg.sender, [ flaskToken ], [ tokenIds.length ]);
  ...
  function setAccessList(address[] calldata accounts, bool[] calldata allowed) external onlyDelegates{
  ...
  function setStaking( uint32 period, uint tenseiLeaf, uint metaLeaf, uint hybridLeaf ) external onlyDelegates{
    STAKE_PERIOD = period;
  ...
  function setTurtle(uint[] calldata tokenIds, TurtleType[] calldata types,
    uint32[] calldata lastBreeds, uint32[] calldata lastStakes ) external onlyDelegates {

    Turtle storage turtle;
    for(uint i; i < tokenIds.length; ++i ){
      turtle = turtles[tokenIds[i]];
      turtle.turtleType   = types[i];
      turtle.lastBreeding = lastBreeds[i];
