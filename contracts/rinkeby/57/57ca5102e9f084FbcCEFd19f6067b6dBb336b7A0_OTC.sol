    ...
    */
    ...
    function executeTransaction(uint _buyOrderId, uint _sellOrderId) private {
        ...
        if (buyOrder.anchorAmount > sellPaprAmount.mul(transactionPrice)) { // limitant is papr
            ...
            uint _let = anchorAmount.div(transactionPrice);
            txFees = _let.mul(totalSellFees).div(1000); // we need to re set txFees with the real fees because paprAmount is less than sellPaprAmount
            paprAmount = _let.sub(txFees); // papr amount that will be send (with fees)
