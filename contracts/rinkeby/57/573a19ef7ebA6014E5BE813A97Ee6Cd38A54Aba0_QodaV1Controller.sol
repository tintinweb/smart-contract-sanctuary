...
contract QodaV1Controller is ErrorCode {
  ...
  function addAsset(address tokenAddress, address _oracleFeed) public{
  ...
  function depositCollateral(address tokenAddress, uint amount) external returns(uint){
    ...
    transferFrom(tokenAddress, msg.sender, address(this), amount);
  ...
  function getPriceFeed(address oracleFeed) public view returns(uint80, int256, uint256, uint256, uint80){
