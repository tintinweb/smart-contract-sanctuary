...
contract OlympusSalesLite {
    ...
    address public daiDistributor;
    address public vault;

    bool public salesEnabled;
    bool public useStakingDistributor;

    constructor( 
        address OHM_, 
        address DAI_, 
        address stakingDistributor_, 
        address daiDistributor_,
        address vault_, 
    ...
    function makeSale() external returns ( bool ) {
        ...
        IVault( vault ).depositReserves( IERC20( DAI ).balanceOf(address(this)), DAI ); // Mint OHM
        ...
        if ( useStakingDistributor ) { 
            StakingDistributor( stakingDistributor ).distribute(); // Distribute epoch rebase
        } else { // if dai distributor still in use
            DAIDistributor( daiDistributor ).distribute();
        }
        return true;
    }
    ...
    function switchToStakingDistrbutor() external returns ( bool ) {
        require( msg.sender == owner, "Only owner" );
        useStakingDistributor = true;
        return true;
    }
