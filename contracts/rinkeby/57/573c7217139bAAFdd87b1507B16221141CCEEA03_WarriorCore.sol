...
contract WarriorCore is OriginControl, WarriorGeneration {
    ...
    address public feeReceiver; // address of fee receiever

    uint256 public mintFee = 8 * (10**16);
    ...
    function generateWarrior(
    ...
    ) public populationCheck payable onlyOrigin(_owner, _metadata, _originSignature) {
        require(
            areAssetsRegistered,
        ...
        require(
            msg.value == mintFee,
            "WarriorCore: transfer correct mint fee"
        );
     ...
     */
    function registerAssets(uint256 _totalLayers, bytes32 _assetsCid) external onlyAdmin {
        require(
     ...
     * @param _mintFee new mint fee in 10^18 denomination
     */
    ...
    function setFeeReceiver(address _feeReceiver) external onlyAdmin {
        require(
            _feeReceiver != address(0),
            "WarriorCore: fee receiver address cannot be zero"
        );
        feeReceiver = _feeReceiver;
    }
}
...
contract WarriorPopulation is Warriors{
    ...
    uint256 public maxPopulationPerGen = 50000;
    ...
    uint256 public growthRate = 395;   // 3.95
