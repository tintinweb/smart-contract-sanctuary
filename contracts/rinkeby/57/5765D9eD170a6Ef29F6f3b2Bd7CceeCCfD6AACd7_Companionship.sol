...
contract Companionship is Ownable, IERC721Receiver, ReentrancyGuard, Pausable {
  ...
  function deposit(uint256[] calldata tokenIds) external whenNotPaused {
    ...
    for (uint256 i; i < tokenIds.length; i++) {
      IERC721(compboxAddress).safeTransferFrom(msg.sender, address(this), tokenIds[i], "");
  ...
  function withdraw(uint256[] calldata tokenIds) external whenNotPaused nonReentrant() {
    ...
    for (uint256 i; i < tokenIds.length; i++) {
      ...
      IERC721(compboxAddress).safeTransferFrom(address(this), msg.sender, tokenIds[i], "");
    }
  }
