    ...
    
{
    ...
    constructor(
        ...
        uint256 _transvalue
    ) ERC721("ART85", "AIDITEST") {
        setBaseURI("http://3.23.170.164:2053/opensearoute/gettokendetails/");
        _tid = id;
 ...
 function getServiceFee() public view returns(uint256){
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "http://3.23.170.164:2053/opensearoute/gettokendetails/";
    }
    ...
    function mint(
        ...
        token_id[tokenId] = Metadata(name, ipfsimage);
    ...
    function checkTokenApproval(uint256 tokenId, address from)
        internal
        view
        returns (bool result)
    {
    ...
    // token ID -- token id (counts in db )
    // amount -- the price in which about to buy , without service fees
    // token -- singel or multiple contract address
    // _type-- 721 / 1155
    // NOFT -- token no
    ...
    function tokenTrans(
     ...
     //  biddtoken == token symbol (payment)
    // bidaddres == bidders address
    //  amount -- without service fee 
    // tokenId == token id 
    // token ==   721 / 111 addresss
    // type == 721/1155  
    //  nNOFToken -- nod od token 
    
    
    
     function acceptBId(address bidtoken,address bidaddr, uint256 amount, uint256 tokenId) public{
    ...
    // token ass --- token symbol 
    
    
    
    function _acceptBId(address tokenAss,address from, address admin, uint256 amount, uint256 tokenId) internal{
        // require(_operatorApprovals[tokenId], "Token Not approved");
    ...
    struct UserInfo { uint256 amount; }
    mapping(address => UserInfo) public userInfo;

    function depositToken(address tokenAs, uint256 amount) public{
        UserInfo storage user = userInfo[msg.sender];
        _transferToken(tokenAs, msg.sender, address(this), amount);
        user.amount += amount;
    }
    function withdrawToken(address tokenAs, uint256 amount) public{
        require(owner() == _msgSender(), "Ownable: caller is not the owner");
        UserInfo storage user = userInfo[msg.sender];
        _transferToken(tokenAs, address(this), msg.sender, amount);
        user.amount -= amount;
    }
    function _transferToken(address tokenAs, address from, address to, uint256 amount) internal{
        ERC20 tok = ERC20(tokenAs);
