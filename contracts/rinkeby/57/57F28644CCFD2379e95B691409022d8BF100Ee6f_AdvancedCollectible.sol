...
contract AdvancedCollectible is ERC721, Ownable {
    ...
    uint256 private price = 100000000000000; // 0.0001 Ether
    bool public paused = false;

    constructor () public ERC721 ("Rainbow", "RNBW"){
    }
    ...
    function tokenURI(uint256 token_id) public view override returns (string memory) {
        return string(abi.encodePacked(baseURI(), "/", token_id.toString(), ".json"));
    }

    function reservemint(address _to, uint256 _count) public onlyOwner{
        uint supply = totalSupply();
        require(supply + _count < MAX_TOKENS + 1, "Exceeds maximum tokens available for purchase");
        for (uint i = 0; i < _count; i++) {
            _safeMint(_to, supply + i);
        }
    }

    function salemint(uint256 _count) public payable {
        require(!paused, "Sale is not yet active");
