...
contract QodaV1Controller is ErrorCode {
  ...
  function getBorrowValue(address tokenAddress, address account) public view returns(uint){
  ...
  function _getDepositValue(address account) internal view returns(uint){
    ...
    for(uint i=0; i<accountAssets[account].length; i++){
      ...
      if(asset.oracleFeed == address(0)){
        ...
        uint value = _getValueInUSD(tokenAddress, asset.oracleFeed, balanceInLocal);
  ...
  /// @param oracleFeed Address of the chainlink oracle feed
  /// @param valueInLocal Amount, denominated in terms of the ERC20 token
  function _getValueInUSD(
    ...
    // Convert `valueInLocal` to USD
    uint valueInUSD = valueInLocal.mul(exchRate).mul(stablecoinMantissa);

    // Divide by mantissas last for maximum precision
    valueInUSD = valueInUSD.div(tokenMantissa).div(exchRateMantissa);
