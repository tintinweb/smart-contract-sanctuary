...
interface ICompoundProxyUserTemplate {
    function init( address _op, address _treasuryFund, bytes16 _lendingId, address _user, address _rewardComp ) external;
    ...
    function repayBorrowBySelf(address _asset, address _liquidatePool,address _underlyingToken) external;
...
interface ICompoundPoolFactory {
    function CreateCompoundRewardPool(address rewardToken, address op) external returns (address);
    function CreateCompoundInterestRewardPool(address rewardToken, address op) external returns (address);
...
contract CompoundProxyUserTemplate {
    ...
    function repayBorrowBySelf(
        ...
        if (_underlyingToken != address(0)) {
            ...
            uint256 surplusBal = IERC20(_underlyingToken).balanceOf(
                address(this)
            );
            IERC20(_underlyingToken).safeTransfer(_liquidatePool, surplusBal);
        } else {
    ...
    function repayBorrowErc20(
        ...
        IERC20(_token).safeApprove(_asset, 0);
        IERC20(_token).safeApprove(_asset, _amount);
        ...
        if (bal > 0) {
            IERC20(_token).safeTransfer(op, bal);
        }
    ...
    function recycle(address _asset, address _underlyingToken)
        external
        onlyInited
        onlyOp
    {
        ...
        if (borrows == 0) {
            if (_underlyingToken != address(0)) {
                ...
                IERC20(_underlyingToken).safeTransfer(user, surplusBal);
            } else {
                if (address(this).balance > 0) {
...
contract CompoundBooster is Ownable {
    ...
    address public veLendFlareReward;
    ...
    function demoInit() public {
        init(
            0xCc1Ad05F58869F8c01944f6514BaF4b8fe484233,
    ...
    function init(
        ...
        address _lendflareDepositer,
        address _veLendFlareReward
    ) public {
        ...
        veLendFlareReward = _veLendFlareReward;

        compoundProxyUserTemplate = address(new CompoundProxyUserTemplate());
    }

    function Demo() public {
        addPool(0xd6801a1DfFCd0a410336Ef88DeF4320D6DF1883e, false); //  cEther
        addPool(0x5B281A6DdA0B271e91ae35DE655Ad301C976edb1, true); //  cUsdc
        addPool(0x6D7F0754FFeb405d23C51CE938289d4835bE3b14, true); //  cDai

        setCompoundComptroller(0x2EAa9D77AE4D8f9cdD9FAAcd44016E746485bddb);
    }
        ...
        returns (bool)
    {
        ...
        // address rewardCompPool = address(
        //     new CompoundRewardPool(rewardComp, address(this))
        // );
        address rewardCompPool = ICompoundPoolFactory(compoundPoolFactory)
            .CreateCompoundRewardPool(rewardComp, address(this));
        address rewardLendflareTokenPool = ICompoundPoolFactory(
            compoundPoolFactory
        ).CreateCompoundRewardPool(lendflareToken, address(this));
        ...
        if (_isErc20) {
            ...
            interestRewardPool = ICompoundPoolFactory(compoundPoolFactory)
                .CreateCompoundInterestRewardPool(underlyToken, address(this));
        } else {
            interestRewardPool = ICompoundPoolFactory(compoundPoolFactory)
                .CreateCompoundInterestRewardPool(address(0), address(this));
        }
    ...
    function deposit(
        ...
        ICompRewardPool(pool.rewardCompPool).stakeFor(_msgSender(), mintToken);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public returns (bool) {
        ...
        uint256 depositAmount = ICompRewardPool(pool.rewardCompPool).balanceOf(
            _msgSender()
        );
        ...
        ICompRewardPool(pool.rewardCompPool).withdrawFor(_msgSender(), _amount);
    ...
    function borrow(
        ...
        // lendingInfo.utilizationRate = getUtilizationRate(_pid);
        // lendingInfo.borrowRatePerBlock = getBorrowRatePerBlock(_pid);
    ...
    function liquidate(bytes16 _lendingId) external returns (address) {
        ...
        ICompoundProxyUserTemplate(lendingInfo.proxyUser).repayBorrowBySelf(
            pool.lpToken,
            veLendFlareReward,
            lendingInfo.underlyToken
        );
