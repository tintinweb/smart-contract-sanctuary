...
interface ICloneablePaymentSplitter is IERC165 {
    ...
    function pendingPayment(address account) external returns (uint256);
    function pendingPayment(IERC20 token, address account) external returns (uint256);
}
...
contract CloneablePaymentSplitter is Context, ERC165, ICloneablePaymentSplitter {    
    ...
    function pendingPayment(address account) public virtual override returns (uint256) {
        uint256 totalReceived = address(this).balance + totalReleased();
        uint256 payment = _pendingPayment(account, totalReceived, released(account));
        return payment;
    }

    function pendingPayment(IERC20 token, address account) public virtual override returns (uint256) {
        uint256 totalReceived = token.balanceOf(address(this)) + totalReleased(token);
        uint256 payment = _pendingPayment(account, totalReceived, released(token, account));
        return payment;
    }
