...
contract SpaceCoin is ERC20 {
  ...
  constructor(address _ico, address _treasury) ERC20("Space", "SPC") {
    uint icoAmount = SpaceLib.ONE_COIN * 30000 * 5; // 5:1 ratio for 30k ether
  ...
  function _transfer(address sender, address recipient, uint amount) internal virtual override {
    ...
    if (shouldTax) {
      // In general: Increase before decreasing values when doing math, to avoid losing precision.
      // In general: Check your number upper bound before increasing values.
  ...
  //   console.log('balance', balanceOf(sender));
  //
...
library SpaceLib {
  ...
  uint constant MAX_COINS = 500000;
}
