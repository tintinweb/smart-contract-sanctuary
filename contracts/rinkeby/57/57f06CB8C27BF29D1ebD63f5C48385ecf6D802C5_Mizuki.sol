...
contract Mizuki is ERC721, ERC721Enumerable, VRFConsumerBase, Ownable {
    ...
    function expandRandomNumber(uint256 n)
        internal
        view
    ...
    function generateMetadata(uint16[][10] memory rarities) public onlyOwner {
        require(randomNumber > 0, "Random number is not ready yet");

        for (uint256 i = 0; i < 10; i++) {
            require(rarities[i].length > 0, "The rarities data is invalid");
        }

        uint256[] memory expandedRandomNumbers = expandRandomNumber(
            maxSupply * 10
        );
        ...
        for (uint256 i = 0; i < maxSupply; i++) {
            ...
            for (uint256 j = 0; j < 10; j++) {
                uint256 randomTrait = expandedRandomNumbers[i * 10 + j].mod(
                    rarities[j].length
                );

                while (rarities[j][randomTrait] == 0) {
                    randomTrait = (++randomTrait).mod(rarities[j].length);
                }

                if (rarities[j].length < 0x10) {
                    traits = (uint64)(traits.mul(2**4).add(randomTrait));
                } else {
                    traits = (uint64)(traits.mul(2**8).add(randomTrait));
                }

                rarities[j][randomTrait]--;
            }

            metadata[i] = traits;
        }

        // Again shuffle metadata array
        for (uint256 i = 0; i < maxSupply; i++) {
            uint256 n = i +
                (uint256(keccak256(abi.encodePacked(block.timestamp))) %
                    (maxSupply - i));
            uint64 temp = metadata[n];
