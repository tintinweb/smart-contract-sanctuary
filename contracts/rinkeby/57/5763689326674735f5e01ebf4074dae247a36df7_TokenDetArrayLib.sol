...
contract BrokerV2 is ERC721Holder, BrokerModifiers {
        ...
        activeAuction(tokenID, _mintableToken)
    {
        ...
        if (_auction.erc20Token == address(0)) {
            require(msg.value >= amount, "amount is not less  than msg value");

            offerprice[msg.sender][tokenID] = OfferDetails(tokenID, amount);
            offererAddressArray.push(msg.sender);
        } else {
        ...
        activeAuction(tokenID, _mintableToken)
    {
        ...
        uint256 stackdeepFixamount = amount;
        address stackDeepFixAddress = slectedAddress;
        address _mintableTokenstackdeepfix = _mintableToken;

        require(
            stackdeepFixamount ==
                offerprice[slectedAddress][stackdeepFixtokenID].amount,
            "selected candidate amount not match "
        );
        if (
            offerprice[slectedAddress][stackdeepFixtokenID].tokenId ==
            stackdeepFixtokenID
        ) {
            ...
            uint256 royality = (royalities * stackdeepFixamount) / 10000;
            uint256 brokerageAmount = (brokerage * stackdeepFixamount) / 10000;

            uint256 lastOwner_funds = stackdeepFixamount -
                royality -
                brokerageAmount;
            ...
            if (_auction.erc20Token == address(0)) {
                ...
                for (
                    uint256 index = 0;
                    index < offererAddressArray.length;
                    index++
                ) {
                  
                   address  _revertAddress =  address( uint160(offererAddressArray[index])) ;
                    if (stackDeepFixAddress != _revertAddress ) {
                      
                          _revertOffer(
                                address( uint160(offererAddressArray[index])),
                                stackdeepFixtokenID
                            );
            ...
            // address stackDeepFix2 = _auction.erc20Token;
            // uint256 stackDeepFix3 = _auction.buyPrice;
            ...
            delete auctions[_tokenDet.NFTAddress][_tokenDet.tokenID];
               delete offerprice[stackDeepFixAddress][
                            stackdeepFixtokenID
                        ];
                        delete  offererAddressArray;
        }
    }

    function _revertOffer(address payable revertAddress, uint256 tokenID)
        private
    {
        for (uint256 index = 0; index < offererAddressArray.length; index++) {
            if (revertAddress == offererAddressArray[index]) {
                revertAddress.transfer(
                    offerprice[revertAddress][tokenID].amount
                );
                offererAddressArray[index] = offererAddressArray[
                    offererAddressArray.length - 1
                ];
                offererAddressArray.pop();
                delete offerprice[revertAddress][tokenID];
            }
        }
    }

    function revertOffer(address payable revertAddress, uint256 _tokenID)
        public
    {
        _revertOffer(revertAddress, _tokenID);
    }

    function revertOffererc20Token(
        address payable revertAddress,
        uint256 _tokenID,
        address _mintableToken
    ) public {
        _revertOffererc20Token(revertAddress, _tokenID, _mintableToken);
    }

    function _revertOffererc20Token(
