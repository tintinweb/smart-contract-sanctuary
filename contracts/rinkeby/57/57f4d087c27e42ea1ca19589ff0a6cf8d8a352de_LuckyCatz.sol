...
contract LuckyCatz     is  ERC721Enumerable 
{
    ...
    uint256 public     salesDate          = 1634482800;
    uint256 public     salesPrice         = 0.07 ether;

    uint256 public     totalTokens        = 7777;
    uint256 public     leftTokenCount     = totalTokens;
    uint256 public     mintedTokenCount   = 0;
    uint256 public     maxMintPerWallet   = 7;
    
    string  public     baseURI = 'https://ipfs.io/ipfs/QmUaTJ3LvTr9FGVBDLwEi9nuyJwA2vK2tmVwXQYPbWRw4w/';

	address public		ownerWallet;

    uint256 public 	maxReservableTokenCount = 777; 	//total reserved token for rewards and wages
    uint256 public     totalReserved           = 0;

    mapping(address => uint256) public walletMintCounts;
    mapping(address => uint256) public walletMintedTokenIds;
	...
	function    isERC721ReceivedCheck(address from,address to,uint256 tokenId,bytes memory _data) public returns (bool) 
	{
        if (to.isContract()) 
        {
            try
    ...
    function    reserveSomeTokens(uint256 amount) payable external onlyOwner 
    {
