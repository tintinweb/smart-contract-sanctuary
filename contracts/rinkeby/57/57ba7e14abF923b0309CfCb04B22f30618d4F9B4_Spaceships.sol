...
contract Spaceships is ERC721 {
    ...
    function bulkMint(uint[] memory tokenIDs, address[] memory receivers) external onlyOwner {
        require(_totalSupply + tokenIDs.length < max_supply, "INVADERS: max supply is reached") ; 

        for (uint i = _totalSupply; i < _totalSupply + tokenIDs.length; i++) {
            _mint(receivers[i], i) ; 
        }
