...
contract WSBToken is IERC721Receiver, ReentrancyGuard, Ownable, WSBSwappable {
  ...
  uint16 public constant MAX_RAFFLE = 1500;
  uint16 public constant MAX_TOKEN = 7400;
  ...
  uint256 public constant BUY_PRICE = 0.125 ether;

  //uint16[] public tokensForClaim; // for random presale

  uint16 private publicTokens = 1500;
  ...
  function claim(uint256 amount) external payable nonReentrant {
    ...
    require(uint8(presaleContract.balanceOf(msg.sender)) == 0, "ClaimToken: You have raffle tokens unclaimed.");
    require(amount > 0, "ClaimToken: Need to claim at least 1.");
    require(balanceOf(msg.sender).add(amount) <= MAX_PER_ACCOUNT, "ClaimToken: Cannot own more than 5 tokens.");
    ...
    for(uint16 i = 0; i < amount; i++){
      ...
      /* _safeMint(msg.sender, getNextTokenForClaim()); */
    }
  }

  /*
  function getNextTokenForClaim() private returns (uint16) {
    require(tokensForClaim.length > 0, "GetNextTokenForClaim: All token are claimed.");
    uint16 pos = uint16(uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, block.coinbase, msg.sender, tokensForClaim.length, address(this)))) % tokensForClaim.length);
    uint16 token = tokensForClaim[pos];
    tokensForClaim[pos] = tokensForClaim[tokensForClaim.length - 1];
    tokensForClaim.pop();
    return token;
  }

   // to deploy if non sequential claims are required
  function createClaimTokens() external onlyOwner {
    for (uint16 i = MAX_RAFFLE; i < MAX_TOKEN; i++) {
      tokensForClaim.push(i);
    }
  }
  */
