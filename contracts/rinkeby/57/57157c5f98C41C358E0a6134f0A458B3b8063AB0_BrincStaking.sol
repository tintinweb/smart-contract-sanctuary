...
import "./mock/MockGovBrincToken.sol";
...
contract BrincStaking is Ownable {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.brcStakedAmount * accGovBrincPerShare) - user.receivableReward
    ...
    // The Treasury Address. All unused rewards will be sent to the treasury
    ...
    */
    ...
    /// #if_succeeds {:msg "pendingRewards: the pending rewards of a given user should be correct - case: maturity has passed with no pending rewards"}
            /// let accGovBrincPerShare := old(poolInfo[userInfo[_user].mode].accGovBrincPerShare) in
            /// let totalSupply := old(getTotalSupplyOfAllPools()) in
            /// let multiplier := old(block.number.sub(lastRewardBlock)) in
            /// let govBrincReward := multiplier.mul(govBrincPerBlock).mul(poolInfo[userInfo[_user].mode].supply).mul(poolInfo[userInfo[_user].mode].weight).div(totalSupply) in
            /// let scaled := govBrincReward.mul(1e12).div(poolInfo[userInfo[_user].mode].supply) in
            /// let updatedAccGovBrincPerShare := accGovBrincPerShare.add(scaled) in
            /// userInfo[_user].timestamp <= block.timestamp && (block.number < lastRewardBlock || totalSupply == 0) ==> $result == userInfo[_user].brcStakedAmount.mul(accGovBrincPerShare).div(1e12).sub(userInfo[_user].receivableReward);
    /// #if_succeeds {:msg "pendingRewards: the pending rewards of a given user should be correct - case: maturity has passed with pending rewards"}
            /// let accGovBrincPerShare := poolInfo[userInfo[_user].mode].accGovBrincPerShare in
    ...
    function pendingRewards(address _user) external view returns (uint256) {
        ...
        return
            user.brcStakedAmount.mul(accGovBrincPerShare).div(1e12).sub(user.receivableReward);
    }
    ...
    function updateLockTime(StakeMode _mode, uint256 _updatedLockTime)
        public
        onlyOwner
    {
    ...
    function updateWeight(StakeMode _mode, uint256 _weight)
        public
        onlyOwner
    {
        massUpdatePools();
    ...
    /// #if_succeeds {:msg "updateRatioBrcToGov: sets ratioBrcToGov correctly"}
        /// ratioBrcToGov == _updatdeRatioBrcToGov;
    function updateRatioBrcToGov(uint256 _updatdeRatioBrcToGov)
        public
        onlyOwner
    {
    ...
    function updateTreasury(address _updatdeTreasury)
        public
        onlyOwner
    {
        address oldTreasury = treasury;
        treasury = _updatdeTreasury;
    ...
    function massUpdatePools() internal {
        ...
        if (totalSupply == 0) {
            if (block.number > lastRewardBlock) {
                ...
                govBrincToken.mint(treasury, unusedReward);
            }
        } else {
    ...
    /// #if_succeeds {:msg "updatePool: updates pool's information and mint's reward"}
        ...
        /// let multiplier := block.number.sub(lastRewardBlock) in
        /// let govBrincReward := multiplier.mul(govBrincPerBlock).mul(poolInfo[mode].supply).mul(poolInfo[mode].weight).div(totalSupply) in
        /// poolInfo[mode].accGovBrincPerShare == poolInfo[mode].accGovBrincPerShare.add(govBrincReward.mul(1e12).div(poolInfo[mode].supply));
    function updatePool(StakeMode mode) internal {
    ...
    /// #if_succeeds {:msg "deposit: user information is updated correctly"}
        /// userInfo[msg.sender].brcStakedAmount == _amount && userInfo[msg.sender].timestamp == block.timestamp.add(poolInfo[_mode].lockTime) && userInfo[msg.sender].receivableReward == userInfo[msg.sender].brcStakedAmount.mul(poolInfo[_mode].accGovBrincPerShare).div(1e12) && userInfo[msg.sender].mode == _mode;
    /// #if_succeeds {:msg "deposit: pool supply is updated correctly"}
        /// poolInfo[_mode].supply == old(poolInfo[_mode].supply) + userInfo[msg.sender].brcStakedAmount;
    function deposit(uint256 _amount, StakeMode _mode) public {
        UserInfo storage user = userInfo[msg.sender];
        require(user.brcStakedAmount == 0, "deposit: already deposited.");
        ...
        user.receivableReward = user.brcStakedAmount.mul(pool.accGovBrincPerShare).div(1e12);
        user.mode = _mode;
    ...
    /// #if_succeeds {:msg "withdraw: token deducted from staking contract correctly"}
        /// let _amount := userInfo[msg.sender].brcStakedAmount in
    ...
    /// #if_succeeds {:msg "withdraw: user information is updated correctly"}
        /// let _amount := userInfo[msg.sender].brcStakedAmount in
        /// userInfo[msg.sender].brcStakedAmount == _amount && userInfo[msg.sender].timestamp == block.timestamp.add(poolInfo[userInfo[msg.sender].mode].lockTime) && userInfo[msg.sender].receivableReward == userInfo[msg.sender].brcStakedAmount.mul(poolInfo[userInfo[msg.sender].mode].accGovBrincPerShare).div(1e12) && userInfo[msg.sender].mode == userInfo[msg.sender].mode;
    /// #if_succeeds {:msg "withdraw: pool supply is updated correctly"}
        /// poolInfo[userInfo[msg.sender].mode].supply == old(poolInfo[userInfo[msg.sender].period].supply).sub(userInfo[msg.sender].brcStakedAmount);
    function withdraw() public {
        ...
        PoolInfo storage pool = poolInfo[user.mode];
        uint256 pending =
            user.brcStakedAmount.mul(pool.accGovBrincPerShare).div(1e12).sub(
