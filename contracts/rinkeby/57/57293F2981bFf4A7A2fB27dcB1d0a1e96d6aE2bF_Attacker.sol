...
/* contract IAD {
  ...
  mapping(address => uint) public purchaseTxs;
} */
...
contract Attacker {
  ...
  constructor(){
    target = AD(0x897D43559F096D03ccdf0664B3a25F11Be60d827);
  }

  function balanceOfTokens() public view returns (uint) {
    return target.balanceOf(msg.sender, 0);
  }

  function purchasesByAddress(address adr) public view returns (uint) {
    return target.purchaseTxs(adr);
  }

  uint public counter;
  uint public pricePerToken = 0.002 ether;

  function purchaseFromContract() public payable {
    //purchase
    //transfer
    //selfdestruct
    //repeat
...
contract Attacker {
  ...
  IAD target = new IAD(0x897D43559F096D03ccdf0664B3a25F11Be60d827);
