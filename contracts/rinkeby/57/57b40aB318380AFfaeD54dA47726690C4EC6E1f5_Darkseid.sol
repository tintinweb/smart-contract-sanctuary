    ...
    Ownable
{
    ...
    //Allows users to claim a Maybe Baby for free, and buy a pack of 3 for a discount
    
    function claimFree() external {
        require(balanceOf(_msgSender()) == 0, "Sorry boss, you've claimed your freebie for the day.");
        _mint(_msgSender(), _getNextTokenId());
        _incrementTokenId();
    }
    
 
    function MintAlien(uint amount, uint cost) external payable {
        require(amount.mul(Mint_price) <= msg.value);
        require(amount <= 12, "Only 12 can fit in the spaceship");
        _mint(_msgSender(), _getNextTokenId());
            _incrementTokenId();
    }
    ...
    function buyThree() public payable {
        ...
        require(msg.value <= Mint_price.mul(3), "Incorrect Ether amount.");
        uint256 num = 3;
