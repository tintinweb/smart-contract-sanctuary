...
contract IDXYS {
  ...
  */
    ...
    /// @param _assetId  ERC2020 the asset to deposit
    /// @param _amount uint the amount to deposit
    /// @param _profileId the profile we will deposit to


  function depositErc(uint256 _assetId, uint256 _amount, uint256 _profileId)  public  {
    // check the profile for this user
    _Assets memory _asset = assets[_assetId];                                                      // load the asset in memory                                          
    require(_amount>0,'Zero amount?');                                                             // canot be Zero
    require(_asset.assetAddr != ETHER,'Use depositEth');                                           // is not ether
    require(assetRegistered[_asset.assetAddr],'Asset Unknown?');                                  // is registered
    IERC20 asset = IERC20(_asset.assetAddr);
    require(asset.transferFrom(msg.sender, address(this), _amount),'Available Funds?');            // is transfered   <----------------------- 1 * approval
     balances[msg.sender][_asset.assetAddr] = balances[msg.sender][_asset.assetAddr].add(_amount); // The balance 
    _Deposits storage _deposit = deposits[depositCount];                                           // load deposits storage  
    
    _deposit.id = depositCount;                // record the deposit
    _deposit.owner = msg.sender;
    _deposit.amount = _amount;                // should be the interest bearing token amount  cToken, aToken, yvToken
    _deposit.asset = _asset.assetAddr;        // should be the interest bearing token addresd                           // even is UI display it in Dai     
    _deposit.timestamp = block.timestamp;
    depositCount = depositCount + 1; 
    
    // the profile of that user for that asset 
     _depositOnComp(_asset.assetAddr, _asset.pTokens[0], _amount);      
    
    emit Deposit(_asset.assetAddr, msg.sender, _amount);
    }

    /// @notice WITHDRAW  ERC20
    /// @dev This public function call the internal _trade
    /// @param _id The deposit in position
    /// @param _amount the amount to withdraw

  function withdrawErc(uint _id, uint256 _amount)  public  {
                    
  //   require(_asset.transfer(msg.sender, _amount));   
     _Deposits storage _deposit = deposits[_id];                                                                         //load the deposit 
      require(balances[msg.sender][_deposit.asset] >= _amount, 'Assets Funds?');                       // Have the funds
      require(msg.sender == _deposit.owner,'Owner');                                                           // msg.sender is owner
      balances[msg.sender][_deposit.asset] = balances[_deposit.asset][msg.sender].sub(_amount);        // set the balance
      _deposit.end = block.timestamp;                                                                  // mark the end   Might have funds left

    // _withdrawFromMarket(_market, _asset, _amount)   

     emit Withdraw(_deposit.asset, msg.sender, _amount);
    }
   ...
   /// @notice DEPOSIT ETH 
   /// @dev This public function call the internal PROTOCOL
   /// @param _market the market where we deposit

  function depositETH(uint _market)  public payable  {
    require(msg.value > 0,'Zero ETH?');                                           // must deposit amount
    require(userAccessLevel[msg.sender][_market]);                                // user is registered on market
    require(assetRegistered[ETHER],'Asset Unknown?');                            // is registered
    balances[msg.sender][ETHER] = balances[ETHER][msg.sender].add(msg.value);     // set balance
    _Deposits storage _deposit = deposits[depositCount];  
    _deposit.id = depositCount;
    _deposit.owner = msg.sender;
    _deposit.amount = msg.value;
    _deposit.asset = ETHER;
    _deposit.timestamp = block.timestamp;
    depositCount = depositCount + 1; 

     // _depositToMarket(_market, _asset, _amount);

     emit Deposit(ETHER, msg.sender, msg.value);
    }

    /// @notice WITHDRAW ETH 
    /// @dev This public function call the internal PROTOCOL
    /// @param _id The position to withdraw

  function withdrawETH(uint256 _id, uint256 _amount)  public  {

    _Deposits storage _deposit = deposits[_id];
    require(_deposit.owner == msg.sender,'Owner?');
    require(balances[msg.sender][ETHER] >= _amount, 'ETH Funds?');
    balances[msg.sender][_deposit.asset] = balances[msg.sender][_deposit.asset].sub(_amount);
    

   // _withdrawFromMarket(_market, _asset, _amount)  

    emit Withdraw(ETHER, msg.sender, _amount);
    ...
    /// @param _amount The amount to deposit
     ...
     function _depositOnComp(address _asset,address _cToken, uint256 _amount) payable public {
        // require(msg.sender == owner);
        // address[] memory cTokens = new address[](1);         // we enter one market 
        ...
        /// @notice WITHDRAW CTOKEN AMOUNT.
        /// @dev Called by controller
        /// @param _cToken CToken address
        /// @param _amount Must be Ctoken amount with 8 decimals

      function _withdrawCtoken(address _cToken,uint256 _amount) public {
        require(msg.sender == owner);
        CErc20 cToken = CErc20(_cToken);
        require(cToken.redeem(_amount) == 0, "CToken Withdraw?");
      }
