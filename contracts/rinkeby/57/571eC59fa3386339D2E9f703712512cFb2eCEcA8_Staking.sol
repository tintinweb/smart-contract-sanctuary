...
contract Staking is Context, Pausable, AccessControlEnumerable, ReentrancyGuard {
	...
	uint256 public constant MINIMUM_CLAIM_WAIT = 1 days;
    ...
    modifier depositExists(uint256 _pid, uint256 _id) {
    	require(_id <=  _deposits[_msgSender()], "MadStaking: depositExists bad id parameter");
	 ...
	 */
	function depositInfo(uint256 _pid, uint256 _id) public view depositExists(_pid, _id) returns(DepositInfo memory) {
		return _depositInfo[_pid][_msgSender()][_id];
	}
	 ...
	 */
    function getEmission(uint256 _pid, uint256 _id, uint256 _amount, uint256 _delay) public view depositExists(_pid, _id) returns (uint256) {

        if (depositInfo(_pid, _id).depositDate <= 0 || depositInfo(_pid, _id).depositDate > _now()) return 0;
        
		uint256 timePassed = _now() - depositInfo(_pid, _id).depositDate;
        if (timePassed == 0) return 0;

        // MAD_AMOUNT * (SIGMOID_RES) * (MULTIPLICATOR + DELAY_IN_BLOCKS) 
        uint256 userEmissionRate = _amount * (poolInfo(_pid).sigmoid.calculate(int256(timePassed))) * (poolInfo(_pid).multiplicator + _delay);
     ...
     * @param _delay Additional number of blocks to wait before withdraw
     */
	function claim(uint256 _pid, uint256 _id, uint256 _amount, uint256 _delay) external nonReentrant depositExists(_pid, _id) returns (bool) {
		require(depositInfo(_pid, _id).amount >= _amount, "MadStaking: claim not enough balance");

		uint256 timePassed = _now() - depositInfo(_pid, _id).depositDate;
		require(timePassed >= 0, "MadStaking: claim the has not come yet");

		uint256 issuance = getEmission(_pid, _id, _amount, _delay);
	 ...
	 */
    function withdraw(uint256 _pid, uint256 _id, uint256 _amount) external nonReentrant depositExists(_pid, _id) returns (bool) {
    	require(depositInfo(_pid, _id).claims >= _amount, "MadStaking: withdraw not enough claimed balance");
    	require(depositInfo(_pid, _id).claimedDate <= _now(), "MadStaking: withdraw claim not ready yet");
	 ...
	 * @param _id Unique user's deposit ID
	 */
    function emergencyWithdraw(uint256 _pid, uint256 _id) external nonReentrant depositExists(_pid, _id) returns (bool) {
    	require(depositInfo(_pid, _id).amount > 0, "MadStaking: emergencyWithdraw insufficient funds to withdraw");
    ...
    function _deposit(uint256 _pid, uint256 _amount) private returns (bool) {
    	uint256 depositId = depositLength() + 1;
    ...
    function _claim(uint256 _pid, uint256 _id, uint256 _amount, uint256 _delay) private returns (bool) {
		...
		_depositInfo[_pid][_msgSender()][_id].claimedDate = _now() + MINIMUM_CLAIM_WAIT + _delay;
		_poolInfo[_pid].totalStaked -= _amount;
    ...
    function _emergencyWithdraw(uint256 _pid, uint256 _id) private returns (bool) {
    	uint256 amount = depositInfo(_pid, _id).amount;
