...
contract SlimHoodsSpecials is ERC1155, Ownable {
  ...
  function unmintedSlimHoodIds(uint256 specialNumber) public view returns (uint256[] memory) {
    uint balance = slimhoodsContract.balanceOf(msg.sender);
    uint[] memory result = new uint[](balance);
    uint256 tokenId;
    uint counter = 0;

    for (uint i = 0; i < balance; i++) {
      tokenId = slimhoodsContract.tokenOfOwnerByIndex(msg.sender, i);
