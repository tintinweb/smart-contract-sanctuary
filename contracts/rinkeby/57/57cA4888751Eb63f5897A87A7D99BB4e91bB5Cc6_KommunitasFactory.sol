...
interface IKommunitasFactory{
    ...
    function createProject(address, address, uint, uint, uint, uint, uint[3] memory, uint[2] memory, uint, uint) external returns (address);
...
contract KommunitasProject{
    ...
    uint public constant BOOSTER_RUNNING = 120; // 86400; // 1 day
    uint public constant BOOSTER_DELAY = 1; // 1; // 1 secs
    ...
    function initialize(
        ...
        tge = _tge;
        fee = _fee;
        minPublicBuy = _minMaxPublicBuy[0].mul(10**tokenProject.decimals()).div(1e6);
        maxPublicBuy = _minMaxPublicBuy[1].mul(10**tokenProject.decimals()).div(1e6);
    }
    ...
    function getAmountOut(address _tokenIn, uint _amountIn) public view returns(uint amountOut){
        (uint256 tokenInReserve, uint256 tokenOutReserve) = UniswapV2Library.getReserves(factory.swapFactory(), _tokenIn, address(payment));
    ...
    function buyTokenbyETH(address[] calldata _path) payable isBuyNotEnded isBoosterProgress public {
        ...
        uint tokenReceived = buyAmount.mul(10 ** tokenProject.decimals()).div(booster[boosterProgress].price);
    ...
    function buyToken(uint _amountIn, address _tokenIn) isBuyNotEnded isBoosterProgress public {
        ...
        uint tokenReceived = _amountIn.mul(10 ** tokenProject.decimals()).div(booster[boosterProgress].price);
...
contract KommunitasFactory is IKommunitasFactory{
    ...
    function createProject(
        ...
        require(_payment != address(0) && _tokenProject != address(0), "Can't assign to address(0)");
