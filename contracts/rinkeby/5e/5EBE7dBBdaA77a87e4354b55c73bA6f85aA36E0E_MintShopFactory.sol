...
/**
  @title A basic smart contract for spawning and tracking the ownership of SuperFarm MintShops.
  @author 0xthrpw;

  This is the governing registry of all SuperFarm MintShops.
*/
contract MintShopFactory is ReentrancyGuard {

  /// A version number for this record contract's interface.
  uint256 public version = 1;

  /// A mapping for an array of all MintShops deployed by a particular address.
  mapping (address => address[]) public shopRecords;

  /// A mapping of a mintshop address to its corresponding farm address
  mapping (address => address) public farmMap;

  /// An event for tracking the creation of a new MintShop.
  ...
  function createShop(
    ...
    MintShop1155 newShop = new MintShop1155(_owner, _paymentReceiver, _globalPurchaseLimit);
    address newShopAddress = address(newShop);
    shopRecords[msg.sender].push(newShopAddress);
    farmMap[newShopAddress] = _farm;
    emit ShopCreated(newShopAddress, msg.sender);
    return newShopAddress;
  }

  /**
    Get the number of entries in the MintShop records mapping for the given user.

    @param _user The address of the shop deployer
    @return The number of MintShops added for a given address.
  */
