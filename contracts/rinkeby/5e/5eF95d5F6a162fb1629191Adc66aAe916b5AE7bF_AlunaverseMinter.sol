...
contract AlunaverseMinter is Ownable {
  ...
  function toggleTokenPublicSale(uint256 tokenId)
    public
    onlyOwner
  {
  ...
  function updateWhitelistSigner(address newWhitelistSigner)
    public
    onlyOwner
  {
  ...
  function updateWithdrawalAddress(address payable newAddress) external onlyOwner {
    withdrawalAddress = newAddress;
  }

  function whitelistMint(bytes memory signature, uint256 tokenId, uint256 numberOfTokens, uint256 approvedLimit) external payable {
    require(whitelistSigner != address(0), "NO_WHITELIST_SIGNER");
    require(msg.value == tokenMintPrice[tokenId] * numberOfTokens, "INCORRECT_PAYMENT");
    require((whitelistMinted[msg.sender][tokenId] + numberOfTokens) <= approvedLimit, "WALLET_LIMIT_EXCEEDED");
    whitelistMinted[msg.sender][tokenId] = whitelistMinted[msg.sender][tokenId] + numberOfTokens;
    
    bytes32 digest = keccak256(abi.encodePacked("\x19\x01", DOMAIN_SEPARATOR, keccak256(abi.encode(WHITELIST_TYPEHASH, msg.sender, tokenId, approvedLimit))));

    address signer = digest.recover(signature);

    require(signer != address(0) && signer == whitelistSigner, "INVALID_SIGNATURE");

    Alunaverse.mint(msg.sender, tokenId, numberOfTokens, "");
  }

  function publicMint(uint256 tokenId, uint256 numberOfTokens) external payable {
