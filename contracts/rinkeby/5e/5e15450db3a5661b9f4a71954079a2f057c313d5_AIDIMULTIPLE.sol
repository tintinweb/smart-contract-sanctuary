...
contract AIDIMULTIPLE is ERC1155, Ownable, ERC1155Burnable {
    event Approve(
    ...
    function mint(uint256 id, uint256 value, uint256 royal, uint256 nooftoken) public {
    ...
    function _saleToken(
        ...
        if(keccak256(abi.encodePacked((bidtoken))) == keccak256(abi.encodePacked(("ETH")))){   
            require( msg.value == val, "Amount doesnot matches");
            ...
            require( msg.value == _adminfee.add(roy.add(netamount)), "Admin Fee issues");
            ...
            require( approveValue >= val, "Insufficient Balance in Token approved");
            (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], serviceValue);
            require( approveValue >= _adminfee.add(roy.add(netamount)), "Insufficient Balance in admin fee");
