...
contract QeyStroke is ERC1155Burnable, Ownable, ReentrancyGuard, Pausable, PaymentSplitter {
    ...
    bytes32 public root;
    // only Parcels are mintable 1261 * 3 
    uint256 public supply = 3783;
    // Genesis are grouped under one id
    uint256 public genesisID = 3784;
    ...
    event GenesisMinted(uint256[] tokenIds, uint8[] parcelIDs, address owner);
    
    constructor() ERC1155(tokenURI) PaymentSplitter(_addresses, _shares) {
        parcelCount[0] = 1261;
        parcelCount[1] = 1261;
        parcelCount[2] = 1261;
        parcelCount[3] = 156;
    ...
    function callMint(uint256 amount) internal {
        ...
        for(uint256 i; i < amount; i++) {
            ...
            uint8 parcelID = getParcelID(nextToken);
            parcelCount[parcelID] -= 1;
            _mint(_msgSender(), nextToken, 1, "");
            tokenToParcel[nextToken] = parcelID;
            emit ParcelMinted(nextToken, parcelID, _msgSender());
        }        
    }
    ...
    function setGenesis(uint256 _genesis) external onlyOwner {
        genesisID = _genesis;
    }
    ...
    /**
        @dev burning a single parcel 4 will mint a genesis with id 3940 (+1 from supply)
    ...
    function burnOne(uint256 id) external {
        ...
        require(tokenToParcel[id] == 3, "First should be parcel 4");
        _burn(_msgSender(), id, 1);
        _mint(_msgSender(), genesisID, 1, "");
        uint256[] memory tokenIds = new uint256[](1);
        tokenIds[0] = id;
        uint8[] memory parcelIDs = new uint8[](1);
        parcelIDs[0] = tokenToParcel[id];
        emit GenesisMinted(tokenIds, parcelIDs, _msgSender());
    }

    /**
        @dev burn 3 parcels of type 1,2 and 3 to mint a genesis with id 3940 (+1 from supply)
    ...
    function burnThree(uint256[] memory ids) external {
        ...
        uint256 third = ids[2];
        require(tokenToParcel[first] == 0, "First should be parcel 1");
        require(tokenToParcel[second] == 1, "Second should be parcel 2");
        require(tokenToParcel[third] == 2, "Third should be parcel 3");
        ...
        emit GenesisMinted(ids, parcelIDs, _msgSender());
    }
