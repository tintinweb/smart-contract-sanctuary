...
//        uint256 quantity;
   ...
   // event OnNftSent(address from, uint nftId, uint quantity);
    event OnNftReceived(address to, uint nftId);
    event OnNftSent(address from, uint nftId);
    ...
    function ListNft(uint256 nftId, uint256 price, uint256 endDate) public {
    ...
    function Purchase(uint256 orderId) public payable {
        ...
        // require valid time with abi encodePacked not working right
//      require(block.timestamp > order.listDate, string(abi.encodePacked("Listing in future : " , string(order.listDate) , " -- " , block.timestamp)));
//      require(order.endDate > block.timestamp, string(abi.encodePacked("Sale expired : " , string(order.endDate) , " -- " , block.timestamp)));
...
//        emit OnNftSent(order.seller, order.nftId, order.quantity);
        emit OnNftReceived(msg.sender, order.nftId);
        emit OnNftSent(order.seller, order.nftId);
    }
}