...
contract NiftyArtist is ERC721EnumerableUpgradeable,  AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    address public constant creatorAddress = 0x1Abe2FA6908F74863BB857029e353cA3997feefA;

    bool public hasSaleStarted;

    // Max NFTs total.
    //uint public constant MAX_TOKENS = 9241;
    uint public constant MAX_TOKENS = 17; // For testing.
    ...
    function mint(address to) public virtual payable nonReentrant {
        require(totalSupply() < MAX_TOKENS,
           "We are at max supply. Check out the secondary markets.");

        require(msg.value >= calculatePrice(),
    ...
    */
    function reserveGiveaway(address to, uint256 numTokens) public {
        require(hasRole(ADMIN_ROLE, msg.sender), "Caller is not an admin");
        require(hasSaleStarted == true, "Sale has not started");
        require(totalSupply() < MAX_TOKENS,
                "We're at max supply!");
