...
contract rarity_gold {
    string public constant name = "Rarity RAR";
    ...
    function claimable(uint summoner) external view returns (uint amount) {
        require(maxSupply >= totalSupply, "There will be only 21 million RAR in existence");
        require(_isApprovedOrOwner(summoner));
        uint _current_level = rm.level(summoner);
        require (_current_level > 1, "Only true heroes can claim RAR!");
