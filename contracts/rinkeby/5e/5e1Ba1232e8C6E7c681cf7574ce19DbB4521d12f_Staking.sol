...
contract Staking is Deposit{
    ...
    function computStakingProfit(uint _id,uint _time)  public view returns (uint profit){
        ...
        uint computMonth = signDeposit[_id].lastExtractTime/100%100;
        uint num;
        uint monthSecord;
        uint computEndTime = signDeposit[_id].lastExtractTime;
        // return 10;
        for(uint i=0;i<passMonths;i++){
            ...
            num = num/getDaysInMonth(computMonth,computYear)/decimals;
            if(i==passMonths){
                monthSecord = getYmd(_time) - computEndTime%100;
                profit += num*monthSecord;
            }else{
                monthSecord = getDaysInMonth(computMonth,computYear) - computEndTime%100 + 1;
                profit += num*monthSecord;
            }            
            if(computMonth < 12){
                computEndTime = computYear*10000+computMonth*100+100+1;
                computMonth = computMonth + 1;
            }else{
                computEndTime = computYear*10000+10000+100+1;
                computMonth = 1;
    ...
    function passMonthCount(uint _id,uint _time) public view returns (uint passMonths) {
        ...
        if(curDate/100%100>signDeposit[_id].lastExtractTime/100%100){
            passMonths = curDate/100%100 - signDeposit[_id].lastExtractTime/100%100 + 1;
        }else{
            passMonths = 12 - signDeposit[_id].lastExtractTime/100%100 + curDate/100%100 + 1;
        }
    }
