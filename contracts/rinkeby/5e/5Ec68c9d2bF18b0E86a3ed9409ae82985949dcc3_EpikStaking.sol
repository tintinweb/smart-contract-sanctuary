...
contract EpikStaking is ReentrancyGuard, Pausable {
    ...
    /* ========== MEMBERSHIP ============== */
    uint256[] public tier_thresholds = [100, 1000, 10000];
    ...
    function membershipOf(address account) external view returns (uint256) {
        for (uint i = 0; i < tier_thresholds.length; i++) {
            if (_primeBalances[account] < tier_thresholds[i]) {
                return i;
            }
        }
        return 0;
        ...
        updateReward(msg.sender)
    {
        ...
        // Prime balance
        _primeBalances[msg.sender] = _primeBalances[msg.sender].sub(amount);
