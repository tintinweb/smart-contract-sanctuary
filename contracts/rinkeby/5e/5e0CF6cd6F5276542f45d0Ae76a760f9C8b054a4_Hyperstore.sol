...
contract Hyperstore is AccessControl, Pausable {
    /// A bundle is the atomic item that can be bought from the store.
    /// @param cost the cost of the bundle (in Stardust)
    /// @param name the name of the bundle
    /// @param description the description of the bundle
    /// @param items an array of items that can be bought
    /// @param quantities the quantities of items
    /// @param available whether the bundle can be bought
    /// Note: items[] and quantities[] are two separate arrays because this is what mintBatch() accepts.
    struct Bundle {
        uint256 cost;
        string name;
        string description;
    ...
    event BundleUpdate(uint256 indexed id, Bundle bundle);

    /// Event emitted when a Bundle is purchased.
    /// @param buyer the address of the buyer
    /// @param id the ID of the purchased bundle
    event BundlePurchase(address indexed buyer, uint256 indexed id);
    ...
    function setBundle(uint256 id, Bundle memory bundle)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            bundle.items.length == bundle.quantities.length,
            "Array length mismatch"
        );

        bundles[id] = bundle;
        emit BundleUpdate(id, bundle);
    }

    function getBundle(uint256 id) external view returns (Bundle memory) {
        return bundles[id];
    }
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        emit BundleUpdate(id, bundles[id]);
    }

    /// @dev purchase a bundle, burning stardust in the process
    /// @param id the id of the bundle to purchase
    function _buyBundle(uint256 id) private {
        require(bundles[id].available, "Bundle not available for sale");

        // Burn stardust
        stardustContract.burnFrom(msg.sender, bundles[id].cost);

        planetPassItemsContract.mintBatch(
            msg.sender,
            bundles[id].items,
            bundles[id].quantities,
            ""
        );

        emit BundlePurchase(msg.sender, id);
    }
    ...
    /// Buy an item from the store, burning Stardust in the process.
    /// Note: This requires Stardust spend permission from `msg.sender`. Otherwise will revert.
    /// @param id array of bundle IDs to purchase
    function buy(uint256[] calldata id) public {
...
contract PlanetPassItems is ERC1155, ERC1155Burnable, AccessControl {
    ...
    /// Event emitted when an item type is updated.
    /// @param tokenId the token ID being updated
    /// @param itemType 32 bytes digest of the new type
