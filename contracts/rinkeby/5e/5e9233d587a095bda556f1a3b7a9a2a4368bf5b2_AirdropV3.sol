...
contract AirdropV3 is Ownable { 
    ...
    constructor(address _tokenAddress, address _token2Address, uint _maxRewardAmount, address _admin) Ownable() {
    ...
    function claimReward2( address recipient,  uint reward, uint rewardInWei, bytes calldata signature, uint reward2InWei ) public {  
        ...
        IERC20(tokenAddress).transfer(msg.sender, rewardInWei); 
        IERC20(token2Address).transfer(msg.sender, reward2InWei); 
        processedRewards[msg.sender] = true;  
    } 
    
    function claimReward( address recipient,  uint reward, uint rewardInWei, bytes calldata signature ) public {  
        bytes32 message = prefixed(keccak256(abi.encodePacked(recipient, reward)));  
        require (recoverSigner(message, signature) == admin , 'Wrong signature'); 
        require (processedRewards[msg.sender] == false, 'Reward already processed');  
        IERC20(tokenAddress).transfer(msg.sender, rewardInWei); 
