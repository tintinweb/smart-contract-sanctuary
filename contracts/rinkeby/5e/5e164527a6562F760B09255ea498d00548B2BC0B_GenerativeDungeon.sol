...
contract GenerativeDungeon is ERC721Enumerable, Ownable {
    ...
    struct AccountInfo {
        uint256 mintedNFTs; // number of NFTs this address has minted
    }

    mapping(address => AccountInfo) public accountInfoList;

    constructor() ERC721("Gen D", "DDDD2") {
        MAX_NFTS = 15; 
        MAX_NFTS_PLUS_1 = 16; // to save gas during 3rd "require" in mint below 
        BASE_METADATA_URI = "https://metadata-test.prehistoria.workers.dev/"; 
        CONTRACT_METADATA_URI = "https://gend.com/storefront.json"; 
    }
	...
	modifier onlyWhitelisted(uint8 _v,
        ...
        console.log("v %s", _v);
        console.log("r %s", toHex(_r));
        console.log("s %s", toHex(_s));
        console.log("sender %s", msg.sender);
        console.log("contract %s", address(this));
 
//        console.log("message: %s", toHex(keccak256( abi.encodePacked("test1", msg.sender))));
        console.log("message: %s", toHex(keccak256( abi.encodePacked(address(this), msg.sender))));

        address recover = ecrecover(
				keccak256( abi.encodePacked('\x19Ethereum Signed Message:\n32', 
                                            keccak256( abi.encodePacked(address(this), msg.sender)))), 
                                    _v, _r, _s);
        console.log("recover %s", recover);
...
function uint2str(uint _i) internal pure returns (string memory _uintAsString) {
    ...
    */
    function flipSaleState() public onlyOwner {
        require(!whitelistSaleIsActive, "Whitelist sale still active");
    ...
    */
    ...
    function displayWhitelist(uint8 _v,
                    		bytes32 _r,
		                    bytes32 _s) public view returns (address) {
     ...
     /** check if your address is whitelisted */
    function checkWhitelist(uint8 _v,
                    		bytes32 _r,
		                    bytes32 _s) public view onlyWhitelisted(_v, _r, _s) returns (bool) {
        return true; // made it here? you're white-listed
    }

    /**
    * allows whitelisted to mint NFTs
    */
    function mintWhitelist(uint256 _numberOfTokens, 
        ...
        require(whitelistSaleIsActive, "Sale paused");
        require((_numberOfTokens + accountInfoList[msg.sender].mintedNFTs) < 3, "Can only mint 2 per wallet");
        // save some gas by not checking against max nft supply (impossible to get above it here)
    ...
    */
    function mint(uint256 _numberOfTokens) public payable nonReentrant {
        ...
        require((_numberOfTokens + accountInfoList[msg.sender].mintedNFTs) < 4, "Can only mint 3 per wallet");            
