...
interface IPerpetualDepositManager {
    ...
    function withdrawFromDefaultFund(uint256 _poolId, address _receiver, int128 _fAmount) external;
}
...
contract PerpetualDepositManager is PerpetualBaseFunctions, IFunctionList, IPerpetualDepositManager {
    using ABDKMath64x64 for int128;
    using ConverterDec18 for int128;
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;
    ...
    function depositToDefaultFund(uint256 _poolId, int128 _fAmount) external override onlyOwner {
        ...
        emit UpdateDefaultFundCash(_poolId, _fAmount, pool.fDefaultFundCashCC, block.timestamp);
    }

    /**
     * Withdraw from the default fund.
     *
     * @param   _poolId      Reference to liquidity pool
    ...
    function withdrawFromDefaultFund(
        ...
        emit UpdateDefaultFundCash(_poolId, _fAmount.neg(), pool.fDefaultFundCashCC, block.timestamp);
    }
    ...
    function _deposit(
        ...
        emit TokensDeposited(_perpetual.id, _traderAddr, _fAmount, block.timestamp);
    }
