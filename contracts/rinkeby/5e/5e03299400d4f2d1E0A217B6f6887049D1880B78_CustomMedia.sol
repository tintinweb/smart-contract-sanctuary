...
import {ICustomMedia} from "./interfaces/ICustomMedia.sol";
...
/**
 * @title CustomMedia contract
 *
 * @author Aito
 * @notice A media contract inheriting from a very slightly modified Zora Media NFT, allowing for the following:
 *      1. Bidding on behalf of another user (notably useful for WETH Gateway single-tx ETH bids)
 *      2. Batch minting using a mint limit to mint multiple copies of an NFT with the same hash.   
 */
contract CustomMedia is Media, ICustomMedia {
    using Counters for Counters.Counter;
    using EnumerableSet for EnumerableSet.UintSet;

    // Mapping from content hash to associated remaining mints
    mapping(bytes32 => uint256) public remainingMintsByHash;

    // Mapping from content hash to associated creator
    mapping(bytes32 => address) public creatorByHash;

    //keccak256("BatchMintWithSig(bytes32 contentHash,bytes32 metadataHash,uint256 creatorShare,uint256 amount,uint256 nonce,uint256 deadline)")
    bytes32 public constant BATCH_MINT_WITH_SIG_TYPEHASH =
        0xef044c1b609f2a40aa35e6e0672bbbf1d71366a46154a3f7bdd0ac51fb47f19e;

    constructor(address marketContractAddress) Media(marketContractAddress) {}

    /**
     * @notice Overrides the default Zora setBid function to allow bidding on behalf of another address. This allows
     * notably the use of a WETH Gateway contract for single-transaction ETH bids.
     * 
     * See IMedia for the original description.
     */
    ...
    function batchMint(
        ...
        // We need to allow minting 1 NFT in a batch mint, in case the creator only wants one out of N NFTs,
        // or they only have 1 NFT left to mint.
        require(amount > 0, "Media: zero batch amount");

        _batchMintForCreator(creator, data, bidShares, amount, mintLimit);
    }

    /**
     * @dev This internal function allows for minting multiple copies of the same content, up to a specified
     * mintLimit.
     */
    function _batchMintForCreator(
        address creator,
        MediaData memory data,
        IMarket.BidShares memory bidShares,
        uint256 amount,
        ...
        if (_contentHashes[data.contentHash] == true) {
            remainingMints = remainingMintsByHash[data.contentHash];
            require(
                creatorByHash[data.contentHash] == creator,
                "Media: not original creator"
            );
        } else {
            require(mintLimit > 1, "Media: insufficient mint limit");
            creatorByHash[data.contentHash] = creator;
            remainingMints = mintLimit;
            _contentHashes[data.contentHash] = true;
        }

        require(remainingMints > 0, "Media: no remaining mints");
        require(
            remainingMints >= amount,
            "Media: amount greater than remaining mints"
        );
        ...
        remainingMintsByHash[data.contentHash] = remainingMints - amount;
    }
}
...
/**
 * @title ICustomMedia interface
 */
interface ICustomMedia {
    /**
     * @notice Mints a batch of new media for the given creator.
     * This function has the peculiarity that, for the same batch of media, will be possible
     * to have the same content hash, as the integrity check will only be done once as long as there
     * remains mintable copies. Furthermore, we allow minting on behalf of any creator.
     */
    function batchMint(
...
/**
 * @title Interface for Custom Protocol's Market
 */
interface IMarket {
    struct Bid {
...
contract Media is IMedia, ERC721Burnable, ReentrancyGuard {
    ...
    mapping(address => EnumerableSet.UintSet) internal _creatorTokens;
    ...
    mapping(bytes32 => bool) internal _contentHashes;
     ...
     */
    constructor(address marketContractAddr) ERC721("Custom", "CUSTOM") {
