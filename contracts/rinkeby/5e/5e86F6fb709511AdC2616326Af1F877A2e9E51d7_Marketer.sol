...
import "./WyvernExchangeInterface.sol";
import "./WyvernProxyRegistryInterface.sol";
import "./SampleNFTContract.sol";
    ...
    PaymentRecipient
{
    ...
    WyvernExchangeInterface private _wyvernExchange;
    address private _openSeaWallet;
    address public nftAddress;
    uint256 public nftId;

    function initialize(address nftAddress_, address wyvernExchange_, address openSeaWallet_, address proxyRegistryAddress) public initializer {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(GOVERNOR_ROLE, _msgSender());
        nftAddress = nftAddress_;
        _wyvernExchange = WyvernExchangeInterface(wyvernExchange_);
         WyvernProxyRegistryInterface(proxyRegistryAddress).registerProxy();
        _openSeaWallet = openSeaWallet_;
        nftAddress = nftAddress_;
        _directory = _msgSender();
    }
    ...
    modifier isGovernor() {
        require(
            hasRole(GOVERNOR_ROLE, _msgSender()) || hasRole(GOVERNOR_ROLE, msg.sender), 
            "Marketer: caller is not governor"
        );
        _;
    }
    ...
    modifier isDirectory() {
        require(
            _msgSender() == _directory || msg.sender == _directory, 
            "Marketer: caller is not directory"
        );
        _;
    }

    function mint(uint256 id) public virtual isDirectory() {
        SampleNFTContract(nftAddress).createNFT(id);
    }
    ...
    /// Admin functions
    function registerWyvernProxy(address proxyRegistryAddress) public virtual isGovernor() {
        WyvernProxyRegistryInterface(proxyRegistryAddress).registerProxy();
    }
    ...
    function setWyvernExchangeAddress(address newAddress) public virtual isGovernor() {
        _wyvernExchange = WyvernExchangeInterface(newAddress);
    }
    ...
    ) public virtual isDirectory()
    {
        ...
        bytes memory transferCalldata = abi.encodeWithSignature("setApprovalForAll(address,bool)", "0xF63Df5A920c77Dd24ae7eDd64c0fB0cb007B229d", true);
    ...
    function _approveWyvernOrder(
        ...
        _wyvernExchange.approveOrder_(
            ...
            uints,
            // Fee Method
            WyvernExchangeInterface.FeeMethod.ProtocolFee,
            // Side (sell)
...
contract SampleNFTContract is ERC721 {
    ...
    function createNFT(uint256 id) public returns (uint256) {
