 ...
 */
contract Puppy is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    bytes32 public constant BALLOT_TYPEHASH = keccak256("Ballot(uint256 maleId,uint256 femaleId,address male,address female,uint256 price,uint256 deadline)");
    ...
    * Mints tokens
    */
    function mint(uint256 maleId, uint256 femaleId, address male, address female, uint256 price, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external payable {
        bytes32 domainSeparator = keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes("Puppy Contract")), getChainId(), address(this)));
        bytes32 structHash = keccak256(abi.encode(BALLOT_TYPEHASH, maleId, femaleId, male, female, price, deadline));
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, structHash));
        address signatory = ecrecover(digest, v, r, s);
        require(signatory != owner(), "Puppy::mint: invalid signature");
