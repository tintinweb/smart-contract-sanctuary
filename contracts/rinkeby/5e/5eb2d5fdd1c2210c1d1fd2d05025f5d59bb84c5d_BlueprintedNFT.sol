...
/*
Arg [0] : _baseURI (string): https://mint.blueprinted.net/tokens/
...
contract BlueprintedNFT is ERC721Enumerable, Ownable {
    ...
    uint256 public constant     WL_priceInWei       = 0.06 ether;
    ...
    function mint(uint256 count, bytes32[] calldata proof) public payable {
        ...
        if (saleStatus == 1) {
            require(MerkleProof.verify(proof, whitelistMerkleRoot1, leaf), 'Not on whitelist. Merkle Proof fail.');
            require(addressToMinted[_msgSender()] + count <= BL_MAX_PER_TX, "Exceeds whitelist supply"); 
