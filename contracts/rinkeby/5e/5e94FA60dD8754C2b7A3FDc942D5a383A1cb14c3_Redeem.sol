...
contract Redeem is Ownable {
    ...
    mapping(IERC721 => IERC20) public feeCurrencies;

    StakeLogicV1 public lockV1;
    StakeLogicV1 public lockV2;

    constructor(StakeLogicV1 lockV1_, StakeLogicV1 lockV2_) Ownable() {
    ...
    function setPaymentCurrency(IERC721 nft, IERC20 feeCurrency)
        public
        onlyOwner
    {
        feeCurrencies[nft] = feeCurrency;
    }
    ...
    function addNFTRedeemDetails(
        ...
        for (uint256 i = 0; i < seriesIds.length; i++) {
            nftRedeemDetails[nft][collectionId][
                seriesIds[i]
            ] = NFTRedeemDetails({redeemable: true, redeemFee: fees[i]});
        }
        feeCurrencies[nft] = feeCurrency;
    }
    ...
    function redeemNFTs(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            address currentNftOwner = nft.ownerOf(tokenId);
            if (currentNftOwner == address(lockV1)) {
                require(
                    lockV1.nftOwner(nft, tokenId) == msg.sender,
                    "NOT_NFT_OWNER"
                );
            } else if (currentNftOwner == address(lockV2)) {
                require(
                    lockV2.nftOwner(nft, tokenId) == msg.sender,
                    "NOT_NFT_OWNER"
                );
            } else {
                require(currentNftOwner == msg.sender, "NOT_NFT_OWNER");
            }
            ...
            require(
                nftRedeemDetails[nft][collectionId][seriesId].redeemable,
                "NOT_REDEEMABLE"
            );
        ...
        takePayment(address(feeCurrencies[nft]), summedRedeemShippingFee);
    }
