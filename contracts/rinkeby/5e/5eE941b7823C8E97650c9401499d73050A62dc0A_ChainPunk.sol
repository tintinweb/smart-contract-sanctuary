...
contract ChainPunk {
  ...
  function setTraitChunk(uint8 cid, bytes calldata b) public requireAdmin {
    require(cid < 32);  
    trait_chunks[cid] = bytes(b);  
    emit TraitChunk(cid);    
  }
  
  function setTraitTable(bytes calldata b) public requireAdmin {
    uint i = 0;
    while (i < b.length) {
      uint8 id = uint8(b[i]);
  ...
  function setPunks(uint8 cid, bytes calldata b) public requireAdmin {
    require(cid < 256);
...
//      console.log("encoding punk %d, tid %d, len %d",id,tid,len);
    ...
    //    console.log("Encoded %d bytes\n",total_len);
    emit PunkChunk(cid);    
  }

  //format byte 0: index, byte1: r value, byte2: g value, byte3: b value
  function setColors(bytes memory b) public requireAdmin {
  ...
  function decodePunk(uint32 id) public view returns (uint8[576] memory) {
    ...
    console.log("Using template punk %d",tid);
