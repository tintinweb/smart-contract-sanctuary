...
contract TDN is ERC721Enumerable, Ownable, ReentrancyGuard {
	...
	uint256 public maxFreeNFTperID = 0; //can claim one more
	...
	function getReward(uint256 _nftID) public nonReentrant {
		...
		require(maxFreeNFTperID > 0, "max NFT per token must be > 0");
	...
	//sets how many free NFTs will be available per ID [only owner]
	function setFreeNFTPerID(uint256 _amount) external onlyOwner {
		maxFreeNFTperID = _amount;
	}
