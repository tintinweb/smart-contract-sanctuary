...
contract ChickenSuspects is  ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, VRFConsumerBase {
    ...
    uint256 public constant TIME_BETWEEN_DRAW = 60 * 60 * 24 *6; // 60 seconds * 60 minutes * 24 hours * 6 days
    ...
    uint16[5] public weeklySuspects;
        ...
        ) 
    {
        ...
        lastDraw = block.timestamp - TIME_BETWEEN_DRAW;
    }
    ...
    function getWeeklySuspects() public onlyOwner {
        
        require(lastDraw + TIME_BETWEEN_DRAW < block.timestamp , "You need to wait at least 6 days to get the new suspects !");
        
        bytes32 randomValue = getRandomNumber();
        
        for (uint256 i = 0; i < 5; i++) {
            weeklySuspects[i] = uint16(uint256(keccak256(abi.encode(randomValue, i)))%4419);
        }
