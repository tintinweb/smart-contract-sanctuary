...
contract MasterChef is Ownable {
    ...
    function deposit(
        ...
        require(_pid != 0, "You can not deposit in 0 pool");
        updatePool(_pid);
        if (_amount > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        require(_pid != 0, "You can not withdraw from 0 pool");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (_pendingAmount > 0) {
            if (canHarvest(_pid, msg.sender)) {
                ...
                if (isBurn) {
                    ...
                    lockedUpBurn = lockedUpBurn.add(_pendingAmount);
