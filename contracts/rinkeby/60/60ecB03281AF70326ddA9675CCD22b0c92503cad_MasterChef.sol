...
import "./BeachToken.sol";

// MasterChef is the master of Beach. He can make Beach and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // The EGG TOKEN!
    BeachToken public beach;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public beachPerBlock;
    // Bonus muliplier for early beach makers.
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accEggPerShare = accEggPerShare.add(beachReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 beachReward = multiplier.mul(beachPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        beach.mint(devaddr, beachReward.div(10));
        beach.mint(address(this), beachReward);
        pool.accEggPerShare = pool.accEggPerShare.add(beachReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe beach transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 beachBal = beach.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > beachBal) {
            transferSuccess = beach.transfer(_to, beachBal);
        } else {
            transferSuccess = beach.transfer(_to, _amount);
        }
    ...
    function updateEmissionRate(uint256 _beachPerBlock) public onlyOwner {
        massUpdatePools();
        beachPerBlock = _beachPerBlock;
        emit UpdateEmissionRate(msg.sender, _beachPerBlock);
    }
}
