...
contract Nview is NPassCore {
  ...
  function getColorFromTokenId(uint[8] memory tokenIds) public pure returns(string memory){
    ...
    // TODO replace this placeholder for opacity change depending on j
    uint j = 88;
    // check sum of numbers for corresponding color
    // removed 4th parameter , 255 - j + 90 which would be alpha or opacity

    if(sum_numbers <= 25){
      c = colorRGBtoHex(245, 183, 177); //red
    }
    else if(sum_numbers > 25 && sum_numbers <= 30){
      c = colorRGBtoHex(245, 137, 252);  //purple
    }
    else if(sum_numbers >30 && sum_numbers <= 35){
      c = colorRGBtoHex(114, 198, 255 );  //blue
    }
    else if(sum_numbers > 35 && sum_numbers <= 40){
      c = colorRGBtoHex(136, 237, 217);  //green-blueish
    }
    else if(sum_numbers > 40 && sum_numbers <= 45){
      c = colorRGBtoHex(131, 238, 177);  //green
    }
    else if(sum_numbers > 45 && sum_numbers <= 50){
  ...
  function colorRGBtoHex(uint r, uint g, uint b) public pure returns(string memory){
    ...
    result = string(abi.encodePacked("#",toHexString2(r), toHexString2(g), toHexString2(b) ));
    return result;
  }

  function toHexString2(uint256 value) internal pure returns (string memory) {
    if (value == 0) {
        return "00";
    }
    uint256 temp = value;
    ...
    return toHexString2(value, length);
  }

  function toHexString2(uint256 value, uint256 length) internal pure returns (string memory) {
  ...
  function getTokenSVG(uint tokenId) public view returns (string memory) {
    ...
    parts[0] = '<svg width="500" height="500" xmlns="http://www.w3.org/2000/svg">';
  ...
  function getCirclesSVG(uint[8] memory numbers, string memory color) public pure returns(string memory){
    ...
    for(uint i = 0; i < numbers.length; i++){
      for(uint j = 0; j < numbers[i]; j++){
        string memory currentNumberSVG = getCircle(10 * i, 10 * j, color);
  ...
  function getCircle(uint x, uint y, string memory color) public pure returns(string memory) {
    uint radius = 20;
    return 
      ...
      bytes(
        string(
          abi.encodePacked(
            '{"name": "nview #',
            tokenId.toString(),
            '", "description": "nview is just code.", "image": "data:image/svg+xml;base64,',
