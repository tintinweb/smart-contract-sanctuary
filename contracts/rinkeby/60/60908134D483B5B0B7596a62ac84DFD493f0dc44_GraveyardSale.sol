...
contract RBAC is Ownable {
  ...
  function addPremintList(address[] memory list) external onlyOwner{
    for(uint i = 0; i < 420; i++)
    {
      preminters.add(list[i]);
    }
  }

  function isPreminter(address toCheck) public view returns(bool)
  {
    return preminters.has(toCheck);
  }
...
contract GraveyardSale is ERC721, Ownable, RBAC {
  ...
  {
    require(canSetURI == true, "This function is no longer available");
