...
contract MechCraftPresale is Ownable {  
  ...
  function buyChestWithT1Operator(uint256 _amount, bytes memory operator) external returns (uint256) {
    require(_amount >= token1Price, "Insufficient payment");

    IERC777(token1).operatorSend(msg.sender, owner(), _amount, "", operator);
    chestQty[msg.sender]++;
    return chestQty[msg.sender];
  }
