...
contract EBMarketSales is ERC721Holder, ERC1155Holder, Ownable, ReentrancyGuard, TokensHandler {
    ...
    function buy(uint saleId) external payable isAccesible(saleId) nonReentrant {
        ...
        uint bmarketFee = sales[saleId].price.div(fee);
        if ( sales[saleId].currency != address(0) ){
            require(IERC20(sales[saleId].currency).transferFrom(msg.sender,sales[saleId].seller,sales[saleId].price.sub(bmarketFee)), "Transfer of tokens to seller failed");
            ...
            require(payable(msg.sender).send(sales[saleId].price.sub(bmarketFee)),"Transfer of ETH to seller failed");
