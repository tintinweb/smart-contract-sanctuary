  ...
  function swapByUnicorn(uint256 _id, uint256 _amount)
    external payable nonReentrant {
      ...
      if (_balance < _totalPrice) {
        ...
        uint256 _amountToWithdraw = msg.value.sub(_totalPrice.sub(_balance).div(unicornToEth).mul(1 ether));
  ...
  function swapByRainbow(uint256 _id, uint256 _amount)
    external payable nonReentrant {
      ...
      if (_balance < _totalPrice) {
        ...
        uint256 _amountToWithdraw = msg.value.sub(_totalPrice.sub(_balance).div(rainbowToEth).mul(1 ether));
