...
import {IUniversalVault} from "../custosVault/CustosVault.sol";
import {CustosVaultFactory} from "../custosVault/CustosVaultFactory.sol";
...
interface IHypercustos is IRageQuit {
    /* admin events */

    event HypercustosERC20Created(address rewardPool, address powerSwitch);
    event HypercustosERC20Funded(address token, uint256 amount);
...
/// Access Control
/// - Power controller:
///     Can power off / shutdown the Hypercustos
...
/// - Hypercustos admin:
///     Can add funds to the Hypercustos, register bonus tokens, and whitelist new vault factories
///     Is a subset of owner permissions
/// - User:
///     Can stake / unstake / ragequit / claim airdrop / claim vested rewards
/// Hypercustos State Machine
/// - Online:
///     Hypercustos is operating normally, all functions are enabled
/// - Offline:
///     Hypercustos is temporarely disabled for maintenance
...
/// - Shutdown:
///     Hypercustos is permanently disabled
...
contract HypercustosERC20 is IHypercustos, Powered {
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;

    /* constants */
    string public constant VIP = "VIP";
    string public constant PRIMARY = "PRIMARY";
    string public constant GOLD = "GOLD";
    string public constant RARE = "RARE";
    string public constant STANDARD = "STANDARD";
    uint256 public VIP_LM_REWARD_MULTIPLIER_NUM = 6;
    uint256 public VIP_LM_REWARD_MULTIPLIER_DENOM = 2;
    uint256 public PRIMARY_LM_REWARD_MULTIPLIER_NUM = 5;
    uint256 public PRIMARY_LM_REWARD_MULTIPLIER_DENOM = 2;
    uint256 public GOLD_LM_REWARD_MULTIPLIER_NUM = 2;
    uint256 public GOLD_LM_REWARD_MULTIPLIER_DENOM = 1;
    uint256 public RARE_LM_REWARD_MULTIPLIER_NUM = 3;
    uint256 public RARE_LM_REWARD_MULTIPLIER_DENOM = 2;
    ...
    /// @notice Initizalize Hypercustos
    ...
    /// @param rewardTokenAddress address The address of the reward token for this Hypercustos
    constructor(
        ...
        weeklyAirdropAmounts[VIP] = uint256(166).mul(1e18);
        weeklyAirdropAmounts[PRIMARY] = uint256(18).mul(1e18);
        weeklyAirdropAmounts[GOLD] = uint256(4).mul(1e18);
        weeklyAirdropAmounts[RARE] = uint256(1).mul(1e18);
        weeklyAirdropAmounts[STANDARD] = uint256(0).mul(1e18);

        balancesRequiredToClaim[VIP] = uint256(166).mul(1e18);
        balancesRequiredToClaim[PRIMARY] = uint256(18).mul(1e18);
        balancesRequiredToClaim[GOLD] = uint256(4).mul(1e18);
        balancesRequiredToClaim[RARE] = uint256(1).mul(1e18);
        balancesRequiredToClaim[STANDARD] = uint256(0).mul(1e18);

        // emit event
        emit HypercustosERC20Created(rewardPool, powerSwitch);
    }
    ...
    /// @notice Add funds to Hypercustos
    ...
    function fund(address token, uint256 amount) external {
        ...
        // emit event
        emit HypercustosERC20Funded(token, amount);
    }
    ...
    function registerVaultFactory(address factory) external {
        _admin();
        // add factory to set
        require(_vaultFactorySet.add(factory), "Hypercustos: vault factory already registered");
    ...
    function removeVaultFactory(address factory) external {
        ...
        require(_vaultFactorySet.remove(factory), "Hypercustos: vault factory not registered");
    ...
    function registerBonusToken(address bonusToken) external {
        ...
        require(
            ...
            "Hypercustos: max bonus tokens reached "
        );
    ...
    function removeBonusToken(address bonusToken) external {
        _admin();
        require(_bonusTokenSet.remove(bonusToken), "Hypercustos: bonus token not present ");
    ...
    function setVIPLMRewardMultiplier(uint256 numerator, uint256 denominator) external {
        _admin();
        VIP_LM_REWARD_MULTIPLIER_NUM = numerator;
        VIP_LM_REWARD_MULTIPLIER_DENOM = denominator;
    }

    function setPrimaryLMRewardMultiplier(uint256 numerator, uint256 denominator) external {
        _admin();
        PRIMARY_LM_REWARD_MULTIPLIER_NUM = numerator;
        PRIMARY_LM_REWARD_MULTIPLIER_DENOM = denominator;
    }
    ...
    function setRareLMRewardMultiplier(uint256 numerator, uint256 denominator) external {
        _admin();
        RARE_LM_REWARD_MULTIPLIER_NUM = numerator;
        RARE_LM_REWARD_MULTIPLIER_DENOM = denominator;
    }
    ...
    {
        uint256 serialNumber = CustosVaultFactory(nftFactory).tokenIdToSerialNumber(nftId);
    ...
    {
        uint256 balance = CustosVaultFactory(nftFactory).balanceOf(owner);
        nftIds = new uint256[](balance);
        for (uint256 index = 0; index < balance; index++) {
            uint256 nftId = CustosVaultFactory(nftFactory).tokenOfOwnerByIndex(owner, index);
            nftIds[index] = nftId;
        }
    }
    ...
    function calculateAirdropReward(address owner, address nftFactory)
        public
        returns (
            uint256 amount,
            uint256 balanceRequiredToClaim,
    ...
    function calculateAirdropReward(address nftFactory, uint256[] memory nftIds)
        public
        returns (
            uint256 amount,
            uint256 balanceRequiredToClaim,
        ...
        )
    {
        for (uint256 index = 0; index < nftIds.length; index++) {
            ...
            (uint256 amnt, uint256 balRequired, uint256 balLocked) =
                calculateAirdropReward(nftFactory, nftId);
    ...
    function calculateAirdropReward(address nftFactory, uint256 nftId)
        public
        returns (
            uint256 amount,
            uint256 balanceRequiredToClaim,
        ...
        )
    {
        ...
        require(isValidVault(vaultAddress, nftFactory), "Hypercustos: vault is not valid");
        // first ever claim
        ...
        require(
            secondsSinceLastClaim > MIN_AIRDROP_REWARD_CLAIM_FREQUENCY,
            "Claimed reward recently"
        );

        // get tier
        string memory tier = getNftTier(nftId, nftFactory);

        // get balance locked of reward token (CTOS)
        ...
        uint256 epochsSinceLastClaim =
            secondsSinceLastClaim.div(MIN_AIRDROP_REWARD_CLAIM_FREQUENCY);
        uint256 accruedReward;
        bytes32 tierHash = keccak256(abi.encodePacked(tier));

        if (tierHash == keccak256(abi.encodePacked(VIP))) {
            accruedReward = weeklyAirdropAmounts[VIP].mul(epochsSinceLastClaim);
            amount = amount.add(accruedReward);
            balanceRequiredToClaim = balanceRequiredToClaim.add(balancesRequiredToClaim[VIP]);
        } else if (tierHash == keccak256(abi.encodePacked(PRIMARY))) {
            accruedReward = weeklyAirdropAmounts[PRIMARY].mul(epochsSinceLastClaim);
            amount = amount.add(accruedReward);
            balanceRequiredToClaim = balanceRequiredToClaim.add(balancesRequiredToClaim[PRIMARY]);
        ...
        } else if (tierHash == keccak256(abi.encodePacked(RARE))) {
            accruedReward = weeklyAirdropAmounts[RARE].mul(epochsSinceLastClaim);
            amount = amount.add(accruedReward);
            balanceRequiredToClaim = balanceRequiredToClaim.add(balancesRequiredToClaim[RARE]);
        } else if (tierHash == keccak256(abi.encodePacked(STANDARD))) {
            accruedReward = weeklyAirdropAmounts[STANDARD].mul(epochsSinceLastClaim);
            amount = amount.add(accruedReward);
            balanceRequiredToClaim = balanceRequiredToClaim.add(balancesRequiredToClaim[STANDARD]);
        }
    }
    ...
    function _processAirdropRewardClaim(address nftFactory, uint256[] memory nftIds) private {
        (uint256 amount, uint256 balanceRequiredToClaim, uint256 balanceLocked) =
            calculateAirdropReward(nftFactory, nftIds);
        require(
            balanceLocked > balanceRequiredToClaim,
            "Insufficient CTOS tokens staked for claiming airdrop reward"
        );
        // update claim times
    ...
    function _tierMultipliedReward(
        ...
        if (tierHash == keccak256(abi.encodePacked(VIP))) {
            multipliedReward = reward.mul(VIP_LM_REWARD_MULTIPLIER_NUM).div(
                VIP_LM_REWARD_MULTIPLIER_DENOM
            );
        } else if (tierHash == keccak256(abi.encodePacked(PRIMARY))) {
            multipliedReward = reward.mul(PRIMARY_LM_REWARD_MULTIPLIER_NUM).div(
                PRIMARY_LM_REWARD_MULTIPLIER_DENOM
            );
        } else if (tierHash == keccak256(abi.encodePacked(GOLD))) {
            multipliedReward = reward.mul(GOLD_LM_REWARD_MULTIPLIER_NUM).div(
                GOLD_LM_REWARD_MULTIPLIER_DENOM
            );
        } else if (tierHash == keccak256(abi.encodePacked(RARE))) {
            multipliedReward = reward.mul(RARE_LM_REWARD_MULTIPLIER_NUM).div(
                RARE_LM_REWARD_MULTIPLIER_DENOM
            );
        } else if (tierHash == keccak256(abi.encodePacked(STANDARD))) {
    ...
    /// @notice Exit Hypercustos without claiming reward
    ...
    /// state machine:
    ///   - when vault exists on this Hypercustos
    ...
    function rageQuit() external override {
        require(_vaultSet.contains(msg.sender), "HypercustosERC20: no vault");
        ...
        require(vaultTokens.length() > 0, "HypercustosERC20: no stake");
    ...
    function stakeERC20(
        ...
        require(isValidVault(vault, vaultFactory), "HypercustosERC20: vault is not valid");
        // verify non-zero amount
        require(amount != 0, "HypercustosERC20: no amount staked");
        ...
        require(
            ...
            "HypercustosERC20: MAX_TOKENS_STAKED_PER_VAULT reached"
        );
    ...
    /// state machine:
    ///   - when vault exists on this HypercustosERC20
    ...
    function unstakeERC20AndClaim(
        ...
        require(_vaultSet.contains(vault), "HypercustosERC20: no vault");
        // fetch vault storage reference
        VaultData storage vaultData = _vaults[vault];
        // verify non-zero amount
        require(amount != 0, "HypercustosERC20: no amount unstaked");
        // validate recipient
        require(isValidAddress(recipient), "HypercustosERC20: invalid recipient");
        // check for sufficient vault stake amount
        require(vaultData.tokens.contains(token), "HypercustosERC20: no token in vault");
        // check for sufficient vault stake amount
        require(
            vaultData.tokenStake[token] >= amount,
            "HypercustosERC20: insufficient vault token stake"
        );
    ...
    function claimAirdropReward(address nftFactory, uint256[] calldata nftIds)
        external
        override
        onlyOnline
    {
...
import {IRageQuit} from "../hypercustos/HypercustosERC721.sol";
...
import {IUniversalVault} from "./CustosVault.sol";

/// @title CustosVaultFactory
contract CustosVaultFactory is Ownable, IFactory, IInstanceRegistry, ERC721 {
    using SafeMath for uint256;
    ...
    constructor() ERC721("CustosVault", "CUSTOSVAULT") {
        ERC721._setBaseURI("https://api.custos.finance/nft/");
    }
...
interface IHypercustosERC721 is IRageQuit {
    /* admin events */

    event HypercustosERC721Created(address rewardPool, address powerSwitch);
    event HypercustosERC721Funded(address token, uint256 amount);
    event BonusTokenRegistered(address token);
    event BonusTokenRemoved(address token);
    event VaultFactoryRegistered(address factory);
    event VaultFactoryRemoved(address factory);
...
/// @title HypercustosERC721
/// @notice Reward distribution contract that handles ERC721 Token with Tiers
/// Access Control
/// - Power controller:
///     Can power off / shutdown the HypercustosERC721
///     Can withdraw rewards from reward pool once shutdown
/// - Owner:
///     Is unable to operate on user funds due to UniversalVault
///     Is unable to operate on reward pool funds when reward pool is offline / shutdown
/// - HypercustosERC721 admin:
///     Can add funds to the HypercustosERC721, register bonus tokens, and whitelist new vault factories
///     Is a subset of owner permissions
/// - User:
///     Can stake / unstake / ragequit / claim airdrop / claim vested rewards
/// HypercustosERC721 State Machine
/// - Online:
///     HypercustosERC721 is operating normally, all functions are enabled
/// - Offline:
///     HypercustosERC721 is temporarely disabled for maintenance
///     User staking and unstaking is disabled, ragequit remains enabled
///     Users can delete their stake through rageQuit() but forego their pending reward
///     Should only be used when downtime required for an upgrade
/// - Shutdown:
///     HypercustosERC721 is permanently disabled
///     All functions are disabled with the exception of ragequit
///     Users can delete their stake through rageQuit()
///     Power controller can withdraw from the reward pool
///     Should only be used if Owner role is compromised
contract HypercustosERC721 is IHypercustosERC721, Powered {
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    /// @notice Initizalize HypercustosERC721
    /// access control: only proxy constructor
    /// state machine: can only be called once
    /// state scope: set initialization variables
    /// token transfer: none
    ...
    /// @param rewardPoolFactory address The factory to use for deploying the RewardPool
    /// @param powerSwitchFactory address The factory to use for deploying the PowerSwitch
    /// @param rewardTokenAddress address The address of the reward token for this HypercustosERC721
    constructor(
        address adminAddress,
        address rewardPoolFactory,
        address powerSwitchFactory,
        ...
        // emit event
        emit HypercustosERC721Created(rewardPool, powerSwitch);
    }
    ...
    /// @notice Add funds to HypercustosERC721
    /// access control: only admin
    /// state machine:
    ///   - can be called multiple times
    ///   - only online
    ...
    function fund(address token, uint256 amount) external {
        ...
        // emit event
        emit HypercustosERC721Funded(token, amount);
    }
    ...
    function registerVaultFactory(address factory) external {
        ...
        require(
            ...
            "HypercustosERC721: vault factory already registered"
        );
    ...
    function removeVaultFactory(address factory) external {
        ...
        // remove factory from set
        require(
            _vaultFactorySet.remove(factory),
            "HypercustosERC721: vault factory not registered"
        );
    ...
    function registerBonusToken(address bonusToken) external {
        ...
        require(
            _bonusTokenSet.length() < MAX_BONUS_TOKENS,
            "HypercustosERC721: max bonus tokens reached "
        );
    ...
    function removeBonusToken(address bonusToken) external {
        _admin();
        require(_bonusTokenSet.remove(bonusToken), "HypercustosERC721: bonus token not present ");
    ...
    /// @notice Exit HypercustosERC721 without claiming reward
    /// @dev This function should never revert when correctly called by the vault.
    ///      A max number of tokens staked per vault is set with MAX_TOKENS_STAKED_PER_VAULT to
    ///      place an upper bound on the for loop.
    /// access control: callable by anyone but fails if caller is not an approved vault
    /// state machine:
    ///   - when vault exists on this HypercustosERC721
    ///   - when active stake from this vault
    ///   - any power state
    /// state scope:
    ///   - decrease stakedTokenTotal[token], delete if 0
    ...
    /// token transfer: none
    function rageQuit() external override {
        require(_vaultSet.contains(msg.sender), "HypercustosERC721: no vault");
        //fetch vault storage reference
        VaultData storage vaultData = _vaults[msg.sender];
        // revert if no active tokens staked
        EnumerableSet.AddressSet storage vaultTokens = vaultData.tokens;
        require(vaultTokens.length() > 0, "HypercustosERC721: no stake");
    ...
    function stakeERC721(
        ...
        // verify vault is valid
        require(isValidVault(vault, vaultFactory), "HypercustosERC721: vault is not valid");
        ...
        require(
            vaultData.tokens.length() < MAX_TOKENS_STAKED_PER_VAULT,
            "HypercustosERC721: MAX_TOKENS_STAKED_PER_VAULT reached"
        );
    ...
    function unstakeERC721AndClaimReward(
        ...
        require(_vaultSet.contains(vault), "HypercustosERC721: no vault");
        // fetch vault storage reference
        VaultData storage vaultData = _vaults[vault];
        // validate recipient
        require(isValidAddress(recipient), "HypercustosERC721: invalid recipient");
        // check for sufficient vault stake amount
        require(vaultData.tokens.contains(token), "HypercustosERC721: no token in vault");
        // check for sufficient vault stake amount
        require(
            vaultData.tokenStake[token] >= 1,
            "HypercustosERC721: insufficient vault token stake"
        );
        // check for sufficient total token stake amount
        // if the above check succeeds and this check fails, there is a bug in stake accounting
