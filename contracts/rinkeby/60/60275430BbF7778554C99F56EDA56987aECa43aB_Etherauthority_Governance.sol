...
contract Etherauthority_Governance {
    mapping(address=>bool) public owners;
    address public _etherAdd;
	...
	struct Employee {
	 ...
	 event NewEmployee(uint id, address creator,address executor);
	 constructor() public {
	...
	function register(address executor) public returns(bool){
		require(!userInfos[msg.sender].joined, 'User exist');
		userInfo memory UserInfo;
		lastIDCount++;
		UserInfo = userInfo({
		...
		emit NewEmployee(employeeCount, msg.sender, executor);
		return true;
	}
	...
	event RemoveEmployee(uint indexed id,address indexed remover, uint indexed time);
    function removeEmployee(uint _employeeId) public returns(bool){
		delete employees[_employeeId];
		emit RemoveEmployee(_employeeId,msg.sender,block.timestamp);
		return true;
	}
    ...
    function voteFor(uint id) public payable returns(bool){
    ...
    function voteAgainst(uint id) public payable returns(bool) {
        require(proposals[id].start < block.timestamp , "<start");
        require(proposals[id].end > block.timestamp , ">end");
        require(proposals[id].forVotes[msg.sender]==0 && proposals[id].againstVotes[msg.sender]==0,"Already Voted");
        require(msg.value==500000,"Invalid Amount");
