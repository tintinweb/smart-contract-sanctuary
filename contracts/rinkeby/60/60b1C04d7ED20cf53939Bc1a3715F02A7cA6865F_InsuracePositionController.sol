...
contract InsuracePositionController is AbstractController {
    ...
    function __InsuracePositionController_init(
        ...
        if (_stakingAssetAddress[0] != ETHER) {
    ...
    function stake(uint256 _amount) external override onlyIndex {
        ...
        if(_stakingAsset == ETHER){
            investmentAmount =  address(this).balance;
         } else {
            investmentAmount = IERC20(_stakingAsset).balanceOf(address(this));
         }
    ...
    */
    ...
    function outstandingRewards() public view override returns (uint256) {
        return _stakersPool.showHarvestRewards(address(this), address(_lpToken));
    }
    ...
    function netWorth() external view override returns (uint256) {
        ...
        return
            _priceFeed.howManyBaseAssetTokensInInsur(
                address(base),
...
abstract contract AbstractController is OwnableUpgradeable, IPositionController {
    ...
    function _swapInsuraceAsset(
        ...
        if (_investAsset == ETHER) {
            address[] memory pairs = new address[](2);
            pairs[0] = address(_baseAsset);
            pairs[1] = _priceFeed.wethToken();
