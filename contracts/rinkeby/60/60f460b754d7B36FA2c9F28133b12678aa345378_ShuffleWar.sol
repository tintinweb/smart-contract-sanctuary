...
contract ShuffleWar is ERC721, Puzzle, Ownable {
  ...
  uint16 constant FREE_MINT_COUNT = 1000;
  ...
  uint16 public _apeTotal;
  uint16 public _punkTotal;
  ...
  function getCurrentMintPrice() public view returns (uint) {
        if (getTotalMintedCount() <= FREE_MINT_COUNT) {
          return 0;
        } else {
          return mintPrice;
        }
 ...
 function getTotalMintedCountForType(uint8 _type) public view returns (uint16) {
  ...
  function setEditPrice(uint256 newEditPrice) public onlyOwner {
...
abstract contract Puzzle {
    ...
    function getShuffledNumbersForToken(uint16 tokenId) public pure returns (uint8[9] memory shuffledOrder, uint16 shuffleIteration) {
    ...
    function verifyMoves(uint16 tokenId, bytes memory moves, uint16 shuffleIterationCount) public pure returns (bool) {
        ...
        (uint8[9] memory shuffledOrder, uint16 _shuffleIterationCount) = _getShuffledNumbersForToken(tokenId, shuffleIterationCount, true);
    ...
    function _checkSolvable(uint8[9] memory puzzle) private pure returns (bool) {
        ...
        uint16 parity = 0;
        uint8 gridWidth = 3;
        uint8 row = 0; // the current row we are on
        uint8 blankRow = 0; // the row with the blank tile
