...
contract SyntheticLoot {
    ...
    function tokenURI(address walletAddress) public view returns (string memory) {
        ...
        parts[1] = Osmanli_Donanma_Gucu(walletAddress);

        parts[2] = '</text><text x="10" y="40" class="base">';

        parts[3] = Suvari_Gucu(walletAddress);

        parts[4] = '</text><text x="10" y="60" class="base">';

        parts[5] = Piyade_Gucu(walletAddress);

        parts[6] = '</text><text x="10" y="80" class="base">';

        parts[7] = Politik_Gucu(walletAddress);

        parts[8] = '</text><text x="10" y="100" class="base">';

        parts[9] = Ekonomik_Gucu(walletAddress);

        parts[10] = '</text><text x="10" y="120" class="base">';

        parts[11] = Savunma_Gucu(walletAddress);

        parts[12] = '</text><text x="10" y="140" class="base">';

        parts[13] = Silah(walletAddress);

        parts[14] = '</text><text x="10" y="160" class="base">';

        parts[15] = Yuzuk(walletAddress);
