...
// 
contract iAnonymice is ERC721Enumerable {
    /*
  ___                                    _          
 / _ \                                  (_)         
...
\_| |_/_| |_|\___/|_| |_|\__, |_| |_| |_|_|\___\___|
                         ...
                         |___/                      
*/
    using iAnonymiceLibrary for uint8;
 ...
 |_|  |_|_|_||_\__|_|_||_\__, | |_| \_,_|_||_\__|\__|_\___/_||_/__/
   ...
   */
    ...
    function mintInternal() internal {
        ...
        require(!iAnonymiceLibrary.isContract(msg.sender));
        ...
        returns (string memory)
    {
        ...
        for (uint8 i = 0; i < 9; i++) {
            uint8 thisTraitIndex = iAnonymiceLibrary.parseInt(
                iAnonymiceLibrary.substring(_hash, i, i + 1)
            );
            ...
            for (
                ...
                string memory thisPixel = iAnonymiceLibrary.substring(
                ...
                uint8 x = letterToNumber(
                    iAnonymiceLibrary.substring(thisPixel, 0, 1)
                );
                uint8 y = letterToNumber(
                    iAnonymiceLibrary.substring(thisPixel, 1, 2)
                );
                ...
                svgString = string(
                    abi.encodePacked(
                        svgString,
                        "<rect class='c",
                        iAnonymiceLibrary.substring(thisPixel, 2, 4),
                        "' x='",
                        x.toString(),
                        "' y='",
                        y.toString(),
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    iAnonymiceLibrary.encode(
                        bytes(
                            string(
                                abi.encodePacked(
                                    '{"name": "Anonymice #',
                                    iAnonymiceLibrary.toString(_tokenId),
        ...
        returns (string memory)
    {
        ...
        if (ownerOf(_tokenId) == 0x000000000000000000000000000000000000dEaD) {
            tokenHash = string(
                abi.encodePacked(
                    "1",
                    iAnonymiceLibrary.substring(tokenHash, 1, 9)
                )
            );
        }
