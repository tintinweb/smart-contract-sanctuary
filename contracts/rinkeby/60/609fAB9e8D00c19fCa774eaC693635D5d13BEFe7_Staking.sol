...
contract Staking {
    ...
    function stake(uint8 _period, uint8 _amount) public {
        ...
        uint256 stakingPeriod = periods[_period];
    ...
    function getUSDTPrice(uint256 _amount) public view returns(uint256){
        IUniswapV2Pair lime_weth = IUniswapV2Pair(LIME_WETHPairAddress);
        (uint256 ml, uint256 me,) = lime_weth.getReserves();
        IUniswapV2Pair weth_usdt = IUniswapV2Pair(WETH_USDTPairAddress);
    ...
    function setItemToRates(uint256 _row, uint256 _col, uint256 _value) public restricted {
        uint256 length = rates.length;
        if(_row >= length){
            for(uint i = 0; i < _row - length + 1; i++){
                rates.push();
            }
        }
        length = rates[_row].length;
        if(_col >= length){
            for(uint i = 0; i < _col - length + 1; i++){
                rates[_row].push();
            }
        }
    ...
    function removeItemFromRates(uint256 _row, uint256 _col) public restricted {
        ...
        for (uint256 i = _col; i < rates[_row].length - 1; i++){
            rates[_row][i] = rates[_row][i + 1];
        }
        rates[_row].pop();
    }
    function resetArrays() public restricted {
        periods = [90, 180, 360];
        amounts = [20000, 60000, 120000, 260000, 510000, 900000];
        rates = [[26, 28, 30, 32, 34, 36],
                    [65, 70, 75, 80, 85, 90],
