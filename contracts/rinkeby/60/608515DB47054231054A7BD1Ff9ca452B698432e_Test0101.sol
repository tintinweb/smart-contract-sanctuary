...
contract Test0101 is ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  // ERC721 params
  string private tokenName = 'Test0101';
  string private tokenId = 'T010';
  string private _baseTokenURI =
    'ipfs://QmYSp8bXno4SfDihmg5dJr8qAy4Tuc7N8t6WBRsCMvrD52/';

  // Withdraw address
  address private WITHDRAW_ADDRESS = 0x89DA91eaD9e61C85d95B088BdE49A1Ea6D307536; // TODO: type the right withraw address

  // ECDSA
  address private constant SIGNER_ADDRESS =
    0xFA025098d2298c5a3f6bE56B7C9B0Be63098E2B5; // TODO: change it
  mapping(string => bool) private _isNonceUsed;

  // Collection params
  uint256 public constant TOT = 8888;
  uint256 public constant GIVEAWAY = 88; // TODO: type the right number of giveaways
  uint256 public constant MINTABLE = TOT - GIVEAWAY;
  uint256 public constant PUBLIC_MINT_LIMIT = 8;
  uint256 public constant PRICE = 0.1 ether;

  // Utils
  uint256 public currentTokenId;
  uint256 public giveawaySupply = GIVEAWAY;
  uint256 public mintableSupply = MINTABLE;
  ...
  // CLOSED (0) <- only for consistency
  // DIAMOND TIER (9 premint + 1 for free)
  // GOLD TIER (4 premint)
  // SILVER TIER (2 premint)
  // BRONZE TIER (1 premint)
  ...
  // Private premint function for Diamond, Gold, Silver and Bronze tiers
  function premint(uint256 _amount) private {
    ...
    if (
      _premintTier[msg.sender] == MintStatus.DIAMOND &&
  ...
  function publicMint(uint256 _amount) private {
    require(_mintStatus == MintStatus.PUBLIC, 'Minting is closed for public');
    require(
      _amount <= PUBLIC_MINT_LIMIT,
  ...
  // Private mint function for raffle mint -> Only 1 NFT
  function raffleMint(uint256 _amount) private {
    require(_mintStatus == MintStatus.RAFFLE, 'Raffle is closed');
    require(
      _amount <= TIER_LIMIT[uint256(_mintStatus)],
      'Exceeded the max amount of NFT mintable in raffle tier'
    );
    ...
    nonReentrant
  {
    if (_mintStatus == MintStatus.PUBLIC) {
      publicMint(_amount);
    } else if (_mintStatus == MintStatus.RAFFLE) {
      raffleMint(_amount);
    } else {
      premint(_amount);
    }
  ...
  function addPremint(address[] calldata _addresses) external onlyOwner {
    for (uint256 i = 0; i < _addresses.length; i++) {
      if (_premintList[_addresses[i]]) {
        ...
        _premintClaimed[_addresses[i]] = 0;
        _premintAvailable[_addresses[i]] = 1;
        _premintTier[_addresses[i]] = MintStatus.BRONZE;
      }
    }
  }
    ...
    returns (string memory)
  {
    ...
    if (reveal) {
      ...
      return string(abi.encodePacked(_baseTokenURI, 'default'));
    }
  }
