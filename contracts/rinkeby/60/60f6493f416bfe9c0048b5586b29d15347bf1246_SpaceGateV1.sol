...
contract SpaceGateV1 is Ownable{
    ...
    struct GateInfo {
        ...
        uint256 claimFee; // Fee claimed based in amount
        uint256 fee; // Fee system based in percent
    ...
    // deactive existed gate (*)
    function unActiveGate(
        string memory _gate
    )
        public
        onlyOwner()
        ...
        onlyOwner()
    {
        ...
        Gates[_gate].flag = true;
        Gates[_gate].min = _min;
        Gates[_gate].contractToken = _contractToken;
        Gates[_gate].max = _max;
        Gates[_gate].fee = _fee;
        Gates[_gate].claimFee = _claimFee;
        Gates[_gate].totalFee = 0;
        Gates[_gate].totalClaim = 0;
    }
        ...
        external
    {
        ...
        uint256 claimFee = Gates[_gate].claimFee; 
        uint256 fee = _amount.mul(Gates[_gate].fee).div(Percent);
        uint256 amountWithFee = _amount.add(fee).add(claimFee);
        
        bool isToken = _contractToken != address(0);
