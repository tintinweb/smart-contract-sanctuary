  ...
  NFTMarketReserveAuction
{
  ...
  function adminUpdateConfig(
    ...
    _updateReserveAuctionConfig(minBidIncrementInBasisPoints, duration);
...
{
  /**
   * @dev If the creator is not available then fees should be sent to the current owner instead.
   */
  function getCreator(address nftContract, uint256 tokenId) internal view returns (address payable) {
    try FNDNFT721(nftContract).tokenCreator(tokenId) returns (address payable creator) {
      return creator;
    } catch {
      return address(0);
    }
  ...
  SendValueWithFallbackWithdraw
{
  ...
  mapping(address => mapping(uint256 => bool)) private nftContractToTokenIdToPrimaryCompleted;
  ...
  function _getIsPrimary(
    ...
    return !nftContractToTokenIdToPrimaryCompleted[nftContract][tokenId] && getCreator(nftContract, tokenId) == seller;
  }
  ...
  /**
   * @notice Returns how funds will be distributed for a sale.
   * @dev On the primary sale fees will be reported as `ownerRev`.
   */
  function getFees(
    address nftContract,
    uint256 tokenId,
    ...
    )
  {
    ...
    if (_getIsPrimary(nftContract, tokenId, seller)) {
      ...
      creatorFeeBasisPoints = _secondaryCreatorFeeBasisPoints;
    }
    // SafeMath is not required for the divs below since we are dividing by a constant value > 0.
    foundationFee = price.mul(foundationFeeBasisPoints) / BASIS_POINTS;
    creatorSecondaryFee = price.mul(creatorFeeBasisPoints) / BASIS_POINTS;
    ...
    )
  {
    ...
    nftContractToTokenIdToPrimaryCompleted[nftContract][tokenId] = true;
  ...
  /**
   * @dev Currently used by the temporary adminMigrateNFT function.
   */
  function _enterSecondarySaleState(address nftContract, uint256 tokenId) internal {
    nftContractToTokenIdToPrimaryCompleted[nftContract][tokenId] = true;
  }
 ...
 * @dev This contract will be expanded with reusable calls and data as more auction types are added.
 */
  ...
  NFTMarketAuction
{
  ...
  uint256 private _minBidIncrementInBasisPoints;
   ...
   * or bubbles the call up to check the current owner instead.
   */
   ...
   */
  function getReserveAuctionConfig() public view returns (uint256 minBidIncrementInBasisPoints, uint256 duration) {
    minBidIncrementInBasisPoints = _minBidIncrementInBasisPoints;
    duration = _duration;
  }
  ...
  function _updateReserveAuctionConfig(uint256 minBidIncrementInBasisPoints, uint256 duration) internal {
    require(minBidIncrementInBasisPoints <= BASIS_POINTS, "NFTMarketReserveAuction: Min increment must be <= 100%");
    require(duration > 0, "NFTMarketReserveAuction: Duration must be greater than 0");
    // Cap the max duration so that overflows will not occur
    require(duration <= MAX_MAX_DURATION, "NFTMarketReserveAuction: Duration must be <= 1000 days");
    require(EXTENSION_DURATION <= duration, "NFTMarketReserveAuction: Extension must be <= duration");
    _minBidIncrementInBasisPoints = minBidIncrementInBasisPoints;
    _duration = duration;

    emit ReserveAuctionConfigUpdated(minBidIncrementInBasisPoints, 0, duration, EXTENSION_DURATION, 0);
  }

  /**
   * @notice Creates an auction for the given NFT.
   * The NFT is held in escrow until the auction is finalized or canceled.
   * @dev ATM only approved creators can create auctions for Foundation approved NFTs.
   */
  ...
  function cancelReserveAuction(uint256 auctionId) public {
    ...
    // Update state first to avoid possible reentrancy
  ...
  function placeBid(uint256 auctionId) public payable nonReentrant {
    ...
    // Cache and update state before a possible reentrancy
  ...
  /**
   * @notice Once the countdown has expired for an auction, anyone can finalize it.
  ...
  function finalizeReserveAuction(uint256 auctionId) public nonReentrant {
    ...
    // By deleting first we are more resilient to potential reentrancy attacks
    delete nftContractToTokenIdToAuctionId[auction.nftContract][auction.tokenId];
    delete auctionIdToAuction[auctionId];
  ...
  function _getMinBidAmountForReserveAuction(uint256 currentBidAmount) private view returns (uint256) {
    uint256 minIncrement = currentBidAmount.mul(_minBidIncrementInBasisPoints) / BASIS_POINTS;
    if (minIncrement == 0) {
