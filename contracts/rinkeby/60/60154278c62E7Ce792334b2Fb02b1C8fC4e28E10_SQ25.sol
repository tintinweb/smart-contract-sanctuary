...
contract SQ25 is Ownable, ERC721A, ReentrancyGuard {
    ...
    function mint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable callerIsUser  {
        ...
        if (msg.sender != owner()) {
            ...
            if (sale == Sale.WHITELIST) {
               ...
               require(_mintAmount <= maxBatchSize, "Too many items minted");
  ...
  // Token reserved for team
    function reserve(uint256 _mintReserve, address destination) public onlyOwner  {
        uint256 supply = totalSupply();
        require(_mintReserve > 0, "mint amount > 0");
        require(_mintReserve <= maxBatchSize, "mint amount < batch");
        require(supply + _mintReserve <= collectionSize, "max NFT limit exceeded");
        _safeMint(destination, _mintReserve);
    ...
    function withdraw() public payable onlyOwner nonReentrant{
        (bool yay,) = payable(msg.sender).call{value : address(this).balance}("");
        require(yay, "WITHDRAW ERROR");
    }
