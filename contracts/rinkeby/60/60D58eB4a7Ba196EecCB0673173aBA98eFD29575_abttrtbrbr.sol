...
contract abttrtbrbr is ERC721PresetMinterPauserAutoId, VRFConsumerBase, Data  {
    ...
    address public tokenAddress = 0x0CC8c52D259aCb911E0Eec1Ac920fAA727b840e7;
    
    uint256 internal reward = 1 * 10**18; // 1 token
    uint256 internal passPrice = 100 * 10**18; // 100 token
    uint256 internal nftPrice = 1570 * 10**18; // 1570 token

    uint256 internal ONE_DAY = 86400; // 1 day
    uint256 internal ONE_HOUR = 3600; // 1 hour
    ...
    event Hatched(uint256 tokenId, uint256 timestamp, address tokenOwner, uint256 frogColor);
    event Feed(uint256 tokenId, uint256 feedLevel);
    event Play(uint256 tokenId, uint256 newGenerationTick);
    ...
    constructor(
        ...
        ERC721PresetMinterPauserAutoId("brsawed", "GEAED", "ipfs://") 
    {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (deviceGrowthStage(tokenId) == DeviceGeneration.Gen1) {
            return string(abi.encodePacked(_baseURI(), nameIPFS[keccak256(abi.encode(name))]));
        }
    ...
    // initate purchasable nfts
    function enableMint() public onlyOwner {
        mintEnabled = true;
    }

    // initate hatchable nfts
    function enableHatching() public onlyOwner {
    ...
    function purchaseNFT() public {
        require(mintEnabled, "!mintEnabled");
        require(TokenInterface(tokenAddress).allowance(msg.sender, address(this)) >= nftPrice, "!approved");
        require(TokenInterface(tokenAddress).balanceOf(msg.sender) >= nftPrice, "!balanceOf");
    ...
    function hatch(uint256 tokenId) public {
        require(hatchingEnabled, "!hatchingEnabled");
        require(_isApprovedOrOwner(_msgSender(), tokenId), "!owner");
        
        Device storage device = unit[tokenId];
        
        require(device.createdAt == MAX_TIMESTAMP, "!egg");
    ...
    function getRandomColor(uint256 tokenId) internal returns (bytes32 requestId) {
        require(LINK.balanceOf(address(this)) >= vrfFee, "!LINK");
    ...
    function play(uint256 tokenId) public {
        ...
        uint256 timeBetweenPlays = ONE_HOUR; // 1 hr
        uint256 elapsed = block.timestamp.sub(device.lastPlayTime);
        
        require(elapsed > timeBetweenPlays, "!timeBetweenPlays");
        require(device.generation <= 9, "must prestige");
        ...
        emit Play(tokenId, device.generationTicker);
        return;
    }
    ...
    function generationUp(uint256 tokenId) internal {
        ...
        uint256 maxPlaysPerGen = 3; // 3 for testing
            ...
            if (device.generation < 9) {
                ...
                for (uint i = 0; i < 9; i++) {
                    ...
                    if (device.generation == i) {
                        ...
                        emit GenerationUp(tokenId, device.generation, amount);
                        return;
                    }
                }
            }
    ...
    function prestige(uint256 tokenId) public {
        ...
        require(keccak256(bytes(device.name)) != keccak256(bytes(nameList[0])) ||
                keccak256(bytes(device.name)) != keccak256(bytes(nameList[10])) ||
                keccak256(bytes(device.name)) != keccak256(bytes(nameList[20])) ||
                keccak256(bytes(device.name)) != keccak256(bytes(nameList[30])) ||
                keccak256(bytes(device.name)) != keccak256(bytes(nameList[40])), "isGen1");
        require(device.rewards == 0, "rewards > 0");
        require(device.generation == 10, "!gen10");
        require(device.feedLevel > fullCycles, "must feed");
    ...
    function getReward(uint256 tokenId) public {
        ...
        require(device.rewards > 0, "!rewards");
    ...
    function purchasePass(uint256 tokenId, uint256 amount) public {
        ...
        require(TokenInterface(tokenAddress).allowance(msg.sender, address(this)) >= amount.mul(passPrice), "!approved");
        require(TokenInterface(tokenAddress).balanceOf(msg.sender) >= amount.mul(passPrice), "!balanceOf");
    ...
    function redeemPass(uint256 tokenId, uint256 amount) public {
        ...
        require(amount <= device.availableFreeDays, "!days");
    ...
    function revival(uint256 tokenId) public {
        ...
        require(TokenInterface(tokenAddress).allowance(msg.sender, address(this)) >= device.tokensToRevive, "!approved");
        require(TokenInterface(tokenAddress).balanceOf(msg.sender) >= device.tokensToRevive, "!CPETBalance");
        
        require(device.rewards == 0, "rewards > 0");
        require(keccak256(bytes(device.name)) != keccak256(bytes(egg)), "isEgg");
    ...
    function internalData() public view returns(
        uint256 reward_, uint256 passPrice_, uint256 nftPrice_,
        uint256 ONE_DAY_, uint256 ONE_HOUR_, uint256 MAX_TIMESTAMP_,
        uint256 pendingRewards_, uint256 paidRewards_, 
        bool mintEnabled_, bool hatchingEnabled_ 
        ) {
            
        return (
            reward, passPrice, nftPrice,
            ONE_DAY, ONE_HOUR, MAX_TIMESTAMP,
            pendingRewards, paidRewards,
            mintEnabled, hatchingEnabled
        );
    }
    
    function getOwnersTokenIds(address account) public view returns (uint256[] memory accountsTokens_) {
        uint256 currentBalance = balanceOf(account);
        uint256[] memory tokenIdArray = new uint[](currentBalance);
        
        for(uint256 i = 0; i < currentBalance; i++) {
            tokenIdArray[i] = tokenOfOwnerByIndex(account, i);
        }
        return tokenIdArray;
    }
    ...
    function getTimeData(uint256 tokenId) public view returns (
        bool isItPlayTime_, uint256 secondsUntilNextPlay_, uint256 secondsSinceLastPlay_, 
        ...
        uint256 elapsed;
        uint256 timeInbetweenPlays = 3600;
        ...
        if (device.createdAt != MAX_TIMESTAMP && alive(tokenId)) {
            ...
            uint256 secondsUntilNextFeed = device.feedLevel.mul(ONE_DAY).sub(feedElapsedTime);
            
            if (block.timestamp.sub(device.lastPlayTime) > timeInbetweenPlays) {
                elapsed = block.timestamp.sub(device.lastPlayTime);
                
                if (device.feedLevel > fullCycles) {
                    return (true, 0, elapsed, false, secondsUntilNextFeed, feedWindow );
                }
                
                if (device.feedLevel == fullCycles && modulo < feedWindow)  {
                    return (true, 0, elapsed, true, 0,  feedWindow.sub(modulo));
                }
            } else {
                elapsed = block.timestamp.sub(device.lastPlayTime);
                
                if (device.feedLevel > fullCycles) {
                    return (false, timeInbetweenPlays.sub(elapsed), elapsed, false, secondsUntilNextFeed, feedWindow);
                }
                
                if (device.feedLevel == fullCycles && modulo < feedWindow)  {
                    return (false, timeInbetweenPlays.sub(elapsed), elapsed, false, 0, feedWindow.sub(modulo));
                }
            }
        }
        return (false, MAX_TIMESTAMP, MAX_TIMESTAMP, false, MAX_TIMESTAMP, MAX_TIMESTAMP);
    }
...
contract Data is Ownable {
    ...
    string internal constant egg = "egg";
    string public EGG_IPFS;
    string public DEAD_IPFS;
    
    string[50] public nameList;
    string[50] public nameIPFSConstant;
    
    function a_change_nameIPFSConstant(string[50] memory newIpfs) public onlyOwner {
        nameIPFSConstant = newIpfs;
    }
    
    function a_change_nameListConstant(string[50] memory newName) public onlyOwner {
        nameList = newName;
    }
    
    function a_change_EGG_IPFS(string memory _EGG_IPFS) public onlyOwner {
        EGG_IPFS = _EGG_IPFS;
    }
    
    function a_change_DEAD_IPFS(string memory _DEAD_IPFS) public onlyOwner {
        DEAD_IPFS = _DEAD_IPFS;
    }
}
