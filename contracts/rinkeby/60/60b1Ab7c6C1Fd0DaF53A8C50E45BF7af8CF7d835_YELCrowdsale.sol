 ...
 */
contract YELCrowdsale is TimedCrowdsale, AllowanceCrowdsale {
    using SafeMath for uint256;
    ...
    constructor (uint256 initialRate, uint256 factor, uint256 threshold,
        uint256 openingTime, uint256 closingTime, IERC20 token, address tokenWallet)

        Crowdsale(initialRate, address(uint160(tokenWallet)), token)
     ...
     * Note that, as price _increases_ after the purchases exceed a certain threshold.
     ...
     */
    function _getTokenAmount(uint256 weiAmount) internal view returns (uint256) {
        return weiAmount.mul(_initialRate);
    }
    ...
    function withdrawTokens(address beneficiary) public {
        ...
        amount = amount.mul(getCurrentRate()).div(_initialRate);
    ...
    function balanceOf(address account) public view returns (uint256) {
        ...
        return amount.mul(getCurrentRate()).div(_initialRate);
    }
}
