 ...
 */
contract ChaosArtNFT is ERC721Tradable {
...
abstract contract ERC721Tradable is ContextMixin, ERC721Enumerable, NativeMetaTransaction, Ownable {
    ...
    mapping(uint256 => bytes32) tokenURIs;
     ...
     * @param _tokenURI IPFS hash of the token metadata JSON
     */
    function mintToken(address _to, bytes32 _tokenURI) public onlyOwner {
    ...
    function tokenURI(uint256 _tokenId) override public view returns (string memory) {
        if(tokenURIs[_tokenId] != 0) {
            string memory ipfsHashStr = _toBase58(abi.encodePacked(IPFS_PREFIX, tokenURIs[_tokenId]));
            return string(abi.encodePacked(baseTokenURI(), ipfsHashStr, "/metadata.json"));
        } else {
            return "";
        }
    }
