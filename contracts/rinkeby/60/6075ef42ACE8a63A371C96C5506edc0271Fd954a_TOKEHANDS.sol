...
contract TOKEHANDS is ERC721, Ownable {
    ...
    uint256 private saleDate = 1642941979; // dev date
        ...
        mintCompliance(msg.sender, _qty)
    {
        ...
        if (!isMintActive()) revert("Not on sale");
        if (ownerTokenCount + _qty >= getMaxPerWallet()) revert("Max TKHN");
        if (msg.value < cost * _qty) revert("Low Funds");
    ...
    function redeemForTokenId(uint256 _tokenId) public onlyOwner {
        redeemedForTattoo[_tokenId] = true;
    }

    function redeemByIds(uint256[] memory _tokehandsIds) public {
        if (!revealed) revert("Claiming is not active at the moment");
