...
contract OneCoinGlobal is Ownable {
    ...
    */
    function userInfo(address _addr) 
        view 
        external 
        returns(address _referral, uint256 _referral_num, uint256 _is_crowd, uint256 _payout_token, uint256 _payout_eth, uint256 _referral_bonus, uint256 _match_bonus, uint256 _holder_single_bonus, uint256 _holder_full_bonus,uint256 _last_payout) 
    {
        Player storage player = players[_addr];
        uint256 payout = this.payoutOf(_addr);
        uint256 payout_eth = this.payoutEthOf(_addr);
