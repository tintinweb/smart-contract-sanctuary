...
contract CryptoRabbit is ERC721, Ownable {
    ...
    function adoptRabbit(uint256 numCryptoRabbit) public payable {
        ...
        for (uint i = 0; i < numCryptoRabbit; i++) {
            ...
            _setTokenURI(mintIndex, bytesToHexString(keccak256(abi.encodePacked(mintIndex,"rabbit"))));
        }
    }

    function bytesToHexString(bytes32 _input) private pure returns (string memory hexString) {
        assembly {
            hexString := mload(add(_input,32))
        } 
    }
