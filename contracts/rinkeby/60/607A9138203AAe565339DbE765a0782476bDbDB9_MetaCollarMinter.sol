...
contract MetaCollarMinter is MetaCollar {
    ...
    mapping(address => uint256) public _whitelistTotals;
    ...
    */
    function mint(uint256 quantity) payable public virtual nonReentrant whenNotSoldOut {
        require(!paused, 'Paused');

        if(presaleLive) {
            require(isOnAWhitelist(_msgSender()), '!whitelist');
            require(msg.value >= PRESALE_PRICE * quantity, '<cost');
            require(_whitelistTotals[_msgSender()] + quantity  < 9, "whitelist limit");
        }else{
            require(msg.value >= FULL_PRICE*quantity, 'Less than cost');
        }
    ...
    */
    // Update this to airdrop 800 on a separate id
    function mintByOwner(address to, uint256 quantity) public nonReentrant onlyOwner{
        for(uint256 i= 0; i < quantity; i++){
    ...
    WhiteList
{
    ...
    uint256 public currentTokenId = 801; // Starting at 801 becase 1-800 are held back for airdrops.
...
contract WhiteList is Ownable{
    ...
    mapping(address => uint256) whitelistTotals;
