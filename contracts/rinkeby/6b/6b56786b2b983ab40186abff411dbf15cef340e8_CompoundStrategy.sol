...
contract CompoundStrategy is IRhoStrategy, PausableUpgradeable, AccessControlEnumerableUpgradeable {
    ...
    function initialize(address _underlyingAddr, address _ctokenAddr, address _comptrollerAddr) public initializer {
    ...
    function getSupplyRate() external override pure returns (uint256 _amount) {
        return 1;
    }
    function getDeployableAmount() external override pure returns (uint256 _amount) {
        return 1;
    }
    ...
    function withdrawAll() external override onlyRole(VAULT_ROLE) {
        require(cToken.redeem(cToken.balanceOf(address(this))) == 0, "CompoundStrategy: Fail to withdraw from compound");
    }
    ...
    function setRewardConversionThreshold(uint256 _threshold) external override onlyRole(VAULT_ROLE) returns (uint256) {
        _rewardConversionThreshold = _threshold;
        return _rewardConversionThreshold;
    }
    function rewardConversionThreshold() external view override returns (uint256) {
        return _rewardConversionThreshold;
    }
...
interface IRhoStrategy {
     ...
     */
    function getSupplyRate() external returns (uint256 _amount);

    /**
     * @dev Returns the optimal amount of deployable funds
     * @return _amount optimal deployable amount, denominated in the same decimals of underlying stable coins
     */
    function getDeployableAmount() external returns (uint256 _amount);
    ...
    /**
     * @dev Return the balance of the underlying asset
     */
...
interface ICERC20 is IERC20Upgradeable {
