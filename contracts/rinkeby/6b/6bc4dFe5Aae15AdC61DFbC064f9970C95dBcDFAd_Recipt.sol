...
contract Recipt is 
    ...
    uint256 public constant maxSupply = 2; // Genesis set
    ...
    function generateMetadata(uint256 tokenId, bool image) public view returns (string memory) {
        ...
        string memory time = string(abi.encodePacked(uint2str(dt.hour),":",uint2str(dt.minute)));
        string memory mdy = string(abi.encodePacked(uint2str(dt.month),"/",uint2str(dt.day),"/",uint2str(dt.year)));
