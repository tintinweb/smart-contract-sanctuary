...
contract Crowdsale is Context {
   ...
   */
  constructor (uint256 rate, address wallet, IERC20 token, IERC20 collateral) {
  ...
  */
   ...
   * @param collateralAmount Amount of collateral tokens involved in the purchase
   */
  function _preValidatePurchase(address beneficiary, uint256 collateralAmount) internal virtual view {
   ...
   * @param collateralAmount Amount of collateral tokens involved in the purchase
   */
  function _postValidatePurchase(address beneficiary, uint256 collateralAmount) internal virtual view {
   ...
   * @param collateralAmount Amount of collateral tokens involved in the purchase
   */
  function _updatePurchasingState(address beneficiary, uint256 collateralAmount) internal virtual {
    // solhint-disable-previous-line no-empty-blocks
  }
   ...
   */
  function _getTokenAmount(uint256 collateralAmount) internal virtual view returns (uint256) {
   ...
   */
  function _forwardFunds(uint256 collateralAmount) internal virtual {
...
contract DefiYieldIndividualCrowdsale is IndividuallyCappedCrowdsale {
  using SafeMath for uint256;
  using SafeERC20 for IERC20;
  ...
  constructor(uint256 rate, address wallet, IERC20 token, IERC20 collateral, TokenVesting vesting, uint8 pool)
    Crowdsale(rate, wallet, token, collateral) {
    ...
    require(
      ...
      "DefiYieldIndividualCrowdsale::constructor: can`t approve token spending."
    );
  }
   ...
   * @dev Returns direct withdraw percentage
   */
  function setDirectWithdrawPercentage(uint256 percentage) external onlyAdmin {
    require(percentage >= 0 && percentage <= 100, "DefiYieldCrowdsale: withdraw percentage should be >= 0 and <= 100");
    _directWithdrawPercentage = percentage;
  }

  /**
   * @dev Source of tokens. Overridden so tokens are transferred to vesting.
   * @param beneficiary Address performing the token purchase
   * @param tokenAmount Number of tokens to be emitted
   */
  function _deliverTokens(address beneficiary, uint256 tokenAmount) internal virtual override {
    uint256 directWithdrawAmount = tokenAmount.mul(_directWithdrawPercentage).div(100);
    uint256 vestingAmount = tokenAmount.sub(directWithdrawAmount);

    if (directWithdrawAmount > 0) {
      getToken().safeTransfer(beneficiary, directWithdrawAmount);
    }

    if (vestingAmount > 0) {
      _vesting.addBeneficiary(_pool, beneficiary, vestingAmount);
    }
  }
 ...
 */
abstract contract IndividuallyCappedCrowdsale is Crowdsale, AdminRole {
  using SafeMath for uint256;
   ...
   * @param cap collateral tokens limit for individual contribution
   */
  function setCap(address beneficiary, uint256 cap) external onlyAdmin {
   ...
   * @param collateralAmount Amount of collateral tokens involved in the purchase
   */
  function _preValidatePurchase(address beneficiary, uint256 collateralAmount) override internal view {
    super._preValidatePurchase(beneficiary, collateralAmount);
    // solhint-disable-next-line max-line-length
    require(_contributions[beneficiary].add(collateralAmount) <= _caps[beneficiary], "IndividuallyCappedCrowdsale: beneficiary's cap exceeded");
  }
   ...
   * @param collateralAmount Amount of collateral tokens involved in the purchase
   */
  function _updatePurchasingState(address beneficiary, uint256 collateralAmount) override internal {
    super._updatePurchasingState(beneficiary, collateralAmount);
    _contributions[beneficiary] = _contributions[beneficiary].add(collateralAmount);
  }
}
...
contract TokenVesting is Ownable, ReentrancyGuard {
  ...
  uint256 constant public ALLOWED_VESTING_UPDATE_THRESHOLD = 120 days;
  ...
  function setVestingPool(uint8 _pid, string calldata _name, uint256 _startTime, uint256 _endTime) external onlyOwner {
    ...
    require(lockPools[_pid].originalStartTime.add(ALLOWED_VESTING_UPDATE_THRESHOLD) >= _startTime, "TokenVesting: new start date is to large");
    require(lockPools[_pid].originalStartTime.sub(ALLOWED_VESTING_UPDATE_THRESHOLD) <= _startTime, "TokenVesting: new start date is to small");
    require(lockPools[_pid].originalEndTime.add(ALLOWED_VESTING_UPDATE_THRESHOLD) >= _endTime, "TokenVesting: new end date is to large");
    require(lockPools[_pid].originalEndTime.sub(ALLOWED_VESTING_UPDATE_THRESHOLD) <= _endTime, "TokenVesting: new end date is to small");
   ...
   */
  function addBeneficiary(uint8 _pid, address _beneficiary, uint256 _lockedAmount) external nonReentrant {
   ...
   */
  function addBeneficiaryBatches(uint8 _pid, address[] calldata _beneficiaries, uint256[] calldata _lockedAmounts) external nonReentrant {
