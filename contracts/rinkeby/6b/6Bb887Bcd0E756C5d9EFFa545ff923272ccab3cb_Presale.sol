...
contract Presale is ERC721Enumerable, Ownable {
  ...
  uint16 public constant MAX_TOKEN = 50000;
  ...
  function claim(uint amount) external payable {
    ...
    require(block.timestamp >= presaleOpenTime && block.timestamp <= presaleCloseTime, "Claim: Presale closed.");
    ...
    require(tokens.length.add(amount) <= MAX_TOKEN, "Claim: All tokens are sold.");
    require(balanceOf(msg.sender).add(amount) <= MAX_PER_ACCOUNT, "Claim: Can not claim that many.");
  ...
  function burnRedeem(uint256 index) external {
    require(redeemableContract == msg.sender, "Can only burn from redeem contract");
    _burn(index);
  }
  ...
  // remove in prod
  function showTokens() external view returns (uint16[] memory) {
    return tokens;
  }
  ...
  function getTokenValue(uint16 index) external view returns (uint16) {
    require(redeemableContract == msg.sender, "Can only be called from redeem contract");
    return tokens[index];
  }
