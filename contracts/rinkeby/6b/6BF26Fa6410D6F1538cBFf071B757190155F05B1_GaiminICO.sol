...
contract GaiminICO is Ownable, GaiminERC1155Ids, ReentrancyGuard, ERC1155Holder {
  ...
  bool public isAlloctionExecuted;
  ...
  function purchaseRarity(bytes32[] memory proof) external payable nonReentrant {
    ...
    // Transfer NFT;
    token.safeTransferFrom(address(this), msg.sender, UNKNOWN, numOfRarityPurchased, "0x0");
  ...
  function delegatePurchase(address[] memory newUsers, uint256[] memory amounts) external onlyOwner nonReentrant{
    ...
    // Transfer NFT;
      token.safeTransferFrom(address(this), newUser, UNKNOWN, amount, "0x0");     
    }
  ...
  function allocateRarity(uint256[] memory amounts, address[] memory receivers) external onlyOwner nonReentrant {
    ...
    if (!isAlloctionExecuted) {
      randomNumber = uint256(blockhash(block.number - 1) ^ blockhash(block.number - 2) ^ blockhash(block.number - 3));
      isAlloctionExecuted = true;
    } 
    ...
    for (uint256 i = 0; i < receivers.length; i++) {
      ...
      for (uint256 j = 0; j < amount; j++) {
        ...
        _randomNumber++; 
        uint256 rarity = (_randomNumber % 3) + 1;

        if ((rarity == SILVER && mintedRarity[SILVER] == MAX_TOTAL_SILVER) || (rarity == GOLD && mintedRarity[GOLD] == MAX_TOTAL_GOLD) || (rarity == BLACKGOLD && mintedRarity[BLACKGOLD] == MAX_TOTAL_BLACKGOLD)) {
          for (uint256 k = 1; k <= 3; k++) {
            _randomNumber++; 

            if ((k == SILVER && mintedRarity[SILVER] != MAX_TOTAL_SILVER) || (k == GOLD && mintedRarity[GOLD] != MAX_TOTAL_GOLD) || (k == BLACKGOLD && mintedRarity[BLACKGOLD] != MAX_TOTAL_BLACKGOLD)) {
  ...
  function withdrawUnknownRarity(address receiver) external onlyOwner nonReentrant {
    require(receiver != address(0), "Not a valid address");
    uint256 unknownRarityBalance = token.balanceOf(address(this), UNKNOWN);
    require(unknownRarityBalance > 0, "Contract have zero unknown rarity balance");

    token.safeTransferFrom(address(this), receiver, UNKNOWN, unknownRarityBalance, "0x0");
  }
