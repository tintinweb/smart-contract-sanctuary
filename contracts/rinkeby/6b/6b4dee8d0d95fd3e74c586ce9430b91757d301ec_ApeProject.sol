...
function claim() public  {
   require(nextMinted==TOTALSUPPLY,"Not All The Apes Have Been Minted");
    ...
    uint256 amountToClaim=setClaimAmount(msg.sender);
...
function setClaimAmount(address user) internal returns (uint256){
  require(hidden==false,"Apes Not Revealed Yet");
  uint256 amount;
  for(uint i = 0; i < TOTALSUPPLY; i++){
      if(ownerOf(i)==user){
        amount=amount+(block.number-lastClaim[i])*apeTypes[i]*RewardTokenPerBlock;
...
function pushApes(uint256 [] memory types) public onlyOwner(){
...
function getClaimAmount(address user) public view returns (uint256){
 ...
 if(MiningEquipment[user].useSilver>0){
      amount=amount*3/2;
    }
          if(MiningEquipment[msg.sender].useGold>0){
      amount=amount*5/2;
