...
contract HewerClanMergeProtocol is Ownable, ERC721Enumerable {
    ...
    mapping(address => string) addressToNotificationUsername;
    ...
    function mintMergeRequest(uint256 tagTokenId, uint256 tokenId, string memory collectionName, string memory usernameToNotify) public onlyRegisteredCollection(collectionName) {
        require(bytes(usernameToNotify).length > 5, "Discord username too short");
