...
contract Vaccine is VaccineApply {
    constructor () VaccineApply(0xd254f675cebae99FA6F28e7Db83B3D4C206053dd) {}
}
...
abstract contract VaccineApply is VaccineSelling {
    ...
    uint8 constant maxVaccinationProgress = 3;
    ...
    function mutatePork(uint256 vaccineId, uint256 porkId) public {
        ...
        uint chanceOfMutation = 25;
        if (vaccineId == 2) {
            chanceOfMutation = 50;
        } else if (vaccineId == 3) {
            chanceOfMutation = 100;
        }
        
        _mutatePorkWithChance(vaccineId, porkId, chanceOfMutation);
    }
    
    function _mutatePorkWithChance(uint256 vaccineId, uint256 porkId, uint chanceOfMutation) private {
        require(!isFullyVaccinated(porkId), "This pork has already become a mutant");
        
        uint8 randomNumber = _getRandomInteger() % 100;
        
        if (randomNumber >= chanceOfMutation) {
            _stackVaccineOnPork(porkId);
            return;
        } else {
            _mutatePork(porkId);
        }
    ...
    function _stackVaccineOnPork(uint256 porkId) private {
        _vaccinationProgress[porkId] += 1;
    }
    
    function _getRandomInteger() private view returns (uint8) {
...
abstract contract VaccineSelling is ERC1155, Ownable, Pausable, PausableClaim {
    ...
    function addForSale(uint amountToAdd) public onlyOwner {
        currentSupply += amountToAdd;
        require(currentSupply <= maxTotalSupply);
    }
    
    function leftForSale() public view returns(uint) {
        return currentSupply - totalMinted;
    }
    ...
    function howManyFreeTokensForAddress(address target) public view returns (uint16) {
        // TODO: some magic will happen here
        return 0;
    }
    ...
    function buy(uint256 id, uint256 tokensToBuy) public payable whenNotPaused {
        ...
        _mint(msg.sender, id, tokensToBuy, "100500");
    }
    
    function claim(uint256 id) public whenClaimNotPaused {
        uint16 tokensToMint = howManyFreeTokens();
        require(tokensToMint > 0);
        
        totalMinted += tokensToMint;
        _mint(msg.sender, id, tokensToMint, "100500");
    }
    ...
    function contractURI() public view returns (string memory) {
        return "https://api.pork1984.io/contract/vaccine";
    }
}
