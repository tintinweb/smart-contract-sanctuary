...
contract StakePool is IStakePool, Initializable, Owned {
  ...
  mapping(address => Stake) stakeMap; // owner => Stake
  ...
  modifier poolLock() {
    require(lock == UNLOCKED, 'SP: execution locked');
    lock = LOCKED;
    _;
    lock = UNLOCKED;
  }

  /// @dev Accrue interests in the pool
  modifier accrue() {
    _accrueInterest();
    _;
  }
  ...
  event UnstakeEvent(address staker, uint amount, uint amountPlusInterest);
  event AddRewardEvent(address rewarder, uint amount);
  ...
  function stake(uint amount) external override poolLock accrue {
    require(amount > 0, 'SP: zero amount');
    Stake storage stk = stakeMap[msg.sender];
    require(stk.owner == address(0) || stk.owner == msg.sender, 'SP: bad stake owner');
    ...
    if (stk.owner == msg.sender) { // adding amount to an existing stake
      if (stk.amount > 0 && cusumIpsE18 > stk.cusumIpsStartE18) {
        uint earnedInterestE18 = stk.amount * ( cusumIpsE18 - stk.cusumIpsStartE18);
        stk.earnedInterestE18 += earnedInterestE18;
      }
    } else { // new stake
      stk.owner = msg.sender;
    }
    stk.cusumIpsStartE18 = cusumIpsE18;
    stk.amount += amount;

    emit StakeEvent(stk.owner, amount);
  }

  /// @notice Unstake from the pool
  /// @param amount The amount to be unstaked
  function unstake(uint amount) external override poolLock accrue returns(uint) {
    Stake storage stk = stakeMap[msg.sender];
    require(stk.owner == msg.sender, 'SP: empty stake');
    require(amount > 0 && amount <= stk.amount, 'SP: invalid amount to unstake');

    // calculate unstake amount, plus interest
    uint interestE18 = ( stk.amount * ( cusumIpsE18 - stk.cusumIpsStartE18) ) + stk.earnedInterestE18 ;
    if (interestE18 > totalInterestE18 ) {
      interestE18 = totalInterestE18;
    }
    uint unstakeAmount = amount + (interestE18 / 1e18);

    // transfer unstaked amount to the owner
    IERC20Upgradeable(token).safeTransfer(stk.owner, unstakeAmount);
    ...
    stk.amount -= amount;
    stk.cusumIpsStartE18 = cusumIpsE18;
    stk.earnedInterestE18 = 0;
    if (stk.amount == 0) {
      delete stakeMap[stk.owner];
    }

    emit UnstakeEvent(stk.owner, amount, unstakeAmount);
...
//    console.log('StakePool:_accruedInterest: in:', totalRewardE18, totalInterestE18, totalStake);
    ...
    latestAccruedBlock = block.number;
//    console.log('StakePool:_accruedInterest:out:', totalRewardE18, interestE18, cusumIpsE18);
  }

  /// @dev Set the APY per block of the pool
  /// @param apyE4 The APY multiplied by 1e4, e.g., 400 means 4% APY
  function setApy(uint apyE4) external virtual poolLock onlyOwner {
  ...
  function addReward(uint amount) external override poolLock {
    IERC20Upgradeable(token).safeApprove(address(this), 0);
    IERC20Upgradeable(token).safeApprove(address(this), amount);
    IERC20Upgradeable(token).safeTransferFrom(msg.sender, address(this), amount);
  ...
  function getStake(address stakerAddress) external virtual view returns(Stake memory) {
    return stakeMap[stakerAddress];
  }
  ...
  function getPoolInfo() external virtual view returns(
    ...
    return (totalStake, totalRewardE18, totalInterestE18, cusumIpsE18, cusumReward, cusumStake, latestAccruedBlock, apyPerBlockE18);
  }
}
