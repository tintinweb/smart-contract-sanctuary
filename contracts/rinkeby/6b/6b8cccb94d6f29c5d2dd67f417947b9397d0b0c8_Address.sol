...
/**
 * @title BFERC1155Contract
 * BFERC1155Contract - ERC1155 contract that whitelists an operator address, has create and mint functionality, and supports useful standards from OpenZeppelin,
  like _exists(), name(), symbol(), and totalSupply()
 */
contract BFERC1155Contract is
    ContextMixin,
    ERC1155,
    ...
    Ownable
{
    ...
    //Token id accumulate
    uint256 public _token_id_accumulate;

    // contract URI
    string public contractURIname = "https://bf/contract/Trad";
    ...
    function create(
        ...
        uint256 tmpTokenId = _token_id_accumulate + 1;

        require(!_exists(tmpTokenId), "token _id already exists");
        creators[tmpTokenId] = _msgSender();

        if (bytes(_uri).length > 0) {
            customUri[tmpTokenId] = _uri;
            emit URI(_uri, tmpTokenId);
        }

        _mint(_initialOwner, tmpTokenId, _initialSupply, _data);

        tokenSupply[tmpTokenId] = _initialSupply;
        return tmpTokenId;
    }
