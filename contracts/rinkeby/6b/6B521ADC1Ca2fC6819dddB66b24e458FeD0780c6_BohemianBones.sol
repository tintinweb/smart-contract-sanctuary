 ...
 */
contract BohemianBones is ERC721, Ownable, PaymentSplitter, ReentrancyGuard {
    ...
    // Default bohemian bone token metrics
    uint256 public bohemianBonePrice = 8e16; //0.08 ETH
    uint256 public maxBohemianBonePurchase = 2;
    uint256 public MAX_BOHOBONES = 10;
    ...
    // many Bohemian Bones remain that they can purchase during presale.
    ...
    constructor(string memory name, string memory symbol, uint256 saleStart, address[] memory treasuryWallets, uint256[] memory treasuryShares)
      ...
      PaymentSplitter(treasuryWallets, treasuryShares) {
        REVEAL_TIMESTAMP = saleStart + (86400 * 9); // 48 Hours after sale start
    ...
    /**
     * @param newPrice The new price for an individual bohemian bone.
     * @dev Sets a new price for an individual bohemian bone.
     * @dev Can only be called by owner.
     */
    function setBohemianBonePrice(uint256 newPrice) public onlyOwner {
        bohemianBonePrice = newPrice;
    }

    /**
     * @param newMaxBohemianBonePurchase The new maximum number of bohemian bones that can be minted per transaction.
     * @dev Sets the new maximum number of bohemian bone purchases per transaction.
     * @dev Can only be called by owner.
     */
    function setMaxBohemianBonePurchase(uint256 newMaxBohemianBonePurchase) public onlyOwner {
        maxBohemianBonePurchase = newMaxBohemianBonePurchase;
    }

    /**
     * @param numBohemianBones The number of bohemian bones to reserve.
     * @dev Reserves bohemian bones for giveaways.
     * @dev Will not allow reservations above maximum reserve threshold.
     * @dev Can only be called by owner.
     */
    function reserveBohemianBones(uint256 numBohemianBones) public onlyOwner {
        require(numBohemianBones <= numReservationsLeft, "BohemianBones: Reservations would exceed max reservation threshold of 500.");
        require(totalSupply().add(numBohemianBones) <= MAX_BOHOBONES, "BohemianBones: Reservations would exceed max supply of BohemianBones.");
        
        uint256 supply = totalSupply();
        uint256 i;
        
        for (i = 0; i < numBohemianBones; i++) {
            _safeMint(msg.sender, supply + i);

            // Increase num total minted
     ...
     * @param numPresaleBohemianBones Amount of presale bohemian bones to give to the address.
     * @dev Can only be called by owner.
     */
    function addPresaleAddress(address presaleAddress, uint256 numPresaleBohemianBones) public onlyOwner {
        require(presaleAddress != address(0), "BohemianBones: Cannot add burn address to the presale.");
         
        presaleVouchers[presaleAddress] = numPresaleBohemianBones;
    }

    /**
     * @param newPresaleAddresses Addresses to be added to the list of verified presale addresses.
     * @param voucherAmount Amount of presale bohemian bones to give per address.
     * @dev Can only be called by owner.
     */
     ...
     */
    function mintBohemianBone(uint256 numberOfTokens) public payable nonReentrant {
        require(saleStatus == SaleState.ACTIVE || saleStatus == SaleState.PRESALE, "BohemianBones: Sale must be active or in presale mint bohemian bone.");

        if (saleStatus == SaleState.PRESALE) {
            require(presaleVouchers[msg.sender] >= numberOfTokens, "BohemianBones: You don't not have enough presale vouchers to mint that many bohemian bones.");
        }

        require(numberOfTokens <= maxBohemianBonePurchase, "BohemianBones: Please try to mint a lower amount of Bohemian Bones.");
        require(totalSupply().add(numberOfTokens) <= MAX_BOHOBONES, "BohemianBones: Purchase would exceed max supply of BohemianBones.");
        require(bohemianBonePrice.mul(numberOfTokens) <= msg.value, "BohemianBones: Ether value sent is not correct.");
    ...
    function setStartingIndex() public onlyOwner  {
        require(startingIndex == 0, "BohemianBones: Starting index is already set");
        require(startingIndexBlock != 0, "BohemianBones: Starting index block must be set");
