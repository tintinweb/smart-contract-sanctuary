    ...
    ReentrancyGuardUpgradeable
{
    ...
    {
        partiesList.create(_partyId, _creator, _minimumParticipants, _duration);
        emit PartyCreated(
            _partyId,
            _creator,
            block.number,
    ...
    function participantWithdraw(uint256 _partyId, address _participant)
        public
        nonReentrant
    {
        PartiesStorageLibrary.PartyState state;

        (state, , , , , , , , , ) = partiesList.get(_partyId);
        ...
        partiesList.decreaseNumberOfParticipants(_partyId);
        emit ParticipantWithdraw(_partyId, _participant, amount);
    }

    function startYielding(uint256 _partyId) public onlyRole(BACKEND_ADMIN_ROLE) {
        (PartiesStorageLibrary.PartyState state, , , , , , , , , ) = partiesList
            .get(_partyId);
        require(
     ...
     */
    function stopYielding(uint256 _partyId) public {
        (
            PartiesStorageLibrary.PartyState state,
            ,
            ,
    ...
    function distributeRewards(
        ...
        PartiesStorageLibrary.PartyState state;
        uint256 minimumParticipants;
        (state, , , , , minimumParticipants, , , , ) = partiesList.get(
            _partyId
        );
        require(
            state == PartiesStorageLibrary.PartyState.Rewarding,
        ...
        for (uint256 i = 0; i < _prizes.length; i++) {
            multiToken.mint(_winners[i], _partyId, _prizes[i], _prizes[i], "");
            partiesList.increasePrizes(_partyId, _prizes[i]);
            partiesList.increaseBalance(_partyId, _prizes[i]);
        }

        address[] memory participants = partiesList.getParticipants(_partyId);
        if (minimumParticipants > 0) {
            for (uint256 i = 0; i < participants.length; i++) {
                transferAndBurn(_partyId, participants[i]);
            }
        ...
        uint256 prizes_amount;
        (, , , , , , , , , prizes_amount) = partiesList.get(_partyId);
        emit RewardsDistributed(_partyId, prizes_amount, _winners, _prizes);
    }
    ...
    function addVaultManager(address _vaultManager)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _setupRole(VAULT_ADMIN_ROLE, _vaultManager);
    }

    function revokeVaultManager(address _vaultManager)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        revokeRole(VAULT_ADMIN_ROLE, _vaultManager);
    }
}
    ...
    IERC1155MetadataURIUpgradeable
{
    ...
    function _safeTransferFrom(
        ...
        uint256 twaAmount = _calculateTwaAmount(id, from, amount);
    ...
    function _burnBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            uint256 id = ids[i];
            uint256 amount = amounts[i];
            uint256 twaAmount = _calculateTwaAmount(id, account, amount);
    ...
    function _calculateTwaAmount(
        uint256 id,
        address account,
        uint256 realAmount
    ) internal virtual returns (uint256 _twaAmount) {
...
library PartiesStorageLibrary {
    ...
    function decreaseNumberOfParticipants(
        PartiesList storage _self,
        uint256 _partyId
    ) external {
        _self.parties[_partyId].participantsNumber -= 1;
