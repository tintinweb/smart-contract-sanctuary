    ...
    ReentrancyGuard
{
    ...
    function withdraw(
        ...
        uint256 unusedAmount0 = _getBalance0().mul(shares).div(totalSupply);
        uint256 unusedAmount1 = _getBalance1().mul(shares).div(totalSupply);
    ...
    function _burnLiquidityShare(
        ...
        if (liquidity > 0) {
            assert(liquidity <= type(uint128).max);
    ...
    function rebalance(
        ...
        if (swapAmount != 0) {
            ...
            balance0 = _getBalance0();
            balance1 = _getBalance1();
        }
    ...
    function getTotalAmounts() public view override returns (uint256 total0, uint256 total1) {
        ...
        total0 = _getBalance0().add(baseAmount0).add(limitAmount0);
        total1 = _getBalance1().add(baseAmount1).add(limitAmount1);
    }
    ...
    function getLiquidity(int24 tickLower, int24 tickUpper) external view returns (uint128) {
        (uint128 liquidity, , , , ) = _position(tickLower, tickUpper);
        return liquidity;
    }
    ...
    function emergencyBurn(
        ...
        emit CollectFees(feesFromPool0, feesFromPool1, 0, 0);
    }
...
contract AlphaStrategy {
    ...
    function rebalance(
        ...
        int256 swapAmount,
        uint160 sqrtPriceLimitX96
    ) external onlyOpen onlyKeeper {
        int24 _tickSpacing = tickSpacing;
        {
            (bool ok, string memory errMsg) =
                _shouldRebalance(tick, _baseThreshold, _limitThreshold);
            require(ok, errMsg);
        }
        ...
        int24 tickCeil = tickFloor + _tickSpacing;
    ...
    function shouldRebalance(
        ...
        return _shouldRebalance(_tick, _baseThreshold, _limitThreshold);
    }
    ...
    function _shouldRebalance(
        ...
        if (block.timestamp < lastRebalance.add(period)) {
            return (false, "within period");
        }

        int24 tickMove = _tick > lastTick ? _tick - lastTick : lastTick - _tick;
        if (tickMove < minTickMove) {
            return (false, "within minTickMove");
        }

        int24 twap = _getTwap();
        int24 twapDeviation = _tick > twap ? _tick - twap : twap - _tick;
        if (twapDeviation > maxTwapDeviation) {
            return (false, "exceed maxTwapDeviation");
        }

        int24 maxThreshold =
            _baseThreshold > _limitThreshold ? _baseThreshold : _limitThreshold;
        if (
            _tick < TickMath.MIN_TICK + maxThreshold + tickSpacing ||
            _tick > TickMath.MAX_TICK - maxThreshold - tickSpacing
        ) {
            return (false, "exceed threshold");
        }
    ...
    function setMinTickMove(int24 _minTickMove) external onlyAdmin {
        require(_minTickMove >= 0, "minTickMove must be >= 0");
        minTickMove = _minTickMove;
    }

    function toggleStrategy(bool _value) external onlyAdmin {
