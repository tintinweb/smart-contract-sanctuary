...
contract Apes is BaseERC721 {
  ...
  function manualMint(
    ...
    Ape memory _toCreate = Ape(idx, id, level, _age, _edition, block.timestamp, false, 0);
    apes.push(_toCreate);
  ...
  function migrate(uint256 _tokenId) public {
    IOldApe oldApeInstance = IOldApe(address(0x7aB5Aee062c17AB665EE1E318ED5b6Af71D945Ec));
    (
      uint256 unUsedIdx,
      uint256 unUsedId,
      uint8 level,
      ...
      uint256 unUsedPrice,
      address owner
    ) = oldApeInstance.monkeys(_tokenId - 1);
    // require(msg.sender == owner, "not owner");
    // oldApeInstance.transferFrom(owner, devWallet, _tokenId);
    // string memory name = oldApeInstance.tokenIdToName(_tokenId);
    // string memory tokenURI = oldApeInstance.tokenURI(_tokenId);

    // uint256 idx = apes.length;
    // uint256 id = idx + 1;
    // Ape memory _toCreate = Ape(
    //   idx, id,
    //   intToLevel(level), age, edition,
    //   bornAt, false,
    //   0
    // );
    // tokenIdToName[id] = name;
    // apes.push(_toCreate);
    // _mint(msg.sender, id);
...
abstract contract IOldApe {
  function monkeys(uint256 idx) external virtual
    returns (uint256, uint256, uint8, uint256, string memory, uint256, bool, uint256, address);
