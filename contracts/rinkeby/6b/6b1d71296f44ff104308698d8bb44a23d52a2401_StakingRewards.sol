...
contract StakingRewards is OlympusAccessControlled, ReentrancyGuard {
    ...
    function earned(address account) public returns (uint256) {
        ...
        uint256 newEarned = userInfos[account].tazorNum.mul(1).mul(_apr).div(10 ** 9); // reward TAZ amount
    ...
    function setAPRvalue() private {
        userInfos[msg.sender].apr = userInfos[msg.sender].tazNum.mul(10000).mul(5).add(10**8);
    }
