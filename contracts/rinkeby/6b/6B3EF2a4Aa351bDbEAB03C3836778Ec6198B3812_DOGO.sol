...
contract DOGO is ERC721Enumerable, Ownable, ReentrancyGuard {

	uint256 public constant DOGO_MAX_AMOUNT = 4500;
	uint256 public DOGO_PER_TX = 15;
	uint256 public PRICE = 0.077 ether;
	address public founder = 0xB4B39dfB2ba6320F6a31C52D8bb790Eb19F457Fa;
	...
	function mintPublic(uint256 amount) external payable nonReentrant() {
		...
		require(amount <= DOGO_PER_TX, "You can't mint that many tokens");
		require(totalSupply() + amount <= DOGO_MAX_AMOUNT, "Mint would exceed max supply of tokens");
	...
	function enableClaimFoundingMembers(address[] memory members, uint256[] memory amounts) external onlyOwner {
		...
		for (uint256 i = 0; i < members.length; i++) {
			...
			require(amounts[i] <= DOGO_PER_TX, "amount is more than the allowable");
	...
	function setIpfsHash(string[] memory ipfsHashs, uint256[] memory idTokens) external onlyOwner {
		...
		for (uint256 i = 0; i < idTokens.length; i++) {
			...
			require(idTokens[i] <= DOGO_MAX_AMOUNT, "id is more than the allowable");
	...
	function makePresaleMemberMint(uint256 tokenAmount, uint256 etherAmount) private {
		...
		require(totalSupply() + tokenAmount <= DOGO_MAX_AMOUNT, "Mint would exceed max supply of tokens");
		require(mintedPresaleCount + tokenAmount <= DOGO_MAX_AMOUNT, "Mint would exceed max pre-sale tokens");
	...
	function setEntityBirthdaysPerTx(uint256 newPerTx) public onlyOwner {
		...
		emit SetNewValue("DOGO_PER_TX", DOGO_PER_TX, newPerTx);
		DOGO_PER_TX = newPerTx;
	}
