...
contract PlayerStoxToken is Initializable, Context, IERC20 {
     ...
     */
    function initialize(string memory name_, string memory symbol_, uint8 decimals_, address factoryAddress_, address USDT_)
        external initializer {
        _name = name_;
        _symbol = symbol_;
        _decimals = decimals_;
        factoryAddress = address(factoryAddress_);
    ...
    /**
     * @dev Update the `transferEnabled` for admin.
     * @param value_ The new value of transferEnabled.
     */
    function updateTransferEnabled(bool value_) public onlyAdmin {
    ...
    function buyExactStoxForTokens(uint amountMax_, uint256 quantity_, string memory orderId_) external {
        uint256 currentAmount = quote(quantity_);
        require(currentAmount <= amountMax_, "Price has fluctuated beyond slippage");
        IPlayerStoxFactory(factoryAddress).userDeposit(USDT, msg.sender, quantity_);
    ...
    function buyStoxForExactTokens(uint256 amount_, uint256 quantityMin_) external {
        uint256 currentAmount = quote(quantityMin_);
        require(currentAmount <= amount_, "Price has fluctuated beyond slippage");
        IPlayerStoxFactory(factoryAddress).userDeposit(USDT, msg.sender, quantityMin_);
        _mint(msg.sender, amount_);
        // TODO: save order history
    }

    // input is fixed, output is variable
    function sellExactStoxForTokens(uint256 quantity_) external {
        // TODO: check if user is getting minimum rate for that quantity based on slippage
        IPlayerStoxFactory(factoryAddress).userWithdraw(USDT, msg.sender, quantity_);
    ...
    // input is variable, output is fixed
    function sellStoxForExactTokens(uint256 quantity_) external {
        // TODO: check if user is getting minimum rate for that quantity based on slippage
        IPlayerStoxFactory(factoryAddress).userWithdraw(USDT, msg.sender, quantity_);
        uint256 amount = quote(quantity_);
        _burn(msg.sender, amount);
