...
contract PlayerStoxToken is Initializable, Context, IERC20 {
    ...
    function quote(uint256 quantity_) public view returns (uint256) {
        uint256 newQuantity = _totalSupply.add(quantity_);
        // assuming curve y=mx+c => totalSupply = 2(price) - 4
        uint256 price = newQuantity.add(4);
        price = price.div(2);
        return price;
    }
    ...
    function buyExactStoxForTokens(address token_, uint amountMax_, uint256 quantity_, string memory orderId_) external {
        uint256 currentAmount = quote(quantity_);
        require(currentAmount <= amountMax_, "Price has fluctuated beyond slippage");
        IPlayerStoxFactory(factoryAddress).userDeposit(token_, msg.sender, quantity_);
        _mint(msg.sender, currentAmount);
    ...
    function buyStoxForExactTokens(address token_, uint256 amount_, uint256 quantityMin_) external {
        uint256 currentAmount = quote(quantityMin_);
        require(currentAmount <= amount_, "Price has fluctuated beyond slippage");
        IPlayerStoxFactory(factoryAddress).userDeposit(token_, msg.sender, quantityMin_);
        _mint(msg.sender, amount_);
        // TODO: save order history
    }

    // input is fixed, output is variable
    function sellExactStoxForTokens(address token_, uint256 quantity_) external {
        // TODO: check if user is getting minimum rate for that quantity based on slippage
        IPlayerStoxFactory(factoryAddress).userWithdraw(token_, msg.sender, quantity_);
        uint256 amount = quote(quantity_);
        _burn(msg.sender, amount);
        // TODO: save order history
    }

    // input is variable, output is fixed
    function sellStoxForExactTokens(address token_, uint256 quantity_) external {
        // TODO: check if user is getting minimum rate for that quantity based on slippage
        IPlayerStoxFactory(factoryAddress).userWithdraw(token_, msg.sender, quantity_);
        uint256 amount = quote(quantity_);
        _burn(msg.sender, amount);
