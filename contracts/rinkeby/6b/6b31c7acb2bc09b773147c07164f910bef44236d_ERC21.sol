...
interface IERC21 {
     ...
     * @dev Gets fee
    */
    function fee(string memory chain) external view returns (uint256);
    ...
    */ 
    function redeemToken(address addressTo, uint256 amount) external returns (bool);
    
    /**
     * @dev Set the required fee for cross transfer
     */
    function setFee(string memory chain, uint256 feeAmount) external returns (bool);
     ...
     * a call to {MoveToken}.
     */
    event MovedToken(address indexed owner, uint256 fee, uint256 value, string chain);
    
    /**
     * @dev Emitted when the user wants to redeem tokens from another chain is set by 
     * a call to {RedeemToken}.
     */
    event RedeemedToken(address indexed addressTo, uint256 amount);
}
...
contract ERC21 is Context, IERC21, IERC21Metadata, Ownable, RecoverableToken, WithdrawableToken {
     ...
     * @dev Gets fee
    */
    function fee(string memory chain) public view virtual override returns (uint256) {
        return _fees[chain];
    }
    ...
    */ 
    function moveToken(uint256 amount, string memory chain) public virtual payable override returns (bool){
        ...
        require(_fees[chain] == 0, "Chain is not supported");
        
        // Check the fee has been sent
        require(_fees[chain] < msg.value, "Fee has not been met");
        ...
        // Kick off the event
        emit MovedToken(_msgSender(), msg.value, amount, chain);
     ...
     * Emits a {MovedToken} event.
    */ 
    function redeemToken(address addressTo, uint256 amount) public virtual onlyOwner override returns (bool){
        // Mint the tokens being redeemed
        require(_mint(addressTo, amount), "The mint failed");
        
        // Kick off the event
        emit RedeemedToken(addressTo, amount);
        
        return true;
    }
    
    function getDataLength(uint256 value)public view returns(uint256){
     ...
     * @dev Set the required fee for cross transfer
     */
    function setFee(string memory chain, uint256 feeAmount) public virtual override onlyOwner returns (bool) {
        _fees[chain] = feeAmount;
        return true;
    }
