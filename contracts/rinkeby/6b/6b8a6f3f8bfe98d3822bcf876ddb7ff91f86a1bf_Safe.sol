...
contract Safe is ISafe, ERC20, Ownable {
  ...
  /**
   * mints $SAFE to a recipient
   * @param to the recipient of the $SAFE
   * @param amount the amount of $SAFE to mint
   */
  ...
  /**
   * burns $SAFE from a holder
   * @param from the holder of the $SAFE
   * @param amount the amount of $SAFE to burn
   */
  ...
  function transferFrom(
      ...
      address recipient,
      uint256 amount
  ) public virtual override(ERC20, ISafe) disallowIfStateIsChanging returns (bool) {
    // NICE TRY MOUSE VARIANT
    ...
    // If it's not an admin entity (game contract, safezone, etc)
    // The entity will need to be given permission to transfer these funds
    // For instance, someone can't just make a contract and siphon $SAFE from every account
