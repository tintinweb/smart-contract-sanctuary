...
contract SockPops is Ownable, ERC721 {
  ...
  function freeClaim(uint256 amount) external {
    require(amount <= 3,"Max 3 free mints per Transaction!");
    uint256 minted = freeMintedByAddress[msg.sender];
    require(minted < 3, "Max free mint cap is reached for this address");
    require(minted + amount <= 3, "This would pass the max free cap mint for this address");
    uint256 freeClaimCount = _freeClaimCounter.current();
    require(freeClaimCount + amount <= maxFreeMintSupply, "Not enough free tokens left");
  ...
  function withdrawAll() public onlyOwner {
    ...
    artist.transfer(balance/3);
    teamLead.transfer(balance/3);
