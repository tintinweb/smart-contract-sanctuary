...
contract Whitelist is AccessControl, Configurable, IWhitelist {
  ...
  constructor(bytes memory arguments_) {
    ...
    for (uint8 i = 0; i < members_.length; i++) {
      ...
      members[member.account] = Whitelisted(member, 0);
    }
  ...
  function update(
    ...
    for (uint8 i = 0; i < toCreate.length; i++) {
      ...
      if (members[member.account].member.account != address(0x0)) {
        revert AccountAlreadyWhitelisted(member.account);
      }

      // optional allowance, used 0
      members[member.account] = Whitelisted(Member(member.account, member.allowance), 0);
    }
    ...
    for (uint8 i = 0; i < toUpdate.length; i++) {
      ...
      if (member.allowance > 0 && used > member.allowance) {
        revert UsedBiggerThanAllowance(member.account, used, member.allowance);
      }

      // allowance updated, preserve used
      members[member.account].member.allowance = member.allowance;
    }
    ...
    for (uint8 i = 0; i < toDelete.length; i++) {
      ...
      if (members[account].member.account == address(0x0)) {
        revert AccountDoesNotExist(account);
      }

      // empty storage
      members[account] = Whitelisted(Member(address(0x0), 0), 0);
    }
    ...
    returns (bool)
  {
    ...
    // not whitelisted
    if (whitelisted.member.account != sender) {
      emit AccountNotWhitelisted(sender);
      return false;
    }

    // limit not enough
    uint256 allowance = whitelisted.member.allowance;
    if (allowance > 0 && allowance < whitelisted.used + amount) {
