...
contract MiningTool {
    ...
    function getUserInfoArray(address miningContractAddr, address userAddr) public view returns(UserInfo[] memory) {
        MiningContract miningContract = MiningContract(miningContractAddr);

        uint256 poolLength = miningContract.poolLength();
        UserInfo[] memory userInfoArray = new UserInfo[](poolLength);  

        for(uint256 i = 0; i < poolLength; i++){
            (uint256 _amount, uint256 _miningTokenPledgeAmount,uint256 _rewardDebt,uint256 _rewardToClaim) =  miningContract.getUserInfo(i, userAddr);
