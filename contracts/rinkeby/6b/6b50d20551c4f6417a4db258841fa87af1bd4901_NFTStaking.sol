...
contract NFTStaking is ReentrancyGuard, Pausable, Ownable {
    ...
    mapping(uint256 => bool) private supportedSeries;
    uint256[] public nftTokens;
    mapping(uint256 => NftToken) public nftTokenMap;
    mapping(address => uint256) public percentMultiplierTokenId;
    mapping(uint256 => uint256) public percentMultiplier;
    mapping(address => bool) private percentMultiplierApplied;

    /* ========== CONSTRUCTOR ========== */

    constructor(address _NFT, address _NDR) public {
        NDR = IERC20(_NDR);
        NFT = IERC1155(_NFT);
    }
    ...
    /**
     * @dev Total staked card by user
     */
    function balanceByTokenIdOf(uint256 tokenId, address account) external view returns (uint256) {
        return nftTokenMap[tokenId].balances[account];
    }

    /**
     * @dev Returns all NFT tokenids
     */
    ...
    /**
     * @dev Sets percent multiplier
     */
    function setPercentMultiplier(uint256 _tokenId, uint256 _percentMultiplier) public onlyOwner {
       percentMultiplier[_tokenId] = _percentMultiplier;
    }
    ...
    /**
     * @dev Sets supported series
     */
    function setSupportedSeries(uint256 _series, bool supported) public onlyOwner {
       supportedSeries[_series] = supported;
    }
    ...
    function stakeInternal(uint256 tokenId, uint256 amount) internal {
        ...
        if(percentMultiplierApplied[msg.sender]) {
            strengthWeight[msg.sender] = pureStrengthWeight[msg.sender].add(pureStrengthWeight[msg.sender].mul(percentMultiplier[percentMultiplierTokenId[msg.sender]]).div(100));
        } else {
    ...
    function withdrawNFTInternal(uint256 tokenId, uint256 amount) internal {
        ...
        if(series == 3) {
            require(amount == 1, "only one nft with series 3 allowed");
            require(percentMultiplierApplied[msg.sender] == true, "nft with series 3 is not staked");
            percentMultiplierApplied[msg.sender] = false;
        }
    ...
    /**
     * @dev Get all rewards. used internally
     */
