...
contract GameFiShibaCoinPool is Ownable, Pausable {
    ...
    function setLB(uint longestBlockNumber_) public onlyOwner{
        _longestBlockNumber = longestBlockNumber_;
        emit LBChange(msg.sender, longestBlockNumber_);
    }

    function setNPB(uint numPerBlockNumber_) public onlyOwner{
    ...
    function withdraw(uint256 tokenId) public  returns (uint256) {
        uint256 _nunmOfBlock = estimatedIncome(tokenId);
    ...
    function withdraw(uint256[] memory tokenIds) public whenNotPaused returns (uint256 _nunmOfBlock) {
        for(uint i=0;i<tokenIds.length;i++){
            _nunmOfBlock+=withdraw(tokenIds[i]);
        }
    }

    function estimatedIncome(uint[] memory tokenIds) public whenNotPaused view returns (uint256 _nunmOfBlock){
        for(uint i=0;i<tokenIds.length;i++){
            if(block.number > blockNumber[tokenIds[i]]){
                _nunmOfBlock+=estimatedIncome(tokenIds[i]);
            }
        }
    } 

    function estimatedIncome(uint256 tokenId) public view returns (uint256) {
        require(
            _cryptoshiba.ownerOf(tokenId) == msg.sender,
        ...
        _nunmOfBlock = _nunmOfBlock > _longestBlockNumber
            ? _longestBlockNumber
            : _nunmOfBlock;
        return _sire.knighthood(tokenId) > 2 ? _nunmOfBlock.mul(_numPerBlockNumber).mul(10) : _nunmOfBlock.mul(_numPerBlockNumber);
    }
    ...
    function withdraw() public whenPaused onlyOwner{
        _gameFiToken.transfer(msg.sender, _gameFiToken.balanceOf(address(this)));
    }
}
