...
contract EthUniswapPCVController is PCVRef, IUniswapPCVController, Permissions {
    ...
    event getMesss(uint256 a,uint256 b,uint256 c);
    ...
    function getPrice() external view returns (Decimal.D256 memory) {
        (Decimal.D256 memory price, , ) = _getUniswapPrice();
        return price;
    }

    function getBelowPeg()
        external
        view
        returns (Decimal.D256 memory distance)
    {
    ...
    {
        require(
            _reweightWithdrawBPs <= BASIS_POINTS_GRANULARITY,
            "EthUniswapPCVController: withdraw percent too high"
        );
    ...
    function _returnToPeg() internal {
        ...
        uint256 amountEth =
            _getAmountToPegOther(taiReserves, ethReserves, _peg); //计算返回锚定价格需要的eth数量
    ...
    function _swapEth(
        ...
        emit getMesss(balance,amount,amountEth);
        // uint256 amountOut =
