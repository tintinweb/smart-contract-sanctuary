...
contract EventsController is Ownable {
    ...
    modifier isNotMade (bytes32 eventId) {
        require (events.getStart(eventId) == 0, "Event has been already maded");
        _;
    }

    modifier minBond() {
        require (msg.value >= admin.getMinEventBond(), "The value should be bigger than mintEventBond ");
        _;
    }

    modifier isActive(bytes32 eventId) {
        require (events.getActive(eventId), "Event should be active");
        _;
    }
    ...
    modifier onlyVerified() {
        require (oracleVerif.checkVerification(msg.sender), "Only verified oracle is allowed");
        _;
    }
    ...
    function makeEvent (bytes32 id, uint256 startTime, uint256 duration,  bytes32[] calldata teams, bool drawPossible)
        onlyVerified
        isNotMade(id)
        minBond
        external
        payable
    {
        require (startTime > 0 && duration > 0, "StartTime and duration should be bigger than zero");
        require (teams.length > 1, "At least one team should be exist");

        events.makeStandardEvent(id, startTime, duration, teams, drawPossible, msg.value, payable(msg.sender));
    ...
    function finalizeEvent (bytes32 eventId) oraclePeriodOver(eventId) isActive(eventId) external {
        if (block.timestamp < events.getEndTime(eventId) + admin.getOraclePeriod() + admin.getEventMakerFinalizeCushion()) {
            ...
            mevu.transferEth(payable(msg.sender), events.getMakerBond(eventId) + events.getTotalAmountBet(eventId)/admin.getEventMakerRewardDivider());
        } else  {
            events.finalizeEvent(eventId);
            mevu.transferEth(payable(msg.sender), events.getMakerBond(eventId) + events.getTotalAmountBet(eventId)/admin.getEventMakerRewardDivider());
        }
