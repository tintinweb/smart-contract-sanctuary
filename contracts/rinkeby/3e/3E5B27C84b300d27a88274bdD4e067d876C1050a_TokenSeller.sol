...
contract TokenSeller{
		...
		// initializer
	{
        ...
        priceFeed1 = AggregatorV3Interface(0x8A753747A1Fa494EC906cE90E9f37563A8AF630e);
	}
	...
	function setProxyInfoForChainLink(address[] memory _proxies) external {
		...
		for (uint256 i = 0; i < proxies.length; i++) {
			priceFeed.push(AggregatorV3Interface(proxies[i]));
		}
	}
	function setPriceFromChainLink() public view returns (uint256[] memory) {
		require(tokenOwner == address(msg.sender), 'setPriceFromChainLink : not an owner');
    ...
    function getLatestPrice() public view returns (int) {
		...
		(,int tokenPrice,,,) = priceFeed1.latestRoundData();
        return tokenPrice;
    }
}
