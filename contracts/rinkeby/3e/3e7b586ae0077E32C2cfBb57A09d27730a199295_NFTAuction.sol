...
contract NFTAuction is Ownable, Context{
  ...
  function changeTrustedForwarder(address newForwarder) external  onlyOwner() {
        ...
        isAuctionNotStartedByOwner( _tokenId)
    {
        ...
        require(IERC1155(_nftContractAddress).balanceOf(msg.sender,_tokenId) != 0,  "Not Allowed : Sender is not owner");
    ...
    external {
     require(escrowOwner[msg.sender], "Not Allowed: Msg sender should be escorw Owner");
     IERC1155(_nftContractAddress).safeTransferFrom(
                address(this),
                _newOwner,
                _tokenId,
    ...
    function resetAuction( string memory _tokenId)external {
        nftContractAuctions[_nftContractAddress][_tokenId] = address(0);
    }
