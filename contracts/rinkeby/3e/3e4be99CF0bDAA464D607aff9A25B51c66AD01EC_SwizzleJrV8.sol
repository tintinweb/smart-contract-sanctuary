...
contract SwizzleJrV8 is NFTCBase {
    bytes32 private constant TOKEN_URI_PREFIX = "/ss";
    bytes32 private constant TOKEN_URI_POSTFIX = ".json";
        ...
        override
    {
        ...
        for (uint256 i = 1; i <= __numberOfFlavors; i++) {
            if (__lastBitEnabled) {
                // Some numerical magic here to make the numbers more compatible with forging.
                uint256 flavorIdx = ((i + 1) * 100) + 1;

                _assignFlavor(flavorIdx, defaultFlavorURI);
                _assignFlavor(flavorIdx + 1, defaultFlavorURI);
            } else {
                // No fuss numbering.
                _assignFlavor(i * 100, defaultFlavorURI);
            }
        }
    }
...
abstract contract NFTCBase is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function _assignFlavor(uint256 flavorIndex, string memory flavorUri)
        internal
    {
    ...
    function _setFlavorUri(
        ...
        _assignFlavor(flavorIndex, flavorUri);
    }
