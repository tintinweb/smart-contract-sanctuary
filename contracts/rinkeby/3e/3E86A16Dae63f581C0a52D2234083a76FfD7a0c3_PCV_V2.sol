...
contract PCV_V2 {
    ...
    function convertTokenToLP(address _token, address _lp) external onlyGov {
        // Get tokens from LP
        ...
        if (IERC20(_token).allowance(address(this), converter) < tokenAmount) {
            IERC20(_token).approve(converter, type(uint).max);
        }
        uint half = tokenAmount.div(2);
        if (_token == token0) {
            // use half of _token to buy token1
            IConverter(converter).swap(_token, token1, half);
        } else if (_token == token1) {
            // use half of _token to buy token0
            IConverter(converter).swap(_token, token0, half);
        } else {
    ...
    function massConvertTokenToLP(address[] calldata _tokens, address _lpDesired) external onlyGov {
        for (uint i=0; i<_tokens.length; i++) {
            convertLPToToken(_tokens[i], _lpDesired);
        }
    }
