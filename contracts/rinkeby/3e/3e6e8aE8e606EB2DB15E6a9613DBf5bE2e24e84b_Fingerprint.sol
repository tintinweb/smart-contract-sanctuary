...
contract Fingerprint is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, ERC721Burnable {
    ...
    function _preRender(Palette memory palette, uint16 r) internal pure returns(PreRendered memory) {
        return PreRendered(string(abi.encodePacked("\\\" x1=\\\"0\\\" stroke=\\\"%23", palette.fg, "\\\" stroke-linecap=\\\"round\\\" />")), string(abi.encodePacked("\\\" x2=\\\"1920\\\" stroke=\\\"%23", palette.fg, "\\\" stroke-linecap=\\\"round\\\" />")), string(abi.encodePacked("\\\" r=\\\"", _uintToString(r), "\\\" fill=\\\"%23", palette.accent, "\\\" stroke=\\\"none\\\" />")));
    }
    ...
    function _drawRow(uint256 chunk, uint8 i, PrecalculatedValues memory precalc, PreRendered memory pre) internal pure returns(Image memory) {
        ...
        if (rowValues.l1x2 > 0) {
            line1 = string(abi.encodePacked("<line stroke-width=\\\"", precalc.strokeWidthStr, "\\\" y1=\\\"",  rowValues.y1Str, "\\\" x2=\\\"", rowValues.l1x2Str, "\\\" y2=\\\"", rowValues.y2Str, pre.line1));
        }

        if (rowValues.l2x1 < _canvasWidth) {
            line2 = string(abi.encodePacked("<line stroke-width=\\\"", precalc.strokeWidthStr, "\\\" x1=\\\"", rowValues.l2x1Str, "\\\" y1=\\\"", rowValues.y1Str, "\\\" y2=\\\"", rowValues.y2Str, pre.line2));
        }
        ...
        if (precalc.isOutline) {
            if (rowValues.l1x2 > 0) {
                outline1 = string(abi.encodePacked("<line stroke-width=\\\"", precalc.outlineStrokeWidthStr, "\\\" y1=\\\"",  rowValues.y1Str, "\\\" x2=\\\"", rowValues.l1x2Str, "\\\" y2=\\\"", rowValues.y2Str, pre.line1));
            }

            if (rowValues.l2x1 < _canvasWidth) {
                outline2 = string(abi.encodePacked("<line stroke-width=\\\"", precalc.outlineStrokeWidthStr, "\\\" x1=\\\"", rowValues.l2x1Str, "\\\" y1=\\\"", rowValues.y1Str, "\\\" y2=\\\"", rowValues.y2Str, pre.line2));
            }
        }
