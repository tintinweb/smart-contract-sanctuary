...
contract Personax is ERC721Enumerable, Ownable {
    ...
    address private Authorized = 0x45D29A1087F810390b40611B673e94948da47d8c;
    

    uint256 public constant PersonaxPrice_whitelist = 20000000000000000;
	uint256 public constant PersonaxPrice_public = 80000000000000000;
    uint public constant maxPersonaxTx = 5;
	uint public constant maxPersonaxPurchase = 10;
	
    uint256 public constant MAX_Personax = 6666;
    uint public PersonaxReserve = 150;
	uint public PersonaxWhitelistReserve = 4000;
    ...
    constructor() ERC721("Personax", "PER") { }
    ...
    function reservePersonax(address _to, uint256 _reserveAmount) public onlyAuthorized {        
        uint supply = totalSupply();
        require(_reserveAmount > 0 && _reserveAmount <= PersonaxReserve, "Not enough reserve");
        for (uint i = 0; i < _reserveAmount; i++) {
            _safeMint(_to, supply + i);
        }
        PersonaxReserve = PersonaxReserve.sub(_reserveAmount);
    }
    ...
    function flipSaleState() public onlyAuthorized {
        saleIsActive = !saleIsActive;
    }
    
       
     function mintPersonax(uint numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint Personax");
        require(numberOfTokens > 0 && numberOfTokens <= maxPersonaxTx, "Can only mint 2 tokens at a time");
        require(msg.value == PersonaxPrice_public.mul(numberOfTokens), "Ether value sent is not correct");
        require(max_mints_per_address[msg.sender].add(numberOfTokens) <= maxPersonaxPurchase,"Max 10 mints per wallet allowed");
        
        for(uint i = 0; i < numberOfTokens; i++) {
            if ((totalSupply() < MAX_Personax) && (totalSupply() < PersonaxWhitelistReserve)) {
                _safeMint(msg.sender, totalSupply());
                max_mints_per_address[msg.sender] = max_mints_per_address[msg.sender].add(1);
            } else {
               saleIsActive = !saleIsActive;
                payable(msg.sender).transfer(numberOfTokens.sub(i).mul(PersonaxPrice_public));
                break;
            }
        }
    }
   ...
   // to set the merkle root
    ...
    function whitelistedMints(uint numberOfTokens, bytes32[] calldata merkleProof ) payable external  {
        ...
        require(msg.value == PersonaxPrice_whitelist.mul(numberOfTokens), "Ether value sent is not correct");
