...
interface iMinoanKings {
    function balanceGenesis(address owner) external view returns (uint256);
}

contract Ovolos is ERC20, Ownable {
    iMinoanKings public MinoanKings;
    ...
    constructor(address kaijuAddress) ERC20("Ovolos", "Ovolos") {
        MinoanKings = iMinoanKings(kaijuAddress);
        START = block.timestamp;
    }

    function updateReward(address from, address to) external {
        require(msg.sender == address(MinoanKings));
        if (from != address(0)) {
    ...
    function burn(address user, uint256 amount) external {
        require(
            allowedAddresses[msg.sender] || msg.sender == address(MinoanKings),
    ...
    function getPendingReward(address user) internal view returns (uint256) {
        return
            (MinoanKings.balanceGenesis(user) *
                BASE_RATE *
                (block.timestamp -
    ...
    function togglePauseReward() public onlyOwner {
