...
contract PaymentGateway is Operable {
    using SafeMath for uint256;

    event PaymentReceived(address indexed payer, uint256 indexed weiAmount, uint256 indexed id);
     ...
     */
    constructor(address payable _wallet) Operable(msg.sender) {
        escrow = new Escrow();
        wallet = _wallet;
    }
     ...
     */
    function sendPayment(uint256 id) external payable {
        escrow.deposit{value: msg.value}(wallet);

        emit PaymentReceived(msg.sender, msg.value, id);
    }
     ...
     * @return the balance
     */
    function balance() external view onlyOperator returns (uint256) {
