...
contract DivineObjects is  ERC1155Supply, Ownable {
    ...
    // * STORAGE * //
    mapping(address => mapping(uint256 => bool)) public canClaim;
    ...
    constructor() ERC1155("") {
        // placeholder IPFS hash for KEY metadata
        _maxSupply[KEY] = 88;
        _tokenMetadata[KEY] = "QmavQkogutNeynaLkAFgAMV3kQijM34s613jXu1jxpRyG5";
    }

    function mint(uint256 tokenId) public tokenExists(tokenId) {
        require(totalSupply(tokenId) < _maxSupply[tokenId], "Max supply already claimed.");
        require(canClaim[msg.sender][tokenId] == true, "Not allowed to claim.");
        _mint(msg.sender, tokenId, 1, "");
        canClaim[msg.sender][tokenId] = false;
    }
    ...
    function addClaimer(address wallet, uint256 tokenId) public tokenExists(tokenId) onlyOwner {
        require(balanceOf(wallet, tokenId) == 0, "Already claimed.");
        require(canClaim[wallet][tokenId] == false, "Already allowed.");
        canClaim[wallet][tokenId] = true;
    }
