...
contract ForgottenQuests {
    ...
    uint256 public BASE_EXPIRATION = 604800; // 1 week
    uint256 public BASE_DURATION = 600; //1209600; // 2 weeks
    uint256 public TIME_ADJUSTMENT = 30; //172800; // 2 days
    uint256 public COOLDOWN = 300; //21600; //6 hours
    uint256 public BASE_FEE = 10**16; //0.01 ETH
    ...
    struct Quest {
        ...
        uint256 accepted_at;
        uint256 wizardId;
        uint16 positive_affinity;
        uint16 negative_affinity;
        string lore;
        uint256 ends_at;
        uint256 expires_at;
        bool isCustom;
    ...
    function newQuest() public {
        ...
        Quest memory quest = Quest({
            ...
            wizardId: 10000,
            positive_affinity: _getAffinity(questLog.length),
            negative_affinity: _getAffinity(questLog.length.add(1)),
            lore: "",
            ends_at: 0,
            expires_at: block.timestamp + BASE_EXPIRATION,
            isCustom: false,
            rewardNFT: rewardNFT,
            rewardId: 0
        });
        questLog.push(quest);
        nextQuestAvailableAt = block.timestamp.add(COOLDOWN);
    }
    ...
    function newCustomQuest(
        ...
        require(
            nextQuestAvailableAt.add(COOLDOWN) < block.timestamp,
            "Quest Cooldown not elapsed"
        );
        Quest memory quest = Quest({
            id: questLog.length,
        ...
        questLog.push(quest);
        nextQuestAvailableAt = block.timestamp;
        ERC721(_rewardNFT).transferFrom(_rewardOwner, address(this), _rewardId);
    }
    ...
    function acceptQuest(
        ...
        require(!quest.isCustom, "Can not accept Custom quest with lore");
        _acceptQuest(wizard, quest, lore);
    }

    function acceptCustomQuest(uint256 id, uint256 wizard) public {
        Quest storage quest = questLog[id];
        require(quest.isCustom, "Can only accept custom quest");
        _acceptQuest(wizard, quest, quest.lore);
    }
    ...
    function completeQuest(uint256 id) public {
        ...
        if (!quest.isCustom) {
            RewardNFT(quest.rewardNFT).mint(
                msg.sender,
                quest.id, //rewardNFT.totalSupply(),
                quest.wizardId,
                quest.lore,
                quest.positive_affinity,
                duration
            );
        } else {
    ...
    function getQuestDuration(uint256 id, uint256 wizardId)
        public
        view
        returns (uint256)
    {
        ...
        for (uint8 i = 0; i < wizAffinities.length; i++) {
            if (wizAffinities[i] == quest.positive_affinity) {
        ...
        for (uint8 i = 0; i < wizAffinities.length; i++) {
            if (wizAffinities[i] == quest.negative_affinity) {
        ...
        uint256 duration = BASE_DURATION -
    ...
    function _acceptQuest(
        ...
        require(wizards.ownerOf(wizardId) == msg.sender, "Not owner of wizard");
        wizards.transferFrom(msg.sender, address(this), wizardId);
        ...
        uint256 duration = getQuestDuration(quest.id, wizardId);
        quest.ends_at = block.timestamp.add(duration);
    }

    function _getAffinity(uint256 _nonce) internal view returns (uint16) {
...
contract WizardStorage {
    ...
    function stopStoring() public {
        require(canStore, "Store is already over");
        canStore = false;
    }
    ...
    function storeWizardTraits(
        ...
        require(canStore, "Storing is over");
