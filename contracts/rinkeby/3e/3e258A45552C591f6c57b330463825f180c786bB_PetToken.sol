...
contract PetToken is RawToken {
    ...
    constructor() RawToken("PET-token", "PET", "https://google.com"){
    ...
    function setPurchaseRules(
        ...
        require(hasRole(BACKEND_ROLE, _msgSender()), "PetToken: must have backend role to change price");
    ...
    function incrementAmountToSell(uint256 totalTokens, bool isResource) public override {
    ...
    function decrementAmountToSell(uint256 totalTokens, bool isResource) public override {
        require(hasRole(BACKEND_ROLE, _msgSender()), "PetToken: must have backend role to change total tokens");
        if (isResource) {
            require(tokensToSaleR - totalTokens >= 0, 'PetToken: tokens to sale is 0');
            tokensToSaleR -= totalTokens;
        } else {
            require(tokensToSaleF - totalTokens >= 0, 'PetToken: tokens to sale is 0');
            tokensToSaleF -= totalTokens;
        }
    }
    ...
    function futureToResource(uint256 tokenId) public override {
        require(hasRole(BACKEND_ROLE, _msgSender()), "PetToken: must have backend role to change token type");
        require(!tokenInfo[tokenId].is_resource_token, 'PetToken: Token already is resource');
        require(block.timestamp - tokenInfo[tokenId].purchase_date
            <= tokenInfo[tokenId].benefits_period, 'PetToken: token expired');
        require(tokenInfo[tokenId].purchase_date / TROPICAL_YEAR == block.timestamp / TROPICAL_YEAR,
            'PetToken: forbidden - FToken was minted at past year');
...
ERC721Burnable {
    ...
    function incrementAmountToSell(uint256 totalTokens, bool isResource) public virtual;

    function decrementAmountToSell(uint256 totalTokens, bool isResource) public virtual;
