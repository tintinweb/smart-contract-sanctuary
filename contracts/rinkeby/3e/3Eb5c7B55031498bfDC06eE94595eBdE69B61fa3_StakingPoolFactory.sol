...
contract StakingPoolFactory is PoolCreator {
    ...
    constructor(DARTToken _dARTToken, RewardManager _rewardManager) {
...
contract StakingPool is Context, Ownable, ReentrancyGuard {
    ...
    */
        ...
        returns (bool) 
    {
        ...
        if (remaining > (poolSize.sub(stakedTotal))) {
            remaining = poolSize.sub(stakedTotal);
        }
        ...
        stakes[staker] = stakes[staker].add(remaining);
        ...
        _realAddress(_msgSender())
    {
        ...
        require(stakes[staker] > 0, "Zero staked dART");

        uint256 amount = stakes[staker];
        stakes[staker] = 0;
        _payDirect(staker, amount);

        if (block.timestamp > stakeTimes[staker].add(maturityDays.mul(60))) {
            uint256 reward = stakes[staker].mul(poolApy).mul(maturityDays).div(36000);
        ...
        returns (bool) 
    {
        ...
        return dARTToken.transferFrom(allower, receiver, amount);
    }
    ...
    function _payDirect(address to, uint256 amount)
        ...
        returns (bool) 
    {
        return dARTToken.transfer(to, amount);
    }
    ...
    modifier _hasAllowance(address allower, uint256 amount) {
        ...
        uint256 ourAllowance = dARTToken.allowance(allower, address(this));
